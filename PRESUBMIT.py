...
def _CheckCrbugLinkified(input_api, output_api):
  """Checks that crbug links are correctly prefixed by http[s]://"""
  pattern = input_api.re.compile(r'//.*(?<!:\/\/)crbug[.com]*')
  problems = []
  for f in input_api.AffectedFiles():
    if not f.LocalPath().endswith(('h', 'cc', 'mm', 'cpp')):
      continue;
    for line_num, line in f.ChangedContents():
      if pattern.search(line):
...
def CheckChangeOnUpload(input_api, output_api):
  ...
  results.extend(_CheckCrbugLinkified(input_api, output_api))
  return results
