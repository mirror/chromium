...
def _CheckEntriesInWATCHLISTSAreSorted(contents, input_api, output_api):
  ...
  if sorted_watchlists != watchlists:
    return [output_api.PresubmitError('WATCHLISTS entries are not sorted')]

  if watchlist_definitions != watchlists:
    return [output_api.PresubmitError(
        'WATCHLIST_DEFINITIONS and WATCHLISTS don\'t match')]
  ...
  for f in input_api.AffectedFiles(include_deletes=False):
    if f.LocalPath() == 'WATCHLISTS':
      ...
      try:
        ...
        return [output_api.PresubmitError('Cannot parse WATCHLISTS')]
