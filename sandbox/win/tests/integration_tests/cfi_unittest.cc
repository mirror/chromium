...
TEST(CFGSupportTests, BinaryCheck) {
  ...
  ASSERT_EQ(DWORD{IMAGE_NT_SIGNATURE}, nt_headers->Signature);

  ASSERT_EQ(WORD{IMAGE_NT_OPTIONAL_HDR_MAGIC},
  ...
  ASSERT_TRUE((load_config->VirtualAddress != 0) &&
              (load_config->VirtualAddress < optional_header->SizeOfImage));
