...
scoped_refptr<TargetPolicy> BrokerServicesBase::CreatePolicy(
  ...
  scoped_refptr<PolicyBase> policy(new PolicyBase);
  if (profile && base::win::GetVersion() >= base::win::VERSION_WIN8)
    policy->SetAppContainerProfile(profile);
