...
SBOX_TESTS_COMMAND int TestImageLoadHijack(int argc, wchar_t** argv) {
  ...
  // loaded in process.
  HMODULE check = ::GetModuleHandleW(hijack_dlls::g_hijack_dll_file);
  if (check != NULL)
  ...
  hijack_dlls::CheckHijackResultFunction check_hijack_result =
      reinterpret_cast<hijack_dlls::CheckHijackResultFunction>(
          shim_dll.GetFunctionPointer(hijack_dlls::g_hijack_shim_func));
