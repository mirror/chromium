...
TEST(SidTest, AppContainer) {
  ...
  ASSERT_FALSE(sid_nullptr.valid());

  Sid sid_empty = Sid::FromNamedCapability(L"");
  ASSERT_FALSE(sid_empty.valid());
  ...
  ASSERT_FALSE(sid_invalid_well_known.valid());

  for (size_t index = 0; index < _countof(capabilities); ++index) {
    ASSERT_TRUE(
        EqualSid(Sid::FromNamedCapability(capabilities[index].capability_name_),
                 capabilities[index].sddl_sid_));
    ASSERT_TRUE(
        EqualSid(Sid::FromKnownCapability(capabilities[index].capability_),
                 capabilities[index].sddl_sid_));
  }
}
