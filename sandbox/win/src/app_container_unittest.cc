...
struct SECURITY_ATTRIBUTES_SDDL : public SECURITY_ATTRIBUTES {
...
TEST(AppContainerTest, SecurityCapabilities) {
  ...
  vector<Sid> capabilities;
  SecurityCapabilities no_capabilities(package_sid);
  EXPECT_TRUE(ValidSecurityCapabilities(no_capabilities.get(), package_sid,
                                        capabilities));

  capabilities.push_back(Sid(::WinWorldSid));
  SecurityCapabilities one_capability(package_sid, capabilities);
  EXPECT_TRUE(ValidSecurityCapabilities(one_capability.get(), package_sid,
                                        capabilities));

  capabilities.push_back(Sid(::WinLocalSid));
  SecurityCapabilities two_capabilities(package_sid, capabilities);
  EXPECT_TRUE(ValidSecurityCapabilities(two_capabilities.get(), package_sid,
                                        capabilities));
}
...
TEST(AppContainerTest, CreateAndOpenAppContainerProfile) {
  ...
  ASSERT_NE(nullptr, profile.get());
  EXPECT_TRUE(::EqualSid(profile->package_sid(), open_profile->package_sid()));
  Sid package_sid(open_profile->package_sid());
  EXPECT_TRUE(AppContainerProfile::Delete(package_name.c_str()));
  EXPECT_FALSE(ProfileExist(package_name));
...
TEST(AppContainerTest, SetLowPrivilegeAppContainer) {
  ...
  scoped_refptr<AppContainerProfile> profile =
      AppContainerProfile::Open(package_name.c_str());
  ASSERT_NE(nullptr, profile.get());
  profile->set_enable_low_privilege_app_container(true);
  auto profile_base = static_cast<AppContainerProfileBase*>(profile.get());
  EXPECT_TRUE(profile_base->enable_low_privilege_app_container());
}
...
TEST(AppContainerTest, OpenAppContainerProfileAndGetSecurityCapabilities) {
  ...
  SecurityCapabilities no_capabilities =
      profile_base->get_security_capabilities();
  ASSERT_TRUE(ValidSecurityCapabilities(
      no_capabilities.get(), Sid(profile->package_sid()), capabilities));
  ...
  SecurityCapabilities with_capabilities =
      profile_base->get_security_capabilities();
  ASSERT_TRUE(ValidSecurityCapabilities(
      with_capabilities.get(), Sid(profile->package_sid()), capabilities));
}
...
TEST(AppContainerTest, AccessCheckFile) {
  ...
  AccessCheckFile(profile.get(), path, Sid(profile->package_sid()),
                  FILE_READ_DATA, FILE_READ_DATA, TRUE);
  AccessCheckFile(profile.get(), path,
                  Sid::FromKnownCapability(kInternetClient), FILE_READ_DATA,
                  FILE_READ_DATA, TRUE);
  AccessCheckFile(profile.get(), path, Sid::FromKnownCapability(kRegistryRead),
...
TEST(AppContainerTest, AccessCheckRegistry) {
  ...
  ASSERT_EQ(KEY_QUERY_VALUE, granted_access);
  RegDeleteKey(HKEY_CURRENT_USER, package_name.c_str());
}
