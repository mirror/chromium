...
bool IsDeviceHarddiskPath(const base::string16& path,
  ...
  const wchar_t pattern[] = L"\\Device\\HarddiskVolume";
  ...
  size_t match = path.find_first_of(L'\\', patternLen - 1);
  if (match == base::string16::npos) {
    *trimmed_path = path;
    return false;
  }

  // Remove up to and including the path separator.
  *removed = path.substr(0, match + 1);
  // Remaining path starts after the path separator.
  *trimmed_path = path.substr(match + 1);
  return true;
}
...
bool GetDriveLetter(const base::string16& win32_path,
  ...
  if (win32_path.size() >= kWIN32PrefixLen &&
    ...
    temp = win32_path.substr(kNTDotPrefixLen);
             ...
             0 == win32_path.compare(0, sandbox::kNTPrefixLen,
                                     sandbox::kNTPrefix)) {
