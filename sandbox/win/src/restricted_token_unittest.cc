...
TEST(RestrictedTokenTest, LowBoxToken) {
  ...
  ASSERT_EQ(ERROR_INVALID_PARAMETER,
            CreateLowBoxToken(nullptr, PRIMARY, nullptr, nullptr, 0, &token));

  Sid package_sid = Sid::FromSddlString(L"S-1-15-2-1-2-3-4-5-6-7");
  SecurityCapabilities caps_no_capabilities(package_sid);
  ASSERT_EQ(ERROR_SUCCESS,
            CreateLowBoxToken(nullptr, PRIMARY, caps_no_capabilities.get(),
                              nullptr, 0, &token));
  ASSERT_TRUE(token.IsValid());
  CheckLowBoxToken(token, ::TokenPrimary, caps_no_capabilities.get());

  ASSERT_EQ(ERROR_SUCCESS,
            CreateLowBoxToken(nullptr, IMPERSONATION,
                              caps_no_capabilities.get(), nullptr, 0, &token));
  ASSERT_TRUE(token.IsValid());
  CheckLowBoxToken(token, ::TokenImpersonation, caps_no_capabilities.get());
  ...
  ASSERT_EQ(ERROR_SUCCESS,
            CreateLowBoxToken(nullptr, PRIMARY, caps_with_capabilities.get(),
                              nullptr, 0, &token));
  ASSERT_TRUE(token.IsValid());
  CheckLowBoxToken(token, ::TokenPrimary, caps_with_capabilities.get());

  RestrictedToken restricted_token;
  base::win::ScopedHandle token_handle;
  ASSERT_EQ(ERROR_SUCCESS, restricted_token.Init(nullptr));
  ASSERT_EQ(ERROR_SUCCESS,
            restricted_token.AddRestrictingSid(ATL::Sids::World().GetPSID()));
  ASSERT_EQ(ERROR_SUCCESS, restricted_token.GetRestrictedToken(&token_handle));

  ASSERT_EQ(ERROR_SUCCESS, CreateLowBoxToken(token_handle.Get(), PRIMARY,
                                             caps_with_capabilities.get(),
                                             nullptr, 0, &token));
  ASSERT_TRUE(token.IsValid());
  CheckLowBoxToken(token, ::TokenPrimary, caps_with_capabilities.get());
  ...
  ASSERT_EQ(ERROR_SUCCESS,
            CreateLowBoxObjectDirectory(caps_for_handles.get()->AppContainerSid,
                                        true, &object_handle));
  HANDLE saved_handles[] = {object_handle.Get()};

  ASSERT_EQ(ERROR_SUCCESS, CreateLowBoxToken(token_handle.Get(), PRIMARY,
                                             caps_for_handles.get(),
                                             saved_handles, 1, &token));
  ASSERT_TRUE(token.IsValid());
  object_handle.Close();
  ASSERT_FALSE(object_handle.IsValid());
  ASSERT_EQ(ERROR_ALREADY_EXISTS,
            CreateLowBoxObjectDirectory(caps_for_handles.get()->AppContainerSid,
                                        false, &object_handle));
}
