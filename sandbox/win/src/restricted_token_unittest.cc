...
TEST(RestrictedTokenTest, LowBoxToken) {
  ...
  ASSERT_EQ(static_cast<DWORD>(ERROR_INVALID_PARAMETER),
  ...
  base::win::ScopedHandle token_handle;
  ASSERT_EQ(static_cast<DWORD>(ERROR_SUCCESS), restricted_token.Init(nullptr));
  ASSERT_EQ(static_cast<DWORD>(ERROR_SUCCESS),
  ...
  ASSERT_EQ(static_cast<DWORD>(ERROR_ALREADY_EXISTS),
