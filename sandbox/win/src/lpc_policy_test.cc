...
std::wstring GetLocaleInfoString(LCID lcid, LCTYPE type) {
  int buffer_size_with_nul = ::GetLocaleInfo(lcid, type, 0, 0);
  if (buffer_size_with_nul <= 0)
    return std::wstring();

  wchar_t buffer[LOCALE_NAME_MAX_LENGTH] = {0};
  ::GetLocaleInfo(lcid, type, buffer, buffer_size_with_nul);
  return std::wstring(buffer, buffer_size_with_nul - 1);
}
...
BOOL SetLocaleHelper() {
  ...
  GetLocaleInfoString(0xc00, LOCALE_SSHORTDATE);
  BOOL updated =
...
DWORD WINAPI ChangeLocaleFunc(LPVOID lpdwThreadParam) {
  ...
  // step 2: wait for renderer_up_event
  ...
  // step 4: open broker change locale event
...
TEST(LpcPolicyTest, TestGetLocaleInfoOutsideOfRenderer) {
  ...
  GetLocaleInfoString(0xc000, LOCALE_SSHORTDATE);
...
SBOX_TESTS_COMMAND int Lpc_TestChangeLocale(int argc, wchar_t** argv) {
  ...
  NlsGetCacheUpdateCount();
  // NOTE: We expect this to break;
  return Lpc_GetLocaleInfo(argc - 2, argv + 2);
}
