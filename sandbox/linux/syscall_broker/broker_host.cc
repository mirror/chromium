...
void OpenFileForIPC(uint32_t allowed_command_mask,
  ...
  if (!CommandOpenIsSafe(allowed_command_mask, policy,
...
void AccessFileForIPC(uint32_t allowed_command_mask,
                      const BrokerPolicy& policy,
                      const std::string& requested_filename,
                      int mode,
                      base::Pickle* write_pickle) {
  const char* file_to_access = NULL;
  if (!CommandAccessIsSafe(allowed_command_mask, policy,
...
// |write_pickle|.
void StatFileForIPC(uint32_t allowed_command_mask,
                    const BrokerPolicy& policy,
                    BrokerCommand command_type,
                    const std::string& requested_filename,
                    base::Pickle* write_pickle) {
  const char* file_to_access = nullptr;
  if (!CommandStatIsSafe(allowed_command_mask, policy,
...
void RenameFileForIPC(uint32_t allowed_command_mask,
                      const BrokerPolicy& policy,
  ...
  if (!CommandRenameIsSafe(allowed_command_mask, policy, old_filename.c_str(),
...
void ReadlinkFileForIPC(uint32_t allowed_command_mask,
                        const BrokerPolicy& policy,
                        const std::string& filename,
                        base::Pickle* write_pickle) {
  const char* file_to_access = nullptr;
  if (!CommandReadlinkIsSafe(allowed_command_mask, policy, filename.c_str(),
                             &file_to_access)) {
    write_pickle->WriteInt(-policy.denied_errno());
    return;
  }
...
bool HandleRemoteCommand(uint32_t allowed_command_mask,
                         const BrokerPolicy& policy,
  ...
  switch (command_type) {
    case COMMAND_ACCESS: {
      ...
      if (!iter.ReadString(&requested_filename) || !iter.ReadInt(&flags))
        return false;
      AccessFileForIPC(allowed_command_mask, policy, requested_filename, flags,
                       write_pickle);
      break;
    }
    case COMMAND_OPEN: {
      ...
      if (!iter.ReadString(&requested_filename) || !iter.ReadInt(&flags))
        return false;
      OpenFileForIPC(allowed_command_mask, policy, requested_filename, flags,
    ...
    case COMMAND_STAT64: {
      ...
      if (!iter.ReadString(&requested_filename))
        return false;
      StatFileForIPC(allowed_command_mask, policy,
    ...
    case COMMAND_RENAME: {
      ...
      if (!iter.ReadString(&old_filename) || !iter.ReadString(&new_filename))
        return false;
      RenameFileForIPC(allowed_command_mask, policy, old_filename, new_filename,
                       write_pickle);
      break;
    }
    case COMMAND_READLINK: {
      ...
      if (!iter.ReadString(&filename))
        return false;
      ReadlinkFileForIPC(allowed_command_mask, policy, filename, write_pickle);
      break;
    }
    default:
...
BrokerHost::RequestStatus BrokerHost::HandleRequest() const {
  ...
  base::Pickle write_pickle;
  std::vector<int> opened_files;
  bool result = HandleRemoteCommand(allowed_command_mask_, broker_policy_, iter,
                                    &write_pickle, &opened_files);
