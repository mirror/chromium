...
TEST(BrokerProcess, TestOpenAccessNull) {
  ...
  command_set.set(COMMAND_ACCESS);
  command_set.set(COMMAND_OPEN);
...
TEST(BrokerProcess, CreateFile) {
  ...
  // so as to prevent spying on any pre-existing files.
  fd = open_broker.Open(tempfile_name, O_RDWR | O_CREAT);
  ASSERT_EQ(fd, -kFakeErrnoSentinel);

  // Opening a temp file in a normal way must not be allowed, either.
  fd = open_broker.Open(tempfile_name, O_RDWR);
  ASSERT_EQ(fd, -kFakeErrnoSentinel);

  // Opening a temp file with both O_CREAT and O_EXCL is allowed.
  fd = open_broker.Open(tempfile_name, O_RDWR | O_CREAT | O_EXCL);
  ASSERT_GE(fd, 0);
  close(fd);

  // Manually create a conflict for the temp filename.
  fd = open(tempfile_name, O_RDWR | O_CREAT);
  ASSERT_GE(fd, 0);
  close(fd);

  // Opening a temp file with both O_CREAT and O_EXCL is allowed but fails
  // pre the OS when there is a conflict.
  fd = open_broker.Open(tempfile_name, O_RDWR | O_CREAT | O_EXCL);
  ASSERT_EQ(fd, -EEXIST);

  // Opening a permanent file without specifying O_EXCL is allowed.
  fd = open_broker.Open(permfile_name, O_RDWR | O_CREAT);
  ASSERT_GE(fd, 0);
  close(fd);

  // Re-opening a permanent file without specifying O_EXCL is allowed.
  fd = open_broker.Open(permfile_name, O_RDWR | O_CREAT);
  ASSERT_GE(fd, 0);
  close(fd);

  // Re-opening an existing file with O_EXCL is allowed but fails per the OS.
...
void TestRenameHelper(bool fast_check_in_client) {
  ...
  BrokerCommandSet command_set;
  command_set.set(COMMAND_RENAME);
