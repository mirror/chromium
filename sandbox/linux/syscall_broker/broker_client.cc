...
int BrokerClient::Rename(const char* oldpath, const char* newpath) {
  if (fast_check_in_client_) {
    bool ignore;
    if (!broker_policy_.GetFileNameIfAllowedToOpen(oldpath, W_OK, nullptr,
                                                   &ignore) ||
        !broker_policy_.GetFileNameIfAllowedToOpen(newpath, W_OK, nullptr,
                                                   &ignore)) {
      return -broker_policy_.denied_errno();
    }
  }
...
int BrokerClient::Readlink(const char* path, char* buf, size_t bufsiz) {
  if (fast_check_in_client_) {
    bool ignore;
    if (!broker_policy_.GetFileNameIfAllowedToOpen(path, R_OK, nullptr,
                                                   &ignore)) {
      return -broker_policy_.denied_errno();
    }
  }
  RAW_CHECK(bufsiz < kMaxMessageLength - 16);

  base::Pickle write_pickle;
  write_pickle.WriteInt(COMMAND_READLINK);
  write_pickle.WriteString(path);
  write_pickle.WriteUInt32(static_cast<uint32_t>(bufsiz));
  RAW_CHECK(write_pickle.size() <= kMaxMessageLength);
  ...
  if (!iter.ReadData(&return_data, &return_length))
    return -ENOMEM;
  if (return_length < 0 || static_cast<size_t>(return_length) + 1 > bufsiz)
    return -ENOMEM;
