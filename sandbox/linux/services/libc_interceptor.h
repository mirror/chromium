...
// the current module. Use SetLocaltimeOverride() and SetAmZygoteOrRenderer()
...
// Gets the well-known file descriptor number onto which the IPC back-channel
// to the parent process will be established. At various points in the process
// lifecycle, there may not be an actual open FD here.  This is exposed so other
// layers can put additional methods onto this channel.
SANDBOX_EXPORT int GetBackChannelFDNumber();
...
// Turns on the libc interception. Called by the zygote and inherited by it
// children.
