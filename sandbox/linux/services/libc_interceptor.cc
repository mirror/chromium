...
// The global |g_am_zygote_or_descendent| is true iff we are in a zygote or
...
bool g_am_zygote_or_descendent = false;
...
__attribute__((__visibility__("default"))) struct tm* localtime_override(
    const time_t* timep) {
  if (g_am_zygote_or_descendent && g_use_localtime_override) {
...
void SetAmZygoteOrDescendant(bool enable, int backchannel_fd) {
  g_am_zygote_or_descendent = enable;
