...
SeatbeltExecClient::SeatbeltExecClient() {
  if (pipe(pipe_) != 0)
    SandboxLogging::log(LogLevel::FATAL, "pipe: %s", strerror(errno));
}
...
bool SeatbeltExecClient::WriteString(std::string* str) {
  ...
  if (written < 0) {
    SandboxLogging::log(LogLevel::ERR, "writev: %s", strerror(errno));
    return false;
  }
...
bool SeatbeltExecServer::InitializeSandbox() {
  ...
  if (!policy.ParseFromString(policy_string)) {
    SandboxLogging::log(LogLevel::ERR, "ParseFromString failed");
    return false;
  }
...
bool SeatbeltExecServer::ApplySandboxProfile(const mac::SandboxPolicy& policy) {
  ...
  if (error) {
    SandboxLogging::log(LogLevel::ERR, "Failed to initialize sandbox: %d %s",
                        rv, error);
...
bool SeatbeltExecServer::ReadString(std::string* str) {
  ...
  if (read_length < 0) {
    SandboxLogging::log(LogLevel::ERR, "readv: %s", strerror(errno));
    return false;
  }
