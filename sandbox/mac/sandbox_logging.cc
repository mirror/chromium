...
void DoLogging(Level level, const char* fmt, va_list args) {
  char message[4096];
...
void Fatal(const char* fmt, ...) {
  va_list args;
  va_start(args, fmt);
  DoLogging(Level::FATAL, fmt, args);
  va_end(args);
...
void PError(const char* fmt, ...) {
  std::string final_fmt = std::string(fmt) + ": " + strerror(errno);
  va_list args;
  va_start(args, fmt);
  DoLogging(Level::ERR, final_fmt.c_str(), args);
  va_end(args);
}
...
void PFatal(const char* fmt, ...) {
  ...
  va_list args;
  va_start(args, fmt);
  DoLogging(Level::FATAL, final_fmt.c_str(), args);
  va_end(args);
