...
Buffer WriteUpdatedTargetInfo(const std::vector<AvPair>& av_pairs,
  ...
  for (const AvPair& pair : av_pairs) {
    ...
    if (pair.avid == TargetInfoAvId::kFlags) {
      // Flags are treated specially because during the update it may have
      // had the kMicPresent flag added, or the AVPair was added explicitly
      // in which case the |buffer| field will not be populated.
...
void GenerateResponseDesl(const uint8_t* hash,
  ...
  DCHECK_EQ(kChallengeLen, block_size);
  DCHECK_EQ(kResponseLenV1, block_count * block_size);
...
void GenerateNtlmHashV2(const base::string16& domain,
  ...
  uint32_t outlen = kNtlmHashLen;
  v2_hash =
      HMAC(EVP_md5(), v1_hash, kNtlmHashLen, input_writer.GetBuffer().data(),
           input_writer.GetLength(), v2_hash, &outlen);
  DCHECK_NE(nullptr, v2_hash);
  DCHECK_EQ(kNtlmHashLen, outlen);
}
...
void GenerateSessionBaseKeyV2(const uint8_t* v2_hash,
                              ...
                              uint8_t* session_key) {
  uint32_t outlen = kSessionKeyLenV2;
