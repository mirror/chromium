...
bool NtlmBufferReader::ReadTargetInfo(size_t target_info_len,
  ...
  // TODO: Should this fail if there is no terminator?
  ...
  while ((GetCursor() < target_info_end) &&
    ...
    // pair.ptr = (pair.avlen == 0) ? nullptr : GetBufferAtCursor();
    if (pair.avid == ntlm::TargetInfoAvId::FLAGS) {
    ...
    } else if (pair.avid == ntlm::TargetInfoAvId::TIMESTAMP) {
  ...
  // The buffer wasn't properly formed.
  if ((GetCursor() != target_info_end) || (av_pairs->size() == 0))
    return false;

  // TODO: Additional check for EOL termination
  return true;
}
