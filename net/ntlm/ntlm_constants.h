...
// Flags to control specific features.
enum class NtlmFeatures {
  NONE = 0,
  DISABLE_MIC = 0x01,
  DISABLE_EPA = 0x02,
};

using TFeaturesInt = std::underlying_type<NtlmFeatures>::type;

constexpr inline NtlmFeatures operator|(NtlmFeatures lhs, NtlmFeatures rhs) {
  return static_cast<NtlmFeatures>(static_cast<TFeaturesInt>(lhs) |
                                   static_cast<TFeaturesInt>(rhs));
}
...
// Flags used in an TargetInfoAvId::FLAGS AV Pair. See [MS-NLMP] 2.2.2.1
...
// equal to |EOL| and |avlen| equal to zero signals that no further pairs
...
// are populated during the initial parsing phase for AVIDs |FLAGS| and
// |TIMESTAMP| respectively. This avoids subsequent code having to
...
static constexpr uint16_t PROOF_INPUT_VERSION = 0x0101;
...
static constexpr NtlmFeatures kDefaultFeatures = NtlmFeatures::NONE;
