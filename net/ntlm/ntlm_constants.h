...
struct AvPair {
  AvPair() {}
  explicit AvPair(TargetInfoAvId avid, uint16_t avlen)
      : avid(avid), avlen(avlen) {}
  AvPair(TargetInfoAvId avid, Buffer buffer)
      : buffer(buffer), avid(avid), avlen(buffer.size()) {}
...
static constexpr NtlmFeatures kDefaultFeatures = NtlmFeatures::kNone;
