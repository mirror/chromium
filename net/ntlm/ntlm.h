...
// Generates the NTLMv2 Hash and writes the 16 byte result to |v2_hash|.
...
// [MS-NLMP] calls "temp". "temp" is part of the input to generate the
// NTLMv2 proof. "temp" is composed of a fixed 28 byte prefix (the Proof
// Input), then the variable length updated target info that is sent in the
// authenticate message, then followed by 4 zero bytes. See [MS-NLMP] Section
// 2.2.2.7.
//
...
NET_EXPORT_PRIVATE void GenerateProofInputV2(
    uint64_t timestamp,
    const uint8_t* client_challenge,
...
// a hash of the servers certificate.
//
// The |channel_bindings| string is supplied out of band (usually from a web
// broswer) and is a 53 byte ASCII string with the following format defined by
// RFC 5929 Section 4;
//
...
// messages in the NTLM protocol. See [MS-NLMP] Section 3.1.5.1.2.
//
// It is defined as;
//     mic = HMAC_MD5(
//         session_key,
