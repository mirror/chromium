...
AvPair MakeDomainAvPair() {
  return AvPair(test::kNtlmDomainRaw, TargetInfoAvId::DOMAIN_NAME,
                arraysize(test::kNtlmDomainRaw), UINT64_MAX,
                TargetInfoAvFlags::NONE);
}

AvPair MakeServerAvPair() {
  return AvPair(test::kServerRaw, TargetInfoAvId::SERVER_NAME,
                arraysize(test::kServerRaw), UINT64_MAX,
                TargetInfoAvFlags::NONE);
}

void CompareBuf(const uint8_t* lhs, const uint8_t* rhs, size_t len) {
  for (size_t i = 0; i < len; i++) {
    std::string ls(4, '\0');
    ls[0] = '0';
    ls[1] = 'x';
    uint8_t high = lhs[i] >> 4;
    uint8_t low = lhs[i] & 0xf;
    ls[2] = (high < 10) ? '0' + high : 'A' + high - 10;
    ls[3] = (low < 10) ? '0' + low : 'A' + low - 10;
    if (lhs[i] != rhs[i]) {
      std::string rs(4, '\0');
      rs[0] = '0';
      rs[1] = 'x';
      high = rhs[i] >> 4;
      low = rhs[i] & 0xf;
      rs[2] = (high < 10) ? '0' + high : 'A' + high - 10;
      rs[3] = (low < 10) ? '0' + low : 'A' + low - 10;

      LOG(ERROR) << "Diff @" << i << " " << ls << " != " << rs;
    } else {
      LOG(ERROR) << "Same @" << i << " " << ls;
    }
  }
...
TEST(NtlmTest, GenerateProofInputV2SpecTests) {
  ...
  ASSERT_EQ(0, memcmp(test::kExpectedTempFromSpecV2, proof_input.get(),
                      kProofInputLenV2));
}
...
TEST(NtlmTest, GenerateNtlmProofV2SpecTests) {
  ...
  GenerateNtlmProofV2(test::kExpectedNtlmHashV2, test::kExpectedTempFromSpecV2,
                      test::kServerChallenge,
                      test::kExpectedTargetInfoFromSpecV2,
                      arraysize(test::kExpectedTargetInfoFromSpecV2), v2_proof);
...
TEST(NtlmTest, GenerateSessionBaseKeyWithClientTimestampV2SpecTests) {
  ...
  CompareBuf(test::kExpectedSessionBaseKeyWithClientTimestampV2,
...
TEST(NtlmTest, GenerateMicSpecResponseV2) {
  ...
  std::unique_ptr<uint8_t[]> authenticate_msg_raw(
      new uint8_t[authenticate_msg_len]);
  memcpy(authenticate_msg_raw.get(),
         test::kExpectedAuthenticateMsgSpecResponseV2, authenticate_msg_len);
  memset(authenticate_msg_raw.get() + kMicOffsetV2, 0x00, kMicLenV2);
  Buffer authenticate_msg(authenticate_msg_raw.get(), authenticate_msg_len);

  uint8_t mic[kMicLenV2];
  GenerateMicV2(test::kExpectedSessionBaseKeyWithClientTimestampV2,
                negotiate_msg, challenge_msg, authenticate_msg, mic);
  CompareBuf(test::kExpectedMicV2, mic, kMicLenV2);
...
TEST(NtlmTest, GenerateUpdatedTargetInfo) {
  ...
  GenerateUpdatedTargetInfo(true, true, test::kChannelBindings, test::kNtlmSpn,
                            server_av_pairs, &server_timestamp,
                            &updated_target_info, &updated_target_info_len);
...
TEST(NtlmTest, GenerateUpdatedTargetInfoNoEpaOrMic) {
  ...
  // the client.
  GenerateUpdatedTargetInfo(false, false, test::kChannelBindings,
...
TEST(NtlmTest, GenerateNtlmProofV2) {
  ...
  // are read.
  GenerateNtlmProofV2(
      test::kExpectedNtlmHashV2, test::kExpectedTempFromSpecV2,
      test::kServerChallenge, test::kExpectedTargetInfoSpecResponseV2,
      arraysize(test::kExpectedTargetInfoSpecResponseV2), proof);
  ASSERT_EQ(0,
...
TEST(NtlmTest, GenerateNtlmProofWithClientTimestampV2) {
  ...
  GenerateNtlmProofV2(
      test::kExpectedNtlmHashV2, test::kExpectedTempWithClientTimestampV2,
      test::kServerChallenge, test::kExpectedTargetInfoSpecResponseV2,
      arraysize(test::kExpectedTargetInfoSpecResponseV2), proof);
  CompareBuf(test::kExpectedProofSpecResponseWithClientTimestampV2, proof,
             kNtlmProofLenV2);
