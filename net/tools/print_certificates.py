...
def decode_tls_certificate_message(certificate_message):
  ...
  message_length = parser.consume_int24()
  if parser.remaining_byte_count() != message_length:
    sys.stderr.write(
        'message_length(%d) != remaining_byte_count(%d)\n' % (
            message_length, parser.remaining_byte_count()))
    return []

  certificate_list_length = parser.consume_int24()
  if parser.remaining_byte_count() != certificate_list_length:
    sys.stderr.write(
        'certificate_list_length(%d) != remaining_byte_count(%d)\n' % (
            certificate_list_length, parser.remaining_byte_count()))
    return []
