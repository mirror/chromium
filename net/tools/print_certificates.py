...
def create_temp_file(data):
  """Writes |data| to a temporary file and returns its file path."""
  path = create_temp_filepath()
...
def create_temp_filepath():
  """Returns a file path to be used for temporary files. This path will be
  deleted on exist."""
  global g_temp_paths
  f, path = tempfile.mkstemp()
  # TODO(eroman): How to close the file handle?
  g_temp_paths.append(path)
  return path


def clear_temp_filepaths():
  """Deletes all the opened temporary files."""
  global g_temp_paths
  for path in g_temp_paths:
    os.remove(path)
  g_temp_paths = []


def convert_certificate_from_pem_to_der(pem_bytes):
  inpath = create_temp_file(pem_bytes)
  outpath = create_temp_filepath()
  p = subprocess.Popen(["openssl", "x509", "-inform", "PEM",
                        "-in", inpath, "-outform", "DER", "-out", outpath],
                        stdin=subprocess.PIPE,
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE)
  result = p.communicate()
  ...
  if result.returncode == 0:
    return read_file_to_string(outpath)
  return ""
...
def extract_certificates_from_pem(pem_bytes):
  ...
  # If it looks like PEM, strip spaces at the start of each line. This is needed
  # to parse certificates from NetLogs, as they are space-indented and openssl
  # won't accept it directly.
  pem_bytes = strip_indentation_whitespace(pem_bytes)
...
def pretty_print_certificate(certificate_der):
  inpath = create_temp_file(certificate_der)
  outpath = create_temp_filepath()
  p = subprocess.Popen(["openssl", "x509", "-text",
                        "-inform", "DER", "-in", inpath,
                        "-outform", "PEM", "-out", outpath],
                        stdin=subprocess.PIPE,
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE)
  result = p.communicate()
  ...
  if p.returncode == 0:
    return result[0] + read_file_to_string(outpath)
