...
def pretty_print_certificates(certificates_der, pretty_printers):
  ...
  for i in range(len(certificates_der)):
    ...
    for pretty_printer in pretty_printers:
      pretty_printed = pretty_printer(certificate_der)
      if pretty_printed:
...
def parse_outputs(outputs):
  pretty_printers = []
  output_map = {"der2ascii": der2ascii_pretty_print_certificate,
                "openssl": openssl_pretty_print_certificate}
...
from NetLogs).''')
  ...
  parser.add_argument(
      '--output', dest='outputs', action='store', default="der2ascii,openssl",
      help='output formats to use. Default: "der2ascii,openssl"')
  ...
  print pretty_print_certificates(certificates_der, pretty_printers)
