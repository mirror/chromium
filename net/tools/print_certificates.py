...
def openssl_pretty_print_certificate(certificate_der):
  return pretty_print_certificate(["openssl", "x509", "-text", "-inform",
                                   "DER", "-outform", "PEM"], certificate_der)


def der2ascii_pretty_print_certificate(certificate_der):
  return pretty_print_certificate(["der2ascii"], certificate_der)


def pretty_print_certificates(certificates_der, args):
  result = ""
  for i in range(len(certificates_der)):
    certificate_der = certificates_der[i]
    pretty = openssl_pretty_print_certificate(certificate_der)
    if args.der2ascii:
      pretty = (der2ascii_pretty_print_certificate(certificate_der) + "\n" +
                pretty)
    result += """===========================================
Certificate%d
===========================================
...
When multiple SOURCEs are listed, all certificates in them are
concatenated. If no SOURCE is given then data will be read from
stdin.
...
from NetLogs).''')
  ...
  parser.add_argument('--der2ascii', dest='der2ascii', action='store_true',
                      help='Include der2ascii form in output as well.\n'
                           'Requires https://github.com/google/der-ascii' )
  ...
  print pretty_print_certificates(certificates_der, args)
