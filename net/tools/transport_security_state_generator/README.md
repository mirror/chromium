...
decoder can be found in `net::DecodeHSTSPreloadRaw`.
...
dispatch-table     = prefix-part         ; a common prefix for the node and its children
                     value-part          ; any number of values or pointers to children
...
value-part         = 1*(huffman-character node-value)
...
node-entry         = preloaded-entry  ; encoded preload configuration for one hostname
...
hpkp-part               = BIT                 ; whether to enable pinning
                         ...
                         [include-subdomains] ; HPKP includeSubdomains flag, only present
...
expect-staple-part      = BIT                 ; whether to enable Expect-Staple
                          ...
                          ; only present when Expect-Staple is enabled
