...
def get_event_file_sort_key(path):
  '''Order event files by creation timestamp, and then file index
  This will crash if the file name is in a unexpected format (shouldn't
  happen)'''
...
def get_ordered_event_files():
  '''Returns a list of file paths the event files ("event_file_%d.json"). The
  order of the files is from oldest to newest. Because mtime granularity is
  fairly course, file modification times may be the same. Ties are broken
...
def main():
  ...
  with open(output_path, "w") as stitched_file:
    try:
      ...
      sys.stderr.write("File \"constants.json\" not found.\n")
      sys.exit(1)
    ...
    if os.path.exists("end_netlog.json"):
      try:
        ...
        with file:
          ...
          sys.stderr.write("Failed reading \"end_netlog\".\n")
          sys.exit(1)
    else:
      # end_netlog probably won't exist (since it is only written at the end).
      # In that case there just won't be any polled data, so close the events.
      stitched_file.write("]}\n")
