...
// entries.
bool GetInputReader(const base::CommandLine::StringType& value,
    std::unique_ptr<EntryReader>* reader) {
  base::FilePath path = base::FilePath(value);
  ...
  if (base::PathExists(path.AppendASCII("entries.bin"))) {
    ...
    *reader = CreateEntryReaderForCertificateTransparencyDb(path);
    if (!(*reader))
      return false;

    return true;
  } else {
    *reader = CreateEntryReaderForPemFiles(path);
    return true;
  }
...
WARN_UNUSED_RESULT bool ParseCommandLine(
  ...
  if (!GetInputReader(command_line.GetArgs()[0], reader))
    return false;
