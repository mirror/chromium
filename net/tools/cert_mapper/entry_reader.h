...
// Interface for iterating over a corpus of "entries".
//
// An entry is generally speaking a certificate chain.
//
// Implementations must be thread-safe, as this may be called in paralel from
// worker threads to process all entries.
class EntryReader {
 public:
  virtual ~EntryReader();
