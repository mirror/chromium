...
SpawnedTestServerConfig::SpawnedTestServerConfig() = default;
SpawnedTestServerConfig::~SpawnedTestServerConfig() {}

SpawnedTestServerConfig::SpawnedTestServerConfig(
    const SpawnedTestServerConfig& other) = default;
SpawnedTestServerConfig& SpawnedTestServerConfig::operator=(
    const SpawnedTestServerConfig&) = default;

SpawnedTestServerConfig SpawnedTestServerConfig::Load() {
  base::ThreadRestrictions::ScopedAllowIO allow_io;

  SpawnedTestServerConfig result;
  ...
  if (!config->GetString("name", &result.name_)) {
    LOG(WARNING) << "name isn't specified in test server config.";
  }
  ...
  // 'localhost' must always be 127.0.0.1 .
  CHECK(result.name_ != "localhost" ||
        result.address_ == IPAddress::IPv4Localhost());
...
GURL SpawnedTestServerConfig::GetSpawnerUrl(const std::string& command) const {
