...
bool RemoteTestServer::StartInBackground() {
  ...
  // If the server is not on localhost and the OCSP is enabled then pass
  // OCSP proxy port number to the testserver, so it can generate certificates
  // for the OCSP server that will be valid for the proxied port.
  bool ocsp_enabled =
      type() == TYPE_HTTPS && (ssl_options().server_certificate ==
                                   SSLOptions::CERT_AUTO_AIA_INTERMEDIATE ||
                               !ssl_options().GetOCSPArgument().empty());

  if (config_.address() != IPAddress::IPv4Localhost() && ocsp_enabled) {
    ocsp_proxy_ =
