...
bool RemoteTestServer::Start() {
  ...
  // Map testserver domain name to it's address.
  host_resolver_proc_->AddRule(config().name(), config().address().ToString());

  // All other addresses are mapped to 127.0.0.1 . This reflect the default rule
  // configured in NetTestSuite.
  host_resolver_proc_->AddRule("*", "127.0.0.1");

  spawner_communicator_.reset(new SpawnerCommunicator(config()));
