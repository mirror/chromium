...
std::string GetServerTypeString(BaseTestServer::Type type) {
  switch (type) {
    case BaseTestServer::TYPE_BASIC_AUTH_PROXY:
      return "auth_proxy";
...
bool RemoteTestServer::BlockUntilStarted() {
  ...
  bool request_result = start_request_->WaitResult(&server_data);
...
bool RemoteTestServer::Stop() {
  ...
  if (remote_port_) {
    ...
    if (!kill_request->WaitResult(nullptr))
...
// DIR_SOURCE_ROOT here to get the actual path of document root on the
// Androidfda device.
