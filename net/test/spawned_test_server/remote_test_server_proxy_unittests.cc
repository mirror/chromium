...
class RemoteTestServerProxyTest : public testing::Test {
 public:
  ...
  void SendAndReceiveData(StreamSocket* socket1, StreamSocket* socket2) {
    ...
    const int kMessageSize = sizeof(kTestMessage);

    scoped_refptr<IOBufferWithSize> write_buffer =
        new IOBufferWithSize(kMessageSize);
    memcpy(write_buffer->data(), kTestMessage, kMessageSize);
    TestCompletionCallback write_callback;
    int write_result = socket1->Write(write_buffer.get(), write_buffer->size(),
    ...
    ASSERT_EQ(write_callback.GetResult(write_result), kMessageSize);
    ASSERT_EQ(read_callback.GetResult(read_result), kMessageSize);

    EXPECT_EQ(std::string(kTestMessage, kMessageSize),
              std::string(read_buffer->data(), kMessageSize));
  }
