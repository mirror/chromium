...
class TcpSocketProxy::Core {
 public:
  ...
  // Creates local socket for accepting incoming connections. Caller should use
  // local_port() to check if initialization was successful.
  ...
  // Local port port number for the proxy. Returns 0 if Initialize() hasn't been
  // called or initialization wasn't successful.
...
void TcpSocketProxy::Core::Start(const IPEndPoint& remote_address) {
  DCHECK(socket_);
...
void TcpSocketProxy::Start(const IPEndPoint& remote_address) {
  io_task_runner_->PostTask(
