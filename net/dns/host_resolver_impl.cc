...
void RecordTotalTime(bool speculative,
  ...
  if (speculative) {
    UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.TotalTime_speculative", duration);
  } else {
  ...
  if (!from_cache) {
    if (speculative) {
      UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.TotalTimeNotCached_speculative",
                                   duration);
    } else {
...
class HostResolverImpl::ProcTask
 ...
 private:
  ...
  void RecordTaskHistograms(const base::TimeTicks& start_time,
    ...
    if (error == OK)
      UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.ProcTaskSuccess", duration);
    else
      UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.ProcTaskFail", duration);
...
class HostResolverImpl::Job : public PrioritizedDispatcher::Job,
 ...
 private:
  ...
  void OnProcTaskComplete(base::TimeTicks start_time,
    ...
    if (dns_task_error_ != OK) {
      ...
      if (net_error == OK) {
        ...
        UMA_HISTOGRAM_SPARSE_SLOWLY("Net.DNS.DnsTaskError",
  ...
  void OnDnsTaskFailure(const base::WeakPtr<DnsTask>& dns_task,
                        base::TimeDelta duration,
                        int net_error) {
    UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.DnsTaskFail", duration);
  ...
  void OnDnsTaskComplete(base::TimeTicks start_time,
    ...
    UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.DnsTaskSuccess", duration);
  ...
  void RecordJobHistograms(int error) {
    ...
    if (error == OK) {
      if (had_non_speculative_request_) {
        ...
        UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.ResolveSuccess", duration);
        switch (key_.address_family) {
          case ADDRESS_FAMILY_IPV4:
            UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.ResolveSuccess_IPV4",
                                         duration);
            break;
          case ADDRESS_FAMILY_IPV6:
            UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.ResolveSuccess_IPV6",
                                         duration);
            break;
          case ADDRESS_FAMILY_UNSPECIFIED:
            UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.ResolveSuccess_UNSPEC",
                                         duration);
            break;
        }
      } else {
        category = RESOLVE_SPECULATIVE_SUCCESS;
        UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.ResolveSuccess_speculative",
                                     duration);
      }
    } else {
      if (had_non_speculative_request_) {
        category = RESOLVE_FAIL;
        UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.ResolveFail", duration);
        switch (key_.address_family) {
          case ADDRESS_FAMILY_IPV4:
            UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.ResolveSuccess_IPV4",
                                         duration);
        ...
        UMA_HISTOGRAM_LONG_TIMES_100("Net.DNS.ResolveFail_speculative",
                                     duration);
      }
    }
