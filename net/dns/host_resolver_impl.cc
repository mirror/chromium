...
HostResolverImpl::HostResolverImpl(
    ...
    : max_queued_jobs_(0),
      ...
      async_dns_refresher_enabled_(true),
      worker_task_runner_(std::move(worker_task_runner)),
    ...
    if (cache_entry->has_ttl()) {
      if (async_dns_refresher_enabled_ && !cache_entry->refreshing() &&
        ...
        HostCache::Entry new_entry(cache_entry->error(),
                                   cache_entry->addresses(), cache_entry->ttl(),
                                   true);
      ...
      if (async_dns_refresher_enabled_ && !cache_entry->refreshing() &&
        ...
        HostCache::Entry new_entry(cache_entry->error(),
                                   cache_entry->addresses(), true);
        cache_->Set(key, new_entry, cache_entry->expires() - cache_entry->ttl(),
                    cache_entry->ttl());
        DnsRefresher::Refresh(info, this);
      }
    }
