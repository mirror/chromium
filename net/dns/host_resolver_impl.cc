...
class HostResolverImpl::ProcTask
 ...
 private:
  ...
  void StartLookupAttempt() {
    DCHECK(network_task_runner_->BelongsToCurrentThread());
    base::TimeTicks start_time = clock()->NowTicks();
    ++attempt_number_;
  ...
  void OnLookupComplete(const AddressList& results,
    ...
    if (was_retry_attempt) {
      ...
      retry_attempt_finished_time_ = clock()->NowTicks();
    }
  ...
  void RecordTaskHistograms(const base::TimeTicks& start_time,
    ...
    DCHECK(network_task_runner_->BelongsToCurrentThread());
    base::TimeDelta duration = clock()->NowTicks() - start_time;
    if (error == OK)
  ...
  void RecordAttemptHistograms(const base::TimeTicks& start_time,
    ...
    if (!first_attempt_to_complete && is_first_attempt && !was_canceled()) {
      UMA_HISTOGRAM_LONG_TIMES_100(
          ...
          clock()->NowTicks() - retry_attempt_finished_time_);
    }
 ...
 public:
  ...
  DnsTask(DnsClient* client,
      ...
      : client_(client),
        ...
        clock_(clock),
        task_start_time_(clock->NowTicks()) {
    DCHECK(client);
    DCHECK(delegate_);
  }
...
class HostResolverImpl::Job : public PrioritizedDispatcher::Job,
 ...
 public:
  ...
  base::TickClock* clock() { return resolver_->clock(); }
 ...
 private:
  ...
  void UpdatePriority() {
    if (is_queued()) {
      if (priority() != static_cast<RequestPriority>(handle_.priority()))
        priority_change_time_ = clock()->NowTicks();
    ...
    proc_task_ =
        new ProcTask(key_, resolver_->proc_params_,
                     base::Bind(&Job::OnProcTaskComplete,
                                base::Unretained(this), clock()->NowTicks()),
                     proc_task_runner_, net_log_, clock());
  ...
  void RecordJobHistograms(int error) {
    ...
    base::TimeDelta duration = clock()->NowTicks() - start_time_;
    if (error == OK) {
    ...
    while (!requests_.empty()) {
      ...
      if (did_complete) {
        ...
        RecordTotalTime(req->info().is_speculative(), false,
                        clock()->NowTicks() - req->request_time());
      }
...
base::TickClock* HostResolverImpl::RequestImpl::clock() {
  return job_->clock();
}
...
bool HostResolverImpl::ServeFromCache(const Key& key,
  ...
  if (allow_stale)
    cache_entry = cache_->LookupStale(key, clock()->NowTicks(), stale_info);
  else
    cache_entry = cache_->Lookup(key, clock()->NowTicks());
  if (!cache_entry)
    return false;
  ...
  if (*net_error == OK) {
    *addresses = EnsurePortOnAddressList(cache_entry->addresses(), info.port());
    ...
    if (async_dns_refresher_enabled_ && !cache_entry->refreshing() &&
        ttl / 2 > cache_entry->expires() - clock()->NowTicks()) {
...
void HostResolverImpl::CacheResult(const Key& key,
  ...
  if (cache_.get() && (entry.error() == OK || ttl > base::TimeDelta()))
    cache_->Set(key, entry, clock()->NowTicks(), ttl);
}
...
bool HostResolverImpl::IsIPv6Reachable(const NetLogWithSource& net_log) {
  ...
  bool cached = true;
  if ((clock()->NowTicks() - last_ipv6_probe_time_).InMilliseconds() >
      kIPv6ProbePeriodMs) {
    last_ipv6_probe_result_ =
        IsGloballyReachable(IPAddress(kIPv6ProbeAddress), net_log);
    last_ipv6_probe_time_ = clock()->NowTicks();
    cached = false;
  }
