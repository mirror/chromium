...
class DnsHTTPAttempt : public DnsAttempt, public URLRequest::Delegate {
 public:
  DnsHTTPAttempt(std::unique_ptr<DnsQuery> query,
    ...
    extra_request_headers.SetHeader("Accept", "application/dns-udpwireformat");
  ...
  void ResponseCompleted(int net_error) {
    DCHECK_NE(net::ERR_IO_PENDING, net_error);
    int rv = ERR_DNS_MALFORMED_RESPONSE;
    if (net_error == OK && response_ && buffer_.get() && buffer_->size())
      rv = DoReadResponseComplete();
    callback_.Run(rv);
  }
 ...
 public:
  ...
  void AddDnsOverHttpsServer(GURL server, bool use_post) override {
    dns_over_https_servers_.push_back(
        DnsOverHttpsServerConfig(server, use_post));
  }
