...
class DnsHTTPAttempt : public DnsAttempt, public URLFetcherDelegate {
 public:
  DnsHTTPAttempt(std::unique_ptr<DnsQuery> query, GURL server)
      : DnsAttempt(0),
    ...
    GURL url(server.ReplaceComponents(replacements));
...
class DnsTransactionImpl : public DnsTransaction,
 ...
 private:
  ...
  AttemptResult MakeHTTPAttempt(const std::vector<GURL>& servers) {
    doh_attempt_ = true;
    unsigned attempt_number = attempts_.size();
    uint16_t id = session_->NextQueryId();
    std::unique_ptr<DnsQuery> query;
    ...
    DnsHTTPAttempt* attempt =
        new DnsHTTPAttempt(std::move(query), servers[doh_attempts_]);
    ++doh_attempts_;
    attempts_.push_back(base::WrapUnique(attempt));
    ++attempts_count_;
