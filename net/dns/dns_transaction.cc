...
class DnsHTTPAttempt : public DnsAttempt, public URLRequest::Delegate {
 public:
  DnsHTTPAttempt(std::unique_ptr<DnsQuery> query,
    ...
    extra_request_headers.SetHeader("Accept", kContentType);

    request_ =
        url_request_context_getter->GetURLRequestContext()->CreateRequest(
            url, request_priority_, this,
            net::DefineNetworkTrafficAnnotation("dns-over-https",
                                                "DNS lookups over HTTPS"));
  ...
  void OnResponseStarted(net::URLRequest* request, int net_error) override {
    ...
    if (request_->GetResponseCode() != 200 ||
        !request->response_headers()->HasHeader(
            HttpRequestHeaders::kContentLength) ||
        !request->response_headers()->GetMimeType(&content_type) ||
        0 != content_type.compare(kContentType)) {
      ResponseCompleted(ERR_DNS_MALFORMED_RESPONSE);
      return;
    }

    size_t response_length = request_->response_headers()->GetContentLength();
    if (response_length <= 0 ||
        response_length > 2 * 1024 * 1024 /* HttpStreamParser::kMaxBufSize */) {
      ResponseCompleted(ERR_DNS_MALFORMED_RESPONSE);
      return;
    }
    response_.reset(new DnsResponse(response_length + 1));
    buffer_ =
        new DrainableIOBuffer(response_->io_buffer(), response_length + 1);
    if (buffer_->size() <= static_cast<int>(response_length)) {
      // Failed to allocate a big enought buffer.
      ResponseCompleted(ERR_DNS_MALFORMED_RESPONSE);
      return;
    }
  ...
  void StartReading(bool is_continuation) {
    DCHECK(buffer_->data());
    DCHECK_GT(buffer_->size(), 0);

    int bytes_read = request_->Read(buffer_.get(), buffer_->BytesRemaining());
    ...
    if (!is_continuation || bytes_read <= 0) {
      ...
      base::SequencedTaskRunnerHandle::Get()->PostTask(
          FROM_HERE,
          base::BindOnce(&DnsHTTPAttempt::OnReadCompleted,
                         base::Unretained(this), request_.get(), bytes_read));
    }
  }
  ...
  void ResponseCompleted(int net_error) {
    DCHECK_NE(net::ERR_IO_PENDING, net_error);
    if (net_error != OK)
      callback_.Run(net_error);
    else if (!response_ || !buffer_.get() || 0 == buffer_->size())
      callback_.Run(ERR_DNS_MALFORMED_RESPONSE);
    else if (!response_->InitParse(buffer_->BytesConsumed(), *query_))
      callback_.Run(ERR_DNS_MALFORMED_RESPONSE);
    else if (response_->rcode() == dns_protocol::kRcodeNXDOMAIN)
      callback_.Run(ERR_NAME_NOT_RESOLVED);
    else if (response_->rcode() != dns_protocol::kRcodeNOERROR)
      callback_.Run(ERR_DNS_SERVER_FAILED);
    else
      callback_.Run(OK);
  }

 private:
  static constexpr const char* kContentType = "application/dns-udpwireformat";
...
class DnsTransactionImpl : public DnsTransaction,
 ...
 public:
  ...
  void SetRequestContext(URLRequestContextGetter* context) override {
    url_request_context_getter_ = context;
  }
 ...
 private:
  ...
  AttemptResult MakeAttempt() {
    ...
    if (config.dns_over_https_servers_.size() > doh_attempts_
        // Check that we're not looking up this server with itself.
        && (0 != DNSDomainToString(qnames_.front())
                     .compare(config.dns_over_https_servers_[doh_attempts_]
                                  .server.host())
            // This makes the assumption that we don't have duplicates in the
            // list, but DnsHTTPAttempt::Start() will catch that.
            || config.dns_over_https_servers_.size() > ++doh_attempts_)) {
      return MakeHTTPAttempt(config.dns_over_https_servers_);
    }
  ...
  AttemptResult MakeHTTPAttempt(
    ...
    DnsHTTPAttempt* attempt = new DnsHTTPAttempt(
        std::move(query), servers[doh_attempts_].server,
        servers[doh_attempts_].use_post, url_request_context_getter_.get(),
        request_priority_);
    ++doh_attempts_;
    attempts_.push_back(base::WrapUnique(attempt));
    ++attempts_count_;
  ...
  bool MoreAttemptsAllowed() const {
    ...
    return attempts_.size() < config.attempts * config.nameservers.size() +
                                  config.dns_over_https_servers_.size();
  }
