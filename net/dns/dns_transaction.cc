...
class DnsHTTPAttempt : public DnsAttempt, public URLRequest::Delegate {
 public:
  DnsHTTPAttempt(std::unique_ptr<DnsQuery> query,
      ...
      : DnsAttempt(0), total_bytes_read_(0), query_(std::move(query)) {
    GURL url(server);
    if (!use_post) {
  ...
  void ResponseCompleted(int net_error) {
    ...
    else if (response_ && buffer_.get() && buffer_->size())
      callback_.Run(DoReadResponseComplete());
    else
...
class DnsTransactionImpl : public DnsTransaction,
 ...
 private:
  ...
  AttemptResult MakeHTTPAttempt(
      const std::vector<DnsOverHttpsServerConfig>& servers) {
    doh_attempt_ = true;
    unsigned attempt_number = attempts_.size();
    uint16_t id = session_->NextQueryId();
    std::unique_ptr<DnsQuery> query;
