...
std::unique_ptr<OptRecordRdata> OptRecordRdata::Create(
  ...
  while (reader.remaining() > 0) {
    ...
    if (!(reader.ReadU16(&code) && reader.ReadU16(&size) &&
          reader.ReadPiece(&data, size))) {
      return nullptr;
    }
    rdata->opts_.push_back(Opt(code, data));
  }
  return rdata;
}
...
void OptRecordRdata::AddOpt(const Opt& opt) {
  base::StringPiece optData = opt.data();

  // Resize buffer to accommodate new OPT.
  const size_t orig_rdata_size = buf_.size();
  buf_.resize(orig_rdata_size + Opt::kHeaderSize + optData.size());

  // Start writing from the end of the existing rdata.
  base::BigEndianWriter writer(buf_.data() + orig_rdata_size, buf_.size());
  DCHECK(writer.WriteU16(opt.code()) && writer.WriteU16(optData.size()) &&
         writer.WriteBytes(optData.data(), optData.size()));
