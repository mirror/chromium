...
void RuleBasedHostResolverProc::AddRuleInternal(const Rule& rule) {
  ...
  // * If it's an invalid domain name, switch to a fail rule.
  if (fixed_rule.resolver_type == Rule::kResolverTypeSystem) {
    IPAddress ip_address;
    int valid_address = ip_address.AssignFromIPLiteral(fixed_rule.replacement);
    if (valid_address) {
      fixed_rule.resolver_type = Rule::kResolverTypeIPLiteral;
    } else if (!IsValidDNSDomain(fixed_rule.replacement)) {
