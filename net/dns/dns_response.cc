...
DnsResponse::DnsResponse(size_t length)
    : io_buffer_(new IOBufferWithSize(length)), io_buffer_size_(length) {}
...
bool DnsResponse::InitParse(int nbytes, const DnsQuery& query) {
  ...
  if (nbytes < query.io_buffer()->size() ||
      nbytes >= static_cast<int>(io_buffer_size_))
    return false;
...
bool DnsResponse::InitParseWithoutQuery(int nbytes) {
  ...
  if (nbytes < static_cast<int>(kHeaderSize) ||
      nbytes >= static_cast<int>(io_buffer_size_))
    return false;
