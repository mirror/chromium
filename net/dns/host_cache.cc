...
bool HasCacheEntry(HostCache* cache,
  ...
  const HostCache::Entry* entry =
      cache->LookupStale(cache_key, base::TimeTicks::Now(), stale_out);
  if (entry == nullptr) {
    // Might not have found the cache entry because the address_family or
    // host_resolver_flags in entry do not match those used for the original
    ...
    entry = cache->LookupStale(cache_key, base::TimeTicks::Now(), stale_out);
    if (entry == nullptr)
      return false;
  }
