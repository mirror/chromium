...
class NET_EXPORT HostCache {
 public:
  ...
  // Returns the number of entries that were restored in RestoreFromListValue().
  size_t restore_size() const { return restore_size_; }
  void set_restore_size(size_t size) { restore_size_ = size; }
 ...
 private:
  ...
  // Number of cache entries that were restored in RestoreFromListValue().
  // Used in histograms.
