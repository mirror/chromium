...
class DnsTransactionTest : public testing::Test {
 public:
  ...
  void ConfigDoHServers(bool clearUDP, bool use_post) {
    if (clearUDP) {
      config_.nameservers.clear();
      ConfigureFactory();
    }
    ...
    session_->dohservers.push_back(std::make_tuple(url, use_post));
  }
