...
class TransactionHelper {
 public:
  ...
  void add_doh_server(const GURL& url, bool use_post) {
 ...
 public:
  ...
  std::unique_ptr<net::test_server::HttpResponse> HandleRequest(
    ...
    for (auto server : config_.dns_over_https_servers_) {
      ...
      std::string::size_type query_start = request.relative_url.find('?');
      if (query_start == std::string::npos)
        url = url.Resolve(request.relative_url);
      else
        url = url.Resolve(request.relative_url.substr(0, query_start));
      if (server.server == url &&
          (server.use_post
               ? test_server::HttpMethod::METHOD_POST
               : test_server::HttpMethod::METHOD_GET) == request.method) {
        method_match = true;
        break;
      }
    }
    if (!method_match) {
      ...
      resp->set_code(HTTP_BAD_REQUEST);
      return resp;
    }
