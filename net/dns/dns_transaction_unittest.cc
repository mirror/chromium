...
TEST_F(DnsTransactionTest, HttpsMarkUdpBad) {
  ...
  TransactionHelper helper1(kT1HostName, kT1Qtype, kT1RecordCount);
  EXPECT_TRUE(helper0.RunUntilDone(transaction_factory_.get()));
  // Server 1 (UDP) should be marked bad. Server 0 (UDP) should be good
  ...
  // Since 1 was bad to start, we started with 0 which will now be the
  // most recent failure, so Server - (UDP) should be marked bad.
  // Server 1 (UDP) should be good and since 2 is our only Doh server.
...
TEST_F(DnsTransactionTest, HttpsPostTestNoCookies) {
  ...
  helper0.request_context().cookie_store()->GetAllCookiesForURLAsync(
      config_.dns_over_https_servers_[0].server,
  ...
  helper1.request_context().cookie_store()->SetCookieWithOptionsAsync(
      config_.dns_over_https_servers_[0].server, "test-cookie=you-still-fail",
      options,
...
TEST_F(DnsTransactionTest, HttpsPostWithError) {
  ...
  AddQueryAndResponse(0, kT0HostName, kT0Qtype, kT0ResponseDatagram,
                      ...
                      Transport::HTTPS);
  TransactionHelper helper0(kT0HostName, kT0Qtype, ERR_DNS_MALFORMED_RESPONSE);
  SetResponseModifierCallback(base::BindRepeating(MakeResponseWithError));
  EXPECT_TRUE(helper0.RunUntilDone(transaction_factory_.get()));
}
...
TEST_F(DnsTransactionTest, HttpsCantLookupDohServers) {
  ConfigDohServers(true /* clear_udp */, true /* use_post */, 2);
  const char* hostname =
      config_.dns_over_https_servers_[0].server.host().c_str();
  TransactionHelper helper0(hostname, kT0Qtype, ERR_CONNECTION_REFUSED);
  transaction_ids_.push_back(0);
