...
class NET_EXPORT_PRIVATE SerialWorker
 ...
 public:
  ...
  // Allow DoWork() to use //base sync primitives (see
  // base::WithBaseSyncPrimitives()). Must be called before WorkNow().
  void AllowBaseSyncPrimitivesForTesting();
 ...
 protected:
  ...
  // Used to verify that the constructor, WorkNow(), Cancel(),
  // AllowBaseSyncPrimitivesForTesting() and OnWorkJobFinished() are called on
  // the same sequence.
 ...
 private:
  ...
  // Used to post a DoWork() task to TaskScheduler.
  base::TaskTraits do_work_task_traits_;
