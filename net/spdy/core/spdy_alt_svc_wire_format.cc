...
const int HEX_DIGIT_TO_VALUE[256] = {
...
bool LittleEndianUIntFromHexString(const std::string& hex_string,
  ...
  *output = 0;
  ...
  while (i >= 1) {
    int upper_nibble = HEX_DIGIT_TO_VALUE[static_cast<int>(hex_string[i - 1])];
    int lower_nibble = HEX_DIGIT_TO_VALUE[static_cast<int>(hex_string[i])];
  ...
  for (const AlternativeService& altsvc : altsvc_vector) {
    ...
    if (!altsvc.version.empty()) {
      if (is_ietf_format_quic) {
        for (uint32_t quic_version_tag : altsvc.version) {
          value.append("; quic=");
