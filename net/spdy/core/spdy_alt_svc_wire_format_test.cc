...
void FuzzHeaderFieldValue(
  ...
  if (i & 1 << 0) {
    expected_altsvc->protocol_id = "quic";
  ...
  if (i & 1 << 6) {
    if (is_ietf_format_quic) {
      const QuicVersion quic_version = QUIC_VERSION_41;
      expected_altsvc->version.push_back(quic_version);
      header_field_value->append("; quic=");
      QuicTag quic_version_tag = QuicVersionToQuicTag(quic_version);
      header_field_value->append(
