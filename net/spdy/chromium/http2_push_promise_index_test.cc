...
// For simplicity, these tests use nullptr instead of actual SpdySession
// instances (which would be necessary to create a non-null weak pointer).
// Streams are distinguished only by their SpdyStreamId.
...
class TestDelegate : public Http2PushPromiseIndex::Delegate {
 public:
  ...
  // SpdySession implements Delegate and deletes claimed streams from its
  // internal index, which calls back to Http2PushPromiseIndex::Unregister().
  // TestDelegate mimics this behavior.
