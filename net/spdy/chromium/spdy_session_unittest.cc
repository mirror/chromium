...
class SpdySessionTest : public PlatformTest {
 ...
 protected:
  ...
  void MaybeSendPrefacePing() {
    SpdySessionPeer::MaybeSendPrefacePing(session_.get());
  }

  void WritePingFrame(SpdyPingId unique_id, bool is_ack) {
    SpdySessionPeer::WritePingFrame(session_.get(), unique_id, is_ack);
  }

  void CheckPingStatus(base::TimeTicks last_check_time) {
    SpdySessionPeer::CheckPingStatus(session_.get(), last_check_time);
  }

  void RecordPushedStreamVaryResponseHeaderHistogram(
      const SpdyHeaderBlock& headers) {
    SpdySessionPeer::RecordPushedStreamVaryResponseHeaderHistogram(headers);
  }

  bool OnUnknownFrame(SpdyStreamId stream_id, uint8_t frame_type) {
    return SpdySessionPeer::OnUnknownFrame(session_.get(), stream_id,
                                           frame_type);
  }

  void IncreaseSendWindowSize(int delta_window_size) {
    SpdySessionPeer::IncreaseSendWindowSize(session_.get(), delta_window_size);
  }

  void DecreaseSendWindowSize(int32_t delta_window_size) {
    SpdySessionPeer::DecreaseSendWindowSize(session_.get(), delta_window_size);
  }

  void IncreaseRecvWindowSize(int delta_window_size) {
    SpdySessionPeer::IncreaseRecvWindowSize(session_.get(), delta_window_size);
  }

  void DecreaseRecvWindowSize(int32_t delta_window_size) {
    SpdySessionPeer::DecreaseRecvWindowSize(session_.get(), delta_window_size);
  }

  void set_in_io_loop(bool in_io_loop) {
    SpdySessionPeer::set_in_io_loop(session_.get(), in_io_loop);
  }

  void set_stream_hi_water_mark(SpdyStreamId stream_hi_water_mark) {
    SpdySessionPeer::set_stream_hi_water_mark(session_.get(),
                                              stream_hi_water_mark);
  }

  void set_last_accepted_push_stream_id(
      SpdyStreamId last_accepted_push_stream_id) {
    SpdySessionPeer::set_last_accepted_push_stream_id(
        session_.get(), last_accepted_push_stream_id);
  }

  size_t num_pushed_streams() {
    return SpdySessionPeer::num_pushed_streams(session_.get());
  }

  size_t num_active_pushed_streams() {
    return SpdySessionPeer::num_active_pushed_streams(session_.get());
  }

  size_t max_concurrent_streams() {
    return SpdySessionPeer::max_concurrent_streams(session_.get());
  }

  void set_max_concurrent_streams(size_t max_concurrent_streams) {
    SpdySessionPeer::set_max_concurrent_streams(session_.get(),
                                                max_concurrent_streams);
  }

  void set_max_concurrent_pushed_streams(size_t max_concurrent_pushed_streams) {
    SpdySessionPeer::set_max_concurrent_pushed_streams(
        session_.get(), max_concurrent_pushed_streams);
  }

  int64_t pings_in_flight() {
    return SpdySessionPeer::pings_in_flight(session_.get());
  }

  SpdyPingId next_ping_id() {
    return SpdySessionPeer::next_ping_id(session_.get());
  }

  base::TimeTicks last_read_time() {
    return SpdySessionPeer::last_read_time(session_.get());
  }

  void set_last_read_time(base::TimeTicks last_read_time) {
    SpdySessionPeer::set_last_read_time(session_.get(), last_read_time);
  }

  bool check_ping_status_pending() {
    return SpdySessionPeer::check_ping_status_pending(session_.get());
  }

  void set_check_ping_status_pending(bool check_ping_status_pending) {
    SpdySessionPeer::set_check_ping_status_pending(session_.get(),
                                                   check_ping_status_pending);
  }
  int32_t session_send_window_size() {
    return SpdySessionPeer::session_send_window_size(session_.get());
  }

  int32_t session_recv_window_size() {
    return SpdySessionPeer::session_recv_window_size(session_.get());
  }

  void set_session_recv_window_size(int32_t session_recv_window_size) {
    SpdySessionPeer::set_session_recv_window_size(session_.get(),
                                                  session_recv_window_size);
  }

  int32_t session_unacked_recv_window_bytes() {
    return SpdySessionPeer::session_unacked_recv_window_bytes(session_.get());
  }

  int32_t stream_initial_send_window_size() {
    return SpdySessionPeer::stream_initial_send_window_size(session_.get());
  }

  void set_connection_at_risk_of_loss_time(base::TimeDelta duration) {
    SpdySessionPeer::set_connection_at_risk_of_loss_time(session_.get(),
                                                         duration);
  }

  void set_hung_interval(base::TimeDelta duration) {
    SpdySessionPeer::set_hung_interval(session_.get(), duration);
  }

  size_t pending_create_stream_queue_size(RequestPriority priority) {
    return SpdySessionPeer::pending_create_stream_queue_size(session_.get(),
                                                             priority);
  }

  size_t num_active_streams() {
    return SpdySessionPeer::num_active_streams(session_.get());
  }

  size_t num_created_streams() {
    return SpdySessionPeer::num_created_streams(session_.get());
  }

  size_t num_unclaimed_pushed_streams() {
    return SpdySessionPeer::num_unclaimed_pushed_streams(session_.get());
  }

  bool has_unclaimed_pushed_stream_for_url(const GURL& url) {
    return SpdySessionPeer::has_unclaimed_pushed_stream_for_url(session_.get(),
                                                                url);
  }
