...
TEST_F(SpdySessionTest, ResumeSessionWithStalledStream) {
  ...
  // When session is unstalled, ResumeSendStalledStreams() is called.  This
  // should not drop |stream1| on the floor.
  UnstallSessionSend(kBodyDataSize);

  EXPECT_TRUE(stream1->send_stalled_by_flow_control());
  EXPECT_FALSE(stream2->send_stalled_by_flow_control());

  // If the session is not stalled when the stream is unstalled, then
  // SpdyStream::PossiblyResumeIfSendStalled() resumes the stream, hiding the
  // bug.
