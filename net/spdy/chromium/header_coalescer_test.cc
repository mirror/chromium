...
// RFC 7540 Section 10.3 and RFC 7230 Section 3.2.
// Valid header value is defined as:
...
TEST_F(HeaderCoalescerTest, HeaderValueInvalid0) {
  header_coalescer_.OnHeader("foo", "");
  EXPECT_TRUE(header_coalescer_.error_seen());
  ExpectEntry("foo", "", "Header value must not be empty.");
}

TEST_F(HeaderCoalescerTest, HeaderValueInvalid1) {
  header_coalescer_.OnHeader("foo", " ");
  EXPECT_TRUE(header_coalescer_.error_seen());
  ExpectEntry("foo", "%20",
...
TEST_F(HeaderCoalescerTest, HeaderValueInvalid2) {
  header_coalescer_.OnHeader("foo", " foo");
  EXPECT_TRUE(header_coalescer_.error_seen());
  ExpectEntry("foo", "%20foo",
              "Header value must start and end with field-vchar.");
}

TEST_F(HeaderCoalescerTest, HeaderValueInvalid3) {
  header_coalescer_.OnHeader("foo", "foo ");
  EXPECT_TRUE(header_coalescer_.error_seen());
  ExpectEntry("foo", "foo%20",
              "Header value must start and end with field-vchar.");
}

TEST_F(HeaderCoalescerTest, HeaderValueInvalid4) {
  header_coalescer_.OnHeader("foo", "\t");
  EXPECT_TRUE(header_coalescer_.error_seen());
  ExpectEntry("foo", "%09",
              "Header value must start and end with field-vchar.");
...
TEST_F(HeaderCoalescerTest, HeaderValueInvalid5) {
  header_coalescer_.OnHeader("foo", "\tfoo");
  EXPECT_TRUE(header_coalescer_.error_seen());
  ExpectEntry("foo", "%09foo",
              "Header value must start and end with field-vchar.");
}

TEST_F(HeaderCoalescerTest, HeaderValueInvalid6) {
  header_coalescer_.OnHeader("foo", "foo\t");
  EXPECT_TRUE(header_coalescer_.error_seen());
  ExpectEntry("foo", "foo%09",
              "Header value must start and end with field-vchar.");
}

TEST_F(HeaderCoalescerTest, HeaderValueInvalid7) {
  header_coalescer_.OnHeader("foo", "bar\x7f");
  EXPECT_TRUE(header_coalescer_.error_seen());
  ExpectEntry("foo", "bar%7F",
              "Header value must start and end with field-vchar.");
}

TEST_F(HeaderCoalescerTest, HeaderValueInvalid8) {
  header_coalescer_.OnHeader("foo", "foo\nbar");
  EXPECT_TRUE(header_coalescer_.error_seen());
  ExpectEntry("foo", "foo%0Abar", "Invalid character in header value.");
}

TEST_F(HeaderCoalescerTest, HeaderValueInvalid9) {
  header_coalescer_.OnHeader("foo", "foo\rbar");
  EXPECT_TRUE(header_coalescer_.error_seen());
  ExpectEntry("foo", "foo%0Dbar", "Invalid character in header value.");
}

TEST_F(HeaderCoalescerTest, HeaderValueInvalidA) {
  header_coalescer_.OnHeader("foo", SpdyStringPiece("foo\0bar", 7));
  EXPECT_TRUE(header_coalescer_.error_seen());
  ExpectEntry("foo", "foo%00bar", "Invalid character in header value.");
}

TEST_F(HeaderCoalescerTest, HeaderValueInvalidB) {
