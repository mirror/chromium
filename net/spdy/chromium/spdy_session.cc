...
void SpdySession::MaybeSendPrefacePing() {
  if (pings_in_flight_ || check_ping_status_pending_ ||
      !enable_ping_based_connection_checking_) {
    return;
  }
  ...
  if (time_func_() > last_activity_time_ + connection_at_risk_of_loss_time_)
...
void SpdySession::CheckPingStatus(base::TimeTicks last_check_time) {
  ...
  if (now > last_activity_time_ + hung_interval_ ||
      last_activity_time_ < last_check_time) {
    DoDrainSession(ERR_SPDY_PING_FAILED, "Failed ping.");
    return;
  }

  // Check the status of connection after a delay.
  const base::TimeDelta delay = last_activity_time_ + hung_interval_ - now;
  base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE, base::Bind(&SpdySession::CheckPingStatus,
