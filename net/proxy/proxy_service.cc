...
class ProxyService::Request
    : public base::RefCounted<ProxyService::Request> {
 public:
...
int ProxyService::ResolveProxyHelper(const GURL& raw_url,
  ...
  scoped_refptr<Request> req(new Request(
...
void ProxyService::CancelPacRequest(Request* req) {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  DCHECK(req);
  req->Cancel();
