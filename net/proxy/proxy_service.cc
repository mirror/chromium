...
//
// During this time window, any resolve requests sent to the ProxyResolutionService will
...
GURL SanitizeUrl(const GURL& url, ProxyResolutionService::SanitizeUrlPolicy policy) {
  DCHECK(url.is_valid());
 ...
 private:
  ...
  void OnProxyScriptDeciderCompleted(int result) {
    if (HasScriptDataChanged(result, decider_->script_data())) {
      // Something has changed, we must notify the ProxyResolutionService so it can
  ...
  void NotifyProxyServiceOfChange(
      ...
      const scoped_refptr<ProxyResolverScriptData>& script_data,
      const ProxyConfig& effective_config) {
    // Note that |this| may be deleted after calling into the ProxyResolutionService.
...
// ProxyResolutionService::PacRequest ---------------------------------------------------

class ProxyResolutionService::PacRequest
    : public base::RefCounted<ProxyResolutionService::PacRequest> {
 public:
  PacRequest(ProxyResolutionService* service,
             const GURL& url,
 ...
 private:
  friend class base::RefCounted<ProxyResolutionService::PacRequest>;
  ...
  // Note that we don't hold a reference to the ProxyResolutionService. Outstanding
  // requests are cancelled during ~ProxyResolutionService, so this is guaranteed
...
// static
std::unique_ptr<ProxyResolutionService> ProxyResolutionService::CreateUsingSystemProxyResolver(
...
// static
std::unique_ptr<ProxyResolutionService> ProxyResolutionService::CreateWithoutProxyResolver(
    std::unique_ptr<ProxyConfigService> proxy_config_service,
    NetLog* net_log) {
  return std::make_unique<ProxyResolutionService>(
      std::move(proxy_config_service),
...
// static
std::unique_ptr<ProxyResolutionService> ProxyResolutionService::CreateFixed(const ProxyConfig& pc) {
...
std::unique_ptr<ProxyResolutionService> ProxyResolutionService::CreateDirectWithNetLog(
    NetLog* net_log) {
...
// static
std::unique_ptr<ProxyResolutionService> ProxyResolutionService::CreateFixedFromPacResult(
...
int ProxyResolutionService::ResolveProxyHelper(const GURL& raw_url,
                                     const std::string& method,
                                     ProxyInfo* result,
                                     const CompletionCallback& callback,
                                     PacRequest** pac_request,
...
int ProxyResolutionService::TryToCompleteSynchronously(const GURL& url,
                                             ProxyDelegate* proxy_delegate,
                                             ProxyInfo* result) {
...
int ProxyResolutionService::ReconsiderProxyAfterError(const GURL& url,
                                            const std::string& method,
                                            int net_error,
                                            ProxyInfo* result,
                                            const CompletionCallback& callback,
...
void ProxyResolutionService::CancelPacRequest(PacRequest* req) {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  DCHECK(req);
  req->Cancel();
...
LoadState ProxyResolutionService::GetLoadState(const PacRequest* req) const {
  CHECK(req);
...
bool ProxyResolutionService::ContainsPendingRequest(PacRequest* req) {
  return pending_requests_.count(req) == 1;
}

void ProxyResolutionService::RemovePendingRequest(PacRequest* req) {
  DCHECK(ContainsPendingRequest(req));
  pending_requests_.erase(req);
}

int ProxyResolutionService::DidFinishResolvingProxy(const GURL& url,
                                          const std::string& method,
                                          ProxyDelegate* proxy_delegate,
                                          ProxyInfo* result,
                                          int result_code,
...
ProxyResolutionService::State ProxyResolutionService::ResetProxyConfig(bool reset_fetched_config) {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
// static
const ProxyResolutionService::PacPollPolicy* ProxyResolutionService::set_pac_script_poll_policy(
...
void ProxyResolutionService::InitializeUsingDecidedConfig(
  ...
  int rv = init_proxy_resolver_->StartSkipDecider(
      ...
      script_data, base::Bind(&ProxyResolutionService::OnInitProxyResolverComplete,
                              base::Unretained(this)));
