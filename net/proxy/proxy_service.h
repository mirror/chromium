...
class NET_EXPORT ProxyResolutionService : public NetworkChangeNotifier::IPAddressObserver,
 ...
 public:
  ...
  // ones.  Must be called before the URLRequestContext the ProxyResolutionService was
  // created with is torn down, if it's torn down before th ProxyResolutionService itself.
  void OnShutdown();

  // Tells this ProxyResolutionService to start using a new ProxyConfigService to
  ...
  static std::unique_ptr<ProxyResolutionService> CreateFixed(const ProxyConfig& pc);
  static std::unique_ptr<ProxyResolutionService> CreateFixed(const std::string& proxy);
  ...
  static std::unique_ptr<ProxyResolutionService> CreateDirectWithNetLog(NetLog* net_log);

  // This method is used by tests to create a ProxyResolutionService that returns a
