...
class NET_EXPORT_PRIVATE ProxyConfigServiceLinux : public ProxyConfigService {
 public:
  ...
  // the ProxyConfigService's main SequencedTaskRunner, which passed to its
  ...
  // config is posted to the IO thread through Delegate::SetNewProxyConfig(). We
  // then notify observers on the main TaskRunner of the configuration change.
  //
  ...
  class Delegate : public base::RefCountedThreadSafe<Delegate> {
   public:
    ...
    // must be called from the UI thread. The message loop for the
