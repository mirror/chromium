...
// will get invoked even if the creation fails, but not for memory caches.
// The invocation will always be via the event loop, and never direct.
//
// Note that this will not wait for |post_cleanup_callback| of a previous
// instance for |path| to complete.
NET_EXPORT int CreateCacheBackend(net::CacheType type,
                                  net::BackendType backend_type,
                                  const base::FilePath& path,
                                  int max_bytes,
