...
class SimpleSynchronousEntry {
 ...
 private:
  ...
  // Helpers for grabbing File objects from SimpleFileTracker. ReleaseFile must
  // be called once for each AcquireFile call. It's safe to call AcquireFile
  // multiple times before the subsequent ReleaseFiles.
  ...
  AcquiredFileState acquired_files_[kSimpleEntrySubFileCount];
};
