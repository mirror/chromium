...
TEST_F(SimpleFileTrackerTest, Basic) {
  ...
  SimpleFileTracker::EntryFileKey key(1);
  ...
  base::File file_0(path_0, base::File::FLAG_CREATE | base::File::FLAG_WRITE);
  base::File file_1(path_1, base::File::FLAG_CREATE | base::File::FLAG_WRITE);
  ASSERT_TRUE(file_0.IsValid());
  ASSERT_TRUE(file_1.IsValid());

  file_tracker_.Register(entry.get(), key, SimpleFileTracker::SubFile::FILE_0,
                         std::move(file_0));
  file_tracker_.Register(entry.get(), key, SimpleFileTracker::SubFile::FILE_1,
                         std::move(file_1));

  base::File* borrow_0 = file_tracker_.Acquire(
      entry.get(), key, SimpleFileTracker::SubFile::FILE_0);
  base::File* borrow_1 = file_tracker_.Acquire(
      entry.get(), key, SimpleFileTracker::SubFile::FILE_1);
  ...
  // For stream 0 do release/close, for stream 1 do close/release.
  file_tracker_.Release(entry.get(), key, SimpleFileTracker::SubFile::FILE_0);
  file_tracker_.Close(entry.get(), key, SimpleFileTracker::SubFile::FILE_0);

  file_tracker_.Close(entry.get(), key, SimpleFileTracker::SubFile::FILE_1);
  file_tracker_.Release(entry.get(), key, SimpleFileTracker::SubFile::FILE_1);
...
TEST_F(SimpleFileTrackerTest, Collision) {
  ...
  base::File file2(path2, base::File::FLAG_CREATE | base::File::FLAG_WRITE);
  ASSERT_TRUE(file.IsValid());
  ASSERT_TRUE(file2.IsValid());

  file_tracker_.Register(entry.get(), key, SimpleFileTracker::SubFile::FILE_0,
                         std::move(file));
  file_tracker_.Register(entry2.get(), key, SimpleFileTracker::SubFile::FILE_0,
                         std::move(file2));

  base::File* borrow = file_tracker_.Acquire(
      entry.get(), key, SimpleFileTracker::SubFile::FILE_0);
  base::File* borrow2 = file_tracker_.Acquire(
      entry2.get(), key, SimpleFileTracker::SubFile::FILE_0);
  ...
  file_tracker_.Release(entry2.get(), key, SimpleFileTracker::SubFile::FILE_0);
  file_tracker_.Close(entry2.get(), key, SimpleFileTracker::SubFile::FILE_0);
...
TEST_F(SimpleFileTrackerTest, PointerStability) {
  // Make sure the base::File* lent out doesn't get screwed up as we update
  // the state.
  ...
  SimpleFileTracker::EntryFileKey key(1);
  base::File file_0(cache_path_.AppendASCII("0"),
                    base::File::FLAG_CREATE | base::File::FLAG_WRITE);
  ASSERT_TRUE(file_0.IsValid());
  file_tracker_.Register(entries[0].get(), key,
                         SimpleFileTracker::SubFile::FILE_0, std::move(file_0));

  base::File* borrow = file_tracker_.Acquire(
      entries[0].get(), key, SimpleFileTracker::SubFile::FILE_0);
  for (int i = 1; i < kEntries; ++i) {
    base::File file_n(cache_path_.AppendASCII(base::IntToString(i)),
                      base::File::FLAG_CREATE | base::File::FLAG_WRITE);
    ASSERT_TRUE(file_n.IsValid());
    file_tracker_.Register(entries[i].get(), key,
  ...
  EXPECT_EQ(static_cast<int>(msg.size()),
            borrow->Write(0, msg.data(), msg.size()));
  file_tracker_.Release(entries[0].get(), key,
                        SimpleFileTracker::SubFile::FILE_0);

  for (int i = 0; i < kEntries; ++i)
    file_tracker_.Close(entries[i].get(), key,
                        SimpleFileTracker::SubFile::FILE_0);
