...
void SimpleSynchronousEntry::WriteData(const EntryOperationData& in_entry_op,
  ...
  if (header_and_key_check_needed_[file_index] &&
    ...
    FileHandle file(this, SubFileForFileIndex(file_index));
    if (!CheckHeaderAndKey(file.get(), file_index)) {
      *out_result = net::ERR_FAILED;
      Doom();
      return;
    }
...
void SimpleSynchronousEntry::Close(
  ...
  for (int i = 0; i < kSimpleEntryFileCount; ++i) {
    ...
    // ### this is kinda thrashy, may be we should grab the entire thing at once
    // here, and release right before the close?
    if (header_and_key_check_needed_[i]) {
      FileHandle file(this, SubFileForFileIndex(i));
      if (!CheckHeaderAndKey(file.get(), i))
        Doom();
    }
...
SimpleSynchronousEntry::FileHandle::FileHandle(
  ...
  file_ =
      entry_->file_tracker_->Acquire(entry_, entry_->entry_file_key_, subfile_);
}

SimpleSynchronousEntry::FileHandle::~FileHandle() {
  entry_->file_tracker_->Release(entry_, entry_->entry_file_key_, subfile_);
}
...
base::File* SimpleSynchronousEntry::FileHandle::get() const {
  return file_;
}
