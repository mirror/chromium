...
void SimpleSynchronousEntry::Close(
  ...
  for (int i = 0; i < kSimpleEntryFileCount; ++i) {
    ...
    file_tracker_.Close(this, entry_file_key_, SubFileForFileIndex(i));
...
bool SimpleSynchronousEntry::MaybeOpenFile(
  ...
  *out_error = file.error_details();
  ...
  if (file.IsValid()) {
    file_tracker_.Register(this, path_, entry_file_key_,
                           SubFileForFileIndex(file_index), std::move(file));
    return true;
  }
  return false;
}
...
void SimpleSynchronousEntry::CloseFile(int index) {
  if (empty_file_omitted_[index]) {
    ...
    file_tracker_.Close(this, entry_file_key_, SubFileForFileIndex(index));
  }
...
bool SimpleSynchronousEntry::OpenSparseFileIfExists(
  ...
  int flags = File::FLAG_OPEN | File::FLAG_READ | File::FLAG_WRITE |
              File::FLAG_SHARE_DELETE;
  base::File sparse_file(filename, flags);
...
void SimpleSynchronousEntry::CloseSparseFile() {
  DCHECK(sparse_file_open());
  file_tracker_.Close(this, entry_file_key_,
                      SimpleFileTracker::SubFile::FILE_SPARSE);
  sparse_file_open_ = false;
}
...
base::File* SimpleSynchronousEntry::AcquireFile(
  ...
  if (state->ref_count == 1)
    state->file = file_tracker_.Acquire(this, entry_file_key_, subfile);
  return state->file;
}
...
void SimpleSynchronousEntry::ReleaseFile(SimpleFileTracker::SubFile subfile) {
  ...
  if (state->ref_count == 0) {
    file_tracker_.Release(this, entry_file_key_, subfile);
    state->file = nullptr;
  }
}
