...
bool SimpleSynchronousEntry::CheckHeaderAndKey(base::File* file,
                                               int file_index) {
  // TODO(morlovich): Frequently we are doing this at the same time as we read
  // from the beginning of an entry. It might improve performance to make a
  // single read(2) call rather than two separate reads. On the other hand, it
  // would mean an extra memory to memory copy. In the case where we are opening
  // an entry without a key, the kInitialHeaderRead setting means that we are
