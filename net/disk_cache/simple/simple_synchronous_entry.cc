...
SimpleSynchronousEntry::FileHandle::FileHandle(
    ...
    : entry_(entry), stream_(stream) {
  file_ = entry_->file_tracker_.Acquire(entry_->entry_file_key_, stream);
}

SimpleSynchronousEntry::FileHandle::~FileHandle() {
  // Note that we actually don't want a deep copy of entry_file_key_ here, since
  // it may get updated by DoomEntry, and we will want a new name here.
  entry_->file_tracker_.Release(entry_->entry_file_key_, stream_);
}
