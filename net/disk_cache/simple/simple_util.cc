...
std::string GetFilenameFromEntryFileKeyAndFileIndex(
  ...
  if (key.doom_generation == 0)
    ...
    return base::StringPrintf("todelete_%016" PRIx64 "_%1d_%u", key.entry_hash,
                              file_index, key.doom_generation);
}
...
std::string GetSparseFilenameFromEntryFileKey(
  ...
  if (key.doom_generation == 0)
    ...
    return base::StringPrintf("todelete_%016" PRIx64 "_s_%u", key.entry_hash,
                              key.doom_generation);
}
