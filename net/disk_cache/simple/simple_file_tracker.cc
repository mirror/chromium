...
// ### pass in file_limit
SimpleFileTracker::SimpleFileTracker() : file_limit_(1), open_files_(0) {}
...
SimpleFileTracker::FileHandle SimpleFileTracker::Acquire(
...

  {
    ...
    // Check to see if we have to reopen the file. That might push us over fd
    // limit.
...
void SimpleFileTracker::CloseFilesIfTooManyOpen(
    std::vector<std::unique_ptr<base::File>>* files_to_close) {
  std::list<TrackedFiles*>::reverse_iterator i = lru_.rbegin();
  while (open_files_ <= file_limit_ && i != lru_.rend()) {
    ...
    for (int j = 0; j < kSimpleEntryTotalFileCount; ++j) {
      if (tracked_files->state[j] == TrackedFiles::TF_REGISTERED &&
        ...
        tracked_files->files[j] = nullptr;  // Closes it.
        --open_files_;
      }
    }

    // Advance iterator before potentially removing this from lru_
    ++i;
