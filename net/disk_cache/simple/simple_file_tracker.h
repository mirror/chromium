...
class NET_EXPORT_PRIVATE SimpleFileTracker {
 public:
  ...
  SimpleFileTracker(int file_limit = 0);
  ~SimpleFileTracker();
 ...
 private:
  ...
  // returns it.
  uint32_t FindFreeDoomGeneration(
      const std::vector<std::unique_ptr<TrackedFiles>>& active);
  ...
  // Tries to reopen given file, updating |open_files_| if successful.
  void ReopenFile(TrackedFiles* owners_file, SubFile subfile);
