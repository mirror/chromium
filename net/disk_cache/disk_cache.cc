...
int CacheCreator::TryMakeCleanupContextAndRun() {
  // Before creating a cache Backend, a CleanupContext object is needed so
  // there is a place to keep track of outstanding I/O even after the backend
  // object itself is destroyed, so that further use of the directory
  // doesn't race with those outstanding disk I/O ops.

  // This method's purpose it to grab exlusive ownership of one for the
  // path, and then move on to Run(), which will take care of creating the
  // actual cache backend. It's possible that something else is currently
  // making use of the directory, so the attempt might not succeed, but in that
  // case TryMakeContext will justhave us run again at an opportune time to make
  // another attempt.

  // The resulting CleanupContext is stored into a scoped_refptr member so that
  // it's kept alive while |this| CacheCreator exists, so that in the case
  // Run() needs to retry Backend creation, the same CleanupContext is used for
  // both attempts, and |post_cleanup_callback_| gets called after the second
  // try, not first one.
