...
def CreateCert(name, signer=None, ocsp=False):
  ...
  if ocsp:
    cert.add_extensions([crypto.X509Extension("extendedKeyUsage", False,
                                              "OCSPSigning")])
  if signer:
...
def CreateSingleResponse(cert=CERT, status=0, next=None, revoke_time=None,
                         reason=None, extensions=[]):
  ...
  nameHash = hashlib.sha1(encoder.encode(issuer_tbs.getComponentByName(
      'subject'))).digest()
  keyHash = hashlib.sha1(encoder.encode(issuer_tbs.getComponentByName(
      'subjectPublicKeyInfo').getComponentByName('subjectPublicKey'))[4:]
  ).digest()
  ...
  if status == 0:
    ...
    ri.setComponentByName('revocationTime', useful.GeneralizedTime(
  ...
  sr.setComponentByName('thisUpdate', useful.GeneralizedTime(
...
def Create(signer=None, response_status=0, response_type='1.3.6.1.5.5.7.48.1.1',
           signature=None, version=1, responder=None, responses=None,
           extensions=None, certs=None, sigAlg='sha1'):
  ...
  tbs.setComponentByName('producedAt', useful.GeneralizedTime(
  ...
  basic.setComponentByName('signature', univ.BitString("'%s'H" % (
  ...
  rbytes.setComponentByName('responseType', univ.ObjectIdentifier(
      response_type))
...
Store('bad_ocsp_type', CA,              Create(
    response_type='1.3.6.1.5.5.7.48.1.2'))
...
Store('ocsp_sign_bad_indirect', CA,     Create(signer=CA_BADLINK,
                                               certs=[CA_BADLINK]))
...
Store('good_response', CA,       Create(responses=[
    CreateSingleResponse(CERT, 0)]))
Store('good_response_sha256', CA,       Create(responses=[
    CreateSingleResponse(CERT, 0)], sigAlg='sha256'))
Store('good_response_next_update', CA,  Create(responses=[
    CreateSingleResponse(CERT, 0, next=NEXT_DATE)]))
Store('revoke_response', CA,            Create(responses=[
    CreateSingleResponse(CERT, 1)]))
Store('revoke_response_reason', CA,     Create(responses=[
    CreateSingleResponse(CERT, 1, revoke_time=REVOKE_DATE, reason=1)]))
Store('unknown_response', CA,           Create(responses=[
    CreateSingleResponse(CERT, 2)]))
Store('multiple_response', CA,          Create(responses=[
    CreateSingleResponse(CERT, 0), CreateSingleResponse(CERT, 2)]))
Store('other_response', CA,             Create(responses=[
    CreateSingleResponse(JUNK_CERT, 0), CreateSingleResponse(JUNK_CERT, 1)]))
Store('has_single_extension', CA,       Create(responses=[
