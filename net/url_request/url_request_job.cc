  ...
  if (IsRedirectResponse(&new_location, &http_status_code)) {
    ...
    std::unique_ptr<net::RedirectInfo> redirect_info =
        RedirectInfo::ComputeRedirectInfo(
            http_status_code, request_->method(), request_->url(), new_location,
            CopyFragmentOnRedirect(new_location), request_->site_for_cookies(),
            request_->first_party_url_policy(), request_->referrer_policy(),
            request_->referrer(), request_->ssl_info().token_binding_negotiated,
