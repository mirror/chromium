...
bool HasParentDirLink(const std::string& data) {
  return data.find(kParentDirLink) != std::string::npos;
}

bool HasParentDirEntry(const std::string& data) {
...
TEST_F(URLRequestFileDirTest, DirectoryWithASingleFileSync) {
  ...
  EXPECT_FALSE(HasParentDirLink(data));
  ASSERT_EQ(2, GetEntryCount(data));
  EXPECT_TRUE(HasEntry(data, path));
  EXPECT_TRUE(HasParentDirEntry(data));
}
...
TEST_F(URLRequestFileDirTest, DirectoryWithASingleFileAsync) {
  ...
  EXPECT_TRUE(HasParentDirEntry(delegate.data_received()));
}
