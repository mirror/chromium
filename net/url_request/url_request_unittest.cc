...
TEST_F(HTTPSRequestTest, ResumeTest) {
...

  {
    ...
    //   lookup *** (random TLS 1.3 fake session ID)
    //   insert abc
    //   lookup abc
    //   insert xyz
    //
...
// that exactly two different sessions were inserted, with no extraneous
// lookups, other than those from the random TLS 1.3 fake session ID.
static void AssertTwoDistinctSessionsInserted(const string& session_info) {
  std::vector<std::string> lines = base::SplitString(
      session_info, "\n", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
  ASSERT_EQ(5u, lines.size()) << session_info;
