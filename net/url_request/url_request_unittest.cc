...
TEST_F(URLRequestTestHTTP, HeadersCallbacks) {
...

  {
    ...
    scoped_refptr<HttpResponseHeaders> raw_resp_headers;
    ...
    r->SetResponseHeadersCallback(base::Bind(
        [](scoped_refptr<HttpResponseHeaders>* left,
           scoped_refptr<HttpResponseHeaders> right) { *left = right; },
  ...
  }
  {
    ...
    r->SetResponseHeadersCallback(
        base::Bind([](scoped_refptr<HttpResponseHeaders>) {
          FAIL() << "Callback should not be called unless request is sent";
        }));
    r->Start();
    base::RunLoop().Run();
...
TEST_F(URLRequestTest, HeadersCallbacksConnectFailed) {
  ...
  r->SetResponseHeadersCallback(
      base::Bind([](scoped_refptr<net::HttpResponseHeaders>) {
        FAIL() << "Callback should not be called unless request is sent";
      }));
  r->Start();
  base::RunLoop().Run();
...
TEST_F(URLRequestTestHTTP, HeadersCallbacksAuthRetry) {
  ...
  using RespHeadersVector = std::vector<scoped_refptr<HttpResponseHeaders>>;
  ...
  r->SetRequestHeadersCallback(base::Bind(
      ...
      &raw_req_headers));
  r->SetResponseHeadersCallback(base::Bind(
      [](RespHeadersVector* vec, scoped_refptr<HttpResponseHeaders> headers) {
  ...
  base::RunLoop().Run();
  EXPECT_FALSE(r->is_pending());
  ASSERT_GE(raw_req_headers.size(), 2u);
  std::string value;
