...
TEST_F(HTTPSEarlyDataTest, TLSEarlyDataTest) {
...

  {
    TestDelegate d;
    std::unique_ptr<URLRequest> r(context_.CreateRequest(
        test_server_.GetURL("/echoheader?Early-Data"), DEFAULT_PRIORITY, &d,
        TRAFFIC_ANNOTATION_FOR_TESTS));
    r->Start();
    EXPECT_TRUE(r->is_pending());
...
TEST_F(HTTPSEarlyDataTest, TLSEarlyDataPOSTTest) {
...

  {
    ...
    // send POSTs over Early Data, and the handler should return "None".
    EXPECT_EQ(4, r->GetRawBodyBytes());
  }
}

// TLSEarlyDataTest tests that we handle 425 TOO EARLY correctly.
TEST_F(HTTPSEarlyDataTest, TLSEarlyDataTooEarlyTest) {
...

  {
    TestDelegate d;
    std::unique_ptr<URLRequest> r(context_.CreateRequest(
        test_server_.GetURL("/echo?status=425"), DEFAULT_PRIORITY, &d,
        TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(HTTPSEarlyDataTest, TLSEarlyDataRejectTest) {
...

  {
    ...
    EXPECT_TRUE(
        test_server_.GetCertificate()->Equals(r->ssl_info().cert.get()));
    EXPECT_FALSE(old_cert->Equals(r->ssl_info().cert.get()));
