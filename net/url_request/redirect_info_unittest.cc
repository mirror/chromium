...
TEST(RedirectInfoTest, MethodForRedirect) {
  ...
  const GURL original_url = GURL("https://foo.test/original");
  const GURL original_site_for_cookies = GURL("https://foo.test/");
  const URLRequest::FirstPartyURLPolicy original_first_party_url_policy =
      net::URLRequest::NEVER_CHANGE_FIRST_PARTY_URL;
  const URLRequest::ReferrerPolicy original_referrer_policy =
      net::URLRequest::NEVER_CLEAR_REFERRER;
  const std::string original_referrer = "";
  const GURL new_location = GURL("https://foo.test/redirected");
  const bool token_binding_negotiated = false;
  const bool copy_fragment = true;
  ...
  for (const auto& test : kTests) {
    ...
    RedirectInfo redirect_info = RedirectInfo::ComputeRedirectInfo(
        test.original_method, original_url, original_site_for_cookies,
        original_first_party_url_policy, original_referrer_policy,
        original_referrer, nullptr /* response_headers */,
        test.http_status_code, new_location, token_binding_negotiated,
        copy_fragment);

    EXPECT_EQ(test.expected_new_method, redirect_info.new_method);
    EXPECT_EQ(test.http_status_code, redirect_info.status_code);
    EXPECT_EQ(new_location, redirect_info.new_url);
  }
}
...
TEST(RedirectInfoTest, CopyFragment) {
  ...
  const std::string original_method = "GET";
  const GURL original_site_for_cookies = GURL("https://foo.test/");
  const URLRequest::FirstPartyURLPolicy original_first_party_url_policy =
      net::URLRequest::NEVER_CHANGE_FIRST_PARTY_URL;
  const URLRequest::ReferrerPolicy original_referrer_policy =
      net::URLRequest::NEVER_CLEAR_REFERRER;
  const std::string original_referrer = "";
  const int http_status_code = 301;
  const bool token_binding_negotiated = false;
  ...
  for (const auto& test : kTests) {
    ...
    RedirectInfo redirect_info = RedirectInfo::ComputeRedirectInfo(
        original_method, GURL(test.original_url), original_site_for_cookies,
        original_first_party_url_policy, original_referrer_policy,
        original_referrer, nullptr /* response_headers */, http_status_code,
        GURL(test.new_location), token_binding_negotiated, test.copy_fragment);
...
TEST(RedirectInfoTest, FirstPartyURLPolicy) {
  ...
  const std::string original_method = "GET";
  const GURL original_url = GURL("https://foo.test/");
  const GURL original_site_for_cookies = GURL("https://foo.test/");
  const URLRequest::ReferrerPolicy original_referrer_policy =
      net::URLRequest::NEVER_CLEAR_REFERRER;
  const std::string original_referrer = "";
  ...
  for (const auto& test : kTests) {
    ...
    RedirectInfo redirect_info = RedirectInfo::ComputeRedirectInfo(
        original_method, original_url, original_site_for_cookies,
        test.original_first_party_url_policy, original_referrer_policy,
        original_referrer, nullptr /* response_headers */, http_status_code,
        new_location, token_binding_negotiated, copy_fragment);
...
TEST(RedirectInfoTest, ReferrerPolicy) {
  ...
  for (const auto& test : kTests) {
    ...
    RedirectInfo redirect_info = RedirectInfo::ComputeRedirectInfo(
        original_method, original_url, original_site_for_cookies,
        original_first_party_url_policy, test.original_referrer_policy,
        test.original_referrer, response_headers.get(),
        response_headers->response_code(), new_location,
        token_binding_negotiated, copy_fragment);
...
TEST(RedirectInfoTest, ReferredTokenBinding) {
  ...
  for (const auto& test : kTests) {
    ...
    RedirectInfo redirect_info = RedirectInfo::ComputeRedirectInfo(
        original_method, original_url, original_site_for_cookies,
        original_first_party_url_policy, original_referrer_policy,
        original_referrer, response_headers.get(),
        response_headers->response_code(), new_location,
        test.token_binding_negotiated, copy_fragment);
