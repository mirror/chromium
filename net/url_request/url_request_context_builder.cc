...
class ContainerURLRequestContext final : public URLRequestContext {
 public:
  ...
  ~ContainerURLRequestContext() override {
    ...
    // Shut down the ProxyResolutionService, as it may have pending URLRequests using this
    ...
    // cancels the ProxyResolutionService's URLRequests.
...
std::unique_ptr<URLRequestContext> URLRequestContextBuilder::Build() {
  ...
  if (!proxy_service_) {
    ...
    // ProxyResolutionService::CreateSystemProxyConfigService()'s signature doesn't suck.
    if (!proxy_config_service_) {
      proxy_config_service_ = ProxyResolutionService::CreateSystemProxyConfigService(
...
std::unique_ptr<ProxyResolutionService> URLRequestContextBuilder::CreateProxyService(
