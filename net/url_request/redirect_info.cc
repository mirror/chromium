...
std::unique_ptr<net::RedirectInfo> RedirectInfo::ComputeRedirectInfo(
    int http_status_code,
    ...
    const HttpResponseHeaders* headers) {
  DCHECK(!headers || headers->response_code() == http_status_code);
  ...
  redirect_info->new_method =
      ComputeMethodForRedirect(method, http_status_code);
  ...
  if (original_url.is_valid() && original_url.has_ref() &&
      !location.has_ref() && copy_fragment) {
  ...
  redirect_info->new_referrer_policy =
      ProcessReferrerPolicyHeaderOnRedirect(original_referrer_policy, headers);

  // Alter the referrer if redirecting cross-origin (especially HTTP->HTTPS).
  redirect_info->new_referrer = URLRequestJob::ComputeReferrerForPolicy(
                                    redirect_info->new_referrer_policy,
                                    GURL(referrer), redirect_info->new_url)
                                    .spec();

  if (headers) {
    std::string include_referer;
    headers->GetNormalizedHeader("include-referred-token-binding-id",
                                 &include_referer);
