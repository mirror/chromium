...
TEST_F(URLRequestHttpJobWithMockSocketsTest,
  ...
  for (int priority = 0; priority <= 5; priority++) {
    for (int request_index = 0; request_index <= priority; ++request_index) {
      ...
      TestDelegate delegate;
      std::unique_ptr<URLRequest> request = context_->CreateRequest(
          GURL("http://www.example.com/"), (net::RequestPriority)priority,
          &delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
  ...
  histograms.ExpectTotalCount("Net.HttpJob.TotalTimeSuccess.Priority0", 1);
  histograms.ExpectTotalCount("Net.HttpJob.TotalTimeSuccess.Priority1", 2);
  histograms.ExpectTotalCount("Net.HttpJob.TotalTimeSuccess.Priority2", 3);
  histograms.ExpectTotalCount("Net.HttpJob.TotalTimeSuccess.Priority3", 4);
  histograms.ExpectTotalCount("Net.HttpJob.TotalTimeSuccess.Priority4", 5);
  histograms.ExpectTotalCount("Net.HttpJob.TotalTimeSuccess.Priority5", 6);
}
