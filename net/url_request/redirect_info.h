...
struct NET_EXPORT RedirectInfo {
  ...
  static RedirectInfo ComputeRedirectInfo(
      ...
      // (ex: URLRequestFileJob). We have |http_status_code| and |new_location|
      // as separate arguments for such case.
      ...
      // We copy the URL fragment of the original URL to the new URL by default.
      // Set false only when the network delegate has set the desired redirect
      // URL (with or without fragment), so it must not be changed any more.
