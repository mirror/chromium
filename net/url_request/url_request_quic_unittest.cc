...
TEST_F(URLRequestQuicTest, RequestHeadersCallback) {
  ...
  std::string value;
  EXPECT_TRUE(raw_headers.FindHeader("x-foo", &value));
  EXPECT_EQ("bar", value);
  EXPECT_TRUE(raw_headers.FindHeader("accept-encoding", &value));
  EXPECT_EQ("gzip, deflate", value);
  EXPECT_TRUE(raw_headers.FindHeader(":path", &value));
  EXPECT_EQ("/hello.txt", value);
  EXPECT_TRUE(raw_headers.FindHeader(":authority", &value));
