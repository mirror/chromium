...
class SQLitePersistentCookieStorePerfTest : public testing::Test {
 public:
  const int kNumDomains = 300;
  const int kCookiesPerDomain = 50;

  // Prime number noticeably larger than kNumDomains or kCookiesPerDomain;
  const int kRandomSeed = 13093;
...
TEST_F(SQLitePersistentCookieStorePerfTest, TestDeletePerformance) {
  ...
  // Figure out the kNumToDelete victim cookies.
  std::vector<CanonicalCookie> cookies;
  ...
  base::PerfTimeLogger timer("Delete (and add back) lots of cookies");
  ...
  timer.Done();
  PrintPerfTest("time", elapsed_time.InMilliseconds(), "ms");
}
...
TEST_F(SQLitePersistentCookieStorePerfTest, TestUpdatePerformance) {
  ...
  base::PerfTimeLogger timer("Update access time for lots of cookies");
  base::Time start_time(base::Time::Now());
  for (int i = 0; i < kNumIterations; ++i) {
    // Update and flush
    for (int cookie = 0; cookie < kNumToDelete; ++cookie) {
