...
TEST_F(SQLitePersistentCookieStoreTest, UpgradeToSpecUnique) {
  const int kUpgradeFromVersionNumber = 9;

  // Open db
  sql::Connection connection;
  ASSERT_TRUE(connection.Open(temp_dir_.GetPath().Append(kCookieFilename)));
  sql::MetaTable meta_table;
  ASSERT_TRUE(meta_table.Init(&connection, kUpgradeFromVersionNumber, 3));
  ...
  ASSERT_TRUE(connection.Execute(stmt.c_str()));
  ASSERT_TRUE(connection.Execute("CREATE INDEX domain ON cookies(host_key)"));
  ASSERT_TRUE(
      connection.Execute("CREATE INDEX is_transient ON cookies(persistent) "
                         "where persistent != 1"));

  // Create a selection of cookies.  Intentionally have cookies
  // with duplicate (name,domain,path) to make sure upgrade handles them
  // properly.
  {
    sql::Statement add_smt(connection.GetCachedStatement(
        SQL_FROM_HERE,
        "INSERT INTO cookies (creation_utc, host_key, name, value, "
        "encrypted_value, path, expires_utc, secure, httponly, firstpartyonly, "
        "last_access_utc, has_expires, persistent, priority) "
        "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)"));
    ASSERT_TRUE(add_smt.is_valid());
  ...
  EXPECT_EQ(6u, read_in_cookies.size());
