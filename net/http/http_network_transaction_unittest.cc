...
class CaptureGroupNameSocketPool : public ParentPool {
 public:
  ...
  const void* last_socket_params() const { return last_socket_params_; }
  ...
  int RequestSocket(const std::string& group_name,
    ...
    last_socket_params_ = socket_params;
 ...
 private:
  ...
  const void* last_socket_params_ = nullptr;
};
...
TEST_F(HttpNetworkTransactionTest, GroupNameForDirectConnections) {
  ...
  for (size_t i = 0; i < arraysize(tests); ++i) {
    ...
    EXPECT_EQ(tests[i].ssl, (ssl_conn_pool->last_socket_params() != nullptr));
    // When SSL proxy is not in use, socket must be requested from
    // |transport_conn_pool|.
    EXPECT_EQ(!tests[i].ssl,
              (transport_conn_pool->last_socket_params() != nullptr));
  }
}
