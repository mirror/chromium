...
void HttpCache::ProcessDoneHeadersQueue(ActiveEntry* entry) {
  ...
  if (IsWritingInProgress(entry)) {
    if (is_partial || transaction->mode() == Transaction::READ) {
      ...
      // throughout. May be at this point the ordering is not important but that
      // would be optimizing a rare scenario where write mode transactions are
      // insterspersed with read-only transactions.
      return;
    }
