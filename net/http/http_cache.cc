...
void HttpCache::DoneWithEntry(ActiveEntry* entry,
  ...
  bool should_restart =
      !did_finish && HasDependentTransactions(entry, transaction);
  ...
  if (transaction == entry->headers_transaction) {
    ...
    // failure.
    entry->headers_transaction = nullptr;
    ImpactQueuedTransactions(entry, !should_restart);
    return;
  }
  ...
  if (entry->writers->HasTransaction(transaction)) {
    ...
    if (entry->writers->IsEmpty())
      WritersDoneWritingToEntry(entry, !should_restart, TransactionSet());
    return;
  }
