...
bool HttpCache::ActiveEntry::HasNoTransactionsAndOutstandingWritersState() {
  return !writers && readers.empty() && add_to_entry_queue.empty() &&
...
void HttpCache::FinalizeDoomedEntry(ActiveEntry* entry) {
  DCHECK(entry->doomed);
  DCHECK(entry->HasNoTransactionsAndOutstandingWritersState());
...
void HttpCache::DoomEntryValidationNoMatch(ActiveEntry* entry) {
  ...
  entry->headers_transaction = nullptr;
  if (entry->HasNoTransactionsAndOutstandingWritersState() &&
      !entry->will_process_queued_transactions) {
...
void HttpCache::AddTransactionToWriters(ActiveEntry* entry,
                                        Transaction* transaction) {
  if (!entry->writers) {
    entry->writers.reset(new Writers(this, entry));
  }
