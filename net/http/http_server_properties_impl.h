...
class NET_EXPORT HttpServerPropertiesImpl
 ...
 private:
  ...
  typedef std::map<url::SchemeHostPort, url::SchemeHostPort> CanonicalAltSvcMap;
  typedef std::map<QuicServerId, QuicServerId> CanonicalServerInfoMap;
  ...
  CanonicalAltSvcMap::const_iterator GetAltSvcCanonicalHost(
  ...
  // Update |canonical_server_info_map_| with the new |origin| (canonical host).
  // The |origin| should have the corresponding server info associated with it
  // in |quic_server_info_map_|. If |canonical_server_info_map_| doesn't
  // have an entry with |origin|, the method will add one.
  void UpdateHostInCanonicalServerInfoMap(const QuicServerId& origin);
