...
void HttpStreamFactoryImpl::JobController::OnStreamReady(
  ...
  // The request has already been served by pooling to another SpdySession, but
  // |job| could not have been deleted, because it was waiting for async
  // callback from InitSocket.
  if (!request_ || request_->completed()) {
    DCHECK_NE(bound_job_, job);
    // Note that it is possible that
    // |request_ != nullptr && request_->completed() && !job_bound_|,
    // in which case |job| is not orphaned, but should still be deleted.
