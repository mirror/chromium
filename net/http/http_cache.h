...
class NET_EXPORT HttpCache : public HttpTransactionFactory {
 public:
  ...
  // captures that aspect. This is also used to log metrics so should be
  // consistent with the values in enums.xml.
  enum ParallelWritingPattern {
    // Used as the default value till the transaction reaches the response body
    // phase.
    PARALLEL_WRITING_NONE,
    // The transaction creates a writers object.
    PARALLEL_WRITING_CREATE,
    // The transaction joins existing writers.
    PARALLEL_WRITING_JOIN,
    // The transaction cannot join existing writers since either it itself or
    // the
    // existing writers is serving a range request.
    PARALLEL_WRITING_NOT_JOIN_RANGE,
    // The transaction cannot join existing writers since either it itself or
    // the
    // existing writers is serving a non GET request.
    ...
    // The transaction cannot join existing writers since writers had a cache
    // write failure.
    PARALLEL_WRITING_NOT_JOIN_CACHE_WRITE_FAIL,
    // The transaction cannot join existing writers since writers have stopped
    // cachine.
