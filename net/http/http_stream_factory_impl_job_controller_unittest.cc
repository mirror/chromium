...
// the second proxy server. The result of the last job (ERR_CONNECTION_FAILED)
...
TEST_F(JobControllerReconsiderProxyAfterErrorTest,
  ...
  tcp_data_2.set_connect_data(MockConnect(SYNCHRONOUS, ERR_CONNECTION_FAILED));
  ...
  EXPECT_CALL(request_delegate_, OnStreamFailed(ERR_CONNECTION_FAILED, _, _))
      .Times(1);
...
TEST_F(JobControllerReconsiderProxyAfterErrorTest,
  ...
  quic_data_1.AddWrite(SYNCHRONOUS, OK);
  ...
  quic_data_2.AddWrite(SYNCHRONOUS, OK);
