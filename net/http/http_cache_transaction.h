...
class NET_EXPORT_PRIVATE HttpCache::Transaction : public HttpTransaction {
 public:
  ...
  // Fills in pointers/copies of members in WritersTransactionContext. The
  // pointers are only valid while the Transaction is alive.
  std::unique_ptr<WritersTransactionContext> GetWritersTransactionContext();
 ...
 private:
  ...
  // entry_->writers->MayBeSetNetworkReadOnly which implies the response will
  // not be subsequently written to the cache else
  // HeadersPhaseContinueWithoutEntry will be invoked.
  // If |keep_entry| is false, entry will also be doomed.
  void ContinueInNetworkReadOnlyMode(bool keep_entry);

  // Called when the transaction doesn't have to write to the cache anymore
  // during the headers phase.
  // For a headers phase transaction, remove the transaction from entry_ and
  // impact other queued transactions via DoneWritingToEntry.
  // If |keep_entry| is false, entry will also be doomed.
  void HeadersPhaseContinueWithoutEntry(bool keep_entry);

  // Called when the request is complete and |this| needs to be removed fromi
  // entry.
