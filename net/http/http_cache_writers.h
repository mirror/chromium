...
class NET_EXPORT_PRIVATE HttpCache::Writers {
 public:
  ...
  void AddTransaction(Transaction* transaction,
                      ...
                      TransactionWritersSharedMemory& info);
  ...
  void ResetPartialNetworkTransaction(Transaction* transaction);
  ...
  // attempt to truncate the entry will be done. Works when only |transaction|
  // is present in writers, else it will be a no-op.
  void MayBeSetNetworkReadOnly(Transaction* transaction, bool success);
 ...
 private:
  ...
  using TransactionMap = std::map<Transaction*, TransactionWritersSharedMemory>;
