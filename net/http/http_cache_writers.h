...
class NET_EXPORT_PRIVATE HttpCache::Writers {
 public:
  ...
  // else an attempt to truncate the entry will be done. May only be called if
  // the calling transaction is the only member of writers.
 ...
 private:
  ...
  enum class State {
    ...
    // read/cache write. We will then process the truncation after the ongoing
    // operation completes.
  ...
  // Called to reset network transaction when all transaction references are
  // removed from |this|.
  void ResetNetworkTransactionInternal();
