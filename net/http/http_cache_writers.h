...
class NET_EXPORT_PRIVATE HttpCache::Writers {
 public:
  ...
  // DoneWithEntry() during its destruction. This will also ensure any
  ...
  void TransactionDoneWithEntry(Transaction* transaction, bool should_truncate);
  ...
  // written to the cache. The entry will be doomed if |keep_entry| is false,
  // else attempt to truncate the entry will be done. May only be called if the
  // calling transaction is the only member of writers.
  void SetNetworkReadOnly(bool keep_entry);
 ...
 private:
  ...
  // Returns true if either the entry is marked as truncated or the entry is
  // already completely written and false if the entry cannot be resumed later
  // so no need to mark truncated.
  void TruncateEntry();
