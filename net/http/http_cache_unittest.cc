...
TEST(HttpCache, SetTruncatedFlag) {
  ...
  // Verify that the entry is not marked as incomplete while Read is in
  // progress since Writers will complete the ongoing Read.
  VerifyTruncatedFlag(&cache, kSimpleGET_Transaction.url, false, 0);

  // After Read is complete, entry should be marked as incomplete.
  base::RunLoop().RunUntilIdle();
