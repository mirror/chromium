...
TEST(HttpCache, SimpleGET_CacheSignal_Failure) {
  for (bool use_memory_entry_data : {false, true}) {
    ...
    EXPECT_EQ(!use_memory_entry_data, response_info->was_cached);
    EXPECT_EQ(2, cache.network_layer()->transaction_count());
...
TEST(HttpCache, GET_NoConditionalization) {
  for (bool use_memory_entry_data : {false, true}) {
    ...
    // ### before commit: figure out the way to actually test that.
    // The open results in a useless entry since
    // &RangeTransactionServer::RangeHandler returns headers without an ETag
    // or a non-range request (not the things we set above).
    RunTransactionTest(cache.http_cache(), transaction);
    EXPECT_EQ(3, cache.network_layer()->transaction_count());
    if (use_memory_entry_data) {
      EXPECT_EQ(1, cache.disk_cache()->open_count());
