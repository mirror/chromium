...
bool HttpCache::Writers::StopCaching(Transaction* transaction) {
  ...
  if (all_writers_.size() == 1) {
    ...
    cache_->WritersDoneWritingToEntry(entry_, false /* success */, nullptr);
    return true;
  }
  return false;
}
...
void HttpCache::Writers::DoneReading(Transaction* transaction) {
  ...
  // from |this|.
  cache_->WritersDoneWritingToEntry(entry_, true /* success */, transaction);
}
...
void HttpCache::Writers::AddTransaction(Transaction* transaction,
  ...
  auto transactionInfo = std::make_pair(transaction, info);
  all_writers_.insert(transactionInfo);
...
void HttpCache::Writers::ResetPartialNetworkTransaction(
  ...
  DCHECK_EQ(transaction, it->first);
...
void HttpCache::Writers::MayBeSetNetworkReadOnly(Transaction* transaction,
                                                 bool success) {
  DCHECK(all_writers_.count(transaction));
  ...
  cache_->WritersDoneWritingToEntry(entry_, success, nullptr);
}
...
void HttpCache::Writers::RemoveTransaction(Transaction* transaction) {
  ...
  // Since the transaction can no longer access the netwoek transaction, save
...
bool HttpCache::Writers::TruncateEntry() {
    ...
    if (all_writers_.size() == 1u) {
      ...
      if (it->second.partial) {
        if (!*(it->second.truncated))
          return true;
        // Since its possible that network transaction may have been reset for a
        // partial transaction, get the response info from the transaction
        // itself.
        response_info_truncation_ = it->second.response_info;
      }
    }
  }
...
void HttpCache::Writers::OnNetworkReadFailure(int result) {
  ...
  cache_callback_ =
      base::BindOnce(&HttpCache::WritersDoneWritingToEntry,
                     cache_->GetWeakPtr(), entry_, false, active_transaction_);
...
void HttpCache::Writers::OnDataReceived(int result) {
  ...
  if (result == 0) {
    ...
    cache_callback_ =
        base::BindOnce(&HttpCache::WritersDoneWritingToEntry,
                       cache_->GetWeakPtr(), entry_, true, active_transaction_);
  }
