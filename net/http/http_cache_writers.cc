...
void HttpCache::Writers::RemoveTransaction(Transaction* transaction,
  ...
  bool in_idle_state = (next_state_ == State::NONE);
  bool initiated_truncate = false;
  if (!success) {
    DCHECK_NE(State::CACHE_WRITE_TRUNCATED_RESPONSE, next_state_);
    initiated_truncate = InitiateTruncateEntry();
    if (in_idle_state && initiated_truncate) {
      DoLoop(OK);
      return;
    }
  }

  if (!initiated_truncate)
...
void HttpCache::Writers::EraseTransaction(Transaction* transaction,
  ...
  if (all_writers_.empty() && next_state_ == State::NONE)
    ResetNetworkTransactionInternal();
  else
    UpdatePriority();
...
LoadState HttpCache::Writers::GetLoadState() {
  if (network_transaction_)
...
void HttpCache::Writers::OnNetworkReadFailure(int result) {
  ...
  if (!InitiateTruncateEntry()) {
    ...
    cache_callback_ =
        base::BindOnce(&HttpCache::CurrentWritersDoneWritingToEntry,
...
void HttpCache::Writers::ResetNetworkTransactionInternal() {
  // This needs to be called to handle the edge case where even before Read is
  // invoked all transactions are removed. In that case the network_transaction_
  // will still have a valid request info and so it should be destroyed before
  // its consumer is destroyed (request info is a raw pointer owned by its
  // consumer).
  DCHECK(all_writers_.empty());
  network_transaction_.reset();
}
