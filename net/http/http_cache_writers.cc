...
void HttpCache::Writers::RemoveTransaction(Transaction* transaction,
  ...
  bool truncate_initiated = false;
  if (!success) {
    DCHECK_NE(State::CACHE_WRITE_TRUNCATED_RESPONSE, next_state_);
    truncate_initiated = InitiateTruncateEntry();
  }

  if (in_idle_state && truncate_initiated)
    DoLoop(OK);
  else if (!truncate_initiated)
...
void HttpCache::Writers::OnNetworkReadFailure(int result) {
  ...
  bool truncate_initiated = InitiateTruncateEntry();

  if (!truncate_initiated) {
    cache_callback_ =
