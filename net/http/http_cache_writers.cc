...
void HttpCache::Writers::RemoveTransaction(Transaction* transaction,
  ...
  if (next_state_ == State::CACHE_WRITE_TRUNCATED_RESPONSE)
    DoLoop(OK);
  else if (next_state_ != State::CACHE_WRITE_DATA_COMPLETE_PRE_TRUNCATE)
...
void HttpCache::Writers::EraseTransaction(Transaction* transaction) {
  ...
  transaction->WriterAboutToBeRemovedFromEntry(OK);
...
bool HttpCache::Writers::CanAddWriters() {
  if (next_state_ == State::CACHE_WRITE_DATA_COMPLETE_PRE_TRUNCATE ||
...
void HttpCache::Writers::TruncateEntry() {
  ...
  if (next_state_ == State::CACHE_WRITE_DATA_COMPLETE)
    next_state_ = State::CACHE_WRITE_DATA_COMPLETE_PRE_TRUNCATE;
  else
  ...
  do {
    ...
    switch (state) {
      ...
      case State::CACHE_WRITE_DATA_COMPLETE:
        ...
        break;
      case State::CACHE_WRITE_DATA_COMPLETE_PRE_TRUNCATE:
        rv = DoCacheWriteDataCompletePreTruncate(rv);
        break;
...
int HttpCache::Writers::DoCacheWriteDataCompletePreTruncate(int result) {
