...
void HttpServerPropertiesImpl::SetQuicServerInfoMap(
  ...
  for (QuicServerInfoMap::reverse_iterator it = quic_server_info_map_.rbegin();
    ...
    UpdateHostInCanonicalServerInfoMap(it->first);
  }
}
...
HttpServerPropertiesImpl::GetAlternativeServiceInfos(
  ...
  CanonicalAltSvcMap::const_iterator canonical = GetAltSvcCanonicalHost(origin);
...
bool HttpServerPropertiesImpl::SetQuicServerInfo(
  ...
  UpdateHostInCanonicalServerInfoMap(server_id);
  return changed;
}
...
HttpServerPropertiesImpl::GetCanonicalServerInfoHost(
  ...
  QuicServerId canonical_server(*canonical_suffix, server.port(),
                                server.privacy_mode());
  return canonical_server_info_map_.find(canonical_server);
}
...
void HttpServerPropertiesImpl::UpdateHostInCanonicalServerInfoMap(
    const QuicServerId& origin) {
  const std::string* suffix = GetCanonicalSuffix(origin.host());
  if (suffix) {
    QuicServerId canonical_id(*suffix, origin.port(), origin.privacy_mode());
    canonical_server_info_map_[canonical_id] = origin;
  }
}
...
HttpServerPropertiesImpl::GetAlternateProtocolIterator(
  ...
  CanonicalAltSvcMap::const_iterator canonical = GetAltSvcCanonicalHost(server);
  if (canonical == canonical_alt_svc_map_.end()) {
...
HttpServerPropertiesImpl::GetAltSvcCanonicalHost(
