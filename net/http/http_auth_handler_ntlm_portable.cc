...
uint64_t GetMSTime() {
  const uint64_t offset = 11644473600;
  const uint64_t multiplier = 10 * 1000 * 1000;

  return (static_cast<uint64_t>(time(NULL)) + offset) * multiplier;
}
...
HttpAuthHandlerNTLM::HttpAuthHandlerNTLM()
    : ntlm_client_(ntlm::NtlmVersion::kNtlmV1, ntlm::kDefaultFeatures) {}
