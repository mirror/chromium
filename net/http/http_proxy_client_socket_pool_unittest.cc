  ...
  HttpProxyClientSocketPoolTest()
      : transport_socket_pool_(kMaxSockets,
        ...
        pool_(std::make_unique<HttpProxyClientSocketPool>(
            ...
            &ssl_socket_pool_,
            &estimator_,
            GetParam() != HTTP /* is_secure_connection */,
            nullptr)) {
...
TEST_P(HttpProxyClientSocketPoolTest, ProxyPoolTimeoutWithExperiment) {
  int ssl_http_rtt_multiplier = 4;
  int non_ssl_http_rtt_multiplier = 3;
  base::TimeDelta min_timeout = base::TimeDelta::FromSeconds(8);
  base::TimeDelta max_timeout = base::TimeDelta::FromSeconds(20);

  InitAdaptiveTimeoutFieldTrialWithParams(false, ssl_http_rtt_multiplier,
                                          non_ssl_http_rtt_multiplier,
                                          min_timeout, max_timeout);
  EXPECT_LE(base::TimeDelta(), pool_->ConnectionTimeout());

  const int multiplier = GetParam() == HTTP ? non_ssl_http_rtt_multiplier
                                            : ssl_http_rtt_multiplier;

  // Timeout should be |multiplier| times the HTTP RTT estimate.
  ...
  base::TimeDelta expected_connection_timeout = multiplier * rtt_estimate;
  if (expected_connection_timeout > max_timeout) {
    expected_connection_timeout = max_timeout;
  }
  ...
  estimator()->set_start_time_null_http_rtt(rtt_estimate);
  expected_connection_timeout = multiplier * rtt_estimate;
  if (expected_connection_timeout > max_timeout) {
    expected_connection_timeout = max_timeout;
  }
  EXPECT_EQ(expected_connection_timeout, pool_->ConnectionTimeout());
...
TEST_P(HttpProxyClientSocketPoolTest,
  ...
  int ssl_http_rtt_multiplier = 3;
  int non_ssl_http_rtt_multiplier = 1;
  ...
  estimator()->set_start_time_null_transport_rtt(rtt_estimate);
  estimator()->set_start_time_null_http_rtt(rtt_estimate);
  EXPECT_EQ(multiplier * rtt_estimate, pool_->ConnectionTimeout());
