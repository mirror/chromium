...
HttpCache::Transaction::~Transaction() {
  ...
  if (cache_) {
    if (entry_) {
      DoneWithEntry(false /* did_finish */);
...
void HttpCache::Transaction::AboutToBeRemovedFromWriters(int result) {
  RecordHistograms();
  ...
  if (result < 0 && (next_state_ == STATE_NONE ||
...
void HttpCache::Transaction::ContinueInNetworkReadOnlyMode(bool success) {
  ...
  if (InWriters()) {
    mode_ = NONE;
    entry_->writers->SetNetworkReadOnly(success /* keep_entry */);
  } else if (entry_) {
    DoneWithEntry(success /* did_finish */);
  }
}
