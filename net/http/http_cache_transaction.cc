...
HttpCache::Transaction::~Transaction() {
  ...
  if (cache_) {
    if (entry_) {
      cache_->DoneWithEntry(entry_, this, false /* did_finish */,
                            partial_ != nullptr);
...
void HttpCache::Transaction::ContinueInNetworkReadOnlyMode(bool keep_entry) {
  ...
  if (InWriters()) {
    ...
    DoneWithEntry(keep_entry /* did_finish */);
  }
}

void HttpCache::Transaction::DoneWithEntry(bool did_finish) {
  if (!entry_)
    return;

  RecordHistograms();

  cache_->DoneWithEntry(entry_, this, did_finish, partial_ != nullptr);
  entry_ = nullptr;
...
int HttpCache::Transaction::OnCacheReadError(int result, bool restart) {
  ...
  if (restart) {
    DCHECK(!reading_);
    DCHECK(!network_trans_.get());
    cache_->DoneWithEntry(entry_, this, true /* did_finish */,
                          partial_ != nullptr);
    entry_ = NULL;
    is_sparse_ = false;
    partial_.reset();
