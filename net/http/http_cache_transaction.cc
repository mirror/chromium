...
int HttpCache::Transaction::TransitionToReadingState() {
  ...
  // If entry_ is present, the transaction is either a writer or reader. If it's
  // a writer and it is partial then it may need to read from the cache or from
  // the network based on whether network transaction is present or not. If it's
...
void HttpCache::Transaction::StopCaching() {
  ...
  // from the net.
  if (cache_.get() && (mode_ & WRITE) && SelfInEntryWriters() &&
      entry_->writers->network_transaction() && !is_sparse_ &&
      !range_requested_) {
    if (entry_->writers->StopCaching(this))
      // Any dependent transactions would be restarted and
...
std::unique_ptr<WritersTransactionContext>
HttpCache::Transaction::GetWritersTransactionContext() {
  return base::MakeUnique<WritersTransactionContext>(
      &network_transaction_info_, partial_.get(), &truncated_, response_);
}
...
int HttpCache::Transaction::DoCacheReadResponseComplete(int result) {
  ...
  if (!cache_->IsWritingInProgress(entry_)) {
    ...
    if ((truncated_ || response_.headers->response_code() == 206) &&
      ...
      HeadersPhaseContinueWithoutEntryDoomEntry(false);
      TransitionToState(STATE_SEND_REQUEST);
      return OK;
    }
  }
...
int HttpCache::Transaction::DoSuccessfulSendRequest() {
  ...
  if (mode_ == WRITE && (method_ == "PUT" || method_ == "DELETE")) {
    ...
    HeadersPhaseContinueWithoutEntryDoomEntry(true);
  }
...
int HttpCache::Transaction::DoFinishHeadersComplete(int rv) {
  ...
  if (network_trans_ && SelfInEntryWriters()) {
...
void HttpCache::Transaction::IgnoreRangeRequest() {
  ...
  if (mode_ & WRITE) {
    RecordHistograms();
    ContinueInNetworkReadOnlyMode(mode_ != WRITE);
...
void HttpCache::Transaction::ContinueInNetworkReadOnlyMode(bool success) {
  ...
  if (SelfInEntryWriters()) {
    ...
    HeadersPhaseContinueWithoutEntryDoomEntry(success);
  }
}

void HttpCache::Transaction::HeadersPhaseContinueWithoutEntryDoomEntry(
    bool success) {
  if (entry_) {
...
bool HttpCache::Transaction::SelfInEntryWriters() {
