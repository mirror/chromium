...
void HttpCache::Transaction::SetParallelWritingPatternForMetrics(
    HttpCache::ParallelWritingPattern pattern) {
  // Its possible a transaction could not join existing writers and then creates
  // a new writers. In that case the original reason for not being able to join
  // writers should be the one logged.
