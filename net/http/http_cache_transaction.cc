...
int HttpCache::Transaction::DoFinishHeaders(int result) {
  ...
  // anything now. Note that its possible that the consumer invokes a Read
  // because user ends up cancelling the auth request and that would then be a
  // special case where the headers_transaction will be reading body from the
  // network.
  if (auth_response_.headers.get())
    return OK;
