...
bool HttpCache::Transaction::GetFullRequestHeaders(
  ...
  const HttpTransaction* transaction = network_transaction_for_info_retrieval();
  if (transaction) {
...
HttpCache::Transaction::network_transaction_for_info_retrieval() const {
  if (network_trans_)
    return network_trans_.get();
  if (InWriters() && moved_owned_network_transaction_to_writers_)
    return entry_->writers->network_transaction();
  return nullptr;
}
