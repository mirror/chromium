...
void HttpServerPropertiesManager::UpdateCacheFromPrefs() {
  ...
  bool is_initial_load = !is_initialized_;
  is_initialized_ = true;

  const base::DictionaryValue* http_server_properties_dict =
      pref_delegate_->GetServerProperties();
  // If there are no preferences set, or this is the initial load
  // and there's an empty preferences dictionary, do nothing. The
  // latter check is primarily to prevent clearing properties
  // learned in unit tests before properties have been loaded from
  // disk.
  if (!http_server_properties_dict ||
      (is_initial_load && http_server_properties_dict->empty())) {
    return;
  }
