...
void DhcpProxyScriptFetcherWin::OnGetCandidateAdapterNamesDone(
  ...
  net_log_.EndEvent(
      ...
      base::Bind(&DhcpProxyScriptFetcherWin::GetAdaptersNetLogCallback,
                 base::Unretained(query.get())));
...
void DhcpProxyScriptFetcherWin::OnFetcherDone(size_t fetcher_i, int result) {
  ...
  net_log_.AddEvent(NetLogEventType::WPAD_DHCP_WIN_ON_FETCHER_DONE,
                    base::Bind(&FetcherDoneNetlogCallback, fetcher_i, result));
...
void DhcpProxyScriptFetcherWin::TransitionToDone() {
  ...
  net_log_.EndEvent(NetLogEventType::WPAD_DHCP_WIN_FETCH,
                    NetLog::IntCallback("fetcher_i", fetcher_i));
...
DhcpProxyScriptFetcherWin::GetAdaptersNetLogCallback(
    AdapterQuery* query,
    NetLogCaptureMode /* capture_mode */) {
...
std::unique_ptr<base::Value> MakeAdaptersDictionaryForNetLog(
...
bool DhcpProxyScriptFetcherWin::GetCandidateAdapterNames(
  ...
  if (error == ERROR_NO_DATA) {
    ...
    result->netlog_params =
        MakeAdaptersDictionaryForNetLog(start_time, end_time, error, nullptr);
    return true;
  }
  ...
  result->netlog_params = MakeAdaptersDictionaryForNetLog(
      start_time, end_time, error, adapters.get());
  return true;
}
...
std::unique_ptr<base::Value>
DhcpProxyScriptFetcherWin::AdapterQuery::ReleaseNetLogParams() {
