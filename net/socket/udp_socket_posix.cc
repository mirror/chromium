...
const int kActivityMonitorBytesThreshold = 64 * 1024;
...
void UDPSocketPosix::ActivityMonitor::Increment(uint32_t bytes) {
  ...
  if (UNLIKELY(increments_ < kActivityMonitorMinimumSamplesForThroughputEstimate ||
               bytes_ > kActivityMonitorBytesThreshold)) {
    Update();
    if (timer_running_)
      timer_.Reset();
  }
  if (UNLIKELY(!timer_running_)) {
...
void UDPSocketPosix::LogWrite(int result,
  ...
  if (UNLIKELY(net_log_is_capturing_)) {
    net_log_.AddEvent(
