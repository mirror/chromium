...
int GetIPv4AddressFromIndex(int socket, uint32_t index, uint32_t* address) {
  ...
  // net::GetNetworkList() adds 1 to indices of the network interfaces in order
  // to ensure that all indices are above 0.
  index -= 1;
...
#if defined(OS_FUCHSIA)
        // Decrement interface index because net::GetNetworkList() increments
        // returned interface_index.
        interface_index -= 1;
#endif  // defined(OS_FUCHSIA)
