...
// Tests that |is_fatal_cert_error| gets set correctly by checking it is not set
// for a certificate error, unless it is on an HSTS host.
TEST_F(SSLClientSocketTest, IsFatalErrorSetCorrectly) {
  cert_verifier_->set_default_result(ERR_CERT_DATE_INVALID);
  SpawnedTestServer::SSLOptions ssl_options(
      SpawnedTestServer::SSLOptions::CERT_CHAIN_WRONG_ROOT);
  ASSERT_TRUE(StartTestServer(ssl_options));
