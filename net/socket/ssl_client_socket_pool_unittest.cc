...
TEST_F(SSLClientSocketPoolTest, Tag) {
  ...
  rv = handle.socket()->Write(write_buffer1.get(), strlen(kRequest1),
                              write_callback1.callback(),
                              TRAFFIC_ANNOTATION_FOR_TESTS);
  EXPECT_EQ(static_cast<int>(strlen(kRequest1)), write_callback1.GetResult(rv));
  ...
  for (auto& tcp_handle : tcp_handles) {
    rv = tcp_handle.Init("a", tcp_params, LOW, tag1,
                         ClientSocketPool::RespectLimits::ENABLED,
