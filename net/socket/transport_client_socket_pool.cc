...
// Tag socket before calling completion callback.
void TagAndCallback(const CompletionCallback& callback,
                    const SocketTag& tag,
                    ClientSocketHandle* handle,
                    int rv) {
  if (rv == OK) {
    handle->socket()->ApplySocketTag(tag);
  }
  callback.Run(rv);
}
...
int TransportClientSocketPool::RequestSocket(const std::string& group_name,
  ...
  // Tag socket if successfully being returned, either in CompletionCallback if
  // returned asynchronously or in this method if returned synchronously.
  ...
  if (rv == OK)
    handle->socket()->ApplySocketTag((*casted_params)->socket_tag());
  return rv;
}
