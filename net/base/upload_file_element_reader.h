...
class NET_EXPORT UploadFileElementReader : public UploadElementReader {
 public:
  // |file| must be valid and opened reading. On Windows, the file must have
 ...
 private:
  enum class State {
    ...
    // There's a pending operation (Other than Open) that was interrupted by
    // Reset().
    // When it completes, just do nothing. When in this state.
    IGNORE_PREVIOUS_OPERATION,
  };
  ...
  void OnGetFileInfoCompleted(base::File::Info* file_info, int result);
