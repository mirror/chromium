...
class NET_EXPORT NetworkChangeNotifier {
 public:
  ...
  // Creates the process-wide, platform-specific NetworkChangeNotifier. You may
  // call this on any thread. You may also avoid creating this entirely (in
  // which case nothing will be monitored), but if you do create it, you must do
  // so before any other threads try to access the API below, and it must
  // outlive all other threads which might try to use it.
