...
// Illustrates how to use net::TestCompletionOnceCallback.
...
typedef PlatformTest TestCompletionOnceCallbackTest;

TEST_F(TestCompletionOnceCallbackTest, Simple) {
  ExampleEmployer boss;
  TestCompletionOnceCallback callback;
  bool queued = boss.DoSomething(callback.CreateCallback());
  EXPECT_TRUE(queued);
  int result = callback.WaitForResult();
  EXPECT_EQ(result, kMagicResult);
}

TEST_F(TestCompletionOnceCallbackTest, Closure) {
  ExampleEmployer boss;
  TestOnceClosure closure;
  bool did_check_result = false;
  CompletionOnceCallback completion_callback =
      base::BindOnce(&CallClosureAfterCheckingResult, closure.CreateClosure(),
