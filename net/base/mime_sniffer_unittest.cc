...
static void TestArray(SnifferTest* tests, size_t count) {
  ...
  for (size_t i = 0; i < count; ++i) {
    SniffMimeType(tests[i].content, tests[i].content_len, GURL(tests[i].url),
                  tests[i].type_hint,
                  false /* allow_sniffing_files_urls_as_html*/, &mime_type);
...
TEST(MimeSnifferTest, BoundaryConditionsTest) {
  ...
  SniffMimeType(buf, 0, url, type_hint,
                false /* allow_sniffing_files_urls_as_html*/, &mime_type);
  EXPECT_EQ("text/plain", mime_type);
  SniffMimeType(buf, 1, url, type_hint,
                false /* allow_sniffing_files_urls_as_html*/, &mime_type);
  EXPECT_EQ("text/plain", mime_type);
  SniffMimeType(buf, 2, url, type_hint,
                false /* allow_sniffing_files_urls_as_html*/, &mime_type);
...
TEST(MimeSnifferTest, SniffFilesAsHtml) {
  ...
  SniffMimeType(kContent.c_str(), kContent.length(), kUrl, "" /* type_hint */,
                true /* allow_sniffing_files_urls_as_html*/, &mime_type);
...
TEST(MimeSnifferTest, XMLTestLargeNoAngledBracket) {
  ...
  EXPECT_TRUE(SniffMimeType(content.data(), content.size(), GURL(), "text/xml",
                            false /* allow_sniffing_files_urls_as_html*/,
                            &mime_type));
...
TEST(MimeSnifferTest, LooksBinary) {
  ...
  std::string mime_type;
  EXPECT_TRUE(
      SniffMimeType(content.data(), content.size(), GURL(), "text/plain",
                    false /* allow_sniffing_files_urls_as_html*/, &mime_type));
  EXPECT_EQ("application/octet-stream", mime_type);
}
