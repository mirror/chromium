...
std::pair<size_t, bool> GetRegistryLengthImpl(
  ...
  if (host.empty())
    return std::make_pair(std::string::npos, false);

  // Skip leading dots.
  const size_t host_check_begin = host.find_first_not_of('.');
  if (host_check_begin == std::string::npos)
    return std::make_pair(0, false);  // Host is only dots.
  ...
  if (host[host_check_len - 1] == '.') {
    ...
    if (host[host_check_len - 1] == '.')
      return std::make_pair(0, false);  // Multiple trailing dots.
  }
  ...
  if (next_dot >= host_check_len)  // Catches std::string::npos as well.
    return std::make_pair(0, false);  // This can't have a registry + domain.
  while (1) {
    ...
    if (do_check) {
      ...
      if (type & kDafsaWildcardRule && (prev_start != std::string::npos)) {
        ...
        return (prev_start == host_check_begin)
                   ? std::make_pair<size_t, bool>(0, true)
                   : std::make_pair(host.length() - prev_start, false);
      }
      ...
      if (type & kDafsaExceptionRule) {
        ...
        return std::make_pair(host.length() - next_dot - 1, false);
      }
      ...
      return (curr_start == host_check_begin)
                 ? std::make_pair<size_t, bool>(0, true)
                 : std::make_pair(host.length() - curr_start, false);
    }
  ...
  return unknown_filter == INCLUDE_UNKNOWN_REGISTRIES
             ? std::make_pair(host.length() - curr_start, false)
             : std::make_pair<size_t, bool>(0, false);
}
