...
int ObservationBuffer::GetWeightedPercentilePerSubnet(
    base::TimeTicks begin_timestamp,
    const base::Optional<int32_t>& current_signal_strength,
    int percentile,
    const std::vector<NetworkQualityObservationSource>*
  ...
  DCHECK_GE(Capacity(), Size());
  DCHECK(disallowed_observation_sources);
  ...
  std::map<int64_t, vector<WeightedObservation>> subnet_to_observation_map;
  std::map<int64_t, double> subnet_to_total_weight_map;
  ...
  for (const auto& weighted_observation : weighted_observations) {
    ...
    subnet_to_observation_map.try_emplace(weighted_observation.subnet_id,
                                          vector<WeightedObservation>());
    subnet_to_total_weight_map.try_emplace(weighted_observation.subnet_id, 0.0);
    subnet_to_observation_map[weighted_observation.subnet_id].push_back(
  ...
  for (auto& subnet_observations : subnet_to_observation_map) {
    const int64_t subnet_id = subnet_observations->first;
