...
base::Optional<float> ObservationBuffer::GetPercentile(
...
base::Optional<float> ObservationBuffer::GetWeightedAverage(
    base::TimeTicks begin_timestamp,
    const base::Optional<int32_t>& current_signal_strength,
    const std::vector<NetworkQualityObservationSource>&
        disallowed_observation_sources) const {
  ...
  return static_cast<float>(total_weight_times_value / total_weight);
}

base::Optional<float> ObservationBuffer::GetUnweightedAverage(
    base::TimeTicks begin_timestamp,
    const base::Optional<int32_t>& current_signal_strength,
    const std::vector<NetworkQualityObservationSource>&
        disallowed_observation_sources) const {
