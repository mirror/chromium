...
void ObservationBuffer::GetPercentileForEachSubnetWithCounts(
    ...
    const std::vector<NetworkQualityObservationSource>&
        disallowed_observation_sources,
    std::map<nqe::internal::SubnetID, int32_t>* subnet_keyed_percentiles,
    std::map<nqe::internal::SubnetID, size_t>* subnet_keyed_counts) const {
  ...
  // presence of a valid subnet ID. Split the observations into a map keyed by
  // the subnet ID to make it easy to calculate percentiles for each subnet.
  std::map<nqe::internal::SubnetID, std::vector<int32_t>>
  ...
  for (const auto& observation : observations_) {
    ...
    // Skip 0 values of RTT. This may appear due to localhost sockets being used
    // for RTT measurement or some other bug in the code.
    if (observation.value < 1)
      continue;

    nqe::internal::SubnetID subnet_id = observation.subnet_id.value();
  ...
  for (auto& subnet_data : subnet_keyed_observations) {
    nqe::internal::SubnetID subnet_id = subnet_data.first;
