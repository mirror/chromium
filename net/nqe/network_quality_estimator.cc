...
NetworkQualityEstimator::NetworkQualityEstimator(
  ...
  // Measure the increase in RTT periodically.
...
void NetworkQualityEstimator::IncreaseInTransportRTTUpdater() {
  ...
  if (!increase_in_transport_rtt_) {
    increase_in_transport_rtt_updater_running_ = false;
    return;
  }

  increase_in_transport_rtt_updater_running_ = true;
  base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE,
...
NetworkQualityEstimator::ComputeIncreaseInTransportRTT() {
  ...
  std::map<IPHash, int32_t> recent_median_rtts;
  std::map<IPHash, size_t> recent_observation_counts;
  ...
  std::set<IPHash> recent_hosts_set;
  ...
  std::map<IPHash, int32_t> historical_min_rtts;
  std::map<IPHash, size_t> historical_observation_counts;
  rtt_ms_observations_.GetPercentileForEachHostWithCounts(
  ...
  // Compute the common hosts between recent data and historical data, and
  // calculate the total counts for the common hosts.
  size_t total_historical_count = 0;
  size_t total_recent_count = 0;
  for (const auto& recent_median_rtts_for_host : recent_median_rtts) {
    IPHash host = recent_median_rtts_for_host.first;
...
void NetworkQualityEstimator::OnUpdatedRTTAvailable(
  ...
  // If there were no tasks posted for updating the increase in RTT, post a task
  // now because a new RTT observation.
  if (!increase_in_transport_rtt_updater_running_)
    IncreaseInTransportRTTUpdater();
}
