...
base::Optional<int32_t> NetworkQualityEstimator::GetRTTEstimateInternalMs(
  ...
  if (!statistic) {
    ...
    return rtt_ms_observations_.GetPercentile(start_time, signal_strength_,
                                              percentile,
                                              disallowed_observation_sources);
  }
  ...
  switch (statistic.value()) {
    ...
    case STATISTIC_WEIGHTED_AVERAGE:
      return rtt_ms_observations_.GetWeightedAverage(
          start_time, signal_strength_, disallowed_observation_sources);
    case STATISTIC_UNWEIGHTED_AVERAGE:
      return rtt_ms_observations_.GetUnweightedAverage(
          start_time, signal_strength_, disallowed_observation_sources);
  }
...
base::TimeDelta NetworkQualityEstimator::GetRTTEstimateInternal(
  ...
  return base::TimeDelta::FromMilliseconds(
      GetRTTEstimateInternalMs(disallowed_observation_sources, start_time,
