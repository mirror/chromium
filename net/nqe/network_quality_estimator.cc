...
void NetworkQualityEstimator::MaybeComputeEffectiveConnectionType() {
  ...
  LOG(WARNING) << "xxx mbcect cp0 "
                  "ros_l="
               << rtt_observations_size_at_last_ect_computation_
               << " rms.Size()=" << rtt_ms_observations_.Size()
               << " ect_=" << effective_connection_type_ << " tosaect="
               << throughput_observations_size_at_last_ect_computation_
               << " dtko.Size()="
               << downstream_throughput_kbps_observations_.Size()
               << " nrtto=" << new_rtt_observations_since_last_ect_computation_
               << " ntto="
               << new_throughput_observations_since_last_ect_computation_
               << " now=" << now
               << " last_ect=" << last_effective_connection_type_computation_;
  ...
  if (now - last_effective_connection_type_computation_ <
    ...
    LOG(WARNING) << "xxx mbcect cp1";
    return;
  }
  LOG(WARNING) << "xxx mbcect cp2";
  ComputeEffectiveConnectionType();
}
