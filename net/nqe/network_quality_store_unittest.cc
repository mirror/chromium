...
TEST(NetworkQualityStoreTest, TestCachingClosestSignalStrength) {
...

  {
    ...
    // If an exact matching entry is not found, the entry with the signal
    // strength closest to |signal_strength| should be read.
    for (size_t signal_strength = 0; signal_strength <= 4; ++signal_strength) {
...

  {
    ...
    // If an exact matching entry is not found, the entry with the signal
    // strength closest to |signal_strength| should be read.
    size_t signal_strength = INT32_MIN;
    nqe::internal::NetworkID network_id(NetworkChangeNotifier::CONNECTION_2G,
                                        "test1", signal_strength);
    nqe::internal::CachedNetworkQuality read_network_quality;
    EXPECT_TRUE(
