...
TEST(ThroughputAnalyzerTest, TestHangingRequestsCheckedOnlyPeriodically) {
  ...
  TestThroughputAnalyzer throughput_analyzer(&network_quality_provider,
                                             &params);
  throughput_analyzer.SetTickClockForTesting(std::move(tick_clock));
  ...
  // seconds.
  for (size_t i = 0; i < 2; ++i) {
    tick_clock_ptr->Advance(base::TimeDelta::FromMilliseconds(1000));
    throughput_analyzer.NotifyStartTransaction(*requests_not_local.at(i));
  }

  EXPECT_EQ(2u, throughput_analyzer.CountInFlightRequests());
  tick_clock_ptr->Advance(base::TimeDelta::FromMilliseconds(3500));
  ...
  tick_clock_ptr->Advance(base::TimeDelta::FromMilliseconds(600));
  ...
  tick_clock_ptr->Advance(base::TimeDelta::FromMilliseconds(400));
...
TEST(ThroughputAnalyzerTest, TestLastReceivedTimeIsUpdated) {
  ...
  tick_clock_ptr->Advance(base::TimeDelta::FromMilliseconds(4000));
  ...
  tick_clock_ptr->Advance(base::TimeDelta::FromMilliseconds(2000));
...
TEST(ThroughputAnalyzerTest, TestRequestDeletedImmediately) {
  ...
  tick_clock_ptr->Advance(base::TimeDelta::FromMilliseconds(1900));
  ...
  // seconds.
  tick_clock_ptr->Advance(base::TimeDelta::FromMilliseconds(500));
  throughput_analyzer.NotifyBytesRead(*request_not_local);
  EXPECT_EQ(0u, throughput_analyzer.CountInFlightRequests());
}
