...
class TestThroughputAnalyzer : public internal::ThroughputAnalyzer {
 public:
  ...
  using internal::ThroughputAnalyzer::count_in_flight_requests;
...
TEST(ThroughputAnalyzerTest, TestHangingRequests) {
  static const struct {
    ...
    bool expect_throughput_observation;
      ...
      {base::TimeDelta::FromSeconds(0), base::TimeDelta::FromSeconds(1), false},
      {base::TimeDelta::FromMilliseconds(1000),
       base::TimeDelta::FromMilliseconds(2000), true},
      {base::TimeDelta::FromMilliseconds(100),
       base::TimeDelta::FromMilliseconds(1000), false},
      {base::TimeDelta::FromSeconds(2), base::TimeDelta::FromSeconds(1), true},
  ...
  for (const auto& test : tests) {
    ...
    size_t num_requests = params.throughput_min_requests_in_flight();
    TestThroughputAnalyzer throughput_analyzer(&network_quality_provider,
                                               &params);
    TestDelegate test_delegate;
    TestURLRequestContext context;
    ...
    // params).
    if (test.sleep_time >= base::TimeDelta())
      base::PlatformThread::Sleep(test.sleep_time);

    EXPECT_EQ(requests_not_local.size(),
              throughput_analyzer.count_in_flight_requests());
    ...
    for (size_t i = 0; i < requests_not_local.size(); ++i) {
      ...
      if (!test.expect_throughput_observation) {
        ...
        EXPECT_EQ(0u, throughput_analyzer.count_in_flight_requests());
      } else {
