...
void ThroughputAnalyzer::EraseHangingRequests() {
  ...
  // check for field trial.

  for (Requests::iterator it = requests_.begin(); it != requests_.end();) {
    base::TimeDelta time_since_last_received = now - it->second;

    if (time_since_last_received >= 5 * http_rtt &&
        time_since_last_received >= base::TimeDelta::FromMilliseconds(1)) {
  ...
  // flight, which may lead to inaccuracies in the estimate computation.
