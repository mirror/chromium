...
TEST_F(SSLClientSessionCacheTest, BasicSingleUse) {
  ...
  bssl::UniquePtr<SSL_SESSION> session1 = NewSSLSession(TLS1_3_DRAFT_VERSION);
  bssl::UniquePtr<SSL_SESSION> session2 = NewSSLSession(TLS1_3_DRAFT_VERSION);
  bssl::UniquePtr<SSL_SESSION> session3 = NewSSLSession(TLS1_3_DRAFT_VERSION);
  EXPECT_EQ(1u, session1->references);
  EXPECT_EQ(1u, session2->references);
  EXPECT_EQ(1u, session3->references);
