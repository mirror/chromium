  ...
  void AcquirePrivateKey(
    ...
    if (base::PostTaskAndReplyWithResult(
            ...
            base::Bind(&FetchClientCertPrivateKey,
                       ...
                       base::Passed(ScopedCERTCertificate(
                           CERT_DupCertificate(cert_certificate_.get()))),
...
void ClientCertStoreNSS::FilterCertsOnWorkerThread(
  ...
  for (auto examine_iter = identities->begin();
    ...
    std::vector<ScopedCERTCertificate> nss_intermediates;
...
void ClientCertStoreNSS::GetPlatformCertsOnWorkerThread(
  ...
  for (CERTCertListNode* node = CERT_LIST_HEAD(found_certs);
    ...
    identities->push_back(base::MakeUnique<ClientCertIdentityNSS>(
        cert, ScopedCERTCertificate(CERT_DupCertificate(node->cert)),
        password_delegate));
  }
