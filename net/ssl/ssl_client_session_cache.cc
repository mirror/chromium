...
bssl::UniquePtr<SSL_SESSION> SSLClientSessionCache::Lookup(
  ...
  if (!session || IsExpired(session.get(), clock_->Now().ToTimeT())) {
    cache_.Erase(iter);
    return nullptr;
  }
...
SSL_SESSION* SSLClientSessionCache::Entry::Peek() {
...
bool SSLClientSessionCache::Entry::IsEmpty() {
  return sessions[0] == nullptr;
}
