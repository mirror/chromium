...
class TestSSLPlatformKey : public ThreadedSSLPrivateKey::Delegate {
 public:
  ...
  std::vector<uint16_t> GetPreferences() override {
    return SSLPrivateKey::DefaultPreferences(EVP_PKEY_id(key_.get()),
                                             true /* supports PSS */);
  }
  ...
  Error Sign(uint16_t algorithm,
    ...
    size_t sig_len = 0;
    if (!EVP_DigestSign(ctx.get(), NULL, &sig_len, input_ptr, input_len))
      return ERR_SSL_CLIENT_AUTH_SIGNATURE_FAILED;
    signature->resize(sig_len);
    if (!EVP_DigestSign(ctx.get(), signature->data(), &sig_len, input_ptr,
                        input_len)) {
      return ERR_SSL_CLIENT_AUTH_SIGNATURE_FAILED;
    }
    signature->resize(sig_len);
