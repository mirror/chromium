...
TEST(X509CertificateTest, InvalidPrintableStringIsUtf8) {
  ...
  EXPECT_FALSE(X509Certificate::CreateFromHandle(
...
TEST(X509CertificateTest, ExtractCRLURLsFromDERCert) {
  ...
  EXPECT_TRUE(asn1::ExtractCRLURLsFromDERCert(
      x509_util::CryptoBufferAsStringPiece(cert->cert_buffer()), &crl_urls));
...
// Tests CRYPTO_BUFFER deduping via X509Certificate::CreateFromHandle.  We
...
TEST(X509CertificateTest, Cache) {
  ...
  scoped_refptr<X509Certificate> cert1(
      X509Certificate::CreateFromHandle(std::move(google_cert_handle), {}));
  ASSERT_TRUE(cert1);
...
TEST(X509CertificateTest, IntermediateCertificates) {
  ...
  scoped_refptr<X509Certificate> cert2;
  cert2 = X509Certificate::CreateFromHandle(std::move(google_handle),
