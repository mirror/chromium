...
class EncodeOCSPTest : public ::testing::TestWithParam<const char*> {};

const char* kEncodeOCSPTests[] = {
    "foo",
};

INSTANTIATE_TEST_CASE_P(,
                        EncodeOCSPTest,
                        ::testing::ValuesIn(kEncodeOCSPTests));

TEST_P(EncodeOCSPTest, FromFile) {
  std::string ocsp_data;
  std::string ca_data;
  std::string cert_data;
  const PemBlockMapping mappings[] = {
      {"OCSP REQUEST", &ocsp_data},
      {"CA CERTIFICATE", &ca_data},
      {"CERTIFICATE", &cert_data},
  };

  ASSERT_TRUE(ReadTestDataFromPemFile(GetFilePath(GetParam()), mappings));
  ...
  ASSERT_TRUE(CreateOCSPRequest(cert.get(), issuer.get(), &encoded));

  EXPECT_EQ(der::Input(encoded.data(), encoded.size()), der::Input(&ocsp_data));
}
