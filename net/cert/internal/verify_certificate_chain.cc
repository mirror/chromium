...
void PathVerifier::BasicCertificateProcessing(
  ...
  // Check with the delegate whether this signature algorithm is allowed.
  if (!delegate_->IsSignatureAlgorithmAcceptable(cert.signature_algorithm(),
                                                 errors)) {
    errors->AddError(kUnacceptableSignatureAlgorithm);
  } else {
    // Parse the key from SPKI, and ask the delegate if it is acceptable. If
    // either check fails, adds an error to |errors| and returns nullptr.
    bssl::UniquePtr<EVP_PKEY> pkey =
        ParseAndCheckPublicKey(working_spki_, errors);
...
void PathVerifier::WrapUp(const ParsedCertificate& cert, CertErrors* errors) {
  ...
  // other certificates is already checked by BasicCertificateProcessing().
