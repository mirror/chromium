...
WARN_UNUSED_RESULT bool ParseGeneralSubtrees(const der::Input& value,
  ...
  while (sequence_parser.HasMore()) {
    ...
    if (!ParseGeneralName(raw_general_name,
      ...
      errors->AddError("Failed parsing general name");
      return false;
    }
...
std::unique_ptr<GeneralNames> GeneralNames::Create(
  ...
  if (!parser.ReadSequence(&sequence_parser)) {
    errors->AddError("GeneralNames is not a sequence");
    return nullptr;
  }
  // Should not have trailing data after GeneralNames sequence.
  if (parser.HasMore()) {
    errors->AddError("GeneralNames contains trailing data after the sequence");
    return nullptr;
  }
  // The GeneralNames sequence should have at least 1 element.
  if (!sequence_parser.HasMore()) {
    errors->AddError("GeneralNames is a sequence of 0 elements");
    return nullptr;
  }
