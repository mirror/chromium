...
// Test fixture for running the path builder over a simple chain, while varying
// that the path-builder specific delegate methods are called correctly.
...
class CertPathBuilderDelegateBase : public SimplePathBuilderDelegate {
 public:
  ...
  void CheckPathAfterVerification(const CertPath& path,
    ...
    ADD_FAILURE() << "Must override this";
  }
};
...
// Use a CertPathBuilderDelegate that doesn't set any errors in its
// CheckPathAfterVerification() callback. should be accepted since it is valid.
// (This is what the other tests are implicitly relying on working).
...
class SetWarningPathBuilderDelegate : public CertPathBuilderDelegateBase {
 public:
  void CheckPathAfterVerification(const CertPath& path,
                                  CertPathErrors* errors) override {
...
TEST_F(PathBuilderHooksTest, SetWarningPostVerificationHook) {
  ...
  SetWarningPathBuilderDelegate delegate;
  RunPathBuilder(nullptr, &delegate, &result);
  ASSERT_TRUE(result.HasValidPath());
...
class SetErrorPathBuilderDelegate : public CertPathBuilderDelegateBase {
 public:
  void CheckPathAfterVerification(const CertPath& path,
                                  CertPathErrors* errors) override {
    // Note: Re-using an existing error ID.
...
TEST_F(PathBuilderHooksTest, SetErrorPostVerificationHook) {
  ...
  SetErrorPathBuilderDelegate set_error_checker;
...
TEST_F(PathBuilderHooksTest, PostVerificationHookNotCalledOnFailure) {
  ...
  StrictMock<MockPathBuilderDelegate> do_nothing_checker;
