...
class NET_EXPORT CertPathBuilderDelegate : public CertificateSignatureVerifier {
 public:
  // This is called during path building on candidate paths which have already
  // passed certificate verification (RFC 5280 verification including signature
  // checks). The delegate can choose to add errors to the path for things like
  // revocation checking, which will cause path building to keep searching for a
  // valid path.
  virtual void CheckPathAfterVerification(const CertPath& path,
                                          CertPathErrors* errors) const = 0;
};
...
class NET_EXPORT CertPathBuilder {
 public:
  ...
  //               final chain. See CertPathBuilderDelegate for more
  //               information.
 ...
 private:
  ...
  const CertPathBuilderDelegate* delegate_;
