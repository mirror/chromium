...
WARN_UNUSED_RESULT bool VerifyOCSPResponseSignature(
  ...
  scoped_refptr<const ParsedCertificate> responder_certificate;
  if (CheckResponderIDMatchesCertificate(response_data.responder_id,
                                         issuer_certificate)) {
    // No additional checks are done on the issuer certificate if it is being
    // used directly to sign OCSP.
    responder_certificate = issuer_certificate;
  } else {
    ...
    for (const auto& responder_cert_tlv : response.certs) {
      ...
      if (cur_responder_certificate &&
          CheckResponderIDMatchesCertificate(response_data.responder_id,
                                             cur_responder_certificate.get())) {
        responder_certificate = cur_responder_certificate;
        break;
      }
    }

    // No authorized responder found.
    if (!responder_certificate)
      return false;

    // Verify that the authorized responder certificate has authority to sign
    // OCSP responses.
    if (!VerifyAuthorizedResponderCert(responder_certificate.get(),
                                       issuer_certificate)) {
      return false;
    }
  }

  // Now that a valid responder certificate has been found, check the signature
  // of the OCSP response using it.
  //
  // TODO(eroman): Must check the signature algorithm against policy.
  return VerifySignedData(*(response.signature_algorithm), response.data,
                          response.signature,
                          responder_certificate->tbs().spki_tlv);
}
