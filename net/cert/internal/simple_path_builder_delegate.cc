...
unsigned int GetRsaModulusLengthInBits(EVP_PKEY* pkey) {
...
bool SimplePathBuilderDelegate::VerifyCertificateSignature(
    const SignatureAlgorithm& signature_algorithm,
    const der::Input& tbs_certificate_tlv,
    const der::BitString& signature_value,
    const der::Input& public_key_spki,
    CertErrors* errors) const {
  if (!IsAcceptableSignatureAlgorithm(signature_algorithm)) {
  ...
  if (!ParsePublicKey(signature_algorithm, public_key_spki, &public_key)) {
    // TODO(eroman): Add error description.
    return false;
  }
  ...
  switch (signature_algorithm.algorithm()) {
    ...
    case SignatureAlgorithmId::RsaPkcs1: {
      ...
      unsigned int modulus_length_bits =
          GetRsaModulusLengthInBits(public_key.get());

      if (modulus_length_bits < min_rsa_modulus_length_bits_) {
        errors->AddError(kRsaModulusTooSmall,
                         CreateCertErrorParams2SizeT(
                             "actual", modulus_length_bits, "minimum",
    ...
    case SignatureAlgorithmId::Ecdsa: {
      ...
      EC_KEY* ec = EVP_PKEY_get0_EC_KEY(public_key.get());
      if (!ec)
    ...
    case SignatureAlgorithmId::Dsa:
      // Already handled at start, shouldn't reach.
      return false;
  }
