...
// Enum to indicate whether the revocation status for the chain is known. When a
// certificate in the chain is identified as being revoked an error is added to
// it, so this enum just categorizes (revoked || good) vs unknown.
enum class ChainRevocationStatus {
  kUnknown,
  kKnown,
};
...
class PathBuilderDelegateImpl : public SimplePathBuilderDelegate {
 public:
  ...
  // This is called for each built chain, including ones which failed.
  void CheckPathAfterVerification(const CertPath& path,
                                  CertPathErrors* errors) override {
    if (crl_set_)
      CheckRevocationUsingCRLSet(path, errors);

    // TODO(eroman): Check revocation using OCSP and CRL as well.
  }

 private:
  // Checks the revocation status of the target certificate using the CRLSet. If
  // any certificate is identified to be revoked an kCertificateRevoked
  // high-severity error is added to its errors.
  ChainRevocationStatus CheckRevocationUsingCRLSet(const CertPath& path,
                                                   CertPathErrors* errors) {
    // Iterate from root certificate towards the leaf.
    ...
    for (size_t reverse_i = 0; reverse_i < path.certs.size(); ++reverse_i) {
      ...
      bool is_root = reverse_i == 0;
      // True if |cert| is the leaf certificate of the chain.
      bool is_target = i == 0;

      // Check for revocation using the certificates SPKI.
      ...
      switch (result) {
        case CRLSet::REVOKED:
          // If any certificate was revoked, add the error and can then stop
          // looking.
          errors->GetErrorsForCert(i)->AddError(kCertificateRevoked);
          return ChainRevocationStatus::kKnown;
        case CRLSet::UNKNOWN:
          // If the status is unknown, keep going (check subordinate's
          // revocation status).
          break;
        case CRLSet::GOOD:
    ...
    return ChainRevocationStatus::kUnknown;
  }
