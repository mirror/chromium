...
RevocationPolicy NoRevocationChecking() {
  ...
  const bool networking_allowed = false;
  const bool allow_missing_info = true;
  const bool allow_network_failure = true;

  return RevocationPolicy(check_revocation, networking_allowed,
                          allow_missing_info, allow_network_failure);
}
...
void GetEVPolicyOids(EVRootCAMetadata* ev_metadata,
...
bool IsEVCandidate(EVRootCAMetadata* ev_metadata,
                   const ParsedCertificate* cert) {
  std::set<der::Input> oids;
...
class PathBuilderDelegateImpl : public SimplePathBuilderDelegate {
 public:
  class PathInfo : public CertPathBuilderDelegateData {
   public:
    bool enabled_revocation_checking = false;
    bool enabled_ev = false;
  };
  ...
  static PathInfo* GetPathInfo(CertPathBuilderDelegateData* data) {
    return reinterpret_cast<PathInfo*>(data);
  }
  ...
  void CheckPathAfterVerification(CertPathBuilderResultPath* path) override {
    // Associate some extra delegate-specific information with this path
    // (consumer expects this to be non-null).
    DCHECK(!path->delegate_data);
    path->delegate_data = std::make_unique<PathInfo>();
    PathInfo* path_info = GetPathInfo(path->delegate_data.get());
    ...
    // fail.
    // TODO(eroman): Should revocation status still be checked via offline means
    //               to give a better error if also known to be revoked?
    if (!path->IsValid())
      return;

    // If EV was requested the certificate must chain to a recognized EV root
    // and have one of its recognized EV policy OIDs.
    if (do_ev_ && !ConformsToEVPolicy(path)) {
    ...
    path_info->enabled_ev = do_ev_;
    path_info->enabled_revocation_checking = policy.check_revocation;
    ...
    // reflect the revocation status of the chain.
    CheckCertChainRevocation(*path, policy, stapled_leaf_ocsp_response_,
 ...
 private:
  ...
  RevocationPolicy ChooseRevocationPolicy(
    ...
    *crlset_leaf_coverage_sufficient = false;
    ...
    // Use hard-fail revocation checking for local trust anchors.
    if ((flags_ & CertVerifier::VERIFY_REV_CHECKING_REQUIRED_LOCAL_ANCHORS) &&
        !certs.empty() && !ssl_trust_store_->IsKnownRoot(certs.back().get())) {
      const bool check_revocation = true;
      const bool networking_allowed =
          (flags_ & CertVerifier::VERIFY_CERT_IO_ENABLED);
      bool allow_missing_info = true;
      const bool allow_network_failure = false;

      // If EV verification is additionally being requested, choose a policy
      // that is equivalent to running revocation checking for both serially.
      // (The EV policy is more strict in regards to |allow_missing_info|, and
      // less strict in regards to crlset_leaf_coverage_sufficient|).
      if (do_ev_ && (flags_ & CertVerifier::VERIFY_EV_CERT))
    ...
    if (do_ev_ && (flags_ & CertVerifier::VERIFY_EV_CERT)) {
      ...
      *crlset_leaf_coverage_sufficient = true;
      ...
      const bool allow_missing_info = false;
      const bool allow_network_failure = false;
...
scoped_refptr<ParsedCertificate> ParseCertificateFromOSHandle(
  ...
  std::string pem;
  X509Certificate::GetPEMEncodedFromDER(cert_bytes, &pem);
  LOG(ERROR) << "\n" << pem << "\n";
...
void TryBuildPath(const scoped_refptr<ParsedCertificate>& target,
  ...
  PathBuilderDelegateImpl path_builder_delegate(crl_set, net_fetcher, do_ev,
                                                flags, ssl_trust_store,
                                                ocsp_response, ev_metadata);
...
int AssignVerifyResult(X509Certificate* input_cert,
  ...
  const PathBuilderDelegateImpl::PathInfo* delegate_info =
      PathBuilderDelegateImpl::GetPathInfo(partial_path.delegate_data.get());
  ...
  if (path_is_valid && delegate_info->enabled_ev)
    verify_result->cert_status |= CERT_STATUS_IS_EV;

  if (delegate_info->enabled_revocation_checking)
...
int CertVerifyProcBuiltin::VerifyInternal(
  ...
  // Try to build a path and verify the target certificate.
  //
  // Path building may be attempted multiple times with different parameters
  // (first attempt to build an EV path, then for DV).
  CertPathBuilder::Result result;

  for (bool attempt_ev : {true, false}) {
    // Only attempt EV if it was requested and if the target certificate could
    // be EV.
    if (attempt_ev && !((flags & CertVerifier::VERIFY_EV_CERT) &&
                        IsEVCandidate(ev_metadata, target.get()))) {
      continue;
    }

    TryBuildPath(target, &intermediates, ssl_trust_store.get(),
                 base::Time::Now(), attempt_ev, flags, ocsp_response, crl_set,
                 net_fetcher, ev_metadata, &result);

    // Once a valid path is found, stop trying other attempts (earlier attempts
    // have higher priority).
    if (result.HasValidPath())
      break;
  }

  // Fill |*verify_result| with the resulting path.
  return AssignVerifyResult(input_cert, hostname, result, ssl_trust_store.get(),
                            verify_result);
}
