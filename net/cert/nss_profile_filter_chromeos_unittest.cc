...
ScopedCERTCertificateVector ListCertsInSlot(PK11SlotInfo* slot) {
...
class NSSProfileFilterChromeOSTest : public testing::Test {
 ...
 protected:
  ScopedCERTCertificateVector certs_;
...
TEST_F(NSSProfileFilterChromeOSTest, RootCertsAllowed) {
  ...
  ScopedCERTCertificateVector root_certs(
      ListCertsInSlot(root_certs_slot.get()));
...
TEST_F(NSSProfileFilterChromeOSTest, SoftwareSlots) {
  ...
  ScopedCERTCertificateVector certs_2 = CreateCERTCertificateListFromFile(
      GetTestCertsDirectory(), "ok_cert.pem", X509Certificate::FORMAT_AUTO);
  ASSERT_EQ(1U, certs_2.size());
  CERTCertificate* cert_2 = certs_2[0].get();
  ScopedCERTCertificateVector system_certs = CreateCERTCertificateListFromFile(
