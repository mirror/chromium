...
bool AddTime(CBB* cbb, base::Time time) {
  ...
  static constexpr size_t kUTCTimeLength = 13;
  static constexpr size_t kGeneralizedTimeLength = 15;
  char buf[kGeneralizedTimeLength + 1];
  CBB child;
  if (1950 <= generalized_time.year && generalized_time.year < 2050) {
    uint32_t year = generalized_time.year - 1900;
    if (year > 100)
      year -= 100;
    if (base::snprintf(buf, sizeof(buf), "%02u%02u%02u%02u%02u%02uZ", year,
                       generalized_time.month, generalized_time.day,
                       generalized_time.hours, generalized_time.minutes,
                       generalized_time.seconds) != kUTCTimeLength ||
        !CBB_add_asn1(cbb, &child, CBS_ASN1_UTCTIME) ||
        !CBB_add_bytes(&child, reinterpret_cast<const uint8_t*>(buf),
                       kUTCTimeLength)) {
      return false;
    }
  } else {
    if (base::snprintf(buf, sizeof(buf), "%04u%02u%02u%02u%02u%02uZ",
                       generalized_time.year, generalized_time.month,
                       generalized_time.day, generalized_time.hours,
                       generalized_time.minutes,
                       generalized_time.seconds) != kGeneralizedTimeLength ||
        !CBB_add_asn1(cbb, &child, CBS_ASN1_GENERALIZEDTIME) ||
        !CBB_add_bytes(&child, reinterpret_cast<const uint8_t*>(buf),
                       kGeneralizedTimeLength)) {
      return false;
    }
  }
...
bool CreateSelfSignedCert(crypto::RSAPrivateKey* key,
  ...
  if (!CBB_init(cbb.get(), tbs_cert_len) ||
      ...
      !EVP_DigestSign(ctx.get(), nullptr, &sig_len, tbs_cert_bytes,
                      tbs_cert_len) ||
      !CBB_reserve(&signature, &out, sig_len) ||
      // Actually sign the TBSCertificate.
      !EVP_DigestSign(ctx.get(), out, &sig_len, tbs_cert_bytes, tbs_cert_len) ||
      !CBB_did_write(&signature, sig_len) ||
      !CBB_finish(cbb.get(), &der, &der_len)) {
    return false;
  }
  bssl::UniquePtr<uint8_t> delete_der(der);
  der_encoded->assign(reinterpret_cast<char*>(der), der_len);
  return true;
}
