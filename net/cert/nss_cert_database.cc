...
std::vector<ScopedCERTCertificate> NSSCertDatabase::ListCertsSync() {
...
int NSSCertDatabase::ExportToPKCS12(const ScopedCERTCertificateVector& certs,
                                    const base::string16& password,
...
int NSSCertDatabase::ImportUserCert(const std::string& data) {
  ...
  int result = psm::ImportUserCert(
      x509_util::CreateCERTCertificateFromX509Certificate(certificates[0].get())
          .get());
...
bool NSSCertDatabase::ImportCACerts(
    const ScopedCERTCertificateVector& certificates,
...
// static
std::vector<ScopedCERTCertificate> NSSCertDatabase::ListCertsImpl(
  ...
  for (node = CERT_LIST_HEAD(cert_list); !CERT_LIST_END(node, cert_list);
    ...
    certs.push_back(ScopedCERTCertificate(CERT_DupCertificate(node->cert)));
  }
