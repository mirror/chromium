...
void RecordRootCertHistogram(const HashValueVector& hashes) {
  for (const auto& hash : hashes) {
    int32_t id = GetNetRootCertHistogramIdForHashValue(hash);
    if (id == 0)
      continue;
    UMA_HISTOGRAM_SPARSE_SLOWLY("Net.Certificate.RootCertUsage.Verify", id);
    break;
  }
}
...
int CertVerifyProc::Verify(X509Certificate* cert,
  ...
  if (rv == OK)
    RecordRootCertHistogram(verify_result->public_key_hashes);
