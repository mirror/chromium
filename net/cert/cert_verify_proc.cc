...
void CheckOCSP(const std::string& raw_response,
  ...
  // Get the certificate that signed |certificate|, as this is needed for
  // checking OCSP.
  std::string cert_der;
  ...
  if (intermediates.empty() ||
      !X509Certificate::GetDEREncoded(certificate.os_cert_handle(),
                                      &cert_der) ||
      !X509Certificate::GetDEREncoded(intermediates.front(), &issuer_der)) {
    *verify_result = OCSPVerifyResult();
    return;
  }
