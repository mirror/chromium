...
bool IsCertInCertificateList(const X509Certificate* cert,
                             const ScopedCERTCertificateVector& cert_list) {
  for (ScopedCERTCertificateVector::const_iterator it = cert_list.begin();
       it != cert_list.end(); ++it) {
    if (x509_util::IsSameCertificate(it->get(), cert))
      return true;
  }
  return false;
}
...
void SwapCertLists(ScopedCERTCertificateVector* destination,
                   ScopedCERTCertificateVector source) {
...
TEST_F(NSSCertDatabaseChromeOSTest, ImportCACerts) {
  ...
  ScopedCERTCertificateVector certs_1 = CreateCERTCertificateListFromFile(
  ...
  ScopedCERTCertificateVector user_1_certlist = db_1_->ListCertsSync();
  ScopedCERTCertificateVector user_2_certlist = db_2_->ListCertsSync();
  ...
  ScopedCERTCertificateVector user_1_certlist_async;
  ScopedCERTCertificateVector user_2_certlist_async;
  db_1_->ListCerts(
...
TEST_F(NSSCertDatabaseChromeOSTest, NoCrashIfShutdownBeforeDoneOnWorkerPool) {
  ScopedCERTCertificateVector certlist;
...
TEST_F(NSSCertDatabaseChromeOSTest, ListCertsReadsSystemSlot) {
  ...
  ScopedCERTCertificateVector certs = db_1_->ListCertsSync();
...
TEST_F(NSSCertDatabaseChromeOSTest, ListCertsDoesNotCrossReadSystemSlot) {
  ...
  scoped_refptr<X509Certificate> cert_2(
      ImportClientCertAndKeyFromFile(GetTestCertsDirectory(),
                                     ...
                                     "client_2.pk8",
                                     system_db_.slot()));
  ScopedCERTCertificateVector certs = db_2_->ListCertsSync();
  EXPECT_TRUE(IsCertInCertificateList(cert_1.get(), certs));
