...
// kBlockedSPKICRLSet contains a single limited subject hash that allows only
// the all-zero SPKI hash.
static const uint8_t kBlockedSubjectCRLSet[] = {
    0xc2, 0x00, 0x7b, 0x22, 0x56, 0x65, 0x72, 0x73, 0x69, 0x6f, 0x6e, 0x22,
    ...
    0x65, 0x6e, 0x74, 0x73, 0x22, 0x3a, 0x30, 0x2c, 0x22, 0x4c, 0x69, 0x6d,
    0x69, 0x74, 0x65, 0x64, 0x53, 0x75, 0x62, 0x6a, 0x65, 0x63, 0x74, 0x73,
    0x22, 0x3a, 0x7b, 0x22, 0x72, 0x42, 0x65, 0x52, 0x75, 0x54, 0x36, 0x31,
    0x71, 0x4b, 0x47, 0x50, 0x4e, 0x6d, 0x34, 0x53, 0x36, 0x43, 0x61, 0x65,
    0x67, 0x74, 0x48, 0x6d, 0x33, 0x6d, 0x64, 0x34, 0x75, 0x72, 0x48, 0x6c,
    0x4d, 0x7a, 0x32, 0x30, 0x31, 0x4a, 0x6e, 0x64, 0x31, 0x77, 0x67, 0x3d,
    0x22, 0x3a, 0x5b, 0x22, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41,
...
TEST(CRLSetTest, BlockedSubjects) {
  base::StringPiece s(reinterpret_cast<const char*>(kBlockedSubjectCRLSet),
                      sizeof(kBlockedSubjectCRLSet));
  scoped_refptr<CRLSet> set;
  EXPECT_TRUE(CRLSetStorage::Parse(s, &set));
  ASSERT_TRUE(set.get() != NULL);

  // C=US, ST=California, L=Mountain View, O=Google Inc, CN=www.google.com
  const uint8_t kSubject[] = {
      ...
      0x15, 0x06, 0x03, 0x55, 0x04, 0x03, 0x0c, 0x0e, 0x77, 0x77, 0x77, 0x2e,
      0x67, 0x6f, 0x6f, 0x67, 0x6c, 0x65, 0x2e, 0x63, 0x6f, 0x6d,
  };

  const base::StringPiece subject(reinterpret_cast<const char*>(kSubject),
                                  sizeof(kSubject));
  ...
  // When used with the known-good hash, that subject should be accepted.
  const char kAcceptedSPKIHash[32] = {0};
  EXPECT_EQ(
      CRLSet::GOOD,
      set->CheckSubject(subject, base::StringPiece(kAcceptedSPKIHash,
                                                   sizeof(kAcceptedSPKIHash))));
}
