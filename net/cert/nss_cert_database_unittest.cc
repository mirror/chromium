...
void SwapCertList(ScopedCERTCertificateVector* destination,
...
class CertDatabaseNSSTest : public testing::Test {
 ...
 protected:
  ...
  static bool ReadCertIntoList(const std::string& name,
                               ScopedCERTCertificateVector* certs) {
  ...
  ScopedCERTCertificateVector ListCerts() {
...
TEST_F(CertDatabaseNSSTest, ListCertsSync) {
  ...
  // for leaks.
  ScopedCERTCertificateVector certs = cert_db_->ListCertsSync();
...
TEST_F(CertDatabaseNSSTest, ImportFromPKCS12AsExtractableAndExportAgain) {
  ...
  ScopedCERTCertificateVector cert_list = ListCerts();
...
TEST_F(CertDatabaseNSSTest, ImportCACert_SSLTrust) {
  ScopedCERTCertificateVector certs = CreateCERTCertificateListFromFile(
...
TEST_F(CertDatabaseNSSTest, ImportServerCert) {
  ...
  ScopedCERTCertificateVector certs_to_import;
  ASSERT_TRUE(
  ...
  int error = verify_proc->Verify(
      x509_util::CreateX509CertificateFromCertCertificate(found_server_cert)
          .get(),
      "127.0.0.1", std::string(), flags, NULL, empty_cert_list_,
      &verify_result);
...
TEST_F(CertDatabaseNSSTest, ImportServerCert_SelfSigned) {
  ...
  CertVerifyResult verify_result;
  int error = verify_proc->Verify(
      x509_util::CreateX509CertificateFromCertCertificate(puny_cert).get(),
      "xn--wgv71a119e.com", std::string(), flags, NULL, empty_cert_list_,
      &verify_result);
  EXPECT_THAT(error, IsError(ERR_CERT_AUTHORITY_INVALID));
  EXPECT_EQ(CERT_STATUS_AUTHORITY_INVALID, verify_result.cert_status);
}
...
TEST_F(CertDatabaseNSSTest, ImportCaAndServerCert) {
  ScopedCERTCertificateVector ca_certs = CreateCERTCertificateListFromFile(
      GetTestCertsDirectory(), "root_ca_cert.pem",
      X509Certificate::FORMAT_AUTO);
  ...
  // XXX shouldn't this fail to compile since ScopedCERTCertificateVector
  // isn't copyable?
  EXPECT_TRUE(cert_db_->ImportServerCert(certs, NSSCertDatabase::TRUST_DEFAULT,
                                         &failed));
  EXPECT_EQ(0U, failed.size());
  ...
  CertVerifyResult verify_result;
  int error = verify_proc->Verify(
      x509_util::CreateX509CertificateFromCertCertificate(certs[0].get()).get(),
      "127.0.0.1", std::string(), flags, NULL, empty_cert_list_,
      &verify_result);
  EXPECT_THAT(error, IsOk());
  EXPECT_EQ(0U, verify_result.cert_status);
...
TEST_F(CertDatabaseNSSTest, TrustIntermediateCa) {
  ...
  ScopedCERTCertificateVector intermediate_certs =
...
TEST_F(CertDatabaseNSSTest, ImportDuplicateCommonName) {
  ...
  ScopedCERTCertificateVector new_certs = ListCerts();
  ASSERT_EQ(1U, new_certs.size());

  // Now attempt to import a different certificate with the same common name.
  ScopedCERTCertificateVector certs2 = CreateCERTCertificateListFromFile(
