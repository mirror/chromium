...
bool GeneralizedTimeToBaseTime(const der::GeneralizedTime& generalized,
  ...
  // TODO(mattm): consider consolidating this with SaturatedTimeFromUTCExploded
  // from cookie_util.cc
  if (sizeof(time_t) == 4 && generalized.year >= 2038) {
    *result = base::Time::Max();
    return true;
  }
  if (sizeof(time_t) == 4 && generalized.year < 1970) {
