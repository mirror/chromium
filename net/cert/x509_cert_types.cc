...
bool CertPrincipal::ParseDistinguishedName(
  ...
  auto x509_name_attribute_printable_string_handling =
      ...
      if (name_attribute.type == TypeCommonNameOid()) {
        if (common_name.empty() &&
            !name_attribute.ValueAsStringWithUnsafeOptions(
                x509_name_attribute_printable_string_handling, &common_name)) {
          return false;
        }
      } else if (name_attribute.type == TypeLocalityNameOid()) {
        if (locality_name.empty() &&
            !name_attribute.ValueAsStringWithUnsafeOptions(
                x509_name_attribute_printable_string_handling,
                &locality_name)) {
          return false;
        }
        ...
        if (country_name.empty() &&
            !name_attribute.ValueAsStringWithUnsafeOptions(
                x509_name_attribute_printable_string_handling, &country_name)) {
          return false;
        }
      } else if (name_attribute.type == TypeStreetAddressOid()) {
        std::string s;
        if (!name_attribute.ValueAsStringWithUnsafeOptions(
                x509_name_attribute_printable_string_handling, &s))
          return false;
