...
// CrashResistantNetLogObserver watches the NetLog event stream, and sends all
// entries to a file specified on creation, immediately.
//
// Each line in the text file is a URL.
class NET_EXPORT CrashResistantNetLogObserver : public NetLog::ThreadSafeObserver {
 public:
  CrashResistantNetLogObserver();
  ~CrashResistantNetLogObserver() override;
  ...
  // before destruction of the CrashResistantNetLogObserver and the NetLog.
  //
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(CrashResistantNetLogObserver);
};
