...
class NET_EXPORT FileNetLogObserver : public NetLog::ThreadSafeObserver {
 public:
  ...
  //
  // |max_total_size| is the limit on how many filesystem bytes logging can use.
  // This is an approximate limit, and in practice FileNetLogObserver may
  // slighly exceed it.
  //
  // |total_num_event_files| sets the total number of event files that are used
  // to write the events. It must be greater than 0.
  //
  ...
  // Shorcut for calling CreateBounded() with kLimit.
