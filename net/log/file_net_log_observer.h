...
class NET_EXPORT FileNetLogObserver : public NetLog::ThreadSafeObserver {
 public:
  ...
  // restrictions.
  //
  // |total_num_event_files| sets the total number of event files that are used
  // to write the events. It must be greater than 0. If using kNoLimit for
  // |max_total_size|, this should be set to 1.
  // TODO(eroman): Remove this parameter from the public API, or make it have a
  // default. The implementation can pick an appropriate file chunk size.
  //
  ...
  // Shortcut for calling CreateBounded() with kLimit.
 ...
 private:
  ...
  // Same as CreateBounded() but you can specify how many event files it uses
  // internally.
