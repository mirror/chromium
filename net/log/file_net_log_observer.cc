...
class FileNetLogObserver::BoundedFileWriter
 ...
 private:
  ...
  // Gets the path to an internally managed directory that contains the chunked
  // event files.
  ...
  // Returns the corresponding index for for |file_number|. File "numbers" are a
  // monotonically increasing identifier, whereas the file "index" is a bounded
  // value that wraps and identifies the file path to use.
  //
  ...
  // since after copying the data the source events files are deleted one by
  // one.
...
// TODO(eroman): Move this down to match declaration order.
...
void FileNetLogObserver::BoundedFileWriter::Initialize(
  ...
  // When operating in bounded mode, create the internal events directory, and
  // open the first events file. If creating the directory fails because the
  // directory already exists, things will proceed as usual but the directory
  // won't be cleaned up once stopped.
  if (IsBounded()) {
    if (!base::CreateDirectory(GetEventFileDirectory())) {
...
void FileNetLogObserver::BoundedFileWriter::Stop(
  ...
  // If operating in bounded mode, the events were written to separate files in
...
// TODO(eroman): This could be optimized for memory usage. It does a dumb copy
// by reading the entire file to a string. This assumes that files are not
// unreasonably large.
void FileNetLogObserver::BoundedFileWriter::MoveEventFilesToLogFile() {
  // Make all the events files are flushed, since will be reading them next.
