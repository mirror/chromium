...
// ParsedNetLog describes the parsed contents contents of a NetLog file
// (constants, events, polled data).
struct ParsedNetLog {
  // Initialize the ParsedNetLog by parsing a JSON file.
  ...
  // Returns the event at index |i|, or nullptr if there isn't one.
...
std::unique_ptr<ParsedNetLog> ReadNetLogFromDisk(
  ...
  // TODO(eroman): Should return nullptr on failure for consistency.
  EXPECT_TRUE(result->InitFromFileContents(input));
  return result;
}
...
class FileNetLogObserverBoundedTest : public ::testing::Test {
 public:
  ...
  // Returns the path for where a particular numbered event file is saved (this
  // is an internal detail of FileNetLogObserver).
...
TEST_F(FileNetLogObserverBoundedTest, EventFileFailsToOpen) {
  ...
  std::unique_ptr<ParsedNetLog> log = ReadNetLogFromDisk(log_path_);
  ASSERT_TRUE(log);
  // The log file has no events, because creation of events file was blocked.
  EXPECT_EQ(0u, log->events->GetSize());
  ...
  // to reasonable.
  EXPECT_FALSE(base::PathExists(GetInternalEventDirectory()));
}
...
TEST_F(FileNetLogObserverBoundedTest, InternalEventDirectoryBlocked) {
  ...
  // FileNetLogObserver unconditionally deletes the event directory (even though
  // it didn't actually create this file).
  EXPECT_FALSE(base::PathExists(GetInternalEventDirectory()));
}
