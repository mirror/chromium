...
class AndroidNetworkLibrary {
    ...
    /**
     * Class to wrap TrafficStats.setThreadStatsUid(int uid) which is hidden and so must be accessed
     * via reflection.
     */
    private static class ThreadStatsUid {
        /**
         * A UID value that indicates no UID should be set. Matches:
        ...
        // Get reference to TrafficStats.setThreadStatsUid(int uid) via reflection.
        static {
            try {
                sSetThreadStatsUid =
    ...
    private static void tagSocket(int fd, int uid, int tag)
        ...
        if (uid != ThreadStatsUid.UNSET_UID) {
        ...
        if (uid != ThreadStatsUid.UNSET_UID) {
            ThreadStatsUid.set(ThreadStatsUid.UNSET_UID);
        }
    }
}
