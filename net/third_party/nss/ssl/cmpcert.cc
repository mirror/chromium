...
bool GetIssuerAndSubject(CERTCertificate* cert,
  ...
  *issuer = base::StringPiece(
      reinterpret_cast<const char*>(cert->derIssuer.data), cert->derIssuer.len);
  *subject =
      base::StringPiece(reinterpret_cast<const char*>(cert->derSubject.data),
                        cert->derSubject.len);
  return true;
}

// XXX move to x509_util ?
bool GetIssuerAndSubject(X509Certificate* cert,
  ...
  *issuer = tbs.issuer_tlv.AsStringPiece();
  *subject = tbs.subject_tlv.AsStringPiece();
  return true;
#else
...
bool MatchClientCertificateIssuers(
  ...
  while (intermediates->size() < kMaxDepth) {
    ...
    SECItem issuer_item;
    issuer_item.len = static_cast<unsigned int>(issuer.length());
    issuer_item.data =
        reinterpret_cast<unsigned char*>(const_cast<char*>(issuer.data()));
    ...
    intermediates->emplace_back(std::move(nextcert));
  }
