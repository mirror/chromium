...
PacketNumberQueue::const_iterator::const_iterator(
    const PacketNumberQueue::const_iterator& it)
    : vector_it_(it.vector_it_),
      deque_it_(it.deque_it_),
      use_deque_it_(it.use_deque_it_) {}
...
PacketNumberQueue::const_reverse_iterator::const_reverse_iterator(
    const PacketNumberQueue::const_reverse_iterator& it)
    : vector_it_(it.vector_it_),
      deque_it_(it.deque_it_),
      use_deque_it_(it.use_deque_it_) {}
...
void PacketNumberQueue::Add(QuicPacketNumber packet_number) {
  if (use_deque_) {
    ...
    size_t num_packets = static_cast<int>(packet_number_deque_.size());
    // Iterating through the queue backwards
    // to find a proper place for the packet
    while (num_packets > 0) {
      size_t i = num_packets - 1;
      ...
      if (packet_number_deque_[i].max() == packet_number) {
        ...
        if (i < packet_number_deque_.size() - 1 &&
