...
bool QuicConnection::OnStreamFrame(const QuicStreamFrame& frame) {
  ...
  // Stream frame is received in the current packet. Probing packet only
  // contains a PING and full padding.
  SetPacketContent(NOT_PADDED_PING);
...
bool QuicConnection::OnAckFrame(const QuicAckFrame& incoming_ack) {
  ...
  // Ack frame is received in the current packet. Probing packet only
  // contains a PING and full padding.
  SetPacketContent(NOT_PADDED_PING);
...
bool QuicConnection::OnStopWaitingFrame(const QuicStopWaitingFrame& frame) {
  ...
  // Stop waiting frame is received in the current packet. Probing packet only
  // contains a PING and full padding.
  SetPacketContent(NOT_PADDED_PING);
...
bool QuicConnection::OnPaddingFrame(const QuicPaddingFrame& frame) {
  DCHECK(connected_);
  SetPacketContent(SECOND_FRAME_IS_PADDING);
...
bool QuicConnection::OnPingFrame(const QuicPingFrame& frame) {
  DCHECK(connected_);
  SetPacketContent(FIRST_FRAME_IS_PING);
...
bool QuicConnection::OnRstStreamFrame(const QuicRstStreamFrame& frame) {
  ...
  // Reset stream frame is received in the current packet. Probing packet only
  // contains a PING and full padding.
  SetPacketContent(NOT_PADDED_PING);
...
bool QuicConnection::OnConnectionCloseFrame(
  ...
  // Connection close frame is received in the current packet. Probing packet
  // only contains a PING and full padding.
  SetPacketContent(NOT_PADDED_PING);
...
bool QuicConnection::OnGoAwayFrame(const QuicGoAwayFrame& frame) {
  ...
  // GoAway frame is received in the current packet. Probing packet only
  // contains a PING and full padding.
  SetPacketContent(NOT_PADDED_PING);
...
bool QuicConnection::OnWindowUpdateFrame(const QuicWindowUpdateFrame& frame) {
  ...
  // Window update frame is received in the current packet. Probing packet only
  // contains a PING and full padding.
  SetPacketContent(NOT_PADDED_PING);
...
bool QuicConnection::OnBlockedFrame(const QuicBlockedFrame& frame) {
  ...
  // Blocked frame is received in the current packet. Probing packet only
  // contains a PING and full padding.
  SetPacketContent(NOT_PADDED_PING);
...
void QuicConnection::SetPacketContent(PacketContent type) {
  if (current_packet_content_ == NOT_PADDED_PING) {
    // We have already learned the current packet is not a connectivity probing
    // packet. Peer migration should have already been started earlier if
    // needed.
    return;
  }
