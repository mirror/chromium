...
string SpdyUtils::GetPromisedUrlFromHeaderBlock(
  ...
  // safe.
  //
  // Since the CONNECT method is not safe, it cannot be the method of a
  // PUSH_PROMISE. Therefore, the header must include one valid value for the
  // ":method", ":scheme", and ":path" pseudo-header fields.
  SpdyHeaderBlock::const_iterator it = headers.find(":method");
  if (it == headers.end() || it->second == "CONNECT") {
    return string();
  }
