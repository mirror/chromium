...
namespace {
  ...
  /*const QuicStreamId kServerDataStreamId1 = kHeadersStreamId + 1;*/
} // namespace
 ...
 protected:
  ...
  static size_t GetStreamFrameDataLengthFromPacketLength(
    ...
    size_t min_packet_length =
        NullEncrypter(Perspective::IS_CLIENT).GetCiphertextSize(min_data_length) +
  ...
  void Initialize() {
    ...
    connection_ =
        new QuicConnection(connection_id_,
                           QuicSocketAddress(QuicSocketAddressImpl(peer_addr_)),
                           helper_.get(),
                           alarm_factory_.get(),
                           writer,
                           true /* owns_writer */,
    ...
    QuicConnectionPeer::SetSendAlgorithm(connection_, send_algorithm_);
...
/*QuicServerId(kProxyHost, kProxyPort, PRIVACY_MODE_DISABLED),*/
  ...
  std::unique_ptr<QuicReceivedPacket> ConstructAckAndRstPacket(
    ...
    return client_maker_.MakeAckAndRstPacket(packet_number, !kIncludeVersion,
        kClientDataStreamId1, error_code, largest_received, smallest_received,
        least_unacked, kSendFeedback);
  }
  ...
  std::unique_ptr<QuicReceivedPacket> ConstructAckAndRstPacket(
    ...
    return client_maker_.MakeAckAndRstPacket(packet_number, !kIncludeVersion,
        kClientDataStreamId1, error_code, largest_received, smallest_received,
        least_unacked, kSendFeedback, bytes_written);
  }
  ...
  std::unique_ptr<QuicReceivedPacket> ConstructAckAndDataPacket(
      ...
      const char* data,
      int length) {
    return client_maker_.MakeAckAndDataPacket(packet_number, !kIncludeVersion,
        kClientDataStreamId1, largest_received, smallest_received,
        least_unacked, !kFin, offset, QuicStringPiece(data, length));
  }
  ...
  std::unique_ptr<QuicReceivedPacket> ConstructAckPacket(
    ...
    return client_maker_.MakeAckPacket(packet_number, largest_received,
        smallest_received, least_unacked, kSendFeedback);
  }
  ...
  std::unique_ptr<QuicReceivedPacket> ConstructConnectReplyPacket(
      QuicPacketNumber packet_number,
  ...
  std::unique_ptr<QuicReceivedPacket> ConstructConnectAuthReplyPacket(
      QuicPacketNumber packet_number,
      /*QuicStreamId stream_id,*/
      bool fin
      /*size_t* spdy_headers_frame_length*/) {
  ...
  std::unique_ptr<QuicReceivedPacket> ConstructConnectRedirectReplyPacket(
      QuicPacketNumber packet_number,
      bool fin) {
    SpdyHeaderBlock block;
    block[":status"] = "302";
  ...
  std::unique_ptr<QuicReceivedPacket> ConstructConnectErrorReplyPacket(
      QuicPacketNumber packet_number,
      bool fin) {
    SpdyHeaderBlock block;
    block[":status"] = "500";
  ...
  void AssertAsyncReadEquals(const char* data, int len) {
    ...
    ASSERT_EQ(ERR_IO_PENDING,
              sock_->Read(buf.get(), len, callback.callback()));
    EXPECT_TRUE(sock_->IsConnected());

    ResumeAndRun();

    EXPECT_EQ(len, callback.WaitForResult());
    EXPECT_TRUE(sock_->IsConnected());
    ASSERT_EQ(SpdyString(data, len), SpdyString(buf->data(), len));
  }
  ...
  /*std::unique_ptr<MockRead[]> mock_reads_;
  ...
  // Used only by AssertReadStarts(), AssertReadReturns(), and the tests that
  // call them.
  TestCompletionCallback read_callback_;
...
TEST_P(QuicProxyClientSocketTest, ConnectSendsCorrectRequest) {
  ...
  mock_quic_data_.AddRead(ConstructConnectReplyPacket(1, !kFin));
...
TEST_P(QuicProxyClientSocketTest, ConnectWithAuthRequested) {
  mock_quic_data_.AddWrite(ConstructSettingsPacket(1));
  mock_quic_data_.AddWrite(ConstructConnectRequestPacket(2));
  mock_quic_data_.AddRead(ConstructConnectAuthReplyPacket(1, !kFin));
  mock_quic_data_.AddRead(SYNCHRONOUS, ERR_IO_PENDING);
  mock_quic_data_.AddWrite(
      ConstructAckAndRstPacket(3, QUIC_STREAM_CANCELLED, 1, 1, 1));
...
TEST_P(QuicProxyClientSocketTest, ConnectWithAuthCredentials) {
  ...
  http_auth_cache_.Add(GURL(kProxyUrl), "MyRealm1",
...
TEST_P(QuicProxyClientSocketTest, ConnectRedirects) {
  mock_quic_data_.AddWrite(ConstructSettingsPacket(1));
  mock_quic_data_.AddWrite(ConstructConnectRequestPacket(2));
  mock_quic_data_.AddRead(ConstructConnectRedirectReplyPacket(1, !kFin));
  mock_quic_data_.AddRead(SYNCHRONOUS, ERR_IO_PENDING);
  mock_quic_data_.AddWrite(
      ConstructAckAndRstPacket(3, QUIC_STREAM_CANCELLED, 1, 1, 1));
...
TEST_P(QuicProxyClientSocketTest, GetPeerAddressReturnsCorrectValues) {
  ...
  mock_quic_data_.AddWrite(ConstructConnectRequestPacket(2));
  mock_quic_data_.AddRead(ConstructConnectReplyPacket(1, !kFin));
  mock_quic_data_.AddRead(ASYNC, ERR_IO_PENDING); // is this righ???????? causes SequencedSocketData to pause
  mock_quic_data_.AddRead(ASYNC, 0);  // EOF
...
TEST_P(QuicProxyClientSocketTest, WriteSendsDataInDataFrame) {
  ...
  mock_quic_data_.AddRead(ConstructConnectReplyPacket(1, !kFin));
  mock_quic_data_.AddRead(SYNCHRONOUS, ERR_IO_PENDING);
  mock_quic_data_.AddWrite(ConstructAckAndDataPacket(3, 1, 1, 1,
                                                     0, kMsg1, kLen1));
  mock_quic_data_.AddWrite(ConstructClientDataPacket(4, kLen1, kMsg2, kLen2));
  mock_quic_data_.AddWrite(ConstructRstPacket(5, QUIC_STREAM_CANCELLED,
                                              kLen1 + kLen2));
...
TEST_P(QuicProxyClientSocketTest, WriteSplitsLargeDataIntoMultiplePackets) {
  ...
  for (int i = 0; i < kNumDataPackets; ++i) {
    ...
    mock_quic_data_.AddWrite(ConstructClientDataPacket(4 + i, offset,
                                                 data.c_str(), data_length));
    data_lengths.push_back(data_length);
    offset += data_length;
    total_data_length += data_length;
  }
  mock_quic_data_.AddWrite(ConstructRstPacket(4 + kNumDataPackets,
                                              QUIC_STREAM_CANCELLED, offset));

  Initialize();

  AssertConnectSucceeds();

  // Make small write. An ACK and STOP_WAITING will be bundled. This prevents
...
TEST_P(QuicProxyClientSocketTest, ReadDataFromBufferedFrames) {
  ...
  mock_quic_data_.AddWrite(ConstructAckAndRstPacket(4, QUIC_STREAM_CANCELLED,
                                                    3, 3, 1));
...
TEST_P(QuicProxyClientSocketTest, MultipleShortReadsThenMoreRead) {
  ...
  mock_quic_data_.AddWrite(ConstructAckAndRstPacket(5, QUIC_STREAM_CANCELLED,
                                                    5, 5, 1));
...
TEST_P(QuicProxyClientSocketTest, MultipleReadsFromSameLargeFrame) {
  ...
  TestCompletionCallback callback;
  ASSERT_EQ(kLen3, sock_->Read(buf.get(), kLen33, callback.callback()));
...
TEST_P(QuicProxyClientSocketTest, ReadErrorResponseBody) {
  mock_quic_data_.AddWrite(ConstructSettingsPacket(1));
  mock_quic_data_.AddWrite(ConstructConnectRequestPacket(2));
  mock_quic_data_.AddRead(ConstructConnectErrorReplyPacket(1, !kFin));
...
TEST_P(QuicProxyClientSocketTest, AsyncReadAroundWrite) {
  ...
  mock_quic_data_.AddWrite(ConstructClientDataPacket(4, 0, kMsg2, kLen2));

  mock_quic_data_.AddRead(ASYNC, ERR_IO_PENDING); // Pause

  mock_quic_data_.AddRead(ConstructServerDataPacket(3, kLen1, kMsg3, kLen3)); // async read
  mock_quic_data_.AddRead(SYNCHRONOUS, ERR_IO_PENDING);
  ...
  ResumeAndRun();   // seq 4, 5, will complete
  ...
  AssertSyncWriteSucceeds(kMsg2, kLen2);  // 6 completes, 7 starts

  ASSERT_FALSE(read_callback_.have_result());
  ResumeAndRun(); // 8, 9 complete
...
TEST_P(QuicProxyClientSocketTest, AsyncWriteAroundReads) {
  ...
  mock_quic_data_.AddAsyncWrite(ConstructClientDataPacket(4, 0, kMsg2, kLen2));
  mock_quic_data_.AddWrite(ConstructAckAndDataPacket(5, 3, 3, 1,
                                                     kLen2, kMsg2, kLen2));

  mock_quic_data_.AddWrite(ConstructRstPacket(6, QUIC_STREAM_CANCELLED,
                                              kLen2 + kLen2));
  ...
  ResumeAndRun(); // seq 4, 5 will complete, 6 begins
  AssertSyncReadEquals(kMsg1, kLen1);
  ...
  AssertAsyncReadEquals(kMsg3, kLen3);  // 7, 8 will complete, 9 begins, read_callback_ runs

  ASSERT_FALSE(write_callback_.have_result());

  // Now the write will complete
  ResumeAndRun(); // 10, 11 will complete 
...
TEST_P(QuicProxyClientSocketTest, ReadOnDisconnectSocketReturnsNotConnected) {
  ...
  ASSERT_EQ(ERR_SOCKET_NOT_CONNECTED, sock_->Read(nullptr, 1,
                                                  CompletionCallback()));
}
