...
class NET_EXPORT_PRIVATE QuicStreamRequest {
 public:
  ...
  // requires host resolution and it hasn't completed yet. If true is returned,
  // |callback| will run when host resolution completes. It will be called with
  // the result after host resolution during the connection process. For
  // example, if host resolution returns OK and then crypto handshake
