...
class QuicStreamFactory::Job {
 ...
 private:
  ...
  bool notify_requests_host_resolution_success_;
...
QuicStreamFactory::Job::Job(QuicStreamFactory* factory,
    ...
    : io_state_(STATE_RESOLVE_HOST),
      ...
      session_(nullptr),
      notify_requests_host_resolution_success_(false),
      weak_factory_(this) {
  net_log_.BeginEvent(
...
int QuicStreamFactory::Job::DoLoop(int rv) {
  ...
  if (notify_requests_host_resolution_success_) {
    // Only notify requests of host resolution success if the job hasn't ended
    // yet.
    if (rv == ERR_IO_PENDING) {
      for (QuicStreamRequest* request : stream_requests_) {
        request->OnHostResolutionSuccess();
      }
    }
    notify_requests_host_resolution_success_ = false;
  }
...
int QuicStreamFactory::Job::DoResolveHostComplete(int rv) {
  ...
  notify_requests_host_resolution_success_ = true;
...
QuicStreamRequest::QuicStreamRequest(QuicStreamFactory* factory)
    : factory_(factory), expect_host_resolution_(false) {}
...
int QuicStreamRequest::Request(const HostPortPair& destination,
  ...
  DCHECK(host_resolution_success_callback_.is_null());
  DCHECK(factory_);
...
void QuicStreamRequest::OnRequestComplete(int rv) {
  ...
  host_resolution_success_callback_.Reset();
  factory_ = nullptr;
  base::ResetAndReturn(&callback_).Run(rv);
}

void QuicStreamRequest::OnHostResolutionSuccess() {
  if (expect_host_resolution_) {
    expect_host_resolution_ = false;
    if (!host_resolution_success_callback_.is_null()) {
      base::ResetAndReturn(&host_resolution_success_callback_).Run();
    }
  }
}
