  ...
  int GetSourcePortForNewSessionInner(const HostPortPair& destination,
    ...
    EXPECT_EQ(ERR_IO_PENDING,
              request.Request(
                  ...
                  base::OnceClosure(), callback_.callback()));
  ...
  void VerifyInitialization() {
    ...
    EXPECT_EQ(ERR_IO_PENDING,
              request.Request(quic_server_id.host_port_pair(), version_,
                              ...
                              &net_error_details_, base::OnceClosure(),
                              callback_.callback()));
    EXPECT_THAT(callback_.WaitForResult(), IsOk());
...
TEST_P(QuicStreamFactoryTest, Create) {
  ...
  EXPECT_EQ(ERR_IO_PENDING,
            request.Request(host_port_pair_, version_, privacy_mode_,
                            DEFAULT_PRIORITY, /*cert_verify_flags=*/0, url_,
                            net_log_, &net_error_details_, base::OnceClosure(),
                            callback_.callback()));
...
TEST_P(QuicStreamFactoryTest, PoolByOrigin) {
  ...
  EXPECT_EQ(OK, request2.Request(destination2, version_, privacy_mode_,
                                 DEFAULT_PRIORITY, /*cert_verify_flags=*/0,
                                 url_, net_log_, &net_error_details_,
                                 base::OnceClosure(), callback2.callback()));
  std::unique_ptr<HttpStream> stream2 = CreateStream(&request2);
  EXPECT_TRUE(stream2.get());
...
TEST_P(QuicStreamFactoryTest, HostResolverUsesRequestPriority) {
  ...
  EXPECT_EQ(ERR_IO_PENDING,
            request.Request(host_port_pair_, version_, privacy_mode_,
                            MAXIMUM_PRIORITY, /*cert_verify_flags=*/0, url_,
                            net_log_, &net_error_details_, base::OnceClosure(),
                            callback_.callback()));
...
// Checks that the |host_resolution_callback| passed to Request() is executed
// only after host resolution is finished. Also makes sure that
// |host_resolution_callback| is executed regardless of errors after host
// resolution (in this case, getting ERR_FAILED on socket reads/writes).
TEST_P(QuicStreamFactoryTest, HostResolutionCallback) {
  Initialize();
  ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
  crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
  ...
  EXPECT_EQ(ERR_IO_PENDING,
            request.Request(
                ...
                /*cert_verify_flags=*/0, url_, net_log_, &net_error_details_,
                host_resolution_callback.closure(), callback_.callback()));
