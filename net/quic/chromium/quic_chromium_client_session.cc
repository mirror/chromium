...
void QuicChromiumClientSession::OnProbeNetworkSucceeded(
  ...
  if (network == default_network_) {
    ...
    DVLOG(2) << "Client successfully got off default network after "
  ...
  if (stream_factory_) {
    ...
    if (migrate_session_on_network_change_v2_) {
      ...
      if (alternate_network != NetworkChangeNotifier::kInvalidNetworkHandle) {
        // Probe alternative network, we will migrate to the probed network on
        // success.
...
void QuicChromiumClientSession::TryMigrateBackToDefaultNetwork(
  ...
  if (probing_manager_.current_probing_network() != default_network_) {
    probing_manager_.CancelProbingIfAny();
