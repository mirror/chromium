...
TEST_P(QuicNetworkTransactionTest, QuicProxyAuth) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    server_maker.reset(new QuicTestPacketMaker(
      version_, 0, &clock_, kDefaultServerHostName, Perspective::IS_SERVER,
      false)); 
    ...
    mock_quic_data.AddWrite(                                          // seq 0
      client_maker->MakeInitialSettingsPacket(1, &client_header_stream_offset));
    
    mock_quic_data.AddWrite(                                          // seq 1
        client_maker->MakeRequestHeadersPacketWithOffsetTracking(
            2, GetNthClientInitiatedStreamId(0), true, false, default_priority,
            client_maker->ConnectRequestHeaders("CONNECT", "mail.example.org:443"),
            0, &client_header_stream_offset));
    ...
    mock_quic_data.AddWrite(client_maker->MakeRstPacket(               // seq 5
        4, false, GetNthClientInitiatedStreamId(0), QUIC_STREAM_CANCELLED,
        client_data_offset));

    headers =
        client_maker->ConnectRequestHeaders("CONNECT", "mail.example.org:443");
    headers["proxy-authorization"] = "Basic Zm9vOmJheg==";
    mock_quic_data.AddWrite(                                          // seq 6
        client_maker->MakeRequestHeadersPacketWithOffsetTracking(
    ...
    headers["content-length"] = "10";
    mock_quic_data.AddRead(
        server_maker->MakeResponseHeadersPacketWithOffsetTracking(    // seq 7
            3, GetNthClientInitiatedStreamId(1), false, false,
            std::move(headers), &server_header_stream_offset));

    mock_quic_data.AddRead(SYNCHRONOUS, ERR_IO_PENDING);  // read hangs
