...
// Send a CONNECT request to a QUIC proxy and receive a 500 response.
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    mock_quic_data.AddWrite(
        client_maker->MakeRequestHeadersPacketWithOffsetTracking(
            ...
            client_maker->ConnectRequestHeaders("CONNECT",
    ...
    TestCompletionCallback callback;
    int rv =
        trans.RestartWithAuth(AuthCredentials(kFoo, kBaz), callback.callback());
    EXPECT_EQ(ERR_IO_PENDING, rv);
    EXPECT_EQ(OK, callback.WaitForResult());
