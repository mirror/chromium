...
TEST_P(QuicChromiumClientSessionTest, IsFatalErrorSetCorrectly) {
  ...
  details.cert_verify_result.verified_cert =
      ImportCertFromFile(GetTestCertsDirectory(), "spdy_pooling.pem");
  details.cert_verify_result.cert_status = ERR_CERT_DATE_INVALID;
  CompleteCryptoHandshake();
  session_->OnProofVerifyDetailsAvailable(details);

  ASSERT_TRUE(session_->GetSSLInfo(&ssl_info));
  ASSERT_FALSE(ssl_info.is_fatal_cert_error);

  const base::Time expiry =
      base::Time::Now() + base::TimeDelta::FromSeconds(1000);
  transport_security_state_.AddHSTS(session_->server_id().host(), expiry, true);
  ASSERT_TRUE(session_->GetSSLInfo(&ssl_info));
  ASSERT_TRUE(ssl_info.is_fatal_cert_error);
}
