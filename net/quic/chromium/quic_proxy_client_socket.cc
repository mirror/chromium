...
if (rv >= 0) {
  printf("\n\nSYNC complete QuicProxyClientSocket::Read():\n");
...
printf("\n\nASYNC complete QuicProxyClientSocket::Read():\n");
for (int i = 0; i < rv; ++i) {
  putchar(read_buf_->data()[i]);
}
putchar('\n');
    read_buf_ = nullptr;
