...
CookieMonster::CookieMonster(PersistentCookieStore* store,
  ...
  if (channel_id_service_ && store_) {
    ...
    //
    // TODO(nharper): I think there's still a race condition here, where the
    // PersistentCookieStore calls Run on the callback immediately before the
    // CookieMonster d'tor runs (and the ChannelIDStore is destroyed immediately
    // after the CookieMonster is destroyed).
