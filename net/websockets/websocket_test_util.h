...
// Fake WebSocketHandshakeStreamBase implementation that ends up being quite
// "real" because this object has to really send data on the mock socket. It
// might be easier to use the real implementation, but the fact that the
// WebSocket code is not compiled on iOS makes that difficult.
