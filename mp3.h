...
template<class T> class RemotableObject : public RemotableObjectBase {
    public:
    template<typename...Args> RemotableObject(Args... args) : t_(args...),
        task_runner_(base::SequencedTaskRunnerHandle::Get()),
       weak_factory_(&t_) {
    }

    // permit calls inline.  note that, since a RemotableObject can't be sent
    // except as a wp, and wps don't let you get to the underlying
    // RemotableObject, this effectively means that only the original owner
...
template<class T> class weak_ptr {
    public:
        ...
        weak_ptr(RemotableObject<T>& thiz) : task_runner_(thiz.task_runner_), wp_(thiz.weak_factory_.GetWeakPtr()) {}
        weak_ptr(RemotableObject<T>* thiz) : task_runner_(thiz->task_runner_), wp_(thiz->weak_factory_.GetWeakPtr()) {}
...
void fn() {
    ...
    threadsafe::RemotableObject<Foo> foo;
