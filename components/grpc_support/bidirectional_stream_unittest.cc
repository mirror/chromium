...
TEST_P(BidirectionalStreamTest, StartExampleBidiStream) {
  ...
  bidirectional_stream_delay_request_headers_until_flush(test.stream,
                                                         GetParam());
  bidirectional_stream_start(test.stream,
                             net::QuicSimpleTestServer::kTestServerUrl, 0,
                             "POST", &kTestHeadersArray, false);
  test.BlockForDone();
  ASSERT_EQ(std::string(net::QuicSimpleTestServer::kHelloStatus),
            test.response_headers[net::QuicSimpleTestServer::kStatusHeader]);
  ASSERT_EQ(std::string(net::QuicSimpleTestServer::kHelloHeaderValue),
            test.response_headers[net::QuicSimpleTestServer::kHelloHeaderName]);
  ASSERT_EQ(TestBidirectionalStreamCallback::ON_SUCCEEDED, test.response_step);
  ASSERT_EQ(std::string(net::QuicSimpleTestServer::kHelloBodyValue, 2),
            test.read_data.front());
  // Verify that individual read data joined using empty separator match
  // expected body.
  ASSERT_EQ(std::string(net::QuicSimpleTestServer::kHelloBodyValue),
            base::StrCat(test.read_data));
  ASSERT_EQ(
      std::string(net::QuicSimpleTestServer::kHelloTrailerValue),
      test.response_trailers[net::QuicSimpleTestServer::kHelloTrailerName]);
