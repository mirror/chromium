...
void InfoBarDelegate::OnNavigation(const NavigationDetails& details) {
  if (!launch_entry_id_) {
    launch_entry_id_ = details.entry_id;
  } else if (launch_entry_id_ != details.entry_id) {
    navigated_away_from_launch_url_ = true;
  }
}
