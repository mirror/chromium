...
const char kNELFieldTrialName[] = "NEL";
// Name of boolean to enable Reporting API.
const char kNELEnable[] = "enable";
...
void URLRequestContextConfig::ParseAndSetExperimentalOptions(
  ...
  for (base::DictionaryValue::Iterator it(*dict.get()); !it.IsAtEnd();
    ...
    if (it.key() == kQuicFieldTrialName) {
    ...
    } else if (it.key() == kNELFieldTrialName) {
      ...
      nel_args->GetBoolean(kNELEnable, &nel_enable);
  ...
  if (nel_enable) {
    ...
    policy->clear_reports_on_network_changes = true;
    policy->clear_clients_on_network_changes = true;
