 ...
 public:
  ...
  class Delegate {
   public:
    ...
    virtual void OnSucceeded(int64 received_byte_count) = 0;
    ...
    virtual void OnMetricsCollected(const base::Time& request_start_time,
                                    ...
                                    bool socket_reused,
                                    int64 sent_bytes_count,
                                    int64 received_bytes_count) = 0;
  };
  ...
  bool AddRequestHeader(const std::string& jname, const std::string& value);
  ...
  // Reads more data.
  bool ReadData(scoped_refptr<net::IOBuffer> buffer, int max_bytes);
