...
class CronetMetricsDelegate : public net::MetricsDelegate {
 public:
  ...
  void OnStopNetRequest(net::Metrics metrics) override;
  ...
  // client.
  static net::Metrics MetricsForTask(NSURLSessionTask* task);

 private:
  static std::map<NSURLSessionTask*, net::Metrics> task_metrics_map_
