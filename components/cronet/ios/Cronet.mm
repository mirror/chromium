...
class CronetMetricsDelegateAdapter : public net::MetricsDelegate {
 public:
  void didFinishCollectingMetrics(NSURLRequest* request,
...
void CronetMetricsDelegateAdapter::didFinishCollectingMetrics(
  ...
  if (@available(iOS 10, *)) {
    ...
    [metrics
        setDomainLookupStartDate:ticksToDate(load_timing_info,
                                             load_timing_info.connect_timing
                                                 .dns_start)];
    [metrics
        setDomainLookupEndDate:ticksToDate(
                                   load_timing_info,
                                   load_timing_info.connect_timing.dns_end)];

    [metrics
        setConnectStartDate:ticksToDate(
                                load_timing_info,
                                load_timing_info.connect_timing.connect_start)];
    [metrics
        setSecureConnectionStartDate:ticksToDate(load_timing_info,
                                                 load_timing_info.connect_timing
                                                     .ssl_start)];
    [metrics
        setSecureConnectionEndDate:ticksToDate(load_timing_info,
                                               load_timing_info.connect_timing
                                                   .ssl_end)];
    [metrics
        setConnectEndDate:ticksToDate(
                              load_timing_info,
                              load_timing_info.connect_timing.connect_end)];

    [metrics setRequestStartDate:[NSDate dateWithTimeIntervalSince1970:
                                             load_timing_info.request_start_time
                                                 .ToDoubleT()]];
    [metrics setRequestEndDate:ticksToDate(load_timing_info,
                                           load_timing_info.send_end)];
    [metrics
        setResponseStartDate:ticksToDate(load_timing_info,
                                         load_timing_info.receive_headers_end)];
    [metrics
        setResponseEndDate:[NSDate
                               dateWithTimeIntervalSince1970:response_info
                                                                 .response_time
                                                                 .ToDoubleT()]];

    [metrics setNetworkProtocolName:
                 [NSString
                     stringWithUTF8String:response_info.alpn_negotiated_protocol
                                              .c_str()]];
    [metrics setProxyConnection:[NSString
                                    stringWithUTF8String:getProxy(response_info)
                                                             .c_str()]];
    ...
    for (id<CronetMetricsDelegate> delegate in gMetricsDelegates) {
      [delegate URLSession:nil task:nil didFinishCollectingMetrics:nil];
    }
  }
}
