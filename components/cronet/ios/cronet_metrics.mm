...
CronetTransactionMetrics* NativeToIOSMetrics(Metrics& metrics)
  ...
  transaction_metrics.networkProtocolName = base::SysUTF8ToNSString(
      net::HttpResponseInfo::ConnectionInfoToString(response_info.connection_info));
...
NSNumber* gTaskMetricsMapLock;
std::map<const NSURLSessionTask*, std::unique_ptr<Metrics>>
    CronetMetricsDelegate::task_metrics_map_;

std::unique_ptr<Metrics> CronetMetricsDelegate::MetricsForTask(
    const NSURLSessionTask* task) {
...
- (void)URLSession:(NSURLSession*)session
  ...
  NSLog(@"response obj: %@", metrics.transactionMetrics.firstObject.response);
