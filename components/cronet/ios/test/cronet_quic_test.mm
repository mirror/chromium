...
// Tests a request with enabled "enable_socket_optimization" QUIC experimental
// option.
TEST_F(QuicTest, RequestWithSocketOptimizationEnabled) {
  ...
  [Cronet setExperimentalOptions:
              @"{\"QUIC\" : {\"enable_socket_optimization\" : true} }"];
  StartCronet();
  ...
  ASSERT_TRUE([metrics.networkProtocolName containsString:@"quic"])
