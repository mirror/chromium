...
TEST_F(HttpTest, PostRequest) {
  ...
  NSString* request_string =
      [NSString stringWithFormat:@"Post Data %@", date_string];
  NSData* post_data = [request_string dataUsingEncoding:NSUTF8StringEncoding];
  ...
  ASSERT_STREQ(base::SysNSStringToUTF8(request_string).c_str(),
