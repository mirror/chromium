...
static const NSUInteger kRequestBodyBufferLength = 1024;
static const NSUInteger kLargeRequestBodyBufferLength =
    100 * kRequestBodyBufferLength;
static const NSInteger kRequestBodyWriteTimes = 16;
  ...
  switch (event) {
    case NSStreamEventHasSpaceAvailable: {
      if (_count < kRequestBodyWriteTimes) {
        uint8_t buffer[kRequestBodyBufferLength] = {0};
...
TEST_F(HttpTest, PostRequestWithBodyStream) {
  ...
  StreamBodyRequestDelegate* stream_delegate =
      [StreamBodyRequestDelegate alloc];
  ...
  ASSERT_EQ(nil, [delegate_ error]);
  NSString* response_body = [delegate_ responseBody];
  NSMutableString* request_body = [NSMutableString string];
  for (int i = 0; i < kRequestBodyWriteTimes; ++i) {
    uint8_t buffer[kRequestBodyBufferLength] = {0};
    memset(buffer, 'a' + i, kRequestBodyBufferLength);
    [request_body
        appendString:[[NSString alloc] initWithBytes:buffer
...
TEST_F(HttpTest, PostRequestWithLargeBodyStream) {
  ...
  uint8_t buffer[kLargeRequestBodyBufferLength] = {0};
