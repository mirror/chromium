...
class PerfTest : public CronetTestBase,
    ...
    for (auto const& entry : perf_test_results) {
      NSString* formatted_entry = [NSString
          stringWithFormat:
              ...
              8 * entry.second.total_bytes_downloaded / entry.second.total / 10e6,
 ...
 protected:
  ...
  void SetUp() override {
    ...
    if (kUseExternalUrl) {
      NSString* external_host = [[NSURL URLWithString:
          [NSString stringWithUTF8String:kExternalUrl]] host];
...
TEST_P(PerfTest, NSURLSessionReceivesImageLoop) {
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    if (kUseExternalUrl && i == 0) {
      LOG(INFO) << "Downloaded " << [delegate_ totalBytesReceivedPerTask][task] << " bytes on first iteration.";
    }
  ...
  perf_test_results[GetParam()] = {elapsed_total, elapsed_total / iterations,
                                   elapsed_max, total_bytes_received,
                                   failed_iterations};
