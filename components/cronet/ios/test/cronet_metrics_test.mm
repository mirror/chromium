...
TEST_F(CronetMetricsTest, ProtocolIsQuic) {
  if (@available(iOS 10, *)) {
    ...
    ASSERT_NE([[metrics domainLookupStartDate]
        compare:[metrics domainLookupEndDate]], NSOrderedDescending);
    ASSERT_NE([[metrics connectStartDate]
        compare:[metrics connectEndDate]], NSOrderedDescending);
    ASSERT_NE([[metrics secureConnectionStartDate]
        compare:[metrics secureConnectionEndDate]], NSOrderedDescending);
    ASSERT_NE([[metrics requestStartDate]
        compare:[metrics requestEndDate]], NSOrderedDescending);

    // TODO(lilyhoughton) This doesn't yet work.
    // I think it's because responseEndDate is populated from a field in
    // HttpResponseInfo of type base::Time, instead of base::TimeTicks - which
    // means it's not guaranteed to be monotonic; and the two times are so close
    // together that they end up getting mismeasured.
    // I'm not sure what to do about it, though.
    //ASSERT_NE([[metrics responseStartDate] compare:[metrics responseEndDate]], NSOrderedDescending);
...
TEST_F(CronetMetricsTest, PlatformComparison) {
  if (@available(iOS 10, *)) {
    ...
    ASSERT_TRUE([cronet_metrics.networkProtocolName isEqualToString:platform_metrics.networkProtocolName]);
  }
}
