...
class CronetDisabledMetricsTest : public CronetMetricsTest {
 protected:
  CronetDisabledMetricsTest() {}
  ~CronetDisabledMetricsTest() override {}
...
TEST_F(CronetMetricsTest, ProtocolIsQuic) {
  if (@available(iOS 10, *)) {
    ...
    EXPECT_NE(
        ...
        NSOrderedDescending);
    EXPECT_NE([[metrics connectStartDate] compare:[metrics connectEndDate]],
              NSOrderedDescending);
    EXPECT_NE([[metrics secureConnectionStartDate]
                  compare:[metrics secureConnectionEndDate]],
              NSOrderedDescending);
    EXPECT_NE([[metrics requestStartDate] compare:[metrics requestEndDate]],
              NSOrderedDescending);
    EXPECT_NE([[metrics responseStartDate] compare:[metrics responseEndDate]],
              NSOrderedDescending);

    EXPECT_FALSE([metrics isProxyConnection]);
...
TEST_F(CronetMetricsTest, PlatformComparison) {
  if (@available(iOS 10, *)) {
    ...
    EXPECT_TRUE([cronet_metrics.networkProtocolName
