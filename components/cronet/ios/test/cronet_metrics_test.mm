...
void LogMetrics(NSURLSessionTaskTransactionMetrics* metrics) {
  NSLog(@"fetchStartDate: %@", [metrics fetchStartDate]);
  NSLog(@"domainLookupStartDate: %@", [metrics domainLookupStartDate]);
  NSLog(@"domainLookupEndDate: %@", [metrics domainLookupEndDate]);
  NSLog(@"connectStartDate: %@", [metrics connectStartDate]);
  NSLog(@"secureConnectionStartDate: %@", [metrics secureConnectionStartDate]);
  NSLog(@"secureConnectionEndDate: %@", [metrics secureConnectionEndDate]);
  NSLog(@"connectEndDate: %@", [metrics connectEndDate]);
  NSLog(@"requestStartDate: %@", [metrics requestStartDate]);
  NSLog(@"requestEndDate: %@", [metrics requestEndDate]);
  NSLog(@"responseStartDate: %@", [metrics responseStartDate]);
  NSLog(@"responseEndDate: %@", [metrics responseEndDate]);
  NSLog(@"networkProtocolName: %@", [metrics networkProtocolName]);
  NSLog(@"proxyConnection: %i", [metrics isProxyConnection]);
  NSLog(@"reusedConnection: %i", [metrics isReusedConnection]);
  NSLog(@"resourceFetchType: %lu", [metrics resourceFetchType]);
}
...
TEST_F(CronetMetricsTest, ProtocolIsQuic) {
  if (@available(iOS 10, *)) {
    ...
    ASSERT_TRUE([metrics.secureConnectionStartDate
...
TEST_F(CronetMetricsTest, PlatformComparison) {
  if (@available(iOS 10, *)) {
    ...
    // Confirm that we set the right protocol name.
...
TEST_F(CronetMetricsTest, InvalidURL) {
  if (@available(iOS 10, *)) {
    NSURL* url = net::NSURLWithGURL(GURL("http://notarealurl"));
    ...
    // This is different on Cronet and platform
    // The platform stack has this be nil with a request to an invalid url
    // But the Cronet metrics stack sets it unconditionally.
    //ASSERT_FALSE([metrics responseEndDate]);
  }
}

TEST_F(CronetMetricsTest, PlatformCanceledRequest) {
  if (@available(iOS 10, *)) {
    NSURL* url = net::NSURLWithGURL(GURL(grpc_support::kTestServerSimpleUrl));
    //NSURL* url = net::NSURLWithGURL(GURL(TestServer::GetSimpleURL()));
    ...
    StartDataTaskAndWaitForCompletion(task, 100);
    [task cancel];
    ...
    NSURLSessionTaskMetrics* task_metrics = [delegate_ taskMetrics];
    EXPECT_FALSE(task_metrics);

    NSURLSessionTaskTransactionMetrics* metrics =
        task_metrics.transactionMetrics.firstObject;
    //ASSERT_TRUE([metrics isMemberOfClass:[CronetTransactionMetrics class]]);
    NSLog(@"class: %@", [metrics class]);
