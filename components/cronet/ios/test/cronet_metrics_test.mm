...
class CronetMetricsTest : public CronetTestBase {
 protected:
  ...
  void TearDown() override {
    ASSERT_FALSE([Cronet taskMetricsMapSizeForTesting]);
...
TEST_F(CronetMetricsTest, ReusedConnection) {
  if (@available(iOS 10, *)) {
    ...
    ASSERT_TRUE([platform_metrics isReusedConnection]);
    ASSERT_FALSE([platform_metrics domainLookupStartDate]);
    ASSERT_FALSE([platform_metrics domainLookupEndDate]);
    ASSERT_FALSE([platform_metrics connectStartDate]);
    ASSERT_FALSE([platform_metrics secureConnectionStartDate]);
    ASSERT_FALSE([platform_metrics secureConnectionEndDate]);
    ASSERT_FALSE([platform_metrics connectEndDate]);
  }
}
