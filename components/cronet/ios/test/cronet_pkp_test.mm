...
class PkpTest : public CronetTestBase {
 protected:
  void SetUp() override {
    ...
    server_host_ = base::SysUTF8ToNSString(quic_server()->host());
    server_domain_ = base::SysUTF8ToNSString(quic_server()->domain());
    request_url_ = net::NSURLWithGURL(GURL(quic_server()->simple_url()));
