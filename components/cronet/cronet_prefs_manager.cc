...
CronetPrefsManager::CronetPrefsManager(
    ...
    : network_qualities_prefs_manager_(nullptr),
      host_cache_persistence_manager_(nullptr) {
  ...
  context_builder->SetHttpServerProperties(
      std::unique_ptr<net::HttpServerPropertiesManager>(http_server_properties_manager_));
}
...
void CronetPrefsManager::CommitPendingWrites() {
  DCHECK(thread_checker_.CalledOnValidThread());
  if (pref_service_)
    pref_service_->CommitPendingWrite();
}
void CronetPrefsManager::ShutdownOnPrefSequence() {
  DCHECK(thread_checker_.CalledOnValidThread());
