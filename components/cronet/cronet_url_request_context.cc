...
class NetLogWithNetworkChangeEvents {
 public:
  ...
  void EnsureInitializedOnInitThread() {
    DCHECK(cronet::global_state::IsOnInitThread());
...
CronetURLRequestContext::~CronetURLRequestContext() {
  ...
  // Notify Delegate that network thread is getting destroyed, so it could clean
  // it up.
  delegate_->OnDestroyNetworkThread(this);
}
