...
// Struct Annotation.
TEST_F(CronetStructTest, TestAnnotation) {
  AnnotationPtr first = Annotation_Create();
  AnnotationPtr second = Annotation_Create();

  // Copy values from |first| to |second|.
  Annotation_Destroy(first);
  Annotation_Destroy(second);
}

// Struct RawBuffer.
TEST_F(CronetStructTest, TestRawBuffer) {
  RawBufferPtr first = RawBuffer_Create();
  RawBufferPtr second = RawBuffer_Create();

  // Copy values from |first| to |second|.
  // TODO(mef): Test array |data|.
  RawBuffer_Destroy(first);
  RawBuffer_Destroy(second);
}
...
TEST_F(CronetStructTest, TestUrlRequestParams) {
  ...
  // TODO(mef): Test substruct |annotation|.
