 ...
 public:
  ...
  class Delegate {
   public:
    ...
    // Invoked when this |context| about to be destoyed on |network_thread_|.
    virtual void OnDestroyNetworkThread(CronetURLRequestContext* context) = 0;

    // Invoked on |network_thread_| to initialze |cert_verifier| using
    // |cert_verifier_data|.
    virtual void OnInitCertVerifierData(
        net::CertVerifier* cert_verifier,
        const std::string& cert_verifier_data) = 0;

    // Invoked on |network_thread_| to save |cert_verifier| data for use in
  ...
  // Constructs CronetURLRequestContext using |context_config|. The |delegate|
  // is not owned by |this| and must outlive |this|. All |delegate| methods are
