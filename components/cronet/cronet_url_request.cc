...
CronetURLRequest::CronetURLRequest(CronetURLRequestContext* context,
  ...
  int load_flags(context->default_load_flags());
  if (disable_cache)
    load_flags |= net::LOAD_DISABLE_CACHE;
  if (disable_connection_migration)
    load_flags |= net::LOAD_DISABLE_CONNECTION_MIGRATION;
  network_tasks_ = std::make_unique<NetworkTasks>(
      std::move(callback), url, priority, load_flags, enable_metrics);
}
...
void CronetURLRequest::GetStatus() const {
  context_->PostTaskToNetworkThread(
      FROM_HERE, base::Bind(&CronetURLRequest::NetworkTasks::GetStatus,
                            base::Unretained(network_tasks_.get())));
}
...
bool CronetURLRequest::ReadData(net::IOBuffer* raw_read_buffer, int max_size) {
  ...
  context_->PostTaskToNetworkThread(
      FROM_HERE, base::Bind(&CronetURLRequest::NetworkTasks::ReadData,
                            base::Unretained(network_tasks_.get()), read_buffer,
                            max_size));
  return true;
}
...
CronetURLRequest::NetworkTasks::~NetworkTasks() {
  ...
  // TODO(mef): Delete owning CronetURLRequest?
  // callback_->OnDestroy();
}
...
void CronetURLRequest::NetworkTasks::GetStatus() const {
  DCHECK_CALLED_ON_VALID_THREAD(network_thread_checker_);
...
void CronetURLRequest::NetworkTasks::Destroy(CronetURLRequest* request,
  ...
  // Deleting owner request will also delete |this|.
  delete request;
}
