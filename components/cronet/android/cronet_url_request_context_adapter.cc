...
CronetURLRequestContextAdapter::~CronetURLRequestContextAdapter() {
  ...
  if (cronet_prefs_manager_) {
    // Commit pending writes before shutting down; otherwise, the changes
    // may be lost and the observers not called.
    cronet_prefs_manager_->CommitPendingWrites();
    cronet_prefs_manager_->ShutdownOnPrefSequence();
  }
  ...
  // Destroy |sdch_owner_| before |cronet_prefs_manager_|. That prevents
  // |sdch_owner_| from accessing destroyed JsonPrefStore owned by
  // |cronet_prefs_manager_|.
  sdch_owner_.reset(nullptr);

  // Destroy |cronet_prefs_manager_| before destroying
  // |network_quality_estimator_|. Otherwise, NetworkQualitiesPrefsManager,
  // that is owned by |cronet_prefs_manager_|, may call already destroyed
  // |network_quality_estimator_| through the weak reference.
  cronet_prefs_manager_.reset(nullptr);

  // Destroy |network_quality_estimator_| last.
  network_quality_estimator_.reset(nullptr);
