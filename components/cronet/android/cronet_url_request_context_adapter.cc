...
CronetURLRequestContextAdapter::CronetURLRequestContextAdapter(
    ...
    : context_(
          std::make_unique<CronetURLRequestContext>(std::move(context_config),
                                                    this)) {}
...
void CronetURLRequestContextAdapter::InitRequestContextOnInitThread(
  ...
  jcronet_url_request_context_tmp_.Reset(env, jcaller);
...
void CronetURLRequestContextAdapter::OnInitNetworkThread(
  ...
  jcronet_url_request_context_.Reset(env,
                                     jcronet_url_request_context_tmp_.obj());
  jcronet_url_request_context_tmp_.Reset(env, nullptr);
...
void CronetURLRequestContextAdapter::OnGetCertVerifierData(
    net::CertVerifier* cert_verifier) {
...
void CronetURLRequestContextAdapter::Destroy(
    ...
    const JavaParamRef<jobject>& jcaller) {
  context_->Destroy();
  // TODO(mef): We also need to destroy |this| on network thread.
  // Should Destroy() do that?
  // Probably not by default as this is wrong for Native.
  // Should we have an 'OnDestroy()' callback?
}
