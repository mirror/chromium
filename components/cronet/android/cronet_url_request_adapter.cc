...
CronetURLRequestAdapter::CronetURLRequestAdapter(
    ...
    jboolean jdisable_connection_migration,
    jboolean jenable_metrics)
    : request_(context->cronet_url_request_context(),
               this,
               url,
               priority,
...
void CronetURLRequestAdapter::GetStatus(
  ...
  // status_listener_ref_.Reset(env, jstatus_listener);
  request_.GetStatus();
}
...
void CronetURLRequestAdapter::OnReceivedRedirect(
  ...
  cronet::Java_CronetUrlRequest_onRedirectReceived(
      ...
      GetResponseHeaders(env, headers), was_cached ? JNI_TRUE : JNI_FALSE,
...
void CronetURLRequestAdapter::OnReadCompleted(
    ...
    int64_t received_byte_count) {
  scoped_refptr<IOBufferWithByteBuffer> read_buffer(
      static_cast<IOBufferWithByteBuffer*>(buffer.get()));
  JNIEnv* env = base::android::AttachCurrentThread();
...
CronetURLRequestAdapter::GetResponseHeaders(
    JNIEnv* env,
    const net::HttpResponseHeaders* headers) {
...
void CronetURLRequestAdapter::OnStatus(int load_status) {
  JNIEnv* env = base::android::AttachCurrentThread();
  cronet::Java_CronetUrlRequest_onStatus(env, owner_, status_listener_ref_,
                                         load_status);
  status_listener_ref_.Reset();
}
