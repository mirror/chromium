...
public class CronetEngineBuilderTest {
    ...
    public void testProviderOrdering() {
        final CronetProvider[] availableProviders = new CronetProvider[] {
                new FakeProvider(getTargetContext(), PROVIDER_NAME_APP_PACKAGED, "99.77", true),
                new FakeProvider(getTargetContext(), PROVIDER_NAME_FALLBACK, "99.99", true),
                new FakeProvider(getTargetContext(), "Some other provider", "99.88", true),
        };

        ArrayList<CronetProvider> providers = new ArrayList<>(Arrays.asList(availableProviders));
        List<CronetProvider> orderedProviders =
                CronetEngine.Builder.getEnabledCronetProviders(getTargetContext(), providers);
    ...
    public void testThatDisabledProvidersAreExcluded() {
        final CronetProvider[] availableProviders = new CronetProvider[] {
                new FakeProvider(getTargetContext(), PROVIDER_NAME_FALLBACK, "99.99", true),
                new FakeProvider(getTargetContext(), PROVIDER_NAME_APP_PACKAGED, "99.77", true),
                new FakeProvider(getTargetContext(), "Some other provider", "99.88", false),
        };
