...
TestUrlRequestCallback::UrlResponseInfo::UrlResponseInfo(
    ...
    : url(Cronet_UrlResponseInfo_get_url(response_info)),
      http_status_code(
          Cronet_UrlResponseInfo_get_httpStatusCode(response_info)),
      http_status_text(
          Cronet_UrlResponseInfo_get_httpStatusText(response_info)),
      was_cached(Cronet_UrlResponseInfo_get_wasCached(response_info)),
      negotiated_protocol(
          Cronet_UrlResponseInfo_get_negotiatedProtocol(response_info)),
      proxy_server(Cronet_UrlResponseInfo_get_proxyServer(response_info)),
      received_byte_count(
          Cronet_UrlResponseInfo_get_receivedByteCount(response_info)) {
  for (uint32_t url = 0;
       url < Cronet_UrlResponseInfo_get_urlChainSize(response_info); ++url) {
    url_chain.push_back(
        Cronet_UrlResponseInfo_get_urlChainAtIndex(response_info, url));
  }
  for (uint32_t i = 0;
       i < Cronet_UrlResponseInfo_get_allHeadersListSize(response_info); ++i) {
    Cronet_HttpHeaderPtr header =
        Cronet_UrlResponseInfo_get_allHeadersListAtIndex(response_info, i);
    all_headers.push_back(std::pair<std::string, std::string>(
        Cronet_HttpHeader_get_name(header),
        Cronet_HttpHeader_get_value(header)));
  }
}
...
Cronet_ExecutorPtr TestUrlRequestCallback::CreateExecutor() {
  executor_thread_ =
      std::make_unique<base::Thread>("TestUrlRequestCallback executor");
  executor_thread_->Start();
  Cronet_ExecutorPtr executor =
      Cronet_Executor_CreateStub(TestUrlRequestCallback::Execute);
  Cronet_Executor_SetContext(executor, this);
  return executor;
}
...
void TestUrlRequestCallback::Execute(Cronet_ExecutorPtr self,
  ...
  TestUrlRequestCallback* callback =
      static_cast<TestUrlRequestCallback*>(Cronet_Executor_GetContext(self));
  CHECK(callback);
  CHECK(callback->executor_thread_ || callback->allow_direct_executor_);
  // Post |command| onto executor thread.
  if (!callback->allow_direct_executor_) {
  ...
  // Run |command| directly as direct executor is allowed.
  Cronet_Runnable_Run(command);
  Cronet_Runnable_Destroy(command);
}
