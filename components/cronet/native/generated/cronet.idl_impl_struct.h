// Copyright 2017 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

/* DO NOT EDIT. Generated from components/cronet/native/generated/cronet.idl */

#ifndef COMPONENTS_CRONET_NATIVE_GENERATED_CRONET_IDL_IMPL_STRUCT_H_
#define COMPONENTS_CRONET_NATIVE_GENERATED_CRONET_IDL_IMPL_STRUCT_H_

#include "components/cronet/native/generated/cronet.idl_c.h"

#include <memory>
#include <string>
#include <vector>

#include "base/macros.h"

// Struct cr_Buffer.
struct cr_Buffer {
 public:
  cr_Buffer();
  ~cr_Buffer();

  uint32_t size = 0U;
  uint32_t limit = 0U;
  uint32_t position = 0U;
  RawDataPtr data;
  cr_BufferCallbackPtr callback;

 private:
  DISALLOW_COPY_AND_ASSIGN(cr_Buffer);
};

// Struct cr_CronetException.
struct cr_CronetException {
 public:
  cr_CronetException();
  ~cr_CronetException();

  cr_CronetException_ERROR_CODE error_code =
      cr_CronetException_ERROR_CODE_ERROR_CALLBACK;
  int32_t cronet_internal_error_code = 0;
  bool immediately_retryable = false;
  int32_t quic_detailed_error_code = 0;

 private:
  DISALLOW_COPY_AND_ASSIGN(cr_CronetException);
};

// Struct cr_QuicHint.
struct cr_QuicHint {
 public:
  cr_QuicHint();
  ~cr_QuicHint();

  std::string host;
  int32_t port = 0;
  int32_t alternatePort = 0;

 private:
  DISALLOW_COPY_AND_ASSIGN(cr_QuicHint);
};

// Struct cr_PublicKeyPins.
struct cr_PublicKeyPins {
 public:
  cr_PublicKeyPins();
  ~cr_PublicKeyPins();

  std::string host;
  std::vector<RawDataPtr> pinsSha256;
  bool includeSubdomains = false;

 private:
  DISALLOW_COPY_AND_ASSIGN(cr_PublicKeyPins);
};

// Struct cr_CronetEngineParams.
struct cr_CronetEngineParams {
 public:
  cr_CronetEngineParams();
  ~cr_CronetEngineParams();

  std::string userAgent;
  std::string storagePath;
  bool enableQuic = false;
  bool enableHttp2 = true;
  bool enableBrotli = true;
  cr_CronetEngineParams_HTTP_CACHE_MODE httpCacheMode =
      cr_CronetEngineParams_HTTP_CACHE_MODE_DISABLED;
  int64_t httpCacheMaxSize = 0;
  std::vector<std::unique_ptr<cr_QuicHint>> quicHints;
  std::vector<std::unique_ptr<cr_PublicKeyPins>> publicKeyPins;
  bool enablePublicKeyPinningBypassForLocalTrustAnchors = true;

 private:
  DISALLOW_COPY_AND_ASSIGN(cr_CronetEngineParams);
};

// Struct cr_HttpHeader.
struct cr_HttpHeader {
 public:
  cr_HttpHeader();
  ~cr_HttpHeader();

  std::string name;
  std::string value;

 private:
  DISALLOW_COPY_AND_ASSIGN(cr_HttpHeader);
};

// Struct cr_UrlResponseInfo.
struct cr_UrlResponseInfo {
 public:
  cr_UrlResponseInfo();
  ~cr_UrlResponseInfo();

  std::string url;
  std::vector<std::string> urlChain;
  int32_t httpStatusCode;
  std::string httpStatusText;
  std::vector<std::unique_ptr<cr_HttpHeader>> allHeadersList;
  bool wasCached;
  std::string negotiatedProtocol;
  std::string proxyServer;
  int64_t receivedByteCount;

 private:
  DISALLOW_COPY_AND_ASSIGN(cr_UrlResponseInfo);
};

// Struct cr_UrlRequestParams.
struct cr_UrlRequestParams {
 public:
  cr_UrlRequestParams();
  ~cr_UrlRequestParams();

  std::string httpMethod;
  std::vector<std::unique_ptr<cr_HttpHeader>> requestHeaders;
  bool disableCache = false;
  cr_UrlRequestParams_REQUEST_PRIORITY priority =
      cr_UrlRequestParams_REQUEST_PRIORITY_REQUEST_PRIORITY_MEDIUM;
  cr_UploadDataProviderPtr uploadDataProvider;
  cr_ExecutorPtr uploadDataProviderExecutor;
  bool allowDirectExecutor = false;
  std::vector<RawDataPtr> annotations;

 private:
  DISALLOW_COPY_AND_ASSIGN(cr_UrlRequestParams);
};

#endif  // COMPONENTS_CRONET_NATIVE_GENERATED_CRONET_IDL_IMPL_STRUCT_H_
