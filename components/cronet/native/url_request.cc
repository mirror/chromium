...
std::unique_ptr<Cronet_UrlResponseInfo> ConvertCronet_UrlResponseInfo(
  ...
  respone_info->url = url_chain.back();
  for (const auto& url : url_chain)
    respone_info->urlChain.push_back(url);
  respone_info->httpStatusCode = http_status_code;
  respone_info->httpStatusText = http_status_text;
...
void Cronet_UrlRequestImpl::Callback::OnReceivedRedirect(
  ...
  Cronet_RunnablePtr runnable = new cronet::OnceClosureRunnable(
      base::BindOnce(Cronet_UrlRequestCallback_OnRedirectReceived, callback_,
                     ...
                     url_request_->response_info_->urlChain.front().c_str()));
