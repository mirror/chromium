...
Cronet_RESULT Cronet_UrlRequestImpl::InitWithParams(
  ...
  if (!url || std::string(url).empty())
    return engine_impl->CheckResult(Cronet_RESULT_NULL_POINTER_URL);
  if (!callback)
    return engine_impl->CheckResult(Cronet_RESULT_NULL_POINTER_CALLBACK);
  if (!executor)
    return engine_impl->CheckResult(Cronet_RESULT_NULL_POINTER_EXECUTOR);
  ...
  for (const auto& request_header : params->requestHeaders) {
    if (request_header->name.empty())
      return engine_impl->CheckResult(Cronet_RESULT_NULL_POINTER_HEADER_NAME);
    if (request_header->value.empty())
      return engine_impl->CheckResult(Cronet_RESULT_NULL_POINTER_HEADER_VALUE);
    request_->AddRequestHeader(request_header->name, request_header->value);
  }
  return engine_impl->CheckResult(Cronet_RESULT_SUCCESS);
}
...
void Cronet_UrlRequestImpl::Callback::OnReadCompleted(
  ...
  Cronet_RunnablePtr runnable = new cronet::OnceClosureRunnable(base::BindOnce(
      ...
      response_info_.get(), url_request_->read_buffer_, bytes_read));
