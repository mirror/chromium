...
const char* kPartialUserAgent = "EngineTest/1";
...
TEST_F(EngineTest, StartCronetEngine) {
  ...
  Cronet_EngineParams_set_userAgent(engine_params, kPartialUserAgent);
...
TEST_F(EngineTest, CronetEngineDefaultUserAgent) {
  ...
  std::string default_agent =
      Cronet_Engine_GetDefaultUserAgent(engine, kPartialUserAgent);
  EXPECT_NE(default_agent.find(version), std::string::npos);
  EXPECT_NE(default_agent.find(kPartialUserAgent), std::string::npos);
  Cronet_Engine_Destroy(engine);
}
