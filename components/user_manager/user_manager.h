...
class USER_MANAGER_EXPORT UserManager {
 public:
  ...
  virtual bool IsDeviceLocalAccountMarkedForRemoval(const AccountId& account_id) const = 0;
  ...
  UserType CalculateUserType(const AccountId& account_id, const User* user, const bool browser_restart, const bool is_child) const;
