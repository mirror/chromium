...
void UserManagerBase::UserLoggedIn(const AccountId& account_id,
  ...
  const UserType user_type = CalculateUserType(account_id, user, browser_restart, is_child);
  ...
  if (IsDemoApp(account_id)) {
    ...
    switch(user_type) {
     ...
     case USER_TYPE_PUBLIC_ACCOUNT:
      PublicAccountUserLoggedIn(user ? user : User::CreatePublicAccountUser(account_id));
      break;
  ...
  for (std::vector<AccountId>::const_iterator it = regular_users.begin();
    ...
    if (IsSupervisedAccountId(*it)) {
      ...
      if (prefs_user_types->GetIntegerWithoutPathExpansion(it->GetUserEmail(),
        ...
        user = User::CreateRegularUser(*it, USER_TYPE_CHILD);
      } else {
        user = User::CreateRegularUser(*it, USER_TYPE_REGULAR);
      }
    }
...
void UserManagerBase::RegularUserLoggedIn(const AccountId& account_id, const UserType user_type) {
...
void UserManagerBase::RegularUserLoggedInAsEphemeral(
    const AccountId& account_id, const UserType user_type) {
  DCHECK(!task_runner_ || task_runner_->RunsTasksInCurrentSequence());
  SetIsCurrentUserNew(true);
