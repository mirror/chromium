...
ActivationList GetListForThreatTypeAndMetadata(
  ...
  if (is_phishing_interstitial) {
    ...
    // For now, hardcode when we have both ABUSIVE and BETTER_ADS. In the future
    // these will be separated.
    bool has_abusive = false;
    bool has_better_ads = false;
    for (const auto& it : threat_type_metadata.subresource_filter_match) {
      if (it.first == safe_browsing::SubresourceFilterType::ABUSIVE)
        has_abusive = true;
      else if (it.first == safe_browsing::SubresourceFilterType::BETTER_ADS)
        has_better_ads = true;
    }
