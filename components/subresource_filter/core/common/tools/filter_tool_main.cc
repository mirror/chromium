...
const char kHelpMsg[] = R"(
  ...
  Filtertool is a utility for querying a ruleset, and provides multiple
  commands:
    ...
    * match --origin=<origin> --url=<request_url> --type=<request_type>
        ...
        matching ruleset rule (if one matches). The output format is:
            <BLOCKED/ALLOWED> <UrlRule if any> <origin> <request_url> <type>
...
int main(int argc, char* argv[]) {
  ...
  CHECK(ruleset->length() > 0u) << "Empty ruleset file";
  ...
  if (cmd == "match") {
    ...
    const std::string origin = command_line.GetSwitchValueASCII(kSwitchOrigin);
    ...
    filter_tool.Match(origin, url, type);

    return 0;
  }

  if (cmd != "match_rules" && cmd != "match_batch") {
