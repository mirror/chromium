...
class FilterTool {
 public:
  ...
  // Checks the ruleset for a request with |origin|, |url|, and |type|. If a
  // blacklist rule matches the request, it is returned. If multiple blacklist
  // rules match, one is arbitrarily chosen and returned. If a blacklist rule
  // matches and a whitelist rule matches, a whitelist rule is returned.
  // The output is written to |output_| in a space-delimited line. The first
  // column is either BLOCKED or ALLOWED. The second is any matching rule. The
  // following columns are the input arguments.
  void Match(const std::string& origin,
             const std::string& url,
             const std::string& type);

  // Like Match, but for multiple requests. The requests are provided in
  // |request_stream| in form: "origin url type\n".
 ...
 private:
  ...
  const url_pattern_index::flat::UrlRule* MatchImpl(base::StringPiece origin,
                                                    base::StringPiece url,
                                                    base::StringPiece type,
                                                    bool* blocked);
