...
void FilterTool::Match(const std::string& origin,
                       const std::string& url,
                       const std::string& type) {
  bool blocked;
  const url_pattern_index::flat::UrlRule* rule =
      MatchImpl(origin, url, type, &blocked);
  PrintResult(blocked, rule, origin, url, type);
}

void FilterTool::MatchBatch(std::istream* request_stream) {
  MatchBatchImpl(request_stream, true, 1);
}

void FilterTool::MatchRules(std::istream* request_stream, int min_match_count) {
  MatchBatchImpl(request_stream, false, min_match_count);
}
...
void FilterTool::PrintResult(bool blocked,
  ...
  *output_ << origin << " " << url << " " << type << std::endl;
}
...
const url_pattern_index::flat::UrlRule* FilterTool::MatchImpl(
    ...
    bool* blocked) {
  const url_pattern_index::flat::UrlRule* rule =
      FindMatchingUrlRule(ruleset_.get(), ParseOrigin(origin),
...
void FilterTool::MatchBatchImpl(std::istream* request_stream,
  ...
  while (std::getline(*request_stream, line)) {
    ...
    CHECK_EQ(3u, strings.size());

    bool blocked;
    const url_pattern_index::flat::UrlRule* rule =
        MatchImpl(strings[0], strings[1], strings[2], &blocked);

    if (rule)
      matched_rules[rule] += 1;

    if (print_each_request)
      PrintResult(blocked, rule, strings[0], strings[1], strings[2]);
  }
