 ...
 protected:
  ...
  void ResetRulesetService() {
    ...
    service_ = std::make_unique<RulesetService>(
        &pref_service_, task_runner_, background_task_runner_,
  ...
  void AssertNoBackgroundPendingTasks() {
    ASSERT_FALSE(background_task_runner_->HasPendingTask());
  }
...
TEST_F(SubresourceFilteringRulesetServiceTest,
  ...
  ASSERT_NO_FATAL_FAILURE(AssertNoBackgroundPendingTasks());
