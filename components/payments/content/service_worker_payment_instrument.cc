...
void ServiceWorkerPaymentInstrument::InvokePaymentApp(
  ...
  event_data->top_level_origin = top_level_origin;
  event_data->payment_request_origin = frame_origin;

  if (details.id.has_value())
    event_data->payment_request_id = details.id.value();

  event_data->total = mojom::PaymentCurrencyAmount::New();
  *(event_data->total) = *(details.total->amount);
  ...
  for (const auto& data : method_data) {
    std::vector<std::string>::const_iterator it =
  ...
  content::PaymentAppProvider::GetInstance()->InvokePaymentApp(
      browser_context, stored_payment_app_info_->registration_id,
      std::move(event_data),
