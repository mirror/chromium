...
// TODO(crbug.com/782270): Uses cache to accelerate crawling procedure.
// TODO(crbug.com/782270): Adds integration tests for this class.
...
void InstallablePaymentAppCrawler::OnPaymentWebAppInstallationInfo(
  ...
  // Checks and completes relative url.
  if (app_info != nullptr && base::IsStringUTF8(app_info->sw_js_url) &&
      base::IsStringUTF8(app_info->sw_scope)) {
    if (!GURL(app_info->sw_js_url).is_valid()) {
      app_info->sw_js_url =
          web_app_manifest_url.Resolve(app_info->sw_js_url).spec();
    }

    if (!app_info->sw_scope.empty() && !GURL(app_info->sw_scope).is_valid()) {
      // Do not add '/' again since GetWithoutFilename will leave a '/' at the
      // end.
      std::string relative_url = app_info->sw_scope.at(0) == '/'
                                     ? app_info->sw_scope.substr(1)
                                     : app_info->sw_scope;
      app_info->sw_scope =
          web_app_manifest_url.GetWithoutFilename().spec() + relative_url;
    }
