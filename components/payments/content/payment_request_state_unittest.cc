...
TEST_F(PaymentRequestStateTest, ModifierAppliedToTotal) {
  ...
  EXPECT_EQ("2.00", state()->GetTotal()->amount->value);
  EXPECT_EQ(1U, state()->GetDisplayItems().size());

  state()->SetSelectedInstrument(nullptr);
  EXPECT_FALSE(state()->selected_instrument());
  EXPECT_EQ("1.00", state()->GetTotal()->amount->value);
  EXPECT_EQ(0U, state()->GetDisplayItems().size());
}
