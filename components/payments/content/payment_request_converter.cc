...
PaymentItem CreatePaymentItem(const mojom::PaymentItemPtr& item_entry) {
  ...
  if (item_entry->amount)
    item.amount = CreatePaymentCurrencyAmount(item_entry->amount);
...
PaymentDetailsModifier CreatePaymentDetailsModifier(
  ...
  if (modifier_entry->total) {
    modifier.total =
        base::MakeUnique<PaymentItem>(CreatePaymentItem(modifier_entry->total));
  }
...
PaymentShippingOption CreatePaymentShippingOption(
  ...
  if (option_entry->amount)
    option.amount = CreatePaymentCurrencyAmount(option_entry->amount);
...
PaymentDetails CreatePaymentDetails(
  ...
  if (details_entry->total) {
    details.total =
        base::MakeUnique<PaymentItem>(CreatePaymentItem(details_entry->total));
  }
