...
bool OriginSecurityChecker::IsValidWebPaymentsUrl(
  ...
  if (!base::StartsWith(url_string, kHttpsPrefix,
    ...
    *error_message_suffix = std::string(
                                "is not a URL with HTTPS scheme and is not a "
                                "URL with HTTP scheme (for localhost with --") +
                            switches::kAllowWebPaymentsLocalhostUrls +
                            " command line flag).";
    return false;
  }
  ...
  *url = GURL(url_string);
  ...
  if (url->has_username()) {
    *error_message_suffix = "contains username.";
    return false;
  }

  if (url->has_password()) {
    *error_message_suffix = "contains password.";
    return false;
  }
