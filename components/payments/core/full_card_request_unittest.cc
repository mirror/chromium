...
class MockUIDelegate : public FullCardRequest::UIDelegate,
 ...
 public:
  ...
  MOCK_METHOD1(OnUnmaskVerificationResult, void(PaymentsRpcResult));
};

// The personal data manager.
class MockPersonalDataManager : public autofill::PersonalDataManager {
 public:
  MockPersonalDataManager() : autofill::PersonalDataManager("en-US") {}
...
class FakeRiskDataLoader : public autofill::RiskDataLoader {
 public:
...
class FullCardRequestTest : public testing::Test,
 ...
 public:
  FullCardRequestTest()
      : request_context_(new net::TestURLRequestContextGetter(
        ...
        request_(&risk_data_loader_, &payments_client_, &personal_data_) {
 ...
 private:
  ...
  FakeRiskDataLoader risk_data_loader_;
...
MATCHER_P2(CardMatches, record_type, card_number, "") {
  return arg.record_type() == record_type &&
         arg.GetRawInfo(autofill::CREDIT_CARD_NUMBER) ==
...
MATCHER_P4(CardMatches, record_type, number, month, year, "") {
  return arg.record_type() == record_type &&
         arg.GetRawInfo(autofill::CREDIT_CARD_NUMBER) ==
             base::ASCIIToUTF16(number) &&
         arg.GetRawInfo(autofill::CREDIT_CARD_EXP_MONTH) ==
             base::ASCIIToUTF16(month) &&
...
TEST_F(FullCardRequestTest, GetFullCardPanAndCvcForMaskedServerCard) {
  ...
  EXPECT_CALL(*ui_delegate(), OnUnmaskVerificationResult(SUCCESS));

  request()->GetFullCard(
      CreditCard(CreditCard::MASKED_SERVER_CARD, "server_id"),
      UNMASK_FOR_AUTOFILL, result_delegate()->AsWeakPtr(),
...
TEST_F(FullCardRequestTest, GetFullCardPanAndCvcForLocalCard) {
  ...
  CreditCard card;
  autofill::test::SetCreditCardInfo(&card, nullptr, "4111", "12", "2050", "1");
  request()->GetFullCard(card, UNMASK_FOR_AUTOFILL,
...
TEST_F(FullCardRequestTest, GetFullCardPanAndCvcForFullServerCard) {
  ...
  autofill::test::SetCreditCardInfo(&full_server_card, nullptr, "4111", "12",
                                    "2050", "1");
  request()->GetFullCard(full_server_card, UNMASK_FOR_AUTOFILL,
                         result_delegate()->AsWeakPtr(),
                         ui_delegate()->AsWeakPtr());
  CardUnmaskDelegate::UnmaskResponse response;
  response.cvc = base::ASCIIToUTF16("123");
...
TEST_F(FullCardRequestTest, GetFullCardPanAndCvcForExpiredFullServerCard) {
  ...
  autofill::test::SetCreditCardInfo(
      &full_server_card, nullptr, "4111", "12",
      base::StringPrintf("%d", today.year - 1).c_str(), "1");
...
TEST_F(FullCardRequestTest, OneRequestAtATime) {
  ...
  request()->GetFullCard(
      ...
      UNMASK_FOR_PAYMENT_REQUEST, result_delegate()->AsWeakPtr(),
      ui_delegate()->AsWeakPtr());
}
...
TEST_F(FullCardRequestTest, SecondRequestOkAfterFirstFinished) {
  ...
  EXPECT_CALL(*ui_delegate(), OnUnmaskVerificationResult(SUCCESS)).Times(2);
...
TEST_F(FullCardRequestTest, PermanentFailure) {
  EXPECT_CALL(*result_delegate(), OnFullCardRequestFailed());
  EXPECT_CALL(*ui_delegate(), ShowUnmaskPrompt(_, _, _));
  EXPECT_CALL(*ui_delegate(), OnUnmaskVerificationResult(PERMANENT_FAILURE));
  ...
  response.cvc = base::ASCIIToUTF16("123");
  card_unmask_delegate()->OnUnmaskResponse(response);
  OnDidGetRealPan(PERMANENT_FAILURE, "");
  card_unmask_delegate()->OnUnmaskPromptClosed();
}
...
TEST_F(FullCardRequestTest, NetworkError) {
  EXPECT_CALL(*result_delegate(), OnFullCardRequestFailed());
  EXPECT_CALL(*ui_delegate(), ShowUnmaskPrompt(_, _, _));
  EXPECT_CALL(*ui_delegate(), OnUnmaskVerificationResult(NETWORK_ERROR));
...
TEST_F(FullCardRequestTest, TryAgainFailureGiveUp) {
  EXPECT_CALL(*result_delegate(), OnFullCardRequestFailed());
  EXPECT_CALL(*ui_delegate(), ShowUnmaskPrompt(_, _, _));
  EXPECT_CALL(*ui_delegate(), OnUnmaskVerificationResult(TRY_AGAIN_FAILURE));
  ...
  response.cvc = base::ASCIIToUTF16("123");
  card_unmask_delegate()->OnUnmaskResponse(response);
  OnDidGetRealPan(TRY_AGAIN_FAILURE, "");
  card_unmask_delegate()->OnUnmaskPromptClosed();
}
...
TEST_F(FullCardRequestTest, UpdateExpDateForFullServerCard) {
  ...
  CreditCard full_server_card(CreditCard::FULL_SERVER_CARD, "server_id");
  autofill::test::SetCreditCardInfo(&full_server_card, nullptr, "4111", "10",
                                    "2000", "1");
  request()->GetFullCard(full_server_card, UNMASK_FOR_AUTOFILL,
                         result_delegate()->AsWeakPtr(),
                         ui_delegate()->AsWeakPtr());
