...
bool PaymentDetails::FromDictionaryValue(const base::DictionaryValue& value,
  ...
  if (value.GetList(kPaymentDetailsModifiers, &modifiers_list)) {
    for (size_t i = 0; i < modifiers_list->GetSize(); ++i) {
      ...
      const base::DictionaryValue* method_data_dict = nullptr;
      if (!modifiers_list->GetDictionary(i, &method_data_dict) ||
          !modifier.method_data.FromDictionaryValue(*method_data_dict)) {
        return false;
      }
