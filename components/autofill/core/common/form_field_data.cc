...
bool IsLabelSame(const FormFieldData& field1, const FormFieldData& field2) {
  return field1.label == field2.label ||
         (base::FeatureList::IsEnabled(
              features::kAutofillSkipInferredLabelInComparing) &&
...
bool FormFieldData::SimilarFieldAs(const FormFieldData& field) const {
  return IsLabelSame(*this, field) && name == field.name && id == field.id &&
         form_control_type == field.form_control_type &&
         IsCheckable(check_status) == IsCheckable(field.check_status);
}
