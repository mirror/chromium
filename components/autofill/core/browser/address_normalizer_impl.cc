...
bool AddressNormalizerImpl::NormalizeAddressSync(AutofillProfile* profile) {
  ...
  if (!address_validator_) {
    ...
    AddNormalizationRequestForRegion(std::unique_ptr<NormalizationRequest>(),
                                     region_code);
    return false;
  }

  if (!AreRulesLoadedForRegion(region_code)) {
    // May start loading the rules for that region so that they are available
    // next time.
    LoadRulesForRegion(region_code);
    return false;
  }
...
void AddressNormalizerImpl::OnAddressValidatorCreated(
  ...
  std::vector<std::string> region_keys(pending_normalization_.size());
...
void AddressNormalizerImpl::AddNormalizationRequestForRegion(
  ...
  auto it = pending_normalization_.find(region_code);
  if (it == pending_normalization_.end()) {
    // If no entry exists yet for |region_code|, create the entry and assign it
    // to |it|.
