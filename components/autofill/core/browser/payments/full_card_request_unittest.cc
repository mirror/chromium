...
class FullCardRequestTest : public testing::Test,
 ...
 public:
  FullCardRequestTest()
    ...
    autofill_client_.reset(new TestAutofillClient());
    autofill_client_->SetPrefs(std::move(pref_service));
    payments_client_.reset(new PaymentsClient(
        request_context_.get(), autofill_client_->GetPrefs(), this));
    request_.reset(new FullCardRequest(
        autofill_client_.get(), payments_client_.get(), &personal_data_));
