...
bool PersonalDataManager::ImportCreditCard(
  ...
  *imported_credit_card_matches_masked_server_credit_card = false;
  ...
  // We can offer to save locally even if we already have this stored as another
  // masked server card with the same |TypeAndLastFourDigits| as long as the
  // AutofillOfferLocalSaveIfServerCardManuallyEntered flag is enabled, so that
  // the user can enter the full card number without having to unmask the card.
  for (const auto& card : server_credit_cards_) {
