...
void PersonalDataManager::LogStoredProfileMetrics() const {
  if (!has_logged_stored_profile_metrics_) {
    // Update the histogram of how many profiles the user has stored.
    AutofillMetrics::LogStoredProfileCount(web_profiles_.size());

    // Update the histogram of how many profiles would be considered disused.
    ...
    // Update the histogram of profile's days since last use.
    ...
    // Only log this info once per profile load.
