...
void AutofillManager::ImportFormData(const FormStructure& submitted_form) {
  ...
  if (!IsCreditCardUploadEnabled() ||
    ...
    // Additional note: If the "send detected values metadata" experiment is
    // enabled, ignore the above and always ping Google Payments regardless.
    // Include what data was found as part of the request, and let Payments
    // decide whether upload save should be allowed.
...
int AutofillManager::GetFormImportDetectedValues() const {
  ...
  // Go through the upload request profiles and set the following as detected:
  ...
  //    not set
  //  - Any other address fields found on any addresses
