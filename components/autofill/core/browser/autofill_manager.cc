...
AutofillManager::AutofillManager(
    ...
    : AutofillHandler(driver),
      ...
      credit_card_save_manager_(base::MakeUnique<CreditCardSaveManager>(
          client,
          payments_client_.get(),
          app_locale,
          client->GetPersonalDataManager())),
      form_data_importer_(
          base::MakeUnique<FormDataImporter>(credit_card_save_manager_.get(),
                                             client->GetPersonalDataManager(),
                                             app_locale)),
...
void AutofillManager::OnDidGetUploadDetails(
  ...
  credit_card_save_manager_->CheckUploadGetDetailsResult(
      result, context_token, std::move(legal_message));
}

void AutofillManager::OnDidUploadCard(AutofillClient::PaymentsRpcResult result,
                                      const std::string& server_id) {
  credit_card_save_manager_->CheckUploadSaveCardResult(result, server_id);
}
