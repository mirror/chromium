...
// TODO(crbug/752539): "US" should be VALID as a country code, and the address
// fields should be considered as UNVALIDATED, since there is no rule.
TEST_F(AutofillAddressValidationTest, ValidateFullProfile_RulesNotLoaded) {
  // This is a US profile, therefore the rules are not loaded, and it cannot be
...
TEST_F(AutofillAddressValidationTest, ValidateAddress_ValidZipNoSpace) {
  // TODO(crbug/752614): postal codes in small letter should also be considered
  // as valid. Now, they are considered as INVALID.
