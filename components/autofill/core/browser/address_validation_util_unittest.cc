...
class AutofillAddressValidationTest : public testing::Test,
 ...
 public:
  AutofillAddressValidationTest() {
    ...
    file_path = file_path.Append(FILE_PATH_LITERAL("libaddressinput"));
    file_path = file_path.Append(FILE_PATH_LITERAL("src"));
    file_path = file_path.Append(FILE_PATH_LITERAL("testdata"));
    file_path = file_path.Append(FILE_PATH_LITERAL("countryinfo.txt"));

    validator_ = std::make_unique<AddressValidator>(
        std::unique_ptr<Source>(new TestdataSource(true, file_path.value().c_str())),
