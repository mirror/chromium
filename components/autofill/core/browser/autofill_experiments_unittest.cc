...
class AutofillExperimentsTest : public testing::Test {
 ...
 protected:
  void SetUp() override {
    ResetPrefServiceWithAutofillWalletImportEnabledPrefValue(true);
    sync_service_.reset(new TestSyncService());
  }

  void ResetPrefServiceWithAutofillWalletImportEnabledPrefValue(
      bool autofill_wallet_import_enabled_pref) {
    pref_service_.reset(new TestingPrefServiceSimple());
    pref_service_->registry()->RegisterBooleanPref(
        prefs::kAutofillWalletImportEnabled,
        autofill_wallet_import_enabled_pref);
  }
  ...
  bool IsCreditCardUploadEnabled(const std::string& user_email,
    ...
    return autofill::IsCreditCardUploadEnabled(pref_service(), sync_service(),
                                               user_email);
  }

  PrefService* pref_service() { return pref_service_.get(); }
  TestSyncService* sync_service() { return sync_service_.get(); }
...
TEST_F(AutofillExperimentsTest,
  ...
  ResetPrefServiceWithAutofillWalletImportEnabledPrefValue(false);
  EXPECT_FALSE(IsCreditCardUploadEnabled());
}
