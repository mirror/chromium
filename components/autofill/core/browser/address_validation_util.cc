...
const FieldProblemMap* GetFilter() {
  static const FieldProblemMap* filter = CreateFieldProblemMap();
  return filter;
}
...
void InitializeAddressFromProfile(const AutofillProfile& profile,
  ...
  address.postal_code = base::UTF16ToUTF8(profile.GetRawInfo(ADDRESS_HOME_ZIP));
}
...
AutofillProfile::ValidityState ValidateAddress(
  ...
  if (!base::ContainsValue(
    ...
    // profile are invalid, but other fields cannot be validated,
    // because it is unclear which, if any, rule should apply.
  ...
  if (status == AddressValidator::SUCCESS) {
    ...
    // MISSING_REQUIRED_FIELD. In this case, the address
