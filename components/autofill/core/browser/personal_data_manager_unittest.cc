...
class PersonalDataManagerTestBase {
 protected:
  ...
  void CreateDeletableExpiredAndDisusedCreditCard(base::Time now) {
...
TEST_F(PersonalDataManagerTest, DeleteDisusedAddress_DoNothingWhenDisabled) {
  // Make sure feature is disabled by default.
  EXPECT_FALSE(base::FeatureList::IsEnabled(kAutofillDeleteDisusedCreditCards));

  CreateDeletableExpiredAndDisusedCreditCard(AutofillClock::Now());
