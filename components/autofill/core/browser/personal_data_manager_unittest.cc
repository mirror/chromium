...
TEST_F(PersonalDataManagerTest,
  ...
  CreditCard credit_card2(CreditCard::MASKED_SERVER_CARD, "c789");
  credit_card2.set_use_date(AutofillClock::Now() -
                            base::TimeDelta::FromDays(181));
  test::SetCreditCardInfo(&credit_card2, "Jane Doe", "4351", "01", "1998", "1");
  ...
  CreditCard credit_card3("1141084B-72D7-4B73-90CF-3D6AC154673B",
                          "https://www.example.com");
  credit_card3.set_use_date(AutofillClock::Now() -
  ...
  // Query with empty string only returns card0 and card 1.
  {
    std::vector<Suggestion> suggestions =
        personal_data_->GetCreditCardSuggestions(
            AutofillType(CREDIT_CARD_NAME_FULL), base::string16());
  ...
  // Query with card number prefix for card0 returns card0 and card3.
  {
    std::vector<Suggestion> suggestions =
        personal_data_->GetCreditCardSuggestions(
            AutofillType(CREDIT_CARD_NUMBER), base::ASCIIToUTF16("5105"));

    ASSERT_EQ(2U, suggestions.size());
    EXPECT_EQ(base::UTF8ToUTF16(std::string("Mastercard") +
                                kUTF8MidlineEllipsis + "5100"),
              suggestions[0].value);

    EXPECT_EQ(
        base::UTF8ToUTF16(std::string("Visa") + kUTF8MidlineEllipsis + "4351"),
        suggestions[1].value);
  }
}
