...
AutofillType AutofillField::Type() const {
  std::unique_ptr<AutofillType> html_type;
  std::unique_ptr<AutofillType> server_type;
  ...
  if (html_type_ != HTML_TYPE_UNSPECIFIED &&
    ...
    html_type =
        std::make_unique<AutofillType>(AutofillType(html_type_, html_mode_));
  }
  ...
  if (overall_server_type_ != NO_SERVER_DATA) {
    ...
    if (believe_server)
      server_type =
          std::make_unique<AutofillType>(AutofillType(overall_server_type_));
  }
  ...
  if (html_type && html_type->GetStorableType() == PHONE_HOME_WHOLE_NUMBER &&
      server_type &&
