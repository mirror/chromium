...
bool DuplicatedFilling(const FormStructure& form, const AutofillField& field) {
  for (size_t i = 0; i < form.field_count(); i++) {
    if (form.field(i)->SameFieldAs(field))
      break;
    if (field.value == form.field(i)->value && form.field(i)->is_autofilled)
      return true;
  }
  return false;
}
...
void LogPredictionQualityMetricsForFieldsOnlyFilledWhenFocused(
    ...
    std::string rationalization_quality_histogram,
    ServerFieldType predicted_type,
...
void LogPredictionQualityMetricsForFieldsNormal(
    std::string aggregate_histogram,
    std::string type_specific_histogram,
    ServerFieldType predicted_type,
    ServerFieldType actual_type,
...
void LogPredictionQualityMetrics(
  ...
  std::string rationalization_quality_histogram =
      base::JoinString({"Autofill.RationalizationQuality", suffix}, "");
  ...
  LogPredictionQualityMetricsForFieldsNormal(
