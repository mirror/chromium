...
void LogPredictionQualityMetrics(
  ...
  form_interactions_ukm_logger->LogFieldType(
      form_parsed_time, form.form_signature(), field.GetFieldSignature(),
      prediction_source, metric_type, predicted_type, actual_type);
...
void AutofillMetrics::LogHeuristicPredictionQualityMetrics(
  ...
  LogPredictionQualityMetrics(
      ...
      form_interactions_ukm_logger, form, field, metric_type, form_parsed_time);
}
...
void AutofillMetrics::FormEventLogger::OnDidShowSuggestions(
    const AutofillField& field,
    const base::TimeTicks& form_parsed_time) {
  form_interactions_ukm_logger_->LogSuggestionsShown(field, form_parsed_time);
...
void AutofillMetrics::FormEventLogger::OnDidSelectMaskedServerCardSuggestion(
  ...
  form_interactions_ukm_logger_->LogSelectedMaskedServerCard(form_parsed_time);
...
void AutofillMetrics::FormEventLogger::OnDidFillSuggestion(
  ...
  form_interactions_ukm_logger_->LogDidFillSuggestion(
      static_cast<int>(credit_card.record_type()), form_parsed_time);
...
void AutofillMetrics::FormEventLogger::OnDidFillSuggestion(
  ...
  DCHECK(!is_for_credit_card_);
  form_interactions_ukm_logger_->LogDidFillSuggestion(
      static_cast<int>(profile.record_type()), form_parsed_time);
...
int64_t AutofillMetrics::FormInteractionsUkmLogger::MillisecondsSinceFormParsed(
  ...
  return (now - form_parsed_time).InMilliseconds();
}
