...
class FormStructure {
 ...
 private:
  ...
  void RationalizeCreditCardFieldsPredictions();

  // A function that detects if predictions suggest there are more phone fields
  // than one whole phone number can fill, and override those extranous fields
  // to avoid duplicated fillings.
  void FillPhoneNumberFieldsOnceOnly();
