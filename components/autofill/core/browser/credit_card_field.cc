...
bool FieldIsSkippable(AutofillField* field) {
  return !field->is_focusable || field->name.length() == 0;
}
...
std::unique_ptr<FormField> CreditCardField::Parse(AutofillScanner* scanner) {
  ...
  size_t last_used_cursor = saved_cursor;
  ...
  for (int fields = 0; !scanner->IsEnd(); ++fields) {
    if (!just_skipped) {
      last_used_cursor = scanner->SaveCursor();
    }
    if (!just_skipped) {
    ...
    scanner->RewindTo(last_used_cursor);
    break;
  }
...
bool CreditCardField::ParseExpirationDate(AutofillScanner* scanner) {
  ...
  if (LikelyCardMonthSelectField(scanner)) {
    ...
    scanner->Advance();
    // Allow one skip between two selects if one the field is hidden.
    bool likely_year_select = LikelyCardYearSelectField(scanner);
    if (!likely_year_select && !scanner->IsEnd() &&
        FieldIsSkippable(scanner->Cursor())) {
      scanner->Advance();
      likely_year_select = LikelyCardYearSelectField(scanner);
    }
    if (likely_year_select) {
