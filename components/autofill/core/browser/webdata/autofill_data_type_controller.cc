...
void AutofillDataTypeController::StopModels() {
  ...
  // sync is disabled completely, or when autofill sync is disabled. In the
  // cases where autofill sync or sync in general is disabled, clear autofill
  // data copied from the server. This is different than other sync
  ...
  // data type indicates if autofill sync is enabled, and
  // currently_enabled_ indicates if the other prefs are enabled. All of these
  // have to be enabled to sync autofill data/metadata.
