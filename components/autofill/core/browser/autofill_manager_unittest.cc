...
// Test that we correctly fill multiple phone number fields in one form.
TEST_F(AutofillManagerTest, FillMultiplePhoneNumber) {
  AutofillProfile* work_profile = autofill_manager_->GetProfileWithGUID(
      "00000000-0000-0000-0000-000000000002");
  ASSERT_TRUE(work_profile != nullptr);
  work_profile->SetRawInfo(PHONE_HOME_WHOLE_NUMBER,
  ...
  // componentized number fields.
  {
    ...
    test::CreateTestFormField("country code", "country_code", "", "text",
                              &field);
    form_with_multiple_componentized_phone_fields.fields.push_back(field);
  ...
  // Verify only the first number is filled when there are multiple
  // whole-number fields.
  {
