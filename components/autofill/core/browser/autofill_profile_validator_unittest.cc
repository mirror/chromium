...
// Validate a validate profile, for which the rules are not loaded, yet.
TEST_F(AutofillProfileValidatorTest, ValidateFullValidProfile_RulesNotLoaded) {
  ...
  EXPECT_EQ(false, validator_->AreRulesLoadedForRegion(country_code));
...
TEST_F(AutofillProfileValidatorTest, ValidateAddress_RulesLoaded) {
  ...
  std::string country_code =
      base::UTF16ToUTF8(profile.GetRawInfo(ADDRESS_HOME_COUNTRY));
  validator_->LoadRulesForRegion(country_code);
  EXPECT_EQ(true, validator_->AreRulesLoadedForRegion(country_code));
