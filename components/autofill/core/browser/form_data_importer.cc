  ...
  if (!candidate_credit_card.IsValid()) {
    if (!candidate_credit_card.IsValidCardNumber()) {
      AutofillMetrics::LogSubmittedCardStateMetric(
          AutofillMetrics::VALID_CARD_INVALID_NUMBER);
    }
    if (!candidate_credit_card.IsValidExpirationDate()) {
      AutofillMetrics::LogSubmittedCardStateMetric(
          AutofillMetrics::VALID_CARD_INVALID_EXPIRY_DATE);
    }

    return false;
  }
  AutofillMetrics::LogSubmittedCardStateMetric(AutofillMetrics::VALID_CARD);
