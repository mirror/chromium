...
class ValidationRequest : public AutofillProfileValidator::Request {
 public:
  ...
  void OnRulesLoaded(bool success) override {
    ...
    // what to do if can't load the rules?
    // if (!success) { //say invalidate
    // if not success, if any invalid, then invalid, else valid

    std::string language = profile_->language_code();
    LOG(ERROR) << language;  // empty unless set_language is used.
    AddressData address;
    InitializeAddress(&address);
    AutofillProfile::ValidityState profile_valid = ValidateAddress(address);
    std::move(on_validated_).Run(profile_valid);
  }

 private:
  static constexpr const AddressField kFields[] = {COUNTRY, ADMIN_AREA, LOCALITY,
                                             DEPENDENT_LOCALITY, POSTAL_CODE};
  static constexpr const AddressProblem kProblems[] = {
  ...
  FieldProblemMap filter;  // change these
  ...
  AutofillProfile::ValidityState ValidateAddress(const AddressData& address) {
    ...
    LOG(ERROR) << address.region_code << " " << address.administrative_area;
    ...
    for (auto problem : problems) {
      SetValidityStateForAddressField(problem.first, AutofillProfile::INVALID);
      LOG(ERROR) << "problem is:" << problem.first << " " << problem.second;
    }
    return AutofillProfile::VALID;
  }

  void SetValidityStateForAddressField(
      AddressField address_field,
      AutofillProfile::ValidityState state) const {
    ServerFieldType server_field = i18n::TypeForField(address_field, false);
    if (server_field == UNKNOWN_TYPE)  // unnecessary?
      return;
    LOG(ERROR) << "field: " << server_field;
    profile_->SetValidityState(server_field, AutofillProfile::INVALID);
  }
