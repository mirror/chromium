...
TEST_F(CreditCardFieldTest, ParseMiniumCreditCardWithUnfocusableFields) {
  ...
  field.label = ASCIIToUTF16("A technical field");
  field.name = ASCIIToUTF16("");
  field.is_focusable = false;
  list_.push_back(
      std::make_unique<AutofillField>(field, ASCIIToUTF16("nameless")));
  ...
  EXPECT_EQ(
      UNKNOWN_TYPE,
      field_candidates_map_[ASCIIToUTF16("nameless")].BestHeuristicType());
}

TEST_F(CreditCardFieldTest, ParseMiniumCreditCardWithDateUnfocusableField) {
  FormFieldData field;
  field.is_focusable = true;
  field.form_control_type = "text";
