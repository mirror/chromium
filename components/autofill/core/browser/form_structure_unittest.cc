...
TEST_F(FormStructureTest, ParseQueryResponse_FillWholePhoneNumberOnceOnly) {
  FormData form;
  form.origin = GURL("http://foo.com");
  FormFieldData field;
  field.form_control_type = "text";
  ...
  field.label = ASCIIToUTF16("Full Name");
  field.name = ASCIIToUTF16("fillName");
  form.fields.push_back(field);
  ...
  AutofillQueryResponseContents response;
  response.add_field()->set_autofill_type(NAME_FULL);
  response.add_field()->set_autofill_type(ADDRESS_HOME_STREET_ADDRESS);
  response.add_field()->set_autofill_type(PHONE_HOME_WHOLE_NUMBER);
  response.add_field()->set_autofill_type(PHONE_HOME_WHOLE_NUMBER);

  std::string response_string;
  ASSERT_TRUE(response.SerializeToString(&response_string));

  // Verify second phone field is override to be unknown when feature is on.
  {
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(
        autofill::kAutofillRationalizeFieldTypePredictions);
    ...
    EXPECT_FALSE(forms[0]->field(2)->only_fill_when_highlighted());

    EXPECT_EQ(PHONE_HOME_WHOLE_NUMBER, forms[0]->field(3)->server_type());
    EXPECT_TRUE(forms[0]->field(3)->only_fill_when_highlighted());
  }
  ...
  // Sanity check that the enable/disabled works.
  {
    ...
    EXPECT_EQ(PHONE_HOME_WHOLE_NUMBER, forms[0]->field(3)->server_type());
    EXPECT_FALSE(forms[0]->field(3)->only_fill_when_highlighted());
  }
}
...
TEST_F(FormStructureTest,
  ...
  response.add_field()->set_autofill_type(NAME_FULL);
  response.add_field()->set_autofill_type(ADDRESS_HOME_STREET_ADDRESS);
  response.add_field()->set_autofill_type(PHONE_HOME_COUNTRY_CODE);
  response.add_field()->set_autofill_type(PHONE_HOME_CITY_CODE);
  response.add_field()->set_autofill_type(PHONE_HOME_NUMBER);
  response.add_field()->set_autofill_type(PHONE_HOME_WHOLE_NUMBER);
  response.add_field()->set_autofill_type(PHONE_HOME_CITY_CODE);
  ...
  // Verify second phone field is override to be unknown when feature is on.
  {
    ...
    EXPECT_EQ(PHONE_HOME_COUNTRY_CODE, forms[0]->field(2)->server_type());
    EXPECT_FALSE(forms[0]->field(2)->only_fill_when_highlighted());

    EXPECT_EQ(PHONE_HOME_CITY_CODE, forms[0]->field(3)->server_type());
    EXPECT_FALSE(forms[0]->field(3)->only_fill_when_highlighted());

    EXPECT_EQ(PHONE_HOME_NUMBER, forms[0]->field(4)->server_type());
    EXPECT_FALSE(forms[0]->field(4)->only_fill_when_highlighted());

    EXPECT_EQ(PHONE_HOME_WHOLE_NUMBER, forms[0]->field(5)->server_type());
    EXPECT_TRUE(forms[0]->field(5)->only_fill_when_highlighted());

    EXPECT_EQ(PHONE_HOME_CITY_CODE, forms[0]->field(6)->server_type());
    EXPECT_TRUE(forms[0]->field(6)->only_fill_when_highlighted());
  }
  ...
  // Sanity check that the enable/disabled works.
  {
    ...
    EXPECT_EQ(PHONE_HOME_WHOLE_NUMBER, forms[0]->field(5)->server_type());
    EXPECT_FALSE(forms[0]->field(5)->only_fill_when_highlighted());

    EXPECT_EQ(PHONE_HOME_CITY_CODE, forms[0]->field(6)->server_type());
    EXPECT_FALSE(forms[0]->field(6)->only_fill_when_highlighted());
  }
}
...
TEST_F(FormStructureTest,
  ...
  // Verify second phone field is override to be unknown when feature is on.
  {
    ...
    EXPECT_EQ(PHONE_HOME_COUNTRY_CODE, forms[0]->field(2)->server_type());
    EXPECT_TRUE(forms[0]->field(2)->only_fill_when_highlighted());

    EXPECT_EQ(PHONE_HOME_CITY_CODE, forms[0]->field(3)->server_type());
    EXPECT_TRUE(forms[0]->field(3)->only_fill_when_highlighted());

    EXPECT_EQ(PHONE_HOME_WHOLE_NUMBER, forms[0]->field(4)->server_type());
    EXPECT_FALSE(forms[0]->field(4)->only_fill_when_highlighted());

    EXPECT_EQ(PHONE_HOME_NUMBER, forms[0]->field(5)->server_type());
    EXPECT_TRUE(forms[0]->field(5)->only_fill_when_highlighted());
...
TEST_F(
  ...
  // Verify second phone field is override to be unknown when feature is on.
  {
    ...
    EXPECT_EQ(PHONE_HOME_CITY_CODE, forms[0]->field(2)->server_type());
    EXPECT_FALSE(forms[0]->field(2)->only_fill_when_highlighted());

    EXPECT_EQ(PHONE_HOME_NUMBER, forms[0]->field(3)->server_type());
    EXPECT_FALSE(forms[0]->field(3)->only_fill_when_highlighted());

    EXPECT_EQ(PHONE_HOME_WHOLE_NUMBER, forms[0]->field(4)->server_type());
    EXPECT_TRUE(forms[0]->field(4)->only_fill_when_highlighted());

    EXPECT_EQ(PHONE_HOME_CITY_CODE, forms[0]->field(5)->server_type());
    EXPECT_TRUE(forms[0]->field(5)->only_fill_when_highlighted());
  }
