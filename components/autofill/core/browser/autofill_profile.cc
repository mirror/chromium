...
static std::vector<ServerFieldType> supported_types_for_validation = {
    ADDRESS_HOME_COUNTRY,
    ADDRESS_HOME_STATE,
    ADDRESS_HOME_ZIP,
    ADDRESS_HOME_CITY,
...
bool AutofillProfile::IsValidationSupportedForType(ServerFieldType type) {
  return std::find(supported_types_for_validation.begin(),
                   supported_types_for_validation.end(),
                   type) != supported_types_for_validation.end();
}
