...
ScopedJavaLocalRef<jobject> FormFieldDataAndroid::GetJavaPeer() {
  ...
  if (obj.is_null()) {
    ...
    obj = Java_FormFieldData_createFormFieldData(
        ...
        field_ptr_->should_autocomplete, jplaceholder, jtype, jid, joption_values, joption_contents, IsCheckable(field_ptr_->check_status), IsChecked(field_ptr_->check_status));
