...
class MockWebState : public web::TestWebState {
 public:
  ...
  MOCK_METHOD1(AddObserver, void(web::WebStateObserver* observer));
  MOCK_METHOD1(RemoveObserver, void(web::WebStateObserver* observer));
...
class AutofillAgentTests : public PlatformTest {
 public:
  ...
  void SetUp() override {
    ...
    mock_web_state_ = base::MakeUnique<MockWebState>();
    id mock_js_injection_receiver =
        [OCMockObject mockForClass:[CRWJSInjectionReceiver class]];
    // MockWebState* mock = mock_web_state_.get();

    EXPECT_CALL(*mock_web_state_, AddObserver(testing::_)).Times(1);
    EXPECT_CALL(*mock_web_state_, RemoveObserver(testing::_)).Times(1);
