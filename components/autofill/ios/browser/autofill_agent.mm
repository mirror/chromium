...
// Returns whether Autofill is enabled for |webState| by checking if Autofill is
// turned on and if the URL has a web scheme and the page content is HTML.
- (BOOL)isAutofillEnabledForWebState:(web::WebState*)webState;
...
- (void)checkIfSuggestionsAvailableForForm:(NSString*)formName
  ...
  if (![self isAutofillEnabledForWebState:webState]) {
    completion(NO);
    return;
  }
...
- (BOOL)isAutofillEnabledForWebState:(web::WebState*)webState {
  ...
  return web::UrlHasWebScheme(pageURL) && webState->ContentIsHTML();
}
