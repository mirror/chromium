...
// DevTools console.
class ConsoleLogger : public PasswordManagerLogger {
 public:
  ConsoleLogger(blink::WebLocalFrame* frame)
      : PasswordManagerLogger(), frame_(frame) {}
...
std::vector<FormInputCollection> ExtractFormsForAnalysis(
  ...
  for (unsigned i = 0; i < password_inputs.size(); ++i) {
    ...
    logger->Send("Password field is not contained in a form:",
                 PasswordManagerLogger::kVerbose, password_inputs[i]);
  }
  ...
  for (const auto& pair : nodes_for_id) {
    ...
    if (nodes.size() <= 1)
      continue;
    if (!id_attr.empty()) {
      logger->Send("Found " + std::to_string(nodes.size()) +
                       " elements with non-unique id #" + id_attr + ":",
                   PasswordManagerLogger::kError, nodes);
    } else {
...
void AnalyseForm(const FormInputCollection& form_input_collection,
  ...
  if (!has_text_field || text_inputs[0] > password_inputs[0]) {
    ...
    logger->Send(
        ...
        PasswordManagerLogger::kVerbose, form);
  } else {
  ...
  for (size_t i = 0; i < inputs.size(); ++i) {
    if (autocomplete_suggestions.count(i) &&
      ...
      logger->Send(
          ...
          PasswordManagerLogger::kVerbose, inputs[i]);
  }
}
