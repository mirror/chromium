...
const std::string type_attributes[] = {"text", "email", "tel", "password"};
...
void DevToolsPagePasswordsAnalyser::AnalyseDocumentDOM(
  ...
  for (const FormInputCollection& form_input_collection : forms) {
    ...
    // flag it as an issue.
    bool has_username_field = text_inputs.size() != 0;
    size_t username_field_guess;
    if (!has_username_field || text_inputs[0] > password_inputs[0]) {
