...
class ConsoleLogger {
 public:
  ...
  void SendVerbose(const std::string& message) { Send(kVerbose, message); }
...
void DevToolsPagePasswordsAnalyser::AnalyseDocumentDOM(
    ...
    if (node.IsElementNode()) {
      ...
      if (element.TagName() == "INPUT" && element.HasAttribute("type") &&
        ...
        if (in_form)
          ...
          console_logger.Nodes(node).SendVerbose(
  ...
  for (const std::string& id_attr : duplicateIds) {
    ...
    if (!id_attr.empty()) {
      ...
      console_logger.Nodes(nodes).SendError(
          "The id attribute must be unique and non-empty; there are " +
          std::to_string(nodes.size()) + " elements with an empty id:");
    }
  }
  ...
  for (const FormInputCollection& form_input_collection : forms) {
    ...
    if (!has_username_field || username_inputs[0] > password_inputs[0]) {
      ...
      console_logger.Nodes(form).SendVerbose(
    ...
    if (has_username_field) {
      ...
      struct InputHint {
        ...
        InputHint(std::string pattern) : match(std::string::npos) {
    ...
    for (size_t i = 0; i < inputs.size(); ++i) {
      if (autocomplete_suggestions.count(i) &&
        ...
        console_logger.Nodes(inputs[i]).SendVerbose(
