...
// |TraversalInfo| is a simple wrapper that provides some extra data about a
// nodes during the DOM traversal (e.g. whether it lies within a <form>
...
// |FormInputCollection| collects the important elements in a form that are
...
// |InputHint| represents a common <label> content text-pattern that indicates
...
struct InputHint {
  ...
  InputHint(re2::RE2* regex) : regex(regex), match(std::string::npos) {}

  InputHint(re2::RE2* regex, std::string tokens) : InputHint(regex) {
    this->tokens += " " + tokens;
  }
...
// other fields.
void InferUsernameField(std::map<size_t, std::string>& autocomplete_suggestions,
...
void AnalyseForm(const FormInputCollection& form_input_collection,
  ...
  // flag it as an issue.
  bool has_text_field = text_inputs.size() != 0;
  size_t username_field_guess;
  ...
  std::map<size_t, std::string> autocomplete_suggestions;
  if (has_text_field)
    InferUsernameField(autocomplete_suggestions, form, inputs,
                       username_field_guess);

  GuessAutocompleteAttributesForPasswordFields(autocomplete_suggestions,
                                               password_inputs, has_text_field);
...
void DevToolsPagePasswordsAnalyser::AnalyseDocumentDOMLazily(
