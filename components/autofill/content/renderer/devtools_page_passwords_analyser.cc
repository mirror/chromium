...
void TrackElementId(
    ...
    std::map<std::string, std::set<blink::WebElement>>* nodes_for_id) {
  if (element.HasAttribute("id")) {
    std::string id_attr = element.GetAttribute("id").Utf8();
    (*nodes_for_id)[id_attr].insert(element);
  }
}
...
std::vector<FormInputCollection> ExtractFormsForAnalysis(
  ...
  for (const blink::WebFormElement& form : forms) {
    ...
    for (const blink::WebFormControlElement& input : form_control_elements) {
      if (ShouldSkipElement(input, skip_nodes, &nodes_for_id))
        continue;
  ...
  for (unsigned i = 0; i < password_inputs.size(); ++i) {
    if (ShouldSkipElement(password_inputs[i], skip_nodes, &nodes_for_id))
      continue;
  ...
  for (const char* text_type : type_text_attributes)
  ...
  for (const blink::WebElement& text_input : text_inputs)
    TrackElementId(text_input, &nodes_for_id);
  ...
  for (const auto pair : nodes_for_id) {
    ...
    std::vector<blink::WebNode> nodes(pair.second.begin(), pair.second.end());
    if (nodes.size() <= 1)
      continue;
    if (!id_attr.empty()) {
