...
// Stores an element's id in a set, |ids|, for duplicity-checking.
void TrackElementId(const blink::WebElement& element,
...
// The id of |node| will additionally be added to |ids|.
bool ShouldSkipElement(const blink::WebElement& node,
...
std::vector<FormInputCollection> ExtractFormsForAnalysis(
  ...
  for (const blink::WebFormElement& form : forms) {
    ...
    for (const blink::WebFormControlElement& input : form_control_elements) {
      if (ShouldSkipElement(input, skip_nodes, &ids))
        continue;
    ...
    TrackElementId(form, &ids);
    skip_nodes->insert(form);
  }
  ...
  for (unsigned i = 0; i < password_inputs.size(); ++i) {
    if (ShouldSkipElement(password_inputs[i], skip_nodes, &ids))
      continue;
  ...
  for (const std::string& id_attr : ids) {
    blink::WebVector<blink::WebElement> elements(
        document.QuerySelectorAll(blink::WebString::FromUTF8("#" + id_attr)));
    std::vector<blink::WebNode> nodes(elements.begin(), elements.end());
    if (nodes.size() <= 1)
      continue;
    if (!id_attr.empty()) {
