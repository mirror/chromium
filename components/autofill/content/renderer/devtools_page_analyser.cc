...
void DevToolsPageAnalyser::AnalyseDocumentDOM(
  ...
  for (const FormInputCollection& form_input_collection : forms) {
    ...
    if (has_username_field) {
      ...
      std::vector<std::unique_ptr<re2::RE2>> label_indicator_patterns;
#define ADD_LABEL_PATTERN(PATTERN)    \
  label_indicator_patterns.push_back( \
      std::unique_ptr<re2::RE2>(new re2::RE2((PATTERN), case_insensitivity)));
      ADD_LABEL_PATTERN(R"(user(name)?|login)")
      ADD_LABEL_PATTERN(R"(email(address)?)")
#undef ADD_LABEL_PATTERN
      std::vector<size_t> matches(label_indicator_patterns.size());
      std::fill(matches.begin(), matches.end(), (size_t)-1);
      ...
      for (blink::WebElement item = labels.FirstItem(); !item.IsNull();
        ...
        if (!control.IsNull() && control.IsFormControlElement()) {
          ...
          if (found != inputs.end()) {
            ...
            for (size_t i = 0; i < label_indicator_patterns.size(); ++i) {
              if (re2::RE2::FullMatch(label_content,
                                      *label_indicator_patterns[i]))
                matches[i] = found - inputs.begin();
            }
          }
        }
      }
      ...
      for (size_t i = 0; i < label_indicator_patterns.size(); ++i) {
        if (matches[i] != (size_t)-1) {
          username_field_guess = matches[i];
          break;
        }
      }
