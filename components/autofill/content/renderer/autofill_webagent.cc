...
AutofillWebagent::AutofillWebagent(
  ...
  GetFrame().GetDocument().addEventListener(
      // TODO: is there any problem with capturing |this|?
      "submit",
      std::make_unique<EventTarget::EventListener>([this](Event event) {
        FireHostSubmitEventsWebagents(event.target().ToHTMLFormElement(),
...
void AutofillWebagent::FireHostSubmitEventsWebagents(
...
void AutofillWebagent::FormControlElementClickedWebagents(
    ...
    bool was_focused) {
  if (!element.IsHTMLInputElement() && !element.IsHTMLTextAreaElement())
    return;
...
void AutofillWebagent::TextFieldDidChange(const HTMLInputElement& element) {
  // TODO:
  /*
    DCHECK(ToWebInputElement(&element) ||
    ...
    Note
    // that pastes aren't necessarily user gestures because Blink's conception
    of
...
void AutofillWebagent::TextFieldDidChangeImpl(const HTMLInputElement& element) {
  // TODO:
  /*
    ...
    { is_popup_possibly_visible_ = true; return;
      }
...
void AutofillWebagent::ShowSuggestions(const HTMLInputElement& element,
