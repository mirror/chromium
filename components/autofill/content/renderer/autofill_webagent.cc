...
void AutofillWebagent::TextFieldDidChange(Event event) {
  ...
  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE, base::Bind(&AutofillWebagent::TextFieldDidChangeImpl,
                            ...
                            event.target().ToHTMLElement()));
}

void AutofillWebagent::TextFieldDidChangeImpl(const HTMLElement& element) {
...
*/
    ...
    FormData form;
    FormFieldData field;
    if (form_util::FindFormAndFieldForFormControlElementWebagents(
            element, &form, &field)) {
      ...
      GetAutofillDriver()->TextFieldDidChange(
          form, field, gfx::RectF(bounding_box_in_window),
          base::TimeTicks::Now());
    }
}
...
bool AutofillWebagent::CollectFormlessElements(FormData* output) {
  ...
  return form_util::UnownedCheckoutFormElementsAndFieldSetsToFormDataWebagents(
      fieldsets, control_elements, base::Optional<Element>(), document,
      extract_mask, output, nullptr);
}

void AutofillWebagent::ShowSuggestions(const Element& element,
  ...
  int selection_start;
  int selection_end;
  bool is_auto_fillable = false;
  bool is_password_field = false;
  if (element.IsHTMLInputElement()) {
    const HTMLInputElement input = element.ToHTMLInputElement();
    if (input.disabled() || input.readOnly() || input.type() != "text" ||
        !input.NotStandardSuggestedValue().IsEmpty())
      return;
    value = input.NotStandardEditingValue();
    selection_start = input.NotStandardSelectionStart();
    selection_end = input.NotStandardSelectionEnd();
    is_auto_fillable = form_util::IsAutofillableInputElementWebagents(input);
    is_password_field = input.type() == "password";
  } else {
    DCHECK(element.IsHTMLTextAreaElement());
    const HTMLTextAreaElement textarea = element.ToHTMLTextAreaElement();
    if (textarea.disabled() || textarea.readOnly() ||
        !textarea.NotStandardSuggestedValue().IsEmpty())
      return;
    value = textarea.NotStandardEditingValue();
    selection_start = textarea.NotStandardSelectionStart();
    selection_end = textarea.NotStandardSelectionEnd();
  }
  ...
  if (value.length() > kMaxDataLength ||
      ...
      (options.requires_caret_at_end &&
       (selection_start != selection_end ||
        selection_end != static_cast<int>(value.length())))) {
...
void AutofillWebagent::QueryAutofillSuggestions(
  ...
  if (!form_util::FindFormAndFieldForFormControlElementWebagents(element, &form,
    ...
    WebFormControlElementToFormFieldWebagents(element, form_util::EXTRACT_VALUE,
                                              &field);
  }
  ...
  blink::WebRect bounding_box_in_window = element.NotStandardBoundsInViewport();
  render_frame()->GetRenderView()->ConvertViewportToWindowViaWidget(
      &bounding_box_in_window);
