...
bool AutofillAgent::GetSubmittedForm(FormData* form) {
  ...
  if (!last_interacted_form_.IsNull()) {
    ...
    if (CollectFormlessElements(form)) {
      ...
      LOG(ERROR) << "bt:" << __FUNCTION__ << "e";
      *form = *provisionally_saved_form_;
      return true;
    }
  }
  LOG(ERROR) << "bt:" << __FUNCTION__ << "f";
  return false;
}
