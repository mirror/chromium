...
bool PasswordAutofillAgent::FillUserNameAndPassword(
  ...
  // |current_username| is the username for credentials that're goint to be
  // autofilled. It's taken according to the algorithm:
  // 1. From the page, if it's prefilled.
  // 2. Default username from |fill_data| if username field is autocompletable.
  // 3. Empty if username field doesn't exist or is empty and not
  // autocompletable (no username case).
  ...
  if (!username_element->IsNull() &&
      IsElementAutocompletable(*username_element)) {
    // Fill username only when it's not emppty and not prefilled by the page.
