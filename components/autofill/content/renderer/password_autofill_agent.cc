...
// Supporting function for IsPublicSuffixDomainMatch
...
// Supporting function for FillFormOnPasswordReceived
...
bool FillFormOnPasswordReceived(
  ...
  while (cur_frame->Parent()) {
    ...
    if (!IsPublicSuffixDomainMatch(
            bottom_frame_origin.Ascii(),
            cur_frame->GetSecurityOrigin().ToString().Ascii()))
      return false;
  }
