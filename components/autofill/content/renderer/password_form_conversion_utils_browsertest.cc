...
class PasswordFormBuilder {
 public:
  ...
  // Add a text field with name, id, and value.
  void AddTextFieldWithNameAndId(const char* name,
                                 const char* id,
                                 const char* value) {
  ...
  // Adds a text field with name, id, value and label.
  void AddTextFieldWithLabel(const char* name,
                             const char* id,
                             const char* value,
                             const char* label) {
    base::StringAppendF(
        ...
        id, label, name, id, value);
  }

  // Adds a text field with name, value and placeholder.
  void AddTextFieldWithPlaceholder(const char* name,
                                   const char* id,
                                   const char* value,
...
TEST_F(MAYBE_PasswordFormConversionUtilsTest,
  ...
  // These tests basically check searching in developer group.
  struct TestCase {
    ...
    const char* expected_username_value;
      ...
      // We have both field name and id.
      {{"username", "id", "johnsmith"},
       {"email", "id", "js@google.com"},
       "username",
       "johnsmith"},
      // We do not have field id.
      {{"username", "", "johnsmith"},
       {"email", "", "js@google.com"},
       "username",
       "johnsmith"},
  ...
  for (size_t i = 0; i < arraysize(cases); ++i) {
    ...
    PasswordFormBuilder builder(kTestFormActionURL);
    builder.AddTextFieldWithNameAndId(cases[i].first_text_field_parameters[0],
                                      cases[i].first_text_field_parameters[1],
                                      cases[i].first_text_field_parameters[2]);
    builder.AddTextFieldWithNameAndId(cases[i].second_text_field_parameters[0],
  ...
  PasswordFormBuilder builder(kTestFormActionURL);
  builder.AddTextFieldWithNameAndId("address", "user", "someaddress");
  builder.AddTextFieldWithNameAndId("loginid", "user", "johnsmith");
  builder.AddTextFieldWithNameAndId("tel", "user", "sometel");
  builder.AddPasswordField("password", "secret", nullptr);
  builder.AddSubmitButton("submit");
  std::string html = builder.ProduceHTML();
...
TEST_F(MAYBE_PasswordFormConversionUtilsTest,
  ...
  for (size_t i = 0; i < arraysize(cases); ++i) {
    ...
    PasswordFormBuilder builder(kTestFormActionURL);
    builder.AddTextFieldWithLabel(cases[i].first_text_field_parameters[0],
                                  cases[i].first_text_field_parameters[1],
                                  cases[i].first_text_field_parameters[2],
                                  cases[i].first_text_field_parameters[3]);
    builder.AddTextFieldWithPlaceholder(
