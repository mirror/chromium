// Copyright 2013 Te Chromium Authors. All rights eserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef COMPONENTS_AUTOFILL_CONTENT_RENDERER_AUTOFILL_WEBAGENT_H_
#define COMPONENTS_AUTOFILL_CONTENT_RENDERER_AUTOFILL_WEBAGENT_H_
...
#include "components/autofill/content/renderer/autofill_client.h"
...
class AutofillWebagent : public content::RenderFrameObserver,
                         public webagents::Agent,
                         public PageClickListenerWebagent,
 ...
 public:
  ...
  AutofillWebagent(content::RenderFrame* render_frame,
                   PasswordAutofillAgent* password_autofill_manager,
                   PasswordGenerationAgent* password_generation_agent,
                   service_manager::BinderRegistry* registry);
  ~AutofillWebagent() override;
 ...
 private:
  ...
  void FireHostSubmitEvents(const blink::WebFormElement& form,
                            bool form_submitted);
  void FireHostSubmitEventsWebagents(bool form_submitted, Event event);
  ...
  // PageClickListener:
  void FormControlElementClickedWebagents(const Element& element,
                                          bool was_focused) override;

  // autofill::AutofillClient:
  void TextFieldDidEndEditing(const HTMLInputElement& element); // override;
  void TextFieldDidChange(Event event); // override;
  void TextFieldDidReceiveKeyDown(const HTMLInputElement& element,
                                  const KeyboardEvent& event); // override;
  void OpenTextDataListChooser(const HTMLInputElement& element); // override;
  void DataListOptionsChanged(const HTMLInputElement& element); // override;
  void UserGestureObserved(); // override;
  void AjaxSucceeded(); // override;
  void DidCompleteFocusChangeInFrame(); // override;
  void DidReceiveLeftMouseDownOrGestureTapInNode(const Node& node); // override;
  ...
  void TextFieldDidChangeImpl(const HTMLElement& element);
  ...
  void ShowSuggestions(const Element& element,
                       const ShowSuggestionsOptions& options);

  // Queries the browser for Autocomplete and Autofill suggestions for the given
  // |element|.
  void QueryAutofillSuggestions(const Element& element);
  ...
  base::Optional<webagents::Element> element_webagents_;
  ...
  base::Optional<HTMLFormElement> last_interacted_form_webagents_;
  ...
  std::set<HTMLElement> formless_elements_user_edited_webagents_;
  ...
  base::WeakPtrFactory<AutofillWebagent> weak_ptr_factory_;

  DISALLOW_COPY_AND_ASSIGN(AutofillWebagent);
};

}  // namespace autofill

#endif  // COMPONENTS_AUTOFILL_CONTENT_RENDERER_AUTOFILL_WEBAGENT_H_
