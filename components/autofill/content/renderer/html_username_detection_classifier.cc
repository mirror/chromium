...
// ([a-z]). These changes will be applied to a field name + id value, before
...
std::string OnlyKeepLettersInASCII(const base::string16& str) {
  std::string new_str = base::UTF16ToASCII(base::ToLowerASCII(str));
  new_str.erase(std::remove_if(new_str.begin(), new_str.end(),
                               [](char x) { return base::IsAsciiLower(x); }),
                new_str.end());
  return new_str;
}
