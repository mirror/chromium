  ...
  if (base::FeatureList::IsEnabled(
    ...
    for (const WebInputElement& password_element : passwords) {
      ...
      bool element_has_autofilled_value = FieldHasPropertiesMask(
          ...
          FieldPropertiesFlags::AUTOFILLED);
      LOG(ERROR) << password_element.NameForAutofill().Utf8() << ": "
                 << element_has_autofilled_value;
      passwords_has_autofilled_value |= element_has_autofilled_value;
      if (find(other_possible_passwords.begin(), other_possible_passwords.end(),
               value) == other_possible_passwords.end()) {
        other_possible_passwords.push_back(std::move(value));
      }
    }
    ...
    if (!other_possible_passwords.empty()) {
      ...
      LOG(ERROR) << "value=" << password_form->passwords_has_autofilled_value;
    }
  }
