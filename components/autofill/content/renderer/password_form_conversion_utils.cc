...
// others are ignored. Criteria for password and non-password fields are
// calculated separately. For example, if there is a password field with user
// input, the password fields without user input are ignored (independently
// whether the fields are visible or not).
enum FieldFilteringLevel { NO_FILTER = 0, VISIBILITY = 1, USER_INPUT = 2 };
...
FieldFilteringLevel GetFiltertingLevelForField(
  ...
  if (FieldHasPropertiesMask(field_value_and_properties_map, element,
    ...
    return USER_INPUT;
  } else if (form_util::IsWebElementVisible(element)) {
    return VISIBILITY;
  } else {
    return NO_FILTER;
  }
...
// Calculates the maximal filtering levels for password and non-password fields
// and saves them to |non_password_fields_level| and |password_fields_level|.
// The criteria for non-password fields considers only the fields before the
// first password field that has the maximal filtering level.
void GetFieldFilteringLevels(
    const std::vector<blink::WebFormControlElement>& control_elements,
    const FieldValueAndPropertiesMaskMap* field_value_and_properties_map,
    FieldFilteringLevel* non_password_fields_level,
    FieldFilteringLevel* password_fields_level) {
  DCHECK(password_fields_level);
  DCHECK(non_password_fields_level);
  *non_password_fields_level = NO_FILTER;
  *password_fields_level = NO_FILTER;

  FieldFilteringLevel max_level_found_for_non_password_fields = NO_FILTER;
  for (auto& control_element : control_elements) {
    ...
    if (input_element->IsPasswordField()) {
      if (*password_fields_level < current_field_filtering_level) {
        ...
        *non_password_fields_level = max_level_found_for_non_password_fields;
      }
    } else {
      max_level_found_for_non_password_fields =
          std::max(max_level_found_for_non_password_fields,
                   current_field_filtering_level);
    }
  }
}
...
bool GetPasswordForm(
  ...
  // Calculate filtering levels for password and non-password fields. For
  // details see to comments to |FieldFilteringLevel|.
  FieldFilteringLevel non_password_fields_level = NO_FILTER;
  FieldFilteringLevel password_fields_filter = NO_FILTER;
  GetFieldFilteringLevels(form.control_elements, field_value_and_properties_map,
                          &non_password_fields_level, &password_fields_filter);
  ...
  for (size_t i = 0; i < form.control_elements.size(); ++i) {
    ...
    if (input_element->IsPasswordField()) {
      if (current_field_level < password_fields_filter)
        continue;
      layout_sequence.push_back('P');
    } else {
      if (current_field_level == USER_INPUT)
        ++number_of_non_empty_text_non_password_fields;
      if (current_field_level < non_password_fields_level)
        continue;
