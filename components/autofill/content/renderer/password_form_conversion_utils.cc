...
bool GetPasswordForm(
  ...
  UsernameDetectionMethod username_detection_method = NO_USERNAME_DETECTED;
  ...
  if (form_predictions) {
    ...
    if (map_has_username_prediction) {
      ...
      username_detection_method = SERVER_SIDE_PREDICTION;
    }
  }
      ...
      if (HasAutocompleteAttributeValue(*input_element,
        ...
        if (password_form->username_marked_by_site) {
          ...
          if (username_element.IsNull()) {
            ...
            username_detection_method = AUTOCOMPLETE_ATTRIBUTE;
          }
        }
      } else {
        if (password_form->username_marked_by_site) {
  ...
  if (base::FeatureList::IsEnabled(
    ...
    if (username_element.IsNull()) {
      ...
      if (!username_element.IsNull())
        username_detection_method = HTML_BASED_CLASSIFIER;
    }
  }
  ...
  if (username_element.IsNull()) {
    ...
    username_detection_method = BASE_HEURISTIC;
  } else if (base_heuristic_prediction == username_element) {
    // To measure the impact of server-side predictions and HTML based
    // classifier compared to the based heuristic (impact of autocomplete
    // attribute is neglected), this block was added. When crbug.com/786404 is
    // fixed, this block can be removed and the base heuristic can be run
    // only if |username_element.IsNull()|.
    username_detection_method = BASE_HEURISTIC;
  }
