...
// Check if script modified username is suitable for autofill to remember.
bool ScriptModifiedUsernameAcceptable(
  ...
  // Minimal size of field value that will be matched.
  ...
  // Check if username was generated by javascript based on user typed name.
  if (typed_username.size() >= kMinMatchSize &&
      username_value.find(typed_username) != base::string16::npos)
    return true;

  // Check if username was generated by javascript based on user typed or
  // autofilled field values.
  for (const auto& field_prop : *field_value_and_properties_map) {
    FieldPropertiesMask prop_mask = field_prop.second.second;
    const WebInputElement* input_element = ToWebInputElement(&field_prop.first);
    if (((prop_mask & USER_TYPED) || (prop_mask & AUTOFILLED)) &&
        input_element && input_element->IsTextField() &&
        !input_element->IsPasswordField()) {
      const auto field_name = input_element->NameForAutofill().Utf16();
      const auto field_value = input_element->Value().Utf16();
      if (field_value.size() >= kMinMatchSize && !field_name.empty() &&
          field_name != password_form->username_element &&
          field_name != password_form->password_element &&
          field_name != password_form->new_password_element &&
          field_name != password_form->confirmation_password_element) {
        if (username_value.find(base::i18n::ToLower(field_value)) !=
            base::string16::npos)
          return true;
      }
    }
