...
// others are ignored. For example, if there is a password field with user
...
// Calculates the maximal filtering criteria for password and non-password
// fields and saves them to |non_password_fields_level| and
// |password_fields_level|. The criteria for non-password fields considers only
// the fields before the first password field that has maximal filtering level.
void GetFieldFilteringLevels(
  ...
  *password_fields_level = NO_FILTER;
  ...
  FieldFilteringLevel max_non_password_field_level_found = NO_FILTER;
  for (auto& control_element : control_elements) {
    ...
    if (input_element->IsPasswordField()) {
      if (*password_fields_level < current_field_filtering_level) {
        ...
        *non_password_fields_level = max_non_password_field_level_found;
      }
    } else {
      max_non_password_field_level_found = std::max(
          max_non_password_field_level_found, current_field_filtering_level);
    }
  }
}
...
bool GetPasswordForm(
  ...
  FieldFilteringLevel non_password_fields_filter = NO_FILTER;
  ...
  GetFieldFilteringLevels(form.control_elements, field_value_and_properties_map,
                          &non_password_fields_filter, &password_fields_filter);
  ...
  for (size_t i = 0; i < form.control_elements.size(); ++i) {
    ...
    if (input_element->IsPasswordField()) {
      ...
      if (current_field_level < non_password_fields_filter)
        continue;
