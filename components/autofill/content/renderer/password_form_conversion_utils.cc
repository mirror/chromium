...
// Fills |passwords| with all password fields from |control_elements|. Also
// fills |last_text_input_before_password|.
// If |ignore_elemenets_without_user_input|, only fields with user input is
// considered.
...
bool GetPasswordForm(
  ...
  LOG(ERROR) << "user input " << ignore_elemenets_without_user_input;
  ...
  LOG(ERROR) << " ignore_invisible_passwords " << ignore_invisible_passwords
             << " ignore_invisible_usernames " << ignore_invisible_usernames;
  ...
  // and also mark that the form should be available only for fallback saving
  // (automatic bubble will not pop up).
