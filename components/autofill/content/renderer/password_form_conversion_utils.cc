...
bool IsPasswordFieldMarkedByAutocompleteAttribute(
    const blink::WebInputElement& input_element) {
  return input_element.IsPasswordField() &&
...
// Fills |credit_card_elements| with credit card related fields. The fields
// should be skipped by the password manager. Filtering criteria is based on
// server-side predictions (|form_predictions|) and client-side heuristics
// (|IsCreditCardRelatedField()|).
// TODO(crbug.com/): A user should be able to save these fields despite server-
// and client-side predictions via the manual fallback.
void FindCreditCardElements(
    const std::vector<blink::WebFormControlElement>& control_elements,
    const std::map<WebInputElement, PasswordFormFieldPredictionType>&
        form_predictions,
    std::set<blink::WebFormControlElement>* credit_card_elements) {
  DCHECK(credit_card_elements);
  ...
  for (auto& control_element : control_elements) {
    ...
    // Autocomplete attribute should override out predictions.
    if (IsPasswordFieldMarkedByAutocompleteAttribute(*input_element))
      continue;

    auto predicted_element_iterator = form_predictions.find(*input_element);
    if ((predicted_element_iterator != form_predictions.end() &&
         predicted_element_iterator->second == PREDICTION_NOT_PASSWORD) ||
        IsCreditCardRelatedField(*input_element)) {
      LOG(ERROR) << "find CC field";
      credit_card_elements->insert(*input_element);
    }
  }
}
...
void SetWhatFieldsShouldBeConsidered(
    const std::vector<blink::WebFormControlElement>& control_elements,
    const std::set<blink::WebFormControlElement>& credit_card_elements,
    ...
    bool* form_has_password_fields) {
  DCHECK(found_visible_password);
  DCHECK(found_visible_username_before_visible_password);
  DCHECK(found_password_with_user_input);
  DCHECK(form_has_password_fields);

  *found_visible_password = false;
  *found_visible_username_before_visible_password = false;
  *form_has_password_fields = false;

  *found_password_with_user_input =
      field_value_and_properties_map &&
      std::find_if(
          field_value_and_properties_map->begin(),
          field_value_and_properties_map->end(),
          [credit_card_elements](
              const std::pair<const blink::WebFormControlElement,
                              std::pair<std::unique_ptr<base::string16>,
                                        FieldPropertiesMask>>& e) {
            const WebInputElement* input_element = ToWebInputElement(&e.first);
            return input_element && input_element->IsPasswordField() &&
                   e.second.first.get() &&
                   credit_card_elements.find(*input_element) ==
                       credit_card_elements.end();
          }) != field_value_and_properties_map->end();
  if (*found_password_with_user_input) {
    *form_has_password_fields = true;
    return;
  }
  ...
  for (auto& control_element : control_elements) {
    ...
    if (!input_element || !input_element->IsEnabled() ||
        ...
        credit_card_elements.find(*input_element) != credit_card_elements.end())
      continue;

    bool is_password_field = input_element->IsPasswordField();
    *form_has_password_fields |= is_password_field;
...
void FillPasswordsListWithoutHeuristics(
    const std::vector<blink::WebFormControlElement> control_elements,
    bool ignore_elemenets_without_user_input,
    const FieldValueAndPropertiesMaskMap* field_value_and_properties_map,
    std::vector<WebInputElement>* passwords,
    std::map<blink::WebInputElement, blink::WebInputElement>*
        last_text_input_before_password) {
  DCHECK(passwords);
  DCHECK(last_text_input_before_password);

  passwords->clear();
  last_text_input_before_password->clear();
...
bool GetPasswordForm(
  ...
  std::set<blink::WebFormControlElement> credit_card_elements;
  FindCreditCardElements(form.control_elements, predicted_elements,
                         &credit_card_elements);
  ...
  bool form_has_password_fields = false;
  SetWhatFieldsShouldBeConsidered(
      form.control_elements, credit_card_elements,
      field_value_and_properties_map, &ignore_invisible_passwords,
      &ignore_invisible_usernames, &ignore_elemenets_without_user_input,
      &form_has_password_fields);
  LOG(ERROR) << "form_has_password_fields = " << form_has_password_fields;
  if (!form_has_password_fields)
    return false;
  ...
  for (size_t i = 0; i < form.control_elements.size(); ++i) {
    ...
    if (credit_card_elements.find(*input_element) != credit_card_elements.end())
      continue;
    ...
    bool password_marked_by_autocomplete_attribute =
        IsPasswordFieldMarkedByAutocompleteAttribute(*input_element);
  ...
  if (passwords.empty()) {
    FillPasswordsListWithoutHeuristics(
        form.control_elements, ignore_elemenets_without_user_input,
        field_value_and_properties_map, &passwords,
        &last_text_input_before_password);
  }
  DCHECK(!passwords.empty());
