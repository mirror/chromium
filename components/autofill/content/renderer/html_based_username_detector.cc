...
void GetUsernameFieldBasedOnHtmlAttributes(
  ...
  if (!username_found) {
    username_found =
        SearchForPartialMatch(control_elements, form_data, usernameTranslations,
                              LABEL, username_element);
  }

  // 1.2. Search for full words.
  if (!username_found) {
    username_found = SearchForFullMatch(
        control_elements, form_data, userTranslations, LABEL, username_element);
  }
  if (!username_found) {
    username_found =
        SearchForFullMatch(control_elements, form_data, loginTranslations,
                           LABEL, username_element);
  }
  if (!username_found) {
    username_found = SearchForFullMatch(
        control_elements, form_data, idTranslations, LABEL, username_element);
  }
  ...
  if (!username_found) {
    username_found =
        SearchForPartialMatch(control_elements, form_data, positiveWords,
                              NAME_AND_ID, username_element);
  }
  ...
  if (!username_found) {
    username_found =
        SearchForPartialMatch(control_elements, form_data, possibleWords,
                              NAME_AND_ID, username_element);
  }
}
