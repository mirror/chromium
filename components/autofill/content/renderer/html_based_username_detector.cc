...
// It might also be useful to concatenate aria-label to label group, to get
...
// variable names, and not as stand-alone words. We look for these words
// only in name group.
...
// Words that might point to a username field, if they appear in name group.
...
// because it is a short word, and cannot be trusted (maybe surround it by
// guards etc).
const char* const weakWords[] = {"login", "mail", "@"};
...
// Given a field, return name group, i.e. concatenation of field name and id,
// with safety guard ('$') in between, and with special characters removed.
std::string GetNameGroup(const FormFieldData& field) {
  std::string guard = "$";
  return OnlyKeepLettersInASCII(field.name + base::ASCIIToUTF16(guard) +
                                field.id);
}

// Given a field, return label group. Only keep ASCII letters and foreign
// letters, and no other special characters or digits.
std::string GetLabelGroup(const FormFieldData& field) {
  return OnlyKeepLettersInNonASCII(field.label);
}

// Check if a string (which can name group or label group) is definitely not
// username group. This happens when it contains, as substring, at least a
// negative word (label groups can have latin and foreign negative words,
// while name groups can only have latin negative words).
void MarkNegatives(
    const std::vector<blink::WebInputElement>& all_possible_usernames,
    const FormData& form_data,
    std::vector<bool> negatives) {
  size_t last_index = 0;
  for (size_t i = 0; i < all_possible_usernames.size(); ++i) {
    WebInputElement possible_username = all_possible_usernames[i];
    ...
    for (size_t j = last_index; j < form_data.fields.size(); ++j) {
      ...
      if (!possible_username.NameForAutofill().IsEmpty()) {
        ...
        if (field.name == possible_username.NameForAutofill().Utf16()) {
          ...
          std::string name_group = GetNameGroup(field);
          std::string label_group = GetLabelGroup(field);
          ...
          if (!negatives[j]) {
            for (size_t k = 0; k < arraysize(negativesForeign); ++k) {
              if (label_group.find(negativesForeign[k]) != std::string::npos) {
                negatives[j] = true;
                break;
              }
            }
          ...
          if (field.name == possible_username.NameForAutofill().Utf16()) {
            ...
            if (!negatives[j]) {
              WebInputElement* input_element =
                  ToWebInputElement(&possible_username);

              std::string analyzed_group;
              (attribute == NAME_GROUP) ? analyzed_group = GetNameGroup(field)
                                        : analyzed_group = GetLabelGroup(field);

              if (analyzed_group.find(dictionary[k]) != std::string::npos) {
                if (!count)
...
void GetUsernameFieldBasedOnHtmlAttributes(
  ...
  MarkNegatives(all_possible_usernames, form_data, negatives);
  ...
  static std::pair<const char* const*, HTMLAttribute> all_heuristics[] = {
  ...
  for (std::pair<const char* const*, HTMLAttribute> heuristic :
       all_heuristics) {
    if (SearchForPartialMatch(all_possible_usernames, form_data,
                              heuristic.first, heuristic.second, negatives,
                              username_element)) {
      break;
    }
  }
