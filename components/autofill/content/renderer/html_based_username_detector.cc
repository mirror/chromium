...
enum CategoryName { NEGATIVE, USERNAME, USER, TECHNICAL, WEAK };
...
// |mask| indicates in which category we search.
...
const CategoryOfWords kNegativeCategory{kNegativeLatin, kNegativeLatinSize,
                                        kNegativeNonLatin,
                                        kNegativeNonLatinSize};
const CategoryOfWords kUsernameCategory{kUsernameLatin, kUsernameLatinSize,
                                        kUsernameNonLatin,
                                        kUsernameNonLatinSize};
const CategoryOfWords kUserCategory{kUserLatin, kUserLatinSize, kUserNonLatin,
                                    kUserNonLatinSize};
const CategoryOfWords kTechnicalCategory{kTechnicalWords, kTechnicalWordsSize,
                                         nullptr, 0};
const CategoryOfWords kWeakCategory{kWeakWords, kWeakWordsSize, nullptr, 0};

CategoryOfWords GetWordCategory(CategoryName name) {
  switch (name) {
    case NEGATIVE:
      return kNegativeCategory;
    case USERNAME:
      return kUsernameCategory;
    case USER:
      return kUserCategory;
    case TECHNICAL:
      return kTechnicalCategory;
    case WEAK:
      return kWeakCategory;
  }
}

// List of separators that can appear in HTML attribute values.
const std::string kDelimiters = "\"\'?%*@!\\/&^#:+~`;,>|<.[](){}-_ 0123456789";
...
void BuildValueAndShortTokens(
  ...
  *field_data_value = base::JoinString(tokens, base::ASCIIToUTF16(""));
...
// information.
bool SearchFieldInDictionary(base::string16 value,
  ...
  for (size_t i = 0; i < dictionary_size; ++i) {
    if (strlen(dictionary[i]) < kMinimumWordLength) {
      ...
      for (base::string16 token : tokens) {
...
bool ContainsWordFromCategory(const UsernameFieldData& possible_username,
  ...
  return
      // For user value, search in latin and non-latin dictionaries, because
      // this value is user visible.
      SearchFieldInDictionary(
          possible_username.user_value, possible_username.user_short_tokens,
          category.latin_dictionary, category.latin_dictionary_size) ||
      SearchFieldInDictionary(
          possible_username.user_value, possible_username.user_short_tokens,
          category.non_latin_dictionary, category.non_latin_dictionary_size) ||
...
// usernames.
// If at least one negative word appears in developer or user value, then the
  ...
  possible_usernames_data->erase(
      std::remove_if(possible_usernames_data->begin(),
                     ...
                     [](const UsernameFieldData& possible_username) {
                       return ContainsWordFromCategory(
                           possible_username, GetWordCategory(NEGATIVE));
                     }),
      possible_usernames_data->end());
}
...
bool GetUsernameFieldBasedOnHtmlAttributes(
  ...
  for (const CategoryName name : {USERNAME, USER, TECHNICAL, WEAK}) {
    if (FormContainsWordFromCategory(possible_usernames_data,
                                     GetWordCategory(name), username_element)) {
      return true;
    }
  }
  return false;
