...
const char kDelimiters[] = "$\"\'?%*@!\\/&^#:+~`;,>|<.[](){}-_ 0123456789";
...
struct UsernameFieldData {
  ...
  std::set<base::string16> developer_short_tokens;
  base::string16 user_value;
  std::set<base::string16> user_short_tokens;
};
...
void AppendValueAndShortTokens(
    ...
    std::set<base::string16>* field_data_short_tokens) {
  ...
  std::transform(
      ...
      tokens.end(),
      std::inserter(*field_data_short_tokens, field_data_short_tokens->end()),
...
bool CheckFieldWithDictionary(const base::string16& value,
                              const std::set<base::string16>& tokens,
