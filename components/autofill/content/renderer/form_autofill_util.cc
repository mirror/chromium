...
base::string16 InferLabelFromSiblingWebagents(const AutofillElement& element,
                                              bool forward) {
  base::string16 inferred_label;
...
base::string16 InferLabelFromPreviousWebagents(const AutofillElement& element) {
...
base::string16 InferLabelFromNextWebagents(const AutofillElement& element) {
...
base::string16 InferLabelFromListItemWebagents(const AutofillElement& element) {
...
base::string16 InferLabelFromTableRowWebagents(const AutofillElement& element) {
  CR_DEFINE_STATIC_LOCAL(WebString, kTableCell, ("td"));
  base::string16 inferred_label;
...
base::string16 InferLabelFromDivTableWebagents(const AutofillElement& element) {
...
bool ExtractFieldsFromControlElementsWebagents(
    ...
    std::vector<std::unique_ptr<FormFieldData>>* form_fields,
    std::vector<bool>* fields_extracted,
    std::map<AutofillElement, FormFieldData*>* element_map) {
  DCHECK(form_fields->empty());
  DCHECK(element_map->empty());
  DCHECK_EQ(control_elements.size(), fields_extracted->size());

  for (size_t i = 0; i < control_elements.size(); ++i) {
    const AutofillElement& control_element = control_elements[i];
...
void MatchLabelsAndFieldsWebagents(
  ...
  for (webagents::Element item : labels) {
    ...
    } else if (base::Optional<AutofillElement> form_control =
                   AutofillElement::FromElement(control->ToElement())) {
...
bool FormOrFieldsetsToFormDataWebagents(
    ...
    const base::Optional<AutofillElement> form_control_element,
...
bool UnownedFormElementsAndFieldSetsToFormDataWebagents(
    const std::vector<webagents::Element>& fieldsets,
    const std::vector<AutofillElement>& control_elements,
    const base::Optional<AutofillElement> element,
...
bool ExtractFormDataWebagents(const webagents::HTMLFormElement& form_element,
  ...
  return WebFormElementToFormDataWebagents(
      form_element, base::Optional<AutofillElement>(), nullptr,
      static_cast<form_util::ExtractMask>(form_util::EXTRACT_VALUE |
                                          form_util::EXTRACT_OPTION_TEXT |
                                          form_util::EXTRACT_OPTIONS),
      data, NULL);
...
std::vector<AutofillElement> ExtractAutofillableElementsFromSetWebagents(
    const std::vector<AutofillElement>& control_elements) {
  std::vector<AutofillElement> autofillable_elements;
  for (size_t i = 0; i < control_elements.size(); ++i) {
    AutofillElement element = control_elements[i];
...
void WebFormControlElementToFormFieldWebagents(
  ...
  // Save both id and name attributes, if present. If there is only one of them,
  // it will be saved to |name|. See HTMLFormControlElement::nameForAutofill.
  base::string16 name = element.name().Utf16();
  base::TrimWhitespace(name, base::TRIM_ALL, &name);
  base::string16 id = element->getAttribute(kId).Utf16();
  base::TrimWhitespace(id, base::TRIM_ALL, &id);
  if (name.empty())
    name = id;
  field->name = name;
  if (id != field->name)
  ...
  field->form_control_type = element.type().Utf8();
  field->autocomplete_attribute = element->getAttribute(kAutocomplete).Utf8();
  if (field->autocomplete_attribute.size() > kMaxDataLength) {
    // Discard overly long attribute values to avoid DOS-ing the browser
    // process.  However, send over a default string to indicate that the
  ...
  if (IsAutofillableInputElementWebagents(*element) ||
    ...
    if (!g_prevent_layout)
      field->is_focusable = element->IsFocusable();
    // moved below - different logic for input, textarea, select
    // field->should_autocomplete = element.AutoComplete();
  ...
  if (element->IsHTMLSelectElement() && (extract_mask & EXTRACT_OPTION_TEXT)) {
    ...
    // Convert the |select_element| value to text if requested.
    webagents::HTMLOptionsCollection options = select_element.options();
    for (webagents::Element option_or_optgroup : options) {
      if (option_or_optgroup.IsHTMLOptionElement()) {
        const webagents::HTMLOptionElement option_element =
            option_or_optgroup.ToHTMLOptionElement();
        if (option_element.value().Utf16() == value) {
...
bool WebFormElementToFormDataWebagents(
  ...
  for (webagents::Element element : form_element.elements()) {
    base::Optional<AutofillElement> autofill_element =
        AutofillElement::FromElement(element);
    DCHECK(autofill_element);
    control_elements.push_back(*autofill_element);
  }
...
std::vector<AutofillElement> GetUnownedFormFieldElementsWebagents(
  ...
  std::vector<AutofillElement> unowned_fieldset_children;
  for (webagents::Element element : elements) {
    if (base::Optional<AutofillElement> control =
            AutofillElement::FromElement(element)) {
      if (!control->form())
        unowned_fieldset_children.push_back(*control);
    }
...
bool UnownedCheckoutFormElementsAndFieldSetsToFormDataWebagents(
  ...
  // autocomplete attribute to the formless autofill.
  CR_DEFINE_STATIC_LOCAL(WebString, kOffAttribute, ("off"));
  std::vector<AutofillElement> elements_with_autocomplete;
  for (const AutofillElement& element : control_elements) {
    blink::WebString autocomplete = element->getAttribute("autocomplete");
    if (autocomplete.length() && autocomplete != kOffAttribute)
      elements_with_autocomplete.push_back(element);
  }
...
bool FindFormAndFieldForFormControlElementWebagents(
  ...
  return WebFormElementToFormDataWebagents(
      *form_element, base::Optional<AutofillElement>(element), nullptr,
      extract_mask, form, field);
  return true;
}
