...
bool IsElementInsideFormOrFieldSetWebagents(const webagents::Element& element) {
  for (base::Optional<webagents::Node> parent_node = element.parentNode();
       parent_node; parent_node = parent_node->parentNode()) {
    if (!parent_node->IsElementNode())
      continue;

    webagents::Element cur_element = parent_node->ToElement();
    if (cur_element.tagName() == "form" || cur_element.tagName() == "fieldset")
      return true;
  }
  return false;
}
...
bool IsTraversableContainerElementWebagents(const webagents::Node& node) {
  if (!node.IsElementNode())
    return false;

  WebString tag_name = node.ToElement().tagName();
  return tag_name == "dd" || tag_name == "div" || tag_name == "fieldset" ||
         tag_name == "li" || tag_name == "td" || tag_name == "table";
}
...
size_t CalculateTableCellColumnSpanWebagents(
    const webagents::Element& element) {
  DCHECK(element.tagName() == "td" || element.tagName() == "th");
...
base::string16 FindChildTextInnerWebagents(
    base::Optional<webagents::Node> node,
    int depth,
    const std::set<webagents::Node>& divs_to_skip) {
  if (depth <= 0 || !node)
    return base::string16();

  // Skip over comments.
  if (node->IsCommentNode())
    return FindChildTextInnerWebagents(node->nextSibling(), depth - 1,
                                       divs_to_skip);
  ...
  // Ignore elements known not to contain inferable labels.
  if (node->IsElementNode()) {
    const webagents::Element element = node->ToElement();
    if (element.IsHTMLOptionElement() || element.IsHTMLScriptElement() ||
        element.IsHTMLNoScriptElement() ||
        // TODO(joelhockey): Handle FormControlElement.
        // Using HTMLInputElement hack for now.
        (element.IsHTMLInputElement() &&
         IsAutofillableElementWebagents(element))) {
      return base::string16();
    }

    if (element.tagName() == "div" && base::ContainsKey(divs_to_skip, *node))
      return base::string16();
  }
  ...
  // Preserve inter-element whitespace separation.
  base::string16 child_text =
      FindChildTextInnerWebagents(node->firstChild(), depth - 1, divs_to_skip);
  bool add_space = node->IsTextNode() && node_text.empty();
  node_text = CombineAndCollapseWhitespace(node_text, child_text, add_space);
  ...
  // Again, preserve inter-element whitespace separation.
  base::string16 sibling_text =
      FindChildTextInnerWebagents(node->nextSibling(), depth - 1, divs_to_skip);
  add_space = node->IsTextNode() && node_text.empty();
  node_text = CombineAndCollapseWhitespace(node_text, sibling_text, add_space);
...
base::string16 FindChildTextWithIgnoreListWebagents(
    const webagents::Node& node,
    const std::set<webagents::Node>& divs_to_skip) {
  if (node.IsTextNode())
    return node.nodeValue().Utf16();

  base::Optional<webagents::Node> child = node.firstChild();

  const int kChildSearchDepth = 10;
  base::string16 node_text =
      FindChildTextInnerWebagents(child, kChildSearchDepth, divs_to_skip);
  base::TrimWhitespace(node_text, base::TRIM_ALL, &node_text);
  return node_text;
}
...
base::string16 FindChildTextWebagents(const webagents::Node& node) {
  return FindChildTextWithIgnoreListWebagents(node,
                                              std::set<webagents::Node>());
}
...
base::string16 InferLabelFromSiblingWebagents(const webagents::Element& element,
                                              bool forward) {
  base::string16 inferred_label;
  base::Optional<webagents::Node> sibling = element;
  while (true) {
    sibling = forward ? sibling->nextSibling() : sibling->previousSibling();
    if (!sibling)
      break;
    ...
    blink::WebString tag_name;
    if (sibling->IsElementNode())
      tag_name = sibling->ToElement().tagName();
    ...
    CR_DEFINE_STATIC_LOCAL(WebString, kSpan, ("span"));
    CR_DEFINE_STATIC_LOCAL(WebString, kFont, ("font"));
    if (sibling->IsTextNode() || tag_name == kBold || tag_name == kStrong ||
        tag_name == kSpan || tag_name == kFont) {
      base::string16 value = FindChildTextWebagents(*sibling);
      // A text node's value will be empty if it is for a line break.
      bool add_space = sibling->IsTextNode() && value.empty();
      inferred_label =
          CombineAndCollapseWhitespace(value, inferred_label, add_space);
    ...
    CR_DEFINE_STATIC_LOCAL(WebString, kImage, ("img"));
    CR_DEFINE_STATIC_LOCAL(WebString, kBreak, ("br"));
    if (tag_name == kImage || tag_name == kBreak)
      continue;

    // We only expect <p> and <label> tags to contain the full label text.
    CR_DEFINE_STATIC_LOCAL(WebString, kPage, ("p"));
    CR_DEFINE_STATIC_LOCAL(WebString, kLabel, ("label"));
    if (tag_name == kPage || tag_name == kLabel)
      inferred_label = FindChildTextWebagents(*sibling);
...
base::string16 InferLabelFromPreviousWebagents(
    const webagents::Element& element) {
  return InferLabelFromSiblingWebagents(element, false /* forward? */);
}
...
base::string16 InferLabelFromNextWebagents(const webagents::Element& element) {
  return InferLabelFromSiblingWebagents(element, true /* forward? */);
}
...
base::string16 InferLabelFromPlaceholderWebagents(
    const webagents::Element& element) {
  CR_DEFINE_STATIC_LOCAL(WebString, kPlaceholder, ("placeholder"));
  if (element.hasAttribute(kPlaceholder))
    return element.getAttribute(kPlaceholder).Utf16();
...
base::string16 InferLabelFromValueAttrWebagents(
    const webagents::Element& element) {
  CR_DEFINE_STATIC_LOCAL(WebString, kValue, ("value"));
  // TODO(joelhockey): Handle FormControlElements.
  // Using hack now to assume HTMLInputElement.
  if (element.hasAttribute(kValue) && element.IsHTMLInputElement() &&
      element.getAttribute(kValue) == element.ToHTMLInputElement().value()) {
    return element.getAttribute(kValue).Utf16();
  }
...
base::string16 InferLabelFromListItemWebagents(
    const webagents::Element& element) {
  base::Optional<webagents::Node> parent = element.parentNode();
  CR_DEFINE_STATIC_LOCAL(WebString, kListItem, ("li"));
  while (parent && parent->IsElementNode() &&
         parent->ToElement().tagName() != kListItem) {
    parent = parent->parentNode();
  }

  if (parent && parent->IsElementNode() &&
      parent->ToElement().tagName() == kListItem) {
    return FindChildTextWebagents(*parent);
  }
...
base::string16 InferLabelFromEnclosingLabelWebagents(
    const webagents::Element& element) {
  base::Optional<webagents::Node> parent = element.parentNode();
  CR_DEFINE_STATIC_LOCAL(WebString, kLabel, ("label"));
  while (parent && parent->IsElementNode() &&
         parent->ToElement().tagName() != kLabel) {
    parent = parent->parentNode();
  }
...
base::string16 InferLabelFromTableColumnWebagents(
    const webagents::Element& element) {
  CR_DEFINE_STATIC_LOCAL(WebString, kTableCell, ("td"));
  base::Optional<webagents::Node> parent = element.parentNode();
  while (parent && parent->IsElementNode() &&
         parent->ToElement().tagName() != kTableCell) {
    parent = parent->parentNode();
  }
  ...
  // non-empty text block.
  base::string16 inferred_label;
  base::Optional<webagents::Node> previous = parent->previousSibling();
  CR_DEFINE_STATIC_LOCAL(WebString, kTableHeader, ("th"));
  while (inferred_label.empty() && previous) {
    WebString tag_name;
    if (previous->IsElementNode())
      tag_name = previous->ToElement().tagName();
    if (tag_name == kTableCell || tag_name == kTableHeader)
      inferred_label = FindChildTextWebagents(*previous);
...
base::string16 InferLabelFromTableRowWebagents(
    const webagents::Element& element) {
  CR_DEFINE_STATIC_LOCAL(WebString, kTableCell, ("td"));
  base::string16 inferred_label;

  // First find the <td> that contains |element|.
  base::Optional<webagents::Node> cell = element.parentNode();
  while (cell) {
    if (cell->IsElementNode() && cell->ToElement().tagName() == kTableCell) {
      break;
    }
    cell = cell->parentNode();
  }
  ...
  // Count the cell holding |element|.
  size_t cell_count = CalculateTableCellColumnSpanWebagents(cell->ToElement());
  size_t cell_position = 0;
  size_t cell_position_end = cell_count - 1;

  // Count cells to the left to figure out |element|'s cell's position.
  for (base::Optional<webagents::Node> cell_it = cell->previousSibling();
       cell_it; cell_it = cell_it->previousSibling()) {
    if (cell_it->IsElementNode() &&
        cell_it->ToElement().tagName() == kTableCell) {
      cell_position +=
          CalculateTableCellColumnSpanWebagents(cell_it->ToElement());
    }
  }

  // Count cells to the right.
  for (base::Optional<webagents::Node> cell_it = cell->nextSibling(); cell_it;
       cell_it = cell_it->nextSibling()) {
    if (cell_it->IsElementNode() &&
        cell_it->ToElement().tagName() == kTableCell) {
      cell_count += CalculateTableCellColumnSpanWebagents(cell_it->ToElement());
    }
  }
  ...
  base::Optional<webagents::Node> parent = element.parentNode();
  while (parent && parent->IsElementNode() &&
         parent->ToElement().tagName() == kTableRow) {
    parent = parent->parentNode();
  }
  ...
  // Now find the previous row.
  base::Optional<webagents::Node> row_it = parent->previousSibling();
  while (row_it) {
    if (row_it->IsElementNode() && row_it->ToElement().tagName() == kTableRow) {
      break;
    }
    row_it = row_it->previousSibling();
  }
  ...
  // with the current row, infer the label from the cell above.
  if (row_it) {
    base::Optional<webagents::Node> matching_cell;
    size_t prev_row_count = 0;
    base::Optional<webagents::Node> prev_row_it = row_it->firstChild();
    CR_DEFINE_STATIC_LOCAL(WebString, kTableHeader, ("th"));
    while (prev_row_it) {
      if (prev_row_it->IsElementNode()) {
        webagents::Element prev_row_element = prev_row_it->ToElement();
        if (prev_row_element.tagName() == kTableCell ||
            prev_row_element.tagName() == kTableHeader) {
          size_t span = CalculateTableCellColumnSpanWebagents(prev_row_element);
          size_t prev_row_count_end = prev_row_count + span - 1;
          if (prev_row_count == cell_position &&
              prev_row_count_end == cell_position_end) {
            matching_cell = prev_row_it;
    ...
    if ((cell_count == prev_row_count) && matching_cell) {
      inferred_label = FindChildTextWebagents(*matching_cell);
      if (!inferred_label.empty())
        return inferred_label;
    }
  }
...
base::string16 InferLabelFromDivTableWebagents(
    const webagents::Element& element) {
  base::Optional<webagents::Node> node = element.parentNode();
  bool looking_for_parent = true;
  std::set<webagents::Node> divs_to_skip;

  // Search the sibling and parent <div>s until we find a candidate label.
  base::string16 inferred_label;
  CR_DEFINE_STATIC_LOCAL(WebString, kDiv, ("div"));
  CR_DEFINE_STATIC_LOCAL(WebString, kLabel, ("label"));
  while (inferred_label.empty() && node) {
    WebString tag_name;
    if (node->IsElementNode())
    ...
    if (tag_name == kDiv) {
      if (looking_for_parent)
        inferred_label =
            FindChildTextWithIgnoreListWebagents(*node, divs_to_skip);
      else
        inferred_label = FindChildTextWebagents(*node);

      // Avoid sibling DIVs that contain autofillable fields.
      if (!looking_for_parent && !inferred_label.empty()) {
        CR_DEFINE_STATIC_LOCAL(WebString, kSelector,
                               ("input, select, textarea"));
        base::Optional<webagents::Element> result_element =
            node->querySelector(kSelector);
        if (result_element) {
          inferred_label.clear();
          divs_to_skip.insert(*node);
        }
      ...
      looking_for_parent = false;
    } else if (!looking_for_parent && tag_name == kLabel) {
      webagents::HTMLLabelElement label_element = node->ToHTMLLabelElement();
      if (!label_element.control())
        inferred_label = FindChildTextWebagents(*node);
    } else if (looking_for_parent &&
               IsTraversableContainerElementWebagents(*node)) {
      // If the element is in a non-div container, its label most likely is too.
      break;
    }
...
base::string16 InferLabelFromDefinitionListWebagents(
    const webagents::Element& element) {
  CR_DEFINE_STATIC_LOCAL(WebString, kDefinitionData, ("dd"));
  base::Optional<webagents::Node> parent = element.parentNode();
  while (parent && parent->IsElementNode() &&
         parent->ToElement().tagName() != kDefinitionData)
    parent = parent->parentNode();
  ...
  if (!previous || (previous->IsElementNode() &&
                    previous->ToElement().tagName() != kDefinitionTag)) {
    return base::string16();
  }

  return FindChildTextWebagents(*previous);
}
...
std::vector<std::string> AncestorTagNamesWebagents(
    const webagents::Element& element) {
  std::vector<std::string> tag_names;
  for (base::Optional<webagents::Node> parent_node = element.parentNode();
       parent_node; parent_node = parent_node->parentNode()) {
    ...
    tag_names.push_back(parent_node->ToElement().tagName().Utf8());
  }
  return tag_names;
}
...
base::string16 InferLabelForElementWebagents(
    const webagents::Element& element,
    const std::vector<base::char16>& stop_words) {
  base::string16 inferred_label;

  if (element.IsHTMLInputElement() &&
      IsCheckableElementWebagents(element.ToHTMLInputElement())) {
    inferred_label = InferLabelFromNextWebagents(element);
    if (IsLabelValid(inferred_label, stop_words))
      return inferred_label;
  }

  inferred_label = InferLabelFromPreviousWebagents(element);
  if (IsLabelValid(inferred_label, stop_words))
    return inferred_label;

  // If we didn't find a label, check for placeholder text.
  inferred_label = InferLabelFromPlaceholderWebagents(element);
  if (IsLabelValid(inferred_label, stop_words))
    return inferred_label;

  // For all other searches that involve traversing up the tree, the search
  // order is based on which tag is the closest ancestor to |element|.
  std::vector<std::string> tag_names = AncestorTagNamesWebagents(element);
  std::set<std::string> seen_tag_names;
  for (const std::string& tag_name : tag_names) {
    if (base::ContainsKey(seen_tag_names, tag_name))
      continue;
    ...
    seen_tag_names.insert(tag_name);
    if (tag_name == "LABEL") {
      inferred_label = InferLabelFromEnclosingLabelWebagents(element);
    } else if (tag_name == "DIV") {
      inferred_label = InferLabelFromDivTableWebagents(element);
    } else if (tag_name == "TD") {
      inferred_label = InferLabelFromTableColumnWebagents(element);
      if (!IsLabelValid(inferred_label, stop_words))
        inferred_label = InferLabelFromTableRowWebagents(element);
    } else if (tag_name == "DD") {
      inferred_label = InferLabelFromDefinitionListWebagents(element);
    } else if (tag_name == "LI") {
      inferred_label = InferLabelFromListItemWebagents(element);
    } else if (tag_name == "FIELDSET") {
      break;
    }
  ...
  // If we didn't find a label, check the value attr used as the placeholder.
  inferred_label = InferLabelFromValueAttrWebagents(element);
  if (IsLabelValid(inferred_label, stop_words))
    return inferred_label;
  else
    return base::string16();
...
void GetOptionStringsFromElementWebagents(
    const webagents::HTMLSelectElement& select_element,
    std::vector<base::string16>* option_values,
    std::vector<base::string16>* option_contents) {
  option_values->clear();
  option_contents->clear();
  // TODO(joelhockey): Is this the same as
  // blink::HTMLSelectElement::GetListItems?
  webagents::HTMLOptionsCollection options = select_element.options();

  // Constrain the maximum list length to prevent a malicious site from DOS'ing
  // the browser, without entirely breaking autocomplete for some extreme
  // legitimate sites: http://crbug.com/49332 and http://crbug.com/363094
  if (options.length() > static_cast<int>(kMaxListSize))
    return;

  option_values->reserve(options.length());
  option_contents->reserve(options.length());
  for (webagents::Element element : options) {
    if (element.IsHTMLOptionElement()) {
      webagents::HTMLOptionElement option = element.ToHTMLOptionElement();
      option_values->push_back(option.value().Utf16());
...
bool ExtractFieldsFromControlElementsWebagents(
    ...
    std::vector<std::unique_ptr<FormFieldData>>* form_fields,
    std::vector<bool>* fields_extracted,
    std::map<webagents::Element, FormFieldData*>* element_map) {
  DCHECK(form_fields->empty());
  DCHECK(element_map->empty());
  DCHECK_EQ(control_elements.size(), fields_extracted->size());

  for (size_t i = 0; i < control_elements.size(); ++i) {
    const webagents::Element& control_element = control_elements[i];

    if (!IsAutofillableElementWebagents(control_element))
      continue;

    // Create a new FormFieldData, fill it out and map it to the field's name.
    FormFieldData* form_field = new FormFieldData;
    WebFormControlElementToFormFieldWebagents(control_element,
                                              field_value_and_properties_map,
                                              extract_mask, form_field);
    form_fields->push_back(base::WrapUnique(form_field));
    (*element_map)[control_element] = form_field;
...
void MatchLabelsAndFieldsWebagents(
    const webagents::HTMLCollection& labels,
    std::map<webagents::Element, FormFieldData*>* element_map) {
  CR_DEFINE_STATIC_LOCAL(WebString, kFor, ("for"));
  CR_DEFINE_STATIC_LOCAL(WebString, kHidden, ("hidden"));

  for (webagents::Element item : labels) {
    webagents::HTMLLabelElement label = item.ToHTMLLabelElement();
    base::Optional<webagents::HTMLElement> control = label.control();
    FormFieldData* field_data = nullptr;
    ...
    if (!control) {
      ...
      // TODO(joelhockey): Handle element.IsFormControlElement.
      // Hack for now is to only check of HTMLInputElement.
    } else if (control->IsHTMLInputElement()) {
      // TODO(joelhockey): This element should be any type of
      // FormControlElement. I believe this is any element with form attr.
      webagents::HTMLInputElement form_control = control->ToHTMLInputElement();
      if (form_control.type() == kHidden)
        continue;
      // Typical case: look up |field_data| in |element_map|.
      auto iter = element_map->find(form_control);
      if (iter == element_map->end())
    ...
    base::string16 label_text = FindChildTextWebagents(label);
...
bool FormOrFieldsetsToFormDataWebagents(
    base::Optional<webagents::HTMLFormElement> form_element,
    const base::Optional<webagents::Element> form_control_element,
  ...
  if (!ExtractFieldsFromControlElementsWebagents(
          control_elements, field_value_and_properties_map, extract_mask,
          &form_fields, &fields_extracted, &element_map)) {
    return false;
  }
  ...
  if (form_element) {
    ...
    // previously created FormFieldData and set the FormFieldData's label to the
    // label.firstChild().nodeValue() of the label element.
    webagents::HTMLCollection labels =
        form_element->getElementsByTagName(kLabel);
    MatchLabelsAndFieldsWebagents(labels, &element_map);
  } else {
    // Same as the if block, but for all the labels in fieldsets.
    for (size_t i = 0; i < fieldsets.size(); ++i) {
      webagents::HTMLCollection labels =
          fieldsets[i].getElementsByTagName(kLabel);
      MatchLabelsAndFieldsWebagents(labels, &element_map);
    }
  }
  ...
  for (size_t i = 0, field_idx = 0;
    ...
    if (form_fields[field_idx]->label.empty()) {
      form_fields[field_idx]->label =
          InferLabelForElementWebagents(control_element, stop_words);
    }
    TruncateString(&form_fields[field_idx]->label, kMaxDataLength);
...
bool UnownedFormElementsAndFieldSetsToFormDataWebagents(
    const std::vector<webagents::Element>& fieldsets,
    const std::vector<webagents::Element>& control_elements,
    const base::Optional<webagents::Element> element,
    ...
    FormData* form,
    FormFieldData* field) {
  form->origin = GetCanonicalOriginForDocumentWebagents(document);
  form->is_form_tag = false;

  return FormOrFieldsetsToFormDataWebagents(
      base::Optional<webagents::HTMLFormElement>(), element, fieldsets,
      control_elements, field_value_and_properties_map, extract_mask, form,
      field);
}
...
bool ExtractFormDataWebagents(const webagents::HTMLFormElement& form_element,
  ...
  return WebFormElementToFormDataWebagents(
      form_element, base::Optional<webagents::Element>(), nullptr,
      static_cast<form_util::ExtractMask>(form_util::EXTRACT_VALUE |
                                          form_util::EXTRACT_OPTION_TEXT |
                                          form_util::EXTRACT_OPTIONS),
      data, NULL);
...
GURL GetCanonicalActionForFormWebagents(
    const webagents::HTMLFormElement& form) {
  WebString action = form.action();
  if (action.IsNull())
    action = WebString("");  // missing 'action' attribute implies current URL.
  GURL full_action(form.ownerDocument().CompleteURL(action));
  return StripAuthAndParams(full_action);
}
...
bool IsAutofillableInputElementWebagents(const webagents::Element& element) {
  if (!element.IsHTMLInputElement())
    return false;
  webagents::HTMLInputElement input = element.ToHTMLInputElement();
  return input.type() == "text" || input.type() == "month" ||
         IsCheckableElementWebagents(input);
}
...
bool IsAutofillableElementWebagents(const webagents::Element& element) {
  return IsAutofillableInputElementWebagents(element) ||
...
const base::string16 GetFormIdentifierWebagents(
    const webagents::HTMLFormElement& form) {
...
bool FormAutocomplete(base::Optional<webagents::HTMLFormElement> form) {
  return !form || base::ToLowerASCII(form->autocomplete().Ascii()) != "off";
}
...
void WebFormControlElementToFormFieldWebagents(
  ...
  // it will be saved to |name|. See HTMLFormControlElement::nameForAutofill.
  // TODO(joelhockey): Handle FormControlElements.
  // field->name = element.NotStandardNameForAutofill().Utf16();
  base::string16 id = element.getAttribute(kId).Utf16();
  if (id != field->name)
    field->id = id;

  // TODO(joelhockey): Handle element.FormControlType.
  // Hack for now is to just check HTMLInputElement.
  // field->form_control_type = element.FormControlType().Utf8();
  if (element.IsHTMLInputElement()) {
    webagents::HTMLInputElement input = element.ToHTMLInputElement();
    field->name = input.NotStandardNameForAutofill().Utf16();
    field->form_control_type = input.type().Utf8();
  }
  field->autocomplete_attribute = element.getAttribute(kAutocomplete).Utf8();
  if (field->autocomplete_attribute.size() > kMaxDataLength) {
    // Discard overly long attribute values to avoid DOS-ing the browser
  ...
  if (field_value_and_properties_map) {
    FieldValueAndPropertiesMaskMapWebagents::const_iterator it =
        field_value_and_properties_map->find(element);
    if (it != field_value_and_properties_map->end())
      field->properties_mask = it->second.second;
  }
  ...
  if (IsAutofillableInputElementWebagents(element) ||
      element.IsHTMLTextAreaElement() || element.IsHTMLSelectElement()) {
    field->is_autofilled = element.NotStandardIsAutofilled();
    if (!g_prevent_layout)
      field->is_focusable = element.IsFocusable();
    // moved below
    // field->should_autocomplete = element.AutoComplete();
    ...
    // Use 'text-align: left|right' if set or 'direction' otherwise.
    // See crbug.com/482339
    blink::WebString direction =
        element.getComputedStyle().getPropertyValue("direction");
    field->text_direction = direction == "rtl" ? base::i18n::RIGHT_TO_LEFT
                                               : base::i18n::LEFT_TO_RIGHT;
    blink::WebString text_align =
        element.getComputedStyle().getPropertyValue("textAlign");
    if (text_align == "left")
      field->text_direction = base::i18n::LEFT_TO_RIGHT;
  ...
  base::string16 value;
  if (IsAutofillableInputElementWebagents(element)) {
    webagents::HTMLInputElement input_element = element.ToHTMLInputElement();
    value = input_element.value().Utf16();
    blink::WebString input_autocomplete = input_element.autocomplete();
    field->should_autocomplete =
        (input_autocomplete.length() > 0 &&
         base::ToLowerASCII(input_autocomplete.Ascii()) == "on") ||
        FormAutocomplete(input_element.form());
    if (input_element.type() == "text")
      field->max_length = input_element.maxLength();

    SetCheckStatus(field, IsCheckableElementWebagents(input_element),
                   input_element.checked());
  } else if (element.IsHTMLTextAreaElement()) {
    const webagents::HTMLTextAreaElement text_area_element =
        element.ToHTMLTextAreaElement();
    value = text_area_element.value().Utf16();
    field->should_autocomplete = FormAutocomplete(text_area_element.form());
  } else if (extract_mask & EXTRACT_OPTIONS) {
    // Set option strings on the field if available.
    DCHECK(element.IsHTMLSelectElement());
    const webagents::HTMLSelectElement select_element =
        element.ToHTMLSelectElement();
    value = select_element.value().Utf16();
    field->should_autocomplete = FormAutocomplete(select_element.form());
    GetOptionStringsFromElementWebagents(select_element, &field->option_values,
                                         &field->option_contents);
  }
  ...
  if (element.IsHTMLSelectElement() && (extract_mask & EXTRACT_OPTION_TEXT)) {
    const webagents::HTMLSelectElement select_element =
        element.ToHTMLSelectElement();
    // Convert the |select_element| value to text if requested.
    webagents::HTMLOptionsCollection options = select_element.options();
    for (webagents::Element element : options) {
      if (element.IsHTMLOptionElement()) {
        const webagents::HTMLOptionElement option_element =
            element.ToHTMLOptionElement();
        if (option_element.value().Utf16() == value) {
...
bool WebFormElementToFormDataWebagents(
    ...
    FormData* form,
    FormFieldData* field) {
  if (!form_element.ownerDocument().NotStandardIsAttachedToFrame())
    return false;

  form->name = GetFormIdentifierWebagents(form_element);
  form->origin =
      GetCanonicalOriginForDocumentWebagents(form_element.ownerDocument());
  form->action =
      form_element.ownerDocument().CompleteURL(form_element.action());
  ...
  // TODO(joelhockey): Is this the same as
  // blink::WebFormElement::GetFormControlElements()?
  for (webagents::Element element : form_element.elements())
    control_elements.push_back(element);

  std::vector<webagents::Element> dummy_fieldset;
  return FormOrFieldsetsToFormDataWebagents(
      form_element, form_control_element, dummy_fieldset, control_elements,
      field_value_and_properties_map, extract_mask, form, field);
  return true;
...
std::vector<webagents::Element> GetUnownedFormFieldElementsWebagents(
  ...
  for (webagents::Element element : elements) {
    // TODO(joelhockey): Implement some way to access FormControlElement.
    // I think a FormControlElement is any element with form attr.
    // Hack for now is to only consider HTMLInputElement.
    if (element.IsHTMLInputElement()) {
      webagents::HTMLInputElement control = element.ToHTMLInputElement();
      if (!control.form())
        unowned_fieldset_children.push_back(control);
    }

    if (fieldsets && element.tagName() == "fieldset" &&
        !IsElementInsideFormOrFieldSetWebagents(element)) {
      fieldsets->push_back(element);
    }
  }
  return unowned_fieldset_children;
...
std::vector<webagents::Element>
GetUnownedAutofillableFormFieldElementsWebagents(
    const webagents::HTMLAllCollection& elements,
    std::vector<webagents::Element>* fieldsets) {
...
bool UnownedCheckoutFormElementsAndFieldSetsToFormDataWebagents(
    const std::vector<webagents::Element>& fieldsets,
    const std::vector<webagents::Element>& control_elements,
    base::Optional<webagents::Element> element,
    const webagents::Document& document,
    ExtractMask extract_mask,
    FormData* form,
    FormFieldData* field) {
  ...
  // false positives found on the non-checkout web. See
  // http://crbug.com/462375.
  base::Optional<webagents::Element> html_element = document.documentElement();
  ...
  if (!lang.empty() &&
      !base::StartsWith(lang, "en", base::CompareCase::INSENSITIVE_ASCII)) {
    return UnownedFormElementsAndFieldSetsToFormDataWebagents(
        fieldsets, control_elements, element, document, nullptr, extract_mask,
        form, field);
  }
  ...
  // autocomplete attribute to the formless autofill.
  CR_DEFINE_STATIC_LOCAL(WebString, kOffAttribute, ("off"));
  std::vector<webagents::Element> elements_with_autocomplete;
  for (const webagents::Element& element : control_elements) {
    blink::WebString autocomplete = element.getAttribute("autocomplete");
    if (autocomplete.length() && autocomplete != kOffAttribute)
      elements_with_autocomplete.push_back(element);
  }
...
bool FindFormAndFieldForFormControlElementWebagents(
  ...
  return WebFormElementToFormDataWebagents(
      *form_element, base::Optional<webagents::Element>(element), nullptr,
      extract_mask, form, field);
  return true;
}
