...
base::string16 FindChildTextInner(const WebNode& node,
  ...
  while (!child.IsNull()) {
    base::string16 child_text =
        FindChildTextInner(child, depth - 1, divs_to_skip, right_limits);
    node_text = CombineAndCollapseWhitespace(node_text, child_text, add_space);
    add_space = child.IsTextNode() && node_text.empty();
...
// Same as FindChildText() below, but with a list of div nodes to skip, and a
// list of element that mark the right limit of the DFS
