...
TEST(TaskTrackerTest, GetTabTasks) {
  ...
  TabTasks* tab_tasks =
      task_tracker.GetTabTasks(kTab1, TabTasks::kInvalidTaskID);
  ASSERT_NE(tab_tasks, nullptr);
  EXPECT_EQ(task_tracker.GetTabTasks(kTab1, TabTasks::kInvalidTaskID),
            tab_tasks);
  EXPECT_NE(task_tracker.GetTabTasks(kTab2, TabTasks::kInvalidTaskID),
            tab_tasks);
}
