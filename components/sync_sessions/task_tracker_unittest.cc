...
TEST(TaskTrackerTest, GetTabTasks) {
  ...
  TabTasks* tab_tasks = task_tracker.GetTabTasks(kTab1, kInvalidTaskID);
  ASSERT_NE(tab_tasks, nullptr);
  EXPECT_EQ(task_tracker.GetTabTasks(kTab1, kInvalidTaskID), tab_tasks);
  EXPECT_NE(task_tracker.GetTabTasks(kTab2, kInvalidTaskID), tab_tasks);
}
...
TEST(TaskTrackerTest, CreateTabTasksFromSourceTab) {
  TaskTracker task_tracker;
  TabTasks* source_tab_tasks = task_tracker.GetTabTasks(kTab1, kInvalidTaskID);
