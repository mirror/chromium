...
ui_devtools::protocol::Response UIDevToolsCSSAgent::getMatchedStylesForNode(
  ...
  *inline_style = GetStylesForNode(ui_element);
  if (!inline_style)
...
ui_devtools::protocol::Response UIDevToolsCSSAgent::setStyleTexts(
  ...
  for (size_t i = 0; i < edits->length(); i++) {
    ...
    bool visible = false;
    if (!GetPropertiesForNodeId(ui_element, &updated_bounds, &visible))
      return NodeNotFoundError(node_id);
    ...
    if (!SetPropertiesForNodeId(ui_element, updated_bounds, visible))
      return NodeNotFoundError(node_id);
  }
...
void UIDevToolsCSSAgent::OnNodeBoundsChanged(UIElement* ui_element) {
  InvalidateStyleSheet(ui_element);
}

std::unique_ptr<ui_devtools::protocol::CSS::CSSStyle>
UIDevToolsCSSAgent::GetStylesForNode(UIElement* ui_element) {
  gfx::Rect bounds;
  bool visible = false;
  return GetPropertiesForNodeId(ui_element, &bounds, &visible)
...
bool UIDevToolsCSSAgent::GetPropertiesForNodeId(UIElement* ui_element,
                                                gfx::Rect* bounds,
                                                bool* visible) {
  if (ui_element) {
...
bool UIDevToolsCSSAgent::SetPropertiesForNodeId(UIElement* ui_element,
                                                const gfx::Rect& bounds,
                                                bool visible) {
  if (ui_element) {
