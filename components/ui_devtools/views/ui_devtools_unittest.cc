...
TEST_F(UIDevToolsTest, OneUIElementContainsAnother) {
  ...
  int show_distances = dom_agent()->show_distances();

  // Swapping R1 and R2 shouldn't change show_distances case.
  dom_agent()->ShowDistances(inside_rect_id, outside_rect_id);
  DCHECK_EQ(show_distances, dom_agent()->show_distances());
...
TEST_F(UIDevToolsTest, OneUIElementStaysHorizontalLeftOfAnother) {
  ...
  EXPECT_TRUE(element_outside.second == outside_rect);
  EXPECT_TRUE(element_inside.second == inside_rect);
  DCHECK_EQ(dom_agent()->show_distances(),
            RectPositionsType::R1_HORIZONTAL_FULL_LEFT_R2);
}
