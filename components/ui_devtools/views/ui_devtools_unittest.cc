...
TEST_F(UIDevToolsTest, FindElementIdTargetedByPoint) {
  ...
  EXPECT_NE(0, dom_agent()->FindElementIdTargetedByPoint(gfx::Point(1, 1)));
}

// Tests that user can enter, click on a targeted UI element and cleanly
// exit inspect mode.
TEST_F(UIDevToolsTest, EnterAndExitInspectMode) {
  std::unique_ptr<views::Widget> widget(
      CreateTestWidget(gfx::Rect(1, 1, 1, 1)));
  ...
  gfx::Point p(1, 1);
  int node_id = dom_agent()->FindElementIdTargetedByPoint(p);
  ...
  EXPECT_NE(0, GetOverlayNodeHighlightRequestedCount(node_id));
  EXPECT_EQ(0, GetOverlayInspectNodeRequestedCount(node_id));

  // Clicking on the widget should request that element be inspected.
  generator.PressLeftButton();
  EXPECT_NE(0, GetOverlayInspectNodeRequestedCount(node_id));
