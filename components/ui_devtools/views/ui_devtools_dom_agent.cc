...
void UIDevToolsDOMAgent::ShowDistances(int pinned_id, int element_id) {
  ...
  if (r1.Contains(r2)) {
    ...
    if (r1.y() <= r2.y() && r2.y() <= r1.y() + r1.height())
      show_distances_ = RectPositionsType::R1_HORIZONTAL_LEFT_R2;
...
void DrawTextOnLine(const gfx::Rect& hovered_rect,
  ...
  if (hovered_rect.width() == 0)
    utf16_text = base::UTF8ToUTF16(std::to_string(hovered_rect.height()));
  else if (hovered_rect.height() == 0)
    utf16_text = base::UTF8ToUTF16(std::to_string(hovered_rect.width()));
  else
    utf16_text = base::UTF8ToUTF16(hovered_rect.size().ToString());
...
void UIDevToolsDOMAgent::Draw_R1_CONTAINS_R2(const gfx::RectF& pinned_rectF,
...
void UIDevToolsDOMAgent::OnPaintLayer(const ui::PaintContext& context) {
  ...
  switch (show_distances_) {
    case RectPositionsType::R1_CONTAINS_R2:
      Draw_R1_CONTAINS_R2(pinned_rectF, hovered_rectF, flags, canvas_);
      return;
    case RectPositionsType::R1_HORIZONTAL_LEFT_R2:
      NOTIMPLEMENTED();
      return;
