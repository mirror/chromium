...
UIDevToolsDOMAgent::UIDevToolsDOMAgent()
    : is_building_tree_(false),
      ...
      show_distances_(HighlightRectsConfiguration::NO_DRAW) {
...
void UIDevToolsDOMAgent::ShowDistancesInHighlightOverlay(int pinned_id,
  ...
  if (r1.Contains(r2)) {
    show_distances_ = HighlightRectsConfiguration::R1_CONTAINS_R2;
    ...
    if ((r1.y() <= r2.y() && r2.y() <= r1.bottom()) ||
      ...
      show_distances_ = HighlightRectsConfiguration::R1_HORIZONTAL_FULL_LEFT_R2;
    } else if (r1.bottom() <= r2.y()) {
      show_distances_ = HighlightRectsConfiguration::R1_TOP_FULL_LEFT_R2;
    } else if (r1.y() >= r2.bottom()) {
      show_distances_ = HighlightRectsConfiguration::R1_BOTTOM_FULL_LEFT_R2;
    }
  } else if (r1.x() <= r2.x() && r2.x() <= r1.right()) {
    if (r1.bottom() <= r2.y())
      show_distances_ = HighlightRectsConfiguration::R1_TOP_PARTIAL_LEFT_R2;
    else if (r1.y() >= r2.bottom())
      show_distances_ = HighlightRectsConfiguration::R1_BOTTOM_PARTIAL_LEFT_R2;
    else if (r1.Intersects(r2))
      show_distances_ = HighlightRectsConfiguration::R1_INTERSECTS_R2;
    else
      NOTREACHED();
  } else {
    show_distances_ = HighlightRectsConfiguration::NO_DRAW;
...
void UIDevToolsDOMAgent::OnPaintLayer(const ui::PaintContext& context) {
  ...
  // Display size only.
  if (show_distances_ == HighlightRectsConfiguration::NO_DRAW) {
