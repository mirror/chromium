...
int UIDevToolsDOMAgent::FindElementByEventHandler(const gfx::Point& p) {
  for (auto* element_window : window_element_root_->children()) {
    ...
    if (!widget)
      return FindUIElementIdForWindow(element_window, point_window);

    views::View* view = widget->GetRootView();
    if (!view)
      return FindUIElementIdForWidget(element_window, widget);
    ...
    DCHECK(point_view);
    return FindUIElementIdForView(element_window, point_view);
  }
  return 0;
}
