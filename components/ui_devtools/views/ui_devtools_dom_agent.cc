...
void UIDevToolsDOMAgent::OnPaintLayer(const ui::PaintContext& context) {
  ...
  if (show_size_on_canvas_) {
    ...
    ResourceBundle* const rb = &ResourceBundle::GetSharedInstance();
    const gfx::FontList base_font =
        rb->GetFontList(ResourceBundle::BaseFont)
            .DeriveWithHeightUpperBound(text_size.height());
    render_text_->SetFontList(base_font);
