...
void UIDevToolsDOMAgent::OnPaintLayer(const ui::PaintContext& context) {
  ui::PaintRecorder recorder(context, screen_bounds_.size());
  gfx::Canvas* canvas_ = recorder.canvas();
  gfx::RectF rectF(hovered_element_bounds_);
...
void UIDevToolsDOMAgent::UpdateHighlight(
  ...
  screen_bounds_ = root->bounds();
  layer_for_highlighting_->SetBounds(screen_bounds_);
  ...
  layer_for_highlighting_->SchedulePaint(screen_bounds_);
}
