...
void UIDevToolsDOMAgent::OnPaintLayer(const ui::PaintContext& context) {
  ...
  gfx::RectF rectF(hovered_element_bounds);
  ...
  canvas_->DrawLine(gfx::PointF(0.0f, rectF.y()),
                    gfx::PointF(screen_bounds.right(), rectF.y()), flags);

  // Bottom horizontal dotted line from left to right.
  canvas_->DrawLine(gfx::PointF(0.0f, rectF.bottom() + 0.5f),
                    gfx::PointF(screen_bounds.right(), rectF.bottom() + 0.5f),
                    flags);

  // Left vertical dotted line from top to bottom.
  canvas_->DrawLine(gfx::PointF(rectF.x() - 0.5f, 0.0f),
                    gfx::PointF(rectF.x() - 0.5f, screen_bounds.bottom()),
                    flags);

  // Right vertical dotted line from top to bottom.
  canvas_->DrawLine(gfx::PointF(rectF.right() + 0.5f, 0.0f),
                    gfx::PointF(rectF.right() + 0.5f, screen_bounds.bottom()),
                    flags);

  // Draw ui element bounds.
  canvas_->DrawRect(rectF, SK_ColorBLUE);
}
