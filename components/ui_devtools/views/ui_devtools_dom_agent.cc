...
void DrawTextWithAnyBounds(float x1,
  ...
  if (x2 > x1 || y2 > y1) {
    DrawTextOnLine(gfx::Rect(x1, y1, x2 - x1, y2 - y1), side, canvas,
                   render_text);
  } else {
    DrawTextOnLine(gfx::Rect(x2, y2, x1 - x2, y1 - y2), side, canvas,
                   render_text);
  }
}
...
void DrawR1HorizontalFullLeftR2(const gfx::RectF& pinned_rectF,
                                ...
                                gfx::RenderText* render_text) {
  // Horizontal left distance line.
  float x1 = pinned_rectF.x() + pinned_rectF.width();
  float y1 = pinned_rectF.y() + pinned_rectF.height() / 2;
  float x2 = hovered_rectF.x();
  float y2 = pinned_rectF.y() + pinned_rectF.height() / 2;
  canvas->DrawLine(gfx::PointF(x1, y1), gfx::PointF(x2, y2), flags);
...
void UIDevToolsDOMAgent::OnPaintLayer(const ui::PaintContext& context) {
  ...
  gfx::RectF hovered_rectF(hovered_rect_);
  ...
  if (show_distances_ == RectPositionsType::NO_DRAW) {
    ...
    // Draw hovered_rectF bounds.
    flags.setPathEffect(0);
    canvas->DrawRect(hovered_rectF, flags);

    if (show_size_on_canvas_)
      DrawTextOnLine(hovered_rect_, RectSide::BOTTOM_SIDE, canvas,
                     render_text_.get());
    return;
  }
  flags.setPathEffect(0);
  ...
  gfx::RectF pinned_rectF(pinned_rect_);

  // Draw pinned_rectF bounds in blue.
  canvas->DrawRect(pinned_rectF, flags);

  // Draw hovered_rectF bounds in green.
  ...
  if (pinned_rectF.x() < hovered_rectF.x())
    std::swap(pinned_rectF, hovered_rectF);
  ...
  switch (show_distances_) {
    ...
    case RectPositionsType::R1_HORIZONTAL_FULL_LEFT_R2:
      DrawR1HorizontalFullLeftR2(pinned_rectF, hovered_rectF, flags, canvas,
                                 render_text_.get());
      return;
