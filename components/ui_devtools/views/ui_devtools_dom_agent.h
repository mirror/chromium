...
class UIDevToolsDOMAgent : public ui_devtools::UiDevToolsBaseAgent<
 ...
 public:
  ...
  // |p| is in the local coodinate space of |window_element_root_|. The function
  // first searches for the targeted window, then the targeted widget (if one
  // exists), then the targeted view (if one exists).
 ...
 private:
  ...
  // TODO: |layer_for_highlighting_| should be owned by the overlay agent.
