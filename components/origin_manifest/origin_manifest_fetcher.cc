...
void OriginManifestFetcher::Fetch(
  ...
  */
  ...
  VLOG(1) << "Going to fetch: '" << origin_ << "originmanifest/" << version_
          << ".json"
          << "'";
...
void OriginManifestFetcher::OnURLFetchComplete(const net::URLFetcher* source) {
  ...
  std::move(callback_).Run(mojom::OriginManifest::New(
      origin_, version_, response_str, mojom::CORSPreflight::New(),
