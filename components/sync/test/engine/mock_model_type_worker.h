...
class MockModelTypeWorker : public CommitQueue {
 public:
  ...
  // Verify that the |n|th commit request list has the corresponding commit requests for |tag_hashes|
  // with |value| set.
  ...
  // // Kept for convience for comptability with the existing code.
  // // The assumption that each pending commit contains a single CommitRequestData doesn't hold anymore.
  // void VerifyPendingCommits(const std::vector<std::string>& tag_hashes);

  // Verify the pending commits each contain a list of CommitRequestData and they
  // have the same hashes in the same order as |tag_hashes|.
  void VerifyPendingCommits(const std::vector<std::vector<std::string>>& tag_hashes);
