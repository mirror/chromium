...
// broadly and avoid LOCAL_T). If that were possible, we could migrate away
// from typed stuff.
// TODO: See DeviceInfo TestWithLocalMetadata and TestWithLocalDataAndMetadata.
template <typename N, typename S, typename Impl>
class ModelTypeSyncBridgeTester {
 public:
  // LOCAL_T must hold sufficient information to create local entities.
  using LOCAL_T = N;
  // SPECIFICS is the sync proto, holding analogous information to LOCAL_T.
  using SPECIFICS = S;
  ...
  // the storage key.
  virtual std::string StoreEntityInLocalModel(LOCAL_T* local) = 0;
  ...
  struct ConflictResolution {
    LOCAL_T local;
    std::unique_ptr<SPECIFICS> remote;
    std::unique_ptr<SPECIFICS> expected_result;
  };

  // Conflict resolution. By default, server wins.
  virtual std::vector<ConflictResolution> GetConflictResolutionTests() const {
    // TODO(mastiz): Populate some default tests here.
    return {};
  }
  ...
  TestEntityDataWithStorageKey CreateAndStoreLocalTestEntity1() {
    ...
    std::string storage_key = StoreEntityInLocalModel(&entity.local);
    return {storage_key, std::move(entity.local), std::move(entity.specifics)};
  }
...
class SyncBridgeTest : public testing::Test {
 public:
  ...
  using LOCAL_T = typename TESTER::LOCAL_T;
...
TYPED_TEST_P(SyncBridgeTest, InitWithNonEmptyModel) {
  this->InitializeBridgeAndWaitUntilReady();
  this->tester()->CreateAndStoreLocalTestEntity1();
...
TYPED_TEST_P(SyncBridgeTest, GetAllData) {
  this->InitializeBridgeAndWaitUntilReady();
  const auto entity1 = this->tester()->CreateAndStoreLocalTestEntity1();
  const auto entity2 = this->tester()->CreateAndStoreLocalTestEntity2();
...
TYPED_TEST_P(SyncBridgeTest, GetAllDataAfterRestart) {
  this->InitializeBridgeAndWaitUntilReady();
  const auto entity1 = this->tester()->CreateAndStoreLocalTestEntity1();
  const auto entity2 = this->tester()->CreateAndStoreLocalTestEntity2();
...
TYPED_TEST_P(SyncBridgeTest, SimpleMerge) {
  using testing::_;
  this->InitializeBridgeAndWaitUntilReady();
  const auto entity1 = this->tester()->CreateAndStoreLocalTestEntity1();
...
TYPED_TEST_P(SyncBridgeTest, LocalAddAfterSyncStarted) {  // AddLocalTypedUrl
  ...
  const std::string storage_key =
      this->tester()->StoreEntityInLocalModel(&entity1.local);
  loop.Run();
...
TYPED_TEST_P(SyncBridgeTest,
  ...
  this->tester()->ApplySyncChangesAndWait(std::move(metadata_changes), {});
