...
class ModelTypeSyncBridgeTester {
 public:
  ...
  virtual std::unique_ptr<ModelTypeSyncBridgeTester> Restart(
      std::unique_ptr<ModelTypeSyncBridgeTester> this_unique_ptr) = 0;
  ...
  // Mutations.
  virtual sync_pb::EntitySpecifics GetMutatedTestEntity1Specifics() const = 0;
  virtual void MutateStoredTestEntity1() = 0;
  // Some data types (DeviceInfo) have no actual local API for mutations, and
  // instead the set of locally created entities is fixed.
  virtual std::vector<TestEntityDataWithStorageKey> HasFixedLocalEntities();
...
class SyncBridgeTest
 ...
 public:
  ...
  template <typename T>
  class Future : public base::Callback<T()> {
   public:
    Future(base::Callback<T()> cb) : base::Callback<T()>(std::move(cb)) {}
    T WaitForResult() { return this->Run(); }
  };

  Future<std::unique_ptr<MetadataBatch>> InitializeBridge() WARN_UNUSED_RESULT;
  ...
  Future<base::Optional<ModelError>> StartSyncing(
 ...
 private:
  Future<std::unique_ptr<MetadataBatch>> GetModelReadyToSyncFuture();
