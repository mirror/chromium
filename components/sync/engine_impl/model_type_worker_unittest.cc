...
class ModelTypeWorkerTest : public ::testing::Test {
 public:
  ...
  CommitRequestDataList GenerateCommitRequest(const std::string& name, const std::string& value) {
    return GenerateCommitRequest(GenerateTagHash(name), GenerateSpecifics(name, value));
  }

  CommitRequestDataList GenerateCommitRequest(const std::string& tag_hash,
  ...
  void DeliverRawUpdates(const SyncEntityList& list) {
    DVLOG(0) << __FUNCTION__ << "{PAV}1";
    worker()->ProcessGetUpdatesResponse(
        server()->GetProgress(), server()->GetContext(), list, nullptr);
    DVLOG(0) << __FUNCTION__ << "{PAV}2";
    worker()->ApplyUpdates(nullptr);
    DVLOG(0) << __FUNCTION__ << "{PAV}3";
  }
...
// {PAV} Need to implement blocking on encryption first.
...
// {PAV} In this test cryptographer is ready, but worker still contains
// undecryptable entity. Its encrypted data is corrupt. We should drop it on the
// floor.
