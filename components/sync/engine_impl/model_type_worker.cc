...
SyncerError ModelTypeWorker::ProcessGetUpdatesResponse(
  ...
  *model_type_state_.mutable_progress_marker() = progress_marker;
  ...
  for (const sync_pb::SyncEntity* update_entity : applicable_updates) {
    ...
    WorkerEntityTracker* entity = GetOrCreateEntityTracker(data.client_tag_hash);

    if (!entity->UpdateContainsNewVersion(response_data)) {
      // {PAV} This counter will be broken.
    ...
    if (!specifics.has_encrypted()) {
      ...
      if (DecryptSpecifics(specifics, &data.specifics)) {
        DVLOG(0) << __FUNCTION__ << "{PAV} Decrypt succeeded";
        response_data.entity = data.PassToPtr();
        ...
        DVLOG(0) << __FUNCTION__ << "{PAV} Decrypt failed";
      }
    } else {
...
void ModelTypeWorker::NudgeForCommit() {
  DCHECK(thread_checker_.CalledOnValidThread());
  DVLOG_IF(0, GetModelType() == AUTOFILL) << __FUNCTION__ << "{PAV}";
  has_local_changes_ = true;
  NudgeIfReady();
}

void ModelTypeWorker::NudgeIfReady() {
...
std::unique_ptr<CommitContribution> ModelTypeWorker::GetContribution(
  ...
  if (!request->WasCancelled()) {
    ...
    DVLOG_IF(0, GetModelType() == AUTOFILL) << __FUNCTION__ << "{PAV}: Valid response: size="
        << response.size();
    has_local_changes_ = false;
  }

  // Add local changes to entity trackers.
  ...
  for (const CommitRequestData& commit : response) {
    ...
    CreateEntityTracker(data.client_tag_hash)->RequestCommit(commit);
  }

  // Copy local changes into on-the-wire protobuf.
  google::protobuf::RepeatedPtrField<sync_pb::SyncEntity> commit_entities;

  // TODO(rlarocque): Avoid iterating here.
  for (const auto& entity_kv : entities_) {
    WorkerEntityTracker* entity = entity_kv.second.get();
