...
// bigest number in the vector |times|, be aware, this function will change the
// order of the values in |times|.
...
void ExpireEntriesByItemLimit(syncable::Directory* dir,
  ...
  std::vector<base::Time> all_proto_times;
  for (size_t i = 0; i < handles.size(); ++i) {
    syncable::ModelNeutralMutableEntry entry(trans, syncable::GET_BY_HANDLE,
                                             handles[i]);
    all_proto_times.push_back(entry.GetMtime());
  }
  base::Time expired_time =
      FindTheNthBigestTime(all_proto_times, limited_number);
