...
class TestModelTypeSyncBridge : public FakeModelTypeSyncBridge {
 public:
  ...
  base::OnceClosure GetDataCallback() { return data_callback_; }
...
TEST_F(SharedModelTypeProcessorTest, StartErrors) {
  ...
  ExpectError();
  OnSyncStarting();
  // TODO(mamir): Commit data aren't loaded automatically now on startup.
  // Simply remove this?
  // OnCommitDataLoaded();
}
...
TEST_F(SharedModelTypeProcessorTest, LoadPendingCommit) {
  ...
  // They was 1 before the worker is never ready before loading the data
