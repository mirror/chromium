...
bool UserEventServiceImpl::ShouldRecordEvent(
    const UserEventSpecifics& specifics) {
  // TODO(crbug.com/781765): Consent recording should not fail if sync_service
  // is not yet initialized.
  if (!sync_service_->IsEngineInitialized()) {
    return false;
  }
  // Consent recording doesn't require history sync to be enabled because it
  // is not related to history. It is also not user data, so it doesn't get
  // disabled for users with passphrase.
