...
void UserEventServiceImpl::RecordUserEvent(
  ...
  if (ShouldRecordEvent(*specifics)) {
    ...
    UserEventSpecifics::EventCase event_case = specifics->event_case();
    bridge_->RecordUserEvent(std::move(specifics));
    trial_recorder_.OnEventCaseRecorded(event_case);
  }
}
...
void UserEventServiceImpl::RegisterDependentFieldTrial(
  ...
  trial_recorder_.RegisterDependentFieldTrial(event_case, trial_name);
}
