...
class TestUserEventService : public UserEventServiceImpl {
 public:
  TestUserEventService(SyncService* sync_service,
                       std::unique_ptr<UserEventSyncBridge> bridge)
      : UserEventServiceImpl(sync_service, std::move(bridge)) {}
  // Satisfies KeyedService contract of calling Shutdown before destructor
  // without cluttering individual test cases.
  ~TestUserEventService() override { Shutdown(); }
};
...
TEST_F(UserEventServiceImplTest, ShouldRecord) {
  TestUserEventService service(sync_service(), MakeBridge());
...
TEST_F(UserEventServiceImplTest, ShouldRecordNoHistory) {
  ...
  TestUserEventService service(&no_history_sync_service, MakeBridge());
...
TEST_F(UserEventServiceImplTest, ShouldRecordPassphrase) {
  ...
  TestUserEventService service(&passphrase_sync_service, MakeBridge());
...
TEST_F(UserEventServiceImplTest, ShouldRecordEngineOff) {
  ...
  TestUserEventService service(&engine_not_initialized_sync_service,
                               MakeBridge());
...
TEST_F(UserEventServiceImplTest, SessionIdIsDifferent) {
  TestUserEventService service1(sync_service(), MakeBridge());
  ...
  TestUserEventService service2(sync_service(), MakeBridge());
