...
class DeviceInfoTester : public syncer::ModelTypeSyncBridgeTester,
 ...
 public:
  ...
  EntitySpecifics GetMutatedTestEntity1Specifics() const override {
    return WrapSpecifics(CreateSpecifics(7));
  }
  ...
  std::vector<TestEntityDataWithStorageKey> HasFixedLocalEntities() override {
    ...
    std::vector<TestEntityDataWithStorageKey> entities;
    entities.emplace_back(
        TestEntityData(std::move(wrapped_device_info),
                       WrapSpecifics(CreateSpecifics(kDefaultLocalSuffix))),
        guid);
    return entities;
  }
