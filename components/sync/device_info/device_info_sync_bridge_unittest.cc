...
class DeviceInfoTester
 ...
 public:
  static void Restart(std::unique_ptr<DeviceInfoTester>* tester) {
    ...
    auto provider = std::move((*tester)->provider_);
    auto store = (*tester)->bridge_->StealStoreForTest();
    tester->reset();
    *tester = std::make_unique<DeviceInfoTester>(factory, std::move(provider),
                                                 std::move(store));
  }
  ...
  TestEntityData GetTestEntity1() const override {
    return {CreateModel(1), SpecificsToEntity(CreateSpecifics(1))};
  }

  TestEntityData GetTestEntity2() const override {
    return {CreateModel(2), SpecificsToEntity(CreateSpecifics(2))};
  }

  std::string StoreEntityInLocalModel(
      std::unique_ptr<DeviceInfo>* local) override {
    std::string guid = (*local)->guid();
    // TODO/DONOTSUBMIT
  ...
  EntityDataPtr GetMutatedTestEntity1Specifics() const override {
    return SpecificsToEntity(CreateSpecifics(7));
  }
...
class DeviceInfoSyncBridgeTest : public testing::Test {
 protected:
  ...
  void RestartBridge() {
    DeviceInfoTester::Restart(&tester_);
    base::RunLoop().RunUntilIdle();
  }
...
TEST_F(DeviceInfoSyncBridgeTest, MergeEmpty) {
  ...
  EXPECT_EQ(1, change_count());
  auto error =
      MergeSyncData(bridge()->CreateMetadataChangeList(), EntityChangeList());
  EXPECT_FALSE(error);
  EXPECT_EQ(1, change_count());
