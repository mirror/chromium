    ...
    public static void verify(RandomAccessFile apkFile, PublicKey publicKey)
        ...
        if (certChains.length == 0) {
            throw new SignatureNotFoundException(
                    "Signature verifier returned no certificate chains.");
        }
        ...
        // lastException should only ever be null if certChains does contain at least one
        // X509Certificate array, but all of them are empty.
        if (lastException == null) {
            throw new SignatureNotFoundException(
                    "Signature verifier returned empty certificate chains.");
        }
        throw lastException;
    }
