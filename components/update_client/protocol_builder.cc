...
std::string BuildUpdateCheckRequest(
  ...
  for (const auto& id : ids_checked) {
    ...
    const auto& disable_reasons = crx_component.disable_reasons;
    if (disable_reasons.reasons) {
      base::StringAppendF(&app, "<disable");
      for (int enum_value = 1; enum_value < disable_reasons.last_reason;
           enum_value <<= 1) {
        if (disable_reasons.reasons & enum_value) {
          base::StringAppendF(&app, " reason=\"%d\"", enum_value);
        }
      }
      base::StringAppendF(&app, "/>");
    }
