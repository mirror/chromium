...
std::string BuildUpdateCheckRequest(
  ...
  for (const auto& id : ids_checked) {
    ...
    base::StringAppendF(&app, " enabled=\"%d\">",
                        disabled_reasons.reasons ? 0 : 1);

    if (disabled_reasons.reasons) {
      int reasons = disabled_reasons.reasons;
      for (int enum_value = 1; enum_value < disabled_reasons.last_reason;
           enum_value <<= 1) {
        if (reasons & enum_value) {
          base::StringAppendF(&app, "<disabled reason=\"%d\"/>", enum_value);
          reasons ^= enum_value;
        }
      }
      if (reasons) {
        // There are still other disabled reasons that we don't know yet.
        base::StringAppendF(&app, "<disabled/>");
      }
    }
