...
TEST_F(ComponentUnpackerTest, VerifyAPKSignature) {
  ...
  ScopedJavaLocalRef<jobject> signed1_apk(
      env, env->NewObject(raf_class.obj(), raf_constructor,
                          ConvertUTF8ToJavaString(
                              env, apk_test_file("signed-1.apk").MaybeAsASCII())
                              .obj(),
                          perm_string.obj()));
  // Signed with #2.
  ScopedJavaLocalRef<jobject> signed2_apk(
      env, env->NewObject(raf_class.obj(), raf_constructor,
                          ConvertUTF8ToJavaString(
  ...
  ScopedJavaLocalRef<jobject> signed_both_apk(
      env,
      env->NewObject(raf_class.obj(), raf_constructor,
                     ConvertUTF8ToJavaString(
                         env, apk_test_file("signed-1-2.apk").MaybeAsASCII())
                         .obj(),
                     perm_string.obj()));

  ScopedJavaLocalRef<jobject> unsigned_apk(
      env, env->NewObject(raf_class.obj(), raf_constructor,
                          ConvertUTF8ToJavaString(
