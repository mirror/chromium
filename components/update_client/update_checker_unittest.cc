...
TEST_F(UpdateCheckerTest, ComponentDisabled) {
  ...
  crx_component.disabled_reasons = DisabledReasons(256, 0);
  update_checker_ = UpdateChecker::Create(config_, metadata_.get());
  update_checker_->CheckForUpdates(
      std::vector<std::string>{kUpdateItemId}, components, "", false,
      base::Bind(&UpdateCheckerTest::UpdateCheckComplete,
  ...
  crx_component.disabled_reasons = DisabledReasons(256, 256);
  update_checker_->CheckForUpdates(
      std::vector<std::string>{kUpdateItemId}, components, "", false,
      base::Bind(&UpdateCheckerTest::UpdateCheckComplete,
                 base::Unretained(this)));
  ...
  EXPECT_NE(string::npos,
            post_interceptor_->GetRequests()[2].find("<disabled/>"));

  crx_component.disabled_reasons = DisabledReasons(256, 1);
  update_checker_ = UpdateChecker::Create(config_, metadata_.get());
  update_checker_->CheckForUpdates(
      std::vector<std::string>{kUpdateItemId}, components, "", false,
      base::Bind(&UpdateCheckerTest::UpdateCheckComplete,
  ...
  crx_component.disabled_reasons = DisabledReasons(256, 28);  // 28 = 4 + 8 + 16
  update_checker_ = UpdateChecker::Create(config_, metadata_.get());
  update_checker_->CheckForUpdates(
      std::vector<std::string>{kUpdateItemId}, components, "", false,
      base::Bind(&UpdateCheckerTest::UpdateCheckComplete,
  ...
  // 540 = 4 + 8 + 16 + 512
  crx_component.disabled_reasons = DisabledReasons(256, 540);
  update_checker_ = UpdateChecker::Create(config_, metadata_.get());
  update_checker_->CheckForUpdates(
      std::vector<std::string>{kUpdateItemId}, components, "", false,
      base::Bind(&UpdateCheckerTest::UpdateCheckComplete,
  ...
  EXPECT_NE(string::npos, post_interceptor_->GetRequests()[5].find(
                              "<disabled reason=\"16\"/>"));
  EXPECT_NE(string::npos,
            post_interceptor_->GetRequests()[5].find("<disabled/>"));
}
