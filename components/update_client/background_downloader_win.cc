...
bool JobCreationOlderThanDaysPredicate(int num_days,
...
bool JobFileUrlEqualPredicate(const ComPtr<IBackgroundCopyJob>& job,
                              const base::string16& remote_name) {
  std::vector<ComPtr<IBackgroundCopyFile>> files;
  HRESULT hr = GetFilesInJob(job, &files);
  if (FAILED(hr))
    return false;
...
HRESULT CleanupStaleJobs(const ComPtr<IBackgroundCopyManager>& bits_manager) {
  ...
  std::vector<ComPtr<IBackgroundCopyJob>> jobs;
  /*
  HRESULT hr = FindBitsJobIf([](const ComPtr<IBackgroundCopyJob>& job) { return
  JobCreationOlderThanDaysPredicate(job, kPurgeStaleJobsAfterDays); },
                             bits_manager, &jobs);
  */

  auto predicate = std::bind1st(std::ptr_fun(JobCreationOlderThanDaysPredicate),
                                kPurgeStaleJobsAfterDays);
  HRESULT hr = FindBitsJobIf(predicate, bits_manager, &jobs);
...
HRESULT BackgroundDownloader::CreateOrOpenJob(const GURL& url,
  ...
  HRESULT hr = FindBitsJobIf(
      [&url](const ComPtr<IBackgroundCopyJob>& job) {
        return JobFileUrlEqualPredicate(job, base::SysUTF8ToWide(url.spec()));
      },
      bits_manager_, &jobs);
