...
int64_t ConvertSourceId(int64_t source_id, int64_t instance_id) {
  ...
  // Neither ID should have high bits set, since they are from 32 bit counters.
  DCHECK_EQ(source_id, source_id & low_bits);
  DCHECK_EQ(instance_id, instance_id & low_bits);
  return (instance_id << 32) | source_id;
}
