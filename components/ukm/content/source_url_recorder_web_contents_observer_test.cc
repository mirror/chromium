...
TEST_F(SourceUrlRecorderWebContentsObserverTest, InvalidScheme) {
  NavigationSimulator::NavigateAndCommitFromBrowser(web_contents(),
                                                    GURL("about:blank"));
  EXPECT_EQ(0ul, test_ukm_recorder_.sources_count());
}

TEST_F(SourceUrlRecorderWebContentsObserverTest, Subframe) {
...
TEST_F(SourceUrlRecorderWebContentsObserverTest, Download) {
  // TODO(bmcquade): figure out how to write a unittest that simulates download
  // behavior.
}
