...
void DebugPage::StartDataRequest(
  ...
  if (ukm_service) {
    ...
    for (const auto& kv : ukm_service->sources_) {
      source_data[kv.first] = SourceData{kv.second.get()};
    }

    for (const auto& v : ukm_service->entries_) {
      SourceId sid = v.get()->source_id;
      if (!source_data.count(sid))
        source_data[sid] = SourceData{};
      source_data[sid].entries.push_back(v.get());
    }
