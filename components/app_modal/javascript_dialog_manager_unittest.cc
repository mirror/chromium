...
TEST(JavaScriptDialogManagerTest, GetTitle) {
  struct Case {
    ...
    const char* expected_android;
      ...
      // filesystem:
      // - main frame:
      {"filesystem:http://foo.com/temporary/foo/bar.html",
       "filesystem:http://foo.com/temporary/foo/bar.html", "foo.com Says",
       "foo.com says", "foo.com says"},
      // - subframe:
      {"http://bar.com/", "filesystem:http://foo.com/temporary/foo/bar.html",
       "An Embedded Page at foo.com Says", "An embedded page at foo.com says",
       "An embedded page at foo.com says"},
  };
