...
class MockLanguageModel : public language::LanguageModel {
 public:
  MockLanguageModel(const std::vector<LanguageDetails>& details)
      : details_(details) {}
  ...
  std::vector<LanguageDetails> details_;
};
...
// is supported.
TEST_F(TranslateManagerTest, GetTargetLanguage) {
  // Try with a single, supported language.
  ASSERT_TRUE(TranslateDownloadManager::IsSupportedLanguage("en"));
  mock_language_model_.details_ = {
      MockLanguageModel::LanguageDetails("en", 1.0)};
  EXPECT_EQ("en", TranslateManager::GetTargetLanguage(&mock_language_model_));
  ...
  EXPECT_EQ("de", TranslateManager::GetTargetLanguage(&mock_language_model_));
  ...
  EXPECT_EQ("", TranslateManager::GetTargetLanguage(&mock_language_model_));
  ...
  EXPECT_EQ("iw", TranslateManager::GetTargetLanguage(&mock_language_model_));
  ...
  EXPECT_EQ("tl", TranslateManager::GetTargetLanguage(&mock_language_model_));
  ...
  EXPECT_EQ("no", TranslateManager::GetTargetLanguage(&mock_language_model_));
}
