...
TEST_F(TranslatePrefsTest, GetLanguageInfoListCorrectLocale) {
  ...
  translate_prefs_->GetLanguageInfoList("en-US", &language_list);
  ...
  codes.clear();
  l10n_util::GetAcceptLanguagesForLocale("jp", &expected_codes);
  translate_prefs_->GetLanguageInfoList("jp", &language_list);
  ...
  translate_prefs_->GetLanguageInfoList("es-AR", &language_list);
  codes = ExtractLanguageCodes(language_list);
  EXPECT_THAT(codes, UnorderedElementsAreArray(expected_codes));
}
...
TEST_F(TranslatePrefsTest, GetLanguageInfoListOutput) {
  ...
  translate_prefs_->GetLanguageInfoList("", &language_list);
  EXPECT_TRUE(language_list.empty());

  // Output is sorted.
  language_list.clear();
  translate_prefs_->GetLanguageInfoList("en-US", &language_list);
  std::vector<std::string> display_names = ExtractDisplayNames(language_list);
...
TEST_F(TranslatePrefsTest, GetLanguageInfoListSampleLanguages) {
  ...
  language_list.clear();
  translate_prefs_->GetLanguageInfoList("it", &language_list);
