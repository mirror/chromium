...
TEST_F(TranslatePrefsTest, GetLanguageInfoListSampleLanguages) {
  ...
  EXPECT_FALSE(language.display_name_rtl);
  ...
  EXPECT_EQ("Chinese (Hong Kong)", language.display_name);
  EXPECT_EQ("中文（香港）", language.native_display_name);
  EXPECT_FALSE(language.display_name_rtl);
  EXPECT_FALSE(language.supports_translate);
  ...
  //-----------------------------------
  // Test with Arabic locale for RTL display names.
  language_list.clear();
  TranslatePrefs::GetLanguageInfoList("ar", &language_list);

  language = GetLanguageByCode("en-US", language_list);
  EXPECT_EQ("en-US", language.code);
  EXPECT_TRUE(language.display_name_rtl);
  EXPECT_FALSE(language.supports_translate);

  language = GetLanguageByCode("ar", language_list);
  EXPECT_EQ("ar", language.code);
  EXPECT_TRUE(language.display_name_rtl);
  EXPECT_TRUE(language.supports_translate);

  language = GetLanguageByCode("ja", language_list);
  EXPECT_EQ("ja", language.code);
  EXPECT_TRUE(language.display_name_rtl);
  EXPECT_TRUE(language.supports_translate);

  language = GetLanguageByCode("fr", language_list);
  EXPECT_EQ("fr", language.code);
  EXPECT_TRUE(language.display_name_rtl);
  EXPECT_TRUE(language.supports_translate);
  ...
  language = GetLanguageByCode("de-CH", language_list);
  EXPECT_EQ("de-CH", language.code);
  EXPECT_TRUE(language.display_name_rtl);
  EXPECT_FALSE(language.supports_translate);
}
