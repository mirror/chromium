 ...
 protected:
  LogoTrackerTest()
    ...
    logo_tracker_ = base::MakeUnique<LogoTracker>(
        base::FilePath(), task_environment_.GetMainThreadTaskRunner(),
  ...
  void TearDown() override {
    // |logo_tracker_| owns |logo_cache_|, which gets destroyed on the file
    // thread after |logo_tracker_|s destruction. Ensure that |logo_cache_| is
...
TEST_F(LogoTrackerTest, ValidateCachedLogo) {
  ...
  EXPECT_TRUE(logo_cache_->GetCachedLogoMetadata() != nullptr);
