...
std::unique_ptr<EncodedLogo> GetLogoFromCacheOnFileThread(LogoCache* logo_cache,
  ...
  std::unique_ptr<EncodedLogo> logo = logo_cache->GetCachedLogo();
  if (logo) {
    DCHECK(logo->encoded_image);
  }
  return logo;
}
...
void LogoTracker::OnCachedLogoRead(std::unique_ptr<EncodedLogo> cached_logo) {
  ...
  if (cached_logo) {
    LOG(WARNING) << cached_logo.get();
    LOG(WARNING) << cached_logo->metadata.source_url;
    LOG(WARNING) << cached_logo->encoded_image.get();
    DCHECK(cached_logo->encoded_image);
