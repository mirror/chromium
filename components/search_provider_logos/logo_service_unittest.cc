 ...
 public:
  ...
  void SetCachedLogoInternal(const EncodedLogo* logo) {
    if (logo) {
      ...
      logo_ = std::make_unique<EncodedLogo>(*logo);
      metadata_ = std::make_unique<LogoMetadata>(logo->metadata);
    } else {
      logo_ = nullptr;
      metadata_ = nullptr;
    }
  ...
  std::unique_ptr<EncodedLogo> GetCachedLogo() override {
    OnGetCachedLogo();
    if (logo_) {
      DCHECK(logo_->encoded_image);
      std::unique_ptr<EncodedLogo> logo = std::make_unique<EncodedLogo>(*logo_);
      DCHECK(logo->encoded_image);
      return logo;
    }
    return nullptr;
