...
using MockLogoFunction = MockFunction<LogoCallback::RunType>;
using MockEncodedLogoFunction = MockFunction<EncodedLogoCallback::RunType>;
...
base::Callback<Function> BindMockFunction(MockFunction<Function>* function) {
  return base::Bind(&MockFunction<Function>::Call, base::Unretained(function));
}
...
MATCHER(IsNullopt, "") {
  return arg == base::nullopt;
}

MATCHER_P(NonNullopt, m, "") {
  return Match(arg, Not(IsNullopt()), result_listener) &&
         Match(arg.value(), m, result_listener);
}
...
class LogoServiceTest : public ::testing::Test {
 protected:
  ...
  void GetLogo(MockLogoFunction* on_cached_logo_available,
               MockLogoFunction* on_fresh_logo_available);
  void GetEncodedLogo(MockEncodedLogoFunction* on_cached_logo_available,
                      MockEncodedLogoFunction* on_fresh_logo_available);
  void GetLogoWithCallbacks(LogoCallback on_cached_logo_available,
                            LogoCallback on_fresh_logo_available);
  void GetEncodedLogoWithCallbacks(EncodedLogoCallback on_cached_logo_available,
...
void LogoServiceTest::GetLogo(MockLogoFunction* on_cached_logo_available,
                              MockLogoFunction* on_fresh_logo_available) {
  GetLogoWithCallbacks(BindMockFunction(on_cached_logo_available),
                       BindMockFunction(on_fresh_logo_available));
}

void LogoServiceTest::GetEncodedLogo(
    MockEncodedLogoFunction* on_cached_logo_available,
    MockEncodedLogoFunction* on_fresh_logo_available) {
  GetEncodedLogoWithCallbacks(BindMockFunction(on_cached_logo_available),
                              BindMockFunction(on_fresh_logo_available));
}

void LogoServiceTest::GetLogoWithCallbacks(
    LogoCallback on_cached_logo_available,
    LogoCallback on_fresh_logo_available) {
...
void LogoServiceTest::GetEncodedLogoWithCallbacks(
...
TEST_F(LogoServiceTest, DownloadAndCacheLogo) {
  StrictMock<MockLogoFunction> cached;
  StrictMock<MockLogoFunction> fresh;
  ...
  EXPECT_CALL(cached, Call(IsNullopt()));
  EXPECT_CALL(fresh, Call(NonNullopt(logo)));
  GetLogo(&cached, &fresh);
}

TEST_F(LogoServiceTest, DownloadAndCacheEncodedLogo) {
  StrictMock<MockEncodedLogoFunction> cached;
  StrictMock<MockEncodedLogoFunction> fresh;
  Logo logo = GetSampleLogo(DoodleURL(), test_clock_->Now());
  EncodedLogo encoded_logo = EncodeLogo(logo);
  SetServerResponse(ServerResponse(logo));
  logo_cache_->ExpectSetCachedLogo(&logo);
  EXPECT_CALL(cached, Call(IsNullopt()));
  EXPECT_CALL(fresh, Call(NonNullopt(encoded_logo)));
  GetEncodedLogo(&cached, &fresh);
}
...
TEST_F(LogoServiceTest, EmptyCacheAndFailedDownload) {
...

  {
    ...
    EXPECT_CALL(cached, Call(IsNullopt()));
    EXPECT_CALL(fresh, Call(_)).Times(0);
    GetLogo(&cached, &fresh);
  }
...
TEST_F(LogoServiceTest, ReturnCachedLogo) {
  ...
  StrictMock<MockLogoFunction> cached;
  StrictMock<MockLogoFunction> fresh;
  EXPECT_CALL(cached, Call(NonNullopt(cached_logo)));
  EXPECT_CALL(fresh, Call(_)).Times(0);
...
TEST_F(LogoServiceTest, UpdateCachedLogoMetadata) {
  ...
  // metadata.
  {
    ...
    StrictMock<MockLogoFunction> cached;
    StrictMock<MockLogoFunction> fresh;
    EXPECT_CALL(cached, Call(NonNullopt(expected_logo)));
    EXPECT_CALL(fresh, Call(_)).Times(0);
    GetLogo(&cached, &fresh);
  }
}
...
TEST_F(LogoServiceTest, UpdateCachedLogo) {
  ...
  StrictMock<MockLogoFunction> fresh;
  EXPECT_CALL(cached, Call(NonNullopt(cached_logo)));
  EXPECT_CALL(fresh, Call(NonNullopt(fresh_logo)));
  GetLogo(&cached, &fresh);
}
...
TEST_F(LogoServiceTest, InvalidateCachedLogo) {
  ...
  StrictMock<MockLogoFunction> fresh;
  EXPECT_CALL(cached, Call(NonNullopt(cached_logo)));
  EXPECT_CALL(fresh, Call(IsNullopt()));
...
TEST_F(LogoServiceTest, LogoWithTTLCannotBeShownAfterExpiration) {
  ...
  SetServerResponse(ServerResponse(logo));
  GetLogoWithCallbacks(LogoCallback(), LogoCallback());
...
void EnqueueObservers(
    ...
    const std::vector<std::unique_ptr<LogoCallback>>& cached_callbacks,
    const std::vector<std::unique_ptr<LogoCallback>>& fresh_callbacks,
    size_t start_index) {
  ...
  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE, base::Bind(&EnqueueObservers, logo_service,
                            base::ConstRef(cached_callbacks),
                            base::ConstRef(fresh_callbacks), start_index + 1));
}
...
TEST_F(LogoServiceTest, MAYBE_SupportOverlappingLogoRequests) {
  ...
  std::vector<std::unique_ptr<MockLogoFunction>> mocks;
  std::vector<std::unique_ptr<LogoCallback>> cached_callbacks;
  std::vector<std::unique_ptr<LogoCallback>> fresh_callbacks;
  for (int i = 0; i < kNumListeners; ++i) {
    mocks.push_back(std::make_unique<MockLogoFunction>());
    EXPECT_CALL(*mocks.back(), Call(NonNullopt(cached_logo)));
    cached_callbacks.push_back(
        std::make_unique<LogoCallback>(BindMockFunction(mocks.back().get())));

    mocks.push_back(std::make_unique<MockLogoFunction>());
    EXPECT_CALL(*mocks.back(), Call(NonNullopt(fresh_logo)));
    fresh_callbacks.push_back(
        std::make_unique<LogoCallback>(BindMockFunction(mocks.back().get())));
  }
...
TEST_F(LogoServiceTest, DeleteObserversWhenLogoURLChanged) {
  StrictMock<MockLogoFunction> first_cached;
  StrictMock<MockLogoFunction> first_fresh;
  EXPECT_CALL(first_cached, Call(_)).Times(0);
  EXPECT_CALL(first_fresh, Call(_)).Times(0);
  logo_service_->GetLogo(BindMockFunction(&first_cached),
                         BindMockFunction(&first_fresh));
  ...
  StrictMock<MockLogoFunction> second_cached;
  StrictMock<MockLogoFunction> second_fresh;
  EXPECT_CALL(second_cached, Call(IsNullopt()));
  EXPECT_CALL(second_fresh, Call(NonNullopt(logo)));
  logo_service_->GetLogo(BindMockFunction(&second_cached),
                         BindMockFunction(&second_fresh));
