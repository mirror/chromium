...
TEST_F(LogoServiceTest, DownloadAndCacheLogo) {
  ...
  EXPECT_CALL(cached, Run(IsNullopt()));
  EXPECT_CALL(fresh, Run(NonNullopt(logo)));
...
TEST_F(LogoServiceTest, DownloadAndCacheEncodedLogo) {
  ...
  logo_cache_->ExpectSetCachedLogo(&logo);
  EXPECT_CALL(cached, Run(IsNullopt()));
  EXPECT_CALL(fresh, Run(NonNullopt(encoded_logo)));
...
TEST_F(LogoServiceTest, ReturnCachedLogo) {
  ...
  StrictMock<MockLogoCallback> cached;
  StrictMock<MockLogoCallback> fresh;
  EXPECT_CALL(cached, Run(NonNullopt(cached_logo)));
  EXPECT_CALL(fresh, Run(_)).Times(0);
...
TEST_F(LogoServiceTest, UpdateCachedLogoMetadata) {
  ...
  // metadata.
  {
    ...
    StrictMock<MockLogoCallback> cached;
    StrictMock<MockLogoCallback> fresh;
    EXPECT_CALL(cached, Run(NonNullopt(expected_logo)));
    EXPECT_CALL(fresh, Run(_)).Times(0);
    GetLogo(cached.Get(), fresh.Get());
  }
}
...
TEST_F(LogoServiceTest, UpdateCachedLogo) {
  ...
  StrictMock<MockLogoCallback> fresh;
  EXPECT_CALL(cached, Run(NonNullopt(cached_logo)));
  EXPECT_CALL(fresh, Run(NonNullopt(fresh_logo)));
  GetLogo(cached.Get(), fresh.Get());
}
...
TEST_F(LogoServiceTest, InvalidateCachedLogo) {
  ...
  StrictMock<MockLogoCallback> fresh;
  EXPECT_CALL(cached, Run(NonNullopt(cached_logo)));
  EXPECT_CALL(fresh, Run(IsNullopt()));
...
TEST_F(LogoServiceTest, MAYBE_SupportOverlappingLogoRequests) {
  ...
  for (int i = 0; i < kNumListeners; ++i) {
    ...
    EXPECT_CALL(*mocks.back(), Run(NonNullopt(cached_logo)));
    cached_callbacks.push_back(
        std::make_unique<LogoCallback>(mocks.back()->Get()));

    mocks.push_back(std::make_unique<MockLogoCallback>());
    EXPECT_CALL(*mocks.back(), Run(NonNullopt(fresh_logo)));
...
TEST_F(LogoServiceTest, DeleteObserversWhenLogoURLChanged) {
  ...
  EXPECT_CALL(second_cached, Run(IsNullopt()));
  EXPECT_CALL(second_fresh, Run(NonNullopt(logo)));
