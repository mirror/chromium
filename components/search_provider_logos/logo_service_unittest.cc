...
class LogoServiceTest : public ::testing::Test {
 protected:
  LogoServiceTest()
    ...
    search_url.SetKeyword(base::ASCIIToUTF16("ex"));
    search_url.SetShortName(base::ASCIIToUTF16("Logo Example"));
    search_url.SetURL("https://example.com/?q={searchTerms}");
    search_url.doodle_url = GURL("https://example.com/logo.png");
    template_url_service_.SetUserSelectedDefaultSearchProvider(
        template_url_service_.Add(base::MakeUnique<TemplateURL>(search_url)));

    search_url.SetKeyword(base::ASCIIToUTF16("cr"));
    search_url.SetShortName(base::ASCIIToUTF16("Chromium"));
    search_url.SetURL("https://www.chromium.org/?q={searchTerms}");
    search_url.doodle_url = GURL("https://chromium.org/logo.png");
    template_url_service_.Add(base::MakeUnique<TemplateURL>(search_url));

    search_url.SetKeyword(base::ASCIIToUTF16("nologo"));
    search_url.SetShortName(base::ASCIIToUTF16("Search Without Logo"));
    search_url.SetURL("https://nologo.com/?q={searchTerms}");
    search_url.doodle_url = GURL();
    template_url_service_.Add(base::MakeUnique<TemplateURL>(search_url));
...
TEST_F(LogoServiceTest, DeleteObserversWhenLogoURLChanged) {
  ...
  template_url_service_.SetUserSelectedDefaultSearchProvider(
      template_url_service_.GetTemplateURLForKeyword(base::ASCIIToUTF16("cr")));
  Logo logo = GetSampleLogo(DoodleURL(), test_clock_->Now());
  SetServerResponse(ServerResponse(logo));
