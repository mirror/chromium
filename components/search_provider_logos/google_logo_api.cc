...
namespace {
const std::string kParamName = "async";
...
GURL AppendQueryparam(const GURL& url, const std::string& value) {
  bool replaced = false;
...
GURL AppendFingerprintParamToDoodleURL(const GURL& logo_url,
  ...
  return AppendQueryparam(logo_url, "es_dfp:" + fingerprint);
}
...
GURL AppendPreliminaryParamsToDoodleURL(bool gray_background,
  ...
  return AppendQueryparam(logo_url, api_params);
}
