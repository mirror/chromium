...
TEST(GoogleNewLogoApiTest, ParsesCapturedApiResult) {
  ...
  ASSERT_TRUE(PathService::Get(base::DIR_SOURCE_ROOT, &test_data));
  test_data = test_data.AppendASCII("chrome/test/data/search_provider_logos");
  ...
  for (const TestCase& test_case : test_cases) {
    std::string json;
    ASSERT_TRUE(
        base::ReadFileToString(test_data.AppendASCII(test_case.file), &json));
