...
bool operator==(const LanguageHistogram::LanguageInfo& lhs,
                const LanguageHistogram::LanguageInfo& rhs) {
  return lhs.language_code == rhs.language_code;
}

TEST(LanguageHistogramTest, ListSorted) {
  TestingPrefServiceSimple prefs;
  LanguageHistogram::RegisterProfilePrefs(prefs.registry());
  LanguageHistogram hist(&prefs);
  ...
  EXPECT_THAT(hist.GetTopLanguages(),
              ElementsAre(LanguageHistogram::LanguageInfo(kLang1, 0.0f),
                          LanguageHistogram::LanguageInfo(kLang2, 0.0f)));
}

TEST(LanguageHistogramTest, ListSortedReversed) {
  TestingPrefServiceSimple prefs;
  LanguageHistogram::RegisterProfilePrefs(prefs.registry());
  LanguageHistogram hist(&prefs);
...
TEST(LanguageHistogramTest, RightFrequencies) {
  TestingPrefServiceSimple prefs;
  LanguageHistogram::RegisterProfilePrefs(prefs.registry());
  LanguageHistogram hist(&prefs);
...
TEST(LanguageHistogramTest, RareLanguageDiscarded) {
  TestingPrefServiceSimple prefs;
  LanguageHistogram::RegisterProfilePrefs(prefs.registry());
  LanguageHistogram hist(&prefs);
  ...
  EXPECT_THAT(hist.GetTopLanguages(),
              ElementsAre(LanguageHistogram::LanguageInfo{kLang1, 1}));
}

TEST(LanguageHistogramTest, ShouldClearHistoryIfAllTimes) {
  TestingPrefServiceSimple prefs;
  LanguageHistogram::RegisterProfilePrefs(prefs.registry());
  LanguageHistogram hist(&prefs);
...
TEST(LanguageHistogramTest, ShouldNotClearHistoryIfNotAllTimes) {
  TestingPrefServiceSimple prefs;
  LanguageHistogram::RegisterProfilePrefs(prefs.registry());
  LanguageHistogram hist(&prefs);
