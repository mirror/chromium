...
// TODO(martis): Move LanguageDetectionDetails to the language component.
namespace translate {
...
// Common functionality for classes offering language detection. Maintains a list
// of listeners and updates them when a page's language has been determined.
//
...
class LanguageDetector {
 public:
  class Observer {
   public:
    virtual void OnLanguageDetected(const translate::LanguageDetectionDetails& details) = 0;
  };
  ...
  void NotifyLanguageDetectionObservers(const translate::LanguageDetectionDetails& details);
