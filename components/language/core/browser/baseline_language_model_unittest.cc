...
TEST_F(BaselineLanguageModelTest, UiOnly) {
  ...
  EXPECT_THAT(model.GetLanguages(), ElementsAre(EqualsLd(Ld("en", 1.0))));
}
...
TEST_F(BaselineLanguageModelTest, UiAndHist) {
  ...
  EXPECT_THAT(model.GetLanguages(),
              ElementsAre(EqualsLd(Ld("en", 1.0)), EqualsLd(Ld("it", 1.0 / 2)),
                          EqualsLd(Ld("de", 1.0 / 3))));
}
...
TEST_F(BaselineLanguageModelTest, UiAndAccept) {
  ...
  BaselineLanguageModel model(&prefs_, "en", kAcceptPref);
  EXPECT_THAT(model.GetLanguages(),
              ElementsAre(EqualsLd(Ld("en", 1.0)), EqualsLd(Ld("ja", 1.0 / 2)),
                          EqualsLd(Ld("fr", 1.0 / 3))));
}
...
TEST_F(BaselineLanguageModelTest, UiAndHistAndAccept) {
  ...
  EXPECT_THAT(
      model.GetLanguages(),
      ElementsAre(EqualsLd(Ld("en", 1.0)), EqualsLd(Ld("it", 1.0 / 2)),
                  EqualsLd(Ld("de", 1.0 / 3)), EqualsLd(Ld("ja", 1.0 / 4)),
                  EqualsLd(Ld("fr", 1.0 / 5))));
}
...
TEST_F(BaselineLanguageModelTest, Regions) {
  ...
  EXPECT_THAT(
      model.GetLanguages(),
      ElementsAre(EqualsLd(Ld("en", 1.0)), EqualsLd(Ld("en-AU", 1.0 / 2)),
                  EqualsLd(Ld("en-CA", 1.0 / 3)),
                  EqualsLd(Ld("en-GB", 1.0 / 4))));
}
