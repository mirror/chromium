...
TEST(ScoresToDetailsTest, MultipleEntries) {
  ...
  EXPECT_THAT(
      ScoresToDetails({{"it", 0.5}, {"en", 0.5}}),
      UnorderedElementsAre(EqualsLd(Ld("en", 1.0)), EqualsLd(Ld("it", 1.0))));

  EXPECT_THAT(
      ScoresToDetails({{"en", 0.0}, {"it", 0.0}}),
      UnorderedElementsAre(EqualsLd(Ld("en", 0.0)), EqualsLd(Ld("it", 0.0))));
}
