...
TEST(PromoteRegionScoresTest, NoRegions) {
  ...
  EXPECT_THAT(PromoteRegionScores({{"it", 1.0}}),
              Eq(LangScoreMap({{"it", 1.0}})));
  EXPECT_THAT(PromoteRegionScores({{"en", 1.0}, {"fr", 0.5}, {"it", 0.25}}),
              Eq(LangScoreMap({{"en", 1.0}, {"fr", 0.5}, {"it", 0.25}})));
}

// Test a single region for a single language.
TEST(PromoteRegionScores, SingleRegion) {
  EXPECT_THAT(PromoteRegionScores({{"en-AU", 1.0}}),
              Eq(LangScoreMap({{"en-AU", 1.0}})));
  EXPECT_THAT(PromoteRegionScores({{"en-AU", 1.0}, {"en", 0.5}}),
              Eq(LangScoreMap({{"en-AU", 1.5}})));
}

// Test multiple regions for a single language.
TEST(PromoteRegionScores, MultipleRegions) {
  EXPECT_THAT(PromoteRegionScores({{"en-AU", 1.0}, {"en-US", 0.5}}),
              Eq(LangScoreMap({{"en-AU", 1.0}, {"en-US", 0.5}})));
  EXPECT_THAT(
      PromoteRegionScores({{"en-AU", 1.0}, {"en-US", 0.5}, {"en", 0.25}}),
      Eq(LangScoreMap({{"en-AU", 1.25}, {"en-US", 0.75}})));
}
...
TEST(PromoteRegionScoresTest, MultipleBases) {
  EXPECT_THAT(
      PromoteRegionScores(
          {{"en-AU", 1.0}, {"fr-FR", 0.5}, {"en-US", 0.25}, {"fr-CA", 0.125}}),
      Eq(LangScoreMap({{"en-AU", 1.0},
                       {"fr-FR", 0.5},
                       {"en-US", 0.25},
                       {"fr-CA", 0.125}})));
  ...
  EXPECT_THAT(
      PromoteRegionScores(
          {{"en-AU", 1.0}, {"en-US", 0.5}, {"en", 0.25}, {"fr", 0.125}}),
      Eq(LangScoreMap({{"en-AU", 1.25}, {"en-US", 0.75}, {"fr", 0.125}})));

  EXPECT_THAT(
      PromoteRegionScores({{"fr", 1.0},
                           {"en", 0.9},
                           {"fr-FR", 0.8},
                           {"fr-CA", 0.7},
                           {"en-AU", 0.6},
                           {"en-US", 0.5}}),
      Eq(LangScoreMap(
          {{"fr-FR", 1.8}, {"fr-CA", 1.7}, {"en-AU", 1.5}, {"en-US", 1.4}})));
}
...
TEST(UpdateLanguageScoresTest, EmptyCombination) {
  LangScoreMap out;
  UpdateLanguageScores(0.1, 1.0, {}, &out);
  UpdateLanguageScores(0.1, 1.0, {}, &out);
...
TEST(UpdateLanguageScoresTest, NonEmptyCombination) {
  ...
  UpdateLanguageScores(0.25, 1.00, in1, &out);
  UpdateLanguageScores(0.25, 0.75, in2, &out);

  EXPECT_THAT(
      out,
      Eq(LangScoreMap({{"en", 1.5}, {"it", 1.5}, {"zh", 0.5}, {"fr", 0.25}})));
}
...
TEST(UpdateLanguageScoresTest, MaxPenalty) {
  ...
  UpdateLanguageScores(0.75, 1.0, in1, &out);
  UpdateLanguageScores(0.50, 0.5, in2, &out);

  EXPECT_THAT(
      out,
      Eq(LangScoreMap({{"en", 1.0}, {"it", 0.75}, {"zh", 0.0}, {"fr", 0.0}})));
}
...
TEST(ScoresToDetailsTest, Singleton) {
  EXPECT_THAT(ScoresToDetails({{"en", 1.0}}),
              ElementsAre(EqualsLd(Ld("en", 1.0))));
  EXPECT_THAT(ScoresToDetails({{"en", 0.5}}),
              ElementsAre(EqualsLd(Ld("en", 1.0))));
  EXPECT_THAT(ScoresToDetails({{"en", 0.0}}),
              ElementsAre(EqualsLd(Ld("en", 0.0))));
}

// Test multiple entries.
TEST(ScoresToDetailsTest, MultipleEntries) {
  EXPECT_THAT(ScoresToDetails({{"it", 0.5}, {"zh", 1.0}, {"en", 2.0}}),
              ElementsAre(EqualsLd(Ld("en", 1.0)), EqualsLd(Ld("zh", 0.5)),
                          EqualsLd(Ld("it", 0.25))));

  EXPECT_THAT(ScoresToDetails({{"it", 0.5}, {"en", 0.5}}),
              ElementsAre(EqualsLd(Ld("it", 1.0)), EqualsLd(Ld("en", 1.0))));

  EXPECT_THAT(ScoresToDetails({{"en", 0.0}, {"it", 0.0}}),
              ElementsAre(EqualsLd(Ld("it", 0.0)), EqualsLd(Ld("en", 0.0))));
}
...
std::vector<LanguageModel::LanguageDetails> TestModel(
  ...
  const float weights[] = {2.0, 1.1, 1.0, 0.5};
...
TEST(MakeHeuristicLanguageListTest, Scores) {
  EXPECT_THAT(
      TestModel({{"fr"}, {"en", "en-CA", "en-US"}, {"en"}, {"en-US"}}),
      ElementsAre(EqualsLd(Ld("en-US", 1.0)), EqualsLd(Ld("en-CA", 2.1 / 2.59)),
                  EqualsLd(Ld("fr", 2.0 / 2.59))));
}
...
TEST(GetHistogramLanguagesTest, Empty) {
  ...
  EXPECT_THAT(GetHistogramLanguages(0.0, hist), IsEmpty());
}
...
TEST(GetHistogramLanguagesTest, FrequencyCutoff) {
  ...
  EXPECT_THAT(GetHistogramLanguages(0.0, hist), ElementsAre("en", "it", "zh"));
  EXPECT_THAT(GetHistogramLanguages(1.0 / 3, hist), ElementsAre("en", "it"));
  EXPECT_THAT(GetHistogramLanguages(1.0 / 2, hist), ElementsAre("en"));
}
