...
constexpr float kHistogramFrequencyCutoff = 0.3;

// Input source weights.
constexpr float kUiWeight = 2.0;
constexpr float kAcceptWeight = 1.1;
constexpr float kHistogramWeight = 1.0;
constexpr float kUlpWeight = 0.5;
constexpr float kScoreDelta = 0.01;
...
std::vector<std::string> PromoteRegions(
  ...
  // For each input language, output its region-specializations if they exist,
  // otherwise just the language itself.
...
std::vector<LanguageModel::LanguageDetails> ScoresToDetails(
  ...
  // Sort the details in descending order of score.
  const auto cmp = [](const LanguageModel::LanguageDetails& a,
                      const LanguageModel::LanguageDetails& b) {
    return a.score > b.score;
  };
  std::stable_sort(details.begin(), details.end(), cmp);

  // Normalize the scores to lie in [0, 1].
  const float scale = details[0].score == 0 ? 1.0 : details[0].score;
