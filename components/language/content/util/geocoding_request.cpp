...
bool ParseGeodingResponseHasError(bool http_success,
                                  int status_code,
                                  const std::string& response_body,
                                  std::string* admin_district) {
  // Default unknown admin district.
...
void GeocodingRequest::OnURLFetchComplete(const net::URLFetcher* source) {
  ...
  std::string admin_district;
  bool parse_error = ParseGeodingResponseHasError(
      status.is_success(), response_code, data, &admin_district);
