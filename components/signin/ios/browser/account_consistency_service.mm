...
void AccountConsistencyService::RemoveChromeConnectedCookies(
  ...
  if (last_cookie_update_map_.size() == 0)
    return;
...
void AccountConsistencyService::GoogleSignedOut(const std::string& account_id,
                                                const std::string& username) {
  // Cookies should be removed before Google signed out. The issue is
  // CHROME-CONNECTED cookie should not be part the logout request. Cookie
  // removal will be trigged by GaiaCookieManagerService before the logout
  // request is started.
}
