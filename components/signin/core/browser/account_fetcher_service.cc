...
bool AccountFetcherService::AreAllUserImagesFetched() const {
  return IsAllUserInfoFetched() && user_image_requests_.empty();
}
...
void AccountFetcherService::OnRefreshTokenRevoked(
  ...
  if (!network_fetches_enabled_)
    return;
  user_info_requests_.erase(account_id);
  user_image_requests_.erase(account_id);
#if defined(OS_ANDROID)
  UpdateChildInfo();
#endif
