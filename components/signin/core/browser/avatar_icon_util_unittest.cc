...
TEST(AvatarIconUtilTest, GetImageURLWithOptionsNoInitialSize) {
  GURL in("http://example.com/-A/AAAAAAAAAAI/AAAAAAAAACQ/B/photo.jpg");
  GURL result =
      signin::GetImageURLWithOptions(in, 128, false /* no_silhouette */);
  GURL expected(
      "http://example.com/-A/AAAAAAAAAAI/AAAAAAAAACQ/B/s128-c/photo.jpg");
  EXPECT_EQ(result, expected);
}

TEST(AvatarIconUtilTest, GetImageURLWithOptionsSizeAlreadySpecified) {
...
TEST(AvatarIconUtilTest, GetImageURLWithOptionsOtherSizeSpecified) {
  // If there's already a size specified in the URL, it should be changed to the
  // specified size in the resulting URL.
  GURL in("http://example.com/-A/AAAAAAAAAAI/AAAAAAAAACQ/B/s128-c/photo.jpg");
  GURL result =
      signin::GetImageURLWithOptions(in, 64, false /* no_silhouette */);
  GURL expected(
      "http://example.com/-A/AAAAAAAAAAI/AAAAAAAAACQ/B/s64-c/photo.jpg");
  EXPECT_EQ(result, expected);
}

TEST(AvatarIconUtilTest, GetImageURLWithOptionsSameSize) {
...
TEST(AvatarIconUtilTest, GetImageURLWithOptionsNoFileNameInPath) {
...
TEST(AvatarIconUtilTest, GetImageURLWithOptionsNoSilhouette) {
  // If there's already a size specified in the URL, it should be changed to the
  // specified size in the resulting URL.
  GURL in("http://example.com/-A/AAAAAAAAAAI/AAAAAAAAACQ/B/photo.jpg");
  GURL result =
      signin::GetImageURLWithOptions(in, 64, true /* no_silhouette */);
  GURL expected(
      "http://example.com/-A/AAAAAAAAAAI/AAAAAAAAACQ/B/s64-c-ns/photo.jpg");
  EXPECT_EQ(result, expected);
}

TEST(AvatarIconUtilTest, GetImageURLWithOptionsSizeReplaceNoSilhouette) {
...
TEST(AvatarIconUtilTest, GetImageURLWithOptionsUnknownShouldBePreserved) {
