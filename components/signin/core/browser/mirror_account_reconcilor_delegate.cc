...
MirrorAccountReconcilorDelegate::MirrorAccountReconcilorDelegate(
    ...
    : reconcilor_(reconcilor), signin_manager_(signin_manager) {
  DCHECK(reconcilor_);
...
bool MirrorAccountReconcilorDelegate::IsEnabled() const {
...
bool MirrorAccountReconcilorDelegate::AbortReconcileIfPrimaryHasError() const {
  return true;
}
...
void MirrorAccountReconcilorDelegate::GoogleSigninSucceeded(
  ...
  reconcilor_->Initialize(false);
}

void MirrorAccountReconcilorDelegate::GoogleSignedOut(
    const std::string& account_id,
    const std::string& username) {
  reconcilor_->Stop(true);
  reconcilor_->PerformLogoutAllAccountsAction();
}
