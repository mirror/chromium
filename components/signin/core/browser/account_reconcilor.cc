...
void AccountReconcilor::SetDelegate(
...
void AccountReconcilor::Stop(bool abort_current_reconcile) {
  if (abort_current_reconcile)
    AbortReconcile();
...
void AccountReconcilor::StartReconcile() {
  ...
  if (!delegate_->IsEnabled() || !client_->AreSigninCookiesAllowed()) {
  ...
  if (!ValidateAccountsFromTokenService() &&
      delegate_->AbortReconcileIfPrimaryHasError()) {
...
void AccountReconcilor::FinishReconcile() {
  ...
  std::string first_account = delegate_->GetFirstGaiaAccountForReconcile(
      chrome_accounts_, gaia_accounts_, primary_account_, first_execution_);
  ...
  if (first_account.empty()) {
    DCHECK(!delegate_->AbortReconcileIfPrimaryHasError());
