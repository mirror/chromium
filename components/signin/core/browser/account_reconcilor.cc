...
void AccountReconcilor::StartReconcile() {
  ...
  if (cookie_manager_service_->ListAccounts(&gaia_accounts_,
                                            &signed_out_accounts,
                                            kSource)) {
    VLOG(1) << "AccountReconcilor::StartReconcile: Synchronous listaccounts";
...
void AccountReconcilor::ValidateAccountsFromTokenService() {
  ...
  // TODO: Dice: do not abort reconcile on primary account error.
...
void AccountReconcilor::FinishReconcile() {
  ...
  bool do_full_logout = first_account.empty();
  // |first_account| must be in |chrome_accounts_|.
  DCHECK(do_full_logout ||
  ...
  if (do_full_logout) {
    ...
    DCHECK((primary_account_mismatch && rebuild_cookie) ||
           (number_gaia_accounts == 0));
    // Clear all Chrome tokens, except the Sync account.
    for (const std::string& account : chrome_accounts_) {
      if (account != primary_account_) {
        VLOG(1) << "AccountReconcilor::FinishReconcile: Revoking token for "
                << account;
