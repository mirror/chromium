...
void AccountReconcilor::OnBlockReconcile() {
  ...
  if (is_reconcile_started_) {
    VLOG(1) << "AccountReconcilor::OnBlockReconcile: abort existing reconcile.";
    AbortReconcile();
    reconcile_on_unblock_ = true;
  }
  for (auto& observer : observer_list_)
    observer.OnReconcilorBlocked();
}

void AccountReconcilor::OnUnblockReconcile() {
  DCHECK(!IsReconcileBlocked());
  for (auto& observer : observer_list_)
    observer.OnReconcilorUnblocked();
  if (reconcile_on_unblock_) {
    VLOG(1) << "AccountReconcilor::OnUnblockReconcile: resume reconcile.";
    reconcile_on_unblock_ = false;
    StartReconcile();
  }
}
