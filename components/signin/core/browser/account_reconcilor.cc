...
void AccountReconcilor::OnGaiaAccountsInCookieUpdated(
  ...
  if (error.state() == GoogleServiceAuthError::NONE) {
    ...
    std::vector<std::string> filtered_chrome_accounts;
    {
      std::vector<std::string> chrome_accounts =
          LoadValidAccountsFromTokenService(primary_account_);
      for (std::string& account : chrome_accounts) {
        if (delegate_->ShouldReconcileAccount(account, verified_gaia_accounts,
                                              primary_account_)) {
          filtered_chrome_accounts.emplace_back(std::move(account));
        }
      }
    }

    FinishReconcile(filtered_chrome_accounts,
...
std::vector<std::string> AccountReconcilor::LoadValidAccountsFromTokenService(
    const std::string& primary_account) const {
