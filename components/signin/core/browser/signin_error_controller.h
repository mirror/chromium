...
class SigninErrorController : public KeyedService {
 public:
  enum class AccountMode {
    ...
    // Only errors on the main account are reported. Other accounts are ignored.
    MAIN_ACCOUNT
  };
  ...
  explicit SigninErrorController(AccountMode mode = AccountMode::ANY_ACCOUNT);
  ...
  // Sets the main account id. Only used in the MAIN_ACCOUNT account mode.
  void SetMainAccountID(const std::string& main_account_id);
 ...
 private:
  ...
  // The main account ID. Only used in the MAIN_ACCOUNT account mode.
  std::string main_account_id_;
