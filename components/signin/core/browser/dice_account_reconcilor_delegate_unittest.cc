...
TEST(DiceAccountReconcilorDelegateTest, RevokeTokens) {
...

  {
    ...
    EXPECT_TRUE(delegate.ShouldReconcileAccount(
        "main", std::vector<gaia::ListedAccount>(), "main"));
    EXPECT_TRUE(delegate.ShouldReconcileAccount(
        "secondary", std::vector<gaia::ListedAccount>(), "main"));
  ...
  }
  {
    ...
    EXPECT_TRUE(delegate.ShouldReconcileAccount(
        "main", std::vector<gaia::ListedAccount>{gaia_account}, "main"));
    EXPECT_TRUE(delegate.ShouldReconcileAccount(
        "secondary", std::vector<gaia::ListedAccount>{gaia_account}, "main"));
    EXPECT_TRUE(token_service.RefreshTokenIsAvailable("main"));
    EXPECT_TRUE(token_service.RefreshTokenIsAvailable("secondary"));
    ...
    // Revoke the secondary account.
    EXPECT_FALSE(delegate.ShouldReconcileAccount(
        "secondary", std::vector<gaia::ListedAccount>(), "main"));
    EXPECT_FALSE(token_service.RefreshTokenIsAvailable("secondary"));

    // No primary account, revoke everything.
    EXPECT_FALSE(delegate.ShouldReconcileAccount(
        "main", std::vector<gaia::ListedAccount>(), ""));
