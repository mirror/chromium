...
DiceResponseParams DiceHeaderHelper::BuildDiceSigninResponseParams(
  ...
  if ((params.gaia_id.size() != 1u) || (params.email.size() != 1u) ||
      (params.session_index.size() != 1u)) {
    DLOG(WARNING) << "Invalid parameter count for Dice SIGNIN header: "
                  << header_value;
    params.user_intention = DiceAction::NONE;
    return params;
  }

  if (params.authorization_code.empty()) {
...
DiceResponseParams DiceHeaderHelper::BuildDiceSignoutResponseParams(
  ...
  for (; it != header_dictionary.end(); ++it) {
    ...
    if (key_name == kSignoutObfuscatedIDAttrName) {
      ...
      base::TrimString(value, "\"", &params.gaia_id.back());
    } else if (key_name == kSignoutEmailAttrName) {
      // The email is wrapped in quotes.
      params.email.push_back(value);
      base::TrimString(value, "\"", &params.email.back());
  ...
  if ((params.gaia_id.size() != params.email.size()) ||
      (params.gaia_id.size() != params.session_index.size())) {
