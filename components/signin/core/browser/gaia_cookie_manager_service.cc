...
void GaiaCookieManagerService::LogOutAllAccounts(const std::string& source) {
  ...
  if (!log_out_queued) {
    ...
    if (requests_.size() == 1) {
      fetcher_retries_ = 0;
      signin_client_->DelayNetworkCall(
          base::Bind(&GaiaCookieManagerService::StartPreGaiaLogOut,
                     base::Unretained(this)));
    }
  }
}
...
void GaiaCookieManagerService::StartPreGaiaLogOut() {
  DCHECK(requests_.front().request_type() == GaiaCookieRequestType::LOG_OUT);
  VLOG(1) << "GaiaCookieManagerService::StartPreGaiaLogOut";
