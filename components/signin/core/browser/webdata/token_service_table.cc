...
bool TokenServiceTable::RemoveAllTokens() {
  VLOG(1) << "Remove all token";
  sql::Statement s(db_->GetUniqueStatement("DELETE FROM token_service"));

  bool result = s.Run();
  LOG_ASSERT(result) << "Failed to remove all tokens";
  return result;
}
...
bool TokenServiceTable::RemoveTokenForService(const std::string& service) {
  ...
  bool result = s.Run();
  LOG_ASSERT(result) << "Failed to remove token for " << service;
  return result;
}
...
bool TokenServiceTable::SetTokenForService(const std::string& service,
  ...
  bool result = s.Run();
  LOG_ASSERT(result) << "Failed to insert or replace token for " << service;
  return result;
}
