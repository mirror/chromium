...
class AccountFetcherService : public KeyedService,
 ...
 public:
  ...
  // Indicates if all user images have been fetched. If the result is false,
  // either some user information is still missing or some image requests are
  // still in the fetch queue.
  virtual bool AreAllUserImagesFetched() const;
 ...
 private:
  ...
  // Holds all account ids for which an image is being fetched.
  std::unordered_set<std::string> user_image_requests_;
