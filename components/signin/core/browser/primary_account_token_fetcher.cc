...
PrimaryAccountTokenFetcher::PrimaryAccountTokenFetcher(
...
void PrimaryAccountTokenFetcher::Start() {
...
void PrimaryAccountTokenFetcher::WaitForRefreshToken() {
...
void PrimaryAccountTokenFetcher::StartAccessTokenRequest() {
...
void PrimaryAccountTokenFetcher::GoogleSigninSucceeded(
...
void PrimaryAccountTokenFetcher::GoogleSigninFailed(
...
void PrimaryAccountTokenFetcher::OnRefreshTokenAvailable(
    const std::string& account_id) {
  DCHECK(waiting_for_refresh_token_);
  DCHECK(!waiting_for_sign_in_);
...
void PrimaryAccountTokenFetcher::OnRefreshTokensLoaded() {
  DCHECK(waiting_for_refresh_token_);
  DCHECK(!waiting_for_sign_in_);
  DCHECK(!access_token_request_);
...
void PrimaryAccountTokenFetcher::OnGetTokenSuccess(
...
void PrimaryAccountTokenFetcher::OnGetTokenFailure(
    const OAuth2TokenService::Request* request,
    const GoogleServiceAuthError& error) {
  DCHECK_EQ(request, access_token_request_.get());
  std::unique_ptr<OAuth2TokenService::Request> request_deleter(
