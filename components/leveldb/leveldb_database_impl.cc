...
Status ForEachWithPrefix(leveldb::DB* db,
                         const Slice& key_prefix,
                         FunctionType function) {
  ReadOptions prefix_read_options;
  // Disable filling the cache for bulk scans.
    ...
    switch (operations[i]->type) {
      ...
      case mojom::BatchOperationType::COPY_PREFIXED_KEY: {
        if (!operations[i]->value) {
          ...
          std::move(callback).Run(DatabaseError::INVALID_ARGUMENT);
          return;
        }
