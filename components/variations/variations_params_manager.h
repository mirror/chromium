...
class VariationParamsManager {
 public:
  ...
  // SetVariationParams.
  //
  // The function creates a new trial group, used only for testing (note that
  // trials using params shouldn't be using group name for business logic).
  //
  ...
  static void AppendVariationParams(
      const std::string& trial_name,
      const std::string& study_group_name,
      const std::map<std::string, std::string>& param_values,
