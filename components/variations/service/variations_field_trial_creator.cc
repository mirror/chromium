...
std::string VariationsFieldTrialCreator::GetLatestCountry() const {
  ...
  return !override_country.empty()
             ? override_country
             : seed_store_.local_state()->GetString(prefs::kVariationsCountry);
}
...
bool VariationsFieldTrialCreator::CreateTrialsFromSeed(
  ...
  const int64_t last_fetch_time_internal =
      seed_store_.local_state()->GetInt64(prefs::kVariationsLastFetchTime);
...
VariationsFieldTrialCreator::GetClientFilterableStateForVersion(
  ...
  state->reference_date =
      GetReferenceDateForExpiryChecks(seed_store_.local_state());
...
std::string VariationsFieldTrialCreator::LoadPermanentConsistencyCountry(
  ...
  const base::ListValue* list_value = seed_store_.local_state()->GetList(
  ...
  if (latest_country.empty()) {
    if (!is_pref_valid)
      seed_store_.local_state()->ClearPref(
          prefs::kVariationsPermanentConsistencyCountry);
...
void VariationsFieldTrialCreator::StorePermanentCountry(
  ...
  seed_store_.local_state()->Set(prefs::kVariationsPermanentConsistencyCountry,
                                 new_list_value);
}
...
void VariationsFieldTrialCreator::RecordLastFetchTime() {
  ...
  seed_store_.local_state()->SetInt64(prefs::kVariationsLastFetchTime,
...
bool VariationsFieldTrialCreator::MemberPointersAreNotNullForTesting() const {
  return client_ && seed_store_.local_state();
}
