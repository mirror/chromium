...
VariationsService::VariationsService(
    ...
    : state_manager_(state_manager),
      ...
      request_count_(0),
      field_trial_creator_(local_state, std::move(client), ui_string_overrider),
      weak_ptr_factory_(this) {
  DCHECK(field_trial_creator_.client().get());
...
void VariationsService::PerformPreMainMessageLoopStartup() {
  DCHECK(field_trial_creator_.thread_checker().CalledOnValidThread());
...
void VariationsService::SetCreateTrialsFromSeedCalledForTesting(bool called) {
  DCHECK(field_trial_creator_.thread_checker().CalledOnValidThread());
  field_trial_creator_.set_create_trials_from_seed_called(called);
}
...
GURL VariationsService::GetVariationsServerURL(
  ...
  const std::string restrict_param =
      !restrict_mode_override.empty()
          ...
          : GetRestrictParameterPref(field_trial_creator_.client().get(),
                                     policy_pref_service);
...
void VariationsService::DoActualFetch() {
  ...
  pending_seed_request_->SetRequestContext(
      field_trial_creator_.client()->GetURLRequestContext());
...
bool VariationsService::StoreSeed(const std::string& seed_data,
  ...
  base::PostTaskWithTraitsAndReplyWithResult(
      ...
      field_trial_creator_.client()->GetVersionForSimulationCallback(),
...
void VariationsService::OnURLFetchComplete(const net::URLFetcher* source) {
  ...
  if (response_code == net::HTTP_OK ||
    ...
    if (!response_date.is_null()) {
      field_trial_creator_.client()->GetNetworkTimeTracker()->UpdateNetworkTime(
          response_date,
...
std::string VariationsService::GetStoredPermanentCountry() {
  const base::ListValue* list_value =
      field_trial_creator_.local_state()->GetList(
