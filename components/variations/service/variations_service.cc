...
bool VariationsService::DoFetchToURL(GURL url) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void VariationsService::OnURLFetchComplete(const net::URLFetcher* source) {
  ...
  if (status.status() != net::URLRequestStatus::SUCCESS) {
    ...
    if (was_https && !insecure_variations_server_url_.is_empty()) {
      // If the insecure fetch finishes, we return (since the next fetch will
      // call OnURLFetchComplete on its response, otherwise, if it was
      // interrupted (likely due to an encryption failure), we finish executing
      // this function for the previous fetch.
      if (DoFetchToURL(insecure_variations_server_url_)) {
        return;
      }
    }
