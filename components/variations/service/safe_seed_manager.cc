...
// Decision: In the case of true positives, it's more helpful to fall back to
// the safe seed relatively quickly. Hence, use a threshold that achieves this,
// at the cost of some false positives for users in the bottom 2% of
// connectivity.
// TODO(isherman): If we're happy to set a threshold that only works for 98% of
// users (rather than 99% on desktop and 99.5% on mobile), we can set a
// threshold of 3 across all platforms... except for the mysterious Dev channel.
// TODO(isherman): Set to 4 on mobile and 5 on desktop.
constexpr int kFetchFailureStreakThreshold = 4;
...
bool SafeSeedManager::ShouldRunInSafeMode() const {
  ...
  // TODO(isherman): Add test coverage.
  int num_crashes = local_state->GetInteger(prefs::kVariationsCrashStreak);
  int num_failed_fetches =
      local_state->GetInteger(prefs::kVariationsFailedToFetchSeedStreak);
