...
bool VariationsSeedStore::StoreSafeSeed(
  ...
  // Performance optimization: Avoid an expensive no-op of overwriting the
  ...
  if (base64_seed_data != previous_safe_seed) {
    // It's theoretically possible to overwrite an existing safe seed value that
    // was identical to the latest seed with a different value. This could
