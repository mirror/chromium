...
void VizMainImpl::TearDownFromGpuThread(base::WaitableEvent* wait) {
  ...
  if (compositor_thread_) {
    // The compositor thread will post tasks back to the GPU thread during
    // shutdown. Continue to handle those tasks with a nested RunLoop.
    base::RunLoop run_loop(base::RunLoop::Type::kNestableTasksAllowed);
    compositor_thread_task_runner_->PostTask(
        FROM_HERE,
        base::BindOnce(&VizMainImpl::TearDownOnCompositorThread,
