...
void HitTestAggregator::Swap() {
  SwapHandles();
  if (read_handle_replaced_ && write_handle_replaced_) {
    delegate_->OnAggregatedHitTestRegionListUpdated(
        ...
        write_size_, use_handle_one_);
  } else if (read_handle_replaced_) {
    delegate_->OnAggregatedHitTestRegionListUpdated(
        read_handle_->Clone(mojo::SharedBufferHandle::AccessMode::READ_ONLY),
        read_size_, mojo::ScopedSharedBufferHandle(), 0, use_handle_one_);
  } else if (write_handle_replaced_) {
    delegate_->OnAggregatedHitTestRegionListUpdated(
...
void HitTestAggregator::SwapHandles() {
  ...
  swap(read_handle_replaced_, write_handle_replaced_);
  use_handle_one_ = !use_handle_one_;
}
