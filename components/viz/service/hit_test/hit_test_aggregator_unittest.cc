 ...
 public:
  ...
  void SwapHandles() {
    delegate_->OnAggregatedHitTestRegionListUpdated(
        mojo::ScopedSharedBufferHandle(), 0, mojo::ScopedSharedBufferHandle(),
        0, use_handle_one_);
  }
 ...
 public:
  ...
  void OnAggregatedHitTestRegionListUpdated(
    ...
    if (handle_one.is_valid()) {
      handle_one_buffer_ =
          handle_one->Map(handle_one_size * sizeof(AggregatedHitTestRegion));
    }
    if (handle_two.is_valid()) {
      handle_two_buffer_ =
          handle_two->Map(handle_two_size * sizeof(AggregatedHitTestRegion));
    }
    if (use_handle_one) {
      active_list_ =
