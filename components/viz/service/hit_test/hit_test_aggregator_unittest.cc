...
class TestHitTestAggregator : public HitTestAggregator {
 public:
  ...
  void SwapHandles() {
    delegate_->SharedMemoryHandlesUpdated(mojo::ScopedSharedBufferHandle(), 0,
...
class TestGpuRootCompositorFrameSink : public HitTestAggregatorDelegate {
 public:
  ...
  void SharedMemoryHandlesUpdated(mojo::ScopedSharedBufferHandle read_handle,
                                  uint32_t read_size,
...
class TestHostFrameSinkManager : public HostFrameSinkManager {
 public:
  ...
  void OnSharedMemoryHandlesReceived(
    ...
    active_mapping_ =
        read_handle->Map(read_size * sizeof(AggregatedHitTestRegion));
    idle_mapping_ =
        write_handle->Map(write_size * sizeof(AggregatedHitTestRegion));
  }

  AggregatedHitTestRegion* regions() {
    return static_cast<AggregatedHitTestRegion*>(active_mapping_.get());
  }
 ...
 private:
  ...
  mojo::ScopedSharedBufferMapping active_mapping_;
  mojo::ScopedSharedBufferMapping idle_mapping_;
...
class TestFrameSinkManagerImpl : public FrameSinkManagerImpl {
 public:
  ...
  void OnSharedMemoryHandlesReceived(
    ...
    if (host_client_) {
      host_client_->OnSharedMemoryHandlesReceived(
          frame_sink_id,
...
void TestGpuRootCompositorFrameSink::SharedMemoryHandlesUpdated(
    mojo::ScopedSharedBufferHandle read_handle,
    uint32_t read_size,
    mojo::ScopedSharedBufferHandle write_handle,
    uint32_t write_size) {
  frame_sink_manager_->OnSharedMemoryHandlesReceived(
      frame_sink_id_,
      read_handle->Clone(mojo::SharedBufferHandle::AccessMode::READ_ONLY),
      read_size,
      write_handle->Clone(mojo::SharedBufferHandle::AccessMode::READ_ONLY),
