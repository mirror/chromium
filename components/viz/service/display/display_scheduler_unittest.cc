...
class FakeDisplaySchedulerClient : public DisplaySchedulerClient {
 public:
  ...
  void SurfaceDamaged(const cc::SurfaceId& surface_id) {
 ...
 protected:
  ...
  std::set<cc::SurfaceId> undrawn_surfaces_;
};
...
class DisplaySchedulerTest : public testing::Test {
 public:
  ...
  void AdvanceTimeAndBeginFrameForTest(
      const std::vector<cc::SurfaceId>& observing_surfaces) {
    now_src_.Advance(base::TimeDelta::FromMicroseconds(10000));
...
TEST_F(DisplaySchedulerTest, ResizeHasLateDeadlineUntilNewRootSurface) {
  ...
  AdvanceTimeAndBeginFrameForTest(std::vector<cc::SurfaceId>());
...
TEST_F(DisplaySchedulerTest, SurfaceDamaged) {
  ...
  cc::SurfaceId sid1(kArbitraryFrameSinkId,
                     cc::LocalSurfaceId(2, base::UnguessableToken::Create()));
  cc::SurfaceId sid2(kArbitraryFrameSinkId,
                     cc::LocalSurfaceId(3, base::UnguessableToken::Create()));
