  ...
  for (const auto& pass : frame->render_pass_list) {
    ...
    for (auto quad = pass->quad_list.begin(); quad != quad_list_end;) {
      ...
      if (last_sqs != quad->shared_quad_state) {
        ...
        // translation transforms. Note that 0 scale transform will make
        // |visible_quad_layer_rect| map to an empty rect such that
        // |current_sqs_intersects_occlusion| will be set to false.
        if (current_sqs_intersects_occlusion &&
          ...
          // Scale transform can be inverted by multiplying 1/scale (scale !=0)
          // and translation transform can be inverted by applying the reversed
          // directional translation. Therefore, |transform| is always
          // invertible.
