    ...
    for (auto quad = pass->quad_list.begin(); quad != quad_list_end;) {
      ...
      if (last_sqs != quad->shared_quad_state) {
        ...
        if (current_sqs_intersects_occlusion &&
          ...
          // Positive scale transform can be inverted by multiplying 1/scale and
          // translation transform can be inverted by applying the reversed
          ...
          // define occluded region in the quad content space with inversed
          // transform.
