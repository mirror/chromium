...
void Display::SetLocalSurfaceId(const cc::LocalSurfaceId& id,
  ...
  current_surface_id_ = cc::SurfaceId(frame_sink_id_, id);
  device_scale_factor_ = device_scale_factor;
...
bool Display::SurfaceDamaged(const cc::SurfaceId& surface_id,
...
void Display::SurfaceDiscarded(const cc::SurfaceId& surface_id) {
  if (aggregator_)
    aggregator_->ReleaseResources(surface_id);
}

bool Display::SurfaceHasUndrawnFrame(const cc::SurfaceId& surface_id) const {
  if (!surface_manager_)
    return false;
...
const cc::SurfaceId& Display::CurrentSurfaceId() {
