...
void Display::RemoveOverdrawQuads(CompositorFrame* frame) {
  ...
  for (const auto& pass : frame->render_pass_list) {
    ...
    if (pass->has_transparent_background)
      continue;

    // TODO(yiyix): Perform draw occlusion inside the render pass containing the
    // copy request.
    ...
    for (auto quad = pass->quad_list.begin(); quad != pass->quad_list.end();) {
      ...
      if (!current_sqs)
        current_sqs = quad->shared_quad_state;

      gfx::Transform transform =
          quad->shared_quad_state->quad_to_target_transform;

      if (current_sqs != quad->shared_quad_state) {
        if (current_sqs->opacity == 1 && current_sqs->are_contents_opaque &&
            current_sqs->quad_to_target_transform.Preserves2dAxisAlignment()) {
          gfx::Rect clipped_rect =
              cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(
                  current_sqs->quad_to_target_transform,
                  current_sqs->visible_quad_layer_rect);

          if (current_sqs->is_clipped)
            clipped_rect.Intersect(current_sqs->clip_rect);

          occlusion_region.Union(clipped_rect);
        }
        // If the visible_rect of the currect shared quad state does not
        // intersects with the occlusion rect, we can skip draw occlusion checks
        // for quads in the currect shared quad state.
        skip_quads =
            !occlusion_region.Intersects(cc::MathUtil::MapEnclosingClippedRect(
                transform, quad->shared_quad_state->visible_quad_layer_rect));
      }
