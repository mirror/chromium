...
bool Display::DrawAndSwap() {
  ...
  if (should_draw) {
    if (settings_.enable_draw_occlusion) {
      ...
      UMA_HISTOGRAM_COUNTS_100("Compositing.Display.Draw.Occlusion",
  ...
  for (const auto& pass : frame->render_pass_list) {
    for (auto quad = pass->quad_list.begin(); quad != pass->quad_list.end();) {
      if (last_sqs != quad->shared_quad_state && last_sqs->opacity == 1 &&
        ...
        //        LOG(ERROR) << "rect_to_add: " << rect_to_add.ToString();
      ...
      //      LOG(ERROR) << "occlusion_rect " << occlusion_rect->ToString();
      //      LOG(ERROR) << "draw_quad " << draw_quad.ToString();
