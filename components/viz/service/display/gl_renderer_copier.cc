    ...
    case ResultFormat::RGBA_BITMAP: {
      if (request->is_scaled()) {
        const GLuint result_texture = RenderResultTexture(
            *request, output_rect, internal_format, framebuffer_texture,
...
GLuint GLRendererCopier::RenderResultTexture(
  ...
  gfx::Rect sampling_rect =
      request.is_scaled() ? output_rect
                          : (result_rect + output_rect.OffsetFromOrigin());
  sampling_rect = window_rect_callback_.Run(sampling_rect);
