...
void GLRendererCopier::CopyFromTextureOrFramebuffer(
  ...
  // Finalize the result rect, either as the entirety of the RenderPass's
  // output or the client-provided area clamped to the RenderPass's output size.
  // If there will be zero pixels of output or the scaling ratio was not
  // reasonable, do not proceed.
  const gfx::Rect result_clip =
      request->is_scaled()
          ? copy_output::ComputeResultRect(output_rect, request->scale_from(),
                                           request->scale_to())
          : output_rect;
  gfx::Rect result_rect;
  if (request->has_area()) {
    result_rect = request->area();
    result_rect.Intersect(result_clip);
  } else {
    result_rect = result_clip;
  }
  ...
  switch (request->result_format()) {
    case ResultFormat::RGBA_BITMAP: {
      if (request->is_scaled()) {
        const GLuint result_texture =
            RenderResultTexture(*request, output_rect, internal_format,
                                framebuffer_texture, framebuffer_texture_size,
                                result_clip.OffsetFromOrigin(), result_rect);
        ...
        StartReadbackFromFramebuffer(std::move(request),
...
GLuint GLRendererCopier::RenderResultTexture(
  ...
  gfx::Rect sampling_rect = window_rect_callback_.Run(
      request.is_scaled() ? output_rect : result_rect);
  ...
  if (request.is_scaled()) {
    ...
    scaler->Scale(source_texture, source_texture_size,
                  ...
                  result_rect - result_clip_offset);
