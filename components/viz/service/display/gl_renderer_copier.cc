...
void GLRendererCopier::FreeUnusedCachedResources() {
  ...
  // resources they held.
  const auto IsTooOld = [this](const CacheEntry& entry) {
    return static_cast<int32_t>(purge_counter_ -
                                entry.purge_count_at_last_use) >=
           kKeepalivePeriod;
  };
  for (CacheEntry& entry : cache_) {
    if (IsTooOld(entry))
...
GLuint GLRendererCopier::RenderResultTexture(
  ...
  if (framebuffer_texture != 0) {
    ...
    // return it as the result texture.
    const int purpose =
...
GLRendererCopier::CacheEntry* GLRendererCopier::GetOrCreateCacheEntry(
...
GLuint GLRendererCopier::TakeCachedObjectOrCreate(
  ...
  if (!for_source.is_empty()) {
    GLuint& cached_object_name =
        GetOrCreateCacheEntry(for_source)->object_names[which];
...
void GLRendererCopier::CacheObjectOrDelete(
  ...
  auto* const entry = GetOrCreateCacheEntry(for_source);
...
GLRendererCopier::TakeCachedScalerOrCreate(
  ...
  if (for_request.has_source()) {
    std::unique_ptr<GLHelper::ScalerInterface>& cached_scaler =
        GetOrCreateCacheEntry(for_request.source())->scaler;
    if (cached_scaler) {
