...
class GLPixelBufferI420Result : public CopyOutputResult {
 public:
  ...
  bool ReadI420Planes(uint8_t* y_out,
    ...
    const int chrome_row_bytes = (size().width() + 1) / 2;
    DCHECK_GE(u_out_stride, chrome_row_bytes);
    DCHECK_GE(v_out_stride, chrome_row_bytes);
    ...
    if (pixels) {
      ...
      CopyPlane(pixels, chroma_stride_, chrome_row_bytes, chroma_height, u_out,
                u_out_stride);
      pixels += chroma_stride_ * chroma_height;
      CopyPlane(pixels, chroma_stride_, chrome_row_bytes, chroma_height, v_out,
                v_out_stride);
      gl->UnmapBufferCHROMIUM(GL_PIXEL_PACK_TRANSFER_BUFFER_CHROMIUM);
    }
    gl->BindBuffer(GL_PIXEL_PACK_TRANSFER_BUFFER_CHROMIUM, 0);
...
class ReadI420PlanesWorkflow
 ...
 public:
  ReadI420PlanesWorkflow(std::unique_ptr<CopyOutputRequest> copy_request,
                         scoped_refptr<ContextProvider> context_provider,
                         const std::array<GLuint, 3>& plane_textures,
                         const std::array<GLuint, 3>& plane_framebuffers,
                         GLenum readback_format,
                         const gfx::Rect& result_rect)
      : copy_request_(std::move(copy_request)),
        result_rect_(result_rect),
        context_provider_(std::move(context_provider)) {
    // Note: See gl_helper.h for an explanation of how planar data is packed
    // into RGBA textures, and how Y/Chroma plane rounding work.
    const gfx::Size y_texture_size =
        I420Converter::GetYPlaneTextureSize(result_rect.size());
    y_stride_ = kRGBABytesPerPixel * y_texture_size.width();
    const int y_plane_bytes = y_stride_ * y_texture_size.height();
    const gfx::Size chroma_texture_size =
        I420Converter::GetChromaPlaneTextureSize(result_rect.size());
    chroma_stride_ = kRGBABytesPerPixel * chroma_texture_size.width();
    const int chroma_plane_bytes =
        chroma_stride_ * chroma_texture_size.height();
    ...
    // Execute three asynchronous read-pixels operations, one for each plane,
    // with a query that notifies when the pixels are available in the transfer
    // buffer.
    gl->GenQueriesEXT(3, queries_.data());
    DCHECK(std::find(queries_.begin(), queries_.end(), 0) == queries_.end());
    auto* const context_support = context_provider_->ContextSupport();
    for (int plane = 0; plane < 3; ++plane) {
      gl->BindFramebuffer(GL_FRAMEBUFFER, plane_framebuffers[plane]);
      gl->FramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                               GL_TEXTURE_2D, plane_textures[plane], 0);
      gl->BeginQueryEXT(GL_ASYNC_PIXEL_PACK_COMPLETED_CHROMIUM,
                        queries_[plane]);
      ...
      if (plane > 0)
        offset_in_buffer += y_plane_bytes;
      if (plane == 2)
        offset_in_buffer += chroma_plane_bytes;
      gl->ReadPixels(0, 0, texture_size.width(), texture_size.height(),
                     readback_format, GL_UNSIGNED_BYTE, offset_in_buffer);
      gl->EndQueryEXT(GL_ASYNC_PIXEL_PACK_COMPLETED_CHROMIUM);
  ...
  void OnFinishedPlane(int plane) {
    ...
    if (queries_ == std::array<GLuint, 3>{{0, 0, 0}}) {
      copy_request_->SendResult(std::make_unique<GLPixelBufferI420Result>(
          result_rect_, context_provider_, transfer_buffer_, y_stride_,
          chroma_stride_));
...
void GLRendererCopier::StartI420ReadbackFromTexture(
  ...
  // The ReadI420PlanesWorkflow starts the asynchronous readback workflow and,
  // because it is ref-counted, it self-deletes when complete.
  new ReadI420PlanesWorkflow(std::move(request), context_provider_,
                             plane_textures, plane_framebuffers,
                             converter->GetReadbackFormat(), result_rect);
