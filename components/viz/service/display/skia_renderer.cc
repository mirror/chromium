...
SkiaRenderer::SkiaRenderer(const viz::RendererSettings* settings,
...
viz::ResourceFormat SkiaRenderer::BackbufferFormat() const {
  // From GL Renderer.
...
void SkiaRenderer::BeginDrawingFrame() {
  ...
  // TODO(weiliangc): Implement use_sync_query_.
  if (use_sync_query_) {
    NOTIMPLEMENTED();
  } else {
...
bool SkiaRenderer::BindFramebufferToTexture(const ScopedResource* texture) {
  DCHECK(texture->id());
...
void SkiaRenderer::DrawRenderPassQuad(const RenderPassDrawQuad* quad) {
  ...
  LOG(ERROR) << "texture maybe? " << content_texture->id();
  ResourceProvider::ScopedReadLockSkImage lock(resource_provider_,
                                               content_texture->id());
  if (!lock.sk_image())
    return;

  LOG(ERROR) << "yes texture " << content_texture->id();
...
sk_sp<SkShader> SkiaRenderer::GetBackgroundFilterShader(
  ...
  // TODO(weiliangc): properly implement background filters.
  NOTIMPLEMENTED();
  return nullptr;
}
