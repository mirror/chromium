...
class GLRendererCopierTest : public testing::Test {
 public:
  ...
  size_t GetCopierCacheSize() {
    return std::distance(copier_->cache_.begin(), copier_->cache_.end());
  }
  bool CacheContainsObject(const base::UnguessableToken& source,
                           int which,
                           GLuint name) {
    return !copier_->cache_.empty() &&
           copier_->cache_.front().request_source == source &&
           copier_->cache_.front().object_names[which] == name;
  }
  bool CacheContainsScaler(const base::UnguessableToken& source,
    ...
    return !copier_->cache_.empty() &&
           copier_->cache_.front().request_source == source &&
           copier_->cache_.front().scaler &&
           copier_->cache_.front().scaler->IsSameScaleRatio(
