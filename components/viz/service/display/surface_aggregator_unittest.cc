 ...
 private:
  static void AddSurfaceQuad(cc::RenderPass* pass,
    ...
    shared_quad_state->SetAll(
        ...
        clip_rect, is_clipped, is_contents_opaque, opacity, blend_mode, 0);
...
void AddSolidColorQuadWithBlendMode(const gfx::Size& size,
  ...
  bool is_contents_opaque = SkColorGetA(color) == 0xFF;
  float opacity = 1.f;

  bool force_anti_aliasing_off = false;
  auto* sqs = pass->CreateAndAppendSharedQuadState();
  sqs->SetAll(layer_to_target_transform, layer_rect, visible_layer_rect,
              clip_rect, is_clipped, is_contents_opaque, opacity, blend_mode,
              0);
