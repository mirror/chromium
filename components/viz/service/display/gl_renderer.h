...
class VIZ_SERVICE_EXPORT GLRenderer : public DirectRenderer {
 ...
 private:
  ...
  void ScheduleCALayers(OverlayResourceLockList* overlay_resources);
  void ScheduleDCLayers(OverlayResourceLockList* overlay_resources);
  void ScheduleOverlays(OverlayResourceLockList* overlay_resources);
  ...
  // OverlayTextures that have been given to the gpu. Each vector represents
  // a single frame, and may be empty if none were used in that frame. Ordered
  // from oldest to most recent frame.
  std::vector<std::vector<std::unique_ptr<OverlayTexture>>>
      in_flight_overlay_textures_;
