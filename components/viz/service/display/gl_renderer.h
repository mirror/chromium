...
class VIZ_SERVICE_EXPORT GLRenderer : public DirectRenderer {
 ...
 private:
  ...
  using OverlayResourceLockMac =
      std::unique_ptr<cc::LocalResourceProvider::ScopedLocalReadGL>;
  using OverlayResourceLockListMac = std::vector<OverlayResourceLockMac>;

  // Resources that have been sent to the GPU process, but not yet swapped.
  OverlayResourceLockListMac pending_overlay_resources_mac_;

  // Resources that should be shortly swapped by the GPU process.
  base::circular_deque<OverlayResourceLockListMac>
      swapping_overlay_resources_mac_;

  // Resources that the GPU process has finished swapping. The key is the
  // texture id of the resource.
  std::map<unsigned, OverlayResourceLockMac>
      swapped_and_acked_overlay_resources_mac_;
