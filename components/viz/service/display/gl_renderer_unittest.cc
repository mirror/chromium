...
// For Resource from cc::LayerTreeResourceProvider to
// cc::DisplayResourceProvider, the verified_flush_ of gpu::SyncToken will be
// marked as true. So for the resource transfer case, no need to check flag
// verified_flush_.
bool static CompareSyncToken(const gpu::SyncToken& a, const gpu::SyncToken& b) {
  return a.namespace_id() == b.namespace_id() &&
         a.extra_data_field() == b.extra_data_field() &&
         a.command_buffer_id() == b.command_buffer_id() &&
         a.release_count() == b.release_count();
}

MATCHER_P(MatchesSyncTokenWithoutVerify, sync_token, "") {
  gpu::SyncToken other;
  memcpy(&other, arg, sizeof(other));
  return CompareSyncToken(sync_token, other);
}
...
static cc::ReturnCallback GetReturnCallback(
  ...
  return base::Bind(&CollectResources, array);
}
...
TEST_F(GLRendererTest, DontOverlayWithCopyRequests) {
  ...
  std::unique_ptr<SharedBitmapManager> shared_bitmap_manager(
      new cc::TestSharedBitmapManager());
  std::unique_ptr<cc::DisplayResourceProvider> parent_resource_provider =
      cc::FakeResourceProvider::Create<cc::DisplayResourceProvider>(
          output_surface->context_provider(), shared_bitmap_manager.get());
  ...
  std::unique_ptr<cc::LayerTreeResourceProvider> child_resource_provider =
...
TEST_F(GLRendererTest, OverlaySyncTokensAreProcessed) {
  ...
  EXPECT_CALL(*context,
              waitSyncToken(MatchesSyncTokenWithoutVerify(sync_token)))
      .Times(1);
