...
void GLRenderer::DrawContentQuadNoAA(const ContentDrawQuadBase* quad,
  ...
  bool has_tex_clamp_rect = !fills_right_edge || !fills_bottom_edge;
  gfx::SizeF tex_clamp_size(texture_size);
  if (!fills_right_edge)
    tex_clamp_size.set_width(tex_coord_rect.width() - 0.5f);
  if (!fills_bottom_edge)
    tex_clamp_size.set_height(tex_coord_rect.height() - 0.5f);
