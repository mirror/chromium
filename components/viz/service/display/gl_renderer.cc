...
class GLRenderer::SyncQuery {
 public:
  ...
  scoped_refptr<internal::RemotableResource::Fence> Begin() {
    DCHECK(!IsPending());
    ...
    // query is required.
    return make_scoped_refptr<internal::RemotableResource::Fence>(
 ...
 private:
  class Fence : public internal::RemotableResource::Fence {
   public:
    explicit Fence(base::WeakPtr<GLRenderer::SyncQuery> query)
        : query_(query) {}

    // internal::RemotableResource::Fence implementation.
    void Set() override {
      DCHECK(query_);
      query_->Set();
    }
...
void GLRenderer::BeginDrawingFrame() {
  ...
  scoped_refptr<internal::RemotableResource::Fence> read_lock_fence;
  if (use_sync_query_) {
...
std::unique_ptr<cc::ScopedResource> GLRenderer::GetBackdropTexture(
  ...
  device_background_texture->Allocate(
      bounding_rect.size(), RemotableResourceTextureHint::kDefault,
