...
class GLRenderer::SyncQuery {
 public:
  ...
  scoped_refptr<RemotableResourceFence> Begin() {
    DCHECK(!IsPending());
 ...
 private:
  class Fence : public RemotableResourceFence {
   public:
    explicit Fence(base::WeakPtr<GLRenderer::SyncQuery> query)
        : query_(query) {}

    // RemotableResourceFence implementation.
    void Set() override {
      DCHECK(query_);
      query_->Set();
    }
...
void GLRenderer::ScheduleRenderPassDrawQuad(
  ...
  if (!overlay_resource_pool_) {
    overlay_resource_pool_ = cc::ResourcePool::Create(
        ...
        RemotableResourceTextureHint::kOverlay, base::TimeDelta::FromSeconds(3),
