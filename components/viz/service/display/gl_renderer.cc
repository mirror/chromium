...
void GLRenderer::DidReceiveTextureInUseResponses(
  ...
  for (const gpu::TextureInUseResponse& response : responses) {
    ...
    DCHECK(it != awaiting_release_overlay_textures_.end());
...
void GLRenderer::CopyRenderPassDrawQuadToOverlayResource(
  ...
  auto match_with_id = [&](const std::unique_ptr<OverlayTexture>& overlay) {
    return overlay->render_pass_id == params.quad->render_pass_id &&
  ...
  if (!*overlay_texture) {
    ...
    auto match = [&](const std::unique_ptr<OverlayTexture>& overlay) {
      return overlay->texture.size().width() >= iosurface_width &&
             overlay->texture.size().height() >= iosurface_height &&
             overlay->texture.size().width() <= iosurface_width * 2 &&
             overlay->texture.size().height() <= iosurface_height * 2;
    };
    ...
    if (it != available_overlay_textures_.end()) {
      (*it)->render_pass_id = params.quad->render_pass_id;
