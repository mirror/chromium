    ...
    if (filter) {
      ...
      if (filter) {
        ...
        if (params->contents_texture) {
          params->color_space = current_framebuffer_colorspace_;
          id = params->contents_texture->id();
          target = GL_TEXTURE_2D;
          size = params->contents_texture->size();
        } else if (params->bypass_quad_texture) {
          ...
          id = prefilter_bypass_quad_texture_lock.texture_id();
          target = prefilter_bypass_quad_texture_lock.target();
          size = prefilter_bypass_quad_texture_lock.size();
        }
        if (params->contents_texture || params->bypass_quad_texture)
          params->filter_image = ApplyImageFilter(
              ScopedUseGrContext::Create(this), src_rect, params->dst_rect,
              quad->filters_scale, std::move(filter), id, target, size, &offset,
              &subset, params->flip_texture, quad->filters_origin);
...
void GLRenderer::UpdateRPDQTexturesForSampling(
  ...
  if (params->filter_image) {
    ...
    gl_->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    gl_->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // |params->color_space| was populated when |params->filter_image|
    // was populated.
    ...
    if (params->contents_texture) {
      ...
      params->color_space = params->bypass_quad_resource_lock->color_space();
      params->source_needs_flip = params->flip_texture;
    }
  }
}
...
void GLRenderer::ChooseRPDQProgram(DrawRenderPassDrawQuadParams* params,
  ...
  SetUseProgram(ProgramKey::RenderPass(
                ...
                params->color_space, target_color_space);
}
...
void GLRenderer::UpdateRenderPassTextures(
  ...
  for (const auto& pair : render_pass_textures_) {
    ...
    bool hint_appropriate =
        (ResourceTextureHint::kFramebuffer & required_hint) == required_hint;
...
void GLRenderer::AllocateRenderPassResourceIfNeeded(
  ...
  if (!resource) {
    ...
    resource = std::make_unique<ScopedRenderPassTexture>(
        output_surface_->context_provider()->ContextGL(), caps.texture_usage,
        caps.texture_storage);
  }
