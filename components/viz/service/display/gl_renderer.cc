...
void GLRenderer::SetShaderColorTemperature(
    DrawRenderPassDrawQuadParams* params) {
  if (!current_program_ ||
      current_program_->temperature_blue_scale_location() == -1 ||
      current_program_->temperature_green_scale_location() == -1) {
    return;
  }
  ...
  gl_->Uniform1f(current_program_->temperature_blue_scale_location(),
                 layer_blue_scale);
  gl_->Uniform1f(current_program_->temperature_green_scale_location(),
                 layer_green_scale);
}
