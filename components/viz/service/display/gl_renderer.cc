...
void GLRenderer::SetUseProgram(const ProgramKey& program_key_no_color,
  ...
  const bool has_output_color_matrix = !output_color_matrix.isIdentity();
  ...
  if (is_root_render_pass && has_output_color_matrix) {
    CHECK_NE(current_program_->output_color_matrix_location(), -1);
    float matrix[16];
