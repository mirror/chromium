...
GLenum GLRenderer::GetFramebufferCopyTextureFormat() {
  ...
  if (!current_framebuffer_texture_)
    ...
    format = GLCopyTextureInternalFormat(ResourceFormat::RGBA_8888);
...
void GLRenderer::DrawRenderPassQuad(const RenderPassDrawQuad* quad,
  ...
  if (bypass != render_pass_bypass_quads_.end()) {
    ...
    ScopedRenderPassTexture* contents_texture = &contents_texture_it->second;
      ...
      if (filter) {
        ...
        if (params->contents_texture) {
          params->contents_and_bypass_color_space =
              current_framebuffer_colorspace_;
...
void GLRenderer::UpdateRPDQTexturesForSampling(
  ...
  if (params->filter_image) {
    ...
    params->contents_and_bypass_color_space = current_framebuffer_colorspace_;
...
void GLRenderer::BindFramebufferToTexture(const RenderPassId render_pass_id) {
  ...
  ScopedRenderPassTexture* texture = &contents_texture_it->second;
  DCHECK(texture);
  DCHECK(texture->id());
...
void GLRenderer::UpdateRenderPassTextures(
  ...
  for (const auto& pair : render_pass_textures_) {
    ...
    if (render_pass_it == render_passes_in_frame.end()) {
      auto it = render_pass_textures_.find(pair.first);
      if (it != render_pass_textures_.end())
        render_pass_textures_.erase(it);
    }
  }
}
...
void GLRenderer::AllocateRenderPassResourceIfNeeded(
  ...
  auto contents_texture_it = render_pass_textures_.find(render_pass_id);
  ScopedRenderPassTexture* resource = &contents_texture_it->second;

  if (contents_texture_it != render_pass_textures_.end() && resource->id()) {
    bool size_appropriate = resource->size().width() >= enlarged_size.width() &&
                            resource->size().height() >= enlarged_size.height();
    bool hint_appropriate = (resource->hint() & texturehint) == texturehint;
    if (size_appropriate && hint_appropriate) {
      return;
    }
    int id = resource->id();
    ScopedRenderPassTexture resource_again(
        output_surface_->context_provider()->ContextGL(), id, enlarged_size,
        texturehint, caps.texture_usage, caps.texture_storage,
        caps.texture_npot);
    // TODO(xing.xu): erase will free resource, will lead to real Free. But how
    // to avoid real free by erase? set_id(0) is ugly idea.
    resource->set_id(0);
    render_pass_textures_.erase(render_pass_id);
    render_pass_textures_.insert(
        std::make_pair(render_pass_id, std::move(resource_again)));
  } else {
    ScopedRenderPassTexture resource_again(
        output_surface_->context_provider()->ContextGL(), enlarged_size,
        texturehint, caps.texture_usage, caps.texture_storage,
        caps.texture_npot);
    // TODO(xing.xu): render_pass_textures_.erase(render_pass_id);
    render_pass_textures_.insert(
        std::make_pair(render_pass_id, std::move(resource_again)));
  }
