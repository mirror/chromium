...
void GLRenderer::AllocateRenderPassResourceIfNeeded(
  ...
  if (contents_texture_it != render_pass_textures_.end() &&
    ...
    bool size_appropriate =
        contents_texture.size().width() >= enlarged_size.width() &&
        contents_texture.size().height() >= enlarged_size.height();
    bool hint_appropriate =
        (contents_texture.hint() & texturehint) == texturehint;
    if (size_appropriate && hint_appropriate) {
      return;
    }
    // Move constructor will move the gl_id_ from contents_texture to the
    // size&hint updated contents_texture_updated.
    ScopedRenderPassTexture contents_texture_updated =
        std::move(contents_texture);
    contents_texture_updated.set_size(enlarged_size);
    contents_texture_updated.set_hint(texturehint);
    render_pass_textures_.erase(render_pass_id);
    render_pass_textures_.insert(
        std::make_pair(render_pass_id, std::move(contents_texture_updated)));
  } else {
    ScopedRenderPassTexture contents_texture_updated(
