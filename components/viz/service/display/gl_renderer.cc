...
struct DrawRenderPassDrawQuadParams {
  ...
  ScopedLocalResource* contents_texture = nullptr;
...
void GLRenderer::DrawRenderPassQuad(const RenderPassDrawQuad* quad,
  ...
  if (bypass != render_pass_bypass_quads_.end()) {
    ...
    ScopedLocalResource* contents_texture =
  ...
  if (params->filter_image) {
    ...
    if (params->contents_texture) {
      DCHECK_EQ(static_cast<GLenum>(GL_TEXTURE_2D),
                params->contents_texture->target());
      params->contents_texture->BindForSampling(GL_TEXTURE0);
...
void GLRenderer::BindFramebufferToTexture(const RenderPassId render_pass_id) {
  ScopedLocalResource* texture = render_pass_textures_[render_pass_id].get();
  DCHECK(texture);
  DCHECK(texture->id());
...
void GLRenderer::UpdateRenderPassTextures(
  ...
  for (const auto& pair : render_pass_textures_) {
    ...
    ScopedLocalResource* texture = pair.second.get();
    DCHECK(texture);
...
void GLRenderer::AllocateRenderPassResourceIfNeeded(
  ...
  if (!resource) {
    const ResourceTextureSettings* settings = resource_provider_->GetSettings();
    resource = std::make_unique<ScopedLocalResource>(
        output_surface_->context_provider()->ContextGL(),
        settings->use_texture_usage_hint, settings->use_texture_storage);
  }
  resource->Allocate(enlarged_size,
                     current_frame()->current_render_pass->color_space);
}
...
bool GLRenderer::IsRenderPassResourceAllocated(
  ...
  ScopedLocalResource* texture = texture_it->second.get();
  DCHECK(texture);
  return texture->id() != 0;
}
