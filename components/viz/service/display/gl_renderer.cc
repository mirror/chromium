...
void GLRenderer::FinishDrawingFrame() {
  ...
  // Locks for resources that will be given to the gpu for overlays, to keep
  // them marked as busy.
  OverlayResourceLockList overlay_resources;

  ScheduleCALayers(&overlay_resources);
  ScheduleDCLayers(&overlay_resources);
  ScheduleOverlays(&overlay_resources);

  swapping_overlay_resources_.push_back(std::move(overlay_resources));
...
void GLRenderer::SwapBuffersComplete() {
  if (settings_->release_overlay_resources_after_gpu_query) {
    ...
    if (in_flight_overlay_textures_.size() > 1) {
      for (auto& overlay_texture : in_flight_overlay_textures_.front())
        available_overlay_textures_.push_back(std::move(overlay_texture));
      in_flight_overlay_textures_.erase(in_flight_overlay_textures_.begin());
    }
  }
}
...
void GLRenderer::DidReceiveTextureInUseResponses(
  ...
  for (const gpu::TextureInUseResponse& response : responses) {
    ...
    for (auto it = awaiting_release_overlay_textures_.begin();
         it != awaiting_release_overlay_textures_.end(); ++it) {
      if ((*it)->texture.id() == response.texture) {
...
void GLRenderer::ScheduleCALayers(OverlayResourceLockList* overlay_resources) {
  scoped_refptr<CALayerOverlaySharedState> shared_state;
  size_t copied_render_pass_count = 0;

  std::vector<std::unique_ptr<OverlayTexture>> overlay_textures;
  ...
  for (const CALayerOverlay& ca_layer_overlay :
    ...
    if (ca_layer_overlay.rpdq) {
      ...
      if (overlay_texture)
        overlay_textures.push_back(std::move(overlay_texture));
  ...
  // together.
  in_flight_overlay_textures_.push_back(std::move(overlay_textures));
  // Drop any leftover textures that aren't in use.
  available_overlay_textures_.clear();
}

void GLRenderer::ScheduleDCLayers(OverlayResourceLockList* overlay_resources) {
    ...
    for (const auto& contents_resource_id : dc_layer_overlay.resources) {
      if (contents_resource_id) {
        overlay_resources->push_back(
            base::MakeUnique<cc::DisplayResourceProvider::ScopedReadLockGL>(
                resource_provider_, contents_resource_id));
        texture_ids[i] = overlay_resources->back()->texture_id();
        ids_to_send = i + 1;
      }
      i++;
    }
...
void GLRenderer::ScheduleOverlays(OverlayResourceLockList* overlay_resources) {
...
void GLRenderer::CopyRenderPassDrawQuadToOverlayResource(
  ...
  // stable and less likely to clobber eachothers textures.
  for (auto it = available_overlay_textures_.begin();
       it != available_overlay_textures_.end(); ++it) {
    OverlayTexture* overlay = it->get();
    if (overlay->render_pass_id != params.quad->render_pass_id ||
        overlay->texture.size().width() < iosurface_width ||
        overlay->texture.size().height() < iosurface_height ||
        overlay->texture.size().width() > iosurface_width * 2 ||
        overlay->texture.size().height() > iosurface_height * 2)
      continue;
    *overlay_texture = std::move(*it);
  ...
  if (!*overlay_texture) {
    ...
    for (auto it = available_overlay_textures_.begin();
         it != available_overlay_textures_.end(); ++it) {
      OverlayTexture* overlay = it->get();
      if (overlay->texture.size().width() < iosurface_width ||
          overlay->texture.size().height() < iosurface_height ||
          overlay->texture.size().width() > iosurface_width * 2 ||
          overlay->texture.size().height() > iosurface_height * 2)
        continue;
      overlay->render_pass_id = params.quad->render_pass_id;
  ...
  // Lastly, make a new one if we could not find a match. Sadtimes.
  if (!*overlay_texture) {
    *overlay_texture = std::make_unique<OverlayTexture>();
    (*overlay_texture)->render_pass_id = params.quad->render_pass_id;
    (*overlay_texture)->texture = ScopedGpuMemoryBufferTexture(
        output_surface_->context_provider(),
  ...
  *new_bounds = gfx::RectF(updated_dst_rect.origin(),
                           gfx::SizeF(iosurface_width, iosurface_height));
