...
void GLRenderer::AllocateRenderPassResourceIfNeeded(
  ...
  auto& resource_again = render_pass_textures_[render_pass_id];
  const auto& caps = output_surface_->context_provider()->ContextCapabilities();
  resource_again = std::make_unique<ScopedRenderPassTexture>(
      output_surface_->context_provider()->ContextGL(), enlarged_size,
      texturehint, caps.texture_usage, caps.texture_storage, caps.texture_npot);
