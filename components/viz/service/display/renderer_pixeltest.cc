...
TEST_F(GLRendererPixelTest, SolidColorWithTemperature_NonRootRenderPass) {
  ...
  gfx::Rect child_rect(0, 0, viewport_rect.width() / 2,
                       viewport_rect.height() / 2);
  // Child pass.
  int child_pass_id = 2;
  RenderPass* child_pass =
      cc::AddRenderPass(&render_passes_in_draw_order, child_pass_id, child_rect,
  ...
  RenderPass* root_pass = cc::AddRenderPass(
      ...
      gfx::Transform(), cc::FilterOperations());
  cc::AddQuad(root_pass, viewport_rect, SK_ColorYELLOW);
  root_pass->damage_rect = viewport_rect;
