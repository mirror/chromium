...
gfx::Rect SurfaceAggregator::DamageRectForSurface(
  ...
  const cc::SurfaceId& previous_surface_id =
...
void SurfaceAggregator::HandleSurfaceQuad(
  ...
  cc::SurfaceId surface_id = surface_quad->surface_id;
...
gfx::Rect SurfaceAggregator::PrewalkTree(const cc::SurfaceId& surface_id,
  ...
  struct SurfaceInfo {
    SurfaceInfo(const cc::SurfaceId& id,
...
cc::CompositorFrame SurfaceAggregator::Aggregate(
    const cc::SurfaceId& surface_id) {
  uma_stats_.Reset();
...
void SurfaceAggregator::ReleaseResources(const cc::SurfaceId& surface_id) {
