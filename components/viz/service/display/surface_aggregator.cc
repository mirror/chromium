...
void SurfaceAggregator::HandleSurfaceQuad(
  ...
  if (!surface || !surface->HasActiveFrame()) {
    SurfaceId fallback_surface_id =
        surface_quad->fallback_surface_id.has_value()
            ? *surface_quad->fallback_surface_id
            : SurfaceId();
    if (fallback_surface_id.is_valid()) {
      Surface* fallback_surface =
          manager_->GetSurfaceForId(fallback_surface_id);
      ...
      EmitFallbackSurface(surface_quad, fallback_surface, primary_rect,
...
void SurfaceAggregator::EmitFallbackSurface(
  ...
  fallback_surface_draw_quad.SetNew(
      ...
      fallback_surface->surface_id(), base::nullopt,
      SurfaceDrawQuadType::FALLBACK,
      primary_surface_quad->default_background_color, nullptr);
