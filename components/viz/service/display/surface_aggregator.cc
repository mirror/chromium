...
gfx::Rect SurfaceAggregator::PrewalkTree(Surface* surface,
  ...
  for (const auto& surface_info : child_surfaces) {
    ...
    gfx::Rect surface_damage = surface_info.quad_rect;
    if (!surface || !surface->HasActiveFrame()) {
      // contained_surfaces_[surface_info.primary_id] = 0;
    ...
    if (surface) {
      surface_damage = PrewalkTree(surface, surface_info.has_moved_pixels,
