...
TEST_F(DisplayTest, CompositorFrameWithEpsilonScaleTransform) {
  ...
  SkMScalar epsilon = 0.000000001;
  SkMScalar larger_than_epsilon = 0.00000001;
...
TEST_F(DisplayTest, CompositorFrameWithNegativeScaleTransform) {
...

  {
    ...
    EXPECT_EQ(2u, frame.render_pass_list.front()->quad_list.size());
    display_->RemoveOverdrawQuads(&frame);
    // |negative_scale| transform is invertible. Since the x-axis is negated,
    // |quad2| is not under occlusion rect after transforming as shown in the
    // figure below, so no quads removed.
    // After transforming:
    //          |
    //  q2 +----|----+ occlusuin rect
    //     |    |    |
    ...
    //          |
    //          |
    EXPECT_TRUE(negative_scale.GetInverse(&inverted));
    EXPECT_EQ(2u, frame.render_pass_list.front()->quad_list.size());
    EXPECT_EQ(rect.ToString(), frame.render_pass_list.front()
                                   ->quad_list.ElementAt(0)
                                   ->rect.ToString());
...

  {
    ...
    EXPECT_EQ(2u, frame.render_pass_list.front()->quad_list.size());
    display_->RemoveOverdrawQuads(&frame);
    // |negative_scale| transform is invertible. Since the y-axis is negated,
    // |quad2| is not under occlusion rect after transforming as shown in the
    // figure below, so no quads removed.
    // After transforming:
    //          |
...

  {
    ...
    EXPECT_EQ(2u, frame.render_pass_list.front()->quad_list.size());
    display_->RemoveOverdrawQuads(&frame);
    // |negative_scale| transform is invertible. Since z-axis is missing in a
    // 2d plane, negate z-axis cause |q2| to stay where it was before.
    // After transforming:
    //          |
    //          |----+ occlusuin rect
    //          |    |   q2
