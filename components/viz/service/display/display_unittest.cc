...
TEST_F(DisplayTest, CompositorFrameWithEpsilonScaleTransform) {
...

  {
    ...
    // case. |larger_epsilon_scale| transform has the scale set tot 10^-7.
...
TEST_F(DisplayTest, CompositorFrameWithNegativeScaleTransform) {
  ...
  SkMScalar negative = -0.5;
  gfx::Transform negative_scale;
  negative_scale.Scale(negative, negative);
  bool is_clipped = false;
  bool are_contents_opaque = true;
  float opacity = 1.f;
  SharedQuadState* shared_quad_state =
...

  {
    ...
    EXPECT_EQ(2u, frame.render_pass_list.front()->quad_list.size());
    display_->RemoveOverdrawQuads(&frame);
    // |negative_scale| transform is invertible. |quad2| is not under
    // occlusion rect after transform, so no quads removed.
    // After transforming:
    //          |
    //          |----+ occlusuin rect
    //          |    |
