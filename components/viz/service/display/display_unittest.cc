...
// Check if draw occlusion does not remove any drawquads when no quads is being
// covered completely.
TEST_F(DisplayTest, DrawOcclusionWithNonCoveringDrawQuad) {
  ...
  // +----+
  {
    ...
    // DrawQuad, so the size of quad_list remain unchanged after calling
    // RemoveOverdrawQuads.
    EXPECT_EQ(1u, frame.render_pass_list.front()->quad_list.size());
  ...
  //   +----+
  {
    ...
    // (define by rect2 (50, 50, 100x100)), the |quad_list| size remain the same
  ...
  //   +--+                                    +--+
  {
    ...
    display_->RemoveOverdrawQuads(&frame);
    // Since |quad| (defined by rect1 (0, 0, 100x100)) cannot cover |quad2|
    // (define by rect3 (25, 25, 50x100)), the |quad_list| size remain the same
    // after calling RemoveOverdrawQuads. The visible region of |quad2| on
  ...
  //  +--+                                        +--+
  {
    ...
    // (define by rect6 (25, 0, 50x160)), the |quad_list| size remain the same
    // after calling RemoveOverdrawQuads. The visible region of |quad2| on
    // screen is rect6 - rect6 U rect7 = (25, 0, 50x20 U 25, 120, 50x40), which
  ...
  // +----+
  {
    ...
    // Since |quad| (defined by rect1 (0, 0, 100x100)) cannot cover |quad2|
    // (define by rect4 (150, 0, 50x50)), the |quad_list| size remain the same
    // after calling RemoveOverdrawQuads. The visible region of |quad2| on
  ...
  // +------+
  {
    ...
    // Since |quad| (defined by rect1 (0, 0, 100x100)) cannot cover |quad2|
    // (define by rect5 (0, 0, 120x120)), the |quad_list| size remain the same
    // after calling RemoveOverdrawQuads. The visible region of |quad2| on
    // screen is rect5 - rect1 U rect5 = (100, 0, 20x100 U 0, 100, 100x20),
...
TEST_F(DisplayTest, CompositorFrameWithNegativeScaleTransform) {
...

  {
    ...
    // intersects with |quad| any more, so no quad is removed.
    // In target space:
    //          |
    //  q2 +----|----+ occlusion rect
    //     |    |    |
...

  {
    ...
    // at where it was as before. |quad2| overlaps with |quad|.
    // In target space:
    //          |
    //          |----+ occlusion rect
    //          |    |   q2
...
TEST_F(DisplayTest, CompositorFrameWithRotation) {
  ...
  auto* quad2 = frame.render_pass_list.front()
                    ->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
  {
    // Apply ratation transform on |rect1| only.
  ...
  {
    // Apply ratation transform on |rect1| and |rect2|.
    shared_quad_state->SetAll(rotate, rect1, rect1, rect1, is_clipped,
                              are_contents_opaque, opacity,
                              SkBlendMode::kSrcOver, 0);
...
TEST_F(DisplayTest, CompositorFrameWithCoveredRenderPass) {
  ...
  {
    // rect1 is a drawquad from SQS1 and which is also the render pass rect
...
TEST_F(DisplayTest, CompositorFrameWithClip) {
...

  {
    ...
    // (50, 50, 20x10)). |quad| interscts with |quad2| in the target space. The
...
TEST_F(DisplayTest, CompositorFrameWithCopyRequest) {
                    ...
                    ->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
  {
    ...
    // occlusion with copy_request on root render pass, |quad_list| reduces its
...
TEST_F(DisplayTest, CompositorFrameWithMultipleDrawQuadInSharedQuadState) {
  ...
  {
    // A Shared quad states that contains 4 drawquads intersects with another
    // shared quad state that contains 1 drawquad.
    // +--+-++--+
    // |  | +|--+
    // +--+--+
    // |  |  |
  ...
  {
    // A Shared quad states that contains 4 DrawQuads intersects with another
    // shared quad state that contains 2 DrawQuads.
    // +-+--+--+-+
    // +-|--|--|-+
    //   +--+--+
    //   |  |  |
