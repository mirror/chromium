  ...
  void RunTest(int num_surfaces,
    ...
    for (int i = 0; i < num_surfaces; i++) {
      child_supports[i] = CompositorFrameSinkSupport::Create(
          nullptr, &manager_, cc::FrameSinkId(1, i + 1), kIsChildRoot,
          kHandlesFrameSinkIdInvalidation, kNeedsSyncPoints);
    }
    aggregator_ = base::MakeUnique<SurfaceAggregator>(
        &manager_, resource_provider_.get(), optimize_damage);
    for (int i = 0; i < num_surfaces; i++) {
      cc::LocalSurfaceId local_surface_id(i + 1, kArbitraryToken);
      ...
      if (i >= 1) {
        ...
        surface_quad->SetNew(
            ...
            cc::SurfaceId(cc::FrameSinkId(1, i),
                          cc::LocalSurfaceId(i, kArbitraryToken)),
    ...
    auto root_support = CompositorFrameSinkSupport::Create(
        nullptr, &manager_, cc::FrameSinkId(1, num_surfaces + 1), kIsRoot,
        kHandlesFrameSinkIdInvalidation, kNeedsSyncPoints);
    timer_.Reset();
    do {
      auto pass = cc::RenderPass::Create();
      ...
      surface_quad->SetNew(
          ...
          cc::SurfaceId(cc::FrameSinkId(1, num_surfaces),
                        cc::LocalSurfaceId(num_surfaces, kArbitraryToken)),
          cc::SurfaceDrawQuadType::PRIMARY, nullptr);
      ...
      root_support->SubmitCompositorFrame(
          cc::LocalSurfaceId(num_surfaces + 1, kArbitraryToken),
          std::move(frame));

      cc::CompositorFrame aggregated = aggregator_->Aggregate(
          cc::SurfaceId(cc::FrameSinkId(1, num_surfaces + 1),
                        cc::LocalSurfaceId(num_surfaces + 1, kArbitraryToken)));
      timer_.NextLap();
