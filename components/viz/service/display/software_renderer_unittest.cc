...
class SoftwareRendererScaledCopyOutputTest
 ...
 public:
  // Returns a string containing all the R, G, B or A values from an SkBitmap in
  // a human-readable format.
  static std::string MakeHumanReadableChannelOfBitmap(const SkBitmap& bitmap,
                                                      int channel) {
    std::ostringstream out;
    out << std::setfill('0') << std::setbase(16);
    for (int y = 0; y < bitmap.height(); y++) {
      for (int x = 0; x < bitmap.width(); x++) {
        const SkColor color = bitmap.getColor(x, y);
        const unsigned value = color >> (channel * 8) & 0xff;
  ...
  // Returns a human-readable diagnostic string that compares the two bitmaps.
  static std::string MakeHumanReadableComparison(const SkBitmap& expected,
                                                 const SkBitmap& actual) {
    std::ostringstream out;
    int bpp = expected.bytesPerPixel();
    for (int i = 0; i < bpp; i++) {
      out << "Channel " << i << " expected:" << std::endl
          << MakeHumanReadableChannelOfBitmap(expected, i) << "Channel " << i
          << " actual:" << std::endl
          << MakeHumanReadableChannelOfBitmap(actual, i) << std::endl;
    }
    return out.str();
  }
};
...
TEST_P(SoftwareRendererScaledCopyOutputTest, ScaledCopyOfDrawnFrame) {
  ...
  EXPECT_EQ(0, num_bad_pixels)
      << MakeHumanReadableComparison(expected_bitmap, result_bitmap)
