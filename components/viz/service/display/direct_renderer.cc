...
bool DirectRenderer::UseRenderPass(const RenderPass* render_pass) {
  ...
  gfx::Size enlarged_size = render_pass->output_rect.size();
  enlarged_size.Enlarge(enlarge_pass_texture_amount_.width(),
                        enlarge_pass_texture_amount_.height());
  bool did_alloc = AllocateRenderPassResourceIfNeeded(
      render_pass->id, enlarged_size, RenderPassTextureHint(render_pass));
  if (!did_alloc &&
      ((render_pass->cache_render_pass &&
        !render_pass->has_damage_from_contributing_content) ||
       current_frame()->ComputeScissorRectForRenderPass().IsEmpty())) {
    return false;
  }

  if (BindFramebufferToTextureAndInitializeViewport(render_pass->id)) {
    return true;
  }
