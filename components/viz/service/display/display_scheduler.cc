...
void DisplayScheduler::SetNewRootSurface(const cc::SurfaceId& root_surface_id) {
...
void DisplayScheduler::ProcessSurfaceDamage(const cc::SurfaceId& surface_id,
...
bool DisplayScheduler::UpdateHasPendingSurfaces() {
  ...
  for (const std::pair<cc::SurfaceId, SurfaceBeginFrameState>& entry :
       surface_states_) {
    const cc::SurfaceId& surface_id = entry.first;
...
void DisplayScheduler::OnSurfaceDestroyed(const cc::SurfaceId& surface_id) {
  auto it = surface_states_.find(surface_id);
  if (it == surface_states_.end())
    return;
...
bool DisplayScheduler::OnSurfaceDamaged(const cc::SurfaceId& surface_id,
                                        const cc::BeginFrameAck& ack) {
...
void DisplayScheduler::OnSurfaceDiscarded(const cc::SurfaceId& surface_id) {
  client_->SurfaceDiscarded(surface_id);
}

void DisplayScheduler::OnSurfaceDamageExpected(const cc::SurfaceId& surface_id,
...
void DisplayScheduler::OnSurfaceWillDraw(const cc::SurfaceId& surface_id) {}
