...
void SoftwareRenderer::CopyDrawnRenderPass(
  ...
  // Finalize the result rect, either as the entirety of the RenderPass's output
  // or the client-provided area clamped to the RenderPass's output size. If
  // there will be zero pixels of output or the scaling ratio was not
  ...
  if (request->is_scaled()) {
    ...
    const SkIRect relative_result_rect{result_rect.x() - result_clip.x(),
                                       result_rect.y() - result_clip.y(),
                                       result_rect.right() - result_clip.x(),
                                       result_rect.bottom() - result_clip.y()};
    bitmap =
        ImageOperations::Resize(render_pass_output, method, result_clip.width(),
                                result_clip.height(), relative_result_rect);
