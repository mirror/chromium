...
void SoftwareRenderer::CopyDrawnRenderPass(
  ...
  const gfx::Rect result_clip = copy_output::ComputeResultRect(
      gfx::Rect(output_rect.size()), request->scale_from(),
      request->scale_to());
  ...
  if (request->is_scaled()) {
    ...
    bitmap = ImageOperations::Resize(
        render_pass_output, method, result_clip.width(), result_clip.height(),
