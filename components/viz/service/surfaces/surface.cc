...
void Surface::RejectCompositorFramesToFallbackSurfaces() {
  base::flat_map<FrameSinkId, SurfaceId> frame_sink_ids_for_dependencies;
  for (const SurfaceId& surface_id :
       GetPendingFrame().metadata.activation_dependencies) {
    frame_sink_ids_for_dependencies[surface_id.frame_sink_id()] = surface_id;
  }
  ...
  for (const SurfaceId& surface_id :
    ...
    auto it = frame_sink_ids_for_dependencies.find(surface_id.frame_sink_id());
    bool is_fallback_surface = it != frame_sink_ids_for_dependencies.end();
    if (!is_fallback_surface)
      continue;

    Surface* fallback_surface =
        surface_manager_->GetLatestInFlightSurface(it->second, surface_id);
