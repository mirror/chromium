...
class VideoDetectorTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    detector_->tick_clock_ = mock_task_runner_->GetMockTickClock();
    detector_->video_inactive_timer_ =
        std::make_unique<base::OneShotTimer>(detector_->tick_clock_.get());
    detector_->video_inactive_timer_->SetTaskRunner(mock_task_runner_);
...
TEST_F(VideoDetectorTest, ReportOnceForMultipleClients) {
  constexpr gfx::Rect kClientBounds(gfx::Point(), gfx::Size(1024, 768));
  ...
  base::TimeDelta d;
  for (; d < 2 * kMinDuration; d += time_between_updates) {
