...
// destroyed when either: 1) Just after the mojo binding has closed; or 2) The
// FrameSinkManagerImpl is shutting down. The capturer also works with
// FrameSinkManagerImpl to resolve the capture target, a CapturableFrameSink,
// from a given FrameSinkId. The implementation accounts for a possible race
// between when this capturer learns of a new FrameSinkId and when
// FrameSinkManagerImpl learns of it.
//
// Once the CapturableFrameSink target is resolved, this capturer attaches to it
// to receive events of interest regarding the frame flow, display timiming, and
// changes to the frame sink's surface. For some subset of frames, decided by
// media::VideoCaptureOracle, this capturer will make a CopyOutputRequest on the
// surface.
//
// As this is a mojo service that could be delivering frames to a consumer
// running in another process, the output video frames are pooled and backed by
// shared memory, to minimize coping and transport the video stream efficiently.
class VIZ_SERVICE_EXPORT FrameSinkVideoCapturerImpl
 ...
 public:
  ...
  // Binds this instance to the Mojo message pipe endpoint in |request| as well
  // as the owner/manager of this instance (which is, of course, expected to
  // outlive this instance).
  void Bind(mojom::FrameSinkVideoCapturerRequest request,
            FrameSinkVideoCapturerManager* frame_sink_manager);

  // The currently-requested frame sink for capture. FrameSinkManagerImpl calls
  // this when it learns of a new CapturableFrameSink to see if the target can
  // be resolved.
  ...
  // (if any), and attaching to the new target. This is called by
  // FrameSinkManagerImpl. If |target| is null, the capturer goes idle and
  // expects this method to be called again in the near future, once the target
  // becomes known to FrameSinkManagerImpl.
  ...
  // FrameSinkId associated with it has become invalid. This is called by
 ...
 private:
  ...
  void DidCopyFrame(OracleFrameNumber frame_number,
  ...
  void DidCaptureFrame(OracleFrameNumber frame_number,
                       scoped_refptr<media::VideoFrame> frame);
  ...
  // consumer.
  void MaybeDeliverFrame(OracleFrameNumber frame_number,
                         scoped_refptr<media::VideoFrame> frame);

  // An unguessable token representing this instance as an issuer of
  // CopyOutputRequests. The Surface uses this to auto-cancel stale requests
  // (i.e., prior requests that did not execute). Also, the implementations that
  // execute CopyOutputRequests use this as a hint to cache objects for a huge
  ...
  // Owner/Manager of this instance. It is null before the call to Bind(), and
  // not-null thereafter.
  FrameSinkVideoCapturerManager* frame_sink_manager_ = nullptr;
  ...
  // BeginFrameArgs passed to OnBeginFrame() calls. This array is an associative
  // mapping from |sequence_number % kDesignLimitMaxFrames| to the estimated
  // display time (|frame_time + interval|).
  ...
  struct CapturedFrame {
    OracleFrameNumber frame_number;
    scoped_refptr<media::VideoFrame> frame;
    CapturedFrame(OracleFrameNumber n, scoped_refptr<media::VideoFrame> f);
  ...
  // When delivering frames, this is the number of the frame that must be
  // delivered next, to ensure in-order frame delivery to the consumer.
  OracleFrameNumber next_delivery_frame_number_;
