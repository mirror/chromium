...
FrameSinkVideoCapturerImpl::FrameSinkVideoCapturerImpl()
    : copy_request_source_(base::UnguessableToken::Create()),
      ...
      next_delivery_frame_number_(oracle_.next_frame_number()),
...
void FrameSinkVideoCapturerImpl::Bind(
  ...
  binding_.set_connection_error_handler(
      base::BindOnce(&FrameSinkVideoCapturerManager::OnCapturerConnectionLost,
                     base::Unretained(frame_sink_manager), this));
}
...
void FrameSinkVideoCapturerImpl::SetResolvedTarget(
  ...
  if (resolved_target_) {
    ...
    // If a consumer has been waiting on a resolved capture target, ensure a
    // first frame is sent to the consumer, just in case the target frame
    // sink is idle (i.e., not compositing new frames at the moment).
    if (consumer_) {
      RequestRefreshFrame();
    }
  } else {
...
void FrameSinkVideoCapturerImpl::SetMinCapturePeriod(
  ...
  constexpr base::TimeDelta kMinMinCapturePeriod =
      base::TimeDelta::FromMicroseconds(1000000 /
  ...
  if (!base::TimeTicks::IsHighResolution()) {
    constexpr base::TimeDelta kMinLowResCapturePeriod =
        base::TimeDelta::FromMicroseconds(1000000 / 30);
...
void FrameSinkVideoCapturerImpl::SetResolutionConstraints(
  ...
  // TODO(miu): Consider limiting the |max_size|, based on available GPU and
  // system memory.
...
void FrameSinkVideoCapturerImpl::ChangeTarget(
  ...
  DCHECK(frame_sink_manager_) << "ChangeTarget() called on unbound instance.";
  SetResolvedTarget(
...
void FrameSinkVideoCapturerImpl::Start(
  ...
  // Ensure a first frame is sent to the consumer, just in case the target frame
  // sink is idle (i.e., not compositing new frames at the moment).
  RequestRefreshFrame();
}
...
void FrameSinkVideoCapturerImpl::RequestRefreshFrame() {
  ...
  MaybeCaptureFrame(VideoCaptureOracle::kPassiveRefreshRequest,
                    gfx::Rect(oracle_.source_size()), base::TimeTicks::Now());
}
...
void FrameSinkVideoCapturerImpl::MaybeCaptureFrame(
  ...
  //
  // TODO(miu): Consider limiting the capture frame rate when composited frames
  // are close-to or missing the BeginFrameArgs deadlines.
  ...
  const OracleFrameNumber frame_number = oracle_.next_frame_number();
  ...
  // TODO(miu): Set VideoFrameMetadata::INTERACTIVE_CONTENT once mouse cursor
  // rendering is moved to VIZ.
  ...
  // If there is currently no resolved target, deliver a blank black frame.
  if (!resolved_target_) {
    media::FillYUV(frame.get(), 0x00, 0x80, 0x80);
    DidCaptureFrame(frame_number, std::move(frame));
    return;
  }
  ...
  // TODO(miu): As an optimization, set the result selection to just the part of
  // the result that would have changed due to aggregated damage over all the
  // frames that weren't captured.
  request->set_result_selection(gfx::Rect(content_rect.size()));
  resolved_target_->RequestCopyOfNextFrame(std::move(request));
}
...
void FrameSinkVideoCapturerImpl::DidCaptureFrame(
  ...
  delivery_queue_.emplace(frame_number, std::move(frame));
  while (delivery_queue_.top().frame_number == next_delivery_frame_number_) {
    MaybeDeliverFrame(next_delivery_frame_number_, delivery_queue_.top().frame);
...
void FrameSinkVideoCapturerImpl::MaybeDeliverFrame(
    ...
    scoped_refptr<VideoFrame> frame) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  DCHECK_EQ(frame_number, next_delivery_frame_number_);
  ...
  if (frame) {
    const bool had_reference_time = frame->metadata()->GetTimeTicks(
        VideoFrameMetadata::REFERENCE_TIME, &media_ticks);
    DCHECK(had_reference_time);
  }
  const bool should_deliver_frame =
      oracle_.CompleteCapture(frame_number, !!frame, &media_ticks);
  ...
  // Construct a new InFlightFrameDelivery instance for this frame and deliver
  // the frame to the consumer.
  mojom::FrameSinkVideoFrameCallbacksPtr callbacks;
  mojo::MakeStrongBinding(
      std::make_unique<InFlightFrameDelivery>(
          ...
          base::BindOnce(&VideoCaptureOracle::RecordConsumerFeedback,
                         feedback_weak_factory_.GetWeakPtr(), frame_number)),
      mojo::MakeRequest(&callbacks));
  const gfx::Rect update_rect = frame->visible_rect();
  consumer_->OnFrameCaptured(mojom::FrameSinkVideoFrame::New(
      std::move(frame), update_rect, std::move(callbacks)));
}

FrameSinkVideoCapturerImpl::CapturedFrame::CapturedFrame(
    OracleFrameNumber n,
    scoped_refptr<media::VideoFrame> f)
    : frame_number(n), frame(std::move(f)) {}
...
bool FrameSinkVideoCapturerImpl::CapturedFrame::operator<(
  ...
  // Reverse sort order, so std::priority_queue becomes a min-heap.
