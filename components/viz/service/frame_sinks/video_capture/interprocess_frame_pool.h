...
class VIZ_SERVICE_EXPORT InterprocessFramePool {
 ...
 private:
  using PoolBufferId = decltype(media::VideoCaptureBufferPool::kInvalidId + 0);

  // Helper to build a media::MojoSharedBufferVideoFrame instance backed by a
  // specific buffer from the |buffer_pool_|.
  scoped_refptr<media::VideoFrame> WrapBuffer(PoolBufferId buffer_id,
                                              const gfx::Size& size,
                                              media::VideoPixelFormat format);
  ...
  base::flat_map<const media::VideoFrame*, PoolBufferId> buffer_map_;
  ...
  PoolBufferId resurrectable_buffer_id_ =
      media::VideoCaptureBufferPool::kInvalidId;
