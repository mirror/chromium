...
TEST_F(DirectLayerTreeFrameSinkTest, HitTestRegionList) {
  ...
  auto hit_test_region_list =
      std::move(frame_sink_manager_.hit_test_region_list_);
  ...
  hit_test_region_list = std::move(frame_sink_manager_.hit_test_region_list_);
  EXPECT_TRUE(hit_test_region_list);
  EXPECT_EQ(display_rect_, hit_test_region_list->bounds);
  EXPECT_EQ(mojom::kHitTestMine, hit_test_region_list->flags);
  ...
  EXPECT_EQ(gfx::Rect(20, 20), hit_test_region_list->regions[0]->rect);
}
