...
class TestDirectLayerTreeFrameSink : public DirectLayerTreeFrameSink {
 public:
  ...
  cc::CompositorFrameSinkSupport* support() const { return support_.get(); }
};
...
class DirectLayerTreeFrameSinkTest : public testing::Test {
 public:
  DirectLayerTreeFrameSinkTest()
    ...
    std::unique_ptr<cc::FakeOutputSurface> display_output_surface =
        cc::FakeOutputSurface::Create3d();
    display_output_surface_ = display_output_surface.get();

    begin_frame_source_.reset(new cc::BackToBackBeginFrameSource(
