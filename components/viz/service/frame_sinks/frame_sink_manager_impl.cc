...
void FrameSinkManagerImpl::CreateVideoCapturer(
  ...
  video_capturers_.emplace_back(new FrameSinkVideoCapturerImpl(this));
  video_capturers_.back()->Bind(std::move(request));
}
...
void FrameSinkManagerImpl::OnCapturerConnectionLost(
  ...
  for (auto it = video_capturers_.begin(); it != video_capturers_.end(); ++it) {
    if (it->get() == capturer) {
      video_capturers_.erase(it);
      break;
    }
  }
}
