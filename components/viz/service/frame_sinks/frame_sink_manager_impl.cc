...
void FrameSinkManagerImpl::InvalidateFrameSinkId(
  ...
  DestroyCompositorFrameSinkInternal(frame_sink_id);
}
...
void FrameSinkManagerImpl::DestroyCompositorFrameSinkInternal(
    const FrameSinkId& frame_sink_id) {
  {
  ...
  // |iter| may have been invalidated when destroying |sink|.
  {
    auto iter = frame_sink_data_map_.find(frame_sink_id);
    if (iter != frame_sink_data_map_.end() && iter->second.empty())
