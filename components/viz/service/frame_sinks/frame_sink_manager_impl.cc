...
void FrameSinkManagerImpl::CreateRootCompositorFrameSink(
  ...
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  DCHECK_EQ(0u, support_map_.count(params->frame_sink_id));
  ...
  DCHECK(base::ContainsKey(support_map_, params->frame_sink_id));
  frame_sink_data_map_[params->frame_sink_id].sink = std::move(frame_sink);
}
...
void FrameSinkManagerImpl::CreateCompositorFrameSink(
  ...
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  DCHECK_EQ(0u, support_map_.count(frame_sink_id));
  ...
  frame_sink_data_map_[frame_sink_id].sink = std::move(frame_sink);
}
...
void FrameSinkManagerImpl::UnregisterFrameSinkHierarchy(
  ...
  RecursivelyDetachBeginFrameSource(child_frame_sink_id, mapping.source);
...
void FrameSinkManagerImpl::UnregisterCompositorFrameSinkSupport(
    const FrameSinkId& frame_sink_id) {
  DCHECK_EQ(support_map_.count(frame_sink_id), 1u);
...
void FrameSinkManagerImpl::RecursivelyAttachBeginFrameSource(
    const FrameSinkId& frame_sink_id,
    BeginFrameSource* source) {
  FrameSinkData& mapping = frame_sink_data_map_[frame_sink_id];
  if (!mapping.source) {
