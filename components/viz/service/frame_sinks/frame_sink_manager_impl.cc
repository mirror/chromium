...
void FrameSinkManagerImpl::InvalidateFrameSinkId(
  ...
  if (iter != frame_sink_data_map_.end()) {
    ...
    CleanupFrameSinkData(frame_sink_id);
  }
}
...
void FrameSinkManagerImpl::UnregisterFrameSinkHierarchy(
  ...
  if (!data.source) {
    CleanupFrameSinkData(parent_frame_sink_id);
    return;
  }
...
bool FrameSinkManagerImpl::CleanupFrameSinkData(
    const FrameSinkId& frame_sink_id) {
  auto iter = frame_sink_data_map_.find(frame_sink_id);
  if (iter != frame_sink_data_map_.end()) {
    auto& data = iter->second;
    if (!data.source && data.children.empty() && !data.sink) {
...
void FrameSinkManagerImpl::RecursivelyDetachBeginFrameSource(
  ...
  if (data.source == source) {
    data.source = nullptr;
    auto support_iter = support_map_.find(frame_sink_id);
    if (support_iter != support_map_.end())
      support_iter->second->SetBeginFrameSource(nullptr);
  }

  // Delete the FrameSinkData for |frame_sink_id| if empty.
  if (CleanupFrameSinkData(frame_sink_id))
    return;
