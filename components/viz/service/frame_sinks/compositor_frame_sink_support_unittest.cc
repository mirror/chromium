...
constexpr cc::FrameSinkId kArbitraryFrameSinkId(1, 1);
constexpr cc::FrameSinkId kAnotherArbitraryFrameSinkId(2, 2);
constexpr cc::FrameSinkId kYetAnotherArbitraryFrameSinkId(3, 3);
...
class CompositorFrameSinkSupportTest : public testing::Test {
 public:
  ...
  void RefCurrentFrameResources() {
    cc::Surface* surface = manager_.GetSurfaceForId(
        cc::SurfaceId(support_->frame_sink_id(), local_surface_id_));
...
TEST_F(CompositorFrameSinkSupportTest, AddDuringEviction) {
  ...
  cc::LocalSurfaceId local_surface_id(6, kArbitraryToken);
  ...
  EXPECT_CALL(mock_client, DidReceiveCompositorFrameAck(_))
      .WillOnce(testing::InvokeWithoutArgs([&support, &mock_client]() {
        cc::LocalSurfaceId new_id(7, base::UnguessableToken::Create());
...
TEST_F(CompositorFrameSinkSupportTest, EvictCurrentSurface) {
  ...
  auto support = CompositorFrameSinkSupport::Create(
      &mock_client, &manager_, kAnotherArbitraryFrameSinkId, kIsRoot,
      kHandlesFrameSinkIdInvalidation, kNeedsSyncPoints);
  cc::LocalSurfaceId local_surface_id(7, kArbitraryToken);
  cc::SurfaceId id(kAnotherArbitraryFrameSinkId, local_surface_id);
...
TEST_F(CompositorFrameSinkSupportTest,
  ...
  cc::SurfaceId surface_id(kAnotherArbitraryFrameSinkId, local_surface_id);
...
TEST_F(CompositorFrameSinkSupportTest,
  ...
  EXPECT_CALL(mock_client, DidReceiveCompositorFrameAck(returned_resources))
      .Times(1);
  manager_.SatisfySequence(
      cc::SurfaceSequence(kYetAnotherArbitraryFrameSinkId, 4));
  EXPECT_FALSE(manager_.GetSurfaceForId(surface_id));
}

TEST_F(CompositorFrameSinkSupportTest, DestroySequence) {
  cc::LocalSurfaceId local_surface_id2(5, kArbitraryToken);
  ...
  cc::SurfaceId id2(kYetAnotherArbitraryFrameSinkId, local_surface_id2);
...
TEST_F(CompositorFrameSinkSupportTest, InvalidFrameSinkId) {
  cc::FrameSinkId frame_sink_id(1234, 5678);

  cc::LocalSurfaceId local_surface_id(5, kArbitraryToken);
  cc::SurfaceId id(support_->frame_sink_id(), local_surface_id);
...
TEST_F(CompositorFrameSinkSupportTest, SurfaceInfo) {
  ...
  support_->SubmitCompositorFrame(local_surface_id_, std::move(frame));
  cc::SurfaceId expected_surface_id(support_->frame_sink_id(),
                                    local_surface_id_);
...
TEST_F(CompositorFrameSinkSupportTest, ZeroFrameSize) {
  cc::SurfaceId id(support_->frame_sink_id(), local_surface_id_);
