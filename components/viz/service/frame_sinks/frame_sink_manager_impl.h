...
class VIZ_SERVICE_EXPORT FrameSinkManagerImpl
 ...
 private:
  ...
  // Removes |frame_sink_data_map_| entry for |frame_sink_id| if it contains no
  // data.
  void CleanupFrameSinkData(const FrameSinkId& frame_sink_id);
  ...
  // [Root]CompositorFrameSinkImpl. Be very careful about re-using iterators to
  // items in this map as many operations will modify the map and invalidate
  // iterators.
