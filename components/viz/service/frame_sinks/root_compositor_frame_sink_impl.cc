...
RootCompositorFrameSinkImpl::RootCompositorFrameSinkImpl(
    ...
    : compositor_frame_sink_client_(std::move(client)),
      ...
      external_begin_frame_controller_binding_(
          external_begin_frame_controller_.get()),
...
void RootCompositorFrameSinkImpl::InitializeExternalBeginFrameController(
    mojom::ExternalBeginFrameControllerAssociatedRequest controller,
    mojom::ExternalBeginFrameControllerClientPtr client) {
  if (!external_begin_frame_controller_) {
    controller.ResetWithReason(
        mojom::DisplayPrivate::kErrorExternalBeginFramesDisabled,
        "External BeginFrames disabled");
    return;
  }

  if (external_begin_frame_controller_binding_.is_bound()) {
    controller.ResetWithReason(
        mojom::DisplayPrivate::
            kErrorExternalBeginFrameControllerAlreadyInitialized,
        "ExternalBeginFrameController already initialized");
    return;
  }

  external_begin_frame_controller_binding_.Bind(std::move(controller));
  external_begin_frame_controller_->SetClient(std::move(client));
}
