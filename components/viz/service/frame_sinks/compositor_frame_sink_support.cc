...
CompositorFrameSinkSupport::~CompositorFrameSinkSupport() {
  ...
  if (referenced_local_surface_id_.has_value()) {
    auto reference = MakeTopLevelRootReference(
        cc::SurfaceId(frame_sink_id_, referenced_local_surface_id_.value()));
...
void CompositorFrameSinkSupport::EvictCurrentSurface() {
  if (!current_surface_id_.is_valid())
    return;
  cc::SurfaceId to_destroy_surface_id = current_surface_id_;
  current_surface_id_ = cc::SurfaceId();
...
bool CompositorFrameSinkSupport::SubmitCompositorFrame(
  ...
  if (prev_surface &&
    ...
    current_surface_id_ = cc::SurfaceId(frame_sink_id_, local_surface_id);
...
void CompositorFrameSinkSupport::UpdateSurfaceReferences(
    const cc::LocalSurfaceId& local_surface_id,
    const std::vector<cc::SurfaceId>& active_referenced_surfaces) {
  cc::SurfaceId surface_id(frame_sink_id_, local_surface_id);

  const auto& existing_referenced_surface_set =
      surface_manager_->GetSurfacesReferencedByParent(surface_id);

  base::flat_set<cc::SurfaceId> new_referenced_surfaces(
  ...
  GetSurfaceReferenceDifference(surface_id, existing_referenced_surface_set,
                                new_referenced_surfaces, &references_to_add,
                                &references_to_remove);

  // Check if this is a display root surface and the cc::SurfaceId is changing.
  if (is_root_ && (!referenced_local_surface_id_.has_value() ||
                   referenced_local_surface_id_.value() != local_surface_id)) {
    // Make the new cc::SurfaceId reachable from the top-level root.
    references_to_add.push_back(MakeTopLevelRootReference(surface_id));

    // Make the old cc::SurfaceId unreachable from the top-level root if
    // applicable.
    if (referenced_local_surface_id_.has_value()) {
...
void CompositorFrameSinkSupport::Init(cc::SurfaceManager* surface_manager) {
