...
GLOutputSurfaceSurfaceless::GLOutputSurfaceSurfaceless(
...
void GLOutputSurfaceSurfaceless::BindFramebuffer() {
  DCHECK(buffer_queue_);
  buffer_queue_->BindFramebuffer();
}

// We call this on every frame that a value changes, but changing the size once
// we've allocated backing NativePixmapSurfaceless instances will cause a DCHECK
...
// reports.
void GLOutputSurfaceSurfaceless::Reshape(const gfx::Size& size,
...
void GLOutputSurfaceSurfaceless::SwapBuffers(OutputSurfaceFrame frame) {
  DCHECK(buffer_queue_);
...
uint32_t GLOutputSurfaceSurfaceless::GetFramebufferCopyTextureFormat() {
  return buffer_queue_->internal_format();
}

bool GLOutputSurfaceSurfaceless::IsDisplayedAsOverlayPlane() const {
  // TODO(rjkroege): implement remaining overlay functionality.
  return true;
}

unsigned GLOutputSurfaceSurfaceless::GetOverlayTextureId() const {
  return buffer_queue_->GetCurrentTextureId();
}

gfx::BufferFormat GLOutputSurfaceSurfaceless::GetOverlayBufferFormat() const {
  DCHECK(buffer_queue_);
  return buffer_queue_->buffer_format();
}

void GLOutputSurfaceSurfaceless::DidReceiveSwapBuffersAck(
