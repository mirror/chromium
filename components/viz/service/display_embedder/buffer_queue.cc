...
uint32_t BufferQueue::internalformat(const gpu::Capabilities& capabilities) const {
  DCHECK(current_surface_);
  return current_surface_->buffer->GetInternalformat(capabilities.texture_format_bgra8888);
}
