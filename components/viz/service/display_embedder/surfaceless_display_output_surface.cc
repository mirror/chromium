...
SurfacelessDisplayOutputSurface::SurfacelessDisplayOutputSurface(
  ...
  buffer_queue_.reset(new BufferQueue(
      ...
      &gl_helper_, gpu_memory_buffer_manager, widget));
...
void SurfacelessDisplayOutputSurface::BindFramebuffer() {
...
// reports.
void SurfacelessDisplayOutputSurface::Reshape(
    const gfx::Size& size,
...
void SurfacelessDisplayOutputSurface::SwapBuffers(OutputSurfaceFrame frame) {
  DCHECK(buffer_queue_);
...
uint32_t SurfacelessDisplayOutputSurface::GetFramebufferCopyTextureFormat() {
  return buffer_queue_->internal_format();
}

bool SurfacelessDisplayOutputSurface::IsDisplayedAsOverlayPlane() const {
  // TODO(rjkroege): implement remaining overlay functionality.
  return true;
}

unsigned SurfacelessDisplayOutputSurface::GetOverlayTextureId() const {
  return buffer_queue_->GetCurrentTextureId();
}

gfx::BufferFormat SurfacelessDisplayOutputSurface::GetOverlayBufferFormat()
    const {
  DCHECK(buffer_queue_);
  return buffer_queue_->buffer_format();
}

void SurfacelessDisplayOutputSurface::DidReceiveSwapBuffersAck(
