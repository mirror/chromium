...
std::unique_ptr<Display> GpuDisplayProvider::CreateDisplay(
  ...
  NOTIMPLEMENTED();
  gfx::AcceleratedWidget widget = gfx::kNullSurfaceHandle;
#else
  auto widget = reinterpret_cast<gfx::AcceleratedWidget>(surface_handle);
#endif
  ...
  if (!gpu_compositing) {
    ...
    auto device = std::make_unique<SoftwareOutputDeviceOzone>(widget);
#elif defined(USE_X11)
    auto device = std::make_unique<SoftwareOutputDeviceX11>(widget);
#endif
    output_surface = std::make_unique<SoftwareOutputSurface>(std::move(device),
                                                             task_runner_);
  } else {
    auto context_provider = base::MakeRefCounted<InProcessContextProvider>(
        gpu_service_, widget, gpu_memory_buffer_manager_.get(), image_factory_,
...
#if defined(USE_OZONE)
      output_surface = base::MakeUnique<DisplayOutputSurfaceOzone>(
