...
void HostFrameSinkManager::OnAggregatedHitTestRegionListUpdated(
  ...
  const auto hit_test_query = display_hit_test_query_.find(frame_sink_id);
  if (hit_test_query == display_hit_test_query_.end()) {
  ...
  hit_test_query->second->OnAggregatedHitTestRegionListUpdated(
...
void HostFrameSinkManager::SwitchActiveAggregatedHitTestRegionList(
  ...
  hit_test_query->second->SwitchActiveAggregatedHitTestRegionList(
      active_handle_index);
}
