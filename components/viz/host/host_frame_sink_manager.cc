...
void HostFrameSinkManager::CreateCompositorFrameSink(
  ...
  LOG(ERROR) << "CreateCompositorFrameSink " << frame_sink_id;
...
HostFrameSinkManager::CreateCompositorFrameSinkSupport(
  ...
  DCHECK_EQ(frame_sink_data_map_.count(frame_sink_id), 0u);
  LOG(ERROR) << "CreateCompositorFrameSinkSupport " << frame_sink_id
             << ", is_root=" << is_root;
...
void HostFrameSinkManager::DestroyCompositorFrameSink(
    const FrameSinkId& frame_sink_id) {
  LOG(ERROR) << "DestroyCompositorFrameSink " << frame_sink_id;
...
void HostFrameSinkManager::RegisterFrameSinkHierarchy(
  ...
  LOG(ERROR) << "RegisterFrameSinkHierarchy parent=" << parent_frame_sink_id
             << ", child=" << child_frame_sink_id;
...
void HostFrameSinkManager::UnregisterFrameSinkHierarchy(
    const FrameSinkId& parent_frame_sink_id,
    const FrameSinkId& child_frame_sink_id) {
  LOG(ERROR) << "UnregisterFrameSinkHierarchy parent=" << parent_frame_sink_id
             << ", child=" << child_frame_sink_id;
