...
class HostFrameSinkManagerTestBase : public testing::Test {
 public:
  ...
  const HostFrameSinkManager::FrameSinkData& GetFrameSinkData(
      const FrameSinkId& frame_sink_id) {
    DCHECK(FrameSinkDataExists(frame_sink_id));
    return host_manager_->frame_sink_data_map_[frame_sink_id];
  }
...
class HostFrameSinkManagerMojoTest : public HostFrameSinkManagerTestBase {
 public:
  HostFrameSinkManagerMojoTest() = default;
  ~HostFrameSinkManagerMojoTest() override = default;

  // Destroys FrameSinkManagerImpl which kill the message pipes.
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(HostFrameSinkManagerMojoTest);
};
...
TEST_F(HostFrameSinkManagerMojoTest, RestartOnGpuCrash) {
  FakeHostFrameSinkClient host_client;
  ...
  host().CreateCompositorFrameSink(
      ...
      compositor_frame_sink_client.GetInterfacePtr());
  EXPECT_TRUE(
      GetFrameSinkData(kFrameSinkChild1).has_created_compositor_frame_sink);
  ...
  // HostFrameSinkManager should clear out |has_created_compositor_frame_sink|
  // when GPU crashes.
  EXPECT_FALSE(
      GetFrameSinkData(kFrameSinkChild1).has_created_compositor_frame_sink);
