...
TEST_F(HostFrameSinkManagerTest, HierarchyMultipleParents) {
  ...
  // Register |id_parent1| in heirarchy first, this is the original window
  // embedding.
  EXPECT_CALL(impl(), RegisterFrameSinkHierarchy(id_parent1, id_child));
  host().RegisterFrameSinkHierarchy(id_parent1, id_child);

  // Register |id_parent2| in heirarchy second, this is a second embedding for
  ...
  // Unregistering hierarchy with multiple parents works should also work.
