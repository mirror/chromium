...
class MockMojoFrameSinkManager : public cc::mojom::FrameSinkManager {
 public:
  MockMojoFrameSinkManager() : binding_(this) {}
  ~MockMojoFrameSinkManager() override = default;
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(MockMojoFrameSinkManager);
};
...
class FrameSinkManagerHostTest : public testing::Test {
 public:
  ...
  MockMojoFrameSinkManager& manager_mojo() { return *manager_mojo_; }

  // testing::Test:
  void SetUp() override {
    // Create and then connect FrameSinkManagerHost and MojoFrameSinkManager.
    manager_mojo_ = base::MakeUnique<MockMojoFrameSinkManager>();
    ...
    manager_mojo_->BindAndSetClient(std::move(manager_mojo_request),
                                    std::move(host_mojo));
 ...
 private:
  ...
  std::unique_ptr<MockMojoFrameSinkManager> manager_mojo_;
...
TEST_F(FrameSinkManagerHostTest, UnregisterHierarchyOnDestroy) {
  ...
  EXPECT_CALL(manager_mojo(),
              RegisterFrameSinkHierarchy(kFrameSinkId2, kFrameSinkId1));

  // Unregister should be called implicitly when the CompositorFrameSink is
  // destroyed with hierarchy still registered.
  EXPECT_CALL(manager_mojo(),
