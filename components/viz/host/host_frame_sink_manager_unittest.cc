...
class HostFrameSinkManagerTestBase : public testing::Test {
 public:
  ...
  //  void UpdateFrameTokenInDataMap(const FrameSinkId& frame_sink_id, uint32_t
  //  frame_token) {
  //    host_manager_->frame_sink_data_map_.[frame_sink_id]
  //  }
...
TEST_F(HostFrameSinkManagerLocalTest, CommunicateFrameToken) {
  ...
  std::unordered_map<FrameSinkId, std::unique_ptr<CompositorFrameSinkSupport>,
                     FrameSinkIdHash>
      supports;
  testing::NiceMock<MockCompositorFrameSinkClient> support_client;
  FrameSinkManagerImpl frame_sink_manager;
  FakeSurfaceObserver surface_observer(false);
  frame_sink_manager.surface_manager()->AddObserver(&surface_observer);
  frame_sink_manager.SetLocalClient(&host());
  host().RegisterFrameSinkId(kParentFrameSink, &host_client_parent);
  host().RegisterFrameSinkId(kChildFrameSink1, &host_client_child);

  bool kIsRootRoot = true;
  bool kIsChildRoot = false;
  bool kNeedsSyncPoints = true;
  supports[kParentFrameSink] = CompositorFrameSinkSupport::Create(
      &support_client, &frame_sink_manager, kParentFrameSink, kIsRootRoot,
      kNeedsSyncPoints);

  supports[kChildFrameSink1] = CompositorFrameSinkSupport::Create(
      &support_client, &frame_sink_manager, kChildFrameSink1, kIsChildRoot,
      kNeedsSyncPoints);
  ...
  supports[kParentFrameSink]->SubmitCompositorFrame(
      parent_id1.local_surface_id(), std::move(compositor_frame));

  Surface* parent_surface =
      supports[kParentFrameSink]->GetCurrentSurfaceForTesting();
  EXPECT_TRUE(parent_surface->has_deadline());
  EXPECT_FALSE(parent_surface->HasActiveFrame());
  EXPECT_TRUE(parent_surface->HasPendingFrame());
  // Since the frame is not activicated, |frame_token| is not sent to
  // HostFrameSinkClient.
  EXPECT_EQ(0u, host_client_parent.frame_token_seen());

  parent_surface = supports[kParentFrameSink]->GetCurrentSurfaceForTesting();
  parent_surface->ActivatePendingFrameForDeadline();
  EXPECT_FALSE(parent_surface->has_deadline());
  EXPECT_TRUE(parent_surface->HasActiveFrame());
  EXPECT_FALSE(parent_surface->HasPendingFrame());
  // Since the frame is now activicated, |frame_token| is sent to
  // HostFrameSinkClient.
  EXPECT_EQ(10u, host_client_parent.frame_token_seen());

  frame_sink_manager.surface_manager()->RemoveObserver(&surface_observer);
  frame_sink_manager.SetLocalClient(nullptr);
  supports.clear();
