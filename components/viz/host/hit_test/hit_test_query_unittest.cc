...
class HitTestQueryTest : public testing::Test {
 ...
 private:
  // testing::Test:
  void SetUp() override {
    uint32_t active_handle_size = 100;
    size_t num_bytes = active_handle_size * sizeof(AggregatedHitTestRegion);
    mojo::ScopedSharedBufferHandle active_handle =
        mojo::SharedBufferHandle::Create(num_bytes);
    active_buffer_ = active_handle->Map(num_bytes);
    hit_test_query_.OnSharedMemoryHandlesReceived(
        std::move(active_handle), active_handle_size,
