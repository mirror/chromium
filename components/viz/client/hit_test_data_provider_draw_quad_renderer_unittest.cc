...
TEST(HitTestDataProviderDrawQuadRenderer, HitTestDataRenderer) {
  std::unique_ptr<HitTestDataProvider> hit_test_data_provider =
      std::make_unique<HitTestDataProviderDrawQuadRenderer>();
  ...
  EXPECT_EQ(mojom::kHitTestChildSurface | mojom::kHitTestAsk,
