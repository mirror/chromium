...
struct VIZ_COMMON_EXPORT RemotableResource {
  ...
  RemotableResource(const gfx::Size& size,
                    Origin origin,
  ...
  RemotableResourceTextureHint hint = RemotableResourceTextureHint::kImmutable;
  // The type of backing for the resource (such as gpu vs software).
  RemotableResourceType type = RemotableResourceType::kBitmap;
 ...
 private:
  ...
  // A SyncToken to wait on for synchronizing a texture-backed or
  // GpuMemoryBuffer-backed resource with changes to the resource made on
  // another GL context.
