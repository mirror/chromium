...
struct VIZ_COMMON_EXPORT RemotableResource {
  ...
  // If true the texture-backed or GpuMemoryBuffer-backed resource has a
  // SyncToken to wait for.
  bool ShouldWaitSyncToken() const;
  // The caller must wait on the returned SyncToken. This updates the resource
  // to track that the token was waited on.
  const gpu::SyncToken& WillWaitSyncToken();
