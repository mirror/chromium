...
TEST_F(ExternalBeginFrameSourceTest, OnSideEffectsOnlyBeginFrameOptIn) {
  EXPECT_BEGIN_FRAME_SOURCE_PAUSED(*obs_, false);
  EXPECT_CALL((*client_), OnNeedsBeginFrames(true)).Times(1);
  source_->AddObserver(obs_.get());

  // By default, an observer doesn't receive side-effects-only BeginFrames.
  BeginFrameArgs args = CreateBeginFrameArgsForTesting(
      BEGINFRAME_FROM_HERE, 0, 2, TicksFromMicroseconds(10000));
  args.side_effects_only = true;
  source_->OnBeginFrame(args);

  // When opting in, an observer receives side-effects-only BeginFrames.
  args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 2,
                                        TicksFromMicroseconds(10000));
  args.side_effects_only = true;
  EXPECT_CALL(*obs_, WantsSideEffectsOnlyBeginFrames())
