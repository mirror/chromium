...
class VIZ_COMMON_EXPORT BeginFrameSource {
 public:
  // This should be used for BeginFrameSources that don't have to worry about
  // process restart. If a BeginFrameSource won't generate and forward
  // BeginFrameArgs to another process, or is in a process that won't crash,
  // then this constant is appropriate to use.
  static constexpr uint32_t kNoRestartProcessRestartId = 0;
  ...
  // should be incremented. This ensures that |source_id_| is still unique after
  // process restart.
  explicit BeginFrameSource(uint32_t process_restart_id);
...
class VIZ_COMMON_EXPORT SyntheticBeginFrameSource : public BeginFrameSource {
 public:
  explicit SyntheticBeginFrameSource(uint32_t process_restart_id);
