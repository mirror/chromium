...
// side-effects-only BeginFrame, the observer may not be notified of the
// BeginFrame.
void FilterAndIssueBeginFrame(BeginFrameObserver* observer,
                              const BeginFrameArgs& args) {
  if (args.side_effects_only && !observer->WantsSideEffectsOnlyBeginFrames())
    return;
...
bool BeginFrameObserverBase::WantsSideEffectsOnlyBeginFrames() const {
