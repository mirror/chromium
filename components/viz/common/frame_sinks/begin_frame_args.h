...
struct VIZ_COMMON_EXPORT BeginFrameArgs {
  ...
  // CompositorFrame, but instead only run the side effects of the BeginFrame,
  // such as updating animations and layout. Such a BeginFrame effectively
  // advances an observer's view of frame time, which in turn may trigger side
  // effects such as loading of new resources.
