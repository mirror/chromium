...
TextureAllocation TextureAllocation::Allocate(
  ...
  if (overlay_textures) {
    texture_target = gpu::GetBufferTextureTarget(
        gfx::BufferUsage::GPU_READ_CPU_READ_WRITE, BufferFormat(format), caps);
  ...
  if (overlay_textures) {
    gl->TexStorage2DImageCHROMIUM(texture_target, TextureStorageFormat(format),
                                  GL_SCANOUT_CHROMIUM, size.width(),
                                  size.height());
    if (color_space.IsValid()) {
      gl->SetColorSpaceMetadataCHROMIUM(
          texture_id, reinterpret_cast<GLColorSpace>(
                          const_cast<gfx::ColorSpace*>(&color_space)));
    }
  } else if (caps.texture_storage) {
    gl->TexStorage2DEXT(texture_target, 1, TextureStorageFormat(format),
                        size.width(), size.height());
  } else {
    gl->TexImage2D(texture_target, 0, GLInternalFormat(format), size.width(),
