 ...
 public:
  ...
  class ScalerInterface {
   public:
    ...
    // This method must be called before Scale(), for shaders that perform color
    // transforms.
    virtual void SetColorWeights(int plane, const GLfloat color_weights[4]) = 0;
    ...
    void Scale(GLuint src_texture,
      ...
      ScaleToMultipleOutputs(src_texture, src_texture_size,
                             (GLuint[2]){dest_texture, 0u}, output_rect);
    }

    // Same as above, but for shaders that output to multiple textures.
    virtual void ScaleToMultipleOutputs(GLuint src_texture,
                                        const gfx::Size& src_texture_size,
                                        const GLuint dest_textures[2],
...
class ReadbackYUVInterface {
 public:
  ...
  // Returns true if this Readback YUV scales input by the given ratio. That
  // means, if SetScaler() was never called (or was set to null), then this
  // returns true iff from == to.
  virtual bool ScalesBy(const gfx::Vector2d& from,
                        const gfx::Vector2d& to) const = 0;
};
