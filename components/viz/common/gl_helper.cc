...
class I420ConverterImpl : public I420Converter {
 ...
 protected:
  ...
  void EnsureTexturesSizedFor(const gfx::Size& output_size,
                              ...
                              gfx::Size* chroma_texture_size,
                              bool intermediate_texture_needed);
...
void I420ConverterImpl::Convert(GLuint src_texture,
  ...
  gfx::Size chroma_plane_size;
  EnsureTexturesSizedFor(output_rect.size(), y_plane_texture, u_plane_texture,
                         v_plane_texture, &y_plane_size, &chroma_plane_size,
                         !!optional_scaler);
  ...
  if (use_mrt()) {
    y_planerizer_->ScaleToMultipleOutputs(texture, texture_size,
                                          ...
                                          gfx::Rect(y_plane_size));
    u_planerizer_->ScaleToMultipleOutputs(uv_->id(), y_plane_size,
                                          u_plane_texture, v_plane_texture,
                                          gfx::Rect(chroma_plane_size));
  } else {
...
void I420ConverterImpl::EnsureTexturesSizedFor(
  ...
  if (intermediate_texture_needed) {
  ...
  *y_texture_size = GetYPlaneTextureSize(output_size);
  if (use_mrt()) {
    uv_.emplace(gl_);
  ...
  SetRGBATextureSize(*y_texture_size);
  // Chroma planes are half as long in both dimensions.
  *chroma_texture_size = GetChromaPlaneTextureSize(output_size);
