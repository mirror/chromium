 ...
 private:
  ...
  class ReadbackYUVImpl : public ReadbackYUVInterface {
   public:
    ...
    bool ScalesBy(const gfx::Vector2d& from,
...
void GLHelper::CopyTextureToImpl::ReadbackYUVImpl::ReadbackYUV(
  ...
  if (use_mrt()) {
    y_planerizer_->ScaleToMultipleOutputs(texture, texture_size,
                                          (GLuint[2]){y_->texture(), uv_->id()},
                                          gfx::Rect(y_->size()));
    u_planerizer_->ScaleToMultipleOutputs(
        uv_->id(), y_->size(), (GLuint[2]){u_->texture(), v_->texture()},
        gfx::Rect(u_->size()));
  } else {
...
bool GLHelper::CopyTextureToImpl::ReadbackYUVImpl::ScalesBy(
    const gfx::Vector2d& from,
    const gfx::Vector2d& to) const {
  if (scaler_)
    return scaler_->IsSameScaleRatio(from, to);
  else
    return from == to;
}
