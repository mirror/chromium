...
class VIZ_COMMON_EXPORT CopyOutputResult {
 public:
  enum class Format : uint8_t {
    ...
    RGBA_TEXTURE,
    // Three planes (Y, U, and V) in I420 format (Rec. 709 color space). This
    // format only used internally within the VIZ component and does not travel
    // across mojo interfaces.
    I420_PLANES,
  };
  ...
  // coordinate space. For the I420_PLANES result format, all corners of the
  // rect will be an even number (or zero).
  ...
  // Returns a pointer to the Y/U/V plane data and stride. The result format
  // must be I420_PLANES.
  virtual const uint8_t* GetYPlane() const;
  virtual const uint8_t* GetUPlane() const;
  virtual const uint8_t* GetVPlane() const;
  virtual size_t GetYPlaneBytesPerRow() const;
  virtual size_t GetUPlaneBytesPerRow() const;
  virtual size_t GetVPlaneBytesPerRow() const;

 protected:
  // Called zero or once (on-demand) to populate the SkBitmap returned by
  // AsSkBitmap(). Subclasses that support conversion to SkBitmap override this.
  virtual void PopulateBitmap(SkBitmap* bitmap) const;

 private:
  const Format format_;
  const gfx::Rect rect_;

  // Initialized on-demand by AsSkBitmap().
  mutable std::unique_ptr<SkBitmap> lazy_bitmap_;
...
class VIZ_COMMON_EXPORT CopyOutputSkBitmapResult : public CopyOutputResult {
 ...
 private:
  void PopulateBitmap(SkBitmap* bitmap) const override;
