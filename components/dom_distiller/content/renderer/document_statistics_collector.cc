...
bool IsVisible(const HTMLElement& element) {
  const CSSStyleDeclaration style = element.getComputedStyle();
  // offsetWidth is null if element or any ancestor has display:none.
...
void CollectFeatures(const Element& root,
  ...
  for (Node node : NodeIterator::ChildrenOf(root)) {
    ...
    const HTMLElement element = node.ToHTMLElement();
    // TODO: HTMLNames
