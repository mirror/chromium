...
void SetSecurityLevelAndRelatedFieldsForNonSecureFieldTrial(
  ...
  bool feature_enabled =
      base::FeatureList::IsEnabled(features::kMarkHttpAsFeature);
  std::string parameter = feature_enabled
                              ? base::GetFieldTrialParamValueByFeature(
                                    features::kMarkHttpAsFeature,
                                    features::kMarkHttpAsFeatureParameterName)
                              : std::string();

  if (feature_enabled && parameter == features::kMarkHttpAsParameterDangerous) {
    security_info->security_level = DANGEROUS;
    return;
  }

  if (feature_enabled && parameter == features::kMarkHttpAsParameterWarning) {
  ...
  if (feature_enabled &&
    ...
    // Do not set |fiedl_edit_downgraded_security_level| here because that field
    // is for specifically for when the security level was downgraded from NONE
    // to HTTP_SHOW_WARNING, not from HTTP_SHOW_WARNING to DANGEROUS as in this
    // case.
    return;
  }
