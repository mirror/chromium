...
class ChangeListProcessorTest : public testing::Test {
 protected:
  ...
  // Applies the |changes| of the default corpus to |metadata_| as a delta
  // update. Delta changelists should contain their changestamp in themselves.
  FileError ApplyUserChangeList(
      std::vector<std::unique_ptr<ChangeList>> changes,
      FileChange* changed_files) {
    return ApplyChangeList(std::string(), std::move(changes), changed_files);
  }
  ...
  // Team Drive's ID which is giving the change, or an empty string if the
  // |changes| is the user's changelist.
  FileError ApplyChangeList(const std::string& team_drive_id,
                            std::vector<std::unique_ptr<ChangeList>> changes,
                            FileChange* changed_files) {
    std::unique_ptr<google_apis::AboutResource> about_resource(
        new google_apis::AboutResource);
    ...
    ChangeListProcessor processor(metadata_.get(), nullptr);
    FileError error =
        processor.Apply(std::move(about_resource), team_drive_id,
...
TEST_F(ChangeListProcessorTest, DeltaFileAddedInNewDirectoryInTeamDrive) {
  std::vector<std::unique_ptr<ChangeList>> change_lists;
  change_lists.push_back(base::MakeUnique<ChangeList>());
  ...
  EXPECT_EQ(
      FILE_ERROR_OK,
      ApplyChangeList(kTeamDriveId, std::move(change_lists), &changed_files));
