...
const int64_t kBaseResourceListChangestampForTeamDrive = 12;
const char kRootId[] = "fake_root";
const char kTeamDriveId[] = "theTeamDriveId";
const char kEmptyTeamDriveId[] = "";
...
TEST_F(ChangeListProcessorTest, DeltaFileAddedInNewDirectory) {
  ...
  EXPECT_EQ(FILE_ERROR_OK,
            ApplyChangeList(kEmptyTeamDriveId, std::move(change_lists),
                            &changed_files));
...
TEST_F(ChangeListProcessorTest, DeltaFileAddedInNewDirectoryInTeamDrive) {
  ...
  new_folder.set_title("New Directory");
  new_folder.mutable_file_info()->set_is_directory(true);
  new_folder.set_parent_local_id(team_drive_root->local_id());
  change_lists[0]->mutable_entries()->push_back(new_folder);
  ...
  std::unique_ptr<ResourceEntry> result_team_drive_root =
      GetResourceEntry("drive/team_drives");
