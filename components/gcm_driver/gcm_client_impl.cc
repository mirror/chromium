...
void GCMClientImpl::OnRegisterCompleted(
  ...
  // If there was a cached token already, then verify that the new token
  // received is the same. (registration_id gets stored as the token).
  auto registrations_iter = registrations_.find(registration_info);
  if (registrations_iter != registrations_.end()) {
    DCHECK(registrations_iter->second == registration_id);
  }
