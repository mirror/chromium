...
std::string ToGCMDecryptionResultDetailsString(GCMDecryptionResult result) {
  switch (result) {
    ...
    case GCMDecryptionResult::INVALID_BINARY_HEADER_PAYLOAD_LENGTH:
      return "The payload is smaller than the smallest valid message (104 "
             "bytes)";
    case GCMDecryptionResult::INVALID_BINARY_HEADER_RECORD_SIZE:
      return "The record size is smaller than the smallest valid record size "
             "(18 bytes)";
    case GCMDecryptionResult::INVALID_BINARY_HEADER_PUBLIC_KEY_LENGTH:
      return "The public key must be a valid P-256 ECDH uncompressed point "
             "that is 65 bytes in length.";
    case GCMDecryptionResult::INVALID_BINARY_HEADER_PUBLIC_KEY_FORMAT:
      return "The public key must be a valid P-256 ECDH uncompressed poin that "
