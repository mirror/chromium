...
std::string InstanceIDTokenInfo::GetSerializedKey() const {
  ...
  std::string key = base::StringPrintf(
      "%s%s%c%s%c%s", kInstanceIDSerializationPrefix, app_id.c_str(),
      kSerializedKeySeparator, authorized_entity.c_str(),
      kSerializedKeySeparator, scope.c_str());
  return key;
}
...
bool ExistsGCMRegistrationInMap(const RegistrationInfoMap& map,
                                ...
                                bool* registration_is_fresh) {
  std::unique_ptr<GCMRegistrationInfo> gcm_registration(
      new GCMRegistrationInfo);
  gcm_registration->app_id = app_id;

  auto iter =
      map.find(make_linked_ptr<RegistrationInfo>(gcm_registration.release()));
  bool reg_exists = (iter != map.end());
  if (!registration_is_fresh)
    return reg_exists;
  if (iter->first.get()) {
    *registration_is_fresh =
        (base::Time::Now() - iter->first.get()->last_validated <
         base::TimeDelta::FromDays(kTokenInvalidationDurationDays));
  } else {
    *registration_is_fresh = false;
  }
  return reg_exists;
}
