...
bool GCMRegistrationInfo::operator==(const RegistrationInfo& compare_to) {
  const GCMRegistrationInfo* gcm_reg_info = FromRegistrationInfo(&compare_to);
  DCHECK(gcm_reg_info);
  return ((app_id == gcm_reg_info->app_id) &&
          (sender_ids == gcm_reg_info->sender_ids));
}
...
std::string GCMRegistrationInfo::GetSerializedValue(
  ...
  // Serialize as:
  // sender1,sender2,...=
  // reg_id<kSerializedValidationTimeSeparator>time_of_last_validation
  std::string value;
  ...
  value += kSerializedValidationTimeSeparator;
...
bool InstanceIDTokenInfo::operator==(const RegistrationInfo& compare_to) {
  const InstanceIDTokenInfo* iid_token_info = FromRegistrationInfo(&compare_to);
  DCHECK(iid_token_info);
  return ((app_id == iid_token_info->app_id) &&
          (authorized_entity == iid_token_info->authorized_entity) &&
          (scope == iid_token_info->scope) &&
          (options == iid_token_info->options));
}
