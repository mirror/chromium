...
bool GCMRegistrationInfo::Deserialize(const std::string& serialized_key,
  ...
  if (pos_equals == std::string::npos)
    return false;
  // Note that it's valid for pos_hash to be std::string::npos. The portion
  // after the # was included with the fix for Issue 799483.
...
void FindMatchInRegistrationInfoMapAndCheckIfStale(
    const RegistrationInfoMap& map,
    const RegistrationInfo& to_search,
    bool* app_id_matches,
    bool* sender_ids_match_or_are_irrelevant,
    bool* token_is_stale) {
  if (app_id_matches)
    app_id_matches = false;
  if (sender_ids_match)
    sender_ids_match = false;
  if (token_is_stale)
    token_is_stale = true;
  if (!app_id_matches && !sender_ids_match && !token_is_stale)
    return;
  for (auto iter = map.begin(); iter != map.end(); ++iter) {
    if (iter->first.get()->app_id != to_search.app_id)
      return;

    if (app_id_matches)
      *app_id_matches = true;
    // If this is a GCM registration, we also match the sender IDs since
    // multiple registrations are not supported.
    const GCMRegistrationInfo* to_search_gcm_registration_info =
        GCMRegistrationInfo::FromRegistrationInfo(to_search.get());
    if (to_search_gcm_registration_info) {
      const GCMRegistrationInfo* map_gcm_registration_info =
          GCMRegistrationInfo::FromRegistrationInfo(iter->first.get());
      DCHECK(map_gcm_registration_info);
      if (to_search_gcm_registration_info->sender_ids ==
          map_gcm_registration_info->sender_ids) {
        *sender_ids_match_or_are_irrelevant = true;
        // Check if Token is fresh.
        auto last_validated_at = iter->first.get()->last_validated;
      ...
      // This is not a GCM registration.
      *sender_ids_match_or_are_irrelevant = true;
      return;
    }
  }
