...
TEST_F(GCMClientImplTest, SerializeAndDeserialize) {
  ...
  LOG(ERROR) << "serialized_key is: " << serialized_key
             << " and serialized_value is: " << serialized_value;
  std::unique_ptr<GCMRegistrationInfo> gcm_info_rehydrated(
      new GCMRegistrationInfo);
  std::string registration_id_rehydrated;
  EXPECT_TRUE(gcm_info_rehydrated->Deserialize(serialized_key, serialized_value,
                                               &registration_id_rehydrated));

  EXPECT_EQ(gcm_info->app_id, gcm_info_rehydrated->app_id);
  EXPECT_EQ(gcm_info->sender_ids, gcm_info_rehydrated->sender_ids);
  EXPECT_EQ(gcm_info->sender_ids, gcm_info_rehydrated->sender_ids);
  EXPECT_EQ(gcm_info->last_validated, gcm_info_rehydrated->last_validated);
  EXPECT_EQ(kRegistrationId, registration_id_rehydrated);
}
