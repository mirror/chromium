...
BasePredictor::BasePredictor(const PredictorConfig& config) : config_(config) {
  ...
  if (config_.field_trial) {
    ...
    LOG(ERROR) << "No field trial specified";
  }
}
...
bool BasePredictor::IsQueryEnabled() {
...
void BasePredictor::LogFeatureToUkm(const std::string& feature_name,
  ...
  if (config_.feature_whitelist->count(feature_name) == 0) {
    VLOG(0) << "Feature not whitelisted: " << feature_name;
    return;
  }
...
void BasePredictor::LogExampleToUkm(const RankerExample& example,
  ...
  if (config_.log_type != LOG_UKM) {
    LOG(ERROR) << "Wrong log type in predictor config: " << config_.log_type;
    return;
  }
  ...
  if (config_.feature_whitelist->empty()) {
    VLOG(0) << "Empty whitelist, examples will not be logged.";
    return;
  }
  ...
  if (builder) {
    ...
    VLOG(0) << "Could not get UKM Entry Builder.";
  }
  // Releasing the builder triggers logging.
  builder.reset();
}
