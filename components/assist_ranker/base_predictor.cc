...
namespace {
const char kTargetName[] = "_target";
}  // namespace
...
void BasePredictor::LogExampleToUkm(const RankerExample& example,
  ...
  std::unique_ptr<ukm::UkmEntryBuilder> builder =
      ukm_recorder->GetEntryBuilder(source_id, GetModelName().c_str());
  for (const auto& feature_kv : example.features()) {
    LogFeatureToUkm(feature_kv.first, feature_kv.second, builder.get());
  }
  if (example.has_target()) {
    LogFeatureToUkm(kTargetName, example.target(), builder.get());
  } else {
    // FIXME LOG error.
  }
}
