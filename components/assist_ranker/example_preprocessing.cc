...
constexpr char ExamplePreprocessor::kMissingFeatureDefaultName[];
constexpr char ExamplePreprocessor::kVectorizedFeatureDefaultName[];

std::string ExamplePreprocessor::FeatureValueToKey(
  ...
  for (const MapPair<std::string, ExamplePreprocessorConfig::Boundaries>&
    ...
    switch (feature.feature_type_case()) {
      ...
      case Feature::kFloatValue:
        ...
        break;
      default:
        LOG(WARNING) << "Can't bucketize feature type: "
...
int ExamplePreprocessor::Vectorization(RankerExample* example,
  ...
  if (config_.feature_indices().empty()) {
    LOG(WARNING) << "Feature indices are empty, can't vectorize.";
    return kSuccess;
  }
  ...
  for (const auto& field : ExampleFloatIterator(*example)) {
    ...
    if (find_index != config_.feature_indices().end()) {
      ...
      LOG(WARNING) << "Feature has no index: " << field.fullname;
...
ExampleFloatIterator::Field ExampleFloatIterator::operator*() const {
  ...
  switch (feature.feature_type_case()) {
    ...
    case Feature::kStringValue:
      field.fullname = ExamplePreprocessor::FeatureValueToKey(
    ...
    case Feature::kStringList:
      if (string_list_index_ < feature.string_list().string_value_size()) {
        ...
        field.fullname =
            ExamplePreprocessor::FeatureValueToKey(feature_name, string_value);
      } else {
    ...
    default:
      ...
      LOG(WARNING) << "Feature type not supported:  "
                   << feature.feature_type_case();
      break;
  }
  return field;
