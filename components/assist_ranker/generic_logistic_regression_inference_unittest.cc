...
class GenericLogisticRegressionInferenceTest : public testing::Test {
 protected:
  ...
  const float sparse_elem1_weight_ = -2.2;
  const float sparse_elem2_weight_ = 3.1;
  const float sparse_default_weight_ = 4.4;

  const std::string bucketized_name_ = "bucketized_feature";
  const float bucketization_boundaries_[2] = {0.3, 0.7};
  const float bucketization_weights_[3] = {-1.0, 1.0, 3.0};
  const float bucketization_default_weight_ = -3.3;
};
...
TEST_F(GenericLogisticRegressionInferenceTest, PreprossessedModel) {
  ...
  weights[ExamplePreprocessor::FeatureFullname(
      one_hot_name_, one_hot_elem1_name_)] = elem1_weight_;
  weights[ExamplePreprocessor::FeatureFullname(
      one_hot_name_, one_hot_elem2_name_)] = elem2_weight_;
  weights[ExamplePreprocessor::FeatureFullname(
      one_hot_name_, one_hot_elem3_name_)] = elem3_weight_;
  weights[ExamplePreprocessor::FeatureFullname(
  ...
  features[scalar1_name_].set_bool_value(true);
  features[scalar2_name_].set_int32_value(42);
  features[scalar3_name_].set_float_value(0.666);
  features[one_hot_name_].set_string_value(one_hot_elem1_name_);
  features[sparse_name_].mutable_string_list()->add_string_value(
      sparse_elem1_name_);
  features[sparse_name_].mutable_string_list()->add_string_value(
      sparse_elem2_name_);
  features[bucketized_name_].set_float_value(0.98);

  // Inference.
  float score = predictor.PredictScore(example);
  float expected_score =
      Sigmoid(bias_ + 1.0 * scalar1_weight_ + 42.0 * scalar2_weight_ +
              0.666 * scalar3_weight_ + elem1_weight_ + sparse_elem1_weight_ +
