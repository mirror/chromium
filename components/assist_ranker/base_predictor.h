...
class BasePredictor : public base::SupportsWeakPtr<BasePredictor> {
 public:
  ...
  // Logs the |example| using the logging type defined in the predictor config.
  // If logging to UKM, a |page_url| must be provided.
  void LogExample(const RankerExample& example, const GURL& page_url = GURL());
