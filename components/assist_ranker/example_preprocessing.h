// This file contains Preprocessors for preprocessing RankerExample into formats
// (RankerExample, std::vector<float>, etc.) needed by Ranker Predictors.
...
class ExamplePreprocessor {
 public:
  ...
  static constexpr char kMissingFeatureDefaultName[] = "_MissingFeature";

  // Default feature name for vectorized features.
  static constexpr char kVectorizedFeatureDefaultName[] = "_VectorizedFeature";

  // This function is used to generate a key from feature_name and feature_value
  // A user should call this function instead of depending on the implementation
  // bool_value, int64_value and float_value should use
  //   FeatureValueToKey(feature_name)
  // string_value, string_list should use
  //   FeatureValueToKey(feature_name, feature_value)
  // Bucketized Feature should use
  //   FeatureValueToKey(feature_name, string_form_of_the_bucket_index);
  static std::string FeatureValueToKey(const std::string& feature_name,
