...
TEST_F(ExamplePreprocessorTest, Vectorization) {
  ...
  feature_indices[ExamplePreprocessor::FeatureValueToKey(one_hot_name_,
                                                         one_hot_value_)] = 3;
  feature_vector.Add(1.0);

  // string list value is vectorized as 1.0.
  feature_indices[ExamplePreprocessor::FeatureValueToKey(sparse_name_,
                                                         elem1_)] = 4;
  feature_indices[ExamplePreprocessor::FeatureValueToKey(sparse_name_,
                                                         elem2_)] = 5;
  feature_vector.Add(1.0);
  ...
  // place as 0.0;
  feature_indices[ExamplePreprocessor::FeatureValueToKey(sparse_name_, "foo")] =
      5;
...
TEST_F(ExamplePreprocessorTest, MultipleErrorCode) {
  ...
  // Error code contains eatures in example_ but not in feature_indices.
...
TEST_F(ExamplePreprocessorTest, ExampleFloatIterator) {
  ...
  feature_map[ExamplePreprocessor::FeatureValueToKey(one_hot_name_,
                                                     one_hot_value_)]
      .set_float_value(1.0);
  feature_map[ExamplePreprocessor::FeatureValueToKey(sparse_name_, elem1_)]
      .set_float_value(1.0);
  feature_map[ExamplePreprocessor::FeatureValueToKey(sparse_name_, elem2_)]
      .set_float_value(1.0);
