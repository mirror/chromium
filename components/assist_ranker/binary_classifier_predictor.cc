...
std::unique_ptr<BinaryClassifierPredictor> BinaryClassifierPredictor::Create(
  ...
  if (predictor->IsQueryEnabled()) {
    const std::string model_url = predictor->GetModelUrl();
    VLOG(0) << "Creating predictor instance for " << predictor->GetModelName();
    VLOG(0) << "Model URL: " << model_url;
    auto model_loader = base::MakeUnique<RankerModelLoaderImpl>(
        base::BindOnce(&BinaryClassifierPredictor::ValidateModel),
        base::BindOnce(&BinaryClassifierPredictor::OnModelAvailable,
    ...
    VLOG(0) << "Query disabled, bypassing model loading.";
  }
  return predictor;
}
...
bool BinaryClassifierPredictor::Predict(const RankerExample& example,
                                        bool* prediction) {
  if (!IsReady()) {
    VLOG(0) << "Predictor " << GetModelName() << " not ready for prediction.";
    return false;
  }

  *prediction = inference_module_->Predict(PreprocessExample(example));
  VLOG(0) << "Predictor " << GetModelName() << " predicted: " << *prediction;
  return true;
}
...
bool BinaryClassifierPredictor::Initialize() {
  if (ranker_model_->proto().model_case() ==
    ...
    LOG(ERROR) << "Could not initialize inference module.";
    return false;
  }
}
