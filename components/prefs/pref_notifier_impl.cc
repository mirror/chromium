...
void PrefNotifierImpl::AddAnyPrefObserver(PrefObserver* observer) {
  DCHECK(thread_checker_.CalledOnValidThread());
  any_pref_observers_.AddObserver(observer);
}

void PrefNotifierImpl::RemoveAnyPrefObserver(PrefObserver* observer) {
  DCHECK(thread_checker_.CalledOnValidThread());
  any_pref_observers_.RemoveObserver(observer);
}
...
void PrefNotifierImpl::FireObservers(const std::string& path) {
  ...
  for (auto& observer : any_pref_observers_)
