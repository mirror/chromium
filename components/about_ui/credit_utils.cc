...
void GetCredits(std::string& response, bool isInline) {
  ...
  if (!isInline) {
    // If not inline, add necessary JS files.
    response +=
...
static base::android::ScopedJavaLocalRef<jstring> GetJavaWrapperCredits(
    JNIEnv* env,
    const base::android::JavaParamRef<jclass>& clazz) {
  std::string html_content;
  GetCredits(html_content, true);
  return base::android::ConvertUTF8ToJavaString(env, html_content);
}
