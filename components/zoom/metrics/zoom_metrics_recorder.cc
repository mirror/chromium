...
ZoomMetricsRecorder::ZoomMetricsRecorder(content::WebContents* web_contents)
    : content::WebContentsObserver(web_contents),
      ...
      skip_delays_for_testing_(false) {}
...
void ZoomMetricsRecorder::OnUserZoom(float zoom_factor,
  ...
  request_timer_.Start(
      FROM_HERE,
      (skip_delays_for_testing_ ? base::TimeDelta() : kStyleRequestDelay),
      base::BindRepeating(&ZoomMetricsRecorder::RequestStyleInfo,
...
void ZoomMetricsRecorder::RequestStyleInfo() {
  ...
  for (content::RenderFrameHost* frame : web_contents()->GetAllFrames()) {
    ...
    if (skip_delays_for_testing_) {
      GetStyleCollector(frame)->GetStyleImmediatelyForTesting(
          base::BindOnce(&ZoomMetricsRecorder::OnReceivedStyleFromFrame,
...
void ZoomMetricsRecorder::OnReceivedStyleFromFrame(
  ...
  if (pending_requests_ == 0) {
    ...
    if (wait_for_collection_done_for_testing_) {
      std::move(wait_for_collection_done_for_testing_).Run();
      wait_for_collection_done_for_testing_.Reset();
    }
...
ZoomMetricsRecorder::GetStyleCollector(content::RenderFrameHost* frame) {
  ...
  insertion.first->second.set_connection_error_handler(base::BindOnce(
      &ZoomMetricsRecorder::OnConnectionError, base::Unretained(this), frame));
  return insertion.first->second;
}

void ZoomMetricsRecorder::SkipDelaysForTesting() {
  skip_delays_for_testing_ = true;
}

void ZoomMetricsRecorder::CollectionDoneNotificationForTesting(
    base::OnceClosure done) {
  wait_for_collection_done_for_testing_ = std::move(done);
}
