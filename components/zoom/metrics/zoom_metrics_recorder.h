...
class ZoomMetricsRecorder : public content::WebContentsObserver {
 public:
  ...
  // |zoom_factor| is the zoom factor set by the user.
  // |browser_handled| is whether the zoom is performed by the browser
  // or by an extension.
  void OnUserZoom(double zoom_factor, bool browser_handled);
 ...
 private:
  // Send requests for style information to the renderer(s) for the page.
  void RequestStyleInfo();

  // The response from the renderer(s).
