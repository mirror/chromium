...
size_t FindSubdomainAnchoredSubpattern(base::StringPiece url,
  ...
  // Try to avoid some worst case runtime behavior here:
  size_t host_begin = static_cast<size_t>(host.begin);
  for (size_t position = host_begin;
       position <= static_cast<size_t>(host.end()); ++position) {
    // Never call find if |position| isn't already anchored at a sub-domain.
    // This avoids us potentially finding a match that is between |position| and
    // the next sub-domain.
