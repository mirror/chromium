...
const flat::UrlRule* UrlPatternIndexMatcher::FindMatch(
  ...
  if (rule) {
    std::ostringstream rule_str;
    rule_str << UrlPattern(*rule);
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("url_pattern_index"),
                 "UrlPatternIndexMatcher::FindMatch", "matching_rule",
                 rule_str.str());
  }
  return rule;
}
