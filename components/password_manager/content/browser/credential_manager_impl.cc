...
void RunMojoGetCallback(
    password_manager::mojom::CredentialManager::GetCallback callback,
    const CredentialInfo& info) {
  std::move(callback).Run(
      password_manager::mojom::CredentialManagerError::SUCCESS, info);
}
...
void CredentialManagerImpl::Get(CredentialMediationRequirement mediation,
  ...
  if (pending_request_ || !store) {
    ...
    std::move(callback).Run(
        pending_request_
            ? password_manager::mojom::CredentialManagerError::PENDINGREQUEST
  ...
  if (!client_->IsFillingEnabledForCurrentPage() ||
      !client_->OnCredentialManagerUsed()) {
    std::move(callback).Run(
        password_manager::mojom::CredentialManagerError::SUCCESS,
        CredentialInfo());
