...
TEST_F(HashPasswordManagerTest, Saving) {
  ...
  base::string16 password(base::ASCIIToUTF16("sync_password"));
  ...
  SyncPasswordData sync_password_data(password, /*force_update=*/true);
  EXPECT_TRUE(sync_password_data.MatchesPassword(password));
