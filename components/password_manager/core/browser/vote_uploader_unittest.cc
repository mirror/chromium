...
class MockAutofillManager : public autofill::AutofillManager {
 public:
  ...
  using set_download_manager;

  // Workaround for std::unique_ptr<> lacking a copy constructor.
  void StartUploadProcess(std::unique_ptr<FormStructure> form_structure,
                          const base::TimeTicks& timestamp,
                          bool observed_submission) {
    StartUploadProcessPtr(form_structure.get(), timestamp, observed_submission);
  }
...
class TestPasswordManagerClient : public StubPasswordManagerClient {
 public:
  TestPasswordManagerClient()
    ...
    auto download_manager = base::MakeUnique<MockAutofillDownloadManager>(
        &test_autofill_driver_, &mock_autofill_manager_);
    mock_autofill_download_manager_ = download_manager.get();
    mock_autofill_manager_.set_download_manager(std::move(download_manager));
  }
 ...
 private:
  MockAutofillDownloadManager* mock_autofill_download_manager_ = nullptr;
...
class VoteUploaderTest : public testing::Test {
 protected:
  // Sets up the expectation that no votes of any type are uploaded.
  void ExpectNoVote() {
    EXPECT_CALL(client_->mock_autofill_download_manager(),
                StartUploadRequest(_, _, _, _, _))
        .Times(0);
    EXPECT_CALL(client_->mock_autofill_manager(),
                StartUploadProcessPtr(_, _, _))
        .Times(0);
  }

  // Sets up the expectation that StartUploadProcess is called. It binds
  // |target_form_structure| to receive the FormStructure passed to the upload
  // process.
  void ExpectStartUploadProcessPtr(
      std::unique_ptr<FormStructure>* target_form_structure) {
    EXPECT_CALL(client_->mock_autofill_manager(),
                StartUploadProcessPtr(_, _, true))
        .WillOnce(WithArg<0>(SaveToUniquePtr(&target_form_structure)));
  }

  // Sets up the expectation that StartUploadRequest is called, with
  // |expected_field_types| and |expected_form_signature|.
  void ExpectStartUploadRequest(const ServerFieldTypeSet& expected_field_types,
                                const std::string& expected_form_signature, ) {
    EXPECT_CALL(client_->mock_autofill_download_manager(),
                StartUploadRequest(_, false, expected_field_types,
                                   expected_form_signature, true));
  }

  TestPasswordManagerClient client_;
};

TEST_F(VoteUploaderTest, UploadPasswordVote) {}
...
#if 0
TEST_F(VoteUploaderTest, UploadPasswordForm) {
...
TEST_F(VoteUploaderTest, UploadFormData_NewPassword) {
...
TEST_F(VoteUploaderTest, UploadFormData_NewPassword_Blacklist) {
...
TEST_F(VoteUploaderTest, UploadChangePasswordForm) {
...
TEST_F(VoteUploaderTest, GeneratedVoteUpload) {
  bool kFalseTrue[] = {false, true};
...
TEST_F(VoteUploaderTest, FormClassifierVoteUpload) {
...
TEST_F(VoteUploaderTest, FieldPropertiesMasksUpload) {
  PasswordForm form(*observed_form());
  form.form_data = saved_match()->form_data;
...
TEST_F(VoteUploaderTest, ProbablyAccountCreationUpload) {
  PasswordForm form(*observed_form());
  form.form_data = saved_match()->form_data;
...
TEST_F(VoteUploaderTest, UploadUsernameCorrectionVote) {
...
TEST_F(VoteUploaderTest, UploadSignInForm_WithAutofillTypes) {
...
// with 1 field.
TEST_F(VoteUploaderTest, NoUploadsForSubmittedFormWithOnlyOneField) {
  autofill::FormFieldData field;
  field.name = ASCIIToUTF16("Passwd");
  field.form_control_type = "password";
  observed_form()->form_data.fields.push_back(field);
