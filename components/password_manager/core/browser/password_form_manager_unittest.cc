...
class MockVoteUploader : public VoteUploader {
 public:
  MOCK_METHOD4(SendVotesOnSave,
               void(const autofill::PasswordForm& observed_form,
                    const autofill::PasswordForm& submitted_form,
                    bool has_generated_password,
                    autofill::PasswordForm* pending_credentials));
  MOCK_METHOD5(SendVoteOnCredentialsReuse,
               void(const autofill::PasswordForm& observed,
                    const autofill::PasswordForm& submitted_form,
                    bool has_generated_password,
                    bool username_edited,
                    autofill::PasswordForm* pending));
  MOCK_METHOD1(NotifyUsernameCorrected,
               void(autofill::PasswordForm corrected_username));
  MOCK_METHOD6(UploadPasswordVote,
               bool(const autofill::PasswordForm& form_to_upload,
                    const autofill::PasswordForm* submitted_form,
                    autofill::ServerFieldType field_type,
                    const std::string& login_form_signature,
                    bool has_generated_password,
                    bool observed_is_change_password_form));
  MOCK_METHOD1(NotifyPasswordChangedByUser, void(bool changed));

  // VoteUploader, non-interesting overrides.
...
class MockPasswordManagerDriver : public StubPasswordManagerDriver {
 public:
  MockPasswordManagerDriver() {
    auto prefs = base::MakeUnique<TestingPrefServiceSimple>();
...
class PasswordFormManagerTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    form_manager_->set_vote_uploader_for_testing(&vote_uploader_);
  }
  ...
  MockVoteUploader* vote_uploader() { return &vote_uploader_; }
 ...
 private:
  ...
  // Define |fake_form_fetcher_| and |vote_uploader_| before |form_manager_|,
  // because the |form_manager_| uses them and therefore needs them to outlive
  // it.
  FakeFormFetcher fake_form_fetcher_;
  MockVoteUploader vote_uploader_;
...
// Test that VoteUploader is notified properly if username is corrected by the
// user.
TEST_F(PasswordFormManagerTest, VoteUploader_NotifyUsernameCorrected) {
  PasswordForm saved_form(*observed_form());
  saved_form.username_value = ASCIIToUTF16("username");
  saved_form.password_value = ASCIIToUTF16("1234");
  saved_form.other_possible_usernames = {
      {ASCIIToUTF16("alternative name"), ASCIIToUTF16("alternative field")}};
  fake_form_fetcher()->SetNonFederated({&saved_form}, 0u);

  PasswordForm form_to_save(saved_form);
  form_to_save.preferred = true;
  form_to_save.username_value = ASCIIToUTF16("alternative name");
  form_to_save.password_value = ASCIIToUTF16("1234");

  PasswordForm corrected_form(saved_form);
  corrected_form.username_element = ASCIIToUTF16("alternative field");
  EXPECT_CALL(*vote_uploader(), NotifyUsernameCorrected(corrected_form));
  form_manager()->ProvisionallySave(
      form_to_save, PasswordFormManager::IGNORE_OTHER_POSSIBLE_USERNAMES);
}

// Test that VoteUploader is not notified if username is not corrected to one of
// the "other possible usernames".
TEST_F(PasswordFormManagerTest, VoteUploader_No_NotifyUsernameCorrected) {
  PasswordForm saved_form(*observed_form());
  saved_form.username_value = ASCIIToUTF16("username");
  saved_form.password_value = ASCIIToUTF16("1234");
  saved_form.other_possible_usernames = {
  ...
  PasswordForm form_to_save(saved_form);
  form_to_save.preferred = true;
  form_to_save.username_value = ASCIIToUTF16("yet another name");
  form_to_save.password_value = ASCIIToUTF16("1234");

  EXPECT_CALL(*vote_uploader(), NotifyUsernameCorrected(_)).Times(0);
  form_manager()->ProvisionallySave(
      form_to_save, PasswordFormManager::IGNORE_OTHER_POSSIBLE_USERNAMES);
}

TEST_F(PasswordFormManagerTest,
       VoteUploader_NotifyPasswordChangedByUser_Presave) {
  fake_form_fetcher()->SetNonFederated(std::vector<const PasswordForm*>(), 0u);

  // Presave a form once. The first presaved password is the one Chrome
  // generated, so the vote for "changed by user" should be false.
  EXPECT_CALL(*vote_uploader(), NotifyPasswordChangedByUser(false));
  form_manager()->PresaveGeneratedPassword(*saved_match());

  // Presave a form twice. The second value is an edit by the user.
  PasswordForm second(*saved_match());
  second.password_value = ASCIIToUTF16("1234");
  EXPECT_CALL(*vote_uploader(), NotifyPasswordChangedByUser(true));
  form_manager()->PresaveGeneratedPassword(second);

  // Cancel the generation. This should be reported as "not changed by user"
  // again.
  EXPECT_CALL(*vote_uploader(), NotifyPasswordChangedByUser(false));
  form_manager()->PasswordNoLongerGenerated();
}

// Test that the correct forms are passed to SendVotesOnSave.
TEST_F(PasswordFormManagerTest, VoteUploader_SendVotesOnSave_CorrectForms) {
  fake_form_fetcher()->SetNonFederated(std::vector<const PasswordForm*>(), 0u);

  form_manager()->ProvisionallySave(
      *saved_match(), PasswordFormManager::IGNORE_OTHER_POSSIBLE_USERNAMES);
  PasswordForm pending;
  EXPECT_CALL(*vote_uploader(),
              SendVotesOnSave(*observed_form(), *saved_match(), false, _))
      .WillOnce(SaveArgPointee<3>(&pending));
  EXPECT_CALL(*vote_uploader(), SendVoteOnCredentialsReuse(_, _, _, _, _))
      .Times(0);
  form_manager()->Save();
  // In this case (new credential), the pending credentials differ from the
  // submitted form in particular by having |date_created| set to the time of
  // saving.
  EXPECT_NE(pending.date_created, saved_match()->date_created);
  EXPECT_EQ(saved_match()->username_value, pending.username_value);
  EXPECT_EQ(saved_match()->password_value, pending.password_value);
  EXPECT_EQ(saved_match()->username_element, pending.username_element);
  EXPECT_EQ(saved_match()->password_element, pending.password_element);
}

// Test that the correct forms are passed to SendVotesOnSave.
TEST_F(PasswordFormManagerTest,
       VoteUploader_SendVotesOnCredentialsReuse_CorrectForms) {
  fake_form_fetcher()->SetNonFederated({saved_match()}, 0u);

  PasswordForm submitted_form(*saved_match());
  ...
  EXPECT_CALL(*vote_uploader(),
              SendVoteOnCredentialsReuse(*observed_form(), submitted_form,
                                         false, false, _))
      .WillOnce(SaveArgPointee<4>(&pending));
  EXPECT_CALL(*vote_uploader(), SendVotesOnSave(_, _, _, _)).Times(0);
  form_manager()->Save();
  EXPECT_EQ(submitted_form.date_created, pending.date_created);
  EXPECT_EQ(submitted_form.username_value, pending.username_value);
  EXPECT_EQ(submitted_form.password_value, pending.password_value);
  EXPECT_EQ(submitted_form.username_element, pending.username_element);
  EXPECT_EQ(submitted_form.password_element, pending.password_element);
}

// Test that the correct inputs are passed to UploadPasswordVote on Update().
TEST_F(PasswordFormManagerTest, VoteUploader_UploadPasswordVote_Update) {
  PasswordForm observed(*observed_form());
  observed.username_element.clear();
  observed.new_password_element = ASCIIToUTF16("NewPasswd");
  FakeFormFetcher fetcher;
  fetcher.Fetch();
  PasswordFormManager form_manager(password_manager(), client(),
                                   client()->driver(), observed,
                                   base::MakeUnique<MockFormSaver>(), &fetcher);
  form_manager.set_vote_uploader_for_testing(vote_uploader());
  fetcher.SetNonFederated({saved_match()}, 0u);

  PasswordForm credentials(*saved_match());
  credentials.new_password_value = ASCIIToUTF16("test2");
  credentials.preferred = true;
  form_manager.ProvisionallySave(
      credentials, PasswordFormManager::IGNORE_OTHER_POSSIBLE_USERNAMES);
  EXPECT_CALL(
      *vote_uploader(),
      UploadPasswordVote(observed, Pointee(credentials), autofill::NEW_PASSWORD,
                         Not(IsEmpty()), false, true));
  form_manager.Update(credentials);
}

// Test that the correct inputs are passed to UploadPasswordVote in
// OnNopeUpdateClicked().
TEST_F(PasswordFormManagerTest,
       VoteUploader_UploadPasswordVote_NopeUpdateClicked) {
  fake_form_fetcher()->SetNonFederated({saved_match()}, 0u);
  ...
  EXPECT_CALL(
      *vote_uploader(),
      UploadPasswordVote(*observed_form(), Pointee(submitted_form),
                         autofill::NOT_NEW_PASSWORD, IsEmpty(), false, false));
  form_manager()->OnNopeUpdateClicked();
}

// Test that the correct inputs are passed to UploadPasswordVote in
// OnNopeUpdateClicked().
TEST_F(PasswordFormManagerTest, VoteUploader_UploadPasswordVote_NeverClicked) {
  fake_form_fetcher()->SetNonFederated({saved_match()}, 0u);
  ...
  EXPECT_CALL(
      *vote_uploader(),
      UploadPasswordVote(submitted_form, Pointee(submitted_form),
                         autofill::UNKNOWN_TYPE, IsEmpty(), false, false));
  form_manager()->OnNeverClicked();
}

// Test that the correct inputs are passed to UploadPasswordVote in
// OnNoInteraction().
TEST_F(PasswordFormManagerTest, VoteUploader_UploadPasswordVote_NoInteraction) {
  fake_form_fetcher()->SetNonFederated({saved_match()}, 0u);
  ...
  EXPECT_CALL(*vote_uploader(),
              UploadPasswordVote(*observed_form(), Pointee(submitted_form),
                                 autofill::PROBABLY_NEW_PASSWORD, IsEmpty(),
                                 false, false));
  form_manager()->OnNoInteraction(true);
  EXPECT_CALL(
      *vote_uploader(),
      UploadPasswordVote(submitted_form, Pointee(submitted_form),
                         autofill::UNKNOWN_TYPE, IsEmpty(), false, false));
  form_manager()->OnNoInteraction(false);
}

// Test that inappropriate votes are not sent on change password forms.
TEST_F(PasswordFormManagerTest, VoteUploader_ChangePasswordForms) {
  PasswordForm observed(*observed_form());
  observed.username_element.clear();
  observed.new_password_element = ASCIIToUTF16("NewPasswd");
  FakeFormFetcher fetcher;
  ...
  PasswordForm credentials(observed);
  credentials.password_value = saved_match()->password_value;
  credentials.new_password_value = ASCIIToUTF16("test2");
  credentials.preferred = true;
  form_manager.ProvisionallySave(
...
// Test that the |has_generated_password| bit is passed to SendVotesOnSave.
TEST_F(PasswordFormManagerTest, VoteUploader_SendVotesOnSave_Generated) {
  fake_form_fetcher()->SetNonFederated(std::vector<const PasswordForm*>(), 0u);

  PasswordForm generated_password = *observed_form();
  generated_password.username_element.clear();
  generated_password.password_value = saved_match()->password_value;
  generated_password.new_password_element = ASCIIToUTF16("NewPasswd");
  generated_password.new_password_value = ASCIIToUTF16("new_password");
  generated_password.preferred = true;
  form_manager()->PresaveGeneratedPassword(generated_password);
  form_manager()->ProvisionallySave(
      *saved_match(), PasswordFormManager::IGNORE_OTHER_POSSIBLE_USERNAMES);
  EXPECT_CALL(*vote_uploader(), SendVotesOnSave(_, _, true, _));
  form_manager()->Save();
}

// Test that cancellation of the |has_generated_password| bit is passed to
// SendVotesOnSave.
TEST_F(PasswordFormManagerTest,
       VoteUploader_SendVotesOnSave_NoLongerGenerated) {
  fake_form_fetcher()->SetNonFederated(std::vector<const PasswordForm*>(), 0u);
  ...
  form_manager()->ProvisionallySave(
      *saved_match(), PasswordFormManager::IGNORE_OTHER_POSSIBLE_USERNAMES);
  EXPECT_CALL(*vote_uploader(), SendVotesOnSave(_, _, false, _));
  form_manager()->Save();
}

// Test that password update triggers a vote on credential reuse.
TEST_F(PasswordFormManagerTest, VoteUploader_SendVoteOnCredentialReuse) {
  fake_form_fetcher()->SetNonFederated({saved_match()}, 0u);
  ...
  form_manager()->ProvisionallySave(
      submitted_form, PasswordFormManager::IGNORE_OTHER_POSSIBLE_USERNAMES);
  EXPECT_CALL(*vote_uploader(),
              SendVoteOnCredentialsReuse(_, _, false, false, _));
  // No vote on saving, though, because this was not a new login.
  EXPECT_CALL(*vote_uploader(), SendVotesOnSave(_, _, _, _)).Times(0);
  form_manager()->Save();
}

// Test that if the user chooses an alternative username from "other possible
// usernames", this is signalled for vote uploading.
TEST_F(PasswordFormManagerTest, VoteUploader_UsernameEdited) {
  PasswordForm saved_form(*observed_form());
  saved_form.username_value = ASCIIToUTF16("username");
  saved_form.password_value = ASCIIToUTF16("1234");
  saved_form.other_possible_usernames = {
  ...
  form_manager()->ProvisionallySave(
      form_to_save, PasswordFormManager::ALLOW_OTHER_POSSIBLE_USERNAMES);
  EXPECT_CALL(*vote_uploader(),
              SendVoteOnCredentialsReuse(_, _, false, true, _));
  // No vote on saving, though, because this was not a new login.
  EXPECT_CALL(*vote_uploader(), SendVotesOnSave(_, _, _, _)).Times(0);
  form_manager()->Save();
}

TEST_F(PasswordFormManagerTest, VoteUploader_Blacklist) {
  ...
  EXPECT_CALL(*vote_uploader(), SendVoteOnCredentialsReuse(_, _, _, _, _))
      .Times(0);
  EXPECT_CALL(*vote_uploader(), UploadPasswordVote(_, _, _, _, _, _)).Times(0);
  form_manager()->PermanentlyBlacklist();
}
