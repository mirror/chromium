...
TEST_F(PasswordFormManagerTest, TestUpdateUsernameToExisting) {
  ...
  EXPECT_CALL(
      ...
      Update(expected_pending,
             testing::ElementsAre(testing::Pair(saved_match()->username_value,
                                                Pointee(*saved_match()))),
             testing::Pointee(IsEmpty()), nullptr));
  form_manager()->Save();
}
...
TEST_F(PasswordFormManagerTest, TestUpdatePSLUsername) {
  ...
  EXPECT_CALL(
      MockFormSaver::Get(form_manager()),
      Save(_,
           testing::ElementsAre(testing::Pair(psl_saved_match()->username_value,
