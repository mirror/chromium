...
TEST_F(PasswordFormManagerTest, TestUpdateUsernameToExisting) {
  ...
  // ProvisionallySave() increases the times_used variable from 0 to 1.
  PasswordForm expected_pending(form_manager()->pending_credentials());
  expected_pending.times_used = 1;

  // Empty vector to test the credentials_to_update parameter.
  std::vector<autofill::PasswordForm> credentials_to_update;

  // User clicks save, edited username is saved, password updated.
  PasswordForm saved_result;
  EXPECT_CALL(MockFormSaver::Get(form_manager()),
              Update(expected_pending, form_manager()->best_matches(),
                     testing::Pointee(credentials_to_update), nullptr))
      .WillOnce(SaveArg<0>(&saved_result));
  form_manager()->Save();
  EXPECT_EQ(saved_match()->username_value, saved_result.username_value);
  EXPECT_EQ(ASCIIToUTF16("different_pass"), saved_result.password_value);
}
...
TEST_F(PasswordFormManagerTest, TestUpdatePSLUsername) {
  ...
  PasswordForm saved_result;
  EXPECT_CALL(MockFormSaver::Get(form_manager()),
              Save(_, form_manager()->best_matches(), nullptr))
      .WillOnce(SaveArg<0>(&saved_result));
  form_manager()->Save();
