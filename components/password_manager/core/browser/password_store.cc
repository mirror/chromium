...
void PasswordStore::GetLoginsRequest::NotifyConsumerWithResults(
  ...
  // Per https://crbug.com/756587, exclude passwords saved for about:blank and
  // about:srcdoc documents.  Saving such passwords is not possible anymore,
  // and this prevents previously saved passwords for these documents from
  // being unsafely used for autofill.
  if (ignore_about_blank_) {
    base::EraseIf(results,
                  [this](const std::unique_ptr<PasswordForm>& credential) {
                    return credential->origin.SchemeIs(url::kAboutScheme);
                  });
  }
