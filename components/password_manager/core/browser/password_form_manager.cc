...
void PasswordFormManager::CreatePendingCredentials() {
  ...
  // This first match via FindBestSavedMatch focusses on matches by username and
  ...
  if (saved_form != nullptr) {
    ...
    // We have stored forms (!best_matches_.empty()) but they did not match the
    // submitted form under either of these criteria:
    // - match by username
    // - submitted form consists of only a password field (no username and the
    //   password field is not a "new-password" field) in which the password
    //   mached.
    ...
    // A retry password form is one that consists of only an old password field,
    // i.e. one that is not annotated as "new-password".
...
const PasswordForm* PasswordFormManager::FindBestSavedMatch(
  ...
  bool kSubmittedFormHasUsername = !submitted_form->username_element.empty();
  bool kSubmittedFormHasNewPasswordElement =
      !submitted_form->new_password_element.empty();
  if (kSubmittedFormHasUsername || kSubmittedFormHasNewPasswordElement)
    return nullptr;
