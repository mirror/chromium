...
void PasswordFormManager::UpdateUsername(const base::string16& new_username) {
  ...
  // |has_username_edited_vote_| will be true only if |new_username| was typed
  // in another field. Otherwise, |has_username_edited_vote_| is false and no
  ...
  if (!new_username.empty()) {
    for (size_t i = 0; i < pending_credentials_.other_possible_usernames.size();
      ...
      if (pending_credentials_.other_possible_usernames[i].first ==
        ...
        // Replace |other_possible_usernames[i]| with (|old_username_value|,
        // |old_username_element|). A user may make a mistake in username
        // editing in a prompt. Saving the captured username allows to send a
        // correct vote when the user overrides the username on login.
  ...
  // No match may happen because of user mistake. So, save (old_username_value,
