...
PasswordFormManager::PasswordFormManager(
    ...
    : observed_form_(observed_form),
      ...
      metrics_(client->IsMainFrameSecure()) {
...
int PasswordFormManager::GetHistogramSampleForSuppressedAccounts(
  ...
  mixed_base_encoding += metrics_.GetActionsTakenNew();
...
void PasswordFormManager::Save() {
  ...
  if ((user_action_ == UserAction::kUserActionNone) &&
      DidPreferenceChange(best_matches_, pending_credentials_.username_value)) {
    SetUserAction(UserAction::kUserActionChoose);
  }
...
void PasswordFormManager::ProcessFrameInternal(
  ...
  if (wait_for_username) {
    metrics_.SetManagerAction(PasswordFormMetricsRecorder::kManagerActionNone);
  } else {
    has_autofilled_ = true;
    metrics_.SetManagerAction(
...
void PasswordFormManager::CreatePendingCredentials() {
  ...
  if (saved_form != nullptr) {
    ...
    if (IsPendingCredentialsPublicSuffixMatch()) {
      ...
      is_new_login_ = true;
      SetUserAction(password_overridden_
                        ? UserAction::kUserActionOverridePassword
                        : UserAction::kUserActionChoosePslMatch);
      ...
      is_new_login_ = false;
      if (password_overridden_)
        SetUserAction(UserAction::kUserActionOverridePassword);
    }
  ...
  if (user_action_ == UserAction::kUserActionOverridePassword &&
      pending_credentials_.type == PasswordForm::TYPE_GENERATED &&
...
void PasswordFormManager::CreatePendingCredentialsForNewCredentials() {
  ...
  SetUserAction(UserAction::kUserActionOverrideUsernameAndPassword);
...
void PasswordFormManager::SetHasGeneratedPassword(bool generated_password) {
  ...
  metrics_.SetHasGeneratedPassword(generated_password);
}

void PasswordFormManager::LogSubmitPassed() {
  metrics_.LogSubmitPassed();
}

void PasswordFormManager::LogSubmitFailed() {
  metrics_.LogSubmitFailed();
}

void PasswordFormManager::MarkGenerationAvailable() {
  metrics_.MarkGenerationAvailable();
}
...
void PasswordFormManager::SetUserAction(UserAction user_action) {
  ...
  metrics_.SetUserAction(user_action);
}
