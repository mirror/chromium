...
void PasswordFormManager::UpdateUsername(const base::string16& new_username) {
  ...
  // found, the field name is saved to |correct_username_element_|.
  // Otherwise, |correct_username_element_| has no value.
  ...
  if (!trimmed_username_value.empty()) {
    correct_username_element_.reset();
...
bool PasswordFormManager::UploadPasswordVote(
  ...
  if (password_type != autofill::USERNAME) {
    ...
    if (correct_username_element_.has_value()) {
      field_types[correct_username_element_.value()] = autofill::USERNAME;
      username_vote_type =
