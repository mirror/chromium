...
void PasswordFormManager::UpdateUsername(const base::string16& new_username) {
  ...
  const base::string16 old_username_value =
      std::move(credential.username_value);
  const base::string16 old_username_element =
      std::move(credential.username_element);
  ...
  credential.username_element = base::string16();
  ...
  // (old_username_value, old_username_element). When the user has to override
  // the username, Chrome will send a username vote.
  if (!old_username_value.empty()) {
