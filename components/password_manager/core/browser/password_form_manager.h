...
class PasswordFormManager : public FormFetcher::Consumer {
 public:
  ...
  VoteUploader* vote_uploader() { return vote_uploader_; }

  void set_vote_uploader_for_testing(VoteUploader* vote_uploader) {
    vote_uploader_ = vote_uploader;
  }
 ...
 private:
  ...
  // Used to handle sending form-related votes to autofill servers.
  VoteUploaderImpl vote_uploader_impl_;

  // A pointer to access VoteUploader. In production code it should point to
  // |vote_uploader_impl_|, in tests it's possible to point it to a mocked
  // version.
  VoteUploader* vote_uploader_;
