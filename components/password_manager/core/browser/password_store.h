...
class PasswordStore : protected PasswordStoreSync,
 ...
 public:
  ...
  // Same as above, but also fills in affiliation information for Android
  // credentials.
  virtual void GetAutofillableLoginsWithAffiliationInformation(
      PasswordStoreConsumer* consumer);
  ...
  // Same as above, but also fills in affiliation information for Android
  // credentials.
  virtual void GetBlacklistLoginsWithAffiliationInformation(
      PasswordStoreConsumer* consumer);
...
// TODO(crbug.com/706392): Fix password reuse detection for Android.
 ...
 private:
  ...
  // Same as above, but also fills in affiliation information for Android
  // credentials.
  void GetAutofillableLoginsWithAffiliationInformationImpl(
      std::unique_ptr<GetLoginsRequest> request);
  ...
  // Same as above, but also fills in affiliation information for Android
  // credentials.
  void GetBlacklistLoginsWithAffiliationInformationImpl(
      std::unique_ptr<GetLoginsRequest> request);
  ...
  // Retrieves and fills in affiliation information for Android
