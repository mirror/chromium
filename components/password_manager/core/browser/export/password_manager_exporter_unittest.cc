...
base::MockCallback<WriteCallback>* s_write_callback = nullptr;

// Mock for base::WriteFile. Expectations should be set on |s_write_callback|.
int WriteFunction(const base::FilePath& filename, const char* data, int size) {
  if (s_write_callback)
    return s_write_callback->Get().Run(filename, data, size);
  return size;
}
...
class PasswordManagerExporterTest : public testing::Test {
 public:
  PasswordManagerExporterTest()
    ...
    s_write_callback = &mock_write_file_;
    exporter_.SetWriteForTesting(&WriteFunction);
  }

  ~PasswordManagerExporterTest() override { s_write_callback = nullptr; }
...
TEST_F(PasswordManagerExporterTest, PasswordExportSetPasswordListFirst) {
  ...
  EXPECT_CALL(
      mock_on_progress_,
      Run(password_manager::ExportProgressStatus::COMPLETED, IsEmpty()));
...
TEST_F(PasswordManagerExporterTest, WriteFileFailed) {
  ...
  EXPECT_CALL(mock_on_progress_,
              Run(password_manager::ExportProgressStatus::FAILED_CANNOT_EXPORT,
...
TEST_F(PasswordManagerExporterTest, GetProgressReturnsLastCallbackStatus) {
  ...
  password_manager::ExportProgressStatus status =
      password_manager::ExportProgressStatus::NEVER_STARTED;
