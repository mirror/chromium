...
class PasswordManagerExporter {
 ...
 private:
  ...
  // Callback after the passwords have been serialised. |destination| is the
  // folder we wrote to. |count| is the number of passwords exported. |success|
  // is whether they were actually written.
  ...
  // |task_runner_| is used for time-consuming tasks during exporting. The
  // tasks will dereference a WeakPtr to |*this|, which means they all need to
