...
// A wrapper for |write_function_|, which can be bound and keep a copy of it's
// data on the closure.
...
void PasswordManagerExporter::PreparePasswordsForExport() {
  ...
  size_t passoword_list_size = password_list.size();
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&PasswordManagerExporter::SetSerialisedPasswordList,
                     weak_factory_.GetWeakPtr(), passoword_list_size));
}
