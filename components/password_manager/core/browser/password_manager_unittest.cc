...
TEST_F(PasswordManagerTest, ProcessAutofillPredictions) {
  ...
  // |AutofillField::server_type_| will store only autofill vote, but not
  // password vote. |AutofillField::server_predictions_| should store all
  // predictions.
  ...
  field1->set_autofill_type(autofill::PASSWORD);
  autofill::AutofillQueryResponseContents_Field_FieldPrediction*
      field_prediction2 = field1->add_predictions();
  field_prediction2->set_autofill_type(autofill::PASSWORD);
  autofill::AutofillQueryResponseContents_Field_FieldPrediction*
      field_prediction3 = field1->add_predictions();
  field_prediction3->set_autofill_type(autofill::PROBABLY_NEW_PASSWORD);
