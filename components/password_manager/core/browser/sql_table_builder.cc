...
// The table name defaults to "logins".
SQLTableBuilder::SQLTableBuilder() : kTableName_("logins") {}

SQLTableBuilder::SQLTableBuilder(const std::string& table_name)
    : kTableName_(table_name) {}
...
bool SQLTableBuilder::CreateTable(sql::Connection* db) {
  ...
  if (db->DoesTableExist(kTableName_.c_str()))
    return true;
...
bool SQLTableBuilder::MigrateToNextFrom(unsigned old_version,
  ...
  if (needs_temp_table) {
    ...
    const std::string kTempTableName = "temp_" + kTableName_;

    sql::Transaction transaction(db);
    if (!(transaction.Begin() &&
          db->Execute(base::StringPrintf(
                          "CREATE TABLE %s (%s, %s)", kTempTableName.c_str(),
                          new_names.c_str(), unique_constraint_.c_str())
                          .c_str()) &&
          db->Execute(
              base::StringPrintf("INSERT OR REPLACE INTO %s SELECT %s FROM %s",
                                 kTempTableName.c_str(), old_names.c_str(),
                                 kTableName_.c_str())
                  .c_str()) &&
          db->Execute(base::StringPrintf("DROP TABLE %s", kTableName_.c_str())
                          .c_str()) &&
          db->Execute(base::StringPrintf("ALTER TABLE %s RENAME TO %s",
                                         kTempTableName.c_str(),
                                         kTableName_.c_str())
                          .c_str()) &&
          db->Execute(
              base::StringPrintf("CREATE INDEX %s_signon ON %s (signon_realm)",
