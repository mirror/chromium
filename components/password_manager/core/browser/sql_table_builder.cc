...
void SQLTableBuilder::AddIndex(std::string name,
  ...
  // Check if all entries of |columns| are unique. There might be faster ways to
  // check this, however this is very concise and performance is not critial
  // given this is wrapped in a DCHECK and won't be part of release builds.
...
void SQLTableBuilder::RenameIndex(const std::string& old_name,
  ...
  // Check that the old index has not been renamed yet.
...
void SQLTableBuilder::DropIndex(const std::string& name) {
  ...
  // Check that the index has not been deleted yet.
