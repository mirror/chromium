...
// This metric records what the user does with a form or password manager UI.
// Values correspond to the enum DetailedUserAction. In contrast to
// kUkmUserActionSimplified, ths metric is intended to be extensible with new
// user action types.
...
class PasswordFormMetricsRecorder
 ...
 public:
  ...
  // contrast to UserAction, it is intended to be extensible.
  enum class DetailedUserAction {
    kUnknown = 0,

    // Interactions with password bubble.
    kEditedUsernameInBubble = 1000,
  };
 ...
 private:
  ...
  // Returns true if an |action| should be returned only once per life-cyle of a
  // PasswordFormMetricsRecorder.
  static bool IsOneTimeReportUserAction(DetailedUserAction action);
