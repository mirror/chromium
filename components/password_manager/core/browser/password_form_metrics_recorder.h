...
constexpr char kUkmManagerFillEvent[] = "ManagerFillAction";

// This metric records the activity of the password manager for filling a form.
}  // namespace internal
...
class PasswordFormMetricsRecorder
 ...
 public:
  ...
  enum ManagerFillEvent {
    // No credential existed that could be filled into a password form.
    kManagerFillEventNoCredential = 0,
    // A credential could be filled into a password form but was not due to a
    // policy. E.g. incognito mode requires a user interaction before filling
    // can happen. PSL matches are not autofilled and also on password change
    // forms we do not autofill.
    kManagerFillEventBlockedOnInteraction,
    // A credential was autofilled into a form.
    kManagerFillEventFilled
  };
  ...
  void RecordFillEvent(ManagerFillEvent event);
