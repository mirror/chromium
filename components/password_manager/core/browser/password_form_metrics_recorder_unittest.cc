...
TEST(PasswordFormMetricsRecorder, RecordPasswordBubbleShown) {
  using Trigger = PasswordFormMetricsRecorder::SaveBubbleTrigger;
  static constexpr struct {
    // Stimuli:
    ...
    bool expected_update_prompt_shown;
      ...
      {metrics_util::CredentialSourceType::kCredentialManagementAPI,
       metrics_util::AUTOMATIC_WITH_PASSWORD_PENDING,
       internal::kUkmSavingPromptTrigger, Trigger::kCredentialMgmtAPIAutomatic,
       true, false},
      {metrics_util::CredentialSourceType::kCredentialManagementAPI,
       metrics_util::MANUAL_WITH_PASSWORD_PENDING,
       internal::kUkmSavingPromptTrigger, Trigger::kCredentialMgmtAPIManual,
       true, false},
      // Source = Credential Management API, Updating.
      {metrics_util::CredentialSourceType::kCredentialManagementAPI,
       metrics_util::AUTOMATIC_WITH_PASSWORD_PENDING_UPDATE,
       internal::kUkmUpdatingPromptTrigger,
       Trigger::kCredentialMgmtAPIAutomatic, false, true},
      {metrics_util::CredentialSourceType::kCredentialManagementAPI,
       metrics_util::MANUAL_WITH_PASSWORD_PENDING_UPDATE,
       internal::kUkmUpdatingPromptTrigger, Trigger::kCredentialMgmtAPIManual,
       false, true},
  };
  ...
  for (const auto& test : kTests) {
    SCOPED_TRACE(testing::Message()
                 ...
                 << ", display_position = " << test.display_disposition);
    ukm::TestUkmRecorder test_ukm_recorder;
    {
      auto recorder = base::MakeRefCounted<PasswordFormMetricsRecorder>(
    ...
    // Verify data
    ExpectUkmValueCount(&test_ukm_recorder, test.metric,
                        static_cast<int64_t>(test.expected_trigger), 1);
    ExpectUkmValueCount(&test_ukm_recorder, internal::kUkmSavingPromptShown,
                        test.expected_save_prompt_shown ? 1 : 0, 1);
    ExpectUkmValueCount(&test_ukm_recorder, internal::kUkmUpdatingPromptShown,
                        test.expected_update_prompt_shown ? 1 : 0, 1);
  }
}
...
TEST(PasswordFormMetricsRecorder, RecordUIDismissalReason) {
  static constexpr struct {
    ...
    // Expectations:
    const char* metric;
    PasswordFormMetricsRecorder::DismissalReason expected_dismissal_reason;
  } kTests[] = {
      {metrics_util::AUTOMATIC_WITH_PASSWORD_PENDING,
       metrics_util::CLICKED_SAVE, internal::kUkmSavingPromptInteraction,
       PasswordFormMetricsRecorder::DismissalReason::kAccepted},
  ...
  for (const auto& test : kTests) {
    ...
    ukm::TestUkmRecorder test_ukm_recorder;
    {
      auto recorder = base::MakeRefCounted<PasswordFormMetricsRecorder>(
          true /*is_main_frame_secure*/,
          CreateUkmEntryBuilder(&test_ukm_recorder));
      recorder->RecordUIDismissalReason(test.display_disposition,
                                        test.dismissal_reason);
    }
    // Verify data
    ExpectUkmValueCount(&test_ukm_recorder, test.metric,
                        static_cast<int64_t>(test.expected_dismissal_reason),
                        1);
  }
}
