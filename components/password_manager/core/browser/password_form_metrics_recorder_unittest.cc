...
TEST(PasswordFormMetricsRecorder, Actions) {
  ...
  for (const auto& test : kTests) {
    ...
    // on destruction.
    {
      ...
      if (test.user_action != UserAction::kUserActionNone)
    ...
    switch (test.user_action) {
      case UserAction::kUserActionNone:
        break;
      case UserAction::kUserActionChoose:
        EXPECT_EQ(1, user_action_tester.GetActionCount(
                         "PasswordManager_UsedNonDefaultUsername"));
        break;
      case UserAction::kUserActionChoosePslMatch:
        EXPECT_EQ(1, user_action_tester.GetActionCount(
                         "PasswordManager_ChoseSubdomainPassword"));
        break;
      case UserAction::kUserActionOverridePassword:
        EXPECT_EQ(1, user_action_tester.GetActionCount(
                         "PasswordManager_LoggedInWithNewPassword"));
        break;
      case UserAction::kUserActionOverrideUsernameAndPassword:
        EXPECT_EQ(1, user_action_tester.GetActionCount(
                         "PasswordManager_LoggedInWithNewUsername"));
        break;
      case UserAction::kUserActionMax:
        break;
    }
  }
}
