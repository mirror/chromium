...
PasswordManagerMetricsRecorder::PasswordManagerMetricsRecorder(
    ...
    : ukm_entry_builder_(std::move(that.ukm_entry_builder_)),
      user_modified_password_field_(that.user_modified_password_field_) {}
...
PasswordManagerMetricsRecorder::CreateUkmEntryBuilder(
  ...
  if (!ukm_recorder)
    return nullptr;
  return ukm_recorder->GetEntryBuilder(source_id, "PasswordManager");
}
