...
TEST_F(PasswordStoreTest, GetLoginsWithAffiliationAndBrandingInformation) {
  ...
  for (bool blacklisted : {false, true}) {
    ...
    std::vector<MockAffiliatedMatchHelper::AffiliationAndBrandingInformation>
        info = {{kTestWebRealm1, kTestAndroidName1, GURL(kTestAndroidIconURL1)},
    ...
    for (size_t i = 0; i < expected_results.size(); ++i) {
      ...
      expected_results[i]->app_display_name = info[i].app_display_name;
      expected_results[i]->app_icon_url = info[i].app_icon_url;
    }
