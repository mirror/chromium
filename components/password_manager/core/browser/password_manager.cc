...
// TODO
std::vector<std::unique_ptr<PasswordFormManager>>::const_iterator
FindMatchedManager(const autofill::PasswordForm& form,
...
void PasswordManager::ProvisionallySavePassword(
  ...
  SaveMatchedManager(form, matched_manager_it, logger);
...
void PasswordManager::SaveMatchedManager(
    const PasswordForm& form,
    FormManagersList::const_iterator& matched_manager_it,
    BrowserSavePasswordProgressLogger* logger) {
  DCHECK(matched_manager_it != pending_login_managers_.end());
