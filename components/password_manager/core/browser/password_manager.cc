...
PasswordFormManager* FindMatchedManager(
  ...
  if (matched_manager_it != pending_login_managers.end()) {
    (*matched_manager_it)->SaveSubmittedFormTypeForMetrics(form);
    return matched_manager_it->get();
  } else {
    return nullptr;
  }
}
...
void PasswordManager::ProvisionallySavePassword(
  ...
  SaveMatchedManager(form, matched_manager, logger.get());
