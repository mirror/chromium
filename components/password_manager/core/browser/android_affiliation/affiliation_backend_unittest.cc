...
AffiliatedFacets GetTestEquivalenceClassBeta() {
  return AffiliatedFacets{
      ...
      {FacetURI::FromCanonicalSpec(kTestFacetURIBeta2), FacetBrandingInfo{}},
  };
}

AffiliatedFacets GetTestEquivalenceClassGamma() {
  return AffiliatedFacets{
      {FacetURI::FromCanonicalSpec(kTestFacetURIGamma1), FacetBrandingInfo{}},
  };
}
...
class AffiliationBackendTest : public testing::Test {
 ...
 protected:
  void GetAffiliations(MockAffiliationConsumer* consumer,
    ...
    backend_->GetAffiliations(facet, cache_miss_strategy,
                              consumer->GetResultCallback(),
                              consumer_task_runner());
  }

  void Prefetch(const Facet& facet, base::Time keep_fresh_until) {
    backend_->Prefetch(facet, keep_fresh_until);
  }

  void CancelPrefetch(const Facet& facet, base::Time keep_fresh_until) {
    backend_->CancelPrefetch(facet, keep_fresh_until);
  }
  ...
  void GetAffiliationsAndExpectResultWithoutFetch(
      ...
      StrategyOnCacheMiss cache_miss_strategy,
      const AffiliatedFacets& expected_result) {
    GetAffiliations(mock_consumer(), facet, cache_miss_strategy);
    ASSERT_NO_FATAL_FAILURE(ExpectNoFetchNeeded());
    mock_consumer()->ExpectSuccessWithResult(expected_result);
    consumer_task_runner_->RunUntilIdle();
    testing::Mock::VerifyAndClearExpectations(mock_consumer());
  ...
  void GetAffiliationsAndExpectFailureWithoutFetch(const Facet& facet) {
    GetAffiliations(mock_consumer(), facet, StrategyOnCacheMiss::FAIL);
    ASSERT_NO_FATAL_FAILURE(ExpectFailureWithoutFetch(mock_consumer()));
  }

  void PrefetchAndExpectFetch(const Facet& facet, base::Time keep_fresh_until) {
    Prefetch(facet, keep_fresh_until);
    ASSERT_NO_FATAL_FAILURE(ExpectNeedForFetchAndLetItBeSent());
    ASSERT_NO_FATAL_FAILURE(ExpectAndCompleteFetch(facet.uri));
  }
  ...
  void ExpectThatEquivalenceClassIsServedFromCache(
    ...
    for (const Facet& facet : affiliated_facets) {
      ...
      ASSERT_NO_FATAL_FAILURE(GetAffiliationsAndExpectResultWithoutFetch(
          facet, StrategyOnCacheMiss::FAIL, affiliated_facets));
      ASSERT_NO_FATAL_FAILURE(GetAffiliationsAndExpectResultWithoutFetch(
          facet, StrategyOnCacheMiss::FAIL, affiliated_facets));
    }
  }
  ...
  bool IsCachedDataFreshForFacet(const Facet& facet) {
    std::unique_ptr<base::Clock> clock(backend_task_runner_->GetMockClock());
    return FacetManager(facet, backend(), clock.get()).IsCachedDataFresh();
  }

  bool IsCachedDataNearStaleForFacet(const Facet& facet) {
    std::unique_ptr<base::Clock> clock(backend_task_runner_->GetMockClock());
    return FacetManager(facet, backend(), clock.get()).IsCachedDataNearStale();
  }
...
TEST_F(AffiliationBackendTest, ConcurrentUnrelatedRequests) {
  Facet facet_alpha{FacetURI::FromCanonicalSpec(kTestFacetURIAlpha1),
                    FacetBrandingInfo{}};
  Facet facet_beta{FacetURI::FromCanonicalSpec(kTestFacetURIBeta1),
                   FacetBrandingInfo{}};
  Facet facet_gamma{FacetURI::FromCanonicalSpec(kTestFacetURIGamma1),
                    FacetBrandingInfo{}};
  ...
  second_fetch_uris.push_back(facet_beta.uri);
  second_fetch_uris.push_back(facet_gamma.uri);
  ASSERT_NO_FATAL_FAILURE(ExpectAndCompleteFetch(facet_alpha.uri));
  ASSERT_NO_FATAL_FAILURE(ExpectNeedForFetchAndLetItBeSent());
...
TEST_F(AffiliationBackendTest, ConcurrentUnrelatedRequests2) {
  ...
  fetched_uris.push_back(facet_alpha.uri);
  fetched_uris.push_back(facet_beta.uri);
  fetched_uris.push_back(facet_gamma.uri);
  ASSERT_NO_FATAL_FAILURE(ExpectNeedForFetchAndLetItBeSent());
...
TEST_F(AffiliationBackendTest, RetryIsMadeOnFailedFetch) {
  Facet facet{FacetURI::FromCanonicalSpec(kTestFacetURIAlpha1),
              FacetBrandingInfo{}};

  GetAffiliations(mock_consumer(), facet,
                  StrategyOnCacheMiss::FETCH_OVER_NETWORK);
  ASSERT_NO_FATAL_FAILURE(ExpectNeedForFetchAndLetItBeSent());
  ASSERT_NO_FATAL_FAILURE(ExpectAndFailFetch(facet.uri));
...
TEST_F(AffiliationBackendTest,
  ...
  Facet facet1{FacetURI::FromCanonicalSpec(kTestFacetURIAlpha1),
               FacetBrandingInfo{}};
  Facet facet2{FacetURI::FromCanonicalSpec(kTestFacetURIAlpha2),
               FacetBrandingInfo{}};

  MockAffiliationConsumer second_consumer;
  MockAffiliationConsumer third_consumer;
  GetAffiliations(mock_consumer(), facet1,
                  StrategyOnCacheMiss::FETCH_OVER_NETWORK);
  ASSERT_NO_FATAL_FAILURE(ExpectNeedForFetchAndLetItBeSent());
  GetAffiliations(&second_consumer, facet1,
                  StrategyOnCacheMiss::FETCH_OVER_NETWORK);
  GetAffiliations(&third_consumer, facet2,
                  StrategyOnCacheMiss::FETCH_OVER_NETWORK);

  ASSERT_NO_FATAL_FAILURE(ExpectAndCompleteFetch(facet1.uri));
  ASSERT_NO_FATAL_FAILURE(ExpectNoFetchNeeded());
...
TEST_F(AffiliationBackendTest,
  ...
  Prefetch(facet1, base::Time::Max());
  ASSERT_NO_FATAL_FAILURE(ExpectNeedForFetchAndLetItBeSent());
  Prefetch(facet1, base::Time::Max());
  Prefetch(facet2, base::Time::Max());
...
TEST_F(AffiliationBackendTest,
  ...
  ASSERT_NO_FATAL_FAILURE(GetAffiliationsAndExpectFetchAndThenResult(
      facet, GetTestEquivalenceClassAlpha()));
  AdvanceTime(GetCacheHardExpiryPeriod());
  EXPECT_FALSE(IsCachedDataFreshForFacet(facet));
...
TEST_F(AffiliationBackendTest, TrimCacheRetainsDataThatNeededByFacetManagers) {
  ...
  ASSERT_NO_FATAL_FAILURE(PrefetchAndExpectFetch(
      facet, backend_task_runner()->Now() + GetCacheHardExpiryPeriod()));
  backend()->TrimCache();
  ...
  AdvanceTime(GetCacheHardExpiryPeriod() - Epsilon());
  EXPECT_TRUE(IsCachedDataFreshForFacet(facet));
  ASSERT_NO_FATAL_FAILURE(ExpectThatEquivalenceClassIsServedFromCache(
      GetTestEquivalenceClassAlpha()));
  AdvanceTime(Epsilon());
  EXPECT_FALSE(IsCachedDataFreshForFacet(facet));
  ASSERT_NO_FATAL_FAILURE(GetAffiliationsAndExpectFailureWithoutFetch(facet));
}
...
TEST_F(AffiliationBackendTest,
  ...
  Facet preserved_facet{FacetURI::FromCanonicalSpec(kTestFacetURIBeta1),
                        FacetBrandingInfo{}};
  ASSERT_NO_FATAL_FAILURE(GetAffiliationsAndExpectFetchAndThenResult(
      {FacetURI::FromCanonicalSpec(kTestFacetURIAlpha1), FacetBrandingInfo{}},
      GetTestEquivalenceClassAlpha()));
  ASSERT_NO_FATAL_FAILURE(GetAffiliationsAndExpectFetchAndThenResult(
      preserved_facet, GetTestEquivalenceClassBeta()));
  EXPECT_EQ(2u, GetNumOfEquivalenceClassInDatabase());
  ...
  // i.e., that it expires when it would normally have expired.
  AdvanceTime(GetCacheHardExpiryPeriod() - Epsilon());
  EXPECT_TRUE(IsCachedDataFreshForFacet(preserved_facet));
  ASSERT_NO_FATAL_FAILURE(ExpectThatEquivalenceClassIsServedFromCache(
      GetTestEquivalenceClassBeta()));
  AdvanceTime(Epsilon());
  EXPECT_FALSE(IsCachedDataFreshForFacet(preserved_facet));
  ASSERT_NO_FATAL_FAILURE(
      GetAffiliationsAndExpectFailureWithoutFetch(preserved_facet));
}
