...
// web sites affiliated with the given Android application as well as branding
// information into password forms, if any.
TEST_F(AffiliatedMatchHelperTest, InjectAffiliationAndBrandingInformation) {
  ...
  EXPECT_EQ(kTestAndroidFacetNameAlpha3, results[0]->branding_info_name);
  EXPECT_EQ(kTestAndroidFacetIconURLAlpha3,
            results[0]->branding_info_icon_url.possibly_invalid_spec());
  EXPECT_THAT(results[1]->affiliated_web_realm,
              testing::Eq(kTestWebRealmBeta1));
  EXPECT_EQ(kTestAndroidFacetNameBeta2, results[1]->branding_info_name);
  EXPECT_EQ(kTestAndroidFacetIconURLBeta2,
            results[1]->branding_info_icon_url.possibly_invalid_spec());
