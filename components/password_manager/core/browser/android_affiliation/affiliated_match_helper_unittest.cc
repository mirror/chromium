...
const char kTestAndroidFacetPlayNameAlpha3[] = "Play Name Alpha 3";
...
const char kTestAndroidFacetPlayNameBeta2[] = "Play Name Beta 2";
...
const char kTestAndroidFacetPlayNameBeta3[] = "Play Name Beta 3";
...
AffiliatedFacets GetTestEquivalenceClassAlpha() {
  return {
      ...
      {FacetURI::FromCanonicalSpec(kTestAndroidFacetURIAlpha3),
       FacetBrandingInfo{kTestAndroidFacetPlayNameAlpha3,
...
AffiliatedFacets GetTestEquivalenceClassBeta() {
  return {
      ...
      {FacetURI::FromCanonicalSpec(kTestAndroidFacetURIBeta2),
       FacetBrandingInfo{kTestAndroidFacetPlayNameBeta2,
                         GURL(kTestAndroidFacetIconURLBeta2)}},
      {FacetURI::FromCanonicalSpec(kTestAndroidFacetURIBeta3),
       FacetBrandingInfo{kTestAndroidFacetPlayNameBeta3,
...
class AffiliatedMatchHelperTest : public testing::Test {
 ...
 protected:
  ...
  InjectAffiliationInformation(
      std::vector<std::unique_ptr<autofill::PasswordForm>> forms) {
    expecting_result_callback_ = true;
    match_helper()->InjectAffiliationInformation(
        std::move(forms),
...
// Verifies that InjectAffiliationInformation() injects the realms of web sites
// affiliated with the given Android application into password forms, if any.
TEST_F(AffiliatedMatchHelperTest, InjectAffiliationInformation) {
  std::vector<std::unique_ptr<autofill::PasswordForm>> forms;
  ...
  std::vector<std::unique_ptr<autofill::PasswordForm>> results(
      InjectAffiliationInformation(std::move(forms)));
  ...
  EXPECT_EQ(kTestAndroidFacetPlayNameAlpha3, results[0]->affiliated_play_name);
  EXPECT_EQ(kTestAndroidFacetIconURLAlpha3,
            results[0]->affiliated_icon_url.possibly_invalid_spec());
  EXPECT_THAT(results[1]->affiliated_web_realm,
              testing::Eq(kTestWebRealmBeta1));
  EXPECT_EQ(kTestAndroidFacetPlayNameBeta2, results[1]->affiliated_play_name);
  EXPECT_EQ(kTestAndroidFacetIconURLBeta2,
            results[1]->affiliated_icon_url.possibly_invalid_spec());
