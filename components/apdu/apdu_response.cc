...
std::unique_ptr<APDUResponse> APDUResponse::CreateFromMessage(
  ...
  return std::make_unique<APDUResponse>(std::move(data), response_status);
}

APDUResponse::APDUResponse(std::vector<uint8_t> message, Status response_status)
