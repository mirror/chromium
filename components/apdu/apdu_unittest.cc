...
TEST(ApduTest, TestDeserializeBasic) {
  constexpr uint8_t cla = 0xAA;
  constexpr uint8_t ins = 0xAB;
  constexpr uint8_t p1 = 0xAC;
  constexpr uint8_t p2 = 0xAD;
  std::vector<uint8_t> message = {cla, ins, p1, p2};
  std::unique_ptr<ApduCommand> cmd =
      ApduCommand::CreateFromMessageForTesting(message);
  ...
  // Invalid length
  message = {cla, ins, p1};
  EXPECT_EQ(nullptr, ApduCommand::CreateFromMessageForTesting(message));
  ...
  message.push_back(0);
  message.push_back(0);
  EXPECT_NE(nullptr, ApduCommand::CreateFromMessageForTesting(message));
  message.push_back(0);
  EXPECT_EQ(nullptr, ApduCommand::CreateFromMessageForTesting(message));
}
...
TEST(ApduTest, TestDeserializeComplex) {
  ...
  auto cmd_no_response = ApduCommand::CreateFromMessageForTesting(message);
  ...
  auto cmd = ApduCommand::CreateFromMessageForTesting(message);
...
TEST(ApduTest, TestDeserializeResponse) {
  ...
  auto status = ApduResponse::Status::SW_CONDITIONS_NOT_SATISFIED;
  ...
  message = {static_cast<uint8_t>(static_cast<uint16_t>(status) >> 8),
             static_cast<uint8_t>(status)};
  std::vector<uint8_t> test_vector({0x01, 0x02, 0xEF, 0xFF});
...
TEST(ApduTest, TestSerializeCommand) {
  ...
  EXPECT_THAT(expected, testing::ContainerEq(
                            ApduCommand::CreateFromMessageForTesting(expected)
                                ->GetEncodedCommand()));
...
TEST(ApduTest, TestSerializeEdgeCases) {
  ...
  EXPECT_THAT(cmd->GetEncodedCommand(),
              testing::ContainerEq(ApduCommand::CreateFromMessageForTesting(
...
//   auto cmd = APDUCommand::CreateU2FSign(appid, challenge, key_handle);
...
//   auto cmd = APDUCommand::CreateU2FRegister(appid, challenge,
...
//   cmd =
//       APDUCommand::CreateU2FRegister(appid, challenge,
//       kIndividualAttestation);
//   ASSERT_NE(nullptr, cmd);
...
//   auto cmd = APDUCommand::CreateU2FVersion();
...
//   auto cmd = APDUCommand::CreateU2FLegacyVersion();
//   // Legacy version command contains 2 extra null bytes compared to ISO
//   7816-4
//   // format
//   std::vector<uint8_t> expected = {
