...
TEST_F(ContentSettingsRegistryTest, GetInitialDefaultSetting) {
  ...
  const ContentSettingsInfo* background_sync =
      registry()->Get(CONTENT_SETTINGS_TYPE_BACKGROUND_SYNC);
  EXPECT_EQ(CONTENT_SETTING_ALLOW, background_sync->GetInitialDefaultSetting());
