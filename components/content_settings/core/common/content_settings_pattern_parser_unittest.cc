...
TEST(ContentSettingsPatternParserTest, ParseChromePatterns) {
  ...
  EXPECT_CALL(builder, WithScheme("chrome-extension")).Times(1).WillOnce(
      ::testing::Return(&builder));
  EXPECT_CALL(builder, WithHost("peoadpeiejnhkmpaakpnompolbglelel")).Times(1)
  ...
  EXPECT_CALL(builder, WithScheme("chrome-search")).Times(1).WillOnce(
      ::testing::Return(&builder));
  EXPECT_CALL(builder, WithHost("local-ntp")).Times(1).WillOnce(
      ::testing::Return(&builder));
  EXPECT_CALL(builder, WithPath("/")).Times(1).WillOnce(
      ::testing::Return(&builder));
  content_settings::PatternParser::Parse(
      "chrome-search://local-ntp/", &builder);
  ::testing::Mock::VerifyAndClear(&builder);
}
