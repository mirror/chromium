...
std::set<std::string>* non_port_non_domain_wildcard_schemes_ = nullptr;
...
// static
void ContentSettingsPattern::PutNonWildcardDomainNonPortScheme(
    const char* scheme) {
  DCHECK(scheme);
  if (!non_port_non_domain_wildcard_schemes_) {
    non_port_non_domain_wildcard_schemes_ = new std::set<std::string>();
  }
  non_port_non_domain_wildcard_schemes_->insert(std::string(scheme));
}

// static
bool ContentSettingsPattern::IsNonWildcardDomainNonPortScheme(
    const std::string& scheme) {
  DCHECK(!non_port_non_domain_wildcard_schemes_->empty());
  return non_port_non_domain_wildcard_schemes_->find(scheme) !=
         non_port_non_domain_wildcard_schemes_->end();
}
