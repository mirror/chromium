...
enum ContentSettingsType {
  ...
  // the browser to remember. Link to Spec:
  // http://httpwg.org/http-extensions/client-hints.html#accept-ch-lifetime.
  // The setting is stored as a dictionary that includes mapping from different
  // client hints to their respective expiration times (seconds since epoch).
  // The browser is expected to send all the unexpired client hints in the HTTP
  // request headers for every resource requested from that origin.
