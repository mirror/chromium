...
enum ContentSettingsType {
  ...
  // The setting is stored as an ordered list of integers and i'th entry
  // in the list corresponds to the expiration time (seconds since epoch) of the
  // corresponding client hint. The browser is expected to send all the
  // unexpired client hints in the HTTP request headers for every resource
  // requested from that origin.
