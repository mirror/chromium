...
class TextEliderTest : public testing::Test {
 public:
  TextEliderTest() = default;
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(TextEliderTest);
};
  ...
  for (const auto& testcase : testcases) {
    ...
    for (size_t i = 0; i < testcase.output.size(); i++) {
      ...
      if (expected_utf16 != elided) {
        LOG(INFO) << "  Expected " << expected_utf16 << ", got " << elided
                  << ", but being tolerant.";
        ...
        LOG(INFO) << "  Too many mismatches";
        EXPECT_EQ(expected_utf16, elided);
        break;
      }
      LOG(INFO) << "  Matched " << elided;
      mismatches = 0;
...
TEST_F(TextEliderTest, TestGeneralEliding) {
...
// case.
TEST_F(TextEliderTest, TestTrailingEllipsisSlashEllipsisHack) {
  const std::string kEllipsisStr(gfx::kEllipsis);
...
TEST_F(TextEliderTest, TestElisionSpecialCases) {
#if defined(OS_WIN)
...
TEST_F(TextEliderTest, TestFileURLEliding) {
  const std::string kEllipsisStr(gfx::kEllipsis);
  const std::vector<ProgressiveTestcase> progressive_testcases = {
...
TEST_F(TextEliderTest, TestHostEliding) {
  const std::string kEllipsisStr(gfx::kEllipsis);
  Testcase testcases[] = {
...
TEST_F(TextEliderTest, FormatUrlForSecurityDisplay) {
...
TEST_F(TextEliderTest, FormatOriginForSecurityDisplay) {
  for (size_t i = 0; i < arraysize(common_tests); ++i) {
