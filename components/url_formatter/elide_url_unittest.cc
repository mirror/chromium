...
base::string16 Elide(const GURL& url,
  ...
  switch (method) {
    case kMethodSecure: {
      url::Parsed parsed;
      return url_formatter::SecurelyElideUrl(url, font_list, available_width,
                                             &parsed);
    }
...
// Test eliding of commonplace URLs.
TEST(TextEliderTest, TestSecureElisionMethod) {
  const std::string kEllipsisStr(gfx::kEllipsis);
  const std::vector<ProgressiveTestcase> testcases = {
  ...
  RunProgressiveElisionTest(testcases, kMethodSecure);
}
...
void RunElisionParsingTest(const std::vector<ParsingTestcase>& testcases) {
  ...
  for (const auto& testcase : testcases) {
    ...
    url::Parsed parsed;
    auto elided = url_formatter::SecurelyElideUrl(url, font_list,
                                                  available_width, &parsed);
