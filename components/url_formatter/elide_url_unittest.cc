...
// character-width variation across platforms, the test allows individual
// expected strings to be skipped mid-run, but not more than one at a time. The
// first and last expected strings must be matched. Example:
//
  ...
  for (const auto& testcase : testcases) {
    ...
    for (size_t i = 0; i < testcase.output.size(); i++) {
      ...
      if (expected_utf16 != elided) {
        if (!must_match_next && i < testcase.output.size() - 1) {
