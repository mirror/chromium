...
// Verify that one or more URLs passes through an explicit sequence of truncated
// strings as available space progressively decreases. This helps ensure that
// transitional corner cases are handled properly.
  ...
  for (const auto& testcase : testcases) {
    ...
    for (const auto& expected : testcase.output) {
      ...
      if (expected_utf16 != elided)
        break;
      width = gfx::GetStringWidthF(elided, font_list) - 1.0f;
    }
  }
}
