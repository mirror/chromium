...
void RunProgressiveElisionTest(
  ...
  for (const auto& testcase : testcases) {
    ...
    // Occasionally, a parsed URL can grow in length.
    ASSERT_FALSE(testcase.output.empty());
    float width = std::max(
        gfx::GetStringWidthF(base::UTF8ToUTF16(testcase.input), font_list),
        gfx::GetStringWidthF(base::UTF8ToUTF16(testcase.output.front()), font_list));
    ...
    for (size_t i = 0; i < testcase.output.size(); i++) {
      ...
      if (expected_utf16 != elided) {
        if (mismatches < max_mismatches && i > 0 && i < testcase.output.size() - 1) {
          mismatches++;
          continue;
        }
