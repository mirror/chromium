...
base::string16 ElideUrl(const GURL& url,
                        const gfx::FontList& font_list,
                        float available_pixel_width) {
  // On Mac, ElideUrl() is only used for the status bar, which uses the same
  // typesetter as the Browser.
...
base::string16 ElideUrlSimple(const GURL& url,
                              ...
                              float available_pixel_width,
                              url::Parsed* parsed) {
  // Note ElideUrlSimple() isn't used on Mac except in tests, but using BROWSER
  // makes it consistent with ElideUrl().
  const gfx::Typesetter typesetter = gfx::Typesetter::BROWSER;
