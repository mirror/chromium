...
const char kReportType[] = "network-error";
...
void NetworkErrorLoggingService::OnNetworkError(
  ...
  if (!GetTypeFromNetError(error, &error_type))
    return;

  ErrorDetails error_details;
  std::move(details_callback).Run(&error_details);

  reporting_service_->QueueReport(
      error_details.uri, policy->report_to, kReportType,
      CreateReportBody(error_type, error_details, tick_clock_->NowTicks()));
}
