...
const char kHostname[] = "flowers-to-the-world.com";
...
constexpr base::TimeDelta kZeroTTL = base::TimeDelta::FromSeconds(0);
...
TEST_F(SingleTreeTrackerTest, DiscardsSCTWhenHostnameNotLookedUpUsingDNS) {
  ...
  // Exactly one value should be logged, indicating the SCT could not be checked
  // for inclusion.
  histograms.ExpectTotalCount(kCanCheckForInclusionHistogramName, 1);
  histograms.ExpectBucketCount(kCanCheckForInclusionHistogramName, 4, 1);
  // Nothing should be logged in the result histogram since an inclusion check
  // wasn't performed.
