...
void SingleTreeTracker::OnSCTVerified(base::StringPiece hostname,
  ...
  // Check that a DNS lookup for hostname has already occurred, and it used the
  // current network (i.e. the DNS resolver already knows that the user has been
  // accessing that host). If not, the DNS resolver may not know that the user
  // has been accessing that host, but performing an inclusion check would
  // reveal that information so abort to preserve the user's privacy.
  net::HostCache::EntryStaleness staleness;
  const net::HostCache::Entry* cache_entry =
      host_resolver_->GetHostCacheEntry(cache_key, &staleness);
  if (cache_entry == nullptr ||
      cache_entry->source() != net::HostCache::Entry::SOURCE_DNS ||
