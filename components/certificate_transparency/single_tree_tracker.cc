...
enum SCTCanBeCheckedForInclusion {
  ...
  // This SCT was not audited because it could compromise the user's privacy,
  // e.g. by leaking their browsing history.
  NOT_AUDITED_FOR_PRIVACY = 5,
...
// The chance that an SCT will be checked for inclusion in the CT log that
// issued it. Not all SCTs should be checked, as this could leak browsing
// history in certain cases (e.g. if the user has a dedicated DNS resolver).
const double kProbabilityOfInclusionCheck = 0.2;
...
void SingleTreeTracker::OnSCTVerified(base::StringPiece hostname,
  ...
  if (base::RandDouble() >= kProbabilityOfInclusionCheck) {
    LogCanBeCheckedForInclusionToUMA(NOT_AUDITED_FOR_PRIVACY);
    return;
  }
