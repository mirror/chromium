...
bool SingleTreeTracker::WasLookedUpOverDNS(base::StringPiece hostname) const {
  ...
  // host_resolver_flags in cache_entry do not match those used for the original
  // DNS lookup. Try another common combination of address_family and
  // host_resolver_flags in an attempt to find a matching cache entry.
  ...
  cache_entry = host_resolver_->GetHostCacheEntry(cache_key, &staleness);
