...
class TestObserver : public OptimizationGuideServiceObserver {
 public:
  ...
  void OnHintsProcessed(const Configuration& config) override {
  ...
  Configuration received_config() const { return received_config_; }
...
TEST_F(OptimizationGuideServiceTest, ProcessHintsIssuesNotification) {
  ...
  Configuration received_config = observer()->received_config();
