...
class ExoPerfTest::WaylandWatcher {
 public:
...
#else
class ExoPerfTest::WaylandWatcher : public base::MessagePumpLibevent::Watcher {
 public:
  explicit WaylandWatcher(exo::wayland::Server* server)
...
ExoPerfTest::ExoPerfTest() {}

ExoPerfTest::~ExoPerfTest() {}

void ExoPerfTest::SetUp() {
...
void ExoPerfTest::RunMessageLoop() {
  DCHECK(!run_loop_);
...
void ExoPerfTest::QuitMessageLoop() {
  DCHECK(run_loop_);
  run_loop_->Quit();
}

void ExoPerfTest::RunClient(const ClientEntry& client_entry) {
  client_thread_->message_loop()->task_runner()->PostTask(
      FROM_HERE, base::Bind(&ExoPerfTest::RunClientOnClientThread,
                            base::Unretained(this), client_entry));
  RunMessageLoop();
}

void ExoPerfTest::RunClientOnClientThread(const ClientEntry& client_entry) {
  result_ = client_entry.Run();
  QuitMessageLoop();
}
