...
ExoTestWindow::ExoTestWindow(std::unique_ptr<gfx::GpuMemoryBuffer> gpu_buffer,
  ...
  if (bounds_mode == ShellSurface::BoundsMode::CLIENT) {
    client_controlled_shell_surface_.reset(
        new ClientControlledShellSurface(surface_.get(), false, container));
  } else {
...
ExoTestWindow ExoTestHelper::CreateWindow(
    int width,
    int height,
    bool is_modal,
    ShellSurface::BoundsMode bounds_mode) {
  return ExoTestWindow(CreateGpuMemoryBuffer(gfx::Size(width, height)),
                       is_modal, bounds_mode);
}

std::unique_ptr<ShellSurface> ExoTestHelper::CreateShellSurface(
    Surface* surface) {
