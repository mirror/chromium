...
class Keyboard : public ui::EventHandler,
 ...
 private:
  ...
  // Process expired key state changes in |pending_key_acks_| as it's not
  // acknowledged.
  ...
  // Indicate that each key event is expected to be acknowledged.
  ...
  typedef std::pair<ui::KeyEvent, base::TimeTicks> KeyStateChange;
