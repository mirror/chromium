...
std::vector<uint8_t> ReadDataOnTaskScheduler(base::ScopedFD fd) {
...
class DataSource::DataSourceReader {
 public:
  DataSourceReader(DataSourceDelegate* delegate, ReadDataCallback callback)
      : callback_(std::move(callback)), weak_ptr_factory_(this) {
    base::ScopedFD read_fd;
    base::ScopedFD write_fd;
    CreatePipe(&read_fd, &write_fd);
    delegate->OnSend(kTextMimeTypeUtf8, std::move(write_fd));

    base::PostTaskWithTraitsAndReplyWithResult(
        FROM_HERE,
        {base::MayBlock(), base::TaskPriority::USER_BLOCKING,
         base::TaskShutdownBehavior::SKIP_ON_SHUTDOWN},
        base::BindOnce(&ReadDataOnTaskScheduler, std::move(read_fd)),
        base::BindOnce(&DataSourceReader::OnDataRead,
 ...
 private:
  ...
  base::WeakPtrFactory<DataSourceReader> weak_ptr_factory_;

  DISALLOW_COPY_AND_ASSIGN(DataSourceReader);
};

DataSource::DataSource(DataSourceDelegate* delegate)
    : delegate_(delegate), cancelled_(false) {}
...
void DataSource::ReadData(ReadDataCallback callback) {
  ...
  // Unretained is OK as DataSource always outlives DataSourceReader.
  data_source_reader_ = std::make_unique<DataSourceReader>(
      delegate_, base::BindOnce(&DataSource::OnDataRead, base::Unretained(this),
                                std::move(callback)));
}

void DataSource::OnDataRead(ReadDataCallback callback,
                            const std::vector<uint8_t>& data) {
  DCHECK(data_source_reader_);
  data_source_reader_.reset();
