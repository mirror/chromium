...
// static
void LayerTreeFrameSinkHolder::DestroyWhenLastResourceHasBeenReclaimed(
  ...
  // by adding it as an aura observer. The holder will destroy itself when aura
...
void LayerTreeFrameSinkHolder::ReclaimResources(
  ...
  if (!surface_tree_host_ && release_callbacks_.empty()) {
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::Bind(&LayerTreeFrameSinkHolder::DeleteHolder,
...
void LayerTreeFrameSinkHolder::OnWillDestroyEnv() {
  DeleteHolder();
}
...
void LayerTreeFrameSinkHolder::DeleteHolder() {
  DCHECK(!surface_tree_host_);
  delete this;
}
