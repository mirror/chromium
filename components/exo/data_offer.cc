...
void WriteFileDescriptorComplete(base::ScopedFD fd,
                                 scoped_refptr<base::RefCountedMemory> memory,
                                 bool result) {
  if (!result)
    LOG(ERROR) << "Failed to write drop data";
}
...
void DataOffer::Receive(const std::string& mime_type, base::ScopedFD fd) {
  ...
  base::PostTaskWithTraitsAndReplyWithResult(
      ...
      base::BindOnce(&base::WriteFileDescriptor, fd.get(),
                     reinterpret_cast<const char*>(it->second->front()),
                     static_cast<int>(it->second->size())),
      base::BindOnce(&WriteFileDescriptorComplete, std::move(fd), it->second));
}
