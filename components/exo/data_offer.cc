...
bool GetUrlListFromDataFile(FileHelper* file_helper,
  ...
  if (data.GetFilenames(&files)) {
    for (const auto& info : files) {
      GURL url;
      // TODO(hirono): Need to fill the correct app_id.
...
bool GetUrlListFromDataPickle(FileHelper* file_helper,
  ...
  if (!file_helper->GetUrlsFromPickle(pickle, &app_urls))
    return false;
  ...
  LOG(ERROR) << "--------------------------- GetUrlListFromDataPickle "
                "url_list_string= "
             << url_list_string;
  return !url_list_string->empty();
}
...
void DataOffer::SetDropData(FileHelper* file_helper,
  ...
  LOG(ERROR) << "--------------------------- app type="
             << static_cast<int>(GetFocusedAppType());
  base::string16 url_list_string;
  bool found_urls =
