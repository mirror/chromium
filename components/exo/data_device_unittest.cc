...
class TestDataDeviceDelegate : public DataDeviceDelegate {
 public:
  ...
  DataOffer* OnDataOffer() override {
    ...
    data_offer_.reset(new DataOffer(nullptr));
...
class DataDeviceTest : public test::ExoTestBase {
 public:
  void SetUp() override {
    ...
    ui::Event::DispatcherApi(&event_).set_target(surface_->window());
  }
 ...
 protected:
  ...
  ui::DropTargetEvent event_{data_, gfx::Point(), gfx::Point(),
...
TEST_F(DataDeviceTest, DeleteDataOfferDuringDrag) {
  ...
  EXPECT_EQ(ui::DragDropTypes::DRAG_NONE, device_->OnDragUpdated(event_));
