...
class DataSourceDelegate {
 public:
  ...
  virtual void Target(const std::string& mime_type) = 0;
  virtual void Send(const std::string& mime_type, base::ScopedFD fd) = 0;
  virtual void Cancelled() = 0;
  virtual void DndDropPerformed() = 0;
  virtual void DndFinished() = 0;
  virtual void Action(uint32_t dnd_action) = 0;
};
