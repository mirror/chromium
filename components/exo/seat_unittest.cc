...
class TestDataSource : public DataSourceDelegate, public DataSource {
 public:
  ...
  bool IsCallbackNull() const { return callback_.is_null(); }
  void RunCallback(std::vector<uint8_t> data, DataSource* source) {
    ASSERT_FALSE(callback_.is_null());
    std::move(callback_).Run(data, source);
  }
...
TEST_F(SeatTest, OnSetSelection) {
  ...
  source.RunCallback(test_data_bytes, &source);
...
TEST_F(SeatTest, OnSetSelection_TwiceSame) {
  ...
  seat.SetSelection(&source);
  source.RunCallback(std::vector<uint8_t>(), &source);

  seat.SetSelection(&source);
  EXPECT_TRUE(source.IsCallbackNull());
}
...
TEST_F(SeatTest, OnSetSelection_TwiceDifferent) {
  ...
  source1.RunCallback(std::vector<uint8_t>(), &source1);

  TestDataSource source2;
  seat.SetSelection(&source2);
  source2.RunCallback(std::vector<uint8_t>(), &source2);
...
TEST_F(SeatTest, OnSetSelection_SourceDestroyed) {
  Seat seat;
  ...
  source.RunCallback(test_data_bytes, nullptr);
