...
void Pointer::SetVisible(bool visible) {
  LOG(ERROR) <<"Peng SetVisible " << visible << " " << cursor_visible_;
  ...
  if (!root_window)
    return;
  LOG(ERROR) <<"Peng SetVisible22222222222 " << visible << " " << cursor_visible_ << " " << root_window;
  aura::client::CursorClient* cursor_client =
      aura::client::GetCursorClient(root_window);
  if (!cursor_client)
    return;
  LOG(ERROR) <<"Peng SetVisible33333333333 " << visible << " " << cursor_visible_ << " " << cursor_client;
  if (visible) {
