 ...
 public:
  ...
  void HandleBoundsRequest(ash::wm::WindowState* window_state,
    ...
    shell_surface_->OnBoundsChangeEvent(
        window_state->GetStateType(), display_id, bounds_in_screen,
...
class ClientControlledShellSurface::ScopedLockToRoot {
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(ScopedLockToRoot);
};
...
void ClientControlledShellSurface::OnBoundsChangeEvent(
  ...
  if (!bounds.IsEmpty() && !bounds_changed_callback_.is_null()) {
    bounds_changed_callback_.Run(current_state, display_id, bounds,
                                 bounds_change);
  }
}
...
gfx::Size ClientControlledShellSurface::GetMaximumSize() const {
  // Chrome treats the window that thas maximum size can'be maximzied
  // even if CanMaximize() return true. Just return empty size because
  // the maximum size is used only to tell the resizability, but not
  // real maximum size in ClientControlledShellSurface.
  return gfx::Size();
}
