...
gfx::Size ClientControlledShellSurface::GetMaximumSize() const {
  ...
  // even if CanMaximize() returns true. Just return empty size because
  // the maximum size in ClientControlledShellSurface is used only to tell
  // the resizability, but not real maximum size.
  return gfx::Size();
}
