...
void ClientControlledState::OnWMEvent(WindowState* window_state,
  ...
  switch (event->type()) {
    ...
    case WM_EVENT_SET_BOUNDS: {
      ...
      LOG(ERROR) << "SETBOUNDS:" << set_bounds_event->requested_bounds().ToString();
...
bool ClientControlledState::ProcessCompoundEvents(WindowState* window_state,
  ...
  switch (event->type()) {
    case WM_EVENT_TOGGLE_MAXIMIZE_CAPTION:
      ...
      return true;
    case WM_EVENT_TOGGLE_MAXIMIZE:
      LOG(ERROR) << "TOGGLE MAXIMIZE";
      if (window_state->IsFullscreen()) {
        LOG(ERROR) << "=> TOGGLE_FULLSCREEN";
        const wm::WMEvent event(wm::WM_EVENT_TOGGLE_FULLSCREEN);
        window_state->OnWMEvent(&event);
      } else if (window_state->IsMaximized()) {
        LOG(ERROR) << "RESTORE";
    ...
    case WM_EVENT_TOGGLE_HORIZONTAL_MAXIMIZE: {
      ...
      if (window_state->IsNormalStateType() &&
        ...
        delegate_->SendBoundsRequest(window_state->GetStateType(), new_bounds);
      }
      return true;
    }
    case WM_EVENT_TOGGLE_FULLSCREEN:
  ...
  switch (event->type()) {
    ...
    case WM_EVENT_DISPLAY_BOUNDS_CHANGED: {
      ...
      if (window_state->window()->GetTargetBounds() != bounds)
        RequestBoundsChange(snapped_bounds);
...
// static
bool ClientControlledState::SetMaximizedOrFullscreenBounds(
    WindowState* window_state) {
...
// static
void ClientControlledState::CenterWindow(WindowState* window_state) {
  if (!window_state->IsNormalOrSnapped())
    return;
  aura::Window* window = window_state->window();
  if (window_state->IsSnapped()) {
