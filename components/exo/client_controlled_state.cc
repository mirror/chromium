...
ClientControlledState::ClientControlledState(
    mojom::WindowStateType initial_state_type)
...
void ClientControlledState::OnWMEvent(WindowState* window_state,
  ...
  switch (event->type()) {
    ...
    case WM_EVENT_TRUSTED_PIN:
      delegate_->SendWindowStateRequest(event->type());
      break;
    case WM_EVENT_SET_BOUNDS: {
      const SetBoundsEvent* set_bounds_event =
          static_cast<const SetBoundsEvent*>(event);
      delegate_->SendBoundsRequest(event->requested_bounds);
      break;
    }
  ...
  switch (event->type()) {
    ...
    case WM_EVENT_TOGGLE_VERTICAL_MAXIMIZE: {
      ...
      if (window_state->HasRestoreBounds() &&
        ...
        delegate_->SendSetBoundsRequest(
    ...
    case WM_EVENT_TOGGLE_HORIZONTAL_MAXIMIZE: {
      ...
      if (window_state->IsNormalStateType() &&
        ...
        delegate_->SendSetBoundsRequest(new_bounds);
      }
      return true;
    }
    case WM_EVENT_TOGGLE_FULLSCREEN:
