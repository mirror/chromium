        ...
        if (focus_ && !consumed_by_ime) {
          ...
          if (are_keyboard_key_acks_needed_)
            pending_key_acks_.insert(
                {serial, {*event, base::TimeTicks::Now()}});
        }
...
void Keyboard::ProcessExpiredPendingKeyAcks() {
  ...
  while (it != pending_key_acks_.end()) {
    ...
    base::TimeTicks expiration_time_for_event =
        it->second.second + expiration_delay_for_pending_key_acks_;

    if (expiration_time_for_event > base::TimeTicks::Now())
      break;
  ...
  base::TimeDelta delay_for_next_expiration =
      pending_key_acks_.begin()->second.second +
      expiration_delay_for_pending_key_acks_ - base::TimeTicks::Now();
  ScheduleProcessExpiredPendingKeyAcks(delay_for_next_expiration);
}
