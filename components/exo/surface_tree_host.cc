...
void SurfaceTreeHost::SetRootSurface(Surface* root_surface) {
  DCHECK(host_window_);
  DCHECK_NE(root_surface, root_surface_);
  if (root_surface_) {
    ...
    root_surface_->RemoveSurfaceObserver(this);
  ...
  if (root_surface) {
    ...
    root_surface_->AddSurfaceObserver(this);
...
void SurfaceTreeHost::DestroyHostWindow() {
