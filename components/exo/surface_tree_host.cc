...
void SurfaceTreeHost::OnSurfaceCommit() {
  Composite(false /* full_damage */, true /* needs_commit */);
}
...
void SurfaceTreeHost::OnLostResources() {
  ...
  Composite(true /* full_damage */, false /* needs_commit */);
}

void SurfaceTreeHost::Composite(bool full_damage, bool needs_commit) {
  DCHECK(root_surface_);
  ...
  root_surface_->CommitSurfaceHierarchy(
      ...
      &frame_callbacks_, &presentation_callbacks_, full_damage, needs_commit);
