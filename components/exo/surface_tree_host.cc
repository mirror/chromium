...
void SurfaceTreeHost::OnSurfaceCommit() {
  SubmitCompositorFrame(true /* do_commit */);
}
...
void SurfaceTreeHost::OnLostResources() {
  ...
  SubmitCompositorFrame(false /* do_commit */);
}

void SurfaceTreeHost::SubmitCompositorFrame(bool do_commit) {
  DCHECK(root_surface_);
  ...
  root_surface_->DrawSurfaceHierarchy(gfx::Point(), device_scale_factor,
  ...
  // If the frame is not for commit, we need set the damage rect to the whole
  // output rect.
  if (!do_commit) {
