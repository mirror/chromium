...
void SurfaceTreeHost::OnSurfaceCommit() {
  gfx::Rect bounds;
  bool opaque;
  std::tie(bounds, opaque) = root_surface_->CommitSurfaceHierarchy(
      gfx::Point(), &frame_callbacks_, &presentation_callbacks_);

  host_window_->SetBounds(
      gfx::Rect(host_window_->bounds().origin(), bounds.size()));
  host_window_->layer()->SetFillsBoundsOpaquely(opaque);

  OnSubmitCompositorFrame(!bounds.IsEmpty());
