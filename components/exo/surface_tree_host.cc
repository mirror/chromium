...
void SurfaceTreeHost::OnSurfaceCommit() {
  constexpr bool full_damage = false;
  constexpr bool commit_if_necessary = true;
  Composite(full_damage, commit_if_necessary);
}
...
void SurfaceTreeHost::OnLostResources() {
  if (!host_window_->GetSurfaceId().is_valid())
    return;
  root_surface_->RestoreResources(layer_tree_frame_sink_holder_.get());
  constexpr bool full_damage = true;
  constexpr bool commit_if_necessary = false;
  Composite(full_damage, commit_if_necessary);
}

void SurfaceTreeHost::Composite(bool full_damage, bool commit_if_necessary) {
  DCHECK(root_surface_);
  ...
  root_surface_->CommitSurfaceHierarchy(
      gfx::Point(0, 0), &frame, layer_tree_frame_sink_holder_.get(),
      &frame_callbacks_, &presentation_callbacks_, full_damage,
      commit_if_necessary);
