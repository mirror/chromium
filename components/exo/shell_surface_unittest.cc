...
bool DoesShellSurfaceHaveBackdrop(
    std::unique_ptr<ShellSurface>& shell_surface) {
  return shell_surface->GetWidget()->GetNativeWindow()->GetProperty(
...
TEST_P(ShellSurfaceBoundsModeTest, Maximize) {
  ...
  ash::wm::GetWindowState(window)->OnWMEvent(&maximize_event);
  EXPECT_TRUE(shell_surface->GetWidget()->IsMaximized());
  // Put the backdrop only when it is instructed from client.
  EXPECT_FALSE(HasBackdrop());
}
...
TEST_P(ShellSurfaceBoundsModeTest, SetFullscreen) {
  ...
  surface->Attach(buffer.get());
  surface->Commit();
  EXPECT_EQ(IsClientBoundsMode(), DoesShellSurfaceHaveBackdrop(shell_surface));
  if (!IsClientBoundsMode()) {
  ...
  surface->Commit();
  EXPECT_FALSE(DoesShellSurfaceHaveBackdrop(shell_surface));
