...
void ShellSurface::OnSurfaceCommit() {
  ...
  if (pending_window_state_ != ui::SHOW_STATE_DEFAULT) {
      if (pending_geometry_ != geometry_) {
        // Sometimes OnSurfaceCommit delivers a frame still for the previous window state.
        // This is clearly wrong, and should be fixed. For now, animate only when the new
        // frame has a new geometry, which means that most likely it's for the new window
        // state. b/XYZ.
