...
void ShellSurface::SetWindowSelectorBehavior(uint32_t behavior) {
  if (!widget_)
    CreateShellSurfaceWidget(ui::SHOW_STATE_NORMAL);

  widget_->GetNativeWindow()->SetProperty(
      aura::client::kWindowSelectorBehaviorKey,
      static_cast<aura::client::WindowSelectorBehavior>(behavior));
}
