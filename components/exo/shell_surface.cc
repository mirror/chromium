...
void ShellSurface::OnSurfaceCommit() {
  // When the shadow underlay is in the surface and the surface's bounds is
  // changed, ARC++ wants to synchronize the shadow bounds change with the next
  // frame, so we have to submit the next frame to a new surface, and let the
  // host_window() uses the new surface.
