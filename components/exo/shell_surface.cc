...
void ShellSurface::SetFullscreen(bool fullscreen) {
  ...
  LOG(ERROR) << "SetFullscreen:" << fullscreen;
...
void ShellSurface::OnPostWindowStateTypeChange(
  ...
  if (bounds_mode_ != BoundsMode::CLIENT) {
    if (old_type != new_type)
      SendWindowStateRequest(old_type, new_type);
  }
...
void ShellSurface::CreateShellSurfaceWidget(ui::WindowShowState show_state) {
  DCHECK(enabled());
  DCHECK(!widget_wrapper_);
  LOG(ERROR) << "CreateShellSurfaceWidget:" << show_state;
  ...
  widget_wrapper_ = ShellSurfaceWidgetWrapper::Create(bounds_mode_);
  views::Widget* widget = GetWidget();
  widget->Init(params);
...
bool ShellSurface::ToggleFullscreen(ash::mojom::WindowStateType current_state) {
  ...
  SendWindowStateRequest(current_state, next_state);
  return true;
}
