...
void ShellSurface::SetSystemModal(bool system_modal) {
  ...
  bool non_modal_system_window_was_active = !system_modal_ && widget_ && widget_->IsActive();
  ...
  if (widget_) {
    ...
    if (!system_modal_ && !non_modal_system_window_was_active_) {
      widget_->Deactivate();
    }
  }

  non_modal_system_window_was_active_ = non_modal_system_window_was_active;
}
