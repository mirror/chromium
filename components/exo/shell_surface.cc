...
void ShellSurface::OnSurfaceCommit() {
  ...
  if (pending_window_state_ != ui::SHOW_STATE_DEFAULT) {
    // For ARC Wayland client OnSurfaceCommit may deliver a frame still for the
    // previous window state, causing pending_window_state_ and
    // pending_geometry_ to be inconsistent. As a temporary workaround, disable
    // animations for such commits.
    // TODO(mtomasz): Fix ARC synchronization. b/65271032
    if (pending_geometry_ != geometry_)
      next_frame_is_good_for_crossfade_ = true;
    ...
    next_frame_is_good_for_crossfade_ = false;
  }
...
void ShellSurface::OnPreWindowStateTypeChange(
  ...
  if (!next_frame_is_good_for_crossfade_) {
