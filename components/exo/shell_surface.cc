...
class CustomWindowResizer : public ash::WindowResizer {
 public:
  ...
  void CompleteDrag() override {
    LOG(ERROR) << "CompleteDrag";
  }
  void RevertDrag() override {
    LOG(ERROR) << "RevertDrag";
  }
...
void ShellSurface::AttemptToStartDrag(int component) {
  ...
  if (bounds_mode_ == BoundsMode::SHELL) {
    ...
    std::unique_ptr<ash::WindowResizer> resizer(ash::WorkspaceWindowResizer::Create(window_state, std::vector<aura::Window*>()));
    resizer_ = ash::ShellPort::Get()->CreateDragWindowResizer(std::move(resizer), window_state);
...
void ShellSurface::SendBoundsChangeEvent(
    ...
    bool drag,
    bool resize) {
  LOG(ERROR) << "Set Bounds:" << bounds.ToString()
             << ", drag=" << drag
             << ", resize==" << resize
