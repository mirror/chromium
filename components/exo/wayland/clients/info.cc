...
std::string AuraOutputScaleFactorToString(int32_t scale) {
  switch (scale) {
    ...
    case ZAURA_OUTPUT_SCALE_FACTOR_2000:
      return base::StringPrintf("%f", scale / 1000.0);
    default:
      return base::StringPrintf("unknown (%f)", scale / 1000.0);
  }
}
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Geometry: " << std::endl
            ...
            << std::endl
            << "\tPhysical height:\t" << info.geometry.physical_height << " mm"
            << std::endl
            << "\tSubpixel:\t\t"
  ...
  for (auto& mode : info.modes) {
    std::cout << "\t" << mode.width << "x" << mode.height << "\t\t"
              << (mode.refresh / 1000.0) << " Hz"
              << "\t\t" << OutputModeFlagsToString(mode.flags) << std::endl;
  }
  std::cout << std::endl;
  std::cout << "DSF:\t" << info.device_scale_factor << std::endl;
  if (!info.scales.empty()) {
    std::cout << std::endl;
    std::cout << "Scales: " << std::endl;
    for (auto& scale : info.scales) {
      std::cout << "\t" << AuraOutputScaleFactorToString(scale.scale) << "\t\t"
