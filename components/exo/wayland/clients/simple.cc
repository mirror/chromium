...
void FrameCallback(void* data, wl_callback* callback, uint32_t time) {
  ...
  DCHECK_EQ(presentation->frame_callback.get(), callback);
  presentation->frame_callback = nullptr;
...
void Simple::Run(int frames, Result* result) {
  ...
  do {
    ...
    presentation.frame_callback.reset(wl_surface_frame(surface_.get()));
    wl_callback_add_listener(presentation.frame_callback.get(), &frame_listener,
                             &presentation);
