...
TEST_F(WaylandClientPerfTests, Simple) {
  ...
  if (result.num_frames_presented) {
    ...
    perf_test::PrintResult(
        ...
        (size_t)average_latency.InMicroseconds(), "us", true);
    auto average_ipc_latency =
        result.presentation_ipc_latency / result.num_frames_presented;
    perf_test::PrintResult(
        "WaylandClientPerfTests", "", "Simple::presentation_ipc_latency",
        (size_t)average_ipc_latency.InMicroseconds(), "us", true);
  } else {
