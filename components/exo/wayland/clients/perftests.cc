...
TEST_F(WaylandClientPerfTests, Simple) {
  ...
  perf_test::PrintResult("WaylandClientPerfTests", "", "Simple::frame_rate",
                         fps, "frames/s", true);
  if (result.frame_latency != base::TimeDelta()) {
    perf_test::PrintResult(
        "WaylandClientPerfTests", "", "Simple::frame_latency",
        (size_t)result.frame_latency.InMicroseconds(), "us", true);
  } else {
    perf_test::PrintResult("WaylandClientPerfTests", "",
                           "Simple::frame_latency", "(not avaliable)", "us",
                           true);
  }
  if (result.presentation_latency != base::TimeDelta()) {
    perf_test::PrintResult(
        "WaylandClientPerfTests", "", "Simple::presentation_latency",
        (size_t)result.presentation_latency.InMicroseconds(), "us", true);
  } else {
    perf_test::PrintResult("WaylandClientPerfTests", "",
                           "Simple::presentation_latency", "(not avaliable)",
                           "us", true);
  }

  if (result.presentation_ipc_latency != base::TimeDelta()) {
    perf_test::PrintResult(
        "WaylandClientPerfTests", "", "Simple::presentation_ipc_latency",
        (size_t)result.presentation_ipc_latency.InMicroseconds(), "us", true);
  } else {
    perf_test::PrintResult("WaylandClientPerfTests", "",
                           "Simple::presentation_ipc_latency",
                           "(not avaliable)", "us", true);
  }
}
