...
VkResult VulkanInitialize(VkInstance* instance,
                          VkDevice* device,
                          VkQueue* queue,
  ...
  uint32_t queue_family_index =
      VulkanChooseGraphicsQueueFamily(physical_device);
...
bool ClientBase::Init(const InitParams& params) {
  ...
  if (params.use_drm) {
    ...
    VkResult result = VulkanInitialize(&vk_instance, &vk_device, &vk_queue_,
                                       &vk_command_pool);

    // Take ownership of Vulkan handles.
    vk_instance_.reset(new ScopedVkInstance(vk_instance));
    vk_device_.reset(new ScopedVkDevice(vk_device));
    vk_command_pool_.reset(
        new ScopedVkCommandPool(vk_command_pool, {vk_device}));
    ...
    vk_render_pass_.reset(new ScopedVkRenderPass(vk_render_pass, {vk_device}));
    CHECK_EQ(VK_SUCCESS, result);
  ...
  if (device_) {
...
#if defined(USE_VULKAN)
    {
      ...
      PFN_vkCreateDmaBufImageINTEL create_dma_buf_image_intel =
          reinterpret_cast<PFN_vkCreateDmaBufImageINTEL>(vkGetDeviceProcAddr(
              vk_device_->get(), "vkCreateDmaBufImageINTEL"));
      ...
      buffer->vk_memory.reset(
          new ScopedVkDeviceMemory(vk_device_memory, {vk_device_->get()}));
      buffer->vk_image.reset(new ScopedVkImage(vk_image, {vk_device_->get()}));
      ...
      buffer->vk_image_view.reset(
          new ScopedVkImageView(vk_image_view, {vk_device_->get()}));
      if (result != VK_SUCCESS) {
      ...
      buffer->vk_framebuffer.reset(
          new ScopedVkFramebuffer(vk_framebuffer, {vk_device_->get()}));
    }
#endif  // defined(USE_VULKAN)
  }
#endif  // defined(USE_GBM)
