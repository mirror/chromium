...
uint32_t VulkandChooseGraphicsQueueFamily(VkPhysicalDevice device) {
  ...
  properties.resize(properties_number);
...
VkResult VulkanInitialize(VkDevice* device,
                          VkQueue* queue,
  ...
  VkInstance instance;
  result = vkCreateInstance(&create_info, allocator, &instance);
  if (result != VK_SUCCESS) {
    LOG(ERROR) << "Failed to create a Vulkan instance. Do you have an ICD "
                  "driver (e.g: "
  ...
  result = vkEnumeratePhysicalDevices(instance, &physical_devices_number,
                                      &physical_device);
  if (result != VK_SUCCESS) {
  ...
  uint32_t queue_family_index =
      VulkandChooseGraphicsQueueFamily(physical_device);
  DCHECK_NE(UINT32_MAX, queue_family_index);

  float prioritiess[] = {1.0f};
  VkDeviceQueueCreateInfo device_queue_create_info[]{
      {
          .sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO,
          .queueFamilyIndex = queue_family_index,
          .queueCount = 1,
          .pQueuePriorities = prioritiess,
  ...
  result =
      vkCreateDevice(physical_device, &device_create_info, allocator, device);
  if (result != VK_SUCCESS) {
    LOG(ERROR) << "Can't create Vulkan device.";
    return result;
  }
  ...
  result = vkCreateCommandPool(*device, &command_pool_create_info, nullptr,
                               command_pool);
  if (result != VK_SUCCESS) {
    LOG(ERROR) << "Can't create Vulkan command pool.";
    return result;
  }
  return result;
}
...
bool ClientBase::Init(const InitParams& params) {
  ...
  if (params.use_drm) {
    ...
    VkResult result =
        VulkanInitialize(&vk_device_, &vk_queue_, &vk_command_pool_);
    ...
    VkSubpassDescription subpass_desk[]{
        {
    ...
    VkRenderPassCreateInfo create_pass_info{
        .sType = VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO,
        .attachmentCount = 1,
        .pAttachments = attach_desc,
        .subpassCount = 1,
        .pSubpasses = subpass_desk,
    };
    result = vkCreateRenderPass(vk_device_, &create_pass_info, nullptr,
                                &vk_renderpass_);
    CHECK_EQ(VK_SUCCESS, result);
...
#if defined(USE_VULKAN)
    {
      PFN_vkCreateDmaBufImageINTEL create_dma_buf_image_intel =
          reinterpret_cast<PFN_vkCreateDmaBufImageINTEL>(
              vkGetDeviceProcAddr(vk_device_, "vkCreateDmaBufImageINTEL"));
      ...
      VkDmaBufImageCreateInfo dma_buf_image_create_info{
          ...
          .fd = gbm_bo_get_plane_fd(buffer->bo.get(), 0),
          .format = VK_FORMAT_A8B8G8R8_UNORM_PACK32,
      ...
      VkResult result = create_dma_buf_image_intel(
          vk_device_, &dma_buf_image_create_info, nullptr, &buffer->vk_memory,
          &buffer->vk_image);
      ...
      VkImageViewCreateInfo vk_iamge_view_create_info{
          .sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
          .image = buffer->vk_image,
      ...
      result = vkCreateImageView(vk_device_, &vk_iamge_view_create_info,
                                 nullptr, &buffer->vk_image_view);
      if (result != VK_SUCCESS) {
      ...
      VkFramebufferCreateInfo vk_framebuffer_create_info{
          ...
          .attachmentCount = 1,
          .pAttachments = &buffer->vk_image_view,
          .width = size.width(),
          .height = size.height(),
          .layers = 1,
      };
      result = vkCreateFramebuffer(vk_device_, &vk_framebuffer_create_info,
                                   nullptr, &buffer->vk_framebuffer);
      if (result != VK_SUCCESS) {
      ...
      result =
          vkAllocateCommandBuffers(vk_device_, &command_buffer_allocate_info,
