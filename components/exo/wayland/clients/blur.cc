    ...
    if (blur_filter) {
      ...
      while (!blur_surfaces.empty()) {
        ...
        if (content_surfaces.empty()) {
          canvas->scale(
              static_cast<double>(surface->width()) / size_.width(),
              static_cast<double>(surface->height()) / size_.height());
      ...
      while (!content_surfaces.empty()) {
        blur_surfaces.push_back(content_surfaces.back());
        content_surfaces.pop_back();
      }
    } else {
      SkCanvas* canvas = buffer->sk_surface->getCanvas();
      DrawContents(grid_image_.get(), cell_size, elapsed_time, canvas);
