...
void surface_set_buffer_transform(wl_client* client,
  ...
  // |transform| is counter clockwise and |rotation| is clockwise.
  switch (transform) {
  ...
  GetUserDataAs<Surface>(resource)->SetBufferTransform(rotation);
}
...
wl_output_transform OutputTransform(display::Display::Rotation rotation) {
  // |rotation| is clockwise and wl_output_transform is counter clockwise.
  switch (rotation) {
...
class WaylandRemoteShell : public WMHelper::TabletModeObserver,
 ...
 private:
  ...
  wl_output_transform DisplayTransform(display::Display::Rotation rotation) {
    // Wayland output transforms are typically counter clockwise but
    // remote shell sends clockwise transform.
    switch (rotation) {
      case display::Display::ROTATE_0:
        return WL_OUTPUT_TRANSFORM_NORMAL;
      case display::Display::ROTATE_90:
