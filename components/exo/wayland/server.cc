...
void remote_surface_set_can_resize(wl_client* client,
                                   wl_resource* resource,
                                   uint32_t can_resize) {
  GetUserDataAs<ClientControlledShellSurface>(resource)->SetCanResize(
      can_resize != 0);
}
...
void remote_surface_set_can_maximize(wl_client* client,
  ...
  GetUserDataAs<ClientControlledShellSurface>(resource)->SetCanMaximize(
      can_maximize != 0);
}
...
void HandleRemoteSurfaceDragStartedCallback(wl_resource* resource,
                                            int component) {
  zcr_remote_surface_v1_send_drag_started(resource,
...
void HandleRemoteSurfaceDragFinishedCallback(wl_resource* resource,
                                             bool canceled) {
  zcr_remote_surface_v1_send_drag_finished(resource, canceled ? 1 : 0);
  wl_client_flush(wl_resource_get_client(resource));
}
