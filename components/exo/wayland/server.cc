 ...
 public:
  WaylandRemoteShell(Display* display, wl_resource* remote_shell_resource)
    ...
    if (wl_resource_get_version(remote_shell_resource_) >= 8) {
      ...
      // Send using int by multiplying 1000 to avoid conversion error.
      const int kMultiplier = 1000;
      zcr_remote_shell_v1_send_default_device_pixel_ratio(
          remote_shell_resource_,
          static_cast<int32_t>(scale_factor * kMultiplier));
    }
