...
std::vector<std::pair<int32_t, uint64_t>> dmabuf_supported_attributes = {
...
void linux_buffer_params_create(wl_client* client,
                                ...
                                uint32_t format,
                                uint32_t flags) {
  const auto supported_attribute = std::find_if(
      std::begin(dmabuf_supported_attributes),
      std::end(dmabuf_supported_attributes),
      [format](const std::pair<int32_t, uint64_t>& supported_attribute) {
        return supported_attribute.first == (int32_t)format;
      });
  ...
  gfx::BufferFormat buffer_format =
      ui::GetBufferFormatFromFourCCFormat(supported_attribute->first);
...
void bind_linux_dmabuf(wl_client* client,
  ...
  std::vector<std::pair<int32_t, uint64_t>> attribs =
  ...
  for (const auto attrib : dmabuf_supported_attributes) {
    uint32_t modifier_lo = attrib.second & 0xFFFFFFFF;
    uint32_t modifier_hi = attrib.second >> 32;
    zwp_linux_dmabuf_v1_send_modifier(resource, attrib.first, modifier_hi,
                                      modifier_lo);
  }
}
