...
class WaylandDataSourceDelegate : public DataSourceDelegate {
 public:
  ...
  void OnCancelled() override { wl_data_source_send_cancelled(source_); }

  void OnDndDropPerformed() override {
    wl_data_source_send_dnd_drop_performed(source_);
  }

  void OnDndFinished() override { wl_data_source_send_dnd_finished(source_); }

  void OnAction(uint32_t dnd_action) override {
...
class WaylandDataOfferDelegate : public DataOfferDelegate {
 public:
  ...
  void OnSourceActions(uint32_t source_actions) override {
    wl_data_offer_send_source_actions(offer_, source_actions);
  }

  void OnAction(uint32_t action) override {
...
class WaylandDataDeviceDelegate : public DataDeviceDelegate {
 public:
  ...
  class DataOffer* OnDataOffer(std::vector<std::string> mime_types,
                               uint32_t source_actions,
                               uint32_t dnd_action) override {
  ...
  void OnLeave() override { wl_data_device_send_leave(device_); }

  void OnMotion(uint32_t time, float x, float y) override {
    wl_data_device_send_motion(device_, time, wl_fixed_from_double(x),
                               wl_fixed_from_double(y));
  }

  void OnDrop() override { wl_data_device_send_drop(device_); }
