  ...
  WaylandRemoteShell(Display* display, wl_resource* remote_shell_resource)
    ...
    if (wl_resource_get_version(remote_shell_resource_) >= 8) {
      float scale_factor=
          ...
          ? WMHelper::GetInstance()->GetDefaultDeviceScaleFactor() : 1.0;
      int32_t scale_factor_x100 = static_cast<int32_t>(scale_factor * 100);
      int32_t density_dpi =
          static_cast<int32_t>(std::max(1.0f, scale_factor * 0.75f) * 160);
      int32_t notification_density_dpi =
          static_cast<int32_t>(std::max(1.0f, scale_factor * 0.875f) * 160);
