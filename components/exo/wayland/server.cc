...
void HandleRemoteSurfaceStatesAndBoundsChangedCallback(
    wl_resource* resource,
    ash::mojom::WindowStateType current_state_type,
    ash::mojom::WindowStateType new_state_type,
    int64_t display_id,
    const gfx::Rect& bounds) {
  DCHECK(new_state_type == ash::mojom::WindowStateType::LEFT_SNAPPED ||
         new_state_type == ash::mojom::WindowStateType::RIGHT_SNAPPED);

  zcr_remote_surface_v1_bounds_change_reason reason =
      (new_state_type == ash::mojom::WindowStateType::LEFT_SNAPPED)
          ? ZCR_REMOTE_SURFACE_V1_BOUNDS_CHANGE_REASON_SNAP_TO_LEFT
...
void remote_shell_get_remote_surface(wl_client* client,
  ...
  if (wl_resource_get_version(remote_surface_resource) >= 11) {
    shell_surface->set_state_bounds_changed_callback(
        base::BindRepeating(&HandleRemoteSurfaceStatesAndBoundsChangedCallback,
                            base::Unretained(remote_surface_resource)));
  }
