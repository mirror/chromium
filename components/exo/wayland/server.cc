...
#include "components/exo/dnd_action.h"
...
uint32_t DndActionToUint32(DndAction action) {
  switch (action) {
...
uint32_t DndActionsToUint32(const base::flat_set<DndAction>& dnd_actions) {
  uint32_t actions = 0;
  for (DndAction action : dnd_actions) {
    actions |= DndActionToUint32(action);
  }
  return actions;
}

DndAction Uint32ToDndAction(uint32_t value) {
  switch (value) {
...
base::flat_set<DndAction> Uint32ToDndActions(uint32_t value) {
...
void data_source_set_actions(wl_client* client,
  ...
  GetUserDataAs<DataSource>(resource)->SetActions(
      Uint32ToDndActions(dnd_actions));
}
...
class WaylandDataOfferDelegate : public DataOfferDelegate {
 public:
  ...
  void OnSourceActions(
    ...
    wl_data_offer_send_source_actions(data_offer_resource_,
                                      DndActionsToUint32(source_actions));
  }
  void OnAction(DndAction action) override {
    wl_data_offer_send_action(data_offer_resource_, DndActionToUint32(action));
  }
...
void data_offer_set_actions(wl_client* client,
  ...
  GetUserDataAs<DataOffer>(resource)->SetActions(
      Uint32ToDndActions(dnd_actions), Uint32ToDndAction(preferred_action));
}
...
class WaylandDataDeviceDelegate : public DataDeviceDelegate {
 public:
  WaylandDataDeviceDelegate(wl_client* client, wl_resource* device_resource)
      : client_(client), data_device_resource_(device_resource), serial_(0u) {}
  ...
  class DataOffer* OnDataOffer(const std::vector<std::string>& mime_types,
    ...
    std::unique_ptr<DataOffer> data_offer = base::MakeUnique<class DataOffer>(
  ...
  void OnEnter(Surface* surface,
    ...
    wl_data_device_send_enter(
        data_device_resource_, serial_++, GetSurfaceResource(surface),
...
void data_device_start_drag(wl_client* client,
                            ...
                            uint32_t serial) {
  // TODO(hirono): Check if serial is valid.
  GetUserDataAs<DataDevice>(resource)->StartDrag(
      ...
      GetUserDataAs<Surface>(icon_resource));
}
...
void data_device_set_selection(wl_client* client,
  ...
  GetUserDataAs<DataDevice>(resource)->SetSelection(
      *GetUserDataAs<DataSource>(data_source));
}
