...
class WaylandInputDelegate
    : public base::SupportsWeakPtr<WaylandInputDelegate> {
 public:
  class SendTimestampObserver {
   public:
    virtual ~SendTimestampObserver() = default;

    virtual void OnSendTimestamp(base::TimeTicks time_stamp) = 0;
  };

  void AddSendTimestampObserver(SendTimestampObserver* observer) {
    observers_.AddObserver(observer);
  }

  void RemoveSendTimestampObserver(SendTimestampObserver* observer) {
 ...
 private:
  base::ObserverList<SendTimestampObserver> observers_;
...
class WaylandInputTimestamps
    : public WaylandInputDelegate::SendTimestampObserver {
 public:
  WaylandInputTimestamps(wl_resource* resource,
                         base::WeakPtr<WaylandInputDelegate> delegate)
      : resource_(resource), delegate_(delegate) {
    if (delegate_)
      delegate_->AddSendTimestampObserver(this);
  }

  ~WaylandInputTimestamps() override {
    if (delegate_)
      delegate_->RemoveSendTimestampObserver(this);
  }

  // Overridden from WaylandInputDelegate::SendTimestampObserver:
  void OnSendTimestamp(base::TimeTicks time_stamp) override {
    timespec ts = (time_stamp - base::TimeTicks()).ToTimeSpec();

    zwp_input_timestamps_v1_send_timestamp(resource_, (uint64_t)ts.tv_sec >> 32,
                                           ts.tv_sec & 0xffffffff, ts.tv_nsec);
  }

 private:
  wl_resource* const resource_;
  base::WeakPtr<WaylandInputDelegate> delegate_;
...
void input_timestamps_manager_get_timestamps(wl_client* client,
  ...
  auto* delegate = static_cast<WaylandInputDelegate*>(
  ...
  auto input_timestamps = std::make_unique<WaylandInputTimestamps>(
      input_timestamps_resource, delegate->AsWeakPtr());
...
#if !BUILDFLAG(USE_XKBCOMMON)
void input_timestamps_manager_get_timestamps_no_impl(
    wl_client* client,
    wl_resource* resource,
    uint32_t id,
    wl_resource* input_resource) {
...
#else
        input_timestamps_manager_get_timestamps_no_impl,
#endif
