...
TEST_F(DataSourceTest, ReadData) {
  ...
  data_source.ReadData(base::BindOnce(
      [](const std::vector<uint8_t>& data, exo::DataSource* source) {
  ...
  TestDataSourceDelegate delegate;
  {
    ...
    data_source.ReadData(base::BindOnce(
        [](const std::vector<uint8_t>& data, exo::DataSource* source) {
          std::string string_data(data.begin(), data.end());
          EXPECT_EQ(std::string(kTestData), string_data);
          // source must be null if DataSource is destroyed while reading data.
          EXPECT_FALSE(source);
        }));
  }
  scoped_task_environment_.RunUntilIdle();
