...
NotificationSurface::NotificationSurface(NotificationSurfaceManager* manager,
    ...
    : SurfaceTreeHost(new CustomWindowDelegate(this)),
      manager_(manager),
...
void NotificationSurface::OnSurfaceTreeCommitted() {
  gfx::Rect bounds = window_->bounds();
  auto& size = window_->bounds().size();
  if (bounds.size() != size) {
...
void NotificationSurface::OnSurfaceDetached() {
  window_.reset();
}
