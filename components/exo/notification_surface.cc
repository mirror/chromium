...
class CustomWindowDelegate : public aura::WindowDelegate {
 public:
  explicit CustomWindowDelegate(SurfaceTreeHost* surface_tree_host)
  ...
  gfx::NativeCursor GetCursor(const gfx::Point& point) override {
    return surface_tree_host_->GetCursor(point);
  }
...
NotificationSurface::NotificationSurface(NotificationSurfaceManager* manager,
    ...
    : manager_(manager), notification_key_(notification_key) {
  surface_tree_host_ = base::MakeUnique<SurfaceTreeHost>(this);
  surface_tree_host_->AttachSurface(surface);
  window_ = base::MakeUnique<aura::Window>(
      new CustomWindowDelegate(surface_tree_host_.get()));
...
void NotificationSurface::OnSurfaceTreeCommit() {
  gfx::Rect bounds = window_->bounds();
  if (bounds.size() != GetSize()) {
    bounds.set_size(GetSize());
    window_->SetBounds(bounds);
  }

  // Defer AddSurface until there are contents to show.
  if (!added_to_manager_ && !GetSize().IsEmpty()) {
    added_to_manager_ = true;
    manager_->AddSurface(this);
  }
}

void NotificationSurface::OnSurfaceDetached(Surface* surface) {
  window_.reset();
}
