...
TEST_F(BufferTest, SurfaceTreeHostDestruction) {
  ...
  // Destroy surface tree host. Resources are reclaimed at destruction and so
  // the weak_frame_sink_holder will become invalid.
  surface_tree_host.reset();
  ASSERT_EQ(release_call_count, 1);
  EXPECT_FALSE(weak_frame_sink_holder);
}
