...
class CustomWindowTargeter : public aura::WindowTargeter {
 public:
  ...
  bool EventLocationInsideBounds(aura::Window* window,
    ...
    if (!client_controlled_move_resize_ &&
        window == widget_->GetNativeWindow() &&
 ...
 private:
  bool IsInResizeHandle(aura::Window* window,
    ...
    // For resizeable ClientControlledShellSurface, try the
    // container's targeter first, if any, which may provide the
    // targeting logic for resize handler for mouse and touch.
    // TODO(oshima): Consider doing the same for xdg shell.
