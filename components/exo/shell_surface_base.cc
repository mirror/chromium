...
class CustomWindowTargeter : public aura::WindowTargeter {
 public:
  ...
  bool EventLocationInsideBounds(aura::Window* window,
    ...
    if (widget_->GetNativeWindow() == window && widget_->widget_delegate()->CanResize()) {
      // For resizeable ARC window, try the container's targeter first, if any, which
      // may provide the outsets to start resizing for mouse and touch.
      ui::EventTarget* parent = static_cast<ui::EventTarget*>(window)->GetParentTarget();
      if (parent) {
        ...
        if (parent_targeter) {
          ...
          if (parent_targeter->GetHitTestRects(window, &mouse_rect,
            ...
            if (event.IsTouchEvent() || event.IsGestureEvent() ?
                touch_rect.Contains(local_point) : mouse_rect.Contains(local_point)) {
              return true;
            }
          }
        }
