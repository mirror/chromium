...
// The accelerator keys used to close ShellSurfaceBases.
const struct {
...
void ShellSurfaceBase::SetSystemModal(bool system_modal) {
  ...
  // managed system modal dialogs using a single ShellSurfaceBase instance.
...
void ShellSurfaceBase::OnSurfaceCommit() {
  ...
  if (widget_) {
    ...
    // windows using a single ShellSurfaceBase instance.  If hit-test
...
void ShellSurfaceBase::OnSurfaceDestroying(Surface* surface) {
  ...
  // destroyed callback may destroy the ShellSurfaceBase instance. This call
  // needs to be last so that the instance can be destroyed.
...
views::NonClientFrameView* ShellSurfaceBase::CreateNonClientFrameView(
  ...
  // ShellSurfaceBases always use immersive mode.
...
void ShellSurfaceBase::CreateShellSurfaceWidget(
  ...
  // ShellSurfaceBases in system modal container are only activatable if input
