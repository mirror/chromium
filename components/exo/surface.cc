...
void Surface::CommitSurfaceHierarchy(
  ...
  if (needs_commit && sub_surfaces_reordered_) {
    ...
    for (const auto& sub_surface : pending_sub_surfaces_) {
      Surface* surface = sub_surface.first;
      ...
      surface->CommitSurfaceHierarchy(
          origin + sub_surface.second.OffsetFromOrigin(), frame,
          frame_sink_holder, frame_callbacks, presentation_callbacks,
          full_damage, commit_if_necessary);
    ...
    for (const auto& sub_surface : base::Reversed(sub_surfaces_)) {
      auto* surface = sub_surface.first;
      surface->CommitSurfaceHierarchy(
          origin + sub_surface.second.OffsetFromOrigin(), frame,
          frame_sink_holder, frame_callbacks, presentation_callbacks,
          full_damage, commit_if_necessary);
      ...
      if (needs_commit) {
        ...
        surface->window()->SetBounds(
            gfx::Rect(sub_surface.second, surface->window()->bounds().size()));
      }
    }
  }

  Composite(origin, frame, full_damage, needs_commit);
...
void Surface::RestoreResources(LayerTreeFrameSinkHolder* frame_sink_holder) {
  UpdateResource(frame_sink_holder, false);
  for (const auto& sub_surface : base::Reversed(sub_surfaces_))
    sub_surface.first->RestoreResources(frame_sink_holder);
}
...
void Surface::UpdateResource(LayerTreeFrameSinkHolder* frame_sink_holder,
  ...
  if (current_buffer_.buffer() &&
      current_buffer_.buffer()->ProduceTransferableResource(
          frame_sink_holder, frame_sink_holder->AllocateResourceId(),
