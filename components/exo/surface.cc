...
Surface::Surface(DataDeviceManager* data_device_manager)
...
void Surface::OnDragEntered(const ui::DropTargetEvent& event) {
  if (data_device_manager_)
    data_device_manager_->OnDragEntered(this, event);
}

int Surface::OnDragUpdated(const ui::DropTargetEvent& event) {
  if (data_device_manager_)
    return data_device_manager_->OnDragUpdated(this, event);
  return ui::DragDropTypes::DRAG_NONE;
}

void Surface::OnDragExited() {
  if (data_device_manager_)
    data_device_manager_->OnDragExited(this);
}

int Surface::OnPerformDrop(const ui::DropTargetEvent& event) {
  if (data_device_manager_)
    return data_device_manager_->OnPerformDrop(this, event);
  return 0;
}
