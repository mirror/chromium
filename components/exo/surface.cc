...
void Surface::RemoveSubSurface(Surface* sub_surface) {
  ...
  pending_damage_.Union(gfx::Rect(it->second, sub_surface->content_size()));
...
bool Surface::HitTestRect(const gfx::Rect& rect) const {
  // TODO(domlaskowski): This check does not prevent the client from setting an
  // input region that exceeds surface bounds, but ARC relies on this flaw.
  if (HasHitTestMask())
...
void Surface::SurfaceHierarchyResourcesLost() {
  ...
  needs_update_resource_ = true;
  damage_ = gfx::Rect(content_size_);
