...
void Surface::CommitSurfaceHierarchy(
    ...
    bool full_damage,
    bool needs_commit) {
  const bool do_commit = needs_commit && needs_commit_surface_hierarchy_;
  if (do_commit) {
  ...
  // descendants.
  if (do_commit && sub_surfaces_changed_) {
    sub_surfaces_.clear();
    aura::Window* stacking_target = nullptr;
    for (const auto& sub_surface_and_position : pending_sub_surfaces_) {
      Surface* sub_surface = sub_surface_and_position.first;
      sub_surfaces_.push_back(sub_surface_and_position);
      ...
      sub_surface->window()->SetBounds(
          gfx::Rect(sub_surface_and_position.second,
  ...
  for (const auto& sub_surface_and_position : base::Reversed(sub_surfaces_)) {
    auto* sub_surface = sub_surface_and_position.first;
    // Synchronsouly commit all pending state of the sub-surface and its
    // decendents.
    sub_surface->CommitSurfaceHierarchy(
        origin + sub_surface_and_position.second.OffsetFromOrigin(), frame,
        frame_sink_holder, frame_callbacks, presentation_callbacks, full_damage,
        needs_commit);
  }

  AppendContentsToFrame(origin, full_damage, do_commit, frame);
