...
void Surface::CommitSurfaceHierarchy(
  ...
  for (auto* sub_surface : pending_sub_surfaces_) {
    ...
    // decendents.
    if (surface->needs_commit_surface_hierarchy()) {
      surface->CommitSurfaceHierarchy(frame, frame_sink_holder, frame_callbacks,
    ...
    if (stacking_target)
      window_->StackChildAbove(surface->window(), stacking_target);
    ...
    surface->window()->SetBounds(
        gfx::Rect(sub_surface->position(), surface->window()->layer()->size()));
  }
}
...
bool Surface::HasLayerHierarchyChanged() const {
  ...
  for (const auto* sub_surface : pending_sub_surfaces_) {
    if (sub_surface->surface()->HasLayerHierarchyChanged())
      return true;
  }
  return false;
}
...
void Surface::SetSurfaceHierarchyNeedsCommitToNewSurfaces() {
  ...
  for (auto* sub_surface : pending_sub_surfaces_) {
    sub_surface->surface()->SetSurfaceHierarchyNeedsCommitToNewSurfaces();
  }
}
