  ...
  bool ToggleFullscreen(ash::wm::WindowState* window_state) override {
    ...
    switch (window_state->GetStateType()) {
      ...
      default:
        ...
        LOG(ERROR) << "UNKNOWN CURRENT STATE " << window_state->GetStateType();
        return false;
    }
  ...
  void Init() override {
    ...
    std::unique_ptr<ash::wm::ClientControlledState::Delegate> delegate =
        g_delegate_factory.get()
            ...
            : std::make_unique<ClientControlledStateDelegate>(shell_surface_);
    std::unique_ptr<ClientControlledWindowStateDelegate> window_delegate =
