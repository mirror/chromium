...
class UrlLogBrowserTest : public URL_LOG_BASE_BROWSER_TEST_CLASS {
 public:
  ...
  std::string getUrl(const base::FilePath path, std::string child) {
    base::FilePath full_path = test_data_dir_.Append(FILE_PATH_LITERAL(child));
...
IN_PROC_BROWSER_TEST_F(UrlLogBrowserTest, WriteUrlLog) {
  ...
  expecteds = {"test.html", "testharness.js", "styles.css", "child.html",
               "child.js"};
  for (std::string filename : expecteds) {
    std::string full_path(getUrl(test_data_dir_, filename));
