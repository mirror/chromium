...
class MostVisitedSites : public history::TopSitesObserver,
 ...
 private:
  ...
  // Creates tiles for |popular_sites|. The returned vector will neither contain
  // more than |num_required_tiles| nor include sites in |hosts_to_skip|.
  NTPTilesVector CreatePopularSitesTiles(
      const PopularSites::SitesVector& popular_sites,
      const std::set<std::string>& hosts_to_skip,
      size_t num_required_tiles);
