...
std::map<SectionType, PopularSites::SitesVector> ParseVersion6OrAbove(
  ...
  for (size_t i = 0; i < list.GetSize(); i++) {
    ...
    sections[section_type] = std::move(sites);
  }
...
PopularSitesImpl::PopularSitesImpl(
    ...
    : prefs_(prefs),
      ...
      is_fallback_(false),
      sections_(ParseSites(*prefs->GetList(prefs::kPopularSitesJsonPref),
...
bool PopularSitesImpl::MaybeStartFetch(bool force_download,
  ...
  const bool version_mismatch =
      version_ != prefs_->GetInteger(prefs::kPopularSitesVersionPref);

  // Download forced, or we need to download a new file.
  if (force_download || download_time_is_future || version_mismatch ||
...
GURL PopularSitesImpl::GetURLToFetch() {
  ...
  base::StringToInt(version, &version_);
...
void PopularSitesImpl::RegisterProfilePrefs(
  ...
  user_prefs->RegisterListPref(prefs::kPopularSitesJsonPref,
                               DefaultPopularSites());
  user_prefs->RegisterIntegerPref(prefs::kPopularSitesVersionPref,
                                  kSitesExplorationBakedInVersion);
}
...
void PopularSitesImpl::OnJsonParsed(std::unique_ptr<base::Value> json) {
  ...
  prefs_->SetInteger(prefs::kPopularSitesVersionPref, version_);
  prefs_->SetString(prefs::kPopularSitesURLPref, pending_url_.spec());

  sections_ = ParseSites(*list, version_);
  callback_.Run(true);
}
