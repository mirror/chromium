...
PopularSites::SitesVector ParseSiteList(const base::ListValue& list) {
  ...
  for (size_t i = 0; i < list.GetSize(); i++) {
    ...
    int source;
    if (item->GetInteger("source", &source)) {
      sites.back().source = static_cast<TileSource>(source);
    }
  }
  return sites;
}
...
std::unique_ptr<base::ListValue> DefaultPopularSites() {
  ...
  for (base::Value& site : *sites) {
    ...
    dict.SetInteger("source", static_cast<int>(TileSource::POPULAR_BAKED_IN));
  }
