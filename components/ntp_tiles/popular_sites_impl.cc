...
PopularSites::SitesVector ParseSiteList(const base::ListValue& list) {
  ...
  for (size_t i = 0; i < list.GetSize(); i++) {
    ...
    TileNameSource name_source = TileNameSource::UNKNOWN;
    int name_source_int;
    if (item->GetInteger("title_source", &name_source_int) &&
        name_source_int <= static_cast<int>(TileNameSource::LAST_NAME_SOURCE) &&
        name_source_int > 0) {
      name_source = static_cast<TileNameSource>(name_source_int);
    }

    sites.emplace_back(title, GURL(url), GURL(favicon_url),
                       GURL(large_icon_url), GURL(thumbnail_url), name_source);
