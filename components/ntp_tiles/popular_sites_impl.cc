...
bool PopularSitesImpl::MaybeStartFetch(bool force_download,
  ...
  const bool version_mismatch =
      GetVersionToFetch() != prefs_->GetString(prefs::kPopularSitesVersionPref);
...
void PopularSitesImpl::RegisterProfilePrefs(
  ...
  user_prefs->RegisterListPref(prefs::kPopularSitesJsonPref,
                               DefaultPopularSites());
  user_prefs->RegisterStringPref(
      prefs::kPopularSitesVersionPref,
      base::IntToString(kSitesExplorationBakedInVersion));
}
...
void PopularSitesImpl::OnJsonParsed(std::unique_ptr<base::Value> json) {
  ...
  prefs_->SetString(prefs::kPopularSitesVersionPref, version_);
