...
class Builder {
 public:
  ...
  Builder& WithDataGenerationTime(const base::Time& data_generation_time) {
...
TEST(RecordTileImpressionTest, ShouldRecordAge) {
  const base::TimeDelta kSuggestionAge = base::TimeDelta::FromMinutes(2);
  const base::TimeDelta kBucketTolerance = base::TimeDelta::FromSeconds(10);
  base::HistogramTester histogram_tester;
  ...
  EXPECT_THAT(
      histogram_tester.GetAllSamples("NewTabPage.SuggestionsImpressionsAge"),
      ElementsAre(
          IsBucketBetween((kSuggestionAge - kBucketTolerance).InMilliseconds(),
                          (kSuggestionAge + kBucketTolerance).InMilliseconds(),
                          /*count=*/1)));
  EXPECT_THAT(histogram_tester.GetAllSamples(
                  "NewTabPage.SuggestionsImpressionsAge.server"),
