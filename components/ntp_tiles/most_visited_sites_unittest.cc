...
MATCHER_P3(FirstTileIs,
  ...
  if (arg.find(SectionType::PERSONALIZED) == arg.end()) {
    return false;
  }
...
class MostVisitedSitesTest : public ::testing::TestWithParam<bool> {
 protected:
  MostVisitedSitesTest()
    ...
    // TODO(fhorschig): Remove when OnMostVisitedURLsAvailable is removed.
    // As OnMostVisitedURLsAvailable is deprecated now, ignore calls to it.
  ...
  bool VerifyAndClearExpectations() {
    ...
    // TODO(fhorschig): Remove when OnMostVisitedURLsAvailable is removed.
    // For convenience, restore the expectations for deprecated methods, too.
    EXPECT_CALL(mock_observer_, OnMostVisitedURLsAvailable(_))
        .Times(AnyNumber());
    return success;
  }
...
TEST_P(MostVisitedSitesTest, ShouldIncludeTileForHomePage) {
  ...
  EXPECT_CALL(*mock_top_sites_, IsBlacklisted(Eq(GURL(kHomePageUrl))))
      .Times(AnyNumber())
      .WillRepeatedly(Return(false));
  EXPECT_CALL(mock_observer_, OnExplorationTilesAvailable(FirstTileIs(
...
TEST_P(MostVisitedSitesTest, ShouldNotIncludeHomePageWithoutClient) {
  ...
  EXPECT_CALL(mock_observer_,
              OnExplorationTilesAvailable(Contains(
                  Pair(Eq(SectionType::PERSONALIZED),
...
TEST_P(MostVisitedSitesTest, ShouldUpdateHomePageTileOnHomePageStateChanged) {
  ...
  EXPECT_CALL(mock_observer_, OnExplorationTilesAvailable(Not(FirstTileIs(
                                  "", kHomePageUrl, TileSource::HOMEPAGE))));
...
TEST_P(MostVisitedSitesTest, ShouldNotIncludeHomePageIfNoTileRequested) {
  ...
  EXPECT_CALL(*mock_top_sites_, IsBlacklisted(Eq(GURL(kHomePageUrl))))
      .Times(AnyNumber())
      .WillRepeatedly(Return(false));
  EXPECT_CALL(mock_observer_, OnExplorationTilesAvailable(Contains(Pair(
                                  Eq(SectionType::PERSONALIZED), IsEmpty()))));
  most_visited_sites_->SetMostVisitedURLsObserver(&mock_observer_,
                                                  /*num_sites=*/0);
  base::RunLoop().RunUntilIdle();
}
...
TEST_P(MostVisitedSitesTest, ShouldContainHomePageInFirstFourTiles) {
  ...
  EXPECT_THAT(sections.at(SectionType::PERSONALIZED)[3],
...
TEST_P(MostVisitedSitesTest, ShouldDeduplicateHomePageWithTopSites) {
  ...
  EXPECT_CALL(
      mock_observer_,
      OnExplorationTilesAvailable(Contains(Pair(
          Eq(SectionType::PERSONALIZED),
...
TEST_P(MostVisitedSitesTest, ShouldNotIncludeHomePageIfEmptyUrl) {
  ...
  EXPECT_CALL(*mock_top_sites_, IsBlacklisted(Eq(kEmptyHomePageUrl)))
      ...
      .WillRepeatedly(Return(false));
  EXPECT_CALL(mock_observer_,
              OnExplorationTilesAvailable(Not(
...
TEST_P(MostVisitedSitesTest, ShouldContainSiteExplorationsWhenFeatureEnabled) {
  ...
  if (!IsPopularSitesEnabledViaVariations()) {
    EXPECT_THAT(
        sections,
        Contains(Pair(Eq(SectionType::PERSONALIZED),
                      ElementsAre(MatchesTile("Site 1", "http://site1/",
                                              TileSource::TOP_SITES)))));
    return;
  }
  EXPECT_THAT(sections.size(),
              Eq(most_visited_sites_->popular_sites()->sections().size()));
  EXPECT_THAT(
      sections,
      Contains(Pair(
          Eq(SectionType::PERSONALIZED),
...
TEST_P(MostVisitedSitesTest, ShouldHandleTopSitesCacheHit) {
  ...
  EXPECT_CALL(mock_observer_, OnMostVisitedURLsAvailable(_)).Times(AnyNumber());
  EXPECT_CALL(mock_observer_, OnExplorationTilesAvailable(_));
...
TEST_P(MostVisitedSitesWithCacheHitTest, ShouldTruncateList) {
  EXPECT_CALL(mock_observer_, OnExplorationTilesAvailable(Contains(Pair(
                                  Eq(SectionType::PERSONALIZED), SizeIs(4)))));
  suggestions_service_callbacks_.Notify(
      MakeProfile({MakeSuggestion("Site 4", "http://site4/"),
                   MakeSuggestion("Site 5", "http://site5/"),
                   MakeSuggestion("Site 6", "http://site6/"),
...
TEST_P(MostVisitedSitesWithEmptyCacheTest,
  ...
  if (IsPopularSitesEnabledViaVariations()) {
    ...
    EXPECT_CALL(mock_observer_,
                OnExplorationTilesAvailable(ElementsAre(
                    Pair(Eq(SectionType::PERSONALIZED), IsEmpty()))));
  }
  suggestions_service_callbacks_.Notify(SuggestionsProfile());
