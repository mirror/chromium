...
const char kNameSource[] = "title_source";
 ...
 protected:
  PopularSitesTest()
      : kWikipedia{
            ...
            {kNameSource, "1"},  // Name extracted from title tag.
        },
        kYouTube{
            {kTitle, "YouTube"},
            {kUrl, "https://m.youtube.com/"},
            {kLargeIconUrl, "https://s.ytimg.com/apple-touch-icon.png"},
            {kNameSource, "2"},  // Name extracted from manifest.
        },
        kChromium{
  ...
  std::unique_ptr<base::ListValue> CreateListFromTestSites(
    ...
    for (const TestPopularSite& site : sites) {
      ...
      for (const std::pair<std::string, std::string>& kv : site) {
        if (kv.first == kNameSource) {
          int source;
          base::StringToInt(kv.second, &source);
...
TEST_F(PopularSitesTest, ShouldSucceedFetching) {
  ...
  EXPECT_THAT(sites[0].name_source, Eq(TileNameSource::TITLE));
}
...
TEST_F(PopularSitesTest, Fallback) {
  ...
  EXPECT_THAT(sites[0].favicon_url, URLEq(""));
  EXPECT_THAT(sites[0].name_source, Eq(TileNameSource::MANIFEST));
  ...
  EXPECT_THAT(sites[1].name_source, Eq(TileNameSource::UNKNOWN));
}
