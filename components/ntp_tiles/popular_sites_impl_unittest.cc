...
TEST_F(PopularSitesTest, FetchesExplorationSitesWithFeature) {
  ...
  EXPECT_THAT(sections, SizeIs(4ul));
  EXPECT_THAT(sections, Contains(Pair(SectionType::PERSONALIZED, SizeIs(1))));
  EXPECT_THAT(sections, Contains(Pair(SectionType::NEWS, SizeIs(1))));
  EXPECT_THAT(sections, Contains(Pair(SectionType::ENTERTAINMENT, SizeIs(2))));
  EXPECT_THAT(sections, Contains(Pair(SectionType::TOOLS, IsEmpty())));
  EXPECT_THAT(sections, ElementsAre(Pair(SectionType::PERSONALIZED, _),
                                    Pair(SectionType::ENTERTAINMENT, _),
                                    Pair(SectionType::NEWS, _),
                                    Pair(SectionType::TOOLS, _)));
}
