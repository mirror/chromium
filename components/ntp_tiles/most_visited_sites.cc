...
void MostVisitedSites::OnMostVisitedURLsAvailable(
  ...
  for (size_t i = 0; i < num_tiles; ++i) {
    ...
    // MostVisitedURLs are either the title or the URL which is treated exactly
    // as the title. Differentiating here is not worth the overhead.
    tile.name_source = TileNameSource::TITLE;
...
void MostVisitedSites::BuildCurrentTilesGivenSuggestionsProfile(
  ...
  for (size_t i = 0; i < num_tiles; ++i) {
    ...
    // The backend might change this so we can not be sure unless we submit it.
    // As of this writing, it's an aggregated title.
    tile.name_source = TileNameSource::UNKNOWN;
...
NTPTilesVector MostVisitedSites::CreatePopularSitesTiles(
  ...
  for (const PopularSites::Site& popular_site : sites_vector) {
    ...
    tile.name_source = popular_site.name_source;
...
NTPTilesVector MostVisitedSites::InsertHomeTile(
  ...
  if (!home_tile_added) {
    ...
    home_tile.name_source = TileNameSource::TITLE;  // Fetched from history.
