...
void MostVisitedSites::OnMostVisitedURLsAvailable(
  ...
  for (size_t i = 0; i < num_tiles; ++i) {
    ...
    tile.name_source = TileNameSource::TITLE_TAG;
...
void MostVisitedSites::BuildCurrentTilesGivenSuggestionsProfile(
  ...
  for (size_t i = 0; i < num_tiles; ++i) {
    ...
    // The backend might change this so this is subject to change. Currently,
...
NTPTilesVector MostVisitedSites::InsertHomeTile(
  ...
  if (!home_tile_added) {
    ...
    home_tile.name_source = TileNameSource::TITLE_TAG;  // Fetched from history.
