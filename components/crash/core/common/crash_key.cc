...
TransitionalCrashKeyStorage* GetCrashKeyStorage() {
  if (!g_storage) {
    g_storage = new TransitionalCrashKeyStorage();
  }
  return g_storage;
}
...
void CrashKeyStringImpl::Set(const char* value) {
  ...
  base::StringPiece value_to_set =
      base::StringPiece(value).substr(0, kValueMaxLength);

  // If there is only one slot for the value, then handle it directly.
  if (index_array_count_ == 1) {
    std::string value_string = value_to_set.as_string();
    if (is_set()) {
  ...
  for (size_t i = 0; i < index_array_count_; ++i) {
    if (offset < value_to_set.length()) {
      ...
      base::StringPiece chunk =
          value_to_set.substr(offset, kCrashKeyStorageValueSize - 1);
      offset += chunk.length();
