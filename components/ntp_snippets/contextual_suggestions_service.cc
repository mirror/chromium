...
void ContextualSuggestionsService::OnFetchContextualSuggestions(
  ...
  if (fetched_categories.has_value()) {
    for (const auto& fetched_category : *fetched_categories) {
      for (const std::unique_ptr<RemoteSuggestion>& snippet :
           fetched_category.snippets) {
        suggestions.emplace_back(snippet->ToContentSuggestion(
            Category::FromKnownCategory(KnownCategories::CONTEXTUAL)));
        ContentSuggestion::ID id = suggestions.back().id();
        GURL image_url = snippet->salient_image_url();
