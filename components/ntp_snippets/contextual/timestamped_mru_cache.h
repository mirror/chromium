...
// This non-persistant cache guarantees to hold no more than max_capacity
// items and returns only items younger than max_age.
template <class KeyType, class PayloadType>
class TimestampedMRUCache {
 public:
  TimestampedMRUCache(std::size_t max_capacity, base::TimeDelta max_age)
  ...
  base::Optional<PayloadType> Get(const KeyType& key) {
    ...
    PayloadType& value = found_iterator->second.data;
    base::Time& timestamp = found_iterator->second.time;
