...
void BreakingNewsGCMAppHandler::DidReceiveTokenForValidation(
  ...
  if (result != InstanceID::SUCCESS) {
    metrics::OnTokenValidated(
        time_since_last_validation,
        /*was_token_valid_before_validation=*/base::nullopt);
    return;
  }
  ...
  metrics::OnTokenValidated(
      time_since_last_validation,
      /*was_token_valid_before_validation=*/old_token == new_token);
...
void BreakingNewsGCMAppHandler::OnMessage(const std::string& app_id,
  ...
  if (it == message.data.end()) {
    ...
    metrics::OnMessageReceived(/*contains_pushed_news=*/false);
    return;
  }

  std::string news = it->second;
  metrics::OnMessageReceived(/*contains_pushed_news=*/true);
