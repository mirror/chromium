...
class SubscriptionManagerTest : public testing::Test {
 public:
  ...
  void RespondToSubscriptionRequestSuccessfully(bool isSignedIn) {
  ...
  void RespondToUnsubscriptionRequestSuccessfully(bool isSignedIn) {
    net::TestURLFetcher* url_fetcher = GetRunningFetcher();
    if (isSignedIn) {
  ...
  void RespondToSubscriptionWithError(bool isSignedIn, int error_code) {
    net::TestURLFetcher* url_fetcher = GetRunningFetcher();
    if (isSignedIn) {
      ASSERT_EQ(GURL(kSubscriptionUrlSignedIn), url_fetcher->GetOriginalURL());
    } else {
  ...
  void RespondToUnsubscriptionWithError(bool isSignedIn, int error_code) {
    net::TestURLFetcher* url_fetcher = GetRunningFetcher();
    if (isSignedIn) {
      ASSERT_EQ(GURL(kUnsubscriptionUrlSignedIn),
                url_fetcher->GetOriginalURL());
...
TEST_F(SubscriptionManagerTest, SubscribeSuccessfully) {
  ...
  RespondToSubscriptionRequestSuccessfully(/*isSignedIn=*/false);
...
TEST_F(SubscriptionManagerTest,
  ...
  ASSERT_FALSE(manager.IsSubscribed());
  RespondToSubscriptionRequestSuccessfully(/*isSignedIn=*/true);
  ASSERT_TRUE(manager.IsSubscribed());
...
TEST_F(SubscriptionManagerTest, ShouldNotSubscribeIfError) {
  ...
  manager.Subscribe(subscription_token);
  RespondToSubscriptionWithError(/*isSignedIn=*/false, net::ERR_TIMED_OUT);
...
TEST_F(SubscriptionManagerTest, UnsubscribeSuccessfully) {
  ...
  ASSERT_TRUE(manager.IsSubscribed());
  manager.Unsubscribe();
  RespondToUnsubscriptionRequestSuccessfully(/*isSignedIn=*/false);
  EXPECT_FALSE(manager.IsSubscribed());
  EXPECT_FALSE(
...
TEST_F(SubscriptionManagerTest,
  ...
  ASSERT_TRUE(manager.IsSubscribed());
  manager.Unsubscribe();
  RespondToUnsubscriptionWithError(/*isSignedIn=*/false, net::ERR_TIMED_OUT);
  ASSERT_TRUE(manager.IsSubscribed());
  EXPECT_EQ(subscription_token, GetPrefService()->GetString(
                                    prefs::kBreakingNewsSubscriptionDataToken));
}
