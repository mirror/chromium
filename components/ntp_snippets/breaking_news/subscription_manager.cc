...
void SubscriptionManager::AccessTokenFetchFinished(
  ...
  if (error.state() != GoogleServiceAuthError::NONE) {
    // In case of error, return. Will retry on next Chrome restart.
    return;
  }
  DCHECK(!access_token.empty());
  SubscribeInternal(access_token);
}

void SubscriptionManager::DidSubscribe(bool is_authenticated,
                                       const Status& status) {
  // Delete the request only after we leave this method (which is called from
  // the request itself).
  std::unique_ptr<internal::SubscriptionJsonRequest>
      subscription_request_deleter(std::move(subscription_request_));
...
void SubscriptionManager::Unsubscribe(const std::string& token,
                                      bool is_resubscribe) {
  // If there is an unsubscription request in flight, cancel it.
  if (unsubscription_request_) {
    unsubscription_request_->Cancel();
  }
  ...
  unsubscription_request_->Start(
      base::BindOnce(&SubscriptionManager::DidUnsubscribe,
                     base::Unretained(this), is_resubscribe));
}
...
void SubscriptionManager::Resubscribe(const std::string& old_token,
  ...
  Unsubscribe(old_token, true);
}

void SubscriptionManager::DidUnsubscribe(bool is_resubscribe,
                                         const Status& status) {
  // Delete the request only after we leave this method (which is called from
  // the request itself).
  std::unique_ptr<internal::SubscriptionJsonRequest>
      unsubscription_request_deleter(std::move(unsubscription_request_));
...
void SubscriptionManager::GoogleSigninSucceeded(const std::string& account_id,
                                                const std::string& username) {
  // If subscribed already, resubscribe
  if (IsSubscribed()) {
    std::string token =
        pref_service_->GetString(prefs::kBreakingNewsSubscriptionDataToken);
    Resubscribe(token, token);
  }
}

void SubscriptionManager::GoogleSignedOut(const std::string& account_id,
                                          const std::string& username) {
  // If subscribed already, resubscribe
  if (IsSubscribed()) {
    std::string token =
