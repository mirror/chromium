...
BreakingNewsSuggestionsProvider::BreakingNewsSuggestionsProvider(
  ...
  breaking_news_raw_data_provider_->StartListening(
      base::Bind(&BreakingNewsSuggestionsProvider::OnNewArticleSuggestion,
                 base::Unretained(this)));
}
...
void BreakingNewsSuggestionsProvider::OnNewArticleSuggestion(
    std::unique_ptr<RemoteSuggestion> article_suggestion) {
  std::vector<std::unique_ptr<RemoteSuggestion>> suggestions;
  suggestions.push_back(std::move(article_suggestion));
