...
class SubscriptionManagerImplTest
 ...
 public:
  ...
  SubscriptionManagerImpl* GetSubscriptionManager() {
    // Lazily initialize |subscription_manager_| as some tests do custom setup
    // work on its dependencies first.
    if (!subscription_manager_) {
      subscription_manager_ = std::make_unique<SubscriptionManagerImpl>(
 ...
 private:
  ...
  std::unique_ptr<SubscriptionManagerImpl> subscription_manager_;
};

TEST_F(SubscriptionManagerImplTest, SubscribeSuccessfully) {
  std::string subscription_token = "1234567890";
  SubscriptionManagerImpl* manager = GetSubscriptionManager();
