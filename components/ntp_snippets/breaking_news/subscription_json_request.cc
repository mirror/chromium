...
SubscriptionJsonRequest::~SubscriptionJsonRequest() {
  if (!request_completed_callback_.is_null()) {
    std::move(request_completed_callback_)
        .Run(Status(StatusCode::TEMPORARY_ERROR, "cancelled"));
  }
}
...
void SubscriptionJsonRequest::Cancel() {
  DCHECK(!request_completed_callback_.is_null()) << "Request not running!";
  url_fetcher_.reset();
  // Delete the callback.
  CompletedCallback request_completed_callback_deleter(
      std::move(request_completed_callback_));
}
...
SubscriptionJsonRequest::Builder::Build() const {
  ...
  VLOG(1) << "Building a subscription request to " << url_ << ":\n"
          << headers << "\n"
          << body;
