...
class MockBreakingNewsListener : public BreakingNewsListener {
 public:
  MOCK_METHOD1(StartListening, void(OnNewArticleSuggestionCallback callback));
 ...
 protected:
  void InitializeBreakingNewsSuggestionsProvider() {
    ...
    EXPECT_CALL(*listener_, StartListening(_))
        .WillOnce(SaveArg<0>(&on_new_article_suggestion_callback_))
        .RetiresOnSaturation();
  ...
  BreakingNewsListener::OnNewArticleSuggestionCallback
      on_new_article_suggestion_callback_;
...
TEST_F(BreakingNewsSuggestionsProviderTest,
  ...
  on_new_article_suggestion_callback_.Run(std::move(suggestion));
}
