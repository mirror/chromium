...
class MockRemoteSuggestionsProvider : public RemoteSuggestionsProvider {
 public:
  ...
  // ImageFetchedCallback, FetchStatusCallback. As a work-around, this function
  // calls the mock function with value passed by pointer. The mock function may
  // then be checked with EXPECT_CALL.
  ...
  // calls the mock function with value passed by pointer. The mock function may
  // then be checked with EXPECT_CALL.
  void RefetchForDisplay(
      RemoteSuggestionsProvider::FetchStatusCallback callback) override {
    RefetchForDisplay(&callback);
  }
  MOCK_METHOD1(RefetchForDisplay,
               void(RemoteSuggestionsProvider::FetchStatusCallback*));
...
TEST_F(RemoteSuggestionsSchedulerImplTest,
       ShouldNotRefetchForDisplayBeforeDefaultDelay) {
  // Activating the provider should schedule the persistent background fetches.
  EXPECT_CALL(*persistent_scheduler(), Schedule(_, _)).Times(2);
  // First enable the scheduler -- this will trigger the persistent scheduling.
  ActivateProviderAndEula();
  ...
  EXPECT_CALL(*provider(), RefetchForDisplay(_)).Times(0);
  EXPECT_CALL(*provider(), RefetchInTheBackground(_));
...
TEST_F(RemoteSuggestionsSchedulerImplTest,
       ShouldRefetchForDisplayAfterDefaultDelay) {
  // Activating the provider should schedule the persistent background fetches.
  EXPECT_CALL(*persistent_scheduler(), Schedule(_, _)).Times(2);
  // First enable the scheduler -- this will trigger the persistent scheduling.
  ActivateProviderAndEula();
  ...
  // Long enough: stale.
  EXPECT_CALL(*provider(), RefetchForDisplay(_));
  EXPECT_CALL(*provider(), RefetchInTheBackground(_)).Times(0);
  // Use the SurfaceOpened trigger as this has the shortest intervals and thus
  // allows to test both short and long delay in the same way.
  scheduler()->OnSuggestionsSurfaceOpened();
...
TEST_F(RemoteSuggestionsSchedulerImplTest,
       ShouldNotRefetchForDisplayBeforeConfigrableDelay) {
  SetVariationParameter("min_age_for_stale_fetch_hours", "18");
  // Activating the provider should schedule the persistent background fetches.
  EXPECT_CALL(*persistent_scheduler(), Schedule(_, _)).Times(2);
  // First enable the scheduler -- this will trigger the persistent scheduling.
  ActivateProviderAndEula();
  ...
  // The staleness threshold by default coincides with the startup interval.
  test_clock()->Advance(base::TimeDelta::FromHours(18) -
                        base::TimeDelta::FromMinutes(1));
...
TEST_F(RemoteSuggestionsSchedulerImplTest,
       ShouldRefetchForDisplayAfterConfigurableDelay) {
  SetVariationParameter("min_age_for_stale_fetch_hours", "18");
  // Activating the provider should schedule the persistent background fetches.
  EXPECT_CALL(*persistent_scheduler(), Schedule(_, _)).Times(2);
  // First enable the scheduler -- this will trigger the persistent scheduling.
...
TEST_F(RemoteSuggestionsSchedulerImplTest,
       ShouldNotRefetchForDisplayBeforeFallbackConfigrableDelay) {
  SetVariationParameter("startup_fetching_interval_hours-wifi-active_ntp_user",
                        "12");
  // Activating the provider should schedule the persistent background fetches.
  EXPECT_CALL(*persistent_scheduler(), Schedule(_, _)).Times(2);
...
TEST_F(RemoteSuggestionsSchedulerImplTest,
       ShouldRefetchForDisplayAfterFallbackConfigurableDelay) {
  SetVariationParameter("startup_fetching_interval_hours-wifi-active_ntp_user",
                        "12");
  // Activating the provider should schedule the persistent background fetches.
  EXPECT_CALL(*persistent_scheduler(), Schedule(_, _)).Times(2);
