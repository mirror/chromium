...
const int kDefaultStartupInterval = 24;
...
TEST_F(RemoteSuggestionsSchedulerImplTest,
  ...
  scheduler()->OnBrowserForegrounded();
  std::move(signal_fetch_done).Run(Status::Success());
  test_clock()->Advance(base::TimeDelta::FromHours(kDefaultStartupInterval));
  scheduler()->OnBrowserForegrounded();
}
...
TEST_F(RemoteSuggestionsSchedulerImplTest,
       ShouldNotRefetchWhileDisplayingBeforeFallbackConfigrableDelay) {
