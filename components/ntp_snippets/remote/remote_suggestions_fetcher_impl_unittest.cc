...
class RemoteSuggestionsFetcherImplTestBase : public testing::Test {
 public:
  ...
  void ResetFetcherWithAPIKey(const std::string& api_key) {
    ...
    mock_clock_ = mock_task_runner_->GetMockClock();
    fetcher_->SetClockForTesting(mock_clock_.get());
  }
 ...
 private:
  // TODO(tzik): Remove |mock_clock_| after updating GetMockTickClock to own the
  // instance.
  std::unique_ptr<base::Clock> mock_clock_;
