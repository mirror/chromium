...
class RemoteSuggestionsFetcher {
 public:
  struct FetchedCategory {
    ...
    FetchedCategory(Category category, CategoryInfo&& info);
  ...
  virtual const std::string& last_status_for_debugging() const = 0;

  // Returns the last JSON fetched from the server.
  virtual const std::string& last_json_for_debugging() const = 0;

  // Returns the URL endpoint used by the fetcher.
  virtual const GURL& fetch_url_for_debugging() const = 0;
};
