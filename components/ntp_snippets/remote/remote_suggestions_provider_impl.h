...
class RemoteSuggestionsProviderImpl final : public RemoteSuggestionsProvider {
 ...
 private:
  ...
  void NukeSuggestions(Category category);
  ...
  // Set containing the categories for which NukeSuggestions has been called
  // while the service isn't ready. The nuke will be executed once the service
  // finishes initialization or enters the READY state.
  std::set<Category, Category::CompareByID> categories_nuke_when_initialized_;
