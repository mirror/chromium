...
class RemoteSuggestionsProviderImpl final : public RemoteSuggestionsProvider {
 ...
 private:
  ...
  // If |hide_loading_indicator_after_timeout| is true, the indicator is hidden
  // if the fetch does not finish within a certain amount of time (the fetch
  // itself is not canceled, though).
  void FetchSuggestionsWithLoadingIndicator(
      bool interactive_request,
      FetchStatusCallback callback,
      bool hide_loading_indicator_after_timeout);
