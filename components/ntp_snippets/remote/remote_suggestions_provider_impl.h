...
class RemoteSuggestionsProviderImpl final : public RemoteSuggestionsProvider {
 public:
  ...
  void RefetchForDisplay(FetchStatusCallback callback) override;
 ...
 private:
  ...
  void NotifyFetchForDisplayStarted();
  void NotifyFetchForDisplayFailed();
  void OnRefetchForDisplayFinished(FetchStatusCallback callback, Status status);
