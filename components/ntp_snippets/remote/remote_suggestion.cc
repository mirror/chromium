...
RemoteSuggestion::CreateFromContentSuggestionsDictionary(
  ...
  if (image_dominant_color_value) {
    ...
    if (image_dominant_color_value->is_int()) {
      ...
      // Round to the nearest integer in case the number cannot be represented
      // exactly.
      image_dominant_color =
          static_cast<uint32_t>(image_dominant_color_value->GetDouble() + 0.5);
    }
...
ContentSuggestion RemoteSuggestion::ToContentSuggestion(
  ...
  if (image_dominant_color_.has_value()) {
    suggestion.set_image_dominant_color(*image_dominant_color_);
  }
  return suggestion;
}
