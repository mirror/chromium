...
RemoteSuggestion::CreateFromContentSuggestionsDictionary(
  ...
  // content_type_ of the class |RemoteSnippet| is therefore initialized to
  // ContentType::UNKNOWN.
  ...
  if (dict.GetString("contentType", &content_type)) {
    if (content_type == "VIDEO") {
      ...
      // In a debug build validate that the content_type has a valid value.
      DCHECK(content_type == "UNKNOWN");
    }
  }
