...
const bool kDeleteCategoriesNotPresentInLastFetchResponseDefault = false;
const char kDeleteCategoriesNotPresentInLastFetchResponseParamName[] =
    "delete_categories_not_present_in_last_fetch_response";

bool ShouldDeleteCategoriesNotPresentInLastFetchResponse() {
  return base::GetFieldTrialParamByFeatureAsBool(
      ntp_snippets::kArticleSuggestionsFeature,
      kDeleteCategoriesNotPresentInLastFetchResponseParamName,
      kDeleteCategoriesNotPresentInLastFetchResponseDefault);
}
...
void RemoteSuggestionsProviderImpl::OnFetchFinished(
  ...
  if (fetched_categories &&
      ShouldDeleteCategoriesNotPresentInLastFetchResponse()) {
    for (auto& item : category_contents_) {
      Category category = item.first;
      CategoryContent* content = &item.second;
