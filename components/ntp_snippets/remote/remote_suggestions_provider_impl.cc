...
void RemoteSuggestionsProviderImpl::NukeSuggestions(Category category) {
  if (!initialized()) {
    categories_nuke_when_initialized_.insert(category);
    return;
  }
...
void RemoteSuggestionsProviderImpl::EnterState(State state) {
  ...
  switch (state) {
    ...
    case State::READY:
      ...
      if (!categories_nuke_when_initialized_.empty()) {
        for (auto category : categories_nuke_when_initialized_) {
          NukeSuggestions(category);
        }
        categories_nuke_when_initialized_.clear();
      }
