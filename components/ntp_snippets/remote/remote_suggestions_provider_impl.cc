...
void RemoteSuggestionsProviderImpl::FetchSuggestions(
    ...
    FetchStatusCallback callback) {
  debug_logger_->Log("RemoteSuggestionsProviderImpl::FetchSuggestions");
  if (!ready()) {
...
void RemoteSuggestionsProviderImpl::Fetch(
  ...
  debug_logger_->Log("RemoteSuggestionsProviderImpl::Fetch");
  if (!ready()) {
...
void RemoteSuggestionsProviderImpl::OnFetchFinished(
    ...
    Status status,
    RemoteSuggestionsFetcher::OptionalFetchedCategories fetched_categories) {
  debug_logger_->Log("RemoteSuggestionsProviderImpl::OnFetchFinished");
  if (!ready()) {
  ...
  if (fetched_categories) {
    debug_logger_->Log(
        base::StringPrintf("OnFetchFinished: fetched categories count = %d",
    ...
    for (FetchedCategory& fetched_category : *fetched_categories) {
      if (fetched_category.category == articles_category_) {
        ...
        debug_logger_->Log(base::StringPrintf(
            "OnFetchFinished: articles category size = %d",
