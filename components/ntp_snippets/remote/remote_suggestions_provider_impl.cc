...
void RemoteSuggestionsProviderImpl::IntegrateSuggestions(
  ...
  if (IsKeepingPrefetchedSuggestionsEnabled() &&
    ...
    std::vector<std::unique_ptr<RemoteSuggestion>>
        recent_prefetched_suggestions, other_suggestions;
    for (auto& remote_suggestion : content->suggestions) {
      if (prefetched_pages_tracker_->PrefetchedOfflinePageExists(
              remote_suggestion->url()) &&
          clock_->Now() - remote_suggestion->publish_date() <
              kMaxAgeForAdditionalPrefetchedSuggestion) {
        recent_prefetched_suggestions.push_back(std::move(remote_suggestion));
      } else {
    ...
    if (recent_prefetched_suggestions.size() >
        kMaxAdditionalPrefetchedSuggestions) {
      std::sort(recent_prefetched_suggestions.begin(),
                recent_prefetched_suggestions.end(),
                [](const std::unique_ptr<RemoteSuggestion>& first,
                   const std::unique_ptr<RemoteSuggestion>& second) {
                  return first->publish_date() > second->publish_date();
                });
      while (recent_prefetched_suggestions.size() >
             kMaxAdditionalPrefetchedSuggestions) {
        other_suggestions.push_back(
            std::move(recent_prefetched_suggestions.back()));
        recent_prefetched_suggestions.pop_back();
      }
    }

    for (auto& remote_suggestion : recent_prefetched_suggestions) {
    ...
    for (auto& remote_suggestion : other_suggestions) {
      content->suggestions.push_back(std::move(remote_suggestion));
    }
  }
