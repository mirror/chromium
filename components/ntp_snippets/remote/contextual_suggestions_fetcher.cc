...
CategoryInfo BuildContextualSuggestionsCategoryInfo(
  ...
  return CategoryInfo(
      ...
      /*show_if_empty=*/false,
      // contextual suggestions do not require a no_suggestions_message
      base::string16());
}
...
void ContextualSuggestionsFetcher::JsonRequestDone(
  ...
  if (!JsonToSuggestions(*result, &categories)) {
...
bool ContextualSuggestionsFetcher::JsonToSuggestions(
  ...
  for (const auto& v : *categories_value) {
    ...
    categories->push_back(FetchedCategory(
        category,
        BuildContextualSuggestionsCategoryInfo(base::UTF8ToUTF16(utf8_title))));
