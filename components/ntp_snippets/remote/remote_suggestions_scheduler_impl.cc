...
void RemoteSuggestionsSchedulerImpl::RefetchIfAppropriate(TriggerType trigger) {
  ...
  // TODO(jkrcal): Watch how it behaves in practice and consider restricting
  // this treatment only to the SURFACE_OPENED trigger. (This would mean less
  // time on startup to fetch results before opening the surface but also lower
  // likelyhood of flickering).
  if (trigger != TriggerType::PERSISTENT_SCHEDULER_WAKE_UP &&
