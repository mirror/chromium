...
void RemoteSuggestionsSchedulerImpl::OnSuggestionsCleared() {
  debug_logger_->Log("RemoteSuggestionsSchedulerImpl::OnSuggestionsCleared");
...
void RemoteSuggestionsSchedulerImpl::OnPersistentSchedulerWakeUp() {
  debug_logger_->Log(
      "RemoteSuggestionsSchedulerImpl::OnPersistentSchedulerWakeUp");
...
void RemoteSuggestionsSchedulerImpl::OnBrowserForegrounded() {
  debug_logger_->Log("RemoteSuggestionsSchedulerImpl::OnBrowserForegrounded");
...
void RemoteSuggestionsSchedulerImpl::OnBrowserColdStart() {
  debug_logger_->Log("RemoteSuggestionsSchedulerImpl::OnBrowserColdStart");
...
void RemoteSuggestionsSchedulerImpl::OnSuggestionsSurfaceOpened() {
  debug_logger_->Log(
      "RemoteSuggestionsSchedulerImpl::OnSuggestionsSurfaceOpened");
...
void RemoteSuggestionsSchedulerImpl::RefetchInTheBackgroundIfAppropriate(
    TriggerType trigger) {
  debug_logger_->Log(
      "RemoteSuggestionsSchedulerImpl::RefetchInTheBackgroundIfAppropriate");

  if (background_fetch_in_progress_) {
    debug_logger_->Log(
        "RefetchInTheBackgroundIfAppropriate: stop due to ongoing fetch");
    return;
  }

  if (enabled_triggers_.count(trigger) == 0) {
    debug_logger_->Log(
        "RefetchInTheBackgroundIfAppropriate: stop due to disabled trigger");
    return;
  }
  ...
  if (net::NetworkChangeNotifier::IsOffline()) {
    ...
    debug_logger_->Log(
        "RefetchInTheBackgroundIfAppropriate: stop due to being offline");
    return;
  }

  if (!IsReadyForBackgroundFetches()) {
    debug_logger_->Log(
        "RefetchInTheBackgroundIfAppropriate: delay until ready");
  ...
  if (trigger != TriggerType::PERSISTENT_SCHEDULER_WAKE_UP &&
    ...
    debug_logger_->Log(
        "RefetchInTheBackgroundIfAppropriate: stop, because too soon");
    return;
  }

  if (!AcquireQuota(/*interactive_request=*/false)) {
    debug_logger_->Log(
        "RefetchInTheBackgroundIfAppropriate: stop due to quota");
    return;
  }
  ...
  debug_logger_->Log("RefetchInTheBackgroundIfAppropriate: issuing a fetch");
