...
class RemoteSuggestionBuilder {
 public:
  ...
  std::unique_ptr<RemoteSuggestion> Build() {
    SetAbsentValuesToDefault();
    return BuildFromFields();
  }

 private:
  void SetAbsentValuesToDefault() {
    if (!title_) {
    ...
    if (!salient_image_url_) {
      salient_image_url_ = std::string("http://image_url.com/");
    }
    if (!publish_date_) {
      publish_date_ = GetDefaultCreationTime();
    }
    if (!expiry_date_) {
      expiry_date_ = GetDefaultExpirationTime();
    }
    if (!score_) {
      score_ = 1;
    }
    ...
    if (!remote_category_id_) {
      remote_category_id_ = 1;
    }
    if (!url_) {
      url_ = std::string("http://url.com/");
    }
    if (!publisher_name_) {
      publisher_name_ = std::string("Publisher");
    }
    if (!amp_url_) {
      amp_url_ = std::string("http://amp_url.com/");
    }
    if (!fetch_date_) {
      fetch_date_ = base::Time::Now();
    }
    if (!ids_) {
      ids_ = std::vector<std::string>({*url_});
    }
  }

  std::unique_ptr<RemoteSuggestion> BuildFromFields() {
    SnippetProto proto;
    for (const auto& id : *ids_) {
      proto.add_ids(id);
    }
    proto.set_title(*title_);
    proto.set_snippet(*snippet_);
    proto.set_salient_image_url(*salient_image_url_);
    proto.set_publish_date(publish_date_->ToInternalValue());
    proto.set_expiry_date(expiry_date_->ToInternalValue());
    proto.set_score(*score_);
    proto.set_dismissed(*is_dismissed_);
    proto.set_remote_category_id(*remote_category_id_);
    auto* source = proto.add_sources();
    source->set_url(*url_);
    source->set_publisher_name(*publisher_name_);
    source->set_amp_url(*amp_url_);
    proto.set_fetch_date(fetch_date_->ToInternalValue());
...
class FetchedCategoryBuilder {
 public:
  ...
  FetchedCategoryBuilder& AddSuggestion(
      std::unique_ptr<RemoteSuggestion> suggestion) {
    if (!suggestions_) {
      suggestions_ = std::vector<std::unique_ptr<RemoteSuggestion>>();
    }
    suggestions_->push_back(std::move(suggestion));
    return *this;
  }

  FetchedCategory Build() {
    SetAbsentValuesToDefault();
    FetchedCategory result = FetchedCategory(
        *category_, CategoryInfo(*title_, *card_layout_, *additional_action_,
                                 *show_if_empty_, *no_suggestions_message_));

    result.suggestions = std::move(*suggestions_);
    return result;
  }
 ...
 private:
  void SetAbsentValuesToDefault() {
    if (!category_) {
      category_ = Category::FromRemoteCategory(1);
    }
    if (!title_) {
      title_ = base::UTF8ToUTF16("Category title");
    }
    if (!card_layout_) {
      card_layout_ = ContentSuggestionsCardLayout::FULL_CARD;
    }
    if (!additional_action_) {
      additional_action_ = ContentSuggestionsAdditionalAction::FETCH;
    }
    if (!show_if_empty_) {
      show_if_empty_ = false;
    }
    if (!no_suggestions_message_) {
      no_suggestions_message_ = base::UTF8ToUTF16("No suggestions message");
    }
    if (!suggestions_) {
      suggestions_ = std::vector<std::unique_ptr<RemoteSuggestion>>();
    }
  ...
  base::Optional<std::vector<std::unique_ptr<RemoteSuggestion>>> suggestions_;
};
...
TEST_F(RemoteSuggestionsProviderImplTest, Full) {
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          ...
          .AddSuggestion(RemoteSuggestionBuilder()
...
TEST_F(RemoteSuggestionsProviderImplTest, CategoryTitle) {
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          ...
          .AddSuggestion(RemoteSuggestionBuilder().Build())
          .Build());
  FetchSuggestions(provider.get(), /*interactive_request=*/true,
                   Status(StatusCode::SUCCESS, "message"),
                   std::move(fetched_categories));
...
TEST_F(RemoteSuggestionsProviderImplTest, ExperimentalCategoryInfo) {
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(Category::FromRemoteCategory(1))
          .AddSuggestion(RemoteSuggestionBuilder().AddId("1").Build())
          .Build());
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          ...
          .AddSuggestion(RemoteSuggestionBuilder().AddId("2").Build())
          .Build());
...
TEST_F(RemoteSuggestionsProviderImplTest, AddRemoteCategoriesToCategoryRanker) {
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          ...
          .AddSuggestion(RemoteSuggestionBuilder().AddId("11").Build())
          .Build());
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(Category::FromRemoteCategory(13))
          .AddSuggestion(RemoteSuggestionBuilder().AddId("13").Build())
          .Build());
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(Category::FromRemoteCategory(12))
          .AddSuggestion(RemoteSuggestionBuilder().AddId("12").Build())
          .Build());
  {
...
TEST_F(RemoteSuggestionsProviderImplTest,
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          ...
          .AddSuggestion(RemoteSuggestionBuilder().AddId("14").Build())
          .Build());
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(Category::FromRemoteCategory(13))
...
TEST_F(RemoteSuggestionsProviderImplTest, PersistSuggestions) {
  auto provider = MakeSuggestionsProvider(
      /*use_mock_suggestions_fetcher=*/true, /*set_empty_response=*/true);
  std::vector<FetchedCategory> fetched_categories;
  fetched_categories.push_back(FetchedCategoryBuilder()
                                   .SetCategory(Category::FromRemoteCategory(1))
                                   .AddSuggestion(RemoteSuggestionBuilder()
                                                      .AddId("1")
...
TEST_F(RemoteSuggestionsProviderImplTest, ReplaceSuggestions) {
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(articles_category())
          .AddSuggestion(RemoteSuggestionBuilder().AddId(first).Build())
          .Build());
  FetchSuggestions(provider.get(), /*interactive_request=*/true,
                   Status(StatusCode::SUCCESS, "message"),
                   std::move(fetched_categories));
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(articles_category())
          .AddSuggestion(RemoteSuggestionBuilder().AddId(second).Build())
          .Build());
  FetchSuggestions(provider.get(), /*interactive_request=*/true,
                   Status(StatusCode::SUCCESS, "message"),
                   std::move(fetched_categories));
...
TEST_F(RemoteSuggestionsProviderImplTest,
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(articles_category())
          .AddSuggestion(RemoteSuggestionBuilder().AddId("id").Build())
          .Build());
  FetchSuggestions(provider.get(), /*interactive_request=*/true,
                   Status(StatusCode::SUCCESS, "message"),
                   std::move(fetched_categories));
...
TEST_F(RemoteSuggestionsProviderImplTest,
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(articles_category())
          .AddSuggestion(
              RemoteSuggestionBuilder().AddId("http://old.com/").Build())
          .Build());
  FetchSuggestions(provider.get(), /*interactive_request=*/true,
                   Status(StatusCode::SUCCESS, "message"),
                   std::move(fetched_categories));
...
TEST_F(RemoteSuggestionsProviderImplTest,
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(articles_category())
          .AddSuggestion(RemoteSuggestionBuilder().AddId("http://id-1").Build())
          .AddSuggestion(RemoteSuggestionBuilder().AddId("http://id-2").Build())
          .Build());
  FetchSuggestions(provider.get(), /*interactive_request=*/true,
                   Status(StatusCode::SUCCESS, "message"),
                   std::move(fetched_categories));
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(articles_category())
          .AddSuggestion(
              RemoteSuggestionBuilder().AddId("http://new-id-1").Build())
          .AddSuggestion(
              RemoteSuggestionBuilder().AddId("http://new-id-2").Build())
          .Build());
  FetchSuggestions(provider.get(), /*interactive_request=*/true,
                   Status(StatusCode::SUCCESS, "message"),
                   std::move(fetched_categories));
...
TEST_F(RemoteSuggestionsProviderImplTest,
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(articles_category())
          .AddSuggestion(
              RemoteSuggestionBuilder().AddId("http://abc.com/").Build())
          .Build());
  FetchSuggestions(provider.get(), /*interactive_request=*/false,
...
TEST_F(RemoteSuggestionsProviderImplTest, Dismiss) {
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(articles_category())
          .AddSuggestion(
              RemoteSuggestionBuilder().AddId("http://site.com").Build())
          .Build());
  FetchSuggestions(provider.get(), /*interactive_request=*/false,
                   Status(StatusCode::SUCCESS, "message"),
                   std::move(fetched_categories));
...
TEST_F(RemoteSuggestionsProviderImplTest, RemoveExpiredDismissedContent) {
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(articles_category())
          .AddSuggestion(
              RemoteSuggestionBuilder().AddId("http://second/").Build())
          .Build());
  FetchSuggestions(provider.get(), /*interactive_request=*/true,
                   Status(StatusCode::SUCCESS, "message"),
                   std::move(fetched_categories));
...
TEST_F(RemoteSuggestionsProviderImplTest, TestSingleSourceWithMissingData) {
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(articles_category())
          .AddSuggestion(
              RemoteSuggestionBuilder().SetPublisher("").SetAmpUrl("").Build())
          .Build());
  FetchSuggestions(provider.get(), /*interactive_request=*/false,
                   Status(StatusCode::SUCCESS, "message"),
                   std::move(fetched_categories));
...
TEST_F(RemoteSuggestionsProviderImplTest, ImageReturnedWithTheSameId) {
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(articles_category())
          .AddSuggestion(
              RemoteSuggestionBuilder().AddId(kSuggestionUrl).Build())
          .Build());
  FetchSuggestions(provider.get(), /*interactive_request=*/true,
                   Status(StatusCode::SUCCESS, "message"),
                   std::move(fetched_categories));
...
TEST_F(RemoteSuggestionsProviderImplTest, ClearHistoryRemovesAllSuggestions) {
  ...
  fetched_categories.push_back(
      FetchedCategoryBuilder()
          .SetCategory(articles_category())
          .AddSuggestion(
              RemoteSuggestionBuilder().AddId("http://first/").Build())
          .AddSuggestion(
              RemoteSuggestionBuilder().AddId("http://second/").Build())
          .Build());
  FetchSuggestions(provider.get(), /*interactive_request=*/true,
...
TEST_F(RemoteSuggestionsProviderImplTest,
  ...
  for (int i = 0; i < kSuggestionsCount; ++i) {
    category_builder.AddSuggestion(
        RemoteSuggestionBuilder()
            .AddId(base::StringPrintf("http://abc.com/%d/", i))
            .Build());
  }
...
TEST_F(RemoteSuggestionsProviderImplTest,
  ...
  for (int i = 0; i < kSuggestionsPerCategory; ++i) {
    first_category_builder.AddSuggestion(
        RemoteSuggestionBuilder()
            .AddId(base::StringPrintf("http://abc.com/%d/", i))
            .Build());
  }
  ...
  for (int i = 0; i < kSuggestionsPerCategory; ++i) {
    second_category_builder.AddSuggestion(
        RemoteSuggestionBuilder()
            .AddId(base::StringPrintf("http://other.com/%d/", i))
            .Build());
  }
