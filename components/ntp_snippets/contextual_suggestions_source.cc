...
void ContextualSuggestionsSource::FetchContextualSuggestions(
  ...
  contextual_suggestions_fetcher_->FetchContextualSuggestions(
      url,
      base::BindOnce(&ContextualSuggestionsSource::OnFetchContextualSuggestions,
...
void ContextualSuggestionsSource::FetchContextualSuggestionImage(
  ...
  if (image_url_by_id_.count(id_within_category) > 0) {
    ...
    ImageFetchedCallback image_fetched_callback = base::Bind(
        &ContextualSuggestionsSource ::OnFetchContextualSuggestionImage,
...
void ContextualSuggestionsSource::OnFetchContextualSuggestions(
    const GURL& url,
    FetchContextualSuggestionsCallback callback,
    Status status,
...
void ContextualSuggestionsSource::OnFetchContextualSuggestionImage(
    const ImageFetchedCallback& callback,
