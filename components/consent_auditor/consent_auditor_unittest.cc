...
TEST_F(ConsentAuditorTest, LocalConsentPrefRepresentation) {
  ...
  EXPECT_EQ(version->GetString(), version_info::GetVersionNumber());
  ...
  const std::string kFeature2NewProductVersion = "1.2.3.4";
  consent_auditor()->set_product_version_for_testing(
      kFeature2NewProductVersion);
