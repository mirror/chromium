...
// and |locale|, in that order, from |record|.
void LoadEntriesFromLocalConsentRecord(const base::Value* record,
...
TEST_F(ConsentAuditorTest, LocalConsentPrefRepresentation) {
  ...
  std::string version;
  std::string locale;
  LoadEntriesFromLocalConsentRecord(record, &description, &confirmation,
                                    &version, &locale);
