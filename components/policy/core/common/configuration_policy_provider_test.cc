...
TEST_P(Configuration3rdPartyPolicyProviderTest, Load3rdParty) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    dict->Set("subdict", base::MakeUnique<base::Value>(policy_dict.Clone()));
    list->Append(std::move(dict));
  }
  policy_dict.Set("list", std::move(list));
  policy_dict.Set("dict", base::MakeUnique<base::Value>(policy_dict.Clone()));
  ...
  policy_3rdparty.Set("extensions.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                      base::MakeUnique<base::Value>(policy_dict.Clone()));
