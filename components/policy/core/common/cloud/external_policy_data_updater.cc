...
void ExternalPolicyDataUpdater::FetchJob::OnFetchFinished(
  ...
  switch (result) {
    ...
    case ExternalPolicyDataFetcher::MAX_SIZE_EXCEEDED:
      ...
      LOG(WARNING) << "Failed to fetch the data due to the excessize size";
...
void ExternalPolicyDataUpdater::FetchJob::OnFailed(net::BackoffEntry* entry) {
  if (entry) {
    ...
    base::TimeDelta rescheduling_delay = entry->GetTimeUntilRelease();
    DVLOG(1) << "Rescheduling the fetch in " << rescheduling_delay;
    ...
    updater_->task_runner_->PostDelayedTask(
        FROM_HERE, base::Bind(&FetchJob::Reschedule, AsWeakPtr()),
        rescheduling_delay);
  }
...
void ExternalPolicyDataUpdater::FetchExternalData(
  ...
  if (job) {
    ...
    if (job->request() == request) {
      DVLOG(2) << "Skipping fetching for " << key
               << " due to the existing job with the same parameters";
      return;
    }
...
void ExternalPolicyDataUpdater::OnJobFailed(FetchJob* job) {
  ...
  // Don't touch |job_map_|; deletion of |FetchJobs| cause a call to this
