...
// To ignore the return value of FreeLibrary.
void FreeLibraryLocal(HINSTANCE hinstance) {
...
bool IsDomainJoined() {
  ...
  if (PathService::Get(base::DIR_SYSTEM, &path)) {
    HINSTANCE net_api_library = ::LoadLibraryEx(
        path.Append(FILE_PATH_LITERAL("Netapi32_function.dll")).value().c_str(),
        nullptr, LOAD_WITH_ALTERED_SEARCH_PATH);
    free_library.ReplaceClosure(
        base::BindOnce(&FreeLibraryLocal, net_api_library));
    if (net_api_library) {
