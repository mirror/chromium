...
//
// This class assumes that it is created on the same thread that the PrefService
// lives on. During a pref change for either the block list or allow list, we
// schedule an event back to the main thread. This allows us to restrict the
// number of blocklist changes we have to make to one per event loop cycle.
// When we process the scheduled event, we send a task to a background tasks
// which crunches the data and returns back to the main thread.
class POLICY_EXPORT URLBlacklistManager {
 ...
 private:
  ...
  // Used to schedule tasks on the main look to avoid rebuilding the blocklist
