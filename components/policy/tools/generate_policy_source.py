...
class PolicyDetails:
  ...
  def __init__(self, policy, chrome_major_version, os, is_chromium_os,
    ...
    self.supported_cros_management = \
        policy.get('supported_cros_management',
...
def _GetSupportedChromeOSPolicies(policies, type):
  ...
  return filter(lambda policy: policy.is_supported and
                    (type != 'user' or not policy.is_device_only) and
                    (type != 'device' or policy.is_device_only) and
                    'active_directory' in policy.supported_cros_management,
                policies)
