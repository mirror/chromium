...
WriterDesc = collections.namedtuple(
    'WriterDesc', ['type', 'is_per_language', 'encoding'])

_WRITER_DESCS = [
  WriterDesc('adm', True, 'utf-16'),
  WriterDesc('adml', True, 'utf-16'),
  WriterDesc('admx', False, 'utf-16'),
  WriterDesc('google_adml', True, 'utf-16'),
  WriterDesc('google_admx', False, 'utf-16'),
  WriterDesc('android_policy', False, 'utf-8'),
  WriterDesc('reg', False, 'utf-16'),
  WriterDesc('doc', True, 'utf-8'),
  WriterDesc('json', False, 'utf-8'),
  WriterDesc('plist', False, 'utf-8'),
  WriterDesc('plist_strings', True, 'utf-8')
]
...
def _GetWriterConfiguration(grit_defines):
  ...
  Args:
    grit_defines: Grid defines, see grit_rule.gni.
  '''
  # Build a dictionary from grit defines, which can be plain DEFs or KEY=VALUEs.
  grit_defines = {}
  for define in grit_defines:
    parts = define.split('=', 1)
    grit_defines[parts[0]] = parts[1] if len(parts) > 1 else 1
  return writer_configuration.GetConfigurationForBuild(grit_defines)
  ...
  for lang in languages:
    ...
    for writer_desc in _WRITER_DESCS:
      ...
      if (writer_desc.is_per_language):
        ...
        output_path = output_path.replace(_LANG_PLACEHOLDER, lang)
      else:
