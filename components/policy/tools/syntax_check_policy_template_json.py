...
class PolicyTemplateChecker(object):
  ...
  def _CheckFormat(self, filename):
    ...
    # inside a multiple line strings.
    three_quotes_cnt = 0
  ...
  def Main(self, filename, options):
    ...
    policy_names = [policy['name'] for policy in policy_definitions
    ...
    for group in [policy for policy in policy_definitions
      ...
      for policy in group['policies']:
        if not policy in policy_names:
          self._Error('Group %s contains policy %s which does not exist' %
                      (group['name'], policy), None, None, group)
        elif policy in policy_in_groups:
          self._Error('Policy %s defined in several groups.' % (policy))
        else:
          policy_in_groups.add(policy)
