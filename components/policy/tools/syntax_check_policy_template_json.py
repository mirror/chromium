...
class PolicyTemplateChecker(object):
  ...
  def _CheckFormat(self, filename):
    ...
    if three_quotes_cnt % 2 != 0:
      self._Error('String literal started with three quotes is not closed.')
      ...
      for policy_name in group['policies']:
        if policy_name not in policy_names:
          self._Error('Group %s contains policy %s which does not exist' %
                      (group['name'], policy_name), None, None, group)
        elif policy_name in policy_in_groups:
