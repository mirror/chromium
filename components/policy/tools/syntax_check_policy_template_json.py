...
class PolicyTemplateChecker(object):
  ...
  def _CheckPolicy(self, policy, is_in_group, policy_ids):
    ...
    if policy_type == 'group':
      ...
      # Chrome OS policies may have a non-empty supported_cros_management list
      # with either 'ad' or 'gc' or both.
      supported_cros_management = \
          self._CheckContains(policy, 'supported_cros_management', list, True)
      if supported_cros_management is not None:
        # Must be on Chrome OS.
        if (supported_on is not None and
            not any('chrome_os:' in str for str in supported_on)):
          self._Error('"supported_cros_management" is only supported on Chrome'
              ' OS', 'policy', policy, supported_on)
        # Must be non-empty.
        if len(supported_cros_management) == 0:
          self._Error('"supported_cros_management" must be non-empty', 'policy',
              policy)
        # Must be either 'ad' or 'gc'.
        if (any(str != 'gc' and str != 'ad'
            for str in supported_cros_management)):
          self._Error('Values in "supported_cros_management" must be either '
              '"ad" or "gc"', 'policy', policy, supported_cros_management)
