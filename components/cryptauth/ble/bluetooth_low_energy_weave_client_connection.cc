  ...
  if (!bluetooth_device) {
    ...
    DestroyConnection(
        BleWeaveConnectionResult::
            BLE_WEAVE_CONNECTION_RESULT_ERROR_SETTING_CONNECTION_LATENCY);
    return;
  }
...
void BluetoothLowEnergyWeaveClientConnection::OnTimeoutForSubStatus(
  ...
  switch (status) {
    ...
    default:
      result = BleWeaveConnectionResult::
          BLE_WEAVE_CONNECTION_RESULT_TIMEOUT_UNKNOWN_REASON;
      break;
  }
...
void BluetoothLowEnergyWeaveClientConnection::OnRemoteCharacteristicWritten() {
  ...
  if (!pending_write_request_) {
    ...
    DestroyConnection(
        BleWeaveConnectionResult::
            BLE_WEAVE_CONNECTION_RESULT_ERROR_NO_PENDING_WRITE_REQUEST);
    return;
  }
