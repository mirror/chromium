...
void BluetoothLowEnergyWeaveClientConnection::OnRemoteCharacteristicWritten() {
  ...
  if (pending_write_request_->request_type ==
    ...
    if (sent_message.get() != pending_write_request_->associated_wire_message) {
      PA_LOG(ERROR) << "Wrote a packet, but it does not correspond to the "
                    << "pending WireMessage. Destroying connection to "
                    << GetDeviceInfoLogString();
      DestroyConnection();
      return;
    }
...
void BluetoothLowEnergyWeaveClientConnection::OnWriteRemoteCharacteristicError(
  ...
  if (pending_write_request_->request_type == WriteRequestType::REGULAR ||
    ...
    if (failed_message.get() !=
      ...
      PA_LOG(ERROR) << "Failed to write a packet, but it does not correspond "
                    << "to the pending WireMessage. Destroying connection to "
                    << GetDeviceInfoLogString();
      DestroyConnection();
      return;
    }
