...
class RemoteDeviceProviderTest : public testing::Test {
 public:
  ...
  std::vector<cryptauth::ExternalDeviceInfo> test_device_infos_;
...
// Test that the observers are actually notified upon Remote Device syncing.
TEST_F(RemoteDeviceProviderTest, TestOnSyncCalls) {
  ...
  remote_device_provider_->RemoveObserver(test_observer.get());
  remote_device_provider_->OnSyncFinished(
      CryptAuthDeviceManager::SyncResult::SUCCESS,
      CryptAuthDeviceManager::DeviceChangeResult::CHANGED);
  test_device_loader_factory_->InvokeLastCallback();
  // removed, so should be the same as last time.
  EXPECT_EQ(2, test_observer->numTimesOnSyncDeviceListchangedCalled);
}

// Test that the Remote Devices are actually saved correctly.
TEST_F(RemoteDeviceProviderTest, TestValidityOfRemoteDevices) {
  ...
  EXPECT_EQ(5, len);
  for (int i = 0; i < len; i++) {
    EXPECT_EQ(test_devices_[i], saved_devices[i]);
  }
}

// Tests that updates do change the state of the Remote Device Provider.
TEST_F(RemoteDeviceProviderTest, TestRemoteDeviceUpdatesProperly) {
  EXPECT_EQ(0u, remote_device_provider_->GetSyncedDevices().size());
  remote_device_provider_->OnSyncFinished(
      CryptAuthDeviceManager::SyncResult::SUCCESS,
      CryptAuthDeviceManager::DeviceChangeResult::CHANGED);
  test_device_loader_factory_->InvokeLastCallback();
  EXPECT_EQ(5u, remote_device_provider_->GetSyncedDevices().size());
  test_devices_.pop_back();
  remote_device_provider_->OnSyncFinished(
      CryptAuthDeviceManager::SyncResult::SUCCESS,
      CryptAuthDeviceManager::DeviceChangeResult::CHANGED);
  test_device_loader_factory_->InvokeLastCallback();
  EXPECT_EQ(4u, remote_device_provider_->GetSyncedDevices().size());
}
