...
void DeviceCapabilityManager::IsDevicePromotable(
    const std::string& public_key,
    Capability capability,
    const base::Callback<void(const bool&)>& success_callback,
    const base::Callback<void(const std::string&)>& error_callback) {
  current_cryptauth_client_ = crypt_auth_client_factory_->CreateInstance();
  pending_requests_.emplace(
      base::MakeUnique<Request>(RequestType::IS_DEVICE_PRMOTABLE, capability,
                                public_key, success_callback, error_callback));
  ProcessRequestQueue();
}
...
DeviceCapabilityManager::Request::Request(
    ...
    Capability capability,
    std::string public_key,
    const base::Callback<void(const bool&)> is_device_promotable_callback,
    const base::Callback<void(const std::string&)>& error_callback)
    : request_type_(request_type),
      error_callback_(error_callback),
      capability_(capability),
      public_key_(public_key),
      is_device_promotable_callback_(is_device_promotable_callback) {}
...
void DeviceCapabilityManager::ProcessFindEligibleRequest() {
  DCHECK(current_request_->capability_ == Capability::CAPABILITY_UNLOCK_KEY);
...
void DeviceCapabilityManager::ProcessIsDevicePromotableRequest() {
  DCHECK(current_request_->capability_ == Capability::CAPABILITY_UNLOCK_KEY);
...
void DeviceCapabilityManager::IsDeviceUnlockPromotable() {
  ...
  request.set_promoter_public_key(current_request_->public_key_);
...
void DeviceCapabilityManager::OnIsDeviceUnlockPromotableResponse(
  ...
  current_cryptauth_client_.reset();
  current_request_->is_device_promotable_callback_.Run(
...
void DeviceCapabilityManager::ProcessRequestQueue() {
  ...
  switch (current_request_->request_type_) {
    ...
    case RequestType::FIND_ELIGIBLE_DEVICES:
      ProcessFindEligibleRequest();
      return;
    case RequestType::IS_DEVICE_PRMOTABLE:
      ProcessIsDevicePromotableRequest();
      return;
    default:
      NOTREACHED();
      return;
