...
class OfflineContentAggregator : public OfflineContentProvider,
 ...
 private:
  ...
  void OnGetAllItemsDone(const std::string& name_space,
                         const OfflineItemList& items);
  void OnGetItemByIdDone(const ContentId& id, const OfflineItem* item);
  ...
  // Used by GetItemById and the corresponding callback.
  using SingeItemCallbackMap = std::map<ContentId, SingleItemCallback>;
  SingeItemCallbackMap single_item_get_callbacks_;
