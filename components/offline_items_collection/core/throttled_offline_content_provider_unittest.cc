...
TEST_F(ThrottledOfflineContentProviderTest, TestBasicPassthrough) {
  ...
  provider_.GetAllItems(base::Bind(multi_item_callback, 1));

  auto single_item_callback = [](const OfflineItem& expected,
                                 const OfflineItem* item) {
    EXPECT_EQ(expected.id, item->id);
  };
  provider_.GetItemById(id, base::Bind(single_item_callback, item));
}
...
TEST_F(ThrottledOfflineContentProviderTest, TestGetItemByIdOverridesUpdate) {
  ...
  auto single_item_callback = [](const OfflineItem* item) {};
  provider_.GetItemById(id1, base::Bind(single_item_callback));
...
TEST_F(ThrottledOfflineContentProviderTest, TestGetAllItemsOverridesUpdate) {
  ...
  provider_.GetAllItems(base::Bind(callback));
