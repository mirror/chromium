...
SuggestionsServiceImpl::SuggestionsServiceImpl(
  ...
  blacklist_upload_backoff_.InformOfRequest(true);
}
...
void SuggestionsServiceImpl::SetNextBlacklistDelayForTesting(
    base::TimeDelta delay) {
  blacklist_upload_backoff_.SetCustomReleaseTime(
      blacklist_upload_backoff_.GetTimeTicksNow() + delay);
  if (blacklist_upload_timer_.IsRunning()) {
    ScheduleBlacklistUpload();
  }
}
...
void SuggestionsServiceImpl::UpdateBlacklistDelay(
  ...
  blacklist_upload_backoff_.InformOfRequest(last_request_successful);
}
