...
void InProgressMetadataCache::AddOrReplaceMetadata(const DownloadEntry& entry) {
  AddOrReplaceEntryInEntries(
      InProgressMetadataConversions::DownloadEntryToProto(entry));
  task_runner_->PostTask(
      FROM_HERE, base::Bind(&InProgressMetadataCache::WriteEntriesToFile,
                            weak_ptr_factory_.GetWeakPtr()));
}

base::Optional<DownloadEntry> InProgressMetadataCache::RetrieveMetadata(
...
void InProgressMetadataCache::AddOrReplaceEntryInEntries(
...
base::Optional<DownloadEntry> InProgressMetadataCache::GetEntryFromEntries(
    const std::string& guid) {
  int entry_index = GetIndexFromEntries(guid);
  if (entry_index < 0) {
    return base::nullopt;
  }
  metadata_pb::DownloadEntry* entry_ptr = entries_.mutable_entry(entry_index);
