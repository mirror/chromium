...
std::pair<bool, std::set<base::FilePath>> HardRecoverOnFileThread(
    const base::FilePath& directory) {
  base::ThreadRestrictions::AssertIOAllowed();
  std::set<base::FilePath> files_in_dir;
  GetFilesInDirectory(directory, files_in_dir);
  ...
  return std::make_pair(CreateDirectoryIfNotExists(directory), files_in_dir);
}
...
void FileMonitorImpl::HardRecover(const HardRecoverCallback& callback) {
  base::PostTaskAndReplyWithResult(
      file_thread_task_runner_.get(), FROM_HERE,
