...
void FileMonitorImpl::DeleteUnknownFilesOnFileThread(
  ...
  std::vector<base::FilePath> files_to_remove =
      base::STLSetDifference<std::vector<base::FilePath>>(files_in_dir,
                                                          download_file_paths);
  VLOG(0) << "FileMonitorImpl::DeleteUnknownFilesOnFileThread, files_to_remove "
          << files_to_remove.size() << " files_in_dir " << files_in_dir.size()
          << " download_file_paths " << download_file_paths.size();
...
void FileMonitorImpl::DeleteFilesOnFileThread(
  ...
  VLOG(0) << "shakti, num_delete_attempted " << num_delete_attempted
          << " num_delete_failed " << num_delete_failed
          << " num_delete_by_external " << num_delete_by_external;
