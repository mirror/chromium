...
TEST_F(DownloadServiceControllerImplTest, Cancel) {
  ...
  // The driver should have removed the download.
  EXPECT_FALSE(driver_->Find(entry.guid).has_value());
...
TEST_F(DownloadServiceControllerImplTest, OnDownloadFailed) {
  ...
  EXPECT_CALL(*client_, OnDownloadFailed(entry.guid));
...
TEST_F(DownloadServiceControllerImplTest, OnDownloadSucceeded) {
  ...
  EXPECT_CALL(*client_, OnDownloadSucceeded(entry.guid, path,
