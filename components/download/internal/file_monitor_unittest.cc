...
TEST_F(FileMonitorTest, TestDeleteUnknownFiles) {
  ...
  EXPECT_TRUE(base::CreateTemporaryFileInDir(
      download_dir_, &driver_entry2.temporary_physical_file_path));
  ...
  VLOG(0) << "shakti, download_dir_ " << download_dir_.value();
  VLOG(0) << "shakti, entry1.target_file_path "
          << entry1.target_file_path.value()
          << " driver_entry1.temporary_physical_file_path "
          << driver_entry1.temporary_physical_file_path.value();

  EXPECT_TRUE(base::PathExists(download_dir_));
  ...
  VLOG(0) << "shakti, check_file_existence1 ";
  ...
  VLOG(0) << "shakti, check_file_existence2,  ";
  ...
  monitor_->DeleteUnknownFiles(entries, driver_entries);
  task_runner_->RunUntilIdle();
  VLOG(0) << "shakti, check_file_existence3 ";
  check_file_existence(true, true, true, true, false, false);
  ...
  monitor_->DeleteUnknownFiles(entries, driver_entries);
  task_runner_->RunUntilIdle();
  VLOG(0) << "shakti, check_file_existence4 ";
  ...
  monitor_->DeleteUnknownFiles(entries, driver_entries);
  task_runner_->RunUntilIdle();
  VLOG(0) << "shakti, check_file_existence5 ";
