...
void FileMonitor::DeleteUnknownFiles(
  ...
  file_thread_task_runner_->PostTask(
      FROM_HERE, base::Bind(&FileMonitor::DeleteUnknownFilesOnFileThread,
                            weak_factory_.GetWeakPtr(), download_file_paths));
}

std::vector<Entry*> FileMonitor::CleanupFilesForCompletedEntries(
  ...
  file_thread_task_runner_->PostTask(
      FROM_HERE, base::Bind(&FileMonitor::DeleteFilesOnFileThread,
...
void FileMonitor::DeleteUnknownFilesOnFileThread(
