...
void FileMonitor::CancelOrphanedRequests(const ClientSet* clients) {
  ...
  file_thread_task_runner_->PostTaskAndReply(
      ...
      base::Bind(&FileMonitor::DeleteFiles, weak_factory_.GetWeakPtr(),
                 files_to_remove, stats::FileCleanupReason::UNKNOWN_CLIENT),
      base::Bind(&FileMonitor::OnFilesDeleted, weak_factory_.GetWeakPtr(),
                 entries_to_remove));
}
...
void FileMonitor::RemoveUnassociatedFilesOnFileThread(
  ...
  base::FileEnumerator file_enumerator(file_dir_, false, /* recursive */
...
void FileMonitor::OnFilesDeleted(const std::vector<Entry*>& entries) {
  for (auto* entry : entries) {
