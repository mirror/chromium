...
// 3. After network IO is done, Blob related work is done on IO thread based on
// |io_task_runner_| with |blob_task_proxy_|, then notify the result to main
// thread.
...
class InMemoryDownload : public net::URLFetcherDelegate {
 ...
 private:
  ...
  // Delegate to communicate with and report download status, can be null.
  Delegate* delegate_;

  uint64_t bytes_downloaded_ = 0u;
