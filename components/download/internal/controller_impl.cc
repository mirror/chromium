...
bool ControllerImpl::OnStopScheduledTask(DownloadTaskType task_type) {
  ...
  stats::LogScheduledTaskStatus(task_type,
                                stats::ScheduledTaskStatus::CANCELLED_ON_STOP);
  return true;
}

void ControllerImpl::ProcessScheduledTasks() {
  // TODO(shaktisahu): Run the scheduled tasks.

  for (auto task_entry : task_finished_callbacks_) {
    stats::LogScheduledTaskStatus(
        task_entry.first, stats::ScheduledTaskStatus::COMPLETED_NORMALLY);
  }
}
...
void ControllerImpl::NotifyTaskFinished(bool needs_reschedule,
  ...
  for (auto& task_entry : task_finished_callbacks_) {
    base::ResetAndReturn(&task_entry.second).Run(needs_reschedule);
    stats::LogScheduledTaskStatus(task_entry.first, status);
  }
