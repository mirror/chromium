...
Entry::Entry(const Entry& other) {
  ...
  if (other.has_traffic_annotation_tag()) {
    traffic_annotation_.reset(new net::NetworkTrafficAnnotationTag(
        other.get_traffic_annotation_tag()));
  }
}
...
Entry::Entry(const DownloadParams& params)
    : client(params.client),
      ...
      attempt_count(0),
      traffic_annotation_(
          new net::NetworkTrafficAnnotationTag(params.traffic_annotation_)) {}
...
bool Entry::operator==(const Entry& other) const {
  return client == other.client && guid == other.guid &&
         ...
         completion_time == other.completion_time &&
         attempt_count == other.attempt_count &&
         (has_traffic_annotation_tag() == other.has_traffic_annotation_tag() &&
          (!has_traffic_annotation_tag() ||
           get_traffic_annotation_tag() == other.get_traffic_annotation_tag()));
}
