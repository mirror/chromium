...
void TaskManager::UnscheduleTask(DownloadTaskType type) {
  auto it = scheduled_params_.find(type);
  if (it != scheduled_params_.end() && it->second == base::nullopt)
    return;
  scheduled_params_[type] = base::nullopt;

  if (IsRunning(type))
    return;

  // Always cancel even if we did not have a task queued up that we're aware of
  // for this type of download.
...
void TaskManager::FinishTask(DownloadTaskType type,
  ...
  if (has_params) {
    ...
    // This is a precaution.  We should not have to do this.  It means we've
    // either (1) never scheduled a new task or (2) already cancelled one.
    scheduler_->CancelTask(type);
  }
}
