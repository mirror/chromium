...
// Default value for start up delay to wait for network stack to be ready.
const base::TimeDelta kDefaultStartUpDelay = base::TimeDelta::FromSeconds(5);
...
std::unique_ptr<Configuration> Configuration::CreateFromFinch() {
  ...
  config->start_up_delay = base::TimeDelta::FromMilliseconds(
      base::saturated_cast<int>(GetFinchConfigUInt(
          kStartUpDelayMsConfig, kDefaultStartUpDelay.InMilliseconds())));
...
Configuration::Configuration()
    : max_concurrent_downloads(kDefaultMaxConcurrentDownloads),
      ...
      start_up_delay(kDefaultStartUpDelay),
