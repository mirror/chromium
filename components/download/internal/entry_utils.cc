...
DownloadMetaData BuildDownloadMetaData(Entry* entry) {
  ...
  if (entry->state == Entry::State::COMPLETE) {
    meta_data.completion_info = CompletionInfo();
    meta_data.completion_info->path = entry->target_file_path;
    meta_data.completion_info->bytes_downloaded = entry->bytes_downloaded;
  }
  return meta_data;
}
