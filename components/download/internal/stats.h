...
// Converts DownloadClient to a histogram suffix.
...
// Logs download completion event, including the completion type, retry count,
// the file size and time span for the download.
void LogDownloadCompletion(CompletionType type,
                           unsigned int attempt_count,
                           const base::TimeDelta& time_span);

// Logs the download file size, only for succeeded downloads.
...
// Logs the file life time for successfully completed download. The metric will
// be recorded even if the file deletion fails.
...
// Logs the if the final download file path is different from the requested file
// path.
