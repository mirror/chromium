...
Entry* SchedulerImpl::Next(const Model::EntryList& entries,
  ...
  for (size_t i = 0; i < download_clients_.size(); ++i) {
    ...
    // Records the first available candidate. Keep iterating because there might
    // be UI priority entries created by other clients.
    if (!entry) {
      entry = candidate;
      DCHECK(entry);
    ...
    // Entry with UI priority should be processed immediately.
    if (candidate->scheduling_params.priority ==
        SchedulingParams::Priority::UI) {
      entry = candidate;
      DCHECK(entry);
      current_client_index_ = (index + i + 1) % download_clients_.size();
      break;
...
std::map<DownloadClient, Entry*> SchedulerImpl::FindCandidates(
    ...
    const DeviceStatus& device_status) {
  std::map<DownloadClient, Entry*> candidates;
  for (DownloadClient client : download_clients_)
    candidates[client] = nullptr;
