...
// The default delay to notify the observer when network is connected.
...
void DeviceStatusListener::OnConnectionTypeChanged(
  ...
  if (change_to_online) {
    // It's unreliable to send requests immediately after the network changes to
    // online, notify the observer after a delay.
    timer_.Start(FROM_HERE, kNotifyNetworkOnlineDelay, this,
