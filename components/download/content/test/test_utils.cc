...
class TesteEmptyLogger : public EmptyLogger, public BlackHoleLogSink {
 public:
  TesteEmptyLogger() = default;
  ~TesteEmptyLogger() override = default;

 private:
  DISALLOW_COPY_AND_ASSIGN(TesteEmptyLogger);
};

DownloadService* DownloadServiceTestHarness::CreateDownloadServiceForTest(
  ...
  device_listener_ = device_status_listener.get();
  ...
  auto logger = base::MakeUnique<TesteEmptyLogger>();
...
void DownloadServiceTestHarness::SetDeviceStatus(bool battery_charging,
                                                 int network_status) {
  DCHECK(device_listener_);
  DeviceStatus status;
  status.battery_status =
      battery_charging ? BatteryStatus::CHARGING : BatteryStatus::NOT_CHARGING;
  status.network_status = static_cast<NetworkStatus>(network_status);
  device_listener_->SetDeviceStatus(status);
}
