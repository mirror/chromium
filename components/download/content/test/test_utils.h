...
namespace test {
class TestDeviceStatusListener;

// Test harness to access necessary internal functionalities.
class DownloadServiceTestHarness {
 public:
  DownloadServiceTestHarness() = default;
  ~DownloadServiceTestHarness() = default;
  ...
  void SetDeviceStatus(bool battery_charging, int network_status);

 private:
  TestDeviceStatusListener* device_listener_ = nullptr;

  DISALLOW_COPY_AND_ASSIGN(DownloadServiceTestHarness);
};
