...
std::vector<char> ReadEntriesFromFile(base::FilePath file_path) {
  LOG(ERROR) << "joy: ReadEntriesFromFile";
  ...
  if (!entries_file.Read(0, file_data.data(), size)) {
    LOG(ERROR) << "Could not read download entries from file "
...
void InProgressCacheImpl::Initialize(const base::Closure& callback) {
  ...
  if (initialization_status_ == CACHE_INITIALIZED) {
    LOG(ERROR) << "joy: cache initialized";
...
void InProgressCacheImpl::OnInitialized(std::vector<char> entries) {
  ...
  while (!pending_actions_.empty()) {
    LOG(ERROR) << "joy: pending_actions_";
    task_runner_->PostTask(FROM_HERE, pending_actions_.front());
