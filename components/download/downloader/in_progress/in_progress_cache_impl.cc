...
void ReadEntriesFromFile(base::FilePath file_path,
                         metadata_pb::DownloadEntries* entries) {
...
void InProgressCacheImpl::Initialize(const base::Closure& callback) {
  ...
  task_runner_->PostTaskAndReply(
      FROM_HERE, base::BindOnce(&ReadEntriesFromFile, file_path_, &entries_),
      base::BindOnce(&OnInitialized, callback));
}
...
bool InProgressCacheImpl::IsInitialized() {
  return is_initialized_;
}

void InProgressCacheImpl::AddOrReplaceEntry(const DownloadEntry& entry) {
  if (!is_initialized_) {
    LOG(ERROR) << "Cache is not initialized, cannot perform operation "
               << "AddOrReplaceEntry." return;
  }
...
base::Optional<DownloadEntry> InProgressCacheImpl::RetrieveEntry(
  ...
  if (!is_initialized_) {
    LOG(ERROR) << "Cache is not initialized, cannot perform operation "
               << "RetrieveEntry." return base::nullopt;
  }
