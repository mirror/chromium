...
class COMPONENTS_DOWNLOAD_EXPORT DownloadItem : public base::SupportsUserData {
 public:
  ...
  // Delegate methods-----------------------------------------------------------
  // Sets/Gets the delegate for this DownloadItem. The DownloadItem will take
  // the ownership of the delegate.
  virtual void SetDelegate(std::unique_ptr<DownloadItemDelegate> delegate) = 0;
  virtual DownloadItemDelegate* GetDelegate() const = 0;
