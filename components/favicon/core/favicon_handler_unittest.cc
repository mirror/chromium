...
class FakeImageDownloader {
 public:
  ...
  int DownloadImage(const GURL& url,
    ...
    for (SkBitmap& bitmap : response.bitmaps) {
      // Apply maximum image size.
      if (bitmap.width() > max_image_size || bitmap.height() > max_image_size) {
        bitmap = skia::ImageOperations::Resize(
            bitmap, skia::ImageOperations::RESIZE_LANCZOS3, max_image_size,
            max_image_size);
      }
    }
...
TEST_F(FaviconHandlerTest, MultipleFavicons404) {
  ...
  EXPECT_CALL(delegate_, OnFaviconUpdated(_, _, kIconURL16x16, _, _));
  RunHandlerWithSimpleFaviconCandidates({k404IconURL, kIconURL16x16});
  EXPECT_THAT(delegate_.downloads(), ElementsAre(kIconURL16x16));
}
...
TEST_F(FaviconHandlerTest, MultipleFaviconsLast404) {
  ...
  EXPECT_CALL(delegate_, OnFaviconUpdated(_, _, kIconURL16x16, _, _));
  RunHandlerWithSimpleFaviconCandidates({kIconURL16x16, k404IconURL});
  EXPECT_THAT(delegate_.downloads(), ElementsAre(kIconURL16x16));
}
