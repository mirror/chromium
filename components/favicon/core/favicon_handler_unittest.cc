...
// Test that favicon mappings are not deleted when a page URL is not in history
// even if the page lists no favicons.
...
// Test that favicon mappings are deleted when a page in history lists no
// candidates.
...
// Test that favicon mappings are not deleted when a page URL is not in history
// even if all icons listed in the page return a 404.
...
TEST_F(FaviconHandlerManifestsEnabledTest,
  ...
  EXPECT_CALL(favicon_service_, DeleteFaviconMappings(std::set<GURL>{kPageURL},
                                                      WEB_MANIFEST_ICON));
