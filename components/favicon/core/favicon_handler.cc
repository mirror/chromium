...
void FaviconHandler::FetchFavicon(const GURL& page_url, bool is_same_document) {
  ...
  if (!is_same_document || non_manifest_original_candidates_.has_value()) {
    page_urls_.clear();
  }
  ...
  manifest_url_ = GURL();
  non_manifest_original_candidates_.reset();
  candidates_.clear();
  ...
  // up-to-date / latest URL for DB lookups.
...
void FaviconHandler::OnUpdateCandidates(
  ...
  if ((!manifests_feature_enabled || manifest_url_ == manifest_url) &&
      ((!non_manifest_original_candidates_.has_value() && candidates.empty()) ||
