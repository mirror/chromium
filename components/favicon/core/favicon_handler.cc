...
void FaviconHandler::OnPageNavigation(const GURL& page_url,
  ...
  if (!is_same_document || !non_manifest_original_candidates_.empty()) {
    page_urls_.clear();
  }
  page_urls_.push_back(page_url);
  ...
  service_->GetFaviconForPageURL(
      page_url, icon_types_, preferred_icon_size(),
...
void FaviconHandler::NotifyFaviconUpdated(const GURL& icon_url,
  ...
  delegate_->OnFaviconUpdated(page_urls_.back(), handler_type_, icon_url,
...
void FaviconHandler::OnUpdateCandidates(
  ...
  // Even if the page lists no favicons, content infers /favicon.ico.
  DCHECK(!candidates.empty());

  if (page_urls_.empty() || page_urls_.back() != page_url)
    return;
...
void FaviconHandler::GetFaviconAndUpdateMappingsUnlessIncognito(
  ...
  if (delegate_->IsOffTheRecord()) {
    ...
    service_->UpdateFaviconMappingsAndFetch(
        page_urls_.back(), icon_url, icon_type, preferred_icon_size(), callback,
        &cancelable_task_tracker_for_candidates_);
  }
}
