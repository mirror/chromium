...
void AdjustPageParamsForDpi(PrintHostMsg_DidPrintDocument_Params& page_params,
  ...
  if (dpi.width() == dpi.height())
    return;
  int dpi_min = std::min(dpi.width(), dpi.height());
  page_params.page_size =
      gfx::Size(page_params.page_size.width() * dpi.width() / dpi_min,
                page_params.page_size.height() * dpi.height() / dpi_min);
  page_params.content_area =
      gfx::Rect(page_params.content_area.x() * dpi.width() / dpi_min,
                page_params.content_area.y() * dpi.height() / dpi_min,
                page_params.content_area.width() * dpi.width() / dpi_min,
                page_params.content_area.height() * dpi.height() / dpi_min);
