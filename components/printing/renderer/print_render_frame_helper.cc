...
void PrepareFrameAndViewForPrint::ResizeForPrinting() {
  ...
  // PDFs and Flash do not need to be resized. Resizing PDFs causes a flicker
  // in the top left corner behind the preview. See crbug.com/739973.
...
void PrepareFrameAndViewForPrint::RestoreSize() {
  ...
  // Do not restore PDFs or Flash since they are not resized.
  if (PrintingNodeOrPdfFrame(frame(), node_to_print_))
    return;
