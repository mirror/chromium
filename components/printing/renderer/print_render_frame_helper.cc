...
#if defined(OS_WIN)
void AdjustPageParamsForDpi(PrintHostMsg_DidPrintDocumentParams& page_params,
                            const gfx::Size& dpi) {
  ...
  if (dpi.width() == dpi.height())
    return;
  int dpi = std::min(dpi.width(), dpi.height());
  page_params.page_size =
      gfx::Size(page_params.page_size.width() * dpi.width() / dpi,
                page_params.page_size.height() * dpi.height() / dpi);
  page_params.content_area =
      gfx::Rect(page_params.content_area.x() * dpi.width() / dpi,
                page_params.content_area.y() * dpi.height() / dpi,
                page_params.content_area.width() * dpi.width() / dpi,
                page_params.content_area.height() * dpi.height() / dpi);
