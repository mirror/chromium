...
bool PrintRenderFrameHelper::CreatePreviewDocument() {
  ...
  if (!print_preview_context_.CreatePreviewDocument(
          std::move(prep_frame_view_), pages,
          print_params.requested_doc_type)) {
    return false;
  }
...
bool PrintRenderFrameHelper::RenderPreviewPage(
  ...
  if (print_preview_context_.IsModifiable() && is_print_ready_metafile_sent_) {
    draft_metafile =
        base::MakeUnique<PdfMetafileSkia>(print_params.requested_doc_type);
...
bool PrintRenderFrameHelper::FinalizePrintReadyDocument() {
  ...
  if (!CopyMetafileDataToSharedMem(
          *metafile, &preview_params.metafile_data_handle,
          &preview_params.data_size, &preview_params.printed_doc_type)) {
...
bool PrintRenderFrameHelper::PreviewPageRendered(int page_number,
  ...
  if (!CopyMetafileDataToSharedMem(*metafile,
                                   ...
                                   &preview_page_params.printed_doc_type)) {
    LOG(ERROR) << "CopyMetafileDataToSharedMem failed";
    print_preview_context_.set_error(PREVIEW_ERROR_METAFILE_COPY_FAILED);
    return false;
  }
