...
TEST_F(PdfCompositorImplTest, IsReadyToComposite) {
  ...
  subframe_content_info.push_back(ContentToFrameInfo::New(3u, 2u));
  base::flat_set<uint64_t> pending_subframes;
  bool is_ready = impl.IsReadyToComposite(1u, std::move(subframe_content_info),
                                          &pending_subframes);
  ...
  subframe_content_info.clear();
  subframe_content_info.push_back(ContentToFrameInfo::New(2u, 3u));
  is_ready = impl.IsReadyToComposite(1u, std::move(subframe_content_info),
                                     &pending_subframes);
  EXPECT_TRUE(is_ready);
  EXPECT_TRUE(pending_subframes.empty());
  ...
  subframe_content_info.clear();
  subframe_content_info.push_back(ContentToFrameInfo::New(1u, 4u));
  subframe_content_info.push_back(ContentToFrameInfo::New(2u, 3u));
  subframe_content_info.push_back(ContentToFrameInfo::New(3u, 2u));
  is_ready = impl.IsReadyToComposite(1u, std::move(subframe_content_info),
                                     &pending_subframes);
...
TEST_F(PdfCompositorImplTest, MultiLayerDependency) {
  ...
  std::vector<ContentToFrameInfoPtr> subframe_content_info;
  subframe_content_info.push_back(ContentToFrameInfo::New(1u, 1u));
  impl.AddSubframeContent(3u, mojo::SharedBufferHandle::Create(10),
  ...
  subframe_content_info.clear();
  subframe_content_info.push_back(ContentToFrameInfo::New(3u, 3u));
  base::flat_set<uint64_t> pending_subframes;
  bool is_ready = impl.IsReadyToComposite(5u, std::move(subframe_content_info),
                                          &pending_subframes);
  EXPECT_FALSE(is_ready);
  ASSERT_EQ(pending_subframes.size(), 1u);
  ...
  subframe_content_info.clear();
  subframe_content_info.push_back(ContentToFrameInfo::New(1u, 5u));
  is_ready = impl.IsReadyToComposite(6u, std::move(subframe_content_info),
                                     &pending_subframes);
  EXPECT_FALSE(is_ready);
  ASSERT_EQ(pending_subframes.size(), 1u);
  ...
  subframe_content_info.clear();
  subframe_content_info.push_back(ContentToFrameInfo::New(3u, 3u));
  is_ready = impl.IsReadyToComposite(5u, std::move(subframe_content_info),
                                     &pending_subframes);
  EXPECT_TRUE(is_ready);
  EXPECT_TRUE(pending_subframes.empty());
...
TEST_F(PdfCompositorImplTest, DependencyLoop) {
  ...
  base::flat_set<uint64_t> pending_subframes;
  subframe_content_info.clear();
  subframe_content_info.push_back(ContentToFrameInfo::New(1u, 3u));
  bool is_ready = impl.IsReadyToComposite(5u, std::move(subframe_content_info),
                                          &pending_subframes);
  EXPECT_TRUE(is_ready);
  EXPECT_TRUE(pending_subframes.empty());
  ...
  subframe_content_info.clear();
  subframe_content_info.push_back(ContentToFrameInfo::New(7u, 7u));
  ...
  // depends on frame 7.
  subframe_content_info.clear();
  subframe_content_info.push_back(ContentToFrameInfo::New(6u, 6u));
  is_ready = impl.IsReadyToComposite(7u, std::move(subframe_content_info),
                                     &pending_subframes);
  EXPECT_TRUE(is_ready);
  EXPECT_TRUE(pending_subframes.empty());
}
