 ...
 protected:
  ...
  mojo::ScopedSharedBufferHandle CreateMSKP() {
    ...
    if (shared_memory.Create(options) && shared_memory.Map(len)) {
      ...
      auto handle = base::SharedMemory::DuplicateHandle(shared_memory.handle());
      return mojo::WrapSharedMemoryHandle(handle, handle.GetSize(), true);
    }
  ...
  void CallCompositorWithSuccess(mojom::PdfCompositorPtr ptr) {
    ...
    ptr->CompositeDocumentToPdf(
        kFrameGuid, std::move(handle), std::vector<ContentToFrameInfoPtr>(),
...
TEST_F(PdfCompositorServiceTest, InvokeCallbackOnContentError) {
  ...
  compositor_->CompositeDocumentToPdf(
      5u, mojo::SharedBufferHandle::Create(10),
      std::vector<ContentToFrameInfoPtr>(),
      base::BindOnce(&PdfCompositorServiceTest::OnCompositeToPdfCallback,
                     base::Unretained(this)));
  run_loop_->Run();
}
...
TEST_F(PdfCompositorServiceTest, IndependentServiceInstances) {
  ...
  // to subframe 1.
  compositor_->AddSubframeContent(1u, CreateMSKP(),
                                  std::vector<ContentToFrameInfoPtr>());

  // Original connection can use this subframe 1.
  EXPECT_CALL(*this, CallbackOnCompositeSuccess(testing::_)).Times(1);
  std::vector<ContentToFrameInfoPtr> subframe_content_ids;
  subframe_content_ids.push_back(ContentToFrameInfo::New(2u, 1u));

  compositor_->CompositeDocumentToPdf(
      4u, CreateMSKP(), std::move(subframe_content_ids),
      base::BindOnce(&PdfCompositorServiceTest::OnCompositeToPdfCallback,
                     base::Unretained(this)));
  run_loop_->Run();
  ...
  compositor2->AddSubframeContent(1u, CreateMSKP(),
                                  std::vector<ContentToFrameInfoPtr>());
  run_loop_ = std::make_unique<base::RunLoop>();
  run_loop_->Run();
}
