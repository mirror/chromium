...
class PdfCompositorImpl : public mojom::PdfCompositor {
 public:
  ...
  // mojom::PdfCompositor
  void AddSubframeMap(int frame_id, uint64_t content_id) override;
 ...
 private:
  ...
  struct FrameNodeInfo {
    FrameNodeInfo(int id, int parent_id)
        : frame_id(id), parent_frame_id(parent_id) {}
    int frame_id;
    int parent_frame_id;
  };
  ...
  using RequestId = std::pair<int, uint32_t>;
  ...
  base::flat_map<int, std::vector<uint64_t>> frame_to_content_id_map_;
