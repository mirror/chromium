...
class PdfCompositorImpl : public mojom::PdfCompositor {
 ...
 private:
  ...
  struct FrameInfo {
    ...
    std::set<uint64_t> pending_subframe_content_ids;
  };
  ...
  std::map<uint64_t, std::pair<int, int>> content_to_frame_id_map_;
  // Map to track each frame id with all content id associated with it.
  std::map<int, std::vector<uint64_t>> frame_to_content_id_map_;
