 ...
 protected:
  ...
  PrintRenderFrameHelper* GetPrintRenderFrameHelper(const char* frame_name) {
    return PrintRenderFrameHelper::Get(content::RenderFrame::FromWebFrame(
        GetMainFrame()
            ->FindFrameByName(WebString::FromUTF8(frame_name))
            ->ToWebLocalFrame()));
  }
...
TEST_F(PrintRenderFrameHelperTest, BasicBeforePrintAfterPrint) {
  const char kHtml[] =
      "<body>Hello"
      "<script>"
...
TEST_F(PrintRenderFrameHelperTest, BasicBeforePrintAfterPrintSubFrame) {
