...
IPC_STRUCT_TRAITS_BEGIN(PrintMsg_PrintFrame_Params)
  ...
  // Cookie who is unique for a document with its setting. Together with the
  // following |page_number|, it is used to associate this painted frame with
  // its original print request.
  IPC_STRUCT_TRAITS_MEMBER(document_cookie)
...
IPC_MESSAGE_ROUTED3(PrintHostMsg_DidPrintFrameContent,
                    int /* rendered document cookie */,
