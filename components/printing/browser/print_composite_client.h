...
class PrintCompositeClient
 ...
 public:
  ...
  void PrintSubframe(const gfx::Rect& rect,
                     uint64_t content_id,
                     content::RenderFrameHost* dst_host) override;
 ...
 private:
  ...
  // Each frame is identied by its render process id and frame id.
  base::flat_set<std::pair<int, int>> pending_subframes_;
