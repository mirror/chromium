...
void PrintCompositeClient::PrintRemoteFrame(content::RenderFrameHost* dst_host,
                                            const gfx::Rect& rect,
                                            uint32_t uid) {
  ...
  // Store the mapping between uid and subframe id.
  AddSubframeMap(connector_.get(), dst_frame_id, uid);
  ...
  dst_host->Send(new PrintMsg_PrintFrameContent(dst_frame_id, rect, uid));
}
