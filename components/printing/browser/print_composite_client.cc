...
namespace {
std::vector<uint64_t> ConvertToUniqueIds(
    int frame_id,
    const std::vector<uint32_t>& content_ids) {
  std::vector<uint64_t> uids;
  for (auto id : content_ids)
    uids.push_back(OopPicUniqueId(frame_id, id));
  return uids;
}
}  // namespace
...
void PrintCompositeClient::PrintSubframe(int src_frame_id,
                                         const gfx::Rect& rect,
                                         uint32_t content_id,
  ...
  AddSubframeMap(connector_.get(), dst_frame_id,
                 OopPicUniqueId(src_frame_id, content_id));

  // Check whether this subframe has been requested.
  auto result = pending_subframe_ids_.insert(dst_frame_id);
  if (!result.second)
    return;
...
void PrintCompositeClient::OnDidPrintFrameContent(
  ...
  AddSubframeContent(connector_.get(), frame_id, params.metafile_data_handle,
                     params.data_size,
                     ConvertToUniqueIds(frame_id, params.subframe_content_ids));
}
...
void PrintCompositeClient::DoCompositeToPdf(
  ...
  std::vector<uint64_t> content_ids =
      ConvertToUniqueIds(frame_id, subframe_content_ids);
  // Check whether it is ok to composite, if so, composite to PDF.
  DoCompositeToPdfImpl(frame_id, page_num, handle, data_size, content_ids,
                       std::move(callback));
}
