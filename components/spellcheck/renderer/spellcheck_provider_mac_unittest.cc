...
TEST_F(SpellCheckProviderMacTest, SingleRoundtripSuccess) {
  ...
  RequestTextCheckCallback& callback =
      provider_.text_check_requests.back().second;
  EXPECT_EQ(text, base::ASCIIToUTF16("hello "));

  std::vector<base::string16> fake_results;
...
TEST_F(SpellCheckProviderMacTest, TwoRoundtripSuccess) {
  ...
  RequestTextCheckCallback& callback1 = provider_.text_check_requests[0].second;
  EXPECT_EQ(text1, base::ASCIIToUTF16("hello "));

  const base::string16& text2 = provider_.text_check_requests_[1].first;
  RequestTextCheckCallback& callback2 = provider_.text_check_requests[1].second;
  EXPECT_EQ(text1, base::ASCIIToUTF16("bye "));
