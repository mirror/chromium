...
spellcheck::mojom::SpellCheckHost& PlatformSpellingEngine::GetSpellCheckHost() {
  if (spell_check_host_)
    return *spell_check_host_;

  // |embedder_provider_| is nullptr in unit tests.
  DCHECK(embedder_provider_);
...
bool PlatformSpellingEngine::CheckSpelling(const base::string16& word_to_check,
  ...
  GetSpellCheckHost().CheckSpelling(word_to_check, tag, &word_correct);
  return word_correct;
}
...
void PlatformSpellingEngine::FillSuggestionList(
  ...
  GetSpellCheckHost().FillSuggestionList(wrong_word, optional_suggestions);
}
