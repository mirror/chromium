...
void NetMetricsLogUploader::UploadLogToInsecureURL(
    const std::string& compressed_log_data,
    const std::string& log_hash,
    const ReportingInfo& reporting_info) {
  if (!insecure_server_url_.is_empty()) {
    UploadLogToURL(compressed_log_data, log_hash, reporting_info,
                   insecure_server_url_);
  }
}

std::string NetMetricsLogUploader::GetInsecureUploadURL() {
  return insecure_server_url_.GetContent();
}
