...
void ReportingService::OnLogUploadComplete(int response_code,
  ...
  try_next_upload_over_http_ =
      was_https && error_code != 0 &&
      !log_uploader_->GetInsecureUploadURL().empty() &&
      base::CommandLine::ForCurrentProcess()->HasSwitch("retry-uma-over-http");
