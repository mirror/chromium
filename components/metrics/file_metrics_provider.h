...
class FileMetricsProvider : public MetricsProvider,
 ...
 public:
  ...
  using FilterCallback =
      base::RepeatingCallback<FilterAction(const Params& params)>;

  // Parameters for RegisterSource, defined as as a structure to allow new
  // ones to be added (with default values) that doesn't require changes
  // to all call sites.
  struct Params {
    Params(base::FilePath&& path,
           SourceType type,
    ...
    Params(const base::FilePath& path,
           ...
           SourceAssociation association,
           base::StringPiece prefs_key)
        : Params(base::FilePath(path), type, association, prefs_key) {}

    Params(base::FilePath&& path,
           SourceType type,
           SourceAssociation association)
        : Params(std::move(path), type, association, base::StringPiece()) {}

    Params(const base::FilePath& path,
           SourceType type,
           SourceAssociation association)
        : Params(base::FilePath(path), type, association, base::StringPiece()) {
    }
 ...
 private:
  ...
  enum AccessResult {
    ...
    // File is schedule to be tried again later.
