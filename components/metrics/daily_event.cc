...
void DailyEvent::RegisterPref(PrefRegistrySimple* registry,
                              const char* pref_name) {
  registry->RegisterInt64Pref(pref_name,
                              base::Time().since_origin().InMilliseconds());
}
...
void DailyEvent::CheckInterval() {
  ...
  if (last_fired_.is_null()) {
    ...
    last_fired_ = base::Time() + base::TimeDelta::FromMilliseconds(
...
void DailyEvent::OnInterval(base::Time now) {
  ...
  pref_service_->SetInt64(pref_name_,
                          last_fired_.since_origin().InMilliseconds());
