...
void MetricsService::ClearSavedStabilityMetrics() {
  metrics_providers_.ClearSavedStabilityMetrics();
}
...
void MetricsService::InitializeMetricsState() {
  ...
  const bool is_initial_stability_log_required =
      metrics_providers_.HasPreviousSessionData() ||
...
void MetricsService::FinishedInitTask() {
  ...
  if (!initial_metrics_log_.get()) {
    ...
    metrics_providers_.OnDidCreateMetricsLog();
  }
...
void MetricsService::StartInitTask() {
  metrics_providers_.AsyncInit(base::Bind(&MetricsService::FinishedInitTask,
                                          self_ptr_factory_.GetWeakPtr()));
}
...
void MetricsService::RecordCurrentHistograms() {
  ...
  metrics_providers_.RecordHistogramSnapshots(&histogram_snapshot_manager_);
}
...
void MetricsService::RecordCurrentStabilityHistograms() {
  ...
  metrics_providers_.RecordInitialHistogramSnapshots(
      &histogram_snapshot_manager_);
}
...
bool MetricsService::PrepareProviderMetricsLog() {
  ...
  for (auto& provider : metrics_providers_.GetProviders()) {
