...
const int kMaxReusedDomains = 1000;
...
PasswordProtectionRequest::PasswordProtectionRequest(
  ...
  if (trigger_type_ == LoginReputationClientRequest::PASSWORD_REUSE_EVENT) {
    DCHECK(matches_sync_password_ || matching_domains_.size() > 0);
  }
}
  ...
  switch (trigger_type_) {
    ...
    case LoginReputationClientRequest::PASSWORD_REUSE_EVENT: {
      ...
      if (password_protection_service_->IsExtendedReporting()) {
        int reused_domains_count = 0;
        for (const auto& domain : matching_domains_) {
          reuse_event->domains_matching_password(domain);
          if (++reused_domanins_count >= kMaxReusedDomains)
            break;
        }
      }
      break;
