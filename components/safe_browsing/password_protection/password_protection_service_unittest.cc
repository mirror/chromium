...
TEST_P(PasswordProtectionServiceTest, VerifyIsEventLoggingEnabled) {
...

  {
    ...
    // Event logging should be enable for all Gmail users.
    password_protection_service_->set_sync_account_type(
        LoginReputationClientRequest::PasswordReuseEvent::GMAIL);
    ...
    // For GSuite users, event logging should be disabled if the password
    ...
    // For GSuite users, event logging should be enabled if the password
