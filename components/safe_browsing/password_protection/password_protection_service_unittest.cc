...
TEST_P(PasswordProtectionServiceTest, VerifyShouldShowModalWarning) {
...

  {
    ...
    password_protection_service_->set_password_protection_trigger(
        PHISHING_REUSE);
    EXPECT_EQ(PHISHING_REUSE, password_protection_service_
                                  ->GetPasswordProtectionWarningTriggerPref());
...
TEST_P(PasswordProtectionServiceTest, VerifyIsEventLoggingEnabled) {
...

  {
    ...
    // For regular Gmail user, event logging should be enable.
    ...
    // For GSuite user, event logging should be disabled if the password
    // protection trigger set to PASSWORD_PROTECTION_OFF.
    password_protection_service_->set_sync_account_type(
        LoginReputationClientRequest::PasswordReuseEvent::GSUITE);
    ...
    EXPECT_EQ(
        PASSWORD_PROTECTION_OFF,
        password_protection_service_->GetPasswordProtectionRiskTriggerPref());
    EXPECT_FALSE(password_protection_service_->IsEventLoggingEnabled());

    // For GSuite user, event logging should be enabled if the password
    // protection trigger set to PHISHING_REUSE.
    password_protection_service_->set_password_protection_trigger(
        PHISHING_REUSE);
    EXPECT_EQ(
        PHISHING_REUSE,
