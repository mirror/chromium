...
BaseParallelResourceThrottle::~BaseParallelResourceThrottle() {
  if (url_loader_throttle_holder_->inside_delegate_calls() > 0) {
    // The throttle is calling into this object. In this case, delay destruction
    // of the throttle, so that it doesn't need to worry about any delegate
    // calls may destory it synchronously.
