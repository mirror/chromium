...
class TestResourceThrottleDelegate
 ...
 public:
  size_t cancel_called() const { return cancel_called_; }
  size_t resume_called() const { return resume_called_; }
 ...
 private:
  size_t cancel_called_ = 0;
  size_t resume_called_ = 0;
};
...
class TestDataBaseManager : public TestSafeBrowsingDatabaseManager {
 public:
  ...
  size_t check_browse_url_called() const { return check_browse_url_called_; }

  void DelayCheckBrowseUrlResult(size_t num_calls) {
 ...
 private:
  ...
  size_t check_browse_url_called_ = 0;
  Client* last_client_ = nullptr;
  GURL last_url_;
  size_t delay_check_browse_url_calls_ = 0;
};
...
TEST_F(BaseParallelResourceThrottleTest, Resume) {
  ...
  EXPECT_FALSE(defer);
  EXPECT_EQ(1u, database_manager_->check_browse_url_called());
  ...
  EXPECT_EQ(0u, resource_throttle_delegate_.resume_called());

  database_manager_->CompleteNextCheckBrowseUrl(true);
  EXPECT_EQ(2u, database_manager_->check_browse_url_called());
  database_manager_->CompleteNextCheckBrowseUrl(true);

  EXPECT_EQ(1u, resource_throttle_delegate_.resume_called());
}
...
TEST_F(BaseParallelResourceThrottleTest, CancelWhenDeferred) {
  ...
  EXPECT_EQ(0u, resource_throttle_delegate_.cancel_called());

  database_manager_->CompleteNextCheckBrowseUrl(false);

  EXPECT_EQ(1u, resource_throttle_delegate_.cancel_called());
}
