...
void BrowserURLLoaderThrottle::OnCompleteCheck(bool slow_check,
  ...
  if (proceed) {
    if (pending_slow_checks_ == 0 && slow_check)
      delegate_->ResumeCachingResponseBody();
...
void BrowserURLLoaderThrottle::OnCheckUrlResult(
  ...
  // pause caching response body to minimize the chance that we write unsafe
  // contents into cache, until we get the results. According to the results, we
  // may resume caching or cancel the resource load.
  if (pending_slow_checks_ == 1)
    delegate_->PauseCachingResponseBody();
