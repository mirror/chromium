...
bool TriggerThrottler::TriggerCanFire(const TriggerType trigger_type) const {
  ...
  LOG(ERROR) << "Lpz: ts vec contents:";
  for (auto t : timestamps) {
    LOG(ERROR) << t;
  }
  base::Time min_timestamp = clock_->Now() - kOneDayTimeDelta;
  LOG(ERROR) << "Lpz: now=" << clock_->Now().ToTimeT()
             << " min ts=" << min_timestamp.ToTimeT();
  LOG(ERROR) << "Lpz: vec size = " << timestamps.size()
             << " trig_quota=" << trigger_quota;
  const size_t pos = timestamps.size() - trigger_quota + 1;
  LOG(ERROR) << "Lpz: pos=" << pos << " ts[pos]=" << timestamps[pos];
