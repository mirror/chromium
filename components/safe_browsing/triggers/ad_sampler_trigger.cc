...
void AdSamplerTrigger::DidFinishNavigation(
  ...
  LOG(ERROR) << "Lpz: navigation finished, url="
             << navigation_handle->GetURL().spec()
             << " nid=" << navigation_handle->GetNavigationId();
  ...
  if (ShouldCheckForAd(sampler_frequency_denominator_) &&
    ...
    content::BrowserThread::GetTaskRunnerForThread(content::BrowserThread::UI)
        ->PostTask(FROM_HERE,
                   base::BindOnce(&AdSamplerTrigger::CollectAdDataOnUIThread,
                                  base::Unretained(this)));
  }
  LOG(ERROR) << "Lpz: done with nid=" << navigation_handle->GetNavigationId();
}

void AdSamplerTrigger::CollectAdDataOnUIThread() {
