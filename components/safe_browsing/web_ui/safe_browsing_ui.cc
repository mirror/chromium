...
void SafeBrowsingUIHandler::AddStoreInfo(
  ...
  databaseInfoList->GetList().push_back(
      base::Value(static_cast<double>(store_info->file_size())));
  // status for each database
  databaseInfoList->GetList().push_back(base::Value("Update merged status"));
  databaseInfoList->GetList().push_back(
...
void SafeBrowsingUIHandler::AddDatabaseInfo(
  ...
  databaseInfoList->GetList().push_back(
      base::Value(static_cast<double>(database_info->database_size())));
...
void SafeBrowsingUIHandler::AddUpdateInfo(
  ...
  if (update_info->network_status_code()) {
    ...
    databaseInfoList->GetList().push_back(base::Value(
        UTF16ToASCII(TimeFormatShortDateAndTime(base::Time::FromJavaTime(
