...
WebUIInfoSingleton::reports_sent() const {
...
void WebUIInfoSingleton::AddToReportsSent(
  ...
  for (auto* webui_listener : webui_instances_)
    webui_listener->GetThreatDetailsUpdate(report_request.get());
...
const std::vector<SafeBrowsingUIHandler*>& WebUIInfoSingleton::webui_instances()
    const {
...
void SafeBrowsingUIHandler::GetSentThreatDetails(const base::ListValue* args) {
  const std::vector<std::unique_ptr<ClientSafeBrowsingReportRequest>>&
      threat_details_list = WebUIInfoSingleton::GetInstance()->reports_sent();
