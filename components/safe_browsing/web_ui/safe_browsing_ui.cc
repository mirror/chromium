...
void ParseFullHashInfo(
  ...
  if (full_hash_info.has_full_hash()) {
    std::string* full_hash = nullptr;
    base::Base64Encode(full_hash_info.full_hash(), full_hash);
...
void ParseFullHashCache(const FullHashCacheInfo::FullHashCache full_hash_cache,
  ...
  if (full_hash_cache.has_hash_prefix()) {
    std::string* hash_prefix = nullptr;
    base::Base64Encode(full_hash_cache.hash_prefix(), hash_prefix);
