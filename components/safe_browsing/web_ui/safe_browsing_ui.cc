...
std::vector<ClientSafeBrowsingReportRequest>
    SafeBrowsingUIHandler::threat_details_;
// static
int SafeBrowsingUIHandler::webui_counter_ = 0;
// static
void SafeBrowsingUIHandler::AddSerializedThreatDetail(
    ClientSafeBrowsingReportRequest threat_detail) {
  if (webui_counter_)
    threat_details_.push_back(threat_detail);
}
...
SafeBrowsingUIHandler::~SafeBrowsingUIHandler() {
  webui_counter_--;
  if (!webui_counter_) {
    std::vector<ClientSafeBrowsingReportRequest>().swap(threat_details_);
  }
}
...
void SafeBrowsingUIHandler::GetThreatDetails(const base::ListValue* args) {
  AllowJavascript();
  for (unsigned int i = 0; i < threat_details_.size(); i++) {
    CallJavascriptFunction(
        "safe_browsing.addThreatDetailsInfo",
        base::Value(ParseThreatDetailsInfo(threat_details_[i])));
  }
}
