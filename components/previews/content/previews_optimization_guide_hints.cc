...
PreviewsOptimizationGuideHints::Hints::Hints() {}

PreviewsOptimizationGuideHints::Hints::~Hints() {}

// static
std::unique_ptr<PreviewsOptimizationGuideHints::Hints>
PreviewsOptimizationGuideHints::Hints::CreateFromConfig(
...
bool PreviewsOptimizationGuideHints::Hints::IsWhitelisted(const GURL& url,
                                                          PreviewsType type) {
...
PreviewsOptimizationGuideHints::PreviewsOptimizationGuideHints(
  ...
  // TODO(crbug.com/777892): Add |this| as an observer of
  // OptimizationGuideService.
}

PreviewsOptimizationGuideHints::~PreviewsOptimizationGuideHints() {
  // TODO(crbug.com/777892): Remove |this| as an observer of
  // optimization_guide_service.
}

bool PreviewsOptimizationGuideHints::IsWhitelisted(
    const net::URLRequest& request,
    PreviewsType type) const {
  return hints_->IsWhitelisted(request.url(), type);
}

void PreviewsOptimizationGuideHints::OnHintsProcessed(
    const optimization_guide::proto::Configuration& config) {
  DCHECK(io_task_runner_->BelongsToCurrentThread());

  base::PostTaskAndReplyWithResult(
      background_task_runner_.get(), FROM_HERE,
      base::BindOnce(&PreviewsOptimizationGuideHints::Hints::CreateFromConfig,
                     config),
      base::BindOnce(&PreviewsOptimizationGuideHints::UpdateHints,
                     io_weak_ptr_factory_.GetWeakPtr()));
}

void PreviewsOptimizationGuideHints::UpdateHints(std::unique_ptr<Hints> hints) {
  DCHECK(io_task_runner_->BelongsToCurrentThread());
