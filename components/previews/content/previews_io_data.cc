...
void PreviewsIOData::LogPreviewDecisionMade(
    ...
    base::Time time,
    PreviewsType type,
    std::unique_ptr<PassedPreviewsEligibilities> passed_reasons) const {
...
bool PreviewsIOData::ShouldAllowPreviewAtECT(
  ...
  std::unique_ptr<PassedPreviewsEligibilities> passed_reasons =
      base::MakeUnique<PassedPreviewsEligibilities>();
  ...
  if (!blacklist_ignored_) {
    ...
    PreviewsEligibilityReason status = previews_black_list_->IsLoadedAndAllowed(
        request.url(), type, passed_reasons.get());
