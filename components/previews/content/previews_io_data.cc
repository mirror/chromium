...
bool PreviewsIOData::ShouldAllowPreviewAtECT(
  ...
  LogPreviewDecisionMade(PreviewsEligibilityReason::HOST_BLACKLISTED_BY_SERVER,
                         request.url(), base::Time::Now(), type,
                         !host_blacklisted_by_user);
  if (host_blacklisted_by_user)
    return false;
...
void PreviewsIOData::LogBlackListDecision(PreviewsEligibilityReason reason,
                                          ...
                                          base::Time time,
                                          PreviewsType type) const {
  // The ordered of decision checked in PreviewBlackList.
  PreviewsEligibilityReason reasons[] = {
  ...
  for (auto current : reasons) {
    if (current == reason) {
      LogPreviewDecisionMade(current, url, time, type, false /* checked */);
      return;
    }
    LogPreviewDecisionMade(current, url, time, type, true /* checked */);
  }
}
