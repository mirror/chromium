...
class TestPreviewsBlackList : public PreviewsBlackList {
 public:
  ...
  PreviewsEligibilityReason IsLoadedAndAllowed(
      const GURL& url,
      PreviewsType type,
      PassedPreviewsEligibilities* passed_reasons) const override {
...
class TestPreviewsUIService : public PreviewsUIService {
 public:
  ...
  const std::vector<std::unique_ptr<PassedPreviewsEligibilities>>&
 ...
 private:
  ...
  std::vector<std::unique_ptr<PassedPreviewsEligibilities>>
...
TEST_F(PreviewsIODataTest, LogPreviewDecisionMadePassInCorrectParams) {
  ...
  PassedPreviewsEligibilities* expected_passed_reasons = passed_reasons.get();
  ...
  EXPECT_EQ(expected_passed_reasons,
            ui_service()->decision_passed_reasons()[0].get());
}  // namespace
...
TEST_F(PreviewsIODataTest, LogDecisionMadeNetworkQualityNotAvailable) {
  ...
  PassedPreviewsEligibilities checked_decisions = {
      PreviewsEligibilityReason::BLACKLIST_UNAVAILABLE,
      PreviewsEligibilityReason::BLACKLIST_DATA_NOT_LOADED,
      PreviewsEligibilityReason::USER_RECENTLY_OPTED_OUT,
      PreviewsEligibilityReason::USER_BLACKLISTED,
  ...
  EXPECT_EQ(1UL, ui_service()->decision_passed_reasons().size());
  EXPECT_EQ(checked_decisions.size(),
            ui_service()->decision_passed_reasons()[0]->size());
}
