...
class TestPreviewsUIService : public PreviewsUIService {
 public:
  ...
  const std::vector<bool>& decision_reversed() const {
    return decision_reversed_;
  }

  // Count number of reversed decisions.
  size_t CountReversedDecisions() const {
    size_t total = 0;
    for (bool decision : decision_reversed_) {
      total += decision ? 1 : 0;
    }
    return total;
  }
 ...
 private:
  ...
  void LogPreviewDecisionMade(PreviewsEligibilityReason reason,
    ...
    decision_reversed_.push_back(reversed);
  }
  ...
  std::vector<bool> decision_reversed_;
...
TEST_F(PreviewsIODataTest, LogPreviewDecisionMadePassInCorrectParams) {
  ...
  io_data()->LogPreviewDecisionMade(reason, url, time, type, reversed);
  base::RunLoop().RunUntilIdle();
  ...
  EXPECT_THAT(ui_service()->decision_reversed(),
              ::testing::ElementsAre(reversed));
}
...
TEST_F(PreviewsIODataTest, LogDecisionMadeNetworkQualityNotAvailable) {
  ...
  EXPECT_THAT(ui_service()->decision_types(),
              ::testing::Contains(expected_type));
  EXPECT_EQ(expected_total_checked, ui_service()->CountReversedDecisions());
