...
class TestPreviewsLogger : public PreviewsLogger {
 public:
  TestPreviewsLogger()
      : decision_passed_reasons_(nullptr),
  ...
  void LogPreviewDecisionMade(
      ...
      base::Time time,
      PreviewsType type,
      std::unique_ptr<PassedPreviewsEligibilities> passed_reasons) override {
  ...
  PassedPreviewsEligibilities* decision_passed_reasons() const {
    return decision_passed_reasons_.get();
  }
 ...
 private:
  ...
  std::unique_ptr<PassedPreviewsEligibilities> decision_passed_reasons_;
...
TEST_F(PreviewsUIServiceTest, TestLogPreviewDecisionMadePassesCorrectParams) {
  ...
  std::unique_ptr<PassedPreviewsEligibilities> passed_reasons_a =
      base::MakeUnique<PassedPreviewsEligibilities>();
  passed_reasons_a->push_back(PreviewsEligibilityReason::NETWORK_NOT_SLOW);
  passed_reasons_a->push_back(
      PreviewsEligibilityReason::USER_RECENTLY_OPTED_OUT);
  passed_reasons_a->push_back(PreviewsEligibilityReason::RELOAD_DISALLOWED);
  PassedPreviewsEligibilities* expected_passed_reasons_a =
      passed_reasons_a.get();
  ...
  std::unique_ptr<PassedPreviewsEligibilities> passed_reasons_b =
      base::MakeUnique<PassedPreviewsEligibilities>();
  passed_reasons_b->push_back(
      PreviewsEligibilityReason::HOST_NOT_WHITELISTED_BY_SERVER);
  passed_reasons_b->push_back(
      PreviewsEligibilityReason::NETWORK_QUALITY_UNAVAILABLE);
  PassedPreviewsEligibilities* expected_passed_reasons_b =
      passed_reasons_b.get();
  ...
  EXPECT_EQ(expected_passed_reasons_b, logger_ptr_->decision_passed_reasons());
}
