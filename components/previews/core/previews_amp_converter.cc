...
// TODO(rajendrant): Remove this field trial based enabling and use the
// base::Feature based config retrieval.
...
PreviewsAMPConverter::PreviewsAMPConverter() {
  ...
  for (const auto& entry : *entries) {
    ...
    MatchingScheme matching_scheme = matching_scheme_str == "http"
                                         ? MatchingScheme::HTTP
                                         : matching_scheme_str == "https"
                                               ? MatchingScheme::HTTPS
                                               : MatchingScheme::BOTH;

    // If matching scheme is HTTPS or BOTH, then AMP scheme should not be http,
    // which will redirect https to http.
    DCHECK(matching_scheme == MatchingScheme::HTTP || scheme_amp != "http");
...
bool PreviewsAMPConverter::GetAMPURL(const GURL& url, GURL* new_amp_url) const {
  ...
  if (!entry.scheme_amp.empty()) {
    ...
    if (url.scheme() == "https" && entry.scheme_amp == "http")
      return false;
