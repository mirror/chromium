...
PreviewsAMPConverter::PreviewsAMPConverter() {
  ...
  LOG(WARNING) << "config " << config_text;
  ...
  for (const auto& entry : *entries) {
    ...
    dict->GetString("suffix_html", &suffix_html);
    std::unique_ptr<re2::RE2> pattern_re2(new re2::RE2(pattern, options));
    if (host.empty() || !pattern_re2->ok())
      continue;
    amp_converter_.insert(std::make_pair(
        host, base::MakeUnique<AMPConverterEntry>(host, std::move(pattern_re2),
                                                  host_amp, prefix, suffix,
                                                  suffix_html)));
  }
}
