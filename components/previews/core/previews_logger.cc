...
std::vector<PreviewsLogger::MessageLog> PreviewsLogger::decisions_logs() const {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  return std::vector<MessageLog>(decisions_logs_.begin(),
                                 decisions_logs_.end());
}

std::vector<PreviewsLogger::MessageLog> PreviewsLogger::navigations_logs()
    const {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  return std::vector<MessageLog>(navigations_logs_.begin(),
                                 navigations_logs_.end());
}
