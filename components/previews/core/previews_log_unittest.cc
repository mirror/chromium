...
TEST_F(PreviewsLogTest, AddPreviewNavigationLogMessage) {
  ...
  const PreviewsType type_a = PreviewsType::OFFLINE;
  const PreviewsType type_b = PreviewsType::LOFI;
  const GURL url_a("http://www.url_a.com/url_a");
  const GURL url_b("http://www.url_b.com/url_b");
  ...
  logger->AddPreviewNavigation(navigation_a);
  logger->AddPreviewNavigation(navigation_b);
