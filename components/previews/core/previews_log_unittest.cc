...
class PreviewsLoggerTest : public testing::Test {
 public:
  ...
  void SetUp() override { logger = base::MakeUnique<PreviewsLogger>(); }
...
TEST_F(PreviewsLoggerTest, AddLogMessageToLogger) {
  ...
  const MessageLog expected_messages[] = {
      MessageLog("Event_a", "Some description a",
                 GURL("http://www.url_a.com/url_a"), base::Time::Now()),
      MessageLog("Event_b", "Some description b",
                 GURL("http://www.url_b.com/url_b"), base::Time::Now()),
      MessageLog("Event_c", "Some description c",
                 GURL("http://www.url_c.com/url_c"), base::Time::Now()),
  };

  for (auto message : expected_messages) {
    logger->LogMessage(message.event_type, message.event_description,
...
TEST_F(PreviewsLoggerTest, LogPreviewNavigationLogMessage) {
  ...
  PreviewNavigation navigation_a(url_a, type_a, true /* opt_out */, time);
  PreviewNavigation navigation_b(url_b, type_b, false /* opt_out */, time);
