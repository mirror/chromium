...
const size_t kMaximumDecisionLogs = 10;
...
TEST_F(PreviewsLoggerTest, PreviewsLoggerSaveCorrectTypesHistoricalLogs) {
  const std::string event_types[] = {"Event_0", "Event_1", "Event_2"};
  ...
  EXPECT_EQ(expected_size, logger_->navigations_logs().size());
  EXPECT_EQ(expected_size, logger_->decisions_logs().size());
}
...
TEST_F(PreviewsLoggerTest, LogPreviewsDecisionMadeLogMessage) {
  ...
  auto actual = logger_->decisions_logs();
...
TEST_F(PreviewsLoggerTest, LogPreviewNavigationLogMessage) {
  ...
  auto actual = logger_->navigations_logs();
...
TEST_F(PreviewsLoggerTest, PreviewsLoggerOnlyKeepsCertainNumberOfDecisionLogs) {
  ...
  EXPECT_EQ(kMaximumDecisionLogs, logger_->decisions_logs().size());
}
...
TEST_F(PreviewsLoggerTest,
  ...
  EXPECT_EQ(kMaximumNavigationLogs, logger_->navigations_logs().size());
}
...
TEST_F(PreviewsLoggerTest, ObserverIsNotifiedOfHistoricalNavigationsWhenAdded) {
  ...
  logger_->LogPreviewsDecisionMade(reason, urls[0], times[0], type);
  logger_->LogPreviewNavigation(urls[1], type, true /* opt_out */, times[1]);
  logger_->LogPreviewsDecisionMade(reason, urls[2], times[2], type);
