...
std::unique_ptr<PreviewsBlackListItem> CreateBlacklistItemForHost() {
...
// Mock class to test that PreviewsBlackList notifies the observer with correct
...
class PreviewsBlackListTest : public testing::Test {
 public:
  ...
  void StartTest(bool null_opt_out) {
    ...
    black_list_ = base::MakeUnique<PreviewsBlackList>(
        std::move(opt_out_store), std::move(test_clock), &observer_);
 ...
 protected:
  ...
  TestPreviewsBlacklistObserver observer_;
...
TEST_F(PreviewsBlackListTest, ObserverIsNotifiedOnHostBlacklisted) {
  ...
  base::RunLoop().RunUntilIdle();
  EXPECT_THAT(observer_.blacklisted_hosts(), ::testing::SizeIs(0));
  ...
  EXPECT_THAT(observer_.blacklisted_hosts(), ::testing::SizeIs(1));
  EXPECT_EQ(blacklisted_time,
            observer_.blacklisted_hosts().find(url_.host())->second);
  ...
  EXPECT_FALSE(observer_.blacklist_cleared());

  test_clock_->Advance(base::TimeDelta::FromSeconds(1));
  black_list_->ClearBlackList(start_, test_clock_->Now());
  base::RunLoop().RunUntilIdle();

  EXPECT_TRUE(observer_.blacklist_cleared());
  EXPECT_EQ(test_clock_->Now(), observer_.blacklist_cleared_time());
}
...
TEST_F(PreviewsBlackListTest, ObserverIsNotifiedOnUserBlacklisted) {
  ...
  EXPECT_THAT(observer_.blacklisted_hosts(), ::testing::SizeIs(0));
  EXPECT_FALSE(observer_.user_blacklisted());
  ...
  for (size_t i = 0; i < host_indifferent_threshold; ++i) {
    ...
    if (i < host_indifferent_threshold - 1) {
      ...
      EXPECT_TRUE(observer_.user_blacklisted());
    }
  }
...
TEST_F(PreviewsBlackListTest, ObserverIsNotifiedOfHistoricalBlacklistedHosts) {
  ...
  std::unique_ptr<PreviewsBlackListItem> item_a = CreateBlacklistItemForHost();
  std::unique_ptr<PreviewsBlackListItem> item_b = CreateBlacklistItemForHost();
  ...
  item_map->insert({url_a.host(), std::move(item_a)});
  item_map->insert({url_b.host(), std::move(item_b)});
  ...
  ASSERT_THAT(observer_.blacklisted_hosts(), ::testing::SizeIs(1));
  EXPECT_EQ(blacklisted_time,
            observer_.blacklisted_hosts().find(url_a.host())->second);
}
