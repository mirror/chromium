...
class TestPreviewsBlacklistObserver : public PreviewsBlacklistObserver {
 public:
  TestPreviewsBlacklistObserver()
      : user_blacklisted_(false),
        user_blacklisted_time_(base::Time::Now()),
        user_not_blacklisted_time_(base::Time::Now()),
  ...
  void OnNewBlacklistedHost(const std::string& host, base::Time time) override {
    blacklisted_hosts_[host] =
        (blacklisted_hosts_.find(host) != blacklisted_hosts_.end())
            ? std::max(blacklisted_hosts_[host], time)
            : time;
  }
  void OnUserBlacklisted(base::Time time) override {
    user_blacklisted_ = true;
  ...
  // Gets the event time of user blacklisted.
  base::Time user_blacklisted_time() const { return user_blacklisted_time_; }

  // Gets the event time of user not blacklisted.
 ...
 private:
  ...
  // The time when user is blacklisted.
  base::Time user_blacklisted_time_;

  // The time when user is not blacklisted.
...
TEST_F(PreviewsBlackListTest, ObserversAreNotifiedOnUserBlacklisted) {
  ...
  for (size_t i = 0; i < host_indifferent_threshold; ++i) {
    ...
    for (size_t j = 0; j < number_of_obs; ++j) {
      ...
      if (i < host_indifferent_threshold - 1) {
        ...
        EXPECT_EQ(test_clock_->Now(), observers[j].user_blacklisted_time());
      }
    }
  }
  ...
  for (size_t i = 0; i < number_of_obs; i++) {
    EXPECT_FALSE(observers[i].user_blacklisted());
    EXPECT_EQ(test_clock_->Now(), observers[i].user_not_blacklisted_time());
  }
}
