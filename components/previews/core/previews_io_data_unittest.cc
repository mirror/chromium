...
TEST_F(PreviewsIODataTest, LogPreviewNavigationPassInCorrectParams) {
  ...
  EXPECT_EQ(expected_size, ui_service()->navigation_urls().size());
  EXPECT_EQ(url, ui_service()->navigation_urls()[0]);

  EXPECT_EQ(expected_size, ui_service()->navigation_opt_outs().size());
  EXPECT_EQ(opt_out, ui_service()->navigation_opt_outs()[0]);

  EXPECT_EQ(expected_size, ui_service()->navigation_types().size());
  EXPECT_EQ(type, ui_service()->navigation_types()[0]);

  EXPECT_EQ(expected_size, ui_service()->navigation_times().size());
  EXPECT_EQ(time, ui_service()->navigation_times()[0]);
}
...
TEST_F(PreviewsIODataTest, LogPreviewsDecisionMadePassInCorrectParams) {
  ...
  const size_t expected_size = 1;
  EXPECT_EQ(expected_size, ui_service()->decision_reasons().size());
  EXPECT_EQ(reason, ui_service()->decision_reasons()[0]);

  EXPECT_EQ(expected_size, ui_service()->decision_urls().size());
  EXPECT_EQ(url, ui_service()->decision_urls()[0]);

  EXPECT_EQ(expected_size, ui_service()->decision_types().size());
  EXPECT_EQ(type, ui_service()->decision_types()[0]);

  EXPECT_EQ(expected_size, ui_service()->decision_times().size());
  EXPECT_EQ(time, ui_service()->decision_times()[0]);
}
