...
// Mock class of PreviewsLog for checking passed in parameters.
class TestPreviewsLog : public PreviewsLog {
 public:
  TestPreviewsLog() {}

  void LogPreviewNavigation(const PreviewNavigation& navigation) override {
    navigation_ = base::MakeUnique<PreviewNavigation>(navigation);
  }

  // Return the passed in navigation.
  PreviewNavigation navigation() const { return *navigation_; }

 private:
  std::unique_ptr<PreviewNavigation> navigation_;
};
...
class PreviewsUIServiceTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    std::unique_ptr<TestPreviewsLog> logger =
        base::MakeUnique<TestPreviewsLog>();
...
TEST_F(PreviewsUIServiceTest, TestLogPreviewNavigationPassInCorrectParams) {
  ...
  PreviewNavigation actual_a = logger_ptr->navigation();
  ...
  PreviewNavigation actual_b = logger_ptr->navigation();
