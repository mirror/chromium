...
class PreviewsBlacklistObserver {
 public:
  // Notifies |this| that |host| has been blacklisted at |time|.
  virtual void OnNewBlacklistedHost(const std::string& host,
                                    base::Time time) = 0;

  // Notifies |this| that the user is blacklisted at |time|.
  virtual void OnUserBlacklisted(base::Time time) = 0;

  // Notifies |this| that the user is not blacklisted at |time|.
