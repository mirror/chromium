...
TEST_F(PreviewsUserDataTest, DeepCopy) {
  ...
  EXPECT_EQ(0, deep_copy->data_savings_inflation_percent());
  EXPECT_FALSE(deep_copy->cache_control_no_transform_directive());
  EXPECT_EQ(previews::PreviewsType::NONE, deep_copy->committed_previews_type());
}
