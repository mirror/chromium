...
TEST_F(PreviewsUserDataTest, AddToURLRequest) {
  ...
  PreviewsIOData previews_io_data(nullptr, nullptr);
  data = PreviewsUserData::Create(fake_request.get(), &previews_io_data);
  EXPECT_TRUE(data);
  EXPECT_EQ(data, PreviewsUserData::GetData(*fake_request.get()));

  EXPECT_EQ(data,
            PreviewsUserData::Create(fake_request.get(), &previews_io_data));
}
