...
class TestCastSocketBase : public CastSocketImpl {
 public:
  TestCastSocketBase(const CastSocketConfig& config, Logger* logger)
      : CastSocketImpl(config, logger, AuthContext::Create()),
        ip_(config.ip_endpoint),
...
class MockTestCastSocket : public TestCastSocketBase {
 public:
  static std::unique_ptr<MockTestCastSocket> CreateSecure(
      const CastSocketConfig& config,
      Logger* logger) {
    return std::unique_ptr<MockTestCastSocket>(
        new MockTestCastSocket(config, logger));
  }

  using TestCastSocketBase::TestCastSocketBase;

  MockTestCastSocket(const CastSocketConfig& config, Logger* logger)
      : TestCastSocketBase(config, logger), mock_net_log_(config.net_log) {}
 ...
 public:
  static std::unique_ptr<SslTestCastSocket> CreateSecure(
    ...
    return std::unique_ptr<SslTestCastSocket>(
        new SslTestCastSocket(config, logger));
  }
...
class CastSocketTestBase : public testing::Test {
 protected:
  ...
  CastSocketConfig socket_config_;
...
class MockCastSocketTest : public CastSocketTestBase {
 protected:
  ...
  void CreateCastSocketSecure() {
    socket_ = MockTestCastSocket::CreateSecure(socket_config_, logger_);
  }
...
class SslCastSocketTest : public CastSocketTestBase {
 protected:
  ...
  void CreateSockets() {
    socket_ = SslTestCastSocket::CreateSecure(socket_config_, logger_);
