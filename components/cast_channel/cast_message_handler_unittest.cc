...
class CastMessageHandlerTest : public testing::Test {
 public:
  ...
  base::test::ScopedTaskEnvironment& environment() { return environment_; }
  CastMessageHandler& handler() { return handler_; }
  MockCastSocket& cast_socket() { return cast_socket_; }
...
TEST_F(CastMessageHandlerTest, VirtualConnectionCreatedOnlyOnce) {
  ...
  EXPECT_CALL(*cast_socket().mock_transport(), SendMessage(_, _, _))
  ...
  handler().RequestAppAvailability(
      &cast_socket(), "AAAAAAAA",
      base::BindOnce(&CastMessageHandlerTest::OnAppAvailability,
                     base::Unretained(this)));
  handler().RequestAppAvailability(
      &cast_socket(), "BBBBBBBB",
      base::BindOnce(&CastMessageHandlerTest::OnAppAvailability,
                     base::Unretained(this)));
...
TEST_F(CastMessageHandlerTest, RecreateVirtualConnectionAfterError) {
  ...
  handler().OnError(cast_socket(), ChannelError::TRANSPORT_ERROR);
...
TEST_F(CastMessageHandlerTest, RequestAppAvailability) {
  ...
  response.set_destination_id(handler().sender_id());
  ...
  handler().OnMessage(cast_socket(), response);
}

TEST_F(CastMessageHandlerTest, RequestAppAvailabilityTimesOut) {
  EXPECT_CALL(*cast_socket().mock_transport(), SendMessage(_, _, _)).Times(2);
  handler().RequestAppAvailability(
      &cast_socket(), "ABCDEFAB",
      base::BindOnce(&CastMessageHandlerTest::OnAppAvailability,
                     base::Unretained(this)));
  ...
  environment().FastForwardBy(base::TimeDelta::FromSeconds(5));
}
