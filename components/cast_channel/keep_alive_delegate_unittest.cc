...
TEST_F(KeepAliveDelegateTest, TestLivenessTimerResetAfterSendingMessage) {
  ...
  base::Timer* liveness_timer = new base::Timer(true, false, tick_clock.get());
  base::Timer* ping_timer = new base::Timer(true, false, tick_clock.get());
  ping_timer->SetTaskRunner(mock_time_task_runner);
  liveness_timer->SetTaskRunner(mock_time_task_runner);
  keep_alive_->SetTimersForTest(base::WrapUnique(ping_timer),
                                base::WrapUnique(liveness_timer));
