...
int CastSocketService::OpenSocket(const CastSocketConfig& config,
                                  const CastSocket::OnOpenCallback& open_cb,
                                  CastSocket::Observer* observer) {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  DCHECK(observer);

  auto* socket = GetSocket(config.ip_endpoint);
  ...
  if (!socket) {
    ...
    if (socket_for_test_) {
      ...
      socket = new CastSocketImpl(config, logger_);
...
int CastSocketService::OpenSocket(const net::IPEndPoint& ip_endpoint,
  ...
  cast_channel::CastSocketConfig config(ip_endpoint, net_log, connect_timeout,
                                        liveness_timeout, ping_interval,
                                        CastDeviceCapability::NONE);

  return OpenSocket(config, open_cb, observer);
}
