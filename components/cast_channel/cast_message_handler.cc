...
CastMessageHandler* CastMessageHandler::GetInstance() {
  static CastMessageHandler* instance = new CastMessageHandler(
      CastSocketService::GetInstance(), base::DefaultTickClock::GetInstance());
  return instance;
}
...
void CastMessageHandler::OnMessage(const CastSocket& socket,
  ...
  if (!IsReceiverMessage(message) || !IsPlatformSenderMessage(message))
    return;
