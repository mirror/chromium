...
TEST_F(TopSitesImplTest, SetForcedTopSites) {
  ...
  for (size_t i = 0; i < kNonForcedTopSitesNumber; ++i) {
  ...
  EXPECT_EQ(kNumOldForcedURLs + kNonForcedTopSitesNumber,
            last_num_urls_changed());

  TopSitesQuerier querier;
  // Query only non-forced URLs first.
  querier.QueryTopSites(top_sites(), false);
  ASSERT_EQ(kNonForcedTopSitesNumber, querier.urls().size());
  ...
  ASSERT_EQ(kForcedTopSitesNumber + kNonForcedTopSitesNumber,
            querier.urls().size());
  ...
  EXPECT_EQ("http://newnonforced/0",
            querier.urls()[kForcedTopSitesNumber].url.spec());
  EXPECT_TRUE(querier.urls()[kForcedTopSitesNumber].last_forced_time.is_null());
  EXPECT_EQ("http://newnonforced/9",
            querier.urls()[querier.urls().size() - 1].url.spec());
  EXPECT_TRUE(
      querier.urls()[querier.urls().size() - 1].last_forced_time.is_null());
}
