...
TEST_F(TopSitesImplTest, SetForcedTopSites) {
  ...
  for (size_t i = 0; i < TopSitesImpl::kNonForcedTopSitesNumber; ++i) {
  ...
  EXPECT_EQ(kNumOldForcedURLs + TopSitesImpl::kNonForcedTopSitesNumber,
            last_num_urls_changed());

  TopSitesQuerier querier;
  // Query only non-forced URLs first.
  querier.QueryTopSites(top_sites(), false);
  ASSERT_EQ(TopSitesImpl::kNonForcedTopSitesNumber, querier.urls().size());
  ...
  ASSERT_EQ(TopSitesImpl::kForcedTopSitesNumber +
                TopSitesImpl::kNonForcedTopSitesNumber,
            querier.urls().size());
  ...
  EXPECT_EQ("http://newnonforced/0",
            querier.urls()[TopSitesImpl::kForcedTopSitesNumber].url.spec());
  EXPECT_TRUE(querier.urls()[TopSitesImpl::kForcedTopSitesNumber]
