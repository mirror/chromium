...
TEST_F(TopSitesImplTest, SetForcedTopSites) {
  ...
  for (size_t i = 0; i < top_sites()->kNonForcedTopSitesNumber; ++i) {
  ...
  EXPECT_EQ(kNumOldForcedURLs + top_sites()->kNonForcedTopSitesNumber,
            last_num_urls_changed());

  TopSitesQuerier querier;
  // Query only non-forced URLs first.
  querier.QueryTopSites(top_sites(), false);
  ASSERT_EQ(top_sites()->kNonForcedTopSitesNumber, querier.urls().size());
  ...
  ASSERT_EQ(top_sites()->kForcedTopSitesNumber +
                top_sites()->kNonForcedTopSitesNumber,
            querier.urls().size());
  ...
  size_t non_forced_begin_index = top_sites()->kForcedTopSitesNumber;
  EXPECT_EQ("http://newnonforced/0",
            querier.urls()[non_forced_begin_index].url.spec());
  EXPECT_TRUE(
      querier.urls()[non_forced_begin_index].last_forced_time.is_null());
