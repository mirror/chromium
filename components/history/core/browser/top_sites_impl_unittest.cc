...
TEST_F(TopSitesImplTest, SetForcedTopSites) {
  ...
  const size_t kNumOldForcedURLs = 7;
  ...
  const size_t kNumNonForcedURLs = 10;  // Maximum number of non-forced URLs.
  ...
  // We should have reached the maximum of 10 forced URLs.
  ASSERT_EQ(10 + kNumNonForcedURLs, querier.urls().size());
  ...
  EXPECT_EQ("http://newnonforced/0", querier.urls()[10].url.spec());
  EXPECT_TRUE(querier.urls()[10].last_forced_time.is_null());
  EXPECT_EQ("http://newnonforced/9", querier.urls()[19].url.spec());
  EXPECT_TRUE(querier.urls()[19].last_forced_time.is_null());
}
