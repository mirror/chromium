...
BrowsingHistoryService::BrowsingHistoryService(
  ...
  if (web_history) {
    ...
    // attach a listener. If |sync_service_| is null then syncing is not
    // possible, such as when disabled by flag or incognito/guest mode.
...
void BrowsingHistoryService::WebHistoryQueryComplete(
  ...
  // TODO(skym): Compare |request| to |web_history_request_|. Using the timer is
