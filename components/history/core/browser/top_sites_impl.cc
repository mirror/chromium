...
void TopSitesImpl::StartQueryForMostVisited() {
  ...
  // Always replace the existing provider to ensure that in-flight updates do
  // not collide.
  most_visited_provider_ = base::MakeUnique<TopSitesMostVisitedProvider>(
      most_visited_provider_->most_engaged_provider());
