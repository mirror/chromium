...
// The delay for the first HistoryService query after startup.
constexpr base::TimeDelta kShortDelayForUpdates =
    base::TimeDelta::FromSeconds(15);

// The delay for the all HistoryService queries other than the first one.
#if defined(OS_IOS) || defined(OS_ANDROID)
// On mobile, having the max at 30 minutes results in the topsites database
// being not updated often enough since the app isn't usually running for long
// stretches of time.
constexpr base::TimeDelta kLongDelayForUpdates =
...
void TopSitesImpl::OnNavigationCommitted(const GURL& url) {
  ...
  if (can_add_url_to_history_.Run(url))
    ScheduleUpdateTimer(kLongDelayForUpdates);
}
...
void TopSitesImpl::ScheduleUpdateTimer(base::TimeDelta delta) {
  if (timer_.IsRunning())
    return;

  timer_.Start(FROM_HERE, delta, this, &TopSitesImpl::StartQueryForMostVisited);
}
...
void TopSitesImpl::OnGotMostVisitedThumbnails(
  ...
  ScheduleUpdateTimer(kShortDelayForUpdates);
}
