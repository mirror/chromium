...
void TopSitesDatabase::RemoveURL(const MostVisitedURL& url) {
  ...
  sql::Transaction transaction(db_.get());
  transaction.Begin();
  RemoveURLNoTransaction(url);
  transaction.Commit();
}

void TopSitesDatabase::RemoveURLNoTransaction(const MostVisitedURL& url) {
  int old_rank = GetURLRank(url);
  if (old_rank == kRankOfNonExistingURL)
    return;
...
void TopSitesDatabase::UpdateTopSites(const TopSitesDelta& delta) {
  sql::Transaction transaction(db_.get());
  transaction.Begin();
