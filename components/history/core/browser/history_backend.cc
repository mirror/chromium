...
void HistoryBackend::CloneFaviconMappingsForPages(
  ...
  for (const IconMapping& icon_mapping : icon_mappings) {
    std::vector<GURL> v = SetFaviconMappingsForPages(
        page_urls_to_update_mappings.begin(),
        page_urls_to_update_mappings.end(), icon_mapping.icon_type,
...
bool HistoryBackend::SetFaviconMappingsForPageAndRedirects(
  ...
  bool mappings_changed =
      !SetFaviconMappingsForPages(redirects.begin(), redirects.end(), icon_type,
                                  icon_id)
           .empty();
  if (page_url.has_ref() &&
    ...
    mappings_changed |=
        !SetFaviconMappingsForPages(redirects.begin(), redirects.end(),
                                    icon_type, icon_id)
             .empty();
  }
...
std::vector<GURL> HistoryBackend::SetFaviconMappingsForPages(
  ...
  for (std::vector<GURL>::const_iterator it = page_urls_begin;
       it != page_urls_end; ++it) {
    const GURL& page_url = *it;
