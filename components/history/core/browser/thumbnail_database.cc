...
// For historical reasons, and for backward compatibility, the icon type values
// stored in the DB are powers of two.
int ToPersistedIconType(favicon_base::IconType icon_type) {
...
// Same as above but does the oposite conversion.
favicon_base::IconType FromPersistedIconType(int icon_type) {
  if (icon_type == 0) {
    return favicon_base::IconType::kInvalid;
  }
  return static_cast<favicon_base::IconType>(
      1 + base::bits::Log2Floor(icon_type));
}
