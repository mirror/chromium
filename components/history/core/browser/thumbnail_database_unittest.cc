...
// Test that |fallback_to_host| is used purely as a fallback, and only matches
// hosts exactly if it is true.
TEST_F(ThumbnailDatabaseTest, GetIconMappingsForPageURLWithFallbackToHost) {
  ThumbnailDatabase db(nullptr);
  ASSERT_EQ(sql::INIT_OK, db.Init(file_name_));
  db.BeginTransaction();
  ...
  const GURL kPageUrlHttpSamePrefix("http://www.google.com.au");
  const GURL kPageUrlHttpSameSuffix("http://m.www.google.com");
  const GURL kPageUrlDifferentScheme("ftp://www.google.com");
  AddAndMapFaviconSimple(&db, kPageUrlHttpSamePrefix, kIconUrl2,
                         favicon_base::IconType::kFavicon);
  AddAndMapFaviconSimple(&db, kPageUrlHttpSameSuffix, kIconUrl3,
                         favicon_base::IconType::kFavicon);

  {
    // There should be no mappings for kPageUrlHttp, whether or not
    // |fallback_to_host| is true.
    std::vector<IconMapping> icon_mappings;
    EXPECT_FALSE(db.GetIconMappingsForPageURL(
        kPageUrlHttp, {favicon_base::IconType::kFavicon}, false,
        &icon_mappings));
    EXPECT_TRUE(icon_mappings.empty());

    EXPECT_FALSE(db.GetIconMappingsForPageURL(
        kPageUrlHttp, {favicon_base::IconType::kFavicon}, true,
        &icon_mappings));
    EXPECT_TRUE(icon_mappings.empty());
  }

  AddAndMapFaviconSimple(&db, kPageUrlHttps, kIconUrl1,
                         favicon_base::IconType::kFavicon);

  {
    // When |fallback_to_host| is true, we find the added mapping for
    // kPageUrlHttps.
    std::vector<IconMapping> icon_mappings;
    EXPECT_FALSE(db.GetIconMappingsForPageURL(
        kPageUrlHttp, {favicon_base::IconType::kFavicon}, false,
    ...
    EXPECT_TRUE(db.GetIconMappingsForPageURL(kPageUrlHttp,
                                             {favicon_base::IconType::kFavicon},
                                             true, &icon_mappings));
  ...
  // We should only find the kPageUrlHttp mapping now whether or not
  // |fallback_to_host| is true since there is an exact match.
  {
    std::vector<IconMapping> icon_mappings;
    EXPECT_TRUE(db.GetIconMappingsForPageURL(kPageUrlHttp,
                                             {favicon_base::IconType::kFavicon},
  ...
  // Not having a HTTP/HTTPS scheme should never fall back.
  {
    std::vector<IconMapping> icon_mappings;
    EXPECT_FALSE(db.GetIconMappingsForPageURL(
        kPageUrlDifferentScheme, {favicon_base::IconType::kFavicon}, false,
        &icon_mappings));
    EXPECT_TRUE(icon_mappings.empty());

    EXPECT_FALSE(db.GetIconMappingsForPageURL(
        kPageUrlDifferentScheme, {favicon_base::IconType::kFavicon}, true,
        &icon_mappings));
    EXPECT_TRUE(icon_mappings.empty());
  }
}
