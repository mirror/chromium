...
class TypedURLTester : public syncer::ModelTypeSyncBridgeTester {
 public:
  ...
  void Restart() override {
    ...
    this->~TypedURLTester();
    new (this) TypedURLTester(factory, std::move(test_dir));
  }
  ...
  std::vector<TestEntityData> GetTestEntities() override {
    std::vector<TestEntityData> entities;
...

    {
      ...
      typed_url->add_visit_transitions(ui::PAGE_TRANSITION_TYPED);
      entities.emplace_back(std::make_unique<URLRowAndVisits>(
                                GURL(kURLStr2), kTitle2, 1, 4, false),
                            specifics);
    }
  ...
  void MutateStoredTestEntity(const std::string& storage_key,
                              LocalType* local) override {
    VisitRow visit = static_cast<URLRowAndVisits*>(local)->MakeVisit(
        ui::PAGE_TRANSITION_TYPED, 7);
    fake_history_backend_->AddVisitsForUrl(GURL(kURLStr), {visit});
  }

  void MutateTestEntitySpecifics(sync_pb::EntitySpecifics* specifics) override {
    TypedUrlSpecifics* typed_url = specifics->mutable_typed_url();
