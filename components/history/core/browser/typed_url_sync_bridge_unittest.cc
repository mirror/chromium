...
class TypedURLTester
 ...
 public:
  ...
  sync_pb::TypedUrlSpecifics GetTestSpecifics1() const {
  ...
  TestEntityData GetTestEntity1() const override {
    return {URLRowAndVisits::Make(GURL(kURLStr), kTitle, 1, 3, false),
            SpecificsToEntity(GetTestSpecifics1())};
  }
  ...
  TestEntityData GetTestEntity2() const override {
    ...
    typed_url.add_visit_transitions(ui::PAGE_TRANSITION_TYPED);
    return {URLRowAndVisits::Make(GURL(kURLStr2), kTitle2, 1, 4, false),
            SpecificsToEntity(typed_url)};
  }
  ...
  EntityDataPtr GetMutatedTestEntity1Specifics() const override {
    EntityData data;
    data.client_tag_hash = "ignored";
    auto typed_url = data.specifics.mutable_typed_url();
    *typed_url = GetTestSpecifics1();
...
class TypedURLSyncBridgeTest : public testing::Test {
 public:
  ...
  void StartSyncing(const std::vector<TypedUrlSpecifics>& specifics) {
    std::vector<EntityDataPtr> initial_data;
    for (const TypedUrlSpecifics& typed_url : specifics) {
      initial_data.push_back(SpecificsToEntity(typed_url));
    }
    tester_.StartSyncingAndWait(initial_data);
  }
