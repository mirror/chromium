...
class TypedURLTester
 ...
 public:
  ...
  TestEntityData GetTestEntity1() const override {
    ...
    entity.local = URLRowAndVisits::Make(GURL(kURLStr), kTitle, 1, 3, false);
  ...
  TestEntityData GetTestEntity2() const override {
    TestEntityData entity;
    entity.local = URLRowAndVisits::Make(GURL(kURLStr2), kTitle2, 1, 4, false);
  ...
  std::string StoreEntityInLocalModel(URLRowAndVisits* local) override {
    fake_history_backend_->AddRowAndVisits(local);
    return IntToStorageKey(local->row.id());
  }
  ...
  void MutateStoredTestEntity1() override {
    LOCAL_T local = GetTestEntity1().local;
    local.AddNewestVisit(ui::PAGE_TRANSITION_TYPED, 7);
    fake_history_backend_->AddVisitsForUrl(local.row.url(),
                                           {local.visits.back()});
  }
