...
struct URLRowAndVisits {
  URLRow row;
  VisitVector visits;

  // Create a new row object and the typed visit çorresponding with the time at
  // |last_visit|.
  static URLRowAndVisits Make(const GURL& url,
                              const std::string& title,
                              int typed_count,
                              int64_t last_visit,
                              bool hidden) {
    URLRowAndVisits result;
    // Give each URL a unique ID, to mimic the behavior of the real database.
    result.row = URLRow(url);
    result.row.set_title(base::UTF8ToUTF16(title));
    result.row.set_typed_count(typed_count);

    base::Time last_visit_time = base::Time::FromInternalValue(last_visit);
    result.row.set_last_visit(last_visit_time);

    if (typed_count > 0) {
      // Add a typed visit for time |last_visit|.
      result.visits.push_back(VisitRow(result.row.id(), last_visit_time, 0,
    ...
    result.row.set_visit_count(result.visits.size());
    return result;
  }
...
class TestHistoryBackend : public HistoryBackend {
 public:
  ...
  URLRowAndVisits AddRowAndVisits(const GURL& url,
                                  ...
                                  bool hidden) {
    auto row_and_visits =
        URLRowAndVisits::Make(url, title, typed_count, last_visit, hidden);
...
class TypedURLTester
    : public syncer::ModelTypeSyncBridgeTester<URLRowAndVisits,
                                               sync_pb::TypedUrlSpecifics,
                                               TypedURLTester> {
 public:
  static void Restart(std::unique_ptr<TypedURLTester>* tester) {
    auto factory = (*tester)->change_processor_factory_;
    auto test_dir = std::move((*tester)->test_dir_);
    tester->reset();
    *tester = std::make_unique<TypedURLTester>(factory, std::move(test_dir));
  }
  ...
  TypedURLTester(
    ...
    std::unique_ptr<TypedURLSyncBridge> bridge =
        base::MakeUnique<TypedURLSyncBridge>(fake_history_backend_.get(),
  ...
  static EntityDataPtr SpecificsToEntityImpl(
  ...
  static const sync_pb::TypedUrlSpecifics& SelectSpecificsImpl(
  ...
  TestEntityData GetTestEntity1() const override {
    ...
    entity.native = URLRowAndVisits::Make(GURL(kURLStr), kTitle, 1, 3, false);
    entity.specifics.set_url(kURLStr);
    entity.specifics.set_title(kTitle);
    entity.specifics.set_hidden(false);
    entity.specifics.add_visits(3);
    entity.specifics.add_visit_transitions(ui::PAGE_TRANSITION_TYPED);
    return entity;
  }

  TestEntityData GetTestEntity2() const override {
    TestEntityData entity;
    entity.native = URLRowAndVisits::Make(GURL(kURLStr2), kTitle2, 1, 4, false);
    entity.specifics.set_url(kURLStr2);
    entity.specifics.set_title(kTitle2);
    entity.specifics.set_hidden(false);
    entity.specifics.add_visits(4);
    entity.specifics.add_visit_transitions(ui::PAGE_TRANSITION_TYPED);
    return entity;
  }

  std::string StoreEntityInLocalModel(URLRowAndVisits* native) override {
    fake_history_backend_->AddRowAndVisits(native);
    return IntToStorageKey(native->row.id());
  }
  ...
  SPECIFICS GetMutatedTestEntity1Specifics() const override {
    ...
    entity.specifics.add_visits(7);
    entity.specifics.add_visit_transitions(ui::PAGE_TRANSITION_TYPED);
    return entity.specifics;
  }

  void MutateStoredTestEntity1() override {
    NATIVE_T native = GetTestEntity1().native;
    native.AddNewestVisit(ui::PAGE_TRANSITION_TYPED, 7);
    fake_history_backend_->AddVisitsForUrl(native.row.url(),
                                           {native.visits.back()});
  }
...
class TypedURLSyncBridgeTest : public testing::Test {
 public:
  ...
  sync_pb::TypedUrlSpecifics GetLastUpdateForURL(const GURL& url) {
  ...
  static sync_pb::TypedUrlSpecifics MakeTypedUrlSpecifics(
      const GURL& url,
      const std::string& title,
      int64_t last_visit,
      bool hidden) {
...
TEST_F(TypedURLSyncBridgeTest, MergeUrlNoTypedUrl) {
  ...
  auto row_and_visits = URLRowAndVisits::Make(kURL, kTitle, 0, 3, false);
  ...
  tester_.StartSyncingAndWait(std::vector<TypedUrlSpecifics>());
...
TEST_F(TypedURLSyncBridgeTest, MergeUrlOldLocal) {
  ...
  URLRowAndVisits server_row_and_visits =
      URLRowAndVisits::Make(kURL, kTitle2, 1, 6, false);
  ...
  *typed_url = ToTypedUrlSpecifics(server_row_and_visits);
  tester_.StartSyncingAndWait({*typed_url});
...
TEST_F(TypedURLSyncBridgeTest, MergeUrlOldSync) {
  ...
  URLRowAndVisits server_row_and_visits =
      URLRowAndVisits::Make(kURL, kTitle, 1, 2, false);
  sync_pb::EntitySpecifics entity_specifics;
  sync_pb::TypedUrlSpecifics* typed_url = entity_specifics.mutable_typed_url();
  *typed_url = ToTypedUrlSpecifics(server_row_and_visits);
  tester_.StartSyncingAndWait({*typed_url});
...
TEST_F(TypedURLSyncBridgeTest, MergeUrlsWithUsernameAndPassword) {
  ...
  URLRowAndVisits server_row_and_visits =
      URLRowAndVisits::Make(kURLWithUsernameAndPassword, kTitle, 1, 2, false);
  sync_pb::EntitySpecifics entity_specifics;
  sync_pb::TypedUrlSpecifics* typed_url = entity_specifics.mutable_typed_url();
  *typed_url = ToTypedUrlSpecifics(server_row_and_visits);
...
TEST_F(TypedURLSyncBridgeTest, TypedVisitLocalTypedUrl) {
  ...
  // Update the URL row, adding another typed visit to the visit vector.
  URLRowAndVisits row_and_visits =
      URLRowAndVisits::Make(kURL, kTitle, 1, 3, false);
...
TEST_F(TypedURLSyncBridgeTest, AddExpiredUrlAndVisits) {
  ...
  sync_pb::TypedUrlSpecifics specifics = ToTypedUrlSpecifics(
      URLRowAndVisits::Make(kURL, kTitle, 1, kExpiredVisit, false));

  tester_.ApplySyncChangesAndWait(
      bridge()->CreateMetadataChangeList(),
      {EntityChange::CreateAdd(std::string(),
                               TypedURLTester::SpecificsToEntity(specifics))});
...
TEST_F(TypedURLSyncBridgeTest, WriteTypedUrlSpecifics) {
  auto row_and_visits = URLRowAndVisits::Make(kURL, kTitle, 0, 1, false);
  ...
  sync_pb::TypedUrlSpecifics typed_url = ToTypedUrlSpecifics(row_and_visits);
...
TEST_F(TypedURLSyncBridgeTest, TooManyVisits) {
  ...
  auto row_and_visits =
      URLRowAndVisits::Make(kURL, kTitle, 1, timestamp++, false);
  for (int i = 0; i < 100; ++i)
...
TEST_F(TypedURLSyncBridgeTest, TooManyTypedVisits) {
  int64_t timestamp = 1000;
  auto row_and_visits =
      URLRowAndVisits::Make(kURL, kTitle, 0, timestamp++, false);
  row_and_visits.visits.clear();
  for (int i = 0; i < 102; ++i) {
...
TEST_F(TypedURLSyncBridgeTest, NoTypedVisits) {
  auto row_and_visits = URLRowAndVisits::Make(kURL, kTitle, 0, 1000, false);
  sync_pb::TypedUrlSpecifics typed_url;
...
TEST_F(TypedURLSyncBridgeTest, MergeUrls) {
  auto row_and_visits1 = URLRowAndVisits::Make(kURL, kTitle, 2, 3, false);
  ...
  auto row_and_visits2 = URLRowAndVisits::Make(kURL, kTitle, 2, 3, false);
  sync_pb::TypedUrlSpecifics specs2(
      MakeTypedUrlSpecifics(kURL, kTitle, 3, true));
  VisitVector expected_visits2;
  auto expected2 = URLRowAndVisits::Make(kURL, kTitle, 2, 3, true);
  ...
  auto row_and_visits3 = URLRowAndVisits::Make(kURL, kTitle, 2, 3, false);
  sync_pb::TypedUrlSpecifics specs3(
      MakeTypedUrlSpecifics(kURL, kTitle2, 3, true));
  auto expected3 = URLRowAndVisits::Make(kURL, kTitle2, 2, 3, true);
  ...
  auto row_and_visits4 = URLRowAndVisits::Make(kURL, kTitle, 2, 3, false);
  sync_pb::TypedUrlSpecifics specs4(
      MakeTypedUrlSpecifics(kURL, kTitle2, 4, false));
  auto expected4 = URLRowAndVisits::Make(kURL, kTitle2, 2, 4, false);
  ...
  auto row_and_visits5 = URLRowAndVisits::Make(kURL, kTitle, 1, 4, false);
  sync_pb::TypedUrlSpecifics specs5(
      MakeTypedUrlSpecifics(kURL, kTitle, 3, false));
  auto expected5 = URLRowAndVisits::Make(kURL, kTitle2, 2, 3, false);
...
TEST_F(TypedURLSyncBridgeTest, MergeUrlsAfterExpiration) {
  ...
  auto row_and_visits = URLRowAndVisits::Make(kURL, kTitle, 2, 2, false);
  row_and_visits.AddNewestVisit(ui::PAGE_TRANSITION_TYPED, 3);
...
INSTANTIATE_TYPED_TEST_CASE_P(TypedURL,
                              SyncBridgeTest,
                              history::TypedURLTester);
