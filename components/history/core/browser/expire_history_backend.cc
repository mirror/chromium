...
void ExpireHistoryBackend::ExpireVisits(const VisitVector& visits) {
  ...
  base::TimeDelta get_redirects_time = base::Time::Now() - start;
  ...
  base::TimeDelta expire_visits_time = base::Time::Now() - start;
  UMA_HISTOGRAM_TIMES("History.ExpireVisitsTime", expire_visits_time);
  if (!expire_visits_time.is_zero()) {
    UMA_HISTOGRAM_COUNTS_100("History.GetRedirectsTimePercentage",
