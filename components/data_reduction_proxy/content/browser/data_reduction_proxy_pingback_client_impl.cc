...
void DataReductionProxyPingbackClientImpl::SendPingback(
  ...
  if (timing.host_id != content::ChildProcessHost::kInvalidUniqueID) {
    ...
    UMA_HISTOGRAM_ENUMERATION(
        kHistogramCrash, CrashAction::kSentWithoutAnalysis, CrashAction::kLast);
...
void DataReductionProxyPingbackClientImpl::OnCrashDumpProcessed(
  ...
  // The crash processing time should be between 1 ms and 5 seconds, but the
  // buckets go to 10 seconds to record cases where the delayed task is deferred
  // for longer than that.
  UMA_HISTOGRAM_TIMES(
      "DataReductionProxy.Pingback.CrashProcessTime",
      base::TimeTicks::Now() - std::get<2>(crash_page_load_information));

  UMA_HISTOGRAM_ENUMERATION(kHistogramCrash, CrashAction::kAnalyzed,
                            CrashAction::kLast);
...
void DataReductionProxyPingbackClientImpl::AddRequestToCrashMap(
  ...
  crash_map_.insert(std::make_pair(
      timing.host_id,
      std::make_tuple(request_data, timing, base::TimeTicks::Now())));
...
void DataReductionProxyPingbackClientImpl::RemoveFromCrashMap(
  ...
  UMA_HISTOGRAM_ENUMERATION(kHistogramCrash, CrashAction::kSentWithoutAnalysis,
                            CrashAction::kLast);
