...
enum class CrashAction {
  ...
  CRASH_ANALYZED = 1,
  // The crash dump was not analyzed, but the information was queued to be sent.
  CRASH_SENT_WITHOUT_ANALYSIS = 2,
  // The crash dump was successfully sent to the server.
  CRASH_SENT_SUCCESSFULLY = 3,
  // The crash dump request completed unsuccessfully.
  CRASH_SENT_UNSUCCESSFULLY = 4,
  LAST = CRASH_SENT_UNSUCCESSFULLY + 1,
};
...
DataReductionProxyPingbackClientImpl::DataReductionProxyPingbackClientImpl(
    ...
    : url_request_context_(url_request_context),
      ...
      pending_fetcher_message_count_(0u),
      current_fetcher_crash_count_(0u),
      pending_fetcher_crash_count_(0u),
...
void DataReductionProxyPingbackClientImpl::OnURLFetchComplete(
  ...
  for (size_t crash = 0u; crash < current_fetcher_crash_count_; ++crash) {
    UMA_HISTOGRAM_ENUMERATION(kHistogramCrash,
                              (source->GetStatus().is_success()
                                   ? CrashAction::CRASH_SENT_SUCCESSFULLY
                                   : CrashAction::CRASH_SENT_UNSUCCESSFULLY),
                              CrashAction::LAST);
  }
  current_fetcher_crash_count_ = 0u;
...
void DataReductionProxyPingbackClientImpl::SendPingback(
  ...
  if (timing.host_id != content::ChildProcessHost::kInvalidUniqueID) {
    UMA_HISTOGRAM_ENUMERATION(kHistogramCrash, CrashAction::CRASH_DETECTED,
                              CrashAction::LAST);
#if defined(OS_ANDROID)
    ...
    UMA_HISTOGRAM_ENUMERATION(kHistogramCrash,
                              CrashAction::CRASH_SENT_WITHOUT_ANALYSIS,
                              CrashAction::LAST);
...
void DataReductionProxyPingbackClientImpl::OnCrashDumpProcessed(
  ...
  UMA_HISTOGRAM_ENUMERATION(kHistogramCrash, CrashAction::CRASH_ANALYZED,
                            CrashAction::LAST);
...
void DataReductionProxyPingbackClientImpl::CreateReport(
    ...
    PageloadMetrics_RendererCrashType crash_type) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  ++pending_fetcher_message_count_;
  if (crash_type != PageloadMetrics_RendererCrashType_NO_CRASH) {
    ++pending_fetcher_crash_count_;
  }
...
void DataReductionProxyPingbackClientImpl::CreateFetcherForDataAndStart() {
  ...
  current_fetcher_message_count_ = pending_fetcher_message_count_;
  pending_fetcher_message_count_ = 0;

  current_fetcher_crash_count_ = pending_fetcher_crash_count_;
  pending_fetcher_crash_count_ = 0;
