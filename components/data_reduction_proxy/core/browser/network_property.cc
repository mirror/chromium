...
static const char kSecureProxiesAllowed[] = "secure_proxies_allowed=";
static const char kInsecureProxiesAllowed[] = "insecure_proxies_allowed=";
...
NetworkProperty::NetworkProperty(const std::string& serialized) {
  ...
  for (const std::string& token : tokens) {
    ...
    if (idx == 0) {
      int value = -1;
      base::StringToInt(token.substr(arraysize(kSecureProxiesAllowed) - 1),
                        &value);
    ...
    if (idx == 0) {
      int value = -1;
      base::StringToInt(token.substr(arraysize(kInsecureProxiesAllowed) - 1),
                        &value);
      DCHECK(value >= 0 && value <= PROXY_STATE_LAST);
      if (value >= 0 && value <= PROXY_STATE_LAST)
