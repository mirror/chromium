...
std::string NetworkProperty::ToString() const {
  DCHECK_LE(0, secure_proxies_state_);
  DCHECK_GE(kMaxProxyState, secure_proxies_state_);
  DCHECK_LE(0, insecure_proxies_state_);
...
bool NetworkProperty::IsSecureProxyAllowed() const {
  return secure_proxies_state_ == PROXY_STATE_FLAG_ALLOWED;
}

bool NetworkProperty::IsInsecureProxyAllowed() const {
  return insecure_proxies_state_ == PROXY_STATE_FLAG_ALLOWED;
}
