...
void DataReductionProxyConfig::ReloadConfig() {
  ...
  if (enabled_by_user_ && !params::IsIncludedInHoldbackFieldTrial() &&
    ...
    configurator_->Enable(!network_property_.IsSecureProxyAllowed(),
                          !network_property_.IsInsecureProxyAllowed(),
...
void DataReductionProxyConfig::HandleCaptivePortal() {
  ...
  UMA_HISTOGRAM_BOOLEAN("DataReductionProxy.CaptivePortalDetected.Platform",
                        is_captive_portal);
  if (is_captive_portal == network_property_.IsCaptivePortal())
    return;
  network_property_.SetIsCaptivePortal(is_captive_portal);
  ReloadConfig();
}
...
void DataReductionProxyConfig::UpdateConfigForTesting(
  ...
  enabled_by_user_ = enabled;
  network_property_.SetIsSecureProxyDisallowedByCarrier(
      !secure_proxies_allowed);
  network_property_.SetHasWarmupURLProbeFailed(false,
...
void DataReductionProxyConfig::HandleSecureProxyCheckResponse(
  ...
  bool secure_proxy_allowed_past =
      !network_property_.IsSecureProxyDisallowedByCarrier();
  network_property_.SetIsSecureProxyDisallowedByCarrier(!success_response);
  if (!enabled_by_user_)
    return;

  if (!network_property_.IsSecureProxyDisallowedByCarrier() !=
...
void DataReductionProxyConfig::OnInsecureProxyWarmupURLProbeStatusChange(
    bool insecure_proxies_allowed) {
  DCHECK(thread_checker_.CalledOnValidThread());
  bool old_status = network_property_.IsInsecureProxyAllowed();
  network_property_.SetHasWarmupURLProbeFailed(false,
                                               !insecure_proxies_allowed);

  if (old_status == network_property_.IsInsecureProxyAllowed())
    return;
  ReloadConfig();
}
...
net::ProxyConfig DataReductionProxyConfig::ProxyConfigIgnoringHoldback() const {
  ...
  return configurator_->CreateProxyConfig(
      !network_property_.IsSecureProxyAllowed(),
      !network_property_.IsInsecureProxyAllowed(),
      config_values_->proxies_for_http());
}
...
bool DataReductionProxyConfig::secure_proxy_allowed() const {
  DCHECK(thread_checker_.CalledOnValidThread());
  return network_property_.IsSecureProxyAllowed();
}

bool DataReductionProxyConfig::insecure_proxies_allowed() const {
  DCHECK(thread_checker_.CalledOnValidThread());
  return network_property_.IsInsecureProxyAllowed();
}
