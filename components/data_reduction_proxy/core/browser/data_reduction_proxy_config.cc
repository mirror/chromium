...
void DataReductionProxyConfig::FetchWarmupProbeURL() {
  ...
  size_t previous_attempt_counts = GetPreviousAttemptCounts();
...
size_t DataReductionProxyConfig::GetPreviousAttemptCounts() const {
  DCHECK(thread_checker_.CalledOnValidThread());
  return network_properties_manager_->GetAttemptCount(
      warmup_url_fetch_in_flight_secure_proxy_,
      warmup_url_fetch_in_flight_core_proxy_);
}
