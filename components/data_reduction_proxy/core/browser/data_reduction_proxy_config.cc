...
void DataReductionProxyConfig::ReloadConfig() {
  ...
  if (enabled_by_user_ && !params::IsIncludedInHoldbackFieldTrial() &&
    ...
    configurator_->Enable(!network_property_.IsProxyAllowed(true),
                          !network_property_.IsProxyAllowed(false),
...
void DataReductionProxyConfig::OnInsecureProxyAllowedStatusChange(
    bool insecure_proxies_allowed) {
  DCHECK(thread_checker_.CalledOnValidThread());
  bool old_status = network_property_.IsProxyAllowed(false);
  network_property_.SetHasWarmupURLProbeFailed(false,
                                               !insecure_proxies_allowed);

  if (old_status == network_property_.IsProxyAllowed(false))
    return;
  ReloadConfig();
}
...
net::ProxyConfig DataReductionProxyConfig::ProxyConfigIgnoringHoldback() const {
  ...
  return configurator_->CreateProxyConfig(
      !network_property_.IsProxyAllowed(true),
      !network_property_.IsProxyAllowed(false),
      config_values_->proxies_for_http());
}
...
bool DataReductionProxyConfig::secure_proxy_allowed() const {
  DCHECK(thread_checker_.CalledOnValidThread());
  return network_property_.IsProxyAllowed(true);
}

bool DataReductionProxyConfig::insecure_proxies_allowed() const {
  DCHECK(thread_checker_.CalledOnValidThread());
  return network_property_.IsProxyAllowed(false);
}
