  ...
  for (const auto test : tests) {
...

    {
      ...
      delegate.OnResolveProxy(test.gurl, "GET", empty_proxy_retry_info,
                              &proxy_info);
      ASSERT_TRUE(first_proxy == proxy_info.proxy_server());
...

    {
      ...
      ASSERT_TRUE(second_proxy == proxy_info.proxy_server());
...

    {
      ...
      delegate.OnResolveProxy(test.gurl, "GET", empty_proxy_retry_info,
                              &proxy_info);
      ASSERT_TRUE(proxy_info.proxy_server() == non_drp_proxy_server);
    ...
    if (expect_alternative_proxy_server_to_first_proxy) {
      ...
      delegate.OnResolveProxy(test.gurl, "GET", empty_proxy_retry_info,
                              &proxy_info);
      ASSERT_TRUE(proxy_info.proxy_server() == first_proxy);
