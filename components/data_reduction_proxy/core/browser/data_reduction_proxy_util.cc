...
int64_t EstimateOriginalHeaderBytes(const net::URLRequest& request,
  ...
  // If this is an auto-reload of an image, then the original size would be zero
  // as the previous request already tracked the estimate of the full size.
...
int64_t EstimateOriginalBodySize(const net::URLRequest& request,
                                 const LoFiDecider* lofi_decider) {
  if (lofi_decider) {
    // If this is an auto-reload of an image, then the original size would be
    // zero as the previous request already tracked the estimate of the full
    // size.
