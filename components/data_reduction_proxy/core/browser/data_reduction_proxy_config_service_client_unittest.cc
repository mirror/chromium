...
// than 24 hours old.
TEST_F(DataReductionProxyConfigServiceClientTest, PersistedDelta) {
  Init(true);
  SetDataReductionProxyEnabled(true, true);
  ...
  const struct {
    ...
    bool expect_valid_config;
      ...
      {
          true, base::nullopt, true,
      },
      {
          true, base::TimeDelta::FromHours(25), false,
      },
      {
          true, base::TimeDelta::FromHours(1), true,
      },
  };
  ...
  for (const auto& test : tests) {
    ...
    if (test.persist_config) {
      // Apply a valid config. This should be stored on the disk.
      AddMockSuccess();
      config_client()->RetrieveConfig();
    } else {
      // Invalidate the config which should reset the secure session key.
      test_context_->io_data()->test_request_options()->Invalidate();
    }
    RunUntilIdle();
    ...
    RunUntilIdle();
    EXPECT_NE(test.persist_config, persisted_config().empty());
    EXPECT_NE(test.persist_config, persisted_config_retrieval_time().is_null());
