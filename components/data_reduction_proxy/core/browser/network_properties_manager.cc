...
bool NetworkPropertiesManager::IsSecureProxyAllowed(bool is_core_proxy) const {
  bool warmup_url_failed =
      is_core_proxy ? network_properties_.secure_core_proxy_flags()
                          .disallowed_due_to_warmup_probe_failure()
                    : network_properties_.secure_non_core_proxy_flags()
                          .disallowed_due_to_warmup_probe_failure();

  return !network_properties_.secure_proxy_disallowed_by_carrier() &&
         !network_properties_.has_captive_portal() && !warmup_url_failed;
}

bool NetworkPropertiesManager::IsInsecureProxyAllowed(
    bool is_core_proxy) const {
  return is_core_proxy ? !network_properties_.insecure_core_proxy_flags()
                              .disallowed_due_to_warmup_probe_failure()
                       : !network_properties_.insecure_non_core_proxy_flags()
                              .disallowed_due_to_warmup_probe_failure();
}
