...
NetworkPropertiesManager::NetworkPropertiesManager(
  ...
  failed_warmup_url_fetch_counts_secure_core_ = 0;
  failed_warmup_url_fetch_counts_secure_non_core_ = 0;
  failed_warmup_url_fetch_counts_insecure_core_ = 0;
  failed_warmup_url_fetch_counts_insecure_non_core_ = 0;
...
bool NetworkPropertiesManager::ShouldFetchWarmupURL(bool secure_proxy,
                                                    bool is_core_proxy) const {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

  if (secure_proxy && is_core_proxy) {
    return !has_warmup_url_succeded_secure_core_ &&
           failed_warmup_url_fetch_counts_secure_core_ <
               kMaxWarmupURLFetchAttempts;
  } else if (secure_proxy && !is_core_proxy) {
    return !has_warmup_url_succeded_secure_non_core_ &&
           failed_warmup_url_fetch_counts_secure_non_core_ <
               kMaxWarmupURLFetchAttempts;
  } else if (!secure_proxy && is_core_proxy) {
    return !has_warmup_url_succeded_insecure_core_ &&
           failed_warmup_url_fetch_counts_insecure_core_ <
               kMaxWarmupURLFetchAttempts;
  } else {
    return !has_warmup_url_succeded_insecure_non_core_ &&
           failed_warmup_url_fetch_counts_insecure_non_core_ <
               kMaxWarmupURLFetchAttempts;
  }
}
