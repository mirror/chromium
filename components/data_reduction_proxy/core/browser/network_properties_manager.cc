...
void NetworkPropertiesManager::OnChangeInNetworkPropertyOnUIThread(
  ...
  if (prefs_as_dict->size() > kMaxCacheSize) {
    ...
    for (const auto& it : prefs_.DictItems()) {
      ...
      base::Base64Decode(value_string, &base64_decoded);

      NetworkProperties network_properties_it;
      network_properties_it.ParseFromString(base64_decoded);
      int64_t timestamp = network_properties_it.last_modified();
