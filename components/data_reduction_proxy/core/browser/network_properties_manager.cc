...
bool NetworkPropertiesManager::IsSecureProxyAllowed() const {
  return !network_properties_.secure_proxy_disallowed_by_carrier() &&
         ...
         !network_properties_
              .secure_proxy_disallowed_due_to_warmup_probe_failure();
}

bool NetworkPropertiesManager::IsInsecureProxyAllowed() const {
  return !network_properties_
              .insecure_proxy_disallowed_due_to_warmup_probe_failure();
}
...
void NetworkPropertiesManager::SetHasWarmupURLProbeFailed(
  ...
  if (secure_proxy) {
    network_properties_.set_secure_proxy_disallowed_due_to_warmup_probe_failure(
        warmup_url_probe_failed);
  } else {
    network_properties_
        .set_insecure_proxy_disallowed_due_to_warmup_probe_failure(
            warmup_url_probe_failed);
  }
}
