...
class NetworkPropertyTest : public NetworkProperty {
 public:
  ...
  using NetworkProperty::ProxyState;
  ...
  for (int secure_proxy_state = 0;
    ...
    for (int insecure_proxy_state = 0;
      ...
      NetworkPropertyTest network_property(
          static_cast<NetworkPropertyTest::ProxyState>(secure_proxy_state),
          static_cast<NetworkPropertyTest::ProxyState>(insecure_proxy_state));
      ...
      EXPECT_EQ(network_property.IsInsecureProxyAllowed(),
                network_property_reversed.IsInsecureProxyAllowed());
      EXPECT_EQ(network_property.IsSecureProxyAllowed(),
                network_property_reversed.IsSecureProxyAllowed());
      EXPECT_EQ(network_property.IsSecureProxyDisallowedByCarrier(),
                network_property_reversed.IsSecureProxyDisallowedByCarrier());
      EXPECT_EQ(network_property.IsCaptivePortal(),
                network_property_reversed.IsCaptivePortal());
      EXPECT_EQ(network_property.HasWarmupURLProbeFailed(false),
                network_property_reversed.HasWarmupURLProbeFailed(false));
      EXPECT_EQ(network_property.HasWarmupURLProbeFailed(true),
                network_property_reversed.HasWarmupURLProbeFailed(true));
...
TEST(NetworkPropertyTest, TestSetterGetterCaptivePortal) {
  ...
  EXPECT_TRUE(network_property.IsInsecureProxyAllowed());
  EXPECT_TRUE(network_property.IsSecureProxyAllowed());

  network_property.SetIsCaptivePortal(true);
  EXPECT_TRUE(network_property.IsInsecureProxyAllowed());
  EXPECT_FALSE(network_property.IsSecureProxyAllowed());
...
TEST(NetworkPropertyTest, TestWarmupURLFailedOnInSecureProxy) {
  ...
  EXPECT_FALSE(network_property.IsInsecureProxyAllowed());
  EXPECT_TRUE(network_property.IsSecureProxyAllowed());
  EXPECT_FALSE(network_property.IsSecureProxyDisallowedByCarrier());
  EXPECT_FALSE(network_property.IsCaptivePortal());
