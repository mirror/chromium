  ...
  for (int secure_proxy_state = 0; secure_proxy_state <= PROXY_STATE_LAST;
    ...
    for (int insecure_proxy_state = 0; insecure_proxy_state <= PROXY_STATE_LAST;
      ...
      EXPECT_EQ(static_cast<ProxyState>(secure_proxy_state),
                network_property.secure_proxies_allowed());
      EXPECT_EQ(static_cast<ProxyState>(insecure_proxy_state),
                network_property.insecure_proxies_allowed());
      ...
      EXPECT_EQ(network_property.secure_proxies_allowed(),
                network_property_reversed.secure_proxies_allowed());
      EXPECT_EQ(network_property.insecure_proxies_allowed(),
                network_property_reversed.insecure_proxies_allowed());
