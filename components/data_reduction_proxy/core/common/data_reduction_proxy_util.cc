...
int64_t CalculateEffectiveOCL(const net::URLRequest& request) {
  ...
  int64_t original_content_length_from_header =
      GetDataReductionProxyOFCL(response_headers);
  ...
  if (response_headers->response_code() == net::HTTP_PARTIAL_CONTENT) {
    ...
    if (response_headers->GetContentRangeFor206(&first, &last,
        ...
        range_content_length > 0 && original_content_length_from_header > 0) {
      ...
      original_content_length_from_header = ScaleByteCountByRatio(
          request.received_response_content_length(),
          original_content_length_from_header, range_content_length);
    }
  }
  ...
  int64_t content_length_from_header = response_headers->GetContentLength();
