...
namespace {
// Copies the password value from |reply| to |password| and notifies that
// the result is ready.
...
// Copies the initialisation result from |result| to |success| and notifies that
// the result is ready.
...
std::unique_ptr<KeyStorageLinux> KeyStorageLinux::CreateService(
  ...
  if (selected_backend == os_crypt::SelectedLinuxBackend::GNOME_ANY ||
    ...
    if (key_storage->InitOnTaskRunner()) {
...
bool KeyStorageLinux::InitOnTaskRunner() {
