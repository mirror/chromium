...
// static
std::unique_ptr<network::SimpleURLLoader> ServiceURLLoader::Create(
  ...
  std::unique_ptr<network::SimpleURLLoader> default_loader =
      network::SimpleURLLoader::Create(std::move(resource_request),
                                       annotation_tag);
  // Wrap a raw new because ServiceURLLoader is protected, so make_unique can
  // not be used.
  return base::WrapUnique<ServiceURLLoader>(
      new ServiceURLLoader(std::move(default_loader), service_name));
}
...
void ServiceURLLoader::DownloadAsStream(
    network::mojom::URLLoaderFactory* url_loader_factory,
