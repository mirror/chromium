...
uint8_t testing_private_key[X25519_PRIVATE_KEY_LEN];
bool is_testing_key_set = false;
...
void SetPrivateKeyForTesting(const uint8_t* key) {
  memcpy(testing_private_key, key, sizeof(testing_private_key));
  is_testing_key_set = true;
}
...
bool EncryptSerializedMessage(const uint8_t* server_public_key,
  ...
  if (is_testing_key_set) {
    memcpy(private_key, testing_private_key, sizeof(private_key));
  } else {
