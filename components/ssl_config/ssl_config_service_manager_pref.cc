...
SSLConfigServiceManagerPref::SSLConfigServiceManagerPref(
  ...
  const std::string tls13_variant_group =
      base::FieldTrialList::FindFullName(kTLS13VariantExperimentName);
  if (StartsWith(tls13_variant_group, switches::kTLS13VariantDisabled,
                 base::CompareCase::INSENSITIVE_ASCII)) {
    local_state->SetDefaultPrefValue(
        ssl_config::prefs::kTLS13Variant,
        new base::Value(switches::kTLS13VariantDisabled));
  } else if (StartsWith(tls13_variant_group, switches::kTLS13VariantDraft,
                        base::CompareCase::INSENSITIVE_ASCII)) {
    local_state->SetDefaultPrefValue(
        ssl_config::prefs::kTLS13Variant,
  ...
  } else if (StartsWith(tls13_variant_group, switches::kTLS13VariantExperiment,
                        base::CompareCase::INSENSITIVE_ASCII)) {
    local_state->SetDefaultPrefValue(
        ssl_config::prefs::kTLS13Variant,
  ...
  // Initialize from UI thread.  This is okay as there shouldn't be anything
  // on the IO thread trying to access it yet.
...
void SSLConfigServiceManagerPref::GetSSLConfigFromPrefs(
  ...
  if (tls13_variant_str == switches::kTLS13VariantDisabled) {
    ...
    config->tls13_variant = net::TLS13_VARIANT_DRAFT;
  } else if (tls13_variant_str == switches::kTLS13VariantExperiment) {
    config->tls13_variant = net::TLS13_VARIANT_EXPERIMENT;
  }
