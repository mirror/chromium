...
SSLConfigServiceManagerPref::SSLConfigServiceManagerPref(
  ...
  if (base::StartsWith(tls13_variant_group, "Disabled",
                       base::CompareCase::INSENSITIVE_ASCII)) {
    local_state->SetDefaultPrefValue(
        ssl_config::prefs::kTLS13Variant,
        new base::Value(switches::kTLS13VariantDisabled));
  } else if (base::StartsWith(tls13_variant_group, "DraftEnabled",
                              base::CompareCase::INSENSITIVE_ASCII)) {
    local_state->SetDefaultPrefValue(
        ssl_config::prefs::kTLS13Variant,
  ...
  } else if (base::StartsWith(tls13_variant_group, "ExperimentEnabled",
                              base::CompareCase::INSENSITIVE_ASCII)) {
    local_state->SetDefaultPrefValue(
        ssl_config::prefs::kTLS13Variant,
