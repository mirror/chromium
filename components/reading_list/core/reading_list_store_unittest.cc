...
class ReadingListTester : public syncer::ModelTypeSyncBridgeTester {
 public:
  ...
  void Restart() override {
    ...
    EXPECT_TRUE(store);
    this->~ReadingListTester();
    new (this) ReadingListTester(factory, std::move(store));
  }
  ...
  std::vector<TestEntityData> GetTestEntities() override {
...

    {
      ...
      entities.emplace_back(std::move(local), remote);
    }
  ...
  void MutateStoredTestEntity(const std::string& storage_key,
    ...
    auto* reading_list = static_cast<ReadingListAddParams*>(local);
    reading_list->title = "zzz mutated title";
    model_->SetEntryTitle(GURL(storage_key), reading_list->title);
  }

  void MutateTestEntitySpecifics(sync_pb::EntitySpecifics* specifics) override {
    sync_pb::ReadingListSpecifics* reading_list =
        specifics->mutable_reading_list();
    reading_list->set_title("zzz mutated title");
  }
