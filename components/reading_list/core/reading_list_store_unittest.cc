...
class ReadingListTester : public syncer::ModelTypeSyncBridgeTester {
 public:
  ...
  std::unique_ptr<ModelTypeSyncBridgeTester> Restart(
      std::unique_ptr<ModelTypeSyncBridgeTester> this_unique_ptr) override {
    CHECK_EQ(this, this_unique_ptr.get());
    auto factory = change_processor_factory_;
    std::unique_ptr<syncer::ModelTypeStore> store =
        reading_list_store_->StealStoreForTest();
    EXPECT_TRUE(store);
    this_unique_ptr.reset();
  ...
  sync_pb::EntitySpecifics GetMutatedTestEntity1Specifics() const override {
    int64_t time_us =
        (clock_->Now() - base::Time::UnixEpoch()).InMicroseconds();
    ...
    reading_list->set_status(sync_pb::ReadingListSpecifics::UNREAD);
    reading_list->set_first_read_time_us(time_us);
    return specifics;
  }

  void MutateStoredTestEntity1() override {
    model_->SetReadStatus(GURL("http://read.example.com/"), true);
  }
...
INSTANTIATE_TEST_CASE_P(
    ReadingList,
    SyncBridgeTest,
    ::testing::Values(ReadingListTester::GetFactory<ReadingListTester>()));
