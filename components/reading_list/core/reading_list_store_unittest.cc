...
class ReadingListTester
 ...
 public:
  static void Restart(std::unique_ptr<ReadingListTester>* tester) {
    ...
    ASSERT_TRUE((*tester)->reading_list_store_);
    std::unique_ptr<syncer::ModelTypeStore> store =
        (*tester)->reading_list_store_->StealStoreForTest();
    ASSERT_TRUE(store);
    tester->reset();
  ...
  std::string StoreEntityInLocalModel(LOCAL_T* local) override {
    model_->AddEntry(local->url, local->title,
                     reading_list::ADDED_VIA_CURRENT_APP);
    model_->SetReadStatus(local->url, local->is_read);
    return local->url.spec();
  }
  ...
  SPECIFICS GetMutatedTestEntity1Specifics() const override {
    ...
    specifics.set_status(sync_pb::ReadingListSpecifics::UNREAD);
    specifics.set_first_read_time_us(time_us);
    return specifics;
  }

  void MutateStoredTestEntity1() override {
    model_->SetReadStatus(GetTestEntity1().local.url, true);
  }
