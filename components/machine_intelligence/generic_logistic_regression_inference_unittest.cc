...
class GenericLogisticRegressionInferenceTest : public testing::Test {
 protected:
  ...
  const float bias_ = 1.5;
  const float threshold_ = 0.6;
  const float scalar1_weight_ = 0.8;
  const float scalar2_weight_ = -2.4;
  const float scalar3_weight_ = 0.01;
  const float elem1_weight_ = -1.0;
  const float elem2_weight_ = 5.0;
  const float elem3_weight_ = -1.5;
  const float one_hot_default_weight_ = 10.0;
  const float epsilon_ = 0.001;
};
...
TEST_F(GenericLogisticRegressionInferenceTest, BaseTest) {
  ...
  features[scalar2_name_].set_int64_value(42);
  ...
  float expected_score =
      Sigmoid(bias_ + 1.0 * scalar1_weight_ + 42.0 * scalar2_weight_ +
              0.666 * scalar3_weight_ + elem1_weight_);
...
TEST_F(GenericLogisticRegressionInferenceTest, UnknownFeatures) {
  ...
  features["foo2"].set_int64_value(42);
  features["foo3"].set_float_value(0.666);
...
TEST_F(GenericLogisticRegressionInferenceTest, Threshold) {
  ...
  auto& features = *example.mutable_features();
  features[scalar1_name_].set_bool_value(true);
  features[scalar2_name_].set_int64_value(2);
  features[one_hot_name_].set_string_value(one_hot_elem1_name_);
  ...
  features[scalar3_name_].set_float_value(0.01);
  float score = predictor.PredictScore(example);
...
TEST_F(GenericLogisticRegressionInferenceTest, NoThreshold) {
  ...
  float score = predictor.PredictScore(example);
  EXPECT_NEAR(0.5, score, epsilon_);
