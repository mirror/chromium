...
std::unique_ptr<RankerModelLoader> AssistRankerServiceImpl::InitializeModel(
    GURL model_url,
    std::string model_filename,
    std::string uma_prefix,
    AssistRankerService::OnModelAvailableCallback on_model_available_callback) {
  auto model_loader = base::MakeUnique<RankerModelLoader>(
      base::Bind(&ValidateModel), on_model_available_callback,
      url_request_context_getter_.get(), GetModelPath(model_filename),
      model_url, uma_prefix);
  // Kick off the initial load from cache.
  model_loader->NotifyOfRankerActivity();
  return model_loader;
}
