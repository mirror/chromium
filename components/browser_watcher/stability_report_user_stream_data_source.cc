...
void CollectProcessMemoryMetrics(crashpad::ProcessSnapshot* process_snapshot,
  ...
  auto process_states = report->mutable_process_states();
  for (int i = 0; i < process_states->size(); ++i) {
    ProcessState* temp = process_states->Mutable(i);
  ...
  if (!process_state) {
    process_state = process_states->Add();
  ...
  // Grab the requensted allocation size in case of OOM exception.
