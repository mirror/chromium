...
void RecordExitCodes(const base::string16& registry_path) {
  ...
  if (num > std::numeric_limits<base::HistogramBase::Count>::max()) {
    ...
    LOG(FATAL) << "Value count would cause histogram count overflow.";
  }
  ...
  base::HistogramBase* exit_code_histogram = nullptr;
    ...
    if (error != ERROR_SUCCESS) {
      ...
      if (regkey.ReadValueDW(name.c_str(), &exit_code) == ERROR_SUCCESS) {
        ...
        if (exit_code != STILL_ACTIVE || IsDeadProcess(name)) {
          ...
          if (!exit_code_histogram) {
            exit_code_histogram = base::SparseHistogram::FactoryGet(
  ...
  if (exit_code_histogram) {
    ...
    auto corruption = exit_code_histogram->FindCorruption(*samples);
