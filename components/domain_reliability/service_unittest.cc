...
class DomainReliabilityServiceTest : public testing::Test {
 protected:
  ...
  size_t discarded_upload_count() const {
    return monitor_->uploader_->discarded_upload_count();
  }
...
TEST_F(DomainReliabilityServiceTest, UploadAllowed) {
  ...
  // TODO)juliatuttle): I think I can get away without setting
  // HttpResponseHeaders here?
  ...
  EXPECT_EQ(1u, permission_manager_->get_permission_status_count());

  network_task_runner_->RunPendingTasks();
  EXPECT_EQ(1u, discarded_upload_count());
}
...
TEST_F(DomainReliabilityServiceTest, UploadForbidden) {
  ...
  network_task_runner_->RunPendingTasks();
  EXPECT_EQ(0u, discarded_upload_count());
}
