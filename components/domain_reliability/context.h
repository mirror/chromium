...
class DOMAIN_RELIABILITY_EXPORT DomainReliabilityContext {
 public:
  ...
  using UploadAllowedCallback =
      base::Callback<void(const GURL&, const base::Callback<void(bool)>&)>;
  ...
  // XXX do not land, this is just to make upload_allowed_callback_ not-unused.
  const UploadAllowedCallback& upload_allowed_callback() const {
