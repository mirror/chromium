...
const LocalizedErrorMap* LookupErrorMap(blink::WebURLError::Domain error_domain,
                                        int error_code,
                                        bool is_post) {
  if (error_domain == blink::WebURLError::Domain::kNet) {
    ...
    return FindErrorMapInArray(net_error_options,
                               ...
                               error_code);
  } else if (error_domain == blink::WebURLError::Domain::kHttp) {
    return FindErrorMapInArray(http_error_options,
                               arraysize(http_error_options),
                               error_code);
  } else if (error_domain == blink::WebURLError::Domain::kDnsProbeError) {
...
const char* GetIconClassForError(blink::WebURLError::Domain error_domain,
                                 int error_code) {
  if ((error_code == net::ERR_INTERNET_DISCONNECTED &&
       error_domain == blink::WebURLError::Domain::kNet) ||
      (error_code == error_page::DNS_PROBE_FINISHED_NO_INTERNET &&
       error_domain == blink::WebURLError::Domain::kDnsProbeError))
    return "icon-offline";
...
void LocalizedError::GetStrings(
  ...
  if (error_domain == blink::WebURLError::Domain::kNet) {
    ...
    DCHECK_EQ(blink::WebURLError::Domain::kHttp, error_domain);
...
base::string16 LocalizedError::GetErrorDetails(
    blink::WebURLError::Domain error_domain,
    int error_code,
    bool is_post) {
...
bool LocalizedError::HasStrings(blink::WebURLError::Domain error_domain,
                                int error_code) {
