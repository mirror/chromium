...
base::Value PrefModelAssociator::MergeDictionaryValues(
  ...
  for (const auto& it : from_value.DictItems()) {
    base::Value* result_dict =
        result.FindKeyOfType(it.first, base::Value::Type::DICTIONARY);
    if (result_dict && it.second.is_dict()) {
      *result_dict = MergeDictionaryValues(it.second, *result_dict);
      // Note that for all other types we want to preserve the "to"
      // values so we do nothing here.
    } else {
      result.SetKey(it.first, it.second.Clone());
    }
  }
  return result;
}
