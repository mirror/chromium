...
void OmniboxEditModel::StartAutocomplete(bool has_selected_text,
  ...
  if (is_keyword_selected()) {
    // If there is user text, the cursor is effectively past keyword and
    // doesn't account for its size.  Add its size to the position passed to
    // autocomplete.
    if (!user_text_.empty()) {
      cursor_position += input_text.length() - user_text_.length();
    } else {
      // Otherwise, cursor may point into keyword. Pass a cursor at end
      // of input to autocomplete.
