...
bool OmniboxEditModel::ResetDisplayURLs() {
  ...
  const bool visibly_changed_permanent_text =
      (GetCurrentPermanentURLText() != old_current_permanent_url) &&
...
void OmniboxEditModel::OnWillKillFocus() {
  ...
  // TODO(tommycli): This seems redundant with the RevertAll call in the View
  // code. Find a way to consolidate these call.
  if (!user_input_in_progress_ &&
      base::FeatureList::IsEnabled(
          omnibox::kUIExperimentHideSteadyStateUrlSchemeAndSubdomains)) {
    view_->RevertAll();
