...
void OmniboxEditModel::StartAutocomplete(bool has_selected_text,
  ...
  input_ = AutocompleteInput(input_text, cursor_position, std::string(),
                             client_->GetSchemeClassifier());
  input_.set_current_url(client_->GetURL());
  input_.set_current_title(client_->GetTitle());
  input_.set_current_page_classification(ClassifyPage());
...
void OmniboxEditModel::AcceptInput(WindowOpenDisposition disposition,
  ...
  if (control_key_state_ == DOWN_WITHOUT_CHANGE && !is_keyword_selected() &&
    ...
    AutocompleteInput input(
        ...
        input_.cursor_position(), "com", client_->GetSchemeClassifier());
    input.set_current_page_classification(input_.current_page_classification());
...
void OmniboxEditModel::OpenMatch(AutocompleteMatch match,
  ...
  AutocompleteInput alternate_input(input_text, base::string16::npos,
                                    std::string(),
                                    client_->GetSchemeClassifier());
  ...
  alternate_input.set_current_page_classification(ClassifyPage());
  alternate_input.set_allow_exact_keyword_match();
  alternate_input.set_want_asynchronous_matches();
...
void OmniboxEditModel::OnSetFocus(bool control_down) {
  ...
  if (client_->CurrentPageExists() && !user_input_in_progress_) {
    ...
    input_ = AutocompleteInput(permanent_text_, base::string16::npos,
                               std::string(), client_->GetSchemeClassifier());
    input_.set_current_url(client_->GetURL());
    input_.set_current_title(client_->GetTitle());
    input_.set_current_page_classification(ClassifyPage());
