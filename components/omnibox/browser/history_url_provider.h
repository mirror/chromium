...
class HistoryURLProvider : public HistoryProvider {
 ...
 private:
  ...
  // has been visited by the user in the past, the return value is built from
  // the canonicalized version of the autocomplete input in |params|. The scheme
  // and host format (e.g. prefixed with "www.") of the return value is the same
  // as one of the corresponding entries in the database.
