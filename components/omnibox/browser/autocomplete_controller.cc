...
void AutocompleteController::Start(const AutocompleteInput& input) {
  ...
  // If the input looks like a query, we send a signal predicting that the user
  // is going to issue a search (either to the default search engine or to a
  ...
  // dereference.
  if (base::FeatureList::IsEnabled(omnibox::kSpeculativeServiceWorkerStart) &&
