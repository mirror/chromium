...
void AutocompleteResult::SortAndCull(
  ...
  struct timeval tv2;
  gettimeofday(&tv2, 0);
  krb_cumulative_time += (tv2.tv_sec - tv1.tv_sec) * 1000000 +
  ...
  alternate_nav_url_ = matches_.empty() ?
      GURL() : ComputeAlternateNavUrl(input, matches_.front());
}
...
void AutocompleteResult::SortAndDedupMatches(
  ...
  for (ACMatches::iterator i(matches->begin()); i != matches->end(); ++i) {
    for (auto j = std::next(i); j != matches->end() &&
                    AutocompleteMatch::DestinationsEqual(*i, *j);
         ++j) {
  ...
  struct timeval tv2;
  gettimeofday(&tv2, 0);
  krb_cumulative_time5 += (tv2.tv_sec - tv1.tv_sec) * 1000000 +
      tv2.tv_usec - tv1.tv_usec;
...
void AutocompleteResult::CopyFrom(const AutocompleteResult& rhs) {
  ...
  // reconstruct them.
  default_match_ = (rhs.default_match_ == rhs.end()) ? end() :
