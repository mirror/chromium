...
void ContextualSuggestionsService::CreateContextualSuggestionsRequest(
    const GURL& base_suggest_url,
    const std::string& current_url,
    net::URLFetcherDelegate* fetcher_delegate,
    ContextualSuggestionsCallback callback) {
  const bool is_experimental =
      UseExperimentalZeroSuggestSuggestions(current_url);
  std::unique_ptr<net::URLFetcher> fetcher = CreateRequest(
      base_suggest_url, current_url, is_experimental, fetcher_delegate);

  const bool should_fetch_access_token = is_experimental &&
                                         signin_manager_ != nullptr &&
                                         token_service_ != nullptr;
  // If authentication services are unavailable, run the contextual service
  // without access tokens.
  if (!should_fetch_access_token) {
...
std::unique_ptr<net::URLFetcher> ContextualSuggestionsService::CreateRequest(
  ...
  const GURL suggest_url =
      ZeroSuggestURL(base_suggest_url, current_url, is_experimental);
