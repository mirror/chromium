...
// Scope for the authentication token attached to the request sent to the
// experimental service.
const char kScopeForExperimentalService[] =
...
void ContextualSuggestionsService::CreateContextualSuggestionsRequest(
  ...
  const bool experimental = UseExperimentalZeroSuggestSuggestions(visited_url);
  std::unique_ptr<net::URLFetcher> fetcher =
      CreateRequest(visited_url, experimental, fetcher_delegate);
  if (fetcher == nullptr) {
  ...
  if (experimental) {
    ...
    OAuth2TokenService::ScopeSet scopes{kScopeForExperimentalService};
...
GURL ContextualSuggestionsService::ZeroSuggestURL(
  ...
  if (experimental) {
    return ExperimentalZeroSuggestURL(visited_url);
  }
  ...
  if (!visited_url.empty()) {
    search_term_args.current_page_url = visited_url;
  }
...
std::unique_ptr<net::URLFetcher> ContextualSuggestionsService::CreateRequest(
  ...
  const GURL suggest_url = ZeroSuggestURL(visited_url, experimental);
