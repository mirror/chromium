...
// If the user is in zero suggest field trial that enables search-for
// queries as suggestions and either
//   * the user is not signed-in or
//   * the user does not have Google set up as their default search engine,
// the personalized service is replaced with the most visited service.
...
void ZeroSuggestProvider::Start(const AutocompleteInput& input,
  ...
  Stop(true, false);
  service_running_ = ZeroSuggestService::NONE;
  ...
  service_running_ = ServiceToRun(input.current_url(), suggest_url);
  if (service_running_ == ZeroSuggestProvider::NONE)
    return;
  ...
  done_ = false;
  if (service_running_ == ZeroSuggestProvider::MOST_VISITED) {
  ...
  const std::string current_url =
      service_running_ == ZeroSuggestProvider::DEFAULT_SERP_WITH_URL
          ? current_query_
          : std::string();
...
void ZeroSuggestProvider::Stop(bool clear_cached_results,
  ...
  service_running_ = ZeroSuggestProvider::NONE;
}
...
bool ZeroSuggestProvider::UpdateResults(const std::string& json_data) {
  ...
  // resposes.
  if (service_running_ == ZeroSuggestService::PERSONALIZED_SUGGESTIONS) {
    client()->GetPrefs()->SetString(omnibox::kZeroSuggestCachedResults,
                                    json_data);
    ...
    const bool non_empty_parsed_list = data->GetAsList(&root_list) &&
                                       ...
                                       !results_list->empty();
    const bool non_empty_cache_results = !matches_.empty();
    if (non_empty_parsed_list && non_empty_cache_results)
      return false;
  }
  return ParseSuggestResults(*data, kDefaultZeroSuggestRelevance, false,
                             &results_);
}
...
void ZeroSuggestProvider::OnMostVisitedUrlsAvailable(
  ...
  if (service_running_ != ZeroSuggestService::MOST_VISITED)
    return;
...
void ZeroSuggestProvider::MaybeUseCachedSuggestions() {
  if (service_running_ != ZeroSuggestProvider::PERSONALIZED_SUGGESTIONS)
    return;
...
ZeroSuggestProvider::ZeroSuggestService ZeroSuggestProvider::ServiceToRun(
  ...
  if (OmniboxFieldTrial::InZeroSuggestPersonalizedFieldTrial(
    ...
    return PersonalizedServiceShouldFallBackToMostVisited(
               ...
               ? ZeroSuggestService::MOST_VISITED
               : ZeroSuggestService::PERSONALIZED_SUGGESTIONS;
  ...
  if (OmniboxFieldTrial::InZeroSuggestMostVisitedWithoutSerpFieldTrial(
    ...
    return ZeroSuggestService::NONE;

  if (OmniboxFieldTrial::InZeroSuggestMostVisitedFieldTrial(
          client()->GetPrefs()))
    return ZeroSuggestService::MOST_VISITED;

  return can_send_current_url ? ZeroSuggestService::DEFAULT_SERP_WITH_URL
                              : ZeroSuggestService::DEFAULT_SERP_WITHOUT_URL;
}
