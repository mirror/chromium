...
void ZeroSuggestProvider::Start(const AutocompleteInput& input,
  ...
  Stop(true, false);
  result_type_processed_ = ResultType::NONE;
  ...
  result_type_processed_ =
      TypeOfResultToProcess(input.current_url(), suggest_url);
  if (result_type_processed_ == ZeroSuggestProvider::NONE)
    return;
  ...
  if (result_type_processed_ == ZeroSuggestProvider::MOST_VISITED) {
  ...
  const std::string current_url =
      result_type_processed_ == ZeroSuggestProvider::DEFAULT_SERP_FOR_URL
          ? current_query_
          : std::string();
...
void ZeroSuggestProvider::Stop(bool clear_cached_results,
  ...
  result_type_processed_ = ZeroSuggestProvider::NONE;
}
...
bool ZeroSuggestProvider::UpdateResults(const std::string& json_data) {
  ...
  if (result_type_processed_ == ResultType::DEFAULT_SERP_FROM_CACHE &&
      !json_data.empty()) {
    client()->GetPrefs()->SetString(omnibox::kZeroSuggestCachedResults,
                                    json_data);
    ...
    const bool non_empty_parsed_list = data->GetAsList(&root_list) &&
                                       ...
                                       !results_list->empty();
    const bool non_empty_results = !matches_.empty();
    if (non_empty_parsed_list && non_empty_results)
      return false;
  }
...
void ZeroSuggestProvider::OnMostVisitedUrlsAvailable(
  ...
  if (result_type_processed_ != ResultType::MOST_VISITED)
    return;
...
void ZeroSuggestProvider::MaybeUseCachedSuggestions() {
  if (result_type_processed_ != ZeroSuggestProvider::DEFAULT_SERP_FROM_CACHE)
    return;
...
ZeroSuggestProvider::ResultType ZeroSuggestProvider::TypeOfResultToProcess(
  ...
  if (OmniboxFieldTrial::InZeroSuggestPersonalizedFieldTrial(
    ...
    return PersonalizedServiceShouldFallBackToMostVisited(
               ...
               : ResultType::DEFAULT_SERP_FROM_CACHE;
