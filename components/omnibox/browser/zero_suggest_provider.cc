...
// Used to determine whether or not Most Visited URLs are allowed to be
// displayed. This is true in either of these two cases:
//   1. The user is in zero suggest most visited field trial.
//   2. The user is in zero suggest field trial that enables search-for
//   queries as suggestions and the user does not have Google set up as their
//   default search engine.
bool AllowZeroSuggestMostVisitedURLs(
    PrefService* prefs,
...
void ZeroSuggestProvider::Start(const AutocompleteInput& input,
  ...
  if (AllowZeroSuggestMostVisitedURLs(client()->GetPrefs(),
