...
struct TestURLInfo {
  ...
  int age_in_days;
    ...
    {"https://www.abc.def.com/path", "URL with subdomain", 1, 1, 80},
    {"https://www.hij.com/path", "URL with www only", 1, 1, 80},
};
...
void HistoryURLProviderTest::ExpectFormattedFullMatch(
  ...
  // Without sorting, the last match contains the full URL. This is match we use
  // to test all the variations of URL formatting on.
  EXPECT_EQ(base::WideToUTF16(expected_match_contents),
            autocomplete_->matches().back().contents);
}
