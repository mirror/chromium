...
struct TestURLInfo {
  ...
  int age_in_days;
    ...
    {"http://www.intra/www", "Intranet www", 1, 1, 80},
...
TEST_F(HistoryURLProviderTest, IntranetURLCompletion) {
  ...
  const UrlAndLegalDefault expected8[] = {{"http://www.intra/www", true}};
  ASSERT_NO_FATAL_FAILURE(RunTest(ASCIIToUTF16("intra/w"), std::string(), false,
                                  expected7, arraysize(expected7)));
  EXPECT_LE(1400, matches_[0].relevance);
  EXPECT_LT(matches_[0].relevance, 1410);
}
