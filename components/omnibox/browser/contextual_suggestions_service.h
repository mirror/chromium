...
class ContextualSuggestionsService : public KeyedService {
 public:
  ...
  // Creates a fetcher for contextual suggestions for |current_url| and it
  // passes the fetcher to the provided callback.
  //
  // |current_url| may be empty, in which case the system will never use the
  // experimental suggestions service.
  //
  ...
  // an argument. Otherwise |callback|
 ...
 private:
  ...
  // It uses the experimental suggest URL if |is_experimental| is true.
