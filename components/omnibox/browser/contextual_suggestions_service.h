...
class ContextualSuggestionsService : public KeyedService {
 public:
  ...
  // |request_context| and |template_url_service| may be null, but some services
  // may be disabled.
  ...
  //
  // |base_suggest_url| is used as a fallback if the experimental suggestions
  // service cannot be used for any reason.
  //
 ...
 private:
  ...
  // It uses the experimental suggest URL if |is_experimental| is true, and
  // uses |base_suggest_url| if |is_experimental| is false.
  GURL ZeroSuggestURL(const GURL& base_suggest_url,
                      const std::string& current_url,
                      bool is_experimental) const;

  // Returns true if all the folowing conditions are true:
