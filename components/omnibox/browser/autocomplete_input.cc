...
metrics::OmniboxInputType AutocompleteInput::Parse(
  ...
  *canonicalized_url =
  ...
  // non-navigable and thus is treated like a non-compliant hostname.
  ...
  // See https://tools.ietf.org/html/rfc6761. Note that DomainIs also covers the
  // Note that DomainIs also covers the host, e.g. just the word localhost, that
  // is not desired for "example" or "test".
