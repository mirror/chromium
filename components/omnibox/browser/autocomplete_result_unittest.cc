...
TEST_F(AutocompleteResultTest, SortAndCullDuplicateSearchURLs) {
  ...
  PopulateAutocompleteMatches(data, arraysize(data), &matches);
  {
    ...
    ++match;
    match->destination_url = GURL("http://www.foo.com/s?q=foo2");
    ++match;
    match->destination_url = GURL("http://www.foo.com/s?q=foo&oq=f");
    ++match;
    match->destination_url = GURL("http://www.foo.com/s?q=foo&aqs=0");
    ++match;
    match->destination_url = GURL("http://www.foo.com/");
  }
  AutocompleteInput input(base::ASCIIToUTF16("a"), base::string16::npos,
                          std::string(), GURL(), base::string16(),
                          OmniboxEventProto::INVALID_SPEC, false, false, false,
...
TEST_F(AutocompleteResultTest, SortAndCullWithMatchDups) {
  ...
  PopulateAutocompleteMatches(data, arraysize(data), &matches);
  {
    ...
    match->destination_url = GURL("http://www.foo.com/");
    ++match;
    match->destination_url = GURL("http://www.foo.com/s?q=foo2&oq=f");
  }
  AutocompleteInput input(base::ASCIIToUTF16("a"), base::string16::npos,
                          std::string(), GURL(), base::string16(),
                          OmniboxEventProto::INVALID_SPEC, false, false, false,
  ...
  auto duplicate = result.match_at(0)->duplicate_matches.begin();
  ++duplicate;
  auto expected_match = matches.begin();
  ++expected_match;
  ++expected_match;
  EXPECT_EQ(expected_match->destination_url, duplicate->destination_url);
  ++duplicate;
  EXPECT_EQ(dup_match.destination_url, duplicate->destination_url);
  ++duplicate;
  ++expected_match;
  EXPECT_EQ(expected_match->destination_url, duplicate->destination_url);
  ...
  ++expected_match;
  ++expected_match;
  EXPECT_EQ(expected_match->destination_url,
...
TEST_F(AutocompleteResultTest, SortAndCullReorderForDefaultMatch) {
...

  {
    ...
    ACMatches matches;
    PopulateAutocompleteMatches(data, arraysize(data), &matches);
    matches.front().allowed_to_be_default_match = false;
    auto i = matches.begin();
    ++i;
    i->allowed_to_be_default_match = false;
    AutocompleteInput input(base::ASCIIToUTF16("a"), base::string16::npos,
                            std::string(), GURL(), base::string16(),
                            OmniboxEventProto::HOME_PAGE, false, false, false,
                            true, false, test_scheme_classifier);
...
TEST_F(AutocompleteResultTest, InlineTailPrefixes) {
  ...
  // Tail suggestion needs one-off initialization.
  {
    auto i = matches.begin();
    ++i;
    i->RecordAdditionalInfo(kACMatchPropertyContentsStartIndex, "5");
    i->RecordAdditionalInfo(kACMatchPropertySuggestionText, "superstar");
  }
  AutocompleteResult result;
  result.AppendMatches(AutocompleteInput(), matches);
