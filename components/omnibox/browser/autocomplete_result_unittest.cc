...
TEST_F(AutocompleteResultTest, InlineTailPrefixes) {
  ...
  for (size_t i = 0; i < sizeof(test_data) / sizeof(test_data[0]); ++i) {
    AutocompleteMatch match;
    match.type = test_data[i].type;
    match.contents = base::UTF8ToUTF16(test_data[i].before_contents);
    for (const auto& classification : test_data[i].before_contents_class)
  ...
  for (size_t i = 0; i < sizeof(test_data) / sizeof(test_data[0]); ++i) {
    EXPECT_EQ(result.match_at(i)->contents,
              base::UTF8ToUTF16(test_data[i].after_contents));
    EXPECT_TRUE(EqualClassifications(result.match_at(i)->contents_class,
                                     test_data[i].after_contents_class));
  }
}
