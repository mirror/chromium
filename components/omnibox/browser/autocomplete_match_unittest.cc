...
TEST(AutocompleteMatchTest, InlineNontailPrefix) {
  ...
  for (const auto& kase : cases) {
    ...
    match.contents = base::UTF8ToUTF16(kase.contents);
    match.contents_class = kase.before_contents_class;
    match.InlineTailPrefix(base::UTF8ToUTF16("12345678"));
    EXPECT_TRUE(
        EqualClassifications(match.contents_class, kase.after_contents_class));
  }
}
