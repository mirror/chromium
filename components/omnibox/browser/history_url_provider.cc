...
AutocompleteMatch HistoryURLProvider::HistoryMatchToACMatch(
  ...
  size_t match_end = match_start + params.input.text().length();
  base::OffsetAdjuster::AdjustOffset(adjustments, &match_start);
  base::OffsetAdjuster::AdjustOffset(adjustments, &match_end);

  if ((match_start != base::string16::npos) && autocomplete_offset_valid &&
      (match_end != match_start)) {
    DCHECK(match_end > match_start);
    AutocompleteMatch::ClassifyLocationInString(
