...
void KeywordProvider::Start(const AutocompleteInput& input,
  ...
  if (matches.front().first->keyword() == keyword) {
    ...
    // If this is an extension keyword, ask it for suggestions, but only do so
    // if we're explicitly in keyword mode (as identified by
    // input.allow_exact_keyword_match()) as otherwise it's surprising.
