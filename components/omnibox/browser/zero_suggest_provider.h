...
class ZeroSuggestProvider : public BaseSearchProvider,
 ...
 private:
  ...
  // ZeroSuggestProvider is processing any of the following type of results
  // at any time.
  enum ResultType {
    NONE,
    ...
    DEFAULT_SERP_FOR_URL,     // The default search provider is queried for
                              ...
                              // to the visited URL.
    DEFAULT_SERP_FROM_CACHE,  // Use a cache service for suggestions
                              // provided by the default search provider.
    MOST_VISITED
  };
  ...
  //
  // If the provider is using cached results,
  //    * the cache is updated if |json_data| has a non-empty list of
  //    suggestions, and
  //    * |results_| are updated only if the cache stores an empty list of
  //    suggestions.
  //
  ...
  // Returns the type of results that should be processed for the current
  // context.
  // Logs UMA metrics. Should be called exactly once, on Start(), otherwise the
  // meaning of the data logged would change.
  ResultType TypeOfResultToProcess(const GURL& current_url,
  ...
  // When the provider is not running, the result type is set tu NONE.
  ResultType result_type_processed_;
