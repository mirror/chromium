...
TEST_F(OmniboxEditTest, SetSelectedLine) {
  ...
  model()->autocomplete_controller()->result_.AppendMatches(AutocompleteInput(),
                                                            matches);
  // Nope, still need to set the default match. Looks like only way is to add a
  // test provider, perhaps like the one in autocomplete_controller_unittest.
