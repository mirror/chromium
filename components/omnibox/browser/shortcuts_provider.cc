...
// Upper bound using a one-armed comparison function.
// Saves constructing a value.
template <typename CIT, typename CMP>
CIT upper_bound(
    CIT beg,
    CIT end,
    CMP mine_is_less) {
  unsigned lo = 0, hi = end - beg;
  while (hi - lo > 2) {
    unsigned mi = lo + (hi - lo) / 2;
    if (mine_is_less(*(beg + mi)))
      hi = mi;
    else
      lo = mi;
  }
  if (beg + lo == end || mine_is_less(*(beg + lo)))
    return beg + lo;
  if (beg + lo + 1 == end || mine_is_less(*(beg + lo + 1)))
    return beg + lo + 1;
  return beg + lo + 2;
}
...
ACMatchClassifications ShortcutsProvider::ClassifyAllMatchesInString(
  ...
  while (last_position < text_lowercase.length()) {
    ...
    // Find upper bound, then search backwards until first character
    // doesn't match.
    WordList::const_iterator i = ::upper_bound(
        find_words.begin(), find_words.end(),
        [&](const base::string16& w) {
          return text_lowercase.compare(last_position, w.length(), w) < 0;
        });
    if (i == find_words.end())
      --i;
    while ((*i)[0] >= text_lowercase[last_position]) {
