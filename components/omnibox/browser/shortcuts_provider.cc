...
ACMatchClassifications ShortcutsProvider::ClassifyAllMatchesInString(
  ...
  // Now, starting with |last_position|, in |text_lowercase| to see if we have
  // words starting with that character in |find_words|.  If so, check each of
  // them to see if they match the portion of |text_lowercase| beginning with
  // |last_position|. Accept the first matching word found (which should be the
  // longest possible match at this location, given the construction of
  // |find_words|) and add a MATCH region to |match_class|, moving
  // |last_position| to be after the matching word.  If we found no matching
  // words, move to the next character and repeat.
