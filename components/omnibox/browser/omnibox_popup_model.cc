...
OmniboxPopupModel::OmniboxPopupModel(OmniboxPopupView* popup_view,
    ...
    : favicons_cache_(AutocompleteResult::GetMaxMatches()),
      view_(popup_view),
...
void OmniboxPopupModel::OnFaviconFetched(const GURL& page_url,
  ...
  // Cache empty results too, so we don't bother to try to fetch it again.
  favicons_cache_.Put(page_url, icon);

  // But don't notify for empty results.
  if (icon.IsEmpty())
    return;

  // Notify all affected matches.
  for (size_t i = 0; i < result().size(); ++i) {
    if (result().match_at(i).destination_url == page_url)
