...
OmniboxPopupModel::OmniboxPopupModel(
  ...
  delegate_(delegate), client_(client),
      ...
      selected_line_(kNoMatch), selected_line_state_(NORMAL) {}

  OmniboxPopupModel::~OmniboxPopupModel() {}

  // static
  void OmniboxPopupModel::ComputeMatchMaxWidths(
      int contents_width, int separator_width, int description_width,
      int available_width, bool description_on_separate_line,
      bool allow_shrinking_contents, int* contents_max_width,
  ...
  bool OmniboxPopupModel::IsOpen() const { return view_->IsOpen(); }
  ...
  void OmniboxPopupModel::Move(int count) {
    ...
    SetSelectedLine(
        ((count < 0) && (new_line >= selected_line_)) ? 0 : new_line, false,
        false);
  }
