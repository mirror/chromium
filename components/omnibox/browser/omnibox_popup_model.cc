...
void OmniboxPopupModel::OnResultChanged() {
  ...
  if (base::FeatureList::IsEnabled(
    ...
    match_icon_task_tracker_.TryCancelAll();
    ...
    for (size_t i = 0; i < result.size(); i++) {
      ...
      MatchIcon& match_icon = match_icons_[i];

      if (AutocompleteMatch::IsSearchType(match.type)) {
        // Clear any existing icon.
        if (!match_icon.icon.IsEmpty() || !match_icon.page_url.is_empty())
          OnMatchIconFetched(i, GURL(), gfx::Image());
        continue;
      }

      // If we already show the correct favicon, skip refetching to avoid
      // hitting the favicon database and to avoid flicker.
      if (match.destination_url == match_icon.page_url)
        continue;

      edit_model_->client()->GetFaviconForPageUrl(
          &match_icon_task_tracker_, match.destination_url,
          base::Bind(&OmniboxPopupModel::OnMatchIconFetched,
...
void OmniboxPopupModel::OnMatchIconFetched(size_t match_index,
                                           const GURL& page_url,
                                           const gfx::Image& icon) {
  DCHECK_LT(match_index, match_icons_.size());

  match_icons_[match_index].page_url = page_url;
  match_icons_[match_index].icon = icon;
