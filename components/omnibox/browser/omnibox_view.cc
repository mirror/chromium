...
base::string16 OmniboxView::StripJavascriptSchemas(const base::string16& text) {
  ...
  for (size_t i = 0; i < text.size(); ++i) {
    ...
    if (!base::EqualsCaseInsensitiveASCII(text.substr(i, kJsPrefix.length()),
                                          kJsPrefix))
      break;
    trim_before = i + kJsPrefix.length();
    i = trim_before - 1;
  }
  ...
  // the scheme.
  if (trim_before) {
    while (trim_before < text.size() &&
           (base::IsUnicodeWhitespace(text[trim_before]) ||
            (text[trim_before] < 0x20)))
      ++trim_before;
    return text.substr(trim_before);
  }
