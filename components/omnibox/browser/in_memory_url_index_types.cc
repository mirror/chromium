...
String16Vector String16VectorFromString16(
  ...
  if (experiment_enabled && !break_on_space) {
    TailoredWordBreakIterator iter(cleaned_uni_string, break_mode);
    if (!iter.Init())
      return words;
    while (iter.Advance()) {
      if (break_on_space || iter.IsWord()) {
        String16VectorFromString16Internal(
            &words, word_starts, iter.GetString(), iter.prev(), break_on_space);
      }
    }
  } else {
...
String16Vector String16VectorFromString16(
  ...
  while (iter.Advance()) {
    if (iter.IsWord()) {
      String16VectorFromString16Internal(&words, word_starts, iter.GetString(),
                                         iter.prev(), false);
    }
  }
  return words;
