...
void String16VectorFromString16Internal(String16Vector& words,
                                        base::string16 word,
...
String16Vector String16VectorFromString16(
  ...
  if ((experiment_enabled || breaking_at_underscore) && !break_on_space) {
    BreakIteratorAdapter iter(cleaned_uni_string, break_mode);
    if (!iter.Init())
      return words;
    while (iter.Advance()) {
      if (break_on_space || iter.IsWord())
        String16VectorFromString16Internal(words, iter.GetString(), word_starts,
                                           iter.prev(), break_on_space);
    }
  } else {
