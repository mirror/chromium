...
ScoredHistoryMatch::ScoredHistoryMatch(
  ...
  const float topicality_score = GetTopicalityScore(
      terms_vector.size(), gurl, terms_to_word_starts_offsets, word_starts,
      adjustments);
...
float ScoredHistoryMatch::GetTopicalityScore(
    ...
    const WordStarts& terms_to_word_starts_offsets,
    const RowWordStarts& word_starts,
    const base::OffsetAdjuster::Adjustments& cleanup_adjustments) {
  ...
  const size_t query_pos = GetAdjustedOffsetForComponent(
      url, cleanup_adjustments, url::Parsed::QUERY);
  const size_t host_pos = GetAdjustedOffsetForComponent(
      url, cleanup_adjustments, url::Parsed::HOST);
  const size_t path_pos = GetAdjustedOffsetForComponent(
      url, cleanup_adjustments, url::Parsed::PATH);
    ...
    if (term_word_offset >= query_pos) {
      ...
      const size_t registry_length =
          net::registry_controlled_domains::GetRegistryLength(
              url,
              // Don't count unknown registries, so we err on match generosity.
              net::registry_controlled_domains::EXCLUDE_UNKNOWN_REGISTRIES,
              // Do not include private registries, such as appspot.com.
              net::registry_controlled_domains::EXCLUDE_PRIVATE_REGISTRIES);
      const size_t last_part_of_host_pos = path_pos - registry_length - 1;
