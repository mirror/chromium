...
float ScoredHistoryMatch::GetFinalRelevancyScore(float topicality_score,
  ...
  // - a single typed visit yesterday, no other visits -> 1.5
  //   ( = 1.5 typed visit score)
  // - a visit every three days, no typed visits -> about 7.5
  //   ( 10+ visits, averaging about 0.75 recency score each)
  // - a typed visit once a week -> about 7.5
  //   ( 10+ visits, average of 1.5 typed visit score * 0.5 recency score)
  // - a visit every day, no typed visits -> about 9.0
  //   ( 10+ visits, average about 0.9 recency score each)
  // - a typed visit every three days -> about 11
  //   ( 10+ visits, averaging about 1.5 typed visit *  0.75 recency score each)
  // - at least ten typed visits today -> 15
...
ScoredHistoryMatch::GetHQPBuckets() {
  ...
  std::string relevance_buckets_str = "0.0:550,1:625,9.0:1300,90.0:1399";
