...
MetricsServicesManager::MetricsServicesManager(
    ...
    : client_(std::move(client)),
      may_upload_(false),
      may_record_(false),
      may_record_ignoring_feature_(false) {
  DCHECK(client_);
}
...
void MetricsServicesManager::UpdatePermissions(
    bool current_may_record,
    bool current_may_record_ignoring_feature,
    bool current_may_upload) {
  DCHECK(thread_checker_.CalledOnValidThread());
  // If the user has opted out of metrics, delete local UKM state. We do not
  // currently use MetricsReporting feature for UKM.
  if (may_record_ignoring_feature_ && !current_may_record_ignoring_feature) {
  ...
  may_record_ignoring_feature_ = current_may_record_ignoring_feature;
...
void MetricsServicesManager::UpdateUkmService() {
  ...
  if (may_record_ignoring_feature_ && sync_enabled & !is_incognito) {
    ukm->EnableRecording();
    if (may_upload_)
      ukm->EnableReporting();
    else
...
void MetricsServicesManager::UpdateUploadPermissions(bool may_upload) {
  ...
  UpdatePermissions(client_->IsMetricsReportingEnabled(),
                    client_->IsMetricsReportingEnabledIgnoringFeature(),
                    may_upload);
}
