...
class ArcClipboardBridge
 ...
 private:
  ...
  void ConvertAndSendBitmapToARC(const SkBitmap& bitmap);
  void ConvertBitmapAndSendFdToARC(const SkBitmap& bitmap);

  void SendImageToARC(const std::vector<uint8_t>& png_bytes);
  void SendFileToARC(const std::string& mime_type,
                     base::Optional<base::FilePath> file_path);
  void SendHTMLToARC(const std::string& url,
                     const std::string& markup,
                     const std::string& text);
  void SendTextToARC(const std::string& clip_text);
  void SendEmptyToARC();
  ...
  uint64_t clipboard_seq_before_arc_;
