...
class ArcClipboardBridge
 ...
 private:
  // class that sets whether or not the 'copy' event was generated at Instance
  class ScopedEventAtInstanceSetter {
   public:
    explicit ScopedEventAtInstanceSetter(ArcClipboardBridge* bridge);
    ~ScopedEventAtInstanceSetter();

   private:
    // weak pointer, but lives longer than 'this'
    ArcClipboardBridge* bridge_;
  };

  static mojom::ClipRepresentationPtr CreateHTML(ui::Clipboard* clipboard);
  static mojom::ClipRepresentationPtr CreatePlainText(ui::Clipboard* clipboard);

  static void ProcessHTML(mojom::ClipRepresentation* repr,
                          ui::ScopedClipboardWriter* writer);
  static void ProcessPlainText(mojom::ClipRepresentation* repr,
                               ui::ScopedClipboardWriter* writer);

  void SetEventOriginatedAtInstance(bool originated_at_instance);
