...
bool DoesClipFitIntoInstance(const mojom::ClipDataPtr& clip_data) {
  ...
  // 1: Sending the Clip via Mojo to Instance (memory * 2 + time)
  // 2: Converting the char* (UTF8) to Java UTF16 Strings (memory * 2 again + t)
  // 3: Creating a temp Parcel with the clip data (memory * 2 again + time)
  //
  ...
  for (const auto& repr : clip_data->representations) {
    if (repr->value->is_text())
      size_at_instance_in_bytes += repr->value->get_text().size() * 2;
    else
