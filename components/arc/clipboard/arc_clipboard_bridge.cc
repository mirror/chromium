...
ArcClipboardBridge::ArcClipboardBridge(ArcBridgeService* bridge_service)
    : ArcService(bridge_service),
      ...
      clipboard_seq_before_arc_(0xffffffffffffffff) {
...
void ArcClipboardBridge::OnClipboardDataChanged() {
  ...
  if (current_seq != clipboard_seq_before_arc_ + 1) {
    ...
    mojom::ClipboardInstance* clipboard_instance = ARC_GET_INSTANCE_FOR_METHOD(
        arc_bridge_service()->clipboard(), SetClipVersion);
    if (!clipboard_instance)
      return;
    clipboard_instance->SetClipVersion(host_clip_version_,
                                       instance_clip_version_);
  } else {
...
void ArcClipboardBridge::SetHostClip(mojom::ClipDataPtr clip_data) {
  ...
  if (clip_data->content->is_html()) {
  ...
  } else if (clip_data->content->is_png_bytes()) {
    /* Deprecated */
    ...
    writer.WriteText(base::UTF8ToUTF16(clip_data->content->get_text()));
  }
}
...
void ArcClipboardBridge::GetHostClip() {
  ...
  if (clipboard->IsFormatAvailable(ui::Clipboard::GetBitmapFormatType(),
    ...
    // ConvertAndSendBitmapToARC(bitmap);
    ConvertBitmapAndSendFdToARC(bitmap);
    ...
    SendHTMLToARC(src_url, base::UTF16ToUTF8(markup), text);
...
void ArcClipboardBridge::ConvertAndSendBitmapToARC(const SkBitmap& bitmap) {
  LOG(ERROR) << "ConvertAndSendImageToARC() -- ENTER";
  ...
  base::PostTaskWithTraitsAndReplyWithResult(
      ...
      base::Bind(&ArcClipboardBridge::SendImageToARC, base::Unretained(this)));

  LOG(ERROR) << "ConvertAndSendImageToARC() -- LEAVE";
}

void ArcClipboardBridge::ConvertBitmapAndSendFdToARC(const SkBitmap& bitmap) {
  LOG(ERROR) << "ConvertBitmapAndSendFdToARC() -- ENTER";
  ...
  base::PostTaskWithTraitsAndReplyWithResult(
      ...
      base::Bind(&ArcClipboardBridge::SendFileToARC, base::Unretained(this),
                 "image/png"));

  LOG(ERROR) << "ConvertBitmapAndSendFdToARC() -- LEAVE";
}

void ArcClipboardBridge::SendFileToARC(
    const std::string& mime_type,
    base::Optional<base::FilePath> file_path) {
  LOG(ERROR) << "SendFileToARC() -- ENTER";
  ...
  if (fd == -1) {
    LOG(ERROR) << "Failed to convert stream to fd... weird";
    return;
  }
  ...
  LOG(ERROR) << "SendFileToARC() -- LEAVE";
}

void ArcClipboardBridge::SendImageToARC(const std::vector<uint8_t>& png_bytes) {
  LOG(ERROR) << "SendImageToARC() -- ENTER";
  ...
  mojom::ClipDataPtr data(mojom::ClipData::New());
  data->content = mojom::ClipDataContent::NewPngBytes(png_bytes);
  clipboard_instance->OnGetHostClip(std::move(data));

  LOG(ERROR) << "SendImageToARC() -- LEAVE";
}

void ArcClipboardBridge::SendHTMLToARC(const std::string& url,
                                       const std::string& markup,
                                       const std::string& text) {
  LOG(ERROR) << "SendHTMLToARC() -- ENTER";
  ...
  LOG(ERROR) << "SendHTMLToARC() -- LEAVE";
}

void ArcClipboardBridge::SendTextToARC(const std::string& clip_text) {
  LOG(ERROR) << "SendTextToARC() -- ENTER";
  ...
  mojom::ClipDataPtr data(mojom::ClipData::New());
  data->content = mojom::ClipDataContent::NewText(clip_text);
  data->mime_type = "text/plain";
  clipboard_instance->OnGetHostClip(std::move(data));

  LOG(ERROR) << "SendTextToARC() -- LEAVE";
}

void ArcClipboardBridge::SendEmptyToARC() {
  LOG(ERROR) << "SendEmptyToARC() -- ENTER";
  ...
  mojom::ClipDataPtr data(mojom::ClipData::New());
  data->content = mojom::ClipDataContent::NewText("<empty>");
  clipboard_instance->OnGetHostClip(std::move(data));

  LOG(ERROR) << "SendEmptyToARC() -- LEAVE";
}
