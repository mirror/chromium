...
void ArcClipboardBridge::SetClipContent(mojom::ClipDataPtr clip_data) {
  ...
  // reuse ScopedClipboardWriter so that all representations belong to the same
  // clip and don't generate more than once sequence increment
  ui::ScopedClipboardWriter writer(ui::CLIPBOARD_TYPE_COPY_PASTE);

  for (const auto& representation : clip_data->representations) {
    const std::string& mime_type(representation->mime_type);
    if (mime_type == ui::Clipboard::kMimeTypeHTML) {
      ProcessHTML(representation.get(), &writer);
    } else if (mime_type == ui::Clipboard::kMimeTypeText) {
      ProcessPlainText(representation.get(), &writer);
    }
  }
}

void ArcClipboardBridge::ProcessHTML(mojom::ClipRepresentation* representation,
                                     ui::ScopedClipboardWriter* writer) {
  std::string html(std::begin(representation->data),
                   std::end(representation->data));
  writer->WriteHTML(base::UTF8ToUTF16(html), "");
}

void ArcClipboardBridge::ProcessPlainText(
    mojom::ClipRepresentation* representation,
    ui::ScopedClipboardWriter* writer) {
  std::string text(std::begin(representation->data),
                   std::end(representation->data));
  writer->WriteText(base::UTF8ToUTF16(text));
}
...
mojom::ClipRepresentationPtr ArcClipboardBridge::CreateHTML(
  ...
  return mojom::ClipRepresentation::New(ui::Clipboard::kMimeTypeHTML, data);
}
...
mojom::ClipRepresentationPtr ArcClipboardBridge::CreatePlainText(
  ...
  return mojom::ClipRepresentation::New(ui::Clipboard::kMimeTypeText, data);
}
