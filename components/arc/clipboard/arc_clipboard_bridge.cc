...
void ArcClipboardBridge::GetClipContent(
  ...
  if (clipboard->IsFormatAvailable(ui::Clipboard::GetHtmlFormatType(),
    ...
    //                 ui::Clipboard::GetImageTextFormatType(),
    //                 ui::CLIPBOARD_TYPE_COPY_PASTE)) {
    //    SendBlob(clipboard, callback);
  } else if (clipboard->IsFormatAvailable(
                 ui::Clipboard::GetPlainTextFormatType(),
                 ui::CLIPBOARD_TYPE_COPY_PASTE)) {
    SendPlainText(clipboard, callback);
  }
}
...
void ArcClipboardBridge::SendBlob(ui::Clipboard* clipboard,
                                  const GetClipContentCallback& callback) {
  NOTIMPLEMENTED();
}

void ArcClipboardBridge::SendPlainText(ui::Clipboard* clipboard,
                                       const GetClipContentCallback& callback) {
  std::string text;
