...
void ArcClipboardBridge::GetClipContent(
  ...
  // Order is important.
  // A Bitmap could have an HTML representation as well. If so, prefer HTML.
  // Pasting "text/html" is easier than "image/png".
  // Defined order:
  //  1: HTML
  //  2: Bitmap
  //  3: Text

  // FIXME(ricardoq): Instead of sending just one representation, all possible
  // representations should be sent. And create a multi-item ClipData on the
  // Instance side
  ...
  } else if (clipboard->IsFormatAvailable(ui::Clipboard::GetBitmapFormatType(),
                                          ui::CLIPBOARD_TYPE_COPY_PASTE)) {
