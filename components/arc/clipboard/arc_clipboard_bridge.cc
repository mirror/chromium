...
void ArcClipboardBridge::SetClipContent(mojom::ClipDataPtr clip_data) {
  ...
  // TODO(ricardoq): A possible race condition could occur if a 'copy'
  // event is generated between the destructors of ScopedClipboardWriter and
  // ScopedEventAtInstanceSetter. If that is the case, Instance won't get
  // notified of that 'copy' event.
...
mojom::ClipRepresentationPtr ArcClipboardBridge::CreateHTML(
  ...
  std::string markup8(base::UTF16ToUTF8(markup16));

  // TODO(ricardoq): Report errors in a better way
  if (markup8.size() > kBinderMaxTransactionSize) {
    markup8 = "";
  }
