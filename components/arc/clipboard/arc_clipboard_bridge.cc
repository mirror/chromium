...
// Payload in an Android Binder transaction should be less than 800 Kb.
constexpr size_t kBinderMaxTransactionSize = 799 * 1024;
...
void ProcessHTML(const mojom::ClipRepresentation* repr,
  ...
  DCHECK(repr && repr->value->is_text());
  DCHECK(writer);
...
void ArcClipboardBridge::SetTextContentDeprecated(const std::string& text) {
  ...
  // Order is important. AutoReset should live longer than ScopedClipboardWriter
