...
constexpr size_t kMaxBinderParcelSize = 800 * 1024 - 512;
...
mojom::ClipRepresentationPtr CreateHTML(const ui::Clipboard* clipboard,
                                        size_t* out_size_in_parcel) {
  DCHECK(clipboard);
  DCHECK(out_size_in_parcel);
  ...
  // UTF8 strings are represented as UTF16 at Instance Parcel.
  *out_size_in_parcel += text.size() * 2;
...
void ArcClipboardBridge::GetClipContent(
  ...
  for (const auto& mime_type16 : mime_types) {
    ...
    if (mime_type == ui::Clipboard::kMimeTypeHTML) {
      clip_data->representations.push_back(
          CreateHTML(clipboard, &size_in_parcel));
    } else if (mime_type == ui::Clipboard::kMimeTypeText) {
      clip_data->representations.push_back(
          CreatePlainText(clipboard, &size_in_parcel));
    } else {
  ...
  if (size_in_parcel >= kMaxBinderParcelSize) {
