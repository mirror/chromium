...
base::Optional<ArcContainerLifetimeEvent> GetArcContainerLifetimeEvent(
  ...
  if (stop_reason != ArcStopReason::CRASH &&
      stop_reason != ArcStopReason::GENERIC_BOOT_FAILURE) {
    return base::nullopt;
  }
  ...
  if (stop_reason == ArcStopReason::GENERIC_BOOT_FAILURE) {
    return base::make_optional(
        ArcContainerLifetimeEvent::CONTAINER_FAILED_TO_START);
  }

  return base::make_optional(
      was_running ? ArcContainerLifetimeEvent::CONTAINER_CRASHED
