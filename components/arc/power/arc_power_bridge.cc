...
void ArcPowerBridge::OnAcquireDisplayWakeLock(mojom::DisplayWakeLockType type) {
  switch (type) {
    case mojom::DisplayWakeLockType::BRIGHT:
      AcquireWakeLock(ArcPowerBridge::WakeLockType::FULL_WAKE_LOCK_BRIGHT);
      return;
    case mojom::DisplayWakeLockType::DIM:
      AcquireWakeLock(ArcPowerBridge::WakeLockType::FULL_WAKE_LOCK_DIM);
      return;
  }
  NOTREACHED();
}
...
void ArcPowerBridge::OnAcquirePartialWakeLock() {
  AcquireWakeLock(ArcPowerBridge::WakeLockType::PARTIAL_WAKE_LOCK);
}

void ArcPowerBridge::OnReleaseDisplayWakeLock(mojom::DisplayWakeLockType type) {
  switch (type) {
    case mojom::DisplayWakeLockType::BRIGHT:
      ReleaseWakeLock(ArcPowerBridge::WakeLockType::FULL_WAKE_LOCK_BRIGHT);
      return;
    case mojom::DisplayWakeLockType::DIM:
      ReleaseWakeLock(ArcPowerBridge::WakeLockType::FULL_WAKE_LOCK_DIM);
      return;
  }
  NOTREACHED();
}
...
void ArcPowerBridge::OnReleasePartialWakeLock() {
  ReleaseWakeLock(ArcPowerBridge::WakeLockType::PARTIAL_WAKE_LOCK);
}
...
void ArcPowerBridge::AcquireWakeLock(WakeLockType type) {
  auto it = wake_locks_.find(type);
  if (it != wake_locks_.end()) {
    it->second.count++;
    return;
  }
  // TODO(ravisadi): Remove this check if not needed.
  ...
  WakeLockInfo new_wake_lock;
  switch (type) {
    case ArcPowerBridge::WakeLockType::FULL_WAKE_LOCK_BRIGHT:
      new_wake_lock.id = controller->AddScreenWakeLock(
          chromeos::PowerPolicyController::REASON_OTHER, kWakeLockReason);
      break;
    case ArcPowerBridge::WakeLockType::FULL_WAKE_LOCK_DIM:
      new_wake_lock.id = controller->AddDimWakeLock(
          chromeos::PowerPolicyController::REASON_OTHER, kWakeLockReason);
      break;
    case ArcPowerBridge::WakeLockType::PARTIAL_WAKE_LOCK:
      new_wake_lock.id = controller->AddSystemWakeLock(
          chromeos::PowerPolicyController::REASON_OTHER, kWakeLockReason);
      break;
  }
  new_wake_lock.count = 1;
  wake_locks_.emplace(type, new_wake_lock);
}

void ArcPowerBridge::ReleaseWakeLock(WakeLockType type) {
  if (!chromeos::PowerPolicyController::IsInitialized()) {
    LOG(WARNING) << "PowerPolicyController is not available";
    return;
  }
  ...
  DCHECK_LT(0, it->second.count);
  it->second.count--;
  if (it->second.count)
    return;
  controller->RemoveWakeLock(it->second.id);
