...
class ArcPowerBridgeTest : public testing::Test {
 public:
  ArcPowerBridgeTest() {
    ...
    instance_ = std::make_unique<ArcPowerBridge>(nullptr /* context */,
  ...
  ~ArcPowerBridgeTest() override {
    chromeos::PowerPolicyController::Shutdown();
    // fake_power_manager_client_ will be deleted when DBUSThreadManager is
    // shutdown.
...
TEST_F(ArcPowerBridgeTest, TestWakeLocksOnInstanceClosed) {
  // All wake locks should be released when the instance is closed.
  instance_->OnAcquirePartialWakeLock();
  instance_->OnAcquireDisplayWakeLock(mojom::DisplayWakeLockType::DIM);
