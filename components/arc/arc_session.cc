...
class ArcSessionImpl : public ArcSession,
 ...
 public:
  ...
  // RUNNING
  //
  // Also, EmitLoginPromptVisibleCalled() may start ARC instance with
  ...
  //
  // Start() can also be used at any of these 4 state (from NOT_STARTED to
  ...
  // but will eventually stop it. As long as you have called Start() before
  // calling Stop(), the actual stop will be notified via
  ...
  //
  // Note: When Start() hasn't been called, in other words, when the instance is
  // for login screen, Stop() will eventually change the state to NOT_STARTED,
  // rather than STOPPED. Observers' OnSessionStopped() may or may not be
  // called. It is recommended to NOT add an observer to ArcSession until you
  // call either Start() or Shutdown().
  //
 ...
 private:
  ...
  // When EmitLoginPromptVisibleCalled() is called, this flag is set. After
  // that, When Start() is called to resume the boot, the flag is unset.
  bool login_screen_instance_requested_ = false;

  // The handle EmitLoginPromptVisibleCalled() has created. The variable has a
...
void ArcSessionImpl::StopArcInstance() {
  ...
  // Synchronously change the |state_| to State::NOT_STARTED because we don't
  // really care about the shutdown progress of the stateless container. To
  // accomplish that, clear |container_instance_id_| by calling OnStopped()
  // here. Once the ID is cleared, ArcInstanceStopped() will do nothing.
  // Note that ignoring the shutdown progress is safe because on session_manager
  // side, StopArcInstance() synchronously kills the whole container process
  // group. ArcSessionImpl may send StartArcInstance D-Bus request while
  // session_manager is killing the processes, but it handles the start request
  // only after the previous container is gone.
  OnStopped(ArcStopReason::SHUTDOWN);
}
...
void ArcSessionImpl::EmitLoginPromptVisibleCalled() {
  // 'login-prompt-visible' Upstart signal starts all Upstart jobs the container
  // may depend on such as cras. Now it is safe to start the container for login
  // screen.
...
void ArcSessionImpl::OnStopped(ArcStopReason reason) {
  ...
  // If |this| object is for login screen, set the |state_| back to NOT_STARTED.
  // This, for example, allows ArcSessionRunner to Start() a session even after
  // the login screen instance crashes for some reason.
  if (login_screen_instance_requested_)
...
void ArcSessionImpl::ResetLoginScreenSession() {
  DCHECK_EQ(true, login_screen_instance_requested_);

  VLOG(2) << "Resetting the state to NOT_STARTED for future reuse";
