...
static void xxx(chromeos::SessionManagerClient::StartArcInstanceResult r, const std::string& x) {
  LOG(ERROR) << "!!! StartArcInstanceForLoginScreen done, ID=" << x;
}
...
void ArcSessionImpl::OnSocketCreated(
  ...
  session_manager_client->StartArcInstanceForLoginScreen(base::Bind(xxx));
  base::PostDelayedTask(FROM_HERE,
                        base::Bind(&ArcSessionImpl::Upgrade, weak_factory_.GetWeakPtr(), cryptohome_id, skip_boot_completed_broadcast, scan_vendor_priv_app, base::Passed(&socket_fd)),
