...
void GpuArcVideoDecodeAccelerator::ProvidePictureBuffers(
  ...
  if (secure_mode_) {
    if (secure_pixel_format_ == media::PIXEL_FORMAT_UNKNOWN)
      secure_pixel_format_ = format;
    DCHECK_EQ(secure_pixel_format_, format);
  }
...
void GpuArcVideoDecodeAccelerator::ImportBufferForPicture(
  ...
  if (secure_mode_) {
    ...
    pixel_format = secure_pixel_format_;
    ...
    if (!VerifyDmabuf(pixel_format, handle_fd.get(), planes)) {
...
bool GpuArcVideoDecodeAccelerator::VerifyDmabuf(
  ...
  for (size_t i = 0; i < planes.size(); ++i) {
    ...
    size_t rows =
        media::VideoFrame::Rows(i, pixel_format, coded_size_.height());
