...
void ProtectedBufferAllocator::Initialize(InitializeCallback callback) {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  protected_buffer_manager_ = ProtectedBufferManager::Create();
...
void ProtectedBufferAllocator::AllocateProtectedBuffer(
  ...
  case mojom::BufferType::SHARED_MEMORY:
    ...
    result = protected_buffer_manager_->AllocateProtectedSharedMemory(std::move(fd), metadata->size);
    break;
  case mojom::BufferType::NATIVE_PIXMAP:
    ...
    result = protected_buffer_manager_->AllocateProtectedNativePixmap(std::move(fd), media::VideoPixelFormatToGfxBufferFormat(metadata->pixel_format), size);
    break;
  default:
...
void ProtectedBufferAllocator::DeallocateProtectedBuffer(
  ...
  std::move(callback).Run(protected_buffer_manager_->DeallocateProtectedBuffer(std::move(fd)));
}
} // namespace arc
}
