...
// Although weak_factory_ is not used for new design, it is harmless.
// Therefore, the same destructor is used by Create() as well.
// weak_factory_ will be gotten rid of at the end of migration.
...
// Instantiate
std::weak_ptr<ProtectedBufferManager>
ProtectedBufferManager::protected_buffer_manager_;
std::mutex ProtectedBufferManager::pbm_mutex_;

// static
std::shared_ptr<ProtectedBufferManager> ProtectedBufferManager::Create() {
  std::lock_guard<std::mutex> lock(pbm_mutex_);
  if (protected_buffer_manager_.use_count() == 0) {
    auto pbm = std::make_shared<ProtectedBufferManager>();
    protected_buffer_manager_ = pbm;
    return pbm;
  } else {
    return nullptr;
  }
...
// static
ProtectedBufferManager* ProtectedBufferManager::GetInstance() {
  std::lock_guard<std::mutex> lock(pbm_mutex_);
  return protected_buffer_manager_.lock().get();
}
...
ProtectedBufferManager::AllocateProtectedSharedMemoryDeprecated(base::ScopedFD dummy_fd,
                                                                size_t size) {
  VLOGF(2) << "dummy_fd: " << dummy_fd.get() << ", size: " << size;
...
std::unique_ptr<ProtectedBufferHandle>
ProtectedBufferManager::AllocateProtectedNativePixmapDeprecated(base::ScopedFD dummy_fd,
                                                                gfx::BufferFormat format,
                                                                const gfx::Size& size) {
  VLOGF(2) << "dummy_fd: " << dummy_fd.get() << ", format: " << (int)format
           << ", size: " << size.ToString();
