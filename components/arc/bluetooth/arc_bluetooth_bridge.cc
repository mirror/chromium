...
void ArcBluetoothBridge::EnableAdapter(const EnableAdapterCallback& callback) {
  ...
  if (IsPowerChangeInitiatedByLocal(TURN_ON)) {
    DequeueLocalPowerChange(TURN_ON);
  } else {
    if (!bluetooth_adapter_->IsPowered()) {
      EnqueueRemotePowerChange(TURN_ON, callback);
      return;
    }
  }
...
void ArcBluetoothBridge::DisableAdapter(
  ...
  DCHECK(bluetooth_adapter_);
  if (IsPowerChangeInitiatedByLocal(TURN_OFF)) {
    DequeueLocalPowerChange(TURN_OFF);
  } else {
    if (bluetooth_adapter_->IsPowered()) {
      EnqueueRemotePowerChange(TURN_OFF, callback);
      return;
    }
  }
...
void ArcBluetoothBridge::SendInitialPowerChange() {
  // The default power state of Bluetooth on Android is off, so there is no need
  // to send an intent to turn off Bluetooth if the initial power state is off.
  if (bluetooth_adapter_ && bluetooth_adapter_->IsPowered())
    EnqueueLocalPowerChange(TURN_ON);
}
