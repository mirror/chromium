...
void ArcNetHostImpl::OnInstanceReady() {
  ...
  if (default_network && default_network->type() == shill::kTypeVPN &&
    ...
    GetNetworkConnectionHandler()->DisconnectNetwork(
        ...
        base::Bind(&ArcNetHostImpl::ArcVpnSuccessCallback,
                   weak_factory_.GetWeakPtr()),
        base::Bind(&ArcNetHostImpl::ArcVpnErrorCallback,
                   weak_factory_.GetWeakPtr()));
  }
}
...
void ArcNetHostImpl::DefaultNetworkChanged(
  ...
  // beleving that the current physical network is the default network.
...
void ArcNetHostImpl::ConnectArcVpn(const std::string& service_path,
                                   const std::string& guid) {
  // Note that |guid| is not provided by all callers so it should not be
  // relied on.
  VLOG(1) << "ConnectArcVpn " << service_path;
...
void ArcNetHostImpl::ArcVpnSuccessCallback() {
  VLOG(1) << "ArcVpnSuccessCallback";
}

void ArcNetHostImpl::ArcVpnErrorCallback(
    const std::string& error_name,
    std::unique_ptr<base::DictionaryValue> error_data) {
...
std::unique_ptr<base::Value> ArcNetHostImpl::TranslateStringListToValue(
  ...
  std::unique_ptr<base::Value> result(new base::Value(base::Value::Type::LIST));
  for (const auto& item : string_list) {
    result->GetList().push_back(base::Value(item));
  }
  return result;
}
