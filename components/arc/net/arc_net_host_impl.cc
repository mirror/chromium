...
void ArcNetHostImpl::OnInstanceReady() {
  ...
  if (default_network &&
      default_network->type() == shill::kTypeVPN &&
...
std::unique_ptr<base::DictionaryValue>
  ...
  top_dict->SetStringWithoutPathExpansion(onc::network_config::kName,
                                          ...
                                          cfg.app_label : cfg.session_name);
  ...
  ip_dict->SetStringWithoutPathExpansion(onc::ipconfig::kIPAddress,
                                         cfg.ipv4_gateway);
  ip_dict->SetIntegerWithoutPathExpansion(onc::ipconfig::kRoutingPrefix, 32);
...
void ArcNetHostImpl::AndroidVpnConnected(
  ...
  if (LookupArcVpnGuid(&guid)) {
    ...
    VLOG(1) << "AndroidVpnConnected: reusing " << guid << " @ "
            << service_path;
