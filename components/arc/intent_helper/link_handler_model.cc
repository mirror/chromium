...
bool LinkHandlerModel::Init(content::BrowserContext* context, const GURL& url) {
  ...
  instance->RequestUrlHandlerList(
      rewritten.spec(), base::Bind(&LinkHandlerModel::OnUrlHandlerList,
...
std::unique_ptr<LinkHandlerModel> LinkHandlerModel::Create(
    ...
    const GURL& link_url) {
  auto impl = base::MakeUnique<LinkHandlerModel>();
...
void LinkHandlerModel::OpenLinkWithHandler(const GURL& url,
                                           uint32_t handler_id) {
  auto* arc_service_manager = ArcServiceManager::Get();
  if (!arc_service_manager)
    return;
