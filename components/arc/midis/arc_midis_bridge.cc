...
void ArcMidisBridge::OnInstanceReady() {
  ...
  DVLOG(1) << "Calling Init back on other side of Midis";
  mojom::MidisManagerGetterPtr getter_proxy;
  binding_.Bind(mojo::MakeRequest(&getter_proxy));
  midis_instance->Init(std::move(getter_proxy));
  binding_.set_connection_error_handler(base::Bind(
      &mojo::Binding<MidisManagerGetter>::Close, base::Unretained(&binding_)));
}
...
void ArcMidisBridge::Connect() {
  ...
  midis_manager_getter_ptr_.Bind(
      mojo::InterfacePtrInfo<mojom::MidisManagerGetter>(std::move(server_pipe),
                                                        0u));
  DVLOG(1) << "Bound remote MidisManagerGetter interface to pipe";
  midis_manager_getter_ptr_.set_connection_error_handler(
      base::Bind(&mojo::InterfacePtr<mojom::MidisManagerGetter>::reset,
                 base::Unretained(&midis_manager_getter_ptr_)));
