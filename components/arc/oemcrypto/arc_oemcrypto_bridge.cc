...
void ArcOemCryptoBridge::OnInstanceReady() {
  VLOG(2) << "ArcOemCryptoBridge::OnInstanceReady() called";
  ...
  VLOG(2) << "Calling Init back on other side of OemCrypto";
...
void ArcOemCryptoBridge::OnBootstrapMojoConnection(
  ...
  VLOG(2) << "ArcOemCryptoBridge succeeded with Mojo bootstrapping.";
  oemcrypto_host_ptr_->Connect(std::move(request));
}

void ArcOemCryptoBridge::Connect(mojom::OemCryptoServiceRequest request) {
  VLOG(2) << "ArcOemCryptoBridge::Connect called";
  if (oemcrypto_host_ptr_.is_bound()) {
    VLOG(2) << "Re-using bootstrap connection for OemCryptoService Connect";
    oemcrypto_host_ptr_->Connect(std::move(request));
    return;
  }
  VLOG(2) << "Bootstrapping the OemCrypto connection via D-Bus";
  ...
  VLOG(2) << "Bound remote OemCryptoHost interface to pipe";
