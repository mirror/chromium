...
void FakeTimerInstance::CallCreateTimers(const std::vector<clockid_t>& clocks) {
  ...
  for (const clockid_t& clock : clocks) {
    arc::CreateTimerRequest request;
  ...
  host_ptr_->CreateTimers(
      ...
      base::BindOnce(
          [](FakeTimerInstance* instance, base::OnceClosure quit_callback,
             base::Optional<std::vector<mojom::CreateTimerResponsePtr>>
                 result) {
...
bool FakeTimerInstance::CallStartTimer(
    clockid_t clock_id,
...
bool FakeTimerInstance::IsTimerPresent(clockid_t clock_id) const {
...
void FakeTimerInstance::CreateTimersCallback(
  ...
  if (responses_size != arc_timers_.size()) {
