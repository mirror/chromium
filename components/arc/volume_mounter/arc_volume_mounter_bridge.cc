...
void RemountAll(ArcVolumeMounterBridge* bridge) {
...
void ArcVolumeMounterBridge::OnMountEvent(
  ...
  if (error_code != chromeos::MountError::MOUNT_ERROR_NONE) {
    VLOG(4) << "Mount error is not none.";
    return;
  }
  ...
  if (!volume_mounter_instance) {
    DLOG(WARNING) << "Volume Mounter Instance is not ready yet.";
    return;
  }
