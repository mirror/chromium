...
class ArcMetricsServiceTest : public testing::Test {
 public:
  ...
  ~ArcMetricsServiceTest() override {
    ...
    browser_context_dependency_manager_->DestroyBrowserContextServices(
        context_.get());
    context_.reset();
    prefs_.reset();
 ...
 protected:
  void SetArcStartTimeInMs(const uint64_t& arc_start_time_in_ms) {
  ...
  std::vector<mojom::BootProgressEventPtr> GetBootProgressEvents(
      const uint64_t& start_in_ms,
      const uint64_t& step_in_ms) {
 ...
 private:
  ...
  ArcMetricsService* metrics_service_;
...
TEST_F(ArcMetricsServiceTest, ReportBootProgress_InvalidBootType) {
  ...
  for (const std::string& suffix :
    ...
    tester.ExpectTotalCount("Arc." + std::string(kBootEvents.front()) + suffix,
                            0);
