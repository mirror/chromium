...
class ArcUsbHostUiDelegate {
 public:
  ...
  // Checks is package have access to usb device.
  ...
  // ephemeral device permission is the device is not persistent.
  virtual void DeviceRemoved(const std::string& guid) = 0;

  // Clear all pending permission requests. Called when usb host instance
  // connection closed.
