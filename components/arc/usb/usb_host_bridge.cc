...
void OnDeviceOpened(mojom::UsbHost::OpenDeviceCallback callback,
                    base::ScopedFD fd) {
  DCHECK(fd.is_valid());
...
void OnDeviceOpenError(mojom::UsbHost::OpenDeviceCallback callback,
...
using CheckedCallback =
    base::OnceCallback<void(const std::string& guid, bool success)>;
...
void OnGetDevicesComplete(
  ...
  for (const scoped_refptr<device::UsbDevice>& device : devices) {
    device->CheckUsbAccess(
        base::BindOnce(std::move(callback), device.get()->guid()));
  }
}
...
UsbHostBridge::~UsbHostBridge() {
  ...
  device::UsbService* usb_service =
      device::DeviceClient::Get()->GetUsbService();
  if (usb_service)
    usb_service->RemoveObserver(this);
}
...
void UsbHostBridge::OnInstanceReady() {
  ...
  binding_.set_connection_error_handler(base::BindOnce(
      &mojo::Binding<UsbHost>::Close, base::Unretained(&binding_)));
...
void UsbHostBridge::OnDeviceAdded(scoped_refptr<device::UsbDevice> device) {
  device->CheckUsbAccess(base::BindOnce(&UsbHostBridge::OnDeviceChecked,
                                        weak_factory_.GetWeakPtr(),
                                        device.get()->guid()));
}
