...
mojom::ArcTimerResult convertDbusResultToMojoResult(
  ...
  if (result == power_manager::ArcTimerResult::ARC_TIMER_RESULT_SUCCESS) {
...
void ArcTimerBridge::DeleteArcTimersCallback(
    power_manager::ArcTimerResult result) {
  VLOG(2) << "Received DeleteArcTimersCallback result="
          << static_cast<int32_t>(result);
}

void ArcTimerBridge::OnInstanceClosed() {
  VLOG(2) << "OnInstanceClosed";
...
void ArcTimerBridge::CreateArcTimersCallback(
    ...
    power_manager::ArcTimerResult result) {
  VLOG(2) << "Received CreateArcTimersCallback result="
          << static_cast<int32_t>(result);
  callback.Run(convertDbusResultToMojoResult(result));
}
...
void ArcTimerBridge::SetArcTimerCallback(const SetTimerCallback& callback,
                                         power_manager::ArcTimerResult result) {
  VLOG(2) << "Received SetArcTimerCallback result="
          << static_cast<int32_t>(result);
  callback.Run(convertDbusResultToMojoResult(result));
}
