...
ArcTimerBridge::CreateArcTimers(
    ...
    scoped_refptr<base::SequencedTaskRunner> timers_task_runner,
  ...
  for (auto& request : arc_timer_requests) {
    ...
    result.timers.push_back(std::unique_ptr<ArcTimer, DeleteOnSequence>(
        ...
        DeleteOnSequence(timers_task_runner)));
...
void ArcTimerBridge::OnArcTimersCreated(
  ...
  DCHECK(
      timers_and_proxies->clocks.size() == timers_and_proxies->timers.size() &&
      timers_and_proxies->clocks.size() == timers_and_proxies->proxies.size() &&
      timers_and_proxies->timers.size() == timers_and_proxies->proxies.size());
...
void ArcTimerBridge::DeleteArcTimers() {
  // The timer objects are deleted on |timers_task_runner_|.
  arc_timers_.clear();
}
