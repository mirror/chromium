...
ArcTimerBridge::ArcTimerBridge(content::BrowserContext* context,
    ...
    : timer_task_runner_(
      ...
      binding_(this),
      weak_ptr_factory_(base::in_place, this),
...
void ArcTimerBridge::CreateTimers(
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&CreateArcTimers, !arc_timers_.empty(), timer_task_runner_,
...
void ArcTimerBridge::OnConnectionError(ArcTimer* timer) {
  ...
  // This runs on |timer_task_runner_|. To safely access and delete |timer| from
  // |arc_timers_| schedule a task on the main thread. It is safe to access
  // |weak_self_| as it's a constant.
  content::BrowserThread::GetTaskRunnerForThread(content::BrowserThread::UI)
      ->PostTask(FROM_HERE, base::BindOnce(&ArcTimerBridge::DeleteArcTimer,
                                           weak_self_, timer));
}
...
struct ArcTimerBridge::TimersAndProxies {
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(TimersAndProxies);
};
...
ArcTimerBridge::CreateArcTimers(
  ...
  for (auto& request : arc_timer_requests) {
    ...
    result.timers.push_back(std::unique_ptr<ArcTimer, DeleteOnSequence>(
        new ArcTimer(std::move(expiration_fd),
                     mojo::MakeRequest(&timer_proxy_info)),
...
void ArcTimerBridge::OnArcTimersCreated(
  ...
  // Add observer for timers. This will delete the timers in case of a mojo
  // connection error in |ArcTimer|.
  for (auto& arc_timer : arc_timers_) {
    arc_timer->AddObserver(this);
  }
...
void ArcTimerBridge::DeleteArcTimer(ArcTimer* timer) {
  ...
  auto it = std::find_if(
      arc_timers_.begin(), arc_timers_.end(),
      [&](std::unique_ptr<ArcTimer, DeleteOnSequence> const& element) {
...
void ArcTimerBridge::DeleteArcTimers() {
  // Remover observer for each timer before deleting them.
  for (auto& arc_timer : arc_timers_) {
    arc_timer->RemoveObserver(this);
  }
