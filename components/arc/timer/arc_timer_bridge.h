...
class ArcTimerBridge : public KeyedService,
 ...
 public:
  ...
  // ArcTimer::Observer overrides. Runs on |timer_task_runner_|. Schedules task
  // on main thread, to delete |timer|, since it needs to access class members.
  void OnConnectionError(ArcTimer* timer) override;
 ...
 private:
  ...
  // Deletes specific |ArcTimer|.
  void DeleteArcTimer(ArcTimer* timer);
  ...
  base::Optional<base::WeakPtrFactory<ArcTimerBridge>> weak_ptr_factory_;
  // This is used in |OnConnectionError| which runs on |timer_task_runner_|.
  // To be thread safe it has to be a const.
  const base::WeakPtr<ArcTimerBridge> weak_self_;
