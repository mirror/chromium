...
ArcTimer::ArcTimer(base::ScopedFD expiration_fd,
  ...
  // It's safe to pass |this| because the error handler will be reset when the
  // |binding_| is unbound or closed.
...
void ArcTimer::Start(base::TimeTicks absolute_expiration_time,
  ...
  // It's safe to pass |this| because on destruction of this object the timer is
  // stopped and the callback can't be invoked.
  timer_.Start(
      FROM_HERE, delay,
...
void ArcTimer::Observer::OnConnectionError(ArcTimer* timer) {}

void ArcTimer::AddObserver(Observer* observer) {
  observers_.AddObserver(observer);
}

void ArcTimer::RemoveObserver(Observer* observer) {
...
void ArcTimer::HandleConnectionError() {
  ...
  for (auto& observer : observers_) {
    observer.OnConnectionError(this);
  }
}
...
void ArcTimer::OnExpiration() {
  ...
  // timerfd expiration. The timerfd API expects this to be the number of
