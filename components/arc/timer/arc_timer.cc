...
// The delay used to fire an alarm when it's absolute expiration time is already
// past.
int64_t kMinTimerDelayNs = 1;
...
void ArcTimer::Start(base::TimeTicks absolute_expiration_time,
  ...
  if (absolute_expiration_time <= base::TimeTicks::Now()) {
    delay = base::TimeDelta::FromMillisecondsD(
        (double)kMinTimerDelayNs / (base::Time::kNanosecondsPerMicrosecond *
                                    base::Time::kMicrosecondsPerMillisecond));
  } else {
    delay = base::TimeDelta(absolute_expiration_time - base::TimeTicks::Now());
  }
