...
class ArcTimerStore {
 public:
  ...
  bool GetTimerReadFd(clockid_t clock_id, int* read_fd) {
    ...
    *read_fd = arc_timers_[clock_id].read_fd.get();
    return true;
  }
  ...
  bool SetTimerProxy(clockid_t clock_id, mojom::TimerPtrInfo timer) {
    ...
    if (arc_timers_.find(clock_id) == arc_timers_.end()) {
      return false;
    }
    auto& arc_timer_info = arc_timers_[clock_id];
    arc_timer_info.timer = mojom::TimerPtr(std::move(timer));
    return true;
  }
...
void CreateTimersCallbackHelper(
...
void CreateTimersCallback(
    ...
    base::Optional<std::vector<mojom::CreateTimerResponsePtr>> result) {
  CreateTimersCallbackHelper(arc_timer_store, std::move(result));
...
bool WaitForExpiration(clockid_t clock_id, ArcTimerStore* arc_timer_store) {
  ...
  // socket pair.
  int timer_read_fd;
  EXPECT_TRUE(arc_timer_store->GetTimerReadFd(clock_id, &timer_read_fd));
...
TEST_F(ArcTimerTest, CreateTimers) {
...
TEST_F(ArcTimerTest, StartTimer) {
  ...
  if (!timer) {
    ADD_FAILURE() << "Timer entry not found";
    return;
  }
  if (!*timer) {
