...
class ArcTimerStore {
 public:
  ...
  base::Optional<mojom::TimerPtr*> GetTimerProxy(clockid_t clock_id) {
    if (!IsTimerPresent(clock_id))
      return base::nullopt;
    return base::Optional<mojom::TimerPtr*>(&arc_timers_[clock_id].timer);
  }

  bool IsTimerPresent(clockid_t clock_id) {
...
// Stores timer proxies returned by |mojom::TimerHost::CreateTimers|.
void CreateTimersCallback(
  ...
  base::ScopedClosureRunner quit_runner =
      base::ScopedClosureRunner(std::move(quit_callback));
...
bool CreateTimersTest(const std::vector<clockid_t>& clocks,
                      ArcTimerStore* arc_timer_store,
  ...
  timer_instance->CallCreateTimers(
      ...
      base::BindOnce(&CreateTimersCallback, arc_timer_store,
                     loop.QuitClosure()));
  loop.Run();
  // Check if timer for each clock got created successfully.
  for (const clockid_t& clock : clocks)
    EXPECT_TRUE(arc_timer_store->IsTimerPresent(clock));
  return true;
}
...
bool WaitForExpiration(clockid_t clock_id, ArcTimerStore* arc_timer_store) {
  if (!arc_timer_store->IsTimerPresent(clock_id)) {
  ...
  // socket pair.
  base::Optional<int> fd = arc_timer_store->GetTimerReadFd(clock_id);
  EXPECT_NE(fd, base::nullopt);
  int timer_read_fd = fd.value();
...
TEST_F(ArcTimerTest, StartTimer) {
  ...
  EXPECT_TRUE(
      CreateTimersTest(clocks, &arc_timer_store, GetFakeTimerInstance()));
  base::Optional<mojom::TimerPtr*> timer_opt =
      arc_timer_store.GetTimerProxy(CLOCK_BOOTTIME_ALARM);
  EXPECT_NE(timer_opt, base::nullopt);
  mojom::TimerPtr* timer = timer_opt.value();
  if (!*timer) {
    ADD_FAILURE() << "Timer proxy not set";
    return;
  }
