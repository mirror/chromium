...
class ArcTimer : public mojom::Timer {
 ...
 private:
  ...
  // Callback fired when the timer in |ArcTimer| expires. Runs on the main
  // thread.
  void OnExpiration(scoped_refptr<RefCountedFd> expiration_fd);
  ...
  // Task runner for writing to |expiration_fd_| when |timer_| expires. This is
  // required because I/O operations are not allowed on the main thread.
  scoped_refptr<base::SequencedTaskRunner> expiration_io_task_runner_;

  // The timer that will be scheduled. Only accessed from the main thread.
