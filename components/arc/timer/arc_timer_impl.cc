...
void ArcTimer::ExpirationCallback() {
  ...
  // Schedule a task to indicate timer expiry to the instance. For low latency
  // use |USER_BLOCKING| trait.
  base::PostTaskWithTraitsAndReply(
      FROM_HERE, {base::MayBlock(), base::TaskPriority::USER_BLOCKING},
      base::Bind(&ArcTimer::IndicateExpiration, weak_factory_.GetWeakPtr()),
      base::Bind(&ArcTimer::LogExpirationIndicationStatus,
                 weak_factory_.GetWeakPtr()));
}

void ArcTimer::IndicateExpiration() {
...
void ArcTimer::LogExpirationIndicationStatus() {
  DVLOG(1) << "Write completion";
}
