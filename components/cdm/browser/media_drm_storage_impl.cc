...
// Create sessions dict for |origin|. It will create a origin dict if it doesn't
// exist.
base::DictionaryValue* CreateSessionsDict(
    base::DictionaryValue* storage_dict,
    const std::string& origin,
    const base::UnguessableToken& origin_id) {
  DCHECK(storage_dict);

  base::Value* origin_value =
      storage_dict->FindKeyOfType(origin, base::Value::Type::DICTIONARY);
  base::DictionaryValue* origin_dict = nullptr;
  if (origin_value) {
    origin_value->GetAsDictionary(&origin_dict);
  } else {
    origin_dict = storage_dict->SetDictionaryWithoutPathExpansion(
        origin, OriginData(origin_id).ToDictValue());
  }

  DCHECK(origin_dict);
  return origin_dict->SetDictionary(kSessions,
...
void MediaDrmStorageImpl::OnProvisioned(OnProvisionedCallback callback) {
  ...
  CreateSessionsDict(storage_dict, origin_string_, origin_id_);
