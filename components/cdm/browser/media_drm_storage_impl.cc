...
// Extract base::Time from |dict|. Returns true if |dict| contains a valid time
// value.
...
class OriginData {
 ...
 private:
  OriginData(base::UnguessableToken origin_id, base::Time time)
...
class SessionData {
 public:
  ...
  const std::vector<uint8_t>& key_set_id() const { return key_set_id_; }
...
void MediaDrmStorageImpl::SavePersistentSession(
  ...
  if (!origin_dict->GetDictionary(kSessions, &sessions_dict)) {
    DVLOG(1) << __func__ << ": No session exists; creating a new dict.";
...
void MediaDrmStorageImpl::LoadPersistentSession(
  ...
  const base::DictionaryValue* storage_dict =
      pref_service_->GetDictionary(kMediaDrmStorage);
  if (!storage_dict) {
    DVLOG(1) << __func__ << ": No storage dict for origin " << origin_string_;
    std::move(callback).Run(nullptr);
    return;
  }
