...
static SupportedCodecs GetSupportedCodecs(
  ...
  for (const auto& info : kVideoCodecsToQuery) {
    if ((request.codecs & info.eme_codec) &&
        ...
        media::MediaCodecUtil::CanDecode(info.codec, is_hw_secure)) {
...
CdmMessageFilterAndroid::CdmMessageFilterAndroid(bool can_use_hw_secure_codecs)
    : BrowserMessageFilter(EncryptedMediaMsgStart),
      can_use_hw_secure_codecs_(can_use_hw_secure_codecs) {}
...
void CdmMessageFilterAndroid::OnQueryKeySystemSupport(
  ...
  response->non_hw_secure_codecs = GetSupportedCodecs(request, false);
  if (can_use_hw_secure_codecs_)
    response->hw_secure_codecs = GetSupportedCodecs(request, true);
