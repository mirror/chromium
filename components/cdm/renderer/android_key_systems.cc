...
SupportedKeySystemResponse QueryKeySystemSupport(
  ...
  DCHECK(!(response.non_hw_secure_codecs & ~media::EME_CODEC_ALL))
      << "unrecognized codec";
  DCHECK(!(response.hw_secure_codecs & ~media::EME_CODEC_ALL))
      << "unrecognized codec";
  return response;
}
...
void AddAndroidWidevine(
  ...
  if (response.non_hw_secure_codecs != media::EME_CODEC_NONE) {
    DVLOG(3) << __func__ << " Widevine supported.";
    concrete_key_systems->emplace_back(new WidevineKeySystemProperties(
        response.non_hw_secure_codecs,         // Regular codecs.
        response.hw_secure_codecs,             // Hardware-secure codecs.
    ...
    DCHECK(response.hw_secure_codecs == media::EME_CODEC_NONE);
  }
}
  ...
  for (std::vector<std::string>::const_iterator it = key_system_names.begin();
    ...
    if (response.non_hw_secure_codecs != media::EME_CODEC_NONE) {
      concrete_key_systems->emplace_back(new AndroidPlatformKeySystemProperties(
          *it, response.non_hw_secure_codecs));
    }
  }
}
