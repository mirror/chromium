...
struct NotificationInstructions {
  // DeviceSyncActionType is FORCE_ENROLLMENT_NOW
  NotificationInstructions(bool enrollment_success)
  ...
  NotificationInstructions(SyncResult synced, DeviceChangeResult changed)
...
class DeviceSyncImpltest : public service_manager::test::ServiceTest {
 public:
  ...
  void VerifyNoEnrollmentSinceLastCheck() {
    EXPECT_EQ(fake_enrollment_manager_->GetAndClearInvocationReason(),
              InvocationReason::INVOCATION_REASON_UNKNOWN);
  }

  void VerifyNoSyncSinceLastCheck() {
  ...
  void ForceManagerAction(NotificationInstructions instructions) {
...
TEST_F(DeviceSyncImpltest, TestValidEnrollmentOnInit) {
  ...
  VerifyNoEnrollmentSinceLastCheck();
  VerifyNoSyncSinceLastCheck();
...
TEST_F(DeviceSyncImpltest, TestForceEnrollment) {
  ...
  // Testing ForceEnrollment succeeded, notifying Observers of success.
  ForceManagerAction(NotificationInstructions(true /* enrollment_success */));
  ...
  // Testing ForceEnrollment failed, notifying Observers of failure.
  ForceManagerAction(NotificationInstructions(false /* enrollment_success */));
  ConfirmObserversNotified(DeviceSyncActionType::FORCE_ENROLLMENT_NOW,
                           1 /* num_expected_success_callbacks */
                           ,
...
TEST_F(DeviceSyncImpltest, TestForceSync) {
  ...
  ForceManagerAction(NotificationInstructions(SyncResult::SUCCESS,
  ...
  ForceManagerAction(NotificationInstructions(SyncResult::FAILURE,
                                              DeviceChangeResult::CHANGED));
  ConfirmObserversNotified(DeviceSyncActionType::FORCE_SYNC_NOW,
                           1 /* num_expected_success_callbacks */
                           ,
  ...
  // Testing successful sync on ForceSync still works
  ForceManagerAction(NotificationInstructions(SyncResult::SUCCESS,
                                              DeviceChangeResult::CHANGED));
  ConfirmObserversNotified(DeviceSyncActionType::FORCE_SYNC_NOW,
...
TEST_F(DeviceSyncImpltest, TestGetSyncedDevices) {
  ...
  expected_synced_devices = cryptauth::GenerateTestRemoteDevices(10);
  fake_remote_device_factory_->last_instance()->set_synced_remote_devices(
      expected_synced_devices);
  device_sync_->GetSyncedDevices(
      base::Bind(&DeviceSyncImpltest::VerifySyncedDevices,
