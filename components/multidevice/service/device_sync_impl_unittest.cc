...
class FakeIdentityManager : public identity::mojom::IdentityManager {
 public:
  ...
  void GetPrimaryAccountWhenAvailable(
    ...
    // TODO(hsuregan): Address case in which identity manager hangs forever.
    // If someone calls ForceEnrollNow or ForceSyncNow in that event, should
    // this cause DeviceSync to crash OR cause DeviceSync to reattempt to query
    // for account info?  For now, cause DeviceSync to crash via DCHECK (client
    // should handle error in that case).
    PrimaryAccountInfoAvailable();
  }

  void PrimaryAccountInfoAvailable() {
    info_.account_id = kAccountId;
    std::move(callback_).Run(info_, identity::AccountState());
  }
  ...
  class DeviceSyncObserverImpl : public device_sync::mojom::DeviceSyncObserver {
   public:
    ...
    void ClearNotificationCount() {
      num_times_enrollment_finished_called_ = ObserverCallbackCount();
      num_times_device_synced_ = ObserverCallbackCount();
    }
  ...
  void ConfirmObserversNotified(DeviceSyncActionType action_type,
    ...
    for (auto& observer : observers_) {
      if (action_type == DeviceSyncActionType::FORCE_ENROLLMENT_NOW) {
        EXPECT_EQ(observer->GetNumCallsEnrollment(success_count), count);
      } else if (action_type == DeviceSyncActionType::FORCE_SYNC_NOW) {
        EXPECT_EQ(observer->GetNumCallsSync(success_count), count);
      }
    }
  }
...
TEST_F(DeviceSyncImpltest, TestValidEnrollmentOnInit) {
  ...
  ConfirmObserversNotified(DeviceSyncActionType::FORCE_ENROLLMENT_NOW,
                           true /* success_count */, 0 /* count */);
  ConfirmObserversNotified(DeviceSyncActionType::FORCE_ENROLLMENT_NOW,
                           false /* success_count */, 0 /* count */);
}
...
TEST_F(DeviceSyncImpltest, TestForceEnrollment) {
  ...
  fake_enrollment_manager_->SetIsEnrollmentValid(false /* status */);
  DeviceSyncActionType(DeviceSyncActionType::FORCE_ENROLLMENT_NOW);
  ConfirmObserversNotified(DeviceSyncActionType::FORCE_ENROLLMENT_NOW,
                           true /*success_count*/, 0 /* count */);
  ConfirmObserversNotified(DeviceSyncActionType::FORCE_ENROLLMENT_NOW,
                           false /*success_count*/, 1 /* count */);
  EXPECT_EQ(fake_enrollment_manager_->GetAndClearInvocationReason(),
            InvocationReason::INVOCATION_REASON_MANUAL);
  ClearAllObserverNotificationCounts();
}
...
TEST_F(DeviceSyncImpltest, TestForceSync) {
  ...
  fake_device_manager_->SetExpectedChangeResult(ChangeResult::CHANGED);
  ...
  fake_device_manager_->SetExpectedSyncResult(SyncResult::FAILURE);
  fake_device_manager_->SetExpectedChangeResult(ChangeResult::UNCHANGED);
  DeviceSyncActionType(DeviceSyncActionType::FORCE_SYNC_NOW);
  ConfirmObserversNotified(DeviceSyncActionType::FORCE_SYNC_NOW,
                           true /*success_count*/, 1 /* count */);
  ConfirmObserversNotified(DeviceSyncActionType::FORCE_SYNC_NOW,
                           false /* success_count */, 2 /* count */);
  EXPECT_EQ(fake_device_manager_->GetAndClearInvocationReason(),
            InvocationReason::INVOCATION_REASON_MANUAL);
