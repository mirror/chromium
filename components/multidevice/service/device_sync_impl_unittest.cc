...
CreateIneligibleDeviceInfosForExternalDeviceInfos(int nDevices, int nReasons) {
  ...
  for (int i = 0; i < nDevices; i++) {
    ...
    ineligible_devices[i].mutable_device()->set_public_key("publicKey" +
                                                           std::to_string(i));
    for (int j = 0; j < nReasons; j++) {
      ineligible_devices[i].add_reasons("reason" + std::to_string(j));
    }
  }
...
TEST_F(DeviceSyncImpltest, TestFindEligibleDevicesForCapability) {
  ...
  std::vector<cryptauth::IneligibleDevice> expected_ineligible_devices =
      CreateIneligibleDeviceInfosForExternalDeviceInfos(3 /* nDevices */,
                                                        4 /* nReasons */);
