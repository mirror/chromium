...
DeviceSyncImpl::Factory::NewInstance(
  ...
  return factory_instance_->BuildInstance(
      ...
      enrollment_manager, remote_device_provider,
...
DeviceSyncImpl::DeviceSyncImpl(
    ...
    : secure_message_delegate_factory_(secure_message_delegate_factory),
      ...
      remote_device_provider_raw_(remote_device_provider),
...
void DeviceSyncImpl::Bind(device_sync::mojom::DeviceSyncRequest request) {
...
void DeviceSyncImpl::GetSyncedDevices(GetSyncedDevicesCallback callback) {
  std::move(callback).Run(GetRemoteDeviceProvider()->GetSyncedDevices());
}
...
void DeviceSyncImpl::OnSyncFinished(
  ...
  LOG(ERROR) << "OnSyncfinishedCalled";
...
cryptauth::RemoteDeviceProvider* DeviceSyncImpl::GetRemoteDeviceProvider() {
  if (remote_device_provider_) {
    DCHECK(!remote_device_provider_raw_);
    return remote_device_provider_.get();
  }

  DCHECK(remote_device_provider_raw_);
  return remote_device_provider_raw_;
}
...
void DeviceSyncImpl::CreateManagers() {
  ...
  enrollment_manager_ = base::MakeUnique<cryptauth::CryptAuthEnrollmentManager>(
      ...
      GetGcmDeviceInfo(), gcm_manager_, pref_service_.get());
}
