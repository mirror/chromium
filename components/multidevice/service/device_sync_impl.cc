...
void DeviceSyncImpl::OnEnrollmentFinished(bool success) {
  ...
  if (state_ == State::FIRST_ENROLL && success)
...
void DeviceSyncImpl::SetStateFirstEnrollAndEnroll() {
  state_ = State::FIRST_ENROLL;
...
void DeviceSyncImpl::CreateManagers() {
  ...
  pref_service_ = PrefServiceFactory().Create(registry_);
