...
DeviceSyncImpl::Factory::NewInstance(
  ...
  return factory_instance_->BuildInstance(
      std::move(service_ref), identity_manager, gcm_manager, device_manager,
      enrollment_manager, secure_message_delegate_factory, gcm_device_info);
}
...
DeviceSyncImpl::DeviceSyncImpl(
    ...
    : secure_message_delegate_factory_(secure_message_delegate_factory),
      ...
      identity_manager_(identity_manager),
      gcm_manager_(gcm_manager),
      enrollment_manager_raw_(enrollment_manager),
      device_manager_raw_(device_manager),
      gcm_device_info_(gcm_device_info),
...
void DeviceSyncImpl::SetCapabilityEnabled(
  ...
  device_capability_manager_->SetCapabilityEnabled(
      ...
      base::Bind(&DeviceSyncImpl::CapabilityEnabledCallbackWrapper,
...
void DeviceSyncImpl::FindEligibleDevicesForCapability(
  ...
  device_capability_manager_->FindEligibleDevicesForCapability(
      capability,
      base::Bind(
          &DeviceSyncImpl::SuccessEligibleDevicesForCapabilityCallbackWrapper,
          weak_ptr_factory_.GetWeakPtr()),
      base::Bind(
          &DeviceSyncImpl::ErrorEligibleDevicesForCapabilityCallbackWrapper,
          weak_ptr_factory_.GetWeakPtr()));
}
...
void DeviceSyncImpl::IsCapabilityPromotable(
  ...
  device_capability_manager_->IsCapabilityPromotable(
      ...
      base::Bind(&DeviceSyncImpl::SuccessCapabilityPromotableCallbackWrapper,
                 weak_ptr_factory_.GetWeakPtr()),
      base::Bind(&DeviceSyncImpl::ErrorCapabilityPromotableCallbackWrapper,
                 weak_ptr_factory_.GetWeakPtr()));
}
...
void DeviceSyncImpl::OnSyncDeviceListChanged() {
  observers_.ForAllPtrs([](device_sync::mojom::DeviceSyncObserver* observer) {
    observer->OnDevicesSynced(true /* sync_result */,
                              true /* device_change_result */);
  });
}
...
void DeviceSyncImpl::CreateManagers() {
  device_manager_ = base::MakeUnique<cryptauth::CryptAuthDeviceManager>(
      ...
      gcm_manager_, pref_service_.get());
  ...
  enrollment_manager_ = base::MakeUnique<cryptauth::CryptAuthEnrollmentManager>(
      ...
      gcm_device_info_, gcm_manager_, pref_service_.get());
}
...
void DeviceSyncImpl::CapabilityEnabledCallbackWrapper(
    const std::string& response) {
  DCHECK(set_capability_enabled_once_callback_);
...
void DeviceSyncImpl::SuccessEligibleDevicesForCapabilityCallbackWrapper(
...
void DeviceSyncImpl::ErrorEligibleDevicesForCapabilityCallbackWrapper(
...
void DeviceSyncImpl::SuccessCapabilityPromotableCallbackWrapper(
    bool is_promotable) {
...
void DeviceSyncImpl::ErrorCapabilityPromotableCallbackWrapper(
