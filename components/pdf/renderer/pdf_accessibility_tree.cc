  ...
  for (size_t i = 0; i < text_runs.size(); ++i) {
    ...
    if (!para_node) {
      ...
      if (heading_font_size_threshold > 0 &&
        ...
        para_node->AddIntAttribute(ax::mojom::IntAttribute::HIERARCHICAL_LEVEL,
                                   2);
        para_node->AddStringAttribute(ax::mojom::StringAttribute::HTML_TAG,
                                      "h2");
      }
    ...
    ui::AXNodeData* inline_text_box_node =
        CreateNode(ax::mojom::Role::INLINE_TEXT_BOX);
    static_text_node->child_ids.push_back(inline_text_box_node->id);

    inline_text_box_node->AddStringAttribute(ax::mojom::StringAttribute::NAME,
                                             chars_utf8);
    ...
    inline_text_box_node->AddIntListAttribute(
        ax::mojom::IntListAttribute::CHARACTER_OFFSETS, char_offsets);
    ...
    if (i == text_runs.size() - 1) {
      static_text_node->AddStringAttribute(ax::mojom::StringAttribute::NAME,
                                           static_text);
      break;
    }
...
ui::AXNodeData* PdfAccessibilityTree::CreateNode(ax::mojom::Role role) {
  ...
  node->role = role;
  node->SetRestriction(ax::mojom::Restriction::READ_ONLY);
