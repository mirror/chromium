...
void PdfAccessibilityTree::SetAccessibilityDocInfo(
  ...
  doc_info_ = doc_info;
  doc_node_ = CreateNode(ax::mojom::Role::GROUP);
...
void PdfAccessibilityTree::SetAccessibilityPageInfo(
  ...
  ui::AXNodeData* page_node = CreateNode(ax::mojom::Role::REGION);
  ...
  for (size_t i = 0; i < text_runs.size(); ++i) {
    ...
    if (!para_node) {
      para_node = CreateNode(ax::mojom::Role::PARAGRAPH);
      page_node->child_ids.push_back(para_node->id);

      if (heading_font_size_threshold > 0 &&
          text_run.font_size > heading_font_size_threshold) {
        para_node->role = ax::mojom::Role::HEADING;
        para_node->AddIntAttribute(ax::mojom::IntAttribute::HIERARCHICAL_LEVEL, 2);
        para_node->AddStringAttribute(ax::mojom::StringAttribute::HTML_TAG, "h2");
      }

      // This node is for the text inside the paragraph, it includes
      // the text of all of the text runs.
      static_text_node = CreateNode(ax::mojom::Role::STATIC_TEXT);
    ...
    ui::AXNodeData* inline_text_box_node = CreateNode(
        ax::mojom::Role::INLINE_TEXT_BOX);
    static_text_node->child_ids.push_back(inline_text_box_node->id);

    inline_text_box_node->AddStringAttribute(ax::mojom::StringAttribute::NAME, chars_utf8);
    ...
    inline_text_box_node->AddIntListAttribute(ax::mojom::IntListAttribute::CHARACTER_OFFSETS,
                                              char_offsets);
    ...
    if (i == text_runs.size() - 1) {
      static_text_node->AddStringAttribute(ax::mojom::StringAttribute::NAME, static_text);
      break;
    }
...
ui::AXNodeData* PdfAccessibilityTree::CreateNode(ax::mojom::Role role) {
  ...
  node->role = role;
  node->AddIntAttribute(ax::mojom::IntAttribute::RESTRICTION,
