...
class PdfListenerImpl : public mojom::PdfListener {
 public:
  PdfListenerImpl(mojom::PdfListenerRequest request, PepperPDFHost* host)
      : binding_(this, std::move(request)), host_(host) {}
  ~PdfListenerImpl() override {}

  // mojom::PdfListener
  void SetSelectionLeftCoordinates(const gfx::PointF& point) override {
    host_->SetSelectionLeftCoordinates(point);
  }
  void SetSelectionRightCoordinates(const gfx::PointF& point) override {
    host_->SetSelectionRightCoordinates(point);
  }
  void SetSelectionCoordinates(const gfx::PointF& left,
                               const gfx::PointF& right) override {
    host_->SetSelectionCoordinates(left, right);
  }
...
PepperPDFHost::PepperPDFHost(content::RendererPpapiHost* host,
  ...
  listener_impl_ =
      base::MakeUnique<PdfListenerImpl>(mojo::MakeRequest(&listener), this);
  service->AddListener(std::move(listener));
}
...
void PepperPDFHost::SetSelectionLeftCoordinates(const gfx::PointF& pt) {
  content::PepperPluginInstance* instance =
      host_->GetPluginInstance(pp_instance());
  instance->SetSelectionLeftCoordinates(pt);
}

void PepperPDFHost::SetSelectionRightCoordinates(const gfx::PointF& pt) {
  content::PepperPluginInstance* instance =
      host_->GetPluginInstance(pp_instance());
  instance->SetSelectionRightCoordinates(pt);
}

void PepperPDFHost::SetSelectionCoordinates(const gfx::PointF& left,
                                            const gfx::PointF& right) {
  content::PepperPluginInstance* instance =
      host_->GetPluginInstance(pp_instance());
  instance->SetSelectionCoordinates(left, right);
}
