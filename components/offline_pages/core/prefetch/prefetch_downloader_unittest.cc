...
class PrefetchDownloaderTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    auto downloader = base::MakeUnique<PrefetchDownloader>(kTestChannel);
  ...
  const std::vector<TestPrefetchDispatcher::DownloadResult>&
  completed_downloads() const {
    return test_prefetch_dispatcher_->completed_downloads_;
  }
