...
TEST_F(PrefetchNetworkRequestFactoryTest, GetOperationRequestDoneUMA) {
  ...
  // Have the test ssytem callb ack into GetOperationRequestDone with an error
  // code that weill cause the "sholulld_retry_without_backoff" response.
  RespondWithHttpError(net::HTTP_NOT_IMPLEMENTED);

  // TODO: Check that operation names have been cleaned up.
  // generate_page_bundle_requests.
  EXPECT_FALSE(request_factory()->HasOutstandingRequests());
  ...
  histogram_tester.ExpectUniqueSample(
      "OfflinePages.Prefetching.OfflinePageServiceGetOperationStatus",
...
TEST_F(PrefetchNetworkRequestFactoryTest, GeneratePageBundleRequestDoneUMA) {
  ...
  // method with an error code that will produce SHOULD_SUSPEND.
  ...
  // Ensure that the status was recorded in UMA.
  histogram_tester.ExpectUniqueSample(
      "OfflinePages.Prefetching.OfflinePageServiceGetPageBundleStatus",
