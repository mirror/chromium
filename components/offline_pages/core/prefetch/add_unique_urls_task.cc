...
AddUniqueUrlsTask::Result AddUrlsAndCleanupZombiesSync(
    ...
    sql::Connection* db) {
  if (!CheckDb(db))
    return AddUniqueUrlsTask::Result::ERROR;
  ...
  for (auto prefetch_url : candidate_prefetch_urls) {
    ...
    if (iter == existing_items.end()) {
      if (!CreatePrefetchItemSync(db, name_space, prefetch_url))
        return AddUniqueUrlsTask::Result::ERROR;  // Transaction rollback.
      else
  ...
  for (auto existing_item : existing_items) {
    // Skips all the entries that are not ZOMBIES.
