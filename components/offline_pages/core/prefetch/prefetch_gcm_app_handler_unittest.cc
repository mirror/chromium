...
class PrefetchGCMAppHandlerTest : public testing::Test {
 public:
  PrefetchGCMAppHandlerTest() {
    auto dispatcher = base::MakeUnique<PrefetchDispatcherStub>();
    auto gcm_app_handler =
        base::MakeUnique<PrefetchGCMAppHandler>(dispatcher.get());

    dispatcher_stub_ = dispatcher.get();
  ...
  PrefetchDispatcherStub* dispatcher() { return dispatcher_stub_; }
  PrefetchGCMAppHandler* handler() { return handler_; }

 private:
  PrefetchServiceStubTaco prefetch_service_taco_;
  // Owned by the taco.
  PrefetchDispatcherStub* dispatcher_stub_;
  // Owned by the taco.
