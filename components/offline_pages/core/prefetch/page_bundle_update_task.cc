...
void MarkUrlFailedSync(sql::Connection* db,
  ...
  statement.BindInt(1,
                    static_cast<int>(PrefetchItemErrorCode::ARCHIVING_FAILED));
...
PageBundleUpdateResult UpdateWithOperationResults(
    const std::string operation_name,
...
void PageBundleUpdateTask::Run() {
  store_->Execute(
      base::BindOnce(&UpdateWithOperationResults, operation_name_, pages_),
      base::BindOnce(&PageBundleUpdateTask::FinishedWork,
                     weak_factory_.GetWeakPtr()));
}

void PageBundleUpdateTask::FinishedWork(PageBundleUpdateResult result) {
  if (result)
