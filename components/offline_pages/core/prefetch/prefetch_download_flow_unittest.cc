...
TEST_F(PrefetchDownloadFlowTest, DownloadServiceReadyAfterPrefetchSystemReady) {
  // Download service is not ready initially.
  EXPECT_FALSE(prefetch_downloader()->IsDownloadServiceReady());

  // Create something to hold onto the background task.
  PrefetchURL prefetch_url("id", GURL("https://www.chromium.org"),
                           base::string16());
  prefetch_dispatcher()->AddCandidatePrefetchURLs(
      "Foo", std::vector<PrefetchURL>(1, prefetch_url));
  RunUntilIdle();
  ...
  // The item should not be changed.
  ...
  EXPECT_TRUE(prefetch_downloader()->IsDownloadServiceReady());
  RunUntilIdle();
...
TEST_F(PrefetchDownloadFlowTest, DelayRunningDownloadCleanupTask) {
  ...
  prefetch_downloader()->OnDownloadServiceReady(outgoing_downloads,
                                                success_downloads);
  EXPECT_TRUE(prefetch_downloader()->IsDownloadServiceReady());
  RunUntilIdle();
