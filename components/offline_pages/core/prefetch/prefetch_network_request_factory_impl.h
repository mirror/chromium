...
class PrefetchNetworkRequestFactoryImpl : public PrefetchNetworkRequestFactory {
 public:
  ...
  GeneratePageBundleRequest* CurrentGeneratePageBundleRequest() override;
  void CancelCurrentGeneratePageBundleRequest() override;
  ...
  void CancelGetOperationRequestByName(
      const std::string& operation_name) override;
 ...
 private:
  ...
  std::unique_ptr<GeneratePageBundleRequest>
      current_generate_page_bundle_request_;
  std::map<std::string, std::unique_ptr<GetOperationRequest>>
      current_get_operation_requests_;
