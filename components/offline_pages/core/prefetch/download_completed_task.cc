...
// Mirrors the FinishedDownloadStatus metrics enum. Updates here should be
// reflected there and vice versa. New values should be appended and existing
// values never deleted.
enum class ArchiveDownloadStatus {
  SUCCESFUL,
  FAILED,
  MAX  // Must always be the last element.
};
...
void DownloadCompletedTask::OnPrefetchItemUpdated(bool successful_download,
  ...
  if (row_was_updated) {
    ...
    ArchiveDownloadStatus status =
        (successful_download ? ArchiveDownloadStatus::SUCCESFUL
                             : ArchiveDownloadStatus::FAILED);
    UMA_HISTOGRAM_ENUMERATION(
        "OfflinePages.Prefetching.NoItemForFinishedDownload", status,
        ArchiveDownloadStatus::MAX);
  }
