...
typedef DownloadArchivesTask::DownloadItem DownloadItem;
typedef DownloadArchivesTask::ItemsToDownload ItemsToDownload;
...
std::unique_ptr<ItemsToDownload> SelectAndMarkItemsForDownloadSync(
  ...
  for (const auto& ready_item : ready_items) {
    ...
    if (!MarkItemAsDownloading(db, ready_item.offline_id, guid))
      return nullptr;
    items_to_download->push_back(ready_item);
    items_to_download->back().guid = guid;
...
void DownloadArchivesTask::SendItemsToPrefetchDownloader(
  ...
  if (items_to_download) {
    for (const auto& download_item : *items_to_download) {
      ...
      // Reports expected archive size in KB (accepting values up to 100 MB).
