...
// and kept in ascending order. Deprecated values should be labeled as such but
// never removed. Assigned values should never be reused. Remember to update the
// MAX value if adding a trailing new item.
//
// Changes to this enum must be reflected in the respective metrics enum named
// PrefetchItemErrorCode in enums.xml. Use the exact same integer value for
// both.
enum class PrefetchItemErrorCode {
  ...
  // Note: Must always have the same value as the actual last item.
  MAX = GENERATE_PAGE_BUNDLE_REQUEST_MAX_ATTEMPTS_REACHED
};
