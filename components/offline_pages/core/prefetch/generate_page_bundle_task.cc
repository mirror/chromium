...
static std::unique_ptr<std::vector<std::string>> SelectUrlsToPrefetchSync(
    sql::Connection* db) {
  if (!db)
    return nullptr;
  ...
  // Is we've got more than kMaxUrlsToSend URLs, mark the extra ones FINISHED
  ...
  for (const auto& url : *urls) {
    if (!UpdateStateSync(db, url))
      return nullptr;
    url_specs->push_back(std::move(url.requested_url_));
  }
