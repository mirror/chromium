...
void PrefetchServiceImpl::OnDownloadCompleted(
  ...
  // TODO(jianli): To hook up with prefetch service.
  /*GURL url;
  GURL original_url;
  base::string16 title;
  prefetch_importer_->ImportDownload(
      url, original_url, title,
      ClientId(kSuggestedArticlesNamespace, result.download_id),
      result.file_path, result.file_size,
      base::Bind(&PrefetchServiceImpl::OnImportCompleted,
                 // Importer is owned by this instance.
                 base::Unretained(this),
                 result.download_id));*/
}

void PrefetchServiceImpl::OnImportCompleted(const std::string& id,
                                            bool success,
                                            int64_t offline_id) {
  logger_.RecordActivity("Import " + id + (success ? " succeeded" : " failed"));
}
