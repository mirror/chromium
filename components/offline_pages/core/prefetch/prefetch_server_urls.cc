...
GURL GetGeneratePageBundleRequestURL(version_info::Channel channel) {
  GURL server_url =
      GetServerURLForPath(kGeneratePageBundleRequestURLPath, channel);
  return AppendApiKeyToURL(server_url, channel);
}

GURL GetGetOperationRequestURL(const std::string& name,
                               version_info::Channel channel) {
...
GURL GetPrefetchDownloadURL(const std::string& download_location,
                            version_info::Channel channel) {
