...
std::unique_ptr<std::set<std::string>> GetAllOperationsSync(
    sql::Connection* db) {
  ...
  std::unique_ptr<std::set<std::string>> operation_names;
  while (statement.Step()) {
    if (!operation_names)
      operation_names = base::MakeUnique<std::set<std::string>>();
    operation_names->emplace(statement.ColumnString(0));
  }
...
bool UpdateOperationSync(const std::string& operation_name,
  ...
  static const char kSql[] =
      ...
      " SET state = CASE WHEN get_operation_attempts < ? THEN ? ELSE ? END,"
