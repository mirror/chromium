...
TEST_F(GeneratePageBundleReconcileTaskTest, NoUpdateForOtherStates) {
  ...
  items.push_back(item_generator()->CreateItem(PrefetchItemState::NEW_REQUEST));
  items.push_back(
  ...
  items.push_back(item_generator()->CreateItem(PrefetchItemState::DOWNLOADING));
  items.push_back(item_generator()->CreateItem(PrefetchItemState::IMPORTING));
  items.push_back(item_generator()->CreateItem(PrefetchItemState::FINISHED));
  items.push_back(item_generator()->CreateItem(PrefetchItemState::ZOMBIE));
  for (auto& item : items) {
    item.generate_bundle_attempts =
        GeneratePageBundleReconcileTask::kMaxGenerateBundleAttempts;
    ASSERT_TRUE(store_util()->InsertPrefetchItem(item));
  ...
  std::vector<PrefetchItem> store_items;
  for (const auto& item : items) {
    std::unique_ptr<PrefetchItem> store_item =
        store_util()->GetPrefetchItem(item.offline_id);
    ASSERT_TRUE(store_item);
    store_items.push_back(*store_item);
  }
