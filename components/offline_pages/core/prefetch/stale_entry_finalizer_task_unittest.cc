...
void StaleEntryFinalizerTaskTest::SetUp() {
  ...
  stale_finalizer_task_ =
      base::MakeUnique<StaleEntryFinalizerTask>(store(), dispatcher());
...
TEST_F(StaleEntryFinalizerTaskTest, EmptyRun) {
  ...
  RunUntilIdle();
  EXPECT_EQ(Result::SUCCESS_NO_WORK_IN_QUEUE,
            stale_finalizer_task_->final_status());
  EXPECT_EQ(0U, store_util()->GetAllItems(&no_items));
}
...
TEST_F(StaleEntryFinalizerTaskTest, HandlesFreshnessTimesCorrectly) {
  ...
  stale_finalizer_task_->Run();
  RunUntilIdle();
  EXPECT_EQ(Result::SUCCESS, stale_finalizer_task_->final_status());
...
TEST_F(StaleEntryFinalizerTaskTest, WorkInQueue) {
  ...
  for (auto& state : work_states) {
    ...
    StaleEntryFinalizerTask task(store(), dispatcher());
    ExpectTaskCompletes(&task);
    task.Run();
    RunUntilIdle();
    EXPECT_EQ(Result::SUCCESS, task.final_status());
