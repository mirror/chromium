...
int GetFileSizeEnumValueFor(int64_t archive_body_length, int64_t file_size) {
  ...
  if (archive_body_length == file_size)
    return 11;  // size matches (100%)
  double ratio = double(file_size) / archive_body_length;
  if (archive_body_length > file_size)
    return std::max(0, std::min(9, int(ratio * 10))) + 1;  // 1 <= x <= 11
  return std::max(10, std::min(20, int(ratio * 10))) + 2;  // 12 <= x <= 22
}
...
void ReportMetricsFor(const PrefetchItemStats& url, const base::Time now) {
  ...
  int64_t lifetime_s = (now - url.creation_time).InSeconds();
  if (successful) {
    UMA_HISTOGRAM_CUSTOM_COUNTS(
        "OfflinePages.Prefetching.ItemLifetime.Successful", lifetime_s, 1,
        kFourWeeksInSeconds, 50);
  } else {
    UMA_HISTOGRAM_CUSTOM_COUNTS("OfflinePages.Prefetching.ItemLifetime.Failed",
                                lifetime_s, 1, kFourWeeksInSeconds, 50);
  }
