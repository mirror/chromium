...
bool SelectUrlsToPrefetchSync(sql::Connection* db) {
  ...
  for (const auto& url : *urls) {
    DVLOG(1) << "Finalized prefetch item: (" << url.offline_id << ", "
             ...
             << ")";
    MarkUrlAsZombie(db, url.offline_id);
  }
