...
PrefetchNetworkRequestFactoryImpl::CurrentGeneratePageBundleRequest() {
  return current_generate_page_bundle_request_.get();
}

void PrefetchNetworkRequestFactoryImpl::
    CancelCurrentGeneratePageBundleRequest() {
  current_generate_page_bundle_request_.reset();
}

void PrefetchNetworkRequestFactoryImpl::MakeGetOperationRequest(
    const std::string& operation_name,
    const PrefetchRequestFinishedCallback& callback) {
  current_get_operation_requests_[operation_name] =
...
void PrefetchNetworkRequestFactoryImpl::GetOperationRequestDone(
    ...
    const std::vector<RenderPageInfo>& pages) {
  callback.Run(status, operation_name, pages);
  current_get_operation_requests_.erase(operation_name);
}

GetOperationRequest*
PrefetchNetworkRequestFactoryImpl::FindGetOperationRequestByName(
    const std::string& operation_name) {
  return current_get_operation_requests_[operation_name].get();
}

void PrefetchNetworkRequestFactoryImpl::CancelGetOperationRequestByName(
    const std::string& operation_name) {
  current_get_operation_requests_.erase(operation_name);
}
