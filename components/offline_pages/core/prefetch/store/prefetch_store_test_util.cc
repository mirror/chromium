...
bool InsertPrefetchItemSync(const PrefetchItem& item, sql::Connection* db) {
  ...
  statement.BindInt64(
      6, OfflinePageStoreUtils::ToDatabaseTime(item.creation_time));
  statement.BindInt64(
      7, OfflinePageStoreUtils::ToDatabaseTime(item.freshness_time));
  ...
  statement.BindString(
      17, OfflinePageStoreUtils::ToDatabaseFilePath(item.file_path));
...
void PopulatePrefetchItem(const sql::Statement& statement, PrefetchItem* item) {
  ...
  item->creation_time =
      OfflinePageStoreUtils::FromDatabaseTime(statement.ColumnInt64(6));
  item->freshness_time =
      OfflinePageStoreUtils::FromDatabaseTime(statement.ColumnInt64(7));
  ...
  item->file_path =
      OfflinePageStoreUtils::FromDatabaseFilePath(statement.ColumnString(17));
