...
std::unique_ptr<PrefetchItem> FetchPrefetchItemSync(int64_t offline_id,
                                                    sql::Connection* db) {
  static const char kSql[] =
      "SELECT "
      "  offline_id, state, request_archive_attempt_count,"
...
// TODO: change param to PrefetchItem.
...
std::unique_ptr<PrefetchItem> PrefetchStoreTestUtil::GetPrefetchItem(
  ...
  store_->Execute(
      base::BindOnce(&FetchPrefetchItemSync, offline_id),
      base::BindOnce(
          [](std::unique_ptr<PrefetchItem>* result,
             std::unique_ptr<PrefetchItem> item) { *result = std::move(item); },
          &item));
  RunUntilIdle();
  return item;
}
