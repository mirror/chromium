...
bool InsertPrefetchItemSync(const PrefetchItem& item, sql::Connection* db) {
  static const char kSql[] =
      ...
      "  download_initiation_attempts, archive_body_length,"
  ...
  statement.BindInt64(6, SerializeTime(item.creation_time));
  statement.BindInt64(7, SerializeTime(item.freshness_time));
...
void PopulatePrefetchItem(const sql::Statement& statement, PrefetchItem* item) {
  ...
  item->creation_time = DeserializeTime(statement.ColumnInt64(6));
  item->freshness_time = DeserializeTime(statement.ColumnInt64(7));
...
bool PrefetchStoreTestUtil::ZombifyPrefetchItems(const std::string& name_space,
                                                 const GURL& url) {
  bool success = false;
  store_->Execute(
      base::BindOnce(&UpdateItemsStateSync, name_space, url.spec(),
                     PrefetchItemState::ZOMBIE),
      base::BindOnce([](bool* alias, int result) { *alias = result; },
                     &success));
  RunUntilIdle();
  return success;
}
