...
bool PrefetchStoreSchema::CreateOrUpgradeIfNeeded() {
  ...
  // Schema upgrade code starts here. Be very careful when authoring this code
  // as it must robust to any sort of inconsistent schema state.
  ...
  if (current_version <= 1) {
    // Raze and recreate the latest schema version and return immediately.
    meta_table.Reset();
    return db_->Raze() && CreateLatestSchema(db_) &&
           meta_table.Init(db_, kCurrentVersion, kCompatibleVersion);
  }

  // Returns false when an error happens or if |from_version| is unknown.
  return true;
}
