...
TEST_F(PrefetchStoreTest, ConfirmZombify) {
  PrefetchItem item1(
      item_generator()->CreateItem(PrefetchItemState::NEW_REQUEST));
  EXPECT_FALSE(store_util()->ZombifyPrefetchItems(item1.client_id.name_space,
                                                  item1.url));
  store_util()->InsertPrefetchItem(item1);
  EXPECT_TRUE(store_util()->ZombifyPrefetchItems(item1.client_id.name_space,
                                                 item1.url));
  EXPECT_TRUE(store_util()->ZombifyPrefetchItems(item1.client_id.name_space,
                                                 item1.url));
}
