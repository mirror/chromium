...
class PrefetchStore {
 public:
  ...
  // FAILURE, in which case the store will |CheckDb| function will return false
  // and work should fail. That ensures that store can be purged of pending
  // tasks upon failed initialization and callers are not waiting indefinitely.
  template <typename T>
  void Execute(RunCallback<T> run_callback, ResultCallback<T> result_callback) {
    ...
    pending_command_ = base::BindOnce(
 ...
 private:
  ...
  // Task deferred until initialization is complete.
  base::OnceClosure pending_command_;
