...
class PrefetchStoreSchemaTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    schema_ = base::MakeUnique<PrefetchStoreSchema>();
  ...
  void CheckTablesExistence() {
    ...
    EXPECT_FALSE(db_->DoesTableExist("prefetch_items_temp"));
    EXPECT_FALSE(db_->DoesTableExist("prefetch_downloader_quota_temp"));
  }
...
TEST_F(PrefetchStoreSchemaTest, TestSchemaCreationFromNothing) {
  EXPECT_TRUE(schema_->CreateOrUpgradeIfNeeded(db_.get()));
