...
TEST_F(OfflinePageArchiverTest, PublishArchive) {
  ...
  std::unique_ptr<PublishArchiveResult> archive_result(
      new PublishArchiveResult());

  // Call method under test
  OfflinePageArchiver::PublishArchive(offline_page, public_archive_dir_path(),
                                      download_manager.get(),
                                      archive_result.get());

  // Check results
  EXPECT_EQ(new_file_path, archive_result->new_file_path);
  EXPECT_EQ(SavePageResult::SUCCESS, archive_result->move_result);
  EXPECT_EQ(kDownloadId, archive_result->download_id);
  // Check there is a file in the new location
  EXPECT_TRUE(
      public_archive_dir_path().IsParent(archive_result->new_file_path));
  EXPECT_TRUE(base::PathExists(archive_result->new_file_path));
