...
std::unique_ptr<OfflinePageModelImpl> OfflinePageModelImplTest::BuildModel(
  ...
  if (temporary_dir_same_with_persistent_) {
    archive_manager = base::MakeUnique<ArchiveManager>(
        persistent_dir_path(), persistent_dir_path(),
        base::ThreadTaskRunnerHandle::Get());
  } else {
    archive_manager = base::MakeUnique<ArchiveManager>(
...
// This test is affected by crbug/725685, which only affects windows platform.
#if !defined(OS_WIN)
TEST_F(OfflinePageModelImplTest, DeleteTemporaryPagesInWrongDir) {
