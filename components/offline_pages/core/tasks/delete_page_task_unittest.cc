...
class DeletePageTaskTest : public testing::Test,
 ...
 public:
  ...
  void OnDeletePageDone(const std::vector<OfflinePageItem>& deleted_pages,
  ...
  const std::vector<OfflinePageItem>& last_deleted_pages() {
    return last_deleted_pages_;
  }
 ...
 private:
  ...
  std::vector<OfflinePageItem> last_deleted_pages_;
...
void DeletePageTaskTest::SetUp() {
  ...
  archive_manager_ = base::MakeUnique<ArchiveManager>(
...
void DeletePageTaskTest::OnDeletePageDone(
    ...
    DeletePageResult result) {
  last_deleted_pages_ = deleted_pages;
...
TEST_F(DeletePageTaskTest, DeletePageByOfflineId) {
  ...
  std::unique_ptr<DeletePageTask> task(
      DeletePageTaskFactory::CreateTaskWithOfflineIds(
          store(), archive_manager(), offline_ids,
...
TEST_F(DeletePageTaskTest, DeletePageByClientId) {
  ...
  std::unique_ptr<DeletePageTask> task(
      DeletePageTaskFactory::CreateTaskWithClientIds(
          store(), archive_manager(), client_ids,
          base::Bind(&DeletePageTaskTest::OnDeletePageDone, AsWeakPtr())));
  task->Run();
  PumpLoop();
...
TEST_F(DeletePageTaskTest, DeletePageByPagePredicate) {
  // Add 3 pages and try to delete 2 of them using client id.
  OfflinePageItem page(kTestUrl1, kTestOfflineId1, kTestClientId1,
                       kTestFilePath, kTestFileSize);
  OfflinePageItem page2(kTestUrl2, kTestOfflineId2, kTestClientId2,
  ...
  // Delete all pages with even offline ids.
  PagePredicate predicate = base::Bind(
      [](const OfflinePageItem& page) -> bool { return page.offline_id % 2; });

  std::unique_ptr<DeletePageTask> task(
      DeletePageTaskFactory::CreateTaskWithPagePredicate(
          store(), archive_manager(), predicate,
          base::Bind(&DeletePageTaskTest::OnDeletePageDone, AsWeakPtr())));
  task->Run();
  PumpLoop();

  EXPECT_EQ(DeletePageResult::SUCCESS, last_delete_page_result());
  for (const auto& deleted_page : last_deleted_pages())
    EXPECT_TRUE(predicate.Run(deleted_page));
  EXPECT_EQ(predicate.Run(page), !base::PathExists(page.file_path));
  EXPECT_EQ(predicate.Run(page2), !base::PathExists(page2.file_path));
  EXPECT_EQ(predicate.Run(page3), !base::PathExists(page3.file_path));
}

TEST_F(DeletePageTaskTest, DeletePageByPagePredicateNotFound) {
  // Add 3 pages and try to delete 2 of them using client id.
  OfflinePageItem page(kTestUrl1, kTestOfflineId1, kTestClientId1,
                       kTestFilePath, kTestFileSize);
  OfflinePageItem page2(kTestUrl2, kTestOfflineId2, kTestClientId2,
  ...
  PagePredicate predicate =
      base::Bind([](const OfflinePageItem& page) -> bool { return false; });
  ...
  EXPECT_EQ(DeletePageResult::SUCCESS, last_delete_page_result());
  EXPECT_TRUE(last_deleted_pages().empty());
  EXPECT_TRUE(base::PathExists(page.file_path));
  EXPECT_TRUE(base::PathExists(page2.file_path));
  EXPECT_TRUE(base::PathExists(page3.file_path));
}
