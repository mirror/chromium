...
void SnapshotController::RenovationsCompleted() {
  if (renovations_enabled_) {
    ...
    // take a snapshot.
    if (current_page_quality_ < PageQuality::GOOD)
      return;
...
void SnapshotController::DocumentAvailableInMainFrame() {
  ...
  DVLOG(0) << "@@@@@@ low bar " << __func__;
  low_bar_time_ = base::TimeTicks::Now();
...
void SnapshotController::DocumentOnLoadCompletedInMainFrame() {
  ...
  if (renovations_enabled_) {
    ...
    // Post a delayed task check the progress.  This allows a small amount of
    // time for the first resource requests to trickle in.
    task_runner_->PostDelayedTask(
        FROM_HERE,
        base::Bind(&SnapshotController::CheckProgressAndMaybeSnapshot,
                   weak_ptr_factory_.GetWeakPtr()),
        base::TimeDelta::FromMilliseconds(
            delay_after_document_on_load_completed_ms_));
  } else {
    ...
    DVLOG(0) << "@@@@@@ You forgot to set the flag for RP! " << __func__;
    if (!loaded_error_page_)
...
void SnapshotController::CheckProgressAndMaybeSnapshot() {
  // If all the conditions are met, post a task to snapshot and then stop this
  // controller.
  if (ProgressIsGoodEnoughForSnapshot()) {
    DVLOG(0) << "@@@@@@ starting snapshot " << __func__;
...
// An alternative for Right Moment Detection (RMD).  Instead of just waiting for
// a timer after a load event, use signals to check the load progress, and
// snapshot when the page looks mostly complete.
...
bool SnapshotController::ProgressIsGoodEnoughForSnapshot() {
  ...
  // Ensure a small amount of time has passed since we saw the low bar,
  // so resource requests have had a chance to queue up.
  base::TimeDelta elapsed = base::TimeTicks::Now() - low_bar_time_;
  if (elapsed.InMilliseconds() < delay_after_document_on_load_completed_ms_)
    return false;
  ...
  // We want all images to have arrived.
  if (images_requested_ > images_completed_)
    return false;
  ...
  if (renovations_enabled_ && !renovations_completed_)
    return false;
  DVLOG(0) << "@@@@@@ High bar met, ordering snapshot " << __func__;
  DVLOG(0) << "@@@@@@ images requested: " << images_requested_
           << " completed: " << images_completed_ << " " << __func__;
  DVLOG(0) << "@@@@@@ css requested: " << css_requested_
           << " completed: " << css_completed_ << " " << __func__;
  return true;
}
