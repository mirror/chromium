...
void GetStorageStatsImpl(const std::vector<base::FilePath>& archives_dirs,
  ...
  // DCHECK that if there're temporary and persistent directories, they are on
  // the same volume. In case in the future they're separated onto different
  // volumes.
  if (archives_dirs.size() > 1) {
    DCHECK(base::SysInfo::AmountOfFreeDiskSpace(archives_dirs[1]) ==
           storage_stats.free_disk_space);
  }
  for (const auto& archives_dir : archives_dirs)
