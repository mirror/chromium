  ...
  if (!base::DirectoryExists(archives_dir)) {
    ...
    if (is_temp) {
      UMA_HISTOGRAM_ENUMERATION(
          "OfflinePages.ArchiveManager.ArchiveDirsCreationResult.Temporary",
          -error, -base::File::FILE_ERROR_MAX);
    } else {
      UMA_HISTOGRAM_ENUMERATION(
          "OfflinePages.ArchiveManager.ArchiveDirsCreationResult.Persistent",
          -error, -base::File::FILE_ERROR_MAX);
    }
  }
}
...
void GetStorageStatsImpl(const base::FilePath& temporary_archives_dir,
  ...
  storage_stats.free_disk_space =
      base::SysInfo::AmountOfFreeDiskSpace(persistent_archives_dir);
  ...
  DCHECK(temporary_archives_dir.empty() ||
