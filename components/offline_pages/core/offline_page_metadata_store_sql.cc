...
bool CreateLatestSchema(sql::Connection* db) {
  ...
  if (!db->DoesTableExist(OFFLINE_PAGES_TABLE_NAME)) {
    ...
    if (!meta_table.Init(db, kCurrentVersion, kFirstPostLegacyVersion))
      return false;
  }
...
bool RunLegacyUpgrade(sql::Connection* db) {
  // If you create a transaction but don't Commit() it is automatically
  // rolled back by its destructor when it falls out of scope.
  sql::Transaction transaction(db);
  if (!transaction.Begin())
  ...
  sql::MetaTable meta_table;
  if (!meta_table.Init(db, kFirstPostLegacyVersion, kFirstPostLegacyVersion))
    return false;

  return transaction.Commit();
}

bool RunUpgradeFromVersion1ToVersion2(sql::Connection* db,
                                      sql::MetaTable* meta_table) {
  sql::Transaction transaction(db);
  if (!transaction.Begin())
    return false;
...
bool CreateSchema(sql::Connection* db) {
  ...
  if (meta_table.GetVersionNumber() == 1) {
    if (!RunUpgradeFromVersion1ToVersion2(db, &meta_table))
      return false;
  }
