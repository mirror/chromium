...
bool CreateOfflinePagesTable(sql::Connection* db) {
  const char kSql[] = "CREATE TABLE IF NOT EXISTS " OFFLINE_PAGES_TABLE_NAME
                      ...
                      " origin_package VARCHAR NOT NULL DEFAULT ''"
                      ")";
...
bool CreateSchema(sql::Connection* db) {
  ...
  if (!db->DoesColumnExist(OFFLINE_PAGES_TABLE_NAME, "expiration_time") &&
    ...
    if (!UpgradeFrom56(db))
      return false;
  } else if (!db->DoesColumnExist(OFFLINE_PAGES_TABLE_NAME, "origin_package")) {
    if (!UpgradeFrom57(db))
      return false;
  }
...
OfflinePageItem MakeOfflinePageItem(sql::Statement* statement) {
  ...
  std::string origin_package = statement->ColumnString(11);
  ...
  item.origin_package = origin_package;
  return item;
}
...
ItemActionStatus Insert(sql::Connection* db, const OfflinePageItem& item) {
  ...
  const char kSql[] =
      ...
      "title, original_url, origin_package)"
      " VALUES "
      " (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  ...
  statement.BindString(11, item.origin_package);
  if (!statement.Run())
...
bool Update(sql::Connection* db, const OfflinePageItem& item) {
  const char kSql[] =
      ...
      " access_count = ?, title = ?, original_url = ?, origin_package = ?"
      " WHERE offline_id = ?";
  ...
  statement.BindString(10, item.origin_package);
