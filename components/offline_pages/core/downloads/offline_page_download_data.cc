...
namespace {
const char kOfflinePageDownloadDataKey[] = "";
}  // namespace
...
void OfflinePageDownloadData::Attach(net::URLRequest* request,
  ...
  request->SetUserData(kOfflinePageDownloadDataKey, std::move(request_data));
}

// static
OfflinePageDownloadData* OfflinePageDownloadData::Get(
    const net::URLRequest* request) {
  return static_cast<OfflinePageDownloadData*>(
      request->GetUserData(kOfflinePageDownloadDataKey));
}
