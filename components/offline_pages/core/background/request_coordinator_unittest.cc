...
class ObserverStub : public RequestCoordinator::Observer {
 public:
  ...
  void OnAdded(const SavePageRequest& request) override {
    ...
    available_state_ = request.available_state();
  }
  ...
  SavePageRequest::AvailableState available_state() { return available_state_; }
 ...
 private:
  ...
  SavePageRequest::AvailableState available_state_;
};
...
TEST_F(RequestCoordinatorTest, ResumeRequestWhenNoNetwork) {
  // Add a request to the queue.
  AddRequest1();
  PumpLoop();
  ...
  EXPECT_TRUE(observer().changed_called());
  EXPECT_EQ(SavePageRequest::RequestState::AVAILABLE, observer().state());
  EXPECT_EQ(SavePageRequest::AvailableState::WAITING_NETWORK,
            observer().available_state());
}

TEST_F(RequestCoordinatorTest, ResumeRequestWhenOfflining) {
  // Add a request to the queue.
  AddRequest1();
  PumpLoop();
  ...
  // Verify the request is pending another download completion.
  EXPECT_TRUE(observer().changed_called());
  EXPECT_EQ(SavePageRequest::RequestState::AVAILABLE, observer().state());
  EXPECT_EQ(
...
TEST_F(RequestCoordinatorTest, AddRequestWhenNoNetwork) {
  SetNetworkConnected(false);
...
TEST_F(RequestCoordinatorTest, AddRequestWhenOfflining) {
  ...
  // Verify second request is pending another download completion.
  EXPECT_TRUE(observer().added_called());
  EXPECT_EQ(SavePageRequest::RequestState::AVAILABLE, observer().state());
  EXPECT_EQ(
      SavePageRequest::AvailableState::WAITING_ANOTHER_DOWNLOAD_COMPLETION,
