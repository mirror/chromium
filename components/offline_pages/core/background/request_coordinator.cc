...
RequestCoordinator::RequestCoordinator(
    ...
    : is_low_end_device_(base::SysInfo::IsLowEndDevice()),
      ...
      pending_state_updater_(new PendingStateUpdater(this)),
...
void RequestCoordinator::TryNextRequest(bool is_start_of_processing) {
  ...
  if (connection_type != previous_connection_type &&
      connection_type ==
          net::NetworkChangeNotifier::ConnectionType::CONNECTION_NONE) {
    pending_state_updater_->UpdateRequestsLossOfNetwork();
  }
...
void RequestCoordinator::RequestPicked(
  ...
  if (state_ == RequestCoordinatorState::PICKING) {
    ...
    // Update the pending state for available requests if applicable
    pending_state_updater_->UpdateNonPickedRequests(
        picked_request, std::move(available_requests));
  }
