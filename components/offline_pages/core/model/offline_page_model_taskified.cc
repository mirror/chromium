...
void OfflinePageModelTaskified::PostCheckMetadataConsistencyTask(
  ...
  base::TimeDelta delay;
  // TODO(romax): Pick a better delay than reusing the one for clear storage.
  if (is_initializing) {
    delay = kClearStorageStartingDelay;
  }
  base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE,
      base::Bind(&OfflinePageModelTaskified::CheckTemporaryPagesConsistency,
                 weak_ptr_factory_.GetWeakPtr()),
      delay);
  base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE,
      base::Bind(&OfflinePageModelTaskified::CheckPersistentPagesConsistency,
                 weak_ptr_factory_.GetWeakPtr()),
      delay);
}
