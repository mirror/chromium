...
bool PersistentPagesConsistencyCheckTaskTest::IsPageRemoved(
    const OfflinePageItem& page) {
  return !base::PathExists(page.file_path) &&
         !store_test_util()->GetPageByOfflineId(page.offline_id);
}

TEST_F(PersistentPagesConsistencyCheckTaskTest, TestDeleteFileWithoutDbEntry) {
...
TEST_F(PersistentPagesConsistencyCheckTaskTest, TestDeleteDBEntryWithoutFile) {
...
TEST_F(PersistentPagesConsistencyCheckTaskTest, TestKeepingNonMhtmlFile) {
  ...
  base::FilePath mp3_path = path.AddExtension("mp3");
  ...
  EXPECT_EQ(0LL, store_test_util()->GetPageCount());
  EXPECT_EQ(1LL, GetFileCountInDir(persistent_dir()));
  EXPECT_TRUE(IsPageRemoved(page1));
}
