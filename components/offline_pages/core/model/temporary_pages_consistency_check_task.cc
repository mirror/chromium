...
SyncOperationResult CheckConsistencySync(
  ...
  if (offline_ids_to_delete.size() > 0) {
    ...
    LOG(ERROR) << "TemporaryPagesConsistencyCheckTask deleted " << offline_ids_to_delete.size() << " offline pages";
  }
  ...
  if (files_to_delete.size() > 0) {
    ...
    LOG(ERROR) << "TemporaryPagesConsistencyCheckTask deleted " << files_to_delete.size() << " files";
  }
...
void TemporaryPagesConsistencyCheckTask::OnCheckConsistencyDone(
  ...
  TaskComplete();
  LOG(ERROR) << "TemporaryPagesConsistencyCheckTask FINISHED";
}
