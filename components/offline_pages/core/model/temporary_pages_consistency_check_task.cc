...
SyncOperationResult CheckConsistencySync(
  ...
  if (offline_ids_to_delete.size() > 0) {
    UMA_HISTOGRAM_COUNTS(
        "OfflinePages.Consistency.Temporary.PagesMissingArchiveFileCount",
  ...
  if (files_to_delete.size() > 0) {
    UMA_HISTOGRAM_COUNTS(
        "OfflinePages.Consistency.Temporary.PagesMissingDbEntryCount",
...
void TemporaryPagesConsistencyCheckTask::OnCheckConsistencyDone(
  ...
  UMA_HISTOGRAM_ENUMERATION(
      "OfflinePages.Consistency.Temporary.Result", static_cast<int>(result),
      static_cast<int>(SyncOperationResult::RESULT_COUNT));
  TaskComplete();
}
