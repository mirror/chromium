...
void ReadPagesByNamespaceChecked(sql::Connection* db,
...
ReadResult ReadPagesByNamespaceSync(const std::string& name_space,
                                    sql::Connection* db) {
  ReadResult result;
  if (!db)
    return result;

  ReadPagesByNamespaceChecked(db, name_space, &result);
...
std::unique_ptr<GetPagesTask> GetPagesTask::CreateTaskMatchingNamespace(
    ...
    const std::string& name_space) {
  return std::unique_ptr<GetPagesTask>(new GetPagesTask(
      store, base::BindOnce(&ReadPagesByNamespaceSync, name_space), callback));
}
