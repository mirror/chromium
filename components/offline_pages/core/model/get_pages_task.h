...
class GetPagesTask : public Task {
 public:
  ...
  static std::unique_ptr<GetPagesTask> CreateTaskForAllPages(
      OfflinePageMetadataStoreSQL* store,
      const MultipleOfflinePageItemCallback& callback);

  // Creates |GetPagesTask| reading pages matching provided |client_ids| from
  // DB.
  static std::unique_ptr<GetPagesTask> CreateTaskForClientIds(
      OfflinePageMetadataStoreSQL* store,
      const MultipleOfflinePageItemCallback& callback,
  ...
  // from DB.
  static std::unique_ptr<GetPagesTask> CreateTaskForRequestOrigin(
      OfflinePageMetadataStoreSQL* store,
      const MultipleOfflinePageItemCallback& callback,
      const std::string& request_origin);

  // Creates |GetPagesTask| reading pages matching provided |url| from DB.
  // The url will be matched against original URL and final URL, after fragments
  // are removed. Only a match on a single field is necessary.
  static std::unique_ptr<GetPagesTask> CreateTaskForUrl(
      OfflinePageMetadataStoreSQL* store,
      const MultipleOfflinePageItemCallback& callback,
      const GURL& url);
  ...
  // from DB.
  static std::unique_ptr<GetPagesTask> CreateTaskForOfflineId(
      OfflinePageMetadataStoreSQL* store,
