...
class TemporaryConsistencyCheckTaskTest : public testing::Test {
 public:
  TemporaryConsistencyCheckTaskTest();
  ~TemporaryConsistencyCheckTaskTest() override;
  ...
  const base::FilePath& abandoned_dir() { return abandoned_dir_.GetPath(); }
 ...
 private:
  ...
  base::ScopedTempDir abandoned_dir_;
  base::ScopedTempDir temporary_dir_;
  bool remove_db_entry_;
  bool remove_file_;
};

TemporaryConsistencyCheckTaskTest::TemporaryConsistencyCheckTaskTest()
...
void TemporaryConsistencyCheckTaskTest::SetUp() {
  store_test_util_.BuildStoreInMemory();
  ASSERT_TRUE(abandoned_dir_.CreateUniqueTempDir());
  ASSERT_TRUE(temporary_dir_.CreateUniqueTempDir());
  policy_controller_ = base::MakeUnique<ClientPolicyController>();
}

void TemporaryConsistencyCheckTaskTest::TearDown() {
  store_test_util_.DeleteStore();
  if (!abandoned_dir_.Delete())
...
OfflinePageItem TemporaryConsistencyCheckTaskTest::AddPage(
    const std::string& name_space,
    const base::FilePath& archive_dir) {
  generator()->SetNamespace(name_space);
  OfflinePageItem page = generator()->CreateItemWithTempFileInDir(archive_dir);
  if (!remove_db_entry_)
...
void TemporaryConsistencyCheckTaskTest::SetRemoveDbEntryAndFile(
...
// platform.
#if defined(OS_WIN)
#define MAYBE_TestDeletePageInAbandonedDir DISABLED_TestDeletePageInAbandonedDir
#else
#define MAYBE_TestDeletePageInAbandonedDir TestDeletePageInAbandonedDir
#endif
TEST_F(TemporaryConsistencyCheckTaskTest, MAYBE_TestDeletePageInAbandonedDir) {
  // The temporary pages in abandoned directory will be deleted from both DB and
  // disk.
  SetRemoveDbEntryAndFile(false, false);
  OfflinePageItem page1 = AddPage(kLastNNamespace, abandoned_dir());
  OfflinePageItem page2 = AddPage(kDownloadNamespace, abandoned_dir());
  OfflinePageItem page3 = AddPage(kLastNNamespace, temporary_dir());

  EXPECT_EQ(3LL, store_test_util()->GetPageCount());
  EXPECT_EQ(2LL, GetFileCountInDir(abandoned_dir()));

  auto task = base::MakeUnique<TemporaryConsistencyCheckTask>(
      store(), policy_controller(), temporary_dir(), abandoned_dir());
  runner()->RunTask(std::move(task));

  EXPECT_EQ(2LL, store_test_util()->GetPageCount());
  EXPECT_EQ(1LL, GetFileCountInDir(abandoned_dir()));
  EXPECT_EQ(1LL, GetFileCountInDir(temporary_dir()));
  EXPECT_EQ(OfflinePageModel::kInvalidOfflineId,
            store_test_util()->GetPageByOfflineId(page1.offline_id).offline_id);
}

TEST_F(TemporaryConsistencyCheckTaskTest, TestDeleteFileWithoutDbEntry) {
  // Since the files in abandoned directory are missing DB entries, there's no
  // way to know if the pages were temporary or persistent. So they'll not be
  // removed, only the file without DB entry and in temporary directory will be
  // deleted.
...
TEST_F(TemporaryConsistencyCheckTaskTest, TestDeleteDBEntryWithoutFile) {
  ...
  EXPECT_EQ(3LL, store_test_util()->GetPageCount());
  EXPECT_EQ(0LL, GetFileCountInDir(abandoned_dir()));
  EXPECT_EQ(0LL, GetFileCountInDir(temporary_dir()));
  ...
  EXPECT_EQ(OfflinePageModel::kInvalidOfflineId,
            store_test_util()->GetPageByOfflineId(page3.offline_id).offline_id);
}
...
#endif
TEST_F(TemporaryConsistencyCheckTaskTest, MAYBE_CombinedTest) {
  ...
  OfflinePageItem page3 = AddPage(kLastNNamespace, temporary_dir());
  SetRemoveDbEntryAndFile(true, false);
  OfflinePageItem page4 = AddPage(kLastNNamespace, abandoned_dir());
  OfflinePageItem page5 = AddPage(kDownloadNamespace, abandoned_dir());
  OfflinePageItem page6 = AddPage(kLastNNamespace, temporary_dir());
  SetRemoveDbEntryAndFile(false, true);
  OfflinePageItem page7 = AddPage(kLastNNamespace, abandoned_dir());
  OfflinePageItem page8 = AddPage(kDownloadNamespace, abandoned_dir());
  OfflinePageItem page9 = AddPage(kLastNNamespace, temporary_dir());

  EXPECT_EQ(6LL, store_test_util()->GetPageCount());
  EXPECT_EQ(4LL, GetFileCountInDir(abandoned_dir()));
  ...
  EXPECT_EQ(3LL, store_test_util()->GetPageCount());
  EXPECT_EQ(3LL, GetFileCountInDir(abandoned_dir()));
  EXPECT_EQ(1LL, GetFileCountInDir(temporary_dir()));
}
