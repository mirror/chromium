...
TEST_F(OfflinePageModelTaskifiedTest, MAYBE_CheckPagesSavedInSeparateDirs) {
  ...
  SavePageWithArchiverResult(kTestUrl, kTestClientId1,
                             ArchiverResult::SUCCESSFULLY_CREATED);
  int64_t temporary_id = last_save_page_offline_id();
  // Save a persistent page.
  SavePageWithArchiverResult(kTestUrl2, kTestUserRequestedClientId,
                             ArchiverResult::SUCCESSFULLY_CREATED);
  int64_t persistent_id = last_save_page_offline_id();
