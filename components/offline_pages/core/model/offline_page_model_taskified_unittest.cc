...
class OfflinePageModelTaskifiedTest
 ...
 public:
  ...
  void SetTestingClock(base::Clock* clock) { model_->testing_clock_ = clock; }
...
#endif
TEST_F(OfflinePageModelTaskifiedTest, CheckPagesSavedInSeparateDirs) {
...
#endif
TEST_F(OfflinePageModelTaskifiedTest, StartUp_ConsistencyCheckExecuted) {
  ...
  DLOG(ERROR) << "ROMAX page paths:";
  DLOG(ERROR) << temp_page1.file_path.value();
  DLOG(ERROR) << temp_page2.file_path.value();
  DLOG(ERROR) << temp_page3.file_path.value();
  DLOG(ERROR) << persistent_page1.file_path.value();
  DLOG(ERROR) << persistent_page2.file_path.value();
  DLOG(ERROR) << persistent_page3.file_path.value();
  DLOG(ERROR) << "ROMAX dir paths:";
  DLOG(ERROR) << temporary_dir_path().value();
  test_util::IterateDirFiles(temporary_dir_path());
  DLOG(ERROR) << persistent_dir_path().value();
  test_util::IterateDirFiles(persistent_dir_path());
  ...
  DLOG(ERROR) << temporary_dir_path().value();
  DLOG(ERROR) << persistent_dir_path().value();
  DLOG(ERROR) << store_test_util()->GetPageCount();
  DLOG(ERROR) << test_util::GetFileCountInDirectory(temporary_dir_path());
  test_util::IterateDirFiles(temporary_dir_path());
  DLOG(ERROR) << test_util::GetFileCountInDirectory(persistent_dir_path());
  test_util::IterateDirFiles(persistent_dir_path());
...
TEST_F(OfflinePageModelTaskifiedTest, ClearStorage) {
  ...
  clock->SetNow(now);
  SetTestingClock(clock.get());

  PumpLoop();
  EXPECT_EQ(now, last_clear_page_time());
