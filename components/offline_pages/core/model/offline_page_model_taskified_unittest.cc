...
TEST_F(OfflinePageModelTaskifiedTest, MAYBE_CheckPagesSavedInSeparateDirs) {
  ...
  // For a page in the "downloads" namespace, it gets moved to the
  // public directory, and has a download ID set.
  EXPECT_TRUE(public_archive_dir_path().IsParent(persistent_page_path));
  EXPECT_EQ(kDownloadId, persistent_page->system_download_id);
