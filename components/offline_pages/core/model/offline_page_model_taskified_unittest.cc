...
const ClientId kTestClientId3(kDefaultNamespace, "42");
...
TEST_F(OfflinePageModelTaskifiedTest, GetPagesByURL) {
  generator()->SetArchiveDirectory(temporary_dir_path());
  generator()->SetUrl(kTestUrl);
  OfflinePageItem page = generator()->CreateItemWithTempFile();
  AddPage(page);
...
TEST_F(OfflinePageModelTaskifiedTest, EnsureArchivesDirCreatedHappend) {
  histograms().ExpectTotalCount("OfflinePages.Model.ArchiveDirCreationTime", 1);
}
...
TEST_F(OfflinePageModelTaskifiedTest, ExtraActionTriggeredWhenSaveSuccess) {
  ...
  ResetResults();
  CheckTaskQueueIdle();
  histograms().ExpectTotalCount("OfflinePages.ClearStorageResult", 0);
  ...
  EXPECT_TRUE(observer_add_page_called());
  EXPECT_TRUE(observer_delete_page_called());
  histograms().ExpectTotalCount("OfflinePages.ClearStorageResult", 1);
  CheckTaskQueueIdle();
}

// The following commented tests needs support of Store failure, hopefully in
// store_test_utils().
/*
TEST_F(OfflinePageModelTaskifiedTest, ClearCachedPagesTriggeredWhenSaveFailed) {
  ...
  std::unique_ptr<OfflinePageTestArchiver> archiver(BuildArchiver(
      kTestUrl, ArchiverResult::SUCCESSFULLY_CREATED));
  OfflinePageTestArchiver* archiver_ptr = archiver.get();
...
TEST_F(OfflinePageModelTaskifiedTest, SavePageOfflineCreationStoreWriteFailure){
...
// The following tests for save page might be possible to test using a
// MockCallback as well.
// TODO(romax): see if there's a way to convert the tests below using
// MockCallbacks, and do it if possible.
...
TEST_F(OfflinePageModelTaskifiedTest, CustomTabsNamespace) {
...
TEST_F(OfflinePageModelTaskifiedTest, DownloadNamespace) {
...
TEST_F(OfflinePageModelTaskifiedTest, NewTabPageNamespace) {
