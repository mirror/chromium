...
SyncOperationResult CheckConsistencySync(
  ...
  if (offline_ids_to_delete.size() > 0) {
    UMA_HISTOGRAM_COUNTS(
        "OfflinePages.Consistency.Persistent.PagesMissingArchiveFileCount",
  ...
  if (files_to_delete.size() > 0) {
    UMA_HISTOGRAM_COUNTS(
        "OfflinePages.Consistency.Persistent.PagesMissingDbEntryCount",
...
void PersistentPagesConsistencyCheckTask::OnCheckConsistencyDone(
  ...
  UMA_HISTOGRAM_ENUMERATION(
      "OfflinePages.Consistency.Persistent.Result", static_cast<int>(result),
