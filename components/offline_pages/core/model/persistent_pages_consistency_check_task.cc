...
SyncOperationResult CheckConsistencySync(
  ...
  if (offline_ids_to_delete.size() > 0) {
    ...
    LOG(ERROR) << "PersistentPagesConsistencyCheckTask deleted " << offline_ids_to_delete.size() << " offline pages";
  }
  ...
  if (files_to_delete.size() > 0) {
    ...
    LOG(ERROR) << "PersistentPagesConsistencyCheckTask deleted " << files_to_delete.size() << " files";
  }
...
void PersistentPagesConsistencyCheckTask::OnCheckConsistencyDone(
  ...
  TaskComplete();
  LOG(ERROR) << "PersistentPagesConsistencyCheckTask FINISHED";
}
