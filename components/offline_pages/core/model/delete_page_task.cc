...
bool GetDeletedPageInfoWrapperByOfflineIdSync(
  ...
  if (statement.Step()) {
    ...
    info_wrapper->file_path =
        OfflinePageStoreUtils::FromDatabaseFilePath(statement.ColumnString(2));
...
std::vector<DeletedPageInfoWrapper> GetDeletedPageInfoWrappersByClientIdSync(
  ...
  while (statement.Step()) {
    ...
    info_wrapper.file_path =
        OfflinePageStoreUtils::FromDatabaseFilePath(statement.ColumnString(1));
...
GetCachedDeletedPageInfoWrappersByUrlPredicateSync(
  ...
  while (statement.Step()) {
    ...
    info_wrapper.file_path =
        OfflinePageStoreUtils::FromDatabaseFilePath(statement.ColumnString(3));
