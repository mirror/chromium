...
const int64_t kRenovationTimeoutMs = 10000;

class FooRenovation : public PageRenovation {
 public:
...
class BarRenovation : public PageRenovation {
 public:
  bool ShouldRun(const GURL& url) const override {
...
class PageRenovatorBrowserTest : public content::ContentBrowserTest {
 ...
 protected:
  ...
  std::unique_ptr<RenderFrameScriptInjector> script_injector_;
...
void PageRenovatorBrowserTest::SetUpOnMainThread() {
  ...
  renovations.emplace_back(new FooRenovation);
  renovations.emplace_back(new BarRenovation);
  renovations.emplace_back(new AlwaysRenovation);
  ...
  script_injector_.reset(new RenderFrameScriptInjector(
...
IN_PROC_BROWSER_TEST_F(PageRenovatorBrowserTest, CorrectRenovationsRun) {
  ...
  // Timeout for renovation scripts.
  content::BrowserThread::PostDelayedTask(
      content::BrowserThread::UI, FROM_HERE,
      content::GetDeferredQuitTaskForRunLoop(run_loop_.get()),
      base::TimeDelta::FromMilliseconds(kRenovationTimeoutMs));

  // This should run FooRenovation and AlwaysRenovation, but not BarRenovation.
