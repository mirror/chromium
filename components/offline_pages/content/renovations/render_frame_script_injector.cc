...
void RenderFrameScriptInjector::Inject(base::string16 script,
  ...
  // base::Value&|.
  base::RepeatingCallback<void(const base::Value*)> js_callback;
  if (callback) {
    js_callback = base::Bind(
        [](ResultCallback callback, const base::Value* result) {
          base::Value new_result;
          if (result) {
            new_result = *result;
          }
  ...
  render_frame_host_->ExecuteJavaScriptInIsolatedWorld(
      script, std::move(js_callback), isolated_world_id_);
}
