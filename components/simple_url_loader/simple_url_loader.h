...
class SimpleURLLoader : public content::mojom::URLLoaderClient {
 ...
 private:
  ...
  // Called by BodyHandler when the stream has complete or there has been a mojo
  // error reading from the pipe. SimpleURLLoader will wait for OnComplete, if
  // not already received, and prefer any error message it returns.
  void OnBodyComplete(net::Error net_error);
  ...
  net::Error body_net_error_ = net::OK;
