...
class FeatureConfigStorageValidator : public StorageValidator {
 public:
  ...
  // Resets the full state of this validator to be used in tests. After calling
  // this method it is valid to call InitializeFeatures() again.
 ...
 private:
  ...
  // Contains an entry for each of the events that should be stored.
  std::unordered_set<std::string> should_store_event_names_;

  // Contains the longest time to store each event, as a number of days.
