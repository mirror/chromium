...
void RecordNotifyEvent(const std::string& event_name,
  ...
  for (auto it = features.begin(); it != features.end(); ++it) {
    const base::Feature* feature = it->first;
    const FeatureConfig& feature_config = it->second;
    ...
    if (feature_config.used.name == event_name) {
      ...
      base::RecordAction(base::UserMetricsAction(used_event_action.c_str()));
      break;
    }
  ...
  base::RecordAction(base::UserMetricsAction(event_action.c_str()));
}
...
void RecordShouldTriggerHelpUI(const base::Feature& feature,
  ...
  base::RecordAction(base::UserMetricsAction(name.c_str()));
