...
*   Make showing In-Product Help context aware.
    ...
    *   Other events might be required to have happened first that would make it
        ...
        *   Example: Having seen the Chrome offline dino 10 times the last week
            might make the user more happy if they are informed that they can
...
*   Users should be able to use try out a feature on their own for some time
    ...
    *   Only if they seem to not use it when we believe it would be helpful to
        them should we show In-Product Help.
...
tracked within **daily time buckets**. This tracking is done only
**locally on the device** itself.

The client-side backend uses the Chrome Variations framework to control how
often IPH should be shown for end users. It does this by setting thresholds
in the experiment params and compare these numbers to the local state.
...
First of all, you need to create a `base::Feature` that represents your
In-Product Help feature, that enables the whole feature to be controlled
server side. The name should be on the form:
...
First of all, to retrieve the `FeatureEngagementTracker` you need to use your
platform specific way for how to retrieve a `KeyedService`. For example for
desktop platforms and Android, you can use the `FeatureEngagementTrackerFactory`
in
