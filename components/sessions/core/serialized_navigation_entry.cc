...
sync_pb::SyncEnums_PageTransition ToSyncPageTransition(
  ...
  switch (ui::PageTransitionStripQualifier(transition_type)) {
    ...
    // Non-core values listed here althogh unreachable:
...
size_t SerializedNavigationEntry::ReplacedEntryData::EstimateMemoryUsage()
    const {
...
SerializedNavigationEntry SerializedNavigationEntry::FromSyncData(
  ...
  if (sync_data.has_replaced_navigation()) {
    navigation.replaced_entry_data_ = ReplacedEntryData();
    navigation.replaced_entry_data_->url =
        GURL(sync_data.replaced_navigation().url());
    navigation.replaced_entry_data_->timestamp = syncer::ProtoTimeToTime(
        sync_data.replaced_navigation().timestamp_msec());
    navigation.replaced_entry_data_->transition_type = FromSyncPageTransition(
        sync_data.replaced_navigation().page_transition());
  }
...
sync_pb::TabNavigation SerializedNavigationEntry::ToSyncData() const {
  ...
  if (replaced_entry_data_.has_value()) {
    ...
    replaced_navigation->set_url(replaced_entry_data_->url.spec());
    replaced_navigation->set_timestamp_msec(
        syncer::TimeToProtoTime(replaced_entry_data_->timestamp));
    replaced_navigation->set_page_transition(
        ToSyncPageTransition(replaced_entry_data_->transition_type));
  }
...
size_t SerializedNavigationEntry::EstimateMemoryUsage() const {
  ...
  return EstimateMemoryUsage(referrer_url_) +
         ...
         EstimateMemoryUsage(replaced_entry_data_.value_or(
             SerializedNavigationEntry::ReplacedEntryData())) +
