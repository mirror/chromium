...
base::string16 TemplateURLService::CleanUserInputKeyword(
  ...
  // The idea behind this and the other early returns in this function is to
  // only try to clean up the passed-in keyword to the extent likely necessary.
  // For example, "http:" should be allowed to be a registered keyword.
  // Without the early returns, the "http:" will be stripped from the keyword
  // and this function would return an empty keyword.
  if (GetTemplateURLForKeyword(result) != nullptr)
    return result;
