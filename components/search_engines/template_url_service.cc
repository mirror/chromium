...
void TemplateURLService::AddMatchingKeywords(
  ...
  AddMatchingKeywordsHelper(keyword_to_turl_and_length_, prefix,
                            supports_replacement_only, matches);
}
...
void TemplateURLService::AddMatchingDomainKeywords(
    ...
    bool supports_replacement_only,
    TURLsAndMeaningfulLengths* matches) {
  AddMatchingKeywordsHelper(keyword_domain_to_turl_and_length_, prefix,
                            supports_replacement_only, matches);
}
    ...
    if (iter->change_type() == syncer::SyncChange::ACTION_DELETE) {
      if (!existing_turl) {
        error = sync_error_factory_->CreateAndUploadError(
            FROM_HERE, "ProcessSyncChanges failed on ChangeType ACTION_DELETE");
        continue;
      }
      ...
      if (existing_turl) {
        error = sync_error_factory_->CreateAndUploadError(
            FROM_HERE, "ProcessSyncChanges failed on ChangeType ACTION_ADD");
        continue;
      }
      ...
      if (!existing_turl) {
        error = sync_error_factory_->CreateAndUploadError(
            FROM_HERE, "ProcessSyncChanges failed on ChangeType ACTION_UPDATE");
        continue;
      }
      ...
      error = sync_error_factory_->CreateAndUploadError(
          FROM_HERE, "ProcessSyncChanges received an ACTION_INVALID");
    }
  }
  ...
  for (SyncDataMap::const_iterator iter = sync_data_map.begin();
    ...
    if (local_turl) {
      ...
      if (sync_turl->last_modified() > local_turl->last_modified()) {
        ...
        merge_result.set_num_items_modified(merge_result.num_items_modified() +
                                            1);
...
void TemplateURLService::ProcessTemplateURLChange(
  ...
  changes.push_back(syncer::SyncChange(from_here, type, sync_data));
...
void TemplateURLService::AddToMap(TemplateURL* template_url) {
  const base::string16& keyword = template_url->keyword();
  keyword_to_turl_and_length_[keyword] = TURLAndMeaningfulLength(
...
TemplateURL* TemplateURLService::FindNonExtensionTemplateURLForKeyword(
  ...
  for (const auto& turl : template_urls_) {
    if ((turl->type() == TemplateURL::NORMAL) && (turl->keyword() == keyword))
      return turl.get();
  }
  return nullptr;
}
...
void TemplateURLService::ResolveSyncKeywordConflict(
  ...
  TemplateURL* loser =
      applied_turl_is_better ? unapplied_sync_turl : applied_sync_turl;
