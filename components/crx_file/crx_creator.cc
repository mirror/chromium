...
bool WriteBuffer(base::File* file, const uint8_t buffer[], int len) {
  static_assert(sizeof(char) == sizeof(uint8_t), "Unsupported char size.");
  return file->WriteAtCurrentPos(reinterpret_cast<const char*>(buffer), len) ==
         len;
}
...
CreatorResult Create(const base::FilePath& output_path,
  ...
  static_assert(sizeof(char) == sizeof(uint8_t), "Unsupported char size.");
  if (!WriteBuffer(&crx, reinterpret_cast<const uint8_t*>(kCrxFileHeaderMagic),
                   kCrxFileHeaderMagicSize) ||
      !WriteBuffer(&crx, format_version_octets,
                   arraysize(format_version_octets)) ||
      !WriteBuffer(&crx, header_size_octets, arraysize(header_size_octets)) ||
      !WriteBuffer(&crx, reinterpret_cast<const uint8_t*>(header_str.c_str()),
                   header_str.length()) ||
