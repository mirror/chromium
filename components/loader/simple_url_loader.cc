...
class SimpleURLLoader::BodyHandler {
 public:
  ...
  // either SimpleURLLoader::OnBodyComplete() when the body pipe is closed, or
  // SimpleURLLoader::FinishWithResult() with an error code if an error occurs.
  // Must not call both.
 ...
 public:
  ...
  void InvokeCallback() override {
    ...
    std::move(body_as_string_callback_)
        .Run(simple_url_loader(), std::move(body_));
  }
 ...
 private:
  ...
  void ReadData() {
    while (true) {
      ...
      // If the pipe was closed, unclear if it was an error of success. Notify
      // the SimpleURLLoader of how much data was received.
      if (result != MOJO_RESULT_OK) {
        ClosePipe();
        simple_url_loader()->OnBodyComplete(body_->size());
        return;
      }

      // Check size against th elimit.
...
void SimpleURLLoader::OnComplete(
  ...
  // No result should have been reported yet.
  DCHECK_EQ(result_net_error_, net::ERR_IO_PENDING);
...
void SimpleURLLoader::OnConnectionError() {
  ...
  request_completed_ = true;
  request_net_error_ = net::ERR_FAILED;
  MaybeComplete();
}

void SimpleURLLoader::OnBodyComplete(int64_t received_body_size) {
