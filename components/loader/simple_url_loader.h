...
class SimpleURLLoader : public content::mojom::URLLoaderClient {
 public:
  ...
  // of unbounded size, potentially causing a crash if the amount of addressable
  // memory is excessded. It's recommended consumers use DownloadToString
  // instead.
 ...
 private:
  ...
  // Completes the request if OnComplete() either no body pipe was ever
  // received, or the body pipe was closed.
  void MaybeComplete();
