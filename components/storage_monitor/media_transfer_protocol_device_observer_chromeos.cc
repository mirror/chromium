...
bool MediaTransferProtocolDeviceObserverChromeOS::GetStorageInfoForPath(
  ...
  // device location.
  StorageLocationToInfoMap::const_iterator info_it = storage_map_.find(
      storage_info_utils::GetDeviceLocationFromStorageName(path_components[1]));
...
void MediaTransferProtocolDeviceObserverChromeOS::StorageChanged(
  ...
  if (!is_attached) {
    StorageLocationToInfoMap::iterator it = storage_map_.find(
        storage_info_utils::GetDeviceLocationFromStorageName(storage_name));
  ...
  std::string device_id =
      storage_info_utils::GetDeviceIdFromStorageInfo(*mtp_storage_info);
  base::string16 storage_label =
      storage_info_utils::GetDeviceLabelFromStorageInfo(*mtp_storage_info);
  std::string location =
      storage_info_utils::GetDeviceLocationFromStorageName(storage_name);
