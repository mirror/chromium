...
namespace {
using MtpStorageInfoPtr = device::mojom::MtpStorageInfoPtr;
...
std::string GetDeviceIdFromStorageInfo(const MtpStorageInfoPtr& storage_info) {
...
base::string16 GetDeviceLabelFromStorageInfo(
    const MtpStorageInfoPtr& storage_info) {
...
void GetStorageInfo(const std::string& storage_name,
  ...
  DCHECK(!storage_name.empty());
  MtpStorageInfoPtr storage_info = mtp_manager->GetStorageInfo(storage_name);

  if (storage_info.is_null())
    return;
