...
class StorageMonitorCros : public StorageMonitor,
 ...
 private:
  ...
  // Adds the mount point in |mount_info| to |mount_map_| and send a device
  // attach notification.
  void AddDisk(const chromeos::disks::DiskMountManager::Disk& disk);

  // Removes the mount point in |mount_info| to |mount_map_| and send a device
  // detach notification.
  void RemoveDisk(const chromeos::disks::DiskMountManager::Disk& disk);
