...
TEST_F(StorageMonitorCrosTest, FixedStroageTest) {
  ...
  // Fixed storage added
  const std::string mount_point = "/dev/sda1";
  ...
  const std::string new_mount_point = "/dev/sda2";
  const chromeos::disks::DiskMountManager::Disk remounted_disk(
      "", new_mount_point, false, "", "", label, "", "", "", "", "", uuid, "",
      chromeos::DEVICE_TYPE_UNKNOWN, 0, false, false, false, false, false,
      false, false, "", "");
  monitor_->OnDiskEvent(DiskMountManager::DiskEvent::DISK_CHANGED,
                        &remounted_disk);
  disks = monitor_->GetAllAvailableStorages();
  EXPECT_EQ(1U, disks.size());
  EXPECT_EQ(new_mount_point, disks[0].location());
  EXPECT_EQ(base::UTF8ToUTF16(label), disks[0].storage_label());

  // Fixed storage removed
  monitor_->OnDiskEvent(DiskMountManager::DiskEvent::DISK_REMOVED,
                        &remounted_disk);
  disks = monitor_->GetAllAvailableStorages();
