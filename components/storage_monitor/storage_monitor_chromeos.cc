...
void StorageMonitorCros::OnBootDeviceDiskEvent(
  ...
  switch (event) {
    ...
    default:
      // Fall back for DiskMountManager::DiskEvent::DISK_CHANGED. It should not
...
void StorageMonitorCros::AddDisk(const DiskMountManager::Disk& disk) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);

  // Only track stateful partition, because it is meaningful for the user.
...
void StorageMonitorCros::RemoveDisk(const DiskMountManager::Disk& disk) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
