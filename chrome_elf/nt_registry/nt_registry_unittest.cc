...
TEST_F(NtRegistryTest, API_ENUMERATION) {
  ...
  // Clean up
  // ------------------------------
  EXPECT_TRUE(nt::DeleteRegKey(nt::HKCU, nt::NONE, subkey1));
  EXPECT_TRUE(nt::DeleteRegKey(nt::HKCU, nt::NONE, subkey2));
  EXPECT_TRUE(nt::DeleteRegKey(nt::HKCU, nt::NONE, subkey3));
  EXPECT_TRUE(nt::DeleteRegKey(key_handle));
  nt::CloseRegKey(key_handle);
}
