...
bool ReadInArray(HANDLE file) {
  ...
  assert(g_module_array_size <= UINT_MAX / sizeof(PackedWhitelistModule));
  DWORD buffer_size =
  ...
  if (!::ReadFile(file, g_module_array, buffer_size, &bytes_read, FALSE) ||
      ...
      !std::is_sorted(g_module_array, g_module_array + g_module_array_size,
                      HashBinaryPredicate)) {
...
FileStatus InitInternal() {
  ...
  if (file == INVALID_HANDLE_VALUE)
    return FileStatus::kNoComponentFile;

  bool success = ReadInArray(file);
  ::CloseHandle(file);

  return success ? FileStatus::kSuccess : FileStatus::kInitArrayFail;
}
