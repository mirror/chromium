...
const wchar_t* const kTestBins[] = {L"ole32.dll", L"gdi32.dll", L"gdi32.dll",
                                    L"crypt32.dll"};
const char* const kTestBinBasenames[] = {"ole32.dll", "gdi32.dll", "gdi32.dll",
                                         "crypt32.dll"};

constexpr DWORD kPageSize = 4096;
constexpr uint32_t kTestArraySize = arraysize(kTestBins);
static_assert(kTestArraySize == arraysize(kTestBinBasenames),
...
bool GetTestModules(std::vector<TestModule>* test_modules,
  ...
  for (size_t i = 0; i < kTestArraySize; i++) {
    ...
    test_module.basename.assign(kTestBinBasenames[i]);
