...
DWORD CalculateExtraSpaceNeeded(DWORD string_length) {
  ...
  DWORD temp = (string_length > 3) ? (string_length - 3) : 0;

  // The full entry needs to be 32-bit aligned.  Get the modulo, and leave 0
  // alone.
  temp = (sizeof(LogEntry) + temp) % 4;
  if (temp) {
...
void TranslateEntry(bool blocked, const LogEntryInternal& src, LogEntry* dst) {
  ...
  ::memset(dst->path + dst->path_len + 1, 0, dst->path_extra_size);
...
void AddLoadAttemptLog(bool blocked,
  ...
  ::memcpy(&entry.basename_hash, &basename_hash, sizeof(entry.basename_hash));
  ::memcpy(&entry.code_id_hash, &code_id_hash, sizeof(entry.code_id_hash));
...
uint32_t DrainLog(uint8_t* buffer,
  ...
  for (auto entry : blocked) {
    entry_size = sizeof(LogEntry) + entry.full_path.size();
