...
bool ReadInArray(HANDLE file) {
  ...
  *module_array_size = metadata.module_count;
  ...
  assert((*module_array_size * sizeof(PackedWhitelistModule)) <= UINT_MAX);
  DWORD buffer_size =
      static_cast<DWORD>(*module_array_size * sizeof(PackedWhitelistModule));
  *module_array =
...
ComponentStatus InitComponent() {
  ...
  assert(*GetComponentArray() == nullptr);
