...
bool IsMicrosoftIme(const wchar_t* ime_guid) {
  ...
  assert(
      std::is_sorted(kMicrosoftImeGuids, kMicrosoftImeGuids + kGuidLength + 1));

  // The binary predicate (compare function) must return true if the first
  // argument is ordered before the second.
  return std::binary_search(kMicrosoftImeGuids,
                            kMicrosoftImeGuids + kGuidLength + 1, ime_guid,
...
const uint32_t kGuidLength = ::wcslen(kMicrosoftImeGuids[0]);
