...
bool GetModuleData(const wchar_t* path,
  ...
  assert(path && is_32_bit && size_of_image && time_date_stamp);
...
IMEStatus InitIMEs() {
  std::vector<IME>* ime_list = GetImeList();
  ...
  if (!nt::OpenRegKey(nt::HKLM, kImeRegistryKey, KEY_READ, &key_handle,
    ...
    if (ntstatus == STATUS_OBJECT_NAME_NOT_FOUND)
      return kImeMissingRegKey;
    if (ntstatus == STATUS_ACCESS_DENIED)
      return kImeRegAccessDenied;
    return kImeErrorGeneric;
  }

  ULONG count = 0;
  if (!nt::QueryRegEnumerationInfo(key_handle, &count)) {
    nt::CloseRegKey(key_handle);
    return kImeGenericRegFailure;
  }
  ...
  for (ULONG i = 0; i < count; i++) {
    ...
    bool target_32 = false;
    DWORD size = 0;
    DWORD date_time = 0;
    if (!GetModuleData(dll_path.c_str(), &target_32, &size, &date_time)) {
      assert(false);
      continue;
    }

    // No need to add IME DLLs with conflicting bitness to the list.
    if (self_32 != target_32)
      continue;

    // For each guid: store guid, size, timestamp, path.
    ime_list->emplace_back(subkey_name, size, date_time, dll_path);
  }
