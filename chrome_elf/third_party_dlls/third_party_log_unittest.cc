...
#include "chrome_elf/third_party_dlls/third_party_log.h"
...
#include "chrome_elf/third_party_dlls/third_party_packed_list_format.h"
...
struct NotificationHandlerParam {
...
// - |lpParameter| should be a NotificationHandlerParam*.
// - Returns 0 for successful retrieval of expected number of LogEntries.
DWORD WINAPI NotificationHandler(LPVOID lpParameter) {
  NotificationHandlerParam* args =
      reinterpret_cast<NotificationHandlerParam*>(lpParameter);
...
TEST(ThirdParty, LogNotifications) {
  ...
  NotificationHandlerParam lpParam;
  lpParam.logs_expected = arraysize(kTestLogs);
  lpParam.notification_event.reset(
      new base::WaitableEvent(base::WaitableEvent::ResetPolicy::AUTOMATIC,
                              base::WaitableEvent::InitialState::NOT_SIGNALED));

  // Register the event.
  ASSERT_TRUE(RegisterLogNotification(lpParam.notification_event->handle()));

  // Fire off a thread to handle the notifications.
  HANDLE thread =
      ::CreateThread(nullptr, 0, &NotificationHandler, &lpParam, 0, nullptr);

  for (size_t i = 0; i < lpParam.logs_expected; ++i) {
    // Add blocked entries - type doesn't matter in this test.
    LogLoadAttempt(LogType::kBlocked, kTestLogs[i].basename_hash,
                   kTestLogs[i].code_id_hash, nullptr);
  }

  EXPECT_EQ(::WaitForSingleObject(thread, kWaitTimeoutMs * 2), WAIT_OBJECT_0);
  DWORD exit_code = 1;
