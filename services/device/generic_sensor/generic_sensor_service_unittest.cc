...
void RunLoopQuit(base::RunLoop* run_loop) {
  run_loop->Quit();
}
...
class TestSensorClient : public mojom::SensorClient {
 public:
  ...
  void SetBarrierClosure(base::RepeatingClosure barrier_closure) {
    barrier_closure_ = std::move(barrier_closure);
  }
 ...
 private:
  ...
  // Test Clients set |quit_closure_| or |barrier_closure_| and start a RunLoop
  // in main thread, then expect those closure will quit the RunLoop in
  // SensorReadingChanged(). In this way we guarantee the SensorReadingChanged()
  // does be triggered.
...
TEST_F(GenericSensorServiceTest, GetSensorTest) {
  ...
  sensor_provider_->GetSensor(
      ...
      base::Bind(&TestSensorClient::OnSensorCreated,
...
TEST_F(GenericSensorServiceTest, GetDefaultConfigurationTest) {
  ...
  sensor->GetDefaultConfiguration(
      base::Bind(&TestSensorClient::OnGetDefaultConfiguration,
                 base::Unretained(client.get()), run_loop.QuitClosure()));
...
TEST_F(GenericSensorServiceTest, ValidAddConfigurationTest) {
  ...
  client->SetQuitClosure(
      base::BindOnce(&RunLoopQuit, base::Unretained(&run_loop)));
  run_loop.Run();
}
...
TEST_F(GenericSensorServiceTest, InvalidAddConfigurationTest) {
  ...
  sensor->AddConfiguration(
      configuration,
      base::Bind(&TestSensorClient::OnAddOrRemoveConfiguration,
                 base::Unretained(client.get()),
...
TEST_F(GenericSensorServiceTest, MultipleClientsTest) {
  ...
  // Expect the SensorReadingChanged() will be called for both clients.
  {
    ...
    base::RepeatingClosure barrier_closure = base::BarrierClosure(
        2, base::BindOnce(&RunLoopQuit, base::Unretained(&run_loop)));
    client_1->SetCheckValueCallback(base::BindOnce(&CheckValue, 48.0));
    client_1->SetBarrierClosure(barrier_closure);
    client_2->SetCheckValueCallback(base::BindOnce(&CheckValue, 48.0));
    client_2->SetBarrierClosure(barrier_closure);
    run_loop.Run();
  }
}
...
TEST_F(GenericSensorServiceTest, ClientMojoConnectionBrokenTest) {
  ...
  auto client_2 = base::MakeUnique<TestSensorClient>();
  {
    ...
    sensor_provider_->GetSensor(
        ...
        base::Bind(&TestSensorClient::OnSensorCreated,
                   base::Unretained(client_2.get()), run_loop.QuitClosure()));
    run_loop.Run();
  }
