...
TEST_F(PlatformSensorAndProviderTestWin,
  ...
  SensorReadingSharedBuffer* buffer =
      static_cast<SensorReadingSharedBuffer*>(mapping.get());
  EXPECT_THAT(buffer->reading.gyro.x, -x_ang_accel * kDegToRad);
  EXPECT_THAT(buffer->reading.gyro.y, -y_ang_accel * kDegToRad);
  EXPECT_THAT(buffer->reading.gyro.z, -z_ang_accel * kDegToRad);
  EXPECT_TRUE(sensor->StopListening(client.get(), configuration));
}
