...
std::unique_ptr<PlatformSensorFusionAlgorithm>
CreateLinearAccelerationSensor() {
...
//   B: ACCELEROMETER
std::unique_ptr<PlatformSensorFusionAlgorithm>
CreateRelativeOrientationEulerAnglesSensor() {
  return base::MakeUnique<
...
std::unique_ptr<PlatformSensorFusionAlgorithm>
CreateRelativeOrientationQuaternionSensor() {
  return base::MakeUnique<OrientationQuaternionFusionAlgorithmUsingEulerAngles>(
      false /* absolute */);
}
...
void PlatformSensorProviderLinux::CreateFusionSensor(
  ...
  std::unique_ptr<PlatformSensorFusionAlgorithm> fusion_algo;
  switch (type) {
    case mojom::SensorType::LINEAR_ACCELERATION:
      fusion_algo = CreateLinearAccelerationSensor();
      break;
    case mojom::SensorType::RELATIVE_ORIENTATION_EULER_ANGLES:
      fusion_algo = CreateRelativeOrientationEulerAnglesSensor();
      break;
    case mojom::SensorType::RELATIVE_ORIENTATION_QUATERNION:
      fusion_algo = CreateRelativeOrientationQuaternionSensor();
      break;
    default:
      NOTREACHED();
  }
  ...
  DCHECK(fusion_algo);
  PlatformSensorFusion::Create(std::move(mapping), this, std::move(fusion_algo),
                               callback);
}
