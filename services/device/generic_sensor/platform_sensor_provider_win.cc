...
class PlatformSensorProviderWin::SensorThread final : public base::Thread {
 public:
  ...
  base::win::ScopedComPtr<ISensorManager> SensorManager() {
...
PlatformSensorProviderWin::CreateSensorReader(mojom::SensorType type) {
  ...
  if (!sensor_thread_->SensorManager())
    return nullptr;
  return PlatformSensorReaderWin::Create(type, sensor_thread_->SensorManager());
}
