...
AbsoluteOrientationEulerAnglesFusionAlgorithmUsingAccelerometerAndMagnetometer::
    AbsoluteOrientationEulerAnglesFusionAlgorithmUsingAccelerometerAndMagnetometer() {
  gravity_reading_.values[0].value() = 0.0;
  gravity_reading_.values[1].value() = 0.0;
  gravity_reading_.values[2].value() = 0.0;
  gravity_reading_.values[3].value() = 0.0;
}
...
bool AbsoluteOrientationEulerAnglesFusionAlgorithmUsingAccelerometerAndMagnetometer::
  ...
  // angles.
  DCHECK(readings.size() == 2);

  // Only generate a new sensor value when the accelerometer reading changes.
  if (!IsReadingSignificantlyDifferent(gravity_reading_, readings[0]))
    return false;

  gravity_reading_ = readings[0];

  double gravity_x = readings[0].values[0].value();
  double gravity_y = readings[0].values[1].value();
  double gravity_z = readings[0].values[2].value();
  double geomagnetic_x = readings[1].values[0].value();
  double geomagnetic_y = readings[1].values[1].value();
  double geomagnetic_z = readings[1].values[2].value();
