...
TEST_F(PlatformSensorFusionTest, SourceSensorWorksSeparately) {
  CreateAccelerometer();
  auto accel = provider_->GetSensor(SensorType::ACCELEROMETER);
  EXPECT_TRUE(accel);
  EXPECT_FALSE(accel->IsActive());
