...
class TestWakeLockProvider::TestWakeLock : public mojom::WakeLock,
 ...
 private:
  ...
  // True after RequestWakeLock is called.
  bool active_ = false;

  DISALLOW_COPY_AND_ASSIGN(TestWakeLock);
};

TestWakeLockProvider::TestWakeLockProvider() : binding_(this) {}
...
void TestWakeLockProvider::OnBindInterface(
  ...
  binding_.Bind(mojom::WakeLockProviderRequest(std::move(interface_pipe)));
}

void TestWakeLockProvider::OnWakeLockRequested(TestWakeLock* wake_lock) {
  CHECK(!active_wake_locks_.count(wake_lock));
