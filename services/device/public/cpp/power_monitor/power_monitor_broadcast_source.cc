...
PowerMonitorBroadcastSource::PowerMonitorBroadcastSource(
    ...
    : client_(base::MakeUnique<ClientImpl>()), task_runner_(task_runner) {
  if (connector) {
    task_runner_->PostTask(
        FROM_HERE,
        base::BindOnce(&PowerMonitorBroadcastSource::ClientImpl::Init,
...
PowerMonitorBroadcastSource::ClientImpl::ClientImpl()
    : last_reported_battery_power_state_(false), binding_(this) {}

PowerMonitorBroadcastSource::ClientImpl::~ClientImpl() {}

void PowerMonitorBroadcastSource::ClientImpl::Init(
...
bool PowerMonitorBroadcastSource::ClientImpl::
    last_reported_battery_power_state() {
  return base::subtle::NoBarrier_Load(&last_reported_battery_power_state_) != 0;
}

void PowerMonitorBroadcastSource::ClientImpl::PowerStateChange(
...
void PowerMonitorBroadcastSource::ClientImpl::Suspend() {
  ProcessPowerEvent(PowerMonitorSource::SUSPEND_EVENT);
}

void PowerMonitorBroadcastSource::ClientImpl::Resume() {
