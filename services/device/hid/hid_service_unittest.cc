...
class HidServiceTest : public DeviceServiceTestBase {
 public:
  HidServiceTest() {}
  ...
  void SetUp() override {
    ...
    HidService::SetHidServiceForTesting(std::move(mock_hid_service));
...
// Test the GetDevices.
TEST_F(HidServiceTest, GetDevicesOnly) {
  // Add two hid devices
  scoped_refptr<device::HidDeviceInfo> device0 = new device::HidDeviceInfo(
      kTestDeviceIds[0], 0, 0, "Hid Service Unit Test", "HidDevice-0",
      device::mojom::HidBusType::kHIDBusTypeUSB, std::vector<uint8_t>());

  scoped_refptr<device::HidDeviceInfo> device1 = new device::HidDeviceInfo(
...
// interface.
TEST_F(HidServiceTest, GetDevicesAndSetClient) {
  // Add one hid device.
  scoped_refptr<device::HidDeviceInfo> device0 = new device::HidDeviceInfo(
      kTestDeviceIds[0], 0, 0, "Hid Service Unit Test", "HidDevice-0",
      device::mojom::HidBusType::kHIDBusTypeUSB, std::vector<uint8_t>());
...
TEST_F(HidServiceTest, TestHidConnectionInterface) {
  // Add one hid device.
