...
void DeviceService::BindPublicIpAddressGeolocationProviderRequest(
  ...
  // When posting to io_task_runner_,
  // public_ip_address_geolocation_provider_ is safe to pass as base::Unretained
  // because it is destroyed via a task posted to io_task_runner_ (from
  // ~DeviceService()).
