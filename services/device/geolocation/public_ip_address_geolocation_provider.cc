...
void PublicIpAddressGeolocationProvider::CreateGeolocation(
  ...
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  DCHECK(public_ip_address_location_notifier_);
  // geolocation_binding_set_.AddBinding(std::make_unique<PublicIpAddressGeolocator>(tag,
  // public_ip_address_location_notifier_,
  // base::Bind(&PublicIpAddressGeolocationProvider::OnBadMessage,
  //            base::Unretained(this))), std::move(request));
}

void PublicIpAddressGeolocationProvider::OnBadMessage(
    const std::string& error) {
  geolocation_binding_set_.ReportBadMessage(error);
  geolocation_binding_set_.CloseAllBindings();
}
