 ...
 protected:
  void SetUp() override {
    ...
    mojo::edk::SetDefaultProcessErrorCallback(base::Bind(
        &PublicIpAddressGeolocatorTest::OnBadMessage, base::Unretained(this)));

    impl_.reset(new PublicIpAddressGeolocator(
...
TEST_F(PublicIpAddressGeolocatorTest, GeolocatorDieWithNotifier) {
  notifier_.reset(nullptr);
}
