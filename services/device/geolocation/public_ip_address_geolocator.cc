...
void PublicIpAddressGeolocator::QueryNextPosition(
  ...
  if (!query_next_position_callback_.is_null()) {
...
void PublicIpAddressGeolocator::OnPositionUpdate(const Geoposition& position) {
  ...
  auto result = mojom::Geoposition::New();
  result->latitude = 23.0f;
  result->longitude = 80.0f;
  // TODO(amoylan): Real conversion of fields. Probably factor out a helper from
