...
constexpr char kTestTrafficAnnoationTag[] = R"(
  setting:
    "Just for test."
  chrome_policy {
  })";
...
GetTestPartialNetworkTrafficAnnotationTag() {
  return net::DefinePartialNetworkTrafficAnnotation("public_ip_geolocation",
                                                    "network_location_request",
                                                    kTestTrafficAnnoationTag);
}
...
class PublicIpAddressLocationNotifierTest : public testing::Test {
 protected:
  ...
  void ExpectValidPosition(const base::Optional<mojom::Geoposition>& position,
    ...
    EXPECT_THAT(position->latitude, testing::Eq(latitude));
  }
...
TEST_F(PublicIpAddressLocationNotifierTest, SingleQueryReturns) {
  ...
  notifier_.QueryNextPositionAfterTimestamp(
      base::Time::Now(), GetTestPartialNetworkTrafficAnnotationTag(),
      query.MakeCallback());
...
TEST_F(PublicIpAddressLocationNotifierTest, OlderQueryReturnsCached) {
  ...
  notifier_.QueryNextPositionAfterTimestamp(
      time, GetTestPartialNetworkTrafficAnnotationTag(),
  ...
  notifier_.QueryNextPositionAfterTimestamp(
      time - base::TimeDelta::FromMinutes(5),
      GetTestPartialNetworkTrafficAnnotationTag(), query_2.MakeCallback());
...
TEST_F(PublicIpAddressLocationNotifierTest,
  ...
  TestPositionQuery query_2;
  notifier_.QueryNextPositionAfterTimestamp(
      base::Time::FromDoubleT(query_1.position()->timestamp),
      GetTestPartialNetworkTrafficAnnotationTag(), query_2.MakeCallback());
...
TEST_F(PublicIpAddressLocationNotifierTest, MutipleWaitingQueries) {
  ...
  notifier_.QueryNextPositionAfterTimestamp(
      base::Time::FromDoubleT(query_1.position()->timestamp),
      GetTestPartialNetworkTrafficAnnotationTag(), query_3.MakeCallback());
