 ...
 protected:
  ...
  class TestPositionQuery {
   public:
    ...
    const base::Optional<Geoposition>& position() const { return position_; }

   private:
    void OnQueryNextPositionResponse(const Geoposition& position) {
      position_ = position;
      loop_.Quit();
    }

    base::RunLoop loop_;
    base::Optional<Geoposition> position_;
  };
  ...
  // |latitude|.
  void ExpectValidPosition(const base::Optional<Geoposition>& position,
                           const float latitude) {
...
TEST_F(PublicIpAddressLocationNotifierTest, OlderQueryReturnsCached) {
  ...
  notifier_.QueryNextPositionAfterTimestamp(time, query_1.MakeCallback());
  net::TestURLFetcher* const fetcher = GetTestUrlFetcher();
  EXPECT_THAT(fetcher, testing::NotNull());
  RespondToFetchWithLatitude(fetcher, 1.0f);
  ...
  notifier_.QueryNextPositionAfterTimestamp(
      time - base::TimeDelta::FromMinutes(5), query_2.MakeCallback());
...
TEST_F(PublicIpAddressLocationNotifierTest,
  ...
  TestPositionQuery query_2;
  notifier_.QueryNextPositionAfterTimestamp(query_1.position()->timestamp,
