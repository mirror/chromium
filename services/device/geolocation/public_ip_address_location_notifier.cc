...
constexpr char kGenericTrafficAnnoationTag[] = R"(
  setting:
    "This request is merged from several clients."
    "Each client has separate settings."
  chrome_policy {
  })";
}  // namespace
...
void PublicIpAddressLocationNotifier::QueryNextPositionAfterTimestamp(
  ...
  if (latest_geoposition_.has_value() &&
      base::Time::FromDoubleT(latest_geoposition_->timestamp) > timestamp) {
...
void PublicIpAddressLocationNotifier::MakeNetworkLocationRequestWithContext(
  ...
  // If there's only 1 client, use their tag, else will use generic tag.
      ...
      partial_traffic_annotation =
          std::make_unique<net::PartialNetworkTrafficAnnotationTag>(
              net::DefinePartialNetworkTrafficAnnotation(
                  "public_ip_geolocation", "network_location_request",
                  kGenericTrafficAnnoationTag));
  if (client_partial_network_annotation_tags_.size() == 1) {
    partial_traffic_annotation.reset(
        &(*client_partial_network_annotation_tags_.begin()));
  }
...
void PublicIpAddressLocationNotifier::
    AddClientPartialNetworkTrafficAnnotationTag(
        const net::PartialNetworkTrafficAnnotationTag tag) {
  client_partial_network_annotation_tags_.insert(tag);
}

// Remove the partial network traffic annotation for the client.
void PublicIpAddressLocationNotifier::
    RemoveClientPartialNetworkTrafficAnnotationTag(
        const net::PartialNetworkTrafficAnnotationTag tag) {
  const auto iter = client_partial_network_annotation_tags_.find(tag);
  DCHECK(iter != client_partial_network_annotation_tags_.end());
  if (iter != client_partial_network_annotation_tags_.end()) {
    client_partial_network_annotation_tags_.erase(iter);
  }
}
}  // namespace device
