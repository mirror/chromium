...
// * Must be created and destroyed on the same sequence.
// * Subscribers must subscribe using the same sequence as the |task_runner|
//   with which this class is constructed (which need not be the
//   creation/destruction sequence).
class PublicIpAddressLocationNotifier
    : public net::NetworkChangeNotifier::NetworkChangeObserver {
 public:
  // Creates a notifier that will interact with subscribers on the specified
  // |task_runner|. Network location requests use the specified Google |api_key|
  // and use a URL request context produced by |request_context_producer|.
 ...
 private:
  // Sequence checker for tasks which must run on the same sequence that this
  // object is created on.
  ...
  // The parts of startup which run on the |task_runner| passed to the
  // constructor.
