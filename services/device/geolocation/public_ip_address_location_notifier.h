...
class PublicIpAddressLocationNotifier
 ...
 public:
  ...
  // |time_of_prev_position|. Specifically: * If a position has been obtained
  // subsequent to |time_of_prev_position|, returns it. * Otherwise, returns an
  // updated position once a network change has occurred. * Note that it is
  // possible for |callback| to never be called if no network change ever occurs
  // // * after |time_of_prev_position|.
  void QueryNextPositionAfterTimestamp(
      base::Time timestamp,
      const net::PartialNetworkTrafficAnnotationTag& tag,
      QueryNextPositionCallback callback);

  // Callback to be executed if the Notifier dies.
  typedef base::Callback<void()> OnDestructionCallback;

  // Add destruction callback.
  std::unique_ptr<base::CallbackList<void()>::Subscription>
  AddDestructionCallback(const OnDestructionCallback& callback);
 ...
 private:
  ...
  // Clients waiting for the death of PublicIpAddressLocationNotifier.
  base::CallbackList<void()> on_destruction_callbacks_;
