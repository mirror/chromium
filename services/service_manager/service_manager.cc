...
void ReportBlockedInterface(const std::string& source_service_name,
  ...
  LOG(ERROR)
      ...
      << "\" through some capability and that \"" << source_service_name
...
class ServiceManager::Instance
 ...
 private:
  ...
  void StartService(const Identity& in_target,
    ...
    Identity target = in_target;
    mojom::ConnectResult result =
        ValidateConnectParams(&target, nullptr, nullptr, base::nullopt);
    if (!Succeeded(result)) {
      std::move(callback).Run(result, Identity());
      return;
    }
  ...
  void StartServiceWithProcess(
    ...
    mojom::ConnectResult result = ValidateConnectParams(
        &target, &service, &pid_receiver_request, base::nullopt);
    if (!Succeeded(result)) {
      std::move(callback).Run(result, Identity());
      return;
    }
  ...
  mojom::ConnectResult ValidateConnectParams(
      ...
      mojom::ServicePtr* service,
      mojom::PIDReceiverRequest* pid_receiver_request,
      base::Optional<std::string> target_interface_name) {
    ...
    if (!Succeeded(result))
      return result;
    return ValidateConnectionSpec(*target, std::move(target_interface_name));
  }
  ...
  mojom::ConnectResult ValidateConnectionSpec(
    ...
    if (target_interface_name.has_value()) {
