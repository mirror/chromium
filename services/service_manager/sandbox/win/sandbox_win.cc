...
sandbox::ResultCode GetAppContainerProfile(
  ...
  // Generate a unique GPU sandbox AC profile for the current install based
  // on the SHA1 hash of the main executable's path. This does not need to
  // be secure so using SHA1 isn't an issue.
  ...
  auto profile = sandbox::AppContainerProfile::Create(
  ...
  if (!profile->AccessCheck(file_exe.value().c_str(), SE_FILE_OBJECT,
...
sandbox::ResultCode SandboxWin::AddAppContainerPolicy(
    sandbox::TargetPolicy* policy,
    service_manager::SandboxType sandbox_type) {
  DCHECK(IsAppContainerEnabledForSandbox(sandbox_type));

  scoped_refptr<sandbox::AppContainerProfile> profile;
  sandbox::ResultCode result = GetAppContainerProfile(sandbox_type, &profile);
  if (result != sandbox::SBOX_ALL_OK)
    return result;
...
bool SandboxWin::IsAppContainerEnabledForSandbox(
  ...
  if (command_line.HasSwitch(
    ...
    return false;
  if (command_line.HasSwitch(service_manager::switches::kEnableGpuAppContainer))
    return true;
  return base::StartsWith(appcontainer_group_name, "Enabled",
                          base::CompareCase::INSENSITIVE_ASCII);
}
