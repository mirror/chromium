...
class Gpu::EstablishRequest
 ...
 public:
  ...
  void FinishOnMain() {
    ...
    finished_ = true;
    // |this| may be deleted running Gpu::OnEstablishedGpuChannel().
...
void Gpu::SendEstablishGpuChannelRequest() {
  ...
  io_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&EstablishRequest::SendRequest, pending_request_, gpu_ptr_));
}
...
void Gpu::OnEstablishedGpuChannel() {
  ...
  if (pending_request_->client_id() &&
    ...
    gpu_channel_ = gpu::GpuChannelHost::Create(
        ...
        IPC::ChannelHandle(pending_request_->channel_handle().release()),
