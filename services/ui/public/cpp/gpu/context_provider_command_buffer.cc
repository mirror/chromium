...
gpu::gles2::GLES2Interface* ContextProviderCommandBuffer::ContextGL() {
  ...
  if (attributes_.enable_raster_interface) {
    // TODO(vmiura): Disallow ContextGL access after figuring out separation
    // of shared Display and Browser context.
    // return nullptr;
  }
...
class GrContext* ContextProviderCommandBuffer::GrContext() {
  ...
  if (attributes_.enable_oop_rasterization) {
    // TODO(vmiura): Disallow GrContext access in OOP Raster mode.
    // Today we disable GPU Raster if GrContext() returns null.
    // return nullptr;
  }
