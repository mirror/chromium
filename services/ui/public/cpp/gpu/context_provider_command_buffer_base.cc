...
ContextProviderCommandBufferBase::SharedProviders::SharedProviders() = default;
ContextProviderCommandBufferBase::SharedProviders::~SharedProviders() = default;

ContextProviderCommandBufferBase::ContextProviderCommandBufferBase(
...
ContextProviderCommandBufferBase::GetCommandBufferProxy() {
  return command_buffer_.get();
}

uint32_t ContextProviderCommandBufferBase::GetCopyTextureInternalFormat() {
...
gpu::ContextResult ContextProviderCommandBufferBase::BindToCurrentThread() {
  ...
  {
    ContextProviderCommandBufferBase* shared_context_provider = nullptr;
  ...
  gles2_impl_->SetLostContextCallback(
      base::Bind(&ContextProviderCommandBufferBase::OnLostContext,
  ...
  base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(
      this, "ContextProviderCommandBufferBase", std::move(task_runner));
  return bind_result_;
}

gpu::gles2::GLES2Interface* ContextProviderCommandBufferBase::ContextGL() {
  DCHECK(bind_tried_);
...
ContextProviderCommandBufferBase::RasterInterface() {
  DCHECK(bind_tried_);
  DCHECK_EQ(bind_result_, gpu::ContextResult::kSuccess);
  CheckValidThreadOrLockAcquired();
...
gpu::ContextSupport* ContextProviderCommandBufferBase::ContextSupport() {
  return gles2_impl_.get();
}

class GrContext* ContextProviderCommandBufferBase::GrContext() {
  DCHECK(bind_tried_);
  DCHECK_EQ(bind_result_, gpu::ContextResult::kSuccess);
  CheckValidThreadOrLockAcquired();
  ...
  // TODO(vmiura): https://crbug.com/793508 Disable access to GrContext if
  ...
  gr_context_.reset(new skia_bindings::GrContextForGLES2Interface(
      gles2_impl_.get(), ContextCapabilities(), max_resource_cache_bytes,
      max_glyph_cache_texture_bytes));
...
ContextProviderCommandBufferBase::CacheController() {
  CheckValidThreadOrLockAcquired();
  return cache_controller_.get();
}

void ContextProviderCommandBufferBase::InvalidateGrContext(uint32_t state) {
  if (gr_context_) {
    DCHECK(bind_tried_);
    DCHECK_EQ(bind_result_, gpu::ContextResult::kSuccess);
    CheckValidThreadOrLockAcquired();
...
void ContextProviderCommandBufferBase::SetDefaultTaskRunner(
    scoped_refptr<base::SingleThreadTaskRunner> default_task_runner) {
  DCHECK(!bind_tried_);
  default_task_runner_ = std::move(default_task_runner);
}

base::Lock* ContextProviderCommandBufferBase::GetLock() {
  DCHECK(support_locking_);
  return &context_lock_;
}

const gpu::Capabilities& ContextProviderCommandBufferBase::ContextCapabilities()
    const {
  DCHECK(bind_tried_);
  DCHECK_EQ(bind_result_, gpu::ContextResult::kSuccess);
  CheckValidThreadOrLockAcquired();
...
const gpu::GpuFeatureInfo& ContextProviderCommandBufferBase::GetGpuFeatureInfo()
    const {
  DCHECK(bind_tried_);
  DCHECK_EQ(bind_result_, gpu::ContextResult::kSuccess);
  CheckValidThreadOrLockAcquired();
...
void ContextProviderCommandBufferBase::OnLostContext() {
  CheckValidThreadOrLockAcquired();
...
void ContextProviderCommandBufferBase::AddObserver(
    viz::ContextLostObserver* obs) {
  observers_.AddObserver(obs);
}

void ContextProviderCommandBufferBase::RemoveObserver(
    viz::ContextLostObserver* obs) {
  observers_.RemoveObserver(obs);
}

bool ContextProviderCommandBufferBase::OnMemoryDump(
