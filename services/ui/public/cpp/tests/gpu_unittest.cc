...
class GpuTest : public testing::Test {
 public:
  ...
  // Ensure all tasks posted to IO thread and any tasks posted back to main
  // thread are processed.
  void FlushMainAndIO() {
    base::RunLoop run_loop;
    io_task_runner_->PostTask(
        FROM_HERE, base::Bind([](base::Closure callback) { callback.Run(); },
                              run_loop.QuitClosure()));
    run_loop.Run();
  }

  // TODO(kylechar): Find a better way to do this.
  void BlockMainFlushIO(int num_tasks) {
 ...
 private:
  ...
  void DestroyOnIO(base::WaitableEvent* event) {
...
TEST_F(GpuTest, EstablishRequestOnFailureOnPreviousRequest) {
  ...
  {
    base::RunLoop run_loop;
    gpu()->EstablishGpuChannel(base::Bind(callback, run_loop.QuitClosure()));
...
// Tests that if EstablishGpuChannelSync() is called after response for
// EstablishGpuChannel() has returned that response is used immediately.
TEST_F(GpuTest, EstablishRequestAsyncThenSyncWithResponse) {
  ...
  // Block the main thread while the IO thread runs tasks. This will put the Gpu
  // object into a state where a response for EstablishGpuChannel() has been
  // received and a task has been posted to the main thread to process it.
  // TODO(kylechar): Come up with a better way to test this. sleep(1) also
  // works but is equally bad.
  BlockMainFlushIO(5);
  EXPECT_EQ(0, counter);

  // Run EstablishGpuChannelSync() before the posted task can run. The results
  // should be used immediately, the pending callback should fire and then
  // EstablishGpuChannelSync() should return.
  bool connection_error = false;
  scoped_refptr<gpu::GpuChannelHost> host =
      gpu()->EstablishGpuChannelSync(&connection_error);
  EXPECT_FALSE(connection_error);
  ...
  // The original posted task will do nothing when it runs after.
  FlushMainAndIO();
}
