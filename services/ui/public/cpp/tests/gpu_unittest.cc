...
class GpuTest : public testing::Test {
 public:
  ...
  // Ensure all tasks posted to IO thread are run while blocking main thread.
...
TEST_F(GpuTest, EstablishRequestAsyncThenSyncWithResponse) {
  ...
  // Block the main thread while the IO thread runs tasks. There are four tasks
  // that run on the IO thread that schedule new tasks on back on the IO thread.
  BlockMainFlushIO(4);
