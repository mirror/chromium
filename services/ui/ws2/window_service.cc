...
WindowData* WindowService::GetWindowDataForWindow(aura::Window* window) {
  WindowData* data = WindowData::Get(window);
  if (data)
    return data;

  const WindowId window_id = WindowId(kWindowServerClientId, next_window_id_++);
  CHECK_NE(0, next_window_id_);
  const ClientWindowId client_window_id =
      ClientWindowId(window_id.client_id, window_id.window_id);
  return WindowData::Create(window, nullptr, window_id, client_window_id);
}
