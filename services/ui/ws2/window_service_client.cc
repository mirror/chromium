...
WindowServiceClient::~WindowServiceClient() {
  ...
  while (!client_created_windows_.empty()) {
    client_created_windows_.empty();
    aura::Window* window = *client_created_windows_.begin();
    UnregisterWindow(window);
    DCHECK(!CreatedWindow(window));
    delete window;
  }
}
...
bool WindowServiceClient::CreatedWindow(aura::Window* window) {
...
bool WindowServiceClient::NewWindowImpl(
  ...
  WindowData::Create(window, this, window_id, client_window_id);
  client_created_windows_.insert(window);
  RegisterWindow(window, client_window_id);
  return true;
}
...
bool WindowServiceClient::AddWindowImpl(const ClientWindowId& parent_id,
  ...
  if (CreatedWindow(child) &&
      (IsRoot(parent) ||
       (CreatedWindow(parent) && !IsWindowRootOfAnotherClient(parent)))) {
...
bool WindowServiceClient::RemoveWindowFromParentImpl(
  ...
  if (CreatedWindow(window) || IsRoot(window)) {
...
bool WindowServiceClient::SetWindowBoundsImpl(
  ...
  if (ShouldRouteToDelegate(window)) {
    ...
    window_service_->delegate()->ClientRequestedWindowBoundsChange(window,
                                                                   bounds);
    // Pass in false for success, which triggers the client reverting to
  ...
  // TODO: need to pass through |local_surface_id|
  return true;
}
...
bool WindowServiceClient::EmbedImpl(
  ...
  aura::Window* window = GetWindowByClientId(window_id);
  if (!window || !CreatedWindow(window))
    return false;
  ...
  auto new_client_binding = std::make_unique<WindowServiceClientBinding>(
      window_service_, std::move(window_tree_client), intercepts_events,
      window);
...
void WindowServiceClient::NewTopLevelWindow(
  ...
  WindowEmbeddings* embedding = CreateWindowEmbedding(top_level, nullptr);
  window_tree_client_->OnTopLevelCreated(change_id, WindowToWindowData(window),
                                         display_id, window->IsVisible(),
                                         embedding->GetLocalSurfaceId());
}
