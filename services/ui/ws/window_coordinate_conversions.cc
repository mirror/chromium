...
gfx::Transform GetTransformToRoot(const ServerWindow* root,
  ...
  // WindowManagerDisplayRoot works.
...
gfx::Point ConvertPointFromRootForEventDispatch(
  ...
  // The client side expects to convert this using root->transform() (in
  // reverse).
  if (!root->transform().IsIdentity())
    root->transform().TransformPoint(&location_in_root3);
