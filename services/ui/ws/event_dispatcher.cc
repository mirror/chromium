...
void EventDispatcher::ProcessEvent(const ui::Event& event,
  ...
  // Can we just put the changes here? And not care about the crud in
  // WindowEventDispatcher::PreDispatchMouseEvent()?
...
void EventDispatcher::ProcessPointerEventOnFoundTarget(
  ...
  if (is_mouse_event) {
    ...
    // further filtering.
    delegate_->OnEventChangesMouseEventsEnabled(event, true);
    ...
    // When we have a non-pointer event that wasn't synthesized, hide the mouse
    // cursor until the next non-synthesized mouse event.
    delegate_->OnEventChangesMouseEventsEnabled(event, false);
  }
