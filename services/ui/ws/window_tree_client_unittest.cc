...
bool ScheduleEmbed(WindowTree* tree,
                   mojom::WindowTreeClientPtr client,
  ...
  base::RunLoop run_loop;
  {
    tree->ScheduleEmbed(
        std::move(client),
        base::Bind(&ScheduleEmbedCallbackImpl, &run_loop, &result, token));
  }
  run_loop.Run();
  return result;
}
...
TEST_F(WindowTreeClientTest, EmbedUsingToken) {
  ...
  base::UnguessableToken token;
  ASSERT_TRUE(ScheduleEmbed(wt1(), std::move(client3_ptr), &token));
  client2.WaitForOnEmbed();
