...
FrameGeneratorSubordinate::FrameGeneratorSubordinate(
    const viz::SurfaceId& source_surface_id)
    : surface_info_(source_surface_id, 1.0f, gfx::Size(400,400)) {
  timer_.Start(FROM_HERE, base::TimeDelta::FromSeconds(2),
            base::BindRepeating(&FrameGeneratorSubordinate::SetNeedsBeginFrame, base::Unretained(this), true));
}
// FrameGeneratorSubordinate::FrameGeneratorSubordinate(const viz::SurfaceInfo& source_surface_info) : source_surface_info_(source_surface_info) {}

FrameGeneratorSubordinate::~FrameGeneratorSubordinate() = default;

void FrameGeneratorSubordinate::SetDeviceScaleFactor(float device_scale_factor) {
...
void FrameGeneratorSubordinate::SetHighContrastMode(bool enabled) {
...
void FrameGeneratorSubordinate::OnFirstSurfaceActivation(
    const viz::SurfaceInfo& surface_info) {
  LOG(ERROR) << "MSW FrameGeneratorSubordinate::OnFirstSurfaceActivation... HMM"; 
...
void FrameGeneratorSubordinate::SwapSurfaceWith(FrameGeneratorSubordinate* other) {
  LOG(ERROR) << "MSW FrameGeneratorSubordinate::SwapSurfaceWith... HMM"; 
...
void FrameGeneratorSubordinate::OnWindowDamaged() {
  SetNeedsBeginFrame(true);
}

void FrameGeneratorSubordinate::OnWindowSizeChanged(const gfx::Size& pixel_size) {
...
void FrameGeneratorSubordinate::Bind(
    std::unique_ptr<viz::mojom::CompositorFrameSink> compositor_frame_sink) {
  DCHECK(!compositor_frame_sink_);
  compositor_frame_sink_ = std::move(compositor_frame_sink);
}

void FrameGeneratorSubordinate::ReclaimResources(
    const std::vector<viz::ReturnedResource>& resources) {
  // Nothing to do here because FrameGeneratorSubordinate CompositorFrames don't reference
  // any resources.
  DCHECK(resources.empty());
}

void FrameGeneratorSubordinate::DidReceiveCompositorFrameAck(
    const std::vector<viz::ReturnedResource>& resources) {}

void FrameGeneratorSubordinate::DidPresentCompositorFrame(uint32_t presentation_token,
                                               base::TimeTicks time,
...
void FrameGeneratorSubordinate::DidDiscardCompositorFrame(uint32_t presentation_token) {
  NOTIMPLEMENTED();
}

void FrameGeneratorSubordinate::OnBeginFrame(const viz::BeginFrameArgs& begin_frame_args) {
  LOG(ERROR) << "MSW FrameGeneratorSubordinate::OnBeginFrame A"; 
  DCHECK(compositor_frame_sink_);
  ...
  if (begin_frame_args.type == viz::BeginFrameArgs::MISSED) {
    LOG(ERROR) << "MSW FrameGeneratorSubordinate::OnBeginFrame B"; 
  ...
  LOG(ERROR) << "MSW FrameGeneratorSubordinate::OnBeginFrame C"; 
  ...
  if (!local_surface_id_.is_valid() ||
    ...
    LOG(ERROR) << "MSW FrameGeneratorSubordinate::OnBeginFrame D"; 
    last_device_scale_factor_ = frame.device_scale_factor();
    last_submitted_frame_size_ = frame.size_in_pixels();
    local_surface_id_ = id_allocator_.GenerateId();
  }

  LOG(ERROR) << "MSW FrameGeneratorSubordinate::OnBeginFrame E"; 
  DCHECK(local_surface_id_.is_valid());
  compositor_frame_sink_->SubmitCompositorFrame(
      local_surface_id_, std::move(frame), GenerateHitTestRegionList(), 0);

  SetNeedsBeginFrame(false);
  LOG(ERROR) << "MSW FrameGeneratorSubordinate::OnBeginFrame F"; 

}

viz::CompositorFrame FrameGeneratorSubordinate::GenerateCompositorFrame() {
  ...
  if (high_contrast_mode_enabled_) {
    ...
    gfx::Size scaled_bounds = gfx::ScaleToCeiledSize(
        pixel_size_, surface_info_.device_scale_factor(),
  ...
  if (surface_info_.is_valid())
    frame.metadata.referenced_surfaces.push_back(surface_info_.id());

  return frame;
}

viz::mojom::HitTestRegionListPtr FrameGeneratorSubordinate::GenerateHitTestRegionList()
    const {
  ...
  viz::SurfaceId surface_id = surface_info_.id();
...
void FrameGeneratorSubordinate::DrawWindow(viz::RenderPass* pass) {
  DCHECK(surface_info_.is_valid());
  // if (!surface_info_.is_valid())
  //   return;

  const gfx::Rect bounds_at_origin(surface_info_.size_in_pixels());

  static size_t msw = 0; if (msw % 1000 == 0) {
    LOG(ERROR) << "MSW FrameGeneratorSubordinate::DrawWindow"
              //  << " window_manager_surface_info_:" << window_manager_surface_info_.id()
               << " surface_info_:" << surface_info_.id()
               << " bounds_at_origin: " << bounds_at_origin.ToString();
  }
  ...
  quad->SetAll(sqs, bounds_at_origin /* rect */,
               ...
               surface_info_.id(), surface_info_.id(),
               SK_ColorWHITE /* default_background_color */,
               false /* stretch_content_to_fill_bounds */);

  auto* debug = pass->CreateAndAppendDrawQuad<viz::SolidColorDrawQuad>(); 
  debug->SetNew(sqs, bounds_at_origin, bounds_at_origin, SK_ColorGREEN, false); 
}

void FrameGeneratorSubordinate::SetNeedsBeginFrame(bool needs_begin_frame) {
  // needs_begin_frame &= window_manager_surface_info_.is_valid();
  LOG(ERROR) << "MSW FrameGeneratorSubordinate::SetNeedsBeginFrame!"; 
