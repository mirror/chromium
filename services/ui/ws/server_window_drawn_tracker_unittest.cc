...
TEST(ServerWindowDrawnTrackerTest, ChangeBecauseOfDeletionAndVisibility) {
  ...
  std::unique_ptr<ServerWindow> window(
      new ServerWindow(&server_window_delegate, WindowId(1, 1)));
...
TEST(ServerWindowDrawnTrackerTest, VisibilityChangeFromNonParentAncestor) {
  TestServerWindowDelegate server_window_delegate;
  ServerWindow root(&server_window_delegate, WindowId(1, 1));
  ServerWindow child1(&server_window_delegate, WindowId(1, 2));
  ServerWindow child2(&server_window_delegate, WindowId(1, 3));
  ServerWindow child3(&server_window_delegate, WindowId(1, 4));
  server_window_delegate.set_root_window(&root);
...
TEST(ServerWindowDrawnTrackerTest, TreeHierarchyChangeFromNonParentAncestor) {
  ...
  ServerWindow child1(&server_window_delegate, WindowId(1, 2));
  ServerWindow child2(&server_window_delegate, WindowId(1, 3));
  ServerWindow child11(&server_window_delegate, WindowId(1, 4));
  ServerWindow child111(&server_window_delegate, WindowId(1, 5));
  server_window_delegate.set_root_window(&root);
