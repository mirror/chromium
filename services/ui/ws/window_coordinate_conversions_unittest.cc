...
TEST(WindowCoordinateConversions, Transform) {
  ...
  const gfx::Point converted_point =
      ConvertPointFromRoot(&child, &child_child, gfx::Point(14, 20));
  // The converted point is 3,4 in |child_child|'s coordinates, but aura
  // expects to be able to also apply the root transform (in reverse) so this
  // code applys the root transform to the local point.
  EXPECT_EQ(gfx::Point(6, 8), converted_point);
}
