    ...
    if (hit_test_query) {
      ...
      if (base::CommandLine::ForCurrentProcess()->HasSwitch(
        ...
        ServerWindow* target_window =
            event_targeter_delegate_->GetWindowWith(target.frame_sink_id);
        if (!target_window) {
...
// the asynchronously asking blink case correctly.
