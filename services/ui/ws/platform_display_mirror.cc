...
#include "services/ui/ws/frame_generator_subordinate.h"
...
PlatformDisplayMirror::~PlatformDisplayMirror() {
  ...
  frame_generator_subordinate_.reset();
...
void PlatformDisplayMirror::SetCursor(const ui::CursorData& cursor_data) {
  if (!image_cursors_)
    return;
...
FrameGenerator* PlatformDisplayMirror::GetFrameGenerator() {
  return frame_generator_subordinate_.get();
}
...
void PlatformDisplayMirror::UpdateViewportMetrics(
  ...
  metrics_ = metrics;
  if (frame_generator_subordinate_) {
    frame_generator_subordinate_->SetDeviceScaleFactor(metrics_.device_scale_factor);
    frame_generator_subordinate_->OnWindowSizeChanged(metrics_.bounds_in_pixels.size());
  }
}
...
void PlatformDisplayMirror::SetCursorConfig(
...
void PlatformDisplayMirror::OnDamageRect(const gfx::Rect& damaged_region) {
  if (frame_generator_subordinate_)
    frame_generator_subordinate_->OnWindowDamaged();
}
...
void PlatformDisplayMirror::OnAcceleratedWidgetAvailable(
  ...
  // Pass the mirror source's platform window... 
  LOG(ERROR) << "MSW PlatformDisplayMirror::OnAcceleratedWidgetAvailable"; 
  frame_generator_subordinate_ =
      base::MakeUnique<FrameGeneratorSubordinate>(mirror_source_surface_id_);
  auto frame_sink_client_binding =
      base::MakeUnique<CompositorFrameSinkClientBinding>(
          frame_generator_subordinate_.get(),
          std::move(compositor_frame_sink_client_request),
          std::move(compositor_frame_sink), std::move(display_private));
  frame_generator_subordinate_->Bind(std::move(frame_sink_client_binding));
  frame_generator_subordinate_->OnWindowSizeChanged(root_window_->bounds().size());
  frame_generator_subordinate_->SetDeviceScaleFactor(metrics_.device_scale_factor);
}
