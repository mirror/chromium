...
void PlatformDisplayDefault::OnAcceleratedWidgetAvailable(
  ...
  // TODO(ccameron): |display_private_client| is not bound. This will need to
  // change to support macOS.
  viz::mojom::DisplayPrivateAssociatedPtr display_private;
  viz::mojom::DisplayPrivateClientPtr display_private_client;
  viz::mojom::DisplayPrivateClientRequest display_private_client_request =
      mojo::MakeRequest(&display_private_client);

  root_window_->CreateRootCompositorFrameSink(
      widget_, mojo::MakeRequest(&compositor_frame_sink),
      std::move(compositor_frame_sink_client),
      mojo::MakeRequest(&display_private), std::move(display_private_client));
