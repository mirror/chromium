...
void WindowManagerState::SetMouseEventsEnabled(bool enabled) {
  cursor_state_.SetMouseEventsEnabled(enabled);
}
...
void WindowManagerState::OnEventChangesMouseEventsEnabled(
    const ui::Event& event,
    bool enabled) {
  if (event.IsSynthesized())
    return;

  // Setting mouse events enabled needs to cause a callback which notifies a
...
void WindowManagerState::OnMouseEventsEnabledChanged(bool enabled) {
  window_tree_->OnMouseEventsEnabledChanged(enabled);
}
