...
void WindowManagerState::AdjustEventLocation(int64_t display_id,
  ...
  LOG(ERROR) << "location=" << event->location().ToString();
  ...
  for (Display* display : display_manager()->displays()) {
    LOG(ERROR) << "bounds=" << display->GetDisplay().bounds().ToString()
               << " in_pixels="
               << display->GetViewportMetrics().bounds_in_pixels.ToString();
  }
  ...
  //
  const gfx::Point location_in_containg_display =
      screen_pixels -
  ...
  const gfx::Point screen_dip_location =
      ...
      PixelsToDips(containing_display, location_in_containg_display)
          .OffsetFromOrigin();
  ...
  LOG(ERROR) << "location_in_containg_display="
             << location_in_containg_display.ToString()
             << "screen_dip_location=" << screen_dip_location.ToString()
             << " pixel_relative_location="
             << pixel_relative_location.ToString();
