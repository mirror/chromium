...
void WindowTree::OnWindowInputEventAck(uint32_t event_id,
  ...
  if (!event_queue_.empty()) {
    ...
    if (GetDisplay(target)) {
      ...
      // If the window is no longer valid (or not in a dislay), then no point in
      // dispatching to the client, but we need to run the callback so that
      // WindowManagerState isn't still waiting for an ack. We only need run
      // the last callback as they should all target the same
      // WindowManagerState.
