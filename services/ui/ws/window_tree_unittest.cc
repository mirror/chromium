...
const std::string kNextWindowTreeIdString =
    std::to_string(kWindowManagerTreeId + 1);
...
TEST_F(WindowTreeTest, CaptureNotifiesWm) {
  ...
  EXPECT_EQ("OnCaptureChanged new_window=" + kNextWindowTreeIdString +
                ",1 old_window=null",
            ChangesToDescription1(*wm_client()->tracker()->changes())[0]);
  ...
  ASSERT_TRUE(!wm_client()->tracker()->changes()->empty());
  EXPECT_EQ("OnCaptureChanged new_window=" + kWindowManagerTreeIdString +
                ",1 old_window=" + kNextWindowTreeIdString + ",1",
            ChangesToDescription1(*wm_client()->tracker()->changes())[0]);
  EXPECT_TRUE(embed_client->tracker()->changes()->empty());
  wm_client()->tracker()->changes()->clear();
