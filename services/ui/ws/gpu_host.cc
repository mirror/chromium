...
bool HasSplitGpuProcess() {
  constexpr char kMusGpuProcess[] = "mus-gpu-process";
  return base::CommandLine::ForCurrentProcess()->HasSwitch(kMusGpuProcess);
}
...
DefaultGpuHost::DefaultGpuHost(GpuHostDelegate* delegate,
  ...
  if (connector && HasSplitGpuProcess()) {
