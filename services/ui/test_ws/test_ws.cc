...
class TestWS : public service_manager::Service,
 ...
 private:
  ...
  aura::Window* NewTopLevel(
      const std::unordered_map<std::string, std::vector<uint8_t>>& properties)
      override {
    aura::Window* top_level = new aura::Window(nullptr);
...
void WindowTreeHostFactory::CreateWindowTreeHost(
    ...
    mojom::WindowTreeClientPtr tree_client) {
  LOG(ERROR) << "CreateWindowTreeHost";
  test_ws_->AddNewClient(std::move(host), std::move(tree_client));
  LOG(ERROR) << "CreateWindowTreeHost - done";
}
