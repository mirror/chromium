...
`OnFetchEventFinished()` calls `ServiceWorkerVersion::FinishRequest()` which is
a dec ref matching the `StartRequest()`.
...
=============================================================
The main observation is that service worker is almost entirely a
browser-implemented feature (though this was pretty obvious based on `wc` too.)

```
$ wc -l content/renderer/service_worker/* | grep total
    2848 total
$ wc -l content/browser/service_worker/* | grep total
   48449 total
```
