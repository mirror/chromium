...
const uint32_t kMaxPendingSendRequestsUpperbound = 128;
...
void UDPSocket::NegotiateMaxPendingSendRequests(
  ...
  if (requested_size != 0) {
    max_pending_send_requests_ =
        std::min(kMaxPendingSendRequestsUpperbound, requested_size);
  }
  std::move(callback).Run(max_pending_send_requests_);
