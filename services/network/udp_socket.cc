...
const uint32_t kDefaultMaxPendingSendRequests = 32;
...
class SocketWrapperImpl : public UDPSocket::SocketWrapper {
 public:
  ...
  int AllowAddressReuse() override { return socket_.AllowAddressReuse(); }
  int SetMulticastTimeToLive(int ttl) override {
  ...
  int SetBroadcast(bool enabled) override {
  ...
  int Write(net::IOBuffer* buf,
            int buf_len,
            const net::CompletionCallback& callback) override {
    return socket_.Write(buf, buf_len, callback, NO_TRAFFIC_ANNOTATION_YET);
  }
...
int UDPSocket::Connect(const net::IPEndPoint& remote_addr,
                       net::IPEndPoint* local_addr_out) {
  if (!is_opened_)
    return net::ERR_UNEXPECTED;
  if (is_connected_)
    return net::ERR_SOCKET_IS_CONNECTED;
  int result = wrapped_socket_->Connect(remote_addr);
  if (result == net::OK) {
    is_connected_ = true;
...
int UDPSocket::Bind(const net::IPEndPoint& local_addr,
                    net::IPEndPoint* local_addr_out) {
  if (!is_opened_)
    return net::ERR_UNEXPECTED;
  if (is_connected_)
    return net::ERR_SOCKET_IS_CONNECTED;
  int result = wrapped_socket_->Bind(local_addr);
  if (result == net::OK) {
    is_connected_ = true;
    result = wrapped_socket_->GetLocalAddress(local_addr_out);
  }
...
int UDPSocket::SetAllowAddressReuse() {
  if (!is_opened_ || is_connected_)
    return net::ERR_UNEXPECTED;
  return wrapped_socket_->AllowAddressReuse();
}

int UDPSocket::SetMulticastTimeToLive(int ttl) {
  if (!is_opened_ || is_connected_)
    return net::ERR_UNEXPECTED;
  return wrapped_socket_->SetMulticastTimeToLive(ttl);
}

int UDPSocket::SetMulticastLoopbackMode(bool loopback) {
  if (!is_opened_ || is_connected_)
    return net::ERR_UNEXPECTED;
  return wrapped_socket_->SetMulticastLoopbackMode(loopback);
}
...
void UDPSocket::SetBroadcast(bool enabled, SetBroadcastCallback callback) {
  if (!is_opened_ || !is_connected_) {
    std::move(callback).Run(net::ERR_UNEXPECTED);
    return;
  }
  int net_result = wrapped_socket_->SetBroadcast(enabled);
  std::move(callback).Run(net_result);
}
...
void UDPSocket::DoSendToOrWrite(const net::IPEndPoint* dest_addr,
                                ...
                                SendToCallback callback) {
  if (send_buffer_.get()) {
    if (pending_send_requests_.size() >= kDefaultMaxPendingSendRequests) {
...
void UDPSocket::OnSendToCompleted(int net_result) {
  ...
  DoSendToOrWrite(request->addr.get(), request->data,
