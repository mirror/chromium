...
// There are two types of UDPSocketImpl:
//   (1) ClientSocket, and
//   (2) ServerSocket.
// It's important to differentiate them because mojom::UDPSocketRequest is
// satisfied with a bound socket (in the server's case) or connected socket (in
// the client's case). The params for instantiating a ClientSocket and those for
// ServerSocket are different, so there are two slightly static helpers to
// create them.
class UDPSocketImpl : public mojom::UDPSocket {
 public:
  ...
  // Creates a server socket. |local_addr| is the desired local address of
  // the server socket. Caller can use port 0 in |local_addr| for the OS to
  // pick an available port. Upon success, returns net::OK and writes out the
  // real local address to |local_addr_out|. Otherwise, returns a net error code
