...
TEST_F(UDPSocketTest, TestUnexpectedSequences) {
  ...
  // These Setters can be called anytime.
  EXPECT_EQ(32u, test::UDPSocketTestHelper::NegotiateMaxPendingSendRequestsSync(
                     &socket_ptr, 0));
  EXPECT_EQ(16u, test::UDPSocketTestHelper::NegotiateMaxPendingSendRequestsSync(
                     &socket_ptr, 16));
...
TEST_F(UDPSocketTest, TestInsufficientResources) {
  ...
  impl.wrapped_socket_ = std::make_unique<HangingUDPSocket>();
...
TEST_F(UDPSocketTest, TestReadSend) {
  ...
  UDPSocket client_impl(std::move(client_receiver_ptr),
...
TEST_F(UDPSocketTest, TestReadSendTo) {
  ...
  UDPSocket impl(std::move(receiver_ptr), mojo::MakeRequest(&server_socket));
...
TEST_F(UDPSocketTest, TestSendToInvalidAddress) {
  ...
  EXPECT_EQ(net::ERR_ADDRESS_INVALID, result);
}
