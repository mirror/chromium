...
TEST_F(UDPSocketTest, Settings) {
  ...
  size_t kNewNumMaxPendingSendRequest = 16;
  {
    base::RunLoop run_loop;
    client_socket->NegotiateMaxPendingSendRequests(
        kNewNumMaxPendingSendRequest,
        base::BindOnce(
            [](base::RunLoop* run_loop, uint32_t result) {
  ...
  // Make sure send requests beyond kNewNumMaxPendingSendRequest will receive
  // ERR_INSUFFICIENT_RESOURCES.
  // FIXME: This expectation doesn't pass.
  {
    base::RunLoop run_loop;
    for (size_t i = 0; i < kNewNumMaxPendingSendRequest + 1; ++i) {
      std::vector<uint8_t> test_msg(
          CreateTestMessage(static_cast<uint8_t>(i), 255));
      client_socket->SendTo(
          local_addr, test_msg,
          base::BindOnce(
              [](base::RunLoop* run_loop, size_t request_num, size_t max_num,
                 int result) {
                if (request_num == max_num) {
                  EXPECT_EQ(net::ERR_INSUFFICIENT_RESOURCES, result);
                  run_loop->Quit();
                }
              },
              base::Unretained(&run_loop), i, kNewNumMaxPendingSendRequest));
    }
    run_loop.Run();
  }
}
