...
class TestCallbackBase {
 public:
  ...
  ~TestCallbackBase() { state_->set_test_callback(nullptr); }
 ...
 protected:
  ...
  void Initialize(StateBase* state) {
    ...
    callback_ =
        CallbackType(static_cast<typename CallbackType::Runnable*>(state_));
  }
...
class TestCallback : public TestCallbackBase<Callback<void(NetworkErrorPtr)>> {
 public:
  TestCallback() { Initialize(new State()); }
  ~TestCallback() {}
...
class TestCallbackWithAddress
 ...
 public:
  TestCallbackWithAddress() { Initialize(new State()); }
...
class TestCallbackWithUint32
 ...
 public:
  TestCallbackWithUint32() : result_(0) { Initialize(new State()); }
...
class TestReceiveCallback
 ...
 public:
  TestReceiveCallback() { Initialize(new State()); }
  ~TestReceiveCallback() {}
...
class UDPSocketReceiverImpl : public UDPSocketReceiver {
 public:
  ...
  std::queue<ReceiveResult*>* results() { return &results_; }
...
TEST_F(UDPSocketTest, TestConnectedReadWrite) {
  ...
  for (size_t i = 0; i < kDatagramCount; ++i) {
    TestCallback callback;
    client_socket->SendTo(
        nullptr, CreateTestMessage(static_cast<uint8_t>(i), kDatagramSize),
        callback.callback());
    callback.WaitForResult();
    EXPECT_EQ(255, callback.result()->code);
  }
  ...
  for (size_t i = 0; i < kDatagramCount; ++i) {
    TestCallback callback;
    socket_->SendTo(client_addr.Clone(),
                    CreateTestMessage(static_cast<uint8_t>(i), kDatagramSize),
                    callback.callback());
    callback.WaitForResult();
    EXPECT_EQ(255, callback.result()->code);
...
TEST_F(UDPSocketTest, TestWrapperConnectedReadWrite) {
  ...
  for (size_t i = 1; i < kDatagramCount; ++i) {
    ...
    for (size_t j = 0; j < i; ++j) {
      client_socket.SendTo(
          nullptr, CreateTestMessage(static_cast<uint8_t>(j), kDatagramSize),
          send_callbacks[j].callback());
  ...
  for (size_t i = 1; i < kDatagramCount; ++i) {
    ...
    for (size_t j = 0; j < i; ++j) {
      socket.SendTo(client_addr.Clone(),
                    CreateTestMessage(static_cast<uint8_t>(j), kDatagramSize),
                    send_callbacks[j].callback());
