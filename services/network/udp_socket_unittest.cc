...
TEST_F(UDPSocketTest, Settings) {
  ...
  ASSERT_EQ(net::OK, impl.Open(any_port.GetFamily()));
  net::IPEndPoint local_addr;
  ASSERT_EQ(net::OK, impl.Bind(any_port, &local_addr));
...
TEST_F(UDPSocketTest, TestSendWithBind) {
  ...
  // Bind() the socket.
  ASSERT_EQ(net::OK, impl.Open(server_addr.GetFamily()));
  ASSERT_EQ(net::OK, impl.Bind(server_addr, &server_addr));
...
TEST_F(UDPSocketTest, TestUnexpectedSequences) {
  ...
  net::IPEndPoint local_addr;
  ASSERT_EQ(net::ERR_UNEXPECTED, impl.Bind(any_port, &local_addr));
  ASSERT_EQ(net::ERR_UNEXPECTED, impl.Connect(any_port, &local_addr));
  ...
  ASSERT_EQ(net::ERR_SOCKET_IS_CONNECTED, impl.Connect(any_port, &local_addr));
...
TEST_F(UDPSocketTest, TestReadSend) {
  ...
  ASSERT_EQ(net::OK, client_impl.Open(client_addr.GetFamily()));
  ASSERT_EQ(net::OK, client_impl.Connect(server_addr, &client_addr));
...
// Mojo serialization/deserialization of net::IPEndPoint handles invalid
// net::IPEndPoints, so there is no need for mojom::UDPSocket implementations to
// handle them.
...
// TODO(xunjieli): Test various multicast options.
