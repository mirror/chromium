...
TEST_F(UDPSocketTest, Settings) {
  ...
  ASSERT_EQ(net::OK,
            UDPSocketTestHelper::OpenSync(&socket_ptr, any_port.GetFamily()));
  net::IPEndPoint local_addr;
  ASSERT_EQ(net::OK,
            UDPSocketTestHelper::BindSync(&socket_ptr, any_port, &local_addr));
  EXPECT_NE(local_addr.ToString(), any_port.ToString());
  EXPECT_EQ(net::OK,
            UDPSocketTestHelper::SetSendBufferSizeSync(&socket_ptr, 1024));
  EXPECT_EQ(net::OK,
            UDPSocketTestHelper::SetReceiveBufferSizeSync(&socket_ptr, 2048));
  EXPECT_EQ(32u, UDPSocketTestHelper::NegotiateMaxPendingSendRequestsSync(
                     &socket_ptr, 0));
  EXPECT_EQ(16u, UDPSocketTestHelper::NegotiateMaxPendingSendRequestsSync(
                     &socket_ptr, 16));
}
...
TEST_F(UDPSocketTest, TestInsufficientResources) {
  ...
  EXPECT_EQ(10u, UDPSocketTestHelper::NegotiateMaxPendingSendRequestsSync(
  ...
  EXPECT_EQ(net::ERR_INSUFFICIENT_RESOURCES,
            SendToSync(&socket_ptr, GetLocalHostWithAnyPort(), test_msg));
  ...
  EXPECT_EQ(kQueueSize / 2,
            UDPSocketTestHelper::NegotiateMaxPendingSendRequestsSync(
...
TEST_F(UDPSocketTest, TestReadWrite) {
  ...
  ASSERT_EQ(net::OK, UDPSocketTestHelper::OpenSync(&server_socket,
                                                   server_addr.GetFamily()));
  ASSERT_EQ(net::OK, UDPSocketTestHelper::BindSync(&server_socket, server_addr,
                                                   &server_addr));
  ...
  ASSERT_EQ(net::OK, UDPSocketTestHelper::OpenSync(&client_socket,
                                                   client_addr.GetFamily()));
  ASSERT_EQ(net::OK, UDPSocketTestHelper::ConnectSync(
  ...
  for (size_t i = 0; i < kDatagramCount; ++i) {
    std::vector<uint8_t> test_msg(
        CreateTestMessage(static_cast<uint8_t>(i), kDatagramSize));
    int result =
        UDPSocketTestHelper::SendToSync(&client_socket, server_addr, test_msg);
