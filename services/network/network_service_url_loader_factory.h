...
// its bindings are closed and no URLLoaders are using it, or when the parent
// NetworkContext is destroyed.
class COMPONENT_EXPORT(NETWORK_SERVICE) NetworkServiceURLLoaderFactory
    : public mojom::URLLoaderFactory {
 public:
  // NOTE: |context| must outlive this instance.
  NetworkServiceURLLoaderFactory(NetworkContext* network_context,
                                 uint32_t process_id,
                                 mojom::URLLoaderFactoryRequest request);

  ~NetworkServiceURLLoaderFactory() override;

  // Called when the associated NetworkContext is being destroyed.
  void Cleanup();
  ...
  NetworkContext* network_context() const { return network_context_; }
