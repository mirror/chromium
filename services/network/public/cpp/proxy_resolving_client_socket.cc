...
ProxyResolvingClientSocket::ProxyResolvingClientSocket(
  ...
  // TODO(xunjieli): Handle invalid URLs more gracefully (at mojo API layer or
  // when the request is created).
  DCHECK(url_.is_valid());

  // TODO(xunjieli): crbug.com/793066. Avoid creating one context per socket.
...
int ProxyResolvingClientSocket::Connect(
  ...
  // interact with the new interface for proxy delegate. crbug.com/793071.
...
void ProxyResolvingClientSocket::ConnectToProxy(int net_error) {
  ...
  // ReconsiderProxyAfterError() path. crbug.com/793076.
...
int ProxyResolvingClientSocket::GetPeerAddress(net::IPEndPoint* address) const {
  ...
  *address = net::IPEndPoint(ip_address, host_port_pair.port());
  return net::OK;
}
