...
bool StructTraits<network::mojom::ProxyConfigDataView, net::ProxyConfig>::Read(
  ...
  GURL pac_url;
  net::MutableNetworkTrafficAnnotationTag traffic_anntoation;
  if (!data.ReadPacUrl(&pac_url) ||
      !data.ReadProxyRules(&out_proxy_config->proxy_rules()) ||
      !data.ReadTrafficAnnotation(&traffic_anntoation)) {
    return false;
  }
  out_proxy_config->set_pac_url(pac_url);
  out_proxy_config->set_traffic_annotation(
      net::NetworkTrafficAnnotationTag(traffic_anntoation));
