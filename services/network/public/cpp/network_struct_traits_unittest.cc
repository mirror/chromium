...
class NetworkStructTraitsTest : public testing::Test, public mojom::TraitsTestService {
 ...
 private:
  // TraitsTestService:
  void EchoHttpRequestHeaders(const net::HttpRequestHeaders& header,
...
/*
TEST_F(NetworkStructTraitsTest, HttpRequestHeaders_Basic_Mojo) {
  net::HttpRequestHeaders output;
  std::vector<mojom::HttpRequestHeaderKeyValuePairPtr> header_array;
  header_array.push_back(mojom::HttpRequestHeaderKeyValuePair::New("Foo", "bar"));
  mojom::HttpRequestHeaders headers(header_array);
  mojom::TraitsTestServicePtr proxy = GetTraitsTestProxy();
  proxy->EchoHttpRequestHeaders(headers, &output);
  std::string value;
  EXPECT_TRUE(output.GetHeader("Foo", &value));
