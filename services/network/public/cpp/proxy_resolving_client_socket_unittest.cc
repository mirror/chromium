...
TEST_F(ProxyResolvingClientSocketTest, ConnectError) {
  ...
  for (auto test : tests) {
    ...
    ProxyResolvingClientSocket proxy_resolving_socket(
        &socket_factory, url_request_context_getter_, net::SSLConfig(),
        GURL("https://" + dest.ToString()));
...
TEST_F(ProxyResolvingClientSocketTest, ReadWriteErrors) {
  ...
  for (auto test : tests) {
    ...
    ProxyResolvingClientSocket proxy_resolving_socket(
        &socket_factory, url_request_context_getter_, net::SSLConfig(), url);
    net::TestCompletionCallback callback;
    int status = proxy_resolving_socket.Connect(callback.callback());
    EXPECT_EQ(net::ERR_IO_PENDING, status);
    status = callback.WaitForResult();
...
TEST_F(ProxyResolvingClientSocketTest, ReusesHTTPAuthCache_Lookup) {
  ...
  ProxyResolvingClientSocket proxy_resolving_socket(
      &socket_factory, url_request_context_getter, net::SSLConfig(),
      GURL("https://" + dest.ToString()));
...
TEST_F(ProxyResolvingClientSocketTest, URLSanitized) {
  GURL url("http://username:password@www.example.com/?ref#hash#hash");
  ...
  net::MockClientSocketFactory socket_factory;
  ProxyResolvingClientSocket proxy_resolving_socket(
      &socket_factory, url_request_context_getter, net::SSLConfig(), url);
  ...
  EXPECT_EQ(GURL("http://www.example.com/?ref"),
