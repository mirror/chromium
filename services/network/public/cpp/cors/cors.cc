...
base::Optional<mojom::CORSError> CheckAccess(
  ...
  if (allow_all_origins) {
    ...
    //  2. We do not have a good way to construct url::Origin from a string,
    //     *allow_origin_header, that may be broken. Unfortunately
    //     url::Origin::Create(GURL(*allow_origin_header)) can accept malformed
    //     URL and construct a valid origin with unnecessary fixes.
    ...
    // validation, but would be ok for providing error details to developers.
