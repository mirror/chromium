...
void FaceDetectionProviderWin::CreateFaceDetection(
  ...
  // When |provider|[1] goes out of scope it will immediately closes its end of
  // the message pipe, then |async_create_detector_ops_| will be deleted and the
  // callback OnFaceDetectorCreated will be not called, the
  // PauseIncomingMethodCallProcessing prevent |binding_| from closing.
