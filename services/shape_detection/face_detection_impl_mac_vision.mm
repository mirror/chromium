...
FaceDetectionImplMacVision::FaceDetectionImplMacVision() {
  Class rectanglesRequestClass =
      NSClassFromString(@"VNDetectFaceRectanglesRequest");
  if (rectanglesRequestClass)
    rectangle_request_.reset([[rectanglesRequestClass alloc] init]);
}
...
bool FaceDetectionImplMacVision::PerformFaceRequest(const SkBitmap& bitmap) {
  Class imageHandlerClass = NSClassFromString(@"VNImageRequestHandler");
