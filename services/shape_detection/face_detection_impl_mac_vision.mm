...
FaceDetectionImplMacVision::FaceDetectionImplMacVision() {
  Class landmarks_request_class =
      NSClassFromString(@"VNDetectFaceLandmarksRequest");
  if (landmarks_request_class)
    landmarks_request_.reset([[landmarks_request_class alloc] init]);
}
