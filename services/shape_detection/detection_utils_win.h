...
enum class AsyncOperationStatus {
  ...
  ASYNC_FAILED,
  ASYNC_IDLE,
  ASYNC_CREATE_DETECTOR_SUCCEEDED
};

// A class represents an asynchronous operation which returns a result
// upon completion, internal async callback will be not called if
// the instance is deleted. RuntimeType is Windows Runtime APIs that
// have a result.
...
class AsyncOperation {
 public:
  ...
  // IAsyncOperationPtr is IAsyncOperation ComPtr type.
  ...
  // Create a AsyncOperation instance which set Callback to be called
  // when the asynchronous action completes.
  static std::unique_ptr<AsyncOperation<RuntimeType>> Create(
      Callback callback,
      IAsyncOperationPtr async_op_ptr) {
    auto async_operation = std::make_unique<AsyncOperation<RuntimeType>>(
        std::move(callback), async_op_ptr);

    base::WeakPtr<AsyncOperation> weak_ptr = async_operation->GetWeakPtr();
    ...
    const HRESULT hr =
        std::move(async_op_ptr)->put_Completed(asyncCallback.Get());
    if (FAILED(hr)) {
 ...
 private:
  base::WeakPtr<AsyncOperation> GetWeakPtr() {
