...
enum class AsyncOperationStatus { KOperating, KFailed, KIdle };
...
class AsyncOperation {
 public:
  ...
  // IAsyncOperationPtr is IAsyncOperation ComPtr type that will
  // be run with an empty async_op_ptr_ in case of error.
  ...
  static std::unique_ptr<AsyncOperation<RuntimeType>> Create(
    ...
    std::unique_ptr<AsyncOperation<RuntimeType>> instance;
    instance.reset(
        new AsyncOperation<RuntimeType>(std::move(callback), async_op_ptr));

    base::WeakPtr<AsyncOperation> weak_ptr = instance->GetWeakPtr();
