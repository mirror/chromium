...
void FaceDetectionImplWin::Detect(const SkBitmap& bitmap,
  ...
  if (async_status_ == ASYNC_OPERATING) {
    // TODO(junwei.fu): Save |callback| and |bitmap| for detecting seriality.
  } else if (async_status_ == ASYNC_IDLE) {
    // TODO(junwei.fu): Detect face async.
  } else if (async_status_ == ASYNC_FAILED) {
    // Return face 0.
...
HRESULT FaceDetectionImplWin::CreateFaceDetector() {
  ...
  auto callback = base::Bind(&FaceDetectionImplWin::OnFaceDetectorCreated,
                             base::Unretained(this));
  auto async_operation = std::make_unique<AsyncOperation<FaceDetector>>(
      std::move(callback), std::move(async_op));

  hr = async_operation->PutCompleted();
  if (FAILED(hr)) {
    DLOG(ERROR) << "Async Operate failed: "
                << logging::SystemErrorCodeToString(hr);
    return hr;
  }

  async_create_detector_ops_ = std::move(async_operation);
  async_status_ = ASYNC_OPERATING;
...
void FaceDetectionImplWin::OnFaceDetectorCreated(
    Microsoft::WRL::ComPtr<IAsyncOperation<FaceDetector*>> async_op) {
  HRESULT hr = async_op->GetResults(face_detector_.GetAddressOf());
  if (FAILED(hr)) {
    DLOG(ERROR) << "GetResults failed: "
                << logging::SystemErrorCodeToString(hr);
    async_status_ = ASYNC_FAILED;
    return;
  }

  async_status_ = ASYNC_IDLE;
}
