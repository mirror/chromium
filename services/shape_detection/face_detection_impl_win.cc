...
// Runs |callback| with an empty array of results.
void DetectedEmptyFace(mojom::FaceDetection::DetectCallback callback) {
...
void FaceDetectionImplWin::Detect(const SkBitmap& bitmap,
  ...
  if ((async_detect_face_ops_ = BeginDetect(bitmap))) {
    ...
    DetectedEmptyFace(std::move(callback));
  }
}
...
FaceDetectionImplWin::BeginDetect(const SkBitmap& bitmap) {
  ...
  // The |win_bitmap| need to be saved because detect face is async and it will
  // be destroyed in OnFaceDetected callback.
...
void FaceDetectionImplWin::OnFaceDetected(
  ...
  if (FAILED(hr)) {
    DLOG(ERROR) << "GetResults failed: "
                << logging::SystemErrorCodeToString(hr);
    DetectedEmptyFace(std::move(detected_face_callback_));
    return;
  }
