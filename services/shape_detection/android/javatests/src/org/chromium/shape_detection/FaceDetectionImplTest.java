...
public class FaceDetectionImplTest extends InstrumentationTestCase {
    ...
    @SmallTest
    @Feature({"ShapeDetection"})
    public void testDetectEulerYFaceWithGmsCore() {
        if (!TestUtils.IS_GMS_CORE_SUPPORTED) {
            return;
        }
    ...
    private void detectEulerZFace(Matrix matrix) {
    ...
    @SmallTest
    @Feature({"ShapeDetection"})
    public void testDetectEulerZFaceWithGmsCore() {
        if (!TestUtils.IS_GMS_CORE_SUPPORTED) {
            return;
        }
