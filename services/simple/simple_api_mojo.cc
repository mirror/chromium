...
class MyMath : public simple::Math {
  ...
  int32_t Add(int32_t x, int32_t y) override {
    ...
    (void)math_->Add(
        ...
        base::BindOnce(&MyMath::StoreResult, base::Unretained(this),
                       base::Unretained(&loop), base::Unretained(&sum)));
 ...
 private:
  void StoreResult(base::RunLoop* loop, int32_t* sum, int32_t result) {
    DBG("StoreResult %d\n", result);
    *sum = result;
    loop->Quit();
  }
...
class MyEcho : public simple::Echo {
 public:
  ...
  std::string Ping(const std::string& str) override {
    ...
    //              on Linux!?!
    base::RunLoop loop;
    echo_->Ping(str, base::BindOnce(
                         &MyEcho::StoreResult, base::Unretained(this),
 ...
 private:
  void StoreResult(base::RunLoop* loop,
                   std::string* out,
