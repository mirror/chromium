...
class MyMath : public simple::Math {
 public:
  ...
  int32_t Add(int32_t x, int32_t y) override {
    ...
    (void)math_->Add(
        x, y,
        base::BindOnce(&MyMath::StoreResult, base::Unretained(this),
                       base::Unretained(&sum)));
    loop.RunUntilIdle();
#endif
    return sum;
 ...
 private:
  void StoreResult(int32_t* sum, int32_t result) { *sum = result; }
...
class MyEcho : public simple::Echo {
 public:
  ...
  std::string Ping(const std::string& str) override {
    ...
    base::RunLoop loop;
    echo_->Ping(str,
                base::BindOnce(&MyEcho::StoreResult, base::Unretained(this),
 ...
 private:
  void StoreResult(std::string* out, const std::string& result) {
    *out = result;
  }
...
#ifdef USE_MOJO_SYNC_METHOD
  return "Use Mojo synchronous C++ bindings.";
#else
  return "Use Mojo asynchronous C++ bindings + RunUntilIdle().";
#endif
}
