...
void IdentityManager::GetAccounts(GetAccountsCallback callback) {
  ...
  std::vector<std::string> accounts_with_refresh_token =
      token_service_->GetAccounts();
  for (std::vector<std::string>::const_iterator it =
           accounts_with_refresh_token.begin();
       it != accounts_with_refresh_token.end(); ++it) {
    AccountInfo account_info = account_tracker_->GetAccountInfo(*it);
    AccountState account_state = GetStateOfAccount(account_info);
