...
operator=(IdentityManager::AccessTokenRequest&& other) {
  if (this != &other) {
    ...
    consumer_callback = std::move(other.consumer_callback);
  }
  return *this;
}
...
void IdentityManager::GetAccessToken(const std::string& account_id,
  ...
  access_token_request.consumer_callback = std::move(callback);
...
void IdentityManager::OnAccessTokenRequestCompleted(
  ...
  GetAccessTokenCallback consumer_callback =
      std::move(it->second.consumer_callback);

  access_token_requests_.erase(request);
  std::move(consumer_callback).Run(access_token, expiration_time, error);
}
