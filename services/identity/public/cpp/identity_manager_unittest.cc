...
class CustomFakeProfileOAuth2TokenService
 ...
 public:
  void set_on_access_token_invalidated_info(
      ...
      std::string expected_access_token,
      base::OnceClosure callback) {
    expected_account_id_ = expected_account_id;
    expected_scopes_ = expected_scopes;
    expected_access_token_ = expected_access_token;
 ...
 private:
  ...
  void InvalidateAccessTokenImpl(const std::string& account_id,
    ...
    if (on_access_token_invalidated_callback_) {
      ...
      EXPECT_EQ(expected_scopes_, scopes);
      EXPECT_EQ(expected_access_token_, access_token);
      std::move(on_access_token_invalidated_callback_).Run();
    }
  }

  std::string expected_account_id_;
  std::set<std::string> expected_scopes_;
