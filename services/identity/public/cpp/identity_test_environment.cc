...
void IdentityTestEnvironment::MakePrimaryAccountAvailable(
  ...
  internals_->signin_manager()->SetAuthenticatedAccountInfo(gaia_id,
                                                            email_address);
  internals_->token_service()->UpdateCredentials(
      internals_->signin_manager()->GetAuthenticatedAccountId(), refresh_token);
}
