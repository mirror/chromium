...
class TestIdentityProxyObserver : IdentityProxy::Observer {
 public:
  ...
  void set_on_login_callback(const base::OnceClosure& callback) {
    on_login_callback_ = std::move(callback);
  }
  void set_on_logout_callback(const base::OnceClosure& callback) {
...
// TODO(blundell): Should this be only !CrOS?
