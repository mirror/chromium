...
#if 0
  // Spin the runloop long enough that the Identity Manager will receive
  // and respond to this request *if* the connection is allowed. Local
  // experimentation indicates that 2500 microseconds is long enough to allow
  // this round trip to occur; we double that to give padding. Note that if this
  // time is ever too short it won't cause this test to hang; it will simply
  // mean that in that case the test might pass because the connection wasn't
  // made *yet* as opposed to wasn't made *at all*.
  base::RunLoop run_loop;
