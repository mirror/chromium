...
class IdentityManagerTest : public service_manager::test::ServiceTest {
 ...
 protected:
  ...
  void TearDown() override {
    ...
    // destroyed before proceeding, since the signin classes that it is passed
    // must outlive it.
...
// Check that NotifyOnPrimaryAccountAvailable() returns immediately in the
// case where the primary account is available when the call is received.
TEST_F(IdentityManagerTest, NotifyOnPrimaryAccountAvailableSignedIn) {
  signin_manager()->SetAuthenticatedAccountInfo(kTestGaiaId, kTestEmail);
  token_service()->UpdateCredentials(
      signin_manager()->GetAuthenticatedAccountId(), kTestRefreshToken);
  ...
  base::RunLoop run_loop;
  identity_manager_->NotifyOnPrimaryAccountAvailable(base::Bind(
...
// Check that NotifyOnPrimaryAccountAvailable() returns the expected account
// info in the case where the primary account is made available *after* the
// call is received.
TEST_F(IdentityManagerTest, NotifyOnPrimaryAccountAvailableSignInLater) {
  AccountInfo account_info;
  AccountState account_state;
...
TEST_F(IdentityManagerTest, NotifyOnPrimaryAccountAvailableOverlappingCalls) {
