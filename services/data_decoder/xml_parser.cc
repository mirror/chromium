...
// Returns true if an empty dictionary was found and removed, false otherwise.
...
base::Value* AddContentToNode(base::Value* parent_node,
  ...
  if (node->is_string() || node->is_dict()) {
    // There is already a value in this node, replace it with a list.
  ...
  if (node->is_list()) {
    // There are already multiple values in this node.
