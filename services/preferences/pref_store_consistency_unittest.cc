...
constexpr int kPrefStoreConsistencyUnittestInitialValue = 1;
constexpr char kPrefStoreConsistencyUnittestKey[] = "key";
constexpr char kPrefStoreConsistencyUnittestChildKey[] = "child";
constexpr char kPrefStoreConsistencyUnittestOtherKey[] = "other_key";
constexpr char kPrefStoreConsistencyUnittestDictionaryKey[] =
    "a.dictionary.pref";
 ...
 public:
  explicit PrefServiceConnection(PersistentPrefStoreImpl* pref_store)
    ...
    auto connection = pref_store->CreateConnection({
        kPrefStoreConsistencyUnittestKey, kPrefStoreConsistencyUnittestOtherKey,
        kPrefStoreConsistencyUnittestDictionaryKey,
    });
    ...
    pref_registry->RegisterIntegerPref(
        kPrefStoreConsistencyUnittestKey,
        kPrefStoreConsistencyUnittestInitialValue);
    pref_registry->RegisterIntegerPref(
        kPrefStoreConsistencyUnittestOtherKey,
        kPrefStoreConsistencyUnittestInitialValue);
...
TEST_F(PersistentPrefStoreConsistencyTest, TwoPrefs) {
  pref_store()->SetValue(
      kPrefStoreConsistencyUnittestKey,
      std::make_unique<base::Value>(kPrefStoreConsistencyUnittestInitialValue),
      0);
  ...
  EXPECT_EQ(kPrefStoreConsistencyUnittestInitialValue,
            pref_service.GetInteger(kPrefStoreConsistencyUnittestKey));
  EXPECT_EQ(kPrefStoreConsistencyUnittestInitialValue,
            pref_service2.GetInteger(kPrefStoreConsistencyUnittestKey));

  pref_service.SetInteger(kPrefStoreConsistencyUnittestKey, 2);
  pref_service2.SetInteger(kPrefStoreConsistencyUnittestKey, 3);
  ...
  EXPECT_EQ(2, pref_service.GetInteger(kPrefStoreConsistencyUnittestKey));

  // This connection already has a later value. It should not move backwards in
  // time.
  connection2->ForwardUpdates(1);
  EXPECT_EQ(3, pref_service2.GetInteger(kPrefStoreConsistencyUnittestKey));

  connection->ForwardUpdates(1);
  EXPECT_EQ(3, pref_service.GetInteger(kPrefStoreConsistencyUnittestKey));
  connection2->ForwardUpdates(1);
  EXPECT_EQ(3, pref_service2.GetInteger(kPrefStoreConsistencyUnittestKey));
}
...
TEST_F(PersistentPrefStoreConsistencyTest, TwoSubPrefs) {
  ...
  auto& pref_service2 = connection2->pref_service();
  {
    ScopedDictionaryPrefUpdate update(
        &pref_service, kPrefStoreConsistencyUnittestDictionaryKey);
    update->SetInteger(kPrefStoreConsistencyUnittestKey, 2);
  }
  {
    ScopedDictionaryPrefUpdate update(
        &pref_service2, kPrefStoreConsistencyUnittestDictionaryKey);
    update->SetInteger(kPrefStoreConsistencyUnittestKey, 3);
  }
  ...
  two_dict.SetInteger(kPrefStoreConsistencyUnittestKey, 2);
  base::DictionaryValue three_dict;
  three_dict.SetInteger(kPrefStoreConsistencyUnittestKey, 3);

  connection->ForwardUpdates(1);
  EXPECT_EQ(two_dict, *pref_service.GetDictionary(
                          kPrefStoreConsistencyUnittestDictionaryKey));

  // This connection already has a later value. It should not move backwards in
  // time.
  connection2->ForwardUpdates(1);
  EXPECT_EQ(three_dict, *pref_service2.GetDictionary(
                            kPrefStoreConsistencyUnittestDictionaryKey));

  connection->ForwardUpdates(1);
  EXPECT_EQ(three_dict, *pref_service.GetDictionary(
                            kPrefStoreConsistencyUnittestDictionaryKey));
  connection2->ForwardUpdates(1);
  EXPECT_EQ(three_dict, *pref_service2.GetDictionary(
                            kPrefStoreConsistencyUnittestDictionaryKey));
...
TEST_F(PersistentPrefStoreConsistencyTest, DifferentSubPrefs) {
  ...
  }
  {
    ScopedDictionaryPrefUpdate update(
        &pref_service2, kPrefStoreConsistencyUnittestDictionaryKey);
    update->SetInteger(kPrefStoreConsistencyUnittestOtherKey, 3);
  }
  ...
  expected_dict.SetInteger(kPrefStoreConsistencyUnittestKey, 2);
  expected_dict.SetInteger(kPrefStoreConsistencyUnittestOtherKey, 3);
  ...
  // time.
  connection2->ForwardUpdates(1);
  EXPECT_EQ(expected_dict, *pref_service2.GetDictionary(
                               kPrefStoreConsistencyUnittestDictionaryKey));

  connection->ForwardUpdates(1);
  EXPECT_EQ(expected_dict, *pref_service.GetDictionary(
                               kPrefStoreConsistencyUnittestDictionaryKey));
  connection2->ForwardUpdates(1);
  EXPECT_EQ(expected_dict, *pref_service2.GetDictionary(
                               kPrefStoreConsistencyUnittestDictionaryKey));
...
TEST_F(PersistentPrefStoreConsistencyTest, WriteParentThenChild) {
  ...
  initial_value->SetDictionary(kPrefStoreConsistencyUnittestDictionaryKey,
                               std::make_unique<base::DictionaryValue>());
  pref_store()->SetValue(kPrefStoreConsistencyUnittestDictionaryKey,
                         std::move(initial_value), 0);
  auto connection = CreateConnection();
  auto connection2 = CreateConnection();
  auto& pref_service = connection->pref_service();
  auto& pref_service2 = connection2->pref_service();
  {
    ScopedDictionaryPrefUpdate update(
        &pref_service, kPrefStoreConsistencyUnittestDictionaryKey);
    update->SetInteger(kPrefStoreConsistencyUnittestKey, 4);
    update->Clear();
    update->SetInteger(kPrefStoreConsistencyUnittestKey, 2);
    update->SetInteger(kPrefStoreConsistencyUnittestOtherKey, 4);
  }
  {
    ScopedDictionaryPrefUpdate update(
        &pref_service2, kPrefStoreConsistencyUnittestDictionaryKey);
  ...
  two_and_four_dict.SetInteger(kPrefStoreConsistencyUnittestKey, 2);
  two_and_four_dict.SetInteger(kPrefStoreConsistencyUnittestOtherKey, 4);
  base::DictionaryValue three_dict;
  three_dict.SetInteger(kPrefStoreConsistencyUnittestKey, 3);
  three_dict.SetDictionary(kPrefStoreConsistencyUnittestDictionaryKey,
                           std::make_unique<base::DictionaryValue>());
  base::Value five_dict = three_dict.Clone();
  five_dict.SetKey(kPrefStoreConsistencyUnittestKey, base::Value(5));
  base::DictionaryValue expected_dict;
  expected_dict.SetInteger(kPrefStoreConsistencyUnittestKey, 3);
  expected_dict.SetInteger(kPrefStoreConsistencyUnittestOtherKey, 4);

  connection->ForwardUpdates(1);
  EXPECT_EQ(two_and_four_dict, *pref_service.GetDictionary(
                                   kPrefStoreConsistencyUnittestDictionaryKey));
  connection->ForwardUpdates(1);
  EXPECT_EQ(expected_dict, *pref_service.GetDictionary(
                               kPrefStoreConsistencyUnittestDictionaryKey));
  ...
  // service.
  {
    ScopedDictionaryPrefUpdate update(
        &pref_service2, kPrefStoreConsistencyUnittestDictionaryKey);
    update->SetInteger(kPrefStoreConsistencyUnittestKey, 5);
  }
  connection2->ForwardWrites(1);

  expected_dict.SetInteger(kPrefStoreConsistencyUnittestKey, 5);
  connection->ForwardUpdates(1);
  EXPECT_EQ(expected_dict, *pref_service.GetDictionary(
                               kPrefStoreConsistencyUnittestDictionaryKey));
  ...
  connection2->ForwardUpdates(1);
  EXPECT_EQ(five_dict, *pref_service2.GetDictionary(
                           kPrefStoreConsistencyUnittestDictionaryKey));
...
TEST_F(PersistentPrefStoreConsistencyTest, WriteChildThenDeleteParent) {
  ...
  auto& pref_service = connection->pref_service();
  auto& pref_service2 = connection2->pref_service();
  pref_service.ClearPref(kPrefStoreConsistencyUnittestDictionaryKey);
  EXPECT_FALSE(
      pref_service.HasPrefPath(kPrefStoreConsistencyUnittestDictionaryKey));
  {
    ScopedDictionaryPrefUpdate update(
        &pref_service2, kPrefStoreConsistencyUnittestDictionaryKey);
    update->SetInteger(kPrefStoreConsistencyUnittestKey, 3);
  ...
  connection2->ForwardUpdates(1);
  EXPECT_FALSE(
      pref_service2.HasPrefPath(kPrefStoreConsistencyUnittestDictionaryKey));
}

TEST_F(PersistentPrefStoreConsistencyTest, DeleteParentThenWriteChild) {
  auto initial_value = std::make_unique<base::DictionaryValue>();
  initial_value->SetInteger(kPrefStoreConsistencyUnittestOtherKey, 5);
  pref_store()->SetValue(kPrefStoreConsistencyUnittestDictionaryKey,
                         std::move(initial_value), 0);
  auto connection = CreateConnection();
  auto connection2 = CreateConnection();
  ...
  intermediate_dict.SetInteger(kPrefStoreConsistencyUnittestKey, 3);
  intermediate_dict.SetInteger(kPrefStoreConsistencyUnittestOtherKey, 5);
  ...
  connection2->ForwardUpdates(1);
  EXPECT_EQ(intermediate_dict, *pref_service2.GetDictionary(
                                   kPrefStoreConsistencyUnittestDictionaryKey));
...
TEST_F(PersistentPrefStoreConsistencyTest, WriteParentThenDeleteChild) {
  ...
  auto& pref_service2 = connection2->pref_service();
  {
    ScopedDictionaryPrefUpdate update(
        &pref_service, kPrefStoreConsistencyUnittestDictionaryKey);
    update->SetInteger(kPrefStoreConsistencyUnittestKey, 1);
    update->Clear();
    update->SetInteger(kPrefStoreConsistencyUnittestKey, 2);
    update->SetInteger(kPrefStoreConsistencyUnittestOtherKey, 4);
  }
  {
    ScopedDictionaryPrefUpdate update(
        &pref_service2, kPrefStoreConsistencyUnittestDictionaryKey);
    update->SetInteger(kPrefStoreConsistencyUnittestKey, 3);
    update->RemovePath(kPrefStoreConsistencyUnittestKey, nullptr);
  }
  ...
  connection2->ForwardUpdates(1);
  EXPECT_EQ(empty_dict, *pref_service2.GetDictionary(
                            kPrefStoreConsistencyUnittestDictionaryKey));
...
TEST_F(PersistentPrefStoreConsistencyTest, ReplaceParentThenWriteChild) {
  ...
  }
  {
    ScopedDictionaryPrefUpdate update(
        &pref_service2, kPrefStoreConsistencyUnittestDictionaryKey);
    update->SetPath({kPrefStoreConsistencyUnittestKey,
                     kPrefStoreConsistencyUnittestChildKey},
                    base::Value(2));
  }
  ...
  simple_dict.SetInteger(kPrefStoreConsistencyUnittestKey, 1);
  base::DictionaryValue nested_dict;
  nested_dict.SetPath(
      {kPrefStoreConsistencyUnittestKey, kPrefStoreConsistencyUnittestChildKey},
      base::Value(2));
  nested_dict.SetPath(
      {kPrefStoreConsistencyUnittestKey, kPrefStoreConsistencyUnittestOtherKey},
      base::Value(5));
  base::DictionaryValue expected_dict;
  expected_dict.SetPath(
      {kPrefStoreConsistencyUnittestKey, kPrefStoreConsistencyUnittestChildKey},
      base::Value(2));

  connection->ForwardUpdates(1);
  EXPECT_EQ(simple_dict, *pref_service.GetDictionary(
                             kPrefStoreConsistencyUnittestDictionaryKey));
  connection->ForwardUpdates(1);
  EXPECT_EQ(expected_dict, *pref_service.GetDictionary(
                               kPrefStoreConsistencyUnittestDictionaryKey));
  ...
  connection2->ForwardUpdates(1);
  EXPECT_EQ(nested_dict, *pref_service2.GetDictionary(
                             kPrefStoreConsistencyUnittestDictionaryKey));
...
TEST_F(PersistentPrefStoreConsistencyTest, WriteChildThenReplaceParent) {
  ...
  connection2->ForwardUpdates(1);
  EXPECT_EQ(simple_dict, *pref_service2.GetDictionary(
                             kPrefStoreConsistencyUnittestDictionaryKey));
}

TEST_F(PersistentPrefStoreConsistencyTest, NestedWriteParentThenChild) {
  pref_store()->SetValue(kPrefStoreConsistencyUnittestKey,
                         std::make_unique<base::DictionaryValue>(), 0);
  auto connection = CreateConnection();
  auto connection2 = CreateConnection();
  auto& pref_service = connection->pref_service();
  auto& pref_service2 = connection2->pref_service();
  {
    ScopedDictionaryPrefUpdate update(
        &pref_service, kPrefStoreConsistencyUnittestDictionaryKey);
    auto nested_dict =
        update->SetDictionary(kPrefStoreConsistencyUnittestKey,
                              std::make_unique<base::DictionaryValue>());
    nested_dict->SetInteger(kPrefStoreConsistencyUnittestChildKey, 2);
    nested_dict->SetInteger(kPrefStoreConsistencyUnittestOtherKey, 4);
  }
  {
    ScopedDictionaryPrefUpdate update(
        &pref_service2, kPrefStoreConsistencyUnittestDictionaryKey);
...
TEST_F(PersistentPrefStoreConsistencyTest,
  ...
  connection->ForwardUpdates(1);
  EXPECT_EQ(empty_dict, *pref_service.GetDictionary(
                            kPrefStoreConsistencyUnittestDictionaryKey));
