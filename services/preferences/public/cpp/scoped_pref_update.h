...
//
//   prefs::DictionaryValueUpdate* nested_dictionary = nullptr;
...
class ScopedDictionaryPrefUpdate {
 ...
 private:
  ...
  DictionaryValueUpdate* Insert(base::DictionaryValue* value,
  ...
  // Owns all DictionaryValueUpdates created by requests for nested
  // DictionaryValues, including the pref DictionaryValue.
  std::vector<std::unique_ptr<DictionaryValueUpdate>> updates_;

  // Value from user pref store (set between Get() and Notify() calls).
  // Owned by |updates_|.
...
class DictionaryValueUpdate {
 ...
 private:
  ...
  ScopedDictionaryPrefUpdate* const update_;
