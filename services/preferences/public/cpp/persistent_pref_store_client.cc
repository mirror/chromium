...
class PersistentPrefStoreClient::InFlightWriteTrie {
 public:
  enum class Decision {
    ...
    // The write has already been superceded locally and should be blocked.
    kBlock,

    // The write may have been partially superceded locally and should be
    // blocked and an updated value is needed.
    kResolve
  };
  ...
  bool Remove(It path_start, Jt path_end) {
    if (path_start == path_end) {
      DCHECK_GT(value_, 0);
      return --value_ == 0 && children_.empty();
    }
    ...
    return children_.empty() && value_ == 0;
  }
