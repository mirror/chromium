...
ScopedDictionaryPrefUpdate::ScopedDictionaryPrefUpdate(PrefService* service,
  ...
  value_ = Insert(value, std::vector<std::string>());
}
...
DictionaryValueUpdate* ScopedDictionaryPrefUpdate::Insert(
  ...
  updates_.push_back(base::WrapUnique(
      new DictionaryValueUpdate(this, value, std::move(path))));
...
DictionaryValueUpdate* DictionaryValueUpdate::SetDictionary(
  ...
  return update_->Insert(dictionary_value, ConcatPath(path_, path));
}
...
DictionaryValueUpdate* DictionaryValueUpdate::SetDictionaryWithoutPathExpansion(
  ...
  return update_->Insert(dictionary_value, std::move(full_path));
}
...
bool DictionaryValueUpdate::GetDictionary(base::StringPiece path,
  ...
  *out_value = update_->Insert(dictionary_value, ConcatPath(path_, path));
  return true;
}
...
bool DictionaryValueUpdate::GetDictionaryWithoutPathExpansion(
  ...
  *out_value = update_->Insert(dictionary_value, std::move(full_path));
  return true;
}
