...
class AudioSystemToMojoAdapterTest : public testing::Test {
 ...
 protected:
  ...
  // Called when AudioSystemInfo bind request is received. Nice mock because
  // AudioSystem conformance tests won't set expecnations.
  NiceMock<MockFunction<void(void)>> audio_system_info_bind_requested_;
 ...
 private:
  void BindAudioSystemInfoRequest(mojom::AudioSystemInfoRequest info_request) {
    ...
    audio_system_info_bind_requested_.Call();
    audio_system_info_binding_->Bind(std::move(info_request));
  }
...
TEST_F(AudioSystemToMojoAdapterTest,
  ...
  {  // GetInputStreamParameters succeeds.
    ...
    // Should bind:
    EXPECT_CALL(audio_system_info_bind_requested_, Call()).Times(Exactly(1));
  ...
  {  // GetInputStreamParameters succeeds second time.
    ...
    EXPECT_CALL(audio_system_info_bind_requested_, Call()).Times(Exactly(0));
    audio_system_->GetInputStreamParameters(
        media::AudioDeviceDescription::kDefaultDeviceId,
        expectations.GetAudioParamsCallback(
            FROM_HERE, wait_loop.QuitClosure(),
...
// AudioSystemInfo method. DO NOT COMMIT BEFORE THAT.
