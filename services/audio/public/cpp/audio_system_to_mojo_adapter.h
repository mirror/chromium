...
// Provides AudioSystem implementation on top of Audio service.
class AudioSystemToMojoAdapter : public media::AudioSystem {
 public:
  using BindSystemInfoCb =
      base::RepeatingCallback<void(mojom::SystemInfoRequest request)>;

  static std::unique_ptr<media::AudioSystem> Create(
      service_manager::Connector* connector);

  explicit AudioSystemToMojoAdapter(BindSystemInfoCb bind_system_info_cb);
 ...
 private:
  ...
  BindSystemInfoCb bind_system_info_cb_;
