...
void BindSystemInfo(service_manager::Connector* connector,
...
std::unique_ptr<media::AudioSystem> AudioSystemToMojoAdapter::Create(
  ...
  DCHECK(connector);
  // The callback will own a copy of the connector which will be bound to the
  // thread AudioSystemToMojoAdapter is used on.
  return std::make_unique<AudioSystemToMojoAdapter>(base::BindRepeating(
      &BindSystemInfo, base::Owned(connector->Clone().release())));
}
