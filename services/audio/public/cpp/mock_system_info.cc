...
MockSystemInfo::MockSystemInfo(media::AudioManager* audio_manager)
    : audio_manager_(audio_manager),
      system_info_(nullptr,
                   base::OnTaskRunnerDeleter(audio_manager_->GetTaskRunner())) {
}

MockSystemInfo::~MockSystemInfo() = default;
...
void MockSystemInfo::Bind(const std::string& interface_name,
  ...
  if (!system_info_)
    system_info_.reset(new SystemInfo(audio_manager_));
  system_info_->Bind(mojom::SystemInfoRequest(std::move(handle)));
}

base::SequencedTaskRunner* MockSystemInfo::GetTaskRunner() {
