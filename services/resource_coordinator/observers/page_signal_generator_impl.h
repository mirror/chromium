...
class PageSignalGeneratorImpl : public CoordinationUnitGraphObserver,
 ...
 public:
  // The state transitions for the LoadedAndAlmostIdle signal. In general a
  // page transitions through these states from top to bottom.
  enum LoadIdleState {
    // The initial state. Can only transition the kLoading from here.
    kLoadingNotStarted,
    ...
    // be emitted and no further state transitions will be tracked.
    kLoadedAndIdle
  };
 ...
 private:
  ...
  // appropriate Page variant, where they real work is done.
