...
// considered loaded and idle. This is used in UpdateLoadIdleState transitions.
constexpr base::TimeDelta kLoadedAndIdlingTimeout =
...
void PageSignalGeneratorImpl::OnCoordinationUnitCreated(
  ...
  auto* page_cu = reinterpret_cast<const PageCoordinationUnitImpl*>(cu);
  DCHECK_EQ(0u, page_data_.count(page_cu));  // No data should exist yet.
...
void PageSignalGeneratorImpl::UpdateLoadIdleStatePage(
  ...
  page_data->idling_timer.Start(
      ...
      base::BindOnce(&PageSignalGeneratorImpl::UpdateLoadIdleStatePage,
...
PageSignalGeneratorImpl::PageData* PageSignalGeneratorImpl::GetPageData(
  ...
  // 1. Via On*PropertChange calls. Ths backing PageData is guaranteed to
  ...
  //    have been canceled.
  auto it = page_data_.find(page_cu);
  DCHECK(it != page_data_.end());
  auto* page_data = &(it->second);
  return page_data;
}
...
bool PageSignalGeneratorImpl::IsIdling(
  ...
  const FrameCoordinationUnitImpl* main_frame_cu = nullptr;
  for (auto* frame_cu : page_cu->GetFrameCoordinationUnits()) {
    if (frame_cu->IsMainFrame()) {
      main_frame_cu = frame_cu;
      break;
    }
  }
  if (!main_frame_cu)
  ...
  // Get the associated task and network loads.
  ...
  // Calculate whether or not this page is idling.
  bool is_idling = main_thread_task_load_is_low && is_network_almost_idle;
  return is_idling;
}
