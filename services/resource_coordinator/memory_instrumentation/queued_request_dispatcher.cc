...
void QueuedRequestDispatcher::Finalize(QueuedRequest* request,
  ...
  for (const auto& response : request->responses) {
    ...
    if (request->add_to_trace) {
      bool trace_os_dump_success = tracing_observer->AddOsDumpToTraceIfEnabled(
          request->args, pid, os_dump.get(), &raw_os_dump->memory_maps);
      if (!trace_os_dump_success)
        request->failed_memory_dump_count++;

      if (raw_chrome_dump) {
        bool trace_chrome_dump_success = AddChromeMemoryDumpToTrace(
            request->args, pid, *raw_chrome_dump, *global_graph,
            pid_to_process_type, tracing_observer);
        if (!trace_chrome_dump_success)
          request->failed_memory_dump_count++;
      }
    }
