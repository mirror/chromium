...
void QueuedRequestDispatcher::SetUpAndDispatch(
  ...
  TRACE_EVENT_NESTABLE_ASYNC_BEGIN2(
      ...
      TRACE_ID_LOCAL(request->request_args.dump_guid), "dump_type",
  ...
  for (const auto& client_info : clients) {
    ...
    if (request->wants_chrome_dumps()) {
      ...
      client->RequestChromeMemoryDump(request->request_args,
...
void QueuedRequestDispatcher::Finalize(QueuedRequest* request,
  ...
  for (const auto& response : request->responses) {
    ...
    if (request->args.add_to_trace) {
      tracing_observer->AddOsDumpToTraceIfEnabled(
          request->request_args, pid, os_dump.get(), &raw_os_dump->memory_maps);
    }
  ...
  callback.Run(global_success, request->request_args.dump_guid,
  ...
  char guid_str[20];
  sprintf(guid_str, "0x%" PRIx64, request->request_args.dump_guid);
  TRACE_EVENT_NESTABLE_ASYNC_END2(
      base::trace_event::MemoryDumpManager::kTraceCategory, "GlobalMemoryDump",
      TRACE_ID_LOCAL(request->request_args.dump_guid), "dump_guid",
