...
// get around to sandboxing/selinux restrictions (see crbug.com/461788).
  ...
  request->responses[browser_client] = browser_client_pid;
  const auto callback = base::Bind(os_callback, browser_client);
...
// static
QueuedRequestDispatcher::VmRegions QueuedRequestDispatcher::Finalize(
    QueuedVmRegionRequest* request) {
  VmRegions results;
  for (auto& response : request->responses) {
    ...
    for (auto& kv : extra_os_dumps) {
      ...
      DCHECK(results[original_pid].empty());

      results[original_pid] = std::move(kv.second->memory_maps);
    }
#endif
  }
  return results;
