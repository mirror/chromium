...
void QueuedRequestDispatcher::SetUpAndDispatch(
  ...
  for (const auto& client_info : clients) {
    ...
    // with different pid.
    if (request->pid && client_info.pid != request->pid.value())
      continue;
...
// so ask each process to do so Linux is special see below.
  ...
  if (request->pid && request->pending_responses.empty()) {
...
// get around to sandboxing/selinux restrictions (see crbug.com/461788).
  ...
  pids.reserve(request->pid ? 1 : clients.size());
  for (const auto& client_info : clients) {
    if (!request->pid || client_info.pid == request->pid.value()) {
  ...
  // In the single pid case, we should have exactly one pid we are requesting
  // data for.
  DCHECK(!request->pid || pids.size() == 1);
...
void QueuedRequestDispatcher::Finalize(QueuedRequest* request,
  ...
  std::move(request->callback)
      .Run(global_success, request->args.dump_guid,
           !global_success && request->pid ? nullptr : std::move(global_dump));
