...
TEST(GraphProcessorTest, Process) {
  ...
  auto global_dump = ProcessGlobalDump(std::move(process_dumps));
  for (auto& id_to_dump : global_dump->container_dumps()) {
    auto* first_child = id_to_dump.second->FindNodeInGraph("test1");
    ...
    auto* direct = id_to_dump.second->FindNodeInGraph("test1/test2/test3");
