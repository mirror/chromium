...
TEST_F(GraphProcessorTest, Process) {
  ...
  auto global_dump = processor.Process(std::move(process_dumps));
  for (auto& id_to_dump : global_dump->container_dumps()) {
    auto* root = id_to_dump.second->dump_root();

    auto* first_child = root->GetChild("test1");
