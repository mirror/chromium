...
TEST_F(GraphProcessorTest, CalculateDumpOwnershipCoefficient) {
  ...
  ASSERT_EQ(owned->owned_coefficient(), 2.0 / 10.0);
  ASSERT_EQ(owner_1->owning_coefficient(), 3.0 / 6.0);
  ASSERT_EQ(owner_2->owning_coefficient(), 4.0 / 7.0);
  ASSERT_EQ(owner_3->owning_coefficient(), 0.0 / 5.0);
  ASSERT_EQ(owner_4->owning_coefficient(), 1.0 / 8.0);
}
...
TEST_F(GraphProcessorTest, CalculateDumpCumulativeOwnershipCoefficient) {
  ...
  ASSERT_EQ(c1_c1->cumulative_owning_coefficient(), 0.123);
  ASSERT_EQ(c1_c1->cumulative_owned_coefficient(), 0.456 * 0.321);

  CalculateDumpCumulativeOwnershipCoefficient(c1_c2);
  ASSERT_EQ(c1_c2->cumulative_owning_coefficient(), 0.135 * 0.789);
  ASSERT_EQ(c1_c2->cumulative_owned_coefficient(), 0.456 * 0.246);
}
