...
TEST_F(GraphProcessorTest, VisitInDepthFirstPostOrder) {
  GlobalDumpGraph graph;
  Process process(1, &graph);
  Node* root = process.root();
  ...
  // |c3_c2| owns |c2_c2|. Note this is opposite of tests above.
  Edge edge(&c3_c2, &c2_c2, 0);
  c2_c2.AddOwnedByEdge(&edge);
  c3_c2.SetOwnsEdge(&edge);
