...
TEST_F(CoordinatorImplTest, VmRegionsForHeapProfiler) {
  base::RunLoop run_loop;
  constexpr base::ProcessId kBrowserPid = 1;
  constexpr base::ProcessId kRendererPid = 2;
...
// platforms, they are expected to come from each individual client.
  ...
  EXPECT_CALL(callback, OnCall(true, NotNull()))
      .WillOnce(Invoke([&run_loop, kBrowserPid, kRendererPid](
                           bool success, GlobalMemoryDump* global_dump) {
