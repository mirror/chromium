 ...
 public:
  ...
  class Node {
   public:
    ...
    Node* GetChild(base::StringPiece subpath);

    // Inserts the given |node| as a child of the current node
    // with the given |subpath| as the key.
    void InsertChild(base::StringPiece subpath, Node* node);
  ...
  class Edge {
   ...
   private:
    GlobalDumpGraph::Node* source_;
    GlobalDumpGraph::Node* target_;
    const int priority_;
    bool weak_;
  };
  ...
  // and edge priority.
  bool AddNodeOwnershipEdge(Node* owner, Node* owned, int priority);
