...
Node* Process::CreateNode(MemoryAllocatorDumpGuid guid,
  ...
  base::StringTokenizer tokenizer(path.as_string(), "/");
...
Node* Node::GetChild(base::StringPiece subpath) {
  DCHECK(!subpath.empty());
  DCHECK_EQ(subpath.find('/'), std::string::npos);

  auto child = children_.find(subpath.as_string());
  return child == children_.end() ? nullptr : child->second;
}

void Node::InsertChild(base::StringPiece subpath, Node* node) {
  DCHECK(!subpath.empty());
  DCHECK_EQ(subpath.find('/'), std::string::npos);
