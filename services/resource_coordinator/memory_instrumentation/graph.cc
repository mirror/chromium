...
DumpNode* ContainerDump::CreateNodeInGraph(const std::string& path) {
...
DumpNode* ContainerDump::FindNodeInGraph(const std::string& path) {
  auto pieces = base::SplitString(path, "/", base::KEEP_WHITESPACE,
                                  base::SPLIT_WANT_NONEMPTY);
  DumpNode* current = root_;
  for (std::string& key : pieces) {
...
void DumpNode::InsertChild(std::string subpath, DumpNode* node) {
  DCHECK_NE(subpath, "");
