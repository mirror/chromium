...
GraphProcessor::ComputeSharedFootprintFromGraph(GlobalDumpGraph* global_graph) {
  // Go through all nodes associated with global dumps and find if they are
  // owned by shared memory nodes.
  Process* global_process = global_graph->shared_memory_graph();
  Node* global_node = global_process->root()->GetChild("global");

  std::map<Node*, std::list<Edge*>> global_node_to_shared_owners;
  for (const auto& path_to_child : *global_node->children()) {
    Node* child = path_to_child.second;

    // If there's no size to attribute, there's no point in propogating
    // anything.
    if (child->entries().find("size") == child->entries().end())
      continue;

    for (auto* edge : *child->owned_by_edges()) {
      // Find if the source node's path starts with "shared/" which indcates
      // shared memory.
      ...
      if (source_root->GetChild("shared_memory") == current)
        global_node_to_shared_owners[child].push_back(edge);
    }
  }
  ...
  for (auto& global_to_shared_edges : global_node_to_shared_owners) {
    std::list<Edge*>* edges = &global_to_shared_edges.second;
    auto max_edge_iter = std::max_element(
        edges->begin(), edges->end(), [](Edge* first, Edge* second) {
          return first->priority() < second->priority();
        });
    int max_priority = (*max_edge_iter)->priority();
    edges->remove_if(
  ...
  for (const auto& global_to_shared_edges : global_node_to_shared_owners) {
    ...
    const auto& edges = global_to_shared_edges.second;
