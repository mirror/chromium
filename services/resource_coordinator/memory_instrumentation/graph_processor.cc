...
void AddEdges(const base::trace_event::ProcessMemoryDump& source,
  ...
  for (auto& guid_to_edge : source.allocator_dumps_edges_for_testing()) {
    ...
    if (source_lost) {
      ...
      // If the target is lost but the source is present, then TODO.
      continue;
    } else {
...
void MarkWeakNodesRecursively(Node* node) {
  ...
  for (auto& path_to_node : *node->children()) {
    MarkWeakNodesRecursively(path_to_node.second);
  }
}

void RemoveWeakNodesRecursively(std::map<std::string, Node*>* nodes) {
  for (auto path_to_node_it = nodes->begin();
       path_to_node_it != nodes->end();) {
    Node* node = path_to_node_it->second;
    if (node->is_weak()) {
      path_to_node_it = nodes->erase(path_to_node_it);
    } else {
      RemoveWeakNodesRecursively(node->children());
...
void RemoveWeakDumps(GlobalDumpGraph* graph) {
  // First pass: mark recursively nodes as weak if they own a node which is
  // weak. Implicitly consider child nodes as weak if parent is weak.
  for (auto& pid_to_process : graph->process_dump_graphs()) {
    MarkWeakNodesRecursively(pid_to_process.second->root());
  }

  // Second pass: remove all nodes which are weak (including their descendants)
  // and clean owned by edges to match.
  for (auto& pid_to_process : graph->process_dump_graphs()) {
    RemoveWeakNodesRecursively(pid_to_process.second->root()->children());
  }
}
...
std::unique_ptr<GlobalDumpGraph> ComputeMemoryGraph(
  ...
  // Third pass: remove all weak dumps in the graph.
  RemoveWeakDumps(global_graph.get());
