...
void CoordinatorImpl::RegisterClientProcess(
  ...
  base::ProcessId pid = GetProcessIdForClientIdentity(identity);
  auto client_info = base::MakeUnique<ClientInfo>(
      std::move(identity), std::move(client_process_ptr), pid, process_type);
...
void CoordinatorImpl::PerformNextQueuedGlobalMemoryDump() {
  ...
  QueuedRequestDispatcher::SetUpAndDispatch(request, clients_, chrome_callback,
                                            os_callback);
