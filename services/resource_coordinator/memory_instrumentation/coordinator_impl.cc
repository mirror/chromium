...
void CoordinatorImpl::GetVmRegionsForHeapProfiler(
  ...
  base::trace_event::MemoryDumpRequestArgs args{
      0, base::trace_event::MemoryDumpType::VM_REGIONS_ONLY,
...
void CoordinatorImpl::PerformNextQueuedGlobalMemoryDump() {
  ...
  for (const auto& kv : clients_) {
    ...
    if (!dump_only_vm_regions) {
      ...
      // TODO(hjd): remove this when removing the RawProcessMemoryDump.
      auto raw_dump = mojom::RawProcessMemoryDump::New();
      raw_dump->chrome_dump = mojom::ChromeMemDump::New();
      request->responses[client].dump_ptr = std::move(raw_dump);
    }
