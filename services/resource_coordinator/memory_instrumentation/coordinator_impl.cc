...
void CoordinatorImpl::UnregisterClientProcess(
  ...
  if (request == nullptr) {
    VLOG(1) << "No current dump request.";
    NOTREACHED();
    return;
  }

  // Check if we are waiting for an ack from this client process.
  std::set<QueuedMemoryDumpRequest::PendingResponse>::iterator it =
      request->pending_responses.begin();
  while (it != request->pending_responses.end()) {
    ...
    // current element.
    std::set<QueuedMemoryDumpRequest::PendingResponse>::iterator current = it++;
...
void CoordinatorImpl::PerformNextQueuedGlobalMemoryDump() {
  using DumpType = QueuedMemoryDumpRequest::PendingResponse::Type;
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  QueuedMemoryDumpRequest* request = GetCurrentRequest();
  if (request == nullptr) {
    VLOG(1) << "No current dump request.";
  ...
  for (const auto& kv : clients_) {
    ...
    request->pending_responses.insert({client, DumpType::kProcessDump});
...
// so ask each process to do so. For the awkward case where only the browser
// process has that privilege see below.
#if !defined(OS_LINUX)
    request->pending_responses.insert({client, DumpType::kOSDump});
    auto callback = base::Bind(&CoordinatorImpl::OnOSMemoryDumpResponse,
                               base::Unretained(this), client);
    client->RequestOSMemoryDump({pid}, callback);
...
// get around to sandboxing/selinux restrictions (see crbug.com/461788).
  ...
  if (browser_client) {
    request->pending_responses.insert({browser_client, DumpType::kOSDump});
    auto callback = base::Bind(&CoordinatorImpl::OnOSMemoryDumpResponse,
...
void CoordinatorImpl::OnProcessMemoryDumpResponse(
  ...
  auto it = request->pending_responses.find(
      {client_process,
       QueuedMemoryDumpRequest::PendingResponse::Type::kProcessDump});
  ...
  if (process_memory_dump) {
    request->responses[client_process].dump_ptr =
...
void CoordinatorImpl::OnOSMemoryDumpResponse(
  ...
  auto it = request->pending_responses.find(
      {client_process,
       QueuedMemoryDumpRequest::PendingResponse::Type::kOSDump});
  ...
  request->responses[client_process].os_dumps = os_dumps;
