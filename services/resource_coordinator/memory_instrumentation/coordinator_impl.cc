...
void CoordinatorImpl::RequestGlobalMemoryDumpInternal(
  ...
  request_args.dump_guid = ++next_dump_id_;
  ...
  queued_memory_dump_requests_.emplace_back(args, request_args, callback);
...
void CoordinatorImpl::OnChromeMemoryDumpResponse(
  ...
  if (chrome_memory_dump && request->args.add_to_trace) {
    bool added_to_trace = tracing_observer_->AddChromeDumpToTraceIfEnabled(
        request->request_args, response->process_id, chrome_memory_dump.get());
