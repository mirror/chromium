...
using base::trace_event::IsBackgroundDump;
using base::trace_event::IsDetailedDump;
using base::trace_event::IsLightDump;
...
using base::trace_event::MockMemoryDumpProvider;
...
class ClientProcessImplTest;

class MockCoordinator : public Coordinator, public mojom::Coordinator {
 public:
  MockCoordinator(ClientProcessImplTest* client) : client_(client) {}
...
class ClientProcessImplTest : public testing::Test {
 public:
  ...
  void InitializeClientProcess(mojom::ProcessType process_type) {
    ...
    client_process_->SetCoordinatorForTesting(coordinator_.get());
  }
...
// been initialized.
TEST_F(ClientProcessImplTest, DumpWithoutInitializingTracing) {
...
// enabled.
TEST_F(ClientProcessImplTest, DumpWithMDMInitializedForTracingButDisabled) {
  InitializeClientProcess(mojom::ProcessType::RENDERER);
  MockMemoryDumpProvider mdp;
  RegisterDumpProvider(&mdp, base::ThreadTaskRunnerHandle::Get(),
                       MemoryDumpProvider::Options());
...
TEST_F(ClientProcessImplTest, SummaryOnlyDumpsArentAddedToTrace) {
  InitializeClientProcess(mojom::ProcessType::RENDERER);
...
TEST_F(ClientProcessImplTest, InitializedAfterStartOfTracing) {
  EnableMemoryInfraTracing();
...
TEST_F(ClientProcessImplTest, TestBackgroundTracingSetup) {
...
TEST_F(ClientProcessImplTest, TraceConfigExpectations) {
  InitializeClientProcess(mojom::ProcessType::RENDERER);
...
TEST_F(ClientProcessImplTest, TraceConfigExpectationsWhenIsCoordinator) {
  InitializeClientProcess(mojom::ProcessType::BROWSER);
...
TEST_F(ClientProcessImplTest, PeriodicDumpingWithMultipleModes) {
  InitializeClientProcess(mojom::ProcessType::BROWSER);
...
TEST_F(ClientProcessImplTest, TestWhitelistingMDP) {
  InitializeClientProcess(mojom::ProcessType::RENDERER);
  base::trace_event::SetDumpProviderWhitelistForTesting(kTestMDPWhitelist);
...
TEST_F(ClientProcessImplTest, TestSummaryComputation) {
  InitializeClientProcess(mojom::ProcessType::RENDERER);
  MockMemoryDumpProvider mdp;
  RegisterDumpProvider(&mdp, base::ThreadTaskRunnerHandle::Get(),
                       MemoryDumpProvider::Options());
...
TEST_F(ClientProcessImplTest, DumpOnBehalfOfOtherProcess) {
  InitializeClientProcess(mojom::ProcessType::RENDERER);
  using trace_analyzer::Query;
...
TEST_F(ClientProcessImplTest, TestPollingOnDumpThread) {
  InitializeClientProcess(mojom::ProcessType::RENDERER);
  std::unique_ptr<MockMemoryDumpProvider> mdp1(new MockMemoryDumpProvider());
  std::unique_ptr<MockMemoryDumpProvider> mdp2(new MockMemoryDumpProvider());
