...
TEST(OSMetricsTest, ParseProcSmaps) {
  ...
  SetProcSmapsForTesting(empty_file.get());
  DumpProcessMemoryMaps(base::kNullProcessId, &pmd_invalid);
  ...
  SetProcSmapsForTesting(temp_file1.get());
  DumpProcessMemoryMaps(base::kNullProcessId, &pmd_1);
  ...
  SetProcSmapsForTesting(temp_file2.get());
  DumpProcessMemoryMaps(base::kNullProcessId, &pmd_2);
...
TEST(OSMetricsTest, TestWinModuleReading) {
  ...
  ASSERT_TRUE(DumpProcessMemoryMaps(base::kNullProcessId, &dump));
