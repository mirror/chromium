...
TEST_F(StructTraitsTest, MemoryAllocatorDumpEdge) {
  ...
  MemoryAllocatorDumpEdge input{source, target, importance, nullptr,
                                overridable};
...
TEST_F(StructTraitsTest, ProcessMemoryDump) {
  ...
  mad1->AddScalarF("two", "double", 2.0);
  mad1->AddString("three", "string", "3");
  ...
  EXPECT_THAT(
      actual_mad1->integers(),
      ElementsAre(Eq(MemoryAllocatorDump::IntegerEntry{"one", "integer", 1})));
  EXPECT_THAT(
      actual_mad1->doubles(),
      ElementsAre(Eq(MemoryAllocatorDump::DoubleEntry{"two", "double", 2.0})));
  EXPECT_THAT(
      actual_mad1->strings(),
      ElementsAre(Eq(MemoryAllocatorDump::StringEntry{"three", "string", "3"})));
