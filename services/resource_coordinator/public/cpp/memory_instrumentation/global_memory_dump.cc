...
GlobalMemoryDump::GlobalMemoryDump(
  ...
  for (mojom::ProcessMemoryDumpPtr& process_dump : process_dumps) {
    process_dumps_.emplace_front(std::move(process_dump));
  }
}
GlobalMemoryDump::~GlobalMemoryDump() = default;

std::unique_ptr<GlobalMemoryDump> GlobalMemoryDump::Wrap(
...
GlobalMemoryDump::ProcessDump::ProcessDump(
    ...
    : mojo_dump_(std::move(process_dump)) {}
GlobalMemoryDump::ProcessDump::~ProcessDump() = default;

base::Optional<uint64_t> GlobalMemoryDump::ProcessDump::GetMetricForDump(
    const std::string& dump_name,
    const std::string& metric_name) {
  auto dump_it =
      mojo_dump_->chrome_dump->entries_for_allocator_dumps.find(dump_name);
  if (dump_it == mojo_dump_->chrome_dump->entries_for_allocator_dumps.end())
    return base::nullopt;
