...
tracing::Coordinator* g_coordinator;
...
void Coordinator::StartTracing(const std::string& config,
  ...
  is_tracing_ = true;
  config_ = config;
  pending_start_tracing_count_ = 0;
  ...
  if (pending_start_tracing_count_ == 0) {
    callback.Run(true);
    return;
  }
...
void Coordinator::SendStartTracingToAgent(
  ...
  pending_start_tracing_count_++;
  auto closure_id = agent_entry->NextDisconnectClosureId();
  agent_entry->AddDisconnectClosure(
      closure_id,
      base::BindOnce(&Coordinator::OnTracingStarted, base::Unretained(this),
                     base::Unretained(agent_entry), closure_id));
...
void Coordinator::OnTracingStarted(AgentRegistry::AgentEntry* agent_entry,
                                   uint32_t closure_id) {
  if (!agent_entry->RemoveDisconnectClosure(closure_id))
    return;

  DCHECK(pending_start_tracing_count_);
  if (--pending_start_tracing_count_ == 0 &&
      !start_tracing_callback_.is_null()) {
    // We remove the callback before running it. This way, if the callback
    // directly calls into the coordinator, we do not loop forever.
    StartTracingCallback callback = std::move(start_tracing_callback_);
    callback.Run(true);
  }
}
...
void Coordinator::OnRecorderDataChange(const std::string& label) {
  ...
  while (true) {
    if (!streaming_label_.empty() && StreamEventsForCurrentLabel())
      return;

    // We are not waiting for data from any particular label now. So, we look at
    // the recorders that has some data available and select the next label to
    // stream.
    streaming_label_.clear();
...
void Coordinator::GetCategories(const GetCategoriesCallback& callback) {
  ...
  pending_get_categories_count_ = 0;
  agent_registry_->ForAllAgents([this](AgentRegistry::AgentEntry* agent_entry) {
    auto closure_id = agent_entry->NextDisconnectClosureId();
    agent_entry->AddDisconnectClosure(
        closure_id,
...
void Coordinator::OnGetCategoriesResponse(
  ...
  DCHECK(pending_get_categories_count_);
  ...
  if (--pending_get_categories_count_ == 0) {
