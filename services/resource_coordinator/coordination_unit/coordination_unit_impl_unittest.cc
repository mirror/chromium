...
TEST_F(CoordinationUnitImplTest, GetSetProperty) {
  ...
  coordination_unit->SetProperty(mojom::Property::New(
      mojom::PropertyType::kTest, base::MakeUnique<base::Value>(41)));
  EXPECT_EQ(1u, coordination_unit->properties_for_testing().size());
  EXPECT_EQ(base::Value(41), coordination_unit->GetProperty(mojom::PropertyType::kTest));
...
TEST_F(CoordinationUnitImplTest,
  ...
  auto tabs_associated_with_process = cu_graph.process->GetAssociatedCoordinationUnitsOfType(
  ...
  auto processes_associated_with_tab = cu_graph.tab->GetAssociatedCoordinationUnitsOfType(
...
TEST_F(CoordinationUnitImplTest,
  ...
  EXPECT_EQ(1u, processes_associated_with_tab.count(cu_graph.process.get()));
  EXPECT_EQ(1u, processes_associated_with_tab.count(cu_graph.other_process.get()));
}
...
TEST_F(CoordinationUnitImplTest,
  ...
  auto tabs_associated_with_other_process =
      cu_graph.other_process->GetAssociatedCoordinationUnitsOfType(
          CoordinationUnitType::kWebContents);
  EXPECT_EQ(1u, tabs_associated_with_other_process.size());
  EXPECT_EQ(1u, tabs_associated_with_other_process.count(cu_graph.other_tab.get()));
  ...
  EXPECT_EQ(1u, processes_associated_with_other_tab.count(cu_graph.process.get()));
  EXPECT_EQ(1u, processes_associated_with_other_tab.count(cu_graph.other_process.get()));
}
