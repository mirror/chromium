...
void CoordinationUnitIntrospectorImpl::GetProcessToURLMap(
  ...
  for (CoordinationUnitImpl* process_cu : process_cus) {
    ...
    for (CoordinationUnitImpl* page_cu : page_cus) {
      ...
      if (page_data_map_[page_cu->id()].is_visible) {
        process_info->time_since_last_visible.push_back(-1);
      } else {
        // DCHECK(page_data_map_[page_cu->id()] != nullptr);
        // DCHECK(page_data_map_[page_cu->id()].last_invisible_time > 0);
...
void CoordinationUnitIntrospectorImpl::OnPagePropertyChanged(
  ...
  if (property_type == mojom::PropertyType::kVisible) {
    if (value) {
      ...
      page_data_map_[page_cu_id].is_visible = true;
      return;
    }
    page_data_map_[page_cu_id].is_visible = false;
