...
template <class UKMBuilderClass, bool should_report_child_frame_ukm>
class UKMReportDelegate {};

template <class UKMBuilderClass>
class UKMReportDelegate<UKMBuilderClass, true> {
 public:
...
template <class UKMBuilderClass>
class UKMReportDelegate<UKMBuilderClass, false> {
 public:
  void ReportUKM(int64_t ukm_source_id,
                 bool is_main_frame,
                 int64_t duration_in_ms,
...
template <class UKMBuilderClass,
          ...
          bool should_report_child_frame_ukm>
class BackgroundMetricsReporter {
 public:
  ...
  void OnSignalReceived(bool is_main_frame,
    ...
    if (!uma_reported_) {
      uma_reported_ = true;
      HEURISTICS_HISTOGRAM(metric_name, duration);
    }
 ...
 private:
  void ReportUKMIfNeeded(bool is_main_frame,
                         ...
                         ukm::MojoUkmRecorder* ukm_recorder) {
    if (ukm_source_id_ == -1 ||
        (!should_report_child_frame_ukm && ukm_reported_) ||
        (should_report_child_frame_ukm &&
  ...
  UKMReportDelegate<UKMBuilderClass, should_report_child_frame_ukm>
      ukm_reporter_;
};
