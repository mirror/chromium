...
// static
PageCoordinationUnitImpl* PageCoordinationUnitImpl::Create(
    const CoordinationUnitID& id,
    std::unique_ptr<service_manager::ServiceContextRef> service_ref) {
  auto* cu =
      CoordinationUnitBase::CreateCoordinationUnit(id, std::move(service_ref));
  return static_cast<PageCoordinationUnitImpl*>(cu);
}

// static
const PageCoordinationUnitImpl*
PageCoordinationUnitImpl::FromCoordinationUnitBase(
...
// static
PageCoordinationUnitImpl* PageCoordinationUnitImpl::FromCoordinationUnitBase(
...
void PageCoordinationUnitImpl::Bind(
    mojom::PageCoordinationUnitRequest request) {
  binding_.Bind(std::move(request));
}

void PageCoordinationUnitImpl::GetID(
    const mojom::PageCoordinationUnit::GetIDCallback& callback) {
  callback.Run(id_);
}

void PageCoordinationUnitImpl::AddBinding(
    mojom::PageCoordinationUnitRequest request) {
...
// static
PageCoordinationUnitImpl* PageCoordinationUnitImpl::GetCoordinationUnitByID(
    const CoordinationUnitID cu_id) {
  DCHECK(cu_id.type == CoordinationUnitType::kPage);
  auto* cu = CoordinationUnitBase::GetCoordinationUnitByID(cu_id);
  DCHECK(cu->id().type == CoordinationUnitType::kPage);
  return static_cast<PageCoordinationUnitImpl*>(cu);
}

bool PageCoordinationUnitImpl::HasAncestor(CoordinationUnitBase* ancestor) {
  return false;
}

bool PageCoordinationUnitImpl::HasDescendant(CoordinationUnitBase* descendant) {
