...
void TabSignalGeneratorImpl::TabCPUUsageCollector::CollectCPUUsage(double cpu_usage) {
...
bool TabSignalGeneratorImpl::TabCPUUsageCollector::IsCollectingCPUUsage() const {
  return ukm_entry_builder_ && num_cpu_usage_measurements_ < kMaxCPUUsageMeasurements;
}

void TabSignalGeneratorImpl::TabCPUUsageCollector::ResetUkmEntryBuilder(std::unique_ptr<ukm::UkmEntryBuilder> ukm_entry_builder) {
...
void TabSignalGeneratorImpl::OnTabPropertyChanged(
    ...
    const mojom::PropertyType property_type,
    const base::Value& value) {
  TabCPUUsageCollector& tab_cpu_usage_collector = tab_cpu_usage_collector_map_[coordination_unit->id()];
  if (property_type == mojom::PropertyType::kCPUUsage) {
    ...
    tab_cpu_usage_collector.ResetUkmEntryBuilder(coordination_unit_manager()->CreateUkmEntryBuilder(
