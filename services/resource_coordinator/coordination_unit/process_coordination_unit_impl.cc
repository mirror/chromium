...
// static
ProcessCoordinationUnitImpl* ProcessCoordinationUnitImpl::Create(
...
const ProcessCoordinationUnitImpl*
ProcessCoordinationUnitImpl::FromCoordinationUnitBase(
    const CoordinationUnitBase* cu) {
  DCHECK(cu->id().type == CoordinationUnitType::kProcess);
  return static_cast<const ProcessCoordinationUnitImpl*>(cu);
}
...
void ProcessCoordinationUnitImpl::Bind(
    mojom::ProcessCoordinationUnitRequest request) {
  binding_.Bind(std::move(request));
}

void ProcessCoordinationUnitImpl::GetID(
    const mojom::ProcessCoordinationUnit::GetIDCallback& callback) {
  callback.Run(id_);
}
void ProcessCoordinationUnitImpl::AddBinding(
    mojom::ProcessCoordinationUnitRequest request) {
...
ProcessCoordinationUnitImpl*
ProcessCoordinationUnitImpl::GetCoordinationUnitByID(
    const CoordinationUnitID cu_id) {
  DCHECK(cu_id.type == CoordinationUnitType::kProcess);
  auto* cu = CoordinationUnitBase::GetCoordinationUnitByID(cu_id);
  DCHECK(cu->id().type == CoordinationUnitType::kProcess);
  return static_cast<ProcessCoordinationUnitImpl*>(cu);
}

bool ProcessCoordinationUnitImpl::HasAncestor(CoordinationUnitBase* ancestor) {
  return false;
}

bool ProcessCoordinationUnitImpl::HasDescendant(
    CoordinationUnitBase* descendant) {
  DCHECK(descendant->id().type == CoordinationUnitType::kFrame);
  for (auto* child : frame_coordination_units_) {
    if (child == descendant)
      return true;
  }
  return false;
