...
void MetricsCollector::OnCoordinationUnitCreated(
  ...
  if (coordination_unit->id().type == CoordinationUnitType::kWebContents) {
    metrics_report_record_map_.insert(std::make_pair(
        coordination_unit->id(),
        MetricsReportRecord(
            WebContentsCoordinationUnitImpl::ToWebContentsCoordinationUnit(
                coordination_unit))));
  }
}
...
MetricsCollector::MetricsReportRecord::MetricsReportRecord(
    const WebContentsCoordinationUnitImpl* web_contents_cu)
    : first_alert_fired(kTabFromBackgroundedToFirstAlertFiredUMA,
                        web_contents_cu),
      first_audible(kTabFromBackgroundedToFirstAudioStartsUMA, web_contents_cu),
      first_favicon_updated(kTabFromBackgroundedToFirstFaviconUpdatedUMA,
                            web_contents_cu),
      first_non_persistent_notification_created(
          kTabFromBackgroundedToFirstNonPersistentNotificationCreatedUMA,
          web_contents_cu),
      first_title_updated(kTabFromBackgroundedToFirstTitleUpdatedUMA,
                          web_contents_cu) {}
