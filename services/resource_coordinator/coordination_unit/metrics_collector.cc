...
size_t GetNumCoresidentTabs(const CoordinationUnitImpl* cu) {
  DCHECK_EQ(CoordinationUnitType::kPage, cu->id().type);
  std::set<CoordinationUnitImpl*> coresident_tabs;
  for (auto* process_cu : cu->GetAssociatedCoordinationUnitsOfType(
           CoordinationUnitType::kProcess)) {
    for (auto* page_cu : process_cu->GetAssociatedCoordinationUnitsOfType(
             CoordinationUnitType::kPage)) {
      coresident_tabs.insert(page_cu);
    }
  }
