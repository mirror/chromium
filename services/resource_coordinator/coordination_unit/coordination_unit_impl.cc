...
void CoordinationUnitImpl::AddChild(const CoordinationUnitID& child_id) {
  ...
  if (child_iter != g_cu_map().end()) {
    ...
    if (HasAncestor(child) || child->HasDescendant(this)) {
      DCHECK(false)
          << "Cyclic reference in coordination unit graph is detected!";
      return;
    }
...
bool CoordinationUnitImpl::HasAncestor(CoordinationUnitImpl* unit) {
  for (CoordinationUnitImpl* parent : parents_) {
    if (parent == unit || parent->HasAncestor(unit)) {
      return true;
    }
  }
...
bool CoordinationUnitImpl::HasDescendant(CoordinationUnitImpl* unit) {
  for (CoordinationUnitImpl* child : children_) {
    if (child == unit || child->HasDescendant(unit)) {
      return true;
    }
  }
