...
const char* kCPUUsageEntryName = "CPUUsage";
const char* kCPUUsageMeasurementMetricsName = "CPUUsageMeasurement";
const char* kNumCoresidentTabsEntryName = "NumberOfCoresidentTabs";
const char* kTickEntryName = "Tick";
...
void TabMetricsCollector::RecordCPUUsageForUkm(
  ...
  auto ukm_entry_builder = coordination_unit_manager().CreateUkmEntryBuilder(
      state.ukm_source_id, kCPUUsageMeasurementMetricsName);
  ukm_entry_builder->AddMetric(kTickEntryName,
                               state.num_cpu_usage_measurements++);
  ukm_entry_builder->AddMetric(kCPUUsageEntryName, cpu_usage);
  ukm_entry_builder->AddMetric(kNumCoresidentTabsEntryName,
                               num_coresident_tabs);
}
