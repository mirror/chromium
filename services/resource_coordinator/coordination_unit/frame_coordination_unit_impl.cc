...
// static
FrameCoordinationUnitImpl* FrameCoordinationUnitImpl::Create(
    const CoordinationUnitID& id,
    std::unique_ptr<service_manager::ServiceContextRef> service_ref) {
  auto* cu =
      CoordinationUnitBase::CreateCoordinationUnit(id, std::move(service_ref));
  return static_cast<FrameCoordinationUnitImpl*>(cu);
}

// static
const FrameCoordinationUnitImpl*
FrameCoordinationUnitImpl::FromCoordinationUnitBase(
    const CoordinationUnitBase* cu) {
  DCHECK(cu->id().type == CoordinationUnitType::kFrame);
  return static_cast<const FrameCoordinationUnitImpl*>(cu);
}

// static
FrameCoordinationUnitImpl* FrameCoordinationUnitImpl::FromCoordinationUnitBase(
...
void FrameCoordinationUnitImpl::Bind(
    mojom::FrameCoordinationUnitRequest request) {
  binding_.Bind(std::move(request));
}

void FrameCoordinationUnitImpl::GetID(
    const mojom::FrameCoordinationUnit::GetIDCallback& callback) {
  callback.Run(id_);
}

void FrameCoordinationUnitImpl::AddBinding(
    mojom::FrameCoordinationUnitRequest request) {
...
// static
FrameCoordinationUnitImpl* FrameCoordinationUnitImpl::GetCoordinationUnitByID(
    const CoordinationUnitID cu_id) {
