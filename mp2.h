...
template<typename Arg> struct CheckIfWrongPtr {
    enum {
        ...
        value = std::is_pointer<Arg>::value && (
                std::is_convertible<Arg, WeakPtrableBase*>::value ||
                std::is_convertible<Arg, WeakPtrableBase&>::value ||
                std::is_convertible<Arg, WeakPtrableBase&&>::value
                )
    };
...
template<class T> class WeakRemoteObject {
    public:
    template<typename...Args> WeakRemoteObject(Args... args) : t_(args...),
        task_runner_(base::SequencedTaskRunnerHandle::Get()),
       weak_factory_(&t_) {
    }

    // it would be nice to be able to refer to this as weak_ptr<Foo> rather than
    // WeakRemoteObject<Foo>::weak_ptr.  i guess we could make it not an inner
    // class, and friend it.
    class weak_ptr {
        public:
            // TODO(liberato): is there any case where we'd need to do this on
            // some other thread?  i doubt it.  all of these won't work.
            weak_ptr(WeakRemoteObject& thiz) : task_runner_(thiz.task_runner_), wp_(thiz.weak_factory_.GetWeakPtr()) {}
            //weak_ptr(T* thiz) : task_runner_(thiz->task_runner_), wp_(thiz->weak_factory_.GetWeakPtr()) {}
            //weak_ptr(std::unique_ptr<T>& thiz) : task_runner_(thiz.get()->task_runner_), wp_(thiz.get()->weak_factory_.GetWeakPtr()) {}
...
void fn() {
    threadsafe::WeakRemoteObject<Goo> goo;
    threadsafe::WeakRemoteObject<Goo>::weak_ptr wp_goo(goo);// = goo.create_weak_ptr();
    wp_goo.Post(&Goo::method1, 123);
    ...
    // when sending as a ref, though.
    //wp(&Foo::method4, *foo.get());  // ref to Foo is not okay.  use wp instead.
