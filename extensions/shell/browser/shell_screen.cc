...
ShellScreen::ShellScreen(ShellDesktopControllerAura* desktop_controller,
                         const gfx::Size& size)
    : ShellScreen(size) {
  // |desktop_controller| is required in the public constructor.
  DCHECK(desktop_controller);
...
void ShellScreen::OnHostResized(const aura::WindowTreeHost* host) {
  // Based on ash::WindowTreeHostManager.
  display::Display display =
      GetDisplayNearestWindow(const_cast<aura::Window*>(host->window()));
