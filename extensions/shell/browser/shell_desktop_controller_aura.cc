...
class ShellNativeCursorManager : public wm::NativeCursorManager {
 ...
 private:
  ...
  void SetCursorOnAllRootWindows(gfx::NativeCursor cursor) {
    if (desktop_controller_->root_window_controller())
      desktop_controller_->root_window_controller()->host()->SetCursor(cursor);
  }
...
void ShellDesktopControllerAura::CloseRootWindowController(
  ...
  // TearDownWindowManager().
  // TODO(michaelpg): Migrate to RunLoop.
...
ui::EventDispatchDetails ShellDesktopControllerAura::DispatchKeyEventPostIME(
  ...
  return root_window_controller_->host()->DispatchKeyEventPostIME(key_event);
}
...
void ShellDesktopControllerAura::InitWindowManager() {
  ...
  screen_.reset(new ShellScreen(this, GetStartingWindowSize()));
  ...
  cursor_manager_.reset(
      new wm::CursorManager(std::unique_ptr<wm::NativeCursorManager>(
          new ShellNativeCursorManager(this))));
...
void ShellDesktopControllerAura::CreateRootWindowController() {
  ...
  root_window_controller_->CreateWindowTreeHost(
      screen_.get(), gfx::Rect(screen_->GetPrimaryDisplay().GetSizeInPixel()));

  // Initialize the root window with our clients.
  aura::Window* window = root_window_controller_->host()->window();
  window->AddPreTargetHandler(root_window_event_filter_.get());
  aura::client::SetFocusClient(window, focus_controller_.get());
  window->AddPreTargetHandler(focus_controller_.get());
  wm::SetActivationClient(window, focus_controller_.get());
...
void ShellDesktopControllerAura::FinalizeWindowManager() {
  ...
  ui::InputMethod* input_method =
      root_window_controller_->host()->GetInputMethod();
  ...
  // WindowTreeHosts.
  capture_client_.reset(new aura::client::DefaultCaptureClient(
      root_window_controller_->host()->window()));
}

void ShellDesktopControllerAura::TearDownWindowManager() {
  root_window_controller_->host()->window()->RemovePreTargetHandler(
  ...
  wm::SetActivationClient(root_window_controller_->host()->window(), nullptr);
