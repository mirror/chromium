 ...
 private:
  ...
  void SetCursorOnAllRootWindows(gfx::NativeCursor cursor) {
    aura::Window::Windows root_windows =
        desktop_controller_->GetAllRootWindows();
...
void ShellDesktopControllerAura::AddAppWindow(AppWindow* app_window,
  ...
  const display::Display& display =
      screen_->GetDisplayMatching(window->GetBoundsInScreen());
...
void ShellDesktopControllerAura::CloseRootWindowController(
  ...
  RemoveRootWindowControllerForDisplay(it->first);
...
void ShellDesktopControllerAura::TearDownWindowManager() {
  for (auto it = root_window_controllers_.cbegin();
       it != root_window_controllers_.cend();) {
    int64_t display_id = it->first;
    ++it;
    RemoveRootWindowControllerForDisplay(display_id);
  }
...
ShellDesktopControllerAura::CreateRootWindowControllerForDisplay(
  ...
  std::unique_ptr<RootWindowController> root_window_controller =
      base::MakeUnique<RootWindowController>(this, display.bounds(),
                                             browser_context_);
...
void ShellDesktopControllerAura::RemoveRootWindowControllerForDisplay(
    const int64_t display_id) {
  RootWindowController* root = root_window_controllers_.at(display_id).get();
  DCHECK(root);
  ...
  root_window_controllers_.erase(display_id);
}
