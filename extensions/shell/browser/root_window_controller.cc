...
RootWindowController::RootWindowController(DesktopDelegate* desktop_delegate,
                                           ShellScreen* screen,
                                           const gfx::Rect& bounds)
    : desktop_delegate_(desktop_delegate), screen_(screen) {
...
void RootWindowController::CloseAppWindows() {
  for (AppWindow* app_window : app_windows_) {
    // Remove the observer before closing the window to avoid triggering
    // OnAppWindowRemoved, which would mutate |app_windows_|.
    AppWindowRegistry::Get(app_window->browser_context())->RemoveObserver(this);
