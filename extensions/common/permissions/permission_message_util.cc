...
std::set<std::string> GetDistinctHosts(const URLPatternSet& host_patterns,
  ...
  for (const URLPattern& pattern : host_patterns) {
    ...
    std::string temp_scheme = "http://";
    std::string url_secure_format = base::UTF16ToUTF8(
        url_formatter::FormatUrlForSecurityDisplay(GURL(temp_scheme + host)));
    host = url_secure_format.substr(temp_scheme.length(),
                                    url_secure_format.length());
