...
enum DisableReason {
  ...
  DISABLE_NOT_VERIFIED = 1 << 8,  // Disabled because we could not
                                  // verify the install.
  DISABLE_GREYLIST = 1 << 9,
  DISABLE_CORRUPTED = 1 << 10,
  DISABLE_REMOTE_INSTALL = 1 << 11,
  // DISABLE_INACTIVE_EPHEMERAL_APP = 1 << 12,  // Deprecated.
  DISABLE_EXTERNAL_EXTENSION = 1 << 13,           // External extensions might
                                                  // be disabled for user
                                                  // prompting.
  DISABLE_UPDATE_REQUIRED_BY_POLICY = 1 << 14,    // Doesn't meet
                                                  // minimum version
                                                  // requirement.
  DISABLE_CUSTODIAN_APPROVAL_REQUIRED = 1 << 15,  // Supervised user
                                                  // needs approval by
                                                  // custodian.
