...
Feature::Availability ComplexFeature::IsAvailableToManifest(
  ...
  Feature::Availability first_availability =
      features_[0]->IsAvailableToManifest(hashed_extension_id, type, location,
  ...
  for (FeatureList::const_iterator it = features_.begin() + 1;
    ...
    Availability availability = (*it)->IsAvailableToManifest(
        hashed_extension_id, type, location, manifest_version, platform);
...
bool ComplexFeature::IsIdInBlacklist(
    const std::string& hashed_extension_id) const {
  for (FeatureList::const_iterator it = features_.begin();
       it != features_.end();
       ++it) {
    if ((*it)->IsIdInBlacklist(hashed_extension_id))
      return true;
  }
  return false;
}

bool ComplexFeature::IsIdInWhitelist(
    const std::string& hashed_extension_id) const {
  for (FeatureList::const_iterator it = features_.begin();
       it != features_.end();
       ++it) {
    if ((*it)->IsIdInWhitelist(hashed_extension_id))
      return true;
  }
  return false;
}
