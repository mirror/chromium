...
Feature::Availability IsAvailableToManifestForBind(
  ...
  return feature->IsAvailableToManifest(hashed_extension_id, type, location,
...
bool IsWhitelistedForTest(const std::string& hashed_extension_id) {
  ...
  return !whitelisted_id.empty() && whitelisted_id == hashed_extension_id;
}
...
Feature::Availability SimpleFeature::IsAvailableToManifest(
  ...
  Availability manifest_availability = GetManifestAvailability(
      hashed_extension_id, type, location, manifest_version);
  ...
  return CheckDependencies(base::Bind(&IsAvailableToManifestForBind,
                                      hashed_extension_id, type, location,
                                      manifest_version, platform));
}
...
bool SimpleFeature::IsIdInBlacklist(
    const std::string& hashed_extension_id) const {
  return IsIdInList(hashed_extension_id, blacklist_);
}

bool SimpleFeature::IsIdInWhitelist(
    const std::string& hashed_extension_id) const {
  return IsIdInList(hashed_extension_id, whitelist_);
}
...
// static
bool SimpleFeature::IsIdInList(const std::string& hashed_extension_id,
                               const std::vector<std::string>& list) {
  if (!IsValidHashedExtensionId(hashed_extension_id))
    return false;

  return base::ContainsValue(list, hashed_extension_id);
}
...
bool SimpleFeature::IsValidHashedExtensionId(
  ...
  return hashed_extension_id.length() == 40;
}
...
Feature::Availability SimpleFeature::GetManifestAvailability(
  ...
  if (IsIdInBlacklist(hashed_extension_id))
  ...
  if (!whitelist_.empty() && !IsIdInWhitelist(hashed_extension_id) &&
      !IsWhitelistedForTest(hashed_extension_id)) {
