...
void RunValidateExtensionTest(std::vector<std::string> extra_directories,
  ...
  LOG(INFO) << " BOB: " << ext_path.MaybeAsASCII();
...
TEST_F(FileUtilTest, ValidateExtensionWithMetadataFolder) {
  RunValidateExtensionTest(
      {"_metadata"},
      base::Optional<std::string>(
          "_metadata is a reserved directory that will "
...
TEST_F(FileUtilTest, LoadExtensionWithMetadataFolder) {
  RunDirectoryTest({"_metadata"}, base::Optional<std::string>());
}

TEST_F(FileUtilTest, LoadExtensionWithUnderscoreFolder) {
  RunDirectoryTest({"_badfolder"}, base::Optional<std::string>());
}

TEST_F(FileUtilTest, LoadExtensionWithMetadataAndUnderscoreFolders) {
  RunDirectoryTest({"_metadata", "_badfolder"}, base::Optional<std::string>());
}
