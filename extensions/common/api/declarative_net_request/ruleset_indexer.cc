...
namespace {
using UrlRuleOffset = ::flatbuffers::Offset<::url_pattern_index::flat::UrlRule>;
...
// Returns offset to a flatbuffer vector of shared strings corresponding to
// |vec|. If |vec| is empty, returns an empty offset.
FlatStringListOffset CreateVectorOfSharedStrings(
  ...
  // COMMENT: subresource_filter converts to lowercase, necessary?
...
api::declarative_net_request::RuleActionType GetRuleActionType(
  ...
  if (indexed_rule.options & url_pattern_index::flat::OptionFlag_IS_WHITELIST)
...
void RulesetIndexer::AddUrlRule(const IndexedRule& indexed_rule) {
  DCHECK(
      std::is_sorted(indexed_rule.domains.begin(), indexed_rule.domains.end()));
  DCHECK(std::is_sorted(indexed_rule.excluded_domains.begin(),
                        indexed_rule.excluded_domains.end()));
  DCHECK_NE(url_pattern_index::flat::UrlPatternType_REGEXP,
            indexed_rule.url_pattern_type);

  cnt_++;

  auto domains_included_offset =
      CreateVectorOfSharedStrings(&builder_, indexed_rule.domains);
  auto domains_excluded_offset =
      CreateVectorOfSharedStrings(&builder_, indexed_rule.excluded_domains);

  // COMMENT: subresource_filter uses CreateString, why?
  auto url_pattern_offset =
      builder_.CreateSharedString(indexed_rule.url_pattern);

  UrlRuleOffset offset = url_pattern_index::flat::CreateUrlRule(
...
RulesetIndexer::SerializedData RulesetIndexer::FinishAndGetData() {
  ...
  UMA_HISTOGRAM_COUNTS_100000(
      "Extensions.DeclarativeNetRequest.ManifestRulesCount", cnt_);
