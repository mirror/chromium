...
uint16_t kAllElementTypesMask =
    flat_rule::ElementType_ANY & ~flat_rule::ElementType_OBJECT_SUBREQUEST;

// Returns true if bitmask |sub| is a subset of |super|.
bool IsSubset(unsigned int sub, unsigned int super) {
...
ParseResult ComputeElementTypes(const dnr_api::RuleCondition& condition,
  ...
  if (exclude_element_type_mask == kAllElementTypesMask)
...
ParseResult CreateIndexedRule(std::unique_ptr<dnr_api::Rule> parsed_rule,
  ...
  if (is_redirect_rule && !GURL(*parsed_rule->action.redirect_url).is_valid())
