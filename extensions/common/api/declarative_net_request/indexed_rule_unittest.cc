...
TEST_F(IndexedRuleTest, PriorityParsing) {
  ...
  for (size_t i = 0; i < arraysize(cases); i++) {
    ...
    rule->action.redirect_url = MakeUnique<std::string>("http://google.com");
...
TEST_F(IndexedRuleTest, UrlFilterParsing) {
  struct {
    unique_ptr<std::string> input_url_filter;
...
TEST_F(IndexedRuleTest, RedirectUrlParsing) {
  struct {
    ...
    const std::string expected_redirect_url;
      ...
      {MakeUnique<std::string>("http://google.com"), ParseResult::SUCCESS,
       "http://google.com"},
      {MakeUnique<std::string>("abc"), ParseResult::ERROR_INVALID_REDIRECT_URL,
       ""}};
