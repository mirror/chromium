...
std::string GenerateErrorDescription(
...
ParseInfo IndexAndPersistRuleset(const base::Value& rules,
  ...
  RulesetIndexer indexer;
  {
    ...
    for (auto iter = rules_list->begin(); iter != rules_list->end(); iter++) {
      ...
      // FIXME(karandeepb): Populate error string. Seems should be possible.
...
bool IndexAndPersistRuleset(const base::FilePath& json_rules_path,
  ...
  base::TimeTicks start_deserialization_time = base::TimeTicks::Now();
  ...
  if (info.result != ParseResult::SUCCESS) {
    if (error)
      *error =
          GenerateErrorDescription(info, json_rules_path.BaseName().value());
    return false;
  }
  return true;
}
