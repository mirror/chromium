...
constexpr char kRulesFileName[] = "rules_file.json";
...
std::unique_ptr<base::DictionaryValue> GetGenericManifest() {
  return DictionaryBuilder()
      ...
      .Set(keys::kDeclarativeNetRequestRulesetLocation, kRulesFileName)
      .Set("version", "1.0")
...
class RulesManifestTest : public testing::Test {
 ...
 protected:
  ...
  void Finish() {
    ...
    // Persist rules file
    JSONFileValueSerializer(temp_dir_.GetPath().Append(rules_file_path_))
        .Serialize(*rules_);
  ...
  void LoadAndExpectSuccess() {
    ...
    base::PathExists(file_util::GetIndexedRulesetPath(temp_dir_.GetPath()));
  }
 ...
 private:
  ...
  base::FilePath rules_file_path_ =
      base::FilePath(FILE_PATH_LITERAL(kRulesFileName));
...
TEST_F(RulesManifestTest, DuplicateIDs) {
  ...
  LoadAndExpectError(ErrorUtils::FormatErrorMessage(
      errors::kRulesFileDuplicateID, kRulesFileName, "1"));
}
...
TEST_F(RulesManifestTest, DuplicateResourceTypes) {
  ...
  AddRule(rule);
  LoadAndExpectError(ErrorUtils::FormatErrorMessage(
      errors::kRulesFileDuplicateResourceType, kRulesFileName, "0"));
}
...
TEST_F(RulesManifestTest, EmptyRedirectRuleUrl) {
  ...
  LoadAndExpectError(
      ErrorUtils::FormatErrorMessage(errors::kRulesFileRedirectRuleKeyEmpty,
                                     kRulesFileName, "0", kRedirectUrlKey));
}
...
TEST_F(RulesManifestTest, InvalidRuleID) {
  ...
  AddRule(rule);
  LoadAndExpectError(ErrorUtils::FormatErrorMessage(
      errors::kRulesFileInvalidKey, kRulesFileName, "0", kIDKey,
...
TEST_F(RulesManifestTest, InvalidRulePriority) {
  ...
  AddRule(rule);
  LoadAndExpectError(ErrorUtils::FormatErrorMessage(
      errors::kRulesFileInvalidKey, kRulesFileName, "0", kPriorityKey,
...
TEST_F(RulesManifestTest, MeaningfulRule1) {
  ...
  AddRule(rule);
  LoadAndExpectError(ErrorUtils::FormatErrorMessage(
      errors::kRulesFileRuleNotMeaningful, kRulesFileName, "0"));
}
...
TEST_F(RulesManifestTest, NoRuleID) {
  ...
  AddRule(rule);
  LoadAndExpectError(ErrorUtils::FormatErrorMessage(
      errors::kRulesFileJSONParseError, kRulesFileName, "0"));
}
