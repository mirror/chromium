...
std::unique_ptr<base::DictionaryValue> GetGenericManifest() {
  return DictionaryBuilder()
      ...
      .Set(keys::kDeclarativeNetRequestKey,
           DictionaryBuilder()
               .Set(keys::kDeclarativeRulesFileKey, "rules_file.json")
               .Build())
 ...
 protected:
  ...
  void LoadAndExpectSuccess() {
    ...
    const ExtensionResource* resource =
        RulesManifestData::GetRulesetResource(extension.get());
...
TEST_F(RulesManifestTest, InvalidManifestKey) {
...
TEST_F(RulesManifestTest, InvalidRulesFileKey) {
  std::unique_ptr<base::DictionaryValue> manifest = GetGenericManifest();
  manifest->SetInteger(GetRulesFileKey(), 3);
  SetManifest(std::move(manifest));
...
TEST_F(RulesManifestTest, NonExistentRulesFile) {
  std::unique_ptr<base::DictionaryValue> manifest = GetGenericManifest();
  manifest->SetString(GetRulesFileKey(), "invalid_rules_file.json");
  SetManifest(std::move(manifest));
  LoadAndExpectError(ErrorUtils::FormatErrorMessage(
...
TEST_F(RulesManifestTest, RulesFileInNestedDirectory) {
  ...
  std::unique_ptr<base::DictionaryValue> manifest = GetGenericManifest();
  manifest->SetString(GetRulesFileKey(), nested_path.AsUTF8Unsafe());
  SetManifest(std::move(manifest));
  LoadAndExpectSuccess();
}
