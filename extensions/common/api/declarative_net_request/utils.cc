...
ParseInfo IndexAndPersistRuleset(const base::Value& rules,
  ...
  RulesetIndexer indexer;
  {
    ...
    for (auto iter = rules_list->begin(); iter != rules_list->end(); iter++) {
      ...
      // COMMENT: use generate_error_messages in schema to obtain the actual
      // parse error? It is used only in manifest_types.json currently.
...
bool IsAPIAvailable() {
  // TODO(crbug.com/696822): Use Feature::IsAvailableToEnvironment() once it
  // behaves correctly.
