...
ContentVerifyJob::TestObserver* g_ContentVerifyJob_test_observer = NULL;
...
void ContentVerifyJob::Start() {
  ...
  if (g_ContentVerifyJob_test_observer)
    g_ContentVerifyJob_test_observer->JobStarted(hash_reader_->extension_id(),
...
void ContentVerifyJob::DoneReading() {
  ...
  if (hashes_ready_) {
    if (!FinishBlock()) {
      DispatchFailureCallback(HASH_MISMATCH);
    } else if (g_ContentVerifyJob_test_observer) {
      g_ContentVerifyJob_test_observer->JobFinished(
...
void ContentVerifyJob::SetObserverForTests(TestObserver* observer) {
  g_ContentVerifyJob_test_observer = observer;
}
