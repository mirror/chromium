...
class AccessRateLimiterTest : public ::testing::Test {
 public:
  AccessRateLimiterTest() : test_clock_(new base::SimpleTestTickClock) {
  ...
  ~AccessRateLimiterTest() override {}

 protected:
  std::unique_ptr<base::SimpleTestTickClock> test_clock_;

  std::unique_ptr<AccessRateLimiter> limiter_;
};

TEST_F(AccessRateLimiterTest, MaxAccessCountOfZero) {
  limiter_.reset(new AccessRateLimiter(0, TimeDelta::FromMilliseconds(100),
                                       test_clock_.get()));
...
TEST_F(AccessRateLimiterTest, NormalRepeatedAccess) {
  limiter_.reset(new AccessRateLimiter(5, TimeDelta::FromMilliseconds(100),
                                       test_clock_.get()));
...
TEST_F(AccessRateLimiterTest, RechargeTimeOfZero) {
  limiter_.reset(new AccessRateLimiter(5, TimeDelta::FromMilliseconds(0),
                                       test_clock_.get()));
