...
class SerialConnection : public ApiResource,
 ...
 private:
  ...
  void OnAsyncReadComplete(int bytes_read, device::mojom::ReceiveError error);

  // Receives write completion notification from the |io_handler_|.
  void OnAsyncWriteComplete(int bytes_sent, device::mojom::SendError error);
...
template <>
struct TypeConverter<device::mojom::HostControlSignalsPtr,
                     extensions::api::serial::HostControlSignals> {
  static device::mojom::HostControlSignalsPtr Convert(
      const extensions::api::serial::HostControlSignals& input);
};

template <>
struct TypeConverter<device::mojom::ConnectionOptionsPtr,
                     extensions::api::serial::ConnectionOptions> {
  static device::mojom::ConnectionOptionsPtr Convert(
