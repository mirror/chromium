...
class FakeSerialDeviceEnumerator : public device::SerialDeviceEnumerator {
 public:
  ...
  std::vector<device::mojom::DeviceInfoPtr> GetDevices() override {
    std::vector<device::mojom::DeviceInfoPtr> devices;
    device::mojom::DeviceInfoPtr device0(device::mojom::DeviceInfo::New());
    device0->path = "/dev/fakeserialmojo";
    device::mojom::DeviceInfoPtr device1(device::mojom::DeviceInfo::New());
    device1->path = "\\\\COM800\\";
...
class FakeEchoSerialIoHandler : public device::TestSerialIoHandler {
 public:
  ...
  MOCK_METHOD1(SetControlSignals,
               bool(const device::mojom::HostControlSignals&));
