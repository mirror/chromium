  ...
  BrowserThread::PostTask(
      ...
      base::BindOnce(
          [](device::mojom::SerialIoHandlerRequest request) {
            ...
            // In unit tests environment there is not ServiceManagerConnection.
          ...
          mojo::MakeRequest(&io_handler_mojo_)));
  io_handler_mojo_.set_connection_error_handler(base::BindOnce(
      &SerialConnection::OnConnectionError, base::Unretained(this)));
}

SerialConnection::~SerialConnection() {
  io_handler_mojo_->CancelRead(device::mojom::SerialReceiveError::DISCONNECTED);
  io_handler_mojo_->CancelWrite(device::mojom::SerialSendError::DISCONNECTED);
}
...
void SerialConnection::set_paused(bool paused) {
  DCHECK(io_handler_mojo_);
  paused_ = paused;
  if (paused) {
    io_handler_mojo_->CancelRead(device::mojom::SerialReceiveError::NONE);
  }
}

void SerialConnection::set_connection_error_handler(
    base::OnceClosure connection_error_handler) {
  if (io_handler_mojo_.encountered_error()) {
...
void SerialConnection::Configure(const api::serial::ConnectionOptions& options,
  ...
  if (options.send_timeout.get())
    set_send_timeout(*options.send_timeout);
  io_handler_mojo_->ConfigurePort(
      device::mojom::SerialConnectionOptions::From(options),
      ScopedCallbackRunner(std::move(callback), false));
  io_handler_mojo_->CancelRead(device::mojom::SerialReceiveError::NONE);
}
...
void SerialConnection::GetInfo(GetInfoCompleteCallback callback) const {
  ...
  io_handler_mojo_->GetPortInfo(ScopedCallbackRunner(
      std::move(resp_callback), device::mojom::SerialConnectionInfoPtr()));
}

void SerialConnection::Flush(FlushCompleteCallback callback) const {
  DCHECK(io_handler_mojo_);
  return io_handler_mojo_->Flush(
      ScopedCallbackRunner(std::move(callback), false));
}
...
void SerialConnection::GetControlSignals(
  ...
  auto resp_callback = base::BindOnce(
      ...
      std::move(callback));
  io_handler_mojo_->GetControlSignals(
      ScopedCallbackRunner(std::move(resp_callback),
...
void SerialConnection::SetControlSignals(
  ...
  DCHECK(io_handler_mojo_);
  io_handler_mojo_->SetControlSignals(
...
void SerialConnection::SetBreak(SetBreakCompleteCallback callback) {
  DCHECK(io_handler_mojo_);
  io_handler_mojo_->SetBreak(ScopedCallbackRunner(std::move(callback), false));
}

void SerialConnection::ClearBreak(ClearBreakCompleteCallback callback) {
  DCHECK(io_handler_mojo_);
  io_handler_mojo_->ClearBreak(
      ScopedCallbackRunner(std::move(callback), false));
}
...
void SerialConnection::OnReceiveTimeout() {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  DCHECK(io_handler_mojo_);
  io_handler_mojo_->CancelRead(device::mojom::SerialReceiveError::TIMEOUT);
}

void SerialConnection::OnSendTimeout() {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  DCHECK(io_handler_mojo_);
  io_handler_mojo_->CancelWrite(device::mojom::SerialSendError::TIMEOUT);
}
