...
void SerialConnectFunction::AsyncWorkStart() {
  ...
  connection_ =
      base::MakeUnique<SerialConnection>(params_->path, extension_->id());
  ...
  if (!connected || !is_all_info || !info) {
    ...
    GetSerialConnection(id)->set_connection_error_handler(base::BindOnce(
        [](ApiResourceManager<SerialConnection>* manager,
          ...
          DCHECK(manager);
          manager->Remove(extension_id, api_resource_id);
        },
        manager_, extension_->id(), id));
