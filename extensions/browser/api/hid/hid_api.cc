...
void HidReceiveFunction::StartWork(device::mojom::HidConnection* connection) {
  connection->Read(ScopedCallbackRunner(
      base::BindOnce(&HidReceiveFunction::OnFinished, this), false,
      base::nullopt));
}
...
void HidSendFunction::StartWork(device::mojom::HidConnection* connection) {
  ...
  buffer.push_back(static_cast<uint8_t>(parameters_->report_id));

  const uint8_t* data =
      reinterpret_cast<const uint8_t*>(parameters_->data.data());
  buffer.insert(buffer.end(), data, data + parameters_->data.size());

  connection->Write(
      buffer, ScopedCallbackRunner(
