...
void HidDeviceManager::LazyInitialize() {
  ...
  // Init enumeration and register self as observer of HidManager.
  DCHECK(!hid_manager_);
  ...
  binding_.Bind(mojo::MakeRequest(&observer_info));
  ...
  hid_manager_->GetDevicesAndRegister(
      std::move(observer_info),
