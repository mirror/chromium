...
class FakeHidConnectionManager : public device::mojom::HidConnection {
 public:
  FakeHidConnectionManager(scoped_refptr<device::HidDeviceInfo> device)
      : device_(device) {}

  ~FakeHidConnectionManager() override = default;
  ...
  void Read(device::mojom::HidConnection::ReadCallback callback) override {
    ...
    data.push_back(report_id);
    for (auto& r : kResult) {
      if (r)  // don't copy the '\0' at end.
        data.push_back(r);
    }
  ...
  void Write(const std::vector<uint8_t>& buffer,
    ...
    if (buffer.size() != sizeof(kExpected)) {
  ...
  void GetFeatureReport(uint8_t report_id,
                        device::mojom::HidConnection::GetFeatureReportCallback
                            callback) override {
    int expected_report_id = device_->has_report_id() ? 1 : 0;
    if (static_cast<int>(report_id) != expected_report_id) {
...
class FakeHidManager : public device::mojom::HidManager {
 public:
  ...
  void GetDevices(
      device::mojom::HidManager::GetDevicesCallback callback) override {
    // Clients of HidManager in extensions only use GetDevicesAndRegister().
    DCHECK(0);
  }
  ...
  void Connect(const std::string& device_guid,
    ...
    // Strong binds a instance of FakeHidConnctionManager.
    device::mojom::HidConnectionPtr client;
    auto binding = mojo::MakeStrongBinding(
        base::MakeUnique<FakeHidConnectionManager>(device),
  ...
  void RemoveDevice(const HidPlatformDeviceId& platform_device_id) {
    ...
    if (!guid.empty()) {
      ...
      observers_.ForAllPtrs(
          [device_info](device::mojom::HidObserver* observer) {
