...
void NoopHidCallback(const std::vector<device::mojom::HidDeviceInfo*>&) {}
...
class HidDeviceInfo : public DevicePermissionsPrompt::Prompt::DeviceInfo {
 public:
  ...
  const device::mojom::HidDeviceInfoPtr& device() const { return device_; }

 private:
  // HidDeviceInfo takes ownership of the passed
...
class HidDevicePermissionsPrompt : public DevicePermissionsPrompt::Prompt,
 ...
 private:
  ...
  void OnDeviceAdded(device::mojom::HidDeviceInfoPtr device) override {
    if (HasUnprotectedCollections(device.get()) &&
        (filters_.empty() ||
         HidDeviceFilter::MatchesAny(device.get(), filters_))) {
      std::unique_ptr<DeviceInfo> device_info(
          new HidDeviceInfo(std::move(device)));
#if defined(OS_CHROMEOS)
      chromeos::PermissionBrokerClient* client =
          chromeos::DBusThreadManager::Get()->GetPermissionBrokerClient();
      DCHECK(client) << "Could not get permission broker client.";
      const HidDeviceInfo* hid_device_info =
          static_cast<HidDeviceInfo*>(device_info.get());
      client->CheckPathAccess(
          hid_device_info->device()->device_node,
