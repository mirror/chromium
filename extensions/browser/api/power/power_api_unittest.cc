...
// wake lock being requested or cancelled.
enum Request {
...
// Tests instantiate this class to make PowerAPI's calls to simulate request
...
class FakeWakeLockManager {
 public:
  explicit FakeWakeLockManager(content::BrowserContext* context)
      : browser_context_(context), outstanding_(false) {
    PowerAPI::Get(browser_context_)
        ->SetWakeLockFunctionForTesting(
            base::Bind(&FakeWakeLockManager::RequestWakeLock,
  ...
  ~FakeWakeLockManager() {
    PowerAPI::Get(browser_context_)
        ->SetWakeLockFunctionForTesting(PowerAPI::RequestWakeLockFunction(),
 ...
 private:
  // Request a new fake wake lock of type |type|.
  void RequestWakeLock(device::mojom::WakeLockType type) {
    if (outstanding_) {
      if (type == type_)
        return;

      // Has a wake lock already, perform ChangeType:
      switch (type) {
    ...
    // Doesn't have a wake lock, make a new request:
    if (!outstanding_) {
      switch (type) {
        case device::mojom::WakeLockType::PreventAppSuspension:
          requests_.push_back(BLOCK_APP_SUSPENSION);
