...
void PowerAPI::SetWakeLockFunctionForTesting(
    const RequestWakeLockFunction& request_function,
    const CancelWakeLockFunction& cancel_function) {
  request_wake_lock_function_ =
      !request_function.is_null()
          ? request_function
          : base::Bind(&PowerAPI::RequestWakeLock, base::Unretained(this));
...
void PowerAPI::RequestWakeLock(device::mojom::WakeLockType type) {
  // ChangeType() requests a new type of wake lock before cancel the old one.
  // So there isn't a brief period where power management is unblocked.
  GetWakeLock()->ChangeType(type, base::Bind(&f));
  if (!has_wake_lock_) {
...
void PowerAPI::UpdateWakeLock() {
  ...
  if (!has_wake_lock_ || new_level != current_level_) {
    device::mojom::WakeLockType type = LevelToWakeLockType(new_level);
    request_wake_lock_function_.Run(type);
