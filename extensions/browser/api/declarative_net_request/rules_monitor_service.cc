...
void UnloadRulesetOnIOThread(std::string extension_id, InfoMap* info_map) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::IO);
...
void LoadRulesetOnFileTaskRunner(std::string extension_id,
  ...
  base::OnceClosure task = base::BindOnce(
      &LoadRulesetOnIOThread, std::move(extension_id), install_time,
...
void RulesMonitorService::OnExtensionLoaded(
  ...
  base::OnceClosure task = base::BindOnce(
      ...
      prefs->GetInstallTime(extension->id()), ruleset_checksum,
