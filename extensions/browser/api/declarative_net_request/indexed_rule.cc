...
class UrlFilterParser {
 public:
  ...
  static void Parse(std::string url_filter, IndexedRule* indexed_rule) {
    DCHECK(indexed_rule);
    UrlFilterParser(std::move(url_filter), indexed_rule).ParseImpl();
  }
...
ParseResult IndexedRule::CreateIndexedRule(
  ...
  // |url_pattern| fields.
  UrlFilterParser::Parse(parsed_rule->condition.url_filter
