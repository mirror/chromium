...
bool RulesetManager::ShouldRedirectRequest(const net::URLRequest& request,
  ...
  // TODO(crbug.com/696822): Alternatively, maintain |rules_map_| sorted from
  // most to least recently installed extensions, to prevent redundant ruleset
  // evaluation.
  for (const auto& pair : rules_map_) {
  ...
  return winning_extension_install_time != base::Time::Min();
}
