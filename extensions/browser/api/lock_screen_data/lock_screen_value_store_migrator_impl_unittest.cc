...
class LockScreenValueStoreMigratorImplTest : public testing::Test {
 ...
 protected:
  ...
  void RunMigrator(const std::set<std::string>& extensions_to_migrate) {
    migrator_->Run(
  ...
  std::vector<std::vector<char>> GetItemContent(
    ...
    for (const auto& item_id : item_ids) {
      ...
      } else if (read_result == OperationResult::kWrongKey) {
        items.emplace_back();
  ...
  const std::vector<std::string>& migrated_extension_ids() const {
  ...
  void WaitMigrationDone(const std::string& extension_id) {
 ...
 private:
  ...
  void OnExtensionMigrated(const std::string& extension_id) {
  ...
  std::vector<std::string> migrated_extension_ids_;

  std::unique_ptr<LockScreenValueStoreMigratorImpl> migrator_;

  std::map<std::string, base::RunLoop> extension_waiters_;
...
TEST_F(LockScreenValueStoreMigratorImplTest,
  ...
  RunMigrator({app_1->id()});
  EXPECT_TRUE(migrator()->MigratingExtensionData(app_1->id()));
  EXPECT_FALSE(migrator()->MigratingExtensionData(app_2->id()));
...
// Tests storage state at the moment the target item contens is written.
TEST_F(LockScreenValueStoreMigratorImplTest,
...
TEST_F(LockScreenValueStoreMigratorImplTest,
  ...
  RunTaskRunnerTasks();
  EXPECT_FALSE(migrator()->MigratingExtensionData(app_1->id()));
  EXPECT_FALSE(migrator()->MigratingExtensionData(app_2->id()));
  EXPECT_EQ(std::vector<std::string>({app_1->id()}), migrated_extension_ids());
...
TEST_F(LockScreenValueStoreMigratorImplTest,
  ...
  // Whie the extension data has been moved at this point, the migrator was
...
TEST_F(LockScreenValueStoreMigratorImplTest,
  ...
  EXPECT_FALSE(migrator()->MigratingExtensionData(app_1->id()));
  EXPECT_TRUE(migrator()->MigratingExtensionData(app_2->id()));

  run_loop.Run();

  EXPECT_FALSE(base::ContainsValue(migrated_extension_ids(), app_1->id()));
...
TEST_F(LockScreenValueStoreMigratorImplTest,
  ...
  EXPECT_FALSE(migrator()->MigratingExtensionData(app->id()));
