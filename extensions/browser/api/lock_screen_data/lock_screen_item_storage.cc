...
void LockScreenItemStorage::CreateItemImpl(const std::string& extension_id,
                                           const CreateCallback& callback) {
  if (data_item_cache_.find(extension_id) == data_item_cache_.end())
    return;
...
void LockScreenItemStorage::RemoveExtensionFromLocalState(
  ...
  if (it->second.state == CachedExtensionData::State::kLoading) {
    it->second.state = CachedExtensionData::State::kLoaded;
  } else {
