...
void SocketsUdpCreateFunction::Work() {
  ...
  ResumableUDPSocket* socket =
      new ResumableUDPSocket(std::move(factory_ptr), extension_->id());
...
void SocketsUdpBindFunction::AsyncWorkStart() {
  ...
  socket->Bind(params_->address, params_->port,
               base::Bind(&SocketsUdpBindFunction::OnCompleted,
...
void SocketsUdpJoinGroupFunction::AsyncWorkStart() {
  ...
  socket->JoinGroup(
      params_->address,
      base::Bind(&SocketsUdpJoinGroupFunction::OnJoinGroupComplete,
...
void SocketsUdpLeaveGroupFunction::AsyncWorkStart() {
  ...
  socket->LeaveGroup(
      params_->address,
      base::Bind(&SocketsUdpLeaveGroupFunction::OnLeaveGroupComplete,
                 base::Unretained(this)));
}
...
void SocketsUdpSetBroadcastFunction::Work() {
  ...
  socket->SetBroadcast(
      params_->enabled,
      base::Bind(&SocketsUdpSetBroadcastFunction::OnSetBroadcastComplete,
                 base::Unretained(this), base::Unretained(socket)));
}
