...
void WebRequestEventDetails::SetTLSInfo(const net::URLRequest* request) {
  ...
  if (tls_info.unverified_cert != nullptr) {
    info_dict->Set(keys::kTLSSentChainKey,
                   helpers::ExtractCertificateChain(*tls_info.unverified_cert));
  }

  if (tls_info.cert != nullptr) {
    info_dict->Set(keys::kTLSBuiltChainKey,
                   helpers::ExtractCertificateChain(*tls_info.cert));
  }
