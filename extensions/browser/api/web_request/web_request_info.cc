...
WebRequestInfo::WebRequestInfo(net::URLRequest* url_request)
  ...
  if (info) {
    DCHECK(render_process_id == -1 || render_process_id == info->GetChildID());
  ...
  if (navigation_ui_data) {
    ...
    // FrameData available from the NavigationUIData. Use that.
    ...
    // For subresources loads, attempt to resolve the FrameData immediately
