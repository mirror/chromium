...
// Test the handling if too many requests are being tracked and some need to be
// discarded.
TEST(ExtensionWebRequestTimeTrackerTest, Overflow) {
  base::HistogramTester histogram_tester;
  ...
  for (int i = 0; i < 200; ++i)
    tracker.LogRequestStartTime(i, start);

  for (int i = 0; i < 200; ++i)
    tracker.IncrementTotalBlockTime(i, kModerateDelay);
  ...
  histogram_tester.ExpectTimeBucketCount("Extensions.NetworkDelay",
                                         kModerateDelay, 101);
  histogram_tester.ExpectTotalCount("Extensions.NetworkDelay", 101);

  histogram_tester.ExpectBucketCount("Extensions.NetworkDelayPercentage", 25,
                                     101);
  histogram_tester.ExpectTotalCount("Extensions.NetworkDelayPercentage", 101);
}
