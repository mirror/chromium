...
UDPSocket::UDPSocket(network::mojom::UDPSocketPtr socket,
  ...
  // FIXME: set connection error handler.
}
...
void UDPSocket::OnOpenCompleted(const net::CompletionCallback& callback,
  ...
  // FIXME: might need to do null check here.
  if (should_connect) {
    socket_->Connect(
        ip_end_point,
...
ResumableUDPSocket::ResumableUDPSocket(
    network::mojom::UDPSocketPtr socket,
    network::mojom::UDPSocketReceiverRequest receiver_request,
    const std::string& owner_extension_id)
