...
void UDPSocket::Connect(const net::AddressList& address,
  ...
  socket_->Open(
      ...
      base::BindOnce(&UDPSocket::OnOpenCompleted, base::Unretained(this),
...
void UDPSocket::SendTo(scoped_refptr<net::IOBuffer> io_buffer,
  ...
  socket_->SendTo(address, data,
                  net::MutableNetworkTrafficAnnotationTag(
                      Socket::GetNetworkTrafficAnnotationTag()),
                  base::BindOnce(&UDPSocket::OnSendToCompleted,
                                 base::Unretained(this), callback));
}
...
void UDPSocket::OnOpenCompleted(const net::CompletionCallback& callback,
                                bool should_connect,
...
void UDPSocket::OnSendToCompleted(const net::CompletionCallback& callback,
                                  int result) {
  callback.Run(result);
}

void UDPSocket::OnJoinGroupCompleted(
...
void UDPSocket::LeaveGroup(const std::string& address,
  ...
  socket_->LeaveGroup(ip, base::BindRepeating(&UDPSocket::OnLeaveGroupCompleted,
                                              base::Unretained(this), callback,
                                              normalized_address));
}
