...
class UDPSocket : public Socket, public network::mojom::UDPSocketReceiver {
 public:
  explicit UDPSocket(content::BrowserContext* browser_context,
 ...
 private:
  ...
  void OnOpenComplete(const net::CompletionCallback& user_callback,
                      bool should_connect,
                      const net::IPEndPoint& ip_end_point,
                      int result);
  void OnConnectComplete(const net::CompletionCallback& user_callback,
                         const net::IPEndPoint& remote_addr,
                         int result,
                         const base::Optional<net::IPEndPoint>& local_addr);
  void OnBindComplete(const net::CompletionCallback& user_callback,
                      int result,
                      const base::Optional<net::IPEndPoint>& local_addr);

  void OnWriteComplete(const net::CompletionCallback& user_callback,
                       int result);
  void OnSendToComplete(const net::CompletionCallback& user_callback,
                        int result);
  ...
  std::unique_ptr<net::IPEndPoint> local_addr_;
  std::unique_ptr<net::IPEndPoint> peer_addr_;

  ReadCompletionCallback read_callback_;

  RecvFromCompletionCallback recv_from_callback_;

  CompletionCallback connect_or_bind_callback_;
...
class ResumableUDPSocket : public UDPSocket {
 public:
  explicit ResumableUDPSocket(content::BrowserContext* browser_context,
                              const std::string& owner_extension_id);
