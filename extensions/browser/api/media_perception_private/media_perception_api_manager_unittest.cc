...
void RecordServiceErrorAndRunClosure(
...
media_perception::ServiceError SetStateAndWaitForResponse(
  ...
  media_perception::ServiceError service_error;
  manager->SetState(state, base::Bind(&RecordServiceErrorAndRunClosure,
...
media_perception::ServiceError GetStateAndWaitForResponse(
  ...
  base::RunLoop run_loop;
  media_perception::ServiceError service_error;
  manager->GetState(base::Bind(&RecordServiceErrorAndRunClosure,
                               run_loop.QuitClosure(), &service_error));
  run_loop.Run();
  return service_error;
}
...
media_perception::ServiceError GetDiagnosticsAndWaitForResponse(
  ...
  base::RunLoop run_loop;
  media_perception::ServiceError service_error;
  manager->GetDiagnostics(base::Bind(&RecordServiceErrorAndRunClosure,
                                     run_loop.QuitClosure(), &service_error));
  run_loop.Run();
  return service_error;
}
...
TEST_F(MediaPerceptionAPIManagerTest, UpstartFailure) {
  ...
  base::RunLoop run_loop;
  media_perception::ServiceError service_error;
  manager_->SetState(state, base::Bind(&RecordServiceErrorAndRunClosure,
                                       run_loop.QuitClosure(), &service_error));
