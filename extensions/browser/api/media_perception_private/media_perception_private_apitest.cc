  ...
  void LoadCrOSComponent(
    ...
    if (type == media_perception::COMPONENT_TYPE_LIGHT) {
      base::ThreadTaskRunnerHandle::Get()->PostTask(
          FROM_HERE,
          base::BindOnce(std::move(load_callback), "non-empty string"));
      return;
    }
...
class TestExtensionsAPIClient : public ShellExtensionsAPIClient {
 public:
  ...
  std::unique_ptr<MediaPerceptionAPIDelegate> GetMediaPerceptionAPIDelegate()
      const override {
    return std::make_unique<TestMediaPerceptionAPIDelegate>();
  }
};
...
IN_PROC_BROWSER_TEST_F(MediaPerceptionPrivateApiTest, SetAnalyticsComponent) {
  ...
  ASSERT_TRUE(RunAppTest("media_perception/component")) << message_;
}
