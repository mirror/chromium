...
void EventRouter::AddFilteredEventListener(const std::string& event_name,
  ...
  bool added = listeners_.AddListener(is_for_service_worker ?
      EventListener::ForExtensionServiceWorker(
...
void EventRouter::RemoveFilteredEventListener(
  ...
  const bool is_for_service_worker = sw_identifier.has_value();
  std::unique_ptr<EventListener> listener = is_for_service_worker ?
    EventListener::ForExtensionServiceWorker(
        event_name, extension_id, process,
        sw_identifier->scope,
        sw_identifier->thread_id,
...
void EventRouter::OnExtensionLoaded(content::BrowserContext* browser_context,
  ...
  //std::set<std::string> registered_events;
  //GetRegisteredEvents(extension->id(), RegisteredEventType::kLazy, &registered_events);
  // THIS SHOULD BE A SEPARATE CL.
  //GetRegisteredEvents(extension->id(), RegisteredEventType::kServiceWorker, &registered_events);
