...
class ContentHashReader : public base::RefCountedThreadSafe<ContentHashReader> {
 public:
  ...
  // These return whether we found valid verified_contents.json and
  // computed_hashes.json files. Note that this can return true but we still
  // didn't find an entry for |relative_path_| in them.
