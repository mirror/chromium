...
TEST_F(EventRouterFilterTest, Basic) {
  ...
  for (size_t i = 0; i < arraysize(kHostSuffixes); ++i) {
    ...
    event_router()->AddFilteredEventListener(
        kEventName, render_process_host(), kExtensionId,
        base::Optional<ExtensionHostMsg_ServiceWorkerIdentifier>(), *filter,
        true);
  ...
  event_router()->RemoveFilteredEventListener(
      kEventName, render_process_host(), kExtensionId,
      base::Optional<ExtensionHostMsg_ServiceWorkerIdentifier>(), *filters[1],
      true);
  ASSERT_TRUE(ContainsFilter(kExtensionId, kEventName, *filters[0]));
  ...
  event_router()->RemoveFilteredEventListener(
      kEventName, render_process_host(), kExtensionId,
      base::Optional<ExtensionHostMsg_ServiceWorkerIdentifier>(), *filters[0],
      true);
  ...
  event_router()->RemoveFilteredEventListener(
      kEventName, render_process_host(), kExtensionId,
      base::Optional<ExtensionHostMsg_ServiceWorkerIdentifier>(), *filters[2],
      true);
  ASSERT_FALSE(ContainsFilter(kExtensionId, kEventName, *filters[0]));
  ASSERT_FALSE(ContainsFilter(kExtensionId, kEventName, *filters[1]));
