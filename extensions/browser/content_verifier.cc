...
ContentVerifier::TestObserver* g_ContentVerifier_test_observer = NULL;
...
void ContentVerifier::SetObserverForTests(TestObserver* observer) {
  g_ContentVerifier_test_observer = observer;
}
...
void ContentVerifier::OnFetchComplete(
  ...
  if (g_ContentVerifier_test_observer)
    g_ContentVerifier_test_observer->OnFetchComplete(extension_id, success);
