...
class ValueStore {
 public:
  ...
  struct Status {
    ...
    explicit Status(const Status& other);
    Status(Status&& other);
    ~Status();
  ...
  class ReadResult {
   public:
    ReadResult(std::unique_ptr<base::DictionaryValue> settings,
               const Status& status);
    explicit ReadResult(const Status& status);
  ...
  class WriteResult {
   public:
    WriteResult(std::unique_ptr<ValueStoreChangeList> changes,
                const Status& status);
    explicit WriteResult(const Status& status);
