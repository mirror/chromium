...
ValueStore::ReadResult LeveldbValueStore::Get(
  ...
  return ReadResult(std::move(settings), status);
}
...
ValueStore::WriteResult LeveldbValueStore::Set(WriteOptions options,
  ...
  return status.ok() ? WriteResult(std::move(changes), status)
                     : WriteResult(status);
}
...
ValueStore::WriteResult LeveldbValueStore::Remove(
  ...
  return WriteResult(std::move(changes), status);
}
...
ValueStore::WriteResult LeveldbValueStore::Clear() {
  ...
  if (!read_result.status().ok())
    return WriteResult(read_result.status());
  ...
  DeleteDbFile();
  return WriteResult(std::move(changes), read_result.status());
}
