...
class SandboxedUnpacker : public base::RefCountedThreadSafe<SandboxedUnpacker> {
 ...
 private:
  ...
  enum FailureReason {
    ...
    // SandboxedUnpacker::IndexAndPersistRulesetIfNeeded()
  ...
  // Indexes |json_ruleset| and persists the corresponding indexed file for the
  // Declarative Net Request API. Returns false and reports failure in case of
  // an error.
  bool IndexAndPersistRuleset(std::unique_ptr<base::Value> json_ruleset);
