...
class SandboxedUnpacker : public base::RefCountedThreadSafe<SandboxedUnpacker> {
 ...
 private:
  ...
  void ReportUnpackingError(const std::string& error);
  ...
  // Returns a bound interface ptr to the JsonParser that can be used on the
  // |unpacker_io_task_runner_|.
  data_decoder::mojom::JsonParserPtr* GetJsonParserPtr();

  // Parses the JSON file at |path| on a thread that can block and invokes
  // |callback| on the calling thread. |callback| is called with a null
  // parameters if parsing failed.
