...
void SandboxedUnpacker::UnpackExtensionSucceeded(
  ...
  if (json_ruleset && !IndexAndPersistRuleset(std::move(json_ruleset)))
    return;  // Failure was already reported.

  ReportSuccess(std::move(manifest), install_icon);
}

bool SandboxedUnpacker::IndexAndPersistRuleset(
    std::unique_ptr<base::Value> json_ruleset) {
  DCHECK(extension_);
  DCHECK(json_ruleset);
  ...
  if (!declarative_net_request::IndexAndPersistRuleset(
