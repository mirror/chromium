...
data_decoder::mojom::JsonParserPtr* SandboxedUnpacker::GetJsonParserPtr() {
  DCHECK(unpacker_io_task_runner_->RunsTasksInCurrentSequence());
  if (!json_parser_ptr_.is_bound()) {
  ...
  return &json_parser_ptr_;
}

void SandboxedUnpacker::ReportUnpackingError(const std::string& error) {
  unpacker_io_task_runner_->PostTask(
