...
bool SandboxedUnpacker::IndexAndPersistRulesIfNeeded(
  ...
  // Delete extension provided indexed ruleset file, since its a reserved file
  // name. This helps ensure that we only use one generated by the Extension
  // system.
  ...
  if (!json_ruleset) {
    ReportFailure(ERROR_PARSING_DECLARATIVE_RULESET,
                  l10n_util::GetStringFUTF16(
                      IDS_EXTENSION_PACKAGE_INSTALL_ERROR,
                      ASCIIToUTF16("ERROR_PARSING_DECLARATIVE_RULESET")));
    return false;
  }
...
base::string16 SandboxedUnpacker::FailureReasonToString16(
  ...
  switch (reason) {
    ...
    case ERROR_PARSING_DECLARATIVE_RULESET:
      return ASCIIToUTF16("ERROR_PARSING_DECLARATIVE_RULESET");
