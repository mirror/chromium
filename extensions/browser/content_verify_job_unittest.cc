...
TEST_F(ContentVerifyJobUnittest, DeletedAndMissingFiles) {
  ...
  {
    // Now create a resource foo.js which exists but is not in the extension's
    // verified_contents.json. Verification should result in NO_HASHES_FOR_FILE
    // since the extension is trying to load a file the extension should not
    // have.
