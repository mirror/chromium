 ...
 private:
  ...
  void OnFilePathAndLastModifiedTimeRead(base::FilePath* read_file_path,
    ...
    response_info_.headers = BuildHttpHeaders(
        ...
        send_cors_header_,
        *last_modified_time);
    // Set the mime type for the request. We use
    // GetWellKnownMimeTypeFromExtension() to ensure that configurations that
    // may have been set by other programs on a user's machine don't affect
    ...
    bool found_mime_type = !file_extension.empty() &&
