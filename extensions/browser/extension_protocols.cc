...
bool GetDirectoryForExtensionURL(const GURL& url,
  ...
  DLOG(WARNING) << "Failed to get directory for extension " << extension_id;
  return false;
}

bool IsWebViewRequest(int child_id, net::URLRequest* request) {
...
ExtensionProtocolHandler::MaybeCreateJob(
  ...
  if (extension) {
    GetSecurityPolicyForURL(request->url(), extension, info->GetChildID(),
                            IsWebViewRequest(info->GetChildID(), request),
...
void LoadExtensionResourceFromFileOnBackgroundSequence(
  ...
  content::CreateFileURLLoaderWithPathOverride(
      request, resource.GetFilePath(), std::move(loader), std::move(client));
}
  ...
  void CreateLoaderAndStart(content::mojom::URLLoaderRequest loader,
    ...
    if (extension) {
      const bool is_web_view_request =
          ...
          nullptr;
      GetSecurityPolicyForURL(request.url, extension, process_host->GetID(),
...
MaybeCreateExtensionSubresourceURLLoaderFactory(
  ...
  // cannot have any active content scripts running. For now we always create
  // a factory, because the loader itself correctly prevents disallowed
  // resources from loading in an invalid context.
