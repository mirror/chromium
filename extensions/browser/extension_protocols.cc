...
class ExtensionURLLoaderFactory : public network::mojom::URLLoaderFactory {
 public:
  ...
  void CreateLoaderAndStart(network::mojom::URLLoaderRequest loader,
    ...
    // TODO(cmumford): After network service is enabled eliminate hop to IO
    // thread as per http://crbug.com/805971.
    const ExtensionSet& enabled_extensions =
        ExtensionRegistry::Get(browser_context)->enabled_extensions();
    const ProcessMap* process_map = ProcessMap::Get(browser_context);
    content::BrowserThread::PostTaskAndReplyWithResult(
        content::BrowserThread::IO, FROM_HERE,
        base::BindOnce(&GetExtensionInfo, request, extension_info_map_,
                       &enabled_extensions, process_host->GetID(),
                       browser_context->IsOffTheRecord(), process_map),
 ...
 private:
  ...
  void OnGotExtensionInfo(network::mojom::URLLoaderRequest loader,
    ...
    if (SharedModuleInfo::IsImportedPath(path)) {
      ...
      if (SharedModuleInfo::ImportsExtensionById(extension_info.extension.get(),
                                                 new_extension_id) &&
          new_extension) {
  ...
  void OnFilePathAndLastModifiedTimeRead(
    ...
    content::BrowserThread::PostTask(
        ...
        base::BindOnce(&StartVerifyJob, std::move(request), std::move(loader),
                       std::move(client), resource, std::move(headers),
