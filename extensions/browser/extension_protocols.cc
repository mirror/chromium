...
bool ExtensionCanLoadInIncognito(bool is_main_frame,
  ...
  return extension && IncognitoInfo::IsSplitMode(extension);
}
...
bool AllowExtensionResourceLoad(const GURL& url,
  ...
  if (is_incognito &&
      !ExtensionCanLoadInIncognito(is_main_frame, url.host(),
                                   extension_info_map)) {
    return false;
  }
  ...
  // request.
  if (extension_info_map->process_map().Contains(url.host(), child_id))
    return true;
...
bool GetDirectoryForExtensionURL(const GURL& url,
  ...
  if (path.empty()) {
    ...
    const Extension* disabled_extension =
        extension_info_map->disabled_extensions().GetByID(extension->id());
  ...
  DLOG(WARNING) << "Failed to get directory for extension " << extension->id();
  return false;
}

bool IsWebViewRequest(int child_id,
  ...
  const ExtensionNavigationUIData* data =
      ExtensionsBrowserClient::Get()->GetExtensionNavigationUIData(
          navigation_ui_data);
  return data && data->is_web_view();
}

void GetSecurityPolicyForURL(
    const GURL& url,
    const Extension* extension,
    int child_id,
    const content::NavigationUIData* navigation_ui_data,
  ...
  if (!IsWebViewRequest(child_id, navigation_ui_data)) {
...
ExtensionProtocolHandler::MaybeCreateJob(
  ...
  if (!info) {
    ...
    if (!ResourceRequestInfo::OriginatedFromServiceWorker(request)) {
      LOG(ERROR) << "Allowing load of " << request->url().spec()
                ...
                extension, extension_info_map_)) {
    return new net::URLRequestErrorJob(request, network_delegate,
                                       net::ERR_BLOCKED_BY_CLIENT);
  }

  base::FilePath directory_path;
  if (!GetDirectoryForExtensionURL(request->url(), extension,
                                   extension_info_map_, &directory_path)) {
    return nullptr;
  }
  ...
  if (extension) {
    GetSecurityPolicyForURL(
        request->url(), extension, info->GetChildID(),
        info->GetNavigationUIData(), &content_security_policy,
...
using LoaderFactoryMap = std::map<content::ResourceContext*,
                                  std::unique_ptr<ExtensionURLLoaderFactory>>;
base::LazyInstance<LoaderFactoryMap>::Leaky g_factories;

class ExtensionURLLoaderFactory : public content::mojom::URLLoaderFactory {
 public:
  explicit ExtensionURLLoaderFactory(content::ResourceContext* resource_context)
      : resource_context_(resource_context),
        extension_info_map_(ExtensionsBrowserClient::Get()->GetExtensionInfoMap(
            resource_context_)) {
  ...
  void Bind(content::mojom::URLLoaderFactoryRequest request,
            std::unique_ptr<content::NavigationUIData> navigation_ui_data,
            int child_id,
            bool is_incognito) {
    bindings_.AddBinding(this, std::move(request),
                         LoaderContext(child_id, is_incognito, std::move(navigation_ui_data)));
  }

 private:
  struct LoaderContext {
    LoaderContext(LoaderContext&&) = default;
    LoaderContext(int child_id, bool is_incognito, std::unique_ptr<content::NavigationUIData> navigation_ui_data)
        : child_id(child_id), is_incognito(is_incognito),
          navigation_ui_data(std::move(navigation_ui_data)) {}
    ~LoaderContext() = default;

    LoaderContext& operator=(LoaderContext&&) = default;

    LoaderContext Clone() const {
      return LoaderContext(child_id, is_incognito, navigation_ui_data->Clone());
    }
    ...
    DISALLOW_COPY_AND_ASSIGN(LoaderContext);
  };
  ...
  void CreateLoaderAndStart(content::mojom::URLLoaderRequest loader,
    ...
    const Extension* extension =
        extension_info_map_->extensions().GetByID(extension_id);
    LOG(ERROR) << "Got loader request for " << request.url.spec()
               << " with context " << resource_context_;
    LOG(ERROR) << "Extension " << extension_id << " is " << extension;

    const LoaderContext& context = bindings_.dispatch_context();
    if (!AllowExtensionResourceLoad(request.url, request.resource_type,
                                    request.transition_type, context.child_id,
                                    context.is_incognito, extension,
                                    extension_info_map_)) {
      client->OnComplete(
    ...
    if (extension) {
      GetSecurityPolicyForURL(
          request.url, extension, -1, context.navigation_ui_data.get(),
          &content_security_policy, &send_cors_header, &follow_symlinks_anywhere);
    }

    if (IsBackgroundPageURL(request.url)) {
      // Special case for the generated background page.
      // --
      content::ResourceResponseHead head;
      std::string source;
      head.headers = BuildHttpHeaders(content_security_policy, send_cors_header, base::Time::Now());
      GenerateBackgroundPageContents(extension, &head.mime_type, &head.charset,
                                     &source);

      mojo::DataPipe pipe(static_cast<uint32_t>(source.size()));
      client->OnReceiveResponse(head, base::nullopt, nullptr);
      client->OnStartLoadingResponseBody(std::move(pipe.consumer_handle));

      auto producer = base::MakeUnique<mojo::StringDataPipeProducer>(
          std::move(pipe.producer_handle));
      auto* raw_producer = producer.get();
      producer->Write(
          source,
          base::BindOnce(
              &ExtensionURLLoaderFactory::OnBackgroundPageResponseComplete,
              base::Unretained(this), raw_producer, std::move(client)));
      background_page_producers_.emplace(raw_producer, std::move(producer));
      return;
    }

    NOTIMPLEMENTED()
        << "Need to be able to fetch other extension resources from disk.";
  }

  void Clone(content::mojom::URLLoaderFactoryRequest request) override {
    bindings_.AddBinding(this, std::move(request),
                         bindings_.dispatch_context().Clone());
  }

  void OnBackgroundPageResponseComplete(
      mojo::StringDataPipeProducer* producer,
      content::mojom::URLLoaderClientPtr client,
      MojoResult result) {
    LOG(ERROR) << "FINISHED SENDING BG PAGE DATA.";
    background_page_producers_.erase(producer);
  ...
  extensions::InfoMap* const extension_info_map_;
  mojo::BindingSet<content::mojom::URLLoaderFactory, LoaderContext> bindings_;
  base::flat_map<mojo::StringDataPipeProducer*,
                 std::unique_ptr<mojo::StringDataPipeProducer>>
      background_page_producers_;

  DISALLOW_COPY_AND_ASSIGN(ExtensionURLLoaderFactory);
};

void CreateExtensionURLLoaderFactoryOnIOThread(
    content::ResourceContext* resource_context,
    const content::ResourceRequest& request,
    std::unique_ptr<content::NavigationUIData> navigation_ui_data,
    int child_id,
    bool is_incognito,
    content::mojom::URLLoaderFactoryRequest factory_request) {
  std::unique_ptr<ExtensionURLLoaderFactory>& factory =
      g_factories.Get()[resource_context];
  if (!factory)
    factory = base::MakeUnique<ExtensionURLLoaderFactory>(resource_context);
  factory->Bind(std::move(factory_request), std::move(navigation_ui_data), child_id, is_incognito);
}
...
content::mojom::URLLoaderFactoryPtrInfo CreateExtensionURLLoaderFactory(
    ...
    int child_id,
    bool is_incognito) {
  content::mojom::URLLoaderFactoryPtrInfo factory;
  content::BrowserThread::GetTaskRunnerForThread(content::BrowserThread::IO)
      ->PostTask(FROM_HERE,
                 base::BindOnce(&CreateExtensionURLLoaderFactoryOnIOThread,
                                resource_context, request, navigation_ui_data ? navigation_ui_data->Clone() : nullptr, child_id,
                                is_incognito, mojo::MakeRequest(&factory)));
  return factory;
}
