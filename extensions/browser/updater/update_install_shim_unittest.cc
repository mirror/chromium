...
TEST_F(UpdateInstallShimTest, Install_InvalidUnpackedDir) {
...
TEST_F(UpdateInstallShimTest, Install_EmptyInstallCallback) {
  ...
  EXPECT_TRUE(executed_);
  EXPECT_EQ(static_cast<int>(InstallError::GENERIC_ERROR), result_.error);
  EXPECT_FALSE(base::PathExists(unpacked_dir.GetPath()));
}

TEST_F(UpdateInstallShimTest, Install_BasicInstallOperation_Error) {
  base::ScopedTempDir root_dir;
  ASSERT_TRUE(root_dir.CreateUniqueTempDir());
  ASSERT_TRUE(base::PathExists(root_dir.GetPath()));
  scoped_refptr<UpdateInstallShim> installer =
...
TEST_F(UpdateInstallShimTest, Install_BasicInstallOperation_Success) {
  base::ScopedTempDir root_dir;
  ASSERT_TRUE(root_dir.CreateUniqueTempDir());
  ASSERT_TRUE(base::PathExists(root_dir.GetPath()));
  scoped_refptr<UpdateInstallShim> installer =
