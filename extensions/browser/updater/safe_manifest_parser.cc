...
using data_decoder::GetXmlElementChildWithName;
...
bool ParseSingleAppTag(const base::Value& app_element,
  ...
  std::string updatecheckName =
      GetXmlQualifiedName(xml_namespace, "updatecheck");
  int updatecheck_count =
      data_decoder::GetXmlElementChildrenCount(app_element, updatecheckName);
  ...
  const base::Value* updatecheck =
      data_decoder::GetXmlElementChildWithName(app_element, updatecheckName);
...
void ParseXmlDone(ParseUpdateManifestCallback callback,
  ...
  const base::Value* daystart = GetXmlElementChildWithName(
  ...
  data_decoder::GetAllXmlElementChildrenWithName(
