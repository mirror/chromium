...
void UpdateService::StartUpdateCheck(const UpdateCheckParams& update_params) {
  DCHECK(update_client_);
  if (extension_cache_) {
    extension_cache_->Start(base::BindRepeating(&UpdateService::DoUpdateCheck,
                                                weak_ptr_factory_.GetWeakPtr(),
                                                update_params));
  } else {
    DoUpdateCheck(update_params);
  }
}
...
void UpdateService::DoUpdateCheck(const UpdateCheckParams& update_params) {
  std::map<std::string, UpdateDataProvider::ExtensionUpdateData> update_data;
  std::vector<std::string> extension_ids;
  for (const ExtensionUpdateInfo& info : update_params.update_info) {
  ...
  if (extension_ids.empty())
    return;
  if (update_params.priority == UpdateCheckParams::BACKGROUND) {
