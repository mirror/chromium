...
ContentHash ReadContentHashAfterVerifiedContentsWritten(
  ...
  if (force) {
    mode |= static_cast<int>(
        ContentHash::Mode::kForceRecreateExistingComputedHashesFile);
  }

  ContentHash content_hash =
      ContentHash::Create(extension_key, mode, is_cancelled);
  return content_hash;
}
...
ContentHash ContentHashFetcherJob::LoadVerifiedContents() {
  ...
  int mode =
      static_cast<int>(ContentHash::Mode::kDeleteInvalidVerifiedContents);
  if (force_) {
    mode |= static_cast<int>(
        ContentHash::Mode::kForceRecreateExistingComputedHashesFile);
  }
