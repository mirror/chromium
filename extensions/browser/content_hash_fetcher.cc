...
ContentHash ReadContentHashAfterVerifiedContentsWritten(
    const ContentHash::ExtensionKey& extension_key,
    bool force,
    scoped_refptr<CancellationStatus> cancellation_status) {
  // We just wrote verified_contents.json, so expect it to be valid. If
  // something goes wrong, do not delete verified_contents.json event if the
  // file is invalid.
  bool only_read_verified_contents = true;

  ContentHash content_hash = ContentHash::Create(
      extension_key, only_read_verified_contents, force, cancellation_status);
  return content_hash;
}
...
// directory.
class ContentHashFetcherJob : public CancellationStatus,
 ...
 public:
  ...
  // CancellationStatus:
  // The intent is to attempt to stop I/O operations sooner than just waiting
  // for the entire job to complete. Safe to call from any thread.
 ...
 private:
  ...
  // Returns content hashes are trying to load them.
  // Invalid verified_contents.json will be removed from disk.
  ContentHash LoadVerifiedContents();
  ...
  void DoneCheckingForVerifiedContents(ContentHash content_hash);
  ...
  void ReadCompleteAfterWrite(ContentHash content_hash);
  ...
  // Note: This is read from multiple threads/sequences.
...
ContentHash ContentHashFetcherJob::LoadVerifiedContents() {
  // Will delete invalid verified contents file.
  const bool only_read_verified_contents = false;
...
void ContentHashFetcherJob::DoneCheckingForVerifiedContents(
  ...
  if (IsCancelled())
    return;
  bool found_verified_contents = content_hash.has_verified_contents();
  if (found_verified_contents) {
...
void ContentHashFetcherJob::OnVerifiedContentsWritten(size_t expected_size,
  ...
  base::PostTaskAndReplyWithResult(
      GetExtensionFileTaskRunner().get(), FROM_HERE,
      // TODO(lazyboy): Consider creating VerifiedContents directly from
      // |response| instead of reading the file later again.
      base::BindOnce(&ReadContentHashAfterVerifiedContentsWritten,
                     extension_key_, force_, WrapRefCounted(this)),
      base::BindOnce(&ContentHashFetcherJob::ReadCompleteAfterWrite, this));
}

void ContentHashFetcherJob::ReadCompleteAfterWrite(ContentHash content_hash) {
  if (IsCancelled())
    return;
