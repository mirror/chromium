...
class TestEventRouter : public EventRouter {
 public:
  ...
  class EventObserver {
   public:
    ...
    virtual void OnBroacastEvent(const Event& event);
  ...
  TestEventRouter(content::BrowserContext* context);
  ...
  void set_expected_extension_id(const std::string& extension_id) {
 ...
 private:
  ...
  // If set, the extension dispatched events are expected to be sent to.
