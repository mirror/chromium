...
void MimeHandlerViewService::RegisterMimeHandlerViewFrame(
  ...
  frames_[frame_tree_node_id].original_frame_routing_id =
...
bool MimeHandlerViewService::MaybeDeferNavigation(
  ...
  bool registered_frame = frames_.count(node_id) != 0;
  if (!util::ForPDFExtension(handle->GetURL())) {
    // the frame associated with MimeHandlerView is navigating somewhere else.
    // Simply clear the corresponding state.
    if (registered_frame)
  ...
  if (!registered_frame) {
    // TODO(ekaramad): Could it happen that frame registration request never
    // arrive? IT might be best to make the registration synchronous on the same
    // associated channel as that of frame navigation. Despite sending a request
    // for frame registration before navigation, there is still no guarantee
    // that RegisterMimeHandlerViewFrame is called before getting here.
    if (handle->IsInMainFrame()) {
      // Omnibox navigation to PDF extension. This does not require any
      // coordination with an embedder process since none exist.
      return false;
    }
  ...
  bool already_navigating =
      frames_[node_id].stream && (frames_[node_id].navigation_handle ||
                                  frames_[node_id].did_finish_navigation);
...
void MimeHandlerViewService::DidFinishNavigation(
  ...
  // TODO(ekaramad): This DCHECK fires if we navigate window.frames and
  // interestingly, that navigation never reaches MaybeDeferNavigation to
  // properly clean things up.
  DCHECK_EQ(frames_[node_id].navigation_handle, handle);
  frames_[node_id].did_finish_navigation = true;
  frames_[node_id].navigation_handle = nullptr;
}

void MimeHandlerViewService::DocumentLoadedInFrame(
    content::RenderFrameHost* render_frame_host) {
  if (render_frame_host->GetParent() != render_frame_host_)
    return;
  if (!util::ForPDFExtension(
          render_frame_host->GetSiteInstance()->GetSiteURL()))
    return;
  int node_id = render_frame_host->GetFrameTreeNodeId();
  DCHECK(frames_[node_id].did_finish_navigation &&
         !frames_[node_id].document_loaded);
  DCHECK(GetRendererInterface());
  GetRendererInterface()->DocumentLoadedInFrame(
      frames_[node_id].original_frame_routing_id);
}
...
MimeHandlerViewService::GetRendererInterface() {
...
MimeHandlerViewService::FrameData::FrameData() {}
