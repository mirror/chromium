...
void MimeHandlerViewService::RegisterMimeHandlerViewFrame(
    ...
    const RegisterMimeHandlerViewFrameCallback& callback) {
  int process_id = render_frame_host_->GetProcess()->GetID();
  content::RenderFrameHost* navigating_frame =
      content::RenderFrameHost::FromID(process_id, navigating_frame_routing_id);
  if (!navigating_frame) {
    callback.Run(false);
    return;
  }
  ...
  callback.Run(rendeer_should_request_resource);
}
