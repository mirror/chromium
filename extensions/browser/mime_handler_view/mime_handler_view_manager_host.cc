...
void MimeHandlerViewManagerHost::OnInterfaceRequestFromFrame(
  ...
  int32_t node_id = render_frame_host->GetFrameTreeNodeId();
  if (!navigation_helpers_.count(node_id))
    return;

  registry_map_[node_id].TryBindInterface(interface_name, interface_pipe);
}
...
void MimeHandlerViewManagerHost::AddMimeHandlerServiceInterface(
  ...
  DCHECK(registry_map_.count(frame_tree_node_id));
