...
class MimeHandlerViewManagerHost
 ...
 private:
  ...
  struct ControllerInfo {
    ...
    // The original resource URL to render inside the extension.
    std::string GetResourceURLString() const;

    // A GUID which uniquely identifies the StreamContainer to be used by the
    // controller. It also helps identifying the controller on the renderer
    // side.
  ...
  void ReleaseStream(int32_t routing_id, const std::string& stream_id) override;
  void DropStream(const std::string& stream_id) override;

 private:
  void OnStreamAborted(int32_t frame_tree_node_id,
