...
class MimeHandlerViewManagerHost
 ...
 public:
  ...
  bool InterceptNavigation(
      MimeHandlerViewExtensionNavigationThrottle* throttler);
  void OnNavigationThrottleDestroyed(
 ...
 private:
  ...
  struct NavigationHelper {
   public:
    ...
    // A reference to the NavigationThrottle. We need this to either Resume or
    // Cancel the navigation after deferring it to wait for the stream.
    MimeHandlerViewExtensionNavigationThrottle* navigation_throttle = nullptr;
    // Original PDF resource URL.
 ...
 private:
  ...
  void RemoveMimeHandlerServiceInterface(int frame_tree_node_id);
  ...
  std::unordered_map<int32_t, NavigationHelper> navigation_helpers_;
  ...
  service_manager::BinderRegistry registry_for_manager_host_;
