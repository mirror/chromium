...
MimeHandlerViewExtensionNavigationThrottle::
    ~MimeHandlerViewExtensionNavigationThrottle() {
  content::RenderFrameHost* parent_frame = content::RenderFrameHost::FromID(
      parent_frame_process_id_, parent_frame_routing_id_);
  if (!parent_frame)
    return;
  MimeHandlerViewManagerHost::Get(parent_frame)
      ->OnNavigationThrottleDestroyed(this);
}
...
MimeHandlerViewExtensionNavigationThrottle::WillStartRequest() {
  ...
  parent_frame_process_id_ = parent_frame->GetProcess()->GetID();
  parent_frame_routing_id_ = parent_frame->GetRoutingID();
