...
TEST_F(NativeRendererMessagingServiceTest, ValidateMessagePort) {
  ...
  EXPECT_FALSE(messaging_service()->HasPortForTesting(script_context, port_id));
  ...
  messaging_service()->CreatePortForTesting(script_context, "channel", port_id);
  ...
  EXPECT_CALL(*ipc_message_sender(), SendCloseMessagePort(0, port_id, false))
      .Times(0);
  messaging_service()->ValidateMessagePort(*script_context_set(), port_id,
                                           nullptr);
  ::testing::Mock::VerifyAndClearExpectations(ipc_message_sender());
...
TEST_F(NativeRendererMessagingServiceTest, OpenMessagePort) {
  ...
  external_connection_info.source_url = GURL("http://example.com");
  ...
  ASSERT_TRUE(messaging_service()->HasPortForTesting(script_context, port_id));

  EXPECT_EQ("true", GetStringPropertyFromObject(context->Global(), context,
                                                "eventFired"));
  EXPECT_EQ(R"({"frameId":0,"tab":{"tabId":10},"url":"http://example.com"})",
...
TEST_F(NativeRendererMessagingServiceTest, DeliverMessageToPort) {
  ...
  const char kPort2Message[] = "port2Message";
  {
    ...
    ASSERT_FALSE(add_on_message_listener.IsEmpty());
    ASSERT_TRUE(add_on_message_listener->IsFunction());
    v8::Local<v8::Value> args[] = {port1.ToV8()};
    ASSERT_FALSE(args[0].IsEmpty());
...
TEST_F(NativeRendererMessagingServiceTest, SendMessageFromJS) {
  v8::HandleScope handle_scope(isolate());
  v8::Local<v8::Context> context = MainContext();
