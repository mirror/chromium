...
void MimeHandlerViewFrameController::DidFinishLoading(double finish_time) {
  ...
  if (should_send_view_id_)
    manager_->DidReceiveViewId(id_);
  if (loader_)
    loader_.reset(nullptr);
  should_send_view_id_ = false;
}
...
void MimeHandlerViewFrameController::ResetURL(const GURL& resource_url) {
  if (loader_) {
    // We should cancel any ongoing request and start over.
    loader_->Cancel();
    loader_.reset(nullptr);
  }
  view_id_ = "";
