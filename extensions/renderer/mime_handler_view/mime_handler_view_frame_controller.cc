...
void MimeHandlerViewFrameController::DidFinishLoading(double finish_time) {
  ...
  if (GetPluginFrame()) {
    manager_->DidReceiveStreamId(this);
  } else {
...
void MimeHandlerViewFrameController::ReplaceResource(const GURL& resource_url) {
  resource_url_ = resource_url;
  if (loader_) {
    loader_->Cancel();
    loader_.reset(nullptr);
...
void MimeHandlerViewFrameController::DidAbortStream() {
  DCHECK(!stream_id_.empty());
  stream_id_.clear();
  RequestResource();
}
...
void MimeHandlerViewFrameController::PostMessage(v8::Isolate* isolate,
  ...
  v8::Local<v8::Object> window_proxy = GetPluginFrame()->GlobalProxy();
...
blink::WebFrame* MimeHandlerViewFrameController::GetPluginFrame() const {
