...
MimeHandlerViewManager* MimeHandlerViewManager::Get(
  ...
  if (!g_web_local_frame_map_.Get().count(owner_frame))
    g_web_local_frame_map_.Get()[owner_frame] = new MimeHandlerViewManager();
...
MimeHandlerViewManager::MimeHandlerViewManager() : binding_(this) {}
...
void MimeHandlerViewManager::ResetParams(HandlerId id,
                                         const blink::WebURL& original_url,
                                         const blink::WebString& mime_type) {
  DCHECK(frame_controller_map_.count(id));
  if (original_url == blink::WebURL(frame_controller_map_[id]->resource_url()))
    return;
  frame_controller_map_[id]->ResetURL(original_url);
}
...
void MimeHandlerViewManager::DidPauseNavigation(int32_t render_frame_routing_id,
  ...
  if (id == kHandlerIdNone) {
    ...
    id = blink::WebMimeHandlerViewManager::GetHandlerId(
