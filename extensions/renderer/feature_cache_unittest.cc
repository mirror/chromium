...
TEST_F(FeatureCacheTest, Basic) {
  ...
  // should.
  EXPECT_EQ(
      0u, cache.GetAvailableFeatures(script_context_a.get()).count("storage"));
  EXPECT_EQ(
      1u, cache.GetAvailableFeatures(script_context_b.get()).count("storage"));
  ...
  EXPECT_EQ(
      0u, cache.GetAvailableFeatures(script_context_a.get()).count("storage"));
  EXPECT_EQ(
      0u, cache.GetAvailableFeatures(script_context_b.get()).count("storage"));
