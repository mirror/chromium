...
void ScriptInjectionManager::OnInjectionFinished(
  ...
  if (!last_injection_finish_time_.is_null() && elapsed)
    UMA_HISTOGRAM_TIMES("Extensions.TimeYieldedBetweenContentScriptRuns",
                        now - last_injection_finish_time_ - *elapsed);
  last_injection_finish_time_ = now;
  auto iter =
...
void ScriptInjectionManager::InjectScripts(
  ...
  last_injection_finish_time_ = base::TimeTicks();
