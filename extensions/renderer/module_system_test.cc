    ...
    RouteFunction(
        ...
        base::Bind(
            [](ScriptContext* context,
              ...
              if (bindings_system) {
                api =
                    bindings_system->GetAPIObjectForTesting(context, api_name);
              } else {
                ...
                CHECK(full_binding
                          ->Get(context->v8_context(),
                                gin::StringToSymbol(context->isolate(),
                                                    "binding"))
                          .ToLocal(&api_value))
                    << "Failed to get: " << api_name;
...
ModuleSystemTestEnvironment::ModuleSystemTestEnvironment(
  ...
  if (FeatureSwitch::native_crx_bindings()->IsEnabled()) {
    bindings_system_ = base::MakeUnique<NativeExtensionBindingsSystem>(
        NativeExtensionBindingsSystem::SendRequestIPCMethod(),
        NativeExtensionBindingsSystem::SendEventListenerIPCMethod());
  }
