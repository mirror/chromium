...
class TestIPCSender : public IPCMessageSender {
 public:
  TestIPCSender() {}
  ~TestIPCSender() override {}
  ...
  void SendRemoveUnfilteredLazyEventListenerIPC(
      ScriptContext* context,
      const std::string& event_name) override {}
  void SendAddFilteredEventListenerIPC(
      ScriptContext* context,
      const std::string& event_name,
      const base::DictionaryValue& filter,
      bool is_lazy) override {}
  void SendRemoveFilteredEventListenerIPC(
      ScriptContext* context,
      const std::string& event_name,
      const base::DictionaryValue& filter,
...
class GetAPINatives : public ObjectBackedNativeHandler {
 public:
  GetAPINatives(ScriptContext* context,
    ...
    RouteFunction("get",
                  base::Bind([](ScriptContext* context,
                     NativeExtensionBindingsSystem* bindings_system,
      ...
      if (bindings_system) {
        ...
        CHECK(context->module_system()->Require(api_name).ToLocal(
                  &full_binding)) << "Failed to get: " << api_name;
        v8::Local<v8::Value> api_value;
        CHECK(full_binding->Get(context->v8_context(), gin::StringToSymbol(context->isolate(), "binding")).ToLocal(&api_value)) << "Failed to get: " << api_name;
...
ModuleSystemTestEnvironment::ModuleSystemTestEnvironment(
  ...
  {
    auto context = base::MakeUnique<ScriptContext>(context_holder_->context(),
                                   nullptr,  // WebFrame
                                   nullptr,  // Extension
  ...
  if (FeatureSwitch::native_crx_bindings()->IsEnabled()) {
    bindings_system_ =
        base::MakeUnique<NativeExtensionBindingsSystem>(
            base::MakeUnique<TestIPCSender>());
  }
