...
void ModuleSystem::RegisterNativeHandler(
  ...
  // Without native bindings, we initialize handlers immediately. Otherwise, we
  // initialize them lazily on first access.
...
v8::MaybeLocal<v8::Object> ModuleSystem::RequireNativeFromString(
  ...
  // Initialize native handlers lazily on first access if native bindings are
  // enabled.
  if (base::FeatureList::IsEnabled(features::kNativeCrxBindings) &&
