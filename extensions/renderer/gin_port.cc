...
void GinPort::PostMessageHandler(gin::Arguments* arguments,
  ...
  // JSON.stringify can either fail (with unserializable objects) or can return
  // undefined. If it returns undefined, the v8 API then coerces it to the
  // string value "undefined". Throw an error if we were passed unserializable
  // objects.
  if (!success ||
      std::strcmp(*v8::String::Utf8Value(stringified), "undefined") == 0) {
