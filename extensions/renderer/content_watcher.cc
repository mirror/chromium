...
void NotifyBrowserOfChange(blink::WebLocalFrame* changed_frame);
...
class FrameContentWatcher
 ...
 public:
  FrameContentWatcher(content::RenderFrame* render_frame,
                      blink::WebVector<blink::WebString> css_selectors);
  ...
  const std::set<std::string> matching_selectors() {
    return matching_selectors_;
  }
...
void FrameContentWatcher::OnDestruct() { delete this; }
...
void FrameContentWatcher::DidMatchCSS(
  ...
  NotifyBrowserOfChange(render_frame()->GetWebFrame());
}
...
void NotifyBrowserOfChange(blink::WebLocalFrame* changed_frame) {
  ...
  for (blink::WebFrame* frame = top_frame; frame;
    ...
    if (frame->IsWebLocalFrame() &&
      ...
      FrameContentWatcher* watcher =
          FrameContentWatcher::Get(content::RenderFrame::FromWebFrame(frame->ToWebLocalFrame()));
      if (watcher && !watcher->matching_selectors().empty()) {
        transitive_selectors.insert(watcher->matching_selectors().begin(),
                                    watcher->matching_selectors().end());
      }
    }
  }
