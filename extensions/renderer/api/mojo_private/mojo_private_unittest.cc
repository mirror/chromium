...
class MojoPrivateApiTest : public ApiTestBase {
 public:
  ...
  scoped_refptr<const Extension> CreateExtension() override {
    // Use an extension that's whitelisted for the mojoPrivate API.
    ...
    return ExtensionBuilder().SetManifest(std::move(manifest)).SetID("pkedcjkdefgpdelpbcmbmeomcjbeemfm").Build();
  }
