...
TEST_F(APIRequestHandlerTest, ErrorInCallback) {
  v8::HandleScope handle_scope(isolate());
  v8::Local<v8::Context> context = MainContext();
  ...
  logged_error.reset();
  v8::Local<v8::Function> more_fun_callback =
      FunctionFromString(context, "(function() { throw {message: 'hello'} })");
  request_id = request_handler.AddPendingRequest(context, more_fun_callback);
  request_handler.CompleteRequest(request_id, base::ListValue(), std::string());
  EXPECT_FALSE(try_catch.HasCaught());
  ASSERT_TRUE(logged_error);
  EXPECT_EQ("Error handling response: Uncaught #<Object>", *logged_error);

  logged_error.reset();
  v8::Local<v8::Function> most_fun_callback = FunctionFromString(
      context,
      "(function() { throw { toString: function() { throw 'goodbye'; } }; })");
  request_id = request_handler.AddPendingRequest(context, most_fun_callback);
  request_handler.CompleteRequest(request_id, base::ListValue(), std::string());
  EXPECT_FALSE(try_catch.HasCaught());
  ASSERT_TRUE(logged_error);
  EXPECT_EQ("Error handling response: Uncaught [object Object]", *logged_error);
}
