...
// A helper method to run all listeners and collect and return the results.
// This is abstracted out in order to run this function when script is not
// suspended.
void RunListenersHelper(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  // Note: This is safe because the ExceptionHandler should be valid as long as
  // the context is, which we checked above.
  ...
  for (uint32_t i = 0; i < length; ++i) {
    ...
    // (through e.g. calling alert() or print()). Theoretically, that will
    // suspend this message loop as well (though a nested message loop will
    // run). This is a bit ugly, but should hopefully be safe.
...
void EventEmitter::DispatchImpl(v8::Local<v8::Context> context,
  ...
  if (callback) {
    // If there's a callback that needs the response of dispatching the event
    // to the listeners, we have to handle this a bit differently. We can't just
    // fire-and-forget, since the response is important. Additionally, we can't
    // just pass the |callback| to the JSRunner, because there are (potentially)
    // multiple listeners. Instead, create a helper function to dispatch the
    // event to the listeners and return the value, and pass that function to
    // the JSRunner. We curry information about the listeners and arguments, as
    // well as the ExceptionHandler, into the helper function.
    v8::HandleScope handle_scope(isolate);
  ...
  for (const auto& listener : listeners) {
    ...
    // TODO(devlin): This try-catch won't help if JS runs asynchronously. Should
    // we leverage the above implementation to always catch errors properly?
    if (try_catch.HasCaught()) {
