...
bool ArgumentSpec::ParseArgumentToFundamental(
  ...
  switch (type_) {
    ...
    case ArgumentType::STRING: {
      ...
      if (out_value) {
        if (str.empty()) {  // We may have converted above for an enum check.
          // We already checked that this is a string, so this should never
          // fail.
...
bool ArgumentSpec::ParseArgumentToObject(
  ...
  // We can convert to a new v8 object that only has the specified properties
  // and avoids getters and setters only if it doesn't allow for additional,
