...
// Certain argument types will be passed through directly to the v8 arguments
// because we won't be able to parse them properly.
TEST_F(ArgumentSpecUnitTest, TestV8ValuePassedThrough) {
  ...
  auto test_is_same_value = [this, context](ArgumentType type,
                                            const char* value_str) {
    SCOPED_TRACE(value_str);
    ArgumentSpec spec(type);
    ...
    EXPECT_TRUE(value_out->Equals(context, value_in).ToChecked());
  };

  // Functions are passed directly because we reply directly to the passed-in
  // function.
  test_is_same_value(ArgumentType::FUNCTION, "(function() {})");
  // 'Any' and 'Binary' arguments are passed directly because we can't know if
  // we can copy them safely, accurately, and efficiently.
  test_is_same_value(ArgumentType::ANY, "({foo: 'bar'})");
  test_is_same_value(ArgumentType::BINARY, "(new ArrayBuffer())");
}
