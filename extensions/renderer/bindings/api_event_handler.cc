...
void APIEventHandler::FireEventInContext(
  ...
  if (massager_iter == data->massagers.end()) {
    ...
    {
      // Massagers expect an array of v8 values. This conversion is potentially
      // visible to script, but we'd be dispatching the event anyway, so it's
      // not a security risk. If anything wonky happens, bail out.
      v8::TryCatch try_catch(isolate);
      for (size_t i = 0; i < arguments->size(); ++i) {
        v8::Maybe<bool> success =
            args_array->CreateDataProperty(context, static_cast<uint32_t>(i),
                                           arguments->at(i));
