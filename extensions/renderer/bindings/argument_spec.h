...
class ArgumentSpec {
 public:
  ...
  // parsed value into v8_out_value is that for defined objects, the result will
  // be a data object with no getters/setters, so clients don't need to worry
  // about re-checking the arguments. Note that this will *not* apply in the
  // case of ArgumentType::ANY, ArgumentType::BINARY, or ArgumentType::FUNCTION,
  // where we do not copy the (arbitrary) contents.
