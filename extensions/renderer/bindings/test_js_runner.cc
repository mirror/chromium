...
TestJSRunner::Scope::Scope(std::unique_ptr<JSRunner> runner)
    : runner_(std::move(runner)), old_runner_(JSRunner::Get()) {
  DCHECK_NE(runner_.get(), old_runner_);
  JSRunner::SetThreadInstance(runner_.get());
}

TestJSRunner::Scope::~Scope() {
  DCHECK_EQ(runner_.get(), JSRunner::Get());
  JSRunner::SetThreadInstance(old_runner_);
}
...
TestJSRunner::TestJSRunner(const base::Closure& did_call_js)
    : did_call_js_(did_call_js) {}
TestJSRunner::~TestJSRunner() {}
