...
void NativeRendererMessagingService::ClosePort(
    v8::Local<v8::Context> context,
    const PortId& port_id,
    int routing_id) {
  ScriptContext* script_context =
...
gin::Handle<GinPort> NativeRendererMessagingService::CreatePortForTesting(
    ScriptContext* script_context,
    const PortId& port_id) {
  return CreatePort(script_context, port_id);
}
...
const base::UnguessableToken& NativeRendererMessagingService::GetContextId(
...
void NativeRendererMessagingService::DispatchOnConnectToListeners(
  ...
  if (extension) {
    if (!source->tab.empty() && !extension->is_platform_app()) {
      sender_builder.Set(
          "tab",
          content::V8ValueConverter::Create()->ToV8Value(&source->tab,
                                                         context));
    }
    ...
    if (info.guest_process_id != content::ChildProcessHost::kInvalidUniqueID) {
      ...
      sender_builder.Set("guestProcessId", info.guest_process_id)
                    .Set("guestRenderFrameRoutingId",
                         info.guest_render_frame_routing_id);
    }
  }

  v8::Local<v8::Value> sender = sender_builder.Build();
  gin::Handle<GinPort> port = CreatePort(script_context, target_port_id);
  ...
  bindings_system_->api_system()->event_handler()->FireEventInContext(
      event_name, context, &args, nullptr);
}
...
void NativeRendererMessagingService::DispatchOnDisconnectToListeners(
  ...
  if (!error_message.empty()) {
    bindings_system_->api_system()->request_handler()->last_error()->SetError(context, error_message);
  }
  port->DispatchOnDisconnect(context);
  if (!error_message.empty()) {
    bindings_system_->api_system()->request_handler()->last_error()->
        ClearError(context, true);
  }
...
gin::Handle<GinPort> NativeRendererMessagingService::CreatePort(
  ...
  gin::Handle<GinPort> port_handle = gin::CreateHandle(
      isolate,
      new GinPort(port_id, routing_id, std::string() /* name? */,
...
gin::Handle<GinPort> NativeRendererMessagingService::GetPort(
  ...
  gin::Converter<GinPort*>::FromV8(
      context->GetIsolate(), data->ports[port_id].Get(isolate), &port);
  CHECK(port);
