...
void NativeExtensionBindingsSystem::OnEventListenerChanged(
  ...
  } else {  // Unfiltered events.
    std::string extension_id = script_context->GetExtensionID();
    if (change == binding::EventListenersChanged::HAS_LISTENERS) {
      ...
      ipc_message_sender_->SendAddUnfilteredEventListenerIPC(
          script_context, extension_id, event_name);
      if (lazy) {
