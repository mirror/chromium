...
RequestResult RuntimeHooksDelegate::HandleGetPackageDirectoryEntryCallback(
  ...
  if (background_page && background_page != script_context->GetRenderFrame() &&
  ...
  v8::MaybeLocal<v8::Value> script_result =
      JSRunner::Get(v8_context)
          ->RunJSFunctionSync(
              get_bind_directory_entry_callback.As<v8::Function>(), v8_context,
              0, nullptr);
  v8::Local<v8::Value> callback;
  if (!script_result.ToLocal(&callback) || !callback->IsFunction()) {
    NOTREACHED();
    // Abort, and consider the request handled.
    return RequestResult(RequestResult::HANDLED);
  }
