...
HRESULT GetUrlFromShellItem(IShellItem* shell_item, base::string16* url) {
  ...
  HRESULT hr = shell_item->BindToHandler(NULL, BHID_DataObject, IID_IDataObject,
...
STDMETHODIMP CommandExecuteImpl::Execute() {
  ...
  base::string16 app_id = ShellUtil::GetBrowserModelId(is_per_user_install);
  ...
  if (launch_scheme_ == INTERNET_SCHEME_FILE &&
    ...
    hr = activation_manager->ActivateApplication(app_id.c_str(), verb_.c_str(),
                                                 AO_NONE, &pid);

  } else {
    AtlTrace("Activating for protocol\n");
    hr = activation_manager->ActivateForProtocol(app_id.c_str(), item_array_,
                                                 &pid);
  }
...
bool CommandExecuteImpl::GetLaunchScheme(base::string16* display_name,
...
HRESULT CommandExecuteImpl::LaunchDesktopChrome() {
  ...
  BOOL ret =
      CreateProcess(chrome_exe_.value().c_str(), &command_line[0], NULL, NULL,
                    FALSE, 0, NULL, NULL, &start_info_, &temp_process_info);
  if (ret) {
...
EC_HOST_UI_MODE CommandExecuteImpl::GetLaunchMode() {
  ...
  if (!FindChromeExe(&chrome_exe) || ShellUtil::GetChromeDefaultStateFromPath(
                                         chrome_exe) != ShellUtil::IS_DEFAULT) {
