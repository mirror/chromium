...
#ifndef WIN10_DELEGATE_EXECUTE_DELEGATE_EXECUTE_OPERATION_H_
#define WIN10_DELEGATE_EXECUTE_DELEGATE_EXECUTE_OPERATION_H_
...
class DelegateExecuteOperation {
 public:
  ...
  const base::string16& relaunch_flags() const { return relaunch_flags_; }

  const base::string16& mutex() const { return mutex_; }

  // Returns the parent process.
  base::Process GetParent() const;

  const base::FilePath& shortcut() const { return relaunch_shortcut_; }
...
#endif  // WIN10_DELEGATE_EXECUTE_DELEGATE_EXECUTE_OPERATION_H_