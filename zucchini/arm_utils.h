...
#ifndef ZUCCHINI_ARM_UTILS_H_
#define ZUCCHINI_ARM_UTILS_H_
...
class ARM32Rel32Parser {
 public:
  ...
  template <typename CODE_T,
            AddrType TYPE,
            CODE_T (ARM32Rel32Parser::*FETCH)(Region::const_iterator) const,
            void (ARM32Rel32Parser::*STORE)(Region::iterator, CODE_T) const,
  ...
  class AddrTraits {
   public:
    ...
    static constexpr CODE_T (ARM32Rel32Parser::*Fetch)(
        Region::const_iterator) const = FETCH;
    static constexpr void (ARM32Rel32Parser::*Store)(Region::iterator,
                                                     CODE_T) const = STORE;
  ...
  inline uint32_t FetchARMCode32(Region::const_iterator cur) const {
    return Region::at<uint32_t>(cur);
  }

  // Fetches the 16-bit THUMB2 instruction |code| at |cur| from image data.
  inline uint16_t FetchTHUMB2Code16(Region::const_iterator cur) const {
    return Region::at<uint16_t>(cur);
  }

  // Fetches the next 32-bit THUMB2 instruction |code| at |cur| from image data.
  inline uint32_t FetchTHUMB2Code32(Region::const_iterator cur) const {
    ...
    return (static_cast<uint32_t>(Region::at<uint16_t>(cur)) << 16) |
           Region::at<uint16_t>(cur + 2);
  }

  // Stores the 32-bit ARM instruction |code| to image data at |cur|.
  inline void StoreARMCode32(Region::iterator cur, uint32_t code) const {
    Region::at<uint32_t>(cur) = code;
  }

  // Stores the 16-bit THUMB2 instruction |code| to image data at |cur|.
  inline void StoreTHUMB2Code16(Region::iterator cur, uint16_t code) const {
    Region::at<uint16_t>(cur) = code;
  }

  // Stores the next 32-bit THUMB2 instruction |code| to image data at |cur|.
  inline void StoreTHUMB2Code32(Region::iterator cur, uint32_t code) const {
    Region::at<uint16_t>(cur) = static_cast<uint16_t>(code >> 16);
    Region::at<uint16_t>(cur + 2) = static_cast<uint16_t>(code & 0xFFFF);
  }
  ...
  inline rva_t GetARMTargetRVAFromDisplacement(arm_disp_t disp) const {
    ...
    // Align down.
    assert(align_by_ != kUnintializedAlignBy);
...
class AArch64Rel32Parser {
 public:
  ...
  inline uint32_t Fetch(Region::const_iterator cur) const {
    return Region::at<uint32_t>(cur);
  }

  inline void Store(Region::iterator cur, uint32_t code) const {
    Region::at<uint32_t>(cur) = code;
  }
  ...
  class AddrTraits {
   public:
    ...
    static constexpr uint32_t (AArch64Rel32Parser::*Fetch)(
        Region::const_iterator) const = &AArch64Rel32Parser::Fetch;
    static constexpr void (AArch64Rel32Parser::*Store)(Region::iterator,
                                                       uint32_t) const =
        &AArch64Rel32Parser::Store;
