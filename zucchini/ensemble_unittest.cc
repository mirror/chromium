...
// first byte read from a given |sub_image|.
class TestSingleProgramDetector : public SingleProgramDetector {
 public:
  TestSingleProgramDetector() {}

  bool Run(Region sub_image) override {
    DCHECK_GT(sub_image.size(), 0U);
    uint8_t v = *sub_image.begin();
    auto it = lut_.find(v);
    if (it == lut_.end())
      return false;
...
TEST(EnsembleTest, ParseImposedMatches) {
  ...
  Region old_image(&old_data[0], old_data.size());
  Region new_image(&new_data[0], new_data.size());

  TestSingleProgramDetector detector;
  detector.Add('W', EXE_TYPE_WIN32_X86);
  detector.Add('w', EXE_TYPE_WIN32_X86);
  detector.Add('E', EXE_TYPE_ELF_X86);
  detector.Add('e', EXE_TYPE_ELF_X86);
  ...
  auto run_test = [&](const std::string& imposed_matches) -> bool {
    ...
    bool ret = EnsembleMatcher::ParseImposedMatches(imposed_matches, old_image,
                                                    new_image, &detector, oss,
                                                    &num_identical, &matches);
    message = oss.str();
    return ret;
  };

  auto run_check = [&](const EnsembleMatcher::Match& match,
                       ExecutableType exe_type, offset_t old_offset,
                       size_t old_size, offset_t new_offset, size_t new_size) {
    EXPECT_EQ(exe_type, match.old_element.exe_type) << prev_imposed_matches;
    EXPECT_EQ(old_image, match.old_element.base_image) << prev_imposed_matches;
    EXPECT_EQ(Region(old_image.begin() + old_offset, old_size),
              match.old_element.sub_image)
        << prev_imposed_matches;
    EXPECT_EQ(exe_type, match.new_element.exe_type) << prev_imposed_matches;
    EXPECT_EQ(new_image, match.new_element.base_image) << prev_imposed_matches;
    EXPECT_EQ(Region(new_image.begin() + new_offset, new_size),
              match.new_element.sub_image)
        << prev_imposed_matches;
  };
  ...
  for (const std::string& imposed_matches :
    ...
    run_check(matches[0], EXE_TYPE_ELF_X86, 6, 4, 2, 3);
    run_check(matches[1], EXE_TYPE_WIN32_X86, 1, 2, 12, 4);
  }
  ...
  run_check(matches[0], EXE_TYPE_WIN32_X86, 1, 2, 12, 4);

  // Single subregion match. We're lax with redundant 0.
  EXPECT_TRUE(run_test("6+04=02+10"));
  EXPECT_EQ(0U, num_identical);
  EXPECT_EQ(1U, matches.size());
  run_check(matches[0], EXE_TYPE_ELF_X86, 6, 4, 2, 10);
  ...
  EXPECT_EQ(0U, num_identical);
  EXPECT_EQ(2U, matches.size());
  run_check(matches[0], EXE_TYPE_WIN32_X86, 1, 1, 12, 1);
  run_check(matches[1], EXE_TYPE_WIN32_X86, 2, 1, 13, 1);

  // Overlap in "old" file is okay.
  EXPECT_TRUE(run_test("1+2=12+2,1+2=14+2"));
  EXPECT_EQ(0U, num_identical);
  EXPECT_EQ(2U, matches.size());
  run_check(matches[0], EXE_TYPE_WIN32_X86, 1, 2, 12, 2);
  run_check(matches[1], EXE_TYPE_WIN32_X86, 1, 2, 14, 2);
