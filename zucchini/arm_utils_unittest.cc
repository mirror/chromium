...
// "Clean slate" |code|s for branch instruction encodings with |disp| = 0.
...
// Rules for |pattern|:
// - Spaces are ignored.
// - '0' and '1' are expectation tokens; mismatch leads to failure..
// - '.' means "don't care".
// - A single letter specifies a single-letter variable.
// - "(var)" specifies multi-letter variable "var".
// - If first letter of a variable is upper case, then it's a singleton bit.
//   If there are multiple occurrences, then uniformity check is applied.
// - If first letter of a variable is lower case, then it spans multiple bits.
...
// Tester for Decide/Encode functions for ARM.
template <class Parser, typename TRAITS>
class ARMRelParserTest {
 public:
  using INT_T = typename TRAITS::code_t;

  ARMRelParserTest() {}

  // For each instruction (with |code_clean_slate| in |instr_list|, and for each
  // |disp| in |good_disp_list|, forms |code| with |encode_fun()| and checks for
  // success. Extracts |disp_out| with |decode_fun()| and verifies that it's the
  // original |disp|. For each (|disp|, |code|) pair, extracts components using
  // SplitBits(), and verifies that components from |toks_list| are identical.
  // For each |disp| in |bad_disp_list|, verifies that |decode_fun_()| fails.
  void Run(const std::string& disp_pattern,
           const std::vector<std::string>& toks_list,
           const std::vector<ARMRelInstruction<INT_T>>& instr_list,
           const std::vector<arm_disp_t>& good_disp_list,
           const std::vector<arm_disp_t>& bad_disp_list) {
    bool (Parser::*encode_fun)(arm_disp_t, INT_T*) = TRAITS::Encode;
    bool (Parser::*decode_fun)(INT_T, arm_disp_t*) = TRAITS::Decode;

    Parser parser(0x1000);  // Dummy 4-byte aligned address.
    for (const ARMRelInstruction<INT_T> instr : instr_list) {
      ...
      for (arm_disp_t disp : good_disp_list) {
        INT_T code = instr.clean_slate_code;
        // Encode |disp| into |code|.
        EXPECT_TRUE((parser.*encode_fun)(disp, &code)) << disp;
        arm_disp_t disp_out;
        ...
        EXPECT_TRUE((parser.*decode_fun)(code, &disp_out));
        EXPECT_EQ(disp, disp_out);

        // Sanity check: Reencode |disp| into |code| which it should already
        // have, ensure no change.
        INT_T code_copy = code;
        EXPECT_TRUE((parser.*encode_fun)(disp, &code));
        EXPECT_EQ(code_copy, code);

        // Encode 0, ensure we get clean slate |code| back.
        EXPECT_TRUE((parser.*encode_fun)(0, &code));
        EXPECT_EQ(instr.clean_slate_code, code);
      }

      for (arm_disp_t disp : bad_disp_list) {
        INT_T code = instr.clean_slate_code;
        EXPECT_FALSE((parser.*encode_fun)(disp, &code)) << disp;
...
TEST(ARM32Rel32Parser, Fetch) {
  std::vector<uint8_t> bytes = {0x10, 0x32, 0x54, 0x76, 0x98, 0xBA, 0xDC, 0xFE};
  Region region(&bytes[0], bytes.size());
  ARM32Rel32Parser parser(0x100);  // Dummy RVA.
  EXPECT_EQ(0x76543210U, parser.FetchARMCode32(region.begin()));
  EXPECT_EQ(0xFEDCBA98U, parser.FetchARMCode32(region.begin() + 4));

  EXPECT_EQ(0x3210U, parser.FetchTHUMB2Code16(region.begin()));
  EXPECT_EQ(0xFEDCU, parser.FetchTHUMB2Code16(region.begin() + 6));

  EXPECT_EQ(0x32107654U, parser.FetchTHUMB2Code32(region.begin()));
  EXPECT_EQ(0xBA98FEDCU, parser.FetchTHUMB2Code32(region.begin() + 4));
}

TEST(ARM32Rel32Parser, Store) {
  ...
  ARM32Rel32Parser parser(0x100);  // Dummy RVA.
  parser.StoreARMCode32(region.begin() + 4, 0x76543210U);
  parser.StoreTHUMB2Code16(region.begin() + 10, 0x8642U);
  parser.StoreTHUMB2Code32(region.begin() + 14, 0xFEDCBA98U);
...
// which in ARM32Rel32Parser comments.
TEST(ARM32Rel32Parser, EncodeDecode) {
  // A24 tests.
  ARMRelParserTest<ARM32Rel32Parser, ARM32Rel32Parser::AddrTraits_A24> test_A24;
  ...
  // T8 tests.
  ARMRelParserTest<ARM32Rel32Parser, ARM32Rel32Parser::AddrTraits_T8> test_T8;
  for (int cond = 0; cond <= 0x0E; ++cond) {
  ...
  // T11 tests.
  ARMRelParserTest<ARM32Rel32Parser, ARM32Rel32Parser::AddrTraits_T11> test_T11;
  ...
  // T21 tests.
  ARMRelParserTest<ARM32Rel32Parser, ARM32Rel32Parser::AddrTraits_T21> test_T21;
  for (int cond = 0; cond <= 0x0E; ++cond) {
  ...
  // T24 tests.
  ARMRelParserTest<ARM32Rel32Parser, ARM32Rel32Parser::AddrTraits_T24> test_T24;
  ...
  // For BLX encoding T2, |disp| must be product of 4.
...
TEST(ARM32Rel32Parser, Main) {
  // ARM mode (32-bit).
  // 00103050: 00 01 02 EA    B     00183458 ; B encoding A1.
  {
    rva_t instr_rva = 0x00103050U;
    ARM32Rel32Parser parser(instr_rva);
    std::vector<uint8_t> bytes = {0x00, 0x01, 0x02, 0xEA};
    Region region(&bytes[0], bytes.size());
    uint32_t code = parser.FetchARMCode32(region.begin());
    EXPECT_EQ(0xEA020100U, code);
    arm_disp_t disp = 0;
    EXPECT_TRUE(parser.DecodeA24(code, &disp));
    EXPECT_EQ(0x00080400, disp);
    rva_t target_rva = parser.GetARMTargetRVAFromDisplacement(disp);
  ...
  // 001030A2: F3 E7          B     0010308C ; B encoding T2.
  {
    ...
    Region region(&bytes[0], bytes.size());
    uint32_t code = parser.FetchTHUMB2Code16(region.begin());
    EXPECT_EQ(0xE7F3U, code);  // Sii iiiiiiii = 111 11110011 = -1101 = -0x0D.
    arm_disp_t disp = 0;
    EXPECT_TRUE(parser.DecodeT11(code, &disp));
    EXPECT_EQ(-0x0000001A, disp);  // -0x0D * 2 = -0x1A.
    rva_t target_rva = parser.GetTHUMB2TargetRVAFromDisplacement(disp);
  ...
  // 001030A2: 00 F0 01 FA    BL    001034A8 ; BL encoding T1.
  {
    ...
    Region region(&bytes[0], bytes.size());
    uint32_t code = parser.FetchTHUMB2Code32(region.begin());
    EXPECT_EQ(0xF000FA01U, code);
    arm_disp_t disp = 0;
    EXPECT_TRUE(parser.DecodeT24(code, &disp));
...
TEST(ARM32Rel32Parser, BLXComplication) {
  ...
  run_test(0x001030A0U,  // Multiple of 4.
           {0x01, 0xF0, 0x06, 0xB0}, 0xF001B006U, 0x0040100C,
  ...
  run_test(0x001030A0U,  // Multiple of 4.
           {0x01, 0xF0, 0x06, 0xE0}, 0xF001E006U, 0x0040100C,
...
TEST(AArch64Rel32Parser, FetchStore) {
  std::vector<uint8_t> bytes = {0x10, 0x32, 0x54, 0x76, 0x98, 0xBA, 0xDC, 0xFE};
  std::vector<uint8_t> expected = {0xAB, 0x33, 0x22, 0x11,
                                   0x69, 0x5A, 0xFF, 0x00};
  Region region(&bytes[0], bytes.size());
  AArch64Rel32Parser parser(0x100);  // Dummy RVA.
  EXPECT_EQ(0x76543210U, parser.Fetch(region.begin()));
  EXPECT_EQ(0xFEDCBA98U, parser.Fetch(region.begin() + 4));

  parser.Store(region.begin(), 0x112233ABU);
  parser.Store(region.begin() + 4, 0x00FF5A69);
  EXPECT_EQ(expected, bytes);
}

TEST(AArch64Rel32Parser, EncodeDecode) {
  // Immd14 tests.
  ARMRelParserTest<AArch64Rel32Parser, AArch64Rel32Parser::AddrTraits_Immd14>
      test_immd14;
  ...
  // Immd19 tests.
  ARMRelParserTest<AArch64Rel32Parser, AArch64Rel32Parser::AddrTraits_Immd19>
      test_immd19;
  for (int cond = 0; cond <= 0x0E; ++cond) {
  ...
  // Immd26 tests.
  ARMRelParserTest<AArch64Rel32Parser, AArch64Rel32Parser::AddrTraits_Immd26>
      test_immd26;
...
// Typical usage in |target_rva| extraction.
TEST(AArch64Rel32Parser, Main) {
  // 00103050: 02 01 02 14    B     00183458
  rva_t instr_rva = 0x00103050U;
  AArch64Rel32Parser parser(instr_rva);
  std::vector<uint8_t> bytes = {0x02, 0x01, 0x02, 0x14};
  Region region(&bytes[0], bytes.size());
  uint32_t code = parser.Fetch(region.begin());
  EXPECT_EQ(0x14020102U, code);
  arm_disp_t disp = 0;
  EXPECT_TRUE(parser.DecodeImmd26(code, &disp));
  EXPECT_EQ(0x00080408, disp);
  rva_t target_rva = parser.GetTargetRVAFromDisplacement(disp);
  // 0x00103050 + 0 + 0x00080408.
  EXPECT_EQ(0x00183458U, target_rva);
  EXPECT_EQ(disp, parser.GetDisplacementFromTargetRVA(target_rva));
}
