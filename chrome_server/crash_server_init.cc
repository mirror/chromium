...
#include "chrome_server/crash_server_init.h"
...
bool IsRunningSystemInstall() {
  ...
  GetModuleFileName(reinterpret_cast<HMODULE>(&__ImageBase), exe_path,
                    _countof(exe_path));
...
std::unique_ptr<google_breakpad::ExceptionHandler> InitializeCrashReporting() {
  ...
  return std::unique_ptr<google_breakpad::ExceptionHandler>(
