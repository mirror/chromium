...
bool LowLevelKeyboardProc::LockKeys(PlatformWindowDelegate* callback) {
  callback_ = callback;
  if (hook_ == NULL) {
...
bool LowLevelKeyboardProc::UnlockKeys(PlatformWindowDelegate* callback) {
...
bool LowLevelKeyboardProc::ProcessKeyEvent(WPARAM w_param,
  ...
  event.set_flags(event.flags() | EF_RESERVED);
  callback_->DispatchEvent(&event);
