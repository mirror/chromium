...
Layer::Layer()
    : type_(LAYER_TEXTURED),
      ...
      background_blur_sigma_(0),
      background_blur_tile_mode_(cc::FilterOperation::CLAMP),
...
std::unique_ptr<Layer> Layer::Clone() const {
  ...
  // Background filters.
  clone->SetBackgroundBlur(background_blur_sigma_, background_blur_tile_mode_);
...
void Layer::SetBackgroundBlur(int blur_sigma,
                              cc::FilterOperation::BlurTileMode tile_mode) {
  background_blur_sigma_ = blur_sigma;
  background_blur_tile_mode_ = tile_mode;
...
void Layer::SetLayerBackgroundFilters() {
  ...
  if (background_blur_sigma_) {
    filters.Append(cc::FilterOperation::CreateBlurFilter(
        background_blur_sigma_, background_blur_tile_mode_));
  }
