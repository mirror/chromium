...
Compositor::~Compositor() {
  ...
  if (context_factory_private_) {
    auto* manager = context_factory_private_->GetFrameSinkManagerHost();
    for (auto& client : child_frame_sinks_) {
      DCHECK(client.is_valid());
      manager->UnregisterFrameSinkHierarchy(frame_sink_id_, client, false);
    }
...
void Compositor::AddFrameSink(const cc::FrameSinkId& frame_sink_id) {
  if (!context_factory_private_)
    return;
  context_factory_private_->GetFrameSinkManagerHost()
      ->RegisterFrameSinkHierarchy(frame_sink_id_, frame_sink_id, false);
