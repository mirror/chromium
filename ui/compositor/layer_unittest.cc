...
// Tests that in deferring paint request, the layer still can update UI damage.
TEST_F(LayerWithNullDelegateTest, UpdateDamageWhenDeferringPaint) {
  ...
  EXPECT_EQ(gfx::Rect(), root->damaged_region_since_last_send_for_testing());
  EXPECT_EQ(gfx::Rect(), root->deferred_paint_region_for_testing());
  WaitForCommit();
  ...
  // Deferring paint.
  root->SetPaintDeferred(true);

  // During deferring paint request, invalid_rect can be set to
  // cc_layer_->inputs_->update_rect, but the paint_region is empty.
  gfx::Rect bound1(gfx::Rect(100, 100));
  root->SchedulePaint(bound1);
  EXPECT_EQ(gfx::Rect(), root->damaged_region_for_testing());
  EXPECT_EQ(bound1, root->damaged_region_since_last_send_for_testing());
  EXPECT_EQ(bound1, root->deferred_paint_region_for_testing());

  root->SendDamagedRects();
  EXPECT_EQ(bound1, root->cc_layer_for_testing()->update_rect());
  EXPECT_EQ(gfx::Rect(), root->damaged_region_since_last_send_for_testing());
  EXPECT_EQ(bound1, root->deferred_paint_region_for_testing());
  ...
  // During deferring paint request, a new invalid_rect can be set to
  // cc_layer_->inputs_->update_rect, but the paint_region is still empty.
  gfx::Rect bound2(gfx::Rect(100, 200, 100, 100));
  gfx::Rect bound_union(bound1);
  bound_union.Union(bound2);
  root->SchedulePaint(bound2);
  EXPECT_EQ(gfx::Rect(), root->damaged_region_for_testing());
  EXPECT_EQ(bound2, root->damaged_region_since_last_send_for_testing());
  EXPECT_EQ(bound_union, root->deferred_paint_region_for_testing().bounds());

  root->SendDamagedRects();
  EXPECT_EQ(bound2, root->cc_layer_for_testing()->update_rect());
  EXPECT_EQ(gfx::Rect(), root->damaged_region_since_last_send_for_testing());
  EXPECT_EQ(bound_union, root->deferred_paint_region_for_testing().bounds());
  ...
  root->SetPaintDeferred(false);
  // The invalidation region should be accumulated invalid_rect during deferring
