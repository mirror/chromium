...
void LayerAnimator::RemoveAndDestroyOwnedObserver(
  ...
  class IsEqual {
   public:
    explicit IsEqual(ImplicitAnimationObserver* animation_observer)
        : animation_observer_(animation_observer) {}
    bool operator()(const std::unique_ptr<ImplicitAnimationObserver>& other) {
      return (other.get() == animation_observer_);
    }

   private:
    const ImplicitAnimationObserver* animation_observer_;
  };
  owned_observer_list_.erase(
      std::remove_if(owned_observer_list_.begin(), owned_observer_list_.end(),
