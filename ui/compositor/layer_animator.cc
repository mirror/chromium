...
LayerAnimator::LayerAnimator(base::TimeDelta transition_duration)
  ...
  // created via use of LayerAnimationElement. We mark the player as
  // one that will change animating properties ahead of actual need so
  // that it will report true for the various property specific
  // has-potentially-animating method checks which, in turn, lead to
  // creation of transform/effect nodes for the involved layer.
