...
GetKeyboardTopRowLayout(const base::FilePath& device_path) {
  ...
  auto layout = device::UdevDeviceGetPropertyValue(device.get(),
      kLayoutProperty);
  if (layout.empty())
    return EventRewriterChromeOS::kKbdTopRowLayoutDefault;

  int layoutId;
  if (!base::StringToInt(layout, &layoutId)) {
    LOG(WARNING) << "Failed to parse " << kLayoutProperty
                 << " value '" << layout << "'";
    return EventRewriterChromeOS::kKbdTopRowLayoutDefault;
  }
  if (layoutId < EventRewriterChromeOS::kKbdTopRowLayoutMin ||
      layoutId > EventRewriterChromeOS::kKbdTopRowLayoutMax) {
    LOG(WARNING) << "Unknown layout '" << layout << "'";
    return EventRewriterChromeOS::kKbdTopRowLayoutDefault;
  }
  return static_cast<EventRewriterChromeOS::KeyboardTopRowLayout>(layoutId);
}
...
EventRewriterChromeOS::KeyboardDeviceAddedForTesting(
  ...
  return KeyboardDeviceAddedInternal(device_id, device_name,
                                     kUnknownVendorId, kUnknownProductId,
                                     layout);
}
...
void EventRewriterChromeOS::DeviceKeyPressedOrReleased(int device_id) {
  auto iter = device_id_to_info_.find(device_id);
  DeviceType type;
...
void EventRewriterChromeOS::RewriteFunctionKeys(const ui::KeyEvent& key_event,
  ...
  if ((state->key_code >= ui::VKEY_F1) && (state->key_code <= ui::VKEY_F12)) {
    ...
    if (top_row_keys_are_function_keys == search_is_pressed) {
      ...
      // This is the original ChromeOS layout.
      ...
      KeyboardTopRowLayout layout;
      if (iter != device_id_to_info_.end()) {
        layout = iter->second.topRowLayout;
      } else {
        layout = kKbdTopRowLayoutDefault;
      }

      const KeyboardRemapping *mapping;
      size_t mappingSize;
      switch (layout) {
