...
// and //chrome provides helpers for working with accelerator combinations but
// we cannot depend on them here. In addition to mapping a KeyboardShortcutItem
// to an accelerator, the key + modifiers combination provides information to
// populate the values of |replacement_codes| field in KeyboardShortcutItem, in
// order to show the key and modifiers in the viewer with different styles.
...
struct KSV_EXPORT KeyboardShortcutItem {
  ...
  // A string resource id. The associated string describes what the keyboard
  // shortcut can do.
  int description_string_id;

  // A string resource id. The associated string describes what the shortcut
  // keys are.
  ...
  // |replacement_codes| provides information how to display the key and
  // modifiers involved in the shortcut string.
  // Because the |shortcut_string_id| associated string could have placeholders
  // in the string resource, it can be replaced by texts for the modifiers and
  // key, which values are contained in |replacement_codes|. The representation
  // of a key could be a text or an icon. The corresponding texts or icon will
  // be determined by the values of ui::KeyboardCode.
  // For example of shortcut "Alt + left arrow", |replacement_codes| will be
