...
void SetupSearchIllustrationView(views::View* illustration_view,
  ...
  image_view->SetImage(
      gfx::CreateVectorIcon(icon, SkColorSetARGBMacro(0xFF, 0xCA, 0xCA, 0xCA)));
  image_view->SetImageSize(gfx::Size(125, 125));
  illustration_view->AddChildView(image_view);
  views::Label* text = new views::Label(l10n_util::GetStringUTF16(message_id));
  text->SetEnabledColor(SkColorSetARGBMacro(0xFF, 0x20, 0x21, 0x24));
...
views::Widget* KeyboardShortcutView::Show(gfx::NativeWindow context) {
  if (g_ksv_view) {
    ...
    views::Widget::CreateWindowWithContextAndBounds(
        new KeyboardShortcutView(), context, gfx::Rect(0, 0, 768, 512));
...
void KeyboardShortcutView::InitViews() {
  ...
  search_results_container_->SetBorder(
      views::CreateEmptyBorder(gfx::Insets(0, 96, 0, 96)));
  AddChildView(search_results_container_);

  // Init views of KeyboardShortcutItemView.
  std::vector<KeyboardShortcutItemView*> shortcut_views;
  for (const auto& item : GetKeyboardShortcutItemList())
    for (auto category : item.categories)
      shortcut_views.emplace_back(new KeyboardShortcutItemView(item, category));
  std::sort(shortcut_views.begin(), shortcut_views.end(),
  ...
  for (auto* item_view : shortcut_views) {
    ShortcutCategory category = item_view->category();
    if (current_category != category) {
      current_category = category;
      item_list_view = new KeyboardShortcutItemListView();
      tabbed_pane_->AddTab(GetStringForCategory(current_category),
                           item_list_view);
      tabbed_pane_contents_.emplace_back(item_list_view);
    }
...
void KeyboardShortcutView::Layout() {
  const int kSearchBoxTopPadding = 8;
  const int kSearchBoxBottomPadding = 16;
  gfx::Rect search_box_bounds(search_box_view_->GetPreferredSize());
  search_box_bounds.set_x((width() - search_box_bounds.width()) / 2);
  ...
  content_view->SetBounds(0, search_box_used_height, width(),
                          height() - search_box_used_height);
}
...
void KeyboardShortcutView::QueryChanged(search_box::SearchBoxViewBase* sender) {
  ...
  base::i18n::FixedPatternStringSearchIgnoringCaseAndAccents finder(
      new_contents);
  KeyboardShortcutItemListView* found_items_list_view =
      new KeyboardShortcutItemListView();
  for (auto* item_list_view : tabbed_pane_contents_) {
    ...
    for (int i = 0; i < item_views->child_count(); ++i) {
      ...
      if (finder.Search(description_text, nullptr, nullptr) ||
        ...
        found_items_list_view->AddItemView(new KeyboardShortcutItemView(
            *item_view->shortcut_item(), category));
      }
    }
  }
  search_results_container_->AddChildView(
      found_items_list_view->shortcut_item_views()->has_children()
          ? found_items_list_view
          : search_no_result_view_.get());
  Layout();
  SchedulePaint();
}
...
int KeyboardShortcutView::GetCategoryNumberForTests() const {
  return tabbed_pane_contents_.size();
}
