...
void KeyboardShortcutView::InitViews() {
  ...
  scroller->SetContents(search_start_view_.get());
  search_results_container_->AddChildView(scroller);
...
void KeyboardShortcutView::Layout() {
  ...
  search_box_bounds.set_x((rect.width() - search_box_bounds.width()) / 2);
  search_box_bounds.set_y(rect.y() + kSearchBoxTopPadding);
  ...
  content_view->SetBounds(rect.x(), rect.y() + search_box_used_height,
                          rect.width(), rect.height() - search_box_used_height);
}
...
void KeyboardShortcutView::QueryChanged(search_box::SearchBoxViewBase* sender) {
  ...
  views::ScrollView* const scroller = CreateScrollView();
  scroller->SetContents(found_items_list_view->has_children()
                            ? found_items_list_view
                            : query_empty ? search_start_view_.get()
