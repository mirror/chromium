...
KeyboardShortcutItemView::KeyboardShortcutItemView(
    ...
    : category_(category), shortcut_item_(&item) {
  description_label_view_ = new views::StyledLabel(
      l10n_util::GetStringUTF16(item.description_message_id), nullptr);
  // StyledLabel will flip the alignment so that |description_label_view_|
  // always aligns to left.
  ...
  // StyledLabel will flip the alignment so that |shortcut_label_view_|
  // always aligns to left.
  ...
  SetBorder(views::CreateEmptyBorder(gfx::Insets(10, 0, 10, 0)));
}
...
void KeyboardShortcutItemView::Layout() {
  ...
  const int top = GetInsets().top();
  description_label_view_->SetBoundsRect(
      gfx::Rect(0, top, description_view_width, height));
  shortcut_label_view_->SetBoundsRect(
      gfx::Rect(description_view_width, top, shortcut_view_width, height));
}
