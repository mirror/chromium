...
// Ratio of the width of |shortcut_label_view_| to the parent view width.
...
int KeyboardShortcutItemView::GetHeightForWidth(int w) const {
  const int shortcut_view_width = w * kShortcutViewWitdhRatio;
  const int description_view_width = w - shortcut_view_width;
  const int shortcut_view_height =
      shortcut_label_view_->GetHeightForWidth(shortcut_view_width);
  const int description_view_height =
      description_label_view_->GetHeightForWidth(description_view_width);
