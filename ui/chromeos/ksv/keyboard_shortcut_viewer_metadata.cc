...
// TODO(https://crbug.com/803502): Provides I18n string for VKEY without US
// keyboard layout mapping (VKEY => DomKey) or VKEY requiring special labels.
base::Optional<bool> GetSpecialStringForKeyboardCode(
  ...
  *key_label = l10n_util::GetStringUTF16(msg_id);
  return true;
}
...
base::string16 GetStringForKeyboardCode(ui::KeyboardCode key_code) {
  ...
  base::string16 key_label;
  if (GetSpecialStringForKeyboardCode(key_code, &key_label))
    return key_label;

  bool has_mapping =
      ui::DomCodeToUsLayoutDomKey(dom_code, 0, &dom_key, &keycode_ignored);
  DCHECK(has_mapping);
