...
constexpr float kSlopDistanceFromEdge = kMaxDistanceFromEdge + 40;
...
TouchExplorationController::TouchExplorationController(
    ...
    : root_window_(root_window),
      ...
      gesture_provider_(std::make_unique<GestureProviderAura>(this, this)),
...
ui::EventRewriteStatus TouchExplorationController::RewriteEvent(
  ...
  if (!exclude_bounds_.IsEmpty()) {
    ...
    if (in_exclude_area) {
      if (state_ == NO_FINGERS_DOWN)
        return EVENT_REWRITE_CONTINUE;
      if (touch_event.type() == ET_TOUCH_MOVED ||
          touch_event.type() == ET_TOUCH_PRESSED) {
  ...
  const EventType type = touch_event.type();
  ...
  if (type == ET_TOUCH_PRESSED) {
  ...
  } else if (type == ET_TOUCH_RELEASED || type == ET_TOUCH_CANCELLED) {
    ...
    if (it == current_touch_ids_.end()) {
      ...
      if (!seen_press_) {
        auto new_event = std::make_unique<TouchEvent>(
            ET_TOUCH_CANCELLED, gfx::Point(), touch_event.time_stamp(),
  ...
  if (gesture_provider_) {
    TouchEvent mutable_touch_event = touch_event;
...
ui::EventRewriteStatus TouchExplorationController::InSingleTapPressed(
  ...
  if (type == ET_TOUCH_RELEASED || type == ET_TOUCH_CANCELLED) {
    ...
    } else if (current_touch_ids_.empty()) {
      SET_STATE(NO_FINGERS_DOWN);
    }
    return EVENT_REWRITE_DISCARD;
  }
  if (type == ET_TOUCH_MOVED) {
...
TouchExplorationController::InSingleTapOrTouchExploreReleased(
  ...
  if (type == ET_TOUCH_RELEASED && anchor_point_state_ == ANCHOR_POINT_NONE) {
...
ui::EventRewriteStatus TouchExplorationController::InTouchReleasePending(
    ...
    std::unique_ptr<ui::Event>* rewritten_event) {
  const EventType type = event.type();
  if (type == ET_TOUCH_PRESSED || type == ET_TOUCH_MOVED)
    return EVENT_REWRITE_DISCARD;
  if (type == ET_TOUCH_RELEASED || type == ET_TOUCH_CANCELLED) {
    if (!current_touch_ids_.empty())
      return EVENT_REWRITE_DISCARD;
...
ui::EventRewriteStatus TouchExplorationController::InTouchExploration(
  ...
  *rewritten_event = CreateMouseMoveEvent(event.location_f(), event.flags());
  last_touch_exploration_ = std::make_unique<TouchEvent>(event);
...
ui::EventRewriteStatus TouchExplorationController::InCornerPassthrough(
  ...
  if (event.pointer_details().id == initial_press_->pointer_details().id) {
    ...
    if (type == ET_TOUCH_MOVED && in_a_bottom_corner)
      return EVENT_REWRITE_DISCARD;
...
ui::EventRewriteStatus TouchExplorationController::InTouchExploreSecondPress(
  ...
  if (type == ET_TOUCH_PRESSED) {
    ...
    // cancelled.
    auto new_event = std::make_unique<TouchEvent>(
        ET_TOUCH_CANCELLED, gfx::Point(), event.time_stamp(),
...
void TouchExplorationController::SendSimulatedClickOrTap() {
  ...
  if (anchor_point_state_ == ANCHOR_POINT_EXPLICITLY_SET) {
    delegate_->HandleAccessibilityGesture(AX_GESTURE_CLICK);
    return;
  }
  SendSimulatedTap();
}
...
void TouchExplorationController::SendSimulatedTap() {
  std::unique_ptr<TouchEvent> touch_press;
  touch_press = std::make_unique<TouchEvent>(
      ET_TOUCH_PRESSED, gfx::Point(), Now(), initial_press_->pointer_details());
  touch_press->set_location_f(anchor_point_);
  touch_press->set_root_location_f(anchor_point_);
  DispatchEvent(touch_press.get());

  auto touch_release =
      std::make_unique<TouchEvent>(ET_TOUCH_RELEASED, gfx::Point(), Now(),
                                   initial_press_->pointer_details());
...
ui::EventRewriteStatus TouchExplorationController::InSlideGesture(
  ...
  if (!(edge & RIGHT_EDGE) && (type != ET_TOUCH_RELEASED)) {
    if (sound_timer_.IsRunning()) {
      sound_timer_.Stop();
    }
    return EVENT_REWRITE_DISCARD;
  ...
  if (!sound_timer_.IsRunning()) {
    sound_timer_.Start(FROM_HERE, kSoundDelay, this,
...
ui::EventRewriteStatus TouchExplorationController::InTwoFingerTap(
  ...
  if (type == ET_TOUCH_MOVED) {
    ...
    const gfx::Point& original_location =
        initial_presses_[event.pointer_details().id];
  ...
  if (type == ET_TOUCH_RELEASED) {
    delegate_->HandleAccessibilityGesture(AX_GESTURE_TAP_2);
    SET_STATE(NO_FINGERS_DOWN);
    return EVENT_REWRITE_DISCARD;
  }
  return EVENT_REWRITE_DISCARD;
...
void TouchExplorationController::OnTapTimerFired() {
  switch (state_) {
    ...
    case TOUCH_EXPLORE_RELEASED:
      SET_STATE(NO_FINGERS_DOWN);
      last_touch_exploration_ = std::make_unique<TouchEvent>(*initial_press_);
      anchor_point_ = last_touch_exploration_->location_f();
      anchor_point_state_ = ANCHOR_POINT_FROM_TOUCH_EXPLORATION;
      return;
    case DOUBLE_TAP_PENDING: {
      ...
      auto passthrough_press = std::make_unique<TouchEvent>(
          ET_TOUCH_PRESSED, gfx::Point(), Now(),
  ...
  EnterTouchToMouseMode();
  std::unique_ptr<Event> mouse_move = CreateMouseMoveEvent(
...
void TouchExplorationController::ProcessGestureEvents() {
  ...
  for (const auto& gesture : gestures) {
    if (gesture->type() == ET_GESTURE_SWIPE && state_ == GESTURE_IN_PROGRESS) {
...
void TouchExplorationController::OnSwipeEvent(ui::GestureEvent* swipe_gesture) {
  ...
  AXGesture gesture = AX_GESTURE_NONE;
  if (event_details.swipe_left()) {
    switch (num_fingers) {
      case 1:
        gesture = AX_GESTURE_SWIPE_LEFT_1;
        break;
      case 2:
        gesture = AX_GESTURE_SWIPE_LEFT_2;
        break;
      case 3:
        gesture = AX_GESTURE_SWIPE_LEFT_3;
        break;
      case 4:
        gesture = AX_GESTURE_SWIPE_LEFT_4;
        break;
      default:
        break;
    }
    ...
    switch (num_fingers) {
      case 1:
        gesture = AX_GESTURE_SWIPE_UP_1;
        break;
      case 2:
        gesture = AX_GESTURE_SWIPE_UP_2;
        break;
      case 3:
        gesture = AX_GESTURE_SWIPE_UP_3;
        break;
      case 4:
        gesture = AX_GESTURE_SWIPE_UP_4;
        break;
      default:
        break;
    }
    ...
    switch (num_fingers) {
      case 1:
        gesture = AX_GESTURE_SWIPE_RIGHT_1;
        break;
      case 2:
        gesture = AX_GESTURE_SWIPE_RIGHT_2;
        break;
      case 3:
        gesture = AX_GESTURE_SWIPE_RIGHT_3;
        break;
      case 4:
        gesture = AX_GESTURE_SWIPE_RIGHT_4;
        break;
      default:
        break;
    }
    ...
    switch (num_fingers) {
      case 1:
        gesture = AX_GESTURE_SWIPE_DOWN_1;
        break;
      case 2:
        gesture = AX_GESTURE_SWIPE_DOWN_2;
        break;
      case 3:
        gesture = AX_GESTURE_SWIPE_DOWN_3;
        break;
      case 4:
        gesture = AX_GESTURE_SWIPE_DOWN_4;
        break;
      default:
        break;
    }
...
void TouchExplorationController::DispatchKeyWithFlags(ui::KeyboardCode key,
                                                      int flags) {
...
TouchExplorationController::CreateMouseMoveEvent(const gfx::PointF& location,
  ...
  auto event = std::make_unique<MouseEvent>(ET_MOUSE_MOVED, gfx::Point(),
                                            gfx::Point(), Now(), flags, 0);
...
void TouchExplorationController::VlogEvent(const ui::TouchEvent& touch_event,
  ...
  VLOG(1) << "\n Function name: " << function_name << "\n Event Type: " << type
          << "\n Location: " << location.ToString()
          << "\n Touch ID: " << touch_id;
  prev_event_ = std::make_unique<TouchEvent>(touch_event);
}
