...
GLImageIOSurfaceEGL::GLImageIOSurfaceEGL(const gfx::Size& size,
  ...
  // Swiftshader on Mac, there's only every one config. Query it from EGL.
  EGLint numConfigs = 0;
  EGLBoolean result =
...
bool GLImageIOSurfaceEGL::BindTexImageImpl(unsigned internalformat) {
  ...
  // in the constructor we're going to be used to bind plane 0 to a texture, or
  // to transform YUV to RGB.
  if (pbuffer_ == EGL_NO_SURFACE) {
