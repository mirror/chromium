...
class GL_EXPORT NativeViewGLSurfaceGLX : public GLSurfaceGLX {
 ...
 private:
  ...
  base::queue<std::unique_ptr<GPUTimer>> free_gpu_timers_;
  using Frame = std::pair<std::unique_ptr<GPUTimer>, PresentationCallback>;
  base::queue<Frame> pending_frames_;

  base::TimeTicks vsync_timebase_;
  base::TimeDelta vsync_interval_;

  bool task_is_pending_ = false;
