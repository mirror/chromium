...
// Only correct for supported formats.
...
bool SupportedFormat(gfx::BufferFormat format) {
  switch (format) {
    case gfx::BufferFormat::RGBA_8888:
    case gfx::BufferFormat::RGBX_8888:
      return true;
...
bool GLImageDXGIHandle::Initialize(HANDLE handle) {
  ...
  if (FAILED(d3d11_device1->OpenSharedResource1(
          handle, IID_PPV_ARGS(texture_.GetAddressOf())))) {
    return false;
  }
  D3D11_TEXTURE2D_DESC desc;
...
bool GLImageDXGIHandle::BindTexImage(unsigned target) {
  if (!texture_ || !keyed_mutex_)
    return false;  // We haven't been properly initialized.
  ...
  HRESULT hrWait = keyed_mutex_->AcquireSync(
      KEY_BIND,
      1000);  // todo: if this fails, emit a GL error, so submit doesn't occur
              // eventually, this should never occur

  if (FAILED(hrWait) || hrWait == WAIT_TIMEOUT || hrWait == WAIT_ABANDONED) {
    DCHECK(false);
  }
...
void GLImageDXGIHandle::ReleaseTexImage(unsigned target) {
  if (!texture_ || !keyed_mutex_)
    return;  // We haven't been properly initialized.
