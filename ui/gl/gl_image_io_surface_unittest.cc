...
class GLImageIOSurfaceTestDelegate : public GLImageTestDelegateBase {
 public:
  ...
  scoped_refptr<GLImage> CreateSolidColorImage(const gfx::Size& size,
    ...
    LOG(ERROR) << reinterpret_cast<const char*>(glGetString(GL_VENDOR));
    LOG(ERROR) << "|is_nvidia| " << is_nvidia;
    ...
    if (format == gfx::BufferFormat::RGBA_8888) {
      ...
      LOG(ERROR) << "|is_nvidia| true and format == BGRX_1010102";

      LOG(ERROR) << "skipping the swizzling";
      ...
      // rdar://36824694.
      corrected_color[0] = color[0];
      corrected_color[1] = color[1];
      corrected_color[2] = color[2];
      corrected_color[3] = color[3];
    } else {
