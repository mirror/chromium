...
#if defined(OS_ANDROID)
  // Android currently doesn't report this extension even when it's present,
  // and older devices may export a useless wrapper function. See
  // crbug.com/775707 for details. In short, if the symbol is present and we're
  // on Android N or newer, assume that it's usable.
...
#else
  DCHECK(!g_egl_android_native_fence_sync_supported);
#endif
