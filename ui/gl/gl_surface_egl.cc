...
bool NativeViewGLSurfaceEGL::Initialize(GLSurfaceFormat format) {
  ...
  // implemenatation. EGL_DISPLAY_PRESENT_TIME_ANDROID support, in particular,
  // is spotty.
...
gfx::SwapResult NativeViewGLSurfaceEGL::SwapBuffers(
  ...
  if (pending_latency_infos_.size() <= framesAgoToGetServerTimestamps) {
    // |latency_info| was just std::move'd from. Make sure to put it in a
    // valid state before returning it to the caller.
    latency_info->clear();
    return gfx::SwapResult::SWAP_ACK;
  }
  ...
  *latency_info = std::move(pending_latency_infos_.front().latency_info);
  ...
  if (!eglGetFrameTimestampsANDROID(GetDisplay(), surface_, queryFrameId,
                                    supported_egl_timestamps_.size(),
                                    &supported_egl_timestamps_[0],
                                    &egl_timestamps[0])) {
