...
class GLImageDXGITestDelegate {
 public:
  scoped_refptr<GLImage> CreateSolidColorImage(const gfx::Size& size,
    ...
    bool rv = image->Initialize(handle);
...
template <gfx::BufferFormat format>
bool ShouldRunTest(GLImageDXGITestDelegate<format>*) {
  D3D_FEATURE_LEVEL level =
      QueryD3D11DeviceObjectFromANGLE()->GetFeatureLevel();
  if (level < D3D_FEATURE_LEVEL_11_1) {
    LOG(WARNING)
        << "GL implementation feature level is too low, skipping test.";
    return false;
  }
  return true;
}
