...
GLImageEGL::QueryDmaBufFormatsWithModifiers() {
  ...
  std::vector<std::pair<int32_t, uint64_t>> supported;
  ...
  for (int i = 0; i < num_formats; i++) {
    ...
    if (num_modifiers == 0)
      supported.push_back(
          std::pair<int32_t, uint64_t>(drm_formats[i], DRM_FORMAT_MOD_INVALID));
    for (int j = 0; j < num_modifiers; j++)
      supported.push_back(
          std::pair<int32_t, uint64_t>(drm_formats[i], drm_modifiers[j]));
