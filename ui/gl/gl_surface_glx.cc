...
void NativeViewGLSurfaceGLX::Destroy() {
  ...
  for (auto& frame : pending_frames_) {
    frame.timer->Destroy(!!gl_context_ /* has_context */);
...
bool NativeViewGLSurfaceGLX::OnMakeCurrent(GLContext* context) {
  CHECK(context == gl_context_ || !gl_context_ || !context);
  if (context == gl_context_)
    return GLSurfaceGLX::OnMakeCurrent(context);

  // If context is changed, we assume SwapBuffers issuesed for previous context
  // will be discarded.
...
void NativeViewGLSurfaceGLX::CheckPendingFrames() {
  ...
  bool error_happened =
      gpu_timing_client_ &&
      gpu_timing_client_->timer_type() == GPUTiming::kTimerTypeDisjoint &&
      !gpu_timing_client_->CheckAndResetTimerErrors();
  if (error_happened || !gpu_timing_client_) {
    // If GPUTimer is not avaliable or some error happened, we just run
