...
void RealGLApi::glReadPixelsFn(GLint x,
  ...
  if (version_->is_es) {
    switch (gl_type) {
      case GL_HALF_FLOAT:
      case GL_HALF_FLOAT_OES: {
        // TODO(zmo): once ANGLE changes its ES3 behavior to always return
        // HALF_FLOAT as IMPLEMENTATION_COLOR_READ_TYPE as other native ES3
        // implementations, we can simply call GetPixelType() here.
        GLint param = 0;
