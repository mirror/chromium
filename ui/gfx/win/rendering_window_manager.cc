...
bool RenderingWindowManager::RegisterChild(HWND parent, HWND child) {
  ...
  // DoSetParentOnChild() was already called for |child|. Call ::SetParent() now
  // but from a worker thread instead of the IO thread. The ::SetParent() call
  // could block the IO thread waiting on the UI thread and deadlock.
  if (info.call_set_parent) {
    base::PostTaskWithTraits(
        FROM_HERE, {base::TaskPriority::USER_BLOCKING},
        base::BindOnce(base::IgnoreResult(&::SetParent), parent, child));
  }
