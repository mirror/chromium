...
#define WDCHECK(statement) DCHECK(statement) << GetLastError()

DirectManipulationHelper DirectManipulationHelper::instance_;
...
HRESULT DirectManipulationHandler::OnViewportStatusChanged(
  ...
  if (current == DIRECTMANIPULATION_READY)
    DirectManipulationHelper::GetInstance()->ResetViewport();
...
HRESULT DirectManipulationHandler::OnContentUpdated(
  ...
  LOG(ERROR) << std::setprecision(2) << "scale=" << xform[0] << "," << xform[3]
