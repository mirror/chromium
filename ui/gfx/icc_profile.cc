...
ICCProfile ICCProfile::FromDataWithId(const void* data_as_void,
  ...
  std::vector<uint8_t> new_profile_data;
  new_profile_data.insert(new_profile_data.begin(), data_as_byte,
                          data_as_byte + size);
...
ColorSpace ICCProfile::GetColorSpace() const {
  ...
  if (internals_->is_parametric_) {
    ...
    color_space = ColorSpace::CreateCustom(internals_->to_XYZD50_,
                                           ColorSpace::TransferID::LUT_BASED);
...
ICCProfile::Internals::Internals(std::vector<uint8_t> data, uint64_t id)
