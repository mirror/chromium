...
void ParamTraits<scoped_refptr<gfx::ICCProfile>>::Write(
    base::Pickle* m,
    const scoped_refptr<gfx::ICCProfile>& p) {
  if (p) {
    WriteParam(m, p->GetData());
  } else {
    std::vector<char> empty_data;
    WriteParam(m, empty_data);
  }
}

bool ParamTraits<scoped_refptr<gfx::ICCProfile>>::Read(
    const base::Pickle* m,
    base::PickleIterator* iter,
    scoped_refptr<gfx::ICCProfile>* r) {
...
void ParamTraits<scoped_refptr<gfx::ICCProfile>>::Log(
    const scoped_refptr<gfx::ICCProfile>& p,
    std::string* l) {
