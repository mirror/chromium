...
bool GLSurfaceDRM::Initialize() {
  native_surface_ = gbm_surface_create(GetNativeDisplay(),
...
bool NativeViewGLSurfaceDRM::SwapBuffers() {
  ...
  if (!gbm_surface_has_free_buffers(native_surface_)) {
    LOG(ERROR) <<  "out of free frame buffers";
    return false;
  }
  ...
  ret = drmModePageFlip(fd_, window_->encoder->crtc_id, next_fb_id_,
