...
TEST(VectorIconUtilMacTest, SamePixels) {
  ...
  NSImage* image = NSImageWithIcon(kSquareIcon, color);
...
TEST(VectorIconUtilMacTest, DefaultSize) {
  NSImage* image = NSImageWithIcon(kSquareIcon, SK_ColorBLACK);
...
TEST(VectorIconUtilMacTest, UsesProvidedSize) {
  ...
  NSImage* image = NSImageWithIcon(kSquareIcon, SK_ColorBLACK, size, false);

  EXPECT_TRUE(NSEqualSizes([image size], size));
}

// Test that when |flip_x| is true, the image is pixel-identiccal to rasterizing
// to ImageSkia and reversing in Cocoa.
TEST(VectorIconUtilMacTest, SamePixelsRTL) {
  ...
  NSImage* image = NSImageWithIcon(kSquareIcon, color, size, true);
