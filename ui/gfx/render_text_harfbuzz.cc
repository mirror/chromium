...
RangeF TextRunHarfBuzz::GetGraphemeBounds(RenderTextHarfBuzz* render_text,
  ...
  if (chars.length() > 1 && render_text->GetGraphemeIterator()) {
    ...
    // provide the bounds of the last one.
    if (before == total)
      --before;
  ...
  for (size_t line_index = 0; line_index < lines().size(); ++line_index) {
    ...
    for (const internal::LineSegment& segment : line.segments) {
      ...
      if (!intersection.is_empty()) {
        ...
        // "ceil" here matches what's done in GetGlyphBounds(), which determines
        // where the cursor is drawn (which doesn't yet consider multiline).
        // Since |line_start_x| is already rounded, ensure the entire line
        // highlights if the span is near the start of a line.
        DCHECK_LE(line_start_x, selected_span.start());
        int start_x = 0;
        if ((selected_span.start() - line_start_x - 1) > FLT_EPSILON)
          start_x = std::ceil(selected_span.start()) - line_start_x;
