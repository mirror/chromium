...
Range RenderTextHarfBuzz::GetCursorSpan(const Range& text_range) {
  ...
  // for the entire cursor jump. This can occur for Unicode Variation Selectors,
  // which specify a glyph variant for the codepoint that precedes it. The
  // variation selector has zero width, so it's safe to ignore the second run.
  // But it's a bug that the variation selector is in the next run.
  // TODO(crbug.com/775404) put the variation selector in the correct run.
