...
Range RenderTextHarfBuzz::GetCursorSpan(const Range& text_range_query) {
  EnsureLayout();
  Range text_range =
      text_range_query.is_reversed()
          ? Range(text_range_query.end(), text_range_query.start())
          : text_range_query;
  const size_t query_index = text_range.start();
  const size_t run_index =
      GetRunContainingCaret(SelectionModel(text_range.start(), CURSOR_FORWARD));
  internal::TextRunList* run_list = GetRunList();
  ...
  for (size_t line_index = 0; line_index < lines().size(); ++line_index) {
    ...
    for (const internal::LineSegment& segment : line.segments) {
      ...
      if (!intersection.is_empty()) {
        ...
        // Note "ceil" here matches what's done in GetCursorSpan().
