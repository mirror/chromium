...
ScopedAndroidHardwareBuffer::ScopedAndroidHardwareBuffer(AHardwareBuffer* buf) {
  DVLOG(2) << __FUNCTION__ << ": FromBuffer constructor, buf=" << buf;
  reset(buf);
}

ScopedAndroidHardwareBuffer::ScopedAndroidHardwareBuffer(
    const ScopedAndroidHardwareBuffer& other) {
  DVLOG(2) << __FUNCTION__ << ": Copy constructor, other=" << &other;
...
ScopedAndroidHardwareBuffer::ScopedAndroidHardwareBuffer(
  ...
  DVLOG(2) << __FUNCTION__ << ": Move constructor, other=" << &other;
...
ScopedAndroidHardwareBuffer& ScopedAndroidHardwareBuffer::operator=(
    const ScopedAndroidHardwareBuffer& other) {
  DVLOG(2) << __FUNCTION__ << ": Copy assignment, other=" << &other;
  if (this == &other)
    return *this;
...
ScopedAndroidHardwareBuffer& ScopedAndroidHardwareBuffer::operator=(
    ScopedAndroidHardwareBuffer&& other) {
  DVLOG(2) << __FUNCTION__ << ": Move assignment, other=" << &other;
  if (this == &other)
    return *this;
...
void ScopedAndroidHardwareBuffer::reset(AHardwareBuffer* buf) {
  Release();
  data_ = buf;
  Acquire();
}
...
AHardwareBuffer* ScopedAndroidHardwareBuffer::get() const {
  return data_;
}
...
ScopedAndroidHardwareBuffer ScopedAndroidHardwareBuffer::CreateFromSpec(
  ...
  return ScopedAndroidHardwareBuffer(ahb);
}
