...
enum class GpuFenceHandleType {
  ...
  LAST = ANDROID_NATIVE_FENCE_SYNC
};
...
struct GFX_EXPORT GpuFenceHandle {
 ...
 private:
  ...
  // For use by GpuFence which takes ownership of a GpuFenceHandle. Don't call
  // this on a handle whose resources were consumed for creating a GLFence or
  // that was passed to IPC.
  void Close();
};
