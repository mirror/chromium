...
bool ColorSpace::operator==(const ColorSpace& other) const {
  ...
  if (transfer_ == TransferID::LUT_BASED) {
...
bool ColorSpace::IsParametric() const {
  return transfer_ != TransferID::LUT_BASED;
}
...
std::string ColorSpace::ToString() const {
  ...
  switch (transfer_) {
    ...
    PRINT_ENUM_CASE(TransferID, LUT_BASED)
...
bool ColorSpace::GetTransferFunction(SkColorSpaceTransferFn* fn) const {
  ...
  switch (transfer_) {
    ...
    // LUT-based transfer functions fall back to sRGB.
    case ColorSpace::TransferID::LUT_BASED:
