...
static HomogeneousCoordinate ProjectHomogeneousPoint(const Transform& transform,
                                                     const PointF& p) {
  SkMScalar z =
...
static HomogeneousCoordinate MapHomogeneousPoint(const Transform& transform,
                                                 const Point3F& p) {
...
static PointF ComputeClippedCartesianPoint2dForEdge(
...
static Point3F ComputeClippedCartesianPoint3dForEdge(
    const HomogeneousCoordinate& h1,
    const HomogeneousCoordinate& h2) {
  // Points h1 and h2 form a line in 4d, and any point on that line can be
  // represented as an interpolation between h1 and h2:
...
static inline bool IsNearlyTheSame(const PointF& lhs, const PointF& rhs) {
  return IsNearlyTheSame(lhs.x(), rhs.x()) && IsNearlyTheSame(lhs.y(), rhs.y());
}

static inline bool IsNearlyTheSame(const Point3F& lhs, const Point3F& rhs) {
  return IsNearlyTheSame(lhs.x(), rhs.x()) &&
         IsNearlyTheSame(lhs.y(), rhs.y()) && IsNearlyTheSame(lhs.z(), rhs.z());
}

static inline void AddVertexToClippedQuad3d(const Point3F& new_vertex,
...
Rect MathUtil::MapEnclosingClippedRect(const Transform& transform,
                                       const Rect& src_rect) {
  if (transform.IsIdentityOrIntegerTranslation()) {
    Vector2d offset(static_cast<int>(transform.matrix().getFloat(0, 3)),
                    static_cast<int>(transform.matrix().getFloat(1, 3)));
    return src_rect + offset;
  }
  RectF mapped_rect = MapClippedRect(transform, RectF(src_rect));

  // ToEnclosingRect crashes if called on a RectF with any NaN coordinate.
  if (std::isnan(mapped_rect.x()) || std::isnan(mapped_rect.y()) ||
      std::isnan(mapped_rect.right()) || std::isnan(mapped_rect.bottom()))
    return Rect();

  return ToEnclosingRect(mapped_rect);
}

RectF MathUtil::MapClippedRect(const Transform& transform,
                               const RectF& src_rect) {
  if (transform.IsIdentityOrTranslation()) {
    Vector2dF offset(transform.matrix().getFloat(0, 3),
...
Rect MathUtil::ProjectEnclosingClippedRect(const Transform& transform,
                                           const Rect& src_rect) {
  if (transform.IsIdentityOrIntegerTranslation()) {
    Vector2d offset(static_cast<int>(transform.matrix().getFloat(0, 3)),
                    static_cast<int>(transform.matrix().getFloat(1, 3)));
  ...
  RectF projected_rect = ProjectClippedRect(transform, RectF(src_rect));
  ...
  return ToEnclosingRect(projected_rect);
}

RectF MathUtil::ProjectClippedRect(const Transform& transform,
                                   const RectF& src_rect) {
  if (transform.IsIdentityOrTranslation()) {
    Vector2dF offset(transform.matrix().getFloat(0, 3),
                     transform.matrix().getFloat(1, 3));
...
Rect MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(
  ...
  if (transform.IsIdentityOrTranslation()) {
    Vector2dF offset(transform.matrix().getFloat(0, 3),
                     transform.matrix().getFloat(1, 3));
    return ToEnclosedRect(RectF(rect) + offset);
  }
  ...
  PointF top_left(hc0.CartesianPoint2d());
  PointF bottom_right(hc1.CartesianPoint2d());
  return ToEnclosedRect(BoundingRect(top_left, bottom_right));
}

bool MathUtil::MapClippedQuad3d(const Transform& transform,
                                const QuadF& src_quad,
                                Point3F clipped_quad[6],
                                int* num_vertices_in_clipped_quad) {
  HomogeneousCoordinate h1 =
      MapHomogeneousPoint(transform, Point3F(src_quad.p1()));
  HomogeneousCoordinate h2 =
      MapHomogeneousPoint(transform, Point3F(src_quad.p2()));
  HomogeneousCoordinate h3 =
      MapHomogeneousPoint(transform, Point3F(src_quad.p3()));
  HomogeneousCoordinate h4 =
      MapHomogeneousPoint(transform, Point3F(src_quad.p4()));
...
RectF MathUtil::ComputeEnclosingRectOfVertices(const PointF vertices[],
                                               int num_vertices) {
  if (num_vertices < 2)
    return RectF();
  ...
  return RectF(PointF(xmin, ymin), SizeF(xmax - xmin, ymax - ymin));
}

RectF MathUtil::ComputeEnclosingClippedRect(const HomogeneousCoordinate& h1,
                                            const HomogeneousCoordinate& h2,
                                            const HomogeneousCoordinate& h3,
                                            const HomogeneousCoordinate& h4) {
  // This function performs clipping as necessary and computes the enclosing 2d
  // RectF of the vertices. Doing these two steps simultaneously allows us
  ...
  if (!something_clipped) {
    QuadF mapped_quad = QuadF(h1.CartesianPoint2d(), h2.CartesianPoint2d(),
...
QuadF MathUtil::MapQuad(const Transform& transform,
                        const QuadF& q,
                        bool* clipped) {
  if (transform.IsIdentityOrTranslation()) {
    QuadF mapped_quad(q);
    mapped_quad += Vector2dF(transform.matrix().getFloat(0, 3),
                             transform.matrix().getFloat(1, 3));
    *clipped = false;
    return mapped_quad;
  }

  HomogeneousCoordinate h1 = MapHomogeneousPoint(transform, Point3F(q.p1()));
  HomogeneousCoordinate h2 = MapHomogeneousPoint(transform, Point3F(q.p2()));
  HomogeneousCoordinate h3 = MapHomogeneousPoint(transform, Point3F(q.p3()));
  HomogeneousCoordinate h4 = MapHomogeneousPoint(transform, Point3F(q.p4()));
  ...
  *clipped = h1.ShouldBeClipped() || h2.ShouldBeClipped() ||
  ...
  return QuadF(h1.CartesianPoint2d(), h2.CartesianPoint2d(),
               h3.CartesianPoint2d(), h4.CartesianPoint2d());
}

PointF MathUtil::MapPoint(const Transform& transform,
                          const PointF& p,
                          bool* clipped) {
  HomogeneousCoordinate h = MapHomogeneousPoint(transform, Point3F(p));
...
PointF MathUtil::ProjectPoint(const Transform& transform,
                              const PointF& p,
                              bool* clipped) {
...
Point3F MathUtil::ProjectPoint3D(const Transform& transform,
                                 const PointF& p,
                                 bool* clipped) {
  HomogeneousCoordinate h = ProjectHomogeneousPoint(transform, p, clipped);
  if (!h.w())
...
RectF MathUtil::ScaleRectProportional(const RectF& input_outer_rect,
                                      const RectF& scale_outer_rect,
                                      const RectF& scale_inner_rect) {
  RectF output_inner_rect = input_outer_rect;
  ...
  Vector2dF top_left_diff =
      scale_inner_rect.origin() - scale_outer_rect.origin();
  Vector2dF bottom_right_diff =
...
Vector2dF MathUtil::ComputeTransform2dScaleComponents(
    const Transform& transform,
...
float MathUtil::ComputeApproximateMaxScale(const Transform& transform) {
  Vector3dF unit(1, 1, 0);
  transform.TransformVector(&unit);
  return std::max(std::abs(unit.x()), std::abs(unit.y()));
}

float MathUtil::SmallestAngleBetweenVectors(const Vector2dF& v1,
                                            const Vector2dF& v2) {
  double dot_product = DotProduct(v1, v2) / v1.Length() / v2.Length();
  // Clamp to compensate for rounding errors.
  dot_product = std::max(-1.0, std::min(1.0, dot_product));
  return static_cast<float>(Rad2Deg(std::acos(dot_product)));
}

Vector2dF MathUtil::ProjectVector(const Vector2dF& source,
                                  const Vector2dF& destination) {
  float projected_length =
      DotProduct(source, destination) / destination.LengthSquared();
  return Vector2dF(projected_length * destination.x(),
                   projected_length * destination.y());
}

std::unique_ptr<base::Value> MathUtil::AsValue(const Size& s) {
...
std::unique_ptr<base::Value> MathUtil::AsValue(const Rect& r) {
...
bool MathUtil::FromValue(const base::Value* raw_value, Rect* out_rect) {
  ...
  *out_rect = Rect(x, y, w, h);
  return true;
}

std::unique_ptr<base::Value> MathUtil::AsValue(const PointF& pt) {
  std::unique_ptr<base::ListValue> res(new base::ListValue());
...
Vector3dF MathUtil::GetXAxis(const Transform& transform) {
  return Vector3dF(transform.matrix().getFloat(0, 0),
                   transform.matrix().getFloat(1, 0),
                   transform.matrix().getFloat(2, 0));
}

Vector3dF MathUtil::GetYAxis(const Transform& transform) {
  return Vector3dF(transform.matrix().getFloat(0, 1),
...
bool MathUtil::IsNearlyTheSameForTesting(const PointF& left,
                                         const PointF& right) {
  return IsNearlyTheSame(left, right);
}

bool MathUtil::IsNearlyTheSameForTesting(const Point3F& left,
                                         const Point3F& right) {
  return IsNearlyTheSame(left, right);
}
