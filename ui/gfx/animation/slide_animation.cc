...
base::TimeDelta SlideAnimation::GetDuration() {
  if (showing_) {
    return base::TimeDelta::FromMilliseconds(static_cast<int>(
        slide_duration_ * (1 - pow(value_current_, dampening_value_))));
  }
  return base::TimeDelta::FromMilliseconds(static_cast<int>(
      slide_duration_ * pow(value_current_, dampening_value_)));
}
