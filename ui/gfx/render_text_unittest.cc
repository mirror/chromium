...
TEST_P(RenderTextTest, ObscuredEmoji) {
  ...
  render_text->SetText(UTF8ToUTF16("\U0001F601y"));
  render_text->Draw(canvas());
  // Test two U+1F4F7 📷 "Camera" characters in a row.
  render_text->SetText(UTF8ToUTF16("\U0001F4F7\U0001F4F7"));
  render_text->Draw(canvas());
}
...
TEST_P(RenderTextTest, ElidedText) {
  ...
  struct {
    ...
    const bool elision_expected;
      ...
      {"0123\U0001D11E\U0001D122", "0123\U0001D11E\u2026", true},
  ...
  for (size_t i = 0; i < arraysize(cases); i++) {
    SCOPED_TRACE(
        base::StringPrintf("Testing cases[%" PRIuS "] '%s'", i, cases[i].text));

    // Compute expected width
    expected_render_text->SetText(UTF8ToUTF16(cases[i].display_text));
    int expected_width = expected_render_text->GetContentWidth();

    base::string16 input = UTF8ToUTF16(cases[i].text);
    ...
    EXPECT_EQ(UTF8ToUTF16(cases[i].display_text),
              render_text->GetDisplayText());
...
TEST_P(RenderTextTest, TruncatedText) {
  ...
  for (size_t i = 0; i < arraysize(cases); i++) {
    ...
    EXPECT_EQ(UTF8ToUTF16(cases[i].text), render_text->text());
    EXPECT_EQ(UTF8ToUTF16(cases[i].display_text), render_text->GetDisplayText())
...
TEST_P(RenderTextHarfBuzzTest, GraphemePositions) {
  ...
  const base::string16 kText3 = UTF8ToUTF16("ab\U0001D11Ecd");
...
TEST_P(RenderTextHarfBuzzTest, FindCursorPosition_GraphemeBoundaries) {
  struct {
    ...
    std::set<size_t> expected_cursor_positions;
      ...
      {UTF8ToUTF16("ab\U0001D11Ecd"), {0, 1, 2, 4, 5, 6}}};
...
TEST_P(RenderTextHarfBuzzTest, Multiline_HorizontalAlignment) {
  constexpr struct {
    ...
    const base::i18n::TextDirection display_text_direction;
      ...
      {"abcdef\nghi", ALIGN_LEFT, base::i18n::LEFT_TO_RIGHT},
      {"abc\ndefghi", ALIGN_LEFT, base::i18n::LEFT_TO_RIGHT},
  ...
  for (size_t i = 0; i < arraysize(kTestStrings); ++i) {
    ...
    if (kTestStrings[i].alignment == ALIGN_LEFT) {
      ...
      EXPECT_EQ(3u, std::min(lines[0].length(), lines[1].length()));
      EXPECT_EQ(6u, std::max(lines[0].length(), lines[1].length()));
      const internal::TextRunList* run_list = GetHarfBuzzRunList();
...
TEST_P(RenderTextHarfBuzzTest, HarfBuzz_BreakRunsByEmoji) {
  ...
  render_text->SetText(UTF8ToUTF16("x\U0001F601y\u2728"));
  test_api()->EnsureLayout();
  EXPECT_EQ(ToString16Vec({"x", "😁", "y", "✨"}), GetRunListStrings());
...
TEST_P(RenderTextHarfBuzzTest, HarfBuzz_BreakRunsByAscii) {
  ...
  render_text->SetText(UTF8ToUTF16("\U0001F431."));
  test_api()->EnsureLayout();
...
TEST_P(RenderTextHarfBuzzTest, HarfBuzz_UniscribeFallback) {
  ...
  font_win->font_ref_->font_name_ = UTF8ToUTF16("\u30e1\u30a4\u30ea\u30aa");
