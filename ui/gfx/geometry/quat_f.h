// Copyright 2017 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef UI_GFX_QUAT_F_
#define UI_GFX_QUAT_F_

#include <string>

#include "ui/gfx/gfx_export.h"

namespace gfx {

class Vector3dF;

class GFX_EXPORT QuatF {
 public:
  constexpr QuatF() = default;
  constexpr QuatF(double x, double y, double z, double w)
      : x_(x), y_(y), z_(z), w_(w) {}
  QuatF(const Vector3dF& axis, double angle);

  constexpr double x() const { return x_; }
  void set_x(double x) { x_ = x; }

  constexpr double y() const { return y_; }
  void set_y(double y) { y_ = y; }

  constexpr double z() const { return z_; }
  void set_z(double z) { z_ = z; }

  constexpr double w() const { return w_; }
  void set_w(double w) { w_ = w; }

  QuatF operator+(const QuatF& q) const {
    return {q.x_ + x_, q.y_ + y_, q.z_ + z_, q.w_ + w_};
  }

  QuatF operator*(const QuatF& q) const {
    return {w_ * q.x_ + x_ * q.w_ + y_ * q.z_ - z_ * q.y_,
            w_ * q.y_ - x_ * q.z_ + y_ * q.w_ + z_ * q.x_,
            w_ * q.z_ + x_ * q.y_ - y_ * q.x_ + z_ * q.w_,
            w_ * q.w_ - x_ * q.x_ - y_ * q.y_ - z_ * q.z_};
  }

  QuatF inverse() const { return {-x_, -y_, -z_, w_}; }

  // Blends with the given quaternion, |q|, via spherical linear interpolation.
  QuatF Slerp(const QuatF& q, double t) const;

  // Blends with the given quaternion, |q|, via linear interpolation. This is
  // rarely what you want. Use only if you know what you're doing.
  QuatF Lerp(const QuatF& q, double t) const;

  std::string ToString() const;

 private:
  double x_ = 0.0;
  double y_ = 0.0;
  double z_ = 0.0;
  double w_ = 1.0;
};

inline QuatF operator*(const QuatF& q, double s) {
  return QuatF(q.x() * s, q.y() * s, q.z() * s, q.w() * s);
}

inline QuatF operator*(double s, const QuatF& q) {
  return QuatF(q.x() * s, q.y() * s, q.z() * s, q.w() * s);
}

}  // namespace gfx

#endif  // UI_GFX_QUAT_F_
