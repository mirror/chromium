...
QuatF::QuatF(const Vector3dF& axis, float theta) {
...
QuatF QuatF::Lerp(const QuatF& q, float t) const {
  return ((1.0f - t) * *this) + (t * q);
}

// Taken from http://www.w3.org/TR/css3-transforms/.
QuatF QuatF::Slerp(const QuatF& q, float t) const {
  double dot = x_ * q.x_ + y_ * q.y_ + z_ * q.z_ + w_ * q.w_;

  // Clamp dot to -1.0 <= dot <= 1.0.
  dot = std::min(std::max(dot, -1.0), 1.0);

  if (std::abs(dot - 1.0) < kEpsilon)
    return *this;

  // TODO(vmpstr): In case the dot is -1, the vectors are exactly opposite
