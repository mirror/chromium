...
Rect RenderText::GetCursorBounds(const SelectionModel& caret,
  ...
  bool at_boundary;
  if (insert_mode) {
    // Index 0 is always a boundary. The end, however, is not at a boundary when
    // the string ends in RTL text and there is any LTR text preceding it.
    at_boundary = caret_pos == 0 ||
                  (caret_pos == text().length() &&
                   base::i18n::GetLastStrongCharacterDirection(text()) !=
