...
void RenderText::MoveCursor(BreakType break_type,
  ...
  SelectionModel cursor =
      ResolveDirectedCursor(direction, selection_behavior == SELECTION_NONE);
...
void RenderText::SetSelectionModel(const SelectionModel& model,
  ...
  DCHECK(!key_directed || has_directed_selection_);
  has_directed_selection_ = key_directed;
}
...
SelectionModel RenderText::ResolveDirectedCursor(
    VisualCursorDirection direction,
    bool clearing_selection) {
  SelectionModel end(cursor_position(), selection_model_.caret_affinity());
  if (has_directed_selection_ && !clearing_selection)
    return end;
...
bool RenderText::UpdateCursor(const SelectionModel& model, bool key_directed) {
  ...
  SetSelectionModel(sel, key_directed);
  return changed;
}
