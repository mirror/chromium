...
void RenderText::UpdateForceLTRs() {
  ...
  // optimization. This might not be worth it, depending on how complex the
  // below iteration becomes.
  ...
  LOG(ERROR) << "Special URL rendering for \"" << text_ << "\"; "
             << force_ltrs_.breaks().size() << " breaks.";
}
