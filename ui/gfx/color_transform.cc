...
class ColorTransformFromLinear : public ColorTransformPerChannelTransferFn {
 public:
...
#else
                "  mat4 tmp = mat4(-7.5, 0.5, -5.5, 2.5, 4.5, -3.5, 6.5,\n"
                "                  -1.5, -4.5, 3.5,-6.5,1.5, 7.5, -0.5,\n"
                "                  5.5, -2.5) / 16.0 / 255.0;\n"
                " v += tmp[x_mod][y_mod];\n"
#endif
                "  return v;\n";
  ...
  void AppendShaderSource(std::stringstream* hdr,
                          std::stringstream* src,
                          size_t step_index) const override {
    if (transfer_ == ColorSpace::TransferID::PSEUDO_HDR) {
      // Soft clip luminance
      *src << "  vec3 y_dir = vec3(0.299, 0.587, 0.114);\n"
              "  float luminance = dot(y_dir, color) / 5.0;\n"
              "  float knee = 0.5;\n"
              "  if (luminance > (1.0 - knee)) {\n"
              "    float l2 = 1.0 - exp((1.0 - knee - luminance) / knee) * "
              "knee;\n"
              "    color *= l2 / luminance;\n"
              "  }\n";
...
#if 0
      // Clip to white
      *src << "  float maximum = max(max(color.x, color.y), color.z);\n"
              "  if (maximum > 5.0) {\n"
              "    color /= maximum;\n"
              "    maximum /= 5.0;\n"
              "    float l = dot(color, y_dir);\n"
              "    float w = l * (maximum - 1.0) / (1.0 - l);\n"
              "    color = vec3(w) + color * (1.0 - w);\n"
              "    color *= 5.0;\n"
              "  }";
#else
      // Clip to white
      *src << "  float maximum = max(max(color.x, color.y), color.z);\n"
              ...
              "    maximum /= 5.0;\n"
              "    float l = dot(color, y_dir);\n"
              "    color += vec3((1.0 - 1.0 / maximum) * l);\n"
              "    color *= 5.0;\n"
              "  }";
#endif
    }
    ColorTransformPerChannelTransferFn::AppendShaderSource(hdr, src,
                                                           step_index);
  }
