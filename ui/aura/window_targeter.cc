...
bool WindowTargeter::EventLocationInsideBounds(
  ...
  if (!GetHitTestRects(window, &mouse_rect, &touch_rect))
    return false;
  mouse_rect.Offset(-window->bounds().x(), -window->bounds().y());
  touch_rect.Offset(-window->bounds().x(), -window->bounds().y());
  if (window->parent()) {
    Window::ConvertRectToTarget(window, window->parent(), &mouse_rect);
    Window::ConvertRectToTarget(window, window->parent(), &touch_rect);
  }
