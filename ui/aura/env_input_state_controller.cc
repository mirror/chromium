...
void EnvInputStateController::SetLastMouseLocation(
  ...
  if (client) {
    ...
    LOG(ERROR) << "EISC::SetLastMouseLocation -> converting " << location_in_root.ToString() << " to " << location_in_screen.ToString();
    Env::GetInstance()->set_last_mouse_location(location_in_screen);
  } else {
    LOG(ERROR) << "EISC::SetLastMouseLocation -> blankly setting " << location_in_root.ToString();
