...
WindowTreeHostMus* WindowTreeClient::GetWindowTreeHostForDisplayId(
    int64_t display_id) {
  if (!window_manager_delegate_)
    return nullptr;
...
void WindowTreeClient::OnWindowInputEvent(uint32_t event_id,
  ...
  // Is the event location sent here not transformed correctly? When zoomed in,
  // having the cursor on the left/right side of the web contents area, there's
  // a zone outside of it where to the left the x() location is negative and
  // the target is the RWHVA.
  if (event->IsLocatedEvent()) {
    ui::LocatedEvent* l = event->AsLocatedEvent();
    LOG(ERROR) << " ----- ";
    LOG(ERROR) << "Dispatching loc=" << l->location().ToString()
               << ", root=" << l->root_location().ToString()
               << " to host=" << GetWindowTreeHostMus(window)  << ", window="
