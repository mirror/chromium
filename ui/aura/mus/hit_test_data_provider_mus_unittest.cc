...
TEST_F(HitTestDataProviderMusTest, Stacking) {
  auto hit_test_data = hit_test_provider()->GetHitTestData();
  Window* expected_order_1[] = {window3(), window4(), window2()};
  EXPECT_EQ(arraysize(expected_order_1), hit_test_data->regions.size());
  int i = 0;
  for (const auto& region : hit_test_data->regions) {
    EXPECT_EQ(region->flags,
    ...
    EXPECT_EQ(region->window_id, server_id(expected_order_1[i]));
  ...
  EXPECT_EQ(arraysize(expected_order_2), hit_test_data->regions.size());
  i = 0;
  for (const auto& region : hit_test_data->regions) {
    EXPECT_EQ(region->flags,
              viz::mojom::kHitTestMouse | viz::mojom::kHitTestTouch);
    ...
    EXPECT_EQ(region->surface_id, surface_id);
    EXPECT_EQ(region->window_id, server_id(expected_order_2[i]));
    EXPECT_EQ(region->rect.ToString(),
...
TEST_F(HitTestDataProviderMusTest, CustomTargeter) {
  ...
  for (const auto& region : hit_test_data->regions) {
    ...
    EXPECT_EQ(region->surface_id, surface_id);
    EXPECT_EQ(region->window_id, server_id(expected_windows[i]));
