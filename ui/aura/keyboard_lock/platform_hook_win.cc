...
// static
PlatformHookWin* me = nullptr;

PlatformHookWin::PlatformHookWin(KeyEventFilter* filter)
    : PlatformHook(filter),
      platform_key_event_filter(filter) {
  DCHECK(me == nullptr);
  me = this;
}
...
void PlatformHookWin::Register(const std::vector<NativeKeyCode>& codes,
  ...
  registered_keys_ += codes;
  if (registered_keys_ == codes) {
...
LRESULT PlatformHookWin::ProcessKeyEvent(int code,
  ...
  if (code == 0 && me->platform_key_event_filter_.OnKeyEvent(msg)) {
    return 1;
  }
