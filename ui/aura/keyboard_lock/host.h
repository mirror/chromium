...
class Host {
 public:
  // see //ui/events/keycodes/dom/keycode_converter.h for the meaning of
  // NativeKeyCode.
  using NativeKeyCode = int;

  Host() : runner_(base::ThreadTaskRunnerHandle::Get()),
           active_key_event_filter(tracker_),
           platform_hook_(&active_key_event_filter) {
    DCHECK(runner_);
  }
  ...
  void Reserve(const CLIENT_ID& client,
    ...
    StateKeeper* state_keeper = collection_.Find(client);
    if (state_keeper) {
  ...
  void Reserve(const CLIENT_ID& client,
    ...
    Reserve(client, native_codes, on_result);
  }

  void ClearReserved(const CLIENT_ID& client,
                     base::Callback<void(bool)> on_result) {
    // TODO(zijiehe): Should we also notify cooresponding |observer|?
    if (!runner_->BelongsToCurrentThread()) {
      if (!runner_->PostTask(FROM_HERE, base::BindOnce(
              &Host::ClearReserved,
              base::Unretained(this),
 ...
 private:
  ...
  const scoped_refptr<SingleThreadTaskRunner> runner_;
