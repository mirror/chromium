...
class AURA_EXPORT WindowOcclusionTracker : public ui::LayerAnimationObserver,
 ...
 private:
  ...
  // root of |window| as dirty. Then, recomputes occlusion states of dirty roots
  // if not in the scope of a ScopedPauseOcclusionTracking. |predicate| is not
  // evaluated if the root of |window| is already dirty when this is called.
  ...
  // Returns true if changing the transform, bounds or stacking order if
