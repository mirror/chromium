...
class AURA_EXPORT WindowOcclusionTracker : public ui::LayerAnimationObserver,
 ...
 private:
  ...
  void RecomputeWindowOcclusionStatesImpl(
      Window* window,
  ...
  // Adds |window| to |animated_windows_| and observe animations in |window| if
  // its bounds or transform are animated.
  void MaybeObserveAnimatedWindow(Window* window);
  ...
  // If the root of |window| is in |root_windows_| and not dirty, returns true
  // and sets |dirty| to |root_windows_[window->GetRootWindow()]|.
  bool RootWindowIsNotDirty(Window* window, bool** dirty);
  ...
  // Returns true if window occlusion states should be recomputed when |window|
  // changes.
  bool ShouldRecomputeOcclusionStatesWhenWindowChanges(Window* window) const;
  ...
  // Windows which are being animated.
  //
  ...
  // aborted. Occlusion is recomputed when a window is removed from this set.
