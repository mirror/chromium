...
void WindowOcclusionTracker::MarkRootWindowAsDirtyAndMaybeRecomputeOcclusionIf(
  ...
  // after Window::AddChild() has added |window| to a new root but before it has
  // called OnWindowAddedToRootWindow() on |this|. In that case, do nothing here
  // and rely on OnWindowAddedToRootWindow() to mark the new root as dirty.
  if (root_window_state_it == root_windows_.end()) {
