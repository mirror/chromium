...
// Returns true if |window| is completely opaque.
bool IsOpaque(Window* window) {
...
void WindowOcclusionTracker::Track(Window* window) {
  ...
  if (root_window) {
    RootWindowState& root_window_state = tracker->root_windows_[root_window];
    ++root_window_state.num_tracked_windows;
    tracker->ScheduleRecomputeWindowOcclusionStates(root_window);
  }
}
...
bool WindowOcclusionTracker::IsTracked(Window* window) const {
...
void WindowOcclusionTracker::OnWindowAdded(Window* new_window) {
  ScheduleRecomputeWindowOcclusionStates(new_window->GetRootWindow());
}

void WindowOcclusionTracker::OnWillRemoveWindow(Window* window) {
  ScheduleRecomputeWindowOcclusionStates(window->GetRootWindow());
}
