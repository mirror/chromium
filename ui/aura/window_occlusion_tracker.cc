...
void WindowOcclusionTracker::MaybeRecomputeWindowOcclusionStates(
    Window* root_window) {
  if (num_changing_bounds_or_destroying_) {
...
void WindowOcclusionTracker::RemoveObserverFromWindowAndDescendants(
    Window* window) {
  if (WindowIsTracked(window) ||
      base::ContainsKey(windows_being_destroyed_, window)) {
...
void WindowOcclusionTracker::OnWindowBoundsWillChange(
  ...
  ++num_changing_bounds_or_destroying_;
}
...
void WindowOcclusionTracker::OnWindowDestroying(Window* window) {
  ++num_changing_bounds_or_destroying_;
  windows_being_destroyed_.insert(window);
}
...
void WindowOcclusionTracker::OnWindowDestroyed(Window* window) {
  ...
  windows_being_destroyed_.erase(window);
