...
class AURA_EXPORT Window : public ui::LayerDelegate,
 ...
 public:
  ...
  enum class OcclusionState {
    ...
    UNKNOWN,
    // The window or one of its descendants IsVisible() and:
    // - Its bounds aren't completely covered by fully opaque windows, or,
    // - Its transform, bounds or opacity is animated.
    NOT_OCCLUDED,
    // The window and all its descendants are either !IsVisible() or:
    // - Have bounds are completely covered by fully opaque windows, and,
    // - Have no transform, bounds or opacity animation.
    OCCLUDED,
    // Note: A window is "fully opaque" if:
