...
TEST_F(WindowOcclusionTrackerTest,
  ...
  // of its animator (after |observer|).
  window->layer()->SetOpacity(0.5f);
  ...
  // Complete animations on the window. |observer| should delete the window. If
  // WindowOcclusionTracker is notified that the animation ended and tries to
  // access the window, a crash will occur.
  window->layer()->GetAnimator()->StopAnimating();
}
