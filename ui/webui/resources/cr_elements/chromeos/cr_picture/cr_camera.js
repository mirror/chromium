 ...
 * @const
 */
var CAPTURE_INTERVAL = 1000 / 10;

/**
 * Duration of camera capture (milliseconds).
 * @const
 */
var CAPTURE_DURATION = 1000;
...
Polymer({
  ...
  takePhoto: function() {
    if (!this.cameraOnline_)
      return;
    var captureSize = this.captureSize_();
    var canvas =
    ...
    var interval = setInterval(function() {
      ...
      capturedFrames.push(photoDataUrl);
      if (!this.videoMode_ ||
          capturedFrames.length >= (CAPTURE_DURATION / CAPTURE_INTERVAL)) {
        ...
        if (capturedFrames.length == 1) {
          var photoDataUrl = capturedFrames[0];
        ...
        this.writeUInt32_(IHDR, captureSize.width, 8);
        this.writeUInt32_(IHDR, captureSize.height, 12);
        this.writeUInt8_(IHDR, PNG_BIT_DEPTH, 16);
        this.writeUInt8_(IHDR, PNG_COLOUR_TYPE, 17);
        ...
        this.writeUInt32_(acTL, (capturedFrames.length * 2) - 2, 8);
        this.writeUInt32_(acTL, 0, 12);
        this.writeUInt32_(acTL, this.getCRC_(acTL, 4, 16), 16);
        chunks.push(acTL);

        /** Append captured frames. */
        var sequenceNumber = 0;
        for (var i = 0; i < capturedFrames.length; ++i) {
          sequenceNumber = this.appendFrame_(
              chunks, capturedFrames[i], sequenceNumber, i == 0);
        }
        for (var i = capturedFrames.length - 2; i > 0; --i) {
          sequenceNumber =
              this.appendFrame_(chunks, capturedFrames[i], sequenceNumber);
        }
        ...
        var photoDataUrl = 'data:image/png;base64,' +
            btoa(chunks
                     .map(function(chunk) {
  ...
  onTapFlipPhoto_: function() {
    ...
    this.$.userImageStreamCrop.classList.toggle('flip-x', this.isFlipped_);
    this.$.flipPhoto.classList.toggle('flip-x', this.isFlipped_);
  ...
  onTapSwitchMode_: function() {
    this.videoMode_ = !this.videoMode_;
    this.$.userImageStreamCrop.classList.toggle('video-mode', this.videoMode_);
    this.$.switchMode.classList.toggle('video-mode', this.videoMode_);
    this.$.takePhoto.classList.toggle('video-mode', this.videoMode_);
  },
   ...
   * @param {int} offset Offset in buffer to read UInt32 at.
   ...
   * @param {int} offset Offset in buffer to read string at.
   ...
   * @param {bytes} bytes Array of bytes to be written.
   * @param {int} offset Offset in buffer to write bytes at.
   * @private
   */
   ...
   * @param {int} u8 UInt8 to be written.
   * @param {int} offset Offset in buffer to write UInt8 at.
   * @private
   */
   ...
   * @param {int} u16 UInt16 to be written.
   * @param {int} offset Offset in buffer to write UInt16 at.
   * @private
   */
   ...
   * @param {int} u32 UInt32 to be written.
   * @param {int} offset Offset in buffer to write UInt32 at.
   * @private
   */
   ...
   * @param {int} offset Offset in buffer to write string at.
   * @private
   */
   ...
   * @param {int} offset Offset in buffer to write FourCC code at.
   * @private
   */
   ...
   * @param {int} start Start index in buffer.
   * @param {int} end End index in buffer.
   * @return {int} The computed CRC.
   * @private
   */
  ...
  /**
   * Append frame from data URL.
   * @param {array} chunks Array to add frame chunks to.
   * @param {string} dataURL Data URL for frame.
   * @param {int} sequenceNumber Next sequence number.
   * @param {boolean} firstFrame True if this is the first frame.
   * @return {int} Next sequence number.
   * @private
   */
  appendFrame_: function(chunks, dataURL, sequenceNumber, firstFrame) {
    ...
    this.writeUInt32_(fcTL, sequenceNumber, 8);
    this.writeUInt32_(fcTL, captureSize.width, 12);
    this.writeUInt32_(fcTL, captureSize.height, 16);
      ...
      switch (type) {
        ...
        case 'IDAT':
          ...
          if (firstFrame) {
            ...
            this.writeUInt32_(fdAT, sequenceNumber, 8);
