...
class TouchSelectionControllerTest : public testing::Test,
 ...
 public:
  ...
  void OnDragUpdate(const gfx::PointF& position) override {
    last_event_observer_position_ = position;
  }
  ...
  const gfx::PointF& GetLastEventObserverPosition() const {
    return last_event_observer_position_;
  }
 ...
 private:
  ...
  gfx::PointF last_event_observer_position_;
...
TEST_F(TouchSelectionControllerTest, InsertionObserverPosition) {
  base::TimeTicks event_time = base::TimeTicks::Now();
  float line_height = 10.f;
  ...
  ChangeInsertion(insertion_rect, visible);
  EXPECT_THAT(GetAndResetEvents(), ElementsAre(INSERTION_HANDLE_SHOWN));
  EXPECT_EQ(gfx::PointF(0.f, 0.f), GetLastEventObserverPosition());
  ...
  EXPECT_TRUE(controller().WillHandleTouchEvent(event));
  EXPECT_THAT(GetAndResetEvents(), ElementsAre(INSERTION_HANDLE_DRAG_STARTED));
  EXPECT_EQ(gfx::PointF(10.f, 5.f), GetLastEventObserverPosition());
  ...
  EXPECT_EQ(gfx::PointF(12.f, 5.f), GetLastEventObserverPosition());
  ...
  EXPECT_EQ(gfx::PointF(11.f, 6.f), GetLastEventObserverPosition());
...
TEST_F(TouchSelectionControllerTest, SelectionObserverPosition) {
  base::TimeTicks event_time = base::TimeTicks::Now();
  float line_height = 10.f;
  gfx::RectF start_rect(10, 0, 0, line_height);
  gfx::RectF end_rect(50, 0, 0, line_height);
  ...
  EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLES_MOVED));
  // Don't follow the y-coordinate change but only x-coordinate change.
  EXPECT_EQ(gfx::PointF(16.f, 5.f), GetLastEventObserverPosition());
  ...
  EXPECT_TRUE(controller().WillHandleTouchEvent(event));
  EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLE_DRAG_STARTED));
  EXPECT_EQ(gfx::PointF(50.f, 5.f), GetLastEventObserverPosition());
  ...
  EXPECT_TRUE(controller().WillHandleTouchEvent(event));
  EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLES_MOVED));
  EXPECT_EQ(gfx::PointF(45.f, 5.f), GetLastEventObserverPosition());
