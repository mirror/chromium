...
/**
 * Returns whether to show task picker when opened by the default action.
 * It is shown when there's 2 or more non-generic handler app, and none has
 * been selected by the preference.
 *
 * @return {boolean}
 */
FileTasks.prototype.shouldShowPickerByDefault_ = function() {
  return this.defaultTask_ == null && this.tasks_.length >= 2;
};
...
FileTasks.prototype.executeDefaultInternal_ = function(opt_callback) {
  ...
  if (this.shouldShowPickerByDefault_()) {
    this.showTaskPicker(
