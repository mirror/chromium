 ...
 * @param {string} iconUrl Url of the icon for the provider.
 * @param {string} name Already localized name of the provider.
 * @private
 */
ProvidersMenu.prototype.addProvider_ = function(providerId, iconUrl, name) {
  var item = this.addMenuItem_();
  item.label = name;

  var iconImage = 'url(' + iconUrl + ')';
...
ProvidersMenu.prototype.onUpdate_ = function(event) {
  this.model_.getMountableProviders().then(function(providers) {
    ...
    providers.forEach(function(provider) {
      this.addProvider_(provider.providerId, provider.iconUrl, provider.name);
    }.bind(this));
