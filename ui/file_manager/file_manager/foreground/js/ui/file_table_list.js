...
/** @override */
FileTableList.prototype.mergeItems =
...
/**
 * Processes touch events and give abstract events.
 * @constructor
 */
 ...
 * @param {!Event} e
 * @param {number} index
 * @param {function(!Event, number, string)} callback called when a tap event
 *     is detected. Should return ture if it has taken any action, and false
 *     if it ignroes the event.
 */
FileTapHandler.prototype.handleTouchEvents = function(e, index, callback) {
  // TODO: define enum for long press, long tap, and short tap.
  switch (e.type) {
    case 'touchstart':
      this.tapping_ = true;
      this.isLongTap_ = false;
    ...
    case 'touchmove':
      ...
      break;
    case 'touchend':
      // TODO: veirfy this is on the same item as pointerdown
      if (this.longTapDetectorTimeout_ != null) {
        clearTimeout(this.longTapDetectorTimeout_);
      }
      if (this.tapping_) {
        if (this.isLongTap_) {
          if (this.hasLongPressProcessed_ || callback(e, index, 'longtap')) {
            e.preventDefault();
          }
        } else {
          if (callback(e, index, 'tap')) {
            e.preventDefault();
          }
        }
      }
...
filelist.handleTap = function(e, index, event_type) {
  ...
  var sm = /** @type {!FileListSelectionModel|!FileListSingleSelectionModel} */
      (this.selectionModel);
  var isTap = event_type == 'tap' || event_type == 'longtap';
  if (sm.multiple && sm.getCheckSelectMode() && isTap && !e.shiftKey) {
    ...
    sm.endChange();
    return true;
  } else if (event_type == 'tap' && !e.shiftKey) {
    sm.beginChange();
  ...
  console.log('fall back to mouse');
  return false;
};
