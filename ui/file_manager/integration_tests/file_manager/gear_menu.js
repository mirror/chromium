...
/**
 * Test for the "paste-into-current" menu item
 */
testcase.showPasteInGearMenu = function() {
  ...
  StepsRunner.run([
    ...
    // #paste-into-current command is shown. It should be disabled because no
    // file has been copied to clipboard.
    function(result) {
      remoteCall
          .waitForElement(
              appId,
              '#gear-menu cr-menu-item' +
                  '[command=\'#paste-into-current\'][disabled]:not([hidden])')
          .then(this.next);
    },
    function() {
      remoteCall.callRemoteTestUtil(
      ...
      remoteCall
          .waitForElement(
              ...
              '#gear-menu:not([hidden])' +
                  ' cr-menu-item[command=\'#paste-into-current\']' +
