...
ViewAndroid::ViewAndroid(ViewClient* view_client, LayoutType layoutType)
    : parent_(nullptr),
      client_(view_client),
      layout_params_(layoutType, 0, 0, 0, 0) {}
...
void ViewAndroid::OnSizeChanged(int width, int height) {
  ...
  DCHECK(!layout_params_.match_parent());
...
void ViewAndroid::SetLayoutForTesting(LayoutType layout,
                                      int x,
                                      int y,
                                      int width,
                                      int height) {
  layout_params_ = {layout, x, y, width, height};
}
