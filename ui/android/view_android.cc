...
void ViewAndroid::SetAnchorRect(const JavaRef<jobject>& anchor,
                                const gfx::RectF& bounds_css) {
  ScopedJavaLocalRef<jobject> delegate(GetViewAndroidDelegate());
  if (delegate.is_null())
    return;

  float scale = GetDipScale();
  int left_margin = std::round(bounds_css.x() * scale);
  int top_margin = std::round((content_offset() + bounds_css.y()) * scale);
  const gfx::RectF bounds_px = gfx::ScaleRect(bounds_css, scale);
  JNIEnv* env = base::android::AttachCurrentThread();
