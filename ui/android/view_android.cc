...
void ViewAndroid::SetAnchorRect(const JavaRef<jobject>& anchor,
  ...
  float scaled_x = bounds.x() * scale;
  float scaled_y = bounds.y() * scale;
  float scaled_width = bounds.width() * scale;
  float scaled_height = bounds.height() * scale;
  JNIEnv* env = base::android::AttachCurrentThread();
  Java_ViewAndroidDelegate_setViewPosition(
      env, delegate, anchor, scaled_x, scaled_y, scaled_width, scaled_height,
