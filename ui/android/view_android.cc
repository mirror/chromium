...
void ViewAndroid::OnCursorChanged(int type,
  ...
  if (type == WebCursorInfo::kTypeCustom) {
    if (custom_image.drawsNothing() || custom_image.width() <= 0 ||
        custom_image.height() <= 0 ||
        (custom_image.colorType() != kRGB_565_SkColorType &&
         custom_image.colorType() != kN32_SkColorType)) {
