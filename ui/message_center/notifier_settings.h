...
struct MESSAGE_CENTER_EXPORT NotifierId {
  ...
  // only used for ChromeOS multi-profile support. If |profile_id| is non-empty,
  // the notification will only display to the profile specified by
  // |profile_id|. Otherwise, the notification will display to any profile. This
  // property should be empty if the notification is an Ash system notification.
