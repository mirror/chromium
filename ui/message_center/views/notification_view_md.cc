...
SkColor GetAccentColorOrDefault(const Notification& notification) {
  if (notification.accent_color() == SkColorSetARGB(0, 0, 0, 0))
    return kActionButtonTextColor;
  else
    return notification.accent_color();
}
    ...
    base::string16 app_name =
        notification.display_source().empty()
            ? l10n_util::GetStringFUTF16(
                  ...
                  controller()->GetProductOSName())
            : notification.display_source();
    header_row_->SetAppName(app_name);
    header_row_->SetAccentColor(GetAccentColorOrDefault(notification));
  } else {
...
void NotificationViewMD::CreateOrUpdateActionButtonViews(
  ...
  for (size_t i = 0; i < buttons.size(); ++i) {
    ...
    if (notification.notifier_id().type == NotifierId::SYSTEM_COMPONENT) {
      SkColor accent_color = GetAccentColorOrDefault(notification);
