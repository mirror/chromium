...
void NotificationViewMD::CreateOrUpdateViews(const Notification& notification) {
  ...
  CreateOrUpdateStatusView(notification);
...
void NotificationViewMD::CreateOrUpdateCompactTitleMessageView(
  ...
  if (!notification.status().empty() && !notification.sub_status().empty()) {
    compact_title_message_view_->set_message(notification.sub_status());
  } else {
...
void NotificationViewMD::CreateOrUpdateStatusView(
    const Notification& notification) {
  bool has_status_texts =
      !notification.status().empty() && !notification.sub_status().empty();
  ...
  status_view_->SetText(notification.status());
}
