...
NotificationInputMD::NotificationInputMD(const std::string& id)
    : id_(id), delegate_(nullptr), index_(0) {
  set_controller(this);
...
bool NotificationInputMD::HandleKeyEvent(views::Textfield* sender,
  ...
  if (delegate_ && event.type() == ui::ET_KEY_PRESSED &&
      event.key_code() == ui::VKEY_RETURN) {
    delegate_->ClickOnNotificationButtonWithReply(id_, index_, text());
    return true;
  }
  return event.type() == ui::ET_KEY_RELEASED;
}

void NotificationInputMD::set_button_info(
    const message_center::ButtonInfo& button_info) {
  set_placeholder_text(button_info.placeholder);
}
...
NotificationViewMD::NotificationViewMD(MessageViewDelegate* controller,
  ...
  inline_reply_ = new NotificationInputMD(notification_id());
...
void NotificationViewMD::ButtonPressed(views::Button* sender,
  ...
  for (size_t i = 0; i < action_buttons_.size(); ++i) {
    if (sender != action_buttons_[i])
      continue;
    if (action_buttons_[i]->button_info().type ==
        message_center::ButtonType::TEXT) {
      inline_reply_->set_delegate(delegate());
      inline_reply_->set_index(i);
      inline_reply_->set_button_info(action_buttons_[i]->button_info());
  ...
  for (size_t i = 0; i < buttons.size(); ++i) {
    ...
    if (new_buttons) {
      NotificationButtonMD* button =
          new NotificationButtonMD(this, button_info);
