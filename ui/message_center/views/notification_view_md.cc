...
NotificationViewMD::NotificationViewMD(const Notification& notification)
  ...
  auto temp_content_row_layout = std::make_unique<views::BoxLayout>(
      views::BoxLayout::kHorizontal, kContentRowPadding, 0);
  temp_content_row_layout->set_cross_axis_alignment(
      views::BoxLayout::CROSS_AXIS_ALIGNMENT_START);
  auto* content_row_layout =
      content_row_->SetLayoutManager(std::move(temp_content_row_layout));
