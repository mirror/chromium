...
void NotificationViewMD::CreateOrUpdateIconView(
  ...
  // If |show_image_as_right_icon| is set, use |image| as the icon on the right
  // side, instead of |icon|.
  gfx::ImageSkia icon;
  if (notification.show_image_as_right_icon())
    icon = notification.image().AsImageSkia();
  else
    icon = notification.icon().AsImageSkia();
  icon_view_->SetImage(icon, icon.size());

  // If |show_image_as_right_icon| is set, hide the icon on the right side when
  // the notification is expanded.
  hide_icon_on_expanded_ = notification.show_image_as_right_icon();
}
