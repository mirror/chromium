...
NotificationInputTextfieldMD::NotificationInputTextfieldMD(
    ...
    : controller_(controller), index_(0) {
...
void NotificationInputContainerMD::OnAfterUserAction(views::Textfield* sender) {
  if (textfield_is_empty_ && !textfield_->text().empty()) {
    textfield_is_empty_ = false;
    button_->SetNormalImage();
  } else if (!textfield_is_empty_ && textfield_->text().empty()) {
    textfield_is_empty_ = true;
