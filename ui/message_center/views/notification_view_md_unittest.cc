...
TEST_F(NotificationViewMDTest, ShowImageAsRightIcon) {
  // TODO(tetsui): Remove duplicated integer literal in CreateOrUpdateIconView.
  const int kNotificationIconSize = 30;
  ...
  // Test notification with show_image_as_right_icon e.g. screenshot preview.
  notification()->set_show_image_as_right_icon(true);
  UpdateNotificationViews();
  EXPECT_TRUE(notification_view()->icon_view_->visible());
