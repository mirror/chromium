...
// Paddings of the views of texts. 9px is 11px (from the mock) - 2px (outer
// padding), 6px from the mock
constexpr gfx::Insets kTextViewPadding(9, 0, 6, 0);

// Paddings of the entire header. 14px is 16px (from the mock) - 2px (outer
// padding). 2px is the minimum padding between the control buttons and the
// header.
constexpr gfx::Insets kHeaderPadding(0, 14, 0, 2);

// Paddings of the app icon (small image). 8px is 10px (from the mock) - 2px
// (outer padding). 4px is 6px (from the mock, the padding between the app icon
// and app name) - kHeaderHorizontalSpacing (padding automatically added
// BoxLayout).
constexpr gfx::Insets kAppIconPadding(8, 0, 4, 4);

// Size of the expand icon. 8px is 32px - 15px - 9px (values from the mock).
constexpr int kExpandIconSize = 8;
// Paddings of the expand buttons. 13px is 15px (from the mock) - 2px (outer
// padding) and 9px from the mock
...
NotificationHeaderView::NotificationHeaderView(
  ...
  CHECK_EQ(kInnerHeaderHeight, app_icon_view_->GetPreferredSize().height());
  ...
  CHECK_EQ(15, font_list.GetHeight());
  ...
  CHECK_EQ(kInnerHeaderHeight, app_name_view_->GetPreferredSize().height());
  ...
  CHECK_EQ(kInnerHeaderHeight, summary_text_view_->GetPreferredSize().height());
  ...
  CHECK_EQ(kInnerHeaderHeight, timestamp_divider_->GetPreferredSize().height());
  ...
  CHECK_EQ(kInnerHeaderHeight, timestamp_view_->GetPreferredSize().height());
  ...
  CHECK_EQ(kInnerHeaderHeight, expand_button_->GetPreferredSize().height());
