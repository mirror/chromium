...
void NotificationHeaderView::Layout() {
  // Temporality make |ink_drop_container_| invisible so layout manager would
  // not care about it.
  bool previously_visible = ink_drop_container_->visible();
  ink_drop_container_->SetVisible(false);
  CustomButton::Layout();
  ink_drop_container_->SetVisible(previously_visible);
