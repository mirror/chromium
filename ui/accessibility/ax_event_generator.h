...
class AX_EXPORT AXEventGenerator : public AXTreeDelegate {
 public:
  ...
  // Get all events queued up so far and clear the queue.
  std::vector<TargetedEvent> PopAllQueuedEvents();
  ...
  // to the queue and pop them later.
  //
  // Note that events are organized by node and then by event id to
  // efficiently remove duplicates, so events won't be popped in the
  // same order they were pushed.
  void PushEvent(ui::AXNode* node, Event event);
 ...
 private:
  ...
  std::map<int32_t, std::set<Event>> tree_events_;
};
