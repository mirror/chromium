...
void AXEventGenerator::AddEvent(ui::AXNode* node,
  ...
  if (node->data().role == ax::mojom::Role::INLINE_TEXT_BOX)
    return;
...
void AXEventGenerator::OnNodeDataWillChange(AXTree* tree,
  ...
  if (new_node_data.child_ids != old_node_data.child_ids &&
      new_node_data.role != ax::mojom::Role::STATIC_TEXT) {
...
void AXEventGenerator::OnStateChanged(AXTree* tree,
  ...
  if (state == ax::mojom::State::EXPANDED) {
    AddEvent(node, new_value ? Event::EXPANDED : Event::COLLAPSED);
    if (node->data().role == ax::mojom::Role::ROW ||
        node->data().role == ax::mojom::Role::TREE_ITEM) {
  ...
  if (state == ax::mojom::State::SELECTED) {
...
void AXEventGenerator::OnStringAttributeChanged(AXTree* tree,
  ...
  switch (attr) {
    case ax::mojom::StringAttribute::NAME:
      ...
      if (node->data().HasStringAttribute(ax::mojom::StringAttribute::CONTAINER_LIVE_STATUS)) {
    ...
    case ax::mojom::StringAttribute::LIVE_STATUS:
      if (node->data().role != ax::mojom::Role::ALERT)
...
void AXEventGenerator::OnFloatAttributeChanged(AXTree* tree,
  ...
  if (attr == ax::mojom::FloatAttribute::VALUE_FOR_RANGE)
    AddEvent(node, Event::VALUE_CHANGED);
  else
    AddEvent(node, Event::OTHER_ATTRIBUTE_CHANGED);
}
...
void AXEventGenerator::OnAtomicUpdateFinished(
  ...
  for (const auto& change : changes) {
    if ((change.type == NODE_CREATED || change.type == SUBTREE_CREATED)) {
      if (change.node->data().HasStringAttribute(ax::mojom::StringAttribute::LIVE_STATUS)) {
        if (change.node->data().role == ax::mojom::Role::ALERT)
          ...
          AddEvent(change.node, Event::LIVE_REGION_CREATED);
                     ...
                     ax::mojom::StringAttribute::CONTAINER_LIVE_STATUS) &&
                 change.node->data().HasStringAttribute(ax::mojom::StringAttribute::NAME)) {
...
void AXEventGenerator::FireLiveRegionEvents(AXNode* node) {
  ...
  while (live_root &&
         !live_root->data().HasStringAttribute(ax::mojom::StringAttribute::LIVE_STATUS))
    live_root = live_root->parent();

  if (live_root && !live_root->data().GetBoolAttribute(ax::mojom::BoolAttribute::BUSY)) {
    // Fire LIVE_REGION_NODE_CHANGED on each node that changed.
    if (!node->data().GetStringAttribute(ax::mojom::StringAttribute::NAME).empty())
...
void AXEventGenerator::FireActiveDescendantEvents() {
  for (AXNode* node : active_descendant_changed_) {
    AXNode* descendant = tree_->GetFromId(
        node->data().GetIntAttribute(ax::mojom::IntAttribute::ACTIVEDESCENDANT_ID));
    if (!descendant)
      continue;
