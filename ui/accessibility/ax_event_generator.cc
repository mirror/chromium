...
AXEventGenerator::PopAllQueuedEvents() {
  ...
  for (auto& entry : tree_events_) {
    ...
    // Consolidate events here.
    //
    // The ALERT event is special, it fires when an element with
    // role="alert" gets added to the page, or when the alert role
    // gets added to an existing element.  However, an alert is also a
    // live region, so it triggers the LIVE_REGION_CREATED event,
    // which is redundant. If we see both on the same node, suppress
    // the LIVE_REGION_CREATED event.
...
void AXEventGenerator::OnStateChanged(AXTree* tree,
  ...
  if (state == ui::AX_STATE_EXPANDED) {
    PushEvent(node, new_value ? Event::EXPANDED : Event::COLLAPSED);
