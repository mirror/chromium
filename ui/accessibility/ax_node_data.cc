...
void AXNodeData::SetName(const std::string& name) {
  ...
  string_attributes.push_back(
      std::make_pair(ax::mojom::StringAttribute::NAME, name));
}
...
void AXNodeData::SetValue(const std::string& value) {
  ...
  string_attributes.push_back(
      std::make_pair(ax::mojom::StringAttribute::VALUE, value));
}
  ...
  switch (action_enum) {
    ...
    case ax::mojom::Action::FOCUS: {
      ax::mojom::Action excluded_action =
          (action_enum == ax::mojom::Action::BLUR) ? ax::mojom::Action::FOCUS
    ...
    switch (intlist_attributes[i].first) {
      ...
      case ax::mojom::IntListAttribute::MARKER_TYPES: {
        ...
        for (size_t i = 0; i < values.size(); ++i) {
          ...
          if (type &
              static_cast<int32_t>(ax::mojom::MarkerType::ACTIVE_SUGGESTION))
