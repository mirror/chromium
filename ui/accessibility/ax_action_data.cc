...
// that works fine because this is normally printed by ax::mojom::Action::ToString.
std::string AXActionData::ToString() const {
  ...
  if (IsFlagSet(flags, ax::mojom::ActionFlags::REQUEST_IMAGES))
    result += " flag_request_images";

  if (IsFlagSet(flags, ax::mojom::ActionFlags::REQUEST_INLINE_TEXT_BOXES))
