...
RoleMap BuildRoleMap() {
  const RoleMap::value_type roles[] = {
      ...
      {ui::AX_ROLE_LAYOUT_TABLE, NSAccessibilityGroupRole},
      {ui::AX_ROLE_LAYOUT_TABLE_CELL, NSAccessibilityGroupRole},
      {ui::AX_ROLE_LAYOUT_TABLE_COLUMN, NSAccessibilityGroupRole},
      {ui::AX_ROLE_LAYOUT_TABLE_ROW, NSAccessibilityGroupRole},
...
- (BOOL)accessibilityIsAttributeSettable:(NSString*)attributeName {
  ...
  const ui::AXRestriction restriction = node_->GetData().GetRestriction();
...
- (NSNumber*)AXEnabled {
  return @(node_->GetData().GetRestriction() != ui::AX_RESTRICTION_DISABLED);
}
