...
- (BOOL)accessibilityIsAttributeSettable:(NSString*)attributeName {
  ...
  const int restrictions =
      node_->GetData().GetIntAttribute(ui::AX_ATTR_RESTRICTIONS);
  if (restrictions == ui::AX_RESTRICTIONS_DISABLED)
    return NO;
  ...
  if ([attributeName isEqualToString:NSAccessibilityValueAttribute]) {
    ...
    return restrictions != ui::AX_RESTRICTIONS_READ_ONLY;
  }

  // Change what text is selected (can occur in a read only field).
  if ([attributeName isEqualToString:NSAccessibilitySelectedTextRangeAttribute])
    return YES;

  // Change the selected text to other text (cannot occur in a read only field).
...
- (NSNumber*)AXEnabled {
  return @(node_->GetData().GetIntAttribute(ui::AX_ATTR_RESTRICTIONS) !=
           ui::AX_RESTRICTIONS_DISABLED);
}
