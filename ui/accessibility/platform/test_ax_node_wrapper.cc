...
bool TestAXNodeWrapper::AccessibilityPerformAction(
  ...
  if (data.action == ax::mojom::Action::SET_SELECTION) {
    ReplaceIntAttribute(data.anchor_node_id, ax::mojom::IntAttribute::TEXT_SEL_START,
                        data.anchor_offset);
    ReplaceIntAttribute(data.anchor_node_id, ax::mojom::IntAttribute::TEXT_SEL_END,
                        data.focus_offset);
    return true;
  }
...
std::set<int32_t> TestAXNodeWrapper::GetReverseRelations(ax::mojom::IntAttribute attr,
                                                         int32_t dst_id) {
