...
void AXPlatformNodeWin::NotifyAccessibilityEvent(ax::mojom::Event event_type) {
  ...
  if (event_type == ax::mojom::Event::SELECTION &&
      GetData().role == ax::mojom::Role::MENU_ITEM)
...
STDMETHODIMP AXPlatformNodeWin::get_accDefaultAction(
  ...
  int action;
  if (!target->GetIntAttribute(ax::mojom::IntAttribute::DEFAULT_ACTION_VERB,
                               &action)) {
    *def_action = nullptr;
    return S_FALSE;
  }
...
STDMETHODIMP AXPlatformNodeWin::get_accDescription(
  ...
  return target->GetStringAttributeAsBstr(
      ax::mojom::StringAttribute::DESCRIPTION, desc);
}
...
STDMETHODIMP AXPlatformNodeWin::get_accKeyboardShortcut(
  ...
  return target->GetStringAttributeAsBstr(
      ax::mojom::StringAttribute::KEY_SHORTCUTS, acc_key);
}
...
STDMETHODIMP AXPlatformNodeWin::get_accName(
  ...
  HRESULT result =
      target->GetStringAttributeAsBstr(ax::mojom::StringAttribute::NAME, name);
...
STDMETHODIMP AXPlatformNodeWin::get_accValue(VARIANT var_id, BSTR* value) {
  ...
  // cases. For example, if the node role is ax::mojom::Role::COLOR_WELL, we do
  // not care at all about the node's ax::mojom::StringAttribute::VALUE
  ...
  if (result.empty() && target->IsRangeValueSupported()) {
    float fval;
    if (target->GetFloatAttribute(ax::mojom::FloatAttribute::VALUE_FOR_RANGE,
                                  &fval)) {
...
STDMETHODIMP AXPlatformNodeWin::scrollTo(enum IA2ScrollType scroll_type) {
  ...
  // ax::mojom::Action::SCROLL_TO_MAKE_VISIBLE wants a target rect in *local*
  // coords.
  ...
  for (int r = 0; r < rows; ++r) {
    ...
    if (cell && cell->GetData().role == ax::mojom::Role::COLUMN_HEADER) {
      base::string16 cell_name =
          cell->GetString16Attribute(ax::mojom::StringAttribute::NAME);
      if (cell_name.size() > 0) {
      ...
      cell_name =
          cell->GetString16Attribute(ax::mojom::StringAttribute::DESCRIPTION);
      if (cell_name.size() > 0) {
        *description = SysAllocString(cell_name.c_str());
        return S_OK;
      }
...
int AXPlatformNodeWin::MSAARole() {
  ...
  switch (GetData().role) {
    ...
    case ax::mojom::Role::POP_UP_BUTTON: {
      std::string html_tag =
          GetData().GetStringAttribute(ax::mojom::StringAttribute::HTML_TAG);
      if (html_tag == "select")
        return ROLE_SYSTEM_COMBOBOX;
  ...
  switch (GetData().role) {
    ...
    case ax::mojom::Role::REGION: {
      base::string16 html_tag =
          GetString16Attribute(ax::mojom::StringAttribute::HTML_TAG);
...
std::vector<base::string16> AXPlatformNodeWin::ComputeIA2Attributes() {
  ...
  StringAttributeToIA2(result, ax::mojom::StringAttribute::PLACEHOLDER,
                       "placeholder");

  StringAttributeToIA2(result, ax::mojom::StringAttribute::AUTO_COMPLETE,
                       "autocomplete");
  StringAttributeToIA2(result, ax::mojom::StringAttribute::ROLE_DESCRIPTION,
                       "roledescription");
  StringAttributeToIA2(result, ax::mojom::StringAttribute::KEY_SHORTCUTS,
                       "keyshortcuts");

  IntAttributeToIA2(result, ax::mojom::IntAttribute::HIERARCHICAL_LEVEL,
                    "level");
  ...
  StringAttributeToIA2(result, ax::mojom::StringAttribute::LIVE_RELEVANT,
                       "relevant");
  ...
  StringAttributeToIA2(result,
                       ...
                       "container-relevant");
  BoolAttributeToIA2(result, ax::mojom::BoolAttribute::CONTAINER_LIVE_ATOMIC,
                     "container-atomic");
  BoolAttributeToIA2(result, ax::mojom::BoolAttribute::CONTAINER_LIVE_BUSY,
                     "container-busy");
  ...
  if (GetIntAttribute(ax::mojom::IntAttribute::ARIA_CURRENT_STATE,
                      &aria_current_state)) {
  ...
  if (IsTableLikeRole(GetData().role)) {
    IntAttributeToIA2(result, ax::mojom::IntAttribute::ARIA_COLUMN_COUNT,
                      "colcount");
    IntAttributeToIA2(result, ax::mojom::IntAttribute::ARIA_ROW_COUNT,
                      "rowcount");
  }
  ...
  if (IsCellOrTableHeaderRole(GetData().role) ||
    ...
    if (GetData().role != ax::mojom::Role::ROW)
      IntAttributeToIA2(result, ax::mojom::IntAttribute::ARIA_CELL_COLUMN_INDEX,
                        "colindex");
    IntAttributeToIA2(result, ax::mojom::IntAttribute::ARIA_CELL_ROW_INDEX,
                      "rowindex");
  ...
  if ((MSAARole() == ROLE_SYSTEM_COLUMNHEADER ||
      ...
      GetIntAttribute(ax::mojom::IntAttribute::SORT_DIRECTION,
                      &sort_direction)) {
...
AXHypertext AXPlatformNodeWin::ComputeHypertext() {
  ...
  for (int i = 0; i < child_count; ++i) {
    ...
    if (child->IsTextOnlyObject()) {
      hypertext +=
          child->GetString16Attribute(ax::mojom::StringAttribute::NAME);
    } else {
...
int AXPlatformNodeWin::MSAAState() {
  ...
  if (data.HasState(ax::mojom::State::INVISIBLE) ||
      GetData().role == ax::mojom::Role::IGNORED) {
  ...
  switch (checked_state) {
    case ax::mojom::CheckedState::TRUE_VALUE:
      msaa_state |= data.role == ax::mojom::Role::TOGGLE_BUTTON
  ...
  if (data.role == ax::mojom::Role::MENU_BAR &&
      !(data.HasState(ax::mojom::State::INVISIBLE))) {
    msaa_state |= STATE_SYSTEM_FOCUSED;
  }
...
void AXPlatformNodeWin::GetSelectionOffsets(int* selection_start,
  ...
  if (IsPlainTextField() &&
      GetIntAttribute(ax::mojom::IntAttribute::TEXT_SEL_START,
                      selection_start) &&
