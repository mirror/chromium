...
STDMETHODIMP AXPlatformNodeWin::get_accValue(VARIANT var_id, BSTR* value) {
  ...
  // For range controls, e.g. sliders and spin buttons, the value attribute
  // holds the aria-valuetext if present. The actual value, provided either via
  // aria-valuenow or the actual control's value is held in
