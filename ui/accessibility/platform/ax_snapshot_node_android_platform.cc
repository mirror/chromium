...
bool IsRichTextEditable(const AXNode* node) {
  ...
  return node->data().HasState(ax::mojom::State::RICHLY_EDITABLE) &&
         (!parent ||
          !parent->data().HasState(ax::mojom::State::RICHLY_EDITABLE));
}
...
base::string16 GetValue(const AXNode* node, bool show_password) {
  base::string16 value =
      node->data().GetString16Attribute(ax::mojom::StringAttribute::VALUE);
...
base::string16 GetText(const AXNode* node, bool show_password) {
  ...
  base::string16 text =
      node->data().GetString16Attribute(ax::mojom::StringAttribute::NAME);
  ...
  if (text.empty() && (AXSnapshotNodeAndroid::AXRoleIsLink(node->data().role) ||
    ...
    base::string16 url =
        node->data().GetString16Attribute(ax::mojom::StringAttribute::URL);
...
AXSnapshotNodeAndroid::WalkAXTreeDepthFirst(
  ...
  if (node->data().HasFloatAttribute(ax::mojom::FloatAttribute::FONT_SIZE)) {
    gfx::RectF text_size_rect(
        0, 0, 1,
        node->data().GetFloatAttribute(ax::mojom::FloatAttribute::FONT_SIZE));
    ...
    result->color =
        node->data().GetIntAttribute(ax::mojom::IntAttribute::COLOR);
    result->bgcolor =
        node->data().GetIntAttribute(ax::mojom::IntAttribute::BACKGROUND_COLOR);
    result->bold =
        (text_style &
