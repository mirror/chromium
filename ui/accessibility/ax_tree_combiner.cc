...
void AXTreeCombiner::ProcessTree(const AXTreeUpdate* tree) {
  ...
  for (size_t i = 0; i < tree->nodes.size(); ++i) {
    ...
    int32_t child_tree_id = node.GetIntAttribute(ax::mojom::IntAttribute::CHILD_TREE_ID);
    ...
    // ax::mojom::IntAttribute::CHILD_TREE_ID attribute.
    for (size_t j = 0; j < node.int_attributes.size(); ++j) {
      ...
      if (attr.first == ax::mojom::IntAttribute::CHILD_TREE_ID) {
        attr.first = ax::mojom::IntAttribute::NONE;
        attr.second = 0;
      }
    }
