...
gfx::RectF AXTree::RelativeToTreeBounds(const AXNode* node,
  ...
  while (node != nullptr) {
    ...
    int scroll_x = 0;
    int scroll_y = 0;
    if (container->data().GetIntAttribute(ax::mojom::IntAttribute::SCROLL_X,
                                          &scroll_x) &&
        container->data().GetIntAttribute(ax::mojom::IntAttribute::SCROLL_Y,
                                          &scroll_y)) {
    ...
    if (container->data().GetBoolAttribute(
            ax::mojom::BoolAttribute::CLIPS_CHILDREN)) {
