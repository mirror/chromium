...
gfx::RectF AXTree::RelativeToTreeBounds(const AXNode* node,
  ...
  while (node != nullptr) {
    ...
    if (container->data().role == ui::AX_ROLE_ROOT_WEB_AREA) {
      ...
      // Cannot use Intersect() test as a 0x0 element can be
      // contained within the screen bounds.
      if (offscreen != nullptr && (bounds.x() >= container_bounds.right() ||
                                   bounds.right() < container_bounds.x() ||
                                   bounds.y() >= container_bounds.bottom() ||
                                   bounds.bottom() < container_bounds.y()))
        *offscreen |= true;
