...
gfx::RectF AXTree::RelativeToTreeBounds(const AXNode* node,
  ...
  while (node != nullptr) {
    ...
    int scroll_x = 0;
    int scroll_y = 0;
    if (container->data().GetIntAttribute(ax::mojom::IntAttribute::SCROLL_X, &scroll_x) &&
        container->data().GetIntAttribute(ax::mojom::IntAttribute::SCROLL_Y, &scroll_y)) {
    ...
    if (container->data().GetBoolAttribute(ax::mojom::BoolAttribute::CLIPS_CHILDREN)) {
...
void AXTree::CallNodeChangeCallbacks(AXNode* node, const AXNodeData& new_data) {
  ...
  auto bool_callback = [this, node](ax::mojom::BoolAttribute attr, const bool& old_bool,
                                    const bool& new_bool) {
  ...
  auto int_callback = [this, node](ax::mojom::IntAttribute attr, const int& old_int,
                                   const int& new_int) {
...
void AXTree::UpdateReverseRelations(AXNode* node, const AXNodeData& new_data) {
  const AXNodeData& old_data = node->data();
  int id = new_data.id;
  auto int_callback = [this, node, id](ax::mojom::IntAttribute attr, const int& old_int,
                                       const int& new_int) {
