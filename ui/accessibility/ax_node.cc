...
bool AXNode::IsTextNode() const {
  return data().role == ax::mojom::Role::STATIC_TEXT ||
         data().role == ax::mojom::Role::LINE_BREAK ||
         data().role == ax::mojom::Role::INLINE_TEXT_BOX;
}
...
std::vector<int> AXNode::GetOrComputeLineStartOffsets() {
  ...
  if (data().GetIntListAttribute(ax::mojom::IntListAttribute::CACHED_LINE_STARTS, &line_offsets))
    return line_offsets;

  int start_offset = 0;
  ComputeLineStartOffsets(&line_offsets, &start_offset);
  data_.AddIntListAttribute(ax::mojom::IntListAttribute::CACHED_LINE_STARTS, line_offsets);
  return line_offsets;
}
...
void AXNode::ComputeLineStartOffsets(std::vector<int>* line_offsets,
  ...
  for (const AXNode* child : children()) {
    ...
    if (*start_offset &&
        !child->data().HasIntAttribute(ax::mojom::IntAttribute::PREVIOUS_ON_LINE_ID)) {
    ...
    base::string16 text = child->data().GetString16Attribute(ax::mojom::StringAttribute::NAME);
