...
void ContainerFloatingBehavior::SavePosition(const gfx::Rect& keyboard_bounds,
  ...
  if (position == nullptr) {
    ...
    position = default_position_.get();
  }

  if (left_distance < right_distance) {
    position->horizontal_anchor_direction = HorizontalAnchorDirection::LEFT;
    position->offset.set_x(left_distance);
  } else {
    position->horizontal_anchor_direction = HorizontalAnchorDirection::RIGHT;
    position->offset.set_x(right_distance);
  }
  if (top_distance < bottom_distance) {
    position->vertical_anchor_direction = VerticalAnchorDirection::TOP;
    position->offset.set_y(top_distance);
  } else {
    position->vertical_anchor_direction = VerticalAnchorDirection::BOTTOM;
    position->offset.set_y(bottom_distance);
  }
}
