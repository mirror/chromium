...
KeyboardController::KeyboardController(std::unique_ptr<KeyboardUI> ui,
  ...
  container_behavior_.reset(new ContainerFullWidthBehavior());
  ...
  switch (state_) {
    ...
    case KeyboardControllerState::SHOWN: {
...

      {
        ...
        container_behavior_.get()->DoHidingAnimation(window, &hiding_settings);
      }
...
void KeyboardController::PopulateKeyboardContent(int64_t display_id,
  ...
  switch (state_) {
    ...
    case KeyboardControllerState::HIDDEN: {
      ...
      container_behavior_.get()->InitializeShowAnimationStartingState(
          container_.get());
      break;
    }
    default:
  ...
  container_behavior_.get()->DoShowingAnimation(container_.get(), &settings);
