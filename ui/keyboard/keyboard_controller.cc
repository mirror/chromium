...
void KeyboardController::HideAnimationFinished() {
  if (state_ == KeyboardControllerState::HIDDEN && queued_container_behavior_) {
    SetContainerBehaviorInternal(queued_container_behavior_->container_type());
    ShowKeyboard(false /* lock */);
  }
}
...
void KeyboardController::PopulateKeyboardContent(int64_t display_id,
  ...
  // gets destroyed.
  queued_container_behavior_ = nullptr;
...
void KeyboardController::SetContainerType(const ContainerType type) {
  SetContainerType(type, base::BindOnce(&KeyboardController::Noop,
                                        weak_factory_noop_.GetWeakPtr()));
}

void KeyboardController::Noop(bool ignored) const {}
...
void KeyboardController::SetContainerType(
  ...
  queued_container_behavior_ = std::make_unique<QueuedContainerBehavior>(
