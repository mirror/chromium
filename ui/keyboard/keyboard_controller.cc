...
void KeyboardController::NotifyKeyboardBoundsChanging(
  ...
  if (ui_->HasWebContentsWindow() && ui_->GetWebContentsWindow()->IsVisible()) {
    for (KeyboardControllerObserver& observer : observer_list_)
...
bool KeyboardController::IsKeyboardWindowCreated() {
  return keyboard_container_initialized() && ui_->HasWebContentsWindow();
}
...
void KeyboardController::OnWindowBoundsChanged(aura::Window* window,
  ...
  // rotate.
  if (!keyboard_container_initialized() || !ui_->HasWebContentsWindow())
    return;
