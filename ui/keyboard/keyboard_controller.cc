...
const std::set<std::pair<keyboard::KeyboardControllerState,
    ...
    kAllowedStateTransition = {
        // The first ShowKeyboard scenario
        // INITIAL -> WAIT_RESIZE -> SHOWING -> SHOWN.
        ...
        // SHOWING -> WILL_HIDE is possible when focus is removed while keyboard
        // being shown.
        {keyboard::KeyboardControllerState::SHOWING,
         keyboard::KeyboardControllerState::WILL_HIDE},

        // WAIT_RESIZE -> INITIAL is possible when |HideKeyboard| is called
        // while wating for resize.
        ...
        // WAIT_RESIZE -> WAIT_RESIZE is possible when focused window is changed
        // and |ShowKeyboard| is called while waiting for resize. See
...
std::string StateToStr(keyboard::KeyboardControllerState state) {
  ...
  CHECK(false) << "Unknownstate: " << static_cast<int>(state);
}
...
void KeyboardController::OnTextInputStateChanged(
  ...
  DLOG(INFO) << "OnTextInputStateChanged";
  if (!container_.get()) {
...
void KeyboardController::ShowKeyboardInternal(int64_t display_id) {
  ...
  //    // Do nothing if the function is called while waiting for extensions to
  //    be
  ...
  DLOG(INFO) << "VK: keyboard_visible_ = true";
  keyboard_visible_ = true;
  ...
  if (container_->IsVisible() &&
    ...
    DCHECK(
        state_ == KeyboardControllerState::HIDING
...
void KeyboardController::ChangeState(KeyboardControllerState state) {
  ...
  DLOG(INFO) << StateToStr(state_) << " -> " << StateToStr(state);
  state_ = state;
  for (KeyboardControllerObserver& observer : observer_list_)
