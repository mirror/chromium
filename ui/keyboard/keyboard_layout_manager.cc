...
void KeyboardLayoutManager::SetChildBounds(aura::Window* child,
  ...
  bool controller_had_size = old_bounds.height() != 0;
  bool child_has_size = child->bounds().height() != 0;

  if (!controller_had_size && child_has_size && controller_->show_on_resize()) {
    ...
    if (!controller_had_size && child_has_size &&
