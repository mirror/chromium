...
class DummyTextInputClientWithSourceInfo : public ui::DummyTextInputClient {
 public:
  DummyTextInputClientWithSourceInfo(ui::TextInputType type, const GURL& url)
      : ui::DummyTextInputClient(type), source_info_(url) {}

  const GURL& GetClientSourceInfo() const override { return source_info_; }

 private:
  const GURL source_info_;
};
