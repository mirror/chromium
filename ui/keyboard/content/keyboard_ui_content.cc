...
aura::Window* KeyboardUIContent::GetWebContentsWindow() {
...
bool KeyboardUIContent::HasWebContentsWindow() const {
...
void KeyboardUIContent::ReloadKeyboardIfNeeded() {
  ...
  if (keyboard_contents_->GetURL() != GetVirtualKeyboardUrl()) {
    if (keyboard_contents_->GetURL().GetOrigin() !=
      ...
      GetWebContentsWindow()->SetBounds(gfx::Rect());
...
bool KeyboardUIContent::ShouldEnableInsets(aura::Window* window) {
  aura::Window* web_contents_window = GetWebContentsWindow();
  return (web_contents_window->GetRootWindow() == window->GetRootWindow() &&
