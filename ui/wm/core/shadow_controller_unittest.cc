...
TEST_F(ShadowControllerTest, Shadow) {
  ...
  SetShadowElevation(window.get(), kInactiveNormalShadowElevation);
  EXPECT_TRUE(shadow->layer()->visible());
...
TEST_F(ShadowControllerTest, ShadowStyle) {
  ...
  EXPECT_EQ(kActiveNormalShadowElevation, shadow1->desired_elevation());
  ...
  EXPECT_EQ(kInactiveNormalShadowElevation, shadow1->desired_elevation());
  EXPECT_EQ(kActiveNormalShadowElevation, shadow2->desired_elevation());
}
...
TEST_F(ShadowControllerTest, ShowState) {
  ...
  ASSERT_TRUE(shadow != NULL);
  EXPECT_EQ(kInactiveNormalShadowElevation, shadow->desired_elevation());
...
TEST_F(ShadowControllerTest, SmallShadowsForTooltipsAndMenus) {
  ...
  EXPECT_EQ(kMenuOrTooltipShadowElevation, tooltip_shadow->desired_elevation());
  ...
  EXPECT_EQ(kMenuOrTooltipShadowElevation, menu_shadow->desired_elevation());
}
