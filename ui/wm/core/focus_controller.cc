...
void FocusController::FocusAndActivateWindow(
  ...
  if (window &&
    ...
    StackActiveWindow();
    // TODO(msw): This early return seems to cause failed activation attempts.
    // return;
  }
  ...
  LOG(ERROR) << "MSW FocusController::FocusAndActivateWindow C " << window
             << " focusable:" << focusable << " activatable:" << activatable;
