...
#if DCHECK_IS_ON()
#define AX_CHECKS
// For testing only -- accessibility checks run in browsertests post-test.
...
class VIEWS_EXPORT View : public ui::LayerDelegate,
 ...
 public:
  ...
  static void TurnOnAccessibilityChecking() {
    isAccessibilityCheckingOn = true;
  }
  static bool IsCheckingAccessibility() { return isAccessibilityCheckingOn; }
  base::debug::StackTrace* GetConstructorStackForDebugging() {
    return isAccessibilityCheckingOn ? constructor_stack_for_debugging_.get()
                                     : nullptr;
  }
#endif
...
#if defined(AX_CHECKS)
  static bool isAccessibilityCheckingOn;

  // Keep stack trace for constructor, useful for debugging how inaccessible
  // views were created.
  const std::unique_ptr<base::debug::StackTrace>
      constructor_stack_for_debugging_ =
          View::isAccessibilityCheckingOn
              ? std::make_unique<base::debug::StackTrace>(15)
              : nullptr;
#endif
