...
void BoxLayout::Layout(View* host) {
  ...
  for (int i = 0; i < host->child_count(); ++i) {
    ...
    // Clip to the original content bounds so that clipping ignores margins.
    // This makes clipping easier to notice and debug.
    bounds.Intersect(host->GetContentsBounds());
