...
void ColumnSet::ResizeUsingMin(int total_delta) {
  ...
  // |delta| is negative, but easier to do operations when positive.
  ...
  for (size_t i = 0; i < columns_.size(); ++i) {
    ...
    const int available = preferred_column_sizes[i] - column->Size();
    DCHECK_GE(available, 0);
    // Reset the size back to the preferred. We'll reset the size if necessary
    // later.
  ...
  // resize. This may take multiple iterations as the min is hit. Generally
  // there are not that many columns in a GridLayout, so this code is not
  // particularly efficient. Any time the column hits the min it is removed from
  // |resize_data|.
