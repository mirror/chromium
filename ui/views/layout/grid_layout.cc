...
void GridLayout::SizeRowsAndColumns(bool layout, int width, int height,
  ...
  for (const auto& view_state : view_states_) {
    ...
    // If the view is given a different width than it's preferred (and the
    // preferred height isn't fixed) requery for the preferred height. This is
    // necessary as the preferred height may depend upon the width.
    int actual_width = view_state->column_set->GetColumnWidth(
        view_state->start_col, view_state->col_span);
    int x = 0;  // Not needed in this pass.
