...
gfx::Rect GetCornerScaledRecordingBounds(const ui::PaintContext& context,
                                         const gfx::Rect& child_bounds) {
  if (!context.is_pixel_canvas())
    return child_bounds;

  // Compute the size using "scale-corner-points" logic, which is to
  // compute the pixel size for views, except that a view touches the
  // left/bottom edge of the parent (hence, empty bounds/size below).
  // Such case should be handled case by cases.
...
PaintInfo::PaintInfo(const ui::PaintContext& layer_context,
    ...
    : paint_recording_scale_x_(layer_context.is_pixel_canvas()
      ...
      paint_recording_scale_y_(paint_recording_scale_x_),
      paint_recording_bounds_(
          GetCornerScaledRecordingBounds(layer_context, bounds)),
