...
TEST_F(ViewTest, ConversionsToFromScreen) {
  ...
  gfx::Rect rect = rect_in_screen;
  View::ConvertRectFromScreen(child, &rect);
  EXPECT_EQ(rect_in_child.ToString(), rect.ToString());

  View::ConvertRectToScreen(child, &rect);
  EXPECT_EQ(rect_in_screen.ToString(), rect.ToString());
}
