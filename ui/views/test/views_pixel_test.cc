...
#if defined(OS_MACOSX)
constexpr const char kPlatformName[] = "m";
#elif defined(OS_CHROMEOS)
constexpr const char kPlatformName[] = "c";
#elif defined(OS_WIN)
constexpr const char kPlatformName[] = "w";
#else
constexpr const char kPlatformName[] = "l";
#endif
...
bool PixelTest(const SkBitmap& bitmap, const char* name, bool* used_fallback) {
  ...
  const std::vector<std::string> paths = {"common", "l", "c", "w",
                                          "w10",    "m", "m9"};
  ...
  if (base::win::GetVersion() >= base::win::VERSION_WIN10 &&
      base::PathExists(data_path.AppendASCII("w10").AppendASCII(name))) {
    start_index = index_of("w10");
  }
#endif
#if defined(OS_MACOSX)
  if (base::mac::IsOS10_9() &&
      base::PathExists(data_path.AppendASCII("m9").AppendASCII(name))) {
    start_index = index_of("m9");
  }
#endif
