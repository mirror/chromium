...
// is available. If there is neither a platform-specific for the current
...
// test coverage in the default trybots appropriate for the test.
bool PixelTest(const SkBitmap& bitmap, const char* name, bool* used_fallback);

// Write the reference image to disk in the appropriate place on disk for the
// current platform. This writes to the checkout location and should never be
// run on a bot.
