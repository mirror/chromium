...
class PenProcessorTest : public testing::Test {
 public:
  PenProcessorTest(bool direct_manipulation_enabled)
      : id_generator_(0),
        processor_(&id_generator_, direct_manipulation_enabled) {}
  ~PenProcessorTest() override {}

 private:
  ui::SequentialIDGenerator id_generator_;

 protected:
  PenEventProcessor processor_;

 private:
  DISALLOW_COPY_AND_ASSIGN(PenProcessorTest);
};

class PenProcessorDirectManipulationEnabledTest : public PenProcessorTest {
 public:
  PenProcessorDirectManipulationEnabledTest() : PenProcessorTest(true) {}
};

class PenProcessorDirectManipulationDisbledTest : public PenProcessorTest {
 public:
  PenProcessorDirectManipulationDisbledTest() : PenProcessorTest(false) {}
};

TEST_F(PenProcessorDirectManipulationDisbledTest, TypicalCase) {
  POINTER_PEN_INFO pen_info;
  gfx::Point point(100, 100);
  memset(&pen_info, 0, sizeof(POINTER_PEN_INFO));

  std::unique_ptr<ui::Event> event =
      processor_.GenerateEvent(WM_POINTERENTER, 0, pen_info, point);
  ...
  event = processor_.GenerateEvent(WM_POINTERDOWN, 0, pen_info, point);
  ASSERT_TRUE(event->IsMouseEvent());
  ...
  event = processor_.GenerateEvent(WM_POINTERUPDATE, 0, pen_info, point);
  ASSERT_TRUE(event->IsMouseEvent());
  ...
  event = processor_.GenerateEvent(WM_POINTERUP, 0, pen_info, point);
  ASSERT_TRUE(event->IsMouseEvent());
  ...
  event = processor_.GenerateEvent(WM_POINTERLEAVE, 0, pen_info, point);
  ASSERT_TRUE(event->IsMouseEvent());
  EXPECT_EQ(ui::ET_MOUSE_EXITED, event->AsMouseEvent()->type());
}

TEST_F(PenProcessorDirectManipulationEnabledTest, TypicalCase) {
  POINTER_PEN_INFO pen_info;
  gfx::Point point(100, 100);
  memset(&pen_info, 0, sizeof(POINTER_PEN_INFO));
...
TEST_F(PenProcessorDirectManipulationEnabledTest, UnpairedPointerDownTouch) {
  POINTER_PEN_INFO pen_info;
  gfx::Point point(100, 100);
  memset(&pen_info, 0, sizeof(POINTER_PEN_INFO));
  ...
  std::unique_ptr<ui::Event> event =
      processor_.GenerateEvent(WM_POINTERUP, 0, pen_info, point);
  EXPECT_EQ(nullptr, event.get());
}

TEST_F(PenProcessorDirectManipulationEnabledTest, UnpairedPointerDownMouse) {
  POINTER_PEN_INFO pen_info;
  gfx::Point point(100, 100);
  memset(&pen_info, 0, sizeof(POINTER_PEN_INFO));
