...
int GetTouchEventFlags(UINT message, POINTER_INFO pointer_info) {
...
const base::Feature kMakePenBehaveLikeTouch{"MakePenBehaveLikeTouch",
...
LRESULT HWNDMessageHandler::HandlePointerEventTypePen(UINT message,
  ...
  bool make_pen_touch_flag =
      base::FeatureList::IsEnabled(kMakePenBehaveLikeTouch);
  if (!make_pen_touch_flag) {
    ...
    ui::MouseEvent event(event_type, point, point, event_time, flag, flag,
                         pointer_details);
  ...
  int flags = GetTouchEventFlags(message, pointer_pen_info.pointerInfo);
  bool is_hover_event = false;
  ...
  // If the flag is enabled, we send mouse events when the pen is hovering in
  // the range of tablet but not pressing, otherwise sending touch events when
  // the pen is pressing the tablet.
  bool handled = TRUE;
  if (is_hover_event) {
    ui::MouseEvent event(event_type, point, point, event_time, 0, 0,
                         pointer_details);
    handled = delegate_->HandleMouseEvent(event);
  } else {
    int rotation_angle = static_cast<int>(rotation) % 180;
    if (rotation_angle < 0)
      rotation_angle += 180;
