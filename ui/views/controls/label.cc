...
void Label::SetFontList(const gfx::FontList& font_list) {
  ...
  unelided_text_->SetFontList(font_list);
  ResetLayout();
}
...
void Label::SetText(const base::string16& new_text) {
  if (new_text == text())
    return;
  is_first_paint_text_ = true;
  unelided_text_->SetText(new_text);
  ResetLayout();
...
void Label::SetShadows(const gfx::ShadowValues& shadows) {
  if (unelided_text_->shadows() == shadows)
    return;
  is_first_paint_text_ = true;
  unelided_text_->set_shadows(shadows);
  ResetLayout();
}
...
void Label::SetHorizontalAlignment(gfx::HorizontalAlignment alignment) {
  ...
  if (horizontal_alignment() == alignment)
    return;
  is_first_paint_text_ = true;
  unelided_text_->SetHorizontalAlignment(alignment);
  ResetLayout();
}
...
void Label::SetLineHeight(int height) {
  if (line_height() == height)
    return;
  is_first_paint_text_ = true;
  unelided_text_->SetMinLineHeight(height);
  ResetLayout();
}
...
void Label::SetMultiLine(bool multi_line) {
  ...
  unelided_text_->SetMultiline(multi_line);
  unelided_text_->SetReplaceNewlineCharsWithSymbols(!multi_line);
  ResetLayout();
}
...
void Label::SetObscured(bool obscured) {
  if (this->obscured() == obscured)
    return;
  is_first_paint_text_ = true;
  unelided_text_->SetObscured(obscured);
  if (obscured)
    SetSelectable(false);
  ResetLayout();
}
...
void Label::SetAllowCharacterBreak(bool allow_character_break) {
  ...
  if (unelided_text_->word_wrap_behavior() == behavior)
    return;
  unelided_text_->SetWordWrapBehavior(behavior);
  if (multi_line()) {
    is_first_paint_text_ = true;
    ResetLayout();
  }
...
bool Label::IsSelectionSupported() const {
  return !obscured() && unelided_text_->IsSelectionSupported();
}
...
int Label::GetHeightForWidth(int w) const {
  ...
  if (!multi_line() || text().empty() || w <= 0) {
    ...
    // Be careful to invoke |unelided_text_->SetDisplayRect(gfx::Rect())| to
    ...
    // and |unelided_text_| can cache the height.
    unelided_text_->SetDisplayRect(gfx::Rect(0, 0, w, 0));
    int string_height = unelided_text_->GetStringSize().height();
  ...
  height -= gfx::ShadowValue::GetMargin(unelided_text_->shadows()).height();
...
void Label::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  ...
  // Note that |unelided_text_| is never elided (see the comment in Init() too).
  node_data->SetName(unelided_text_->GetDisplayText());
}
...
bool Label::GetTooltipText(const gfx::Point& p, base::string16* tooltip) const {
  ...
  if (ShouldShowDefaultTooltip()) {
    // Note that |unelided_text_| is never elided (see the comment in Init()
    // too).
    tooltip->assign(unelided_text_->GetDisplayText());
    return true;
  }
...
std::unique_ptr<gfx::RenderText> Label::CreateRenderText(
  ...
  std::unique_ptr<gfx::RenderText> render_text(
      unelided_text_->CreateInstanceOfSameType());
...
gfx::Rect Label::GetFocusRingBounds() const {
  ...
  if (!display_text_) {
    ...
    gfx::Point origin;
    origin += display_text_->GetLineOffset(0);
    focus_bounds.Union(gfx::Rect(origin, display_text_->GetStringSize()));
  }
...
const gfx::RenderText* Label::GetRenderTextForSelectionController() const {
  ...
  // This may by null when the content bounds of the view are empty.
  return display_text_.get();
}

void Label::Init(const base::string16& text, const gfx::FontList& font_list) {
  unelided_text_ = gfx::RenderText::CreateInstanceForToolkitUI();
  DCHECK(unelided_text_->MultilineSupported());
  unelided_text_->SetHorizontalAlignment(gfx::ALIGN_CENTER);
  unelided_text_->SetDirectionalityMode(gfx::DIRECTIONALITY_FROM_TEXT);
  // NOTE: |unelided_text_| should not be elided at all. This is used to keep
  // some properties and to compute the size of the string.
  unelided_text_->SetElideBehavior(gfx::NO_ELIDE);
  unelided_text_->SetFontList(font_list);
  unelided_text_->SetCursorEnabled(false);
  unelided_text_->SetWordWrapBehavior(gfx::TRUNCATE_LONG_WORDS);
...
void Label::MaybeBuildDisplayText() const {
  ...
  gfx::DirectionalityMode directionality =
      unelided_text_->directionality_mode();
  if (multi_line()) {
    // Force the directionality and alignment of the first line on other lines.
    bool rtl =
        unelided_text_->GetDisplayTextDirection() == base::i18n::RIGHT_TO_LEFT;
  ...
  render_text->SetWordWrapBehavior(unelided_text_->word_wrap_behavior());
  ...
  display_text_ = std::move(render_text);
  stored_selection_range_ = gfx::Range::InvalidRange();
  ApplyTextColors();
}
...
gfx::Size Label::GetTextSize() const {
  ...
  if (text().empty()) {
    ...
    // Cancel the display rect of |unelided_text_|. The display rect may be
    ...
    unelided_text_->SetDisplayRect(gfx::Rect(0, 0, width(), 0));
    size = unelided_text_->GetStringSize();
  }
