...
TableView::TableView(ui::TableModel* model,
  ...
  constexpr int font_context = style::CONTEXT_TABLE_ROW;
  constexpr int font_style = style::STYLE_PRIMARY;
  font_list_ = style::GetFont(font_context, font_style);
  row_height_ = std::max(style::GetLineHeight(font_context, font_style),
                         LayoutProvider::Get()->GetDistanceMetric(
                             DISTANCE_TABLE_ROW_MINIMUM_HEIGHT));
