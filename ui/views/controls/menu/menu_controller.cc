...
bool MenuController::OnMouseDragged(SubmenuView* source,
  ...
  if (part.type == MenuPart::MENU_ITEM) {
    ...
    // If there is a sibling menu, show it and exit if statement. Otherwise,
    // if the user has selected a menu item with no accompanying sibling menu
    // or submenu, move selection back to the parent menu item
