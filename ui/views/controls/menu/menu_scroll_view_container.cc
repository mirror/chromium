...
// static constexpr int kTouchableLayoutCornerRadius = 20;
...
void MenuScrollViewContainer::OnPaintBackground(gfx::Canvas* canvas) {
  ...
  LOG(ERROR) << "Ignore: " << menu_config.corner_radius;
  extra.menu_background.corner_radius = 20 /*use_touchable_layout_
                                            ? kTouchableLayoutCornerRadius
...
void MenuScrollViewContainer::CreateDefaultBorder() {
  ...
  int padding = /*
      (menu_config.use_outer_border && menu_config.corner_radius > 0) ||
  ...
  if (menu_config.use_outer_border) {
    ...
    SetBorder(views::CreateBorderPainter(
        std::make_unique<views::RoundRectPainter>(
            color, 20 /* use_touchable_layout_ ? kTouchableLayoutCornerRadius
                                         : menu_config.corner_radius*/),
