...
gfx::Rect TreeView::GetAuxiliaryTextBoundsForNode(InternalNode* node) {
  gfx::Rect text_bounds = GetTextBoundsForNode(node);
  if (base::i18n::IsRTL()) {
    return gfx::Rect(bounds().x() + kTextHorizontalPadding, text_bounds.y(),
                     text_bounds.x() - kTextHorizontalPadding * 2,
                     text_bounds.height());
  } else {
    int width =
        bounds().width() - text_bounds.right() - 2 * kTextHorizontalPadding;
    return gfx::Rect(bounds().right() - width - kTextHorizontalPadding,
                     text_bounds.y(), width, text_bounds.height());
  }
}
