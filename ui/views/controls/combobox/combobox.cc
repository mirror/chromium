...
void Combobox::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  ...
  // ax::mojom::Role::POP_UP_BUTTON to match an HTML <select> element.
  node_data->role = ax::mojom::Role::POP_UP_BUTTON;

  node_data->SetName(accessible_name_);
  node_data->SetValue(model_->GetItemAt(selected_index_));
  if (enabled()) {
    node_data->SetDefaultActionVerb(ax::mojom::DefaultActionVerb::OPEN);
  }
  node_data->AddIntAttribute(ax::mojom::IntAttribute::POS_IN_SET,
                             selected_index_);
  node_data->AddIntAttribute(ax::mojom::IntAttribute::SET_SIZE,
                             model_->GetItemCount());
}
...
bool Combobox::HandleAccessibleAction(const ui::AXActionData& action_data) {
  ...
  // forwarding ax::mojom::Action::DO_DEFAULT to View and then forwarding the
