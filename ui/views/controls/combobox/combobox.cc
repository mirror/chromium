...
SkColor GetTextColorForEnableState(bool enabled, const Combobox& combobox) {
...
void Combobox::PaintText(gfx::Canvas* canvas) {
  ...
  SkColor text_color = GetTextColorForEnableState(enabled(), *this);
  DCHECK_GE(selected_index_, 0);
  DCHECK_LT(selected_index_, model()->GetItemCount());
  if (selected_index_ < 0 || selected_index_ > model()->GetItemCount())
    selected_index_ = 0;
  ...
  if (UseMd()) {
    ...
    path.close();
    cc::PaintFlags flags;
    SkColor arrow_color = GetTextColorForEnableState(true, *this);
    if (!enabled())
