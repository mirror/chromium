...
void Combobox::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  ...
  // field, which views::Combobox does not have. Use ax::mojom::Role::POP_UP_BUTTON to
  ...
  node_data->AddIntAttribute(ax::mojom::IntAttribute::POS_IN_SET, selected_index_);
  node_data->AddIntAttribute(ax::mojom::IntAttribute::SET_SIZE, model_->GetItemCount());
}
...
bool Combobox::HandleAccessibleAction(const ui::AXActionData& action_data) {
  ...
  // forwarding ax::mojom::Action::DO_DEFAULT to View and then forwarding the mouse event
  ...
  if (enabled() && action_data.action == ax::mojom::Action::DO_DEFAULT) {
    ShowDropDownMenu(ui::MENU_SOURCE_KEYBOARD);
    return true;
  }
...
void Combobox::OnPerformAction() {
  NotifyAccessibilityEvent(ax::mojom::Event::VALUE_CHANGED, true);
  SchedulePaint();
