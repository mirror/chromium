...
void AdvanceOneLine(int* line,
                    ...
                    bool new_line = true) {
  const int x_delta = HorizontalAdjustment(offset->x(), width, alignment);
  for (auto* view : *views_in_a_line) {
    gfx::Rect bounds = view->bounds();
    bounds.set_x(bounds.x() + x_delta);
    bounds.set_y(insets_top + offset->y() +
                 (*max_line_height - bounds.height()) / 2.0f);
...
gfx::Size StyledLabel::CalculateAndDoLayout(int width, bool dry_run) {
  ...
  int line = 0;
  // The offset (in pixels) of the view on the line we're on, relative to
  // content bounds.
  gfx::Point offset;
  int total_height = 0;
  ...
  while (!remaining_string.empty()) {
    ...
    // If current range is not a custom_view, then we use ElideRectangleText to
    // determine the line wrapping. Note: if it is a custom_view, then the
    // |position| should equals |range.start()| because the custom_view is
    // treated as one unit.
    if (position != range.start() || (current_range != style_ranges_.end() &&
      ...
      if (substrings[0].empty() || truncated_chunk) {
        ...
        AdvanceOneLine(&line, &offset, &max_line_height, default_line_height,
                       width, insets.top(), horizontal_alignment_,
                       &views_in_a_line,
                       /*new_line=*/(offset.x() != 0 || line > 0));
        continue;
      }
    }
    ...
    int view_used_width =
        offset.x() + view_size.width() - focus_border_insets.width();
    // For custom view, it could be outside of the width.
    if (custom_view)
      view_used_width = std::min(view_used_width, width);
    offset.set_x(view_used_width);
    total_height =
  ...
  AdvanceOneLine(&line, &offset, &max_line_height, default_line_height, width,
                 insets.top(), horizontal_alignment_, &views_in_a_line, false);
