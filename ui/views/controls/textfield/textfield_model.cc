...
gfx::Range GetFirstEmphasizedRange(const ui::CompositionText& composition) {
  for (size_t i = 0; i < composition.ime_spans.size(); ++i) {
    const ui::ImeSpan& underline = composition.ime_spans[i];
    if (underline.thick)
...
void TextfieldModel::SetCompositionText(
  ...
  if (composition.ime_spans.size() > 0 && composition.ime_spans[0].color != 0)
