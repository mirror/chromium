...
TEST_F(TextfieldTest, PasteViaResponderChain) {
  InitTextfield();
  // Target the "WIDGET". This means on Mac, that keystrokes will be sent to a
  // dummy 'Edit' menu which will dispatch into the responder chain as a "cut:"
  // selector rather than a keydown.
  event_generator_->set_target(ui::test::EventGenerator::Target::WIDGET);

  SendKeyEvent(ui::VKEY_X, false, false);      // Type "o".
