...
TEST_F(TextfieldTest, AccessiblePasswordTest) {
  ...
  EXPECT_EQ(ax::mojom::Role::TEXT_FIELD, node_data_regular.role);
  EXPECT_EQ(ASCIIToUTF16("password"),
            node_data_regular.GetString16Attribute(ax::mojom::StringAttribute::VALUE));
  EXPECT_FALSE(node_data_regular.HasState(ax::mojom::State::PROTECTED));

  textfield_->SetTextInputType(ui::TEXT_INPUT_TYPE_PASSWORD);
  ui::AXNodeData node_data_protected;
  textfield_->GetAccessibleNodeData(&node_data_protected);
  EXPECT_EQ(ax::mojom::Role::TEXT_FIELD, node_data_protected.role);
  EXPECT_EQ(UTF8ToUTF16("••••••••"),
            node_data_protected.GetString16Attribute(ax::mojom::StringAttribute::VALUE));
  EXPECT_TRUE(node_data_protected.HasState(ax::mojom::State::PROTECTED));
}
