...
TEST_F(TextfieldModelTest, CompositionTextTest) {
  ...
  composition.ime_spans.push_back(ui::ImeSpan(0, 3, 0, false));
  ...
  composition.ime_spans.clear();
  composition.ime_spans.push_back(ui::ImeSpan(0, 2, 0, true));
  composition.ime_spans.push_back(ui::ImeSpan(2, 3, 0, false));
  model.SetCompositionText(composition);
  EXPECT_TRUE(model.HasCompositionText());
  EXPECT_TRUE(model.HasSelection());
#if !defined(OS_CHROMEOS)
  // |composition.selection| is ignored because SetCompositionText checks
  // if a bold ime_span exists first.
  ...
  // Also test the case where a selection exists but a bold ime_span doesn't.
