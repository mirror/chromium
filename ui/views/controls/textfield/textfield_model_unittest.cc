...
#ifdef OS_WIN  // Move right by word goes past space/punctuation.
...
#else  // Non-Windows: move right by word does NOT go past space/punctuation.
  model.MoveCursor(gfx::WORD_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_NONE);
  EXPECT_EQ(3U, model.GetCursorPosition());
  model.MoveCursor(gfx::WORD_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_NONE);
...
TEST_F(TextfieldModelTest, RangeTest) {
...
#ifdef OS_WIN  // Move/select right by word includes space/punctuation.
  model.MoveCursor(gfx::WORD_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_RETAIN);
  ...
  EXPECT_EQ(11U, range.start());
  EXPECT_EQ(11U, range.end());
#else  // Non-Windows: move/select right by word does NOT include space/punct.
  model.MoveCursor(gfx::WORD_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_RETAIN);
  range = model.render_text()->selection();
  EXPECT_FALSE(range.is_empty());
  EXPECT_FALSE(range.is_reversed());
...
#ifdef OS_WIN  // Select word to right includes trailing space/punctuation.
  model.MoveCursor(gfx::WORD_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_RETAIN);
