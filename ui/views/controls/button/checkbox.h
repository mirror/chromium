...
class VIEWS_EXPORT Checkbox : public LabelButton {
 ...
 private:
  // BaseFocusRing descendant used to paint the focus ring around the Checkbox
  // icon. The icon is painted separately.
  class IconFocusRing : public BaseFocusRing {
   public:
    explicit IconFocusRing(Checkbox* checkbox);
    ~IconFocusRing() override;

    static void Show(Checkbox* checkbox);
    static void Hide(Checkbox* checkbox);
  ...
  void PaintFocusRing(View* view, gfx::Canvas* canvas);
