...
void Checkbox::OnFocus() {
  ...
  if (!UseMd())
    ...
    FocusRing::Install(this, ui::NativeTheme::kColorId_NumColors, this);
}
...
void Checkbox::LayoutFocusRing(FocusRing* focus_ring) {
  DCHECK(focus_ring);
  gfx::Rect focus_bounds = image()->bounds();
  focus_bounds.Inset(gfx::Insets(-2.f));
  focus_ring->SetBoundsRect(focus_bounds);
}

void Checkbox::PaintFocusRing(FocusRing* focus_ring, gfx::Canvas* canvas) {
...
void Checkbox::PaintFocusRing(FocusRing* focus_ring,
                              gfx::Canvas* canvas,
                              const cc::PaintFlags& flags) {
  gfx::RectF focus_rect(focus_ring->GetLocalBounds());
