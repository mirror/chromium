...
void MdTextButton::UpdateColors() {
  ...
  if (!explicitly_set_normal_color()) {
    ...
    // since a descendant could have overridden the label enalbed color.
  ...
  if (is_prominent_) {
    ...
    if (is_disabled) {
      ...
      // value will take the disabled button colors, a1a192 @ 1.0 alpha,
      // 9e9e9e @ 1.0 alpha for Harmony and turn it into e6e6e6 @ 1.0 alpha or
      // an effective 000000 @ 0.1 alpha for the stroke color. The same alpha
      // value will work with both Harmony ana non-Harmony colors.
      stroke_alpha = 0x43;
    } else {
      // These alpha values will take the enabled button colors, 5a5a5a @ 1.0
      // alpha, 757575 @ 1.0 alpha for Harmony and turn it into an effective
      // cccccc @ 1.0 alpha or 000000 @ 0.2 for the stroke_color.
      stroke_alpha = UseMaterialSecondaryButtons() ? 0x5f : 0x4e;
    }
    // For simplicity, the stroke color is the same for both Harmony and
    // non-Harmony.
