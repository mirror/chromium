...
TEST_F(LabelButtonTest, LabelWrapAndImageAlignment) {
  ...
  EXPECT_EQ(button_->image()->y(),
            (button_->height() - button_->image()->height()) / 2);

  button_->SetImageAlignment(views::Button::ALIGN_TOP);
  button_->Layout();
  ...
  button_->SetImageAlignment(views::Button::ALIGN_BOTTOM);
  button_->Layout();

  EXPECT_EQ(button_->image()->y(),
            button_->height() - button_insets.bottom() - font_list.GetHeight() +
                ((font_list.GetHeight() - button_->image()->height()) / 2));
}
