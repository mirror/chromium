...
class VIEWS_EXPORT NativeViewHost : public View {
 public:
  ...
  // of this View when it is being captured. If the size does not match, scaling
  // will occur. Pass an empty size to revert to the default behavior, where the
  // NatieView's rendering size always equals this View's size.
  void SetCaptureRenderingSize(const gfx::Size& size);
