...
class TabLabel : public Label {
 public:
  ...
  void GetAccessibleNodeData(ui::AXNodeData* data) override {
    ...
    data->role = ax::mojom::Role::IGNORED;
  }
};
...
void Tab::GetAccessibleNodeData(ui::AXNodeData* data) {
  data->role = ax::mojom::Role::TAB;
  data->SetName(title()->text());
  data->AddState(ax::mojom::State::SELECTABLE);
  if (selected())
    data->AddState(ax::mojom::State::SELECTED);
}

bool Tab::HandleAccessibleAction(const ui::AXActionData& action_data) {
  if (action_data.action != ax::mojom::Action::SET_SELECTION || !enabled())
    return false;

  // It's not clear what should happen if a tab is 'deselected', so the
  // ax::mojom::Action::SET_SELECTION action will always select the tab.
  tabbed_pane_->SelectTab(this);
  return true;
}
...
void Tab::OnFocus() {
  ...
  // with focus will send an ax::mojom::Event::FOCUS of its own, so there's no need to
  // send one in OnBlur().
  if (contents())
    contents()->NotifyAccessibilityEvent(ax::mojom::Event::FOCUS, true);
  SchedulePaint();
}
...
void TabbedPane::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  node_data->role = ax::mojom::Role::TAB_LIST;
}
