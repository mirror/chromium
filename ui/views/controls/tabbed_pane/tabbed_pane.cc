...
MdTab::MdTab(TabbedPane* tabbed_pane,
  ...
  if (tabbed_pane_->IsHorizontal()) {
    ...
    const int kBorderThicknessHorizontal = 16;
    SetBorder(CreateEmptyBorder(
...
void TabStrip::OnPaintBorder(gfx::Canvas* canvas) {
  ...
  if (selected_tab_index >= 0) {
    ...
    if (tabbed_pane_->IsHorizontal()) {
      ...
      tab_width = SkIntToScalar(width()) - kTabBorderThickness;
...
MdTabStrip::MdTabStrip(TabbedPane* tabbed_pane) : TabStrip(tabbed_pane) {
  auto layout = std::make_unique<BoxLayout>(tabbed_pane_->IsHorizontal()
