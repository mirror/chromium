...
Tab::Tab(TabbedPane* tabbed_pane, const base::string16& title, View* contents)
  ...
  const bool is_highlight_style =
      tabbed_pane_->GetTabStripStyle() == TabbedPane::TabStripStyle::kHighlight;

  if (is_vertical)
    title_->SetHorizontalAlignment(gfx::HorizontalAlignment::ALIGN_LEFT);
  if (is_highlight_style)
...
void Tab::OnStateChanged() {
  ...
  switch (tab_state_) {
    case TAB_INACTIVE:
      ...
      break;
    case TAB_ACTIVE:
      title_->SetEnabledColor(tabbed_pane_->GetTabStripStyle() ==
...
gfx::Size Tab::CalculatePreferredSize() const {
  ...
  if (tabbed_pane_->GetTabStripStyle() ==
          TabbedPane::TabStripStyle::kHighlight &&
      tabbed_pane_->GetOrientation() == TabbedPane::Orientation::kVertical) {
    size.SetToMax(
...
void TabStrip::OnPaintBorder(gfx::Canvas* canvas) {
  ...
  if (tab_strip_style_ == TabbedPane::TabStripStyle::kBorder &&
...
void TabbedPane::SetTabStripStyle(TabStripStyle style) {
  tab_strip_->set_tab_strip_style(style);
  SchedulePaint();
}

TabbedPane::TabStripStyle TabbedPane::GetTabStripStyle() const {
  return tab_strip_->tab_strip_style();
}
