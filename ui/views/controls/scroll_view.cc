...
ScrollView::ScrollView()
  ...
  // Just make sure the more_content indicators aren't visible for not. They'll
...
void ScrollView::PositionOverflowIndicator(Separator* indicator,
  ...
  switch (edge) {
    case ContentEdge::kBottom:
      content_bounds.set_y(content_bounds.bottom() - pref_size.height());
    // fall through
    case ContentEdge::kTop:
      content_bounds.set_height(pref_size.height());
      break;
    case ContentEdge::kRight:
      content_bounds.set_x(content_bounds.right() - pref_size.width());
    // fall through
    case ContentEdge::kLeft:
      content_bounds.set_width(pref_size.width());
      break;
  }
  indicator->SetBoundsRect(content_bounds);
}

void ScrollView::UpdateOverflowIndicators(const gfx::ScrollOffset& offset) {
  SetControlVisibility(more_content_top_,
                       show_edges_with_hidden_content_ && vert_sb_->visible() &&
