...
void ScrollView::SetContents(View* a_view) {
  ...
  if (ScrollsWithLayers()) {
    ...
    a_view->layer()->SetOnScrollCallback(
        base::Bind(&ScrollView::OnLayerScrolled, base::Unretained(this)));
    a_view->layer()->SetScrollContainerBounds(
        contents_viewport_->bounds().size());
  }
...
void ScrollView::Layout() {
  ...
  if (contents_ && ScrollsWithLayers()) {
    ...
    contents_->layer()->SetScrollContainerBounds(viewport_bounds.size());
  }
