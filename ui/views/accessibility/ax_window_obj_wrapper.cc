...
void AXWindowObjWrapper::Serialize(ui::AXNodeData* out_node_data) {
  ...
  if (role != ax::mojom::Role::NONE)
    ...
    out_node_data->role =
        is_alert_ ? ax::mojom::Role::ALERT : ax::mojom::Role::WINDOW;
  ...
  if (child_ax_tree_id != ui::AXTreeIDRegistry::kNoAXTreeID) {
    ...
    out_node_data->AddIntAttribute(ax::mojom::IntAttribute::CHILD_TREE_ID,
                                   child_ax_tree_id);
  }
}
...
void AXWindowObjWrapper::OnWindowBoundsChanged(
  ...
  if (widget) {
    ...
    if (root_view)
      root_view->NotifyAccessibilityEvent(ax::mojom::Event::LOCATION_CHANGED,
                                          true);
  }
}
...
void AXWindowObjWrapper::OnWindowPropertyChanged(aura::Window* window,
  ...
  if (window == window_ && key == ui::kChildAXTreeID) {
    AXAuraObjCache::GetInstance()->FireEvent(
        this, ax::mojom::Event::CHILDREN_CHANGED);
  }
}
