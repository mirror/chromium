...
class NativeViewAccessibilityTest : public ViewsTestBase {
 public:
  ...
  bool SetFocused(NativeViewAccessibilityBase* view_accessibility,
    ...
    data.action = focused ? ax::mojom::Action::FOCUS : ax::mojom::Action::BLUR;
...
TEST_F(NativeViewAccessibilityTest, RoleShouldMatch) {
  EXPECT_EQ(ax::mojom::Role::BUTTON, button_accessibility()->GetData().role);
  ...
  // role of its own.
  EXPECT_EQ(ax::mojom::Role::IGNORED, label_accessibility()->GetData().role);
  ...
  EXPECT_EQ(ax::mojom::Role::STATIC_TEXT, label_accessibility()->GetData().role);
}
...
TEST_F(NativeViewAccessibilityTest, LabelIsChildOfButton) {
  ...
  EXPECT_EQ(button_->GetNativeViewAccessible(),
            label_accessibility()->GetParent());
  EXPECT_NE(ax::mojom::Role::IGNORED, label_accessibility()->GetData().role);
}

// Verify Views with invisible ancestors have ax::mojom::State::INVISIBLE.
TEST_F(NativeViewAccessibilityTest, InvisibleViews) {
  EXPECT_TRUE(widget_->IsVisible());
  EXPECT_FALSE(
      button_accessibility()->GetData().HasState(ax::mojom::State::INVISIBLE));
  EXPECT_FALSE(
      label_accessibility()->GetData().HasState(ax::mojom::State::INVISIBLE));
...
class AxTestViewsDelegate : public TestViewsDelegate {
 public:
  ...
  void NotifyAccessibilityEvent(View* view, ax::mojom::Event event_type) override {
