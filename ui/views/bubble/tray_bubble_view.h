...
class VIEWS_EXPORT TrayBubbleView : public BubbleDialogDelegateView,
 ...
 private:
  // This reroutes receiving key events to the widget passed in the constructor.
  // TrayBubbleView is not activated by default. But we want to activate it if
  // user tries to interact it with keyboard. To capture those key events in
  // early stage, RerouteEventHandler installs this handler to aura::Env.
  class RerouteEventHandler : public ui::EventHandler {
   public:
    explicit RerouteEventHandler(Widget* widget);
   ...
   private:
    // Widget to which key events are going to be rerouted. Not owned.
    Widget* widget_;
