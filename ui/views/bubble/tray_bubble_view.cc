...
void TrayBubbleView::RerouteEventHandler::OnKeyEvent(ui::KeyEvent* event) {
  ...
  // RerouteEventHandler always consumes key event regardless of whether the
  // widget has consumed it or not.
...
void TrayBubbleView::InitializeAndShowBubble() {
  ...
  if (!CanActivate()) {
    ...
    activation_handler_ = base::MakeUnique<RerouteEventHandler>(GetWidget());
  }
}
