...
void BubbleFrameView::Layout() {
  ...
  DCHECK_EQ(GetTitleLabelInsets().right(), width() - title_label_right);
...
gfx::Insets BubbleFrameView::GetTitleLabelInsets() const {
  ...
  if (close_->visible()) {
    const int close_margin =
        LayoutProvider::Get()->GetDistanceMetric(DISTANCE_CLOSE_BUTTON_MARGIN);
    insets_right =
        std::max(insets_right,
                 border_insets.right() + 2 * close_margin + close_->width());
  }
...
int BubbleFrameView::GetTitleWidthForClientWidth(int client_width) const {
  // A request for a client_width of 0 typically occurs during Widget::Init().
  // Use a title width of 0 in that case to avoid typesetting in a tiny width.
  if (client_width == 0)
    return 0;
