...
gfx::Size BubbleFrameView::GetFrameSizeForClientSize(
  ...
  // First get the frame width, since that determines how many lines are needed
  // for the title. This must account for the minimum title bar width then
  // including insets and possible snapping. It does not include the border.
  const int frame_width = [this, client_size] {
    ...
    const int client_area_width =
        client_size.width() + content_margins_.width();
...
bool BubbleFrameView::HasTitle() const {
  return custom_title_ != nullptr ||
         (default_title_->visible() && !default_title_->text().empty()) ||
