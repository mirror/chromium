...
class TestBubbleDialogDelegateView : public BubbleDialogDelegateView {
 public:
  ...
  void ChangeCloseVisibility(bool visible) {
    has_close_ = visible;
    GetWidget()->non_client_view()->ResetWindowControls();
    SizeToContents();
    // If a size change isn't required, forcing the BubbleFrameView to re-layout
    // is complicated. Changing visibility doesn't invalidate layout so (since
    // BubbleFrameView doesn't use a LayoutManager) Layout() won't propagate
    // down from above. We must Layout() the BubbleFrameView explicitly, which
    // NonClientView does.
  ...
  bool ShouldShowCloseButton() const override { return has_close_; }
...
TEST_F(BubbleFrameViewTest, LayoutEdgeCases) {
  ...
  // a +17 change in height and title length of 53 characters.
  const int two_line_height = bubble->GetWindowBoundsInScreen().height();
  EXPECT_LT(10, two_line_height - min_bubble_height);
  EXPECT_GT(20, two_line_height - min_bubble_height);
