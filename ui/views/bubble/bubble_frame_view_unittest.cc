...
TEST_F(BubbleFrameViewTest, GetUpdatedWindowBoundsCenterArrows) {
  TestBubbleFrameView frame(this);
  gfx::Rect window_bounds;
  // Bubbles have a thicker shadow on the top in MD.
  // https://cs.chromium.org/chromium/src/ui/views/bubble/bubble_border.cc?rcl=63a6c9032333566b7bb370391031fa1ffe22ccae&l=376
  const int kLargeShadowVerticalOffset = UseMd() ? 2 : 0;

  // Some of these tests will go away once --secondary-ui-md becomes the
  // default.
