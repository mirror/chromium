...
void BubbleDialogDelegateView::HandleVisibilityChanged(Widget* widget,
  ...
  // Fire ax::mojom::Event::ALERT for bubbles marked as
  // ax::mojom::Role::ALERT_DIALOG; this instructs accessibility tools to read
  ...
  if (widget == GetWidget() && visible) {
    if (GetAccessibleWindowRole() == ax::mojom::Role::ALERT_DIALOG)
      widget->GetRootView()->NotifyAccessibilityEvent(ax::mojom::Event::ALERT,
                                                      true);
  }
}
