...
void Widget::CloseAllSecondaryWidgets() {
  ...
  for (NSWindow* window in starting_windows) {
    ...
    // TODO(tapted): Remove the delegate key. windowInfoKey is probably useful
    // to keep to help diagnose leaked observers crashing in Widget code.
    static crash_reporter::CrashKeyString<256> windowInfoKey("windowInfo");
    static crash_reporter::CrashKeyString<256> windowDelegateInfoKey(
        "windowDelegateInfo");
    std::string value = base::SysNSStringToUTF8(
        [NSString stringWithFormat:@"Closing %@ (%@)", [window title],
                                   [window className]]);
    crash_reporter::ScopedCrashKeyString scopedWindowKey(&windowInfoKey, value);
