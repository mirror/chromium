...
TEST_F(NativeWidgetMacTest, TouchBar) {
  ...
  if (@available(macOS 10.12.2, *)) {
    ...
    EXPECT_TRUE(group);
    // Sadly, here, we run out of public APIs to test further. But we can ensure
    // the touchBar is recreated by comparing pointers.
  ...
  if (@available(macOS 10.12.2, *)) {
    EXPECT_NSNE(old_touch_bar, [content touchBar]);
  }
