...
TEST_F(X11TopmostWindowFinderTest, NonRectangular) {
  ...
  auto region1 = base::MakeUnique<Widget::ShapeRects>();
  region1->emplace_back(0, 10, 10, 90);
  region1->emplace_back(10, 0, 90, 100);
  widget1->SetShape(std::move(region1));
...
TEST_F(X11TopmostWindowFinderTest, NonRectangularEmptyShape) {
  ...
  XID xid1 = widget1->GetNativeWindow()->GetHost()->GetAcceleratedWidget();
  auto region1 = base::MakeUnique<Widget::ShapeRects>();
  region1->emplace_back();
  // Widget takes ownership of |region1|.
  widget1->SetShape(std::move(region1));
