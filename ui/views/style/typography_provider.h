...
class VIEWS_EXPORT TypographyProvider {
 public:
  ...
  // Gets the FontList for the given |context| and |style|. |view| is the View
  // requesting the font.
  virtual const gfx::FontList& GetFont(const View& view,
                                       int context,
                                       int style) const = 0;

  // Gets the color for the given |context| and |style|. |view| is the View
  // requesting the color.
  virtual SkColor GetColor(const View& view, int context, int style) const = 0;

  // Gets the line spacing, or 0 if it should be provided by gfx::FontList.
  // |view| is the View requesting the line height.
  virtual int GetLineHeight(const View& view, int context, int style) const = 0;
...
class VIEWS_EXPORT DefaultTypographyProvider : public TypographyProvider {
 public:
  ...
  SkColor GetColor(const View& view, int context, int style) const override;
  int GetLineHeight(const View& view, int context, int style) const override;
