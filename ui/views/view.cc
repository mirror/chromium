...
void View::UpdateChildLayerBounds(const LayerOffsetData& offset_data) {
  if (layer()) {
    SetLayerBounds(GetLocalBounds() + offset_data.offset, offset_data);
  } else {
    internal::ScopedChildrenLock lock(this);
    for (auto* child : children_) {
...
void View::OnDeviceScaleFactorChanged(float device_scale_factor) {
  ...
  if (layer()->parent() && layer()->GetCompositor() &&
    ...
    LayerOffsetData offset_data(
        ...
        layer()->GetCompositor()->device_scale_factor());

    // Do a parent walk until we find the parent layer or the root view.
    const View* view = this;
    while (view->parent() && view->parent()->layer() != layer()->parent()) {
      view = view->parent();
      offset_data.AddOffset(view->GetMirroredPosition().OffsetFromOrigin());
    }
...
void View::SnapLayerToPixelBoundary(const LayerOffsetData& offset_data) {
  ...
  if (snap_layer_to_pixel_boundary_ && layer()->parent() &&
    ...
    if (layer()->GetCompositor()->is_pixel_canvas()) {
      gfx::Vector2dF subpixel_offset(offset_data.total_subpixel_offset);
      subpixel_offset.Scale(1.f / layer()->device_scale_factor());
      layer()->SetSubpixelPositionOffset(subpixel_offset);
    } else {
...
void View::BoundsChanged(const gfx::Rect& previous_bounds) {
  ...
  if (layer()) {
    if (parent_) {
      ...
      SetLayerBounds(bounds_, LayerOffsetData());
    }
