...
class InputDispatcher : public base::RefCounted<InputDispatcher> {
 ...
 private:
  ...
  InputDispatcher(const base::Closure& task,
  ...
  long expected_mouse_move_x_;
  long expected_mouse_move_y_;
...
InputDispatcher::InputDispatcher(const base::Closure& task,
                                 WPARAM message_waiting_for)
    : InputDispatcher(task, message_waiting_for, 0, 0) {}

InputDispatcher::InputDispatcher(const base::Closure& task,
                                 long mouse_x,
                                 long mouse_y)
    : InputDispatcher(task, WM_MOUSEMOVE, mouse_x, mouse_y) {}
...
InputDispatcher::InputDispatcher(const base::Closure& task,
    ...
    : task_(task),
      ...
      expected_mouse_move_x_(mouse_x),
      expected_mouse_move_y_(mouse_y) {
  InstallHook(this, message_waiting_for_ == WM_KEYUP);
}
...
void InputDispatcher::DispatchedMessage(WPARAM message) {
  if (message == message_waiting_for_) {
    if (message == WM_MOUSEMOVE) {
      ...
      if (expected_mouse_move_x_ != current_pos.x ||
          expected_mouse_move_y_ != current_pos.y) {
        LOG(ERROR) << "Mouse moved to (" << current_pos.x << ", "
                   << current_pos.y << ") rather than ("
                   << expected_mouse_move_x_ << ", " << expected_mouse_move_y_
...
bool SendMouseMoveImpl(long screen_x,
                       ...
                       const base::Closure& task) {
  if (task) {
    // Mouse move notification only works if this proc has a window at the
    // desired point, as hooking is done only for windows in this process (to be
    // more specific: only for this thread, but we always do windowing on a
    // single thread).
