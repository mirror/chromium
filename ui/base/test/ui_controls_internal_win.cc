...
class InputDispatcher : public base::RefCounted<InputDispatcher> {
 ...
 private:
  ...
  void InstallHook(bool key_hook);
  ...
  gfx::Point expected_mouse_location_;
...
InputDispatcher::InputDispatcher(WPARAM message_waiting_for,
    ...
    : callback_(callback),
      ...
      weak_factory_(this) {
  InstallHook(message_waiting_for_ == WM_KEYUP);
}
...
void InputDispatcher::InstallHook(bool key_hook) {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  ...
  if (key_hook) {
    ...
    // Things don't go well sometimes. Bail out if it takes too long.
...
void InputDispatcher::DispatchedMessage(WPARAM mouse_message) {
  ...
  if (mouse_message == message_waiting_for_) {
    if (mouse_message == WM_MOUSEMOVE) {
      ...
      if (expected_mouse_location_ != gfx::Point(current_pos)) {
