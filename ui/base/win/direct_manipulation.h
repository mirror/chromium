...
class DirectManipulationHandler
 ...
 private:
  ...
  float last_x_offset_ = 0.0f;
  float last_y_offset_ = 0.0f;
...
class UI_BASE_EXPORT DirectManipulationHelper {
 public:
  ...
  void ResetViewport(bool need_animtation);

  // Pass the pointer hit test to Direct Manipulation. Return need begin frame.
  bool OnPointerHitTest(WPARAM w_param);

  // Return still need begin frame.
