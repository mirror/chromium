...
void DirectManipulationHelper::Update() {
  ...
  base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
      ...
      base::TimeDelta::FromSecondsD(1. / 1000));
}
...
HRESULT DirectManipulationHandler::OnViewportStatusChanged(
  ...
  if (current == DIRECTMANIPULATION_READY ||
    ...
    if (gesture_ != NONE) {
      ...
      if (gesture_ == SCROLL) {
        // event_target_->HandleDManipEndMessage(ET_GESTURE_SCROLL_END);
      } else if (gesture_ == FLING) {
        event_target_->HandleDManipEndMessage(ET_SCROLL_FLING_CANCEL);
      } else {  // PINCH
        event_target_->HandleDManipEndMessage(ET_GESTURE_PINCH_END);
      }
      gesture_ = NONE;
    }
  } else if (current == DIRECTMANIPULATION_INERTIA) {
    event_target_->HandleDManipEndMessage(ET_GESTURE_SCROLL_END);
    event_target_->HandleDManipBeginMessage(0, 0, ET_SCROLL_FLING_START);
    gesture_ = FLING;
  }
  return S_OK;
}
...
HRESULT DirectManipulationHandler::OnContentUpdated(
  ...
  DirectManipulationGesture gesture = gesture_;
  float scale = xform[0];
  float x_offset = xform[4];
  float y_offset = xform[5];

  // Ignore float point rounding error.
  if (Equals(scale, 1.0) && Equals(x_offset, last_x_offset_) &&
      Equals(y_offset, last_y_offset_)) {
    return true;
  }
  ...
  // For first change determind it is scroll, fling or pinch.
  if (gesture == NONE) {
    if (status == DIRECTMANIPULATION_INERTIA) {
      gesture = FLING;
    } else if (fabs(1.0f - scale) < std::numeric_limits<float>::epsilon()) {
      gesture = SCROLL;
    } else {
      gesture = PINCH;
    }
  ...
  // scroll gesture may upgrade to pinch.
  if (gesture_ == SCROLL) {
  ...
  // LOG(ERROR) << std::setprecision(8) << scale << "," << x_offset << ","
  //            << y_offset << "," << gesture_ << "," << gesture;
  ...
  if (gesture != gesture_) {
    if (gesture == SCROLL) {
      event_target_->HandleDManipEndMessage(ET_SCROLL_FLING_CANCEL);
      event_target_->HandleDManipBeginMessage(x_offset - last_x_offset_,
                                              y_offset - last_y_offset_,
                                              ET_GESTURE_SCROLL_BEGIN);
    } else if (gesture == PINCH) {
      event_target_->HandleDManipPinch(scale / last_scale_,
                                       ET_GESTURE_PINCH_BEGIN);
    }
  }

  // Send MouseWheel or PinchUpdate
  if (gesture != PINCH) {
    result = event_target_->HandleDManipScroll(x_offset - last_x_offset_,
                                               y_offset - last_y_offset_);
  } else {  // PINCH
    result = event_target_->HandleDManipPinch(scale / last_scale_,
