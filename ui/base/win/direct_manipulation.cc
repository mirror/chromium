...
bool DirectManipulationHelper::OnPointerHitTest(WPARAM w_param) {
  ...
  if (get_pointer_type && get_pointer_type(pointer_id, &pointer_type) &&
    ...
    need_begin_frame_ = true;
  }
  return need_begin_frame_;
}

void DirectManipulationHelper::ResetViewport(bool need_begin_frame) {
  RECT rect;
  ...
  need_begin_frame_ = need_begin_frame;
}

bool DirectManipulationHelper::OnBeginFrame() {
  // Simulate 1 frame in update_manager_.
  LOG(ERROR) << "OnBeginFrame ?";
  update_manager_->Update(nullptr);
  LOG(ERROR) << "OnBeginFrame " << need_begin_frame_;
  return need_begin_frame_;
}
...
HRESULT DirectManipulationHandler::OnViewportStatusChanged(
  ...
  // at identity.
  LOG(ERROR) << "OnViewportStatusChanged " << current;

  if (current == DIRECTMANIPULATION_ENABLED) {
    helper_->ResetViewport(false);
    last_scale_ = 1.0f;
    last_x_offset_ = 0.0f;
    last_y_offset_ = 0.0f;
    reseted_ = false;
    ...
    // begin frame at the second ready.
    LOG(ERROR) << "ready " << reseted_;
    reseted_ = !reseted_;
    helper_->ResetViewport(reseted_);
    last_scale_ = 1.0f;
    last_x_offset_ = 0.0f;
    last_y_offset_ = 0.0f;
  }
...
HRESULT DirectManipulationHandler::OnContentUpdated(
  ...
  LOG(ERROR) << "OnContentUpdated " << xform[0] << " " << xform[4] << " "
             << xform[5];
