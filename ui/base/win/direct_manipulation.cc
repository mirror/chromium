...
HRESULT DirectManipulationHandler::OnContentUpdated(
  ...
  if (FloatEquals(scale, 1.0)) {
    hr = event_target_->ApplyDManipTransform(last_scale_ / last_scale_,
                                             last_x_offset_ - last_x_offset_,
                                             last_y_offset_ - last_y_offset_);
  } else {  // Pinch zoom
    if (!pinch_begin_sent_)
      event_target_->SendGesturePinchBegin();
    event_target_->SendGesturePinchUpdate(last_scale_ / last_scale_);
  }
