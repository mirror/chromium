  ...
  if (gesture == PINCH) {
    ...
    if (gesture_ != gesture) {
      ui::EventType event_type =
          gesture == FLING ? ET_SCROLL_FLING_START : ET_GESTURE_SCROLL_BEGIN;
      result = event_target_->HandleDManipScroll(
          x_offset - last_x_offset_, y_offset - last_y_offset_, event_type);
    }

    result = event_target_->HandleDManipScroll(
        x_offset - last_x_offset_, y_offset - last_y_offset_, ET_SCROLL);
  }
