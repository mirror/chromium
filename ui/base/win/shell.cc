...
void ClearWindowPropertyStore(HWND hwnd) {
  ...
  HRESULT result = pps->GetCount(&property_count);
  if (FAILED(result))
    return;

  PROPVARIANT empty_property_variant = {};
  for (int i = static_cast<int>(property_count) - 1; i >= 0; i--) {
    PROPERTYKEY key;
    if (SUCCEEDED(pps->GetAt(static_cast<DWORD>(i), &key))) {
  ...
  if (SUCCEEDED(pps->GetCount(&property_count)))
    DCHECK_EQ(0u, property_count);
#endif
}
