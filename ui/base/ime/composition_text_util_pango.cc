...
void ExtractCompositionTextFromGtkPreedit(const gchar* utf8_text,
  ...
  if (attrs) {
    ...
    do {
      ...
      if (background_attr || underline_attr) {
        ...
        ImeSpan ime_span(char16_offsets[start], char16_offsets[end],
        ...
        if (background_attr) {
          ...
          if (ime_span.start_offset == cursor_offset) {
            composition->selection.set_start(ime_span.end_offset);
            composition->selection.set_end(cursor_offset);
          } else if (ime_span.end_offset == cursor_offset) {
            composition->selection.set_start(ime_span.start_offset);
            composition->selection.set_end(cursor_offset);
          }
        }
        if (underline_attr) {
          ...
          if (type == PANGO_UNDERLINE_DOUBLE)
            ...
            ime_span.color = SK_ColorRED;
        }
        composition->ime_spans.push_back(ime_span);
      }
  ...
  // Use a black thin underline by default.
  if (composition->ime_spans.empty()) {
    composition->ime_spans.push_back(
        ImeSpan(0, length, SK_ColorBLACK, false, SK_ColorTRANSPARENT));
  }
}
