...
ui::EventDispatchDetails InputMethodAuraLinux::DispatchKeyEvent(
  ...
  suppress_commits_until_ = base::TimeTicks::UnixEpoch();
  composition_changed_ = false;
  result_text_.clear();
...
void InputMethodAuraLinux::ResetContext() {
  ...
  if (!composition_.text.empty()) {
    // If the IME has an open composition, ignore attempts to commit text for a
    // brief duration of time (we don't have a better way to ignore them since
    // commits can happen asynchronously).
    suppress_commits_until_ =
        base::TimeTicks::Now() +
...
bool InputMethodAuraLinux::IgnoringNonKeyInput() const {
  return base::TimeTicks::Now() < suppress_commits_until_;
}
