...
void InputMethodUtil::RemoveInputMethods(const InputMethodDescriptors& imes) {
  ...
  for (size_t i = 0; i < imes.size(); ++i) {
    ...
    for (auto it = language_code_to_ids_.begin(); it != language_code_to_ids_.end(); ) {
      if (it->second == input_method.id()) {
          VLOG(1) << "@@@@@       remove: lang=" << it->first << " id=" << input_method.id();
...
void InputMethodUtil::AppendInputMethods(const InputMethodDescriptors& imes) {
  ...
  for (size_t i = 0; i < imes.size(); ++i) {
    ...
    for (size_t j = 0; j < language_codes.size(); ++j) {
      ...
      if (it == range.second) {
        VLOG(1) << "@@@@@       insert: lang=" << language_codes[j] << " id=" << input_method.id();
