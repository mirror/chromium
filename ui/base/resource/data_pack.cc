...
// 4) The file is closed automatically on scope exit.
//
...
class ScopedFileWriter {
 public:
  ...
  void Write(const void* data, size_t data_size) {
    if (valid_ && fwrite(data, data_size, 1, file_.get()) != 1) {
...
bool DataPack::WritePack(const base::FilePath& path,
  ...
  uint16_t alias_count = static_cast<uint16_t>(aliases.size());
  ...
  for (const auto& alias : aliases) {
    ...
    file.Write(&id_index, sizeof(id_index));
  }

  for (const auto& entry : resources) {
    file.Write(entry.second.data(), entry.second.length());
  }
