...
TEST_F(HoverButtonTest, CustomHitbox) {
  ...
  EXPECT_FALSE(NSPointInRect(inside_hit_point, button_.trackingAreas[0].rect));
  EXPECT_FALSE(NSPointInRect(outside_hit_point, button_.trackingAreas[0].rect));
  EXPECT_NE(button_, [button_ hitTest:inside_hit_point]);
  EXPECT_EQ(nil, [button_ hitTest:outside_hit_point]);

  button_.hitbox = hitbox;
  EXPECT_TRUE(NSPointInRect(inside_hit_point, button_.trackingAreas[0].rect));
  EXPECT_FALSE(NSPointInRect(outside_hit_point, button_.trackingAreas[0].rect));
