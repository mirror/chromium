...
std::unique_ptr<PlatformHook> PlatformHook::Create(
  ...
  return std::make_unique<PlatformHookWin>(std::move(callback));
}
...
// static
LRESULT CALLBACK PlatformHookWin::ProcessKeyEvent(int code,
                                                  WPARAM w_param,
                                                  LPARAM l_param) {
  ...
  if (code == HC_ACTION && !IsOSReservedKey(ll_hooks->vkCode)) {
    ...
    event.set_flags(event.flags() | ui::EF_PRE_PLATFORM_HANDLED);
