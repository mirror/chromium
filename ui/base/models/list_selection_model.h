...
class UI_BASE_EXPORT ListSelectionModel {
 public:
  ...
  // Invoked when an item moves. |from| is the original index, |to| is the
  // target index, and |length| is the number of items that are moving.
  //
  // NOTE: this matches the TabStripModel API. If moving to a greater index,
  // |to| should be the index *after* removing the elements at the index range
  // [from, from + length). For example, consider three tabs 'A B C', to move A
  // to the end of the list, this should be invoked with '0, 2, 1'.
