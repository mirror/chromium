...
void GetApplicationLocaleInternal(const std::string& pref_locale,
  ...
  *string_locale = l10n_util::GetLocaleOverride();
  LOG(INFO) << "string_locale from GetLocaleOverride is [" << *string_locale
            << "]";
  if (string_locale->empty()) {
    LOG(INFO) << "string_locale falls back to pref_locale, which is "
              << pref_locale;
  ...
  if (string_locale->empty()) {
    *string_locale = "en-US";
    LOG(INFO) << "string_locale falls back to en-US";
  }
  LOG(INFO) << "string_locale is set to " << *string_locale;

  *format_locale = l10n_util::GetFormatLocaleOverride();
  LOG(INFO) << "format_locale from GetFormatLocaleOverride is ["
            << *format_locale << "]";
  if (format_locale->empty())
    *format_locale = *string_locale;
  LOG(INFO) << "format_locale is set to " << *format_locale;
...
std::string GetApplicationLocale(const std::string& pref_locale,
  ...
  if (set_icu_locale && !format_locale.empty()) {
    LOG(INFO) << "ICU locale is set to " << format_locale;
