...
void ArcNotificationManager::OnNotificationPosted(
  ...
  if (it == items_.end()) {
    ...
    // pass notification id to GetAppIdToPackageNameMap.
    message_center_controller_->GetAppIdToPackageNameMap(
        this, data_ptr_->package_name.value(), key, item->GetNotificationId());
    // wait for callback.
    return;
  }
  ContinueNotificationPosted(key, std::string());
}

void ArcNotificationManager::ContinueNotificationPosted(
    const std::string& key,
    const std::string& app_id) {
  auto it = items_.find(key);

  LOG(ERROR) << "AppId at ContinueNotificationPosted: " << app_id;
