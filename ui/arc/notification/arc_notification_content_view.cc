...
bool ArcNotificationContentView::HandleAccessibleAction(
  ...
  if (item_ && action_data.action == ax::mojom::Action::DO_DEFAULT) {
    item_->ToggleExpansion();
    return true;
  }
  return false;
...
void ArcNotificationContentView::GetAccessibleNodeData(
  ...
  if (surface_ && surface_->GetAXTreeId() != -1) {
    ...
    node_data->AddIntAttribute(ax::mojom::IntAttribute::CHILD_TREE_ID,
                               surface_->GetAXTreeId());
  } else {
    node_data->role = ax::mojom::Role::BUTTON;
    node_data->AddStringAttribute(
        ax::mojom::StringAttribute::ROLE_DESCRIPTION,
...
void ArcNotificationContentView::OnNotificationSurfaceAdded(
  ...
  // Notify ax::mojom::Event::CHILDREN_CHANGED to force AXNodeData of this view updated.
  ...
  NotifyAccessibilityEvent(ax::mojom::Event::CHILDREN_CHANGED, false);
}
