...
TouchPoint::TouchPoint(gfx::Point location, wl_surface* current_surface)
...
void WaylandTouch::RemoveTouchPoints(const WaylandWindow* window) {
  for (const auto& point : current_points_) {
    if (point.second->surface == window->surface())
      current_points_.erase(point.first);
  }
}
...
void WaylandTouch::Down(void* data,
  ...
  touch->current_points_[id] = base::MakeUnique<TouchPoint>(location, surface);
}
