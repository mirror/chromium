      ...
      for (uint32_t j = 0; j < drm_plane_properties->count_props; j++) {
        ...
        if (strcmp(property->name, "IN_FORMATS") == 0) {
          ...
          struct drm_format_modifier_blob* header =
              reinterpret_cast<struct drm_format_modifier_blob*>(blob->data);
          uint32_t* formats =
              reinterpret_cast<uint32_t*>(data + header->formats_offset);
          struct drm_format_modifier* modifiers =
              reinterpret_cast<struct drm_format_modifier*>(
