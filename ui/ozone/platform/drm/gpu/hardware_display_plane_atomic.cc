...
bool HardwareDisplayPlaneAtomic::SetPlaneData(
  ...
  if (in_fence_fd_.id && render_fence_fd >= 0) {
    plane_set_succeeded =
        plane_set_succeeded &&
        drmModeAtomicAddProperty(property_set, plane_id_, in_fence_fd_.id,
                                 render_fence_fd);
  }
...
bool HardwareDisplayPlaneAtomic::InitializeProperties(
  ...
  in_fence_fd_.Initialize(drm, kInFenceFdPropName, plane_props);
  return true;
}
