...
void CrtcController::OnPageFlipEvent(unsigned int frame,
  ...
  const uint32_t kFlags = gfx::PresentationFlags::kVSync |
                          gfx::PresentationFlags::kHWClock |
                          gfx::PresentationFlags::kHWCompletion;
  SignalPageFlipRequest(
      gfx::SwapResult::SWAP_ACK,
      gfx::PresentationFeedback(
          time_of_last_flip_, base::TimeDelta::FromSeconds(1) / mode_.vrefresh,
          kFlags));
}
