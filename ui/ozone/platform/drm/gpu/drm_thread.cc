...
void DrmThread::SchedulePageFlip(gfx::AcceleratedWidget widget,
  ...
  base::OnceClosure planes_ready_callback = base::BindOnce(
      &DrmThread::OnPlanesReadyForPageFlip, weak_ptr_factory_.GetWeakPtr(),
      widget, planes, base::Passed(&callback));

  drm_device->plane_manager()->RequestPlanesReadyCallback(
      planes, std::move(planes_ready_callback));
}
