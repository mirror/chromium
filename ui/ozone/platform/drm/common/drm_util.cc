...
std::unique_ptr<display::DisplaySnapshot> CreateDisplaySnapshot(
  ...
  display::DisplayMode* current_mode = nullptr;
  display::DisplayMode* native_mode = nullptr;
  for (int i = 0; i < info->connector()->count_modes; ++i) {
    ...
    if (info->crtc()->mode_valid && SameMode(info->crtc()->mode, mode))
      current_mode =
          const_cast<display::DisplayMode*>((modes.end() - 1)->get());

    if (mode.type & DRM_MODE_TYPE_PREFERRED)
      native_mode = const_cast<display::DisplayMode*>((modes.end() - 1)->get());
  }
  ...
  if (!native_mode && !modes.empty())
    native_mode = const_cast<display::DisplayMode*>(modes.begin()->get());
