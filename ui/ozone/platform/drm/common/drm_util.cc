...
bool IsDisplayModeEqual(const DisplayMode_Params& lhs,
...
gfx::ColorSpace GetColorSpaceFromEdid(const std::vector<uint8_t>& edid) {
  ...
  // If |edid| has no Gamma, return a gfx::ColorSpace the standard sRGB of 2.2.
  double gamma = 0.0;
  if (!display::ParseGammaValue(edid, &gamma)) {
    return gfx::ColorSpace::CreateCustom(color_space_as_matrix,
                                         gfx::ColorSpace::TransferID::GAMMA22);
  }
  // SkColorSpaceTransferFn uses a number of terms in a particular order.
  return gfx::ColorSpace::CreateCustom(
      color_space_as_matrix,
      SkColorSpaceTransferFn({1.0 / gamma, 1.f, 0.f, 1.f, 0.f, 0.f, 0.f}));
}
...
std::unique_ptr<display::DisplaySnapshot> CreateDisplaySnapshot(
  ...
  for (const auto& mode : params.modes) {
    ...
    if (IsDisplayModeEqual(params.current_mode, mode))
      current_mode = modes.back().get();
    if (IsDisplayModeEqual(params.native_mode, mode))
      native_mode = modes.back().get();
  }
