...
GetAvailableDisplayControllerInfos(int fd) {
  ...
  std::vector<ScopedDrmConnectorPtr::element_type*> available_connectors;
  for (int i = 0; i < resources->count_connectors; ++i) {
    ...
    if (connector->connection == DRM_MODE_CONNECTED &&
      ...
      available_connectors.emplace_back(connector.get());
    }
  ...
  // fewest crtcs first.
  std::stable_sort(
      available_connectors.begin(), available_connectors.end(),
      [&connector_crtcs](ScopedDrmConnectorPtr::element_type* const c1,
                         ScopedDrmConnectorPtr::element_type* const c2) {
