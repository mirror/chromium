...
// Partially valid EDID: the byte containing gamma information (23rd) is 0xFF.
...
TEST_F(DrmUtilTest, GetColorSpaceFromEdid) {
  ...
  const gfx::ColorSpace hpz32x_color_space = gfx::ColorSpace::CreateCustom(
      hpz32x_toXYZ50_matrix, SkColorSpaceTransferFn({2.2, 1, 0, 1, 0, 0, 0}));
  ...
  const gfx::ColorSpace samus_color_space = gfx::ColorSpace::CreateCustom(
      samus_toXYZ50_matrix, SkColorSpaceTransferFn({2.5, 1, 0, 1, 0, 0, 0}));
  ...
  const gfx::ColorSpace eve_color_space = gfx::ColorSpace::CreateCustom(
      eve_toXYZ50_matrix, SkColorSpaceTransferFn({2.2, 1, 0, 1, 0, 0, 0}));
  ...
  EXPECT_TRUE(no_gamma_color_space.IsValid());

  SkColorSpaceTransferFn transfer;
  EXPECT_TRUE(no_gamma_color_space.GetTransferFunction(&transfer));
  EXPECT_EQ(1.0f, transfer.fG);
}
