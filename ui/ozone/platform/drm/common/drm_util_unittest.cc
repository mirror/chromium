...
// EDID collected in the wild: valid but with malformed primaries.
...
TEST_F(DrmUtilTest, GetInvalidColorSpaceFromEdid) {
  ...
  const std::vector<uint8_t> edid_sst210(kSST210, kSST210 + arraysize(kSST210));
  const gfx::ColorSpace sst210_color_space = GetColorSpaceFromEdid(edid_sst210);
