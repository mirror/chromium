...
TEST_F(X11WindowOzoneTest, SendPlatformEventToRightTarget) {
  ...
  auto window =
      CreatePlatformWindow(&delegate, gfx::Rect(10, 10, 100, 100), &widget);
  ...
  auto window_2 =
      CreatePlatformWindow(&delegate_2, gfx::Rect(5, 5, 101, 101), &widget_2);
  ...
  // Set capture to the first window, but send an event to another window
  // target. The event must be received by the captured window instead.
  window->SetCapture();
  EXPECT_CALL(delegate, DispatchEvent(_)).WillOnce(CloneEvent(&event));
  EXPECT_CALL(delegate_2, DispatchEvent(_)).Times(0);

  DispatchXEvent(xi_event, widget_2);
  EXPECT_FALSE(event_2);
  EXPECT_EQ(ui::ET_MOUSE_PRESSED, event->type());
