...
ManagedDisplayInfo* GetInternalDisplay(DisplayInfoPointerList* displays) {
  ...
  DisplayInfoPointerList::iterator display_it =
...
void AssociateToSingleDisplay(DisplayInfoPointerList* displays,
  ...
  std::size_t num_of_displays_excluding_internal = displays->size();
  ManagedDisplayInfo* internal_display = GetInternalDisplay(displays);
  if (internal_display)
    num_of_displays_excluding_internal--;

  // We only associate to one display.
  if (num_of_displays_excluding_internal != 1 || devices->empty())
    return;

  ManagedDisplayInfo* display = *displays->begin();
  if (display == internal_display)
    display = *(displays->begin() + 1);
...
void AssociateFromDisplayPreferences(DisplayInfoPointerList* displays,
  ...
  for (auto device_it = devices->begin(); device_it != devices->end();) {
    ...
    for (auto* new_display_info : *displays) {
      ...
      // Check the existing ManagedDisplayInfo for the given display, to see
      // get touch device associations saved in display preferences.
...
void AssociateRemainingDevicesToInternalDisplay(
  ...
  if (!internal_display) {
    ...
    VLOG(2) << "Could not find any intenrnal display. Matching all devices to "
            << " a non internal display.";
  }
...
void AssociateTouchscreens(
  ...
  // internal display.
  if (devices.size())
    AssociateRemainingDevicesToInternalDisplay(&displays, &devices);
}
