...
bool DisplayConfigurator::DisplayLayoutManagerImpl::FindExactMatchingMirrorMode(
  ...
  // Put each display's all display modes in |mode_lists| and sort the display
  // modes for each display by size area and refresh rate.
  std::vector<std::vector<const DisplayMode*>> mode_lists;
  for (auto* d : displays) {
    std::vector<const DisplayMode*> mode_list;
    for (auto& mode : d->display->modes()) {
      if (!preserve_aspect || (is_first_internal && d == displays[0]) ||
          IsNativeAspectRatio(d->display->native_mode(), mode.get())) {
