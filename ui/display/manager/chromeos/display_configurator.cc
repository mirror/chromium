...
class DisplayConfigurator::DisplayLayoutManagerImpl
 ...
 private:
  ...
  bool HasSameAspectRatioAsNativeMode(const DisplayMode* mode,
...
bool DisplayConfigurator::DisplayLayoutManagerImpl::
    HasSameAspectRatioAsNativeMode(const DisplayMode* mode,
                                   const DisplayMode* native_mode) const {
  ...
  for (auto* d : displays) {
    ...
    for (auto& mode : d->display->modes()) {
      if (d->display->type() != DISPLAY_CONNECTION_TYPE_INTERNAL &&
          ...
          !HasSameAspectRatioAsNativeMode(mode.get(),
                                          d->display->native_mode())) {
