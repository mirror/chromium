...
MultipleDisplayState DisplayChangeObserver::GetStateForDisplayIds(
  ...
  bool* previous_mirror_mode = display_manager_->previous_mirror_mode();
  if (!previous_mirror_mode) {
    // Mirror mode should be on if the registered mirror mode is on for current
    // display list.
    DisplayIdList list =
    ...
    return display_manager_->layout_store()->GetMirrorMode(list)
               ? MULTIPLE_DISPLAY_STATE_DUAL_MIRROR
               : MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED;
  }
  // Mirror mode should be on if mirror mode is on in the previous
  // configuration.
  return *previous_mirror_mode ? MULTIPLE_DISPLAY_STATE_DUAL_MIRROR
                               : MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED;
}
