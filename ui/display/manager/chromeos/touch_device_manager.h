...
class DISPLAY_MANAGER_EXPORT TouchDeviceIdentifier {
 public:
  TouchDeviceIdentifier(const ui::TouchscreenDevice& touch_device);
  TouchDeviceIdentifier(uint32_t identifier);
...
class DISPLAY_MANAGER_EXPORT TouchDeviceManager {
 public:
  struct TouchAssociationInfo {
    ...
    // The id of the display this calibration information is associated with.
    int64_t display_id;
  ...
  using TouchAssociationMap =
      std::map<TouchDeviceIdentifier, std::vector<TouchAssociationInfo>>;
  ...
  // touch. The associations will be stored in |active_touch_associations_|.
 ...
 private:
  ...
  // data. This may contain devices and displays that are not currently to the
  // system. This is a history of all calibration and association information
  // for this system.
  ...
  // items which are currently active.
  std::map<TouchDeviceIdentifier, int64_t> active_touch_associations_;

  // Is set when |active_touch_associations_| needs to be updated and may have
  // dirty values.
  bool active_touch_associations_dirty_ = false;
