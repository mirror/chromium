...
class DISPLAY_MANAGER_EXPORT ManagedDisplayInfo {
 public:
  ...
  // DEPRECATED: Use |SetTouchCalibrationData(uint32_t, TouchCalibrationData)|
  // instead. This method is only present to provide support for legacy
  // calibration data.
  void SetTouchCalibrationData(const TouchCalibrationData& calibration_data);

  // DEPRECATED: Use |GetTouchCalibrationData(uint32_t)| instead.
  TouchCalibrationData GetTouchCalibrationData() const & {
    return legacy_touch_calibration_data_;
  }

  // DEPRECATED: Use |HasTouchCalibrationData(uint32_t)| instead.
  bool has_legacy_touch_calibration_data() const {
    return has_legacy_touch_calibration_data_;
  }

  // DEPRECATED: Use |ClearTouchCalibrationData(uint32_t)| instead.
  void clear_touch_calibration_data() {
    has_legacy_touch_calibration_data_ = false;
  }
 ...
 private:
  ...
  // DEPRECATED: Do not use this for any future changes.
  bool has_legacy_touch_calibration_data_;
  ...
  // DEPRECATED: Do not use this for any future changes.
  TouchCalibrationData legacy_touch_calibration_data_;
