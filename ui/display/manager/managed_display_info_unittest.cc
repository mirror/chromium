...
TEST_F(DisplayInfoTest, LegacyTouchCalibrationTest) {
  ManagedDisplayInfo info =
      ManagedDisplayInfo::CreateFromSpecWithID("200x100", 10);

  EXPECT_FALSE(info.has_legacy_touch_calibration_data());
  ...
  EXPECT_TRUE(info.has_legacy_touch_calibration_data());
