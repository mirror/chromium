...
TEST(JsonConverterTest, JsonFromToDisplayLayout) {
  ...
  layout.primary_id = 1;
  layout.mirroring_source_id = 1;
  layout.mirroring_destination_ids.push_back(2);
  layout.mirroring_destination_ids.push_back(3);
  ...
  const char data[] =
      "{\n"
      "  \"primary-id\": \"1\",\n"
      "  \"mirroring_source_id\": \"1\",\n"
      "  \"mirroring_destination_ids\": [\"2\",\"3\"],\n"
  ...
  EXPECT_EQ(read_layout.mirroring_source_id, layout.mirroring_source_id);
  EXPECT_EQ(read_layout.mirroring_destination_ids,
...
TEST(JsonConverterTest, OldJsonToDisplayLayout) {
  ...
  DisplayLayout read_layout;
  EXPECT_TRUE(JsonToDisplayLayout(*read_value, &read_layout));
  EXPECT_EQ(1, read_layout.mirroring_source_id);
  DisplayIdList expected_destination_ids;
  expected_destination_ids.emplace_back(2);
  expected_destination_ids.emplace_back(3);
  EXPECT_EQ(expected_destination_ids, read_layout.mirroring_destination_ids);
