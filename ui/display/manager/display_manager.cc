...
bool DisplayManager::ShouldSetMirrorModeOn(const DisplayIdList& id_list) {
  ...
  if (should_restore_mirror_mode_from_prefs_) {
    ...
    for (int64_t id : id_list) {
      if (!Display::IsInternalDisplayId(id) &&
          external_display_mirror_info_.count(id)) {
        return true;
      }
    }
    return false;
  }
  // Mirror mode should remain unchanged.
...
void DisplayManager::UpdateInfoForRestoringMirrorMode() {
  if (num_connected_displays_ <= 1) {
    should_restore_mirror_mode_from_prefs_ = true;
    return;
  }
  should_restore_mirror_mode_from_prefs_ = false;

  for (auto id : GetCurrentDisplayIdList()) {
    if (IsInMirrorMode())
      external_display_mirror_info_.emplace(id);
    else
      external_display_mirror_info_.erase(id);
  }
}
