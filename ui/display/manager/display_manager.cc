...
gfx::Size DisplayManager::GetUnifiedDesktopMatrixDimensions() const {
  ...
  return gfx::Size(current_matrix_.size(), current_matrix_[0].size());
}
...
void DisplayManager::CreateUnifiedDesktopDisplayInfo(
  ...
  if (current_matrix_.empty() || current_matrix_[0].empty() ||
    ...
    // Create default matrix where displays are laid out horizontally from left
    // to right.
  ...
  for (const auto& row : current_matrix_) {
    ...
    for (const auto& id : row) {
      ...
      if (Display::IsInternalDisplayId(id) && default_mode_bounds.IsEmpty()) {
  ...
  *iter = base::MakeRefCounted<ManagedDisplayMode>(
  ...
  // 5 - Now that we know everything, Create the mirroring displays with their
  //     final bounds and store them in |software_mirroring_display_list_|.
  last_bottom = 0;
  for (size_t i = 0; i < row_size; ++i) {
    const auto& row = current_matrix_[i];
    int last_right = 0;
    ...
    for (size_t j = 0; j < column_size; ++j) {
      ...
      const float scale =
          info->size_in_pixel().GetArea() * row_size * column_size /
          static_cast<float>(unified_display_info.size_in_pixel().GetArea());
      const gfx::Point origin(last_right, last_bottom);
      // The display is scaled to fit the unified desktop size.
      Display display =
          CreateMirroringDisplayFromDisplayInfoById(id, origin, 1.0f / scale);
