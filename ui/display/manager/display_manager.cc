...
const ManagedDisplayMode* GetDefaultDisplayMode(
    const ManagedDisplayInfo& info) {
...
const ManagedDisplayMode* DisplayManager::GetActiveModeForDisplayId(
    int64_t display_id) const {
  const ManagedDisplayMode* selected_mode =
  ...
  for (auto iter = display_modes.begin(); iter != display_modes.end(); ++iter) {
    if (GetDisplayIdForUIScaling() == display_id) {
      if (info.configured_ui_scale() == iter->ui_scale())
        return &(*iter);
    } else if (iter->native()) {
      return &(*iter);
    }
...
const ManagedDisplayMode* DisplayManager::GetSelectedModeForDisplayId(
    int64_t id) const {
...
bool DisplayManager::ResetDisplayToDefaultMode(int64_t id) {
  ...
  const ManagedDisplayMode* mode = GetDefaultDisplayMode(info);
