...
// Validates the matrix is not empty and all rows have equal non-zero widths.
bool ValidateMatrixDimensions(const UnifiedDesktopLayoutMatrix& matrix) {
  if (matrix.empty())
    return false;

  const size_t row_size = matrix[0].size();
  if (row_size == 0)
    return false;
...
void DisplayManager::CreateUnifiedDesktopDisplayInfo(
  ...
  if (!ValidateMatrixDimensions(current_matrix_) ||
  ...
  // 3 - Using the total bounds of all rows, calculate the maximum width of all
  //     rows.
  int max_total_width = std::numeric_limits<int>::min();
  for (const auto& row_bound : rows_bounds)
    max_total_width = std::max(max_total_width, row_bound.width());

  // 4 - Using the maximum row width, adjust the display scales so that the each
  ...
  // 5 - Compute the unified display size by computing the unified display size
  ...
  modes_param_list.reserve(row_size * column_size);
  ...
  for (size_t i = 0; i < row_size; ++i) {
    ...
    for (size_t j = 0; j < column_size; ++j) {
      ...
      if (Display::IsInternalDisplayId(id))
        internal_display_index = i * column_size + j;
  ...
  // 6 - Create the Unified display info and its modes.
  ...
  if (default_mode_iter != modes.end()) {
    const scoped_refptr<ManagedDisplayMode>& default_mode = *default_mode_iter;
  ...
  // 8 - Now that we know the final unified display bounds, update the displays
  ...
  for (size_t i = 0; i < row_size; ++i) {
    ...
    for (size_t j = 0; j < column_size; ++j) {
      ...
      current_display.set_bounds(
          gfx::Rect(origin, gfx::ToCeiledSize(scaled_size)));
