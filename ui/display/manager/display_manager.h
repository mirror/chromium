...
#endif
{
 public:
  ...
  const MixedMirrorModeParams* mixed_mirror_mode_params() const {
    return mixed_mirror_mode_params_.get();
  }
  ...
  void set_mixed_mirror_mode_params(
      std::unique_ptr<MixedMirrorModeParams> mixed_params) {
  ...
  // Turn on/off the mirror mode. Ignore |mixed_params| if |mirrored| is
  ...
  void SetMirrorMode(
      bool mirrored,
      std::unique_ptr<MixedMirrorModeParams> mixed_params = nullptr);
...
// SoftwareMirroringController override:
 ...
 private:
  ...
  // mirror mode should not be turned.
  std::unique_ptr<MixedMirrorModeParams> mixed_mirror_mode_params_;
