...
#endif
{
 public:
  ...
  void set_should_restore_mirror_mode_from_prefs_for_test(bool should_restore) {
    should_restore_mirror_mode_from_prefs_ = should_restore;
  }

  bool should_restore_mirror_mode_from_prefs() const {
    return should_restore_mirror_mode_from_prefs_;
  }
...
// SoftwareMirroringController override:
 ...
 private:
  ...
  // True if mirror mode should be restored from preferences (When the Chrome OS
  // just boots up or we just connected the second display).
  bool should_restore_mirror_mode_from_prefs_ = true;
