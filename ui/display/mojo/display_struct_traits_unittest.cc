...
void CheckDisplayModesEqual(const DisplayMode* lhs, const DisplayMode* rhs) {
  if (lhs == nullptr && rhs == nullptr)
    return;

  EXPECT_NE(lhs, rhs);  // Make sure they aren't the same object.
  EXPECT_EQ(lhs->size(), rhs->size());
  EXPECT_EQ(lhs->is_interlaced(), rhs->is_interlaced());
  EXPECT_EQ(lhs->refresh_rate(), rhs->refresh_rate());
}
