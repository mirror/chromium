...
const static float kPrimariesPrecision = 0.0001;
...
::testing::AssertionResult SkColorSpacePrimariesEquals(
  ...
  if (cc::MathUtil::ApproximatelyEqual(lhs.fRX, rhs.fRX, kPrimariesPrecision))
    return AssertionFailure() << "fRX: " << lhs.fRX << " != " << rhs.fRX;
  if (cc::MathUtil::ApproximatelyEqual(lhs.fRY, rhs.fRY, kPrimariesPrecision))
    return AssertionFailure() << "fRY: " << lhs.fRY << " != " << rhs.fRY;
  if (cc::MathUtil::ApproximatelyEqual(lhs.fGX, rhs.fGX, kPrimariesPrecision))
    return AssertionFailure() << "fGX: " << lhs.fGX << " != " << rhs.fGX;
  if (cc::MathUtil::ApproximatelyEqual(lhs.fGY, rhs.fGY, kPrimariesPrecision))
    return AssertionFailure() << "fGY: " << lhs.fGY << " != " << rhs.fGY;
  if (cc::MathUtil::ApproximatelyEqual(lhs.fBX, rhs.fBX, kPrimariesPrecision))
    return AssertionFailure() << "fBX: " << lhs.fBX << " != " << rhs.fBX;
  if (cc::MathUtil::ApproximatelyEqual(lhs.fBY, rhs.fBY, kPrimariesPrecision))
    return AssertionFailure() << "fBY: " << lhs.fBY << " != " << rhs.fBY;
  if (cc::MathUtil::ApproximatelyEqual(lhs.fWX, rhs.fWX, kPrimariesPrecision))
    return AssertionFailure() << "fWX: " << lhs.fWX << " != " << rhs.fWX;
  if (cc::MathUtil::ApproximatelyEqual(lhs.fWY, rhs.fWY, kPrimariesPrecision))
...
TEST(EDIDParserTest, ParseChromaticityCoordinates) {
  ...
  const SkColorSpacePrimaries kNormalDisplayPrimaries = {
      0.6777, 0.3086, 0.2080, 0.6923, 0.1465, 0.0546, 0.3125, 0.3291};
  ...
  const SkColorSpacePrimaries kInternalDisplayPrimaries = {
      0.5849, 0.3603, 0.3769, 0.5654, 0.1552, 0.0996, 0.3125, 0.3291};
  ...
  // any standard color gamut.
  const SkColorSpacePrimaries kHz32xPrimaries = {
  ...
  const SkColorSpacePrimaries kSamusPrimaries = {
      0.6337, 0.3476, 0.3212, 0.5771, 0.1513, 0.0908, 0.3144, 0.3291};
  ...
  const SkColorSpacePrimaries kEvePrimaries = {0.6396, 0.3291, 0.2998, 0.5996,
                                               0.1494, 0.0595, 0.3144, 0.3281};
