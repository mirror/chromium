...
// Chromaticies in EDID are specified with 10 bits precision.
const static float kChromaticityPrimariesPrecision = 0.0001;
...
::testing::AssertionResult SkColorSpacePrimariesEquals(
  ...
  if (fabs(lhs.fRX - rhs.fRX) > kChromaticityPrimariesPrecision)
    return AssertionFailure() << "fRX: " << lhs.fRX << " != " << rhs.fRX;
  if (fabs(lhs.fRY - rhs.fRY) > kChromaticityPrimariesPrecision)
    return AssertionFailure() << "fRY: " << lhs.fRY << " != " << rhs.fRY;
  if (fabs(lhs.fGX - rhs.fGX) > kChromaticityPrimariesPrecision)
    return AssertionFailure() << "fGX: " << lhs.fGX << " != " << rhs.fGX;
  if (fabs(lhs.fGY - rhs.fGY) > kChromaticityPrimariesPrecision)
    return AssertionFailure() << "fGY: " << lhs.fGY << " != " << rhs.fGY;
  if (fabs(lhs.fBX - rhs.fBX) > kChromaticityPrimariesPrecision)
    return AssertionFailure() << "fBX: " << lhs.fBX << " != " << rhs.fBX;
  if (fabs(lhs.fBY - rhs.fBY) > kChromaticityPrimariesPrecision)
    return AssertionFailure() << "fBY: " << lhs.fBY << " != " << rhs.fBY;
  if (fabs(lhs.fWX - rhs.fWX) > kChromaticityPrimariesPrecision)
    return AssertionFailure() << "fWX: " << lhs.fWX << " != " << rhs.fWX;
  if (fabs(lhs.fWY - rhs.fWY) > kChromaticityPrimariesPrecision)
