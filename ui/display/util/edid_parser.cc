...
DISPLAY_UTIL_EXPORT double GetGammaValue(const std::vector<uint8_t>& edid) {
  // Constants are taken from [1].
  // [1] "VESA Enhanced EDID Standard " Release A, Revision 2, September 25,
  // 2006, Sec. 3.6.3 "Display Transfer Characteristics (GAMMA ): 1 Byte"
  const unsigned int kGammaOffset = 23;
...
DISPLAY_UTIL_EXPORT int GetBitsPerChannel(const std::vector<uint8_t>& edid) {
  // Offsets, masks and the table are taken from [1].
  // [1] "VESA Enhanced EDID Standard " Release A, Revision 1, Feb 2000, Sec 3.6
  ...
  const unsigned int kVideoInputDefinitionOffset = 20;
  ...
  // the Video Input Definition register to describe a digital interface.
  if (edid[kEDIDRevisionNumberOffset] < kEDIDRevision4Value ||
      !(edid[kVideoInputDefinitionOffset] & kDigitalInfoMask)) {
    return 0;
  }

  DVLOG(1) << "bits per primary colour "
           << kBitsPerChannelTable[(
