...
void AppListModel::RecordItemsInFoldersForUMA() {
  ...
  for (size_t i = 0; i < top_level_item_list_->item_count(); i++) {
    ...
    if (item->GetItemType() == AppListFolderItem::kItemType)
      continue;
    AppListFolderItem* folder = static_cast<AppListFolderItem>(item);
    number_of_items_in_folders += folder->ChildItemCount();
  }
  char histogram_name[] = "Apps.AppsInFolders";
  if (features::IsFullscreenAppListEnabled())
    histogram_name += ".FullscreenAppListEnabled";
  UMA_HISTOGRAM_COUNTS_100(histogram_name, number_of_items_in_folders);
}
