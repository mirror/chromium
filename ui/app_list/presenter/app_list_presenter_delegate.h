...
class APP_LIST_PRESENTER_EXPORT AppListPresenterDelegate {
 public:
  ...
  virtual int GetVisibilityAnimationDuration(aura::Window* root_window,
                                             bool is_visible) = 0;
 ...
 protected:
  ...
  const int kAnimationDurationMsFullscreenMs = 300;

  // Duration for hide animation for fullscreen app list while in side shelf.
  const int kAnimationDurationMsSideShelfFullscreenMs = 150;
  ...
  const int kAnimationOffsetFullscreen = 400;
