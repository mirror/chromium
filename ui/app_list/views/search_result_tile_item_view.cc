...
// The two ways of displaying suggested apps. This enum must not have its
// order altered as it is used in histograms.
enum AppListSuggestedAppLaunched {
  kFullscreenAppListEnabled = 0,
  kFullscreenAppListDisabled = 1,
  kMaxAppListSuggestedAppLaunched = 3,
};
...
void SearchResultTileItemView::ButtonPressed(views::Button* sender,
  ...
  if (is_suggested_app_) {
    UMA_HISTOGRAM_ENUMERATION(kAppListLaunchSuggestedAppHistogram,
                              is_fullscreen_app_list_enabled()
                                  ? kFullscreenAppListEnabled
                                  : kFullscreenAppListDisabled,
                              kMaxAppListSuggestedAppLaunched);
  }
