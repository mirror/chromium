...
void AppListView::InitChildWidgets() {
  ...
  search_box_widget_->GetNativeWindow()->SetProperty(
      ui::kAXRoleOverride,
      static_cast<ax::mojom::Role>(ax::mojom::Role::GROUP));
...
ax::mojom::Role AppListView::GetAccessibleWindowRole() const {
  // Default role of root view is ax::mojom::Role::WINDOW which traps ChromeVox
  // focus within the root view. Assign ax::mojom::Role::GROUP here to allow the
  // focus to move from elements in app list view to search box.
...
void AppListView::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  ...
  node_data->role = ax::mojom::Role::DESKTOP;
}
