...
void ContentsView::Init(AppListModel* model) {
  ...
  // fullscreen app list.
  AddLauncherPage(apps_container_view_, ash::AppListState::STATE_START);
  ...
  AddLauncherPage(apps_container_view_, ash::AppListState::STATE_APPS);

  int initial_page_index = GetPageIndexForState(ash::AppListState::STATE_START);
...
ash::AppListState ContentsView::GetStateForPageIndex(int index) const {
  ...
  if (it == view_to_state_.end())
    return ash::AppListState::INVALID_STATE;
...
void ContentsView::ActivePageChanged() {
  ash::AppListState state = ash::AppListState::INVALID_STATE;
  ...
  const bool folder_active = state == ash::AppListState::STATE_APPS &&
...
void ContentsView::ShowSearchResults(bool show) {
  int search_page =
      GetPageIndexForState(ash::AppListState::STATE_SEARCH_RESULTS);
...
bool ContentsView::IsShowingSearchResults() const {
  return IsStateActive(ash::AppListState::STATE_SEARCH_RESULTS);
}
...
bool ContentsView::Back() {
  ...
  switch (state) {
    ...
    case ash::AppListState::STATE_APPS:
      if (apps_container_view_->IsInFolderView()) {
        ...
        is_fullscreen_app_list_enabled_
            ...
            : SetActiveState(ash::AppListState::STATE_START);
      }
      break;
    ...
    case ash::AppListState::INVALID_STATE:  // Falls through.
      NOTREACHED();
      break;
  }
  return true;
