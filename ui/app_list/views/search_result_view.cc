...
void SearchResultView::CreateTitleRenderText() {
  ...
  const bool double_rows_url =
      result_->is_omnibox_url() && !result_->details().empty();
...
void SearchResultView::CreateDetailsRenderText() {
  ...
  render_text->SetText(result_->is_omnibox_url() ? result_->title()
                                                 : result_->details());
  ui::ResourceBundle& rb = ui::ResourceBundle::GetSharedInstance();
  render_text->SetFontList(rb.GetFontList(ui::ResourceBundle::BaseFont));
  render_text->SetColor(result_->is_omnibox_url() ? kUrlColor
                                                  : kDefaultTextColor);
