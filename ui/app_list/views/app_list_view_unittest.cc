...
class AppListViewTest : public views::ViewsTestBase {
 ...
 protected:
  ...
  bool SetAppListState(ash::mojom::AppListState state) {
  ...
  bool IsStateShown(ash::mojom::AppListState state) {
    ContentsView* contents_view = view_->app_list_main_view()->contents_view();
    bool success = true;
...
TEST_F(AppListViewTest, SearchBoxCornerRadiusDuringDragging) {
  ...
  EXPECT_TRUE(IsStateShown(ash::mojom::AppListState::STATE_APPS));
  ...
  // during drag.
  EXPECT_TRUE(SetAppListState(ash::mojom::AppListState::STATE_SEARCH_RESULTS));
  ...
  EXPECT_TRUE(SetAppListState(ash::mojom::AppListState::STATE_APPS));
  timestamp += base::TimeDelta::FromMilliseconds(25);
...
TEST_F(AppListViewTest, DisplayTest) {
  ...
  ash::mojom::AppListState expected = ash::mojom::AppListState::STATE_START;
...
TEST_F(AppListViewTest, StartPageTest) {
  ...
  EXPECT_TRUE(SetAppListState(ash::mojom::AppListState::STATE_START));
...
TEST_F(AppListViewTest, PageSwitchingAnimationTest) {
  ...
  contents_view->SetActiveState(ash::mojom::AppListState::STATE_START);
  contents_view->Layout();

  IsStateShown(ash::mojom::AppListState::STATE_START);

  // Change pages. View should not have moved without Layout().
  contents_view->SetActiveState(ash::mojom::AppListState::STATE_SEARCH_RESULTS);
  IsStateShown(ash::mojom::AppListState::STATE_START);

  // Change to a third page. This queues up the second animation behind the
  // first.
  contents_view->SetActiveState(ash::mojom::AppListState::STATE_APPS);
  IsStateShown(ash::mojom::AppListState::STATE_START);

  // Call Layout(). Should jump to the third page.
  contents_view->Layout();
  IsStateShown(ash::mojom::AppListState::STATE_APPS);
}
...
TEST_F(AppListViewTest, SearchResultsTest) {
  ...
  EXPECT_TRUE(IsStateShown(ash::mojom::AppListState::STATE_SEARCH_RESULTS));
  ...
  EXPECT_TRUE(SetAppListState(ash::mojom::AppListState::STATE_START));
  view_->Layout();
  EXPECT_TRUE(IsStateShown(ash::mojom::AppListState::STATE_START));
...
TEST_F(AppListViewTest, BackTest) {
  ...
  // Show the apps grid.
  SetAppListState(ash::mojom::AppListState::STATE_APPS);
