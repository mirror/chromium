...
TEST_F(AppListViewFullscreenTest, StartPageTabFocusTest) {
  ...
  // Hitting tab key when focus is on expand arrow clears focus.
  search_box_view()->search_box()->OnKeyEvent(&tab);
  EXPECT_EQ(FOCUS_NONE, search_box_view()->get_focused_view_for_test());
  EXPECT_EQ(StartPageView::kNoSelection,
            start_page_view()->GetSelectedIndexForTest());
...
TEST_F(AppListViewFullscreenTest, StartPageShiftTabFocusTest) {
  ...
  // Hitting shift+tab key sets focus on expand arrow.
  ...
  // Hitting shift+tab key when focus is on search box moves focus through
  // suggestion apps reversely.
  for (size_t i = 0; i < apps_num; i++) {
    search_box_view()->search_box()->OnKeyEvent(&shift_tab);
    EXPECT_EQ(FOCUS_CONTENTS_VIEW,
              search_box_view()->get_focused_view_for_test());
  ...
  // Hitting shift+tab key when focus is on the first suggestion app moves focus
  // to search box.
  search_box_view()->search_box()->OnKeyEvent(&shift_tab);
  EXPECT_EQ(FOCUS_SEARCH_BOX, search_box_view()->get_focused_view_for_test());
  EXPECT_EQ(StartPageView::kNoSelection,
  ...
  // Hitting shift+tab key when focus is on search box clears focus.
  search_box_view()->search_box()->OnKeyEvent(&shift_tab);
  EXPECT_EQ(FOCUS_NONE, search_box_view()->get_focused_view_for_test());
  EXPECT_EQ(StartPageView::kNoSelection,
            start_page_view()->GetSelectedIndexForTest());
...
TEST_F(AppListViewFullscreenTest, StartPageRightArrowFocusTest) {
  ...
  // Hitting right key sets focus on search box.
  ui::KeyEvent right(ui::ET_KEY_PRESSED, ui::VKEY_RIGHT, ui::EF_NONE);
  search_box_view()->search_box()->OnKeyEvent(&right);
  EXPECT_EQ(FOCUS_SEARCH_BOX, search_box_view()->get_focused_view_for_test());
  EXPECT_EQ(StartPageView::kNoSelection,
            start_page_view()->GetSelectedIndexForTest());

  // Hitting right key when focus is on search box moves focus through
  // suggestion apps.
  for (size_t i = 0; i < apps_num; i++) {
    search_box_view()->search_box()->OnKeyEvent(&right);
    EXPECT_EQ(FOCUS_CONTENTS_VIEW,
  ...
  // Hitting right key when focus is on the last suggestion app moves focus to
  // expand arrow.
  search_box_view()->search_box()->OnKeyEvent(&right);
  EXPECT_EQ(FOCUS_CONTENTS_VIEW,
            search_box_view()->get_focused_view_for_test());
  ...
  // Hitting right key when focus is on expand arrow clears focus.
  search_box_view()->search_box()->OnKeyEvent(&right);
  EXPECT_EQ(FOCUS_NONE, search_box_view()->get_focused_view_for_test());
  EXPECT_EQ(StartPageView::kNoSelection,
            start_page_view()->GetSelectedIndexForTest());
...
TEST_F(AppListViewFullscreenTest, StartPageLeftArrowFocusTest) {
  ...
  // Hitting left key sets focus on expand arrow.
  ui::KeyEvent left(ui::ET_KEY_PRESSED, ui::VKEY_LEFT, ui::EF_NONE);
  search_box_view()->search_box()->OnKeyEvent(&left);
  EXPECT_EQ(FOCUS_CONTENTS_VIEW,
            search_box_view()->get_focused_view_for_test());
  EXPECT_EQ(StartPageView::kExpandArrowSelection,
            start_page_view()->GetSelectedIndexForTest());

  // Hitting left key when focus is on search box moves focus through suggestion
  // apps reversely.
  for (size_t i = 0; i < apps_num; i++) {
    search_box_view()->search_box()->OnKeyEvent(&left);
    EXPECT_EQ(FOCUS_CONTENTS_VIEW,
  ...
  // Hitting left key when focus is on the first suggestion app moves focus to
  // search box.
  search_box_view()->search_box()->OnKeyEvent(&left);
  EXPECT_EQ(FOCUS_SEARCH_BOX, search_box_view()->get_focused_view_for_test());
  EXPECT_EQ(StartPageView::kNoSelection,
  ...
  // Hitting left key when focus is on search box clears focus.
  search_box_view()->search_box()->OnKeyEvent(&left);
  EXPECT_EQ(FOCUS_NONE, search_box_view()->get_focused_view_for_test());
  EXPECT_EQ(StartPageView::kNoSelection,
            start_page_view()->GetSelectedIndexForTest());
