...
class AppListViewTest : public views::ViewsTestBase,
 ...
 public:
  ...
  void SetUp() override {
    ...
    // If the current test is not parameterized, that is testing for fullscreen
    // app list, otherwise for both bubble and fullscreen app list.
...
// Tests that the back button and Esc navigate through the app list correctly.
// This test is for bubble app list.
// TODO(warx): test coverage for fullscreen app list.
TEST_P(AppListViewTest, BackTest_bubble) {
  if (test_with_fullscreen_)
    return;
  EXPECT_FALSE(view_->GetWidget()->IsVisible());
  EXPECT_EQ(-1, GetPaginationModel()->total_pages());
  ...
  view_->GetWidget()->Show();
  SearchBoxView* search_box_view = main_view_->search_box_view();
  ...
  EXPECT_TRUE(contents_view_->Back());
  contents_view_->Layout();
  EXPECT_TRUE(IsStateShown(AppListModel::STATE_START));
