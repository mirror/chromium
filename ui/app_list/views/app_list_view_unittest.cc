...
TEST_F(AppListViewTest, SearchBoxCornerRadiusDuringDragging) {
  ...
  EXPECT_TRUE(IsStateShown(ash::AppListState::STATE_APPS));
  ...
  // during drag.
  EXPECT_TRUE(SetAppListState(ash::AppListState::STATE_SEARCH_RESULTS));
  EXPECT_TRUE(view_->is_in_drag());
  EXPECT_EQ(kSearchBoxBorderCornerRadius,
            search_box_view()->GetSearchBoxBorderCornerRadiusForState(
                ash::AppListState::STATE_SEARCH_RESULTS));

  // Ends to drag the launcher.
  EXPECT_TRUE(SetAppListState(ash::AppListState::STATE_APPS));
  timestamp += base::TimeDelta::FromMilliseconds(25);
...
TEST_F(AppListViewTest, DisplayTest) {
  ...
  ash::AppListState expected = ash::AppListState::STATE_START;
...
TEST_F(AppListViewTest, StartPageTest) {
  ...
  EXPECT_TRUE(SetAppListState(ash::AppListState::STATE_START));
...
TEST_F(AppListViewTest, PageSwitchingAnimationTest) {
  ...
  contents_view->SetActiveState(ash::AppListState::STATE_START);
  contents_view->Layout();

  IsStateShown(ash::AppListState::STATE_START);

  // Change pages. View should not have moved without Layout().
  contents_view->SetActiveState(ash::AppListState::STATE_SEARCH_RESULTS);
  IsStateShown(ash::AppListState::STATE_START);

  // Change to a third page. This queues up the second animation behind the
  // first.
  contents_view->SetActiveState(ash::AppListState::STATE_APPS);
  IsStateShown(ash::AppListState::STATE_START);

  // Call Layout(). Should jump to the third page.
  contents_view->Layout();
  IsStateShown(ash::AppListState::STATE_APPS);
}
...
TEST_F(AppListViewTest, SearchResultsTest) {
  ...
  contents_view->Layout();
  EXPECT_TRUE(
      contents_view->IsStateActive(ash::AppListState::STATE_SEARCH_RESULTS));

  EXPECT_TRUE(IsStateShown(ash::AppListState::STATE_SEARCH_RESULTS));
  ...
  EXPECT_TRUE(SetAppListState(ash::AppListState::STATE_START));
  view_->Layout();
  EXPECT_TRUE(IsStateShown(ash::AppListState::STATE_START));
...
TEST_F(AppListViewTest, BackTest) {
  ...
  // Show the apps grid.
  SetAppListState(ash::AppListState::STATE_APPS);
