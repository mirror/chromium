...
TEST_F(AppListViewFullscreenTest, StartPageTabFocusTest) {
  ...
  const int apps_num = 3;
  for (int i = 0; i < apps_num; i++)
    model->results()->Add(base::MakeUnique<TestStartPageSearchResult>());
  EXPECT_TRUE(SetAppListState(AppListModel::STATE_START));
  start_page_view->UpdateForTesting();
  EXPECT_EQ(3u, GetVisibleViews(start_page_view->tile_views()));

  // Initial focus should on search box text field.
  EXPECT_TRUE(search_box_view->get_focused_view_for_test() == FOCUS_SEARCH_BOX);
  EXPECT_EQ(-1, start_page_view->GetSelectedIndexForTest());
  ...
  for (int i = 0; i < apps_num; i++) {
    ...
    EXPECT_TRUE(search_box_view->get_focused_view_for_test() ==
                FOCUS_CONTENTS_VIEW);
    EXPECT_EQ(i, start_page_view->GetSelectedIndexForTest());
  }
  ...
  EXPECT_TRUE(search_box_view->get_focused_view_for_test() ==
              FOCUS_CONTENTS_VIEW);
  EXPECT_EQ(-2, start_page_view->GetSelectedIndexForTest());
