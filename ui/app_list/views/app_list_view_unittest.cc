...
TEST_F(AppListViewFullscreenTest, TypingPeekingToHalf) {
  ...
  base::string16 new_search_text = base::UTF8ToUTF16("nice");
...
TEST_F(AppListViewFullscreenTest, EscapeKeyHalfToPeeking) {
  ...
  AppListMainView* main_view = view_->app_list_main_view();
  SearchBoxView* search_box_view = main_view->search_box_view();
  base::string16 new_search_text = base::UTF8ToUTF16("doggie");
...
// Tests that pressing escape when in fullscreen changes the state to peeking.
TEST_F(AppListViewFullscreenTest, EscapeKeyFullscreenToPeeking) {
  Initialize(0, false, false);
  view_->SetState(AppListView::FULLSCREEN_ALL_APPS);
