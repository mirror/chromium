...
void FolderHeaderView::Update() {
  ...
  if (folder_name_visible_) {
    ...
    // Update the folder name if it is shortened.
    if (elided_folder_name != folder_name) {
      folder_item_->RemoveObserver(this);
      delegate_->SetItemName(folder_item_,
                             base::UTF16ToUTF8(elided_folder_name));
      folder_item_->AddObserver(this);
    }
  }
...
void FolderHeaderView::ContentsChanged(views::Textfield* sender,
  ...
  std::string folder_name =
      base::UTF16ToUTF8(GetElidedFolderName(folder_name_view_->text()));
  if (folder_name != folder_item_->name())
    delegate_->SetItemName(folder_item_, folder_name);
  folder_item_->AddObserver(this);
