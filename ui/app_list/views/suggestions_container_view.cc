...
SearchResultTileItemView* SuggestionsContainerView::GetTileViewAt(int index) {
...
void SuggestionsContainerView::UpdateSelectedIndex(int old_selected,
  ...
  if (old_selected >= 0 && old_selected < num_results())
    GetTileViewAt(old_selected)->SetSelected(false);

  if (new_selected >= 0 && new_selected < num_results())
    GetTileViewAt(new_selected)->SetSelected(true);
}
