...
bool AppsGridView::OnKeyPressed(const ui::KeyEvent& event) {
  if (is_app_list_focus_enabled_) {
    ...
    // focused app is the 3rd app in |suggestions_container_|, the global index
    // is 2. If focused app is 3rd app in 2nd page, the global index is
    // cols_*rows_per_page_+2, no matter how many apps are in
    // |suggestions_container_|. In folder view, since we do not have
    // |suggestions_container_|, the global index is equal to the focused view's
    // index in |view_model_|.
    int global_index;

    // |suggestions_container_| does not exist in folder.
    const int suggestions_app_max_num = suggestions_container_ ? cols_ : 0;

    // Calculate current global focus index.
    if (suggestions_container_ &&
        suggestions_container_->Contains(focused_view)) {
    ...
    if (target_global_index >= tile_total &&
    ...
    } else if (suggestions_container_ && target_global_index >= 0 &&
      ...
      target_global_index =
          std::min(suggestions_container_->num_results(), target_global_index);
    }
    ...
    } else if (suggestions_container_ &&
...
int AppsGridView::CalculateNextIndex(int current, bool arrow_up) {
  const int tile_total =
      view_model_.view_size() + (suggestions_container_ ? cols_ : 0);
  const int row_total = tile_total / cols_ + ((tile_total % cols_) ? 1 : 0);
  int next = current + (arrow_up ? -cols_ : cols_);

  if (next < 0 || next >= cols_ * row_total) {
    return -1;
  }
  if (next >= tile_total) {
    next = tile_total - 1;
  ...
  if (next < cols_) {
    next = std::min(suggestions_container_->num_results(), next);
  }
  return next;
}
