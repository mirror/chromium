...
// The index offset of an AppLitItemView as a child in the AppsGridView. This is
// used to correct the focus movement after an item is dragged. Suggestions
// container, all apps indicator and expand arrow view are added before app
// list item view in the root grid view. (Update the number below when a new
// view is added.)
constexpr int kAppListItemViewIndexOffsetInRootGridView = 3;
...
size_t AppsGridView::GetAppListItemViewIndexOffset() {
  return folder_delegate_ ? 0 : kAppListItemViewIndexOffsetInRootGridView;
}
  ...
  if (folder_item_id != target_view_item_id) {
    ...
    if (item_list_->FindItemIndex(folder_item_id, &folder_item_index)) {
      ...
      AddChildViewAt(target_folder_view,
                     GetAppListItemViewIndexOffset() + target_view_index);
    } else {
