...
void AppsGridView::CalculateIdealBounds() {
  ...
  for (int i = 0; i < total_views; ++i) {
    ...
    const PaginationModel::Transition& transition =
        pagination_model_.transition();
    // When transition is progressing, eliminating empty spaces between pages.
    if (is_fullscreen_app_list_enabled_ && transition.progress > 0) {
      const int current_page = pagination_model_.selected_page();
      const int dir = transition.target_page > current_page ? -1 : 1;
      // When transiting to next page, elimnating empty space from just previous
      // page since only the previous page is visiable; vice versa.
      if (dir > 0 && view_index.page == current_page - 1) {
      ...
      } else if (dir < 0 && view_index.page == current_page + 1) {
        offset.set_y(offset.y() - GetHeightOnTopOfAllAppsTiles(current_page));
      }
    }
...
gfx::Size AppsGridView::GetTileGridSize() const {
  ...
  bounds.Inset(0,
               -GetHeightOnTopOfAllAppsTiles(pagination_model_.selected_page()),
               0, 0);
