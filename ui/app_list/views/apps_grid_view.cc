...
void AppsGridView::EndDrag(bool cancel) {
  ...
  if (forward_events_to_drag_and_drop_host_) {
    ...
    if (IsDraggingForReparentInRootLevelGridView()) {
      ...
      AppsGridViewDelegate* delegate = contents_view_->app_list_main_view();
      delegate->CancelDragInActiveFolder();
      return;
    }
...
void AppsGridView::Layout() {
  ...
  LayoutIndicator(suggested_apps_indicator_, &rect);
  ...
  LayoutIndicator(all_apps_indicator_, &rect);
...
void AppsGridView::LayoutIndicator(IndicatorChipView* indicator,
                                   gfx::Rect* rect) {
  if (!indicator)
    return;
  ...
  const gfx::Vector2d page_zero_offset = CalculateTransitionOffset(0);
  if (indicator == suggested_apps_indicator_) {
    ...
    rect->Inset(0, suggested_apps_indicator_->GetPreferredSize().height(), 0,
                0);
  } else if (indicator == all_apps_indicator_) {
...
void AppsGridView::ButtonPressed(views::Button* sender,
  ...
  if (contents_view_) {
    ...
    delegate->ActivateApp(pressed_item_view->item(), event.flags());
  }
}
