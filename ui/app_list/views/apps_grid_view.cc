...
void AppsGridView::UpdateOpacity(int app_list_y_position_in_screen) {
  ...
  // Updates the opaicty of suggestions container.y = 2.5 * (x - 0.3)
  ...
  float opacity = std::min((fraction - kSuggestedAppsOpacityStartFraction) /
  ...
  opacity = std::min((fraction + kAllAppsIndicatorOpacityEndFraction -
  ...
  // Update the opacity of all apps. The opacity of the app starting at 0.f when
  // the ceterline of the app is |kAllAppsOpacityStartPx| above the bottom of
  // work area and transitioning to 1.0f by the time the centerline reaches
  // |kAllAppsOpacityEndPx| above the
  // work area bottom.
  ...
  for (int i = 0; i < view_model_.view_size(); ++i) {
    ...
    if (item_view != drag_view_) {
      ...
      opacity = std::min((centerline_above_work_area - kAllAppsOpacityStartPx) /
  ...
  // update the opacity of page switcher buttons. The same rule as all apps.
