...
class AppsGridView::FadeoutLayerDelegate : public ui::LayerDelegate {
 ...
 private:
  ...
  void OnPaintLayer(const ui::PaintContext& context) override {
    ...
    gfx::Rect top_zone(layer()->size());
    top_zone.set_height(kFadeoutZoneHeight);
    canvas->DrawRect(top_zone, flags);
    // Draw bottom gradient zone.
    const int layer_height = layer()->size().height();
    flags.setShader(gfx::CreateGradientShader(layer_height - kFadeoutZoneHeight,
                                              layer_height, SK_ColorBLACK,
                                              SK_ColorTRANSPARENT));
    gfx::Rect bottom_zone(layer()->size());
    bottom_zone.set_y(layer_height - kFadeoutZoneHeight);
    bottom_zone.set_height(kFadeoutZoneHeight);
    canvas->DrawRect(bottom_zone, flags);
  }
