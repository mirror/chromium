...
gfx::Size GetTileViewSizeFullscreen() {
...
gfx::Insets GetTilePaddingFullscreen() {
  return gfx::Insets(
...
// static
gfx::Size AppsGridView::GetTotalTileSizeFullscreen() {
  gfx::Rect rect(GetTileViewSizeFullscreen());
  rect.Inset(GetTilePaddingFullscreen());
  return rect.size();
}
...
void AppsGridView::UpdatePulsingBlockViews() {
  ...
  while (pulsing_blocks_model_.view_size() < desired) {
    PulsingBlockView* view = new PulsingBlockView(
        is_fullscreen_app_list_enabled_ ? GetTotalTileSizeFullscreen()
                                        : GetTotalTileSize(),
        true);
...
void AppsGridView::AnimateToIdealBounds() {
  ...
  for (int i = 0; i < view_model_.view_size(); ++i) {
    ...
    const gfx::Size& tile_size = is_fullscreen_app_list_enabled_
                                     ? GetTotalTileSizeFullscreen()
                                     : GetTotalTileSize();
    if (visible && y_diff && y_diff % tile_size.height() == 0) {
...
void AppsGridView::AnimationBetweenRows(AppListItemView* view,
  ...
  const gfx::Size& total_tile_size = is_fullscreen_app_list_enabled_
                                         ? GetTotalTileSizeFullscreen()
                                         : GetTotalTileSize();
...
gfx::Size AppsGridView::GetTileGridSize() const {
  ...
  bounds.Inset(is_fullscreen_app_list_enabled_ ? GetTilePaddingFullscreen()
                                               : GetTilePadding());
  return bounds.size();
}
...
gfx::Rect AppsGridView::GetExpectedTileBounds(int row, int col) const {
  ...
  gfx::Rect tile_bounds(gfx::Point(bounds.x() + col * total_tile_size.width(),
                        ...
                        total_tile_size);
  tile_bounds.Inset(is_fullscreen_app_list_enabled_
                        ? -GetTilePaddingFullscreen()
                        : -GetTilePadding());
  return tile_bounds;
}
