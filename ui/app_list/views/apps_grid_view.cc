...
void AppsGridView::UpdateControlVisibility(
  ...
  if (all_apps_indicator_) {
    all_apps_indicator_->SetVisible(
  ...
  for (int i = 0; i < view_model_.view_size(); ++i) {
    AppListItemView* view = GetItemViewAt(i);
    view->SetVisible(app_list_state == AppListView::FULLSCREEN_ALL_APPS ||
                             is_in_drag
                         ? true
                         : false);
  }
  ...
  page_switcher_view_->SetVisible(
      app_list_state == AppListView::FULLSCREEN_ALL_APPS || is_in_drag ? true
                                                                       : false);
}
