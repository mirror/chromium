...
void AppsGridView::Layout() {
  ...
  AppListView::AppListState app_list_state = AppListView::PEEKING;

  bool is_in_drag = false;
  if (contents_view_->app_list_view()) {
    is_in_drag = contents_view_->app_list_view()->is_in_drag();
    app_list_state = contents_view_->app_list_view()->app_list_state();
  }
  ...
  if (!folder_delegate_) {
    ...
    if (all_apps_indicator_) {
      if (app_list_state == AppListView::FULLSCREEN_ALL_APPS || is_in_drag) {
    ...
    if (expand_arrow_view_) {
      if (app_list_state == AppListView::PEEKING) {
