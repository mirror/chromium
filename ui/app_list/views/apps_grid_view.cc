...
class AppsGridView::FadeoutLayerDelegate : public ui::LayerDelegate {
 ...
 private:
  ...
  void OnPaintLayer(const ui::PaintContext& context) override {
    ...
    recorder.canvas()->DrawRect(initial_rect, flags);
    ...
    for (int i = 1; i <= kFadeoutZoneHeight; ++i) {
      flags.setAlpha(255 - i * alpha_step);
      top_left.Offset(0, -1);
      top_right.Offset(0, -1);
      bottom_left.Offset(0, 1);
      bottom_right.Offset(0, 1);
      recorder.canvas()->DrawLine(gfx::PointF(top_left), gfx::PointF(top_right),
                                  flags);
      recorder.canvas()->DrawLine(gfx::PointF(bottom_left),
                                  gfx::PointF(bottom_right), flags);
    }
  }
