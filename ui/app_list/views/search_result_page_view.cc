...
gfx::Rect SearchResultPageView::GetPageBoundsForState(
  ...
  if (state != ash::AppListState::STATE_SEARCH_RESULTS) {
...
void SearchResultPageView::OnAnimationUpdated(double progress,
  ...
  if (from_state != ash::AppListState::STATE_SEARCH_RESULTS &&
      to_state != ash::AppListState::STATE_SEARCH_RESULTS) {
    return;
  }
  ...
  gfx::Rect onscreen_bounds(
      GetPageBoundsForState(ash::AppListState::STATE_SEARCH_RESULTS));
