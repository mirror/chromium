...
class APP_LIST_EXPORT AppsGridView : public views::View,
 ...
 private:
  ...
  // Returns all apps tiles per page based on if the page is the first all apps
  // page, indicated by |page_zero|.
  int TilesPerPage(bool page_zero) const;
  ...
  // Get height on top of the all apps tiles. For the first page, that includes
  // suggested apps indicator, suggestioned apps tiles, all apps indicator
  ...
  AppsGridViewDelegate* delegate_ = nullptr;
