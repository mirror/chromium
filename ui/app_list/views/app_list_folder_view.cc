...
class BackgroundAnimation : public gfx::SlideAnimation,
 ...
 private:
  ...
  AppListFolderView* folder_view_;  // Not owned.
...
class FolderItemTitleAnimation : public gfx::SlideAnimation,
 ...
 private:
  // gfx::AnimationDelegate
  void AnimationProgressed(const gfx::Animation* animation) override {
    folder_view_->activated_folder_item_view()->title()->SetEnabledColor(
...
// folder.
class TopIconAnimationDelegate : public TopIconAnimationObserver {
 public:
  TopIconAnimationDelegate(bool show,
  ...
  ~TopIconAnimationDelegate() override = default;

  void ScheduleAnimation() {
    // Hide the original items in the folder until the animation ends.
    SetFirstPageItemViewsVisible(false);
    DCHECK(folder_view_->activated_folder_item_view());
    folder_view_->activated_folder_item_view()->icon()->SetVisible(false);
    ...
    // Record the number of items that should animate, this is used to signal
    // cleaning up after all animations are done.
    first_page_item_animation_pending_count_ =
        first_page_item_views_bounds.size();
    ...
    for (size_t i = 0; i < first_page_item_views_bounds.size(); ++i) {
      ...
      top_icon_views_.back()->SetBoundsRect(first_page_item_views_bounds[i]);
      static_cast<TopIconAnimationView*>(top_icon_views_.back())
          ->TransformView();
    }
  }
  ...
  void OnTopIconAnimationsComplete() override {
    ...
    if (!first_page_item_animation_pending_count_) {
      ...
      if ((!show_ || hide_for_reparent_) &&
          folder_view_->activated_folder_item_view()) {
        folder_view_->activated_folder_item_view()->icon()->SetVisible(true);
      }
    }
  }

  size_t first_page_item_animation_pending_count() const {
    return first_page_item_animation_pending_count_;
  }
 ...
 private:
  ...
  size_t first_page_item_animation_pending_count_ = 0u;

  DISALLOW_COPY_AND_ASSIGN(TopIconAnimationDelegate);
};
...
class ContentsContainerAnimationDelegate
    : public ui::ImplicitAnimationObserver {
 public:
  ContentsContainerAnimationDelegate(bool show,
                                     bool hide_for_reparent,
                                     AppListFolderView* folder_view)
      : show_(show),
        hide_for_reparent_(hide_for_reparent),
  ...
  ~ContentsContainerAnimationDelegate() override = default;
  ...
  void ScheduleAnimation() {
    ...
    gfx::Rect scaled_rect(folder_view_->folder_item_icon_bounds());
    gfx::Rect rect(folder_view_->contents_container()->bounds());
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(ContentsContainerAnimationDelegate);
};
...
void AppListFolderView::ScheduleShowHideAnimation(bool show,
  ...
  background_animation_.reset(new BackgroundAnimation(show, this));
  background_animation_->Show();

  // Animate the folder item's title's opacity.
  folder_item_title_animation_.reset(new FolderItemTitleAnimation(show, this));
  folder_item_title_animation_->Show();

  // Animate the bounds and opacity of items in the first page of the opened
  // folder.
  top_icon_animation_delegate_.reset(
      new TopIconAnimationDelegate(show, hide_for_reparent, this));
  static_cast<TopIconAnimationDelegate*>(top_icon_animation_delegate_.get())
      ->ScheduleAnimation();

  // Animate the bounds and opacity of the contents container.
  contents_container_animation_delegate_.reset(
      new ContentsContainerAnimationDelegate(show, hide_for_reparent, this));
  static_cast<ContentsContainerAnimationDelegate*>(
      contents_container_animation_delegate_.get())
      ->ScheduleAnimation();
}
...
bool AppListFolderView::IsAnimationRunning() const {
  return top_icon_animation_delegate_ &&
         static_cast<TopIconAnimationDelegate*>(
             top_icon_animation_delegate_.get())
             ->first_page_item_animation_pending_count();
}
...
void AppListFolderView::UpdatePreferredBounds() {
  AppsGridView* root_level_grid_view = container_view_->apps_grid_view();
  activated_folder_item_view_ =
      root_level_grid_view->activated_folder_item_view();
  DCHECK(activated_folder_item_view_);

  // Calculate the folder icon's bounds relative to AppsContainerView.
  gfx::RectF rect(activated_folder_item_view_->GetIconBounds());
  views::View::ConvertRectToTarget(activated_folder_item_view_, container_view_,
                                   &rect);
...
void AppListFolderView::HideViewImmediately() {
  ...
  // Directly transit all the states immediately to the end of folder closing
  // animation.
  background_view_->SetBackground(nullptr);
  background_view_->SchedulePaint();
  if (activated_folder_item_view_) {
    activated_folder_item_view_->icon()->SetVisible(true);
    activated_folder_item_view_->title()->SetEnabledColor(kGridTitleColor);
    activated_folder_item_view_->title()->SetVisible(true);
  }
}
