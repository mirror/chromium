...
class AppsGridViewTest : public views::ViewsTestBase,
 ...
 public:
  ...
  void SetUp() override {
    ...
    std::cout << base::i18n::GetConfiguredLocale() << std::endl;
    if (testing::UnitTest::GetInstance()->current_test_info()->value_param()) {
      is_rtl_ = GetParam();
      base::i18n::SetICUDefaultLocale(is_rtl_ ? "he" : "en-US");
    }
  ...
  void TearDown() override {
    ...
    if (!base::i18n::GetConfiguredLocale().compare("en-US"))
...
// Tests that moving selection up from the first app selects the Nothing.
...
// Tests that moving selection forward from nothing selects the first
// suggested app.
TEST_P(AppsGridViewTest, SelectionForwardFromSearchBoxSelectsFirstApp) {
  model_->PopulateApps(5);
...
TEST_P(AppsGridViewTest, SelectionTraversesAllSuggestedApps) {
  ...
  // Check selection at the next suggested app.
  EXPECT_FALSE(expand_arrow_view_->selected());
  EXPECT_EQ(2, suggestions_container_->selected_index());
  EXPECT_FALSE(apps_grid_view_->has_selected_view());
...
TEST_P(AppsGridViewTest, SelectionMovesToFirstElementOfPrevPageWithPrevKey) {
  ...
  EXPECT_FALSE(expand_arrow_view_->selected());
  /////
  /// first app in apps grid view is selected.
  /// We are expecting the wrong one.
  ///
  EXPECT_TRUE(apps_grid_view_->has_selected_view());
  EXPECT_TRUE(apps_grid_view_->IsSelectedView(GetItemViewAt(0)));
  EXPECT_EQ(-1, suggestions_container_->selected_index());
