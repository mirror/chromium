...
class AppsGridViewTest : public views::ViewsTestBase {
 ...
 protected:
  ...
  int GetTilesPerPage() const {
    return apps_grid_view_->cols() * apps_grid_view_->rows_per_page();
  }
...
TEST_F(AppsGridViewTest, EnsureHighlightedVisible) {
  ...
  EXPECT_EQ(0, GetPaginationModel()->selected_page());
  model_->HighlightItemAt(GetTilesPerPage() - 1);
  EXPECT_EQ(0, GetPaginationModel()->selected_page());
...
TEST_F(AppsGridViewTest, HighlightWithKeyboard) {
  const int kPages = 3;
  const int kItems = (kPages - 1) * GetTilesPerPage() + 1;
  ...
  const int last_index_on_page1 = GetTilesPerPage() - 1;
  const int first_index_on_page2 = GetTilesPerPage();
  const int first_index_on_page2_last_row =
      2 * GetTilesPerPage() - apps_grid_view_->cols();
  const int last_index_on_page2_last_row = 2 * GetTilesPerPage() - 1;
