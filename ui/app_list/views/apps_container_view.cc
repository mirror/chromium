...
gfx::Rect AppsContainerView::GetPageBoundsDuringDragging(
  ...
  if (drag_amount <= (kPeekingAppListHeight - kShelfSize)) {
    // App list is dragged from shelf mode to peeking, which moved up at most
    // |kPeekingAppListHeight - kShelfSize| (272px). The y inset of apps
    ...
    // |peeking_to_fullscreen_height|. The y inset of apps container view
    ...
    y = std::ceil((final_y - peeking_final_y) *
                  ...
                  peeking_final_y);
    y = std::max(y, peeking_final_y);
    y = contents_view()->app_list_view()->is_in_drag() ? y : final_y;
  }
...
int AppsContainerView::GetSearchBoxFinalTopPadding() const {
  ...
  // Top padding of the searchbox should not smaller than
...
int AppsContainerView::GetSearchBoxTopPaddingDuringDragging() const {
  ...
  if (drag_amount <= (kPeekingAppListHeight - kShelfSize)) {
    // App list is dragged from shelf state to peeking, which moved up at most
    // |kPeekingAppListHeight - kShelfSize| (272px). The y inset of search box
    // changes from |kSearchBoxInitalTopPadding| to
    ...
    // App list is dragged from peeking to fullscreen, which moved up at most
    // |peeking_to_fullscreen_height|. The y inset of search box changes from
    // |kSearchBoxPeekingTopPadding| to |searchbox_final_y|.
