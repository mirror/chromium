...
void SearchBoxView::UpdateOpacity() {
  ...
  ContentsView* view = static_cast<ContentsView*>(contents_view());
  int app_list_y_position_in_screen =
      view->app_list_view()->app_list_y_position_in_screen();
  float fraction =
      std::max<float>(0, view->app_list_view()->GetScreenBottom() - kShelfSize -
  ...
  AppListView* app_list_view = view->app_list_view();
  ...
  view->search_results_page_view()->layer()->SetOpacity(
