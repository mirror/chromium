  ...
  if (selected) {
    ...
    if (IsSearchBoxTrimmedQueryEmpty()) {
      ...
      if (!search_box_->text().empty()) {
        // If query is a string of spaces, we need to select the entire text
        // range.
        search_box_->SelectAll(false);
      }
    } else {
      // The opened search box is shown, we should not show the ring, but we
      // still need to select the entire text range.
      search_box_->SelectAll(false);
    }
  } else {
    search_box_->ClearSelection();
    SetDefaultBorder();
  }
  Layout();
  SchedulePaint();
