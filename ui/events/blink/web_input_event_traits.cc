...
void ApppendEventDetails(const WebMouseWheelEvent& event, std::string* result) {
  StringAppendF(
      ...
      " Phase: (%d, %d)",
      event.delta_x, event.delta_y, event.wheel_ticks_x, event.wheel_ticks_y,
      event.acceleration_ratio_x, event.acceleration_ratio_y, event.scroll_by_page,
      event.has_precise_scrolling_deltas, event.phase, event.momentum_phase);
}
...
LatencyInfo WebInputEventTraits::CreateLatencyInfoForWebGestureEvent(
  ...
  if (event.source_device == blink::WebGestureDevice::kWebGestureDeviceTouchpad) {
