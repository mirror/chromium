...
class ScrollElasticityControllerTest : public testing::Test {
 public:
  ...
  void SendGestureScrollUpdate(
    ...
    scroll_result.scroll_boundary_behavior = scroll_boundary_behavior;
...
// Verify that ScrollBoundaryBehaviorTypeNone disables the stretching on the
// specified axis.
TEST_F(ScrollElasticityControllerTest, ScrollBoundaryBehavior) {
  helper_.SetScrollOffsetAndMaxScrollOffset(gfx::ScrollOffset(0, 0),
                                            gfx::ScrollOffset(0, 0));

  // If we set ScrollBoundaryBehaviorTypeNone on x, we should not see a stretch
  // in the X direction.
  SendGestureScrollBegin(NonMomentumPhase);
  SendGestureScrollUpdate(
      ...
      cc::ScrollBoundaryBehavior(
          ...
          cc::ScrollBoundaryBehavior::kScrollBoundaryBehaviorTypeAuto));
  EXPECT_EQ(0, helper_.set_stretch_amount_count());
  EXPECT_EQ(0.f, helper_.StretchAmount().x());
  EXPECT_EQ(0.f, helper_.StretchAmount().y());
  helper_.SetStretchAmount(gfx::Vector2dF());
  ...
  // If we set ScrollBoundaryBehaviorTypeNone on x, we could still see a stretch
  // in the Y direction
  SendGestureScrollBegin(NonMomentumPhase);
  SendGestureScrollUpdate(
  ...
  // If we set ScrollBoundaryBehaviorTypeNone on y, we should not see a stretch
  // in the Y direction.
  SendGestureScrollBegin(NonMomentumPhase);
  SendGestureScrollUpdate(
      NonMomentumPhase, gfx::Vector2dF(0, 10), gfx::Vector2dF(0, 10),
  ...
  // If we set ScrollBoundaryBehaviorTypeNone on y, we could still see a stretch
  // in the X direction.
  SendGestureScrollBegin(NonMomentumPhase);
  SendGestureScrollUpdate(
      NonMomentumPhase, gfx::Vector2dF(10, 0), gfx::Vector2dF(10, 0),
