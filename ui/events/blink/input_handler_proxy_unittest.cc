...
WebScopedInputEvent CreateGestureScrollFlingPinch(WebInputEvent::Type type,
  ...
  if (type == WebInputEvent::kGestureScrollUpdate) {
    ...
    gesture.data.fling_start.velocity_y = deltaYOrScale;
  } else if (type == WebInputEvent::kGesturePinchUpdate) {
    gesture.data.pinch_update.scale = deltaYOrScale;
    gesture.x = x;
    gesture.y = y;
  }
...
TEST_P(InputHandlerProxyTest, MouseWheelNoListener) {
  ...
  WebMouseWheelEvent wheel(WebInputEvent::kMouseWheel, WebInputEvent::kControlKey,
                           WebInputEvent::kTimeStampForTesting);
...
TEST_P(InputHandlerProxyTest, GestureScrollByPage) {
  ...
  gesture_.SetType(WebInputEvent::kGestureScrollBegin);
  gesture_.data.scroll_begin.delta_hint_units = WebGestureEvent::ScrollUnits::kPage;
  EXPECT_EQ(expected_disposition_, input_handler_->HandleInputEvent(gesture_));
...
TEST_P(InputHandlerProxyTest, DISABLED_GestureScrollByCoarsePixels) {
  ...
  gesture_.SetType(WebInputEvent::kGestureScrollUpdate);
  gesture_.data.scroll_update.delta_units = WebGestureEvent::ScrollUnits::kPixels;
...
void InputHandlerProxyTest::GestureFlingAnimatesTouchpad() {
  ...
  // special (reserved for zoom), so don't set that here.
  int modifiers = WebInputEvent::kShiftKey | WebInputEvent::kAltKey;
  gesture_ = CreateFling(blink::kWebGestureDeviceTouchpad,
                         fling_delta,
                         fling_point,
                         fling_global_point,
                         modifiers);
...
TEST_P(InputHandlerProxyTest, GestureFlingAnimatesTouchscreen) {
  ...
  gesture_ = CreateFling(blink::kWebGestureDeviceTouchscreen,
                         fling_delta,
                         fling_point,
                         fling_global_point,
                         modifiers);
...
TEST_P(InputHandlerProxyTest, GestureFlingCancelledByKeyboardEvent) {
  ...
  WebKeyboardEvent key_event(WebInputEvent::kKeyDown, WebInputEvent::kNoModifiers,
                             WebInputEvent::kTimeStampForTesting);
...
TEST_P(InputHandlerProxyTest, FlingBoost) {
  ...
  WebPoint fling_point = WebPoint(7, 13);
  StartFling(
      time, blink::kWebGestureDeviceTouchscreen, fling_delta, fling_point);
...
TEST_P(InputHandlerProxyEventQueueTest, VSyncAlignedCoalesceScrollAndPinch) {
  ...
  EXPECT_EQ(WebInputEvent::kGestureScrollEnd, event_queue()[2]->event().GetType());
  EXPECT_EQ(WebInputEvent::kGesturePinchBegin, event_queue()[3]->event().GetType());
  EXPECT_EQ(WebInputEvent::kGestureScrollUpdate,
  ...
  EXPECT_EQ(WebInputEvent::kGesturePinchEnd, event_queue()[6]->event().GetType());
  testing::Mock::VerifyAndClearExpectations(&mock_input_handler_);
}
...
TEST_P(InputHandlerProxyEventQueueTest, GestureScrollFlingOrder) {
  ...
  EXPECT_EQ(WebInputEvent::kGestureScrollUpdate,
            event_queue()[0]->event().GetType());
  EXPECT_EQ(WebInputEvent::kGestureFlingStart, event_queue()[1]->event().GetType());
