...
#include "ui/events/gestures/blink/fling_booster.h"
...
InputHandlerProxy::EventDisposition InputHandlerProxy::HandleInputEvent(
  ...
  if (fling_booster_) {
    ...
    bool should_filter_event = fling_booster_->FilterInputEventForFlingBoosting(
        event, &cancel_current_fling);
    ...
    if (should_filter_event) {
      if (event.GetType() == WebInputEvent::kGestureFlingStart) {
        ...
        UpdateCurrentFlingState(gesture_event,
                                fling_booster_->CurrentFlingVelocity());
        TRACE_EVENT_INSTANT2(
            "input",
            fling_booster_->FlingBoosted() ? "InputHandlerProxy::FlingBoosted"
                                           : "InputHandlerProxy::FlingReplaced",
            TRACE_EVENT_SCOPE_THREAD, "vx", current_fling_velocity_.x(), "vy",
        ...
        DCHECK(fling_booster_->FlingCancellationIsDeferred());
...
InputHandlerProxy::EventDisposition InputHandlerProxy::HandleGestureFlingStart(
  ...
  switch (scroll_status.thread) {
    case cc::InputHandler::SCROLL_ON_IMPL_THREAD: {
      ...
      has_fling_animation_started_ = false;
      fling_booster_.reset(new FlingBooster(current_fling_velocity_,
...
void InputHandlerProxy::Animate(base::TimeTicks time) {
  ...
  fling_booster_->SetLastFlingAnimateTime(monotonic_time_sec);
...
bool InputHandlerProxy::CancelCurrentFlingWithoutNotifyingClient() {
  ...
  if (fling_booster_) {
    if (fling_booster_->FlingCancellationIsDeferred()) {
      WebGestureEvent last_fling_boost_event = fling_booster_->LastBoostEvent();
      fling_booster_.reset();
...
bool InputHandlerProxy::ScrollBy(const WebFloatSize& increment,
  ...
  DCHECK(fling_booster_);
  fling_booster_->SetCurrentFlingVelocity(current_fling_velocity_);
