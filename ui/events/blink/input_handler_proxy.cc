  ...
  for (const auto& touch : touch_event.touches) {
    ...
    if (event_listener_type !=
      ...
      *is_touching_scrolling_layer =
      ...
      // from the compositor.
      // To start, we only use the whitelisted touch action when the event is
      // a scroll begin or update. Other events such as pinch or tab will be
      // added later.
      if (is_compositor_touch_action_enabled_ && white_listed_touch_action &&
          *white_listed_touch_action != cc::kTouchActionNone &&
          (touch_event.GetType() == WebInputEvent::kTouchStart ||
           touch_event.GetType() == WebInputEvent::kTouchMove)) {
        result = DID_HANDLE_NON_BLOCKING;
      } else {
        result = DID_NOT_HANDLE;
      }
