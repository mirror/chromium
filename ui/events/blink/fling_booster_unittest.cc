...
class FlingBoosterTest : public testing::Test {
 public:
  FlingBoosterTest()
      : fling_booster_(nullptr), dt_(base::TimeDelta::FromMilliseconds(10)) {
    gesture_scroll_event_.source_device = blink::kWebGestureDeviceTouchscreen;
  }

  void TearDown() override { fling_booster_.reset(); }
  ...
  void StartFirstFling() {
    time_ = base::TimeTicks() + dt_;
    fling_booster_.reset(new FlingBooster(
        gfx::Vector2dF(1000, 1000), blink::kWebGestureDeviceTouchscreen, 0));
    fling_booster_->set_last_fling_animation_time(InSecondsF(time_));
  }

  void CancelFling() {
    WebGestureEvent fling_cancel_event =
        CreateFlingCancel(time_, blink::kWebGestureDeviceTouchscreen);
...
TEST_F(FlingBoosterTest, FlingBoost) {
  ...
  gesture_scroll_event_.SetTimeStampSeconds(InSecondsF(time_));
  ...
  WebGestureEvent fling_start_event =
      CreateFlingStart(time_, blink::kWebGestureDeviceTouchscreen,
  ...
  CancelFling();
  time_ += base::TimeDelta::FromMilliseconds(100);
  fling_booster_->set_last_fling_animation_time(InSecondsF(time_));
...
TEST_F(FlingBoosterTest, NoFlingBoostIfFlingTooSlow) {
  ...
  WebGestureEvent fling_start_event = CreateFlingStart(
      time_, blink::kWebGestureDeviceTouchscreen, gfx::Vector2dF(100, 100), 0);
  bool cancel_current_fling;
  EXPECT_TRUE(fling_booster_->FilterGestureEventForFlingBoosting(
      fling_start_event, &cancel_current_fling));
  EXPECT_FALSE(cancel_current_fling);
...
TEST_F(FlingBoosterTest, NoFlingBoostIfDifferentFlingSourceDevices) {
  ...
  // fling_booster.
  WebGestureEvent fling_start_event = CreateFlingStart(
      time_, blink::kWebGestureDeviceTouchpad, gfx::Vector2dF(500, 500), 0);
  bool cancel_current_fling;
  EXPECT_FALSE(fling_booster_->FilterGestureEventForFlingBoosting(
      fling_start_event, &cancel_current_fling));
  EXPECT_TRUE(cancel_current_fling);
