...
class EVENTS_EXPORT XEventDispatcher {
 public:
  ...
  // Returns PlatformEventDispatcher if this XEventDispatcher is a
  // PlatformEventDispatcher as well. Used to explicitly add a
...
class EVENTS_EXPORT X11EventSourceLibevent
 ...
 public:
  ...
  // in case if XEventDispatcher is also a PlatformEventDispatcher. Thus,
  ...
  // list if the XEventDispatcher is also a PlatformEventDispatcher.
 ...
 private:
  ...
  // translated event is going to be sent next, then dispatches the event and
