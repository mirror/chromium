...
KeyboardEvdev::KeyboardEvdev(EventModifiers* modifiers,
  ...
  auto_repeat_handler_.SetCallback(base::BindRepeating(
      &KeyboardEvdev::DispatchKey, weak_ptr_factory_.GetWeakPtr()));
}
...
void KeyboardEvdev::SetAutoRepeatRate(const base::TimeDelta& delay,
  ...
  return auto_repeat_handler_.SetAutoRepeatRate(delay, interval);
}
