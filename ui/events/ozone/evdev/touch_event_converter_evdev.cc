...
void TouchEventConverterEvdev::ProcessMultitouchEvent(
  ...
  if (input.type == EV_SYN) {
    ...
    LOG(ERROR) << "EV_ABS: " << input.code << " = " << input.value;
    ...
    LOG(ERROR) << "EV_KEY: " << input.code << " = " << input.value;
    ProcessKey(input);
...
void TouchEventConverterEvdev::ReportEvents(base::TimeTicks timestamp) {
  ...
  for (size_t i = 0; i < events_.size(); i++) {
    ...
    if (event->altered && (event->cancelled ||
      ...
      LOG(ERROR) << "Slot " << i << " cancelled. cause: " <<  cause;
      CancelAllTouches();
      break;
    }
  }
  ...
  for (size_t i = 0; i < events_.size(); i++) {
    ...
    LOG(ERROR) << "Slot " << i << " event type: " << static_cast<int>(event_type);
