...
class GIN_EXPORT V8Initializer {
 public:
  ...
  enum class V8SnapshotFileType {
    ...
    // Snapshot augmented with an additional context specific snapshot, which
    // can be deserialized using v8::Context::FromSnapshot.
    kContext,
  };

  // Load V8 snapshot from default resources, if they are available.
  static void LoadV8Snapshot(
      SnapshotFileType snapshot_file_type = SnapshotFileType::kDefault);
