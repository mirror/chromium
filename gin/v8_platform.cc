...
// shared_ptr.
class TaskRunnerWrapper : public v8::TaskRunner {
 public:
  TaskRunnerWrapper(std::shared_ptr<v8::TaskRunner> taskrunner)
  ...
  void PostIdleTask(std::unique_ptr<v8::IdleTask> task) override {
    taskrunner_->PostIdleTask(std::move(task));
  }
...
std::unique_ptr<v8::TaskRunner> V8Platform::GetForegroundTaskRunner(
    v8::Isolate* isolate) {
  PerIsolateData* data = PerIsolateData::From(isolate);
  return std::make_unique<TaskRunnerWrapper>(data->task_runner());
}

std::unique_ptr<v8::TaskRunner> V8Platform::GetBackgroundTaskRunner(
    v8::Isolate* isolate) {
  return std::make_unique<V8BackgroundTaskRunner>();
}
