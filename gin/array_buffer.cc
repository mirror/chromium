...
void ArrayBufferAllocator::SetProtection(void* data,
  ...
  switch (protection) {
    case Protection::kNoAccess:
      CHECK(mprotect(data, length, PROT_NONE) == 0);
    case Protection::kReadWrite:
      CHECK(mprotect(data, length, PROT_READ | PROT_WRITE) == 0);
    default:
      NOTREACHED();
  }
  ...
  switch (protection) {
    case Protection::kNoAccess:
      CHECK(VirtualFree(data, length, MEM_DECOMMIT));
    case Protection::kReadWrite:
      CHECK(VirtualAlloc(data, length, MEM_COMMIT, PAGE_READWRITE));
    default:
      NOTREACHED();
  }
#endif  // !defined(OS_POSIX)
