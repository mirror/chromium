...
void V8Initializer::Initialize(IsolateHolder::ScriptMode mode,
  ...
  if (GetV8ContextSnapshotData(IsolateHolder::SnapshotType::kV8ContextSnapshot,
                               &snapshot) !=
      IsolateHolder::SnapshotType::kNone) {
...
IsolateHolder::SnapshotType V8Initializer::GetV8ContextSnapshotData(
    IsolateHolder::SnapshotType offering_type,
    v8::StartupData* snapshot) {
  CHECK(snapshot);

  if (offering_type == IsolateHolder::SnapshotType::kV8ContextSnapshot) {
    GetMappedFileData(g_mapped_v8_context_snapshot, snapshot);
    if (snapshot->data) {
      return IsolateHolder::SnapshotType::kV8ContextSnapshot;
    }
  }
  if (offering_type == IsolateHolder::SnapshotType::kV8ContextSnapshot ||
      offering_type == IsolateHolder::SnapshotType::kV8Snapshot) {
    GetMappedFileData(g_mapped_snapshot, snapshot);
    if (snapshot->data) {
      return IsolateHolder::SnapshotType::kV8Snapshot;
    }
  }
