...
V8Initializer::SnapshotFileType g_snapshot_file_type =
    V8Initializer::SnapshotFileType::kNone;
...
const char* GetSnapshotFileName(
    const V8Initializer::SnapshotFileType file_type) {
  switch (file_type) {
    case V8Initializer::SnapshotFileType::kDefault:
      return kSnapshotFileName;
    case V8Initializer::SnapshotFileType::kV8Context:
      return kV8ContextSnapshotFileName;
    case V8Initializer::SnapshotFileType::kNone:
      NOTREACHED();
      return nullptr;
  }
  NOTREACHED();
...
// static
void V8Initializer::LoadV8Snapshot(SnapshotFileType snapshot_file_type) {
  if (g_mapped_snapshot) {
    DCHECK_EQ(g_snapshot_file_type, snapshot_file_type);
    return;
  }
