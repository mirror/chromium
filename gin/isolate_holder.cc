...
IsolateHolder::IsolateHolder(
  ...
  V8Initializer::GetV8ContextSnapshotData(&snapshot_blob_);
  if (snapshot_blob_.data) {
    params.snapshot_blob = &snapshot_blob_;
...
IsolateHolder::IsolateHolder() : access_mode_(AccessMode::kSingleThread) {
  ...
  snapshot_creator_.reset(
      new v8::SnapshotCreator(g_reference_table, &snapshot_blob_));
