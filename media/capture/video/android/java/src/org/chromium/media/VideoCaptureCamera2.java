...
public class VideoCaptureCamera2 extends VideoCapture {
    ...
    private class CrPreviewSessionListener extends CameraCaptureSession.StateCallback {
        private final CaptureRequest.Builder mPreviewRequestBuilder;
        CrPreviewSessionListener(CaptureRequest.Builder previewRequestBuilder) {
    ...
    private ImageReader mPhotoReader = null;
    ...
    private boolean restartPreview(Handler handler) {
        ...
        try {
            // (Re)trigger the preview. A CaptureCallback is registered to receive the actual
            // capture result details. If |handler| is null we'll work on the current Thread
            // Looper.
    ...
    private boolean createPreviewObjectsAndCaptureSession() {
        ...
        // Create an ImageReader with a background thread looper into it to have readback take place
        ...
        // A dummy Surface for precapture operations.  We could also have an(other) ImageReader and
        ...
        try {
            mCameraDevice.createCaptureSession(
                    surfaceList, new CrPreviewSessionListener(mPreviewRequestBuilder), null);
        } catch (CameraAccessException | IllegalArgumentException | SecurityException ex) {
    ...
    public boolean takePhoto(final long callbackId) {
        ...
        // Do not touch FLASH_MODE (see http://stackoverflow.com/a/36069908); anyway either of
        ...
        if (mFillLightMode == AndroidFillLightMode.AUTO) {
            Log.d(TAG, "takePhoto() Flash Auto");
            photoRequestBuilder.set(CaptureRequest.CONTROL_AE_MODE,
                    mRedEyeReduction ? CameraMetadata.CONTROL_AE_MODE_ON_AUTO_FLASH_REDEYE
                                     : CameraMetadata.CONTROL_AE_MODE_ON_AUTO_FLASH);
        } else {
            assert mFillLightMode == AndroidFillLightMode.FLASH;
            Log.d(TAG, "takePhoto() Flash ON");
        ...
        final CameraCaptureSession.CaptureCallback precaptureStateObserver =
                new CameraCaptureSession.CaptureCallback() {
                    ...
                    // Some devices trigger onCaptureProgressed, others onCaptureCompleted(),
                    // funnel them all via this method.
                    void process(CaptureResult result) {
                        Integer controlAeState = result.get(CaptureResult.CONTROL_AE_STATE);
                        if (controlAeState == null) return;

                        // After the first convergence, discard eventual subsequent fragments.
                        if (mPhotoCaptureTriggered) return;

                        // Wait until the AE routine has converged,
                        // https://developer.android.com/reference/android/hardware/camera2/CaptureResult.html#CONTROL_AE_STATE
                        if ((controlAeState == CaptureRequest.CONTROL_AE_STATE_FLASH_REQUIRED
                                    || controlAeState
                            ...
                            try {
                                ...
                                Log.e(TAG, "mPhotoSession.capture(): " + ex);
                            }
                        }
                    }
                };
        ...
        try {
            mCaptureSession.setRepeatingRequest(
                    mPreviewRequestBuilder.build(), precaptureStateObserver, mBackgroundHandler);
        } catch (CameraAccessException | IllegalArgumentException | SecurityException ex) {
