...
class GpuMemoryBufferImplGbm : public gfx::GpuMemoryBuffer {
 public:
  GpuMemoryBufferImplGbm(gfx::BufferFormat format, gbm_bo* bo)
      : format_(format), bo_(bo), mapped_(false) {
    ...
    handle_.native_pixmap_handle.fds.push_back(
        base::FileDescriptor(gbm_bo_get_fd(bo), false));
  ...
  bool Map() override {
    ...
    size_t num_planes = gbm_bo_get_num_planes(bo_);
    uint32_t stride;
  ...
  void Unmap() override {
    for (size_t i = 0; i < mapped_planes_.size(); ++i) {
      if (mapped_planes_[i].addr) {
        gbm_bo_unmap(bo_, mapped_planes_[i].mapped_data);
  ...
  int stride(size_t plane) const override {
    return gbm_bo_get_plane_stride(bo_, plane);
  }
...
LocalGpuMemoryBufferManager::CreateGpuMemoryBuffer(
  ...
  uint32_t camera_gbm_usage = GBM_BO_USE_LINEAR | GBM_BO_USE_CAMERA_WRITE;
  if (!drm_format) {
  ...
  gbm_bo* bo = gbm_bo_create(gbm_device_, size.width(), size.height(),
                             drm_format, camera_gbm_usage);
  if (!bo) {
    LOG(ERROR) << "Failed to create GBM bo";
    return std::unique_ptr<gfx::GpuMemoryBuffer>();
  }

  return base::MakeUnique<GpuMemoryBufferImplGbm>(format, bo);
}
