...
class CAPTURE_EXPORT CameraBufferFactory {
 public:
  ...
  virtual std::unique_ptr<gfx::GpuMemoryBuffer> CreateGpuBuffer(
      const gfx::Size& size,
      gfx::BufferFormat format);

  virtual ChromiumFormat ResolveStreamBufferFormat(
      arc::mojom::HalPixelFormat hal_format);

 private:
  std::unordered_map<arc::mojom::HalPixelFormat, ChromiumFormat>
