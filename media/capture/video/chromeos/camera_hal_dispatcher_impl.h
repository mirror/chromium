...
class CAPTURE_EXPORT CameraHalDispatcherImpl final
 ...
 private:
  // A WeakPtr wrapper class around arc::mojom::CameraHalServerPtr.
  class CameraHalServer : public base::SupportsWeakPtr<CameraHalServer> {
   public:
    explicit CameraHalServer(arc::mojom::CameraHalServerPtr camera_hal_server);

    void CreateChannel(arc::mojom::CameraModuleRequest camera_module_request);

   private:
    arc::mojom::CameraHalServerPtr camera_hal_server_;
    DISALLOW_IMPLICIT_CONSTRUCTORS(CameraHalServer);
  };
  ...
  std::vector<std::unique_ptr<mojo::Binding<arc::mojom::CameraHalDispatcher>>>
      bindings_;

  std::unique_ptr<CameraHalServer> camera_hal_server_;
