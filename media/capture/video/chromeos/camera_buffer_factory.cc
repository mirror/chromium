// Copyright 2017 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#include "media/capture/video/chromeos/camera_buffer_factory.h"

#include "media/capture/video/chromeos/video_capture_device_factory_chromeos.h"

namespace media {

CameraBufferFactory::CameraBufferFactory() {}

CameraBufferFactory::~CameraBufferFactory() {}

std::unique_ptr<gfx::GpuMemoryBuffer> CameraBufferFactory::CreateGpuBuffer(
    const gfx::Size& size,
    gfx::BufferFormat format) {
  // TODO(jcliang): Add support for connecting to the GPU service in video
  // capture service.
  auto buf_manager = VideoCaptureDeviceFactoryChromeOS::GetBufferManager();
  if (!buf_manager) {
    LOG(ERROR) << "GpuMemoryBufferManager not set";
    return std::unique_ptr<gfx::GpuMemoryBuffer>();
  }
  return buf_manager->CreateGpuMemoryBuffer(
      size, format, gfx::BufferUsage::SCANOUT_CAMERA_READ_WRITE,
      gpu::kNullSurfaceHandle);
}

// There's no good way to resolve the HAL pixel format to the platform-specific
// DRM format, other than to actually allocate the buffer and see if the
// allocation succeeds.
ChromiumFormat CameraBufferFactory::ResolveStreamBufferFormat(
    arc::mojom::HalPixelFormat hal_format) {
  ChromiumFormat kUnsupportedFormat{PIXEL_FORMAT_UNKNOWN,
                                    gfx::BufferFormat::RGBX_8888};
  size_t kDummyBufferWidth = 128, kDummyBufferHeight = 128;

  if (resolved_hal_formats_.find(hal_format) != resolved_hal_formats_.end()) {
    return resolved_hal_formats_[hal_format];
  }
  std::vector<ChromiumFormat> cr_formats = PixFormatHalToChromium(hal_format);
  if (cr_formats.empty()) {
    return kUnsupportedFormat;
  }
  for (const auto& f : cr_formats) {
    auto buffer = CreateGpuBuffer(
        gfx::Size(kDummyBufferWidth, kDummyBufferHeight), f.gfx_format);
    if (buffer) {
      resolved_hal_formats_[hal_format] = f;
      return f;
    }
  }
  return kUnsupportedFormat;
}

}  // namespace media
