...
class CAPTURE_EXPORT VideoCaptureDeviceMFWin : public VideoCaptureDevice {
 public:
  ...
  void set_photo_callback_factory_for_testing(
      std::unique_ptr<PhotoCallbackFactoryMFWin> factory);

 private:
  void OnError(const base::Location& from_here, HRESULT hr);

  VideoCaptureDeviceDescriptor descriptor_;
  std::unique_ptr<PhotoCallbackFactoryMFWin> photo_callback_factory_;
