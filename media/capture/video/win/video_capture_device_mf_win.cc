...
static bool GetFormatFromMediaType(DWORD stream,
                                   IMFMediaType* type,
                                   VideoCaptureFormat* format) {
...
HRESULT FillCapabilities(DWORD stream,
  ...
  while (
    ...
    VideoCaptureFormat format;
    if (GetFormatFromMediaType(stream, type.Get(), &format))
...
void VideoCaptureDeviceMFWin::AllocateAndStart(
  ...
  hr = FillCapabilities(kPreferredPhotoStream, source.Get(),
                        &photo_capabilities);
  if (FAILED(hr)) {
    OnError(FROM_HERE, hr);
    return;
...
void VideoCaptureDeviceMFWin::TakePhoto(TakePhotoCallback callback) {
  ...
  VideoCaptureFormat format;
  hr = GetFormatFromMediaType(kPreferredPhotoStream, photo_media_type.Get(),
                              &format)
           ? S_OK
           : E_FAIL;
  if (FAILED(hr)) {
...
void VideoCaptureDeviceMFWin::GetPhotoState(GetPhotoStateCallback callback) {
  ...
  hr = source->GetCurrentDeviceMediaType(use_photo_stream_to_take_photo_
                                             ? kPreferredPhotoStream
                                             : kPreferredVideoPreviewStream,
                                         current_media_type.GetAddressOf());
  if (FAILED(hr)) {
    LogError(FROM_HERE, hr);
    return;
