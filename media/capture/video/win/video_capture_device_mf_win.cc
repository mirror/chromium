...
HRESULT GetAvailableDeviceMediaType(IMFCaptureSource* source,
   ...
   * undocumented MF_E_INVALIDREQUEST. Retrying one or twice solves the issue.
   */
...
void VideoCaptureDeviceMFWin::AllocateAndStart(
  ...
  if (SUCCEEDED(hr)) {
    hr = FillCapabilities(preferredVideoPreviewStream, source.Get(),
                          &capabilities);
    from_here = FROM_HERE;
  }
  ...
  if (SUCCEEDED(hr)) {
    ...
    ComPtr<IMFMediaType> type;
    hr = GetAvailableDeviceMediaType(source.Get(), preferredVideoPreviewStream,
                                     found_capability.stream_index,
                                     type.GetAddressOf());
    from_here = FROM_HERE;

    if (SUCCEEDED(hr)) {
      hr = source.Get()->SetCurrentDeviceMediaType(preferredVideoPreviewStream,
                                                   type.Get());
      from_here = FROM_HERE;
    }
    ...
    if (SUCCEEDED(hr)) {
      hr = preview_sink->AddStream(preferredVideoPreviewStream, type.Get(),
  ...
  if (capture_) {
    ...
    if (SUCCEEDED(hr)) {
      hr = source.Get()->GetCurrentDeviceMediaType(
          preferredPhotoStream, currentMediaType.GetAddressOf());
      from_here = FROM_HERE;
    }
    ...
    if (SUCCEEDED(hr)) {
      hr = source.Get()->SetCurrentDeviceMediaType(preferredPhotoStream,
                                                   photoMediaType.Get());
      from_here = FROM_HERE;
    }
    ...
    if (SUCCEEDED(hr)) {
      hr = photoSink.Get()->AddStream(
          preferredPhotoStream, photoMediaType.Get(), NULL, &dwSinkStreamIndex);
      from_here = FROM_HERE;
    }
    if (SUCCEEDED(hr)) {
...
void VideoCaptureDeviceMFWin::GetPhotoState(GetPhotoStateCallback callback) {
  ...
  if (capture_) {
    ...
    CapabilityList capabilities;
    if (SUCCEEDED(hr)) {
      hr = FillCapabilities(preferredPhotoStream, source.Get(), &capabilities);
      from_here = FROM_HERE;
    }
    ComPtr<IMFMediaType> currentMediaType;
    if (SUCCEEDED(hr)) {
  ...
  if (capture_) {
    ...
    if (SUCCEEDED(hr) && (settings->has_height || settings->has_width)) {
      ...
      if (SUCCEEDED(hr)) {
        ...
        ComPtr<IMFMediaType> type;
        hr = GetAvailableDeviceMediaType(source.Get(), preferredPhotoStream,
                                         best_match->stream_index,
                                         type.GetAddressOf());
        from_here = FROM_HERE;
        if (SUCCEEDED(hr)) {
          source.Get()->SetCurrentDeviceMediaType(preferredPhotoStream,
                                                  type.Get());
        }
      }
    }
