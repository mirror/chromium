...
void VideoCaptureDeviceMFWin::TakePhoto(TakePhotoCallback callback) {
  ...
  if (SUCCEEDED(hr)) {
    std::unique_ptr<IMFCaptureEngineOnSampleCallback> photo_callback =
        photo_callback_factory_->build(std::move(callback), std::move(format));
    hr = photo_sink.Get()->SetSampleCallback(photo_callback.release());
    from_here = FROM_HERE;
  }
  if (SUCCEEDED(hr)) {
