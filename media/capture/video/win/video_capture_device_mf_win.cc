...
VideoCaptureDeviceMFWin::VideoCaptureDeviceMFWin(
    ...
    : descriptor_(device_descriptor),
      photo_callback_factory_(new PhotoCallbackFactoryMFWin()),
      is_started_(false) {
...
void VideoCaptureDeviceMFWin::AllocateAndStart(
  ...
  hr = sink.Get()->QueryInterface(IID_PPV_ARGS(preview_sink.GetAddressOf()));
  if (FAILED(hr)) {
    OnError(FROM_HERE, hr);
    return;
  }
...
void VideoCaptureDeviceMFWin::TakePhoto(TakePhotoCallback callback) {
  ...
  hr =
      photo_sink.Get()->AddStream(kPreferredPhotoStream, photo_media_type.Get(),
  ...
  scoped_refptr<IMFCaptureEngineOnSampleCallback> photo_callback =
      photo_callback_factory_->CreateCallback(std::move(callback), format);
