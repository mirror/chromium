...
static bool GetFormatFromMediaType(IMFMediaType* type,
  ...
  if (FAILED(type->GetGUID(MF_MT_MAJOR_TYPE, &major_type_guid)) ||
      (major_type_guid != MFMediaType_Image && photo) ||
...
VideoCaptureDeviceMFWin::VideoCaptureDeviceMFWin(
    ...
    : descriptor_(device_descriptor),
      ...
      is_started_(false),
      video_stream_(static_cast<DWORD>(
          MF_CAPTURE_ENGINE_PREFERRED_SOURCE_STREAM_FOR_VIDEO_PREVIEW)),
      photo_stream_(static_cast<DWORD>(
...
void VideoCaptureDeviceMFWin::AllocateAndStart(
  ...
  hr = FillCapabilities(photo_stream_, source.Get(), true, &photo_capabilities);
  if (FAILED(hr)) {
    OnError(FROM_HERE, hr);
    return;
  }
  ...
  hr =
      FillCapabilities(video_stream_, source.Get(), false, &video_capabilities);
  if (FAILED(hr)) {
    OnError(FROM_HERE, hr);
    return;
  }
  ...
  ComPtr<IMFMediaType> type;
  hr = GetAvailableDeviceMediaType(source.Get(), video_stream_,
  ...
  hr = source->SetCurrentDeviceMediaType(video_stream_, type.Get());
  if (FAILED(hr)) {
    OnError(FROM_HERE, hr);
    return;
  }
  ...
  hr = preview_sink->AddStream(video_stream_, type.Get(), NULL,
...
void VideoCaptureDeviceMFWin::TakePhoto(TakePhotoCallback callback) {
  ...
  ComPtr<IMFMediaType> current_media_type;
  hr = source->GetCurrentDeviceMediaType(photo_stream_,
  ...
  hr = source->SetCurrentDeviceMediaType(photo_stream_, photo_media_type.Get());
  if (FAILED(hr)) {
    LogError(FROM_HERE, hr);
    return;
  }
  ...
  DWORD dw_sink_stream_index = 0;
  hr = photo_sink->AddStream(photo_stream_, photo_media_type.Get(), NULL,
                             &dw_sink_stream_index);
  if (FAILED(hr)) {
    LogError(FROM_HERE, hr);
    return;
...
void VideoCaptureDeviceMFWin::GetPhotoState(GetPhotoStateCallback callback) {
  ...
  CapabilityList capabilities;
  if (use_photo_stream_to_take_photo_)
    hr = FillCapabilities(photo_stream_, source.Get(), true, &capabilities);
  ...
  hr = source->GetCurrentDeviceMediaType(
      use_photo_stream_to_take_photo_ ? photo_stream_ : video_stream_,
      current_media_type.GetAddressOf());
  if (FAILED(hr)) {
    LogError(FROM_HERE, hr);
    return;
  ...
  photo_capabilities->height = mojom::Range::New(
      max_size.height(), min_size.height(), current_size.height(),
      std::min(max_size.height() - min_size.height(), 1));
  photo_capabilities->width = mojom::Range::New(
      max_size.width(), min_size.width(), current_size.width(),
      std::min(max_size.width() - min_size.width(), 1));
...
void VideoCaptureDeviceMFWin::SetPhotoOptions(
  ...
  if (use_photo_stream_to_take_photo_ &&
    ...
    ComPtr<IMFMediaType> type;
    hr = GetAvailableDeviceMediaType(source.Get(), photo_stream_,
    ...
    hr = source->SetCurrentDeviceMediaType(photo_stream_, type.Get());
    if (FAILED(hr)) {
      LogError(FROM_HERE, hr);
      return;
    }
