...
int GetCameraRotation() {
  ...
  if (!HasInternalDisplayDevice(&internal_display_device)) {
    return rotation;
  }
...
HRESULT CheckPathInfoForInternal(const PCWSTR device_name) {
  ...
  if (SUCCEEDED(hr)) {
    ...
    for (UINT32 path_index = 0; path_index < path_info_array_size;
      ...
      hr = HRESULT_FROM_WIN32(DisplayConfigGetDeviceInfo(&source_name.header));
      if (SUCCEEDED(hr)) {
