...
// or external USB camera) with Win32 APIs (UWP APIs are not available in
// Chromium yet.), assume it's always front camera when auto rotation is enabled
// for now.
...
// When display is only on external monitors, the auto-rotation state still may
