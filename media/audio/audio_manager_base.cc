...
enum StreamFormat {
  ...
  STREAM_FORMAT_BITSTREAM = 1,
  STREAM_FORMAT_PCM_LINEAR = 2,
  STREAM_FORMAT_PCM_LOW_LATENCY = 3,
  STREAM_FORMAT_PCM_LOW_LATENCY_FALLBACK_TO_FAKE = 4,
  STREAM_FORMAT_FAKE = 5,
  STREAM_FORMAT_MAX = 5,
};
...
AudioOutputStream* AudioManagerBase::MakeAudioOutputStreamProxy(
  ...
  if (!params.IsValid()) {
    UMA_HISTOGRAM_ENUMERATION("Media.AudioOutputStreamProxy.StreamFormat",
                              STREAM_FORMAT_INVALID, STREAM_FORMAT_MAX + 1);
    return nullptr;
  }
  ...
  if (params.format() == AudioParameters::AUDIO_PCM_LOW_LATENCY) {
    ...
    switch (output_params.format()) {
      ...
      case AudioParameters::AUDIO_FAKE:
        ...
        break;
      default:
        DCHECK(output_params.IsBitstreamFormat());
        UMA_HISTOGRAM_ENUMERATION("Media.AudioOutputStreamProxy.StreamFormat",
