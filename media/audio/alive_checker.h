...
//
// The AliveChecker can pause checking when the machine suspending, i.e. between
// suspend and resume notification from base::PowerMonitor. Checking during this
// period can cause false positives. Shorter timeout gives higher risk of false
// positives.
//
// It's bound to the task runner it's created on. All functions must be called
// on this task runner except for NotifyAlive() which can be called on any task
// runner. It must also be deleted on this task runner.
//
// It supports stopping at the first NotifyAlive(). This can be useful for
// example if the platform doesn't support suspend/resume notifications, as
// Linux.
class MEDIA_EXPORT AliveChecker {
 public:
  ...
  using CreatePowerObserverHelperCallback =
