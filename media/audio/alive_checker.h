...
// A class that checks if a mechanism that is expected to have regular activity
// is alive. For example, audio streams expect regular callbacks from the
// operating system. The mechanism to be checked informs regularly that it's
// alive by calling AliveNotification(). At a certain interval the AliveChecker
// checks that it has been notified within a timeout period. If not, it runs a
// callback to inform about detecting dead.
//
...
// with the expception of AliveNotification() which can be called on any task
// runner.
//
// It supports stopping the checking after at first AliveNotification(). This
// can be useful for example if the platform doesn't support suspend/resume
// notifications, as Linux.
//
// TODO: Use SequencedTaskRunner?
class AliveChecker {
 public:
  AliveChecker(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner,
  ...
  // TODO: Take the timeout, callback, etc. here? Or remove Start/Stop, and do
  // so in ctor/dtor?
  void Start();
  void Stop();

  // Returns whether dead was detected since started.
  bool DetectedDead();

  // Called regularly to inform that the observed mechanism is alive. Posts task
  // to run AliveNotificationOnTaskRunner().
 ...
 private:
  ...
  // Updates last alive notification time and maybe stops the timer.
  void AliveNotificationOnTaskRunner();
