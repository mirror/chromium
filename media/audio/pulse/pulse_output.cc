  ...
  while (bytes_remaining > 0) {
    ...
    if (source_callback_) {
      ...
      do {
        ...
        frames_to_copy = std::min(frames_to_copy,
                                  audio_bus_->frames() - frame_offset_in_bus);
        ...
        if (bytes_remaining > 0) {
          // Despite telling Pulse to only request certain buffer sizes, it will
          // not always obey.  In these cases we need to avoid back to back
          // reads from the renderer as it won't have time to complete the
          // request.
          //
          // We can't defer the callback as Pulse will never call us again until
          // we've satisfied writing the requested number of bytes.
          //
          // TODO(dalecurtis): It might be worth choosing the sleep duration
          // based on the hardware latency return above.
          // Watch http://crbug.com/366433 to see if a more complicated wait
          // process is necessary. We may also need to see if a PostDelayedTask
          // should be used here to avoid blocking the PA command thread.
        ...
        if (unwritten_frames_in_bus > 0) {
          ...
          pa_stream_begin_write(pa_stream_, &pa_buffer, &pa_buffer_size);
