...
void AudioInputDevice::OnSuspend() {
  task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&AudioInputDevice::SetIsSuspendingOnIOThread, this));
}

void AudioInputDevice::OnResume() {
  task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&AudioInputDevice::SetIsResumingOnIOThread, this));
}
...
void AudioInputDevice::SetIsSuspendingOnIOThread() {
  DCHECK(task_runner()->BelongsToCurrentThread());

  // We leave the timer running since we anyway need this flag to check it at
  // create. Also, we'd need to check the state and stopping hack under a lock
  // here. That makes this approach simpler and less error prone.
  is_suspending_ = true;
}

void AudioInputDevice::SetIsResumingOnIOThread() {
  DCHECK(task_runner()->BelongsToCurrentThread());
  SetLastCallbackTimeToNowOnIOThread();
