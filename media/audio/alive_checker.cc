...
AliveChecker::AliveChecker(base::RepeatingClosure dead_callback,
    ...
    : AliveChecker(std::move(dead_callback),
                   ...
                   CreatePowerObserverHelperCallback()) {}
...
AliveChecker::AliveChecker(
    ...
    : AliveChecker(std::move(dead_callback),
                   ...
                   stop_at_first_alive_notification,
                   true,
                   std::move(create_power_observer_helper_callback)) {}
...
AliveChecker::AliveChecker(
  ...
  if (pause_check_during_suspend) {
    ...
    if (create_power_observer_helper_callback.is_null()) {
    ...
    // If |pause_check_during_suspend| is false, we expect an empty create
    // callback.
    DCHECK(create_power_observer_helper_callback.is_null());
  }
}
