...
OSStatus AUHALStream::Render(AudioUnitRenderActionFlags* flags,
  ...
  if (voice_processing_) {
    DLOG_IF(ERROR, 1u != data->mBuffers[0].mNumberChannels)
        << "vp: " << data->mBuffers[0].mNumberChannels;
  } else {
    DLOG_IF(ERROR, 2u != data->mBuffers[0].mNumberChannels)
        << "df: " << data->mBuffers[0].mNumberChannels
        << " layout: " << params_.channel_layout();
  }
