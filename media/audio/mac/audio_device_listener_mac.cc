...
AudioDeviceListenerMac::AudioDeviceListenerMac(
  ...
  bool success_default_output =
      AddPropertyListener(default_output_listener_.get());
  bool success_all = success_default_output;

  bool success_default_input = true;
  if (success_all && monitor_default_input) {
    default_input_listener_ = std::make_unique<PropertyListener>(
        &kDefaultInputDeviceChangePropertyAddress, this);
    success_default_input = AddPropertyListener(default_input_listener_.get());
    success_all &= success_default_input;
  }

  bool success_addition_removal = true;
  if (success_all && monitor_addition_removal) {
    addition_removal_listener_ =
        std::make_unique<PropertyListener>(&kDevicesPropertyAddress, this);
    success_addition_removal =
        AddPropertyListener(addition_removal_listener_.get());
    success_all &= success_addition_removal;
  }
