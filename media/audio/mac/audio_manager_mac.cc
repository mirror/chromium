...
static bool GetDeviceChannels(AudioDeviceID device,
  ...
  CHECK(channels);
  /*
    // TODO: Go through active input/output streams and see if we can get
    // the information from there instead of having to open the device here.
    if (element == AUElement::OUTPUT) {
      for (auto* stream : output_streams_) {
        if (stream->device_id() == device)
          return GetDeviceChannels(stream->audio_unit(), element, channels)
      }
    } else {

    }
  */
  if (is_communications) {
    *channels = 1;
    return true;
  } else {
    *channels = 2;
  ...
  ScopedAudioUnit au(device, element, is_communications);
  if (!au.is_valid())
    return false;
...
AudioParameters AudioManagerMac::GetInputStreamParameters(
  ...
  const bool is_communications =
      (device_id == AudioDeviceDescription::kCommunicationsDeviceId);

  int channels = 0;
  int effects = AudioParameters::NO_EFFECTS;
  ChannelLayout channel_layout = CHANNEL_LAYOUT_STEREO;
  DLOG(ERROR) << "Checking device channels for: " << device_id;

  bool got_device_channels = false;
  {
    for (auto* stream : low_latency_input_streams_) {
      if (stream->device_id() == device) {
        got_device_channels = GetDeviceChannels(stream->audio_unit(),
                                                AUElement::INPUT, &channels);
        LOG_IF(ERROR, got_device_channels)
            << "got_device_channels(input1) " << channels;
        break;
      }
    }
    /*
        if (!got_device_channels) {
          for (auto* stream : basic_input_streams_) {
            if (stream->device_id() == device) {
              got_device_channels =
                  GetDeviceChannels(stream->audio_unit(), AUElement::INPUT,
                                    &channels);
              LOG_IF(ERROR, got_device_channels)
                  << "got_device_channels(input2) " << channels;
              break;
            }
          }
        }*/
  ...
  if (!got_device_channels &&
      GetDeviceChannels(device, AUElement::INPUT, is_communications,
                        &channels) &&
      channels <= 2) {
    if (is_communications) {
      DCHECK(channels == 1);
    ...
    DLOG(ERROR) << "Failed to get the device channels, use stereo as default "
                ...
                << " is_communications: " << is_communications;
  }
  ...
  params.set_effects(effects | params.effects());
...
AudioInputStream* AudioManagerMac::MakeLowLatencyInputStream(
  ...
  if (audio_device_id != kAudioObjectUnknown) {
    ...
    if (device_id == AudioDeviceDescription::kCommunicationsDeviceId) {
      // HACK
      const_cast<AudioParameters&>(params).set_effects(
...
AudioParameters AudioManagerMac::GetPreferredOutputStreamParameters(
  ...
  const bool is_communications =
      (output_device_id == AudioDeviceDescription::kCommunicationsDeviceId);

  LOG(ERROR) << "Chosen buffer size for output: " << buffer_size
             << " comms: " << is_communications;
...

  {
    ...
    for (auto* stream : output_streams_) {
      if (stream->device_id() == device) {
        got_device_channels = GetDeviceChannels(
            stream->audio_unit(), AUElement::OUTPUT, &hardware_channels);
        LOG_IF(ERROR, got_device_channels)
            << "got_device_channels " << hardware_channels;
        break;
      }
    }

    if (!got_device_channels) {
      if (!GetDeviceChannels(device, AUElement::OUTPUT, is_communications,
                             &hardware_channels)) {
        hardware_channels = is_communications ? 1 : 2;
      }
    }
  }
  ...
  // Commented out a we're fetching the device channels, not everything is
  // hardcoded
  DLOG_IF(ERROR, !effects || channel_layout == CHANNEL_LAYOUT_MONO)
      << "Non mono communications device? fx=" << effects
      << " layout=" << channel_layout;
  // DCHECK(!effects || channel_layout == CHANNEL_LAYOUT_MONO);
  DLOG_IF(ERROR, (channel_layout == CHANNEL_LAYOUT_STEREO || effects))
      << "fx:" << effects << " layout: " << channel_layout
      << " input: " << input_params.channel_layout()
