...
base::TimeDelta AudioManagerMac::GetHardwareLatency(AudioUnit au,
                                                    AudioDeviceID device_id,
                                                    AudioUnitElement element,
                                                    int sample_rate) {
  if (!au || device_id == kAudioObjectUnknown) {
  ...
  OSStatus result = AudioUnitGetProperty(au, kAudioUnitProperty_Latency,
  ...
  AudioObjectPropertyAddress property_address = {
      kAudioDevicePropertyLatency, element, kAudioObjectPropertyElementMaster};
  ...
  DLOG_IF(WARNING, result != noErr) << "Could not get stream latency.";
