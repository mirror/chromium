...
enum DispatchOverrideStatus {
  ...
  STATUS_RESUMEIO_CALLSITE_FOUND = 6,
  STATUS_PAUSEIO_CALLSITE_FOUND = 7,
  STATUS_LOOKUP_MISS = 8,
  STATUS_MAX = STATUS_LOOKUP_MISS
};
...
bool AddressIsPauseOrResume(intptr_t address) {
  ...
  if (!resumeio_callsite && info.dli_sname &&
    ...
    base::subtle::NoBarrier_CompareAndSwap(&g_resumeio_callsite, 0,
                                           resumeio_callsite);
    LogStatusChange(STATUS_RESUMEIO_CALLSITE_FOUND);
    ...
    base::subtle::NoBarrier_CompareAndSwap(&g_pauseio_callsite, 0,
                                           pauseio_callsite);
    LogStatusChange(STATUS_PAUSEIO_CALLSITE_FOUND);
  } else {
    LogStatusChange(STATUS_LOOKUP_MISS);
  }
