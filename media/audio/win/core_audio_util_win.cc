...
using UMALogCallback = base::RepeatingCallback<void(LogStep, HRESULT)>;

// Empty UMA logging callback to be passed to functions that don't need to log
// any UMA stats
void LogUMAEmptyCb(LogStep step, HRESULT hr) {}
...
ComPtr<IMMDeviceEnumerator> CreateDeviceEnumeratorInternal(
  ...
  uma_log_cb.Run(CREATE_DEVICE_ENUMERATOR, hr);
...
ComPtr<IMMDevice> CreateDeviceInternal(const std::string& device_id,
  ...
  if (FAILED(hr)) {
    ...
    uma_log_cb.Run(CREATE_DEVICE, hr);
    return endpoint_device;
  }
...
ComPtr<IMMDevice> CreateDeviceByID(const std::string& device_id,
  ...
  if (device_id == AudioDeviceDescription::kLoopbackInputDeviceId ||
    ...
    return CreateDeviceInternal("", eRender, eConsole, uma_log_cb);
  } else if (device_id == AudioDeviceDescription::kCommunicationsDeviceId)
    return CreateDeviceInternal("", data_flow, eCommunications, uma_log_cb);
...
ComPtr<IAudioClient> CreateClientInternal(IMMDevice* audio_device,
  ...
  uma_log_cb.Run(CREATE_CLIENT, hr);
  return audio_client;
}
...
HRESULT GetPreferredAudioParametersInternal(IAudioClient* client,
  ...
  uma_log_cb.Run(GET_MIX_FORMAT, hr);
  if (FAILED(hr))
    return hr;
  ...
  hr = CoreAudioUtil::GetDevicePeriod(client, AUDCLNT_SHAREMODE_SHARED,
                                      &default_period);
  uma_log_cb.Run(GET_DEVICE_PERIOD, hr);
  if (FAILED(hr))
    return hr;
...
ComPtr<IAudioClient> CoreAudioUtil::CreateClient(const std::string& device_id,
                                                 EDataFlow data_flow,
                                                 ERole role) {
  ComPtr<IMMDevice> device(CreateDeviceInternal(
      device_id, data_flow, role, base::BindRepeating(&LogUMAEmptyCb)));
