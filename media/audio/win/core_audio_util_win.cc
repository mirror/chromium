...
void LogPreferredAudioParamsUMA(PreferredParamFailStep err,
                                HRESULT hr,
                                bool is_output_device) {
  if (is_output_device) {
    switch (err) {
...
HRESULT CoreAudioUtil::GetPreferredAudioParameters(IAudioClient* client,
  ...
  LogPreferredAudioParamsUMA(GET_MIX_FORMAT, hr, is_output_device);
  if (FAILED(hr))
    return hr;

  REFERENCE_TIME default_period = 0;
  hr = GetDevicePeriod(client, AUDCLNT_SHAREMODE_SHARED, &default_period);
  LogPreferredAudioParamsUMA(GET_DEVICE_PERIOD, hr, is_output_device);
  if (FAILED(hr))
    return hr;
...
HRESULT CoreAudioUtil::GetPreferredAudioParameters(const std::string& device_id,
  ...
  LogPreferredAudioParamsUMA(CREATE_DEVICE, device.second, is_output_device);
  if (!device.first.Get())
    return device.second;

  std::pair<ComPtr<IAudioClient>, HRESULT> client(
      CreateClientInternal(device.first.Get()));
  LogPreferredAudioParamsUMA(CREATE_CLIENT, client.second, is_output_device);
  if (!client.first.Get())
