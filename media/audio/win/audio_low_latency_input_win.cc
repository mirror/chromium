      ...
      case WAIT_OBJECT_0 + 1: {
        ...
        while (fifo_->available_blocks()) {
          if (converter_) {
            ...
            capture_time += AudioTimestampHelper::FramesToTime(
                convert_bus_->frames(), input_format_.nSamplesPerSec);
          } else {
...
void WASAPIAudioInputStream::ReportOpenResult(HRESULT hr) const {
  ...
  if (open_result_ != OPEN_RESULT_OK &&
    ...
    log_callback_.Run(base::StringPrintf(
        ...
        open_result_, hr, input_format_.wFormatTag, input_format_.nChannels,
        input_format_.nSamplesPerSec, input_format_.wBitsPerSample,
        input_format_.nBlockAlign, input_format_.nAvgBytesPerSec,
        input_format_.cbSize, output_format_.wFormatTag,
        output_format_.nChannels, output_format_.nSamplesPerSec,
        output_format_.wBitsPerSample, output_format_.nBlockAlign,
        output_format_.nAvgBytesPerSec, output_format_.cbSize));
  }
}
