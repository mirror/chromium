...
bool WASAPIAudioInputStream::DesiredFormatIsSupported(HRESULT* hr) {
  ...
  if (hresult == S_FALSE) {
    ...
    new_format.wBitsPerSample = format_.wBitsPerSample;
    if (closest_match->wFormatTag == WAVE_FORMAT_PCM) {
      new_format.wBitsPerSample = closest_match->wBitsPerSample;
    } else if (closest_match->wFormatTag == WAVE_FORMAT_EXTENSIBLE) {
      WAVEFORMATEXTENSIBLE* closest_match_ex =
          reinterpret_cast<WAVEFORMATEXTENSIBLE*>(closest_match.get());
      if (closest_match_ex->SubFormat == KSDATAFORMAT_SUBTYPE_PCM)
        new_format.wBitsPerSample = closest_match->wBitsPerSample;
    }
