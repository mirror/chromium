...
bool WASAPIAudioInputStream::Open() {
  ...
  if (opened_) {
    log_callback_.Run("WASAPI Input Stream already open");
    return false;
  }
...
HRESULT WASAPIAudioInputStream::DesiredFormatIsSupported() {
...
void WASAPIAudioInputStream::ReportOpenResult(HRESULT hr) const {
  ...
  if (open_result_ != OPEN_RESULT_OK &&
    ...
    log_callback_.Run(base::StringPrintf(
        "WASAPI Input Open failed: result = %d, hresult = %#lx", open_result_,
        hr));
  }
}
