...
bool WASAPIAudioInputStream::DesiredFormatIsSupported(HRESULT* hr) {
  ...
  *hr = audio_client_->IsFormatSupported(AUDCLNT_SHAREMODE_SHARED, &format_,
                                         &closest_match);
  ...
  // At this point, |*hr| == S_OK if the desired format is supported. If
  // |*hr| == S_FALSE, the OS supports a closest match but we don't support
