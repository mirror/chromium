...
AudioManagerHelper::AudioManagerHelper(AudioManager* audio_manager)
...
void AudioManagerHelper::GetInputStreamParameters(
...
void AudioManagerHelper::GetOutputStreamParameters(
    const std::string& device_id,
    OnAudioParamsCallback on_params_cb) {
  DCHECK(GetTaskRunner()->BelongsToCurrentThread());
...
void AudioManagerHelper::HasInputDevices(OnBoolCallback on_has_devices_cb) {
  DCHECK(GetTaskRunner()->BelongsToCurrentThread());
  std::move(on_has_devices_cb).Run(audio_manager_->HasAudioInputDevices());
}

void AudioManagerHelper::HasOutputDevices(OnBoolCallback on_has_devices_cb) {
  DCHECK(GetTaskRunner()->BelongsToCurrentThread());
  std::move(on_has_devices_cb).Run(audio_manager_->HasAudioOutputDevices());
}

void AudioManagerHelper::GetDeviceDescriptions(
    bool for_input,
...
void AudioManagerHelper::GetAssociatedOutputDeviceID(
...
void AudioManagerHelper::GetInputDeviceInfo(
    const std::string& input_device_id,
...
base::SingleThreadTaskRunner* AudioManagerHelper::GetTaskRunner() {
  return audio_manager_->GetTaskRunner();
}

AudioParameters AudioManagerHelper::ComputeInputParameters(
    const std::string& device_id) {
  DCHECK(GetTaskRunner()->BelongsToCurrentThread());
...
AudioParameters AudioManagerHelper::ComputeOutputParameters(
    const std::string& device_id) {
  DCHECK(GetTaskRunner()->BelongsToCurrentThread());
