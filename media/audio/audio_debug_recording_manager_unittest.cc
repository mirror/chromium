...
void OnFileCreatedEmptyCallback(base::File) {}
...
TEST_F(AudioDebugRecordingManagerTest, FileCreation) {
  EXPECT_CALL(manager_, DoOnFileCreated(true));
  manager_.CreateFile(base_file_path_,
                      base::BindOnce(&OnFileCreatedEmptyCallback));

  scoped_task_environment_.RunUntilIdle();
  base::FilePath debug_file_name(
      base_file_path_.AddExtension(FILE_PATH_LITERAL("wav")));
  EXPECT_TRUE(base::PathExists(debug_file_name));
  EXPECT_TRUE(base::DeleteFile(debug_file_name, false));
}

TEST_F(AudioDebugRecordingManagerTest, FileCreationError) {
  base::FilePath file_path;  // Empty file name.
  EXPECT_CALL(manager_, DoOnFileCreated(_)).Times(0);
  manager_.CreateFile(file_path, base::BindOnce(&OnFileCreatedEmptyCallback));
  scoped_task_environment_.RunUntilIdle();
}
