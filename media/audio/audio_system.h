...
class MEDIA_EXPORT AudioSystem {
 public:
  // Replies are sent asynchronously to a thread the calls are issued on.
  // Instance is bound to a thread it's called on the first time.
  ...
  static std::unique_ptr<AudioSystem> GetInstance();
