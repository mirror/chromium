...
void AudioOutputDevice::OnDeviceAuthorized(
  ...
  auth_timeout_action_.reset();
  // 30000 ms is the hung renderer timeout, and times over 30 s should be very
  // rare, so we don't lose interesting data by making 30 s the upper limit.
  // We use equally-sized buckets to better track the amount of very long
  // authorization times.
  UMA_HISTOGRAM_CUSTOM_COUNTS(
      "Media.Audio.Render.OutputDeviceAuthorizationTime",
      (base::TimeTicks::Now() - auth_start_time_).InMilliseconds(), 1, 30000,
      300);
