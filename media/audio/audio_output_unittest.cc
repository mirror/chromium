...
// This test produces actual audio for .5 seconds on the default device.
TEST_F(AudioOutputTest, Play200HzTone) {
  ABORT_AUDIO_TEST_IF_NOT(audio_manager_device_info_->HasAudioOutputDevices());

  stream_params_ =
      audio_manager_device_info_->GetDefaultOutputStreamParameters();
  uint32_t samples_100_ms = stream_params_.sample_rate() / 10;
  stream_params_.set_frames_per_buffer(samples_100_ms);
