...
class MEDIA_EXPORT AudioDebugRecordingHelper : public AudioDebugRecorder {
 public:
  ...
  // CreateFileCallback is ran to create debug recording file.
  virtual void EnableDebugRecording(const base::FilePath& file_name);
  // Reply callback passed to CreateFileCallback, called after debug recording
  // file was created
 ...
 private:
  ...
  // Callback used for creating debug recording file. Created file is passed to
  // through reply callback, EnableDebugRecordingToFile.
  // This callback is bound to unretained AudioDebugRecordingManager, it must
  // not outlive helper - intended to be Run in helper, not passed to another
  // thread.
