...
class AudioDebugFileWriter::AudioFileWriter {
 ...
 private:
  ...
  void CreateRecordingFile(base::File file_handle);
...
void AudioDebugFileWriter::Start(base::File file_handle) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(client_sequence_checker_);
  DCHECK(!file_writer_);
  file_writer_ = AudioFileWriter::Create(std::move(file_handle), params_,
                                         file_task_runner_);
}
