...
std::string MicPositions(const chromeos::AudioDeviceList& devices) {
  // An internal mic with a non-empty positions field indicates the machine has
  // a beamforming capable mic array.
...
std::string GetInternalSpeakerId(const chromeos::AudioDeviceList& devices) {
  for (const auto& device : devices) {
...
void AudioManagerCras::GetAudioInputDeviceNames(
  ...
  chromeos::AudioDeviceList devices;
  GetAudioDevices(&devices);
  mic_positions_ = ParsePointsFromString(MicPositions(devices));
...
void AudioManagerCras::GetAudioDevices(chromeos::AudioDeviceList* devices) {
  ...
  main_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&AudioManagerCras::GetAudioDevicesOnMainThread, weak_this_,
                 base::Unretained(devices), base::Unretained(&event)));
  base::WaitableEvent* waitables[] = {&event, &on_shutdown_};
  base::WaitableEvent::WaitMany(waitables, arraysize(waitables));
  event.Wait();
}
...
void AudioManagerCras::GetPrimaryActiveOutputNodeOnMainThread(
  ...
  auto* audio_handler = chromeos::CrasAudioHandler::Get();
  if (audio_handler) {
    *active_output_node_id = audio_handler->GetPrimaryActiveOutputNode();
  }
  event->Signal();
}
