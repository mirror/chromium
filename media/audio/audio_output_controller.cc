...
void AudioOutputController::ErrorStatisticsTracker::OnPlay() {
  ...
  // it.
  //
  // Note that the timer is stopped in case the stream is paused or closed,
  // and restarted when the stream starts playing again.
  if (!wedge_status_logged_) {
    wedge_timer_.Start(FROM_HERE, TimeDelta::FromSeconds(5), this,
                       &ErrorStatisticsTracker::WedgeCheck);
  }
}

void AudioOutputController::ErrorStatisticsTracker::OnPause() {
  wedge_timer_.AbandonAndStop();
}
