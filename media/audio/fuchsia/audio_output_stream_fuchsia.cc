...
void AudioOutputStreamFuchsia::PumpSamples() {
  ...
  // Restart the stream if we missed timer deadline, e.g. after the system was
  // suspended.
  if (!started_time_.is_null() &&
      base::TimeTicks::Now() > GetCurrentStreamTime()) {
    started_time_ = base::TimeTicks();
  }

  bool reset_pts = false;
  ...
  do {
    // Presentation time (PTS) has to be specified only for the first frame
    ...
    zx_time_t presentation_time =
        reset_pts
            ? (zx_time_get(ZX_CLOCK_MONOTONIC) + presentation_delay_ns_ + 1000)
            : 0;
    reset_pts = false;
