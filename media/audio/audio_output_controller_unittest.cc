...
class AudioOutputControllerTest : public TestWithParam<bool> {
 ...
 protected:
  ...
  void PlayWhileDiverting() {
    ...
    EXPECT_CALL(mock_stream_, DidStart()).WillOnce(RunClosure(wait_for_all));
    EXPECT_CALL(mock_event_handler_, OnControllerPlaying())
        .WillOnce(RunClosure(wait_for_all));
    EXPECT_CALL(mock_sync_reader_, RequestMoreData(_, _, _))
        .WillOnce(RunClosure(wait_for_all))
  ...
  void DivertWhilePlaying() {
    ...
    base::RepeatingClosure wait_for_all =
        base::BarrierClosure(4, loop.QuitClosure());
    EXPECT_CALL(mock_stream_, DidOpen()).WillOnce(RunClosure(wait_for_all));
    EXPECT_CALL(mock_stream_, SetVolume(kTestVolume))
        .WillOnce(RunClosure(wait_for_all));
    EXPECT_CALL(mock_stream_, DidStart()).WillOnce(RunClosure(wait_for_all));
    EXPECT_CALL(mock_event_handler_, OnControllerPlaying())
  ...
  void StopDuplicating(MockAudioPushSink* sink) {
    {
      // First, verify we're still getting callbacks.
      base::RunLoop loop;
      audio_manager_->GetTaskRunner()->PostTask(
          FROM_HERE,
          base::BindOnce(
              [](MockAudioPushSink* sink, base::OnceClosure done_closure) {
