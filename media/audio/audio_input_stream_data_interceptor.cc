...
AudioInputStreamDataInterceptor::AudioInputStreamDataInterceptor(
    ...
    : register_debug_recording_source_cb_(
          std::move(register_debug_recording_source_cb)),
      stream_(stream) {
  DCHECK(register_debug_recording_source_cb);
  DCHECK(stream_);
}
...
void AudioInputStreamDataInterceptor::Start(
  ...
  DCHECK(stream_);
  callback_ = callback;
  if (register_debug_recording_source_cb_)
    debug_recorder_ = register_debug_recording_source_cb_.Run();
  stream_->Start(this);
}
