...
// Returns file name suffix created by appending |id| to |direction|.
base::FilePath GetDebugRecordingFileNameSuffix(
    const base::FilePath::StringType& direction,
    int id) {
  return base::FilePath(direction).AddExtension(IntToStringType(id));
}
...
void AudioDebugRecordingManager::EnableDebugRecording(
  ...
  create_file_callback_ = create_file_callback;

  for (const auto& it : debug_recording_helpers_) {
    int id = it.first;
    AudioDebugRecordingHelper* recording_helper = it.second.first;
    const base::FilePath::StringType& direction = it.second.second;
    recording_helper->EnableDebugRecording(
        GetDebugRecordingFileNameSuffix(direction, id), create_file_callback_);
  }
}
...
AudioDebugRecordingManager::RegisterDebugRecordingSource(
  ...
  debug_recording_helpers_[id] =
      std::make_pair(recording_helper.get(), direction);
