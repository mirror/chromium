...
// Appends |file_name_extension| and |id| to be
// added to debug recording file name as extensions.
base::FilePath GetDebugRecordingFileNameExtensions(
    const base::FilePath::StringType& file_name_extension,
    int id) {
  return base::FilePath(file_name_extension).AddExtension(IntToStringType(id));
}
...
void AudioDebugRecordingManager::EnableDebugRecording(
  ...
  for (const auto& it : debug_recording_helpers_) {
    ...
    recording_helper->EnableDebugRecording(
        GetDebugRecordingFileNameExtensions(file_name_extension, id),
