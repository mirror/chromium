...
class AudioDebugRecordingSessionImplTest : public testing::Test {
 public:
  void CreateAudioManager() {
    mock_audio_manager_ =
        std::make_unique<MockAudioManager>(std::make_unique<AudioThreadImpl>());
    ASSERT_TRUE(AudioManager::Get() != nullptr);
  }
...
TEST_F(AudioDebugRecordingSessionImplTest,
       EnableDisableNotCalledAfterAudioManagerShutdown) {
  CreateAudioManager();
  ShutdownAudioManager();

  EXPECT_CALL(*mock_audio_manager_, EnableDebugRecording(file_path_)).Times(0);
  CreateDebugRecordingSession();

  EXPECT_CALL(*mock_audio_manager_, DisableDebugRecording()).Times(0);
  DestroyDebugRecordingSession();
}
}  // namespace media
