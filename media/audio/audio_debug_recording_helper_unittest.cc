...
TEST_F(AudioDebugRecordingHelperTest, EnableDisable) {
  ...
  base::File file_reopened(
      file_path_, base::File::FLAG_OPEN_ALWAYS | base::File::FLAG_WRITE);
  EXPECT_TRUE(file_reopened.IsValid());
  recording_helper->EnableDebugRecording(std::move(file_reopened));
  EXPECT_CALL(*static_cast<MockAudioDebugFileWriter*>(
                  recording_helper->debug_writer_.get()),
              Stop());
  DisableDebugRecording(recording_helper.get());
