...
class MEDIA_EXPORT AudioDebugRecordingManager {
 public:
  using CreateFileRepeatingCallback = base::RepeatingCallback<void(
      const base::FilePath&,
  ...
  virtual void EnableDebugRecording(
      CreateFileRepeatingCallback create_file_callback);
  virtual void DisableDebugRecording();

  // Registers a source and returns a wrapped recorder. |stream_type| is
 ...
 private:
  ...
  CreateFileRepeatingCallback create_file_callback_;
