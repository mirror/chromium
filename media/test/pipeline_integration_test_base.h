...
class PipelineIntegrationTestBase : public Pipeline::Client {
 public:
  ...
  // Temporarily caches a test callback that will be given to the
  // AudioRendererImpl upon next CreateRenderer().
  void set_first_audio_packet_timestamp_cb(
      AudioRendererImpl::FirstPacketTimestampCBForTesting cb) {
    first_audio_packet_timestamp_cb_ = std::move(cb);
  }
 ...
 protected:
  ...
  AudioRendererImpl::FirstPacketTimestampCBForTesting
      first_audio_packet_timestamp_cb_;
