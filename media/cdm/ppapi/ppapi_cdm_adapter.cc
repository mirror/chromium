...
void PpapiCdmAdapter::RequestStorageId() {
  ...
  if (allow_persistent_state_) {
    linked_ptr<PepperStorageIdResponse> response(new PepperStorageIdResponse());
    int32_t result = platform_verification_.GetStorageId(
        &response->storage_id,
        callback_factory_.NewCallback(&PpapiCdmAdapter::RequestStorageIdDone,
                                      response));
    if (result == PP_OK_COMPLETIONPENDING)
      return;
...
void PpapiCdmAdapter::RequestStorageIdDone(
  ...
  if (result == PP_OK)
    storage_id = response->storage_id.AsString();
