...
class MEDIA_EXPORT CdmProxy {
 public:
  ...
  // Callback for Process(). |output| is the output of processing.
  using ProcessCB =
      base::OnceCallback<void(Status status,
                              const std::vector<uint8_t>& output)>;

  // Processes and updates the state of the proxy.
  // |expected_output_size| is the size of the output data. Whether this value
  // is required or not is protocol dependent.
  ...
  // |private_output_data| is set to a valid value if
  // |crypto_session_private_data| was non-empty.
  using CreateMediaCryptoSessionCB =
      base::OnceCallback<void(Status status,
                              uint32_t crypto_session_id,
                              uint64_t private_output_data)>;

  // Creates a crypto session for handling media.
  // If extra data has to be passed to further setup the video crypto session,
  // pass data to |crypto_session_private_data|.
  ...
  virtual void CreateMediaCryptoSession(
      const std::vector<uint8_t>& crypto_session_private_data,
