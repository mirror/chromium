...
// The interface for the CDM to talk to the proxy.
class CdmProxy {
 public:
  // Client of the proxy.
  class Client {
  ...
  enum Protocol {
    ...
    KIntelCSMEv10,
    // Method using Intel PAVP.
    kIntelPAVP,
    // RSA method where session keys are handled in hardware.
    kRsaHardware,
    // RSA method where session keys are handled in software.
    kRsaSoftware,
  };
  ...
  CdmProxy(std::unique_ptr<Client> client);
  virtual ~CdmProxy();
  ...
  virtual void Process(uint32_t function_id,
                       uint32_t crypto_session_id,
