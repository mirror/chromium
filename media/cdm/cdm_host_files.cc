...
void CdmHostFiles::InitializeWithAdapter(
  ...
  OpenCdmFilesWithAdapter(cdm_adapter_path);
...
void CdmHostFiles::OpenCdmFilesWithAdapter(
    const base::FilePath& cdm_adapter_path) {
  ...
  cdm_specific_files_.push_back(std::move(cdm_adapter_file));
  cdm_specific_files_.push_back(std::move(cdm_file));
}

void CdmHostFiles::OpenCdmFiles(const base::FilePath& cdm_path) {
  std::unique_ptr<CdmHostFile> cdm_file =
      CdmHostFile::Create(cdm_path, GetSigFilePath(cdm_path));
