...
class MojoCdmProxyTest : public MojoCdmProxy::Delegate, public ::testing::Test {
 public:
  ...
  void Initialize(Status expected_status = Status::kOk,
    ...
    // Client should always be called no matter has connection or not.
    EXPECT_CALL(client_, OnInitialized(StatusEq(expected_status), _, _))
        .WillOnce(SaveArg<2>(&crypto_session_id_));

    if (has_connection) {
      EXPECT_CALL(*mock_cdm_proxy_, OnInitialize(_, _))
  ...
  void Process(Status expected_status = Status::kOk,
    ...
    if (has_connection) {
      EXPECT_CALL(*mock_cdm_proxy_, OnProcess(_, _, _, _, _))
          .WillOnce(
              RunOnceCallback<4>(expected_status, std::vector<uint8_t>()));
    }

    std::vector<uint8_t> input_data = {1, 2};
    cdm_proxy_->Process(
        cdm::CdmProxy::Function::kIntelNegotiateCryptoSessionKeyExchange,
        crypto_session_id_, input_data.data(), input_data.size(), 5);
    base::RunLoop().RunUntilIdle();
  }
  ...
  void CreateMediaCryptoSession(Status expected_status = Status::kOk,
    ...
    if (has_connection) {
      EXPECT_CALL(*mock_cdm_proxy_, OnCreateMediaCryptoSession(_, _))
          .WillOnce(RunOnceCallback<1>(expected_status, 0, 0));
    }

    std::vector<uint8_t> input_data = {1, 2};
    cdm_proxy_->CreateMediaCryptoSession(input_data.data(), input_data.size());
    base::RunLoop().RunUntilIdle();
  }

  void SetKey() {
    EXPECT_CALL(*mock_cdm_proxy_, SetKey(_, _, _));
    const std::vector<uint8_t> key_id = {3, 4};
    const std::vector<uint8_t> key_blob = {5, 6};
  ...
  void RemoveKey() {
    EXPECT_CALL(*mock_cdm_proxy_, RemoveKey(_, _));
    const std::vector<uint8_t> key_id = {3, 4};
