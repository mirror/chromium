...
class MEDIA_MOJO_EXPORT VideoDecodePerfHistory
 ...
 public:
  VideoDecodePerfHistory(std::unique_ptr<VideoDecodeStatsDBFactory> db_factory);
 ...
 private:
  ...
  // Queue of bound public API calls, to be run once DB initialization
  // completes.
  std::queue<base::OnceClosure> init_deferred_api_calls_;
