...
void MojoVideoEncodeAccelerator::Encode(int32_t buffer_id,
  ...
  if (!encoder_)
    return;
  if (buffer_id < 0) {
    DLOG(ERROR) << __func__ << " buffer_id=" << buffer_id << " must be >= 0";
  ...
  */
  // TODO(mcasas): bind a |frame| destruction observer to
  /*
...
void MojoVideoEncodeAccelerator::UseOutputBitstreamBuffer(
  ...
  if (memory_size > output_buffer_size_) {
    DLOG(ERROR) << __func__ << " buffer_id=" << buffer_id << " has a size of "
                << memory_size << "B, larger than expected "
                << output_buffer_size_ << "B";
    NotifyError(::media::VideoEncodeAccelerator::kPlatformFailureError);
    return;
  }

  encoder_->UseOutputBitstreamBuffer(
      BitstreamBuffer(buffer_id, handle, memory_size));
}
...
void MojoVideoEncodeAccelerator::EncodeFrameFinished(int32_t buffer_id) {
  DVLOG(1) << __func__;
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  DCHECK(vea_client_);
  vea_client_->NotifyInputDone(buffer_id);
}
