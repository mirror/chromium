...
void MojoDecryptorService::ResetDecoder(StreamType stream_type) {
  ...
  GetBufferReader(stream_type)
      ->Reset(base::Bind(&MojoDecryptorService::OnReaderResetDone, weak_this_,
                         stream_type));
}
...
void MojoDecryptorService::OnReaderResetDone(StreamType stream_type) {
  DVLOG(1) << __func__ << ": stream_type = " << stream_type;
...
MojoDecoderBufferReader* MojoDecryptorService::GetBufferReader(
  ...
  if (stream_type != StreamType::kAudio && stream_type != StreamType::kVideo) {
    NOTREACHED() << "Unexpected stream_type: " << stream_type;
    return nullptr;
  }

  return stream_type == StreamType::kAudio ? audio_buffer_reader_.get()
                                           : video_buffer_reader_.get();
}
