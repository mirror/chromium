...
void MojoCdmFileIO::DoRead(int64_t num_bytes) {
  ...
  int bytes_read =
      file_.Read(0, reinterpret_cast<char*>(buffer.data()), bytes_to_read);
...
void MojoCdmFileIO::Write(const uint8_t* data, uint32_t data_size) {
  ...
  cdm_file_->OpenTempFileForWrite(base::BindOnce(
      &MojoCdmFileIO::OnWriteFileOpened, weak_factory_.GetWeakPtr(),
      std::vector<uint8_t>(data, data + data_size)));
}

void MojoCdmFileIO::OnWriteFileOpened(const std::vector<uint8_t>& data,
  ...
  if (!temporary_file.IsValid()) {
    // Failed to open temporary file. Setting |state_| to Opened in case it's
    // transient. Original file is still open for reading.
    state_ = State::kOpened;
    OnError(ErrorType::kWriteError);
    return;
  }
  ...
  DCHECK_EQ(0u, temporary_file.GetLength());
  ...
  if (bytes_to_write > 0) {
    ...
    if (bytes_written != bytes_to_write) {
      // Failed to write to the temporary file. Setting |state_| to Opened
      // as the original file is still open for reading.
      OnError(ErrorType::kWriteError);
      return;
    }
  }

  // Close both the original file and the temporary file returned before
  // renaming.
  file_.Close();
  temporary_file.Close();
  cdm_file_->RenameTempFileAndReopen(base::BindOnce(
      &MojoCdmFileIO::OnFileWritten, weak_factory_.GetWeakPtr()));
}

void MojoCdmFileIO::OnFileWritten(base::File reopened_file) {
  DVLOG(3) << __func__ << " file: " << file_name_;
  DCHECK_EQ(State::kWriting, state_);
  DCHECK(!file_.IsValid()) << "Original file was not closed.";
