...
void MojoCdmFileIO::Open(const char* file_name, uint32_t file_name_size) {
  ...
  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE,
      base::BindOnce(&MojoCdmFileIO::DoOpen, weak_factory_.GetWeakPtr(), name));
}

void MojoCdmFileIO::DoOpen(const std::string& file_name) {
...
void MojoCdmFileIO::Write(const uint8_t* data, uint32_t data_size) {
  ...
  cdm_file_->OpenFileForWrite(base::BindOnce(
...
void MojoCdmFileIO::OnWriteFileOpened(const std::vector<uint8_t>& data,
                                      base::File file) {
  DVLOG(3) << __func__ << " file: " << file_name_
           << ", result: " << base::File::ErrorToString(file.error_details());
  ...
  // Close both the original file and the temp file returned.
  file_.Close();
  file.Close();
  cdm_file_->DoneWriting(base::BindOnce(&MojoCdmFileIO::OnFileWritten,
                                        weak_factory_.GetWeakPtr()));
}

void MojoCdmFileIO::OnFileWritten(base::File file) {
  DVLOG(3) << __func__ << " file: " << file_name_;
