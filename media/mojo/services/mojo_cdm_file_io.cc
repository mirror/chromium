...
bool MojoCdmFileIO::StateManager::SetState(OperationType op) {
  switch (op) {
...
MojoCdmFileIO::FileInUseTracker::FileInUseTracker() : binding_(this) {}

MojoCdmFileIO::FileInUseTracker::~FileInUseTracker() = default;

mojom::CdmFileInUsePtr
MojoCdmFileIO::FileInUseTracker::CreateInterfacePtrAndBind() {
  mojom::CdmFileInUsePtr file_in_use;
...
MojoCdmFileIO::MojoCdmFileIO(const std::string& key_system,
...
void MojoCdmFileIO::Open(const char* file_name, uint32_t file_name_size) {
  ...
  storage_->Initialize(key_system_,
                       base::BindOnce(&MojoCdmFileIO::OnInitializeComplete,
                                      weak_factory_.GetWeakPtr()));
}

void MojoCdmFileIO::OnInitializeComplete(bool status) {
