...
void MojoCdmFileIO::Read() {
  ...
  // Read only allowed if the current state is kOpened. DoRead() will check
  // whether it succeeded or not as the client must be notified asynchronously.
  ...
  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE, base::BindOnce(&MojoCdmFileIO::DoRead,
                                weak_factory_.GetWeakPtr(), original_state));
}

void MojoCdmFileIO::DoRead(State original_state) {
  DVLOG(3) << __func__ << " file: " << file_name_;
  ...
  // If another read/write operation is in progress, we can't proceed.
  if (original_state == State::kReading || original_state == State::kWriting) {
    NotifyClientOfReadFailure(Result::kFileInUse);
    return;
  }

  // If the file hasn't been opened yet, we can't proceed. This also covers
  // open in progress and error states.
  if (original_state != State::kOpened) {
...
void MojoCdmFileIO::NotifyClientOfReadFailure(Result result) {
  ...
  ClientStatus client_status;
  switch (result) {
    case Result::kOperationSucceeded:
      // Successful read. However, since data needs to be returned,
      // NotifyClientOfReadSuccess() should have been called instead.
      NOTREACHED() << __func__ << " called with kOperationSucceeded.";
      state_ = State::kOpened;
      client_status = ClientStatus::kSuccess;
      break;
    case Result::kOperationFailed:
    ...
    case Result::kFileNotOpen:
      ...
      client_status = ClientStatus::kError;
      break;
    case Result::kFileInUse:
      // File is being accessed by another operation concurrently.
      client_status = ClientStatus::kInUse;
      break;
    case Result::kFileNotUseable:
      // Something wrong with the file.
      state_ = State::kError;
      client_status = ClientStatus::kError;
      break;
    case Result::kFileTooBig:
      // File is too big to read. This is an error, but the file can be
      // overwritten, so report an error but let future writes succeed.
      state_ = State::kOpened;
      client_status = ClientStatus::kError;
      break;
  }
...
void MojoCdmFileIO::Write(const uint8_t* data, uint32_t data_size) {
  ...
  if (state_ == State::kReading || state_ == State::kWriting) {
    ReportWriteResult(Result::kFileInUse);
    return;
  }
...
void MojoCdmFileIO::NotifyClientOfWriteResult(Result result) {
  ...
  switch (result) {
    ...
    case Result::kOperationFailed:
      ...
      break;
    case Result::kFileNotOpen:
      // File not open. No need to reset the state as it wasn't changed.
      client_status = ClientStatus::kError;
      break;
    case Result::kFileInUse:
      // File is being accessed by another operation concurrently.
    ...
    case Result::kFileNotUseable:
      ...
      break;
    case Result::kFileTooBig:
      // Too much data to write. This is an error, but the existing file is
      // still valid, so report an error but let subsequent attempts try again.
      state_ = State::kOpened;
      client_status = ClientStatus::kError;
      break;
  }
