...
MojoCdmFileIO::MojoCdmFileIO(cdm::FileIOClient* client,
                             ...
                             FirstFileReadCB first_file_read_cb)
    : client_(client),
      cdm_storage_(cdm_storage),
      first_file_read_cb_(std::move(first_file_read_cb)),
      weak_factory_(this) {
  DVLOG(3) << __func__;
  DCHECK(client_);
  DCHECK(cdm_storage_);
  // |first_file_read_cb_| may be null in tests.
}
...
void MojoCdmFileIO::DoRead(int64_t num_bytes) {
  ...
  if (first_file_read_cb_)
    std::move(first_file_read_cb_).Run(bytes_to_read);
