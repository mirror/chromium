...
void MojoCdmFileIO::Write(const uint8_t* data, uint32_t data_size) {
  ...
  // Now open a temporary file for writing. Close |file_| as it won't be used
  // again.
  state_ = State::kWriting;
  file_.Close();
  cdm_file_->OpenTempFileForWriting(base::BindOnce(
      &MojoCdmFileIO::OnTempFileOpenedForWrite, weak_factory_.GetWeakPtr(),
      std::vector<uint8_t>(data, data + data_size)));
}

void MojoCdmFileIO::OnTempFileOpenedForWrite(const std::vector<uint8_t>& data,
  ...
  cdm_file_->RenameTempFileAndReopen(base::BindOnce(
      &MojoCdmFileIO::OnTempFileRenamed, weak_factory_.GetWeakPtr()));
}

void MojoCdmFileIO::OnTempFileRenamed(base::File reopened_file) {
  DVLOG(3) << __func__ << " file: " << file_name_;
  DCHECK_EQ(State::kWriting, state_);
  DCHECK(!file_.IsValid()) << "Original file was not closed.";
