...
void MojoCdmFileIO::DoRead() {
  ...
  // Files are limited to 32MB, so fail if file too big. Not setting |state_|
  // to kError so that the CDM could write something valid to this file.
...
void MojoCdmFileIO::Write(const uint8_t* data, uint32_t data_size) {
  ...
  if (bytes_to_write > 0) {
    ...
    if (bytes_written != bytes_to_write) {
      // Failed to write what was requested, so fail. File may be messed up
      // if a partial write occurred.
      // TODO(jrummell): Add a UMA to track how often this happens.
      PostWriteResult(ClientStatus::kError);
      return;
    }
  }
  ...
  if (!file_.SetLength(bytes_to_write)) {
    // If this fails the file may be messed up (different file length than the
    // size of the valid contents).
    PostWriteResult(ClientStatus::kError);
    return;
  }

  PostWriteResult(ClientStatus::kSuccess);
}
