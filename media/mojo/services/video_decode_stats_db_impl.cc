...
void VideoDecodeStatsDBImpl::Initialize(
  ...
  // Customize simple opitons to use WebBlockCache. DB I/O is triggered by web
  // APIs. See docs for declaration of  GetSharedWebBlockCache().
  leveldb_env::Options db_options = leveldb_proto::CreateSimpleOptions();
  db_options.block_cache = leveldb_chrome::GetSharedWebBlockCache();

  db_->Init(kDatabaseClientName, dir, db_options,
...
void VideoDecodeStatsDBImpl::WriteUpdatedEntry(
  ...
  // TODO(chcunningham): DCHECK?
  if (!IsDatabaseReady("Update"))
    return;

  if (!stats_proto) {
    stats_proto.reset(new DecodeStatsProto());
    stats_proto->set_frames_decoded(0);
    stats_proto->set_frames_dropped(0);
  }

  uint64_t sum_frames_decoded =
      stats_proto->frames_decoded() + entry.frames_decoded;
  uint64_t sum_frames_dropped =
      stats_proto->frames_dropped() + entry.frames_dropped;

  stats_proto->set_frames_decoded(sum_frames_decoded);
  stats_proto->set_frames_dropped(sum_frames_dropped);
  ...
  std::unique_ptr<ProtoDecodeStatsEntry::KeyEntryVector> entries(
      new ProtoDecodeStatsEntry::KeyEntryVector());

  entries->emplace_back(SerializeKey(key), *stats_proto);
