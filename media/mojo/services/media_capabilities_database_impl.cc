...
void MediaCapabilitiesDatabaseImpl::AppendInfoToEntry(
    const Entry& entry,
    const Info& info,
  ...
  if (iter != cache_.end()) {
    ...
    ReadDatabaseAndUpdateWithInfo(entry, info, std::move(callback));
  }
}
...
void MediaCapabilitiesDatabaseImpl::OnDatabaseUpdated(
    AppendInfoToEntryCB callback,
    bool success) {
  // TODO(chcunningham): record UMA.
