...
void VideoDecodeStatsRecorder::SetPageInfo(const url::Origin& top_frame_origin,
                                           bool is_top_frame) {
  DVLOG(2) << __func__ << " top_frame_origin:" << top_frame_origin
...
void VideoDecodeStatsRecorder::FinalizeRecord() {
  ...
  perf_history_->SavePerfRecord(top_frame_origin_, is_top_frame_, profile_,
                                natural_size_, frames_per_sec_, frames_decoded_,
                                frames_dropped_,
