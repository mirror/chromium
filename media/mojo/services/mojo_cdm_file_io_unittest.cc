 ...
 protected:
  ...
  void SetUp() override {
    ...
    file_io_ = std::make_unique<MojoCdmFileIO>(
        client_.get(), base::BindOnce(&MojoCdmFileIOTest::OpenTemporaryFile,
                                      base::Unretained(this)));
  }

  // MojoCdmFileIO calls a callback to actually open the file. Simulate this
  // by creating a temporary file and returning it. Note that |file_name| is
  // ignored, but it doesn't matter as MojoCdmFileIO only works with a single
  // file anyway.
  void OpenTemporaryFile(const std::string& file_name,
    ...
    std::move(callback).Run(
        mojom::CdmStorage::Status::SUCCESS,
  ...
  // Use a temporary directory to hold the actual files. As a ScopedTempDir,
  // it will clean itself up on destruction.
