...
TEST_F(MojoCdmFileIOTest, TwoReads) {
  ...
  EXPECT_CALL(*client_.get(), OnReadComplete(Status::kSuccess, _, 0)).Times(2);
  file_io_->Read();
  file_io_->Read();
  base::RunLoop().RunUntilIdle();
}
...
TEST_F(MojoCdmFileIOTest, TwoWrites) {
  ...
  const std::vector<uint8_t> data = {1, 2, 3, 4, 5, 6, 7, 8};
  EXPECT_CALL(*client_.get(), OnWriteComplete(Status::kSuccess)).Times(2);
  file_io_->Write(data.data(), data.size());
  file_io_->Write(data.data(), data.size());
  base::RunLoop().RunUntilIdle();
}
...
TEST_F(MojoCdmFileIOTest, WriteShorterFile) {
  const std::string kFileName = "testfile";
  EXPECT_CALL(*client_.get(), OnOpenComplete(Status::kSuccess));
  file_io_->Open(kFileName.data(), kFileName.length());
  base::RunLoop().RunUntilIdle();
  ...
  // Write some data to the file.
  const std::vector<uint8_t> short_data = {1, 2, 3};
  const std::vector<uint8_t> long_data = {1, 2, 3, 4, 5, 6, 7, 255, 0, 'z'};

  {
    ::testing::InSequence s;
    EXPECT_CALL(*client_.get(), OnWriteComplete(Status::kSuccess));
    EXPECT_CALL(*client_.get(), OnReadComplete(Status::kSuccess, _, _))
        .WillOnce(CheckData(long_data));
    EXPECT_CALL(*client_.get(), OnWriteComplete(Status::kSuccess));
    EXPECT_CALL(*client_.get(), OnReadComplete(Status::kSuccess, _, _))
        .WillOnce(CheckData(short_data));
  }

  file_io_->Write(long_data.data(), long_data.size());
  base::RunLoop().RunUntilIdle();
  file_io_->Read();
  base::RunLoop().RunUntilIdle();
  file_io_->Write(short_data.data(), short_data.size());
  base::RunLoop().RunUntilIdle();
  file_io_->Read();
  base::RunLoop().RunUntilIdle();
}
