...
class MEDIA_MOJO_EXPORT MojoCdmProxyService : public mojom::CdmProxy {
 public:
  // Constructs a MojoCdmProxyService and strongly binds it to the |request|.
  MojoCdmProxyService(MojoCdmServiceContext* context);

  ~MojoCdmProxyService() final;

  // mojom::CdmProxy implementation.
  void Initialize(mojom::CdmProxyClientPtr client,
  ...
  void SetKeyInfo(const std::vector<media::CdmProxyKeyInfo>& key_infos) final;
 ...
 private:
  ...
  mojom::CdmProxyClientPtr client_;

  base::WeakPtr<MojoCdmProxyService> weak_this_;
