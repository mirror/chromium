...
TEST_F(VideoDecodePerfHistoryTest, GetPerfInfo_Smooth) {
  // NOTE: The owned DB is lazy-initialized, triggered by the first
  // SavePerfRecord() call below. All EXPECT_CALLs are then delayed until
  // we db_->CompleteInitialize() at the end of the test. testing::InSequence
  // enforces that EXPECT_CALLs arrive in top-to-bottom order.
  ...
  // Complete DB initialization (see comment at top of test).
...
TEST_F(VideoDecodePerfHistoryTest, GetPerfInfo_FailedInitialize) {
  const VideoCodecProfile kProfile = VP9PROFILE_PROFILE0;
  const gfx::Size kSize(100, 200);
  const int kFrameRate = 30;

  // DB is lazy-initialized by this first call to GetPerfInfo() below. When
  // initialization fails, callback should optimistically claim both smooth and
