...
class VideoDecodePerfHistoryTest : public ::testing::Test {
 public:
  VideoDecodePerfHistoryTest() = default;
  ~VideoDecodePerfHistoryTest() override {}

  void SetUp() override {
    // Sniff the database pointer so tests can inject entries.
    database_ = new FakeCapabilitiesDatabase();
    ...
    perf_history_ = new VideoDecodePerfHistory(base::WrapUnique(database_));
  }
 ...
 protected:
  ...
  // The fake database, owned by |perf_history_|.
  FakeCapabilitiesDatabase* database_;
};
