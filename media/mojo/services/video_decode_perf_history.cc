...
void VideoDecodePerfHistory::InitDB() {
...
void VideoDecodePerfHistory::OnDatabseInit(bool success) {
  ...
  while (!init_deferred_api_calls_.empty()) {
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, std::move(init_deferred_api_calls_.front()));
    init_deferred_api_calls_.pop();
  }
}
...
void VideoDecodePerfHistory::GetPerfInfo(VideoCodecProfile profile,
  ...
  if (db_init_status_ != COMPLETE) {
    init_deferred_api_calls_.push(base::BindOnce(
