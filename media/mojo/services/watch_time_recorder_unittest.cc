...
class WatchTimeRecorderTest : public testing::Test {
 public:
  ...
  ~WatchTimeRecorderTest() override { CycleRunLoop(); }
  ...
  void ExpectWatchTime(const std::vector<base::StringPiece>& keys,
    ...
    for (int i = 0; i <= static_cast<int>(WatchTimeKey::kLastWatchTimeKey);
         ++i) {
  ...
  void ExpectUkmWatchTime(const std::vector<base::StringPiece>& keys,
    ...
    const auto& source = *(test_recorder_->GetSources().begin()->second);
    for (auto key : keys) {
  ...
  void ResetMetricRecorders() {
    ...
    test_recorder_.reset(new ukm::TestUkmRecorder());
  }
...
TEST_F(WatchTimeRecorderTest, TestBasicReporting) {
  ...
  // Don't include kLastWatchTimeKey, since we'll use that as a placeholder to
  // ensure only the keys requested for finalize are finalized.
  const base::StringPiece kLastKey =
      WatchTimeKeyToString(WatchTimeKey::kLastWatchTimeKey);
  for (int i = 0; i < static_cast<int>(WatchTimeKey::kLastWatchTimeKey); ++i) {
    const WatchTimeKey key = static_cast<WatchTimeKey>(i);

    Initialize(true, false, true, true);
    wtr_->RecordWatchTime(WatchTimeKey::kLastWatchTimeKey, kWatchTime1);
