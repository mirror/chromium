...
TEST_F(WatchTimeRecorderTest, TestFinalizeNoDuplication) {
  ...
  for (const auto* entry : entries) {
    ...
    EXPECT_UKM(UkmEntry::kAudioDecoderNameHashName, 0);
    EXPECT_UKM(UkmEntry::kVideoDecoderNameHashName, 0);
...
TEST_F(WatchTimeRecorderTest, BasicUkmAudioVideoWithExtras) {
  ...
  for (const auto* entry : entries) {
    ...
    EXPECT_HAS_UKM(UkmEntry::kPlayerIDName);
    EXPECT_UKM(UkmEntry::kAudioDecoderNameHashName,
               base::PersistentHash(kAudioDecoderName));
    EXPECT_UKM(UkmEntry::kVideoDecoderNameHashName,
