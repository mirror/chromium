...
TEST_F(WatchTimeRecorderTest, FinalizeWithoutWatchTime) {
  ...
  // Finalize everything. Since we're not destroying the recorder, this should
  // not report UKM values. Those should only be reported at destruction if we
  // haven't reported UKM previously.
  wtr_->FinalizeWatchTime({});
  const auto& empty_entries =
      test_recorder_->GetEntriesByName(UkmEntry::kEntryName);
  EXPECT_EQ(0u, empty_entries.size());

  // Now that we're destructing we should get the UKM report.
  wtr_.reset();
  base::RunLoop().RunUntilIdle();
