...
class MEDIA_MOJO_EXPORT MojoCdmFileIO : public CdmFileIO {
 ...
 private:
  ...
  void OnTempFileOpenedForWriting(const std::vector<uint8_t>& data,
                                  base::File temporary_file);

  // Called after the temp file has been renamed to replace the original file.
  void OnTempFileRenamed(base::File reopened_file);
