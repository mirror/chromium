...
the core "media" target to support most out-of-process use cases. Currently the
“media” target does not depend on mojo, so that other applications can use the
“media” target without having to pull in mojo dependency.
...
* A **media renderer** decodes compressed audio and/or video buffers, handles
  the synchronization between the two, and renders them on the display and sound
  card.
* An **audio decoder** or **video decoder** decodes compressed audio or
  video buffers and returns the uncompressed audio or video frames.
* A **content decryption module** (CDM) decrypts (and sometimes decodes)
  encrypted and compressed audio and/or video buffers. A CDM is required for a
  *media renderer*, *audio decoder* or *video decoder* to handle protected
  content.
...
talks to a `media::mojom::RendererPtr`. `MojoRendererService` implements
`media::mojom::RendererPtr`, and can host any `media::Renderer` implementation.
...
`media::mojom::InterfaceFactory` which is used to request all media player mojo
interfaces within that RenderFrame. In the browser process, each
`RenderFrameHostImpl` owns a `MediaInterfaceProxy`, which implements
...
In Blink, both media element and EME MediaKeys belong to a `WebFrame`. In
...
`MojoCdmServiecContext` to get the `CdmContext` associated with the CDM ID, and
...
* NOT using `MediaService`. Instead, `MojoRendererService` is hosted by
  `RenderFrameHostImpl`/`MediaInterfaceProxy`  in the browser process directly.
...
> TODO(xhwang): Add documentation on other mojo services, e.g. capture,
remoting, etc.
