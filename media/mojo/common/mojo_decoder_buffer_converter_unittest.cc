...
TEST(MojoDecoderBufferConverterTest, ResetWithoutRead) {
  base::MessageLoop message_loop;
  base::RunLoop run_loop;

  base::MockCallback<base::OnceClosure> mock_reset_cb;
  EXPECT_CALL(mock_reset_cb, Run());

  MojoDecoderBufferConverter converter;
  converter.reader->Reset(mock_reset_cb.Get());

  run_loop.RunUntilIdle();
}

TEST(MojoDecoderBufferConverterTest, ResetAfterRead) {
  base::MessageLoop message_loop;
  base::RunLoop run_loop;
...
TEST(MojoDecoderBufferConverterTest, ResetBeforeRead) {
  base::MessageLoop message_loop;
  base::RunLoop run_loop;
...
TEST(MojoDecoderBufferConverterTest, ResetBeforeChunkedRead) {
  base::MessageLoop message_loop;
  base::RunLoop run_loop;
...
TEST(MojoDecoderBufferConverterTest, ResetDuringChunkedRead) {
  base::MessageLoop message_loop;
  base::RunLoop run_loop;
  ...
  // Reset callback should be fired after read callback.
  ::testing::InSequence sequence;
  EXPECT_CALL(mock_read_cb, Run(MatchesDecoderBuffer(media_buffer)));
  EXPECT_CALL(mock_reset_cb, Run())
      .WillOnce(testing::InvokeWithoutArgs(&run_loop, &base::RunLoop::Quit));
...
TEST(MojoDecoderBufferConverterTest, ResetDuringConcurrentReads) {
  base::MessageLoop message_loop;
  base::RunLoop run_loop;
  ...
  reader->Reset(mock_reset_cb.Get());
