...
void MojoDecoderBufferReader::CancelAllPendingReads() {
  DVLOG(4) << __func__;
  ...
  while (!pending_read_cbs_.empty()) {
    ...
    // Perhaps these should be posted, or merged into a single error callback?
...
void MojoDecoderBufferReader::CompleteEmptyReads() {
  DVLOG(4) << __func__;
...
void MojoDecoderBufferReader::OnPipeReadable(
  ...
  // TODO(sandersd): Check for never readable?
  if (result != MOJO_RESULT_OK) {
    OnPipeError(result);
    return;
  }
...
void MojoDecoderBufferWriter::OnPipeWritable(
  ...
  // TODO(sandersd): Check for never writable?
  if (result != MOJO_RESULT_OK) {
    OnPipeError(result);
    return;
  }
