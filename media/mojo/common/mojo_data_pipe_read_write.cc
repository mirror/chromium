...
void MojoDataPipeReader::Read(uint32_t num_bytes,
                              uint8_t* buffer,
                              DoneCB done_cb) {
  DVLOG(3) << __func__;
  ...
  if (!done_cb_.is_null() || current_buffer_ || current_buffer_size_) {
    VLOG(1) << __func__ << ": Called when previous reading is not completed.";
...
void MojoDataPipeReader::OnPipeReadable(MojoResult result,
  ...
  DCHECK_GT(current_buffer_size_, 0u);

  uint32_t num_bytes = current_buffer_size_ - bytes_read_;
  DCHECK_GT(num_bytes, 0u);

  result = consumer_handle_->ReadData(current_buffer_ + bytes_read_, &num_bytes,
...
void MojoDataPipeWriter::Write(uint8_t* buffer,
  ...
  DCHECK(!done_cb.is_null());
  if (!done_cb_.is_null() || current_buffer_ || current_buffer_size_) {
    VLOG(1) << __func__ << ": Called when previous writing is not completed.";
    std::move(done_cb).Run(false);
    return;
  }
