...
class VideoEncodeAcceleratorClient
 ...
 public:
  ...
  void NotifyError(mojom::VideoEncodeAccelerator::Error error) override;
...
void VideoEncodeAcceleratorClient::NotifyError(
    mojom::VideoEncodeAccelerator::Error mojo_error) {
  DVLOG(1) << __func__;
  ...
  switch (mojo_error) {
    case mojom::VideoEncodeAccelerator::Error::ILLEGAL_STATE_ERROR:
      error = ::media::VideoEncodeAccelerator::kIllegalStateError;
      break;
    case mojom::VideoEncodeAccelerator::Error::INVALID_ARGUMENT_ERROR:
      error = ::media::VideoEncodeAccelerator::kInvalidArgumentError;
      break;
    case mojom::VideoEncodeAccelerator::Error::PLATFORM_FAILURE_ERROR:
...
MojoVideoEncodeAcceleratorHost::MojoVideoEncodeAcceleratorHost(
    ...
    : vea_(std::move(vea)), gpu_info_(gpu_info) {
  DVLOG(1) << __func__;
  DCHECK(vea_);
}
...
bool MojoVideoEncodeAcceleratorHost::Initialize(
  ...
  vea_->Initialize(input_format, input_visible_size, output_profile,
                   initial_bitrate, std::move(vea_client));
