...
void MojoDecryptor::Decrypt(StreamType stream_type,
  ...
  remote_decryptor_->Decrypt(
      ...
      base::BindOnce(
          ...
          ScopedCallbackRunner(static_cast<DecryptOnceCB>(decrypt_cb), kError,
                               nullptr)));
}
...
void MojoDecryptor::InitializeAudioDecoder(const AudioDecoderConfig& config,
  ...
  remote_decryptor_->InitializeAudioDecoder(
      config,
      ScopedCallbackRunner(static_cast<DecoderInitOnceCB>(init_cb), false));
}
...
void MojoDecryptor::DecryptAndDecodeAudio(
  ...
  remote_decryptor_->DecryptAndDecodeAudio(
      ...
      base::BindOnce(
          ...
          ScopedCallbackRunner(static_cast<AudioDecodeOnceCB>(audio_decode_cb),
                               kError, AudioFrames())));
}
...
void MojoDecryptor::DecryptAndDecodeVideo(
  ...
  remote_decryptor_->DecryptAndDecodeVideo(
      std::move(mojo_buffer),
      base::BindOnce(
          &MojoDecryptor::OnVideoDecoded, weak_factory_.GetWeakPtr(),
          ScopedCallbackRunner(static_cast<VideoDecodeOnceCB>(video_decode_cb),
                               kError, nullptr)));
}
