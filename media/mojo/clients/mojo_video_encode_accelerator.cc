...
void MojoVideoEncodeAccelerator::Encode(const scoped_refptr<VideoFrame>& frame,
  ...
  //
  // TODO(crbug.com/793446): This should be changed to wrap the frame buffer
  // handle as read-only, but VideoFrame does not seem to guarantee that its
  // shared_memory_handle() is in fact read-only.
...
void MojoVideoEncodeAccelerator::UseOutputBitstreamBuffer(
  ...
  // TODO(crbug.com/793446): Only wrap read-only handles here and change the
  // protection status to kReadOnly.
