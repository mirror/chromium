...
// Provides a thread-safe channel for VideoFrame destruction events. This class
// will be destructed when both its owner and all outstanding VideoFrames have
// been destructed.
class MojoVideoFrameReleaser
    : public base::RefCountedThreadSafe<MojoVideoFrameReleaser> {
 public:
  REQUIRE_ADOPTION_FOR_REFCOUNTED_TYPE();

  explicit MojoVideoFrameReleaser(
      mojom::VideoFrameReleaserAssociatedPtrInfo video_frame_releaser_ptr_info);

  VideoFrame::ReleaseMailboxCB CreateReleaseMailboxCB(
      const base::UnguessableToken& release_token);

 private:
  friend class base::RefCountedThreadSafe<MojoVideoFrameReleaser>;
  ~MojoVideoFrameReleaser();

  void OnVideoFrameReleased(const base::UnguessableToken& release_token,
                            const gpu::SyncToken& release_sync_token);

  mojom::VideoFrameReleaserAssociatedPtr video_frame_releaser_;
};
...
class MojoVideoDecoder final : public VideoDecoder,
 ...
 private:
  ...
  scoped_refptr<MojoVideoFrameReleaser> mojo_video_frame_releaser_;
