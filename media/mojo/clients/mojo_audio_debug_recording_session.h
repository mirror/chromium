...
class MojoDebugRecordingFileProvider;

class MEDIA_MOJO_EXPORT DebugRecordingSession {
 public:
  explicit DebugRecordingSession(const base::FilePath& file_name_base);
  ~DebugRecordingSession();

 private:
  std::unique_ptr<MojoDebugRecordingFileProvider> file_provider_impl_;
  mojom::DebugRecordingPtr debug_recorder_;
