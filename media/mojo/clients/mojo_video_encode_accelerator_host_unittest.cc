...
// Mock implementation of the client of MojoVideoEncodeAcceleratorHost.
...
// Test wrapper for a MojoVideoEncodeAcceleratorHost, which translates between a
// pipe to a remote mojom::MojoVideoEncodeAccelerator, and a local
// media::VideoEncodeAccelerator::Client.
class MojoVideoEncodeAcceleratorHostTest : public ::testing::Test {
 public:
  MojoVideoEncodeAcceleratorHostTest() = default;
  ...
  void SetUp() override {
    ...
    mojo_vea_host_.reset(new MojoVideoEncodeAcceleratorHost(std::move(mojo_vea),
                                                            gpu::GPUInfo()));
  }
  ...
  VideoEncodeAccelerator* mojo_vea_host() { return mojo_vea_host_.get(); }
  ...
  void Initialize(const gfx::Size& visible_size,
    ...
    DVLOG(1) << std::hex << mock_vea_client;
    ...
    EXPECT_TRUE(mojo_vea_host()->Initialize(kInputFormat, visible_size,
 ...
 private:
  ...
  std::unique_ptr<VideoEncodeAccelerator> mojo_vea_host_;

  DISALLOW_COPY_AND_ASSIGN(MojoVideoEncodeAcceleratorHostTest);
};

TEST_F(MojoVideoEncodeAcceleratorHostTest, CreateAndDestroy) {}

// This test verifies the Initialize() communication prologue in isolation.
TEST_F(MojoVideoEncodeAcceleratorHostTest,
...
TEST_F(MojoVideoEncodeAcceleratorHostTest, EncodeOneFrame) {
  const gfx::Size kInputVisibleSize(64, 48);
  std::unique_ptr<MockVideoEncodeAcceleratorClient> mock_vea_client =
      base::MakeUnique<MockVideoEncodeAcceleratorClient>();
  Initialize(kInputVisibleSize, mock_vea_client.get());
  ...
  const int32_t kBistreamBufferId = 17;
  {
    ...
    mojo_vea_host()->UseOutputBitstreamBuffer(
...

  {
    ...
    mojo_vea_host()->Encode(video_frame, is_keyframe);
    base::RunLoop().RunUntilIdle();
  }
}

// Tests that a RequestEncodingParametersChange() ripples through correctly.
TEST_F(MojoVideoEncodeAcceleratorHostTest, EncodingParametersChange) {
  ...
  mojo_vea_host()->RequestEncodingParametersChange(kNewBitrate, kNewFramerate);
  base::RunLoop().RunUntilIdle();
}
