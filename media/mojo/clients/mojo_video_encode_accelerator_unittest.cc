...
// Initialize() call, responds to |client| with a RequireBitstreamBuffers(), and
// upon Encode(), responds with a BitstreamBufferReady() with the bitstream
// buffer id previously configured by a call to UseOutputBitstreamBuffer(). This
// mock class only allows for one bitstream buffer in flight.
...
class MojoVideoEncodeAcceleratorTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    mojo_vea_.reset(
        new MojoVideoEncodeAccelerator(std::move(mojo_vea), gpu::GPUInfo()));
  }
  ...
  void Initialize(const gfx::Size& visible_size,
                  MockVideoEncodeAcceleratorClient* mock_vea_client) {
    ...
    EXPECT_CALL(*mock_mojo_vea(),
                DoInitialize(kInputFormat, visible_size, kOutputProfile,
                             kInitialBitrate, _));
    EXPECT_CALL(*mock_vea_client,
                RequireBitstreamBuffers(
                    _, visible_size,
                    VideoFrame::AllocationSize(kInputFormat, visible_size)));

    EXPECT_TRUE(mojo_vea()->Initialize(kInputFormat, visible_size,
...
TEST_F(MojoVideoEncodeAcceleratorTest, EncodeOneFrame) {
  ...
  const int32_t kBistreamBufferId = 17;
  {
    ...
    EXPECT_CALL(*mock_mojo_vea(),
                DoUseOutputBitstreamBuffer(kBistreamBufferId, _));
...

  {
    ...
    EXPECT_CALL(*mock_vea_client,
                BitstreamBufferReady(kBistreamBufferId, _, is_keyframe,
