...
bool IsMicrosoftSynthesizer(IDeviceInformation* info) {
  Microsoft::WRL::ComPtr<IMidiSynthesizerStatics> midi_synthesizer_statics;
  HRESULT hr =
...
struct MidiPort {
  ...
  uint32_t index;
  Microsoft::WRL::ComPtr<InterfaceType> handle;
...
class MidiManagerWinrt::MidiPortManager {
 public:
  ...
  bool StartWatcher() {
    ...
    Microsoft::WRL::ComPtr<IDeviceInformationStatics> dev_info_statics;
 ...
 private:
  ...
  Microsoft::WRL::ComPtr<StaticsInterfaceType> midi_port_statics_;

  // DeviceWatcher instance and event registration tokens for unsubscribing
  // events in destructor.
  Microsoft::WRL::ComPtr<IDeviceWatcher> watcher_;
    ...
    HRESULT hr = handle->add_MessageReceived(
        WRL::Callback<
            ...
            [weak_ptr, task_runner](IMidiInPort* handle,
              ...
              Microsoft::WRL::ComPtr<IMidiMessage> message;
