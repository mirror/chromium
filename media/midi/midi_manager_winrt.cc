...
ScopedComPtr<InterfaceType> WrlStaticsFactory() {
  ...
  HRESULT hr = base::win::com_base_util::RoGetActivationFactory(
...
std::string GetIdString(T* obj) {
  ...
  ScopedHString string = ScopedHString::Create(result);
...
class MidiManagerWinrt::MidiPortManager {
 ...
 private:
  ...
  void OnAdded(std::string dev_id, std::string dev_name) {
    ...
    ScopedHString dev_id_hstring =
        ScopedHString::Create(base::UTF8ToWide(dev_id).c_str());
