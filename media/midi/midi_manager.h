...
class MIDI_EXPORT MidiManager {
 public:
  ...
  // Called before the destruction to handle platform dependent finalization
  // that are implemented in sub-classes and rely on virtual methods.
  void Shutdown();
  ...
  // CompleteStartSession() will be called with mojom::Result::OK if the session
  // is started. Otherwise CompleteStartSession() is called with a proper
  // mojom::Result code.
 ...
 protected:
  ...
  // destructor will be called immediately, and the I/O thread runs all posted
  // tasks to stop soon.
  ...
  // Following methods for inheritances can be called on any thread.
