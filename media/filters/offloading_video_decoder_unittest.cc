...
TEST_F(OffloadingVideoDecoderTest, BasicFunctionality) {
  ...
  EXPECT_EQ(offloading_decoder_->GetDisplayName(), decoder_->GetDisplayName());
  ...
  offloading_decoder_->Initialize(TestVideoConfig::Normal(kCodecVP9), false,
                                  nullptr, ExpectInitCB(true),
                                  ExpectOutputCB());
  ...
  EXPECT_CALL(*decoder_, Detach())
      .WillOnce(VerifyOn(task_env_.GetMainThreadTaskRunner()));
  offloading_decoder_->Initialize(offload_config, false, nullptr,
