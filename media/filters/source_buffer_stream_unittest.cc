...
// BIG TODO: continue here (probably need something like
// EXPECT_STREAM_OP_STATUS_EQ)?? (need to use Pts vs Dts statuses...)

// BIG TODO: separate CL to elevate various ops to fixture methods? Or do here?
...
// we use in the test instance's SourceBufferStream<>.
 ...
 protected:
  ...
  void CheckExpectedBuffers(int starting_position,
    ...
    for (; current_position <= ending_position; current_position++) {
      ...
      StreamType::Status status = STREAM_OP(GetNextBuffer(&buffer));
  ...
  void CheckExpectedBuffers(
    ...
    std::stringstream ss;
    const StreamType::Type type = STREAM_OP(GetType());
    for (size_t i = 0; i < timestamps.size(); i++) {
      ...
      if (base::EndsWith(timestamps[i], "P", base::CompareCase::SENSITIVE)) {
        ...
        // duration.
        ASSERT_EQ(StreamType::kSuccess, STREAM_OP(GetNextBuffer(&buffer)));
  ...
  void CheckNoNextBuffer() {
    scoped_refptr<StreamParserBuffer> buffer;
    EXPECT_EQ(StreamType::kNeedBuffer, STREAM_OP(GetNextBuffer(&buffer)));
  }

  void CheckEOSReached() {
    scoped_refptr<StreamParserBuffer> buffer;
    EXPECT_EQ(StreamType::kEndOfStream, STREAM_OP(GetNextBuffer(&buffer)));
  }
...
TEST_P(SourceBufferStreamTest, Seek_StartOfGroup) {
  ...
  EXPECT_EQ(STREAM_OP(GetNextBuffer(&buffer)), StreamType::kSuccess);
...
TEST_P(SourceBufferStreamTest, PresentationTimestampIndependence) {
  ...
  for (int i = 0; i < 20; i++) {
    scoped_refptr<StreamParserBuffer> buffer;
    ASSERT_EQ(STREAM_OP(GetNextBuffer(&buffer)), StreamType::kSuccess);
...
TEST_P(SourceBufferStreamTest, ConfigChange_Basic) {
  ...
  // has happened.
  EXPECT_EQ(STREAM_OP(GetNextBuffer(&buffer)), StreamType::kConfigChange);
...
TEST_P(SourceBufferStreamTest, Audio_SpliceFrame_NoSplice) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    EXPECT_EQ(StreamType::kSuccess, STREAM_OP(GetNextBuffer(&buffer)));
...
TEST_P(SourceBufferStreamTest, Audio_SpliceTrimming_ExistingTrimming) {
  ...
  EXPECT_EQ(StreamType::kSuccess, STREAM_OP(GetNextBuffer(&read_buffer)));
...
TEST_P(SourceBufferStreamTest, Audio_ConfigChangeWithPreroll) {
  ...
  // has happened.
  scoped_refptr<StreamParserBuffer> buffer;
  EXPECT_EQ(StreamType::kConfigChange, STREAM_OP(GetNextBuffer(&buffer)));
