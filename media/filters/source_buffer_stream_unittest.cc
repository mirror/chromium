...
TEST_P(SourceBufferStreamTest, RangeCoalescenceOnFudgeRoomIncrease) {
  ...
  // Increase the fudge room again while appending a coded frame group that
  // would starts in the original first buffered range, but with a first buffer
  // that would have been in the original second buffered range.
  NewCodedFrameGroupAppend(base::TimeDelta::FromMilliseconds(10), "90D5K");
  CheckExpectedRangesByTimestamp(
      "{ [0,120) [500,520) [1000,1044) [2000,2045) }");

  SeekToTimestampMs(0);
  CheckExpectedBuffers("0K 90K 100K 110K");
  CheckNoNextBuffer();
