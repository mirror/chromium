...
TEST_P(SourceBufferStreamTest, ZeroDurationBuffersThenIncreasingFudgeRoom) {
  ...
  // Grow the fudge room enough such that the first range's end time would be
  // beyond both the second range's end time and the third range's start time if
  // fudge room (or half of it, which is the maximum interbuffer DTS distance or
  // duration seen so far) was incorrectly used in the calculating buffered
  // range end time for ranges ending with 0-duration buffers. See
  // https://crbug.com/803018 for example.
