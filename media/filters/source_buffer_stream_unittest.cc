...
class SourceBufferStreamTest : public testing::Test {
 protected:
  ...
  // If |timestamps_as_milliseconds| is false, compares the timestamps as
  // microseconds.
  void CheckExpectedRangesByTimestamp(const std::string& expected,
                                      bool timestamps_as_milliseconds = true) {
    Ranges<base::TimeDelta> r = stream_->GetBufferedTime();
  ...
  // If |timestamps_as_milliseconds| is false, compares timestamps as
  // microseconds.
...
TEST_F(SourceBufferStreamTest,
       OverlappingAppendOneMicrosecond_RangeMembership_1) {
  Seek(0);

  // Append a buffer at time 23ms, with estimated duration.
  NewCodedFrameGroupAppend("23D63EK");
  CheckExpectedRangesByTimestamp("{ [23000,86000) }", false);

  // Append a buffer 1 microsecond earlier, with estimated duration.
  NewCodedFrameGroupAppend("22999uD63EK");
  CheckExpectedRangesByTimestamp("{ [22999,86000) }", false);

  // Append that same buffer again, but without any discontinuity signalled / no
  // new coded frame group.
  // BIG TODO The following line hit the CHECK in the bug.
  AppendBuffers("22999uD63EK");
  CheckExpectedRangesByTimestamp("{ [22999,86000) }", false);

  Seek(0);
  CheckExpectedBuffers("22999K 22999K 23000K", false);
  CheckNoNextBuffer();
}

TEST_F(SourceBufferStreamTest,
       OverlappingAppendOneMicrosecond_RangeMembership_2) {
  Seek(0);

  // Append a buffer at time 23ms.
  NewCodedFrameGroupAppend("23D63K");
  CheckExpectedRangesByTimestamp("{ [23000,86000) }", false);

  // Append a buffer 1 microsecond earlier, with 1 microsecond duration.
  NewCodedFrameGroupAppend("22999uD1uK");
  CheckExpectedRangesByTimestamp("{ [22999,86000) }", false);

  // Append that same buffer again, but without any discontinuity signalled / no
  // new coded frame group.
  // BIG TODO The following line hit the CHECK in the bug.
  AppendBuffers("22999uD1uK");
  CheckExpectedRangesByTimestamp("{ [22999,86000) }", false);
...
TEST_F(SourceBufferStreamTest,
       OverlappingAppendOneMicrosecond_RangeMembership_3) {
  Seek(0);
  ...
  // Append a buffer 1 microsecond earlier, with almost fully overlapping
  // duration (short by 1 microsecond).
  NewCodedFrameGroupAppend("22999uD63K");
  CheckExpectedRangesByTimestamp("{ [22999,85999) }", false);

  // Append that same buffer again, but without any discontinuity signalled / no
  // new coded frame group.
  // BIG TODO The following line did *not* hit the CHECK in the bug. This test
  // passes before attempting any fix.
  AppendBuffers("22999uD63K");
  CheckExpectedRangesByTimestamp("{ [22999,85999) }", false);

  Seek(0);
  CheckExpectedBuffers("22999K 22999K", false);
  CheckNoNextBuffer();
}
