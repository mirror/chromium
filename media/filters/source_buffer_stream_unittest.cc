...
// BIG TODO: This test passes before the fix because the
// final append overlaps the most recently appended buffer which
// started more than 1 microsecond before the next buffer in its range.
TEST_F(SourceBufferStreamTest,
  ...
  NewCodedFrameGroupAppend("22998uD2uK");
  CheckExpectedRangesByTimestamp("{ [22998,86000) }", false);

  // Append that same buffer again, but without any discontinuity signalled / no
  // new coded frame group.
  AppendBuffers("22998uD2uK");
  CheckExpectedRangesByTimestamp("{ [22998,86000) }", false);

  Seek(0);
  CheckExpectedBuffers("22998K 22998K 23000K", false);
  CheckNoNextBuffer();
}
