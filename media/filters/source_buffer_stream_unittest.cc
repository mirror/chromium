  ...
  void AppendBuffers(const std::string& buffers_to_append,
    ...
    if (start_new_coded_frame_group) {
      ...
      if (start_timestamp == kNoDecodeTimestamp()) {
        start_timestamp = buffering_api_ == BufferingApi::kLegacyByDts
                              ? buffers[0]->GetDecodeTimestamp()
                              : DecodeTimestamp::FromPresentationTime(
                                    buffers[0]->timestamp());
      }

      ASSERT_TRUE(buffering_api_ == BufferingApi::kLegacyByDts
...
TEST_P(SourceBufferStreamTest, End_Overlap_Selected_NoKeyframeAfterNew2) {
  ...
  // If ByPts, the buffer at position 16 in track buffer is adjacent to the next
...
TEST_P(SourceBufferStreamTest, GarbageCollection_TrackBuffer) {
  ...
  // If ByPts, the buffer at position 16 in track buffer was adjacent to the
  // next keyframe, so no warning should be emitted on that track buffer
  // exhaustion.
  if (buffering_api_ == BufferingApi::kLegacyByDts)
