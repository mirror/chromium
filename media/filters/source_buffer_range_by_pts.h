...
class MEDIA_EXPORT SourceBufferRangeByPts : public SourceBufferRange {
 public:
  ...
  // range (within the fudge room) (if nonkeyframe, in DTS order; if keyframe,
  // in PTS order).
  // If |new_buffers_group_start_pts| is set otherwise, then that time
  // must come directly after the last buffer in this range (within the fudge
  // room), and the first buffer in |buffers| must be a keyframe.
  ...
  // fudge room, yet its group start was.  During append, |highest_frame_| is
  ...
  bool CanAppendBuffersToEnd(
      ...
      DecodeTimestamp new_buffers_group_start_pts) const; // BIG TODO continue fix here
