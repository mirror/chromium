...
class MEDIA_EXPORT SourceBufferRange {
 public:
  ...
  // During append, range end time tracking is updated, if necessary.
  ...
  // When a buffer in this range has had its duration reduced as part of audio
  // splice frame trimming, this method must be called to maintain the
  // correctness of the cached calculated range end time maintained internally.
  // This is the only known case where public requests to update the end time
  // are expected.
  void UpdateEndTimeDueToSpliceTrimming();
  ...
  // Range end time tracking is updated, if necessary.
 ...
 private:
  friend class SourceBufferStreamTest;  // For IsNextInPTS testing.

  typedef std::map<DecodeTimestamp, int> KeyframeMap;

  // Resets |range_highest_frame_time_| and |range_end_time_|.
  void ResetEndTime();
  ...
  // Updates |range_highest_frame_time_| and |range_end_time_| if |timestamp|
  // and |duration| indicate a coded frame with a higher time or end time.
  void UpdateEndTime(base::TimeDelta timestamp, base::TimeDelta duration);

  // Updates |range_highest_frame_time_| and |range_end_time_| based on the end
  // of the presentation interval of the last GOP in this range. Normally,
  // incremental additions to this range should just use UpdateEndTime(). When
  // removing buffers from this range (which could be out of order presentation
  // vs decode order), inspecting the last buffer in decode order of this range
  // can be insufficient to determine the correct presentation end time of this
  // range. Hence this helper method. If there are no buffers in this range,
  // resets the end times to kNoTimestamp.
  void UpdateEndTimeUsingLastGOP();

  // Returns true if |timestamp| is allowed in this range as the timestamp of
  // the next buffer in presentation sequence at or after
  // |range_highest_frame_time_|.  |buffers_| must not be empty, and
  // |range_highest_frame_time_| must not be kNoTimestamp.
  ...
  // These cache the highest coded frame PTS and PTS+duration currently in
  // |buffers_| for use in determining range adjacency and overlap conditions.
  // These are kNoTimestamp if this range is empty.
  // TODO(wolenetz): Switch to using these in CanAppendBuffersToEnd(), etc.,
  // when switching to managing ranges by their presentation interval between
  // GOPs, and by their decode sequence within GOPs. See
  // https://crbug.com/718641.
  base::TimeDelta range_highest_frame_time_;
  base::TimeDelta range_end_time_;
