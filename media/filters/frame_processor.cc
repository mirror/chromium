...
bool FrameProcessor::ProcessFrame(
  ...
  while (true) {
    ...
    //
    // Otherwise, if the buffer's DTS or PTS indicate that a new coded frame
    // group needs signalling, signal just the buffer's track buffer. This can
    // happen in both sequence and segments append modes when the first
    // processed track's frame following a discontinuity has a higher DTS (or
    // PTS) than this later processed track's first frame following that
    // discontinuity.
    if (pending_notify_all_group_start_ ||
      ...
      if (pending_notify_all_group_start_) {
        ...
        // Don't signal later times than previously signalled.
        DecodeTimestamp updated_dts = std::min(
            track_buffer->last_processed_decode_timestamp(), decode_timestamp);
        base::TimeDelta updated_pts = std::min(
            track_buffer->pending_group_start_pts(), presentation_timestamp);
        DCHECK(updated_pts != kNoTimestamp);
