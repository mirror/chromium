...
bool FrameProcessor::ProcessFrame(
  ...
  while (true) {
    ...
    if (range_api_ == ChunkDemuxerStream::RangeApi::kNewByPts &&
      ...
      // decreasing PTS versus the previous keyframe for that track in the
      // current coded frame group, signal a new coded frame group for that
      // track buffer so that it can correctly process overlap-removals of the
      // new GOP.
      if (track_buffer->last_keyframe_presentation_timestamp() !=
              kNoTimestamp &&
          track_buffer->last_keyframe_presentation_timestamp() >
              presentation_timestamp) {
        signal_new_cfg = true;
        // In case there is currently a decreasing keyframe PTS sequence later
        // followed by a removal of media buffered before now and a jump forward
        // into that media's time frame, ensure that jump is detected.
        // BIG TODO add test case...
        track_buffer->ResetHighestPresentationTimestamp();
      }

      // BIG TODO morph the above clause to be for keyframe PTS < track buffer
      // highest PTS (not just last track buffer keyframe PTS).
      // BIG TODO add test case... (this should fix twitch issue)
