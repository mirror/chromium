...
class MseTrackBuffer {
 ...
 private:
  ...
  // the stream (should increase and) update this. This is managed separately
  // from |last_decode_timestamp_| because |last_processed_decode_timestamp_| is
  // not reset during Reset(), to especially be able to track the need to signal
  // coded frame group start time for muxed sequence mode cases.
