...
bool FrameProcessor::ProcessFrames(
  ...
  if (sequence_mode_ && track_buffers_.size() > 1) {
    LIMITED_MEDIA_LOG(DEBUG, media_log_, num_muxed_sequence_mode_warnings_,
        ...
        << "Detected attempt to use MSE 'sequence' AppendMode with a "
           "multitrack bytestream. Even with spec-compliant processing, this "
           "can result in loss of track synchronization. In some cases, "
           "buffered range gaps and playback stalls can occur. It is "
           "recommended to instead use 'segments' mode for multitrack "
           "bytestreams.";
  }
