...
DecoderStream<StreamType>::~DecoderStream() {
  ...
  if (!init_cb_.is_null()) {
    task_runner_->PostTask(FROM_HERE,
                           base::BindOnce(base::ResetAndReturn(&init_cb_), false));
  }
  if (!read_cb_.is_null()) {
...
void DecoderStream<StreamType>::Read(const ReadCB& read_cb) {
  ...
  if (state_ == STATE_ERROR) {
    task_runner_->PostTask(
        FROM_HERE, base::BindOnce(read_cb, DECODE_ERROR, scoped_refptr<Output>()));
    return;
  }

  if (state_ == STATE_END_OF_STREAM && ready_outputs_.empty()) {
    task_runner_->PostTask(
        FROM_HERE, base::BindOnce(read_cb, OK, StreamTraits::CreateEOSOutput()));
    return;
  }
...
void DecoderStream<StreamType>::Reset(const base::Closure& closure) {
  ...
  if (!read_cb_.is_null()) {
    task_runner_->PostTask(FROM_HERE,
                           base::BindOnce(base::ResetAndReturn(&read_cb_), ABORTED,
