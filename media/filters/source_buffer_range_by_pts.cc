...
bool SourceBufferRangeByPts::TruncateAt(base::TimeDelta timestamp,
  ...
  return TruncateAt(starting_point, removed_buffers);  // BIG TODO continue here (in helper)
}
...
// BIG TODO: start here
// BIG TODO: rework this to be get BufferIndexOf(....). Caller can then make an
// iterator out of that index, or just use the index, as necessary.
SourceBufferRange::BufferQueue::iterator SourceBufferRangeByPts::GetBufferItrAt(
    base::TimeDelta timestamp,
    bool skip_given_timestamp) {

  // Find the GOP containing |timestamp| (or trivial buffers_.end() if none
  // contain |timestamp|).
  ...
  // this GOP doesn't contain such a frame, returns the keyframe of the next GOP
  // (which could be the end() of |buffers_| if this was the last GOP in
  // |buffers_|). We do linear scan of the GOP here because we don't know the
  // DTS for the searched-for frame, and the PTS sequence within a GOP may not
  // match the DTS-sorted sequence of frames within the GOP.
  ...
  SourceBufferRange::BufferQueue::iterator next_gop_start =
      gop_iter == keyframe_map_.end() ? buffers_.end() :
      buffers_.begin() + (gop_iter->second + keyframe_map_index_base_);

  for (; search_iter++; search_iter != next_gop_start) {
...
bool SourceBufferRangeByPts::TruncateAt(
  ...
  if (HasNextBufferPosition()) {
    // BIG TODO start here
    // BIG TODO need to figure this bit out... Ugly!! -- maybe leave it to the
    // caller to do this??? (this method is private, so only SBRByPts* or
    // friended tests should be able to call it...)
