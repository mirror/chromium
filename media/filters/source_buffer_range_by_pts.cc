...
void SourceBufferRangeByPts::AppendBuffersToEnd(
  ...
  // BIG TODO non-sap-type-2 support now...
  // TODO(wolenetz): In addition to SAP-2, muxed streams in general may need
  // special handling in callers to make sure this passes. For example, parsers
  // emit CFG in decode sequence across tracks in muxed stream, but first buffer
  // in track that starts with later DTS may have PTS < the group start PTS (so
  // FP needs to re-signal new CFG start like muxed sequence mode had to do
  // previously in the old dts impl.) See https://crbug.com/718641.
