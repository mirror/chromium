...
bool SourceBufferRangeByPts::TruncateAt(base::TimeDelta timestamp,
  ...
  // truncate from there.
  return TruncateAt(GetBufferIndexAt(timestamp, is_exclusive), deleted_buffers);  // BIG TODO continue here (in helper) start here
}
...
size_t SourceBufferRangeByPts::GetBufferIndexAt(base::TimeDelta timestamp, bool skip_given_timestamp) {
  ...
  SourceBufferRange::BufferQueue::iterator search_iter = buffers_.begin() + search_index;
  gop_iter++;
  ...
  for (; search_index++, search_iter++; search_iter != next_gop_start) {
