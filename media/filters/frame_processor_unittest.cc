...
TEST_P(FrameProcessorTest, AudioVideo_Discontinuity_TimestampOffset) {
  ...
  if (using_sequence_mode) {
    ...
    // after switching to 'sequence' mode. Hence 65, not 55 here. Similarly,
    // 'segments' mode muxed tracks where discontinuity is followed by tracks
    // whose first frames decrease in DTS relative to each other (allowed since
    // media segments are not required to contain frames for every track) could
    // result in decreasing range start times for those later tracks. See
...
TEST_P(FrameProcessorTest, AudioVideo_OutOfSequence_After_Discontinuity) {
  ...
  // shouldn't notify all tracks that a new coded frame group is starting
  // (unless we decide to deviate from spec and drop non-keyframes again from
  // all tracks, treating this as a *full* second discontinuity). Rather, we
  // should notify just the track encountering this new type of discontinuity.
  // Since MSE doesn't require all media segments to contain media from every
  // track, these append sequences can occur.
  InSequence s;
  AddTestTracks(HAS_AUDIO | HAS_VIDEO);
  bool using_sequence_mode = GetParam();
  frame_processor_->SetSequenceMode(using_sequence_mode);
  ...
  if (using_sequence_mode) {
    // Allow room in the timeline for the last append in this test to not be
    // AppendWindowStart-clipped.
    SetTimestampOffset(frame_duration_ * 10);
  }
  EXPECT_CALL(callbacks_, PossibleDurationIncrease(frame_duration_ * 14));
  ...
  // detected while appending the video frame, above, a second discontinuity is
  // not triggered.
  if (using_sequence_mode) {
