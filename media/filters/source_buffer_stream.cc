...
void SourceBufferStream::TrimSpliceOverlap(const BufferQueue& new_buffers) {
  ...
  // Notify the range that it may need to update its end time, since the
  // trimmed buffer may have been the one at the end of the range.
  (*range_itr)->UpdateEndTimeDueToSpliceTrimming();
