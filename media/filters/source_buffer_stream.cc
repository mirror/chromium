...
bool SourceBufferStream<RangeClass>::Append(const BufferQueue& buffers) {
  ...
  if (range_for_next_append_ != ranges_.end()) {
    ...
    last_appended_buffer_timestamp_ =
        buffers_for_new_range->back()->GetDecodeTimestamp();  // BIG TODO...
  ...
  if (!track_buffer_.empty()) {
    ...
    // BIG TODO find all other GetDecodeTimestamps that need to vary and fix
    // 'em!!!
...
void SourceBufferStream<RangeClass>::UpdateLastAppendStateForRemove(
  ...
  if (range_for_next_append_ != ranges_.end()) {
    if (last_appended_buffer_timestamp_ != kNoDecodeTimestamp()) {
      // BIG TODO make sure we're storing a PTS (when SBRByPTS) into
      // last_appended_buffer_timestamp_...
...
bool SourceBufferStream<RangeClass>::IsDtsMonotonicallyIncreasing(
  ...
  DCHECK(!buffers.empty());
  DecodeTimestamp prev_timestamp =
      last_appended_buffer_timestamp_;  // BIG TODO rename to decode?
...
size_t SourceBufferStream<RangeClass>::FreeBuffers(size_t total_bytes_to_free,
  ...
  while (!ranges_.empty() && bytes_freed < total_bytes_to_free) {
    ...
    if (end_timestamp == last_appended_buffer_timestamp_) {  // BIG TODO
...
void SourceBufferStream<RangeClass>::PrepareRangesForNextAppend(
  ...
  DecodeTimestamp next_timestamp =
      new_buffers.front()->GetDecodeTimestamp();  // BIG TODO
  ...
  DecodeTimestamp end = new_buffers.back()->GetDecodeTimestamp();  // BIG TODO
  ...
  if (!track_buffer_.empty()) {
    ...
    *out_buffer = next_buffer;
    ...
    last_output_buffer_timestamp_ =
        (*out_buffer)->GetDecodeTimestamp();  // BIG TODO
...
void SourceBufferStream<RangeClass>::WarnIfTrackBufferExhaustionSkipsForward(
  ...
  // BIG TODO make this warn in terms of PTS if ByPts.
...
void SourceBufferStream<RangeClass>::SetSelectedRangeIfNeeded(
  ...
  DecodeTimestamp seek_timestamp = FindNewSelectedRangeSeekTimestamp(
      start_timestamp);  // BIG TODO last_output and seek here, etc need to vary
                         // and be by PTS
