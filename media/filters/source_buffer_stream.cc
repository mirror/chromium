...
void SourceBufferStream<RangeClass>::OnStartOfCodedFrameGroupInternal(
  ...
  if (range_for_next_append_ == ranges_.end() ||
    ...
    if (range_for_next_append_ != ranges_.end()) {
      ...
      // range's start time. Note that no ::Remove or ::RemoveInternal is
      // possible between now and the next ::Append call. This update helps
      // prevent discontinuity from being introduced by the ::RemoveInternal
      // processing during the next ::Append call.
