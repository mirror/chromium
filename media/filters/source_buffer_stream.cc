...
// BIG TODO: Fix CFG signalling to be by PTS when pipeline setup that way.
// BIG TODO: Look into correctness/fixing the "remove since last append and new
// stuff" in ::Append (the first new buffer doesn't necessarily have the lowest
// PTS, and the last previously appended buffer doesn't necessarily have the
// highest previous PTS).
// BIG TODO: Other BIG TODO sprinkled throughout: fix or comment why not fixed
// with TODOs and later CLs.
...
void SourceBufferStream<RangeClass>::OnStartOfCodedFrameGroup(
  ...
  if (range_for_next_append_ == ranges_.end() ||
      !AreAdjacentInSequence(highest_timestamp_in_append_sequence_,
...
bool SourceBufferStream<RangeClass>::Append(const BufferQueue& buffers) {
  ...
  DCHECK(coded_frame_group_start_time_ <= BufferGetTimestamp(buffers.front()));
  // BIG TODO adjust FP to signal new CFG start on PTS when ByPts
  // BIG TODO adjust FP to signal (a new method) on PTS dropping below earlier
  // CFG start when ByPts, and for SBS to remove between old CFG start and new
  // (lower) CFG start in that case, but *not* set "new coded frame group". The
  // latter is reserved only for CFG discontinuity signalling.
  DCHECK(!end_of_stream_);
...
bool SourceBufferStream<RangeClass>::GarbageCollectIfNeeded(
  ...
  if (last_appended_buffer_timestamp_ != kNoDecodeTimestamp() &&
      last_appended_buffer_duration_ != kNoTimestamp &&
      highest_timestamp_in_append_sequence_ != kNoDecodeTimestamp() &&
      media_time > highest_timestamp_in_append_sequence_) {
...
size_t SourceBufferStream<RangeClass>::FreeBuffersAfterLastAppended(
    ...
    DecodeTimestamp media_time) {
  DVLOG(4) << __func__ << " highest_timestamp_in_append_sequence_="
           << highest_timestamp_in_append_sequence_.InSecondsF()
           << " media_time=" << media_time.InSecondsF();

  DecodeTimestamp remove_range_start = highest_timestamp_in_append_sequence_;
...
void SourceBufferStream<RangeClass>::PrepareRangesForNextAppend(
  ...
  DecodeTimestamp next_timestamp =
      new_buffers.front()->GetDecodeTimestamp();  // BIG TODO rewrite

  DecodeTimestamp remove_start_timestamp = kNoDecodeTimestamp();
  DecodeTimestamp remove_end_timestamp = kNoDecodeTimestamp();
  GetTimestampInterval(new_buffers, &remove_start_timestamp,
                       &remove_end_timestamp);
  DCHECK(remove_start_timestamp != kNoDecodeTimestamp());
  DCHECK(remove_end_timestamp != kNoDecodeTimestamp());
  ...
  if (highest_timestamp_in_append_sequence_ != kNoDecodeTimestamp() &&
      highest_timestamp_in_append_sequence_ < remove_start_timestamp) {
    remove_start_timestamp = highest_timestamp_in_append_sequence_;
  }
  ...
  if (new_coded_frame_group_) {
    ...
    remove_start_timestamp =
        std::min(coded_frame_group_start_time_, remove_start_timestamp);
  }
  ...
  const bool exclude_start = prev_timestamp == remove_start_timestamp &&
  ...
  RemoveInternal(remove_start_time, remove_end_time, exclude_start,
                 deleted_buffers);
}
...
SourceBufferStreamStatus SourceBufferStream<RangeClass>::GetNextBufferInternal(
  ...
  if (!track_buffer_.empty()) {
    ...
    *out_buffer = next_buffer;
    ...
    last_output_buffer_timestamp_ = BufferGetTimestamp(*out_buffer);
