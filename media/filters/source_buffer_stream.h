...
class MEDIA_EXPORT SourceBufferStream {
 ...
 private:
  ...
  // The timestamp (DTS if ByDts, PTS as DTS type if ByPts) on the last buffer
  // returned by the most recent GetNextBuffer() call. Set to
  // kNoDecodeTimestamp() if GetNextBuffer() hasn't been called yet or a seek
  // has happened since the last GetNextBuffer() call.
