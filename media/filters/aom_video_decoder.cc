...
static int GetThreadCount(const VideoDecoderConfig& config) {
  ...
  if (threads.empty() || !base::StringToInt(threads, &decode_threads)) {
    ...
    if (width >= 8192)
      ...
      decode_threads = 4;
    return std::min(decode_threads, base::SysInfo::NumberOfProcessors());
  }
...
// TODO(dalecurtis): Do we need to handle all these? Currently this just handles
// the same formats as the ffmpeg based aom decoder.
...
static void SetColorSpaceForFrame(const aom_image_t* img,
  ...
  if (img->cs == AOM_CS_BT_709)
    color_space = COLOR_SPACE_HD_REC709;
  else if (img->cs == AOM_CS_BT_601 || img->cs == AOM_CS_SMPTE_170)
...
void AomVideoDecoder::Initialize(const VideoDecoderConfig& config,
  ...
  if (aom_codec_dec_init(context.get(), aom_codec_av1_dx(), &aom_config,
    ...
    DLOG(ERROR) << "aom_codec_dec_init() failed: "
...
void AomVideoDecoder::Reset(const base::Closure& closure) {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  state_ = DecoderState::kNormal;
...
bool AomVideoDecoder::DecodeBuffer(const DecoderBuffer* buffer) {
  ...
  if (aom_codec_decode(
    ...
    DLOG(ERROR) << "aom_codec_decode() failed: "
                << aom_codec_error(aom_decoder_.get()) << (detail ? ", " : "")
                << (detail ? detail : "");
    return false;
  }
  ...
  while (aom_image_t* img = aom_codec_get_frame(aom_decoder_.get(), &iter)) {
    ...
    if (!frame) {
      DLOG(ERROR) << "Failed to produce video frame from aom_image_t.";
      return false;
    }
...
scoped_refptr<VideoFrame> AomVideoDecoder::CopyImageToVideoFrame(
  ...
  if (img->d_w > img->w || img->d_h > img->h) {
    DLOG(ERROR) << base::StringPrintf(
        "Display dimensions %dx%d exceed storage %dx%d\n", img->d_w, img->d_h,
        img->w, img->h);
    return nullptr;
  }
  ...
  // TODO(dalecurtis): Is this statement still true?
  const gfx::Size coded_size(img->w, img->d_h);
  const gfx::Size visible_size(img->d_w, img->d_h);

  auto frame = frame_pool_.CreateFrame(pixel_format, visible_size,
  ...
  if (needs_packing) {
    ...
    DCHECK_EQ(pixel_format, PIXEL_FORMAT_YV12);
    ...
    libyuv::I420Copy(img->planes[AOM_PLANE_Y], img->stride[AOM_PLANE_Y],
                     ...
                     frame->stride(VideoFrame::kVPlane), coded_size.width(),
                     coded_size.height());
  }
