...
void AudioRendererImpl::Initialize(DemuxerStream* stream,
  ...
  // Target ~20ms for our buffer size, but if the hardware needs something even
  // larger (say for Bluetooth devices) prefer that.
  //
  // Even if we're not resampling, where the actual buffer size is controlled
  // via the size of the AudioBus provided to Render(), we should choose a value
  // here based on hardware parameters since it affects the initial buffer size
  // used by AudioRendererAlgorithm. Too small and we may underflow when the
  // hardware asks for a larger buffer.
  ...
  if (use_stream_params) {
    ...
    // On some systems we do not need to  let the OS level resampler handle
    // resampling for power efficiency unless the initial sample rate is too
    // low; this allows support for sample rate adaptations where necessary.
