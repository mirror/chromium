...
bool AudioRendererImpl::HandleDecodedBuffer_Locked(
  ...
  if (buffer->end_of_stream()) {
    ...
    if (buffer->IsBitstreamFormat() && state_ == kPlaying) {
      ...
      // Skip the buffer since we are unable to trim a compressed audio buffer.
      if (buffer->timestamp() + buffer->duration() / 2 < start_timestamp_)
        return true;
  ...
  int frames_written = 0;
  {
    ...
    if (is_passthrough_ && algorithm_->frames_buffered() > 0) {
      ...
      if (play_delay > base::TimeDelta()) {
        MEDIA_LOG(INFO, media_log_)
            << "Delay is required for a compressed audio bitstream format!"
            << " Play Delay: " << play_delay;
      }
