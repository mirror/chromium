...
static const size_t kMaxBufferedFrames = 16;
...
void VideoRendererImpl::Flush(const base::Closure& callback) {
  ...
  pending_read_ = false;
  last_read_time_ = base::TimeTicks();
  ...
  max_read_duration_ = base::TimeDelta();
}
...
void VideoRendererImpl::OnTimeStopped() {
  ...
  if (buffering_state_ == BUFFERING_HAVE_NOTHING) {
    ...
    if (min_buffered_frames_ < kMaxBufferedFrames) {
      ...
      DVLOG(2) << "Increased min_buffered_frames_ to " << min_buffered_frames_;
    }
  }
}
...
void VideoRendererImpl::FrameReady(VideoFrameStream::Status status,
  ...
  const base::TimeDelta read_duration =
      base::TimeTicks::Now() - last_read_time_;
  last_read_time_ = base::TimeTicks();
  ...
  if (read_duration > max_read_duration_) {
    max_read_duration_ = read_duration;
    DVLOG(1) << "Updated max read duration: "
             << max_read_duration_.InMillisecondsF() << "ms";
  }
...
void VideoRendererImpl::AttemptRead_Locked() {
  ...
  if (algorithm_->effective_frames_queued() >= max_buffered_frames_)
    return;

  switch (state_) {
    case kPlaying:
      pending_read_ = true;
      last_read_time_ = base::TimeTicks::Now();
...
void VideoRendererImpl::UpdateMaxBufferedFrames() {
  ...
  if (was_background_rendering_ || max_read_duration_ <= frame_duration) {
    max_buffered_frames_ = min_buffered_frames_;
    return;
  }
  ...
  // of total_frames * (decode_duration - frame_duration) / frame_duration. We
  ...
  const size_t max_buffered_frames =
      std::min(min_buffered_frames_ *
                   static_cast<size_t>(std::ceil(
                       (max_read_duration_ - frame_duration).InMillisecondsF() /
                       frame_duration.InMillisecondsF())),
               kMaxBufferedFrames);

  if (max_buffered_frames_ != max_buffered_frames) {
    DVLOG(1) << "Updating max buffered frames to " << max_buffered_frames
             << ", average frame duration: " << frame_duration.InMillisecondsF()
             << "ms, max read duration: "
             << max_read_duration_.InMillisecondsF() << "ms.";
  }
