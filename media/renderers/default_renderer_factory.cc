...
DefaultRendererFactory::CreateVideoDecoders(
  ...
  if (gpu_factories) {
    ...
    // If kMojoVideoDecoder (for MCVD) is not enabled, then include AVDA.  We
    // don't want MCVD failures to fall back to AVDA; AVDA shouldn't be able to
    // do things that MCVD can't.
    include_gpu_decoder =
