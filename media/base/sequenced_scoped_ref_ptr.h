...
// the proper thread.
// TODO(liberato): this can't depend on the type.  it has to memorize the
// destructor and call it on |t| manually.  otherwise, we can't use an owner
// record from a subclass with a refptr to a superclass.  wait -- do we just
// need to make |T| SequencedObjectBase, and make everybody cast it?
