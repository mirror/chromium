// Copyright 2017 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#include "media/base/scoped_callback_runner.h"

#include <memory>
#include <utility>

#include "testing/gtest/include/gtest/gtest.h"

namespace media {

namespace {

void SetBool(bool* var, bool val) {
  *var = val;
}

void SetIntegers(int* a_var, int* b_var, int a_val, int b_val) {
  *a_var = a_val;
  *b_var = b_val;
}

void SetFromUniquePtr(int* var, std::unique_ptr<int> val) {
  *var = *val;
}

}  // namespace

TEST(ScopedCallbackRunnerTest, Closure_Run) {
  bool bool_var = false;
  base::OnceClosure cb =
      ScopedCallbackRunner(base::BindOnce(&SetBool, &bool_var, true));
  std::move(cb).Run();
  EXPECT_TRUE(bool_var);
}

TEST(ScopedCallbackRunnerTest, Closure_Destruction) {
  bool bool_var = false;
  {
    base::OnceClosure cb =
        ScopedCallbackRunner(base::BindOnce(&SetBool, &bool_var, true));
  }
  EXPECT_TRUE(bool_var);
}

TEST(ScopedCallbackRunnerTest, SetBool_Run) {
  bool bool_var = false;
  base::OnceCallback<void(bool)> cb =
      ScopedCallbackRunner(base::BindOnce(&SetBool, &bool_var), true);
  std::move(cb).Run(true);
  EXPECT_TRUE(bool_var);
}

TEST(ScopedCallbackRunnerTest, SetBool_Destruction) {
  bool bool_var = false;
  {
    base::OnceCallback<void(bool)> cb =
        ScopedCallbackRunner(base::BindOnce(&SetBool, &bool_var), true);
  }
  EXPECT_TRUE(bool_var);
}

TEST(ScopedCallbackRunnerTest, SetIntegers_Run) {
  int a = 0;
  int b = 0;
  base::OnceCallback<void(int, int)> cb =
      ScopedCallbackRunner(base::BindOnce(&SetIntegers, &a, &b), 3, 4);
  std::move(cb).Run(1, 2);
  EXPECT_EQ(a, 1);
  EXPECT_EQ(b, 2);
}

TEST(ScopedCallbackRunnerTest, SetIntegers_Destruction) {
  int a = 0;
  int b = 0;
  {
    base::OnceCallback<void(int, int)> cb =
        ScopedCallbackRunner(base::BindOnce(&SetIntegers, &a, &b), 3, 4);
  }
  EXPECT_EQ(a, 3);
  EXPECT_EQ(b, 4);
}

TEST(ScopedCallbackRunnerTest, SetFromUniquePtr_Run) {
  int a = 0;
  base::OnceCallback<void(std::unique_ptr<int>)> cb = ScopedCallbackRunner(
      base::BindOnce(&SetFromUniquePtr, &a), base::MakeUnique<int>(1));
  std::move(cb).Run(base::MakeUnique<int>(2));
  EXPECT_EQ(a, 2);
}

TEST(ScopedCallbackRunnerTest, SetFromUniquePtr_Destruction) {
  int a = 0;
  {
    base::OnceCallback<void(std::unique_ptr<int>)> cb = ScopedCallbackRunner(
        base::BindOnce(&SetFromUniquePtr, &a), base::MakeUnique<int>(1));
  }
  EXPECT_EQ(a, 1);
}

}  // namespace media
