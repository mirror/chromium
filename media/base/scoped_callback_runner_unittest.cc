...
TEST(ScopedCallbackRunnerTest, Closure_Run) {
  bool a = false;
  auto cb =
      ScopedCallbackRunner(base::BindOnce(&SetBool, &a, DEFAULT_ARGS(true)));
  std::move(cb).Run();
  EXPECT_TRUE(a);
}
...
TEST(ScopedCallbackRunnerTest, SetIntegers_Run) {
  int a = 0;
  int b = 0;
  auto cb = ScopedCallbackRunner(base::BindOnce(&SetIntegers, &a, &b),
                                 DEFAULT_ARGS(3, 4));
  std::move(cb).Run(1, 2);
  EXPECT_EQ(a, 1);
  EXPECT_EQ(b, 2);
...
TEST(ScopedCallbackRunnerTest, SetIntegerFromUniquePtr_Run) {
  ...
  auto cb = ScopedCallbackRunner(base::BindOnce(&SetIntegerFromUniquePtr, &a),
                                 DEFAULT_ARGS(base::MakeUnique<int>(1)));
...
TEST(ScopedCallbackRunnerTest, SetString_Run) {
  std::string a;
  auto cb = ScopedCallbackRunner(base::BindOnce(&SetString, &a),
                                 DEFAULT_ARGS("hello"));
