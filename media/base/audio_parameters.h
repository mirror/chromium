...
// data, using |paremeters| (or alternatively |channels| and |frames|).
MEDIA_SHMEM_EXPORT base::CheckedNumeric<uint32_t> ComputeAudioInputBufferSize(
...
// This convenience function safely computes the size required for an
// AudioOutputBuffer with enough memory for AudioBus data using |parameters| (or
// alternatively |channels| and |frames|).
MEDIA_SHMEM_EXPORT base::CheckedNumeric<uint32_t> ComputeAudioOutputBufferSize(
    const AudioParameters& parameters);
MEDIA_SHMEM_EXPORT base::CheckedNumeric<uint32_t> ComputeAudioOutputBufferSize(
    int channels,
    int frames);
