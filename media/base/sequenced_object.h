...
//  - is not easily passed to other sequences.
...
// SequencedWeakPointer as a function / method argument:
//
//   void AnyFunction(SequencedWeakPointer<Foo> swp) { ... }
...
// from yours, please see RemoteSequencedObjectOwner .
//
...
class SequencedObject : public base::UnbindableAsPointer {
 public:
  ...
  // SequencedWeakPointer<Foo>(this).BindOnce(&Foo::Method, ...);
...
//
// Callbacks are not tied to the SequencedWeakPointer instance that created
// them.  It's legal to do this:
//
// SequencedWeakPointer<Foo>(&foo).BindOnce(...);
template <typename T>
class SequencedWeakPointer {
 public:
...
//   };
//
// It's safe to base::Unretained(this), since |goo_owner_| is a member variable,
// and will cancel any pending callback into |this| if it's destroyed.  If it
// weren't, then you should use a WeakPtr (or a callback made by
...
// |goo_owner_| is destroyed.  Once the callback completes, one may call:
//
//   goo_owner_.GetSequencedWeakPointer()
//
...
class RemoteSequencedObjectOwner {
 ...
 private:
  ...
  // This is run on the local sequence to receive the unique ptr, and also a
  // SequencedWeakPointer.  We can't actually construct one here, since that
  // requries accessing the remote object.  We can only copy existing ones.
  ...
  // SWP that we're given, that we copy as needed.
