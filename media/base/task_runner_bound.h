...
class TaskRunnerBound {
 ...
 public:
  ...
  template <typename U>
  TaskRunnerBound<C>& operator=(TaskRunnerBound<U>&& other) {
    if (other.is_null()) {
      reset();
    } else {
      ptr_ = other.ptr_;
 ...
 protected:
  ...
  TaskRunnerBound(scoped_refptr<base::SequencedTaskRunner> task_runner,
    ...
    ConstructionHelper* helper =
        static_cast<ConstructionHelper*>(malloc(sizeof(ConstructionHelper)));
    ...
    task_runner_->PostTask(
        FROM_HERE, base::BindOnce(
                       [](ConstructionHelper* c, Args&&... args) {
                         new (c)
                             ConstructionHelper(std::forward<Args>(args)...);
                       },
                       base::Unretained(helper), std::forward<Args>(args)...));
  }
