...
//   MyObject(const char* widget_title) {}
//   ~MyObject() { ... }
//   void doSomething(int arg) { ... }
// };
//
// // Meanwhile, on the media thread...
// scoped_refptr<TaskRunner> main_task_runner = ...;
// TaskRunnerBound<MainThreadWidget> widget(main_task_runner, "My Widget");
//
...
//
// auto widget =
//   TaskRunnerBound<MainThreadWidgetImpl::Create(main_task_runner, "...");
// auto c = new SomeMediaThreadConstruct(std::move(widget));
//
// We could also do this to construct |c| on the media thread:
...
class TaskRunnerBound {
 public:
  ...
  static TaskRunnerBound Create(scoped_refptr<base::TaskRunner> task_runner,
                                Args&&... args) {
 ...
 protected:
  template <typename... Args>
  TaskRunnerBound(scoped_refptr<base::TaskRunner> task_runner, Args&&... args)
