...
TEST_F(SequencedObjectTest, CastFromDerivedObjectWorks) {
  // Test casting of various types.
  SequencedUniquePtr<DerivedObject> derived_unique(
      base::SequencedTaskRunnerHandle::Get());

  // TODO(liberato): verify that all of these actually work.
  ...
  // should not work.
  // SequencedWeakPtr<DerivedObject> swp_derived_2(swp_test);
  SequencedUniquePtr<TestObject> test_unique(std::move(derived_unique));

  // TODO(liberato): check move construction and move assignment too.
}
