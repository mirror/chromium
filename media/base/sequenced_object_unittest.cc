...
TEST_F(SequencedObjectTest, WeakRefsPostToProperThread) {
  ...
  std::unique_ptr<TestObject> test_ = base::MakeUnique<TestObject>();
  SequencedWeakPtr<TestObject> swp(test_);

  // Check that the method is not called until we RunUntilIdle.
  const int value = 123;
  EXPECT_CALL(*test_, MethodWithInt(_)).Times(0);
  swp.BindOnce(&TestObject::MethodWithInt).Run(value);
  testing::Mock::VerifyAndClearExpectations(test_.get());
  EXPECT_CALL(*test_, MethodWithInt(value)).Times(1);
...
TEST_F(SequencedObjectTest, RefPtrsCanBeCopiedAndReset) {
  ...
  // Resetting the other should do it.
  test_ref_2.reset();
  scoped_task_environment_.RunUntilIdle();
  ASSERT_EQ(swp.GetRawPointerForTesting(), nullptr);
}

TEST_F(SequencedObjectTest, UniquePtrsConstructCanBeReset) {
  const int value = 123;
  ...
  // Resetting |test_unique| should post destruction, but not invalidate the
