...
TEST_F(PostOnlyTest, ConstructThenPostThenReset) {
  auto derived = PostOnlyOwner<Derived>::Create(task_runner_, &value);
...
TEST_F(PostOnlyTest, MoveConstructionFromSameClass) {
  ...
  auto derived_old = PostOnlyOwner<Derived>::Create(task_runner_, &value);
...
TEST_F(PostOnlyTest, MultiplyDerviedDestructionWorksLeftSuper) {
  ...
  auto mderived =
      PostOnlyOwner<MultiplyDerived>::Create(task_runner_, &value, &value2);
