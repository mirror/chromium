...
class RemoteSequencedObjectOwner {
 public:
  ...
  RemoteSequencedObjectOwner(
      ...
      Args... args)
      : impl_task_runner_(std::move(impl_task_runner)), weak_factory_(this) {
    // Construct a weak ref, which we will fill in on the impl sequence.
    scoped_refptr<typename SequencedObject<T>::InternalWeakRef> weak_ref(
        new typename SequencedObject<T>::InternalWeakRef);
    swp_ = SequencedWeakPtr<T>(impl_task_runner_, weak_ref);
    ...
    impl_task_runner_->PostTask(
        ...
        base::BindOnce(
            ...
            base::SequencedTaskRunnerHandle::Get(), std::move(weak_ref),
 ...
 private:
  ...
  static void ConstructOnImplSequence(
    ...
    // TODO(liberato): handle the case where |wp| fails to promote.  it will
    // currently free |ptr| on the owner's thread instead of the impl thread.
  ...
  // This is run on the local sequence to receive the unique ptr.
  ...
  // SWP that we're given, which we copy as needed.
  SequencedWeakPtr<T> swp_;
