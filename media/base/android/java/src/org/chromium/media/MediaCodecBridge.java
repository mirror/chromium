        ...
        try {
            ...
            if (indexOrStatus >= 0) {
                ...
                if (isConfigFrame) {
                    ...
                    mConfigData.put(getMediaCodecOutputBuffer(indexOrStatus));
                    // Log few SPS header bytes to check profile and level.
                    String spsData = "";
                    for (int i = 0; i < (info.size < 8 ? info.size : 8); i++) {
                        spsData += Integer.toHexString(mConfigData.get(i) & 0xff) + " ";
                    }
                    Log.i(TAG, spsData);
                    mConfigData.rewind();
                    // Release buffer back.
            ...
            if (indexOrStatus >= 0) { // index!
                ...
                boolean isKeyFrame = Build.VERSION.SDK_INT > Build.VERSION_CODES.KITKAT
