...
// This is the same implementation of base::android::ToJavaArrayOfByteArray,
// but with an argument signature of vector<vector<uint8_t>> instead of
// vector<string>.
// TODO(sanfin): move this to base/android/jni_array.h with OWNERS' blessing.
ScopedJavaLocalRef<jobjectArray> ToJavaArrayOfByteArray(
    JNIEnv* env,
    const std::vector<CodecSpecificData>& v) {
  ScopedJavaLocalRef<jclass> byte_array_clazz =
      base::android::GetClass(env, "[B");
  jobjectArray joa =
      env->NewObjectArray(v.size(), byte_array_clazz.obj(), NULL);
  base::android::CheckException(env);

  for (size_t i = 0; i < v.size(); ++i) {
    ScopedJavaLocalRef<jbyteArray> byte_array = base::android::ToJavaByteArray(
        env, reinterpret_cast<const uint8_t*>(v[i].data()), v[i].size());
...
bool GetCodecSpecificDataForAudio(AudioCodec codec,
                                  ...
                                  std::vector<CodecSpecificData>* output_csds,
  ...
  switch (codec) {
    case kCodecVorbis: {
      ...
      CodecSpecificData first_header;
      first_header.assign(current_pos, current_pos + header_length[0]);
      output_csds->push_back(std::move(first_header));

      // The last header is the codec header.
      CodecSpecificData last_header;
      last_header.assign(extra_data + total_length,
                         extra_data + extra_data_size - total_length);
      output_csds->push_back(std::move(last_header));
      break;
    }
    case kCodecAAC: {
      ...
      csd.push_back(profile << 3 | frequency_index >> 1);
      csd.push_back((frequency_index & 0x01) << 7 | channel_config << 3);
      output_csds->push_back(std::move(csd));
    ...
    case kCodecOpus: {
      ...
      csd0.assign(extra_data, extra_data + extra_data_size);
      output_csds->push_back(std::move(csd0));

      // csd1 - Codec Delay
      CodecSpecificData csd1;
      const uint8_t* codec_delay_ns_ptr =
          reinterpret_cast<const uint8_t*>(&codec_delay_ns);
      csd1.assign(codec_delay_ns_ptr, codec_delay_ns_ptr + sizeof(int64_t));
      output_csds->push_back(std::move(csd1));

      // csd2 - Seek Preroll
      CodecSpecificData csd2;
      const uint8_t* seek_preroll_ns_ptr =
          reinterpret_cast<const uint8_t*>(&seek_preroll_ns);
      csd2.assign(seek_preroll_ns_ptr, seek_preroll_ns_ptr + sizeof(int64_t));
      output_csds->push_back(std::move(csd2));
      break;
    }
    default:
...
std::unique_ptr<MediaCodecBridge> MediaCodecBridgeImpl::CreateAudioDecoder(
  ...
  std::vector<CodecSpecificData> csds;
  bool output_frame_has_adts_header = false;

  if (!GetCodecSpecificDataForAudio(config.codec(), config.extra_data().data(),
                                    config.extra_data().size(), codec_delay_ns,
                                    seek_preroll_ns, &csds,
                                    &output_frame_has_adts_header)) {
    return nullptr;
  }

  ScopedJavaLocalRef<jobjectArray> j_csds = ToJavaArrayOfByteArray(env, csds);
  ...
  ScopedJavaGlobalRef<jobject> j_bridge(
      Java_MediaCodecBridgeBuilder_createAudioDecoder(
          ...
          j_csds, output_frame_has_adts_header));

  if (j_bridge.is_null())
    return nullptr;

  return base::WrapUnique(new MediaCodecBridgeImpl(j_bridge));
}
