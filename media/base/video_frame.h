...
class MEDIA_EXPORT VideoFrame : public base::RefCountedThreadSafe<VideoFrame> {
 public:
  ...
  class BasicBuffer : public Buffer {
   public:
    explicit BasicBuffer(size_t data_size) : data_(data_size) {}

    size_t size() const override { return data_.size(); }
    void resize(size_t data_size) override { data_.resize(data_size); }
    const uint8_t* data() const override { return &data_[0]; }
    uint8_t* data() override { return &data_[0]; }
