...
class TaskRunnerBoundTest : public ::testing::Test {
 public:
...
TEST_F(TaskRunnerBoundTest, ConstructThenPostThenReset) {
  auto derived = TaskRunnerBound<Derived>::Create(task_runner_, &value);
...
TEST_F(TaskRunnerBoundTest, PostBeforeConstruction) {
...
TEST_F(TaskRunnerBoundTest, MoveConstructionFromSameClass) {
  // Verify that we can move-construct with the same class.
  auto derived_old = TaskRunnerBound<Derived>::Create(task_runner_, &value);
  ...
  derived_new = TaskRunnerBound<Derived>();
  EXPECT_EQ(value, kDerivedCtorValue);
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(value, kDerivedDtorValue);
}

TEST_F(TaskRunnerBoundTest, MoveConstructionFromDerivedClass) {
  // Verify that we can move-construct to a base class from a derived class.
  auto derived = TaskRunnerBound<Derived>::Create(task_runner_, &value);
  TaskRunnerBound<Base> base(std::move(derived));
  EXPECT_TRUE(derived.is_null());
  ...
  // Derived properly.
  base = TaskRunnerBound<Base>();
  EXPECT_EQ(value, kDerivedCtorValue);
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(value, kDerivedDtorValue);
}

TEST_F(TaskRunnerBoundTest, MoveAssignmentFromSameClass) {
  // Test move-assignment using the same classes.
  auto derived_old = TaskRunnerBound<Derived>::Create(task_runner_, &value);
  TaskRunnerBound<Derived> derived_new;
...
TEST_F(TaskRunnerBoundTest, MoveAssignmentFromDerivedClass) {
  // Move-assignment from a derived class to a base class.
  auto derived = TaskRunnerBound<Derived>::Create(task_runner_, &value);
  TaskRunnerBound<Base> base;
...
TEST_F(TaskRunnerBoundTest, NonVirtualDestructorIsCalled) {
  ...
  // behavior if it does.
  auto derived = TaskRunnerBound<Derived>::Create(task_runner_, &value);
  TaskRunnerBound<NonVirtualBase> non_virtual_base = std::move(derived);
...
TEST_F(TaskRunnerBoundTest, MultiplyDerviedDestructionWorksLeftSuper) {
  ...
  auto mderived =
      TaskRunnerBound<MultiplyDerived>::Create(task_runner_, &value, &value2);
  TaskRunnerBound<Other> other = std::move(mderived);
...
TEST_F(TaskRunnerBoundTest, MultiplyDerviedDestructionWorksRightSuper) {
  // Verify that everything works when we're casting around in ways that might
  ...
  auto mderived =
      TaskRunnerBound<MultiplyDerived>::Create(task_runner_, &value, &value2);
  TaskRunnerBound<Base> base = std::move(mderived);
