...
#include "media/cast/net/udp_transport_host_impl.h"
...
bool UdpTransportHostImpl::OnPacketReceived(std::unique_ptr<Packet> packet) {
  DVLOG(3) << __func__;
  if (!client_)
    return true;
...
void UdpTransportHostImpl::SendPacket(std::unique_ptr<Packet> packet) {
  DVLOG(3) << __func__;
