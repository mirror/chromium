...
class UdpTransportHostImplTest : public ::testing::Test {
 public:
  UdpTransportHostImplTest()
    ...
    receiver_transport_ = std::make_unique<UdpTransport>(
        nullptr, scoped_task_environment_.GetMainThreadTaskRunner(),
 ...
 protected:
  ...
  std::unique_ptr<UdpTransport> receiver_transport_;
...
TEST_F(UdpTransportHostImplTest, ReceivingPackets) {
  ...
  base::MockCallback<base::RepeatingClosure> done_callback;
  EXPECT_TRUE(receiver_transport_->SendPacket(
      new base::RefCountedData<Packet>(packet1), cb));
  scoped_task_environment_.RunUntilIdle();

  // Receiver sends |packet2| to sender. Expected to be done successfully.
  EXPECT_TRUE(receiver_transport_->SendPacket(
      new base::RefCountedData<Packet>(packet2), cb));
  scoped_task_environment_.RunUntilIdle();
