...
bool UdpTransport::SendPacket(PacketRef packet, const base::Closure& cb) {
  ...
  if (client_connected_) {
    ...
    result =
        udp_socket_->Write(NO_TRAFFIC_ANNOTATION_YET, buf.get(),
                           static_cast<int>(packet->data.size()), callback);
  } else if (!IsEmpty(remote_addr_)) {
    result = udp_socket_->SendTo(NO_TRAFFIC_ANNOTATION_YET, buf.get(),
