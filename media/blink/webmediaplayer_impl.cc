...
WebMediaPlayerImpl::WebMediaPlayerImpl(
    ...
    : frame_(frame),
      ...
      submitter_creation_callback_(params->create_submitter_callback()),
...
// static
void WebMediaPlayerImpl::OnSubmitterCreated(
    const base::WeakPtr<WebMediaPlayerImpl>& web_media_player,
    const scoped_refptr<base::SingleThreadTaskRunner>& task_runner,
    std::unique_ptr<blink::WebVideoFrameSubmitter> submitter) {
  if (web_media_player)
    web_media_player->submitter_ = std::move(submitter);
  else
    task_runner->DeleteSoon(FROM_HERE, std::move(submitter));
}
  ...
  if (HasVideo()) {
    ...
    if (!surface_layer_for_video_enabled_) {
      ...
      base::PostTaskAndReplyWithResult(
          ...
          base::Bind(&OnSubmitterCreated, AsWeakPtr(), vfc_task_runner_));
    }
  }
