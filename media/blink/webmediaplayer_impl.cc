...
WebMediaPlayerImpl::~WebMediaPlayerImpl() {
  ...
  if (chunk_demuxer_) {
    // Destroy |chunk_demuxer_| on the |media_task_runner_| to avoid racing
    // other pending tasks on |chunk_demuxer_| on that runner while not further
    // blocking |main_task_runner_| to perform the destruction.
