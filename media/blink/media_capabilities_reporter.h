...
class MEDIA_BLINK_EXPORT MediaCapabilitiesReporter {
 public:
  ...
  // FIXME not sure if these are needed. Ideally, decoded frames and dropped
  // frames stats would still be valid during background playback.
  // Alternatively, we can implement these and respond by stopping/starting
  // reporting. OnHidden/Visible()

 private:
  void UpdateStats();

  int GetFrameRateBucket(PipelineStatistics stats);
  ...
  int num_consecutive_fps_changes_ = 0;
