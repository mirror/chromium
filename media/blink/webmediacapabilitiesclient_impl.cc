...
void BindToHistoryService(mojom::VideoDecodePerfHistoryPtr* history_ptr) {
  // FIXME do these ptrs always exist?
  ...
  const std::string& service_name = platform->GetBrowserServiceName();

  connector->BindInterface(service_name, mojo::MakeRequest(history_ptr));
}
...
void WebMediaCapabilitiesClientImpl::DecodingInfo(
  ...
  // FIXME - split this up with helper methods.
