...
class MEDIA_BLINK_EXPORT VideoFrameCompositor : public VideoRendererSink,
 ...
 public:
  ...
  void StartSubmitting(const viz::FrameSinkId& id);

  // Indicates whether the endpoint for the VideoFrame exists.
  // Note: This will be more relavent once we are able to obtain signals of
  // creation/destruction from the SurfaceLayerImpl;
  bool ClientSinkAvailable();
  ...
  void set_submitter_for_test(blink::WebVideoFrameSubmitter* submitter) {
    submitter_.reset(submitter);
  }
 ...
 private:
  ...
  // media thread. The VideoFrameCompositor is called, run, and destroyed on
  // |task_runner|.
