...
bool VideoFrameCompositor::ClientSinkAvailable() {
  if (!surface_layer_for_video_enabled_)
    return client_;
  else
...
VideoFrameCompositor::GetCurrentFrameAndUpdateIfStale() {
  ...
  if (ClientSinkAvailable() || !rendering_ || !is_background_rendering_)
    return current_frame_;
...
void VideoFrameCompositor::BackgroundRender() {
  ...
  if (!surface_layer_for_video_enabled_) {
    if (new_frame && ClientSinkAvailable())
      client_->DidReceiveFrame();
  } else {
    if (new_frame && ClientSinkAvailable())
      submitter_->DidReceiveFrame();
