...
scoped_refptr<VideoFrame> VideoFrameCompositor::GetCurrentFrameOnAnyThread() {
  ...
  if (first_frame_consumed_cb_)
    std::move(first_frame_consumed_cb_).Run();
...
void VideoFrameCompositor::SetFirstFrameConsumedCallback(
    base::OnceClosure first_frame_consumed_cb) {
  first_frame_consumed_cb_ = std::move(first_frame_consumed_cb);
}
