...
void VideoFrameCompositor::StartSubmitting(const viz::FrameSinkId& id) {
  DCHECK(task_runner_->BelongsToCurrentThread());
...
void VideoFrameCompositor::OnRendererStateUpdate(bool new_state) {
  ...
  if (surface_layer_for_video_enabled_ && !submitter_)
    return;

  if (!surface_layer_for_video_enabled_ && !client_)
    return;
...
void VideoFrameCompositor::PaintSingleFrame(
  ...
  if (!surface_layer_for_video_enabled_) {
    ...
    if (ProcessNewFrame(frame, repaint_duplicate_frame) && submitter_)
...
VideoFrameCompositor::GetCurrentFrameAndUpdateIfStale() {
  ...
  if (!surface_layer_for_video_enabled_) {
    ...
    if (submitter_ || !rendering_ || !is_background_rendering_)
      return current_frame_;
  }
