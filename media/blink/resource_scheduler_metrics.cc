...
void ResourceSchedulerMetrics::OnPlayerCreated(WebMediaPlayer* player,
  ...
  player_rec.priority = client->ComputePlayerPriority();
  players_.emplace(std::make_pair(player, player_rec));

  // Record that a player was created, including whether it had a priority.
  UMA_HISTOGRAM_BOOLEAN("Media.ResourceScheduler.OnPlayerCreated",
                        player_rec.priority != INT_MIN);
}
...
void ResourceSchedulerMetrics::OnPlayerPlayed(WebMediaPlayer* player) {
  ...
  int right = 0;
  int wrong = 0;
  int total = players_.size();
  // Limit the total because UMA needs it.
  if (total >= kMaxPlayers)
    total = kMaxPlayers - 1;
  ...
  for (auto iter = players_.begin(); iter != players_.end(); iter++) {
    ...
    // that it would receive resources to preroll sooner if possible.
  ...
  LOG(ERROR) << "AVDA: " << __func__ << " right: " << right
             << " wrong: " << wrong << " total: " << players_.size();
}
...
void ResourceSchedulerMetrics::OnFirstFrameDrawn(WebMediaPlayer* player) {
  ...
  for (auto iter = players_.begin(); iter != players_.end(); iter++) {
    ...
    // right by accident -- if they are constructed earler and start sooner,
