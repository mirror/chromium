...
const base::TimeDelta VideoDecodeStatsReporter::kRecordingInterval =
    base::TimeDelta::FromSeconds(2);
const base::TimeDelta VideoDecodeStatsReporter::kTinyFpsWindowDuration =
...
void VideoDecodeStatsReporter::OnNaturalSizeChanged(
  ...
  if (natural_size_.IsEmpty()) {
    ...
    // begin reporting. Also, resetting framerate state above may give us
    // another chance at framerate detection if past attempts failed. Avoid
...
void VideoDecodeStatsReporter::UpdateStats() {
  ...
  // Check framerate for changes. Bail if framerate needs more samples to
  // stabilize.
