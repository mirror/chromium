...
TEST_P(FFmpegRegressionTest, BasicPlayback) {
  if (GetParam().init_status == PIPELINE_OK) {
    ASSERT_EQ(PIPELINE_OK, Start(GetParam().filename, kFFmpegRegression));
    Play();
    ...
    EXPECT_NE(PIPELINE_OK, Start(GetParam().filename, kFFmpegRegression));
  }
}

TEST_P(FlakyFFmpegRegressionTest, BasicPlayback) {
  if (Start(GetParam().filename, kFFmpegRegression) == PIPELINE_OK) {
    Play();
