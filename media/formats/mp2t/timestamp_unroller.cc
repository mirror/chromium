...
int64_t TimestampUnroller::GetUnrolledTimestamp(int64_t timestamp) {
  ...
  // Bit mask for the high 31 bits of unrolled timestamp, to avoid
  // left shifting of non-zero bits, which might results in
  // undefined behavior.
  const int64 unrolled_time_high_mask = (1 << 32) - 1;
