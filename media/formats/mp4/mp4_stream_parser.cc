...
ParseResult MP4StreamParser::EnqueueSample(BufferQueueMap* buffers) {
  ...
  if (!runs_->IsRunValid()) {
    ...
    // TODO(sandersd): As I understand it, either the trim has already happened,
    // or this can discard at most up to the end of the current moof. Is this
    // the right place for behavior like that?
    DCHECK(queue_.Trim(mdat_tail_));
