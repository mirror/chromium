...
// Tries to parse |data| to extract the VP9 Profile ID, or return Profile 0.
media::VideoCodecProfile GetVP9CodecProfile(const std::vector<uint8_t>& data) {
  ...
  const uint8_t kVP9ProfileFieldId = 0x01;
  const uint8_t kVP9ProfileFieldLength = 1;
  if (data.size() < 3 || data[0] != kVP9ProfileFieldId ||
      data[1] != kVP9ProfileFieldLength || data[2] > 3) {
    return VP9PROFILE_PROFILE0;
  }

  return static_cast<enum VideoCodecProfile>(
