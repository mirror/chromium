...
TEST_F(GpuJpegDecodeAcceleratorTest, InitializeAndDecode) {
  ...
  GpuJpegDecodeAccelerator::Create(mojo::MakeRequest(&jpeg_decoder_));
  ...
  jpeg_decoder_->Decode(
      BitstreamBuffer(kArbitraryBitstreamBufferId, base::SharedMemoryHandle(),
                      kInputBufferSizeInBytes),
      kDummyFrameCodedSize, std::move(output_frame_handle),
