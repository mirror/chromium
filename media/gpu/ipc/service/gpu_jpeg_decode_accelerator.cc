...
void GpuJpegDecodeAccelerator::VideoFrameReady(int32_t bitstream_buffer_id) {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  base::ResetAndReturn(&notify_decode_status_cb_)
      .Run(bitstream_buffer_id, mojom::Error::NO_ERRORS);
}
...
void GpuJpegDecodeAccelerator::NotifyError(int32_t bitstream_buffer_id,
  ...
  base::ResetAndReturn(&notify_decode_status_cb_)
      .Run(bitstream_buffer_id, static_cast<mojom::Error>(error));
}
...
void GpuJpegDecodeAccelerator::Decode(
  ...
  notify_decode_status_cb_ = BindToCurrentLoop(
  ...
  io_task_runner_->PostTask(
      ...
      base::Bind(&GpuJpegDecodeAccelerator::DecodeOnIOThread,
                 weak_ptr_factory_.GetWeakPtr(), base::Passed(&input_buffer),
...
void GpuJpegDecodeAccelerator::DecodeOnIOThread(
  ...
  if (!VerifyDecodeParams(coded_size, &output_handle, output_buffer_size)) {
    base::ResetAndReturn(&notify_decode_status_cb_)
        .Run(input_buffer->id, mojom::Error::INVALID_ARGUMENT);
    return;
  }
  ...
  if (!output_shm->Map(output_buffer_size)) {
    LOG(ERROR) << "Could not map output shared memory for input buffer id "
               << input_buffer->id;
    base::ResetAndReturn(&notify_decode_status_cb_)
        .Run(input_buffer->id, mojom::Error::PLATFORM_FAILURE);
    return;
  }
...
void GpuJpegDecodeAccelerator::NotifyDecodeStatus(DecodeCallback callback,
                                                  int32_t bitstream_buffer_id,
                                                  mojom::Error error) {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  std::move(callback).Run(bitstream_buffer_id, error);
}
