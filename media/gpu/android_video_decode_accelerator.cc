...
void AndroidVideoDecodeAccelerator::SendDecodedFrameToClient(
  ...
  // TODO(liberato): only do this for M+, since it doesn't help otherwise.
  const bool want_promotion_hint = true;
  ...
  // We unconditionally mark this as allow_overlay if we're requesting a
  // promotion hint, but we'll also mark that it's a surface texture.
