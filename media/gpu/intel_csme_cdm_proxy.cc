...
IntelCSMECdmProxy::KeyInfo::KeyInfo() = default;
IntelCSMECdmProxy::KeyInfo::KeyInfo(uint32_t crypto_session_id,
                                    const std::vector<uint8_t>& key_id,
                                    const std::vector<uint8_t>& key_blob)
    : crypto_session_id(crypto_session_id),
      key_id(key_id),
      key_blob(key_blob) {}
IntelCSMECdmProxy::KeyInfo::KeyInfo(const KeyInfo&) = default;
IntelCSMECdmProxy::KeyInfo::~KeyInfo() {}
...
void IntelCSMECdmProxy::SetKey(uint32_t crypto_session_id,
                               const std::vector<uint8_t>& key_id,
                               const std::vector<uint8_t>& key_blob) {
  KeyInfo key_info(crypto_session_id, key_id, key_blob);
  key_info_map_[key_id] = key_info;
}

void IntelCSMECdmProxy::RemoveKey(uint32_t crypto_session_id,
                                  const std::vector<uint8_t>& key_id) {
