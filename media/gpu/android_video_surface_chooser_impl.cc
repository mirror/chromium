...
void AndroidVideoSurfaceChooserImpl::Choose() {
  ...
  bool power_efficient = current_state_.promote_aggressively;
  ...
  if (new_overlay_state == kUsingSurfaceTexture)
    ...
    SwitchToOverlay(power_efficient);
}
...
void AndroidVideoSurfaceChooserImpl::SwitchToOverlay(bool power_efficient) {
  ...
  config.power_efficient = power_efficient;
  config.power_cb =
...
void AndroidVideoSurfaceChooserImpl::OnPowerEfficientState(
  ...
  // If the ovrlay has become power efficient, then take no action.
