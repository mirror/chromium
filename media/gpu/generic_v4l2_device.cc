...
scoped_refptr<gl::GLImage> GenericV4L2Device::CreateGLImage(
  ...
  gfx::BufferFormat gmb_format = ui::GetBufferFormatFromFourCCFormat(fourcc);
  scoped_refptr<gfx::NativePixmap> pixmap =
      ui::OzonePlatform::GetInstance()
          ->GetSurfaceFactoryOzone()
          ->CreateNativePixmapFromHandle(0, size, gmb_format,
  ...
  bool ret = image->Initialize(pixmap.get(), gmb_format);
  DCHECK(ret);
  return image;
}
