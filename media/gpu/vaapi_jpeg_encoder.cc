...
using JPEGHeader = uint8_t[JPEG_HEADER_SIZE];
...
size_t FillJpegHeader(const gfx::Size& input_size,
  ...
  // Start Of Input.
  const uint8_t kSOI[] = {0xFF, JPEG_SOI};
  ...
  for (size_t i = 0; i < 2; ++i) {
    const uint8_t kQuantSegment[] = {
        0xFF, JPEG_DQT, 0x00,
        0x03 + DCTSIZE2,         // Segment length:67 (2-byte).
  ...
  const uint8_t kDcSegment[] = {
      0xFF, JPEG_DHT, 0x00,
      0x1F,  // Segment length:31 (2-byte).
  };
  const uint8_t kAcSegment[] = {
      0xFF, JPEG_DHT, 0x00,
