    ...
    switch (status) {
      case MEDIA_CODEC_OK: {
        if (eos) {
          ...
          // buffers because we submit the EOS flag on empty input buffers.
          // The MediaCodec docs leave open the possibility that the last
          // non-empty output buffer has the EOS flag but we haven't seen that
          // happen.
