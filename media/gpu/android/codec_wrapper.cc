...
class CodecWrapperImpl : public base::RefCountedThreadSafe<CodecWrapperImpl> {
 public:
  ...
  using CodecWrapper::DequeueStatus;
  using CodecWrapper::QueueStatus;
...
QueueStatus CodecWrapperImpl::QueueInputBuffer(
...
DequeueStatus CodecWrapperImpl::DequeueOutputBuffer(
    base::TimeDelta* presentation_time,
    bool* end_of_stream,
    std::unique_ptr<CodecOutputBuffer>* codec_buffer) {
  DVLOG(4) << __func__;
