...
// to a helper class hosted on the gpu thread. It guarantees that none of the
// response callbacks will run after it's destructed.
class MEDIA_GPU_EXPORT VideoFrameFactoryImpl : public VideoFrameFactory {
 ...
 private:
  ...
  std::unique_ptr<GpuVideoFrameFactory> gpu_side_;
