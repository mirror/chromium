...
TEST_F(MediaCodecVideoDecoderTest, CodecIsReleasedOnDestruction) {
  ...
  EXPECT_CALL(
      *codec_allocator_,
      MockReleaseMediaCodec(codec_allocator_->most_recent_codec(), _, _));
}

TEST_F(MediaCodecVideoDecoderTest,
       SurfaceChooserNotInitializedWithOverlayFactory) {
  InitializeWithSurfaceTexture();
  // The surface chooser should not have an overlay factory because SetOverlay()
  // was not called before it was initialized.
...
TEST_F(MediaCodecVideoDecoderTest,
  ...
  Initialize(TestVideoConfig::NormalH264());
  mcvd_->SetOverlay(123, base::nullopt);
  mcvd_->Decode(nullptr, decode_cb_.Get());
  // The surface chooser should have an overlay factory because SetOverlay() was
  // called before it was initialized.
  ASSERT_TRUE(surface_chooser_->factory_);
}

TEST_F(MediaCodecVideoDecoderTest, SetOverlayIsValidBeforeInitialize) {
  mcvd_->SetOverlay(123, base::nullopt);
  Initialize(TestVideoConfig::NormalH264());
  mcvd_->Decode(nullptr, decode_cb_.Get());
  ASSERT_TRUE(surface_chooser_->factory_);
...
TEST_F(MediaCodecVideoDecoderTest, CodecIsCreatedWithChosenOverlay) {
  ...
  EXPECT_CALL(*codec_allocator_, MockCreateMediaCodecAsync(overlay.get(), _));
