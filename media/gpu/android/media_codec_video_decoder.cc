...
void MediaCodecVideoDecoder::OnSurfaceDestroyed(AndroidOverlay* overlay) {
  ...
  // If codec allocation is in progress, we go into state SURFACE_DESTROYED.
  ...
  // available this will result in a failed playback. We should handle this
  // gracefully by allocating a new codec.
