...
class InitVideoFrameFactoryFlow : public Flow {
 public:
  InitVideoFrameFactoryFlow(State* state, base::Closure done_cb)
      : Flow(state, done_cb), weak_factory_(this) {}

  void Start() override {
    DVLOG(1) << "InitVideoFrameFactoryFlow::" << __func__;
    DCHECK(!state_->video_frame_factory);
    factory_.reset(new VideoFrameFactory(state_->gpu_task_runner));
    factory_->Initialize(
        state_->get_command_buffer_stub_cb,
...
// Drain the codec if necessary and call ReleaseMediaCodec().
// TODO(watk): This should potentially wait for the actual release to complete
// to avoid the surface reuse problem.
class ReleaseCodecFlow : public Flow {
  using Flow::Flow;
};

// XXX: Potentially we should also do resource acquisition in this flow.
...
class CreateCodecFlow : public Flow {
 public:
  CreateCodecFlow(State* state, base::Closure done_cb)
      : Flow(state, done_cb), weak_factory_(this) {}
  ...
  void Start() override {
    ...
    state_->codec_allocator->CreateMediaCodecAsync(
        client_.weak_factory.GetWeakPtr(), codec_config);
  }
...
class DestructFlow : public Flow {
 public:
  DestructFlow(State* state, base::Closure done_cb)
      : Flow(state, done_cb), weak_factory_(this) {}
  ...
  void Cancel() override {
    ...
    if (releaser_) {
      releaser_->Cancel();
      set_status(releaser_->status());
    }
  }

  void CodecReleased() {
    DVLOG(1) << "DestructFlow::" << __func__;
    // This may reentrantly delete |this| and *state_.
    delete state_->mcvd;
  }
...
class ResetFlow : public Flow {
 public:
  ...
  void Start() override {
    ...
    releaser_ = base::MakeUnique<ReleaseCodecFlow>(
        state_,
        base::Bind(&ResetFlow::ResetComplete, weak_factory_.GetWeakPtr()));
    releaser_->Start();
  }
...
class DecodeFlow : public Flow {
 public:
  ...
  // XXX Awkward that this can arrive after Cancel(). Maybe Flow should
  // track canceled state and ignore events when it's canceled? Or MCVD
  // shouldn't deliver these if the flow is canceled.
  void HandleDecode() override {
    repeating_timer_.Start(
        base::Bind(&DecodeFlow::PumpDecoder, base::Unretained(this)));
  }
...
MediaCodecVideoDecoder::MediaCodecVideoDecoder(
    ...
    : status_(Status::kOk),
      did_first_init_(false),
      destruction_pending_(false),
      flow_weak_factory_(this),
      weak_factory_(this) {
  // XXX: inject it for testing.
  state_.surface_chooser = base::MakeUnique<AndroidVideoSurfaceChooserImpl>();
  state_.gpu_task_runner = gpu_task_runner;
  state_.get_command_buffer_stub_cb = get_command_buffer_stub_cb;
  state_.codec_allocator = codec_allocator;
}
...
void MediaCodecVideoDecoder::Initialize(const VideoDecoderConfig& config,
  ...
  if (!state_.codec_allocator->StartThread(&state_.codec_allocator_client)) {
    bound_init_cb.Run(false);
    return;
  }
  ...
  if (!did_first_init_) {
    state_.surface_chooser->Initialize(
...
void MediaCodecVideoDecoder::CancelPendingDecodes(DecodeStatus decode_status) {
  for (auto& pending_decode : state_.pending_decodes)
    pending_decode.decode_cb.Run(decode_status);
  state_.pending_decodes.clear();
}
...
void MediaCodecVideoDecoder::StartNextFlow() {
  ...
  if (destruction_pending_) {
    flow_ = base::MakeUnique<DestructFlow>(&state_, done_cb);
  } else if (status_ == Status::kError) {
    // If we're in an error state only DestructFlow should run.
    return;
  } else if (state_.reset_cb) {
    flow_ = base::MakeUnique<ResetFlow>(&state_, done_cb);
  } else if (!state_.codec && state_.pending_decodes.empty()) {
    // Wait for a pending decode before initializing.
    return;
  } else if (!state_.video_frame_factory) {
    flow_ = base::MakeUnique<InitVideoFrameFactoryFlow>(&state_, done_cb);
  } else if (!state_.codec) {
    flow_ = base::MakeUnique<CreateCodecFlow>(&state_, done_cb);
    // } else if (NeedsSetSurface()) {
    //   flow_ = base::MakeUnique<TransitionSurfaceFlow>();
  } else {
...
void MediaCodecVideoDecoder::OnSurfaceTransition(
  ...
  // XXX: Temporary. We should make SurfaceChooser not make this call
  // reentrantly from Initialize.
  if (did_first_init_)
    StartNextFlow();
}
