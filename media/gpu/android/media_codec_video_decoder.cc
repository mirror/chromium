...
void MediaCodecVideoDecoder::Decode(const scoped_refptr<DecoderBuffer>& buffer,
  ...
  if (lazy_init_pending_) {
    ...
    StartLazyInit();
    return;
  } else if (state_ == State::kBeforeSurfaceInit) {
    // Wait for initialization to finish.
    return;
  }

  if (codec_needs_flush_) {
    codec_needs_flush_ = false;
    FlushCodec();
  }
...
bool MediaCodecVideoDecoder::DequeueOutput() {
  ...
  // Discard |output_buffer| without rendering it if we don't need it.
  if (drain_type_ == DrainType::kForReset ||
      drain_type_ == DrainType::kForDestroy) {
    return true;
  }
...
void MediaCodecVideoDecoder::StartDrainingCodec(DrainType drain_type) {
  DVLOG(2) << __func__;
  DCHECK(drain_type == DrainType::kForDestroy ||
         drain_type == DrainType::kForReset);
...
void MediaCodecVideoDecoder::OnCodecDrained() {
  ...
  if (drain_type_ == DrainType::kForReset) {
    ...
    // Flush on the next decode.
    codec_needs_flush_ = true;
  }
  drain_type_.reset();
}
