...
bool MediaCodecVideoDecoder::QueueInput() {
  ...
  // Flush the codec when there are no unrendered codec buffers. This lets us
  // avoid unbacking any frames when we flush.
  if (codec_->IsDrained()) {
...
bool MediaCodecVideoDecoder::DequeueOutput() {
  ...
  // While draining we can discard |output_buffer| without rendering it.
  if (drain_type_) {
    if (eos) {
      OnCodecDrained();
      return false;
  ...
  if (eos && eos_decode_cb_) {
    // The codec is now drained, but it may still be backing unrendered frames
    // near EOS so we can't flush it immediately. Instead it will be flushed
    // when all outstanding buffers are released.
    // Note: It's important to post |eos_decode_cb_| through the gpu task runner
    // to ensure it follows all previous outputs.
