...
void MediaCodecVideoDecoder::SetOverlay(
    base::Optional<int32_t> surface_id,
    OverlayInfo::RoutingToken overlay_routing_token) {
  DVLOG(2) << __func__;
  surface_id_ = std::move(surface_id);
  overlay_routing_token_ = std::move(overlay_routing_token);

  // If we're in kBeforeSurfaceInit we haven't initialized surface chooser yet.
...
AndroidOverlayFactoryCB MediaCodecVideoDecoder::CreateOverlayFactoryCb() {
  // Return a null cb by default.
  AndroidOverlayFactoryCB factory;
  if (surface_id_)
    factory = base::Bind(&CreateContentVideoViewOverlay, *surface_id_);
