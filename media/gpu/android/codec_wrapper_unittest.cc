...
TEST_F(CodecWrapperTest, CodecOutputBuffersAreInitiallyValid) {
  auto codec_buffer = DequeueCodecOutputBuffer();
  ASSERT_FALSE(codec_buffer->HasBeenInvalidated());
  ASSERT_TRUE(wrapper_->HasValidCodecOutputBuffers());
}

TEST_F(CodecWrapperTest, FlushInvalidatesCodecOutputBuffers) {
  auto codec_buffer = DequeueCodecOutputBuffer();
  wrapper_->Flush();
  ASSERT_TRUE(codec_buffer->HasBeenInvalidated());
}
...
TEST_F(CodecWrapperTest, CodecOutputBuffersAreAllInvalidatedTogether) {
  ...
  wrapper_->Flush();
  ASSERT_TRUE(codec_buffer1->HasBeenInvalidated());
  ASSERT_TRUE(codec_buffer2->HasBeenInvalidated());
  ASSERT_FALSE(wrapper_->HasValidCodecOutputBuffers());
}
...
TEST_F(CodecWrapperTest, CodecOutputBufferReleaseDoesNotInvalidateOthers) {
  ...
  ASSERT_FALSE(codec_buffer2->HasBeenInvalidated());
}
