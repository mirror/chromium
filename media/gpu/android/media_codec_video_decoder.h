...
class MEDIA_GPU_EXPORT MediaCodecVideoDecoder : public VideoDecoder {
 public:
  ...
  // Sets the overlay to use. At most one one of |surface_id| and
  // |routing_token| should be valid. This can be called before Initialize() to
  // set the first overlay.
  void SetOverlay(base::Optional<int32_t> surface_id,
 ...
 private:
  ...
  // Creates an overlay factory cb based on the current value of surface_id_ and
  ...
  // The surface id or overlay routing token for the overlay that we should be
  // rendering to. At most one of these has a valid value at any given time.
  base::Optional<int32_t> surface_id_;
