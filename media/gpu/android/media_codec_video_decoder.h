...
class MEDIA_GPU_EXPORT MediaCodecVideoDecoder
 ...
 private:
  ...
  enum class State {
    // We're initializing.
    kInitializing,
    // Initialization has completed and we're running.
    kRunning,
    // A fatal error occurred. A terminal state.
    kError,
    // The output surface was destroyed. This is also a terminal state, but it
    // isn't reported as a decode error.
    kSurfaceDestroyed,
  };
  ...
  // The surface that our codec should be using. Returns |overlay_bundle_| if
  // it's valid, or |surface_texture_bundle_| otherwise.
  scoped_refptr<AVDASurfaceBundle> TargetSurface();

  // Whether we have a codec and its surface is not equal to TargetSurface().
  bool SurfaceTransitionPending();

  // Sets |codecs_|'s output surface to TargetSurface().
  ...
  void ReleaseCodec(base::Closure released_cb);
  ...
  // A surface bundle that is set when our target output surface is an overlay.
  scoped_refptr<AVDASurfaceBundle> overlay_bundle_;
