...
AVDACodecAllocator* AVDACodecAllocator::GetInstance(
  ...
  static AVDACodecAllocator* allocator = new AVDACodecAllocator(task_runner);

  // Verify that this caller agrees on the task runner.
  DCHECK_EQ(allocator->task_runner_, task_runner);
...
void AVDACodecAllocator::CreateMediaCodecAsyncInternal(
  ...
  if (!task_runner) {
    // Thread failed to start -- post a null codec.
    client_task_runner->PostTask(
        FROM_HERE, base::Bind(&AVDACodecAllocatorClient::OnCodecConfigured,
                              client, nullptr, codec_config->surface_bundle));
    return;
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::Bind(&AVDACodecAllocator::ForwardOrDropCodec,
                 base::Unretained(this), client_task_runner, client, *task_type,
