...
void SurfaceChooserHelper::SetIsFullscreen(bool is_fullscreen) {
  // TODO(liberato): avda currently only sets is_expecting_relayout when
...
void SurfaceChooserHelper::NotifyPromotionHintAndUpdateChooser(
  ...
  if (hints_until_clear_relayout_flag_ > 0) {
    ...
    if (hints_until_clear_relayout_flag_ == 0) {
      // TODO(liberato): make sure that MCVD sets this.  more generally, make
      // sure that it handles overlay info similarly to avda.
  ...
  LOG(ERROR) << "AVDA: promotable: " << promotable << " "
             << surface_chooser_state_.is_compositor_promotable;
