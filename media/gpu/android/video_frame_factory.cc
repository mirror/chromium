...
void VideoFrameFactoryImpl::Initialize(GetStubCb get_stub_cb, InitCb init_cb) {
...
void VideoFrameFactoryImpl::NewVideoFrame(
  ...
  base::PostTaskAndReplyWithResult(
      gpu_task_runner_.get(), FROM_HERE,
      base::Bind(&GpuVideoFrameFactory::NewVideoFrame,
...
scoped_refptr<VideoFrame> GpuVideoFrameFactory::NewVideoFrame(
    std::unique_ptr<CodecOutputBuffer> output_buffer,
    scoped_refptr<SurfaceTextureGLOwner> surface_texture,
    base::TimeDelta timestamp) {
  if (!stub_)
  ...
  auto texture_ref = CreateTexture(decoder, texture_manager, std::move(image),
                                   std::move(surface_texture), size);
  ...
  // TODO(watk): Is this correct for MediaCodec?
  gfx::Rect visible_rect(size);
  gfx::Size natural_size(size);
