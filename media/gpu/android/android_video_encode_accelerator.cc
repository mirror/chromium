...
void AndroidVideoEncodeAccelerator::QueueInput() {
  ...
  // mapping to the generated |presentaion_timestamp_|, and will read them out
  ...
  presentaion_timestamp_ += base::TimeDelta::FromMicroseconds(
      base::Time::kMicrosecondsPerSecond / INITIAL_FRAMERATE);
  DCHECK(frame_timestamp_map_.find(presentaion_timestamp_) ==
         frame_timestamp_map_.end());
  frame_timestamp_map_[presentaion_timestamp_] = frame->timestamp();

  status = media_codec_->QueueInputBuffer(input_buf_index, nullptr, queued_size,
                                          presentaion_timestamp_);
