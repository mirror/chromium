...
void MaybeRenderEarly(std::vector<Image*>* image_vector_ptr) {
  ...
  // If there's no image in the front buffer we can safely render one. Iterate
  // until we successfully render one to skip over invalidated images.
...
// VideoFrameFactory creates CodecOutputBuffer backed VideoFrames. and attempts
// to eagerly render them to their surface to release the buffers back to the
// decoder as soon as possible.
// Not thread safe.
// Virtual for testing; see VideoFrameFactoryImpl.
class VideoFrameFactory {
 public:
  ...
  // Initializes the factory and runs |init_cb| thread when it's complete. If
  // the returned surface texture is null, initialization failed.
  virtual void Initialize(GetStubCb get_stub_cb, InitCb init_cb) = 0;
  ...
  // instead. Runs |frame_created_cb| to return the frame.
  virtual void NewVideoFrame(
...
// VideoFrameFactoryImpl is single threaded and can be used on any thread.
// It internally posts calls to GpuVideoFrameFactory.
class VideoFrameFactoryImpl : public VideoFrameFactory {
 public:
  explicit VideoFrameFactoryImpl(
      scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner);
  ~VideoFrameFactoryImpl() override;

  void Initialize(GetStubCb get_stub_cb, InitCb init_cb) override;
  void NewVideoFrame(std::unique_ptr<CodecOutputBuffer> output_buffer,
                     scoped_refptr<SurfaceTextureGLOwner> surface_texture,
                     base::TimeDelta timestamp,
...
class GpuVideoFrameFactory {
 public:
  ...
  scoped_refptr<VideoFrame> NewVideoFrame(
      std::unique_ptr<CodecOutputBuffer> output_buffer,
      scoped_refptr<SurfaceTextureGLOwner> surface_texture,
      base::TimeDelta timestamp);
