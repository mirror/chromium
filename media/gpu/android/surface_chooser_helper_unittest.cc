...
TEST_F(SurfaceChooserHelperTest, SetIsFullscreen) {
  ...
  ASSERT_TRUE(most_recent_state_.is_expecting_relayout);
...
TEST_F(SurfaceChooserHelperTest, SetIsOverlayRequired) {
  ...
  UpdateChooserState();
  ASSERT_TRUE(most_recent_state_.is_required);

  helper_->SetIsOverlayRequired(false);
  UpdateChooserState();
  ASSERT_FALSE(most_recent_state_.is_required);
}
...
TEST_F(SurfaceChooserHelperTest, SetInsecureSurface) {
  ...
  UpdateChooserState();
  ASSERT_FALSE(most_recent_state_.is_secure);
  ASSERT_FALSE(most_recent_state_.is_required);
}
...
TEST_F(SurfaceChooserHelperTest, SetRequestedSecureSurface) {
  ...
  UpdateChooserState();
  ASSERT_TRUE(most_recent_state_.is_secure);
  ASSERT_FALSE(most_recent_state_.is_required);
}
...
TEST_F(SurfaceChooserHelperTest, SetPromoteAggressively) {
  ...
  UpdateChooserState();
  ASSERT_TRUE(most_recent_state_.promote_aggressively);

  helper_->SetPromoteAggressively(false);
  UpdateChooserState();
  ASSERT_FALSE(most_recent_state_.promote_aggressively);
}
...
TEST_F(SurfaceChooserHelperTest, PromotionHintsRelayPosition) {
  ...
  ASSERT_EQ(most_recent_state_.initial_position, rect);
}
...
TEST_F(SurfaceChooserHelperTest, PromotionHintsRelayPromotable) {
  ...
  helper_->NotifyPromotionHintAndUpdateChooser(hint, false);
  ASSERT_FALSE(most_recent_state_.is_compositor_promotable);
  ...
  helper_->NotifyPromotionHintAndUpdateChooser(hint, false);
  ASSERT_TRUE(most_recent_state_.is_compositor_promotable);
}
...
TEST_F(SurfaceChooserHelperTest, PromotionHintsClearRelayoutFlag) {
  ...
  ASSERT_FALSE(most_recent_state_.is_expecting_relayout);
}
