...
template<class T> class weak_ptr {
    ...
    public:
        ...
        // automatically if given a wp.
        template<typename Method, typename... Args> auto BindOnce(Method method, Args... args) {
            // TODO(liberato): make base::Bind check the method args too (or
            // instead).  Right now, it checks the bound arguments, so one can
            // still create a callback with an unbound ptr.
            static_assert(GetMadIfPointerMethod<Method>::TheResult, "WeakPtrable types must not be sent as raw ptrs.");
...
// Foo is a RemotableObject.  It can create weak_ptrs itself.
class Foo : public threadsafe::RemotableObject<Foo> {
    public:
        ...
        // method3 is directly, rather than binding it.  We don't want pointers
        // too instances of Foo to be sent via callback to other threads,
        // though, so method3 isn't bindable.
...
// fn3 cannot be bound to a raw ptr via base::*Bind, just like Foo::method3.
// TODO(liberato): currently, one could leave Foo* unbound, pass it as a
// callback, and call Run(&foo).  that's not good either.  We should modify
// base::Bind to check the functor's arguments, in addition to the bound args,
// just like we when binding as part of a wp.
void fn3(Foo* x) {
}

void fn() {
    // Create a Foo, which is a RemotableObject.
    Foo foo;
    ...
    // Sending |&foo| doesn't work now that Bind checks for ptrs.
    //base::Bind(&fn2, &foo);
    base::Bind(&fn2, wp);
    ...
    // This won't work, since rmap is a RemotableObject.
    //base::BindOnce([](std::map<int,int>*){}, &rmap);
}
