...
void AndroidVideoSurfaceChooserImpl::Choose() {
  ...
  if (current_state_.is_fullscreen) {
    ...
    // We could keep this as true, also, but we don't just to preserve legacy
    // behavior.  Probably, it makes no difference.
...
void AndroidVideoSurfaceChooserImpl::SwitchToOverlay(
  ...
  // |power_efficient|, then we should cancel it anyway.  In practice, this
  // doesn't happen, so we ignore it.
  if (overlay_)
    return;
