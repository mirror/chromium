...
class VaapiJpegEncodeAccelerator::Encoder {
 ...
 public:
  ...
  void NotifyErrorFromEncoderThread(int video_frame_id, Status status);
...
VaapiJpegEncodeAccelerator::Encoder::Encoder(
    ...
    : cached_output_buffer_size_(0),
      ...
      video_frame_ready_cb_(video_frame_ready_cb),
      notify_error_cb_(notify_error_cb) {}
...
void VaapiJpegEncodeAccelerator::Encoder::NotifyErrorFromEncoderThread(
    int video_frame_id,
    Status status) {
  notify_error_cb_.Run(video_frame_id, status);
}
...
JpegEncodeAccelerator::Status VaapiJpegEncodeAccelerator::Initialize(
  ...
  encoder_.reset(
      new Encoder(std::move(vaapi_wrapper),
                  base::BindRepeating(
...
void VaapiJpegEncodeAccelerator::Encode(
  ...
  if (video_frame->format() != VideoPixelFormat::PIXEL_FORMAT_I420) {
    ...
    task_runner_->PostTask(
        FROM_HERE, base::Bind(&VaapiJpegEncodeAccelerator::NotifyError,
                              weak_this_factory_.GetWeakPtr(), video_frame_id,
                              INVALID_ARGUMENT));
    return;
  }
  ...
  encoder_task_runner_->PostTask(
      ...
      base::Bind(&VaapiJpegEncodeAccelerator::Encoder::EncodeTask,
                 base::Unretained(&*encoder_), base::Passed(&request)));
}
