...
void VaapiJpegEncodeAccelerator::Encoder::EncodeTask(
  ...
  if (!shm->Map()) {
    ...
    notify_error_cb_.Run(video_frame_id, INACCESSIBLE_OUTPUT_BUFFER);
    return;
  }
...
JpegEncodeAccelerator::Status VaapiJpegEncodeAccelerator::Initialize(
  ...
  encoder_ = std::unique_ptr<Encoder>(
      new Encoder(std::move(vaapi_wrapper),
                  BindToCurrentLoop(base::BindRepeating(
                      &VaapiJpegEncodeAccelerator::VideoFrameReady, weak_this)),
                  BindToCurrentLoop(base::BindRepeating(
                      &VaapiJpegEncodeAccelerator::NotifyError, weak_this))));
