...
void VaapiJpegEncodeAccelerator::VideoFrameReady(int video_frame_id) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  client_->VideoFrameReady(video_frame_id);
}
...
void VaapiJpegEncodeAccelerator::EncodeTask(
  ...
  coded_size_ = request->video_frame->coded_size();
  std::vector<VASurfaceID> va_surfaces;
  if (!vaapi_wrapper_->CreateSurfaces(va_rt_format_,
                                      coded_size_,
                                      1,
                                      &va_surfaces)) {
    LOG(ERROR) << "Failed to create VA surface";
    NotifyErrorFromEncoderThread(video_frame_id,
                                 PLATFORM_FAILURE);
    return;
  }
  ...
  if (!vaapi_wrapper_->DownloadAndDestroyCodedBuffer(
      output_buffer_id,
      va_surface_id_,
      static_cast<uint8_t*>(request->shm->memory()),
      request->shm->size(),
      &encoded_size)) {
    ...
    LOG(ERROR) << "Successfully retrieve output image from VA coded buffer";

  }

  task_runner_->PostTask(
      FROM_HERE, base::Bind(&VaapiJpegEncodeAccelerator::VideoFrameReady,
                            weak_this_, video_frame_id));
}
...
void VaapiJpegEncodeAccelerator::Encode(
  ...
  TRACE_EVENT1("jpeg", "Encode", "input_id", video_frame->unique_id());
  ...
  std::unique_ptr<EncodeRequest> request(new EncodeRequest(
      video_frame, std::move(shm)));
