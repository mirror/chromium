...
void VaapiJpegEncodeAccelerator::VideoFrameReady(
  ...
  client_->VideoFrameReady(bitstream_buffer, encoded_picture_size);
}
...
void VaapiJpegEncodeAccelerator::EncodeTask(
  ...
  if (!vaapi_wrapper_->DownloadAndDestroyCodedBuffer(
          output_buffer_id, va_surface_id, static_cast<uint8_t*>(shm->memory()),
          shm->size(), &encoded_size)) {
  ...
  task_runner_->PostTask(
      FROM_HERE, base::Bind(&VaapiJpegEncodeAccelerator::VideoFrameReady,
                            weak_this_, request->output_buffer, encoded_size));
}
...
size_t VaapiJpegEncodeAccelerator::GetMaxCodedBufferSize(
  ...
  return VaapiJpegEncoder::GetMaxCodedBufferSize(picture_size.width(),
                                                 picture_size.height());
}
