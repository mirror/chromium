...
void AVDACodecAllocator::ForwardOrDropCodec(
  ...
  if (!client) {
    ...
    // stopping.  We can't post without restarting them / cancelling any pending
    // stop request.  In the former case, the threads aren't running.  In the
    // latter case, we might hang the main thread if the release hangs, if the
    // main thread hasn't joined by then.
    //
