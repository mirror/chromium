...
void VaapiVideoEncodeAccelerator::EndFrame() {
  ...
  submitted_encode_jobs_.push(base::MakeUnique(current_encode_job_.release()));
}
...
void VaapiVideoEncodeAccelerator::TryToReturnBitstreamBuffer() {
  ...
  std::unique_ptr<EncodeJob> encode_job = submitted_encode_jobs_.front();
...
void VaapiVideoEncodeAccelerator::EncodeFrameTask() {
  ...
  std::unique_ptr<InputFrameRef> frame_ref = encoder_input_queue_.front();
...
void VaapiVideoEncodeAccelerator::UseOutputBitstreamBufferTask(
  ...
  available_bitstream_buffers_.push(base::MakeUnique(buffer_ref.release()));
  TryToReturnBitstreamBuffer();
}
