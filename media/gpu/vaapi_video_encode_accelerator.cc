...
void VaapiVideoEncodeAccelerator::EndFrame() {
  ...
  submitted_encode_jobs_.push(current_encode_job_.release());
}
...
void VaapiVideoEncodeAccelerator::TryToReturnBitstreamBuffer() {
  ...
  std::unique_ptr<EncodeJob> encode_job(
      std::move(submitted_encode_jobs_.front()));
...
void VaapiVideoEncodeAccelerator::EncodeTask(
  ...
  encoder_input_queue_.push(
      std::move(base::MakeUnique(new InputFrameRef(frame, force_keyframe))));
  EncodeFrameTask();
}
...
void VaapiVideoEncodeAccelerator::EncodeFrameTask() {
  ...
  std::unique_ptr<InputFrameRef> frame_ref(
      std::move(encoder_input_queue_.front()));
...
void VaapiVideoEncodeAccelerator::UseOutputBitstreamBufferTask(
  ...
  available_bitstream_buffers_.push(buffer_ref.release());
  TryToReturnBitstreamBuffer();
}
