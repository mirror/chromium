...
static unsigned int VaSurfaceFormatForJpeg(
  ...
  DVLOGF(4) << "Unsupported sampling factor: num_components="
...
VaapiJpegDecodeAccelerator::~VaapiJpegDecodeAccelerator() {
  DCHECK(task_runner_->BelongsToCurrentThread());
  DVLOGF(2) << "Destroying VaapiJpegDecodeAccelerator";
...
void VaapiJpegDecodeAccelerator::Decode(
  ...
  VLOGF(4) << "Mapping new input buffer id: " << bitstream_buffer.id()
