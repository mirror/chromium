...
class VaapiVideoDecodeAcceleratorTest : public TestWithParam<VideoCodecProfile>,
 ...
 public:
  ...
  MOCK_METHOD3(MockCreateVaapiPicture,
               void(VaapiWrapper* /* vaapi_wrapper */,
                    int32_t /* picture_buffer_id */,
                    const gfx::Size& /* size */));
  linked_ptr<VaapiPicture> CreateVaapiPicture(
      ...
      uint32_t texture_id,
      uint32_t client_texture_id) {
    MockCreateVaapiPicture(vaapi_wrapper.get(), picture_buffer_id, size);
...
TEST_P(VaapiVideoDecodeAcceleratorTest,
  ...
  // AssignPictureBuffers() accordingly and expect another go at Decode().
  {
    ...
    EXPECT_CALL(*this, MockCreateVaapiPicture(mock_vaapi_wrapper_.get(), _,
                                              kPictureSize))
        .Times(2);
