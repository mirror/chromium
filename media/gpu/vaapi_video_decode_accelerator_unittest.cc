...
// triggers a SetStream() and then a Decode() on |mock_decoder_|.
TEST_P(VaapiVideoDecodeAcceleratorTest, QueueInputBufferAndDecode) {
  ...
  EXPECT_CALL(*mock_decoder_, Decode())
      .Times(1)
      .WillOnce(DoAll(RunClosure(quit_closure),
                      Return(AcceleratedVideoDecoder::kDecodeError)));
