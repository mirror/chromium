...
void D3D11VideoDecoderImpl::Decode(const scoped_refptr<DecoderBuffer>& buffer,
  ...
  // TODO(liberato): post.  we're not supposed to call back before this
  // returns.  probably doesn't matter since we're in the gpu process.
  // this goes for the error case above, too.
  DoDecode();
}
...
void D3D11VideoDecoderImpl::DoDecode() {
  ...
  while (true) {
    ...
    if (result == media::AcceleratedVideoDecoder::kRanOutOfStreamData) {
      ...
      // Hardware is busy. Try later.
      // TODO(liberato): PostDelayed.
      break;
    } else {
...
void D3D11VideoDecoderImpl::CreatePictureBuffers() {
  ...
  for (size_t i = 0; i < num_buffers; i++) {
    ...
    picture_buffers_[i]->Init(get_stub_cb_, video_device_, out_texture,
...
void D3D11VideoDecoderImpl::OutputResult(D3D11PictureBuffer* buffer) {
  ...
  output_cb_.Run(base::BindOnce(&D3D11VideoDecoderImpl::OnMailboxReleased,
