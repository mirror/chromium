...
void D3D11VideoDecoderImpl::CreatePictureBuffers() {
  ...
  for (size_t i = 0; i < num_buffers; i++) {
    ...
    for (int texture_idx = 0; texture_idx < textures_per_picture;
      ...
      client_ids.push_back(0);  // TODO: isn't there a constant for this?
    ...
    picture_buffers_.push_back(base::MakeUnique<D3D11PictureBuffer>(buffer, i));

    // The D3D11PictureBuffer holds onto the Textures.
    picture_buffers_[i]->texture_refs_ = texture_refs;
    CHECK_EQ(texture_refs.size(), textures_per_picture);
    ...
    for (int texture_idx = 0; texture_idx < textures_per_picture;
      ...
      picture_buffers_[i]->mailbox_holders_[texture_idx] = gpu::MailboxHolder(
    ...
    for (int texture_idx = 0;
         texture_idx < picture_buffers_[i]->texture_refs_.size();
         texture_idx++) {
      texture_manager->SetLevelImage(
          picture_buffers_[i]->texture_refs_[texture_idx].get(),
          GL_TEXTURE_EXTERNAL_OES, 0, picture_buffers_[i]->gl_image().get(),
...
size_t D3D11VideoDecoderImpl::input_buffer_id() const {
  ...
  // OutputResult.  not sure if it should be here or not.
  return 0;
}
...
void D3D11VideoDecoderImpl::OutputResult(D3D11PictureBuffer* buffer,
  ...
  auto frame = VideoFrame::WrapNativeTextures(
      buffer->picture_buffer().pixel_format(), buffer->mailbox_holders_,
  ...
  // after MojoVideoDecoderService is destructed.  See VideoFrameFactoryImpl.
