...
std::unique_ptr<VideoEncodeAccelerator> CreateV4L2VEA() {
  ...
  if (device)
    return std::make_unique<V4L2VideoEncodeAccelerator>(std::move(device));
  return nullptr;
}
#endif
...
std::unique_ptr<VideoEncodeAccelerator> CreateVaapiVEA() {
  return std::make_unique<VaapiVideoEncodeAccelerator>();
}
#endif

#if defined(OS_ANDROID) && BUILDFLAG(ENABLE_WEBRTC)
std::unique_ptr<VideoEncodeAccelerator> CreateAndroidVEA() {
  return std::make_unique<AndroidVideoEncodeAccelerator>();
}
#endif

#if defined(OS_MACOSX)
std::unique_ptr<VideoEncodeAccelerator> CreateVTVEA() {
  return std::make_unique<VTVideoEncodeAccelerator>();
}
#endif

#if defined(OS_WIN)
// Creates a MediaFoundationVEA, compatible or not with Windows 7.
std::unique_ptr<VideoEncodeAccelerator> CreateMediaFoundationVEA(
    bool compatible_with_win7) {
  return std::make_unique<MediaFoundationVideoEncodeAccelerator>(
