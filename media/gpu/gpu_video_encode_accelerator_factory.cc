...
std::unique_ptr<VideoEncodeAccelerator> CreateV4L2VEA() {
  ...
  if (device)
    return new V4L2VideoEncodeAccelerator(std::move(device));
  return nullptr;
}
#endif
...
std::unique_ptr<VideoEncodeAccelerator> CreateVaapiVEA() {
  return new VaapiVideoEncodeAccelerator();
}
#endif

#if defined(OS_ANDROID) && BUILDFLAG(ENABLE_WEBRTC)
std::unique_ptr<VideoEncodeAccelerator> CreateAndroidVEA() {
  return new AndroidVideoEncodeAccelerator();
}
#endif

#if defined(OS_MACOSX)
std::unique_ptr<VideoEncodeAccelerator> CreateVTVEA() {
  return new VTVideoEncodeAccelerator();
}
#endif
...
std::unique_ptr<VideoEncodeAccelerator> CreateMediaFoundationVEA(
  ...
  return new MediaFoundationVideoEncodeAccelerator(compatible_with_win7);
}
#endif
