...
VADisplayState* VADisplayState::Get() {
  ...
  static bool result = display_state->PostSandboxInitialization();
  return result ? display_state : nullptr;
}
...
#if defined(USE_OZONE)
  DCHECK(VADisplayState::Get()->drm_fd_.is_valid())
      << "PreSandboxInitialization() must have been called";
#endif
  StubPathMap paths;
...
// static
SupportedProfiles* SupportedProfiles::Get() {
  static SupportedProfiles* profile_infos = new SupportedProfiles();
  return profile_infos;
}

std::vector<SupportedProfiles::ProfileInfo>
SupportedProfiles::GetSupportedProfileInfosForCodecMode(
    VaapiWrapper::CodecMode mode) {
  return supported_profiles_[mode];
}

bool SupportedProfiles::IsProfileSupported(VaapiWrapper::CodecMode mode,
                                           VAProfile va_profile) {
  DCHECK(va_display_);
...
SupportedProfiles::SupportedProfiles()
    : va_lock_(VADisplayState::Get()->va_lock()),
      va_display_(nullptr),
      report_error_to_uma_cb_(base::Bind(&ReportErrorToUMA)) {
  ...
  for (size_t i = 0; i < VaapiWrapper::kCodecModeMax; ++i) {
    VaapiWrapper::CodecMode mode = static_cast<VaapiWrapper::CodecMode>(i);

    std::vector<VAConfigAttrib> required_attribs = GetRequiredAttribs(mode);
    VAEntrypoint entrypoint =
        (mode == VaapiWrapper::kEncode ? VAEntrypointEncSlice
                                       : VAEntrypointVLD);
    ...
    supported_profiles_[i] = supported_profile_infos;
  }
}

bool SupportedProfiles::GetSupportedVAProfiles(
...
bool SupportedProfiles::IsEntrypointSupported_Locked(VAProfile va_profile,
                                                     VAEntrypoint entrypoint) {
  va_lock_->AssertAcquired();
...
bool SupportedProfiles::AreAttribsSupported_Locked(
    VAProfile va_profile,
    VAEntrypoint entrypoint,
    const std::vector<VAConfigAttrib>& required_attribs) {
  va_lock_->AssertAcquired();
...
bool SupportedProfiles::GetMaxResolution_Locked(
    VAProfile va_profile,
    VAEntrypoint entrypoint,
    std::vector<VAConfigAttrib>& required_attribs,
    gfx::Size* resolution) {
...
// includes a workaround for https://crbug.com/345569: If |profile| is h264
...
VAProfile ProfileToVAProfile(VideoCodecProfile profile,
  ...
  if (!SupportedProfiles::Get()->IsProfileSupported(mode, va_profile) &&
    ...
    if (SupportedProfiles::Get()->IsProfileSupported(
...
VaapiWrapper::GetSupportedEncodeProfiles() {
  ...
  std::vector<SupportedProfiles::ProfileInfo> encode_profile_infos =
      SupportedProfiles::Get()->GetSupportedProfileInfosForCodecMode(kEncode);
...
VaapiWrapper::GetSupportedDecodeProfiles() {
  ...
  std::vector<SupportedProfiles::ProfileInfo> decode_profile_infos =
      SupportedProfiles::Get()->GetSupportedProfileInfosForCodecMode(kDecode);
...
bool VaapiWrapper::IsJpegDecodeSupported() {
  return SupportedProfiles::Get()->IsProfileSupported(kDecode,
