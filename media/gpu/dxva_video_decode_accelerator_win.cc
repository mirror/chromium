...
DXVAVideoDecodeAccelerator::GetSupportedProfiles(
  ...
  for (const auto& supported_profile : kSupportedProfiles) {
    ...
    if (kIsVPX && (!preferences.enable_accelerated_vpx_decode ||
                   workarounds.disable_accelerated_vpx_decode ||
                   max_vpx_resolutions.first.IsEmpty())) {
      continue;
    }
...

    {
      ...
      profile.max_resolution =
          kIsH264 ? max_vpx_resolutions.first : max_vpx_resolutions.first;
      profiles.push_back(profile);
    }

    const gfx::Size kPortraitMax =
        kIsH264 ? max_vpx_resolutions.second : max_vpx_resolutions.second;
