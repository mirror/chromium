...
// Returns a tuple of (LandscapeMax, PortraitMax). If landscape maximum could be
// computed, the value of |default_max| is returned for the landscape maximum
// and a zero size value is returned for portrait max (erring conservatively).
std::pair<gfx::Size, gfx::Size> GetMaxResolutionsForGUIDs(
    const gfx::Size& default_max,
    const std::vector<GUID>& valid_guids,
    const std::vector<gfx::Size>& resolutions_to_test) {
  TRACE_EVENT0("gpu,startup",
               "DXVAVideoDecodeAccelerator::GetMaxResolutionsForGUIDs");
  std::pair<gfx::Size, gfx::Size> result(default_max, gfx::Size());
  ...
  {
    TRACE_EVENT0("gpu,startup",
                 "GetMaxResolutionsForGUIDs. QueryDeviceObjectFromANGLE");
  ...
  {
    TRACE_EVENT0("gpu,startup", "GetMaxResolutionsForGUIDs. GUID search begin");
  ...
  {
    TRACE_EVENT0("gpu,startup",
                 "GetMaxResolutionsForGUIDs. Resolution search begin");

    for (const auto& res : resolutions_to_test) {
      if (!IsResolutionSupportedForDevice(res, decoder_guid,
                                          video_device.Get())) {
        break;
      }
      result.first = res;
    ...
    if (IsResolutionSupportedForDevice(flipped, decoder_guid,
                                       video_device.Get())) {
...
DXVAVideoDecodeAccelerator::GetSupportedProfiles(
  ...
  const std::pair<gfx::Size, gfx::Size> kMaxH264Resolutions =
      GetMaxResolutionsForGUIDs(gfx::Size(1920, 1088),
  ...
  const std::pair<gfx::Size, gfx::Size> kMaxVPXResolutions =
      GetMaxResolutionsForGUIDs(gfx::Size(4096, 2160),
  ...
  for (const auto& supported_profile : kSupportedProfiles) {
...

    {
      ...
      profile.max_resolution =
          kIsH264 ? kMaxH264Resolutions.first : kMaxVPXResolutions.first;
      profiles.push_back(profile);
    }

    const gfx::Size kPortraitMax =
        kIsH264 ? kMaxH264Resolutions.second : kMaxVPXResolutions.second;
