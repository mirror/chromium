...
void VaapiVideoDecodeAccelerator::OutputPictureOnTaskRunner(
    const scoped_refptr<VASurface>& va_surface,
    int32_t input_id,
  ...
  {
    TRACE_EVENT2("Video Decoder", "VAVDA::OutputPictureOnTaskRunner",
...
void VaapiVideoDecodeAccelerator::TryOutputSurfaceOnTaskRunner() {
  DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
  ...
  switch (state_) {
    case kIdle:
      ...
      decoder_thread_task_runner_->PostTask(
          FROM_HERE,
          base::Bind(&VaapiVideoDecodeAccelerator::DecodeOnTaskRunner,
...
bool VaapiVideoDecodeAccelerator::GetInputBufferOnTaskRunner_Locked() {
  DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
...
void VaapiVideoDecodeAccelerator::ReturnCurrInputBufferOnTaskRunner_Locked() {
  lock_.AssertAcquired();
  DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
...
// surfaces, and reschedule DecodeOnTaskRunner instead.
bool VaapiVideoDecodeAccelerator::WaitForSurfacesOnTaskRunner_Locked() {
  DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
  lock_.AssertAcquired();
...
void VaapiVideoDecodeAccelerator::DecodeOnTaskRunner() {
  DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
  TRACE_EVENT0("Video Decoder", "VAVDA::DecodeOnTaskRunner");
  base::AutoLock auto_lock(lock_);
  ...
  // of it.
  while (GetInputBufferOnTaskRunner_Locked()) {
    DCHECK(curr_input_buffer_.get());
    ...
    switch (res) {
      ...
      case AcceleratedVideoDecoder::kRanOutOfStreamData:
        ReturnCurrInputBufferOnTaskRunner_Locked();
        break;

      case AcceleratedVideoDecoder::kRanOutOfSurfaces:
        // No more output buffers in the decoder, try getting more or go to
        // sleep waiting for them.
        if (!WaitForSurfacesOnTaskRunner_Locked())
          return;
...
void VaapiVideoDecodeAccelerator::RecycleVASurfaceOnTaskRunner(
...
void VaapiVideoDecodeAccelerator::AssignPictureBuffers(
  ...
  // Resume DecodeOnTaskRunner if it is still in decoding state.
  if (state_ == kDecoding) {
    decoder_thread_task_runner_->PostTask(
...
void VaapiVideoDecodeAccelerator::ReusePictureBuffer(
  ...
  decoder_thread_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&VaapiVideoDecodeAccelerator::TryOutputSurfaceOnTaskRunner,
                 base::Unretained(this)));
}

void VaapiVideoDecodeAccelerator::FlushOnTaskRunner() {
  VLOGF(2) << __func__;
  DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
...
void VaapiVideoDecodeAccelerator::FinishFlushOnTaskRunner() {
  DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
...
void VaapiVideoDecodeAccelerator::ResetOnTaskRunner() {
  DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
  VLOGF(2) << __func__;
...
void VaapiVideoDecodeAccelerator::Reset() {
  ...
  decoder_thread_task_runner_->PostTask(
      FROM_HERE, base::Bind(&VaapiVideoDecodeAccelerator::ResetOnTaskRunner,
                            base::Unretained(this)));
...
void VaapiVideoDecodeAccelerator::FinishReset() {
  ...
  if (awaiting_va_surfaces_recycle_) {
    ...
    // finish the last DecodeOnTaskRunner, running at the time of Reset().
  ...
  // until we return something. Post a DecodeOnTaskRunner() so that we won't
...
bool VaapiVideoDecodeAccelerator::DecodeSurfaceOnTaskRunner(
...
void VaapiVideoDecodeAccelerator::SurfaceReadyOnTaskRunner(
    const scoped_refptr<VaapiDecodeSurface>& dec_surface) {
  DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
  DCHECK(!awaiting_va_surfaces_recycle_);
...
VaapiVideoDecodeAccelerator::CreateSurfaceOnTaskRunner() {
  ...
  scoped_refptr<VASurface> va_surface(new VASurface(
      ...
      base::Bind(&VaapiVideoDecodeAccelerator::RecycleVASurfaceOnTaskRunner,
                 base::Unretained(this))));
...
VaapiVideoDecodeAccelerator::VaapiH264Accelerator::CreateH264Picture() {
  scoped_refptr<VaapiDecodeSurface> va_surface =
      vaapi_dec_->CreateSurfaceOnTaskRunner();
  if (!va_surface)
    return nullptr;
...
bool VaapiVideoDecodeAccelerator::VaapiH264Accelerator::SubmitDecode(
  ...
  return vaapi_dec_->DecodeSurfaceOnTaskRunner(dec_surface);
}
...
bool VaapiVideoDecodeAccelerator::VaapiH264Accelerator::OutputPicture(
  ...
  vaapi_dec_->SurfaceReadyOnTaskRunner(dec_surface);
