...
void SharedSession::OnSinkGone() {
  ...
  // This will do nothing on the current session, but prevent the clients to
  // start any future remoting sessions.
...
mojom::RemotingStopReason SharedSession::GetLastStopReason() const {
  return stop_reason_;
}
