...
// thus the source is supported when the URL is either http or https, CORS
// header allows playback, video and audio codecs are supported by the remote
// playback device; HLS is supported on Android (which is not detected by the
// pipeline metadata atm).
#if defined(OS_ANDROID)
