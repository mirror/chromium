...
void RendererController::UpdateRemotePlaybackClientWithSource() {
  if (!remote_playback_client_proxy_)
    return;
...
// Chrome on Android (which is not detected by the pipeline metadata atm).
  ...
  remote_playback_client_proxy_->SourceChanged(url_after_redirects_,
                                               is_source_supported);
}
...
void RendererController::SetRemotePlaybackClientProxy(
    std::unique_ptr<WebRemotePlaybackClientProxy>
        remote_playback_client_proxy) {
  DCHECK(thread_checker_.CalledOnValidThread());
  DCHECK(remote_playback_client_proxy);
  DCHECK(!remote_playback_client_proxy_);

  remote_playback_client_proxy_ = std::move(remote_playback_client_proxy);
}
