...
void RendererController::OnDelayedStartTimerFired(StartTrigger start_trigger,
  ...
  double rate = (client_->VideoDecodedByteCount() - decoded_bytes) /
                kDelayedStart.InSecondsF();
  DCHECK_GE(rate, 0);
  // TODO(xjz): Gets the transmission capacity from Remoter.
  const double capacity = kTransmissionCapacity;
  metrics_recorder_.RecordVideoBitrateAndTransmissionCapacity(rate, capacity);
  if (rate <= capacity) {
    StartRemoting(start_trigger);
  } else {
    VLOG(1) << "Media remoting is not supported: bitrate(bytes/s)=" << rate
            << " transmission_capacity(bytes/s)=" << capacity;
