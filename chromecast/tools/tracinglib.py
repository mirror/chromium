...
class TracingBackendAndroid(object):
  ...
  def Connect(self, device_ip, devtools_port_unused=9222, timeout_unused=10):
    """Connect to cast_shell on given device and port.

    Args:
      device_ip: ADB serial for the device.
    """
  ...
  def StartTracing(self,
                   tracing_client_unused=None,
                   custom_categories=None,
                   record_continuously=False,
                   buffer_usage_reporting_interval_unused=0,
                   timeout_unused=10):
    """Begin a tracing session on device.
  ...
  def StopTracing(self, output_file, timeout_unused=30):
    """End a tracing session on device.
