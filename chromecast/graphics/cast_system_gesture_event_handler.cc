...
CastSideSwipeOrigin CastSystemGestureEventHandler::GetDragPosition(
  ...
  } else if (point.x() < (screen_bounds.x() + kSideGestureStartWidth)) {
    return CastSideSwipeOrigin::LEFT;
  } else if (point.x() > (screen_bounds.x() + screen_bounds.width() -
                          kSideGestureStartWidth)) {
    return CastSideSwipeOrigin::RIGHT;
  } else if (point.y() > (screen_bounds.y() + screen_bounds.height() -
                          kSideGestureStartHeight)) {
...
void CastSystemGestureEventHandler::RemoveSideSwipeGestureHandler(
    CastSideSwipeGestureHandlerInterface* handler) {
  SwipeGestureHandlersList::iterator find = std::find(
      swipe_gesture_handlers_.begin(), swipe_gesture_handlers_.end(), handler);
