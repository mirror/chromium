 ...
 *          mState = mController.watch((Unit unused) -> {
 *              Log.d(TAG, "onStart");
...
public class StateMachine {
    ...
    public static abstract class Observable<T> {
        ...
        public final <U> Observable<Both<T, U>> and(Observable<U> other) {
            return new PendingBothState<>(this, other).getState();
        }
        ...
        public final <U> Observable<Either<T, U>> or(Observable<U> other) {
            return new PendingEitherState<>(this, other).getState();
        }
    }
    ...
    private static class PendingBothState<A, B> {
        ...
        private State<Both<A, B>> getState() {
            return new State<>(mController, noSideEffects());
        }
    }
    ...
    private static class PendingEitherState<A, B> {
        ...
        private State<Either<A, B>> getState() {
            return new State<>(mController, noSideEffects());
        }
    }
}
