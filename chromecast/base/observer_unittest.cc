...
TEST_F(ObserverTest, NoExtraEventsForCopy) {
  ...
  Observer<int> observer2 = observer1;
  // New Observers always get an up-to-date value.
  ...
  // Propagate the SetValue events; the first observer should get an event
  // for the SetValue after it was created; the second observer should get no
  // events since SetValue wasn't called after it was created.
  base::RunLoop().RunUntilIdle();
  ...
  EXPECT_EQ(0u, event_values2.size());
}
...
TEST_F(ObserverTest, ObserverOnDifferentThread) {
  std::unique_ptr<ThreadedObservable> original(new ThreadedObservable());
  Observer<int> observer = original->Observe();
  EXPECT_EQ(0, observer.GetValue());
