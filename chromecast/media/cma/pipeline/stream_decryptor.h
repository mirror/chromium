...
class StreamDecryptor {
 public:
  // Returns false if errors happen during buffer push. Decryption error should
  // be reported in GetNextReadyBuffer.
  using PushBufferCompleteCB = base::RepeatingCallback<void(bool)>;

  StreamDecryptor() = default;
  virtual ~StreamDecryptor() = default;

  virtual void Init(const PushBufferCompleteCB& push_buffer_complete_cb) = 0;
  ...
  virtual void PushBuffer(scoped_refptr<DecoderBufferBase> buffer) = 0;
