...
class AudioSinkAudioTrackImpl {
    ...
    private long prepareForShutdown() {
        ...
        if (lastPlayoutTimeNsecs != NO_TIMESTAMP) {
            ...
            // We have no timestamp to estimate how much is left to play, so just assume the worst
            // case and assume the entire internal buffer is full.
            long bufferSizeInNsecs =
                    SEC_IN_NSEC * mAudioTrack.getBufferSizeInFrames() / mSampleRateInHz;
            playtimeLeftNsecs = bufferSizeInNsecs;
        }
