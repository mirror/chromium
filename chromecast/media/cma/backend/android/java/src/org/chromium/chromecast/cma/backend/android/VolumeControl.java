...
class VolumeControl {
    ...
    // Mapping from Cast's AudioContentType::{kMedia, kAlarm, kCommunication} to
    // AudioManager.STREAM_{MUSIC,ALARM,SYSTEM}.
    private SparseIntArray mAndroidTypeToCastTypeMap;

    // Mapping of AudioManager.STREAM_{MUSIC,ALARM,SYSTEM} to their respective Settings instance.
    private SparseArray<Settings> mSettings;

    /** Construction */
    @CalledByNative
    static VolumeControl createVolumeControl(long nativeVolumeControl, int castMediaType,
            int castAlarmType, int castCommunicationType) {
        Log.i(TAG,
                "Creating new VolumeControl instance (mediaType=" + castMediaType + " alarmType="
                        + castAlarmType + "communicationType=" + castCommunicationType + ")");
        return new VolumeControl(
                nativeVolumeControl, castMediaType, castAlarmType, castCommunicationType);
    }
     ...
     */
    private VolumeControl(long nativeVolumeControl, int castMediaType, int castAlarmType,
            int castCommunicationType) {
        mNativeVolumeControl = nativeVolumeControl;

        mContext = ContextUtils.getApplicationContext();
        mAudioManager = (AudioManager) mContext.getSystemService(Context.AUDIO_SERVICE);

        // Create the Cast's AudioContentType to Android's STREAM_* type mapping.
        mAndroidTypeToCastTypeMap = new SparseIntArray(3);
        mAndroidTypeToCastTypeMap.append(AudioManager.STREAM_MUSIC, castMediaType);
        mAndroidTypeToCastTypeMap.append(AudioManager.STREAM_ALARM, castAlarmType);
        mAndroidTypeToCastTypeMap.append(AudioManager.STREAM_SYSTEM, castCommunicationType);

        // Populate settings.
        mSettings = new SparseArray<Settings>(3);
        mSettings.append(castMediaType, new Settings(AudioManager.STREAM_MUSIC));
        mSettings.append(castAlarmType, new Settings(AudioManager.STREAM_ALARM));
        mSettings.append(castCommunicationType, new Settings(AudioManager.STREAM_SYSTEM));
    ...
    private void handleVolumeChange(int androidType) {
        int castType = mAndroidTypeToCastTypeMap.get(androidType);
