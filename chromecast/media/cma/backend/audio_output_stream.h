...
class AudioOutputStream {
 public:
  ...
  // Sets device name. The format of the name is platform-specific.
  ...
  // Start the stream. Caller must call GetSampleRate() to get actual sample
  // rate - it may be different from |requested_sample_rate|, e.g. if
  // IsFixedSampleRate() is true, or the device doesn't support
  ...
  // |data_size| indicates is size of data buffer. Should be divided by number
  // of channels to get number of frames.
  virtual bool Write(const float* data,
                     int data_size,
                     bool* out_playback_interrupted) = 0;

  // Stops the stream. It can be started again by calling Start().
  virtual void Stop() = 0;
};
