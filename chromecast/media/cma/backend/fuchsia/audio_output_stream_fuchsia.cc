...
// Same value as in AudioOutputStreamAlsa. Currently this value is used to
// simulate blocking Write() that ALSA uses, see comments in
// AudioOutputStreamFuchsia::Write().
constexpr int kMaxOutputBufferSizeFrames = 4096;

// static
std::unique_ptr<AudioOutputStream> AudioOutputStream::Create() {
  return std::make_unique<AudioOutputStreamFuchsia>();
}
...
bool AudioOutputStreamFuchsia::IsFixedSampleRate() {
  return false;
}

bool AudioOutputStreamFuchsia::Start(int requested_sample_rate, int channels) {
  DCHECK(!stream_);
...
int AudioOutputStreamFuchsia::GetSampleRate() {
  return sample_rate_;
}

MediaPipelineBackend::AudioDecoder::RenderingDelay
AudioOutputStreamFuchsia::GetRenderingDelay() {
...
bool AudioOutputStreamFuchsia::GetTimeUntilUnderrun(base::TimeDelta* result) {
  if (!started_time_.is_null()) {
...
bool AudioOutputStreamFuchsia::Write(const float* data,
                                     int data_size,
  ...
  do {
    ...
    if (result == ZX_ERR_IO_MISSED_DEADLINE) {
      LOG(ERROR) << "AudioOutputStreamFuchsia::PumpSamples() missed deadline, "
                    "resetting PTS.";
      if (!UpdatePresentatioDelay()) {
        return false;
      }
  ...
  // AudioOutputStreamAlsa uses blocking Write() and StreamMixer relies on that
