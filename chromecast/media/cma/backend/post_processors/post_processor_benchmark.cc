...
// where |start_x| and |end_x| are normalized frequencies (2 * freq_in_hz /
// sample_rate) i.e. 0 - DC, 1 - nyquist.
...
void AudioProcessorBenchmark(AudioPostProcessor* pp, int sample_rate) {
  ...
  std::vector<float> data_in =
      GetStereoChirp(test_size_frames, 0.0, 0.5, 0.0, 0.5);
  ...
  LOG(INFO) << "At " << sample_rate << "Hz CPU usage: "
