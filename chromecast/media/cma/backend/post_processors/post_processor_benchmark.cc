...
int GetUserTimeMs() {
  ...
  if (getrusage(RUSAGE_SELF, &usе_time) == 0) {
    return usе_time.ru_utime.tv_sec * 1000 +
           (usе_time.ru_utime.tv_usec + usе_time.ru_stime.tv_usec) / 1000;
  }
  return -1;
}
...
void AudioProcessorBenchmark(AudioPostProcessor* pp, int sample_rate) {
  ...
  int end_ms = GetUserTimeMs();
  LOG(INFO) << "At " << sample_rate << "frames per second CPU usage: "
            << 100.0 * (end_ms - start_ms) / kTestDurationMs << "%";
}
