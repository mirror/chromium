...
void TestDelay(AudioPostProcessor* pp, int sample_rate) {
  ...
  float search_range = expected_delay + 10;  // search_range is in frames
  for (int offset = 0; offset < search_range * kNumChannels;
       offset += kNumChannels) {
    float sum = 0.0;
    ...
    // Since we search for max, no need in x-correration normalization.
    for (int i = 0; i < upper_search_limit; ++i) {
      sum += data_in[i] * data_out[i + offset];
    }
  ...
  EXPECT_EQ(max_idx, kNumChannels * expected_delay);
}
