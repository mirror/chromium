...
class FilterGroupTest : public testing::Test {
 protected:
  FilterGroupTest() : input_(base::MakeUnique<MockInputQueue>()) {}
  ...
  void MakeFilterGroup(bool mix_to_mono,
    ...
    input_->set_filter_group(filter_group_.get());
...
TEST_F(FilterGroupTest, MonoMixer) {
  ...
  for (int i = 0; i < kInputFrames; ++i) {
    ...
    ASSERT_EQ((LeftInput(i) + RightInput(i)) / 2, interleaved_data[i * 2 + 1]);
  }
}
