...
StreamMixerAlsa::StreamMixerAlsa()
    : output_(AudioOutputStream::Create()),
  ...
  output_->SetDeviceName(device_name);
...
void StreamMixerAlsa::Start() {
  ...
  if (low_sample_rate_cutoff_ != kInvalidSampleRate &&
      requested_sample_rate < low_sample_rate_cutoff_) {
    requested_sample_rate = output_samples_per_second_ != kInvalidSampleRate
...
void StreamMixerAlsa::SetAudioOutputStreamForTest(
    std::unique_ptr<AudioOutputStream> output) {
  if (output_) {
    Close();
  }
...
void StreamMixerAlsa::AddInput(std::unique_ptr<InputQueue> input) {
  ...
  if (input->primary() && !output_->IsFixedSampleRate()) {
