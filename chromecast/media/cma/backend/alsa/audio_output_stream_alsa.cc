...
#include "chromecast/media/cma/backend/alsa/audio_output_stream_alsa.h"
...
// static
std::unique_ptr<AudioOutputStream> AudioOutputStream::Create() {
  return std::make_unique<AudioOutputStreamAlsa>();
}

AudioOutputStreamAlsa::AudioOutputStreamAlsa() {
...
void AudioOutputStreamAlsa::SetAlsaWrapperForTest(
    std::unique_ptr<AlsaWrapper> alsa) {
  DCHECK(!alsa_);
  alsa_ = std::move(alsa);
}

void AudioOutputStreamAlsa::SetDeviceName(const std::string& device_name) {
  alsa_device_name_ = device_name;
}

bool AudioOutputStreamAlsa::IsFixedSampleRate() {
  return fixed_sample_rate_ != kInvalidSampleRate;
}

bool AudioOutputStreamAlsa::Start(int sample_rate, int channels) {
  if (!alsa_) {
  ...
  if (!pcm_) {
    RETURN_FALSE_ON_ERROR(PcmOpen, &pcm_, alsa_device_name_.c_str(),
...
int AudioOutputStreamAlsa::GetSampleRate() {
  return sample_rate_;
}

MediaPipelineBackend::AudioDecoder::RenderingDelay
AudioOutputStreamAlsa::GetRenderingDelay() {
  return rendering_delay_;
}

bool AudioOutputStreamAlsa::GetTimeUntilUnderrun(base::TimeDelta* result) {
...
bool AudioOutputStreamAlsa::Write(const float* data,
                                  int data_size,
...
void AudioOutputStreamAlsa::Stop() {
  if (alsa_) {
...
int AudioOutputStreamAlsa::SetAlsaPlaybackParams(int requested_sample_rate) {
  int err = 0;
...
void AudioOutputStreamAlsa::DefineAlsaParameters() {
...
int AudioOutputStreamAlsa::DetermineOutputRate(int requested_sample_rate) {
  if (fixed_sample_rate_ != kInvalidSampleRate) {
...
void AudioOutputStreamAlsa::UpdateRenderingDelay(int newly_pushed_frames) {
