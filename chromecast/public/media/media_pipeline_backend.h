 ...
 public:
  ...
  class AudioDecryptor {
   public:
    ...
    // |kBufferPending| to stop caller to push more buffers. See comments of
    // Decoder::PushBuffer for more details on buffer pushing.
    //
    ...
    // the buffers.
    //
    // |buffer| and |output| are owned by caller. Caller won't destroy them
    // until Delegate::OnDecryptComplete is called. |output| is long enough to
    // hold clear data. |output| may overlap with the memory carried by
    // |buffer|.
  ...
  // owns the object. This will be multiple times. When the object is deleted,
  // the implementation should abort all the pending operations.
  // The function is optional. The correct implementation must return a valid
