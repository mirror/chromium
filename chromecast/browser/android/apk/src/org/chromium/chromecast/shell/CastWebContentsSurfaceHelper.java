...
class CastWebContentsSurfaceHelper {
    public static final String INTENT_SHOW_WEB_CONTENT = "com.google.assistant.SHOW_WEB_CONTENT";
    public static final String INTENT_STOP_WEB_CONTENT_INTERNAL =
            "com.google.assistant.STOP_WEB_CONTENT_INTERNAL";
    public static final String INTENT_STOP_WEB_CONTENT = "com.google.assistant.STOP_WEB_CONTENT";
    public static final String INTENT_WEB_CONTENT_STOPPED =
            "com.google.assistant.WEB_CONTENT_STOPPED";
    ...
    void onNewWebContents(
        ...
        if (mWindowDestroyedBroadcastReceiver != null) {
            LocalBroadcastManager.getInstance(getActivity())
                    .unregisterReceiver(mWindowDestroyedBroadcastReceiver);
        }
        ...
        if (!mShowInFragment) {
            windowDestroyedIntentFilter.addDataScheme(uri.getScheme());
            windowDestroyedIntentFilter.addDataAuthority(uri.getAuthority(), null);
        ...
        LocalBroadcastManager.getInstance(getActivity())
                .registerReceiver(mWindowDestroyedBroadcastReceiver, windowDestroyedIntentFilter);
        ...
        LocalBroadcastManager.getInstance(getActivity())
                .registerReceiver(mScreenOffBroadcastReceiver, screenOffIntentFilter);
        ...
        mInternalStopReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                in.setAction(INTENT_STOP_WEB_CONTENT);
                in.putExtra("content_uri",
                        CastWebContentsComponent.getInstanceUri(mInstanceId).toString());
                context.sendBroadcast(in);
            }
        };
        IntentFilter mediaStopReceiverFilter = new IntentFilter();
        mediaStopReceiverFilter.addAction(INTENT_STOP_WEB_CONTENT_INTERNAL);
        LocalBroadcastManager.getInstance(getActivity())
                .registerReceiver(mInternalStopReceiver, mediaStopReceiverFilter);
        ...
        mHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                if (currentInstanceId.equals(mInstanceId)) {
                    Log.d(TAG, "Finishing.");
                    if (mShowInFragment) {
                        getActivity().sendBroadcast(new Intent(INTENT_WEB_CONTENT_STOPPED));
                    } else {
    ...
    void onDestroy() {
        ...
        if (mInternalStopReceiver != null) {
            LocalBroadcastManager.getInstance(getActivity())
                    .unregisterReceiver(mInternalStopReceiver);
        }
    }
