    ...
    void onNewWebContents(
        ...
        mInternalStopReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (mInstanceId == null
                        || !mInstanceId.equals(intent.getStringExtra(
                                   CastWebContentsComponent.INTENT_EXTRA_CONTENT_ID))) {
                    Log.d(TAG,
                            "Current instance ID = " + mInstanceId + "; intent ID = "
                                    + intent.getStringExtra(
                                              CastWebContentsComponent.INTENT_EXTRA_CONTENT_ID));
                    return;
                }
                detachWebContentsIfAny();
                Intent in = new Intent(CastIntents.ACTION_ON_WEB_CONTENT_STOPPED);
                in.putExtra(CastWebContentsComponent.INTENT_EXTRA_CONTENT_ID, mInstanceId);
        ...
        LocalBroadcastManager.getInstance(getActivity())
                .registerReceiver(mInternalStopReceiver, internalStopReceiverFilter);
            ...
            public void run() {
                if (currentInstanceId.equals(mInstanceId)) {
                    ...
                    if (mShowInFragment) {
                        ...
                        in.setAction(CastIntents.ACTION_STOP_WEB_CONTENT);
                        in.putExtra(CastWebContentsComponent.INTENT_EXTRA_URI,
