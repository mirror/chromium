...
DefaultCommandLineSwitch kDefaultSwitches[] = {
#if defined(OS_ANDROID)
...
static const size_t kDefaultSwitchesCount =
    sizeof(kDefaultSwitches) / sizeof(*kDefaultSwitches);

void AddDefaultCommandLineSwitches(base::CommandLine* command_line) {
  for (size_t i = 0; i < kDefaultSwitchesCount; ++i) {
    std::string name(kDefaultSwitches[i].switch_name);
    ...
    if (!command_line->HasSwitch(name)) {
      std::string value(kDefaultSwitches[i].switch_value);
