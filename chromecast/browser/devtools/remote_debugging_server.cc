...
class RemoteDebuggingServer::WebContentsObserver : public content::WebContentsObserver {
 public:
  WebContentsObserver(content::WebContents* contents,
                      RemoteDebuggingServer* server) : server_(server) {
    Observe(contents);
  }
...
RemoteDebuggingServer::~RemoteDebuggingServer() {
  ...
  for (auto observer& : observers_)
    DisableWebContentsForDebugging(observer->web_contents());
}
...
void RemoteDebuggingServer::EnableWebContentsForDebugging(
  ...
  observers_.insert(std::make_pair(
      web_contents,
      std::make_unique<WebContentsObserver>(web_contents, this)));
}
