...
// Generate a global unique id inside a browser. The resultant id is
// concatenated by process id and a unique id within a process. For example, a
// frame's global unique id can be generated by process id and frame id.
PRINTING_EXPORT uint64_t GenGlobalUniqueId(int process_id, uint32_t id);

// Convert a sequence of content ids into their global unique ids inside a
// browser.
PRINTING_EXPORT std::vector<uint64_t> GenContentUniqueIds(
    int process_id,
