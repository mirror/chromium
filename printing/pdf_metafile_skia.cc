...
// Return a placeholder picture to be drawn on canvas, and later be replaced
// with the actual picture.
// This currectly serves the purpose, but should be replaced by a better option
// when available. https://bugs.chromium.org/p/skia/issues/detail?id=7536
sk_sp<SkPicture> DrawPlaceHolder(const gfx::Rect& rect) {
  SkPaint paint;
  ...
  SkCanvas* c = rec.beginRecording(sk_rect);
  // Number of ops needs to be more than |kMaxPictureOpsToUnrollInsteadOfRef|
  // (defined in third_party/skia/src/core/SkCanvas.cpp) so the placeholder
  // picture will be referenced instead of being played back during rastering.
  c->drawRect(sk_rect, paint);
  c->drawRect(sk_rect.makeInset(1, 1), paint);
...
//              other formats as well. Also change member variable names without
...
bool PdfMetafileSkia::FinishDocument() {
  ...
  cc::PlaybackParams params(nullptr);
  switch (data_->type_) {
    ...
    case SkiaDocumentType::MSKP:
      ...
      // The callback is only used inside this function.
      params.custom_callback = base::BindRepeating(
          &PdfMetafileSkia::MskpDrawingCallback, base::Unretained(this));
      break;
  }
  ...
  for (const Page& page : data_->pages_) {
    ...
    canvas.drawPicture(page.content_, params);
    doc->endPage();
  }
  doc->close();
...
bool PdfMetafileSkia::FinishFrameContent() {
  ...
  data_->pdf_data_ = stream.detachAsStream();
  data_->subframe_content_info.clear();
  return true;
}
...
std::unique_ptr<PdfMetafileSkia> PdfMetafileSkia::GetMetafileForCurrentPage(
  ...
  auto metafile = std::make_unique<PdfMetafileSkia>(
      type, data_->document_cookie, data_->page_number);
...
uint32_t PdfMetafileSkia::CreateContentForRemoteFrame(const gfx::Rect& rect,
  ...
  sk_sp<SkPicture> pic = DrawPlaceHolder(rect);
...
int PdfMetafileSkia::GetPageNumber() const {
  return data_->page_number;
}

void PdfMetafileSkia::MskpDrawingCallback(SkCanvas* canvas,
                                          uint32_t content_id) {
