...
bool PdfMetafileSkia::InitFromData(const void* src_buffer,
  ...
  data_->pdf_data_ = SkData::MakeWithCopy(src_buffer, src_buffer_size);
  return true;
}
...
bool PdfMetafileSkia::FinishDocument() {
  ...
  data_->pdf_data_ = stream.detachAsData();
  return true;
}
...
bool PdfMetafileSkia::GetData(void* dst_buffer,
  ...
  if (!data_->pdf_data_)
    return false;
  return data_->pdf_data_->copyRange(0, dst_buffer_size, dst_buffer);
}
