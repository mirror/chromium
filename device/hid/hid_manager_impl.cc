...
void HidManagerImpl::GetDevicesAndSetClient(
  ...
  hid_service_->GetDevices(
      base::Bind(&HidManagerImpl::CreateDeviceList, weak_factory_.GetWeakPtr(),
                 base::Passed(&callback), base::Passed(&client)));
}

void HidManagerImpl::GetDevices(GetDevicesCallback callback) {
  hid_service_->GetDevices(base::Bind(&HidManagerImpl::CreateDeviceList,
                                      weak_factory_.GetWeakPtr(),
                                      base::Passed(&callback), nullptr));
}
