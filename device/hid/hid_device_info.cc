...
// To unify the HidPlatformDeviceId in different OS, The HidPlatformDeviceId
// is saved in a std::map. The key is used as identification of that device.
std::map<HidDeviceId, HidPlatformDeviceId> g_device_id_mapping_;

HidDeviceId FindDeviceIdMappingInMap(
...
HidDeviceInfo::HidDeviceInfo(const HidPlatformDeviceId& platform_device_id,
    ...
    : device_id_(FindDeviceIdMappingInMap(platform_device_id)),
  ...
  if (device_id_ == kInvalidHidDeviceId) {
    ...
    g_device_id_mapping_[device_id_] = platform_device_id;
  }
...
// static
HidPlatformDeviceId HidDeviceInfo::GetHidPlatformDeviceIdFromMap(
    const HidDeviceId& device_id) {
  auto it = g_device_id_mapping_.find(device_id);
  return it != g_device_id_mapping_.end() ? it->second
                                          : kInvalidHidPlatformDeviceId;
}

// static
HidDeviceId HidDeviceInfo::RemoveDeviceIdMappingFromMap(
    const HidPlatformDeviceId& platform_device_id) {
  HidDeviceId device_id = FindDeviceIdMappingInMap(platform_device_id);
...
// static
void HidDeviceInfo::InitHidDeviceIDUniqueKey() {
  // Let the valid HidDeviceID start from 1.
  g_unique_key.GetNext();
}
