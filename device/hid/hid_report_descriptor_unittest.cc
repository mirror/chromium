...
class HidReportDescriptorTest : public testing::Test {
 ...
 protected:
  using HidPage = device::mojom::Page;
 ...
 public:
  void ValidateDetails(
    ...
    std::vector<HidCollectionInfoPtr>::const_iterator actual_collections_iter =
        actual_collections.begin();
    std::vector<HidCollectionInfoPtr>::const_iterator
        expected_collections_iter = expected_collections.begin();

    while (expected_collections_iter != expected_collections.end() &&
           actual_collections_iter != actual_collections.end()) {
      auto& expected_collection = *expected_collections_iter;
      auto& actual_collection = *actual_collections_iter;
...
TEST_F(HidReportDescriptorTest, ValidateDetails_Digitizer) {
  ...
  digitizer->usage = HidUsageAndPage::New(
      0x01, static_cast<uint16_t>(HidPage::kPageDigitizer));
...
TEST_F(HidReportDescriptorTest, ValidateDetails_Keyboard) {
  ...
  keyboard->usage = HidUsageAndPage::New(
      0x06, static_cast<uint16_t>(HidPage::kPageGenericDesktop));
  std::vector<HidCollectionInfoPtr> expected;
...
TEST_F(HidReportDescriptorTest, ValidateDetails_Monitor) {
  ...
  monitor->usage =
      HidUsageAndPage::New(0x01, static_cast<uint16_t>(HidPage::kPageMonitor0));
...
TEST_F(HidReportDescriptorTest, ValidateDetails_Mouse) {
  ...
  mouse->usage = HidUsageAndPage::New(
      0x02, static_cast<uint16_t>(HidPage::kPageGenericDesktop));
  std::vector<HidCollectionInfoPtr> expected;
...
TEST_F(HidReportDescriptorTest, ValidateDetails_LogitechUnifyingReceiver) {
  ...
  hidpp_short->usage =
      HidUsageAndPage::New(0x01, static_cast<uint16_t>(HidPage::kPageVendor));
  hidpp_short->report_ids.push_back(0x10);
  auto hidpp_long = HidCollectionInfo::New();
  hidpp_long->usage =
      HidUsageAndPage::New(0x02, static_cast<uint16_t>(HidPage::kPageVendor));
  hidpp_long->report_ids.push_back(0x11);
  auto hidpp_dj = HidCollectionInfo::New();
  hidpp_dj->usage =
      HidUsageAndPage::New(0x04, static_cast<uint16_t>(HidPage::kPageVendor));
