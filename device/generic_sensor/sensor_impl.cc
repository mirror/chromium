...
SensorImpl::SensorImpl(scoped_refptr<PlatformSensor> sensor)
    : sensor_(std::move(sensor)),
      reading_notification_disabled_(false),
      suspended_(false) {
...
void SensorImpl::DisableReadingChangeNotification() {
  reading_notification_disabled_ = true;
}

void SensorImpl::OnSensorReadingChanged() {
  if (reading_notification_disabled_ || suspended_)
    return;
