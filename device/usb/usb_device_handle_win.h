...
class UsbDeviceHandleWin : public UsbDeviceHandle {
 ...
 private:
  ...
  // |hub_handle_| or all the handles for claimed interfaces in |functions_|
  // must outlive their associated |requests_| because individual Request
  // objects hold on to handles for the purpose of calling
  // GetOverlappedResult().
  base::win::ScopedHandle hub_handle_;
  base::win::ScopedHandle function_handle_;

  std::unordered_map<uint8_t, Interface> interfaces_;
  std::unordered_map<uint8_t, Endpoint> endpoints_;
