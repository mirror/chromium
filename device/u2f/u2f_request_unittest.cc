...
TEST_F(U2fRequestTest, TestMultipleDiscoveries) {
  ...
  // successfully.
  auto discovery_1 = std::make_unique<MockU2fDiscovery>();
  auto discovery_2 = std::make_unique<MockU2fDiscovery>();

  EXPECT_CALL(*discovery_1, Start())
      .WillOnce(
          testing::Invoke(discovery_1.get(), &MockU2fDiscovery::StartSuccess));
  EXPECT_CALL(*discovery_2, Start())
      .WillOnce(
          testing::Invoke(discovery_2.get(), &MockU2fDiscovery::StartSuccess));

  FakeU2fRequest request(kTestRelyingPartyId);
  auto& discoveries = request.SetDiscoveriesForTesting(std::move(discovery_1),
                                                       std::move(discovery_2));
  request.Start();

  // Let each discovery find a device.
  auto* mock_discovery_1 = static_cast<MockU2fDiscovery*>(discoveries[0].get());
  auto* mock_discovery_2 = static_cast<MockU2fDiscovery*>(discoveries[1].get());
  ...
  mock_discovery_1->AddDevice(std::move(device_1));
  mock_discovery_2->AddDevice(std::move(device_2));
