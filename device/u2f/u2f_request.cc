...
U2fRequest::U2fRequest(std::vector<U2fDiscovery*> discoveries,
    ...
    : state_(State::INIT),
      ...
      cb_(std::move(cb)),
      weak_factory_(this) {
  std::for_each(discoveries_.begin(), discoveries_.end(),
                [this](auto* discovery) { discovery->AddObserver(this); });
}

U2fRequest::~U2fRequest() {
  std::for_each(discoveries_.begin(), discoveries_.end(),
                [this](auto* discovery) { discovery->RemoveObserver(this); });
}
...
void U2fRequest::Start() {
  if (state_ == State::INIT) {
    state_ = State::BUSY;
    std::for_each(discoveries_.begin(), discoveries_.end(),
                  [](auto* discovery) { discovery->Start(); });
  }
}
