...
struct U2fBleTransaction {
  ...
  U2fBleTransaction(U2fBleFrame::Command command,
                    std::vector<uint8_t> data,
                    size_t max_fragment_size)
      : frame(command, std::move(data)), initial(command, 0, nullptr, 0) {
    frame.ToFragments(max_fragment_size, &initial, &continuation);
  }
...
void U2fBleDevice::SendPing(std::vector<uint8_t> data, PingCallback callback) {
  U2fBleFrame frame(U2fBleFrame::Command::PING, std::move(data));
...
void U2fBleDevice::DeviceTransact(std::unique_ptr<U2fApduCommand> command,
  ...
  U2fBleFrame frame(U2fBleFrame::Command::MSG, command->GetEncodedCommand());
  SendFrame(
      std::move(frame),
      base::Bind(
...
void U2fBleDevice::OnMessage(std::vector<uint8_t> data) {
  ...
  if (!transaction_->frame_assembler) {
    auto fragment =
        U2fBleFrameInitializationFragment::Parse(data.data(), data.size());
    transaction_->frame_assembler.reset(
        new U2fBleFrameAssembler(std::move(fragment)));
  } else {
    auto fragment =
        U2fBleFrameContinuationFragment::Parse(data.data(), data.size());
  ...
  if (frame.command() == transaction_->frame.command()) {
    ...
    if (state_ != State::ERROR)
      state_ = State::READY;
    TrySend();
  } else if (frame.command() == U2fBleFrame::Command::KEEPALIVE) {
    VLOG(2) << "KEEPALIVE";
    transaction_->frame_assembler.reset(nullptr);
  } else {
    DCHECK_EQ(frame.command(), U2fBleFrame::Command::ERROR);
    state_ = State::ERROR;
  }
}
...
void U2fBleDevice::SendFrame(U2fBleFrame frame, MessageCallback callback) {
  ...
  connection_->Write(
      transaction_->initial.Serialize(),
      base::Bind(&U2fBleDevice::OnFragmentSent, weak_factory_.GetWeakPtr()));
}
...
void U2fBleDevice::OnFragmentSent(bool success) {
  ...
  if (transaction_->not_finished()) {
    const auto& fragment = transaction_->next_fragment();
    connection_->Write(
        fragment.Serialize(),
        base::Bind(&U2fBleDevice::OnFragmentSent, weak_factory_.GetWeakPtr()));
    return;
