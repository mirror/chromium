...
U2fBleDevice::U2fBleDevice(std::string address) : weak_factory_(this) {
  connection_ = std::make_unique<U2fBleConnection>(
      std::move(address),
      base::BindRepeating(&U2fBleDevice::OnConnected,
...
void U2fBleDevice::OnControlPointLengthRead(base::Optional<uint16_t> length) {
  if (!length) {
...
void U2fBleDevice::SendFrame(U2fBleFrame frame, MessageCallback callback) {
  ...
  connection_->WriteControlPoint(buffer_,
                                 base::BindOnce(&U2fBleDevice::OnFragmentSent,
                                                weak_factory_.GetWeakPtr()));
}
