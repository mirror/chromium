...
std::vector<uint8_t> to_byte_vector(base::StringPiece str) {
...
BluetoothRemoteGattService* GetMockService(const MockBluetoothDevice& device,
                                           const std::string& identifier) {
  const std::vector<BluetoothRemoteGattService*> services =
      device.GetMockServices();
  auto found = std::find_if(services.begin(), services.end(),
                            [&identifier](const auto* service) {
                              return service->GetIdentifier() == identifier;
                            });
  return found != services.end() ? *found : nullptr;
}

BluetoothRemoteGattCharacteristic* GetMockCharacteristic(
    const MockBluetoothGattService& service,
    const std::string& identifier) {
  const std::vector<BluetoothRemoteGattCharacteristic*> characteristics =
      service.GetMockCharacteristics();
  auto found =
      std::find_if(characteristics.begin(), characteristics.end(),
                   [&identifier](const auto* characteristic) {
                     return characteristic->GetIdentifier() == identifier;
                   });
  return found != characteristics.end() ? *found : nullptr;
}
...
class TestReadCallback {
 public:
  void OnRead(std::vector<uint8_t> value) { value_ = std::move(value); }

  const std::vector<uint8_t> WaitForCallback() {
    base::RunLoop().RunUntilIdle();
    return value_;
  }
 ...
 public:
  ...
  void AddU2Device(const std::string& device_address) {
    ...
    ON_CALL(*u2f_device_, GetGattService(_))
        .WillByDefault(Invoke([this](const auto& identifier) {
          return GetMockService(*u2f_device_, identifier);
        }));
    AddU2fService();
  }

  void ExpectConnectingU2fDevice(const std::string& device_address) {
  ...
  void DisconnectU2fDevice(const std::string& device_address) {
    if (u2f_device_->GetAddress() != device_address)
      return;

    EXPECT_CALL(*u2f_device_, IsGattConnected()).WillOnce(Return(false));
    adapter_->NotifyDeviceChanged(u2f_device_);
  }

  void ChangeDeviceAddress(const std::string& old_address,
                           const std::string& new_address) {
    if (!u2f_device_ || u2f_device_->GetAddress() != old_address)
      return;

    ON_CALL(*u2f_device_, GetAddress())
        .WillByDefault(testing::Return(new_address));
  ...
  void AddU2fService() {
    ...
    ON_CALL(*u2f_service_, GetCharacteristic(_))
        .WillByDefault(Invoke([this](const auto& identifier) {
          return GetMockCharacteristic(*u2f_service_, identifier);
        }));
...
TEST_F(U2fBleConnectionTest, Address) {
  ...
  ChangeDeviceAddress(device_address, "new_device_address");
...
TEST_F(U2fBleConnectionTest, PreConnected) {
  ...
  TestConnectCallback connect_callback;
  AddU2Device(device_address);
  ExpectConnectingU2fDevice(device_address);
...
TEST_F(U2fBleConnectionTest, DeviceDisconnect) {
  ...
  DisconnectU2fDevice(device_address);
  EXPECT_FALSE(connect_callback.WaitForCallback());
}
...
TEST_F(U2fBleConnectionTest, ReadStatusNotifications) {
  ...
  std::vector<uint8_t> payload = to_byte_vector("foo");
  NotifyStatusChanged(payload);
  EXPECT_EQ(payload, read_callback.WaitForCallback());

  payload = to_byte_vector("bar");
  NotifyStatusChanged(payload);
  EXPECT_EQ(payload, read_callback.WaitForCallback());
}
...
TEST_F(U2fBleConnectionTest, ReadControlPointLength) {
  ...
  // EXPECT_FALSE() for payloads of size 0, 1 and 3.
...
TEST_F(U2fBleConnectionTest, ReadServiceRevisions) {
  ...
  SetNextReadServiceRevisionResponse(true, to_byte_vector("bogus"));
  SetNextReadServiceRevisionBitfieldResponse(false, {});
  connection.ReadServiceRevisions(revisions_callback.GetCallback());
  EXPECT_THAT(revisions_callback.WaitForCallback(), IsEmpty());

  SetNextReadServiceRevisionResponse(true, to_byte_vector("1.0"));
  SetNextReadServiceRevisionBitfieldResponse(false, {});
  connection.ReadServiceRevisions(revisions_callback.GetCallback());
  EXPECT_THAT(revisions_callback.WaitForCallback(),
              ElementsAre(U2fBleConnection::ServiceRevision::VERSION_1_0));

  SetNextReadServiceRevisionResponse(true, to_byte_vector("1.1"));
  SetNextReadServiceRevisionBitfieldResponse(false, {});
  connection.ReadServiceRevisions(revisions_callback.GetCallback());
  EXPECT_THAT(revisions_callback.WaitForCallback(),
              ElementsAre(U2fBleConnection::ServiceRevision::VERSION_1_1));

  SetNextReadServiceRevisionResponse(true, to_byte_vector("1.2"));
  SetNextReadServiceRevisionBitfieldResponse(false, {});
  connection.ReadServiceRevisions(revisions_callback.GetCallback());
  EXPECT_THAT(revisions_callback.WaitForCallback(),
  ...
  // error.
  SetNextReadServiceRevisionResponse(true, to_byte_vector("1.3"));
  SetNextReadServiceRevisionBitfieldResponse(false, {});
  connection.ReadServiceRevisions(revisions_callback.GetCallback());
  EXPECT_THAT(revisions_callback.WaitForCallback(), IsEmpty());
