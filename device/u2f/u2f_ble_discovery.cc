...
// TODO(763303): Remove this class once a U2fDevice for BLE is implemented.
...
const BluetoothUUID U2fBleDiscovery::kServiceUUID(U2F_SERVICE_UUID);
...
void U2fBleDiscovery::OnSetPowered() {
  ...
  for (BluetoothDevice* device : adapter_->GetDevices()) {
    if (base::ContainsKey(device->GetUUIDs(), kServiceUUID)) {
