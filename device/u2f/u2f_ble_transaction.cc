...
void U2fBleTransaction::WriteRequestFrame(
    U2fBleFrame request_frame,
    U2fBleDevice::FrameCallback callback) {
  DCHECK(!request_frame_);
  request_frame_ = std::move(request_frame);
  callback_ = std::move(callback);

  std::tie(request_init_fragment_, request_cont_fragments_) =
      request_frame_->ToFragments(control_point_length_);
  buffer_.reserve(control_point_length_);
  WriteRequestFragment(request_init_fragment_);
}
...
void U2fBleTransaction::WriteRequestFragment(
  ...
  // WriteRequestFragment() expects an invokation of OnFragmentWritten() soon
  // after.
  StartTimeout();
}
