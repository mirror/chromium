...
TEST(U2fHidMessageTest, TestPacketSize) {
  uint32_t channel_id = 0x05060708;
  std::vector<uint8_t> data;

  auto init_packet = std::make_unique<U2FHidInitPacket>(
  ...
  auto continuation_packet =
      std::make_unique<U2FHidContinuationPacket>(channel_id, 0, data);
...
TEST(U2fHidMessageTest, TestPacketConstructors) {
  uint32_t channel_id = 0x05060708;
  std::vector<uint8_t> data{10, 11};
  CTAPHIDDeviceCommand cmd = CTAPHIDDeviceCommand::kCtapHidWink;
  auto orig_packet =
  ...
  auto reconstructed_packet =
      U2FHidInitPacket::CreateFromSerializedData(orig_data, &payload_length);
...
TEST(U2fHidMessageTest, TestMaxLengthPacketConstructors) {
  ...
  auto new_msg = U2FHidMessage::CreateFromSerializedData(msg_data);
  it++;
...
TEST(U2fHidMessageTest, TestMessagePartitoning) {
  uint32_t channel_id = 0x01010203;
  std::vector<uint8_t> data(kHIDInitPacketDataSize + 1);
  auto two_packet_message = U2FHidMessage::Create(
  ...
  auto one_packet_message = U2FHidMessage::Create(
      channel_id, CTAPHIDDeviceCommand::kCtapHidPing, data);
...
TEST(U2fHidMessageTest, TestDeserialize) {
  ...
  auto new_message = U2FHidMessage::CreateFromSerializedData(buf);
