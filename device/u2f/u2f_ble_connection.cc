...
void U2fBleConnection::WriteServiceRevision(ServiceRevision service_revision,
  ...
  device_->WriteValueForCharacteristic(
      ...
      base::BindOnce(&U2fBleConnection::OnWrite, std::move(callback)));
}
...
void U2fBleConnection::OnReadServiceRevisionBitfied(
  ...
  if (bitset & 0x3F) {
    LOG(WARNING) << "Service Revision Bitfield has unexpected bits set: "
                 << (bitset & 0x3F) << ". Ignoring all but the first two bits.";
  }
...
void U2fBleConnection::ReturnServiceRevisions(
    ServiceRevisionsCallback callback) const {
  std::move(callback).Run(service_revisions_);
}
