...
void U2fBleConnection::OnCreateGattConnectionError(
  ...
  DLOG(ERROR) << "CreateGattConnection() failed: " << to_string(error_code);
  OnConnectionError();
}
...
void U2fBleConnection::OnStartNotifySessionError(
    BluetoothGattService::GattErrorCode error_code) {
  DLOG(ERROR) << "StartNotifySession() failed: " << to_string(error_code);
  OnConnectionError();
}
...
void U2fBleConnection::OnReadControlPointLengthError(
    ControlPointLengthCallback callback,
    BluetoothGattService::GattErrorCode error_code) {
  DLOG(ERROR) << "Error reading Control Point Length: "
...
void U2fBleConnection::OnReadServiceRevisionError(
    base::OnceClosure callback,
    BluetoothGattService::GattErrorCode error_code) {
  DLOG(ERROR) << "Error reading Service Revision: " << to_string(error_code);
  std::move(callback).Run();
}
...
void U2fBleConnection::OnWriteError(
    WriteCallback callback,
    BluetoothGattService::GattErrorCode error_code) {
  DLOG(ERROR) << "Write Failed: " << to_string(error_code);
  std::move(callback).Run(false);
}
