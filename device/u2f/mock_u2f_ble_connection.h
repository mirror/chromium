...
class MockU2fBleConnection : public U2fBleConnection {
 public:
  ...
  MOCK_METHOD1(ReadControlPointLengthRef, void(ControlPointLengthCallback& cb));
  MOCK_METHOD1(ReadServiceRevisionsRef, void(ServiceRevisionsCallback& cb));
  MOCK_METHOD2(WriteControlPointRef,
               void(const std::vector<uint8_t>& data, WriteCallback& cb));
  MOCK_METHOD2(WriteServiceRevisionRef,
