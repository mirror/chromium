...
TEST_F(U2fMessageTest, TestPacketData) {
  ...
  EXPECT_EQ((channel_id >> 24) & 0xff,
            static_cast<uint8_t>(serialized[index++]));
  EXPECT_EQ((channel_id >> 16) & 0xff,
            static_cast<uint8_t>(serialized[index++]));
  EXPECT_EQ((channel_id >> 8) & 0xff,
            static_cast<uint8_t>(serialized[index++]));
  EXPECT_EQ(channel_id & 0xff, static_cast<uint8_t>(serialized[index++]));
  EXPECT_EQ(cmd, static_cast<uint8_t>(serialized[index++]));

  EXPECT_EQ(data.size() >> 8, static_cast<uint8_t>(serialized[index++]));
  EXPECT_EQ(data.size() & 0xff, static_cast<uint8_t>(serialized[index++]));
