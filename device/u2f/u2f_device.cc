...
void U2fDevice::Register(const std::vector<uint8_t>& app_param,
  ...
  std::unique_ptr<apdu::APDUCommand> register_cmd =
      apdu::APDUCommand::CreateU2FRegister(app_param, challenge_param,
...
void U2fDevice::Sign(const std::vector<uint8_t>& app_param,
                     ...
                     MessageCallback callback,
                     bool check_only) {
  std::unique_ptr<apdu::APDUCommand> sign_cmd =
      apdu::APDUCommand::CreateU2FSign(app_param, challenge_param, key_handle,
                                       check_only);
  if (!sign_cmd) {
    std::move(callback).Run(U2fReturnCode::INVALID_PARAMS,
                            std::vector<uint8_t>());
...
void U2fDevice::Version(VersionCallback callback) {
  std::unique_ptr<apdu::APDUCommand> version_cmd =
      apdu::APDUCommand::CreateU2FVersion();
  if (!version_cmd) {
...
void U2fDevice::OnVersionComplete(
  ...
  if (success && version_response &&
    ...
    auto version_cmd = apdu::APDUCommand::CreateU2FLegacyVersion();
    if (!version_cmd) {
      std::move(callback).Run(false, ProtocolVersion::UNKNOWN);
    } else {
      DeviceTransact(std::move(version_cmd),
