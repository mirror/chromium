...
class U2fDeviceEnumerate {
 public:
  explicit U2fDeviceEnumerate(service_manager::Connector* connector)
      : closure_(),
    ...
    for (auto& device_info : devices) {
      if (filter_.Matches(*device_info))
        u2f_devices.push_front(
            std::make_unique<U2fHidDevice>(std::move(device_info), connector_));
    }
...
TEST_F(U2fHidDeviceTest, TestHidDeviceVersion) {
  ...
  U2fDeviceEnumerate callback(connector_.get());
