...
// Returns byte HID_INIT request to send to device with mock connection.
...
// Returns "U2F_v2" as response to version request with given channel id.
...
TEST_F(U2fHidDeviceTest, TestRetryChannelAllocation) {
  const std::vector<uint8_t> incorrect_nonce = {0x00, 0x00, 0x00, 0x00,
                                                0x00, 0x00, 0x00, 0x00};
  ...
  EXPECT_CALL(mock_connection, WritePtr(_, _, _))
      .WillOnce(WithArgs<1, 2>(
          Invoke([&](const std::vector<uint8_t> buffer,
            ...
            mock_connection.SetNonce(
                std::vector<uint8_t>(buffer.begin() + 7, buffer.begin() + 15));
  ...
  EXPECT_CALL(mock_connection, ReadPtr(_))
      .WillOnce(WithArg<0>(
          Invoke([incorrect_nonce, &mock_connection](
                     device::mojom::HidConnection::ReadCallback& cb) {
            std::move(cb).Run(
                true, 0,
                CreateMockHidInitResponse(
                    incorrect_nonce, mock_connection.connection_channel_id()));
          })))
