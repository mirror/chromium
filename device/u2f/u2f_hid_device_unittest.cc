...
#include "fake_hid_impl_for_testing.h"
...
TEST_F(U2fHidDeviceTest, TestHidDeviceVersion) {
  ...
  U2fDeviceEnumerate callback(hid_manager.get());
  hid_manager->GetDevices(callback.callback());
  std::list<std::unique_ptr<U2fHidDevice>>& u2f_devices =
...
TEST_F(U2fHidDeviceTest, TestConnectionFailure) {
  ...
  device::mojom::HidDeviceInfoPtr hid_device =
  ...
  fake_hid_manager_impl.AddDevice(std::move(hid_device));
  hid_manager->GetDevices(callback.callback());
...
TEST_F(U2fHidDeviceTest, TestDeviceError) {
  ...
  FakeHidConnectionImpl::mock_connection_error_ = true;
  device->state_ = U2fHidDevice::State::IDLE;
  ...
  device->DeviceTransact(U2fApduCommand::CreateVersion(),
                         base::Bind(&ResponseCallback, &response3));
  FakeHidConnectionImpl::mock_connection_error_ = false;
