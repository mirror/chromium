 ...
 public:
  class MockObserver : public Observer {
   public:
    ...
    MOCK_METHOD1(OnDeviceKnownStr, void(const std::string&));
    ...
    void OnDeviceKnown(std::unique_ptr<U2fDevice> device) override;
    void OnDeviceRemoved(base::StringPiece device_id) override;

    base::WeakPtr<MockObserver> GetWeakPtr();

   private:
    base::WeakPtrFactory<MockObserver> weak_factory_;
  };
  ...
  void KnownDevice(std::unique_ptr<U2fDevice> device);
