...
U2FHidPacket::U2FHidPacket(std::vector<uint8_t> data, uint32_t channel_id)
    : data_(std::move(data)), channel_id_(channel_id) {}

U2FHidPacket::U2FHidPacket() = default;

U2FHidPacket::~U2FHidPacket() = default;

// static
std::unique_ptr<U2FHidInitPacket> U2FHidInitPacket::CreateFromSerializedData(
  ...
  *remaining_size = payload_size - data_size;
  ...
  return std::make_unique<U2FHidInitPacket>(channel_id, *command,
...
// 7      (s-7)   Payload data
U2FHidInitPacket::U2FHidInitPacket(uint32_t channel_id,
                                   CTAPHIDDeviceCommand cmd,
                                   std::vector<uint8_t> data,
                                   uint16_t payload_length)
    : U2FHidPacket(std::move(data), channel_id),
      command_(cmd),
      payload_length_(payload_length) {}

U2FHidInitPacket::~U2FHidInitPacket() = default;

std::vector<uint8_t> U2FHidInitPacket::GetSerializedData() const {
...
// static
std::unique_ptr<U2FHidContinuationPacket>
U2FHidContinuationPacket::CreateFromSerializedData(
    base::span<const uint8_t> serialized,
    size_t* remaining_size) {
  if (!remaining_size || serialized.size() != kHIDPacketSize)
    return nullptr;
  ...
  *remaining_size -= data_size;
  ...
  return std::make_unique<U2FHidContinuationPacket>(channel_id, sequence,
                                                    std::move(data));
}
...
// 5      (s-5)   Payload data
U2FHidContinuationPacket::U2FHidContinuationPacket(const uint32_t channel_id,
                                                   const uint8_t sequence,
                                                   std::vector<uint8_t> data)
    : U2FHidPacket(std::move(data), channel_id), sequence_(sequence) {}

U2FHidContinuationPacket::~U2FHidContinuationPacket() = default;

std::vector<uint8_t> U2FHidContinuationPacket::GetSerializedData() const {
  std::vector<uint8_t> serialized;
  serialized.reserve(kHIDPacketSize);
  serialized.push_back((channel_id_ >> 24) & 0xff);
  serialized.push_back((channel_id_ >> 16) & 0xff);
