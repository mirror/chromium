...
TEST_F(U2fHidDiscoveryTest, TestAddRemoveDevice) {
  ...
  EXPECT_CALL(observer, OnDeviceAddedStr(device_id_added));
  ...
  EXPECT_CALL(observer, OnDeviceAddedStr(testing::_)).Times(0);
  fake_hid_manager_->AddDevice(std::move(other_device));

  // Removed non-U2F devices should not be reported at all.
  EXPECT_CALL(observer, OnDeviceRemovedStr(testing::_)).Times(0);
  fake_hid_manager_->RemoveDevice("other");
  scoped_task_environment().RunUntilIdle();

  // Removed U2F devices should be reported as REMOVED.
  EXPECT_CALL(observer, OnDeviceRemovedStr(device_id_known));
  EXPECT_CALL(observer, OnDeviceRemovedStr(device_id_added));
