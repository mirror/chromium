...
TEST_F(U2fRegisterTest, TestSingleDeviceRegistrationWithExclusionList) {
  ...
  EXPECT_CALL(*device.get(), GetId())
      .WillRepeatedly(testing::Return("device0"));
  // DeviceTransact() function will be called four times including three check
  ...
  // create the three key handles provided in exclude list. At the fourth call,
  // MockU2fDevice::NoErrorRegister will be invoked after registration.
  EXPECT_CALL(*device.get(), DeviceTransactPtr(_, _))
      .Times(4)
  ...
  // and TryWink() is called again before Register() function is called.
  EXPECT_CALL(*device.get(), TryWink(_))
      .WillOnce(testing::Invoke(MockU2fDevice::WinkDoNothing))
      .WillOnce(testing::Invoke(MockU2fDevice::WinkDoNothing));
  EXPECT_CALL(*discovery, Start())
...
TEST_F(U2fRegisterTest, TestMultipleDeviceRegistrationWithExclusionList) {
  ...
  // DeviceTransact() function will be called four times. First three times to
  // check for duplicate key handles and the fourth time for registration.
  // Since user will register using "device1", the fourth call will invoke
  ...
  // DeviceTransact() function will invoke MockU2fDevice::NoErrorRegister after
...
// with duplicate key in exclude list. We assume that the duplicate key is the
// last key handle in the exclude list. Therefore, after duplicate key handle is
// found, the process is expect to terminate after calling bogus registration
// which checks for user presence.
TEST_F(U2fRegisterTest, TestSingleDeviceRegistrationWithDuplicateHandle) {
  ...
  // before actual Register() function is called on the device. Therefore,
  // TryWink() function is invoked once.
  EXPECT_CALL(*device.get(), TryWink(_))
      .WillOnce(testing::Invoke(MockU2fDevice::WinkDoNothing));
...
TEST_F(U2fRegisterTest, TestMultipleDeviceRegistrationWithDuplicateHandle) {
  ...
  // exclude list, we expect that check only sign() function should be called
  ...
  // that the first three calls to check only sign() function invoke
