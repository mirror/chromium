...
void U2fHidDiscovery::Stop() {
  binding_.Unbind();
  if (observer_)
    observer_->OnStopped(true);
}

void U2fHidDiscovery::DeviceAdded(device::mojom::HidDeviceInfoPtr device_info) {
  // Ignore non-U2F devices
  if (observer_ && filter_.Matches(*device_info)) {
    observer_->OnDeviceAdded(std::make_unique<U2fHidDevice>(
...
void U2fHidDiscovery::DeviceRemoved(
  ...
  // Ignore non-U2F devices
  if (observer_ && filter_.Matches(*device_info)) {
    observer_->OnDeviceRemoved("hid:" + device_info->guid);
  }
}
  ...
  if (observer_) {
    for (auto& device_info : device_infos) {
      if (filter_.Matches(*device_info)) {
        observer_->OnDeviceKnown(std::make_unique<U2fHidDevice>(
            std::move(device_info), hid_manager_.get()));
      }
    }
