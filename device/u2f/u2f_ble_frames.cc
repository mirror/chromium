...
bool U2fBleFrameInitializationFragment::Parse(
  ...
  const auto command = static_cast<U2fBleFrame::Command>(data[0]);
  const uint16_t data_length = (static_cast<uint16_t>(data[1]) << 8) + data[2];
  if (data_length + 3 < data.size())
    return false;
