...
class U2fHidDevice : public U2fDevice {
 ...
 private:
  ...
  using U2fHidMessageCallback =
      base::OnceCallback<void(bool, std::unique_ptr<U2FHidMessage>)>;
  ...
  void OnReadContinuation(std::unique_ptr<U2FHidMessage> message,
                          U2fHidMessageCallback callback,
                          bool success,
                          uint8_t report_id,
                          const base::Optional<std::vector<uint8_t>>& buf);
  void OnWink(WinkCallback callback,
              bool success,
              std::unique_ptr<U2FHidMessage> response);
