...
TEST_F(BluetoothTest, DiscoverKnownDevice) {
  ...
  auto* device = SimulateLowEnergyDevice(7);
...

  {
    ...
    EXPECT_CALL(delegate, OnDeviceAddedStr("ble:" + device->GetAddress()));
  ...
  // TODO: Delete device and check OnDeviceDeleted invocation.
...
TEST_F(BluetoothTest, DiscoverNewDevice) {
  if (!PlatformSupportsLowEnergy()) {
    LOG(WARNING) << "Low Energy Bluetooth unavailable, skipping unit test.";
    return;
  }
