...
UdevGamepad::UdevGamepad(Type type,
                         int index,
...
// static
std::unique_ptr<UdevGamepad> UdevGamepad::Create(udev_device* dev) {
  ...
  for (const auto& entry : device_roots) {
    ...
    UdevGamepad* pad_info =
        new UdevGamepad(node_type, index_value, node_path, syspath_prefix);
    return std::unique_ptr<UdevGamepad>(pad_info);
  }
