...
bool GamepadDeviceLinux::IsSameDevice(const UdevGamepad& pad_info) {
  return pad_info.syspath_prefix == syspath_prefix_;
}

bool GamepadDeviceLinux::OpenJoydevNode(const UdevGamepad& pad_info,
                                        udev_device* device) {
  DCHECK(pad_info.type == UdevGamepad::Type::JOYDEV);
...
bool GamepadDeviceLinux::OpenEvdevNode(const UdevGamepad& pad_info) {
  DCHECK(pad_info.type == UdevGamepad::Type::EVDEV);
  DCHECK(pad_info.syspath_prefix == syspath_prefix_);
...
bool GamepadDeviceLinux::OpenHidrawNode(const UdevGamepad& pad_info) {
  DCHECK(pad_info.type == UdevGamepad::Type::HIDRAW);
  DCHECK(pad_info.syspath_prefix == syspath_prefix_);
