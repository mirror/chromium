...
const uint8_t kRumbleMagnitudeMaxDualshock = 0xff;

const int kVendorSony = 0x054c;
const int kProductDualshock4 = 0x05c4;
const int kProductDualshock4Slim = 0x09cc;
...
bool IsUdevGamepad(udev_device* dev, device::UdevGamepad* pad_info) {
  ...
  static const std::vector<DeviceRootPair> device_roots = {
      ...
      {device::UdevGamepadType::HIDRAW, "/dev/hidraw"},
  };
  ...
  for (const auto& entry : device_roots) {
    ...
    // If the device is in the /dev/input subsystem but does not have the
    // ID_INPUT_JOYSTICK property, ignore it.
    if ((node_type == device::UdevGamepadType::EVDEV ||
         node_type == device::UdevGamepadType::JOYDEV) &&
        !device::udev_device_get_property_value(dev, "ID_INPUT_JOYSTICK"))
      return false;
...
bool SyspathsDescribeSameDevice(const std::string& syspath1,
                                const std::string& subsystem1,
                                const std::string& syspath2,
                                const std::string& subsystem2) {
  // Syspaths from different subsystems describe the same device if they are
  // identical up to the subsystem node.
  size_t subsystem_start = syspath1.find(subsystem1);
  if (subsystem_start == std::string::npos ||
      syspath2.find(subsystem2) != subsystem_start)
    return false;

  return syspath1.compare(0, subsystem_start, syspath2, 0, subsystem_start) ==
         0;
}
...
bool IsDualshock4(int vendor_id, int product_id) {
  if (vendor_id != kVendorSony)
    return false;

  return product_id == kProductDualshock4 ||
         product_id == kProductDualshock4Slim;
}
...
UdevGamepad::UdevGamepad() = default;
UdevGamepad::~UdevGamepad() = default;
...
GamepadPlatformDataFetcherLinux::GamepadPlatformDataFetcherLinux() {
  ...
  std::fill(joydev_to_hidraw_, joydev_to_hidraw_ + Gamepads::kItemsLengthCap,
            -1);
...
GamepadPlatformDataFetcherLinux::~GamepadPlatformDataFetcherLinux() {
  ...
  for (const auto& entry : hidraw_devices_) {
    const HidrawDeviceInfo& hidraw_info = *entry.second.get();
    CloseFileDescriptorIfValid(hidraw_info.fd);
  }
}
...
void GamepadPlatformDataFetcherLinux::RefreshDevice(udev_device* dev) {
  ...
  if (IsUdevGamepad(dev, &pad_info)) {
    ...
    else if (pad_info.type == UdevGamepadType::HIDRAW)
...
void GamepadPlatformDataFetcherLinux::RefreshJoydevDevice(
  ...
  int hidraw_match = joydev_to_hidraw_[js_index];

  if (pad_info.parent_syspath.empty()) {
    joydev_to_evdev_[js_index] = -1;
    joydev_to_hidraw_[js_index] = -1;
  ...
  if (hidraw_match < 0) {
    for (const auto& entry : hidraw_devices_) {
      const HidrawDeviceInfo& hidraw_info = *entry.second.get();
      if (SyspathsDescribeSameDevice(hidraw_info.syspath, kHidrawSubsystem,
                                     pad_info.parent_syspath,
                                     kInputSubsystem)) {
        hidraw_match = entry.first;
        break;
      }
    }
  }
  ...
  if (hidraw_match >= 0) {
    joydev_to_hidraw_[js_index] = hidraw_match;
  }
  ...
  base::HexStringToInt(vendor_id, &js_info.vendor_id);
  base::HexStringToInt(product_id, &js_info.product_id);
  ...
  pad.vibration_actuator.not_null =
      has_ff_capability || IsDualshock4(js_info.vendor_id, js_info.product_id);
...
void GamepadPlatformDataFetcherLinux::RefreshHidrawDevice(
    udev_device* dev,
    const UdevGamepad& pad_info) {
  const int hidraw_index = pad_info.index;
  int js_match = JsIndexFromHidrawIndex(hidraw_index);
  if (pad_info.syspath.empty()) {
    if (js_match >= 0)
      joydev_to_hidraw_[js_match] = -1;
    hidraw_devices_.erase(hidraw_index);
    return;
  }

  HidrawDeviceInfo& hidraw_info = GetOrCreateHidrawDeviceInfo(hidraw_index);
  if (hidraw_info.syspath != pad_info.syspath)
    js_match = -1;

  if (js_match < 0) {
    for (size_t i = 0; i < Gamepads::kItemsLengthCap; ++i) {
      const auto& js_info = js_devices_[i];
      if (js_info && SyspathsDescribeSameDevice(
                         pad_info.syspath, kHidrawSubsystem,
                         js_info->parent_syspath, kInputSubsystem)) {
        js_match = i;
        break;
      }
    }
  ...
  hidraw_info.fd = device_fd;
  hidraw_info.syspath = pad_info.syspath;

  if (js_match >= 0)
    joydev_to_hidraw_[js_match] = hidraw_index;
}
...
void GamepadPlatformDataFetcherLinux::EnumerateInputDevices() {
...
void GamepadPlatformDataFetcherLinux::EnumerateHidrawDevices() {
  if (!udev_->udev_handle())
    return;
  ScopedUdevEnumeratePtr enumerate(udev_enumerate_new(udev_->udev_handle()));
  if (!enumerate)
    return;
  int ret =
      udev_enumerate_add_match_subsystem(enumerate.get(), kHidrawSubsystem);
  if (ret != 0)
    return;
  ret = udev_enumerate_scan_devices(enumerate.get());
  if (ret != 0)
...
void GamepadPlatformDataFetcherLinux::PlayEffect(
  ...
  // Dualshock4 vibration is controlled through the hidraw device.
  const auto& js_info = js_devices_[pad_id];
  if (IsDualshock4(js_info->vendor_id, js_info->product_id)) {
    PlayEffectDualshock4(pad_id, type, std::move(params), std::move(callback));
    return;
  }
...
void GamepadPlatformDataFetcherLinux::PlayEffectDualshock4(
    int pad_id,
    mojom::GamepadHapticEffectType type,
    mojom::GamepadEffectParametersPtr params,
    mojom::GamepadHapticsManager::PlayVibrationEffectOnceCallback callback) {
  ...
  auto* hidraw_info = HidrawDeviceInfoFromJsIndex(pad_id);
  if (!hidraw_info || hidraw_info->fd < 0) {
    // No hidraw device found for this gamepad.
    std::move(callback).Run(
        mojom::GamepadHapticsResult::GamepadHapticsResultNotSupported);
    return;
  }
  ...
  PlayDualRumbleEffectDualshock4(*hidraw_info, pad_id, params->duration,
...
void GamepadPlatformDataFetcherLinux::ResetVibration(
  ...
  const auto& js_info = js_devices_[pad_id];
  if (IsDualshock4(js_info->vendor_id, js_info->product_id)) {
    ResetVibrationDualshock4(pad_id, std::move(callback));
    return;
  }
...
void GamepadPlatformDataFetcherLinux::ResetVibrationDualshock4(
    int pad_id,
    mojom::GamepadHapticsManager::ResetVibrationActuatorCallback callback) {
  sequence_ids_[pad_id]++;

  if (pending_callbacks_[pad_id]) {
    SetDualshock4Vibration(pad_id, 0.0, 0.0);
    std::move(pending_callbacks_[pad_id])
        .Run(mojom::GamepadHapticsResult::GamepadHapticsResultPreempted);
  }
...
void GamepadPlatformDataFetcherLinux::PlayDualRumbleEffectDualshock4(
    HidrawDeviceInfo& hidraw_info,
    int pad_id,
    double duration,
    double start_delay,
    double strong_magnitude,
  ...
  base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE,
      base::BindOnce(&GamepadPlatformDataFetcherLinux::StartDualshock4Vibration,
                     base::Unretained(this), sequence_ids_[pad_id], pad_id,
                     duration, strong_magnitude, weak_magnitude),
      base::TimeDelta::FromMillisecondsD(start_delay));
}

void GamepadPlatformDataFetcherLinux::StartDualshock4Vibration(
    int sequence_id,
    int pad_id,
    double duration,
    double strong_magnitude,
  ...
  if (sequence_id != sequence_ids_[pad_id])
    return;
  SetDualshock4Vibration(pad_id, strong_magnitude, weak_magnitude);

  base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE,
      base::BindOnce(&GamepadPlatformDataFetcherLinux::StopDualshock4Vibration,
                     base::Unretained(this), sequence_id, pad_id),
      base::TimeDelta::FromMillisecondsD(duration));
}

void GamepadPlatformDataFetcherLinux::StopDualshock4Vibration(int sequence_id,
                                                              int pad_id) {
  if (sequence_id != sequence_ids_[pad_id])
    return;
  SetDualshock4Vibration(pad_id, 0.0, 0.0);
...
void GamepadPlatformDataFetcherLinux::SetDualshock4Vibration(
    int pad_id,
    double strong_magnitude,
    double weak_magnitude) {
  HidrawDeviceInfo* hidraw_info = HidrawDeviceInfoFromJsIndex(pad_id);
  if (hidraw_info == nullptr || hidraw_info->fd < 0)
    return;
  ...
  control_report[4] =
      static_cast<uint8_t>(weak_magnitude * kRumbleMagnitudeMaxDualshock);
  control_report[5] =
      static_cast<uint8_t>(strong_magnitude * kRumbleMagnitudeMaxDualshock);

  write(hidraw_info->fd, &control_report[0], report_length);
}
...
JsDeviceInfo& GamepadPlatformDataFetcherLinux::GetOrCreateJsDeviceInfo(
  ...
  if (!js_info) {
    js_info = std::make_unique<JsDeviceInfo>();
    js_info->fd = -1;
  }
  return *js_info.get();
}
...
EvDeviceInfo& GamepadPlatformDataFetcherLinux::GetOrCreateEvDeviceInfo(
  ...
  auto ev_info = std::make_unique<EvDeviceInfo>();
  ev_info->fd = -1;
...
HidrawDeviceInfo& GamepadPlatformDataFetcherLinux::GetOrCreateHidrawDeviceInfo(
    int hidraw_index) {
  auto find_it = hidraw_devices_.find(hidraw_index);
  if (find_it != hidraw_devices_.end())
    return *find_it->second.get();

  auto hidraw_info = std::make_unique<HidrawDeviceInfo>();
  hidraw_info->fd = -1;
  auto emplace_result =
      hidraw_devices_.emplace(hidraw_index, std::move(hidraw_info));
  return *emplace_result.first->second.get();
}
...
int GamepadPlatformDataFetcherLinux::JsIndexFromHidrawIndex(int hidraw_index) {
  auto* find_it =
      std::find(joydev_to_hidraw_,
                joydev_to_hidraw_ + Gamepads::kItemsLengthCap, hidraw_index);
  if (find_it >= joydev_to_hidraw_ + Gamepads::kItemsLengthCap)
    return -1;
  return std::distance(joydev_to_hidraw_, find_it);
}
...
HidrawDeviceInfo* GamepadPlatformDataFetcherLinux::HidrawDeviceInfoFromJsIndex(
    int js_index) {
  if (js_index < 0 || js_index >= (int)Gamepads::kItemsLengthCap)
    return nullptr;

  int hidraw_index = joydev_to_hidraw_[js_index];
  if (hidraw_index < 0)
    return nullptr;
