...
void BluetoothAdapter::SetPowered(bool powered,
  ...
  if (set_powered_callbacks_) {
    ...
    base::PostTask(FROM_HERE, error_callback);
    return;
  }

  if (powered == IsPowered()) {
    // Return early in case no change of power state is needed.
    base::PostTask(FROM_HERE, callback);
    return;
  }
...
bool BluetoothAdapter::SetPoweredImpl(bool powered) {
  // macOS and Android should override this method, while BlueZ and Windows
  // don't call this method at all.
  NOTREACHED();
  return false;
}
