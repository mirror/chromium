...
#if defined(OS_ANDROID) || defined(OS_MACOSX) || defined(OS_WIN)
// Tests nested ReadRemoteCharacteristic.
TEST_F(BluetoothRemoteGattCharacteristicTest, ReadRemoteCharacteristic_Nested) {
  if (!PlatformSupportsLowEnergy()) {
    LOG(WARNING) << "Low Energy Bluetooth unavailable, skipping unit test.";
    return;
  }
...
#if defined(OS_ANDROID) || defined(OS_MACOSX) || defined(OS_WIN)
// Tests nested WriteRemoteCharacteristic.
TEST_F(BluetoothRemoteGattCharacteristicTest,
       WriteRemoteCharacteristic_Nested) {
  if (!PlatformSupportsLowEnergy()) {
    LOG(WARNING) << "Low Energy Bluetooth unavailable, skipping unit test.";
    return;
  }
