...
BluetoothRemoteGattServiceBlueZ::~BluetoothRemoteGattServiceBlueZ() {
  ...
  for (const auto& el : characteristics) {
    DCHECK(GetAdapter());
    GetAdapter()->NotifyGattCharacteristicRemoved(el.second.get());
  }
}
...
BluetoothRemoteGattServiceBlueZ::GetCharacteristics() const {
  ...
  for (const auto& el : characteristics_) {
    characteristics.push_back(el.second.get());
  }
