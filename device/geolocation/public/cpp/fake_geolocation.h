...
// Test clients must make sure this instance be alive during they call the
// geolocation mojo interfaces.
class ScopedGeolocationOverrider {
 public:
  explicit ScopedGeolocationOverrider(mojom::Geoposition position);
  ScopedGeolocationOverrider(double latitude, double longitude);
  ~ScopedGeolocationOverrider();
  void OverrideGeolocation(mojom::Geoposition position);
