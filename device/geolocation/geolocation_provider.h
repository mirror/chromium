...
class GeolocationProvider {
 public:
  ...
  // Call before using GetInstance() and no more than once.
  ...
  using RequestContextCallback = base::RepeatingCallback<void(
      base::OnceCallback<void(
          const scoped_refptr<net::URLRequestContextGetter>)>)>;

  // Optional: Provide a request context for network geolocation requests.
  // Call before using GetInstance().
  DEVICE_GEOLOCATION_EXPORT static void SetRequestContextCallback(
      RequestContextCallback request_context_callback);
