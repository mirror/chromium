...
base::LazyInstance<std::unique_ptr<LocationProvider>>::Leaky
    g_override_location_provider = LAZY_INSTANCE_INITIALIZER;
...
// static
void GeolocationProviderImpl::SetOverrideLocationProvider(
    std::unique_ptr<LocationProvider> provider) {
  DCHECK(!g_override_location_provider.Get());
  g_override_location_provider.Get() = std::move(provider);
}
...
void GeolocationProviderImpl::Init() {
  ...
  // Pass the embedder's "override location provider" to LocationArbitrator.
  arbitrator_ = std::make_unique<LocationArbitrator>(
      std::move(g_override_location_provider.Get()),
