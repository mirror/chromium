...
LocationArbitrator::LocationArbitrator(
    std::unique_ptr<LocationProvider> custom_location_provider,
    GeolocationProvider::RequestContextProducer request_context_producer,
    const std::string& api_key)
    : custom_location_provider_(std::move(custom_location_provider)),
...
void LocationArbitrator::RegisterSystemProvider() {
  std::unique_ptr<LocationProvider> provider =
      std::move(custom_location_provider_);
  // Use the embedder's provider if it is not null.
  if (!provider)
