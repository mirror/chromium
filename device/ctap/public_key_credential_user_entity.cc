...
void PublicKeyCredentialUserEntity::SetUserName(const std::string& user_name) {
...
void PublicKeyCredentialUserEntity::SetUserIconUrl(const GURL& icon_url) {
...
PublicKeyCredentialUserEntity::CreateFromCBORValue(
  ...
  if (cbor.is_map() && cbor.GetMap().count(cbor::CBORValue("id"))) {
    ...
    if (cbor_map.count(cbor::CBORValue("icon"))) {
      user.SetUserName(
          cbor_map.find(cbor::CBORValue("icon"))->second.GetString());
    }
    return user;
  }
  return base::nullopt;
