...
// static
std::unique_ptr<HidMessage> HidMessage::CreateHidMessageCmd(
    uint32_t channel_id,
...
// static
std::unique_ptr<HidMessage> HidMessage::CreateHidCBORCmd(
    uint32_t channel_id,
    const std::vector<uint8_t>& data) {
...
// static
std::unique_ptr<HidMessage> HidMessage::CreateHidInitCmd(
...
// static
std::unique_ptr<HidMessage> HidMessage::CreateHidPingCmd(uint32_t channel_id) {
  return Create(channel_id, CTAPHIDDeviceCommand::kCtapHidPing,
                std::vector<uint8_t>());
}

// static
std::unique_ptr<HidMessage> HidMessage::CreateHidWinkCmd(uint32_t channel_id) {
  return Create(channel_id, CTAPHIDDeviceCommand::kCtapHidWink,
                std::vector<uint8_t>());
}

// static
std::unique_ptr<HidMessage> HidMessage::CreateHidLockCmd(uint32_t channel_id,
                                                         uint8_t lock_time) {
  return lock_time > 10 ? nullptr
                        : Create(channel_id, CTAPHIDDeviceCommand::kCtapHidLock,
                                 std::vector<uint8_t>{lock_time});
}

// static
std::unique_ptr<HidMessage> HidMessage::CreateHidCancelCmd(uint32_t channel_id,
                                                           uint8_t lock_time) {
...
// static
std::unique_ptr<HidMessage> HidMessage::CreateFromSerializedData(
    const std::vector<uint8_t>& buf) {
  size_t remaining_size = 0;
  if (buf.size() > kPacketSize || buf.size() < kInitPacketHeader)
    return nullptr;

  std::unique_ptr<HidInitPacket> init_packet =
      HidInitPacket::CreateFromSerializedData(buf, &remaining_size);

  if (init_packet == nullptr)
    return nullptr;

  return std::make_unique<HidMessage>(std::move(init_packet), remaining_size);
}

// static
std::unique_ptr<HidMessage> HidMessage::Create(
    uint32_t channel_id,
  ...
  return std::make_unique<HidMessage>(channel_id, type, data);
}

HidMessage::HidMessage(std::unique_ptr<HidInitPacket> init_packet,
...
HidMessage::HidMessage(uint32_t channel_id,
                       CTAPHIDDeviceCommand type,
                       const std::vector<uint8_t>& data)
  ...
  packets_.push_back(std::make_unique<HidInitPacket>(
      channel_id, base::checked_cast<uint8_t>(type),
  ...
  while (remaining_bytes > 0) {
    ...
    packets_.push_back(std::make_unique<HidContinuationPacket>(
        channel_id, sequence, std::vector<uint8_t>(first, last)));
    sequence++;
  }
}

HidMessage::~HidMessage() = default;

std::vector<uint8_t> HidMessage::PopNextPacket() {
...
bool HidMessage::AddContinuationPacket(const std::vector<uint8_t>& buf) {
  size_t remaining_size = remaining_size_;
  std::unique_ptr<HidContinuationPacket> cont_packet =
      HidContinuationPacket::CreateFromSerializedData(buf, &remaining_size);
...
bool HidMessage::MessageComplete() const {
  return remaining_size_ == 0;
}

std::vector<uint8_t> HidMessage::GetMessagePayload() const {
  std::vector<uint8_t> data;
...
size_t HidMessage::NumPackets() const {
