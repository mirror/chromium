...
TEST(CTAPDataTest, TestConstructMakeCredentialRequest) {
  static const std::vector<uint8_t> client_data_hash_bytes = {
  ...
  static const std::vector<uint8_t> user_id = {
  ...
  static const std::vector<uint8_t> serialized_request = {
      // clang format-off
  ...
  request_builder.SetClientDataHash(client_data_hash_bytes);

  PublicKeyCredentialRPEntity rp("acme.com");
  rp.SetRPName("Acme");
  request_builder.SetRP(std::move(rp));

  PublicKeyCredentialUserEntity user(user_id);
  user.SetUserName("johnpsmith@example.com");
  user.SetUserDisplayName("John P. Smith");
  user.SetUserIconUrl("https://pics.acme.com/00/p/aBjjjpqPb.png");
  request_builder.SetUser(std::move(user));

  request_builder.SetPublicKeyCredentialParams(
  ...
  auto serialized_data = req.value().SerializeToCBOR();
  ASSERT_TRUE(serialized_data.has_value());
  EXPECT_THAT(serialized_data.value(),
              testing::ElementsAreArray(serialized_request));
}

TEST(CTAPDataTest, TestMakeCredentialRequestReturnNullOnInsufficientParams) {
  static const std::vector<uint8_t> client_data_hash_bytes = {0xF, 0x0, 0x0,
                                                              0xB, 0xA, 0x2};
  CTAPMakeCredentialRequest::MakeCredentialRequestBuilder
      no_client_data_builder;
  no_client_data_builder.SetUser(PublicKeyCredentialUserEntity({}));
  no_client_data_builder.SetRP(PublicKeyCredentialRPEntity("rp_id"));
  no_client_data_builder.SetPublicKeyCredentialParams(
      PublicKeyCredentialParams({{{"public-key", 7}}, {{"public-key", 257}}}));
  EXPECT_FALSE(no_client_data_builder.BuildRequest().has_value());

  CTAPMakeCredentialRequest::MakeCredentialRequestBuilder no_rp_builder;
  no_rp_builder.SetClientDataHash(client_data_hash_bytes);
  no_rp_builder.SetUser(PublicKeyCredentialUserEntity({}));
  no_rp_builder.SetPublicKeyCredentialParams(
      PublicKeyCredentialParams({{{"public-key", 7}}, {{"public-key", 257}}}));
  EXPECT_FALSE(no_rp_builder.BuildRequest().has_value());

  CTAPMakeCredentialRequest::MakeCredentialRequestBuilder no_user_builder;
  no_rp_builder.SetClientDataHash(client_data_hash_bytes);
  no_user_builder.SetRP(PublicKeyCredentialRPEntity("rp_id"));
  no_user_builder.SetPublicKeyCredentialParams(
      PublicKeyCredentialParams({{{"public-key", 7}}, {{"public-key", 257}}}));
  EXPECT_FALSE(no_user_builder.BuildRequest().has_value());

  CTAPMakeCredentialRequest::MakeCredentialRequestBuilder no_pub_key_builder;
  no_rp_builder.SetClientDataHash(client_data_hash_bytes);
  no_pub_key_builder.SetUser(PublicKeyCredentialUserEntity({}));
  no_pub_key_builder.SetRP(PublicKeyCredentialRPEntity("rp_id"));
  EXPECT_FALSE(no_pub_key_builder.BuildRequest().has_value());
}
...
TEST(CTAPDataTest, TestConstructGetAssertionRequest) {
  ...
  request_builder.SetClientDataHash(client_data_hash_bytes);
  request_builder.SetRPId("acme.com");

  std::vector<PublicKeyCredentialDescriptor> allowed_list;
  allowed_list.emplace_back(PublicKeyCredentialDescriptor(
      "public-key",
  ...
  request_builder.SetAllowList(std::move(allowed_list));
...
TEST(CTAPDataTest, TestGetAssertuinReturnNullOnInsufficientParams) {
  ...
  no_rp_id_builder.SetClientDataHash(client_data_hash_bytes);
  EXPECT_FALSE(no_rp_id_builder.BuildRequest().has_value());

  CTAPGetAssertionRequest::CTAPGetAssertionRequestBuilder
      no_client_data_builder;
  no_client_data_builder.SetRPId("RPID");
  EXPECT_FALSE(no_client_data_builder.BuildRequest().has_value());
}
