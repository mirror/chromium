...
void PublicKeyCredentialRPEntity::SetRPName(std::string rp_name) {
  rp_name_ = rp_name;
};
void PublicKeyCredentialRPEntity::SetRPIconUrl(std::string icon_url) {
...
cbor::CBORValue PublicKeyCredentialRPEntity::ConvertToCBOR() const {
  ...
  if (rp_icon_url_.has_value())
    rp_map[cbor::CBORValue("icon")] = cbor::CBORValue(rp_icon_url_.value());
