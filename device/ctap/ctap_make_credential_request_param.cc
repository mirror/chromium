...
CTAPMakeCredentialRequestParam::MakeCredentialRequestBuilder::
    MakeCredentialRequestBuilder() = default;

CTAPMakeCredentialRequestParam::MakeCredentialRequestBuilder::
    ~MakeCredentialRequestBuilder() = default;

void CTAPMakeCredentialRequestParam::MakeCredentialRequestBuilder::
    set_client_data_hash(std::vector<uint8_t> client_data_hash) {
  client_data_hash_param_ = std::move(client_data_hash);
};

void CTAPMakeCredentialRequestParam::MakeCredentialRequestBuilder::set_rp(
    PublicKeyCredentialRPEntity rp) {
  rp_param_ = std::move(rp);
};

void CTAPMakeCredentialRequestParam::MakeCredentialRequestBuilder::set_user(
    PublicKeyCredentialUserEntity user) {
...
void CTAPMakeCredentialRequestParam::MakeCredentialRequestBuilder::
    set_exclude_list(std::vector<PublicKeyCredentialDescriptor> exclude_list) {
  exclude_list_param_ = std::move(exclude_list);
};

void CTAPMakeCredentialRequestParam::MakeCredentialRequestBuilder::set_pin_auth(
...
bool CTAPMakeCredentialRequestParam::MakeCredentialRequestBuilder::
    CheckRequiredParameters() {
  return client_data_hash_param_ && rp_param_ && user_param_ &&
         public_key_credentials_params_;
};

base::Optional<CTAPMakeCredentialRequestParam>
CTAPMakeCredentialRequestParam::MakeCredentialRequestBuilder::BuildRequest() {
  if (CheckRequiredParameters()) {
    return CTAPMakeCredentialRequestParam(
