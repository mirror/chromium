...
CTAPDeviceResponseCode GetResponseCode(const std::vector<uint8_t>& buffer) {
  if (!buffer.empty()) {
    for (const auto& code : kCTAPResponseCodeList) {
...
ReadCTAPMakeCredentialResponse(CTAPDeviceResponseCode response_code,
  ...
  auto& decoded_map = decoded_response->GetMap();
  ...
  response_map[CBOR("fmt")] = std::move(it->second).Clone();

  it = decoded_map.find(CBOR(2));
  if (it == decoded_map.end() || !it->second.is_bytestring())
    return base::nullopt;

  response_map[CBOR("authData")] = std::move(it->second).Clone();

  it = decoded_map.find(CBOR(3));
  if (it == decoded_map.end() || !it->second.is_map())
    return base::nullopt;

  response_map[CBOR("attStmt")] = std::move(it->second).Clone();
