...
TEST(CTAPRequestTest, TestConstructCtapAuthenticatorRequestParam) {
  static const uint8_t kSerializedGetInfoCmd[] = {0x04};
  static const uint8_t kSerializedGetNextAssertionCmd[] = {0x08};
  static const uint8_t kSerializedCancelCmd[] = {0x03};
  static const uint8_t kSerializedResetCmd[] = {0x07};

  auto get_info_cmd =
      CTAPAuthenticatorRequestParam::CreateAuthenticatorGetInfoParam()
          .SerializeToCBOR();
  ASSERT_TRUE(get_info_cmd);
  EXPECT_THAT(*get_info_cmd, testing::ElementsAreArray(kSerializedGetInfoCmd));
  ...
  EXPECT_THAT(*get_next_assertion_cmd,
              testing::ElementsAreArray(kSerializedGetNextAssertionCmd));

  auto cancel_cmd =
      CTAPAuthenticatorRequestParam::CreateAuthenticatorCancelParam()
          .SerializeToCBOR();
  ASSERT_TRUE(cancel_cmd);
  EXPECT_THAT(*cancel_cmd, testing::ElementsAreArray(kSerializedCancelCmd));

  auto reset_cmd =
      CTAPAuthenticatorRequestParam::CreateAuthenticatorResetParam()
          .SerializeToCBOR();
  ASSERT_TRUE(reset_cmd);
  EXPECT_THAT(*reset_cmd, testing::ElementsAreArray(kSerializedResetCmd));
}
