...
PublicKeyCredentialDescriptor PublicKeyCredentialDescriptor::Clone() const {
  auto descriptor = PublicKeyCredentialDescriptor(credential_type_, id_);
  return descriptor;
}
...
PublicKeyCredentialDescriptor::CreateFromCBORValue(
  ...
  if (cbor.is_map() && cbor.GetMap().count(cbor::CBORValue("id")) &&
      cbor.GetMap().count(cbor::CBORValue("type"))) {
    return PublicKeyCredentialDescriptor(
        cbor.GetMap().find(cbor::CBORValue("type"))->second.GetString(),
