...
class PublicKeyCredentialUserEntity {
 public:
  ...
  PublicKeyCredentialUserEntity& SetUserName(const std::string& user_name);
  PublicKeyCredentialUserEntity& SetDisplayName(
      const std::string& display_name);
  PublicKeyCredentialUserEntity& SetIconUrl(const GURL& icon_url);

  const std::vector<uint8_t>& user_id() const { return user_id_; }
  base::Optional<std::string> user_name() const { return user_name_; }
  base::Optional<std::string> user_display_name() const {
    return user_display_name_;
  }
  base::Optional<GURL> user_icon_url() const { return user_icon_url_; }
