...
#ifndef DEVICE_CTAP_HID_PACKET_H_
#define DEVICE_CTAP_HID_PACKET_H_
...
// packets,
// (HidInitPacket and HidContinuationPackets). HID Packets have header
// information and a payload. If a HidInitPacket cannot store the entire
// payload, further payload information is stored in HidContinuationPackets.
class HidPacket {
 public:
  HidPacket(const std::vector<uint8_t>& data, uint32_t channel_id);
  virtual ~HidPacket();
...
class HidInitPacket : public HidPacket {
 public:
  HidInitPacket(uint32_t channel_id,
                uint8_t cmd,
                const std::vector<uint8_t>& data,
                uint16_t payload_length);
  HidInitPacket(const std::vector<uint8_t>& serialized, size_t* remaining_size);
  ~HidInitPacket() final;
  ...
  static std::unique_ptr<HidInitPacket> CreateFromSerializedData(
...
// 0x00 to 0x7f.
class HidContinuationPacket : public HidPacket {
 public:
  HidContinuationPacket(uint32_t channel_id,
                        uint8_t sequence,
                        const std::vector<uint8_t>& data);
  HidContinuationPacket(const std::vector<uint8_t>& serialized,
                        size_t* remaining_size);
  ~HidContinuationPacket() final;
  ...
  // to expect.
  static std::unique_ptr<HidContinuationPacket> CreateFromSerializedData(
      const std::vector<uint8_t>& serialized,
      size_t* remaining_size);
  std::vector<uint8_t> GetSerializedData() final;
...
#endif  // DEVICE_CTAP_HID_PACKET_H_
