...
class AuthenticatorGetAssertionResponse : public CTAPResponse {
 public:
  AuthenticatorGetAssertionResponse(constants::CTAPResponseCode response_code,
  ...
  const base::Optional<PublicKeyCredentialDescriptor>& get_credential() const {
    return credential_;
  }
  ...
  const PublicKeyCredentialUserEntity& get_user() const { return user_; }
  const base::Optional<uint8_t>& get_num_credentials() const {
    return num_credentials_;
  }
  void SetCredential(const PublicKeyCredentialDescriptor& credential);
  void SetNumCredentials(const uint8_t num_credentials);
