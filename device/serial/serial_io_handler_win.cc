...
int DataBitsEnumToConstant(mojom::DataBits data_bits) {
  switch (data_bits) {
...
int ParityBitEnumToConstant(mojom::ParityBit parity_bit) {
  switch (parity_bit) {
...
int StopBitsEnumToConstant(mojom::StopBits stop_bits) {
  switch (stop_bits) {
...
mojom::DataBits DataBitsConstantToEnum(int data_bits) {
  switch (data_bits) {
    case 7:
      return mojom::DataBits::SEVEN;
    case 8:
    default:
      return mojom::DataBits::EIGHT;
  }
}

mojom::ParityBit ParityBitConstantToEnum(int parity_bit) {
  switch (parity_bit) {
    case EVENPARITY:
      return mojom::ParityBit::EVEN;
    case ODDPARITY:
      return mojom::ParityBit::ODD;
    case NOPARITY:
    default:
      return mojom::ParityBit::NO_PARITY;
  }
}

mojom::StopBits StopBitsConstantToEnum(int stop_bits) {
  switch (stop_bits) {
    case TWOSTOPBITS:
      return mojom::StopBits::TWO;
    case ONESTOPBIT:
    default:
      return mojom::StopBits::ONE;
  }
}
...
void SerialIoHandlerWin::OnDeviceRemoved(const std::string& device_path) {
  ...
  if (port() == com_port)
    CancelRead(mojom::ReceiveError::DEVICE_LOST);
}
...
void SerialIoHandlerWin::ReadImpl() {
  ...
  if (!ok && GetLastError() != ERROR_IO_PENDING) {
    ...
    QueueReadCompleted(0, mojom::ReceiveError::SYSTEM_ERROR);
  }
...
void SerialIoHandlerWin::WriteImpl() {
  ...
  if (!ok && GetLastError() != ERROR_IO_PENDING) {
    ...
    QueueWriteCompleted(0, mojom::SendError::SYSTEM_ERROR);
  }
}
...
void SerialIoHandlerWin::OnIOCompleted(
  ...
  if (context == comm_context_.get()) {
    ...
    if (!ClearCommError(file().GetPlatformFile(), &errors, &status) ||
        errors != 0) {
      if (errors & CE_BREAK) {
        ReadCompleted(0, mojom::ReceiveError::BREAK);
      } else if (errors & CE_FRAME) {
        ReadCompleted(0, mojom::ReceiveError::FRAME_ERROR);
      } else if (errors & CE_OVERRUN) {
        ReadCompleted(0, mojom::ReceiveError::OVERRUN);
      } else if (errors & CE_RXOVER) {
        ReadCompleted(0, mojom::ReceiveError::BUFFER_OVERFLOW);
      } else if (errors & CE_RXPARITY) {
        ReadCompleted(0, mojom::ReceiveError::PARITY_ERROR);
      } else {
        ReadCompleted(0, mojom::ReceiveError::SYSTEM_ERROR);
      }
      return;
    }
    ...
    if (read_canceled()) {
      ...
      ReadCompleted(bytes_transferred, error == ERROR_SUCCESS
                                           ? mojom::ReceiveError::NONE
                                           : mojom::ReceiveError::SYSTEM_ERROR);
    }
    ...
    if (write_canceled()) {
      ...
      if (error == ERROR_GEN_FAILURE && IsReadPending()) {
        ...
        // mojom::OnReceiveError which will notify the app about the
        // disconnection.
        CancelRead(mojom::ReceiveError::SYSTEM_ERROR);
      }
    } else {
      WriteCompleted(bytes_transferred, error == ERROR_SUCCESS
                                            ? mojom::SendError::NONE
                                            : mojom::SendError::SYSTEM_ERROR);
    }
  } else {
...
mojom::DeviceControlSignalsPtr SerialIoHandlerWin::GetControlSignals() const {
  DWORD status;
...
mojom::ConnectionInfoPtr SerialIoHandlerWin::GetPortInfo() const {
  DCB config = {0};
  config.DCBlength = sizeof(config);
  if (!GetCommState(file().GetPlatformFile(), &config)) {
    VPLOG(1) << "Failed to get serial port info";
