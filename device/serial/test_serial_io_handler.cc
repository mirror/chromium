...
void TestSerialIoHandler::ReadImpl() {
  ...
  ReadCompleted(static_cast<uint32_t>(num_bytes), mojom::ReceiveError::NONE);
}
...
void TestSerialIoHandler::WriteImpl() {
  ...
  WriteCompleted(pending_write_buffer_len(), mojom::SendError::NONE);
  if (pending_read_buffer())
    ReadImpl();
}
...
mojom::DeviceControlSignalsPtr TestSerialIoHandler::GetControlSignals() const {
  mojom::DeviceControlSignalsPtr signals(mojom::DeviceControlSignals::New());
  *signals = device_control_signals_;
  return signals;
}

mojom::ConnectionInfoPtr TestSerialIoHandler::GetPortInfo() const {
