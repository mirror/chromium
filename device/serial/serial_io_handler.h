...
class SerialIoHandler : public base::RefCountedThreadSafe<SerialIoHandler> {
 public:
  ...
  virtual void Open(const std::string& port,
                    const mojom::ConnectionOptions& options,
  ...
  void CancelRead(mojom::ReceiveError reason);

  // Attempts to cancel a pending write operation.
  void CancelWrite(mojom::SendError reason);
  ...
  // successfully read.
  virtual mojom::DeviceControlSignalsPtr GetControlSignals() const = 0;
  ...
  virtual bool SetControlSignals(
      const mojom::HostControlSignals& control_signals) = 0;

  // Performs platform-specific port configuration. Returns |true| iff
  // configuration was successful.
  bool ConfigurePort(const mojom::ConnectionOptions& options);
  ...
  virtual mojom::ConnectionInfoPtr GetPortInfo() const = 0;
 ...
 protected:
  ...
  void ReadCompleted(int bytes_read, mojom::ReceiveError error);
  ...
  // if the associated I/O operation was the only thing keeping it alive.
  void WriteCompleted(int bytes_written, mojom::SendError error);
  ...
  void QueueReadCompleted(int bytes_read, mojom::ReceiveError error);
  ...
  // without being reentrant.
  void QueueWriteCompleted(int bytes_written, mojom::SendError error);
  ...
  mojom::ReceiveError read_cancel_reason() const { return read_cancel_reason_; }
  ...
  mojom::SendError write_cancel_reason() const { return write_cancel_reason_; }

  bool write_canceled() const { return write_canceled_; }

  const mojom::ConnectionOptions& options() const { return options_; }
 ...
 private:
  ...
  void MergeConnectionOptions(const mojom::ConnectionOptions& options);
  ...
  mojom::ConnectionOptions options_;

  std::unique_ptr<WritableBuffer> pending_read_buffer_;
  mojom::ReceiveError read_cancel_reason_;
  bool read_canceled_;

  std::unique_ptr<ReadOnlyBuffer> pending_write_buffer_;
  mojom::SendError write_cancel_reason_;
