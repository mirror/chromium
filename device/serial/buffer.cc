...
SendBuffer::SendBuffer(
    ...
    const base::Callback<void(int, device::mojom::SendError)>& callback)
...
void SendBuffer::Done(uint32_t bytes_read) {
  callback_.Run(bytes_read, device::mojom::SendError::NONE);
}

void SendBuffer::DoneWithError(uint32_t bytes_read, int32_t error) {
  callback_.Run(bytes_read, static_cast<device::mojom::SendError>(error));
}

ReceiveBuffer::ReceiveBuffer(
    scoped_refptr<net::IOBuffer> buffer,
    uint32_t size,
    const base::Callback<void(int, device::mojom::ReceiveError)>& callback)
...
void ReceiveBuffer::Done(uint32_t bytes_written) {
  callback_.Run(bytes_written, device::mojom::ReceiveError::NONE);
}

void ReceiveBuffer::DoneWithError(uint32_t bytes_written, int32_t error) {
  callback_.Run(bytes_written, static_cast<device::mojom::ReceiveError>(error));
}
