...
mojom::VREyeParametersPtr GetEyeDetails(ovrSession session,
                                        const ovrHmdDesc& hmdDesc,
                                        ovrEyeType eye) {
  auto eye_parameters = mojom::VREyeParameters::New();
  auto render_desc =
      ovr_GetRenderDesc(session, eye, hmdDesc.DefaultEyeFov[eye]);
...
mojom::VRDisplayInfoPtr CreateVRDisplayInfo(unsigned int id,
  ...
  transform.resize(16);
  display_info->stageParameters->standingTransform = std::vector<float>(16);
  display_info->stageParameters->standingTransform[0] = 1;
  display_info->stageParameters->standingTransform[1] = 0;
  display_info->stageParameters->standingTransform[2] = 0;
  display_info->stageParameters->standingTransform[3] = 0;
  display_info->stageParameters->standingTransform[4] = 0;
  display_info->stageParameters->standingTransform[5] = 1;
  display_info->stageParameters->standingTransform[6] = 0;
  display_info->stageParameters->standingTransform[7] = 0;
  display_info->stageParameters->standingTransform[8] = 0;
  display_info->stageParameters->standingTransform[9] = 0;
  display_info->stageParameters->standingTransform[10] = 1;
  display_info->stageParameters->standingTransform[11] = 0;
  display_info->stageParameters->standingTransform[12] = 0;
  display_info->stageParameters->standingTransform[13] = floor_height;
  display_info->stageParameters->standingTransform[14] = 0;
  display_info->stageParameters->standingTransform[15] = 1;
...
void OculusDevice::RequestPresent(
  ...
  base::Callback<void(bool)> my_callback =
      base::Bind(&OculusDevice::OnRequestPresentResult,
                 weak_ptr_factory_.GetWeakPtr(), base::Passed(&callback));
  render_loop_->task_runner()->PostTask(
      FROM_HERE,
      base::Bind(&OculusRenderLoop::RequestPresent, render_loop_->GetWeakPtr(),
...
void OculusDevice::ExitPresent() {
  render_loop_->task_runner()->PostTask(
      FROM_HERE,
      base::Bind(&OculusRenderLoop::ExitPresent, render_loop_->GetWeakPtr()));
}
