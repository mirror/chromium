...
class FakeOrientationSensor : public mojom::Sensor {
 public:
  FakeOrientationSensor(mojom::SensorRequest request) : binding_(this) {
    binding_.Bind(std::move(request));
  };
  ~FakeOrientationSensor() override = default;
...
class VROrientationDeviceTest : public testing::Test {
 ...
 protected:
  ...
  void WriteToBuffer(gfx::Quaternion q) {
    ...
    reading.orientation_quat.x = q.x();
    reading.orientation_quat.y = q.y();
    reading.orientation_quat.z = q.z();
    reading.orientation_quat.w = q.w();
  ...
  std::unique_ptr<VROrientationDevice> device_ = nullptr;
