...
void VRDevice::RemoveDisplay(VRDisplayImpl* display) {
  ...
  if (listening_for_activate_ == display)
    listening_for_activate_ = nullptr;
}
...
void VRDevice::OnActivate(mojom::VRDisplayEventReason reason,
  ...
  if (listening_for_activate_) {
    listening_for_activate_->OnActivate(reason, on_handled);
  } else {
...
void VRDevice::UpdateListeningForActivate(VRDisplayImpl* display) {
  if (display->ListeningForActivate() && display->InFocusedFrame()) {
    listening_for_activate_ = display;
  } else if (listening_for_activate_ == display) {
    listening_for_activate_ = nullptr;
  }
  OnListeningForActivateChanged(!!listening_for_activate_);
}
