...
void OpenVRDevice::RequestPresent(
  ...
  if (!started_render_thread_)
    started_render_thread_ = render_loop_->Start();
  ...
  if (started_render_thread_) {
    ...
    render_loop_->task_runner()->PostTask(
        FROM_HERE,
        base::Bind(&OpenVRRenderLoop::RequestPresent,
                   render_loop_->GetWeakPtr(), base::Passed(&request),
                   base::Passed(&my_callback)));
  } else {
...
void OpenVRDevice::GetPose(
  ...
  std::move(callback).Run(
      OpenVRPoseToVRPosePtr(rendering_poses[vr::k_unTrackedDeviceIndex_Hmd]));
}
