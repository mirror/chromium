...
class MediaTransferProtocolManagerImpl : public MediaTransferProtocolManager {
 public:
  ...
  // MediaTransferProtocolManager override.
  mojom::MtpStorageInfoPtr GetStorageInfo(
      const std::string& storage_name) const override {
    DCHECK(thread_checker_.CalledOnValidThread());
    const auto it = storage_info_map_.find(storage_name);
    return it != storage_info_map_.end() ? it->second.Clone()
                                         : mojom::MtpStorageInfo::New();
  }
  ...
  void GetStorageInfoFromDevice(
    ...
    if (!base::ContainsKey(storage_info_map_, storage_name) || !mtp_client_) {
      callback.Run(mojom::MtpStorageInfo::New(), true /* error */);
      return;
    }
  ...
  void ReadDirectory(const std::string& storage_handle,
    ...
    DCHECK(thread_checker_.CalledOnValidThread());
    if (!base::ContainsKey(handles_, storage_handle) || !mtp_client_) {
      callback.Run(std::vector<mojom::MtpFileEntryPtr>(),
  ...
  void GetFileInfo(const std::string& storage_handle,
    ...
    DCHECK(thread_checker_.CalledOnValidThread());
    if (!base::ContainsKey(handles_, storage_handle) || !mtp_client_) {
      callback.Run(mojom::MtpFileEntry::New(), true);
      return;
    }
 ...
 private:
  ...
  void OnGetStorageInfo(mojom::MtpStorageInfoPtr storage_info) {
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    storage_info_map_.insert(
        std::make_pair(storage_name, std::move(storage_info)));
    for (auto& observer : observers_)
      observer.StorageChanged(true /* is attach */, storage_name);
  }

  void OnGetStorageInfoFromDevice(mojom::MtpStorageInfoPtr storage_info) {
    get_storage_info_from_device_callbacks_.front().Run(std::move(storage_info),
                                                        false /* no error */);
    get_storage_info_from_device_callbacks_.pop();
  }

  void OnGetStorageInfoFromDeviceError() {
    get_storage_info_from_device_callbacks_.front().Run(
        mojom::MtpStorageInfo::New(), true /* error */);
    get_storage_info_from_device_callbacks_.pop();
  }
  ...
  void OnReadDirectoryEntryIdsToReadDirectory(
    ...
    if (file_ids.empty()) {
      OnGotDirectoryEntries(storage_handle, file_ids, kInitialOffset, max_size,
                            file_ids, std::vector<mojom::MtpFileEntryPtr>());
      return;
    }
  ...
  void OnGotDirectoryEntries(const std::string& storage_handle,
    ...
    read_directory_callbacks_.front().Run(std::move(file_entries), has_more,
                                          false /* no error */);
  ...
  void OnReadDirectoryError() {
    DCHECK(thread_checker_.CalledOnValidThread());
    read_directory_callbacks_.front().Run(std::vector<mojom::MtpFileEntryPtr>(),
  ...
  void OnGetFileInfo(std::vector<mojom::MtpFileEntryPtr> entries) {
    DCHECK(thread_checker_.CalledOnValidThread());
    if (entries.size() == 1) {
      get_file_info_callbacks_.front().Run(std::move(entries[0]),
                                           false /* no error */);
  ...
  void OnGetFileInfoError() {
    DCHECK(thread_checker_.CalledOnValidThread());
    get_file_info_callbacks_.front().Run(device::mojom::MtpFileEntry::New(),
                                         true);
    get_file_info_callbacks_.pop();
  }
  ...
  base::flat_map<std::string, mojom::MtpStorageInfoPtr> storage_info_map_;
