  ...
  def __init__(
    ...
    Args:
      ...
      test_args: List of strings to pass as arguments to the test when
        launching.
      test_cases: List of tests to be included in the test run. If unspecified,
        runs all tests.
...
class DeviceTestRunner(TestRunner):
  ...
  def set_xctest_filters(self, test_filter=None, invert=False):
    ...
    if self.test_cases:
      ...
      if test_filter:
        # Use the intersection or differnce of test_cases and test_filter to
        # define list of tests be included
        filter = (sorted(list(set(filter) - set(test_filter))) if invert
                  else test_filter)
