...
class DeviceTestRunner(TestRunner):
  ...
  def restart_device_and_check(self):
    """Restart the device, check for device online in 2 minutes."""
    if restart:
      print 'Restarting device, check status within two minutes.'
      try:
        subprocess.check_call(
      ...
      while time.time() < endTime:
        if self.udid == subprocess.check_output(
                          ['idevice_id', '--list']).rstrip():
          print 'Device is online. Delay 10 seconds to startup...'
          time.sleep(10)
          return True
        time.sleep(1)
      print 'Device is not online in two minutes after rebooting.'
      return False
