...
class InstallXcodeTest(TestCase):
  ...
  def test_success(self):
    # Use UNIX `true` command to return code 0.
...
class SimulatorTestRunnerTest(TestCase):
  ...
  def setUp(self):
    ...
    self.mock(test_runner, 'find_app', lambda _: 'path/to/app')
    self.mock(test_runner, 'find_iossim', lambda _: 'path/to/iossim')

  def test_app_not_found(self):
    """Ensures AppNotFoundError is raised."""

    self.mock(test_runner, 'find_app', lambda _: None)
  ...
  def test_iossim_not_found(self):
    ...
    self.mock(test_runner, 'find_iossim', lambda _: False)
