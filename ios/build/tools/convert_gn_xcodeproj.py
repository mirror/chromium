...
def UpdateProductsProject(file_input, file_output, configurations):
  ...
  for key in project.objects.keys():
    ...
    if isa == 'PBXShellScriptBuildPhase':
      ...
      if '-j1000 xcuitests' in value['shellScript']:
          value['shellScript'] = value['shellScript'].replace(
            '-j1000 xcuitests',
            '-j1000 xcuitests xcuitests-Runner')
    ...
    if isa == 'XCConfigurationList':
      ...
      build_config_template['buildSettings']['CONFIGURATION_BUILD_DIR'] = \
          '/Users/liaoyuke/bling/src/out/$(CONFIGURATION)$(EFFECTIVE_PLATFORM_NAME)'
  ...
  for key in project.objects.keys():
    value = project.objects[key]
    isa = value['isa']
    if isa == 'PBXNativeTarget' and value['name'] == 'chrome':
      chrome_target_key = key
      break

  target_dependency_key = ''
  target_dependency_value = ''
  container_item_key = ''
  ...
  for key in project.objects.keys():
    value = project.objects[key]
    isa = value['isa']
    if isa == 'PBXNativeTarget' and value['name'] == 'ios_chrome_settings_egtests_module':
      target_dependency_value = project.objects[value['dependencies'][0]]
      container_item_value = project.objects[target_dependency_value['targetProxy']]
      break

  container_item_key = project.AddObject('objects', container_item_value)
  target_dependency_key = project.AddObject('objects', target_dependency_value)

  for key in project.objects.keys():
    value = project.objects[key]
    isa = value['isa']
    if isa == 'PBXNativeTarget' and value['name'] == 'xcuitests':
      value['dependencies'].append(target_dependency_key)
      target_dependency_value['target'] = chrome_target_key
      target_dependency_value['targetProxy'] = container_item_key
      container_item_value['remoteGlobalIDString'] = chrome_target_key
      container_item_value['remoteInfo'] = 'chrome'
      break
