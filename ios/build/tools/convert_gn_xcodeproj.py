...
def UpdateProductsProject(file_input, file_output, configurations):
  ...
  AddMarkdownToProject(project, json_data['rootObject'])

  objects = collections.OrderedDict(sorted(project.objects.iteritems()))
  WriteXcodeProject(file_output, json.dumps(json_data))

def AddMarkdownToProject(project, root_object):
  list_files_cmd = ['git', '-C', 'src', 'ls-files', '*.md']
  paths = subprocess.check_output(list_files_cmd).split()
  for path in paths:
    new_markdown_entry = {
      "fileEncoding": "4",
    ...
    folder = GetFolderFroPath(project, root_object, os.path.dirname(path))
    folder['children'].append(new_markdown_entry_id)


def GetFolderFroPath(project, rootObject, path):
