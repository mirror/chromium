...
namespace {
id<GREYMatcher> TextBeginWith(NSString* text) {
  MatchesBlock matches = ^BOOL(id element) {
    return [[element text] hasPrefix:text];
  };
  DescribeToBlock describe = ^void(id<GREYDescription> description) {
    [description
        appendText:[NSString stringWithFormat:@"beginWithText('%@')", text]];
  };
...
// Tests to open a suggestion item by tapping on it.
- (void)testOpenItem {
  showcase_utils::Open(@"ContentSuggestionsViewController");
...
// Tests to swipe to dismiss an item.
...
- (void)testLongPressItem {
  ...
  [[EarlGrey
      selectElementWithMatcher:grey_allOf(TextBeginWith(
