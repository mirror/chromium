...
class MockChunkedDataStreamUploaderDelegate
 ...
 public:
  MockChunkedDataStreamUploaderDelegate()
      : data_length_(-1), upload_finished_(false), upload_error_(false) {}
  ...
  void OnReadError(int result) override { upload_error_ = true; }
  ...
  bool upload_error() { return upload_error_; }
...
// Tests that read errors is correctly handled by calling OnReadError()
// callback.
TEST_F(ChunkedDataStreamUploaderTest, HandleReadError) {
  uploader_->UploadWhenReady(false);
  ...
  EXPECT_TRUE(delegate_->upload_error());
}
