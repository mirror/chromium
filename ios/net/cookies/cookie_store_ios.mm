...
void CookieStoreIOS::SetCookieWithOptionsAsync(
  ...
  if (success) {
    ...
    if (!callback.is_null())
      closure = BindOnce(std::move(callback), true);
    system_store_->SetCookieAsync(cookie, std::move(closure));
    return;
  }
...
void CookieStoreIOS::RunGetCookiesCallbackOnSystemCookies(
    const net::CookieOptions& options,
...
void CookieStoreIOS::GetCookiesWithOptionsAsync(
  ...
  system_store_->GetCookiesForURLAsync(
      url, base::BindOnce(&CookieStoreIOS::RunGetCookiesCallbackOnSystemCookies,
...
void CookieStoreIOS::FlushStore(base::OnceClosure closure) {
  ...
  if (SystemCookiesAllowed()) {
    __block base::OnceClosure shared_closure = std::move(closure);
    ...
    system_store_->GetAllCookiesAsync(
        base::BindBlockArc(^(NSArray<NSHTTPCookie*>* cookies) {
          ...
          cookie_monster_->FlushStore(std::move(shared_closure));
...
void CookieStoreIOS::DeleteCookiesWithFilter(const CookieFilterFunction& filter,
  ...
  system_store_->GetAllCookiesAsync(base::BindBlockArc(^(
      NSArray<NSHTTPCookie*>* cookies) {
    int to_delete_count = 0;
    for (NSHTTPCookie* cookie in cookies) {
      if (filter.Run(cookie, system_store_->GetCookieCreationTime(cookie))) {
        system_store_->DeleteCookieAsync(
            cookie, SystemCookieStore::SystemCookieCallback());
        to_delete_count++;
      }
...
CookieStoreIOS::AddCallbackForCookie(const GURL& gurl,
  ...
  if (hook_map_.count(key) == 0) {
    UpdateCacheForCookieFromSystem(gurl, name, false);
...
void CookieStoreIOS::UpdateCacheForCookieFromSystem(const GURL& gurl,
                                                    const std::string& name,
                                                    bool run_callbacks) {
  DCHECK(thread_checker_.CalledOnValidThread());
  system_store_->GetCookiesForURLAsync(
      gurl,
      base::BindOnce(&CookieStoreIOS::UpdateCacheForCookies,
                     weak_factory_.GetWeakPtr(), gurl, name, run_callbacks));
}
...
void CookieStoreIOS::UpdateCacheForCookies(const GURL& gurl,
  ...
  for (NSHTTPCookie* nscookie in nscookies) {
    if (nscookie.name.UTF8String == cookie_name) {
