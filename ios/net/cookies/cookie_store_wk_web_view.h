...
class CookieStoreWKWebView : public net::CookieStore {
 public:
  ...
  CookieStoreWKWebView(CookieMonster::PersistentCookieStore* store,
                       WKHTTPCookieStore* cookie_storage);

  ~CookieStoreWKWebView() override;
  ...
  void FlushStore(base::OnceClosure) override;
 ...
 private:
  ...
  base::scoped_nsobject<WKHTTPCookieStore> system_store_;
  ...
  // were added.
  void UpdateCacheForCookieFromSystem(
      const GURL& gurl,
      const std::string& name,
      void (^callback)(const std::vector<net::CanonicalCookie>&,
  ...
  base::WeakPtrFactory<CookieStoreWKWebView> weak_factory_;

  DISALLOW_COPY_AND_ASSIGN(CookieStoreWKWebView);
};
