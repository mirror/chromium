...
class CookieStoreIOS : public net::CookieStore,
 ...
 private:
  ...
  // Returns an empty closure if |callback| is null clalback or binds the
  // callback to |success|.
  base::OnceClosure BindSetCookiesCallback(SetCookiesCallback* callback,
                                           bool success);
  ...
  // Updates the cookie cache with the current set of cookies named
  // |name| that would be sent with a request for |url|.
  ...
  void RunGetCookiesCallbackOnSystemCookies(const net::CookieOptions& options,
