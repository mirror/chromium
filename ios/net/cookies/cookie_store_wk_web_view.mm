...
// TODO ios/net can not use ios/chrome
#include "ios/chrome/browser/callback_counter.h"

// TODO ios/net can not use ios/web
...
@interface CRNCookieStoreObserver : NSObject<WKHTTPCookieStoreObserver> {
  id _block;
}
- (instancetype)initWithBlock:(void (^)(void))block;
@end

@implementation CRNCookieStoreObserver
- (instancetype)initWithBlock:(void (^)(void))block {
  self = [super init];
  if (self) {
    _block = [block copy];
...
NSHTTPCookie* GetNSHTTPCookieFromCookieLine(const std::string& cookie_line,
  ...
  NSArray* cookies = [NSHTTPCookie
      cookiesWithResponseHeaderFields:@{@"Set-Cookie" : ns_cookie_line}
                               forURL:nsurl];
  if ([cookies count] != 1)
    return nil;
...
void GetCookiesForURL(WKHTTPCookieStore* system_store,
                      const GURL& url,
                      CookieCreationTimeManager* manager,
                      void (^callback)(NSArray*)) {
  NSURL* ns_url = net::NSURLWithGURL(url);
  [system_store getAllCookies:^(NSArray<NSHTTPCookie*>* cookies) {
    ...
    [result sortUsingFunction:CompareCookies context:manager];

    // WKHTTPCookieStore calls callbacks on the main thread, while cookie store
    // operates on IO thread.
...
void GetAllCookies(WKHTTPCookieStore* system_store,
                   CookieCreationTimeManager* manager,
                   void (^callback)(NSArray*)) {
  [system_store getAllCookies:^(NSArray<NSHTTPCookie*>* cookies) {
    NSArray* c = cookies;
    callback([c sortedArrayUsingFunction:CompareCookies context:manager]);
  }];
}
...
// Understand what is the canonical way of using OnceCallback in blocks:
// https://docs.google.com/document/d/1jsaJAT-WbFS2ZwLEPkAgGcMTQ6MQLIQlyQfd1JeEupg/edit#heading=h.diu90qbbyngb
void CallCallback(base::OnceCallback<void()> callback) {
  if (!callback.is_null())
    std::move(callback).Run();
}
void CallDeleteCallback(base::OnceCallback<void(bool)> callback, bool arg) {
  if (!callback.is_null())
    std::move(callback).Run(arg);
}
void CallDeleteIntCallback(base::OnceCallback<void(int)> callback, int arg) {
  if (!callback.is_null())
    std::move(callback).Run(arg);
}
void CallGetCallback(
    base::OnceCallback<void(const std::string& cookie)> callback,
    const std::string& arg) {
  if (!callback.is_null())
    std::move(callback).Run(arg);
}
void CallListCallback(base::OnceCallback<void(const net::CookieList&)> callback,
                      const net::CookieList& arg) {
  if (!callback.is_null())
    std::move(callback).Run(arg);
}
...
#pragma mark -
#pragma mark CookieStoreWKWebView

CookieStoreWKWebView::CookieStoreWKWebView(
    CookieMonster::PersistentCookieStore* store,
    WKHTTPCookieStore* cookie_store)
    : cookie_monster_(new net::CookieMonster(store, nullptr)),
      system_store_(cookie_store),
      creation_time_manager_(new CookieCreationTimeManager),
      weak_factory_(this) {
  _observer = [[CRNCookieStoreObserver alloc] initWithBlock:^{
    this->OnSystemCookiesChanged();
  }];
  [system_store_ addObserver:_observer];
}

CookieStoreWKWebView::~CookieStoreWKWebView() {
  [system_store_ removeObserver:_observer];
}

void CookieStoreWKWebView::SetMetricsEnabled() {
  static CookieStoreWKWebView* g_cookie_store_with_metrics = nullptr;
...
void CookieStoreWKWebView::SetCookieWithOptionsAsync(
    const GURL& url,
  ...
  base::Callback<void(bool)> callback_copy =
      base::Bind(&CallDeleteCallback, base::Passed(&callback));
  if (success) {
...
void CookieStoreWKWebView::SetCookieWithDetailsAsync(
    const GURL& url,
    const std::string& name,
  ...
  if (canonical_cookie) {
    ...
    if (cookie != nil) {
      auto creation_date = canonical_cookie->CreationDate();
      [system_store_ setCookie:cookie
             completionHandler:^{
               creation_time_manager_->SetCreationTime(
                   cookie, creation_time_manager_->MakeUniqueCreationTime(
...
void CookieStoreWKWebView::SetCanonicalCookieAsync(
  ...
  if (ns_cookie != nil) {
    auto creation_date = cookie->CreationDate().is_null()
                             ? base::Time::Now()
                             : cookie->CreationDate();
    [system_store_ setCookie:ns_cookie
           completionHandler:^{
             creation_time_manager_->SetCreationTime(
                 ns_cookie,
                 creation_time_manager_->MakeUniqueCreationTime(creation_date));
             callback_copy.Run(true);
           }];
    return;
  }
...
void CookieStoreWKWebView::GetCookiesWithOptionsAsync(
    const GURL& url,
    const net::CookieOptions& options,
  ...
  base::Callback<void(const std::string&)> callback_copy =
      base::Bind(&CallGetCallback, base::Passed(&callback));
  GetCookiesForURL(
      system_store_, url, creation_time_manager_.get(), ^(NSArray* cookies) {
        callback_copy.Run(BuildCookieLineWithOptions(cookies, options));
      });
}

void CookieStoreWKWebView::GetCookieListWithOptionsAsync(
    const GURL& url,
    const net::CookieOptions& options,
  ...
  // TODO(mkwst): If/when iOS supports Same-Site cookies, we'll need to pass
  // options in here as well. https://crbug.com/459154
  base::Callback<void(const net::CookieList&)> callback_copy =
      base::Bind(&CallListCallback, base::Passed(&callback));
  GetCookiesForURL(
      system_store_, url, creation_time_manager_.get(), ^(NSArray* cookies) {
        callback_copy.Run(CanonicalCookieListFromSystemCookies(cookies));
      });
}

void CookieStoreWKWebView::GetAllCookiesAsync(GetCookieListCallback callback) {
  DCHECK(thread_checker_.CalledOnValidThread());
  if (callback.is_null())
    return;
  base::Callback<void(const net::CookieList&)> callback_copy =
      base::Bind(&CallListCallback, base::Passed(&callback));
  GetAllCookies(
      system_store_, creation_time_manager_.get(), ^(NSArray* cookies) {
        callback_copy.Run(CanonicalCookieListFromSystemCookies(cookies));
      });
}

void CookieStoreWKWebView::DeleteCookieAsync(const GURL& url,
  ...
  base::Callback<void()> callback_copy =
      base::Bind(&CallCallback, base::Passed(&callback));

  scoped_refptr<CallbackCounter> callback_counter =
      new CallbackCounter(callback_copy);
  GetCookiesForURL(
      system_store_, url, creation_time_manager_.get(), ^(NSArray* cookies) {
        for (NSHTTPCookie* cookie in cookies) {
          if ([[cookie name]
            ...
            [system_store_ deleteCookie:(NSHTTPCookie*)cookie
                      completionHandler:^{
...
void CookieStoreWKWebView::DeleteCanonicalCookieAsync(
...
void CookieStoreWKWebView::DeleteAllCreatedBetweenAsync(
...
void CookieStoreWKWebView::DeleteAllCreatedBetweenWithPredicateAsync(
    const base::Time& delete_begin,
    const base::Time& delete_end,
...
void CookieStoreWKWebView::DeleteSessionCookiesAsync(DeleteCallback callback) {
  DCHECK(thread_checker_.CalledOnValidThread());
...
void CookieStoreWKWebView::FlushStore(base::OnceClosure closure) {
  DCHECK(thread_checker_.CalledOnValidThread());

  base::Callback<void()> callback_copy =
      base::Bind(&CallCallback, base::Passed(&closure));
  [system_store_ getAllCookies:^(NSArray<NSHTTPCookie*>* cookies) {
    cookie_monster_->FlushStore(callback_copy);
...
void CookieStoreWKWebView::ClearSystemStore() {
  scoped_refptr<CallbackCounter> callback_counter =
...
void CookieStoreWKWebView::WriteToCookieMonster(NSArray* system_cookies) {
  DCHECK(thread_checker_.CalledOnValidThread());
...
void CookieStoreWKWebView::DeleteCookiesWithFilter(
    const CookieFilterFunction& filter,
    DeleteCallback callback) {
  if (callback.is_null())
    return;
  DCHECK(thread_checker_.CalledOnValidThread());
  NSMutableArray* to_delete = [NSMutableArray array];
  base::Callback<void(int)> callback_copy =
      base::Bind(&CallDeleteIntCallback, base::Passed(&callback));
  scoped_refptr<CallbackCounter> callback_counter =
      new CallbackCounter(base::BindBlockArc(^{
        callback_copy.Run(to_delete.count);
      }));

  [system_store_ getAllCookies:^(NSArray* cookies) {
    for (NSHTTPCookie* cookie in cookies) {
      base::Time creation_time =
          creation_time_manager_->GetCreationTime(cookie);
      if (filter.Run(cookie, creation_time))
        [to_delete addObject:cookie];

      for (NSHTTPCookie* cookie in to_delete) {
        callback_counter->IncrementCount();
        [system_store_ deleteCookie:(NSHTTPCookie*)cookie
                  completionHandler:^{
                    callback_counter->DecrementCount();
...
void CookieStoreWKWebView::OnSystemCookiesChanged() {
  DCHECK(thread_checker_.CalledOnValidThread());
  ...
  for (const auto& hook_map_entry : hook_map_) {
    ...
    UpdateCacheForCookieFromSystem(
        key.first, key.second,
        ^(const std::vector<net::CanonicalCookie>& removed_cookies,
          const std::vector<net::CanonicalCookie>& added_cookies) {
          RunCallbacksForCookies(
              key.first, key.second, removed_cookies,
          ...
          flush_closure_.Reset(base::Bind(&CookieStoreWKWebView::FlushStore,
                                          weak_factory_.GetWeakPtr(),
                                          base::Closure()));
          base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
              FROM_HERE, flush_closure_.callback(),
...
CookieStoreWKWebView::AddCallbackForCookie(
    const GURL& gurl,
    const std::string& name,
  ...
  if (hook_map_.count(key) == 0) {
    UpdateCacheForCookieFromSystem(gurl, name,
...
bool CookieStoreWKWebView::IsEphemeral() {
  return cookie_monster_->IsEphemeral();
}

void CookieStoreWKWebView::UpdateCacheForCookieFromSystem(
    const GURL& gurl,
    const std::string& name,
    void (^callback)(const std::vector<net::CanonicalCookie>&,
                     const std::vector<net::CanonicalCookie>&)) {
  DCHECK(thread_checker_.CalledOnValidThread());
  GetSystemCookies(
      gurl, name, ^(const std::vector<net::CanonicalCookie>& system_cookies) {
        std::vector<net::CanonicalCookie> out_removed_cookies;
        std::vector<net::CanonicalCookie> out_added_cookies;
        cookie_cache_->Update(gurl, name, system_cookies, &out_removed_cookies,
                              &out_added_cookies);
        callback(out_removed_cookies, out_added_cookies);
      });
}

void CookieStoreWKWebView::RunCallbacksForCookies(
    const GURL& url,
    const std::string& name,
...
void CookieStoreWKWebView::GetSystemCookies(
    const GURL& url,
    const std::string& name,
    void (^callback)(const std::vector<net::CanonicalCookie>&)) {
  DCHECK(thread_checker_.CalledOnValidThread());
  GetCookiesForURL(
      system_store_, url, creation_time_manager_.get(), ^(NSArray* nscookies) {
          ...
          if (nscookie.name.UTF8String == name) {
            net::CanonicalCookie canonical_cookie =
                CanonicalCookieFromSystemCookie(
                    nscookie,
                    creation_time_manager_->GetCreationTime(nscookie));
...
void CookieStoreWKWebView::GotCookieListFor(
...
void CookieStoreWKWebView::DidClearNSHTTPCookieStorageCookies(
  ...
  auto callback = base::BindOnce(
      &CookieStoreWKWebView::DidClearBinaryCookiesFileCookies,
...
void CookieStoreWKWebView::DidClearBinaryCookiesFileCookies(
    DeleteCallback callback,
...
void CookieStoreWKWebView::UpdateCachesFromCookieMonster() {
  DCHECK(thread_checker_.CalledOnValidThread());
  for (const auto& hook_map_entry : hook_map_) {
    std::pair<GURL, std::string> key = hook_map_entry.first;
    GetCookieListCallback callback =
        base::Bind(&CookieStoreWKWebView::GotCookieListFor,
                   weak_factory_.GetWeakPtr(), key);
    cookie_monster_->GetAllCookiesForURLAsync(key.first, std::move(callback));
  }
}

net::CookieList CookieStoreWKWebView::CanonicalCookieListFromSystemCookies(
    NSArray* cookies) {
  net::CookieList cookie_list;
