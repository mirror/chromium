...
int ChunkedDataStreamUploader::Upload() {
  ...
  if (is_final_chunk_) {
    ...
    // triggered, so ignore this piece and let this internal read remain
    // pending.
    if (bytes_read == 0) {
      pending_internal_read_ = true;
      return ERR_IO_PENDING;
    } else if (bytes_read < 0) {
      // UploadDataStream::Read() currenlty does not support to return failure,
      // so fail the request from the delegate level when NSInputStream returns
      // an error.
      delegate_->OnReadError(bytes_read);
      return ERR_IO_PENDING;
    }
  }
