...
void HttpProtocolHandlerCore::HandleStreamEvent(NSStream* stream,
  ...
  switch (event) {
    ...
    case NSStreamEventHasBytesAvailable: {
      if (chunked_uploader_) {
        pending_stream_ = (NSInputStream*)stream;
...
int HttpProtocolHandlerCore::OnRead(char* buf, int buf_len) {
  int bytes_read = 0;
  if (pending_stream_) {
    bytes_read = [pending_stream_ read:(unsigned char*)buf maxLength:buf_len];
  }
  return bytes_read;
}
