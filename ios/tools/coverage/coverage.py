...
# Name of the directory where code coverage html files are generated.
COVERAGE_HTML_DIR_NAME = 'coverage_html'

# Name of the final profdata file, and this file needs to be passed to
# "llvm-cov" command in order to call it properly.
...
def _GenerateLineByLineFileCoverageInHtml(target, profdata_path,
                                          line_coverage_per_file):
  """Generate line-by-line coverage for files in html using llvm-cov report.
  ...
  Args:
    ...
    line_coverage_per_file: A json object with the following format:
  ...
  Returns:
    A path to the directory where html files are generated.
  """
  ...
  output_dir_name = target + '_at_' + str(datetime.datetime.now()).replace(' ',
                                                                           '-')
  output_dir_path = os.path.join(src_root, BUILD_DIRECTORY,
                                 COVERAGE_HTML_DIR_NAME, output_dir_name)
  cmd.append('-output-dir=' + output_dir_path)

  for source_file in line_coverage_per_file.keys():
...
def _GeneratePerFileLineCoverageReport(target, profdata_path, paths):
  """Generates the per file line coverage report.
  ...
  line_coverage_report = _GenerateLineCoverageReport(target, profdata_path)
  return _FilterLineCoverageReport(line_coverage_report, paths)


def _DisplayTopLevelDirectoryLineCoverage(line_coverage_per_file, paths):
  """Displays the line coverage of top level directories.
  ...
  Args:
    line_coverage_per_file: A json object with the following format:
      ...
      ---- total_lines: int => Number of total lines.
      ---- executed_lines: int => Number of executed lines.
    paths: A list of top level directories.
  """
  coverage_per_path = collections.defaultdict(
      lambda: collections.defaultdict(lambda: 0))
  for file_name in line_coverage_per_file:
    total_lines = line_coverage_per_file[file_name]['total_lines']
    executed_lines = line_coverage_per_file[file_name]['executed_lines']

    matched_paths = _MatchFilePathWithDirectories(file_name, paths)
    for matched_path in matched_paths:
      coverage_per_path[matched_path]['total_lines'] += total_lines
      coverage_per_path[matched_path]['executed_lines'] += executed_lines

    if matched_paths:
      coverage_per_path['aggregate']['total_lines'] += total_lines
      coverage_per_path['aggregate']['executed_lines'] += executed_lines

  print '\nLine Coverage Report for Following Directories: ' + str(paths)
  for path in paths:
    print path + ':'
    _PrintLineCoverageStats(coverage_per_path[path]['total_lines'],
                            coverage_per_path[path]['executed_lines'])

  if len(paths) > 1:
    print 'In Aggregate:'
    _PrintLineCoverageStats(coverage_per_path['aggregate']['total_lines'],
                            coverage_per_path['aggregate']['executed_lines'])
...
def Main():
  ...
  line_coverage_per_file = _GeneratePerFileLineCoverageReport(
      target, profdata_path, args.path)
  _DisplayTopLevelDirectoryLineCoverage(line_coverage_per_file, args.path)
