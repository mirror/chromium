...
def _GeneratePerDirectoryLineCoverageReport(per_file_line_coverage_report):
  ...
  Args:
    per_file_line_coverage_report: A json object with the following format:
        dict => A dictionary of objects describing line covearge for files
        -- file_name: dict => Line coverage summary.
        ---- total: int => Number of total lines.
        ---- executed: int => Number of executed lines.

  Returns:
    A dictionary representing per directory line coverage report with format:

    dict => A dictionary of objects describing line covearge for directory
    -- dir_name: dict => Directory line coverage report.
    ---- data: Line coverage summary.
    ------ total: int => Number of total lines.
    ------ executed: int => Number of executed lines.
    ---- children: Set of sub-directories or sub-files.
  """
  ...
  per_dir_line_coverage_report = collections.defaultdict(
      lambda: {'summary': collections.defaultdict(lambda: 0),
               'children': set()})
  ...
  while paths_to_visit:
    ...
    if os.path.isfile(path):
      total = per_file_line_coverage_report[path]['summary']['total']
      executed = per_file_line_coverage_report[path]['summary']['executed']
    else:
      total = per_dir_line_coverage_report[path]['summary']['total']
      executed = per_dir_line_coverage_report[path]['summary']['executed']

    dir_path = os.path.dirname(path)
    per_dir_line_coverage_report[dir_path]['summary']['total'] += total
    per_dir_line_coverage_report[dir_path]['summary']['executed'] += executed
    per_dir_line_coverage_report[dir_path]['children'].add(path)
    paths_to_visit.append(dir_path)

  return per_dir_line_coverage_report
...
def _GeneratePerFileLineCoverageReport(target, profdata_path):
  ...
  for coverage_content in coverage_content_by_files:
    ...
    # the bug is fixed.
    try:
      total = int(coverage_data[total_lines_index])
      missed = int(coverage_data[missed_lines_index])
    except ValueError as e:
      continue

    executed = total - missed
    line_coverage_report[file_name]['summary']['total'] = total
    line_coverage_report[file_name]['summary']['executed'] = executed
...
def Main():
  ...
  per_dir_line_coverage_report = _GeneratePerDirectoryLineCoverageReport(
  ...
  _PrintLineCoverageStats(
      per_dir_line_coverage_report[norm_path]['summary']['total'],
      per_dir_line_coverage_report[norm_path]['summary']['executed'])
