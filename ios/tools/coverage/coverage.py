...
def _GenerateLineCoverageReport(target, profdata_path):
  ...
  Chrome, "llvm-cov export" takes too long to run, for example, it takes 5
  minutes for ios_chrome_unittests. Therefore, this script uses a more hacky
  solution to get code coverage data by calling "llvm-cov report", which is
  ...
  Executed\tInstantiations\tMissed Insts.\tExecuted\tLines\tMissed Lines\tCover
  ------------------------------------------------------------------------------
  ------------------------------------------------------------------------------
  ...
  # Strip out the noises. The first line is the header and the second line is a
  # '-' separator line. The last line is an empty line break, the second to last
  # line is the in total coverage and the third to last line is a '-' separator
  # line.
