...
def _CreateCoverageProfileDataForTarget(target, jobs_count, gtest_filter):
  ...
  Args:
    ...
    gtest_filter: Only run unit tests whose full name matches the filter.
...
def _GetProfileRawDataPathByRunningTarget(target, gtest_filter):
...
def _RunTestTargetWithCoverageConfiguration(target, gtest_filter):
  ...
  # For iossim arguments, please refer to https://chromium.googlesource.com/chromium/src/+/master/testing/iossim/iossim.mm
  if gtest_filter:
...
def _ParseCommandArguments():
  ...
  arg_parser.add_argument('-c', '--cmd-args', action='append',
                          help='Command arguments that will be passed to the '
                               'test application, and one example use case is '
                               'to filter unit tests by passing '
