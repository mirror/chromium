...
COVERAGE_HTML_DIR_NAME = 'coverage_html'
...
def _ShowLineCoverageDetailsInHtml(target, profdata_path, line_coverage_by_file):
  """Show per-directories and line-by-line coverage in html.
  TODO
  """
  output_dir_path = _GenerateLineByLineFileCoverageInHtml(target, profdata_path, line_coverage_by_file)
  _GenerateDirectoryCoverageInHtml(line_coverage_by_file, output_dir_path)


def _GenerateLineByLineFileCoverageInHtml(target,profdata_path, line_coverage_by_file):
  """
  TODO
  """
  ...
  output_dir_name = target + '_at_' + str(datetime.datetime.now())
  output_dir_path = os.path.join(src_root, BUILD_DIRECTORY, COVERAGE_HTML_DIR_NAME, output_dir_name)
  cmd.append('-output-dir=' + output_dir_path)

  for source_file in line_coverage_by_file.keys():
...
def _GenerateDirectoryCoverageInHtml(coverage_by_file, output_dir_path):
  """
  TODO
  """
  coverage_by_dir, sub_paths_by_dir = _GenerateDirectoryCoverage(coverage_by_file)

  assert False

def _GenerateDirectoryCoverage(coverage_by_file):
  # Get line coverage data and list of sub-directories per-directory.
  coverage_by_dir = collections.defaultdict(lambda: collections.defaultdict(lambda: 0))
  sub_paths_by_dir = collections.defaultdict(lambda: set())

  # Imagine all the file paths as nodes in the tree at the bottom level, and the
  # following algorithm visits all the nodes (including intermidiate 
  # directories) in a layer-to-layer fashion from bottom to top.
  paths_to_visit = collections.deque()
  paths_to_visit.extend(coverage_by_file.keys())
  visited = set()
  ...
  while len(paths_to_visit) > 0:
    ...
    if path in coverage_by_file:
      coverage_by_dir[parent_path]['total_lines'] += coverage_by_file[path]['total_lines']
      coverage_by_dir[parent_path]['executed_lines'] += coverage_by_file[path]['executed_lines']
    else:
      coverage_by_dir[parent_path]['total_lines'] += coverage_by_dir[path]['total_lines']
      coverage_by_dir[parent_path]['executed_lines'] += coverage_by_dir[path]['executed_lines']

    sub_paths_by_dir[parent_path].add(path)
    paths_to_visit.append(parent_path)

  return coverage_by_dir, sub_paths_by_dir


def _GetPerDirectoriesCoverageHtmlTemplate():
  ...
  <html>
    ...
    <body>
      ...
      <p>Click 
        <a href='http://clang.llvm.org/docs/SourceBasedCodeCoverage.html#interpreting-reports'>
          here
        </a> for information about interpreting this report.
      </p>
      <div class='centered'>
        <table>
          <tr>
            <td class='column-entry-left'>Directory/File</td>
            <td class='column-entry'>Line Coverage</td>
          </tr>
          {entries}
        </table>
      </div>
      <h5>Generated by llvm-cov -- llvm version 9.0.0</h5>
    </body>
  </html>"""


def _GetPerDirectoriesCoverageHtmlTableEntry(dir_or_file_name, executed_lines, total_lines, output_dir_path):
  entry = R"""<tr class='light-row'>
            <td>
              <pre>
                <a href='{dir_or_file_html_path}'>{dir_or_file_name}</a>
              </pre>
            </td>
            <td class='{color}'>
              <pre>  {percentage}% ({executed_lines}/{total_lines})</pre>
            </td>
          </tr>"""
...
def _GenerateAndDisplayLineCoverageReport(target, profdata_path, paths):
...
def Main():
  ...
  line_coverage_report = _GenerateAndDisplayLineCoverageReport(
        target, profdata_path, args.path)
  _ShowLineCoverageDetailsInHtml(target, profdata_path, line_coverage_report)
