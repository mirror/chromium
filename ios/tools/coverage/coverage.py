...
def _GenerateLineByLineFileCoverageInHtml(
    targets, profdata_path, file_line_coverage_report, output_dir):
  ...
  Args:
    ...
    profdata_path: An absoluate path to the profdata file.
    file_line_coverage_report: A FileLineCoverageReport object.
  ...
  cmd = ['xcrun', 'llvm-cov', 'show', '-arch=x86_64', '-format=html',
         ...
         '-instr-profile=' + profdata_path, binary_paths[0]]
  cmd.extend(['-object=' + binary_paths[i] for i in range(len(binary_paths))])
...
def _CreateCoverageProfileDataForTargets(targets, output_dir, jobs_count=None,
  ...
  profraw_paths = _GetProfileRawDataPathsByRunningTargets(targets, gtest_filter)
  profdata_path = _CreateCoverageProfileDataFromProfRawData(profraw_paths,
                                                            output_dir)

  print 'Code coverage profile data is created as: ' + profdata_path
  return os.path.abspath(profdata_path)


def _GeneratePerFileLineCoverageReport(targets, profdata_path):
  ...
  Args:
    targets: A list of targets to be tested.
    profdata_path: An absolute path to the profdata file.
...
def _GetProfileRawDataPathsByRunningTargets(targets, gtest_filter=None):
  ...
  for log in logs:
    profraw_found = False
    for line in log:
      if PROFRAW_LOG_IDENTIFIER in line:
        profraw_found = True
        profraw_path = line.split(PROFRAW_LOG_IDENTIFIER)[1][:-1]
        profraw_paths.append(profraw_path)
        break

    if not profraw_found:
...
def _CreateCoverageProfileDataFromProfRawData(profraw_paths, output_dir):
  """Returns the path to the profdata file by merging profraw data file.

  Args:
    profraw_paths: A list of absolute paths to the profraw data files that are
                   to be merged.
    output_dir: A directory to store created coverage profile data file, the
                path needs to be absolute or relative to the root of checkout.
  ...
  try:
    cmd = ['xcrun', 'llvm-profdata', 'merge', '-o', profdata_path]
    cmd.extend(profraw_paths)
    subprocess.check_call(cmd)
...
def Main():
  ...
  if profdata_path:
    ...
    profdata_path = _CreateCoverageProfileDataForTargets(
        args.targets, args.output_dir, jobs, args.gtest_filter)

  print 'Generating code coverge report'
  file_line_coverage_report = _GeneratePerFileLineCoverageReport(
      args.targets, profdata_path)
  ...
  _GenerateLineByLineFileCoverageInHtml(
      args.targets, profdata_path, file_line_coverage_report, args.output_dir)
