...
def DisplayLineCoverageReport(target, profdata_path, filter_paths):
  ...
  """
  coverage_json = _ExportCodeCoverageJson(target, profdata_path)
  ...
  for filter_path in filter_paths:
    coverage_by_filter[filter_path] = {'total_lines': 0, 'executed_lines': 0}
  coverage_by_filter['aggregate'] = {'total_lines': 0, 'executed_lines': 0}
...
def _ExportCodeCoverageJson(target, profdata_path):
  ...
  Returns:
    A json object with the above mentioned format.
  """
  ...
  coverage_json = json.loads(std_out)
...
def _GenerateLineCoverageReport(coverage_json):
  ...
  This function basically strips out information that is not related to line
  coverage summary, and the output has the following format:
  ...
  Args:
    coverage_json: A json object with above mentioned format.
...
def _FilterLineCoverageReport(raw_report, filter_paths):
  """Filter line coverage report by directories.

  The line coverage report has the following format:
  Root: array => List of objects describing line covearge summary for files
  -- File: dict => Line coverage summary for a single file
  ...
  Args:
    raw_report: A json object with the above mentioned format:
    filter_paths: A list of directories used to restrict code coverage results.
...
def _PrintLineCoverageStats(total_lines, executed_lines):
  ...
  percentage_coverage = '{}%'.format(int(coverage * 100)) if coverage else 'NA'
...
def _AssertFilterPathsExist(filter_paths):
  ...
  for filter_path in filter_paths:
    ...
    if not os.path.exists(filter_abspath):
      assert False, (('Filter path: {} doesn\'t exist.\nA valid filter path '
                      'must exist and be relative to the root of source, '
                      'which is {} \nFor example, \'ios/\' is a valid filter.')
                     .format(filter_abspath, src_root))
...
def Main():
  ...
  if not filter_paths:
    filter_paths = DEFAULT_FILTER_PATHS

  _AssertCoverageBuildDirectoryExists()
  _AssertFilterPathsExist(filter_paths)

  profdata_path = CreateCoverageProfileDataForTarget(target, jobs)
  DisplayLineCoverageReport(target, profdata_path, filter_paths)
