...
DIRECTORY_HTML_REPORT_NAME = 'coverage.html'
...
class _DirectoryCoverageReportHtmlGenerator(object):
  ...
  def __init__(self, css_path):
    ...
    header_template_path = os.path.join(templates_dir, 'header.html')
    with open(header_template_path, 'r') as header_file:
      header_template = header_file.read()
      self._header_template = header_template.format(os.path.abspath(css_path))

    # Read table directory header template.
    directory_header_template_path = os.path.join(
        templates_dir, 'directory_header.html')
    with open(directory_header_template_path, 'r') as directory_header_file:
      self._directory_header_template = directory_header_file.read()

    # Read table file header template.
    file_header_template_path = os.path.join(templates_dir, 'file_header.html')
    with open(file_header_template_path, 'r') as file_header_file:
      self._file_header_template = file_header_file.read()

    # Read table file/directory entry template.
    entry_template_path = os.path.join(templates_dir, 'entry.html')
    with open(entry_template_path, 'r') as entry_file:
      self._entry_template = entry_file.read()

    # Read html footer template.
    footer_template_path = os.path.join(templates_dir, 'footer.html')
    with open(footer_template_path, 'r') as footer_file:
      self._footer_template = footer_file.read()

  def AddFileEntry(self, html_report_path, name, total_lines, executed_lines):
    """Adds a file entry to the directory html coverage report.

    Args:
      html_report_path: A path to the file's html report.
      name: Base name of the file.
      total_lines: total number of lines.
      executed_lines: executed number of lines.
    """
    self._file_entries.append({'href': html_report_path,
                               'name': name,
                               'total': total_lines,
                               'executed': executed_lines})

  def AddDirectoryEntry(self, html_report_path, name, total_lines,
                        executed_lines):
    """Adds a sub-directory entry to the directory html coverage report.

    Args:
      html_report_path: A path to the directory's html report.
      name: Base name fo the directory.
      total_lines: total number of lines.
      executed_lines: executed number of lines.
    """
    self._dir_entries.append({'href': html_report_path,
                              'name': name,
                              'total': total_lines,
                              'executed': executed_lines})
  ...
  def WriteHtmlCoverageReport(self, output_path):
    ...
    """
    self._file_entries.sort(
        key=lambda entry: float(entry['executed']) / entry['total'])
    self._dir_entries.sort(
        key=lambda entry: float(entry['executed']) / entry['total'])

    dirs_html = ''.join([self._GenerateHtmlEntry(dir_entry['href'],
                                                 dir_entry['name'],
                                                 dir_entry['total'],
                                                 dir_entry['executed'])
                         for dir_entry in self._dir_entries])

    files_html = ''.join([self._GenerateHtmlEntry(file_entry['href'],
                                                  file_entry['name'],
                                                  file_entry['total'],
                                                  file_entry['executed'])
                          for file_entry in self._file_entries])

    table_html = (self._directory_header_template + dirs_html +
                  self._file_header_template + files_html)
    html = self._header_template + table_html + self._footer_template
    with open(output_path, 'w') as html_file:
      html_file.write(html)

  def _GenerateHtmlEntry(self, href, name, total_lines, executed_lines):
    """Generates a file or sub-directory coverage entry for the report.

    Args:
      href: A path to the entry's coverage html report.
      name: Name of the entry for displaying.
      total_lines: total number of lines.
      executed_lines: executed number of lines.

    Returns:
      Returns a piece of html with the following format ({variable} is to be
      replaced):

      <tr class='light-row'>
        <td>
          <pre>
            <a href='{href to the entry's html report}'>{name of the entry}</a>
          </pre>
        </td>
        <td class='{css class indicating color of the entry}'>
          <pre> {coverage percentage}% ({#executed lines}/{#total lines})</pre>
        </td>
      </tr>
    """
    ...
    if coverage < 0.8:
      ...
      assert False, ('coverage cannot be greater than 100%, however, {} has '
                     'coverage: {}').format(href, coverage)

    percentage_coverage = round(coverage * 100, 2)
    return self._entry_template.format(href, name, color_class,
                                       percentage_coverage, total_lines,
                                       executed_lines)
...
def _GenerateLineByLineFileCoverageInHtml(
  ...
  Returns:
    A path to the directory that contains all the generated html files.
  """
  ...
  return os.path.abspath(os.path.join(output_dir, 'coverage'))


def _GeneratePerDirectoryCoverageHtmlReport(dir_line_coverage_report,
                                            file_line_coverage_report,
                                            html_reports_root_path):
  ...
  Args:
    ...
    file_line_coverage_report: A FileLineCoverageReport object.
    html_reports_root_path: A path to the directory that contains all the
...
def _GenerateCoverageHtmlReportForDirectory(dir_path, dir_line_coverage_report,
  ...
  Args:
    ...
    html_reports_root_path: A path to the directory that contains all the
                            generated html files.
  """
  css_path = os.path.join(os.path.dirname(html_reports_root_path), 'style.css')
  ...
  for sub_name in os.listdir(dir_path):
    ...
    sub_path_html_report_path = _GetCoverageHtmlReportPath(
        sub_path, html_reports_root_path)

    if dir_line_coverage_report.ContainsDirectory(sub_path):
      total_lines, executed_lines = (
          dir_line_coverage_report.GetCoverageForDirectory(sub_path))
      html_generator.AddDirectoryEntry(sub_path_html_report_path,
                                       os.path.basename(sub_path),
                                       total_lines, executed_lines)
    elif file_line_coverage_report.ContainsFile(sub_path):
      total_lines, executed_lines = (
          file_line_coverage_report.GetCoverageForFile(sub_path))
      html_generator.AddFileEntry(sub_path_html_report_path,
                                  os.path.basename(sub_path),
                                  total_lines, executed_lines)

  html_generator.WriteHtmlCoverageReport(
      _GetCoverageHtmlReportPath(dir_path, html_reports_root_path))


def _GetCoverageHtmlReportPath(file_or_dir_path, html_reports_root_path):
  ...
  """
  html_path = html_reports_root_path + os.path.abspath(file_or_dir_path)
  if os.path.isdir(file_or_dir_path):
    return os.path.join(html_path, DIRECTORY_HTML_REPORT_NAME)
  else:
...
def _OverwriteHtmlReportsIndexFile(top_level_dir, dir_line_coverage_report,
  ...
  Args:
    ...
    html_reports_root_path: A path to the directory that contains all the
                            generated html files.
  """
  css_path = os.path.join(os.path.dirname(html_reports_root_path),
                          'style.css')
  ...
  index_html_generator.AddDirectoryEntry(
      html_report_path, os.path.basename(top_level_dir), total_lines,
      executed_lines)

  index_file_path = os.path.join(os.path.dirname(html_reports_root_path),
                                 'index.html')
...
def Main():
  ...
  html_files_root_path = _GenerateLineByLineFileCoverageInHtml(
      target, profdata_path, file_line_coverage_report, args.output_dir)

  _GeneratePerDirectoryCoverageHtmlReport(
      dir_line_coverage_report, file_line_coverage_report, html_files_root_path)
