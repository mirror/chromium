...
- (CWVIdentity*)currentIdentity {
  ...
  AccountInfo accountInfo =
      accountTracker->GetAccountInfo([self authenticatedAccountID]);
...
- (ProfileOAuth2TokenServiceIOSDelegate*)tokenServiceDelegate {
  ProfileOAuth2TokenService* tokenService = [self tokenService];
  return static_cast<ProfileOAuth2TokenServiceIOSDelegate*>(
      tokenService->GetDelegate());
}

- (WebViewProfileOAuth2TokenServiceIOSProviderImpl*)tokenServiceProvider {
  ProfileOAuth2TokenServiceIOSDelegate* tokenServiceDelegate =
      [self tokenServiceDelegate];
  return static_cast<WebViewProfileOAuth2TokenServiceIOSProviderImpl*>(
