...
@implementation CWVAutofillController {
  std::unique_ptr<web::WebStateObserverBridge> _webStateObserverBridgeUniquePtr;
  std::unique_ptr<autofill::AutofillClientIOS> _autofillClientUniquePtr;
}

@synthesize autofillAgent = _autofillAgent;
@synthesize autofillManager = _autofillManager;
@synthesize delegate = _delegate;
@synthesize JSAutofillManager = _JSAutofillManager;
...
- (void)fetchSuggestionsForFormWithName:(NSString*)formName
  ...
  id availableHandler = ^(BOOL suggestionsAvailable) {
    ...
    id retrieveHandler = ^(NSArray* suggestions,
      ...
      completionHandler(autofillSuggestions);
    };
...
- (void)setWebState:(web::WebState*)webState {
  ...
  _webStateObserverBridgeUniquePtr.reset(
  ...
  std::unique_ptr<IdentityProvider> identityProvider(
      new ProfileIdentityProvider(
          ...
          ios_web_view::WebViewOAuth2TokenServiceFactory::GetForBrowserState(
              browserState),
          base::Closure()));
  _autofillClientUniquePtr.reset(new autofill::AutofillClientIOS(
  ...
  autofill::AutofillDriverIOS::CreateForWebStateAndDelegate(
      webState, _autofillClientUniquePtr.get(), self,
...
- (autofill::AutofillClientIOS*)autofillClient {
  return _autofillClientUniquePtr.get();
}

- (web::WebStateObserverBridge*)webStateObserverBridge {
  return _webStateObserverBridgeUniquePtr.get();
}
