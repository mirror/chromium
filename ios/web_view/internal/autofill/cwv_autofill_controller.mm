...
- (void)checkIfPreviousAndNextFieldsAreAvailableForFocusWithCompletionHandler:
    (nullable void (^)(BOOL previous, BOOL next))completionHandler {
  [_JSSuggestionManager
      fetchPreviousAndNextElementsPresenceWithCompletionHandler:^(BOOL previous,
                                                                  BOOL next) {
        if (completionHandler) {
          completionHandler(previous, next);
        }
      }];
}
