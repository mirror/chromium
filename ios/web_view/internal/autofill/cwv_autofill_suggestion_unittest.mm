...
TEST_F(CWVAutofillSuggestionTest, Initialization) {
  ...
  CWVAutofillSuggestion* suggestion =
      [[CWVAutofillSuggestion alloc] initWithFormSuggestion:formSuggestion
                                                   formName:formName
                                                  fieldName:fieldName];
  EXPECT_NSEQ(suggestion.formName, formName);
  EXPECT_NSEQ(suggestion.fieldName, fieldName);
  EXPECT_NSEQ(suggestion.displayDescription, formSuggestion.displayDescription);
  EXPECT_NSEQ(suggestion.value, formSuggestion.value);
}
