...
static NSString* kTestFieldLabel = @"FieldLabel";
...
TEST_F(CWVAutofillControllerTest, FetchSuggestions) {
  ...
  field.label = base::SysNSStringToUTF16(kTestFieldLabel);
...
TEST_F(CWVAutofillControllerTest, FillAndClearForm) {
  ...
  ExecuteJavaScript(@"document.forms[0][0].focus();");

  __block BOOL formFilled = NO;
  __block BOOL formCleared = NO;
  id formClearedBlock = ^{
    formCleared = YES;
    NSString* value = ExecuteJavaScript(@"document.forms[0][0].value;");
    EXPECT_NSEQ(@"", value);
  };
  id formFilledBlock = ^{
    formFilled = YES;
  ...
  @autoreleasepool {
    ...
    [[delegate expect] autofillController:_autofillController
                   didBlurOnFieldWithName:kTestFieldName
                                 formName:kTestFormName
                                    value:kTestFieldValue];
    ExecuteJavaScript(@"document.forms[0][0].blur();");
    ...
    ExecuteJavaScript(@"document.forms[0][0].dispatchEvent(event);");
