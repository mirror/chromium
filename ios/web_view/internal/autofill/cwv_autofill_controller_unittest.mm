    ...
    FormSuggestion* suggestion =
        [FormSuggestion suggestionWithValue:kTestFieldValue
                         displayDescription:nil
                                       icon:nil
                                 identifier:0];
    suggestions[kTestFormName] = @[ suggestion ];
    FakeAutofillAgent* autofill_agent = [[FakeAutofillAgent alloc]
          initWithPrefService:browser_state_->GetPrefs()
                     webState:web_state_.get()
        suggestionsByFormName:suggestions];

    JsAutofillManager* js_autofill_manager =
