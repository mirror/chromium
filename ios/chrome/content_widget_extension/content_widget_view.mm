...
- (CGFloat)widgetExpandedHeight {
  // The expanded height must be at least as tall as the compact height as
  // widgets cannot be smaller than this. The prefered height for the widget
  // when expanded is the height of the two rows of sites + padding at top,
  // bottom and between the two rows.
  return MAX(2 * kTileHeight + 3 * kTileSpacing, self.compactHeight);
}
...
- (CGFloat)firstRowHeight:(BOOL)compact {
  ...
  CGFloat secondRowHeight = kTileHeight + kTileSpacing;
