...
+ (BOOL)continueUserActivity:(NSUserActivity*)userActivity
  ...
  if ([userActivity.activityType
    ...
    // We don't want to call continueUserActivityURL if the launchPaymentRequest
    // flag is set as we will need to handle this startup parameters in the
    // handleStartupParametersWithTabOpener function.
...
+ (void)handleStartupParametersWithTabOpener:(id<TabOpening>)tabOpener
  ...
  if ([U2FController
    ...
    if ([tabOpener shouldStayOnTabWithStartupInformation:startupInformation])
      return;
