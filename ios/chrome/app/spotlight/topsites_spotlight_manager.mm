...
#include "ios/chrome/browser/ui/ntp/google_landing_view_controller.h"
...
- (void)onMostVisitedURLsAvailable:
  ...
  NSUInteger sitesToIndex =
      MIN(top_sites.size(), [GoogleLandingViewController maxSitesShown]);
...
- (void)onSuggestionsProfileAvailable:
  ...
  if (size) {
    NSUInteger sitesToIndex =
        MIN(size, [GoogleLandingViewController maxSitesShown]);
    for (size_t i = 0; i < sitesToIndex; i++) {
