...
- (void)createInitialUI:(ApplicationMode)launchMode {
  ...
  if (firstRun || [self shouldOpenNTPTabOnActivationOfTabModel:tabModel]) {
    NewTabCommand* command = [[NewTabCommand alloc]
        initWithIncognito:(self.currentBVC == self.otrBVC)];
...
- (void)scheduleShowPromo {
  ...
  // Show the signing promo if needed
...
- (void)switchModesAndOpenNewTab:(NewTabCommand*)command {
...
- (void)closeSettingsAndOpenNewIncognitoTab {
  [self closeSettingsAnimated:NO
                   completion:^{
                     [self switchModesAndOpenNewTab:[[NewTabCommand alloc]
