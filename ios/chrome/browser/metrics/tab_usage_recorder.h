...
class TabUsageRecorder : public WebStateListObserver {
 public:
  ...
  // Initialises the TabUsageRecorder to watch |web_state_list|.
  explicit TabUsageRecorder(WebStateList* web_state_list);
  ...
  // user began interacting with |active_web_state|, |primary| should be true.
  // If the user stopped interacting with |active_web_state|, |primary| should
  // be false.
  void RecordPrimaryTabModelChange(bool primary,
  ...
  void RendererTerminated(web::WebState* web_state, bool visible);
 ...
 private:
  ...
  bool ShouldIgnoreTab(web::WebState* web_state);

  // Whether or not the tab has already been evicted.
  bool TabAlreadyEvicted(web::WebState* web_state);

  // Returns the state of the given tab.  Call only once per tab, as it removes
  // the tab from |evicted_tabs_|.
  TabStateWhenSelected ExtractTabState(web::WebState* web_state);
  ...
  // The WebStateList that is contained all the monitored tabs.
