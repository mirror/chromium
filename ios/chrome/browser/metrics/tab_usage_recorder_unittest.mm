 ...
 protected:
  TabUsageRecorderTest()
      : web_state_list_(&web_state_list_delegate_),
        tab_usage_recorder_(&web_state_list_) {}
...
TEST_F(TabUsageRecorderTest, SwitchBetweenInMemoryTabs) {
  web::WebState* web_state_a = InsertTestWebState(kURL, true);
  web::WebState* web_state_b = InsertTestWebState(kURL, true);

  tab_usage_recorder_.RecordTabSwitched(web_state_a, web_state_b);
...
TEST_F(TabUsageRecorderTest, SwitchToEvictedTab) {
  web::WebState* web_state_a = InsertTestWebState(kURL, true);
  web::WebState* web_state_b = InsertTestWebState(kURL, false);
...
TEST_F(TabUsageRecorderTest, SwitchFromEvictedTab) {
  web::WebState* web_state_a = InsertTestWebState(kURL, false);
  web::WebState* web_state_b = InsertTestWebState(kURL, true);
...
TEST_F(TabUsageRecorderTest, CountPageLoadsBeforeEvictedTab) {
  ...
  for (int i = 0; i < kNumReloads; i++) {
    tab_usage_recorder_.RecordPageLoadStart(web_state_a);
  }
  tab_usage_recorder_.RecordTabSwitched(web_state_a, web_state_b);
...
TEST_F(TabUsageRecorderTest, CountNativePageLoadsBeforeEvictedTab) {
  web::WebState* web_state_a = InsertTestWebState(kNativeURL, true);
  web::WebState* web_state_b = InsertTestWebState(kNativeURL, false);
...
TEST_F(TabUsageRecorderTest, TestColdStartTabs) {
  web::WebState* web_state_a = InsertTestWebState(kURL, false);
  web::WebState* web_state_b = InsertTestWebState(kURL, false);
  web::WebState* web_state_c = InsertTestWebState(kURL, false);
  // Set A and B as cold-start evicted tabs.  Leave C just evicted.
  std::vector<web::WebState*> cold_start_web_states = {
      web_state_a, web_state_b,
  };
  tab_usage_recorder_.InitialRestoredTabs(web_state_a, cold_start_web_states);

  // Switch from A (cold start evicted) to B (cold start evicted).
  tab_usage_recorder_.RecordTabSwitched(web_state_a, web_state_b);
  // Switch from B (cold start evicted) to C (evicted).
  tab_usage_recorder_.RecordTabSwitched(web_state_b, web_state_c);
...
TEST_F(TabUsageRecorderTest, TestEvictedTabReloadTime) {
  ...
  web::WebState* web_state_b = InsertTestWebState(kURL, false);
  tab_usage_recorder_.RecordTabSwitched(web_state_a, web_state_b);
  tab_usage_recorder_.RecordPageLoadStart(web_state_b);
  tab_usage_recorder_.RecordPageLoadDone(web_state_b, true);
...
TEST_F(TabUsageRecorderTest, TestEvictedTabReloadFailure) {
  ...
  tab_usage_recorder_.RecordTabSwitched(web_state_a, web_state_b);
  tab_usage_recorder_.RecordPageLoadStart(web_state_b);
  tab_usage_recorder_.RecordPageLoadDone(web_state_b, false);
  histogram_tester_.ExpectUniqueSample(kEvictedTabReloadSuccessRate,
...
TEST_F(TabUsageRecorderTest, TestUserWaitedForEvictedTabLoad) {
  ...
  tab_usage_recorder_.RecordPageLoadStart(web_state_b);
  tab_usage_recorder_.RecordPageLoadDone(web_state_b, true);
  tab_usage_recorder_.RecordTabSwitched(web_state_b, web_state_a);
...
TEST_F(TabUsageRecorderTest, RendererTerminated) {
  ...
  // as alive when web_state_a is evicted.
  ...
  tab_usage_recorder_.RendererTerminated(web_state_a, false);
...
TEST_F(TabUsageRecorderTest, SwitchToRendererTerminatedTab) {
  ...
  tab_usage_recorder_.RendererTerminated(web_state_b, false);
  tab_usage_recorder_.RecordTabSwitched(web_state_a, web_state_b);
