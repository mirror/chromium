...
void RecordRepeatedExternalApplicationPromptBlocked(bool blocked) {
...
- (void)openExternalAppWithURL:(NSURL*)URL
  ...
  [self showExternalAppLauncherPrompt:prompt
                      ...
                      responseHandler:^(BOOL accept) {
                        ...
                        RecordExternalApplicationOpened(accept);
                      }];
}

- (BOOL)requestURLOpen:(const GURL&)toURL
  ...
  NSString* key =
      base::SysUTF8ToNSString(fromURL.GetContent() + " " + toURL.scheme());
  ...
  switch (policy) {
    ...
    case ExternalAppLaunchPolicyPrompt: {
      ...
      [self
          showExternalAppLauncherPrompt:l10n_util::GetNSString(
                                            IDS_IOS_OPEN_REPEATEDLY_ANOTHER_APP)
                            acceptLabel:
                                l10n_util::GetNSString(
                                    IDS_IOS_OPEN_REPEATEDLY_ANOTHER_APP_ALLOW)
                            rejectLabel:
                                l10n_util::GetNSString(
                                    IDS_IOS_OPEN_REPEATEDLY_ANOTHER_APP_BLOCK)
                        responseHandler:^(BOOL allowed) {
                          if (weakSelf) {
                            if (allowed) {
                              ...
                              // TODO(crbug.com/674649): Once non modal dialogs
                              // are implemented, update this to always prompt
                              // instead of blocking the app.
                              [state setStateBlocked];
                            }
                            RecordRepeatedExternalApplicationPromptBlocked(
                                !allowed);
                            weakSelf.promptActive = NO;
                          }
                        }];
      return YES;
    }
