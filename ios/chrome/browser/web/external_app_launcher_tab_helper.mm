...
// TBD
void LaunchExternalApp(NSURL* url, bool shouldLaunchApp) {
  UMA_HISTOGRAM_BOOLEAN("Tab.ExternalApplicationOpened", shouldLaunchApp);
  if (shouldLaunchApp)
...
void OpenExternalAppWithUrl(NSURL* url,
                            NSString* prompt,
                            NSString* open_label) {
  ShowExternalAppLauncherPrompt(prompt, open_label,
                                l10n_util::GetNSString(IDS_CANCEL),
                                base::Bind(&LaunchExternalApp, url));
}
