...
void ShowExternalAppLauncherPrompt(
    NSString* prompt,
...
void OpenExternalAppWithUrl(NSURL* url,
  ...
  ShowExternalAppLauncherPrompt(
      prompt, open_label /* accept_label */,
      l10n_util::GetNSString(IDS_CANCEL) /* reject_label */,
      base::BindRepeating(&LaunchExternalApp, url));
}
...
BOOL OpenUrl(const GURL& gurl, BOOL link_clicked) {
  ...
  if (base::ios::IsRunningOnOrLater(10, 3, 0)) {
    ...
    if (UrlHasPhoneCallScheme(gurl)) {
      OpenExternalAppWithUrl(url, FormatCallArgument(url) /* prompt */,
                             PromptActionString([url scheme]) /* open_label */);
      return YES;
    }
...
BOOL ExternalAppLauncherTabHelper::RequestToOpenUrl(const GURL& url,
  ...
  switch (policy) {
    ...
    case ExternalAppLaunchPolicyPrompt: {
      ...
      base::RepeatingCallback<void(bool)> callback = base::BindRepeating(
