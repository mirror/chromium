...
- (void)setUp {
  [super setUp];
  [ChromeTestCase testServer]->RegisterRequestHandler(
      base::Bind(&RedirectAndWindowLocationHandlers));
  GREYAssertTrue([ChromeTestCase testServer]->Start(),
...
- (void)testHistoryGoNoParameter {
  ...
  const GURL windowHistoryURL =
      [ChromeTestCase testServer]->GetURL(kWindowHistoryGoTestURL);
...
- (void)testHistoryGoDelta {
  const GURL firstURL =
      [ChromeTestCase testServer]->GetURL(kWindowHistoryGoTestURL);
  const GURL secondURL =
      [ChromeTestCase testServer]->GetURL("/memory_usage.html");
  const GURL thirdURL =
      [ChromeTestCase testServer]->GetURL(kSimpleFileBasedTestURL);
  const GURL fourthURL = [ChromeTestCase testServer]->GetURL("/history.html");
...
- (void)testWindowLocationChangeHash {
  const GURL page1URL = [ChromeTestCase testServer]->GetURL(kPage1URL);
  const GURL hashChangedWithHistoryURL =
      [ChromeTestCase testServer]->GetURL(kHashChangedWithHistoryURL);
...
- (void)testWindowLocationReplaceAndChangeHash {
  ...
  const GURL hashChangedWithoutHistoryURL =
      [ChromeTestCase testServer]->GetURL(kHashChangedWithoutHistoryURL);
...
- (void)testJavaScriptRedirect {
  // A starting page.
  const GURL initialURL = [ChromeTestCase testServer]->GetURL(kDefaultPageURL);
  // A page that redirects immediately via the window.open JavaScript method.
  const GURL originURL =
      [ChromeTestCase testServer]->GetURL(kRedirectWindowURL);
  const GURL destinationURL =
      [ChromeTestCase testServer]->GetURL(kDestinationURL);
...
- (void)verifyBackAndForwardAfterRedirect:(std::string)redirectLabel {
  const GURL indexURL([ChromeTestCase testServer]->GetURL(kRedirectIndexURL));
  const GURL destinationURL(
      [ChromeTestCase testServer]->GetURL(kDestinationURL));
  const GURL lastURL([ChromeTestCase testServer]->GetURL(kDefaultPageURL));
