...
- (void)testHistoryGoNoParameter {
  GREYAssertTrue([super testServer]->Start(), @"Test server failed to start.");
  // Load the history test page and ensure that its onload text is visible.
  const GURL windowHistoryURL =
      [super testServer]->GetURL(kWindowHistoryGoTestURL);
...
- (void)testHistoryGoDelta {
  GREYAssertTrue([super testServer]->Start(), @"Test server failed to start.");
  const GURL firstURL = [super testServer]->GetURL(kWindowHistoryGoTestURL);
  const GURL secondURL = [super testServer]->GetURL("/memory_usage.html");
  const GURL thirdURL = [super testServer]->GetURL(kSimpleFileBasedTestURL);
  const GURL fourthURL = [super testServer]->GetURL("/history.html");
...
- (void)testHistoryCrossDocumentLoad {
  ...
  const GURL sampleURL = [super testServer]->GetURL(kSimpleFileBasedTestURL);
...
- (void)testHistoryBackNavigation {
  GREYAssertTrue([super testServer]->Start(), @"Test server failed to start.");
  // Navigate to a URL.
  const GURL firstURL = [super testServer]->GetURL(kSimpleFileBasedTestURL);
  [ChromeEarlGrey loadURL:firstURL];

  // Navigate to an HTML page with a back button.
  const GURL secondURL = [super testServer]->GetURL(kWindowHistoryGoTestURL);
  [ChromeEarlGrey loadURL:secondURL];
...
- (void)testHistoryForwardNavigation {
  ...
  const GURL secondURL = [super testServer]->GetURL(kSimpleFileBasedTestURL);
  [ChromeEarlGrey loadURL:secondURL];
...
// Re-enable this test on device once the bug is fixed.
  ...
  const GURL forwardURL = [super testServer]->GetURL(kWindowHistoryGoTestURL);
...
- (void)testWindowLocationChangeHash {
  [super testServer]->RegisterRequestHandler(
      base::Bind(&WindowLocationHashHandlers));
  GREYAssertTrue([super testServer]->Start(), @"Test server failed to start.");
  const GURL page1URL = [super testServer]->GetURL(kPage1URL);
  const GURL hashChangedWithHistoryURL =
      [super testServer]->GetURL(kHashChangedWithHistoryURL);
...
- (void)testWindowLocationReplaceAndChangeHash {
  ...
  const GURL hashChangedWithoutHistoryURL =
      [super testServer]->GetURL(kHashChangedWithoutHistoryURL);
...
- (void)testJavaScriptRedirect {
  [super testServer]->RegisterRequestHandler(base::Bind(&RedirectHandlers));
  GREYAssertTrue([super testServer]->Start(), @"Test server failed to start.");
  // A starting page.
  const GURL initialURL = [super testServer]->GetURL(kDefaultPageURL);
  // A page that redirects immediately via the window.open JavaScript method.
  const GURL originURL = [super testServer]->GetURL(kRedirectWindowURL);
  const GURL destinationURL = [super testServer]->GetURL(kDestinationURL);
...
- (void)verifyBackAndForwardAfterRedirect:(std::string)redirectLabel {
  [super testServer]->RegisterRequestHandler(base::Bind(&RedirectHandlers));
  GREYAssertTrue([super testServer]->Start(), @"Test server failed to start.");
  const GURL indexURL([super testServer]->GetURL(kRedirectIndexURL));
  const GURL destinationURL([super testServer]->GetURL(kDestinationURL));
  const GURL lastURL([super testServer]->GetURL(kDefaultPageURL));
