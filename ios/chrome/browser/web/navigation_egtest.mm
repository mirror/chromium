...
- (void)setUp {
  ...
  // Registers default handlers and file directory to the server.
  _testServer->AddDefaultHandlers(
      base::FilePath(FILE_PATH_LITERAL(HttpSourceFileDirectory)));
...
- (void)testJavaScriptRedirect {
  // A starting page.
  const GURL initialURL = _testServer->GetURL(kDefaultPageURL);
...
- (void)verifyBackAndForwardAfterRedirect:(std::string)redirectLabel {
  ...
  const GURL destinationURL(_testServer->GetURL(kDestinationURL));
  const GURL lastURL(_testServer->GetURL(kDefaultPageURL));
