...
- (void)testWindowLocationChangeHash {
  ...
  GREYAssert(TapWebViewElementWithId(kHashChangeWithHistoryLabel), @"Failed to tap %s", kHashChangeWithHistoryLabel);
...
- (void)testWindowLocationReplaceAndChangeHash {
  ...
  // Tap link to replace the location value.
    GREYAssert(TapWebViewElementWithId(kHashChangeWithoutHistoryLabel), @"Failed to tap %s", kHashChangeWithoutHistoryLabel);
  [[EarlGrey
      selectElementWithMatcher:OmniboxText(
...
- (void)verifyBackAndForwardAfterRedirect:(std::string)redirectLabel {
  ...
  [ChromeEarlGrey loadURL:indexURL];
    GREYAssert(TapWebViewElementWithId(redirectLabel), @"Failed to tap %s", redirectLabel.c_str());
