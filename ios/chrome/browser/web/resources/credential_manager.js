...
// TODO(crbug.com/435046) Fix FederatedCredential to pass LayoutTests and also
// make its properties readonly, conforming to the spec.
...
function PasswordCredential(init) {
  ...
  if (init instanceof HTMLFormElement) {
    ...
    const elements = init.querySelectorAll( // all submittable elements
        'button, input, object, select, textarea');
    var newPasswordObserved = false;
    for (var elementIndex = 0; elementIndex < elements.length;
        elementIndex++) {
      const field = elements.item(elementIndex);
      ...
      const autocompleteTokens = field.getAttribute('autocomplete').split(' ');
      for (var tokenIndex = 0; tokenIndex < autocompleteTokens.length;
          tokenIndex++) {
        const token = autocompleteTokens[tokenIndex];
