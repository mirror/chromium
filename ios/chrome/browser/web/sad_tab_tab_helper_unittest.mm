...
@property(nonatomic, assign) BOOL feedbackSadTabShown;
@property(nonatomic, assign) BOOL sadTabShown;
@property(nonatomic, assign) BOOL reloadSadTabShown;
@end

@implementation SadTabTabHelperTestDelegate
@synthesize feedbackSadTabShown = _feedbackSadTabShown;
@synthesize sadTabShown = _sadTabShown;
@synthesize reloadSadTabShown = _reloadSadTabShown;

- (void)sadTabHelper:(SadTabTabHelper*)tabHelper
    presentSadTabForRepeatedFailure:(BOOL)repeatedFailure {
  self.sadTabShown = YES;
  self.feedbackSadTabShown = repeatedFailure;
  self.reloadSadTabShown = !repeatedFailure;
}
...
TEST_F(SadTabTabHelperTest, RepeatedFailuresShowCorrectUI) {
  ...
  EXPECT_TRUE(sad_tab_delegate_.reloadSadTabShown);
  EXPECT_FALSE(sad_tab_delegate_.feedbackSadTabShown);

  // On a second render process crash, the content view should be of a
  // FEEDBACK type.
  web_state_.OnRenderProcessGone();
  EXPECT_FALSE(sad_tab_delegate_.reloadSadTabShown);
  EXPECT_TRUE(sad_tab_delegate_.feedbackSadTabShown);
