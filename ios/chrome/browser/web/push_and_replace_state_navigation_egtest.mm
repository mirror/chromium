...
- (void)testHtml5HistoryPushUnicodeCharacters {
  ...
  // Go back and check the unicode in the URL and status. Note that the URL
  // fragment is %-decoded for display in the omnibox.
  [[EarlGrey selectElementWithMatcher:BackButton()] performAction:grey_tap()];
  [self assertStatusText:pushStateUnicode2Status
         withOmniboxText:[pushStateUnicode2URLEncoded.GetContent()
                             stringByReplacingOccurrencesOfString:@"%E2%88%A2"
                                                       withString:@"∢"]
              pageLoaded:NO];

  [[EarlGrey selectElementWithMatcher:BackButton()] performAction:grey_tap()];
  [self assertStatusText:pushStateUnicodeStatus
         withOmniboxText:[pushStateUnicodeURLEncoded.GetContent()
                             stringByReplacingOccurrencesOfString:@"%E1%84%91"
                                                       withString:@"ᄑ"]
              pageLoaded:NO];
}
...
// text is displayed if pageLoaded is YES, and that the omnibox text is as
// expected.
- (void)assertStatusText:(NSString*)status
         withOmniboxText:(std::string)omnibox
  ...
  [[EarlGrey selectElementWithMatcher:chrome_test_util::OmniboxText(omnibox)]
      assertWithMatcher:grey_notNil()];
}
