...
- (void)testHtml5HistoryPushStateThenGoBackAndForward {
  ...
  [self assertStatusText:@"pushStateRootPath":pushStateRootPathURL.GetContent()
              pageLoaded:NO];
...
- (void)testHtml5HistoryPushUnicodeCharacters {
  ...
  [self assertStatusText:pushStateUnicode2Status
         withOmniboxText:pushStateUnicode2URL.GetContent()
              pageLoaded:NO];

  [[EarlGrey selectElementWithMatcher:BackButton()] performAction:grey_tap()];
  [self assertStatusText:pushStateUnicodeStatus
         withOmniboxText:pushStateUnicodeURL.GetContent()
              pageLoaded:NO];
}
...
// Assert that status text |status|, if non-NULL, is displayed in the webview,
