...
@property(nonatomic, strong) id<LogoVendor> doodleController;
...
@synthesize doodleController = _doodleController;
...
- (instancetype)initWithWebStateList:(WebStateList*)webStateList
  ...
  if (self) {
    ...
    // Listen for default search engine changes.
    _observer = std::make_unique<SearchEngineObserverBridge>(
        self, self.templateURLService);
    _doodleController = logoVendor;
  }
  return self;
}
...
- (void)setUp {
  ...
  if (self.webState) {
    ...
    web::NavigationManager* nav = self.webState->GetNavigationManager();
    [self.consumer setCanGoForward:nav->CanGoForward()];
    [self.consumer setCanGoBack:nav->CanGoBack()];
  }

  [self.consumer setLogoVendor:self.doodleController];
