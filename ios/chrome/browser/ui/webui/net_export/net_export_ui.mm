...
void NetExportMessageHandler::SendEmail(const base::FilePath& file_to_send) {
  ...
  int alertTitleStringId = IDS_IOS_NET_EXPORT_NO_EMAIL_ACCOUNTS_ALERT_TITLE;
  int alertMessageStringId = IDS_IOS_NET_EXPORT_NO_EMAIL_ACCOUNTS_ALERT_MESSAGE;

  ShowMailComposerCommand* context = [[ShowMailComposerCommand alloc]
                  initWithToRecipients:nil
                               subject:subject
                                  body:body
        emailNotConfiguredAlertTitleId:alertTitleStringId
      emailNotConfiguredAlertMessageId:alertMessageStringId];
