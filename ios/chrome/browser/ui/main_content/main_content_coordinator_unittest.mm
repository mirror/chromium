...
@synthesize scrollForwarder = _scrollForwarder;

- (instancetype)init {
  if (self = [super init])
    _scrollForwarder = [[MainContentUIForwarder alloc] init];
  return self;
}
...
TEST_F(MainContentCoordinatorTest, BroadcastAfterStarting) {
  ...
  [coordinator start];
  EXPECT_TRUE(coordinator.viewController.scrollForwarder.broadcasting);
  // Stop the coordinator and verify that its UI is no longer being broadcasted.
  [coordinator stop];
  [parent() removeChildCoordinator:coordinator];
  EXPECT_FALSE(coordinator.viewController.scrollForwarder.broadcasting);
}
...
TEST_F(MainContentCoordinatorTest, StopBroadcastingForChildren) {
  ...
  EXPECT_TRUE(childMainCoordinator.viewController.scrollForwarder.broadcasting);
  EXPECT_FALSE(
      parentMainCoordinator.viewController.scrollForwarder.broadcasting);
