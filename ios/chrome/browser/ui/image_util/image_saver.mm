...
- (void)finishSavingImageWithError:(NSError*)error {
  ...
  if (error) {
    ...
    // TODO(crbug.com/797277): Ideally I'd like to show an infobar with a link
    // to switch to the photo application. The current behaviour is to create
    // the photo there but not providing any link to it is suboptimal. That's
    // what Safari is doing, and what the PM want, but it doesn't make it right.
  }
}
