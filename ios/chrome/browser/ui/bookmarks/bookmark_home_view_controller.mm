...
- (void)setupUIStackCacheIfApplicable {
  ...
  if (pathCache.folderId == _rootNode->id()) {
    // We have reached the leaf node, and are done using the cache, now clear
    // it. It is importatnt to clear the cache here, otherwise when user starts
    // popping off the UI stack, setupUIStackCacheIfApplicable will be called
    // (on viewWillAppear) and will cause the UI stack to be recreated.
