...
@interface BookmarkTableView ()<BookmarkModelBridgeObserver,
  ...
  // node.
  BookmarkCollectionViewBackground* _emptyTableBackgroundView;
...
- (void)generateTableViewData {
  // All all bookmarks and folders of the current root node to the table.
...
- (void)showEmptyBackground {
  if (!_emptyTableBackgroundView) {
    ...
    _emptyTableBackgroundView = [[BookmarkCollectionViewBackground alloc]
        initWithFrame:self.tableView.bounds];
    _emptyTableBackgroundView.autoresizingMask =
        UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    _emptyTableBackgroundView.text =
        l10n_util::GetNSString(IDS_IOS_BOOKMARK_NO_BOOKMARKS_LABEL);
    _emptyTableBackgroundView.frame = self.tableView.bounds;
    self.tableView.backgroundView = _emptyTableBackgroundView;
  }
  _emptyTableBackgroundView.alpha = 1;
}

// Hides empty bookmarks background view.
- (void)hideEmptyBackground {
  if (_emptyTableBackgroundView)
    _emptyTableBackgroundView.alpha = 0;
}
