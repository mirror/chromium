...
- (instancetype)initWithBrowserState:(ios::ChromeBrowserState*)browserState
  ...
  if (self) {
    ...
    self.emptyTableBackgroundView.text = [self textWhenTableIsEmpty];
...
// Computes the bookmarks table view based on the current state of all models
...
- (NSString*)textWhenTableIsEmpty {
  return l10n_util::GetNSString(IDS_IOS_BOOKMARK_NO_BOOKMARKS_LABEL);
}

//- (void)updateNavBar {
//  [self.navBar
//  setTitle:bookmark_utils_ios::TitleForBookmarkNode(_currentRootNode)];
//  // Show the back button iff the folder is a permanent primary folder.
//  BOOL isTopFolder = IsRootNode(_currentRootNode, self.bookmarkModel);
//  [self.navBar setBackButtonVisibility:!isTopFolder];
//}
