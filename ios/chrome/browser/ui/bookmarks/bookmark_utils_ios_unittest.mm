...
TEST_F(BookmarkIOSUtilsUnitTest, TestPathCache) {
  ...
  double position = 23;
  bookmark_utils_ios::CacheBookmarkUIPosition(chrome_browser_state_.get(),
                                              folderId, position);

  int64_t resultFolderId;
  double resultPosition;
  bookmark_utils_ios::GetBookmarkUIPositionCache(
      chrome_browser_state_.get(), _bookmarkModel, &resultFolderId,
      &resultPosition);
...
TEST_F(BookmarkIOSUtilsUnitTest, TestPathCacheWhenFolderDeleted) {
  ...
  BOOL result = bookmark_utils_ios::GetBookmarkUIPositionCache(
      chrome_browser_state_.get(), _bookmarkModel, &unusedFolderId,
      &unusedPosition);
  ASSERT_FALSE(result);
}
