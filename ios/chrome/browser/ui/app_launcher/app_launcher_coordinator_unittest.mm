...
@interface TestAppLauncherCoordinator : AppLauncherCoordinator
// The number of times
...
@implementation TestAppLauncherCoordinator
...
class AppLauncherCoordinatorTest : public PlatformTest {
 protected:
  AppLauncherCoordinatorTest() {
    coordinator_ =
        [[TestAppLauncherCoordinator alloc] initWithBaseViewController:nil];
    application_ = OCMClassMock([UIApplication class]);
    OCMStub([application_ sharedApplication]).andReturn(application_);
  }

  id application_ = nil;
  TestAppLauncherCoordinator* coordinator_ = nil;
};

// Tests that an empty URL does not launch application.
TEST_F(AppLauncherCoordinatorTest, EmptyUrl) {
  BOOL app_will_launch = [coordinator_ appLauncherTabHelper:nullptr
                                           launchAppWithURL:GURL::EmptyGURL()
                                                 linkTapped:NO];
  EXPECT_FALSE(app_will_launch);
