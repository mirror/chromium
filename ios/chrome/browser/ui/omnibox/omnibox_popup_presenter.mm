...
- (void)updateHeightAndAnimateAppearanceIfNecessary {
  ...
  if (base::FeatureList::IsEnabled(kCleanToolbar)) {
    ...
    for (UILayoutGuide* guide in [self.positioner popupAnchorView]
             .layoutGuides) {
      if ([guide.identifier isEqualToString:@"omniboxPopupLayoutGuide"]) {
        topLayout = guide;
        break;
      }
    }
    if (newlyAdded) {
      ...
      self.heightConstraint =
          [self.popupContainerView.heightAnchor constraintEqualToConstant:1];
    ...
    if (currentHeight == 1) {
      ...
      if (IsIPadIdiom()) {
        ...
        self.heightConstraint.constant =
            [siblingView superview].frame.size.height;
      }
    }
...
- (void)animateAppearance:(CGFloat)parentHeight {
  if (base::FeatureList::IsEnabled(kCleanToolbar)) {
    if (IsIPadIdiom()) {
      ...
      self.heightConstraint.constant =
          [self.popupContainerView superview].frame.size.height;
    }
