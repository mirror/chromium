...
- (instancetype)initWithPopupPositioner:(id<OmniboxPopupPositioner>)positioner
  ...
  if (self) {
    ...
    // The priority is not UILayoutPriorityRequired because the height is only
    // fixed on iPad. On iPhone the popup is taking the full height. Having the
    // height constraint allows to animate the expansion of the popup on iPhone.
