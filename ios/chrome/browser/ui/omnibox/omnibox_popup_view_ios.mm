...
void OmniboxPopupViewIOS::UpdateEditViewIcon() {
  ...
  delegate_->TopmostSuggestionImageChanged(image_id);
}
...
void OmniboxPopupViewIOS::UpdatePopupAppearance() {
  ...
  delegate_->ResultsChanged(result);
}
...
void OmniboxPopupViewIOS::OpenURLForRow(size_t row) {
  ...
  delegate_->DidSelectMatchForOpening(match, disposition, GURL(),
                                      base::string16(), row);
}
