...
@interface OmniboxPopupMaterialViewController () {
  ...
  NSArray<id<AutocompleteResultWrapper>>* _currentResult;
...
- (void)updateRow:(OmniboxPopupMaterialRow*)row
        withMatch:(id<AutocompleteResultWrapper>)match {
  ...
  if (match.hasImage) {
    [self.imageFetcher fetchImage:match.imageURL
...
- (void)appendButtonTapped:(id)sender {
  ...
  [self.delegate didSelectRowForAppending:row];
}
...
- (void)tableView:(UITableView*)tableView
  ...
  if (editingStyle == UITableViewCellEditingStyleDelete) {
    ...
    [self.delegate didSelectRowForDeletion:indexPath.row];
  }
}
