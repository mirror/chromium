...
class BubbleViewTest : public PlatformTest {
 ...
 protected:
  ...
  NSString* shortText_;
  // Text that is longer than the maximum line width and should wrap onto
  // multiple lines.
  NSString* longText_;
};
...
TEST_F(BubbleViewTest, BubbleSizeShortText) {
  ...
  EXPECT_EQ(80.0f, bubbleSize.width);
  // The bubble should fit the label, which contains one line of text.
  EXPECT_EQ(53.5f, bubbleSize.height);
}
...
TEST_F(BubbleViewTest, BubbleSizeMultipleLineText) {
  ...
  EXPECT_EQ(72.5f, bubbleSize.height);
}
