...
// on the |targetFrame| of the target UI element and the bubble's
// |arrowDirection|. The returned point is in the same coordinate system as
// |targetFrame|.
...
// Convenience method for calculating the maximum size of the bubble. Uses the
...
// width of the bubble's superview. |isRTL| is |true| if the coordinate system
// is in a right-to-left language and |false| otherwise.
...
// Convenience method for calculating the frame of the bubble. Uses the ICU
// default locale of the device to determine whether the language is RTL.
CGRect BubbleFrame(CGPoint anchorPoint,
                   CGSize size,
                   BubbleArrowDirection direction,
                   BubbleAlignment alignment,
