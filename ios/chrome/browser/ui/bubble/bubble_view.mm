...
const CGFloat kLabelHorizontalPadding = 18.0f;
...
- (NSArray<NSLayoutConstraint*>*)generalConstraints {
  NSArray<NSLayoutConstraint*>* constraints = @[
    ...
    // Ensure that the background view is wider than the label, and add padding
    // to the sides of the label.
    [self.background.widthAnchor
    ...
    [self.background.widthAnchor
        constraintGreaterThanOrEqualToConstant:kMinBubbleWidth -
                                               kBubbleMargin * 2],
    // Ensure that the background view is taller than the label, and add padding
    // to the top and bottom of the label.
...
- (NSLayoutConstraint*)arrowAlignmentConstraint {
  switch (self.alignment) {
    case BubbleAlignmentLeading:
      ...
      return [self.arrow.centerXAnchor
    ...
    case BubbleAlignmentCenter:
      ...
      return
          [self.arrow.centerXAnchor constraintEqualToAnchor:self.centerXAnchor];
    case BubbleAlignmentTrailing:
      // The anchor point is at a distance of |kBubbleAlignmentOffset|
      // from the bubble's trailing edge. Center align the arrow with the anchor
      // point by aligning the trailing edge of the bubble view with the center
      // of the arrow and adding an offset of |kBubbleAlignmentOffset|.
      return
