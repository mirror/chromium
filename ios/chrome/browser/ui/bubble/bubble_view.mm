...
// Indicate whether constraints need to be activated.
@property(nonatomic, assign) BOOL needsActivateConstraints;
@end
...
@synthesize needsActivateConstraints = _needsActivateConstraints;
...
- (void)willMoveToSuperview:(UIView*)newSuperview {
  ...
  if (self.needsAddSubviews) {
    ...
    // Perform additional setup and layout operations, such as adding the drop
    // shadow and accessibility label.
    [self addShadow];
...
// Override |updateConstraints| to set up Autolayout constraints.
- (void)updateConstraints {
  // If constraints have not been activated, activate them.
  if (self.needsActivateConstraints) {
    [self activateConstraints];
    // Set |needsActivateConstraints| to NO to ensure that the constraints are
    // only activated once.
    self.needsActivateConstraints = NO;
  }
