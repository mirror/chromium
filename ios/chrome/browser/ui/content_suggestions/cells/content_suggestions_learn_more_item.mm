...
- (NSString*)text {
  return l10n_util::GetNSString(IDS_NTP_LEARN_MORE_ABOUT_SUGGESTED_CONTENT);
}
...
+ (CGFloat)heightForWidth:(CGFloat)width withText:(NSString*)text {
  ...
  CGFloat labelHeight = [label sizeThatFits:CGSizeMake(width, 500)].height;
...
+ (void)configureLabel:(UILabel*)label withText:(NSString*)text {
  ...
  NSString* strippedText =
      ParseStringWithTag(text, &linkRange, @"<link>", @"</link>");
