  ...
  if (!self.headerView) {
    ...
    if (!IsIPadIdiom()) {
      ...
      if (IsUIRefreshPhase1Enabled()) {
        ...
        [vc addChildViewController:self.toolbarViewController];
        [_headerView addToolbarView:self.toolbarViewController.view];
        [self.toolbarViewController didMoveToParentViewController:vc];
        [self.toolbarViewController setTabCount:self.tabCount];
        [self.toolbarViewController setCanGoForward:self.canGoForward];
        [self.toolbarViewController setCanGoBack:self.canGoBack];
      } else {
