...
- (NSString*)accessibilityLabel {
  ...
  [builder append:self.itemLabel.text];
  [builder append:self.notificationLabel.text];
  [builder append:self.priceLabel.text];
  return [builder build];
  // NSMutableArray* components = [[NSMutableArray alloc] init];
  // if ()
  //   [components addObject:self.itemLabel.text];
  // if (self.notificationLabel.text)
  //   [components addObject:self.notificationLabel.text];
  // if (self.priceLabel.text)
  //   [components addObject:self.priceLabel.text];
  // DCHECK_GT([components count], 0U);
