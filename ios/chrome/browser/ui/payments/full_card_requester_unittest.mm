...
typedef void (^mock_full_card_request_did_succeed)(const std::string&,
                                                   const std::string&);
...
TEST_F(PaymentRequestFullCardRequesterTest, InstrumentDetailsReady) {
  ...
  SEL selector = @selector(fullCardRequestDidSucceed:stringifiedDetails:);
  ...
  std::unique_ptr<base::DictionaryValue> response_value =
      payments::data_util::GetBasicCardResponseFromAutofillCreditCard(
          credit_card_, base::ASCIIToUTF16("123"), autofill_profile, appLocale)
          .ToDictionaryValue();
  std::string stringifiedDetails;
  base::JSONWriter::Write(*response_value, &stringifiedDetails);

  full_card_requester.OnInstrumentDetailsReady(methodName, stringifiedDetails);
}
