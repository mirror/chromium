...
- (void)viewDidLoad {
  ...
  if (!IsIPadIdiom() && [self navigationController].navigationBarHidden) {
    // For some reason, on phone devices, the UITableViewController uses the
    // full screen even when the status bar is present, but insets the section
    // headers by the size of the status bar. This will inset the content by the
    // same amount, to ensure they line up properly. Also insets by one more
    // pixel to hide the one pixel gap left in between the navigation bar and
    // the UITableView.
