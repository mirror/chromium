...
// returned by request.show() with the appropriate error message.
- (void)testOpenAndCancel {
  ...
  [self expectWebViewContainsStrings:{"The payment request was canceled."}];
}

// Tests that tapping the link to Chrome Settings closes the UI, rejects the
// Promise returned by request.show() with the appropriate error message, and
// displays the Autofill Settings UI.
...
// Tests that tapping the pay button closes the UI, accepts the Promise returned
// by request.show() with the response object, and accepts the Promise returned
// by response.complete() with the appropriate response message.
- (void)testOpenAndPay {
...
- (void)testAbort {
  ...
  [self tapWebViewElementWithId:"abort"];
  ...
  [self expectWebViewContainsStrings:{"Aborted"}];
}

// Tests that request.abort() fails to abort the Payment Request if the Card
// Unmask Prompt is showing.
- (void)testAbortUnsuccessfulAfterCardUnmaskPromptShown {
  // TODO(crbug.com/602666): implement this feature as well as the test.
}
