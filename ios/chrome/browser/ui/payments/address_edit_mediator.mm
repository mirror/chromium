...
- (void)regionDataLoaderDidSucceedWithRegions:(NSArray<RegionData*>*)regions {
  ...
  if (self.address) {
    ...
    BOOL (^isEqualToRegion)(RegionData*, NSUInteger, BOOL*) =
        ^BOOL(RegionData* testRegion, NSUInteger index, BOOL* stop) {
          return [testRegion.regionCode isEqualToString:region];
        };
    if ([regions indexOfObjectPassingTest:isEqualToRegion] != NSNotFound) {
      self.regionField.value = region;
    } else {
      NSIndexSet* matchingRegions =
          [regions indexesOfObjectsPassingTest:isEqualToRegion];
