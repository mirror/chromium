...
- (NSArray<AutofillProfileItem*>*)createItems {
  ...
  for (size_t index = 0; index < contactProfiles.size(); ++index) {
    autofill::AutofillProfile* contactAddress = contactProfiles[index];
    DCHECK(contactAddress);
    AutofillProfileItem* item = [[AutofillProfileItem alloc] init];
    item.name = GetNameLabelFromAutofillProfile(*contactAddress);
    item.email = GetEmailLabelFromAutofillProfile(*contactAddress);
    item.phoneNumber = GetPhoneNumberLabelFromAutofillProfile(*contactAddress);
    if (_paymentRequest->selected_contact_profile() == contactAddress)
