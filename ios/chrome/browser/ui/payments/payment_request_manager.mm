...
// Sets |paymentRequest| as the pending payment request and returns YES, if
// there is no pending request at the moment. Otherwise, returns NO.
- (BOOL)setActivePaymentRequest:(payments::PaymentRequest*)paymentRequest;
...
- (BOOL)handleRequestShow:(const base::DictionaryValue&)message {
  ...
  if (![self setActivePaymentRequest:paymentRequest]) {
