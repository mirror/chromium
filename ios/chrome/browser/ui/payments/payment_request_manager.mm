...
- (void)stopTrackingWebState:(web::WebState*)webState {
  const auto itr = _paymentRequests.find(webState);
  DCHECK(itr != _paymentRequests.end());
  _paymentRequests.erase(itr);
}
...
- (payments::PaymentRequest*)getOrCreatePaymentRequestFromMessage:
  ...
  const auto itr = _paymentRequests.find(_activeWebState);
  DCHECK(itr != _paymentRequests.end());
...
- (void)paymentRequestCoordinator:(PaymentRequestCoordinator*)coordinator
  ...
  coordinator.paymentRequest->address_normalization_manager()
      ->FinalizeWithCompletionCallback(base::BindBlockArc(^() {
        [weakSelf
