...
- (BOOL)webStateContentIsSecureHTML {
  ...
  if (!web::OriginSecurityChecker::IsContextSecure(lastCommittedURL)) {
    DLOG(ERROR) << "Not in a secure context.";
    return NO;
  }

  if (!web::OriginSecurityChecker::IsSchemeCryptographic(lastCommittedURL) &&
      !web::OriginSecurityChecker::IsOriginLocalhostOrFile(lastCommittedURL)) {
  ...
  return !web::OriginSecurityChecker::IsSchemeCryptographic(lastCommittedURL) ||
         web::OriginSecurityChecker::IsSSLCertificateValid(
...
- (void)webState:(web::WebState*)webState
  ...
  [_paymentRequestJsManager
       setContextSecure:web::OriginSecurityChecker::IsContextSecure(
