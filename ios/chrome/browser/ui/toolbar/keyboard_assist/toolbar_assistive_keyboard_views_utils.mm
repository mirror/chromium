...
NSArray<UIButton*>* ToolbarAssistiveKeyboardLeadingButtons(
  ...
  if (base::FeatureList::IsEnabled(kExternalSearch)) {
    NSString* iconName = ios::GetChromeBrowserProvider()
    ...
    [externalSearchButton
        ...
        forControlEvents:UIControlEventTouchUpInside];
    int idsAccessibilityLabel = ios::GetChromeBrowserProvider()
                                    ->GetExternalSearchProvider()
                                    ->GetButtonIdsAccessibilityLabel();
    SetA11yLabelAndUiAutomationName(externalSearchButton, idsAccessibilityLabel,
                                    @"External Search");
