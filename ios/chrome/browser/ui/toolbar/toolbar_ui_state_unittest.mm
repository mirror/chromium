...
class ToolbarUIStateUpdaterTest : public PlatformTest {
 public:
  ToolbarUIStateUpdaterTest()
      : PlatformTest(),
        web_state_list_(&web_state_list_delegate_),
        toolbar_owner_([[TestToolbarOwner alloc] init]),
        state_([[ToolbarUIState alloc] init]),
        updater_([[ToolbarUIStateUpdater alloc]
            initWithState:state_
             toolbarOwner:toolbar_owner_
             webStateList:&web_state_list_]) {}
  ~ToolbarUIStateUpdaterTest() override { StopUpdating(); }

  // Getters.
  WebStateList* web_state_list() { return &web_state_list_; }
  TestToolbarOwner* toolbar_owner() { return toolbar_owner_; }
  CGFloat toolbar_height() { return state_.toolbarHeight; }
 ...
 private:
  ...
  __strong ToolbarUIState* state_ = nil;
  __strong ToolbarUIStateUpdater* updater_ = nil;
  bool updating_ = false;
};

// Tests that |-startUpdating| resets the state's height when starting.
TEST_F(ToolbarUIStateUpdaterTest, StartUpdating) {
...
TEST_F(ToolbarUIStateUpdaterTest, StopUpdating) {
...
// changes.
TEST_F(ToolbarUIStateUpdaterTest, UpdateActiveWebState) {
  StartUpdating();
  const CGFloat kHeight = 150.0;
  toolbar_owner().toolbarHeight = kHeight;
  EXPECT_EQ(toolbar_height(), 0.0);
...
TEST_F(ToolbarUIStateUpdaterTest, UserInitiatedNavigation) {
  web::TestWebState* web_state = InsertActiveWebState();
  StartUpdating();
  const CGFloat kHeight = 150.0;
  toolbar_owner().toolbarHeight = kHeight;
...
TEST_F(ToolbarUIStateUpdaterTest, RendererInitiatedNavigation) {
  web::TestWebState* web_state = InsertActiveWebState();
  StartUpdating();
  const CGFloat kHeight = 150.0;
  toolbar_owner().toolbarHeight = kHeight;
