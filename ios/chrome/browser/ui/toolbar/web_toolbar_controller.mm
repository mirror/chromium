...
#pragma clang diagnostic pop
  ...
  OmniboxEditModel* model = _locationBar->GetEditView()->model();
  _popupView = base::MakeUnique<OmniboxPopupViewIOS>(
      _browserState, model->client(), model->autocomplete_controller(),
      _locationBar->GetEditView(), model, self);
  model->set_popup_model(_popupView->model());
  _locationBar->GetEditView()->SetPopupProvider(_popupView.get());
