...
- (void)setUpToolbarButtons {
  ...
  for (NSLayoutConstraint* constraint in buttonConstraints) {
    // The buttons are added to a UIStackView. If the priority is |required|,
    // there is a conflict when the buttons are hidden as the stack view is
    // setting their width to 0. Setting the priority to
    // UILayoutPriorityDefaultHight doesn't work as they would have a low
    // priority than the location bar which would expand.
...
- (void)setUpLocationBarContainer {
  ...
  // The button is added to a UIStackView. If the priority is |required|, there
  // is a conflict when the button is hidden as the stack view is setting the
  // width to 0.
