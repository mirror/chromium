...
void SelectNewTabPagePanel(ntp_home::PanelIdentifier panel_type) {
  ...
  if (IsIPadIdiom()) {
    ...
    NSUInteger tag = 0;
    if (panel_type == ntp_home::PanelIdentifier::kBookmarksPanel) {
      tag = IDC_SHOW_BOOKMARK_MANAGER;
    } else if (panel_type == ntp_home::PanelIdentifier::kOpenTabsPanel) {
...
- (void)testToolbarOmniboxTyping {
  ...
  SelectNewTabPagePanel(ntp_home::kHomePanel);
