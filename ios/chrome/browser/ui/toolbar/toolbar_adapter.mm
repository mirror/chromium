...
@property(nonatomic, strong) ToolbarCoordinator* cleanToolbarCoordinator;
@end
...
- (instancetype)initWithDispatcher:
  ...
  self = [super init];
  if (self) {
    _cleanToolbarCoordinator = [[ToolbarCoordinator alloc] init];
    _cleanToolbarCoordinator.webStateList = webStateList;
    _cleanToolbarCoordinator.dispatcher = dispatcher;
...
- (void)setDelegate:(id<WebToolbarDelegate>)delegate {
  _delegate = delegate;
  self.cleanToolbarCoordinator.delegate = delegate;
}
...
- (void)focusOmnibox {
  [self.cleanToolbarCoordinator focusOmnibox];
}

- (void)cancelOmniboxEdit {
  [self.cleanToolbarCoordinator cancelOmniboxEdit];
}

- (void)focusFakebox {
  [self.cleanToolbarCoordinator focusFakebox];
}

- (void)onFakeboxBlur {
  [self.cleanToolbarCoordinator onFakeboxBlur];
}

- (void)onFakeboxAnimationComplete {
  [self.cleanToolbarCoordinator onFakeboxAnimationComplete];
}

#pragma mark - VoiceSearchControllerDelegate

- (void)receiveVoiceSearchResult:(NSString*)voiceResult {
  [self.cleanToolbarCoordinator receiveVoiceSearchResult:voiceResult];
}
