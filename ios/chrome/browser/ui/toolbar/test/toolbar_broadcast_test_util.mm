...
class TestToolbarUIStateModifier {
 public:
  TestToolbarUIStateModifier(TestToolbarUIState* state)
      : state_(state), original_height_(state.toolbarHeight) {
    state_.toolbarHeight += kHeightDelta;
  }
  ~TestToolbarUIStateModifier() { state_.toolbarHeight = original_height_; }

  // The original values of the UI state.
  CGFloat original_height() { return original_height_; }

 private:
  __strong TestToolbarUIState* state_ = nil;
  CGFloat original_height_ = 0.0;
};
}  // namespace

void VerifyToolbarUIBroadcast(TestToolbarUIState* ui_state,
  ...
  TestToolbarUIStateModifier modifier(ui_state);
  // Verify whether the changed or original UI elements are observed.
  if (should_broadcast) {
    EXPECT_TRUE(
        AreCGFloatsEqual(observer.toolbarHeight, ui_state.toolbarHeight));
  } else {
    EXPECT_TRUE(
