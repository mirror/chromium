...
// Logs the result for successful canonical URL retrievals. |canonical_url| is
// the canonical URL that was found, and |visible_url| is the page's URL that is
// visible in the omnibox.
void LogCanonicalUrlSuccessResult(const GURL& canonical_url,
  ...
  void (^javascript_completion)(const base::Value*) =
      ^(const base::Value* value) {
        ...
        if (!canonical_url.is_empty())
          LogCanonicalUrlSuccessResult(canonical_url, visible_url);
