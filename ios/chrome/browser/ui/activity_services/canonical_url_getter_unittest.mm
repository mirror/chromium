...
// has been retrieved, then this property points to an empty GURL.
...
TEST_F(CanonicalURLGetterTest, TestOneCanonicalURLFound) {
  ...
  WaitForCondition(^{
    return getterDelegate.canonicalURLReceived;
  });
...
// Validates that if both the visible and canonical URLs are HTTPS, the correct
