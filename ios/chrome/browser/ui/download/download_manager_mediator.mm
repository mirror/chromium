...
void DownloadManagerMediator::Start() {
  base::FilePath download_dir;
  if (!GetDownloadsDirectory(&download_dir)) {
    consumer_.state = kDownloadManagerStateFailed;
    return;
  }
  ...
  consumer_.state = kDownloadManagerStateInProgress;
...
void DownloadManagerMediator::DownloadWithDestinationDir(
  ...
  base::FilePath path = download_dir.Append(base::UTF16ToUTF8(file_name));
...
void DownloadManagerMediator::UpdateConsumer() {
  consumer_.state = GetDownloadManagerState();
  consumer_.countOfBytesReceived = task_->GetReceivedBytes();
  consumer_.countOfBytesExpectedToReceive = task_->GetTotalBytes();
  consumer_.fileName = base::SysUTF16ToNSString(task_->GetSuggestedFilename());
}
