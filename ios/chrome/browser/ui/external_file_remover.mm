...
void ExternalFileRemover::RemoveFiles(bool all_files,
  ...
  if (all_files) {
    referenced_files = ExternalFileRemover::GetReferencedExternalFiles();
  }
...
NSSet* ExternalFileRemover::GetReferencedExternalFiles() {
  ...
  NSMutableSet* all_tab_model_files;
  for (TabModel* tab_model in GetTabModelsForChromeBrowserState(
    ...
    if (tab_model_files) {
      [all_tab_model_files unionSet:tab_model_files];
    }
  }
  ...
  return [all_tab_model_files setByAddingObjectsFromSet:bookmarked_files];
}
