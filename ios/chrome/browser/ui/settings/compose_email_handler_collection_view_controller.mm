...
- (void)loadModel {
  ...
  if (base::FeatureList::IsEnabled(kMailtoPromptInMdcStyle)) {
    // The second section, if exists, is the toggle switch to always prompt for
    // selection of Mail client app.
...
- (void)didToggleAlwaysAskSwitch:(id)sender {
  ...
  // Clear all seections by iterating through the rows. Text color of each
