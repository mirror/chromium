...
- (void)collectionView:(UICollectionView*)collectionView
  ...
  for (NSIndexPath* indexPath in sortedIndexPaths) {
    ...
    // Remove duplicates from the password store. These entries are not
    // displayed in the UI, so they don't need to be retained in
    // |deletedForms_|.
    ...
    for (password_manager::DuplicatesMap::iterator it = dups.first;
         it != dups.second; ++it) {
      passwordStore_->RemoveLogin(*it->second);
    }
