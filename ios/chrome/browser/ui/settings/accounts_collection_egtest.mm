...
typedef NSString* (^ExpectedTextLabelCallback)(ChromeIdentity* identity);
...
- (void)checkSyncCellWithExpectedTextLabelCallback:
  ...
  NSString* expectedDetailTextLabel = callback(identity);
  [SigninEarlGreyUtils assertSignedInWithIdentity:identity];
  [ChromeEarlGreyUI tapSettingsMenuButton:SettingsAccountButton()];
  ...
  GREYPerformBlock block = ^(id element, NSError* __strong* errorOrNil) {
    ...
    return (bool)[cell.detailTextLabel.text
               isEqualToString:expectedDetailTextLabel] &&
...
- (void)testMDMError {
  ...
  ExpectedTextLabelCallback callback = ^(ChromeIdentity* identity) {
