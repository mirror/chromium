...
#import "ios/third_party/earl_grey/src/EarlGrey/Assertion/GREYAssertionDefines.h"
...
// checks.
GREYElementInteraction* GetInteractionForEntry(NSString* username) {
  return GetInteractionForListItem(ButtonWithAccessibilityLabel(username),
                                   kGREYDirectionDown);
}

// Taps the entry on the password list with the given |username|. Scrolls down
// if necessary.
void TapEntry(NSString* username) {
  [GetInteractionForEntry(username) performAction:grey_tap()];
}
...
GREYElementInteraction* GetInteractionForDetailItem(id<GREYMatcher> matcher) {
  return [[EarlGrey
      selectElementWithMatcher:grey_allOf(matcher, grey_interactable(), nil)]
...
// Taps the item on the detail view identified with the given |matcher|.
// Scrolls down if necessary.
void TapDetailItem(id<GREYMatcher> matcher) {
  [GetInteractionForDetailItem(matcher) performAction:grey_tap()];
}
...
// Posts a task to PasswordStore's background task runner and waits until it is
// finished. Because the background task runner is sequenced, this means that
// previously posted tasks are also finished when this function exits.
...
void OpenPasswordSettings() {
  ...
  // The settings page requested results from PasswordStore. Make sure they have
...
- (void)testAccessibilityOnPasswords {
  ...
  TapEntry(@"https://example.com, concrete username");
  chrome_test_util::VerifyAccessibilityForCurrentScreen();
...
- (void)testCopyPasswordToast {
  ...
  TapDetailItem(CopyPasswordButton());
...
- (void)testCopyUsernameToast {
  ...
  TapDetailItem(CopyUsernameButton());
  NSString* snackbarLabel =
...
- (void)testDeletionInDetailView {
  ...
  [GetInteractionForEntry(@"https://example.com, concrete username")
...
- (void)testCopySiteMenuItem {
  ...
  TapDetailItem(grey_accessibilityLabel(@"https://example.com/"));
...
- (void)testCopyUsernameMenuItem {
  ...
  TapDetailItem(grey_accessibilityLabel(@"concrete username"));
...
- (void)testCopyPasswordMenuItem {
  ...
  TapDetailItem(grey_accessibilityLabel(@"●●●●●●●●●●●●●●●●●"));
...
- (void)testShowHidePasswordMenuItem {
  ...
  TapDetailItem(grey_accessibilityLabel(@"concrete password"));
  ...
  [GetInteractionForDetailItem(grey_accessibilityLabel(@"●●●●●●●●●●●●●●●●●"))
...
- (void)testFederated {
  ...
  TapEntry(@"https://example.com, federated username");

  // Check that the Site, Username, Federation and Delete Saved Password
  // sections are there.
  [GetInteractionForDetailItem(SiteHeader()) assertWithMatcher:grey_notNil()];
  [GetInteractionForDetailItem(UsernameHeader())
      assertWithMatcher:grey_notNil()];
  // For federation check both the section header and content.
  [GetInteractionForDetailItem(FederationHeader())
      assertWithMatcher:grey_notNil()];
  [GetInteractionForDetailItem(grey_text(@"famous.provider.net"))
      assertWithMatcher:grey_notNil()];
  [GetInteractionForDetailItem(DeleteButton()) assertWithMatcher:grey_notNil()];

  // Check that the password is not present.
  [GetInteractionForDetailItem(PasswordHeader()) assertWithMatcher:grey_nil()];
...
- (void)testLayoutNormal {
  ...
  [GetInteractionForDetailItem(siteCell)
      assertWithMatcher:grey_layout(@[ Below() ], SiteHeader())];
  [GetInteractionForDetailItem(CopySiteButton())
...
- (void)testLayoutFederated {
  ...
  [GetInteractionForDetailItem(federationCell)
...
- (void)testStoredEntriesAlwaysShown {
  ...
  for (BOOL expected_state : kExpectedState) {
    ...
    [GetInteractionForEntry(@"https://example.com, concrete username")
        assertWithMatcher:grey_notNil()];
    [GetInteractionForEntry(@"https://blacklisted.com")
        assertWithMatcher:grey_notNil()];
  }
