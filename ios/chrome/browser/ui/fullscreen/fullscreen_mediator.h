...
class FullscreenMediator : public FullscreenModelObserver {
 ...
 private:
  ...
  // Sets up |animator|.  |animator| is expected to be a pointer to an ivar, and
  // is used to reset the variable in the animation completion block.
  void SetUpAnimator(__strong FullscreenAnimator** animator);

  // Stops the current scroll end animation if one is in progress.
  void StopAnimating();

  // Stops |animator|.  |animator| is expected to be a pointer to an ivar, and
  // is used to reset the variable.
  void StopAnimator(__strong FullscreenAnimator** animator);
