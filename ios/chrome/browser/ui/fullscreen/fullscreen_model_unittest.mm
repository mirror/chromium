...
class FullscreenModelTest : public PlatformTest {
 public:
  ...
  // The base offset against which fullscreen progress is being calculated.
  CGFloat base_offset() {
    EXPECT_TRUE(model_.has_base_offset());
    return y_content_offset_ - (1.0 - observer_.progress()) * toolbar_height_;
  }
  ...
  void SimulateUserScrollForProgress(CGFloat progress) {
    ...
    CGFloat final_y_content_offset =
        toolbar_height_ * (1.0 - observer_.progress()) + base_offset();
...
TEST_F(FullscreenModelTest, AnimationEnded) {
  ...
  model().AnimationEndedWithProgress(0.5);
  ...
  EXPECT_EQ(base_offset(), y_content_offset() - 0.5 * kToolbarHeight);
}
...
TEST_F(FullscreenModelTest, UserScroll) {
  SimulateUserScrollForProgress(0.5);
  EXPECT_EQ(observer().progress(), 0.5);
  EXPECT_EQ(y_content_offset(), -kToolbarHeight / 2.0);
}

// Tests that updating the y content offset produces the expected progress
// value.
TEST_F(FullscreenModelTest, ProgrammaticScroll) {
  // Perform a programmatic scroll that would result in a progress of 0.5.
  set_y_content_offset(-kToolbarHeight / 2.0);
  // Verify that the base offset (and not the progress value) is updated for
  // programmatic scrolls.
  EXPECT_EQ(observer().progress(), 1.0);
  EXPECT_EQ(base_offset(), y_content_offset() - kToolbarHeight);
}
