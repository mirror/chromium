...
void FullscreenMediator::ScrollToTop() {
  ...
  if (AreCGFloatsEqual(progress, 1.0))
    return;
...
void FullscreenMediator::StopAnimating() {
  if (scroll_end_animator_) {
    DCHECK_EQ(scroll_end_animator_.state, UIViewAnimatingStateActive);
    model_->AnimationEndedWithProgress(scroll_end_animator_.currentProgress);
    [scroll_end_animator_ stopAnimation:YES];
    scroll_end_animator_ = nil;
  }
}
