...
- (void)viewDidAppear:(BOOL)animated {
  ...
  [self presentNewIncognitoTabTipOnInitialized];
}
...
- (void)presentNewIncognitoTabTipOnInitialized {
  ...
  if (!self.incognitoTabTipBubblePresenter.isUserEngaged) {
    __weak BrowserViewController* weakSelf = self;
    void (^onInitializedBlock)(bool) = ^(bool successfullyLoaded) {
      [weakSelf presentNewIncognitoTabTip];
    };
    ...
    // before checking if the promotion is to be displayed.
    feature_engagement::TrackerFactory::GetForBrowserState(self.browserState)
        ->AddOnInitializedCallback(base::BindBlockArc(onInitializedBlock));
  }
}
