...
- (void)installDelegatesForTab:(Tab*)tab {
  ...
  PasswordTabHelper::FromWebState(tab.webState)->SetDispatcher(self.dispatcher);
  ...
  if (AccountConsistencyService* account_consistency_service =
          ios::AccountConsistencyServiceFactory::GetForBrowserState(
              self.browserState)) {
    account_consistency_service->SetWebStateHandler(tab.webState, self);
  }
}
...
- (void)uninstallDelegatesForTab:(Tab*)tab {
  ...
  // TODO(crbug.com/777557): do not pass the dispatcher to PasswordTabHelper.
  PasswordTabHelper::FromWebState(tab.webState)->SetDispatcher(nil);
  ...
  if (AccountConsistencyService* account_consistency_service =
          ios::AccountConsistencyServiceFactory::GetForBrowserState(
              self.browserState)) {
    account_consistency_service->RemoveWebStateHandler(tab.webState);
  }
}
