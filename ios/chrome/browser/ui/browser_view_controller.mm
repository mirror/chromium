...
// dismissed, remains allocated so |userEngaged| remains accessible.
@property(nonatomic, strong)
...
// Presents a bubble associated with an in-product help promotion if it is valid
// to show the promotion. If the feature engagement tracker determines the
// bubble should be shown, then returns the presented bubble. Else, return
// |nil|. |feature| is the base::Feature object associated with the given
// promotion. |direction| is the direction the bubble's arrow is pointing.
...
- (void)userEnteredTabSwitcher {
  if ([self.tabTipBubblePresenter isUserEngaged]) {
    base::RecordAction(
        UserMetricsAction("UserEnteredTabSwitcherWhileEngagedWithBubble"));
  }
}
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  // If the tab tip bubble has already been presented, it can't be overwritten
  // or set to |nil| or else it will reset the |userEngaged| property.
...
presentBubbleForFeature:(const base::Feature&)feature
  ...
  [bubbleViewControllerPresenter presentInViewController:self
                                                    view:self.view
