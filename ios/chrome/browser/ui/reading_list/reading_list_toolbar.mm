...
@interface ToolbarStackViewButton : UIView

// The actual UIButton object inside ToolbarStackViewButton.
@property(nonatomic, strong) UIButton* button;

// Width constraint for the ToolbarStackViewButton.
...
// Setting the title text of the UIButton.
- (void)setTitle:(NSString*)title;

// Enables or disables the UIButton.
- (void)setEnabled:(BOOL)enabled;

// Get the titleLabel of the UIButton.
- (UILabel*)getTitleLabel;

// Set the maximum width contraint of ToolbarStackViewButton.
- (void)setMaxWidth:(CGFloat)maxWidth;

@end

@implementation ToolbarStackViewButton
...
- (void)setMaxWidth:(CGFloat)maxWidth {
  ...
  _widthConstraint =
      [_button.widthAnchor constraintLessThanOrEqualToConstant:maxWidth];
...
@property(nonatomic, strong) ToolbarStackViewButton* editButton;
// Button that displays "Delete".
@property(nonatomic, strong) ToolbarStackViewButton* deleteButton;
// Button that displays "Delete All Read".
@property(nonatomic, strong) ToolbarStackViewButton* deleteAllButton;
// Button that displays "Cancel".
@property(nonatomic, strong) ToolbarStackViewButton* cancelButton;
// Button that displays the mark options.
@property(nonatomic, strong) ToolbarStackViewButton* markButton;
...
- (void)updateHeight {
  ...
  // availableWidth.
  for (ToolbarStackViewButton* button in buttons) {
    if (!button.hidden) {
      visibleCount++;
      if (visibleCount > 1) {
  ...
  for (ToolbarStackViewButton* button in buttons) {
    if (!button.hidden) {
      CGFloat labelHeight =
          [[button getTitleLabel] sizeThatFits:labelBounds].height;
      if (labelHeight > lineHeight * 2) {
        self.heightConstraint.constant = kToolbarThreeLinesHeight;
        return;
      }
