...
readingListCollectionViewController:
  ...
  // Send the opened reading list item event when the user clicks on an item in
  // their reading list.
  FeatureEngagementTrackerFactory::GetForBrowserState(self.browserState)
            ...
            (ReadingListCollectionViewController*)
                readingListCollectionViewController
                          openNewTabWithURL:(const GURL&)URL
                                  incognito:(BOOL)incognito {
  base::RecordAction(base::UserMetricsAction("MobileReadingListOpen"));
  // Send the opened reading list item event when the user long presses on a
  // reading list item and selects when of the actions in the alert (open in
  // new tab, open in new incognito tab, or open offline version in new tab).
  FeatureEngagementTrackerFactory::GetForBrowserState(self.browserState)
      ->NotifyEvent(feature_engagement_tracker::events::kOpenedReadingListItem);
