...
namespace {
NSArray* const kSnapshotCacheDirectory = @[ @"Chromium", @"Snapshots" ];
...
void ConvertAndSaveGreyImage(NSString* session_id,
  ...
  WriteImageToDisk(grey_image, ImagePath(session_id, IMAGE_TYPE_COLOR,
                                         image_scale, cache_directory));
}
...
- (instancetype)init {
  return [self initWithCacheDirectory:[SnapshotCache cacheDirectory]
                           imageScale:ImageScaleForDevice()];
}
...
+ (CGFloat)snapshotScaleForDevice {
  return ScaleFromImageScale(ImageScaleForDevice());
}
...
- (void)retrieveImageForSessionID:(NSString*)sessionID
  ...
  const ImageScale imageScale = imageScale_;
  ...
  base::PostTaskAndReplyWithResult(
      ...
      FROM_HERE, base::BindBlockArc(^base::scoped_nsobject<UIImage>() {
        ...
        return base::scoped_nsobject<UIImage>(ReadImageForSessionFromDisk(
            sessionID, IMAGE_TYPE_COLOR, imageScale, cacheDirectory));
      }),
  ...
  web::WebThread::PostBlockingPoolSequencedTask(
      kSequenceToken, FROM_HERE, base::BindBlockArc(^{
        for (size_t index = 0; index < arraysize(kImageTypes); ++index) {
          base::DeleteFile(ImagePath(sessionID, kImageTypes[index], imageScale,
                                     cacheDirectory),
                           false /* recursive */);
        }
      }));
...
+ (base::FilePath)imagePathForSessionID:(NSString*)sessionID {
  return ImagePath(sessionID, IMAGE_TYPE_COLOR, ImageScaleForDevice(),
                   [SnapshotCache cacheDirectory]);
}

+ (base::FilePath)greyImagePathForSessionID:(NSString*)sessionID {
  return ImagePath(sessionID, IMAGE_TYPE_GREYSCALE, ImageScaleForDevice(),
                   [SnapshotCache cacheDirectory]);
}
...
- (void)loadGreyImageAsync:(NSString*)sessionID {
  ...
  base::PostTaskAndReplyWithResult(
      ...
      FROM_HERE, base::BindBlockArc(^base::scoped_nsobject<UIImage>() {
        ...
        if (!result) {
          result.reset(ReadImageForSessionFromDisk(sessionID, IMAGE_TYPE_COLOR,
                                                   imageScale, cacheDirectory));
        }
        if (result)
...
- (void)retrieveGreyImageForSessionID:(NSString*)sessionID
  ...
  base::PostTaskAndReplyWithResult(
      ...
      FROM_HERE, base::BindBlockArc(^base::scoped_nsobject<UIImage>() {
        // Retrieve the image on a high priority thread.
        return base::scoped_nsobject<UIImage>(ReadImageForSessionFromDisk(
            sessionID, IMAGE_TYPE_GREYSCALE, imageScale, cacheDirectory));
      }),
      base::BindBlockArc(^(base::scoped_nsobject<UIImage> image) {
        if (image) {
          if (callback)
...
- (void)saveGreyInBackgroundForSessionID:(NSString*)sessionID {
  ...
  web::WebThread::PostBlockingPoolTask(
      FROM_HERE, base::BindBlockArc(^{
        ConvertAndSaveGreyImage(sessionID, imageScale, backgroundingColorImage,
                                cacheDirectory);
      }));
}
