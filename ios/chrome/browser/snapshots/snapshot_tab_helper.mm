...
void SnapshotTabHelper::TakeSnapshot() {
  ...
  if (url.SchemeIs(kChromeUIScheme)) {
    DCHECK(snapshot_provider_);
    [snapshot_provider_ provideSnapshot:^(UIImage* snapshot) {
      [snapshot_cache_ setImage:snapshot withSessionID:tab_id];
    }];
  } else {
...
void SnapshotTabHelper::SetSnapshotProvider(
    id<SnapshotProvider> snapshot_provider) {
