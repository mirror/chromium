...
// Indicates whether the delegate has been instructed to show the mail composer.
@property(nonatomic, readonly) BOOL hasShownMailComposer;

@end

@implementation TestNetExportTabHelperDelegate
@synthesize hasShownMailComposer = _hasShownMailComposer;

- (void)netExportTabHelper:(NetExportTabHelper*)tabHelper
    showMailComposerWithContext:(ShowMailComposerContext*)context {
  _hasShownMailComposer = YES;
}
...
class NetExportTabHelperTest : public web::WebTestWithWebState {
 public:
  NetExportTabHelperTest() = default;
  ~NetExportTabHelperTest() override = default;
...
// Verifies that the delegate is instructed to show the mail composer when the
// NetExportTabHelper is told to do so.
TEST_F(NetExportTabHelperTest, TestShowMailComposer) {
  ...
  EXPECT_TRUE([delegate_ hasShownMailComposer]);
}
