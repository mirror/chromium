...
TEST_F(AutofillControllerJsTest, FindChildTextSkipElementTest) {
  NSArray* testCases = @[
    @{
      @"description" : @"Skip div element",
      @"html" : kSkipElement,
      @"expected" : kSkipElementExpected
    },
  ];
  ...
  for (NSDictionary* testCase in testCases) {
    ...
    NSString* result = ExecuteJavaScript(
        ...
        @"Set(Array.from(document.getElementsByClassName('skip'))), "
        @"new Set(Array.from(document.getElementsByClassName('right'))))");
