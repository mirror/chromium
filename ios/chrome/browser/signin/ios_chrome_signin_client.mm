...
void IOSChromeSigninClient::PreGaiaLogout(base::OnceClosure callback) {
  // There is a weird dependency issue here, but I do not know how this can be
  // fixed (and it is not needed now):
  // When the profile is shut down, GaiaCookieManagerService is killed after the
  // AccountConsistencyService. If GaiaCookieManagerService decides to make a
  // call to Logout after the AccountConsistencyService was shut down, then the
  // client will attempt to get the AccountConsistencyService here and call
  // RemoveChromeConnectedCookies. This will probably will crash somehow.
