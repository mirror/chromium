...
void AppLauncherTabHelper::SetDelegate(
...
bool AppLauncherTabHelper::RequestToLaunchApp(const GURL& url,
  ...
  switch (policy) {
    ...
    case ExternalAppLaunchPolicyAllow: {
      return [delegate_ launchAppWithURL:url linkTapped:link_tapped];
    }
    case ExternalAppLaunchPolicyPrompt: {
      is_prompt_active_ = true;
      base::WeakPtr<AppLauncherTabHelper> weak_this =
          weak_factory_.GetWeakPtr();
      [delegate_ showAlertOfRepeatedLaunchesWithCompletionHandler:^(
                     BOOL user_allowed) {
        if (!weak_this.get())
          return;
        if (user_allowed) {
          // By confirming that user wants to launch the application, there is
          // no need to check for |link_tapped|.
          [delegate_ launchAppWithURL:url linkTapped:YES];
        } else {
