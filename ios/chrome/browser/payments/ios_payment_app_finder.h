...
// Checks and verifies that the URL payment method requested by a given
// merchant has an installed, valid payment app on the user's device. An
// installation check, using the payment method's corresponding URL scheme,
// is performed first to verify that the user has an installed payment app that
// can support the payment method. Following this check, the payment method's
// manifest and web app manifest are installed in order to collect the payment
// app's name, icon, and universal link. If this fails the payment app is not
// deemed to be valid. The manifests are located based on the payment method
// name, which is a URI that starts with "https://". The W3C-published non-URI
// payment method names e.g., "basic-card" are exceptions: these are common
// payment method names that do not have a manifest and can be used by any
// payment app.
class IOSPaymentAppFinder {
 public:
  // Callback to pass back the details of a payment app that can handle the
  // requested payment method.
  typedef base::Callback<void(std::string&)> Callback;
  ...
  // and universal link.
  IOSPaymentAppFinder(const std::string& payment_method_identifier,
 ...
 private:
  ...
  // first valid one.
  bool GetWebAppManifestURL(const std::string& input,
  ...
  bool GetPaymentAppDetails(const std::string& input,
                            std::string& app_name,
                            std::string& app_icon,
