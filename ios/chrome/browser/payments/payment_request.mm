...
void PaymentRequest::DoFullCardRequest(
    ...
    base::WeakPtr<autofill::payments::FullCardRequest::ResultDelegate>
        result_delegate) {
  [payment_request_ui_delegate_
      paymentRequestShouldRequestFullCreditCard:credit_card
                             withResultDelegate:result_delegate];
}
...
void PaymentRequest::PopulatePaymentMethodCache(
  ...
  if (native_app_instruments.empty() && credit_cards_to_suggest.empty()) {
    ...
    [payment_request_ui_delegate_ paymentRequestDidFetchPaymentMethods];
    return;
  }
