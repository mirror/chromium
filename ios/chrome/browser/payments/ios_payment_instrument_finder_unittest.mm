...
class IOSPaymentInstrumentFinderTest : public testing::Test {
 public:
  IOSPaymentInstrumentFinderTest()
      : scoped_task_environment_(
        ...
        ios_payment_instrument_finder_(
            base::MakeUnique<IOSPaymentInstrumentFinder>(
  ...
  const std::vector<std::unique_ptr<IOSPaymentInstrument>>& get_result() {
    return result_;
  }
  ...
  void ExpectUnableToParsePaymentMethodManifest(const std::string& input) {
    ...
    bool success = ios_payment_instrument_finder_
                       ->ParsePaymentManifestForWebAppManifestURL(
  ...
  void ExpectUnableToParseWebAppManifest(const std::string& input) {
    ...
    bool success =
        ios_payment_instrument_finder_->ParseWebAppManifestForPaymentAppDetails(
            input, &actual_app_name, &actual_app_icon, &actual_universal_link);
  ...
  void InstrumentsFoundCallback(
    ...
    if (run_loop_) {
      ...
      run_loop_not_needed_ = true;
    }
  }

  void FindInstrumentsWithMethods(std::vector<GURL>& url_methods) {
    ios_payment_instrument_finder_->GetIOSPaymentInstrumentsForMethods(
        url_methods,
        base::Bind(&IOSPaymentInstrumentFinderTest::InstrumentsFoundCallback,
  ...
  void FindInstrumentsWithWebAppManifest(const GURL& method,
    ...
    ios_payment_instrument_finder_->callback_ =
        base::Bind(&IOSPaymentInstrumentFinderTest::InstrumentsFoundCallback,
                   base::Unretained(this));
    ios_payment_instrument_finder_->num_payment_methods_remaining_ = 1;
    ios_payment_instrument_finder_->OnWebAppManifestDownloaded(method, content);

    RunLoop();
  }

  void RunLoop() {
    if (run_loop_not_needed_) {
      run_loop_not_needed_ = false;
    } else {
 ...
 private:
  ...
  // If the search for instruments returns without running any asynchronous
  // operations this flag is set to true so that the run loop isn't invoked.
  bool run_loop_not_needed_ = false;

  std::unique_ptr<IOSPaymentInstrumentFinder> ios_payment_instrument_finder_;
...
TEST_F(IOSPaymentInstrumentFinderTest, NoMethodsSuppliedNoInstruments) {
  ...
  IOSPaymentInstrumentFinderTest::FindInstrumentsWithMethods(url_methods);

  EXPECT_EQ(0u, num_payment_methods_remaining());
  EXPECT_EQ(0u, get_result().size());
}
...
TEST_F(IOSPaymentInstrumentFinderTest, OneValidMethodSuppliedOneInstrument) {
  IOSPaymentInstrumentFinderTest::FindInstrumentsWithWebAppManifest(
  ...
  EXPECT_EQ(0u, num_payment_methods_remaining());
  EXPECT_EQ(1u, get_result().size());
  EXPECT_EQ("Bobpay", base::UTF16ToASCII(get_result()[0]->GetLabel()));
  EXPECT_EQ("emerald-eon.appspot.com",
            base::UTF16ToASCII(get_result()[0]->GetSublabel()));
}
