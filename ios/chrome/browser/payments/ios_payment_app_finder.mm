...
IOSPaymentAppFinder::IOSPaymentAppFinder(
  ...
  if (PaymentAppRespondsToScheme(payment_method_identifier)) {
    InstallManifests(payment_method_identifier);
  }
}
...
void IOSPaymentAppFinder::InstallManifests(const std::string& method) {
  downloader_ = base::MakeUnique<PaymentManifestDownloader>(context_getter_);
  downloader_->DownloadPaymentMethodManifest(
      GURL(method),
      base::BindOnce(&IOSPaymentAppFinder::OnPaymentManifestDownload,
...
void IOSPaymentAppFinder::OnPaymentManifestDownload(
  ...
  bool success = GetWebAppManifestURL(content, web_app_manifest_url);
...
bool IOSPaymentAppFinder::GetWebAppManifestURL(
    const std::string& input,
    std::string& web_app_manifest_url) {
  DCHECK(web_app_manifest_url.empty());
  ...
  for (size_t i = 0; i < apps_number; ++i) {
    if (!list->GetString(i, &item) && item.empty()) {
      // The entry must be a string in order to proceed.
      continue;
    }

    GURL url(item);
    if (!url.is_valid() || !url.SchemeIs(url::kHttpsScheme)) {
      // The entry must be a valid URL with an HTTPS scheem to proceed.
      continue;
    }

    web_app_manifest_url = item;
    break;
  }

  // If we weren't able to find a valid web app manifest URL then we return
  // false.
  if (web_app_manifest_url.empty()) {
    return false;
  } else {
    return true;
  }
...
void IOSPaymentAppFinder::OnWebAppManifestDownload(const std::string& content) {
  ...
  bool success =
      GetPaymentAppDetails(content, app_name, app_icon, universal_link);

  if (success) {
    callback_.Run(universal_link);
  }
}

bool IOSPaymentAppFinder::GetPaymentAppDetails(const std::string& input,
                                               std::string& app_name,
                                               std::string& app_icon,
                                               std::string& universal_link) {
  std::unique_ptr<base::Value> value(base::JSONReader::Read(input));
  ...
  for (size_t i = 0; i < related_applications_size; ++i) {
    ...
    if (!related_application->HasKey(kRelatedApplicationsID)) {
      LOG(ERROR) << "Each \"platform\": \"itunes\" entry in "
                    "\"related_applications\" must contain \""
                 << kRelatedApplicationsID << "\".";
      continue;
    }
    ...
    if (!related_application->GetString(kRelatedApplicationsID, &link) ||
        link.empty()) {
      LOG(ERROR) << "\"" << kRelatedApplicationsID
