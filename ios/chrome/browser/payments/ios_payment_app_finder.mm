...
bool IOSPaymentAppFinder::PaymentAppRespondsToScheme(
  ...
  // TODO(crbug.com/60266): Implement this method to go through the dictionary
  // of payment method identifiers to scheme names that is maintained in the
  // IOSPaymentApp class which is currently in review. That dictionary will also
  // likely be moved to this class when both CLs land.
  return true;
}

void IOSPaymentAppFinder::InstallManifests(const std::string& method) {
  if (!PaymentAppRespondsToScheme(method)) {
    return;
  }
