...
class IOSPaymentInstrumentFinder {
 ...
 private:
  ...
  // the valid ones. |input| is the json encoded payment method manfiest to
  // parse. |out_web_app_manifest_urls| is set equal to the web app manifest
  ...
  // |web_app_manifest_base_url| is the web app manifest url without the ending
  // filename. This is passed to GetPaymentAppDetailsFromWebAppManifest for
  // validating an icon source path.
  ...
  // is the json encoded web app manifest to parse. |web_app_manifest_base_url|
  // is the web app manifest url without the ending filename which is needed for
  // determining if the relative path to an icon image source is valid.
  // |out_app_name|, |out_app_icon_url|, and |out_universal_link| are set to the
  // name, icon url, and universal link of a payment app.
