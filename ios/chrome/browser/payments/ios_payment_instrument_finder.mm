...
IOSPaymentInstrumentFinder::IOSPaymentInstrumentFinder(
    ...
    : downloader_(new PaymentManifestDownloader(context_getter)),
      image_fetcher_(new image_fetcher::IOSImageDataFetcherWrapper(
          context_getter,
...
IOSPaymentInstrumentFinder::FilterUnsupportedURLPaymentMethods(
  ...
  for (const GURL& method : queried_url_payment_method_identifiers) {
    ...
    // method is supported.
    std::string scheme =
        base::SysWideToUTF8(base::SysUTF8ToWide(enum_map[method.spec()]));
...
IOSPaymentInstrumentFinder::CreateIOSPaymentInstrumentsForMethods(
  ...
  std::vector<GURL> filtered_methods;
  ...
  if (!callback_.is_null()) {
    std::move(callback).Run(std::move(instruments_found_));
    return filtered_methods;
  }

  // The function should immediately return if there are 0 valid methods
  // supplied.
  filtered_methods = FilterUnsupportedURLPaymentMethods(methods);
...
void IOSPaymentInstrumentFinder::OnWebAppManifestDownloaded(
  ...
  GURL app_icon_url;
  GURL universal_link;
  if (!ParseWebAppManifestForPaymentAppDetails(content, web_app_manifest_origin,
