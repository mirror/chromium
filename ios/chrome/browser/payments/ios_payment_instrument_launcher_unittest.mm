...
class IOSPaymentInstrumentLauncherTest : public testing::Test {
 protected:
  ...
  void SetDelegate(IOSPaymentInstrumentLauncher& launcher,
                   payments::PaymentInstrument::Delegate* delegate) {
    launcher.delegate_ = delegate;
  }
...
TEST_F(IOSPaymentInstrumentLauncherTest,
  ...
  expected_dict.SetListWithoutPathExpansion("https://jefpay.com",
                                            std::move(jef_data_list));
  ...
  std::unique_ptr<base::ListValue> alice_data_list =
      base::MakeUnique<base::ListValue>();
  expected_dict.SetListWithoutPathExpansion("https://alicepay.com",
                                            std::move(alice_data_list));
...
TEST_F(IOSPaymentInstrumentLauncherTest,
  ...
  std::string malformed_link = "http://bad-link.com";
...
// readable object then the OnInstrumentDetailsError() function of the
// delegate is called.
TEST_F(IOSPaymentInstrumentLauncherTest,
       ReceiveResponseFromIOSPaymentInstrument_ResponseNotJSON) {
  FakePaymentInstrumentDelegate instrument_delegate;
  IOSPaymentInstrumentLauncher launcher;
  ...
  std::string stringified_response = "Doesn't convert to JSON";
  std::string base_64_params;
  base::Base64Encode(stringified_response, &base_64_params);

  SetDelegate(launcher, &instrument_delegate);
