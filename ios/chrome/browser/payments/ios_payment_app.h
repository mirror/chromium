...
#ifndef IOS_CHROME_BROWSER_PAYMENTS_IOS_PAYMENT_APP_H_
#define IOS_CHROME_BROWSER_PAYMENTS_IOS_PAYMENT_APP_H_
...
// These scheme names are also enumerated in LSApplicationQueriesSchemes
// in the plist file.
...
class IOSPaymentApp : public PaymentInstrument {
 public:
  // Initializes an IOSPaymentApp. |method_name| is the url payment method
  // identifier for this iOS payment app. |universal_link| is the unique
  // link for the app that is used to open it from Chrome. |app_name| is
  // name of the iOS payment app. The IOSPaymentApp takes ownership
  ...
  IOSPaymentApp(const std::string& method_name,
                const std::string& universal_link,
                const std::string& app_name,
                UIImage* icon_image,
                id<PaymentRequestUIDelegate> payment_request_ui_delegate);

  ~IOSPaymentApp() override;
  ...
  // Given that the icon for the iOS Payment App can only be determined at
  // run-time, the icon is obtained using this UIImage object rather than
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(IOSPaymentApp);
};

}  // namespace payments

#endif  // IOS_CHROME_BROWSER_PAYMENTS_IOS_PAYMENT_APP_H_
