...
TEST_F(PaymentRequestPaymentResponseHelperTest, PaymentResponse) {
  ...
  [consumer_mock onSelector:selector
       callBlockExpectation:^(const web::PaymentResponse& response) {
         ...
         EXPECT_EQ(base::ASCIIToUTF16(GetMethodName()), response.method_name);
...
// Tests the the generated PaymentResponse has a shipping address only if one is
// requested.
...
// Tests the the generated PaymentResponse has contact information only if it is
// requested.
