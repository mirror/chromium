...
TEST_F(PaymentRequestTest, SelectedPaymentMethod_ExpiredCard) {
  ...
  TestPaymentRequest payment_request(web_payment_request,
                                     &personal_data_manager);
  payments::AutofillPaymentInstrument* paymentInstrument =
      static_cast<payments::AutofillPaymentInstrument*>(
          payment_request.selected_payment_method());
  EXPECT_EQ(credit_card.guid(), paymentInstrument->credit_card()->guid());
}
...
TEST_F(PaymentRequestTest, SelectedPaymentMethod_Complete) {
  ...
  payments::AutofillPaymentInstrument* paymentInstrument =
      static_cast<payments::AutofillPaymentInstrument*>(
          payment_request.selected_payment_method());
  EXPECT_EQ(credit_card2.guid(), paymentInstrument->credit_card()->guid());
}
