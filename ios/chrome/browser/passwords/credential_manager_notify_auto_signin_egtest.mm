...
// Snackbar should be displayed for 3 seconds. 3.5 is safer timeout to check and
// also smaller than 4, which is default value for MDCSnackbarMessage.duration.
const CFTimeInterval kSnackbarDisappearanceTimeout = 3.5;

}  // namespace

// This class tests that a snackbar saying "Signing in as ..." is displayed on
// auto sign-in and disappears after 3 seconds.
...
- (void)setUp {
  ...
  password_manager::PasswordStore* password_store =
      IOSChromePasswordStoreFactory::GetForBrowserState(
          ...
          .get();
  GREYAssertTrue(password_store != nullptr,
  ...
  password_credential_form_.username_value =
      base::ASCIIToUTF16("johndoe@example.com");
  password_credential_form_.password_value =
      base::ASCIIToUTF16("ilovejanedoe123");
  password_credential_form_.origin =
      chrome_test_util::GetCurrentWebState()->GetLastCommittedURL().GetOrigin();
  password_credential_form_.signon_realm =
      password_credential_form_.origin.spec();
...
- (void)testSnackBarAppearsOnAutoSignIn {
  ...
  // Matches the snackbar by its accessibilityLabel.
  ...
  // Gives some time for the snackbar to appear.
  GREYAssert(testing::WaitUntilConditionOrTimeout(kSnackbarAppearanceTimeout,
                                                  wait_for_appearance),
             @"Snackbar did not appear");
  // Ensures that snackbar disappears after time limit.
