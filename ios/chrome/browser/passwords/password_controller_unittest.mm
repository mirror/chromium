...
TEST_F(PasswordControllerTest, SelectingSuggestionShouldFillPasswordForm) {
  ...
  } const kTestData[] = {{"f1", "u'", "p'"}, {"f1", "u2", "p2"}};
  ...
  for (size_t form_i = 0; form_i < arraysize(kTestData); ++form_i) {
    ...
    const auto& test_data = kTestData[form_i];
    [passwordController_
        retrieveSuggestionsForForm:base::SysUTF16ToNSString(test_data.form_name)
                             field:base::SysUTF16ToNSString(
                                       test_data.username_element)
                         fieldType:@"text"
                              type:@"focus"
                        typedValue:@"abc"
                          webState:web_state()
    ...
    FormSuggestion* suggestion = [FormSuggestion suggestionWithValue:@"abc"
                                                  displayDescription:nil
                                                                icon:nil
                                                          identifier:0];
    NSString* verification_script = [kUsernamePasswordVerificationScript stringByReplacingOccurrencesOfString:@"un" withString:@"oe"]
