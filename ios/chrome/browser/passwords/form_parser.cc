...
// Helper struct that is used for return from parsing function.
struct ParseResult {
...
// Tries to parse based on autocomplete attributes.
// Assumption on usage autocomplete attributes:
...
// Filter fields based on binary conditions.
...
// 1. If there is a focusable password field, return only visible.
...
// Detects different password fields from |passwords|. This function has the
// same logic as LocateSpecificPasswords from password_form_conversion_utils.cc.
...
std::unique_ptr<ParseResult> ParseUsingBaseHeuristics(
  ...
  // If password elements are found. Try to find a username.
  result->username_field =
      FindUsernameFieldBaseHeuristics(fields, passwords[0], mode);
  return result;
}

// Set username and password fields from |parse_result| to |password_form|.
