...
const FormFieldData* FindUsernameFieldBaseHeuristics(
  ...
  // focusable.
  if (HasFocusable(username_candidates))
...
void SetFields(const ParseResult& parse_result, PasswordForm* result) {
  ...
  if (parse_result.new_password_field) {
    result->new_password_element = parse_result.new_password_field->id;
    result->new_password_value = parse_result.new_password_field->value;
  }
