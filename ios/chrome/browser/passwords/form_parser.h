...
struct ParseResult {
    ...
    size_t new_password_index = -1; // todo const NotFound
    size_t confirmation_password_index -1;
};
...
private:
    std::vector<autofill::FormFieldData> FilterOutCreditCardFields(const std::vector<autofill::FormFieldData>& fields);
};
