...
class JsCredentialManagerTest : public web::WebJsTest<web::WebTestWithWebState> {
 public:
  JsCredentialManagerTest() : web::WebJsTest<web::WebTestWithWebState>(@[@"credential_manager"]) {}
  void SetUp() override {
...
TEST_F(JsCredentialManagerTest, ResolveWithVoid) {
  ...
  // Only when the promise is resolved with void, will the |window.test_result_| be true.
  ExecuteJavaScript(
...
// Tests that RejectPromiseWithTypeError resolves the promise with TypeError and correct message.
TEST_F(JsCredentialManagerTest, RejectWithTypeError) {
  ...
  ExecuteJavaScript(
    @"__gCrWeb.credentialManager.createPromise_(100)."
        ...
        "});");
  manager_->RejectPromiseWithTypeError(100, base::ASCIIToUTF16("message with \"quotation\" marks"));
  EXPECT_NSEQ(ExecuteJavaScript(@"window.test_result_valid_type_"), @YES);
  EXPECT_NSEQ(ExecuteJavaScript(@"window.test_result_message_"), @"message with \"quotation\" marks");
}
