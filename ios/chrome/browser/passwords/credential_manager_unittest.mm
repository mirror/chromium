...
// Test that WebStateContentIsSecureHTML returns false for HTTP origin.
...
// Test that WebStateContentIsSecureHTML returns false for HTTPS origin with
...
// Test that file origin is accepted as secure context.
TEST_F(CredentialManagerTest, FileIsSecureContext) {
  LoadUrl(GURL(kFileOrigin));
  EXPECT_TRUE(WebStateContentIsSecureHTML(web_state()));
}

// Test that content must be HTML.
TEST_F(CredentialManagerTest, ContentMustBeHtml) {
