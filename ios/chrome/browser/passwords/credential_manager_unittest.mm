...
class CredentialManagerTest : public testing::Test {
 public:
  void SetUp() override {
    ...
    credential_manager_ =
        base::MakeUnique<CredentialManager>(client_.get(), nullptr);
...
TEST_F(CredentialManagerTest, MessageWithInvalidRequestId) {
  ...
  EXPECT_FALSE(credential_manager_->HandleScriptCommand(
      JSON, GURL(kTestWebOrigin), false));
...
TEST_F(CredentialManagerTest, StorePasswordCredential) {
  ...
  EXPECT_TRUE(credential_manager_->HandleScriptCommand(
      JSON, GURL(kTestWebOrigin), false));
...
TEST_F(CredentialManagerTest, ParseGetArguments) {
  ...
  EXPECT_EQ(CredentialMediationRequirement::kOptional,
            CredentialManager::ParseMediationRequirement(JSON));
  // Check if value of |mediation| is parsed correctly.
  JSON.SetString("mediation", "silent");
  EXPECT_EQ(CredentialMediationRequirement::kSilent,
            CredentialManager::ParseMediationRequirement(JSON));
  JSON.SetString("mediation", "required");
  EXPECT_EQ(CredentialMediationRequirement::kRequired,
            CredentialManager::ParseMediationRequirement(JSON));
