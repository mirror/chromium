...
void ResolveOrRejectPromiseWithValue(web::WebState* web_state,
                                     int promise_id,
                                     bool resolve,
                                     const std::string& value) {
  std::string script = base::StringPrintf(
      "__gCrWeb.credentialManager.%s[%d](%s);",
      resolve ? "resolvers_" : "rejecters_", promise_id, value.c_str());
  web_state->ExecuteJavaScript(base::UTF8ToUTF16(script));
}

}  // namespace

// This function must be used with CredentialInfo obtained from PasswordStore
// because it doesn't validate its fields.
void ResolvePromiseWithCredentialInfo(
  ...
  ResolveOrRejectPromiseWithValue(web_state, promise_id, /*resolve=*/true,
                                  credential_str);
}
...
void RejectPromiseWithTypeError(web::WebState* web_state,
  ...
  ResolveOrRejectPromiseWithValue(web_state, promise_id, /*resolve=*/false,
                                  type_error_str);
}
...
void RejectPromiseWithInvalidStateError(web::WebState* web_state,
  ...
  std::string invalid_state_err_str =
      base::StringPrintf("new DOMException(%s, DOMException.INVALID_STATE_ERR)",
...
void RejectPromiseWithNotSupportedError(web::WebState* web_state,
  ...
  std::string not_supported_err_str =
      base::StringPrintf("new DOMException(%s, DOMException.NOT_SUPPORTED_ERR)",
                         base::GetQuotedJSONString(message).c_str());
  ResolveOrRejectPromiseWithValue(web_state, promise_id, /*resolve=*/false,
