...
bool CredentialManager::ParseFederations(const base::DictionaryValue& json,
  ...
  for (size_t i = 0; i < lst->GetSize(); i++) {
    ...
    GURL gurl(s);
    if (!gurl.is_valid()) {
      // Element of |providers| is invalid URL.
      return false;
    }
    federations.push_back(gurl);
  }
  return true;
}
...
bool CredentialManager::ParseCredentialType(const base::DictionaryValue& json,
  ...
  credential_type = CredentialType::CREDENTIAL_TYPE_EMPTY;
  return false;
}
...
bool CredentialManager::ParseCredentialDictionary(
  ...
  if (!json.GetString("name", &credential.name)) {
    // |name| is required.
    return false;
  }
  std::string iconURL;
  if (json.GetString("iconURL", &iconURL) && iconURL != "") {
    credential.icon = GURL(iconURL);
    if (!credential.icon.is_valid() || !web::IsOriginSecure(credential.icon)) {
