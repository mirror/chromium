...
void AccountSelectFillData::Add(
  ...
  forms_[key] = FormInfo();  // todo better
  FormInfo& form_info = forms_[key];
...
bool AccountSelectFillData::Empty() {
...
bool AccountSelectFillData::IsSuggestionsAvailable(
  ...
  return GetForm(form_name, field_name) != nullptr;
}
...
std::vector<UsernameAndRealm> AccountSelectFillData::RetrieveSuggestions(
  ...
  last_requested_form_ = GetForm(form_name, field_name);
...
const FormInfo* AccountSelectFillData::GetForm(
    const base::string16& form_name,
    const base::string16& field_name) const {
