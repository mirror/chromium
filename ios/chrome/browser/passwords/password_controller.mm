...
- (void)getPasswordForms:(std::vector<autofill::PasswordForm>*)forms
  ...
  if (!autofill::GetExtractedFormsData(JSONNSString, false, base::string16(),
                                       pageURL, &formsData))
    return;
...
- (void)extractSubmittedPasswordForm:(const std::string&)formName
  ...
  id extractSubmittedFormCompletionHandler = ^(NSString* jsonString) {
    std::unique_ptr<base::Value> formJSON = autofill::ParseJSON(jsonString);
    if (!formJSON) {
    ...
    FormData formData;
    if (!autofill::ExtractFormData(*formJSON, false, base::string16(), pageURL,
                                   &formData)) {
      completionHandler(NO, PasswordForm());
      return;
    }
