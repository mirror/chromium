...
DEFINE_WEB_STATE_USER_DATA_KEY(InsecureInputWebStateObserver);

void InsecureInputWebStateObserver::DidShowPasswordFieldInInsecureContext() {
  ...
  if (!input_events->input_events()->password_field_shown) {
    ...
    web_state()->OnVisibleSecurityStateChange();
  }
}

void InsecureInputWebStateObserver::DidInteractWithNonsecureCreditCardInput() {
  DCHECK(!web::IsOriginSecure(web_state()->GetLastCommittedURL()));
...
void InsecureInputWebStateObserver::DidEditFieldInInsecureContext() {
  DCHECK(!web::IsOriginSecure(web_state()->GetLastCommittedURL()));
...
InsecureInputWebStateObserver::~InsecureInputWebStateObserver() = default;

InsecureInputWebStateObserver::InsecureInputWebStateObserver(
    web::WebState* web_state)
    : web::WebStateObserver(web_state) {
  DCHECK(web_state);
}

void InsecureInputWebStateObserver::FormActivityRegistered(
    web::WebState* web_state,
