...
void IOSSSLErrorHandler::HandleSSLError(
  ...
  IOSSSLErrorHandler* error_handler = FromWebState(web_state);
...
void IOSSSLErrorHandler::ShowCaptivePortalInterstitial(GURL landing_url) {
  ...
  IOSCaptivePortalBlockingPage* page = new IOSCaptivePortalBlockingPage(
      web_state_, request_url_, landing_url, delegate, dismissal_callback);
  page->Show();
  // Once an interstitial is displayed, no need to keep the handler around.
  // This is the equivalent of "delete this".
  RemoveFromWebState(web_state_);
