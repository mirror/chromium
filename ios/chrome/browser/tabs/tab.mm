...
- (void)fetchFavicon {
  ...
  if (faviconDriver)
    faviconDriver->OnPageNavigation(url, /*is_same_document=*/false);
}
...
- (void)webState:(web::WebState*)webState
  ...
  if (navigation->IsSameDocument()) {
    ...
    if (faviconDriver)
      faviconDriver->OnPageNavigation(navigation->GetUrl(),
...
- (void)webState:(web::WebState*)webState
  ...
  if (faviconDriver)
    faviconDriver->OnPageNavigation(lastCommittedURL, details.is_in_page);
  [_parentTabModel notifyTabChanged:self];
  if (_parentTabModel) {
    [[NSNotificationCenter defaultCenter]
