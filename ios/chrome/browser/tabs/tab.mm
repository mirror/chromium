...
- (void)openExternalURL:(const GURL&)url
  ...
  GURL copyOfSourceURL = sourceURL;
  void (^callback)(BOOL) = ^(BOOL success) {
    Tab* strongSelf = weakSelf;
    // Since tab can out-live web state, account for possibility of navigation
    // manager being null.
    if (success && strongSelf && [strongSelf navigationManager]) {
      // If navigation manager is still alive, clears pending navigation
      // history after successfully launching the external app.
