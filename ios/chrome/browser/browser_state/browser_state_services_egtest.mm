...
// invoked. |identityManager| is passed simply to ensure that our connection to
// the IdentityManager implementation remains alive long enough for the callback
// to reach us.
...
- (void)testConnectionToIdentityService {
  ...
  // Call GetPrimaryAccountInfo(), making sure to keep our end of the
  // connection alive until the callback is received.
  identity::mojom::IdentityManager* rawIdentityManager = identityManager.get();
  bool getPrimaryAccountInfoCallbackCalled = false;
  rawIdentityManager->GetPrimaryAccountInfo(
      base::BindOnce(&OnGotPrimaryAccountInfo, base::Passed(&identityManager),
