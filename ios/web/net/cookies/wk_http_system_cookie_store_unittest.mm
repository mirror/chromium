...
class API_AVAILABLE(ios(11.0)) WKHTTPSystemCookieStoreTestDelegate {
 public:
  WKHTTPSystemCookieStoreTestDelegate() {
    if (@available(iOS 11, *)) {
      shared_store_ = WKWebsiteDataStore.nonPersistentDataStore.httpCookieStore;
      store_ = base::MakeUnique<web::WKHTTPSystemCookieStore>(shared_store_);
    }
  }

  bool IsTestEnabled() {
    // Test should only Run on iOS 11+.
    if (@available(iOS 11, *))
      return true;
    return false;
  }
  ...
  bool IsCookieSet(NSHTTPCookie* system_cookie, NSURL* url) {
    ...
    bool callback_success = testing::WaitUntilConditionOrTimeout(2.0, ^bool {
