...
bool IsCookieForURL(NSHTTPCookie* cookie, const GURL& url) {
  ...
  // to support cookieOptions this funciton can be modified to support that.
  ...
  return cc.IncludeForRequestURL(url, opts);
}
...
void WKHTTPSystemCookieStore::RunSystemCookieCallbackForCookies(
  ...
  if (callback.is_null())
    return;
  NSArray* result =
      [(NSArray*)cookies sortedArrayUsingFunction:CompareCookies
