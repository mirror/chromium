...
TEST_F(SystemCookieStoreUtilTest, CreateSystemCookieStore) {
  ...
  if (@available(iOS 11, *)) {
    EXPECT_EQ(
        net::SystemCookieStore::SystemCookieStoreType::kWKHTTPSystemCookieStore,
        cookie_store->GetCookieStoreType());
  } else {
    EXPECT_EQ(
        net::SystemCookieStore::SystemCookieStoreType::kNSHTTPSystemCookieStore,
        cookie_store->GetCookieStoreType());
  }
}
