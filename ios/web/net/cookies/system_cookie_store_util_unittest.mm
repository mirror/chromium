...
// Sets |cookie| in SystemCookieStore |store| , and wait until the
bool SetCookieInCookieStore(NSHTTPCookie* cookie,
  ...
  return testing::WaitUntilConditionOrTimeout(testing::kWaitForCookiesTimeout,
                                              ^bool {
...
TEST_F(SystemCookieStoreUtilTest, CreateSystemCookieStore) {
  ...
  std::unique_ptr<net::SystemCookieStore> system_cookie_store =
      web::CreateSystemCookieStore(&browser_state);
