...
void WebMainLoop::CreateStartupTasks(
  ...
  result = CreateThreads(init_params_callback);
  if (result > 0)
    return;
...
int WebMainLoop::CreateThreads(
    TaskSchedulerInitParamsCallback init_params_callback) {
  base::TaskScheduler::InitParams* init_params = nullptr;
  if (!init_params_callback.is_null()) {
    init_params = init_params_callback.Run().get();
  }
  ios_global_state::StartTaskScheduler(init_params);
