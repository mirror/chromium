...
// Details for currently in progress element fetches. The objects are instances
// of HTMLElementFetchRequest and are keyed by
// |HTMLElementFetchRequest.identifier|.
@property(nonatomic, strong) NSMutableDictionary* ongoingElementFetchRequests;
...
@synthesize ongoingElementFetchRequests = _ongoingElementFetchRequests;
...
- (instancetype)initWithWebView:(WKWebView*)webView
  ...
  if (self) {
    ...
    _ongoingElementFetchRequests = [[NSMutableDictionary alloc] init];
...
- (void)didReceiveScriptMessage:(WKScriptMessage*)message {
  ...
  NSString* identifier = response[web::kContextMenuElementRequestID];
  HTMLElementFetchRequest* fetchDetails =
      _ongoingElementFetchRequests[identifier];
  // Validate that the response is genuine by ensuring the request originated
  // from this CRWContextMenuController instance. If not found, a malicious page
  // (or frame within it) could be sending the message, so perform no action.
  if (fetchDetails) {
    [_ongoingElementFetchRequests removeObjectForKey:identifier];
    [self logElementFetchDurationWithStartTime:fetchDetails.fetchStartTime];
    [fetchDetails receivedResponse:response];
  }
}
...
- (void)fetchDOMElementAtPoint:(CGPoint)point
  ...
  if (base::FeatureList::IsEnabled(
    ...
    [_ongoingElementFetchRequests setObject:fetchDetails
                                     forKey:fetchDetails.identifier];
    ...
    completionHandler = ^(id element, NSError*) {
      [self logElementFetchDurationWithStartTime:getElementStartTime];
