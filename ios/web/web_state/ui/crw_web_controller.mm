...
- (void)loadErrorInNativeViewForNavigationItem:(web::NavigationItemImpl*)item
  ...
  if (!web::GetWebClient()->IsSlimNavigationManagerEnabled()) {
    ...
    item->SetErrorRetryState(
        web::ErrorRetryState::DISPLAYING_ERROR_FOR_FAILED_NAVIGATION);
  }
...
- (void)handleLoadError:(NSError*)error
  ...
  if (!web::GetWebClient()->IsSlimNavigationManagerEnabled()) {
    ...
    if (errorRetryState ==
        web::ErrorRetryState::READY_TO_DISPLAY_ERROR_FOR_FAILED_NAVIGATION) {
      ...
      DCHECK_EQ(web::ErrorRetryState::NO_NAVIGATION_ERROR, errorRetryState);
