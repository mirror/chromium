...
- (void)webPageChanged:(const web::NavigationContext*)context;
...
- (void)loadCurrentURLInWebView {
  ...
  if (!targetURL.is_valid()) {
    [self didFinishWithURL:targetURL loadSuccess:NO context:nil];
    return;
  }
...
- (GURL)webURLWithTrustLevel:(web::URLVerificationTrustLevel*)trustLevel {
  DCHECK(trustLevel);
  *trustLevel = web::URLVerificationTrustLevel::kAbsolute;
  // NOTE(danyao): use last committed item URL because _documentURL may be
  // placeholder.
...
- (void)didFinishWithURL:(const GURL&)currentURL
             loadSuccess:(BOOL)loadSuccess
                 context:(const web::NavigationContext*)context {
  DCHECK(_loadPhase == web::PAGE_LOADED);
...
- (void)webPageChanged:(const web::NavigationContext*)context {
  ...
  [self didStartLoading];
  // Do not commit pending item if we are in the middle of loading a placeholder
  // URL. The item will be committed when the native content or webUI is
  // displayed.
  if (!IsPlaceholderUrl(context->GetUrl())) {
    self.navigationManagerImpl->CommitPendingItem();
  }
...
- (void)webViewLoadingStateDidChange {
  ...
  if (!navigationWasCommitted && ![_pendingNavigationInfo cancelled]) {
    ...
    if (!existingContext) {
      ...
      [self webPageChanged:newContext.get()];
      ...
      [self webPageChanged:existingContext];
