...
// go/bling-navigation-experiment for details.
...
GURL ExtractAppSpecificUrlFromPlaceholderUrl(const GURL& url) {
  ...
  if (IsPlaceholderUrl(url) && net::GetValueForKeyInQuery(url, "for", &value)) {
    ...
    if (decoded_url.is_valid() &&
        web::GetWebClient()->IsAppSpecificURL(decoded_url))
      return decoded_url;
  }
...
using crw_web_controller_test_visible::IsPlaceholderUrl;
using crw_web_controller_test_visible::CreatePlaceholderUrlForUrl;
using crw_web_controller_test_visible::ExtractAppSpecificUrlFromPlaceholderUrl;
