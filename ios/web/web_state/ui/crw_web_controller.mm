...
- (void)webView:(WKWebView*)webView
  ...
  if (web::GetWebClient()->IsSlimNavigationManagerEnabled()) {
    ...
    // WKBasedNavigationManager doesn't use |loadCurrentURL| for reload. So this
    // is the first point where a form repost would be detected. Display the
    // confirmation dialog.
    if (action.navigationType == WKNavigationTypeFormResubmitted &&
        [action.request.HTTPMethod isEqual:@"POST"]) {
