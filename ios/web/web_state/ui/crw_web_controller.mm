...
- (void)loadErrorInNativeViewForNavigationItem:(web::NavigationItem*)item
                             navigationContext:
...
- (void)loadErrorInNativeViewForNavigationItem:(web::NavigationItem*)item
  ...
  if (!web::GetWebClient()->IsSlimNavigationManagerEnabled()) {
    ...
    self.navigationManagerImpl->SetErrorRetryState(
        item, web::ErrorRetryState::ERROR_DISPLAYED);
  }
...
- (void)handleLoadError:(NSError*)error
  ...
  DCHECK_EQ(self.currentNavItem->GetURL(), navigationContext->GetUrl());
  ...
  if (!web::GetWebClient()->IsSlimNavigationManagerEnabled()) {
    ...
    web::ErrorRetryState errorRetryState =
        self.navigationManagerImpl->GetErrorRetryState(self.currentNavItem);
    if (errorRetryState == web::ErrorRetryState::HAS_HISTORY_ENTRY) {
      ...
      // The navigation item that failed to load is not yet in the session
      // history. This can arise when the failure happens during the provisional
      // load of a new page. Kick off a placeholder load to force the generation
      // of a history entry. |loadErrorInNativeViewForNavigationItem| will be
      // called when the placeholder finishes loading.
      DCHECK_EQ(web::ErrorRetryState::NO_ERROR_DISPLAYED, errorRetryState);
  ...
  if (web::GetWebClient()->IsSlimNavigationManagerEnabled()) {
    ...
    if (IsPlaceholderUrl(webViewURL)) {
      ...
      if ([self shouldLoadURLInNativeView:item->GetURL()]) {
        ...
        self.navigationManagerImpl->SetErrorRetryState(
            item, web::ErrorRetryState::HAS_HISTORY_ENTRY);
...
- (void)webView:(WKWebView*)webView
  ...
  if (web::GetWebClient()->IsSlimNavigationManagerEnabled()) {
    ...
    self.navigationManagerImpl->SetErrorRetryState(
        self.currentNavItem, web::ErrorRetryState::HAS_HISTORY_ENTRY);
  }
