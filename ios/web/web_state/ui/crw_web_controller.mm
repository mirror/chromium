...
- (void)webView:(WKWebView*)webView
  ...
  if (web::GetWebClient()->IsAppSpecificURL(webViewURL) &&
    ...
    // Restart app specific URL loads to properly capture state. However, if
    // |backForwardList.currentItem| is a placeholder URL for the provisional
    // load, this is an in-progress app-specific and should not be restarted.
  ...
  if (web::GetWebClient()->IsSlimNavigationManagerEnabled()) {
    ...
    if (IsPlaceholderUrl(webViewURL)) {
      ...
      if (isWebUIURL && !_webUIManager) {
        // WebUIManager may not have been initiated if this is a back/forward
        // navigation.
