...
registerLoadRequestForURL:(const GURL&)requestURL
  ...
  if (self.navigationManagerImpl->GetPendingItem()) {
    ...
    web::NavigationItem* lastCommittedItem =
        self.sessionController.lastCommittedItem;
    bool isLastCommittedItemNative =
        lastCommittedItem &&
        [self shouldLoadURLInNativeView:lastCommittedItem->GetURL()];
    if (PageTransitionCoreTypeIs(transition, ui::PAGE_TRANSITION_RELOAD) &&
        !isLastCommittedItemNative) {
