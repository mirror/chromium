...
// Returns an autoreleased instance of WKUserScript to be added to
// configuration's userContentController.
WKUserScript* InternalGetFramePageScript(BrowserState* browser_state) {
  return [[WKUserScript alloc]
        initWithSource:GetFramePageScript(browser_state)
         injectionTime:WKUserScriptInjectionTimeAtDocumentStart
      forMainFrameOnly:NO];
}
...
WKWebViewConfigurationProvider::GetWebViewConfiguration() {
  ...
  if (!configuration_) {
    ...
    [[configuration_ userContentController]
        addUserScript:InternalGetFramePageScript(browser_state_)];
  }
