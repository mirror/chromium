...
TEST_F(NavigationCallbacksTest, NewPageNavigation) {
  ...
  EXPECT_CALL(*observer_, DidStartLoading(_));
  EXPECT_CALL(*decider_, ShouldAllowRequest(_, _)).WillOnce(Return(true));
  EXPECT_CALL(*observer_, DidStartNavigation(_, _))
  ...
  EXPECT_CALL(*observer_, DidFinishNavigation(_, _))
      .WillOnce(VerifyNewPageFinishedContext(web_state(), url, &context));
  EXPECT_CALL(*observer_, DidStopLoading(_));
  LoadUrl(url);
}
