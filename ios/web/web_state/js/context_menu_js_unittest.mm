...
TEST_F(ContextMenuJsTest, LinkOfTextFromTallPage) {
  ...
  CGSize scrollViewSize =
      web_state()->GetWebViewProxy().scrollViewProxy.frame.size;
  CGFloat offset = contentSize.height - scrollViewSize.height;
  ...
  id result = ExecuteGetElementFromPointJavaScript(1, contentSize.height - 5);
  NSDictionary* expected_result = @{
