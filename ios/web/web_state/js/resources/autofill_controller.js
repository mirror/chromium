 ...
 * in chromium/src/components/autofill/content/renderer/form_autofill_util.cc
 *
 * TODO: Make |element_map| a Map when Chrome makes iOS 8 and Safari 8 part of
...
__gCrWeb.autofill['fillForm'] = function(data, forceFillFieldName) {
  ...
  for (var i = 0; i < controlElements.length; ++i) {
    ...
    // TODO: Handle __gCrWeb.autofill.isCheckableElement(element) == true.
    //       |is_checked| is not currently passed in by the caller.
...
__gCrWeb.autofill['clearAutofilledFields'] = function(formName) {
  ...
  for (var i = 0; i < controlElements.length; ++i) {
    ...
    if (__gCrWeb.autofill.isTextInput(element) ||
      ...
      // TODO: Store initial values and reset to the correct one here.
      ...
      // TODO: Handle checkable elements. They aren't properly supported by iOS
      //       Autofill yet.
    }
...
__gCrWeb.autofill.webFormElementToFormData = function(
  ...
  // TODO: Check if method is unused and remove.
 ...
 * @return {string} The label of element.
 */
__gCrWeb.autofill.inferLabelFromEnclosingLabel =
    function(element) {
  if (!element) {
    return '';
  }
  ...
  return '';
     ...
     * from a surrounding div table, e.g. <div>Some Text<span><input
     * ...></span></div> e.g. <div>Some Text</div><div><input ...></div>
     *
     * Because this is already traversing the <div> structure, if it finds a
     * <label> sibling along the way, infer from that <label>.
     *
     * It is based on the logic in
 ...
 */
__gCrWeb.autofill.ancestorTagNames =
    function(element) {
  var tagNames = [];
  var parentNode = element.parentNode;
  while (parentNode) {
  ...
  return tagNames;
     ...
     * DOM, e.g. the contents of the preceding <p> tag or text element.
     *
     * It is based on the logic in
     ...
     *
     * @param {FormControlElement} element An element to examine.
     * @return {string} The inferred label of element, or '' if none could be
     *     found.
     */
