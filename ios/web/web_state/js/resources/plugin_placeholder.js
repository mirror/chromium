...
(function() {
   ...
   * @private
   */
  var getEmbedChild_ = function(node) {
   ...
   * @private
   */
  var getPluginSize_ = function(plugin) {
    var style;
    ...
    var embedChild = getEmbedChild_(plugin);
    if (embedChild) {
   ...
   * @private
   */
  var isSignificantPlugin_ = function(plugin) {
    var windowWidth = window.innerWidth;
    var windowHeight = window.innerHeight;
    var pluginSize = getPluginSize_(plugin);
   ...
   * @private
   */
  var addPluginPlaceholders_ = function(message, plugins) {
    var i;
    for (i = 0; i < plugins.length; i++) {
      var plugin = plugins[i];
      if (!isSignificantPlugin_(plugin)) {
        continue;
      }
 ...
 if (plugins.length > 0) {
   ...
   addPluginPlaceholders_('$(PLUGIN_NOT_SUPPORTED_TEXT)', plugins);
 }
