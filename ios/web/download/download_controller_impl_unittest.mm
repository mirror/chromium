...
TEST_F(DownloadControllerImplTest, OnDownloadCreated) {
  ...
  DownloadTask* task = download_controller()->CreateDownloadTask(
      &web_state_, identifier, url, kContentDisposition, /*total_bytes=*/-1,
      kMimeType);
  ASSERT_TRUE(task);

  ASSERT_EQ(1U, delegate_.alive_download_tasks().size());
  EXPECT_EQ(task, delegate_.alive_download_tasks()[0].second.get());
