...
// Handling renderer-initiated download taks and resume unfinished downloads
// after app relaunch.
//
// Handling renderer-initiated downloads example:
// class MyDownloadManager : public DownloadControllerObserver {
//  public:
//   MyDownloadManager(BrowserState* state) {
//     DownloadController::FromBrowserState(state)->AddObserver(this);
//   }
//   ~MyDownloadManager() {
//     DownloadController::FromBrowserState(state)->RemoveObserver(this);
//   }
//   void OnDownloadCreated(const WebState*, scoped_refptr<DownloadTask> task) {
//     task->SetResponseWriter(GetURLFetcherFileWriter());
//     task->Start();
//     .. retain |task| until download is complete...
//   }
//   void OnDownloadUpdated(const WebState*, DownloadTask* task) {
//     if (task->IsDone()) {
...
class DownloadController {
 public:
  ...
  // initiate a renderer-initiated download (those download tasks are created
  // automatically).
  ...
  // from DownloadTask, which was suspended when the app has been deactivated.
  ...
  // Adds and Removes DownloadControllerObserver. Clients must call
  // RemoveObserver() before destroying BrowserState associated with this
  // DownloadController.
