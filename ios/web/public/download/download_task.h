...
class DownloadTask : public base::RefCounted<DownloadTask> {
 public:
  // Starts the download. Clients must set a valid response writer via
  // SetResponseWriter() before starting the download. Idempotent.
  virtual void Start() = 0;

  // Must be called with a valid |writer| before starting the download. This
  // method allows clients to perform in-memory or in-file downloads.
  virtual void SetResponseWriter(
      std::unique_ptr<net::URLFetcherResponseWriter> writer) = 0;

  // Response writer, which was set via SetResponseWriter(). Can be used to
  // obtain the download data.
  ...
  // downloads after app relaunch (see example in DownloadController class
  // comments).
  virtual NSString* GetIndentifier() const = 0;

  // The URL that the download request originally attempted to fetch. This may
  // differ actually final download URL if there were redirects.
  ...
  // Error code for this download task if any. 0 if the download is still in
  // progress or the download has sucessfully completed. See net_errors.h for
