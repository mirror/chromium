...
void WKBasedNavigationManagerImpl::AddPendingItem(
  ...
  // AddPendingItem is always called in |didStartProvisionalNavigation|. Under
...
void WKBasedNavigationManagerImpl::DiscardNonCommittedItems() {  // TODO: test
  pending_item_.reset();
...
void WKBasedNavigationManagerImpl::GoBack() {  // TODO: add tests
  if (transient_item_) {
    transient_item_.reset();
    return;
  }
...
void WKBasedNavigationManagerImpl::GoForward() {  // TODO: add tests
...
void WKBasedNavigationManagerImpl::SyncNavigationItemsToWKBackForwardList() {
  ...
  // the two to get out of sync at the tail. The following heuristics are used
  // to sync |navigation_item_impls_| to WKBackForwardList, which is considered
  // the ground truth:
...
WKBasedNavigationManagerImpl::CreateNavigationItem(
  ...
  // TODO: move into base class and refactor crw_session_controller
