...
TEST_P(NavigationManagerTest, OffsetsWithPendingTransientEntry) {
  ...
  mock_wk_list_.backList = [NSArray arrayWithObjects:wk_item0, wk_item1, nil];
  navigation_manager()->CommitPendingItem();
  ...
  mock_wk_list_.currentItem = wk_item1;
  mock_wk_list_.backList = [NSArray arrayWithObjects:wk_item0, nil];
  mock_wk_list_.forwardList = [NSArray arrayWithObjects:wk_item2, nil];
  navigation_manager()->AddPendingItem(
      GURL("http://www.url.com/1"), Referrer(), ui::PAGE_TRANSITION_LINK,
      web::NavigationInitiationType::USER_INITIATED,
      web::NavigationManager::UserAgentOverrideOption::INHERIT);
  ...
  ASSERT_EQ(2, navigation_manager()->GetLastCommittedItemIndex());
  //} else {
  //  // Edge case: WKBasedNavigationManagerImpl::GetLastCommittedItemIndex()
  //  // returns the index of WKBackForwardList.currentItem because in most
  //  cases
  //  // currentItem is not updated until the navigation commits. However, under
  //  // back-forward navigation, currentItem is updated right away, so
