...
- (void)start {
  ...
  [self.browser->broadcaster() broadcastValue:@"selectedNTPPanel"
                                     ofObject:self.viewController
                                     selector:@selector(broadcastTest:)];
  [super start];
}
...
- (void)stop {
  ...
  [self.browser->broadcaster()
      stopBroadcastingForSelector:@selector(broadcastTest:)];
...
- (void)childCoordinatorDidStart:(BrowserCoordinator*)coordinator {
  if ([coordinator isKindOfClass:[NTPHomeCoordinator class]]) {
    ...
    self.viewController.test = NO;
    self.viewController.test = YES;
