...
- (void)start {
  ...
  self.googleLandingMediator = [[GoogleLandingMediator alloc]
      initWithConsumer:self.headerCoordinator
  ...
  self.suggestionsMediator.headerProvider = self.headerCoordinator;
  ...
  self.headerCollectionInteractionHandler =
      [[ContentSuggestionsHeaderSynchronizer alloc]
          initWithCollectionController:self.viewController
                      headerController:self.headerCoordinator];
...
- (CGFloat)pinnedOffsetY {
  CGFloat headerHeight = content_suggestions::heightForLogoHeader(
      self.headerCoordinator.logoIsShowing,
...
- (BOOL)isOmniboxFocused {
  return [self.headerCoordinator isOmniboxFocused];
}
...
- (void)promoShown {
  ...
  [self.headerCoordinator setPromoCanShow:notificationPromo->CanShow()];
}
