...
- (void)updateUIForTabHistoryPresentationFrom:(ToolbarButtonType)button {
  ...
  // Change the button appeareance to highlighted as long as the Popup History
  // menu is being displayed.
  [historyButton setHighlightedAppearance:YES];
}

- (void)updateUIForTabHistoryWasDismissed {
  // Change the buttons appeareance back to normal.
  [self.backButton setHighlightedAppearance:NO];
  [self.forwardButton setHighlightedAppearance:NO];
}
