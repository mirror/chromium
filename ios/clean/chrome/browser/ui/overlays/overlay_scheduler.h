...
class OverlayScheduler : public BrowserUserData<OverlayScheduler>,
 ...
 public:
  ...
  // Creates an OverlayScheduler and attaches it to |browser|.  The scheduler
  // will use |dispatcher| to switch foreground tab before starting an overlay.
  static void CreateForBrowser(Browser* browser,
                               id<TabGridCommands> dispatcher);
 ...
 private:
  ...
  // The dispatcher to use for TabGridCommands.
  id<TabGridCommands> dispatcher_;
  ...
  explicit OverlayScheduler(id<TabGridCommands> dispatcher);
