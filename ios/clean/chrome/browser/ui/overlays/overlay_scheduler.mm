...
OverlayScheduler::OverlayScheduler(id<TabGridCommands> dispatcher)
    : queue_manager_(nullptr), dispatcher_(dispatcher) {
  DCHECK(dispatcher_);
}
...
/* static */
void OverlayScheduler::CreateForBrowser(Browser* browser,
                                        id<TabGridCommands> dispatcher) {
  DCHECK(browser);
  if (!FromBrowser(browser)) {
    browser->SetUserData(UserDataKey(),
                         base::WrapUnique(new OverlayScheduler(dispatcher)));
  }
}
