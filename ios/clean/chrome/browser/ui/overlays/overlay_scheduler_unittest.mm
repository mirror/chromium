...
// Test object that handles TabGridCommands.
@interface TestTabGridDispatcher : NSObject<TabGridCommands>
// The index passed to the last dispatched call to |-showTabGridTabAtIndex:|.
@property(nonatomic, readonly) int activeWebStateIndex;
@end

@implementation TestTabGridDispatcher
@synthesize activeWebStateIndex = _activeWebStateIndex;
- (void)showTabGrid {
}
...
class OverlaySchedulerTest : public PlatformTest {
 ...
 private:
  ...
  Browser browser_;
  __strong TestTabGridDispatcher* dispatcher_;
...
TEST_F(OverlaySchedulerTest, SwitchWebStateForOverlay) {
  ...
  EXPECT_EQ(parent.presentedOverlay, overlay);
  EXPECT_TRUE(scheduler()->IsShowingOverlay());
  EXPECT_EQ(dispatcher().activeWebStateIndex, 0);
  [overlay stop];
  EXPECT_EQ(parent.presentedOverlay, nil);
  EXPECT_FALSE(scheduler()->IsShowingOverlay());
}
...
TEST_F(OverlaySchedulerTest, SwitchWebStateForQueuedOverlays) {
  ...
  EXPECT_TRUE(scheduler()->IsShowingOverlay());
  EXPECT_EQ(dispatcher().activeWebStateIndex, 1);
