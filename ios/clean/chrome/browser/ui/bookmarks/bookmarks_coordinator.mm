...
#import "ios/clean/chrome/browser/boxed_url_loader.h"
...
// Url Loader to use with boxed View Controller.
@property(nonatomic, strong) BoxedURLLoader* loader;
@end
...
- (void)start {
  ...
  self.loader = [[BoxedURLLoader alloc] init];
  if (!IsIPadIdiom()) {
...
- (void)dismissBookmarkBrowserAnimated:(BOOL)animated {
  ...
  [static_cast<id<BookmarksCommands>>(self.browser->dispatcher())
      closeBookmarks];
}
