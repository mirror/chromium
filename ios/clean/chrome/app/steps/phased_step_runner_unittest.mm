...
#import "ios/chrome/app/steps/application_step.h"
#import "ios/chrome/app/steps/phased_step_runner.h"
...
TEST(PhasedStepRunnerTest, TestManyDependencies) {
  ...
  EXPECT_EQ(runner.phase, testPhase);
  for (TestStep* task in tasks) {
    EXPECT_TRUE(task.hasRun) << "Expected task "
                             << task.providedFeature.UTF8String
                             << " to have run";
  }
}
