...
// Some arbitrary integers to test WebrtcVideoEncoderSelector::SetDesktopFrame()
// function.
static constexpr int kFrameWidth = 1001;
static constexpr int kFrameHeight = 999;
...
class WebrtcVideoEncoderSelectorTest : public ::testing::Test {
 public:
  WebrtcVideoEncoderSelectorTest() {
    ...
    memset(is_supported_, 0, sizeof(is_supported_));
  }
 ...
 protected:
  ...
  bool is_supported_[kEncoderCount];

 private:
  bool IsProfileSupported(int index,
                          const WebrtcVideoEncoderSelector::Profile& profile) {
    EXPECT_EQ(profile.resolution.width(), kFrameWidth);
    EXPECT_EQ(profile.resolution.height(), kFrameHeight);
...
TEST_F(WebrtcVideoEncoderSelectorTest, ReturnsNullIfCodecIsNotSupported) {
  ASSERT_FALSE(!!selector_.CreateEncoder());
