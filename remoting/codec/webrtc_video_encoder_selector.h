...
class WebrtcVideoEncoderSelector final {
 public:
  ...
  // Creates an WebrtcVideoEncoder according to the |profile_|.
  std::unique_ptr<WebrtcVideoEncoder> CreateEncoder();

  // Extracts information from |frame| and set |profile_|. If the information
  // extracted from |frame| is not consistent with the onces in |profile_|, this
  // function also resets |last_codec_|, so next CreateEncoder() function call
  // returns the first codec which supports the |frame|.
  ...
  // return |nullptr| even |is_supported| returns true.
