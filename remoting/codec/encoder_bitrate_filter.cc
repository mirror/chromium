...
// We use a DescendingSamples to statistic the bandwidth to avoid a sudden
// change significantly impacts the image quality. By using the weight factor as
// 0.95, the weight of a bandwidth one second ago (30 frames before) drops to
// ~21.5%.
...
void EncoderBitrateFilter::SetFrameSize(int width, int height) {
  minimum_bitrate_kbps_ =
      std::max(minimum_bitrate_kbps_per_megapixel_ * width * height / 1000000,
