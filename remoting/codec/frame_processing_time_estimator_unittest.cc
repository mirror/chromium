...
TEST(FrameProcessingTimeEstimatorTest, EstimateDeltaAndKeyFrame) {
  ...
  EXPECT_EQ(base::TimeDelta::FromMilliseconds(18),
...
TEST(FrameProcessingTimeEstimatorTest, NegativeBandwidthShouldBeDropped) {
  ...
  estimator.SetBandwidthKbps(-100);
  ASSERT_EQ(base::TimeDelta::FromMilliseconds(18),
            estimator.EstimatedProcessingTime(true));
}

// TODO(zijiehe): Remove this test before submitting.
TEST(FrameProcessingTimeEstimatorTest, DISABLED_TimeTicksNow_Performance) {
