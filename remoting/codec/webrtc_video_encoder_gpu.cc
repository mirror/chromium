...
// TODO(rkjnsn): Implement either a software fallback or some sort of delay if
...
void WebrtcVideoEncoderGpu::Encode(std::unique_ptr<webrtc::DesktopFrame> frame,
  ...
  DCHECK(params.duration > base::TimeDelta::FromMilliseconds(0));
...
void WebrtcVideoEncoderGpu::BitstreamBufferReady(int32_t bitstream_buffer_id,
  ...
  uint8_t* output_buffer = output_buffers_[bitstream_buffer_id];
  ...
  LOG_ASSERT(callback_it != callbacks_.end())
...
void WebrtcVideoEncoderGpu::FreeOutputBuffers() {
  for (uint8_t* buf : output_buffers_) {
    free(buf);
  }
