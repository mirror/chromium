...
void WebrtcVideoEncoderGpu::Encode(std::unique_ptr<webrtc::DesktopFrame> frame,
  ...
  // TODO(zijiehe): WebRTC never requests keyframe for H264 codec. So we send a
  // key frame once per 500 milliseconds. This logic should be removed once the
  // root cause in client side has been addressed.
  const bool request_key_frame =
      params.key_frame ||
      (new_timestamp - previous_keyframe_timestamp_) >=
       base::TimeDelta::FromMilliseconds(500);
  if (request_key_frame) {
    previous_keyframe_timestamp_ = new_timestamp;
  }
  video_encode_accelerator_->Encode(video_frame, request_key_frame);
}
