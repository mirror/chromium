...
void UpdateCodecParameters(SdpMessage* sdp_message, bool incoming) {
  ...
  if (sdp_message->has_video()) {
    ...
    param_added |= sdp_message->AddCodecParameter(
        "H264", "x-google-min-bitrate=1000; x-google-max-bitrate=100000");
    if (!param_added) {
      if (incoming) {
        LOG(WARNING) << "Neither of VP8/VP9/H264 is found in an incoming SDP.";
      } else {
        LOG(FATAL)
            << "Neither of VP8/VP9/H264 is found in SDP generated by WebRTC.";
      }
    }
  }
