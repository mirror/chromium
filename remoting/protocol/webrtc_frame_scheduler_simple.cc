...
void WebrtcFrameSchedulerSimple::OnTargetBitrateChanged(int bandwidth_kbps) {
  ...
  processing_time_estimator_.SetBandwidthKbps(bandwidth_kbps_.EstimatedKbps());
  pacing_bucket_.UpdateRate(bandwidth_kbps_.EstimatedKbps() * 1000 / 8, now);
  ScheduleNextFrame();
}
...
bool WebrtcFrameSchedulerSimple::OnFrameCaptured(
  ...
  params_out->bitrate_kbps = bandwidth_kbps_.EstimatedKbps();
