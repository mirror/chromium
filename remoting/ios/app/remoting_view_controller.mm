...
- (void)didSelectCell:(HostCollectionViewCell*)cell
  ...
  switch (GetConnectionType()) {
    ...
    case ConnectionType::WWAN: {
      ...
      // MDCAlertController somehow shows the button as white text (which is the
      // default color) on white background and there is no API the change that.
      // We need to use the appearance proxy to change the default text color.
      UIColor* currentButtonTitleColor =
          [UIButton.appearance titleColorForState:UIControlStateNormal];
      [UIButton.appearance setTitleColor:RemotingTheme.flatButtonTextColor
                                forState:UIControlStateNormal];
      [self presentViewController:alert
                         animated:YES
                       completion:^{
                         [UIButton.appearance
                             setTitleColor:currentButtonTitleColor
                                  forState:UIControlStateNormal];
                       }];
      break;
    }
