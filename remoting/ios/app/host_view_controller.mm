...
- (id)initWithClient:(RemotingClient*)client {
  ...
  if (self) {
    ...
    _surfaceCreated = NO;
    _settings = [[RemotingPreferences instance] settingsForHost:client.hostInfo.hostId];
  }
  return self;
}
...
- (void)viewWillDisappear:(BOOL)animated {
  ...
  [[RemotingPreferences instance] setSettings:_settings forHost:_client.hostInfo.hostId];
...
- (void)useDirectInputMode {
  _settings.inputMode = DirectInputMode;
  [self applyInputMode];
}

- (void)useTrackpadInputMode {
  _settings.inputMode = TrackpadInputMode;
  [self applyInputMode];
}
