...
- (void)animateHostSurfaceSize:(CADisplayLink*)link {
  ...
  // rate will drop for some reason. Note the the actual rendering is done on
  // the display thread asynchronously, so unfortunately the animation will not
  // be perfectly synchronized with the keyboard animation.
