...
static NSString* const kUserEmailKey =
    @"kUserEmailKey";
static NSString* const kHostSettingsKey =
    @"kHostSettingsKey";
...
- (HostSettings*)settingsForHost:(NSString*)hostId {
  NSString* key = [NSString stringWithFormat:@"%@-%@", kHostSettingsKey, hostId];
  NSData *encodedSettings = [_defaults objectForKey:key];
  HostSettings* settings = [NSKeyedUnarchiver unarchiveObjectWithData:encodedSettings];
  if (settings == nil) {
...
- (void)setSettings:(HostSettings*)settings forHost:(NSString*)hostId {
  NSString* key = [NSString stringWithFormat:@"%@-%@", kHostSettingsKey, hostId];
  if (settings) {
    NSData *encodedSettings = [NSKeyedArchiver archivedDataWithRootObject:settings];
...
- (void)setActiveUserEmail:(NSString*)email {
  if (email) {
    [_defaults setObject:email forKey:kUserEmailKey];
  } else {
    [_defaults removeObjectForKey:kUserEmailKey];
  }
  [_defaults synchronize];
}

- (NSString*)activeUserEmail {
  return [_defaults objectForKey:kUserEmailKey];
}
