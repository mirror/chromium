...
- (void)storeUserInfo:(UserInfo*)user {
  if (user) {
    [RemotingPreferences instance].activeUserEmail = user.userEmail;
    // TODO(nicholss): Need to match the token with the email.
    [_keychainWrapper setRefreshToken:user.refreshToken];
  } else {
    [RemotingPreferences instance].activeUserEmail = nil;
...
- (UserInfo*)loadUserInfo {
  UserInfo* user = [[UserInfo alloc] init];
  user.userEmail = [RemotingPreferences instance].activeUserEmail;
  // TODO(nicholss): Need to match the token with the email.
