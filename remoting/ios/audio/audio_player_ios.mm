...
std::unique_ptr<AudioPlayerIos> AudioPlayerIos::CreateAudioPlayer(
  ...
  std::unique_ptr<AudioStreamConsumer> consumer =
      (std::unique_ptr<AudioStreamConsumer>)base::WrapUnique(buffer);

  std::unique_ptr<AudioStreamConsumer> consumer_proxy =
      AudioStreamConsumerProxy::Create(
...
void AudioPlayerIos::StartOnAudioThread() {
  switch (state_) {
    case UNALLOCATED:
      ...
      GenerateOutputBuffers(audio_frame_supplier_->GetBytesPerFrame());
      state_ = STOPPED;
...
void AudioPlayerIos::AsyncGetAudioFrameCallback(const void* context,
  ...
  output_buffer->mAudioDataByteSize = audio_frame_supplier_->GetBytesPerFrame();
  Pump(output_buffer);
}
...
void AudioPlayerIos::Prime(AudioQueueBufferRef output_buffer,
  ...
  audio_frame_supplier_->AsyncGetAudioFrame(
      ...
      audio_frame_supplier_->GetBytesPerFrame(),
