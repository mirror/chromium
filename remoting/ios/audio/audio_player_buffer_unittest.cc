...
#include "remoting/client/audio_player_buffer.h"
#include "testing/gtest/include/gtest/gtest.h"

namespace {

const int kAudioSamplesPerFrame = 1200;  // = 48000 Hz * .025 s
const int kAudioSampleBytes = 4;         // = 2 channels * 2 bytes
...
void ConsumeAsyncAudioFrameCallback(const void* context, const void* samples) {
...
class AudioPlayerBufferTest : public ::testing::Test {
 ...
 protected:
  ...
  void ConsumeAudioFrame() {
    ...
    audio_->GetAudioFrame(reinterpret_cast<void*>(buffer), kAudioFrameBytes);
  ...
  void ConsumeAsyncAudioFrame() {
    ...
    audio_->AsyncGetAudioFrame(reinterpret_cast<void*>(buffer),
                               kAudioFrameBytes,
...
TEST_F(AudioPlayerBufferTest, Init) {
  ...
  audio_->AddAudioPacket(CreatePacket48000Hz(10));
...
TEST_F(AudioPlayerBufferTest, ConsumePartialPacket) {
  ...
  // Process 100 samples.
  int packet1_samples = kAudioSamplesPerFrame * 4;
  total_samples += packet1_samples;
  audio_->AddAudioPacket(CreatePacket48000Hz(packet1_samples));
  ...
  // Remaining samples.
  ASSERT_EQ(kAudioSamplesPerFrame * 3, total_samples);
  ASSERT_EQ(kAudioSamplesPerFrame * kAudioSampleBytes, bytes_consumed);
}

TEST_F(AudioPlayerBufferTest, SingleAsyncRequest) {
  // Create a pending Audio Request.
  ConsumeAsyncAudioFrame();
  ASSERT_EQ(1, GetNumQueuedRequests());
}
...
TEST_F(AudioPlayerBufferTest, TwoAsyncRequest) {
  ...
  ConsumeAsyncAudioFrame();
  ConsumeAsyncAudioFrame();
  ASSERT_EQ(2, GetNumQueuedRequests());

  // Add just enough samples to fulfill one request.
  audio_->AddAudioPacket(CreatePacket48000Hz(kAudioSamplesPerFrame));
...
TEST_F(AudioPlayerBufferTest, TwoPartFrameAsyncRequest) {
  // Create a pending Audio Request.
  audio_->AddAudioPacket(CreatePacket48000Hz(kAudioSamplesPerFrame / 2));
  ...
  ASSERT_EQ(0, GetNumQueuedRequests());
}
