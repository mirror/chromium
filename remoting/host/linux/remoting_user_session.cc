...
std::string FindScriptPath() {
  base::FilePath path;
  bool result = base::ReadSymbolicLink(base::FilePath(kExeSymlink), &path);
...
void Relaunch(const std::string& user) {
  ...
  // No need for path manipulation here, so read the symlink directly to reduce
  // the amount of library code we run as root.
  char exe_path[PATH_MAX];
  ssize_t path_size = readlink(kExeSymlink, exe_path, arraysize(exe_path));
  ...
  execl(exe_path, exe_path, kStartCommand, kForegroundFlag, (char*)nullptr);
