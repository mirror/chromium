...
// Usage: user-session start [--foreground] [--user user] [--config config-file]
//
// Options:
...
//   --config      - Launch the host using the specified config file
...
// This exit code is returned when user-session is started without suitable
// privileges. It must be kept in sync with NOT_ROOT_EXIT_CODE in
// linux_me2me_host.py
const int kNotRootExitCode = 31;
...
const char* kPassthroughVariables[] = {
...
void ExecMe2MeScript(base::EnvironmentMap environment,
  ...
  for (const std::string& arg : script_args) {
    ...
    CHECK(escaped_arg) << "Could not escape config path";
    shell_arg += " ";
...
void Relaunch(const std::string& user, std::vector<std::string> script_arg) {
  CHECK(getuid() != 0);
  ...
  std::vector<const char*> arg_ptrs = {gExecutablePath, kStartCommand,
                                       kForegroundFlag, "--"};
  for (const std::string& arg : script_arg) {
...
// config_file is provided, it will be passed to the script using the --config
// flag.
