...
void WaitForMessagesAndExit(int read_fd, const std::string& log_name) {
  ...
  static const char kMessagePrefix[] = {'M', 'S', 'G', ':'};
  static const char kReady[] = {'R', 'E', 'A', 'D', 'Y', '\n'};
  ...
  // never responds (e.g. host crash-looping, daemon killed). This signal will
  // cause the read loop below to stop with an EINTR IOError.
  //
  ...
  while ((line_size = getline(&buffer, &buffer_size, stream)) >= 0) {
    ...
    if (static_cast<std::size_t>(line_size) > arraysize(kMessagePrefix) &&
        std::strncmp(buffer, kMessagePrefix, arraysize(kMessagePrefix)) == 0) {
      fwrite(buffer + arraysize(kMessagePrefix), sizeof(char),
             line_size - arraysize(kMessagePrefix), stderr);
    } else if (line_size == arraysize(kReady) &&
               std::strncmp(buffer, kReady, arraysize(kReady)) == 0) {
      host_ready = true;
    } else {
      fputs("Unrecognized command: ", stderr);
      fwrite(buffer, sizeof(char), line_size, stderr);
    }
  }

  // csh and tcsh close extra file descriptors, so we won't be able to retrieve
  // any messages from session startup if the user uses one of those as their
  // login shell. All other shells leave the pipe fd open for the script.
  // We can look into using a named pipe instead if this becomes an issue.
  if (!std::feof(stream) || !message_received) {
    fputs(
        "Could not determine host startup status. Check log for success/error "
        "messages.\n",
        stderr);
...
// user, the parent process will drop priviledges and wait for the host to
...
void Daemonize() {
  ...
  if (real_uid != 0) {
    ...
    if (pipe_result != 0 || dup2(pipe_fd[1], kMessageFd) != kMessageFd) {
      PLOG(WARNING) << "Failed to create pipe";
    } else {
      pipe = true;
      read_fd = pipe_fd[0];
      close(pipe_fd[1]);
  ...
  // Close all file descriptors except stdio and write_fd, including any we may
  // have inherited.
  if (pipe) {
