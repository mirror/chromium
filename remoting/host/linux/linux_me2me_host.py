...
# This exit code is returned by user-session when it is started without suitable
# privileges. It must be kept in sync with kNotRootExitCode in
# remoting_user_session.cc.
NOT_ROOT_EXIT_CODE = 31;
  ...
  for process in psutil.process_iter():
    ...
    try:
      ...
      if (cmdline[0] == sys.executable and cmdline[1] == sys.argv[0]
          and "--start" in cmdline and "--child-process" in cmdline):
        return process
...
def start_via_user_session(foreground):
  ...
  try:
    ...
    if e.errno == errno.EACCES:
      ...
      # back in. In the mean time, we can try to use sg to switch to the CRD
      # group and execute the wrapper. sg is an odd character, though, and will
      # try to prompt for a password if it can't verify the user is a member of
      # the given group, along with in a few other corner cases. (It will
      # prompt in the non-member case even if the group doesn't have a password
      # set.)
      #
      # To prevent sg from prompting the user for a password that doesn't
      # exist, we redirect stdin and detach sg from the TTY. It will still
      # print something like "Password: crypt: Invalid argument", so redirect
      # stdout and stderr, as well. We then have the shell unredirect them when
      # executing user-session.
      #
      # Finally, it is desirable to have some way to tell whether any errors are
      # from sg or user-session, so use a pipe.
      with open(os.devnull, "r+") as devnull:
        # Pipe to check whether sg successfully ran our command.
        read_fd, write_fd = os.pipe()
        # Dup stdout and stderr so they can be restored for user-session.
        dup_out = os.dup(1)
        dup_err = os.dup(2)
        try:
          # Executes a command that looks something like
          # sg {group-name} 'echo success >&4; exec {command} 1>&5 2>&6'
          # os.setsid will detach subprocess from the TTY
          process = subprocess.Popen(["sg", CHROME_REMOTING_GROUP_NAME,
                                      "echo success >&{fd}; exec "
                                          .format(fd=write_fd)
                                        + " ".join(map(pipes.quote, command))
                                        + " 1>&{out} 2>&{err}"
                                          .format(out=dup_out, err=dup_err)],
          ...
          # Because sg is in it's own session, it won't have gotten the
          # interrupt.
          try:
            os.killpg(os.getpgid(process.pid), signal.SIGINT)
            result = process.wait()
          except OSError:
            logging.warning("Host may still be running")
            result = 1
        finally:
          os.close(dup_out)
          os.close(dup_err)
      ...
      if contents != "success\n":
        # Lack of success message means sg didn't execute our command.
        # (Maybe the user's not a member of the CRD group?)
        logging.error("Failed to access {} group. Is the user a member?"
                      .format(CHROME_REMOTING_GROUP_NAME))
        result = 126
      elif result == 126:
        # A 126 exit code means permission was still denied. (Maybe the admin
        # has it set to be root only?)
        logging.error("Could not execute {}: Permission denied"
                      .format(USER_SESSION_PATH))
    else:
  ...
  if result == NOT_ROOT_EXIT_CODE:
    # Script is apparently not setuid
    logging.error("{} not installed setuid root. Host must be started by "
                  "administrator.".format(USER_SESSION_PATH))
...
Web Store: https://chrome.google.com/remotedesktop"""
  ...
  # Log to messaging pipe if it exists.
  try:
