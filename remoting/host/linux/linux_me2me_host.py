      ...
      with open(os.devnull, "r+") as devnull:
        ...
        try:
          process = subprocess.Popen(
              ["sg", CHROME_REMOTING_GROUP_NAME,
               "echo success >&{pipe}; exec {command} "
                 "1>&{out} 2>&{err} {pipe}>&- {out}>&- {err}>&-"
                 .format(pipe=write_fd, out=dup_out, err=dup_err,
                         command=" ".join(map(pipes.quote, command)))
              stdin=devnull, stdout=devnull, stderr=devnull,
              preexec_fn=lambda: (os.close(read_fd), os.setsid()))
          result = process.wait()
        except OSError as e:
