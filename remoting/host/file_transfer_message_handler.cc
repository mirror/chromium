...
void FileTransferMessageHandler::SendToFileProxy(
  ...
  if (file_proxy_wrapper_->state() == FileProxyWrapper::kFileCreated) {
...
void FileTransferMessageHandler::ParseNewRequest(
  ...
  if (!request_->ParseFromString(message)) {
    LOG(ERROR) << "Failed to parse request protobuf";
    file_proxy_wrapper_->Cancel();
    protocol::FileTransferResponse response;
    response.set_error(
  ...
  if (!PathService::Get(base::DIR_USER_DESKTOP, &target_directory)) {
    LOG(ERROR) << "Failed to get DIR_USER_DESKTOP from PathService::Get";
    file_proxy_wrapper_->Cancel();
    protocol::FileTransferResponse response;
    response.set_error(
        protocol::FileTransferResponse_ErrorCode_UNEXPECTED_ERROR);
