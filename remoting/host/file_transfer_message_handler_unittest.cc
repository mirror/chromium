...
class FakeFileProxyWrapper : public FileProxyWrapper {
 public:
  ...
  void Init(DoneCallback done_callback) override;
...
void FakeFileProxyWrapper::Init(DoneCallback done_callback) {
...
void FakeFileProxyWrapper::RunDoneCallback(
    std::unique_ptr<protocol::FileTransferResponse_ErrorCode> error) {
  std::move(done_callback_).Run(std::move(error));
}
...
TEST_F(FileTransferMessageHandlerTest, WriteTwoChunks) {
  ...
  file_proxy_wrapper_ptr->RunDoneCallback(
      std::unique_ptr<protocol::FileTransferResponse_ErrorCode>(nullptr));
  ...
  std::queue<std::string> actual_sent_messages = fake_pipe_->sent_messages();
...
TEST_F(FileTransferMessageHandlerTest, FileProxyError) {
  ...
  file_proxy_wrapper_ptr->RunDoneCallback(base::WrapUnique(
      new protocol::FileTransferResponse_ErrorCode(fake_error)));
