...
class PolicyWatcher : public policy::PolicyService::Observer {
 public:
  ...
  static std::unique_ptr<PolicyWatcher> CreateFromPolicyService(
      policy::PolicyService* policy_service);

  // Construct and use a new PolicyService (on other OS-es). PolicyWatcher must
  // be used on the thread on which it is created.
  //
  ...
  // of TYPE_IO type.
  static std::unique_ptr<PolicyWatcher> CreateFromTaskRunner(
