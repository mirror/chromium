...
base::FilePath BuildHostBinaryPathImpl() {
#if defined(OS_LINUX)
  // On Linux, the binary name has been changed to "chrome-remote-desktop-host".
  base::FilePath path;
  ...
  // On the test environment of MacOSX, the output of remoting_me2me_host is
  // placed in remoting_me2me_host.app folder.
  base::FilePath path;
  bool result = base::PathService::Get(base::FILE_EXE, &path);
  DCHECK(result);
  if (path.BaseName().value() == FILE_PATH_LITERAL("remoting_me2me_host")) {
...
#elif defined(OS_WIN)
  // On Windows, three binaries are sharing the HostMain(), which can be used as
...
#else
  #error "BuildHostBinaryPathImpl is not implemented for current platform."
#endif
}
...
// be the same.
const base::FilePath& BuildHostBinaryPath() {
  static const base::FilePath result = BuildHostBinaryPathImpl();
  return result;
}
