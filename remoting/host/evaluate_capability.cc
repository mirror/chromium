...
int EvaluateCapability(const std::string& type,
  ...
  // base::GetAppOutputWithExitCode() should be ignored. Instead, 9009 is
  // returned from the OS if the command cannot be executed, i.e. the binary
  // cannot be found, or 5 if the access is denied, which is the right thing we
  // need.
  result = (exit_code != 9009 && exit_code != 5);
#endif
  if (!result) {
