...
// Incoming packets: 4 framws worth
// Consume: 1 frame
TEST_F(AudioPlayerBufferTest, ConsumePartialPacket) {
  ...
  // Process 4x samples.
  uint32_t packet1_samples = audio_->bytes_per_frame() * 4;
  ...
  ConsumeAudioFrame();
  total_samples -= audio_->bytes_per_frame();
  ...
  // Remaining samples.
  ASSERT_EQ(audio_->samples_per_frame() * 3, total_samples);
