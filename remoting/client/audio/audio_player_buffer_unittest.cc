...
std::unique_ptr<AudioPacket> CreatePacket48kHz(int samples) {
  return CreateAudioPacketWithSamplingRate(AudioPacket::SAMPLING_RATE_48000,
                                           samples);
}
...
class AudioPlayerBufferTest : public ::testing::Test {
 ...
 protected:
  ...
  void ConsumeAudioFrame() {
    ...
    audio_->GetAudioFrame(kAudioFrameBytes, reinterpret_cast<void*>(buffer));
  ...
  void ConsumeAsyncAudioFrame() {
    ...
    audio_->AsyncGetAudioFrame(kAudioFrameBytes,
...
TEST_F(AudioPlayerBufferTest, ConsumePartialPacket) {
  ...
  audio_->AddAudioPacket(CreatePacket48kHz(packet1_samples));
...
TEST_F(AudioPlayerBufferTest, TwoAsyncRequest) {
  ...
  audio_->AddAudioPacket(CreatePacket48kHz(kAudioSamplesPerFrame));
...
TEST_F(AudioPlayerBufferTest, TwoPartFrameAsyncRequest) {
  // Create a pending Audio Request.
  audio_->AddAudioPacket(CreatePacket48kHz(kAudioSamplesPerFrame / 2));
