...
class AudioDecodeScheduler::Core {
 public:
...
void AudioDecodeScheduler::Core::ProcessAudioPacket(
    ...
    const base::Closure& done) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  audio_decode_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&AudioDecodeScheduler::Core::DecodePacket,
                 base::Unretained(this), base::Passed(&packet), done));
}
...
void AudioDecodeScheduler::Core::DecodePacket(
  ...
  main_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&AudioDecodeScheduler::Core::ProcessDecodedPacket,
                 base::Unretained(this), base::Passed(&decoded_packet), done));
}
...
void AudioDecodeScheduler::Core::ProcessDecodedPacket(
  ...
  if (packet.get() && audio_buffer_.get()) {
    audio_buffer_->Push(std::move(packet));
  }
  done.Run();
}
