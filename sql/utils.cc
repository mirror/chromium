...
namespace {
bool SmartOpenDatabaseInternal(sql::Connection* db,
  ...
  auto error_callback_func =
      base::Bind([](int* out_error, int error, sql::Statement*) {
...
SQL_EXPORT bool SmartOpenDatabase(sql::Connection* db,
                                  const base::FilePath& db_path) {
  return SmartOpenDatabaseInternal(db, db_path, false);
}

SQL_EXPORT bool SmartOpenDatabaseWithMetaVersion(
    sql::Connection* db,
    const base::FilePath& db_path) {
  return SmartOpenDatabaseInternal(db, db_path, true);
}
