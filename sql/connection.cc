...
void Connection::Preload() {
  ...
  if (!db_) {
    DLOG_IF(DCHECK, !poisoned_) << "Cannot preload null db";
    return;
  }
...
void Connection::ReleaseCacheMemoryIfNeeded(bool implicit_change_performed) {
  ...
  // recovery.
  if (!db_) {
    DLOG_IF(DCHECK, !poisoned_) << "Illegal use of connection without a db";
    return;
  }
...
bool Connection::Raze() {
  ...
  if (!db_) {
    DLOG_IF(DCHECK, !poisoned_) << "Cannot raze null db";
    return false;
  }
...
void Connection::Poison() {
  if (!db_) {
    DLOG_IF(DCHECK, !poisoned_) << "Cannot poison null db";
    return;
  }
...
void Connection::RollbackTransaction() {
  if (!transaction_nesting_) {
    DLOG_IF(DCHECK, !poisoned_) << "Rolling back a nonexistent transaction";
    return;
  }
...
bool Connection::CommitTransaction() {
  if (!transaction_nesting_) {
    DLOG_IF(DCHECK, !poisoned_) << "Committing a nonexistent transaction";
    return false;
  }
  transaction_nesting_--;
...
scoped_refptr<Connection::StatementRef> Connection::GetStatementImpl(
  ...
  if (rc != SQLITE_OK) {
    ...
    DCHECK_EQ(rc, SQLITE_ERROR) << "SQL compile error " << GetErrorMessage();
...
bool Connection::OpenInternal(const std::string& file_name,
  ...
  DLOG_IF(DCHECK, poisoned_) << "sql::Connection is already open.";
  poisoned_ = false;
