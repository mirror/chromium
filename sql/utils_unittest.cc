...
TEST_F(SqlUtilsTest, SmartOpenDatabaseOnCorrrectFile) {
  base::HistogramTester histogram_tester;

  EXPECT_TRUE(utils::SmartOpenDatabase(&db(), file_));
  size_t count;
...
TEST_F(SqlUtilsTest, SmartOpenDatabaseOnCorruptSizeFile) {
  base::HistogramTester histogram_tester;
...
TEST_F(SqlUtilsTest, SmartOpenDatabaseWithMetaOnCorruptSizeFile) {
  base::HistogramTester histogram_tester;
  ...
  ASSERT_TRUE(test::CorruptSizeInHeader(file_));
  EXPECT_TRUE(utils::SmartOpenDatabaseWithMetaVersion(&db(), file_));
  size_t count;
  EXPECT_TRUE(test::CountTableRows(&db(), "x", &count));
  EXPECT_EQ(2U, count);
...
TEST_F(SqlUtilsTest, SmartOpenDatabaseOnTruncatedFile) {
  base::HistogramTester histogram_tester;
...
TEST_F(SqlUtilsTest, SmartOpenDatabaseOnCorruptFileToMinimumSize) {
  base::HistogramTester histogram_tester;

  // Recovery method in SmartOpenDatabaseWithMetaVersion may call LOG(FATAL) in
  // some cases of corrupted database, therefore to diasble default handler.
  logging::ScopedLogAssertHandler s(
      base::Bind([](const char* file, int line, const base::StringPiece message,
                    const base::StringPiece stack_trace){}));
...
TEST_F(SqlUtilsTest, SmartOpenDatabaseOnFileNotADB) {
  base::HistogramTester histogram_tester;
...
TEST_F(SqlUtilsTest, SmartOpenDatabaseOnFileIsDirectory) {
  base::HistogramTester histogram_tester;
...
#if defined(OS_POSIX)
TEST_F(SqlUtilsTest, SmartOpenDatabaseRemoveFailed) {
  base::HistogramTester histogram_tester;
  ...
  EXPECT_FALSE(utils::SmartOpenDatabase(&db(), failed_dir));
