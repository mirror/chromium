...
void GaiaAuthFetcher::OnGetTokenResponseFetched(
  ...
  if (status.is_success() && response_code == net::HTTP_OK) {
    ...
    success = ExtractOAuth2TokenPairResponse(data, NULL, &access_token, NULL,
                                             &expires_in_secs);
  }

  if (success) {
    consumer_->OnGetTokenResponseSuccess(GaiaAuthConsumer::ClientOAuthResult(
        std::string(), access_token, std::string(), expires_in_secs));
  } else {
