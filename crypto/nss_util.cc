...
class NSSInitSingleton {
 public:
  ...
  static void InitializeTPMTokenInThreadPool(CK_SLOT_ID token_slot_id,
    ...
    // code needs to synchronously wait for a task to run but the current thread
    // pool doesn't have enough threads to schedule that task, a deadlock
    // occurs. To prevent that, the base::ScopedBlockingCall below increments
    // the current thread pool's capacity within its scope.
