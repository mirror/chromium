...
#include "media/mojo/services/mojo_cdm_file_io.h"    // nogncheck
...
class MojoCdmHelper : public media::CdmAuxiliaryHelper {
 public:
  ...
  std::unique_ptr<media::CdmFileIO> CreateCdmFileIO(
    ...
    if (!storage.is_bound())
      return nullptr;
    storage->Initialize(key_system_);
    return base::MakeUnique<media::MojoCdmFileIO>(std::move(storage), client);
  }
