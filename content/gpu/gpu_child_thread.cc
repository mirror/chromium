...
GpuChildThread::GpuChildThread(const ChildThreadImpl::Options& options,
    ...
    : ChildThreadImpl(options),
      gpu_main_(this, std::move(gpu_init)),
      weak_factory_(this) {
  if (in_process_gpu()) {
  ...
  //  gpu_service_->set_in_host_process(gpu_main_.gpu_info().in_process_gpu);
}
...
gpu::SyncPointManager* GpuChildThread::GetSyncPointManager() {
  return GetContentClient()->gpu()
             ? GetContentClient()->gpu()->GetSyncPointManager()
             : nullptr;
}

base::WaitableEvent* GpuChildThread::GetShutDownEvent() {
