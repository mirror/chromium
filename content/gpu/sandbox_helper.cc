...
#include "content/gpu/sandbox_helper.h"
...
class GpuSandboxHelper : public SandboxHelper {
 public:
  GPUSandboxHelper(const GPUInfo* gpu_info);
...
#if defined(OS_LINUX)
class ArmGpuSandboxHelper : public GpuSandboxHelper {
 public:
...
class AmdGpuSandboxHelper : public GpuSandboxHelper {
 public:
  ArmGpuSandboxHelper(const GPUInfo* gpu_info);
...
// static
std::unique_ptr<SandboxHelper> SandboxHelper::Create(bool is_service,
                                                     const GPUInfo* gpu_info) {
  if (is_service)
    return std::make_unique<NullSandboxHelper>();
#if defined(OS_CHROMEOS)
#if defined(__arm__)
...
#endif  // defined(OS_CHROMEOS)
  return std::make_unique<GpuSandboxHelper>(gpu_info());
}
