...
class PageStateSerializationTest : public testing::Test {
 public:
  ...
  void ReadBackwardsCompatPageState(const char* suffix,
                                    int version,
                                    ExplodedPageState* page_state) {
    base::FilePath path;
    PathService::Get(content::DIR_TEST_DATA, &path);
    path = path.AppendASCII("page_state")
               .AppendASCII(base::StringPrintf("serialized_%s.dat", suffix));
  ...
  void TestBackwardsCompat(int version) {
    const char* suffix = base::StringPrintf("v%d", version).c_str();
