...
TEST_F(PageStateSerializationTest, LegacyEncodePageStateFrozen) {
  ...
  std::string encoded;
  LegacyEncodePageStateForTesting(state, 25, &encoded);
  ...
  EXPECT_TRUE(base::Base64Decode(trimmed_contents, &decoded_contents));

  ExpectEquality(encoded, decoded_contents);
}
