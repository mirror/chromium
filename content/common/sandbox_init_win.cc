...
bool InitializeSandbox(sandbox::SandboxInterfaceInfo* sandbox_info) {
  ...
  sandbox::SandboxType sandbox_type = SandboxTypeFromCommandLine(command_line);
  ...
  if (broker_services) {
    ...
    if (!sandbox::IsUnsandboxedSandboxType(sandbox_type)) {
  ...
  return sandbox::IsUnsandboxedSandboxType(sandbox_type) ||
