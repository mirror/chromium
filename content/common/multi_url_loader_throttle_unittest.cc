...
class MockURLLoaderThrottle : public URLLoaderThrottle {
 public:
  ...
  MOCK_METHOD1(WillProcessResponse, void(bool*));
...
TEST(MultiURLLoaderThrottle, ThrottlePassthrough) {
  ...
  EXPECT_CALL(*throttle1, WillProcessResponse(_)).Times(1);
  EXPECT_CALL(*throttle2, WillProcessResponse(_)).Times(1);

  bool defer = false;
  multi_throttle.WillStartRequest(ResourceRequest(), &defer);
  multi_throttle.WillRedirectRequest(net::RedirectInfo(), &defer);
  multi_throttle.WillProcessResponse(&defer);
}
...
TEST(MultiURLLoaderThrottle, CancelStopsOtherApiCalls) {
  ...
  EXPECT_CALL(*throttle1, WillProcessResponse(_)).Times(0);
  EXPECT_CALL(*throttle2, WillProcessResponse(_)).Times(0);
