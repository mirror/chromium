...
// Note: the main frame's unique name is always the empty string. Otherwise,
//       assignedName is used if it is unique, falling back to generatedName if
//       not.
//
// assignedName ::= browsing context name
// Note: Only browsing context name changes prior *before* to the first real
//       commit can affect uniqueName. Afterwards, uniqueName is immutable.
//
// generatedName ::= oldGeneratedName newUniqueSuffix
// Note: newUniqueSuffix is only non-empty if oldGeneratedName is not unique.
//
// oldGeneratedName ::= "<!--framePath //" ancestorChain
//                      "/<!--frame" childCount "-->-->"
// Note: oldGeneratedName is generated by the internal helper
//       |GenerateCandidate()|.
//
...
//
// newUniqueSuffix ::= "" | "<!--framePosition-" framePositions "/"
//                     sequenceNumber "-->"
// Note: internally, the unique suffix is generated by |GenerateFramePosition()|
//       and |AppendUniqueSuffix()|.
//
// framePositions ::= index in parent of each frame from child to root. The list
//                    of indices is joined using "-" as the delimiter.
//
// sequenceNumber ::= A counter incremented from 0 to guarantee uniqueness of
//                    generatedName.
//
// Hashing fallback for names exceeding |kMaxSize|:
// When the max size is exceeded, hashedGeneratedName is used as the unique name
// instead. Internally, this is calculated by |CalculateHashedName()|.
//
// hashedName ::= hexEncode(sha256(oldGeneratedName))
// hashedFramePath ::= "<!--framePath //<!--frameHash" hashedName "-->-->"
// hashedGeneratedName :== hashedFramePath newTrimmedUniqueSuffix?
//
// newTrimmedUniqueSuffix ::= trim(newUniqueSuffix) | ""
// newTrimmedUniqueSuffix defaults to matching the original newUniqueSuffix
// generated for generatedName. trim() limits the unique suffix to a maximum
// of ten frame indices. If this is insufficient to guarauntee uniqueness, the
// algorithm will first try to drop the unique suffix altogether--and if the
// result is still not unique, it will generate a new trimmed newUniqueSuffix
// and use that.
//
...
class CONTENT_EXPORT UniqueNameHelper {
 public:
  ...
  // ExplodedFrameState.
  // TODO(dcheng): Rename GetSiblingCount() and CollectAncestorNames() to reduce
  ...
  // Internal state for |UpdateLegacyNameForMaxSize()|.
  struct NameUpdateState {
   private:
    friend UniqueNameHelper;
  ...
  // Updates serialized legacy unique names to obey the max size. Returns true
  // if the serialized unique name should be updated to |new_name|.
  //
  // |legacy_name|: the name read from ExplodedFrameState prior to v25.
  // |update_state|: the replacement to use. This should be the replacement
  //                 returned by the most recent invocation of this function
  //                 on an ancestor frame that returned true, or nullptr if
  //                 there is no such replacement.
  // |new_name|: out parameter for the new name to use. Only valid if the
  //             function returns true.
  // |descndant_update_state|: out parameter: if set, the replacement pattern to
  //                           use for updating names for descendants of this
  //                           frame.
  //
  // Note: |legacy_name| is intentionally passed by value to allow moves.
  static bool UpdateLegacyNameForMaxSize(
 ...
 private:
  // Helper for |UpdateLegacyNameForMaxSize()|. Applies the replacement in
  // |state| if possible. Returns true only if a replacement was applied.
  static bool ApplyReplacementIfPossible(base::StringPiece name,
