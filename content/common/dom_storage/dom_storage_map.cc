...
bool DOMStorageMap::SetItem(
  ...
  if (has_only_keys_) {
    ...
    return SetItemInternal<KeysMapInternal>(&keys_only_, key, value_size,
                                            &unused);
  } else {
...
size_t DOMStorageMap::CountBytes(const DOMStorageValuesMap& values) {
  return CountBytesInternal(values);
}

// static
template <typename MapType>
size_t DOMStorageMap::CountBytesInternal(const MapType& values) {
  if (values.empty())
    return 0;
