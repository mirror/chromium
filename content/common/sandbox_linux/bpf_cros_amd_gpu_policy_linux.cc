...
bool CrosAmdGpuProcessPolicy::PreSandboxHook() {
  ...
  if (NULL == dlopen("libglapi.so", dlopen_flag)) {
    PLOG(ERROR) << "dlopen failed(libglapi.so) with error " << dlerror();
    return false;
  }
  if (NULL == dlopen("/usr/lib64/dri/swrast_dri.so", dlopen_flag)) {
    PLOG(ERROR) << "dlopen(swrast_dri.so) failed with error " << dlerror();
    return false;
  }
  if (NULL == dlopen("/usr/lib64/dri/radeonsi_dri.so", dlopen_flag)) {
    PLOG(ERROR) << "dlopen(radeonsi_dri.so) failed with error " << dlerror();
    return false;
  }
