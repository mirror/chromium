...
void AddAmdGpuWhitelist(std::vector<BrokerFilePermission>* permissions) {
  ...
  int listSize = sizeof(kReadOnlyList)/sizeof(kReadOnlyList[0]);
  ...
  listSize = sizeof(kReadWriteList)/sizeof(kReadWriteList[0]);
...
ResultExpr CrosAmdGpuProcessPolicy::EvaluateSyscall(int sysno) const {
  switch (sysno) {
    // AMD GPU sandbox is started earlier so we need to allow networking
    // in the sandbox.
    case __NR_connect:
