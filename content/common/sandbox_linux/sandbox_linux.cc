...
bool LinuxSandbox::InitializeSandbox(gpu::GPUInfo* gpu_info) {
...
bool LinuxSandbox::StartSeccompBPF(const std::string& process_type, gpu::GPUInfo* gpu_info) {
  CHECK(!seccomp_bpf_started_);
  CHECK(pre_initialized_);
  if (seccomp_bpf_supported()) {
    seccomp_bpf_started_ =
        SandboxSeccompBPF::StartSandbox(process_type, OpenProc(proc_fd_), gpu_info);
  }
...
bool LinuxSandbox::InitializeSandboxImpl(gpu::GPUInfo* gpu_info) {
