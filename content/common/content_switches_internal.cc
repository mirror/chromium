...
#define WIN_CONV(conv, val) (val)
  ...
  // (No object allocations for the args that don't match the prefix.)
  NativeString prefix =
  ...
  for (const NativeString& arg : command_line.argv()) {
    ...
    if (!StartsWith(arg, prefix, base::CompareCase::SENSITIVE))
      continue;
    NativeStringPiece val(arg.begin() + prefix.size(), arg.end());
    if (!IsStringASCII(val))
      continue;
    auto vals = SplitString(WIN_CONV(base::UTF16ToASCII, val), ",",
