...
TEST(NotificationStructTraitsTest, ValidVibrationPattern) {
  const int n_entries = 99;     // valid
  const int duration_ms = 999;  // valid

  PlatformNotificationData notification_data;
  notification_data.title =
      base::ASCIIToUTF16("Notification with 99 x 999ms entries (valid)");

  int vibration_pattern[n_entries];
  for (size_t i = 0; i < arraysize(vibration_pattern); i++)
    vibration_pattern[i] = duration_ms;
...
// Check round-trip fails when there are too many vibration entries.
TEST(NotificationStructTraitsTest, TooLongVibrationPattern) {
  const int n_entries = 100;  // invalid
  const int duration_ms = 1;  // valid
  ...
  notification_data.vibration_pattern.assign(vibration_pattern,
                                             vibration_pattern + n_entries);

  PlatformNotificationData platform_notification_data;

  ASSERT_FALSE(blink::mojom::NotificationData::Deserialize(
      blink::mojom::NotificationData::Serialize(&notification_data),
      &platform_notification_data));
}
...
TEST(NotificationStructTraitsTest, TooLongVibrationDuration) {
  const int n_entries = 1;        // valid
  const int duration_ms = 10001;  // invalid (>10 seconds)
...
TEST(NotificationStructTraitsTest, TooManyActions) {
  const int n_actions = 3;  // invalid (max is 2)

  PlatformNotificationData notification_data;
  notification_data.title =
      base::ASCIIToUTF16("Notification with 3 actions provided (invalid)");

  notification_data.actions.resize(n_actions);
...
TEST(NotificationStructTraitsTest, DataExceedsMaximumSize) {
  const size_t data_size = 1024 * 1024 + 1;
  PlatformNotificationData notification_data;
  notification_data.title =
      base::ASCIIToUTF16("Notification with too much data");

  const char data[data_size] = "mock binary notification data";

  notification_data.data.assign(data, data + arraysize(data));
  ASSERT_EQ(notification_data.data.size(), data_size);
