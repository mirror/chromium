...
const int kMaximumVibrationPatternLength = 99;
...
bool ValidateVibrationPattern(const std::vector<int>& vibration_pattern) {
  ...
  for (const int duration : vibration_pattern) {
    if (duration > kMaximumVibrationDurationMs)
      return false;
  }
  return true;
}
...
bool ValidateActions(
  ...
  return actions.size() <= kWebNotificationMaxActions;
}
...
bool StructTraits<blink::mojom::NotificationDataDataView,
  ...
  return ValidateProperties(*platform_notification_data);
}
