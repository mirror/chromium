...
void URLLoaderFactoryBundle::RegisterFactory(
  ...
  auto result = factories_.insert(
      std::make_pair(std::string(scheme), std::move(factory)));
...
URLLoaderFactoryBundleInfo URLLoaderFactoryBundle::PassInfo() {
  ...
  for (auto& factory : factories_) {
    factories_info.insert(
        std::make_pair(factory.first, factory.second.PassInterface()));
  }
