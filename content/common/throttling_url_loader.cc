...
void ThrottlingURLLoader::OnClientConnectionError() {
  ...
  if (is_synchronous_ || content::IsBrowserSideNavigationEnabled())
    CancelWithError(net::ERR_ABORTED, std::string());
}
...
void ThrottlingURLLoader::DisconnectClient(const std::string& custom_reason) {
  client_binding_.Close();

  if (!custom_reason.empty()) {
    url_loader_.ResetWithReason(
        network::mojom::URLLoader::kClientDisconnectReason, custom_reason);
  } else {
    url_loader_ = nullptr;
  }
