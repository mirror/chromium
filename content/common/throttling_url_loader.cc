...
class ThrottlingURLLoader::ForwardingThrottleDelegate
 ...
 public:
  ...
  void Resume() override { loader_->UnblockForThrottle(throttle_); }
...
void ThrottlingURLLoader::Start(
  ...
  DCHECK(blocking_throttles_.empty());
  if (!throttles_.empty()) {
    bool deferred = false;
...
bool ThrottlingURLLoader::HandleThrottleResult(URLLoaderThrottle* throttle,
                                               ...
                                               bool* should_defer) {
  DCHECK(!blocking_throttles_.count(throttle));
  if (loader_cancelled_)
    return false;
...
void ThrottlingURLLoader::UnblockForThrottle(URLLoaderThrottle* throttle) {
  if (blocking_throttles_.find(throttle) == blocking_throttles_.end())
    return;

  blocking_throttles_.erase(throttle);
  if (blocking_throttles_.empty() && !loader_cancelled_)
    Resume();
}
