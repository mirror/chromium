...
struct StructTraits<content::mojom::EmbeddedWorkerStartParamsDataView,
  ...
  static GURL scope(const content::EmbeddedWorkerStartParams& info) {
    return info.scope;
  }
  static GURL script_url(const content::EmbeddedWorkerStartParams& info) {
  ...
  static base::UnguessableToken devtools_worker_token(
      const content::EmbeddedWorkerStartParams& info) {
