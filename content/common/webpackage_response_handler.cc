...
void EndTraceEvent(void* id) {
  TRACE_EVENT_NESTABLE_ASYNC_END0(
      kTraceEventCategory, "WebPackageResponseHandler", TRACE_ID_LOCAL(id));
}
}  // namespace
...
WebPackageResponseHandler::~WebPackageResponseHandler() {
  if (!*TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED(kTraceEventCategory))
    return;
  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE, base::Bind(EndTraceEvent, this));
}
