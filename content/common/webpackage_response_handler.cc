...
class WebPackageResponseHandler::BufferedStream : public net::SourceStream {
 public:
  class ScopedTemporaryBufferGuard {
   public:
    explicit ScopedTemporaryBufferGuard(BufferedStream* stream)
        : stream_(stream) {}
    ScopedTemporaryBufferGuard(ScopedTemporaryBufferGuard&& in) noexcept
        : stream_(in.stream_) {
      in.stream_ = nullptr;
    }
    ~ScopedTemporaryBufferGuard() {
      if (stream_)
        stream_->CopyRemainingTemporaryBuffer();
    }
  ...
  ScopedTemporaryBufferGuard OnDataReceived(const char* data, size_t size) {
    ...
    return ScopedTemporaryBufferGuard(this);
  }
...
void WebPackageResponseHandler::OnDataReceived(const void* data, size_t size) {
  LOG(ERROR) << "WebPackageResponseHandler::OnDataReceived " << size << "  "
             << request_url_;
  BufferedStream::ScopedTemporaryBufferGuard scoped_guard =
