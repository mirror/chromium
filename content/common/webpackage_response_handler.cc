...
void WebPackageResponseHandler::OnReadable(int net_err) {
  static const char* kTraceEventName = "WebPackageResponseHandler::OnReadable";
  TRACE_EVENT_NESTABLE_ASYNC_BEGIN1(kTraceEventCategory, kTraceEventName,
                                    TRACE_ID_LOCAL(this), "net_err", net_err);
  if (net_err == net::OK) {
    finished_ = true;
...
void WebPackageResponseHandler::ContinueRead() {
  static const char* kTraceEventName =
      "WebPackageResponseHandler::ContinueRead";
  TRACE_EVENT_NESTABLE_ASYNC_BEGIN0(kTraceEventCategory, kTraceEventName,
                                    TRACE_ID_LOCAL(this));
  while (writing_data_size_ < pending_buffer_size_ && !finished_) {
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0(kTraceEventCategory, "SourceStream::Read",
                                      TRACE_ID_LOCAL(this));
  ...
  const auto writing_data_size_back = writing_data_size_;
  writing_data_size_ = 0;
  ...
  TRACE_EVENT_NESTABLE_ASYNC_END1(kTraceEventCategory, kTraceEventName,
                                  TRACE_ID_LOCAL(this), "writing_data_size",
                                  writing_data_size_back);
}
