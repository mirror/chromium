...
//   defer the request.  All throttles will still be notified.
class CONTENT_EXPORT MultiURLLoaderThrottle : public URLLoaderThrottle {
 ...
 private:
  ...
  // One of the owned URLLoaderThrottles has cancelled the request.  Don'
  bool cancel_called_;

  std::set<std::unique_ptr<MultiURLLoaderThrottleDelegate>> delegates_;
