...
TEST(UniqueNameHelper, GeneratedFramePathHashing) {
  ...
  // A child frame should incporated the parent's hashed requested name into its
  // frame path.
  ...
  // Make sure that legacy name updates correctly handles multiple replacements.
  // An unnamed frame is used as the deepest descendant to ensure the requested
  // names from ancestors appear in the frame path.
