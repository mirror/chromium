...
CrossSiteDocumentClassifier::Result CrossSiteDocumentClassifier::SniffForJSON(
    base::StringPiece data) {
  // Currently this function just looks for an opening brace ('{'), followed
  // by a double-quoted string literal, followed by a colon. Such a pattern
  // would be a Javascript syntax error (as the object-initializer syntax
  // is not a valid top-level statement, although it is a valid expression
  // e.g. if surrounded by parentheses).
  //
