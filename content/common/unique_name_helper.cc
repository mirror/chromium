...
bool UniqueNameHelper::UpdateLegacyNameForMaxSize(
  ...
  if (IsNameWithFramePath(legacy_name) && update_state) {
    size_t replaced_index = legacy_name.find(update_state->original_path);
    if (replaced_index != std::string::npos) {
      std::string name_with_replacement;
      const size_t replacement_size_difference =
          update_state->original_path.size() - update_state->hashed_path.size();
      ...
      name_with_replacement += update_state->hashed_path;
      name_with_replacement += legacy_name.substr(
          replaced_index + update_state->original_path.size());
  ...
  *new_name = CalculateHashedName(legacy_name, unique_suffix_index);
  ...
  if (IsNameWithFramePath(legacy_name)) {
    ...
    (*descendant_update_state)->original_path = '/';
    (*descendant_update_state)->original_path += legacy_name;
  }
