...
const content::mojom::KeyDataPtr&
...
const content::mojom::PointerDataPtr&
...
const content::mojom::TouchDataPtr&
...
const content::mojom::GestureDataPtr&
...
void* StructTraits<content::mojom::EventDataView,
  ...
  if (blink::WebInputEvent::IsGestureEventType(event->web_event->GetType())) {
    ...
    context->gesture_data = std::move(gesture_data);
    return context;
  }
