...
using ResourceGetter = WebPackageSubresourceLoaderFactory::ResourceGetter;
using LoaderFactoryGetter =
    WebPackageSubresourceLoaderFactory::LoaderFactoryGetter;
...
class SubresourceLoader : public mojom::URLLoader {
 public:
  SubresourceLoader(
    ...
    resource_getter_.Run(resource_request,
  ...
  void OnFoundResource(
    ...
    if (error_code == net::ERR_FILE_NOT_FOUND) {
      if (allow_fallback) {
        ...
        loader_factory_getter_.Run()->CreateLoaderAndStart(
...
WebPackageSubresourceLoaderFactory::WebPackageSubresourceLoaderFactory(
  ...
  bindings_.set_connection_error_handler(
      base::Bind(&WebPackageSubresourceLoaderFactory::OnConnectionError,
...
void WebPackageSubresourceLoaderFactory::CreateLoaderAndStart(
    ...
    mojom::URLLoaderClientPtr loader_client,
    const net::MutableNetworkTrafficAnnotationTag& traffic_annotation) {
  LOG(INFO) << "** CreateWebPackageSubresourceLoader: " << resource_request.url;
  new SubresourceLoader(std::move(loader_request), resource_getter_,
                        loader_factory_getter_, routing_id, request_id, options,
