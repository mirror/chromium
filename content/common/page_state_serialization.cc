...
void WriteScrollAnchorProto(const ExplodedFrameState& state,
                            SerializeObject* obj) {
  ScrollAnchor scroll_anchor;
  if (!state.scroll_anchor_selector.is_null()) {
    base::string16 selector_string = state.scroll_anchor_selector.string();
    scroll_anchor.set_selector(selector_string.data(),
                               selector_string.length() * sizeof(base::char16));
  }
...
void ReadFrameState(
  ...
  if (obj->version >= 26 && state->did_save_scroll_or_scale_state) {
    ...
    if (scroll_anchor.selector().size() > 0) {
      state->scroll_anchor_selector = base::NullableString16(base::string16(
          reinterpret_cast<const base::char16*>(
              scroll_anchor.selector().data()),
          scroll_anchor.selector().size() / sizeof(base::char16)));
    }
