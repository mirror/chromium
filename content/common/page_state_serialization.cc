...
base::NullableString16 StringToNullableString16(std::string std_string) {
  if (std_string.empty())
    return base::NullableString16();
  else
    return base::NullableString16(base::UTF8ToUTF16((std_string)), false);
}
...
void ReadFrameState(mojom::FrameState* frame, ExplodedFrameState* state) {
  ...
  for (auto s : frame->document_state) {
    state->document_state.push_back(
        base::NullableString16(base::UTF8ToUTF16(s)));
  }
...
void WritePageState(const ExplodedPageState& state, SerializeObject* obj) {
  ...
  for (size_t i = 0; i < state.referenced_files.size(); ++i) {
    base::NullableString16 referenced_file = state.referenced_files[i];
