...
void ReadFrameState(SerializeObject* obj,
                    bool is_top,
                    const UniqueNameHelper::NameUpdateState* target_replacement,
  ...
  base::Optional<UniqueNameHelper::NameUpdateState> new_target_replacement;
  if (obj->version < 25 && !state->target.is_null()) {
    std::string new_name;
    if (UniqueNameHelper::UpdateLegacyNameForMaxSize(
