...
using content::history::mojom::FileSystemFile;
using content::history::mojom::FileSystemFilePtr;
using content::history::mojom::Element;
using content::history::mojom::ElementPtr;
using content::history::mojom::File;
using content::history::mojom::FilePtr;
using content::history::mojom::RequestBody;
using content::history::mojom::RequestBodyPtr;
using content::history::mojom::HttpBody;
using content::history::mojom::HttpBodyPtr;
using content::history::mojom::Point;
using content::history::mojom::PointF;
using content::history::mojom::ScrollRestorationType;
using content::history::mojom::FrameState;
using content::history::mojom::FrameStatePtr;
using content::history::mojom::PageState;
using content::history::mojom::PageStatePtr;
...
void WriteResourceRequestBody(const ResourceRequestBody& request_body,
                              RequestBody* mojo_body) {
  for (const auto& element : *request_body.elements()) {
    ElementPtr data_element = Element::New();
    switch (element.type()) {
      case ResourceRequestBody::Element::TYPE_BYTES: {
      ...
      case ResourceRequestBody::Element::TYPE_FILE: {
        FilePtr file =
            File::New(element.path(), element.offset(), element.length(),
                      element.expected_modification_time());
        data_element->set_file(std::move(file));
        break;
      }
      case ResourceRequestBody::Element::TYPE_FILE_FILESYSTEM: {
        FileSystemFilePtr file_system = FileSystemFile::New(
...
void ReadResourceRequestBody(
  ...
  for (const auto& element : mojo_body->elements) {
    Element::Tag tag = element->which();
    switch (tag) {
      case Element::Tag::BYTES:
        AppendDataToRequestBody(
            request_body,
            reinterpret_cast<const char*>(element->get_bytes().data()),
            element->get_bytes().size());
        break;
      case Element::Tag::FILE: {
        File* file = element->get_file().get();
        AppendFileRangeToRequestBody(request_body, file->path, file->offset,
                                     file->length, file->modification_time);
        break;
      }
      case Element::Tag::FILE_SYSTEM_FILE: {
        FileSystemFile* file_system = element->get_file_system_file().get();
...
void WriteHttpBody(const ExplodedHttpBody& http_body, HttpBody* mojo_body) {
  if (http_body.request_body != nullptr) {
    mojo_body->request_body = RequestBody::New();
...
void ReadHttpBody(HttpBody* mojo_body, ExplodedHttpBody* http_body) {
...
void WriteFrameState(const ExplodedFrameState& state, FrameState* frame) {
  ...
  frame->scroll_offset = Point::New();
  frame->visual_viewport_scroll_offset = PointF::New();
  if (state.did_save_scroll_or_scale_state) {
  ...
  frame->scroll_restoration_type =
      static_cast<ScrollRestorationType>(state.scroll_restoration_type);
  ...
  for (const auto& child : children) {
    FrameStatePtr child_frame = FrameState::New();
    WriteFrameState(child, child_frame.get());
    frame->children.push_back(std::move(child_frame));
  }
}

void ReadFrameState(FrameState* frame, ExplodedFrameState* state) {
...
void ReadMojoPageState(SerializeObject* obj, ExplodedPageState* state) {
  ...
  PageStatePtr page;
  obj->parse_error = !(PageState::Deserialize(tmp, length, &page));
  if (obj->parse_error)
    return;
