...
// 26: Switch to mojom-based serialization
//
...
// A bunch of convenience functions to read from SerializeObjects.  The
...
base::NullableString16 StringToNullableString16(std::string proto_string) {
  if (proto_string.empty())
    return base::NullableString16();
  else
    return base::NullableString16(base::UTF8ToUTF16((proto_string)));
}
...
void WriteResourceRequestBody(const ResourceRequestBody& request_body,
  ...
  for (const auto& element : *request_body.elements()) {
    ...
    switch (element.type()) {
      case ResourceRequestBody::Element::TYPE_BYTES:
        data_element->set_bytes(element.bytes());
        break;
      case ResourceRequestBody::Element::TYPE_FILE: {
        mojom::FilePtr file = mojom::File::New();
        file->path = element.path().AsUTF8Unsafe();
        file->offset = element.offset();
        file->length = element.length();
      ...
      case ResourceRequestBody::Element::TYPE_FILE_FILESYSTEM: {
        ...
        file_system->filesystem_url = element.path().AsUTF8Unsafe();
        file_system->offset = element.offset();
        file_system->length = element.length();
  ...
  for (const auto& element : mojo_body->elements) {
    ...
    switch (tag) {
      case mojom::Element::Tag::BYTES:
        AppendDataToRequestBody(request_body, &element->get_bytes()[0],
                                element->get_bytes().length() - 1);
        break;
      case mojom::Element::Tag::FILE: {
        mojom::File* file = element->get_file().get();
        AppendFileRangeToRequestBody(
            request_body, StringToNullableString16(file->path), file->offset,
      ...
      case mojom::Element::Tag::FILE_SYSTEM: {
        ...
        AppendURLRangeToRequestBody(
            request_body, GURL(base::UTF8ToUTF16(file_system->filesystem_url)),
...
void WriteHttpBody(const ExplodedHttpBody& http_body,
  ...
  if (http_body.request_body != nullptr) {
    mojom::HttpBody* body = frame->http_body.get();
    body->request_body = mojom::RequestBody::New();
    body->contains_passwords = http_body.contains_passwords;
    WriteResourceRequestBody(*http_body.request_body, body->request_body.get());
  }
}

void ReadHttpBody(mojom::HttpBody* body, ExplodedHttpBody* http_body) {
  http_body->contains_passwords = body->contains_passwords;
  if (body->request_body) {
    http_body->request_body = new ResourceRequestBody();
    ReadResourceRequestBody(body->request_body.get(), http_body->request_body);
  }
}

void PopulateFrameState(const ExplodedFrameState& state,
                        mojom::FrameState* frame) {
  frame->url_string = base::UTF16ToUTF8(state.url_string.string());
  frame->target = base::UTF16ToUTF8(state.target.string());
  ...
  frame->referrer = base::UTF16ToUTF8(state.referrer.string());

  for (auto s : state.document_state) {
    frame->document_state.push_back(base::UTF16ToUTF8(s.string()));
  }
  ...
  bool has_state_object = !state.state_object.is_null();
  if (has_state_object)
    frame->state_object = base::UTF16ToUTF8(state.state_object.string());
  WriteHttpBody(state.http_body, frame);

  // NOTE: It is a quirk of the format that we still have to write the
  // http_content_type field when the HTTP body is null.  That's why this code
  // is here instead of inside WriteHttpBody.
  frame->http_body->http_content_type =
      base::UTF16ToUTF8(state.http_body.http_content_type.string());

  // Subitems
  const std::vector<ExplodedFrameState>& children = state.children;
  for (size_t i = 0; i < children.size(); ++i) {
    mojom::FrameStatePtr child_frame = mojom::FrameState::New();
    PopulateFrameState(children[i], child_frame.get());
    frame->children.push_back(std::move(child_frame));
  }
}

void ReadFrameState(mojom::FrameState* frame, ExplodedFrameState* state) {
  state->url_string = StringToNullableString16(frame->url_string);
  state->referrer = StringToNullableString16(frame->referrer);
  state->target = StringToNullableString16(frame->target);
  state->state_object = StringToNullableString16(frame->state_object);

  for (auto s : frame->document_state) {
    state->document_state.push_back(StringToNullableString16(s));
  }
  ...
  if (frame->http_body) {
    ...
    state->http_body.http_content_type =
        StringToNullableString16(frame->http_body->http_content_type);
  } else {
...
void ReadMojoPageState(SerializeObject* obj, ExplodedPageState* state) {
  ...
  for (auto referenced_file : page->referenced_files) {
    state->referenced_files.push_back(
        StringToNullableString16(referenced_file));
  }
...
void WritePageState(const ExplodedPageState& state, SerializeObject* obj) {
  ...
  for (auto referenced_file : state.referenced_files) {
    page->referenced_files.push_back(
        base::UTF16ToUTF8(referenced_file.string()));
  }

  page->top = mojom::FrameState::New();
  PopulateFrameState(state.top, page->top.get());

  std::vector<uint8_t> page_bytes = mojom::PageState::Serialize(&page);
  obj->pickle.WriteData(reinterpret_cast<char*>(&page_bytes[0]),
                        page_bytes.size());
}
