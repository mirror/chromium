...
void MockWebThemeEngine::Paint(blink::WebCanvas* canvas,
  ...
  switch (part) {
    ...
    case WebThemeEngine::kPartMenuList:
      ...
      irect.fLeft =
          std::max(irect.fLeft, extraParams->menu_list.arrow_x -
                                    (extraParams->menu_list.arrow_size + 1) / 2);
      irect.fRight =
          std::min(irect.fLeft + extraParams->menu_list.arrow_size, irect.fRight);

      irect.fTop =
          extraParams->menu_list.arrow_y - (extraParams->menu_list.arrow_size) / 2;
