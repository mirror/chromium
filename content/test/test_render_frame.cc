...
void TestRenderFrame::SetURLOverrideForNextWebURLRequest(GURL url) {
  next_request_url_override_ = std::move(url);
}
...
blink::WebNavigationPolicy TestRenderFrame::DecidePolicyForNavigation(
  ...
  if (IsBrowserSideNavigationEnabled() &&
      info.url_request.CheckForBrowserSideNavigation() && info.form.IsNull()) {
