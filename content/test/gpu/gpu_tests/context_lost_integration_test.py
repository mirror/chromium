...
class ContextLostIntegrationTest(gpu_integration_test.GpuIntegrationTest):
  ...
  def CustomizeBrowserArgs(cls, browser_args):
    ...
    super(ContextLostIntegrationTest, cls).CustomizeBrowserArgs([
  ...
  def GenerateGpuTests(cls, options):
    tests = (('GpuCrash_GPUProcessCrashesExactlyOncePerVisitToAboutGpuCrash',
             ...
             ('ContextLost_WebGLContextLostInHiddenTab',
              'webgl.html?query=kill_after_notification'),
             ('ContextLost_WebGLBlockedAfterward',
  ...
  def SetUpProcess(cls):
    ...
    cls.CustomizeBrowserArgs(['--disable-domain-blocking-for-3d-apis'])
    cls.StartBrowser()
  ...
  def _ContextLost_WebGLBlockedAfterward(self, test_path):
    self.RestartBrowserIfNecessaryWithArgs([])
    self._NavigateAndWaitForLoad(test_path)
