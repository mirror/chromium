...
class GpuTestTriggerer(object):
  ...
  def trigger_tasks(self, args, remaining):
    ...
    for i in xrange(args.shards):
      ...
      try:
        ...
        args_to_pass = self.modify_args(filtered_remaining_args,
                                        gpu_index, json_temp)
        print 'trigger_gpu_test triggering shard %d with arguments:' % i
        print str(args_to_pass)
    ...
    print 'Writing merged json from triggers to file %s :' % args.dump_json
    print str(merged_json)
