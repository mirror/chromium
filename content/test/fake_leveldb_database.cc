...
void FakeLevelDBDatabase::CopyPrefixedHelper(
    const std::vector<uint8_t>& source_key_prefix,
    const std::vector<uint8_t>& destination_key_prefix) {
  LOG(ERROR) << "copying " << VecToStr(source_key_prefix) << " to "
             << VecToStr(destination_key_prefix);
  ...
  for (const auto& row : mock_data_) {
    ...
    LOG(ERROR) << "moving " << VecToStr(row.first) << " to "
               << VecToStr(new_key) << ". " << VecToStr(row.second);
  ...
  LOG(ERROR) << "State:";
  for (const auto& row : mock_data_) {
    LOG(ERROR) << "key: " << VecToStr(row.first)
               << ", value: " << VecToStr(row.second);
  }
}
