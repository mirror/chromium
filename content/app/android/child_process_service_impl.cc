...
void RegisterFileDescriptors(JNIEnv* env,
  ...
  for (jsize i = 0; i < env->GetArrayLength(j_keys); i++) {
    base::android::ScopedJavaLocalRef<jstring> str(
        env, static_cast<jstring>(env->GetObjectArrayElement(j_keys, i)));
    std::unique_ptr<std::string> key;
    if (!str.is_null()) {
      key = base::MakeUnique<std::string>();
      base::android::ConvertJavaStringToUTF8(env, str.obj(), key.get());
    }
  ...
  for (size_t i = 0; i < ids.size(); i++) {
    ...
    const std::unique_ptr<std::string>& key = keys.at(i);
    int id = ids.at(i);
    int fd = fds.at(i);
    if (key) {
