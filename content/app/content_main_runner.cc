...
#define kSnapshotFileType gin::V8Initializer::SnapshotFileType::kV8Context
#else
#define kSnapshotFileType gin::V8Initializer::SnapshotFileType::kDefault
#endif  // USE_V8_CONTEXT_SNAPSHOT

#if defined(OS_POSIX) && !defined(OS_MACOSX)
#if defined(USE_V8_CONTEXT_SNAPSHOT)
  const char* snapshot_data_descriptor = kV8ContextSnapshotDataDescriptor;
#else
  const char* snapshot_data_descriptor = kV8SnapshotDataDescriptor;
#endif  // USE_V8_CONTEXT_SNAPSHOT
  ...
  if (fd.is_valid()) {
    gin::V8Initializer::LoadV8SnapshotFromFD(fd.get(), region.offset,
                                             region.size, kSnapshotFileType);
    return;
  }
#endif  // OS_POSIX && !OS_MACOSX

#if !defined(CHROME_MULTIPLE_DLL_BROWSER)
  gin::V8Initializer::LoadV8Snapshot(kSnapshotFileType);
