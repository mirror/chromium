...
base::Closure Prog2(base::Closure one, base::Closure two) {
  return base::Bind(
      [](base::Closure one, base::Closure two) {
        if (!one.is_null())
          std::move(one).Run();
        if (!two.is_null())
          std::move(two).Run();
...
static void EnterNamespaceSandbox(service_manager::SandboxLinux* linux_sandbox,
  ...
  if (getpid() == 1) {
    CHECK(CreateInitProcessReaper(
        Prog2(base::Bind(DropAllCapabilities, linux_sandbox->proc_fd()),
...
bool ZygoteMain(
  ...
  EnterLayerOneSandbox(
      ...
      base::Bind(&CloseFds, linux_sandbox->GetFileDescriptorsToClose()));
