...
bool ZygoteMain(
  ...
  sandbox::SetAmZygoteOrRenderer(true);
  ...
  for (const auto& fork_delegate : fork_delegates) {
    fork_delegate->Init(sandbox::GetSandboxFD(), using_layer1_sandbox);
  }
  ...
  Zygote zygote(
      sandbox_flags, std::move(fork_delegates),
      base::GlobalDescriptors::Descriptor(
          static_cast<uint32_t>(kSandboxIPCChannel), sandbox::GetSandboxFD()));
