...
static void ZygotePreSandboxInit(base::OnceClosure pre_sandbox_init) {
  base::RandUint64();
  ...
  if (!pre_sandbox_init.is_null())
    std::move(pre_sandbox_init).Run();
}
...
static void EnterLayerOneSandbox(service_manager::SandboxLinux* linux_sandbox,
                                 ...
                                 base::OnceClosure post_fork_parent_callback) {
  ZygotePreSandboxInit(std::move(pre_sandbox_init));
...
bool ZygoteMain(
  ...
  sandbox::SetAmZygoteOrDescendant(true, backchannel_descriptor.fd);
  ...
  for (const auto& fork_delegate : fork_delegates) {
    fork_delegate->Init(backchannel_descriptor.fd, using_layer1_sandbox);
  }

  // Turn on the first layer of the sandbox if the configuration warrants it.
  EnterLayerOneSandbox(
      linux_sandbox, using_layer1_sandbox, std::move(pre_sandbox_init),
  ...
  return Zygote(sandbox_flags, std::move(fork_delegates),
                std::move(backchannel_descriptor))
      .ProcessRequests();
}
