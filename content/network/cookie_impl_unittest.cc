...
#include "content/network/cookie_impl.h"
...
//      # Classes
//      * SynchronousMojoCookieWrapperWrapper: Takes a mojom::Cookie at
//        construction; exposes synchronous interfaces that wrap the
//        mojom::Cookie async interfaces to make testing easier.
...
//      * MojoCookieWrapperTest: Test base class.  Automatically sets up
...
//        sort cookie list responses from the mojom::Cookie.

namespace content {

// Wraps a mojom::Cookie in synchronous, blocking calls to make it easier to
// test.
class SynchronousMojoCookieWrapperWrapper {
 public:
  // Caller must guarantee that |*cookie_service| outlives the
  // SynchronousMojoCookieWrapperWrapper.
  explicit SynchronousMojoCookieWrapperWrapper(mojom::Cookie* cookie_service)
  ...
  std::vector<net::CanonicalCookie> GetAllCookies() {
    ...
    cookie_service_->GetAllCookies(
        base::BindOnce(&SynchronousMojoCookieWrapperWrapper::GetCookiesCallback,
                       &run_loop, &cookies));
    run_loop.Run();
    return cookies;
  }
  ...
  bool SetCanonicalCookie(const net::CanonicalCookie& cookie,
    ...
    cookie_service_->SetCanonicalCookie(
        ...
        base::BindOnce(&SynchronousMojoCookieWrapperWrapper::SetCookieCallback,
                       &run_loop, &result));
    run_loop.Run();
    return result;
  }
  ...
  uint32_t DeleteCookies(mojom::CookieDeletionFilter filter) {
    ...
    cookie_service_->DeleteCookies(
        std::move(filter_ptr),
        base::BindOnce(
            &SynchronousMojoCookieWrapperWrapper::DeleteCookiesCallback,
 ...
 private:
  ...
  mojom::Cookie* cookie_service_;

  DISALLOW_COPY_AND_ASSIGN(SynchronousMojoCookieWrapperWrapper);
};

class MojoCookieWrapperTest : public testing::Test {
 public:
  MojoCookieWrapperTest()
      : connection_error_seen_(false),
        cookie_monster_(nullptr, nullptr),
        cookie_service_(base::MakeUnique<CookieImpl>(&cookie_monster_)) {
    cookie_service_->AddRequest(mojo::MakeRequest(&cookie_service_ptr_));
    service_wrapper_ = base::MakeUnique<SynchronousMojoCookieWrapperWrapper>(
        cookie_service_ptr_.get());
    cookie_service_ptr_.set_connection_error_handler(base::BindOnce(
        &MojoCookieWrapperTest::OnConnectionError, base::Unretained(this)));
  }
  ~MojoCookieWrapperTest() override {}
  ...
  CookieImpl* service_impl() const { return cookie_service_.get(); }

  // Return the cookie service at the client end of the mojo pipe.
  mojom::Cookie* cookie_service_client() { return cookie_service_ptr_.get(); }

  // Synchronous wrapper
  SynchronousMojoCookieWrapperWrapper* service_wrapper() {
    return service_wrapper_.get();
  }
 ...
 private:
  ...
  std::unique_ptr<content::CookieImpl> cookie_service_;
  mojom::CookiePtr cookie_service_ptr_;
  std::unique_ptr<SynchronousMojoCookieWrapperWrapper> service_wrapper_;
  base::Time setup_time_;

  DISALLOW_COPY_AND_ASSIGN(MojoCookieWrapperTest);
};
...
TEST_F(MojoCookieWrapperTest, GetAllCookies) {
...
TEST_F(MojoCookieWrapperTest, GetCookieList) {
...
TEST_F(MojoCookieWrapperTest, SetExtraCookie) {
...
TEST_F(MojoCookieWrapperTest, DeleteThroughSet) {
...
TEST_F(MojoCookieWrapperTest, ConfirmSecureSetFails) {
...
TEST_F(MojoCookieWrapperTest, ConfirmHttpOnlySetFails) {
  EXPECT_FALSE(service_wrapper()->SetCanonicalCookie(
      net::CanonicalCookie("N", "O", "foo_host", "/", base::Time(),
...
TEST_F(MojoCookieWrapperTest, ConfirmHttpOnlyOverwriteFails) {
  EXPECT_FALSE(service_wrapper()->SetCanonicalCookie(
...
TEST_F(MojoCookieWrapperTest, DeleteEverything) {
...
TEST_F(MojoCookieWrapperTest, DeleteByTime) {
  mojom::CookieDeletionFilter filter;
  EXPECT_EQ(4u, service_wrapper()->DeleteCookies(filter));
  base::Time now(base::Time::Now());
...
TEST_F(MojoCookieWrapperTest, DeleteByExcludingDomains) {
  mojom::CookieDeletionFilter filter;
  EXPECT_EQ(4u, service_wrapper()->DeleteCookies(filter));
...
TEST_F(MojoCookieWrapperTest, DeleteByIncludingDomains) {
  mojom::CookieDeletionFilter filter;
  EXPECT_EQ(4u, service_wrapper()->DeleteCookies(filter));
...
TEST_F(MojoCookieWrapperTest, DeleteBySessionStatus) {
  mojom::CookieDeletionFilter filter;
  EXPECT_EQ(4u, service_wrapper()->DeleteCookies(filter));
  base::Time now(base::Time::Now());
...
TEST_F(MojoCookieWrapperTest, DeleteByAll) {
  mojom::CookieDeletionFilter filter;
  EXPECT_EQ(4u, service_wrapper()->DeleteCookies(filter));
  base::Time now(base::Time::Now());
...
// Class to receive and record notifications from the mojom::Cookie.
...
TEST_F(MojoCookieWrapperTest, Notification) {
  GURL notification_url("http://www.testing.com/pathele");
  std::string notification_name("Cookie_Name");
  mojom::CookieChangeNotificationPtr ptr;
  mojom::CookieChangeNotificationRequest request(mojo::MakeRequest(&ptr));
...
// is destroyed.
TEST_F(MojoCookieWrapperTest, NotificationRequestDestroyed) {
  // Create two identical notification interfaces.
  GURL notification_url("http://www.testing.com/pathele");
  std::string notification_name("Cookie_Name");

  mojom::CookieChangeNotificationPtr ptr1;
  mojom::CookieChangeNotificationRequest request1(mojo::MakeRequest(&ptr1));
  ...
  mojom::CookieChangeNotificationPtr ptr2;
  mojom::CookieChangeNotificationRequest request2(mojo::MakeRequest(&ptr2));
...
TEST_F(MojoCookieWrapperTest, ServiceDestructVisible) {
...
// dies.
TEST_F(MojoCookieWrapperTest, CloningAndClientDestructVisible) {
  EXPECT_EQ(1u, service_impl()->GetClientsBoundForTesting());

  // Clone the interface.
  mojom::CookiePtr new_ptr;
  cookie_service_client()->CloneInterface(mojo::MakeRequest(&new_ptr));

  SynchronousMojoCookieWrapperWrapper new_wrapper(new_ptr.get());
