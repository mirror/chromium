...
//      # Classes
//      * SynchronousMojoCookieWrapper: Takes a mojom::Cookie at
...
// test.
class SynchronousMojoCookieWrapper {
 public:
  // Caller must guarantee that |*cookie_service| outlives the
  // SynchronousMojoCookieWrapper.
  explicit SynchronousMojoCookieWrapper(mojom::Cookie* cookie_service)
  ...
  std::vector<net::CanonicalCookie> GetAllCookies() {
    ...
    cookie_service_->GetAllCookies(
        base::BindOnce(&SynchronousMojoCookieWrapper::GetCookiesCallback,
                       &run_loop, &cookies));
    run_loop.Run();
    return cookies;
  }
  ...
  bool SetCanonicalCookie(const net::CanonicalCookie& cookie,
    ...
    cookie_service_->SetCanonicalCookie(
        ...
        base::BindOnce(&SynchronousMojoCookieWrapper::SetCookieCallback,
                       &run_loop, &result));
    run_loop.Run();
    return result;
  }
  ...
  uint32_t DeleteCookies(mojom::CookieDeletionFilter filter) {
    ...
    cookie_service_->DeleteCookies(
        std::move(filter_ptr),
        base::BindOnce(&SynchronousMojoCookieWrapper::DeleteCookiesCallback,
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(SynchronousMojoCookieWrapper);
};
...
class MojoCookieWrapperTest : public testing::Test {
 public:
  MojoCookieWrapperTest()
    ...
    service_wrapper_ = base::MakeUnique<SynchronousMojoCookieWrapper>(
  ...
  // Synchronous wrapper
  SynchronousMojoCookieWrapper* service_wrapper() {
 ...
 private:
  ...
  std::unique_ptr<SynchronousMojoCookieWrapper> service_wrapper_;
...
TEST_F(MojoCookieWrapperTest, CloningAndClientDestructVisible) {
  ...
  SynchronousMojoCookieWrapper new_wrapper(new_ptr.get());
