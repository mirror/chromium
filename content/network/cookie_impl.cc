...
class PredicateWrapper {
 public:
  ...
  bool Predicate(const net::CanonicalCookie& cookie) {
    ...
    if (session_control_ !=
        content::mojom::CookieDeletionSessionControl::IGNORE_CONTROL) {
      // Relies on the assumption that there are only three values possible for
      // session_control.
      result &=
          (cookie.IsPersistent() ==
           (session_control_ ==
            content::mojom::CookieDeletionSessionControl::PERSISTENT_COOKIES));
    }
    return result;
  }
...
CookieImpl::NotificationRegistration::NotificationRegistration() {}

CookieImpl::NotificationRegistration::~NotificationRegistration() {}

CookieImpl::CookieImpl(net::CookieStore* cookie_store)
    : cookie_store_(cookie_store) {}

CookieImpl::~CookieImpl() {}

void CookieImpl::AddRequest(content::mojom::CookieRequest request) {
  bindings_.AddBinding(this, std::move(request));
}

void CookieImpl::GetAllCookies(GetAllCookiesCallback callback) {
  cookie_store_->GetAllCookiesAsync(std::move(callback));
}

void CookieImpl::GetCookieList(const GURL& url,
...
void CookieImpl::SetCanonicalCookie(const net::CanonicalCookie& cookie,
                                    bool secure_source,
...
void CookieImpl::DeleteCookies(content::mojom::CookieDeletionFilterPtr filter,
...
void CookieImpl::RequestNotification(
    const GURL& url,
  ...
  notification_registration->subscription = cookie_store_->AddCallbackForCookie(
      url, name,
      base::Bind(&CookieImpl::CookieChanged,
                 // base::Unretained is safe as destruction of the
                 // CookieImpl will also destroy the
  ...
  notification_registration->notification_pointer.set_connection_error_handler(
      base::Bind(&CookieImpl::NotificationPipeBroken,
                 // base::Unretained is safe as destruction of the
                 // CookieImpl will also destroy the
                 // notifications_registered list (which this object will be
                 // inserted into, below), which will destroy the
...
void CookieImpl::CookieChanged(NotificationRegistration* registration,
...
void CookieImpl::NotificationPipeBroken(
...
void CookieImpl::CloneInterface(mojom::CookieRequest new_interface) {
  AddRequest(std::move(new_interface));
}

std::unique_ptr<mojom::Cookie> CreateCookieService(
    net::CookieStore* cookie_store) {
  return base::MakeUnique<CookieImpl>(cookie_store);
}
