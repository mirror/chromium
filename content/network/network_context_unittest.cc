...
TEST_F(NetworkContextTest, ProxyConfig) {
  ...
  for (const auto& initial_proxy_config : proxy_configs) {
    ...
    mojom::ProxyConfigClientAssociatedPtr config_client;
    context_params->proxy_config_client_request =
        mojo::MakeRequestAssociatedWithDedicatedPipe(&config_client);
    std::unique_ptr<NetworkContext> network_context =
        CreateContextWithParams(std::move(context_params));
...
TEST_F(NetworkContextTest, StaticProxyConfig) {
  ...
  mojom::NetworkContextParamsPtr context_params = CreateContextParams();
  context_params->proxy_config = proxy_config;
  std::unique_ptr<NetworkContext> network_context =
      CreateContextWithParams(std::move(context_params));
...
TEST_F(NetworkContextTest, ProxyConfigPoller) {
  ...
  context_params->proxy_config_poller_client = poller.BindInterface();
  std::unique_ptr<NetworkContext> network_context =
      CreateContextWithParams(std::move(context_params));
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    // Spin the message loop until all polls received.
    scoped_task_environment_.RunUntilIdle();
    if (i == 0) {
      // The first request will trigger two lazy poll events.
      EXPECT_EQ(2, poller.GetAndClearTimesPolled());
    } else {
      EXPECT_EQ(1, poller.GetAndClearTimesPolled());
    }
  }
}
