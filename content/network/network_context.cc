  ...
  if (network_context_params->http_server_properties_path) {
    std::unique_ptr<net::HttpServerPropertiesManager> http_server_properties =
        std::make_unique<net::HttpServerPropertiesManager>(
            ...
            // PrefDelegate.
            new HttpServerPropertiesPrefDelegate(
...
void NetworkContext::ClearNetworkingHistorySince(base::Time time,
  ...
  http_server_properties_manager_->Clear(std::move(completion));
}
