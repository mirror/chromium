...
class TestClient : public mojom::NetworkChangeManagerClient {
 public:
  explicit TestClient(mojom::NetworkService* network_service)
...
class NetworkChangeManagerImplTest : public testing::Test {
 public:
  NetworkChangeManagerImplTest()
    ...
    network_change_manager_client_ =
        std::make_unique<TestClient>(network_service_.get());
  }

  ~NetworkChangeManagerImplTest() override {}

  TestClient* network_change_manager_client() {
 ...
 private:
  ...
  std::unique_ptr<TestClient> network_change_manager_client_;
