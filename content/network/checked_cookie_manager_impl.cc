...
CheckedCookieManagerImpl::CheckedCookieManagerImpl(
...
CheckedCookieManagerImpl::~CheckedCookieManagerImpl() = default;

void CheckedCookieManagerImpl::CreateMojoService(
    BrowserContext* browser_context,
    scoped_refptr<net::URLRequestContextGetter> request_context,
    int render_process_id,
    int render_frame_id,
    blink::mojom::CookieStoreRequest request) {
  mojo::MakeStrongBinding(base::WrapUnique(new CheckedCookieManagerImpl(
                              browser_context, std::move(request_context),
                              render_process_id, render_frame_id)),
                          std::move(request));
}

void CheckedCookieManagerImpl::GetAll(
    const GURL& url,
  ...
  GetRequestContextForURL(url)->cookie_store()->GetCookieListWithOptionsAsync(
      url, net_options,
      base::BindOnce(&CheckedCookieManagerImpl::CookieListToGetAllCallback,
                     weak_ptr_factory_.GetWeakPtr(), url, site_for_cookies,
                     std::move(options), std::move(callback)));
}

net::URLRequestContext* CheckedCookieManagerImpl::GetRequestContextForURL(
    const GURL& url) {
...
void CheckedCookieManagerImpl::CookieListToGetAllCallback(
    const GURL& url,
    const GURL& site_for_cookies,
    blink::mojom::CookieStoreGetOptionsPtr options,
    blink::mojom::CookieStore::GetAllCallback callback,
...
void CheckedCookieManagerImpl::Set(
    const GURL& url,
    const GURL& site_for_cookies,
  ...
  auto cookie = base::MakeUnique<net::CanonicalCookie>(
      options->name, options->value, options->domain, options->path, now,
      expires_at, now, options->secure, options->http_only,
