...
CORSURLLoaderOOR::CORSURLLoaderOOR(
    int32_t routing_id,
    int32_t request_id,
    uint32_t options,
  ...
  network_client_binding_.set_connection_error_handler(base::BindOnce(
      &CORSURLLoaderOOR::OnUpstreamConnectionError, base::Unretained(this)));
...
void CORSURLLoaderOOR::FollowRedirect() {
...
void CORSURLLoaderOOR::SetPriority(net::RequestPriority priority,
...
void CORSURLLoaderOOR::PauseReadingBodyFromNet() {
  DCHECK(!is_waiting_follow_redirect_call_);
  if (network_loader_)
    network_loader_->PauseReadingBodyFromNet();
}

void CORSURLLoaderOOR::ResumeReadingBodyFromNet() {
  DCHECK(!is_waiting_follow_redirect_call_);
  if (network_loader_)
    network_loader_->ResumeReadingBodyFromNet();
}

void CORSURLLoaderOOR::OnReceiveResponse(
...
void CORSURLLoaderOOR::OnReceiveRedirect(
...
void CORSURLLoaderOOR::OnDataDownloaded(int64_t data_len,
...
void CORSURLLoaderOOR::OnUploadProgress(int64_t current_position,
                                     int64_t total_size,
...
void CORSURLLoaderOOR::OnReceiveCachedMetadata(const std::vector<uint8_t>& data) {
  DCHECK(network_loader_);
  DCHECK(forwarding_client_);
  DCHECK(!is_waiting_follow_redirect_call_);
...
void CORSURLLoaderOOR::OnTransferSizeUpdated(int32_t transfer_size_diff) {
  DCHECK(network_loader_);
  DCHECK(forwarding_client_);
  DCHECK(!is_waiting_follow_redirect_call_);
...
void CORSURLLoaderOOR::OnStartLoadingResponseBody(
...
void CORSURLLoaderOOR::OnComplete(
...
void CORSURLLoaderOOR::OnUpstreamConnectionError() {
...
void CORSURLLoaderOOR::HandleComplete(
    const network::URLLoaderCompletionStatus& status) {
