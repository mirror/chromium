...
//      # Classes
//      * SynchronousCookieServiceWrapper: Takes a CookieService at
//        construction; exposes synchronous interfaces that wrap the
//        CookieService async interfaces to make testing easier.
...
//      * CookieServiceTest: Test base class.  Automatically sets up
...
//        sort cookie list responses from the CookieService.
...
class CookieServiceTest : public testing::Test {
 public:
  CookieServiceTest()
    ...
    service_wrapper_ = base::MakeUnique<SynchronousCookieServiceWrapper>(
 ...
 private:
  ...
  std::unique_ptr<SynchronousCookieServiceWrapper> service_wrapper_;
...
TEST_F(CookieServiceTest, DeleteByIncludingDomains) {
  mojom::CookieDeletionFilter filter;
  EXPECT_EQ(4u, service_wrapper()->DeleteCookies(filter));
...
TEST_F(CookieServiceTest, DeleteByExcludingDomains) {
  mojom::CookieDeletionFilter filter;
  EXPECT_EQ(4u, service_wrapper()->DeleteCookies(filter));
...
// Class to receive and record notifications from the CookieService.
...
// is destroyed.
TEST_F(CookieServiceTest, NotificationRequestDestroyed) {
...
TEST_F(CookieServiceTest, ServiceDestructVisible) {
...
// dies.
TEST_F(CookieServiceTest, CloningAndClientDestructVisible) {
  EXPECT_EQ(1u, service_impl()->GetClientsBoundForTesting());

  // Clone the interface.
  mojom::CookieServicePtr new_ptr;
