...
class PredicateWrapper {
 public:
  explicit PredicateWrapper(content::mojom::CookieDeletionFilterPtr filter)
      : use_including_domains_(filter->excluding_domains.has_value()),
        excluding_domains_(filter->excluding_domains.has_value()
                               ...
                               : std::set<std::string>()),
        use_excluding_domains_(filter->including_domains.has_value()),
  ...
  bool Predicate(const net::CanonicalCookie& cookie) {
    ...
    if (session_control_ !=
        content::mojom::CookieDeletionSessionControl::IGNORE) {
