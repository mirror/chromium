...
// static
void ThrottlingController::SetConditions(const std::string& client_id,
                                         bool offline,
  ...
  std::unique_ptr<NetworkConditions> conditions(
      new NetworkConditions(offline, latency.InMillisecondsF(),
                            download_throughput, upload_throughput));
  instance_->SetNetworkConditions(client_id, std::move(conditions));
}

// static
void ThrottlingController::DisableThrottling(const std::string& client_id) {
  if (instance_)
    instance_->SetNetworkConditions(client_id,
