...
class CONTENT_EXPORT ServiceWorkerDispatcher : public WorkerThread::Observer {
 public:
  ...
  // execution context. When a new one is created, increments interprocess
  // references to its versions via ServiceWorkerHandleReference.
  scoped_refptr<WebServiceWorkerRegistrationImpl>
  GetOrCreateRegistrationForController(
      blink::mojom::ServiceWorkerRegistrationObjectInfoPtr info,
      const ServiceWorkerVersionAttributes& attrs,
      scoped_refptr<base::SingleThreadTaskRunner> binding_task_runner =
          nullptr);

  // Returns the existing registration or a newly created one for service worker
  // clients contexts(document, shared worker, dedicated worker). Always adopts
  // interprocess references to its versions via
  // ServiceWorkerHandleReference.
  scoped_refptr<WebServiceWorkerRegistrationImpl>
  GetOrAdoptRegistrationForControllee(
      blink::mojom::ServiceWorkerRegistrationObjectInfoPtr info,
      const ServiceWorkerVersionAttributes& attrs);
