...
class FakeServiceWorkerRegistrationObjectHost
    : public blink::mojom::ServiceWorkerRegistrationObjectHost {
 public:
  FakeServiceWorkerRegistrationObjectHost() = default;
  ~FakeServiceWorkerRegistrationObjectHost() override = default;
...
class ServiceWorkerDispatcherTest : public testing::Test {
 public:
  ...
  void CreateObjectInfoAndVersionAttributes(
    ...
    *info = blink::mojom::ServiceWorkerRegistrationObjectInfo::New();
    ...
    reg_obj_host_.AddBinding(mojo::MakeRequest(&(*info)->host_ptr_info));
  ...
  const FakeServiceWorkerRegistrationObjectHost& reg_obj_host() {
    return reg_obj_host_;
  }
 ...
 private:
  ...
  FakeServiceWorkerRegistrationObjectHost reg_obj_host_;
...
TEST_F(ServiceWorkerDispatcherTest, GetOrCreateRegistration) {
  ...
  CreateObjectInfoAndVersionAttributes(&info1, &attrs1);
  int64_t registration_id1 = info1->registration_id;
  int32_t handle_id1 = info1->handle_id;
  // The 1st ServiceWorkerRegistrationObjectHost mojo connection has been added.
  ASSERT_EQ(1, reg_obj_host().GetBindingCount());
  ...
  EXPECT_EQ(1, reg_obj_host().GetBindingCount());
  ASSERT_EQ(3UL, ipc_sink()->message_count());
  EXPECT_EQ(ServiceWorkerHostMsg_IncrementServiceWorkerRefCount::ID,
            ipc_sink()->GetMessageAt(0)->type());
  EXPECT_EQ(ServiceWorkerHostMsg_IncrementServiceWorkerRefCount::ID,
  ...
  CreateObjectInfoAndVersionAttributes(&info2, &attrs2);
  // The 2nd mojo connection has been added.
  ASSERT_EQ(2, reg_obj_host().GetBindingCount());
  ...
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(0, reg_obj_host().GetBindingCount());
}
...
TEST_F(ServiceWorkerDispatcherTest, GetOrAdoptRegistration) {
  ...
  // refcounts.
  scoped_refptr<WebServiceWorkerRegistrationImpl> registration1(
      dispatcher()->GetOrAdoptRegistration(std::move(info1), attrs1));
  EXPECT_TRUE(registration1);
  EXPECT_TRUE(ContainsRegistration(handle_id1));
  EXPECT_EQ(registration_id1, registration1->RegistrationId());
  EXPECT_EQ(1, reg_obj_host().GetBindingCount());
  ...
  // refcounts.
  scoped_refptr<WebServiceWorkerRegistrationImpl> registration2(
      dispatcher()->GetOrAdoptRegistration(std::move(info2), attrs2));
  EXPECT_TRUE(registration2);
  EXPECT_EQ(registration1, registration2);
  // The 2nd mojo connection has been dropped.
  base::RunLoop().RunUntilIdle();
