...
WebServiceWorkerRegistrationImpl::WebServiceWorkerRegistrationImpl(
  ...
  registration_object_host_ =
      blink::mojom::ThreadSafeServiceWorkerRegistrationObjectHostAssociatedPtr::
          Create(std::move(info_->host_ptr_info),
                 binding_task_runner_ ? binding_task_runner_
...
void WebServiceWorkerRegistrationImpl::Update(
  ...
  // As |registration_object_host_| is only owned by |this|, using
  // base::Unretained() here is safe.
  (*registration_object_host_)
      ->Update(base::BindOnce(&WebServiceWorkerRegistrationImpl::OnUpdated,
