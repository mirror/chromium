...
class FakeControllerServiceWorker : public mojom::ControllerServiceWorker {
 public:
  ...
  // Tells this to respond to fetch events with network fallback.
  ...
  // Tells this to respond to fetch events with the specified stream.
  ...
  void SetUp() override {
    ...
    loader_factory_getter_ =
        base::MakeRefCounted<ChildURLLoaderFactoryGetterImpl>(
            std::move(mock_loader_factory), nullptr);
