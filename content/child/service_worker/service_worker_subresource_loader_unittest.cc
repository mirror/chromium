...
class FackNetworkURLLoaderFactory final : public mojom::URLLoaderFactory {
 public:
  FackNetworkURLLoaderFactory() = default;
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(FackNetworkURLLoaderFactory);
};
...
class ServiceWorkerSubresourceLoaderTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    mojom::URLLoaderFactoryPtr fack_loader_factory;
    mojo::MakeStrongBinding(base::MakeUnique<FackNetworkURLLoaderFactory>(),
                            MakeRequest(&fack_loader_factory));
    loader_factory_getter_ =
        base::MakeRefCounted<ChildURLLoaderFactoryGetterImpl>(
            std::move(fack_loader_factory), nullptr);
