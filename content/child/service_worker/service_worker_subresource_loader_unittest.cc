...
TEST(ServiceWorkerSubresourceLoaderTest, Basic) {
  ...
  MockServiceWorkerEventDispatcher event_dispatcher;

  EXPECT_EQ(0, event_dispatcher.fetch_event_count());

  ServiceWorkerSubresourceLoaderFactory loader_factory(
      &event_dispatcher, loader_factory_getter, request.url.GetOrigin());
  loader_factory.CreateLoaderAndStart(
      mojo::MakeRequest(&url_loader), 0, 0, mojom::kURLLoadOptionNone, request,
      url_loader_client.CreateInterfacePtr(),
      net::MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS));
  base::RunLoop run_loop;
  run_loop.RunUntilIdle();

  EXPECT_EQ(1, event_dispatcher.fetch_event_count());
  EXPECT_EQ(request.url, event_dispatcher.fetch_event_request().url);
  EXPECT_EQ(request.method, event_dispatcher.fetch_event_request().method);
}
