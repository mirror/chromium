...
TEST_F(ServiceWorkerSubresourceLoaderTest, RedirectResponse) {
  fake_controller_.RespondWithRedirect("https://www.example.com/bar.html");
  ...
  const net::RedirectInfo& redirect_info1 = url_loader_client_->redirect_info();
  EXPECT_EQ(302, redirect_info1.status_code);
  EXPECT_EQ("GET", redirect_info1.new_method);
  EXPECT_EQ(GURL("https://www.example.com/bar.html"), redirect_info1.new_url);

  url_loader_client_->ClearHasReceivedRedirect();

  // Redirect once more.
  fake_controller_.RespondWithRedirect("https://www.example.com/baz.html");
  url_loader_->FollowRedirect();
  base::RunLoop().RunUntilIdle();

  EXPECT_EQ(net::OK, url_loader_client_->completion_status().error_code);
  EXPECT_TRUE(url_loader_client_->has_received_redirect());
  const net::RedirectInfo& redirect_info2 = url_loader_client_->redirect_info();
  EXPECT_EQ(302, redirect_info2.status_code);
  EXPECT_EQ("GET", redirect_info2.new_method);
  EXPECT_EQ(GURL("https://www.example.com/baz.html"), redirect_info2.new_url);
