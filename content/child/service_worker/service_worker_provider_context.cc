...
void ServiceWorkerProviderContext::SetController(
  ...
  if (provider_client_) {
    ...
    // You must not access |provider_client_| after setController() because it
    // may fire the controllerchange event that may remove the provider client,
    // for example, by detaching an iframe.
  }
}

void ServiceWorkerProviderContext::set_provider_client(
