...
void ServiceWorkerProviderContext::SetController(
  ...
  // ServiceWorkerProviderContext::SetController() can be called before the
  // ServiceWorkerContainer exists, i.e., the document has not yet been created.
  // In that case, the controllee_state->|web_service_worker_provider| is set
  // when WebServiceWorkerProviderImpl is created, and then
  // ServiceWorkerProviderContext::SetController() is called when its
  // SetClient() is called right after that.
