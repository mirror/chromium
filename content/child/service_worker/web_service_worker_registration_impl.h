...
class CONTENT_EXPORT WebServiceWorkerRegistrationImpl
 ...
 public:
  // |binding_task_runner| is used to bind |host_for_global_scope_|.
 ...
 private:
  ...
  // Either |host_for_global_scope_| or |host_for_client| is non-null.
  //
  // |host_for_global_scope_| is for service worker execution contexts. It is
  // bound on the IO thread and used on the worker thread. The reason why we do
  // not bind it also on the worker thread is that the legacy IPC
  // channel-associated interfaces specifically are not usable on any thread
  // other than the main thread or the IO thread, so we binds it on the IO
  // thread now to keep ordering with other messages sent via
  // |ServiceWorkerDispatcher::thread_safe_sender_|.
