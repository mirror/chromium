...
void ServiceWorkerSubresourceLoader::OnFetchEventFinished(
  ...
  // Status OK and WAIT_UNTIL_REJECTED are expected statuses. If status is OK,
  // then OnResponse*() or OnFallback() was already called or will be called. If
  // status is WAIT_UNTIL_REJECTED, OnResponse() was already called or will be
  // called with an error about the rejected promise.
