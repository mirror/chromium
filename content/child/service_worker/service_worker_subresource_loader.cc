...
ServiceWorkerSubresourceLoader::ServiceWorkerSubresourceLoader(
    ...
    : url_loader_client_(std::move(client)),
      ...
      default_loader_factory_getter_(default_loader_factory_getter),
...
void ServiceWorkerSubresourceLoader::OnResponse(
  ...
  OnResponseStream(response, blink::mojom::ServiceWorkerStreamHandlePtr());
}
...
void ServiceWorkerSubresourceLoader::OnResponseStream(
  ...
  if (!response.blob_uuid.empty() && blob_registry) {
    ...
    blob_registry->RegisterPublicBlobURL(
        blob_url_, blink::WebString::FromASCII(response.blob_uuid));
    if (!response.blob) {
      // Decrement Blob ref-count to offset AddRef in the SW side.
      // TODO(kinuko): Remove this once kMojoBlobs is available (or let
      // this code path use |response.blob| anyways), we should just rely
      // on |response.blob| for lifetime management. (crbug.com/755523)
      DCHECK(!base::FeatureList::IsEnabled(features::kMojoBlobs));
      blink::Platform::Current()->GetBlobRegistry()->RemoveBlobDataRef(
          blink::WebString::FromASCII(response.blob_uuid));
    }
    DVLOG(1) << "Reading blob: " << response.blob_uuid << " / "
             << blob_url_.spec();
    ...
    default_loader_factory_getter_->blob_loader_factory()->CreateLoaderAndStart(
        mojo::MakeRequest(&blob_loader_), routing_id_, request_id_, options_,
        resource_request_, std::move(blob_loader_client), traffic_annotation_);
    return;
  }
...
void ServiceWorkerSubresourceLoader::OnFallback() {
  ...
  default_loader_factory_getter_->network_loader_factory()
...
void ServiceWorkerSubresourceLoaderFactory::CreateAndBind(
  ...
  mojo::MakeStrongBinding(
      base::MakeUnique<ServiceWorkerSubresourceLoaderFactory>(
          event_dispatcher, default_loader_factory_getter, controller_origin),
      std::move(request));
}
