...
// Helper to receive the fetch event response even.
class ServiceWorkerSubresourceLoader::ResponseCallback
 ...
 public:
  ...
  void OnResponseStream(
      ...
      base::Time dispatch_event_time) override {
    if (loader_)
      loader_->StartResponse(response, std::move(body_as_stream));
  }
...
void ServiceWorkerSubresourceLoader::StartResponse(
  ...
  if (!response.blob_uuid.empty() && blob_registry) {
    ...
    if (!response.blob) {
      ...
      // TODO(kinuko): Remove this once kMojoBlob is available, we should
      // just rely on response.blob.
