...
void ServiceWorkerSubresourceLoader::StartResponse(
    ...
    storage::mojom::BlobPtr body_as_blob,
    blink::mojom::ServiceWorkerStreamHandlePtr body_as_stream) {
  ServiceWorkerURLLoaderHelper::PopulateResponseHead(response, &response_head_);

  // Handle a stream response body.
  if (!body_as_stream.is_null() && body_as_stream->stream.is_valid()) {
    ServiceWorkerURLLoaderHelper::HandleStreamResponseBody(
        &response_head_, base::nullopt /* ssl_info_ */, &url_loader_client_,
        &body_as_stream);
...
void ServiceWorkerSubresourceLoader::DeliverErrorResponse() {
  ...
  if (status_ < Status::kSentHeader) {
    ServiceWorkerURLLoaderHelper::Send500(base::nullopt /* ssl_info_ */,
                                          &response_head_, &url_loader_client_);
  }
...
void ServiceWorkerSubresourceLoader::OnReceiveResponse(
  ...
  DCHECK_EQ(Status::kStarted, status_);
  status_ = Status::kSentHeader;
  ServiceWorkerURLLoaderHelper::HandleBlobResponseBody(
