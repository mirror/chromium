...
void ServiceWorkerSubresourceLoader::StartResponse(
  ...
  if (!response.blob_uuid.empty() && blob_registry) {
    DCHECK(loader_factory_container_);
    blink::WebString blob_uuid_webstring =
        blink::WebString::FromUTF8(response.blob_uuid);
    blob_registry->AddBlobDataRef(blob_uuid_webstring);
    blob_url_ =
        GURL("blob:" + controller_origin_.spec() + "/" + response.blob_uuid);
    blob_registry->RegisterPublicBlobURL(blob_url_, blob_uuid_webstring);
    LOG(ERROR) << "** READING:" << response.blob_uuid << " / "
               << blob_url_.spec();
...
void ServiceWorkerSubresourceLoader::OnReceiveResponse(
  ...
  DCHECK_EQ(Status::kStarted, status_);
  status_ = Status::kSentHeader;
  LOG(ERROR) << "** Blob::OnReceiveResponse!! "
