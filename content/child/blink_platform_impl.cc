...
// Function for the embedder to call to hook GC allocations.
void HookBlinkGC(void (*hook_alloc)(uint8_t*, size_t, const char*),
                 void (*hook_free)(uint8_t*)) {
  blink::WebHeap::SetAllocationHook(hook_alloc);
  blink::WebHeap::SetFreeHook(hook_free);
}
...
void BlinkPlatformImpl::InternalInit() {
  ...
  // Content client may not exist in some tests.
  ContentClient* content_client = GetContentClient();
  if (content_client)
    content_client->SetGCHeapAllocationHookFunction(&HookBlinkGC);
}
