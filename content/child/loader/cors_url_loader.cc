...
#include "third_party/WebKit/public/platform/WebCORSPreflightResultCache.h"
...
blink::WebCORS::AccessStatus CheckAccess(
  ...
  return blink::WebCORS::CheckAccess(
      url, head.headers->response_code(),
      blink::WebHTTPHeaderMap(head.headers.get()),
...
std::string AccessControlErrorString(
  ...
  return blink::WebCORS::AccessControlErrorString(
             ...
             blink::WebHTTPHeaderMap(head.headers.get()), security_origin,
...
CORSURLLoader::CORSURLLoader(
  ...
  // prevent_preflight can be used only when the CORS is enabled.
  DCHECK(!request_.cors_prevent_preflight || IsCORSEnabled());

  std::string error_description;

  if (request_.cors_redirect) {
    cors_flag_ = true;
  } else if ((IsNavigation() || IsSameOriginOrWhitelisted(request_.url))) {
...
void CORSURLLoader::OnReceiveRedirect(
  ...
  // the request (i.e. calling forwarding client's OnComplete with error code).
  if (mode_ == Mode::kPreflight)
...
bool CORSURLLoader::NeedsPreflight() const {
  ...
  if (request_.cors_prevent_preflight)
    return false;
  ...
  blink::WebHTTPHeaderMap request_headers(&request_.headers);
...
void CORSURLLoader::AddOriginHeader(ResourceRequest& request) {
  ...
  if (security_origin_.suborigin() != "") {
    request.headers.SetHeader(net::HttpRequestHeaders::kSuborigin,
...
void CORSURLLoader::HandlePreflightResponse(const ResourceResponseHead& head) {
  ...
  if (!result ||
      ...
      !result->AllowsCrossOriginHeaders(
          blink::WebHTTPHeaderMap(&request_.headers), error_description)) {
...
void CORSURLLoader::HandleActualResponse(
  ...
  if (head.was_fetched_via_service_worker) {
    ...
    // wpt/service-workers/service-worker/fetch-event-redirect.https.html.
...
ResourceRequest CORSURLLoader::CreateAccessControlPreflightRequest(
  ...
  preflight_request.headers.SetHeader("Access-Control-Request-Method",
                                      request.method);

  if (request.is_external_request) {
    preflight_request.headers.SetHeader("Access-Control-Request-External",
                                        "true");
  }

  if (!request.headers.IsEmpty()) {
    std::string access_control_request_headers =
        blink::WebCORS::CreateAccessControlRequestHeadersHeader(
            blink::WebHTTPHeaderMap(&request_.headers))
            .Latin1();

    if (!access_control_request_headers.empty()) {
      preflight_request.headers.SetHeader("Access-Control-Request-Headers",
