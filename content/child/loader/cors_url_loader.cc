...
// underlying network loader. The work in progress to implement the CORS
// functionality (currently living in DocumentThreadableLoader) can be found
// here: https://chromium-review.googlesource.com/c/558226.
...
voidCORSURLLoader::OnDataDownloaded(int64_t data_len,
                                    int64_t encoded_data_len) override {
  forwarding_client_->OnDataDownloaded(data_len, encoded_data_len);
}

void CORSURLLoader::OnUploadProgress(
...
void CORSURLLoader::OnReceiveCachedMetadata(
    const std::vector<uint8_t>& data) override {
  forwarding_client_->OnReceiveCachedMetadata(data);
}

void CORSURLLoader::OnTransferSizeUpdated(int32_t transfer_size_diff) override {
...
void CORSURLLoader::OnComplete(
    const ResourceRequestCompletionStatus& status) override {
