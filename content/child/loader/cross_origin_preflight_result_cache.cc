...
bool ParseAccessControlMaxAge(const std::string& string,
  ...
  // TODO(hintzed): Does the above FIXME still hold? (I think
  // IteratorRangeToNumber handles +/-)
...
template <typename StringSet>
bool ParseAccessControlAllowList(const std::string& string, StringSet& set) {
...
CrossOriginPreflightResultCacheItem::CrossOriginPreflightResultCacheItem(
    ...
    : absolute_expiry_time_(0),
      credentials_(blink::WebCORS::ShouldTreatCredentialsModeAsInclude(
...
bool CrossOriginPreflightResultCacheItem::Parse(
  ...
  head.headers.get()->GetNormalizedHeader("Access-Control-Allow-Methods",
                                          &allowed_methods);
  ...
  head.headers.get()->GetNormalizedHeader("Access-Control-Allow-Headers",
                                          &allowed_headers);
  ...
  unsigned expiry_delta;
  std::string max_age;
  head.headers.get()->GetNormalizedHeader("Access-Control-Max-Age", &max_age);
...
bool CrossOriginPreflightResultCacheItem::AllowsCrossOriginMethod(
  ...
  if (methods_.find(method) != methods_.end() ||
      blink::WebCORS::IsCORSSafelistedMethod(
          blink::WebString::FromASCII(method)))
    return true;
...
bool CrossOriginPreflightResultCacheItem::AllowsCrossOriginHeaders(
  ...
  for (net::HttpRequestHeaders::Iterator it(request_headers); it.GetNext();) {
    blink::WebString webstring_name = blink::WebString::FromASCII(it.name());
    blink::WebString webstring_value = blink::WebString::FromASCII(it.value());

    if (headers_.find(it.name()) == headers_.end() &&
        !blink::WebCORS::IsCORSSafelistedHeader(webstring_name,
                                                webstring_value) &&
        !blink::WebCORS::IsForbiddenHeaderName(webstring_name)) {
...
bool CrossOriginPreflightResultCacheItem::AllowsRequest(
  ...
  if (!credentials_ &&
      blink::WebCORS::ShouldTreatCredentialsModeAsInclude(
          static_cast<blink::WebURLRequest::FetchCredentialsMode>(
              credentials_mode)))
    return false;
