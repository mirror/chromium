...
void SyncLoadContext::StartAsyncWithWaitableEvent(
  ...
  context->request_id_ = context->resource_dispatcher_->StartAsync(
      std::move(request), routing_id, nullptr, frame_origin,
...
SyncLoadContext::SyncLoadContext(
  ...
  // Constructs a new ResourceDispatcher specifically for this thread.
...
void SyncLoadContext::OnReceivedResponse(const ResourceResponseInfo& info) {
  // Copies fields of the SyncLoadResponse's base class from the final response.
  *static_cast<ResourceResponseInfo*>(response_) = info;
}
