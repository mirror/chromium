...
class ChildHistogramFactoryImpl : public content::mojom::ChildHistogramFactory {
 public:
  ChildHistogramFactoryImpl();
  ~ChildHistogramFactoryImpl() override;

  static void Create(content::mojom::ChildHistogramFactoryRequest);

 private:
  // content::mojom::ChildHistogramFactory implementation.
  using ChildHistogramCallback =
      content::mojom::ChildHistogramFactory::CreateChildHistogramCallback;

  void CreateChildHistogram(mojo::ScopedSharedBufferHandle memory_handle,
                            ChildHistogramCallback callback) override;
};
