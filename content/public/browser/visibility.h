// Copyright 2017 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef CONTENT_PUBLIC_BROWSER_VISIBILITY_H_
#define CONTENT_PUBLIC_BROWSER_VISIBILITY_H_

namespace content {

enum class Visibility {
  // The view is visible.
  VISIBLE,
  // The view is obscured by other views.
  // Note: A view without full occlusion tracking may be VISIBLE when it is
  // occluded.
  OCCLUDED,
  // The view is hidden (e.g. belongs to a minimized window, doesn't belong to a
  // window).
  HIDDEN,
  // Note: A view in the OCCLUDED or HIDDEN state can still be captured (e.g.
  // for screenshots or mirroring).
};

}  // namespace content

#endif  // CONTENT_PUBLIC_BROWSER_VISIBILITY_H_
