...
class CONTENT_EXPORT RenderProcessHost : public IPC::Sender,
 ...
 public:
  ...
  // Starts a local RTC event log for each peerconnection on the render process.
  // A base_path must be supplied, which will be extended to include
  ...
  virtual bool StartLocalRtcEventLogging(const base::FilePath& base_path) = 0;

  // Stops local recording of RTC events log for all peerconnections on the
  // render process. If no recording was in progress, this call will return
  // false.
  virtual bool StopLocalRtcEventLogging() = 0;
