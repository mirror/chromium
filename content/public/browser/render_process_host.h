...
class CONTENT_EXPORT RenderProcessHost : public IPC::Sender,
 ...
 public:
  ...
  // only happen if the count matches the active view count. If |allow_unsafe|
  // is false and this renderer has any RenderViews with unload handlers, then
  // this function does nothing. Otherwise, the function will ingnore checking
  // for those handlers. Returns true if it was able to do fast shutdown.
  virtual bool FastShutdownIfPossible(size_t page_count = 0,
                                      bool allow_unsafe = false) = 0;
