...
class WebInterfaceBrokerBuilder {
 public:
  ...
  template <typename Interface>
  void AddInterface(FrameInterfaceBinder<Interface> frame_binder,
  ...
  template <typename Interface>
  void AddInterface(OriginInterfaceBinder<Interface> origin_binder,
                    std::unique_ptr<WebInterfaceFilter> filter);
...
void WebInterfaceBrokerBuilder::AddInterface(
    ...
    std::unique_ptr<WebInterfaceFilter> filter) {
  ForwardInterfaceToService(Interface::Name_, service_name, std::move(filter));
}
