...
struct CONTENT_EXPORT BackgroundFetchResult {
  ...
  // reported via |BackgroundFetchDelegate::Client::OnDownloadFailed|.
  enum FailureReason {
    // None of below failures occurred, although the fetch could have failed
    // with an error code such as 404. In such a case
    // |BackgroundFetchDelegate::Client::OnDownloadComplete| would
    // be called instead.
    NONE,
