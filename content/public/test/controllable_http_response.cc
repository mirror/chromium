...
ControllableHttpResponse::ControllableHttpResponse(
  ...
  // Intercept only the first request with the matching |relative_url|;
  auto request_handler =
      [](base::WeakPtr<ControllableHttpResponse> controller,
         scoped_refptr<base::SingleThreadTaskRunner> controller_task_runner,
         const std::unique_ptr<bool>& available,
         const std::string& relative_url,
  ...
  embedded_test_server->RegisterRequestHandler(
      base::Bind(request_handler, weak_ptr_factory_.GetWeakPtr(),
                 base::ThreadTaskRunnerHandle::Get(),
                 std::make_unique<bool>(true), relative_url));
}
...
void ControllableHttpResponse::WaitForRequest() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  DCHECK_EQ(INITIAL, state_) << "WaitForRequest() called twice.";
  loop_.Run();
  DCHECK(embedded_test_server_task_runner_);
  DCHECK(send_);
  DCHECK(done_);
  state_ = READY_TO_SEND_DATA;
}

void ControllableHttpResponse::Send(const std::string& bytes) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  DCHECK_EQ(READY_TO_SEND_DATA, state_) << "Send() called without any opened "
                                           "connection. Did you call "
                                           "WaitForRequest()?";
  base::RunLoop loop;
...
void ControllableHttpResponse::Done() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  DCHECK_EQ(READY_TO_SEND_DATA, state_) << "Done() called without any opened "
                                           "connection. Did you call "
                                           "WaitForRequest()?";
