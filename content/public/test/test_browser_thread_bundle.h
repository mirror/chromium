...
// TestBrowserThreadBundle allows usage of these APIs within its scope:
// - Same APIs as base::test::ScopedTaskEnvironment.
// - content::BrowserThread.
//
// Only tests that need the BrowserThread API should instantiate a
// TestBrowserThreadBundle. Use base::test::ScopedTaskEnvironment when possible.
//
// By default, the BrowserThread::UI and BrowserThread::IO will be backed by a
// single shared MessageLoop on the main thread. If a test truly needs
// BrowserThread::IO tasks to run on a separate thread, it can pass the
// REAL_IO_THREAD option to the constructor. TaskScheduler tasks always run on
// dedicated threads.
//
