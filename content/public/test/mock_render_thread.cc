...
MockRenderThread::TakeInitialInterfacesRequestForFrame(int32_t routing_id) {
  auto it = frame_routing_id_to_initial_interfaces_requests_.find(routing_id);
  if (it == frame_routing_id_to_initial_interfaces_requests_.end())
    return nullptr;
  auto interfaces_request = std::move(it->second);
  frame_routing_id_to_initial_interfaces_requests_.erase(it);
  return interfaces_request;
}
...
void MockRenderThread::OnCreateChildFrame(
  ...
  *new_render_frame_id = new_frame_routing_id_++;
  ...
  frame_routing_id_to_initial_interfaces_requests_.emplace(
      *new_render_frame_id, mojo::MakeRequest(&interfaces_provider));
  *new_interfaces = interfaces_provider.PassInterface().PassHandle().release();
...
void MockRenderThread::OnCreateWindow(
  ...
  frame_routing_id_to_initial_interfaces_requests_.emplace(
      new_window_main_frame_routing_id_,
      mojo::MakeRequest(&reply->main_frame_interfaces));
