...
void NavigationSimulator::SetIsBackNavigation(bool is_back_navigation) {
  if (is_back_navigation)
    SetSessionHistoryOffset(-1);
}

void NavigationSimulator::SetIsForwardNavigation(bool is_forward_navigation) {
  if (is_forward_navigation)
    SetSessionHistoryOffset(-1);
}

void NavigationSimulator::SetSessionHistoryOffset(int session_history_offset) {
  CHECK_EQ(INITIALIZATION, state_) << "Cannot set navigation to be"
                                      "back/forward after the"
                                      "navigation has started";
  CHECK(browser_initiated_)
      << "Cannot set navigation to be back/forward for non"
         "browser-intiated navigations";
  CHECK_EQ(ReloadType::NONE, reload_type_)
      << "Cannot set navigation to be back/forward for"
         "reload navigations";
  if (session_history_offset) {
