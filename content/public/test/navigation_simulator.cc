...
void NavigationSimulator::Start() {
  CHECK_EQ(state_, INITIALIZATION)
...
void NavigationSimulator::PrepareForCommit() {
  CHECK_LE(state_, STARTED) << "NavigationSimulator::PrepareForCommit can only "
                               "be called once, and cannot be called after "
                               "NavigationSimulator::Fail";
  CHECK_EQ(0, num_did_finish_navigation_called_)
      << "NavigationSimulator::PrepareForCommit cannot be called after the "
         "navigation has finished";
...
RenderFrameHost* NavigationSimulator::GetFinalRenderFrameHost() {
  CHECK(state_ == READY_TO_COMMIT || state_ == FINISHED);
