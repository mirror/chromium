...
class NavigationSimulator : public WebContentsObserver {
 public:
  ...
  // Sets the InterfaceProvider interface request to provide as an argument to
  // DidCommitProvisionalLoad for non-same-document navigations. If not called,
  // a stub will be passed in (with no interface requests ever arriving).
  //
  // The interface connection would normally be created by the RenderFrame, with
  // its client end bound to |remote_interfaces_|, to allow the new document to
  // allow it to access access services exposed by the RenderFrameHost.
