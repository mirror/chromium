...
// Returns an InterfaceProvider that will not actually service any  interface
// requests but is safe to call into.
...
void RenderViewTest::SetUp() {
  ...
  view_params->main_frame_interfaces = CreateDeadEndInterfaceProvider();
...
void RenderViewTest::OnLeakDetectionComplete(const Result& result) {
  ...
  EXPECT_EQ(0u, result.number_of_live_suspendable_objects);
