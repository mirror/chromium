...
public class AppWebMessagePort implements MessagePort {
    ...
    private static class MessageHandler extends Handler implements MessageReceiver {
        ...
        public void handleMessage(android.os.Message msg) {
            if (msg.what == MESSAGE_RECEIVED) {
                ...
                if (mMessageCallback == null) {
                    Log.w(TAG,
                            "No handler set for port, dropping message " + message.decodedMessage);
                    return;
                }
                mMessageCallback.onMessage(message.decodedMessage, message.ports);
                return;
            }
        ...
        public boolean accept(Message mojoMessage) {
            try {
                ...
                portMsg.decodedMessage = message;
                portMsg.ports = ports;
    ...
    public void setMessageCallback(MessageCallback messageCallback, Handler handler) {
        ...
        mHandler = new MessageHandler(
                handler == null ? Looper.getMainLooper() : handler.getLooper(), messageCallback);
        mConnector.setIncomingMessageReceiver(mHandler);
        mConnector.start();
    }
