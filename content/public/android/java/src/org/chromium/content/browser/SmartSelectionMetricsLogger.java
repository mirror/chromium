...
public class SmartSelectionMetricsLogger implements SelectionMetricsLogger {
    ...
    public void logSelectionModified(
        ...
        logEvent(eventSelectionModified(
                startOffset, endOffset, (new TextClassification.Builder()).build()));
        logEvent(eventSelectionModified(
                startOffset, endOffset, (new TextSelection.Builder(0, 0)).build()));
    }

    @Override
    public void logSelectionAction(int start, int end, int action, SelectionClient.Result result) {
        if (!logConditionCheck()) {
            return;
        }
        Log.d(TAG, "logSelectionAction");
        logEvent(eventSelectionAction(start, end, action));
        logEvent(eventSelectionAction(
                start, end, action, (new TextClassification.Builder()).build()));
    }
    ...
    // Left inclusive, right exclusive. Check if [al, ar) and [bl, br) overlap
