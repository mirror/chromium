...
public class SmartSelectionMetricsLogger implements SelectionMetricsLogger {
    ...
    @VisibleForTesting
    public void updateSelectionState(String selectionText, int startOffset, boolean force) {
        int endOffset = startOffset + selectionText.length();
        ...
        if (contains(mPreviousSelectionStartOffset, mPreviousSelectionStartOffset, startOffset,
                    endOffset)) {
            return;
        }
    ...
    private void getWordDelta(int start, int end) {
    ...
    private int countWordsBackward(int from, int originalOffset) {
    ...
    private int countWordsForward(int from, int originalOffset) {
        assert from <= originalOffset;
        int wordCount = 0;
        int offset = from;
        while (offset < originalOffset) {
    ...
    private boolean contains(int al, int ar, int bl, int br) {
        return al <= bl && br <= ar;
    }
}
