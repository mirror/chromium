...
public class ChildProcessLauncherHelper {
    ...
    // True if a connection is available for this launcher (mConnection being set when the
    // connection is established and might still be null). If false it means no connection was
        ...
        try {
            ...
            if (sSpareSandboxedConnection != null) {
                mConnection = sSpareSandboxedConnection.getConnection(
                        mCreationParams, true /* sandboxed */, startCallback);
                if (mConnection != null) {
            ...
            allocateAndSetupConnection(startCallback, true /* retryIfNone */);
        } finally {
    ...
    private boolean allocateAndSetupConnection(
            final ChildProcessConnection.StartCallback startCallback, boolean retryIfNone) {
        if (mConnection == null) {
            ...
            System.out.println("** JAY ** allocateAndSetupConnection");
            ...
            if (mConnection == null) {
                if (!retryIfNone) {
                    Log.d(TAG, "Failed to allocate a child connection, no retrying.");
                    return false;
                }
        ...
        assert mConnection != null;
        System.out.println("** JAY ** allocateAndSetupConnection connection=" + mConnection);
