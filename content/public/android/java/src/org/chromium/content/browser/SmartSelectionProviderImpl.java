...
/**
 * The implementation if SmartSelectionProvider. Talks to TextClassificationManager API which is
 * part of Android O SDK.
 * TODO(timav): Move the implementation into SmartSelectionProvider and remove this class after we
 * roll upstream to this SDK.
 */
public class SmartSelectionProviderImpl implements SmartSelectionProvider {
    ...
    private SmartSelectionProvider.ResultCallback mResultCallback;
    ...
    public SmartSelectionProviderImpl(
        ...
        mFailureResponseRunnable = new Runnable() {
            @Override
            public void run() {
                mResultCallback.onClassified(new SmartSelectionProvider.Result());
            }
        };
    }
    ...
    // TODO(timav): Use |TextClassifier| instead of |Object| after we upstream this class.
    @Override
    public void setTextClassifier(Object textClassifier) {
        mTextClassifier = (TextClassifier) textClassifier;
    }
    ...
    private class ClassificationTask extends AsyncTask<Void, Void, SmartSelectionProvider.Result> {
        ...
        ClassificationTask(TextClassifier classifier, @RequestType int requestType,
                CharSequence text, int start, int end, Locale[] locales) {
            Log.v(TAG, "ClassificationTask ctor");
        ...
        @Override
        protected SmartSelectionProvider.Result doInBackground(Void... params) {
            Log.v(TAG, "ClassificationTask.doInBackground");
            ...
            if (mRequestType == SUGGEST_AND_CLASSIFY) {
                ...
                if (isCancelled()) return new SmartSelectionProvider.Result();
            }
        ...
        private SmartSelectionProvider.Result makeResult(
                int start, int end, TextClassification tc) {
            SmartSelectionProvider.Result result = new SmartSelectionProvider.Result();
        ...
        @Override
        protected void onPostExecute(SmartSelectionProvider.Result result) {
