...
public class SelectionPopupControllerTest {
    ...
    public void testHandleObserverSelectionHandle() {
        HandleObserver handleObserver = Mockito.mock(HandleObserver.class);
        InOrder order = inOrder(handleObserver);
        mController.setHandleObserver(handleObserver);

        // Selection handles shown.
        mController.onSelectionEvent(
                SelectionEventType.SELECTION_HANDLES_SHOWN, 0, 0, 0, 0, 0.f, 0.f);
        // Selection handles moved, but drag wasn't triggered, so no handleDragStarted call.
        mController.onSelectionEvent(
                SelectionEventType.SELECTION_HANDLES_MOVED, 0, 0, 0, 0, 0.f, 0.f);
        order.verify(handleObserver, never()).handleDragStarted(anyFloat(), anyFloat());

        // Selection handle drag started.
        mController.onSelectionEvent(
                SelectionEventType.SELECTION_HANDLE_DRAG_STARTED, 0, 0, 0, 0, 0.f, 0.f);
        order.verify(handleObserver).handleDragStarted(0.f, 0.f);

        // Moving.
        mController.onSelectionEvent(
                SelectionEventType.SELECTION_HANDLES_MOVED, 0, 0, 0, 0, 5.f, 5.f);
        order.verify(handleObserver).handleDragStarted(5.f, 5.f);
    ...
    public void testHandleObserverInsertionHandle() {
        ...
        // Insertion handle moved, but drag wasn't triggered, so no handleDragStarted call.
        mController.onSelectionEvent(
