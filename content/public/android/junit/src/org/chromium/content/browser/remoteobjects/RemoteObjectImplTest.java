...
public final class RemoteObjectImplTest {
    ...
    private static class RemoteInvocationResultCapture
        ...
        public RemoteInvocationResult get() {
            Assert.assertNotNull("Response callback must be invoked.", mResult);
            return mResult;
        }
    ...
    public void testHasMethodWithSafeAnnotationClass() {
        ...
        remoteObject.hasMethod(
                "privateExposedMethod", (Boolean result) -> hasMethodResult.value = result);
        Assert.assertEquals(false, hasMethodResult.value);

        // This method is not annotated; it should not be exposed.
        hasMethodResult.reset();
        remoteObject.hasMethod("hiddenMethod", (Boolean result) -> hasMethodResult.value = result);
        Assert.assertEquals(false, hasMethodResult.value);
