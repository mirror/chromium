...
public class ChildProcessLauncherTest {
    ...
    private void testProcessLauncher(final ChildProcessLauncherFactory launcherFactory) {
        ...
        final ConditionVariable onBeforeConnectionAllocatedCondition = new ConditionVariable();
        final AtomicBoolean onBeforeConnectionAllocatedCalled = new AtomicBoolean();
        final ConditionVariable onBeforeConnectionSetupCondition = new ConditionVariable();
        final AtomicBoolean onBeforeConnectionSetupCalled = new AtomicBoolean();
        final ConditionVariable onConnectionEstablishedCondition = new ConditionVariable();
        final AtomicBoolean onConnectionEstablishedCalled = new AtomicBoolean();
        final ConditionVariable onConnectionLostCondition = new ConditionVariable();
        final AtomicBoolean onConnectionLostCalled = new AtomicBoolean();
        ...
        final ChildProcessLauncher.Delegate delegate = new ChildProcessLauncher.Delegate() {
            ...
            public void onBeforeConnectionAllocated(Bundle serviceBundle) {
                ...
                Assert.assertFalse(onBeforeConnectionAllocatedCalled.get());
                onBeforeConnectionAllocatedCalled.set(true);
                serviceBundle.putString(EXTRA_SERVICE_PARAM, EXTRA_SERVICE_PARAM_VALUE);
                onBeforeConnectionAllocatedCondition.open();
            }

            @Override
            public void onBeforeConnectionSetup(Bundle connectionBundle) {
                connectionBundle.putString(EXTRA_CONNECTION_PARAM, EXTRA_CONNECTION_PARAM_VALUE);
                Assert.assertFalse(onBeforeConnectionSetupCalled.get());
                onBeforeConnectionSetupCalled.set(true);
                onBeforeConnectionSetupCondition.open();
            }

            @Override
            public void onConnectionEstablished(ChildProcessConnection connection) {
                Assert.assertFalse(onConnectionEstablishedCalled.get());
                onConnectionEstablishedCalled.set(true);
                onConnectionEstablishedCondition.open();
            }

            @Override
            public void onConnectionLost(ChildProcessConnection connection) {
                Assert.assertFalse(onConnectionLostCalled.get());
                onConnectionLostCalled.set(true);
                onConnectionLostCondition.open();
            }
        };
        ...
        if (!launcherFactory.isConnectionProvided()) {
            Assert.assertTrue(
                    onBeforeConnectionAllocatedCondition.block(CONDITION_WAIT_TIMEOUT_MS));
        }

        Assert.assertTrue(onBeforeConnectionSetupCondition.block(CONDITION_WAIT_TIMEOUT_MS));
        ...
        Assert.assertTrue(onConnectionEstablishedCondition.block(CONDITION_WAIT_TIMEOUT_MS));
        ...
        Assert.assertTrue(onConnectionLostCondition.block(CONDITION_WAIT_TIMEOUT_MS));
    }
