...
public class WebContentsAccessibilityTest {
    ...
    private static class AccessibilityEventCallbackHelper extends CallbackHelper {
        AccessibilityEventCallbackHelper(View view) {
            final AccessibilityEventCallbackHelper helper = this;
    ...
    /**
     * Helper method to call AccessibilityNodeInfo.getChildId using reflection, since it's hidden.
     */
    private int getChildId(AccessibilityNodeInfo node, int index) {
        try {
            ...
            long childId = (long) getChildIdMethod.invoke(node, new Integer(index));
            return (int) (childId & 0xffff); // The lower bits are the virtual view ID.
        } catch (Exception ex) {
            Assert.fail("Unable to call AccessibilityNodeInfo.getChildId: " + ex.toString());
            return 0;
        }
    }
