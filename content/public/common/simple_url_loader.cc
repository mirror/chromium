...
class SimpleURLLoaderImpl : public SimpleURLLoader,
 ...
 private:
  ...
  // Pre-request state. Always non-null, but re-created on redirect.
...
int SimpleURLLoaderImpl::NetError() const {
  ...
  DCHECK_NE(net::ERR_IO_PENDING, state_->net_error);
  return state_->net_error;
}

const ResourceResponseHead* SimpleURLLoaderImpl::ResponseInfo() const {
  // Should only be called once the request is compelete.
  DCHECK(state_->finished);
  return state_->response_info.get();
}

void SimpleURLLoaderImpl::OnBodyHandlerDone(net::Error error,
                                            int64_t received_body_size) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  DCHECK(state_->body_started);
  DCHECK(!state_->body_completed);
  ...
  state_->body_completed = true;
  state_->received_body_size = received_body_size;
  MaybeComplete();
}
...
void SimpleURLLoaderImpl::OnReceiveResponse(
  ...
  state_->response_info = base::MakeUnique<ResourceResponseHead>(response_head);
...
void SimpleURLLoaderImpl::OnComplete(
  ...
  DCHECK(!state_->finished);
  DCHECK(!state_->request_completed);

  // Close pipes to ignore any subsequent close notification.
  client_binding_.Close();
  url_loader_.reset();

  state_->request_completed = true;
  state_->expected_body_size = status.decoded_body_length;
  state_->net_error = status.error_code;
  // If |status| indicates success, but the body pipe was never received, the
  // URLLoader is violating the API contract.
  if (state_->net_error == net::OK && !state_->body_started)
    state_->net_error = net::ERR_UNEXPECTED;
...
void SimpleURLLoaderImpl::OnConnectionError() {
  ...
  DCHECK(!state_->finished);
  DCHECK(!state_->request_completed);
  DCHECK_EQ(net::ERR_IO_PENDING, state_->net_error);

  state_->request_completed = true;
  state_->net_error = net::ERR_FAILED;
...
void SimpleURLLoaderImpl::MaybeComplete() {
  ...
  // read more data.
  if (state_->body_started && !state_->body_completed)
    return;
  ...
  // on the body pipe.
  if (state_->net_error == net::ERR_NETWORK_CHANGED && remaining_retries_ > 0 &&
  ...
  if (state_->net_error == net::OK &&
      state_->expected_body_size != state_->received_body_size) {
    if (state_->expected_body_size > state_->received_body_size) {
  ...
  FinishWithResult(state_->net_error);
}
