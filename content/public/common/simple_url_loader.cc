...
#include "content/public/common/simple_url_loader_stream_handler.h"
...
class SimpleURLLoaderImpl : public SimpleURLLoader,
 ...
 public:
  ...
  void DownloadToTempFile(
      ...
      DownloadToFileCompleteCallback download_to_file_complete_callback,
      int64_t max_body_size) override;
  void DownloadAsStream(network::mojom::URLLoaderFactory* url_loader_factory,
...
class BodyReader {
 public:
  class Delegate {
   public:
    ...
    // closed.  A return value of net::ERR_IO_PENDING means that the BodyReader
    // should stop reading, and not call OnDone(), until its Resume() method is
    // called.
    //
    // It's safe to delete the Delegate during this call.
 ...
 private:
  ...
  // Set to an error code if Delegate::OnDataRead() returns ERR_IO_PENDING, but
  // there was also a pending error (Generally, length limit exceeded).
...
class DownloadAsStreamBodyHandler : public BodyHandler,
 ...
 public:
  ...
  void NotifyConsumerOfCompletion(bool destroy_results) override {
    body_reader_.reset();
    stream_handler_->OnComplete(simple_url_loader()->NetError() == net::OK);
  }

  void PrepareToRetry(base::OnceClosure retry_callback) override {
    body_reader_.reset();
    stream_handler_->OnRetry(std::move(retry_callback));
  }
 ...
 private:
  ...
  net::Error OnDataRead(uint32_t length, const char* data) override {
    ...
    base::WeakPtr<DownloadAsStreamBodyHandler> weak_this(
        weak_ptr_factory_.GetWeakPtr());
    stream_handler_->OnDataReceived(
  ...
  void Resume() {
    // Can't resume before OnDataRead has been called.
  ...
  SimpleURLLoaderStreamHandler* stream_handler_;
...
void SimpleURLLoaderImpl::DownloadAsStream(
  ...
  body_handler_ = std::make_unique<DownloadAsStreamBodyHandler>(
      this, stream_handler, max_body_size);
  Start(url_loader_factory);
}
