 ...
 public:
  class Delegate {
   public:
    ...
    // Called when the pipe is closed either due to OnDataRead() returning an
    // error, the size limit being reached, or an error being returned by
    // OnDataRead. |error| is net::OK if the pipe was closed, or an error value
    // otherwise. It is safe to delete the BodyReader during this callback.
...
class BodyHandler {
 public:
  ...
  // SimpleURLLoaderImpl::OnBodyHandlerDone() once either when the body pipe is
  // closed or when an error occurs, like a write to a file fails.
  ...
  // callbacks. If |destroy_results| is true, any received data should be
  // destroyed instead of being sent to the consumer
...
class SimpleURLLoaderImpl : public SimpleURLLoader,
 ...
 public:
  ...
  // is not net::OK,
  // some error occurred reading or consuming the body. If it is net::OK, the
  // pipe was closed and all data received was successfully handled. This could
  // indicate an error, concellation, or completion. To determine which case
  // this is, the size will also be compared to the size reported in
