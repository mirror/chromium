...
//
// StringStreamer is a class to stream a string to the network service, rather
// than to copy it all at once.
...
class StringUploadDataPipeGetter : public network::mojom::DataPipeGetter {
 public:
  explicit StringUploadDataPipeGetter(const std::string& upload_string)
      : data_pipe_getter_binding_(this), upload_string_(upload_string) {}
  ~StringUploadDataPipeGetter() override = default;

  // Returns a new DataPipeGetter for reading the string the streamer was
  // created with, destroying any previously active one, and any previously
  // active data pipe as well.
  network::mojom::DataPipeGetterPtr CreateDataPipeGetter() {
    data_pipe_getter_binding_.Close();

    // Close any previous body pipe, to avoid confusion between the two, if this
    // is a retry.
    ResetBodyPipe();

    network::mojom::DataPipeGetterPtr data_pipe_getter;
    data_pipe_getter_binding_.Bind(mojo::MakeRequest(&data_pipe_getter));
 ...
 private:
  ...
  mojo::Binding<network::mojom::DataPipeGetter> data_pipe_getter_binding_;
...
void SimpleURLLoaderImpl::StartRequest(
  ...
  if (string_upload_data_pipe_getter_) {
    ...
    resource_request_->request_body->AppendDataPipe(
        string_upload_data_pipe_getter_->CreateDataPipeGetter());
  }
