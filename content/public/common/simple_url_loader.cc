...
class BodyHandler {
 public:
  ...
  // Called before retrying a request. Any pending callbacks should be
  // cancelled, and the data pipe closed. |retry_callback| should be invoked on
  // the SimpleURLLoader's sequence to retry the request. Callback may
  // synchronously call NotifyConsumerOfCompletion().
...
class SaveToFileBodyHandler : public BodyHandler {
 public:
  ...
  ~SaveToFileBodyHandler() override {
    ...
    if (file_writer_) {
      // |file_writer_| is only non-null at this point if the request hasn't
      // completed. Destroy any partially downloaded file.
  ...
  void PrepareToRetry(base::OnceClosure retry_callback) override {
    // If currently writing to a file, stop doing so and invoke |retry_callback|
    // only once any created file has been destroyed.
    if (file_writer_) {
      // Prevent the FileWriter from calling OnDone().
      weak_ptr_factory_.InvalidateWeakPtrs();
  ...
  class FileWriter : public BodyReader::Delegate {
   public:
    ...
    ~FileWriter() override {
      ...
      // File should have been closed before destruction, either in OnDone(), or
      // by DeleteFileOnFileSequence(). Both may have been called.
   ...
   private:
    ...
    // True if a file was successfully created. Set to false if the file is
    // destroyed.
...
class SimpleURLLoaderImpl : public SimpleURLLoader,
 ...
 private:
  ...
  // Starts a request. Wrapper around StartInternal, but can be bound to a
  // callback that owns its |url_loader_factory|.
  void Retry(const ResourceRequest& resource_request,
             mojom::URLLoaderFactoryPtr* url_loader_factory,
             const net::NetworkTrafficAnnotationTag& annotation_tag);

  // Prepares to retry a request. Request is actually started when the reused
  // BodyHandler invokes
  // StartInternal via a callback.
  void PrepareToRetry();
  ...
  // Contains bound information needed to retry a request, if retries are
  // enabled. Must be destroyed on the thread the SimpleURLLoader is running on.
  base::Closure retry_callback_;
...
void SimpleURLLoaderImpl::PrepareToStart(
  ...
  if (remaining_retries_ > 0) {
    // Clone the URLLoaderFactory, to avoid any dependence on its lifetime.
    std::unique_ptr<content::mojom::URLLoaderFactoryPtr> url_loader_factory_ptr;
    url_loader_factory->Clone(mojo::MakeRequest(url_loader_factory_ptr.get()));

    retry_callback_ = base::Bind(
        &SimpleURLLoaderImpl::Retry, weak_ptr_factory_.GetWeakPtr(),
        resource_request, base::Owned(url_loader_factory_ptr.release()),
        annotation_tag);
  }
...
void SimpleURLLoaderImpl::Retry(
    ...
    mojom::URLLoaderFactoryPtr* url_loader_factory,
    const net::NetworkTrafficAnnotationTag& annotation_tag) {
  StartInternal(resource_request, url_loader_factory->get(), annotation_tag);
}

void SimpleURLLoaderImpl::PrepareToRetry() {
  Initialize();
  body_handler_->PrepareToRetry(retry_callback_);
}
...
void SimpleURLLoaderImpl::MaybeComplete() {
  ...
  // Retry on network change errors.
