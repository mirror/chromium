...
class CONTENT_EXPORT NetworkConnectionTracker
 ...
 public:
  ...
  // which case, |callback| will be called on NetworkConnectionTracker's thread
  // when connection type is ready. Please also refer to
  // net::NetworkChangeNotifier::GetConnectionType() for documentation. This
 ...
 private:
  ...
  // Protect access to |connection_type_|, |initial_connection_type_received_|
  // and |connection_type_callbacks_|.
  base::Lock lock_;

  // Whether the initial connection type has been received.
  bool initial_connection_type_received_;

  // Saves the user callback if GetConnectionType() cannot complete
  // synchronously.
