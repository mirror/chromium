...
scoped_refptr<ResourceDevToolsInfo> ResourceDevToolsInfo::FromURLRequest(
  ...
  net::HttpRequestHeaders::HeaderVector request_headers;
  std::string request_line;

  url_request->GetWireRequestHeaders(&request_headers, &request_line);
  for (const auto& pair : request_headers)
    info->request_headers.push_back(std::make_pair(pair.key, pair.value));
  if (report_headers_text) {
    info->request_headers_text =
        request_line + net::HttpRequestHeaders::ToString(request_headers);
  }
