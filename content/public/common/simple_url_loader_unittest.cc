...
class SimpleLoaderTestHelper {
 public:
  ...
  // Same as StartRequest, but takes a URL isntead of a ResourceRequest.
  void StartRequestForURL(mojom::URLLoaderFactory* url_loader_factory,
                          const GURL& url,
                          int64_t max_body_size = -1) {
    ResourceRequest resource_request;
    resource_request.url = url;
    StartRequest(url_loader_factory, resource_request);
  }
...
// Check the case where a URLLoader with a closed Mojo pipe was passed in.
TEST_P(SimpleURLLoaderTest, ClosedURLLoader) {
...
class MockURLLoaderFactory : public mojom::URLLoaderFactory {
 public:
  ...
  void Clone(mojom::URLLoaderFactoryRequest request) override {
    if (clone_should_fail_)
      return;
    binding_set_.AddBinding(this, std::move(request));
  }

  void set_clone_should_fail(bool clone_should_fail) {
    clone_should_fail_ = clone_should_fail;
  }
  ...
  void RunTest(SimpleLoaderTestHelper* test_helper,
    ...
    test_helper->StartRequestForURL(factory.get(), url, max_body_size);
...
TEST_P(SimpleURLLoaderTest, RetryWithUnboundFactory) {
  ...
  loader_factory.set_clone_should_fail(true);
