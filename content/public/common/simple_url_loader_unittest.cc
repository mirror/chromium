...
class MockURLLoaderFactory : public mojom::URLLoaderFactory {
 public:
  ...
  // Sets whether Clone() should just destroy a request rather than binding it.
  // Should be indistinguishable from the consumer from the case where the new
  // URLLoaderFactory is disconnected after being clones.
  void set_clone_destroys_request(bool clone_destroys_request) {
    clone_destroys_request_ = clone_destroys_request;
  }
 ...
 private:
  ...
  bool clone_destroys_request_ = false;
...
TEST_P(SimpleURLLoaderTest, RetryOn5xx) {
  ...
  struct TestCase {
    ...
    uint32_t expected_num_requests;
      ...
      // As many retries allows as 5xx errors.
...
TEST_P(SimpleURLLoaderTest, RetryOnNetworkChange) {
  ...
  struct TestCase {
    ...
    uint32_t expected_num_requests;
      ...
      // As many retries allows as network changes.
...
TEST_P(SimpleURLLoaderTest, RetryWithDisconnectedURLLoaderFactory) {
  MockURLLoaderFactory loader_factory(&scoped_task_environment_);
  loader_factory.AddEvents({TestLoaderEvent::kResponseCompleteNetworkChanged});
  loader_factory.set_clone_destroys_request(true);
  test_helper()->simple_url_loader()->SetRetryOptions(
      1, SimpleURLLoader::RETRY_ON_NETWORK_CHANGE);
  test_helper()->RunRequestForURL(&loader_factory, GURL("foo://bar/"));
