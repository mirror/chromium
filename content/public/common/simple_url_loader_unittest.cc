 ...
 public:
  SimpleURLLoaderTestBase()
    ...
    test_server_.RegisterRequestHandler(
        base::Bind(&FailOnceThenEchoBody, base::Owned(new bool(false))));
  ...
  static base::FilePath GetTestFilePath() {
    return base::FilePath(FILE_PATH_LITERAL("content/test/data/title1.html"));
  }
...
TEST_P(SimpleURLLoaderTest, UploadFile) {
  ...
  // Also make sure the correct method was sent.
...
TEST_P(SimpleURLLoaderTest, UploadNonexistantFile) {
  ...
  test_helper->simple_url_loader()->AttachFileForUpload(
      GetTestFilePath().AppendASCII("/foo"), "text/plain");
  test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
...
// (But after the SimpleURLLoader is started). Main thing this tests is that the
// file is deleted off thread. If it's deleted on the current thread, it will
// assert due to no file IO being allowed on the main thread.
...
// Make sure that when a SimpleURLLoaderHelper is destroyed while holding onto a
// file handle, the file is closed off thread.
