...
class SimpleLoaderTestHelper {
 ...
 private:
  ...
  void DownloadedToFile(const base::FilePath& file_path) {
    ...
    if (file_path.empty()) {
      // File should have been deleted, whether it was created in the first
      // place or not.
      EXPECT_EQ(expect_path_exists_on_error_, base::PathExists(dest_path_));
    } else {
      EXPECT_EQ(dest_path_, file_path);
      EXPECT_TRUE(base::PathExists(dest_path_));
...
TEST_P(SimpleURLLoaderTest, BasicRequest) {
  ...
  test_helper_.RunRequest(url_loader_factory_.get(), resource_request);

  EXPECT_EQ(net::OK, test_helper_.simple_url_loader()->NetError());
  EXPECT_EQ(200, test_helper_.GetResponseCode());
  ASSERT_TRUE(test_helper_.response_body());
  EXPECT_EQ("Expected Response", *test_helper_.response_body());
}
...
TEST_P(SimpleURLLoaderTest, DataURL) {
  ...
  EXPECT_EQ(net::OK, test_helper_.simple_url_loader()->NetError());
  ASSERT_TRUE(test_helper_.simple_url_loader()->ResponseInfo());
  EXPECT_FALSE(test_helper_.simple_url_loader()->ResponseInfo()->headers);
  ASSERT_TRUE(test_helper_.response_body());
  EXPECT_EQ("foo", *test_helper_.response_body());
}
...
TEST_P(SimpleURLLoaderTest, Redirect) {
  ...
  EXPECT_EQ(200, test_helper_.GetResponseCode());
  ASSERT_TRUE(test_helper_.response_body());
  EXPECT_EQ("Echo", *test_helper_.response_body());
}

// Check that no body is returned with an HTTP error response.
TEST_P(SimpleURLLoaderTest, HttpErrorStatusCodeResponse) {
  test_helper_.RunRequestForURL(url_loader_factory_.get(),
                                test_server_.GetURL("/echo?status=400"));

  EXPECT_EQ(net::ERR_FAILED, test_helper_.simple_url_loader()->NetError());
  EXPECT_EQ(400, test_helper_.GetResponseCode());
  EXPECT_FALSE(test_helper_.response_body());
}
...
TEST_P(SimpleURLLoaderTest, HttpErrorStatusCodeResponseAllowed) {
  test_helper_.simple_url_loader()->SetAllowHttpErrorResults(true);
  test_helper_.RunRequestForURL(url_loader_factory_.get(),
                                test_server_.GetURL("/echo?status=400"));
...
TEST_P(SimpleURLLoaderTest, EmptyResponseBody) {
  ...
  EXPECT_EQ(net::OK, test_helper_.simple_url_loader()->NetError());
  EXPECT_EQ(204, test_helper_.GetResponseCode());
  ASSERT_TRUE(test_helper_.response_body());
  // A response body is sent from the NetworkService, but it's empty.
  EXPECT_EQ("", *test_helper_.response_body());
}
...
TEST_P(SimpleURLLoaderTest, BigResponseBody) {
  ...
  EXPECT_EQ(200, test_helper_.GetResponseCode());
  ASSERT_TRUE(test_helper_.response_body());
  EXPECT_EQ(kResponseSize, test_helper_.response_body()->length());
  EXPECT_EQ(std::string(kResponseSize, 'a'), *test_helper_.response_body());
}

TEST_P(SimpleURLLoaderTest, ResponseBodyWithSizeMatchingLimit) {
  const uint32_t kResponseSize = 16;
  test_helper_.RunRequestForURLWithBoundedSize(
      url_loader_factory_.get(),
      test_server_.GetURL(
...
TEST_P(SimpleURLLoaderTest, ResponseBodyWithSizeAboveLimit) {
  ...
  EXPECT_EQ(net::ERR_INSUFFICIENT_RESOURCES,
            test_helper_.simple_url_loader()->NetError());
  EXPECT_FALSE(test_helper_.response_body());
}
...
TEST_P(SimpleURLLoaderTest, ResponseBodyWithSizeAboveLimitPartialResponse) {
  const uint32_t kResponseSize = 16;
  const uint32_t kMaxResponseSize = kResponseSize - 1;
  test_helper_.simple_url_loader()->SetAllowPartialResults(true);
  test_helper_.RunRequestForURLWithBoundedSize(
      url_loader_factory_.get(),
      test_server_.GetURL(
          base::StringPrintf("/response-size?%u", kResponseSize)),
  ...
  EXPECT_EQ(net::ERR_INSUFFICIENT_RESOURCES,
            test_helper_.simple_url_loader()->NetError());
  ASSERT_TRUE(test_helper_.response_body());
  EXPECT_EQ(std::string(kMaxResponseSize, 'a'), *test_helper_.response_body());
  EXPECT_EQ(kMaxResponseSize, test_helper_.response_body()->length());
}
...
TEST_P(SimpleURLLoaderTest, NetErrorBeforeHeaders) {
  ...
  EXPECT_EQ(net::ERR_EMPTY_RESPONSE,
            test_helper_.simple_url_loader()->NetError());
  EXPECT_FALSE(test_helper_.simple_url_loader()->ResponseInfo());
  EXPECT_FALSE(test_helper_.response_body());
}
...
TEST_P(SimpleURLLoaderTest, TruncatedBodyWithPartialResults) {
  ...
  EXPECT_EQ(200, test_helper_.GetResponseCode());
  ASSERT_TRUE(test_helper_.response_body());
  EXPECT_EQ(kTruncatedBody, *test_helper_.response_body());
}
...
TEST_P(SimpleURLLoaderTest, DestroyServiceBeforeResponseStarts) {
  ...
  test_helper_.StartRequest(url_loader_factory_.get(), resource_request);
  network_service_ = nullptr;
  test_helper_.Wait();

  EXPECT_EQ(net::ERR_FAILED, test_helper_.simple_url_loader()->NetError());
  EXPECT_FALSE(test_helper_.response_body());
  ASSERT_FALSE(test_helper_.simple_url_loader()->ResponseInfo());
}
...
TEST_P(SimpleURLLoaderTest, ResponseCompleteBeforeReceivedResponse) {
  ...
  test_helper_.RunRequestForURL(&loader_factory, GURL("foo://bar/"));

  EXPECT_EQ(net::ERR_UNEXPECTED, test_helper_.simple_url_loader()->NetError());
  EXPECT_FALSE(test_helper_.simple_url_loader()->ResponseInfo());
  EXPECT_FALSE(test_helper_.response_body());
}
...
TEST_P(SimpleURLLoaderTest, ErrorAndCloseClientPipeBeforeBodyStarts) {
  ...
  EXPECT_EQ(net::ERR_TIMED_OUT, test_helper_.simple_url_loader()->NetError());
  EXPECT_EQ(200, test_helper_.GetResponseCode());
  EXPECT_FALSE(test_helper_.response_body());
}
...
TEST_P(SimpleURLLoaderTest, SuccessAndCloseClientPipeBeforeBodyComplete) {
  ...
  EXPECT_EQ(200, test_helper_.GetResponseCode());
  ASSERT_TRUE(test_helper_.response_body());
  EXPECT_EQ("a", *test_helper_.response_body());
}
...
TEST_F(SimpleURLLoaderFileTest, OverwriteFile) {
  ...
  ASSERT_EQ(static_cast<int>(junk_data.size()),
            base::WriteFile(test_helper_.dest_path(), junk_data.data(),
                            junk_data.size()));
  ASSERT_TRUE(base::PathExists(test_helper_.dest_path()));
...
TEST_F(SimpleURLLoaderFileTest, FileCreateError) {
  ASSERT_TRUE(base::CreateDirectory(test_helper_.dest_path()));
  ASSERT_TRUE(base::PathExists(test_helper_.dest_path()));

  ResourceRequest resource_request;
  resource_request.url = GURL("data:text/plain,foo");
  // The directory should still exist after the download fails.
  test_helper_.set_expect_path_exists_on_error(true);
  test_helper_.RunRequest(url_loader_factory_.get(), resource_request);

  EXPECT_EQ(net::ERR_ACCESS_DENIED,
            test_helper_.simple_url_loader()->NetError());
  EXPECT_TRUE(test_helper_.simple_url_loader()->ResponseInfo());
  EXPECT_FALSE(test_helper_.response_body());
}
...
TEST_F(SimpleURLLoaderFileTest, DeleteLoaderDuringRequestDestroysFile) {
  struct TestCase {
    bool body_data_read;
    bool body_buffer_closed;
    bool client_pipe_closed;
  };
  // At least one of |body_buffer_closed| and |client_pipe_closed| must be
  // false. Otherwise, the request will have completed successfully.
  const TestCase kTestCases[] = {
      {false /* body_data_read */, false /* body_buffer_closed */,
       false /* client_pipe_closed */},
      {true /* body_data_read */, false /* body_buffer_closed */,
       false /* client_pipe_closed */},
      {false /* body_data_read */, true /* body_buffer_closed */,
       false /* client_pipe_closed */},
      {true /* body_data_read */, true /* body_buffer_closed */,
       false /* client_pipe_closed */},
      {false /* body_data_read */, false /* body_buffer_closed */,
       true /* client_pipe_closed */},
      {true /* body_data_read */, false /* body_buffer_closed */,
       true /* client_pipe_closed */},
  };
  for (const auto& test_case : kTestCases) {
    ...
    loader_factory.AddEvent(TestLoaderEvent::kReceivedResponse);
    loader_factory.AddEvent(TestLoaderEvent::kBodyBufferReceived);
    if (test_case.body_data_read)
      loader_factory.AddEvent(TestLoaderEvent::kBodyDataRead);
    if (test_case.body_buffer_closed)
      loader_factory.AddEvent(TestLoaderEvent::kBodyBufferClosed);
    if (test_case.client_pipe_closed)
      loader_factory.AddEvent(TestLoaderEvent::kClientPipeClosed);
    ...
    // Destination file should have been created, and request should still be in
    // progress.
