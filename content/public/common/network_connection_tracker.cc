...
bool NetworkConnectionTracker::GetConnectionType(
  ...
  if (!task_runner_->RunsTasksInCurrentSequence()) {
    connection_type_callbacks_.emplace_back(
        base::BindOnce(&OnGetConnectionType, base::ThreadTaskRunnerHandle::Get(),
                   std::move(callback)));
  } else {
    connection_type_callbacks_.emplace_back(std::move(callback));
  }
  return false;
}
...
void NetworkConnectionTracker::OnInitialConnectionType(
  ...
  while (!connection_type_callbacks_.empty()) {
    ConnectionTypeCallback callback;
    std::swap(callback, connection_type_callbacks_.front());
