...
TEST(SandboxTypeTest, Empty) {
  ...
  EXPECT_EQ(sandbox::SANDBOX_TYPE_NO_SANDBOX,
  ...
  SetCommandLineFlagsForSandboxType(&command_line,
                                    sandbox::SANDBOX_TYPE_NO_SANDBOX);
  EXPECT_EQ(sandbox::SANDBOX_TYPE_NO_SANDBOX,
            SandboxTypeFromCommandLine(command_line));
...
TEST(SandboxTypeTest, Renderer) {
  ...
  EXPECT_EQ(sandbox::SANDBOX_TYPE_RENDERER,
            SandboxTypeFromCommandLine(command_line));
...
TEST(SandboxTypeTest, Utility) {
  ...
  EXPECT_EQ(sandbox::SANDBOX_TYPE_UTILITY,
            SandboxTypeFromCommandLine(command_line));

  base::CommandLine command_line2(command_line);
  SetCommandLineFlagsForSandboxType(&command_line2,
                                    sandbox::SANDBOX_TYPE_NETWORK);
  EXPECT_EQ(sandbox::SANDBOX_TYPE_NETWORK,
            SandboxTypeFromCommandLine(command_line2));

  base::CommandLine command_line3(command_line);
  SetCommandLineFlagsForSandboxType(&command_line3,
                                    sandbox::SANDBOX_TYPE_WIDEVINE);
  EXPECT_EQ(sandbox::SANDBOX_TYPE_WIDEVINE,
            SandboxTypeFromCommandLine(command_line3));

  base::CommandLine command_line4(command_line);
  SetCommandLineFlagsForSandboxType(&command_line4,
                                    sandbox::SANDBOX_TYPE_NO_SANDBOX);
  EXPECT_EQ(sandbox::SANDBOX_TYPE_NO_SANDBOX,
            SandboxTypeFromCommandLine(command_line4));

  base::CommandLine command_line5(command_line);
  SetCommandLineFlagsForSandboxType(&command_line5,
                                    sandbox::SANDBOX_TYPE_PPAPI);
  EXPECT_EQ(sandbox::SANDBOX_TYPE_PPAPI,
            SandboxTypeFromCommandLine(command_line5));

  base::CommandLine command_line6(command_line);
  command_line6.AppendSwitchASCII(switches::kUtilityProcessSandboxType,
                                  "bogus");
  EXPECT_EQ(sandbox::SANDBOX_TYPE_UTILITY,
            SandboxTypeFromCommandLine(command_line6));
...
TEST(SandboxTypeTest, GPU) {
  ...
  SetCommandLineFlagsForSandboxType(&command_line, sandbox::SANDBOX_TYPE_GPU);
  EXPECT_EQ(sandbox::SANDBOX_TYPE_GPU,
            SandboxTypeFromCommandLine(command_line));
...
TEST(SandboxTypeTest, PPAPIPlugin) {
  ...
  SetCommandLineFlagsForSandboxType(&command_line, sandbox::SANDBOX_TYPE_PPAPI);
  EXPECT_EQ(sandbox::SANDBOX_TYPE_PPAPI,
            SandboxTypeFromCommandLine(command_line));
...
TEST(SandboxTypeTest, Nonesuch) {
  ...
  EXPECT_EQ(sandbox::SANDBOX_TYPE_INVALID,
            SandboxTypeFromCommandLine(command_line));
