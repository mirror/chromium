...
void CookieStoreImpl::GetAll(
  ...
  BrowserThread::PostTask(
      ...
      base::BindOnce(&CookieStoreImpl::DoGetAll, base::Unretained(this), url,
                     first_party_for_cookies, name, std::move(options),
                     std::move(callback_runner), std::move(callback)));
}

void CookieStoreImpl::DoGetAll(
    const GURL& url,
    const GURL& first_party_for_cookies,
    const std::string& name,
    blink::mojom::CookieStoreGetOptionsPtr options,
  ...
  GetRequestContextForURL(url)->cookie_store()->GetCookieListWithOptionsAsync(
      ...
      base::BindOnce(&CookieStoreImpl::CookieListToGetAllCallback,
                     base::Unretained(this), url, first_party_for_cookies, name,
                     std::move(options), std::move(callback_runner),
                     std::move(callback)));
}
...
void CookieStoreImpl::Set(const GURL& url,
  ...
  BrowserThread::PostTask(
      BrowserThread::IO, FROM_HERE,
      base::BindOnce(&CookieStoreImpl::DoSet, base::Unretained(this), url,
                     first_party_for_cookies, name, value, std::move(options),
                     std::move(callback_runner), std::move(callback)));
}

void CookieStoreImpl::DoSet(
    const GURL& url,
    const GURL& first_party_for_cookies,
    const std::string& name,
    const std::string& value,
  ...
  base::Time expires_at = base::Time::FromJavaTime(options->expires);
  ...
  context->cookie_store()->SetCanonicalCookieAsync(
      ...
      base::BindOnce(&CookieStoreImpl::CookieStoreSuccessToSetCallback,
                     base::Unretained(this), std::move(callback_runner),
                     std::move(callback)));
}

void CookieStoreImpl::CookieStoreSuccessToSetCallback(
    scoped_refptr<base::SequencedTaskRunner> callback_runner,
    blink::mojom::CookieStore::SetCallback callback,
    bool success) {
  callback_runner->PostTask(
      FROM_HERE,
      base::BindOnce([](blink::mojom::CookieStore::SetCallback callback,
