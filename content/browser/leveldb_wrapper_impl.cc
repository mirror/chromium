...
LevelDBWrapperImpl::LevelDBWrapperImpl(
    ...
    : prefix_(leveldb::StdStringToUint8Vector(prefix)),
      ...
      database_(database),
      desired_load_state_(
          (options.cache_mode == Options::CACHE_ONLY_KEYS_WHEN_POSSIBLE &&
           database)
              ? LoadState::KEYS_ONLY
...
void LevelDBWrapperImpl::Bind(mojom::LevelDBWrapperRequest request) {
  ...
  // reaching late.
  if (bindings_.size() > 1 && desired_load_state_ == LoadState::KEYS_ONLY)
...
void LevelDBWrapperImpl::SetCacheMode(bool only_keys) {
  LoadState new_desired_state =
      only_keys ? LoadState::KEYS_ONLY : LoadState::KEYS_AND_VALUES;
  if (!database_ || new_desired_state == desired_load_state_)
    return;
  desired_load_state_ = new_desired_state;
  observers_.ForAllPtrs([only_keys](mojom::LevelDBObserver* observer) {
    observer->ShouldSendOldValueOnMutations(only_keys);
  });
  ...
  if (keys_only_map_ && desired_load_state_ == LoadState::KEYS_AND_VALUES &&
