...
std::unique_ptr<LevelDBWrapperImpl> LevelDBWrapperImpl::ForkToNewPrefix(
  ...
  // Copy over our in-memory representation
  ...
  if (map_state_ == MapLoadingState::LOADED) {
    ...
    LoadMap(base::BindOnce(
        &LevelDBWrapperImpl::SendStateToFork, weak_ptr_factory_.GetWeakPtr(),
        base::BindOnce(&LevelDBWrapperImpl::OnForkStateLoaded,
                       forked_wrapper->weak_ptr_factory_.GetWeakPtr())));
    destruction_during_load_listeners_.push_back(
        base::BindOnce(&LevelDBWrapperImpl::OnForkStateLoadFailed,
                       forked_wrapper->weak_ptr_factory_.GetWeakPtr()));
  }
...
void LevelDBWrapperImpl::Put(
  ...
  LOG(ERROR) << "put " << leveldb::Uint8VectorToStdString(key) << " value "
             << leveldb::Uint8VectorToStdString(value);
...
void LevelDBWrapperImpl::LoadMap(base::OnceClosure completion_callback) {
  ...
  LOG(ERROR) << "LevelDBWrapperImpl::LoadMap: calling get prefixed";
  LOG(ERROR) << database_;
...
void LevelDBWrapperImpl::SendStateToFork(
    LoadStateForForkCallback fork_callback) {
  DCHECK(map_state_ == MapLoadingState::LOADED);
  std::move(fork_callback).Run(database_ != nullptr, map_, commit_batch_.get());
}
...
void LevelDBWrapperImpl::OnForkStateLoaded(bool database_enabled,
  ...
  CreateCommitBatchIfNeeded();
  if (commit_batch)
    *commit_batch_ = *commit_batch;
  ScheduleCopyPrefix();
  OnLoadComplete();
}

void LevelDBWrapperImpl::OnForkStateLoadFailed() {
  map_state_ = MapLoadingState::INVALID;

  // Make sure the prefix copy is the first operation in case any observers
  // trigger a commit.
  on_load_complete_tasks_.push_front(base::BindOnce(
      &LevelDBWrapperImpl::ScheduleCopyPrefix, weak_ptr_factory_.GetWeakPtr()));

  LoadMap(base::BindOnce(&base::DoNothing));
}

void LevelDBWrapperImpl::ScheduleCopyPrefix() {
  if (database_) {
    CreateCommitBatchIfNeeded();
    if (!commit_batch_->copy_from_prefix_first)
      commit_batch_->copy_from_prefix_first = std::move(forked_from_prefix_);
  }
  forked_from_prefix_.clear();
}
...
void LevelDBWrapperImpl::OnMapLoaded(
  ...
  DCHECK(map_state_ != MapLoadingState::LOADED);
  LOG(ERROR) << "LevelDBWrapperImpl::OnMapLoaded";
...
void LevelDBWrapperImpl::CommitChanges() {
  ...
  } else if (commit_batch_->copy_from_prefix_first) {
    BatchedOperationPtr item = BatchedOperation::New();
    item->type = leveldb::mojom::BatchOperationType::COPY_PREFIXED_KEY;
    item->key = std::move(commit_batch_->copy_from_prefix_first.value());
    item->value = prefix_;
    operations.push_back(std::move(item));
  }
  size_t data_size = 0;
