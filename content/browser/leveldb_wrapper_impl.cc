...
LevelDBWrapperImpl::LevelDBWrapperImpl(
    ...
    bool cache_only_keys_when_posible,
    Delegate* delegate)
    : prefix_(leveldb::StdStringToUint8Vector(prefix)),
      delegate_(delegate),
      database_(database),
      desired_load_state_((cache_only_keys_when_posible && database)
...
void LevelDBWrapperImpl::Bind(mojom::LevelDBWrapperRequest request) {
  ...
  // reaching late.
  if (bindings_.size() > 1 && desired_load_state_ == LOAD_STATE_KEYS_ONLY)
...
void LevelDBWrapperImpl::OnMemoryDump(
  ...
  if (CurrentLoadState() == LOAD_STATE_UNLOADED)
    return;
...
void LevelDBWrapperImpl::PurgeMemory() {
  if (CurrentLoadState() ==
          LOAD_STATE_UNLOADED ||  // We're not using any memory.
...
void LevelDBWrapperImpl::Put(
  ...
  if (database_) {
    ...
    // CommitChanges() will take values from |keys_values_map_| if it is loaded.
    if (keys_only_map_)
...
void LevelDBWrapperImpl::Get(const std::vector<uint8_t>& key,
  ...
  if (desired_load_state_ != LOAD_STATE_KEYS_AND_VALUES) {
...
void LevelDBWrapperImpl::SetCacheMode(bool only_keys) {
  ...
  if (keys_only_map_ && desired_load_state_ == LOAD_STATE_KEYS_AND_VALUES &&
...
void LevelDBWrapperImpl::LoadMap(const base::Closure& completion_callback) {
  ...
  DCHECK_EQ(LOAD_STATE_UNLOADED, CurrentLoadState());
...
void LevelDBWrapperImpl::OnMapLoaded(
  ...
  keys_values_map_ = base::MakeUnique<ValueMap>();
  storage_used_ = 0;
  for (auto& it : data) {
  ...
  if (!changes.empty()) {
    ...
    for (auto& change : changes) {
      ...
      commit_batch_->changed_values[std::move(change.first)];
    }
    CommitChanges();
  }
...
void LevelDBWrapperImpl::CommitChanges() {
  ...
  DCHECK(database_);
  DCHECK_NE(LOAD_STATE_UNLOADED, CurrentLoadState());
...
void LevelDBWrapperImpl::UnloadMapIfDesired() {
  ...
  if (!database_ || commit_batch_ || commit_batches_in_flight_)
    return;
  if (desired_load_state_ == LOAD_STATE_KEYS_ONLY) {
    keys_only_map_ = base::MakeUnique<KeysOnlyMap>();
    memory_used_ = 0;
...
bool LevelDBWrapperImpl::IsMapReloadNeeded() const {
  // Current state being less than desired state means we either need to load
  // keys or values in the cache. If |on_load_complete_tasks_| is not empty and
  // desired state is achieved, it means that there was a task that required
  // reload to load values (eg: GetAll()). If we continue other tasks without
  // reload, the result returned by the task in queue will be inconsistent since
  // the reply callback for the other task reached earlier. So, we must queue
  // all tasks from that point till reload finishes.
  return CurrentLoadState() < desired_load_state_ ||
         !on_load_complete_tasks_.empty();
}

LevelDBWrapperImpl::LoadState LevelDBWrapperImpl::CurrentLoadState() const {
  if (keys_only_map_)
    return LOAD_STATE_KEYS_ONLY;
  else if (keys_values_map_)
    return LOAD_STATE_KEYS_AND_VALUES;
  else
