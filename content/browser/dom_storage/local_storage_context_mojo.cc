...
bool LocalStorageContextMojo::OnMemoryDump(
  ...
  pmd->CreateSharedGlobalAllocatorDump(memory_dump_id_);
  auto* leveldb_mad = pmd->CreateAllocatorDump(base::StringPrintf(
      "dom_storage/0x%" PRIXPTR "/leveldb", reinterpret_cast<uintptr_t>(this)));
  // Specifies that the current context is responsible for keeping memory alive.
  int kImportance = 2;
  pmd->AddOwnershipEdge(leveldb_mad->guid(), memory_dump_id_, kImportance);
