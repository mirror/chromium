...
class LocalStorageContextMojo::LevelDBWrapperHolder
 ...
 public:
  ...
  void Bind(mojom::LevelDBWrapperRequest request) {
    has_no_bindings_ = false;
    level_db_wrapper()->Bind(std::move(request));
  }

  bool has_no_bindings() const { return has_no_bindings_; }
...
void LocalStorageContextMojo::PurgeUnusedWrappersIfNeeded() {
  ...
  UMA_HISTOGRAM_COUNTS_100000(
      "LocalStorageContext.BrowserLocalStorageCacheSizeInKB",
  ...
  std::string full_histogram_name =
      std::string("LocalStorageContext.BrowserLocalStorageCachePurgedInKB.") +
      purge_reason;
  base::UmaHistogramCounts100000(full_histogram_name, purged_size_kib);
  UMA_HISTOGRAM_COUNTS_100000(
      "LocalStorageContext.BrowserLocalStorageCachePurgedInKB",
      purged_size_kib);
}
