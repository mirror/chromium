...
class SessionStorageContextMojo::LevelDBRefCountedMap final
 ...
 public:
  LevelDBRefCountedMap(SessionStorageContextMojo* context,
      ...
      : context_(context),
        wrapper_impl_(
            std::make_unique<LevelDBWrapperImpl>(context_->database_.get(),
                                                 std::move(map_prefix),
                                                 this,
                                                 GetOptions())),
...
class SessionStorageContextMojo::LevelDBWrapperBinder
 ...
 public:
  ...
  ~LevelDBWrapperBinder() override { refcounted_map_->RemoveMapReference(); }
  ...
  void OnConnectionError() { LOG(ERROR) << "connection error!"; }
 ...
 private:
  void ForkToNewMap() {
    ...
    std::vector<uint8_t> new_prefix = context_->GetAndWriteNextMapPrefix();
...
void SessionStorageContextMojo::OpenSessionStorage(
  ...
  RunWhenConnected(base::BindOnce(
      &SessionStorageContextMojo::BindSessionStorage, base::Unretained(this),
      std::move(namespace_str_id), origin, std::move(request)));
}
...
void SessionStorageContextMojo::BindSessionStorage(
    ...
    mojom::LevelDBWrapperRequest request) {
  LOG(ERROR) << "got bind!";
  GetOrCreateDBWrapper(std::move(namespace_id), origin)
...
SessionStorageContextMojo::GetOrCreateDBWrapper(const std::string& namespace_id,
  ...
  std::vector<uint8_t> map_id = GetAndWriteNextMapPrefix();
  auto data_map_it =
      data_maps_
