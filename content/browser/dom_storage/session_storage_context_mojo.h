...
class CONTENT_EXPORT SessionStorageContextMojo
 ...
 public:
  ...
  void OnDataMapDestruction(const std::vector<uint8_t>& map_prefix);
  void OnCommitResult(leveldb::mojom::DatabaseError error);

  std::vector<uint8_t> GetAndWriteNextMapPrefix();

 private:
  using MapId = std::vector<uint8_t>;

  void InitiateConnectionIfDisconnected();
  ...
  static std::string NextMapIdKey(const std::string& map_id,
  ...
  std::map<MapId, SessionStorageLevelDBWrapper::LevelDBRefCountedMap*>
      data_maps_;

  // This map contains all session storage metadata. A namespace and an origin
  // uniquely identifies a session storage map.
  using AreaKey = std::pair<std::string, url::Origin>;
  using SessionStorageMap =
      std::map<AreaKey, std::unique_ptr<SessionStorageLevelDBWrapper>>;
