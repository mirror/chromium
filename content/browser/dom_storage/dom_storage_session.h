...
class CONTENT_EXPORT DOMStorageSession {
 public:
  ...
  // Constructs a |DOMStorageSession| for the mojo version of session storage
  // and assigns it |namespace_id|.
  static std::unique_ptr<DOMStorageSession> CreateWithNamespace(
      scoped_refptr<DOMStorageContextWrapper> context,
      const std::string& namespace_id);

  // Constructs a |DOMStorageSession| by cloning
  // |namespace_id_to_clone|. Allocates a new ID for it.
  static std::unique_ptr<DOMStorageSession> CloneFrom(
      scoped_refptr<DOMStorageContextImpl> context,
      const std::string& namepace_id_to_clone);

  // Constructs a |DOMStorageSession| for the mojo version of session storage by
  // cloning |namespace_id_to_clone|. Allocates a new ID for it.
  static std::unique_ptr<DOMStorageSession> CloneFrom(
      scoped_refptr<DOMStorageContextWrapper> context,
      const std::string& namepace_id_to_clone);
