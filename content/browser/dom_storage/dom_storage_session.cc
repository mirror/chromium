...
class DOMStorageSession::SequenceHelper {
 public:
  ...
  void CloneSessionNamespace(const std::string& namespace_id_to_clone,
    ...
    mojo_context_->CloneSessionNamespace(namespace_id_to_clone,
                                         clone_namespace_id);
  }
...
std::unique_ptr<DOMStorageSession> DOMStorageSession::Create(
  ...
  return DOMStorageSession::CreateWithNamespace(std::move(context),
                                                context->AllocateSessionId());
}
...
std::unique_ptr<DOMStorageSession> DOMStorageSession::Create(
  ...
  return DOMStorageSession::CreateWithNamespace(
      std::move(context), context->context()->AllocateSessionId());
}
...
std::unique_ptr<DOMStorageSession> DOMStorageSession::CreateWithNamespace(
    scoped_refptr<DOMStorageContextWrapper> context,
    const std::string& namespace_id) {
  std::unique_ptr<DOMStorageSession> result =
      base::WrapUnique(new DOMStorageSession(std::move(context), namespace_id));
...
std::unique_ptr<DOMStorageSession> DOMStorageSession::CloneFrom(
  ...
  return std::unique_ptr<DOMStorageSession>(
      new DOMStorageSession(std::move(context), clone_id));
}
...
std::unique_ptr<DOMStorageSession> DOMStorageSession::CloneFrom(
    ...
    const std::string& namespace_id_to_clone) {
  std::unique_ptr<DOMStorageSession> result =
      base::WrapUnique(new DOMStorageSession(
          std::move(context), context->context()->AllocateSessionId()));
...
bool DOMStorageSession::IsFromContext(DOMStorageContextWrapper* context) {
  return sequence_helper_ ? mojo_context_.get() == context
                          : context_.get() == context->context();
}
