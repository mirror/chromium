...
void DOMStorageMessageFilter::OnSetItem(int connection_id,
  ...
  base::AutoReset<int> auto_reset(&connection_dispatching_message_for_,
                            connection_id);
  bool success =
      host_->SetAreaItem(connection_id, key, value, old_value, page_url);
...
void DOMStorageMessageFilter::OnRemoveItem(
  ...
  base::AutoReset<int> auto_reset(&connection_dispatching_message_for_,
                            connection_id);
  host_->RemoveAreaItem(connection_id, key, old_value, page_url);
  Send(new DOMStorageMsg_AsyncOperationComplete(true));
}
