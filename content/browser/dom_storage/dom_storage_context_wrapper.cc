...
DOMStorageContextWrapper::DOMStorageContextWrapper(
  ...
  if (!base::CommandLine::ForCurrentProcess()->HasSwitch(
    ...
    if (!profile_path.empty())
      session_storage_dir = local_partition_path.AppendASCII(kSessionStorageDirectory);
    mojo_session_state_.reset(new SessionStorageContextMojo(
...
void DOMStorageContextWrapper::OpenSessionStorage(
  ...
  if (!mojo_session_state_)
    return;
  mojo_session_state_->OpenSessionStorage(namespace_id, origin, std::move(request));
}
