...
void InvokeSessionStorageUsageCallbackHelper(
    ...
    std::unique_ptr<std::vector<SessionStorageUsageInfo>> infos) {
  callback.Run(*infos);
}
...
void GetSessionStorageUsageHelper(
    ...
    DOMStorageContextImpl* context,
    const DOMStorageContext::GetSessionStorageUsageCallback& callback) {
  auto infos = std::make_unique<std::vector<SessionStorageUsageInfo>>();
  context->GetSessionStorageUsage(infos.get());
  reply_task_runner->PostTask(
...
void CollectSessionStorageUsage(
    std::vector<SessionStorageUsageInfo>* out_info,
    base::Closure done_callback,
    const std::vector<SessionStorageUsageInfo>& in_info) {
  out_info->insert(out_info->end(), in_info.begin(), in_info.end());
  done_callback.Run();
}
...
DOMStorageContextWrapper::DOMStorageContextWrapper(
  ...
  LOG(ERROR) << profile_path;
  LOG(ERROR) << local_partition_path;
...
void DOMStorageContextWrapper::GetSessionStorageUsage(
  ...
  if (mojo_session_state_) {
    ...
    auto* infos_ptr = infos.get();
    base::RepeatingClosure got_local_storage_usage = base::BarrierClosure(
        2, base::BindOnce(&InvokeSessionStorageUsageCallbackHelper, callback,
                          std::move(infos)));
    ...
    mojo_task_runner_->PostTask(
        FROM_HERE,
        base::BindOnce(
            &SessionStorageContextMojo::GetStorageUsage,
            base::Unretained(mojo_session_state_),
            base::BindOnce(&GotMojoSessionStorageUsage,
                           base::ThreadTaskRunnerHandle::Get(),
                           base::Bind(CollectSessionStorageUsage, infos_ptr,
                                      got_local_storage_usage))));
    return;
  }
  context_->task_runner()->PostShutdownBlockingTask(
  ...
  if (mojo_session_state_) {
    ...
    mojo_task_runner_->PostTask(
        ...
        base::BindOnce(&SessionStorageContextMojo::DeleteStorage,
                       base::Unretained(mojo_session_state_), usage_info.origin,
