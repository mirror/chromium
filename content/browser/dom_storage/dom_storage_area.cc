...
DOMStorageArea::DOMStorageArea(
    ...
    std::unique_ptr<std::vector<std::string>> original_permanent_namespace_ids,
    const GURL& origin,
    SessionStorageDatabase* session_storage_backing,
    DOMStorageTaskRunner* task_runner)
    : namespace_id_(namespace_id),
      persistent_namespace_id_(persistent_namespace_id),
      original_permanent_namespace_ids_(
          std::move(original_permanent_namespace_ids)),
      origin_(origin),
      task_runner_(task_runner),
#if defined(OS_ANDROID)
  ...
  if (session_storage_backing) {
    backing_.reset(new SessionStorageDatabaseAdapter(
        ...
        original_permanent_namespace_ids_ ? *original_permanent_namespace_ids_
...
DOMStorageArea* DOMStorageArea::ShallowCopy(
  ...
  auto original_permanent_namespace_ids =
      base::MakeUnique<std::vector<std::string>>();
  original_permanent_namespace_ids->push_back(persistent_namespace_id_);
  if (original_permanent_namespace_ids_) {
    original_permanent_namespace_ids->insert(
        original_permanent_namespace_ids->end(),
        original_permanent_namespace_ids_->begin(),
        original_permanent_namespace_ids_->end());
  }
  DOMStorageArea* copy = new DOMStorageArea(
      destination_namespace_id, destination_persistent_namespace_id,
      std::move(original_permanent_namespace_ids), origin_,
...
void DOMStorageArea::LoadMapAndApplyUncommittedChangesIfNeeded(
  ...
  // recent clear all. It is possible that the changes in one or more of the
  // commit batches have already been written to the database and reflected in
  // the map returned by ReadAllValues(). It is okay to re-apply these changes.
...
DOMStorageArea::CommitBatch* DOMStorageArea::CreateCommitBatchIfNeeded() {
  ...
  return current_commit_batch()->batch.get();
}
...
void DOMStorageArea::PopulateCommitBatchValues() {
  ...
  if (load_state_ != LOAD_STATE_KEYS_AND_VALUES)
    return;
  CommitBatch* current_batch = current_commit_batch()->batch.get();
...
void DOMStorageArea::StartCommitTimer() {
  if (is_shutdown_ || !current_commit_batch())
    return;
...
void DOMStorageArea::PostCommitTask() {
  ...
  DCHECK(backing_.get());
  CommitBatchHolder* current_batch = current_commit_batch();
  PopulateCommitBatchValues();
...
void DOMStorageArea::OnCommitComplete() {
  ...
  ClearShallowCopiedCommitBatches();
  commit_batches_.pop_back();
  if (current_commit_batch() && !has_commit_batch_in_flight()) {
...
void DOMStorageArea::ShutdownInCommitSequence() {
  ...
  DCHECK(backing_.get());
  if (current_commit_batch()) {
    CommitBatch* batch = current_commit_batch()->batch.get();
