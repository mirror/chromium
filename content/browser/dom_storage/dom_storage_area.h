...
class CONTENT_EXPORT DOMStorageArea
 ...
 public:
  ...
  DOMStorageArea(int64_t namespace_id,
                 ...
                 std::unique_ptr<std::vector<std::string>>
                     original_permanent_namespace_ids,
                 const GURL& origin,
 ...
 private:
  ...
  CommitBatchHolder* current_commit_batch() {
    return (!commit_batches_.empty() &&
            commit_batches_.front().type ==
                CommitBatchHolder::TYPE_CURRENT_BATCH)
               ? &commit_batches_.front()
               : nullptr;
  }
  bool has_commit_batch_in_flight() {
    return !commit_batches_.empty() &&
           commit_batches_.back().type == CommitBatchHolder::TYPE_IN_FLIGHT;
  }
  ...
  std::unique_ptr<std::vector<std::string>> original_permanent_namespace_ids_;
  GURL origin_;
