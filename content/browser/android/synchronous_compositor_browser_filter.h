...
class SynchronousCompositorBrowserFilter : public BrowserMessageFilter {
 public:
  ...
  void RegisterHost(int routing_id,
                    scoped_refptr<SynchronousCompositorHostBridge> host_bridge);
  void UnregisterHost(int routing_id);
 ...
 private:
  ...
  std::map<int, scoped_refptr<SynchronousCompositorHostBridge>> hosts_;
