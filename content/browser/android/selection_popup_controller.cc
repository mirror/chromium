...
bool SelectionPopupController::ShowSelectionMenu(
  ...
  const bool can_suggest =
      !!(params.edit_flags & blink::WebContextMenuData::kCanSuggestSelection);
  ...
  const bool should_suggest = can_suggest && (
      params.source_type == ui::MENU_SOURCE_TOUCH ||
      params.source_type == ui::MENU_SOURCE_LONG_PRESS);
  const bool is_smart_selection_reset = !can_suggest;
