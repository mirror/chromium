...
MotionEventSynthesizerImpl::MotionEventSynthesizerImpl(
  ...
  web_contents_->GetWebContentsAndroid()->SetMotionEventSynthesizer(
      Java_MotionEventSynthesizer_create(env));
}

MotionEventSynthesizerImpl::~MotionEventSynthesizerImpl() {}

void MotionEventSynthesizerImpl::SetTargetView(ui::ViewAndroid* view) {
  base::android::ScopedJavaLocalRef<jobject> obj = GetJavaObject();
  if (obj.is_null())
    return;
  JNIEnv* env = base::android::AttachCurrentThread();
  Java_MotionEventSynthesizer_setTargetView(env, obj, view->GetContainerView());
}
