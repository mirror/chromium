...
#include "content/browser/android/gesture_listener_manager_android.h"
...
DEFINE_WEB_CONTENTS_USER_DATA_KEY(GestureListenerManagerAndroid);

GestureListenerManagerAndroid::GestureListenerManagerAndroid(
...
void GestureListenerManagerAndroid::SetJavaRef(
    JNIEnv* env,
...
GestureListenerManagerAndroid::~GestureListenerManagerAndroid() {
  ...
  // WebContents is in the middle of destruction, therefore
  // |WebContentsAndroid::set_gesture_listener_manager_|
  // needs not resetting here.
}

void GestureListenerManagerAndroid::GestureEventAck(
  ...
  switch (event.GetType()) {
    ...
    case WebInputEvent::kGestureFlingCancel:
      Java_GestureListenerManagerImpl_onFlingCancelEventAck(env, j_obj);
      break;
...
void JNI_GestureListenerManagerImpl_Init(
  ...
  GestureListenerManagerAndroid::CreateForWebContents(web_contents);
  auto* manager = GestureListenerManagerAndroid::FromWebContents(web_contents);
  DCHECK(manager);
  manager->SetJavaRef(env, obj);
}
