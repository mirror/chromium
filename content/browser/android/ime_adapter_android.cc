...
// ui::ImeSpan instance, and append it to |ime_spans_ptr|.
void AppendBackgroundColorSpan(JNIEnv*,
  ...
  std::vector<ui::ImeSpan>* ime_spans =
      reinterpret_cast<std::vector<ui::ImeSpan>*>(ime_spans_ptr);
  ime_spans->push_back(ui::ImeSpan(
...
void AppendUnderlineSpan(JNIEnv*,
  ...
  std::vector<ui::ImeSpan>* ime_spans =
      reinterpret_cast<std::vector<ui::ImeSpan>*>(ime_spans_ptr);
  ime_spans->push_back(ui::ImeSpan(static_cast<unsigned>(start),
...
void ImeAdapterAndroid::SetComposingText(JNIEnv* env,
  ...
  std::vector<ui::ImeSpan> ime_spans =
      GetImeSpansFromJava(env, obj, text, text16);

  // Default to plain underline if we didn't find any span that we care about.
  if (ime_spans.empty()) {
    ime_spans.push_back(ui::ImeSpan(0, text16.length(), SK_ColorBLACK, false,
                                    SK_ColorTRANSPARENT));
  }
  ...
  rwhi->ImeSetComposition(text16, ime_spans, gfx::Range::InvalidRange(),
...
void ImeAdapterAndroid::CommitText(JNIEnv* env,
  ...
  rwhi->ImeCommitText(text16, ime_spans, gfx::Range::InvalidRange(),
                      relative_cursor_pos);
}
...
void ImeAdapterAndroid::SetComposingRegion(JNIEnv*,
  ...
  std::vector<ui::ImeSpan> ime_spans;
  ime_spans.push_back(
      ui::ImeSpan(0, end - start, SK_ColorBLACK, false, SK_ColorTRANSPARENT));
...
std::vector<ui::ImeSpan> ImeAdapterAndroid::GetImeSpansFromJava(
    JNIEnv* env,
    const base::android::JavaParamRef<jobject>& obj,
  ...
  // AppendBackgroundColorSpan()), and populate |ime_spans|.
  Java_ImeAdapter_populateImeSpansFromJava(env, obj, text,
                                           reinterpret_cast<jlong>(&ime_spans));

  // Sort spans by |.startOffset|.
  std::sort(ime_spans.begin(), ime_spans.end());
