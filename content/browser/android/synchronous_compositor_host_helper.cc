...
SynchronousCompositorHostHelper::SynchronousCompositorHostHelper(
...
void SynchronousCompositorHostHelper::BindFilter(
    scoped_refptr<SynchronousCompositorBrowserFilter> filter) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  if (filter_)
    return;
  filter_ = filter;
  scoped_refptr<SynchronousCompositorHostHelper> thiz = this;
  BrowserThread::PostTask(
      BrowserThread::IO, FROM_HERE,
      base::BindOnce(&SynchronousCompositorBrowserFilter::RegisterHost, filter,
                     routing_id_, thiz));
}

void SynchronousCompositorHostHelper::RemoteReady() {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  base::AutoLock lock(lock_);
  remote_closed_ = false;
}

void SynchronousCompositorHostHelper::RemoteClosed() {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  base::AutoLock lock(lock_);
  remote_closed_ = true;
...
bool SynchronousCompositorHostHelper::ReceiveFrame(
  ...
  if (compositor_frame) {
    BrowserThread::PostTask(
        ...
        base::BindOnce(&SynchronousCompositorHostHelper::ProcessFrameMetadata,
...
void SynchronousCompositorHostHelper::BeginFrameResponse(
...
void SynchronousCompositorHostHelper::WaitAfterVSync(
  ...
  window_android_in_vsync_->AddVSyncCompleteCallback(base::BindRepeating(
      &SynchronousCompositorHostHelper::VSyncComplete, this));
}

void SynchronousCompositorHostHelper::SetFrameFuture(
...
void SynchronousCompositorHostHelper::HostDestroyed() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  host_ = nullptr;
}

void SynchronousCompositorHostHelper::VSyncComplete() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
void SynchronousCompositorHostHelper::ProcessFrameMetadata(
