...
void SynchronousCompositorHostBridge::BindFilterOnUIThread() {
  ...
  scoped_refptr<SynchronousCompositorBrowserFilter> filter =
      host_->GetProcess()->synchronous_compositor_filter();
  if (!filter)
    return;
...
void SynchronousCompositorHostBridge::BeginFrameResponseOnIOThread(
...
void SynchronousCompositorHostBridge::HostDestroyedOnUIThread() {
  ...
  if (bound_to_filter_) {
    ...
    if (filter) {
      BrowserThread::PostTask(
          BrowserThread::IO, FROM_HERE,
          base::BindOnce(&SynchronousCompositorBrowserFilter::UnregisterHost,
                         filter, routing_id_));
    }
  }
  host_ = nullptr;
