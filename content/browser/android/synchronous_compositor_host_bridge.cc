...
SynchronousCompositorHostBridge::SynchronousCompositorHostBridge(
...
void SynchronousCompositorHostBridge::BindFilterOnUIThread() {
  ...
  scoped_refptr<SynchronousCompositorHostBridge> thiz = this;
...
void SynchronousCompositorHostBridge::RemoteReady() {
  base::AutoLock lock(lock_);
  remote_closed_ = false;
}

void SynchronousCompositorHostBridge::RemoteClosedOnIOThread() {
  ...
  // Deregister if necessary. Post a task as unregistering will destroy this
  // object.
...
bool SynchronousCompositorHostBridge::ReceiveFrameOnIOThread(
  ...
  if (compositor_frame) {
    BrowserThread::PostTask(
        ...
        base::BindOnce(
            &SynchronousCompositorHostBridge::ProcessFrameMetadataOnUIThread,
...
bool SynchronousCompositorHostBridge::BeginFrameResponseOnIOThread(
...
bool SynchronousCompositorHostBridge::WaitAfterVSyncOnUIThread(
  ...
  window_android_in_vsync_->AddVSyncCompleteCallback(base::BindRepeating(
      &SynchronousCompositorHostBridge::VSyncCompleteOnUIThread, this));
  return true;
}

void SynchronousCompositorHostBridge::SetFrameFutureOnUIThread(
...
void SynchronousCompositorHostBridge::HostDestroyedOnUIThread() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  DCHECK(host_);
  scoped_refptr<SynchronousCompositorBrowserFilter> filter = host_->GetFilter();
  ...
  if (filter) {
    unregister_callback_ = base::BindOnce(base::BindOnce(
        &SynchronousCompositorHostBridge::UnregisterHostOnIOThread, this,
        filter));

    if (frame_futures_.empty())
      BrowserThread::PostTask(BrowserThread::IO, FROM_HERE,
                              std::move(unregister_callback_));
  }
}
...
void SynchronousCompositorHostBridge::UnregisterHostOnIOThread(
    scoped_refptr<SynchronousCompositorBrowserFilter> filter) {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  RemoteClosedOnIOThread();
  if (filter)
