...
#include "content/browser/android/synchronous_compositor_host_bridge.h"
...
class SynchronousCompositorControlHost
 ...
 public:
  SynchronousCompositorControlHost(
      scoped_refptr<SynchronousCompositorHostBridge> bridge)
  ...
  static void Create(mojom::SynchronousCompositorControlHostRequest request,
    ...
    BrowserThread::PostTask(
        ...
        base::BindOnce(&CreateOnIOThread, std::move(request),
                       std::move(bridge)));
  }
  ...
  void ReturnFrame(uint32_t layer_tree_frame_sink_id,
    ...
    bridge_->ReceiveFrameOnIOThread(layer_tree_frame_sink_id, frame);
  }

  void BeginFrameResponse(
      const content::SyncCompositorCommonRendererParams& params) override {
    bridge_->BeginFrameResponseOnIOThread(params);
  }

 private:
  scoped_refptr<SynchronousCompositorHostBridge> bridge_;
};
...
SynchronousCompositorHost::SynchronousCompositorHost(
  ...
  bridge_ = new SynchronousCompositorHostBridge(this);
...
bool SynchronousCompositorHost::IsReadyForSynchronousCall() {
  return bridge_->IsRemoteReadyOnUIThread();
}
