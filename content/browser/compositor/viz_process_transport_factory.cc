...
ui::mojom::GpuPtr BindBrowserGpuRequest() {
  // Thread hop to IO thread in order to bind GpuPtr endpoint.
  ui::mojom::GpuPtr gpu_ptr;
  BrowserThread::PostTask(
      BrowserThread::IO, FROM_HERE,
      base::BindOnce(
          [](ui::mojom::GpuRequest request) {
            GpuProcessHost::Get()->BindBrowserGpuRequest(std::move(request));
          },
          MakeRequest(&gpu_ptr)));
  return gpu_ptr;
}
...
void VizProcessTransportFactory::SetDisplayColorSpace(
  ...
  if (iter == compositor_data_map_.end())
    return;
  // TODO(kylechar): Fix DisplayPrivate interface.
...
viz::FrameSinkManagerImpl* VizProcessTransportFactory::GetFrameSinkManager() {
  // This should never ever get called when viz is enabled.
  NOTREACHED();
  return nullptr;
}
