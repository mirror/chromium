...
bool VizProcessTransportFactory::CreateContextProviders(
  ...
  if (shared_main_context_provider_ &&
      CheckContextLost(shared_main_context_provider_.get())) {
    shared_main_context_provider_->RemoveObserver(this);
    shared_main_context_provider_ = nullptr;
  }

  if (shared_worker_context_provider_ &&
      CheckWorkerContextLost(shared_worker_context_provider_.get()))
    shared_worker_context_provider_ = nullptr;
...
void VizProcessTransportFactory::OnLostMainThreadSharedContext() {
  // It's possible that |shared_main_context_provider_| was already reset in
