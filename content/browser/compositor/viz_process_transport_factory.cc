...
ui::mojom::GpuPtr BindGpuProcessHost() {
  ui::mojom::GpuPtr gpu_ptr;
  BrowserThread::PostTask(
      BrowserThread::IO, FROM_HERE,
      base::BindOnce(
          [](ui::mojom::GpuRequest request) {
            GpuProcessHost::Get()->BindGpuRequest(std::move(request));
          },
          MakeRequest(&gpu_ptr)));
  return gpu_ptr;
}
...
VizProcessTransportFactory::VizProcessTransportFactory()
  ...
  gpu_ =
      ui::Gpu::Create(base::BindRepeating(&BindGpuProcessHost),
...
void VizProcessTransportFactory::ResizeDisplay(ui::Compositor* compositor,
                                               const gfx::Size& size) {
  // Do nothing and resize when the CompositorFrame arrives.
}
