...
void GpuProcessTransportFactory::EstablishedGpuChannel(
  ...
  // If |gpu_channel_host| is null, we leave |use_gpu_compositing| as true to
  // treat it as a transient error and retry.
  if (!use_gpu_compositing || !gpu_channel_host || vulkan_context_provider) {
  ...
  if (!gpu_compositing_ready) {
    ...
    // compositing on ChromeOS. These are bugs and should not happen, either
    // from the client's context request, in the service, or a transient error
    // was miscategorized as fatal.
