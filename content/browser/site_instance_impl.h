...
class CONTENT_EXPORT SiteInstanceImpl final : public SiteInstance,
 ...
 public:
  ...
  // SiteInstance. If no what exists, and there are unmatched service worker
  // processes of the site, the newest unmatched service worker process is
  // reused. If no RenderProcessHost exists a new RenderProcessHost will be
  // created unless the process limit has been reached. When the limit has been
  // reached, the RenderProcessHost reused will be chosen randomly and not based
  // on the site.
