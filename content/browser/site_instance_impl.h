...
class CONTENT_EXPORT SiteInstanceImpl final : public SiteInstance,
 ...
 public:
  ...
  enum class ProcessReusePolicy {
    ...
    // committed navigation. If none exists, and there are service worker only
    // processes for the site, the newest process is reused. If none exists new
    // process will be created. If multiple such processes exist, ones that have
    ...
    // is set, when a new process is created for this SiteInstance, the process
    // will be tracked as a service worker only process until reused by another
    // SiteInstance which reuse policy is DEFAULT or
    // REUSE_PENDING_OR_COMMITTED_SITE. This policy can be used in combination
    // with DEFAULT or REUSE_PENDING_OR_COMMITTED_SITE.
