...
class CONTENT_EXPORT SiteInstanceImpl final : public SiteInstance,
 ...
 public:
  ...
  // SiteInstance. If none exists, and there are unmatched service worker's
  // processes of the site, the newest process is reused. If none exests a new
  ...
  enum class ProcessReusePolicy {
    // In this mode, the RenderProcessHost which is once used for the site will
    // be reused. So all instances of the site will be hosted in the same
    // RenderProcessHost.
    PROCESS_PER_SITE,
    ...
    // In this mode, SiteInstances doesn't proactively reuse processes. The
    ...
    // reached, a randomly chosen RenderProcessHost is reused same as other
    // policy.
    DEFAULT,
  };
