...
class CONTENT_EXPORT SiteInstanceImpl final : public SiteInstance,
 ...
 public:
  ...
  // according to the policy, and there are unmatched service worker processes
  // of the site, the newest unmatched service worker process is reused. If
  // still no RenderProcessHost exists a new RenderProcessHost will be created
  // unless the process limit has been reached. When the limit has been reached,
  // the RenderProcessHost reused will be chosen randomly and not based on the
  // site.
  enum class ProcessReusePolicy {
    ...
    // existing unmatched service worker's process is reused only for
    // navigations, not for service workers. When the process limit has been
    // reached, a randomly chosen RenderProcessHost is reused as in the other
    // policies.
    DEFAULT,
  };
