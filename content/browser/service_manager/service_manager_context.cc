...
void ServiceManagerContext::InitForBrowserProcess() {
  ...
  in_process_context_->SetTaskRunner(task_runner_);
  packaged_services_connection_->SetTaskRunner(task_runner_);
  g_io_thread_connector.Get() =
      packaged_services_connection_->GetConnector()->Clone();
