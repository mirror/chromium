...
ServiceManagerContext::ServiceManagerContext() {
  ...
  // Run the metrics service on the UI thread. base::StatisticsCollector, which
  // manages the merging and collection of histograms must call all
  // HistogramProviders on the UI thread.
  service_manager::EmbeddedServiceInfo metrics_info;
  metrics_info.factory = base::BindRepeating(&metrics::Service::Create);
  metrics_info.task_runner =
