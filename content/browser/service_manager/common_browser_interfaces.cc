...
class ConnectionFilterImpl : public ConnectionFilter {
 public:
  ConnectionFilterImpl() : task_runner_(base::ThreadTaskRunnerHandle::Get()) {
    auto* coordinator = memory_instrumentation::CoordinatorImpl::GetInstance();
    RegisterInterface(base::Bind(
 ...
 private:
  ...
  template <typename Interface>
  void RegisterInterface(const InterfaceBinder<Interface>& binder) {
    registry_.AddInterface(
        base::Bind(&BindOnTaskRunner<Interface>, task_runner_, binder));
  }
