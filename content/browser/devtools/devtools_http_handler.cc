...
void StartServerOnHandlerThread(
  ...
  if (ip_address) {
    std::string message =
        base::StringPrintf("\nDevTools listening on ws://%s%s/%s\n",
                           ip_address->ToString().c_str(), kBrowserUrlPrefix,
                           browser_guid.c_str());
    ...
    if (!output_directory.empty()) {
      ...
      std::string port_string = base::UintToString(ip_address->port());
...
void DevToolsHttpHandler::OnWebSocketRequest(
  ...
  std::string browser_url = delegate_->IsBrowserTargetDiscoverable()
                                ? kBrowserUrlPrefix
                                : base::StringPrintf("%s/%s", kBrowserUrlPrefix,
                                                     browser_guid_.c_str());

  if (base::StartsWith(request.path, browser_url,
                       base::CompareCase::SENSITIVE)) {
...
DevToolsHttpHandler::DevToolsHttpHandler(
    ...
    : frontend_url_(frontend_url),
      ...
      browser_guid_(base::GenerateGUID()),
      delegate_(delegate),
      weak_factory_(this) {
