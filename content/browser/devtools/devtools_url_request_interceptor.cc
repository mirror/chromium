...
DevToolsURLRequestInterceptor::DevToolsURLRequestInterceptor(
  ...
  target_lookup_ = target_registry->TakeLookupHandle();
...
DevToolsURLRequestInterceptor::TargetInfoForRequestInfo(
  ...
  if (frame_node_id != -1)
    return target_lookup_->GetInfoByFrameTreeNodeId(frame_node_id);
  return target_lookup_->GetInfoByRenderFramePair(
