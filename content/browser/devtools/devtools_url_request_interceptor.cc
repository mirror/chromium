...
void DevToolsURLRequestInterceptor::State::RenderFrameHostChangedOnIo(
  ...
  StartInterceptingRequestsForHostInfoInternal(new_host_info, web_contents);
}

void DevToolsURLRequestInterceptor::State::
    StartInterceptingRequestsForHostInfoInternal(RenderFrameHostInfo host_info,
                                                 WebContents* web_contents) {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  DCHECK(intercepted_page_for_web_contents_.count(web_contents));
...
void DevToolsURLRequestInterceptor::State::StartInterceptingRequestsOnIo(
  ...
  for (const auto host_info : host_info_list)
    StartInterceptingRequestsForHostInfoInternal(host_info, web_contents);

  DCHECK_EQ(intercepted_frame_tree_nodes_.size(),
...
void DevToolsURLRequestInterceptor::State::StartInterceptingRequests(
  ...
  BrowserThread::PostTask(
      ...
      base::BindOnce(
          ...
          this, std::move(host_info_list), web_contents, network_handler,
          std::move(interceptedPage)));
