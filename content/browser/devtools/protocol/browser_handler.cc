...
unique_ptr<Browser::Histogram> Convert(const HistogramBase& in_histogram) {
  ...
  auto out_buckets = make_unique<Array<Browser::Bucket>>();
...
Response BrowserHandler::GetHistograms(
    ...
    unique_ptr<Array<Browser::Histogram>>* const out_histograms) {
  // Convert histograms.
  DCHECK(out_histograms);
  *out_histograms = make_unique<Array<Browser::Histogram>>();
  for (const HistogramBase* const h :
       StatisticsRecorder::GetSnapshot(in_query.fromMaybe(""))) {
    DCHECK(h);
