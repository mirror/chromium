...
void TracingHandler::OnTraceDataCollected(
  ...
  const std::string valid_trace_fragment =
      UpdateTraceCache(std::move(trace_fragment));
...
void TracingHandler::OnTraceComplete() {
  ...
  DCHECK(trace_cache_.empty());
  frontend_->TracingComplete();
}

std::string TracingHandler::UpdateTraceCache(
    std::unique_ptr<std::string> trace_fragment) {
  std::string trace_str = trace_cache_ + (*trace_fragment.release());
  size_t start = 0;
  while (start < trace_str.size() && trace_str[start] != '{') {
    start++;
  }
  DCHECK_LE(start, 2u) << trace_str;
  size_t end = start;
  size_t pos = start + 1;
  size_t opens = 1;
  ...
  trace_cache_ = trace_str.substr(end);
...
void TracingHandler::End(std::unique_ptr<EndCallback> callback) {
  ...
  if (return_as_stream_) {
    ...
    StopTracing(endpoint, kChromeTraceEventLabel);
  }
