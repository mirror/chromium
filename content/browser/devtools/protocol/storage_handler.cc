...
void StorageHandler::TrackOrigin(
  ...
  cache_storage_observer_->TrackOrigin(url::Origin(origin_url));
...
void StorageHandler::UntrackOrigin(
  ...
  GetCacheStorageObserver();
  cache_storage_observer_->UntrackOrigin(url::Origin(origin_url));
...
void StorageHandler::GetCacheStorageObserver() {
  if (cache_storage_observer_ == nullptr) {
    CacheStorageObserver* observer = new CacheStorageObserver(
        ptr_factory_.GetWeakPtr(),
        host_->GetProcess()->GetStoragePartition()->GetCacheStorageContext());
    cache_storage_observer_ =
        base::MakeUnique<CacheStorageObserver>(std::move(*observer));
  }
}
...
StorageHandler::CacheStorageObserver::CacheStorageObserver(
    StorageHandler::CacheStorageObserver&& other) = default;
