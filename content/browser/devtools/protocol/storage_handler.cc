...
class StorageHandler::CacheStorageObserver : CacheStorageContextImpl::Observer {
 public:
  ...
  base::flat_set<url::Origin> GetOrigins() { return std::move(origins_); }
...
class StorageHandler::IndexedDBObserver : IndexedDBContextImpl::Observer {
 public:
  IndexedDBObserver(base::WeakPtr<StorageHandler> owner_storage_handler,
      ...
      : context(indexed_db_context), owner_(owner_storage_handler) {
  ...
  ~IndexedDBObserver() override {
    context->TaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&IndexedDBObserver::RemoveObserverOnIDBThread,
                                  base::Unretained(this)));
  }
  ...
  void OnIndexedDBContentChanged(const url::Origin& origin,
    ...
    BrowserThread::PostTask(
        ...
        base::BindOnce(&StorageHandler::NotifyIndexedDBContentChanged, owner_,
                       origin.Serialize(), databaseName, objectStoreName));
  }
 ...
 private:
  ...
  void RemoveObserverOnIDBThread() {
    DCHECK(context->TaskRunner()->RunsTasksInCurrentSequence());
    context->RemoveObserver(this);
  }

  // Maintained on the IDB thread to avoid mutex contention.
  base::flat_set<url::Origin> origins_;
...
void StorageHandler::SetRenderFrameHost(RenderFrameHostImpl* host) {
  ...
  if (cache_storage_observer_) {
    base::flat_set<url::Origin> origins = cache_storage_observer_->GetOrigins();
    cache_storage_observer_ = nullptr;
    for (auto& origin : origins) {
      BrowserThread::PostTask(
          BrowserThread::IO, FROM_HERE,
          base::BindOnce(&CacheStorageObserver::TrackOriginOnIOThread,
                         base::Unretained(GetCacheStorageObserver()), origin));
    }
  }
  if (indexed_db_observer_) {
    base::flat_set<url::Origin> origins = indexed_db_observer_->GetOrigins();
    indexed_db_observer_ = nullptr;
    GetIndexedDBObserver();
    for (auto& origin : origins) {
      indexed_db_observer_->context->TaskRunner()->PostTask(
          FROM_HERE,
          base::BindOnce(&IndexedDBObserver::TrackOriginOnIDBThread,
                         base::Unretained(indexed_db_observer_.get()), origin));
    }
  }
}
...
Response StorageHandler::TrackIndexedDBForOrigin(const std::string& origin) {
  ...
  indexed_db_observer_->context->TaskRunner()->PostTask(
      FROM_HERE, base::BindOnce(&IndexedDBObserver::TrackOriginOnIDBThread,
                                base::Unretained(indexed_db_observer_.get()),
                                url::Origin(origin_url)));
  return Response::OK();
}
...
void StorageHandler::NotifyIndexedDBContentChanged(
  ...
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  frontend_->IndexedDBContentUpdated(origin, databaseName, objectStoreName);
}
