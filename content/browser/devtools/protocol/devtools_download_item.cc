...
void DevToolsDownloadItem::ReadInternal() {
  ...
  if (state == content::DownloadSourceStream::EMPTY && stream_->length() < 0)
    InterruptWithReason(download::DOWNLOAD_INTERRUPT_REASON_NETWORK_FAILED);
}

void DevToolsDownloadItem::Cancel(bool user_cancel) {
  InterruptWithReason(user_cancel
                          ? download::DOWNLOAD_INTERRUPT_REASON_USER_CANCELED
                          : download::DOWNLOAD_INTERRUPT_REASON_USER_SHUTDOWN);
}

void DevToolsDownloadItem::Remove() {
  InterruptWithReason(download::DOWNLOAD_INTERRUPT_REASON_USER_CANCELED);
}
...
download::DownloadInterruptReason DevToolsDownloadItem::GetLastReason() const {
  return last_reason_;
}
...
download::DownloadDangerType DevToolsDownloadItem::GetDangerType() const {
...
void DevToolsDownloadItem::InterruptWithReason(
    download::DownloadInterruptReason reason) {
