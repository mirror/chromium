...
void DevToolsDownloadItem::Initialize(
  ...
  stream_->Initialize();
  stream_->RegisterDataReadyCallback(
      base::Bind(&DevToolsDownloadItem::MarkDataAvailable,
                 weak_ptr_factory_.GetWeakPtr()));
}

void DevToolsDownloadItem::MarkDataAvailable(MojoResult result) {
  if (reading_)
    ReadInternal();
}
...
void DevToolsDownloadItem::SimulateErrorForTesting(DownloadInterruptReason reason) {
