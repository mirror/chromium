...
class TargetHandler::Session : public DevToolsAgentHostClient {
 public:
  static std::string Attach(TargetHandler* handler,
    ...
    new Session(handler, agent_host, id, waiting_for_debugger);
    return id;
  }
  ...
  Session(TargetHandler* handler,
    ...
    handler_->attached_sessions_[id_].reset(this);
    static_cast<DevToolsAgentHostImpl*>(agent_host)->AttachMultiClient(this);
    handler_->frontend_->AttachedToTarget(id_, CreateInfo(agent_host),
...
Response TargetHandler::SetDiscoverTargets(bool discover) {
  ...
  if (discover_) {
    ...
    for (auto& it : reported_hosts_)
      frontend_->TargetDestroyed(it.second->GetId());
...
Response TargetHandler::AttachToTarget(const std::string& target_id,
  ...
  *session_id = Session::Attach(this, agent_host.get(), false);
  return Response::OK();
}
...
void TargetHandler::DevToolsAgentHostDestroyed(DevToolsAgentHost* host) {
  ...
  reported_hosts_.erase(host->GetId());
}
