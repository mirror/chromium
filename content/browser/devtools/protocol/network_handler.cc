...
void DeletedCookiesOnIO(base::OnceCallback<void()> callback,
                        uint32_t num_deleted) {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
...
void DeleteSelectedCookiesOnIO(net::URLRequestContextGetter* context_getter,
  ...
  if (normalized_domain.empty())
    normalized_domain = GURL(url_spec).host();
  else if (normalized_domain[0] != '.')
...
void SetCookieOnIO(net::URLRequestContextGetter* context_getter,
  ...
  if (!url_spec.empty()) {
    url = GURL(url_spec);
    secure = secure || url.SchemeIsCryptographic();
  } else {
    url = GURL((secure ? "https://" : "http://") + domain + path);
  }
  ...
  request_context->cookie_store()->SetCookieWithDetailsAsync(
      url, name, value, domain, path, base::Time(), expiration_date,
...
void NetworkHandler::DeleteCookies(
  ...
  if (!url.isJust() && !domain.isJust()) {
    callback->sendFailure(Response::InvalidParams(
        "At least one of the url and domain need to be specified"));
  }
  BrowserThread::PostTask(
      BrowserThread::IO, FROM_HERE,
