...
String getProtocol(const GURL& url, const ResourceResponseInfo& info) {
...
bool GetPostData(const ResourceRequestBody& request_body, std::string* result) {
  const std::vector<storage::DataElement>* elements = request_body.elements();
  if (elements->empty())
    return false;
  for (const auto& element : *elements) {
    if (element.type() != storage::DataElement::TYPE_BYTES)
      return false;
...
void NetworkHandler::NavigationRequestWillBeSent(
  ...
  const base::Optional<std::string>& initiator_str =
      nav_request.begin_params()->devtools_initiator;
  std::unique_ptr<Network::Initiator> initiator;
  if (initiator_str.has_value()) {
    std::unique_ptr<base::Value> value = base::JSONReader::Read(*initiator_str);
    std::unique_ptr<Value> protocol_value = toProtocolValue(value.get(), 1000);
    ErrorSupport ignored_errors;
    if (protocol_value)
      initiator =
          Network::Initiator::fromValue(protocol_value.get(), &ignored_errors);
  }
  if (!initiator) {
