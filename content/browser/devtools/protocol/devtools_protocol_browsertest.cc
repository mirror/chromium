...
// TODO(dvallet): Figure out why his test timeouts in Android, it might be due
// to the observer not working properly.
#if defined(OS_ANDROID)
...
IN_PROC_BROWSER_TEST_F(DevToolsDownloadContentTest, MAYBE_MultiDownload) {
  ...
  ASSERT_EQ(file1.MaybeAsASCII(), "download1/download-unknown-size");
  ...
  ASSERT_EQ(file2.MaybeAsASCII(), "download2/download-test.lib");
