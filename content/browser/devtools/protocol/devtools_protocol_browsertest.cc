...
//       allow_io_during_test_verification; bool read =
//       base::ReadFileToString(path, &file_contents); EXPECT_TRUE(read) <<
//       "Failed reading file: " << path.value() << std::endl; if (!read)
...
//   DownloadItem* StartDownloadAndReturnItem(HeadlessBrowser* browser, GURL
//   url) {
...
// // Check that download are denied by default.
...
//   std::unique_ptr<DownloadTestObserver> observer2(CreateWaiter(browser(),
//   1)); CreateWebContentsForURL(
