...
IN_PROC_BROWSER_TEST_F(DevToolsDownloadContentTest, StreamSingleDownload) {
  ...
  SendCommand("Page.enable", nullptr, true);
  SetDownloadBehavior("stream", "download");

  NavigationFinishedObserver navigation_finished_observer(
      shell()->web_contents());

  GURL test_url = embedded_test_server()->GetURL("/download/download-test.lib");
  shell()->LoadURL(test_url);
  ...
  LOG(ERROR) << "Waiting for Page.downloadStarted";
  std::unique_ptr<base::DictionaryValue> result =
...
// Check that downloading a single file works.
IN_PROC_BROWSER_TEST_F(DevToolsDownloadContentTest, StreamSingleDownload2) {
  base::ThreadRestrictions::SetIOAllowed(true);
  NavigateToURLBlockUntilNavigationsComplete(shell(), GURL("about:blank"), 1);
  Attach();
...
IN_PROC_BROWSER_TEST_F(DevToolsDownloadContentTest,
  ...
  SendCommand("Page.enable", nullptr, false);
  SetDownloadBehaviorNoWait("stream", "download", 10);
