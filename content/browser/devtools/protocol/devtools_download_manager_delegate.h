...
class CONTENT_EXPORT DevToolsDownloadManagerDelegate
 ...
 public:
  ...
  // Sets a proxy download delegate. When set, this delegate will use the
  // proxy's |GetNextId| function to ensure compatibility. It will also call its
  // |Shutdown| method when sutting down and it will fallback to the proxy if it
  // cannot find any DevToolsDownloadManagerHelper associated with the download.
  void SetProxyDelegate(content::DownloadManagerDelegate* proxy_delegate);
 ...
 private:
  friend class base::RefCountedThreadSafe<DevToolsDownloadManagerDelegate>;
  friend struct base::DefaultSingletonTraits<DevToolsDownloadManagerDelegate>;
  ...
  base::WeakPtrFactory<DevToolsDownloadManagerDelegate> weak_ptr_factory_;
