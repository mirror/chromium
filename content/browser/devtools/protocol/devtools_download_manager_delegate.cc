...
void DevToolsDownloadManagerDelegate::TakeOver(
    content::DownloadManager* download_manager) {
  proxy_download_delegate_ = download_manager->GetDelegate();
  SetDownloadManager(download_manager);
  download_manager->SetDelegate(this);
}
...
bool DevToolsDownloadManagerDelegate::DetermineDownloadTarget(
  ...
  if (!download_helper && proxy_download_delegate_) {
...
bool DevToolsDownloadManagerDelegate::ShouldOpenDownload(
  ...
  if (!DevToolsDownloadManagerHelper::FromWebContents(item->GetWebContents()) ||
      proxy_download_delegate_)
