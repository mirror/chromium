...
DevToolsDownloadManagerDelegate::GetInstance() {
  return base::Singleton<DevToolsDownloadManagerDelegate>::get();
}

void DevToolsDownloadManagerDelegate::TakeOver(
    content::DownloadManager* download_manager) {
  CHECK(download_manager);

  DevToolsDownloadManagerDelegate* download_delegate = GetInstance();
  if (download_manager->GetDelegate() == download_delegate)
    return;
  ...
  download_delegate->default_download_path_ =
      download_delegate->download_manager_->GetBrowserContext()
          ->GetPath()
...
bool DevToolsDownloadManagerDelegate::DetermineDownloadTarget(
  ...
  // Test if we should failback to delegate.
  if (proxy_download_delegate_ &&
      (!download_helper ||
       download_helper->GetDownloadBehavior() ==
           DevToolsDownloadManagerHelper::DownloadBehavior::DEFAULT))
  ...
  if (download_path.empty()) {
    download_path = default_download_path_;
  }
