...
void PageHandler::CaptureScreenshot(
  ...
  if (emulation_enabled && from_surface.fromMaybe(true)) {
    ...
    if (clip.isJust()) {
      // Multiplying by dpfactor here to save on the extra
      // RenderWidgetScreenMetricsEmulator / DevToolsEmulator delegate.
    ...
    if (clip.isJust()) {
      widget_host->GetView()->SetSize(gfx::ScaleToFlooredSize(
