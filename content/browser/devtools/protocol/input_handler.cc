...
void InputHandler::SetRenderFrameHost(RenderFrameHostImpl* host) {
  ...
  if (host) {
    ...
    DCHECK(host_->GetRenderWidgetHost()->GetView());
    mouse_wheel_phase_handler_.reset(new MouseWheelPhaseHandler(
        host_->GetRenderWidgetHost(), host_->GetRenderWidgetHost()->GetView()));
  }
}
