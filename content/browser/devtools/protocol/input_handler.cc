...
bool DoubleToSyntheticPointerActionParamsButton(
    double button,
...
void InputHandler::DispatchPointerActions(
  ...
  for (size_t i = 0; i < pointer_actions->length(); ++i) {
    ...
    if (action_type != content::SyntheticPointerActionParams::PointerActionType::IDLE) {
      // convert button
      if (action->HasButton()) {
        if (action_type ==
        ...
        if (!DoubleToSyntheticPointerActionParamsButton(action->GetButton(0),
                                                        action_button)) {
          callback->sendFailure(Response::InvalidParams(
      ...
      if (action->HasX() && action->HasY()) {
        if (action_type ==
          ...
          callback->sendFailure(
              Response::InvalidParams("Cannot pass x and y for a release or up action"));
          return;
        }

        if (action_type == content::SyntheticPointerActionParams::PointerActionType::MOVE) {
          if (!action->HasInitialX() || !action->HasInitialY()) {
            callback->sendFailure(
                Response::InvalidParams("Must provide initialX and initialY for a move action"));
            return;
          }
          int num_frames =  static_cast<int>(std::ceil(
                action->GetDuration(0) / viz::BeginFrameArgs::DefaultInterval().InSecondsF()));
          for (int j = 0; j < num_frames; ++j) {
            int x = static_cast<int>((j / (double) num_frames) * (action->GetX(0) - action->GetInitialX(0)));
            int y = static_cast<int>((j / (double) num_frames) * (action->GetY(0) - action->GetInitialY(0)));
            LOG(WARNING) << x;
            LOG(WARNING) << y;
            LOG(WARNING) << "";
    ...
    if (action_type != content::SyntheticPointerActionParams::PointerActionType::MOVE) {
      int num_idle = static_cast<int>(std::ceil(
            action->GetDuration(0) / viz::BeginFrameArgs::DefaultInterval().InSecondsF()));
