...
DevToolsFrontendHostImpl::DevToolsFrontendHostImpl(
    ...
    : frame_host_(frame_host),
      ...
      binding_(this) {
  frame_host_->GetRemoteAssociatedInterfaces()->GetInterface(&frontend_);
  ...
  frontend_->SetupDevToolsFrontend(api_script, std::move(host));
}
...
void DevToolsFrontendHostImpl::BadMessageRecieved() {
  bad_message::ReceivedBadMessage(frame_host_->GetProcess(),
