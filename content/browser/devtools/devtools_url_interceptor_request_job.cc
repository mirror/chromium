...
bool DevToolsURLInterceptorRequestJob::InterceptedRequest::IsReceivingBody() {
  return fetch_in_progress_ && read_started_ &&
         read_response_result_ == net::ERR_IO_PENDING;
}
...
void DevToolsURLInterceptorRequestJob::ProcessInterceptionRespose(
  ...
  if (redirect_) {
    ...
    if (!static_cast<InterceptedRequest*>(sub_request_.get())
             ->IsReceivingBody())
      NotifyHeadersComplete();
    return;
  } else {
