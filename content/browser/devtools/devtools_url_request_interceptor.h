...
// This class is constructed on UI thread by only accessed on IO thread.
class DevToolsURLRequestInterceptor : public net::URLRequestInterceptor {
 public:
  ...
  DevToolsURLInterceptorRequestJob* MaybeCreateDevToolsURLInterceptorRequestJob(
      net::URLRequest* request,
      net::NetworkDelegate* network_delegate);
  ...
  std::unique_ptr<DevToolsTargetRegistry::LookupHandle> target_lookup_;
