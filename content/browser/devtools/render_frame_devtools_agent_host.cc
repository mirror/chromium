...
void RenderFrameDevToolsAgentHost::AttachSession(DevToolsSession* session) {
  ...
  protocol::EmulationHandler* emulation_handler = new protocol::EmulationHandler();
  ...
  if (frame_tree_node_ && !frame_tree_node_->parent()) {
    session->AddHandler(base::WrapUnique(new protocol::PageHandler(emulation_handler)));
