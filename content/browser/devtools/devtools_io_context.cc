...
class TempFileStream : public DevToolsIOContext::RWStream {
 public:
  ...
  void Close(bool invokePendingCallbacks) override {}
...
class BlobStream : public DevToolsIOContext::ROStream {
 ...
 private:
  ...
  void CloseOnIO(bool invokePendingCallbacks);
...
void BlobStream::Read(off_t position, size_t max_size, ReadCallback callback) {
  std::unique_ptr<ReadRequest> request =
      base::MakeUnique<ReadRequest>(position, max_size, std::move(callback));
  BrowserThread::PostTask(
      BrowserThread::IO, FROM_HERE,
      base::BindOnce(&BlobStream::ReadOnIO, this, std::move(request)));
}

void BlobStream::Close(bool invokePendingCallbacks) {
  BrowserThread::PostTask(
      BrowserThread::IO, FROM_HERE,
      base::BindOnce(&BlobStream::CloseOnIO, this, invokePendingCallbacks));
}
...
void BlobStream::CloseOnIO(bool invokePendingCallbacks) {
  if (blob_reader_) {
    blob_reader_->Kill();
    blob_reader_.reset();
  }
