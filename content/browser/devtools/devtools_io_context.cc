...
class DownloadStream : public DevToolsIOContext::ROStream {
 ...
 private:
  ...
  void OpenOnDownload(protocol::DevToolsDownloadManager* download_manager,
                      const std::string& download_guid);
...
void DownloadStream::Open(const std::string& download_guid) {
  GetDownloadTaskRunner()->PostTask(
      FROM_HERE,
      base::BindOnce(&DownloadStream::OpenOnDownload, this,
...
void DownloadStream::OpenOnDownload(
    protocol::DevToolsDownloadManager* download_manager,
    const std::string& download_guid) {
  DevToolsDownloadItem* download_item =
      GetDownloadItem(download_manager, download_guid);
  if (!download_item) {
    failed_ = true;
    return;
  }
...
void DownloadStream::Close(bool invoke_pending_callbacks) {
  ...
  download_manager->CloseDownload(guid_);
}
