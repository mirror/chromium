...
// static
// TODO(dvallet): Is it ok to define static method here?
...
class DownloadStream : public DevToolsIOContext::ROStream {
 public:
  DownloadStream(content::BrowserContext* context)
      : DevToolsIOContext::ROStream(
            BrowserThread::GetTaskRunnerForThread(BrowserThread::IO)),
 ...
 private:
  ...
  DevToolsDownloadItem* GetDownloadItem(const std::string& download_guid);

  void ReadOnDownload(DevToolsDownloadItem* download_item,
                      size_t max_size,
                      ReadCallback callback);
  void OnReadComplete(int bytes_read);
...
void DownloadStream::Open(const std::string& download_guid) {
  DevToolsDownloadItem* download_item = GetDownloadItem(download_guid);
  if (!download_item) {
    failed_ = true;
    return;
  }
...
void DownloadStream::Read(off_t position,
  ...
  DevToolsDownloadItem* download_item = GetDownloadItem(guid_);
  if (!download_item) {
    BrowserThread::PostTask(BrowserThread::UI, FROM_HERE,
                            base::BindOnce(std::move(callback), nullptr, false,
                                           ROStream::StatusFailure));
  ...
  GetDownloadTaskRunner()->PostTask(
      FROM_HERE, base::BindOnce(&DownloadStream::ReadOnDownload, this,
                                download_item, max_size, std::move(callback)));
}

void DownloadStream::ReadOnDownload(DevToolsDownloadItem* download_item,
                                    size_t max_size,
                                    ReadCallback callback) {
...
void DownloadStream::ReadComplete(DevToolsDownloadItem* download_item,
  ...
  // TODO(dvallet) Is this needed. Maybe we want to run in task runner?
  BrowserThread::PostTask(
...
void DownloadStream::Close(bool invoke_pending_callbacks) {
  ...
  if (!download_item)
    return;
  // Since we found the download, we can be certain to be dealing with a
  // DevToolsDownloadManager.
  protocol::DevToolsDownloadManager* download_manager =
      protocol::DevToolsDownloadManager::FromBrowserContext(browser_context_);
  if (download_manager)
