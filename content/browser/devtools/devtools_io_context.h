 ...
 public:
  class Stream : public base::RefCountedDeleteOnSequence<Stream> {
   public:
    ...
    using ReadCallback = base::OnceCallback<void(std::string data, int status)>;
   ...
   private:
    Stream(base::SequencedTaskRunner* task_runner);
    ~Stream();
    ...
    void AppendOnFileSequence(std::string data);
