...
PipeWrapper::PipeWrapper(base::WeakPtr<DevToolsPipeHandler> devtools_handler,
  ...
  socket_.reset(new base::SyncSocket(handle));
...
void PipeWrapper::ReadLoop() {
  while (true) {
    ...
    size_t available = socket_->Peek();
    int result1 = 0;
    if (!available) {
      result1 = socket_->Receive(read_buffer_->data(), 1);
      read_buffer_->DidRead(result1);
      available = socket_->Peek();
    }
    size_t capacity = read_buffer_->RemainingCapacity();

    fprintf(stderr, "available %zu\n", available);

    int result2 = socket_->Receive(read_buffer_->data(),
                                   available > capacity ? capacity : available);

    read_buffer_->DidRead(result2);

    if (!HandleReadResult(result1 + result2))
      return;
  }
}
...
bool PipeWrapper::HandleReadResult(int result) {
  ...
  std::string mmm(read_buffer_->StartOfBuffer(), read_buffer_->GetSize());
...
void PipeWrapper::Write(const std::string& message) {
  ...
  while (write_buffer_->GetSizeToWrite()) {
    int result =
        socket_->Send(write_buffer_->data(), write_buffer_->GetSizeToWrite());
    if (result == 0) {
