...
// Remote-bound logs are saved locally with:
// * One byte for the version (for the encoding, metadata format, etc.)
// * Three bytes encoding the length of the metadata, in bytes.
// * Metadata (unless its length was zero) provided by the user (JS hooks).
// * The actual WebRTC event log.
...
class LogFileWriter {
 protected:
  ...
  // Given a peer connection and its associate log file, and given a log
