...
#include "content/public/browser/render_process_host.h"  // TODO: !!!
...
bool WebRtcEventLogManagerLocalLogsHandler::PeerConnectionAdded(
...
bool WebRtcEventLogManagerLocalLogsHandler::PeerConnectionRemoved(
    int render_process_id,
    int lid) {
...
bool WebRtcEventLogManagerLocalLogsHandler::EnableLocalLogging(
...
void WebRtcEventLogManagerLocalLogsHandler::SetLocalLogsObserver(
    WebRtcLocalEventLogsObserver* observer) {
  observer_ = observer;
}

void WebRtcEventLogManagerLocalLogsHandler::InjectClockForTesting(
    base::Clock* clock) {
  clock_for_testing_ = clock;
}

void WebRtcEventLogManagerLocalLogsHandler::StartLocalLogFile(
    int render_process_id,
    int lid) {
  ...
  local_logs_.emplace(key, LogFile(std::move(file), max_log_file_size_bytes_));

  // The observer needs to be able to run on any TaskQueue.
  if (observer_) {
    observer_->OnLocalLogsStarted(key, file_path);
  }
...
void WebRtcEventLogManagerLocalLogsHandler::StopLocalLogFile(
    int render_process_id,
    int lid) {
...
WebRtcEventLogManagerLocalLogsHandler::CloseLocalLogFile(
  ...
  if (observer_) {
    observer_->OnLocalLogsStopped(peer_connection);
  }

  return it;
}

base::FilePath WebRtcEventLogManagerLocalLogsHandler::GetFilePath(
