...
class CONTENT_EXPORT RtcEventLogManager {
 public:
  ...
  void RtcEventLogOpen(int render_process_id, int local_pc_id);
  void RtcEventLogClose(int render_process_id, int local_pc_id);
 ...
 private:
  ...
  std::string GetFilePath(int render_process_id, int local_pc_id) const;

  void OpenLogFile(const std::string& path);

  const std::string base_file_path_;
