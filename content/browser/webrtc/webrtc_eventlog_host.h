...
class CONTENT_EXPORT WebRTCEventLogHost {
 public:
  ...
  // PeerConnection on the render process. If local RTC event logging wasn't in
  // progress, this call will return false.
  // TODO(eladalon): The comment about the return value is unfortunately wrong.
  // In a separate CL, either correct the code or, more likely, remove it.
 ...
 private:
  ...
  // TODO(eladalon): It would be more natural to handle this from
  // WebRtcEventLogManager, once it has the power to instruct WebRTC to
  // stop/start logging.
