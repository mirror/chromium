...
WebRtcEventLogUploaderImpl::WebRtcEventLogUploaderImpl(
  ...
  // https://crbug.com/775415
  observer->OnWebRtcEventLogUploadComplete(true);
}
...
WebRtcEventLogUploaderImplFactory::WebRtcEventLogUploaderImplFactory(
    WebRtcEventLogUploaderObserver* observer)
    : observer_(observer) {}

std::unique_ptr<WebRtcEventLogUploader>
WebRtcEventLogUploaderImplFactory::Create(
    const base::FilePath& log_file) const {
  return std::make_unique<WebRtcEventLogUploaderImpl>(observer_, log_file);
}
