...
void WebRtcEventLogManager::OnWebRtcEventLogWrite(
  ...
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&WebRtcEventLogManager::OnWebRtcEventLogWriteInternal,
                                base::Unretained(this), render_process_id, lid,
...
void WebRtcEventLogManager::PeerConnectionAddedInternal(
  ...
  const bool result = local_logs_handler_.PeerConnectionAdded(render_process_id, lid);
  if (reply) {
...
void WebRtcEventLogManager::PeerConnectionRemovedInternal(
    ...
    base::OnceCallback<void(bool)> reply) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  const bool result = local_logs_handler_.PeerConnectionRemoved(render_process_id, lid);
  if (reply) {
    BrowserThread::PostTask(BrowserThread::UI, FROM_HERE,
                            base::BindOnce(std::move(reply), result));
  }
...
void WebRtcEventLogManager::EnableLocalLoggingInternal(
    ...
    base::OnceCallback<void(bool)> reply) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  const bool result = local_logs_handler_.EnableLocalLogging(base_path, max_file_size_bytes);
    if (reply) {
    BrowserThread::PostTask(BrowserThread::UI, FROM_HERE,
                            base::BindOnce(std::move(reply), result));
  }
...
void WebRtcEventLogManager::OnWebRtcEventLogWriteInternal(
    ...
    base::OnceCallback<void(bool)> reply) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  const bool result = local_logs_handler_.WriteToLocalLogFile(render_process_id, lid, output);
  if (reply) {
    BrowserThread::PostTask(BrowserThread::UI, FROM_HERE,
                            base::BindOnce(std::move(reply), result));
  }
