...
class PeerConnectionTrackerProxyImpl : public PeerConnectionTrackerProxy {
 public:
  ...
  void StartEventLogOutput(WebRtcEventLogPeerConnectionKey pc_key) override {
    RenderProcessHost* host =
        RenderProcessHost::FromID(pc_key.render_process_id);
    if (!host) {
      return;  // The host has been asynchronously removed; not a problem.
    }
    host->Send(new PeerConnectionTracker_StartEventLogOutput(pc_key.lid));
  }
  ...
  void StopEventLogOutput(WebRtcEventLogPeerConnectionKey pc_key) override {
    ...
    host->Send(new PeerConnectionTracker_StopEventLog(pc_key.lid));
  }
};
...
void WebRtcEventLogManager::OnRemoteLogStarted(PeerConnectionKey pc_key,
                                               base::FilePath file_path) {
  if (remote_logs_observer_) {
    remote_logs_observer_->OnRemoteLogStarted(pc_key, file_path);
  }
}

void WebRtcEventLogManager::OnRemoteLogStopped(
    WebRtcEventLogPeerConnectionKey pc_key) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  OnLoggingTargetStopped(LoggingTarget::kRemoteLogging, pc_key);
  if (remote_logs_observer_) {
    remote_logs_observer_->OnRemoteLogStopped(pc_key);
  }
}
...
void WebRtcEventLogManager::OnLoggingTargetStarted(LoggingTarget target,
  ...
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  auto it = peer_connections_with_event_logging_enabled_.find(pc_key);
  if (it != peer_connections_with_event_logging_enabled_.end()) {
    ...
    peer_connections_with_event_logging_enabled_.emplace(pc_key, target);
    BrowserThread::PostTask(
        BrowserThread::UI, FROM_HERE,
        base::BindOnce(&WebRtcEventLogManager::UpdateWebRtcEventLoggingState,
                       base::Unretained(this), pc_key, true));
  }
}
...
void WebRtcEventLogManager::OnLoggingTargetStopped(LoggingTarget target,
  ...
  if (it->second == 0u) {
    ...
    BrowserThread::PostTask(
        BrowserThread::UI, FROM_HERE,
        base::BindOnce(&WebRtcEventLogManager::UpdateWebRtcEventLoggingState,
                       base::Unretained(this), pc_key, false));
  }
}
...
void WebRtcEventLogManager::StartRemoteLoggingInternal(
  ...
  if (result) {
    ...
    OnLoggingTargetStarted(LoggingTarget::kRemoteLogging, pc_key);
  }
  if (reply) {
    BrowserThread::PostTask(BrowserThread::UI, FROM_HERE,
                            base::BindOnce(std::move(reply), result));
...
void WebRtcEventLogManager::InjectPeerConnectionTrackerProxy(
