...
void WebRtcEventLogManager::EnableLocalLoggingInternal(
  ...
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  const bool result =
      local_logs_handler_.EnableLogging(base_path, max_file_size_bytes);
  if (reply) {
    BrowserThread::PostTask(BrowserThread::UI, FROM_HERE,
                            base::BindOnce(std::move(reply), result));
  }
...
void WebRtcEventLogManager::DisableLocalLoggingInternal(
    base::OnceCallback<void(bool)> reply) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  const bool result = local_logs_handler_.DisableLogging();
  if (reply) {
    BrowserThread::PostTask(BrowserThread::UI, FROM_HERE,
                            base::BindOnce(std::move(reply), result));
  }
...
void WebRtcEventLogManager::OnWebRtcEventLogWriteInternal(
  ...
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  const bool result =
      local_logs_handler_.EventLogWrite(render_process_id, lid, output);
  if (reply) {
    BrowserThread::PostTask(BrowserThread::UI, FROM_HERE,
                            base::BindOnce(std::move(reply), result));
  }
...
void WebRtcEventLogManager::UpdateWebRtcEventLoggingState(
    PeerConnectionKey peer_connection,
    bool enabled) {
  // TODO: Add unit-tests that would make sure that we really instruct
  // WebRTC to start/stop event logs. https://crbug.com/775415
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
