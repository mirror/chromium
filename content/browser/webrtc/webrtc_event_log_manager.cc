...
WebRtcEventLogManager::~WebRtcEventLogManager() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);  // TODO(eladalon): !!!
...
void WebRtcEventLogManager::LocalWebRtcEventLogStart(
  ...
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  auto task = base::BindOnce(&WebRtcEventLogManager::StartLocalLogFile,
                             base::Unretained(this), render_process_id, lid,
                             base_path, max_file_size);
  if (!reply) {
    file_task_runner_->PostTask(FROM_HERE, std::move(task));
  } else {
    file_task_runner_->PostTaskAndReply(FROM_HERE, std::move(task),
                                        std::move(reply));
  }
}
...
void WebRtcEventLogManager::OnWebRtcEventLogWrite(int render_process_id,
  ...
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  auto task =
      base::Bind(&WebRtcEventLogManager::WriteToLocalLogFile,
