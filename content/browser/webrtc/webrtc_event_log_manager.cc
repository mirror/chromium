  ...
  void SetWebRtcEventLoggingState(WebRtcEventLogPeerConnectionKey key,
    ...
    BrowserThread::PostTask(
        ...
        base::BindOnce(
            ...
            base::Unretained(this), key, event_logging_enabled));
  }

 private:
  void SetWebRtcEventLoggingStateInternal(WebRtcEventLogPeerConnectionKey key,
...
void WebRtcEventLogManager::DisableForBrowserContext(
    ...
    base::OnceClosure reply) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  // |browser_context| is being destructed; we may not check if it was
  // of the records; we can only use it as an identifier. If it was indeed
  // off the records, and therefore never enabled, we'll just ignore it.
  task_runner_->PostTask(
      FROM_HERE,
...
void WebRtcEventLogManager::StartRemoteLogging(
  ...
  task_runner_->PostTask(
      ...
      base::BindOnce(&WebRtcEventLogManager::StartRemoteLoggingInternal,
                     base::Unretained(this), render_process_id, lid,
                     browser_context, browser_context->GetPath(),
...
void WebRtcEventLogManager::OnWebRtcEventLogWrite(
  ...
  task_runner_->PostTask(
      ...
      base::BindOnce(&WebRtcEventLogManager::OnWebRtcEventLogWriteInternal,
                     base::Unretained(this), render_process_id, lid,
                     GetBrowserContext(render_process_id), message,
                     std::move(reply)));
}
...
void WebRtcEventLogManager::EnableForBrowserContextInternal(
    ...
    base::OnceClosure reply) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  remote_logs_manager_.EnableForBrowserContext(browser_context,
                                               browser_context_dir);
  if (reply) {
...
void WebRtcEventLogManager::DisableForBrowserContextInternal(
    ...
    base::OnceClosure reply) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  remote_logs_manager_.DisableForBrowserContext(browser_context);
  if (reply) {
    BrowserThread::PostTask(BrowserThread::UI, FROM_HERE, std::move(reply));
  }
}
...
void WebRtcEventLogManager::StartRemoteLoggingInternal(
  ...
  if (browser_context->IsOffTheRecord()) {
    ...
    result = remote_logs_manager_.StartRemoteLogging(
        render_process_id, lid, browser_context, browser_context_dir,
        max_file_size_bytes);
  }
