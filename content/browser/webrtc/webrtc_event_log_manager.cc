...
inline void MaybeReplyWithBool(const base::Location& from_here,
  ...
  if (reply) {
    BrowserThread::PostTask(BrowserThread::UI, from_here,
                            base::BindOnce(reply, value));
  }
}

struct MaybeReplyWithBoolOnExit final {
  explicit MaybeReplyWithBoolOnExit(base::Callback<void(bool)>& reply,
                                    bool default_value)
      : reply(reply), value(default_value) {}
  ~MaybeReplyWithBoolOnExit() {
    if (reply) {
      // FROM_HERE loses a bit of meaning this way, but it's worth it.
      MaybeReplyWithBool(FROM_HERE, reply, value);
    }
  }
  base::Callback<void(bool)>& reply;
  bool value;
...
struct MaybeReplyWithFilePathOnExit final {
  explicit MaybeReplyWithFilePathOnExit(
      base::Callback<void(base::FilePath)>& reply)
      : reply(reply) {}
  ~MaybeReplyWithFilePathOnExit() {
    if (reply) {
      // FROM_HERE loses a bit of meaning this way, but it's worth it.
      BrowserThread::PostTask(BrowserThread::UI, FROM_HERE,
                              base::BindOnce(reply, file_path));
    }
  }
  base::Callback<void(base::FilePath)>& reply;
...
void WebRtcEventLogManager::LocalWebRtcEventLogStart(
  ...
  file_task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&WebRtcEventLogManager::StartLocalLogFile,
                                base::Unretained(this), render_process_id, lid,
                                base_path, max_file_size, reply));
}
...
void WebRtcEventLogManager::LocalWebRtcEventLogStop(
  ...
  file_task_runner_->PostTask(
      ...
      base::Bind(&WebRtcEventLogManager::StopLocalLogFile,
                 base::Unretained(this), render_process_id, lid, reply));
}
...
// TODO(eladalon): Closure to ensure exit?
...
void WebRtcEventLogManager::StartLocalLogFile(
  ...
  MaybeReplyWithFilePathOnExit on_exit(reply);
  ...
  // TODO(eladalon): Handle known issue - https://crbug.com/786374
...
void WebRtcEventLogManager::StopLocalLogFile(int render_process_id,
                                             int lid,
                                             base::Callback<void(bool)> reply) {
  DCHECK(file_task_runner_->RunsTasksInCurrentSequence());
  ...
  if (it != local_logs_.end()) {
    ...
    MaybeReplyWithBool(FROM_HERE, reply, true);
  } else {
    MaybeReplyWithBool(FROM_HERE, reply, false);
  }
}
...
void WebRtcEventLogManager::WriteToLocalLogFile(
  ...
  // This object's destructor will ensure a reply (if |reply| is non-empty),
  // regardless of which path is taken out of the function.
  MaybeReplyWithBoolOnExit on_exit(reply, false);

  // TODO(eladalon): !!! Unit-test for empty string.
