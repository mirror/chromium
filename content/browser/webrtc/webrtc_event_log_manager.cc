...
WebRtcEventLogManager::WebRtcEventLogManager()
    : file_closure_observer_for_testing_(nullptr),
...
void WebRtcEventLogManager::SetObserverForTesting(
    FileClosureObserverForTesting* observer) {
  file_closure_observer_for_testing_ = observer;
}
...
void WebRtcEventLogManager::StopLocalLogFile(int render_process_id, int lid) {
  ...
  if (it != local_logs_.end()) {
    ...
    if (file_closure_observer_for_testing_ != nullptr) {
      file_closure_observer_for_testing_->OnFileClosed(render_process_id, lid);
    }
  }
}
