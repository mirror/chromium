...
class WebRtcEventLogManagerTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    manager_->OnBrowserContextInitialized(&default_browser_context_,
                                          VoidReplyClosure());
    WaitForReply();
  }
  ...
  TestBrowserContext default_browser_context_;
  MockRenderProcessHost rph_;
...
TEST_F(WebRtcEventLogManagerTest, LocalLogMultipleActiveFiles) {
  ...
  for (size_t i = 0; i < 3; i++) {
    rphs.emplace_back(&default_browser_context_);
  };
...
TEST_F(WebRtcEventLogManagerTest,
       OnBrowserContextInitializedCreatesDirectoryForRemoteLogs) {
  printf("First = %p\n", &default_browser_context_);
  TestBrowserContext browser_context;
  printf("second = %p\n", &browser_context);
  manager_->OnBrowserContextInitialized(&browser_context, VoidReplyClosure());
  WaitForReply();
...
TEST_F(WebRtcEventLogManagerTest, StartRemoteLoggingCreatesFile) {
  std::cout << default_browser_context_.GetPath() << std::endl;  // TODO: !!!!!
  ASSERT_TRUE(PeerConnectionAdded(rph_.GetID(), kPeerConnectionId));
  EXPECT_TRUE(StartRemoteLogging(rph_.GetID(), kPeerConnectionId));
  while (1);
}
