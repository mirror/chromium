...
#if defined(OS_WIN)
#define ToStringType(x) std::to_wstring(x)
#else
#define ToStringType(x) std::to_string(x)
#endif
...
TEST_F(WebRtcEventLogManagerTest, LocalLogFilenameMatchesExpectedFormat) {
  ...
  expected_path = base_path.InsertBeforeExtension(
      ...
      FILE_PATH_LITERAL("_") + ToStringType(kRenderProcessId) +
      FILE_PATH_LITERAL("_") + ToStringType(kLocalPeerConnectionId));
