...
class MockLocalLogsObserver : public WebRtcEventLogManager::LocalLogsObserver {
 public:
  ...
  MOCK_METHOD1(OnLocalLogsStarted, void(const std::map<PeerConnectionKey, base::FilePath>&));
  MOCK_METHOD1(OnLocalLogsStopped, void(const std::set<PeerConnectionKey>&));
};
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  EXPECT_CALL(observer, OnLocalLogsStarted(_)).Times(0);
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  MockLocalLogsObserver observer;
  SetLocalLogsObserver(&observer);
  EXPECT_CALL(observer, OnLocalLogsStarted(_)).Times(1);
  ASSERT_TRUE(PeerConnectionAdded(kRenderProcessId, kLocalPeerConnectionId));
  ASSERT_TRUE(EnableLocalLogging());
}
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  SetLocalLogsObserver(&observer);
  EXPECT_CALL(observer, OnLocalLogsStopped(_)).Times(1);
  ASSERT_TRUE(PeerConnectionAdded(kRenderProcessId, kLocalPeerConnectionId));
  ASSERT_TRUE(EnableLocalLogging());
  ASSERT_TRUE(DisableLocalLogging());
}
