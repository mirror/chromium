...
TEST_F(WebRtcEventLogManagerTest, UploadOrderDependsOnLastModificationTime) {
  ...
  // If you need to change this, you also need to rewrite the test to use
  // different BrowserContexts, to allows this many pending logs.
  DCHECK_GE(kMaxNumberActiveRemoteWebRtcEventLogFiles, 3u);

  const PeerConnectionKey untracked(rph_->GetID(), 0);
  const PeerConnectionKey tracked[3] = {PeerConnectionKey(rph_->GetID(), 1),
                                        PeerConnectionKey(rph_->GetID(), 2),
                                        PeerConnectionKey(rph_->GetID(), 3)};

  // Suppresses the uploading of the "tracked" peer connections' logs.
  ASSERT_TRUE(PeerConnectionAdded(untracked.render_process_id, untracked.lid));

  // Create remote-bound logs.
  base::Optional<base::FilePath> log_files[arraysize(tracked)];
  for (int i = 0; i < 3; i++) {
    ON_CALL(observer, OnRemoteLogStarted(tracked[i], _))
        .WillByDefault(Invoke(SaveFilePathTo(&log_files[i])));
    ASSERT_TRUE(
        PeerConnectionAdded(tracked[i].render_process_id, tracked[i].lid));
    ASSERT_TRUE(
        StartRemoteLogging(tracked[i].render_process_id, tracked[i].lid));
    ASSERT_TRUE(log_files[i]);
    ASSERT_TRUE(
        PeerConnectionRemoved(tracked[i].render_process_id, tracked[i].lid));
  }

  // Use the same last-accessed time for all files, showing it does not matter.
  base::File::Info file_info;
  ASSERT_TRUE(base::GetFileInfo(*log_files[0], &file_info));
  const base::Time shared_last_accessed_time = file_info.last_accessed;

  // Pick a non-trivial permutation and set the files' last modifiction
  // timestamps to follow.
  base::Time last_modified =
      base::Time::Now() - base::TimeDelta::FromSeconds(3);
  const size_t permutation[3] = {2, 0, 1};
  std::list<base::FilePath> expected_files;
  for (size_t i = 0; i < 3; i++) {
    const base::FilePath& path = *log_files[permutation[i]];

    last_modified += base::TimeDelta::FromSeconds(3);  // Back to the future...
    ASSERT_TRUE(
        base::TouchFile(path, shared_last_accessed_time, last_modified));

    expected_files.push_back(path);
  }

  // Show that, even though the files were created in order 0-1-2, their upload
  // occurs in the order of last modification.
  base::RunLoop run_loop;
  SetWebRtcEventLogUploaderFactoryForTesting(
      std::make_unique<FileListExpectingWebRtcEventLogUploader::Factory>(
          &expected_files, true, &run_loop));

  // Uploading enabled by removing the last active peer connection.
  ASSERT_TRUE(
      PeerConnectionRemoved(untracked.render_process_id, untracked.lid));
  run_loop.Run();
}
