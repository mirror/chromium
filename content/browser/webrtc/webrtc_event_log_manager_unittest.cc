...
class WebRtcEventLogManagerTest : public ::testing::Test {
 protected:
  ...
  void FreezeClockAt(const base::Time::Exploded& frozen_time_exploded) {
    base::Time fronzen_time;
    if (!base::Time::FromLocalExploded(frozen_time_exploded, &fronzen_time)) {
      EXPECT_TRUE(false);
    }
    frozen_clock_.SetNow(fronzen_time);
...
TEST_F(WebRtcEventLogManagerTest, LocalLogFilenameMatchesExpectedFormat) {
  ...
  const std::string user_defined_portion = "user_defined_portion";
  const base::FilePath path = base_dir_.Append(user_defined_portion);

  const base::FilePath file_path = LocalWebRtcEventLogStart(
      kRenderProcessId, kLocalPeerConnectionId, path, kMaxFileSizeBytes);

  // [user_defined]_[date]_[time]_[pid]_[lid].log
  const std::string date = "20170906";
  const std::string time = "1043";
  std::string expected_filename = user_defined_portion + "_" + date + "_" +
                                  time + "_" +
                                  std::to_string(kRenderProcessId) + "_" +
                                  std::to_string(kLocalPeerConnectionId);
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  const base::FilePath file_path_2 = LocalWebRtcEventLogStart(
      kRenderProcessId, kLocalPeerConnectionId, path, kMaxFileSizeBytes);
  const std::string expected_filename_2 = expected_filename + " (1)" +
                                          base::FilePath::kExtensionSeparator +
                                          std::string("log");
...
TEST_F(WebRtcEventLogManagerTest, LocalLogMayNotBeStartedTwice) {
  const base::FilePath file_path = LocalWebRtcEventLogStart(
      kRenderProcessId, kLocalPeerConnectionId, base_path_, kMaxFileSizeBytes);
  ASSERT_FALSE(file_path.empty());
  // TODO(eladalon): Known issue - this will pass even if we comment out
  // the DCHECK that is intended to crash. We know that that DCHECK is
  // triggered, but after a fork(), something else also ends up in a bad state.
