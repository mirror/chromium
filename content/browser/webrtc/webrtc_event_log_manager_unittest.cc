  ...
  class Factory : public WebRtcEventLogUploader::Factory {
   public:
    ...
    std::unique_ptr<WebRtcEventLogUploader> Create(
        const base::FilePath& log_file) override {
...
class SetResultWebRtcEventLogUploader : public WebRtcEventLogUploader {
 public:
  SetResultWebRtcEventLogUploader(WebRtcEventLogUploaderObserver* observer,
  ...
  class Factory : public WebRtcEventLogUploader::Factory {
   public:
    Factory(WebRtcEventLogUploaderObserver* observer, bool result)
        : observer_(observer), result_(result) {}
    ~Factory() override = default;

    std::unique_ptr<WebRtcEventLogUploader> Create(
        const base::FilePath& log_file) override {
      return std::make_unique<SetResultWebRtcEventLogUploader>(
          observer_, log_file, result_);
    }
...
class FileListExpectingWebRtcEventLogUploader : public WebRtcEventLogUploader {
 public:
  ...
  explicit FileListExpectingWebRtcEventLogUploader(
    ...
    observer->OnWebRtcEventLogUploadComplete(base::FilePath(), true);
  }
  ...
  class Factory : public WebRtcEventLogUploader::Factory {
   public:
    Factory(WebRtcEventLogUploaderObserver* observer,
...
TEST_P(WebRtcEventLogManagerTest, FinishedRemoteLogsUploadedAndFileDeleted) {
  ...
  SetWebRtcEventLogUploaderFactoryForTesting(
      std::make_unique<SetResultWebRtcEventLogUploader::Factory>(
          this, upload_result));
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  // Create a directory and seed it with log files, simulating the creation of
  // logs in a previous session.
  std::list<base::FilePath> expected_files;
  base::ScopedTempDir browser_context_dir;
  ASSERT_TRUE(browser_context_dir.CreateUniqueTempDir());
