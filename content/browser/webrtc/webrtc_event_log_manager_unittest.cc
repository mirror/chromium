...
class WebRtcEventLogManagerTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    default_browser_context_ = CreateBrowserContext();
    rph_ =
        std::make_unique<MockRenderProcessHost>(default_browser_context_.get());
  }
  ...
  std::unique_ptr<TestBrowserContext> default_browser_context_;
...
TEST_F(WebRtcEventLogManagerTest, LocalLogMultipleActiveFiles) {
  ...
  for (size_t i = 0; i < 3; i++) {
    rphs.emplace_back(default_browser_context_.get());
  };
