...
// TODO(eladalon): When time allows, add unit tests for incognito mode.
...
TEST_F(WebRtcEventLogManagerTest,
       RemoteLogLimitOnActiveLogFilesUpdatedWhenLogFileClosed) {
  NiceMock<MockWebRtcRemoteEventLogsObserver> observer;
  SetRemoteLogsObserver(&observer);
  ...
  const std::string log = "Look behind you.";
  ...
  // By writing to one of the previously opened until it reaches capacity and is
  // closed, we make room for another remote-bound log.
  EXPECT_EQ(OnWebRtcEventLogWrite(rph_->GetID(), 0, log),
            std::make_pair(false, true));

  const PeerConnectionKey another_pc(rph_->GetID(), i);
  EXPECT_CALL(observer, OnRemoteLogStarted(another_pc, _)).Times(1);
  EXPECT_TRUE(StartRemoteLogging(another_pc.render_process_id, another_pc.lid));
}
