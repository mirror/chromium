...
class WebRtcEventLogManagerTest : public ::testing::TestWithParam<bool>,
 ...
 public:
  ...
  ~WebRtcEventLogManagerTest() override {
    ...
    // A test that would miss values written here would be poorly structured.
    EXPECT_FALSE(last_completed_upload_file_path_);
    EXPECT_FALSE(last_completed_upload_result_);  // Whether the value was set.
  }
  ...
  void SetWebRtcEventLogUploaderFactoryForTesting(
    ...
    // TODO: !!! Consider using WaitForReply.
  ...
  void ExpectUploadComplete(const base::FilePath& file_path,
                            bool upload_successful) {
    uploader_run_loop_->Run();
    // TODO: !!! Isn't this flaky?
    uploader_run_loop_.reset(new base::RunLoop);  // Allow re-blocking.

    ASSERT_TRUE(last_completed_upload_file_path_);
    ASSERT_TRUE(last_completed_upload_result_);  // Whether the value was set.

    EXPECT_EQ(file_path, *last_completed_upload_file_path_);
    EXPECT_EQ(upload_successful, *last_completed_upload_result_);

    last_completed_upload_file_path_.reset();
    last_completed_upload_result_.reset();
  }

  // WebRtcEventLogUploaderObserver implementation.
  void OnWebRtcEventLogUploadComplete(const base::FilePath& path,
                                      bool upload_successful) override {
    // A test that would miss values written here would be poorly structured.
    ASSERT_FALSE(last_completed_upload_file_path_);
    ASSERT_FALSE(last_completed_upload_result_);  // Whether the value was set.

    last_completed_upload_file_path_ = base::Optional<base::FilePath>(path);
    last_completed_upload_result_ = base::Optional<bool>(upload_successful);

    uploader_run_loop_->QuitWhenIdle();
  }
  ...
  base::FilePath GetLogsDirectoryPath(const BrowserContext* browser_context) {
    return GetLogsDirectoryPath(browser_context->GetPath());
  }
  ...
  // unit-under-test to be completed.
  ...
  // The results of the last simulated upload (if any) are stored here.
  base::Optional<base::FilePath> last_completed_upload_file_path_;
  base::Optional<bool> last_completed_upload_result_;
  ...
  // TODO: !!! Do I really need webrtc_state_change_instructions_ as a queue,
  // or do I only ever have one pending event? If I don't only have one, I am
  // running into a race over accessing it anyway...
};
...
// TODO: !!! Consider using FileListExpectingWebRtcEventLogUploader instead.
// Fake uploader that always succeeds / fails, depending on its configuration.
// TODO: !!! So, do I still need to do fancy runloop things?
// TODO: !!! Maybe I could just expect the right things in here?
// TODO: !!! Maybe I was only using it ...?
class SetResultWebRtcEventLogUploader : public WebRtcEventLogUploader {
 public:
  SetResultWebRtcEventLogUploader(
      const base::FilePath& log_file,
      bool result,
      std::unique_ptr<WebRtcEventLogUploaderObserver> observer) {
    EXPECT_TRUE(base::DeleteFile(log_file, false));
    observer->OnWebRtcEventLogUploadComplete(log_file, result);
  }

  ~SetResultWebRtcEventLogUploader() override = default;

  class Factory : public WebRtcEventLogUploader::Factory {
   public:
    Factory(bool result, WebRtcEventLogUploaderObserver* unit_test_observer)
        : result_(result), unit_test_observer_(unit_test_observer) {}
    ~Factory() override = default;

    // Create an uploader which  // TODO: !!! Finish
    std::unique_ptr<WebRtcEventLogUploader> Create(
        const base::FilePath& log_file,
        WebRtcEventLogUploaderObserver* observer) override {
      auto intercpeting_observer =
          std::make_unique<InterceptingWebRtcEventLogUploaderObserver>(
              unit_test_observer_, observer);
      return std::make_unique<SetResultWebRtcEventLogUploader>(
          log_file, result_, std::move(intercpeting_observer));
    }

   private:
    const bool result_;
    WebRtcEventLogUploaderObserver* const unit_test_observer_;
  };
};
...
class FileListExpectingWebRtcEventLogUploader : public WebRtcEventLogUploader {
 public:
  ...
  explicit FileListExpectingWebRtcEventLogUploader(
    ...
    observer->OnWebRtcEventLogUploadComplete(log_file, true);
  }
  ...
  class Factory : public WebRtcEventLogUploader::Factory {
   public:
    Factory(std::list<base::FilePath>* expected_files, base::RunLoop* run_loop)
        : run_loop_(run_loop) {
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  // We now have room for one additional log.
  const PeerConnectionKey last_key(rph_->GetID(), kMaxRemoteLogFiles);
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  const int kMaxPending =
      static_cast<int>(kMaxPendingRemoteLogFilesPerBrowserContext);

  // Allowed to start kMaxPending for each BrowserContext.
  ...
  ASSERT_TRUE(PeerConnectionAdded(rphs[0]->GetID(), kMaxPending));
  EXPECT_FALSE(StartRemoteLogging(rphs[0]->GetID(), kMaxPending));
...
TEST_P(WebRtcEventLogManagerTest, FinishedRemoteLogsUploadedAndFileDeleted) {
  ...
  SetWebRtcEventLogUploaderFactoryForTesting(
      std::make_unique<SetResultWebRtcEventLogUploader::Factory>(upload_result,
                                                                 this));
  ...
  ASSERT_TRUE(PeerConnectionRemoved(key.render_process_id, key.lid));
  ExpectUploadComplete(*log_file, upload_result);

  const base::FilePath remote_logs_path =
      GetLogsDirectoryPath(browser_context_.get());
  EXPECT_TRUE(base::IsDirectoryEmpty(remote_logs_path));
}
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  auto remote_logs_dir = GetLogsDirectoryPath(browser_context_dir.GetPath());
  ASSERT_TRUE(CreateDirectory(remote_logs_dir));

  const auto extension = base::FilePath::StringType(kRemoteBoundLogExtension);
  for (size_t i = 0; i < kMaxPendingRemoteLogFilesPerBrowserContext; i++) {
  ...
  SetWebRtcEventLogUploaderFactoryForTesting(
      std::make_unique<FileListExpectingWebRtcEventLogUploader::Factory>(
          &expected_files, &run_loop));

  auto browser_context = CreateBrowserContext(browser_context_dir.Take());
  auto rph = std::make_unique<MockRenderProcessHost>(browser_context.get());

  // Wait for all of the expectation to be resolved.
  run_loop.Run();
  DestroyUnitUnderTest();  // Blocks on running tasks inside the manager.
}

// TODO: !!! Do we limit pending logs when initializing?

// TODO: !! Change the modification date, etc.?

// TODO: !!! Only log files make a difference for the above test.

// TODO: !!! Files without read permissions in above?
...
// TODO: !!!
// 1. Finds and attempts to upload logs on BrowserContext
// 2. Does not find files which do not end in .log.
// 3. Pending logs counted towards the limit (actually, two tests -
// finished logs as well as logs found on init).
// 4. Only upload when allowed.
// 5. Start uploading if started being allowed to upload.
// 6. Inject randomizer?
