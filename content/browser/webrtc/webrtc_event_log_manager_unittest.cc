...
TEST_F(WebRtcEventLogManagerTest, UploadOrderDependsOnLastModificationTime) {
  // Keep one active peer connection on an independent BrowserContext in order
  // to suppress uploading of pending files during this test.
  const PeerConnectionKey suppress(rph_->GetID(), 0);
  ASSERT_TRUE(PeerConnectionAdded(suppress.render_process_id, suppress.lid));
  ...
  for (size_t i = 0; i < 3; i++) {
    ...
    auto remote_logs_dir = GetLogsDirectoryPath(browser_context_paths[i]);
    ASSERT_TRUE(CreateDirectory(remote_logs_dir));
  ...
  // Remove the suppression of uploading.
  ASSERT_TRUE(PeerConnectionRemoved(suppress.render_process_id, suppress.lid));
...
TEST_F(WebRtcEventLogManagerTest, ExpiredFilesArePrunedRatherThanUploaded) {
  ...
  DCHECK_GE(kMaxPendingRemoteBoundWebRtcEventLogs, 2u)
      << "Please restructure test to use separate browser contexts.";
  ...
  auto remote_logs_dir = GetLogsDirectoryPath(browser_context_path);
  ASSERT_TRUE(CreateDirectory(remote_logs_dir));
  ...
  // Wait for all of the expectation to be resolved. Write to a never added
  // peer connection just as a way to have a task to block on, so that we would
  // be sure we didn't race with the upload.
  // TODO: !!! Maybe I want to formalize this mechanism?
  run_loop.Run();
  const PeerConnectionKey unknown_pc(rph_->GetID(), 1);
  ASSERT_EQ(OnWebRtcEventLogWrite(unknown_pc.render_process_id, unknown_pc.lid,
                                  "log"),
            std::make_pair(false, false));

  // TODO: !!! Show that the old file was actually deleted.
}
...
// TODO: !!! Unit tests for 1. retention 2. comment above PrunePendingLogs().
