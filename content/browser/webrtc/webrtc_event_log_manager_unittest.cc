...
TEST_F(WebRtcEventLogManagerTest,
       RemoteBoundLogWithMaximumSizeEqualToMetadataWrittenCorrectly) {
  NiceMock<MockWebRtcRemoteEventLogsObserver> observer;
  base::Optional<base::FilePath> file_path;
  const PeerConnectionKey key(rph_->GetID(), kPeerConnectionId);
  EXPECT_CALL(observer, OnRemoteLogStarted(key, _))
...
TEST_F(WebRtcEventLogManagerTest,
       RemoteBoundLogWithMaximumSizeLessThanMetadataDisallowed) {
  NiceMock<MockWebRtcRemoteEventLogsObserver> observer;
  base::Optional<base::FilePath> file_path;
  ...
  const std::string metadata = "metadata";
  ASSERT_FALSE(StartRemoteLogging(key.render_process_id, key.lid,
                                  metadata.length() - 1, metadata));
}

// TODO: !!! key.render_process_id, kid.lid in underlying
