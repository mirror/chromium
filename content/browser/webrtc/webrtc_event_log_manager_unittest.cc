...
class WebRtcEventLogManagerTest : public ::testing::Test {
 public:
  ...
  void ExpectWebRtcStateChangeInstruction(int render_process_id,
    ...
    const PeerConnectionKey pc_key(render_process_id, lid);
  ...
  // // SetTaskRunnerForTesting // TODO: !!!
  // // By injecting this task runner into the unit under test, we can make sure
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  ASSERT_TRUE(PeerConnectionAdded(rph_->GetID(), kPeerConnectionId));
  ASSERT_FALSE(OnWebRtcEventLogWrite(rph_->GetID(), kPeerConnectionId, "log"));
}
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  ASSERT_TRUE(EnableLocalLogging());
  ASSERT_TRUE(PeerConnectionAdded(rph_->GetID(), kPeerConnectionId));
  ASSERT_TRUE(OnWebRtcEventLogWrite(rph_->GetID(), kPeerConnectionId, "log"));
}
...
TEST_F(WebRtcEventLogManagerTest, LocalLogFileSizeLimitNotExceeded) {
  ...
  ASSERT_FALSE(
      OnWebRtcEventLogWrite(rph_->GetID(), kPeerConnectionId, "ignored"));
...
TEST_F(WebRtcEventLogManagerTest, LocalLogSanityOverUnlimitedFileSizes) {
  ...
  ASSERT_TRUE(OnWebRtcEventLogWrite(rph_->GetID(), kPeerConnectionId, log1));
  ASSERT_TRUE(OnWebRtcEventLogWrite(rph_->GetID(), kPeerConnectionId, log2));
...
TEST_F(WebRtcEventLogManagerTest, LocalLogNoWriteAfterLogStopped) {
  ...
  ASSERT_TRUE(
      OnWebRtcEventLogWrite(rph_->GetID(), kPeerConnectionId, log_before));
  const auto pc = PeerConnectionKey(rph_->GetID(), kPeerConnectionId);
  EXPECT_CALL(observer, OnLocalLogStopped(pc)).Times(1);
  ASSERT_TRUE(PeerConnectionRemoved(rph_->GetID(), kPeerConnectionId));
  ...
  ASSERT_FALSE(
      OnWebRtcEventLogWrite(rph_->GetID(), kPeerConnectionId, log_after));
...
TEST_F(WebRtcEventLogManagerTest, LocalLogOnlyWritesTheLogsAfterStarted) {
  ...
  ASSERT_FALSE(OnWebRtcEventLogWrite(rph_->GetID(), kPeerConnectionId, log1));
...
TEST_F(WebRtcEventLogManagerTest, LocalLoggingRestartCreatesNewFile) {
  ...
  for (size_t i = 0; i < logs.size(); i++) {
    ...
    ASSERT_TRUE(
        OnWebRtcEventLogWrite(rph_->GetID(), kPeerConnectionId, logs[i]));
    ASSERT_TRUE(DisableLocalLogging());
  }
...
TEST_F(WebRtcEventLogManagerTest, LocalLogFilledLogNotCountedTowardsFileLimit) {
  ...
  // log to be written.
  EXPECT_TRUE(OnWebRtcEventLogWrite(rph_->GetID(), i, log));
...
TEST_F(WebRtcEventLogManagerTest, LocalLogLegalPathWithoutPermissionsSanity) {
  ...
  EXPECT_FALSE(OnWebRtcEventLogWrite(rph_->GetID(), kPeerConnectionId,
...
TEST_F(WebRtcEventLogManagerTest, LocalLogAllPossibleCharacters) {
  ...
  ASSERT_TRUE(
      OnWebRtcEventLogWrite(rph_->GetID(), kPeerConnectionId, all_chars));
...
TEST_F(WebRtcEventLogManagerTest, StartRemoteLoggingCreatesFile) {
  StrictMock<MockWebRtcRemoteEventLogsObserver> observer;
  base::Optional<PeerConnectionKey> key;
  base::Optional<base::FilePath> file_path;
  EXPECT_CALL(observer, OnRemoteLogStarted(_, _))
      .Times(1)
      .WillOnce(Invoke(SaveKeyAndFilePathTo(&key, &file_path)));
  SetRemoteLogsObserver(&observer);

  const auto remote_logs_path = GetLogsDirectoryPath(browser_context_.get());
  ASSERT_TRUE(base::DirectoryExists(remote_logs_path));
  ASSERT_TRUE(base::IsDirectoryEmpty(remote_logs_path));

  ASSERT_TRUE(PeerConnectionAdded(rph_->GetID(), kPeerConnectionId));
  ASSERT_TRUE(StartRemoteLogging(rph_->GetID(), kPeerConnectionId));

  EXPECT_FALSE(base::IsDirectoryEmpty(remote_logs_path));

  // TODO: !!! Check the right file was
}

// TODO: !!! Test that the file is created in the right profile's subdir.
