...
class WebRtcEventLogManagerTest : public ::testing::Test,
 ...
 public:
  ...
  // TODO: !!! Wait, why do I even need this? Is it not enough that the objects
  // are constructed with a factory that gives them |this| as an argument?
  void SetWebRtcEventLogUploaderObserverForTesting(
      WebRtcEventLogUploaderObserver* observer) {
    // TODO: !!! Consider using WaitForReply.
    manager_->SetWebRtcEventLogUploaderObserverForTesting(observer);
  }
  ...
  void ExpectUploadComplete(const base::FilePath& file_path,
    ...
    // StrictMock<MockWebRtcEventLogUploaderObserver> observer;  // TODO: !!!
...
TEST_F(WebRtcEventLogManagerTest, FinishedRemoteLogsUploadedAndFileDeleted) {
  SetWebRtcEventLogUploaderFactoryForTesting(
      std::make_unique<SetResultWebRtcEventLogUploader::Factory>(this, true));
  ...
  ASSERT_TRUE(PeerConnectionRemoved(key.render_process_id, key.lid));
  ExpectUploadComplete(*log_file, true);
...
// TODO: !!!!!!! HEREEE
TEST_F(WebRtcEventLogManagerTest, RemoteLogsThatCouldNotBeUploadedDeleted) {
  SetWebRtcEventLogUploaderFactoryForTesting(
      std::make_unique<SetResultWebRtcEventLogUploader::Factory>(this, false));
  ...
  ASSERT_TRUE(PeerConnectionRemoved(key.render_process_id, key.lid));
  ExpectUploadComplete(*log_file, false);
