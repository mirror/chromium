...
// TODO: !!! Remove key_output sometimes. Or do I even need the key?
...
TEST_F(WebRtcEventLogManagerTest, LocalLogCreatesEmptyFileWhenStarted) {
  ...
  ON_CALL(observer, OnLocalLogStarted(key, _))
      .WillByDefault(Invoke(SaveKeyAndFilePathTo(nullptr, &file_path)));
...
TEST_F(WebRtcEventLogManagerTest, LocalLogMultipleActiveFiles) {
  ...
  for (size_t i = 0; i < keys.size(); i++) {
    ON_CALL(observer, OnLocalLogStarted(*keys[i], _))
        .WillByDefault(Invoke(SaveKeyAndFilePathTo(nullptr, &file_paths[i])));
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  EXPECT_CALL(observer, OnLocalLogStarted(key, _))
      .WillOnce(Invoke(SaveKeyAndFilePathTo(nullptr, &file_path_1)))
      .WillOnce(Invoke(SaveKeyAndFilePathTo(nullptr, &file_path_2)));
