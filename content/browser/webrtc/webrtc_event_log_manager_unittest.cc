...
class WebRtcEventLogManagerTest : public ::testing::Test {
 protected:
  ...
  void LocalWebRtcEventLogStop(int render_process_id,
    ...
    manager_->LocalWebRtcEventLogStop(render_process_id, lid, ExpectBoolReplyClosure(expected_result));
    WaitForReply();
  }
  ...
  std::unique_ptr<WebRtcEventLogManager> manager_;  // Unit under test.
...
TEST_F(WebRtcEventLogManagerTest, LocalLogCreateEmptyFile) {
  ...
  LocalWebRtcEventLogStop(kRenderProcessId, kLocalPeerConnectionId, true);
