...
// This fake uploader always succeeds / fails, depending on its configuration.
...
TEST_F(WebRtcEventLogManagerTest,
  ...
  // Set additional BrowserContexts for the test.
...
// TODO: !!! Make sure that closed / deleted pending logs reduce the number?
//
...
//   const int kMaxActive =
//       static_cast<int>(kMaxNumberActiveRemoteWebRtcEventLogFiles);
//   const int kMaxPending =
//       static_cast<int>(kMaxPendingRemoteLogFilesPerBrowserContext);
//   static_assert(kMaxActive < kMaxPending && 1 < kMaxPending);

//   // Allowed to start
//   for (size_t i = 0; i < 2; i++) {
//     const PeerConnectionKey key(rphs[i]->GetID(), i);
//     ASSERT_TRUE(PeerConnectionAdded(key.render_process_id, key.lid));
//     ASSERT_TRUE(StartRemoteLogging(key.render_process_id, key.lid));
//   }
...
// // TODO: !!! Cannot start new active logs if it would take us over the
// pending
// // remote logs limit.
// TEST_F(WebRtcEventLogManagerTest, PendingLogsCountTowardsActiveLogsLimit) {
//   const int kMaxRemoteLogFiles =
//       static_cast<int>(kMaxNumberActiveRemoteWebRtcEventLogFiles);

//   static_assert(kMaxRemoteLogFiles >= 2, "Can't test this otherwise.");
//   static_assert(kMaxRemoteLogFiles >= 2, "Can't test this otherwise.");
