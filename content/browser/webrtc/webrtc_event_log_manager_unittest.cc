...
class WebRtcEventLogManagerTest : public ::testing::TestWithParam<bool> {
 public:
  ...
  void WaitForPendingTasks(base::RunLoop* run_loop = nullptr) {
    ...
    // Avoid using this LID anywhere else. (We don't  enforce this with DCHECKs,
    // since that would be overkill.)
    constexpr int kReservedLid = 9999;

    if (manager_) {  // Might have been destructed explicitly by the test.
      OnWebRtcEventLogWrite(rph_->GetID(), kReservedLid, "ignored");
    }

    // If those tasks post additional tasks, we won't wait for those, however.
  }
...
TEST_F(WebRtcEventLogManagerTest, ExpiredFilesArePrunedRatherThanUploaded) {
  ...
  // from local disk.
  EXPECT_TRUE(
      base::IsDirectoryEmpty(GetLogsDirectoryPath(browser_context.get())));
}
