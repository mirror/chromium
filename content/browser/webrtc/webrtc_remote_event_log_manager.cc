...
WebRtcRemoteEventLogManager::WebRtcRemoteEventLogManager()
    : log_extension_(FILE_PATH_LITERAL("log")) {
  // TODO: !!!
}
...
bool WebRtcRemoteEventLogManager::StartRemoteLogging(
  ...
  return MaybeStartWritingLog(render_process_id, lid, browser_context,
                              max_file_size_bytes);
}

bool WebRtcRemoteEventLogManager::EventLogWrite(int render_process_id, int lid,
...
void WebRtcRemoteEventLogManager::AddPendingLogs(
    const BrowserContext* browser_context) {
  DCHECK(pending_logs_counts_.find(browser_context) == pending_logs_counts_.end());

  size_t pending_logs_count = 0;

  const base::Time oldest_non_expired =
  ...
  for (auto path = enumerator.Next(); !path.empty(); path = enumerator.Next()) {
    base::Time last_modified_time = enumerator.GetInfo().GetLastModifiedTime();
    if (last_modified_time < oldest_non_expired) {
      // TODO: !!! Make sure that deleting the file really has no effect
      // on the enumerator.
      if (!base::DeleteFile(path, /*recursive=*/false)) {
        LOG(ERROR) << "Failed to delete " << path << ".";
      }
    } else {
      pending_logs_.emplace(path, last_modified_time);
      ++pending_logs_count;
    }
  }

  // Note: We rely on the previous count to have kept pending_logs_count in
  // range. If the user has manipulated that directory, we could end up
  // having more logs than is allowed; that's okay.
  pending_logs_counts_.emplace(browser_context, pending_logs_count);
}

bool WebRtcRemoteEventLogManager::MaybeStartWritingLog(
    int render_process_id,
    int lid,
    const BrowserContext* browser_context,
    size_t max_file_size_bytes) {
...
//void WebRtcRemoteEventLogManager::PrunePendingLogsForBrowserContext(
//    const BrowserContext* browser_context) {
//}
