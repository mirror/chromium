...
void WebRtcRemoteEventLogManager::DisableForBrowserContext(
    const BrowserContext* browser_context) {
  // All of the PeerConnections associated with this BrowserContext will already
  // have been orderly removed, so we don't need to update anything in
  // active_logs_. Pending logs remain pending despite the BrowserContext
  // being torn down, because this only happens in unit tests.
  // TODO(eladalon): !!! Really? What about Chrome closing?

  // We might never have recorded this |browser_context| if it were
  // of-the-records. We don't check if it now, though, so as not to call a
  // virtual function on an object undergoing destruction.
...
void WebRtcRemoteEventLogManager::AddPendingLogs(
    const BrowserContext* browser_context) {
  LOG(ERROR) << "ELAD!!! WebRtcRemoteEventLogManager::AddPendingLogs("
             << ((void*)browser_context) << ").";
  LOG(ERROR) << "ELAD!!! |active_logs_counts_| = " << active_logs_counts_.size()
             << ".";
  LOG(ERROR) << "ELAD!!! |pending_logs_counts_| = "
             << pending_logs_counts_.size() << ".";
