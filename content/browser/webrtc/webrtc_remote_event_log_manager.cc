...
void WebRtcRemoteEventLogManager::AddPendingLogs(
  ...
  printf("pending_logs_ = %lu\n", pending_logs_.size());
  for (auto& it : pending_logs_) {
    printf("PENDING: %s\n", it.path.value().c_str());
  }
...
void WebRtcRemoteEventLogManager::MaybeStartUploading() {
  ...
  if (uploader_) {
    printf("MaybeStartUploading - !uploader_\n");
    return;  // Upload already underway.
  }

  if (pending_logs_.empty()) {
    printf("MaybeStartUploading - pending_logs_.empty()\n");
    return;  // Nothing to upload.
  }

  if (!UploadingAllowed()) {
    printf("MaybeStartUploading - !UploadingAllowed()\n");
    return;
  }
