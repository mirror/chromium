...
// The header consists of one byte for the version and three bytes for the
...
bool WebRtcRemoteEventLogManager::StartRemoteLogging(
  ...
  if (metadata.size() + sizeof(uint32_t) > max_file_size_bytes) {
...
bool WebRtcRemoteEventLogManager::StartWritingLog(
  ...
  const size_t header_and_metadata_size_bytes =
      sizeof(uint32_t) + metadata_length;
