...
bool WebRtcRemoteEventLogManager::StartRemoteLogging(
  ...
  // Limit over the number of pending logs (per BrowserContext).
  const auto count_it = pending_logs_counts_.find(browser_context);
  DCHECK(count_it != pending_logs_counts_.end());
  if (count_it->second >= kMaxPendingLogFilesPerBrowserContext) {
    return false;
  }
...
void WebRtcRemoteEventLogManager::PrunePendingLogs() {
  ...
  for (auto it = pending_logs_.begin(); it != pending_logs_.end();) {
    if (!it->uploading && it->last_modified < oldest_non_expired_timestamp) {
