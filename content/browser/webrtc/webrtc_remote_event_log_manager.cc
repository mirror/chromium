...
namespace {
// TODO: !!! Tests for the retention of files.
...
// unnecessary.
const size_t kMaxNumberActiveRemoteWebRtcEventLogFiles = 3;
const size_t kMaxPendingRemoteLogFilesPerBrowserContext = 5;
static_assert(kMaxNumberActiveRemoteWebRtcEventLogFiles <=
                  kMaxPendingRemoteLogFilesPerBrowserContext,
              "This assumption affects unit test coverage.");

const char* const kRemoteBoundLogExtension = "log";

WebRtcRemoteEventLogManager::WebRtcRemoteEventLogManager(
    WebRtcRemoteEventLogsObserver* observer)
    : observer_(observer),
      uploader_factory_(new WebRtcEventLogUploaderImpl::Factory(this)) {}
...
void WebRtcRemoteEventLogManager::OnWebRtcEventLogUploadComplete(
  ...
  base::SequencedTaskRunnerHandle::Get()->PostTask(
      ...
      base::BindOnce(
          ...
          base::Unretained(this), file_path, upload_successful));
...
void WebRtcRemoteEventLogManager::AddPendingLogs(
  ...
  base::FileEnumerator enumerator(
      GetLogsDirectoryPath(browser_context),
      /*recursive=*/false, base::FileEnumerator::FILES,
      FILE_PATH_LITERAL("*.") +
          base::FilePath::StringType(kRemoteBoundLogExtension));
...
bool WebRtcRemoteEventLogManager::StartWritingLog(
  ...
  const base::FilePath file_path =
      base_path.AppendASCII(unique_filename)
          .AddExtension(base::FilePath::StringType(kRemoteBoundLogExtension));
...
// TODO: !!! Unit tests about retention.
...
// TODO: !!! Unit tests for this. 1. Not uploading when active connections
// exist. 2. When an active connection becomes inactive, uploading starts.
