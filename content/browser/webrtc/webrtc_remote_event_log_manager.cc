...
bool WebRtcRemoteEventLogManager::StartRemoteLogging(
  ...
  if (active_peer_connections_.find(pc_key) == active_peer_connections_.end()) {
    return false;
  }

  // May not restart active remote logs.
  // TODO: !!! Unit test for different file size; original value still used.
  auto it = active_logs_.find(pc_key);
  ...
  if (active_logs_.size() >= kMaxConcurrentActiveLogs) {
    return false;
  }
...
void WebRtcRemoteEventLogManager::MaybeStartUploading() {
  ...
  uploader_ =
      std::make_unique<WebRtcEventLogUploader>(pending_logs_.begin()->path);
