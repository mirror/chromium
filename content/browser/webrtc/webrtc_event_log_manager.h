...
class CONTENT_EXPORT WebRtcEventLogManager {
 public:
  ...
  void LocalWebRtcEventLogStop(int render_process_id, int lid);
 ...
 private:
  ...
  class FileClosureObserverForTesting {
   public:
    virtual void OnFileClosed(int render_process_id, int lid) = 0;
  };
  void SetObserverForTesting(FileClosureObserverForTesting* observer);
  ...
  FileClosureObserverForTesting* file_closure_observer_for_testing_;
  ...
  base::WeakPtrFactory<WebRtcEventLogManager> weak_ptr_factory_;
};
