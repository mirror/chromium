...
class CONTENT_EXPORT WebRtcEventLogManager
 ...
 protected:
  ...
  // This bitmap allows us to see track for which clients (local/remote logging)
  ...
  enum LoggingTarget : int {
    kLocalLogging = 0x01
    // TODO(eladalon): Add kRemoteLogging as 0x02.
  };
  ...
  // Manages local-bound logs - logs which we intend to keep around on the local
  // filesystem indefinitely, and never upload to a remote server.
  WebRtcLocalEventLogManager local_logs_manager_;

  // This keeps track of which peer connections have event loggint turned on
