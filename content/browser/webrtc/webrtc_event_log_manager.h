...
class CONTENT_EXPORT WebRtcEventLogManager {
 public:
  ...
  class LocalLogsObserver {
   public:
    ...
    virtual void OnLocalLogsStarted(
        const std::map<PeerConnectionKey, base::FilePath>& local_logs) = 0;
    virtual void OnLocalLogsStopped(
        const std::set<PeerConnectionKey>& local_logs) = 0;
  };
  ...
  // If a reply is given, it will be posted back to BrowserThread::UI, with
  // true if and only if the operation was successful (failure is only
  ...
  // The file's actually path is derived from |base_path| by adding a timestamp,
  // the render process ID and the PeerConnection's local ID.
  // If a reply is given, it will be posted back to BrowserThread::UI, with true
  // if and only if local logging was *not* already on.
  // Note: If the number of currently active peer connections exceeds the
  ...
  // upload, or both.
  // If a reply is given, it will be posted back to BrowserThread::UI, with true
  // if and only |output| was written in its entirety to both the local log (if
  // any) as well as the remote log (if any). In the edge case that it was
  // written to neither, because neither existed, false will be returned, even
  // though one could interpret that case as "neither failed".
 ...
 protected:
  ...
  base::FilePath StartLocalLogFile(int render_process_id, int lid);
