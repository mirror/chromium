...
void SpeechRecognitionDispatcherHost::Create(
  ...
  mojo::MakeStrongBinding(base::MakeUnique<SpeechRecognitionDispatcherHost>(
                              sender, render_process_id, context_getter),
                          std::move(request));
}
...
void SpeechRecognitionDispatcherHost::StartRequestOnUI(
  ...
  BrowserThread::PostTask(
      ...
      base::BindOnce(&SpeechRecognitionDispatcherHost::StartRequestOnIO,
                     base::Unretained(this), embedder_render_process_id,
                     embedder_render_view_id, std::move(input_params),
                     params_render_frame_id, filter_profanities));
}
...
void SpeechRecognitionDispatcherHost::SendOnIO(IPC::Message* message) {
  if (!BrowserThread::CurrentlyOn(BrowserThread::IO)) {
    // The weak ptr must only be accessed on the IO thread.
    BrowserThread::PostTask(
        BrowserThread::IO, FROM_HERE,
        base::BindOnce(&SpeechRecognitionDispatcherHost::SendOnIO,
...
void SpeechRecognitionDispatcherHost::OnRecognitionStart(int session_id) {
  ...
  SendOnIO(new SpeechRecognitionMsg_Started(context.render_view_id,
                                            context.request_id));
}
...
void SpeechRecognitionDispatcherHost::OnAudioStart(int session_id) {
  const SpeechRecognitionSessionContext& context =
      SpeechRecognitionManager::GetInstance()->GetSessionContext(session_id);
  SendOnIO(new SpeechRecognitionMsg_AudioStarted(context.render_view_id,
                                                 context.request_id));
}
...
void SpeechRecognitionDispatcherHost::OnSoundStart(int session_id) {
  const SpeechRecognitionSessionContext& context =
      SpeechRecognitionManager::GetInstance()->GetSessionContext(session_id);
  SendOnIO(new SpeechRecognitionMsg_SoundStarted(context.render_view_id,
                                                 context.request_id));
}
...
void SpeechRecognitionDispatcherHost::OnSoundEnd(int session_id) {
  const SpeechRecognitionSessionContext& context =
      SpeechRecognitionManager::GetInstance()->GetSessionContext(session_id);
  SendOnIO(new SpeechRecognitionMsg_SoundEnded(context.render_view_id,
                                               context.request_id));
}
...
void SpeechRecognitionDispatcherHost::OnAudioEnd(int session_id) {
  const SpeechRecognitionSessionContext& context =
      SpeechRecognitionManager::GetInstance()->GetSessionContext(session_id);
  SendOnIO(new SpeechRecognitionMsg_AudioEnded(context.render_view_id,
                                               context.request_id));
}
...
void SpeechRecognitionDispatcherHost::OnRecognitionEnd(int session_id) {
  const SpeechRecognitionSessionContext& context =
      SpeechRecognitionManager::GetInstance()->GetSessionContext(session_id);
  SendOnIO(new SpeechRecognitionMsg_Ended(context.render_view_id,
                                          context.request_id));
}
...
void SpeechRecognitionDispatcherHost::OnRecognitionResults(
  ...
  const SpeechRecognitionSessionContext& context =
      SpeechRecognitionManager::GetInstance()->GetSessionContext(session_id);
  SendOnIO(new SpeechRecognitionMsg_ResultRetrieved(
...
void SpeechRecognitionDispatcherHost::OnRecognitionError(
  ...
  const SpeechRecognitionSessionContext& context =
      SpeechRecognitionManager::GetInstance()->GetSessionContext(session_id);
  SendOnIO(new SpeechRecognitionMsg_ErrorOccurred(context.render_view_id,
