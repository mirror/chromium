 ...
 private:
  ...
  void Start(const base::FilePath& profile_path,
    ...
    if (!base::GetFileInfo(path_, &info) || !info.is_directory) {
      client->OnComplete(
          network::ResourceRequestCompletionStatus(net::ERR_FILE_NOT_FOUND));
      return;
    }
    ...
    if (!GetContentClient()->browser()->IsFileAccessAllowed(
      ...
      client->OnComplete(
          network::ResourceRequestCompletionStatus(net::ERR_ACCESS_DENIED));
      return;
    }
 ...
 private:
  ...
  void Start(const base::FilePath& profile_path,
    ...
    if (request.headers.GetHeader(net::HttpRequestHeaders::kRange,
      ...
      if (fail) {
        client->OnComplete(network::ResourceRequestCompletionStatus(
  ...
  void OnFileWritten(MojoResult result) {
    if (result == MOJO_RESULT_OK)
      client_->OnComplete(network::ResourceRequestCompletionStatus(net::OK));
    else
      client_->OnComplete(
          network::ResourceRequestCompletionStatus(net::ERR_FAILED));
    client_.reset();
