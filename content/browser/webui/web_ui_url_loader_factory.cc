...
base::LazyInstance<std::map<int, std::unique_ptr<WebUIURLLoaderFactory>>>::Leaky
    g_webui_url_loader_factories = LAZY_INSTANCE_INITIALIZER;
...
class WebUIURLLoaderFactory : public mojom::URLLoaderFactory,
 ...
 public:
  ...
  void OnFrameTreeNodeDestroyed(FrameTreeNode* node) override {
    g_webui_url_loader_factories.Get().erase(frame_tree_node_id_);
  }
...
mojom::URLLoaderFactoryPtr CreateWebUIURLLoader(FrameTreeNode* node) {
  ...
  if (g_webui_url_loader_factories.Get()[ftn_id].get() == nullptr)
    g_webui_url_loader_factories.Get()[ftn_id] = base::MakeUnique<WebUIURLLoaderFactory>(node);
  return g_webui_url_loader_factories.Get()[ftn_id]->CreateBinding();
}
