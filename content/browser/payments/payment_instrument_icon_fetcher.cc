...
void OnIconFetched(
  ...
  if (bitmap.drawsNothing()) {
    if (icons.empty()) {
      ...
      // If could not decode the chosen image(e.g. not supported, invalid),
      // try it again with remaining icons.
...
WebContents* GetWebContentsFrom(
    const GURL& scope,
...
void StartOnUI(
  ...
  WebContents* web_contents =
      GetWebContentsFrom(scope, std::move(provider_hosts));
  DownloadBestMatchingIcon(web_contents, icons, std::move(callback));
}
