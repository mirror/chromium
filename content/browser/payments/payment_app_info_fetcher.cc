...
void PaymentAppInfoFetcher::FetchPaymentAppManifestCallback(
  ...
  if (manifest.IsEmpty()) {
    WarnIfPossible(
        "Unable to download the payment handler's web app manifest \"" +
        manifest_url_.spec() +
        "\". This manifest defines the payment handler's name and icon. User "
        "may not recognize this payment handler in UI, because it will be "
