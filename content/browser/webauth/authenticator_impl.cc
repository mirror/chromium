...
void AuthenticatorImpl::MakeCredential(
  ...
  u2f_request_ = device::U2fRegister::TryRegistration(
      relying_party_id, {u2f_discovery_.get()}, registered_keys,
      client_data_hash, application_parameter,
...
void AuthenticatorImpl::GetAssertion(
  ...
  std::vector<std::vector<uint8_t>> handles =
      FilterCredentialList(options->allow_credentials);
  ...
  u2f_request_ = device::U2fSign::TrySign(
      relying_party_id, {u2f_discovery_.get()}, handles, client_data_hash,
