...
void AuthenticatorImpl::ParseU2fRegisterResponse(
  ...
  uint8_t cred_length[2];  // Big-endian.
  cred_length[1] = u2f_data[66];
  attestation_data.insert(attestation_data.end(), &cred_length[0],
                          &cred_length[2]);
  ...
  */
  ...
  uint32_t numeric_cred_length =
      (base::checked_cast<uint32_t>(cred_length[0]) << 8) |
      (base::checked_cast<uint32_t>(cred_length[1]));

  // Set Credential ID in attestation_data with the key handle bytes.
  attestation_data.insert(attestation_data.end(), numeric_cred_length,
                          u2f_data[67]);

  // Set the Credential ID to use with raw_id, id.
  credential_id.insert(credential_id.end(), numeric_cred_length, u2f_data[67]);

  // Copy the 65 bytes of the public key from U2FResponse.
  attestation_data.insert(attestation_data.end(), 65, u2f_data[1]);

  // Then, build a U2F attestation statement with the remainder of the data.
  attestation_statement =
      BuildU2FAttestationStatement(u2f_data, numeric_cred_length + 67);
