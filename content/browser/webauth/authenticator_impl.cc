...
void AuthenticatorImpl::MakeCredential(
  ...
  if (ServiceManagerConnection::GetForProcess()) {
    ...
    u2fRequest_ = device::U2fRegister::TryRegistration(
        client_data_hash, app_param, response_callback, connector);
  }
}
...
void AuthenticatorImpl::OnDeviceResponse(MakeCredentialCallback callback,
  ...
  if (status_code == device::U2fReturnCode::SUCCESS) {
    std::unique_ptr<authenticator_utils::RegisterResponseData> response =
        authenticator_utils::ParseU2fRegisterResponse(client_data_json, data);
    std::move(callback).Run(webauth::mojom::AuthenticatorStatus::SUCCESS,
                            From(std::move(response)));
  }
...
// TODO does this need to be more formalized, like a typeconverter?
webauth::mojom::PublicKeyCredentialInfoPtr From(
    const std::unique_ptr<authenticator_utils::RegisterResponseData>
        response_data) {
  ...
  credentialInfo->raw_id = response_data->GetRawId();
