...
class CONTENT_EXPORT MapValue : public CBORValue {
 public:
  ...
  void InsertString(std::string key, base::StringPiece string);

  void InsertUnsigned(std::string key, uint64_t value);

  void InsertBytestring(std::string key, std::vector<uint8_t>& bytestring);

  void InsertArray(std::string key, std::vector<CBORValue>& array);

  void InsertMap(std::string key,
                 base::flat_map<std::string, std::unique_ptr<CBORValue>>& map);

  void InsertMap(std::string key, MapValue map);
};
