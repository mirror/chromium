...
class CONTENT_EXPORT CBORValue {
 public:
  struct CTAPLess {
    ...
    //
    // The CBOR RFC contains similar suggestions
    // (https://tools.ietf.org/html/rfc7049#section-3.9) although it sorts
    // purely by serialised key and doesn't specify that major types are
    // compared first. Thus the shortest key sorts first by the RFC rules
    // (irrespective of the major type), but may not by CTAP rules.
