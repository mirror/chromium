...
// 7049(https://tools.ietf.org/html/rfc7049). This decoder supports canonical
// deserialization of CBOR data bytes.
// Supported:
//  * Major types:
...
//  - Keys for map must be sorted in first by length and then by byte-wise
//    lexical order.
//
...
class CONTENT_EXPORT CBORReader {
 public:
  ...
  enum CBORReaderOption {
    // Parses the input strictly according to section 3.10 of RFC 7049, except
    // for where noted above.
    STRICT_MODE = 0,

    // Allows out of order map keys for major type 5.
    ALLOW_UNSORTED_MAP_KEYS = 1,
  };

  // CBOR nested depth sufficient for most use cases.
  static const size_t kCBORMaxDepth = 16;

  // String versions of decoder error codes.
  static const char kUnsupportedMajorType[];
  ...
  // decoding failures. If |in_strict_mode| is set to true, then the decoder
  // rejects any data that could be interpreted by other decoders in different
  // ways.
  static base::Optional<CBORValue> Read(
      const std::vector<uint8_t>& input_data,
      CBORReaderOption option,
 ...
 private:
  ...
  bool CheckIntegerShortestByte(uint8_t additional_bytes, uint64_t uint_data);
