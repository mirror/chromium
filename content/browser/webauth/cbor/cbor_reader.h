...
//  - Are unknown formats such as unknown additional information byte format are
...
class CONTENT_EXPORT CBORReader {
 public:
  ...
  // Class that encapsulate CBOR meta data. Includes information about major
  // type and length.
  class FirstByte {
   public:
    FirstByte(uint8_t byte_data) : meda_data_(byte_data) {}
    uint8_t GetMajorType() {
      return (meda_data_ & CBORValue::kMajorTypeDataMask) >>
             CBORValue::kMajorTypeBitShift;
    }

    uint8_t GetAdditionalInfo() {
      return meda_data_ & CBORValue::kAdditionalInformationDataMask;
    }

   private:
    const uint8_t meda_data_;
  };
  ~CBORReader();

  // Exposed static CBOR decoder interface to be used by the caller. Reads and
  // parses |input_data| into a CBORValue. If any one of the syntax formats is
  // violated --including unknown additional info and incomplete CBOR data --
  // then an empty optional is returned. Optional |error_code_out| can be
  // provided by the caller to obtain additional information about decoding
  // failures. If |in_strict_mode| is set to true, then decoder enforces
  // canonical CBOR representations. Thus, if any one of syntax formatting
  // and/or canonical CBOR requirements are violated, then empty optional is
  // returned.
 ...
 private:
  ...
  base::Optional<CBORValue> DecodeCBOR(int max_nesting_level,
                                       bool in_strict_mode = false);
  bool ReadLength(uint64_t* length, int additional_info);
  bool ReadInteger(uint64_t* length_data, uint64_t num_bytes);
  bool ReadBytes(std::vector<uint8_t>* buffer, uint64_t num_bytes);
  bool ReadString(std::vector<char>* buffer, uint64_t num_bytes);
  bool ReadCBORArray(CBORValue::ArrayValue* array,
                     uint64_t length,
                     int max_nesting_level);
  bool ReadCBORMap(CBORValue::MapValue* map,
                   uint64_t length,
                   int max_nesting_level);
  bool IteratorBoundCheck(uint64_t step);
