...
//  - Unknown formats are treated as syntax errors.
//  - Input maps with duplicate keys are accepted. However, only the last value
//    of duplicate key value pairs will be decoded for the corresponding key.
...
class CONTENT_EXPORT CBORReader {
 ...
 private:
  ...
  base::Optional<CBORValue> DecodeCBOR(int max_nesting_level,
                                       bool require_canonical_format = false);
