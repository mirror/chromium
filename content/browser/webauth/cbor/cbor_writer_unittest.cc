...
TEST(CBORWriterTest, TestWriterIsValidDepthSingleLayer) {
  ...
  EXPECT_TRUE(CBORWriter::IsValidDepth(
      CBORValue(base::StringPiece("\x44\x01\x02\x03\x04"))));
}
...
//                   "f": 5,
//                   "g": [5, 6]}}}
// Since above CBOR contains 4 nesting levels, encoding this CBOR value
// object as an element to an array or a value to CBOR map would result in
// CBOR data with 5 nesting level. Thus, IsValidDepth() is expected to
// return false.
