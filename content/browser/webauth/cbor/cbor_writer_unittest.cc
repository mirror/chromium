...
TEST(CBORWriterTest, TestWriteSingleLayer) {
  ...
  const CBORValue empty_array = CBORValue((CBORValue::ArrayValue){});
  const CBORValue empty_map = CBORValue((CBORValue::MapValue){});
  ...
  EXPECT_TRUE(CBORWriter::Write(empty_array, 0).has_value());
  EXPECT_TRUE(CBORWriter::Write(empty_map, 0).has_value());
