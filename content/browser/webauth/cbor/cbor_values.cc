...
CBORValue::CBORValue(const BinaryValue& in_bytes) : type_(Type::BYTESTRING) {
...
CBORValue::CBORValue(const ArrayValue& in_array) : type_(Type::ARRAY) {
  array_value_.Init();
...
CBORValue::CBORValue(ArrayValue&& in_array) noexcept : type_(Type::ARRAY) {
  array_value_.Init(std::move(in_array));
}

CBORValue::CBORValue(const MapValue& in_map) : type_(Type::MAP) {
  map_value_.Init();
...
CBORValue::CBORValue(MapValue&& in_map) noexcept : type_(Type::MAP) {
