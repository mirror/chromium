...
TEST(CBORReaderTest, TestUnsupportedMajorType) {
  ...
  EXPECT_FALSE(cbor.has_value());
  EXPECT_EQ(error_code, CBORReader::DecoderError::UNSUPPORTED_MAJOR_TYPE);
}
...
TEST(CBORReaderTest, TestUnsupportedSimplevalue) {
  static const std::vector<std::vector<uint8_t>> zero_padded_cbor_list = {
      ...
      // Simple value (255)
      {0xF8, 0xFF},
      // Unassigned simple value with addition 5 bit (28)
      {0xFC},
      // Unassigned simple value with addition 5 bit (29)
      {0xFD},
      // Unassigned simple value with addition 5 bit (30)
      {0xFE},
      // Unassigned simple value with addition 5 bit (31)
      {0xFF},
  };
