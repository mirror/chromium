...
TEST(CBORReaderTest, TestReadUint) {
  ...
  for (const UintTestCase& test_case : kUintTestCases) {
    ...
    scope_message << "testing uint value : "
                  << base::StringPrintf("%lu", test_case.value);
    SCOPED_TRACE(scope_message);

    base::Optional<CBORValue> cbor =
        CBORReader::Read(test_case.cbor_data, CBORReader::STRICT_MODE);
...
TEST(CBORReaderTest, TestUintEncodedWithNonMinimumByteLength) {
  // Unsigned int 100 encoded with 2 bytes.
  static const std::vector<uint8_t> too_long_uint_byte = {0x19, 0x00, 0x64};
  CBORReader::CBORDecoderError error_code;
  base::Optional<CBORValue> cbor = CBORReader::Read(
      too_long_uint_byte, CBORReader::STRICT_MODE, &error_code);
  EXPECT_EQ(cbor.has_value(), false);
...
TEST(CBORReaderTest, TestReadArray) {
  ...
  base::Optional<CBORValue> cbor =
      CBORReader::Read(kArrayTestCaseCbor, CBORReader::STRICT_MODE);
  ASSERT_TRUE(cbor.has_value());
...
TEST(CBORReaderTest, TestReadMapWithMapValue) {
  ...
  base::Optional<CBORValue> cbor =
      CBORReader::Read(kMapTestCaseCbor, CBORReader::STRICT_MODE);
  ASSERT_TRUE(cbor.has_value());
...
TEST(CBORReaderTest, TestReadMapWithArray) {
  ...
  base::Optional<CBORValue> cbor =
      CBORReader::Read(kMapArrayTestCaseCbor, CBORReader::STRICT_MODE);
  ASSERT_TRUE(cbor.has_value());
  CBORValue cbor_val = std::move(cbor.value());
  ASSERT_EQ(cbor_val.type(), CBORValue::Type::MAP);
...
TEST(CBORReaderTest, TestReadNestedMap) {
  ...
  base::Optional<CBORValue> cbor =
      CBORReader::Read(kNestedMapTestCase, CBORReader::STRICT_MODE);
  ASSERT_TRUE(cbor.has_value());
  CBORValue cbor_val = std::move(cbor.value());
  ASSERT_EQ(cbor_val.type(), CBORValue::Type::MAP);
  ASSERT_EQ(cbor_val.GetMap().size(), 2u);
...
TEST(CBORReaderTest, TestIncompleteCBORDataError) {
  ...
  for (auto incomplete_data : incomplete_cbor_list) {
    ...
    CBORReader::CBORDecoderError error_code;
    base::Optional<CBORValue> cbor =
        CBORReader::Read(incomplete_data, CBORReader::STRICT_MODE, &error_code);
...
TEST(CBORReaderTest, TestUnsupportedKeyFormatError) {
  ...
  CBORReader::CBORDecoderError error_code;
  base::Optional<CBORValue> cbor =
      CBORReader::Read(kMapWithUintKey, CBORReader::STRICT_MODE, &error_code);
  EXPECT_FALSE(cbor.has_value());
...
TEST(CBORReaderTest, TestUnknownAdditionalInfoError) {
  ...
  for (auto incorrect_cbor : kUnknownAdditionalInfoList) {
    ...
    CBORReader::CBORDecoderError error_code;
    base::Optional<CBORValue> cbor =
        CBORReader::Read(incorrect_cbor, CBORReader::STRICT_MODE, &error_code);
    EXPECT_FALSE(cbor.has_value());
...
TEST(CBORReaderTest, TestTooMuchNestingError) {
  ...
  for (auto zero_depth_data : kZeroDepthCBORList) {
    ...
    CBORReader::CBORDecoderError error_code;
    base::Optional<CBORValue> cbor = CBORReader::Read(
        zero_depth_data, CBORReader::STRICT_MODE, &error_code, 0);
  ...
  base::Optional<CBORValue> cbor_single_layer_max = CBORReader::Read(
      kNestedCBORData, CBORReader::STRICT_MODE, &error_code, 1);
  ...
  base::Optional<CBORValue> cbor_double_layer_max = CBORReader::Read(
      kNestedCBORData, CBORReader::STRICT_MODE, &error_code, 2);
...
TEST(CBORReaderTest, TestOutOfOrderKeyError) {
  ...
  base::Optional<CBORValue> cbor = CBORReader::Read(
      kMapWithUnsortedKey, CBORReader::STRICT_MODE, &error_code);
  EXPECT_FALSE(cbor.has_value());
...
TEST(CBORReaderTest, TestDuplicateKeyError) {
  ...
  base::Optional<CBORValue> cbor = CBORReader::Read(
      kMapWithDuplicateKey, CBORReader::STRICT_MODE, &error_code);
  EXPECT_FALSE(cbor.has_value());
...
TEST(CBORReaderTest, TestIncorrectStringEncodingError) {
  ...
  for (auto cbor_byte : utf8_character_encodings) {
    ...
    base::Optional<CBORValue> correctly_encoded_cbor =
        CBORReader::Read(cbor_byte, CBORReader::STRICT_MODE, &error_code);
  ...
  base::Optional<CBORValue> incorrectly_encoded_cbor = CBORReader::Read(
      impossible_utf_byte, CBORReader::STRICT_MODE, &error_code);
  EXPECT_FALSE(incorrectly_encoded_cbor.has_value());
  EXPECT_EQ(error_code, CBORReader::INVALID_UTF8_ERROR);
}
...
TEST(CBORReaderTest, TestExtraneousCBORDataError) {
  ...
  for (auto extraneous_cbor_data : zero_padded_cbor_list) {
    ...
    CBORReader::CBORDecoderError error_code;
    base::Optional<CBORValue> cbor = CBORReader::Read(
        extraneous_cbor_data, CBORReader::STRICT_MODE, &error_code);
    EXPECT_FALSE(cbor.has_value());
    EXPECT_EQ(error_code, CBORReader::EXTRANEOUS_DATA_ERROR);
  }
}
