...
class CONTENT_EXPORT CBORWriter {
 public:
  ...
  // Given a root node, generates a CBOR byte string and puts it into |cbor|.
  // The output string is overwritten and not appended.
  //
  // IMPORTANT: base::Value doesn't support unsigned integers directly. To use
  // unsigned integers, pass the value as a DOUBLE type.
  static void Write(const base::Value& node, std::vector<uint8_t>* cbor);

  // Helper conversion methods to map primitive CBOR types to base::Value types.
  static base::Value Unsigned(uint64_t value);
  static base::Value Bytestring(std::vector<char> bytes);
  static base::Value UTF8String(base::StringPiece string);

 private:
  CBORWriter(std::vector<uint8_t>* cbor);

  // Called recursively to build the CBOR bytestring. When completed,
  // |encoded_cbor| will contain the CBOR.
  void EncodeCBOR(const base::Value& node);
