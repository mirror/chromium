...
class CONTENT_EXPORT CBORWriter {
 public:
  ...
  // Returns a tuple where the first value is generated CBOR byte string and a
  // second value is boolean that represents whether CBOR meets the maximum
  // nesting layer size requirement.  Nested CBOR is defined as any combination
  // of CBOR maps and/or CBOR arrays. Nesting level is defined by number of
  // hash- or arrayrefs that the encoder needs to traverse or the number of {
  // or [ characters without their matching closing parenthesis crossed to
  // reach a given character in a string. By default, maximum nesting size is
  // set to 512. Is is up to the caller or the application to determine
