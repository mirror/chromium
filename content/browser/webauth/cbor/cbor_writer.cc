...
// static
void CBORWriter::Write(const CBORValue& node, std::vector<uint8_t>* cbor) {
  cbor->clear();
...
void CBORWriter::EncodeCBOR(const CBORValue& node) {
  switch (node.type()) {
    ...
    case CBORValue::Type::BYTESTRING: {
      const BytestringValue& bytes = node.GetBytestring();
    ...
    case CBORValue::Type::ARRAY: {
      const ArrayValue& array = node.GetArray();
    ...
    case CBORValue::Type::MAP: {
      const MapValue& map = node.GetMap();
