...
void CBORWriter::StartItem(CBORValue::Type type, uint64_t size) {
  encoded_cbor_->push_back(base::checked_cast<uint8_t>(
      static_cast<unsigned>(type) << authenticator_impl::kMajorTypeBitShift));
  SetUint(size);
}
...
void CBORWriter::SetAdditionalInformation(uint8_t additional_information) {
  ...
  DCHECK_EQ(additional_information &
                authenticator_impl::kAdditionalInformationDataMask,
            additional_information);
  encoded_cbor_->back() |= (additional_information &
                            authenticator_impl::kAdditionalInformationDataMask);
}
...
void CBORWriter::SetUint(uint64_t value) {
  ...
  switch (count) {
    case 0:
      ...
      break;
    case 1:
      SetAdditionalInformation(authenticator_impl::kAdditionalInformation1Byte);
      shift = 0;
      break;
    case 2:
      SetAdditionalInformation(
          authenticator_impl::kAdditionalInformation2Bytes);
      shift = 1;
      break;
    case 4:
      SetAdditionalInformation(
          authenticator_impl::kAdditionalInformation4Bytes);
      shift = 3;
      break;
    case 8:
      SetAdditionalInformation(
          authenticator_impl::kAdditionalInformation8Bytes);
      shift = 7;
      break;
    default:
      NOTREACHED();
