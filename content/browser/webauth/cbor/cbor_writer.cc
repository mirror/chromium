...
void CBORWriter::EncodeCBOR(const CBORValue& node) {
  switch (node.type()) {
    ...
    case CBORValue::Type::BYTESTRING: {
      ...
      StartItem(CBORValue::Type::BYTESTRING,
...
void CBORWriter::StartItem(CBORValue::Type type, uint64_t size) {
  encoded_cbor_->push_back(base::checked_cast<uint8_t>(
      static_cast<unsigned>(type) << CBORValue::kMajorTypeBitShift));
  SetUint(size);
}

void CBORWriter::SetAdditionalInformation(uint8_t additional_information) {
  DCHECK(!encoded_cbor_->empty());
  DCHECK_EQ(additional_information & CBORValue::kAdditionalInformationDataMask,
            additional_information);
  encoded_cbor_->back() |=
      (additional_information & CBORValue::kAdditionalInformationDataMask);
}
...
void CBORWriter::SetUint(uint64_t value) {
  ...
  switch (count) {
    case 0:
      ...
      break;
    case 1:
      SetAdditionalInformation(CBORValue::kAdditionalInformation1Byte);
      shift = 0;
      break;
    case 2:
      SetAdditionalInformation(CBORValue::kAdditionalInformation2Bytes);
      shift = 1;
      break;
    case 4:
      SetAdditionalInformation(CBORValue::kAdditionalInformation4Bytes);
      shift = 3;
      break;
    case 8:
      SetAdditionalInformation(CBORValue::kAdditionalInformation8Bytes);
      shift = 7;
      break;
    default:
      NOTREACHED();
