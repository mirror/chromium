...
const char CBORReader::kMapKeyOutOfOrder[] =
    "Map keys must be sorted in strict mode.";

CBORReader::CBORReader(std::vector<uint8_t>::const_iterator it,
                       std::vector<uint8_t>::const_iterator end,
                       CBORReaderOption option)
    : it_(it), end_(end), error_code_(CBOR_NO_ERROR), option_(option) {}
...
base::Optional<CBORValue> CBORReader::Read(const std::vector<uint8_t>& data,
  ...
  CBORReader reader(data.begin(), data.end(), option);
...
bool CBORReader::ReadUnsignedInt(int additional_info, uint64_t* length) {
  ...
  *length = int_data;
  return CheckUintEncodededByteLength(additional_bytes, int_data);
}

bool CBORReader::CheckUintEncodededByteLength(uint8_t additional_bytes,
                                              uint64_t uint_data) {
...
void CBORReader::CheckOutOfOrderKey(const std::string& new_key,
  ...
  if (option_ != ALLOW_UNSORTED_MAP_KEYS && !map->empty() &&
      comparator(new_key, map->rbegin()->first))
