...
TEST(CBORValuesTest, ConstructNegative) {
  ...
  EXPECT_EQ(100u, value.GetNegative());
}
...
TEST(CBORValuesTest, CopyNegativeInt) {
  ...
  CBORValue copied_value(value.Clone());
  EXPECT_EQ(value.type(), copied_value.type());
  EXPECT_EQ(value.GetNegative(), copied_value.GetNegative());

  CBORValue blank;

  blank = value.Clone();
  EXPECT_EQ(value.type(), blank.type());
  EXPECT_EQ(value.GetNegative(), blank.GetNegative());
}
