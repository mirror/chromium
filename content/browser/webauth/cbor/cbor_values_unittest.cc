...
TEST(CBORValuesTest, TestNothrow) {
  ...
  static_assert(
      std::is_nothrow_constructible<CBORValue, std::vector<uint8_t>&&>::value,
      "IsNothrowMoveConstructibleFromBytestring");
  static_assert(
      std::is_nothrow_constructible<CBORValue, std::vector<CBORValue>&&>::value,
...
// Test constructors
TEST(CBORValuesTest, ConstructInt) {
  CBORValue value(37);
...
TEST(CBORValuesTest, ConstructBytestring) {
  CBORValue value(std::vector<uint8_t>({0xF, 0x0, 0x0, 0xB, 0xA, 0x2}));
  EXPECT_EQ(CBORValue::Type::BYTESTRING, value.type());
  EXPECT_EQ(std::vector<uint8_t>({0xF, 0x0, 0x0, 0xB, 0xA, 0x2}),
            value.GetBytestring());
}
...
TEST(CBORValuesTest, MoveBytestring) {
  const std::vector<uint8_t> bytes({0xF, 0x0, 0x0, 0xB, 0xA, 0x2});
