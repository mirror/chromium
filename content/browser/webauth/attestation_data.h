...
// Information about the authenticator attestation that is appended
...
class CONTENT_EXPORT AttestationData {
 public:
  ...
  static std::unique_ptr<AttestationData> CreateFromU2fResponse(
      const std::vector<uint8_t>& u2f_data,
      std::vector<uint8_t> aaguid,
      std::unique_ptr<PublicKey> public_key);

  const std::vector<uint8_t>& GetCredentialId();
