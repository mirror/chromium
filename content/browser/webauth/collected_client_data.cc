...
std::unique_ptr<CollectedClientData> CollectedClientData::Create(
  ...
  std::unique_ptr<CollectedClientData> client_data(new CollectedClientData(
      std::move(encoded_challenge), std::move(relying_party_id), "SHA-256",
      "unused"));

  return client_data;
}

CollectedClientData::CollectedClientData(const std::string challenge,
...
std::string CollectedClientData::SerializeToJsonString() {
  base::DictionaryValue client_data;
  client_data.SetString(kChallengeKey, challenge_);

  // The serialization of callerOrigin.
  client_data.SetString(kOriginKey, origin_);
  ...
  client_data.SetString(kHashAlg, hash_alg_);
  ...
  // TODO(kpaulhamus): Fetch and add the Token Binding ID public key used to
  // communicate with the origin.
  client_data.SetString(kTokenBindingKey, token_binding_id_);
