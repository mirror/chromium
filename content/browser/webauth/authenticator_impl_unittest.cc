...
TEST_F(AuthenticatorImplTest, TestSerializedPublicKey) {
  std::unique_ptr<ECPublicKey> public_key = ECPublicKey::Create(
      authenticator_utils::kEs256, kTestU2fRegisterResponse);
  EXPECT_EQ(kTestEcKeyCbor, public_key->SerializeToCborEncodedBytes());
}
...
TEST_F(AuthenticatorImplTest, TestRegisterResponseData) {
  ...
  EXPECT_EQ(std::vector<uint8_t>(kTestClientDataJsonString.begin(),
            ...
            response->client_data_json_bytes());
  EXPECT_EQ(kTestCredentialRawIdBytes, response->raw_id());
  EXPECT_EQ(GetTestAttestationObjectBytes(),
            response->cbor_encoded_attestation_object());
}
