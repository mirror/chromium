...
const int32_t kCoseEs256 = -7;

static const std::string kTestRelyingPartyId = "google.com";

static const std::vector<uint8_t> kTestChallengeBytes = {
...
static const std::string kTestClientDataJsonString =
...
//  "alg": "ES256"}
static const std::vector<uint8_t> kTestEcKeyCbor = {
    // clang-format off
    0xA3,  // map(3)
      0x61,  // text(1)
        0x78,  // "x"
...
// kTestEcKeyCbor to get the complete attestation data.
...
static const std::vector<uint8_t> kU2fAttestationStatementCbor = {
    // clang-format off
    0xA2,  // map(2)
      0x63,  // text(3)
        0x73, 0x69, 0x67,  // "sig"
...
static const std::vector<uint8_t> kFormatFidoU2fCbor = {
    // clang-format off
    0xA3,  // map(3)
      0x63,  // text(3)
        0x66, 0x6D, 0x74,  // "fmt"
...
static const std::vector<uint8_t> kAttStmtCbor = {
    // clang-format off
      0x67,  // text(7)
...
std::unique_ptr<CollectedClientData> GetTestClientData() {
  std::unique_ptr<CollectedClientData> client_data =
      CollectedClientData::Create(kTestRelyingPartyId, kTestChallengeBytes);
  return client_data;
}

std::vector<uint8_t> GetTestAttestationDataBytes() {
  // Combine kTestAttestationDataPrefix and kTestEcKeyCbor.
  std::vector<uint8_t> test_attestation_data = kTestAttestationDataPrefix;
  test_attestation_data.insert(test_attestation_data.end(),
                               kTestEcKeyCbor.begin(), kTestEcKeyCbor.end());
  return test_attestation_data;
}

std::vector<uint8_t> GetTestAuthenticatorDataBytes() {
  // Build the test authenticator data.
  std::vector<uint8_t> test_authenticator_data = kTestAuthenticatorDataPrefix;
...
/*std::vector<uint8_t> GetTestAttestationObjectBytes() {
  std::vector<uint8_t> test_authenticator_object = kAttStmtCbor;
  test_authenticator_object.insert(test_authenticator_object.end(),
                                   kU2fAttestationStatementCbor.begin(),
                                   kU2fAttestationStatementCbor.end());
  test_authenticator_object.insert(test_authenticator_object.end(),
  ...
  test_authenticator_object.insert(test_authenticator_object.end(),
                                   kFormatFidoU2fCbor.begin(),
                                   kFormatFidoU2fCbor.end());
  return test_authenticator_object;
}*/
std::vector<uint8_t> GetTestAttestationObjectBytes() {
  std::vector<uint8_t> test_authenticator_object = kFormatFidoU2fCbor;
  test_authenticator_object.insert(test_authenticator_object.end(),
                                   kAttStmtCbor.begin(), kAttStmtCbor.end());
  test_authenticator_object.insert(test_authenticator_object.end(),
                                   kU2fAttestationStatementCbor.begin(),
                                   kU2fAttestationStatementCbor.end());
  test_authenticator_object.insert(test_authenticator_object.end(),
...
TEST_F(AuthenticatorImplTest, TestSerializedPublicKey) {
  std::unique_ptr<ECPublicKey> public_key = ECPublicKey::Create(
      authenticator_internal::kEs256, kTestU2fRegisterResponse);
  EXPECT_EQ(kTestEcKeyCbor, public_key->SerializeToCborEncodedByteArray());
}
...
TEST_F(AuthenticatorImplTest, TestU2fAttestationStatementCbor) {
  std::unique_ptr<FidoAttestationStatement> fido_attestation_statement =
      FidoAttestationStatement::Create(kTestU2fRegisterResponse);
  std::vector<uint8_t> cbor =
      CBORWriter::Write(fido_attestation_statement->GetAsCborMap());
  EXPECT_EQ(kU2fAttestationStatementCbor, cbor);
}
...
TEST_F(AuthenticatorImplTest, TestAttestationData) {
  ...
  std::unique_ptr<AttestationData> attestation_data =
      AttestationData::CreateFromU2fResponse(
          kTestU2fRegisterResponse, std::vector<uint8_t>(16, 0) /* aaguid */,
          std::move(public_key));

  EXPECT_EQ(GetTestAttestationDataBytes(),
            attestation_data->SerializeToByteArray());
}
...
TEST_F(AuthenticatorImplTest, TestAuthenticatorData) {
  ...
  std::unique_ptr<AuthenticatorData> authenticator_data =
      AuthenticatorData::Create(GetTestClientData()->SerializeToJson(),
                                0x00 /* flags */,
...
TEST_F(AuthenticatorImplTest, TestU2fAttestationObject) {
  ...
  EXPECT_EQ(GetTestAttestationObjectBytes(),
            attestation_object->SerializeToCborEncodedByteArray());
}
...
TEST_F(AuthenticatorImplTest, TestRegisterResponseData) {
  ...
  std::unique_ptr<RegisterResponseData> response = RegisterResponseData::Create(
      std::move(client_data), std::move(kTestU2fRegisterResponse));

  EXPECT_EQ(std::vector<uint8_t>(kTestClientDataJsonString.begin(),
                                 kTestClientDataJsonString.end()),
            response->GetClientDataJsonBytes());
  EXPECT_EQ(kTestCredentialRawIdBytes, response->GetRawId());
  EXPECT_EQ(GetTestAttestationObjectBytes(),
