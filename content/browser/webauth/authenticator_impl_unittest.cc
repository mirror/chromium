...
class AuthenticatorImplTest : public content::RenderViewHostTestHarness {
 ...
 protected:
  ...
  void SetConnectorForTesting(service_manager::Connector* connector) {
    authenticator_impl_->SetConnectorForTesting(connector);
  }

  void SetTimerForTesting(std::unique_ptr<base::OneShotTimer> timer) {
    authenticator_impl_->SetTimerForTesting(std::move(timer));
  }
...
TEST_F(AuthenticatorImplTest, TestTimeout) {
  ...
  SetConnectorForTesting(connector.get());

  // Set up a timer for testing.
  scoped_refptr<base::TestMockTimeTaskRunner> task_runner(
      new base::TestMockTimeTaskRunner(base::Time::Now(),
                                       base::TimeTicks::Now()));
  auto timer = base::MakeUnique<base::OneShotTimer>(
      task_runner->GetMockTickClock().get());
