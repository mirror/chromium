...
std::unique_ptr<RegisterResponseData> RegisterResponseData::Create(
  ...
  std::unique_ptr<ECPublicKey> public_key =
      ECPublicKey::Create(kEs256, u2f_data);
  ...
  std::unique_ptr<AttestationData> attestation_data =
      AttestationData::Create(u2f_data, aaguid, std::move(public_key));
  ...
  std::unique_ptr<AuthenticatorData> authenticator_data =
      AuthenticatorData::Create(client_data->SerializeToJsonString(), 0x00,
                                counter, std::move(attestation_data));
  ...
  std::unique_ptr<RegisterResponseData> response_data(
      new RegisterResponseData(std::move(client_data), std::move(credential_id),
...
std::vector<uint8_t> RegisterResponseData::GetClientDataJsonBytes() {
  std::string client_data_json = client_data_->SerializeToJsonString();
