...
// static
std::unique_ptr<AttestationData> AttestationData::Create(
  ...
  std::unique_ptr<AttestationData> data(
      new AttestationData(std::move(aaguid), std::move(id_length),
...
AttestationData::AttestationData(const std::vector<uint8_t> aaguid,
  ...
  CHECK_EQ(16u, aaguid.size());
}

std::vector<uint8_t> AttestationData::GetCredentialId() {
