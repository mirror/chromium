...
// CBOR prefix for the attestation statement.
static const std::vector<uint8_t> kAttestationStatementCBOR = {
    0xA3, 0x67, 0x61, 0x74, 0x74, 0x53, 0x74, 0x6D, 0x74, 0xA2, 0x63};

// CBOR prefix for the signature in the attestation statement.
static const std::vector<uint8_t> kTestSignatureCBOR = {0x73, 0x69, 0x67, 0x58,
                                                        0x46};

static const std::vector<uint8_t> kTestSignature = {
...
// CBOR prefix for the x509 certs in the attestation statement.
static const std::vector<uint8_t> kTestX5cCBOR = {0x63, 0x78, 0x35, 0x63,
                                                  0x81, 0x59, 0x02, 0x4E};

static const std::vector<uint8_t> kTestX5c = {
...
static const std::vector<uint8_t> kTestCredentialRawId = {
...
// The entire attestation_data consists of kTestAttestationPrefix +
// kTestPublicKeyCBOR.
std::vector<uint8_t> GetTestAttestationData() {
  ...
  test_attestation_data.insert(test_attestation_data.end(),
                               kTestAttestationDataPrefix.begin(),
                               kTestAttestationDataPrefix.end());
  test_attestation_data.insert(test_attestation_data.end(),
...
std::vector<uint8_t> GetTestAttestationStatement() {
  std::vector<uint8_t> test_attestation_statement;
  test_attestation_statement.insert(test_attestation_statement.end(),
                                    kAttestationStatementCBOR.begin(),
                                    kAttestationStatementCBOR.end());
  test_attestation_statement.insert(test_attestation_statement.end(),
                                    kTestSignatureCBOR.begin(),
                                    kTestSignatureCBOR.end());
  test_attestation_statement.insert(test_attestation_statement.end(),
                                    kTestSignature.begin(),
                                    kTestSignature.end());
  test_attestation_statement.insert(test_attestation_statement.end(),
                                    kTestX5cCBOR.begin(), kTestX5cCBOR.end());
  test_attestation_statement.insert(test_attestation_statement.end(),
                                    kTestX5c.begin(), kTestX5c.end());
  return test_attestation_statement;
}

// The entire authenticator_data consists of the concatenation of
// kTestRelyingPartyIdHash, kTestRegisterAuthenticatorDataFlagsAndCount, and
//  TestAttestationData.
...
// The entire attestation_object consists of the concatenation of
// GetTestAttestationStatement,  kTestPublicKeyCBOR, kAuthDataCBOR,
//  GetTestRegisterAuthenticatorData, and kFmtCBOR.
std::vector<uint8_t> GetTestAttestationObject() {
  ...
  test_attestation_object.insert(test_attestation_object.end(),
                                 test_attestation_statement.begin(),
                                 test_attestation_statement.end());
...
TEST(AuthenticatorUtilsTest, TestBuildClientData) {
  std::string client_data_json = BuildClientData("google.com", kTestChallenge);
  EXPECT_EQ(client_data_json, kTestClientDataJSON);
}

TEST(AuthenticatorUtilsTest, TestConstructPublicKeyCredentialInfo) {
  webauth::mojom::PublicKeyCredentialInfoPtr credential_info =
      ConstructPublicKeyCredentialInfo(kTestClientDataJSON,
                                       kTestU2fRegisterResponse);
  EXPECT_EQ(credential_info->raw_id, kTestCredentialRawId);
  std::string id;
  base::Base64UrlEncode(base::StringPiece(reinterpret_cast<const char*>(
                                              kTestCredentialRawId.data()),
                                          kTestCredentialRawId.size()),
                        base::Base64UrlEncodePolicy::OMIT_PADDING, &id);
  EXPECT_EQ(credential_info->id, id);
  EXPECT_EQ(credential_info->client_data_json,
            std::vector<uint8_t>(kTestClientDataJSON.begin(),
                                 kTestClientDataJSON.end()));
  EXPECT_EQ(credential_info->response->attestation_object,
            GetTestAttestationObject());
  EXPECT_EQ(true, credential_info->response->authenticator_data.empty());
  EXPECT_EQ(true, credential_info->response->signature.empty());
}
...
TEST(AuthenticatorUtilsTest, TestConstructAuthenticatorAttestationResponse) {
  ...
  EXPECT_EQ(credential_id, kTestCredentialRawId);
...
TEST(AuthenticatorUtilsTest, TestParseU2fRegisterResponse) {
  ...
  ParseU2fRegisterResponse(kTestU2fRegisterResponse, attestation_data,
                           signature, x5c, credential_id);
  EXPECT_EQ(attestation_data, GetTestAttestationData());
  EXPECT_EQ(credential_id, kTestCredentialRawId);
  EXPECT_EQ(signature, kTestSignature);
  EXPECT_EQ(x5c, kTestX5c);
}

TEST(AuthenticatorUtilsTest, TestConstructPublicKeyObjectForRegister) {
  std::vector<uint8_t> public_key;
  ConstructPublicKeyObject(kTestU2fRegisterResponse, public_key);
  EXPECT_EQ(public_key, kTestPublicKeyCBOR);
}

// TODO fix this:
TEST(AuthenticatorUtilsTest, TestExtractU2fAttestationStatement) {
  std::vector<uint8_t> signature;
  std::vector<uint8_t> x5c;

  // Where the key handle ends and x509 cert starts.
  uint32_t x509_start_byte = 131;  // 67 + len of test credential ID (64 bytes).
  ExtractU2fAttestationStatementData(kTestU2fRegisterResponse, signature, x5c,
                                     x509_start_byte);
  EXPECT_EQ(signature, kTestSignature);
  EXPECT_EQ(x5c, kTestX5c);
}

TEST(AuthenticatorUtilsTest, TestSerializeU2fAttestationObject) {
  std::vector<uint8_t> attestation_object_out;
  SerializeU2fAttestationObject(GetTestRegisterAuthenticatorData(),
                                kTestSignature, kTestX5c,
                                attestation_object_out);
