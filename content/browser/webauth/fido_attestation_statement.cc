...
std::unique_ptr<FidoAttestationStatement> FidoAttestationStatement::Create(
  ...
  CHECK_GE(u2f_data.size(), kU2fResponseLengthPos + 1);
  ...
  std::unique_ptr<FidoAttestationStatement> statement(
...
FidoAttestationStatement::FidoAttestationStatement(
    ...
    : AttestationStatement(kFormat),
      signature_(std::move(signature)),
      x509_certs_(std::move(x5c)) {}

base::flat_map<std::string, CBORValue>
FidoAttestationStatement::GetAsCborMap() {
