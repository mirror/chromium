...
float OverscrollWindowAnimation::GetTranslationForOverscroll(float delta_x) {
  ...
  const float bounds_width = delegate_->GetMainWindow()->bounds().width();
...
void OverscrollWindowAnimation::OnOverscrollModeChange(
  ...
  gfx::Rect slide_window_bounds(delegate_->GetMainWindow()->bounds().size());
...
void OverscrollWindowAnimation::OnOverscrollComplete(
  ...
  int content_width = delegate_->GetMainWindow()->bounds().width();
  float translate_x;
