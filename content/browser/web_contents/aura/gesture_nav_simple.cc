...
class Affordance : public ui::LayerDelegate, public gfx::AnimationDelegate {
 ...
 private:
  ...
  // Helper function that returns the location for painted layer according to
  // the overscroll mode.
...
void Affordance::UpdateArrowLayer() {
  ...
  if (mode_ == OVERSCROLL_SOUTH) {
    ...
    arrow_layer_transform.Rotate(kReloadArrowInitialRotation * (1 - progress));
    arrow_layer_transform.Translate(-arrow_offset);
  } else {
