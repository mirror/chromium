...
class CONTENT_EXPORT WebContentsImpl : public WebContents,
 ...
 private:
  ...
  // WebContents::CreateParams::initially_hidden. Updated when the visibility of
  // the view changes.
  Visibility visibility_ = Visibility::HIDDEN;

  // Tracks whether DoWasShown() was ever called. This is used to ensure that
  // DoWasShown() is called when a WebContents whose initial visibility is
  // VISIBLE actually becomes visible.
  bool did_call_do_was_shown_ = false;

  // Whether DoWasHidden() should be called when the visibility of the
  // WebContents becomes HIDDEN. See details in OnVisibilityChanged().
