...
TEST_F(WebContentsImplTest, SimpleNavigation) {
  ...
  // Start the navigation until it is ready to commit.
...
TEST_F(WebContentsImplTest, NavigateTwoTabsCrossSite) {
  ...
  NavigationSimulator::NavigateAndCommitFromBrowser(contents(), url2a);
  ...
  navigation->ReadyToCommit();
  EXPECT_NE(nullptr, contents2->GetPendingMainFrame());
...
TEST_F(WebContentsImplTest, CrossSiteNotPreemptedDuringBeforeUnload) {
  ...
  auto navigation = NavigationSimulator::CreateRendererInitiated(url, orig_rfh);
...
TEST_F(WebContentsImplTest, NavigationEntryContentStateNewWindow) {
  ...
  // TODO(ahemery): Check that a navigation form renderer makes sense here.
  NavigationSimulator::NavigateAndCommitFromDocument(url, orig_rfh);
