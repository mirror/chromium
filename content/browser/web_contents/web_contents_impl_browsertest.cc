...
IN_PROC_BROWSER_TEST_F(WebContentsImplBrowserTest, WindowOpen_SetsPopupBit) {
  ...
  EXPECT_TRUE(
      NavigateToURL(shell(), embedded_test_server()->GetURL("/title1.html")));
  EXPECT_EQ(
      WebContents::CreateParams::PopupInfo::IS_NOT_POPUP,
      static_cast<WebContentsImpl*>(shell()->web_contents())->popup_info_);
  ...
  EXPECT_EQ(WebContents::CreateParams::PopupInfo::IS_POPUP,
            new_contents->popup_info_);
}
