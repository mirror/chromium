...
void WebContentsAndroid::NotifyOnNextCompositorFrameAck(
  ...
  RenderWidgetHostViewAndroid* view = GetRenderWidgetHostViewAndroid();
  if (view) {
    view->NotifyOnNextCompositorFrameAck(base::BindOnce(
        &WebContentsAndroid::OnNextCompositorFrameAck,
        weak_factory_.GetWeakPtr(), ScopedJavaGlobalRef<jobject>(env, obj),
        ScopedJavaGlobalRef<jobject>(env, jcallback)));
  }
}
