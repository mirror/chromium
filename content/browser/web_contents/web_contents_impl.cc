...
void WebContentsImpl::OnVisibilityChanged(Visibility visibility) {
  // Trust that the initial visibility of the WebContents is correct and do
  // nothing if the visibility changes to HIDDEN before it changes to VISIBLE or
  // OCCLUDED (except if the change comes from WasHidden). A change to HIDDEN
  // happens when the WebContents' view is added to a hidden parent that is
  // about to become visible. Calling DoWasHidden() in response to that change
  // would result in releasing resources that would immediately be recreated.
  ...
  if (visibility == Visibility::VISIBLE &&
      (previous_visibility != Visibility::VISIBLE || !did_call_do_was_shown_)) {
    DoWasShown();
  } else if (previous_visibility == Visibility::VISIBLE && !IsBeingCaptured()) {
    DoWasHidden();
  }
