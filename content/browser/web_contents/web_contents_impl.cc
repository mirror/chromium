...
void WebContentsImpl::ReplaceTextAtRange(const gfx::Range& range,
                                         const base::string16& word) {
  RenderFrameHostImpl* focused_frame = GetFocusedFrame();
  if (!focused_frame)
    return;

  focused_frame->GetFrameInputHandler()->ReplaceTextAtRange(range, word);
}
...
content::TextSuggestionInfo WebContentsImpl::GetTextSuggestionInfo() {
  const TextInputManager::TextSelection* selection =
      GetTextInputManager()->GetTextSelection();

  content::TextSuggestionInfo info;
  info.text = selection->text();

  if (selection->selected_text().empty()) {
    info.start = selection->word_offset();
    info.offsetFromCursor = selection->range().end() - selection->word_offset();
  } else {
    info.start = selection->range().start();
    info.offsetFromCursor = 0;
  }

  info.end = selection->range().end();
  info.text_for_suggestion =
      info.text.substr(info.start, info.end - info.start);
  return info;
}
