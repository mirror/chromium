...
const base::string16& WebContentsImpl::GetTitle() const {
  ...
  // only recently started loading, stick to the old page's title. The
  // exception is view source mode --- don't mess with that title.
  if (initial_load_timer_.IsRunning() && entry && entry->GetTitle().empty() &&
      !entry->IsViewSourceMode() && !last_page_title_.empty()) {
    return last_page_title_;
  }

  if (entry)
    last_page_title_ = entry->GetTitleForDisplay();

  // |last_page_title_| is finally used if no title cannot be retrieved.
  return last_page_title_;
}
...
void WebContentsImpl::LoadingStateChanged(bool to_different_document,
  ...
  if (was_waiting_for_response && !waiting_for_response_) {
    initial_load_timer_.Stop();
    NotifyEffectiveTitleChanged();
  } else if (!was_waiting_for_response && waiting_for_response_) {
    initial_load_timer_.Start(
        FROM_HERE,
        base::TimeDelta::FromMilliseconds(kNewContentRenderingDelayMs), this,
        &WebContentsImpl::NotifyEffectiveTitleChanged);
  }
...
void WebContentsImpl::NotifyEffectiveTitleChanged() {
  NavigationEntry* entry = controller_.GetLastCommittedEntry();
  for (auto& observer : observers_)
