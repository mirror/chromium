...
static mojom::NetworkServicePtr* g_network_service_ptr = nullptr;
static NetworkServiceImpl* g_network_service;

void CreateNetworkServiceOnIO(mojom::NetworkServiceRequest request,
                              net::NetLog* net_log) {
  DCHECK(!g_network_service);
  g_network_service =
      new NetworkServiceImpl(nullptr, std::move(request), net_log);
}
...
mojom::NetworkService* GetNetworkService() {
  ...
  if (!g_network_service_ptr->is_bound() ||
    ...
    g_network_service_ptr->get()->SetClient(std::move(client_ptr));
  }
