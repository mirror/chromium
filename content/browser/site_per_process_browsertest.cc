...
IN_PROC_BROWSER_TEST_F(SitePerProcessBrowserTest,
  ...
  // Finally, navigate the child frame to another page on the same origin; its
  // sandbox flags should be completely cleared, and should be cleared in the
  // proxy in the main frame's renderer as well.
  ...
  // Check the sandbox flags in the browser process
  EXPECT_EQ(
  ...
  // Check the sandbox flags in the main frame's renderer by attempting to open
  // a popup. This should succeed.
  bool success = false;
  EXPECT_TRUE(ExecuteScriptAndExtractBool(
      root->child_at(1)->child_at(0),
      "window.domAutomationController.send("
