...
IN_PROC_BROWSER_TEST_F(SitePerProcessBrowserTest,
  ...
  float* delta = &gesture_scroll_begin.data.scroll_begin.delta_x_hint;
#elif defined(OS_ANDROID)
  // For android, we scroll vertically to activate pull-to-refresh.
  float* delta = &gesture_scroll_begin.data.scroll_begin.delta_y_hint;
#endif
  *delta = threshold + 1;
...
#if defined(USE_AURA)
  // For aura, we scroll horizontally to activate an overscroll navigation.
  delta = &gesture_scroll_update.data.scroll_update.delta_x;
#elif defined(OS_ANDROID)
  // For android, we scroll vertically to activate pull-to-refresh.
  delta = &gesture_scroll_update.data.scroll_update.delta_y;
#endif
  *delta = threshold + 1;
