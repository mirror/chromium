...
void OverlapSurfaceHitTestHelper(
  ...
  LOG(ERROR) << "XXXDebug: bounds: " << bounds.ToString() << ", position: "
      << gfx::PointF(button_event.PositionInWidget()).ToString();

  // Check the renderer hit-test API return the correct frame for
  // the cross site iframe. In this page, the button from the main-page overlaps
  // the iframe. So the hit-test should return the main-frame.
  base::RunLoop run_loop;
  viz::FrameSinkId received_frame_sink_id;
  base::Closure quit_closure =
      content::GetDeferredQuitTaskForRunLoop(&run_loop);
  DCHECK_NE(root->current_frame_host()->GetInputTargetClient(), nullptr);
  root->current_frame_host()->GetInputTargetClient()->FrameSinkIdAt(
      gfx::ToCeiledPoint(button_event.PositionInWidget()),
      base::BindLambdaForTesting([&](const viz::FrameSinkId& id) {
        received_frame_sink_id = id;
        quit_closure.Run();
      }));
  content::RunThisRunLoop(&run_loop);
  ASSERT_EQ(
      viz::FrameSinkId(rwhv_root->GetRenderWidgetHost()->GetProcess()->GetID(),
  ...
  // The expected result coordinates are (5, 5), but can get slightly
  // different results due to rounding error with some page scale factors.
  EXPECT_NEAR(button_location.x(), main_frame_monitor.event().PositionInWidget().x, 2);
  EXPECT_NEAR(button_location.y(), main_frame_monitor.event().PositionInWidget().y, 2);
  EXPECT_FALSE(child_frame_monitor.EventWasReceived());

  // Now send an event that should go to the bottom-right edge of child.
  gfx::PointF event_location(145, 145);
  blink::WebMouseEvent child_event(blink::WebInputEvent::kMouseDown,
                                   blink::WebInputEvent::kNoModifiers,
                                   blink::WebInputEvent::kTimeStampForTesting);
  ...
  RouteMouseEventAndWaitUntilDispatch(router, rwhv_root, rwhv_child,
                                      &child_event);
  EXPECT_TRUE(child_frame_monitor.EventWasReceived());
  // The expected result coordinates are (5, 5) from the bottom-right edge, but
  // can get slightly different results due to rounding error with some page
  // scale factors.
