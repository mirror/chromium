...
IN_PROC_BROWSER_TEST_F(SitePerProcessBrowserTest,
  ...
  auto shutdown_observer = std::make_unique<ShutdownObserver>();
  subframe_process->AddObserver(shutdown_observer.get());
  NavigateFrameToURL(root->child_at(0),
                     embedded_test_server()->GetURL("a.com", "/title1.html"));
  shutdown_observer->Wait();
  subframe_process->RemoveObserver(shutdown_observer.get());
