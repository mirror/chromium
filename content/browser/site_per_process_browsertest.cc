...
void OverlapSurfaceHitTestHelper(
  ...
  button_event.SetPositionInWidget(
      gfx::ToCeiledInt((bounds.x() - root_view->GetViewBounds().x() + 5) *
                       scale_factor),
      gfx::ToCeiledInt((bounds.y() - root_view->GetViewBounds().y() + 5) *
                       scale_factor));
  ...
  root->current_frame_host()->GetInputTargetClient()->FrameSinkIdAt(
      gfx::ScaleToCeiledPoint(
          gfx::ToCeiledPoint(button_event.PositionInWidget()), scale_factor,
          scale_factor),
      base::BindLambdaForTesting([&](const viz::FrameSinkId& id) {
        received_frame_sink_id = id;
        quit_closure.Run();
  ...
  // The expected result coordinates are (5, 5), but can get slightly
  // different results due to rounding error with some page scale factors.
  EXPECT_NEAR(55, main_frame_monitor.event().PositionInWidget().x, 2);
  EXPECT_NEAR(55, main_frame_monitor.event().PositionInWidget().y, 2);
  EXPECT_FALSE(child_frame_monitor.EventWasReceived());
  ...
  blink::WebMouseEvent child_event(blink::WebInputEvent::kMouseDown,
                                   ...
                                   blink::WebInputEvent::kTimeStampForTesting);
  child_event.SetPositionInWidget(
      gfx::ToCeiledInt((bounds.right() - root_view->GetViewBounds().x() - 5) *
                       scale_factor),
      gfx::ToCeiledInt((bounds.bottom() - root_view->GetViewBounds().y() - 5) *
                       scale_factor));
  child_event.click_count = 1;
  main_frame_monitor.ResetEventReceived();
  child_frame_monitor.ResetEventReceived();
