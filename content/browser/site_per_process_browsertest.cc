...
IN_PROC_BROWSER_TEST_F(SitePerProcessBrowserTest, ScrollElementIntoView) {
  ...
  // (Sanity check) Verifying that the initial bounds are out of view. Size of
  // document.body (container of iframe) should be larger than viewbounds. Then
  // iframe's x/y should be larger than 110% of viewbounds width/height.
  gfx::Rect view_bounds = main_frame->GetView()->GetViewBounds();
  EXPECT_GT(child_frame_b_bounds_before_scroll.x(),
            static_cast<int>(view_bounds.width() * 1.1));
  EXPECT_GT(child_frame_b_bounds_before_scroll.y(),
            static_cast<int>(view_bounds.height() * 1.1));
  ...
  EXPECT_LE(child_frame_b_bounds_after_scroll.x(), view_bounds.width());
  EXPECT_LE(child_frame_b_bounds_after_scroll.y(), view_bounds.height());
