...
IN_PROC_BROWSER_TEST_F(SitePerProcessBrowserTest,
                       HiddenFrameWillNotCreateCompositorFrame) {
  GURL main_url(
  ...
  GURL cross_site_url_b =
      embedded_test_server()->GetURL("oopif.com", "/counter.html");

  FrameTreeNode* root = web_contents()->GetFrameTree()->root();

  TestNavigationObserver first_observer(shell()->web_contents());

  NavigateFrameToURL(root->child_at(0), cross_site_url_b);
  ASSERT_EQ(cross_site_url_b, first_observer.last_navigation_url());
  ASSERT_TRUE(first_observer.last_navigation_succeeded());

  TestNavigationObserver second_observer(shell()->web_contents());
  NavigateFrameToURL(root->child_at(1), cross_site_url_b);
  ASSERT_EQ(cross_site_url_b, second_observer.last_navigation_url());
  ASSERT_TRUE(second_observer.last_navigation_succeeded());
  ...
  ASSERT_LT(20u, first_counter.GetCount() + 1u);
  ...
  second_counter.WaitForNewFrames(20u);

  ASSERT_LT(20u, second_counter.GetCount() + 1u);
