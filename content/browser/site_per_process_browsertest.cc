...
void DispatchMouseEventAndWaitUntilDispatch(
  ...
  LOG(ERROR) << "Sending mouse-down at: " << root_location.ToString();
...
void HitTestLayerSquashing(
  ...
  std::string iframe_rect;
  EXPECT_TRUE(ExecuteScriptWithoutUserGestureAndExtractString(web_contents,
      "domAutomationController.send(JSON.stringify(I.getBoundingClientRect()))",
      &iframe_rect));

  gfx::Vector2dF child_offset = rwhv_child->GetViewBounds().origin() -
                                rwhv_root->GetViewBounds().origin();
  LOG(ERROR) << "Child offset: " << child_offset.ToString()
             << " scale-factor: " << GetPageScaleFactor(shell)
             << " rect: " << iframe_rect
             << " pixel-ratio: " << GetFrameDeviceScaleFactor(web_contents);
  std::string elem;
  EXPECT_TRUE(ExecuteScriptWithoutUserGestureAndExtractString(web_contents,
      "domAutomationController.send(document.elementFromPoint(395, 11).getAttribute('id'))",
      &elem));
  LOG(ERROR) << "Elememt at (395, 11): " << elem;
  // Send a mouse-down on #B. The main-frame should receive it.
  DispatchMouseEventAndWaitUntilDispatch(web_contents, rwhv_root,
                                         gfx::PointF(395, 11), rwhv_root,
                                         gfx::PointF(395, 11));
  EXPECT_TRUE(ExecuteScriptWithoutUserGestureAndExtractString(web_contents,
      "domAutomationController.send(document.elementFromPoint(395, 30).getAttribute('id'))",
      &elem));
  LOG(ERROR) << "Elememt at (395, 30): " << elem;
  // Send another event just below. The child-frame should receive it.
  DispatchMouseEventAndWaitUntilDispatch(web_contents, rwhv_root,
                                         gfx::PointF(395, 30), rwhv_child,
                                         gfx::PointF(395, 30) - child_offset);
  ...
  DispatchMouseEventAndWaitUntilDispatch(web_contents, rwhv_root,
                                         gfx::PointF(395, 235), rwhv_child,
                                         gfx::PointF(395, 235) - child_offset);
}
