...
void OverlapSurfaceHitTestHelper(
  ...
  button_event.SetPositionInWidget(button_location.x() * scale_factor,
                                   button_location.y() * scale_factor);
  ...
  main_frame_monitor.ResetEventReceived();
  child_frame_monitor.ResetEventReceived();
  LOG(ERROR) << "Dispatching the first event to the button";
  RouteMouseEventAndWaitUntilDispatch(router, rwhv_root, rwhv_root,
                                      &button_event);
  LOG(ERROR) << "Done dispatching the first event.";
  ...
  blink::WebMouseEvent child_event(blink::WebInputEvent::kMouseDown,
                                   blink::WebInputEvent::kNoModifiers,
                                   blink::WebInputEvent::kTimeStampForTesting);
  child_event.SetPositionInWidget(event_location.x() * scale_factor,
                                  event_location.y() * scale_factor);
  child_event.click_count = 1;
  main_frame_monitor.ResetEventReceived();
  child_frame_monitor.ResetEventReceived();
  LOG(ERROR) << "Dispatching the second event to the child";
