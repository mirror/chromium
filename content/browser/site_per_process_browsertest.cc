...
IN_PROC_BROWSER_TEST_F(SitePerProcessBrowserTest, ScrollElementIntoView) {
  ...
  // (Sanity check) Verifying that the initial bounds are out of view. To this
  // end, compare main frame's view bounds (moved to origin to remove the screen
  // position offset) should not intersect with the reported bounding client
  // from <iframe> 'b'.
  gfx::Rect main_frame_bounds_moved_to_origin =
      gfx::Rect(gfx::Point(), main_frame->GetView()->GetViewBounds().size());
  EXPECT_FALSE(main_frame_bounds_moved_to_origin.Intersects(
      child_frame_b_bounds_before_scroll));

  // Scroll the inner most frame's body into view.
  EXPECT_TRUE(ExecuteScript(child_frame_c, "document.body.scrollIntoView();"));

  // Now wait until the bounds of the <iframe> 'b' in root frame changes. This
  // means <iframe> 'c' has already scrolled.
  ...
  // Now verify that the main frame's view bounds intersects with the bounds
  // after from <iframe> 'b' after it has scrolled into view.
  EXPECT_TRUE(main_frame_bounds_moved_to_origin.Intersects(
      child_frame_b_bounds_after_scroll));

  // Repeat the same for the inner most <iframe> 'c'.
  gfx::Rect child_frame_c_bounds_after_scroll =
      get_bounds_for_element(child_frame_b, "document.querySelector('iframe')");

  // Get the view bounds for <iframe> 'b' and remove offset from root frame
  // origin so that it becomes comparable to the bounding client rect from frame
  // 'c'.
  gfx::Rect child_frame_b_bounds_moved_to_origin =
      gfx::Rect(gfx::Point(), child_frame_b->GetView()->GetViewBounds().size());

  EXPECT_TRUE(child_frame_b_bounds_moved_to_origin.Intersects(
      child_frame_c_bounds_after_scroll));
}
