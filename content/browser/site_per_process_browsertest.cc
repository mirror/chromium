...
void SurfaceHitTestTestHelper(
  ...
  root->current_frame_host()->GetInputTargetClient()->FrameSinkIdAt(
      gfx::Point(child_event.PositionInWidget().x * device_scale_factor,
                 child_event.PositionInWidget().y * device_scale_factor),
...
IN_PROC_BROWSER_TEST_F(SitePerProcessBrowserTest, MAYBE_SurfaceHitTestTest) {
  SurfaceHitTestTestHelper(shell(), embedded_test_server(), 1);
}
...
IN_PROC_BROWSER_TEST_F(SitePerProcessHighDPIBrowserTest,
  ...
  SurfaceHitTestTestHelper(shell(), embedded_test_server(), kDeviceScaleFactor);
}
