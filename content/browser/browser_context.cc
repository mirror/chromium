...
void BrowserContext::Initialize(
    BrowserContext* browser_context,
    const base::FilePath& path) {
  LOG(ERROR) << "ELAD!!! BrowserContext::Initialize("
             << ((void*)browser_context) << ", " << path.value() << ").";
...
BrowserContext::~BrowserContext() {
  ...
  // |webrtc_event_log_manager| must not call any functions on |this|, but
  // rather only use it as an identifer. Therefore, we do not check
  // IsOffTheRecord(), in which case it would not have been enabled anyway.
  auto* webrtc_event_log_manager = WebRtcEventLogManager::GetInstance();
  if (webrtc_event_log_manager) {
    webrtc_event_log_manager->DisableForBrowserContext(this);
  }
