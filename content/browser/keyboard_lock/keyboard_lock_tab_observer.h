...
// logic for this feature exists in a window class with an input event
...
// reliably update lock state based on it.
//
...
class KeyboardLockTabObserver final : public content::WebContentsObserver {
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(KeyboardLockTabObserver);
};
