...
// Memo: Based on chrome/browser/extensions/chrome_url_request_util.h.
...
net::URLRequestJob* BrowserProtocolHandler::MaybeCreateJob(
  ...
  if (IsImplemented(permafill)) {
    ...
    GURL response_url(std::string(kBrowserScheme) + "://" + permafill);

    // 1.f.3. Set response's url list to << responseURL >>.
    //
    // Implemented as an internal redirect to responseURL.
    if (!fallback.empty()) {
      // Redirect to browser://<permafill-name> just to set the response URL
      // correctly.
      return new net::URLRequestRedirectJob(
          request, network_delegate, response_url,
          net::URLRequestRedirectJob::REDIRECT_307_TEMPORARY_REDIRECT,
          "Permafill");
    }

    // 1.f.7. Set response's body to the bytes corresponding to the
    // permafill script identified by permafill.
    //
    // The corresponding resource ID is obtained here and its content is
    // obtained in URLRequestResourceBundleJob.

    std::string path = request->url().path();
    if (path.size() > 0 && path[path.size() - 1] == '/') {
      path += "index.js";
    }
    const int resource_id = GetResourceIDFromPath(permafill + path);
    DCHECK_GE(resource_id, 0);

    // 1.f.8. Return response.
    return new URLRequestResourceBundleJob(
        request, network_delegate,
        base::FilePath(FILE_PATH_LITERAL("index.js")), resource_id);
  }
