...
ServiceWorkerDatabase::Status LevelDBStatusToServiceWorkerStatus(
...
ServiceWorkerDatabase::GetOriginsWithRegistrations(std::set<GURL>* origins) {
...

  {
    ...
    for (itr->Seek(kUniqueOriginKey); itr->Valid(); itr->Next()) {
      status = LevelDBStatusToServiceWorkerStatus(itr->status());
      if (status != STATUS_OK) {
        origins->clear();
        break;
      }
...
ServiceWorkerDatabase::Status ServiceWorkerDatabase::ReadRegistrationOrigin(
  ...
  std::string value;
  status = LevelDBStatusToServiceWorkerStatus(
...
ServiceWorkerDatabase::Status ServiceWorkerDatabase::LazyOpen(
  ...
  Status status = LevelDBStatusToServiceWorkerStatus(
...
ServiceWorkerDatabase::Status ServiceWorkerDatabase::ReadResourceRecords(
      ...
      CreateResourceRecordKeyPrefix(registration.version_id);
  {
    std::unique_ptr<leveldb::Iterator> itr(
        db_->NewIterator(leveldb::ReadOptions()));
    for (itr->Seek(prefix); itr->Valid(); itr->Next()) {
      Status status = LevelDBStatusToServiceWorkerStatus(itr->status());
      if (status != STATUS_OK) {
        resources->clear();
        break;
      }
