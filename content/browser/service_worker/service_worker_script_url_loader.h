...
//      service worker script storage and forwards them to the |client| (which
//      is the service worker in the renderer).
...
//      worker script storage and forwards it to the |client|.
//
...
class CONTENT_EXPORT ServiceWorkerScriptURLLoader
 ...
 private:
  ...
  // Called when some error happens. Advances to |State::kCancelled| and calls
  // CommitCompleted().
  void OnFailed(const ResourceRequestCompletionStatus& status);
  ...
  // complete. Called when URLLoaderClient::OnComplete() is called, and when the
  ...
  // Used for transferring the data in the mojo pipe into net::IOBuffer to be
  // written in the service worker script storage.
