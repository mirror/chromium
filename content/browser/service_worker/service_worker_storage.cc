...
void ServiceWorkerStorage::DeleteAndStartOver(const StatusCallback& callback) {
  ...
  // If we're using a memory backend, it won't call the cleanup handler, so do
  // it ourselves. Also if we don't have a backend in the first place.
  if (path_.empty() || !disk_cache_)
...
void ServiceWorkerStorage::InitializeDiskCache() {
  ...
  int rv = disk_cache_->InitWithDiskBackend(
      ...
      base::BindRepeating(&ServiceWorkerStorage::OnDiskCacheCleanupComplete,
                          weak_factory_.GetWeakPtr()),
