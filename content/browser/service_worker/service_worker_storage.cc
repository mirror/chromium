...
void ServiceWorkerStorage::FindRegistrationForDocument(
  ...
  switch (state_) {
    case DISABLED:
      ...
      // TODO: Do we need the TRACE_EVENT here?
  ...
  if (!base::ContainsKey(registered_origins_, document_url.GetOrigin())) {
    ...
    CompleteFindNow(
        std::move(installing_registration), status, std::move(callback));
    return;
  }
...
void ServiceWorkerStorage::FindRegistrationForPattern(
  ...
  switch (state_) {
    case DISABLED:
      CompleteFindSoon(
          FROM_HERE, scoped_refptr<ServiceWorkerRegistration>(),
          SERVICE_WORKER_ERROR_ABORT, std::move(callback));
      return;
    case INITIALIZING: // Fall-through.
    case UNINITIALIZED:
  ...
  database_task_runner_->PostTask(
      ...
      base::BindOnce(
          ...
          base::BindOnce(
              &ServiceWorkerStorage::DidFindRegistrationForPattern,
              weak_factory_.GetWeakPtr(), scope, std::move(callback))));
}
...
void ServiceWorkerStorage::GetAllRegistrationsInfos(
  ...
  switch (state_) {
    ...
    case UNINITIALIZED:
      LazyInitialize(base::BindOnce(
          &ServiceWorkerStorage::GetAllRegistrationsInfos,
          weak_factory_.GetWeakPtr(), std::move(callback)));
      return;
    case INITIALIZED:
      break;
...
void ServiceWorkerStorage::StoreUserData(
  ...
  if (registration_id == blink::mojom::kInvalidServiceWorkerRegistrationId ||
    ...
    RunSoon(FROM_HERE, base::BindOnce(
        std::move(callback), SERVICE_WORKER_ERROR_FAILED));
    return;
  }
...
void ServiceWorkerStorage::GetUserData(
  ...
  switch (state_) {
    ...
    case UNINITIALIZED:
      LazyInitialize(base::BindOnce(
          &ServiceWorkerStorage::GetUserData,
          weak_factory_.GetWeakPtr(), registration_id, keys,
          std::move(callback)));
      return;
    case INITIALIZED:
      break;
  ...
  switch (state_) {
    ...
    case UNINITIALIZED:
      LazyInitialize(base::BindOnce(
          ...
          weak_factory_.GetWeakPtr(), registration_id, key_prefix,
          std::move(callback)));
      return;
    case INITIALIZED:
      break;
...
void ServiceWorkerStorage::GetUserKeysAndDataByKeyPrefix(
  ...
  switch (state_) {
    case DISABLED:
      RunSoon(FROM_HERE,base::BindOnce(
        std::move(callback), base::flat_map<std::string, std::string>(),
        SERVICE_WORKER_ERROR_ABORT));
      return;
    case INITIALIZING: // Fall-through.
    case UNINITIALIZED:
  ...
  database_task_runner_->PostTask(
      FROM_HERE, base::BindOnce(
          ...
          registration_id, key_prefix, base::BindOnce(
              &ServiceWorkerStorage::DidGetUserKeysAndData,
              weak_factory_.GetWeakPtr(), std::move(callback))));
}
...
void ServiceWorkerStorage::ClearUserData(
  ...
  switch (state_) {
    ...
    case UNINITIALIZED:
      LazyInitialize(base::BindOnce(
          &ServiceWorkerStorage::ClearUserData,
          weak_factory_.GetWeakPtr(), registration_id, keys,
          std::move(callback)));
      return;
    case INITIALIZED:
...
void ServiceWorkerStorage::ClearUserDataByKeyPrefixes(
  ...
  switch (state_) {
    ...
    case UNINITIALIZED:
      LazyInitialize(base::BindOnce(
          &ServiceWorkerStorage::ClearUserDataByKeyPrefixes,
...
void ServiceWorkerStorage::GetUserDataForAllRegistrations(
  ...
  switch (state_) {
    ...
    case UNINITIALIZED:
      LazyInitialize(base::BindOnce(
          &ServiceWorkerStorage::GetUserDataForAllRegistrations,
...
void ServiceWorkerStorage::DeleteRegistrationFromDB(
  ...
  if (status != ServiceWorkerDatabase::STATUS_OK) {
    original_task_runner->PostTask(FROM_HERE, base::BindOnce(
        std::move(callback), OriginState::KEEP_ALL, deleted_version,
  ...
  original_task_runner->PostTask(FROM_HERE, base::BindOnce(
      std::move(callback), origin_state, deleted_version,
