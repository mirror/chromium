...
#define LAZY_INITIALIZE(CALLBACK, DISABLED_FUNC) \
  switch (state_) {                              \
    case DISABLED:                               \
      DISABLED_FUNC;                             \
      return;                                    \
...
void ServiceWorkerStorage::FindRegistrationForIdOnly(
    ...
    FindRegistrationCallback callback) {
  LAZY_INITIALIZE(
      base::BindOnce(&ServiceWorkerStorage::FindRegistrationForIdOnly,
                     weak_factory_.GetWeakPtr(), registration_id,
                     std::move(callback)),
      CompleteFindNow(nullptr, SERVICE_WORKER_ERROR_ABORT,
                      std::move(callback)));
...
void ServiceWorkerStorage::StoreUserData(
    ...
    StatusCallback callback) {
  LAZY_INITIALIZE(
      base::BindOnce(&ServiceWorkerStorage::StoreUserData,
                     weak_factory_.GetWeakPtr(), registration_id, origin,
...
void ServiceWorkerStorage::GetUserData(int64_t registration_id,
  ...
  LAZY_INITIALIZE(
      base::BindOnce(&ServiceWorkerStorage::GetUserData,
...
void ServiceWorkerStorage::ClearUserData(int64_t registration_id,
                                         ...
                                         StatusCallback callback) {
  LAZY_INITIALIZE(
      base::BindOnce(&ServiceWorkerStorage::ClearUserData,
                     weak_factory_.GetWeakPtr(), registration_id, keys,
                     std::move(callback)),
      RunSoon(FROM_HERE,
              base::BindOnce(std::move(callback), SERVICE_WORKER_ERROR_ABORT)));
...
void ServiceWorkerStorage::DeleteRegistrationFromDB(
  ...
  if (status != ServiceWorkerDatabase::STATUS_OK) {
    original_task_runner->PostTask(
        FROM_HERE,
        base::BindOnce(std::move(callback), OriginState::KEEP_ALL,
                       deleted_version, std::vector<int64_t>(), status));
    return;
  }
