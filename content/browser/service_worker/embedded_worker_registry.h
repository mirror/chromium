...
class CONTENT_EXPORT EmbeddedWorkerRegistry
 ...
 private:
  ...
  // DetachWorker removes the bookkeeping that binds the worker to the process.
  // This is called when instead of WorkerStopped() in cases when the worker
  // could not be cleanly stopped, e.g., because connection with the renderer
  // was lost.
