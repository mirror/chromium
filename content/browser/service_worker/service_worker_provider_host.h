...
class CONTENT_EXPORT ServiceWorkerProviderHost
 ...
 public:
  ...
  // Called by ServiceWorkerRegistrationObjectHost to add/remove itself into
  // |registration_object_hosts_|.
  void AddServiceWorkerRegistrationObjectHost(
      std::unique_ptr<ServiceWorkerRegistrationObjectHost> host);
  void RemoveServiceWorkerRegistrationObjectHost(
 ...
 private:
  ...
  // Contains all ServiceWorkerRegistrationObjectHost instances corresponding
  // with the service worker registration JavaScript objects for current
  // execution context (service worker global scope or service worker client) in
  // the renderer process. Keyed by service worker registration id.
  std::map<int64_t, std::unique_ptr<ServiceWorkerRegistrationObjectHost>>
      registration_object_hosts_;
