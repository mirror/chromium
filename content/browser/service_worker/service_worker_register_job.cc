...
void ServiceWorkerRegisterJob::OnInstallFinished(
    int request_id,
    std::unique_ptr<InstallEventMethodsReceiver> install_methods_receiver,
    blink::mojom::ServiceWorkerEventStatus2 event_status,
    bool has_fetch_handler,
    base::Time dispatch_event_time) {
  install_methods_receiver.reset();
  bool succeeded =
      event_status == blink::mojom::ServiceWorkerEventStatus2::COMPLETED;
  new_version()->FinishRequest(request_id, succeeded, dispatch_event_time);

  switch (event_status) {
    case blink::mojom::ServiceWorkerEventStatus2::COMPLETED:
      break;
    case blink::mojom::ServiceWorkerEventStatus2::REJECTED:
      OnInstallFailed(SERVICE_WORKER_ERROR_EVENT_WAITUNTIL_REJECTED);
      return;
    case blink::mojom::ServiceWorkerEventStatus2::ABORTED:
      OnInstallFailed(SERVICE_WORKER_ERROR_ABORT);
      return;
  }
