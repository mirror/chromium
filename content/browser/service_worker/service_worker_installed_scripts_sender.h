...
// |pending_scripts| to the renderer. |pending_scripts_| are initially set to
// all of installed scripts and the first member should be the main script. Once
// all of queued scripts are sent, the sender moves to the idle state. When
// RequestInstalledScript() is called over Mojo IPC, the requested script is
// queued to |pending_scripts_| and the sender goes to the sending state if
// it's idle.
class CONTENT_EXPORT ServiceWorkerInstalledScriptsSender
 ...
 public:
  ...
  // FinishedReason is updated when the sender goes to the idle state. Once the
  // worker has been successfully launched, the finished reason should be
 ...
 private:
  ...
  // Abort() stops all tasks even if pending scripts exist, and disconnects the
  // pipe to the renderer. Also, if the installed script stored in disk cache is
  // corrupted, the version is also deleted.
  ...
  // If it's in the idle state, |current_sending_url_| is set to the empty URL.
