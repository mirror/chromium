...
ServiceWorkerStatusCode EmbeddedWorkerInstance::SendStartWorker(
  ...
  if (!context_->GetDispatcherHost(process_id())) {
    ...
    // ServiceWorkerProcessManager knew it crashed, and it looks like the Mojo
    // connection doesn't re-detect that we're using a process that already
    // crashed. See https://crbug.com/732729.
    return SERVICE_WORKER_ERROR_IPC_FAILED;
  }
