...
class CONTENT_EXPORT ServiceWorkerVersion
 ...
 private:
  ...
  struct RequestTimeoutInfo {
    RequestTimeoutInfo(int id,
                       ServiceWorkerMetrics::EventType event_type,
                       const base::TimeTicks& expiration,
                       TimeoutBehavior timeout_behavior);
    ~RequestTimeoutInfo();
    // Compares |expiration|, or |id| if |expiration| is the same.
    bool operator<(const RequestTimeoutInfo& other) const;
  ...
  struct InflightRequest {
    ...
    std::set<RequestTimeoutInfo>::iterator timeout_iter;
  };
  ...
  bool MaybeTimeOutRequest(const RequestTimeoutInfo& info);
  ...
  std::set<RequestTimeoutInfo> request_timeouts_;
