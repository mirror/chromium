...
class Helper : public EmbeddedWorkerTestHelper {
 public:
  ...
  // Tells this helper to simulate early respond.
  // Sets the response mode to "early response", which simulates the promise
  // passed to respondWith() resolving before the waitUntil() promise resolves.
  // In this mode, the helper will respond to fetch events immediately, but will
  // not finish the fetch event until FinishWaitUntil() is called.
