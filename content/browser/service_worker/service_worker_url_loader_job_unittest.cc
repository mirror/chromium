...
TEST_F(ServiceWorkerURLLoaderJobTest, NonExistentBlobUUIDResponse) {
  ...
  EXPECT_EQ(JobResult::kHandledRequest, result);
  const ResourceResponseHead& info = client_.response_head();
  // TODO(falken): Currently our code returns 404 not found (with net::OK) in
  // such cases, but the spec seems to say this should act as if a network error
  // has occured. See https://crbug.com/732750
