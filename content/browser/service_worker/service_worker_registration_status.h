...
// TranslateServiceWorkerStatusCode() which gets out a std::string |message|
// rather than a base::string16. The legacy IPC used base::string16 as an old
// WebKit convention. This should only be called for errors, where status != OK.
...
void TranslateServiceWorkerStatusCode(
    ServiceWorkerStatusCode status,
    const std::string& status_message,
