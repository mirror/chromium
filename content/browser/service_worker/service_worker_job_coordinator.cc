...
void ServiceWorkerJobCoordinator::Register(
  ...
  ServiceWorkerRegisterJob* queued_job = static_cast<ServiceWorkerRegisterJob*>(
      PushOnJobQueue(options.scope, std::move(job)));
...
void ServiceWorkerJobCoordinator::Unregister(
  ...
  ServiceWorkerUnregisterJob* queued_job =
      static_cast<ServiceWorkerUnregisterJob*>(
          PushOnJobQueue(pattern, std::move(job)));
...
void ServiceWorkerJobCoordinator::Update(
  ...
  DCHECK(registration);
  auto job = base::WrapUnique<ServiceWorkerRegisterJobBase>(
      new ServiceWorkerRegisterJob(context_, registration, force_bypass_cache,
                                   false /* skip_script_comparison */));
  PushOnJobQueue(registration->pattern(), std::move(job));
}
...
ServiceWorkerRegisterJobBase* ServiceWorkerJobCoordinator::PushOnJobQueue(
    const GURL& pattern,
    std::unique_ptr<ServiceWorkerRegisterJobBase> job) {
...
void ServiceWorkerJobCoordinator::OnJobTimeoutTimer() {
  ...
  for (auto it = job_queues_.begin(); it != job_queues_.end();) {
    job = static_cast<ServiceWorkerRegisterJob*>(it->second.front());
    if (GetTickDuration(job->start_time()) >= kJobTimeout) {
      job->Abort();
      it->second.Pop(job);
      if (it->second.empty())
