 ...
 public:
  ...
  class Listener {
   public:
    ...
    // Called after the renderer is sent a Stop IPC message.
    virtual void OnStopping() {}

    // Called upon receiving an ACK from renderer that the worker context
    // terminated. OnStopped() is also called if Stop() aborted an ongoing
    // start attempt even before the Start IPC message was sent to the renderer.
    // In this case, OnStopping() is not called; the worker is "stopped"
    // immediately (the Start IPC is never sent).
  ...
  // running worker. Called when connection with the renderer died or the start
  // attempt was aborted before the connection was attempted. Essentially, it
  // throws away any information about the running worker in the
  // renderer, and frees this instance up to start a new worker.
  void Detach();
