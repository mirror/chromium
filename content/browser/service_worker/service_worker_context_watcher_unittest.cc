...
void DidRegisterServiceWorker(int64_t* registration_id_out,
  ...
  DCHECK(registration_id_out);
  EXPECT_EQ(SERVICE_WORKER_OK, status);
  *registration_id_out = registration_id;
}

void DidUnregisteredServiceWorker(ServiceWorkerStatusCode* status_out,
                                  ServiceWorkerStatusCode status) {
  DCHECK(status_out);
  *status_out = status;
}
...
class WatcherCallback {
  ...
  scoped_refptr<ServiceWorkerContextWatcher> StartWatch(
    ...
    scoped_refptr<ServiceWorkerContextWatcher> watcher =
        base::MakeRefCounted<ServiceWorkerContextWatcher>(
            context,
            base::Bind(&WatcherCallback::OnWorkerRegistrationUpdated,
                       weak_factory_.GetWeakPtr()),
            base::Bind(&WatcherCallback::OnWorkerVersionUpdated,
                       weak_factory_.GetWeakPtr()),
 ...
 private:
  ...
  std::map<int64_t, ServiceWorkerRegistrationInfo> registrations_;
  std::map<int64_t, std::map<int64_t, ServiceWorkerVersionInfo>> versions_;
  std::map<int64_t,
           std::map<int64_t,
                    std::vector<ServiceWorkerContextCoreObserver::ErrorInfo>>>
      errors_;
 ...
 protected:
  ...
  ServiceWorkerStatusCode UnregisterServiceWorker(const GURL& scope) {
    ...
    context()->UnregisterServiceWorker(
        scope, base::Bind(&DidUnregisteredServiceWorker, &status));
    base::RunLoop().RunUntilIdle();
    return status;
  }
...
TEST_F(ServiceWorkerContextWatcherTest, StoredServiceWorkers) {
  GURL scope_1 = GURL("https://www1.example.com/worker.js");
  ...
  GURL scope_2 = GURL("https://www2.example.com/worker.js");
