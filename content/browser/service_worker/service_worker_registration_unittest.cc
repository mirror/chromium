...
TEST_F(ServiceWorkerActivationTest, TimeSinceSkipWaiting_Installing) {
  ...
  // Reset version_2 to the installing phase.
  reg->UnsetVersion(version_2.get());
  version_2->SetStatus(ServiceWorkerVersion::INSTALLING);

  // Call skipWaiting(). The time ticks since skip waiting shouldn't start
  // since the version is not yet installed.
  SimulateSkipWaiting(version_2.get(), 77 /* dummy request_id */);
  base::RunLoop().RunUntilIdle();
  clock_2->Advance(base::TimeDelta::FromSeconds(11));
  EXPECT_EQ(base::TimeDelta(), version_2->TimeSinceSkipWaiting());

  // Install the version. Now the skip waiting time starts ticking.
  version_2->SetStatus(ServiceWorkerVersion::INSTALLED);
  reg->SetWaitingVersion(version_2);
  base::RunLoop().RunUntilIdle();
  clock_2->Advance(base::TimeDelta::FromSeconds(33));
  EXPECT_EQ(base::TimeDelta::FromSeconds(33),
            version_2->TimeSinceSkipWaiting());

  // Call skipWaiting again(). It doesn't reset the time.
  SimulateSkipWaiting(version_2.get(), 88 /* dummy request_id */);
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(base::TimeDelta::FromSeconds(33),
            version_2->TimeSinceSkipWaiting());
}
