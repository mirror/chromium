...
TEST_F(ServiceWorkerRegistrationHandleTest, SetVersionAttributes) {
  ...
  // Set an active worker.
  {
    ...
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(1,
              mock_registration_object->set_version_attributes_called_count_);
    ChangedVersionAttributesMask mask(mock_registration_object->changed_mask_);
...
TEST_F(ServiceWorkerRegistrationHandleTest, UpdateFound) {
  ...
  ServiceWorkerRegistration* registration =
      context()->GetLiveRegistration(registration_id);
  ASSERT_NE(nullptr, registration);
  ASSERT_EQ(0, mock_registration_object->update_found_called_count_);
  registration->NotifyUpdateFound();
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(1, mock_registration_object->update_found_called_count_);
}
