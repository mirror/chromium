...
TEST_F(ServiceWorkerRegistrationHandleTest, SetVersionAttributes) {
  ...
  EXPECT_EQ(version_1.get(), registration->active_version());
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(1, mock_registration_object->set_version_attributes_called_count_);
  EXPECT_FALSE(mock_registration_object->installing_);
  EXPECT_FALSE(mock_registration_object->waiting_);
  EXPECT_TRUE(mock_registration_object->active_);
  EXPECT_EQ(version_1_id, mock_registration_object->active_->version_id);
  EXPECT_EQ(kScriptUrl, mock_registration_object->active_->url);

  registration->SetInstallingVersion(version_2);
  EXPECT_EQ(version_2.get(), registration->installing_version());
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(2, mock_registration_object->set_version_attributes_called_count_);
  EXPECT_TRUE(mock_registration_object->installing_);
  EXPECT_FALSE(mock_registration_object->waiting_);
  EXPECT_FALSE(mock_registration_object->active_);
  EXPECT_EQ(version_2_id, mock_registration_object->installing_->version_id);
  EXPECT_EQ(kScriptUrl, mock_registration_object->installing_->url);

  registration->SetWaitingVersion(version_2);
  EXPECT_EQ(version_2.get(), registration->waiting_version());
  EXPECT_FALSE(registration->installing_version());
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(3, mock_registration_object->set_version_attributes_called_count_);
  EXPECT_TRUE(mock_registration_object->installing_);
  EXPECT_TRUE(mock_registration_object->waiting_);
  EXPECT_FALSE(mock_registration_object->active_);
  EXPECT_EQ(version_2_id, mock_registration_object->waiting_->version_id);
  EXPECT_EQ(kScriptUrl, mock_registration_object->waiting_->url);
  EXPECT_EQ(blink::mojom::kInvalidServiceWorkerVersionId,
            mock_registration_object->installing_->version_id);
  EXPECT_EQ(blink::mojom::kInvalidServiceWorkerHandleId,
            mock_registration_object->installing_->handle_id);

  registration->UnsetVersion(version_2.get());
  EXPECT_FALSE(registration->waiting_version());
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(4, mock_registration_object->set_version_attributes_called_count_);
  EXPECT_FALSE(mock_registration_object->installing_);
  EXPECT_TRUE(mock_registration_object->waiting_);
  EXPECT_FALSE(mock_registration_object->active_);
  EXPECT_EQ(blink::mojom::kInvalidServiceWorkerVersionId,
            mock_registration_object->waiting_->version_id);
  EXPECT_EQ(blink::mojom::kInvalidServiceWorkerHandleId,
            mock_registration_object->waiting_->handle_id);
}
