...
class DelayHelper : public EmbeddedWorkerTestHelper {
 public:
  enum Mode {
    DELAY_WORKER_START_ONLY,
    DELAY_RESPONSE_ONLY,
    DELAY_WORKER_START_AND_RESPONSE
  };
  DelayHelper(ServiceWorkerURLRequestJobTest* test, Mode mode)
      : EmbeddedWorkerTestHelper(base::FilePath()), test_(test), mode_(mode) {}
 ...
 protected:
  ...
  void OnFetchEvent(
    ...
    if (mode_ == DELAY_WORKER_START_ONLY) {
      Respond();
    }
  }
...
TEST_F(ServiceWorkerURLRequestJobTest,
  ...
  SetUpWithHelper(base::MakeUnique<DelayHelper>(
      this, DelayHelper::Mode::DELAY_RESPONSE_ONLY));
...
TEST_F(ServiceWorkerURLRequestJobTest,
  ...
  SetUpWithHelper(base::MakeUnique<DelayHelper>(
      this, DelayHelper::Mode::DELAY_WORKER_START_AND_RESPONSE));
  DelayHelper* helper = static_cast<DelayHelper*>(helper_.get());
