...
// ServiceWorkerURLRequestJobTest is for testing the handling
// of URL requests by a service worker.
//
// To use it, call SetUpWithHelper() in your test. This sets up
// the service worker and the scaffolding make the worker
// handle https URLRequests:
//
...
TEST_F(ServiceWorkerURLRequestJobTest, StreamResponse_ConsecutiveRead) {
  ...
  stream_callback->OnCompleted();
  base::RunLoop().RunUntilIdle();
  // TODO(falken): Why is this IO_PENDING and not OK?
