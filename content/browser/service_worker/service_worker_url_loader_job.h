...
class CONTENT_EXPORT ServiceWorkerURLLoaderJob
 ...
 public:
  ...
  //    SingleRequestLoader, which will be then handled by
  ...
  //    SingleRequestLoader if it turns out that we need to fallback to the
  //    network.
  // 4. Otherwise if the SW returned a stream or blob as a response
  //    this job calls |loader_callback| with a SingleRequestLoader bound from
  //    StartResponse().
 ...
 private:
  ...
  // Used as the SingleRequestLoader passed to |loader_callback_| when the
  // service worker chooses to handle a resource request. Returns the response
  // to |client|. |body_as_blob| is kept around until BlobDataHandle is created
  ...
  // Used as the SingleRequestLoader passed to |loader_callback_| on error.
