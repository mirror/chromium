...
ServiceWorkerScriptURLLoader::ServiceWorkerScriptURLLoader(
    ...
    : url_(resource_request.url),
      ...
      done_closure_(base::BarrierClosure(
          2,
          base::BindOnce(
...
void ServiceWorkerScriptURLLoader::OnStartLoadingResponseBody(
  ...
  // Pass the consumer handle to the renderer.
  forwarding_client_->OnStartLoadingResponseBody(
      std::move(forwarding_consumer));

  // Monitor the data reception via the network.
  consumer_ = std::move(consumer);

  MayStartHandleWatcher();
}

void ServiceWorkerScriptURLLoader::MayStartHandleWatcher() {
  if (!consumer_.is_valid()) {
    // Wait until OnStartLoadingResponsesBody is notified.
    return;
  }
  if (!are_headers_written_) {
...
void ServiceWorkerScriptURLLoader::OnDataAvailable(MojoResult) {
  ...
  switch (result) {
    ...
    case MOJO_RESULT_FAILED_PRECONDITION:
      // Closed by peer. This indicates we read all the data from the network
      // service or there is an error. Check it in the closure.
      done_closure_.Run();
      return;
  ...
  const size_t kMaxReadSize = 10;
  size_t bytes_to_be_read = std::min<size_t>(kMaxReadSize, available);
...
void ServiceWorkerScriptURLLoader::Finalize(
  ...
  if (status.error_code == net::OK) {
    written_size = cache_writer_->bytes_written();
  ...
  version_->script_cache_map()->NotifyFinishedCaching(
      url_, written_size, error_code, status_message);
