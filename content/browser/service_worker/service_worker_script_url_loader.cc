...
ResourceRequestCompletionStatus NetErrorToCompletionStatus(net::Error error) {
...
ServiceWorkerScriptURLLoader::ServiceWorkerScriptURLLoader(
  ...
  ServiceWorkerVersion* loading_version =
  ...
  if (resource_request.resource_type == RESOURCE_TYPE_SERVICE_WORKER) {
    ServiceWorkerVersion* stored_version =
        provider_host_->waiting_version() ? provider_host_->waiting_version()
    ...
    if (stored_version && stored_version->script_url() == url_ &&
        loading_version->pause_after_download()) {
  ...
  loading_version->script_cache_map()->NotifyStartedCaching(url_, resource_id);
...
void ServiceWorkerScriptURLLoader::OnReceiveResponse(
  ...
  if (!provider_host_) {
    OnComplete(NetErrorToCompletionStatus(net::ERR_FAILED));
    return;
  }
...
void ServiceWorkerScriptURLLoader::OnComplete(
  ...
  if (status.error_code != net::OK) {
    ...
    loading_version->embedded_worker()->AddMessageToConsole(
  ...
  if (status.error_code == net::OK && !cache_writer_->did_replace()) {
    loading_version->SetStartWorkerStatusCode(SERVICE_WORKER_ERROR_EXISTS);
    loading_version->script_cache_map()->NotifyFinishedCaching(
...
void ServiceWorkerScriptURLLoader::OnWriteHeadersComplete(
  ...
  if (error != net::OK) {
    // TODO(nhiroki): Record the result.
    OnComplete(NetErrorToCompletionStatus(error));
    return;
  }
...
void ServiceWorkerScriptURLLoader::OnDataAvailable(MojoResult) {
  ...
  switch (result) {
    ...
    case MOJO_RESULT_FAILED_PRECONDITION:
      ...
      forwarding_client_->OnComplete(
          NetErrorToCompletionStatus(net::ERR_FAILED));
      return;
  ...
  // Finish.
  OnComplete(NetErrorToCompletionStatus(net::OK));
}
