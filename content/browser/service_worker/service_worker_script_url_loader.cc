...
ServiceWorkerScriptURLLoader::ServiceWorkerScriptURLLoader(
  ...
  if (resource_request.resource_type == RESOURCE_TYPE_SERVICE_WORKER) {
    ...
    // byte-for-byte check.
    if (stored_version &&
        stored_version->script_url() == resource_request.url &&
        loading_version->pause_after_download()) {
      incumbent_resource_id =
          stored_version->script_cache_map()->LookupResourceId(
  ...
  loading_version->script_cache_map()->NotifyStartedCaching(
      resource_request.url, resource_id);
...
void ServiceWorkerScriptURLLoader::OnStartLoadingResponseBody(
  ...
  if (mojo::CreateDataPipe(nullptr, &forwarding_producer_, &forwarding_consumer) != MOJO_RESULT_OK) {
  ...
  watcher_.Watch(consumer_.get(),
                 ...
                 base::Bind(&ServiceWorkerScriptURLLoader::OnDataAvilable,
                            weak_factory_.GetWeakPtr()));
  watcher_.ArmOrNotify();

  forwarding_client_->OnStartLoadingResponseBody(std::move(forwarding_consumer));
}
...
void ServiceWorkerScriptURLLoader::OnDataAvailable(MojoResult) {
  ...
  MojoResult rv =
      consumer_->BeginReadData(&buffer, &bytes_read, MOJO_RAD_DATA_FLAG_NONE);
  switch(rv) {
