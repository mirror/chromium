...
ServiceWorkerScriptURLLoader::ServiceWorkerScriptURLLoader(
    ...
    : request_url_(resource_request.url),
      ...
      version_(version),
      cache_resource_id_(version->context()->storage()->NewResourceId()),
  ...
  // TODO(nhiroki): Handle the case where |cache_resource_id_| is invalid.
  ...
  cache_writer_ = std::make_unique<ServiceWorkerCacheWriter>(
      ...
      storage->CreateResponseWriter(cache_resource_id_));
...
void ServiceWorkerScriptURLLoader::AdvanceState(State new_state) {
  switch (state_) {
    ...
    case State::kWroteHeaders:
      ...
      break;
    case State::kWroteData:
      DCHECK(new_state == State::kCompleted);
      break;
    case State::kCompleted:
...
void ServiceWorkerScriptURLLoader::WriteData(
  ...
  // Notifies the completion of data transfer to the service worker in the
  // renderer process.
