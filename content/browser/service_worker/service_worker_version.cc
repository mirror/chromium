...
void OnOpenWindowFinished(
  ...
  DCHECK(client_info);
  std::move(callback).Run(base::nullopt, std::move(client_info));
}
...
using PaymentHandlerOpenWindowCallback = base::OnceCallback<
    void(ServiceWorkerVersion::OpenWindowResponseCallback, bool, int, int)>;
...
void ServiceWorkerVersion::OpenNewTab(const GURL& url,
  ...
  OnOpenWindow(url, WindowOpenDisposition::NEW_FOREGROUND_TAB,
               std::move(callback));
}
...
void ServiceWorkerVersion::OnOpenWindow(GURL url,
                                        WindowOpenDisposition disposition,
                                        OpenWindowResponseCallback callback) {
  // Just respond failure if we are shutting down.
  if (!context_) {
    std::move(callback).Run(
        std::string("The service worker system is shutting down."), nullptr);
    return;
  }
  ...
  if (!ChildProcessSecurityPolicyImpl::GetInstance()->CanRequestURL(
    ...
    std::move(callback).Run(url.spec() + " cannot be opened.", nullptr);
    return;
  }
