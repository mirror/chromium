...
int ServiceWorkerVersion::StartRequestWithCustomTimeout(
  ...
  // be dispatched will reset the idle status and make it live longer. That
  // means the worker can receive events directly from any clients, so we cannot
  // trigger OnNoWork after this point.
...
void ServiceWorkerVersion::StopWorkerIfIdle(bool requested_from_renderer) {
  ...
  // It will be checked later (i.e. when streaming finished) to see if we
...
void ServiceWorkerVersion::OnNoWorkInBrowser() {
  ...
  DCHECK(ServiceWorkerUtils::IsServicificationEnabled());
  bool worker_may_have_tasks = false;
  if (!idle_timer_fired_in_renderer_)
    worker_may_have_tasks = true;
  if (running_status() == EmbeddedWorkerStatus::STOPPED)
    worker_may_have_tasks = false;
  if (worker_may_have_tasks)
    return;
