...
void ServiceWorkerVersion::NeedsToRestart() {
  retuirn true;
}
...
void ServiceWorkerVersion::OnStoppedInternal(EmbeddedWorkerStatus old_status) {
  ...
  // Non-empty |start_callbacks_| indicates someone requested to start this
  // worker during the stop sequence.
  ...
  if (is_redundant() || in_dtor_) {
    ...
    should_restart = false;
  } else if (old_status == EmbeddedWorkerInstance::STARTING) {
    // This worker stopped because of start failure (e.g., process allocation
    // failure) and restart is likely to fail again.
    should_restart = false;
  }
