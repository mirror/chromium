...
class CacheExtendableMessageEventTestHelper : public EmbeddedWorkerTestHelper {
 public:
  CacheExtendableMessageEventTestHelper()
...
TEST_F(ServiceWorkerDispatcherHostTest, DispatchExtendableMessageEvent) {
  ...
  Initialize(std::make_unique<CacheExtendableMessageEventTestHelper>());
  SetUpRegistration(pattern, script_url);
  ...
  EXPECT_TRUE(called);
  EXPECT_EQ(SERVICE_WORKER_OK, status);
  // A service worker object info of |version_| has been passed via the above
