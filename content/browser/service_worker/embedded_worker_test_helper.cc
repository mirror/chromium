...
EmbeddedWorkerTestHelper::EmbeddedWorkerTestHelper(
...

  {
    auto dispatcher_host =
        base::MakeRefCounted<MockServiceWorkerDispatcherHost>(
            mock_render_process_id(), browser_context_->GetResourceContext(),
            this);
...

  {
    auto dispatcher_host =
        base::MakeRefCounted<MockServiceWorkerDispatcherHost>(
            new_render_process_id(), browser_context_->GetResourceContext(),
            this);
    dispatcher_host->Init(wrapper_.get());
    dispatcher_hosts_[new_render_process_id()] = std::move(dispatcher_host);
  }
