...
void ServiceWorkerProviderHost::SetControllerVersionAttribute(
  ...
  controlling_version_notified_to_clients_ = false;
...
ServiceWorkerProviderHost::PrepareForCrossSiteTransfer() {
  ...
  if (IsProviderForClient() && associated_registration_.get()) {
...
void ServiceWorkerProviderHost::SendSetControllerServiceWorker(
  ...
  if (version) {
    ...
    DCHECK(!controlling_version_notified_to_clients_);
    controlling_version_notified_to_clients_ = true;
  }
