...
void ServiceWorkerProviderHost::CompleteNavigationInitialized(
  ...
  // sent in navigation commit, but we still need this for S13nServiceWorker
  // case for setting the use counter correctly.
...
void ServiceWorkerProviderHost::SendSetControllerServiceWorker(
  ...
  // S13nServiceWorker/NavigationMojoResponse: Send the controller ptr too so
  // that controller changes/ lost are propagated with the new controller ptr.
