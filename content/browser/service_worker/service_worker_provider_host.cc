...
ServiceWorkerProviderHost::CreateRegistrationObjectInfo(
    ServiceWorkerRegistration* registration) {
  DCHECK(dispatcher_host_);
  ServiceWorkerRegistrationHandle* existing_handle =
      dispatcher_host_->FindRegistrationHandle(provider_id(),
                                               registration->id());
  if (existing_handle) {
...
ServiceWorkerProviderHost::CompleteStartWorkerPreparation(
  ...
  provider_info->registration = CreateRegistrationObjectInfo(registration);
...
void ServiceWorkerProviderHost::ReturnRegistrationForReadyIfNeeded() {
  ...
  std::move(*get_ready_callback_)
      .Run(CreateRegistrationObjectInfo(registration));
}
...
void ServiceWorkerProviderHost::RegistrationComplete(
  ...
  std::move(callback).Run(blink::mojom::ServiceWorkerErrorType::kNone,
                          base::nullopt,
                          CreateRegistrationObjectInfo(registration));
}
...
void ServiceWorkerProviderHost::GetRegistrationComplete(
  ...
  if (status == SERVICE_WORKER_OK) {
    ...
    if (!registration->is_uninstalling()) {
      info = CreateRegistrationObjectInfo(registration.get());
    }
  }
...
void ServiceWorkerProviderHost::GetRegistrationsComplete(
  ...
  for (const auto& registration : registrations) {
    DCHECK(registration.get());
    if (!registration->is_uninstalling()) {
      object_infos.push_back(CreateRegistrationObjectInfo(registration.get()));
    }
  }
