...
void ServiceWorkerInstalledScriptsSender::OnFinishSendingScript(
  ...
  current_sending_url_ = GURL::EmptyGURL();

  if (IsSendingMainScript())
    has_main_script_sent_ = true;
...
bool ServiceWorkerInstalledScriptsSender::IsSendingMainScript() const {
  ...
  // |has_main_script_sent_| is false if calling importScripts for the main
  // script.
  return !has_main_script_sent_ && current_sending_url_ == main_script_url_;
}
