 ...
 private:
  void OnReadInfoComplete(scoped_refptr<HttpResponseInfoIOBuffer> http_info,
    ...
    if (!http_info->http_info) {
      CompleteTransferIfNeeded(Status::kNoHttpInfoError);
      return;
    }
    ...
    if (mojo::CreateDataPipe(nullptr, &body_handle_, &body_consumer) !=
        MOJO_RESULT_OK) {
      CompleteTransferIfNeeded(Status::kCreateDataPipeError);
      return;
    }
  ...
  void OnWritableBody(MojoResult result) {
    ...
    switch (rv) {
      ...
      case MOJO_RESULT_BUSY:
        ...
        return;
      case MOJO_RESULT_FAILED_PRECONDITION:
        CompleteTransferIfNeeded(Status::kConnectionError);
        return;
      case MOJO_RESULT_SHOULD_WAIT:
        watcher_.ArmOrNotify();
        return;
  ...
  void OnResponseDataRead(int written_bytes) {
    if (written_bytes < 0) {
      CompleteTransferIfNeeded(Status::kResponseReaderError);
      return;
    }
    body_handle_ = pending_write_->Complete(written_bytes);
    ...
    if (written_bytes == 0) {
      ...
      watcher_.Cancel();
      body_handle_.reset();
      CompleteTransferIfNeeded(Status::kSuccess);
      return;
    }
    watcher_.ArmOrNotify();
  }
  ...
  void OnMetaDataTransferred(MetaDataSender::Status status) {
    ...
    if (status == MetaDataSender::Status::kFailed) {
      watcher_.Cancel();
      body_handle_.reset();
      CompleteTransferIfNeeded(Status::kMetaDataTransferError);
      return;
    }
  ...
  // CompleteTransferIfNeeded notifies the end of data transfer to |owner_|, and
  ...
  void CompleteTransferIfNeeded(Status status) {
    if (status != Status::kSuccess) {
      owner_->OnAbortScriptTransfer(status);
      return;
    }
...
void ServiceWorkerInstalledScriptsSender::Start() {
  ...
  state_ = State::kStreamingMainScript;
...
void ServiceWorkerInstalledScriptsSender::StartPushScript(int64_t resource_id) {
  ...
  DCHECK(state_ == State::kStreamingMainScript ||
         state_ == State::kStreamingImportedScript);
  ...
  TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("ServiceWorker", "ScriptTransfer", this,
                                    "script_url", CurrentStreamingURL().spec());
...
void ServiceWorkerInstalledScriptsSender::StartScriptTransfer(
  ...
  script_info->script_url = CurrentStreamingURL();
...
void ServiceWorkerInstalledScriptsSender::OnHttpInfoRead(
  ...
  DCHECK(state_ == State::kStreamingMainScript ||
         state_ == State::kStreamingImportedScript);
  if (state_ == State::kStreamingMainScript)
...
void ServiceWorkerInstalledScriptsSender::OnFinishScriptTransfer() {
  ...
  if (state_ == State::kStreamingMainScript) {
    ...
    current_streaming_script_ = imported_urls_.begin();
    state_ = State::kStreamingImportedScript;
  } else {
    current_streaming_script_++;
  }
  if (current_streaming_script_ == imported_urls_.end()) {
    DCHECK_EQ(State::kStreamingImportedScript, state_);
    state_ = State::kFinished;
  ...
  StartPushScript(current_streaming_script_->first);
}

void ServiceWorkerInstalledScriptsSender::OnAbortScriptTransfer(Status status) {
  DCHECK(running_sender_);
  DCHECK(state_ == State::kStreamingMainScript ||
         state_ == State::kStreamingImportedScript);
...
const GURL& ServiceWorkerInstalledScriptsSender::CurrentStreamingURL() {
  if (state_ == State::kStreamingMainScript)
    return main_script_url_;
  return current_streaming_script_->second;
}
