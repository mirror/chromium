...
void ServiceWorkerDispatcherHost::RegisterServiceWorkerRegistrationObjectHost(
  ...
  int handle_id = registration_object_host->handle_id();
  registration_object_hosts_.AddWithID(
      base::WrapUnique(registration_object_host), handle_id);
}
...
ServiceWorkerDispatcherHost::FindServiceWorkerRegistrationObjectHost(
  ...
  for (base::IDMap<std::unique_ptr<ServiceWorkerRegistrationObjectHost>>::
    ...
    if (registration_object_host->provider_id() == provider_id &&
        registration_object_host->registration()->id() == registration_id) {
      return registration_object_host;
    }
  }
  return nullptr;
}
