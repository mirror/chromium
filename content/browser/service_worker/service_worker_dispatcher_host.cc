...
void ServiceWorkerDispatcherHost::RemoveServiceWorkerRegistrationHandle(
    int handle_id) {
...
ServiceWorkerDispatcherHost::GetOrCreateRegistrationHandle(
  ...
  auto new_handle = base::MakeUnique<ServiceWorkerRegistrationHandle>(
      GetContext()->AsWeakPtr(), AsWeakPtr(), provider_host, registration);
  ServiceWorkerRegistrationHandle* new_handle_ptr = new_handle.get();
  RegisterServiceWorkerRegistrationHandle(std::move(new_handle));
  return new_handle_ptr->CreateObjectInfo();
}
...
void ServiceWorkerDispatcherHost::GetRegistrationObjectInfoAndVersionAttributes(
  ...
  *out_info = GetOrCreateRegistrationHandle(provider_host, registration);
