...
void ServiceWorkerRegistrationHandle::OnConnectionError() {
  ...
  if (!bindings_.empty())
    return;
  // Because |dispatcher_host_| instance owns |this|, if |dispatcher_host_| has
  // already destroyed, |this| should also have destroyed, then the connection
  // error handler of |bindings_| should never be triggered after that. So,
  // coming here means |dispatcher_host_| is still alive.
  DCHECK(dispatcher_host_.get());
  // Will destroy |this|.
