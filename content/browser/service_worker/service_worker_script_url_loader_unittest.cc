...
class MockHTTPServer {
 public:
  ...
  std::string GetHeaders(const GURL& url) {
    auto found = responses_.find(url);
    DCHECK(found != responses_.end());
    if (found == responses_.end())
      return std::string();
    return found->second.headers;
  }

  std::string GetBody(const GURL& url) {
    auto found = responses_.find(url);
    DCHECK(found != responses_.end());
    if (found == responses_.end())
      return std::string();
...
class MockNetworkURLLoaderFactory final : public mojom::URLLoaderFactory {
 public:
  ...
  void CreateLoaderAndStart(mojom::URLLoaderRequest request,
    ...
    std::string headers = mock_server_->GetHeaders(url_request.url);
    ...
    std::string body = mock_server_->GetBody(url_request.url);
...
class ServiceWorkerScriptURLLoaderTest : public testing::Test {
 public:
  ...
  // Set up ServiceWorkerRegistration and ServiceWorkerVersion that is now
  // starting up. This should be called before DoRequest().
  ...
  void DoRequest(const GURL& request_url) {
    ...
    request.method = "GET";
    if (version_->script_url() == request_url)
  ...
  // Returns false if the entry doesn't exist for |url| in the storage.
...
TEST_F(ServiceWorkerScriptURLLoaderTest, Error404) {
