...
TEST_F(ServiceWorkerScriptURLLoaderTest, Success_LargeBody) {
  // The size of the response data should be larger than the size of the
  // ServiceWorkerScriptURLLoader's read buffer size to test chunked data writes
  // into the mojo's data pipe and the service worker script storage.
  const uint32_t kReadBufferSize = 512;
  loader_->SetReadBufferSizeForTest(kReadBufferSize);
  const uint32_t kDataSize = kReadBufferSize * 4;
