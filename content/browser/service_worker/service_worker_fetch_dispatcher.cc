...
const net::NetworkTrafficAnnotationTag kNavigationPreloadTrafficAnnotation =
    ...
    comments:
      "Chrome would be unable to use service workers if this feature is "
      "disabled, which could result in a degraded experience for websites that "
      "register a service worker. Also as a side effect of disabling service "
      "workers, cookies will also be disabled. Using either URLBlacklist or "
      "URLWhitelist policies (or a combination of both) limits the scope of "
      "these requests."
)");
