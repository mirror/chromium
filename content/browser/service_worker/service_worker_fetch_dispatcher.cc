...
// ResponseCallback is owned by ServiceWorkerVersion via pending_requests_.
class ServiceWorkerFetchDispatcher::ResponseCallback
 ...
 public:
  ...
  ~ResponseCallback() override {
    DCHECK_NE(kInvalidFetchEventId, fetch_event_id_);
  }

  void set_fetch_event_id(int id) {
    DCHECK_EQ(kInvalidFetchEventId, fetch_event_id_);
    fetch_event_id_ = id;
  }
 ...
 private:
  ...
  static void HandleResponse(
    ...
    DCHECK_NE(kInvalidFetchEventId, fetch_event_id);
    ...
    if (!fetch_dispatcher)
      return;
    fetch_dispatcher->DidFinish(fetch_event_id, fetch_result, response,
                                std::move(body_as_stream),
                                std::move(body_as_blob));
  }

  const static int kInvalidFetchEventId = -1;
  ...
  int fetch_event_id_ = kInvalidFetchEventId;
...
void ServiceWorkerFetchDispatcher::DispatchFetchEvent() {
  ...
  ResponseCallback* response_callback_ptr = response_callback.get();
  ...
  response_callback_ptr->set_fetch_event_id(fetch_event_id);
