...
class ServiceWorkerFetchDispatcher::ResponseCallback
 ...
 public:
  ...
  void OnResponseStream(
      ...
      base::Time dispatch_event_time) override {
    HandleResponse(
        response, std::move(body_as_stream), storage::mojom::BlobPtr(),
  ...
  void OnFallback(base::Time dispatch_event_time) override {
    HandleResponse(
        ...
        storage::mojom::BlobPtr(), SERVICE_WORKER_FETCH_EVENT_RESULT_FALLBACK,
        dispatch_event_time);
  }
 ...
 private:
  ...
  // Owns |this| via mojo pipes (the version owns EventDispatcher in the
  // renderer side, which owns this callback via the mojo pipe).
