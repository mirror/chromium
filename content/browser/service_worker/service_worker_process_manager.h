...
class CONTENT_EXPORT ServiceWorkerProcessManager {
 public:
  ...
  struct AllocatedProcessInfo {
    ...
    // |situation| must be one of the following values:
    // - UNKNOWN: Failed to allocate a process.
    // - NEW_PROCESS: A new process is created for the service worker.
    // - EXISTING_UNREADY_PROCESS: An existing process is reused, but it is not
    //   ready. (Ex: The process was created for the navigation by PlzNavigate
    //   but the IPC connection is not established yet.)
    // - EXISTING_READY_PROCESS: An existing ready process is reused.
