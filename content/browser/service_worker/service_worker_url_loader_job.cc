...
void ServiceWorkerURLLoaderJob::StartResponse(
  ...
  // We have a non-redirect response. Sent the headers to the client.
  // TODO(falken): Consider adding the Content-Length header for blob bodies.
  ...
  if (body_as_blob) {
    ...
    uint64_t offset = 0;
    uint64_t length = 0;
    if (!ServiceWorkerUtils::ExtractSinglePartHttpRange(
            resource_request_.headers, &byte_range_set, &offset, &length)) {
      // We don't support multiple range requests in one single URL request,
      // because we need to do multipart encoding here.
      // TODO(falken): Support multipart byte range requests.
      CommitCompleted(net::ERR_REQUEST_RANGE_NOT_SATISFIABLE);
      return;
    }

    if (byte_range_set) {
      body_as_blob_->ReadRange(offset, length,
                               std::move(data_pipe.producer_handle),
                               std::move(blob_reader_client));
    } else {
      body_as_blob_->ReadAll(std::move(data_pipe.producer_handle),
                             std::move(blob_reader_client));
    }
    url_loader_client_->OnStartLoadingResponseBody(
