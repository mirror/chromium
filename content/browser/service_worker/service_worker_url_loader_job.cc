...
std::unique_ptr<net::RedirectInfo> ComputeRedirectInfo(
  ...
  const Referrer referrer(original_request.referrer,
                          original_request.referrer_policy);
  std::string referrer_string;
  if (referrer.url.is_valid() &&
      !base::CommandLine::ForCurrentProcess()->HasSwitch(
          switches::kNoReferrers)) {
    referrer_string = referrer.url.spec();
  }
  ...
  return net::RedirectInfo::ComputeRedirectInfo(
      response_head.headers->response_code(), original_request.method,
      original_request.url, original_request.url.Resolve(new_location),
      true /* copy_fragment */, original_request.site_for_cookies,
      first_party_url_policy, Referrer::ReferrerPolicyForUrlRequest(referrer),
      referrer_string, token_binding_negotiated, response_head.headers.get());
}
...
void ServiceWorkerURLLoaderJob::StartResponse(
  ...
  if (std::unique_ptr<net::RedirectInfo> redirect_info = ComputeRedirectInfo(
