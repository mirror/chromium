...
void ServiceWorkerMetrics::RecordEmbeddedWorkerStartTiming(
    ...
    StartSituation situation) {
  LOG(ERROR) << "start_worker_sent_time: " << start_worker_sent_time;
  LOG(ERROR) << "blink_initialized_time: "
             << start_timing->blink_initialized_time;
  LOG(ERROR) << "start_worker_received_time: "
             << start_timing->start_worker_received_time;

  base::TimeDelta start_worker_message_latency =
  ...
  LOG(ERROR) << "latency: " << start_worker_message_latency;
  if (start_worker_sent_time < start_timing->blink_initialized_time) {
    ...
    LOG(ERROR) << "EmbeddedWorkerInstance.Start.WaitedForRendererSetup.Time: "
               << (start_timing->blink_initialized_time -
