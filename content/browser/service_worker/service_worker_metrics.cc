...
void ServiceWorkerMetrics::RecordNavigationPreloadResponse(
  ...
  UMA_HISTOGRAM_MEDIUM_TIMES("ServiceWorker.NavPreload.ResponseTime",
                             response_start);
  UMA_HISTOGRAM_BOOLEAN("ServiceWorker.NavPreload.FinishedFirst",
                        nav_preload_finished_first);
  UMA_HISTOGRAM_MEDIUM_TIMES("ServiceWorker.NavPreload.ConcurrentTime",
                             concurrent_time);
  if (nav_preload_finished_first) {
    UMA_HISTOGRAM_MEDIUM_TIMES("ServiceWorker.NavPreload.WorkerWaitTime",
                               worker_wait_time);
  }

  if (is_main_frame) {
    UMA_HISTOGRAM_MEDIUM_TIMES(
        "ServiceWorker.NavPreload.ResponseTime_MainFrame", response_start);
    ...
    if (nav_preload_finished_first) {
      UMA_HISTOGRAM_MEDIUM_TIMES(
          "ServiceWorker.NavPreload.WorkerWaitTime_MainFrame",
          worker_wait_time);
    }
  }
  ...
  if (existing_process_startup) {
    UMA_HISTOGRAM_BOOLEAN(
        "ServiceWorker.NavPreload.FinishedFirst_"
        "StartWorkerExistingProcess",
        nav_preload_finished_first);
    UMA_HISTOGRAM_MEDIUM_TIMES(
        "ServiceWorker.NavPreload.ConcurrentTime_StartWorkerExistingProcess",
        concurrent_time);
    if (nav_preload_finished_first) {
      UMA_HISTOGRAM_MEDIUM_TIMES(
          "ServiceWorker.NavPreload.WorkerWaitTime_"
          "StartWorkerExistingProcess",
          worker_wait_time);
    }
  }

  if (is_main_frame && existing_process_startup) {
    UMA_HISTOGRAM_BOOLEAN(
