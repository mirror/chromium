...
void RecordURLMetricOnUI(const std::string& histogram_name, const GURL& url) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  LOG(ERROR) << "recording: " << histogram_name << " for " << url;
  GetContentClient()->browser()->RecordURLMetric(histogram_name, url);
}
