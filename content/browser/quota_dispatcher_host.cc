...
void QuotaDispatcherHost::CreateForWorker(
  ...
  // TODO(sashab): Save the |origin| here and use it rather than the one
  // provided by QuotaDispatcher. crbug.com/595685

  // Bind on the IO thread.
  BrowserThread::PostTask(
      BrowserThread::IO, FROM_HERE,
      base::BindOnce(&BindConnectorOnIOThread, host, -1, std::move(request)));
}
...
void QuotaDispatcherHost::CreateForFrame(
  ...
  BrowserThread::PostTask(BrowserThread::IO, FROM_HERE,
                          base::BindOnce(&BindConnectorOnIOThread, host,
