...
class QuotaDispatcherHost::QueryUsageAndQuotaDispatcher
 ...
 private:
  void DidQueryStorageUsageAndQuota(QuotaStatusCode status,
    ...
    std::move(callback_).Run(request_id(), status, usage, quota);
    Completed();
  }
...
class QuotaDispatcherHost::RequestQuotaDispatcher
 ...
 public:
  ...
  RequestQuotaDispatcher(
      ...
      mojom::QuotaMessages::RequestStorageQuotaCallback callback)
      : RequestDispatcher(dispatcher_host, request_id),
 ...
 private:
  ...
  void DidFinish(QuotaStatusCode status, int64_t usage, int64_t granted_quota) {
    if (!dispatcher_host())
      return;
    DCHECK(dispatcher_host());
    std::move(callback_).Run(request_id(), status, usage, granted_quota);
    Completed();
  }

  int64_t render_frame_id_;
  int64_t request_id_;
  const GURL& origin_url_;
  storage::StorageType storage_type_;
  uint64_t requested_size_;
  mojom::QuotaMessages::RequestStorageQuotaCallback callback_;
...
// static
void QuotaDispatcherHost::Create(int process_id,
                                 QuotaManager* quota_manager,
                                 QuotaPermissionContext* permission_context,
                                 mojom::QuotaMessagesRequest request) {
  mojo::MakeStrongBinding(base::MakeUnique<QuotaDispatcherHost>(
                              process_id, quota_manager, permission_context),
                          std::move(request));
}
...
void QuotaDispatcherHost::RequestStorageQuota(
  ...
  if (storage_type != storage::kStorageTypeTemporary &&
    ...
    std::move(callback).Run(request_id, storage::kQuotaErrorNotSupported, 0, 0);
    return;
  }

  RequestQuotaDispatcher* dispatcher = new RequestQuotaDispatcher(
      weak_factory_.GetWeakPtr(), render_frame_id, request_id, origin_url,
      storage_type, requested_size, std::move(callback));
  dispatcher->Start();
}
