...
typedef std::map<int32_t, const char*> PlatformConstantToNameMap;
}
...
CONTENT_EXPORT base::string16 IAccessibleRoleToString(int32_t ia_role) {
  ...
  // IAccessible2 role.
  static PlatformConstantToNameEntry ia_table[] = {
  ...
  CR_DEFINE_STATIC_LOCAL(PlatformConstantToNameMap, ia_map, ());
  if (!ia_map.size()) {
    for (auto& entry : ia_table)
      ia_map[entry.value] = entry.name;
  }
  auto iter = ia_map.find(ia_role);
  if (iter != ia_map.end())
    return base::ASCIIToUTF16(iter->second);
  return L"";
}
...
CONTENT_EXPORT base::string16 IAccessible2RoleToString(int32_t ia2_role) {
  ...
  static PlatformConstantToNameEntry ia2_table[] = {
  ...
  CR_DEFINE_STATIC_LOCAL(PlatformConstantToNameMap, ia2_map, ());
  if (!ia2_map.size()) {
    for (auto& entry : ia2_table)
      ia2_map[entry.value] = entry.name;
  }
  auto iter = ia2_map.find(ia2_role);
  if (iter != ia2_map.end())
    return base::ASCIIToUTF16(iter->second);
  return L"";
}
...
CONTENT_EXPORT base::string16 AccessibilityEventToString(int32_t event) {
  static PlatformConstantToNameEntry event_table[] = {
      ...
      QUOTE(IA2_EVENT_ACTIVE_DECENDENT_CHANGED),
  ...
  CR_DEFINE_STATIC_LOCAL(PlatformConstantToNameMap, event_map, ());
  if (!event_map.size()) {
    for (auto& entry : event_table)
      event_map[entry.value] = entry.name;
  }
  auto iter = event_map.find(event);
...
#define QUOTE_STATE(X) \
  ...
  static PlatformConstantToNameEntry ia_table[] = {
      QUOTE_STATE(ALERT_HIGH),
      QUOTE_STATE(ALERT_LOW),
      QUOTE_STATE(ALERT_MEDIUM),
      QUOTE_STATE(ANIMATED),
      QUOTE_STATE(BUSY),
      QUOTE_STATE(CHECKED),
      QUOTE_STATE(COLLAPSED),
      QUOTE_STATE(DEFAULT),
      QUOTE_STATE(EXPANDED),
      QUOTE_STATE(EXTSELECTABLE),
      QUOTE_STATE(FLOATING),
      QUOTE_STATE(FOCUSABLE),
      QUOTE_STATE(FOCUSED),
      QUOTE_STATE(HASPOPUP),
      QUOTE_STATE(HOTTRACKED),
      QUOTE_STATE(INVISIBLE),
      QUOTE_STATE(LINKED),
      QUOTE_STATE(MARQUEED),
      QUOTE_STATE(MIXED),
      QUOTE_STATE(MOVEABLE),
      QUOTE_STATE(MULTISELECTABLE),
      QUOTE_STATE(OFFSCREEN),
      QUOTE_STATE(PRESSED),
      QUOTE_STATE(PROTECTED),
      QUOTE_STATE(READONLY),
      QUOTE_STATE(SELECTABLE),
      QUOTE_STATE(SELECTED),
      QUOTE_STATE(SELFVOICING),
      QUOTE_STATE(SIZEABLE),
      QUOTE_STATE(TRAVERSED),
