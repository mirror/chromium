...
STDMETHODIMP BrowserAccessibilityComWin::get_oldText(IA2TextSegment* old_text) {
  ...
  base::string16 old_hypertext = old_hypertext_.hypertext_;
...
STDMETHODIMP BrowserAccessibilityComWin::get_nHyperlinks(
  ...
  *hyperlink_count = hypertext_.hyperlink_offset_to_index_.size();
  return S_OK;
}
...
STDMETHODIMP BrowserAccessibilityComWin::get_hyperlink(
  ...
  if (!hyperlink || index < 0 ||
      index >= static_cast<long>(hypertext_.hyperlinks_.size())) {
    return E_INVALIDARG;
  }
...
STDMETHODIMP BrowserAccessibilityComWin::get_hyperlinkIndex(
  ...
  std::map<int32_t, int32_t>::iterator it =
      hypertext_.hyperlink_offset_to_index_.find(char_index);
  if (it == hypertext_.hyperlink_offset_to_index_.end()) {
    *hyperlink_index = -1;
    return S_FALSE;
  }
...
void BrowserAccessibilityComWin::UpdateStep1ComputeWinAttributes() {
  ...
  win_attributes_->value = ComputeValue();
