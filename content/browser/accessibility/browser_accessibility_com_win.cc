...
STDMETHODIMP BrowserAccessibilityComWin::get_anchorTarget(
  ...
  if (!(MSAAState() & STATE_SYSTEM_LINKED) ||
      FAILED(GetStringAttributeAsBstr(ax::mojom::StringAttribute::URL, &target))) {
...
STDMETHODIMP BrowserAccessibilityComWin::nActions(long* n_actions) {
  ...
  // |IAccessibleAction|.
  if (IsHyperlink() ||
      owner()->HasIntAttribute(ax::mojom::IntAttribute::DEFAULT_ACTION_VERB)) {
    *n_actions = 1;
  } else {
    *n_actions = 0;
  }
...
STDMETHODIMP BrowserAccessibilityComWin::doAction(long action_index) {
  ...
  if (!owner()->HasIntAttribute(ax::mojom::IntAttribute::DEFAULT_ACTION_VERB) ||
      action_index != 0)
    return E_INVALIDARG;
...
STDMETHODIMP BrowserAccessibilityComWin::get_name(long action_index,
  ...
  int action;
  if (!owner()->GetIntAttribute(ax::mojom::IntAttribute::DEFAULT_ACTION_VERB, &action) ||
      action_index != 0) {
    *name = nullptr;
    return E_INVALIDARG;
  }
...
STDMETHODIMP BrowserAccessibilityComWin::get_currentValue(VARIANT* value) {
  ...
  float float_val;
  if (GetFloatAttribute(ax::mojom::FloatAttribute::VALUE_FOR_RANGE, &float_val)) {
    value->vt = VT_R8;
...
STDMETHODIMP BrowserAccessibilityComWin::get_minimumValue(VARIANT* value) {
  ...
  float float_val;
  if (GetFloatAttribute(ax::mojom::FloatAttribute::MIN_VALUE_FOR_RANGE, &float_val)) {
    value->vt = VT_R8;
    value->dblVal = float_val;
    return S_OK;
  }
...
STDMETHODIMP BrowserAccessibilityComWin::get_maximumValue(VARIANT* value) {
  ...
  float float_val;
  if (GetFloatAttribute(ax::mojom::FloatAttribute::MAX_VALUE_FOR_RANGE, &float_val)) {
    value->vt = VT_R8;
    value->dblVal = float_val;
    return S_OK;
  }
...
STDMETHODIMP BrowserAccessibilityComWin::get_nodeInfo(
  ...
  base::string16 tag;
  if (owner()->GetString16Attribute(ax::mojom::StringAttribute::HTML_TAG, &tag))
...
STDMETHODIMP BrowserAccessibilityComWin::get_computedStyle(
  ...
  if (max_style_properties == 0 ||
      !owner()->GetString16Attribute(ax::mojom::StringAttribute::DISPLAY, &display)) {
...
STDMETHODIMP BrowserAccessibilityComWin::get_computedStyleForProperties(
  ...
  for (unsigned short i = 0; i < num_style_properties; ++i) {
    ...
    if (name == L"display") {
      base::string16 display =
          owner()->GetString16Attribute(ax::mojom::StringAttribute::DISPLAY);
...
STDMETHODIMP BrowserAccessibilityComWin::get_innerHTML(BSTR* innerHTML) {
  ...
  if (!owner())
    return E_FAIL;
  if (owner()->GetRole() != ax::mojom::Role::MATH)
    return E_NOTIMPL;

  base::string16 inner_html =
      owner()->GetString16Attribute(ax::mojom::StringAttribute::INNER_HTML);
...
STDMETHODIMP BrowserAccessibilityComWin::get_language(BSTR* language) {
  ...
  *language = nullptr;
  ...
  base::string16 lang =
      owner()->GetInheritedString16Attribute(ax::mojom::StringAttribute::LANGUAGE);
  if (lang.empty())
    lang = L"en-US";
...
STDMETHODIMP BrowserAccessibilityComWin::get_domText(BSTR* dom_text) {
  ...
  return GetStringAttributeAsBstr(ax::mojom::StringAttribute::NAME, dom_text);
}
...
STDMETHODIMP BrowserAccessibilityComWin::get_fontFamily(BSTR* font_family) {
  ...
  *font_family = nullptr;
  ...
  base::string16 family =
      owner()->GetInheritedString16Attribute(ax::mojom::StringAttribute::FONT_FAMILY);
  if (family.empty())
    return S_FALSE;
...
STDMETHODIMP BrowserAccessibilityComWin::GetPatternProvider(
  ...
  if (id == UIA_ValuePatternId || id == UIA_TextPatternId) {
    if (owner()->HasState(ax::mojom::State::EDITABLE)) {
...
BrowserAccessibilityComWin::CreatePositionForSelectionAt(int offset) const {
  ...
  if (position->GetAnchor() &&
      position->GetAnchor()->GetRole() == ax::mojom::Role::INLINE_TEXT_BOX) {
...
void BrowserAccessibilityComWin::UpdateStep1ComputeWinAttributes() {
  ...
  win_attributes_->name = owner()->GetString16Attribute(ax::mojom::StringAttribute::NAME);

  win_attributes_->description =
      owner()->GetString16Attribute(ax::mojom::StringAttribute::DESCRIPTION);
...
void BrowserAccessibilityComWin::UpdateStep3FireEvents(
  ...
  if (old_win_attributes_->ia_role != 0 ||
    ...
    if (is_selected_now || was_selected_before) {
      ...
      if (owner()->PlatformGetParent() &&
          owner()->PlatformGetParent()->HasState(ax::mojom::State::MULTISELECTABLE))
        multiselect = true;
    ...
    int sx = 0;
    int sy = 0;
    if (owner()->GetIntAttribute(ax::mojom::IntAttribute::SCROLL_X, &sx) &&
        owner()->GetIntAttribute(ax::mojom::IntAttribute::SCROLL_Y, &sy)) {
    ...
    if (parent && (parent->owner()->HasState(ax::mojom::State::EDITABLE) ||
...
std::vector<base::string16> BrowserAccessibilityComWin::ComputeTextAttributes()
  ...
  if (owner()->GetRole() == ax::mojom::Role::LIST_MARKER)
  ...
  if (owner()->GetIntAttribute(ax::mojom::IntAttribute::BACKGROUND_COLOR, &color)) {
  ...
  if (owner()->GetIntAttribute(ax::mojom::IntAttribute::COLOR, &color)) {
    unsigned int red = SkColorGetR(color);
    unsigned int green = SkColorGetG(color);
    unsigned int blue = SkColorGetB(color);
    color_value = L"rgb(" + base::UintToString16(red) + L',' +
  ...
  float font_size;
  // Attribute has no default value.
  if (GetFloatAttribute(ax::mojom::FloatAttribute::FONT_SIZE, &font_size)) {
  ...
  int32_t text_style = owner()->GetIntAttribute(ax::mojom::IntAttribute::TEXT_STYLE);
  ...
  int32_t invalid_state = owner()->GetIntAttribute(ax::mojom::IntAttribute::INVALID_STATE);
  switch (static_cast<ax::mojom::InvalidState>(invalid_state)) {
    ...
    case ax::mojom::InvalidState::GRAMMAR: {
      ...
      if (invalid_state & static_cast<int32_t>(ax::mojom::InvalidState::SPELLING))
        spelling_grammar_value = L"spelling";
      else if (invalid_state &
               static_cast<int32_t>(ax::mojom::InvalidState::GRAMMAR))
    ...
    case ax::mojom::InvalidState::OTHER: {
      ...
      if (owner()->GetString16Attribute(ax::mojom::StringAttribute::ARIA_INVALID_VALUE,
                                        &aria_invalid_value)) {
  ...
  auto text_direction = static_cast<ax::mojom::TextDirection>(
      owner()->GetIntAttribute(ax::mojom::IntAttribute::TEXT_DIRECTION));
...
BrowserAccessibilityComWin::GetSpellingAttributes() {
  ...
  if (owner()->IsTextOnlyObject()) {
    const std::vector<int32_t>& marker_types =
        owner()->GetIntListAttribute(ax::mojom::IntListAttribute::MARKER_TYPES);
    const std::vector<int>& marker_starts =
        owner()->GetIntListAttribute(ax::mojom::IntListAttribute::MARKER_STARTS);
    const std::vector<int>& marker_ends =
        owner()->GetIntListAttribute(ax::mojom::IntListAttribute::MARKER_ENDS);
...
LONG BrowserAccessibilityComWin::FindBoundary(
  ...
  ax::mojom::TextAffinity affinity = ax::mojom::TextAffinity::DOWNSTREAM;
...
bool BrowserAccessibilityComWin::IsListBoxOptionOrMenuListOption() {
  ...
  if (role == ax::mojom::Role::LIST_BOX_OPTION &&
      parent_role == ax::mojom::Role::LIST_BOX) {
    return true;
  }

  if (role == ax::mojom::Role::MENU_LIST_OPTION &&
      parent_role == ax::mojom::Role::MENU_LIST_POPUP) {
    return true;
  }
