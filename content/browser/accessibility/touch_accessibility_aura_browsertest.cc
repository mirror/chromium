...
IN_PROC_BROWSER_TEST_F(TouchAccessibilityBrowserTest,
  ...
  AccessibilityNotificationWaiter waiter(
      shell()->web_contents(), ui::kAXModeComplete, ax::mojom::Event::HOVER);
  for (int row = 0; row < 5; ++row) {
    for (int col = 0; col < 7; ++col) {
      ...
      do {
        ...
        cell_text = child->GetData().GetStringAttribute(ax::mojom::StringAttribute::NAME);
...
IN_PROC_BROWSER_TEST_F(TouchAccessibilityBrowserTest,
  ...
  AccessibilityNotificationWaiter waiter(
      child_frame, ax::mojom::Event::HOVER);
  SendTouchExplorationEvent(50, 350);
  waiter.WaitForNotification();
  int target_id = waiter.event_target_id();
  BrowserAccessibility* hit = child_manager->GetFromID(target_id);
  EXPECT_EQ(ax::mojom::Role::BUTTON, hit->GetData().role);
  std::string text = hit->GetData().GetStringAttribute(ax::mojom::StringAttribute::NAME);
