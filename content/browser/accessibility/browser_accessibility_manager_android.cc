...
bool BrowserAccessibilityManagerAndroid::ShouldRespectDisplayedPasswordText() {
  WebContentsAccessibility* wcax = GetWebContentsAXFromRootManager();
...
bool BrowserAccessibilityManagerAndroid::NextAtGranularityForTesting(
    int32_t granularity,
    ...
    int32_t* end_index) {
  auto* wcax = GetWebContentsAXFromRootManager();
  return wcax ? wcax->NextAtGranularity(granularity, cursor_index, node,
                                        start_index, end_index)
              : false;
}

bool BrowserAccessibilityManagerAndroid::PreviousAtGranularityForTesting(
    int32_t granularity,
    int32_t cursor_index,
    BrowserAccessibilityAndroid* node,
    int32_t* start_index,
    int32_t* end_index) {
  auto* wcax = GetWebContentsAXFromRootManager();
  return wcax ? wcax->PreviousAtGranularity(granularity, cursor_index, node,
                                            start_index, end_index)
              : false;
}
