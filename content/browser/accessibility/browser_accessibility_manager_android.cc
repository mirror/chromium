...
BrowserAccessibilityManagerAndroid::BrowserAccessibilityManagerAndroid(
  ...
  if (web_contents_accessibility)
    web_contents_accessibility->set_delegate(this);
  Initialize(initial_tree);
}

BrowserAccessibilityManagerAndroid::~BrowserAccessibilityManagerAndroid() {
  if (web_contents_accessibility_)
    web_contents_accessibility_->set_delegate(nullptr);
}
...
BrowserAccessibility* BrowserAccessibilityManagerAndroid::GetRootNode() {
  return GetRoot();
}

BrowserAccessibility* BrowserAccessibilityManagerAndroid::GetFocusNode() {
  return GetFocus();
}

void BrowserAccessibilityManagerAndroid::SetFocusNode(
    const BrowserAccessibility& node) {
  SetFocus(node);
}

void BrowserAccessibilityManagerAndroid::HitTestNode(const gfx::Point& point) {
  HitTest(point);
}
