 ...
 private:
  const BrowserAccessibility* FindNodeInSubtree(
    ...
    if (node.GetRole() == role &&
        node.GetStringAttribute(ax::mojom::StringAttribute::NAME) == name)
      return &node;
...
IN_PROC_BROWSER_TEST_F(AccessibilityModeTest,
  ...
  AccessibilityNotificationWaiter waiter(
      shell()->web_contents(), ui::kAXModeComplete, ax::mojom::Event::LOAD_COMPLETE);
  GURL url("data:text/html,<p>Para</p>");
  NavigateToURL(shell(), url);
  waiter.WaitForNotification();

  const BrowserAccessibility* text = FindNode(ax::mojom::Role::STATIC_TEXT, "Para");
  ...
  EXPECT_EQ(ax::mojom::Role::INLINE_TEXT_BOX, inline_text->GetRole());
...
IN_PROC_BROWSER_TEST_F(AccessibilityModeTest, AddScreenReaderModeFlag) {
  ...
  const BrowserAccessibility* textbox = FindNode(ax::mojom::Role::TEXT_FIELD, "Foo");
  ASSERT_NE(nullptr, textbox);
  EXPECT_FALSE(textbox->HasStringAttribute(ax::mojom::StringAttribute::PLACEHOLDER));
  ...
  const BrowserAccessibility* textbox2 = FindNode(
      ax::mojom::Role::TEXT_FIELD, "Foo");
  ASSERT_NE(nullptr, textbox2);
  EXPECT_TRUE(textbox2->HasStringAttribute(ax::mojom::StringAttribute::PLACEHOLDER));
