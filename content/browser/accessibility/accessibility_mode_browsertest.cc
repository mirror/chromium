...
IN_PROC_BROWSER_TEST_F(AccessibilityModeTest,
  ...
  const BrowserAccessibility* text =
      FindNode(ax::mojom::Role::STATIC_TEXT, "Para");
...
IN_PROC_BROWSER_TEST_F(AccessibilityModeTest, AddScreenReaderModeFlag) {
  ...
  const BrowserAccessibility* textbox =
      FindNode(ax::mojom::Role::TEXT_FIELD, "Foo");
  ASSERT_NE(nullptr, textbox);
  EXPECT_FALSE(
      textbox->HasStringAttribute(ax::mojom::StringAttribute::PLACEHOLDER));
  ...
  EXPECT_TRUE(
      textbox2->HasStringAttribute(ax::mojom::StringAttribute::PLACEHOLDER));
