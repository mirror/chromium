...
bool AccessibilityTreeContainsLoadedDocWithUrl(BrowserAccessibility* node,
  ...
  if (node->GetRole() == ax::mojom::Role::ROOT_WEB_AREA &&
      node->GetStringAttribute(ax::mojom::StringAttribute::URL) == url) {
  ...
  if (node->GetRole() == ax::mojom::Role::WEB_AREA &&
      node->GetStringAttribute(ax::mojom::StringAttribute::URL) == url) {
...
void DumpAccessibilityTestBase::RunTestForPlatform(
  ...
  if (enable_accessibility_after_navigating_ &&
    ...
    AccessibilityNotificationWaiter accessibility_waiter(
        web_contents, ui::kAXModeComplete, ax::mojom::Event::NONE);
    accessibility_waiter.WaitForNotification();
  } else {
    // Enable accessibility, then load the test html and wait for the
    // "load complete" AX event.
    AccessibilityNotificationWaiter accessibility_waiter(
        web_contents, ui::kAXModeComplete, ax::mojom::Event::LOAD_COMPLETE);
    NavigateToURL(shell(), url);
    accessibility_waiter.WaitForNotification();
  }
  ...
  for (;;) {
    ...
    AccessibilityNotificationWaiter accessibility_waiter(
        main_frame, ax::mojom::Event::NONE);
    for (FrameTreeNode* node : frame_tree->Nodes())
