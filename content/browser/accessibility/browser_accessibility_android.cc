...
bool BrowserAccessibilityAndroid::IsChecked() const {
  return GetData().GetCheckedState() == ax::mojom::CheckedState::TRUE_VALUE;
}
...
bool BrowserAccessibilityAndroid::IsClickable() const {
  ...
  // ax::mojom::DefaultActionVerb::CLICK_ANCESTOR is used when an element with a
...
bool BrowserAccessibilityAndroid::IsContentInvalid() const {
  return HasIntAttribute(ax::mojom::IntAttribute::INVALID_STATE) &&
      GetData().GetInvalidState() != ax::mojom::InvalidState::FALSE_VALUE;
}
...
bool BrowserAccessibilityAndroid::IsEnabled() const {
  return GetData().GetRestriction() != ax::mojom::Restriction::DISABLED;
}
...
base::string16 BrowserAccessibilityAndroid::GetText() const {
  if (IsIframe() || GetRole() == ax::mojom::Role::WEB_AREA) {
  ...
  if (GetRole() == ax::mojom::Role::COLOR_WELL) {
    unsigned int color = static_cast<unsigned int>(
        GetIntAttribute(ax::mojom::IntAttribute::COLOR_VALUE));
...
int BrowserAccessibilityAndroid::GetItemIndex() const {
  ...
  if (IsRangeType()) {
    ...
    float min =
        GetFloatAttribute(ax::mojom::FloatAttribute::MIN_VALUE_FOR_RANGE);
    float max =
        GetFloatAttribute(ax::mojom::FloatAttribute::MAX_VALUE_FOR_RANGE);
...
bool BrowserAccessibilityAndroid::Scroll(int direction) const {
  ...
  } else if (GetRole() == ax::mojom::Role::ROOT_WEB_AREA &&
             PlatformGetParent()) {
...
int BrowserAccessibilityAndroid::AndroidInputType() const {
  std::string html_tag =
      GetStringAttribute(ax::mojom::StringAttribute::HTML_TAG);
  if (html_tag != "input")
...
int BrowserAccessibilityAndroid::AndroidLiveRegionType() const {
  std::string live =
      GetStringAttribute(ax::mojom::StringAttribute::LIVE_STATUS);
  if (live == "polite")
