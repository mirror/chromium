...
HWND GetHwndForProcess(base::ProcessId pid) {
  EnumWindows(&EnumWindowsProcPid, pid);
  return hwnd_;
}
}  // namespace
...
class AccessibilityTreeFormatterWin : public AccessibilityTreeFormatter {
 public:
  ...
  std::unique_ptr<base::DictionaryValue> BuildAccessibilityTree(HWND hwnd);
  std::unique_ptr<base::DictionaryValue> BuildAccessibilityTree(
 ...
 private:
  ...
  // This magic is required for ax_dump_tree tool to work.
...
std::unique_ptr<base::DictionaryValue>
AccessibilityTreeFormatterWin::BuildAccessibilityTree(HWND hwnd) {
  if (!hwnd)
    return nullptr;
  ...
  HRESULT hr = ::AccessibleObjectFromWindow(
      ...
      reinterpret_cast<void**>(start.GetAddressOf()));
  if (FAILED(hr))
    return nullptr;
...
std::unique_ptr<base::DictionaryValue>
AccessibilityTreeFormatterWin::BuildAccessibilityTree(base::ProcessId pid) {
  // Get HWND for process id.
  HWND hwnd = GetHwndForProcess(pid);
  return BuildAccessibilityTree(hwnd);
}
...
void AccessibilityTreeFormatterWin::RecursiveBuildAccessibilityTree(
  ...
  for (size_t index = 0; index < obtained_count; index++) {
    ...
    if (child_variant.type() == VT_DISPATCH) {
      *(dispatch.GetAddressOf()) = V_DISPATCH(child_variant.ptr());
      ...
      } else if (!*(dispatch.GetAddressOf())) {
        // Partial child does not have it's own object.
    ...
    if (*(dispatch.GetAddressOf())) {
      // QueryInterface is not necessary since IAccessible extends IDispatch.
      IAccessible* accessible =
          static_cast<IAccessible*>(*(dispatch.GetAddressOf()));
      if (accessible) {
        RecursiveBuildAccessibilityTree(accessible, child_dict.get(), root_x,
                                        root_y);
        // Get window handle and class name.
        HWND hwnd;
        WindowFromAccessibleObject(accessible, &hwnd);
        TCHAR className[128];
        GetClassName(hwnd, className, sizeof(className));
        child_dict->SetString("window_class", className);
      }
    }
...
const char* const ALL_ATTRIBUTES[] = {"name",
                                      "value",
                                      "states",
                                      "attributes",
                                      "text_attributes",
