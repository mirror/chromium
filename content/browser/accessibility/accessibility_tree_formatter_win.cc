...
static HRESULT QuerySimpleDOMNode(IAccessible* accessible,
  ...
  return SUCCEEDED(hr) ? service_provider->QueryService(IID_ISimpleDOMNode,
                                                        simple_dom_node)
                       : hr;
}
...
static HRESULT QueryIAccessibleAction(IAccessible* accessible,
  ...
  base::win::ScopedComPtr<IServiceProvider> service_provider;
  HRESULT hr = accessible->QueryInterface(service_provider.GetAddressOf());
  return SUCCEEDED(hr) ? service_provider->QueryService(IID_IAccessibleAction,
                                                        accessibleAction)
                       : hr;
}
...
static HRESULT QueryIAccessibleHypertext(
  ...
  base::win::ScopedComPtr<IServiceProvider> service_provider;
  HRESULT hr = accessible->QueryInterface(service_provider.GetAddressOf());
  return SUCCEEDED(hr) ? service_provider->QueryService(
                             IID_IAccessibleHypertext, accessibleHypertext)
                       : hr;
}
...
static HRESULT QueryIAccessibleTable(IAccessible* accessible,
  ...
  base::win::ScopedComPtr<IServiceProvider> service_provider;
  HRESULT hr = accessible->QueryInterface(service_provider.GetAddressOf());
  return SUCCEEDED(hr) ? service_provider->QueryService(IID_IAccessibleTable,
                                                        accessibleTable)
                       : hr;
}
...
static HRESULT QueryIAccessibleValue(IAccessible* accessible,
  ...
  base::win::ScopedComPtr<IServiceProvider> service_provider;
  HRESULT hr = accessible->QueryInterface(service_provider.GetAddressOf());
  return SUCCEEDED(hr) ? service_provider->QueryService(IID_IAccessibleValue,
                                                        accessibleValue)
                       : hr;
}
...
bool AccessibilityTreeFormatterWin::AddIA2Properties(
  ...
  if (ia2->get_states(&states) == S_OK) {
    ...
    base::ListValue* states_list;  // std::unique_ptr?
