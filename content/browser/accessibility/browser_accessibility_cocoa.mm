...
- (content::AXTextEdit)computeTextEdit {
  ...
  for (j = 0;
       ...
       (oldValue[oldValue.length() - j] == newValue[newValue.length() - j] ||
        (base::IsUnicodeWhitespace(oldValue[oldValue.length() - j]) &&
         base::IsUnicodeWhitespace(newValue[newValue.length() - j])));
       ++j) {
  }
  DCHECK_LT(i + j, oldValue.length());
  DCHECK_LT(i + j, newValue.length());

  base::string16 deletedText = oldValue.substr(i, oldValue.length() - j);
  base::string16 insertedText = newValue.substr(i, newValue.length() - j);
