...
- (TextEdit)ComputeTextEdit {
  ...
  if (oldValue.length() < newValue.length()) {
    ...
    while (i < oldValue.length() && i < newValue.length() &&
           (oldValue[i] == newValue[i] || (IsUnicodeWhitespace(oldValue[i]) &&
                                           IsUnicodeWhitespace(newValue[i])))) {
      ++i;
    }
    size_t length = (newValue.length() - i) - (oldValue.length() - i);
    base::string16 insertedText = newValue.substr(i, length);
    return TextEdit(insertedText, false /* is_deleted */);
  } else {
    // Deletion.
    size_t i = 0;
    while (i < oldValue.length() && i < newValue.length() &&
    ...
    return TextEdit(deletedText, true /* is_deleted */);
  }
}
