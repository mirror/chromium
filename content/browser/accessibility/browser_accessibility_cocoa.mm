...
- (BOOL)isRowHeaderForCurrentCell:(BrowserAccessibility*)header {
  ...
  header->GetIntAttribute(ui::AX_ATTR_TABLE_CELL_ROW_INDEX, &header_first_row);
...
- (NSArray*)accessibilityAttributeNames {
  ...
  if ([role isEqualToString:NSAccessibilityTableRole] ||
    ...
    if (browserAccessibility_->GetRole() != ui::AX_ROLE_COLUMN_HEADER) {
      [ret addObjectsFromArray:@[
        NSAccessibilityColumnHeaderUIElementsAttribute,
      ]];
    }
    if (browserAccessibility_->GetRole() != ui::AX_ROLE_ROW_HEADER) {
      [ret addObjectsFromArray:@[
        NSAccessibilityRowHeaderUIElementsAttribute,
      ]];
    }
