...
- (NSString*)accessKey {
  ...
  return NSStringForStringAttribute(browserAccessibility_,
                                    ax::mojom::StringAttribute::ACCESS_KEY);
}
...
- (NSNumber*)ariaColumnCount {
  ...
  if (!browserAccessibility_->GetIntAttribute(
          ax::mojom::IntAttribute::ARIA_COLUMN_COUNT, &count)) {
    return nil;
  }
...
- (NSNumber*)ariaPosInSet {
  ...
  return [NSNumber numberWithInt:browserAccessibility_->GetIntAttribute(
                                     ax::mojom::IntAttribute::POS_IN_SET)];
}
...
- (NSNumber*)ariaRowCount {
  ...
  int count = -1;
  if (!browserAccessibility_->GetIntAttribute(
          ax::mojom::IntAttribute::ARIA_ROW_COUNT, &count)) {
    return nil;
  }
  return [NSNumber numberWithInt:count];
}
...
- (NSNumber*)ariaRowIndex {
  ...
  int index = -1;
  if (!browserAccessibility_->GetIntAttribute(
          ax::mojom::IntAttribute::ARIA_CELL_ROW_INDEX, &index)) {
    return nil;
  }
  return [NSNumber numberWithInt:index];
}
...
- (NSNumber*)ariaSetSize {
  if (![self instanceActive])
    return nil;
  return [NSNumber numberWithInt:browserAccessibility_->GetIntAttribute(
                                     ax::mojom::IntAttribute::SET_SIZE)];
}
...
- (BOOL)isColumnHeaderForCurrentCell:(BrowserAccessibility*)header {
  ...
  browserAccessibility_->GetIntAttribute(
      ax::mojom::IntAttribute::ARIA_CELL_COLUMN_INDEX, &cell_first_col);
  if (cell_first_col < 0) {
    browserAccessibility_->GetIntAttribute(
        ax::mojom::IntAttribute::TABLE_CELL_COLUMN_INDEX, &cell_first_col);
  }
  if (cell_first_col < 0)
    return false;
  browserAccessibility_->GetIntAttribute(
      ax::mojom::IntAttribute::TABLE_CELL_COLUMN_SPAN, &cell_colspan);
  if (cell_colspan <= 0)
    cell_colspan = 1;
  ...
  header->GetIntAttribute(ax::mojom::IntAttribute::TABLE_CELL_COLUMN_SPAN,
                          &header_colspan);
  if (header_colspan <= 0)
    header_colspan = 1;
...
- (NSString*)description {
  ...
  ax::mojom::NameFrom nameFrom =
      static_cast<ax::mojom::NameFrom>(browserAccessibility_->GetIntAttribute(
          ax::mojom::IntAttribute::NAME_FROM));
...
- (NSNumber*)disclosureLevel {
  ...
  if (role == ax::mojom::Role::ROW || role == ax::mojom::Role::TREE_ITEM) {
...
- (NSString*)help {
  if (![self instanceActive])
    return nil;
  return NSStringForStringAttribute(browserAccessibility_,
                                    ax::mojom::StringAttribute::DESCRIPTION);
}
...
- (NSNumber*)insertionPointLineNumber {
  ...
  int selStart, selEnd;
  if (!browserAccessibility_->GetIntAttribute(
          ax::mojom::IntAttribute::TEXT_SEL_START, &selStart) ||
      !browserAccessibility_->GetIntAttribute(
          ax::mojom::IntAttribute::TEXT_SEL_END, &selEnd)) {
    return nil;
  }
...
- (NSString*)placeholderValue {
  ...
  return NSStringForStringAttribute(browserAccessibility_,
                                    ax::mojom::StringAttribute::PLACEHOLDER);
}
...
- (NSArray*)linkedUIElements {
  NSMutableArray* ret = [[[NSMutableArray alloc] init] autorelease];
  [self
      addLinkedUIElementsFromAttribute:ax::mojom::IntListAttribute::CONTROLS_IDS
                                 addTo:ret];
  [self addLinkedUIElementsFromAttribute:ax::mojom::IntListAttribute::FLOWTO_IDS
                                   addTo:ret];

  int target_id;
  if (browserAccessibility_->GetIntAttribute(
          ax::mojom::IntAttribute::IN_PAGE_LINK_TARGET_ID, &target_id)) {
...
- (BOOL)shouldExposeTitleUIElement {
  ...
  std::vector<int32_t> labelledby_ids =
      browserAccessibility_->GetIntListAttribute(
          ax::mojom::IntListAttribute::LABELLEDBY_IDS);
  ...
  std::string labelName =
      label->GetStringAttribute(ax::mojom::StringAttribute::NAME);
...
- (BOOL)isRowHeaderForCurrentCell:(BrowserAccessibility*)header {
  ...
  int cell_rowspan = -1;
  browserAccessibility_->GetIntAttribute(
      ax::mojom::IntAttribute::ARIA_CELL_ROW_INDEX, &cell_first_row);
  if (cell_first_row < 0) {
    browserAccessibility_->GetIntAttribute(
        ax::mojom::IntAttribute::TABLE_CELL_ROW_INDEX, &cell_first_row);
  }
  if (cell_first_row < 0)
    return false;

  browserAccessibility_->GetIntAttribute(
      ax::mojom::IntAttribute::TABLE_CELL_ROW_SPAN, &cell_rowspan);
  if (cell_rowspan <= 0)
    cell_rowspan = 1;
  ...
  header->GetIntAttribute(ax::mojom::IntAttribute::ARIA_CELL_ROW_INDEX,
                          &header_first_row);
  ...
  header->GetIntAttribute(ax::mojom::IntAttribute::TABLE_CELL_ROW_SPAN,
                          &header_rowspan);
  if (header_rowspan <= 0)
    header_rowspan = 1;
...
- (id)value {
  ...
  if ([role isEqualToString:@"AXHeading"]) {
    ...
    switch (checkedState) {
      ...
      default:
        value =
            GetState(browserAccessibility_, ax::mojom::State::SELECTED) ? 1 : 0;
        break;
    }
    ...
    unsigned int color =
        static_cast<unsigned int>(browserAccessibility_->GetIntAttribute(
            ax::mojom::IntAttribute::COLOR_VALUE));
...
- (NSArray*)visibleCells {
  ...
  const std::vector<int32_t>& uniqueCellIds =
      browserAccessibility_->GetIntListAttribute(
          ax::mojom::IntListAttribute::UNIQUE_CELL_IDS);
  for (size_t i = 0; i < uniqueCellIds.size(); ++i) {
    int id = uniqueCellIds[i];
    BrowserAccessibility* cell =
        browserAccessibility_->manager()->GetFromID(id);
  ...
  if ([attribute isEqualToString:
    ...
    for (size_t i = 0;
      ...
      if (child->GetRole() != ax::mojom::Role::ROW)
        continue;
      int rowIndex;
      if (!child->GetIntAttribute(ax::mojom::IntAttribute::TABLE_ROW_INDEX,
                                  &rowIndex)) {
        continue;
      }
      if (rowIndex < row)
      ...
      for (size_t j = 0;
        ...
        if (!cell->GetIntAttribute(
                ax::mojom::IntAttribute::TABLE_CELL_COLUMN_INDEX, &colIndex)) {
          continue;
        }
        if (colIndex == column)
...
- (NSArray*)accessibilityAttributeNames {
  ...
  if (browserAccessibility_->HasIntListAttribute(
      ...
      browserAccessibility_
              ->GetIntListAttribute(ax::mojom::IntListAttribute::LABELLEDBY_IDS)
              .size() > 0) {
