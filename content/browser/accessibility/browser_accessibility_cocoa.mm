...
- (content::AXTextEdit)computeTextEdit {
  ...
  // We assume that Blink will update the accessibility tree when a text
  // insertion or a deletion takes place, but that there will never be both an
  // insertion and a deletion in a single tree update.
  // TODO(nektar): This assumption might need to be revised when a text
  // replacement takes place after a selection.
  ...
  if (oldValue.length() < newValue.length()) {
    ...
    while (i < oldValue.length() && i < newValue.length() &&
           (oldValue[i] == newValue[i] ||
            (base::IsUnicodeWhitespace(oldValue[i]) &&
             base::IsUnicodeWhitespace(newValue[i])))) {
      ++i;
    }
    size_t length = (newValue.length() - i) - (oldValue.length() - i);
    base::string16 insertedText = newValue.substr(i, length);
    return content::AXTextEdit(insertedText, false /* is_deleted */);
  } else {
    // Deletion.
    size_t i = 0;
    while (i < oldValue.length() && i < newValue.length() &&
    ...
    size_t length = (oldValue.length() - i) - (newValue.length() - i);
    base::string16 deletedText = oldValue.substr(i, length);
    return content::AXTextEdit(deletedText, true /* is_deleted */);
  }
}
