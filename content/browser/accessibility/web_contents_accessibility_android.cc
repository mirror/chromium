...
jint WebContentsAccessibilityAndroid::GetRootId(
    ...
    const JavaParamRef<jobject>& obj) {
  if (delegate_) {
    auto* root = delegate_->GetRootNode();
    if (root)
...
void WebContentsAccessibilityAndroid::HitTest(JNIEnv* env,
                                              ...
                                              jint y) {
  if (delegate_)
    delegate_->HitTestNode(gfx::Point(x, y));
}
...
void WebContentsAccessibilityAndroid::Blur(JNIEnv* env,
                                           const JavaParamRef<jobject>& obj) {
  if (delegate_)
    delegate_->SetFocusNode(*delegate_->GetRootNode());
}
...
jint WebContentsAccessibilityAndroid::FindElementType(
  ...
  BrowserAccessibility* root = delegate_->GetRootNode();
  if (!root)
    return 0;
...
jboolean WebContentsAccessibilityAndroid::NextAtGranularity(
  ...
  jint start_index = -1;
  int end_index = -1;
  if (delegate_->NextAtGranularity(granularity, cursor_index, node,
...
jboolean WebContentsAccessibilityAndroid::PreviousAtGranularity(
  ...
  jint start_index = -1;
  int end_index = -1;
  if (delegate_->PreviousAtGranularity(granularity, cursor_index, node,
                                       &start_index, &end_index)) {
    Java_WebContentsAccessibility_finishGranularityMove(
...
void WebContentsAccessibilityAndroid::OnAutofillPopupDisplayed(
  ...
  BrowserAccessibility* current_focus = delegate_->GetFocusNode();
  ...
  g_autofill_popup_proxy_node->Init(
      static_cast<BrowserAccessibilityManagerAndroid*>(delegate_),
