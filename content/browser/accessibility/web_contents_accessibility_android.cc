...
jboolean WebContentsAccessibilityAndroid::PopulateAccessibilityNodeInfo(
  ...
  bool has_character_locations =
      node->GetRole() == ax::mojom::Role::STATIC_TEXT ||
...
jboolean WebContentsAccessibilityAndroid::AdjustSlider(
  ...
  float value =
      node->GetFloatAttribute(ax::mojom::FloatAttribute::VALUE_FOR_RANGE);
  float min =
      node->GetFloatAttribute(ax::mojom::FloatAttribute::MIN_VALUE_FOR_RANGE);
  float max =
      node->GetFloatAttribute(ax::mojom::FloatAttribute::MAX_VALUE_FOR_RANGE);
  if (max <= min)
    return false;
...
void WebContentsAccessibilityAndroid::OnAutofillPopupDisplayed(
  ...
  ax_node_data.SetRestriction(ax::mojom::Restriction::READ_ONLY);
