...
IN_PROC_BROWSER_TEST_F(AccessibilityIpcErrorBrowserTest,
  ...
  const ui::AXTree* tree = nullptr;
  {
    AccessibilityNotificationWaiter waiter(
        shell()->web_contents(), ui::kAXModeComplete, ax::mojom::Event::FOCUS);
    ASSERT_TRUE(
  ...
  EXPECT_EQ(ax::mojom::Role::GENERIC_CONTAINER, live_region->data().role);

  const ui::AXNode* para = live_region->ChildAtIndex(0);
  EXPECT_EQ(ax::mojom::Role::PARAGRAPH, para->data().role);
