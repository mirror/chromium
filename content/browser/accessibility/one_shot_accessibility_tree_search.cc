...
void GetNodeStrings(BrowserAccessibility* node,
  ...
  if (node->HasStringAttribute(ax::mojom::StringAttribute::NAME))
    strings->push_back(node->GetString16Attribute(ax::mojom::StringAttribute::NAME));
  if (node->HasStringAttribute(ax::mojom::StringAttribute::DESCRIPTION))
    strings->push_back(node->GetString16Attribute(ax::mojom::StringAttribute::DESCRIPTION));
  if (node->HasStringAttribute(ax::mojom::StringAttribute::VALUE))
    strings->push_back(node->GetString16Attribute(ax::mojom::StringAttribute::VALUE));
  if (node->HasStringAttribute(ax::mojom::StringAttribute::PLACEHOLDER))
    strings->push_back(node->GetString16Attribute(ax::mojom::StringAttribute::PLACEHOLDER));
}
...
bool OneShotAccessibilityTreeSearch::Matches(BrowserAccessibility* node) {
  ...
  if (visible_only_) {
    if (node->HasState(ax::mojom::State::INVISIBLE) || node->IsOffscreen()) {
      return false;
    }
  }
...
bool AccessibilityArticlePredicate(
  ...
  return node->GetRole() == ax::mojom::Role::ARTICLE;
}
...
bool AccessibilityBlockquotePredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return node->GetRole() == ax::mojom::Role::BLOCKQUOTE;
}

bool AccessibilityCheckboxPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::CHECK_BOX ||
          node->GetRole() == ax::mojom::Role::MENU_ITEM_CHECK_BOX);
}

bool AccessibilityComboboxPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::COMBO_BOX_GROUPING ||
          node->GetRole() == ax::mojom::Role::COMBO_BOX_MENU_BUTTON ||
          node->GetRole() == ax::mojom::Role::TEXT_FIELD_WITH_COMBO_BOX ||
          node->GetRole() == ax::mojom::Role::POP_UP_BUTTON);
}
...
bool AccessibilityControlPredicate(
  ...
  if (ui::IsControl(node->GetRole()))
    return true;
  if (node->HasState(ax::mojom::State::FOCUSABLE) &&
      node->GetRole() != ax::mojom::Role::IFRAME &&
      node->GetRole() != ax::mojom::Role::IFRAME_PRESENTATIONAL &&
      node->GetRole() != ax::mojom::Role::LINK &&
      node->GetRole() != ax::mojom::Role::WEB_AREA &&
      node->GetRole() != ax::mojom::Role::ROOT_WEB_AREA) {
    return true;
  }
  return false;
}
...
bool AccessibilityFocusablePredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  bool focusable = node->HasState(ax::mojom::State::FOCUSABLE);
  if (node->GetRole() == ax::mojom::Role::IFRAME ||
      node->GetRole() == ax::mojom::Role::IFRAME_PRESENTATIONAL ||
      node->GetRole() == ax::mojom::Role::WEB_AREA ||
      node->GetRole() == ax::mojom::Role::ROOT_WEB_AREA) {
    focusable = false;
...
bool AccessibilityGraphicPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return node->GetRole() == ax::mojom::Role::IMAGE;
}

bool AccessibilityHeadingPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::HEADING);
}

bool AccessibilityH1Predicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::HEADING &&
          node->GetIntAttribute(ax::mojom::IntAttribute::HIERARCHICAL_LEVEL) == 1);
}

bool AccessibilityH2Predicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::HEADING &&
          node->GetIntAttribute(ax::mojom::IntAttribute::HIERARCHICAL_LEVEL) == 2);
}

bool AccessibilityH3Predicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::HEADING &&
          node->GetIntAttribute(ax::mojom::IntAttribute::HIERARCHICAL_LEVEL) == 3);
}

bool AccessibilityH4Predicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::HEADING &&
          node->GetIntAttribute(ax::mojom::IntAttribute::HIERARCHICAL_LEVEL) == 4);
}

bool AccessibilityH5Predicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::HEADING &&
          node->GetIntAttribute(ax::mojom::IntAttribute::HIERARCHICAL_LEVEL) == 5);
}

bool AccessibilityH6Predicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::HEADING &&
          node->GetIntAttribute(ax::mojom::IntAttribute::HIERARCHICAL_LEVEL) == 6);
}

bool AccessibilityHeadingSameLevelPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::HEADING &&
          start->GetRole() == ax::mojom::Role::HEADING &&
          (node->GetIntAttribute(ax::mojom::IntAttribute::HIERARCHICAL_LEVEL) ==
           start->GetIntAttribute(ax::mojom::IntAttribute::HIERARCHICAL_LEVEL)));
}
...
bool AccessibilityFramePredicate(
  ...
  if (!node->PlatformGetParent())
    return false;
  return (node->GetRole() == ax::mojom::Role::WEB_AREA ||
          node->GetRole() == ax::mojom::Role::ROOT_WEB_AREA);
}
...
bool AccessibilityLinkPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return node->GetRole() == ax::mojom::Role::LINK;
}

bool AccessibilityListPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::LIST_BOX ||
          node->GetRole() == ax::mojom::Role::LIST ||
          node->GetRole() == ax::mojom::Role::DESCRIPTION_LIST);
}

bool AccessibilityListItemPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::LIST_ITEM ||
          node->GetRole() == ax::mojom::Role::DESCRIPTION_LIST_TERM ||
          node->GetRole() == ax::mojom::Role::LIST_BOX_OPTION);
}

bool AccessibilityLiveRegionPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return node->HasStringAttribute(ax::mojom::StringAttribute::LIVE_STATUS);
}

bool AccessibilityMainPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::MAIN);
}

bool AccessibilityMediaPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  const std::string& tag = node->GetStringAttribute(ax::mojom::StringAttribute::HTML_TAG);
  return tag == "audio" || tag == "video";
}

bool AccessibilityRadioButtonPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return (node->GetRole() == ax::mojom::Role::RADIO_BUTTON ||
          node->GetRole() == ax::mojom::Role::MENU_ITEM_RADIO);
}

bool AccessibilityRadioGroupPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return node->GetRole() == ax::mojom::Role::RADIO_GROUP;
}
...
bool AccessibilityTextStyleBoldPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  int32_t style = node->GetIntAttribute(ax::mojom::IntAttribute::TEXT_STYLE);
  return 0 != (style & static_cast<int32_t>(ax::mojom::TextStyle::TEXT_STYLE_BOLD));
}
...
bool AccessibilityTextStyleUnderlinePredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  int32_t style = node->GetIntAttribute(ax::mojom::IntAttribute::TEXT_STYLE);
  return 0 != (style & static_cast<int32_t>(ax::mojom::TextStyle::TEXT_STYLE_UNDERLINE));
}
...
bool AccessibilityUnvisitedLinkPredicate(
    BrowserAccessibility* start, BrowserAccessibility* node) {
  return node->GetRole() == ax::mojom::Role::LINK &&
         !node->HasState(ax::mojom::State::VISITED);
}
