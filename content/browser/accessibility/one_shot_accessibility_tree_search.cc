...
void OneShotAccessibilityTreeSearch::SetIsAndroid(bool is_android) {
  DCHECK(!did_search_);
...
void OneShotAccessibilityTreeSearch::SearchByWalkingTree() {
  ...
  if (node != scope_node_ || result_limit_ == 1) {
    if (direction_ == FORWARDS)
      ...
      node = tree_->PreviousInTreeOrder(start_node_, is_android_);
  }
  ...
  while (node &&
    ...
    if (direction_ == FORWARDS)
      ...
      node = tree_->PreviousInTreeOrder(node, is_android_);
  }
}
