...
class WebContentsAccessibility::Connector : public RenderWidgetHostConnector {
 public:
  Connector(WebContents* web_contents, WebContentsAccessibility* accessibility);
 ...
 private:
  WebContentsAccessibility* const accessibility_;
};

WebContentsAccessibility::Connector::Connector(
    WebContents* web_contents,
    WebContentsAccessibility* accessibility)
    : RenderWidgetHostConnector(web_contents), accessibility_(accessibility) {
  Initialize();
}

void WebContentsAccessibility::Connector::UpdateRenderProcessConnection(
    RenderWidgetHostViewAndroid* old_rwhva,
...
void WebContentsAccessibility::SetAccessibilityEnabled(
    JNIEnv* env,
    const JavaParamRef<jobject>& obj,
    bool enabled) {
...
bool WebContentsAccessibility::ShouldRespectDisplayedPasswordText() {
  JNIEnv* env = AttachCurrentThread();
  ScopedJavaLocalRef<jobject> obj = java_ref_.get(env);
  if (obj.is_null())
    return false;
...
bool WebContentsAccessibility::ShouldExposePasswordText() {
  JNIEnv* env = AttachCurrentThread();
  ScopedJavaLocalRef<jobject> obj = java_ref_.get(env);
  if (obj.is_null())
    return false;
...
jboolean WebContentsAccessibility::IsNodeValid(JNIEnv* env,
                                               const JavaParamRef<jobject>& obj,
                                               jint id) {
  return GetAXFromUniqueID(id) != NULL;
}
...
BrowserAccessibilityAndroid* WebContentsAccessibility::GetAXFromUniqueID(
    int32_t unique_id) {
...
jlong Init(JNIEnv* env,
  ...
  return reinterpret_cast<intptr_t>(
      new WebContentsAccessibility(env, obj, web_contents));
}
