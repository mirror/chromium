...
// by BrowserAccessibilityManagerAndroid for main frame through the interface
// WebContentsAccesibility::Delegate only. The others for subframes should
// acquire this instance through the root manager to access Java layer.
//
 ...
 public:
  ...
  class Delegate {
   public:
    ...
    virtual BrowserAccessibility* GetRootNode() = 0;
    virtual BrowserAccessibility* GetFocusNode() = 0;
    virtual void SetFocusNode(const BrowserAccessibility& node) = 0;
    virtual void HitTestNode(const gfx::Point& point) = 0;
    // Following methods compute the next start and end index when moving
    // forwards or backwards by character, word, or line. Both of these
    ...
    virtual bool NextAtGranularity(int32_t granularity,
                                   int32_t cursor_index,
                                   BrowserAccessibilityAndroid* node,
                                   int32_t* start_index,
                                   int32_t* end_index) = 0;
    virtual bool PreviousAtGranularity(int32_t granularity,
                                       int32_t cursor_index,
                                       BrowserAccessibilityAndroid* node,
                                       int32_t* start_index,
                                       int32_t* end_index) = 0;
