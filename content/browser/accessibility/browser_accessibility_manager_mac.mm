...
BrowserAccessibility* BrowserAccessibilityManagerMac::GetFocus() {
  ...
  if (focus &&
      (focus->GetRole() == ax::mojom::Role::LIST_BOX ||
...
void BrowserAccessibilityManagerMac::FireGeneratedEvent(
  ...
  switch (event_type) {
    case Event::ACTIVE_DESCENDANT_CHANGED:
      ...
      } else if (node->GetRole() ==
                 ax::mojom::Role::TEXT_FIELD_WITH_COMBO_BOX) {
    ...
    case Event::LOAD_COMPLETE:
      ...
      // Iframes should use |ax::mojom::Event::LAYOUT_COMPLETE| to signify that
