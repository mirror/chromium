...
BrowserAccessibility* FindNodeWithChildTreeId(BrowserAccessibility* node,
  ...
  if (node->GetIntAttribute(ax::mojom::IntAttribute::CHILD_TREE_ID) == ax_tree_id)
    return node;
...
void BrowserAccessibilityManager::FireFocusEventsIfNeeded() {
  ...
  if (focus && focus == focus->manager()->GetRoot() &&
      ...
      !focus->GetBoolAttribute(ax::mojom::BoolAttribute::BUSY) &&
  ...
  if (parent_node_id_from_parent_tree_) {
    ...
    if (parent_node) {
      int parent_child_tree_id =
          parent_node->GetIntAttribute(ax::mojom::IntAttribute::CHILD_TREE_ID);
...
void BrowserAccessibilityManager::OnAccessibilityEvents(
  ...
  for (uint32_t index = 0; index < details.size(); index++) {
    ...
    if (detail.event_type == ax::mojom::Event::HOVER)
...
void BrowserAccessibilityManager::ActivateFindInPageResult(
  ...
  FireBlinkEvent(ax::mojom::Event::SCROLLED_TO_ANCHOR, node);
}
...
BrowserAccessibility* BrowserAccessibilityManager::GetActiveDescendant(
  ...
  if (focus->GetIntAttribute(ax::mojom::IntAttribute::ACTIVEDESCENDANT_ID,
                             &active_descendant_id)) {
    active_descendant = focus->manager()->GetFromID(active_descendant_id);
  }

  if (focus->GetRole() == ax::mojom::Role::POP_UP_BUTTON) {
    BrowserAccessibility* child = focus->InternalGetChild(0);
    if (child && child->GetRole() == ax::mojom::Role::MENU_LIST_POPUP) {
      ...
      if (child->GetIntAttribute(ax::mojom::IntAttribute::ACTIVEDESCENDANT_ID,
                                 &active_descendant_id)) {
...
BrowserAccessibilityManager::GetFocusFromThisOrDescendantFrame() {
  ...
  if (obj->HasIntAttribute(ax::mojom::IntAttribute::CHILD_TREE_ID)) {
    BrowserAccessibilityManager* child_manager =
        BrowserAccessibilityManager::FromID(
            obj->GetIntAttribute(ax::mojom::IntAttribute::CHILD_TREE_ID));
    if (child_manager)
...
void BrowserAccessibilityManager::SetFocus(const BrowserAccessibility& node) {
  ...
  action_data.action = ax::mojom::Action::FOCUS;
...
void BrowserAccessibilityManager::Decrement(
  ...
  ui::AXActionData action_data;
  action_data.action = ax::mojom::Action::DECREMENT;
  action_data.target_node_id = node.GetId();
  delegate_->AccessibilityPerformAction(action_data);
}
...
void BrowserAccessibilityManager::DoDefaultAction(
  ...
  ui::AXActionData action_data;
  action_data.action = ax::mojom::Action::DO_DEFAULT;
  action_data.target_node_id = node.GetId();
  delegate_->AccessibilityPerformAction(action_data);
}
...
void BrowserAccessibilityManager::GetImageData(
  ...
  ui::AXActionData action_data;
  action_data.action = ax::mojom::Action::GET_IMAGE_DATA;
  action_data.target_node_id = node.GetId();
...
void BrowserAccessibilityManager::Increment(
  ...
  ui::AXActionData action_data;
  action_data.action = ax::mojom::Action::INCREMENT;
  action_data.target_node_id = node.GetId();
  delegate_->AccessibilityPerformAction(action_data);
}
...
void BrowserAccessibilityManager::ShowContextMenu(
  ...
  ui::AXActionData action_data;
  action_data.action = ax::mojom::Action::SHOW_CONTEXT_MENU;
  action_data.target_node_id = node.GetId();
  delegate_->AccessibilityPerformAction(action_data);
}
...
void BrowserAccessibilityManager::ScrollToMakeVisible(
  ...
  ui::AXActionData action_data;
  action_data.target_node_id = node.GetId();
  action_data.action = ax::mojom::Action::SCROLL_TO_MAKE_VISIBLE;
...
void BrowserAccessibilityManager::ScrollToPoint(
  ...
  ui::AXActionData action_data;
  action_data.target_node_id = node.GetId();
  action_data.action = ax::mojom::Action::SCROLL_TO_POINT;
...
void BrowserAccessibilityManager::SetScrollOffset(
  ...
  ui::AXActionData action_data;
  action_data.target_node_id = node.GetId();
  action_data.action = ax::mojom::Action::SET_SCROLL_OFFSET;
...
void BrowserAccessibilityManager::SetValue(
  ...
  ui::AXActionData action_data;
  action_data.target_node_id = node.GetId();
  action_data.action = ax::mojom::Action::SET_VALUE;
...
void BrowserAccessibilityManager::LoadInlineTextBoxes(
  ...
  ui::AXActionData action_data;
  action_data.action = ax::mojom::Action::LOAD_INLINE_TEXT_BOXES;
  action_data.target_node_id = node.GetId();
  delegate_->AccessibilityPerformAction(action_data);
}
...
void BrowserAccessibilityManager::HitTest(const gfx::Point& point) {
  ...
  action_data.target_point = point;
  action_data.hit_test_event_to_fire = ax::mojom::Event::HOVER;
  delegate_->AccessibilityPerformAction(action_data);
}
...
BrowserAccessibility* BrowserAccessibilityManager::PreviousInTreeOrder(
  ...
  if (can_wrap_to_last_element &&
      object->GetRole() == ax::mojom::Role::ROOT_WEB_AREA &&
