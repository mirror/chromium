...
class FakeAmbientLightSensor : public device::mojom::Sensor {
 public:
  FakeAmbientLightSensor(device::mojom::SensorRequest request)
 ...
 private:
  mojo::Binding<device::mojom::Sensor> binding_;
...
class FakeSensorProvider : public device::mojom::SensorProvider {
  ...
  void GetSensor(device::mojom::SensorType type,
    ...
    switch (type) {
      case device::mojom::SensorType::AMBIENT_LIGHT: {
        sensor_ =
            base::MakeUnique<FakeAmbientLightSensor>(std::move(sensor_request));

        auto init_params = device::mojom::SensorInitParams::New();
        init_params->memory = sensor_->GetSharedBufferHandle();
        init_params->buffer_offset = sensor_->GetBufferOffset();
        init_params->default_configuration = sensor_->GetDefaultConfiguration();
        init_params->maximum_frequency =
            sensor_->GetMaximumSupportedFrequency();
        init_params->minimum_frequency =
            sensor_->GetMinimumSupportedFrequency();

        std::move(callback).Run(std::move(init_params), sensor_->GetClient());
        break;
      }
      default:
        NOTIMPLEMENTED();
 ...
 private:
  std::unique_ptr<FakeAmbientLightSensor> sensor_;
