...
bool GpuDataManagerImplPrivate::IsDriverBugWorkaroundActive(int feature) const {
  // TODO(zmo): move this feature to be part of GPUInfo, so we can get rid of
  // this API entirely.
  DCHECK_EQ(gpu::DONT_DISABLE_WEBGL_WHEN_COMPOSITOR_CONTEXT_LOST, feature);
...
GpuDataManagerImplPrivate::GetDriverBugWorkarounds() const {
  ...
  for (auto entry : gpu_feature_info_.applied_gpu_driver_bug_list_entries) {
    workarounds.push_back(gpu::GpuDriverBugWorkaroundTypeToString(
        static_cast<gpu::GpuDriverBugWorkaroundType>(entry)));
  }
  return workarounds;
}
