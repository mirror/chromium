...
class SlowGpuThread : public InProcessGpuThread {
 public:
  ...
  SlowGpuThread(const InProcessChildThreadParams& params,
    ...
    unpause = new WaitableEvent(WaitableEvent::ResetPolicy::MANUAL,
 ...
 protected:
  void Init() override {
    // This GPU thread is stalled from its very beginning until its destruction.
    // This simulates an extreme but possible execution order.
    unpause->Wait();
    delete unpause;
