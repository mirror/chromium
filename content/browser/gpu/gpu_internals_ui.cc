...
std::unique_ptr<base::ListValue> displayInfo() {
  auto edid_info = base::MakeUnique<base::ListValue>();
  const std::vector<display::Display> displays =
      display::Screen::GetScreen()->GetAllDisplays();
  for (const auto& display : displays) {
    edid_info->Append(NewDescriptionValuePair("Info ", display.ToString()));
    edid_info->Append(NewDescriptionValuePair(
...
void GpuMessageHandler::OnGpuInfoUpdate() {
  ...
  gpu_info_val->Set("displayInfo", displayInfo());
