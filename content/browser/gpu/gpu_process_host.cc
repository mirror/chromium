...
void GpuProcessHost::RequestGPUInfo(RequestCallback request_cb) {
  if (initialize_succeeded_ || initialize_failed_)
...
void GpuProcessHost::DidInitialize(
  ...
  if (!request_cb_.is_null())
    std::move(request_cb_).Run(GpuDataManagerImpl::GetInstance()->GetGPUInfo());
}
