...
void OriginManifestResourceHandler::OnRequestRedirected(
  ...
  /*
    ...
    if (!response->head.headers->EnumerateHeader(nullptr, "Sec-Origin-Manifest",
                                                 &version)) {
      // There's no header. But we can still try to work with what we have
    already store_->Get(origin_, base::BindOnce(
                      &OriginManifestResourceHandler::OnRequestRedirectedContinue,
                      base::Unretained(this)));
      return;
    }
    ...
    store_->GetOrFetch(origin_, version,
        base::BindOnce(
            &OriginManifestResourceHandler::OnRequestRedirectedContinue,
            base::Unretained(this)));
  */
...
void OriginManifestResourceHandler::ProcessOriginManifestHeaderValue(
    base::OnceCallback<void(origin_manifest::mojom::OriginManifestPtr)>
        callback) {
  DCHECK(response_);
