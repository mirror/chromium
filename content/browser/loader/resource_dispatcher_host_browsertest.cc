...
class PreviewsStateResourceDispatcherHostDelegate
 ...
 public:
  ...
  void RequestBeginning(
    ...
    EXPECT_TRUE(previews_state_ == info->GetPreviewsState() ||
                adjusted_previews_state_ == info->GetPreviewsState());
  }
...
IN_PROC_BROWSER_TEST_F(PreviewsStateResourceDispatcherHostBrowserTest,
  ...
  // override should catch the preveiws state, and set the member variable, so
  // we can check it here.  Note that we don't call CheckResourcesRequested,
  // which would not have seen what it expected because we changed the previews
  // state.
