...
IN_PROC_BROWSER_TEST_F(RequestDataResourceDispatcherHostBrowserTest,
  ...
  EXPECT_EQ(img_url, delegate_->data()[1]->url);
  EXPECT_EQ(top_origin, delegate_->data()[1]->initiator);
  // Respect the "origin" policy set by the <meta> tag.
  EXPECT_EQ(top_url.GetOrigin().spec(), delegate_->data()[1]->referrer);
  EXPECT_TRUE(delegate_->data()[1]->load_flags & net::LOAD_PREFETCH);
}
...
IN_PROC_BROWSER_TEST_F(RequestDataResourceDispatcherHostBrowserTest,
  ...
  // document, so there are no |initiator|.
  EXPECT_EQ(top_url, delegate_->data()[0]->url);
  EXPECT_EQ(top_url, delegate_->data()[0]->first_party);
  EXPECT_FALSE(delegate_->data()[0]->initiator.has_value());
...
IN_PROC_BROWSER_TEST_F(RequestDataResourceDispatcherHostBrowserTest,
  ...
  // initiated outside of a document, so there are no |initiator|.
  EXPECT_EQ(top_url, delegate_->data()[0]->url);
  EXPECT_EQ(top_url, delegate_->data()[0]->first_party);
  EXPECT_FALSE(delegate_->data()[0]->initiator.has_value());
...
IN_PROC_BROWSER_TEST_F(RequestDataResourceDispatcherHostBrowserTest,
  ...
  // User-initiated top-level navigations have a first-party that matches the
  // URL to which they navigate, even if they fail to load. The navigation was
  // initiated from a user outside of a document, so there are no initiator.
  EXPECT_EQ(top_url, delegate_->data()[0]->url);
  EXPECT_EQ(top_url, delegate_->data()[0]->first_party);
  EXPECT_FALSE(delegate_->data()[0]->initiator.has_value());
...
IN_PROC_BROWSER_TEST_F(RequestDataResourceDispatcherHostBrowserTest,
  ...
  // was initiated from a user outside of a document, so there are no
  // initiator.
  EXPECT_EQ(top_url, delegate_->data()[0]->url);
  EXPECT_EQ(top_url, delegate_->data()[0]->first_party);
  EXPECT_FALSE(delegate_->data()[0]->initiator.has_value());
