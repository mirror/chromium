...
class WebPackageReaderAdapter
 ...
 private:
  ...
  class PendingResource {
   public:
    explicit PendingResource(ResourceResponseHead response_head);
    ~PendingResource();
    PendingResource(PendingResource&& other);
    PendingResource& operator=(PendingResource&& other);

    mojo::ScopedDataPipeConsumerHandle StartResponseBody();

    void OnDataReceived(const void* data, size_t size);
    void OnNotifyFinished(int error_code);

   private:
    void OnWriteResponseBody();
    void OnResponseBodyStreamReady();
    ...
    // Will be null during write operations, when |pending_write_| owns the
    // pipe.
    mojo::ScopedDataPipeProducerHandle producer_handle_;

    // Taken when StartResponseBody() is called.
    mojo::ScopedDataPipeConsumerHandle consumer_handle_;

    // This owns the data pipe during write operations.
    scoped_refptr<network::NetToMojoPendingBuffer> pending_write_;

    std::deque<scoped_refptr<net::IOBufferWithSize>> read_buffers_;
    int read_buffer_offset_;

    std::vector<ResourceCallback> pending_callbacks_;
    std::vector<CompletionCallback> pending_completion_callbacks_;
  };
  ...
  void OnFirstResponse(ResourceCallback callback,
                       CompletionCallback completion_callback);
  ...
  std::map<std::pair<GURL, std::string /* method */>, int /* request_id */>
      request_map_;
  std::map<int /* request_id */, PendingResource> response_map_;

  base::OnceClosure waiting_first_response_;
