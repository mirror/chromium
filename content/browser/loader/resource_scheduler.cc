...
// Wrapper that returns the net::EffectiveConnectionType that corresponds to
...
size_t ResourceScheduler::MaxDelayableRequestsNetworkOverrideExperiment::
  ...
  if (network_quality_estimator->GetEffectiveConnectionType() <=
    ...
    if (bandwidth_delay_product) {
      return GetMaxDelayableRequestsForBDP(bandwidth_delay_product.value());
    }
  }
  return kDefaultMaxNumDelayableRequestsPerClient;
}

int ResourceScheduler::MaxDelayableRequestsNetworkOverrideExperiment::
    GetMaxDelayableRequestsForBDP(int64_t bdp_in_kbits) const {
