...
size_t ResourceScheduler::ThrottleDelayble::GetMaxDelayableRequests(
  ...
  for (const auto& range : params_for_network_quality_ranges_) {
    if (ect == range.effective_connection_type) {
...
ResourceScheduler::ThrottleDelayble::GetParamsForNetworkQualityRanges() {
  ...
  const char ect_base[] = "EffectiveConnectionType";
  ...
  while (true) {
    ...
    base::Optional<net::EffectiveConnectionType> ect =
        net::GetEffectiveConnectionTypeForName(
            base::GetFieldTrialParamValueByFeature(
                kThrottleDelayble,
                ect_base + base::IntToString(config_param_index)));
    if (!ect)
      return result;
    ...
    result.push_back(
        {ect.value(), max_delayable_requests, non_delayable_weight});
    config_param_index++;
  }
}
