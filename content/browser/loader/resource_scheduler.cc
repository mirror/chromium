...
class ResourceScheduler::Client {
 ...
 private:
  ...
  int GetNumberOfDelayableRequestsForBDP(int64_t bdp_kbits) const {
    for (auto& range : *max_delayable_requests_by_bdp_ranges_) {
      if (range.bdp_lower_bound_kbits <= bdp_kbits &&
          range.bdp_upper_bound_kbits >= bdp_kbits) {
  ...
  ShouldStartReqResult ShouldStartRequest(
    ...
    // Delayable requests.
    if (max_delayable_requests_override_enabled_) {
      DCHECK(max_delayable_requests_overridden_count_);
