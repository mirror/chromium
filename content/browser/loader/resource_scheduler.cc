...
BDPRangeRequestCounts GetMaxDelayableRequestsExperimentConfig() {
  ...
  while (true) {
    ...
    result.push_back(MaxRequestsForBDPRange(
...
class ResourceScheduler::Client {
 public:
  Client(bool priority_requests_delayable,
         ...
         const BDPRangeRequestCounts* const max_delayable_requests_by_bdp,
      ...
      : is_loaded_(false),
        ...
        max_delayable_requests_by_bdp_(max_delayable_requests_by_bdp),
        max_delayable_requests_threshold_(max_delayable_requests_threshold),
        weak_ptr_factory_(this) {}

  ~Client() {}

  // For the given URL request, this function computes the maximum number of
  // delayable requests to allow based on the configuration of the experiment
 ...
 private:
  ...
  int GetNumberOfDelayableRequestsForBDP(int64_t bdp_in_kbits) const {
    DCHECK_LE(max_delayable_requests_by_bdp_->size(), 20u);

    for (auto& range : *max_delayable_requests_by_bdp_) {
      if (range.min_bdp_kbits <= bdp_in_kbits &&
          range.max_bdp_kbits > bdp_in_kbits) {
  ...
  // each range.
  const BDPRangeRequestCounts* const max_delayable_requests_by_bdp_;
...
ResourceScheduler::ResourceScheduler()
    : priority_requests_delayable_(
      ...
      max_delayable_requests_by_bdp_(GetMaxDelayableRequestsExperimentConfig()),
      max_delayable_requests_threshold_(
...
void ResourceScheduler::OnClientCreated(int child_id,
  ...
  Client* client = new Client(
      ...
      &max_delayable_requests_by_bdp_, max_delayable_requests_threshold_);
