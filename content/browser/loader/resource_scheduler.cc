...
class ResourceScheduler::ScheduledResourceRequest : public ResourceThrottle {
 public:
  ScheduledResourceRequest(const ClientId& client_id,
      ...
      : client_id_(client_id),
        ...
        started_immediately_on_schedule_(false),
  ...
  bool started_immediately_on_schedule() const {
    return started_immediately_on_schedule_;
  }
  void set_started_immediately_on_schedule() {
    started_immediately_on_schedule_ = true;
  }
 ...
 private:
  ...
  // Whether or not the request was started immediately after scheduling.
  bool started_immediately_on_schedule_;
 ...
 public:
  ...
  void ScheduleRequest(net::URLRequest* url_request,
    ...
    if (should_start == START_REQUEST) {
      ...
      request->set_started_immediately_on_schedule();
 ...
 private:
  ...
  void StartRequest(ScheduledResourceRequest* request,
    ...
    if (!RequestAttributesAreSet(request->attributes(), kAttributeDelayable)) {
      ...
      // delayable requests in-flight which were started immediately on
      // scheduling. Don't iterate over more than 50 requests to avoid too
      // much overhead. This will cover about 99 percent of the cases.
      int max_requests_to_scan = 50;
      for (const ScheduledResourceRequest* in_flight_request :
           in_flight_requests_) {
        ...
        if (RequestAttributesAreSet(in_flight_request->attributes(),
                                    kAttributeDelayable) &&
            in_flight_request->started_immediately_on_schedule()) {
          UMA_HISTOGRAM_TIMES(
