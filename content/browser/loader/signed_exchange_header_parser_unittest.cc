...
TEST_F(SignedExchangeHeaderParserTest, ParseSignature) {
  ...
  EXPECT_EQ(signatures[1].label, "srisig");
  EXPECT_EQ(signatures[1].sig,
            std::string(reinterpret_cast<const char*>(decoded_sig2),
                        sizeof(decoded_sig2)));
  EXPECT_EQ(signatures[1].integrity, "mi");
  EXPECT_EQ(signatures[1].validityUrl,
            "https://example.com/resource.validity.1511128380");
  EXPECT_EQ(signatures[1].ed25519Key,
            std::string(reinterpret_cast<const char*>(decoded_ed25519Key),
                        sizeof(decoded_ed25519Key)));
  EXPECT_EQ(signatures[1].date, 1511128380ul);
  EXPECT_EQ(signatures[1].expires, 1511733180ul);
}
