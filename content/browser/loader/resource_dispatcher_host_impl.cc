...
bool IsValidPluginChildId(int plugin_child_id,
  ...
  if (plugin_host) {
    ...
    if (process_type == PROCESS_TYPE_PPAPI_PLUGIN) {
      ...
      // |embedding_render_process_id|. However, |plugin_child_host| is
      // currently only used for network usage monitoring, it seems sufficient
      // to enforce that it's a valid plugin.
      return true;
    }
    if (process_type >= PROCESS_TYPE_CONTENT_END) {
      ...
      if (ppapi_host) {
        ...
        // currently only used for network usage monitoring, it seems sufficient
        // to enforce that it's a valid plugin.
        NOTREACHED() << "The Ppapi host is necessary " << process_type;
        return true;
      }
      NOTREACHED() << "process type big but no external host" << process_type;
      return false;
    }
    NOTREACHED() << "Valid BrowserChildProcessHost but unexpected process_type "
                 << process_type;
    return false;
  }
#endif

  NOTREACHED() << "null plugin_host" << plugin_child_id;
...
bool ResourceDispatcherHostImpl::ShouldServiceRequest(
  ...
  if (!IsValidPluginChildId(request_data.plugin_child_id, child_id,
                            request_data.render_frame_id)) {
    NOTREACHED() << request_data.plugin_child_id << " " << child_id << " "
                 << request_data.render_frame_id;
    return false;
  }
