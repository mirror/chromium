...
// TODO(creis): Add support or omit for now.  Not used yet.  Maybe plugins?
...
// a test parameter to the ResponseBlocking test.
struct TestScenario {
...
// Stream operator to let the GetParam() print show a useful result if the tests
// fail.
::std::ostream& operator<<(::std::ostream& os, const TestScenario& scenario) {
  std::string cors_response;
  switch (scenario.cors_response) {
    case AccessControlAllowOriginHeader::OMIT:
      cors_response = "AccessControlAllowOriginHeader::OMIT";
      break;
    case AccessControlAllowOriginHeader::ALLOW_ANY:
      cors_response = "AccessControlAllowOriginHeader::ALLOW_ANY";
      break;
    case AccessControlAllowOriginHeader::ALLOW_NULL:
      cors_response = "AccessControlAllowOriginHeader::ALLOW_NULL";
      break;
    case AccessControlAllowOriginHeader::ALLOW_INITIATOR_ORIGIN:
      cors_response = "AccessControlAllowOriginHeader::ALLOW_INITIATOR_ORIGIN";
      break;
    case AccessControlAllowOriginHeader::ALLOW_EXAMPLE_DOT_COM:
      cors_response = "AccessControlAllowOriginHeader::ALLOW_EXAMPLE_DOT_COM";
      break;
    default:
      NOTREACHED();
  }
  ...
  switch (scenario.verdict) {
    case Verdict::ALLOW_WITHOUT_SNIFFING:
      verdict = "Verdict::ALLOW_WITHOUT_SNIFFING";
      break;
    case Verdict::BLOCK_WITHOUT_SNIFFING:
      verdict = "Verdict::BLOCK_WITHOUT_SNIFFING";
      break;
    case Verdict::ALLOW_AFTER_SNIFFING:
      verdict = "Verdict::ALLOW_AFTER_SNIFFING";
      break;
    case Verdict::BLOCK_AFTER_SNIFFING:
      verdict = "Verdict::BLOCK_AFTER_SNIFFING";
      break;
    default:
      NOTREACHED();
  }
...
// Tests that verfiy CrossSiteDocumentResourceHandler correctly classifies
...
TEST_P(CrossSiteDocumentResourceHandlerTest, ResponseBlocking) {
  ...
  response->head.mime_type = scenario.response_mime_type;
  scoped_refptr<net::HttpResponseHeaders> response_headers =
      new net::HttpResponseHeaders("");
  // No sniff header.
  if (scenario.include_no_sniff_header)
    response_headers->AddHeader("X-Content-Type-Options: nosniff");
  // CORS header.
  if (scenario.cors_response == AccessControlAllowOriginHeader::ALLOW_ANY) {
    response_headers->AddHeader("Access-Control-Allow-Origin: *");
  } else if (scenario.cors_response ==
             AccessControlAllowOriginHeader::ALLOW_INITIATOR_ORIGIN) {
    response_headers->AddHeader(base::StringPrintf(
        "Access-Control-Allow-Origin: %s", scenario.initiator_origin));
  } else if (scenario.cors_response ==
             AccessControlAllowOriginHeader::ALLOW_NULL) {
    response_headers->AddHeader("Access-Control-Allow-Origin: null");
  } else if (scenario.cors_response ==
             AccessControlAllowOriginHeader::ALLOW_EXAMPLE_DOT_COM) {
  ...
  bool expected_to_block_based_on_headers =
      scenario.verdict == Verdict::BLOCK_WITHOUT_SNIFFING ||
      scenario.verdict == Verdict::BLOCK_AFTER_SNIFFING ||
  ...
  // Tell the ResourceHandlers to allocate the buffer for reading.
  ...
  // it in the test.
  bool expected_to_sniff = scenario.verdict == Verdict::ALLOW_AFTER_SNIFFING ||
                           scenario.verdict == Verdict::BLOCK_AFTER_SNIFFING ||
                           scenario.verdict == Verdict::BLOCK_WITHOUT_SNIFFING;
  ...
  if (scenario.verdict == Verdict::BLOCK_WITHOUT_SNIFFING ||
      scenario.verdict == Verdict::BLOCK_AFTER_SNIFFING) {
    ...
    if (scenario.verdict == Verdict::ALLOW_AFTER_SNIFFING)
...
const TestScenario kScenarios[] = {
    ...
    },
    {
        ...
        RESOURCE_TYPE_XHR,                   // resource_type
        "http://www.a.com/",                 // initiator_origin
        OriginHeader::INCLUDE,               // cors_request
        "text/html",                         // response_mime_type
        CROSS_SITE_DOCUMENT_MIME_TYPE_HTML,  // canonical_mime_type
        false,                               // include_no_sniff_header
        AccessControlAllowOriginHeader::
            ALLOW_INITIATOR_ORIGIN,               // cors_response
        "<html><head>this should sniff as HTML",  // first_chunk
        Verdict::ALLOW_WITHOUT_SNIFFING,          // verdict
    },
    {
        ...
        false,                                      // include_no_sniff_header
        AccessControlAllowOriginHeader::ALLOW_ANY,  // cors_response
    ...
    },
    {
        ...
        false,                                       // include_no_sniff_header
        AccessControlAllowOriginHeader::ALLOW_NULL,  // cors_response
        "{\"x\" : 3}",                               // first_chunk
        Verdict::ALLOW_WITHOUT_SNIFFING,             // verdict
    },
    {
        "Allowed: Cross-site XHR to HTML over FTP without CORS", __LINE__,
    ...
    // Allowed responses due to sniffing:
    {
        ...
        AccessControlAllowOriginHeader::OMIT,  // cors_response
        "foo({\"x\" : 3})",                    // first_chunk
        Verdict::ALLOW_AFTER_SNIFFING,         // verdict
    },
    {
    ...
    // Blocked responses:
    {
        ...
        AccessControlAllowOriginHeader::OMIT,     // cors_response
        "<html><head>this should sniff as HTML",  // first_chunk
        Verdict::BLOCK_AFTER_SNIFFING,            // verdict
    },
    {
    ...
    },
    {
        ...
        AccessControlAllowOriginHeader::OMIT,     // cors_response
        "<html><head>this should sniff as HTML",  // first_chunk
        Verdict::BLOCK_WITHOUT_SNIFFING,          // verdict
    },
    {
    ...
    },
    {
        ...
        CROSS_SITE_DOCUMENT_MIME_TYPE_HTML,  // canonical_mime_type
        false,                               // include_no_sniff_header
        AccessControlAllowOriginHeader::ALLOW_EXAMPLE_DOT_COM,  // cors_response
        "<html><head>this should sniff as HTML",                // first_chunk
        Verdict::BLOCK_AFTER_SNIFFING,                          // verdict
    },
};
