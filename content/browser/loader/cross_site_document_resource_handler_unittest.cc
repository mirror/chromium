...
// passed to the ResponseBlocking test below as test parameters.
...
TEST_P(CrossSiteDocumentResourceHandlerTest, ResponseBlocking) {
  ...
  // CORS header.
  if (scenario.cors_response == AccessControlAllowOriginHeader::kAllowAny) {
    ...
    response_headers->AddHeader(base::StringPrintf(
        "Access-Control-Allow-Origin: %s", scenario.initiator_origin));
             ...
             AccessControlAllowOriginHeader::kAllowNull) {
