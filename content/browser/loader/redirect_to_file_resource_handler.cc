...
RedirectToFileResourceHandler::RedirectToFileResourceHandler(
    ...
    net::URLRequest* request)
    : LayeredResourceHandler(request, std::move(next_handler)),
      create_temporary_file_stream_(std::move(temp_stream_function)),
      buf_(new net::GrowableIOBuffer()),
      buf_write_pending_(false),
      write_cursor_(0),
      writer_(NULL),
...
void RedirectToFileResourceHandler::OnWillStart(
  ...
  // Kick off temporary file creation ASAP but don't block.
...
void RedirectToFileResourceHandler::OnResponseCompleted(
  ...
  int net_error = OnResponseCompletedHook();
  if (net_error < 0) {
    DCHECK_NE(net::ERR_IO_PENDING, net_error);
    next_handler_->OnResponseCompleted(
        net::URLRequestStatus(net::URLRequestStatus::CANCELED, net_error),
        std::move(controller));
    return;
  }
...
void RedirectToFileResourceHandler::DidCreateTemporaryFile(
  ...
  // Resume the request.
  if (pending_temp_file_creation_) {
...
bool RedirectToFileResourceHandler::WriteMore() {
  if (!writer_) {
    LOG(ERROR) << "waiting for writer";
    pending_temp_file_creation_ = true;
    return true;
  }
