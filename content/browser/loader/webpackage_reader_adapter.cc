...
class PushObserverCallbackWrapper {
 public:
  ...
  void OnResponse(int error_code,
    ...
    observer_->OnResponse(request_url_, method_, response_head, std::move(body),
...
void WebPackageReaderAdapter::GetFirstResource(
  ...
  if (state_ < kResponseStarted) {
    ...
    LOG(INFO) << "- Waiting for the first request.";
    return;
  }
  ...
  found->second->RegisterCallbacks(std::move(callback),
                                   std::move(completion_callback));
  LOG(INFO) << "- Added callback on the response_handler_.";
}
...
void WebPackageReaderAdapter::GetResource(
    ...
    ResourceCallback callback,
    CompletionCallback completion_callback) {
  LOG(ERROR) << "** GetResource: [" << resource_request.url.ExtractFileName()
             << "]";
  DCHECK_EQ(kPull, fetch_mode_);
  ...
  if (request_found != request_map_.end()) {
    ...
    if (response_found != response_map_.end()) {
      ...
      handler->RegisterCallbacks(std::move(callback),
                                 std::move(completion_callback));
      return;
    }
  }
...
void WebPackageReaderAdapter::StartPushResources(
  ...
  for (auto& request : request_map_) {
    if (pending_request_map_.find(request.first) != pending_request_map_.end())
      continue;
    push_observer_->OnRequest(request.first.first, request.first.second);
  }
  ...
  for (auto& response : response_map_) {
    ...
    if (handler->has_callback())
      continue;
    LOG(INFO) << "** Pushing: " << handler->request_url();
...
void WebPackageReaderAdapter::OnResourceRequest(
  ...
  LOG(INFO) << "** Reading request: " << request.first;
...
void WebPackageReaderAdapter::OnResourceResponse(
  ...
  LOG(INFO) << "** Reading response: " << request.first;
  ...
  if (pending_found != pending_request_map_.end()) {
    LOG(INFO) << "** pending request -- found: " << request.first;
    handler->RegisterCallbacks(
  ...
  if (pending_first_request_ && request_id == first_request_id_) {
    LOG(INFO) << "** first request -- found: " << request.first;
    handler->RegisterCallbacks(
