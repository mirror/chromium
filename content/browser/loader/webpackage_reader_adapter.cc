...
WebPackageResponseHandler::WebPackageResponseHandler(
  ...
  std::string type;
  size_t iter = 0;
  while (resposne_head_->headers->EnumerateHeader(
...
void WebPackageResponseHandler::OnDataReceived(const void* data, size_t size) {
  ...
  if (producer_handle_.is_valid()) {
    ...
    if (available > 0u && !data_write_finished_) {
      ...
      WriteToPipe(dest_buffer, data, bytes_to_write);
      LOG(ERROR) << "- writing to the pipe: " << bytes_to_write;
...
void WebPackageResponseHandler::OnPipeWritable(MojoResult unused) {
  ...
  while (!read_buffers_.empty()) {
    ...
    WriteToPipe(
        static_cast<char*>(dest_buffer) + num_bytes_written,
        src->data(), bytes_to_write);
    src->DidConsume(bytes_to_write);
    LOG(ERROR) << "- wrote " << bytes_to_write;
...
void WebPackageResponseHandler::WriteToPipe(
