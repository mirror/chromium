...
class WebPackageSourceStream : public net::SourceStream {
 public:
  ...
  int Read(net::IOBuffer* dest_buffer,
    ...
    LOG(ERROR) << "[MEMCPY] " << size << "(" << buffer_size << " / " << src_size << ")";
...
void WebPackageResponseHandler::OnPipeWritable(MojoResult unused) {
  ...
  size_t written = 0;
  while (dest_buffer_size - written) {
    auto& src = read_buffers_.front();
    size_t src_size = base::checked_cast<size_t>(src->BytesRemaining());
    size_t dest_size = dest_buffer_size - written;
    LOG(ERROR) << "- remaining " << src_size << " / available " << dest_size;
    WriteToPipe(
        pending_write.get(), dest_offset, &dest_size, src->data(), &src_size);
    src->DidConsume(src_size);
    dest_offset += dest_size;
    LOG(ERROR) << "- consumed " << src_size << " / written " << dest_size;
    if (src->BytesRemaining() == 0)
      read_buffers_.pop_front();
    if (read_buffers_.empty() || dest_size == 0)
      break;
  }
...
int WebPackageResponseHandler::WriteToPipe(
  ...
  while (true) {
    ...
    LOG(ERROR) << "** rv: " << rv << "/" << consumed_read_size_;
    current_read_pointer_ = nullptr;
    if (rv < 0) {
      LOG(ERROR) << "ERROR: " << request_url_ << ": " << rv;
      return rv;
    }
