...
net::Error MojoPipeWriter::OnWillRead(scoped_refptr<net::IOBuffer>* buf,
                                      int* buf_size) {
...
net::Error MojoPipeWriter::OnReadCompleted(int bytes_read) {
...
void MojoPipeWriter::OnWritable(MojoResult result) {
  DCHECK(!did_defer_on_will_read_ || !did_defer_on_writing_);
  if (did_defer_on_will_read_) {
    ...
    net::Error result = OnWillRead(parent_buffer, parent_buffer_size);
