...
class MojoAsyncResourceHandler::InliningHelper {
 public:
  ...
  void OnResponseReceived(const network::ResourceResponse& response,
 ...
 private:
  ...
  InliningStatus IsInliningApplicable(const network::ResourceResponse& response,
                                      uint32_t url_loader_options) {
    if (!(url_loader_options &
