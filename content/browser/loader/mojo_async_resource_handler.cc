...
void MojoAsyncResourceHandler::OnResponseCompleted(
  ...
  if (IsResourceTypeFrame(resource_type_)) {
    // If |OnResponseCompleted| is received before |OnResponseStarted|, it means
    // there is an error. The URLLoaderClient still lives in the browser process
    // and the SSLInfo must be provided to it.
    // In the second case, the URLLoaderClient now lives in the renderer
    // process. The SSLInfo must not be transmitted to a (potentially)
