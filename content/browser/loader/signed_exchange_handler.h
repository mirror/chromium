...
class SignedExchangeHandler final
 ...
 public:
  ...
  // TODO(803774): Consider making SignedExchangeHandler independent from
  // DataPipe to make it easy to port it in //net.
  explicit SignedExchangeHandler(mojo::ScopedDataPipeConsumerHandle body);

  ~SignedExchangeHandler() override;

  // TODO(803774): Currently SignedExchangeHandler always calls found_callback
  // and then calls finish_callback after reading the all buffer. Need to
  // redesign this callback model when we will introduce SignedExchangeHandler::
  // Reader class to read the body and introduce the cert verification.
