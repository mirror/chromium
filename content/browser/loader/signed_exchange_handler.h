...
class SignedExchangeHandler final : public net::FilterSourceStream {
 ...
 private:
  enum ReadState { kHeaders, kBody };
  ReadState read_state_ = kHeaders;

  void DoHeaderLoop();
  void DidReadForHeaders(int result);
  ...
  // Internal IOBuffer used during reading the header.
  // Note that during parsing the header we don't really
  // need the output buffer, but we still need to give
  // some > 0 buffer.
  scoped_refptr<net::IOBufferWithSize> header_out_buf_;

  // TODO(https://crbug.cxom/803774): Just for now. Implement
