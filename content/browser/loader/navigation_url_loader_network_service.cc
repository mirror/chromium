 ...
 private:
  ...
  void OnComplete(
    ...
    if (net::IsCertificateError(completion_status.error_code)) {
      net::SSLInfo ssl_info_copy = completion_status.ssl_info;
      ssl_info = base::make_optional<net::SSLInfo>(std::move(ssl_info_copy));
      fatal_cert_error = base::make_optional<bool>(ShouldSSLErrorsBeFatal());
    }

    BrowserThread::PostTask(
        BrowserThread::UI, FROM_HERE,
        base::BindOnce(&NavigationURLLoaderNetworkService::OnComplete, owner_,
                       completion_status, ssl_info, fatal_cert_error));
  }
