...
void NavigationURLLoaderNetworkService::OnReceiveResponse(
  ...
  // TODO(arthursonzogni): In NavigationMojoResponseOverMojo, this is false and
  // we need to use the info provided by the network stack where it is computed
  // by the MimeSniffingResourceHandler.
  DCHECK(response);
  ...
  delegate_->OnResponseStarted(
      ...
      std::move(ssl_status), std::unique_ptr<NavigationData>(),
