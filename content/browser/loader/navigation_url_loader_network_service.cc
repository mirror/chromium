...
class WebPackageRequestHandler : public URLLoaderRequestHandler {
 public:
  using ExchangeFoundCallback =
      base::OnceCallback<void(const GURL& /* request url */,
  ...
  class SignedExchangeHandler : public mojo::common::DataPipeDrainer::Client {
   ...
   private:
    ...
    void OnDataComplete() override {
      ...
      std::string payload;
      std::tie(new_request_url, payload) = ParseHTTPExchane();
      pending_data_.clear();
      ...
      data_producer_->Write(
          payload, base::BindOnce(&SignedExchangeHandler::OnDataWritten,
                                  base::Unretained(this)));
    }

    std::tuple<GURL, std::string> ParseHTTPExchane() {
      GURL url;
      std::string payload;
      ...
      std::unique_ptr<base::Value> root = base::JSONReader::ReadAndReturnError(
          pending_data_, base::JSON_PARSE_RFC, &error_code, &error_message);
