...
class NavigationURLLoaderNetworkService::URLLoaderRequestController
 ...
 public:
  ...
  void CreateNonNetworkServiceURLLoader(
    ...
    // create an URLLoader. When it doesn't, do not send OnRequestStarted().
 ...
 private:
  ...
  // Not reached. At this point, the loader and client endpoints must have been
  // unbound and forwarded to the renderer.
  ...
  // Set to true when URLLoaderClient::OnComplete() is called.
...
void NavigationURLLoaderNetworkService::OnReceiveResponse(
  ...
  if (!base::FeatureList::IsEnabled(features::kNetworkService)) {
    ...
    is_download = allow_download_ && IsDownload(*response.get());
    is_stream = false;
