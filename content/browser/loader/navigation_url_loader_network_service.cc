...
class NavigationURLLoaderNetworkService::URLLoaderRequestController
 ...
 public:
  ...
  static uint32_t URLLoadOptions(bool is_main_frame) {
  ...
  void CreateNonNetworkServiceURLLoader(
    ...
    if (ResourceDispatcherHostImpl::Get()) {
      ResourceDispatcherHostImpl::Get()->BeginNavigationRequest(
          ...
          appcache_handle_core, URLLoadOptions(request_info->is_main_frame));
    }
  ...
  void MaybeStartLoader(URLLoaderRequestHandler* handler,
    ...
    uint32_t options = URLLoadOptions(resource_request_->resource_type ==
                                      RESOURCE_TYPE_MAIN_FRAME);
    url_loader_ = ThrottlingURLLoader::CreateLoaderAndStart(
        factory,
        GetContentClient()->browser()->CreateURLLoaderThrottles(
