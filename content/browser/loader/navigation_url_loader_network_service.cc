...
class WebPackageRequestHandler : public URLLoaderRequestHandler {
 public:
  ...
  class SignedExchangeHandler : public mojo::common::DataPipeDrainer::Client {
   ...
   private:
    static constexpr size_t kDefaultUrlPipeSize = 65536;
    ...
    void OnExchangeFound() {
      ...
      mojo::DataPipe pipe(kDefaultUrlPipeSize);
      // TODO(horo): Write Error handling. This could fail.
      DCHECK(pipe.consumer_handle.is_valid());
      mojo::ScopedDataPipeConsumerHandle response_body =
          std::move(pipe.consumer_handle);
      std::move(found_callback_)
          .Run(request_url, resource_response, std::move(response_body));
      // TODO(horo): Get the response body by parsing |body_| with an
      // EnvelopeParser for CBOR. Currently SignedExchangeHandler just pipes the
      // data pipe using DataPipeDrainer and StringDataPipeProducer.
    ...
    void MaybeTransferPendingData() {
      ...
      data_producer_->Write(
          pending_data_, base::BindOnce(&SignedExchangeHandler::OnDataWritten,
    ...
    void MaybeCallFinishCallback() {
      if (all_data_read_ && pending_data_.empty() && finish_callback_) {
