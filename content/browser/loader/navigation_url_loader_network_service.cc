...
class WebPackageRequestHandler : public URLLoaderRequestHandler {
 public:
  class WebPackageLoader final : public network::mojom::URLLoaderClient {
   public:
    WebPackageLoader(const network::ResourceResponseHead& response,
                     network::mojom::URLLoaderClientEndpointsPtr endpoints)
        : url_loader_client_binding_(this), weak_factory_(this) {
      LOG(ERROR) << "WebPackageLoader::WebPackageLoader " << this;
    ...
    void OnStartLoadingResponseBody(
      ...
      LOG(ERROR) << "WebPackageLoader::OnStartLoadingResponseBody";
      client_->OnStartLoadingResponseBody(std::move(body));
    }
    void OnComplete(const network::URLLoaderCompletionStatus& status) override {
      LOG(ERROR) << "WebPackageLoader::OnComplete";
      client_->OnComplete(status);
    }
    void ConnectToClient(network::mojom::URLLoaderClientPtr client) {
      LOG(ERROR) << "WebPackageLoader::ConnectToClient";
      mojo::FuseInterface(std::move(pending_request_), client.PassInterface());
    }

   private:
    void OnConnectionClosed() {
      LOG(ERROR) << "WebPackageLoader::OnConnectionClosed";
    }
  ...
  bool MaybeCreateLoaderForResponse(
    ...
    LOG(ERROR) << "WebPackageRequestHandler::MaybeCreateLoaderForResponse";
    ...
    if (mime_type != "application/http-exchange+cbor")
      return false;
    if (!response.headers->GetNormalizedHeader("SignedHTTPExchangeUrlForDemo",
                                               &new_url_string))
      return false;
