...
const net::NetworkTrafficAnnotationTag kCertFetcherTrafficAnnotation =
    ...
    policy {
      cookies_allowed: NO
      setting: "This feature cannot be disabled by settings. This feature is"
        "not enabled by default yet."
      policy_exception_justification: "Not implemented."
    }
    comments:
      "Chrome would be unable to handle Signed HTTP Exchanges without this type"
      " of request."
    )");
...
SignedExchangeCertFetcher::CreateAndStart(
  ...
  std::unique_ptr<SignedExchangeCertFetcher> cert_fetcher(
      new SignedExchangeCertFetcher(cert_url, force_fetch,
                                    std::move(callback)));
  cert_fetcher->Start(std::move(shared_url_loader_factory));
  return cert_fetcher;
}
...
void SignedExchangeCertFetcher::OnReceiveRedirect(
    ...
    const network::ResourceResponseHead& head) {
  // Currently the cert fetcher doesn't support redirection.
  Abort();
}
...
void SignedExchangeCertFetcher::OnDataAvailable(const void* data,
  ...
  body_string_.append(static_cast<const char*>(data), num_bytes);
}
...
void SignedExchangeCertFetcher::OnDataComplete() {
  ...
  std::move(callback_).Run(
      der_certs ? net::X509Certificate::CreateFromDERCertChain(*der_certs)
...
void SignedExchangeCertFetcher::Start(
  ...
  client_binding_.set_connection_error_handler(
      base::BindOnce(&SignedExchangeCertFetcher::OnClientConnectionError,
                     base::Unretained(this)));

  shared_url_loader_factory->CreateLoaderAndStart(
      mojo::MakeRequest(&url_loader_), 0 /* routing_id */, 0 /* request_id */,
      network::mojom::kURLLoadOptionNone, *resource_request_, std::move(client),
...
void SignedExchangeCertFetcher::Abort() {
  url_loader_ = nullptr;
  client_binding_.Close();
  if (!drainer_ && callback_)
    std::move(callback_).Run(scoped_refptr<net::X509Certificate>());
}

void SignedExchangeCertFetcher::OnClientConnectionError() {
  Abort();
}
