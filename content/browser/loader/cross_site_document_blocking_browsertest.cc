...
IN_PROC_BROWSER_TEST_F(CrossSiteDocumentBlockingTest, BlockDocuments) {
  ...
  for (const char* resource : blocked_resources) {
    ...
    InspectHistograms(histograms, true, true, resource);
  }
  ...
  for (const char* resource : nosniff_blocked_resources) {
    ...
    ASSERT_TRUE(ExecuteScriptAndExtractBool(
        ...
        &was_blocked));
    EXPECT_TRUE(was_blocked);
    InspectHistograms(histograms, true, false, resource);
  }
  ...
  for (const char* resource : sniff_allowed_resources) {
    ...
    InspectHistograms(histograms, false, true, resource);
  }
  ...
  for (const char* resource : allowed_resources) {
    ...
    ASSERT_TRUE(ExecuteScriptAndExtractBool(
        ...
        &was_blocked));
    EXPECT_FALSE(was_blocked);
    InspectHistograms(histograms, false, false, resource);
  }
}
...
IN_PROC_BROWSER_TEST_F(CrossSiteDocumentBlockingTest, RangeRequest) {
...

  {
    ...
    EXPECT_TRUE(was_blocked);
    InspectHistograms(histograms, true, false, "valid.html");
  }
  {
    ...
    EXPECT_FALSE(was_blocked);
    InspectHistograms(histograms, false, false, "valid.js");
  }
  {
    ...
    EXPECT_FALSE(was_blocked);
    InspectHistograms(histograms, false, false, "cors.json");
  }
}
