...
class CrossSiteDocumentBlockingServiceWorkerTest : public ContentBrowserTest {
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    ASSERT_FALSE(SiteInstance::IsSameWebSite(browser()->profile(),
                                             GetURLOnServiceWorkerServer("/"),
                                             GetURLOnCrossOriginServer("/")));
  }
 ...
 private:
  ...
  // 2. One of tests requires fetching a cross-origin resources from the
...
IN_PROC_BROWSER_TEST_F(CrossSiteDocumentBlockingServiceWorkerTest, NoNetwork) {
  ...
  std::string response;
  std::string script = R"(
      // Any cross-origin URL can be used here - it will be intercepted
      // by the service worker and will never go to the network.
...
IN_PROC_BROWSER_TEST_F(CrossSiteDocumentBlockingServiceWorkerTest,
...

  {
    ...
    // Verify that XSDB blocked the cached/opaque response from the service
    // worker from reaching a cross-origin page.
    EXPECT_EQ("", response);
    InspectHistograms(histograms, true /* should_be_blocked */,
                      false /* should_be_sniffed */, "nosniff.html",
                      RESOURCE_TYPE_XHR);
