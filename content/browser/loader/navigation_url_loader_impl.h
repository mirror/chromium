...
class NavigationURLLoaderImpl : public NavigationURLLoader {
 ...
 private:
  ...
  // Notifies the delegate the the request has failed.
  // If |net_error| is a certificate error, the caller must pass values for
  // |ssl_info| and |fatal_cert_error|. If |net_error| is a certificate error,
  // |ssl_info| and |fatal_cert_error| are ignored.
