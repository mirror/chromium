...
void CrossSiteDocumentResourceHandler::OnWillRead(
  ...
  if (should_block_response_) {
    if (no_sniff_) {
      ...
      // renderer an empty response.  First prepare a throwaway buffer (not
      // shared with the renderer) to read a single byte from the stream, since
      // providing a zero size buffer here isn't supported.  We will later
      // complete the response in OnReadCompleted and cancel any further reads
      // in the next call to OnWillRead.
      local_buffer_ =
          base::MakeRefCounted<net::IOBuffer>(static_cast<size_t>(1));
      *buf = local_buffer_;
      *buf_size = 1;
      return;
    }
    ...
    // This is only done when we suspect the response should be blocked and
    // there isn't a no-sniff header.
    //
...
bool CrossSiteDocumentResourceHandler::ShouldBlockResponse() {
  ...
  // Check if the response's site needs to have its documents blocked.  By
  ...
  // expect this to happen.  Sadly, this requires a copy.
