  ...
  if (should_block_based_on_headers_ && !allow_based_on_sniffing_) {
    ...
    if (mime_type_confirmed) {
      ...
      TRACE_EVENT2("navigation",
                   ...
                   "initiator", initiator, "url", request()->url().spec());
...
bool CrossSiteDocumentResourceHandler::ShouldBlockBasedOnHeaders(
  ...
  needs_sniffing_ = true;

  // Skip sniffing if there's a nosniff header.  Also skip for range requests,
  // so that attackers can't request a range that fails to sniff as a protected
  // type to bypass blocking.
