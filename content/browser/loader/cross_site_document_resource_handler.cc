...
void CrossSiteDocumentResourceHandler::OnWillRead(
  ...
  // due to sniffing, read some of the data to sniff it.
  if (should_block_based_on_headers_ && !allow_based_on_sniffing_) {
    ...
    // |no_sniff_| is true, but there's no need for the extra complexity.
...
void CrossSiteDocumentResourceHandler::OnReadCompleted(
  ...
  if (should_block_based_on_headers_ && !allow_based_on_sniffing_) {
    ...
    if (no_sniff_) {
      // TODO(creis): Also confirmed if bytes_read too small
...
bool CrossSiteDocumentResourceHandler::ShouldBlockBasedOnHeaders(
  ...
  if (base::LowerCaseEqualsASCII(nosniff_header, "nosniff"))
    no_sniff_ = true;
  // TODO(creis): Also nosniff_ if range request
