  ...
  if (should_block_based_on_headers_ && !allow_based_on_sniffing_) {
    ...
    if (!needs_sniffing_) {
      ...
      if (confirmed_blockable != CrossSiteDocumentClassifier::kNo) {
        // SniffForFetchOnlyResource() detected .
      } else if (canonical_mime_type_ == CROSS_SITE_DOCUMENT_MIME_TYPE_OTHERS) {
        // For unprotected mime types, the policy is to only block if we see a
        // parser-breaking pattern. Since SniffForFetchOnlyResource didn't find
        // one, allow this response.
        confirmed_blockable = CrossSiteDocumentClassifier::kNo;
...
bool CrossSiteDocumentResourceHandler::ShouldBlockBasedOnHeaders(
  ...
  // Look up MIME type.  Even if it doesn't claim to be a blockable type (i.e., HTML,
  // XML, JSON, or plain text), we still want to apply SniffForFetchOnlyResource().
  //
