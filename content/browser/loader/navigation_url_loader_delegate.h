...
class CONTENT_EXPORT NavigationURLLoaderDelegate {
 public:
  ...
  // SSL info for the request. |bool should_ssl_errors_be_fatal| indicates
  // whether SSL errors for the request should be fatal. If |net_error| is a
  // certificate error, the caller should pass a value for |ssl_info|. If
  // |net_error| is not a certificate error, |ssl_info| is ignored.
