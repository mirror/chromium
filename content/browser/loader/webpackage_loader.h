...
class WebPackageLoader : public mojom::URLLoader,
 ...
 private:
  ...
  // WebPackageReaderClient:
  void OnFoundManifest(const WebPackageManifest& manifest) override;
  void OnFoundResource(const ResourceRequest& request) override;
  ...
  mojo::ScopedDataPipeConsumerHandle webpackage_body_handle_;
  scoped_refptr<URLLoaderFactoryGetter> url_loader_factory_getter_;
  ...
  // For handling dummy main resource contents.
  ResourceResponseHead webpackage_response_head_;
  base::Optional<network::URLLoaderCompletionStatus>
      webpackage_completion_status_;

  std::unique_ptr<WebPackageReader> reader_;
