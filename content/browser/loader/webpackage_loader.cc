...
void ForwardRedirectResponse(const net::RedirectInfo& redirect_info,
...
void WebPackageRequestHandler::MaybeCreateLoader(
  ...
  if (webpackage_loader_ && webpackage_start_url_ == resource_request.url) {
...
void WebPackageLoader::OnReceiveRedirect(
    ...
    const ResourceResponseHead& response_head) {
  std::move(loader_callback_)
      .Run(base::BindOnce(&ForwardRedirectResponse, redirect_info,
                          response_head));
