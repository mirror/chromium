...
class WebPackageLoader::MainResourceLoader : public mojom::URLLoader {
 public:
  MainResourceLoader(WebPackageLoader* webpackage_loader,
                     ...
                     const ResourceResponseHead& webpackage_response_head,
    ...
    // XXX Dummy
    // Assumes that the resource body for the 'package' URL is
    // actually the main resource (just for now).
    DCHECK(webpackage_response_head.headers);
    ResourceResponseHead response_head(webpackage_response_head);
    response_head.headers->RemoveHeader("Content-type");
    response_head.headers->AddHeader("Content-type: text/html");
    response_head.mime_type = "text/html";
    url_loader_client_->OnReceiveResponse(response_head, base::nullopt,
                                          nullptr /* download */);
    url_loader_client_->OnStartLoadingResponseBody(std::move(body_handle));
  }

  void OnComplete(const network::URLLoaderCompletionStatus& status) {
    LOG(ERROR) << "** OnComplete: " << status.error_code;
...
WebPackageLoader::WebPackageLoader(
    ...
    : loader_callback_(std::move(loader_callback)),
      ...
      reader_(std::make_unique<WebPackageReader>(this,
...
void WebPackageLoader::CreateMainResourceLoader(
  ...
  DCHECK(webpackage_body_handle_.is_valid());
  DCHECK(!main_resource_loader_);

  main_resource_loader_ = std::make_unique<MainResourceLoader>(
      this, std::move(webpackage_body_handle_), webpackage_response_head_,
      std::move(loader_request), std::move(loader_client));
  if (webpackage_completion_status_)
    main_resource_loader_->OnComplete(*webpackage_completion_status_);
}
...
void WebPackageLoader::CreateSubresourceLoader(
  ...
  if (reader_->CreateResourceLoader(&loader_request, routing_id, request_id,
                                    options, resource_request, &loader_client,
                                    traffic_annotation)) {
    return;
  }
  if (allow_fallback) {
...
void WebPackageLoader::OnReceiveResponse(
  ...
  webpackage_response_head_ = response_head;
...
void WebPackageLoader::OnStartLoadingResponseBody(
  ...
  DCHECK(reader_);
  DCHECK(!webpackage_body_handle_.is_valid());
  webpackage_body_handle_ = std::move(body);

  // Note: this doesn't really consume anything from this handle *yet*.
  // (And the handle is eventually passed to the dummy MainResourceHandler,
  // just for now.)
  reader_->Consume(&webpackage_body_handle_);
}

void WebPackageLoader::OnComplete(
    const network::URLLoaderCompletionStatus& status) {
  if (!main_resource_loader_) {
    webpackage_completion_status_ = status;
    return;
  }
  main_resource_loader_->OnComplete(status);
}
