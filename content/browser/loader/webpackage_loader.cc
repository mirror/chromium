...
class WebPackageLoader::MainResourceLoader : public mojom::URLLoader {
 public:
  MainResourceLoader(WebPackageLoader* webpackage_loader,
                     WebPackageReaderAdapter* reader,
      ...
      : loader_client_(std::move(client)),
        ...
        reader_(reader),
        weak_factory_(this) {
    // |webpackage_loader| owns |this|, and |binding_| is owned by |this|.
 ...
 private:
  void OnFoundResource(const ResourceResponseHead& response_head,
                       mojo::ScopedDataPipeConsumerHandle body) {
    loader_client_->OnReceiveRsponse(response_head, base::nullopt,
                                     nullptr /* download_file */);
  ...
  // Not owned, |reader_| is owned by the owner of |this|.
  WebPackageReaderAdapter* reader_;
...
class WebPackageLoader::SubresourceLoader : public mojom::URLLoader {
 public:
  SubresourceLoader(WebPackageLoader* webpackage_loader,
                    WebPackageReaderAdapter* reader,
                    const ResourceRequest& resource_request,
                    mojom::URLLoaderRequest request,
                    mojom::URLLoaderClientPtr client)
    ...
    // |webpackage_loader| owns |this|, and |binding_| is owned by |this|.
    binding_.set_connection_error_handler(
        base::BindOnce(&WebPackageLoader::OnFinishSubresourceLoad,
                       base::Unretained(webpackage_loader)));
    reader_->GetResource(
        resource_request,
...
void WebPackageLoader::CreateMainResourceLoader(
  ...
  main_resource_loader_ = std::make_unique<MainResourceLoader>(
      this, reader_.get(), std::move(loader_request), std::move(loader_client),
      webpackage_start_url_);
}
...
void WebPackageLoader::OnStartLoadingResponseBody(
  ...
  DCHECK(!main_resource_loader_);
  DCHECK(!reader_);
  reader_ = std::make_unique<WebPackageReaderAdapter>(this, std::move(body));
}
