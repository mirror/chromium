...
class CONTENT_EXPORT URLLoaderRequestHandler {
 public:
  ...
  // Note that the handler can return null callback to MaybeCreateLoader(),
  // yet can return non-null SubresourceLoaderParams here if it does NOT
  // want to handle the specific request given to MaybeCreateLoader()
  // but want to handle the subsequent resource requests.
