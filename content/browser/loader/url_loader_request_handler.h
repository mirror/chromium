...
class CONTENT_EXPORT URLLoaderRequestHandler {
 public:
  ...
  using LoaderCallback =
      base::OnceCallback<void(SharedURLLoaderFactory::SingleRequestLoader)>;

  // Asks this handler to handle this resource load request.
  // The handler must invoke |callback| eventually with either a non-null
  // SingleRequestLoader -- indicating its willingness to handle the request --
  // or a null SingleRequestLoader to indicate that someone else should handle
  // the request.
