...
void WebPackageSubresourceManagerHost::OnRequest(const GURL& request_url,
                                                 const std::string& method) {
  mojom::WebPackageResourceRequestPtr request =
      mojom::WebPackageResourceRequest::New(request_url, method);
...
void WebPackageSubresourceManagerHost::OnResponse(
  ...
  if (!requests_.empty()) {
    subresource_manager_->OnRequests(std::move(requests_));
    requests_.clear();
  }
  mojom::WebPackageResourceRequestPtr request =
      mojom::WebPackageResourceRequest::New(request_url, method);
  mojom::WebPackageResponseCompletionCallbackPtr callback_ptr;
  subresource_manager_->OnResponse(std::move(request), response_head,
                                   std::move(body),
                                   mojo::MakeRequest(&callback_ptr));
  *callback = base::BindOnce(
      &CompletionCallbackWrapper::OnComplete,
      std::make_unique<CompletionCallbackWrapper>(std::move(callback_ptr)));
}
