...
class ResourceDispatcherHostTest : public testing::Test {
 public:
  ...
  ResourceDispatcherHostTest()
    ...
    requester_info_ = filter_->requester_info_for_test();
 ...
 protected:
  ...
  TestFilter* MakeTestFilter() {
    TestFilter* filter = new TestFilter(browser_context_->GetResourceContext());
...
void ResourceDispatcherHostTest::MakeTestRequestWithRenderFrame(
  ...
  URLLoaderFactoryImpl::CreateLoaderAndStart(
      requester_info_.get(), std::move(loader_request), render_view_id,
      request_id, 0 /* options */, request, std::move(client),
...
void ResourceDispatcherHostTest::MakeTestRequestWithResourceType(
  ...
  ResourceRequest request = CreateResourceRequest("GET", type, url);
  URLLoaderFactoryImpl::CreateLoaderAndStart(
      filter->requester_info_for_test(), std::move(loader_request),
      render_view_id, request_id, 0 /* options */, request, std::move(client),
      net::NetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS));
}
...
void ResourceDispatcherHostTest::
  ...
  URLLoaderFactoryImpl::CreateLoaderAndStart(
      requester_info_.get(), std::move(loader_request), 0, request_id,
      0 /* options */, request, std::move(client),
      net::NetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS));
}
...
void CheckSuccessfulRequest(TestURLLoaderClient* client,
  ...
  if (!reference_data.empty()) {
    ...
    while (true) {
      ...
      auto result = body->ReadData(buffer, &size, MOJO_READ_DATA_FLAG_NONE);
...
TEST_F(ResourceDispatcherHostTest, Cancel) {
  ...
  GlobalRequestID global_request_id(requester_info_->child_id(), 4);
...
TEST_F(ResourceDispatcherHostTest, DownloadToNetworkCache) {
  ...
  URLLoaderFactoryImpl::CreateLoaderAndStart(
      requester_info_.get(), mojo::MakeRequest(&loader2), 0, 2, 0 /* options */,
  ...
  EXPECT_EQ(net::OK, client1.completion_status().error_code);
...
TEST_F(ResourceDispatcherHostTest, DeletedFilterDetached) {
  ...
  URLLoaderFactoryImpl::CreateLoaderAndStart(
      requester_info_.get(), mojo::MakeRequest(&loader1), 0, 1, 0 /* options */,
...
TEST_F(ResourceDispatcherHostTest, DeletedFilterDetachedRedirect) {
  ...
  URLLoaderFactoryImpl::CreateLoaderAndStart(
      requester_info_.get(), mojo::MakeRequest(&loader), 0, 1, 0 /* options */,
...
TEST_F(ResourceDispatcherHostTest, IgnoreCancelForDownloads) {
  ...
  MakeTestRequestWithResourceType(
      ...
      GURL("http://example.com/blah"), RESOURCE_TYPE_MAIN_FRAME,
      mojo::MakeRequest(&loader), client.CreateInterfacePtr());
...
TEST_F(ResourceDispatcherHostTest, TransferNavigationHtml) {
  ...
  constexpr int new_render_view_id = 1;
  constexpr int new_request_id = 2;
  mojom::URLLoaderPtr new_loader;
  TestURLLoaderClient new_client;
  ...
  URLLoaderFactoryImpl::CreateLoaderAndStart(
      requester_info_.get(), mojo::MakeRequest(&new_loader), new_render_view_id,
      new_request_id, 0 /* options */, request, new_client.CreateInterfacePtr(),
      net::NetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS));
  content::RunAllTasksUntilIdle();

  CheckSuccessfulRequest(&new_client, kResponseBody);
...
TEST_F(ResourceDispatcherHostTest, TransferTwoNavigationsHtml) {
  ...
  constexpr int new_render_view_id = 1;
  constexpr int new_request_id = 5;
  mojom::URLLoaderPtr new_loader;
  TestURLLoaderClient new_client;
  ResourceRequest request = CreateResourceRequest(
  ...
  constexpr int new_second_request_id = 6;
  mojom::URLLoaderPtr new_loader2;
  TestURLLoaderClient new_client2;
  ...
  URLLoaderFactoryImpl::CreateLoaderAndStart(
      requester_info_.get(), mojo::MakeRequest(&new_loader2),
      new_render_view_id, new_second_request_id, 0 /* options */,
      second_request, new_client2.CreateInterfacePtr(),
      net::NetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS));
  content::RunAllTasksUntilIdle();

  CheckSuccessfulRequest(&new_client, kResponseBody);
  CheckSuccessfulRequest(&new_client2, kResponseBody);
...
TEST_F(ResourceDispatcherHostTest, TransferNavigationWithProcessCrash) {
  ...
  // Create a first filter that can be deleted before the second one starts.
  {
    mojom::URLLoaderPtr loader;
    TestURLLoaderClient client;
    scoped_refptr<TestFilter> first_filter = MakeTestFilter();
    ...
    URLLoaderFactoryImpl::CreateLoaderAndStart(
        requester_info_.get(), mojo::MakeRequest(&loader), render_view_id,
        request_id, 0 /* options */, first_request, client.CreateInterfacePtr(),
        net::NetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS));
    content::RunAllTasksUntilIdle();
