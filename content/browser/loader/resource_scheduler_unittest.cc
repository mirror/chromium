...
class ResourceSchedulerTest : public testing::Test {
 protected:
  ...
  void InitializeThrottleDelayableExperiment(
    ...
    ResourceScheduler::ParamsForNetworkQualityRanges nq_ranges =
        ResourceScheduler::GetParamsForNetworkQualityRangesForTests();

    if (!lower_delayable_count_enabled && non_delayable_weight <= 0.0) {
      ASSERT_EQ(0u, nq_ranges.size());
      return;
    }

    // Check that the configuration was parsed and stored correctly.
    ASSERT_EQ(lower_delayable_count_enabled ? 2u : 1u, nq_ranges.size());

    EXPECT_EQ(net::EFFECTIVE_CONNECTION_TYPE_SLOW_2G,
              nq_ranges[0].min_effective_connection_type);
    EXPECT_EQ(net::EFFECTIVE_CONNECTION_TYPE_2G,
              nq_ranges[0].max_effective_connection_type);
    EXPECT_EQ(non_delayable_weight > 0.0 ? 10u : 2u,
              nq_ranges[0].max_delayable_requests);
    EXPECT_EQ(non_delayable_weight > 0.0 ? non_delayable_weight : 0.0,
              nq_ranges[0].non_delayable_weight);

    if (lower_delayable_count_enabled) {
      EXPECT_EQ(net::EFFECTIVE_CONNECTION_TYPE_3G,
                nq_ranges[1].min_effective_connection_type);
      EXPECT_EQ(net::EFFECTIVE_CONNECTION_TYPE_3G,
                nq_ranges[1].max_effective_connection_type);
      EXPECT_EQ(4u, nq_ranges[1].max_delayable_requests);
      EXPECT_EQ(0.0, nq_ranges[1].non_delayable_weight);
    }
  }
  ...
  void ReadConfigTestHelper(size_t num_ranges) {
    ...
    for (size_t index = 1; index <= num_ranges; index++) {
      ...
      params["MinEffectiveConnectionType" + index_str] =
          net::GetNameForEffectiveConnectionType(
              static_cast<net::EffectiveConnectionType>(1 + index));
      params["MaxEffectiveConnectionType" + index_str] =
          net::GetNameForEffectiveConnectionType(
              static_cast<net::EffectiveConnectionType>(2 + index));
    ...
    for (size_t index = 1; index <= num_ranges; index++) {
      EXPECT_EQ(1 + index, nq_ranges[index - 1].min_effective_connection_type);
      EXPECT_EQ(2 + index, nq_ranges[index - 1].max_effective_connection_type);
      EXPECT_EQ(index * 10u, nq_ranges[index - 1].max_delayable_requests);
...
TEST_F(ResourceSchedulerTest, ReadInvalidConfigTest) {
  ...
  EXPECT_EQ(1u, nq_ranges.size());
  EXPECT_EQ(net::EFFECTIVE_CONNECTION_TYPE_SLOW_2G,
            nq_ranges[0].min_effective_connection_type);
  EXPECT_EQ(net::EFFECTIVE_CONNECTION_TYPE_2G,
            nq_ranges[0].max_effective_connection_type);
  EXPECT_EQ(10u, nq_ranges[0].max_delayable_requests);
  EXPECT_EQ(0.0, nq_ranges[0].non_delayable_weight);
}
