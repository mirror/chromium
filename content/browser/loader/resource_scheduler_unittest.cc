...
// Tests to see if the maximum number of delayable requests is overridden
...
TEST_F(ResourceSchedulerTest, RequestLimitOverrideConfigTest) {
  for (bool exp_status : {true, false}) {
    ...
    // |InitializeMaxDelayableRequestsExperiment|. Needs to be done before
    // initializing the scheduler because the client is created on the call to
    // |InitializeScheduler|, which is where the initial limits for the
    // delayable requests in flight are computed.
...
// Tests to check that the limit is not overridden when the ECT is not equal to
...
// Tests to check that the limit is not overridden when the ECT is valid, but
// the BDP does not lie in one of the buckets provided in the configuration.
...
// Tests to check that a change in network conditions midway during loading does
// not change the behavior of the resource scheduler.
...
// Tests to check that when the network quality changes such that the new limit
// is lower, and an |OnNavigate| event occurs, the new delayable requests don't
...
TEST_F(ResourceSchedulerTest, RequestLimitReducedAcrossPageLoads) {
  base::test::ScopedFeatureList scoped_feature_list;
  InitializeMaxDelayableRequestsExperiment(&scoped_feature_list, true);
  // BDP value is in range for which the limit must overridden to 4. The
  // effective connection type is set to Slow-2G.
  ...
  // limit must overridden to 4. The effective connection type is set to
  // Slow-2G.
  network_quality_estimator_.set_bandwidth_delay_product_kbits(120);
  network_quality_estimator_.set_effective_connection_type(
      net::EFFECTIVE_CONNECTION_TYPE_SLOW_2G);
