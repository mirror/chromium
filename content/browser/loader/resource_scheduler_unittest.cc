...
TEST_F(ResourceSchedulerTest, NoDelayableInterarrivalNotRecorded) {
  ...
  const std::string histogram_name =
      ...
      "ImmediateDelayableToNonDelayableStart";
  InitializeScheduler();
...
TEST_F(ResourceSchedulerTest, InterarrivalRecordedInFlightDelayable) {
  ...
  base::RunLoop().RunUntilIdle();
  // There are only 5 delayable requests in-flight which were started
  // immediately on scheduling. Add a non-delayable request and verify that
  // there were 5 entries added to the histogram.
  std::unique_ptr<TestRequest> non_delayable_request =
      NewRequest("http://host/high", net::HIGHEST);
  EXPECT_TRUE(non_delayable_request->started());
...
TEST_F(ResourceSchedulerTest, InterarrivalRecordedImmediatelyStartedDelayable) {
  ...
  // in-flight, only 5 of them were started immediately on scheduling. Hence,
  // there must be 5 new entries in the histogram.
  std::unique_ptr<TestRequest> non_delayable_request =
      NewRequest("http://host/high", net::HIGHEST);
  EXPECT_TRUE(non_delayable_request->started());
  histograms.ExpectTotalCount(histogram_name, 5);
