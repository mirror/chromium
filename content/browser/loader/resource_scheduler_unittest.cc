...
// A test scheduler class to access protected members.
class TestResourceScheduler : public ResourceScheduler {
 public:
  typedef ResourceScheduler::MaxRequestsForBDPRanges MaxRequestsForBDPRanges;

  static MaxRequestsForBDPRanges GetMaxDelayableRequestsExperimentConfig() {
    return ResourceScheduler::GetMaxDelayableRequestsExperimentConfig();
  }

  static net::EffectiveConnectionType
  GetMaxDelayableRequestsExperimentMaxECT() {
    return ResourceScheduler::GetMaxDelayableRequestsExperimentMaxECT();
  }
};
...
class ResourceSchedulerTest : public testing::Test {
 protected:
  ...
  void ReadConfigTestHelper(size_t num_bdp_ranges, std::string max_ect_string) {
    base::FieldTrialParamAssociator::GetInstance()->ClearAllParamsForTesting();
    const char kTrialName[] = "TrialName";
    const char kGroupName[] = "GroupName";
    const char kMaxDelayableRequestsNetworkOverride[] =
    ...
    // Use |TestResourceScheduler| to access protected members of
    // |ResourceScheduler|.
    TestResourceScheduler::MaxRequestsForBDPRanges bdp_ranges =
        TestResourceScheduler::GetMaxDelayableRequestsExperimentConfig();

    // Check that the configuration was parsed and stored correctly.
    EXPECT_EQ(bdp_ranges.size(), num_bdp_ranges);
    net::EffectiveConnectionType max_ect;
    net::GetEffectiveConnectionTypeForName(max_ect_string, &max_ect);
    EXPECT_EQ(TestResourceScheduler::GetMaxDelayableRequestsExperimentMaxECT(),
              max_ect);
    for (size_t bdp_range_index = 1; bdp_range_index <= num_bdp_ranges;
         bdp_range_index++) {
