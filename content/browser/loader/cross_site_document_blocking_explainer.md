...
be effectively transparent to websites.  The examples below illustrate how XSDB
can be invisible to websites in a variety of scenarios.
...
* **Tracking pixels**
  * Various techniques try to check that a user has accessed some content
    ...
    to a HTTP URI that frequently replies either with a 204 or with a
  ...
  * XSDB won't impact these techniques, as they don't depend on the actual
    content being returned by the HTTP server.
