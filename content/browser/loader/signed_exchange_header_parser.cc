...
class StructuredHeaderParser {
 ...
 private:
  void SkipWhitespaces() {
    size_t len = input_.find_first_not_of(kWhitespaceChars);
...
bool SignedExchangeHeaderParser::ParseSignature(
  ...
  for (auto& value : values) {
    ...
    if (!base::StringToUint64(value.params["date"], &date)) {
      DVLOG(1) << "ParseSignature: 'date' parameter is not a number: " << date;
      signatures->clear();
      return false;
    }
    if (!base::StringToUint64(value.params["expires"], &expires)) {
