...
SignedExchangeHandler::SignedExchangeHandler(
  ...
  LOG(ERROR) << "SignedExchangeHandler ctor";
...
void SignedExchangeHandler::ReadLoop() {
  ...
  DCHECK(read_buf_);
  LOG(ERROR) << "SignedExchangeHandler::ReadLoop";
  int rv = source_->Read(
...
void SignedExchangeHandler::DidRead(bool completed_syncly, int result) {
  LOG(ERROR) << "SignedExchangeHandler::DidRead " << result;
...
bool SignedExchangeHandler::RunHeadersCallback() {
  ...
  LOG(ERROR) << "SignedExchangeHandler::RunHeadersCallback";
  ...
  LOG(ERROR) << "Running headers callback ";
...
void SignedExchangeHandler::RunErrorCallback(net::Error error) {
  LOG(ERROR) << "SignedExchangeHandler::RunErrorCallback " << error;
