...
void SignedExchangeHandler::OnDataComplete() {
  ...
  // TODO(803774): Get the request url by parsing CBOR format.
  GURL new_request_url = GURL("https://example.com/test.html");
  // TODO(803774): Get the paload by parsing CBOR format.
  std::string payload = original_body_string_;
  original_body_string_.clear();

  // TODO(803774): Get more headers by parsing CBOR.
  ...
  // TODO(803774): Get the certificate by parsing CBOR and verify.
  base::Optional<net::SSLInfo> ssl_info;

  mojo::DataPipe pipe(kPipeSizeForSignedResponseBody);
  // TODO(803774): Write the error handling code. This could fail.
  DCHECK(pipe.consumer_handle.is_valid());
  mojo::ScopedDataPipeConsumerHandle response_body =
      std::move(pipe.consumer_handle);
  std::move(found_callback_)
      .Run(new_request_url, resource_response, std::move(ssl_info),
