...
// TODO(https://crbug.cxom/803774): Just for now, until we implement streaming
// CBOR parser.
...
void SignedExchangeHandler::DidRead(bool completed_syncly, int result) {
  if (result < 0)
    return;  // TODO: propagate the error
  if (result == 0) {
    RunHeadersCallback();
    return;
  }
...
bool SignedExchangeHandler::RunHeadersCallback() {
  ...
  // TODO(https://crbug.cxom/803774): request payload may come here.
  ...
  // TODO(https://crbug.cxom/803774): |mime_type| should be derived from
  // "Content-Type" header.
  response_head_.mime_type = "text/html";

  // TODO(https://crbug.cxom/803774): Check that the Signature header entry has
  // integrity="mi".
  std::string mi_header_value;
  if (!response_head_.headers->EnumerateHeader(nullptr, kMiHeader,
                                               &mi_header_value)) {
    return false;  // TODO: propagate the error
  }
