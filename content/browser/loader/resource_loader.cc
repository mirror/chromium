...
ResourceLoader::~ResourceLoader() {
  ...
  if (body_read_before_paused_ != -1) {
    ...
    bool scheme_to_record = request_->url().SchemeIs(url::kHttpScheme) ||
                            request_->url().SchemeIs(url::kHttpsScheme) ||
                            request_->url().SchemeIs(url::kFtpScheme);
    if (!request_->was_cached() && scheme_to_record) {
...
void ResourceLoader::PauseReadingBodyFromNet() {
  ...
  // cached. This also pauses things that cannot come from the network (chrome
  ...
  // hasn't been cached before. This avoids polluting the histogram data.
