...
bool WebPackageRequestHandler::MaybeCreateLoaderForResponse(
  ...
  *client_request = mojo::MakeRequest(&client);
  ...
  // ThrottlingURLLoader is unbounded here, so URLLoaderThrottles of the
  // ThrottlingURLLoader can't check the redirect response generated by the
  // WebPackageLoader. A new ThrottlingURLLoader will be created when
  // NavigationURLLoaderNetworkService::URLLoaderRequestController follows the
  // redirect, so the URLLoaderThrottles can check the redirected URL in
  // WillStartRequest().
  // TODO(https://crbug.com/80374): Revisit this if URLLoaderThrottles need to
  // check the redirection in WillRedirectRequest().
