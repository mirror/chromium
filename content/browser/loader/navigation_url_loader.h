...
class CONTENT_EXPORT NavigationURLLoader {
 public:
  ...
  // the URLLoader for sending the request, whether response has already been
  // completed, and the completion status.
  using NavigationResponseInterceptionCB =
  ...
  // for handling the URLLoader later on.
  virtual void InterceptNavigationResponse(
      NavigationResponseInterceptionCB callback) {}
