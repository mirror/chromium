...
class CONTENT_EXPORT ResourceScheduler {
 ...
 private:
  ...
  // Reads the experiment parameters to get the weight for non-delayable
  ...
  // quality.
  int GetCurrentNonDelayableThrottlesDelayableMultiplier(
      const net::NetworkQualityEstimator* network_quality_estimator) const;
  ...
  const size_t non_delayable_throttles_delayable_multiplier_;
