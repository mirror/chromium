 ...
 private:
  ...
  class MaxDelayableRequestsNetworkOverrideExperiment {
   public:
    ...
    // value of the bandwidth delay product. It falls back to the default
    ...
    // net::EFFECTIVE_CONNECTION_TYPE_UNKNOWN. 3. If the current value of the
    // BDP is not in any of the ranges in |max_requests_for_bdp_ranges|.
    size_t GetMaxDelayableRequests(
        const net::NetworkQualityEstimator* network_quality_estimator) const;

    // Iterates through |max_requests_for_bdp_ranges| to find the correct number
    // of delayable requests in-flight given |bdp_in_kbits|.
    int GetMaxDelayableRequestsForBDP(int64_t bdp_in_kbits) const;

    // Reads experiment parameters and creates a vector  of
    // |MaxRequestsForBDPRange| to populate |max_requests_for_bdp_ranges|. It
    ...
    // below: "MaxBDPKbits1" = "150"; "MaxDelayableRequests1" = "2"
    // "MaxBDPKbits2" = "200";
