...
void NavigationResourceHandler::OnResponseCompleted(
  ...
  if (core_) {
    ...
    base::Optional<bool> fatal_cert_error = base::nullopt;
    if (net::IsCertificateError(net_error)) {
      net::SSLInfo ssl_info_copy = request()->ssl_info();
      ssl_info = base::make_optional<net::SSLInfo>(std::move(ssl_info_copy));
      fatal_cert_error =
          base::make_optional<bool>(ShouldSSLErrorsBeFatal(request()));
    }

    core_->NotifyRequestFailed(request()->response_info().was_cached, net_error,
                               ssl_info, fatal_cert_error);
    DetachFromCore();
  }
