...
void SSLManager::DidCommitProvisionalLoad(const LoadCommittedDetails& details) {
  ...
  if (!details.is_main_frame) {
    ...
    // For main-frame navigations, clear content status flags. These flags are
    // set based on the content page, and thus should reflect the current
    // content on the page, even if the navigation was to an existing entry that
    // already had content status flags set.
