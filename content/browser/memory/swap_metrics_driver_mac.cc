// Copyright 2017 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#include "content/browser/memory/swap_metrics_driver_mac.h"

#include <mach/mach.h>
#include <mach/mach_vm.h>
#include <memory>

#include "base/mac/mach_logging.h"
#include "base/memory/ptr_util.h"
#include "content/browser/memory/swap_metrics_observer.h"

namespace content {

// static
std::unique_ptr<SwapMetricsDriver> SwapMetricsDriver::Create(
    std::unique_ptr<SwapMetricsObserver> observer,
    const int interval_in_seconds) {
  return base::WrapUnique<SwapMetricsDriver>(
      new SwapMetricsDriverMac(std::move(observer), interval_in_seconds));
}

SwapMetricsDriverMac::SwapMetricsDriverMac(
    std::unique_ptr<SwapMetricsObserver> observer,
    const int interval_in_seconds)
    : SwapMetricsDriver(std::move(observer), interval_in_seconds),
      host_(mach_host_self()) {}

SwapMetricsDriverMac::~SwapMetricsDriverMac() {}

SwapMetricsUpdateResult SwapMetricsDriverMac::UpdateMetricsInternal(
    base::TimeDelta interval) {
  vm_statistics64_data_t statistics;
  mach_msg_type_number_t count = HOST_VM_INFO64_COUNT;
  kern_return_t result =
      host_statistics64(host_.get(), HOST_VM_INFO64,
                        reinterpret_cast<host_info64_t>(&statistics), &count);
  if (result != KERN_SUCCESS) {
    MACH_DLOG(WARNING, result) << "host_statistics64";
    return SwapMetricsUpdateResult::kSwapMetricsUpdateFailed;
  }
  DCHECK_EQ(HOST_VM_INFO64_COUNT, count);

  double swapins = statistics.swapins - last_swapins_;
  double swapouts = statistics.swapouts - last_swapouts_;
  double decompressions = statistics.decompressions - last_decompressions_;
  double compressions = statistics.compressions - last_compressions_;
  last_swapins_ = statistics.swapins;
  last_swapouts_ = statistics.swapouts;
  last_decompressions_ = statistics.decompressions;
  last_compressions_ = statistics.compressions;

  if (interval.is_zero())
    return SwapMetricsUpdateResult::kSwapMetricsUpdateSuccess;

  observer_->SwapsInPerSecond(swapins / interval.InSecondsF(), interval);
  observer_->SwapsOutPerSecond(swapouts / interval.InSecondsF(), interval);
  observer_->DecompressedPagesPerSecond(decompressions / interval.InSecondsF(),
                                        interval);
  observer_->CompressedPagesPerSecond(compressions / interval.InSecondsF(),
                                      interval);

  return SwapMetricsUpdateResult::kSwapMetricsUpdateSuccess;
}

}  // namespace content
