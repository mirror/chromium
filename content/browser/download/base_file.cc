...
#define CONDITIONAL_TRACE(name, ...)            \
  if (download_id_ != DownloadItem::kInvalidId) \
  TRACE_EVENT_##name(__VA_ARGS__)
...
DownloadInterruptReason BaseFile::WriteDataToFile(int64_t offset,
  ...
  CONDITIONAL_TRACE(NESTABLE_ASYNC_BEGIN0, "download", "DownloadFileWrite",
                    download_id_);
...
DownloadInterruptReason BaseFile::Rename(const base::FilePath& new_path) {
  ...
  CONDITIONAL_TRACE(BEGIN2, "download", "DownloadFileRename", "old_filename",
                    full_path_.value(), "new_filename", new_path.value());
...
void BaseFile::Detach() {
  detached_ = true;
  CONDITIONAL_TRACE(INSTANT0, "download", "DownloadFileDetached",
...
void BaseFile::Cancel() {
  ...
  CONDITIONAL_TRACE(INSTANT0, "download", "DownloadCancelled",
                    TRACE_EVENT_SCOPE_THREAD);

  Close();

  if (!full_path_.empty()) {
    CONDITIONAL_TRACE(INSTANT0, "download", "DownloadFileDeleted",
                      TRACE_EVENT_SCOPE_THREAD);
...
DownloadInterruptReason BaseFile::Open(const std::string& hash_so_far) {
  ...
  CONDITIONAL_TRACE(NESTABLE_ASYNC_BEGIN2, "download", "DownloadFileOpen",
                    download_id_, "file_name", full_path_.value(),
                    "bytes_so_far", bytes_so_far_);
...
void BaseFile::ClearFile() {
  ...
  file_.Close();
  CONDITIONAL_TRACE(NESTABLE_ASYNC_END0, "download", "DownloadFileOpen",
                    download_id_);
}
