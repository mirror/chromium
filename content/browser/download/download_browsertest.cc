...
class ErrorInjectionDownloadFileFactory : public DownloadFileFactory {
 public:
  ...
  DownloadFile* CreateFile(
      ...
      const net::NetLogWithSource& net_log,
      base::WeakPtr<DownloadDestinationObserver> observer) override {
    DownloadFileImpl* download_file =
        new DownloadFileImpl(std::move(save_info), default_download_directory,
 ...
 private:
  void InjectErrorIntoDownloadFile() {
    GetDownloadTaskRunner()->PostTask(
        FROM_HERE,
        base::BindOnce(&DownloadFileImpl::InjectErrorForTest,
  ...
  DownloadFileImpl* download_file_;
...
// TestDownloadHttpResponse::OnStartHandler.
class TestRequestStartHandler {
 public:
  ...
  TestDownloadHttpResponse::OnStartHandler GetOnStartHandler() {
 ...
 private:
  void OnStartHandler(const base::Closure& resume_callback) {
...
IN_PROC_BROWSER_TEST_F(DownloadContentTest, RedirectBeforeResume) {
  ...
  GURL second_url = embedded_test_server()->GetURL("example.com", "second-url");
  GURL third_url = embedded_test_server()->GetURL("example.com", "third-url");
  GURL download_url = embedded_test_server()->GetURL("example.com", "download");
...
IN_PROC_BROWSER_TEST_F(DownloadContentTest, DownloadAttributeNetworkError) {
  ...
  GURL server_url = embedded_test_server()->GetURL(url.host(), url.path());
  TestDownloadHttpResponse::Parameters parameters;
  // Simulate a network failure by injecting an error at the beginning.
  parameters.injected_errors.push(0);
  parameters.inject_error_cb = inject_error_callback();
  TestDownloadHttpResponse::StartServing(parameters, server_url);
