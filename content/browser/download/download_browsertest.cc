...
IN_PROC_BROWSER_TEST_F(DownloadContentTest, NonSuccessfulResponseBody) {
  net::EmbeddedTestServer server;
  const std::string kURL = "/404notfound";
  const std::string kResponseBody = "This is response body.";

  server.RegisterRequestHandler(
      CreateBasicResponseHandler(kURL, net::HTTP_NOT_FOUND, base::StringPairs(),
                                 "text/html", kResponseBody));
  ASSERT_TRUE(server.Start());
  GURL url = server.GetURL("/404notfound");
  ...
  // Verify the error response body in the file.
  {
    ...
    EXPECT_EQ(kResponseBody, file_content);
  }
}
