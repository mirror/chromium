...
class MHTMLGenerationManager::Job : public RenderProcessHostObserver {
 ...
 private:
  ...
  static std::tuple<MhtmlSaveStatus, int64_t> FinalizeAndCloseFileOnFileThread(
      ...
      base::File file,
      std::vector<MHTMLExtraDataPart> extra_data_parts);
  ...
  base::WeakPtr<MHTMLExtraPartsImpl> extra_parts_;
...
MHTMLGenerationManager::Job::Job(int job_id,
  ...
  if (extra_parts)
    extra_parts_ = extra_parts->AsWeakPtr();
}
...
void MHTMLGenerationManager::Job::CloseFile(
  ...
  std::vector<MHTMLExtraDataPart> extra_data_parts;
  if (extra_parts_)
    extra_data_parts = extra_parts_->parts();
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::Bind(
          ...
          (save_status == MhtmlSaveStatus::SUCCESS ? mhtml_boundary_marker_
                                                   : std::string()),
          base::Passed(&browser_file_), base::Passed(&extra_data_parts)),
      callback);
}
