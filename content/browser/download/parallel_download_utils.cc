...
bool CanRecoverFromError(
  ...
  DCHECK_NE(-1, error_stream->length())
      << "Length should not be kNoBytesToWrite.";
  ...
  DCHECK(!preceding_neighbor->is_finished());
  ...
  if (error_stream->length() > 0 && preceding_neighbor->length() > 0) {
    int64_t error_stream_upper_range =
        error_stream->offset() + error_stream->length();
    return error_stream_upper_range <=
               preceding_neighbor->offset() + preceding_neighbor->length() ||
           error_stream_upper_range <= preceding_neighbor->offset() +
                                           preceding_neighbor->bytes_written();
  }
