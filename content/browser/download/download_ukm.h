...
class DownloadUkmHelper {
 public:
  DownloadUkmHelper(const int download_id, const GURL& url);
  ~DownloadUkmHelper();

  void RecordDownloadStarted(int file_type);

  void RecordDownloadInterrupted(int change_in_file_size,
                                 int reason,
                                 int resulting_file_size,
                                 base::TimeDelta time_since_start);

  void RecordDownloadInterrupted(int reason,
                                 int resulting_file_size,
                                 base::TimeDelta time_since_start);

  void RecordDownloadResumed(int mode, base::TimeDelta time_since_start);
