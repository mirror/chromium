...
void ReadyCallbackWrapper(const mojo::SimpleWatcher::ReadyCallback& callback) {
  callback.Run(MOJO_RESULT_OK);
}
...
void DownloadFileImpl::SourceStream::RegisterDataReadyCallback(
  ...
  if (handle_watcher_) {
    ...
    stream_reader_->RegisterCallback(
        base::Bind(ReadyCallbackWrapper, callback));
  }
}

void DownloadFileImpl::SourceStream::CancelDataReadyCallback() {
  if (handle_watcher_)
...
void DownloadFileImpl::RenameWithRetryInternal(
  ...
  if (reason != DOWNLOAD_INTERRUPT_REASON_NONE) {
    ...
    for (auto& stream : source_streams_)
      stream.second->CancelDataReadyCallback();
...
void DownloadFileImpl::StreamActive(SourceStream* source_stream,
  ...
  if (reason != DOWNLOAD_INTERRUPT_REASON_NONE) {
    ...
    source_stream->CancelDataReadyCallback();
