  ...
  if (IsSparseFile() && !can_recover_from_error) {
    ...
    if (preceding_neighbor) {
      LOG(ERROR) << "@@@ preceding_neighbor, offset = "<<
          preceding_neighbor->offset() << ",is_finished()  =  " <<
          preceding_neighbor->is_finished() << ", ,length() = "
          << preceding_neighbor->length();
    }

    if (preceding_neighbor &&
        !preceding_neighbor->is_finished() &&
        preceding_neighbor->length() == DownloadSaveInfo::kLengthFullContent) {
