...
class TestDownloadHttpResponse : public net::test_server::HttpResponse {
 public:
  ...
  // OnStartHandler can be used to pause the response until the enclosed
  // callback is called.
  using OnStartHandler = base::Callback<void(const base::Closure&)>;
  ...
  struct Parameters {
    ...
    //
    // Injected error with offset equal to 0 will immediately abort the response
    // without sending anything to the client, even the request is for a byte
    // range larger than 0.
    ...
    // If on_start_handler is valid, it will be invoked before the response is
    // sent.
