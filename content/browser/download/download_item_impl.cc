...
void DownloadItemImpl::SetDelegate(
    std::unique_ptr<download::DownloadItemDelegate> delegate) {
  download_item_delegate_ = std::move(delegate);
}

download::DownloadItemDelegate* DownloadItemImpl::GetDelegate() const {
  return download_item_delegate_.get();
}
...
void DownloadItemImpl::CreateDelegate() {
  download_item_delegate_ = base::MakeUnique<ContentDownloadItemDelegateImpl>(
      GetBrowserContext(), GetWebContents());
}
