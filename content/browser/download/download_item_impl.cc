...
const char* const download_type_names[] = {"NEW_DOWNLOAD", "HISTORY_IMPORT",
                                           "SAVE_PAGE_AS"};

const char* const download_danger_names[] = {
    "NOT_DANGEROUS",     "DANGEROUS_FILE",          "DANGEROUS_URL",
    "DANGEROUS_CONTENT", "MAYBE_DANGEROUS_CONTENT", "UNCOMMON_CONTENT",
    "USER_VALIDATED",    "DANGEROUS_HOST",          "POTENTIALLY_UNWANTED"};
...
class DownloadItemActivatedData
 ...
 public:
  ...
  void AppendAsTraceFormat(std::string* out) const override {
    out->append("{");
    out->append(base::StringPrintf("\"type\":\"%s\",",
                                   download_type_names[download_type_]));
    ...
    out->append(base::StringPrintf("\"danger_type\":\"%s\",",
                                   download_danger_names[danger_type_]));
    out->append(base::StringPrintf("\"start_offset\":\"%ld\",", start_offset_));
    out->append(base::StringPrintf("\"start_offset\":\"%s\"",
...
void DownloadItemImpl::ValidateDangerousDownload() {
  ...
  TRACE_EVENT_INSTANT1("download", "DownloadItemSaftyStateUpdated",
                       ...
                       download_danger_names[GetDangerType()]);
  ...
  if (was_done && !is_done) {
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1(
        ...
        base::MakeUnique<DownloadItemActivatedData>(
            SRC_ACTIVE_DOWNLOAD, GetId(), GetOriginalUrl().spec(),
...
void DownloadItemImpl::SetDangerType(DownloadDangerType danger_type) {
  if (danger_type != danger_type_) {
    TRACE_EVENT_INSTANT1("download", "DownloadItemSaftyStateUpdated",
                         TRACE_EVENT_SCOPE_THREAD, "danger_type",
                         download_danger_names[danger_type]);
  }
...
void DownloadItemImpl::SetFullPath(const base::FilePath& new_path) {
  ...
  TRACE_EVENT_INSTANT2("download", "DownloadItemRenamed",
                       ...
                       destination_info_.current_path.value(), "new_filename",
                       new_path.value());
