...
void DownloadManagerImpl::SetDelegate(DownloadManagerDelegate* delegate) {
  ...
  // If there's a cache, initialize and indicate initialization is complete.
  // Else, just indicate it is ok to continue.
  download::InProgressCache* in_progress_cache =
      delegate_->GetInProgressCache();
  if (in_progress_cache) {
    in_progress_cache->Initialize(base::Bind(
        &DownloadManagerImpl::PostInitialization, base::Unretained(this),
