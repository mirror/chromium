    ...
    if (in_progress_cache) {
      ...
      if (!entry_opt.has_value()) {
        in_progress_cache->AddOrReplaceEntry(
            download::DownloadEntry(download->GetGuid(), base::RandUint64()));
      }
    }
    ...
    if (in_progress_cache) {
      ...
      if (!entry_opt.has_value()) {
        in_progress_cache->AddOrReplaceEntry(download::DownloadEntry(
            download_item->GetGuid(), base::RandUint64()));
      }
    }
  }
}
...
void DownloadManagerImpl::BeginDownloadInternal(
  ...
  if (in_progress_cache) {
    // TODO(jming): Currently the GUID will be null for all new downloads, so,
    // effectively this path is not being called. The relevant parameters need
    // to moved from DownloadUrlParameters so that they are able to be tracked
    // in StartDownloadWithId before this piece of code can be removed.
    if (params->guid() != std::string()) {
      in_progress_cache->AddOrReplaceEntry(download::DownloadEntry(
