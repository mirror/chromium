...
std::unique_ptr<net::HttpRequestHeaders> GetRequestHeaders(
    DownloadUrlParameters* params) {
  std::unique_ptr<net::HttpRequestHeaders> headers(new net::HttpRequestHeaders);
  if (params->offset() == 0 &&
...
DownloadRequestCore::CreateURLRequestOnIOThread(uint32_t download_id,
                                                DownloadUrlParameters* params) {
  ...
  std::unique_ptr<net::HttpRequestHeaders> headers = GetRequestHeaders(params);
  if (!headers->IsEmpty())
...
// static
std::unique_ptr<ResourceRequest> DownloadRequestCore::CreateResourceRequest(
    DownloadUrlParameters* params) {
  DCHECK(params->offset() >= 0);
  ...
  request->first_party_for_cookies = params->url();
