...
class CONTENT_EXPORT DownloadFileImpl : public DownloadFile {
 public:
  ...
  // |input_stream|, and sends updates and status of those reads to the
 ...
 private:
  ...
  // constructor is called. |input_stream| can be added later when the network
  // response is handled.
  //
  ...
  class CONTENT_EXPORT SourceStream : public mojom::DownloadStreamClient {
   public:
    ...
    void OnResponseCompleted(int error_code) override;
