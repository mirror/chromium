...
void SendResponses(const std::string& header,
                   ...
                   const base::Closure& completion_callback,
                   const net::test_server::SendBytesCallback& send) {
  if (!parameters.on_start_handler.is_null()) {
    ...
    params.on_start_handler.Reset();
    BrowserThread::PostTask(
        BrowserThread::UI, FROM_HERE,
        base::Bind(parameters.on_start_handler,
                   base::Bind(&OnResume, base::ThreadTaskRunnerHandle::Get(),
                              base::Bind(&SendResponses, header, body,
                                         std::move(params), completion_callback,
                                         send))));
    return;
  }
  ...
  if (!header.empty()) {
    send.Run(header,
             base::Bind(&SendResponses, std::string(), body,
                        std::move(parameters), completion_callback, send));
    return;
  }
...
void TestDownloadHttpResponse::SendResponse(
  ...
  bool should_abort_immediately = !parameters_.injected_errors.empty() &&
                                  parameters_.injected_errors.front() == 0 &&
  ...
  SendResponses(header, body, std::move(parameters_),
