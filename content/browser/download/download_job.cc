...
void DownloadJob::DestinationUpdate(
  ...
  download_item_->DestinationUpdate(
      bytes_so_far, bytes_per_sec, received_slices);
}
...
void DownloadJob::DestinationError(
  ...
  download_item_->DestinationError(
      reason, bytes_so_far, std::move(hash_state));
}

void DownloadJob::DestinationCompleted(
     int64_t total_bytes,
     std::unique_ptr<crypto::SecureHash> hash_state) {
  download_item_->DestinationCompleted(
      total_bytes, std::move(hash_state));
}
