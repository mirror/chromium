...
mojom::NetworkRequestStatus ConvertMojoNetworkRequestStatusToInterruptReason(
...
DownloadResponseHandler::CreateDownloadCreateInfo(
  ...
  if (head.headers && !head.headers->GetMimeType(
      &create_info->original_mime_type)) {
...
void DownloadResponseHandler::OnComplete(
  ...
  if (client_ptr_) {
    client_ptr_->OnStreamCompleted(
        ConvertMojoNetworkRequestStatusToInterruptReason(reason));
  }
}
