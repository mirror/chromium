...
IndexedDBPreCloseTaskList::PreCloseTask::~PreCloseTask() {}

IndexedDBPreCloseTaskList::IndexedDBPreCloseTaskList(
    std::list<std::unique_ptr<IndexedDBPreCloseTaskList::PreCloseTask>> tasks,
...
void IndexedDBPreCloseTaskList::StopForNewConnection() {
...
void IndexedDBPreCloseTaskList::Start(
  ...
  timeout_timer_->Start(FROM_HERE, timeout_time_,
                        base::Bind(&IndexedDBPreCloseTaskList::StopForTimout,
...
void IndexedDBPreCloseTaskList::OnComplete() {
  DCHECK(!complete_);
  timeout_timer_->Stop();
  complete_ = true;
...
void IndexedDBPreCloseTaskList::StopForTimout() {
  DCHECK(started_);
  if (complete_)
    return;
  while (!tasks_.empty()) {
...
void IndexedDBPreCloseTaskList::StopForMetadataError(
...
void IndexedDBPreCloseTaskList::RunLoop() {
  if (complete_)
    return;
  ...
  task_runner_->PostTask(FROM_HERE,
                         base::Bind(&IndexedDBPreCloseTaskList::RunLoop,
                                    ptr_factory_.GetWeakPtr()));
}
