...
IndexedDBFactoryImpl::IndexedDBFactoryImpl(IndexedDBContextImpl* context)
    : context_(context), clock_(new base::DefaultClock()) {}
...
void IndexedDBFactoryImpl::MaybeStartPreCloseTasks(const Origin& origin) {
  ...
  // Lazily set allowed_sweep_.
  if (allowed_sweep_ == base::Time())
    allowed_sweep_ = GenerateNextGlobalSweepTime(now);

  // Check that the last sweep hasn't run too recently.
  if (allowed_sweep_ > now)
    return;
  ...
  // Check that this origin hasn't run a sweep too recently.
  if (origin_earliest_sweep > now)
    return;

  // A sweep will now happen, so reset the sweep timers.
  allowed_sweep_ = GenerateNextGlobalSweepTime(now);
