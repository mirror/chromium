...
IndexedDBTombstoneSweeper::IndexedDBTombstoneSweeper(Mode mode,
    ...
    : mode_(mode),
      ...
      database_(database),
      ptr_factory_(this) {
  sweep_state_.start_database_seed = base::RandUint64();
  sweep_state_.start_object_store_seed = base::RandUint64();
  sweep_state_.start_index_seed = base::RandUint64();
}
...
bool IndexedDBTombstoneSweeper::IterateIndex(
  ...
  if (sweep_state_.curr_index_key) {
    iterator_->Seek(sweep_state_.curr_index_key.value().Encode());
  ...
  while (iterator_->Valid()) {
    ...
    sweep_state_.curr_index_key.emplace(IndexDataKey());
    if (!IndexDataKey::Decode(&index_key_str,
                              &sweep_state_.curr_index_key.value()) ||
        sweep_state_.curr_index_key.value().IndexId() != index.id) {
      break;
    }
  ...
  sweep_state_.curr_index_key = base::nullopt;
  return true;
}
