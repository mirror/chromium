...
void IndexedDBTombstoneSweeper::RecordUMAStats(
  ...
  if (stop_reason == StopReason::METADATA_ERROR)
    return;
...
leveldb::Status IndexedDBTombstoneSweeper::FlushDeletions() {
  ...
  if (!status.ok()) {
    UMA_HISTOGRAM_ENUMERATION("WebCore.IndexedDB.TombstoneSweeper.",
