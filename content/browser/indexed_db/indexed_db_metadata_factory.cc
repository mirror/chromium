...
IndexedDBMetadataFactory::IndexedDBMetadataFactory() = default;
IndexedDBMetadataFactory::~IndexedDBMetadataFactory() = default;

leveldb::Status IndexedDBMetadataFactory::GetDatabaseNames(
    LevelDBDatabase* db,
...
Status IndexedDBMetadataFactory::ReadFullDatabaseMetadata(
    LevelDBDatabase* db,
    const std::string& origin_identifier,
  ...
  *found = false;
  ...
  s = IndexedDBMetadataFactory::ReadObjectStoresAndIndexes(
      db, metadata->id, &metadata->object_stores);

  return s;
}

leveldb::Status IndexedDBMetadataFactory::CreateAndWriteDatabaseMetadata(
    LevelDBDatabase* db,
    const std::string& origin_identifier,
    base::string16 name,
    int64_t version,
...
void IndexedDBMetadataFactory::UpdateDatabaseVersion(
...
Status IndexedDBMetadataFactory::ReadDatabaseId(
    LevelDBDatabase* db,
    const std::string& origin_identifier,
    const base::string16& name,
    int64_t* id,
...
Status IndexedDBMetadataFactory::ReadObjectStoresAndIndexes(
    LevelDBDatabase* db,
    int64_t database_id,
    std::map<int64_t, IndexedDBObjectStoreMetadata>* object_stores) {
  IDB_TRACE("IndexedDBMetadataFactory::ReadObjectStoresAndIndexes");
  ...
  while (s.ok() && it->IsValid() && CompareKeys(it->Key(), stop_key) < 0) {
    ...
    IndexedDBObjectStoreMetadata metadata(object_store_name, object_store_id,
                                          ...
                                          max_index_id);
    s = IndexedDBMetadataFactory::ReadIndexes(db, database_id, object_store_id,
                                              &metadata.indexes);
    if (!s.ok())
      break;
...
Status IndexedDBMetadataFactory::CreateAndWriteObjectStore(
    LevelDBTransaction* transaction,
    int64_t database_id,
  ...
  PutInt(transaction, last_version_key, 1);
...
Status IndexedDBMetadataFactory::RenameObjectStore(
    LevelDBTransaction* transaction,
    int64_t database_id,
...
Status IndexedDBMetadataFactory::DeleteObjectStoreMetadata(
    LevelDBTransaction* transaction,
    int64_t database_id,
...
Status IndexedDBMetadataFactory::ReadIndexes(
    LevelDBDatabase* db,
    int64_t database_id,
    int64_t object_store_id,
...
Status IndexedDBMetadataFactory::CreateAndWriteIndex(
    LevelDBTransaction* transaction,
    int64_t database_id,
    int64_t object_store_id,
    int64_t index_id,
...
Status IndexedDBMetadataFactory::RenameIndex(LevelDBTransaction* transaction,
                                             int64_t database_id,
                                             int64_t object_store_id,
                                             base::string16 new_name,
                                             base::string16* old_name,
...
leveldb::Status IndexedDBMetadataFactory::DeleteIndexMetadata(
    LevelDBTransaction* transaction,
    int64_t database_id,
    int64_t object_store_id,
