...
class CONTENT_EXPORT IndexedDBFactoryImpl : public IndexedDBFactory {
 public:
  // Defines the maximum time for the period between runs of the
  // IndexedDBSweeper.
  // Visible for testing, and sweeping only occurs after backing store close.
  static constexpr const base::TimeDelta kMaxEarliestGlobalSweepFromNow =
      base::TimeDelta::FromHours(2);
  // Defines the maximum time for the period between runs of the
  // IndexedDBSweeper for a given origin.
  // Visible for testing, and sweeping only occurs after backing store close.
