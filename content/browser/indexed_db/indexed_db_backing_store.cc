...
leveldb::Status IndexedDBBackingStore::GetCompleteMetadata(
  ...
  IndexedDBMetadataFactory metadata_factory;
  leveldb::Status status = leveldb::Status::OK();
  std::vector<base::string16> names;
  status =
      metadata_factory.GetDatabaseNames(db_.get(), origin_identifier_, &names);
  if (!status.ok())
    return status;
  if (names.empty())
    return Status::OK();
  ...
  while (!names.empty()) {
    ...
    output->emplace_back();
    bool found = false;
    status = metadata_factory.ReadFullDatabaseMetadata(
...
Status IndexedDBBackingStore::DeleteDatabase(const base::string16& name) {
  ...
  bool success = false;
  int64_t id = 0;
  s = IndexedDBMetadataFactory().ReadDatabaseId(db_.get(), origin_identifier_,
                                                name, &id, &success);
  if (!s.ok())
    return s;
  if (!success)
