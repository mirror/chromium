...
leveldb::Status IndexedDBBackingStore::GetCompleteMetadata(
  ...
  IndexedDBMetadataCoding metadata_factory;
  leveldb::Status status = leveldb::Status::OK();
  std::vector<base::string16> names;
  status =
      metadata_factory.ReadDatabaseNames(db_.get(), origin_identifier_, &names);
  if (!status.ok())
    return status;
  ...
  for (auto& name : names) {
    output->emplace_back();
    bool found = false;
    status = metadata_factory.ReadDatabase(db_.get(), origin_identifier_, name,
                                           &output->back(), &found);
