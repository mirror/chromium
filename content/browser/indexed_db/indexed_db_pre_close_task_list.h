...
#ifndef CONTENT_BROWSER_INDEXED_DB_INDEXED_DB_PRE_CLOSE_TASK_LIST_H_
#define CONTENT_BROWSER_INDEXED_DB_INDEXED_DB_PRE_CLOSE_TASK_LIST_H_
...
//
// Assumes ownership by the IndexedDBBackingStore.
class IndexedDBPreCloseTaskList {
 public:
  enum class StopReason {
    // A new connection was to the closing backing store.
    NEW_CONNECTION,
  ...
  // instance. Task can be destructed at any time if Chrome is closing,
  // otherwise Stop(...) will be called.
  class PreCloseTask {
   public:
    ...
    // Tells the task to stop before completion. Task will be destroyed after
    // this call. Can be called at any time.
  ...
  IndexedDBPreCloseTaskList(std::list<std::unique_ptr<PreCloseTask>> tasks,
                            base::OnceClosure on_complete,
                            base::TimeDelta max_run_time,
                            std::unique_ptr<base::Timer> timer);
  ~IndexedDBPreCloseTaskList();
 ...
 private:
  ...
  base::WeakPtrFactory<IndexedDBPreCloseTaskList> ptr_factory_;

  DISALLOW_COPY_AND_ASSIGN(IndexedDBPreCloseTaskList);
};

}  // namespace content
#endif  // CONTENT_BROWSER_INDEXED_DB_INDEXED_DB_PRE_CLOSE_TASK_LIST_H_
