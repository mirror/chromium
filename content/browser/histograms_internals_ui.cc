...
const char kHistogramsChartJs[] = "chart.js";
...
WebUIDataSource* CreateHistogramsHTMLSource() {
  ...
  source->AddResourcePath(kHistogramsChartJs, IDR_HISTOGRAMS_CHART_JS);
...
void HistogramsMessageHandler::HandleRequestHistograms(
  ...
  for (base::HistogramBase* histogram :
       base::StatisticsRecorder::GetSnapshot(query))
    histograms_list.Append(histogram->ToDictionaryValue());
