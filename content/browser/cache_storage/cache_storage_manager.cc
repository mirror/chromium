...
void DeleteOriginDidDeleteDir(
  ...
  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE, base::BindOnce(callback, rv ? blink::kQuotaStatusOk
                                             : blink::kQuotaErrorAbort));
}
...
void CacheStorageManager::DeleteOriginDidClose(
  ...
  quota_manager_proxy_->NotifyStorageModified(
      ...
      blink::kStorageTypeTemporary, -1 * origin_size);
  NotifyCacheListChanged(origin);

  if (IsMemoryBacked()) {
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::BindOnce(callback, blink::kQuotaStatusOk));
    return;
  }
