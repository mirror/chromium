...
void CacheStorageDispatcherHost::OnCacheStorageOpen(
  ...
  base::string16 s16(cache_name);
  std::string s8(base::UTF16ToUTF8(cache_name));

  DLOG(ERROR) << "Opening cache_name: " << cache_name << " UTF8: " << s8
              << " " << (int)s8.at(s8.size()-1)
              << " as UTF16: " << s16
              << " " << (int)s16.at(s16.size()-1)
              ;
...
void CacheStorageDispatcherHost::OnCacheStorageKeysCallback(
  ...
  for (const auto& metadata : cache_index.ordered_cache_metadata()) {
    base::string16 s = base::UTF8ToUTF16(metadata.name);
    DLOG(ERROR) << "Key found as UTF8: " << metadata.name
                << " " << (int)metadata.name.at(metadata.name.size()-1)
                << " as UTF16: " << s
                << " " << (int)s.at(s.size()-1)
                ;
...
void CacheStorageDispatcherHost::OnCacheStorageMatchCallback(
  ...
  std::move(callback).Run(blink::mojom::MatchResult::NewResponse(std::move(*response)));
