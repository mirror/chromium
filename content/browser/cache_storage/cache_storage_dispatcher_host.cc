...
void StopPreservingCache(CacheStorageCacheHandle cache_handle) {
  DLOG(ERROR) << " deleting cache_handle...";
}
...
void CacheStorageDispatcherHost::Open(
  ...
  DLOG(ERROR) << "Opening: " << cache_name;
  context_->cache_manager()->OpenCache(
      origin.GetURL(), base::UTF16ToUTF8(cache_name),
      base::BindOnce(&CacheStorageDispatcherHost::OnOpenCallback, this,
                     std::move(callback)));
}
...
void CacheStorageDispatcherHost::Delete(
  ...
  LOG(ERROR) << "Getting context/origin ..." << cache_name;
  url::Origin origin = bindings_.dispatch_context();
  LOG(ERROR) << "context/origin gotten.";
  if (!OriginCanAccessCacheStorage(origin)) {
    LOG(ERROR) << "Invalid origin.";
    bindings_.ReportBadMessage(kInvalidOriginError);
    return;
  }
  LOG(ERROR) << "Origin is valid, delegating to cache_manager DeleteCache.";
  context_->cache_manager()->DeleteCache(
      origin.GetURL(), base::UTF16ToUTF8(cache_name),
      base::BindOnce(&CacheStorageDispatcherHost::OnDeleteCallback, this,
                     std::move(callback)));
}
...
void CacheStorageDispatcherHost::Match(
  ...
  url::Origin origin = bindings_.dispatch_context();
  DLOG(ERROR) << "CacheStorage match origin: " << origin.Serialize();
  if (!OriginCanAccessCacheStorage(origin)) {
    DLOG(ERROR) << "CacheStorage match bad msg.";
    bindings_.ReportBadMessage(kInvalidOriginError);
    return;
  }
...
void CacheStorageDispatcherHost::OnDeleteCallback(
  ...
  LOG(ERROR) << "Running OnDeleteCallback...";
  std::move(callback).Run(error);
  LOG(ERROR) << "Ran.";
}
...
void CacheStorageDispatcherHost::OnMatchCallback(
  ...
  if (error != CacheStorageError::kSuccess) {
    DLOG(ERROR) << "CacheStorage match failed." << error;
