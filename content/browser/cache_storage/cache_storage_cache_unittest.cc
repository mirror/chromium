...
TEST_P(CacheStorageCacheTestP, PutBadMessage) {
  ...
  std::unique_ptr<base::RunLoop> loop(new base::RunLoop());
  base::Optional<bad_message::BadMessageReason> bad_message_reason;
  cache_->BatchOperation(
      std::vector<CacheStorageBatchOperation>(2, operation),
      base::BindOnce(&CacheStorageCacheTestP::ErrorTypeCallback,
                     base::Unretained(this), base::Unretained(loop.get())),
      base::BindOnce(&OnBadMessage, base::Unretained(&bad_message_reason)));
  loop->Run();

  EXPECT_EQ(CacheStorageError::kErrorStorage, callback_error_);
  EXPECT_EQ(bad_message::CSDH_UNEXPECTED_OPERATION, *bad_message_reason);
}
