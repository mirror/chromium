...
void CacheStorageCache::BatchOperation(
  ...
  if (!safe_space_required.IsValid() || !safe_space_required.IsValid()) {
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::BindOnce(std::move(callback),
                                  CacheStorageError::kErrorBadMessage));
    return;
  }
  uint64_t space_required = safe_space_required.ValueOrDefault(0);
  uint64_t side_data_size = safe_side_data_size.ValueOrDefault(0);
...
void CacheStorageCache::BatchDidGetUsageAndQuota(
  ...
  if (status_code != storage::kQuotaStatusOk ||
      safe_space_required.ValueOrDefault(0) > quota) {
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::BindOnce(std::move(callback),
                                  CacheStorageError::kErrorQuotaExceeded));
    return;
  }
  safe_space_required += side_data_size;
  if (!safe_space_required.IsValid()) {
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::BindOnce(std::move(callback),
                                  CacheStorageError::kErrorBadMessage));
  ...
  bool skip_side_data = safe_space_required.ValueOrDefault(0) > quota;
