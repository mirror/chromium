...
class CONTENT_EXPORT CacheStorageDispatcherHost
 ...
 public:
  ...
  // NOTE: Same CacheStorageDispatcherHost instance be bound to different
  // clients on different origins, each context is kept on BindingSet's context,
  // this guarantees that Browser binds the client with the origin known at the
  // binding time, instead of relying on client to provide its origin at every
  // method call.
 ...
 private:
  ...
  friend class base::RefCountedThreadSafe<CacheStorageDispatcherHost>;
  ...
  void OnDeleteCallback(blink::mojom::CacheStorage::DeleteCallback callback,
  ...
  void OnOpenCallback(blink::mojom::CacheStorage::OpenCallback callback,
  ...
  mojo::StrongAssociatedBindingSet<blink::mojom::CacheStorageCache, url::Origin>
      cache_bindings_;
