...
class CacheStorageManagerTest : public testing::Test {
 public:
  CacheStorageManagerTest()
      : browser_thread_bundle_(TestBrowserThreadBundle::IO_MAINLOOP),
        ...
        origin1_(Origin::Create(GURL("http://example1.com"))),
        origin2_(Origin::Create(GURL("http://example2.com"))) {}
  ...
  bool FlushCacheStorageIndex(const Origin& origin) {
    callback_bool_ = false;
    base::RunLoop loop;
  ...
  bool Open(const Origin& origin, const std::string& cache_name) {
    base::RunLoop loop;
  ...
  bool Has(const Origin& origin, const std::string& cache_name) {
    base::RunLoop loop;
  ...
  bool Delete(const Origin& origin, const std::string& cache_name) {
    base::RunLoop loop;
  ...
  size_t Keys(const Origin& origin) {
    base::RunLoop loop;
  ...
  bool StorageMatch(const Origin& origin,
  ...
  bool StorageMatchAll(const Origin& origin,
                       const GURL& url,
                       const CacheStorageCacheQueryParams& match_params =
                           CacheStorageCacheQueryParams()) {
    ServiceWorkerFetchRequest request;
  ...
  CacheStorage* CacheStorageForOrigin(const Origin& origin) {
    return cache_manager_->FindOrCreateCacheStorage(origin);
  }

  int64_t GetOriginUsage(const Origin& origin) {
    base::RunLoop loop;
  ...
  int64_t GetSizeThenCloseAllCaches(const Origin& origin) {
    base::RunLoop loop;
  ...
  int64_t Size(const Origin& origin) {
    base::RunLoop loop;
    CacheStorage* cache_storage = CacheStorageForOrigin(origin);
  ...
  int64_t GetQuotaOriginUsage(const Origin& origin) {
 ...
 protected:
  ...
  const Origin origin1_;
  const Origin origin2_;
...
TEST_F(CacheStorageManagerTest, BadOriginName) {
  ...
  // escape the directory.
  Origin bad_origin(Origin::Create(
      GURL("http://../../../../../../../../../../../../../../foo")));
...
TEST_P(CacheStorageManagerTestP, GetAllOriginsUsage) {
  ...
  int origin1_index = Origin::Create(usage[0].origin) == origin1_ ? 0 : 1;
  int origin2_index = Origin::Create(usage[1].origin) == origin2_ ? 1 : 0;
...
class CacheStorageQuotaClientTest : public CacheStorageManagerTest {
 protected:
  ...
  int64_t QuotaGetOriginUsage(const Origin& origin) {
    base::RunLoop loop;
  ...
  bool QuotaDeleteOriginData(const Origin& origin) {
    base::RunLoop loop;
...
TEST_P(CacheStorageQuotaClientTestP, QuotaGetOriginsForHost) {
  ...
  EXPECT_TRUE(Open(Origin::Create(GURL("http://example.com:8080")), "foo"));
  EXPECT_TRUE(Open(Origin::Create(GURL("http://example.com:9000")), "foo"));
  EXPECT_TRUE(Open(Origin::Create(GURL("ftp://example.com")), "foo"));
  EXPECT_TRUE(Open(Origin::Create(GURL("http://example2.com")), "foo"));
