...
class SQLitePersistentOriginManifestStore::Backend
 ...
 public:
  ...
  std::unique_ptr<OriginManifest> MakeOriginManifestFromSQLStatement(
 ...
 private:
  ...
  class PendingOperation {
   public:
    ...
    PendingOperation(OperationType op, const OriginManifest& om)
        : op_(op), om_(om) {}

    OperationType op() const { return op_; }
    const OriginManifest& om() const { return om_; }
  ...
  */
  ...
  std::vector<std::unique_ptr<OriginManifest>> origin_manifests_;
...
void SQLitePersistentOriginManifestStore::Backend::Notify(
  ...
  std::vector<std::unique_ptr<OriginManifest>> origin_manifests;
  {
...
bool SQLitePersistentOriginManifestStore::Backend::
  ...
  std::unique_ptr<OriginManifest> origin_manifest =
...
std::unique_ptr<OriginManifest> SQLitePersistentOriginManifestStore::Backend::
    MakeOriginManifestFromSQLStatement(sql::Statement* statement) {
  sql::Statement& smt = *statement;
  if (smt.Step()) {
    std::unique_ptr<OriginManifest> om =
        OriginManifest::CreateOriginManifest(smt.ColumnString(0),   // origin
                                             smt.ColumnString(1),   // version
                                             smt.ColumnString(2));  // json
    om->SetCreationDate(Time::FromInternalValue(smt.ColumnInt64(3)));
    om->SetExpiryDate(Time::FromInternalValue(smt.ColumnInt64(4)));
  ...
  for (PendingOperationsList::iterator it = ops.begin(); it != ops.end();
    ...
    switch (po->op()) {
      case PendingOperation::ORIGIN_MANIFEST_ADD:
        ...
        add_smt.BindInt64(3, po->om().CreationDate().ToInternalValue());
        add_smt.BindInt64(4, po->om().ExpiryDate().ToInternalValue());
        if (!add_smt.Run())
...
void SQLitePersistentOriginManifestStore::Load(
  ...
  if (backend_)
    ...
    loaded_callback.Run(std::vector<std::unique_ptr<OriginManifest>>());
}
