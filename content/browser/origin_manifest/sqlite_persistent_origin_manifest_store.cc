...
void SQLitePersistentOriginManifestStore::Backend::LoadAndNotifyInBackground(
  ...
  /*
  U MA_HISTOGRAM_CUSTOM_TIMES("OriginManifest.TimeLoadDBQueueWait",
...
void SQLitePersistentOriginManifestStore::Backend::
  ...
  /*
  U MA_HISTOGRAM_CUSTOM_TIMES("OriginManifest.TimeKeyLoadDBQueueWait",
                             base::Time::Now() - posted_at,
                             base::TimeDelta::FromMilliseconds(1),
                             base::TimeDelta::FromMinutes(1), 50);
  */
...
void SQLitePersistentOriginManifestStore::Backend::
  ...
  /*
  U MA_HISTOGRAM_CUSTOM_TIMES("OriginManifest.TimeKeyLoadTotalWait",
...
void SQLitePersistentOriginManifestStore::Backend::ReportMetricsInBackground() {
  /*
  U MA_HISTOGRAM_CUSTOM_TIMES("OriginManifest.TimeLoad",
...
void SQLitePersistentOriginManifestStore::Backend::ReportMetrics() {
  ...
  {
    base::AutoLock locked(metrics_lock_);
    U MA_HISTOGRAM_CUSTOM_TIMES("OriginManifest.PriorityBlockingTime",
                               priority_wait_duration_,
                               base::TimeDelta::FromMilliseconds(1),
                               base::TimeDelta::FromMinutes(1), 50);

    U MA_HISTOGRAM_COUNTS_100("OriginManifest.PriorityLoadCount",
                             total_priority_requests_);

    U MA_HISTOGRAM_COUNTS_10000("OriginManifest.NumberOfLoadedOriginManifests",
...
bool SQLitePersistentOriginManifestStore::Backend::InitializeDatabase() {
  ...
  if (base::GetFileSize(path_, &db_size))
    U MA_HISTOGRAM_COUNTS_1M("OriginManifest.DBSizeInKB", db_size / 1024);
  */
  ...
  /*
  U MA_HISTOGRAM_CUSTOM_TIMES("OriginManifest.TimeInitializeDB",
                             base::Time::Now() - start,
                             base::TimeDelta::FromMilliseconds(1),
                             base::TimeDelta::FromMinutes(1), 50);
  */
  ...
  /*
  U MA_HISTOGRAM_CUSTOM_TIMES("OriginManifest.TimeLoadOrigins",
                             base::Time::Now() - start,
                             base::TimeDelta::FromMilliseconds(1),
                             base::TimeDelta::FromMinutes(1), 50);
  */
...
bool SQLitePersistentOriginManifestStore::Backend::
  ...
  sql::Statement smt;
  smt.Assign(db_->GetCachedStatement(SQL_FROM_HERE,
...
bool SQLitePersistentOriginManifestStore::Backend::EnsureDatabaseVersion() {
  ...
  if (cur_version < kCurrentVersionNumber) {
    // U MA_HISTOGRAM_COUNTS_100("OriginManifest.CorruptMetaTable", 1);
    ...
    if (!sql::Connection::Delete(path_) || !db_->Open(path_) ||
        !meta_table_.Init(db_.get(), kCurrentVersionNumber,
                          kCompatibleVersionNumber)) {
      /*
      U MA_HISTOGRAM_COUNTS_100("OriginManifest.CorruptMetaTableRecoveryFailed",
      1);
      */
...
void SQLitePersistentOriginManifestStore::Backend::Commit() {
  ...
  /*
  U MA_HISTOGRAM_ENUMERATION("OriginManifest.BackingStoreUpdateResults",
                            succeeded ? 0 : 1, 2);
  */
}
...
void SQLitePersistentOriginManifestStore::Backend::KillDatabase() {
  ...
  if (db_) {
    ...
    // U MA_HISTOGRAM_BOOLEAN("OriginManifest.KillDatabaseResult", success);
    meta_table_.Reset();
    db_.reset();
  }
}
...
SQLitePersistentOriginManifestStore::Backend::DeleteSessionCookiesOnStartup() {
  ...
  U MA_HISTOGRAM_TIMES("Cookie.Startup.TimeSpentDeletingCookies",
                      base::Time::Now() - start_time);
  U MA_HISTOGRAM_COUNTS_1M("Cookie.Startup.NumberOfCookiesDeleted",
