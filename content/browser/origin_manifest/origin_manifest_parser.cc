  ...
  for (base::detail::dict_iterator_proxy::iterator it =
    ...
    switch (OriginManifestParser::GetDirectiveType(it->first)) {
      ...
      case DirectiveType::kContentSecurityPolicy:
        OriginManifestParser::ParseContentSecurityPolicy(origin_manifest.get(),
                                                         std::move(it->second));
        break;
...
blink::OriginManifest::ActivationType
OriginManifestParser::GetCSPActivationType(const bool value) {
  if (value)
