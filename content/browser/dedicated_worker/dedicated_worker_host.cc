...
class DedicatedWorkerFactoryImpl : public blink::mojom::DedicatedWorkerFactory {
 public:
  ...
  void CreateDedicatedWorker(
    ...
    // The worker's origin must either match the creating document's origin, or
    // be unique.
    if (!origin.unique() && !parent_context_origin_.IsSameOriginWith(origin)) {
      mojo::ReportBadMessage("Invalid dedicated worker origin");
      return;
    }
