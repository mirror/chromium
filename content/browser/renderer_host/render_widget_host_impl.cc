...
bool RenderWidgetHostImpl::OnMessageReceived(const IPC::Message &msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderWidgetHostImpl, msg)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_Resize_ACK, OnResizeACK)
...
void RenderWidgetHostImpl::WasShown(const ui::LatencyInfo& latency_info) {
  ...
  // 3. WasHidden
  // 4. OnResizeACK from (1) processed. Does NOT invoke WasResized as view
...
void RenderWidgetHostImpl::OnResizeACK(
    const ViewHostMsg_Resize_ACK_Params& params) {
  TRACE_EVENT0("renderer_host", "RenderWidgetHostImpl::OnResizeACK");
  ...
  bool is_resize_ack =
      ViewHostMsg_Resize_ACK_Flags::is_resize_ack(params.flags);
  ...
  bool is_repaint_ack =
      ViewHostMsg_Resize_ACK_Flags::is_repaint_ack(params.flags);
  if (is_repaint_ack) {
  ...
  UMA_HISTOGRAM_TIMES("MPArch.RWH_OnMsgResizeACK", delta);
}
