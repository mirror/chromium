  ...
  if (enable_surface_synchronization_) {
    if (view_) {
      ...
      first_parent_id_after_navigation_ =
          view_->GetLocalSurfaceId().parent_id();
      WasResized();
    }
  } else {
...
void RenderWidgetHostImpl::SubmitCompositorFrame(
  ...
  } else if (enable_surface_synchronization_) {
...
void RenderWidgetHostImpl::OnFirstSurfaceActivation(
  ...
  uint32_t latest_parent_id = view_->GetLocalSurfaceId().parent_id();
