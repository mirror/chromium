...
RenderWidgetHostImpl::RenderWidgetHostImpl(RenderWidgetHostDelegate* delegate,
  ...
  legacy_widget_input_handler_ =
      base::MakeUnique<LegacyIPCWidgetInputHandler>(this, routing_id_);
...
bool RenderWidgetHostImpl::Send(IPC::Message* msg) {
  CHECK(IPC_MESSAGE_ID_CLASS(msg->type()) != InputMsgStart);
...
void RenderWidgetHostImpl::ScrollFocusedEditableNodeIntoRect(
    const gfx::Rect& rect) {
  GetWidgetInputHandler()->ScrollFocusedEditableNodeIntoRect(rect);
}

void RenderWidgetHostImpl::MoveCaret(const gfx::Point& point) {
  GetWidgetInputHandler()->MoveCaret(point);
}
