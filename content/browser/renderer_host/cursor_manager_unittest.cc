...
class CursorManagerTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    top_view_ = new MockRenderWidgetHostViewForCursors(widget_host_, true);
  }

  RenderWidgetHostImpl* MakeNewWidgetHost() {
    int32_t routing_id = process_host_->GetNextRoutingID();
    return new RenderWidgetHostImpl(&delegate_, process_host_, routing_id,
                                    false);
  }
