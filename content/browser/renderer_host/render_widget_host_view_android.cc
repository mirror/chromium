...
void RenderWidgetHostViewAndroid::OnFrameMetadataUpdated(
  ...
  float css_to_device = IsUseZoomForDSFEnabled() ? 1.f : dip_scale;
  float top_controls_pix = frame_metadata.top_controls_height * css_to_device;
  // |top_content_offset| is in device scale if --use-zoom-for-dsf is enabled.
  // Otherwise, it is in CSS scale.
  float top_content_offset = frame_metadata.top_controls_height *
                             frame_metadata.top_controls_shown_ratio;
  // |top_shown_pix| is in device scale.
  float top_shown_pix = top_content_offset * css_to_device;
  ...
  float bottom_controls_pix =
      frame_metadata.bottom_controls_height * css_to_device;
