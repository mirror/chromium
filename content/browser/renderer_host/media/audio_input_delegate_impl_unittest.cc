...
TEST_F(AudioInputDelegateTest, EnableAndDisableDebugRecording) {
  int session_id = Open("default", "default");

  std::unique_ptr<media::AudioInputDelegate> delegate =
      CreateDelegate(10, session_id);
  EXPECT_NE(delegate, nullptr);
  ...
  delegate->EnableDebugRecording(
      base::FilePath(base::FilePath::StringType("test")));
  delegate->DisableDebugRecording();

  EXPECT_CALL(event_handler_, MockOnStreamCreated(kStreamId, false));
  base::RunLoop().RunUntilIdle();
  Mock::VerifyAndClear(&event_handler_);
  delegate.reset();
  base::RunLoop().RunUntilIdle();
}
