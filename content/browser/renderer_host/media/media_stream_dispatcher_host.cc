...
void MediaStreamDispatcherHost::Create(
  ...
  auto dispatcher_host = base::MakeUnique<MediaStreamDispatcherHost>(
      render_process_id, salt, media_stream_manager);
  base::Closure error_handler =
      base::Bind(&MediaStreamDispatcherHost::OnConnectionError,
                 base::Unretained(dispatcher_host.get()));
  auto binding =
      mojo::MakeStrongBinding(std::move(dispatcher_host), std::move(request));
...
void MediaStreamDispatcherHost::OnConnectionError() {
  DVLOG(1) << __func__;
