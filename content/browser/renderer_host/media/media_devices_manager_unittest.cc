...
TEST_F(MediaDevicesManagerTest, SubscribeDeviceChanges) {
  EXPECT_CALL(*audio_manager_, MockGetAudioOutputDeviceNames(_)).Times(7);
  EXPECT_CALL(*video_capture_device_factory_, MockGetDeviceDescriptors())
      .Times(7);
  EXPECT_CALL(*audio_manager_, MockGetAudioInputDeviceNames(_)).Times(7);
  ...
  for (size_t i = 0; i < NUM_MEDIA_DEVICE_TYPES; ++i) {
    ...
    if (type == MEDIA_DEVICE_TYPE_AUDIO_INPUT)
      EXPECT_EQ(num_audio_input_devices, notification.size());
    else if (type == MEDIA_DEVICE_TYPE_VIDEO_INPUT)
      EXPECT_EQ(num_video_input_devices, notification.size());
    else if (type == MEDIA_DEVICE_TYPE_AUDIO_OUTPUT)
      EXPECT_EQ(num_audio_output_devices, notification.size());

    // Unsubscribe.
    media_devices_manager_->UnsubscribeDeviceChangeNotifications(
        subscription_id);

    // Simulate further device changes. Device-change events notifications will
    // no longer be received as we have unsubscribed.
    ...
    if (type == MEDIA_DEVICE_TYPE_AUDIO_INPUT)
      EXPECT_NE(num_audio_input_devices, notification.size());
    else if (type == MEDIA_DEVICE_TYPE_VIDEO_INPUT)
      EXPECT_NE(num_video_input_devices, notification.size());
    else if (type == MEDIA_DEVICE_TYPE_AUDIO_OUTPUT)
      EXPECT_NE(num_audio_output_devices, notification.size());
  }
}
