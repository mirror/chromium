...
void ServiceVideoCaptureProvider::GetDeviceInfosAsync(
  ...
  auto scoped_callback_runner = media::ScopedCallbackRunner(
      std::move(result_callback), std::vector<media::VideoCaptureDeviceInfo>());
  // Wrap |scoped_callback_runner| in a base::RepeatingCallback, because
  // |device_factory_->GetDeviceInfos()| does not yet support
  // base::OnceCallback.
  // TODO(chfremer): Simplify this as soon as the Mojo generator is switched
  // over to OnceCallback. https://crbug.com/714018
  auto callback = base::Bind(
      [](GetDeviceInfosCallback result_callback,
         const std::vector<media::VideoCaptureDeviceInfo>& infos) {
        std::move(result_callback).Run(infos);
      },
      base::Passed(&scoped_callback_runner));
  device_factory_->GetDeviceInfos(callback);
}
