...
TEST_F(ServiceVideoCaptureProviderTest,
  ...
  EXPECT_CALL(mock_device_factory_, GetDeviceInfos(_))
      .WillOnce(Invoke(
          [&callback_to_be_called_by_service,
           &wait_for_call_to_arrive_at_service](
              const video_capture::mojom::DeviceFactory::GetDeviceInfosCallback&
                  callback) {
            // Hold on to the callback so we can drop it later.
            callback_to_be_called_by_service = callback;
