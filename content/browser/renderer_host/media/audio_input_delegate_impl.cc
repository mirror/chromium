...
class AudioInputDelegateImpl::ControllerEventHandler
 ...
 public:
  explicit ControllerEventHandler(
      ...
      : stream_id_(stream_id), weak_delegate_(weak_delegate) {}
  ...
  void OnLog(const std::string& message) override {
    LogMessage(stream_id_, message, false);
  }

  void OnMuted(bool is_muted) override {
    LogMessage(stream_id_,
               base::StringPrintf("OnMuted: State changed to:%s muted",
                                  (is_muted ? "" : " not")),
               true);
...
std::unique_ptr<media::AudioInputDelegate> AudioInputDelegateImpl::Create(
  ...
  const MediaStreamDevice* device =
      media_stream_manager->audio_input_device_manager()->GetOpenedDeviceById(
          session_id);
  if (!device) {
  ...
  auto writer = AudioInputSyncWriter::Create(
  ...
  return std::make_unique<AudioInputDelegateImpl>(
      subscriber, audio_manager, mirroring_manager, user_input_monitor,
      audio_parameters, std::move(writer), std::move(foreign_socket),
      std::move(audio_log),
#if defined(OS_CHROMEOS)
  ...
  if (WebContentsMediaCaptureId::Parse(device_id, nullptr)) {
    ...
    controller_ = media::AudioInputController::CreateForStream(
        ...
        WebContentsAudioInputStream::Create(
            ...
            mirroring_manager),
        writer_.get(), user_input_monitor, audio_parameters);
    DCHECK(controller_);
...
AudioInputDelegateImpl::~AudioInputDelegateImpl() {
  ...
  LogMessage(stream_id_, "CloseAndDeleteStream", true);
  ...
  controller_->Close(base::BindOnce(
      [](int stream_id, std::unique_ptr<ControllerEventHandler>,
        ...
        LogMessage(stream_id, "DeleteEntry: stream is now closed", true);
      },
...
void AudioInputDelegateImpl::OnRecordStream() {
  LogMessage(stream_id_, "OnRecordStream", true);
  controller_->Record();
...
void AudioInputDelegateImpl::EnableDebugRecording(
  ...
  controller_->EnableDebugRecording(
      file_name.AddExtension(IntToStringType(stream_id_)));
}

#undef IntToStringType

void AudioInputDelegateImpl::DisableDebugRecording() {
  controller_->DisableDebugRecording();
}
