...
url::Origin GetOrigin(int render_process_id, int render_frame_id) {
  RenderFrameHost* frame =
      RenderFrameHost::FromID(render_process_id, render_frame_id);
  if (!frame)
    return url::Origin();
  ...
  if (media::AudioDeviceDescription::UseSessionIdToSelectDevice(session_id,
    ...
    if (info && !info->device.matched_output_device_id.empty()) {
      ...
      BrowserThread::PostTaskAndReplyWithResult(
          ...
          base::BindOnce(&GetOrigin, render_process_id_, render_frame_id),
...
void AudioOutputAuthorizationHandler::AccessChecked(
  ...
  if (!has_access || security_origin.unique()) {
