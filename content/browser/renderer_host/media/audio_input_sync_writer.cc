...
struct AudioInputSyncWriter::OverflowData {
...
bool AudioInputSyncWriter::PushDataToFifo(const AudioBus* data,
  ...
  overflow_data_.emplace_back(new OverflowData(
      volume, key_pressed, capture_time, std::move(audio_bus)));
...
bool AudioInputSyncWriter::WriteDataFromFifoToSharedMemory() {
  ...
  while (data_it != overflow_data_.end() &&
    ...
    WriteParametersToCurrentSegment((*data_it)->volume_,
