...
AudioInputSyncWriter::OverflowData::~OverflowData() {}
AudioInputSyncWriter::OverflowData::OverflowData(
    AudioInputSyncWriter::OverflowData&& other) = default;
...
bool AudioInputSyncWriter::PushDataToFifo(const AudioBus* data,
  ...
  std::unique_ptr<OverflowData> overflowdata =
      base::WrapUnique(new OverflowData);
  overflowdata->volume = volume;
  overflowdata->key_pressed = key_pressed;
  overflowdata->capture_time = capture_time;
  // Push audio data to fifo.
  std::unique_ptr<AudioBus> audio_bus =
      AudioBus::Create(data->channels(), data->frames());
  data->CopyTo(audio_bus.get());
  overflowdata->overflow_buses_ = std::move(audio_bus);
  overflow_data_.push_back(std::move(overflowdata));
...
bool AudioInputSyncWriter::WriteDataFromFifoToSharedMemory() {
  ...
  while (data_it != overflow_data_.end() &&
    ...
    WriteParametersToCurrentSegment((*data_it)->volume, (*data_it)->key_pressed,
                                    (*data_it)->capture_time);

    // Copy data from the fifo into shared memory using pre-allocated audio
    // buses.
    (*data_it)->overflow_buses_->CopyTo(
        audio_buses_[current_segment_id_].get());
