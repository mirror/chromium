...
AudioInputSyncWriter::AudioInputSyncWriter(
    ...
    : socket_(std::move(socket)),
      ...
      shared_memory_segment_size_(shared_memory_->requested_size() /
  ...
  CHECK(socket_);
  CHECK(foreign_socket_);
  CHECK(shared_memory_);
  CHECK_GT(shared_memory_segment_count, 0);
...
std::unique_ptr<AudioInputSyncWriter> AudioInputSyncWriter::Create(
  ...
  if (!requested_memory_size.IsValid() ||
      !shared_memory->CreateAndMapAnonymous(requested_memory_size.ValueOrDie()))
    return nullptr;

  auto socket = base::MakeUnique<base::CancelableSyncSocket>();
  auto foreign_socket = base::MakeUnique<base::CancelableSyncSocket>();
  if (!base::CancelableSyncSocket::CreatePair(socket.get(),
                                              foreign_socket.get()))
    return nullptr;

  return base::MakeUnique<AudioInputSyncWriter>(
      std::move(shared_memory), std::move(socket), std::move(foreign_socket),
      shared_memory_segment_count, params);
}

std::unique_ptr<base::CancelableSyncSocket>
AudioInputSyncWriter::TakeForeignSocket() {
