    ...
    for (const MediaStreamDevice& device : request->devices) {
      ...
      if (device.id == source_id && device.type == stream_type) {
        ...
        if (labeled_request.second->requester) {
          // TODO(c.padhi): Remove this when DeviceStopped's input type is
...
void MediaStreamManager::FinalizeGenerateStream(const std::string& label,
  ...
  for (const MediaStreamDevice& device : request->devices) {
    if (IsAudioInputMediaType(device.type)) {
      // TODO(c.padhi): Remove this when StreamGenerated's input types are
      // changed to MediaStreamDevice, see https://crbug.com/760493.
      StreamDeviceInfo device_info;
      device_info.device = device;
      device_info.session_id = device.session_id;
...
void MediaStreamManager::FinalizeOpenDevice(const std::string& label,
                                            DeviceRequest* request) {
  // TODO(c.padhi): Remove this when DeviceOpened's input type is changed to
  // MediaStreamDevice, see https://crbug.com/760493.
  StreamDeviceInfo device_info;
  device_info.device = request->devices.front();
...
void MediaStreamManager::StopMediaStreamFromBrowser(const std::string& label) {
  ...
  if (request->requester) {
    for (const MediaStreamDevice& device : request->devices) {
      // TODO(c.padhi): Remove this when DeviceStopped's input type is changed
