...
// JavaScript level.
class JsLevelVideoCaptureBrowserTest : public ContentBrowserTest {
 protected:
...
IN_PROC_BROWSER_TEST_F(JsLevelVideoCaptureBrowserTest,
  ...
  BrowserThread::PostTask(
      content::BrowserThread::IO, FROM_HERE, base::Bind([]() {
        BrowserChildProcessHost* host = BrowserChildProcessHost::FromTag(
            video_capture::mojom::kServiceName);
        if (!host) {
          LOG(WARNING) << "Unable to find video capture service process.";
          NOTREACHED();
        }
