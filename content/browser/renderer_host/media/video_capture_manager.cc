...
int VideoCaptureManager::Open(const MediaStreamDevice& device) {
  ...
  EmitLogMessage((std::ostringstream()
                  << "VideoCaptureManager::Open, device.name = " << device.name
                  << ", device.id = " << device.id
...
void VideoCaptureManager::Close(int capture_session_id) {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  EmitLogMessage((std::ostringstream()
                  << "VideoCaptureManager::Close, capture_session_id = "
                  << capture_session_id)
                     .str(),
                 1);
...
void VideoCaptureManager::OnDeviceLaunched(VideoCaptureController* controller) {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  EmitLogMessage(
      (std::ostringstream() << "Launching device has succeeded. device_id = "
...
void VideoCaptureManager::OnDeviceLaunchFailed(
    VideoCaptureController* controller) {
  EmitLogMessage(
      (std::ostringstream() << "Launching device has failed. device_id = "
                            << controller->device_id())
          .str(),
      1);
  controller->OnError();
...
void VideoCaptureManager::OnDeviceConnectionLost(
  ...
  EmitLogMessage(
      (std::ostringstream()
       << "Lost connection to device. device_id = " << controller->device_id())
          .str(),
      1);
  controller->OnError();
}
...
void VideoCaptureManager::ConnectClient(
  ...
  EmitLogMessage(
      (std::ostringstream()
       << "ConnectClient: session_id = " << session_id << ", request: "
...
void VideoCaptureManager::DisconnectClient(
  ...
  EmitLogMessage(
      (std::ostringstream() << "DisconnectClient: session_id = " << session_id)
          .str(),
      1);
...
bool VideoCaptureManager::GetDeviceSupportedFormats(
  ...
  EmitLogMessage(
      (std::ostringstream()
       << "GetDeviceSupportedFormats for device: " << it->second.name)
          .str(),
      1);
...
bool VideoCaptureManager::GetDeviceFormatsInUse(
  ...
  if (it == sessions_.end())
    return false;
  EmitLogMessage((std::ostringstream()
                  << "GetDeviceFormatsInUse for device: " << it->second.name)
                     .str(),
                 1);
...
void VideoCaptureManager::OnDeviceInfosReceived(
  ...
  oss << "VideoCaptureManager::OnDeviceInfosReceived: Recevied "
...
void VideoCaptureManager::DestroyControllerIfNoClients(
  ...
  if (!controller->HasActiveClient() && !controller->HasPausedClient()) {
    EmitLogMessage((std::ostringstream()
                    << "VideoCaptureManager stopping device (stream_type = "
