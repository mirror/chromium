...
TEST_F(RenderProcessHostUnitTest,
  ...
  // it's unsuitable.
  //
  // When in --site-per-process mode, the SW process is locked to the foo.com
  // origin, and future SiteInstances for that origin are always allowed to
  // reuse processes with a compatible origin lock; this takes precedence over
  // the ContentBrowserClient checks.  So in this case, the reuse would be
  // allowed.
  scoped_refptr<SiteInstanceImpl> site_instance =
      SiteInstanceImpl::CreateForURL(browser_context(), kUrl);
  if (!AreAllSitesIsolatedForTesting())
    ...
    EXPECT_EQ(sw_host, site_instance->GetProcess());
