...
void DelegatedFrameHost::MaybeCreateResizeLock() {
  ...
  client_->DelegatedFrameHostDesiredSize(&desired_size_in_dip, nullptr);
  if (desired_size_in_dip.IsEmpty())
    return;
  if (desired_size_in_dip == current_frame_size_in_dip_)
    return;
...
void DelegatedFrameHost::WasResized() {
  const viz::SurfaceId* primary_surface_id =
      client_->DelegatedFrameHostGetLayer()->GetPrimarySurfaceId();
  gfx::Size new_size_in_dip;
  client_->DelegatedFrameHostDesiredSize(&new_size_in_dip, nullptr);
...
void DelegatedFrameHost::UpdateGutters() {
  ...
  client_->DelegatedFrameHostDesiredSize(&size_in_dip, nullptr);
...
void DelegatedFrameHost::GetRequestedRendererSize(gfx::Size* size_in_dip,
                                                  float* scale_factor) const {
  client_->DelegatedFrameHostDesiredSize(size_in_dip, scale_factor);
  if (resize_lock_)
    *size_in_dip = resize_lock_->expected_size();
}
