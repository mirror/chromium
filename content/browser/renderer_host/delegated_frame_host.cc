...
void DelegatedFrameHost::WasResized() {
  ...
  viz::SurfaceInfo surface_info(surface_id, 1.0f, desired_size);
  ImageTransportFactory* factory = ImageTransportFactory::GetInstance();
...
void DelegatedFrameHost::CheckResizeLock() {
  ...
  //// Since we got the size we were looking for, unlock the compositor. But
  ///delay / the release of the lock until we've kicked a frame with the new
  ///texture, to / avoid resizing the UI before we have a chance to draw a
  ///"good" frame.
...
void DelegatedFrameHost::OnCompositingLockStateChanged(
  ...
  //  // the UI to produce a frame before locking it again, so we don't lock
  //  here.
  //  // We release the |resize_lock_| though to allow any other resizes that
  //  are
  ...
  //  // since we're guttering anyways, but not unlimited number of frames as
  //  that
