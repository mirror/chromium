...
class CONTENT_EXPORT RenderViewHostImpl : public RenderViewHost,
 ...
 public:
  ...
  // This function ensures that a particular RenderViewHost never dispatches
  // these events more than once.  For example, if a RenderViewHost transitions
  // from active to inactive after a cross-process navigation (where it no
  // longer has a main frame RenderFrameHost), and then back to active after
  // another cross-process navigation, this function will filter out the second
  // notification.
