...
RenderProcessHost* RenderProcessHostImpl::GetProcessHostForSiteInstance(
  ...
  switch (site_instance->GetBaseProcessReusePolicy()) {
    case SiteInstanceImpl::ProcessReusePolicy::DEFAULT:
      if (!site_instance->IsServiceWorkerReusableProcessPolicy()) {
        // If the site is NOT for a service worker, reuse the existing service
        // worker only processes. We don't reuse the existing one for service
        // worker instance here because REUSE_PENDING_OR_COMMITTED_SITE is used
        // when we want to reuse the existing one for the service worker.
    ...
    case SiteInstanceImpl::ProcessReusePolicy::PROCESS_PER_SITE:
      // We don't use IS_SERVICE_WORKER_REUSABLE_PROCESS flag for the site which
      // should be hosted in the same host.
      DCHECK(!site_instance->IsServiceWorkerReusableProcessPolicy());
    ...
    case SiteInstanceImpl::ProcessReusePolicy::USE_DEFAULT_SUBFRAME_PROCESS:
      ...
      // We don't use IS_SERVICE_WORKER_REUSABLE_PROCESS flag for subframes.
      DCHECK(!site_instance->IsServiceWorkerReusableProcessPolicy());
    ...
    case SiteInstanceImpl::ProcessReusePolicy::REUSE_PENDING_OR_COMMITTED_SITE:
      ...
      if (!render_process_host) {
        // Try to reuse the existing service worker only process.
        render_process_host = ReusableServiceWorkerProcessTracker::TakeForSite(
            browser_context, site_url);
      }
      break;
