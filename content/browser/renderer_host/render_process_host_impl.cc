...
// This class tracks this 'unmatched' service worker processes untill the
// process will be reused for a navigation to the site.
class UnmatchedServiceWorkerProcessTracker
 ...
 public:
  ...
  static RenderProcessHost* MatchWithSite(BrowserContext* browser_context,
    ...
    if (!tracker)
      return nullptr;
    return tracker->TakeFreshestProcessesForSite(site_url);
  }
 ...
 private:
  ...
  RenderProcessHost* TakeFreshestProcessesForSite(const GURL& site_url) {
  ...
  RenderProcessHost* FindFreshestProcessesForSite(const GURL& site_url) const {
    for (auto rit = site_process_set_.crbegin();
         rit != site_process_set_.crend(); ++rit) {
      if (rit->first == site_url)
        return RenderProcessHost::FromID(rit->second);
    }
    return nullptr;
  }
...
RenderProcessHost* RenderProcessHostImpl::GetProcessHostForSiteInstance(
  ...
  // except for the case where the poclicy is DEFAULT and the site instance is
