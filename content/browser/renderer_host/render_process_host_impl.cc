...
void* TryReserveAddressSpace(base::ProcessHandle process, size_t size) {
  ...
  if (avail_size < size)
    return nullptr;
  return VirtualAllocEx(process, nullptr, size, MEM_RESERVE, PAGE_NOACCESS);
}
...
class RendererSandboxedProcessLauncherDelegate
 ...
 public:
  ...
  void PostSpawnTarget(base::ProcessHandle process) override {
    ...
    // Fragmentation of the small address space makes it likely that these
    // allocations will fail in the renderer process.
    if (base::win::OSInfo::GetInstance()->wow64_status() !=
        base::win::OSInfo::WOW64_ENABLED) {
      const size_t kReservationSize = 512 * 1024 * 1024;
      if (!TryReserveAddressSpace(process, kReservationSize)) {
