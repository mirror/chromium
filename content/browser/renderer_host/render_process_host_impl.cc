...
class ReusableServiceWorkerProcessTracker : public base::SupportsUserData::Data,
 ...
 public:
  ...
  static RenderProcessHost* FindForSite(BrowserContext* browser_context,
                                        const GURL& site_url) {
 ...
 private:
  ...
  void RenderProcessHostDestroyed(RenderProcessHost* host) override {
    if (process_to_site_map_.find(host->GetID()) == process_to_site_map_.end())
      return;
...
RenderProcessHost* RenderProcessHostImpl::GetProcessHostForSiteInstance(
  ...
  switch (process_reuse_policy) {
    ...
    case SiteInstanceImpl::ProcessReusePolicy::DEFAULT:
      render_process_host = ReusableServiceWorkerProcessTracker::FindForSite(
          browser_context, site_url);
      break;
    default:
      break;
  ...
  if (!render_process_host) {
    LOG(ERROR) << "create a new process " << site_url;
