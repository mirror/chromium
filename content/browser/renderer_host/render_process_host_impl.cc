...
bool RenderProcessHostImpl::Init() {
  ...
  // Although we called PreEstablishGpuChannel() implicitly in
  // RenderProcessHostImpl constructor when we create a GpuClient, we want to
  // call it again in Init(), because a renderer may crash and call Init()
  // during relaunch, and we want to pre-establish a GPU channel again if the
  // previously cached channel has been consumed.
  if (gpu_client_)
