...
RenderWidgetHostViewBase* RenderWidgetHostInputEventRouter::FindEventTarget(
  ...
  // The compositor is currently only set on Mac and Aura.
  // TODO(kenrb): Why is it sometimes null when we get events? Is this a race
  // around navigations?
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableViz) &&
    ...
    LOG(ERROR) << " ** Point (" << gfx::ToFlooredInt(point.x()) << ", "
               << gfx::ToFlooredInt(point.y()) << "); PointInRoot ("
               << gfx::ToFlooredInt(point_in_root.x()) << ", "
               << gfx::ToFlooredInt(point_in_root.y()) << ")";
    const auto& display_hit_test_query_map =
        GetHostFrameSinkManager()->display_hit_test_query();
    const auto iter =
        display_hit_test_query_map.find(compositor_->frame_sink_id());
