...
void RenderWidgetHostInputEventRouter::DispatchMouseWheelEvent(
  ...
  if (root_view->wheel_scroll_latching_enabled()) {
    if (mouse_wheel_event.phase == blink::WebMouseWheelEvent::kPhaseBegan) {
      ...
      wheel_target_.delta = ComputeTargetToRootDelta(
          root_view, target, mouse_wheel_event.PositionInWidget());
    } else if (!wheel_target_.target &&
  ...
  target->ProcessMouseWheelEvent(mouse_wheel_event, latency);
...
void RenderWidgetHostInputEventRouter::EventAckNoTarget(
    RenderWidgetHostViewBase* root_view,
...
gfx::Vector2dF RenderWidgetHostInputEventRouter::ComputeTargetToRootDelta(
    RenderWidgetHostViewBase* root_view,
    RenderWidgetHostViewBase* target,
    const gfx::PointF& point_in_target) const {
  gfx::PointF point_in_root;
  target->TransformPointToCoordSpaceForView(point_in_target, root_view,
                                            &point_in_root);
  return point_in_target - point_in_root;
}
