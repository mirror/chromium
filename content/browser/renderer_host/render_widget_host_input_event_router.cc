...
void RenderWidgetHostInputEventRouter::RouteTouchscreenGestureEvent(
  ...
  // These gesture events should always a unique_touch_event_id of 0.
  // In this case, or if there is an error that means our unique_touch_event_id
  // has no associated surface, we recalculate.
  bool no_gesture_target =
      event->unique_touch_event_id != 0 && no_matching_id && is_gesture_start;
  DCHECK(!no_gesture_target);
  ...
  if (event->unique_touch_event_id == 0 || no_matching_id && is_gesture_start) {
    gfx::Point transformed_point;
