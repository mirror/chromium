...
void RenderWidgetTargeter::FindTargetAndDispatch(
  ...
  RenderWidgetTargetResult result =
      delegate_->FindTargetSynchronously(root_view, event, &transformed_point);
  if (!result.view)
    return;
  ...
  if (result.should_query_view) {
    DCHECK_EQ(root_view, target);
    QueryClient(root_view, target, *event_ptr, latency);
  } else {
    FoundTarget(root_view, target, *event_ptr, latency);
  }
}
...
void RenderWidgetTargeter::QueryClient(RenderWidgetHostViewBase* root_view,
  ...
  const auto mouse_event = Convert(
      root_view, target, static_cast<const blink::WebMouseEvent&>(event));
...
void RenderWidgetTargeter::FoundFrameSinkId(
  ...
  if (view == target.get()) {
    FoundTarget(root_view.get(), view, event, latency);
  } else {
    QueryClient(root_view.get(), view, event, latency);
  }
}

void RenderWidgetTargeter::FoundTarget(RenderWidgetHostViewBase* root_view,
                                       RenderWidgetHostViewBase* target,
                                       const blink::WebInputEvent& event,
                                       const ui::LatencyInfo& latency) {
  if (!root_view)
  ...
  blink::WebMouseEvent mouse_event = Convert(
      root_view, target, static_cast<const blink::WebMouseEvent&>(event));
  if (mouse_event.GetType() != blink::WebInputEvent::kUndefined)
