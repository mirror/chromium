...
void WebDatabaseHostImpl::GetSpaceAvailable(
  ...
  // quota_manager is only available on the IO thread.
  DCHECK(db_tracker_->task_runner()->RunsTasksInCurrentSequence());
  DCHECK(db_tracker_->quota_manager_proxy());
  BrowserThread::PostTask(
      BrowserThread::IO, FROM_HERE,
      base::BindOnce(&WebDatabaseHostImpl::DatabaseGetSpaceAvailableOnIOThread,
...
void WebDatabaseHostImpl::DatabaseGetSpaceAvailableOnIOThread(
    const url::Origin& origin,
    GetSpaceAvailableCallback callback,
  ...
  if (!IsOriginValid(origin)) {
    bad_message_callback.Run("Invalid Origin.");
    return;
  }
  ...
  quota_manager->GetUsageAndQuota(
      ...
      base::Bind(&WebDatabaseHostImpl::DatabaseDidGetUsageAndQuota,
                 base::Unretained(this), base::Passed(std::move(callback))));
}

void WebDatabaseHostImpl::DatabaseDidGetUsageAndQuota(
    GetSpaceAvailableCallback callback,
  ...
  // We need to make the mojo callback on the same sequence the recieved the
  // inbound call.
  db_tracker_->task_runner()->PostTask(
      FROM_HERE, base::BindOnce(
                     [](GetSpaceAvailableCallback callback, int64_t available) {
                       std::move(callback).Run(available);
                     },
                     std::move(callback), available));
}
