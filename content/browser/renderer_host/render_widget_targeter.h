...
class RenderWidgetTargeter {
 ...
 private:
  ...
  void AsyncHitTestTimedOut(const blink::WebInputEvent& event,
                            const ui::LatencyInfo& latency);
  ...
  base::TimeDelta async_hit_test_timeout_delay_ =
      base::TimeDelta::FromMilliseconds(kAsyncHitTestTimeout);

  std::unique_ptr<TimeoutMonitor> async_hit_test_timeout_;

  // State needed for retargeting events when a renderer process
  // query times out.
  RenderWidgetHostViewBase* last_request_target_ = nullptr;
  gfx::PointF last_target_location_;
  RenderWidgetHostViewBase* current_request_root_view_ = nullptr;
  RenderWidgetHostViewBase* current_request_target_ = nullptr;
  gfx::PointF current_target_location_;
