...
std::unique_ptr<ui::CompositorLock> CompositorImpl::GetCompositorLock(
  ...
  // If we are transitioning from no locks to having locks, we need to set defer
  // commits to true. We will set this back to false when all locks release.
  ...
  // For the first lock, scheduled_timeout is 0, and the check will always pass.
