...
class InputRouterImplClient : public InputRouterClient {
 public:
...
class CONTENT_EXPORT InputRouterImpl
 ...
 private:
  ...
  // Filters |input_event| returns true if event was filtered or
  // false if it should be forwarded to renderer..
  InputEventAckState FilterWebInputEvent(
  ...
  // A queue of keyboard events. Unfortunately the GetLastKeyboardEvent
  // API exists. Once the guest_view is removed we can remove that API
  // and this queue.
