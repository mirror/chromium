...
// progress.
const base::TimeDelta kMaxMicrosecondsFromFlingTimestampToFirstProgress =
    base::TimeDelta::FromMicroseconds(33333);

// FlingProgress will get called once every
// kDefaultFlingProgressCallIntervalMicroseconds during an active fling.
const base::TimeDelta kDefaultFlingProgressCallIntervalMicroseconds =
...
FlingController::FlingController(
    ...
    : gesture_event_queue_(gesture_event_queue),
      ...
      send_wheel_events_nonblockingly_(
...
void FlingController::ScheduleFlingProgress() {
  fling_progress_timer_.Start(
      FROM_HERE, kDefaultFlingProgressCallIntervalMicroseconds,
      base::BindRepeating(&FlingController::ProgressFling,
...
void FlingController::ProgressFling() {
  ...
  if (!has_fling_animation_started_) {
    ...
    if (current_fling_parameters_.start_time.is_null() ||
        monotonic_time <= current_fling_parameters_.start_time ||
        monotonic_time >=
            current_fling_parameters_.start_time +
                kMaxMicrosecondsFromFlingTimestampToFirstProgress) {
      current_fling_parameters_.start_time = monotonic_time;
      return;
    }
  }
  ...
  bool fling_is_active = fling_curve_->Advance(
      (monotonic_time - current_fling_parameters_.start_time).InSecondsF(),
...
void FlingController::GenerateAndSendWheelEvents(
  ...
  synthetic_wheel.event.dispatch_type = send_wheel_events_nonblockingly_
...
bool FlingController::FlingBoostedForTesting() const {
