...
void LegacyInputRouterImpl::OnSetWhiteListedTouchAction(
  ...
  disposition_handler_->OnWhiteListedTouchAction(
      white_listed_touch_disposition_gesture_filter, unique_touch_event_id,
      ack_result);
  // TODO(hayleyferr): ACK events that are released based on the whitelisted
  // touch action. (If OnWhiteListedTouchAction returns true, we need to call
  // ProcessTouchAck)
}
