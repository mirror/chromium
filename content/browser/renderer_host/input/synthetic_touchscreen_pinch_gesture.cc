...
// The slop and span values in the gesture recognizer have epsilon values
// added to them (which aren't reported by the target) so we need to account
// for that here. Since touch points are quantized to pixel, we need to add a
// whole pixel to ensure we cross the threshold.
const float kAccountForEpsilon = 1.f;

// The minimum scaling span is the distance between the pointers at which the
// zoom stops having effect. (i.e. when zooming out, once the fingers get
// super close, the zoom will stop changing). However, we don't ever want to
// cross this threshold. As soon as we do, the *entire* event that caused us
// to cross it will be ignored, causing the delta to be lost and leading to
// significant error. Add this buffer to prevent crossing it.
const float kMinScalingSpanBuffer = 1.f;
...
void SyntheticTouchscreenPinchGesture::SetupCoordinatesAndStopTime(
  ...
  if (params_.scale_factor > 1.0f) {  // zooming in
    initial_distance_to_anchor =
        target->GetMinScalingSpanInDips() / 2.0f + kMinScalingSpanBuffer;
    ...
    initial_distance_to_anchor =
        ...
        target->GetTouchSlopInDips() + kAccountForEpsilon;
...
float SyntheticTouchscreenPinchGesture::GetDeltaForPointer0AtTime(
  ...
  if (!slop_sent_) {
    slop_sent_ = true;
    start_time_ = timestamp;
    return (target->GetTouchSlopInDips() + kAccountForEpsilon) * sign;
  } else {
    float total_abs_delta =
        2.f * (target->GetTouchSlopInDips() + kAccountForEpsilon) +
        params_.relative_pointer_speed_in_pixels_s *
