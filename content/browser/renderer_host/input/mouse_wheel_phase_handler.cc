  ...
  if (has_phase) {
    ...
    switch (scroll_state_) {
      ...
      case SCROLL_MAY_BEGIN:
        mouse_wheel_event.phase = blink::WebMouseWheelEvent::kPhaseBegan;
        scroll_state_ = SCROLL_IN_PROGRESS;
        break;
...
void MouseWheelPhaseHandler::ResetScrollSequence() {
  scroll_state_ = SCROLL_STATE_UNKNOWN;
}

void MouseWheelPhaseHandler::SendWheelEndIfNeeded() {
  if (scroll_state_ == SCROLL_IN_PROGRESS) {
    DCHECK(host_);
    bool should_route_event =
...
void MouseWheelPhaseHandler::ScrollingMayBegin() {
  scroll_state_ = SCROLL_MAY_BEGIN;
}
