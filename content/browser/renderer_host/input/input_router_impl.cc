...
void InputRouterImpl::SendKeyboardEvent(
  ...
  std::unique_ptr<InputEvent> event = base::MakeUnique<InputEvent>(
      ScaleEvent(key_event.event, device_scale_factor_), key_event.latency);
...
void InputRouterImpl::SendMouseEventImmediately(
  ...
  std::unique_ptr<InputEvent> event = base::MakeUnique<InputEvent>(
      ScaleEvent(mouse_event.event, device_scale_factor_), mouse_event.latency);
  if (WebInputEventTraits::ShouldBlockEventStream(
          mouse_event.event, raf_aligned_touch_enabled_,
...
void InputRouterImpl::SendTouchEventImmediately(
  ...
  std::unique_ptr<InputEvent> event = base::MakeUnique<InputEvent>(
      ScaleEvent(touch_event.event, device_scale_factor_), touch_event.latency);
  if (WebInputEventTraits::ShouldBlockEventStream(
          touch_event.event, raf_aligned_touch_enabled_,
          wheel_scroll_latching_enabled_)) {
...
void InputRouterImpl::SendGestureEventImmediately(
  ...
  std::unique_ptr<InputEvent> event = base::MakeUnique<InputEvent>(
      ScaleEvent(gesture_event.event, device_scale_factor_),
      gesture_event.latency);
  if (WebInputEventTraits::ShouldBlockEventStream(
          gesture_event.event, raf_aligned_touch_enabled_,
          wheel_scroll_latching_enabled_)) {
...
void InputRouterImpl::SendMouseWheelEventImmediately(
  ...
  std::unique_ptr<InputEvent> event = base::MakeUnique<InputEvent>(
      ScaleEvent(wheel_event.event, device_scale_factor_), wheel_event.latency);

  if (WebInputEventTraits::ShouldBlockEventStream(
          wheel_event.event, raf_aligned_touch_enabled_,
          wheel_scroll_latching_enabled_)) {
