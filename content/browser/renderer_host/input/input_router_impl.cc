...
void InputRouterImpl::SendMouseEventImmediately(
  ...
  if (WebInputEventTraits::ShouldBlockEventStream(
          mouse_event.event, true, wheel_scroll_latching_enabled_)) {
...
void InputRouterImpl::SendTouchEventImmediately(
  ...
  if (WebInputEventTraits::ShouldBlockEventStream(
          touch_event.event, true, wheel_scroll_latching_enabled_)) {
...
void InputRouterImpl::SendGestureEventImmediately(
  ...
  if (WebInputEventTraits::ShouldBlockEventStream(
          gesture_event.event, true, wheel_scroll_latching_enabled_)) {
...
void InputRouterImpl::SendMouseWheelEventImmediately(
  ...
  if (WasHandled(filtered_state)) {
    if (filtered_state != INPUT_EVENT_ACK_STATE_UNKNOWN) {
      MouseWheelEventHandled(wheel_event, InputEventAckSource::BROWSER,
                             wheel_event.latency, filtered_state,
                             base::nullopt);
    }
    return;
  }
  ...
  if (WebInputEventTraits::ShouldBlockEventStream(
          wheel_event.event, true, wheel_scroll_latching_enabled_)) {
