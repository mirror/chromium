...
void InputRouterImpl::SendKeyboardEvent(
  ...
  InputEventAckState filtered_state =
      FilterWebInputEvent(key_event.event, key_event.latency);
  if (WasHandled(filtered_state)) {
    KeyboardEventHandled(key_event, InputEventAckSource::BROWSER,
                         key_event.latency, filtered_state, base::nullopt);
    return;
  }

  client_->IncrementInFlightEventCount(key_event.event.GetType());
  std::unique_ptr<InputEvent> event =
      base::MakeUnique<InputEvent>(key_event.event, key_event.latency);
...
void InputRouterImpl::SendMouseEventImmediately(
    const MouseEventWithLatencyInfo& mouse_event) {
  InputEventAckState filtered_state =
      FilterWebInputEvent(mouse_event.event, mouse_event.latency);
  if (WasHandled(filtered_state)) {
    MouseEventHandled(mouse_event, InputEventAckSource::BROWSER,
                      mouse_event.latency, filtered_state, base::nullopt);
    return;
  }

  std::unique_ptr<InputEvent> event =
      base::MakeUnique<InputEvent>(mouse_event.event, mouse_event.latency);
  if (WebInputEventTraits::ShouldBlockEventStream(mouse_event.event, true)) {
    client_->IncrementInFlightEventCount(mouse_event.event.GetType());
    ...
    client_->GetWidgetInputHandler()->DispatchNonBlockingEvent(
        std::move(event));
    MouseEventHandled(mouse_event, InputEventAckSource::BROWSER,
                      mouse_event.latency, INPUT_EVENT_ACK_STATE_IGNORED,
                      base::nullopt);
  }
}
...
void InputRouterImpl::SendTouchEventImmediately(
    const TouchEventWithLatencyInfo& touch_event) {
  InputEventAckState filtered_state =
      FilterWebInputEvent(touch_event.event, touch_event.latency);
  if (WasHandled(filtered_state)) {
    TouchEventHandled(touch_event, InputEventAckSource::BROWSER,
                      touch_event.latency, filtered_state, base::nullopt);
    return;
  }

  std::unique_ptr<InputEvent> event =
      base::MakeUnique<InputEvent>(touch_event.event, touch_event.latency);
  if (WebInputEventTraits::ShouldBlockEventStream(touch_event.event, true)) {
    client_->IncrementInFlightEventCount(touch_event.event.GetType());
    ...
    client_->GetWidgetInputHandler()->DispatchNonBlockingEvent(
        std::move(event));
    TouchEventHandled(touch_event, InputEventAckSource::BROWSER,
                      touch_event.latency, INPUT_EVENT_ACK_STATE_IGNORED,
                      base::nullopt);
  }
}
...
void InputRouterImpl::SendGestureEventImmediately(
  ...
  InputEventAckState filtered_state =
      FilterWebInputEvent(gesture_event.event, gesture_event.latency);
  if (WasHandled(filtered_state)) {
    GestureEventHandled(gesture_event, InputEventAckSource::BROWSER,
                        gesture_event.latency, filtered_state, base::nullopt);
    return;
  }

  std::unique_ptr<InputEvent> event =
      base::MakeUnique<InputEvent>(gesture_event.event, gesture_event.latency);
  if (WebInputEventTraits::ShouldBlockEventStream(gesture_event.event, true)) {
    client_->IncrementInFlightEventCount(gesture_event.event.GetType());
    ...
    client_->GetWidgetInputHandler()->DispatchNonBlockingEvent(
        std::move(event));
    GestureEventHandled(gesture_event, InputEventAckSource::BROWSER,
                        gesture_event.latency, INPUT_EVENT_ACK_STATE_IGNORED,
                        base::nullopt);
  }
}
...
void InputRouterImpl::SendMouseWheelEventImmediately(
    const MouseWheelEventWithLatencyInfo& wheel_event) {
  InputEventAckState filtered_state =
      FilterWebInputEvent(wheel_event.event, wheel_event.latency);
  if (WasHandled(filtered_state)) {
    MouseWheelEventHandled(wheel_event, InputEventAckSource::BROWSER,
                           wheel_event.latency, filtered_state, base::nullopt);
    return;
  }

  std::unique_ptr<InputEvent> event =
      base::MakeUnique<InputEvent>(wheel_event.event, wheel_event.latency);

  if (WebInputEventTraits::ShouldBlockEventStream(wheel_event.event, true)) {
    client_->IncrementInFlightEventCount(wheel_event.event.GetType());
    ...
    client_->GetWidgetInputHandler()->DispatchNonBlockingEvent(
        std::move(event));
    MouseWheelEventHandled(wheel_event, InputEventAckSource::BROWSER,
                           wheel_event.latency, INPUT_EVENT_ACK_STATE_IGNORED,
                           base::nullopt);
  }
}
...
void InputRouterImpl::MouseWheelEventHandled(
  ...
  ack_handler_->OnWheelEventAck(event, state);
}

void InputRouterImpl::OnMouseWheelEventAck(
    const MouseWheelEventWithLatencyInfo& event,
    InputEventAckState ack_result) {
  // Don't do anything. Already handled in the MouseWheelEventHandled.
}

InputEventAckState InputRouterImpl::FilterWebInputEvent(
  ...
  return client_->FilterInputEvent(input_event, latency_info);
}
