...
class TouchActionBrowserTest : public ContentBrowserTest {
 ...
 protected:
  void LoadURL(const char* touch_action_url,
    ...
    // regions.
    // When we jank the main thread on purpose to test white listed touch
    // action, then don't wait.
...
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest, MAYBE_PanYMainThreadJanky) {
  LoadURL(kTouchActionJankURL, true);

  float device_scale_factor = GetDeviceScaleFactor();
  float squared_device_scale_factor = device_scale_factor * device_scale_factor;
  gfx::Point touch_point =
      gfx::Point(static_cast<float>(25) / squared_device_scale_factor,
                 static_cast<float>(125) / squared_device_scale_factor);
  DoTouchScroll(touch_point, gfx::Vector2d(0, 45), false, 10000,
                45 * device_scale_factor, 0, true);
}
...
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest, MAYBE_PanXMainThreadJanky) {
  ...
  float squared_device_scale_factor = device_scale_factor * device_scale_factor;
  gfx::Point touch_point =
      gfx::Point(static_cast<float>(125) / squared_device_scale_factor,
                 static_cast<float>(25) / squared_device_scale_factor);
  DoTouchScroll(touch_point, gfx::Vector2d(45, 0), false, 10000, 0,
                45 * device_scale_factor, true);
}
...
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest, MAYBE_PanXYMainThreadJanky) {
  ...
  float squared_device_scale_factor = device_scale_factor * device_scale_factor;
  gfx::Point touch_point =
      gfx::Point(static_cast<float>(75) / squared_device_scale_factor,
                 static_cast<float>(60) / squared_device_scale_factor);
  DoTouchScroll(touch_point, gfx::Vector2d(45, 45), false, 10000,
                45 * device_scale_factor, 45 * device_scale_factor, true);
}
...
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest,
  ...
  gfx::Point touch_point =
      gfx::Point(static_cast<float>(125) / squared_device_scale_factor,
                 static_cast<float>(25) / squared_device_scale_factor);
  DoTouchScroll(touch_point, gfx::Vector2d(45, 45), false, 10000, 0,
                45 * device_scale_factor, true);
}
...
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest,
  ...
  gfx::Point touch_point =
      gfx::Point(static_cast<float>(125) / squared_device_scale_factor,
                 static_cast<float>(75) / squared_device_scale_factor);
  DoTouchScroll(touch_point, gfx::Vector2d(45, 45), false, 10000, 0,
                45 * device_scale_factor, true);
}
...
#endif
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest,
                       MAYBE_PanYAfterPinchZoomMainThreadJanky) {
  LoadURL(kTouchActionJankURL, true);

  float device_scale_factor = GetDeviceScaleFactor();
  float squared_device_scale_factor = device_scale_factor * device_scale_factor;
  gfx::Point pinch_point =
      gfx::Point(static_cast<float>(25) / squared_device_scale_factor,
                 static_cast<float>(25) / squared_device_scale_factor);
  gfx::Point touch_point =
      gfx::Point(static_cast<float>(25) / squared_device_scale_factor,
                 static_cast<float>(125) / squared_device_scale_factor);
  DoTouchPinchZoom(pinch_point, 1.2);
  DoTouchScroll(touch_point, gfx::Vector2d(0, 45), false, 10000,
                45 * device_scale_factor, 0, true);
}
...
#endif
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest,
                       MAYBE_PanYAfterPinchZoomAtOverlapAreaMainThreadJanky) {
  LoadURL(kTouchActionJankURL, true);
  ...
  gfx::Point touch_point =
      gfx::Point(static_cast<float>(75) / squared_device_scale_factor,
                 static_cast<float>(125) / squared_device_scale_factor);
  DoTouchPinchZoom(touch_point, 1.2);
  DoTouchScroll(touch_point, gfx::Vector2d(0, 45), false, 10000,
                45 * device_scale_factor, 0, true);
}
