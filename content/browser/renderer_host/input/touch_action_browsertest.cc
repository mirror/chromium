...
class TouchActionBrowserTest : public ContentBrowserTest {
 public:
  TouchActionBrowserTest() { load_url_called_ = false; }
 ...
 protected:
  ...
  // We synchronously send a raf, inside that raf we change the scroll height
  // and make the main thread sleep. Since the things inside the raf will be
  // executed asynchronously, so what will happen is that we start the gesture
  // even before the scroll height becomes 0, which means the compositor still
  // thinks that the page is scrollable. Then before the main thread wakes up,
  // we check the scroll offset from the compositor frame's metadata.
  void JankMainThread() {
    std::string script =
