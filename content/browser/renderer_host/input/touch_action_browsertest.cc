...
class TouchActionBrowserTest : public ContentBrowserTest {
 ...
 protected:
  ...
  float GetPointScaleFactor() {
    ...
    float point_scale_factor = device_scale_factor * page_scale_factor;
    return point_scale_factor == 0 ? 1 : point_scale_factor;
  }
  ...
  void DoTouchScroll(const gfx::Point& point,
    ...
    if (is_main_thread_janky) {
      ...
      EXPECT_LE(root_scroll_offset.y(),
                expected_scroll_top * point_scale_factor);
      EXPECT_GE(root_scroll_offset.y(),
                (expected_scroll_top - 1) * point_scale_factor);
      EXPECT_LE(root_scroll_offset.x(),
                expected_scroll_left * point_scale_factor);
      EXPECT_GE(root_scroll_offset.x(),
                (expected_scroll_left - 1) * point_scale_factor);
    }
...
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest, MAYBE_PanYMainThreadJanky) {
  ...
  float point_scale_factor = GetPointScaleFactor();
  gfx::Point touch_point =
      gfx::Point(static_cast<float>(25) / point_scale_factor,
                 static_cast<float>(125) / point_scale_factor);
...
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest, MAYBE_PanXMainThreadJanky) {
  ...
  float point_scale_factor = GetPointScaleFactor();
  gfx::Point touch_point =
      gfx::Point(static_cast<float>(125) / point_scale_factor,
                 static_cast<float>(25) / point_scale_factor);
...
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest, MAYBE_PanXYMainThreadJanky) {
  ...
  float point_scale_factor = GetPointScaleFactor();
  gfx::Point touch_point =
      gfx::Point(static_cast<float>(75) / point_scale_factor,
                 static_cast<float>(60) / point_scale_factor);
...
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest,
  ...
  gfx::Point touch_point =
      gfx::Point(static_cast<float>(125) / point_scale_factor,
                 static_cast<float>(75) / point_scale_factor);
  DoTouchScroll(touch_point, gfx::Vector2d(45, 45), false, 10000, 0, 45, true,
                point_scale_factor);
}
