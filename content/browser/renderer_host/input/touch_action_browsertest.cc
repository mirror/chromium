...
class TouchActionBrowserTest : public ContentBrowserTest {
 ...
 protected:
  ...
  void DoTouchScroll(const gfx::Point& point,
                     ...
                     float squared_device_scale_factor = 1) {
    ...
    if (is_main_thread_janky) {
      ...
      EXPECT_LE(root_scroll_offset.y(),
                expected_scroll_top * squared_device_scale_factor);
      EXPECT_GE(root_scroll_offset.y(),
                (expected_scroll_top - 1) * squared_device_scale_factor);
      EXPECT_LE(root_scroll_offset.x(),
                expected_scroll_left * squared_device_scale_factor);
      EXPECT_GE(root_scroll_offset.x(),
                (expected_scroll_left - 1) * squared_device_scale_factor);
    }
