...
class TouchActionBrowserTest : public ContentBrowserTest {
 ...
 protected:
  ...
  void DoTouchPinchZoom(const gfx::Point& point, const double scale) {
  ...
  void DoTouchScroll(const gfx::Point& point,
    ...
    if (is_main_thread_janky) {
      while ((expected_scroll_top > 0 && root_scroll_offset.y() == 0) ||
             (expected_scroll_left > 0 && root_scroll_offset.x() == 0)) {
        frame_watcher.WaitFrames(1);
      }
      // Allow for 1px rounding inaccuracies
      EXPECT_LE(root_scroll_offset.y(), expected_scroll_top);
      EXPECT_GE(root_scroll_offset.y(), expected_scroll_top - 1);
      EXPECT_LE(root_scroll_offset.x(), expected_scroll_left);
      EXPECT_GE(root_scroll_offset.x(), expected_scroll_left - 1);
    }
...
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest, PanYMainThreadJanky) {
  ...
  DoTouchScroll(gfx::Point(25, 125), gfx::Vector2d(0, 45), false, 10000, 45, 0,
                true);
}
...
#endif
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest, PanXMainThreadJanky) {
  LoadURL(kTouchActionJankURL);

  DoTouchScroll(gfx::Point(125, 25), gfx::Vector2d(45, 0), false, 10000, 0, 45,
                true);
}
...
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest, PanXYMainThreadJanky) {
  ...
  DoTouchScroll(gfx::Point(75, 60), gfx::Vector2d(45, 45), false, 10000, 45, 45,
                true);
}
...
#endif
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest, PanXYAtXAreaMainThreadJanky) {
  LoadURL(kTouchActionJankURL);

  DoTouchScroll(gfx::Point(125, 25), gfx::Vector2d(45, 45), false, 10000, 0, 45,
                true);
}
...
#endif
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest, PanXYAtYAreaMainThreadJanky) {
  LoadURL(kTouchActionJankURL);

  DoTouchScroll(gfx::Point(25, 125), gfx::Vector2d(45, 45), false, 10000, 45, 0,
                true);
}

#if defined(OS_MACOSX)
#define MAYBE_PanXYAtXYYOverlapAreaMainThreadJanky \
  DISABLED_PanXYAtXYYOverlapAreaMainThreadJanky
#else
#define MAYBE_PanXYAtXYYOverlapAreaMainThreadJanky \
  PanXYAtXYYOverlapAreaMainThreadJanky
#endif
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest,
                       PanXYAtXYYOverlapAreaMainThreadJanky) {
  LoadURL(kTouchActionJankURL);

  DoTouchScroll(gfx::Point(75, 125), gfx::Vector2d(45, 45), false, 10000, 45, 0,
                true);
}

#if defined(OS_MACOSX)
#define MAYBE_PanXYAtXYXOverlapAreaMainThreadJanky \
  DISABLED_PanXYAtXYXOverlapAreaMainThreadJanky
#else
#define MAYBE_PanXYAtXYXOverlapAreaMainThreadJanky \
  PanXYAtXYXOverlapAreaMainThreadJanky
#endif
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest,
                       PanXYAtXYXOverlapAreaMainThreadJanky) {
  LoadURL(kTouchActionJankURL);

  DoTouchScroll(gfx::Point(125, 75), gfx::Vector2d(45, 45), false, 10000, 0, 45,
                true);
}

#if defined(OS_MACOSX)
#define MAYBE_PanYAfterPinchZoomMainThreadJanky \
  DISABLED_PanYAfterPinchZoomMainThreadJanky
#else
#define MAYBE_PanYAfterPinchZoomMainThreadJanky \
  PanYAfterPinchZoomMainThreadJanky
#endif
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest,
                       PanYAfterPinchZoomMainThreadJanky) {
  LoadURL(kTouchActionJankURL);

  DoTouchPinchZoom(gfx::Point(25, 25), 1.2);
  DoTouchScroll(gfx::Point(25, 125), gfx::Vector2d(0, 45), false, 10000, 45, 0,
                true);
}

#if defined(OS_MACOSX)
#define MAYBE_PanYAfterPinchZoomAtOverlapAreaMainThreadJanky \
  DISABLED_PanYAfterPinchZoomAtOverlapAreaMainThreadJanky
#else
#define MAYBE_PanYAfterPinchZoomAtOverlapAreaMainThreadJanky \
  PanYAfterPinchZoomAtOverlapAreaMainThreadJanky
#endif
IN_PROC_BROWSER_TEST_F(TouchActionBrowserTest,
                       PanYAfterPinchZoomAtOverlapAreaMainThreadJanky) {
  LoadURL(kTouchActionJankURL);

  DoTouchPinchZoom(gfx::Point(75, 125), 1.2);
  DoTouchScroll(gfx::Point(75, 125), gfx::Vector2d(0, 45), false, 10000, 45, 0,
                true);
}
