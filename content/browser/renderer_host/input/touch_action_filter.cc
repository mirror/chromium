...
bool TouchActionFilter::FilterGestureEvent(WebGestureEvent* gesture_event) {
  ...
  bool processing_white_listed = processing_white_listed_;
  if (processing_white_listed_)
    processing_white_listed_ = false;

  bool* suppress_manipulation_events =
      processing_white_listed ? &white_listed_suppress_manipulation_events_
                              : &suppress_manipulation_events_;
  cc::TouchAction touch_action = processing_white_listed_
  ...
  switch (gesture_event->GetType()) {
    ...
    case WebInputEvent::kGestureScrollUpdate:
      ...
      if (processing_white_listed) {
        // TODO(xidachen): Allow the gesture to be completed as a confined
        // scroll (i.e. do the steps held within the else statement).
        // If either value was changed to zero, add the amount of scroll lost
        // to an accumulated x or y scroll counter. This scroll counter will
        // be checked on the receipt of an effective touch action from blink.
        if (IsXAxisActionDisallowed(touch_action) ||
            IsYAxisActionDisallowed(touch_action))
          return true;
        return false;
      } else {
    ...
    case WebInputEvent::kGestureTapUnconfirmed:
      DCHECK_EQ(1, gesture_event->data.tap.tap_count);
      if (processing_white_listed) {
        if ((touch_action & cc::kTouchActionDoubleTapZoom) == 0)
          return true;
        break;
      }
...
bool TouchActionFilter::FilterGestureWhiteListed(
  ...
  processing_white_listed_ = FilterGestureEvent(&gesture_event);
