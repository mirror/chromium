...
// We can start filtering gesture event under either one of the following
// conditions:
...
// to tell us which direction is the gesture moving towards. Then using the
...
// partially matches the whitelisted touch action, we accumulate the delta by
...
bool TouchActionFilter::FilterGestureEvent(WebGestureEvent* gesture_event) {
  ...
  if (suppress_manipulation_events_.has_value() &&
      ...
      !should_suppress_manipulation_events_based_on_allowed_touch_action_ &&
    ...
    should_suppress_manipulation_events_based_on_allowed_touch_action_ = true;
  }
  ...
  switch (gesture_event->GetType()) {
    ...
    case WebInputEvent::kGestureScrollUpdate:
      gesture_event->data.scroll_update.delta_x *= current_pinch_scale_;
      gesture_event->data.scroll_update.delta_y *= current_pinch_scale_;
    ...
    // two fingers and vice versa.
    case WebInputEvent::kGesturePinchUpdate:
      current_pinch_scale_ = gesture_event->data.pinch_update.scale;
...
bool TouchActionFilter::FilterManipulationEventAndResetState() {
  ...
  should_suppress_manipulation_events_based_on_allowed_touch_action_ = false;
