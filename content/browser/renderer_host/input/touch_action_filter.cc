...
bool TouchActionFilter::FilterGestureEvent(WebGestureEvent* gesture_event) {
  ...
  switch (gesture_event->GetType()) {
    case WebInputEvent::kGestureScrollBegin:
      ...
      // This could happen when we received the white listed touch action but
      // the main thread is too busy to send back the real touch action. In this
      // case, we don't filter out this gesture event, because we don't know yet
      // if it will be allowed by the real touch action and we can't currently
      // generate the ScrollBegin later if/when we decide the scroll gesture is
      // allowed. So instead we filter the begin and zero out all scroll updates
      // until we have decided.
      // TODO(crbug.com/780351): It could happen that we don't know the real
      // touch action when it comes to a scroll end. In this case, we will drop
      // the scroll event which is wrong. We could fix this by injecting another
      // scroll update later.
      if (!touch_action_from_main_.has_value())
        return false;
...
bool TouchActionFilter::ShouldSuppressManipulation(
  ...
  if (touch_action_from_main_.has_value())
    ...
    allowed_touch_action = cc::kTouchActionAuto;
