  ...
  switch (gesture_event.event.GetType()) {
    case WebInputEvent::kGestureScrollUpdate:
      if (gesture_event.event.data.scroll_update.inertial_phase ==
        ...
        // These events are genereted from handling the GFS in fling_controller_
        // and should be forwarded since GFS has already stopped debouncing.
        DCHECK(!scrolling_in_progress_);
        return true;
      }
