...
class CONTENT_EXPORT TouchActionFilter {
 ...
 private:
  ...
  // touch-action.
  bool should_suppress_manipulation_events_based_on_allowed_touch_action_ =
      false;
  ...
  // We need to apply the scale from the pinch update to the
  // |accumulated_x_scrolling| and |accumulated_y_scrolling| when applicable.
  float current_pinch_scale_ = 1.0f;
