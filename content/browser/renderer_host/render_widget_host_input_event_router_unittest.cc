...
// routed correctly.
class MockRWHV : public TestRenderWidgetHostView {
 public:
  MockRWHV(RenderWidgetHost* rwh)
...
class MockRootRWHV : public MockRWHV {
 public:
  MockRootRWHV(RenderWidgetHost* rwh,
               std::map<MockRWHV*, viz::FrameSinkId>& frame_sink_id_map)
      : MockRWHV(rwh), frame_sink_id_map_(frame_sink_id_map) {}
  ~MockRootRWHV() override {}

  viz::FrameSinkId FrameSinkIdAtPoint(cc::SurfaceHittestDelegate*,
                                      const gfx::Point&,
                                      gfx::Point*) override {
    return frame_sink_id_map_[current_hittest_result_];
  }

  void SetHittestResult(MockRWHV* view) { current_hittest_result_ = view; }

 private:
  std::map<MockRWHV*, viz::FrameSinkId>& frame_sink_id_map_;
  MockRWHV* current_hittest_result_;
};
...
class RenderWidgetHostInputEventRouterTest : public testing::Test {
 ...
 protected:
  ...
  void SetUp() override {
    ...
    view_root_ =
        base::MakeUnique<MockRootRWHV>(widget_host1_.get(), frame_sink_id_map_);
    view_other_ = base::MakeUnique<MockRWHV>(widget_host2_.get());
  ...
  std::unique_ptr<MockRootRWHV> view_root_;
  std::unique_ptr<MockRWHV> view_other_;
...
TEST_F(RenderWidgetHostInputEventRouterTest,
  ...
  EXPECT_EQ(view_other_.get(), rwhier_.touch_target_.target);
  ...
  rwhier_.RouteGestureEvent(view_root_.get(), &gesture_event,
                            ui::LatencyInfo(ui::SourceEventType::TOUCH));
  EXPECT_EQ(view_other_.get(), rwhier_.touchscreen_gesture_target_.target);
