...
TEST_F(RenderWidgetHostViewAuraTest,
       NewContentRenderingTimeoutWithSurfaceSync) {
  // If Surface Synchronization is off, we have a separate code path for
  // cancelling new content rendering timeout that is tested in
  // RenderWidgetHostTest.NewContentRenderingTimeoutWithoutSurfaceSync.
  if (!features::IsSurfaceSynchronizationEnabled())
    return;
  ...
  widget_host_->StartNewContentRenderingTimeout(5);
  viz::LocalSurfaceId id2 = view_->GetLocalSurfaceId();
  EXPECT_TRUE(id2.is_valid());

  // The new LocalSurfaceId should have a larger parent_sequence_number than the
  // previous one.
  EXPECT_LT(id1.parent_sequence_number(), id2.parent_sequence_number());

  // Submit a frame to the new LocalSurfaceId. The timer should not fire.
  auto frame = viz::CompositorFrameBuilder().AddDefaultRenderPass().Build();
  widget_host_->SubmitCompositorFrame(id2, std::move(frame), nullptr, 0);
  {
    base::RunLoop run_loop;
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
        FROM_HERE, run_loop.QuitClosure(),
  ...
  // Start the timer but only submit to the old page. The timer should fire.
  widget_host_->StartNewContentRenderingTimeout(10);
  viz::LocalSurfaceId id3 = view_->GetLocalSurfaceId();
  EXPECT_LT(id2.parent_sequence_number(), id3.parent_sequence_number());
  frame = viz::CompositorFrameBuilder().AddDefaultRenderPass().Build();
  widget_host_->SubmitCompositorFrame(id2, std::move(frame), nullptr, 0);
  {
    base::RunLoop run_loop;
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
