...
TEST_F(RenderWidgetHostViewAuraTest, SetCompositionText) {
  ...
  // Focused segment
  composition_text.ime_spans.push_back(
      ui::ImeSpan(0, 3, 0xff000000, true, 0x78563412));

  // Non-focused segment, with different background color.
  composition_text.ime_spans.push_back(
      ui::ImeSpan(3, 4, 0xff000000, false, 0xefcdab90));

  const ui::ImeSpans& ime_spans = composition_text.ime_spans;
  ...
  EXPECT_TRUE(view_->has_composition_text_);
  {
    ...
    // ime spans
    ASSERT_EQ(ime_spans.size(), std::get<1>(params).size());
    for (size_t i = 0; i < ime_spans.size(); ++i) {
      EXPECT_EQ(ime_spans[i].start_offset, std::get<1>(params)[i].start_offset);
      EXPECT_EQ(ime_spans[i].end_offset, std::get<1>(params)[i].end_offset);
      EXPECT_EQ(ime_spans[i].color, std::get<1>(params)[i].color);
      EXPECT_EQ(ime_spans[i].thick, std::get<1>(params)[i].thick);
      EXPECT_EQ(ime_spans[i].background_color,
