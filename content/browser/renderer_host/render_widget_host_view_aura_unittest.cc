  ...
  void SendScrollUpdateAck(MockWidgetInputHandler::MessageVector& messages,
    ...
    for (size_t i = 0; i < messages.size(); ++i) {
      MockWidgetInputHandler::DispatchedEventMessage* event =
          messages[i]->ToEvent();
      if (event &&
          event->event_->web_event->GetType() ==
  ...
  void SendScrollBeginAckIfNeeded(
    ...
    if (wheel_scroll_latching_enabled_) {
      for (size_t i = 0; i < messages.size(); ++i) {
        ...
        if (event && event->event_->web_event->GetType() ==
  ...
  ExpectGestureScrollUpdateAfterNonBlockingMouseWheelACK(
    ...
    if (wheel_was_queued) {
      ...
      EXPECT_EQ(WebInputEvent::kMouseWheel,
                events[0]->ToEvent()->event_->web_event->GetType());
...
TEST_F(RenderWidgetHostViewAuraTest, SetCompositionText) {
  ...
  EXPECT_EQ(composition_text.text, ime_message->text_);
  EXPECT_EQ(ime_text_spans, ime_message->text_spans_);
  EXPECT_EQ(gfx::Range::InvalidRange(), ime_message->range_);
  // highlighted range
  EXPECT_EQ(4, ime_message->start_) << "Should be the same to the caret pos";
  EXPECT_EQ(4, ime_message->end_) << "Should be the same to the caret pos";
...
TEST_F(RenderWidgetHostViewAuraWheelScrollLatchingEnabledTest,
  ...
  const WebMouseWheelEvent* wheel_event =
      static_cast<const WebMouseWheelEvent*>(
          events[0]->ToEvent()->event_->web_event.get());
  ...
  gesture_event = static_cast<const WebGestureEvent*>(
      events[1]->ToEvent()->event_->web_event.get());
...
TEST_F(RenderWidgetHostViewAuraWheelScrollLatchingEnabledTest,
  ...
  gesture_event = static_cast<const WebGestureEvent*>(
      events[2]->ToEvent()->event_->web_event.get());
  EXPECT_EQ(WebInputEvent::kGestureScrollBegin, gesture_event->GetType());
