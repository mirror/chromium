...
class RenderWidgetHostViewAuraTest : public testing::Test {
 public:
  ...
  std::vector<WebInputEvent::Type> GetSentInputEventTypesAndReset() {
    std::vector<WebInputEvent::Type> result;
    for (size_t i = 0; i < sink_->message_count(); i++) {
      InputMsg_HandleInputEvent::Param params;
      if (InputMsg_HandleInputEvent::Read(sink_->GetMessageAt(i), &params)) {
        const WebInputEvent* event = std::get<0>(params);
        result.push_back(event->GetType());
      }
    }
    sink_->ClearMessages();
    return result;
...
class RenderWidgetHostViewAuraOverscrollTest
 ...
 protected:
  ...
  void ExpectGestureScrollEndForWheelScrolling(bool is_last) {
    if (wheel_scroll_latching_enabled_ && !is_last) {
      EXPECT_EQ(0U, GetSentMessageCountAndResetSink());
    } else {
      EXPECT_THAT(GetSentInputEventTypesAndReset(),
                  ElementsAre(WebInputEvent::kGestureScrollEnd));
    }
  }
  ...
  void ExpectGestureScrollEventsAfterMouseWheelAckWhenOverscrolling() {
    ...
    //    controller to prevent page scroll (non-async wheel event).
    if (wheel_scroll_latching_enabled_) {
      EXPECT_EQ(0U, GetSentMessageCountAndResetSink());
    } else {
      ...
      // consumed by the overscroll controller to prevent page scroll.
      EXPECT_THAT(GetSentInputEventTypesAndReset(),
                  ElementsAre(WebInputEvent::kGestureScrollBegin,
...
void RenderWidgetHostViewAuraOverscrollTest::WheelNotPreciseScrollEvent() {
  ...
  EXPECT_THAT(GetSentInputEventTypesAndReset(),
              ElementsAre(WebInputEvent::kMouseWheel));
...
TEST_F(RenderWidgetHostViewAuraOverscrollTest, GestureScrollOverscrolls) {
  ...
  EXPECT_EQ(0.f, overscroll_delegate()->delta_y());
  EXPECT_THAT(GetSentInputEventTypesAndReset(),
              ElementsAre(WebInputEvent::kTouchScrollStarted,
...
TEST_F(RenderWidgetHostViewAuraOverscrollTest,
  ...
  EXPECT_THAT(GetSentInputEventTypesAndReset(),
              ElementsAre(WebInputEvent::kGestureScrollUpdate));
  SendInputEventACK(WebInputEvent::kGestureScrollUpdate,
                    INPUT_EVENT_ACK_STATE_NOT_CONSUMED);
  EXPECT_EQ(OVERSCROLL_NONE, overscroll_mode());
  EXPECT_EQ(OverscrollSource::NONE, overscroll_source());
...
TEST_F(RenderWidgetHostViewAuraOverscrollTest, OverscrollWithTouchEvents) {
  ...
  EXPECT_THAT(GetSentInputEventTypesAndReset(),
              ElementsAre(WebInputEvent::kTouchStart));
  ...
  EXPECT_THAT(GetSentInputEventTypesAndReset(),
              ElementsAre(WebInputEvent::kTouchMove));
  ...
  AckLastSentInputEventIfNecessary(INPUT_EVENT_ACK_STATE_NOT_CONSUMED);
  EXPECT_THAT(GetSentInputEventTypesAndReset(),
              ElementsAre(WebInputEvent::kTouchEnd));
  ReleaseTouchPoint(0);
  SendTouchEvent();
  AckLastSentInputEventIfNecessary(INPUT_EVENT_ACK_STATE_NOT_CONSUMED);
  EXPECT_THAT(GetSentInputEventTypesAndReset(),
...
TEST_F(RenderWidgetHostViewAuraOverscrollTest, OverscrollDirectionChange) {
  ...
  // ScrollUpdate event to prevent page scroll.
  ...
  // should be consumed by the overscroll controller to prevent page scroll.
  SimulateGestureScrollUpdateEvent(-20, 0, 0);
  EXPECT_EQ(0U, sink_->message_count());
  EXPECT_EQ(OVERSCROLL_NONE, overscroll_mode());
  EXPECT_EQ(OverscrollSource::NONE, overscroll_source());
...
void RenderWidgetHostViewAuraOverscrollTest::OverscrollMouseMoveCompletion() {
  ...
  EXPECT_EQ(OVERSCROLL_NONE, overscroll_delegate()->current_mode());
  EXPECT_THAT(GetSentInputEventTypesAndReset(),
              ElementsAre(WebInputEvent::kMouseMove));
