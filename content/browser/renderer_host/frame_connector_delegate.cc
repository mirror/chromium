...
void FrameConnectorDelegate::SetRect(const gfx::Rect& frame_rect_in_pixels) {
  frame_rect_in_pixels_ = frame_rect_in_pixels;
  if (use_zoom_for_device_scale_factor_) {
    frame_rect_in_dip_ = gfx::ScaleToEnclosingRect(
        frame_rect_in_pixels, 1.f / screen_info_.device_scale_factor);
  } else {
    frame_rect_in_dip_ = frame_rect_in_pixels;
  }
}
