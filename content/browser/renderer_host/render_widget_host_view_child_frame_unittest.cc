...
TEST_F(RenderWidgetHostViewChildFrameZoomForDSFTest, PhysicalBackingSize) {
  ...
  gfx::Size local_frame_size_in_pixels(1276, 410);
  test_frame_connector_->SetLocalFrameSize(local_frame_size_in_pixels);
  EXPECT_EQ(local_frame_size_in_pixels, view_->GetPhysicalBackingSize());

  gfx::Rect screen_space_rect_in_pixels(local_frame_size_in_pixels);
  screen_space_rect_in_pixels.set_origin(gfx::Point(230, 263));
  test_frame_connector_->SetScreenSpaceRect(screen_space_rect_in_pixels);
  EXPECT_EQ(local_frame_size_in_pixels, view_->GetPhysicalBackingSize());
}
