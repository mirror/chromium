...
std::unique_ptr<net::test_server::HttpResponse> Beacon(
...
std::unique_ptr<net::test_server::HttpResponse> HungBeacon(
    const net::test_server::HttpRequest& request) {
  if (request.relative_url != "/beacon") {
    return nullptr;
  }
...
IN_PROC_BROWSER_TEST_F(RenderProcessHostTest, KeepAliveRendererProcess) {
  ...
  embedded_test_server()->RegisterRequestHandler(base::BindRepeating(Beacon));
  ASSERT_TRUE(embedded_test_server()->Start());
  RenderProcessHostImpl* rph = static_cast<RenderProcessHostImpl*>(
      shell()->web_contents()->GetMainFrame()->GetProcess());
...
IN_PROC_BROWSER_TEST_F(RenderProcessHostTest, KeepAliveRendererProcess_Hung) {
  ...
  feature_list.InitAndEnableFeatureWithParameters(
      features::kKeepAliveRendererForKeepaliveRequests,
      {std::make_pair("timeout_in_sec", "3")});

  embedded_test_server()->RegisterRequestHandler(
      base::BindRepeating(HungBeacon));
  ASSERT_TRUE(embedded_test_server()->Start());
  RenderProcessHostImpl* rph = static_cast<RenderProcessHostImpl*>(
      shell()->web_contents()->GetMainFrame()->GetProcess());
  ...
  EXPECT_GE(base::TimeTicks::Now() - start, base::TimeDelta::FromSeconds(3));
  if (!host_destructions_)
    rph->RemoveObserver(this);
}
