...
class RenderProcessHostObserverForReady
    : public content::RenderProcessHostObserver {
 public:
  explicit RenderProcessHostObserverForReady(
      content::RenderProcessHost* observed_host)
      : render_process_host_ready_(false), observed_host_(observed_host) {
    observed_host->AddObserver(this);
  }

  void WaitUntilRenderProcessHostReady() {
    if (render_process_host_ready_)
      return;
 ...
 private:
  void RenderProcessReady(content::RenderProcessHost* host) override {
    ...
    render_process_host_ready_ = true;
  ...
  bool render_process_host_ready_;
  content::RenderProcessHost* observed_host_;
  scoped_refptr<content::MessageLoopRunner> message_loop_runner_;

  DISALLOW_COPY_AND_ASSIGN(RenderProcessHostObserverForReady);
};
...
IN_PROC_BROWSER_TEST_F(RenderProcessHostTest, SpareRendererOnProcessReuse) {
  ...
  RenderProcessHostObserverForReady renderer_ready_observer(
      shell()->web_contents()->GetMainFrame()->GetProcess());
  renderer_ready_observer.WaitUntilRenderProcessHostReady();
  SetBrowserClientForTesting(old_client);
}
