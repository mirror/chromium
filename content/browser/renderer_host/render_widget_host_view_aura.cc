...
void RenderWidgetHostViewAura::WasResized(
  ...
  if (!window_->IsAllocationPending()) {
    ...
    GetLastChildAllocatedLocalSurfaceId(&id_for_auto_resize);
    if (id_for_auto_resize.is_valid())
      window_->UpdateLocalSurfaceIdWithChildSequenceNumber(id_for_auto_resize);
  } else
...
void RenderWidgetHostViewAura::GetLastChildAllocatedLocalSurfaceId(
  ...
  // as well.
  if (id_for_auto_resize.is_valid()) {
    if (id_for_auto_resize.child_sequence_number() <
