...
void LegacyRenderWidgetHostHWND::OnFinalMessage(HWND hwnd) {
  ...
  if (direct_manipulation_helper_ && host_) {
    host_->RemoveCompositorAnimationObserver(
...
bool LegacyRenderWidgetHostHWND::Init() {
  ...
  if (direct_manipulation_helper_) {
    compositor_animation_observer_.reset(
        new CompositorAnimationObserverForDirectManipulation(this));
...
LRESULT LegacyRenderWidgetHostHWND::OnPointerHitTest(UINT message,
  ...
  if (direct_manipulation_helper_->OnPointerHitTest(w_param))
    host_->AddCompositorAnimationObserver(compositor_animation_observer_.get());
