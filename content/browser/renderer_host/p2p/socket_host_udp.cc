...
// Defaulf send buffer size.
const int kDefaultSendBufferSize = 65536;
...
void P2PSocketHostUdp::SetDefaultSendBufferSize() {
  if (!SetOption(P2P_SOCKET_OPT_SNDBUF, kDefaultSendBufferSize)) {
    LOG(WARNING) << "Failed to set socket send buffer size to "
                 << kDefaultSendBufferSize;
  }
}
