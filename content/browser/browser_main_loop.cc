...
int BrowserMainLoop::CreateThreads() {
...

  {
    ...
    // correctly configures the number of threads in the foreground pool for
    // renderer processes. In the case where renderers live in the browser
    // process, the code below adjusts the number of threads in the foreground
    // pool.
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(
            switches::kSingleProcess)) {
      const SchedulerWorkerPoolParams& current_foreground_worker_pool_params(
      ...
      task_scheduler_init_params->foreground_worker_pool_params =
          base::SchedulerWorkerPoolParams(
              ...
              current_foreground_worker_pool_params.background_compatibility());
    }
