...
void CreateRegistrationTask::StoreRegistration() {
  ...
  registration_->origin = registration_id_.origin();
  registration_->creation_microseconds_since_unix_epoch =
  ...
  registration_proto.set_origin(registration_->origin.Serialize());
  ...
  for (int i = 0; i < base::checked_cast<int>(requests_.size()); i++) {
    ...
    entries.emplace_back(
        PendingRequestKey(registration_->creation_microseconds_since_unix_epoch,
