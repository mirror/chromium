...
class CONTENT_EXPORT BackgroundFetchRegistrationId {
 public:
  ...
  // never re-used. Internal only, never exposed to JavaScript. Always use this
  // instead of |unsafe_id| (APIs that receive an |unsafe_id| from the website
  // must convert it to a |job_guid| as soon as possible).
