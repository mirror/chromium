...
TEST_F(BackgroundFetchDelegateProxyTest, StartRequest) {
  ...
  delegate_proxy_.CreateDownloadJob("jobid1", "Job 1", url::Origin(),
                                    controller.weak_ptr_factory_.GetWeakPtr(),
                                    0, 1, {});

  delegate_proxy_.StartRequest("jobid1", url::Origin(), request);
...
TEST_F(BackgroundFetchDelegateProxyTest, Abort) {
  ...
  delegate_proxy_.CreateDownloadJob("jobid2", "Job 2", url::Origin(),
                                    controller2.weak_ptr_factory_.GetWeakPtr(),
                                    0, 1, {});

  delegate_proxy_.StartRequest("jobid1", url::Origin(), request);
  delegate_proxy_.StartRequest("jobid2", url::Origin(), request2);
