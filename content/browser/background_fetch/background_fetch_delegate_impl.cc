...
void DownloadItemObserver::OnDownloadUpdated(DownloadItem* download_item) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
void DownloadItemObserver::OnDownloadDestroyed(DownloadItem* download_item) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
void BackgroundFetchDelegateImpl::DownloadUrl(
  ...
  std::unique_ptr<DownloadUrlParameters> download_parameters(
      base::MakeUnique<DownloadUrlParameters>(url, request_context_.get(),
                                              traffic_annotation));
  ...
  download_parameters->set_callback(
      base::Bind(&BackgroundFetchDelegateImpl::DidStartRequest,
...
void BackgroundFetchDelegateImpl::DidStartRequest(
  ...
  download_item->AddObserver(new DownloadItemObserver(client_));
