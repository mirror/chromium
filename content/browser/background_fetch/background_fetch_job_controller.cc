...
BackgroundFetchJobController::BackgroundFetchJobController(
    ...
    : registration_id_(registration_id),
      options_(options),
      downloaded_by_complete_requests_cache_(registration.downloaded),
      upload_total_(registration.upload_total),
      uploaded_cache_(registration.uploaded),
...
void BackgroundFetchJobController::DidStartRequest(
  ...
  uploaded_cache_ += request->fetch_request().blob_size;
}
...
void BackgroundFetchJobController::DidUpdateRequest(
  ...
  if (downloaded_by_incomplete_requests_[download_guid] == bytes_downloaded)
    return;

  downloaded_by_incomplete_requests_[download_guid] = bytes_downloaded;

  progress_callback_.Run(
      registration_id_.unique_id(), upload_total_, uploaded_cache_,
      options_.download_total,
      downloaded_by_complete_requests_cache_ + GetInProgressDownloadedBytes());
}
...
void BackgroundFetchJobController::DidCompleteRequest(
  ...
  downloaded_by_incomplete_requests_.erase(download_guid);
  downloaded_by_complete_requests_cache_ += request->GetFileSize();
...
uint64_t BackgroundFetchJobController::GetInProgressDownloadedBytes() {
  uint64_t sum = 0;
  for (const auto& entry : downloaded_by_incomplete_requests_)
    sum += entry.second;
  return sum;
}
