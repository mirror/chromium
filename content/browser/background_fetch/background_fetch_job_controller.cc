...
void BackgroundFetchJobController::DidMarkRequestCompleted(
  ...
  if (finished_callback_)
    std::move(finished_callback_).Run(registration_id_, false /* aborted */);
}

void BackgroundFetchJobController::AbortFromUser() {
  // This call will be ignored if the job has already completed/failed/aborted.
  if (finished_callback_)
    std::move(finished_callback_).Run(registration_id_, true /* aborted */);
}
