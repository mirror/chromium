...
void BackgroundFetchContext::DidGetRegistration(
  ...
  if (error == blink::mojom::BackgroundFetchError::NONE) {
    DCHECK(registration);
    // The data manager only has the number of bytes from completed downloads,
    // so augment this with the number of downloaded bytes from in-progress
    // jobs.
    DCHECK(job_controllers_.count(registration->unique_id));
    registration->downloaded += job_controllers_[registration->unique_id]
