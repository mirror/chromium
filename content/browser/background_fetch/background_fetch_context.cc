...
void IgnoreError(blink::mojom::BackgroundFetchError) {
  // TODO(johnme): Log these errors to UMA.
}
...
void BackgroundFetchContext::DidFinishJob(
    base::OnceCallback<void(blink::mojom::BackgroundFetchError)> mark_callback,
...
void BackgroundFetchContext::DidMarkForDeletion(
    ...
    base::OnceCallback<void(blink::mojom::BackgroundFetchError)> mark_callback,
    blink::mojom::BackgroundFetchError error) {
  std::move(mark_callback).Run(error);
...
void BackgroundFetchContext::DidGetSettledFetches(
  ...
  if (error != blink::mojom::BackgroundFetchError::NONE) {
    CleanupRegistration(registration_id, std::move(blob_handles));
    return;
  }
...
void BackgroundFetchContext::LastObserverGarbageCollected(
  ...
  data_manager_.DeleteRegistration(registration_id,
