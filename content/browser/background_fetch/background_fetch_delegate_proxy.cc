...
class BackgroundFetchDelegateProxy::Core {
 ...
 private:
  ...
  void DidStartRequest(scoped_refptr<BackgroundFetchRequestInfo> request,
    ...
    DCHECK_EQ(downloads_[guid], request);
...
void BackgroundFetchDelegateProxy::Core::DownloadItemObserver::
  ...
  auto iter = core_->downloads_.find(download_item->GetGuid());
...
void BackgroundFetchDelegateProxy::Core::DownloadItemObserver::
  ...
  if (core_.get()) {
    DCHECK_EQ(core_->downloads_.count(download_item->GetGuid()), 1u);
    core_->downloads_.erase(download_item->GetGuid());
  }
