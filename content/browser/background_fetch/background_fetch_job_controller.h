...
class CONTENT_EXPORT BackgroundFetchJobController final
 ...
 private:
  ...
  // Map from in-progress |download_guid|s to the number of bytes downloaded.
  base::flat_map<std::string, uint64_t> downloaded_by_incomplete_requests_;

  // Caches of the byte counts stored by the DataManager, to enable delivering
  // progress events without having to read from the database.
  uint64_t downloaded_by_complete_requests_cache_;
  const uint64_t upload_total_;
  uint64_t uploaded_cache_;
