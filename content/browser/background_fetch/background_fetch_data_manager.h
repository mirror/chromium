...
namespace features {
CONTENT_EXPORT extern const base::Feature kBackgroundFetchPersistence;
}  // namespace features
...
// for Background Fetch. There must only be a single instance of this class per
// BrowserContext, and it must only be used on the IO thread, since it relies on
// there being no other code concurrently reading/writing the Background Fetch
// keys of the database (except for deletions, e.g. it's safe for the Service
