...
enum class SWDBStatus { kOk, kFailed, kNotFound };

SWDBStatus SimplifyServiceWorkerDatabaseStatus(ServiceWorkerStatusCode status) {
  switch (status) {
    case SERVICE_WORKER_OK:
      return SWDBStatus::kOk;
...
class BackgroundFetchDataManager::DatabaseTask {
 ...
 protected:
  ...
  void Finished() {
    // Copy the |data_manager_| pointer onto the stack before deleting |this|.
...
class CreateRegistrationTask : public BackgroundFetchDataManager::DatabaseTask {
 ...
 private:
  void DidGetRegistration(const std::vector<std::string>& data,
    ...
    switch (SimplifyServiceWorkerDatabaseStatus(status)) {
      case SWDBStatus::kNotFound:
