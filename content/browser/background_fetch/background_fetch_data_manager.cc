...
std::string PendingRequestKeyPrefix(
    int64_t registration_creation_microseconds_since_unix_epoch,
    const BackgroundFetchRegistrationId& registration_id) {
  // These keys are ordered by priority rather than by registration_id, so the
  // highest priority pending requests can be looked up by fetching the
