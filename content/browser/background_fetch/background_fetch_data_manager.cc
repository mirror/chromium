...
class BackgroundFetchDataManager::RegistrationData {
 public:
  ...
  uint64_t GetDownloaded(BackgroundFetchDatabaseClient* controller) {
...
void BackgroundFetchDataManager::GetRegistration(
  ...
  BackgroundFetchDatabaseClient* controller =
      controllers_iter != controllers_.end() ? controllers_iter->second
                                             : nullptr;
...
BackgroundFetchDataManager::GetControllerFromUniqueID(
    const std::string& unique_id) {
  auto iter = controllers_.find(unique_id);
