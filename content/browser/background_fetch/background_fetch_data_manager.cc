...
// associated with a different |unique_id| that the given one which should have
...
class MarkRegistrationForDeletionTask
 ...
 public:
  ...
  void Start() override {
    // Lookup if there is already an active |unique_id| entry for this
    // |developer_id|.
    service_worker_context()->GetRegistrationUserData(
        registration_id_.service_worker_registration_id(),
        {ActiveRegistrationUniqueIdKey(registration_id_.developer_id())},
 ...
 private:
  void DidGetActiveUniqueId(const std::vector<std::string>& data,
    ...
    // registrations with the same |developer_id_tuple| (even though only one
    ...
    service_worker_context()->ClearRegistrationUserData(
        registration_id_.service_worker_registration_id(),
        {ActiveRegistrationUniqueIdKey(registration_id_.developer_id())},
        base::Bind(&MarkRegistrationForDeletionTask::DidClearActiveUniqueId,
                   weak_factory_.GetWeakPtr()));
  }

  void DidClearActiveUniqueId(ServiceWorkerStatusCode status) {
    switch (ToDatabaseStatus(status)) {
      case DatabaseStatus::kOk:
      case DatabaseStatus::kNotFound:
        break;
    ...
    // If CleanupTask runs after this, it shouldn't cleanup the
...
class DeleteRegistrationTask : public BackgroundFetchDataManager::DatabaseTask {
 ...
 private:
  void DidGetRegistration(const std::vector<std::string>& data,
                          ServiceWorkerStatusCode status) {
    std::vector<std::string> prefixes_to_clear = {RegistrationKey(unique_id_),
                                                  RequestKeyPrefix(unique_id_)};

    blink::mojom::BackgroundFetchError get_error =
    ...
    switch (ToDatabaseStatus(status)) {
...
#if DCHECK_IS_ON()
          if (registration_proto.has_developer_id()) {
            service_worker_context()->GetRegistrationUserData(
                service_worker_registration_id_,
                {ActiveRegistrationUniqueIdKey(
                    registration_proto.developer_id())},
                base::Bind(&DCheckRegistrationNotActive, unique_id_));
          }
#endif  // DCHECK_IS_ON()
          if (registration_proto.has_creation_microseconds_since_unix_epoch()) {
            prefixes_to_clear.emplace_back(PendingRequestKeyPrefix(
                registration_proto.creation_microseconds_since_unix_epoch(),
                unique_id_));
          } else {
            get_error = blink::mojom::BackgroundFetchError::STORAGE_ERROR;
          }
        } else {
          get_error = blink::mojom::BackgroundFetchError::STORAGE_ERROR;
        }
        // Continue with deletion even if we set |get_error| to STORAGE_ERROR
        // above, since the database claims to be fine (despite not containing
        // the expected information). Though perhaps we should blow it all away.
        break;
      }
      case DatabaseStatus::kFailed: {
        // Abandon deletion since the database gave us kFailed, so this probably
        // isn't a good time to write to it.
        std::move(callback_).Run(
            blink::mojom::BackgroundFetchError::STORAGE_ERROR);
        Finished();  // Destroys |this|.
        return;
      ...
      case DatabaseStatus::kNotFound: {
        get_error = blink::mojom::BackgroundFetchError::INVALID_ID;
        // Continue with deletion anyway even though we're missing a few
        // prefixes, in case it helps clean something up.
        break;
      }
    }

    service_worker_context()->ClearRegistrationUserDataByKeyPrefixes(
        service_worker_registration_id_, prefixes_to_clear,
        base::Bind(&DeleteRegistrationTask::DidDeleteRegistration,
                   weak_factory_.GetWeakPtr(), get_error));
  }

  void DidDeleteRegistration(blink::mojom::BackgroundFetchError get_error,
                             ServiceWorkerStatusCode status) {
    if (get_error != blink::mojom::BackgroundFetchError::NONE) {
      std::move(callback_).Run(get_error);
      Finished();  // Destroys |this|.
      return;
    }
