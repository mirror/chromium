...
class CreateRegistrationTask : public BackgroundFetchDataManager::DatabaseTask {
 ...
 private:
  ...
  void StoreRegistration() {
    ...
    for (const ServiceWorkerFetchRequest& fetch_request : requests_)
      upload_total_ += fetch_request.blob_size;

    // First serialize per-registration (as opposed to per-request) data.
    // TODO(crbug.com/757760): Serialize BackgroundFetchOptions and
    // |upload_total_| as part of this.
  ...
  void DidStoreRegistration(ServiceWorkerStatusCode status) {
    ...
    registration.upload_total = upload_total_;
...
class BackgroundFetchDataManager::RegistrationData {
 public:
  ...
  void MarkRequestAsStarted(BackgroundFetchRequestInfo* request,
    ...
    // This shouldn't double-count when retries happen, since requests with
    // non-empty bodies cannot be retried (unless
    // https://github.com/WICG/background-fetch/issues/58 is implemented).
    uploaded_ += request->fetch_request().blob_size;
  }
  ...
  bool MarkRequestAsComplete(BackgroundFetchRequestInfo* request) {
    ...
    downloaded_by_complete_requests_ += request->GetFileSize();
  ...
  uint64_t Downloaded(Controller* controller) {
    return downloaded_by_complete_requests_ +
           (controller ? controller->GetInProgressDownloadedBytes() : 0);
  }
  uint64_t upload_total() const { return upload_total_; }
  uint64_t uploaded() const { return uploaded_; }

 private:
  BackgroundFetchOptions options_;
  uint64_t downloaded_by_complete_requests_ = 0;
  uint64_t upload_total_ = 0;
...
void BackgroundFetchDataManager::GetRegistration(
  ...
  registration.downloaded = data->Downloaded(controller);
