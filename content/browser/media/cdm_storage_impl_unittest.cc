...
class CdmStorageTest : public RenderViewHostTestHarness {
 protected:
  ...
  bool Write(CdmFile* cdm_file,
             ...
             base::File* file) {
    bool status;
    cdm_file->OpenTempFileForWriting(
        base::Bind(&CdmStorageTest::FileOpenedForWrite, base::Unretained(this),
                   cdm_file, data, file, &status));
    RunAndWaitForResult();
    return status;
  }
 ...
 private:
  ...
  void FileOpenedForWrite(CdmFile* cdm_file,
    ...
    *status = bytes_to_write == bytes_written;
    cdm_file->RenameTempFileAndReopen(
