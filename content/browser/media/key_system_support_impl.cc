...
void SendKeySystemSupportedUMA(const std::string& key_system, bool available) {
  base::UmaHistogramBoolean(
      "Media.EME." + media::GetKeySystemNameForUMA(key_system) + ".Supported",
      available);
}
...
void KeySystemSupportImpl::IsKeySystemSupported(
  ...
  if (!cdm) {
    SendKeySystemSupportedUMA(key_system, false);
  ...
  SendKeySystemSupportedUMA(key_system, true);
