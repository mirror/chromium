...
// TODO(crbug/754872): Determine why content_browsertests with MUS enabled on
// Chrome OS fails. Logs say: gpu_process_transport_factory.cc check failed:
// use_gpu_compositing.
bool IsTestingDisabled() {
...
IN_PROC_BROWSER_TEST_F(WebContentsVideoCaptureDeviceBrowserTest,
  ...
  if (IsTestingDisabled()) {
    LOG(WARNING) << "Test disabled (http://crbug.com/754872).";
    return;
  }
