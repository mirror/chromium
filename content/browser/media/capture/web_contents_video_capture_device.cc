...
class WebContentsVideoCaptureDevice::FrameTracker : public WebContentsObserver {
 public:
...
void WebContentsVideoCaptureDevice::WillStart() {
  ...
  BrowserThread::PostTask(
      BrowserThread::UI, FROM_HERE,
      base::BindOnce(
          [](FrameTracker* tracker, const gfx::Size& capture_size) {
            auto* contents = tracker->web_contents();
            if (!contents) {
              return;
            }

            gfx::Size preferred_size;
            if (auto* view = tracker->GetCurrentView()) {
  ...
  BrowserThread::PostTask(BrowserThread::UI, FROM_HERE,
                          base::BindOnce(
                              [](FrameTracker* tracker) {
                                if (auto* contents = tracker->web_contents()) {
