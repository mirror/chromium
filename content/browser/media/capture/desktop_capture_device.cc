...
void DesktopCaptureDevice::Core::OnCaptureResult(
  ...
  if (frame->size().equals(webrtc::DesktopSize(1, 1))) {
    ...
    if (!frame->size().equals(output_size)) {
      // Make width/height of the captured frame even. Odd width/height will
      // cause blurring in the following scaling process. See
      // https://crbug.com/737278.
    ...
    if (IsFrameUnpackedOrInverted(frame.get())) {
