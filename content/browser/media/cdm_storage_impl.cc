...
const size_t kPathSeparatorsLength = arraysize(kPathSeparators);

// A name used by the PluginPrivateFileSystem should not:
//    (1) be empty,
// or (2) contain any path separators.
bool IsValidFileSystemName(const std::string& name) {
  return (!name.empty() &&
          name.find_first_of(kPathSeparators, kPathSeparatorsLength) ==
              std::string::npos);
}
...
void CdmStorageImpl::Create(RenderFrameHost* render_frame_host,
                            const std::string& file_system_plugin_id,
  ...
  new CdmStorageImpl(render_frame_host, file_system_plugin_id,
...
CdmStorageImpl::CdmStorageImpl(
    ...
    : FrameServiceBase(render_frame_host, std::move(request)),
      file_system_plugin_id_(file_system_plugin_id),
...
void CdmStorageImpl::Open(const std::string& file_name, OpenCallback callback) {
  ...
  if (!IsValidFileSystemName(file_system_plugin_id_)) {
  ...
  if (!IsValidFileSystemName(file_name)) {
  ...
  FileLockKey file_lock_key =
      std::make_tuple(file_system_plugin_id_, origin(), file_name);
  ...
  file_system_context_->OpenPluginPrivateFileSystem(
      ...
      file_system_plugin_id_, storage::OPEN_FILE_SYSTEM_CREATE_IF_NONEXISTENT,
