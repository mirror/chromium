...
void CdmStorageImpl::Open(const std::string& file_name, OpenCallback callback) {
  ...
  // The file system should only be opened once. If it is already open,
  // create and initialize the CdmFileImpl object.
  if (file_system_state_ == FileSystemState::kOpened) {
    CreateCdmFile(file_name, std::move(callback));
    return;
  }

  // If it has been attempted and failed, we can't create the CdmFile objects.
  ...
  pending_open_calls_.push_back(std::make_pair(file_name, std::move(callback)));
...
void CdmStorageImpl::OnFileSystemOpened(base::File::Error error) {
  ...
  if (error != base::File::FILE_OK) {
    ...
    for (auto it = pending_open_calls_.begin(); it != pending_open_calls_.end();
         ++it) {
      std::move(it->second).Run(Status::kFailure, base::File(), nullptr);
    }
    return;
  }
  ...
  for (auto it = pending_open_calls_.begin(); it != pending_open_calls_.end();
       ++it) {
    CreateCdmFile(it->first, std::move(it->second));
  }
}
...
void CdmStorageImpl::CreateCdmFile(const std::string& file_name,
  ...
  auto cdm_file_impl = std::make_unique<CdmFileImpl>(
      file_name, origin(), cdm_file_system_id_, file_system_context_,
