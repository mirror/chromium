...
// Helper function to extract the file name from |key|.
std::string GetFileNameFromKey(const Key& key) {
  const size_t kFileNameIndex = 2;
  return std::get<kFileNameIndex>(key);
}
...
class FileLockMap {
 public:
  ...
  bool AcquireFileLock(const Key& key) {
    DVLOG(3) << __func__ << " file: " << GetFileNameFromKey(key);
...
void CdmStorageImpl::OnFileSystemOpened(const FileLockKey& file_lock_key,
  ...
  std::string root = storage::GetIsolatedFileSystemRootURIString(
                         ...
                         .append(GetFileNameFromKey(file_lock_key));
...
void CdmStorageImpl::OnFileOpened(const FileLockKey& file_lock_key,
  ...
  if (!file.IsValid()) {
    DLOG(WARNING) << "Unable to open file " << GetFileNameFromKey(file_lock_key)
                  << ", error: "
