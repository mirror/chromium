...
CdmStorageImpl::FileLockKey::FileLockKey(const std::string& file_system_id,
                                         url::Origin origin,
...
bool CdmStorageImpl::FileLockKey::operator<(const FileLockKey& other) const {
  return (file_system_id < other.file_system_id) ||
         (file_system_id == other.file_system_id && origin < other.origin) ||
         (file_system_id == other.file_system_id && origin == other.origin &&
...
void CdmStorageImpl::Open(const std::string& file_name, OpenCallback callback) {
  ...
  FileLockKey file_lock_key(file_system_id_, origin(), file_name);
