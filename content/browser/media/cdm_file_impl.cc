...
CdmFileImpl::~CdmFileImpl() {
  ...
  if (lock_state_ == LockState::kOpenForReadAndWrite) {
...
void CdmFileImpl::Initialize(OpenFileCallback callback) {
  ...
  lock_state_ = LockState::kOpenForRead;
...
void CdmFileImpl::OnFileOpenedForReading(
  ...
  DVLOG(3) << __func__ << " file: " << file_name_;
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  DCHECK_EQ(LockState::kOpenForRead, lock_state_);
  DCHECK(pending_open_callback_);
...
void CdmFileImpl::OpenTempFileForWriting(
  ...
  lock_state_ = LockState::kOpenForReadAndWrite;
  pending_open_callback_ = std::move(callback);
...
void CdmFileImpl::OnTempFileOpenedForWriting(
  ...
  DVLOG(3) << __func__ << " file: " << file_name_;
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  DCHECK_EQ(LockState::kOpenForReadAndWrite, lock_state_);
  DCHECK(pending_open_callback_);
...
void CdmFileImpl::RenameTempFileAndReopen(
  ...
  // Fail if this is called out of order. We must have grabbed locks on both
  // the original and temporary file, and there should be no call in progress.
  if (!IsFileLockHeld(file_name_) || !IsFileLockHeld(temp_file_name_) ||
  ...
  // OpenFile() will be called eventually, so save |callback|.
  pending_open_callback_ = std::move(callback);
...
void CdmFileImpl::OnTempFileRenamed(base::File::Error move_result) {
  DVLOG(3) << __func__;
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  DCHECK_EQ(LockState::kOpenForReadAndWrite, lock_state_);
  DCHECK(pending_open_callback_);
