...
class MediaSessionControllersManagerTest
 ...
 public:
  void SetUp() override {
    ...
    mock_media_session_controller_ =
        new StrictMock<MockMediaSessionController>(
 ...
 protected:
  ...
  StrictMock<MockMediaSessionController>* mock_media_session_controller_ = nullptr;
...
TEST_F(MediaSessionControllersManagerTest, RequestPlayNotEnabledReturnsTrue) {
  ...
  controllers_map_ = manager_->GetControllersMapForTest();
  EXPECT_TRUE(controllers_map_->empty());
...
TEST_F(MediaSessionControllersManagerTest, NewMediaPlayersAddSessionsToMap) {
 ...
 EXPECT_TRUE(manager_->RequestPlay(
      ...
      false,
      media::MediaContentType::Transient));
 EXPECT_EQ(2U, controllers_map_->size());
}
...
TEST_F(MediaSessionControllersManagerTest, RenderFrameDeletedNotEnabledNoOps) {
  ...
  (*controllers_map_)[media_player_id_] =
