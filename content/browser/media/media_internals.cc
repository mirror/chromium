...
class MediaInternals::MediaInternalsUMAHandler {
 ...
 private:
  ...
  void RecordUkmPlaybackData(const PipelineInfo& player_info) {
    ...
    if (player_info.watch_time_info.empty())
      return;
    ...
    for (auto& kv : player_info.watch_time_info) {
    ...
    switch (finalize_type) {
      case FinalizeType::EVERYTHING:
        ...
        *underflow_count = 0;
        ...
        RecordWatchTimeWithFilter(watch_time_info,
                                  base::flat_set<base::StringPiece>());
      case FinalizeType::POWER_ONLY:
        RecordWatchTimeWithFilter(watch_time_info, watch_time_power_keys_);
        break;
      case FinalizeType::CONTROLS_ONLY:
        RecordWatchTimeWithFilter(watch_time_info, watch_time_controls_keys_);
        break;
    }
  }
