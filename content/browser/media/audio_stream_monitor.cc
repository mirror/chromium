...
void AudioStreamMonitor::RenderProcessGone(int render_process_id) {
  ...
  for (auto it = streams_.begin(); it != streams_.end();) {
    if (it->first.render_process_id == render_process_id) {
      // Erase the streams locally before updating to avoid removing streams
      // from the process twice, since RenderProcessHost removes the streams
      // on its own when the renderer process is gone.
