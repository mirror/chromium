...
// |plugin_id| must contain only letters (A-Za-z), digits(0-9), or "._-".
static bool IsValidFileSystemId(const std::string& plugin_id) {
  for (const auto& ch : plugin_id) {
...
media::mojom::InterfaceFactory* MediaInterfaceProxy::GetCdmInterfaceFactory(
  ...
  if (!IsValidFileSystemId(cdm_info->file_system_id)) {
