 ...
 public:
  void RunSuspendTest(const std::string& load_until) {
...

    {
      ...
      if (title_watcher.WaitAndGetTitle() != kLoaded) {
        FAIL();
        return;
      }
    }

    VLOG(0) << "Suspending players.";
...

    {
      ...
      title_watcher.AlsoWaitForTitle(kError);
      ASSERT_TRUE(ExecuteScript(shell(), "startPlayback();"));
      EXPECT_EQ(kEnded, title_watcher.WaitAndGetTitle());
    }
...
IN_PROC_BROWSER_TEST_F(MediaSuspendTest, BasicSuspend) {
  RunSuspendTest("canplaythrough");

  // load test, stop at none, metadata, have_enough, have_future
  // force a suspend.
  // toggle playback and wait currentTime > 0 || ended.
}
