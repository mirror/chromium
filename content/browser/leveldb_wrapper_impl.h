...
class CONTENT_EXPORT LevelDBWrapperImpl : public mojom::LevelDBWrapper {
 public:
  ...
  CacheMode cache_mode() { return desired_cache_mode_; }
 ...
 private:
  FRIEND_TEST_ALL_PREFIXES(ParamTest, CommitOnDifferentCacheModes);
  ...
  // internal storage type after the next commit.
  // Notification to the observers when an item is mutated depends on the
  // |client_old_value| when cache contains only keys. Using GetAll during
  // keys-only cache mode will cause extra disk access.
  ...
  MapState map_state_ = MapState::INVALID;
  CacheMode desired_cache_mode_;
