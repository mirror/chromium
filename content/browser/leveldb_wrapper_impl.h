...
class CONTENT_EXPORT LevelDBWrapperImpl : public mojom::LevelDBWrapper {
 ...
 private:
  ...
  // This is called if the source of the fork dies before it loads its own map.
  void OnForkSourceEarlyDeath(std::vector<uint8_t> source_prefix);
  void OnEarlyDeathPrefixCopyResult(leveldb::mojom::DatabaseError error);
  ...
  // If |source_fork_died_before_load_| is true, this wrapper was created as a
  // fork, and the fork source died before loading its own map.
  bool source_fork_died_before_load_ = false;
  // Populated when |source_fork_died_before_load_| is true to allow copying of
  // the prefix before loading this wrapper's map.
  std::vector<uint8_t> source_fork_prefix_;
