...
class SharedWorkerServiceImplTest : public testing::Test {
 ...
 protected:
  SharedWorkerServiceImplTest()
    ...
    SharedWorkerServiceImpl::GetInstance()->s_reserve_process_func_ =
        &SharedWorkerServiceImplTest::MockReserveProcess;
    SharedWorkerServiceImpl::GetInstance()->s_release_process_func_ =
        &SharedWorkerServiceImplTest::MockReleaseProcess;
    SharedWorkerServiceImpl::GetInstance()->s_is_process_alive_func_ =
        &SharedWorkerServiceImplTest::MockIsProcessAlive;
  }
  ...
  static bool MockReserveProcess(int worker_process_id,
                                 int* route_id,
                                 mojom::SharedWorkerFactoryPtr* factory) {
    auto iter = s_reserved_process_map_.find(worker_process_id);
    if (iter == s_reserved_process_map_.end())
      return false;

    // Increment refcount on this process.
    iter->second++;
  ...
  static void MockReleaseProcess(int worker_process_id) {
    auto iter = s_reserved_process_map_.find(worker_process_id);
    if (iter != s_reserved_process_map_.end()) {
  ...
  static bool MockIsProcessAlive(int worker_process_id) {
    return s_reserved_process_map_.find(worker_process_id) !=
