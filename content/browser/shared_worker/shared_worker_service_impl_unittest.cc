...
base::StringPiece VectorToStringPiece(const std::vector<uint8_t>& v) {
  return base::StringPiece(reinterpret_cast<const char*>(v.data()), v.size());
}
...
TEST_F(SharedWorkerServiceImplTest, BasicTest) {
  ...
  connector->local_port().PostMessage(VectorToStringPiece(expected_message),
...
TEST_F(SharedWorkerServiceImplTest, TwoRendererTest) {
  ...
  connector0->local_port().PostMessage(VectorToStringPiece(expected_message1),
                                       std::vector<MessagePort>());
  ...
  connector1->local_port().PostMessage(VectorToStringPiece(expected_message2),
                                       std::vector<MessagePort>());
