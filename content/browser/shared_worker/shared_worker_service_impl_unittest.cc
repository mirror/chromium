...
class MockRendererProcessHost {
 public:
  MockRendererProcessHost(BrowserContext* browser_context,
                          ResourceContext* resource_context,
  ...
  void CreateSharedWorkerConnector(
    ...
    SharedWorkerServiceImplTest::CreateSharedWorkerConnector(
        process_host_->GetID(), frame_id, resource_context_, partition_,
        std::move(request));
  }

  int GetID() { return process_host_->GetID(); }
 ...
 private:
  ...
  std::unique_ptr<content::MockRenderProcessHost> process_host_;
};
...
TEST_F(SharedWorkerServiceImplTest, BasicTest) {
  std::unique_ptr<MockRendererProcessHost> renderer_host(
      new MockRendererProcessHost(browser_context_.get(),
