...
void SharedWorkerServiceImpl::OnProcessClosing(int process_id) {
  ...
  std::vector<WorkerHostID> remove_list;
  for (const auto& it : worker_hosts_) {
    ...
    if (host->process_id() == process_id) {
      remove_list.push_back(
          std::make_pair(host->process_id(), host->route_id()));
    }
  }
  for (const WorkerHostID to_remove : remove_list)
...
void SharedWorkerServiceImpl::DestroyHost(int process_id, int route_id) {
  worker_hosts_.erase(std::make_pair(process_id, route_id));
}
...
void SharedWorkerServiceImpl::RenderProcessReservedCallback(
  ...
  worker_hosts_[std::make_pair(worker_process_id, worker_route_id)] =
      std::move(host);
  for (auto& observer : observers_) {
...
SharedWorkerHost* SharedWorkerServiceImpl::FindSharedWorkerHost(int process_id,
                                                                int route_id) {
  auto iter = worker_hosts_.find(std::make_pair(process_id, route_id));
