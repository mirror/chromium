 ...
 public:
  ...
  void RegisterToSharedWorkerHost(SharedWorkerHost* host) {
    for (const auto& request : requests_) {
      bool mismatch = host->instance()->creation_context_type() !=
                      request->creation_context_type;
      request->client->OnCreated(static_cast<uint32_t>(
          mismatch ? blink::kWebWorkerCreationErrorSecureContextMismatch
...
void SharedWorkerServiceImpl::AddFilter(SharedWorkerMessageFilter* filter) {
  base::AutoLock locked(filters_lock_);
  filters_.insert(
      std::make_pair(filter->render_process_id(), make_scoped_refptr(filter)));
}
...
scoped_refptr<SharedWorkerMessageFilter> SharedWorkerServiceImpl::GetFilter(
    int render_process_id) {
  base::AutoLock locked(filters_lock_);
  return filters_.find(render_process_id)->second;
}
...
void SharedWorkerServiceImpl::CreateWorker(
  ...
  std::unique_ptr<SharedWorkerInstance> instance(new SharedWorkerInstance(
      ...
      static_cast<blink::WebAddressSpace>(info->creation_address_space),
      resource_context, partition_id,
      static_cast<blink::WebSharedWorkerCreationContextType>(
          info->creation_context_type),
...
void SharedWorkerServiceImpl::ReserveRenderProcessToCreateWorker(
  ...
  if (host) {
    ...
    worker_route_id = GetFilter(worker_process_id)->GetNextRoutingID();
  }
