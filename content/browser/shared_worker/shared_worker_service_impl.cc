...
class SharedWorkerServiceImpl::SharedWorkerReserver {
 ...
 private:
  ...
  void TryReserveOnUI(
    ...
    if (!try_reserve_worker_func(worker_process_id_, &factory_info)) {
...
void SharedWorkerServiceImpl::OnSharedWorkerMessageFilterClosing(
    SharedWorkerMessageFilter* filter) {
  ScopedWorkerDependencyChecker checker(this);
  std::vector<SharedWorkerHost::ID> remove_list;
  for (const auto& it : worker_hosts_) {
    if (it.second->process_id() == filter->render_process_id())
      remove_list.push_back(it.second->id());
  }
  for (const SharedWorkerHost::ID to_remove : remove_list)
...
void SharedWorkerServiceImpl::DestroyHost(SharedWorkerHost::ID id) {
  worker_hosts_.erase(id);
} 
...
void SharedWorkerServiceImpl::RenderProcessReservedCallback(
  ...
  SharedWorkerHost::ID id = next_worker_id_++;
  auto host = std::make_unique<SharedWorkerHost>(
      id, pending_instance->release_instance(), worker_process_id,
      worker_route_id);
  ...
  worker_hosts_[id] = std::move(host);
  for (auto& observer : observers_) {
...
void SharedWorkerServiceImpl::RenderProcessReserveFailedCallback(
    ...
    bool pause_on_start) {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  // XXX  worker_hosts_.erase(std::make_pair(worker_process_id,
  // worker_route_id));
  if (!base::ContainsKey(pending_instances_, pending_instance_id))
    return;
  std::unique_ptr<SharedWorkerPendingInstance> pending_instance(
...
SharedWorkerHost* SharedWorkerServiceImpl::FindSharedWorkerHost(int process_id,
  ...
  for (const auto& iter : worker_hosts_) {
    SharedWorkerHost* host = iter.second.get();
    if (host->IsAvailable() && host->process_id() == process_id &&
        host->route_id() == route_id)
      return host;
  }
  return nullptr;
}
