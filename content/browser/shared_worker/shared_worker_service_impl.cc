...
bool TryReserveWorker(int process_id, mojom::SharedWorkerFactoryPtr* factory) {
  auto* host = static_cast<RenderProcessHostImpl*>(
      RenderProcessHost::FromID(process_id));
  if (!IsHostAlive(host))
    return false;
...
void SharedWorkerServiceImpl::ConnectToWorker(
  ...
  // XXX support mocking this
  auto* render_process_host = RenderProcessHost::FromID(worker_process_id);
  CHECK(render_process_host);
  int worker_route_id = render_process_host->GetNextRoutingID();

  mojom::SharedWorkerFactoryPtr factory;
  // Take care to only create a factory instance when creating a new worker.
  // This avoids unnecessary IPC.
  if (!s_try_reserve_worker_func_(worker_process_id, &factory)) {
    // The requesting process must be shutting down, so we can just drop this
    // request.
    return;
  }
  bool pause_on_start =
  ...
  auto new_host = std::make_unique<SharedWorkerHost>(
      std::move(instance), worker_process_id, worker_route_id);
  new_host->Start(std::move(factory), pause_on_start);
  new_host->AddClient(std::move(client), process_id, frame_id, message_port);

  const GURL url = new_host->instance()->url();
  const std::string name = new_host->instance()->name();

  worker_hosts_[WorkerID(worker_process_id, worker_route_id)] =
      std::move(new_host);
...
void SharedWorkerServiceImpl::DestroyHost(int process_id, int route_id) {
  ...
  DecrementWorkerRefCount(process_id);
}
