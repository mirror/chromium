...
void SharedWorkerHost::WorkerConnected(int connection_request_id) {
  ...
  for (const ClientInfo& info : clients_) {
    ...
    for (blink::mojom::WebFeature feature : used_features_)
      features.push_back(feature);
    info.client->OnConnected(std::move(features));
    return;
  }
}
...
void SharedWorkerHost::AddClient(mojom::SharedWorkerClientPtr client,
  ...
  clients_.emplace_back(std::move(client), process_id, frame_id);
  ClientInfo& info = clients_.back();

  // Observe when the client goes away.
  info.client.set_connection_error_handler(base::Bind(
      &SharedWorkerHost::OnClientConnectionLost, weak_factory_.GetWeakPtr()));

  info.connection_request_id = next_connection_request_id_++;
