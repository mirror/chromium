...
SharedWorkerHost::SharedWorkerHost(ID id,
...
void SharedWorkerHost::Start(mojom::SharedWorkerFactoryPtr factory,
  ...
  mojom::SharedWorkerInfoPtr info(mojom::SharedWorkerInfo::New(
      ...
      instance_->creation_address_space(), instance_->creation_context_type(),
      instance_->data_saver_enabled()));
...
void SharedWorkerHost::OnContextDestroyed() {
  // Disconnect all clients to signal destruction of the worker.
  clients_.clear();

  SharedWorkerServiceImpl::GetInstance()->DestroyHost(id_);
}
