 ...
 public:
  ...
  int Read(int index,
    ...
    return disk_cache_entry_->ReadData(
        index, static_cast<int>(offset), buf, buf_len, base::AdaptCallbackForRepeating(std::move(callback)));
  }
  ...
  int Write(int index,
    ...
    return disk_cache_entry_->WriteData(
        index, static_cast<int>(offset), buf, buf_len, base::AdaptCallbackForRepeating(std::move(callback)), kTruncate);
  }
...
int AppCacheDiskCache::InitWithMemBackend(
    int mem_cache_size, OnceCompletionCallback callback) {
...
int AppCacheDiskCache::CreateEntry(int64_t key,
  ...
  if (is_initializing_or_waiting_to_initialize()) {
      pending_calls_.push_back(PendingCall(CREATE, key, entry, std::move(callback)));
  ...
  return ActiveCall::CreateEntry(
      weak_factory_.GetWeakPtr(), key, entry, std::move(callback));
}
...
int AppCacheDiskCache::OpenEntry(int64_t key,
  ...
  if (is_initializing_or_waiting_to_initialize()) {
      pending_calls_.push_back(PendingCall(OPEN, key, entry, std::move(callback)));
    return net::ERR_IO_PENDING;
  }
...
int AppCacheDiskCache::DoomEntry(int64_t key,
  ...
  if (is_initializing_or_waiting_to_initialize()) {
      pending_calls_.push_back(PendingCall(DOOM, key, NULL, std::move(callback)));
    return net::ERR_IO_PENDING;
  }
  ...
  return ActiveCall::DoomEntry(weak_factory_.GetWeakPtr(), key, std::move(callback));
}
...
AppCacheDiskCache::PendingCall::PendingCall(
    ...
    Entry** entry,
    OnceCompletionCallback callback)
    : call_type(call_type), key(key), entry(entry), callback(std::move(callback)) {}

AppCacheDiskCache::PendingCall::PendingCall(AppCacheDiskCache::PendingCall&&) = default;
AppCacheDiskCache::PendingCall::PendingCall& operator=(AppCacheDiskCache::PendingCall&&) = default;
...
void AppCacheDiskCache::OnCreateBackendComplete(int rv) {
  ...
  for (PendingCalls::iterator iter = pending_calls_.begin();
       iter < pending_calls_.end(); ++iter) {
    int rv = net::ERR_FAILED;
    switch (iter->call_type) {
      case CREATE:
          rv = CreateEntry(iter->key, iter->entry, std::move(iter->callback));
        break;
      case OPEN:
          rv = OpenEntry(iter->key, iter->entry, std::move(iter->callback));
        break;
      case DOOM:
          rv = DoomEntry(iter->key, std::move(iter->callback));
        break;
      default:
        NOTREACHED();
        break;
