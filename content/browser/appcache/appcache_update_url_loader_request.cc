...
int AppCacheUpdateJob::UpdateURLLoaderRequest::Read(net::IOBuffer* buf,
                                                    int max_bytes) {
  // If we already have a valid mojo data pipe, then we should setup
  // the pipe reader to initiate a read.
  if (!pipe_reader_.get() && handle_.is_valid()) {
    pipe_reader_.reset(
        new mojo::common::DataPipeDrainer(this, std::move(handle_)));
  }
  buffer_ = buf;
...
void AppCacheUpdateJob::UpdateURLLoaderRequest::OnReceiveResponse(
  ...
  *response_ = response_head;
  ...
  int net_error =
      response_code >= 200 && response_code <= 226 ? net::OK : net::ERR_FAILED;
...
void AppCacheUpdateJob::UpdateURLLoaderRequest::OnStartLoadingResponseBody(
  ...
  if (!pipe_reader_.get() && buffer_.get()) {
    pipe_reader_.reset(
        new mojo::common::DataPipeDrainer(this, std::move(handle_)));
  }
}
...
void AppCacheUpdateJob::UpdateURLLoaderRequest::OnDataComplete() {
  ...
  fetcher_->OnResponseCompleted(response_status_.error_code);
}
