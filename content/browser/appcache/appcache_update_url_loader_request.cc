...
int AppCacheUpdateJob::UpdateURLLoaderRequest::OnDataAvailable(
  ...
  memcpy(buffer_->data(), data, bytes_copied);

  // Clear out the buffers here to ensure that we only read when the caller
  // initiates a read.
  buffer_ = nullptr;
  buffer_size_ = 0;

  fetcher_->OnReadCompleted(bytes_copied);
  return bytes_copied;
}

void AppCacheUpdateJob::UpdateURLLoaderRequest::OnDataComplete() {
  DCHECK(response_status_.error_code == net::OK);
  fetcher_->OnReadCompleted(0);
}
...
void AppCacheUpdateJob::UpdateURLLoaderRequest::StartReading(
  ...
  int bytes_copied = OnDataAvailable(buffer, available);
  result = mojo::EndReadDataRaw(handle_.get(), bytes_copied);
