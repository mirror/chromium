...
bool CanUseExistingResource(const net::HttpResponseInfo* http_info) {
  ...
  // but we special case varying on the "Origin" header in isolation since
  // its invariant for the lifetime of the AppCache.
  std::string value;
  size_t iter = 0;
  if (!http_info->headers->EnumerateHeader(&iter, "vary", &value))
    return true;
  return value == "Origin";
}
