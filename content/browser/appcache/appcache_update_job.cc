...
bool CanUseExistingResource(const net::HttpResponseInfo* http_info) {
  ...
  // but we special case varying on the "Origin" header since its invariant
  // for the lifetime of the AppCache, and on the "Accept-Encoding" header
  // since we storage already decoded response bodies.
  std::string value;
  size_t iter = 0;
  while (http_info->headers->EnumerateHeader(&iter, "vary", &value)) {
    if (!base::EqualsCaseInsensitiveASCII(value, "Origin") &&
        !base::EqualsCaseInsensitiveASCII(value, "Accept-Encoding")) {
      return false;
    }
  }
  return true;
