...
class AppCacheUpdateJobTest : public testing::TestWithParam<RequestHandlerType>,
 ...
 public:
  ...
  void RunTestOnIOThread(Method method) {
    ...
    if (request_handler_type_ == URLLOADER) {
      // The URLLoaderFactory has to be initialized on the IO thread.
      io_thread_->task_runner()->PostTask(
          FROM_HERE, base::Bind(&AppCacheUpdateJobTest::InitializeFactory,
    ...
    if (request_handler_type_ == URLLOADER) {
      // The URLLoaderFactory has to be uninitialized on the IO thread.
      io_thread_->task_runner()->PostTask(
          FROM_HERE, base::BindOnce(&AppCacheUpdateJobTest::UninitializeFactory,
                                    std::move(test_loader_factory_)));
    }
  }

  void InitializeFactory() {
    MockURLLoaderFactory::Create(&test_loader_factory_);

    AppCacheUpdateJob::UpdateURLLoaderRequest::SetLoaderFactoryForTesting(
        test_loader_factory_.get());
  }

  static void UninitializeFactory(mojom::URLLoaderFactoryPtr loader_factory) {
    AppCacheUpdateJob::UpdateURLLoaderRequest::SetLoaderFactoryForTesting(
        nullptr);
  }
 ...
 private:
  ...
  mojom::URLLoaderFactoryPtr test_loader_factory_;
};
...
TEST_P(AppCacheUpdateJobTest, IfModifiedSince) {
