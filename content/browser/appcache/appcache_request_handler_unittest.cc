...
class AppCacheRequestHandlerTest
 ...
 public:
  ...
  void MainResource_Miss() {
    ...
    job_.reset(handler_->MaybeLoadResource(request_->GetDelegate()));
  ...
  void Verify_MainResource_Miss() {
    ...
    std::unique_ptr<AppCacheJob> fallback_job(
        handler_->MaybeLoadFallbackForRedirect(request_->GetDelegate(),
                                               GURL("http://blah/redirect")));
    EXPECT_FALSE(fallback_job);
    fallback_job.reset(
        handler_->MaybeLoadFallbackForResponse(request_->GetDelegate()));
    EXPECT_FALSE(fallback_job);
  ...
  void SimulateResponseCode(int response_code) {
    ...
    if (request_handler_type_ == URLREQUEST) {
      job_factory_->SetJob(base::MakeUnique<MockURLRequestJob>(
          url_request_.get(), request_->GetDelegate(), info));
      request_->AsURLRequest()->Start();
  ...
  void Verify_MainResource_Fallback() {
    ...
    job_.reset(handler_->MaybeLoadFallbackForResponse(request_->GetDelegate()));
    EXPECT_TRUE(job_.get());
    EXPECT_TRUE(job_->IsDeliveringAppCacheResponse());
  ...
  void SubResource_RedirectFallback() {
    ...
    job_.reset(handler_->MaybeLoadFallbackForRedirect(
        request_->GetDelegate(), GURL("http://not_blah/redirect")));
    EXPECT_TRUE(job_.get());
    EXPECT_TRUE(job_->IsDeliveringAppCacheResponse());
  ...
  void DestroyedHost() {
    ...
    EXPECT_FALSE(handler_->MaybeLoadResource(request_->GetDelegate()));
    EXPECT_FALSE(handler_->MaybeLoadFallbackForRedirect(
        request_->GetDelegate(), GURL("http://blah/redirect")));
    EXPECT_FALSE(
        handler_->MaybeLoadFallbackForResponse(request_->GetDelegate()));
  ...
  void UnsupportedScheme() {
    ...
    EXPECT_FALSE(handler_->MaybeLoadResource(request_->GetDelegate()));
    EXPECT_FALSE(handler_->MaybeLoadFallbackForRedirect(
        request_->GetDelegate(), GURL("ftp://blah/redirect")));
    EXPECT_FALSE(
        handler_->MaybeLoadFallbackForResponse(request_->GetDelegate()));
