  ...
  if (bytes_read != net::ERR_IO_PENDING) {
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        ...
        base::Bind(&AppCacheUpdateJob::UpdateURLRequest::OnReadCompleted,
                   weak_factory_.GetWeakPtr(), request_, bytes_read));
  }
}
...
bool AppCacheUpdateJob::UpdateURLRequest::ShouldIgnoreCertificateErrors()
    const {
  const net::HttpNetworkSession::Params* session_params =
      request_->context()->GetNetworkSessionParams();
  return session_params && session_params->ignore_certificate_errors;
}
