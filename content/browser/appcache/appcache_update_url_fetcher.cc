...
void AppCacheUpdateJob::URLFetcher::OnResponseStarted(int net_error) {
  ...
  if (url_.SchemeIsCryptographic()) {
    ...
    if ((net::IsCertStatusError(
         ...
         !request_->ShouldIgnoreCertificateErrors()) ||
...
void AppCacheUpdateJob::URLFetcher::OnReadCompleted(net::IOBuffer* buffer,
  ...
  job_->MadeProgress();
  bool data_consumed = ConsumeResponseData(buffer, bytes_read);
  if (data_consumed) {
    // Initiate an asynchronous read here and return. request_->Read() can
    // synchronously call OnReadCompleted(), which can result in us getting
    // deleted in OnResponseCompleted() below. A task avoids these issues.
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::Bind(&AppCacheUpdateJob::UpdateRequestBase::Read,
...
void AppCacheUpdateJob::URLFetcher::OnResponseCompleted(int net_error) {
  if (net_error == net::OK) {
    job_->MadeProgress();
  } else if ((result_ == AppCacheUpdateJob::UPDATE_OK) && (net_error < 0)) {
    result_ = AppCacheUpdateJob::NETWORK_ERROR;
  }
