...
URLFetcher::URLFetcher(const GURL& url,
    ...
    : url_(url),
      ...
      request_(
          AppCacheUpdateRequestBase::Create(job->service_->request_context(),
                                            url,
                                            this)),
...
void URLFetcher::OnReceivedRedirect(const net::RedirectInfo& redirect_info,
                                    bool* defer_redirect) {
  if (!request_)
    return;
...
void URLFetcher::OnResponseStarted(int net_error) {
  if (!request_)
    return;
...
void URLFetcher::OnReadCompleted(int bytes_read) {
  if (!request_)
    return;
...
bool URLFetcher::MaybeRetryRequest() {
  ...
  ++retry_503_attempts_;
  result_ = AppCacheUpdateJob::UPDATE_OK;
  request_ = AppCacheUpdateRequestBase::Create(
      job_->service_->request_context(), url_, this);
  Start();
  return true;
}
