...
void NavigationRequest::OnResponseStarted(
  ...
  // decision.
  const GURL& base_url = common_params_.base_url_for_data_url.is_empty()
                             ? common_params_.url
                             : common_params_.base_url_for_data_url;
  if (render_frame_host && SiteInstanceImpl::ShouldAssignSiteForURL(base_url)) {
    ...
    if (!instance->HasSite() &&
        SiteInstanceImpl::DoesSiteRequireDedicatedProcess(
            instance->GetBrowserContext(), base_url)) {
      instance->SetSite(base_url);
    }
  }
