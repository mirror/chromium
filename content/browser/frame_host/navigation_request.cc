...
void NavigationRequest::OnRequestFailed(
  ...
  // download. This could happen if URLLoaderClient has posted a OnComplete()
  // call indicating that the request has failed, while at the same time
  // OnResponseStarted() is called to cancel the request due to download
  // interception.
  if (state_ == FAILED) {
