...
RenderFrameHost* AddChildFrame(RenderFrameHost* rfh, const GURL& url) {
  ...
  SimulateNavigation(&child_rfh, url);
  return child_rfh;
}
...
class FrameServiceBaseTest : public RenderViewHostTestHarness {
 protected:
  ...
  void Initialize() {
    main_rfh_ = web_contents()->GetMainFrame();
    RenderFrameHostTester::For(main_rfh_)->InitializeRenderFrameIfNeeded();
    SimulateNavigation(&main_rfh_, GURL(kFooOrigin));
  }
