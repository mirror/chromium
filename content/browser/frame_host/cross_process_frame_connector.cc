...
void CrossProcessFrameConnector::BubbleScrollEvent(
  ...
  gfx::Vector2d offset_from_parent = frame_rect_in_pixels_.OffsetFromOrigin();
...
void CrossProcessFrameConnector::SetRect(const gfx::Rect& frame_rect) {
  ...
  if (IsUseZoomForDSFEnabled()) {
    frame_rect_in_pixels_ = gfx::ScaleToEnclosingRect(
        frame_rect_in_pixels_, 1.f / screen_info_.device_scale_factor);
  }

  if (view_) {
    view_->SetBounds(frame_rect_in_pixels_);
    ...
    if (old_rect.x() != frame_rect_.x() ||
        old_rect.y() != frame_rect_.y()) {
