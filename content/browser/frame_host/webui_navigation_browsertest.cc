...
IN_PROC_BROWSER_TEST_F(WebUINavigationBrowserTest, WebUIMainFrameToWebAllowed) {
  ...
  EXPECT_NE(webui_rfh, root->current_frame_host());
...
IN_PROC_BROWSER_TEST_F(WebUINavigationBrowserTest,
  ...
  // TODO(nasko): Explicitly grant WebUI bindings to the process to simulate
  // the default behavior for WebUIs.
  ChildProcessSecurityPolicyImpl::GetInstance()->GrantWebUIBindings(
      webui_rfh->GetProcess()->GetID());
  ...
  // Create a subframe with WebUI document in it.
  {
    std::string script = base::StringPrintf(
        "var frame = document.createElement('iframe');\n"
        "frame.src = '%s';\n"
  ...
  EXPECT_NE(webui_rfh, new_shell->web_contents()->GetMainFrame());

  // TODO(nasko): These checks are not what we expect to happen. However, it
  // is due to kChromeUIBlobInternalsHost not being an actual WebUI, so it
  // is not considered needed for cross-process navigation. This needs to be
  // fixed and the checks inverted. See http://crbug.com/776900.
  EXPECT_EQ(webui_site_instance,
            new_shell->web_contents()->GetMainFrame()->GetSiteInstance());
  EXPECT_TRUE(webui_site_instance->IsRelatedSiteInstance(
      new_shell->web_contents()->GetMainFrame()->GetSiteInstance()));
}
