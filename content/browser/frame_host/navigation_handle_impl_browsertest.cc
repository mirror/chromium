...
class TestNavigationThrottle : public NavigationThrottle {
 public:
  TestNavigationThrottle(
      ...
      NavigationThrottle::ThrottleAction will_fail_result,
 ...
 private:
  ...
  NavigationThrottle::ThrottleAction will_fail_result_;
...
class TestDeferringNavigationThrottleInstaller
 ...
 private:
  ...
  NavigationThrottle::ThrottleAction will_fail_deferred_result_;
...
IN_PROC_BROWSER_TEST_F(NavigationHandleImplBrowserTest,
  ...
  // TODO(crbug.com/762333): Enable the expectation below:
...
// Ensure that a NavigationThrottle can block the navigation when a navigation
// fails.
IN_PROC_BROWSER_TEST_F(NavigationHandleImplBrowserTest, ThrottleCancelFailure) {
  ...
  {
      // TODO(lgarron): Implement this once crbug.com/762333 is implemented.
      // Otherwise, NavigateToURL() hangs when we cancel cert error failures.
  }

  // A navigation without a cert error should be successful.
  {
    GURL no_failure_url(embedded_test_server()->GetURL("/title2.html"));
    NavigationHandleObserver observer(shell()->web_contents(), no_failure_url);
    TestNavigationThrottleInstaller installer(
        shell()->web_contents(), NavigationThrottle::PROCEED,
        NavigationThrottle::PROCEED, NavigationThrottle::CANCEL,
        NavigationThrottle::PROCEED);

    EXPECT_TRUE(NavigateToURL(shell(), no_failure_url));
