...
class NavigationHandleImplTest : public RenderViewHostImplTestHarness {
 public:
  ...
  void AddDeletingNavigationThrottle() {
    ...
    std::unique_ptr<TestNavigationThrottle> throttle =
        base::MakeUnique<TestNavigationThrottle>(test_handle());
    throttle->SetMethodWillRespondCallback(base::BindRepeating(
        &NavigationHandleImplTest::ResetNavigationHandleCallback,
        base::Unretained(this)));
 ...
 private:
  ...
  void ResetNavigationHandleCallback(
      const TestNavigationThrottle::Status& status) {
    test_handle_ = nullptr;
  }
