...
// Ensures that deleting a speculative RenderFrameHost trying to commit a
// navigation to the pending NavigationEntry will not crash if it happens
// because a reload DISABLE_PREVIEWS navigation to the same pending
// NavigationEntry started. This is a variant of the earlier
// DeleteSpeculativeRFHPendingCommitOfPendingEntryOnInterrupted1 test with
// a different reload type for the second reload.
IN_PROC_BROWSER_TEST_F(
    RenderFrameHostManagerTest,
    DeleteSpeculativeRFHPendingCommitOfPendingEntryOnInterrupted3) {
  const std::string kOriginalPath = "/original.html";
  const std::string kFirstRedirectPath = "/redirect1.html";
  const std::string kSecondRedirectPath = "/reidrect2.html";
  ControllableHttpResponse original_response1(embedded_test_server(),
  ...
  // The user requests a 'Show original' reload (ReloadType::DISABLE_PREVIEWS)
  // while the previous reload hasn't committed yet. The navigation start
  // deletes the speculative RenderFrameHost that was supposed to commit the
  // browser-initiated navigation. This should not crash.
  TestNavigationManager second_reload(shell()->web_contents(), kOriginalURL);
