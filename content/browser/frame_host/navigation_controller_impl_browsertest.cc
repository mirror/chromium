...
IN_PROC_BROWSER_TEST_F(NavigationControllerBrowserTest,
...

  {
    ...
    EXPECT_EQ(url1, entry1->GetReplacedEntryData()->url);
    EXPECT_TRUE(ui::PageTransitionTypeIncludingQualifiersIs(
        entry1->GetReplacedEntryData()->transition_type,
        ui::PageTransitionFromInt(ui::PAGE_TRANSITION_TYPED |
                                  ui::PAGE_TRANSITION_FROM_ADDRESS_BAR)))
        << base::StringPrintf("%X",
                              entry1->GetReplacedEntryData()->transition_type);
...
IN_PROC_BROWSER_TEST_F(NavigationControllerBrowserTest,
...

  {
    ...
    ASSERT_TRUE(entry1->GetReplacedEntryData().has_value());
    EXPECT_EQ(url1, entry1->GetReplacedEntryData()->url);
    EXPECT_EQ(time1, entry1->GetReplacedEntryData()->timestamp);
    EXPECT_TRUE(ui::PageTransitionTypeIncludingQualifiersIs(
        entry1->GetReplacedEntryData()->transition_type,
        ui::PageTransitionFromInt(ui::PAGE_TRANSITION_TYPED |
                                  ui::PAGE_TRANSITION_FROM_ADDRESS_BAR)));
...
IN_PROC_BROWSER_TEST_F(NavigationControllerBrowserTest,
  ...
  const base::Time time1 = entry1->GetTimestamp();

  {
    ...
    // It is questionable whether a copy of the replaced data here too,
    // because of the modified timestamp as well as the new qualifier,
