...
IN_PROC_BROWSER_TEST_F(NavigationControllerBrowserTest,
                       PruningOfEntriesForDynamicFrames) {
  GURL main_url(embedded_test_server()->GetURL("/title1.html"));
  EXPECT_TRUE(NavigateToURL(shell(), main_url));
  ...
  // Verify that the number of history entries stayed low (i.e. that we do not
  // retain history entries for the 5 frames removed by the test).
