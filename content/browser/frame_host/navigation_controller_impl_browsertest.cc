...
// Delays DidCommitProvisionalLoad for a specified URL, so that it can trigger a
// history-back navigation in the WebContents before proceeding with the commit.
class GoBackAndCommitHelper : public DidCommitProvisionalLoadInterceptor {
 public:
  GoBackAndCommitHelper(const GURL& url, WebContentsImpl* web_contents)
      : DidCommitProvisionalLoadInterceptor(web_contents,
                                            web_contents->GetMainFrame()),
        url_(url),
        web_contents_(web_contents) {}
  ~GoBackAndCommitHelper() override {}
 ...
 private:
  ...
  base::Optional<::FrameHostMsg_DidCommitProvisionalLoad_Params>
      pending_dcpl_params_;

  DISALLOW_COPY_AND_ASSIGN(GoBackAndCommitHelper);
};
...
IN_PROC_BROWSER_TEST_F(
  ...
  // Create a GoBackAndCommitHelper, which will delay the commit for a
  ...
  std::unique_ptr<GoBackAndCommitHelper> helper =
      base::MakeUnique<GoBackAndCommitHelper>(cross_origin_url, web_contents);
