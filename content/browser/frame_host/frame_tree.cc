...
void FrameTree::CreateProxiesForSiteInstance(
  ...
  LOG(ERROR) << "FrameTree::CreateProxiesForSiteInstance";
  LOG(ERROR) << "  site_instance=" << site_instance->GetSiteURL();
    ...
    if (current_instance != site_instance) {
      if (node == source) {
        ...
        if (!current_host->IsRenderFrameLive())
          continue;
        LOG(INFO) << "  Creating new proxy for source.  current_instance="
