  ...
  for (FrameTreeNode* node : NodesExceptSubtree(source)) {
    ...
    if (current_instance != site_instance) {
      if (node == source) {
        ...
        if (!current_host->IsRenderFrameLive())
          continue;

        // There's also no need to create proxies if |current_instance| is not
        // in the same BrowsingInstance as |site_instance|, as they won't be
        // able to script each other.
        //
        // TODO(alexmos): the different BrowsingInstance path in
        // CreateProxiesForNewRenderFrameHost should not be necessary, since
        // frames in different BrowsingInstances shouldn't be supported within
        // the same FrameTree. Once that path is removed, this check should be
        // removed as well.
        if (!current_instance->IsRelatedSiteInstance(site_instance))
          continue;
      }
