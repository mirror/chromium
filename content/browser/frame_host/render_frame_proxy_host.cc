...
bool RenderFrameProxyHost::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderFrameProxyHost, msg)
    ...
    IPC_MESSAGE_HANDLER(FrameHostMsg_PrintOopSubframe, OnPrintOopSubframe)
...
void RenderFrameProxyHost::OnPrintOopSubframe(const gfx::Rect& rect,
                                              uint32_t content_id) {
  ...
  src_rfh->delegate()->PrintOopSubframe(src_rfh->GetRoutingID(), rect,
                                        content_id, target_rfh);
}
