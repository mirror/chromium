...
bool RenderFrameProxyHost::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderFrameProxyHost, msg)
    ...
    IPC_MESSAGE_HANDLER(FrameHostMsg_PrintSubframe, OnPrintSubframe)
...
void RenderFrameProxyHost::OnPrintSubframe(const gfx::Rect& rect,
                                           int document_cookie,
                                           int page_number) {
  RenderFrameHostImpl* target_rfh = frame_tree_node_->current_frame_host();

  RenderFrameHostImpl* src_rfh =
      frame_tree_node_->parent()->current_frame_host();
  DCHECK(src_rfh->GetSiteInstance() != target_rfh->GetSiteInstance());
  src_rfh->delegate()->PrintSubframe(rect, document_cookie, page_number,
                                     target_rfh);
}
