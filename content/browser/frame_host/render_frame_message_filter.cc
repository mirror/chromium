      ...
      net::DefineNetworkTrafficAnnotation("renderer_initiated_download", R"(
        semantics {
          ...
          description:
            ...
            "a download via one of the renderer's classification mechanisms or "
            "WebView requested an <canvas> or <img> at a specific location be "
            "downloaded."
          trigger:
            "The user navigated to a destination that was categorized as a "
            "download or WebView triggered saving a <canvas> or <img> tag."
          data: "The URL we are attempting to download."
          destination: WEBSITE
        }
        policy {
