...
class RenderFrameHostInterfaceProviderTest
 ...
 protected:
  BrowserContext* CreateBrowserContext() override {
    // The ServiceManagerContext needs to be created after ThreadBundle is set
    // up, but before the BrowserContext is created.
    service_manager_context_ = base::MakeUnique<TestServiceManagerContext>();
    return RenderViewHostTestHarness::CreateBrowserContext();
  }
