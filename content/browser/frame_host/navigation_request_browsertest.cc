...
class NavigationRequestBrowserTest : public ContentBrowserTest {
 public:
  NavigationRequestBrowserTest() : ContentBrowserTest() {}

  void SetUpOnMainThread() override {
    base::FilePath content_test_data(
        FILE_PATH_LITERAL("content/test/data/navigation_request"));
 ...
 protected:
  ...
  DISALLOW_COPY_AND_ASSIGN(NavigationRequestBrowserTest);
};

class NavigationRequestCSPReportingBrowserTest
    : public NavigationRequestBrowserTest {
 public:
  NavigationRequestCSPReportingBrowserTest() : NavigationRequestBrowserTest() {}
  ...
  void SetUpOnMainThread() override {
    ...
    test_server->RegisterRequestMonitor(
        base::Bind(&NavigationRequestCSPReportingBrowserTest::ReportMonitor,
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(NavigationRequestCSPReportingBrowserTest);
};

IN_PROC_BROWSER_TEST_F(NavigationRequestBrowserTest, FormCSPAllow) {
...
IN_PROC_BROWSER_TEST_F(NavigationRequestBrowserTest, FormCSPDeny) {
  // The NavigationRequest isn't used without PlzNavigate.
  if (!IsBrowserSideNavigationEnabled())
    return;
...
IN_PROC_BROWSER_TEST_F(NavigationRequestBrowserTest, FormCSPDenyPath) {
  // The NavigationRequest isn't used without PlzNavigate.
  if (!IsBrowserSideNavigationEnabled())
    return;
...
IN_PROC_BROWSER_TEST_F(NavigationRequestBrowserTest, FormCSPUpgradeRequest) {
  // The NavigationRequest isn't used without PlzNavigate.
  if (!IsBrowserSideNavigationEnabled())
    return;
...
IN_PROC_BROWSER_TEST_F(NavigationRequestCSPReportingBrowserTest,
                       FormCSPReport) {
  // The NavigationRequest isn't used without PlzNavigate.
  if (!IsBrowserSideNavigationEnabled())
    return;
