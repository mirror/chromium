...
bool FrameTreeNode::CommitPendingFramePolicy() {
  ...
  if (did_change_container_policy)
    replication_state_.frame_policy.container_policy =
        pending_frame_policy_.container_policy;
  replication_state_.total_combined_sandbox_flags =
      pending_frame_policy_.sandbox_flags;
...
void FrameTreeNode::SetTotalCombinedSandboxFlags(
    blink::WebSandboxFlags sandbox_flags) {
  if (parent())
    sandbox_flags |= parent()->total_combined_sandbox_flags();
  replication_state_.total_combined_sandbox_flags = sandbox_flags;
}
