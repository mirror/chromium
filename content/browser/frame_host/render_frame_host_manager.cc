...
RenderFrameHostManager::DetermineSiteInstanceForURL(
  ...
  // Keep subframes that are same-site with the parent in the parent's
  // SiteInstance.
  //
  // TODO(alexmos,creis): This is a temporary workaround to keep same-site
  // subframes inside a hosted app in the app (parent frame's) process.
  // Typically this case would be handled by later finding the existing
  // matching SiteInstance from the parent in the current BrowsingInstance;
  // however, that lookup needs to be refactored to not consider effective
  // URLs.  Once that's done, we can remove this workaround.  Note that for
  // now, this is *only* a heuristic and may not work with more complicated
  // FrameTrees.
  if (!frame_tree_node_->IsMainFrame() &&
      IsCurrentlySameSite(frame_tree_node_->parent()->current_frame_host(),
                          dest_url)) {
...
bool RenderFrameHostManager::IsCurrentlySameSite(RenderFrameHostImpl* candidate,
  ...
  // TODO(creis): This should eventually call out to embedder to help decide, if
  // we can find a way to make decisions about popups based on their opener.
