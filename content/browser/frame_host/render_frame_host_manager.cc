...
RenderFrameHostManager::CreateRenderFrameHost(
  ...
  auto render_frame_host = RenderFrameHostFactory::Create(
  ...
  if (interfaces_request.is_pending())
    render_frame_host->BindInterfacesRequest(std::move(interfaces_request));
...
std::unique_ptr<RenderFrameHostImpl> RenderFrameHostManager::CreateRenderFrame(
  ...
  new_render_frame_host = CreateRenderFrameHost(
      ...
      nullptr /* interfaces_request */, widget_routing_id, hidden, false);
