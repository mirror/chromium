...
void RenderFrameHostImpl::DidCommitProvisionalLoad(
  ...
  if (validated_params->was_within_same_document) {
    // The security origin never changes for same-document navigations, keep
    // using the existing InterfaceProvider interface connection.
    if (interface_provider_request.is_pending()) {
      bad_message::ReceivedBadMessage(
          process, bad_message::RFH_INTERFACE_PROVIDER_SUPERFLUOUS);
    }
  } else {
    // Otherwise, close the old binding to ensure that any pending interface
    // requests originating from the previous document, hence possibly from a
    // different security origin, will no longer dispatched.
    document_scoped_interface_provider_binding_.Close();

    // Expect the RenderFrame to have supplied the request end of a new
    // InterfaceProvider connection that will be used by the newly active
    // document at issue interface requests to access RenderFrameHost services.
    if (interface_provider_request.is_pending()) {
