...
void RenderFrameHostImpl::OnRenderProcessGone(int status, int exit_code) {
  ...
  for (base::IDMap<std::unique_ptr<ExtractSmartClipDataCallback>>::iterator
    ...
    auto request_id = iter.GetCurrentKey();
    ExtractSmartClipDataCallback callback = *iter.GetCurrentValue();
    std::move(callback).Run(base::string16(), base::string16());
    smart_clip_callbacks_.Remove(request_id);
  }
#endif  // defined(OS_ANDROID)
...
void RenderFrameHostImpl::RequestSmartClipExtract(
    const ExtractSmartClipDataCallback& callback,
    gfx::Rect rect) {
  int32_t request_id = smart_clip_callbacks_.Add(
      std::make_unique<ExtractSmartClipDataCallback>(callback));

  frame_->ExtractSmartClipData(
      request_id, rect,
      base::BindOnce(&RenderFrameHostImpl::OnSmartClipDataExtracted,
                     base::Unretained(this), request_id));
}

void RenderFrameHostImpl::OnSmartClipDataExtracted(int32_t id,
                                                   const base::string16& text,
                                                   const base::string16& html) {
  auto& callback = *smart_clip_callbacks_.Lookup(id);
  std::move(callback).Run(text, html);
  smart_clip_callbacks_.Remove(id);
}
#endif  // defined(OS_ANDROID)
