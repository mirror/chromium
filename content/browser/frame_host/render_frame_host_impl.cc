...
void RenderFrameHostImpl::OnSameDocumentCommitProcessed(
  ...
  // If the NavigationHandle was deleted, another navigation commit started to
  // be processed. Let the latest commit go through and stop doing anything.
  if (!navigation_handle_ ||
      navigation_handle_->GetNavigationId() != navigation_id) {
    return;
  }
  if (result == blink::mojom::CommitResult::RestartCrossDocument) {
    ...
    // TODO(clamy): When we extend the lifetime of NavigationRequest, we
    // should use it to restart the navigation as is instead of going through
    // OpenURL.
    frame_tree_node_->navigator()->RequestOpenURL(
        this, navigation_handle_->GetURL(), false /* uses_post */,
        nullptr /* resource_request_body */, std::string() /* extra_headers */,
        navigation_handle_->GetReferrer(), WindowOpenDisposition::CURRENT_TAB,
        should_replace_current_entry, navigation_handle_->HasUserGesture(),
        blink::WebTriggeringEventInfo::kUnknown, base::nullopt);
  }

  if (result == blink::mojom::CommitResult::RestartCrossDocument ||
      result == blink::mojom::CommitResult::Aborted) {
