...
void RenderFrameHostImpl::OnSuddenTerminationDisablerChanged(
  ...
  LOG(ERROR) << "OnSuddenTerminationDisablerChanged: " << present << ", "
             << static_cast<int>(disabler_type);
  DCHECK_NE(GetSuddenTerminationDisablerState(disabler_type), present);
  if (present) {
    sudden_termination_disabler_types_enabled_ |=
        static_cast<blink::WebSuddenTerminationDisablerType>(disabler_type);
  } else {
    sudden_termination_disabler_types_enabled_ &=
        ~static_cast<blink::WebSuddenTerminationDisablerType>(disabler_type);
  }
...
bool RenderFrameHostImpl::GetSuddenTerminationDisablerState(
    blink::WebSuddenTerminationDisabler disabler_type) {
  return (sudden_termination_disabler_types_enabled_ &
          static_cast<blink::WebSuddenTerminationDisablerType>(
              disabler_type)) != 0;
}
