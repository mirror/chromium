...
void RenderFrameHostImpl::CreateAudioInputStreamFactory(
  ...
  AudioInputDeviceManager* aidm =
      bml->media_stream_manager()->audio_input_device_manager();
  DCHECK(bml);
  audio_input_stream_factory_ =
      RenderFrameAudioInputStreamFactoryHandle::CreateFactory(
          base::BindRepeating(
              ...
              base::Unretained(aidm), GetProcess()->GetID(), GetRoutingID()),
          aidm, std::move(request));
}
