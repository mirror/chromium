...
base::LazyInstance<RenderFrameHostImpl::CreateFactoryCallback>::Leaky
    g_url_loader_factory_callback = LAZY_INSTANCE_INITIALIZER;
...
void RenderFrameHostImpl::SetNetworkFactoryForTesting(
    const CreateFactoryCallback& url_loader_factory_callback) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  DCHECK(url_loader_factory_callback.is_null() ||
         g_url_loader_factory_callback.Get().is_null());
  g_url_loader_factory_callback.Get() = url_loader_factory_callback;
}
...
void RenderFrameHostImpl::CommitNavigation(
  ...
  if (base::FeatureList::IsEnabled(features::kNetworkService) &&
    ...
    if (!default_factory.is_bound()) {
      ...
      if (g_url_loader_factory_callback.Get().is_null()) {
        ...
        g_url_loader_factory_callback.Get().Run(
            mojo::MakeRequest(&default_factory), GetProcess()->GetID(),
