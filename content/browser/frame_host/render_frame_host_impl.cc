...
RenderFrameHostImpl::RenderFrameHostImpl(SiteInstance* site_instance,
    ...
    : render_view_host_(render_view_host),
      ...
      frame_host_forwarder_(this),
      frame_host_associated_binding_(&frame_host_forwarder_),
...
void RenderFrameHostImpl::override_mojo_frame_host_receiver_for_testing(
    mojom::FrameHost* receiver) {
  frame_host_forwarder_.set_receiver(receiver ? receiver : this);
}
...
void RenderFrameHostImpl::DidCommitProvisionalLoad(
    const FrameHostMsg_DidCommitProvisionalLoad_Params& params) {
  // TODO(engedy): If we made the corresponding typemap moveonly, there would
  // still be a call to a (hopefully) move ctor, and a copy on the sending side.
  // TODO(engedy): Figure out a way to kill renderer if message fails
  // validation.
  ...
  NotifyResourceSchedulerOfNavigation(process->GetID(), params);
...
void RenderFrameHostImpl::BindInterfaceProviderForNewDocument(
  ...
  // dispatched interface requests pertain strictly to the frame's current
  // document.
