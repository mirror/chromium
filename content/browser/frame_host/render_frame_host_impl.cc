...
void RenderFrameHostImpl::DidCommitProvisionalLoad(
  ...
  if (!ValidateDidCommitParams(validated_params.get()))
    return;

  if (!navigation_handle_) {
    // TODO(ahemery): This should never be true for cross-document navigation
    // apart from race conditions. Remove when the full mojo interface is in
    // place. (https://bugs.chromium.org/p/chromium/issues/detail?id=784904)
    if (!is_loading()) {
  ...
  frame_tree_node()->navigator()->DidNavigate(
      this, *validated_params, std::move(navigation_handle),
...
void RenderFrameHostImpl::DidCommitSameDocumentNavigation(
  ...
  // Since we are doing same site navigation, the last commited and ongoing
  // commit sites should match.
  GURL site_url =
      validated_params->url.is_empty()
          ? GURL()
          : SiteInstance::GetSiteForURL(frame_tree_node_->navigator()
                                            ->GetController()
                                            ->GetBrowserContext(),
                                        validated_params->url);
  DCHECK(last_committed_site_url_ == site_url);
