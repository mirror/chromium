...
NavigationHandleImpl* RenderFrameHostImpl::GetNavigationHandle() {
  return GetNavigationRequest() ? GetNavigationRequest()->navigation_handle()
                                : nullptr;
}
...
void RenderFrameHostImpl::OnDidStopLoading() {
  ...
  is_loading_ = false;
  // TODO(ahemery): Check the reset here
...
RenderFrameHostImpl::TakeNavigationHandleForCommit(
  ...
  if (params.was_within_same_document) {
    ...
    if (is_browser_initiated && navigation_handle &&
      ...
      return GetNavigationRequest()->GetNavigationHandleOwnership();
    }
  ...
  if (navigation_handle) {
    ...
    // TODO(ahemery): Check reset correctness
    frame_tree_node_->ResetNavigationRequest(false, false);
  }
