...
bool NavigatorImpl::Navigate(
  ...
  FrameTreeNode* frame_tree_nod = request->frame_tree_node();
  ...
  navigation_data_.reset(
      new NavigationMetricsData(navigation_start, dest_url, restore_type));

  int nav_entry_id = navigation_request->nav_entry_id();
  bool is_pending_entry =
      controller_->GetPendingEntry() &&
      (nav_entry_id == controller_->GetPendingEntry()->GetID());
  ...
  if (frame_tree_node->navigation_request()) {
    ...
    // called instead.
    if (!FrameMsg_Navigate_Type::IsSameDocument(navigation_type) &&
      ...
      // BEYOND THIS POINT the NavigationRequest might have been destroyed in
      // BeginNavigation(). Don't use
      // |frame_tree_node->navigation_request()->BeginNavigation()| without
      // checking that it's alive first.
  ...
  // Make sure no code called clears the pending entry.
  if (is_pending_entry) {
    CHECK(controller_->GetPendingEntry() &&
          controller_->GetPendingEntry()->GetID() == nav_entry_id);
  }
...
bool NavigatorImpl::HandleRendererDebugURL() {
  ...
  if (dest_url.SchemeIs(url::kJavaScriptScheme)) {
    ...
    render_frame_host->CommitNavigation(
        ...
        nullptr,  // body
        request->common_params(), request->request_params(),
        request->is_view_source(), base::nullopt,
        request->devtools_navigation_token());
...
bool NavigatorImpl::NavigateNewChildFrame(
  ...
  return controller_->NavigateNewChildFrame(render_frame_host,default_url);
}
