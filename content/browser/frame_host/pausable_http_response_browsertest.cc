...
std::unique_ptr<net::test_server::HttpResponse> ExampleRequestHandler(
    PausableHttpResponseController* controller_1,
    PausableHttpResponseController* controller_2,
    const net::test_server::HttpRequest& request) {
  if (request.relative_url == "/page_1") {
    auto response = std::make_unique<PausableHttpResponse>(controller_1);
    response->set_code(net::HTTP_OK);
    response->set_content("<html><head><title>page 1</title></head></html>");
    response->set_content_type("text/html");
    return std::move(response);
  }

  if (request.relative_url == "/page_2") {
    auto response = std::make_unique<PausableHttpResponse>(controller_2);
    response->set_code(net::HTTP_OK);
    response->set_content("<html><head><title>page 2</title></head></html>");
    response->set_content_type("text/html");
    return std::move(response);
  }
...
IN_PROC_BROWSER_TEST_F(ContentBrowserTest, example_use) {
  ...
  embedded_test_server()->RegisterRequestHandler(
      base::Bind(&ExampleRequestHandler, &controller_1, &controller_2));
  ...
  // FIXME(arthursonzogni): This test can still be potential flaky if
  ...
  // The responses's body of the second navigation is received, but it doesn't
  // matter now.
  controller_2.Resume();
}
