...
RenderFrameHostImpl* NavigationHandleImpl::GetRenderFrameHost() {
  // TODO(mkwst): Change the first part of of the check to compare against
  // 'READY_TO_COMMIT' once ReadyToCommitNavigation is available whether or not
  // PlzNavigate is enabled. https://crbug.com/621856
  DCHECK(state_ >= WILL_PROCESS_RESPONSE || state_ == WILL_FAIL_REQUEST ||
      ...
      << "This accessor should only be called after a response has been "
         "delivered for processing, or if the request is failing."
      << state_;
...
void NavigationHandleImpl::WillFailRequest(
  ...
  net_error_code_ = result.net_error_code();
  RunCompleteCallback(result);
}
