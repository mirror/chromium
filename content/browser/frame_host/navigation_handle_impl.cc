...
NavigationHandleImpl::CallWillFailRequestForTesting(
  ...
  NavigationThrottle::ThrottleCheckResult result = NavigationThrottle::DEFER;
  WillFailRequest(net_error, ssl_info, should_ssl_errors_be_fatal,
                  base::Bind(&UpdateThrottleCheckResult, &result));
...
NavigationHandleImpl::CheckWillFailRequest() {
  ...
  for (size_t i = next_index_; i < throttles_.size(); ++i) {
    ...
    switch (result) {
      ...
      // TODO: BLOCK_REQUEST_AND_COLLAPSE and BLOCK_REQUEST cases are copied
      // from CheckWillRedirectRequest.
      case NavigationThrottle::BLOCK_REQUEST_AND_COLLAPSE:
        frame_tree_node_->SetCollapsed(true);  // Fall through.
      case NavigationThrottle::BLOCK_REQUEST:
        CHECK(IsBrowserSideNavigationEnabled())
      ...
      case NavigationThrottle::CANCEL:
      case NavigationThrottle::CANCEL_AND_IGNORE:
        LOG(ERROR) << "CheckWillFailRequest setting state_ = CANCELING";
        state_ = CANCELING;
        return result;
...
void NavigationHandleImpl::CancelDeferredNavigationInternal(
  ...
  DCHECK(result != NavigationThrottle::BLOCK_REQUEST_AND_COLLAPSE ||
         ...
         (state_ == DEFERRING_FAILURE && IsBrowserSideNavigationEnabled()));
