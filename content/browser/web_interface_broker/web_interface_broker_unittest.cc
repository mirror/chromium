...
class AssociatedFrameInterface {
 public:
  static constexpr char Name_[] = "associated frame interface";
};
constexpr char AssociatedFrameInterface::Name_[];
...
class WebInterfaceBrokerTest : public RenderViewHostTestHarness {
 public:
  ...
  void SetUp() override {
    ...
    auto builder = WebInterfaceBrokerBuilder::Create(
        ...
        base::BindRepeating(&WebInterfaceBrokerTest::OnUnknownFrameInterface,
                            base::Unretained(this)),
        base::BindRepeating(
            &WebInterfaceBrokerTest::OnUnknownAssociatedFrameInterface,
            base::Unretained(this)));
    ...
    builder->AddInterface(
        std::make_unique<TestWebInterfaceFilter>(this),
        base::BindRepeating(
            &WebInterfaceBrokerTest::BindFrameOnlyAssociatedInterface,
            base::Unretained(this)));
    builder->AddInterface(
        std::make_unique<TestWebInterfaceFilter>(this),
  ...
  InterfaceRoute BindAssociatedFrameInterface(
      const std::string& interface_name) {
    mojo::AssociatedInterfacePtrInfo<AssociatedFrameInterface> ptr_info;

    broker_->BindAssociatedInterfaceForFrame(
        interface_name, mojo::MakeRequest(&ptr_info).PassHandle(), main_rfh(),
        base::BindOnce(&WebInterfaceBrokerTest::ReportBadMessage,
                       base::Unretained(this)));
    return route_;
  }
  ...
  void ClearBroker() {
    broker_ = WebInterfaceBrokerBuilder::Create({}, {}, {})->Build();
  }
 ...
 private:
  ...
  void OnUnknownAssociatedFrameInterface(
      const std::string& interface_name,
    ...
    EXPECT_EQ(main_rfh(), frame);
    EXPECT_EQ("fake interface", interface_name);
    SetInterfaceRoute(InterfaceRoute::kAssociatedFrameDelegate);
  }
  ...
  void BindFrameOnlyAssociatedInterface(
      mojo::AssociatedInterfaceRequest<AssociatedFrameInterface>,
      RenderFrameHost* frame) {
    SetInterfaceRoute(InterfaceRoute::kAssociatedFrame);
    EXPECT_EQ(main_rfh(), frame);
  }
  ...
  WebInterfaceFilter::Result FilterInterface(WebContextType context_type,
                                             ...
                                             RenderFrameHost* frame,
                                             const url::Origin& origin) {
    if (context_type == WebContextType::kMainFrame)
      EXPECT_EQ(main_rfh(), frame);
    EXPECT_EQ(process(), process_host);
    EXPECT_EQ(main_rfh()->GetLastCommittedOrigin(), origin);
...
TEST_F(WebInterfaceBrokerTest, Worker_AssociatedInterface) {
  EXPECT_EQ(InterfaceRoute::kBadMessage,
            BindWorkerInterface(AssociatedFrameInterface::Name_));
  EXPECT_EQ(bad_message(),
            "Bad request for interface associated frame interface");
}
...
TEST_F(WebInterfaceBrokerTest, Frame_UnknownAssociatedInterface) {
  EXPECT_EQ(InterfaceRoute::kAssociatedFrameDelegate,
            BindAssociatedFrameInterface("fake interface"));
}
...
TEST_F(WebInterfaceBrokerTest, Frame_AssociatedInterface_RequestNotAssociated) {
  EXPECT_EQ(InterfaceRoute::kBadMessage,
            BindFrameInterface(AssociatedFrameInterface::Name_));
  EXPECT_EQ(bad_message(),
            "Bad request for interface associated frame interface");
}

TEST_F(WebInterfaceBrokerTest, Frame_FrameInterface_RequestAssociated) {
  EXPECT_EQ(InterfaceRoute::kBadMessage,
            BindAssociatedFrameInterface(FrameInterface::Name_));
  EXPECT_EQ(bad_message(), "Bad request for interface frame interface");
}

TEST_F(WebInterfaceBrokerTest, Frame_AssociatedFrameInterface) {
  EXPECT_EQ(InterfaceRoute::kAssociatedFrame,
            BindAssociatedFrameInterface(AssociatedFrameInterface::Name_));
}

TEST_F(WebInterfaceBrokerTest, Frame_AssociatedFrameInterface_Deny) {
  SetFilterResult(WebInterfaceFilter::Result::kDeny);
  EXPECT_EQ(InterfaceRoute::kNone,
            BindAssociatedFrameInterface(AssociatedFrameInterface::Name_));
}

TEST_F(WebInterfaceBrokerTest, Frame_AssociatedFrameInterface_BadMessage) {
  SetFilterResult(WebInterfaceFilter::Result::kBadMessage);
  EXPECT_EQ(InterfaceRoute::kBadMessage,
            BindAssociatedFrameInterface(AssociatedFrameInterface::Name_));
  EXPECT_EQ(bad_message(),
...
// Sanity check that providing no delegate is valid.
TEST_F(WebInterfaceBrokerTest, UnknownInterfacesWithoutDelegates) {
  ClearBroker();
  EXPECT_EQ(InterfaceRoute::kNone, BindWorkerInterface("fake interface"));
  EXPECT_EQ(InterfaceRoute::kNone, BindFrameInterface("fake interface"));
  EXPECT_EQ(InterfaceRoute::kNone,
            BindAssociatedFrameInterface("fake interface"));
}
