...
struct StoragePartitionImpl::DataDeletionHelper {
  ...
  DataDeletionHelper(uint32_t remove_mask,
                     uint32_t quota_storage_remove_mask,
                     base::OnceClosure callback,
                     base::OnceClosure storagePartitionCallback)
      : remove_mask(remove_mask),
        quota_storage_remove_mask(quota_storage_remove_mask),
        callback(std::move(callback)),
        storagePartitionCallback(std::move(storagePartitionCallback)),
        task_count(0) {}
...
void StoragePartitionImpl::DeletionHelperDone() {
  deletion_helpers_running_--;
}
...
void StoragePartitionImpl::DataDeletionHelper::DecrementTaskCount() {
  ...
  if (!task_count) {
    std::move(callback).Run();
    std::move(storagePartitionCallback).Run();
    delete this;
  }
}
...
int StoragePartitionImpl::GetDeletionTaskCountForTesting() {
  return deletion_helpers_running_;
};
