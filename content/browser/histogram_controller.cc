...
content::mojom::ChildHistogram& HistogramController::GetChildHistogramInterface(
  ...
  child_histogram.set_connection_error_handler(base::Bind(
      &HistogramController::RemoveChildHistogramInterface<T>,
      base::Unretained(this), base::Unretained(child_histogram_map), host));
  auto result = child_histogram_map->insert(
      std::make_pair(host, std::move(child_histogram)));
