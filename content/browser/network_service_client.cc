...
// This class is created on UI thread, and deleted by DeleteThis() after the
// |callback_| runs. The |callback_| needs to run on UI thread since it is
// from URLLoader.
//
// The |ssl_client_auth_handler_| needs to be created on IO thread, and deleted
// on the same thread, so DeleteThis() needs to run on IO thread.
//
 ...
 public:
  ...
  void RunCallback(scoped_refptr<net::X509Certificate> cert,
    ...
    BrowserThread::PostTask(BrowserThread::IO, FROM_HERE,
                            base::BindOnce(&SSLClientAuthDelegate::DeleteThis,
