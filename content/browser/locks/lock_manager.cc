...
class LockManager::Lock {
 ...
 private:
  ...
  // Exactly one of these is non-null at any given time.
...
class LockManager::OriginState {
 public:
  ...
  Lock* AddRequest(int64_t lock_id,
...
void LockManager::RequestLock(const std::string& name,
  ...
  Lock* lock = origins_[context.origin].AddRequest(
...
int64_t LockManager::NextLockId() {
  ...
  DCHECK_GE(lock_id, kPreemptiveLockId);
  return lock_id;
}
