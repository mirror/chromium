...
void PrintPid(int32_t pid) {
  LOG(ERROR) << " Pid from network_service_test->SimulateCrash():" << pid;
}
...
class NetworkServiceRestartBrowserTest : public ContentBrowserTest {
 public:
  ...
  void SimulateNetworkServiceCrash() {
    LOG(ERROR) << "Begin NetworkServiceRestartBrowserTest.SimulateNetworkServiceCrash";
    ...
    LOG(ERROR) << "1. NetworkServiceRestartBrowserTest.SimulateNetworkServiceCrash";
    network_service_test->SimulateCrash(base::BindOnce(PrintPid));
    LOG(ERROR) << "2. NetworkServiceRestartBrowserTest.SimulateNetworkServiceCrash";
    run_loop.Run();
    // network_service_test.FlushForTesting();
    LOG(ERROR) << "Finished NetworkServiceRestartBrowserTest.SimulateNetworkServiceCrash";
  }
