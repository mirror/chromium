...
void SandboxHostLinux::Init() {
  ...
  sandboxed_socket_ = fds[0];
  // The SandboxIPC client is not expected to read from |sandboxed_socket_|.
  // Instead, it reads from a temporary socket sent with the request.
  PCHECK(0 == shutdown(sandboxed_socket_, SHUT_RD)) << "shutdown";
...
SandboxHostLinux::~SandboxHostLinux() {
  if (initialized_) {
    ...
    if (IGNORE_EINTR(close(sandboxed_socket_)) < 0)
      PLOG(ERROR) << "close";
