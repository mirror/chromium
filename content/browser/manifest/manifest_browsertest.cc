...
IN_PROC_BROWSER_TEST_F(ManifestBrowserTest, 404Manifest) {
  ...
  EXPECT_FALSE(manifest_url().is_empty());
  EXPECT_EQ(0u, console_error_count());
  EXPECT_EQ((std::vector<GURL>{manifest_url()}), reported_manifest_urls());
}
...
IN_PROC_BROWSER_TEST_F(ManifestBrowserTest, DynamicManifest) {
  ...
  GURL first_manifest_url = manifest_url();

  {
    ...
    EXPECT_TRUE(manifest().IsEmpty());
    EXPECT_FALSE(manifest_url().is_empty());
    EXPECT_EQ((std::vector<GURL>{first_manifest_url, manifest_url()}),
              reported_manifest_urls());
  }
  GURL second_manifest_url = manifest_url();

  {
    ...
    EXPECT_TRUE(manifest_url().is_empty());
    EXPECT_EQ(
        (std::vector<GURL>{first_manifest_url, second_manifest_url, GURL()}),
        reported_manifest_urls());
  }
...
IN_PROC_BROWSER_TEST_F(ManifestBrowserTest, CORSManifest) {
  ...
  EXPECT_EQ(0u, console_error_count());
  EXPECT_EQ((std::vector<GURL>{manifest_url()}), reported_manifest_urls());
  GURL old_manifest_url = manifest_url();
  ...
  ASSERT_TRUE(content::ExecuteScript(shell(),
                                     "setManifestTo('" + manifest_link + "')"));
  GetManifestAndWait();
  EXPECT_EQ((std::vector<GURL>{old_manifest_url, manifest_url()}),
            reported_manifest_urls());
}
...
IN_PROC_BROWSER_TEST_F(ManifestBrowserTest, Navigation) {
...

  {
    ...
    EXPECT_EQ(0u, console_error_count());
    EXPECT_TRUE(manifest_url().is_empty());
    EXPECT_EQ((std::vector<GURL>{old_manifest_url}), reported_manifest_urls());
  }
}
