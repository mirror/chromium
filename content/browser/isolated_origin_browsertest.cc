...
IN_PROC_BROWSER_TEST_F(IsolatedOriginTest,
  ...
  // When not in --site-per-process mode, even though the popup is still
  // cross-site from the opener, it should've gone back to the opener's
  // SiteInstance, since at this point, neither popup nor opener are at a site
  ...
  // The popup should be in the same SiteInstance as its same-site opener.
  EXPECT_EQ(new_shell->web_contents()->GetMainFrame()->GetSiteInstance(),
            root->current_frame_host()->GetSiteInstance());
