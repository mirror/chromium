...
IN_PROC_BROWSER_TEST_F(
  ...
  ASSERT_TRUE(policy->IsIsolatedOrigin(url::Origin(isolated_url)));
  EXPECT_TRUE(NavigateToURL(shell(), isolated_url));
...
IN_PROC_BROWSER_TEST_F(
  ...
  GURL isolated_url(embedded_test_server()->GetURL("isolated.foo.com", "/"));
  url::Origin isolated_origin(isolated_url);
  ASSERT_TRUE(policy->IsIsolatedOrigin(url::Origin(isolated_origin)));
  RenderProcessHostImpl::SetStoragePartitionServiceFactoryForTesting(
...
// Verify that a web renderer process cannot read localStorage of an
// extension origin.
IN_PROC_BROWSER_TEST_F(IsolatedOriginTest,
                       LocalStorageOriginEnforcement_WebAccessingExtension) {
  url::Origin extension_origin(
      GURL("chrome-extension://aihpiglmnhnhijdnjghpfnlledckkhja"));
  RenderProcessHostImpl::SetStoragePartitionServiceFactoryForTesting(
      base::Bind(&CreateTestStoragePartitionService, extension_origin));

  GURL web_url(embedded_test_server()->GetURL("blah.bar.com", "/title1.html"));
