...
class CastTracingAgent : public tracing::mojom::Agent {
 ...
 private:
  ...
  void StartTracingOnIO();
  void FinishStartOnIO(chromecast::SystemTracer::Status status);
  void FinishStart(chromecast::SystemTracer::Status status);
  void StopAndFlushOnIO();
  void HandleTraceDataOnIO(chromecast::SystemTracer::Status,
  ...
  std::unique_ptr<chromecast::SystemTracer> cast_system_trace_;
