...
class PerfettoTest : public testing::Test {
 public:
  ...
  void OnPagesComplete(const std::vector<uint32_t>& changed_pages) {
    producer_->OnPagesComplete(changed_pages);
  }
...
TEST_F(PerfettoTest, Testing) {
  ...
  perfetto::SharedMemory* shared_memory =
      producer_service_->producer_endpoint_->shared_memory();
  auto shared_memory_arbiter = perfetto::SharedMemory::Arbiter::CreateInstance(
      shared_memory, 4096 /* TODO where does this come from? */,
      on_pages_complete, &perfetto_task_runner_);

  std::unique_ptr<perfetto::TraceWriter> writer =
      shared_memory_arbiter->CreateTraceWriter(0 /* target_buffer */);
  ASSERT_TRUE(writer);
