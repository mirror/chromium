...
class ProducerBase : public perfetto::mojom::Producer,
 ...
 public:
  ...
  void Initialize(perfetto::Service* service) {
    producer_endpoint_ = service->ConnectProducer(this);
  }

  void AddProducerClient(perfetto::mojom::ProducerClientPtr producer_client) {
    perfetto::mojom::ProducerPtr producer;
    bindings_.AddBinding(this, mojo::MakeRequest(&producer));
    producer_client->ConnectProducer(std::move(producer));

    producer_clients_.AddPtr(std::move(producer_client));
  }
  ...
  void CreateDataSourceInstance(
    ...
    producer_clients_.ForAllPtrs(
        [shared_memory](perfetto::mojom::ProducerClient* producer_client) {
          mojo::ScopedSharedBufferHandle shm = shared_memory->Clone();
          DCHECK(shm.is_valid());
          producer_client->EnableDataSource(std::move(shm),
                                            shared_memory->size());
  ...
  void TearDownDataSourceInstance(perfetto::DataSourceInstanceID) override {
    producer_clients_.ForAllPtrs(
        [](perfetto::mojom::ProducerClient* producer_client) {
          producer_client->DisableDataSource();
        });
  }
 ...
 private:
  mojo::InterfacePtrSet<perfetto::mojom::ProducerClient> producer_clients_;
  mojo::BindingSet<perfetto::mojom::Producer> bindings_;
};
...
class PerfettoService : public perfetto::mojom::ProducerService {
 public:
  ...
  void ConnectToChromeTracingProducer(
    ...
    chrome_tracing_producer_->AddProducerClient(std::move(producer_client));
  }
  ...
  std::unique_ptr<ChromeTracingProducer> chrome_tracing_producer_;
};
