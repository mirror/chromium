...
TEST_F(PerfettoTest, Testing) {
  ...
  perfetto::SharedMemory* shared_memory = producer_service_->producer_endpoint_->shared_memory();
  ...
  std::unique_ptr<perfetto::TraceWriter> writer =
      shared_memory_arbiter->CreateTraceWriter(1 /* target_buffer */);
  ASSERT_TRUE(writer);
