...
TracingControllerImpl::TracingControllerImpl()
  ...
  if (!BrowserThread::CurrentlyOn(BrowserThread::UI)) {
    BrowserThread::PostTask(
        ...
        base::Bind(&TracingControllerImpl::AddAgents, base::Unretained(this)));
  } else {
    AddAgents();
  }
}
...
// Register tracing agents.
  ...
  auto chrome_agent = base::MakeUnique<tracing::ChromeTraceEventAgent>(
      connector, "content_browser");
...
void TracingControllerImpl::OnMetadataAvailable(
  ...
  if (trace_data_endpoint_) {
    ...
    if (metadata_filter.is_null()) {
      ...
      auto filtered_metadata = base::MakeUnique<base::DictionaryValue>();
