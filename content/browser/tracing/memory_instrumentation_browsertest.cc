...
IN_PROC_BROWSER_TEST_F(MemoryInstrumentationTest, PrivateFootprintComputation) {
  Navigate(shell());

  const int64_t kAllocSize = 10 * 1024 * 1024;
  ...
  // Should allocate at least 8*10^6 / 1024 = 7812.5kb.
  ...
  EXPECT_GE(during_renderer_kb - before_renderer_kb, 6000);
}
