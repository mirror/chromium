...
IN_PROC_BROWSER_TEST_F(NetInfoBrowserTest,
  ...
  LOG(WARNING) << "xxx cp4 downlink_mbps=" << downlink_mbps;
  ...
  LOG(WARNING) << "xxx cp5 downlink_mbps=" << downlink_mbps;
  fraction_part = std::modf(downlink_mbps, &int_part);
  LOG(WARNING) << "xxx cp6 downlink_mbps=" << downlink_mbps
               << " int_part=" << int_part
  ...
  LOG(WARNING) << "xxx cp7 downlink_mbps=" << downlink_mbps
               << " int_part=" << int_part
               << " fraction_part=" << fraction_part;

  EXPECT_EQ("4g", RunScriptExtractString("getEffectiveType()"));
  LOG(WARNING) << "xxx cp8 downlink_mbps=" << downlink_mbps
               << " int_part=" << int_part
               << " fraction_part=" << fraction_part;
}
