 ...
 public:
  ...
  LevelDBWrapperImplTest()
    ...
    level_db_wrapper_ = std::make_unique<LevelDBWrapperImpl>(
        &db_, kTestPrefix, &delegate_, options);
 ...
 private:
  ...
  void KeyAdded(const std::vector<uint8_t>& key,
    ...
    observations_.push_back({Observation::kAdd, Uint8VectorToStdString(key), "",
                             Uint8VectorToStdString(value), source, false});
  }
  void KeyChanged(const std::vector<uint8_t>& key,
    ...
    observations_.push_back({Observation::kChange, Uint8VectorToStdString(key),
                             ...
                             Uint8VectorToStdString(new_value), source, false});
  }
...
class LevelDBWrapperImplParamTest
    : public LevelDBWrapperImplTest,
      public testing::WithParamInterface<LevelDBWrapperImpl::CacheMode> {
 public:
...
INSTANTIATE_TEST_CASE_P(
    _,
    LevelDBWrapperImplParamTest,
    testing::Values(LevelDBWrapperImpl::CacheMode::KEYS_ONLY_WHEN_POSSIBLE,
...
TEST_F(LevelDBWrapperImplTest, SetOnlyKeysWithoutDatabase) {
  ...
  LevelDBWrapperImpl::Options options = {
      LevelDBWrapperImpl::CacheMode::KEYS_ONLY_WHEN_POSSIBLE, kTestSizeLimit,
      base::TimeDelta::FromSeconds(5),
      10 * 1024 * 1024 /* max_bytes_per_hour */, 60 /* max_commits_per_hour */};
  LevelDBWrapperImpl level_db_wrapper(nullptr, kTestPrefix, &delegate, options);
  ...
  level_db_wrapper.SetCacheModeForTesting(
...
TEST_F(LevelDBWrapperImplTest, SetCacheModeConsistent) {
  ...
  // Setting cache mode does not unload the keys_values_map till commit or purge
  // happens.
  wrapper_impl()->SetCacheModeForTesting(
      LevelDBWrapperImpl::CacheMode::KEYS_ONLY_WHEN_POSSIBLE);
  ...
  ASSERT_TRUE(wrapper_impl()->keys_values_map_);
  EXPECT_EQ(1u, wrapper_impl()->keys_values_map_->size());
  EXPECT_EQ(value2, wrapper_impl()->keys_values_map_->find(key)->second);
  // Commit should unload the keys_values_map.
  wrapper_impl()->CommitChanges();
  EXPECT_EQ(LevelDBWrapperImpl::LoadState::KEYS_ONLY,
            wrapper_impl()->desired_load_state_);
  ASSERT_TRUE(wrapper_impl()->keys_only_map_);
...
TEST_F(LevelDBWrapperImplTest, SendOldValueObservations) {
  ...
  EXPECT_TRUE(
      DeleteSync(StdStringToUint8Vector("doesn't exist"), base::nullopt));
  wrapper_impl()->SetCacheModeForTesting(
      LevelDBWrapperImpl::CacheMode::KEYS_ONLY_WHEN_POSSIBLE);
  // Flush tasks on mojo thread to observe callback.
  EXPECT_TRUE(
