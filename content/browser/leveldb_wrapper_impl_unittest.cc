...
class LevelDBWrapperImplTest : public testing::Test,
 ...
 public:
  ...
  void DeleteAll(mojom::LevelDBWrapper* wrapper,
                 bool* success,
                 base::OnceClosure done) {
    wrapper->DeleteAll(test_source_, base::BindOnce(&SuccessCallback,
  ...
  void ScheduleForkToPrefix(LevelDBWrapperImpl* source,
    ...
    base::PostTaskAndReplyWithResult(
        ...
        base::BindOnce(&WrapperCallback, std::move(done), destination));
  }
 ...
 protected:
  ...
  const std::vector<uint8_t> test_prefix_vec_ = StrToVec(test_prefix_);
  const std::vector<uint8_t> test_key1_vec_ = StrToVec(test_key1_);
  const std::vector<uint8_t> test_value1_vec_ = StrToVec(test_value1_);
  const std::vector<uint8_t> test_key2_vec_ = StrToVec(test_key2_);
  const std::vector<uint8_t> test_value2_vec_ = StrToVec(test_value2_);
...
TEST_F(LevelDBWrapperImplTest, GetLoadedFromMap) {
  ...
  std::vector<uint8_t> result;
  EXPECT_TRUE(GetSync(test_key2_vec_, &result));
  EXPECT_EQ(test_value2_vec_, result);
...
TEST_F(LevelDBWrapperImplTest, PutLoadsValuesAfterCacheModeUpgrade) {
  std::vector<uint8_t> key = StrToVec("newkey");
  std::vector<uint8_t> value1 = StrToVec("foo");
  std::vector<uint8_t> value2 = StrToVec("bar");
...
TEST_P(LevelDBWrapperImplParamTest, CommitPutToDB) {
  ...
  Put(test_key2_vec_, StrToVec(value1), test_value2_vec_, &put_success1,
      barrier);
  ...
  EXPECT_TRUE(has_mock_data(test_prefix_ + test_key2_));
  EXPECT_EQ(value1, get_mock_data(test_prefix_ + test_key2_));
...
TEST_P(LevelDBWrapperImplParamTest, PutObservations) {
  ...
  EXPECT_TRUE(
      PutSync(StrToVec(key), StrToVec(value2), StrToVec(value2), source2));
  ASSERT_EQ(2u, observations().size());
}
...
TEST_P(LevelDBWrapperImplParamTest, DeleteAllWithPendingMapLoad) {
  ...
  wrapper()->Put(StrToVec(key), StrToVec(value), base::nullopt, test_source_,
...
TEST_P(LevelDBWrapperImplParamTest, PutWhenAlreadyOverQuota) {
  ...
  set_mock_data(test_prefix_ + key, VecToStr(value));
...
TEST_P(LevelDBWrapperImplParamTest, PutWhenAlreadyOverQuotaBecauseOfLargeKey) {
  ...
  set_mock_data(test_prefix_ + VecToStr(key), VecToStr(value));
...
TEST_P(LevelDBWrapperImplParamTest, PutAfterPurgeMemory) {
  wrapper_impl()->SetCacheModeForTesting(GetParam());
  std::vector<uint8_t> result;
  const auto key = test_key2_vec_;
  const auto value = test_value2_vec_;
...
TEST_P(LevelDBWrapperImplParamTest, PurgeMemoryWithPendingChanges) {
  wrapper_impl()->SetCacheModeForTesting(GetParam());
  std::vector<uint8_t> key = test_value2_vec_;
  std::vector<uint8_t> value = StrToVec("foo");
  EXPECT_TRUE(PutSync(key, value, test_value2_vec_));
  EXPECT_EQ(delegate()->map_load_count(), 1);
...
TEST_P(LevelDBWrapperImplParamTest, FixUpData) {
  ...
  EXPECT_FALSE(has_mock_data(test_prefix_ + std::string("123")));
  EXPECT_EQ("foo", get_mock_data(test_prefix_ + std::string("def")));
  EXPECT_EQ("bla", get_mock_data(test_prefix_ + std::string("abc")));
}
...
TEST_F(LevelDBWrapperImplTest, GetAllWhenCacheOnlyKeys) {
  ...
  EXPECT_EQ(2u, data.size());
  EXPECT_TRUE(
      data[1]->Equals(mojom::KeyValue(test_key1_vec_, test_value1_vec_)))
      << VecToStr(data[1]->value) << " vs expected " << test_value1_;
...
TEST_F(LevelDBWrapperImplTest, PrefixForking) {
  ...
  options.cache_mode = CacheMode::KEYS_AND_VALUES;
  {
    ...
    // scheduled.
    Put(fork1.get(), test_key2_vec_, StrToVec(value4), test_value2_vec_,
        &put_success1, barrier);
    fork2 =
        fork1->ForkToNewPrefix(test_copy_prefix2_, &fork2_delegate, options);
    Put(fork1.get(), test_key2_vec_, StrToVec(value5), StrToVec(value4),
        &put_success2, barrier);

    // Do a put on original and create fork 3, which is key-only.
    Put(test_key1_vec_, StrToVec(value3), test_value1_vec_, &put_success3,
        barrier);
  ...
  EXPECT_EQ(test_value1_, GetStrSync(fork1.get(), test_key1_));
  EXPECT_EQ(test_value1_, GetStrSync(fork2.get(), test_key1_));
  ...
  EXPECT_EQ(value5, GetStrSync(fork1.get(), test_key2_));
  EXPECT_EQ(value4, GetStrSync(fork2.get(), test_key2_));
  ...
  EXPECT_EQ(value3, get_mock_data(kTestCopyPrefix3 + test_key1_));
  ...
  EXPECT_EQ(test_value2_, get_mock_data(kTestCopyPrefix3 + test_key2_));
}
...
TEST_P(LevelDBWrapperImplParamTest, PrefixForkAfterLoad) {
  ...
  EXPECT_TRUE(PutSync(test_key1_vec_, kValueVec, base::nullopt));
