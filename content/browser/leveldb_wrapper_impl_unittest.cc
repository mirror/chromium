...
TEST_F(LevelDBWrapperImplTest, TripleForkThenFail) {
  // Fork 3 times and delete the middle two to make sure the final fork reads
  // in the correct map.
  std::unique_ptr<LevelDBWrapperImpl> fork1;
  MockDelegate fork1_delegate;
  std::unique_ptr<LevelDBWrapperImpl> fork2;
  ...
  base::RunLoop loop;
  {
    ...
    fork1 = wrapper_impl()->ForkToNewPrefix(kTestCopyPrefix1, &fork1_delegate);
    fork2 = fork1->ForkToNewPrefix(kTestCopyPrefix2, &fork2_delegate);
    fork3 = fork2->ForkToNewPrefix(kTestCopyPrefix3, &fork3_delegate);
    fork1.reset();
    fork2.reset();
    base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, barrier.Get());
  }
  loop.Run();

  // Test values are accurate before commit
  EXPECT_EQ(kTestValue1, GetStrSync(fork3.get(), kTestKey1));
  EXPECT_EQ(kTestValue2, GetStrSync(fork3.get(), kTestKey2));

  ScheduleCommitChanges(delegate(), wrapper_impl());

  EXPECT_EQ(kTestValue1,
            get_mock_data(std::string(kTestCopyPrefix3) + kTestKey1));
  EXPECT_EQ(kTestValue2,
            get_mock_data(std::string(kTestCopyPrefix3) + kTestKey2));
}
    ...
    for (int64_t i = 0; i < 1000; i++) {
      ...
      if (i % 5 == 0 || i % 6 == 0) {
        ...
        wrappers[forks] = wrappers[i]->ForkToNewPrefix(GetPrefix(&curr_prefix),
                                                       &delegates[forks]);
        if (i % 4 != 0) {
          // We kill the wrapper and hope that the fork will read in the
          // old-prefix.
          wrappers[i].reset();
          continue;
        }
