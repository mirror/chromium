...
TEST_P(LevelDBWrapperImplParamTest, CommitPutToDB) {
  ...
  EXPECT_TRUE(PutSync(StrToVec(key1), StrToVec(value1), test_value2_vec_));
...
TEST_P(LevelDBWrapperImplParamTest, FixUpData) {
  ...
  changes.push_back(std::make_pair(test_key1_vec_, StrToVec("foo")));
  changes.push_back(std::make_pair(test_key2_vec_, base::nullopt));
  changes.push_back(std::make_pair(test_prefix_vec_, StrToVec("bla")));
  ...
  EXPECT_EQ(test_prefix_, VecToStr(data[0]->key));
  EXPECT_EQ("bla", VecToStr(data[0]->value));
  EXPECT_EQ(test_key1_, VecToStr(data[1]->key));
...
TEST_F(LevelDBWrapperImplTest, SetCacheModeConsistent) {
  ...
  // Changing back the cacLevelDBWrapperImpl::CacheModehe mode still works.
