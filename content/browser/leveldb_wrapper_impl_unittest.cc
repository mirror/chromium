...
// const char* kTestCopyPrefix3 = "xxx";
...
class GetAllCallback : public mojom::LevelDBWrapperGetAllCallback {
 public:
  static mojom::LevelDBWrapperGetAllCallbackAssociatedPtrInfo CreateAndBind(
    ...
    auto request = mojo::MakeIsolatedRequest(&ptr);
...
TEST_F(LevelDBWrapperImplTest, PrefixForking) {
  ...
  EXPECT_TRUE(SyncWrapperHasValue(wrapper(), kTestKey1, value3));
  EXPECT_TRUE(SyncWrapperHasValue(fork1.get(), kTestKey1, kTestValue1));
  EXPECT_TRUE(SyncWrapperHasValue(fork2.get(), kTestKey1, kTestValue1));
  EXPECT_TRUE(SyncWrapperHasValue(fork3.get(), kTestKey1, value3));
  EXPECT_TRUE(SyncWrapperHasValue(wrapper(), kTestKey2, kTestValue2));
  EXPECT_TRUE(SyncWrapperHasValue(fork1.get(), kTestKey2, value5));
  EXPECT_TRUE(SyncWrapperHasValue(fork2.get(), kTestKey2, value4));
  EXPECT_TRUE(SyncWrapperHasValue(fork3.get(), kTestKey2, kTestValue2));
