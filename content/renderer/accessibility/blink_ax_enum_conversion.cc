...
void AXStateFromBlink(const blink::WebAXObject& o, ui::AXNodeData* dst) {
  ...
  if (expanded) {
    if (expanded == blink::kWebAXExpandedCollapsed)
      dst->AddState(ax::mojom::State::COLLAPSED);
    else if (expanded == blink::kWebAXExpandedExpanded)
      dst->AddState(ax::mojom::State::EXPANDED);
  }

  if (o.CanSetFocusAttribute())
    dst->AddState(ax::mojom::State::FOCUSABLE);

  if (o.Role() == blink::kWebAXRolePopUpButton || o.AriaHasPopup())
    dst->AddState(ax::mojom::State::HASPOPUP);

  if (o.IsHovered())
    dst->AddState(ax::mojom::State::HOVERED);

  if (!o.IsVisible())
    dst->AddState(ax::mojom::State::INVISIBLE);

  if (o.IsLinked())
    dst->AddState(ax::mojom::State::LINKED);

  if (o.IsMultiline())
    dst->AddState(ax::mojom::State::MULTILINE);

  if (o.IsMultiSelectable())
    dst->AddState(ax::mojom::State::MULTISELECTABLE);

  if (o.IsPasswordField())
    dst->AddState(ax::mojom::State::PROTECTED);

  if (o.IsRequired())
    dst->AddState(ax::mojom::State::REQUIRED);

  if (o.IsSelected() != blink::kWebAXSelectedStateUndefined)
    dst->AddState(ax::mojom::State::SELECTABLE);

  if (o.IsEditable())
    dst->AddState(ax::mojom::State::EDITABLE);

  if (o.IsSelected() == blink::kWebAXSelectedStateTrue)
    dst->AddState(ax::mojom::State::SELECTED);

  if (o.IsRichlyEditable())
    dst->AddState(ax::mojom::State::RICHLY_EDITABLE);

  if (o.IsVisited())
    dst->AddState(ax::mojom::State::VISITED);

  if (o.Orientation() == blink::kWebAXOrientationVertical)
    dst->AddState(ax::mojom::State::VERTICAL);
  else if (o.Orientation() == blink::kWebAXOrientationHorizontal)
    dst->AddState(ax::mojom::State::HORIZONTAL);
...
ax::mojom::TextStyle AXTextStyleFromBlink(blink::WebAXTextStyle text_style) {
  uint32_t browser_text_style =
      static_cast<uint32_t>(ax::mojom::TextStyle::NONE);
