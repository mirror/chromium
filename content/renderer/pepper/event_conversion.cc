...
void AppendMouseWheelEvent(const WebInputEvent& event,
  ...
  InputEventData result = GetEventWithCommonFieldsAndType(event);
  result.event_modifiers = ConvertEventModifiers(mouse_wheel_event.GetModifiers());
...
void AppendTouchEvent(const WebInputEvent& event,
  ...
  SetPPTouchPoints(
      touch_event.touches, touch_event.touches_length, ACTIVE, &result.touches);
...
WebMouseWheelEvent* BuildMouseWheelEvent(const InputEventData& event) {
  WebMouseWheelEvent* mouse_wheel_event = new WebMouseWheelEvent(
      WebInputEvent::kMouseWheel, event.event_modifiers, event.event_time_stamp);
