...
bool PepperGraphics2DHost::PrepareTextureMailbox(
  ...
  if (main_thread_context_) {
    ...
    // BestTextureFormat() is based on what color order skia bitmaps are in.
    // This matches the order that |src| is in.
    uint32_t format =
        viz::GLDataFormat(viz::PlatformColor::BestTextureFormat());
    uint32_t internal_format =
        viz::GLInternalFormat(viz::PlatformColor::BestTextureFormat());
    gl->TexImage2D(GL_TEXTURE_2D, 0, format, image_data_->width(),
                   image_data_->height(), 0, internal_format, GL_UNSIGNED_BYTE,
                   src);
    image_data_->Unmap();
