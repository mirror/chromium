...
bool PepperWebPluginImpl::ExecuteEditCommand(const blink::WebString& name,
  ...
  // If the copy-paste clipboard contains something other than text (e.g. an
  // image), WebClipboard::ReadPlainText() returns an empty string. The empty
  // string is then pasted, replacing any selected text. This behavior is
  // consistent with that of HTML text form fields.
