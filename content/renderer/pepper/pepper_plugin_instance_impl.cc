...
bool PepperPluginInstanceImpl::SendCompositionEventToPlugin(
  ...
  std::vector<blink::WebImeSpan> empty;
  return SendCompositionEventWithImeSpanInformationToPlugin(
      type, text, empty, static_cast<int>(text.size()),
      static_cast<int>(text.size()));
}

bool PepperPluginInstanceImpl::
    SendCompositionEventWithImeSpanInformationToPlugin(
        PP_InputEvent_Type type,
        const base::string16& text,
  ...
  for (size_t i = 0; i < ime_spans.size(); ++i) {
    utf16_offsets.push_back(ime_spans[i].start_offset);
    utf16_offsets.push_back(ime_spans[i].end_offset);
  }
...
bool PepperPluginInstanceImpl::HandleCompositionUpdate(
    ...
    int selection_end) {
  return SendCompositionEventWithImeSpanInformationToPlugin(
      PP_INPUTEVENT_TYPE_IME_COMPOSITION_UPDATE, text, ime_spans,
  ...
  switch (input_event.event_type) {
    ...
    case PP_INPUTEVENT_TYPE_IME_TEXT:
      ...
      render_frame_->SimulateImeCommitText(
          ...
          std::vector<blink::WebImeSpan>(), gfx::Range());
      break;
    default:
      return false;
  }
...
void PepperPluginInstanceImpl::SimulateImeSetCompositionEvent(
  ...
  for (size_t i = 2; i + 1 < offsets.size(); ++i) {
    ...
    ime_span.start_offset = offsets[i];
    ime_span.end_offset = offsets[i + 1];
    if (input_event.composition_target_segment == static_cast<int32_t>(i - 2))
      ime_span.thick = true;
    ime_spans.push_back(ime_span);
  }

  render_frame_->SimulateImeSetComposition(utf16_text, ime_spans, offsets[0],
                                           offsets[1]);
}
