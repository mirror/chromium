...
void BrowserPlugin::UpdateGuestFocusState(blink::WebFocusType focus_type) {
  ...
  LOG(ERROR) << "BrowserPlugin::UpdateFocus to " << should_be_focused << " for plugin id " << browser_plugin_instance_id_ << " type=" << focus_type;
...
bool BrowserPlugin::ShouldGuestBeFocused() const {
  ...
  if (render_view)
    ...
    LOG(ERROR) << "No renderview for frame routing id " << render_frame_routing_id();
  LOG(ERROR) <<"plugin_focused_="<<plugin_focused_<< " embedder_focused="<<embedder_focused;
