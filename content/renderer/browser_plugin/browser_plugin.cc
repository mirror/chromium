...
bool BrowserPlugin::SetComposition(
    ...
    const blink::WebVector<blink::WebImeSpan>& ime_spans,
  ...
  for (size_t i = 0; i < ime_spans.size(); ++i) {
    params.ime_spans.push_back(ime_spans[i]);
  }
...
bool BrowserPlugin::CommitText(
  ...
  std::vector<blink::WebImeSpan> std_ime_spans;
  for (size_t i = 0; i < ime_spans.size(); ++i) {
    std_ime_spans.push_back(ime_spans[i]);
  }
  ...
  BrowserPluginManager::Get()->Send(new BrowserPluginHostMsg_ImeCommitText(
      browser_plugin_instance_id_, text.Utf16(), std_ime_spans,
