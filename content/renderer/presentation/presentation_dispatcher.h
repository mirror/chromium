...
class CONTENT_EXPORT PresentationDispatcher
 ...
 private:
  ...
  // otherwise, returns UNSUPPORTED if any url in |urls| have screen
  // availability UNSUPPORTED; otherwise, returns SOURCE_NOT_COMPATIBLE if any
  // url in |urls| have screen availability SOURCE_NOT_COMPATIBLE; otherwise
  // returns UNAVAILABLE if at least one url in |urls| has screen availability
  // UNAVAILABLE; otherwise returns UNKNOWN.
