...
void ManifestManager::RequestManifestAndDebugInfo(
    RequestManifestAndDebugInfoCallback callback) {
  RequestManifestImpl(base::BindOnce(
      [](RequestManifestAndDebugInfoCallback callback, const GURL& manifest_url,
         const Manifest& manifest,
         const blink::mojom::ManifestDebugInfo* debug_info) {
        std::move(callback).Run(manifest_url, manifest,
