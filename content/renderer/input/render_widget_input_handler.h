...
class CONTENT_EXPORT RenderWidgetInputHandler {
 ...
 private:
  ...
  // Used to intercept touch action notifications while an event is being
  // handled. If the event has an associated touch action, the touch action
  // metadata can be bundle in the event ack, saving an IPC.
  cc::TouchAction handling_touch_action_;
