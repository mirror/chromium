...
RenderWidgetInputHandler::RenderWidgetInputHandler(
    ...
    : delegate_(delegate),
      ...
      handling_touch_action_(cc::kNoTouchAction),
...
void RenderWidgetInputHandler::HandleInputEvent(
  ...
  base::AutoReset<cc::TouchAction> handling_touch_action_resetter(
  ...
  if (callback) {
    std::move(callback).Run(ack_result, swap_latency_info,
                            std::move(event_overscroll), touch_action);
  } else {
...
void RenderWidgetInputHandler::TouchActionFromBlink(
  ...
  if (handling_touch_action_ != cc::kNoTouchAction) {
    handling_touch_action_ = touch_action;
    return;
  }

  delegate_->OnSetTouchAction(touch_action);
}
