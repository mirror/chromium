...
void PointPredictor::Update(WebPointerProperties& event, double timestamp) {
...
void InputEventPredictor::Update(WebPointerProperties& event, double time) {
  pointer_predictor_map_[event.id].Update(event, time);
}

void InputEventPredictor::HandleEvent(ui::WebScopedInputEvent& event) {
  if (WebInputEvent::IsTouchEventType(event->GetType())) {
    WebTouchEvent* touch_event = static_cast<WebTouchEvent*>(event.get());
    ...
    WebMouseEvent* mouse_event = static_cast<WebMouseEvent*>(event.get());
