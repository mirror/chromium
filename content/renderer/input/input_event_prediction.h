...
class PointPredictor {
 public:
  ...
  void Update(WebPointerProperties&, double);
...
class InputEventPredictor {
 public:
  ...
  void Update(WebPointerProperties& event, double time);
  void HandleEvent(ui::WebScopedInputEvent& event);
  void ResampleEvent(blink::WebInputEvent& event, base::TimeTicks frame_time);
