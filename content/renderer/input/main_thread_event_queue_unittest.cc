...
class HandledEventCallbackTracker {
 public:
  ...
  void DidHandleEvent(size_t index,
    ...
    callback_received_[index] = true;
  }

  const std::vector<bool>& GetReceivedCallbacks() const {
    return callback_received_;
  }

 private:
  std::vector<bool> callback_received_;
...
class MainThreadEventQueueTest : public testing::TestWithParam<unsigned>,
 ...
 public:
  ...
  std::vector<bool> GetAndResetCallbackResults() {
...
TEST_P(MainThreadEventQueueTest, NonBlockingWheel) {
  ...
  EXPECT_THAT(GetAndResetCallbackResults(), testing::Each(true));
  EXPECT_FALSE(main_task_runner_->HasPendingTask());
  EXPECT_EQ(0u, event_queue().size());
