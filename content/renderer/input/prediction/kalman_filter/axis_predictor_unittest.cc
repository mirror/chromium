...
namespace {
constexpr int kExpectedStableIterNum = 4;
constexpr double kDoubleDelta = 1e-10;
template <typename T>
void ExpectApproxEq(T expected, T actual) {
  EXPECT_GE(expected + kDoubleDelta, actual);
  EXPECT_LE(expected - kDoubleDelta, actual);
}
}  // namespace
...
void ValidateAxisPredictor(AxisPredictor* predictor, const DataSet& data) {
  ...
  for (size_t i = 0; i < data.observation.size(); i++) {
    ...
    ExpectApproxEq(data.position[i], predictor->GetPosition());
    ExpectApproxEq(data.velocity[i], predictor->GetVelocity());
    ExpectApproxEq(data.acceleration[i], predictor->GetAcceleration());
    ExpectApproxEq(data.jerk[i], predictor->GetJerk());
  }
}

// Test that the predictor will stable.
TEST(AxisPredictorTest, ShouldStable) {
  AxisPredictor predictor;
  for (int i = 0; i < kExpectedStableIterNum; i++) {
    predictor.Update(1);
  }
  EXPECT_EQ(predictor.Stable(), true);
}
