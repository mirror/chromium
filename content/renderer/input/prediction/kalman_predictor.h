...
class KalmanPredictor : public InputPredictor {
 public:
  explicit KalmanPredictor(double predict_interval_seconds);
  ...
  std::vector<InputData> GeneratePrediction() override;
 ...
 private:
  ...
  // Interval to be predicted.
  double predict_interval_seconds_;
