...
KalmanPredictor::KalmanPredictor(double predict_interval)
    : predict_interval_seconds_(predict_interval) {}
...
std::vector<InputData> KalmanPredictor::GeneratePrediction() {
  ...
  int predict_target_sample_num = static_cast<int>(
      ceil(predict_interval_seconds_ / avg_report_delta_time_seconds_));
  ...
  for (int i = 0; i < predict_target_sample_num; i++) {
    ...
    next_point.time_stamp_seconds += avg_report_delta_time_seconds_;
