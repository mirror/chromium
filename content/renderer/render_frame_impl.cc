...
URLLoaderFactoryBundle* RenderFrameImpl::GetSubresourceLoaderFactories() {
  ...
  if (!subresource_loader_factories_) {
    ...
    DCHECK(creator);
    // TODO(yzshen): make sure this direct reference is okay when SetCustom...
    // tries to modify things, this will affect parent if it is a subframe.
    subresource_loader_factories_ = creator->GetSubresourceLoaderFactories();
  }
