...
RenderFrameImpl::CreateWorkerFetchContext() {
  ...
  if (provider->context()) {
    ...
    // TODO(horo): Currently we get the host pointer to call
    // GetControllerServiceWorker() fron the worker thread if S13nServiceWorker
    ...
    if (ServiceWorkerUtils::IsServicificationEnabled()) {
      container_host_ptr_info = provider->context()->CloneContainerHostPtr();
    }
  }
