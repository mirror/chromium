...
void RenderFrameImpl::SimulateImeSetComposition(
    ...
    int selection_start,
    int selection_end) {
  render_view_->OnImeSetComposition(text, text_composition_data,
...
void RenderFrameImpl::SimulateImeCommitText(
  ...
  render_view_->OnImeCommitText(text, text_composition_data, replacement_range,
                                0);
}
  ...
  if (!IsPepperAcceptingCompositionEvents()) {
    ...
    if (!pepper_composition_text_.empty()) {
      focused_pepper_plugin_->HandleCompositionUpdate(
          ...
          text_composition_data.composition_underlines, selection_start,
          selection_end);
    }
  }
}
...
void RenderFrameImpl::OnSetCompositionFromExistingText(
    ...
    const blink::WebTextCompositionData& text_composition_data) {
  ImeEventGuard guard(GetRenderWidget());
  frame_->SetCompositionFromExistingText(start, end, text_composition_data);
}
