...
void RenderFrameImpl::SimulateImeSetComposition(
    ...
    int selection_start,
    int selection_end) {
  render_view_->OnImeSetComposition(text, ime_spans, gfx::Range::InvalidRange(),
...
void RenderFrameImpl::SimulateImeCommitText(
  ...
  render_view_->OnImeCommitText(text, ime_spans, replacement_range, 0);
}
  ...
  if (!IsPepperAcceptingCompositionEvents()) {
    ...
    if (!pepper_composition_text_.empty()) {
      focused_pepper_plugin_->HandleCompositionUpdate(
          pepper_composition_text_, ime_spans, selection_start, selection_end);
    }
  }
}
...
void RenderFrameImpl::OnSetCompositionFromExistingText(
    ...
    const std::vector<blink::WebImeSpan>& ime_spans) {
  ImeEventGuard guard(GetRenderWidget());
  frame_->SetCompositionFromExistingText(start, end, ime_spans);
}
