...
class RenderFrameImpl::FrameURLLoaderFactory
 ...
 public:
  ...
  std::unique_ptr<blink::WebURLLoader> CreateURLLoader(
    ...
    // Currently the tests that need URLLoader but do not have ChildThread
    // (e.g. RenderViewTests) use the
    // Platform::Current()->CreateDefaultURLLoaderFactoryGetter().
...
RenderFrameImpl::CreateURLLoaderFactory() {
  return std::make_unique<FrameURLLoaderFactory>(
      weak_factory_.GetWeakPtr(), RenderThreadImpl::current()
                                      ? GetDefaultURLLoaderFactoryGetter()
                                      : nullptr);
}
