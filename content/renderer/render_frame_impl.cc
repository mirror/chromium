...
void RenderFrameImpl::DidFailProvisionalLoad(
  ...
  WebDocumentLoader* document_loader = frame_->ProvisionalDocumentLoader();
  if (!document_loader)
    return;
...
void RenderFrameImpl::DidCommitProvisionalLoad(
  ...
  const WebURLResponse& web_url_response =
      frame_->DocumentLoader()->GetResponse();
  WebURLResponseExtraDataImpl* extra_data =
...
void RenderFrameImpl::RunScriptsAtDocumentReady(bool document_is_empty) {
  ...
  InternalDocumentStateData* internal_data =
      InternalDocumentStateData::FromDocumentLoader(frame_->DocumentLoader());
  ...
  if (GetContentClient()->renderer()->HasErrorPage(http_status_code)) {
    ...
    LoadNavigationErrorPage(frame_->DocumentLoader()->GetRequest(), error, true,
                            nullptr);
  }
...
void RenderFrameImpl::DidHandleOnloadEvents() {
  if (!frame_->Parent()) {
    FrameMsg_UILoadMetricsReportType::Value report_type =
        static_cast<FrameMsg_UILoadMetricsReportType::Value>(
            frame_->DocumentLoader()
                ->GetRequest()
                .InputPerfMetricReportPolicy());
    base::TimeTicks ui_timestamp =
        base::TimeTicks() +
        base::TimeDelta::FromSecondsD(
            frame_->DocumentLoader()->GetRequest().UiStartTime());
...
void RenderFrameImpl::DidFailLoad(const blink::WebURLError& error,
  ...
  WebDocumentLoader* document_loader = frame_->DocumentLoader();
...
void RenderFrameImpl::DidReceiveResponse(
  ...
  if (!frame_->ProvisionalDocumentLoader() || frame_->Parent())
    return;
...
blink::WebString RenderFrameImpl::UserAgentOverride() {
  ...
  if (main_frame->ProvisionalDocumentLoader())
    document_loader = main_frame->ProvisionalDocumentLoader();
  else
    document_loader = main_frame->DocumentLoader();
...
void RenderFrameImpl::SendDidCommitProvisionalLoad(
    ...
    blink::WebHistoryCommitType commit_type) {
  DCHECK_EQ(frame_, frame);
  WebDocumentLoader* document_loader = frame->DocumentLoader();
  DCHECK(document_loader);
  ...
  if (!frame->Parent()) {
    ...
    params.report_type = static_cast<FrameMsg_UILoadMetricsReportType::Value>(
        frame->DocumentLoader()->GetRequest().InputPerfMetricReportPolicy());
    params.ui_timestamp =
        base::TimeTicks() +
        base::TimeDelta::FromSecondsD(
            frame->DocumentLoader()->GetRequest().UiStartTime());
  } else {
...
void RenderFrameImpl::OnFailedNavigation(
  ...
  // notification.
  bool had_provisional_document_loader = frame_->ProvisionalDocumentLoader();
  if (request_params.nav_entry_id == 0) {
  ...
  // ProvisionalDocumentLoader(), LoadNavigationErrorPage wasn't called, so do
  // it now.
...
WebNavigationPolicy RenderFrameImpl::DecidePolicyForNavigation(
  ...
  GURL old_url(frame_->DocumentLoader()->GetRequest().Url());
...
void RenderFrameImpl::NavigateInternal(
  ...
  // processed.
  if (IsBrowserSideNavigationEnabled()) {
    frame_->DocumentLoader()->ResetSourceLocation();
    if (frame_->ProvisionalDocumentLoader())
      frame_->ProvisionalDocumentLoader()->ResetSourceLocation();
  }
}
