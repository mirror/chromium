...
void RenderFrameImpl::DidCommitProvisionalLoad(
  ...
  if (is_main_frame_) {
    // Updates the PreviewsState and effective connection type states.
    // Subframes inherit from the main frame and should not
    ...
    observer.DidCommitProvisionalLoad(frame_, is_new_navigation);
  {
    ...
    for (auto& observer : observers_) {
      observer.DidCommitProvisionalLoad(
          is_new_navigation, false /* is_same_document_navigation */);
    }
  }
...
RenderFrameImpl::MakeDidCommitProvisionalLoadParams(
  ...
  if (!frame_->Parent()) {
    ...
    // The type depends on whether this navigation
    ...
    if (!navigation_state->common_params().should_replace_current_entry) {
      ...
      render_view_->history_list_length_ =
          render_view_->history_list_offset_ + 1;
             ...
             blink::WebHistoryCommitType::kWebBackForwardCommit) {
    render_view_->history_list_offset_ =
