...
RenderFrameImpl* RenderFrameImpl::Create(
  ...
  CreateParams params(render_view, routing_id, std::move(interfaces_provider),
...
RenderFrameImpl* RenderFrameImpl::CreateMainFrame(
  ...
  RenderFrameImpl* render_frame = RenderFrameImpl::Create(
      render_view, routing_id, std::move(interfaces_provider),
      devtools_frame_token);
...
void RenderFrameImpl::CreateFrame(
  ...
  if (proxy_routing_id == MSG_ROUTING_NONE) {
    ...
    render_frame = RenderFrameImpl::Create(
        parent_proxy->render_view(), routing_id, std::move(interfaces_provider),
        devtools_frame_token);
...
RenderFrameImpl::CreateParams::CreateParams(
    ...
    : render_view(render_view),
      ...
      interfaces_provider(std::move(interfaces_provider)),
...
RenderFrameImpl::RenderFrameImpl(CreateParams params)
  ...
  DCHECK(params.interfaces_provider.is_bound());
  remote_interfaces_ = std::make_unique<service_manager::InterfaceProvider>();
  remote_interfaces_->Bind(std::move(params.interfaces_provider));
...
void RenderFrameImpl::DidCommitProvisionalLoad(
  ...
  if (!navigation_state->WasWithinSameDocument()) {
    ...
    // to avoid GetInterface calls racing with navigation commit.
    service_manager::mojom::InterfaceProviderPtr interfaces_provider;
    remote_interfaces_request = mojo::MakeRequest(&interfaces_provider);
