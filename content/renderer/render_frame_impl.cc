...
void GetRedirectChain(WebDocumentLoader* loader, std::vector<GURL>* result) {
  WebVector<WebURL> urls;
  loader->RedirectChain(urls);
...
bool MaybeGetOverriddenURL(WebDocumentLoader* loader, GURL* output) {
  DocumentState* document_state = DocumentState::FromDocumentLoader(loader);
  ...
  if (loader->HasUnreachableURL()) {
    *output = loader->UnreachableURL();
    return true;
  }
...
// ds->unreachableURL()
GURL GetOriginalRequestURL(WebDocumentLoader* loader) {
  GURL overriden_url;
  if (MaybeGetOverriddenURL(loader, &overriden_url))
    return overriden_url;

  std::vector<GURL> redirects;
  GetRedirectChain(loader, &redirects);
  if (!redirects.empty())
...
void RenderFrameImpl::LoadErrorPage(int reason) {
  ...
  std::string error_html;
  GetContentClient()->renderer()->GetNavigationErrorStrings(
      this, frame_->DocumentLoader()->GetRequest(), error, &error_html,
      nullptr);
...
bool RenderFrameImpl::IsFTPDirectoryListing() {
  WebURLResponseExtraDataImpl* extra_data =
      GetExtraDataFromResponse(frame_->DocumentLoader()->GetResponse());
...
RenderFrameImpl::CreateApplicationCacheHost(
  ...
  DocumentState* document_state =
      frame_->ProvisionalDocumentLoader()
          ? DocumentState::FromDocumentLoader(
                frame_->ProvisionalDocumentLoader())
          : DocumentState::FromDocumentLoader(frame_->DocumentLoader());
...
RenderFrameImpl::CreateWorkerFetchContext() {
  ...
  blink::WebServiceWorkerNetworkProvider* web_provider =
      frame_->DocumentLoader()->GetServiceWorkerNetworkProvider();
  if (web_provider) {
...
void RenderFrameImpl::WillSubmitForm(const blink::WebFormElement& form) {
  DocumentState* document_state =
      DocumentState::FromDocumentLoader(frame_->ProvisionalDocumentLoader());
  NavigationStateImpl* navigation_state =
      static_cast<NavigationStateImpl*>(document_state->navigation_state());
...
void RenderFrameImpl::DidCreateDocumentLoader(
  ...
  if (!document_state) {
    ...
    loader->SetExtraData(document_state);
  ...
  if (content_initiated && webview && webview->MainFrame() &&
      ...
      webview->MainFrame()->ToWebLocalFrame()->DocumentLoader()) {
  ...
  if (IsBrowserSideNavigationEnabled() &&
    ...
    loader->SetSourceLocation(source_location);
  }
  ...
  loader->SetServiceWorkerNetworkProvider(
      ServiceWorkerNetworkProvider::CreateForNavigation(
          routing_id_, navigation_state->request_params(), frame,
          content_initiated));
}

void RenderFrameImpl::DidStartProvisionalLoad(blink::WebDocumentLoader* loader,
...
void RenderFrameImpl::DidFailProvisionalLoad(
  ...
  WebDocumentLoader* loader = frame_->ProvisionalDocumentLoader();
  if (!loader)
    return;

  const WebURLRequest& failed_request = loader->GetRequest();
...
void RenderFrameImpl::DidFailLoad(const blink::WebURLError& error,
  ...
  WebDocumentLoader* loader = frame_->DocumentLoader();
  DCHECK(loader);
...
void RenderFrameImpl::DidFinishLoad() {
  ...
  WebDocumentLoader* loader = frame_->DocumentLoader();
  Send(new FrameHostMsg_DidFinishLoad(routing_id_, loader->GetRequest().Url()));
...
void RenderFrameImpl::WillSendRequest(blink::WebURLRequest& request) {
  WebDocumentLoader* provisional_loader = frame_->ProvisionalDocumentLoader();
  WebDocumentLoader* loader =
      provisional_loader ? provisional_loader : frame_->DocumentLoader();
  ...
  bool should_replace_current_entry = loader->ReplacesCurrentHistoryItem();
...
blink::WebString RenderFrameImpl::UserAgentOverride() {
  ...
  WebDocumentLoader* loader = nullptr;
  if (main_frame->ProvisionalDataSource())
    loader = main_frame->ProvisionalDocumentLoader();
  else
    loader = main_frame->DocumentLoader();

  InternalDocumentStateData* internal_data =
      loader ? InternalDocumentStateData::FromDocumentLoader(loader) : NULL;
...
void RenderFrameImpl::SendDidCommitProvisionalLoad(
    ...
    blink::WebHistoryCommitType commit_type) {
  DCHECK_EQ(frame_, frame);
  WebDocumentLoader* loader = frame->DataSource();
  DCHECK(loader);

  const WebURLRequest& request = loader->GetRequest();
  const WebURLResponse& response = loader->GetResponse();
  ...
  params.url_is_unreachable = loader->HasUnreachableURL();
  params.method = "GET";
  ...
  params.should_replace_current_entry = loader->ReplacesCurrentHistoryItem();
  params.post_id = -1;
  ...
  GetRedirectChain(loader, &params.redirects);
  params.should_update_history =
      !loader->HasUnreachableURL() && response.HttpStatusCode() != 404;
  ...
  if (loader->IsClientRedirect()) {
    params.referrer =
        Referrer(params.redirects[0], loader->GetRequest().GetReferrerPolicy());
  } else {
    params.referrer =
        RenderViewImpl::GetReferrerFromRequest(frame, loader->GetRequest());
  }
  ...
  if (!frame->Parent()) {
    ...
    params.contents_mime_type = loader->GetResponse().MimeType().Utf8();
    ...
    // process.
    params.original_request_url = GetOriginalRequestURL(loader);
...
void RenderFrameImpl::OpenURL(
  ...
  if (IsBrowserInitiated(pending_navigation_params_.get())) {
    ...
    WebDocumentLoader* loader = frame_->ProvisionalDataSource();
    DCHECK(loader);
    params.should_replace_current_entry = loader->ReplacesCurrentHistoryItem();
  } else {
...
GURL RenderFrameImpl::GetLoadingUrl() const {
  WebDocumentLoader* loader = frame_->DataSource();
