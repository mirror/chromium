...
void RenderFrameImpl::SendDidCommitProvisionalLoad(
  ...
  // "inert" commits as creating new NavigationEntries if they replace the
  // current entry on a cross-document navigation, since this will replace the
  // subframes and could go cross-origin.
  params.did_create_new_entry =
      (commit_type == blink::kWebStandardCommit) ||
      (params.should_replace_current_entry && !params.was_within_same_document);
