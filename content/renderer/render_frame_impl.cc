...
URLLoaderFactoryBundle& RenderFrameImpl::GetSubresourceLoaderFactories() {
  ...
  if (!subresource_loader_factories_) {
    // If we have no existing bundle, it's because we haven't committed a
    // navigation. If we haven't committed a navigation but we're loading
    // subresources, we must have been created by a parent frame. Use its bundle
    // in this case.
    RenderFrameImpl* parent = RenderFrameImpl::FromWebFrame(frame_->Parent());
    DCHECK(parent);
    subresource_loader_factories_ =
        parent->GetSubresourceLoaderFactories().Clone();
  }
