...
RenderFrameImpl::CreateWorkerFetchContext() {
  ...
  mojom::ServiceWorkerWorkerClientRequest request =
      provider->CreateWorkerClientRequest();
  ...
  std::unique_ptr<WorkerFetchContextImpl> worker_fetch_context =
      base::MakeUnique<WorkerFetchContextImpl>(
          std::move(request), std::move(url_loader_factory_getter_info));
...
std::unique_ptr<blink::WebURLLoader> RenderFrameImpl::CreateURLLoader(
  ...
  RendererBlinkPlatformImpl* blink_platform_impl =
      RenderThreadImpl::current()->blink_platform_impl();
  ChildThreadImpl* child_thread = ChildThreadImpl::current();
  if (!child_thread) {
    return blink_platform_impl->CreateURLLoader(request, task_runner);
  }

  mojom::URLLoaderFactory* factory = custom_url_loader_factory_.get();
  ChildURLLoaderFactoryGetter* url_loader_factory_getter =
      blink_platform_impl->GetDefaultURLLoaderFactoryGetter();

  if (request.Url().ProtocolIs(url::kBlobScheme)) {
    factory = url_loader_factory_getter->GetBlobLoaderFactory();
    DCHECK(factory);
  }

  if (!factory) {
    factory = url_loader_factory_getter->GetNetworkLoaderFactory();
    DCHECK(factory);
  }
