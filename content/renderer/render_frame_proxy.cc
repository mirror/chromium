...
void RenderFrameProxy::WasResized() {
  ...
  bool synchronized_params_changed =
      !sent_resize_params_ ||
      sent_resize_params_->physical_backing_size !=
          pending_resize_params_.physical_backing_size ||
  ...
  if (enable_surface_synchronization_)
    compositing_helper_->SetPrimarySurfaceId(surface_id,
                                             physical_backing_size());
  ...
  if (rect_changed && mus_embedded_frame_)
    mus_embedded_frame_->SetWindowBounds(local_surface_id_,
                                         gfx::Rect(physical_backing_size()));
#endif

  if (resize_params_changed) {
    // Let the browser know about the updated view rect.
    Send(new FrameHostMsg_UpdateResizeParams(
        routing_id_, frame_rect(), physical_backing_size(), screen_info(),
...
void RenderFrameProxy::FrameRectsChanged(
    ...
    const blink::WebRect& transformed_frame_rect) {
  pending_resize_params_.frame_rect = gfx::Rect(transformed_frame_rect);
  pending_resize_params_.physical_backing_size =
