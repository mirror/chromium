...
DeviceOrientationEventPump::DeviceOrientationEventPump(RenderThread* thread,
    ...
    : DeviceSensorEventPump<blink::WebDeviceOrientationListener>(thread),
      ...
      absolute_(absolute),
      need_to_get_absolute_orientation_sensor_when_necessary_(false) {}
...
void DeviceOrientationEventPump::SendStartMessage() {
  ...
  if (!absolute_orientation_sensor_.sensor &&
    ...
    if (absolute_) {
      ...
      need_to_get_absolute_orientation_sensor_when_necessary_ = true;
...
void DeviceOrientationEventPump::DidStartIfPossible() {
  if (absolute_) {
    ...
    if (relative_orientation_sensor_.sensor ||
        ...
        !need_to_get_absolute_orientation_sensor_when_necessary_) {
      DeviceSensorEventPump::DidStartIfPossible();
    } else if (need_to_get_absolute_orientation_sensor_when_necessary_) {
      need_to_get_absolute_orientation_sensor_when_necessary_ = false;
      // When relative orientation sensor is not available,
      // DeviceOrientationEvent needs to fallback to use absolute orientation
      // sensor.
      GetSensor(&absolute_orientation_sensor_);
    }
  }
