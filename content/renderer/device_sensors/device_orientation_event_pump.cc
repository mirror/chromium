...
void DeviceOrientationEventPump::SendStopMessage() {
  ...
  // the absolute_orientation_sensor_.Start() is called for initializing
  // the absolute orientation sensor in the
...
bool DeviceOrientationEventPump::SensorsReady() const {
  if (!relative_orientation_sensor_.Ready() ||
      !absolute_orientation_sensor_.Ready()) {
    return false;
  }
