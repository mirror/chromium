...
TEST_F(DeviceMotionEventPumpTest, SensorInitializedButItsSharedBufferIsNot) {
  // Initialize the |sensor|, but not initialize its |shared_buffer|, this is
  // to test the state when |sensor| is already initialized but its
  ...
  EXPECT_FALSE(motion_pump()->CanStart());
  ...
  EXPECT_TRUE(motion_pump()->CanStart());
}
