 ...
 public:
  ...
  void SetRelativeOrientationSensorData(bool active,
    ...
    if (active) {
      ...
      relative_orientation_sensor_buffer_->reading.orientation_euler.x = x;
      relative_orientation_sensor_buffer_->reading.orientation_euler.y = y;
      relative_orientation_sensor_buffer_->reading.orientation_euler.z = z;
    } else {
  ...
  void SetAbsoluteOrientationSensorData(bool active,
    ...
    if (active) {
      ...
      absolute_orientation_sensor_buffer_->reading.orientation_euler.timestamp =
          (base::TimeTicks::Now() - base::TimeTicks()).InSecondsF();
      absolute_orientation_sensor_buffer_->reading.orientation_euler.x = x;
      absolute_orientation_sensor_buffer_->reading.orientation_euler.y = y;
      absolute_orientation_sensor_buffer_->reading.orientation_euler.z = z;
    } else {
...
TEST_F(DeviceOrientationEventPumpTest, RelativeOrientationSensorIsActive) {
  ...
  orientation_pump()->SetRelativeOrientationSensorData(
      true /* active */, 1 /* x: beta */, 2 /* y: gamma */, 3 /* z: alpha */);
  orientation_pump()->SetAbsoluteOrientationSensorData(
      true /* active */, 4 /* x: beta */, 5 /* y: gamma */, 6 /* z: alpha */);
  ...
  // available.
  EXPECT_EQ(3, static_cast<double>(received_data.alpha));
  EXPECT_TRUE(received_data.has_alpha);
  EXPECT_EQ(1, static_cast<double>(received_data.beta));
  EXPECT_TRUE(received_data.has_beta);
  EXPECT_EQ(2, static_cast<double>(received_data.gamma));
...
TEST_F(DeviceOrientationEventPumpTest, AbsoluteOrientationSensorIsActive) {
  ...
  EXPECT_EQ(6, static_cast<double>(received_data.alpha));
  EXPECT_TRUE(received_data.has_alpha);
  EXPECT_EQ(4, static_cast<double>(received_data.beta));
  EXPECT_TRUE(received_data.has_beta);
  EXPECT_EQ(5, static_cast<double>(received_data.gamma));
  EXPECT_TRUE(received_data.has_gamma);
...
TEST_F(DeviceOrientationEventPumpTest,
  ...
  orientation_pump()->SetRelativeOrientationSensorData(
      false /* active */, 1 /* x: beta */, 2 /* y: gamma */, 3 /* z: alpha */);
  orientation_pump()->SetAbsoluteOrientationSensorData(
      true /* active */, 4 /* x: beta */, 5 /* y: gamma */, 6 /* z: alpha */);
  orientation_pump()->StartFireEvent();
...
TEST_F(DeviceOrientationEventPumpTest, UpdateRespectsOrientationThreshold) {
  ...
  orientation_pump()->SetRelativeOrientationSensorData(
      true /* active */,
      1 + DeviceOrientationEventPump::kOrientationThreshold / 2.0 /* x: beta */,
      2 /* y: gamma */, 3 /* z: alpha */);
  listener()->set_did_change_device_orientation(false);

  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE,
      base::Bind(&DeviceOrientationEventPumpForTesting::StartFireEvent,
  ...
  orientation_pump()->SetRelativeOrientationSensorData(
      true /* active */,
      1 + DeviceOrientationEventPump::kOrientationThreshold /* x: beta */,
      2 /* y: gamma */, 3 /* z: alpha */);
  listener()->set_did_change_device_orientation(false);
  ...
  EXPECT_EQ(1 + DeviceOrientationEventPump::kOrientationThreshold,
            static_cast<double>(received_data.beta));
}
