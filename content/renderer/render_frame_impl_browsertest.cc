...
// RenderFrameObserver that issues interface requests in response to observing
...
// Expect that the specified InterfaceProvider |interfaces_request| has pending
// SourceAnnotatedInterfaceRequests from the |expected_sources|.
void ExpectPendingInterfaceRequestsFromSources(
  ...
  provider.Bind(std::move(interfaces_request));
  base::RunLoop().RunUntilIdle();
...
TEST_F(RenderFrameRemoteInterfacesTest,
       AvailableToChildFrameBeforeFirstCommittedLoad) {
  using InterfaceProviderRequest =
      service_manager::mojom::InterfaceProviderRequest;

  InterfaceProviderRequest interfaces_request_for_initial_empty_document;
  ...
  auto interfaces_request_for_first_document =
      child_frame->TakeLastInterfacesRequest();
  ASSERT_TRUE(interfaces_request_for_initial_empty_document.is_pending());
  ASSERT_TRUE(interfaces_request_for_first_document.is_pending());

  ExpectPendingInterfaceRequestsFromSources(
      std::move(interfaces_request_for_initial_empty_document),
      {SourceAnnotation(GURL(), kFrameEventOnDemandInterfaceRequest)});

  ExpectPendingInterfaceRequestsFromSources(
      std::move(interfaces_request_for_first_document),
      std::vector<SourceAnnotation>());
}

TEST_F(RenderFrameRemoteInterfacesTest,
       AvailableToMainFrameOfOpenedWindowBeforeFirstCommittedLoad) {
  using InterfaceProviderRequest =
      service_manager::mojom::InterfaceProviderRequest;
  ...
  auto interfaces_request_for_first_document =
      opened_window_main_frame->TakeLastInterfacesRequest();
  ASSERT_TRUE(interfaces_request_for_initial_empty_document.is_pending());
  ASSERT_TRUE(interfaces_request_for_first_document.is_pending());
...
TEST_F(RenderFrameRemoteInterfacesTest, ReplacedOnNavigation) {
  ...
  auto interfaces_request_for_first_document =
      GetMainRenderFrame()->TakeLastInterfacesRequest();
  ASSERT_TRUE(interfaces_request_for_first_document.is_pending());
  ...
  auto interfaces_request_for_second_document =
      GetMainRenderFrame()->TakeLastInterfacesRequest();
  ASSERT_TRUE(interfaces_request_for_second_document.is_pending());
  ...
  ExpectPendingInterfaceRequestsFromSources(
      std::move(interfaces_request_for_second_document),
...
TEST_F(RenderFrameRemoteInterfacesTest, ReusedOnSameDocumentNavigation) {
  ...
  auto interfaces_request = GetMainRenderFrame()->TakeLastInterfacesRequest();
  ASSERT_TRUE(interfaces_request.is_pending());
  ...
  auto interfaces_request_none =
      GetMainRenderFrame()->TakeLastInterfacesRequest();
  EXPECT_FALSE(interfaces_request_none.is_pending());

  requester.RequestInterfaceOnDemand();

  ExpectPendingInterfaceRequestsFromSources(
      std::move(interfaces_request),
      {SourceAnnotation(GURL(kTestFirstURL),
                        kFrameEventOnDemandInterfaceRequest),
       SourceAnnotation(GURL(kTestFirstURL),
