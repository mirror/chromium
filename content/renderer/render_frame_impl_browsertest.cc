...
TEST_F(RenderFrameImplTest, LocalChildFrameWasShown) {
  ...
  // Create and initialize a local child frame of the simulated OOPIF.
  RenderFrameImpl* subframe = RenderFrameImpl::Create(
  ...
  parent_web_frame->CreateLocalChild(blink::WebTreeScopeType::kDocument,
                                     subframe,
                                     subframe->blink_interface_registry_.get());
  subframe->in_frame_tree_ = true;
  subframe->Initialize();

  RenderFrameTestObserver observer(subframe);
