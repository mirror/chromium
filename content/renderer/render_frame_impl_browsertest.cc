...
TEST_F(RenderFrameImplTest, LoFiNotUpdatedOnSubframeCommits) {
  ...
  frame()->DidCommitProvisionalLoad(
      ...
      blink::WebWindowObjectInstance::kNewLocalDOMWindow);
  EXPECT_EQ(SERVER_LOFI_ON, frame()->GetPreviewsState());
...
// RenderFrameObserver that can be used to wait for the first commit in a frame.
...
// lifetime.
class ScopedNewFrameRemoteInterfaceVerifier {
 public:
  explicit ScopedNewFrameRemoteInterfaceVerifier(
      FrameCreationObservingRendererClient* frame_creation_observer,
      base::Optional<GURL> url_override_for_first_load = base::nullopt)
      : frame_creation_observer_(frame_creation_observer),
        url_override_for_first_load_(std::move(url_override_for_first_load)) {
    frame_creation_observer_->set_callback(
        base::Bind(&ScopedNewFrameRemoteInterfaceVerifier::OnFrameCreated,
                   base::Unretained(this)));
  }

  ~ScopedNewFrameRemoteInterfaceVerifier() {
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(ScopedNewFrameRemoteInterfaceVerifier);
};
...
TEST_F(RenderFrameRemoteInterfacesTest, ChildFrameAtFirstCommittedLoad) {
  ...
  for (const auto& test_case : kTestCases) {
    ...
    ScopedNewFrameRemoteInterfaceVerifier verifier(frame_creation_observer());
    ...
    ASSERT_NO_FATAL_FAILURE(
        verifier.ExpectNewFrameAndWaitForLoad(child_frame_url));

    const GURL initial_empty_url(kAboutBlankURL);
    ExpectPendingInterfaceRequestsFromSources(
        verifier.interface_request_for_initial_empty_document(),
    ...
    ExpectPendingInterfaceRequestsFromSources(
        verifier.interface_request_for_first_document(),
...
TEST_F(RenderFrameRemoteInterfacesTest,
  ...
  for (const auto& test_case : kTestCases) {
    ...
    ASSERT_NO_FATAL_FAILURE(
        verifier.ExpectNewFrameAndWaitForLoad(new_window_url));
...
//
// instance using Mojo services,
// make sure these work after the first navigation, so .
TEST_F(RenderFrameRemoteInterfacesTest,
  ...
  ASSERT_FALSE(verifier.interface_request_for_first_document().is_pending());
}
