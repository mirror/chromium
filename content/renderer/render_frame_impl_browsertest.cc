...
class FrameHostTestInterfaceRequestIssuer : public RenderFrameObserver {
 ...
 private:
  ...
  void DidCommitProvisionalLoad(bool is_new_navigation,
    ...
    LOG(ERROR) << "DCPL " << is_same_document_navigation;
...
class ScopedRemoteInterfaceVerifier {
 ...
 private:
  void OnFrameCreated(TestRenderFrame* frame) {
    ...
    // frame->GetWebFrame()->LoadHTMLString("foo", GURL(kTestFirstURL));
  }
...
// Expect that |remote_interfaces_| is bound before the first committed load in
// a child frame, and then re-bound on the first commit.
TEST_F(RenderFrameRemoteInterfacesTest, ChildFrameAtFirstCommittedLoad2) {
  ScopedRemoteInterfaceVerifier verifier(frame_creation_observer());
  std::string html = base::StringPrintf("<iframe srcdoc=\"%s\"></iframe>",
                                        "Hello");
  LoadHTMLWithUrlOverride(html.c_str(), kTestFirstURL);
  /*auto* child = main_frame->CreateChildFrame(
      main_frame->GetWebFrame(), blink::WebTreeScopeType::kDocument, "awesome",
      "awesome", blink::WebSandboxFlags::kNone, blink::ParsedFeaturePolicy(),
      blink::WebFrameOwnerProperties());
  ASSERT_TRUE(child);
  child->LoadHTMLString("Hello", GURL(kTestFirstURL));
  ASSERT_NO_FATAL_FAILURE(
      verifier.ExpectNewFrameAndWaitForLoad(GURL(kTestFirstURL)));
  verifier.VerifyInterfaceProviderRequestFromInitialEmptyDocument();
  verifier.VerifiyIntefaceProviderRequestFromFirstCommittedLoad();*/
}
