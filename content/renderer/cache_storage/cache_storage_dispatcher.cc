...
// Class to outlive WebCache, it's managed by CacheStorageDispatcher.
...
public:
 ...
 ~CacheRef() {
   DLOG(ERROR) << "Disposing CacheRef... (deleting callbacks).";
   ...
   if (dispatcher_)
     dispatcher_->OnCacheRefDestruction(cache_id_);
 }
 void set_cache_id(int32_t cache_id) {
   cache_id_ = cache_id;
 }
 int32_t pending_callbacks(){
   return pending_callbacks_;
 }
 void DispatchMatch(std::unique_ptr<blink::WebServiceWorkerCache::CacheMatchCallbacks> callbacks,
                     const blink::WebServiceWorkerRequest& request,
                     const blink::WebServiceWorkerCache::QueryParams& query_params) {
    int request_id = cache_match_callbacks_.Add(std::move(callbacks));
    DLOG(ERROR) << "Cache->DispatchMatch, request_id: " << request_id;
    pending_callbacks_++;
    cache_ptr_->Match(
  ...
  void CacheMatchCallback(int request_id,
    ...
    DLOG(ERROR) << "CacheMatchCallback, request_id: " << request_id;
    if (result->is_status() &&
        result->get_status() != CacheStorageError::kSuccess) {
      DLOG(ERROR) << " error: " << result->get_status();
  ...
  void DispatchMatchAll(std::unique_ptr<blink::WebServiceWorkerCache::CacheWithResponsesCallbacks> callbacks,
                        const blink::WebServiceWorkerRequest& request,
                        const blink::WebServiceWorkerCache::QueryParams& query_params) {
  ...
  void DispatchKeys(std::unique_ptr<blink::WebServiceWorkerCache::CacheWithRequestsCallbacks> callbacks,
                    const blink::WebServiceWorkerRequest& request,
                    const blink::WebServiceWorkerCache::QueryParams& query_params) {
  ...
  void DispatchBatch(
      std::unique_ptr<blink::WebServiceWorkerCache::CacheBatchCallbacks> callbacks,
      const blink::WebVector<blink::WebServiceWorkerCache::BatchOperation>& batch_operations) {
...
class CacheStorageDispatcher::WebCache : public blink::WebServiceWorkerCache {
 public:
  ...
  //}
  ~WebCache() override {
    DLOG(ERROR) << "Disposing WebCache... (deleting callbacks).";
    //ClearCallbacksMapWithErrors(&cache_keys_callbacks_);
    //ClearCallbacksMapWithErrors(&cache_match_callbacks_);
    //ClearCallbacksMapWithErrors(&cache_match_all_callbacks_);
    //ClearCallbacksMapWithErrors(&cache_batch_callbacks_);
  ...
  void DispatchMatch(std::unique_ptr<CacheMatchCallbacks> callbacks,
    ...
    cache_ref_->DispatchMatch(std::move(callbacks), std::move(request), std::move(query_params));
    //int request_id = cache_match_callbacks_.Add(std::move(callbacks));
    //DLOG(ERROR) << "Cache->DispatchMatch, request_id: " << request_id;
    //cache_ptr_->Match(
    //    FetchRequestFromWebRequest(request),
  ...
  void DispatchMatchAll(std::unique_ptr<CacheWithResponsesCallbacks> callbacks,
                        const blink::WebServiceWorkerRequest& request,
                        const QueryParams& query_params) override {
    cache_ref_->DispatchMatchAll(std::move(callbacks), std::move(request), std::move(query_params));
    //int request_id = cache_match_all_callbacks_.Add(std::move(callbacks));
    //cache_ptr_->MatchAll(
    //    FetchRequestFromWebRequest(request),
    //    QueryParamsFromWebQueryParams(query_params),
  ...
  void DispatchKeys(std::unique_ptr<CacheWithRequestsCallbacks> callbacks,
                    const blink::WebServiceWorkerRequest& request,
                    const QueryParams& query_params) override {
    cache_ref_->DispatchKeys(std::move(callbacks), std::move(request), std::move(query_params));
    //int request_id = cache_keys_callbacks_.Add(std::move(callbacks));
    //cache_ptr_->Keys(FetchRequestFromWebRequest(request),
    //                 QueryParamsFromWebQueryParams(query_params),
  ...
  void DispatchBatch(
      ...
      const blink::WebVector<blink::WebServiceWorkerCache::BatchOperation>& batch_operations) override {

    cache_ref_->DispatchBatch(std::move(callbacks), std::move(batch_operations));
    //int request_id = cache_batch_callbacks_.Add(std::move(callbacks));
    //std::vector<CacheStorageBatchOperation> operations;
    //operations.reserve(batch_operations.size());
    //for (size_t i = 0; i < batch_operations.size(); ++i) {
...
void CacheStorageDispatcher::OnCacheStorageOpenCallback(
  ...
  DLOG(ERROR) << "Start of Open Callback.";
  if (result->is_status() &&
      result->get_status() != CacheStorageError::kSuccess) {
    DLOG(ERROR) << " error";
    callbacks->OnError(result->get_status());
    ...
    CacheRef* c = new CacheRef(weak_factory_.GetWeakPtr(),std::move(result->get_cache()));
    std::unique_ptr<WebCache> web_cache(new WebCache(
        weak_factory_.GetWeakPtr(), std::move(result->get_cache()),
        c));

    //int32_t cache_id = web_caches_.Add(web_cache.get());
    int32_t cache_id = cache_refs_.Add(c);
    c->set_cache_id(cache_id);
    //cache_ptrs_.Add(web_cache.get());
    //web_cache->set_cache_id(cache_id);
  ...
  DLOG(ERROR) << "End of Open callback.";
}
...
void CacheStorageDispatcher::CacheStorageDeleteCallback(
  ...
  LOG(ERROR) << "Running CacheStorageDeleteCallback...";
  if (result == CacheStorageError::kSuccess) {
    LOG(ERROR) << " running success...";
    callbacks->OnSuccess();
  } else {
    callbacks->OnError(result);
  }

  LOG(ERROR) << " ran, removing from delete_callbacks_...";
...
void CacheStorageDispatcher::dispatchOpen(
    ...
    const blink::WebString& cacheName,
    service_manager::InterfaceProvider* provider) {
  DLOG(ERROR) << "Start of dispatchOpen.";
  ...
  cache_storage_ptr_->Open(
      ...
      base::BindOnce(&CacheStorageDispatcher::OnCacheStorageOpenCallback,
                     base::Unretained(this), CurrentWorkerId(), request_id,
                     base::TimeTicks::Now()));
  DLOG(ERROR) << "End of dispatchOpen.";
}
...
void CacheStorageDispatcher::dispatchDelete(
    ...
    const blink::WebString& cacheName,
    service_manager::InterfaceProvider* provider) {
  LOG(ERROR) << "Start of dispatch Delete.";
  int request_id = delete_callbacks_.Add(std::move(callbacks));

  LOG(ERROR) << "setting up interface..";
  SetupInterface(provider);
  LOG(ERROR) << "calling Delete Mojo method.";
...
void CacheStorageDispatcher::dispatchMatch(
  ...
  DLOG(ERROR) << "CacheStorage dispatchMatch";
...
void CacheStorageDispatcher::OnCacheRefDestruction(int cache_id) {
