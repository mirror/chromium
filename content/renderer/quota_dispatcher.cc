...
QuotaDispatcher::QuotaDispatcher(
    ...
    : next_request_id_(0), main_thread_task_runner_(main_thread_task_runner) {
  g_quota_dispatcher_tls.Pointer()->Set(this);

  auto request = mojo::MakeRequest(&quota_messages_);
...
int QuotaDispatcher::GenerateRequestID(int thread_id) {
...
void QuotaDispatcher::QueryStorageUsageAndQuota(
  ...
  DCHECK(callback);
  int request_id = GenerateRequestID(CurrentWorkerId());
