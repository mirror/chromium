...
void RenderThreadImpl::CompositingModeFallbackToSoftware() {
  LOG(ERROR) << "Renderer " << this << " hears about mode switch";
  if (gpu_channel_) {
...
void RenderThreadImpl::RequestNewLayerTreeFrameSink(
...
#if defined(USE_AURA)
  if (!is_gpu_compositing_disabled_ && IsRunningInMash()) {
  ...
  if (is_gpu_compositing_disabled_) {
    LOG(ERROR) << "Renderer Compositor " << this << ": software";
    DCHECK(!layout_test_mode());
    frame_sink_provider_->CreateForWidget(routing_id, std::move(sink_request),
                                          std::move(client));
  ...
  scoped_refptr<gpu::GpuChannelHost> gpu_channel_host =
      EstablishGpuChannelSync();
  if (!gpu_channel_host) {
    LOG(ERROR) << "Renderer Compositor " << this << ": retry";
  ...
  LOG(ERROR) << "Renderer Compositor " << this << ": gpu";
