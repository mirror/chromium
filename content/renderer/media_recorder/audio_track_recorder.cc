...
void AudioTrackRecorder::PcmAudioEncoder::EncodeAudio(
  ...
  buffer.resize(input_bus->frames() * input_bus->channels());
  input_bus->ToInterleaved<media::SignedInt16SampleTypeTraits>(
      input_bus->frames(), &buffer[0]);
  ...
  for (size_t i = 0; i < buffer.size(); ++i) {
    uint16_t le_sample = base::ByteSwapToLE16(buffer[i]);
