...
class AudioTrackRecorderTest : public TestWithParam<ATRTestParams> {
 public:
  ...
  std::unique_ptr<media::AudioBus> GetFirstSourceAudioBus() {
    ...
    cache_bus->ToInterleaved<media::SignedInt16SampleTypeTraits>(
  ...
  void OnEncodedAudio(const media::AudioParameters& params,
    ...
    if (codec_ == AudioTrackRecorder::CodecId::OPUS) {
      ...
      for (size_t b = 0; b + 1 < encoded_data->size() &&
                         first_source_cache_pos_ < first_source_cache_.size();
           b += sizeof(int16_t), ++first_source_cache_pos_) {
        unsigned char lsb = (*encoded_data)[b];
        unsigned char msb = (*encoded_data)[b + 1];
        int16_t sample = lsb | msb << 8;
        ASSERT_EQ(sample, first_source_cache_[first_source_cache_pos_]);
      }
    }
  ...
  std::vector<int16_t> first_source_cache_;
