...
void RenderWidget::Init(const ShowCallback& show_callback,
  ...
  if (base::FeatureList::IsEnabled(features::kMojoInputMessages)) {
    widget_input_handler_ = base::MakeUnique<WidgetInputHandlerImpl>(
        weak_ptr_factory_.GetWeakPtr(),
        RenderThreadImpl::current()->GetRendererScheduler());
  }
...
void RenderWidget::ObserveGestureEventAndResult(
  ...
  } else if (widget_input_handler_) {
    widget_input_handler_->ObserveGestureEvent(gesture_event, scroll_result);
  }
}
...
void RenderWidget::GetWidgetInputHandler(
  ...
  widget_input_handler_->AddInterface(std::move(request));
}
