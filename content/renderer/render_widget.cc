...
void RenderWidget::Resize(const ResizeParams& params) {
  ...
  LOG(ERROR) << "DSF changed:" << device_scale_factor_ << " vs " << screen_info_.device_scale_factor;
  if (device_scale_factor_ != screen_info_.device_scale_factor) {
    device_scale_factor_ = screen_info_.device_scale_factor;
    OnDeviceScaleFactorChanged();
  }
...
blink::WebView* RenderWidget::GetWebView() {
  blink::WebFrameWidget* frame_widget = GetFrameWidget();
  return frame_widget ? frame_widget->LocalRoot()->View() : nullptr;
}
