...
void RenderWidget::SetLocalSurfaceIdForAutoResize(
  ...
  if (content_source_id != current_content_source_id_) {
    LOG(ERROR) << "HERE " << content_source_id << " "
...
void RenderWidget::DidNavigate() {
  ...
  // Ack the resize so we can get a new LocalSurfaceId. The old resize will not
  // get naturally acked because we won't commit and draw.
  DidResizeOrRepaintAck();
}
