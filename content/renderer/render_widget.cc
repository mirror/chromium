...
void RenderWidget::FocusChangeComplete() {
  blink::WebFrameWidget* frame_widget = GetWebFrameWidget();
  if (!frame_widget)
    return;
...
blink::WebFrameWidget* RenderWidget::GetWebFrameWidget() const {
  ...
  if (!web_widget->IsWebFrameWidget()) {
    ...
    // WebViewImpl loses its WebViewFrameWidget but sometimes we recieve IPCs
...
void RenderWidget::TransferActiveWheelFlingAnimation(
  ...
  if (auto* frame_widget = GetWebFrameWidget())
    frame_widget->TransferActiveWheelFlingAnimation(params);
}
...
blink::WebLocalFrame* RenderWidget::GetFocusedWebLocalFrameInWidget() const {
  if (auto* frame_widget = GetWebFrameWidget())
    frame_widget->FocusedWebLocalFrameInWidget();
  return nullptr;
}
