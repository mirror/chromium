...
void RenderViewImpl::Initialize(
  ...
  webview_ =
      WebView::Create(this, is_hidden() ? blink::kWebPageVisibilityStateHidden
...
void RenderViewImpl::FocusedNodeChanged(const WebNode& fromNode,
  ...
  if (!fromNode.IsNull())
    previous_frame = RenderFrameImpl::FromWebFrame(fromNode.GetDocument().GetFrame());
...
void RenderViewImpl::didChangeIcon(WebLocalFrame* frame,
  ...
  for (size_t i = 0; i < icon_urls.size(); i++) {
    ...
    urls.push_back(FaviconURL(
        icon_urls[i].GetIconURL(), ToFaviconType(icon_urls[i].IconType()), sizes));
  }
...
void RenderViewImpl::DidStopLoadingIcons() {
  int icon_types = WebIconURL::kTypeFavicon | WebIconURL::kTypeTouchPrecomposed |
      WebIconURL::kTypeTouch;
...
void RenderViewImpl::OnDiscardInputEvent(
  ...
  std::unique_ptr<InputEventAck> ack(
      new InputEventAck(InputEventAckSource::MAIN_THREAD, input_event->GetType(),
                        INPUT_EVENT_ACK_STATE_NOT_CONSUMED));
