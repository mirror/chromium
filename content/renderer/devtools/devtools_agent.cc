...
class DevToolsAgent::IOSession : public mojom::DevToolsSession {
 public:
  ...
  void InspectElement(const gfx::Point& point) override { DCHECK(false); }
...
void DevToolsAgent::AttachDevToolsSession(
  ...
  session_hosts_[session_id].Bind(std::move(host));
...
bool DevToolsAgent::IsAttached() {
  return !!session_hosts_.size();
}

void DevToolsAgent::DetachAllSessions() {
  for (auto& it : session_hosts_)
