...
class DevToolsAgent::SessionProxy : public mojom::DevToolsSession {
 public:
  ...
  void BindInterface(mojom::DevToolsSessionAssociatedRequest session_request) {
...
void DevToolsAgent::AttachDevToolsSession(
  ...
  if (reattach_state.has_value()) {
    fprintf(stderr, "Here we reattach\n");
  ...
  std::unique_ptr<SessionProxy, base::OnTaskRunnerDeleter> proxy(
      new SessionProxy(session_id, base::ThreadTaskRunnerHandle::Get(),
                       weak_factory_.GetWeakPtr()),
      base::OnTaskRunnerDeleter(io_task_runner));
  io_task_runner->PostTask(FROM_HERE,
      base::BindOnce(&DevToolsAgent::SessionProxy::BindInterface,
                     base::Unretained(proxy.get()),
                     std::move(session_request)));
  session_proxies_.emplace(session_id, std::move(proxy));
  session_hosts_[session_id].Bind(std::move(session_host_info));
  session_hosts_[session_id].set_connection_error_handler(base::BindOnce(
