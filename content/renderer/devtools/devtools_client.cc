...
void DevToolsClient::DidClearWindowObject() {
  if (!api_script_.empty()) {
    // Postpone ExecuteJavaScript to make sure it executes *after*
    // DidCommitProvisionalLoad IPC has been sent to the browser (i.e. after the
    // browser is aware of the new origin the scripts should be able to access).
