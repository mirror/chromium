...
void SpeechRecognitionDispatcher::AbortAllRecognitions() {
  SpeechRecognitionHost()->AbortAllRequests(routing_id());
}
...
void SpeechRecognitionDispatcher::Start(
  ...
  SpeechRecognitionHost()->StartRequest(std::move(msg_params));
}
...
void SpeechRecognitionDispatcher::Stop(
  ...
  if (recognizer_client_ != recognizer_client || !HandleExists(handle))
    return;
  SpeechRecognitionHost()->StopCaptureRequest(routing_id(),
...
void SpeechRecognitionDispatcher::Abort(
  ...
  if (recognizer_client_ != recognizer_client || !HandleExists(handle))
    return;
  SpeechRecognitionHost()->AbortRequest(routing_id(),
                                        GetOrCreateIDForHandle(handle));
}
