...
#include "content/renderer/net/cors_url_loader_factory.h"
...
class CORSURLLoaderTest : public testing::Test {
 ...
 protected:
  void CreateLoaderAndStart(ResourceRequest& request) {
    if (false) {
      // This call works, but I should use CORSUrlLoaderFactory instead
      CORSURLLoader::CreateAndStart(
          &network_simulator_, mojo::MakeRequest(&url_loader_), 0, 0, 0,
          request, client_simulator_.CreateInterfacePtr(),
          TRAFFIC_ANNOTATION_FOR_TESTS);
    } else {
      // This compiles and looks like it works on first glance, but the test
      // fails. From what I observe during debugging, the later call to
      // FlushForTesting causes memory in the CORSURLLoader to be corrupted
      // (e.g. the stored resource_request). Hence it seems to me I am doing
      // something wrong with regards to pointer/memory/mojo here, but I can't
      // figure out what. I cannot rule out, however, that the bug exists in
      // both code paths and only manifests itself in this one.
      CORSURLLoaderFactory::CreateAndBind(
          std::move(network_url_loader_factory_),
    ...
    network_simulator_.factory_ptr().FlushForTesting();
  }
...
TEST_F(CORSURLLoaderTest, CrossOriginRequestFetchRequestModeCORSNoCORSHeader) {
  ...
  printf(
      "1 CrossOriginRequestFetchRequestModeCORSNoCORSHeader Address of "
      "request_.url() is %p\n",
      (void*)&request.url);
  printf(
      "1 CrossOriginRequestFetchRequestModeCORSNoCORSHeader Address of "
      "request_.url.spec() is %p\n",
      (void*)&request.url.spec());
  printf(
      "1 CrossOriginRequestFetchRequestModeCORSNoCORSHeader value of "
      "request_.url.spec() : %s\n",
  ...
  printf(
      "2 CrossOriginRequestFetchRequestModeCORSNoCORSHeader Address of "
      "request_.url() is %p\n",
      (void*)&request.url);
  printf(
      "2 CrossOriginRequestFetchRequestModeCORSNoCORSHeader Address of "
      ...
      (void*)&request.url.spec());
  printf(
      "2 CrossOriginRequestFetchRequestModeCORSNoCORSHeader value of "
      "request_.url.spec() : %s\n",
      request.url.spec().c_str());
