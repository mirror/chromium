 ...
 protected:
  void CreateLoaderAndStart(ResourceRequest& request) {
    if (false) {
      ...
      // This compiles as looks to work on first glance, but the test fails.
      // From what I observe during debuging, the later call to FlushForTesting
      // causes memory in the CORSURLLoader to be corrupted (e.g. the stored
      // resource_request). Hence it seems to me I am doing something wrong with
      // regards to pointer/memory/mojo here, but I can't figure out what.
      // I cannot rule out, however, that the bug exists in both code paths and
      // only manifests itself in this one.
