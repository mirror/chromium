...
void CORSURLLoader::CreateAndStart(
  ...
  mojo::MakeStrongAssociatedBinding(
      base::MakeUnique<CORSURLLoader>(
          ...
          net::MutableNetworkTrafficAnnotationTag(traffic_annotation), factory),
      std::move(request));
}
...
bool CORSURLLoader::IsSameOrigin() {
  return security_origin_.IsSameOriginWith(url::Origin(request_.url));
}
...
void CORSURLLoader::HandleActualResponse(
  ...
  if (head.was_fetched_via_service_worker) {
    ...
    // kWebServiceWorkerResponseTypeOpaqueRedirect as well?
