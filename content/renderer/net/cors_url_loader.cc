...
bool CORSURLLoader::IsSameOriginOrWhitelisted(const GURL& url) {
...
bool CORSURLLoader::IsAllowedRedirect(const GURL& url) const {
  ...
  // do the same here?
  return !cors_flag_ && security_origin_.IsSameOriginWith(url::Origin(url));
}
...
bool CORSURLLoader::NeedsPreflight() {
  ...
  if (should_ignore_preflight_cache ||
      !CrossOriginPreflightResultCache::Shared().CanSkipPreflight(
          security_origin_, request_.url, request_.fetch_credentials_mode,
...
void CORSURLLoader::DispatchRequest(const ResourceRequest& request,
  ...
  if (cors_flag_) {
    net::HttpRequestHeaders request_headers;
    request_headers.AddHeadersFromString(new_request.headers);
    request_headers.SetHeader(net::HttpRequestHeaders::kOrigin,
                              security_origin_.Serialize());
    if (security_origin_.suborigin() != "") {
      // TODO: Extract constant:
      request_headers.SetHeader(net::HttpRequestHeaders::kOrigin, "Suborigin");
    }

    new_request.headers = request_headers.ToString();
  }
  ...
  network_loader_factory_->CreateLoaderAndStart(
      std::move(url_loader_request), routing_id_, request_id, options_,
      new_request, std::move(client_ptr),
...
void CORSURLLoader::HandlePreflightResponse(const ResourceResponseHead& head) {
  ...
  if (access_status != CrossOriginAccessControl::kAccessAllowed) {
    DidFailAccessControlCheck(
        ...
        CrossOriginAccessControl::AccessControlErrorString(
            access_status, head, security_origin_,
  ...
  if (IsMainThread()) {
    ...
    CrossOriginPreflightResultCache::Shared().AppendEntry(
        security_origin_, request_.url, std::move(result));
  }
...
void CORSURLLoader::HandleActualResponse(
  ...
  if (head.was_fetched_via_service_worker) {
    ...
    if (request_.fetch_request_mode != FETCH_REQUEST_MODE_CORS &&
        head.response_type_via_service_worker ==
            blink::mojom::FetchResponseType::kOpaque) {
      DidFailAccessControlCheck(
          CrossOriginAccessControl::AccessControlErrorString(
              CrossOriginAccessControl::kInvalidResponse, head,
              security_origin_, request_.fetch_request_context_type));
      return;
    }
  ...
  if (IsCORSEnabled() && cors_flag_) {
    ...
    if (cors_status != CrossOriginAccessControl::kAccessAllowed) {
      ...
      DidFailAccessControlCheck(
          CrossOriginAccessControl::AccessControlErrorString(
              cors_status, head, security_origin_,
              request_.fetch_request_context_type));
      return;
    }
  }
...
*/
  ...
  if (request_.fetch_redirect_mode == FetchRedirectMode::MANUAL_MODE) {
    ...
    // not related to CORS so for now it should stain in
  ...
  if (redirect_status != CrossOriginAccessControl::kRedirectSuccess) {
    DidFailAccessControlCheck("Redirect from '" + request_.url.spec() +
                              ...
                              CrossOriginAccessControl::RedirectErrorString(
    ...
    // flag is set.
    CrossOriginAccessControl::AccessStatus cors_status =
        CrossOriginAccessControl::CheckAccess(redirect_info.new_url, head,
                                              request_.fetch_credentials_mode,
                                              security_origin_);
    if (cors_status != CrossOriginAccessControl::kAccessAllowed) {
