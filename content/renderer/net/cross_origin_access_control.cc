...
std::string CreateAccessControlRequestHeadersHeader(
  ...
  for (net::HttpRequestHeaders::Iterator it(headers); it.GetNext();) {
    ...
    if (IsForbiddenHeaderName(it.name()))
      continue;
...
AccessStatus CheckAccess(const GURL& request_url,
  ...
  if (allow_origin_header_value == "*") {
    ...
    // TODO(hintzed): I originally used this check, but it fails on both origins
    // are "null" - which makes sense to me, but fails the
    // fetch/api/redirect/redirect-origin.html test. Are we sure that this test
    // should compare the origin strings?
    //
    //  } else if (!security_origin.IsSameOriginWith(
    //                 url::Origin(GURL(allow_origin_header_value)))) {
  } else if (allow_origin_header_value != security_origin.Serialize()) {
