...
const char* CrossOriginAccessControl::ALLOWED_CROSS_ORIGIN_RESPONSE_HEADERS[] =
    {"cache-control", "content-language", "content-type",
     "expires",       "last-modified",    "pragma"};

bool CrossOriginAccessControl::IsOnAccessControlResponseHeaderWhitelist(
    const std::string& name) {
  for (const char* header : ALLOWED_CROSS_ORIGIN_RESPONSE_HEADERS) {
...
static std::string CreateAccessControlRequestHeadersHeader(
    const net::HttpRequestHeaders& headers) {
  std::vector<std::string> filtered_headers;

  for (net::HttpRequestHeaders::Iterator it(headers); it.GetNext();) {
    // Exclude CORS-safelisted headers.
    if (CrossOriginAccessControl::IsCORSSafelistedHeader(it.name(), it.value()))
      continue;
...
ResourceRequest CrossOriginAccessControl::CreateAccessControlPreflightRequest(
  ...
  if (request.is_external_request_)
  ...
  if (!request.headers.empty()) {
    ...
    if (!access_control_request_headers.empty()) {
      preflight_headers.SetHeader(ACCESS_CONTROL_REQUEST_HEADERS,
...
static std::string GetHeaderValue(const ResourceResponseHead& head,
                                  std::string name) {
  std::string value;
...
std::string CrossOriginAccessControl::AccessControlErrorString(
  ...
  switch (status) {
    ...
    case kSubOriginMismatch:
      return "The 'Access-Control-Allow-Suborigin' header has a value '" +
             GetHeaderValue(head, ACCESS_CONTROL_ALLOW_SUBORIGIN) +
    ...
    case kMultipleAllowOriginValues:
      return "The 'Access-Control-Allow-Origin' header contains multiple "
             "values '" +
             GetHeaderValue(head, ACCESS_CONTROL_ALLOW_ORIGIN) +
    ...
    case kDisallowCredentialsNotSetToTrue:
      return "The value of the 'Access-Control-Allow-Credentials' header in "
             "the response is '" +
             GetHeaderValue(head, ACCESS_CONTROL_ALLOW_CREDENTIALS) +
...
CrossOriginAccessControl::CheckPreflight(const ResourceResponseHead& head) {
...
CrossOriginAccessControl::CheckExternalPreflight(
  ...
  if (!head.headers->GetNormalizedHeader(
...
std::string CrossOriginAccessControl::PreflightErrorString(
  ...
  switch (status) {
    ...
    case kPreflightInvalidAllowExternal: {
      return "The 'Access-Control-Allow-External' header in the preflight "
             ...
             GetHeaderValue(head, ACCESS_CONTROL_ALLOW_EXTERNAL) +
...
void CrossOriginAccessControl::ExtractCorsExposedHeaderNamesList(
  ...
  head.headers->GetNormalizedHeader(
      CrossOriginAccessControl::ACCESS_CONTROL_EXPOSE_HEADERS, &expose_headers);

  ParseAccessControlExposeHeadersAllowList(expose_headers, header_set);
}

CrossOriginAccessControl::RedirectStatus
CrossOriginAccessControl::CheckRedirectLocation(const GURL& redirect_url) {
...
std::string CrossOriginAccessControl::RedirectErrorString(
    const RedirectStatus status,
...
std::string CrossOriginAccessControl::ListOfCORSEnabledURLSchemes() {
  std::string list;
