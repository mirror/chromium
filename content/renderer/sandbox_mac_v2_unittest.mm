...
// These tests check that the V2 sandbox compiles, initializes, and correctly
// enforces resource access on all platforms. Note that with the exception of
// certain controlled locations, such as a dummy log file, these tests cannot
// check that write access to system files is blocked. These tests run on
// developers' machines and bots, so if the write access goes through, that
// machine could be corrupted.
