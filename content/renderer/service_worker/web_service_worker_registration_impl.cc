...
WebServiceWorkerRegistrationImpl::CreateForServiceWorkerGlobalScope(
  ...
  // |impl| destruction must be triggered by executing both
  // DetachAndMaybeDestroy() and OnConnectionError(), please see comments about
  // enum class LifecycleState definintion for details. But OnConnectionError()
  // is impossible to be run before BindRequest(), so |impl| should be valid
  // before BindRequest(), using base::Unretained here is safe.
