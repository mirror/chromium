...
struct ServiceWorkerProviderContext::ControlleeState {
  ...
  // For service worker clients. Tracks mapping between registration id and
  // JavaScript service worker registration object against current client
  // context.
...
void ServiceWorkerProviderContext::SetController(
  ...
  state->controller = ServiceWorkerHandleReference::Adopt(
      controller->Clone(), dispatcher->thread_safe_sender());
