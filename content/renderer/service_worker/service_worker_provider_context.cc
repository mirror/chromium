...
ServiceWorkerHandleReference* ServiceWorkerProviderContext::controller() {
  ...
  LOG(ERROR) << "** Controllee->controller() "
             << controllee_state_->controller.get();
...
void ServiceWorkerProviderContext::SetController(
  ...
  LOG(ERROR) << "### SET CONTROLLER() ";
  state->controller = dispatcher->Adopt(controller_info->object_info->Clone());
  ...
  if (ServiceWorkerUtils::IsServicificationEnabled() &&
    ...
    if (state->controller && controller_info->endpoint) {
      state->controller_connector =
          base::MakeRefCounted<ControllerServiceWorkerConnector>(
              container_host_.get(), std::move(controller_info->endpoint));
    } else {
      state->controller_connector = nullptr;
    }
    if (should_notify_controllerchange)
  ...
  if (state->web_service_worker_provider) {
    state->web_service_worker_provider->SetController(
        std::move(controller_info->object_info), state->used_features,
