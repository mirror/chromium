...
class CONTENT_EXPORT ServiceWorkerContextClient
 ...
 public:
  ...
  // idle, the event dispatch could be delayed until it starts to run again
  // (it's when the browser process dispatches events after getting idle).
  void DispatchFetchEventWhenRunning(
 ...
 private:
  ...
  FRIEND_TEST_ALL_PREFIXES(ServiceWorkerContextClientTest,
                           DispatchFetchEventWhenRunning_NotIdle);
