...
class CONTENT_EXPORT ServiceWorkerTimeoutTimer {
 public:
  ...
  // next time when the set of inflight events become empty in EndEvent().
 ...
 private:
  ...
  // Triggers idle timer if |zero_idle_timer_delay_| is true.
  ...
  // True if the idle callback should be fired immediately after all inflight
  // events finish.
