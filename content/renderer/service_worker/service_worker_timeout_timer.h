...
// ServiceWorkerTimeoutTimer manages two types of timeouts: the idle timeout and
...
class CONTENT_EXPORT ServiceWorkerTimeoutTimer {
 ...
 private:
  // S13nServiceWorker: updates the internal states and fires timeout
  // callbacks if any.
  // Non-S13nServiceWorker: does nothing.
  void UpdateStatus();

  // For event timeouts. Contains only the inflight events.
  ...
  // For idle timeouts. The time |kIdleDelay| after the service worker started
  // having no inflight events, or null if there are inflight events.
