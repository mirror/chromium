...
class CONTENT_EXPORT ServiceWorkerTimeoutTimer {
 public:
  ...
  // Sets the idle timeout delay to zero. If it's idle when
  // TriggerIdleTimerImmediately() is called, it immediately triggers the idle
  // callback. If it's not, the last EndEvent() will synchronously trigger the
  // idle callback.
  void TriggerIdleTimerImmediately();
 ...
 private:
  ...
  bool MaybeTriggerTimeoutTimerImmediately();
  ...
  bool trigger_idle_timer_immediately_ = false;
