...
class CONTENT_EXPORT ServiceWorkerTimeoutTimer {
 public:
  ...
  // finished. If there are pending tasks queued by PushPendingTaskWhenIdle(),
  // they will run in StartEvent() synchronouslly for keeping the order.
  ...
  // Pushes a task which is expected to run after any event starts again. The
  // tasks will run at the next StartEvent() call. This should be called during
  // it's idle (IsIdle() returns true).
