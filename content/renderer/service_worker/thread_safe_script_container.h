...
// startup. The IO thread adds scripts to the container, and the worker thread
// takes the scripts. This class uses explicit synchronization because it needs
// to support synchronous importScripts() from the worker thread.
// This class is RefCounted because there is no ordering guarantee of lifetime
// between WebServiceWorkerInstalledScriptsManagerImpl and its Internal
// class. WebSWInstalledScriptsManagerImpl is destroyed earlier than Internal if
// the worker is terminated before all scripts are streamed, and Internal is
// destroyed earlier if all of scripts are received before finishing script
// evaluation.
class CONTENT_EXPORT ThreadSafeScriptContainer
 ...
 private:
  ...
  // is added, or OnAllDataAdded is called. This waits on the worker thread, and
