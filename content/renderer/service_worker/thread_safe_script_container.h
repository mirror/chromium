...
class CONTENT_EXPORT ThreadSafeScriptContainer
 ...
 public:
  ...
  // ScriptStatus shows the current state of the script:
  // - |kReceived| : the script data has been received.
  // - |kTaken| : the script data has been received but it has already been
  //              taken.
  // - |kFailed| : receiving the script has failed.
  // - |kPending| : the script data has not been received yet.
  enum class ScriptStatus { kReceived, kTaken, kFailed, kPending };
  ...
  // Resets the entry to wait. After calling this, ScriptStatus for the
