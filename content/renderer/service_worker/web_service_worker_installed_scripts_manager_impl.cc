...
class Receiver {
 public:
  ...
  Receiver(mojo::ScopedDataPipeConsumerHandle handle, size_t total_size)
      : handle_(std::move(handle)),
        watcher_(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL),
        remaining_bytes_(total_size) {}
  ...
  bool has_all_data_reached() { return remaining_bytes_ == 0; }
...
class Internal : public mojom::ServiceWorkerInstalledScriptsManager {
 public:
  ...
  void OnScriptReceived(mojom::ServiceWorkerScriptInfoPtr script_info) {
    ...
    DCHECK(receivers);
    if (!receivers->body()->has_all_data_reached() ||
        !receivers->meta_data()->has_all_data_reached()) {
