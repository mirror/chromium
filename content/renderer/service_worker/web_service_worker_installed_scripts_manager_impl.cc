...
class Receiver {
 public:
  ...
  void OnReadable(MojoResult /* unused */) {
    ...
    if (available > 0)
      chunks_.emplace_back(static_cast<const char*>(buffer), available);
...
class Receivers {
 public:
  ...
  void Start(base::Closure callback) {
    base::RepeatingClosure wait_all_closure = base::BarrierClosure(2, callback);
...
WebServiceWorkerInstalledScriptsManagerImpl::ThreadSafeScriptContainer::
    ...
    : lock_(), waiting_cv_(&lock_), is_completed_(false) {}
...
WebServiceWorkerInstalledScriptsManagerImpl::GetRawScriptData(
  ...
  std::unique_ptr<RawScriptData> data = script_container_->Take(script_url);
