...
class Receiver {
 public:
  ...
  blink::WebVector<blink::WebVector<char>> TakeChunks() {
 ...
 private:
  ...
  std::vector<blink::WebVector<char>> chunks_;
};
...
class Internal : public mojom::ServiceWorkerInstalledScriptsManager {
 public:
  ...
  // Called on the IO thread when script data is ready.
...
WebServiceWorkerInstalledScriptsManagerImpl::GetRawScriptData(
  ...
  if (!script_container_->Exists(script_url)) {
    ...
    // It could fail due to an error on Mojo pipes.
    if (!success)
      return nullptr;
