...
struct ServiceWorkerContextClient::WorkerContextData {
  ...
  // S13nServiceWorker
  // This should be destructed before |timeout_timer| since callbacks of the
  // events dispatched through this pipe may be silently descarded by
  // disconnecting the pipe before running the callbacks.
...
bool ServiceWorkerContextClient::RequestedTermination() const {
  // ServiceWorkerTimeoutTimer::IsIdle() returns true if
  // ServiceWorkerContextClient::OnIdle() has been called. It means termination
  // has been requested to the browser process.
  return context_->timeout_timer->IsIdle();
}
