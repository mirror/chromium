...
// destructed on the worker thread, even if the worker thread dies before the
// callbacks can run. If the worker thread dies before the callback is run, this
// gets notified in WillStopCurrentWorkerThread() and deletes itself and the
// callback on the worker thread, otherwise, it runs the callback on the worker
// thread as normal and deletes itself.
//
