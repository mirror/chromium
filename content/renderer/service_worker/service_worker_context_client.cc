...
// S13nServiceWorker: Creates a callback which takes an |event_id|, which calls
// the given event's callback with ABORTED status and removes it from |map|.
//
// If S13nServiceWorker is not enabled, returns a no-op callback which takes an
// |event_id|.
template <typename MapType>
base::OnceCallback<void(int /* event_id */)> CreateAbortCallback(MapType* map) {
  if (!ServiceWorkerUtils::IsServicificationEnabled())
    return base::BindOnce([](int /* event_id */) {});
