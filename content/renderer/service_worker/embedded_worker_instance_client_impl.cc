...
EmbeddedWorkerInstanceClientImpl::StartWorkerContext(
    ...
    blink::mojom::WorkerContentSettingsProxyPtr content_settings_proxy,
    service_manager::mojom::InterfaceProviderPtr interface_provider) {
  std::unique_ptr<blink::WebServiceWorkerInstalledScriptsManagerInfo>
      installed_scripts_manager_info;
  std::vector<GURL> installed_scripts_urls;
  ...
  if (ServiceWorkerUtils::IsScriptStreamingEnabled() &&
    ...
    installed_scripts_manager_info =
        std::make_unique<blink::WebServiceWorkerInstalledScriptsManagerInfo>();
    installed_scripts_manager_info->installed_scripts_urls =
        std::move(installed_scripts_info->installed_urls);
    installed_scripts_manager_info->manager_request =
        installed_scripts_info->manager_request.PassMessagePipe();
    installed_scripts_manager_info->manager_host_ptr =
        installed_scripts_info->manager_host_ptr.PassHandle();
    DCHECK(installed_scripts_manager_info->manager_request.is_valid());
    DCHECK(installed_scripts_manager_info->manager_host_ptr.is_valid());
  }

  auto wrapper = std::make_unique<WorkerWrapper>(
      blink::WebEmbeddedWorker::Create(
          std::move(context_client), std::move(installed_scripts_manager_info),
