...
base::OnceClosure CreateCallbackWithCalledFlag(bool* out_is_called) {
  return base::BindOnce([](bool* out_is_called) { *out_is_called = true; },
                        out_is_called);
}
...
TEST_F(ServiceWorkerContextClientTest, DispatchFetchEventWhenRunning_NotIdle) {
  ...
  auto request = std::make_unique<ResourceRequest>();
  request->url = expected_url;
  context_client->DispatchFetchEventWhenRunning(
      *request, nullptr /* preload_handle */, std::move(fetch_callback_ptr),
      base::BindOnce(
          [](blink::mojom::ServiceWorkerEventStatus, base::Time) {}));
  task_runner()->RunUntilIdle();

  EXPECT_FALSE(context_client->IsIdle());
  ASSERT_EQ(1u, mock_proxy.fetch_events().size());
...
TEST_F(ServiceWorkerContextClientTest,
       DispatchFetchEventWhenRunning_IdleAndDie) {
  EnableServicification();
  ContextClientPipes pipes;
  std::unique_ptr<ServiceWorkerContextClient> context_client =
      CreateContextClient(&pipes);
  ...
  // mojom::ControllerServiceWorker should be queued.
  {
    mojom::ServiceWorkerFetchResponseCallbackPtr fetch_callback_ptr;
...
TEST_F(ServiceWorkerContextClientTest,
       DispatchFetchEventWhenRunning_IdleAndWakeUp) {
  EnableServicification();
  ContextClientPipes pipes;
  std::unique_ptr<ServiceWorkerContextClient> context_client =
      CreateContextClient(&pipes);
  ...
  // Dispatching another event from mojom::ServiceWorkerEventDispatcher wakes up
  // the context client.
  {
    mojom::ServiceWorkerFetchResponseCallbackPtr fetch_callback_ptr;
