  ...
  if (image->isTextureBacked()) {
    if (context_provider) {
      if (!gl_helper_) {
        ...
        SetGLHelper(context_provider_impl->context_provider());
      }
...
void CanvasCaptureHandler::SetGLHelper(viz::ContextProvider* context_provider) {
  DCHECK(main_render_thread_checker_.CalledOnValidThread());
  DCHECK(context_provider);
  DCHECK(!gl_helper_);
  context_provider->SetLostContextCallback(base::Bind(
      &CanvasCaptureHandler::OnContextLost, weak_ptr_factory_.GetWeakPtr()));
