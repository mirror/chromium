...
void CanvasCaptureHandler::DeliverNewFrame(const SkImage* image) {
  ...
  if (image->isTextureBacked()) {
    ...
    gl_helper_->ReadbackTextureAsync(
        ...
        media::BindToCurrentLoop(base::Bind(
            &CanvasCaptureHandler::ReadPixelsAsync,
            weak_ptr_factory_.GetWeakPtr(), temp_argb_frame, image_size,
            image->isOpaque(), surface_origin != kTopLeft_GrSurfaceOrigin)));
    return;
  }
  ...
  if (image->readPixels(
          image_info, temp_argb_frame->visible_data(VideoFrame::kYPlane),
...
void CanvasCaptureHandler::ReadPixelsAsync(
  ...
  DeliverAsYUVFrame(isOpaque, flip,
