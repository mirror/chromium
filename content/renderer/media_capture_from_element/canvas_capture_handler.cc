...
void CanvasCaptureHandler::DeliverNewFrame(const SkImage* image) {
  DVLOG(4) << __func__;
  DCHECK(main_render_thread_checker_.CalledOnValidThread());
  DCHECK(image);
  ...
  if (image->peekPixels(&pixmap) &&
    ...
    DeliverAsYUVFrame(
        image->isOpaque(), static_cast<const uint8*>(pixmap.addr(0, 0)),
        pixmap.getSafeSize64(), pixmap.rowBytes(),
  ...
  if (image->isTextureBacked()) {
    if (!gl_helper_) {
      ...
      viz::ContextProvider* context_provider =
          gpu_factories->GetMediaContextProvider();
      DCHECK(context_provider);
      gl_helper_.reset(new viz::GLHelper(context_provider->ContextGL(),
    ...
    gl_helper_->ReadbackTextureAsync(
        ...
        media::BindToCurrentLoop(
            base::Bind(&CanvasCaptureHandler::ReadPixelsAsync,
                       weak_ptr_factory_.GetWeakPtr(), temp_argb_frame,
                       image_size, image->isOpaque())));
    return;
  }

  // Last resort, copy the pixels into memory synchoronously. This call may
  // block main render thread.
  const size_t stride = image_size.width() * kArgbBytesPerPixel;
  SkImageInfo image_info = SkImageInfo::MakeN32(
      image_size.width(), image_size.height(), kUnpremul_SkAlphaType);
  if (image->readPixels(image_info,
                        temp_argb_frame->visible_data(VideoFrame::kYPlane),
                        stride, 0 /*srcX*/, 0 /*srcY*/)) {
    DeliverAsYUVFrame(
        image->isOpaque(), temp_argb_frame->visible_data(VideoFrame::kYPlane),
        stride * image_size.height(), stride, image_size, kN32_SkColorType);
    return;
  }
...
void CanvasCaptureHandler::DeliverAsYUVFrame(bool isOpaque,
  ...
  libyuv::ConvertToI420(
      source_ptr, source_length, video_frame->visible_data(VideoFrame::kYPlane),
      video_frame->stride(VideoFrame::kYPlane),
      video_frame->visible_data(VideoFrame::kUPlane),
      video_frame->stride(VideoFrame::kUPlane),
      video_frame->visible_data(VideoFrame::kVPlane),
      video_frame->stride(VideoFrame::kVPlane), 0 /* crop_x */, 0 /* crop_y */,
      image_size.width(), image_size.height(), image_size.width(),
      image_size.height(), libyuv::kRotate0, source_pixel_format);
  if (!isOpaque) {
...
void CanvasCaptureHandler::ReadPixelsAsync(
  ...
  const size_t stride = image_size.width() * kArgbBytesPerPixel;
  DeliverAsYUVFrame(isOpaque, temp_frame->visible_data(VideoFrame::kYPlane),
                    stride * image_size.height(), stride, image_size,
                    kN32_SkColorType);
}
