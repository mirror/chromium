...
CanvasCaptureHandler::CreateCanvasCaptureHandler(
  ...
  return std::unique_ptr<CanvasCaptureHandler>(new CanvasCaptureHandler(
      size, frame_rate, io_task_runner, context_provider, track));
}

void CanvasCaptureHandler::SendNewFrame(sk_sp<SkImage> image) {
  DCHECK(main_render_thread_checker_.CalledOnValidThread());
...
void CanvasCaptureHandler::DeliverNewFrame(sk_sp<SkImage> image) {
  DVLOG(4) << __func__;
  DCHECK(main_render_thread_checker_.CalledOnValidThread());
  DCHECK(image);
