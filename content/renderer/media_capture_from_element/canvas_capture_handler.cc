...
void CanvasCaptureHandler::SendNewFrame(
  ...
  if (image->peekPixels(&pixmap) &&
    ...
    SendFrame(
        ConvertToYUVFrame(image->isOpaque(), false,
                          static_cast<const uint8*>(pixmap.addr(0, 0)),
                          gfx::Size(pixmap.width(), pixmap.height()),
                          pixmap.rowBytes(), pixmap.colorType(), timestamp),
        timestamp);
    return;
  }
  ...
  // Copy the pixels into memory synchronously. This call may block main render
  // thread.
...
void CanvasCaptureHandler::ReadARGBPixelsSync(sk_sp<SkImage> image) {
  ...
  SendFrame(ConvertToYUVFrame(
                image->isOpaque(), false /* flip */,
                temp_argb_frame->visible_data(VideoFrame::kARGBPlane),
                image_size, temp_argb_frame->stride(VideoFrame::kARGBPlane),
                kN32_SkColorType, timestamp),
            timestamp);
}
...
void CanvasCaptureHandler::ReadYUVPixelsAsync(
  ...
  std::unique_ptr<viz::ReadbackYUVInterface> yuv_reader =
      context_provider->GetGLHelper()->CreateReadbackPipelineYUV(
          surface_origin != kTopLeft_GrSurfaceOrigin, true /* use_mrt */);
  yuv_reader->ReadbackYUV(
      ...
      base::Bind(&CanvasCaptureHandler::OnYUVPixelsReadAsync,
                 ...
                 base::Passed(std::move(yuv_reader)), image, output_frame));
}

void CanvasCaptureHandler::OnARGBPixelsReadAsync(
    sk_sp<SkImage> image,
    const scoped_refptr<media::VideoFrame> temp_argb_frame,
    bool flip,
    bool success) {
  DCHECK(main_render_thread_checker_.CalledOnValidThread());
  if (!success) {
  ...
  const base::TimeTicks timestamp = base::TimeTicks::Now();
  SendFrame(
      ConvertToYUVFrame(image->isOpaque(), flip,
                        temp_argb_frame->visible_data(VideoFrame::kARGBPlane),
...
void CanvasCaptureHandler::OnYUVPixelsReadAsync(
  ...
  SendFrame(yuv_frame, base::TimeTicks::Now());
}
