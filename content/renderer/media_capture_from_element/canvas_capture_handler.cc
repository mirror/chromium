...
void CanvasCaptureHandler::CreateNewFrame(const SkImage* image) {
  ...
  if (size != last_size) {
    ...
    image_info_ = SkImageInfo::MakeN32(size.width(), size.height(),
  ...
  if (image->peekPixels(&pixmap) &&
    ...
    source_size.set_width(pixmap.width());
    source_size.set_height(pixmap.height());
  ...
  libyuv::ConvertToI420(source_ptr, source_length,
                        ...
                        0 /* crop_x */, 0 /* crop_y */, source_size.width(),
                        source_size.height(), size.width(), size.height(),
