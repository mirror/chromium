...
// These tests check that the V2 sandbox compiles, initialiazes, and correctly
...
// check that write access fails to system fails. These tests run on developers
// machines and bots, so if the write access goes through, that machine could be
// hosed.
...
MULTIPROCESS_TEST_MAIN(SandboxProfileProcess) {
  ...
  NSPort* forbidden_mach = [[NSMachBootstrapServer sharedInstance]
      portForName:@"com.apple.cfprefsd.daemon"
             host:nil];
  ...
  // Read the CFUserTextEncoding.
  base::FilePath text_encoding =
      base::GetHomeDir().Append(".CFUserTextEncoding");
  std::string contents;
  CHECK(base::ReadFileToString(text_encoding, &contents));
