...
void GenerateFrameStateFromItem(const WebHistoryItem& item,
  ...
  state->scroll_anchor_selector =
      WebString::ToOptionalString16(item.ScrollAnchorSelector());
  state->scroll_anchor_offset = item.ScrollAnchorOffset();
  state->scroll_anchor_simhash = item.ScrollAnchorSimhash();
}
...
void RecursivelyGenerateHistoryItem(const ExplodedFrameState& state,
  ...
  item.SetScrollAnchorSimhash(state.scroll_anchor_simhash);
  node->set_item(item);
