...
TEST_F(RenderViewImplTest, GetCompositionCharacterBoundsTest) {
  ...
  const std::vector<blink::WebImeSpan> empty_ime_span;
  std::vector<gfx::Rect> bounds;
  view()->OnSetFocus(true);

  // ASCII composition
  const base::string16 ascii_composition = base::UTF8ToUTF16("aiueo");
  view()->OnImeSetComposition(ascii_composition, empty_ime_span,
                              gfx::Range::InvalidRange(), 0, 0);
  ...
  view()->OnImeCommitText(empty_string, std::vector<blink::WebImeSpan>(),
                          gfx::Range::InvalidRange(), 0);

  // Non surrogate pair unicode character.
  const base::string16 unicode_composition = base::UTF8ToUTF16(
      "\xE3\x81\x82\xE3\x81\x84\xE3\x81\x86\xE3\x81\x88\xE3\x81\x8A");
  view()->OnImeSetComposition(unicode_composition, empty_ime_span,
                              gfx::Range::InvalidRange(), 0, 0);
  view()->GetCompositionCharacterBounds(&bounds);
  ASSERT_EQ(unicode_composition.size(), bounds.size());
  for (size_t i = 0; i < bounds.size(); ++i)
    EXPECT_LT(0, bounds[i].width());
  view()->OnImeCommitText(empty_string, empty_ime_span,
                          gfx::Range::InvalidRange(), 0);

  // Surrogate pair character.
  const base::string16 surrogate_pair_char =
      base::UTF8ToUTF16("\xF0\xA0\xAE\x9F");
  view()->OnImeSetComposition(surrogate_pair_char, empty_ime_span,
                              gfx::Range::InvalidRange(), 0, 0);
  view()->GetCompositionCharacterBounds(&bounds);
  ...
  view()->OnImeSetComposition(surrogate_pair_mixed_composition, empty_ime_span,
                              gfx::Range::InvalidRange(), 0, 0);
  view()->GetCompositionCharacterBounds(&bounds);
...
TEST_F(RenderViewImplTest, SetEditableSelectionAndComposition) {
  ...
  const std::vector<blink::WebImeSpan> empty_ime_span;
  frame()->SetCompositionFromExistingText(7, 10, empty_ime_span);
