...
void URLLoaderClientImpl::OnReceiveInlinedDataChunk(
    const std::vector<uint8_t>& data) {
  TRACE_EVENT0("loader", "ResourceDispatcher::OnReceiveInlinedDataChunk");
  DCHECK(!data.empty());
  ...
  request_info->peer->OnReceivedData(
      base::MakeUnique<content::FixedReceivedData>(
          reinterpret_cast<const char*>(&data.front()), data.size()));
}
