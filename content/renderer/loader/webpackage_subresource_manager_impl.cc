 ...
 public:
  ResponseHandler(
      ...
      : handler_(std::make_unique<WebPackageResponseHandler>(
                url, method, response_head, std::move(done_callback),
...
WebPackageSubresourceManagerImpl::PendingResponse::PendingResponse(PendingResponse&& other) {
  *this = std::move(other);
}

WebPackageSubresourceManagerImpl::PendingResponse&
WebPackageSubresourceManagerImpl::PendingResponse::operator=(PendingResponse&& other) {
...
void WebPackageSubresourceManagerImpl::GetResource(
    ...
    ResourceCallback callback,
    CompletionCallback completion_callback) {
  LOG(ERROR) << "** [RENDERER] GetResource: " << resource_request.url;
  ...
  if (pending_response_found != pending_response_map_.end()) {
    ...
    std::move(callback).Run(
        net::OK, std::move(pending.response_head),
...
void WebPackageSubresourceManagerImpl::OnResponse(
  ...
  if (!kStartReadingResponseEarly) {
    auto inserted = pending_response_map_.emplace(
        request,
        PendingResponse(response_head, std::move(response_body),
                        std::move(callback_request)));
    DCHECK(inserted.second);
    return;
  }
  ...
  auto inserted = response_map_.emplace(
      ...
      std::make_unique<ResponseHandler>(
          url, method, response_head, std::move(response_body),
          base::BindOnce(
