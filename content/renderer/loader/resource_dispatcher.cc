...
int ResourceDispatcher::StartAsync(
  ...
  if (main_resource_loader_params) {
...
void ResourceDispatcher::ContinueForNavigation(
    int request_id,
    mojom::MainResourceLoaderParamsPtr main_resource_loader_params) {
  DCHECK(main_resource_loader_params);
  PendingRequestInfo* request_info = GetPendingRequestInfo(request_id);
  if (!request_info)
    return;
  ...
  // TODO(arthursonzogni): Get rid of |body_data| and always use this path.
  if (!main_resource_loader_params->body_data.is_valid()) {
    client_ptr->Bind(std::move(main_resource_loader_params->url_loader),
                     std::move(main_resource_loader_params->url_loader_client));
    return;
  }

  // Start streaming now.
  client_ptr->OnStartLoadingResponseBody(
      std::move(main_resource_loader_params->body_data));
