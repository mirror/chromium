...
TEST_F(CORSURLLoaderTest, CrossOriginRequestFetchRequestModeCORSNoCORSHeader) {
  ...
  ASSERT_TRUE(client().completion_status().cors_error->access);
  EXPECT_EQ(network::mojom::CORSAccessError::kMissingAllowOriginHeader,
            *client().completion_status().cors_error->access);
}
