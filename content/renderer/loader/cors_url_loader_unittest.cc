...
TEST_F(CORSURLLoaderTest, CrossOriginRequestFetchRequestModeCORSNoCORSHeader) {
  ...
  ASSERT_TRUE(client().completion_status().cors_error_status.has_value());
  ASSERT_TRUE(
      client().completion_status().cors_error_status->access.has_value());
  EXPECT_EQ(network::mojom::CORSAccessStatus::kMissingAllowOriginHeader,
            client().completion_status().cors_error_status->access);
}
