...
TEST_F(CORSURLLoaderTest, CrossOriginRequestFetchRequestModeCORSNoCORSHeader) {
  ...
  ASSERT_TRUE(client().completion_status().cors_error_code.has_value());
  EXPECT_EQ(content::ResourceRequestCORSErrorCode::kMissingAllowOriginHeader,
            client().completion_status().cors_error_code);
}
