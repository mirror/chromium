...
media::AudioParameters GetMixerOutputParams(
  ...
  // On certain platforms, when using a fake device, or if the hardware params
  // are invalid we can rely on the playback device to handle resampling, so
  ...
  if (!hardware_params.IsValid() ||
    ...
    output_sample_rate = input_params.frames_per_buffer();
