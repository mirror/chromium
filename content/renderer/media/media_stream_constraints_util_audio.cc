...
AudioCaptureCandidates::AudioCaptureCandidates(
  ...
  bool_sets_[GOOG_AUTO_GAIN_CONTROL] =
      bool_sets_[GOOG_AUTO_GAIN_CONTROL].Intersection(
          DiscreteSet<bool>({properties.goog_auto_gain_control}));
  bool_sets_[GOOG_EXPERIMENTAL_ECHO_CANCELLATION] =
      bool_sets_[GOOG_EXPERIMENTAL_ECHO_CANCELLATION].Intersection(
          DiscreteSet<bool>({properties.goog_experimental_echo_cancellation}));
  bool_sets_[GOOG_TYPING_NOISE_DETECTION] =
      bool_sets_[GOOG_TYPING_NOISE_DETECTION].Intersection(
          DiscreteSet<bool>({properties.goog_typing_noise_detection}));
  bool_sets_[GOOG_NOISE_SUPPRESSION] =
      bool_sets_[GOOG_NOISE_SUPPRESSION].Intersection(
          DiscreteSet<bool>({properties.goog_noise_suppression}));
  bool_sets_[GOOG_EXPERIMENTAL_NOISE_SUPPRESSION] =
      bool_sets_[GOOG_EXPERIMENTAL_NOISE_SUPPRESSION].Intersection(
          DiscreteSet<bool>({properties.goog_experimental_noise_suppression}));
  bool_sets_[GOOG_BEAMFORMING] = bool_sets_[GOOG_BEAMFORMING].Intersection(
      DiscreteSet<bool>({properties.goog_beamforming}));
  bool_sets_[GOOG_HIGHPASS_FILTER] =
      bool_sets_[GOOG_HIGHPASS_FILTER].Intersection(
          DiscreteSet<bool>({properties.goog_highpass_filter}));
  bool_sets_[GOOG_EXPERIMENTAL_AUTO_GAIN_CONTROL] =
      bool_sets_[GOOG_EXPERIMENTAL_AUTO_GAIN_CONTROL].Intersection(
          DiscreteSet<bool>({properties.goog_experimental_auto_gain_control}));
