...
class ScopedVideoDecoder : public webrtc::VideoDecoder {
 public:
  ScopedVideoDecoder(media::GpuVideoAcceleratorFactories* gpu_factories,
                     std::unique_ptr<webrtc::VideoDecoder> decoder)
      : gpu_factories_(gpu_factories), decoder_(std::move(decoder)) {}
  ...
  ~ScopedVideoDecoder() override {
    gpu_factories_->GetTaskRunner()->DeleteSoon(FROM_HERE, decoder_.release());
  }
...
RTCVideoDecoderFactory::CreateVideoDecoder(
  ...
  DVLOG(2) << __func__;
  return base::MakeUnique<ScopedVideoDecoder>(
      gpu_factories_,
      RTCVideoDecoder::Create(webrtc::PayloadStringToCodecType(format.name),
                              gpu_factories_));
}
