...
#include "content/renderer/media/gpu/gpu_video_accelerator_factories.h"
...
// static
std::unique_ptr<GpuVideoAcceleratorFactories>
GpuVideoAcceleratorFactories::Create(
  ...
  return base::WrapUnique(new GpuVideoAcceleratorFactories(
...
bool GpuVideoAcceleratorFactories::CheckContextLost() {
  ...
  if (context_provider_) {
    ...
    if (release_context_provider) {
      ...
      main_thread_task_runner_->PostTask(
          FROM_HERE,
          base::Bind(&GpuVideoAcceleratorFactories::ReleaseContextProvider,
...
bool GpuVideoAcceleratorFactories::IsGpuVideoAcceleratorEnabled() {
  return video_accelerator_enabled_;
}

base::UnguessableToken GpuVideoAcceleratorFactories::GetChannelToken() {
  DCHECK(task_runner_->BelongsToCurrentThread());
  if (CheckContextLost())
...
int32_t GpuVideoAcceleratorFactories::GetCommandBufferRouteId() {
  DCHECK(task_runner_->BelongsToCurrentThread());
  if (CheckContextLost())
    return 0;
...
GpuVideoAcceleratorFactories::CreateVideoDecodeAccelerator() {
...
GpuVideoAcceleratorFactories::CreateVideoEncodeAccelerator() {
  DCHECK(video_accelerator_enabled_);
  DCHECK(task_runner_->BelongsToCurrentThread());
  if (CheckContextLost())
    return nullptr;
...
bool GpuVideoAcceleratorFactories::CreateTextures(
    int32_t count,
    const gfx::Size& size,
...
void GpuVideoAcceleratorFactories::DeleteTexture(uint32_t texture_id) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  if (CheckContextLost())
    return;
...
gpu::SyncToken GpuVideoAcceleratorFactories::CreateSyncToken() {
  cc::ContextProvider::ScopedContextLock lock(context_provider_);
...
void GpuVideoAcceleratorFactories::WaitSyncToken(
...
GpuVideoAcceleratorFactories::CreateGpuMemoryBuffer(const gfx::Size& size,
...
bool GpuVideoAcceleratorFactories::ShouldUseGpuMemoryBuffersForVideoFrames()
    const {
  return enable_gpu_memory_buffer_video_frames_;
}

unsigned GpuVideoAcceleratorFactories::ImageTextureTarget(
    gfx::BufferFormat format) {
...
GpuVideoAcceleratorFactories::VideoFrameOutputFormat() {
  DCHECK(task_runner_->BelongsToCurrentThread());
  if (CheckContextLost())
...
GpuVideoAcceleratorFactories::GetGLContextLock() {
  if (CheckContextLost())
    return nullptr;
...
GpuVideoAcceleratorFactories::CreateSharedMemory(size_t size) {
...
GpuVideoAcceleratorFactories::GetTaskRunner() {
  return task_runner_;
}

media::VideoDecodeAccelerator::Capabilities
GpuVideoAcceleratorFactories::GetVideoDecodeAcceleratorCapabilities() {
...
GpuVideoAcceleratorFactories::GetVideoEncodeAcceleratorSupportedProfiles() {
  return media::GpuVideoAcceleratorUtil::ConvertGpuToMediaEncodeProfiles(
      gpu_channel_host_->gpu_info()
          .video_encode_accelerator_supported_profiles);
}

void GpuVideoAcceleratorFactories::ReleaseContextProvider() {
...
GpuVideoAcceleratorFactories::ContextProviderMainThread() {
  DCHECK(main_thread_task_runner_->BelongsToCurrentThread());
