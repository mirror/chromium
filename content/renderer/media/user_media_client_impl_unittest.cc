...
class UserMediaClientImplUnderTest : public UserMediaClientImpl {
 public:
  ...
  AudioCaptureSettings AudioSettings() const { return AudioCaptureSettingsForCurrentRequest(); }
  VideoCaptureSettings VideoSettings() const { return VideoCaptureSettingsForCurrentRequest(); }
...
TEST_P(UserMediaClientImplTest, DefaultConstraintsPropagate) {
  ...
  // dependent.
  EXPECT_EQ(AudioProcessingProperties().goog_experimental_echo_cancellation, properties.goog_experimental_echo_cancellation);
  ...
  EXPECT_EQ(video_capture_settings.Width(), MediaStreamVideoSource::kDefaultWidth);
  EXPECT_EQ(video_capture_settings.Height(), MediaStreamVideoSource::kDefaultHeight);
  EXPECT_EQ(video_capture_settings.FrameRate(), MediaStreamVideoSource::kDefaultFrameRate);
  EXPECT_EQ(video_capture_settings.ResolutionChangePolicy(), media::RESOLUTION_POLICY_FIXED_RESOLUTION);
  EXPECT_EQ(video_capture_settings.PowerLineFrequency(), media::PowerLineFrequency::FREQUENCY_DEFAULT);
  EXPECT_FALSE(video_capture_settings.noise_reduction());
  EXPECT_EQ(video_capture_settings.min_frame_rate(), 1.0);

  const VideoTrackAdapterSettings& track_settings = video_capture_settings.track_adapter_settings();
  ...
  EXPECT_EQ(track_settings.min_aspect_ratio, 1.0 / MediaStreamVideoSource::kDefaultHeight);
  EXPECT_EQ(track_settings.max_aspect_ratio, MediaStreamVideoSource::kDefaultWidth);
  // 0.0 is the default max_frame_rate and it indicates no frame-rate adjustment
  EXPECT_EQ(track_settings.max_frame_rate, 0.0);
}

TEST_P(UserMediaClientImplTest, DefaultContentCapturePropagate) {
  if (IsOldAudioConstraints())
    return;

  MockConstraintFactory factory;
  factory.basic().media_stream_source.SetExact(blink::WebString::FromASCII(kMediaStreamSourceTab));
  blink::WebMediaConstraints audio_constraints =
      factory.CreateWebMediaConstraints();
  ...
  EXPECT_EQ(video_capture_settings.PowerLineFrequency(), media::PowerLineFrequency::FREQUENCY_DEFAULT);
  EXPECT_FALSE(video_capture_settings.noise_reduction());
  EXPECT_EQ(video_capture_settings.min_frame_rate(), kMinScreenCastFrameRate);
...
TEST_P(UserMediaClientImplTest, NonDefaultAudioConstraintsPropagate) {
  ...
  MockConstraintFactory factory;
  factory.basic().device_id.SetExact(blink::WebString::FromASCII(kFakeAudioInputDeviceId1));
  ...
  factory.basic().goog_array_geometry.SetExact(blink::WebString::FromASCII("1 1 1"));
  blink::WebMediaConstraints audio_constraints =
      factory.CreateWebMediaConstraints();
  ...
  EXPECT_TRUE(audio_capture_settings.HasValue());
  EXPECT_EQ(kFakeAudioInputDeviceId1,
            audio_capture_settings.device_id());
...
TEST_P(UserMediaClientImplTest, CreateWithAllOptionalInvalidDeviceId) {
  ...
  std::string expected_audio_device_id = IsOldAudioConstraints()
      ? std::string() : media::AudioDeviceDescription::kDefaultDeviceId;
  TestValidRequestWithConstraints(audio_constraints, video_constraints,
                                  expected_audio_device_id, kFakeVideoInputDeviceId1);
}
...
TEST_P(UserMediaClientImplTest, AudioParametersChanged) {
  // This test does not apply with old constraints processing.
  if (IsOldAudioConstraints())
    return;
  // Change |media_devices_dispatcher_| device parameters so that they differ
  // from the ones returned by |ms_dispatcher_|.
  media_devices_dispatcher_.AudioParameters().set_sample_rate(10000);
  user_media_client_impl_->RequestUserMediaForTest();
  FakeMediaStreamDispatcherRequestUserMediaComplete();
  EXPECT_EQ(UserMediaClientImplUnderTest::REQUEST_FAILED,
            user_media_client_impl_->request_state());

  media_devices_dispatcher_.ResetAudioParameters();
  media_devices_dispatcher_.AudioParameters().set_effects(media::AudioParameters::ECHO_CANCELLER);
  user_media_client_impl_->RequestUserMediaForTest();
  FakeMediaStreamDispatcherRequestUserMediaComplete();
  EXPECT_EQ(UserMediaClientImplUnderTest::REQUEST_FAILED,
            user_media_client_impl_->request_state());
  ...
  media_devices_dispatcher_.ResetAudioParameters();
  std::vector<media::Point> kMicPositions = {{1,1,1}};
  media_devices_dispatcher_.AudioParameters().set_mic_positions(kMicPositions);
  user_media_client_impl_->RequestUserMediaForTest();
  FakeMediaStreamDispatcherRequestUserMediaComplete();
  EXPECT_EQ(UserMediaClientImplUnderTest::REQUEST_FAILED,
            user_media_client_impl_->request_state());
...
TEST_P(UserMediaClientImplTest, AudioParametersChangedNonAudioDevice) {
  // This test does not apply with old constraints processing.
  if (IsOldAudioConstraints())
    return;
  // Check that a video-only stream is not affected by changed audio parameters.
  media_devices_dispatcher_.AudioParameters().set_sample_rate(10000);
  blink::WebUserMediaRequest request =
      blink::WebUserMediaRequest::CreateForTesting(blink::WebMediaConstraints(),
                                                   CreateDefaultConstraints());
  user_media_client_impl_->RequestUserMediaForTest(request);
  FakeMediaStreamDispatcherRequestUserMediaComplete();
  // At this point the request did not fail due to changed audio parameters, but
  // has not completed because sources have not started yet.
  EXPECT_EQ(UserMediaClientImplUnderTest::REQUEST_NOT_COMPLETE,
            user_media_client_impl_->request_state());
  user_media_client_impl_->CancelUserMediaRequest(request);
}

TEST_P(UserMediaClientImplTest, AudioParametersChangedContentCapture) {
  // This test does not apply with old constraints processing.
  if (IsOldAudioConstraints())
    return;
  // Check that a video-only stream is not affected by changed audio parameters.
  ...
  factory.basic().media_stream_source.SetExact(
        blink::WebString::FromUTF8(kMediaStreamSourceTab));
  blink::WebUserMediaRequest request =
      blink::WebUserMediaRequest::CreateForTesting(blink::WebMediaConstraints(),
                                                   factory.CreateWebMediaConstraints());
  user_media_client_impl_->RequestUserMediaForTest(request);
...
INSTANTIATE_TEST_CASE_P(, UserMediaClientImplTest, testing::Values(true, false));
