...
// This test checks that audio parameters for the associated output device are
// reset if renderToAssociatedSink constrainable property is set to false.
TEST_F(UserMediaClientImplTest, RenderToAssociatedSinkFalseAudioParams) {
  ...
  EXPECT_FALSE(source->device().matched_output);
}
