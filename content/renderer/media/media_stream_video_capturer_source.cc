...
void MediaStreamVideoCapturerSource::RestartSourceImpl(
  ...
  new_capture_params_ = capture_params_;
  new_capture_params_.requested_format = new_format;
  state_ = RESTARTING;
  source_->StartCapture(
      new_capture_params_, frame_callback_,
      base::Bind(&MediaStreamVideoCapturerSource::OnRunStateChanged,
                 base::Unretained(this)));
}
...
void MediaStreamVideoCapturerSource::OnRunStateChanged(bool is_running) {
  ...
  switch (state_) {
    ...
    case RESTARTING:
      if (is_running) {
        state_ = STARTED;
        capture_params_ = new_capture_params_;
      } else {
        state_ = STOPPED;
      }
