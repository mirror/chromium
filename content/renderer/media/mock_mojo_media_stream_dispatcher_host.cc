...
void MockMojoMediaStreamDispatcherHost::GenerateStream(
  ...
  if (controls.audio.requested) {
    ...
    audio_device.id = (test_same_id_ ? "test_id" : controls.audio.device_id) +
  ...
  if (controls.video.requested) {
    ...
    video_device.id = (test_same_id_ ? "test_id" : controls.video.device_id) +
                      base::IntToString(session_id_);
  ...
  std::move(callback).Run(MEDIA_DEVICE_OK,
                          "dummy" + base::IntToString(request_id),
