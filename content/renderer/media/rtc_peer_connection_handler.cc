...
class RTCPeerConnectionHandler::Observer
 ...
 protected:
  ...
  void OnAddTrack(
    ...
    main_thread_->PostTask(
        ...
        base::BindOnce(&RTCPeerConnectionHandler::Observer::OnAddTrackImpl,
                       this, make_scoped_refptr(webrtc_receiver.get()),
  ...
  void OnRemoveTrack(
      rtc::scoped_refptr<webrtc::RtpReceiverInterface> webrtc_receiver,
      const std::vector<rtc::scoped_refptr<MediaStreamInterface>>&
          webrtc_streams) override {
    std::unique_ptr<WebRtcMediaStreamTrackAdapterMap::AdapterRef>
        remote_track_adapter_ref =
            stream_adapter_map_->track_adapter_map()
                ->GetOrCreateRemoteTrackAdapter(webrtc_receiver->track().get());
    // TODO(hbos): Could this ever be called without the adapter already
    // existing? It might not be initialized if we don't wait for promises to
    // resolve but it shouldn't fire for non-existent receivers?
    // Same comment with regards to streams.
  ...
  // TODO(hbos): Remove once |webrtc::PeerConnectionObserver| does not require
  // it to be overriden.
  void OnAddStream(
...
void RTCPeerConnectionHandler::OnAddRemoteTrack(
  ...
  const std::unique_ptr<RTCRtpReceiver>& rtp_receiver = rtp_receivers_.insert(
      std::make_pair(
          receiver_id,
          base::MakeUnique<RTCRtpReceiver>(
              webrtc_receiver.get(),
              std::move(remote_track_adapter_ref),
              std::move(remote_stream_adapter_refs)))).first->second;
  std::unique_ptr<blink::WebRTCRtpReceiver> web_rtp_receiver(
      rtp_receiver->ShallowCopy().release());
  client_->DidAddRemoteTrack(&web_rtp_receiver);
}
...
void RTCPeerConnectionHandler::OnRemoveRemoteTrack(
  ...
  std::unique_ptr<blink::WebRTCRtpReceiver> web_rtp_receiver(
      it->second->ShallowCopy().release());
  client_->DidRemoveRemoteTrack(&web_rtp_receiver);
