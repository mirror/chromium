...
class RTCPeerConnectionHandler::SetRemoteDescriptionObserver
 ...
 public:
  ...
  void OnSuccess(
    ...
    if (handler_) {
      std::vector<WebRtcReceiverState*>
          added_receiver_states;  // intermediate vector not needed?
      for (auto& receiver_state : states.receiver_states) {
        if (ReceiverWasAdded(receiver_state))
          added_receiver_states.push_back(&receiver_state);
      }
      ...
      for (auto* removed_receiver : removed_receivers) {
        handler_->OnRemoveRemoteTrack(
            removed_receiver->webrtc_rtp_receiver_.get(), nullptr);
      }
      for (auto* receiver_state : added_receiver_states) {
 ...
 private:
  ...
  bool ReceiverWasRemoved(
    ...
    for (const auto& receiver_state : receiver_states) {
      if (receiver_state.receiver == receiver.webrtc_rtp_receiver_.get())
        return false;
    }
    return true;
  }
