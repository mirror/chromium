...
RTCPeerConnectionHandler::GetReceiversForTesting() {
  blink::WebVector<std::unique_ptr<blink::WebRTCRtpReceiver>> receivers_vector(
      rtp_receivers_.size());
  size_t i = 0;
  for (const auto& it : rtp_receivers_) {
    const std::unique_ptr<RTCRtpReceiver>& rtp_receiver = it.second;
    receivers_vector[i++].reset(rtp_receiver->ShallowCopy().release());
  }
