...
std::vector<std::unique_ptr<RTCRtpReceiver>>
RTCPeerConnectionHandler::GetReceiversForTesting() {
  std::vector<std::unique_ptr<RTCRtpReceiver>> rtp_receivers_vector;
  for (const auto& it : rtp_receivers_) {
    rtp_receivers_vector.push_back(it.second->ShallowCopy());
  }
  return rtp_receivers_vector;
}
