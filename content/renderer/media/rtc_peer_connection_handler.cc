...
RTCPeerConnectionHandler::GetReceivers() {
  ...
  for (size_t i = 0; i < webrtc_receivers.size(); ++i) {
    std::unique_ptr<blink::WebRTCRtpReceiver> web_receiver =
        GetWebRTCRtpReceiver(webrtc_receivers[i]);
    // |web_receiver| is null if we could not find an initialized track adapter
    // for it. This can happen because adding and removing tracks happens
    // asynchronous.
    if (web_receiver)
