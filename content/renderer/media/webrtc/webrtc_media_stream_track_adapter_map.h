...
class CONTENT_EXPORT WebRtcMediaStreamTrackAdapterMap
 ...
 public:
  ...
  // First variety: if an adapter does not exist or it has not been fully
  // initialized yet null is returned.
  std::unique_ptr<AdapterRef> GetRemoteTrackAdapter(
      const blink::WebMediaStreamTrack& web_track);
  // Second variety: includes adapters that have not yet been fully initialized,
  // see |AdapterRef::is_initialized|. If an adapter does not exist null is
  // returned.
  std::unique_ptr<AdapterRef> GetRemoteTrackAdapter(
      webrtc::MediaStreamTrackInterface* webrtc_track);
