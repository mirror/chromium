...
class CONTENT_EXPORT WebRtcMediaStreamAdapterMap
 ...
 public:
  ...
  // --Invoke on the main thread. Gets a new reference to the local stream
  // adapter by ID, or null if no such adapter was found. When all references
  // are destroyed the adapter is destroyed and removed from the map. References
