...
WebRtcMediaStreamTrackAdapterMap::AdapterRef::~AdapterRef() {
  ...
  scoped_refptr<WebRtcMediaStreamTrackAdapter> removed_adapter;
  {
    ...
    if (adapter->HasOneRef()) {
      ...
      // the primary or secondary key for removal. (Preferring |webrtc_track|
      // because pointer key lookup should be faster than string key.)
      DCHECK(adapter->is_initialized());
      if (type_ == Type::kLocal)
        map_->local_track_adapters_.EraseBySecondary(adapter->webrtc_track());
      else
...
WebRtcMediaStreamTrackAdapterMap::GetLocalTrackAdapter(
  ...
  std::string web_track_unique_id = web_track.UniqueId().Utf8();
  base::AutoLock scoped_lock(lock_);
  scoped_refptr<WebRtcMediaStreamTrackAdapter>* adapter_ptr =
      local_track_adapters_.FindByPrimary(web_track_unique_id);
  if (!adapter_ptr)
    return nullptr;
...
WebRtcMediaStreamTrackAdapterMap::GetOrCreateLocalTrackAdapter(
  ...
  local_track_adapters_.Insert(web_track_unique_id, new_adapter);
  local_track_adapters_.SetSecondaryKey(web_track_unique_id,
...
WebRtcMediaStreamTrackAdapterMap::GetRemoteTrackAdapter(
  ...
  base::AutoLock scoped_lock(lock_);
  scoped_refptr<WebRtcMediaStreamTrackAdapter>* adapter_ptr =
      remote_track_adapters_.FindBySecondary(web_track_unique_id);
...
void WebRtcMediaStreamTrackAdapterMap::OnRemoteTrackAdapterInitialized(
  ...
  std::string web_track_unique_id = adapter_ref->web_track().UniqueId().Utf8();
  {
    base::AutoLock scoped_lock(lock_);
