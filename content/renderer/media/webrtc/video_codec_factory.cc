...
// Wrap in SimulcastWrapperClass if it's VP8.
template <typename SimulcastWrapperClass>
std::unique_ptr<webrtc::VideoEncoder> CreateEncoder(
    webrtc::VideoEncoderFactory* factory,
    const webrtc::SdpVideoFormat& format) {
  if (IsFormatSupported(factory, format)) {
    return cricket::CodecNamesEq(format.name.c_str(), cricket::kVp8CodecName)
               ? base::MakeUnique<SimulcastWrapperClass>(factory)
               : factory->CreateVideoEncoder(format);
  }
  return nullptr;
}
 ...
 public:
  ...
  std::unique_ptr<webrtc::VideoEncoder> CreateVideoEncoder(
    ...
    std::unique_ptr<webrtc::VideoEncoder> internal_encoder =
        CreateEncoder<webrtc::VP8EncoderSimulcastProxy>(
            &internal_encoder_factory_, format);

    std::unique_ptr<webrtc::VideoEncoder> external_encoder =
        CreateEncoder<webrtc::SimulcastEncoderAdapter>(
