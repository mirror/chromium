...
class WebRtcSetRemoteDescriptionObserverForTest
 ...
 public:
  ...
  bool result() const { return error_->ok(); }
  ...
  // WebRtcSetRemoteDescriptionObserver overrides.
  void OnSuccess(WebRtcSetRemoteDescriptionObserver::States states) override {
    error_ = webrtc::RTCError::OK();
    states_ = std::move(states);
  }

  void OnFailure(webrtc::RTCError error) override { error_ = std::move(error); }

 private:
  ~WebRtcSetRemoteDescriptionObserverForTest() override {}

  // Set on complete, if successful this a "no error" error.
  base::Optional<webrtc::RTCError> error_;
...
class WebRtcSetRemoteDescriptionObserverHandlerTest : public ::testing::Test {
  ...
  void InvokeOnComplete(webrtc::RTCError error) {
    ...
    dependency_factory_->GetWebRtcSignalingThread()->PostTask(
        ...
        base::BindOnce(&WebRtcSetRemoteDescriptionObserverHandlerTest::
                           InvokeOnCompleteOnSignalingThread,
 ...
 protected:
  void InvokeOnCompleteOnSignalingThread(webrtc::RTCError error,
                                         base::RunLoop* run_loop) {
    observer_handler_->OnComplete(std::move(error));
    run_loop->Quit();
  }
...
TEST_F(WebRtcSetRemoteDescriptionObserverHandlerTest, OnSuccess) {
  ...
  InvokeOnComplete(webrtc::RTCError::OK());
...
TEST_F(WebRtcSetRemoteDescriptionObserverHandlerTest, OnFailure) {
  ...
  InvokeOnComplete(std::move(error));
  EXPECT_TRUE(observer_->called());
