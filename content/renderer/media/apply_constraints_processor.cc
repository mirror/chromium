...
void ApplyConstraintsProcessor::ProcessVideoContentRequest() {
  ...
  // With content capture, the default is to use the source's frame rate, so
  // using 0.0.
  VideoCaptureSettings video_capture_settings =
      SelectSettingsVideoContentCapture(
          current_request_.Constraints(), std::string() /* stream_source */,
          settings.width, settings.height, 0.0 /* default_frame_rate */,
          settings.device_id.Utf8());
