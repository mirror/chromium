...
void MediaStreamVideoSource::ReconfigureTrack(
  ...
  track_adapter_->ReconfigureTrack(
      track, adapter_settings,
      base::BindOnce(&MediaStreamVideoSource::UpdateTrackSettings,
                     weak_factory_.GetWeakPtr(), track, adapter_settings));
}
  ...
  for (const auto& track : track_descriptors) {
    ...
    if (result == MEDIA_DEVICE_OK) {
      track_adapter_->AddTrack(
          track.track, track.frame_callback, *track.adapter_settings,
          base::BindOnce(&MediaStreamVideoSource::UpdateTrackSettings,
                         weak_factory_.GetWeakPtr(), track.track,
                         *track.adapter_settings));
    }
