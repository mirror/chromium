...
void RendererWebMediaPlayerDelegate::OnPictureInPictureStart(
    viz::FrameSinkId frame_sink_id,
    uint32_t parent_id,
    base::UnguessableToken nonce) {
  Send(new MediaPlayerDelegateHostMsg_OnPictureInPictureStarted(
      routing_id(), frame_sink_id, parent_id, nonce));
}
