...
int WebRtcAudioDeviceImpl::GetAuthorizedDeviceSessionIdForAudioRenderer() {
  ...
  ProcessedLocalAudioSource* capturer = capturers_.back();
  const MediaStreamDevice& device = capturer->device();
  if (device.session_id <= 0 || !capturer->render_to_associated_sink())
    return 0;
