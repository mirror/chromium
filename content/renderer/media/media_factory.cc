...
class FrameFetchContext : public media::ResourceFetchContext {
 public:
  ...
  blink::WebString GetOutgoingReferrer() override {
    return frame_->GetDocument().OutgoingReferrer();
  }
  blink::WebReferrerPolicy GetReferrerPolicy() override {
    return frame_->GetDocument().GetReferrerPolicy();
  }
...
blink::WebMediaPlayer* MediaFactory::CreateMediaPlayer(
  ...
  if (!fetch_context_) {
    ...
    DCHECK(!url_index_.get());
    url_index_ = make_linked_ptr(new media::UrlIndex(fetch_context_.get()));
  }
