...
bool PeerConnectionTracker::OnControlMessageReceived(
  ...
  IPC_BEGIN_MESSAGE_MAP(PeerConnectionTracker, message)
    ...
    IPC_MESSAGE_HANDLER(PeerConnectionTracker_StartEventLogFile, OnStartEventLogFile)
    IPC_MESSAGE_HANDLER(PeerConnectionTracker_StartEventLogOutput, OnStartEventLogOutput)
...
void PeerConnectionTracker::OnStartEventLogFile(int peer_connection_id,
...
void PeerConnectionTracker::TrackRtcEventLogWrite(const std::string& log) {
  DCHECK(main_thread_.CalledOnValidThread());
}
