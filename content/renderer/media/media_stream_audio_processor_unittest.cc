...
class MediaStreamAudioProcessorTest : public ::testing::Test {
 ...
 protected:
  ...
  // TODO(guidou): Remove |scoped_feature_list_| once the code for old
  // constraints processing is enabled. Apart from tests for
  // MediaStreamAudioProcessor, this file contains tests for the old audio
  // constraints-processing algorithm. The purpose of |scoped_feature_list_| is
  // this flag is to allow the old audio constraints algorithm to function.
  // MediaStreamAudioProcessor tests are insensitive to the constraints
  // algorithm, so this field can be removed once the old audio-constraints code
  // is removed. http://crbug.com/706408
...
TEST_F(MediaStreamAudioProcessorTest, SelectsConstraintsArrayGeometryIfExists) {
  std::vector<media::Point> constraints_geometry(1,
                                                  media::Point(-0.02f, 0, 0));
