...
class CONTENT_EXPORT MediaStreamVideoSource : public MediaStreamSource {
 public:
  ...
  // with a different. Unlike MediaStreamVideoSource::StopSource(), a temporary
  // stop for restart does not change the ready state of the source.
  ...
  // This method can only be called when a source has started. Any attempt to
  // invoke StopForRestart() when the source has not started results in
  // no action and |callback| invoked with INVALID_STATE.
  ...
  // This method can only be called after a successful stop for restart. Any
  // attempt to invoke Restart() when the source is not in this state results
  // in no action and |callback| invoked with INVALID_STATE.
 ...
 protected:
  ...
  // An implementation that supports restart must immediately stop producing
  // video frames after this method was called. However, the stop is intended
  // to be temporary and to be followed by a restart. Thus, connected tracks
  // should not be disconnected or notified that about the source no longer
  // producing frames.
  ...
  // An implementation that supports restart must try to start producing frames
  // after this method is called. If successful, the source should return to
  // the same state as if it was started normally and invoke OnRestartDone()
  // with true. If the attempt fails, the source should remain stopped for
  // callback and OnRestartDone() should be invoked with false.
  // This method can only be invoked after a successful call to
  // StopSourceForRestartImpl(). Otherwise behavior is undefined.
