...
void UserMediaClientImpl::OnLocalSourceStopped(
  ...
  // TODO(c.padhi): Remove this when StopStreamDevice's input type is changed to
  // MediaStreamDevice, see https://crbug.com/760493.
  StreamDeviceInfo device_info;
  device_info.device = source_impl->device();
  device_info.session_id = device_info.device.session_id;

  media_stream_dispatcher_->StopStreamDevice(device_info);
}
...
void UserMediaClientImpl::StopLocalSource(
  ...
  if (notify_dispatcher) {
    // TODO(c.padhi): Remove this when StopStreamDevice's input type is changed
