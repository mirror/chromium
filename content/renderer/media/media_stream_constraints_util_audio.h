...
//    always known).
// 3. An "device" whose ID is not known (suitable for content capture, where
...
class CONTENT_EXPORT AudioDeviceCaptureCapability {
 public:
  ...
  // This creates a capability where the device ID is limited to |device_id|
  // and other settings are limited by the given |parameters|.
  ...
  // This creates a capability where the device ID and other settings are
  // restricted to the current settings of |source|. Intended to be used by
  // applyConstraints() for both device and content capture, and by
  // getUserMedia() with device capture for devices that are currently in use.
  AudioDeviceCaptureCapability(MediaStreamAudioSource* source);
  ...
  // default values that work well for processing constraints for content
  // capture.
