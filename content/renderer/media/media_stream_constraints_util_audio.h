...
// In general, the algorithm starts with a set containing all possible candidate
// settings based on hardware capabilities (passed via the |capabilities|
...
// failed_constraint_name() method returns the name of the failed constraints or
// an empty string if the set of candidates was initially empty (e.g., if there
// are no devices in the system).
...
// removed.
// After all constraint sets are applied, the result is selected from the
...
//    constraints are ignored. To decide between content or device capture, the
//    value of the special media_stream_source constraint is used.
...
//    processing. This is how audio-processing properties are set (see the
//    content::AudioProcessingProperties struct) for device capture:
...
//    directly from a corresponding boolean constraint, but this is no true for
