...
class RendererWindowTreeClient : public ui::mojom::WindowTreeClient {
 public:
  ...
  // Creates a new window for a frame and embeds the specified client in it.
  std::unique_ptr<MusEmbeddedFrame> Embed(
...
class MusEmbeddedFrame {
 ...
 private:
  ...
  // Stores any state that needs to pushed to the server before the WindowTree
  // has been obtained.
  struct PendingState {
    PendingState();
    ~PendingState();
  ...
  void CreateChildWindowAndEmbed(
      ui::mojom::WindowTreeClientPtr window_tree_client);
  ...
  const ui::ClientSpecificId window_id_ = 0;
