...
RendererWindowTreeClient::OnRenderFrameProxyCreated(
  ...
  if (!pending_frames_.count(render_frame_proxy->routing_id()))
    return nullptr;

  ui::mojom::WindowTreeClientPtr window_tree_client =
      std::move(pending_frames_[render_frame_proxy->routing_id()]);
  pending_frames_.erase(render_frame_proxy->routing_id());
  return CreateMusEmbeddedFrame(render_frame_proxy,
...
RendererWindowTreeClient::CreateMusEmbeddedFrame(
  ...
  std::unique_ptr<MusEmbeddedFrame> frame = base::WrapUnique<MusEmbeddedFrame>(
      new MusEmbeddedFrame(this, render_frame_proxy, ++next_window_id_,
                           std::move(window_tree_client)));
...
void RendererWindowTreeClient::Embed(
  ...
  if (!render_frame_proxy) {
    pending_frames_[frame_routing_id] = std::move(window_tree_client);
    return;
  }
  render_frame_proxy->SetMusEmbeddedFrame(CreateMusEmbeddedFrame(
      render_frame_proxy, std::move(window_tree_client)));
}
...
MusEmbeddedFrame::MusEmbeddedFrame(
  ...
  if (!window_tree()) {
    ...
    pending_state_->window_tree_client = std::move(window_tree_client);
    return;
  }
  CreateChildWindowAndEmbed(std::move(window_tree_client));
}
...
void MusEmbeddedFrame::CreateChildWindowAndEmbed(
  ...
  window_tree()->AddWindow(GetAndAdvanceNextChangeId(),
                           ...
                           window_id_);
  window_tree()->Embed(window_id_, std::move(window_tree_client), 0,
...
void MusEmbeddedFrame::OnTreeAvailable() {
  ...
  CreateChildWindowAndEmbed(std::move(pending_state->window_tree_client));
