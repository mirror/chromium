...
bool LocalStorageCachedArea::SetItem(const base::string16& key,
  ...
  if (!old_nullable_value.is_null())
    old_value = String16ToUint8Vector(old_nullable_value.string());
  leveldb_->Put(String16ToUint8Vector(key), String16ToUint8Vector(value),
                old_value, PackSource(page_url, storage_area_id),
...
void LocalStorageCachedArea::RemoveItem(const base::string16& key,
  ...
  ignore_key_mutations_[key]++;
  base::Optional<std::vector<uint8_t>> old_optional_value;
  if (should_send_old_value_on_mutations_)
    old_optional_value = String16ToUint8Vector(old_value);
  leveldb_->Delete(String16ToUint8Vector(key), old_optional_value,
