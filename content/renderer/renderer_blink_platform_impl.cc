...
class RendererBlinkPlatformImpl::FileUtilities : public WebFileUtilitiesImpl {
 public:
  explicit FileUtilities(
      scoped_refptr<content::mojom::ThreadSafeFileUtilitiesHostPtr> host)
      : file_utilities_host_(std::move(host)) {}
  bool GetFileInfo(const WebString& path, WebFileInfo& result) override;

 private:
  content::mojom::FileUtilitiesHost& GetFileUtilitiesHost() {
    return **file_utilities_host_;
  }

  scoped_refptr<content::mojom::ThreadSafeFileUtilitiesHostPtr>
...
blink::WebFileUtilities* RendererBlinkPlatformImpl::GetFileUtilities() {
  if (!file_utilities_) {
    file_utilities_.reset(new FileUtilities(
        content::mojom::ThreadSafeFileUtilitiesHostPtr::Create(
