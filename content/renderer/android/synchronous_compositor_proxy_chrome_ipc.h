...
class SynchronousCompositorProxyChromeIPC : public SynchronousCompositorProxy {
 ...
 private:
  ...
  void OnDemandDrawHwAsync(const SyncCompositorDemandDrawHwParams& params);
  ...
  void OnSetScroll(const gfx::ScrollOffset& total_scroll_offset);
  ...
  void SendDemandDrawHwReplyAsync(
      const SyncCompositorCommonRendererParams& common_renderer_params,
      uint32_t layer_tree_frame_sink_id,
      base::Optional<viz::CompositorFrame> frame);
