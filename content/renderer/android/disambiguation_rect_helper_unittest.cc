...
class DisambiguationRectHelperUnittest : public testing::Test {
 public:
  DisambiguationRectHelperUnittest()
...
TEST_F(DisambiguationRectHelperUnittest, ClipByViewport) {
  ...
  gfx::Rect zoom_rect;
  float scale = DisambiguationRectHelper::ComputeZoomAreaAndScaleFactor(
  ...
  EXPECT_LE(kDisambiguationRectMinScale, scale);
...
TEST_F(DisambiguationRectHelperUnittest, MiniTarget) {
  ...
  EXPECT_TRUE(gfx::Rect(kVisibleContentSize_).Contains(zoom_rect));
  EXPECT_EQ(kDisambiguationRectMaxScale, scale);
...
TEST_F(DisambiguationRectHelperUnittest, LongLinks) {
