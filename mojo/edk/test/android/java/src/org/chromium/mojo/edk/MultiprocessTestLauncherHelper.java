    ...
    private final MultiprocessTestClientLauncher.Delegate mLauncherDelegate =
            new MultiprocessTestClientLauncher.Delegate() {
                ...
                public List<IBinder> getExtraClientInterfaces() {
                    // Send 2 interfaces:
                    // - the channel the child process will use to send Parcelables
                    // - the interface the child process will pass its channel on (so the parent
                    //   process can receive messages from the child process).
