...
ScopedPlatformHandle PlatformSharedBuffer::GetPlatformHandleForIPC() {
  ...
  if (handle.IsReadOnly()) {
    ...
    // error.
    // NOTE: This should better be a DCHECK(), but this crashes too many tests
    //       while they run to completion when just sealing the region.
    //       For now, just add an error message. Change that into a
    //       DCHECK() later to fix the unit-tests.
    if (!handle.IsRegionReadOnly()) {
      LOG(ERROR) << "Forcing region read-only before Mojo serialization";
