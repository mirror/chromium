...
PlatformChannelPair::PlatformChannelPair(bool client_is_blocking) {
  ...
  mx_status_t result =
      mx_channel_create(0, &handles[0].handle, &handles[1].handle);
  CHECK_EQ(MX_OK, result);

  // TODO(fuchsia): Do something with the |client_is_blocking| flag?
  server_handle_.reset(handles[0]);
  DCHECK(server_handle_.is_valid());
  client_handle_.reset(handles[1]);
...
PlatformChannelPair::PassClientHandleFromParentProcessFromString(
  ...
  return ScopedPlatformHandle(
      PlatformHandle(mx_get_startup_handle(base::checked_cast<uint32_t>(id))));
}
...
PlatformChannelPair::PrepareToPassClientHandleToChildProcessAsString(
  ...
  handle_passing_info->push_back({id, client_handle_.get().handle});
