...
TEST_F(EmbedderTest, PipeSetup) {
  OutgoingBrokerClientInvitation inv;
  auto p0 = inv.AttachMessagePipe("x");
  auto p1 = CreateChildMessagePipe("x");
  WriteMessage(p0.get().value(), "hey");
  EXPECT_EQ("hey", ReadMessage(p1.get().value()));
}
