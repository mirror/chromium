...
class ClaimedPortMap {
 public:
  ClaimedPortMap() {}
  ~ClaimedPortMap() {}

  bool IsClaimed(const ports::PortRef& port) { base::AutoLock lock(lock_); return claimed_ports_.find(port.name()) != claimed_ports_.end(); }
  void Claim(const ports::PortRef& port) { base::AutoLock lock(lock_); claimed_ports_.emplace(port.name()); }

  base::Lock lock_;
  std::set<ports::PortName> claimed_ports_;
};
base::LazyInstance<ClaimedPortMap>::Leaky g_claimed_ports = LAZY_INSTANCE_INITIALIZER;
...
OutgoingBrokerClientInvitation::~OutgoingBrokerClientInvitation() {
  ...
  for (auto& entry : attached_ports_) {
    if (!g_claimed_ports.Get().IsClaimed(entry.second))
      internal::g_core->GetNodeController()->ClosePort(entry.second);
  }
}

// static
void OutgoingBrokerClientInvitation::ClaimPort(const ports::PortRef& port) {
  g_claimed_ports.Get().Claim(port);
}
...
>>>>>>>
no commit */
  internal::g_core->GetNodeController()->StashGlobalReservedPort(name, port);
