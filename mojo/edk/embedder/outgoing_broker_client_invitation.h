...
class MOJO_SYSTEM_IMPL_EXPORT OutgoingBrokerClientInvitation {
 public:
  ...
  static void ClaimPort(const ports::PortRef& port);
  ...
  // Extracts the an attached message pipe endpoint by name. For use only when
  // this invitation will NOT be sent to a remote process (i.e. Send() will
  // never be called) after all, and the caller wishes to retrieve the message
  // pipe endpoint that would have been received.
  //
  // TODO(rockot): Remove this. It's only here to support the singular use case
  // of in-process child threads in //content. With some extra refactoring the
  // usage can be removed entirely.
