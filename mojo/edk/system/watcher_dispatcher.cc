...
MojoResult WatcherDispatcher::WatchDispatcher(
  ...
  bool remove_now;
  {
    // If we've been closed already, there's a chance our closure raced with the
    // call to AddWatcherRef() above. In that case we want to ensure we've
    // removed our ref from |dispatcher|. Note that this may in turn race with
    // normal removal, but that's fine.
    base::AutoLock lock(lock_);
    remove_now = closed_;
  }
