...
int NodeController::SendMessage(
    const ports::PortRef& port,
    std::unique_ptr<ports::UserMessageEvent> message) {
  int rv = node_->SendMessage(port, std::move(message));
  AcceptIncomingMessages();
  return rv;
}
...
void NodeController::AddPeer(const ports::NodeName& name,
  ...
  while (!pending_messages.empty()) {
    channel->SendMessage(std::move(pending_messages.front()));
...
void NodeController::SendPeerEvent(const ports::NodeName& name,
  ...
  if (peer) {
    peer->SendMessage(std::move(event_message));
    return;
  }
...
void NodeController::ForwardEvent(const ports::NodeName& node,
  ...
  if (schedule_pump_task) {
    ...
    // event is done (i.e. SendMessage, ClosePort, etc). However, it's also
...
void NodeController::OnBroadcast(const ports::NodeName& from_node,
  ...
  for (auto& iter : peers_) {
    ...
    iter.second->SendMessage(SerializeEventMessage(std::move(clone)));
  }
}
