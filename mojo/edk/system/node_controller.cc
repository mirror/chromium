...
void NodeController::MergePortIntoParent(const std::string& name,
  ...
  /*
>>>>>>>
no commit just hacking
*/
  bool was_merged = false;
  {
    base::AutoLock lock(reserved_ports_lock_);
    auto it = global_reserved_ports_.find(name);
    if (it != global_reserved_ports_.end()) {
      was_merged = true;
      OutgoingBrokerClientInvitation::ClaimPort(it->second);
      node_->MergePorts(port, name_, it->second.name());
      global_reserved_ports_.erase(it);
    }
  }
  if (was_merged) {
    AcceptIncomingMessages();
