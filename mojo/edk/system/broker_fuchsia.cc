...
Channel::MessagePtr WaitForBrokerMessage(
  ...
  mx_status_t result = mx_object_wait_one(
      platform_handle.handle, MX_CHANNEL_READABLE, MX_TIME_INFINITE, &observed);
  if (result != MX_OK) {
  ...
  result = mx_channel_read(platform_handle.handle, 0,
  ...
  for (size_t i = 0; i < handles_read; ++i)
    incoming_platform_handles.push_back(PlatformHandle(handles[i]));
...
scoped_refptr<PlatformSharedBuffer> Broker::GetSharedBuffer(size_t num_bytes) {
  ...
  mx_status_t result =
      mx_channel_write(sync_channel_.get().handle, 0, out_message->data(),
