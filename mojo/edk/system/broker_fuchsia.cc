...
Channel::MessagePtr WaitForBrokerMessage(
  ...
  // Ensure that |handles| are closed even if validation step, below, fails.
  ...
  // Validate successful read, of the expected amount of data and handles.
  bool error = false;
  if (result != MX_OK) {
...
scoped_refptr<PlatformSharedBuffer> Broker::GetSharedBuffer(size_t num_bytes) {
  ...
  // Write a request for a shared buffer to the sync channel.
  mx_status_t result =
  ...
  // Wait for a result and un-wrap the handles to return to the caller.
  std::deque<PlatformHandle> incoming_platform_handles;
