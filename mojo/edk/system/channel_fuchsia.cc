...
bool UnwrapPlatformHandle(ScopedPlatformHandle handle,
  ...
  if (result == ZX_OK) {
    // Hurray, we transfered! Let the old FD go as it is no longer valid.
    ignore_result(handle.release());
  } else if (result == ZX_ERR_UNAVAILABLE) {
    // No luck, try cloning instead.
    result = fdio_clone_fd(handle.get().as_fd(), 0, handles, info);
    if (result != ZX_OK) {
      DCHECK(false) << "fdio_clone_fd(" << handle.get().as_fd()
                    << "): " << zx_status_get_string(result);
      return false;
    }
  } else {
    DCHECK(false) << "fdio_transfer_fd(" << handle.get().as_fd()
                  << "): " << zx_status_get_string(result);
    return false;
  }
