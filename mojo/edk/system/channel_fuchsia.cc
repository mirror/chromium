...
bool UnwrapPlatformHandle(ScopedPlatformHandle handle,
  ...
  // Only the |PA_HND_TYPE| of the info really matters, hopefully.
...
class ChannelFuchsia : public Channel,
 ...
 public:
  ...
  bool GetReadPlatformHandles(size_t num_handles,
    ...
    // If there are too few handles then we're not ready yes, so return true
    // indicating things are OK, but leave |handles| empty.
    // TODO(wez): Correct this to account for multi-OS-handle file-descriptors.
