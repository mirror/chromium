...
class ChannelFuchsia : public Channel,
 ...
 private:
  ...
  void WaitForWriteOnIOThreadNoLock() {
    ...
    if (io_task_runner_->RunsTasksInCurrentSequence()) {
      ...
      base::MessageLoopForIO::current()->WatchFileDescriptor(
          ...
          base::MessageLoopForIO::WATCH_WRITE, write_watch_.get(), this);
    } else {
      io_task_runner_->PostTask(
          FROM_HERE, base::Bind(&ChannelFuchsia::WaitForWriteOnIOThread, this));
    }
#endif  // 0
  }
  ...
  void OnMxHandleSignalled(mx_handle_t handle, mx_signals_t signals) override {
    CHECK_EQ(handle, handle_.get().handle);

    DCHECK_EQ(MX_CHANNEL_READABLE, signals);
    ...
    do {
      ...
      mx_status_t read_result = mx_channel_read(
          handle_.get().handle, 0, buffer, handles, buffer_capacity,
          arraysize(handles), &bytes_read, &handles_read);
      if (read_result == MX_OK) {
        for (size_t i = 0; i < handles_read; ++i)
          incoming_platform_handles_.push_back(PlatformHandle(handles[i]));
        ...
        LOG(ERROR) << "CHANNEL READ: " << mx_status_get_string(read_result);
        read_error = true;
        break;
      }
  ...
  */
    ...
    do {
      ...
      if (outgoing_handles) {
        for (size_t i = 0; i < outgoing_handles->size(); ++i)
          handles[i] = outgoing_handles->data()[i].handle;
      }

      write_bytes = std::min(message_view.data_num_bytes(),
                             static_cast<size_t>(MX_CHANNEL_MAX_MSG_BYTES));
      mx_status_t result =
          mx_channel_write(handle_.get().handle, 0, message_view.data(),
                           write_bytes, handles, handles_count);
      if (result != MX_OK) {
        LOG(ERROR) << "CHANNEL WRITE: " << mx_status_get_string(result);
        return false;
        // TODO: Is there an EAGAIN equivalent when writing to Fuchsia channels?
      ...
      if (outgoing_handles) {
        ...
        for (auto& outgoing_handle : *outgoing_handles) {
          outgoing_handle.handle = MX_HANDLE_INVALID;
        }
      }

      LOG(ERROR) << "WRITE-BYTES: " << write_bytes
                 << ", MSG BYTES: " << message_view.data_num_bytes();
    } while (write_bytes < message_view.data_num_bytes());

    LOG(ERROR) << "WROTE MESSAGE";
  ...
  std::unique_ptr<base::MessageLoopForIO::MxHandleWatchController> write_watch_;
