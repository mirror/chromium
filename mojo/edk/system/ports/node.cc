...
int Node::PrepareToForwardUserMessage(const PortRef& forwarding_port_ref,
  ...
  // Accumulate local PortRefs for every port attached to the message.
  ...
  // Now lock the forwarding port and all attached ports. Note that we can't
  // lock attached ports until we have stable local PortRef storage for all of
  // them, hence we do this in a separate step from above.
