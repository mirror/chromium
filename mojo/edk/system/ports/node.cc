...
int Node::PrepareToForwardUserMessage(const PortRef& forwarding_port_ref,
  ...
  for (;;) {
    ...
    // We need to retry this loop if, since we last held the forwardnig port's
    // lock, its peer node has changed from this local node to some other node.
    if (forwarding_port->peer_node_name != target_node_name &&
        target_node_name == name_)
      continue;
