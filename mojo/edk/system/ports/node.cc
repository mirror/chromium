...
int Node::OnObserveProxyAck(std::unique_ptr<ObserveProxyAckEvent> event) {
  ...
  bool try_remove_proxy_immediately;
  {
    ...
    // retrainsmit the ObserveProxy event for this port rather than flagging the
...
int Node::PrepareToForwardUserMessage(const PortRef& forwarding_port_ref,
  ...
  // We must lock the port from which we're forwarding as well as any ports
  ...
  ports_to_lock.container().emplace_back(&forwarding_port_ref);
  for (size_t i = 0; i < message->num_ports(); ++i) {
    PortRef attached_port_ref;
    GetPort(message->ports()[i], &attached_port_ref);
    DCHECK(attached_port_ref.is_valid());
    attached_port_refs.container().emplace_back(std::move(attached_port_ref));
    ports_to_lock.container().push_back(&attached_port_refs.container().back());
  }
