...
public class CoreImpl implements Core {
    ...
    ResultAnd<MessagePipeHandle.ReadMessageResult> readMessage(
        ...
        int[] rawHandles = readResult.getRawHandles();
        if (rawHandles != null && rawHandles.length != 0) {
            List<UntypedHandle> handles = new ArrayList<UntypedHandle>(rawHandles.length);
            for (int rawHandle : rawHandles) {
                handles.add(new UntypedHandleImpl(this, rawHandle));
            }
            readResult.setHandles(handles);
        } else {
            readResult.setHandles(new ArrayList<UntypedHandle>(0));
        }
    ...
    private static ResultAnd<MessagePipeHandle.ReadMessageResult> newReadMessageResult(
        ...
        if (mojoResult == MojoResult.OK) {
            result.setData(data);
            result.setRawHandles(rawHandles);
        }
