...
TEST_F(TestHelperTest, AsyncWaiter) {
  ...
  const std::string kTestString =
      "a machine that goes 'ping'" std::string response;
  ...
  test::HandleTrampolineAsyncWaiter wait_for_trampoline;
  MessagePipe pipe;
  ScopedMessagePipeHandle handle0, handle1;
  WriteMessgaeRaw(pipe.handle0.get(), kTestString.data(), kTestString.size(),
