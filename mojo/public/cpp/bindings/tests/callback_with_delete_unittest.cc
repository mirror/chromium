...
#include "mojo/public/cpp/bindings/callback_with_delete.h"
...
TEST(CallbackWithDeleteTest, SetIntegers_Run) {
  int a = 0;
  int b = 0;
  auto cb = CallbackWithDelete(base::BindOnce(&SetIntegers, &a, &b),
...
TEST(CallbackWithDefaultTest, CallClosure_Run) {
  int a = 0;
  int b = 0;
  auto cb = CallbackWithDefault(base::BindOnce(&CallClosure),
                                base::BindOnce(&SetIntegers, &a, &b, 3, 4));
...
TEST(CallbackWithDefaultTest, Closure_Run) {
  bool a = false;
  auto cb = CallbackWithDefault(base::BindOnce(&SetBool, &a, true));
  std::move(cb).Run();
  EXPECT_TRUE(a);
}
...
TEST(CallbackWithDefaultTest, SetBoolFromRawPtr_Run) {
  ...
  bool c = true;
  auto cb = CallbackWithDefault(base::BindOnce(&SetBoolFromRawPtr, &a),
                                base::Owned(b));
  std::move(cb).Run(&c);
  EXPECT_TRUE(a);
}
...
TEST(CallbackWithDefaultTest, SetIntegers_Run) {
  int a = 0;
  int b = 0;
  auto cb = CallbackWithDefault(base::BindOnce(&SetIntegers, &a, &b), 3, 4);
  std::move(cb).Run(1, 2);
  EXPECT_EQ(a, 1);
  EXPECT_EQ(b, 2);
}
...
TEST(CallbackWithDefaultTest, SetIntegerFromUniquePtr_Run) {
  int a = 0;
  auto cb = CallbackWithDefault(base::BindOnce(&SetIntegerFromUniquePtr, &a),
...
TEST(CallbackWithDefaultTest, SetString_Run) {
  std::string a;
  auto cb = CallbackWithDefault(base::BindOnce(&SetString, &a), "hello");
