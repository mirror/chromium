 ...
 public:
  TestSyncServiceSequencedPool()
      : task_runner_(base::CreateSequencedTaskRunnerWithTraits(
            base::TaskTraits().WithBaseSyncPrimitives())),
        ping_called_(false) {}
...
TYPED_TEST(SyncMethodCommonTest, BasicSyncCallsOnSequencedTaskRunner) {
  ...
  InterfacePtr<InterfaceType> ptr;

  TestSyncServiceSequencedPool<InterfaceType> service_pool;
  service_pool.task_runner()->PostTask(
      FROM_HERE, base::Bind(&TestSyncServiceSequencedPool<InterfaceType>::SetUp,
  ...
  service_pool.task_runner()->PostTaskAndReply(
      FROM_HERE,
      base::Bind(&TestSyncServiceSequencedPool<InterfaceType>::TearDown,
                 base::Unretained(&service_pool)),
      run_loop.QuitClosure());
  run_loop.Run();
}
...
SEQUENCED_TASK_RUNNER_TYPED_TEST_F(
  ...
  void Run() override {
    ...
    this->impl_ = base::MakeUnique<ImplTypeFor<InterfaceType>>(
        MakeRequest<InterfaceType>(std::move(pipe.handle1)));
