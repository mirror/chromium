  ...
  class ForwardToCallingThread : public MessageReceiver {
      ...
      if (response_message_handler_) {
        caller_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(
                           [](base::OnceCallback<void(Message * message)>
                                  response_message_handler) {
                             std::move(response_message_handler).Run(nullptr);
                           },
                           base::Passed(&response_message_handler_)));
      }
    }
   ...
   private:
    bool Accept(Message* message) override {
      ...
      // have to relinquish |response_message_handler_|'s ownership so it does
      // not get deleted.
      caller_task_runner_->PostTask(
          FROM_HERE, base::BindOnce(
                         [](base::OnceCallback<void(Message * message)>
                                response_message_handler,
                            Message message) {
                           std::move(response_message_handler).Run(&message);
                         },
                         base::Passed(&response_message_handler_),
