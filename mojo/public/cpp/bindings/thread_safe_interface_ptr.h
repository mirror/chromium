...
class ThreadSafeForwarder : public MessageReceiverWithResponder {
 ...
 private:
  ...
  // Just removes |request_id| entry from |async_responders_| for null
  // |message|.
  void HandleAsyncResponseMessage(uint64_t request_id,
    ...
    if (message)
      ignore_result(responder->Accept(message.get()));
  }
  ...
  class ForwardToCallingThread : public MessageReceiver {
   public:
    explicit ForwardToCallingThread(
        base::OnceCallback<void(std::unique_ptr<Message> message)>
    ...
    ~ForwardToCallingThread() override {
      if (response_message_handler_) {
        caller_task_runner_->PostTask(
            FROM_HERE,
            base::BindOnce(std::move(response_message_handler_), nullptr));
      }
    }
   ...
   private:
    bool Accept(Message* message) override {
      caller_task_runner_->PostTask(
          FROM_HERE,
          base::BindOnce(
              std::move(response_message_handler_),
              base::Passed(std::make_unique<Message>(std::move(*message)))));
      return true;
    }

    base::OnceCallback<void(std::unique_ptr<Message> message)>
        response_message_handler_;
