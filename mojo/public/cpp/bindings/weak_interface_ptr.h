...
#ifndef MOJO_PUBLIC_CPP_BINDINGS_WEAK_INTERFACE_PTR_H_
#define MOJO_PUBLIC_CPP_BINDINGS_WEAK_INTERFACE_PTR_H_
...
// an InterfaceInvalidator when bound to a message pipe. The interface will be
// automatically closed once the InterfaceInvalidator is destroyed and will
// appear to be in an error state. This is useful for tying the lifetime of
// interface pointers to another object.
template <typename Interface>
class WeakInterfacePtr : public InterfaceInvalidator::Observer {
 public:
  using PtrType = InterfacePtr<Interface>;
  using PtrInfoType = InterfacePtrInfo<Interface>;
  using Proxy = typename Interface::Proxy_;

  // Constructs an unbound WeakInterfacePtr.
  WeakInterfacePtr() {}
  WeakInterfacePtr(decltype(nullptr)) {}
  ...
  explicit WeakInterfacePtr(PtrInfoType&& info,
  ...
  // Returns whether or not this WeakInterfacePtr is bound to a message pipe.
  bool is_bound() const { return interface_ptr_.is_bound(); }
  ...
  // used to setup an WeakInterfacePtr again. See comments on
  ...
  bool Equals(const WeakInterfacePtr& other) const {
    if (this == &other)
      return true;
  ...
  internal::InterfacePtrState<Interface>* internal_state() {
    return interface_ptr_.internal_state();
  }

  // Allow WeakInterfacePtr<> to be used in boolean expressions.
  explicit operator bool() const { return interface_ptr_.is_bound(); }

  // InterfaceInvalidator::Observer
  void OnInvalidate() override {
    interface_ptr_.RaiseError();
    invalidator_->RemoveObserver(this);
    invalidator_.reset();
  }
...
#endif  // MOJO_PUBLIC_CPP_BINDINGS_WEAK_INTERFACE_PTR_H_
