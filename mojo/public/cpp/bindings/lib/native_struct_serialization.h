...
struct NativeStructSerializerImpl {
  ...
  static bool Deserialize(native::internal::NativeStruct_Data* data,
...

    {
      ...
      // NOTE: We are sneaky and we give the reader an IPC::Message which does
      // not correspond to the PickleIterator. This is because readers only use
      // the Message argument to extract attachments in practice.
