...
class MOJO_CPP_BINDINGS_EXPORT UnserializedMessageContext {
 public:
  ...
  MessageType* SafeCast() {
    DCHECK_EQ(&MessageType::kMessageTag, tag_);
  ...
  void set_total_serialized_size(size_t sz) { total_serialized_size_ = sz; }
  size_t total_serialized_size() const { return total_serialized_size_; }

  void GetSerializedSize(size_t* payload_size, size_t* num_handles);
