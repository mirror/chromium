...
### Best practices for dealing with process crashes
A common situation when two endpoints are in different processes is that a
caller wants to know when the other process is torn down. This is especially
important if a method with a callback is run. In that case, the consumer usually
wants to know if the response callback won't be run because the pipe is torn
down. There are different solutions for this problem, depending on how the
`InterfacePtr<T>` is held:
...
2. The consumer doesn't own the `InterfacePtr<T>`: there are two helpers
   ...
   helper should be used. With both of these helpers, the usual callback care
   should be followed to ensure that the callbacks don't run after the consumer
   is destructed (e.g. because the owner of the `InterfacePtr<T>` outlives the
