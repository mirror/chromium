...
struct MojoMessageOperationThunks {
  ...
  // followed immediately by |get_handles()| for the same |context|.
  //
  ...
  // Tentatively passes ownership of any MojoHandles carried by the message
  // associated with |context|. Called immediately after |get_serialized_size()|
  // for the same |context|, if and only if the returned |*num_handles| was
  // non-zero.
  //
  // The implementation must populate |handles| with a contiguous MojoHandle
  // array of exactly |*num_handles| (from above) elements. Ownership of all
  // returned handles is retained by the context unless a subsequent call to
  // |commit_handles| (below) is made for the |context|.
  void (*get_handles)(uintptr_t context, MojoHandle* handles);

  // Commits ownership transfer of any handles returned by |get_handles()|
  // from the context to the serialized message object being built.
  void (*commit_handles)(uintptr_t context);

  // Serializes the message payload bytes into a message buffer. This always
  // follows a call to |get_serialized_size()| (and possibly
  // |get+commit_handles()|) for the same |context|, if and only if |*num_bytes|
...
//       instead call |MojoReleaseMessageContext()| to simultaneously free the
//       message object and retrieve its context value.
...
// Retrieves the context value from a message and frees the message object.
// This value of |context| was provided in a prior call to |MojoCreateMessage()|
// for the given |message| handle. This may be used to recover a message object
// sent from within the same process as the receiver, without incurring any
// serialization or deserialization cost.
//
// Note that if this call is successful, |message| is no longer valid and it is
// neither necessary nor correct to then call |MojoFreeMessage()| on it.
//
// Returns:
//   |MOJO_RESULT_OK| if |message| was a valid message object which has not yet
//       been serialized. |*context| contains the context value originally
//       provided to the |MojoCreateMessage()| call which created |message|.
//       |message| is freed and no longer a valid handle.
