...
* `mojo.InterfacePtrInfo` and `mojo.InterfaceRequest` encapsulate two ends of a
  message pipe. They represents the client end and service end of an interface
...
* `mojo.Binding` owns an `InterfaceRequest`. It listens on the message pipe
  handle and dispatches incoming messages to an user-defined interface
  implementation.
...
endpoint (wrapped in an `InterfaceRequest`), dispatching incoming messages to
the implementation.
...
* `mojo.AssociatedBinding` owns an `AssociatedInterfaceRequest`. It listens on
  the connection and dispatches incoming messages to an user-defined interface
  implementation.
...
`foo.mojom.js`. For exmple, if at build time the two Mojom files are located at:
```
a/b/c/foo.mojom
a/b/d/bar.mojom
```
...
the generated JavaScript bindings conforms to our JavaScript style guide. We
convert:
