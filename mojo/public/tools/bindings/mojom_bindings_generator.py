    ...
    if self._should_generate(rel_filename.path):
      ...
      for language, generator_module in generator_modules.iteritems():
        generator = generator_module.Generator(
            ...
            no_generate_message_ids=args.no_generate_message_ids,
            generate_message_ids_only=args.generate_message_ids_only)
        filtered_args = []
...
def main():
  ...
  generate_parser.add_argument(
      "--no_generate_message_ids",
      help="Disables generation of the message IDs header normally generated "
      "for C++ bindings. Generated bindings depend on this header, but it's "
      "possible to generate them as a separate build step using "
      "--generate_message_ids_only. Note that flag only matters if "
      "--generate_non_variant_code is also specified.", action="store_true")
  generate_parser.add_argument(
      "--generate_message_ids_only",
