...
class MojomProcessor(object):
  ...
  def _GenerateModule(self, args, remaining_args, generator_modules,
    ...
    pickle_path = _PicklePath(rel_filename, args.output_dir)
    try:
      with open(pickle_path, "rb") as f:
        tree = pickle.load(f)
    except Error as e:
      print "%s: Error: %s" % (pickle_path, e.strerror)
      sys.exit(1)
...
def _PicklePath(rel_filename, output_dir):
  module_path = rel_filename.relative_path().replace("\\", "/")
  pickle_path = module_path.replace(".mojom", ".p")
...
def _ParseFile(args, rel_filename):
  ...
  _PickleAST(tree, _PicklePath(rel_filename, args.output_dir))
...
def main():
  ...
  parse_parser = subparsers.add_parser(
      "parse", description="Parse mojom to AST, removing disabled definitions.")
