...
:   The `EnabledIf` attribute is used to conditionally disable definitions at
    interface generation time. If the `mojom` target in the GN file does not
    include the matching `value` in the list of `enabled_flags`, the definition
    will be disabled. This is useful for mojom definitions that only make sense
    on one platform.
