...
def PreprocessSource(input_source, filename, buildflags, defines):
  ...
  emit_stack = [True]
  for line in lines:
    # Every input line always generates a corresponding (but potentially empty)
    # output line to ensure line numbers match after preprocessing.
    output_lines.append('')
    # Not a preprocessor directive: just copy the line to the output if not
    ...
    if line == '#endif':
      emit_stack.pop()
      if not emit_stack:
        raise ParseError(filename, 'Found #endif without matching #if',
                         len(output_lines))
      continue
    ...
    if not match:
      ...
      emit_stack.pop()
      if not emit_stack:
        raise ParseError(filename, 'Found #elif without matching #if',
                         len(output_lines))

    if match:
      emit_stack.append(
          _ProcessConditional(
              filename, len(output_lines), match.group(1), buildflags, defines))
      continue

    if line == '#else':
      if len(emit_stack) == 1:
        raise ParseError(filename, 'Found #else without matching #if',
                         len(output_lines))
