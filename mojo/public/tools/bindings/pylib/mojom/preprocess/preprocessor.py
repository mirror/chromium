...
def PreprocessSource(input_source, filename, buildflags, defines):
  ...
  output_lines = []
  # Tracks whether or not the current preprocessor "block" should suppress
  # output. By definition, the top-level block never suppresses.
  suppression_stack = [False]
  lineno = 0
  for line in lines:
    lineno += 1
    ...
    if line == '#endif':
      suppression_stack.pop()
      if not suppression_stack:
        # TODO(dcheng): Throw an error.
        pass
      continue
    ...
    if match:
      suppression_stack.append(
          _ProcessConditional(filename, lineno,
                              match.group(1), buildflags, defines))
      continue

    if line == '#else':
      if len(suppression_stack) == 1:
        # TODO(dcheng): Throw an error.
        pass
      suppression_stack[-1] = not suppression_stack[-1]
      continue
  return '\n'.join(output_lines)
