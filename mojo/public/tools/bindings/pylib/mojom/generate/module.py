...
def _AnyMethodParameterRecursive(method, predicate, visit_map_keys=False,
                                 visited_kinds=None):
    ...
    if IsStructKind(kind) or IsUnionKind(kind):
      for field in kind.fields:
        if _HasProperty(field.kind):
          return True
    if IsMapKind(kind):
      if visit_map_keys and _HasProperty(kind.key_kind):
        return True
      return _HasProperty(kind.value_kind)
    return False
