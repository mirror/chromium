...
class DiskMountManagerImpl : public DiskMountManager {
 ...
 private:
  // Stores new volume name for a device on which renaming is invoked and set it
  // back to a disk of the device to synchronize changes in OnRenameCompleted.
  // |key| is a device_path and |value| is a new volume_name.
  ...
  void OnRenameCompleted(RenameError error_code,
    ...
    if (iter != disks_.end()) {
      ...
      if (pending_change != pending_rename_changes_.end()) {
        ...
        pending_rename_changes_.erase(pending_change);
      }
    }
