...
class DiskMountManagerImpl : public DiskMountManager {
 ...
 private:
  std::map<std::string, std::string> pendingRenameChanges_;
  ...
  void RenameUnmountedDevice(const std::string& device_path,
    ...
    pendingRenameChanges_[device_path] = volume_name;
  ...
  void OnRenameCompleted(RenameError error_code,
    ...
    if (iter != disks_.end()) {
      ...
      auto pendingChange = pendingRenameChanges_.find(device_path);
      if (pendingChange != pendingRenameChanges_.end()) {
        disk->set_device_label(pendingChange->second);
        pendingRenameChanges_.erase(pendingChange);
      }
    }
