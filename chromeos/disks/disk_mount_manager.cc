...
class DiskMountManagerImpl : public DiskMountManager {
 ...
 private:
  std::map<std::string, FormatChange> pendingFormatChanges_;
  ...
  void FormatUnmountedDevice(const std::string& device_path) {
    ...
    pendingFormatChanges_[device_path] =
        FormatChange(kDefaultFormatVFAT, kDefaultFormattedDeviceName);
  ...
  void OnFormatCompleted(FormatError error_code,
    ...
    if (iter != disks_.end()) {
      ...
      auto pendingChange = pendingFormatChanges_.find(device_path);
      if (pendingChange != pendingFormatChanges_.end()) {
        disk->set_device_label(pendingChange->second.volume_name());
        disk->set_file_system_type(pendingChange->second.file_system_type());
        pendingFormatChanges_.erase(pendingChange);
      }
    }
