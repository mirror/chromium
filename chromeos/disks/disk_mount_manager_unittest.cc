...
const char kDisk1VolumeName[] = "MYUSB";
const char kDisk1FileSystemType[] = "ntfs";
const char kDisk2FileSystemType[] = "exfat";
...
const TestDiskInfo kTestDisks[] = {
    ...
    {kDevice2SourcePath,
     ...
     true,        // is on removable device
     false,       // is hidden
     kDisk2FileSystemType},
    {kReadOnlyDeviceSourcePath, kReadOnlyDeviceMountPath,
     false,  // write_disabled_by_policy
     "/device/prefix/system_path_3", "/device/file_path_3",
     "/device/device_label_3", "/device/drive_label_3", "/device/vendor_id_3",
     "/device/vendor_name_3", "/device/product_id_3", "/device/product_name_3",
     "/device/fs_uuid_3", "/device/prefix", chromeos::DEVICE_TYPE_USB,
     1073741824,  // size in bytes
     false,       // is parent
     true,        // is read only
     true,        // has media
...
TEST_F(DiskMountManagerTest, Format_FormatFails) {
  ...
  fake_cros_disks_client_->SendFormatCompletedEvent(
      chromeos::FORMAT_ERROR_UNKNOWN, kDevice1SourcePath, kDisk1VolumeName, kDisk1FileSystemType);
...
TEST_F(DiskMountManagerTest, Format_FormatSuccess) {
  ...
  fake_cros_disks_client_->SendFormatCompletedEvent(chromeos::FORMAT_ERROR_NONE,
                                                    kDevice1SourcePath, kDisk1VolumeName, kDisk1FileSystemType);
