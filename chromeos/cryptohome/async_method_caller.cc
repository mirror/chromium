...
class AsyncMethodCallerImpl : public AsyncMethodCaller,
 ...
 private:
  ...
  // Similar to HandleAsyncResponse but for signals with a raw data payload.
  void AsyncCallStatusForPrivacyCA(int async_id,
      chromeos::attestation::PrivacyCAOperationStatus return_status,
      const std::string& return_data) override {
    ...
    it->second.task_runner->PostTask(
        FROM_HERE,
        base::Bind(it->second.privacy_ca_callback, return_status, return_data));
  ...
  void RegisterAsyncPrivacyCACallback(PrivacyCACallback callback,
    ...
    if (!async_id.has_value()) {
      base::ThreadTaskRunnerHandle::Get()->PostTask(
          FROM_HERE, base::BindOnce(callback,
              chromeos::attestation::FAILURE,  // return status
              std::string()));
      return;
    }
    if (async_id.value() == 0) {
