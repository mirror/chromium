...
// TODO(crbug.com/797848): Add tests that cover this logic.
void KeyDefAuthDataToKeyAuthData(
    const KeyDefinition::AuthorizationData& key_def_auth_data,
    KeyAuthorizationData* auth_data) {
  switch (key_def_auth_data.type) {
...
void KeyDefinitionToKey(const KeyDefinition& key_def, Key* key) {
  ...
  for (const auto& key_def_auth_data : key_def.authorization_data) {
    ...
    KeyDefAuthDataToKeyAuthData(key_def_auth_data, auth_data);
