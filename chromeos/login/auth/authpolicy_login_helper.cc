...
struct EncryptionType {
  const char* title;
  bool pre_selected;
  authpolicy::KerberosEncryptionTypes kerberos_enc_type;
};

const EncryptionType kEncryptionTypes[] = {
...
AuthPolicyLoginHelper::GetEncryptionTypeList() {
  ...
  for (unsigned long i = 0; i < arraysize(kEncryptionTypes); ++i) {
    auto enc_option = std::make_unique<base::DictionaryValue>();
    enc_option->SetInteger(
        "value", static_cast<int>(kEncryptionTypes[i].kerberos_enc_type));
