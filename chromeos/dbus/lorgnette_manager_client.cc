 ...
 private:
  ...
  class ScanDataReader {
   public:
    ...
    // nullopt.
    using ScanResult = base::Optional<std::string>;
    using CompletionCallback = base::OnceCallback<void(ScanResult)>;
   ...
   private:
    ...
    void OnDataRead(ScanResult data) {
      DCHECK(!data_.has_value());
    ...
    void MaybeCompleted() {
      ...
      if (!data_.has_value() || callback_.is_null())
        return;

      std::move(callback_).Run(std::move(data_).value());
    }

    std::unique_ptr<chromeos::PipeReader> pipe_reader_;
    base::Optional<ScanResult> data_;
