...
class CryptohomeClientImpl : public CryptohomeClient {
 public:
  ...
  void RenameCryptohome(const cryptohome::Identification& cryptohome_id_from,
    ...
    proxy_->CallMethod(
        &method_call, kTpmDBusTimeoutMs,
        base::BindOnce(&CryptohomeClientImpl::OnBaseCallbackMethod,
                       weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
  }
 ...
 private:
  ...
  void OnBaseCallbackMethod(Callback callback, dbus::Response* response) {
    if (!response) {
    ...
    if (reply.has_error() &&
      ...
      std::move(callback).Run(false, MapError(reply.error()));
      return;
    }
    std::move(callback).Run(true, cryptohome::MOUNT_ERROR_NONE);
  }
