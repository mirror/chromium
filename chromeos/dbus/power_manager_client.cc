...
class PowerManagerClientImpl : public PowerManagerClient {
 public:
  ...
  void CreateArcTimers(const std::vector<ArcTimerArgs>& arc_timers_args,
    ...
    for (const ArcTimerArgs& args : arc_timers_args) {
      ...
      struct_writer.AppendFileDescriptor(args.expiry_indicator_fd.get());
  ...
  // otherwise.
  void DeleteArcTimers(const DeleteArcTimersCallback& callback) {
    dbus::MethodCall method_call(power_manager::kPowerManagerInterface,
    ...
    power_manager_proxy_->CallMethod(
        &method_call, dbus::ObjectProxy::TIMEOUT_USE_DEFAULT,
        base::Bind(&PowerManagerClientImpl::HandleDeleteArcTimersCallback,
                   weak_ptr_factory_.GetWeakPtr(), callback));
  }
 ...
 private:
  ...
  void HandleCreateArcTimersCallback(CreateArcTimersCallback callback,
    ...
    if (!response) {
      ...
      std::move(callback).Run(
          static_cast<power_manager::ArcTimerResult>(result));
      return;
    }
  ...
  void HandleSetArcTimerCallback(SetArcTimerCallback callback,
                                 dbus::Response* response) {
    int32_t result = static_cast<int32_t>(
        power_manager::ArcTimerResult::ARC_TIMER_RESULT_FAILURE);
    if (!response) {
  ...
  void HandleDeleteArcTimersCallback(DeleteArcTimersCallback callback,
                                     dbus::Response* response) {
    int32_t result = static_cast<int32_t>(
        power_manager::ArcTimerResult::ARC_TIMER_RESULT_FAILURE);
    if (!response) {
