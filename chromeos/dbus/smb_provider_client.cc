...
int32_t GetErrorAndProto(dbus::Response* response,
...
class SmbProviderClientImpl : public SmbProviderClient {
 ...
 private:
  ...
  template <class T>
  void HandleCallback(
      base::OnceCallback<void(int32_t error, const T& response)> callback,
      dbus::Response* response) {
    T proto;
    int32_t error(GetErrorAndProto(response, &proto));
