...
using EntryDataMethodCallback =
    base::Callback<void(int32_t error,
                        std::vector<EntryData> readDirResult)>;

using EntryMetaDataMethodCallback = base::Callback<void(int32_t error, EntryData result)>;

//using FileDescriptorMethodCallback = base::Callback < vo

class CHROMEOS_EXPORT SmbClientClient : public DBusClient {
 public:
  ...
  virtual void CommunicateToService(int32_t mount_id, const std::string& directoryPath,
                                    EntryDataMethodCallback callback) = 0;

  virtual void GetMetadata(int32_t mount_id, const std::string& entry_path, EntryMetaDataMethodCallback callback) = 0;
