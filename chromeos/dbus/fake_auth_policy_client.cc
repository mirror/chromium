...
void FakeAuthPolicyClient::GetUserStatus(const std::string& object_guid,
  ...
  if (!display_name_.empty())
    user_status.mutable_account_info()->set_display_name(display_name_);
  if (!given_name_.empty())
    user_status.mutable_account_info()->set_given_name(given_name_);
  ...
  PostDelayedClosure(
      ...
      operation_delay_);
  if (!on_get_status_.is_null())
    PostDelayedClosure(std::move(on_get_status_), operation_delay_);
}
