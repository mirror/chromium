...
FakeSessionManagerClient::FakeSessionManagerClient(bool emit_observers,
                                                   bool use_host_stub_policy)
    : start_device_wipe_call_count_(0),
      ...
      delegate_(nullptr),
      use_host_stub_policy_(use_host_stub_policy),
      emit_observers_(emit_observers),
      weak_ptr_factory_(this) {}

FakeSessionManagerClient::FakeSessionManagerClient()
    : FakeSessionManagerClient(true, false) {}
  ...
  if (use_host_stub_policy_) {
    ...
    if (policy.has_new_public_key()) {
      base::PostTaskWithTraits(
          FROM_HERE,
          {base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN},
          base::BindOnce(&StoreFile, owner_key_path, policy.new_public_key()));
    }
