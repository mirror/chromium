...
class SmbClientClientImpl : public SmbClientClient {
 public:
  ...
  void ReadDirectory(int32_t mount_id, const std::string& directoryPath,
  ...
  void GetMetadata(int32_t mount_id, const std::string& entry_path, EntryMetaDataMethodCallback callback) {
    dbus::MethodCall method_call(smbclient::kSmbClientInterface, smbclient::kGetMetadata);
    dbus::MessageWriter writer(&method_call);
    writer.AppendInt32(mount_id);
    writer.AppendString(entry_path);
    proxy_->CallMethod(&method_call, dbus::ObjectProxy::TIMEOUT_USE_DEFAULT,
                       base::Bind(&SmbClientClientImpl::HandleEntryCallback,
                                  weak_ptr_factory_.GetWeakPtr(),
                                  base::Passed(std::move(callback))));
  }
 ...
 private:
  ...
  void HandleJoinCallback(EntryDataMethodCallback callback,
    ...
    LOG(ERROR) << "HANDLING CALLLLL BACKKKK";
    ...
    LOG(ERROR) << "Successfully handled join callback";
    std::move(callback).Run(error, results);
  }

  void HandleEntryCallback(EntryMetaDataMethodCallback callback,
                          dbus::Response* response) {
    EntryData result;
    if (!response) {
      LOG(ERROR) << "Join couldn't call SMB Client";
