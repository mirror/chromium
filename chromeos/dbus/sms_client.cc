 ...
 private:
  void OnPropertyChanged(const std::string& property_name) {
    ...
    if (number_.is_valid() && text_.is_valid() && timestamp_.is_valid()) {
      ...
      // once. As |callback_| may destruct this object, schedule it to the task
      // runner to run after this method returns.
      base::ThreadTaskRunnerHandle::Get()->PostTask(
          FROM_HERE, base::Bind(std::move(callback_), std::move(sms)));
      return;
    }
    ...
    if (state_.is_valid() && state_.value() == kSMSStateReceived &&
        !sms_received_) {
      sms_received_ = true;
      // As GetSMSProperties() modifies |property_set_|, schedule it to the
      // task runner to run after this method returns.
      base::ThreadTaskRunnerHandle::Get()->PostTask(
          FROM_HERE, base::Bind(&SMSReceiveHandler::GetSMSProperties,
                                weak_ptr_factory_.GetWeakPtr()));
    }
  }

  void GetSMSProperties() {
