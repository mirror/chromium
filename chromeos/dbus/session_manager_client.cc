...
base::ScopedFD GetPasswordPipe(const std::string& data) {
  int pipe_fds[2];
  ...
  base::WriteFileDescriptor(pipe_write_end.get(), (char*)&data_size,
                            sizeof(size_t));
...
class SessionManagerClientImpl : public SessionManagerClient {
 public:
  ...
  void SaveLoginPassword(const std::string& password) override {
    ...
    base::ScopedFD fd = GetPasswordPipe(password);
