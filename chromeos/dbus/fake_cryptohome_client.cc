...
void FakeCryptohomeClient::TpmAttestationGetKeyPayload(
  ...
  if (key_type == attestation::KEY_DEVICE) {
    const auto it = device_payload_map_.find(key_name);
    if (it != device_payload_map_.end()) {
      result = true;
      payload = it->second;
    }
  }
...
void FakeCryptohomeClient::TpmAttestationSetKeyPayload(
    ...
    const BoolDBusMethodCallback& callback) {
  bool result = false;
  // Currently KEY_DEVICE case is supported just because there's no user for
  // KEY_USER.
  if (key_type == attestation::KEY_DEVICE) {
    device_payload_map_[key_name] = payload;
    result = true;
  }
  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE, base::BindOnce(callback, DBUS_METHOD_CALL_SUCCESS, result));
...
FakeCryptohomeClient::GetTpmAttestationDeviceKeyPayload(
  ...
  const auto it = device_payload_map_.find(key_name);
  return it == device_payload_map_.end() ? base::nullopt
