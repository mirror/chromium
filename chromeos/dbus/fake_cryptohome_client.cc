...
void FakeCryptohomeClient::TpmAttestationDoesKeyExist(
  ...
  if (key_type == attestation::KEY_USER) {
    result = base::ContainsKey(certificate_map_,
                               CertificateKey(cryptohome_id, key_name));
  }
...
void FakeCryptohomeClient::TpmAttestationGetCertificate(
  ...
  if (key_type == attestation::KEY_USER) {
    auto it = certificate_map_.find(CertificateKey(cryptohome_id, key_name));
...
void FakeCryptohomeClient::SetTpmAttestationCertificate(
    attestation::AttestationKeyType key_type,
    const cryptohome::Identification& cryptohome_id,
    const std::string& key_name,
    const std::string& certificate) {
  if (key_type != attestation::KEY_USER) {
    // No usage at the moment. Skip implementation for now.
    NOTIMPLEMENTED();
    return;
  }

  certificate_map_.insert(
      std::make_pair(CertificateKey(cryptohome_id, key_name), certificate));
}
