...
class CHROMEOS_EXPORT NetworkCertificateHandler : public CertLoader::Observer {
 public:
  ...
  void SetCertificatesForTest(
      const net::ScopedCERTCertificateVector& cert_list);
  void NotifyCertificatsChangedForTest();

 private:
  // CertLoader::Observer
  void OnCertificatesLoaded(const net::ScopedCERTCertificateVector& cert_list,
                            bool initial_load) override;

  void ProcessCertificates(const net::ScopedCERTCertificateVector& cert_list);
