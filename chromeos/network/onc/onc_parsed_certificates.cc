...
bool OncParsedCertificates::HasSameServerOrAuthorityCertificates(
    OncParsedCertificates* other) const {
  if (server_or_authority_certificates_.size() !=
      other->server_or_authority_certificates_.size())
    return false;

  for (size_t i = 0; i < server_or_authority_certificates_.size(); ++i) {
    const ServerOrAuthorityCertificate& cert =
        server_or_authority_certificates_[i];
    const ServerOrAuthorityCertificate& other_cert =
        other->server_or_authority_certificates_[i];

    if (cert.guid() != other_cert.guid())
      return false;

    if (cert.type() != other_cert.type())
      return false;

    if (!cert.certificate()->Equals(other_cert.certificate().get()))
      return false;

    if (cert.web_trust_requested() != other_cert.web_trust_requested())
      return false;
  }
...
bool OncParsedCertificates::HasSameClientCertificates(
    OncParsedCertificates* other) const {
  if (client_certificates_.size() != other->client_certificates_.size())
    return false;

  for (size_t i = 0; i < client_certificates_.size(); ++i) {
    const ClientCertificate& cert = client_certificates_[i];
    const ClientCertificate& other_cert = other->client_certificates_[i];

    if (cert.guid() != other_cert.guid())
      return false;

    if (cert.pkcs12_data() != other_cert.pkcs12_data())
      return false;
  }
