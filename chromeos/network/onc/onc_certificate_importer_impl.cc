...
bool CertificateImporterImpl::ParseServerOrCaCertificate(
  ...
  if (x509_cert.get()->isperm) {
    ...
    std::vector<net::ScopedCERTCertificate> cert_list;
    cert_list.push_back(net::x509_util::DupCERTCertificate(x509_cert));
...
bool CertificateImporterImpl::ParseClientCertificate(
  ...
  SECKEYPrivateKey* private_key =
      PK11_FindPrivateKeyFromCert(cert_result->slot, cert_result,
                                  NULL);  // wincx
  if (private_key) {
