  ...
  switch (mac_addr_randomization_supported_) {
    case MACAddressRandomizationSupport::NOT_REQUESTED:
      GetDeviceProperties(
          device_state->path(),
          base::Bind(
              &NetworkDeviceHandlerImpl::CacheMACAddressRandomizationSupport,
              weak_ptr_factory_.GetWeakPtr(), network_handler::ErrorCallback()),
          network_handler::ErrorCallback());
      return;
...
void NetworkDeviceHandlerImpl::CacheMACAddressRandomizationSupport(
    const network_handler::ErrorCallback& error_callback,
    const std::string& device_path,
    const base::DictionaryValue& properties) {
  bool supported;
