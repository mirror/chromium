...
enum class ResolvingStatus { kResolving, kResolved };
...
struct MatchingCertAndResolveStatus {
  ...
  ResolvingStatus resolving_status = ResolvingStatus::kResolving;

  // This is set to the last resolved client certificate or nullopt if no
  // matching certificate has been found when |resolving_status| is kResolved.
...
void ClientCertResolver::ConfigureCertificates(
  ...
  for (const NetworkAndMatchingCert& match : matches) {
    ...
    if (network_status.resolving_status == ResolvingStatus::kResolved &&
    ...
    network_status.resolving_status = ResolvingStatus::kResolved;
    network_status.matching_cert = match.matching_cert;
