...
bool CompareCertExpiration(const CertAndIssuer& a, const CertAndIssuer& b) {
  base::Time a_not_before, a_not_after;
  base::Time b_not_before, b_not_after;
  net::x509_util::GetValidityTimes(a.cert.get(), &a_not_before, &a_not_after);
  net::x509_util::GetValidityTimes(b.cert.get(), &b_not_before, &b_not_after);
...
std::vector<CertAndIssuer> CreateSortedCertAndIssuerList(
  ...
  for (net::ScopedCERTCertificateVector::iterator it = certs.begin();
       it != certs.end(); ++it) {
...
bool ClientCertResolver::ResolveCertificatePatternSync(
  ...
  if (client_cert_config.onc_source == ::onc::ONC_SOURCE_DEVICE_POLICY) {
    client_certs =
        CreateSortedCertAndIssuerList(net::x509_util::DupCERTCertificateVector(
