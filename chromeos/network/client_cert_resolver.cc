...
struct ClientCertResolver::CertResolvingStatus {
  ...
  // matching certificate has been found when |already_resolved| is true. This
  // is also used to determine if re-resolving a network actually changed any
  // settings.
  base::Optional<MatchingCert> matching_cert;
};
...
// |client_cert_pattern|.
struct ClientCertResolver::NetworkAndCertPattern {
...
void ClientCertResolver::ResolveNetworks(
  ...
  for (const NetworkState* network : networks) {
    ...
    if (networks_status_.find(network->path()) == networks_status_.end())
      networks_status_.insert_or_assign(network->path(), CertResolvingStatus());
...
// static
std::vector<ClientCertResolver::NetworkAndMatchingCert>
ClientCertResolver::FindCertificateMatches(
  ...
  for (const NetworkAndCertPattern& network_and_pattern : *networks) {
    ...
    if (cert_it == client_certs->end()) {
      ...
      matching_cert =
          base::make_optional<MatchingCert>({pkcs11_id, slot_id, identity});
    }
    matches.push_back(NetworkAndMatchingCert(
        network_and_pattern.service_path,
        network_and_pattern.cert_config.location, matching_cert));
  }
  return matches;
}
...
void ClientCertResolver::ConfigureCertificates(
  ...
  for (const NetworkAndMatchingCert& match : matches) {
    CertResolvingStatus& network_status = networks_status_[match.service_path];
