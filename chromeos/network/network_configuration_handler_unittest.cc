...
TEST_F(NetworkConfigurationHandlerTest, CreateConfiguration) {
  constexpr char kServicePath[] = "/service/2";
  constexpr char kNetworkName[] = "MyNetwork";
  ...
  value.SetString(shill::kGuidProperty, kServicePath);
  ...
  ASSERT_TRUE(success);
  EXPECT_EQ(service_path, kServicePath);
  // GUID is the service path in fake.
  EXPECT_EQ(guid, kServicePath);
}
