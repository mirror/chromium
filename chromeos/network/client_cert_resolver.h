...
class CHROMEOS_EXPORT ClientCertResolver : public NetworkStateHandlerObserver,
 ...
 private:
  ...
  // The key is the network's service path. If a network is not present, it has
  // not been resolved yet (or disappeared from the list of known networks after
  // it had been resolved). If a network is present with |already_resolved| set
  // to false, th network is planned for client certificate resolution, but has
  // not been resolved yet. If a network is present with |already_resolved| set
  // to true, client certificate resolution has completed for the network.
  // |matching_cert| is then set to the last resolved certificate info. This is
  // also used to determine if re-resolving a network actually changed any
  // settings.
