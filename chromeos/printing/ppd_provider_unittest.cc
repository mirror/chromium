...
TEST_F(PpdProviderTest, ResolvePrinters) {
  ...
  EXPECT_EQ("printer_a_ref",
            capture0[0].second.second.effective_make_and_model);
  EXPECT_EQ(base::Version("62"), capture0[0].second.first.min_milestone);
  EXPECT_EQ(true, capture0[0].second.first.unstable);

  EXPECT_EQ("printer_b", capture0[1].first);
  EXPECT_EQ("printer_b_ref",
            capture0[1].second.second.effective_make_and_model);
  EXPECT_EQ(base::Version("62"), capture0[1].second.first.min_milestone);
  EXPECT_EQ(true, capture0[1].second.first.unstable);
  ...
  EXPECT_EQ("printer_c_ref",
            capture1[0].second.second.effective_make_and_model);
}
