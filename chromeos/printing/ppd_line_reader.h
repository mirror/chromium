...
class CHROMEOS_EXPORT PpdLineReader {
 public:
  ...
  // header string which is found at the beginning of every PPD file. To verify
  // this a line reader is created which simply attempts to read the first line
  // and checks whether it contains the header string.
  static bool ContainsHeaderString(const std::string& contents,
                                   int max_line_length);
