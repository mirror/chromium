 ...
 public:
  ...
  bool NextLine(std::string* line) override {
    while (true) {
      ...
      // line.  Keep reading characters until we hit another newline, run out of
      // input, or exceed max_line_length_.
      ...
      // Exceeded max line length.  Keep just skipping input until we hit
      // another newline.
      while (true) {
        char c = NextChar();
        if (Eof()) {
