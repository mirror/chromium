...
// Return true if c is a newline in the ppd sense, that is, either newline
// or carriage return.
bool IsNewline(char c) {
  return c == '\n' || c == '\r';
}

// Source stream that reads from a string.  A reference
// is taken to the string used by StringSourceStream; it must
// not be modified while the StringSourceStream exists.
...
class PpdLineReaderImpl : public PpdLineReader {
 public:
  // ppd_contents must remain valid and unchanged while the reader is in use.
  PpdLineReaderImpl(const std::string& ppd_contents, int max_line_length)
  ...
  bool NextLine(std::string* line) override {
    while (true) {
      ...
      // Skip newlines at the start of the stream.
      while (true) {
        char c = NextChar();
        if (Eof()) {
          return false;
      ...
      // If we've gotten here, we have the first character of a new line in
      // line.  Keep reading characters until we hit another newline, run
      // out of input, or exceed max_line_length_.
      for (int i = 1; i <= max_line_length_; ++i) {
        char c = NextChar();
        if (Eof() || IsNewline(c)) {
          return true;
        }
      ...
      // Exceeded max line length.  Keep just skipping input until
      // we hit another newline.
      while (true) {
        char c = NextChar();
        if (Eof()) {
          return false;
 ...
 private:
  ...
  // Read the next char from the source stream.  If there is no more data to
  // be had, return null and set eof.
  char NextChar() {
  ...
  // The input stream we're reading bytes from.  This may be a gzip
  // source stream or string source stream depending on the source data.
