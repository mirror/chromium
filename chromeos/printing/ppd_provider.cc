...
class PpdProviderImpl : public PpdProvider, public net::URLFetcherDelegate {
 ...
 private:
  ...
  void OnReverseIndexComplete() {
    ...
    PpdProvider::CallbackResultCode code =
        ValidateAndParseJSONResponse(&contents);
    reverse_indexed_printers_ = base::MakeUnique<
        std::unordered_map<std::string, std::pair<std::string, std::string>>>();
    if (code != PpdProvider::SUCCESS) {
      LOG(ERROR) << "Failed manufacturer parsing";
    } else {
      for (auto& stuff : contents) {
        (*reverse_indexed_printers_)[std::get<0>(stuff)] =
            std::make_pair(std::get<1>(stuff), std::get<2>(stuff));
      }
      ReverseIndexQueueEntry entry = reverse_index_resolution_queue_.front();
      std::string effective_make_and_model = entry.effective_make_and_model;
      auto found = reverse_indexed_printers_->find(effective_make_and_model);
      if (found != reverse_indexed_printers_->end()) {
        auto manufacturer_and_model = found->second;
        LOG(ERROR) << "HERE1" << manufacturer_and_model.first;
        LOG(ERROR) << "HERE2" << manufacturer_and_model.second;
        LOG(ERROR) << "HERE3" << effective_make_and_model;
        base::SequencedTaskRunnerHandle::Get()->PostTask(
  ...
  PpdProvider::CallbackResultCode ValidateAndParseJSONResponse(
    ...
    for (const auto& entry : *top_list) {
      ...
      DCHECK(list[2].is_string());

      contents->push_back(std::make_tuple(
          list[0].GetString(), list[1].GetString(), list[2].GetString()));
    }
    return PpdProvider::SUCCESS;
  }
