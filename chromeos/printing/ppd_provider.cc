...
class PpdProviderImpl : public PpdProvider, public net::URLFetcherDelegate {
 ...
 private:
  ...
  void ResolvePpdCacheLookupDone(const Printer::PpdReference& reference,
    ...
    //
    // In any other case, we go through the full resolution flow (passing
    // along the cached data, if we got any).
  ...
  //
  // User-based ppd resolutions, which depend on local files, are left intact.
  // Other entries are resolved from the cache, if a cached entry was available,
  // or failed outright.
