...
// Handles disconnections from ongoing tether sessions. When a tether
// connection is in progress, the device is connected to an underlying Wi-Fi
// network. If the tether network is disconnected, tether metadata must be
// updated accordingly, and a message must be send to the host to disable its
// Wi-Fi hotspot. This class tracks ongoing connections and updates this
// metadata and sends a message to disable the Wi-Fi hotspot when necessary.
