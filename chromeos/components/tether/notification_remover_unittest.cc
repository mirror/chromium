...
class NotificationRemoverTest : public NetworkStateTest {
 protected:
  ...
  void NotifyMultiplePotentialHotspotsNearby() {
  ...
  void NotifySetupRequired() {
    notification_presenter_->NotifySetupRequired("testDevice");
    EXPECT_TRUE(
        notification_presenter_->is_setup_required_notification_shown());
  }
