...
// Time past the Unix epoch at the start of the test. Internally,
// HotspotUsageDurationTracker uses the epoch as a sentinel value, so the test
// must not start the clock at that time.
constexpr const base::TimeDelta kStartTime = base::TimeDelta::FromSeconds(1337);
...
class HotspotUsageDurationTrackerTest : public testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    test_clock_->Advance(kStartTime);
...
TEST_F(HotspotUsageDurationTrackerTest, TestNoConnection) {
  ...
  histogram_tester_.ExpectTotalCount(
      "InstantTethering.Performance.HotspotUsageDuration", 0 /* count */);
...
TEST_F(HotspotUsageDurationTrackerTest, TestConnections) {
  ...
  histogram_tester_.ExpectTimeBucketCount(
      "InstantTethering.Performance.HotspotUsageDuration", kTestDuration,
      1 /* count */);
  ...
  fake_active_host_->SetActiveHostDisconnected();
  histogram_tester_.ExpectTimeBucketCount(
      "InstantTethering.Performance.HotspotUsageDuration", 2 * kTestDuration,
      1 /* count */);
}
