...
class DisconnectTetheringRequestSenderTest : public testing::Test {
 public:
  ...
  void SendConcurrentRequestsToTwoDevices(bool first_operation_successful,
    ...
    // DisconnectTetheringRequestSender can send requests to multiple devices
    // concurrently.
  ...
  void SendConcurrentRequestsToOneDevice(bool operation_successful) {
    disconnect_tethering_request_sender_->SendDisconnectRequestToDevice(
        test_devices_[0].GetDeviceId());
    disconnect_tethering_request_sender_->SendDisconnectRequestToDevice(
        test_devices_[0].GetDeviceId());

    // When multiple requests are sent concurrently to the same device, only one
    // DisconnectTetheringOperation is created.
...
TEST_F(
    DisconnectTetheringRequestSenderTest,
    SendMultipleRequests_Concurrent_DifferentDeviceId_BothOperationsSuccessful) {
...
TEST_F(DisconnectTetheringRequestSenderTest,
       SendMultipleRequests_Concurrent_DifferentDeviceId_BothOperationsFailed) {
...
TEST_F(
    DisconnectTetheringRequestSenderTest,
    SendMultipleRequests_Concurrent_DifferentDeviceId_FirstOperationSuccessful) {
  SendConcurrentRequestsToTwoDevices(true /* first_operation_successful */,
                                     false /* second_operation_successful */);
}
...
TEST_F(
    DisconnectTetheringRequestSenderTest,
    SendMultipleRequests_Concurrent_DifferentDeviceId_SecondOperationSuccessful) {
  SendConcurrentRequestsToTwoDevices(false /* first_operation_successful */,
                                     true /* second_operation_successful */);
}

TEST_F(DisconnectTetheringRequestSenderTest,
       SendMultipleRequests_Concurrent_SameDeviceId_OperationSuccessful) {
  SendConcurrentRequestsToOneDevice(true /* operation_successful */);
}

TEST_F(DisconnectTetheringRequestSenderTest,
       SendMultipleRequests_Concurrent_SameDeviceId_OperationFailed) {
  SendConcurrentRequestsToOneDevice(false /* operation_successful */);
}
