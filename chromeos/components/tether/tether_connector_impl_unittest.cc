...
TEST_F(TetherConnectorImplTest,
  ...
  const base::TimeDelta secondConnectToHostAttemptAdditionalTime =
      base::TimeDelta::FromSeconds(2);
  test_clock_->Advance(secondConnectToHostAttemptAdditionalTime);
  SuccessfullyJoinWifiNetwork();
  histogram_tester_.ExpectTimeBucketCount(
      "InstantTethering.Performance.ConnectToHostDuration",
      kConnectToHostTime + secondConnectToHostAttemptAdditionalTime,
      1 /* count */);
}
