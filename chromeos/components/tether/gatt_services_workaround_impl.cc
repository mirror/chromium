...
// Empirically, phones generally pick up scan results in 1-6 seconds. 10 seconds
...
GattServicesWorkaroundImpl::AdvertisementWithTimer::AdvertisementWithTimer(
...
GattServicesWorkaroundImpl::GattServicesWorkaroundImpl(
...
void GattServicesWorkaroundImpl::RequestGattServicesForDevice(
    const cryptauth::RemoteDevice& remote_device) {
  std::string device_id = remote_device.GetDeviceId();
  const auto& it = device_id_to_advertisement_with_timer_map_.find(device_id);
  ...
  timer->Start(FROM_HERE, base::TimeDelta::FromSeconds(kNumSecondsToAdvertise),
               base::Bind(&GattServicesWorkaroundImpl::OnTimerFired,
...
bool GattServicesWorkaroundImpl::HasPendingRequests() {
  return !device_id_to_advertisement_with_timer_map_.empty();
}

void GattServicesWorkaroundImpl::OnTimerFired(const std::string& device_id) {
  PA_LOG(ERROR) << "Stopping workaround advertisement for device with ID \""
  ...
  it->second.advertisement->Stop(
      base::Bind(&GattServicesWorkaroundImpl::OnAdvertisementStopped,
                 weak_ptr_factory_.GetWeakPtr(), device_id));
}

void GattServicesWorkaroundImpl::OnAdvertisementStopped(
...
void GattServicesWorkaroundImpl::SetTimerFactoryForTesting(
