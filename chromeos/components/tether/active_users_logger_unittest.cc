...
class ActiveUsersLoggerTest : public testing::Test {
 protected:
  ...
  void Verify1DA(bool is_expected) {
    histogram_tester_.ExpectUniqueSample("InstantTethering.ActiveUsers.1DA",
                                         ActiveUsersLogger::ActiveUser::ACTIVE,
                                         is_expected ? 1 : 0);
  }

  void Verify7DA(bool is_expected) {
    histogram_tester_.ExpectUniqueSample("InstantTethering.ActiveUsers.7DA",
                                         ActiveUsersLogger::ActiveUser::ACTIVE,
                                         is_expected ? 1 : 0);
  }

  void Verify14DA(bool is_expected) {
    histogram_tester_.ExpectUniqueSample("InstantTethering.ActiveUsers.14DA",
                                         ActiveUsersLogger::ActiveUser::ACTIVE,
                                         is_expected ? 1 : 0);
  }

  void Verify28DA(bool is_expected) {
    histogram_tester_.ExpectUniqueSample("InstantTethering.ActiveUsers.28DA",
                                         ActiveUsersLogger::ActiveUser::ACTIVE,
                                         is_expected ? 1 : 0);
  }
...
TEST_F(ActiveUsersLoggerTest, RecordUserWasActive) {
  ...
  active_users_logger_->OnDailyEvent();
