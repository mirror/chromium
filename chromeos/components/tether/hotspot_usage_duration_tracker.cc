...
// Number of buckets in the metric. One bucket is used for each minute (i.e.,
// one bucket for 0-1 minute, one bucket for 1-2 minutes, etc).
const int kNumMetricsBuckets = kMaxDurationHours * 60;
  ...
  switch (change_info.new_status) {
    ...
    case ActiveHost::ActiveHostStatus::DISCONNECTED: {
      ...
      UMA_HISTOGRAM_CUSTOM_TIMES(
          "InstantTethering.Performance.HotspotUsageDuration", duration,
          base::TimeDelta::FromSeconds(kMinDurationSeconds) /* min */,
          base::TimeDelta::FromDays(kMaxDurationHours) /* max */,
