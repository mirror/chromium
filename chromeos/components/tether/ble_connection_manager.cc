...
void BleConnectionManager::OnConnectionAttemptTimeout(
  ...
  remote_device_to_advertising_start_time_map_[remote_device] = base::Time();
...
void BleConnectionManager::SendSecureChannelStatusChangeEvent(
  ...
  if (new_status == cryptauth::SecureChannel::Status::CONNECTING) {
    remote_device_to_advertising_start_time_map_[remote_device] = clock_->Now();
...
void BleConnectionManager::RecordAdvertisementToConnectionDuration(
    const cryptauth::RemoteDevice& remote_device) {
  if (!base::ContainsKey(remote_device_to_advertising_start_time_map_,
                         remote_device) ||
      remote_device_to_advertising_start_time_map_[remote_device].is_null() ||
      !base::ContainsKey(remote_device_to_status_connected_time_map_,
                         remote_device) ||
      remote_device_to_status_connected_time_map_[remote_device].is_null()) {
  ...
  remote_device_to_advertising_start_time_map_.erase(remote_device);
}

void BleConnectionManager::RecordConnectionToAuthenticationDuration(
    const cryptauth::RemoteDevice& remote_device) {
  if (!base::ContainsKey(remote_device_to_status_connected_time_map_,
                         remote_device) ||
      remote_device_to_status_connected_time_map_[remote_device].is_null()) {
  ...
  remote_device_to_status_connected_time_map_.erase(remote_device);
}
