...
void HostScanner::OnTetherAvailabilityResponse(
  ...
  if (scanned_device_list_so_far.empty() && !is_final_scan_result) {
    was_notification_showing_when_current_scan_started_ =
        notification_presenter_->IsPotentialHotspotsNearbyNotificationShowing();
  }
...
bool HostScanner::CanAvailableHostNotificationBeShown() {
  ...
  if (possibly_active_network &&
    ...
    // shown. There's no need to alert the user of a possible connection if
    // the user already has a connection.
    return false;
  }
  ...
  if (!notification_presenter_
    ...
    // showing, it has been removed, due to either NotificationRemover or due to
