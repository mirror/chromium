...
void InitializerImpl::StartAsynchronousShutdown() {
  ...
  // asynchronous shutdown, so start observering these objects. Once they notify
  // observers that they are finished shutting down, asynchronous shutdown will
  // complete.
  ble_scanner_->AddObserver(this);
  disconnect_tethering_request_sender_->AddObserver(this);

  // All objects which are not dependencies of these objects are no longer
  // needed, so delete them.
  crash_recovery_manager_.reset();
...
void InitializerImpl::FinishAsynchronousShutdownIfPossible() {
  ...
  // If the asynchronous shutdown is not yet complete (e.g., only some of the
