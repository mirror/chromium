...
bool BleAdvertisementDeviceQueue::SetPrioritizedDeviceIds(
  ...
  bool were_devices_inserted =
      InsertPrioritizedDeviceIdsIfNecessary(prioritized_ids);
  bool were_devices_removed = RemoveMapEntriesIfNecessary(prioritized_ids);
  return were_devices_inserted || were_devices_removed;
}
...
bool BleAdvertisementDeviceQueue::InsertPrioritizedDeviceIdsIfNecessary(
  ...
  for (const auto& priotizied_id : prioritized_ids) {
    ...
    if (std::find(device_ids_for_priority.begin(),
                  device_ids_for_priority.end(),
                  priotizied_id.device_id) == device_ids_for_priority.end()) {
...
bool BleAdvertisementDeviceQueue::RemoveMapEntriesIfNecessary(
  ...
  for (auto& map_entry : priority_to_device_ids_map_) {
    const ConnectionPriority& priority = map_entry.first;
    ...
    while (device_ids_it != device_ids_for_priority.end()) {
      if (DoesVectorContainPrioritizedDeviceId(priority, *device_ids_it,
                                               prioritized_ids)) {
        ++device_ids_it;
        continue;
      }
...
void BleAdvertisementDeviceQueue::MoveDeviceToEnd(
  ...
  for (auto& map_entry : priority_to_device_ids_map_) {
    std::vector<std::string>& device_ids_for_priority = map_entry.second;
    auto device_deque_it = device_ids_for_priority.begin();
    while (device_deque_it != device_ids_for_priority.end()) {
      if (*device_deque_it == device_id)
        break;
      ++device_deque_it;
    }

    if (device_deque_it == device_ids_for_priority.end())
      continue;

    // Move the element to the end of |device_ids_for_priority|.
    std::rotate(device_deque_it, device_deque_it + 1,
                device_ids_for_priority.end());
  }
}
