...
class MessageTransferOperation : public BleConnectionManager::Observer {
 ...
 protected:
  ...
  // Returns true if this operation should include a timeout. If
  // ShouldOperationUseTimeout() returns true and a device has been
  // authenticated but not unregistered for at least the amount of time returned
  // by GetTimeoutSeconds(), that host device will be unregistered.
  virtual bool ShouldOperationUseTimeout();
