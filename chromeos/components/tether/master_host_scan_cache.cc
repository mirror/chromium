...
MasterHostScanCache::MasterHostScanCache(
  ...
  for (const auto it : persisted_entries) {
    network_host_scan_cache_->SetHostScanResult(it.second);
  }
}

MasterHostScanCache::~MasterHostScanCache() {
  // Remove all existing cache entries from both |persistent_host_scan_cache_|
  // and |network_host_scan_cache_|.
  std::unordered_map<std::string, HostScanCacheEntry> persisted_entries =
      persistent_host_scan_cache_->GetStoredCacheEntries();
  for (const auto it : persisted_entries) {
    persistent_host_scan_cache_->RemoveHostScanResult(
        it.second.tether_network_guid);
    network_host_scan_cache_->RemoveHostScanResult(
        it.second.tether_network_guid);
  }
}
