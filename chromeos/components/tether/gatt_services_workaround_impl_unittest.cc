...
class TestObserver final : public GattServicesWorkaround::Observer {
 public:
  TestObserver() {}
...
class GattServicesWorkaroundImplTest : public testing::Test {
 protected:
  GattServicesWorkaroundImplTest()
  ...
  void SetUp() override {
    ...
    workaround_ = base::MakeUnique<GattServicesWorkaroundImpl>(
  ...
  std::unique_ptr<GattServicesWorkaroundImpl> workaround_;

 private:
  DISALLOW_COPY_AND_ASSIGN(GattServicesWorkaroundImplTest);
};

TEST_F(GattServicesWorkaroundImplTest, CannotGenerateAdvertisement) {
...
TEST_F(GattServicesWorkaroundImplTest, AdvertiseAndStop) {
...
TEST_F(GattServicesWorkaroundImplTest, TwoRequestsForSameDevice_BeforeTimer) {
  SetAdvertisement(0 /* index */);
  test_timer_factory_->set_device_id_for_next_timer(
      fake_devices_[0].GetDeviceId());
...
TEST_F(GattServicesWorkaroundImplTest, TwoRequestsForSameDevice_AfterTimer) {
  SetAdvertisement(0 /* index */);
  test_timer_factory_->set_device_id_for_next_timer(
      fake_devices_[0].GetDeviceId());
...
TEST_F(GattServicesWorkaroundImplTest, TwoRequestsForDifferentDevices) {
  SetAdvertisement(0 /* index */);
  test_timer_factory_->set_device_id_for_next_timer(
      fake_devices_[0].GetDeviceId());
