...
class PersistentHostScanCache : public HostScanCache {
 ...
 private:
  // Keys to DictionaryValues corresponding to host scan properties.
  static const char kTetherNetworkGuidKey[];
  static const char kDeviceNameKey[];
  static const char kCarrierKey[];
  static const char kBatteryPercentageKey[];
  static const char kSignalStrengthKey[];
  static const char kSetupRequiredKey[];

  static std::unique_ptr<base::DictionaryValue> HostScanCacheEntryToDictionary(
      const HostScanCacheEntry& entry);
  static std::unique_ptr<HostScanCacheEntry> DictionaryToHostScanCacheEntry(
