...
using TpmTokenInfo = CryptohomeClient::TpmTokenInfo;

// Class for getting a user or the system TPM token info from cryptohome during
// TPM token loading.
class CHROMEOS_EXPORT TPMTokenInfoGetter {
 public:
  using TpmTokenInfoCallback =
      base::OnceCallback<void(base::Optional<TpmTokenInfo> token_info)>;
 ...
 private:
  ...
  void OnPkcs11GetTpmTokenInfo(base::Optional<TpmTokenInfo> token_info);
