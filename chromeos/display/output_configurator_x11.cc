...
OutputConfiguratorX11::OutputConfiguratorX11()
  ...
  if (base::chromeos::IsRunningOnChromeOS()) {
    ...
    CHECK(screen != NULL);
    DisplayInfo info(display, screen);
    bool did_detect_outputs = TryRecacheOutputs(info);
    CHECK(did_detect_outputs);
    State current_state = InferCurrentState(info);
    if (current_state == STATE_INVALID) {
      // Unknown state.  Transition into the default state.
      State state = GetDefaultState();
      UpdateCacheAndOutputToState(info, state);
    } else {
...
void OutputConfiguratorX11::UpdateCacheAndOutputToState(const DisplayInfo& info,
                                                        State new_state) {
  // Default rules:
  ...
  XRRScreenResources* screen = info.screen;
  Window window = DefaultRootWindow(display);
...
bool OutputConfiguratorX11::ScreenPowerSet(bool power_on, bool all_displays) {
  VLOG(1) << "OutputConfiguratorX11::SetScreensOn " << power_on
...
bool OutputConfiguratorX11::SetDisplayMode(State new_state) {
  ...
  UpdateCacheAndOutputToState(DisplayInfo(display, screen), new_state);
  XRRFreeScreenResources(screen);
  XUngrabServer(display);
  return true;
}
...
bool OutputConfiguratorX11::TryRecacheOutputs(const DisplayInfo& info) {
  ...
  if (outputs_did_change) {
    ...
    if (output_count_ == 0) {
      ...
      // Ideally, this would be allocated inline in the OutputConfiguratorX11
...
bool OutputConfiguratorX11::RecacheAndUseDefaultState() {
  Display* display = base::MessagePumpAuraX11::GetDefaultXDisplay();
  CHECK(display != NULL);
  XGrabServer(display);
  Window window = DefaultRootWindow(display);
  ...
  DisplayInfo info(display, screen);
  bool did_detect_change = TryRecacheOutputs(info);
...
State OutputConfiguratorX11::InferCurrentState(const DisplayInfo& info) const {
...
bool OutputConfiguratorX11::IsProjecting() const {
