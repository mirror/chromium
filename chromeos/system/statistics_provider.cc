...
// and "serial_number" which are identical except that the latter has a letter
// appended that serves as a check digit. Unfortunately, the sticker on the
// lumpy packaging didn't include that check digit (the sticker on the device
// did though!). The former sticker was the source of the serial number used by
// device management service, so we preferred "Product_S/N" over "serial_number"
// to match the server. As an unintended consequence, older Samsung devices
// display and report a serial number that doesn't match the sticker on the
// device (the check digit is missing).
//
// "Product_S/N" is known to be exist on alex, celes, lumpy, pi, pit, snow,
// winky and possibly some kevin and thus needs to be supported until AUE of
// these devices. It's known *not* to be present on caroline.
