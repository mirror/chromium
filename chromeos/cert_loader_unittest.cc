...
bool IsCertInCertificateList(
  ...
  for (net::ScopedCERTCertificateVector::const_iterator it = cert_list.begin();
       it != cert_list.end(); ++it) {
...
class CertLoaderTest : public testing::Test,
 ...
 protected:
  ...
  void ImportCACert(const std::string& cert_file,
                    ...
                    net::ScopedCERTCertificateVector* imported_certs) {
...
TEST_F(CertLoaderTest, UpdatedOnCertRemoval) {
  ...
  net::ScopedCERTCertificate nss_cert =
      net::x509_util::FindCERTCertificateFromX509Certificate(cert.get());
  ASSERT_TRUE(nss_cert);
  primary_certdb_->DeleteCertAndKey(nss_cert.get());
