...
class CHROMEOS_EXPORT CertLoader {
 public:
  class Observer {
   public:
    ...
    virtual void OnCertificatesLoaded(
        const net::ScopedCERTCertificateVector& all_certs,
        bool initial_load) = 0;
  ...
  // true.
  const net::ScopedCERTCertificateVector& all_certs() const {
  ...
  const net::ScopedCERTCertificateVector& system_certs() const {
    DCHECK(thread_checker_.CalledOnValidThread());
 ...
 private:
  ...
  void UpdateCertificates(net::ScopedCERTCertificateVector all_certs,
  ...
  net::ScopedCERTCertificateVector all_certs_;

  // Cached certificates from system token.
  net::ScopedCERTCertificateVector system_certs_;
