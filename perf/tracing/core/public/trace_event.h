...
#define TRACE_UNIQUE_NAME(name_prefix) TRACE_UNIQUE_NAME2(name_prefix, __LINE__)
...
#define TRACE_EVENT(category, name, ...)                                       \
  ...
  const unsigned char* TRACE_UNIQUE_NAME(category_group_enabled) =             \
      ::tracing::internal::GetCategoryGroupEnabledCached(category,             \
                                                         &cached_ptr);         \
  ...
  if (TRACE_INTERNAL_SHOULD_ADD_EVENT(                                         \
    ...
    /* AddTraceEvent() overloads implemented below in this file */             \
...
inline TracingCoreInterface* tracing_core() {
  return ::tracing::internal::TracingCoreInterface::instance();
}

// TODO types and casts here seem a bit messed up, cleanup.
inline const unsigned char* GetCategoryGroupEnabledCached(
  ...
  if (TRACE_UNLIKELY(!category_group_enabled)) {
    category_group_enabled =
        tracing_core()->GetCategoryGroupEnabled(category_group);
...
inline TraceEventHandle AddTraceEvent(
    ...
    uint32_t flags,
    uint64_t bind_id) {
  tracing_core()->AddEvent(
      phase, category_group_enabled, name, scope, id, bind_id,
      tracing_core()->GetCurrentThreadId(), tracing_core()->GetWallTime(),
...
inline TraceEventHandle AddTraceEventWithThreadIdAndTimestamp(
  ...
  std::unique_ptr<ConvertableToTraceFormat> convertable_values[1] = {
      std::move(arg_value)};
  tracing_core()->AddEvent(phase, category_group_enabled, name, scope, id,
                           bind_id, thread_id, timestamp, 1, arg_names,
                           arg_types, convertable_values, flags);
}
