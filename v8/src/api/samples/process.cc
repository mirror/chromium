...
class HttpRequestProcessor {
 public:
  ...
  virtual void Process(HttpRequest *req) = 0;
...
class JsHttpRequestProcessor : public HttpRequestProcessor {
 public:
  ...
  JsHttpRequestProcessor(const string &script) : script_(script) { }
 ...
 private:
  ...
  // Wrap the options map in a JavaScript object and install it in
  // the global namespace as 'options'.
  bool InstallOptions(map<string, string> *opts);
...
bool JsHttpRequestProcessor::ExecuteScript(const string &script) {
  HandleScope handle_scope;
  // Create a JavaScript string containing the script.
  Handle<String> script_obj = String::New(script.c_str(), script.length());

  // Compile the script and check for errors.
  Handle<Script> compiled_script = Script::Compile(script_obj);
  if (compiled_script.IsEmpty()) {
    // The script failed to compile; bail out.
    return false;
  }

  // We're just about to run the script; set up an error handler to
...
bool JsHttpRequestProcessor::InstallOptions(map<string, string> *opts) {
  HandleScope handle_scope;
...
void JsHttpRequestProcessor::Process(HttpRequest *request) {
  ...
  process_->Call(context_->Global(), argc, argv);
}
...
map<string, string> *JsHttpRequestProcessor::UnwrapMap(Handle<Object> obj) {
  Handle<External> field = Handle<External>::Cast(obj->GetInternal(0));
...
Handle<ObjectTemplate> JsHttpRequestProcessor::MakeMapTemplate() {
  HandleScope handle_scope;
  // The next three lines will be replaced with
  //   Handle<ObjectTemplate> result = ObjectTemplate::New();
  //   result->SetInternalFieldCount(1)
  // when SetInternalFieldCount has been moved
  Handle<FunctionTemplate> fun = FunctionTemplate::New();
  fun->SetInternalFieldCount(1);
  Handle<ObjectTemplate> result = fun->InstanceTemplate();
...
Handle<Object> JsHttpRequestProcessor::WrapRequest(HttpRequest *request) {
  ...
  result->SetInternal(0, request_ptr);
...
#include "process-scaffold.cc"

int main(int argc, char *argv[]) {
  JsHttpRequestProcessor processor(kScript);
  map<string, string> options;
  options["foo"] = "knold";
  bool succeeded = processor.Initialize(&options);
  if (!succeeded) return 1;
  StringHttpRequest req("/", "localhost", "google.com", "process.cc");
