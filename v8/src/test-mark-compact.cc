...
static void TestMarkingStack() {
  ...
  int i = 0;
  while (!s.is_full()) {
    s.Push(reinterpret_cast<HeapObject*>(i++));
  }

  while (!s.is_empty()) {
    int v = reinterpret_cast<int>(s.Pop());
    CHECK_EQ(--i, v);
  }
...
static void TestMarkCompactCollector() {
  InitializeVM();
