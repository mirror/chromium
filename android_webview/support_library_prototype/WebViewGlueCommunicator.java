...
public class WebViewGlueCommunicator {
    ...
    private static final String GLUE_FACTORY_PROVIDER_FETCHER_CLASS = "org.chromium.android_webview_glue.GlueFactoryProviderFetcher";
    ...
    private static InvocationHandler fetchGlueProviderFactoryImpl() {
        try {
            ...
            Method createProviderFactoryMethod = glueFactoryProviderFetcherClass.getDeclaredMethod("createGlueWebViewProviderFactory");
    ...
    private static WebViewProviderFactory createGlueProviderFactory() {
        ...
        return (WebViewProviderFactory) Proxy.newProxyInstance(WebViewGlueCommunicator.class.getClassLoader(),
                new Class[] { WebViewProviderFactory.class },
                invocationHandler);
    }
    ...
    public static Method dupeMethod(Method method)
        ...
        for (int n = 0; n < parameterClasses.length; n++) {
            ...
            // TODO could make sure to only pass non-primitive classes across the boundary instead
            // of this switch-case...
            if (clazz.getName().equals("long")) {
                parameterClasses[n] = long.class;
            } else {
                parameterClasses[n] = Class.forName(
                        clazz.getName() + (clazz.isPrimitive() ? ".class" : ""));
            }
        }
