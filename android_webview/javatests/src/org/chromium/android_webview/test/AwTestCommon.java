...
final class AwTestCommon {
    ...
    void setUp() throws Exception {
        if (mCallback.needsAwBrowserContextCreatedForTestCommon()) {
            createAwBrowserContext();
        }
        if (mCallback.needsBrowserProcessStartedForTestCommon()) {
            startBrowserProcess();
        }
    }
    ...
    void createAwBrowserContext() {
        ...
        mCallback.getInstrumentation().runOnMainSync(new Runnable() {
            ...
            public void run() {
                mBrowserContext =
                        mCallback.createAwBrowserContextOnUiThreadForTestCommon(prefs, appContext);
            }
        });
    }
    ...
    AwTestContainerView createDetachedAwTestContainerView(final AwContentsClient awContentsClient,
        ...
        if (testDependencyFactory == null) {
            testDependencyFactory = mCallback.createTestDependencyFactoryForTestCommon();
        }
    ...
    interface AwTestCommonCallback {
        ...
        AwBrowserContext createAwBrowserContextOnUiThreadForTestCommon(
                InMemorySharedPreferences prefs, Context appContext);
        TestDependencyFactory createTestDependencyFactoryForTestCommon();
        boolean needsAwBrowserContextCreatedForTestCommon();
        boolean needsBrowserProcessStartedForTestCommon();
    }
}
