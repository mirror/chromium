...
AwFormDatabaseService::AwFormDatabaseService(const base::FilePath path)
  ...
  web_database_ = new WebDatabaseService(
      ...
      BrowserThread::GetTaskRunnerForThread(BrowserThread::UI), db_thread);
  ...
  autofill_data_ = new autofill::AutofillWebDataService(
      ...
      db_thread, base::Bind(&DatabaseErrorCallback));
  autofill_data_->Init();
}
...
bool AwFormDatabaseService::HasFormData() {
  ...
  base::PostTask(
      FROM_HERE,
      base::Bind(
          &autofill::AutofillWebDataService::GetCountOfValuesContainedBetween,
