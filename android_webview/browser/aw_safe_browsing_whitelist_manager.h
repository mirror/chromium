...
class AwSafeBrowsingWhitelistManager {
 public:
  ...
  void SetWhitelistOnUiThread(std::unique_ptr<std::vector<std::string>> hosts);

 private:
  // Builds a new whitelist filter on background thread
  void UpdateOnIO(std::unique_ptr<std::vector<std::string>> hosts);
  // Replaces the current whitelist. Must be called on the IO thread.
  void SetWhitelist(std::unique_ptr<URLWhitelist> whitelist);

  scoped_refptr<base::SequencedTaskRunner> background_task_runner_;
  scoped_refptr<base::SequencedTaskRunner> io_task_runner_;
  scoped_refptr<base::SequencedTaskRunner> ui_task_runner_;

  std::unique_ptr<URLWhitelist> whitelist_;
  base::WeakPtrFactory<AwSafeBrowsingWhitelistManager> ui_weak_ptr_factory_;
  base::WeakPtrFactory<AwSafeBrowsingWhitelistManager> io_weak_ptr_factory_;
