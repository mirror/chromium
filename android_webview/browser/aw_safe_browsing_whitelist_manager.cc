...
std::unique_ptr<URLWhitelist> BuildWhitelist(
    std::unique_ptr<std::vector<std::string>> allow) {
  std::unique_ptr<URLWhitelist> whitelist(new URLWhitelist);
  whitelist->AddFilters(*allow.get());
  return whitelist;
}
...
void AwSafeBrowsingWhitelistManager::UpdateOnIO(
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::Bind(&BuildWhitelist, base::Passed(&hosts)),
...
void AwSafeBrowsingWhitelistManager::SetWhitelistOnUiThread(
  ...
  io_task_runner_->PostTask(
      FROM_HERE, base::Bind(&AwSafeBrowsingWhitelistManager::UpdateOnIO,
                            base::Unretained(this), base::Passed(&hosts)));
}
