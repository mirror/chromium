...
// reverse order. For example a whitelist of google.com and example.com will
// have this trie structure
//                       root
//                         | com
//                       Node1
//                google/    \ example
...
// However, the whitelisting also allows matching subdomains by adding a
// leading dot.
//
// In addition to regular trie functions, the trie also allows matching in
// a non-leaf node. If the whitelist is .google.com and a.google.com:
//
//                       root
//                         | com
//                       Node1
...
//                         | a
//                       Node3
// both Node2 and Node3 are valid terminal nodes to terminate a match. The
// boolean is_terminal indicates whether a node can successfully terminate a
// search (aka. whether this rule was entered to the whitelist) and
// match_prefix indicate if this should match exactly, or just do a prefix
// match.
// The structure above allows optimizations such that if a node already
// allows a prefix match, then there is no need for it to have children.
struct TrieNode {
...
bool AddRuleToWhitelist(std::string rule, TrieNode* root) {
  if (rule.empty())
    return false;
  // Leading dot means to do an exact match.
  bool started_with_dot = false;
  if (rule.front() == '.') {
    rule.erase(0, 1);  // Strip it for the rest of the handling.
  ...
  InsertRuleToTrie(components, root, !started_with_dot);
  return true;
}
...
bool IsWhitelisted(const GURL& url, const TrieNode* node) {
  ...
  if (url.HostIsIPAddress()) {
    ...
    components = base::SplitStringPiece(url.host(), ".", base::TRIM_WHITESPACE,
                                        base::SPLIT_WANT_NONEMPTY);
  }
...
void AwSafeBrowsingWhitelistManager::UpdateOnIO(
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::Bind(&BuildWhitelist, std::move(rules)),
