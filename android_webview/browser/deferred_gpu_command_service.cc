...
DeferredGpuCommandService::CreateDeferredGpuCommandService(bool for_testing) {
  ...
  bool success = gpu::InitializeGLThreadSafe(
      ...
      gpu_preferences.log_gpu_control_list_decisions, for_testing, &gpu_info,
      &gpu_feature_info);
  if (!success) {
...
DeferredGpuCommandService* DeferredGpuCommandService::GetInstance(
  ...
  static base::NoDestructor<scoped_refptr<DeferredGpuCommandService>> service(
      CreateDeferredGpuCommandService(for_testing));
  return service->get();
}
