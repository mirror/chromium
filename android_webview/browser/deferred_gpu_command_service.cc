...
DeferredGpuCommandService::CreateDeferredGpuCommandService() {
  ...
  bool success = gpu::InitializeGLThreadSafe(
      ...
      &gpu_feature_info);
  if (!success) {
    // TODO(zmo): Figure out why this fails on webview tests and switch to
    // LOG(FATAL) here.
    LOG(ERROR) << "gpu::InitializeGLThreadSafe() failed.";
  }
...
// static
bool DeferredGpuCommandService::can_support_threaded_texture_mailbox_ = false;

// static
bool DeferredGpuCommandService::CanSupportThreadedTextureMailbox() {
  // This static member is only written in DeferredGpuCommandService
  // constructor. Since we only call this after calling GetInstance(),
  // so it's thread safe.
  return can_support_threaded_texture_mailbox_;
}
...
DeferredGpuCommandService::DeferredGpuCommandService(
    ...
    : gpu::InProcessCommandBuffer::Service(gpu_preferences,
      ...
      gpu_info_(gpu_info) {
  can_support_threaded_texture_mailbox_ =
