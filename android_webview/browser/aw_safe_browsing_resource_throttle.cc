...
void AwSafeBrowsingResourceThrottle::StartApplicationResponse(
  ...
  if (client) {
    base::Callback<void(SafeBrowsingAction)> callback =
...
void AwSafeBrowsingResourceThrottle::DoApplicationResponse(
  ...
  // TODO(ntfschr): implement *_AND_REPORT cases once reporting is finished
  // (crbug/688629)
  switch (action) {
    case SafeBrowsingAction::SHOW_INTERSTITIAL_NO_REPORTING:
    ...
    case SafeBrowsingAction::SHOW_INTERSTITIAL:
      content::BrowserThread::PostTask(
          ...
          base::Bind(&BaseResourceThrottle::StartDisplayingBlockingPage,
                     throttle, ui_manager, resource));
      return;
    case SafeBrowsingAction::PROCEED_AND_REPORT:
    case SafeBrowsingAction::PROCEED:
      proceed = true;
      break;
    case SafeBrowsingAction::BACK_TO_SAFETY_AND_REPORT:
