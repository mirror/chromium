...
base::trace_event::TraceRecordMode convertToTraceRecordMode(
    android_webview::AwTracingController::AwTracingMode mode) {
  switch (mode) {
    case android_webview::AwTracingController::RECORD_UNTIL_FULL:
      return base::trace_event::RECORD_UNTIL_FULL;
    case android_webview::AwTracingController::RECORD_CONTINUOSLY:
      return base::trace_event::RECORD_CONTINUOUSLY;
    case android_webview::AwTracingController::RECORD_AS_MUCH_AS_POSSIBLE:
      return base::trace_event::RECORD_AS_MUCH_AS_POSSIBLE;
    case android_webview::AwTracingController::RECORD_TO_CONSOLE:
      return base::trace_event::ECHO_TO_CONSOLE;
  }
}
...
bool AwTracingController::Start(JNIEnv* env,
  ...
  base::trace_event::TraceConfig trace_config(
      categories, convertToTraceRecordMode(static_cast<AwTracingMode>(jmode)));
...
bool AwTracingController::StopAndFlush(JNIEnv* env,
                                       const JavaParamRef<jobject>& obj) {
  // TODO(timvolodine): replace StringEndpoint with a custom endpoint for better
  // efficiency (crbug.com/797305).
