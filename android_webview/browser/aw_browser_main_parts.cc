...
void AwBrowserMainParts::SetUpFieldTrials() {
  ...
  // Read seed data here. Hardcoding for
  // now------------------------------------------
  ...
  std::vector<std::string> tokens = base::SplitString(tmp_str, "\n", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);

  local_state.SetString(variations::prefs::kVariationsSeedSignature, tokens[0]);
  ...
  if (!base::Time::FromUTCString(tokens[2].c_str(), &fetch_date)) {
    LOG(ERROR) << "Failed to parse seed last fetch date string";
    return;
  }
  local_state.SetInt64(variations::prefs::kVariationsSeedDate, fetch_date.ToInternalValue());

  // Say the seed was last fetched now, so it will not fetch from the
  // server
  local_state.SetInt64(variations::prefs::kVariationsLastFetchTime, base::Time::Now().ToInternalValue());
  // Seed data read-------------------------------------------------------------
  ...
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
    ...
    SetUpFieldTrials();
      ...
      const std::string group_name =
                base::FieldTrialList::FindFullName("UMA-Uniformity-Trial-100-Percent");
      LOG(WARNING) << "kmilka    group_name is: " << group_name;
  }
