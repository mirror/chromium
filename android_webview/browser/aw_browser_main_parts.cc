...
bool ReadVariationsSeedDataFromFile(PrefService* local_state) {
  ...
  // Set last fetched time as now, so a fetch will not be triggered.
...
void AwBrowserMainParts::SetUpFieldTrials() {
  ...
  variations_field_trial_creator_->SetupFieldTrials(
      ...
      switches::kDisableFeatures, CreateLowEntropyProvider(),
      std::move(feature_list), &webview_field_trials_);
}
