...
void SetSafeBrowsingWhiteList(JNIEnv* env,
                              const JavaParamRef<jclass>&,
                              const JavaParamRef<jobjectArray>& jhosts) {
  std::unique_ptr<std::vector<std::string>> hosts;
  hosts.reset(new std::vector<std::string>);
  base::android::AppendJavaStringArrayToStringVector(env, jhosts, hosts.get());
  AwSafeBrowsingWhitelistManager* whitelist_manager =
      AwBrowserContext::GetDefault()->GetSafeBrowsingWhitelistManager();
  whitelist_manager->SetWhitelistOnUiThread(std::move(hosts));
}
