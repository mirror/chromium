...
public class AwContentsClientBridge {
    ...
    public void onSafeBrowsingHit(
        ...
        ValueCallback<Integer> callback = new ValueCallback<Integer>() {
            @Override
            public void onReceiveValue(final Integer action) {
                ThreadUtils.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        nativeTakeSafeBrowsingAction(
                                mNativeContentsClientBridge, action, requestId);
                    }
                });
            }
        };

        mClient.getCallbackHelper().postOnSafeBrowsingHit(request, threatType, callback);
    }
    ...
    private native void nativeTakeSafeBrowsingAction(
            long nativeAwContentsClientBridge, int action, int requestId);
