...
/**
 * AwVariationsConfigurationService is a bind service to share the Finch seed with all the WebViews
 * on the system. When a WebView asks for the seed, it will immediately return the local seed if it
 * is not expired. Otherwise it will schedule a seed fetch job to AwVariationsSeedFetchService and
 * return message just to make the WebViews unbind from the service. When multiple WebViews ask for
 * the seed, sIsJobRunning flag will only allow the first to schedule a job. The tryLock mechanism
 * on the job service side is just to make sure that no seed fetch job will be scheduled when there
...
public class AwVariationsConfigurationService extends Service {
    ...
    private static class SeedRequestHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            AwVariationsConfigurationService.handleSeedRequest(msg.replyTo);
        }
    };
    ...
    private static void handleSeedRequest(Messenger mMessenger) {
        synchronized (sScheduleLock) {
            if (isSeedExpired()) {
                ...
                sendBackMessage(mMessenger, MSG_WITHOUT_SEED_DATA);
            } else {
                sIsJobRunning = false;
                sendBackMessage(mMessenger, MSG_WITH_SEED_DATA);
            }
        }
    }
    ...
    private static void sendBackMessage(Messenger msgr, int msgId) {
        if (msgr == null) {
        ...
        if (msgId == MSG_WITH_SEED_DATA) {
            try {
                bundle.putParcelable(SEED_DATA,
    ...
    private static class SeedDataReceiver extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == MSG_WITH_SEED_DATA) {
