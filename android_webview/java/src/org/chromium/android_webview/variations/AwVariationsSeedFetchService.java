...
/**
 * AwVariationsSeedFetchService is a Job Service to fetch test seed data which is used by Finch to
 * enable AB testing experiments in the native code. The fetched data is stored in the local
 * directory which belongs to the Service process. The tryLock mechanism is just to make sure that
 * no seed fetch job will be scheduled when there is one still running. This is a prototype of the
 * Variations Seed Fetch Service which is one part of the work of adding Finch to Android WebView.
 */
...
public class AwVariationsSeedFetchService extends JobService {
    ...
    public boolean onStopJob(JobParameters params) {
        ...
        // Return false here to not reschedule the job.
        return false;
    }
    ...
    private void fetchVariationsSeed() {
        ...
        try {
            if (AwVariationsConfigurationService.isSeedExpiredInServiceDirectory()) {
    ...
    private static class SeedTransferConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            msg.what = AwVariationsConfigurationService.MSG_SEED_TRANSFER;

            Bundle bundle = new Bundle();
            bundle.putByteArray(AwVariationsConfigurationService.KEY_SEED_DATA, mSeedInfo.seedData);
            ArrayList<String> list = new ArrayList<String>(new SeedPreference(mSeedInfo).toList());
            bundle.putStringArrayList(AwVariationsConfigurationService.KEY_SEED_PREF, list);
            msg.setData(bundle);
