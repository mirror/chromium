...
public class AwPdfExporter {
    ...
    public interface AwPdfExporterCallback {
         ...
         * @param success Whether the PDF file is written to the provided file descriptor
         *                successfully.
         * @param pageCount Native side reports how many pages are needed for the entire print
         *                  source to be printed to PDF with the user selected print settings. Note
         *                  that it is not how many pages will actually be written to PDF, though
         *                  they will be the same when user selects all pages to print.
         */
    ...
    public void exportToPdf(final ParcelFileDescriptor fd, PrintAttributes attributes, int[] pages,
        ...
        if (mNativeAwPdfExporter == 0) {
            resultCallback.pdfWritingDone(false, 0);
            return;
        }
    ...
    private void setNativeAwPdfExporter(long nativePdfExporter) {
        ...
        if (nativePdfExporter == 0 && mResultCallback != null) {
            try {
                mResultCallback.pdfWritingDone(false, 0);
    ...
    @CalledByNative
    private void didExportPdf(boolean success, int pageCount) {
        mResultCallback.pdfWritingDone(success, pageCount);
        mResultCallback = null;
        mAttributes = null;
