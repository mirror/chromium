...
class STORAGE_EXPORT BlobDataBuilder {
 public:
  ...
  const std::vector<BlobEntry::ItemCopyEntry>& copies() const {
    return copies_;
  }

  std::vector<BlobEntry::ItemCopyEntry> ReleaseCopies() {
 ...
 private:
  ...
  base::CheckedNumeric<uint64_t> transport_quota_needed_ = 0;
  base::CheckedNumeric<uint64_t> copy_quota_needed_ = 0;
  bool has_blob_errors_ = false;
  bool found_memory_transport_ = false;
  bool found_file_transport_ = false;

  std::vector<scoped_refptr<ShareableBlobDataItem>> shareable_items_;
  std::vector<scoped_refptr<ShareableBlobDataItem>> pending_transport_items_;
  std::set<std::string> dependent_blob_uuids_;
  std::vector<BlobEntry::ItemCopyEntry> copies_;
