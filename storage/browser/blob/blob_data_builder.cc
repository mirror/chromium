...
void BlobDataBuilder::AppendData(const char* data, size_t length) {
  ...
  shareable_items_.push_back(std::move(shareable_item));
...
void BlobDataBuilder::AppendFile(const FilePath& file_path,
  ...
  if (!unknown_size)
    UMA_HISTOGRAM_COUNTS_1M("Storage.BlobItemSize.File", offset / 1024);
}
...
void BlobDataBuilder::AppendBlob(const std::string& uuid,
  ...
  if (offset == 0 && length == ref_entry->total_size()) {
    for (const auto& shareable_item : ref_entry->items())
      shareable_items_.push_back(shareable_item);
    return;
  }
...
void BlobDataBuilder::SliceBlob(const BlobEntry* source,
  ...
  for (uint64_t total_sliced = 0;
    ...
    if (reusing_blob_item) {
      ...
      shareable_items_.push_back(source_items[item_index]);
    ...
    shareable_items_.push_back(
        new ShareableBlobDataItem(std::move(data_item), state));
    if (need_copy) {
      copies_.push_back(ItemCopyEntry(source_items[item_index], item_offset,
                                      shareable_items_.back()));
    }
    item_offset = 0;
  }
}
...
void BlobDataBuilder::AppendDiskCacheEntry(
  ...
  scoped_refptr<BlobDataItem> item = new BlobDataItem(
      std::move(element), data_handle, disk_cache_entry,
      disk_cache_stream_index, kInvalidDiskCacheSideStreamIndex);
...
std::unique_ptr<BlobDataSnapshot> BlobDataBuilder::CreateSnapshot() const {
  ...
  for (const auto& item : shareable_items_)
...
void PrintTo(const BlobDataBuilder& x, std::ostream* os) {
  ...
  *os << "<BlobDataBuilder>{uuid: " << x.uuid()
  ...
  for (const auto& item : x.shareable_items_) {
