...
void BlobRegistryImpl::BlobUnderConstruction::ResolvedAllBlobUUIDs() {
  ...
  // the blob. Do this before informing waiting blobs that all our UUIDs are now
  // resolved, to ensure this blob has a size calculated before other blobs try
  // to reference this blob.
...
void BlobRegistryImpl::BlobUnderConstruction::ResolvedAllBlobDependencies() {
  ...
  for (size_t i = 0; i < elements_.size(); ++i) {
    const auto& element = elements_[i];
    if (element->is_blob()) {
      builder_.AppendBlob(referenced_blob_uuids_[i],
                          element->get_blob()->offset,
