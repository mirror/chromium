...
class BlobRegistryImpl::BlobUnderConstruction {
 ...
 private:
  ...
  // Used to send out requests for data when transport strategy is IPC. Sends
  // all requests in parallel, so replies could return in any order (since each
  // BytesProvider is a independent message pipe).
  void SendReplyRequests();
  void OnBytesReply(size_t element_index, const std::vector<uint8_t>& data);

  // Used to send out the next data pipe request when transport strategy is
  // SHARED_MEMORY. |next_element| should be the index of the first element that
  // hasn't been transported yet (but doesn't have to actually point to a bytes
  // element, this method will skip over any non-bytes elements).
  void SendDataPipeRequests(
      size_t next_element,
      std::vector<size_t>::const_iterator builder_element_index_iterator);
  void OnDataPipeReadable(
      size_t element_index,
      std::vector<size_t>::const_iterator builder_element_index_iterator,
      MojoResult result);

  // Used to send out requests for data when transport strategy is FILE.
  ...
  std::vector<size_t> block_ids_for_data_pipes_;

  // Used for FILE and IPC transports.
    ...
    if (element->is_bytes()) {
      switch (memory_strategy_) {
        ...
        case MemoryStrategy::SHARED_MEMORY: {
          const size_t max_block_size = limits.max_bytes_data_item_size;
          for (size_t offset = 0; offset < element->get_bytes()->length;
               offset += max_block_size) {
            size_t block_size = std::min<size_t>(
                element->get_bytes()->length - offset, max_block_size);
            size_t block_idx = builder_.AppendFutureData(block_size);
            if (offset == 0)
              block_ids_for_data_pipes_.push_back(block_idx);
          }
          break;
        }
...
void BlobRegistryImpl::BlobUnderConstruction::OnReadyForTransport(
  ...
  switch (memory_strategy_) {
    case MemoryStrategy::IPC:
      ...
      break;
    case MemoryStrategy::SHARED_MEMORY:
      SendDataPipeRequests(0, block_ids_for_data_pipes_.begin());
      break;
    case MemoryStrategy::FILE:
...
void BlobRegistryImpl::BlobUnderConstruction::OnBytesReply(
  ...
  // No longer need BytesProvider for this element, so clear it out to close the
  // pipe.
...
void BlobRegistryImpl::BlobUnderConstruction::SendDataPipeRequests(
  ...
  if (next_element >= elements_.size()) {
    DCHECK(builder_element_index_iterator == block_ids_for_data_pipes_.end());
    CompleteTransport();
    return;
  }
  ...
  data_pipe_watcher_.Watch(
      ...
      base::Bind(&BlobUnderConstruction::OnDataPipeReadable,
                 ...
                 builder_element_index_iterator));
...
void BlobRegistryImpl::BlobUnderConstruction::OnDataPipeReadable(
  ...
  const size_t block_idx_offset = current_data_pipe_offset_ / max_block_size;
  size_t offset_in_block =
      current_data_pipe_offset_ - block_idx_offset * max_block_size;
  size_t builder_element_index =
      *builder_element_index_iterator + block_idx_offset;
  while (true) {
    uint32_t num_bytes = 0;
    const void* source_buffer;
    MojoResult read_result =
        mojo::BeginReadDataRaw(data_pipe_handle_.get(), &source_buffer,
                               &num_bytes, MOJO_READ_DATA_FLAG_NONE);
    if (read_result == MOJO_RESULT_SHOULD_WAIT)
      break;
    DCHECK_EQ(read_result, MOJO_RESULT_OK);

    // Only read as many bytes as can fit in current block.
    uint32_t bytes_to_read =
        std::min<uint32_t>(num_bytes, max_block_size - offset_in_block);

    char* output_buffer = builder_.GetFutureDataPointerToPopulate(
        builder_element_index, offset_in_block, bytes_to_read);
    if (!output_buffer) {
      MarkAsBroken(
          BlobStatus::ERR_INVALID_CONSTRUCTION_ARGUMENTS,
    ...
    read_result = mojo::EndReadDataRaw(data_pipe_handle_.get(), bytes_to_read);
    DCHECK_EQ(read_result, MOJO_RESULT_OK);

    current_data_pipe_offset_ += bytes_to_read;
    ...
    if (current_data_pipe_offset_ >=
      ...
      SendDataPipeRequests(element_index + 1,
                           std::next(builder_element_index_iterator));
      return;
    }

    offset_in_block += bytes_to_read;
    if (offset_in_block >= max_block_size) {
      offset_in_block = 0;
