...
std::pair<FileCreationInfo, int64_t> CreateFileAndWriteItems(
  ...
  for (const auto& item : items) {
    DCHECK_EQ(BlobDataItem::Type::kBytes, item->type());
    size_t length = base::checked_cast<size_t>(item->length());
    size_t bytes_left = length;
    while (bytes_left > 0) {
      bytes_written =
          file.WriteAtCurrentPos(item->bytes().data() + (length - bytes_left),
...
void BlobMemoryController::MaybeScheduleEvictionUntilSystemHealthy(
  ...
  while (total_memory_usage > limits_.effective_max_disk_space ||
    ...
    std::vector<scoped_refptr<BlobDataItem>> items_for_paging;
    for (auto& shared_blob_item : items_to_swap) {
      items_paging_to_file_.insert(shared_blob_item->item_id());
      items_for_paging.push_back(shared_blob_item->item());
    }
    ...
    base::PostTaskAndReplyWithResult(
        ...
        base::BindOnce(&CreateFileAndWriteItems, blob_storage_dir_,
                       ...
                       file_runner_, std::move(items_for_paging),
                       total_items_size),
