...
void CopyOrMoveOperationDelegate::StreamCopyHelper::DidRead(
  ...
  Write(std::move(callback),
        new net::DrainableIOBuffer(io_buffer_.get(), result));
}
...
void CopyOrMoveOperationDelegate::StreamCopyHelper::Write(
  ...
  int result = writer_->Write(
      ...
      base::Bind(&StreamCopyHelper::DidWrite, weak_factory_.GetWeakPtr(),
                 copyable_callback, buffer));
  if (result != net::ERR_IO_PENDING)
    DidWrite(copyable_callback, buffer, result);
}
...
void CopyOrMoveOperationDelegate::StreamCopyHelper::Flush(
  ...
  int result = writer_->Flush(base::Bind(&StreamCopyHelper::DidFlush,
                                         weak_factory_.GetWeakPtr(),
                                         copyable_callback, is_eof));
  if (result != net::ERR_IO_PENDING)
    DidFlush(copyable_callback, is_eof, result);
}
