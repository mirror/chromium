...
void AsyncFileUtilAdapter::EnsureFileExists(
  ...
  const bool success = context_ptr->task_runner()->PostTaskAndReply(
      ...
      base::BindOnce(&EnsureFileExistsHelper::Reply, Owned(helper),
                     std::move(callback)));
  DCHECK(success);
}
...
void AsyncFileUtilAdapter::Touch(
  ...
  const bool success = base::PostTaskAndReplyWithResult(
      context_ptr->task_runner(), FROM_HERE,
      BindOnce(&FileSystemFileUtil::Touch, Unretained(sync_file_util_.get()),
               base::Owned(context_ptr), url, last_access_time,
               last_modified_time),
      std::move(callback));
  DCHECK(success);
}
...
void AsyncFileUtilAdapter::Truncate(
  ...
  const bool success = base::PostTaskAndReplyWithResult(
      context_ptr->task_runner(), FROM_HERE,
      BindOnce(&FileSystemFileUtil::Truncate, Unretained(sync_file_util_.get()),
...
void AsyncFileUtilAdapter::CopyFileLocal(
  ...
  const bool success = base::PostTaskAndReplyWithResult(
      context_ptr->task_runner(), FROM_HERE,
      BindOnce(&FileSystemFileUtil::CopyOrMoveFile,
...
void AsyncFileUtilAdapter::CopyInForeignFile(
  ...
  const bool success = base::PostTaskAndReplyWithResult(
      context_ptr->task_runner(), FROM_HERE,
      BindOnce(&FileSystemFileUtil::CopyInForeignFile,
               Unretained(sync_file_util_.get()), base::Owned(context_ptr),
...
void AsyncFileUtilAdapter::DeleteFile(
  ...
  const bool success = base::PostTaskAndReplyWithResult(
      context_ptr->task_runner(), FROM_HERE,
      BindOnce(&FileSystemFileUtil::DeleteFile,
               Unretained(sync_file_util_.get()), base::Owned(context_ptr),
               url),
      std::move(callback));
  DCHECK(success);
...
void AsyncFileUtilAdapter::DeleteDirectory(
  ...
  const bool success = base::PostTaskAndReplyWithResult(
      context_ptr->task_runner(), FROM_HERE,
      BindOnce(&FileSystemFileUtil::DeleteDirectory,
               Unretained(sync_file_util_.get()), base::Owned(context_ptr),
               url),
      std::move(callback));
  DCHECK(success);
