...
for i in range(0, hashlen, 4):
  ...
  if p == 0xffffffff:  # Not set
    ...
    print '%04x' % p,
    s, _, _, _, fh = gettystr(p)
    assert fh == hash_tystr(s)
    assert i/4 == hash_tystr(s)%(0x200/4)
print
print
...
# First uint32 is offset to type in 1st section if it's an Interface name
# or 0xffffffff else.
# Second uint32 is probably offset into this section, the hashtable "next"
# pointer for hash chaining.
# Third u8 might be flags; 0 for library name, 0x38 for interfaces.
# Fourth u16 might be the hash value; at least the low 7 bits match the
# offset the string has in section 7.
