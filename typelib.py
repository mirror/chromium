...
nwords, = struct.unpack('<I', tlb[0x20:0x24])
#print nwords
...
for i in range(nwords):
  word, = struct.unpack('<I', tlb[0x54 + 4*i:0x54 + 4*i + 4])
  assert word == 100 * i
  print '%d' % word
print
...
for i in range(15):
  off, len, minusone, fifteen = \
      struct.unpack('<IIII', tlb[diroff + i*16: diroff + i*16 + 16])
  assert minusone == 0xffffffff
  assert fifteen == 0xf
  print '%x %x => %x' % (off, len, off+len)
  entries.append((off, len))
