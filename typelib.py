...
def getguid(p):
  ...
  assert 0 <= p
  assert p + 24 <= guidlen
  g0, g1, g2, g3, n0, n1 = struct.unpack_from('<IHH8sii', tlb, guidoff + p)
...
def guidhash(guidstr):
  # This is almost right, but if I set bit 2 or 4 in the two 0s here, things
  # blow up: [ uuid(11121418-1112-1112-1112-91a204a80192) ]
  import operator, uuid
  u = uuid.UUID(guidstr)
  words = struct.unpack('<8H', u.bytes_le)
  return reduce(operator.xor, words) % 0x80
...
# all-0s-guid goes to index 0
# 0001 followed by all 0s goes to 1
# 0002 followed by all 0s goes to 2
# 0010 followed by all 0s goes to 16
# 0100 followed by all 0s goes to 0 (so just mod 0x80)
# 00000001 followed by all 0s goes to 1
# 00000002 followed by all 0s goes to 2
# 00000010 followed by all 0s goes to 16
# 00000100 followed by all 0s goes to 0 (so just mod 0x80)
# 00010001 followed by all 0s goes to 0
# both 0001002 and 00020001 followed by all 0s go to 3
# => first 2 uint16s just get xor'd
# Same seems to be true for 3rd and 4th uint16
# Same seems to be true for rest of GUID, except that words are flipped there
