...
for i in range(nwords):
  ...
  is_struct = ord(tlb[infooff]) == 0x21
  ...
  print '  ...%d methods, %d fields...' % (n_meth, n_field)
  ...
  for i in range(n_meth + n_field):
    ...
    # For fields:
    # @+0x10: initial value (at least for enums). If top byte is 0x8c, the
    #         lower bytes contain value inline.  Else, it references custom data
    #         (section 12).
    ...
    if i < n_meth:
      ...
      elif value & 0x8c000000:
        value = value & ~0x8c000000
      else:
        value = getcust(value)
    values.append(value)
