...
for i in range(nwords):
  ...
  print ts
  n_meth, = struct.unpack_from('<H', tlb, infooff + 0x18)
  print '  ...%d methods...' % n_meth

  methoff, = struct.unpack_from('<I', tlb, infooff + 4)
  # Method data:
  # uint32 offset to past method data (where id table, param name table are)
  # n_methods: uint16 len, that much data
  # n_methods: uint32 (just 1, 2, 3, ...?)
  # n_methods offsets into typetab method names
  # n_methods uint32 offset to parameter dtor for i'th method
  methnameoff = methoff + 4
  helps = []
  params = []
  lastargname = 0
  for i in range(n_meth):
    size, = struct.unpack_from('<H', tlb, methnameoff)
    # In here (@+0 is size):
    # @+0x14: num params
    ...
    if header_size >= 0x1c:
      helpstr, = struct.unpack_from('<II', tlb, methnameoff + 0x1c)
    ...
    for j in range(n_params):
      argtype, argname, argflags = struct.unpack_from(
          '<III', tlb, methnameoff + header_size + j*12)
      if argname == 0xffffffff:
        argname = lastargname  # weird special case
      thisparams.append((argtype, gettystr(argname)[0], argflags))
      lastargname = argname
    params.append(thisparams)

    methnameoff += size
  methnameoff += 4 * n_meth
  for i in range(n_meth):
    name, = struct.unpack_from('<I', tlb, methnameoff + 4*i)
    s, u0, u1, u2, u3 = gettystr(name)
    print '  %s(' % s
    for argtype, argname, argflags in params[i]:
      print '    %s %x %x' % (argname, argtype, argflags)
    print '      ): %s' % helps[i]
print
