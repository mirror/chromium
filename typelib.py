...
for i in range(nwords):
  ...
  # Method data:
  # uint32 header (what does it meeeaaan)
  ...
  for i in range(n_meth):
    size = struct.unpack_from('<H', tlb, methnameoff)[0]
    methnameoff += size
  methnameoff += 4 * n_meth
  for i in range(methnameoff, methnameoff + 4*n_meth, 4):
    s, u0, u1, u2, u3 = gettystr(struct.unpack_from('<I', tlb, i)[0])
    print '  %s' % s
print
