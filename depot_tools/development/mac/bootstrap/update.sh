...
# the url used below is hard coded to work for updating release/mac, so the
...
then
  ...
  then
    ...
    # 3. Change the url from the "mac/bootstrap to "release/mac" so we have the
    #    right thing.
    our_url=`(cd $base_dir ; svn info 2> /dev/null | sed -n -e '/^URL: /s///p')`
    url=`echo $our_url | sed -n -e 's/mac\/bootstrap/release\/mac/p'`
  else
