...
class PresubmitUnittest(unittest.TestCase):
  ...
  def testListRelevantPresubmitFiles(self):
    ...
    self.failUnless(os.path.normpath('PRESUBMIT.py') in presubmit_files)
    self.failUnless(os.path.normpath('foo/haspresubmit/PRESUBMIT.py') in
                    presubmit_files)
  ...
  def testGclChange(self):
    ...
    expected_paths = [os.path.normpath(f[1]) for f in files]
    self.failUnless(
    ...
    files = [[x[0], os.path.normpath(x[1])] for x in files]
  ...
  def testInputApiPresubmitScriptFiltering(self):
    ...
    self.failUnless(affected_files[0].LocalPath() ==
                    os.path.normpath('foo/blat.cc'))
    self.failUnless(affected_files[1].LocalPath() ==
                    os.path.normpath('foo/blat/binary.dll'))
    self.failUnless(affected_files[2].LocalPath() ==
                    os.path.normpath('foo/mat/beingdeleted.txt'))
