...
bool DomainIs(base::StringPiece canonicalized_host, base::StringPiece domain) {
  if (canonicalized_host.empty() || domain.empty())
    return false;
  ...
  if (canonicalized_host.back() == '.' && domain.back() != '.')
    --host_len;

  if (host_len < domain.length())
    return false;
  ...
  const char* host_first_pos =
      canonicalized_host.data() + host_len - domain.length();

  if (base::StringPiece(host_first_pos, domain.length()) != domain) {
    return false;
  }
  ...
  if (domain[0] != '.' && host_len > domain.length() &&
      *(host_first_pos - 1) != '.') {
    return false;
  }
