...
public class MultiWindowIntegrationTest {
    ...
    @CommandLineFlags.Add({ChromeSwitches.DISABLE_TAB_MERGING_FOR_TESTING,
            ChromeSwitches.DISABLE_FIRST_RUN_EXPERIENCE})
    public void
    testMoveTabTwice() throws InterruptedException {
        ...
        // Start second tabbed activity.
        final ChromeTabbedActivity2 cta2 = createSecondChromeTabbedActivity(cta);
        ...
        // At this point cta should have 1 tab (NTP), and cta2 should have 2 tabs (NTP, 'google').
        waitForTab("CTA", cta, 1, Tab.INVALID_TAB_ID);
        waitForTab("CTA2", cta2, 2, googleTabId);
        ...
        // At this point cta2 should have 1 tab (NTP), and cta should have 2 tabs (NTP, 'google').
        waitForTab("CTA2", cta2, 1, Tab.INVALID_TAB_ID);
        waitForTab("CTA", cta, 2, googleTabId);
    }

    /**
     * Waits until 'activity' has 'expectedTabCount' tabs, and its active tab has
     ...
     */
    private static void waitForTab(final String tag, final ChromeTabbedActivity activity,
            final int expectedTabCount, final int expectedActiveTabId) {
        CriteriaHelper.pollUiThread(new Criteria() {
            @Override
            public boolean isSatisfied() {
                int actualTabCount = activity.getTabModelSelector().getModel(false).getCount();
                if (expectedTabCount != actualTabCount) {
                    updateFailureReason(
                            String.format((Locale) null, "%s: tab count is wrong: %d (expected %d)",
                                    tag, actualTabCount, expectedActiveTabId));
                    return false;
                }
                if (expectedActiveTabId != Tab.INVALID_TAB_ID) {
                    ...
                    if (expectedActiveTabId != actualActiveTabId) {
                        updateFailureReason(String.format((Locale) null,
                                "%s: active tab ID is wrong: %d (expected %d)", tag,
                                actualActiveTabId, expectedActiveTabId));
                        return false;
                    }
                }
                return true;
