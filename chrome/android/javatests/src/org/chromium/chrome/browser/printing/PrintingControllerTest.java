...
public class PrintingControllerTest {
    ...
    public void testPrintCloseWindowBeforeStart() throws Throwable {
        ...
        setPendingPrintOnUiThread(printingController, currentTab);

        closeTabOnUiThread();
        Assert.assertTrue("currentTab should be closed already.",
                currentTab == null || !currentTab.isInitialized());

        startPendingPrintOnUiThread(printingController);
    }
    ...
    private void closeTabOnUiThread() {
        try {
            ...
            Assert.fail("Error on closing tab on the UI thread: " + e);
        }
    }

    private void setPendingPrintOnUiThread(final PrintingControllerImpl controller, final Tab tab) {
        try {
            ...
            InstrumentationRegistry.getInstrumentation().runOnMainSync(new Runnable() {
                @Override
                public void run() {
                    controller.setPendingPrint(
                            new TabPrinter(tab), mockPrintManagerDelegate, -1, -1);
                }
            });
        } catch (Throwable e) {
            Assert.fail("Error on calling setPendingPrint of PrintingControllerImpl " + e);
        }
    }

    private void startPendingPrintOnUiThread(final PrintingControllerImpl controller) {
        try {
            InstrumentationRegistry.getInstrumentation().runOnMainSync(new Runnable() {
                @Override
                public void run() {
                    controller.startPendingPrint(null);
                }
            });
        } catch (Throwable e) {
            Assert.fail("Error on calling startPendingPrint of PrintingControllerImpl " + e);
        }
    }
