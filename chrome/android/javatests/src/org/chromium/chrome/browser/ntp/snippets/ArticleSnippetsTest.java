...
public class ArticleSnippetsTest {
    ...
    public void testDownloadSuggestion() throws IOException {
        ...
        ThreadUtils.runOnUiThreadBlocking(new Runnable() {
            ...
            public void run() {
                ...
                mRecyclerView = new SuggestionsRecyclerView(mActivityTestRule.getActivity());
                ContextMenuManager.TouchEnabledDelegate touchEnabledDelegate =
                        new ContextMenuManager.TouchEnabledDelegate() {
                            @Override
        ...
        InstrumentationRegistry.getInstrumentation().waitForIdleSync();
        List<ThumbnailProvider.ThumbnailRequest> requests = mThumbnailProvider.getRequests();
        Assert.assertEquals(1, requests.size());
        final ThumbnailProvider.ThumbnailRequest request = requests.get(0);
    ...
    private class MockUiDelegate implements SuggestionsUiDelegate {
        ...
        private final ImageFetcher mImageFetcher = new MockImageFetcher();
        ...
        private class MockImageFetcher extends ImageFetcher {
            public MockImageFetcher() {
                super(MockUiDelegate.this, null);
            }
            ...
            public ArticleThumbnailRequest makeArticleThumbnailRequest(
                    SnippetArticle suggestion, final Callback<Bitmap> callback) {
            ...
            @Override
            public LargeIconRequest makeLargeIconRequest(final String url,
                    final int largeIconSizePx, final LargeIconBridge.LargeIconCallback callback) {
