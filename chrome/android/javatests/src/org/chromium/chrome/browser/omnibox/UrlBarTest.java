...
public class UrlBarTest {
    ...
    public void testSendCursorPosition() throws InterruptedException, TimeoutException {
        ...
        // text.
        final String textA = "a";
        ThreadUtils.runOnUiThreadBlocking(new Runnable() {
            @Override
            public void run() {
                urlBar.getInputConnection().commitText(textA, 1);
            }
        });
        autocompleteHelper.waitForCallback(0);
        // omnmibox text: a|
        ...
        // text.
        final String textCD = "cd";
        ThreadUtils.runOnUiThreadBlocking(new Runnable() {
            @Override
            public void run() {
                urlBar.getInputConnection().commitText(textCD, 1);
            }
        });
        ...
        final String textB = "b";
        ThreadUtils.runOnUiThreadBlocking(new Runnable() {
            @Override
            public void run() {
                urlBar.getInputConnection().commitText(textB, 1);
            }
        });
