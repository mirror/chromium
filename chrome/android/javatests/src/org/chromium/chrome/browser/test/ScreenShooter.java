 ...
 * <em>screenshot_dir/test_class_directory/test_directory/shot_name random.png</em>.
 * The associated JSON file describing the screenshot is saved as
 * <em>screenshot_dir/test_class_directory/test_directory/shot_name random.json</em>.
 * <p>
 * <em>screenshot_dir</em> comes from the instrumentation test command line, which is set by the
 * test
 * runners. <em>test_class_directory</em> and <em>test_directory</em> can both the set by the
 * {@link ScreenShooter.Directory} annotation. <em>test_class_directory</em> defaults to nothing
 * (i.e. no
 * directory created at this level), and <em>test_directory</em> defaults to the name of the
 * individual test. <em>random</em> is a random value to make the filenames unique.
 * <p>
 ...
 * Catalogue viewer. The metadata fields are contain data (such as the exact time the shot was
 * taken) that are less suitable for filtering screenshot sets, but never the less may be of
...
public class ScreenShooter extends TestWatcher {
    ...
    public void shoot(String shotName, Map<String, String> tags) {
        String default_tags[] = new String[] {TEST_CLASS_TAG, TEST_METHOD_TAG, SCREENSHOT_NAME_TAG,
                DEVICE_MODEL_TAG, DISPLAY_SIZE_TAG, ORIENTATION_TAG, ANDROID_VERSION_TAG,
                CHROME_VERSION_TAG, CHROME_CHANNEL_TAG, LOCALE_TAG, UPLOAD_TIME_TAG};
        List<String> default_tag_list = Arrays.asList(default_tags);
        for (String tag : tags.keySet()) {
            if (default_tag_list.contains(tag)) {
                fail("Test is attempting to override the default tag \"" + tag + "\"");
            }
        }
        shootInternal(shotName, tags);
    }

    private void shootInternal(String shotName, Map<String, String> tags) {
        tags.put(TEST_CLASS_TAG, mTestClassName);
        tags.put(TEST_METHOD_TAG, mTestMethodName);
        tags.put(SCREENSHOT_NAME_TAG, shotName);
        tags.put(DEVICE_MODEL_TAG, Build.MANUFACTURER + " " + Build.MODEL);
        Point displaySize = mDevice.getDisplaySizeDp();
        tags.put(DISPLAY_SIZE_TAG,
        ...
        tags.put(ORIENTATION_TAG,
                orientation == Configuration.ORIENTATION_LANDSCAPE ? "landscape" : "portrait");
        tags.put(ANDROID_VERSION_TAG, Build.VERSION.RELEASE);
        tags.put(CHROME_VERSION_TAG, Integer.toString(ChromeVersionInfo.getProductMajorVersion()));
        ...
        tags.put(CHROME_CHANNEL_TAG, channelName);
        tags.put(LOCALE_TAG, Locale.getDefault().toString());
