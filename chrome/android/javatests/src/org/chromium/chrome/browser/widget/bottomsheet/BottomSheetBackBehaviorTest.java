...
public class BottomSheetBackBehaviorTest {
    ...
    /** A TabObserver for watching page loads. */
    private static class TestTabObserver extends EmptyTabObserver {
        private final CallbackHelper mLoadFinishedCallbackHelper = new CallbackHelper();

        @Override
        public void onPageLoadFinished(Tab tab) {
            mLoadFinishedCallbackHelper.notifyCalled();
        }
    }
    ...
    public void testBackButton_backFromInternalNewTab()
        ...
        Tab tab = launchNewTabFromChrome();

        assertEquals("Tab should be on about:blank.", "about:blank",
                mBottomSheet.getActiveTab().getUrl());
    ...
    public void testBackButton_backWithNavigation()
        ...
        TestTabObserver tabObserver = new TestTabObserver();
        tab.addObserver(tabObserver);

        EmbeddedTestServer testServer = EmbeddedTestServer.createAndStartServer(
                InstrumentationRegistry.getInstrumentation().getContext());

        int loadFinishedCount = tabObserver.mLoadFinishedCallbackHelper.getCallCount();
        final String testUrl = testServer.getURL(TEST_PAGE);
        ThreadUtils.runOnUiThreadBlocking(new Runnable() {
            @Override
            public void run() {
                tab.loadUrl(new LoadUrlParams(testUrl));
            }
        });

        tabObserver.mLoadFinishedCallbackHelper.waitForCallback(loadFinishedCount, 1);
        ...
        loadFinishedCount = tabObserver.mLoadFinishedCallbackHelper.getCallCount();
        pressBackButton();
        endBottomSheetAnimations();
        tabObserver.mLoadFinishedCallbackHelper.waitForCallback(loadFinishedCount, 1);
    ...
    private Tab launchNewTabFromChrome() throws ExecutionException {
