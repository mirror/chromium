...
public class TileGridLayoutTest {
    ...
    @SmallTest
    @Feature({"NewTabPage"})
    public void testNumColumnsPaddingSizes() {
        TileGridLayout tileGridLayout = getTileGridLayout();
        Assert.assertEquals(TileGridLayout.sPaddingStart,
                ApiCompatibilityUtils.getPaddingStart(tileGridLayout));
        Assert.assertEquals(
                TileGridLayout.sPaddingEnd, ApiCompatibilityUtils.getPaddingEnd(tileGridLayout));
    }
    ...
    public void testNumColumnsCalculatedEqualToMeasurement() {
        ...
        TileGridLayout layout = getTileGridLayout();
        layout.setMaxRows(1);
        ...
        Assert.assertEquals(
                "Calculated number of columns does not equal the measured number of columns.",
                visibleTileViews, Math.min(maxColumns, TileGridLayout.calculateNumColumns()));
        Assert.assertTrue("Visible tiles do not contain home page.", hasHomePage);
    }
