...
public class TileGridLayoutTest {
    ...
    public void testModernTileGridAppearance_Full() throws IOException, InterruptedException {
        View v = renderTiles(makeSuggestions(FAKE_MOST_VISITED_URLS.length));

        setOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT, mActivityTestRule.getActivity());
        mRenderTestRule.render(v, "modern_full_grid_portrait");

        setOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE, mActivityTestRule.getActivity());
        mRenderTestRule.render(v, "modern_full_grid_landscape");

        // In landscape, modern tiles should use all available space.
        int tileGridMaxWidthPx =
                v.getResources().getDimensionPixelSize(R.dimen.tile_grid_layout_max_width);
        if (((FrameLayout) v.getParent()).getMeasuredWidth() > tileGridMaxWidthPx) {
            assertThat(v.getMeasuredWidth(), greaterThan(tileGridMaxWidthPx));
        }
    }
    ...
    public void testTileGridAppearance_Full() throws IOException, InterruptedException {
        ...
        setOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT, mActivityTestRule.getActivity());
        mRenderTestRule.render(v, "full_grid_portrait");

        setOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE, mActivityTestRule.getActivity());
        mRenderTestRule.render(v, "full_grid_landscape");

        // In landscape, modern tiles should use at most tile_grid_layout_max_width px.
        int tileGridMaxWidthPx =
                v.getResources().getDimensionPixelSize(R.dimen.tile_grid_layout_max_width);
        if (((FrameLayout) v.getParent()).getMeasuredWidth() > tileGridMaxWidthPx) {
            assertThat(v.getMeasuredWidth(), lessThanOrEqualTo(tileGridMaxWidthPx));
        }
    }
    ...
    public void testModernTileGridAppearance_Two() throws IOException, InterruptedException {
        View v = renderTiles(makeSuggestions(2));

        setOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT, mActivityTestRule.getActivity());
        mRenderTestRule.render(v, "modern_two_tiles_grid_portrait");

        setOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE, mActivityTestRule.getActivity());
        mRenderTestRule.render(v, "modern_two_tiles_grid_landscape");
    }
    ...
    public void testTileGridAppearance_Two() throws IOException, InterruptedException {
        ...
        setOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT, mActivityTestRule.getActivity());
        mRenderTestRule.render(v, "two_tiles_grid_portrait");

        setOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE, mActivityTestRule.getActivity());
        mRenderTestRule.render(v, "two_tiles_grid_landscape");
    }
    ...
    private NewTabPage setUpFakeDataToShowOnNtp(int homePagePosition) throws InterruptedException {
        ...
        NewTabPage mNtp = (NewTabPage) mTab.getNativePage();

        RecyclerViewTestUtils.waitForStableRecyclerView(mNtp.getNewTabPageView().getRecyclerView());
        return mNtp;
    }
