...
public class PaymentManifestDownloaderTest implements ManifestDownloadCallback {
    ...
    public void testDownloadWebAppManifest() throws Throwable {
        final URI uri = new URI(mServer.getURL("/components/payments/test/data/app.json"));
        mRule.runOnUiThread(new Runnable() {
            @Override
            public void run() {
    ...
    public void testDownloadPaymentMethodManifest() throws Throwable {
        final URI uri = new URI(mServer.getURL("/components/payments/test/data/webpay"));
        mRule.runOnUiThread(new Runnable() {
            @Override
            public void run() {
    ...
    public void testSeveralDownloadsAtOnce() throws Throwable {
        ...
        final URI paymentMethodUri2 =
                new URI(mServer.getURL("/components/payments/test/data/webpay"));
        final URI webAppUri1 = new URI(mServer.getURL("/no-such-app.json"));
        final URI webAppUri2 = new URI(mServer.getURL("/components/payments/test/data/app.json"));
        mRule.runOnUiThread(new Runnable() {
            @Override
            public void run() {
