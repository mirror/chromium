...
public class ThumbnailDiskStorageTest {
    ...
    private TestThumbnailRetrievalCallback mTestThumbnailRetrievalCallback;
    ...
    private static class TestThumbnailRetrievalCallback implements ThumbnailRetrievalCallback {
    ...
    private static class TestThumbnailDiskStorage extends ThumbnailDiskStorage {
        // Incremented when adding an existing entry and trimming.
        ...
        public TestThumbnailDiskStorage(
                ThumbnailRetrievalCallback requester, TestThumbnailGenerator thumbnailGenerator) {
        ...
        @Override
        public boolean removeFromDiskHelper(Pair<String, Integer> contentIdSizePair) {
            ++removeCount;
            return super.removeFromDiskHelper(contentIdSizePair);
        }
    ...
    private static class TestThumbnailGenerator extends ThumbnailGenerator {
        ...
        public void retrieveThumbnail(
            ...
            onThumbnailRetrieved(
                    request.getContentId(), request.getIconSize(), null, true, callback);
        }
    }

    @Before
    public void setUp() throws Exception {
        mTestThumbnailRetrievalCallback = new TestThumbnailRetrievalCallback();
        mTestThumbnailGenerator = new TestThumbnailGenerator();
        ThreadUtils.runOnUiThreadBlocking(new Runnable() {
            @Override
            public void run() {
                mTestThumbnailDiskStorage = new TestThumbnailDiskStorage(
                        mTestThumbnailRetrievalCallback, mTestThumbnailGenerator);
            }
        });
        assertInitialized();
    }
    ...
    /**
     * Retrieve thumbnail and assert that {@link ThumbnailRetrievalCallback} has received it.
     */
    private void retrieveThumbnailAndAssertRetrieved(final TestThumbnailRequest request)
        ...
        CriteriaHelper.pollInstrumentationThread(new Criteria() {
            ...
            public boolean isSatisfied() {
                return mTestThumbnailRetrievalCallback.retrievedCount.get() == 1;
            }
    ...
    private HashSet<Integer> getIconSizes(String contentId) {
