...
public class WebApkUtils {
    ...
    public static void applyAlertDialogContentStyle(
        ...
        WebApkUtils.setPadding(titleView, context, DIALOG_TEXT_PADDING, DIALOG_TEXT_PADDING,
                DIALOG_TEXT_PADDING, 0);
    }

    /** Adds padding to the list item which consists of an icon and text. */
    public static void addListItemPadding(Context context, ImageView icon, TextView text) {
        int listItemPaddingHalf = WebApkUtils.LIST_ITEM_PADDING / 2;

        // Sets the top and bottom margin for the icon. We set the margin rather than padding to
        // prevent the icon size changing.
        int iconMargin = dpToPx(context, listItemPaddingHalf);
        ViewGroup.LayoutParams iconLayoutParams = icon.getLayoutParams();
        LinearLayout.LayoutParams layoutParams =
                new LinearLayout.LayoutParams(iconLayoutParams.width, iconLayoutParams.height);
        layoutParams.setMargins(0, iconMargin, 0, iconMargin);
        icon.setLayoutParams(layoutParams);

        // Sets the padding for the text.
        WebApkUtils.setPadding(text, context, WebApkUtils.LIST_ITEM_PADDING, listItemPaddingHalf, 0,
                listItemPaddingHalf);
    }
}
