...
public class WebApkServiceImplWrapper extends IWebApkApi.Stub {
    ...
    public void notifyNotification(String platformTag, int platformID, Notification notification) {
        // The WebApkServiceImplWrapper was introduced at the same time when WebAPKs target to SDK
        // 26. That means, we don't need to check whether the target SDK is less than 26 in a WebAPK
        // that has WebApkServiceImplWrapper class.
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            ensureNotificationChannelExists();
            notification = rebuildNotificationWithChannelId(notification);
        }
    ...
    private Notification rebuildNotificationWithChannelId(Notification notification) {
