...
public class WebApkUtilsTest {
    ...
    /**
     * Tests that if there is only one browser supports WebAPK, and we can't decide which browser to
     * use by looking up cache, metadata and default browser, then auto-select that browser for
     * user.
     */
    @Test
    public void testReturnsHostBrowserIfOnlyOneSupportingWekApk() {
        mockInstallBrowsers(new String[] {BROWSER_INSTALLED_SUPPORTING_WEBAPKS,
                BROWSER_INSTALLED_NOT_SUPPORTING_WEBAPKS},
                null);
        setHostBrowserInMetadata(null);
        setHostBrowserInSharedPreferences(null);
    ...
    public void testReturnsNullIfHostBrowserSpecifiedInManifestIsUninstalled() {
        mockInstallBrowsers(sInstalledBrowsers, BROWSER_INSTALLED_SUPPORTING_WEBAPKS);
    ...
    private void mockInstallBrowsers(String[] browsersToInstall, String defaultBrowser) {
        ...
        if (defaultBrowser == null) {
            // Android returns "android" if no default browser is set on device.
