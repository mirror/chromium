...
public class WebApkServiceImplWrapperTest {
    ...
    private static final String FUNCTION_NAME_NOTIFY_NOTIFICATION_WITH_CHANNEL =
            "TRANSACTION_notifyNotificationWithChannel";
    ...
    private static class MockWebApkServiceImplWrapper extends WebApkServiceImplWrapper {
        ...
        protected int getApiCode(String name) {
            return getApiCodeHelper(name, mDelegate);
        }
    }
    ...
    public void testNotifyNotificationMethodIsCalledOnWrapper() {
        try {
            mWrapper.onTransact(getApiCodeHelper(FUNCTION_NAME_NOTIFY_NOTIFICATION,
                                        (IBinder) mMockWebApkServiceImpl),
    ...
    /**
     * Tests that {@link WebApkServiceImpl#notifyNotificationWithChannel()} is called, not the
     * WebApkServiceImplWrapper's.
     */
    @Test
    public void testOtherMethodIsNotCalledOnWrapper() {
        try {
            mWrapper.onTransact(getApiCodeHelper(FUNCTION_NAME_NOTIFY_NOTIFICATION_WITH_CHANNEL,
                                        (IBinder) mMockWebApkServiceImpl),
                    Mockito.mock(Parcel.class), Mockito.mock(Parcel.class), 0);
    ...
    private static int getApiCodeHelper(String name, IBinder binder) {
        try {
            Field f = binder.getClass().getSuperclass().getSuperclass().getDeclaredField(name);
