...
public class ChromeHomeSurveyController implements InfoBarContainer.InfoBarAnimationListener {
     ...
     */
    @IntDef({RejectedReason.SURVEY_INFOBAR_ALREADY_DISPLAYED,
            RejectedReason.CHROME_HOME_ON_FOR_LESS_THAN_ONE_WEEK,
            RejectedReason.FORCE_SURVEY_ON_COMMAND_PRESENT,
            RejectedReason.USER_ALREADY_SAMPLED_TODAY, RejectedReason.MAX_NUMBER_MISSING,
            RejectedReason.ROLLED_NON_ZERO_NUMBER, RejectedReason.USER_SELECTED_FOR_SURVEY})
    @Retention(RetentionPolicy.SOURCE)
    public @interface RejectedReason {
    ...
    boolean hasInfoBarBeenDisplayed() {
        ...
        if (sharedPreferences.getLong(SURVEY_INFO_BAR_DISPLAYED_KEY, -1L) != -1L) {
            ...
            recordSurveyFilteringResult(RejectedReason.SURVEY_INFOBAR_ALREADY_DISPLAYED);
            return false;
        }
    }
    ...
    boolean wasChromeHomeEnabledForMinimumOneWeek() {
        ...
        if (System.currentTimeMillis() - earliestLoggedDate >= ONE_WEEK_IN_MILLIS) {
            ...
            recordSurveyFilteringResult(RejectedReason.CHROME_HOME_ON_FOR_LESS_THAN_ONE_WEEK);
            return false;
        }
    }
    ...
    boolean isRandomlySelectedForSurvey() {
        ...
        if (lastDate == today) {
            recordSurveyFilteringResult(RejectedReason.USER_ALREADY_SAMPLED_TODAY);
            return false;
        }

        int maxNumber = getMaxNumber();
        if (maxNumber == -1) {
            recordSurveyFilteringResult(RejectedReason.MAX_NUMBER_MISSING);
            return false;
        }

        preferences.edit().putInt(DATE_LAST_ROLLED_KEY, today).apply();
        if (getRandomNumberUpTo(maxNumber) == 0) {
            recordSurveyFilteringResult(RejectedReason.USER_SELECTED_FOR_SURVEY);
            return true;
        } else {
            recordSurveyFilteringResult(RejectedReason.ROLLED_NON_ZERO_NUMBER);
            return false;
        }
    }
    ...
    private SurveyInfoBarDelegate getSurveyInfoBarDelegate() {
        return new SurveyInfoBarDelegate() {
            ...
            @Override
            public void onSurveyInfoBarClosed(boolean viaCloseButton) {
                if (viaCloseButton) {
