...
public class PermissionDialogController implements AndroidPermissionRequester.RequestDelegate {
    ...
    private static final int ANDROID_PROMPT = 5;

    @Retention(RetentionPolicy.SOURCE)
    @IntDef({NO_PROMPT, PROMPT_PENDING, PROMPT_OPEN, PROMPT_ACCEPTED, PROMPT_DENIED,
            ANDROID_PROMPT})
    ...
    // tabs, this queue can only have multiple elements when either:
    ...
    // subsequent tab requests a permission
    ...
    private void scheduleDisplay() {
        if (mState == NO_PROMPT && !mRequestQueue.isEmpty()) dequeueDialog();
    }
    ...
    public void onAndroidPermissionAccepted() {
        assert mState == ANDROID_PROMPT;

        // The tab may have navigated or been closed behind the prompt.
    ...
    public void dequeueDialog() {
        assert mState == NO_PROMPT;

        mDialogDelegate = mRequestQueue.remove(0);
        mState = PROMPT_PENDING;
        final ChromeActivity activity = mDialogDelegate.getTab().getActivity();
        ...
        // Supress modals while Chrome Home is open. Eventually we should also either supress while
        // in the tab switcher or makes these prompts tab-modal instead of app-modal.
        if (activity.getBottomSheet() != null && activity.getBottomSheet().isVisible()) {
            BottomSheetObserver bottomSheetObserver = new EmptyBottomSheetObserver() {
                @Override
                public void onSheetClosed(int reason) {
                    activity.getBottomSheet().removeObserver(this);
                    if (reason == BottomSheet.StateChangeReason.NAVIGATION) {
                        // Just dismiss the prompt as it would otherwise be dismissed momentarily
            ...
            activity.getBottomSheet().addObserver(bottomSheetObserver);
        } else {
            showDialog();
        }
    }
    ...
    public void dismissFromNative(PermissionDialogDelegate delegate) {
        if (mDialogDelegate == delegate) {
            if (mState == PROMPT_OPEN) {
                ...
                assert mState == PROMPT_PENDING || mState == ANDROID_PROMPT;
            }
