...
public class PaymentRequestImpl implements PaymentRequest, PaymentRequestUI.Client,
    ...
    public void onInstrumentsReady(PaymentApp app, List<PaymentInstrument> instruments) {
        ...
        // Also check if at least one payment instrument is complete.
        boolean hasCompletePaymentMethod = false;
        Set<String> uniqueCountryCodes = new HashSet<>();
        for (int i = 0; i < mPendingInstruments.size(); ++i) {
            @Nullable
            ...
            if (mPendingInstruments.get(i).isComplete()) hasCompletePaymentMethod = true;
        }
        ...
        // complete.
        mJourneyLogger.setNumberOfSuggestionsShown(
                Section.PAYMENT_METHOD, mPendingInstruments.size(), hasCompletePaymentMethod);
