...
public class WebappDirectoryManager {
    ...
    private void recordNumberOfStaleWebApkUpdateRequestFiles() {
        ...
        if (children != null) {
            for (File child : children) {
                ...
                long lastRequestMs = storage.getLastCheckForWebManifestUpdateTime();
                if ((System.currentTimeMillis() - lastRequestMs) > TimeUnit.DAYS.toMillis(1L)) {
                    ++count;
                }
            }
        }
    ...
    static final File getWebApkUpdateFilePathForStorage(WebappDataStorage storage) {
        return new File(WebappDirectoryManager.getWebApkUpdateDirectory(), storage.getId());
    }
