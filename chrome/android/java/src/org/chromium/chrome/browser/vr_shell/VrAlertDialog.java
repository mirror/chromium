...
public class VrAlertDialog implements DialogRenderer {
    ...
    public boolean show() {
        ...
        //VrShellDelegate.setDialogView(mView);
        if (mView instanceof ViewGroup)
            loopViews((ViewGroup) mView);
        else
            Log.e("PermissionDialogController", "not a group");
        // TODO Initialize corresponding nativeInitVrAlert based on the view
        if (mTextMessages.size() == 2 && mButtons.size() == 2) {
            mNativeVrAlertDialog = nativeInitVrAlert(mNativeVrShellDelegate,
                    //                (long) mIcons.get(0),
                    0, mTextMessages.get(0).toString(), mTextMessages.get(1).toString(),
            ...
            Log.e(TAG, "VRP: Dismiss! size " + mTextMessages.size()); // + "  -  " +
                                                                      // mTextMessages.get(0) + "<>"
                                                                      // + mTextMessages.get(1) +
                                                                      // "<>"+
                                                                      // mTextMessages.get(2));
            //          Log.e(TAG, "VRP: Dismiss! size " + mTextMessages.size() + "  -  " +
            //          mTextMessages.get(0) + "<>" + mTextMessages.get(1) + "<>"+
            //          mTextMessages.get(2));
