...
public class VrAlertDialog extends VrView {
    ...
    protected ChromeAlertDialog mAlertDialog;
    ...
    public boolean show() {
        ...
        mVrLayout = new LinearLayout(mView.getContext());
        ...
        mVrLayout.setOrientation(LinearLayout.VERTICAL);
        LayoutParams params = new LinearLayout.LayoutParams(1100, 500);

        mVrLayout.addView(mView, params);
        mVrLayout.addView(mButtonLayout);

        mTarget = mVrLayout;
        VrShellDelegate.setDialogView(mVrLayout, mButtons.get(0).getText());
        mNativeVrView = initVrView(mNativeVrShellDelegate);
