...
public class ContextualSearchIPH {
    ...
    void beforePanelShown(boolean wasActivatedByTap, boolean isTapSupported) {
        if (!wasActivatedByTap && isTapSupported) {
            maybeShow(FeatureConstants.CONTEXTUAL_SEARCH_PROMOTE_TAP_FEATURE);
        }
    }
    ...
    void onPanelFinishedShowing(boolean wasActivatedByTap) {
        if (!wasActivatedByTap) {
            maybeShow(FeatureConstants.CONTEXTUAL_SEARCH_WEB_SEARCH_FEATURE);
        }
    }
    ...
    void onEntityDataReceived(boolean wasActivatedByTap) {
        if (wasActivatedByTap) {
            maybeShow(FeatureConstants.CONTEXTUAL_SEARCH_PROMOTE_PANEL_OPEN_FEATURE);
        }
    }
    ...
    private void maybeShow(String featureName) {
        ...
        if (mSearchPanel == null || mParentView == null || mProfile == null) return;
