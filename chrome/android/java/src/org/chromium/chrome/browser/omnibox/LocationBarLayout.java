...
public class LocationBarLayout extends FrameLayout
    ...
    @LocationBarButtonType private int getLocationBarButtonToShow() {
        boolean isOffline =
                mToolbarDataProvider.hasTab() && OfflinePageUtils.isOfflinePage(getCurrentTab());
    ...
    public void revertChanges() {
        if (!mUrlHasFocus) {
            ...
            if (NativePageFactory.isNativePageUrl(tab.getUrl(), tab.isIncognito())) {
                ...
                setUrlBarText(mToolbarDataProvider.getText(), mToolbarDataProvider.getCurrentUrl());
                selectAll();
            }
            hideSuggestions();
            UiUtils.hideKeyboard(mUrlBar);
    ...
    private void updateVerboseStatusVisibility() {
        ...
        boolean verboseStatusVisible = !mUrlHasFocus && mToolbarDataProvider.hasTab()
    ...
    public void onClick(View v) {
        if (v == mDeleteButton) {
            ...
            if (mToolbarDataProvider.hasTab() && mWindowAndroid != null) {
    ...
    /**
     * Sets the original url ensuring it is not set to null (empty string is used in that case).
     * @param url A new value for original url.
     */
    private void setOriginalUrl(String url) {
        // Make sure original URL is not set to null.
        mOriginalUrl = url == null ? "" : url;
    }
