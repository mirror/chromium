...
public class DownloadManagerService
    ...
    private static final List<String> FIRST_SEEN_DOWNLOAD_IDS = new ArrayList<String>();
    ...
    private void updateDownloadProgress(DownloadItem downloadItem, int downloadStatus) {
        ...
        if (progress == null) {
            if (!downloadItem.getDownloadInfo().isPaused()) {
                ...
                if (!FIRST_SEEN_DOWNLOAD_IDS.contains(id)) {
                    FIRST_SEEN_DOWNLOAD_IDS.add(id);
                }
        ...
        switch (downloadStatus) {
            ...
            case DOWNLOAD_STATUS_CANCELLED:
                ...
                updateNotification(progress);
                FIRST_SEEN_DOWNLOAD_IDS.remove(id);
                break;
            case DOWNLOAD_STATUS_INTERRUPTED:
    ...
    public void resumeDownload(ContentId id, DownloadItem item, boolean hasUserGesture) {
        ...
        if (progress == null) {
            ...
            // download is active.
            if (!FIRST_SEEN_DOWNLOAD_IDS.contains(item.getId())) {
                FIRST_SEEN_DOWNLOAD_IDS.add(item.getId());
    ...
    private void removeDownloadProgress(String guid) {
        ...
        FIRST_SEEN_DOWNLOAD_IDS.remove(guid);
    }
