...
public class SimpleAnimationLayout extends Layout {
    ...
    private void tabCreatedInBackground(
        ...
        animationList.add(CompositorAnimator.ofFloatProperty(
                handler, sourceLayoutTab, LayoutTab.SCALE, scale, 1f, BACKGROUND_STEP3_DURATION));
        animationList.add(CompositorAnimator.ofFloatProperty(
        ...
        step3.setInterpolator(BakedBezierInterpolator.TRANSFORM_CURVE);
        step3.playTogether(animationList);

        // TODO(mdjones): Instead of using a pause animator, 'step3' should use a delay.
        //                Unfortunately AnimatorSet uses a ValueAnimator for pauses and Android
        //                animators and compositor animators don't seem to play together nicely.
        //                Switching 'pause' to a ValueAnimator yields the same result.
        Animator pause = new CompositorAnimator(handler);
        pause.setDuration(BACKGROUND_STEP2_DURATION);

        mTabCreatedBackgroundAnimation = new AnimatorSet();
        mTabCreatedBackgroundAnimation.playSequentially(step1, pause, step3);
