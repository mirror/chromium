...
public class SavePasswordsPreferences
    ...
    // This matrix is used to transform colored icons to plainly white icons. It ignores inherent
    // luminosity of colors to brighten up the icon.
    private static final float[] MATRIX_TRANSFORM_TO_WHITE = {
    ...
    private boolean filterPasswords(String query) {
        ...
        return false; // Query been handled. Don't trigger default action of SearchView.
    }
    ...
    void rebuildPasswordLists() {
        ...
        if (mSearchQuery == null) {
            // Create additional preferences only if the filter view is not active.
    ...
    private static Drawable convertToPlainWhite(Drawable icon) {
        icon.setColorFilter(new ColorMatrixColorFilter(new ColorMatrix(MATRIX_TRANSFORM_TO_WHITE)));
        return icon;
    }
