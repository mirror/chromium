...
3. Add a new test in `ChannelsInitializerTest.java` for your new channel's properties.
...
5. After posting a notification, remember to call
...
- Add the new channel to `PredefinedChannels.MAP` in `ChannelDefinitions.java`
- Increment `CHANNELS_VERSION` in `ChannelDefinitions.java`
- Update startup channel tests in `ChannelsInitializerTest.java`.
