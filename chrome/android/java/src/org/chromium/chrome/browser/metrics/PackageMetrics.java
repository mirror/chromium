...
public class PackageMetrics {
            ...
            if (Environment.MEDIA_MOUNTED.equals(storageVolume.getState())) {
                ...
                if (uuid != null) {
                    try {
                        ...
                        pmd.dataSize +=
                                (storageStats.getDataBytes() - storageStats.getCacheBytes());
    ...
    public static void recordPackageStats() {
        PackageMetricsData data = null;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            data = getPackageStatsForAndroidO();
        }
        if (data != null) {
            RecordHistogram.recordCustomCountHistogram("Android.PackageStats.DataSize",
                    Math.round(data.dataSize / 1024 / 1024), 1, 10000, 100);
            RecordHistogram.recordCustomCountHistogram("Android.PackageStats.CacheSize",
                    Math.round(data.cacheSize / 1024 / 1024), 1, 10000, 100);
            RecordHistogram.recordCustomCountHistogram("Android.PackageStats.CodeSize",
                    Math.round(data.codeSize / 1024 / 1024), 1, 500, 100);
        }
    }
}
