...
public class CompositorAnimator extends Animator {
     ...
     * every update.
     */
    private final ArrayList<AnimatorUpdateListener> mClonedList = new ArrayList<>();
    ...
    public void doAnimationFrame(long deltaTimeMs) {
        ...
        for (int i = 0; i < mClonedList.size(); i++) mClonedList.get(i).onAnimationUpdate(this);
        mClonedList.clear();
    ...
    public void start() {
        ...
        if (mHandler.get() != null) mHandler.get().startAnimator(this);
        mTimeSinceStartMs = 0;
