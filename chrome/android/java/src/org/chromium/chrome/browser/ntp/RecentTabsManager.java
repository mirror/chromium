...
public class RecentTabsManager implements AndroidSyncSettingsObserver, SignInStateObserver,
    ...
    /**
     *
     * @return Whether the new signin promo should be displayed.
     */
    public boolean shouldDisplayNewPromo() {
        return SigninPromoController.shouldShowPromo(SigninAccessPoint.RECENT_TABS)
                && SigninManager.get(mContext).isSignInAllowed();
    }

    /**
     * Determine whether the old promo needs to be displayed.
     *
     * @return Whether sync promo should be displayed.
     */
    public boolean shouldDisplayOldPromo() {
        if (ChromeFeatureList.isEnabled(ChromeFeatureList.ANDROID_SIGNIN_PROMOS)) {
            return ChromeSigninController.get().isSignedIn()
                    && (!AndroidSyncSettings.isSyncEnabled(mContext) || mForeignSessions.isEmpty());
        } else {
            return SigninManager.get(mContext).isSignInAllowed()
                    || !AndroidSyncSettings.isSyncEnabled(mContext) || mForeignSessions.isEmpty();
        }
    }
    ...
    public void setupNewSigninPromo(SigninPromoView view) {
