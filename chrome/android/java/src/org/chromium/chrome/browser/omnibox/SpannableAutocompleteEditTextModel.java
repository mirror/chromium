...
public class SpannableAutocompleteEditTextModel implements AutocompleteEditTextModelBase {
    ...
    private void setAutocompleteSpan() {
        ...
        String autocompleteText = mAutocompleteText;
        ...
        if (!userText.equals(mUserTextOnSetAutocomplete)) {
            ...
            if (growth > 0
                ...
                autocompleteText = mAutocompleteText.substring(growth);
            } else {
                if (DEBUG) Log.i(TAG, "setAutocompleteSpan - match broken");
                return;
            }
        }
        if (DEBUG) Log.i(TAG, "setAutocompleteSpan. %s[%s]", userText, autocompleteText);
        ...
        spanString.setSpan(mSpan, 0, autocompleteText.length(), Spanned.SPAN_INTERMEDIATE);
        ...
        if (idx == -1) {
            ...
            if (selStart > idx || selEnd > idx) {
                ...
                notifyAutocompleteTextStateChangedNoCheck(
                        mDelegate.getText().toString(), true, false);
            }
        }
    ...
    private class AutocompleteInputConnection extends InputConnectionWrapper {
        ...
        public boolean endBatchEdit() {
            ...
            // TODO: also check current selection.
            if (!TextUtils.isEmpty(mAutocompleteText) && mPreBatchEditText.startsWith(currText)
                ...
                super.endBatchEdit();
                updateSelectionForTesting();
                mBatchEditNestCount = 1;
                super.beginBatchEdit(); // just to avoid notifyAutocompleteTextStateChanged.
                Editable editable = mDelegate.getEditableText();
                if (DEBUG) {
                    Log.i(TAG, "Deletion detected. restoring diff at the end: " + currText + diff);
                }
                ...
                updateSelectionForTesting();
                notifyAutocompleteTextStateChangedNoCheck(
                        currText + diff, true /*textDeleted*/, false);
