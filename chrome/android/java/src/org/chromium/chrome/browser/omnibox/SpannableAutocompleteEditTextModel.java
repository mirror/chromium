...
public class SpannableAutocompleteEditTextModel implements AutocompleteEditTextModelBase {
    ...
    // A pattern that matches strings consisting only of English and European character sets,
    // numbers, punctuations, and a white space.
    ...
    private void setAutocompleteSpan() {
        ...
        if (DEBUG) {
            Log.i(TAG, "setAutocompleteSpan after: %s->%s", mSetAutocompleteState, mCurrentState);
        }
    }
    ...
    private void notifyAutocompleteTextStateChanged() {
        ...
        if (mCurrentState.equalsExceptAutocompleteText(mPreviouslyNotifiedState)
            ...
            // Autocomplete text is set by controller, we should not notify controller the same
            // information.
            mPreviouslyNotifiedState.copyFrom(mCurrentState);
            return;
        }
        // The current model's mechanism always moves cursor at the end of user text, so we don't
    ...
    private class AutocompleteInputConnection extends BatchEditTrackedInputConnection {
        ...
        public boolean endBatchEdit() {
            ...
            if (mCurrentState.isForwardTypedFrom(mPreBatchEditState)) {
                // Really type something that is not a deletion.
