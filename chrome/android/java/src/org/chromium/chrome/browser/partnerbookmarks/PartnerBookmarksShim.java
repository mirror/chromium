...
public class PartnerBookmarksShim {
    ...
    public static void kickOffReading(Context context) {
        ...
        if (skip) {
            Log.w(TAG, "Skip reading partner bookmarks since recent result was empty.");
        }
    ...
    private static boolean shouldSkipReading() {
        ...
        long last_read = pref.getLong(PartnerBookmarksReader.LAST_READ_PREFS_NAME, 0);
        long elapsed = System.currentTimeMillis() - last_read;
        return elapsed < BAN_DURATION_MS;
    }
}
