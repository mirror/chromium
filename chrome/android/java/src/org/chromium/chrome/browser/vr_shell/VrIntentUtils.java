...
public class VrIntentUtils {
    ...
    public static boolean isVrIntent(Intent intent) {
        ...
        return IntentUtils.safeGetBooleanExtra(intent, DAYDREAM_VR_EXTRA, false)
                && VrShellDelegate.isVrEnabled() && !launchedFromRecentApps(intent);
    }
    ...
    public static boolean isCustomTabVrIntent(Intent intent) {
        if (getHandlerInstance().isTrustedDaydreamIntent(intent)) return true;
        return isVrIntent(intent)
                && ChromeFeatureList.isEnabled(ChromeFeatureList.VR_CUSTOM_TAB_BROWSING);
    }
