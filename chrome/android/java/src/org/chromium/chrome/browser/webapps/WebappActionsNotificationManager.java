...
/**
 * Manages the notification shown for installed Web Apps with display mode 'Standalone' and
 * 'Fullscreen'. It accomplishes number of goals:
 * <ul>
 * <li> Presents the current URL.
 * <li> Exposes 'Share' and 'Open in Chrome' actions.
 * <li> Messages that Web App runs in Chrome.
 * </ul>
 */
class WebappActionsNotificationManager {
    ...
    private Notification createNotification() {
        ...
        return NotificationBuilderFactory
                .createChromeNotificationBuilder(
                        false /* prefer compat */, ChannelDefinitions.CHANNEL_ID_BROWSER)
    ...
    public void cancelNotification() {
        if (!isEnabled()) return;
        NotificationManager mm = (NotificationManager) mWebappActivity.getSystemService(
                Context.NOTIFICATION_SERVICE);
        mm.cancel(NotificationConstants.NOTIFICATION_ID_WEBAPP_ACTIONS);
    }
    ...
    public boolean handleNotificationAction(Intent intent) {
        if (ACTION_SHARE.equals(intent.getAction())) {
            ...
            RecordUserAction.record("WebappNotificationShare");
            return true;
            ...
            // Do nothing, just close notification drawer and focus on PWA.
            RecordUserAction.record("WebappNotificationFocus");
            return true;
        }
        return false;
    }
