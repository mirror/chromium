...
public class AnchoredPopupWindow implements OnTouchListener, RectProvider.Observer {
    ...
    private void updatePopupLayout() {
        ...
        int widthPaddingOffset = paddingX + mMarginPx * 2;
        int maxContentWidth =
                (mMaxWidthPx != 0 ? mMaxWidthPx : mRootView.getWidth()) - widthPaddingOffset;
        ...
        int idealWidth = contentView.getMeasuredWidth();
        ...
        if (mPreferredHorizontalOrientation == HORIZONTAL_ORIENTATION_MAX_AVAILABLE_SPACE) {
            ...
            mPositionToLeft = shouldPositionLeftOfAnchor(spaceLeftOfAnchor, spaceRightOfAnchor,
                    idealWidth, currentPositionToLeft, preferCurrentOrientation);

            int widthBasedOnPosition =
                    (mPositionToLeft ? spaceLeftOfAnchor : spaceRightOfAnchor) - widthPaddingOffset;
            if (widthBasedOnPosition < maxContentWidth) {
                maxContentWidth = widthBasedOnPosition;
    ...
    static boolean shouldPositionLeftOfAnchor(int spaceToLeftOfAnchor, int spaceToRightOfAnchor,
            int idealWidth, boolean currentPositionToLeft, boolean preferCurrentOrientation) {
        ...
        if (preferCurrentOrientation && positionToLeft != currentPositionToLeft) {
            if (currentPositionToLeft && idealWidth <= spaceToLeftOfAnchor) {
                positionToLeft = true;
            }
            if (!currentPositionToLeft && idealWidth <= spaceToRightOfAnchor) {
