...
public class AnchoredPopupWindow implements OnTouchListener, RectProvider.Observer {
    ...
    /**
     * Sets the preferred vertical orientation of the popup with respect to the anchor view such as
    ...
    /**
     * Sets the preferred horizontal orientation of the popup with respect to the anchor view such
     * as centered with respect to the anchor.
    ...
    /**
     * Sets the max width for the popup's content.
     * @param maxWidth The max width for the popup's content.
     */
    ...
    /**
     * Sets whether the popup should overlap the anchor {@link Rect}. Defaults to false.
    ...
    private void updatePopupLayout() {
        ...
        mX = getPopupX(anchorRect, mCachedWindowRect, mWidth, mMarginPx, mOverlapAnchor,
                mCachedPaddingRect, mPreferredHorizontalOrientation);
        mY = getPopupY(anchorRect, mHeight, mOverlapAnchor, mCachedPaddingRect, mPositionBelow);
    ...
    static int getPopupX(Rect anchorRect, Rect windowRect, int popupWidth, int marginPx,
            boolean overlapAnchor, Rect backgroundPaddingRect,
            @HorizontalOrientation int horizontalOrientation) {
        int x;
        int spaceToLeftOfAnchor = anchorRect.left - windowRect.left;
        int spaceToRightOfAnchor = windowRect.right - anchorRect.right;
        boolean idealFitsToLeft = popupWidth <= spaceToLeftOfAnchor;
        boolean idealFitsToRight = popupWidth <= spaceToRightOfAnchor;
        ...
        if (horizontalOrientation == HORIZONTAL_ORIENTATION_CENTER) {
            ...
            x = (overlapAnchor ? anchorRect.right + backgroundPaddingRect.right : anchorRect.left)
                    - popupWidth;
        } else {
            x = overlapAnchor ? anchorRect.left - backgroundPaddingRect.left : anchorRect.right;
        }
    ...
    @VisibleForTesting
    static int getPopupY(Rect anchorRect, int popupHeight, boolean overlapAnchor,
            Rect backgroundPaddingRect, boolean positionBelow) {
        if (positionBelow) {
            return overlapAnchor ? anchorRect.top : anchorRect.bottom;
        } else {
            return overlapAnchor ? anchorRect.bottom + backgroundPaddingRect.bottom - popupHeight
