...
class BookmarkItemsAdapter
    ...
    @IntDef({ViewType.NEW_PROMO_VIEW, ViewType.OLD_PROMO_VIEW, ViewType.FOLDER_VIEW,
            ViewType.BOOKMARK_VIEW})
    @interface ViewType {
        int NEW_PROMO_VIEW = 0;
        int OLD_PROMO_VIEW = 1;
        int FOLDER_VIEW = 2;
        int BOOKMARK_VIEW = 3;
    }
    ...
    public @ViewType int getItemViewType(int position) {
        ...
        if (section == mPromoHeaderSection) {
            assert section.size() == 1
                    && section.get(0) != null : "Promo type should have been specified!";
            return mPromoHeaderSection.get(0);
        } else if (section == mFolderSection) {
            return ViewType.FOLDER_VIEW;
        } else if (section == mBookmarkSection) {
            return ViewType.BOOKMARK_VIEW;
        }
    ...
    private void updateHeader() {
        ...
        switch (mPromoHeaderManager.getPromoState()) {
            ...
            case BookmarkPromoHeader.PromoState.PROMO_SIGNIN_NEW:
                mPromoHeaderSection.add(ViewType.NEW_PROMO_VIEW);
                return;
            case BookmarkPromoHeader.PromoState.PROMO_SIGNIN_OLD:
                mPromoHeaderSection.add(ViewType.OLD_PROMO_VIEW);
                return;
