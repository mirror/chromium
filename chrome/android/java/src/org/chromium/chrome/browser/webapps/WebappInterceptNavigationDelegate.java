...
public class WebappInterceptNavigationDelegate extends InterceptNavigationDelegateImpl {
    ...
    public static class CustomTabTimeSpentLogger {
         ...
         * otherwise null.
         */
        public static CustomTabTimeSpentLogger createInstanceAndStartTimerIfNecessary(
        ...
        public void stopAndLogTime() {
            ...
            if (mActivityType == WebappActivity.ACTIVITY_TYPE_WEBAPP) {
                RecordHistogram.recordTimesHistogram(
                        "Webapp.CustomTab.Duration", timeSpent, TimeUnit.MILLISECONDS);
            }
            if (mActivityType == WebappActivity.ACTIVITY_TYPE_WEBAPK) {
                RecordHistogram.recordTimesHistogram(
                        "WebApk.CustomTab.Duration", timeSpent, TimeUnit.MILLISECONDS);
            }
            if (mActivityType == WebappActivity.ACTIVITY_TYPE_TRUSTED_WEB_ACTIVITY) {
                RecordHistogram.recordTimesHistogram(
                        "TrustedWebActivity.CustomTab.Duration", timeSpent, TimeUnit.MILLISECONDS);
            }
        }
    }
    ...
    public boolean shouldIgnoreNavigation(NavigationParams navigationParams) {
        ...
        if (shouldOpenInCustomTab(
            ...
            mActivity.getWebappInfo().setWebappIntentExtras(customTabIntent.intent);
