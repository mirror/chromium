...
//        mRecyclerView.setBackgroundColor(backgroundColor);
        mRecyclerView.setBackgroundColor(0xFFCCFFCC);
...
//                mScrollY += dy;
                Log.w("mvano", "onScrolled dy: %s", dy);
                Log.w("mvano", "onScrolled isAnimating(): %s", isAnimating());
                if (!isAnimating()) updateLogoTransition();
            }
        });
    ...
    public void onUrlFocusChange(final boolean hasFocus) {
        Log.w("mvano", "onUrlFocusChange hasFocus: %s", hasFocus);
        Log.w("mvano", "onUrlFocusChange mTransitionFraction: %s", mTransitionFraction);
        if (hasFocus && !isAnimating()) mTransitionFractionBeforeAnimation = mTransitionFraction;
        if (isAnimating()) mAnimator.cancel();

//        // If it just gained focus but the logo transition has already completed due to scrolling,
//        // do not start an animation.
//        if (hasFocus && mTransitionFraction == 1.0f) {
//            Log.w("mvano", "onUrlFocusChange not animating because already fully transitioned");
//            return;
//        }

//        if (!hasFocus && mTransitionFractionBeforeAnimation == 1.0f) {
//            Log.w("mvano", "onUrlFocusChange not animating because already fully transitioned");
//            return;
//        }

        float scrollBasedFraction = getScrollBasedTransitionFraction();

        final float startFraction = mTransitionFraction;
//        if (isAnimating()) {
...
//            endFraction = scrollBasedFraction;
            endFraction = mTransitionFractionBeforeAnimation;
        }

        Log.w("mvano", "onUrlFocusChange startFraction: %s", startFraction);
        Log.w("mvano", "onUrlFocusChange endFraction: %s", endFraction);

        if (startFraction == endFraction) {
            Log.w("mvano", "onUrlFocusChange start and end are the same, no need to animate!");
            return;
        }
...
//        Log.w("mvano", "updateLogoTransition showLogo: %s", showLogo);
        ...
        Log.w("mvano", "updateLogoTransition mMaxToolbarOffset: %s", mMaxToolbarOffset);
//        Log.w("mvano", "updateLogoTransition mScrollY: %s", mScrollY);
        Log.w("mvano", "updateLogoTransition mRecyclerView.computeVerticalScrollOffset(): %s", mRecyclerView.computeVerticalScrollOffset());
        Log.w("mvano", "updateLogoTransition mRecyclerView.computeVerticalScrollExtent(): %s", mRecyclerView.computeVerticalScrollExtent());
        Log.w("mvano", "updateLogoTransition mRecyclerView.computeVerticalScrollRange(): %s", mRecyclerView.computeVerticalScrollRange());
        Log.w("mvano", "updateLogoTransition mRecyclerView.isFirstItemVisible(): %s", mRecyclerView.isFirstItemVisible());
...
//        }
        ...
        Log.w("mvano", "updateLogoTransition toolbarOffset: %s", toolbarOffset);
        ...
        if (isAnimating() && mRecyclerView.isFirstItemVisible()) {
//            float transitionDelta = mTransitionFraction - oldTransitionFraction;
//            Log.w("mvano", "updateLogoTransition transitionDelta: %s", transitionDelta);
//            int scrollOffset = (int) (mMaxToolbarOffset * transitionDelta);
            int scrollOffset = (int) (mMaxToolbarOffset - toolbarOffset);
            Log.w("mvano", "updateLogoTransition scrollOffset: %s", scrollOffset);
//            mRecyclerView.scrollBy(0, scrollOffset);
            int currentScroll = mRecyclerView.computeVerticalScrollOffset() + mRecyclerView.computeVerticalScrollExtent();
            int maxScroll = mRecyclerView.computeVerticalScrollRange();
            int availableScroll = maxScroll - currentScroll;
            Log.w("mvano", "updateLogoTransition availableScroll: %s", availableScroll);
            Log.w("mvano", "updateLogoTransition maxScroll: %s", maxScroll);
            if (scrollOffset > availableScroll) {
                int translationOffset = scrollOffset - availableScroll;
                Log.w("mvano", "updateLogoTransition translationOffset: %s", translationOffset);
                mRecyclerView.setTranslationY(-translationOffset);
                scrollOffset = availableScroll;
            }
            mRecyclerView.getLinearLayoutManager().scrollToPositionWithOffset(0, -scrollOffset);
        }
    ...
    private void updatePadding() {
        final int top;
        if (shouldShowLogo()) {
            top = mToolbarHeight + (int) mMaxToolbarOffset;
        } else {
            top = mToolbarHeight;
        }
...
//        final int bottom;
//        if (isAnimating()) {
//            bottom = mRecyclerView.getMeasuredHeight() - top;
//        } else {
//            bottom = 0;
//        }

        Log.w("mvano", "updatePadding top: %s", top);
//        Log.w("mvano", "updatePadding bottom: %s", bottom);
    ...
    private float getScrollBasedTransitionFraction() {
        if (mRecyclerView.isFirstItemVisible()) {
            return Math.min(1.0f, mRecyclerView.computeVerticalScrollOffset() / mMaxToolbarOffset);
        }
        // If the first RecyclerView item is not visible we have scrolled quite far and the
