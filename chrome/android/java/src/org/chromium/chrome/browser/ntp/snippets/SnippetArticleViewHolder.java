...
public class SnippetArticleViewHolder extends CardViewHolder implements ImpressionTracker.Listener {
    ...
    private void setDownloadThumbnail() {
        ...
        if (fileType == DownloadFilter.FILTER_IMAGE) {
            // A promise to be fulfilled when the thumbnail is retrieved.
            final Promise<Bitmap> thumbnailReceivedPromise = new Promise<>();

            // For image downloads, attempt to fetch a thumbnail.
            mThumbnailRequest = mImageFetcher.makeDownloadThumbnailRequest(
                    mArticle, mThumbnailSize, thumbnailReceivedPromise);
            ...
            if (mThumbnailRequest.getPromise().isFulfilled()) {
                ...
                mThumbnailView.setImageBitmap(mThumbnailRequest.getPromise().getResult());
                return;
            } else {
                // Otherwise, we will set a placeholder, wait for the thumbnail to be retrieved and
                // then resize it, cache it and fade it in using the FetchImageCallback.
