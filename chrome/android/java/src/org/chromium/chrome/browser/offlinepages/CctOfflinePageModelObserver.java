...
public class CctOfflinePageModelObserver extends OfflinePageBridge.OfflinePageModelObserver {
    ...
    public void offlinePageAdded(OfflinePageItem addedPage) {
        OfflinePageOrigin origin = new OfflinePageOrigin(addedPage.getRequestOrigin());
        if (!origin.isChrome()) compareSignaturesAndFireIntent(origin);
    }

    @Override
    public void offlinePageDeleted(DeletedPageInfo deletedPageInfo) {
        OfflinePageOrigin origin = new OfflinePageOrigin(deletedPageInfo.getRequestOrigin());
        if (!origin.isChrome()) compareSignaturesAndFireIntent(origin);
    }

    private void compareSignaturesAndFireIntent(OfflinePageOrigin origin) {
