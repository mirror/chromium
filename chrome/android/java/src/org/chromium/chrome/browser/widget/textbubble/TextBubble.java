...
import org.chromium.chrome.browser.widget.popups.AnchoredPopupWindow;
import org.chromium.chrome.browser.widget.popups.ViewAnchoredPopupWindow;
...
public class TextBubble {
    ...
    public TextBubble(Context context, View rootView, @StringRes int stringId,
        ...
        this(context, rootView, stringId, accessibilityStringId, showArrow, null);
    }
     ...
     */
    public TextBubble(Context context, View rootView, @StringRes int stringId,
            @StringRes int accessibilityStringId, boolean showArrow, View anchorView) {
        mContext = context;
        ...
        mPopupWindow = createPopupWindow(context, rootView, anchorView);
        mPopupWindow.setPreferredHorizontalOrientation(
                AnchoredPopupWindow.HorizontalOrientation.CENTER);

        mHandler = new Handler();

        // TODO(twellington): Handle animation styles
    ...
    protected AnchoredPopupWindow createPopupWindow(
            Context context, View rootView, View anchorView) {
        if (anchorView != null) {
            return new ViewAnchoredPopupWindow(context, anchorView, mDrawable, mContentView,
                    context.getResources().getDimensionPixelSize(R.dimen.text_bubble_margin),
                    ViewGroup.LayoutParams.WRAP_CONTENT);
        }
        return new AnchoredPopupWindow(context, rootView, mDrawable, mContentView,
                context.getResources().getDimensionPixelSize(R.dimen.text_bubble_margin),
                ViewGroup.LayoutParams.WRAP_CONTENT);
    }
    ...
    public void setInsetPx(int left, int top, int right, int bottom) {
        if (!(mPopupWindow instanceof ViewAnchoredPopupWindow)) {
            assert false;
            return;
        }
        ((ViewAnchoredPopupWindow) mPopupWindow).setInsetPx(left, top, right, bottom);
    }
    ...
    public void setPreferredVerticalOrientation(
            AnchoredPopupWindow.VerticalOrientation orientation) {
        mPopupWindow.setPreferredVerticalOrientation(orientation);
    }

    // TODO(twellington): hook up via a delegate?c
    private void onPopupPositionUpdated(
            boolean positionBelow, int x, int y, int width, int height) {
        int arrowXOffset = 0;
