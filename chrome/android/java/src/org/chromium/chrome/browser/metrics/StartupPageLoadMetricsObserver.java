...
public class StartupPageLoadMetricsObserver implements PageLoadMetrics.Observer {
    ...
    private boolean mShouldRecordHistograms = false;

    public StartupPageLoadMetricsObserver(WebContents webContents) {
        mWebContents = webContents;
    }

    @Override
    public void onNewNavigation(WebContents webContents, long navigationId) {
        if (mWebContents != webContents || mNavigationId != NO_NAVIGATION_ID) return;
    ...
    public void onFirstContentfulPaint(WebContents webContents, long navigationId,
        ...
        if (webContents != mWebContents || navigationId != mNavigationId
                || !mShouldRecordHistograms) {
            return;
        }
