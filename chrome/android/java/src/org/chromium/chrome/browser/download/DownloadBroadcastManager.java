...
public final class DownloadBroadcastManager {
    ...
    private boolean mIsDelayedStopScheduled;
    ...
    private void scheduleDelayedStopIfNeeded() {
        if (mIsDelayedStopScheduled) return;

        mIsDelayedStopScheduled = true;
        Runnable scheduleDelayedStopTask = new Runnable() {
            @Override
            public void run() {
                mIsDelayedStopScheduled = false;
        ...
        mHandler.postDelayed(scheduleDelayedStopTask, mCumulativeWaitTimeMs);
