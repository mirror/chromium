...
public class LaunchIntentDispatcher implements IntentHandler.IntentHandlerDelegate {
    ...
    public @Action int dispatch() {
        ...
        if (Intent.ACTION_MAIN.equals(mIntent.getAction())) {
            // Read partner browser customizations information asynchronously.
            // We want to initialize early because when there is no tabs to restore, we should
            // possibly show homepage, which might require reading PartnerBrowserCustomizations
            // provider.
        ...
        // All cases below are only applicable for VIEW intents.
    ...
    private @Action int launchTabbedMode() {
        maybePrefetchDnsInBackground();
        ...
        if (shouldRunInLegacyMultiInstanceMode()) {
     ...
     */
    private boolean isChromeBrowserActivityRunning() {
