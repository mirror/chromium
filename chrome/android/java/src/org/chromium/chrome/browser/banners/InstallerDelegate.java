...
public class InstallerDelegate {
    ...
    public @InstallState int determineInstallState(String packageName) {
        android.util.Log.e("ABCD", "Running " + mIsRunning + " " + packageName);
        ...
        if (isInstalled(ContextUtils.getApplicationContext(), packageName)) {
    ...
    public boolean installOrOpenNativeApp(Tab tab, AppData appData, String referrer) {
        if (openApp(ContextUtils.getApplicationContext(), appData.packageName())) {
            return true;
        } else {
    ...
    public void startMonitoring(String packageName) {
        android.util.Log.e("ABCD", "startMonitoring");
