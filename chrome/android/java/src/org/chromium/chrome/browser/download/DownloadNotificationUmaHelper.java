...
public final class DownloadNotificationUmaHelper {
    // NOTE: Keep these lists/classes in sync with Download.Notification.[...] in enums.xml.
    ...
    static void recordServiceStoppedHistogram(int stopType, boolean withForeground) {
        if (withForeground) {
            RecordHistogram.recordEnumeratedHistogram(
                    "Android.DownloadManager.ServiceStopped.Foreground", stopType,
                    ServiceStopped.MAX);
        } else {
            RecordHistogram.recordEnumeratedHistogram(
    ...
    static void recordExistingNotificationsCountHistogram(int count, boolean withForeground) {
        if (withForeground) {
            RecordHistogram.recordCountHistogram(
                    "Android.DownloadManager.ExistingNotificationsCount.ForegroundEnabled", count);
        } else {
            RecordHistogram.recordCountHistogram(
                    "Android.DownloadManager.ExistingNotificationsCount.ForegroundDisabled", count);
        }
    }
    ...
    static void recordNotificationFlickerCountHistogram(int launchType) {
        RecordHistogram.recordEnumeratedHistogram(
                "Android.DownloadManager.NotificationLaunchCount", launchType, LaunchType.MAX);
    }
}
