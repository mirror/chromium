...
public class WebappActivity extends SingleTabActivity {
    ...
    protected void onDeferredStartupWithNullStorage() {
        if (isTrusted()) {
            // Register the WebAPK or TWA which registered when created, but may also become
            ...
            WebappRegistry.getInstance().register(
                    mWebappInfo.id(), new WebappRegistry.FetchWebappDataStorageCallback() {
                        ...
                        public void onWebappDataStorageRetrieved(WebappDataStorage storage) {
                            ...
                            // TODO(yusufo): For now, this is effectively a no-op for Trusted
                            // Web Activities and we should clearly define what can be
                            // registered through the support library here.
    ...
    protected WebappScopePolicy scopePolicy() {
        return isTrusted() ? WebappScopePolicy.STRICT : WebappScopePolicy.LEGACY;
    }
    ...
    /**
     * @return The package name if this Activity is associated with an APK.
     */
    public String getApkPackageName() {
        if (getBrowserSession() == null) return getWebappInfo().apkPackageName();
