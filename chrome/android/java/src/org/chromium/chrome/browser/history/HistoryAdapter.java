...
public class HistoryAdapter extends DateDividedAdapter implements BrowsingHistoryObserver {
    ...
    public void onQueryHistoryComplete(List<HistoryItem> items, boolean hasMorePotentialMatches) {
        ...
        if (!mIsInitialized) {
            if (items.size() > 0 && !mIsSearching) setHeaders(buildHeaderItemArguments());
            mIsInitialized = true;
        }
    ...
    /**
     * @return HeaderItem array to pass in setHeaders() as parameter.
     */
    private HeaderItem[] buildHeaderItemArguments() {
        ...
        return args.toArray(new HeaderItem[0]);
    }
    ...
    void setPrivacyDisclaimerVisibility() {
        ...
        if (mIsInitialized) setHeaders(buildHeaderItemArguments());
    }
