...
public abstract class AsyncInitializationActivity extends AppCompatActivity implements
    ...
    private final void onCreateInternal(Bundle savedInstanceState) {
        ...
        if (maybeAbortLaunch()) {
            boolean finishSucceeded = isFinishing();
            new CachedMetrics.BooleanHistogramSample("Android.Activity.AbortLaunchSucceeded")
                    .record(finishSucceeded);
            if (finishSucceeded) {
                return;
            }
