...
class CustomTabObserver extends EmptyTabObserver {
    ...
    public void onHidden(Tab tab) {
        mNavigationInfoCaptureTrigger.onHidden(tab);
    }
    ...
    public void onFirstContentfulPaint(Tab tab) {
        mNavigationInfoCaptureTrigger.onFirstContentfulPaint(tab);
    }
    ...
    private boolean captureNavigationInfo(final Tab tab) {
        if (mCustomTabsConnection == null) return false;
        // if (!mCustomTabsConnection.shouldSendNavigationInfoForSession(mSession)) return false;

        if (!tab.isHidden() && mCurrentState != STATE_RESET) return false;
        if (tab.getWebContents() == null) return false;

        tab.getWebContents().getContentBitmapAsync(mContentBitmapWidth, mContentBitmapHeight,
                (Bitmap bitmap, int response) -> {
