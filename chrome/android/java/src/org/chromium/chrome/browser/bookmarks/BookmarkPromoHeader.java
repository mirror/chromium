...
class BookmarkPromoHeader implements AndroidSyncSettingsObserver, SignInStateObserver,
     ...
     */
    @IntDef({PROMO_NONE, PROMO_SIGNIN_NEW, PROMO_SIGNIN_OLD, PROMO_SYNC})
    @interface PromoState {}
    static final int PROMO_NONE = 0;
    static final int PROMO_SIGNIN_NEW = 1;
    static final int PROMO_SIGNIN_OLD = 2;
    static final int PROMO_SYNC = 3;
    ...
    private Runnable mPromoHeaderChangeAction;
    ...
    BookmarkPromoHeader(
        ...
        updatePromoState();
        if (shouldShow() && (mPromoState == PROMO_SIGNIN_OLD || mPromoState == PROMO_SYNC)) {
            int promoShowCount =
            ...
            ContextUtils.getAppSharedPreferences()
                    ...
                    .apply();
            if (mPromoState == PROMO_SIGNIN_OLD) {
    ...
    private boolean shouldShow() {
        return mPromoState != PROMO_NONE || sShouldShowForTests;
    }
    ...
    private void updatePromoState() {
        ...
        if (ChromeSigninController.get().isSignedIn()) {
            ...
            mPromoState =
                    !AndroidSyncSettings.isChromeSyncEnabled(mContext) && impressionLimitNotReached
                    ? PROMO_SYNC
                    : PROMO_NONE;
            return;
        }
        ...
        if (ChromeFeatureList.isEnabled(ChromeFeatureList.ANDROID_SIGNIN_PROMOS)) {
            mPromoState = mSignInManager.isSignInAllowed()
                            && SigninPromoController.shouldShowPromo(
            ...
            boolean impressionLimitNotReached =
                    ContextUtils.getAppSharedPreferences().getInt(PREF_SIGNIN_PROMO_SHOW_COUNT, 0)
                    < MAX_SIGNIN_PROMO_SHOW_COUNT;
            mPromoState = mSignInManager.isSignInAllowed() && impressionLimitNotReached
