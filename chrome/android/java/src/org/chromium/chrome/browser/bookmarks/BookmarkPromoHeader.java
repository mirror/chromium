...
class BookmarkPromoHeader implements AndroidSyncSettingsObserver, SignInStateObserver,
    ...
    private static @Nullable @PromoState Integer sShouldShowForTests;
    ...
    BookmarkPromoHeader(
        ...
        if (getPromoState() == PromoState.PROMO_SIGNIN_OLD
                || getPromoState() == PromoState.PROMO_SYNC) {
            int promoShowCount =
    ...
    @PromoState
    int getPromoState() {
        if (sShouldShowForTests != null) {
            mPromoState = sShouldShowForTests;
        }
        return mPromoState;
    }
     ...
     */
    @VisibleForTesting
    public static void setShowForTests(@PromoState int promoState) {
        sShouldShowForTests = promoState;
    }
}
