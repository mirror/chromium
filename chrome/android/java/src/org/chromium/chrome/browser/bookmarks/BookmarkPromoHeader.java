...
class BookmarkPromoHeader implements AndroidSyncSettingsObserver, SignInStateObserver,
    ...
    /**
     * Enum used to specify the states in which the promo header can be in.
     */
    private enum PromoState { PROMO_NONE, PROMO_SIGNIN_NEW, PROMO_SIGNIN_OLD, PROMO_SYNC }
    ...
    private PromoState mPromoState;
    ...
    BookmarkPromoHeader(Context context, PromoHeaderChangeListener showingChangeListener,
        ...
        updatePromoState(false);
        if (shouldShow()
                && (mPromoState == PromoState.PROMO_SIGNIN_OLD
     ...
     */
    boolean shouldShow() {
        return mPromoState != PromoState.PROMO_NONE || sShouldShowForTests;
    }
    ...
    void bindHolder(ViewHolder holder) {
        if (mPromoState == PromoState.PROMO_NONE) {
            return;
        }
        ...
        boolean oldPromo = mPromoState != PromoState.PROMO_SIGNIN_NEW;
        boolean shouldSwitchViews = (oldPromo && currentView instanceof SigninPromoView)
                || (!oldPromo && currentView instanceof SigninAndSyncView);
    ...
    private void updatePromoState(boolean notifyUI) {
        boolean shouldShow = calculateShouldShow();
        if (!shouldShow) {
            mPromoState = PromoState.PROMO_NONE;
        }
        if (notifyUI) {
    ...
    private boolean calculateShouldShow() {
        ...
        if (ChromeFeatureList.isEnabled(ChromeFeatureList.ANDROID_SIGNIN_PROMOS)) {
            mPromoState = PromoState.PROMO_SIGNIN_NEW;
