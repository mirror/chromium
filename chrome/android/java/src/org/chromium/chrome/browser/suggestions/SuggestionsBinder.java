    ...
    private class FetchThumbnailCallback extends Callback<Bitmap> {
        ...
        public void onResult(Bitmap thumbnail) {
            ...
            if (thumbnail.getHeight() != mThumbnailSize || thumbnail.getWidth() != mThumbnailSize) {
                // Resize the thumbnail. If the suggestion is an article, this method is called
                // directly from the native code. The thumbnail is not cached anywhere it is safe to
                // recycle the input bitmap after we get its resized version.
