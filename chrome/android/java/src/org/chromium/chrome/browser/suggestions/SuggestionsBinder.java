    ...
    private class FetchThumbnailCallback extends Callback<Bitmap> {
        ...
        public void onResult(Bitmap thumbnail) {
            ...
            if (thumbnail.getHeight() != mThumbnailSize || thumbnail.getWidth() != mThumbnailSize) {
                // Resize the thumbnail. If the suggestion is an article, the thumbnail is fetched
                // from native code and it is not cached anywhere. Therefore, it is safe to recycle
                // the input bitmap after we get its resized version.
