...
public class CastMessageHandler {
    ...
    boolean handleCastV2Message(JSONObject jsonMessage) throws JSONException {
        ...
        if (clientId == null || !mRouteProvider.getClientIds().contains(clientId)) return false;
        ...
        if ("SET_VOLUME".equals(messageType)) {
            CastSessionController.HandleVolumeMessageResult result =
                    mSessionController.handleVolumeMessage(
    ...
    boolean handleAppMessage(JSONObject jsonMessage) throws JSONException {
        ...
        if (!mSessionController.getRegisteredNamespaces().contains(namespaceName)) return false;
        ...
        if (actualMessageObject instanceof String) {
            ...
            return mSessionController.sendStringCastMessage(
    ...
    boolean sendJsonCastMessage(JSONObject message, final String namespace, final String clientId,
        ...
        if (!mSessionController.getSession().isConnected()) return false;
        ...
        try {
            mSessionController.getSession()
                    .sendMessage(namespace, message.toString())
                    .setResultCallback(new ResultCallback<Status>() {
                        ...
                        public void onResult(Status result) {
                            ...
                            // number.
                            onAppMessageSent(clientId, sequenceNumber);
                        }
                    });
        } catch (Exception e) {
    ...
    void onMediaMessage(String message, RequestRecord request) {
        mSessionController.onMediaMessage(message);

        if (isMediaStatusMessage(message)) {
            // MEDIA_STATUS needs to be sent to all the clients.
            for (String clientId : mRouteProvider.getClientIds()) {
    ...
    public void onApplicationStopped() {
        for (String clientId : mRouteProvider.getClientIds()) {
            Queue<Integer> sequenceNumbersForClient = mStopRequests.get(clientId);
            if (sequenceNumbersForClient == null) {
                sendClientMessageTo(clientId, "remove_session",
        ...
        mSessionController.onApplicationStopped();
    }
    ...
    public void onVolumeChanged() {
        ...
        broadcastClientMessage("update_session", mSessionController.buildSessionMessage());
    }
    ...
    public void sendClientMessageTo(
        ...
        mRouteProvider.notifyClientMessage(
                clientId, buildInternalMessage(type, message, clientId, sequenceNumber));
    }
    ...
    public void notifyReceiverAction(
        ...
        try {
            ...
            jsonReceiver.put("capabilities", CastUtils.getCapabilities(sink.getDevice()));
            ...
            mRouteProvider.notifyClientMessage(clientId, json.toString());
        } catch (JSONException e) {
