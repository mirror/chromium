...
public class PaymentManifestVerifier
     ...
     */
    public PaymentManifestVerifier(URI methodName, Set<ResolveInfo> defaultApplications,
    ...
    public void onPaymentMethodManifestFetched(String[] appIdentifiers) {
        ...
        for (int i = 0; i < appIdentifiers.length; i++) {
            ...
            if (AndroidPaymentAppFinder.looksLikeUriMethod(appIdentifiers[i])) {
                URI uriOrigin = AndroidPaymentAppFinder.parseUriFromString(appIdentifiers[i]);
                if (uriOrigin != null) {
        ...
        if (!cachedDefaultAppPackageNames.containsAll(mDefaultApplications.keySet())
