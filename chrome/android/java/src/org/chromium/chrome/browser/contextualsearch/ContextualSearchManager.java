...
public class ContextualSearchManager
    ...
    /**
     * Returns a {@link SelectionClient} that can handle the current platform's selection logic.
     * On Android O we apply the Smart-Selection client along with the Contextual Search client
     * via a {@link SelectionClientBridge}.  For pre-O we just instantiate the class
     * listed below.
     * @param previousSelectionClient The {@link SelectionClient} that was previously active, which
     *        may be useful to the {@link SelectionClientBridge}.
    ...
    SelectionClient switchSelectionClient(SelectionClient previousSelectionClient) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && previousSelectionClient != null) {
            return new SelectionClientBridge(
                    previousSelectionClient, contextualSearchSelectionClient);
        } else {
    ...
    ContextualSearchInternalStateHandler getContextualSearchInternalStateHandler() {
        return new ContextualSearchInternalStateHandler() {
            ...
            public void showContextualSearchLongpressUi() {
                ...
                // Android-O and beyond have a smart-select feature that subsumes CS on longpress.
                boolean isSmartSelectActive = Build.VERSION.SDK_INT >= Build.VERSION_CODES.O
                        && !ContextualSearchFieldTrial.isSmartSelectOnlyDisabled();
                ContextualSearchUma.logLongpressUxActivated(isSmartSelectActive);
