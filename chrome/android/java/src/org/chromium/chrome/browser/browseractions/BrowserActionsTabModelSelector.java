...
public class BrowserActionsTabModelSelector
    ...
    public void openNewTab(LoadUrlParams loadUrlParams, Callback<Tab> onTabCreated) {
        ...
        if (isTabStateInitialized()) {
            createNewTab(loadUrlParams, onTabCreated);
            return;
        }
            ...
            mTabCreationRunnable = new Runnable() {
                ...
                public void run() {
                    for (int i = 0; i < mPendingUrls.size(); i++) {
                        createNewTab(mPendingUrls.get(i), onTabCreated);
                    }
                    mPendingUrls.clear();
                }
            };
    ...
    private void createNewTab(LoadUrlParams params, Callback<Tab> onTabCreated) {
