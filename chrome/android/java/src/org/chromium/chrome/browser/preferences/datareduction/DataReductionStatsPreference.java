...
public class DataReductionStatsPreference extends Preference {
    ...
    public static void initializeDataReductionSiteBreakdownPref() {
        ...
        // If this is the first time the site breakdown feature is enabled, set the first allowable
        // date the breakdown can be shown.
        if (ChromeFeatureList.isEnabled(ChromeFeatureList.DATA_REDUCTION_SITE_BREAKDOWN)
            ...
            // DAYS_IN_CHART days, don't show the breakdown for another DAYS_IN_CHART days from the
            // last update time. Otherwise the site breakdown can be shown starting now.
            if (config.getDataReductionLastUpdateTime() + DAYS_IN_CHART * DateUtils.DAY_IN_MILLIS
                    > System.currentTimeMillis()) {
                ContextUtils.getAppSharedPreferences()
                        .edit()
                        .putLong(PREF_DATA_REDUCTION_SITE_BREAKDOWN_ALLOWED_DATE,
                                config.getDataReductionLastUpdateTime()
                                        + DAYS_IN_CHART * DateUtils.DAY_IN_MILLIS)
                        .apply();
            } else {
                ContextUtils.getAppSharedPreferences()
                        .edit()
        ...
        mResetStatisticsButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                new AlertDialog.Builder(getContext(), R.style.AlertDialogTheme)
                        .setTitle(R.string.data_reduction_usage_reset_statistics_confirmation)
