...
public class DownloadForegroundServiceManager {
    ...
    void processDownloadUpdateQueue(boolean isProcessingPending) {
        ...
        if (!mIsServiceBound) {
            ...
            if (!isActive(downloadUpdate.mDownloadStatus)) return;
    ...
    private void cleanDownloadUpdateQueue() {
        ...
        while (entries.hasNext()) {
            ...
            if (!isActive(entry.getValue().mDownloadStatus)) mDownloadUpdateQueue.remove(entry);
        }
    }
