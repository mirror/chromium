...
public class DownloadForegroundServiceManager {
    ...
    void processDownloadUpdateQueue(boolean isProcessingPending) {
        DownloadUpdate downloadUpdate = getNextDownloadUpdate();
        ...
        // If there are no active downloads left, stop the foreground service.
        ...
        if (!isActive(mDownloadUpdateQueue.get(mPinnedNotificationId).mDownloadStatus)) {
            startOrUpdateForegroundService(
                    downloadUpdate.mNotificationId, downloadUpdate.mNotification);
        }
    }
    ...
    private boolean hasActiveDownload() {
        for (DownloadUpdate downloadUpdate : mDownloadUpdateQueue.values()) {
            if (isActive(downloadUpdate.mDownloadStatus)) return true;
        }
        return false;
    }

    @Nullable
    private DownloadUpdate getNextDownloadUpdate() {
    ...
    @VisibleForTesting
    void setIsServiceBound(boolean isServiceBound) {
