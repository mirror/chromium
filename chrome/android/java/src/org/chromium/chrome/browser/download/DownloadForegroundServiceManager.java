...
public class DownloadForegroundServiceManager {
    ...
    void stopAndUnbindService(DownloadStatus downloadStatus) {
        ...
        DownloadForegroundService.StopForegroundNotification stopForegroundStatus;
        if (downloadStatus == DownloadStatus.CANCEL) {
            stopForegroundStatus = DownloadForegroundService.StopForegroundNotification.KILL;
        ...
        boolean notificationHandledProperly = stopAndUnbindServiceInternal(
                stopForegroundStatus, mPinnedNotificationId, oldNotification);
