...
import org.chromium.chrome.browser.media.router_caf.cast.CastMediaRouteProvider;
import org.chromium.chrome.browser.media.router_caf.cast.MediaSink;
import org.chromium.chrome.browser.media.router_caf.cast.MediaSource;
// import org.chromium.chrome.browser.media.router_caf.cast.remoting.RemotingMediaRouteProvider;
...
public class CafMediaRouter implements MediaRouteManager {
    ...
    private final Map < String, Map < MediaRouteProvider,
            List<MediaSink>>> mSinksPerSourcePerProvider = new HashMap < String,
                              Map < MediaRouteProvider, List<MediaSink>>>();
    ...
    @VisibleForTesting
            ...
            Map < MediaRouteProvider, List<MediaSink>>> getSinksPerSourcePerProviderForTest() {
    ...
    public void onMessage(String mediaRouteId, String message) {
      Log.i("zqzhang", "onMessage" + mediaRouteId + message);
        if (mNativeMediaRouterAndroidBridge != 0) {
     ...
     * @return an initialized {@link CafMediaRouter} instance
     */
    @CalledByNative
    public static CafMediaRouter create(long nativeMediaRouterAndroidBridge) {
        CafMediaRouter router = new CafMediaRouter(nativeMediaRouterAndroidBridge);
    ...
    public void sendStringMessage(String routeId, String message, int callbackId) {
      Log.i(TAG, "sendStringMessage" + routeId + message + callbackId);
      MediaRouteProvider provider = mRouteIdsToProviders.get(routeId);
        if (provider == null) {
    ...
    @VisibleForTesting
    protected CafMediaRouter(long nativeMediaRouterAndroidBridge) {
