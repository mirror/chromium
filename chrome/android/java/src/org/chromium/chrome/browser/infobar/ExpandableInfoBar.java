...
/**
 * InfoBar that can be displayed in 2 states: Compact and Expanded, without requiring to close of
 * swap the entire infobar.
 * The compact state features a simple message (see {@link #getCompactMessage()} followed by a
 * "Details" link that transitions it to its expanded state. The expanded state view can be built
 * as usual through {@link #createContent(InfoBarLayout)}.
 */
