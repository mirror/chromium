// Copyright 2017 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

package org.chromium.chrome.browser.preferences.password;

import android.app.Dialog;
import android.app.DialogFragment;
import android.os.Bundle;
import android.support.v7.app.AlertDialog;

import org.chromium.chrome.R;

/**
 * Shows the dialog that gives the user some tips for how to treat the exported passwords securely.
 */
public class ExportWarningDialogFragment extends DialogFragment {
    /**
     * Opens the dialog with the warning and sets the button listener to a fragment identified by ID
     * passed in arguments.
     */
    @Override
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        SavePasswordsPreferences savePasswordsFragment =
                (SavePasswordsPreferences) getFragmentManager().findFragmentById(
                        getArguments().getInt(
                                SavePasswordsPreferences.SAVE_PASSWORDS_PREFERENCES_ID));
        return new AlertDialog.Builder(getActivity(), R.style.SimpleDialog)
                .setPositiveButton(R.string.save_password_preferences_export_action_title,
                        savePasswordsFragment.getExportWarningHandler())
                .setNegativeButton(R.string.cancel, savePasswordsFragment.getExportWarningHandler())
                .setMessage(getActivity().getResources().getString(
                        R.string.settings_passwords_export_description))
                .create();
    }
}
