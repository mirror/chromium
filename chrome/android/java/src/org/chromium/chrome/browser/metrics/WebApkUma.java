...
public class WebApkUma {
     ...
     */
    public static void logAvailableSpaceAboveLowSpaceLimitInUMA(
    ...
    public static void logSpaceUsageUMAWhenInstallationFails() {
        new AsyncTask<Void, Void, ArrayList<Long>>() {
            ...
            protected ArrayList<Long> doInBackground(Void... params) {
                long availableSpaceAboveLowSpaceLimitInByte = getAvailableSpaceAboveLowSpaceLimit();
                ...
                result.add(availableSpaceAboveLowSpaceLimitInByte);
                result.add(cacheSizeInByte);
                return result;
            }

            @Override
            protected void onPostExecute(ArrayList<Long> result) {
                logAvailableSpaceAboveLowSpaceLimitInUMA(result.get(0));
                logCacheSizeInUMA(result.get(1));
                logAvailableSpaceAfterFreeUpCache(result.get(0) + result.get(1));
            }
        }.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
    }

    private static void logAvailableSpaceAfterFreeUpCache(
        ...
        // Bound the number to [-1000, 500] and round down to the nearest multiple of 10MB
        // to avoid exploding the histogram.
        availableSpaceAfterFreeUpCacheInMb = Math.max(-1000, availableSpaceAfterFreeUpCacheInMb);
        availableSpaceAfterFreeUpCacheInMb = Math.min(500, availableSpaceAfterFreeUpCacheInMb);
    ...
    private static long getAvailableSpaceAboveLowSpaceLimit() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
            ...
            mPartitionAvailableBytes = blockSize * (long) partitionStats.getAvailableBlocks();
        ...
        return availableBytesForInstallation;
    }
