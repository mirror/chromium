    ...
    public static void logAvailableSpaceAboveLowSpaceLimitInUMA(boolean installSucceeded) {
        ...
        new AsyncTask<Void, Void, Long>() {
            ...
            protected void onPostExecute(Long minimumFreeBytes) {
                ...
                int availableSpaceMb = (int) (availableBytesForInstallation / 1024 / 1024);
                // Bound the number to [-2000, 500] to avoid exploding the histogram.
                availableSpaceMb = Math.max(-2000, availableSpaceMb);
                availableSpaceMb = Math.min(500, availableSpaceMb);

                if (installSucceeded) {
                    RecordHistogram.recordSparseSlowlyHistogram("WebApk.Install.AvailableSpace"
                                    + "AboveLowSpaceLimitForInstallationWhenSuccessInMb",
                            availableSpaceMb);
                } else {
                    RecordHistogram.recordSparseSlowlyHistogram("WebApk.Install."
                                    + "AvailableSpaceAboveLowSpaceLimitForInstallationWhenFailInMb",
                            availableSpaceMb);
                }
            }
