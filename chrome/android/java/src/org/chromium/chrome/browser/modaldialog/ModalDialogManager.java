...
public class ModalDialogManager {
    ...
    public static abstract class Presenter {
         ...
         */
        private void setModalDialog(@Nullable ModalDialogView dialog) {
            if (dialog == null) {
         ...
         */
        protected ModalDialogView getModalDialog() {
            return mModalDialog;
        }
    ...
    private final List<Pair<ModalDialogView, Integer>> mPendingDialogs = new LinkedList<>();
    ...
    public void showDialog(ModalDialogView dialog, @ModalDialogType int dialogType) {
        ...
        mCurrentPresenter.setModalDialog(dialog);
    }
    ...
    public void dismissDialog(ModalDialogView dialog) {
        if (isShowing() && dialog != mCurrentPresenter.getModalDialog()) {
            Iterator<Pair<ModalDialogView, Integer>> it = mPendingDialogs.iterator();
            while (it.hasNext()) {
                if (it.next().first == dialog) {
                    it.remove();
                    break;
                }
            }
        ...
        mCurrentPresenter.setModalDialog(null);
