...
public class CompositorAnimationHandler {
     ...
     * every update.
     */
    private final ArrayList<CompositorAnimator> mClonedList = new ArrayList<>();
     ...
     *             updates.
     */
    public CompositorAnimationHandler(LayoutUpdateHost host) {
        assert host != null;
        mUpdateHost = host;
    }
    ...
    public final void startAnimator(final CompositorAnimator animator) {
        ...
        // If in testing mode, immediately push and update and end the animation.
        if (mIsInTestingMode) {
    ...
    public final void pushUpdate(long deltaTimeMs) {
        ...
        // the update loop.
        mClonedList.addAll(mAnimators);
        for (int i = 0; i < mClonedList.size(); i++) {
            CompositorAnimator currentAnimator = mClonedList.get(i);
