...
public class CompositorAnimationHandler {
    ...
    /** Whether or not an update has already been requested for the next frame. */
    private boolean mWasUpdateRequestedForFrame;
    ...
    /**
     * @param context An Android {@link Context} to get the handler for. If the provided context is
     *                null, no handler (null) is returned.
    ...
    public final void startAnimator(final CompositorAnimator animator) {
        ...
        if (!mWasUpdateRequestedForFrame) {
            mUpdateHost.requestUpdate();
            mWasUpdateRequestedForFrame = true;
        }
    }
    ...
    public final void pushUpdate(long deltaTimeMs) {
        mWasUpdateRequestedForFrame = false;
