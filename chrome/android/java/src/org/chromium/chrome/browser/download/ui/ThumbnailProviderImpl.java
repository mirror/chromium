...
public class ThumbnailProviderImpl implements ThumbnailProvider, ThumbnailStorageDelegate {
    ...
    private Bitmap getBitmapFromCache(String contentId, int bitmapSidePx) {
        Pair<Bitmap, Integer> cachedPair =
                getBitmapCache().get(Pair.create(contentId, bitmapSidePx));
    ...
    public void onThumbnailRetrieved(@NonNull String contentId, @Nullable Bitmap bitmap) {
        if (bitmap != null) {
            // We set the key pair to contain the required size (maximum dimension of the smaller
            // side) instead of the minimal dimension of the thumbnail so that future fetches of
            // this thumbnail can recognise the key in the cache.
