 ...
 */
public class ThumbnailProviderImpl implements ThumbnailProvider, ThumbnailRetrievalCallback {
    ...
    /**
     * Removes the thumbnails (different sizes) with the given contentId from disk (if disk-cached).
     * @param contentId The content ID of the thumbnail to remove.
     * @return Whether
     */
    @Override
    public boolean removeThumbnailsFromDisk(String contentId) {
        return mStorage.removeFromDisk(contentId);
    }
    ...
    private Bitmap getBitmapFromCache(String contentId, int bitmapWidthPx) {
        Pair<Bitmap, Integer> cachedPair =
                getBitmapCache().get(Pair.create(contentId, bitmapWidthPx));
