...
public class MainPreferences extends PreferenceFragment
    ...
    private boolean mWasTemplateUrlLoadListenerRegistered;
    ...
    /**
     * Sets up the mapping between the preference key and its order in the {@link PreferenceScreen}.
     */
    ...
    /**
     * Updates or creates the preferences as needed.
     */
    ...
    private void setupPreferenceCategory(String key, @StringRes int title) {
        ...
        setPreferenceKeyOrderTitle(preferenceCategory, key, title);
        getPreferenceScreen().addPreference(preferenceCategory);
    }

    private void setupSearchEnginePreference() {
        if (!mWasTemplateUrlLoadListenerRegistered) {
            TemplateUrlService.getInstance().registerLoadListener(this);
            mWasTemplateUrlLoadListenerRegistered = true;
        }

        ChromeBasePreference searchEnginePreference = setupChromeBasePreference(
                PREF_SEARCH_ENGINE, R.string.prefs_search_engine, SearchEnginePreference.class);
        if (!TemplateUrlService.getInstance().isLoaded()) {
            TemplateUrlService.getInstance().load();
            searchEnginePreference.setEnabled(false);
            return;
        }
    ...
    private ChromeBasePreference setupChromeBasePreference(String key, @StringRes int title,
        ...
        setPreferenceKeyOrderTitle(chromeBasePreference, key, title);
    ...
    private void setupSavedPasswordsPreference() {
        ChromeBasePreference preference = setupChromeBasePreference(PREF_SAVED_PASSWORDS,
    ...
    private void setupNotificationsPreference() {
        if (BuildInfo.isAtLeastO()) {
            ...
            Preference notifications = setupPreference(PREF_NOTIFICATIONS,
                    R.string.prefs_notifications, NotificationsPreferences.class);
    ...
    private Preference setupPreference(String key, @StringRes int title,
        ...
        setPreferenceKeyOrderTitle(preference, key, title);
    ...
    private void setupDataReductionPreference() {
        Preference dataReductionPreference = setupChromeBasePreference(
                PREF_DATA_REDUCTION, R.string.data_reduction_title, DataReductionPreferences.class);
        dataReductionPreference.setSummary(
                DataReductionPreferences.generateSummary(getResources()));
    }

    private void setPreferenceKeyOrderTitle(
    ...
    public void onSignedIn() {
        ...
        // or disabled.
        new Handler().post(() -> setupPreferences());
    }
