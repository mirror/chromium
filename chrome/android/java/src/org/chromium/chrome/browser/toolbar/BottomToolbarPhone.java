...
public class BottomToolbarPhone extends ToolbarPhone {
    ...
    /**
     * The float used to offset the rect returned by {@link #getLocationBarContentRect(Rect)}
     * when {@link #mUseModernDesign} is true.
     **/
    private float mLocationBarContentVerticalOffset;

    /**
     * The extra margin to apply to the left side of the location bar when it is focused and
     * {@link #mUseModernDesign} is true.
     */
    private int mLocationBarExtraFocusedMargin;
    ...
    public void getLocationBarContentRect(Rect outRect) {
        ...
        outRect.top += mLocationBarContentVerticalOffset;
        outRect.right -= mLocationBarBackgroundPadding.right;
        outRect.bottom -= mLocationBarContentVerticalOffset;
    }
    ...
    protected int getFocusedLocationBarWidth(int containerWidth, int priorVisibleWidth) {
        ...
        return super.getFocusedLocationBarWidth(containerWidth, priorVisibleWidth)
                - mLocationBarExtraFocusedMargin - mLocationBarBackgroundPadding.left
    ...
    protected int getFocusedLocationBarLeftMargin(int priorVisibleWidth) {
        ...
        int baseMargin = mToolbarSidePadding + mLocationBarExtraFocusedMargin;
    ...
    private void initModernDesign() {
        ...
        mLocationBarExtraFocusedMargin =
        ...
        // The location bar content rect is used when anonymizing the URL. Additonal vertical offset
        // is needed so that the anonymize rect doesn't extend past the rounded corners of the
        // toolbar background.
        mLocationBarContentVerticalOffset = (mLocationBarBackgroundCornerRadius / 2)
    ...
    protected void updateVisualsForToolbarState() {
        ...
        if (mUseModernDesign) {
            mLocationBarBackground.setTint(
                    isIncognito() ? Color.WHITE : ApiCompatibilityUtils.getColor(
    ...
    protected int getToolbarColorForVisualState(final VisualState visualState) {
        if (mUseModernDesign && visualState == VisualState.NORMAL) return Color.WHITE;
