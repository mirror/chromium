...
public class BrowserActionsService extends Service {
    ...
    private static Set<Integer> sLoadingUrlSet = new HashSet<Integer>();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (TextUtils.equals(intent.getAction(), ACTION_TAB_CREATION_START)) {
            mUrl = IntentUtils.safeGetStringExtra(intent, EXTRA_LINK_URL);
            String sourcePackageName =
                    IntentUtils.safeGetStringExtra(intent, EXTRA_SOURCE_PACKAGE_NAME);
            mContext = ContextUtils.getApplicationContext();

            createNotification(Tab.INVALID_TAB_ID, false);

            openTabInBackground(mUrl, sourcePackageName);
    ...
    private static void clearLoadingUrlSet() {
        sLoadingUrlSet.clear();
    }

    private void createNotification(int tabId, boolean isUpdate) {
        sendBrowserActionsNotification(tabId, isUpdate);
        if (isUpdate) {
    ...
    private void removeObserver() {
        ...
        if (mTabModelSelectorImpl != null) {
            mTabModelSelectorImpl.removeTabPersistentStoreObserver(mObserver);
        }
    }

    @VisibleForTesting
    static boolean isBackgroundService() {
        return sLoadingUrlSet.isEmpty();
    }
    ...
    private void openTabInBackground(String linkUrl, String sourcePackageName) {
        ...
        if (tab != null) {
            ...
            sLoadingUrlSet.add(tabId);
            createNotification(tabId, true);
            return;
        }
    ...
    private Tab launchTabInRunningTabbedActivity(LoadUrlParams loadUrlParams) {
        for (WeakReference<Activity> ref : ApplicationStatus.getRunningActivities()) {
            ...
            if (activity.getTabModelSelector() != null) {
                mTabModelSelectorImpl = (TabModelSelectorImpl) activity.getTabModelSelector();
                if (mObserver == null) {
                ...
                mTabModelSelectorImpl.addTabPersistentStoreObserver(mObserver);
                Tab tab = mTabModelSelectorImpl.openNewTab(
    ...
    private void launchTabInBrowserActionsModel(LoadUrlParams loadUrlParams) {
        ...
        mBrowserActionsSelector.addTabPersistentStoreObserver(mObserver);
    ...
    private void removeSavedTabs(List<Integer> savedTabIds) {
        for (int tabId : savedTabIds) {
            if (sLoadingUrlSet.contains(tabId)) sLoadingUrlSet.remove(tabId);
        }
    }

    private void sendBrowserActionsNotification(int tabId, boolean isUpdate) {
        ChromeNotificationBuilder builder = createNotificationBuilder(tabId, isUpdate);
        startForeground(NotificationConstants.NOTIFICATION_ID_BROWSER_ACTIONS, builder.build());
    }

    private ChromeNotificationBuilder createNotificationBuilder(int tabId, boolean isUpdate) {
        ...
        sNotificationIntent = buildNotificationIntent(tabId, isUpdate);
    ...
    private Intent buildNotificationIntent(int tabId, boolean isUpdate) {
        Intent intent;
        if (!isUpdate) {
            // Tab has not been created, set an VIEW intent to open the url.
