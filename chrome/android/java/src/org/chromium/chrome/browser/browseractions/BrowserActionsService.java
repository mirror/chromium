...
public class BrowserActionsService extends Service {
    ...
    private static HashSet<Integer> sLoadingTabSet = new HashSet<>();
    ...
    private void finishTabCreation(int tabId) {
        if (sLoadingTabSet.contains(tabId)) {
     ...
     */
    public static void sendOpenLinkInBackgroundIntent(String linkUrl, String packageName) {
        Context context = ContextUtils.getApplicationContext();
        Intent intent = new Intent(context, BrowserActionsService.class);
