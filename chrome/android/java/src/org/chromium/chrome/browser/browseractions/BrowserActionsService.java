...
public class BrowserActionsService extends Service {
            ...
            if (activity.getTabModelSelector() != null) {
                ...
                tab.addObserver(new EmptyTabObserver() {
                    ...
                    public void onPageLoadFinished(Tab tab) {
                        BrowserActionsServiceIntentParams params =
                                new BrowserActionsServiceIntentParams
                                        .Builder(BrowserActionsService.ACTION_TAB_CREATION_FINISH)
                                        .builder();
                        BrowserActionsService.sendIntent(params);
                    }
                });
                return tab.getId();
            }
    ...
    public static void cancelBrowserActionsNotification() {
        ...
        if (sServiceAlive) {
            BrowserActionsServiceIntentParams params =
                    new BrowserActionsServiceIntentParams
                            .Builder(ACTION_TAB_CREATION_CHROME_DISPLAYED)
                            .builder();
            BrowserActionsService.sendIntent(params);
        } else {
     ...
     * @param params The {@link BrowserActionsServiceIntentParams} holding params for the Intent.
     */
    public static void sendIntent(BrowserActionsServiceIntentParams params) {
        Context context = ContextUtils.getApplicationContext();
        Intent intent = new Intent(context, BrowserActionsService.class);
        String action = params.getAction();
        intent.setAction(action);
        if (action == ACTION_TAB_CREATION_START) {
            intent.putExtra(EXTRA_LINK_URL, params.getLinkUrl());
            intent.putExtra(EXTRA_SOURCE_PACKAGE_NAME, params.getSourcePackageName());
        }
