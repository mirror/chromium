...
public class PermissionUmaUtil {
    /**
     * Log an action with the current battery level, rounded down to the nearest 10%.
     */
    @CalledByNative
    private static void recordWithBatteryBucket(String histogram) {
        ...
        int rounded_percentage = ((int) (percentage / 10)) * 10;

        RecordHistogram.recordPercentageHistogram(histogram, rounded_percentage);
    }
}
