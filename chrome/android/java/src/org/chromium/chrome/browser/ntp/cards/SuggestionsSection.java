...
public class SuggestionsSection extends InnerNode {
    ...
    private boolean canUpdateSuggestions() {
        ...
        if (mNumberOfSuggestionsSeen >= getSuggestionsCount() || mHasAppended) {
            // TODO(tschumann): This does not work well when suggestions got removed. If the last
            // suggestion got removed, and all others were seen, then we actually could insert a
            // last one. Not sure if it's a big enough of a problem to fix here, though.
            // (this behavior gets exposed when testUiUpdatesNotSeenItems() has an initial size of 3
            // suggestions instead of 4).
