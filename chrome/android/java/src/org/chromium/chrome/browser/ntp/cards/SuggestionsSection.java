...
public class SuggestionsSection extends InnerNode {
    ...
    public SuggestionsSection(Delegate delegate, SuggestionsUiDelegate uiDelegate,
        ...
        mHeader = isExpandable
                ? new SectionHeader(info.getTitle(), false, this ::updateSuggestionsVisibility)
        ...
        if (!isChromeHomeEnabled) {
            mStatus.setVisible(!hasSuggestions() && shouldShowSuggestions());
        }
    }
    ...
    private void onSuggestionsListCountChanged(int oldSuggestionsCount) {
        ...
        if (mStatus != null) {
            mStatus.setVisible(shouldShowSuggestions() && !hasSuggestions());
        }
