...
public class ShareMenuActionHandler {
    ...
    static boolean shouldFetchCanonicalUrl(final Tab currentTab) {
        ...
        if (!UrlConstants.HTTPS_SCHEME.equals(GURLUtils.getScheme(currentTab.getUrl()))) {
            return false;
        }
    ...
    private static void triggerShareWithCanonicalUrlResolved(final Activity mainActivity,
        ...
        ContentBitmapCallback callback = (bitmap,
                response) -> ShareHelper.saveScreenshotToDisk(bitmap, mainActivity, result -> {
