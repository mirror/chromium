...
public class VrShellDelegate
    ...
    private boolean mEnteredVrForAutopresentation;
    ...
    public void shutdownVr(boolean disableVrMode, boolean canReenter, boolean stayingInChrome) {
        ...
        removeVrViews();
        destroyVrShell(stayingInChrome);
    ...
    private boolean createVrShell() {
        ...
        mEnteredVrForAutopresentation = mAutopresentWebVr && mVrShell != null;
     ...
     */
    private void destroyVrShell(boolean stayingInChrome) {
        if (mVrShell != null) {
            mVrShell.getContainer().setOnSystemUiVisibilityChangeListener(null);
            mVrShell.teardown();
            mVrShell = null;
            if (!stayingInChrome && mEnteredVrForAutopresentation) {
                // If entering VR through an auto present intent, close the tab that was opened by
                // the intent when leaving VR without entering Chrome. This is a workaround for
                // crbug.com/744678.
                mTabModelSelector.closeTab(mActivity.getActivityTab());
                mEnteredVrForAutopresentation = false;
            }
