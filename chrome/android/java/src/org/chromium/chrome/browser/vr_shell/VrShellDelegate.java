...
public class VrShellDelegate
    ...
    private static boolean deviceSupportsMultipleDensities() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) return false;
        Display display = DisplayAndroidManager.getDefaultDisplayForContext(
                ContextUtils.getApplicationContext());
        return display.getSupportedModes().length > 1;
    }
    ...
    private void onEnterVrUnsupported(ChromeActivity activity) {
        ...
        mVrDaydreamApi.launchVrHomescreen();
        // We need to explicitly disable VR mode on Samsung devices so that system UI is visible
        // when the user resumes Chrome.
        if (deviceSupportsMultipleDensities()) {
    ...
    public static void onNewIntentWithNative(ChromeActivity activity, Intent intent) {
        ...
        if (VrIntentUtils.getHandlerInstance().isTrustedAutopresentIntent(intent)) {
            ...
            if (!ChromeFeatureList.isEnabled(ChromeFeatureList.WEBVR_AUTOPRESENT)) {
                instance.onEnterVrUnsupported(activity);
                return;
            }
        ...
        // If Chrome is already running, the animation will be cancelled from onResume. Note that
        // this call is what causes us to enter VR mode when Chrome is cold-started. We start a
        // intermediate activity to cancel the animation which causes onPause and onResume to be
        // called and we enter VR mode in onResume (because we set the mEnterVrOnStartup bit).
        if (!instance.mPaused) {
            instance.cancelStartupAnimationIfNeeded();
        }
    }
    ...
    public static void setVrModeEnabled(Activity activity) {
        // We don't do this for devices that support multiple denities (i.e Samsung devices). This
        // is because they may change the screen density after exiting VR mode which was originally
        // triggered by a VR intent. The density change forces us to restart Chrome which
        // triggers this function again during UI inflation. As a result, we would set VR mode again
        // below, which would cause issues like the System UI never showing up because Samsung
        // handles system UI showing and hiding themselves upon setting the VR mode. That is, we
        // would have a nasty bug where the user would never see the system UI after exiting VR.
        if (!deviceSupportsMultipleDensities()) {
            getVrClassesWrapper().setVrModeEnabled(activity, true);
        }
    }
    ...
    private void setWindowModeForVr() {
        ...
        // Hide system UI. Note: We may have hidden the system UI earlier so that we don't see it
        // when Chrome is started in VR mode. This has the the issue that if we hide it for
        // another reason before entering VR mode, we always show it after exiting VR mode, which is
        // probably okay.
        int flags = mActivity.getWindow().getDecorView().getSystemUiVisibility();
        if (mRestoreSystemUiVisibilityFlag == -1) {
            mRestoreSystemUiVisibilityFlag = flags & ~VR_SYSTEM_UI_FLAGS;
        }
