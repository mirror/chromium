...
public class VrShellDelegate implements ApplicationStatus.ActivityStateListener,
    ...
    private final RealVSyncEstimator mVSyncEstimator;
    ...
    private class RealVSyncEstimator {
        ...
        private static final double VSYNC_DRIFT_THRESHHOLD = 1.2;
        ...
        private final FrameCallback mCallback = new FrameCallback() {
            ...
            public void doFrame(long frameTimeNanos) {
                ...
                if (mVSyncTimebaseNanos == 0) {
                    ...
                    nativeUpdateVSyncInterval(mNativeVrShellDelegate, mVSyncTimebaseNanos,
                            mVSyncIntervalNanos / 1000);
                    return;
                }
                ...
                if (vSyncIntervalNanos > mMaxVSyncIntervalNanos
                    ...
                    // This algorithm for computing "real" VSync becomes unstable if it drifts too
                    // far from the "real" VSync, which shouldn't happen in practice. If this assert
                    // is getting hit, something has gone very wrong, but we should probably do
                    // something reasonable for release builds.
                    assert false;
        ...
        public RealVSyncEstimator() {
            ...
            mMaxVSyncIntervalNanos = (long) (VSYNC_DRIFT_THRESHHOLD * mReportedVSyncNanos);
            mMinVSyncIntervalNanos = (long) (mReportedVSyncNanos / VSYNC_DRIFT_THRESHHOLD);
        }
    ...
    private VrShellDelegate(ChromeActivity activity, VrClassesWrapper wrapper) {
        ...
        mVSyncEstimator = new RealVSyncEstimator();
