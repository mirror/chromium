...
public class VrShellDelegate
    ...
    private void setWindowModeForVr() {
        ...
        // when Chrome is started in VR mode. If that is the case, we don't want to restore
        // to that after we exit VR mode. This has the the issue that if we hide this for another
        // reason before entering VR mode, we always show it after exiting VR mode, which is
        // probably okay.
        int flags = mActivity.getWindow().getDecorView().getSystemUiVisibility();
        boolean systemUiHidden = (flags & VR_SYSTEM_UI_FLAGS) == VR_SYSTEM_UI_FLAGS;
