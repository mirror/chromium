...
import org.chromium.chrome.browser.modaldialog.VrModalPresenter;
...
public class VrShellDelegate
     ...
     */
    public static void setDialogView(VrDialog dialog, View view) {
        if (sInstance == null) return;
        if (view != null) {
    ...
    private void enterVr(final boolean tentativeWebVrMode) {
        ...
        if (ChromeFeatureList.isEnabled(ChromeFeatureList.VR_BROWSING_NATIVE_ANDROID_UI)) {
            mActivity.getModalDialogManager().pushPresentersList(
                    new VrModalPresenter(mActivity), ModalDialogManager.APP_MODAL);
        }
    ...
    protected void shutdownVr(boolean disableVrMode, boolean stayingInChrome) {
        ...
        if (ChromeFeatureList.isEnabled(ChromeFeatureList.VR_BROWSING_NATIVE_ANDROID_UI)) {
            closeVrDialog();
            mActivity.getModalDialogManager().popPresentersList();
            if (mDialog != null) {
                mDialog.dismiss();
                mDialog = null;
            }
            setDialogView(null, null);
        }
        if (mShowingDaydreamDoff) {
            onExitVrResult(true);
            return;
