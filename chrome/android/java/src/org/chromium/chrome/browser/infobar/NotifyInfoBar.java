 ...
 *
 * It has 2 states, tapping on a "Details" link switches form the compact state to the
 * expanded one. It also features an OK button that just closes the InfoBar.
 *
 * See {@link NotifyInfoBarDelegate} to customize the infobar.
 */
public class NotifyInfoBar extends InfoBar {
    ...
    /**
     * Create and show a {@link NotifyInfoBar}.
     * @param tab The tab that the {@link InfoBar} should be shown in.
     * @param nativeInfoBarDelegate The NotifyInfoBarDelegateAndroid that drives the InfoBar.
     */
    public static void showNotifyInfoBar(Tab tab, long nativeInfoBarDelegate) {
        nativeCreate(tab, nativeInfoBarDelegate);
    }
    ...
    @CalledByNative
    private static NotifyInfoBar create(long nativeNotifyInfoBarDelegateAndroid) {
        return new NotifyInfoBar(new NotifyInfoBarDelegate(nativeNotifyInfoBarDelegateAndroid));
    }

    private static native void nativeCreate(Tab tab, long nativeInfoBarDelegate);
}
