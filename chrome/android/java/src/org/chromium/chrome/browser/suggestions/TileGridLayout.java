...
public class TileGridLayout extends FrameLayout {
    ...
    private static final int MARGIN_START_DP = 12;
    private static final int MARGIN_END_DP = 12;

    private static final int DISPLAY_MARGIN_PX = 9;
    ...
    public static int calculateNumColumns() {
        ...
        int totalWidth = Math.min(maxWidth,
                Math.min(metrics.widthPixels, metrics.heightPixels) - 2 * DISPLAY_MARGIN_PX);

        // Determine the number of columns that will fit.
        int gridWidth = totalWidth - PADDING_START_PX - PADDING_END_PX
                - Math.round((MARGIN_START_DP + MARGIN_END_DP) * metrics.density);
