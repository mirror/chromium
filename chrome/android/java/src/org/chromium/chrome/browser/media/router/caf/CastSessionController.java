...
import static org.chromium.chrome.browser.media.router.caf.CastMessageHandler.INVALID_SEQUENCE_NUMBER;
...
public class CastSessionController {
    ...
    public void updateNamespaces() {
      ...
      android.util.Log.i("zqzhang", "registerNamespaces()" + namespacesToRegister);
      android.util.Log.i("zqzhang", "unregisterNamespaces()" + namespacesToUnregister);
    ...
    public String buildSessionMessage() {
        ...
        try {
            ...
            jsonMessage.put("appId",
                    (applicationMetadata != null) ? applicationMetadata.getApplicationId()
                                                  : mSource.getApplicationId());
            jsonMessage.put("displayName",
                    (applicationMetadata != null) ? applicationMetadata.getName()
    ...
    public void onSessionStarted() {
        ...
        mRemoteMediaClient.addListener(new RemoteMediaClientListener());
    }

    public void onMediaMessage(String message) {
      android.util.Log.i("zqzhang", "onMediaMessage" + message);
      if (mRemoteMediaClient != null) {
            // Forwards the message to RemoteMediaClient as it's intercepted.
            mRemoteMediaClient.onMessageReceived(
                    mCastSession.getCastDevice(), CastUtils.MEDIA_NAMESPACE, message);
        }
    }
    ...
    public boolean sendStringCastMessage(final String message, final String namespace,
            final String clientId, final int sequenceNumber) {
    ...
    public HandleVolumeMessageResult handleVolumeMessage(JSONObject volume, final String clientId,
