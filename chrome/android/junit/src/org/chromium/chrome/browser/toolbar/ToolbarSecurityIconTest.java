...
public final class ToolbarSecurityIconTest {
    ...
    public void testGetSecurityLevel() {
        assertEquals(ConnectionSecurityLevel.NONE,
                ToolbarModelImpl.getSecurityLevel(null, !IS_OFFLINE_PAGE, false));
        assertEquals(ConnectionSecurityLevel.NONE,
                ToolbarModelImpl.getSecurityLevel(null, IS_OFFLINE_PAGE, false));
        assertEquals(ConnectionSecurityLevel.NONE,
                ToolbarModelImpl.getSecurityLevel(null, IS_OFFLINE_PAGE, true));
        assertEquals(ConnectionSecurityLevel.NONE,
                ToolbarModelImpl.getSecurityLevel(mTab, IS_OFFLINE_PAGE, false));

        for (int securityLevel : SECURITY_LEVELS) {
            doReturn(securityLevel).when(mTab).getSecurityLevel();
            assertEquals("Wrong icon for security level " + securityLevel, securityLevel,
                    ToolbarModelImpl.getSecurityLevel(mTab, !IS_OFFLINE_PAGE, false));
        }
