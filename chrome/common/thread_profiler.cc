...
PeriodicSamplingScheduler::Dependencies::Dependencies() = default;
...
PeriodicSamplingScheduler::Dependencies::Dependencies(
    ...
    : rand_double(rand_double), now(now) {}

PeriodicSamplingScheduler::Dependencies::Dependencies(
    const Dependencies& other) = default;

PeriodicSamplingScheduler::Dependencies::~Dependencies() = default;

// The scheduler works by splitting execution time into repeated periods such
// that one the time to take one collection represents
...
//
// For the settings at the top of the file (fraction = 0.02 and sampling period
// = 30s), the period length works out to 30s/0.02 = 1500s = 25m.
PeriodicSamplingScheduler::PeriodicSamplingScheduler(
  ...
  if (dependencies_.rand_double.is_null())
    dependencies_.rand_double = base::BindRepeating(&base::RandDouble);
  if (dependencies_.now.is_null())
    dependencies_.now = base::BindRepeating(&base::TimeTicks::Now);
}

base::TimeDelta PeriodicSamplingScheduler::GetTimeToNextCollection() {
  double sampling_offset_seconds =
      (period_duration_ - sampling_duration_).InSecondsF() *
      dependencies_.rand_double.Run();
  ...
  return next_collection_time - dependencies_.now.Run();
}
