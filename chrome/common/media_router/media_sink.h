...
//
// NOTE: This enum is used for recording the MediaRouter.Sink.Type metrics, so
// if we want to reorder it, we must create a separate enum that preserves the
// ordering, and map from this enum to the new one in
