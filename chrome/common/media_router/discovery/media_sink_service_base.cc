...
MediaSinkServiceBase::MediaSinkServiceBase(
    ...
    : discovery_timer_secs_(kFetchCompleteTimeoutSecs),
...
void MediaSinkServiceBase::StartTimer() {
  ...
  base::TimeDelta finish_delay =
      base::TimeDelta::FromSeconds(discovery_timer_secs_);
  discovery_timer_->Start(FROM_HERE, finish_delay,
                          base::Bind(&MediaSinkServiceBase::OnDiscoveryComplete,
                                     base::Unretained(this)));
}

void MediaSinkServiceBase::RestartTimer() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  if (!discovery_timer_ || discovery_timer_->IsRunning())
    return;
