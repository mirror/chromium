...
class StackSamplingConfiguration {
 public:
  ...
  // }
  //
  // This method is now called unsynchronized from two different threads (UI
  // thread and IO thread). This is okay due to the following reasons.
  //
  // 1) The profile configuration is guaranteed to get created when this method
  // is called from the IO thread, because of the access for the UI thread
  // sampling.
  //
  // 2) The profile configuration is constant, and the only other data accessed
  // in this method is the command line.
