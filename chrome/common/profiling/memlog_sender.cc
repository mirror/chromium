...
void InitMemlogSenderIfNecessary(const base::CommandLine& cmdline) {
  // TODO(ajwong): This is called from multiple spots in chrome startup because
  // ChromeMain() is the earliest point in windows for reasonably intercepting
  // process start. On linux though, it needs to be after ZygoteForked().
  //
  // Evaluate if this should just be movined into chrome/child under
