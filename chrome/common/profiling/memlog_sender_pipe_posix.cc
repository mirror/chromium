...
MemlogSenderPipe::Result MemlogSenderPipe::Send(const void* data, size_t sz, int timeout_ms) {
  ...
  while (size > 0) {
    ...
    // An error is either irrecoverable, or an I/O delay. Wait at most timeout_ms
    ...
    // Calculate time left.
    int64_t time_left_ms = ((start_time + base::TimeDelta::FromMilliseconds(timeout_ms)) - now)
                             .InMilliseconds();
    if (time_left_ms <= 0)
