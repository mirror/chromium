...
MemlogSenderPipe::Pipes::Pipes() = default;
MemlogSenderPipe::Pipes::Pipes(Pipes&& other) = default;

// On macOS, the pipe buffer defaults to 16k, but increases to 64k on large
// writes. On Linux, the defalt pipe buffer size is 64k.
...
#if defined(OS_MACOSX)
  // On macOS, suppress SIGPIPE. On Linux, we must use send() and a flag to do
  // this.
  PCHECK(fcntl(fds[0], F_SETNOSIGPIPE, 1) == 0);
  PCHECK(fcntl(fds[1], F_SETNOSIGPIPE, 1) == 0);
#endif
  pipes.receiver.reset(mojo::edk::PlatformHandle(fds[0]));
  pipes.sender.reset(mojo::edk::PlatformHandle(fds[1]));
  return pipes;
}
...
MemlogSenderPipe::Result MemlogSenderPipe::Send(const void* data,
  ...
  while (size > 0) {
#if defined(OS_MACOSX)
    // SIGPIPE is suppressed by F_SETNOSIGPIPE during creation.
    int r = HANDLE_EINTR(write(file_.get(), data, size));
#else
    // SIGPIPE is suppressed by passing |MSG_NOSIGNAL| to send().
    int r = HANDLE_EINTR(send(file_.get(), data, size, kSendFlags));
#endif
