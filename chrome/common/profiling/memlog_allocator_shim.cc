...
static base::OnceClosure g_on_init_allocator_shim_callback_;
static scoped_refptr<base::TaskRunner> g_on_init_allocator_shim_task_runner_;
...
class FrameSerializer {
 public:
  ...
  void AddAllFrames(const base::trace_event::Backtrace& backtrace) {
    CHECK(backtrace.frame_count <= max_stack_size_);
    for (size_t i = 0; i < backtrace.frame_count; ++i) {
      AddFrame(backtrace.frames[i]);
    }
  }

  void AddAllInstructionPointers(size_t frame_count, const void** frames) {
    CHECK(frame_count <= max_stack_size_);
 ...
 private:
  void AddFrame(const base::trace_event::StackFrame& frame) {
    ...
    if (sent_strings->find(frame.value) == sent_strings->end()) {
      ...
      const size_t kMaxCStringLen = 255;
      ...
      char message[length + sizeof(StringMappingPacket)];
      StringMappingPacket* string_mapping_packet =
          new (&message) StringMappingPacket;
      string_mapping_packet->op = kStringMappingPacketType;
...
void InitAllocatorShim(MemlogSenderPipe* sender_pipe,
  ...
  if (g_on_init_allocator_shim_callback_) {
    g_on_init_allocator_shim_task_runner_->PostTask(
        FROM_HERE, std::move(g_on_init_allocator_shim_callback_));
  }
}
...
void AllocatorShimLogAlloc(AllocatorType type,
  ...
  if (address) {
    ...
    FrameSerializer serializer(stack, address, kMaxStackEntries, send_buffers);
...
void SetOnInitAllocatorShimCallbackForTesting(
  ...
  g_on_init_allocator_shim_callback_ = std::move(callback);
  g_on_init_allocator_shim_task_runner_ = task_runner;
}
