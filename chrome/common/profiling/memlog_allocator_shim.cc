...
ShimState* GetShimState() {
  ShimState* state =
      static_cast<ShimState*>(g_tls_shim_state.Get());
...
const char* GetAndLeakThreadName() {
  constexpr size_t kBufferLen = 24;
...
class FrameSerializer {
 public:
  ...
  void AddCString(const char* c_string) {
    ...
    std::unordered_set<const void*>* sent_strings = &GetShimState()->sent_strings;
    ...
    if (sent_strings->find(c_string) == sent_strings->end()) {
      ...
      memcpy(message + sizeof(StringMappingPacket), c_string,
             length);
...
void InitAllocatorShim(MemlogSenderPipe* sender_pipe,
  ...
  if (include_thread_names) {
    g_include_thread_names = include_thread_names;
