...
// Writes to the pointer use atomic store with a barrier.
// Reads from the pointer use atomic load without a barrier.
// This guarantees that reads are atomic, but does not provide ordering
// guarantees between Reads and Writes.
 ...
 public:
  ...
  SendBuffer* Read() {
    return reinterpret_cast<SendBuffer*>(
        base::subtle::NoBarrier_Load(&send_buffers));
  }
...
// When the allocator shim is enabled, this variable holds a pointer to an array
// of SendBuffers. When the allocator shim is disabled, this variable holds
// nullptr. When the allocator shim is disabled, we do not destroy the array of
// SendBuffers. This allows reads to be safely reordering before writes, since
// either:
//   1) The Read() will return a SendBuffer pointer, which is guaranteed to be
//   valid forever.
//   2) The Read() will return nullptr, and the caller will do nothing.
...
void StopAllocatorShimDangerous() {
  // This pointer is leaked on purpose to avoid races. The contention caused by
  // adding a lock adds too much overhead.
