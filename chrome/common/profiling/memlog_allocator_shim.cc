...
// SendBuffers. As such, we do not need barriers to prevent reordering, we only
// need to ensure that writing/reading from this variable is atomic.
base::subtle::AtomicWord g_send_buffers = 0;
...
void InitAllocatorShim(MemlogSenderPipe* sender_pipe) {
  ...
  base::subtle::NoBarrier_Store(&g_send_buffers,
                                reinterpret_cast<base::subtle::AtomicWord>(
                                    new SendBuffer[kNumSendBuffers]));
...
void StopAllocatorShimDangerous() {
  ...
  base::subtle::NoBarrier_Store(
      &g_send_buffers, reinterpret_cast<base::subtle::AtomicWord>(nullptr));
...
void AllocatorShimLogAlloc(AllocatorType type,
                           ...
                           size_t sz,
                           const char* context) {
  SendBuffer* send_buffers = reinterpret_cast<SendBuffer*>(
      base::subtle::NoBarrier_Load(&g_send_buffers));
  if (!send_buffers)
    return;
