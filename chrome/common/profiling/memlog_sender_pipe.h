...
#if defined(OS_WIN)
  // Called when ::WriteFileEx has finished.
  void AsyncWriteFinished(DWORD error, DWORD bytes_written);
#endif
...
#if defined(OS_WIN)
  // The Windows MemlogSenderPipe requires additional state to handle async
  // callbacks from ::WriteFileEx.
  DWORD error_ = ERROR_SUCCESS;
  DWORD bytes_written_ = 0;
