...
AppThemeColorInfo::AppThemeColorInfo()
    : theme_color(kInvalidAppThemeColorValue) {}

AppThemeColorInfo::~AppThemeColorInfo() {}

// static
bool AppThemeColorInfo::GetThemeColor(const Extension* extension,
                                      SkColor* color) {
  int theme_color = GetInfo(extension).theme_color;
  if (theme_color == kInvalidAppThemeColorValue)
    return false;
