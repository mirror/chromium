...
struct AppThemeColorInfo : public Extension::ManifestData {
  ...
  static SkColor GetThemeColor(const Extension* extension);
  static const std::string& GetThemeColorString(const Extension* extension);

  // The color to use for the browser frame.
  SkColor theme_color_;

  // The string representation of the theme color.
  std::string theme_color_string_;
};
