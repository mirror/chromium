...
const Command* CommandsInfo::GetActionCommand(const Extension* extension) {
  ...
  return info ? info->action_command.get() : NULL;
}
...
void CommandsHandler::MaybeSetActionDefault(const Extension* extension,
  ...
  bool has_state_key = extension->manifest()->GetString(
      keys::kActionDefaultState, &default_state);
  bool default_enabled =
      (!has_state_key || (has_state_key && default_state == "enabled"));
  if (extension->manifest()->HasKey(keys::kAction) && default_enabled &&
      !info->action_command.get()) {
    info->action_command.reset(new Command(manifest_values::kActionCommandEvent,
