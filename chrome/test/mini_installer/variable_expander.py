...
class VariableExpander:
  ...
  def __init__(self, mini_installer_path, next_version_mini_installer_path):
    ...
    their values. These are the only acceptable variables:
        ...
        * $CHROME_TOAST_ACTIVATOR_CLSID_KEY: the registry key, excluding the
            root key, of Chrome for toast notication activator.
        * $CHROME_TOAST_ACTIVATOR_CLSID_KEY_BETA: the registry key, excluding
            the root key, of Chrome Beta for toast notication activator.
        * $CHROME_TOAST_ACTIVATOR_CLSID_KEY_DEV: the registry key, excluding the
            root key, of Chrome Dev for toast notication activator.
        * $CHROME_TOAST_ACTIVATOR_CLSID_KEY_SXS: the registry key, excluding the
            root key, of Chrome SxS for toast notication activator.
    ...
    if mini_installer_product_name == 'Google Chrome Installer':
      ...
      self._variable_mapping.update({
          ...
          'CHROME_TOAST_ACTIVATOR_CLSID_KEY': 'Software\\Chromium',
      })
    else:
