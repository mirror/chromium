...
def _GetFileType(file_path):
  """Returns the file type (including bitness) of the given file."""
...
class VariableExpander:
  ...
  def __init__(self, mini_installer_path, next_version_mini_installer_path):
    ...
    binary_type = _GetFileType(mini_installer_abspath)
    if binary_type == win32file.SCS_32BIT_BINARY:
      self._variable_mapping.update({
          'BITNESS': 'x86'
      })
    elif binary_type == win32file.SCS_64BIT_BINARY:
      self._variable_mapping.update({
          'BITNESS': 'x64'
      })
    else:
      raise KeyError("Unknown mini_installer binary type '%s'" % binary_type)
