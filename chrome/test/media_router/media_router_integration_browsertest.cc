...
const char kChooseSinkScript[] =
    ...
    "  sink.click();"
    "}";
const char kChooseLocalMediaCastModeScript[] =
    ...
    "         .castModeList.find(mode => mode.type == "
    "             media_router.CastModeType.LOCAL_FILE).description;"
    "   });"
    "if (mode) {"
    "  mode.click();"
    "}";
...
void MediaRouterIntegrationBrowserTest::OpenDialogAndCastFile() {
  ...
  ClickLocalMediaCastMode(dialog_contents);
...
void MediaRouterIntegrationBrowserTest::ClickLocalMediaCastMode(
    WebContents* dialog_contents) {
  ASSERT_TRUE(
      content::ExecuteScript(dialog_contents, kChooseLocalMediaCastModeScript));
}
