...
const char kChooseSinkScript[] =
    ...
    "  sink.click();"
    "}";
const char kChooseCastModeScript[] =
    "var modes = Array.from(document.getElementById('media-router-container')."
    "  "
    "shadowRoot.getElementById('cast-mode-list').getElementsByTagName('span'));"
    "var mode = modes.find(mode => {console.log(mode.textContent); return "
    "mode.textContent.trim() == '%s';});"
    "if (mode) {"
    "  mode.click();"
    "}";
...
void MediaRouterIntegrationBrowserTest::OpenDialogAndCastFile() {
  ...
  ChooseCastMode(dialog_contents, "Cast file");
...
void MediaRouterIntegrationBrowserTest::ChooseCastMode(
    WebContents* dialog_contents,
    const std::string& cast_mode_text) {
  std::string script =
      base::StringPrintf(kChooseCastModeScript, cast_mode_text.c_str());
...
MediaRouterUI* MediaRouterIntegrationBrowserTest::GetMediaRouterUI(
  ...
  if (!web_ui) {
    VLOG(0) << "Error getting MediaRouterUI, no WebUI at all";
    return nullptr;
  }
...
void MediaRouterIntegrationBrowserTest::FileDialogSelectsFile(
  ...
  class MockMediaRouterFileDialog : public MediaRouterFileDialog {
   public:
    MockMediaRouterFileDialog(MediaRouterFileDialogDelegate* delegate, GURL url)
  ...
  ASSERT_NE(nullptr, media_router_ui);
  media_router_ui->InitForTest(
      base::MakeUnique<MockMediaRouterFileDialog>(media_router_ui, file_url));
}
...
void MediaRouterIntegrationBrowserTest::FileDialogSelectFails(
  ...
  class MockMediaRouterFileDialog : public MediaRouterFileDialog {
   public:
    MockMediaRouterFileDialog(MediaRouterFileDialogDelegate* delegate,
                              const IssueInfo& issue)
  ...
  ASSERT_NE(nullptr, media_router_ui);
  media_router_ui->InitForTest(
      base::MakeUnique<MockMediaRouterFileDialog>(media_router_ui, issue));
}
