...
void ExecuteGetStatus(
  ...
  info.Set("build", base::MakeUnique<base::Value>(build.Clone()));
  info.Set("os", base::MakeUnique<base::Value>(os.Clone()));
...
void OnGetSession(const base::WeakPtr<size_t>& session_remaining_count,
  ...
  session->Set("capabilities", base::MakeUnique<base::Value>(value->Clone()));
