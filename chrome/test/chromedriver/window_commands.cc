...
Status ExecutePerformActions(Session* session,
  ...
  for (size_t i = 0; i < actions_by_tick.GetSize(); i++) {
    ...
    LOG(WARNING) << action_sequence->GetSize();
    ...
    for (size_t j = 0; j < action_sequence->GetSize(); j++) {
      ...
      if (subtype == "keyDown") {
        ...
        key_events.push_back(builder.SetType(kKeyDownEventType)
                                   ->SetText(base::UTF16ToUTF8(raw_key), base::UTF16ToUTF8(raw_key))
                                   ->SetKeyCode(key_code)
                                   ->SetModifiers(0)
                                   ->Build());
        // key_events.push_back(builder.SetType(kCharEventType)
        //                            ->SetText(base::UTF16ToUTF8(raw_key), base::UTF16ToUTF8(raw_key))
        //                            ->SetKeyCode(key_code)
        //                            ->SetModifiers(0)
        //                            ->Build());
