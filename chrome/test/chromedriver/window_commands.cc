  ...
  for (size_t i = 0; i < actions_by_tick.GetSize(); i++) {
    ...
    for (size_t j = 0; j < action_sequence->GetSize(); j++) {
      ...
      if (subtype == "keyDown") {
        ...
        dispatch_params.SetString("type", "keyDown");

        std::string raw_key;
        if (!action->GetString("value", &raw_key))
          return Status(kInvalidArgument, "value");

        dispatch_params.SetString("key", raw_key);
        dispatch_params.SetString("text", raw_key);
        dispatch_params.SetString("unmodifiedText", raw_key);
        // TODO: Set this based on shift and stuff
        dispatch_params.SetInteger("modifiers", 0);
        ...
        // raw key -> normalized key value



        // TODO: check pressed

        // translate raw key -> codes
...
Status PerformPointerMove() {
  return Status(kOk);
}

// make this work with ENUMS and actually clean
double ToPointerActionType(std::string name) {
  if (name == "pointerDown") {
    return 1;
