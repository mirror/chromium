...
TEST(ParseCapabilities, WithAndroidPackage) {
  ...
  caps.SetPath({"goog:chromeOptions", "androidPackage"}, base::Value("abc"));
...
TEST(ParseCapabilities, EmptyAndroidPackage) {
  Capabilities capabilities;
  base::DictionaryValue caps;
  caps.SetPath({"goog:chromeOptions", "androidPackage"},
...
TEST(ParseCapabilities, IllegalAndroidPackage) {
  Capabilities capabilities;
  base::DictionaryValue caps;
  caps.SetPath({"goog:chromeOptions", "androidPackage"}, base::Value(123));
  Status status = capabilities.Parse(caps);
  ASSERT_FALSE(status.IsOk());
}
...
TEST(ParseCapabilities, LogPath) {
  Capabilities capabilities;
  base::DictionaryValue caps;
  caps.SetPath({"goog:chromeOptions", "logPath"},
               base::Value("path/to/logfile"));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, Prefs) {
  ...
  prefs.SetKey("key1", base::Value("value1"));
  prefs.SetPath({"key2", "k"}, base::Value("value2"));
  base::DictionaryValue caps;
...
TEST(ParseCapabilities, LocalState) {
  ...
  local_state.SetKey("s1", base::Value("v1"));
  local_state.SetPath({"s2", "s"}, base::Value("v2"));
  base::DictionaryValue caps;
...
TEST(ParseCapabilities, UnrecognizedProxyType) {
  ...
  proxy.SetKey("proxyType", base::Value("unknown proxy type"));
  base::DictionaryValue caps;
...
TEST(ParseCapabilities, IllegalProxyType) {
  Capabilities capabilities;
  base::DictionaryValue proxy;
  proxy.SetKey("proxyType", base::Value(123));
  base::DictionaryValue caps;
  caps.SetKey("proxy", std::move(proxy));
  Status status = capabilities.Parse(caps);
  ASSERT_FALSE(status.IsOk());
...
TEST(ParseCapabilities, DirectProxy) {
  Capabilities capabilities;
  base::DictionaryValue proxy;
  proxy.SetKey("proxyType", base::Value("DIRECT"));
  base::DictionaryValue caps;
  caps.SetKey("proxy", std::move(proxy));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, SystemProxy) {
  Capabilities capabilities;
  base::DictionaryValue proxy;
  proxy.SetKey("proxyType", base::Value("system"));
  base::DictionaryValue caps;
  caps.SetKey("proxy", std::move(proxy));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, PacProxy) {
  Capabilities capabilities;
  base::DictionaryValue proxy;
  proxy.SetKey("proxyType", base::Value("PAC"));
  proxy.SetKey("proxyAutoconfigUrl", base::Value("test.wpad"));
  base::DictionaryValue caps;
  caps.SetKey("proxy", std::move(proxy));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, MissingProxyAutoconfigUrl) {
  ...
  base::DictionaryValue proxy;
  proxy.SetKey("proxyType", base::Value("PAC"));
  proxy.SetKey("httpProxy", base::Value("http://localhost:8001"));
  base::DictionaryValue caps;
  caps.SetKey("proxy", std::move(proxy));
  Status status = capabilities.Parse(caps);
  ASSERT_FALSE(status.IsOk());
...
TEST(ParseCapabilities, AutodetectProxy) {
  Capabilities capabilities;
  base::DictionaryValue proxy;
  proxy.SetKey("proxyType", base::Value("autodetect"));
  base::DictionaryValue caps;
  caps.SetKey("proxy", std::move(proxy));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, ManualProxy) {
  Capabilities capabilities;
  base::DictionaryValue proxy;
  proxy.SetKey("proxyType", base::Value("manual"));
  proxy.SetKey("ftpProxy", base::Value("localhost:9001"));
  proxy.SetKey("httpProxy", base::Value("localhost:8001"));
  proxy.SetKey("sslProxy", base::Value("localhost:10001"));
  proxy.SetKey("noProxy", base::Value("google.com, youtube.com"));
  base::DictionaryValue caps;
  caps.SetKey("proxy", std::move(proxy));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, EnableAcceptInsecureCerts) {
  Capabilities capabilities;
  base::DictionaryValue caps;
  caps.SetKey("acceptInsecureCerts", base::Value(true));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, LoggingPrefsOk) {
  ...
  logging_prefs.SetKey("Network", base::Value("INFO"));
  base::DictionaryValue caps;
...
TEST(ParseCapabilities, LoggingPrefsNotDict) {
  Capabilities capabilities;
  base::DictionaryValue caps;
  caps.SetKey("loggingPrefs", base::Value("INFO"));
  Status status = capabilities.Parse(caps);
  ASSERT_FALSE(status.IsOk());
}
...
TEST(ParseCapabilities, PerfLoggingPrefsInspectorDomainStatus) {
  ...
  perf_logging_prefs.SetKey("enableNetwork", base::Value(true));
  perf_logging_prefs.SetKey("enablePage", base::Value(false));
...
TEST(ParseCapabilities, PerfLoggingPrefsTracing) {
  ...
  base::DictionaryValue perf_logging_prefs;
  perf_logging_prefs.SetKey("traceCategories",
                            base::Value("benchmark,blink.console"));
  perf_logging_prefs.SetKey("bufferUsageReportingInterval", base::Value(1234));
  desired_caps.SetPath({"goog:chromeOptions", "perfLoggingPrefs"},
                       std::move(perf_logging_prefs));
  Status status = capabilities.Parse(desired_caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, PerfLoggingPrefsInvalidInterval) {
  ...
  perf_logging_prefs.SetKey("bufferUsageReportingInterval", base::Value(0));
  desired_caps.SetPath({"goog:chromeOptions", "perfLoggingPrefs"},
                       std::move(perf_logging_prefs));
  Status status = capabilities.Parse(desired_caps);
  ASSERT_FALSE(status.IsOk());
...
TEST(ParseCapabilities, UseRemoteBrowser) {
  Capabilities capabilities;
  base::DictionaryValue caps;
  caps.SetPath({"goog:chromeOptions", "debuggerAddress"},
               base::Value("abc:123"));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, MobileEmulationUserAgent) {
  ...
  mobile_emulation.SetKey("userAgent", base::Value("Agent Smith"));
  base::DictionaryValue caps;
...
TEST(ParseCapabilities, MobileEmulationDeviceMetrics) {
  Capabilities capabilities;
  base::DictionaryValue mobile_emulation;
  mobile_emulation.SetPath({"deviceMetrics", "width"}, base::Value(360));
  mobile_emulation.SetPath({"deviceMetrics", "height"}, base::Value(640));
  mobile_emulation.SetPath({"deviceMetrics", "pixelRatio"}, base::Value(3.0));
  base::DictionaryValue caps;
  caps.SetPath({"goog:chromeOptions", "mobileEmulation"},
               std::move(mobile_emulation));
  Status status = capabilities.Parse(caps);
...
TEST(ParseCapabilities, MobileEmulationDeviceName) {
  Capabilities capabilities;
  base::DictionaryValue mobile_emulation;
  mobile_emulation.SetKey("deviceName", base::Value("Nexus 5"));
  base::DictionaryValue caps;
  caps.SetPath({"goog:chromeOptions", "mobileEmulation"},
               std::move(mobile_emulation));
  Status status = capabilities.Parse(caps);
...
TEST(ParseCapabilities, MobileEmulationDeviceMetricsNotDict) {
  Capabilities capabilities;
  base::DictionaryValue mobile_emulation;
  mobile_emulation.SetKey("deviceMetrics", base::Value(360));
  base::DictionaryValue caps;
  caps.SetPath({"goog:chromeOptions", "mobileEmulation"},
               std::move(mobile_emulation));
  Status status = capabilities.Parse(caps);
...
TEST(ParseCapabilities, MobileEmulationBadDict) {
  Capabilities capabilities;
  base::DictionaryValue mobile_emulation;
  mobile_emulation.SetKey("deviceName", base::Value("Google Nexus 5"));
  mobile_emulation.SetPath({"deviceMetrics", "width"}, base::Value(360));
  mobile_emulation.SetPath({"deviceMetrics", "height"}, base::Value(640));
  mobile_emulation.SetPath({"deviceMetrics", "pixelRatio"}, base::Value(3.0));
  base::DictionaryValue caps;
