...
TEST(ParseCapabilities, Args) {
  ...
  base::DictionaryValue caps;
  caps.Set("goog:chromeOptions.args",
           base::MakeUnique<base::Value>(args.Clone()));
...
TEST(ParseCapabilities, Prefs) {
  ...
  base::DictionaryValue caps;
  caps.Set("goog:chromeOptions.prefs",
           base::MakeUnique<base::Value>(prefs.Clone()));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, LocalState) {
  ...
  caps.Set("goog:chromeOptions.localState",
           base::MakeUnique<base::Value>(local_state.Clone()));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, Extensions) {
  ...
  caps.Set("goog:chromeOptions.extensions",
           base::MakeUnique<base::Value>(extensions.Clone()));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, UnrecognizedProxyType) {
  ...
  base::DictionaryValue caps;
  caps.Set("proxy", base::MakeUnique<base::Value>(proxy.Clone()));
  Status status = capabilities.Parse(caps);
  ASSERT_FALSE(status.IsOk());
}
...
TEST(ParseCapabilities, LoggingPrefsOk) {
  ...
  base::DictionaryValue caps;
  caps.Set("loggingPrefs",
           base::MakeUnique<base::Value>(logging_prefs.Clone()));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, PerfLoggingPrefsInspectorDomainStatus) {
  ...
  desired_caps.Set("goog:chromeOptions.perfLoggingPrefs",
                   base::MakeUnique<base::Value>(perf_logging_prefs.Clone()));
...
TEST(ParseCapabilities, ExcludeSwitches) {
  ...
  caps.Set("goog:chromeOptions.excludeSwitches",
           base::MakeUnique<base::Value>(exclude_switches.Clone()));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseCapabilities, MobileEmulationUserAgent) {
  ...
  caps.Set("goog:chromeOptions.mobileEmulation",
           base::MakeUnique<base::Value>(mobile_emulation.Clone()));
  Status status = capabilities.Parse(caps);
  ASSERT_TRUE(status.IsOk());
