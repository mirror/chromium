    ...
    for url, results in self._test_results.iteritems():
      ...
      charts[async_missed_string][url] = (
          _GenerateTrace(async_missed_string, 'down', units='us',
                         # Unlike the rest of the data, missed async
                         # reprojection thread vsyncs are only logged when
                         # they happen so it's normal to get an empty list,
                         # which makes numpy and the perf dashboard unhappy
                         std=(0.0 if len(result) == 0 else numpy.std(result)),
                         values=(result or [0.0])))
      charts[async_missed_string]['summary']['values'].extend((result or [0.0]))
