...
// TODO(aleventhal) Refactor/move to external file if we create more rules.
bool CheckViewAccessibility(views::View* view, std::string* error_message) {
  ...
  if (node_data.role == ax::mojom::Role::kWebView)
    return true;  // Exception: the WebView does not require an accessible name.
