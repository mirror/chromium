...
// used. Unittests using the TestProfile violate some fundamental behaviors that
// ProfileNetworkContextService relies upon.
//
// In particular, TestingProfiles can be created BrowserContexts. Each
// TestingProfile has its own StoragePartition, but all TestingProfiles use the
// original BrowserContext for BrowserContextKeyedServices, while
// ProfileNetworkContextService expects, as a BrowserContextKeyedService, it's
// only going to only be used for one primary StoragePartition.
//
// Also, TestProfile bypasses ProfileIOData, so nothing calls
// ProfileNetworkContextService::SetUpProfileIODataMainContext.
//
// TODO(mmenke):  Figure how to get rid of this class.
