...
static bool CheckHasAccessibleNameOrLabel(ui::AXNodeData& data) {
...
static bool CheckVisible(ui::AXNodeData& data) {
...
bool RunUIAccessibilityChecks(std::string* error_message) {
  ...
  for (size_t index = 0; index < active_browser_list->size(); index++) {
    ...
    if (!CheckViewSubtreeAccessibility(browser_view, error_message)) {
      return false;
    }
  }
