...
final class MultiActivityTestCommon {
    ...
    void waitForFullLoad(final ChromeActivity activity, final String expectedTitle)
            throws Exception {
        CallbackHelper newTabCreatorHelper = new CallbackHelper();
        int callCount = newTabCreatorHelper.getCallCount();
        waitForTabCreation(activity, newTabCreatorHelper);
        newTabCreatorHelper.waitForCallback(callCount);
    ...
    private void waitForTabCreation(
            ChromeActivity activity, final CallbackHelper newTabCreatorHelper) {
