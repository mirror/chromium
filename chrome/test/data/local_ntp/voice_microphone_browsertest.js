...
test.microphone.testStartLevelAnimationFromActive = function() {
  ...
  microphone.startInputAnimation();
  assert(microphone.isLevelAnimating_);
  assert(!!microphone.level_.style.getPropertyValue('transition'));
  assertEquals(1, test.microphone.clock.pendingTimeouts.length);
  const stepOneActivationTime =
      test.microphone.clock.pendingTimeouts[0].activationTime;
