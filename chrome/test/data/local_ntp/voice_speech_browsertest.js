 ...
 * initiates speech.
 */
test.speech.testFakeboxClickStarsSpeechWithWorkingView = function() {
  test.speech.initSpeech();
...
test.speech.testHandleFinalSpeechResponse = function() {
  ...
  const encodedQuery = speech.encodeQueryText_(highConfidenceText);
  assertEquals(
      `${test.speech.TEST_BASE_URL}search?q=${encodedQuery}&gs_ivs=1`,
      test.speech.locationUrl);
  // TODO(oskopek): Test UMA calls for submission due to final result.
  test.speech.validateInactive();
};
...
test.speech.testInterruptSpeechInputBeforeResult = function() {
  ...
  test.speech.validateInactive();
  // TODO(oskopek): Test UMA calls due to aborting before result received.
};


/**
 * Tests speech recognition error handling.
 */
test.speech.testSpeechRecognitionError = function() {
  test.speech.initSpeech();
  speech.toggleStartStop();
  ...
  // TODO(oskopek): Test UMA calls due to error.
...
/**
 * Tests behavior when no speech input was received.
 */
test.speech.testNoSpeechInput = function() {
  ...
  // TODO(oskopek): Test UMA calls due to no speech.
...
test.speech.testStopStartErrorHandling = function() {
  // Remove the webkitSpeechRecognition mock.
  test.speech.stubs.reset();
...
test.speech.testEnterToSubmit = function() {
  ...
  test.speech.validateInactive();
  const encodedQuery = speech.encodeQueryText_(query);
  assertEquals(
      `${test.speech.TEST_BASE_URL}search?q=${encodedQuery}&gs_ivs=1`,
      test.speech.locationUrl);
  assertEquals('', speech.interimResult_);
  assertEquals('', speech.finalResult_);
  // TODO(oskopek): Test UMA calls for submission due to enter key.
};
...
test.speech.testClickToSubmit = function() {
  ...
  assertEquals('', speech.interimResult_);
  assertEquals('', speech.finalResult_);
  // TODO(oskopek): Test UMA calls for submission due to click on mic icon.
};
...
test.speech.testClickToAbort = function() {
  ...
  test.speech.validateInactive();
  // TODO(oskopek): Test UMA calls due to abort.
};
...
test.speech.testClickToRetryWhenStopped = function() {
  ...
  assertTrue(speech.isRecognizing_());
  assertEquals(speech.State_.STARTED, speech.currentState_);
  // TODO(oskopek): Test UMA calls due to forced onEnd.
  // TODO(oskopek): Test UMA calls due to click to retry.
};


/**
 * Tests click to retry when the interface is not stopped stops recognition
 * (like a background click).
 */
test.speech.testClickToRetryWhenNotStopped = function() {
  ...
  test.speech.validateInactive();
  // TODO(oskopek): Test UMA calls due to retry.
};
...
test.speech.testNoSpeechInputMatched = function() {
  ...
  // TODO(oskopek): Test UMA calls for no_match.
  test.speech.clock.advanceTime(7999);
  assertEquals(1, test.speech.viewActiveCount);
...
test.speech.testIdleTimeoutWithConfidentSpeechResults = function() {
  ...
  test.speech.clock.pendingTimeouts.shift().callback();
  test.speech.validateInactive();
  // TODO(oskopek): Test UMA calls due to idleTimeout.
  assertEquals(
  ...
  assertEquals(
      `${test.speech.TEST_BASE_URL}search?q=${encodedQuery}&gs_ivs=1`,
      test.speech.locationUrl);
  // TODO(oskopek): Test UMA calls due to idleTimeout submission.
};
...
test.speech.testIdleTimeoutWithNonConfidentSpeechResults = function() {
  ...
  test.speech.clock.pendingTimeouts.shift().callback();
  test.speech.validateInactive();
  assert(!test.speech.locationUrl.startsWith(test.speech.TEST_BASE_URL));
  // TODO(oskopek): Test UMA calls due to idleTimeout.
};
