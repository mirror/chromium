...
// updated by received webrtc streams in content_shell. crbug.com/793808
async function testRTCRtpSenderReplaceTrackSendsNewVideoTrack() {
  ...
  const blueCanvasStream = blueCanvas.captureStream();
  ...
  let receivedColor = await pollNextVideoColor(remoteVideo, remoteVideoCanvas);
  ...
  // Ensure a blue frame is sent by redrawing the canvas.
  fillCanvas(blueCanvas, 'blue');
  receivedColor = await pollNextVideoColor(remoteVideo, remoteVideoCanvas);
 ...
 * canvas and reading the color of the canvas.
 * @private
 */
