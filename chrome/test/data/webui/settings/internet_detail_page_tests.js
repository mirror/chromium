...
suite('InternetDetailPage', function() {
  ...
  function allowSharedProxyHidden() {
    ...
    return allowShared.hasAttribute('hidden');
  }
  ...
  suite('DetailsPage', function() {
    ...
    test('Proxy Shared', function() {
      ...
      internetDetailPage.init('wifi_user_guid', 'WiFi', 'wifi_user');
      return flushAsync().then(() => {
        assertFalse(allowSharedProxyHidden());
      });
    });

    // When proxy settings are managed by the user they are always used so
    ...
    test('Proxy Shared User Managed', function() {
      ...
      internetDetailPage.init('wifi_user_guid', 'WiFi', 'wifi_user');
      return flushAsync().then(() => {
        assertTrue(allowSharedProxyHidden());
      });
    });
