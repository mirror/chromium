...
cr.define('device_page_tests', function() {
  ...
  function TestDevicePageBrowserProxy() {
    ...
    this.setAppEnabledOnLockScreenCount_ = 0;
  }
  ...
  TestDevicePageBrowserProxy.prototype = {
    ...
    /** @override */
    setNoteTakingAppEnabledOnLockScreen: function(appId, enabled) {
      ++this.setAppEnabledOnLockScreenCount_;

      var app = this.noteTakingApps_.find(function(existing) {
        return existing.value == appId;
      });
      assert(!!app);
      assert(app.preferred);

      if (enabled) {
        assertEquals('Supported', app.lockScreenSupport);
        app.lockScreenSupport = 'Enabled';
      } else {
        assertEquals('Enabled', app.lockScreenSupport);
        app.lockScreenSupport = 'Supported';
      }
    ...
    // Test interface
    setAndroidAppsReceived: function() {
      this.androidAppsReceived_ = true;
    ...
    getPreferredAppLockScreenState: function() {
      ...
      return app ? app.lockScreenSupport : '';
    },
    ...
    suite(assert(TestNames.Stylus), function() {
      ...
      test('choose first app if no preferred ones', function() {
        ...
        browserProxy.setNoteTakingApps([
          entry('n1', 'v1', false, 'NotSupported'),
          entry('n2', 'v2', false, 'NotSupported')
        ]);
        Polymer.dom.flush();
      ...
      test('preferred app does not change without interaction', function() {
        ...
        browserProxy.addNoteTakingApp(entry('n', 'v', false, 'NotSupported'));
        Polymer.dom.flush();
        assertEquals('', browserProxy.getPreferredNoteTakingAppId());
      ...
      test('app-visibility', function() {
        ...
        browserProxy.setAndroidAppsReceived();
        assert(!noAppsDiv.hidden);
        assert(waitingDiv.hidden);
        assert(selectAppDiv.hidden);
      ...
      test('enabled-on-lock-screen', function() {
        ...
        return new Promise(function(resolve) {
          ...
          browserProxy.addNoteTakingApp(entry('n2', 'v2', false, 'Supported'));
          return new Promise(function(resolve) { stylusPage.async(resolve); });
        }).then(function() {
          Polymer.dom.flush();
          expectFalse(isVisible(enableAppOnLockScreenToggle()));
      ...
      test('tap-on-enable-note-taking-on-lock-screen', function() {
        return new Promise(function(resolve) {
          ...
          assertEquals(1, browserProxy.setAppEnabledOnLockScreenCount_);
          ...
          expectEquals('Enabled',
                       browserProxy.getPreferredAppLockScreenState());

          MockInteractions.tap(enableAppOnLockScreenToggle());
          assertEquals(2, browserProxy.setAppEnabledOnLockScreenCount_);
      ...
      test('lock-screen-apps-disabled-by-policy', function() {
        ...
        return new Promise(function(resolve) {
          ...
          browserProxy.addNoteTakingApp(
              entry('n2', 'v2', true, 'NotAllowedByPolicy'));
          stylusPage.async(resolve);
        }).then(function() {
          Polymer.dom.flush();
          assert(isVisible(enableAppOnLockScreenToggle()));
          ...
          MockInteractions.tap(enableAppOnLockScreenToggle());
          assertEquals(0, browserProxy.setAppEnabledOnLockScreenCount_);
          ...
          expectEquals('NotAllowedByPolicy',
                       browserProxy.getPreferredAppLockScreenState());
        });
      });
    });
