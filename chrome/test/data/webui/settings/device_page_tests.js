...
cr.define('device_page_tests', function() {
  ...
  var LockScreenSupport = settings.NoteAppLockScreenSupport;
  ...
  TestDevicePageBrowserProxy.prototype = {
    ...
    setNoteTakingAppEnabledOnLockScreen: function(appId, enabled) {
      ...
      if (enabled) {
        assertEquals(LockScreenSupport.SUPPORTED, app.lockScreenSupport);
        app.lockScreenSupport = LockScreenSupport.ENABLED;
      } else {
        assertEquals(LockScreenSupport.ENABLED, app.lockScreenSupport);
        app.lockScreenSupport = LockScreenSupport.SUPPORTED;
      }
    ...
    // Test interface:
    /** Marks android apps list state as received. */
