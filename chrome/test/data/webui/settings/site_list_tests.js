 ...
 * @param {string} origin The origin for the exception to be created.
 * @param {Object} override An object with a subset of the properties of
 *     |RawSiteException|. If it exists, it will be assigned to the returned
 *     value.
...
/**
 * Converts a Map into a SiteSettingsPref object.
 * @param {Map<settings.ContentSettingsTypes, !Array<!RawSiteException>>}
 *     exceptions A map of |ContentSettingsTypes| to |RawSiteException| to
 *     include in the final |SiteSettingsPref| object.
 * @return {SiteSettingsPref}
 */
function createPrefs(exceptions) {
  let siteSettingsPref = {exceptions: {}};
  exceptions.forEach((pref, contentSettingType) => {
    siteSettingsPref.exceptions[contentSettingType] = pref;
  });
  return siteSettingsPref;
}
...
function populateTestExceptions() {
  // Create a map to help populate all the test exception objects.
  let tmpExceptionMap = new Map();

  tmpExceptionMap.set(settings.ContentSettingsTypes.GEOLOCATION, [
  ...
  prefsGeolocation = createPrefs(tmpExceptionMap);

  tmpExceptionMap.clear();
  tmpExceptionMap.set(settings.ContentSettingsTypes.PLUGINS, [
  ...
  prefsControlled = createPrefs(tmpExceptionMap);
  ...
  prefsMixedSchemes = createPrefs(tmpExceptionMap);
  ...
  prefsMixedProvider = createPrefs(tmpExceptionMap);

  tmpExceptionMap.clear();
  tmpExceptionMap.set(settings.ContentSettingsTypes.IMAGES, [
    createExceptionForTest(null, {
  ...
  prefsMixedEmbeddingOrigin = createPrefs(tmpExceptionMap);
  ...
  tmpExceptionMap.set(settings.ContentSettingsTypes.NOTIFICATIONS, [
    createExceptionForTest(null, {
  ...
  prefsVarious = createPrefs(tmpExceptionMap);
  ...
  prefsOneEnabled = createPrefs(tmpExceptionMap);
  ...
  prefsOneDisabled = createPrefs(tmpExceptionMap);

  tmpExceptionMap.clear();
  tmpExceptionMap.set(settings.ContentSettingsTypes.COOKIES, [
    createExceptionForTest(null, {
      origin: 'http://foo-block.com',
      embeddingOrigin: '',
      incognito: false,
  ...
  prefsSessionOnly = createPrefs(tmpExceptionMap);
  ...
  prefsIncognito = createPrefs(tmpExceptionMap);

  tmpExceptionMap.clear();
  tmpExceptionMap.set(settings.ContentSettingsTypes.JAVASCRIPT, [
    createExceptionForTest(null, {
  ...
  prefsChromeExtension = createPrefs(tmpExceptionMap);

  tmpExceptionMap.clear();
  tmpExceptionMap.set(settings.ContentSettingsTypes.GEOLOCATION, []);
  prefsGeolocationEmpty = createPrefs(tmpExceptionMap);
}
