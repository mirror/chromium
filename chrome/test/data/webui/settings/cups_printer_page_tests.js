...
class TestCupsPrintersBrowserProxy extends TestBrowserProxy {
  ...
  addDiscoveredPrinter(printerId) {
    this.methodCalled('addDiscoveredPrinter');
  }
...
suite('CupsAddPrinterDialogTests', function() {
  ...
  test('LogDialogCancelledIpp', function() {
    ...
    var pressCancel = cupsPrintersBrowserProxy.whenCalled('getPrinterInfo')
    ...
    var verifyCancel =
        cupsPrintersBrowserProxy.whenCalled('cancelPrinterSetUp')
            .then(function(newPrinter) {
              assertTrue(!!newPrinter, 'New printer is null');
    ...
    return pressCancel.then(verifyCancel);
  });
  ...
  test('LogDialogCancelledUSB', function() {
    ...
    var selectPrinter =
        cupsPrintersBrowserProxy.whenCalled('startDiscoveringPrinters')
            .then(function(dialog, printer) {
              // Select the printer.
              // TODO(skau): Figure out how to select in a dom-repeat.
              var discoveryDialog = dialog.$$('add-printer-discovery-dialog');
              assertTrue(!!discoveryDialog, 'Cannot find discovery dialog');
              discoveryDialog.selectedPrinter = printer;
              // Run printer setup.
              clickAddButton(discoveryDialog);
            }.bind(this, dialog, newPrinter));

    var requiresManualSetup =
        cupsPrintersBrowserProxy.whenCalled('addDiscoveredPrinter')
            .then(function(printer) {
              cr.webUIListenerCallback(
                  'on-manually-add-discovered-printer', printer);
            }.bind(this, newPrinter));

    var clickCancel =
        cupsPrintersBrowserProxy.whenCalled('getCupsPrinterManufacturersList')
            .then(function(dialog) {
              // Cancel setup with the cancel button.
              clickCancelButton(
    ...
    return selectPrinter.then(requiresManualSetup)
        .then(clickCancel)
        .then(verifyUsbInfo);
  });
});
