...
LineChartTest.SubChart = function() {
  test('SubChart integration test', function() {
    ...
    assertFalse(subChart.shouldBeRender());
    ...
    assertTrue(subChart.shouldBeRender());

    subChart.setLayout(1920, 1080, 14, 1504764695799, 150, 8);
    TestUtil.assertCloseTo(subChart.maxValue_, 2389.333, 1e-2);

    subChart.setLayout(1920, 1080, 14, 1504764695799, 10, 8);
    TestUtil.assertCloseTo(subChart.maxValue_, 4096, 1e-2);

    subChart.setLayout(150, 100, 14, 1504764685799, 100, 8);
    console.log(subChart.getValuesFromDataSeries_(data3));
    TestUtil.assertCloseTo(subChart.maxValue_, 3072, 1e-2);
  });
