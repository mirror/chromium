...
PageTest.Unit = function() {
  suite('Page unit test', function() {
    test('getDiffPerSecAndUpdateCounter_', function() {
      assertEquals(getDiffAndUpdateCounter_('test', 10, 1000), 0);
      assertEquals(getDiffAndUpdateCounter_('test', 20, 2000), 10);
      assertEquals(getDiffAndUpdateCounter_('test', 42, 3000), 22);

      TestUtil.assertCloseTo(
          getDiffPerSecAndUpdateCounter_('test2', 10, 1000), 0, 1e-2);
      TestUtil.assertCloseTo(
          getDiffPerSecAndUpdateCounter_('test2', 20, 3000), 5, 1e-2);
      TestUtil.assertCloseTo(
          getDiffPerSecAndUpdateCounter_('test2', 42, 3720), 30.555, 1e-2);
      TestUtil.assertCloseTo(
          getDiffPerSecAndUpdateCounter_('test2', 59, 4999), 13.291, 1e-2);
    });
