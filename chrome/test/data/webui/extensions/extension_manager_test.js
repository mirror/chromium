...
cr.define('extension_manager_tests', function() {
  ...
  suite('ExtensionManagerTest', function() {
    ...
    function isActiveView(viewElement) {
      expectEquals(
          viewElement, manager.$.viewManager.querySelector('.active').is);
    }
    ...
    test(assert(TestNames.ChangePages), function() {
      ...
      item.onDetailsTap_();
      Polymer.dom.flush();
      isActiveView('extensions-detail-view');
