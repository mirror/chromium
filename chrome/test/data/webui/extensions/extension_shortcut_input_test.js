  ...
  suite('ExtensionShortcutInputTest', function() {
    ...
    test(assert(TestNames.Basic), function() {
      ...
      return input.delegate.whenCalled('setShortcutHandlingSuspended')
          .then((arg) => {
            assertTrue(arg)
            input.delegate.reset('setShortcutHandlingSuspended');
          ...
          .then((arg) => {
            input.delegate.reset('updateExtensionCommand')
            expectEquals('itemid', arg[0]);
            expectEquals('Command', arg[1]);
            expectEquals('Ctrl+A', arg[2]);
