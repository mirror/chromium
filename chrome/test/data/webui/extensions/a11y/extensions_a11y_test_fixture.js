...
var ExtensionsA11yTestFixture = class extends PolymerTest {
  ...
  static get violationFilter() {
    return {
      ...
      'duplicate-id': function(nodeResult) {
        let allIconSets = true;

        nodeResult.any.forEach((hit) => {
          hit.relatedNodes.forEach(node => {
            allIconSets = allIconSets && ExtensionsA11yTestFixture.hasAncestor_(
  ...
  /**
   * Returns true if an ancestor of |node| is a |type| element.
   * @param {!HTMLNode} node
   * @param {string} type
   * @return {boolean}
   * @private
