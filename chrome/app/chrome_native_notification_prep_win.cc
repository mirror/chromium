...
#define CoCreatableClassWithFactory2(className, factory)                       \
  InternalWrlCreateCreatorMap2(className##_COM, &__uuidof(className), nullptr, \
                               CreateClassFactory<factory>, "minATL$__f")

#define InternalWrlCreateCreatorMap2(className, runtimeClassName, trustLevel, \
                                     creatorFunction, section)                \
  FactoryCache __objectFactory__##className = {nullptr, {0}};                 \
  extern const CreatorMap __object_##className = {                            \
      creatorFunction,                                                        \
      {runtimeClassName},                                                     \
      trustLevel,                                                             \
  ...
  extern "C" __declspec(allocate(section))                                    \
      const CreatorMap* const __minATLObjMap_##className =                    \
          &__object_##className;                                              \
...
//
// Namespace Microsoft::WRL is needed for RuntimeClass as there is another
// RuntimeClass defined in namespace Microsoft::WRL::Details.
class DECLSPEC_UUID(__CLSID) NotificationActivator
 ...
 public:
  HRESULT STDMETHODCALLTYPE Activate(
    ...
    ::MessageBoxW(NULL, L"app activated", L"", MB_OK);
    return S_OK;
  }
};
...
HRESULT RegisterAppForNativeNotification() {
  ...
  // TODO(chengx): update the shortcut name
  hr = ::PathCchCombine(
      shortcutPath, ARRAYSIZE(shortcutPath), appData.Get(),
      LR"(Microsoft\Windows\Start Menu\Programs\testnotification.lnk)");
  if (FAILED(hr))
    return hr;
  ...
  // TODO(chengx): remove, for test purpose
  const wchar_t* arguments = L"--user-data-dir=C:\\testnotification";
