...
void CheckShortcutInfo() {
  ...
  // Check if the shorcut exists.
  if (base::PathExists(shortcut_path)) {
    ::MessageBoxW(NULL, L"shortcut exists", L"", MB_OK);
    ...
    // Check if the existing shorcut has no clsid information.
    if (properties.toast_activator_clsid == GUID_NULL)
      ::MessageBoxW(NULL, L"shortcut has no clsid", L"", MB_OK);
    else
      ::MessageBoxW(NULL, L"shortcut has clsid", L"", MB_OK);
  } else {
    ::MessageBoxW(NULL, L"shortcut doesn't exist", L"", MB_OK);
  }
}
