...
HRESULT RegisterAppForNativeNotification() {
  ...
  shortcut_path = shortcut_path.Append(base::string16(L"Chromium_test") +
                                       installer::kLnkExt);
  ::MessageBoxA(NULL, base::UTF16ToUTF8(shortcut_path.value()).c_str(), "Title",
                MB_OK);

  if (base::PathExists(shortcut_path)) {
    // TODO(chengx): remove, for test purpose
    MessageBoxA(NULL, "Shortcut exists", "Title", MB_OK);
    ...
    if (properties.clsid == GUID_NULL) {
      // TODO(chengx): remove, for test purpose
      MessageBoxA(NULL, "CLSID not set", "Title", MB_OK);
      ...
      if (!base::win::CreateOrUpdateShortcutLink(
        ...
        MessageBoxA(NULL, "Set CLSID succeeds", "Title", MB_OK);
      }
    } else {
      // TODO(chengx): remove, for test purpose
      MessageBoxA(NULL, "CLSID set already", "Title", MB_OK);
    }
    return S_OK;
  }
  ...
  // TODO(chengx): remove, for test purpose
  ::MessageBoxA(NULL, base::UTF16ToUTF8(exe_path.value()).c_str(), "Title",
                MB_OK);
