...
_Use_decl_annotations_ HRESULT RegisterComServer(PCWSTR exe_path) {
  ...
  // not running.
  if (key.WriteValue(
          nullptr, reinterpret_cast<const BYTE*>(exe_path),
...
// shortcut and have it installed on the Start menu. An AppUserModelID must be
// set on that shortcut. In addition, it needs to register a COM server with the
...
HRESULT RegisterAppForNativeNotification() {
  ...
  // This code snippet below doesn't work in local-buit Chrome. It returns
  // "Unable to find resource id" as in L60 in GetLocalizedString() in
  // l10n_string_util.cc
  //
  // shortcut_path =
  ...
  // ::MessageBoxW(NULL, shortcut_path.value().c_str(), L"Title", MB_OK);
...
HRESULT RegisterActivator() {
  // These cause linker errors if not including runtimeobject.lib
