...
#include "chrome/app/profiler/scoped_stack_sampling_profiler.h"
...
ScopedStackSamplingProfiler::ScopedStackSamplingProfiler() {
...
ScopedStackSamplingProfiler::~ScopedStackSamplingProfiler() {
  ...
  // it ensures program correctness on shutdown. Without it, there can be a race
  // condition where the sampling thread attempts to profile the sampled thread
  // after the sampled thread has already been shut down.
}
