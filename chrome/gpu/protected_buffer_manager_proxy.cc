...
void GpuArcProtectedBufferManagerProxy::GetProtectedSharedMemoryFromHandle(
    ...
    const GetProtectedSharedMemoryFromHandleCallback& callback) {
  base::ScopedFD unwrapped_handle =
      UnwrapFdFromMojoHandle(std::move(dummy_handle));
  base::ScopedFD shmem_fd =
      protected_buffer_manager_->GetProtectedSharedMemoryFromFd(
          std::move(unwrapped_handle));
