...
ChromeArcVideoDecodeAccelerator::OutputBufferInfo::~OutputBufferInfo() {
  if (!handle.is_null())
    gfx::CloseGpuMemoryBufferHandle(&handle);
}
...
bool ChromeArcVideoDecodeAccelerator::AllocateProtectedBuffer(
    ...
    size_t size) {
  size = base::bits::Align(size, base::SysInfo::VMAllocationGranularity());
  VLOGF(2) << "port=" << port << " index=" << index
           << " handle=" << handle_fd.get() << " size (aligned)=" << size;
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  ...
  if (port == PORT_INPUT) {
    ...
    std::unique_ptr<InputBufferInfo> input_info(new InputBufferInfo());
    input_info->shm_handle = protected_shmem->GetHandle();
    input_info->protected_shmem = std::move(protected_shmem);
    ...
    std::unique_ptr<OutputBufferInfo> output_info(new OutputBufferInfo());
    output_info->handle = protected_pixmap->DuplicateGpuMemoryBufferHandle();
    output_info->protected_pixmap = std::move(protected_pixmap);
...
void ChromeArcVideoDecodeAccelerator::BindDmabuf(
  ...
  std::unique_ptr<OutputBufferInfo> output_info(new OutputBufferInfo());
  output_info->handle.type = gfx::NATIVE_PIXMAP;
  output_info->handle.native_pixmap_handle.fds.emplace_back(
      base::FileDescriptor(dmabuf_fd.release(), true));
  for (const auto& plane : planes) {
    output_info->handle.native_pixmap_handle.planes.emplace_back(
        plane.stride, plane.offset, 0, 0);
  }
  buffers_pending_import_[index] = std::move(output_info);
#else
  ...
  switch (port) {
    case PORT_INPUT: {
      ...
      base::SharedMemoryHandle duplicated_handle =
          input_info->shm_handle.Duplicate();
    ...
    case PORT_OUTPUT: {
      ...
      if (!output_info->handle.is_null()) {
        vda_->ImportBufferForPicture(index, output_info->handle);
        // VDA takes ownership, so just clear out, don't close the handle.
        output_info->handle = gfx::GpuMemoryBufferHandle();
      } else {
