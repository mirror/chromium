...
void ChromeArcVideoDecodeAccelerator::Decode(
    chromeos::arc::BitstreamBuffer&& bitstream_buffer) {
  DVLOGF(2);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  if (!vda_) {
...
void ChromeArcVideoDecodeAccelerator::AssignPictureBuffers(uint32_t count) {
  DVLOGF(2);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  if (!vda_) {
    VLOGF(1) << "VDA not initialized.";
...
void ChromeArcVideoDecodeAccelerator::ImportBufferForPicture(
    int32_t picture_id,
...
void ChromeArcVideoDecodeAccelerator::ReusePictureBuffer(int32_t picture_id) {
  DVLOGF(2);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  if (!vda_) {
    VLOGF(1) << "VDA not initialized.";
