...
void ChromeArcVideoDecodeAcceleratorNew::Decode(
  ...
  media::BitstreamBuffer media_bitstream_buffer(
      bitstream_buffer.bitstream_id,
      base::SharedMemoryHandle(
          base::FileDescriptor(bitstream_buffer.ashmem_fd.get(), true), 0u,
          guid),
      bitstream_buffer.bytes_used, bitstream_buffer.offset);
  vda_->Decode(media_bitstream_buffer);
}
...
void ChromeArcVideoDecodeAcceleratorNew::AssignPictureBuffers(uint32_t count) {
  ...
  for (uint32_t id = 0; id < count; ++id) {
    ...
    buffers.emplace_back(base::checked_cast<int32_t>(id), coded_size_);
  }
...
void ChromeArcVideoDecodeAcceleratorNew::ImportBufferForPicture(
  ...
  handle.native_pixmap_handle.fds.emplace_back(ashmem_fd.get(), true);
  for (const auto& plane : planes) {
