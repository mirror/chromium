...
void GpuArcVideoDecodeAccelerator::ProvidePictureBuffers(
  ...
  ::arc::mojom::HalPixelFormat pixel_format;
  switch (format) {
    ...
    case media::PIXEL_FORMAT_ARGB:
      pixel_format = ::arc::mojom::HalPixelFormat::HAL_PIXEL_FORMAT_BGRA_8888;
      break;
    default:
...
void GpuArcVideoDecodeAccelerator::DismissPictureBuffer(
  ...
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  // no-ops.
  // There is no need to callback DismissPictureBuffer() to container.
  // Picture buffer is dmabuf allocated in container. It will be automatically
  // deallocated when ProvidePictureBuffers() is called or VDA instance
  // in container is reset.
}
