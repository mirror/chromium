...
void GpuArcVideoDecodeAccelerator::PictureReady(const media::Picture& picture) {
  DVLOGF(4);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  auto it = reuse_buffer_callbacks_.find(picture.picture_buffer_id());
  DCHECK(it != reuse_buffer_callbacks_.end());

  it->second.Run(picture.bitstream_buffer_id(), picture.visible_rect());
  reuse_buffer_callbacks_.erase(it);
}
...
void GpuArcVideoDecodeAccelerator::NotifyFlushDone() {
  ...
  base::ResetAndReturn(&flush_callback_).Run();
}
...
void GpuArcVideoDecodeAccelerator::NotifyResetDone() {
  ...
  base::ResetAndReturn(&reset_callback_).Run();
  // Flush is canceled by Reset().
...
void GpuArcVideoDecodeAccelerator::ImportBufferForPicture(
  ...
  if (!reuse_buffer_callbacks_.emplace(picture_buffer_id, callback).second) {
