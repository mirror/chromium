...
GpuArcVideoDecodeAccelerator::PendingRequest::PendingRequest() {}

GpuArcVideoDecodeAccelerator::PendingRequest::PendingRequest(
    Op op,
    PendingCallback callback)
    : op_(op), callback_(std::move(callback)) {}

GpuArcVideoDecodeAccelerator::PendingRequest::~PendingRequest() {
  if (!callback_.is_null()) {
    VLOGF(1) << "Callback isn't called yet and return here"
             << ", op=" << static_cast<uint32_t>(op_);
    base::ResetAndReturn(&callback_)
        .Run(::arc::mojom::VideoDecodeAccelerator::Result::PLATFORM_FAILURE);
  }
  if (bitstream_buffer_.handle().IsValid()) {
    VLOGF(1) << "Handle is closed.";
...
void GpuArcVideoDecodeAccelerator::NotifyFlushDone() {
  VLOGF(2);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  if (pending_flush_callbacks_.size() == 0) {
    VLOGF(1) << "No flush callback is stored.";
    client_->NotifyError(
        ::arc::mojom::VideoDecodeAccelerator::Result::PLATFORM_FAILURE);
    return;
  }
...
void GpuArcVideoDecodeAccelerator::NotifyResetDone() {
  ...
  // When callbacks for preceding Flush()s are not called, Reset() cancels
  // these Flush()s in VDA.
  // In that case, all the pending flush callbacks are called with CANCELLED.
...
void GpuArcVideoDecodeAccelerator::ExecuteRequest(PendingRequest&& request) {
  if (!vda_) {
    VLOGF(1) << "VDA not initialized.";
    if (request.callback_)
      base::ResetAndReturn(&request.callback_)
          .Run(::arc::mojom::VideoDecodeAccelerator::Result::ILLEGAL_STATE);
    return;
  }
  if (error_state_) {
  ...
  switch (request.op_) {
    case PendingRequest::Op::DECODE:
      vda_->Decode(std::move(request.bitstream_buffer_));
      request.bitstream_buffer_ = media::BitstreamBuffer();
      break;
    case PendingRequest::Op::FLUSH:
      pending_flush_callbacks_.emplace(std::move(request.callback_));
      vda_->Flush();
      break;
    case PendingRequest::Op::RESET:
      pending_reset_callback_ = std::move(request.callback_);
      vda_->Reset();
      break;
    default:
      VLOGF(1) << "Unknown Operation, op="
               << static_cast<uint32_t>(request.op_);
      break;
  }
}
...
void GpuArcVideoDecodeAccelerator::Decode(
  ...
  if (!handle_fd.is_valid()) {
    VLOGF(1) << "Invalid file descriptor, handlefd=" << handle_fd.get();
    client_->NotifyError(
        ::arc::mojom::VideoDecodeAccelerator::Result::PLATFORM_FAILURE);
    return;
  }
  ...
  request.op_ = PendingRequest::Op::DECODE;
  if (secure_mode_) {
...
void GpuArcVideoDecodeAccelerator::AssignPictureBuffers(uint32_t count) {
  ...
  if (!base::IsValueInRangeForNumericType<int32_t>(count) ||
      count > kMaxBufferCount) {
...
void GpuArcVideoDecodeAccelerator::ImportBufferForPicture(
  ...
  if (secure_mode_) {
    ...
    if (protected_output_handles_[picture_buffer_id]) {
      VLOGF(2) << "Drop the old protected output buffer handle";
      protected_output_handles_[picture_buffer_id] = nullptr;
    }
...
void GpuArcVideoDecodeAccelerator::Flush(FlushCallback callback) {
  ...
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  ExecuteRequest(
      PendingRequest(PendingRequest::Op::FLUSH, std::move(callback)));
}
...
void GpuArcVideoDecodeAccelerator::Reset(ResetCallback callback) {
  ...
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  ExecuteRequest(
      PendingRequest(PendingRequest::Op::RESET, std::move(callback)));
}
