...
void GpuArcVideoDecodeAccelerator::Decode(
  ...
  ExecuteRequest({base::Bind(&GpuArcVideoDecodeAccelerator::DecodeReqeust,
                             base::Unretained(this),
                             base::Passed(ScopedBitstreamBuffer(
                                 std::move(media_bitstream_buffer)))),
                  PendingCallback()});
}
...
void GpuArcVideoDecodeAccelerator::AssignPictureBuffers(uint32_t count) {
  ...
  if (count + output_buffer_count_ > kMaxOutputBufferCount) {
    VLOGF(1) << "Too many output buffers requested"
             << ", count =" << count
             << ", output_buffer_count_=" << output_buffer_count_;
    client_->NotifyError(
        ::arc::mojom::VideoDecodeAccelerator::Result::INVALID_ARGUMENT);
    return;
  }
