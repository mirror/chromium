...
GpuArcVideoDecodeAccelerator::CallbackManager::callback_t
GpuArcVideoDecodeAccelerator::CallbackManager::PopCallback() {
  callback_t cb;
  if (callback_infos.empty()) {
...
GpuArcVideoDecodeAccelerator::CallbackManager::~CallbackManager() {
  while (!callback_infos.empty()) {
    auto callback_type = callback_infos.front().type;
    auto callback = PopCallback();
    switch (callback_type) {
      ...
      default:
        ...
        NOTREACHED();
        VLOGF(1) << "Not supported callback type: " << callback_type;
        callback.Run(
            ::arc::mojom::VideoDecodeAccelerator::Result::ILLEGAL_STATE);
        break;
    }
...
void GpuArcVideoDecodeAccelerator::NotifyFlushDone() {
  ...
  auto cb = callback_manager_.PopCallback();
  cb.Run(::arc::mojom::VideoDecodeAccelerator::Result::SUCCESS);
