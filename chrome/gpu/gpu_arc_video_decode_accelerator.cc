...
void GpuArcVideoDecodeAccelerator::CallbackManager::RegisterDecodeCallback(
    const DecodeCallback& callback,
    base::Closure func,
    int32_t bitstream_id) {
  VDACallback cb;
  cb.decode_callback = callback;
  callback_infos.push(
      CallbackInfo(CallbackManager::DECODE, cb, func, bitstream_id));
  if (callback_infos.size() == 1) {
    ExecuteNextFunc();
  }
}
void GpuArcVideoDecodeAccelerator::CallbackManager::RegisterResetCallback(
    const ResetCallback& callback,
    base::Closure func) {
  VDACallback cb;
  cb.reset_callback = callback;
  callback_infos.push(CallbackInfo(CallbackManager::RESET, cb, func));
  if (callback_infos.size() == 1) {
    ExecuteNextFunc();
  }
}
void GpuArcVideoDecodeAccelerator::CallbackManager::RegisterFlushCallback(
    const FlushCallback& callback,
    base::Closure func) {
  VDACallback cb;
  cb.flush_callback = callback;
  callback_infos.push(CallbackInfo(CallbackManager::FLUSH, cb, func));
  if (callback_infos.size() == 1) {
    ExecuteNextFunc();
  }
}
...
bool GpuArcVideoDecodeAccelerator::CallbackManager::IsValidCallback(
  ...
  if (callback_infos.front().type != type) {
    VLOGF(1) << "The callback in the top of callback_infos is wrong."
             << " Expected: " << callback_infos.front().type
             << " Actual: " << type;
    return false;
  }
  // In Decode case, check if processed bitstream buffer is the same as
  // one Decode processed.
  if (type == CallbackManager::DECODE &&
      id == callback_infos.front().bitstream_id) {
    VLOGF(1) << "Called NotifyEndOfBitsreamBuffer in different Decode order."
             << " Expected Bitstream id: "
             << callback_infos.front().bitstream_id
             << " Actual Bitstream id: " << id;
  }
  return true;
}

GpuArcVideoDecodeAccelerator::CallbackManager::VDACallback
GpuArcVideoDecodeAccelerator::CallbackManager::PopTopCallback() {
  VDACallback cb;
  if (callback_infos.empty()) {
    VLOGF(1) << "There is no callback which can be called.";
    return cb;
  }
  cb = std::move(callback_infos.front().callback);
  callback_infos.pop();
  return cb;
}
...
void GpuArcVideoDecodeAccelerator::NotifyEndOfBitstreamBuffer(
  ...
  DVLOGF(3);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  if (!callback_manager_.IsValidCallback(CallbackManager::DECODE,
                                         bitstream_buffer_id)) {
  ...
  auto cb = callback_manager_.PopTopCallback();
  cb.decode_callback.Run(bitstream_buffer_id);
...
void GpuArcVideoDecodeAccelerator::Decode(
  ...
  callback_manager_.RegisterDecodeCallback(
      callback,
      base::Bind(
...
void GpuArcVideoDecodeAccelerator::Flush(const FlushCallback& callback) {
  ...
  callback_manager_.RegisterFlushCallback(
      callback, base::Bind(&media::VideoDecodeAccelerator::Flush,
...
void GpuArcVideoDecodeAccelerator::Reset(const ResetCallback& callback) {
  ...
  callback_manager_.RegisterResetCallback(
      callback, base::Bind(&media::VideoDecodeAccelerator::Reset,
                           base::Unretained(vda_.get())));
}
