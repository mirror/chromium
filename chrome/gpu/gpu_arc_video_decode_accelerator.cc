...
#define DVLOGF(level) VLOG(level) << __func__ << "(): "
...
void GpuArcVideoDecodeAccelerator::NotifyError(
  ...
  if (pending_callback_)
    base::ResetAndReturn(&pending_callback_).Run(arc_err);
...
void GpuArcVideoDecodeAccelerator::RunPendingRequests() {
  ...
  while (!pending_callback_ && !pending_requests_.empty()) {
    auto&& pending_request = std::move(pending_requests_.front());
    pending_requests_.pop();
    ExecuteRequest(std::move(pending_request.first),
                   std::move(pending_request.second));
  }
}

void GpuArcVideoDecodeAccelerator::ExecuteRequest(
    base::OnceClosure&& vda_request,
  ...
  if (pending_callback_) {
    pending_requests_.emplace(std::move(vda_request), std::move(callback));
    return;
  }

  pending_callback_ = std::move(callback);
  std::move(vda_request).Run();
}
...
void GpuArcVideoDecodeAccelerator::AllocateProtectedBuffer(
  ...
  protected_input_handles.emplace(std::move(protected_shmem));
...
void GpuArcVideoDecodeAccelerator::Decode(
  ...
  if (secure_mode_) {
    ...
    media_bitstream_buffer = media::BitstreamBuffer(
  ...
  ExecuteRequest(
      base::BindOnce(&media::VideoDecodeAccelerator::Decode,
                     base::Unretained(vda_.get()), media_bitstream_buffer),
      PendingCallback());
}
...
void GpuArcVideoDecodeAccelerator::ImportBufferForPicture(
  ...
  if (picture_buffer_id < 0 ||
      static_cast<uint32_t>(picture_buffer_id) >= output_buffer_size_) {
    VLOGF(1) << "Invalid index: index=" << picture_buffer_id
  ...
  if (secure_mode_) {
    base::ScopedFD dup_handle_fd(dup(handle_fd.get()));
    ...
    if (protected_pixmap_handle.fds.size() == 0) {
      ...
      protected_pixmap_handle = protected_pixmap->native_pixmap_handle();
      protected_output_handles.emplace(std::move(protected_pixmap));
    }
...
void GpuArcVideoDecodeAccelerator::Flush(FlushCallback callback) {
  VLOGF(2);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  ExecuteRequest(base::BindOnce(&media::VideoDecodeAccelerator::Flush,
...
void GpuArcVideoDecodeAccelerator::Reset(ResetCallback callback) {
  VLOGF(2);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  ExecuteRequest(base::BindOnce(&media::VideoDecodeAccelerator::Reset,
                                base::Unretained(vda_.get())),
                 std::move(callback));
}
