...
class ScopedBitstreamBuffer {
 public:
  ScopedBitstreamBuffer(ScopedBitstreamBuffer&& bitstream_buffer_ref) {
    bitstream_buffer_ = std::move(bitstream_buffer_ref.release());
  }
  ~ScopedBitstreamBuffer() {
  ...
  explicit ScopedBitstreamBuffer(media::BitstreamBuffer&& bitstream_buffer) {
...
constexpr uint32_t kMaxOutputBufferCount = 32;
...
void GpuArcVideoDecodeAccelerator::DismissPictureBuffer(
  ...
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  if (picture_buffer_ids_.find(picture_buffer) == picture_buffer_ids_.end()) {
    VLOGF(1) << "Unexpected picture buffer, picture_buffer=" << picture_buffer;
  } else {
    picture_buffer_ids_.erase(picture_buffer);
  }
  if (secure_mode_) {
    if (protected_output_handles_.find(picture_buffer) ==
        protected_output_handles_.end()) {
      // This happens when ImportBufferForPicture() isn't called for
      // picture buffer which is allocated by AssignedPictureBuffers().
      DVLOGF(3) << "Protected output buffer for the picture buffer"
                << " isn't allocated picture buffer, picture_buffer="
                << picture_buffer;
    } else {
      protected_output_handles_.erase(picture_buffer);
    }
  }
}
...
void GpuArcVideoDecodeAccelerator::DecodeReqeust(
    ScopedBitstreamBuffer&& bitstream_buffer_ref,
    PendingCallback cb,
    media::VideoDecodeAccelerator* vda) {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  DCHECK(vda);
...
void GpuArcVideoDecodeAccelerator::Decode(
  ...
  ExecuteRequest(
      {base::Bind(
           &GpuArcVideoDecodeAccelerator::DecodeReqeust, base::Unretained(this),
           base::Passed(ScopedBitstreamBuffer(media::BitstreamBuffer(
               bitstream_buffer->bitstream_id, std::move(shm_handle),
...
void GpuArcVideoDecodeAccelerator::AssignPictureBuffers(uint32_t count) {
  ...
  if (count > kMaxOutputBufferCount ||
      count + picture_buffer_ids_.size() > kMaxOutputBufferCount) {
    VLOGF(1) << "Too many output buffers requested"
             << ", count =" << count
             << ", picture_buffer_ids.size()=" << picture_buffer_ids_.size();
    client_->NotifyError(
        ::arc::mojom::VideoDecodeAccelerator::Result::INVALID_ARGUMENT);
    return;
  }
  std::vector<media::PictureBuffer> buffers;
  for (int32_t id = 0; id < static_cast<int32_t>(kMaxOutputBufferCount); ++id) {
    if (picture_buffer_ids_.find(id) != picture_buffer_ids_.end())
      continue;

    if (secure_mode_) {
      CHECK_EQ(protected_output_handles_.find(id),
               protected_output_handles_.end());
    }
    buffers.push_back(media::PictureBuffer(id, coded_size_));
    picture_buffer_ids_.emplace(id);
  }
...
void GpuArcVideoDecodeAccelerator::ImportBufferForPicture(
  ...
  if (picture_buffer_ids_.find(picture_buffer_id) ==
      picture_buffer_ids_.end()) {
  ...
  if (secure_mode_) {
    ...
    gmb_handle.native_pixmap_handle =
        CloneHandleForIPC(protected_pixmap_handle);
  } else {
...
void GpuArcVideoDecodeAccelerator::Flush(FlushCallback callback) {
  VLOGF(2);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  ExecuteRequest({base::Bind(&GpuArcVideoDecodeAccelerator::FlushRequest,
...
void GpuArcVideoDecodeAccelerator::Reset(ResetCallback callback) {
  VLOGF(2);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  ExecuteRequest({base::Bind(&GpuArcVideoDecodeAccelerator::ResetRequest,
                             base::Unretained(this)),
                  std::move(callback)});
}
