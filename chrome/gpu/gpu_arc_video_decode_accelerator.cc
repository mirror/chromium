...
class GpuArcVideoDecodeAccelerator::ScopedBitstreamBuffer {
 public:
  ScopedBitstreamBuffer(ScopedBitstreamBuffer&& scoped_bitstream_buffer) {
    bitstream_buffer_ = std::move(scoped_bitstream_buffer.release());
  }
...
void GpuArcVideoDecodeAccelerator::NotifyFlushDone() {
  ...
  auto& pending_callback = pending_flush_callbacks_.front();
  base::ResetAndReturn(&pending_callback)
...
void GpuArcVideoDecodeAccelerator::DecodeRequest(
  ...
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  DCHECK(vda);
  vda->Decode(std::move(scoped_bitstream_buffer.release()));
}
...
void GpuArcVideoDecodeAccelerator::Decode(
  ...
  // Use Unretained(this) is safe, this callback will be able to be executed in
  // this member function, ExecuteRequest().
  // So if it is called, |this| is definitely alive.
  ExecuteRequest(
      {base::BindOnce(
...
void GpuArcVideoDecodeAccelerator::Flush(FlushCallback callback) {
  ...
  // Use Unretained(this) is safe, this callback will be able to be executed in
  // this member function, ExecuteRequest().
  // So if it is called, |this| is definitely valid.
