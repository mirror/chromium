...
void GpuArcVideoDecodeAccelerator::NotifyFlushDone() {
  DVLOGF(2);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  CHECK(flush_callback_ && !reset_callback_);
  CHECK_EQ(state_, BUSY);
...
void GpuArcVideoDecodeAccelerator::NotifyResetDone() {
  DVLOGF(2);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  CHECK(reset_callback_ && !flush_callback_);
  CHECK_EQ(state_, BUSY);
...
void GpuArcVideoDecodeAccelerator::RunNextPendingRequest() {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  CHECK_EQ(state_, DECODING);
  while (state_ != BUSY && !pending_requests_.empty()) {
    pending_requests_.front().Run();
    pending_requests_.pop();
  }
}

void GpuArcVideoDecodeAccelerator::ClearPendingCallbacks() {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  while (!pending_requests_.empty()) {
    // Because |state_| is ERROR, this stored GAVDA function
    // just run a callback.
    CHECK_EQ(state_, ERROR);
    pending_requests_.front().Run();
    pending_requests_.pop();
  }
}
...
void GpuArcVideoDecodeAccelerator::Decode(
  ...
  DecodeTask(media::BitstreamBuffer(
...
void GpuArcVideoDecodeAccelerator::DecodeTask(
  ...
  if (state_ == BUSY) {
    pending_requests_.push(base::Bind(&GpuArcVideoDecodeAccelerator::DecodeTask,
