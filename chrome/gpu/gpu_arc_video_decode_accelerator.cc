...
void GpuArcVideoDecodeAccelerator::PictureReady(const media::Picture& picture) {
  DVLOGF(4);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  auto it = import_buffer_callbacks_.find(picture.picture_buffer_id());
  DCHECK(it != import_buffer_callbacks_.end());

  auto& import_buffer_callback = it->second;
  import_buffer_callback.Run(picture.bitstream_buffer_id(),
                             picture.visible_rect());
  import_buffer_callbacks_.erase(it);
}
...
void GpuArcVideoDecodeAccelerator::NotifyEndOfBitstreamBuffer(
  ...
  DVLOGF(3);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  auto it = decode_callbacks_.find(bitstream_buffer_id);
  DCHECK(it != decode_callbacks_.end());
  auto& decode_callback = it->second;
  decode_callback.Run();
}
...
void GpuArcVideoDecodeAccelerator::Decode(
  ...
  decode_callbacks_[bitstream_buffer->bitstream_id] = callback;
...
void GpuArcVideoDecodeAccelerator::ImportBufferForPicture(
    ...
    const ImportBufferForPictureCallback& callback) {
  DVLOGF(3);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  if (!vda_) {
    VLOGF(1) << "VDA not initialized.";
...
#endif
  if (import_buffer_callbacks_.find(picture_buffer_id) !=
      import_buffer_callbacks_.end()) {
    VLOGF(1) << "The given picture buffer id already exists in callback map.";
    client_->NotifyError(
        ::arc::mojom::VideoDecodeAccelerator::Result::INVALID_ARGUMENT);
    return;
  }

  import_buffer_callbacks_[picture_buffer_id] = callback;
...
void GpuArcVideoDecodeAccelerator::ReusePictureBuffer(
    int32_t picture_buffer_id,
    const ReusePictureBufferCallback& callback) {
  DVLOGF(4);
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  if (!vda_) {
    VLOGF(1) << "VDA not initialized.";
  ...
  import_buffer_callbacks_[picture_buffer_id] =
      static_cast<ImportBufferForPictureCallback>(callback);
