...
ProtectedBufferManager::ProtectedBufferManager() : weak_factory_(this) {
  ...
  ui::OzonePlatform* platform = ui::OzonePlatform::GetInstance();
  ui::SurfaceFactoryOzone* factory = platform->GetSurfaceFactoryOzone();
  factory->SetGetProtectedNativePixmapDelegate(
      base::Bind(&ProtectedBufferManager::GetProtectedNativePixmap,
...
ProtectedBufferManager::~ProtectedBufferManager() {
  ...
  ui::SurfaceFactoryOzone* factory = platform->GetSurfaceFactoryOzone();
  factory->SetGetProtectedNativePixmapDelegate(
      ui::SurfaceFactoryOzone::GetProtectedNativePixmapCallback());
#endif
}
...
ProtectedBufferManager::GetProtectedNativePixmap(
    scoped_refptr<gfx::NativePixmap> pixmap) {
  base::AutoLock lock(buffer_map_lock_);
  const auto& iter = buffer_map_.find(pixmap->GetUniqueId());
  return iter == buffer_map_.end() ? pixmap : iter->second->GetNativePixmap();
}
#endif
