...
ProtectedBufferManager::ProtectedSharedMemory::Create(
  ...
  std::unique_ptr<ProtectedSharedMemory> protected_shmem(
      new ProtectedSharedMemory(dummy_handle));
  ...
  protected_shmem->shmem_.reset(new base::SharedMemory(shm_handle, false));
...
ProtectedBufferManager::ProtectedNativePixmap::Create(
  ...
  std::unique_ptr<ProtectedNativePixmap> protected_pixmap(
      new ProtectedNativePixmap(dummy_handle));
...
ProtectedBufferManager::ProtectedBufferManager() : weak_factory_(this) {
  ...
  ui::OzonePlatform* platform = ui::OzonePlatform::GetInstance();
  ui::SurfaceFactoryOzone* factory = platform->GetSurfaceFactoryOzone();
  factory->SetNativePixmapPlanesFromHandleDelegate(
      base::Bind(&ProtectedBufferManager::ProtectedNativePixmapPlanesFromHandle,
...
ProtectedBufferManager::~ProtectedBufferManager() {
  ...
  ui::SurfaceFactoryOzone* factory = platform->GetSurfaceFactoryOzone();
  factory->SetNativePixmapPlanesFromHandleDelegate(
      ui::SurfaceFactoryOzone::NativePixmapPlanesFromHandleCallback());
#endif
}
...
#if defined(USE_OZONE)
void ProtectedBufferManager::ProtectedNativePixmapPlanesFromHandle(
    const gfx::NativePixmapHandle& handle,
    std::vector<base::ScopedFD>* fds,
    std::vector<gfx::NativePixmapPlane>* planes) {
  // Use planes from handle if lookup below fails.
  ...
  if (!handle.fds.empty()) {
    ...
    gfx::NativePixmapHandle native_pixmap_handle =
        GetProtectedNativePixmapHandleFor(std::move(dummy_fd));
    if (!native_pixmap_handle.planes.empty()) {
      fds->clear();
      planes->clear();
