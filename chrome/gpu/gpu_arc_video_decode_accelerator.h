...
class GpuArcVideoDecodeAccelerator
 ...
 private:
  ...
  // in |pending_requests|. Otherwise, the requested VDA operation is executed.
  ...
  // Requested VDA methods are executed in these functions. These are called
  // from ExecuteRequest() with the associated callback and vda pointer.
  ...
  void DecodeReqeust(BitstreamBufferRef&& bitstream_buffer,
                     PendingCallback cb,
  ...
  // input buffers, and it is |output_buffer count_| set in
