...
class GpuArcVideoDecodeAccelerator
 ...
 private:
  ...
  // on_error_ is true, if GAVDA gets an error from VDA.
  ...
  using pending_callback_t =
      base::OnceCallback<void(VideoDecodeAccelerator::Result)>;
  static_assert(std::is_same<ResetCallback, pending_callback_t>::value,
                "The type of callbacks of Reset must be pending_callback_t");
  static_assert(std::is_same<FlushCallback, pending_callback_t>::value,
                "The type of callbacks of Flush must be pending_callback_t");
  ResetCallback pending_callback_;
  ...
  std::set<std::unique_ptr<ProtectedBufferHandle>> input_protected_buffers;
  std::set<std::unique_ptr<ProtectedBufferHandle>> output_protected_buffers;
