  ...
  pair.first->PostTask(
      ...
      base::BindOnce(
          [](SnapshotCallback cb, AllocationCountMap counts,
             ContextMap context, AddressToStringMap mapped_strings) {
            std::move(cb).Run(true, std::move(counts), std::move(context), mapped_strings);
          },
...
void AllocationTracker::OnStringMapping(
  ...
  std::string str(raw_string.data(), raw_string.size());
  std::string dest;
