...
fprintf(stderr, "========== ProfilingProcess::EnsureMojoStarted\n");
  control_invitation_ =
      mojo::edk::IncomingBrokerClientInvitation::AcceptFromCommandLine(
          mojo::edk::TransportProtocol::kLegacy);

  binding_.Bind(mojom::ProfilingControlRequest(
      control_invitation_->ExtractMessagePipe(kControlPipeName)));
}

void ProfilingProcess::AddNewSender(//mojo::ScopedHandle sender_pipe,
                                    int32_t sender_pid) {
  fprintf(stderr, "========== ProfilingProcess::AddNewSender %d\n", sender_pid);
  #if defined(OS_WIN)
  ::MessageBoxA(NULL, "ProfilingProcess::AddNewSender", "ProfilingProcess::AddNewSender", 0);
  #endif
}
