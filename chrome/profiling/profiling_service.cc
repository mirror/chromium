...
void ProfilingService::OnStart() {
  LOG(ERROR) << "Memlog Service start";
  ...
  LOG(ERROR) << "Memlog Service registered";
  registry_.AddInterface(
      base::Bind(&OnMemlogRequest, ref_factory_.get()));
  LOG(ERROR) << "Memlog Service Interface bound to registry";
}
...
void ProfilingService::OnBindInterface(
  ...
  LOG(ERROR) << "Binding new interface";
...
void ProfilingService::MaybeRequestQuitDelayed() {
  ...
  base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE,
      base::Bind(&ProfilingService::MaybeRequestQuit, weak_factory_.GetWeakPtr()),
