...
"""Given a binary, uses dpkg-shlibdeps to check that it's package dependencies
are satisfieable on all supported debian-based distros.
"""
...
def _compare_char(left, right):
  table = '~$ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz+-.:'
  left = table.find(left)
  right = table.find(right)
  assert left >= 0 and right >= 0
  return _compare_int(left, right)
...
def _compare_part(left, right):
  ...
  while i < len(left) and i < len(right):
    comp = _compare_char(left[i], right[i])
    if comp != 0:
      return comp
    i += 1
...
def _compare_component(left, right):
  i = 0
  left = _get_component_parts(left)
  right = _get_component_parts(right)
  while i < len(left) and i < len(right):
    comp = _compare_part(left[i], right[i])
    if comp != 0:
      return comp
    i += 1
  return _compare_int(len(left), len(right))

def _get_component_parts(component):
    part_is_string = True
    parts = []
    part = ''
    for c in component:
...
class DebVersion:
  ...
  # Comparison algorithm is specified in:
  ...
  def __cmp__(self, other):
    ...
    upstream_version_cmp = _compare_component(self.upstream_version,
    ...
    if self.debian_revision == None and other.debian_revision == None:
      ...
      return 1
    return _compare_component(self.debian_revision, other.debian_revision)
