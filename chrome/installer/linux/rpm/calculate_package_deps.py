...
if len(sys.argv) != 3:
  print 'Usage: %s binary_path dep_file' % sys.argv[0]
  sys.exit(1)
...
if exit_code != 0:
  ...
  sys.exit(1)
requires = stdout.rstrip().split('\n')

script_dir = os.path.dirname(os.path.abspath(__file__))
distro_package_provides = json.load(open(os.path.join(
    script_dir, 'dist-package-provides.json')))
