...
#if 0
    int mode_index = 0;
    for (; mode_index < install_static::NUM_INSTALL_MODES; ++mode_index) {
      if (mode_index == current_install_mode_index)
        continue;
      const base::string16 mode_prog_id_prefix(
          install_static::kInstallModes[mode_index].prog_id_prefix);
      // Does the current app either match this mode's ProgID or contain this
      // mode's ProgID as a prefix followed by the '.' separator for a
      // per-user install's suffix?
      if (mode_prog_id_prefix.compare(current_app) == 0 ||
          (InstallUtil::IsPerUserInstall() &&
           current_app_len > mode_prog_id_prefix.length() &&
           current_app[mode_prog_id_prefix.length()] == L'.' &&
           std::char_traits<wchar_t>::compare(mode_prog_id_prefix.c_str(),
                                              current_app,
                                              mode_prog_id_prefix.length()))) {
        // Another install mode is the default for the protocol.
        other_mode_is_default = true;
        break;
      }
    }
    if (mode_index == install_static::NUM_INSTALL_MODES) {
      // Some other app is the default for the protocol.
      return ShellUtil::NOT_DEFAULT;
    }
#endif
  }
