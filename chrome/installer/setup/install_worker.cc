...
// Adds work items to |list| to delete the old registration at
// |notification_helper_path|.
void AddUninstallNativeNotificationWorkItems(
    HKEY root,
    const base::string16& notification_helper_path,
    WorkItemList* list) {
  // Delete both 64 and 32 keys to handle 32->64 or 64->32 migration.
  list->AddDeleteRegKeyWorkItem(root, notification_helper_path,
                                KEY_WOW64_32KEY);
...
void AddNativeNotificationWorkItems(const InstallerState& installer_state,
  ...
  // Delete the old toast activator server registration to handle the case where
  // 32-bit Chrome is being updated to 64-bit, or the inverse.
  AddUninstallNativeNotificationWorkItems(root, toast_activator_reg_path, list);
  ...
  notification_helper = notification_helper.Append(kNotificationHelperExe);
