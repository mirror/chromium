...
bool AddAclToPath(const base::FilePath& path,
  ...
  for (const wchar_t* trustee : trustees) {
    ...
    converted_trustees.push_back(ATL::CSid(static_cast<SID*>(sid)));
    ::LocalFree(sid);
  }
...
void AddInstallWorkItems(const InstallationState& original_state,
  ...
  WorkItem* add_ac_acl_to_target_folder =
      install_list->AddCallbackWorkItem(base::BindRepeating(
          [](const base::FilePath& target_path,
            ...
            UMA_HISTOGRAM_BOOLEAN("Setup.Install.AddAppContainerAce", success);
            return success;
          },
          target_path));
  add_ac_acl_to_target_folder->set_best_effort(true);
  add_ac_acl_to_target_folder->set_rollback_enabled(false);
