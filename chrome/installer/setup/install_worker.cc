...
void AddNativeNotificationWorkItems(const InstallerState& installer_state,
  ...
  if (!toast_activator_reg_path.empty()) {
    ...
    list->AddSetRegValueWorkItem(
        root, toast_activator_reg_path, WorkItem::kWow64Default, L"",
        L"\"" + installer_state.target_path().Append(kChromeExe).value() +
            L"\"",
        true);

    list->AddSetRegValueWorkItem(
        root, toast_activator_reg_path, WorkItem::kWow64Default,
        L"ServerExecutable",
