...
class EquivalenceMap {
 ...
 private:
  ...
  void CreatesCandidates(const std::vector<offset_t>& old_sa,
                         const ImageIndex& old_image,
                         const ImageIndex& new_image,
                         double min_similarity);
  // Sorts candidates by their offset in new image.
  void SortByDestination();
  // Traverses stored candidates, which must be sorted by destination, and
  // prunes them to resolve overlaps and delete candidates that are not good
  // enough.
