...
class OffsetMapper {
 public:
  ...
  // |offset| is used as if it contained |offset|.
  ...
  const std::vector<Equivalence> equivalence() const { return equivalences_; }

  // Sorts |equivalences| by |src_offset| and remove all source overlaps; so a
  ...
  // of a tie, the Equivalence with minimal |src_offset|.
