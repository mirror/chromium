...
// Returns a metric capturing the similarity between content of |old_view| and
// |new_view| at an offset of |src| and |dst|, respectively. Both |src| and
// |dst| must refer to token content within |old_image| and |new_image|.
...
// Returns a metric capturing the similarity between content of |old_view| and
// |new_view| at regions described by to |equivalence|.
...
// VisitEquivalenceSeed(). Note that |equivalence| is passed by value because it
// gets modified.
...
// |old_image| and |new_image|, and returns the result. |minimum_similarity|
// describes the minimum acceptable similarity score and is used as treshold to
...
class EquivalenceMap {
 public:
  ...
  // of an Equivalence is given by |minimum_similarity|.
 ...
 private:
  void FindCandidates(const std::vector<offset_t>& old_sa,
                      const ImageIndex& old_view,
                      const ImageIndex& new_view,
                      double minimum_similarity);
  void Prune(const ImageIndex& old_view,
             const ImageIndex& new_view,
             double minimum_similarity);

  std::vector<EquivalenceCandidate> equivalences_;
