...
// with distinct offsets and distinct indices, and provides functions to:
...
class BaseLabelManager {
 public:
  ...
  static constexpr bool IsOffset(offset_t offset_or_index) {
    return offset_or_index != kUnusedIndex && !IsMarked(offset_or_index);
  }

  // Returns whether |offset_or_index| is a valid marked index.
  static constexpr bool IsMarkedIndex(offset_t offset_or_index) {
    return offset_or_index != kUnusedIndex && IsMarked(offset_or_index);
  }
  ...
  // If |offset_or_index| is a stored marked index, then returns the
  // corresponding (unmarked) offset. Otherwise, returns |offset_or_index|.
...
class OrderedLabelManager : public BaseLabelManager {
 public:
  ...
  // If |index_or_offset| is a stored offset, then returns the corresponding
  // marked index. Otherwise, returns |index_or_offset|.
