...
This data structure warrants more detailed explanation.Proposing :
    ...
    // (4) Represent matching "old" offset and "new" offsets as "old" Label and
    //     "new" Label that share a common index.
    ...
    // "unmarked". So when working with stored targets: - IsMarked()
    // distinguishes offsets (false) from indexes (true). - MarkIndex() is used
    // to encode indexes to their stored value. - UnmarkIndex() is used to
    // decode indexes to their actual value. - Target offsets are stored
    // verbatim, but they must not be marked. This
