...
status::Code MatchAll(ConstBufferView old_image,
  ...
  std::unique_ptr<EnsembleMatcher> matcher =
      std::make_unique<HeuristicEnsembleMatcher>(&out);
  if (!matcher->RunMatch(old_image, new_image)) {
    out << "RunMatch() failed.";
    return status::kStatusFatal;
  }
  out << "Found " << matcher->GetMatches().size() << " nontrivial matches and "
      << matcher->GetNumIdentical() << " identical matches." << std::endl;
