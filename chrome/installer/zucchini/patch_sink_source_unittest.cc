...
template <class T>
T TestParseMember(const std::vector<uint8_t>* buffer) {
  T value;
  BufferSource buffer_source(buffer->data(), buffer->size());
...
template <class T>
void TestInvalidParseMember(const std::vector<uint8_t>* buffer) {
  T value;
  BufferSource buffer_source(buffer->data(), buffer->size());
  EXPECT_FALSE(value.Parse(&buffer_source));
}
...
TEST(ParseSerializeElementMatchTest, Simple) {
  vec data = {
      1, 0, 0, 0,              // old_offset
      3, 0, 0, 0,              // new_offset
  ...
  ElementMatch element_match = TestParse<ElementMatch>(&data);
  ...
  TestSerialize(data, element_match);
}

TEST(ParseElementMatchTest, TooSmall) {
  vec data = {4};
  TestInvalidParse<ElementMatch>(&data);
}

TEST(SerializeElementMatchTest, ExeMismatch) {
...
TEST(SerializeElementMatchTest, TooSmall) {
...
TEST(ParseSerializeBufferTest, Simple) {
  vec data = {
      0, 0, 0, 0, 0, 0, 0, 0  // size
  };
  BufferSource buffer = TestParse<BufferSource>(&data);
  ...
  buffer = TestParse<BufferSource>(&data);
  ...
  TestInvalidParse<BufferSource>(&data);
}

TEST(SerializeBufferTest, TooSmall) {
  std::vector<uint8_t> buffer(4);
  BufferSink buffer_sink(buffer.data(), buffer.size());
...
TEST(EquivalenceSinkSourceTest, Empty) {
  ...
  EquivalenceSource equivalence_source =
      TestParseMember<EquivalenceSource>(&data);

  EXPECT_FALSE(equivalence_source.GetNext());
  EXPECT_TRUE(equivalence_source.Done());

  TestSerializeMember(data, EquivalenceSink());
}
...
TEST(EquivalenceSourceSinkTest, Normal) {
  ...
  TestSerializeMember(data, equivalence_sink);
}
...
TEST(ExtraDataSourceSinkTest, Empty) {
  ...
  ExtraDataSource extra_data_source = TestParseMember<ExtraDataSource>(&data);

  EXPECT_FALSE(extra_data_source.GetNext(2));
  EXPECT_TRUE(extra_data_source.Done());

  TestSerializeMember(data, ExtraDataSink());
}
...
TEST(ExtraDataSourceSinkTest, Normal) {
  ...
  content = {4, 5};
  extra_data_sink.PutNext({content.data(), content.size()});
  TestSerializeMember(data, extra_data_sink);
}
...
TEST(RawDeltaSourceSinkTest, Empty) {
  ...
  RawDeltaSource raw_delta_source = TestParseMember<RawDeltaSource>(&data);

  EXPECT_FALSE(raw_delta_source.GetNext());
  EXPECT_TRUE(raw_delta_source.Done());

  TestSerializeMember(data, RawDeltaSink());
}
...
TEST(RawDeltaSinkSourceSinkTest, Normal) {
  ...
  EXPECT_EQ(1, raw_delta->copy_offset);
  EXPECT_EQ(42, raw_delta->diff);

  raw_delta = raw_delta_source.GetNext();
  EXPECT_TRUE(raw_delta_source.Done());
  EXPECT_TRUE(raw_delta.has_value());
  EXPECT_EQ(3, raw_delta->copy_offset);
  ...
  TestSerializeMember(data, raw_delta_sink);
}
...
TEST(ReferenceDeltaSourceSinkTest, Empty) {
  ...
  ReferenceDeltaSource reference_delta_source =
      TestParseMember<ReferenceDeltaSource>(&data);

  EXPECT_FALSE(reference_delta_source.GetNext());
  EXPECT_TRUE(reference_delta_source.Done());

  TestSerializeMember(data, ReferenceDeltaSink());
}
...
TEST(ReferenceDeltaSourceSinkTest, Normal) {
  ...
  TestSerializeMember(data, reference_delta);
}
...
TEST(TargetSourceSinkTest, Empty) {
  ...
  TargetSource target_source = TestParseMember<TargetSource>(&data);

  EXPECT_FALSE(target_source.GetNext());
  EXPECT_TRUE(target_source.Done());

  TestSerializeMember(data, TargetSink());
}
...
TEST(TargetSourceSinkTest, Normal) {
  ...
  TestSerializeMember(data, target_sink);
}

TEST(PatchElementSourceSinkTest, Normal) {
  vec data = {
  ...
  PatchElementSource patch_element_source =
      TestParseMember<PatchElementSource>(&data);

  ElementMatch element_match = patch_element_source.element_match();
  EXPECT_EQ(kExeTypeWin32X86, element_match.old_element.exe_type);
  EXPECT_EQ(0x01, element_match.old_element.offset);
  EXPECT_EQ(0x02, element_match.old_element.length);
  EXPECT_EQ(kExeTypeWin32X86, element_match.new_element.exe_type);
  EXPECT_EQ(0x03, element_match.new_element.offset);
  EXPECT_EQ(0x04, element_match.new_element.length);

  EXPECT_EQ(3, patch_element_source.pool_count());

  EquivalenceSource equivalence_source =
      patch_element_source.GetEquivalenceSource();
  ...
  ExtraDataSource extra_data_source = patch_element_source.GetExtraDataSource();
  EXPECT_EQ(vec({0x13}), extra_data_source.extra_data());

  RawDeltaSource raw_delta_source = patch_element_source.GetRawDeltaSource();
  EXPECT_EQ(vec({0x14}), raw_delta_source.raw_delta_skip());
  EXPECT_EQ(vec({0x15}), raw_delta_source.raw_delta_diff());

  ReferenceDeltaSource reference_delta_source =
      patch_element_source.GetReferenceDeltaSource();
  EXPECT_EQ(vec({0x16}), reference_delta_source.reference_delta());

  TargetSource target_source =
      patch_element_source.GetExtraTargetSource(PoolTag(0));
  EXPECT_EQ(vec({0x17}), target_source.extra_targets());
  target_source = patch_element_source.GetExtraTargetSource(PoolTag(1));
  EXPECT_EQ(vec({}), target_source.extra_targets());
  target_source = patch_element_source.GetExtraTargetSource(PoolTag(2));
  EXPECT_EQ(vec({0x18}), target_source.extra_targets());

  PatchElementSink patch_element_sink(element_match);

  patch_element_sink.SetEquivalenceSink(
      EquivalenceSink({0x10}, {0x11}, {0x12}));
  patch_element_sink.SetExtraDataSink(ExtraDataSink({0x13}));
  patch_element_sink.SetRawDeltaSink(RawDeltaSink({0x14}, {0x15}));
  patch_element_sink.SetReferenceDeltaSink(ReferenceDeltaSink({0x16}));
  patch_element_sink.SetTargetSink(PoolTag(0), TargetSink({0x17}));
  patch_element_sink.SetTargetSink(PoolTag(2), TargetSink({0x18}));
  TestSerializeMember(data, patch_element_sink);
}

TEST(EnsemblePatchSourceSinkTest, RawPatch) {
  vec data = {
  ...
  EnsemblePatchSource ensemble_patch_source =
      TestParseMember<EnsemblePatchSource>(&data);

  PatchHeader header = ensemble_patch_source.header();
  EXPECT_EQ(header.magic, PatchHeader::kMagic);
  EXPECT_EQ(header.patch_type, PatchType::kRawPatch);
  EXPECT_EQ(header.old_size, 0x76543210);
  EXPECT_EQ(header.old_crc, 0x33221100);
  EXPECT_EQ(header.new_size, 0xFEDCBA98);
  EXPECT_EQ(header.new_crc, 0x77665544);

  std::vector<PatchElementSource> elements = ensemble_patch_source.elements();
  EXPECT_EQ(size_t(1), elements.size());

  EnsemblePatchSink ensemble_patch_sink(header);
  ensemble_patch_sink.AddElement(PatchElementSink(elements[0].element_match()));

  TestSerializeMember(data, ensemble_patch_sink);
}

TEST(EnsemblePatchSourceSinkTest, InvalidMagic) {
  vec data = {
  ...
  TestInvalidParseMember<EnsemblePatchSource>(&data);
}
