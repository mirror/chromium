...
TEST(PatchElementSourceSinkTest, Normal) {
  ...
  TargetSource target_source2 =
      patch_element_source.GetExtraTargetSource(PoolTag(1));
  EXPECT_EQ(vec({}), target_source2.extra_targets());
  TargetSource target_source3 =
      patch_element_source.GetExtraTargetSource(PoolTag(2));
...
TEST(EnsemblePatchSourceSinkTest, RawPatch) {
  ...
  EXPECT_EQ(ensemble_patch_source.patch_type(), PatchType::kRawPatch);
  ...
  ensemble_patch_sink.SetPatchType(PatchType::kRawPatch);
