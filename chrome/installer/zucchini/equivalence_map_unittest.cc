...
TEST(EquivalenceMapTest, OffsetMapper) {
  EXPECT_EQ(std::vector<Equivalence>(),
            OffsetMapper(EquivalenceMap()).equivalence());
  EXPECT_EQ(std::vector<Equivalence>({{0, 0, 1}}),
            OffsetMapper(EquivalenceMap({{{0, 0, 1}, 1.0}})).equivalence());
  EXPECT_EQ(std::vector<Equivalence>({{0, 0, 1}, {1, 1, 1}}),
            OffsetMapper(EquivalenceMap({{{0, 0, 1}, 1.0}, {{1, 1, 1}, 1.0}}))
                .equivalence());
  EXPECT_EQ(std::vector<Equivalence>({{0, 0, 2}, {2, 2, 0}}),
            OffsetMapper(EquivalenceMap({{{0, 0, 2}, 1.0}, {{1, 1, 1}, 1.0}}))
                .equivalence());
  EXPECT_EQ(std::vector<Equivalence>({{0, 0, 2}, {2, 2, 1}}),
            OffsetMapper(EquivalenceMap({{{0, 0, 2}, 1.0}, {{1, 1, 2}, 1.0}}))
                .equivalence());
  EXPECT_EQ(std::vector<Equivalence>({{0, 0, 1}, {1, 1, 3}}),
            OffsetMapper(EquivalenceMap({{{0, 0, 2}, 1.0}, {{1, 1, 3}, 1.0}}))
                .equivalence());
  EXPECT_EQ(
      std::vector<Equivalence>({{0, 0, 3}, {3, 3, 0}, {3, 3, 2}}),
      OffsetMapper(EquivalenceMap(
                       {{{0, 0, 3}, 1.0}, {{1, 1, 3}, 1.0}, {{3, 3, 2}, 1.0}}))
          .equivalence());  // Pruning is greedy
}
