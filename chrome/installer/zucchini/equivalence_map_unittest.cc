...
std::vector<TargetsAffinity> MakeTargetsAffinitiesForTesting(
  ...
  for (const auto& old_target_pool : old_image_index.target_pools()) {
    target_affinities[old_target_pool.first.value()].InferFromSimilarities(
        ...
        new_image_index.at(old_target_pool.first).targets());
  }
...
TEST(EquivalenceMapTest, GetTokenSimilarity) {
  ...
  // Raw match.
  EXPECT_LT(0.0,
            GetTokenSimilarity(image_index, image_index, affinities, 0, 0));
  // Raw mismatch.
  EXPECT_GT(0.0,
            GetTokenSimilarity(image_index, image_index, affinities, 0, 1));
  EXPECT_GT(0.0,
            GetTokenSimilarity(image_index, image_index, affinities, 1, 0));

  // Type mismatch.
  EXPECT_EQ(kMismatchFatal,
            GetTokenSimilarity(image_index, image_index, affinities, 0, 2));
  EXPECT_EQ(kMismatchFatal,
            GetTokenSimilarity(image_index, image_index, affinities, 2, 0));
  EXPECT_EQ(kMismatchFatal,
            GetTokenSimilarity(image_index, image_index, affinities, 2, 10));
  EXPECT_EQ(kMismatchFatal,
            GetTokenSimilarity(image_index, image_index, affinities, 10, 2));

  // Reference strong match.
  EXPECT_LT(0.0,
            GetTokenSimilarity(image_index, image_index, affinities, 2, 2));
  EXPECT_LT(0.0,
            GetTokenSimilarity(image_index, image_index, affinities, 4, 6));

  // Reference weak match.
  EXPECT_LT(0.0,
            GetTokenSimilarity(image_index, image_index, affinities, 6, 4));
  EXPECT_LT(0.0,
            GetTokenSimilarity(image_index, image_index, affinities, 4, 8));
  EXPECT_LT(0.0,
            GetTokenSimilarity(image_index, image_index, affinities, 8, 4));

  // Weak match is not greater than strong match.
  EXPECT_LE(GetTokenSimilarity(image_index, image_index, affinities, 6, 4),
            GetTokenSimilarity(image_index, image_index, affinities, 2, 2));

  // Reference mismatch.
  EXPECT_GT(0.0,
            GetTokenSimilarity(image_index, image_index, affinities, 2, 4));
  EXPECT_GT(0.0,
            GetTokenSimilarity(image_index, image_index, affinities, 2, 6));
}
...
TEST(EquivalenceMapTest, Build) {
  auto test_build_equivalence = [](const ImageIndex old_index,
    ...
    for (const auto& old_target_pool : old_index.target_pools()) {
      ...
      old_view.UpdateLabels(old_target_pool.first, std::move(old_labels),
                            label_bound);
      new_view.UpdateLabels(old_target_pool.first, std::move(new_labels),
                            label_bound);
    }
