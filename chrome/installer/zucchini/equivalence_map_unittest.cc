...
ImageIndex MakeImageIndex(const char* a,
  ...
  std::vector<ReferenceTypeTraits> traits(
      {ReferenceTypeTraits{2, TypeTag(0), PoolTag(0)},
       ReferenceTypeTraits{2, TypeTag(1), PoolTag(0)}});
...
TEST(EquivalenceMapTest, GetTokenSimilarity) {
  ImageIndex old_index = MakeImageIndex(
      "ab1122334455", {{2, MarkIndex(0)}, {4, MarkIndex(1)}, {6, 2}, {8, 2}},
      {{10, 3}});
  ImageIndex new_index = MakeImageIndex(
      "ab1122334455",
...
TEST(EquivalenceMapTest, GetEquivalenceSimilarity) {
  ImageIndex image_index = MakeImageIndex("abcdef1122", {{6, 0}}, {{8, 1}});
...
TEST(EquivalenceMapTest, ExtendEquivalenceForward) {
  auto test_extend_forward =
      [](const ImageIndex old_index, const ImageIndex new_index,
        ...
        return Equivalence(ExtendEquivalenceForward(
            old_index, new_index, equivalence, base_similarity));
      };

  EXPECT_EQ(
      Equivalence({0, 0, 0}),
      test_extend_forward(MakeImageIndex("", {}, {}),
                          MakeImageIndex("", {}, {}), {0, 0, 0, 0.0}, 8.0));

  EXPECT_EQ(Equivalence({0, 0, 6}),
            test_extend_forward(MakeImageIndex("banana", {}, {}),
                                MakeImageIndex("banana", {}, {}),
                                {0, 0, 0, 0.0}, 8.0));
  ...
  EXPECT_EQ(Equivalence({0, 0, 6}),
            test_extend_forward(MakeImageIndex("bananaxx", {}, {}),
                                MakeImageIndex("bananayy", {}, {}),
                                {0, 0, 0, 0.0}, 8.0));

  EXPECT_EQ(Equivalence({0, 0, 8}),
            test_extend_forward(MakeImageIndex("banana11", {{6, 0}}, {}),
                                MakeImageIndex("banana11", {{6, 0}}, {}),
                                {0, 0, 0, 0.0}, 8.0));

  EXPECT_EQ(Equivalence({0, 0, 6}),
            test_extend_forward(MakeImageIndex("banana11", {{6, 0}}, {}),
                                MakeImageIndex("banana22", {}, {{6, 0}}),
                                {0, 0, 0, 0.0}, 8.0));

  EXPECT_EQ(Equivalence({0, 0, 17}),
            test_extend_forward(MakeImageIndex("bananaxxpineapple", {}, {}),
                                MakeImageIndex("bananayypineapple", {}, {}),
                                {0, 0, 0, 0.0}, 8.0));

  EXPECT_EQ(Equivalence({3, 0, 19}),
            test_extend_forward(
                MakeImageIndex("foobanana11xxpineapplexx", {{9, 0}}, {}),
                MakeImageIndex("banana11yypineappleyy", {{6, 0}}, {}),
                {3, 0, 0, 0.0}, 8.0));
}
...
TEST(EquivalenceMapTest, ExtendEquivalenceBackward) {
  auto test_extend_backward =
      [](const ImageIndex old_index, const ImageIndex new_index,
         const EquivalenceCandidate& equivalence, double base_similarity) {
        return Equivalence(ExtendEquivalenceBackward(
            old_index, new_index, equivalence, base_similarity));
      };

  EXPECT_EQ(
      Equivalence({0, 0, 0}),
      test_extend_backward(MakeImageIndex("", {}, {}),
                           MakeImageIndex("", {}, {}), {0, 0, 0, 0.0}, 8.0));

  EXPECT_EQ(Equivalence({0, 0, 6}),
            test_extend_backward(MakeImageIndex("banana", {}, {}),
                                 MakeImageIndex("banana", {}, {}),
                                 {6, 6, 0, 0.0}, 8.0));

  EXPECT_EQ(Equivalence({2, 2, 6}),
            test_extend_backward(MakeImageIndex("xxbanana", {}, {}),
                                 MakeImageIndex("yybanana", {}, {}),
                                 {8, 8, 0, 0.0}, 8.0));

  EXPECT_EQ(Equivalence({0, 0, 8}),
            test_extend_backward(MakeImageIndex("11banana", {{0, 0}}, {}),
                                 MakeImageIndex("11banana", {{0, 0}}, {}),
                                 {8, 8, 0, 0.0}, 8.0));

  EXPECT_EQ(Equivalence({2, 2, 6}),
            test_extend_backward(MakeImageIndex("11banana", {{0, 0}}, {}),
                                 MakeImageIndex("22banana", {}, {{0, 0}}),
                                 {8, 8, 0, 0.0}, 8.0));

  EXPECT_EQ(Equivalence({0, 0, 17}),
            test_extend_backward(MakeImageIndex("bananaxxpineapple", {}, {}),
                                 MakeImageIndex("bananayypineapple", {}, {}),
                                 {8, 8, 9, 9.0}, 8.0));
...
TEST(EquivalenceMapTest, Build) {
  auto test_build_equivalence = [](const ImageIndex old_index,
    ...
    offset_t coverage = 0;
    for (const auto& equivalence : equivalence_map) {
      EXPECT_GE(equivalence.dst_offset, current_dst_offset);
      EXPECT_GT(equivalence.length, offset_t(0));
      EXPECT_LE(equivalence.src_offset + equivalence.length, old_index.size());
      EXPECT_LE(equivalence.dst_offset + equivalence.length, new_index.size());
      EXPECT_GE(equivalence.similarity, minimum_similarity);
      current_dst_offset = equivalence.dst_offset;
      coverage += equivalence.length;
    }
    return coverage;
  };

  EXPECT_EQ(0U, test_build_equivalence(MakeImageIndex("", {}, {}),
                                       MakeImageIndex("", {}, {}), 8.0));

  EXPECT_EQ(6U, test_build_equivalence(MakeImageIndex("banana", {}, {}),
                                       MakeImageIndex("banana", {}, {}), 4.0));

  EXPECT_EQ(6U,
            test_build_equivalence(MakeImageIndex("bananaxx", {}, {}),
                                   MakeImageIndex("bananayy", {}, {}), 4.0));

  EXPECT_EQ(8U, test_build_equivalence(MakeImageIndex("banana11", {{6, 0}}, {}),
                                       MakeImageIndex("banana11", {{6, 0}}, {}),
                                       4.0));

  EXPECT_EQ(6U, test_build_equivalence(MakeImageIndex("banana11", {{6, 0}}, {}),
                                       MakeImageIndex("banana22", {}, {{6, 0}}),
                                       4.0));

  EXPECT_EQ(15U, test_build_equivalence(
                     MakeImageIndex("banana11pineapple", {{6, 0}}, {}),
                     MakeImageIndex("banana22pineapple", {}, {{6, 0}}), 4.0));

  EXPECT_EQ(15U, test_build_equivalence(
                     MakeImageIndex("bananaxxxxxxxxpineapple", {}, {}),
                     MakeImageIndex("bananayyyyyyyypineapple", {}, {}), 4.0));

  EXPECT_EQ(19U,
            test_build_equivalence(
                MakeImageIndex("foobanana11xxpineapplexx", {{9, 0}}, {}),
                MakeImageIndex("banana11yypineappleyy", {{6, 0}}, {}), 4.0));
}
