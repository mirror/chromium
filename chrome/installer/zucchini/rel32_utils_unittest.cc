...
class TestAddressTranslator : public AddressTranslator {
 public:
  ...
  std::unique_ptr<RVAToOffsetTranslator> GetRVAToOffsetTranslator()
      const override {
    class RVAToOffsetImpl : RVAToOffsetTranslator {
      const std::ptrdiff_t offset_to_rva_adjust_;

     public:
      explicit RVAToOffsetImpl(std::ptrdiff_t offset_to_rva_adjust)
          : offset_to_rva_adjust_(offset_to_rva_adjust) {}

      offset_t RVAToOffset(rva_t rva) override {
        return offset_t(rva - offset_to_rva_adjust_);
      }
    };

    return std::unique_ptr<RVAToOffsetTranslator>(
        new RVAToOffsetImpl(offset_to_rva_adjust_));
  }

  std::unique_ptr<OffsetToRVATranslator> GetOffsetToRVATranslator()
      const override {
    class OffsetToRVAImpl : OffsetToRVATranslator {
      const std::ptrdiff_t offset_to_rva_adjust_;

     public:
      explicit OffsetToRVAImpl(std::ptrdiff_t offset_to_rva_adjust)
          : offset_to_rva_adjust_(offset_to_rva_adjust) {}

      rva_t OffsetToRVA(offset_t offset) override {
        return rva_t(offset + offset_to_rva_adjust_);
      }
    };

    return std::unique_ptr<OffsetToRVATranslator>(
        new OffsetToRVAImpl(offset_to_rva_adjust_));
  }
...
TEST(Rel32UtilsTest, Rel32ReaderX86) {
  ...
  TestAddressTranslator translator(kTestOffsetToRVAAdjust);
  std::vector<uint8_t> bytes = {
      0xFF, 0xFF, 0xFF, 0xFF,  // 00030000: (Filler)
      0x00, 0x00, 0x00, 0x80,  // 00030004: jmp  80030008 Marked.
      0x04, 0x00, 0x00, 0x00,  // 00030008: jmp  00030010
      0xFF, 0xFF, 0xFF, 0xFF,  // 0003000C: (Filler)
      0x00, 0x00, 0x00, 0x00,  // 00030010: jmp  00030014
      0xFF, 0xFF, 0xFF, 0xFF,  // 00030014: (Filler)
      0xF4, 0xFF, 0xFF, 0xFF,  // 00030018: jmp  00030010
      0xE4, 0xFF, 0xFF, 0xFF,  // 0003001C: jmp  00030004
  };
  ConstBufferView buffer(bytes.data(), bytes.size());
  // Specify rel32 locations directly, instead of parsing.
  std::vector<offset_t> rel32_locations = {0x0004U, 0x0008U, 0x0010U, 0x0018U,
                                           0x001CU};

  // Generate everything.
  Rel32ReaderX86 reader1(buffer, translator, rel32_locations, 0x0000U, 0x0020U);
  ...
  // Exclude last.
  Rel32ReaderX86 reader2(buffer, translator, rel32_locations, 0x0000U, 0x001CU);
  CheckReader({{0x0008U, 0x0010U}, {0x0010U, 0x0014U}, {0x0018U, 0x0010U}},
              &reader2);

  // Only find one.
  Rel32ReaderX86 reader3(buffer, translator, rel32_locations, 0x000CU, 0x0018U);
...
TEST(Rel32UtilsTest, Rel32WriterX86) {
  ...
  EXPECT_EQ(0x00000004, buffer.Read<uint32_t>(0x08));  // 00030008: jmp 00030010

  writer.PutNext({0x0010U, 0x0014U});
  EXPECT_EQ(0x00000000, buffer.Read<uint32_t>(0x10));  // 00030010: jmp 00030014

  writer.PutNext({0x0018U, 0x0010U});
  EXPECT_EQ(0xFFFFFFF4, buffer.Read<uint32_t>(0x18));  // 00030018: jmp 00030010

  writer.PutNext({0x001CU, 0x0004U});
  EXPECT_EQ(0xFFFFFFE4, buffer.Read<uint32_t>(0x1C));  // 0003001C: jmp 00030004
