...
class TestAddressTranslator : public AddressTranslator {
 public:
  explicit TestAddressTranslator(offset_t image_size,
                                 ptrdiff_t offset_to_rva_adjust) {
    DCHECK_GE(offset_to_rva_adjust, 0);
    AddressTranslatorBuilder builder;
    CHECK_EQ(AddressTranslatorBuilder::kSuccess,
             builder.AddUnit(0, image_size, offset_to_rva_adjust, image_size));
    CHECK_EQ(AddressTranslatorBuilder::kSuccess, builder.Build(this));
  }
};
...
TEST(Rel32UtilsTest, Rel32ReaderX86) {
  constexpr ptrdiff_t kTestImageSize = 0x00100000U;
  constexpr ptrdiff_t kTestOffsetToRVAAdjust = 0x00030000U;
  TestAddressTranslator trans(kTestImageSize, kTestOffsetToRVAAdjust);
  ...
  // Generate everything.
  Rel32ReaderX86 reader1(buffer, 0x0000U, 0x0020U, &rel32_locations, trans);
  ...
  // Exclude last.
  Rel32ReaderX86 reader2(buffer, 0x0000U, 0x001CU, &rel32_locations, trans);
  CheckReader({{0x0008U, 0x0010U}, {0x0010U, 0x0014U}, {0x0018U, 0x0010U}},
              &reader2);

  // Only find one.
  Rel32ReaderX86 reader3(buffer, 0x000CU, 0x0018U, &rel32_locations, trans);
...
TEST(Rel32UtilsTest, Rel32WriterX86) {
  ...
  Rel32WriterX86 writer(buffer, trans);
