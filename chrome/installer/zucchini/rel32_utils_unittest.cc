...
class TestAddressTranslator : public AddressTranslatorFactory {
 public:
  explicit TestAddressTranslator(ptrdiff_t offset_to_rva_adjust)
  ...
  std::unique_ptr<RVAToOffsetTranslator> MakeRVAToOffsetTranslator()
      const override {
    class RVAToOffset : RVAToOffsetTranslator {
...
TEST(Rel32UtilsTest, Rel32ReaderX86) {
  constexpr ptrdiff_t kTestOffsetToRVAAdjust = 0x30000;
  TestAddressTranslator factory(kTestOffsetToRVAAdjust);
  ...
  std::vector<uint8_t> bytes = {
      ...
      0x00, 0x00, 0x00, 0x80,  // 00030004: 80030008 Marked.
...
TEST(Rel32UtilsTest, Rel32WriterX86) {
  ...
  EXPECT_EQ(0x00000004, buffer.Read<uint32_t>(0x08));  // 00030008: 00030010

  writer.PutNext({0x0010U, 0x0014U});
  EXPECT_EQ(0x00000000, buffer.Read<uint32_t>(0x10));  // 00030010: 00030014

  writer.PutNext({0x0018U, 0x0010U});
  EXPECT_EQ(0xFFFFFFF4, buffer.Read<uint32_t>(0x18));  // 00030018: 00030010

  writer.PutNext({0x001CU, 0x0004U});
  EXPECT_EQ(0xFFFFFFE4, buffer.Read<uint32_t>(0x1C));  // 0003001C: 00030004
