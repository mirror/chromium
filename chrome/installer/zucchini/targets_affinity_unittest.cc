...
TEST(TargetsAffinityTest, AssignLabels) {
  ...
  auto test_labels_assignement =
      [&targets_affinity](const EquivalenceMap& equivalence_map,
        ...
        EXPECT_TRUE(
            std::all_of(old_labels.begin(), old_labels.begin(),
                        [bound](offset_t value) { return value < bound; }));
        EXPECT_TRUE(
            std::all_of(new_labels.begin(), new_labels.begin(),
                        [bound](offset_t value) { return value < bound; }));
      };

  test_labels_assignement(EquivalenceMap(), {}, {}, 1.0, {}, {});
  test_labels_assignement(EquivalenceMap({{{0, 0, 8}, 1.0}}), {}, {}, 1.0, {},
                          {});

  test_labels_assignement(EquivalenceMap(), {0, 10}, {0, 5}, 1.0, {0, 0},
                          {0, 0});
  ...
  test_labels_assignement(EquivalenceMap({{{0, 0, 1}, 1.0}}), {0, 10}, {0, 5},
                          15.0, {0, 0}, {0, 0});
  test_labels_assignement(EquivalenceMap({{{0, 0, 1}, 15.0}}), {0, 10}, {0, 5},
                          15.0, {1, 0}, {1, 0});
