...
TEST_F(TargetsAffinityTest, InferFromSimilarities) {
  affinity_.InferFromSimilarities(EquivalenceMap(), {0, 5, 10}, {0, 5});
  ...
  affinity_.InferFromSimilarities(EquivalenceMap({{0, 0, 8, 1.0}}), {0, 5, 10},
                                  {0, 5});
  ...
  affinity_.InferFromSimilarities(
      EquivalenceMap({{5, 0, 2, 1.0}, {0, 5, 2, 2.0}}), {0, 5, 10}, {0, 5});
  ...
  affinity_.InferFromSimilarities(
      EquivalenceMap({{0, 0, 2, 1.0}, {0, 5, 2, 2.0}}), {0, 5, 10}, {0, 5});
  EXPECT_EQ(-2.0, affinity_.AffinityBetween(0, 0));
  EXPECT_EQ(2.0, affinity_.AffinityBetween(0, 1));
}
