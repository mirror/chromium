...
MappedFileWriter::MappedFileWriter(const base::FilePath& file_path,
  ...
  delete_behavior_ = file_handle_.DeleteOnClose(true) ? kAutoDeleteOnClose
                                                      : kManualDeleteOnClose;
  LOG_IF(WARNING, delete_behavior_ != kAutoDeleteOnClose)
      << "Failed to mark file for delete-on-close: " << file_path_.value();
...
MappedFileWriter::~MappedFileWriter() {
  if (delete_behavior_ == kManualDeleteOnClose &&
