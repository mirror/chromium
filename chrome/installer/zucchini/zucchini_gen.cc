  ...
  for (TypeTag type_tag : old_index.GetTypeTags(pool_tag)) {
    ...
    for (const auto& candidate : similarity_map) {
      ...
      for (; dst_ref != dst_refs.end() &&
        ...
        offset_t old_offset =
            old_index.GetTargetPool(pool_tag).GetOffsetForKey(src_ref->target);
        offset_t new_expected_offset =
            equivalence_map.ProjectOffset(old_offset);
        offset_t new_expected_key =
            target_pool.FindKeyForOffset(new_expected_offset);
        offset_t new_offset =
            new_index.GetTargetPool(pool_tag).GetOffsetForKey(dst_ref->target);
...
bool GenerateExecutableElement(ExecutableType exe_type,
  ...
  SimilarityMap similarity_map =
      CreateSimilarityMap(old_image_index, new_image_index);
  EquivalenceMap equivalence_map(similarity_map);

  for (uint8_t pool = 0; pool < old_image_index.PoolCount(); ++pool) {
    TargetPool projected_old_targets =
        old_image_index.GetTargetPool(PoolTag(pool));
    projected_old_targets.Project(equivalence_map);
    std::vector<offset_t> extra_target = FindExtraTargets(
        projected_old_targets.targets(),
        new_image_index.GetTargetPool(PoolTag(pool)).targets());
    ...
    if (!GenerateExtraTargets(extra_target, PoolTag(pool), patch_writer) ||
        !GenerateReferencesDelta(old_image_index, new_image_index,
                                 PoolTag(pool), projected_old_targets,
