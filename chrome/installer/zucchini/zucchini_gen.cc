...
base::Optional<ImageIndex> MakeImageIndex(ConstBufferView image,
                                          Disassembler* disasm) {
  std::vector<ReferenceGroup> ref_groups = disasm->GetReferenceGroups();
...
bool GenerateReferencesDelta(const ImageIndex& old_index,
  ...
  for (uint8_t type = 0; type < old_index.TypeCount(); ++type) {
    ...
    // equivalence_map. |dst_ref| visits ordered references in the destination.
    // Wevisit all intersecting |candidate| and |dst_ref| in lockstep.
    ...
    for (auto dst_ref : new_index.GetReferences(type_tag)) {
      ...
      // Found: Now |dst_ref| and |candidate| intersect.
      // |dst_ref| should not straddle end of |candidate|.
...
bool GenerateExecutableElement(ExecutableType exe_type,
  ...
  // Find extra Labels in |new_element|, emit into patch, merge them to
  // |new_label_manager|, and update new references.
...
status::Code GenerateEnsemble(ConstBufferView old_image,
  ...
  auto old_element = DetectElementFromDisassembler(old_image);
  auto new_element = DetectElementFromDisassembler(new_image);
