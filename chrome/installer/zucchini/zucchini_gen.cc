...
EquivalenceMap CreateEquivalenceMap(
  ...
  // Equivalences are created in 2 iterations. First step finds coarse
  // equivalences used for associating targets. Second step finds refined
  // equivalence based on target associations.
  static constexpr double kEquivalenceSimilarities[] = {
      kLargeEquivalenceSimilarity, kMinEquivalenceSimilarity};
  ...
  for (size_t i = 0; i < 2; ++i) {
    ...
    std::vector<offset_t> old_sa =
        MakeSuffixArrayFromImageIndex(*old_image_index);
    equivalences.Build(old_sa, *old_image_index, *new_image_index,
                       kEquivalenceSimilarities[i]);
