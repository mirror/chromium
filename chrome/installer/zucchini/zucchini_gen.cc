...
status::Code GenerateEnsemble(ConstBufferView old_image,
  ...
  // results  so elements and "gap" results can be computed separately (to
  // reduce peak memory usage), and later, properly serialized to |patch_writer|
  ...
  for (const ElementMatch& match : matches) {
    ...
    auto it_and_success = patch_element_map.emplace(new_region.lo(), match);
  ...
  if (covered_new_bytes < new_image.size()) {
    ...
    covered_new_regions.push_back({new_image.size(), 0});

    for (const BufferRegion& covered : covered_new_regions) {
      offset_t gap_hi = covered.lo();
