  ...
  for (size_t i = 0; i < 2; ++i) {
    ...
    for (uint8_t pool = 0; pool < pool_count; ++pool) {
      target_affinities[pool].InferFromSimilarities(
          equivalence_map, old_image_index.GetTargets(PoolTag(pool)),
          new_image_index.GetTargets(PoolTag(pool)));

      std::vector<size_t> old_labels;
      std::vector<size_t> new_labels;
      size_t label_bound =
          target_affinities[pool].AssignLabels(&old_labels, &new_labels);
...
bool GenerateReferencesDelta(
  ...
  for (uint8_t type = 0; type < old_index.TypeCount(); ++type) {
    ...
    PoolTag pool_tag = old_index.GetPoolTag(type_tag);
    ...
    for (const auto& candidate : equivalence_map) {
      ...
      for (; dst_ref != dst_refs.end() &&
        ...
        offset_t dst_index = new_label_managers[pool_tag.value()].IndexOfOffset(
            new_index.GetTargetOffsetForKey(pool_tag, dst_ref->target));
...
bool GenerateExecutableElement(ExecutableType exe_type,
  ...
  for (uint8_t pool = 0; pool < old_image_index.PoolCount(); ++pool) {
    ...
    new_label_managers[pool].Init(std::move(new_labels));
    ...
    for (TypeTag type : new_image_index.GetTypeTags(PoolTag(pool))) {
      extra_label_manager.InsertOffsets(
          FindExtraTargets(new_image_index.GetReferences(type),
                           new_image_index.GetTargets(PoolTag(pool)),
                           new_label_managers[pool], equivalences));
    }
    ...
    for (offset_t offset : extra_label_manager.Labels())
      new_label_managers[pool].InsertNewOffset(offset);
  }
