...
std::vector<offset_t> FindExtraTargets(
  ...
  for (const Reference& ref : new_references) {
    ...
    if (ref.location >= equivalence->eq.dst_offset &&
        !new_label_manager.Contains(new_targets[ref.target]))
      targets.push_back(new_targets[ref.target]);
  }
  return targets;
}
...
EquivalenceMap CreateEquivalenceMap(const ImageIndex& old_image_index,
  ...
  for (size_t i = 0; i < 2; ++i) {
    ...
    for (const auto& old_target_pool : old_image_index.target_pools()) {
      ...
      size_t label_bound =
          target_affinities[old_target_pool.first.value()].AssignLabels(
...
bool GenerateReferencesDelta(
    const ImageIndex& old_index,
    const ImageIndex& new_index,
    const std::vector<UnorderedLabelManager>& new_label_managers,
    const EquivalenceMap& equivalence_map,
    PatchElementWriter* patch_writer) {
  ...
  for (uint8_t type = 0; type < old_index.TypeCount(); ++type) {
    ...
    PoolTag pool_tag = old_index.at(type_tag).pool_tag();
    size_t ref_width = old_index.at(type_tag).width();

    const ReferenceSet& src_refs = old_index.at(type_tag);
    const ReferenceSet& dst_refs = new_index.at(type_tag);
    ...
    for (const auto& candidate : equivalence_map) {
      ...
      for (; dst_ref != dst_refs.end() &&
        ...
        offset_t dst_index = new_label_managers[pool_tag.value()].IndexOfOffset(
            new_index.at(pool_tag).OffsetForKey(dst_ref->target));

        reference_delta_sink.PutNext(
            static_cast<int32_t>(dst_index - src_ref->target));
      }
      if (dst_ref == dst_refs.end())
        break;  // Done.
      // Straddle check.
...
bool GenerateExecutableElement(ExecutableType exe_type,
  ...
  for (const auto& old_target_pool : old_image_index.target_pools()) {
    ...
    for (TypeTag type : new_target_pool.types()) {
      extra_label_manager.InsertOffsets(FindExtraTargets(
          new_image_index.at(type).references(), new_target_pool.targets(),
  ...
  return GenerateEquivalencesAndExtraData(new_image, equivalences,
         ...
         GenerateReferencesDelta(old_image_index, new_image_index,
                                 new_label_managers, equivalences,
                                 patch_writer);
}
