...
constexpr size_t PADDING = kReferencePaddingRank;
 ...
 protected:
  ...
  void CheckView(std::vector<size_t> expected) const {
    for (offset_t i = 0; i < image_index_.size(); ++i) {
      EXPECT_EQ(expected[i], EncodedView::Rank(image_index_, i)) << i;
    }
...
TEST_F(EncodedViewTest, Unlabeled) {
  std::vector<size_t> expected = {
      0,                         // raw
      kBaseRefRank + 0 + 0 * 3,  // ref 0
      PADDING,
      kBaseRefRank + 1 + 0 * 3,  // ref 1
      PADDING,
      PADDING,
      PADDING,
      7,                         // raw
  ...
  EXPECT_EQ(kBaseRefRank + 3 * 1, encoded_view_.Cardinality());
  CheckView(expected);
}
...
TEST_F(EncodedViewTest, Labeled) {
  ...
  std::vector<size_t> expected = {
      ...
      kBaseRefRank + 0 + 0 * 3,  // ref 0
      PADDING,
      kBaseRefRank + 1 + 2 * 3,  // ref 1
      PADDING,
      PADDING,
      PADDING,
      7,                         // raw
      kBaseRefRank + 0 + 2 * 3,  // ref 0
      PADDING,
      kBaseRefRank + 0 + 1 * 3,  // ref 0
      PADDING,
      kBaseRefRank + 2 + 0 * 3,  // ref 2
      PADDING,
      PADDING,
  ...
  EXPECT_EQ(kBaseRefRank + 3 * 3, encoded_view_.Cardinality());
  CheckView(expected);
}
