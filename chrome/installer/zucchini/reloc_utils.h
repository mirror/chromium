...
//     } units[(block_size_in_bytes - 8) / 2, which should be even];
...
class RelocRvaReaderWin32 {
 public:
  enum { kRelocUnitSize = sizeof(uint16_t) };

  // Parses |image| at |reloc_region| to find beginning offsets of each reloc
  // block. On success, returns true and writes the result to |block_offsets|.
  // Otherwise returns false, and leaves |block_offsets| in an undetermined
  // state.
...
class RelocReaderWin32 : public ReferenceReader {
 public:
  ...
  RelocReaderWin32(std::unique_ptr<RelocRvaReaderWin32> reloc_rva_reader,
                   uint16_t reloc_type,
 ...
 private:
  std::unique_ptr<RelocRvaReaderWin32> reloc_rva_reader_;
...
struct SectionDimsElf {
 public:
  ...
  explicit SectionDimsElf(Elf_Shdr* section)
      : region(BufferRegion{section->sh_offset, section->sh_size}),
