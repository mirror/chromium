...
class TargetPool {
 public:
  ...
  offset_t GetOffsetForKey(offset_t key) const { return targets_[key]; }
...
class ReferenceSet {
 public:
  ...
  void SetTraits(const ReferenceTypeTraits& traits) { traits_ = traits; }
...
class ImageIndex {
 public:
  ...
  const std::vector<offset_t>& GetTargets(PoolTag pool) const {
    DCHECK_LT(pool.value(), pools_.size());
    return pools_[pool.value()].targets();
  }
