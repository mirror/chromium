...
class ImageIndex {
 public:
  ...
  uint8_t GetRawValueAt(offset_t location) const {
    DCHECK_LT(location, size());
  ...
  const TargetPool& at(PoolTag pool_tag) const {
    return target_pools_.at(pool_tag);
  }
  const ReferenceSet& at(TypeTag type_tag) const {
