...
class ImageIndex {
 public:
  ...
  size_t PoolCount() const { return pool_types_.size(); }

  size_t LabelCount(PoolTag pool) const {
    DCHECK_LT(pool.value(), label_count_.size());
    return label_count_[pool.value()];
  }
  ...
  // Returns true if |raw_image[location]| is part of a reference.
  ...
  // Returns the reference of type |type| covering |location|, which is required
  // to is not part of a reference of type |type|.
  ...
  // Replace every target represented as offset whose label is in
  // |label_manager|  by the index of this label.
  void LabelTargets(PoolTag pool, const BaseLabelManager& label_manager);

  // Replace every target represented as offset whose label is in
  // |label_manager| by the index of this label iff this index is also used in
  ...
  // Replace every target represented as an label index by its original offset,
 ...
 private:
  ...
  // Enumerates type_tag for each pool.
  std::vector<std::vector<TypeTag>> pool_types_;
  // Number of labels for each pool.
