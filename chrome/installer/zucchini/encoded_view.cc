...
EncodedView::value_type EncodedView::Projection(const ImageIndex& image_index,
  ...
  if (location != ref.location) {
    // Trailing bytes of a reference are all encoded with the same value.
  ...
  rank += type.value();
  return rank + kBaseReferenceProjection;
}

size_t EncodedView::Cardinality() const {
  size_t max_width = 0;
  for (uint8_t pool = 0; pool < image_index_->PoolCount(); ++pool) {
    // LabelBound() + 1 for the extra case where a Reference is Unmarked.
    max_width =
