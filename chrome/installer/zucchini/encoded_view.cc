...
EncodedView::value_type EncodedView::Projection(offset_t location) const {
  ...
  IndirectReference ref = image_index_.refs(type).at(location);
  DCHECK_GE(location, ref.location);
  DCHECK_LT(location, ref.location + image_index_.refs(type).width());
  ...
  PoolTag pool_tag = image_index_.refs(type).pool_tag();
  ...
  uint32_t label = pool_infos_[image_index_.refs(type).pool_tag().value()]
                       .labels[ref.target_key];
...
size_t EncodedView::Cardinality() const {
  ...
  for (uint8_t pool = 0; pool < image_index_.PoolCount(); ++pool)
    max_width = std::max(pool_infos_[pool].bound, max_width);
