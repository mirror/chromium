...
offset_t BaseLabelManager::OffsetFromMarkedIndex(
    offset_t offset_or_index) const {
  if (IsMarkedIndex(offset_or_index)) {
    offset_t offset = OffsetOfIndex(UnmarkIndex(offset_or_index));
    if (offset != kUnusedIndex)
      return offset;
  }
  return offset_or_index;
}
...
offset_t OrderedLabelManager::MarkedIndexFromOffset(
    offset_t offset_or_index) const {
  if (IsOffset(offset_or_index)) {
    auto it = std::lower_bound(labels_.begin(), labels_.end(), offset_or_index);
    if (it != labels_.end() && *it == offset_or_index)
...
void OrderedLabelManager::InsertTargets(const ReferenceGroup& group,
                                        Disassembler* disasm) {
...
offset_t UnorderedLabelManager::MarkedIndexFromOffset(
    offset_t offset_or_index) const {
  if (IsOffset(offset_or_index)) {
    auto it = labels_map_.find(offset_or_index);
...
void UnorderedLabelManager::Init(std::vector<offset_t>&& labels) {
  ...
  for (offset_t idx = 0; idx < size; ++idx) {
    if (labels_[idx] != kUnusedIndex) {
      assert(labels_map_.find(labels_[idx]) == labels_map_.end());
...
void UnorderedLabelManager::InsertNewOffset(offset_t offset) {
  assert(labels_map_.find(offset) == labels_map_.end());
