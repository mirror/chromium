...
std::vector<int32_t> GenerateReferencesDeltaTest(
  ...
  old_refs.InsertReferences(old_references);
  ReferenceSet new_refs({1, TypeTag(0), PoolTag(0)});
  new_refs.InsertReferences(new_references);

  TargetPool old_targets;
  old_targets.InsertTargets(old_refs.references());
  old_refs.CanonicalizeTargets(old_targets);

  TargetPool new_targets;
  new_targets.InsertTargets(new_refs.references());
  new_refs.CanonicalizeTargets(new_targets);
...
std::vector<offset_t> FindExtraTargetsTest(
  ...
  reference_set.InsertReferences(new_references);
  TargetPool new_targets;
  new_targets.InsertTargets(reference_set.references());
  reference_set.CanonicalizeTargets(new_targets);
