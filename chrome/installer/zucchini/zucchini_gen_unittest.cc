...
TEST(ZucchiniGenTest, FindExtraTargets) {
  ...
  EXPECT_EQ(OffsetVector(), FindExtraTargetsTest({}, {}, {}));
  EXPECT_EQ(OffsetVector(), FindExtraTargetsTest({{10, 0}}, {}, {}));
  EXPECT_EQ(OffsetVector(),
            FindExtraTargetsTest({{10, 0}}, {kUnusedIndex}, {}));
  EXPECT_EQ(OffsetVector(),
            FindExtraTargetsTest({{10, 0}, {13, 0}}, {},
                                 EquivalenceMap({{{12, 11, 2}, 0.0}})));

  EXPECT_EQ(OffsetVector({1}),
            FindExtraTargetsTest({{10, 1}}, {},
                                 EquivalenceMap({{{10, 10, 2}, 0.0}})));
  EXPECT_EQ(OffsetVector({1}),
            FindExtraTargetsTest({{10, 1}}, {kUnusedIndex},
                                 EquivalenceMap({{{10, 10, 2}, 0.0}})));
  EXPECT_EQ(OffsetVector({}),
            FindExtraTargetsTest({{10, 1}}, {1},
                                 EquivalenceMap({{{10, 10, 2}, 0.0}})));
...
TEST(ZucchiniGenTest, GenerateReferencesDelta) {
  ...
  EXPECT_EQ(
      std::vector<int32_t>(),
      GenerateReferencesDeltaTest({{10, 0}}, {{10, 0}}, {}, EquivalenceMap()));

  EXPECT_EQ(std::vector<int32_t>({0}),
            GenerateReferencesDeltaTest({{10, 0}}, {{10, 0}}, {0, 1},
                                        EquivalenceMap({{{10, 10, 4}, 0.0}})));
  EXPECT_EQ(std::vector<int32_t>({1}),
            GenerateReferencesDeltaTest({{10, 0}}, {{10, 0}}, {1, 0},
                                        EquivalenceMap({{{10, 10, 4}, 0.0}})));
  EXPECT_EQ(std::vector<int32_t>({2, -1}),
            GenerateReferencesDeltaTest({{10, 0}, {11, 1}}, {{10, 0}, {11, 1}},
                                        {1, 2, 0},
                                        EquivalenceMap({{{10, 10, 4}, 0.0}})));

  EXPECT_EQ(std::vector<int32_t>({0, 0}),
            GenerateReferencesDeltaTest({{10, 0}, {11, 1}, {12, 2}, {13, 3}},
                                        {{10, 0}, {11, 1}, {12, 2}, {13, 3}},
                                        {0, 1, 2, 3},
                                        EquivalenceMap({{{11, 11, 2}, 0.0}})));

  EXPECT_EQ(std::vector<int32_t>({-1, 1}),
            GenerateReferencesDeltaTest(
                {{10, 0}, {12, 1}}, {{10, 0}, {12, 1}}, {0, 1},
                EquivalenceMap({{{12, 10, 2}, 0.0}, {{10, 12, 2}, 0.0}})));
  EXPECT_EQ(std::vector<int32_t>({0, 0}),
            GenerateReferencesDeltaTest(
                {{0, 0}, {2, 1}}, {{0, 0}, {2, 1}}, {1, 0},
  ...
  EXPECT_EQ(
      ...
      GenerateReferencesDeltaTest(
          {{10, 0}, {12, 1}, {14, 2}}, {{10, 0}, {12, 1}, {14, 2}}, {0, 1, 2},
          EquivalenceMap({{{14, 10, 2}, 0.0}, {{10, 14, 2}, 0.0}})));

  EXPECT_EQ(
      std::vector<int32_t>({-2, 2}),
      GenerateReferencesDeltaTest(
          {{11, 0}, {14, 1}, {17, 2}}, {{11, 0}, {14, 1}, {17, 2}}, {0, 1, 2},
          EquivalenceMap({{{16, 10, 3}, 0.0}, {{10, 16, 3}, 0.0}})));

  EXPECT_EQ(
      std::vector<int32_t>({-2, 2}),
      GenerateReferencesDeltaTest(
          {{10, 0}, {14, 2}, {16, 1}}, {{10, 0}, {14, 2}}, {0, 1, 2},
          EquivalenceMap(
              {{{14, 10, 2}, 0.0}, {{12, 12, 2}, 0.0}, {{10, 14, 2}, 0.0}})));
}
