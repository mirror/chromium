...
// EquivalenceCandiate. Since similarity doesn't affect results in unittests,
// we use this dummy value for simplicity.
...
// Helper function wrapping FindExtraTargets(). |new_references| take rvalue to
// so callers can be simplified.
...
TEST(ZucchiniGenTest, FindExtraTargets) {
  ...
  // Target ignored because of new labels.
  EXPECT_EQ(OffsetVector({}),
...
TEST(ZucchiniGenTest, GenerateReferencesDelta) {
  ...
  EXPECT_EQ(
      std::vector<int32_t>(),
      GenerateReferencesDeltaTest({{10, 0}}, {{20, 0}}, {}, EquivalenceMap()));

  // Simple cases with one equivalence.
  EXPECT_EQ(
      std::vector<int32_t>({0}),
      GenerateReferencesDeltaTest({{10, 3}}, {{20, 3}}, {3, 4},
                                  EquivalenceMap({{{10, 20, 4}, kDummySim}})));
  EXPECT_EQ(
      std::vector<int32_t>({1}),
      GenerateReferencesDeltaTest({{10, 3}}, {{20, 3}}, {4, 3},
                                  EquivalenceMap({{{10, 20, 4}, kDummySim}})));
  EXPECT_EQ(std::vector<int32_t>({2, -1}),
            GenerateReferencesDeltaTest(
                {{10, 3}, {11, 4}}, {{20, 3}, {21, 4}}, {4, 5, 3},
                EquivalenceMap({{{10, 20, 4}, kDummySim}})));
  ...
  EXPECT_EQ(std::vector<int32_t>({0, 0}),
            GenerateReferencesDeltaTest(
                ...
                EquivalenceMap({{{11, 21, 2}, kDummySim}})));
  ...
  EXPECT_EQ(
      ...
      GenerateReferencesDeltaTest(
          ...
          EquivalenceMap({{{12, 10, 2}, 0.0}, {{10, 12, 2}, kDummySim}})));
  EXPECT_EQ(
  ...
  EXPECT_EQ(
      ...
      GenerateReferencesDeltaTest({{10, 0}, {12, 1}, {14, 2}},
                                  {{10, 0}, {12, 1}, {14, 2}}, {0, 1, 2},
                                  EquivalenceMap({{{14, 10, 2}, kDummySim},
                                                  {{10, 14, 2}, kDummySim}})));

  EXPECT_EQ(
      std::vector<int32_t>({-2, 2}),
      GenerateReferencesDeltaTest({{11, 0}, {14, 1}, {17, 2}},
                                  {{11, 0}, {14, 1}, {17, 2}}, {0, 1, 2},
                                  EquivalenceMap({{{16, 10, 3}, kDummySim},
                                                  {{10, 16, 3}, kDummySim}})));
