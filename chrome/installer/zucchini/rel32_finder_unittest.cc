...
TEST(Abs32CollisionDetectorTest, Empty) {
  std::vector<offset_t> abs32_locations = {};
  Abs32CollisionDetector collision_detector(abs32_locations, 2);

  EXPECT_FALSE(collision_detector.Collides(1, 1));
}

TEST(Abs32CollisionDetectorTest, All) {
  std::vector<offset_t> abs32_locations = {4, 10, 15, 20, 22};
  Abs32CollisionDetector collision_detector(abs32_locations, 2);

  EXPECT_FALSE(collision_detector.Collides(1, 1));
  EXPECT_FALSE(collision_detector.Collides(1, 2));
  EXPECT_FALSE(collision_detector.Collides(2, 2));
  EXPECT_TRUE(collision_detector.Collides(2, 3));
  EXPECT_EQ(4 + 2, collision_detector.NextOffset());

  EXPECT_FALSE(collision_detector.Collides(6, 4));
  EXPECT_TRUE(collision_detector.Collides(11, 2));
  EXPECT_EQ(10 + 2, collision_detector.NextOffset());

  EXPECT_FALSE(collision_detector.Collides(12, 2));
  EXPECT_TRUE(collision_detector.Collides(12, 6));
  EXPECT_EQ(15 + 2, collision_detector.NextOffset());

  EXPECT_TRUE(collision_detector.Collides(23, 1));
  EXPECT_EQ(22 + 2, collision_detector.NextOffset());

  EXPECT_FALSE(collision_detector.Collides(24, 2));
}
...
class TestRel32Finder : public Rel32Finder {
 public:
  ...
  ConstBufferView::const_iterator Scan() override {
    cursor_ = next_result.begin();
    return next_result.end();
  }
...
TEST(Rel32FinderTest, Scan) {
  ...
  auto run_test = [&](size_t expected, size_t next_cursor, size_t size) {
    CHECK_LE(expected, kRegionTotal);
    CHECK_LE(next_cursor, kRegionTotal);
    CHECK_LE(next_cursor + size, kRegionTotal);

    EXPECT_EQ(finder.next_cursor(), image.begin() + expected);
    EXPECT_EQ(finder.last(), image.end());

    finder.next_result = ConstBufferView(image.begin() + next_cursor, size);
...
TEST(Rel32FinderX86Test, FindNext) {
  ...
  for (auto location : expected_locations) {
    ...
    EXPECT_EQ(location.first, rel_finder.cursor() - image.begin());
    EXPECT_EQ(location.second, result->location - image.begin());
...
TEST(Rel32FinderX86Test, Discard) {
  constexpr uint8_t data[] = {
      ...
      0xE8, 0x00, 0x00, 0x00, 0xE9,  // 0F: call  E9000014
  };
  ...
  rel_finder.Seek(image.begin() + 0x0F);
  EXPECT_EQ(0x10, next_location(rel_finder));  // Found if we seek beyond 0xE.
}
