...
TEST(Rel32FinderTest, Scan) {
  ...
  auto check_finder_state = [&](const TestRel32Finder& finder,
    ...
    EXPECT_EQ(image.begin() + expected_cursor, finder.cursor());
  ...
  check_finder_state(finder, 10, 10);
...
TEST(Rel32FinderX86Test, Discard) {
  ...
  EXPECT_EQ(0x10, next_location(rel_finder));  // Found if we seek beyond 0x0E.
}
