...
bool DisassemblerWin32<Traits>::ParseHeader() {
  ...
  if (trans_builder.Build(&translator_) != AddressTranslatorBuilder::kSuccess)
    return false;
...
bool DisassemblerWin32<Traits>::ParseAndStoreRel32() {
  ...
  CachedOffsetToRvaTranslator location_offset_to_rva(translator_);
  CachedRvaToOffsetTranslator target_rva_checker(translator_);
