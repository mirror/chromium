...
// assigns |source| to enable further parsing, Otherwise returns false.
...
bool DisassemblerWin32<Traits>::ParseHeader() {
  ...
  std::sort(section_rva_map_.begin(), section_rva_map_.end(),
            [](const std::pair<rva_t, int>& a, const std::pair<rva_t, int>& b) {
              return a.first < b.first;
            });
  std::sort(
      section_offset_map_.begin(), section_offset_map_.end(),
      [](const std::pair<offset_t, int>& a, const std::pair<offset_t, int>& b) {
        return a.first < b.first;
      });
  return true;
}
