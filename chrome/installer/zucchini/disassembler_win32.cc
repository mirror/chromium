...
std::unique_ptr<ReferenceReader> DisassemblerWin32<Traits>::MakeReadRelocs(
  ...
  auto reloc_rva_reader = base::MakeUnique<RelocRvaReaderWin32>(
      image_, reloc_region_, reloc_block_offsets_, lo, hi);
...
std::unique_ptr<ReferenceReader> DisassemblerWin32<Traits>::MakeReadAbs32(
  ...
  class Abs32Reader : public ReferenceReader {
   public:
    Abs32Reader(const DisassemblerWin32* self, offset_t lo, offset_t hi)
        : self_(self),
          image_(self->image_),
          target_rva_to_offset_(self->translator_),
          abs32_locations_end_(self->abs32_locations_.end()),
          hi_(hi) {
      abs32_locations_current_ = std::lower_bound(
          self_->abs32_locations_.begin(), self_->abs32_locations_.end(), lo);
    }

    base::Optional<Reference> GetNext() override {
      while (abs32_locations_current_ < abs32_locations_end_ &&
             *abs32_locations_current_ < hi_) {
        offset_t location = *(abs32_locations_current_++);
        rva_t target_rva = self_->AddressToRva(
            image_.read<typename Traits::Address>(location));
   ...
   private:
    ...
    std::vector<offset_t>::const_iterator abs32_locations_current_;
    std::vector<offset_t>::const_iterator abs32_locations_end_;
    offset_t hi_;
  };

  return base::MakeUnique<Abs32Reader>(this, lo, hi);
}
...
std::unique_ptr<ReferenceWriter> DisassemblerWin32<Traits>::MakeWriteAbs32(
    MutableBufferView image) {
  class Abs32Writer : public ReferenceWriter {
   public:
    Abs32Writer(const DisassemblerWin32* self, MutableBufferView image)
        : self_(self),
          image_(image),
          target_offset_to_rva_(self->translator_) {}

    void PutNext(Reference ref) override {
      rva_t target_rva = target_offset_to_rva_.Convert(ref.target);
      typename Traits::Address address = self_->RvaToAddress(target_rva);

      image_.write<typename Traits::Address>(ref.location, address);
    };
  ...
  return base::MakeUnique<Abs32Writer>(this, image);
}
...
bool DisassemblerWin32<Traits>::ParseHeader() {
  ...
  units.reserve(sections_count + 1);
