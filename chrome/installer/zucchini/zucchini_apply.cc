...
bool ApplyReferencesCorrection(ExecutableType exe_type,
  ...
  EquivalenceMap equivalence_map(patch.GetEquivalenceSource());
  ...
  for (const auto& pool_and_sub_groups : pool_groups) {
    ...
    targets.Project(equivalence_map);
    ...
    for (ReferenceGroup group : sub_groups) {
      ...
      for (auto equivalence = equiv_source.GetNext(); equivalence.has_value();
        ...
        for (auto ref = ref_gen->GetNext(); ref.has_value();
          ...
          offset_t projected_target = equivalence_map.ProjectOffset(ref->target);
          offset_t expected_key = targets.FindKeyForOffset(projected_target);
          ...
          ref->target = targets.GetOffsetForKey(expected_key + delta.value());
          ref->location =
