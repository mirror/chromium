...
void ExtraDataSink::PutNext(BufferView region) {
...
void TargetSink::PutNext(uint32_t symbol) {
  DCHECK(symbol > previous_target_);

  EncodeVarUInt<uint32_t>(symbol - previous_target_,
                          std::back_inserter(extra_targets_));

  previous_target_ = symbol;
}

ElementPatchSink::ElementPatchSink() = default;
ElementPatchSink::ElementPatchSink(ElementMatch element_match)
    : element_match_(element_match) {}
ElementPatchSink::ElementPatchSink(ElementPatchSink&&) = default;
ElementPatchSink::~ElementPatchSink() = default;

size_t ElementPatchSink::SerializedSize() const {
...
bool ElementPatchSink::SerializeInto(BufferSink* sink) const {
  ...
  if (!sink->PutValue<uint64_t>(extra_targets_.size()))
    return false;
  for (const auto& extra_symbols : extra_targets_) {
    if (!sink->PutValue<uint8_t>(extra_symbols.first.value()))
      return false;
    if (!extra_symbols.second.SerializeInto(sink))
      return false;
  }
  return true;
}
...
EnsemblePatchSink::EnsemblePatchSink(BufferView old_image,
                                     BufferView new_image) {
  header_.magic = PatchHeader::kMagic;
  header_.old_size = old_image.size();
  header_.old_crc = crc32(old_image.begin(), old_image.end());
  header_.new_size = new_image.size();
  header_.new_crc = crc32(new_image.begin(), new_image.end());
}
