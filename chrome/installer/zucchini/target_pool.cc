...
void TargetPool::InsertTargets(TargetSource* targets) {
  ...
  // This can be called multiple times in succession. This is done (instead of
  // inserting everything first, then sort and uniquify once) to prioritize
  // memory reduction over running time.
  SortAndUniquify(&targets_);
}
