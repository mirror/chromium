...
class ReferenceSet {
 public:
  ...
  // The initializers below should be called exactly once. These insert all
  // references from |ref_reader/refs| into this class. The targets of these
  // references must be in |target_pool_|.
  ...
  // assumed to be valid, i.e., it must be spanned by some IndirectReference in
  // |references_|.
 ...
 private:
  ...
  // List of IndirectReference instances sorted by location.
