...
RelocRvaReaderWin32::RelocRvaReaderWin32(
    ...
    : image_(image) {
  CHECK_LE(lo, hi);
  lo = reloc_region.InclusiveClip(lo);
  hi = reloc_region.InclusiveClip(hi);
...
bool RelocRvaReaderWin32::LoadRelocBlock(
  ...
  cur_reloc_units_.Reset(block_begin, block_size);
...
RelocReaderWin32::RelocReaderWin32(
    std::unique_ptr<RelocRvaReaderWin32> reloc_rva_reader,
    uint16_t reloc_type,
...
RelocReaderElf::RelocReaderElf(
  ...
  cursor_ = cur_section_dims_->region.offset;
  cur_section_dims_end_ = cur_section_dims_->region.hi();
  if (cursor_ < lo)
  ...
  if (end_section != reloc_section_dims_.begin()) {
    ...
    if (hi_ - end_section->region.offset < end_section->region.size) {
      hi_ = end_section->region.offset +
            ceil<offset_t>(hi_ - end_section->region.offset,
                           end_section->entsize);
    }
  }
}
  ...
  for (; cursor_ + cur_entsize_ <= hi_; cursor_ += cur_entsize_) {
    if (cursor_ >= cur_section_dims_end_) {
      ...
      if (cursor_ + cur_entsize_ > hi_)
        return base::nullopt;
      cur_section_dims_end_ = cursor_ + cur_section_dims_->region.size;
...
RelocWriterElf::RelocWriterElf(MutableBufferView image,
                               int bits,
                               const AddressTranslator& translator)
    : image_(image), bits_(bits), target_offset_to_rva_(translator) {}
