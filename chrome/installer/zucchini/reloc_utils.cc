...
bool RelocRvaReaderWin32::FindRelocBlocks(
  ...
  while (reloc_data.size() >= sizeof(pe::RelocHeader)) {
    ...
    auto size = ReinterpretAs<pe::RelocHeader>(reloc_data.begin()).size;
...
bool RelocRvaReaderWin32::LoadRelocBlock(
  ...
  const auto& header = ReinterpretAs<pe::RelocHeader>(header_buf.begin());
...
void RelocWriterWin32::PutNext(Reference ref) {
  ...
  --block_it;
  rva_t rva_hi_bits =
      ReinterpretAs<pe::RelocHeader>(image_.begin() + *block_it).rva_hi;
