...
// For BinaryDataHistogram, |v| is in [0, 1].
void OutlierDetector::Add(double v) {
  ++n_;
  sum_ += v;
  sum_of_squares_ += v * v;
}
...
std::string OutlierDetector::RenderStats() {
  return base::StringPrintf("Mean = %.5f, StdDev = %.5f over %zu samples",
                            mean_, standard_deviation_, n_);
}

// Constants are chosen for BinaryDataHistogram, so |v| is in [0, 1].
int OutlierDetector::DecideOutlier(double v) {
  ...
  double num_sigma = (v - mean_) / tolerance;
