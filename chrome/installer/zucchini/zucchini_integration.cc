...
status::Code BaseApply(MappedFileReader&& old_file,
                       MappedFileReader&& patch_file,
  ...
  MappedFileWriter new_file(std::move(new_file_handle), header.new_size);
  if (!new_file.IsValid())
...
status::Code Apply(const base::FilePath& old_path,
  ...
  return BaseApply(std::move(old_file), std::move(patch_file),
                   std::move(new_file));
}
