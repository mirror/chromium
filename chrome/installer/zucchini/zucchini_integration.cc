...
struct FileNames {
  FileNames(std::string_view old_name,
      ...
      : old_name(old_name),
        ...
        new_name(new_name),
        external(external) {}
  FileNames(const base::FilePath& old_path,
            const base::FilePath& patch_path,
            const base::FilePath& new_path,
            bool external)
      : old_name(old_path.value()),
        patch_name(patch_path.value()),
        new_name(new_path.value()),
        external(external) {}
...
status::Code ApplyCommon(base::File&& old_file_handle,
  ...
  if (!patch_file.IsValid() || patch_file.HasStatus()) {
  ...
  if (!old_file.IsValid() || old_file.HasStatus()) {
  ...
  MappedFileWriter new_file(
      base::FilePath(names.external ? "" : names.new_name),
      std::move(new_file_handle), header.new_size);
  if (!new_file.IsValid() || new_file.HasStatus()) {
...
status::Code Apply(base::File old_file_handle,
                   base::File patch_file_handle,
                   base::File new_file_handle) {
  const FileNames file_names("externally_opened_old_file",
                             "externally_opened_patch_file",
                             "externally_opened_new_file", true);
...
status::Code Apply(const base::FilePath& old_path,
  ...
  const FileNames file_names(old_path, patch_path, new_path, false);
