...
class RelocUtilsWin32Test : public testing::Test {
 protected:
  ...
  bool Initialize(const std::vector<uint8_t>& image_raw,
    ...
    image_.Reset(image_raw.data(), image_raw.size());
  ...
  Units EmitAll(offset_t lo, offset_t hi) {
    RelocRvaReaderWin32 reader(image_, reloc_region_, reloc_block_offsets_, lo,
                               hi);
    std::vector<RelocUnitWin32> units;
...
TEST_F(RelocUtilsWin32Test, RvaReaderMulti) {
  ...
  EXPECT_EQ(Cat(Cat(exp0, exp1), exp2), EmitAll(0, image_raw.size()));
  // Entire blocks.
  ...
  EXPECT_EQ(Units(), EmitAll(kBlockEnd, image_raw.size()));
...
TEST_F(RelocUtilsWin32Test, ReadWrite) {
  ...
  translator.Initialize({{0, image_.size(), kBaseRva, image_.size()}});
  ...
  auto reloc_rva_reader = base::MakeUnique<RelocRvaReaderWin32>(
      image_, reloc_region_, reloc_block_offsets_, 0, image_.size());
  offset_t offset_bound = image_.size() - kVAWidthX86 + 1;
