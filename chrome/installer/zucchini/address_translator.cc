...
/******** AddressTranslator::CachedForOffsetToRva ********/

AddressTranslator::CachedForOffsetToRva::CachedForOffsetToRva(
    const AddressTranslator& translator)
    : translator_(translator) {}

rva_t AddressTranslator::CachedForOffsetToRva::Convert(offset_t offset) const {
...
/******** AddressTranslator::CachedForRvaToOffset ********/

AddressTranslator::CachedForRvaToOffset::CachedForRvaToOffset(
    const AddressTranslator& translator)
    : translator_(translator) {}

bool AddressTranslator::CachedForRvaToOffset::IsValid(rva_t rva) const {
...
offset_t AddressTranslator::CachedForRvaToOffset::Convert(rva_t rva) const {
  if (!cached_unit_ || !cached_unit_->CoversRva(rva)) {
    const AddressTranslator::Unit* unit = translator_.RvaToUnit(rva);
    if (!unit)
...
AddressTranslator::Status AddressTranslator::Initialize(
  ...
  std::vector<Unit> units(std::move(units_in));
