...
TEST_F(BufferViewTest, CoversArray) {
  ...
  for (uint32_t i = 1; i <= kLen; ++i) {
    EXPECT_TRUE(view.covers_array(0, 1, i));
    EXPECT_TRUE(view.covers_array(0, i, 1));
    EXPECT_TRUE(view.covers_array(0, i, kLen / i));
    EXPECT_TRUE(view.covers_array(0, kLen / i, i));
    if (i < kLen) {
      EXPECT_TRUE(view.covers_array(i, 1, kLen - i));
      EXPECT_TRUE(view.covers_array(i, kLen - i, 1));
    }
    EXPECT_TRUE(view.covers_array(kLen - (kLen / i) * i, 1, kLen / i));
  }

  EXPECT_TRUE(view.covers_array(0, 0, kLen));
  EXPECT_TRUE(view.covers_array(kLen - 1, 0, kLen));
  EXPECT_FALSE(view.covers_array(kLen, 0, kLen));
  EXPECT_TRUE(view.covers_array(0, 0, 0x10000));
  EXPECT_TRUE(view.covers_array(kLen - 1, 0, 0x10000));
  EXPECT_FALSE(view.covers_array(kLen, 0, 0x10000));

  EXPECT_FALSE(view.covers_array(0, 1, kLen + 1));
  EXPECT_FALSE(view.covers_array(0, 2, kLen));
  EXPECT_FALSE(view.covers_array(0, kLen + 11, 1));
  EXPECT_FALSE(view.covers_array(0, kLen, 2));
  EXPECT_FALSE(view.covers_array(1, kLen, 1));

  EXPECT_FALSE(view.covers_array(kLen, 1, 1));
  EXPECT_FALSE(view.covers_array(kLen, 0, 1));
