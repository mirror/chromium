...
TEST_F(BufferViewTest, Includes) {
  EXPECT_FALSE(ConstBufferView().includes({0, 0}));
  EXPECT_FALSE(ConstBufferView().includes({0, 1}));

  ConstBufferView view(std::begin(bytes_), kLen);

  EXPECT_TRUE(view.includes({0, 0}));
  EXPECT_TRUE(view.includes({0, 1}));
  EXPECT_TRUE(view.includes({0, kLen}));
  EXPECT_FALSE(view.includes({0, kLen + 1}));
  EXPECT_FALSE(view.includes({1, kLen}));

  EXPECT_TRUE(view.includes({kLen - 1, 0}));
  EXPECT_TRUE(view.includes({kLen - 1, 1}));
  EXPECT_FALSE(view.includes({kLen - 1, 2}));
  EXPECT_FALSE(view.includes({kLen, 0}));
  EXPECT_FALSE(view.includes({kLen, 1}));

  EXPECT_FALSE(view.includes({1, size_t(-1)}));
  EXPECT_FALSE(view.includes({size_t(-1), 1}));
  EXPECT_FALSE(view.includes({size_t(-1), size_t(-1)}));
}
