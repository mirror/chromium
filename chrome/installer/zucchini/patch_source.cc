...
base::Optional<Equivalence> EquivalenceSource::GetNext() {
  ...
  if (!DecodeVarUInt<uint32_t>(&equivalence.length, &copy_count_))
    return base::nullopt;

  int32_t src_offset_diff = 0;
  if (!DecodeVarInt<int32_t>(&src_offset_diff, &src_skip_))
    return base::nullopt;
  ...
  if (!DecodeVarUInt<uint32_t>(&dst_offset_diff, &dst_skip_))
    return base::nullopt;
...
base::Optional<BufferView> ExtraDataSource::GetNext(offset_t size) {
  BufferView buffer = extra_data_.GetRegion(size);
  if (buffer.size() != size)
    return base::nullopt;
  return buffer;
}
...
base::Optional<RawDeltaUnit> RawDeltaSource::GetNext() {
  if (raw_delta_skip_.empty() || raw_delta_diff_.empty())
    return base::nullopt;
  RawDeltaUnit delta;
  if (!DecodeVarUInt<uint32_t>(&delta.copy_offset, &raw_delta_skip_))
    return base::nullopt;
...
bool TargetSource::Parse(BufferSource* source) {
  return zucchini::Parse(&extra_symbols_, source);
}

ElementPatchSource::ElementPatchSource() = default;
ElementPatchSource::ElementPatchSource(const ElementPatchSource&) = default;
ElementPatchSource::~ElementPatchSource() = default;

bool ElementPatchSource::Parse(BufferSource* source) {
  ...
  for (uint32_t i = 0; i < pool_count; ++i) {
    ...
    if (!extra_target.Parse(source))
      return false;
    extra_targets_.emplace(pool_tag, std::move(extra_target));
  }
  return true;
}
...
bool EnsemblePatchSource::Parse(BufferSource* source) {
  ...
  for (uint32_t i = 0; i != element_count; ++i) {
    ElementPatchSource element_patch;
