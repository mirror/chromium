...
class RVAToOffsetTranslator {
 public:
  ...
  virtual offset_t RVAToOffset(rva_t rva) = 0;
};
...
class OffsetToRVATranslator {
 public:
  ...
  virtual rva_t OffsetToRVA(offset_t offset) = 0;
};
...
class AddressTranslator {
 public:
  ...
  virtual std::unique_ptr<RVAToOffsetTranslator> GetRVAToOffsetTranslator()
      const = 0;

  virtual std::unique_ptr<OffsetToRVATranslator> GetOffsetToRVATranslator()
      const = 0;
};
