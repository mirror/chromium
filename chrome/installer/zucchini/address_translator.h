 ...
 public:
  ...
  struct Unit {
    ...
    friend std::ostream& operator<<(std::ostream& out, const Unit& unit) {
      out << "[" << unit.offset_begin << "," << unit.offset_end() << ")-"
          << "[" << unit.rva_begin << "," << unit.rva_end() << ")";
      return out;
    }
  ...
  // DO_NOT_SUBMIT: Debugging code.
  friend std::ostream& operator<<(std::ostream& out,
                                  const AddressTranslator& trans) {
    for (const auto& unit : trans.units_sorted_by_offset_)
      out << unit;
    out << std::endl;
    for (const auto& unit : trans.units_sorted_by_rva_)
      out << unit;
    out << std::endl;
    out << "Fake offset begin = " << trans.fake_offset_begin_ << std::endl;
    return out;
  }
