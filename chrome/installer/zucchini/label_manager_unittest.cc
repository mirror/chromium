...
TEST(LabelManagerTest, Ordered) {
  ...
  EXPECT_EQ(V({0x11, 0x33, 0x44}), label_manager.Labels());
  ...
  EXPECT_EQ(V({0x00, 0x11, 0x33, 0x44, 0x66}), label_manager.Labels());
...
TEST(LabelManagerTest, Unordered) {
  ...
  // Initialize with some data, test direct lookups.
  label_manager.Init(V({0x33, BAD, BAD, 0x11, 0x44, BAD}));
...
TEST(LabelManagerTest, OrderedBatch) {
  ...
  EXPECT_EQ(V({0x00, 0x11, 0x33, 0x55}), label_manager.Labels());

  // Test data for array conversions.
  V values = {0x22, 0x33, 0x44, MarkIndex(3), 0x11};

  // Convert all stored offsets for marked index.
  for (auto& v : values)
    v = label_manager.MarkedIndexFromOffset(v);
  EXPECT_EQ(V({0x22, MarkIndex(2), 0x44, MarkIndex(3), MarkIndex(1)}), values);

  // Convert all marked index (assumed to be all stored) to offsets.
  for (auto& v : values)
    v = label_manager.OffsetFromMarkedIndex(v);
  EXPECT_EQ(V({0x22, 0x33, 0x44, 0x55, 0x11}), values);
}

TEST(LabelManagerTest, UnorderedBatch) {
  // Initialize Label Manager.
  UnorderedLabelManager label_manager;
  V labels = {0x00, BAD, 0x33, BAD, 0x11, BAD, 0x55};
  label_manager.Init(std::move(labels));

  // Test data for array conversions.
  V values = {0x22, 0x33, 0x44, MarkIndex(6), 0x11};

  // Convert all stored offsets for marked index.
  for (auto& v : values)
    v = label_manager.MarkedIndexFromOffset(v);
  EXPECT_EQ(V({0x22, MarkIndex(2), 0x44, MarkIndex(6), MarkIndex(4)}), values);
