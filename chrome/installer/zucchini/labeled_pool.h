...
// Target are stored in an ordered vector and can be retrieved using binary
// search.
class LabeledPool {
 public:
  LabeledPool();
  ...
  // be set with |SwapLabels|. Once labels are assigned, no additional targets
  // can be inserted.
  ...
  size_t LabelBound() const;

  // Returns the number of targets.
  size_t size() const { return targets_.size(); }

  // Return the ordered targets vector.
