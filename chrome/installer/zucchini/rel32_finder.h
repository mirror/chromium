...
// that lie outside of |abs32_locations| elements, each taking up |abs_width|
...
class Rel32Finder {
 public:
  ...
  // Reset object to start searching in |region|.
 ...
 protected:
  ...
  bool FindNext() {
    ConstBufferView result = Scan(ConstBufferView::FromRange(cursor_, end_));
    cursor_ = result.begin();
  ...
  // region starting at the current seach position, and ending beyond the
  ...
  // unless Accept() was call, in which case |region| will start beyond the last
  // reference.
 ...
 private:
  ...
  ConstBufferView::const_iterator end_;
