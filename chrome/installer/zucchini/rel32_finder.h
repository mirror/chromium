...
// Utility class to detect collisions with abs32 pointers. This is a statefull
// class that keeps track of a position that is update during each query. This
// is typically used along with Rel32Finder to filter out rel32 pointers.
class Abs32CollisionDetector {
 public:
  // |abs32_locations| is an order list of abs32 pointers location.
  Abs32CollisionDetector(const std::vector<offset_t>& abs32_locations,
                         size_t abs32_width)
      : abs32_current_(abs32_locations.begin()),
        abs32_last_(abs32_locations.end()),
  ...
  // Returns an offset beyond the next abs32 pointer. This should be called only
  // once Collides() returns true, to query the next valid offset.
 ...
 private:
  ...
  std::vector<offset_t>::const_iterator abs32_last_;
  size_t abs32_width_;
};
...
class Rel32Finder {
 public:
  ...
  // Discard last reference found. Next call to FindNext() will scan starting
  // at the current search position, instead of beyond the last reference found.
  void Discard() { next_cursor_ = cursor_; }
  ...
  ConstBufferView::const_iterator last() const { return last_; }
 ...
 protected:
  ...
  const ConstBufferView::const_iterator last_;
