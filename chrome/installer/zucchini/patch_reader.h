...
bool ParseVarUInt(BufferSource* source, T* value) {
  auto skip_count = DecodeVarUInt(source->begin(), source->end(), value);
  if (!skip_count) {
...
bool ParseVarInt(BufferSource* source, T* value) {
  auto skip_count = DecodeVarInt(source->begin(), source->end(), value);
  if (!skip_count) {
...
//   otherwise.
// - base::Optional<SOME_TYPE> GetNext(OPT_PARAMS): Decodes consumed data and
...
class ExtraDataSource {
 public:
  ...
  // Core functions.
  bool Initialize(BufferSource* source);
  // |size| is the size in bytes of the buffer returned.
