...
// The Parse*() functions below attempts to extract data of a specific type from
...
// Parses |source| for the next an ElementMatch.
...
bool ParseVarUInt(BufferSource* source, T* value) {
  ...
  if (!it.has_value())
    return false;
  source->remove_prefix(it.value() - source->begin());
  return true;
}
...
bool ParseVarInt(BufferSource* source, T* value) {
  ...
  source->Skip(it.value() - source->begin());
  return true;
}

}  // namespace patch

// StreamableSource<T> is an interface for stateful parsers that extract
// embedded BufferSource chunks from the owner's BufferSource (expected to
// remain valid for the lifetime of the parser), parse the obtained chunks, and
// provide an iterator to extract data units. The interface of a
// StreamableSource<T> is defined as:

//   StreamableSource(const StreamableSource&);
//
...
//
//   Returns the next object available, or nullopt if data is exhausted or ill-
//   formed. The type T is specific to each implemenetation.
//   base::Optional<T> GetNext();
//
//   Returns true if data is exhausted. Otherwise returns false.
//   bool Done() const;

// Note that we are not using inheritance because different sources differ
// by the type extracted, and there is not need for dynamic disptaching.
...
class ExtraDataSource {
 public:
  ...
  // StreamableSource<ConstBufferView>:
  bool Initialize(BufferSource* source);
...
class EnsemblePatchReader {
 public:
  ...
  // crc32. Returns true if file matches expectations, false otherwise.
