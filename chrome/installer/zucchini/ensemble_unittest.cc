...
class TestElementDetector {
 public:
  ...
  base::Optional<Element> Run(ConstBufferView image) const {
    ...
    if (it == lut_.end())
      return base::nullopt;
    return Element(it->second, image.region());
  }
...
TEST(EnsembleTest, ParseImposedMatches) {
  ...
  auto run_test = [&](const std::string& imposed_matches) -> bool {
    ...
    bool ret = EnsembleMatcher::ParseImposedMatches(
  ...
  auto run_check = [&](const ElementMatch& match, ExecutableType exe_type,
    ...
    EXPECT_EQ(old_size, match.old_element.length) << prev_imposed_matches;
    ...
    EXPECT_EQ(new_size, match.new_element.length) << prev_imposed_matches;
  };
