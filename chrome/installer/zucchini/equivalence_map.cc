...
double GetTokenSimilarity(const ImageIndex& old_image,
  ...
  double affinity = targets_affinity[pool.value()].AffinityBetween(
  ...
  return affinity > 0.0 ? old_image.refs(old_type).width() : -2.0;
}
...
void EquivalenceMap::Build(
  ...
  CreateCandidates(old_sa, old_image, new_image, targets_affinities,
                   min_similarity);
  SortByDestination();
  Prune(old_image, new_image, targets_affinities, min_similarity);
  ...
  while (dst_offset < new_image.size()) {
    ...
    for (auto it = match; it != old_sa.end(); ++it) {
      EquivalenceCandidate candidate = VisitEquivalenceSeed(
          old_image.image_index(), new_image.image_index(), targets_affinities,
