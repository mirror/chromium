...
bool EnsemblePatchReader::Initialize(BufferSource* source) {
  ...
  for (uint32_t i = 0; i < element_count; ++i) {
    ...
    if (!element_patch.old_element().FitsIn(header_.old_size))
      return false;
    if (!element_patch.new_element().FitsIn(header_.new_size))
      return false;

    if (element_patch.new_element().offset != current_dst)
      return false;
    current_dst = element_patch.new_element().EndOffset();

    elements_.push_back(std::move(element_patch));
  }
  if (current_dst != header_.new_size)
    return false;
