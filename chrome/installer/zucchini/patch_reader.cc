...
base::Optional<RawDeltaUnit> RawDeltaSource::GetNext() {
  ...
  if (!raw_delta_diff_.GetValue<int8_t>(&delta.diff))
    return base::nullopt;
  delta.copy_offset += prev_copy_offset_;
  prev_copy_offset_ = delta.copy_offset + 1;
  return delta;
}
...
bool EnsemblePatchReader::CheckOldFile(ConstBufferView old_image) const {
  if (old_image.size() != header_.old_size)
    return false;
  if (CalculateCrc32(old_image.begin(), old_image.end()) != header_.old_crc) {
    return false;
  }
  return true;
}
...
bool EnsemblePatchReader::CheckNewFile(ConstBufferView new_image) const {
  if (new_image.size() != header_.new_size)
    return false;
  if (CalculateCrc32(new_image.begin(), new_image.end()) != header_.new_crc) {
    return false;
  }
  return true;
}
