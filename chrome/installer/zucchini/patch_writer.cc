...
void RawDeltaSink::PutNext(const RawDeltaUnit& delta) {
  DCHECK_GE(delta.copy_offset, prev_copy_offset_);
  EncodeVarUInt<uint32_t>(delta.copy_offset - prev_copy_offset_,
...
void TargetSink::PutNext(uint32_t target) {
  ...
  // Targets are delta encoded and biased by -1, hence, we keep track of the
  // previous target, and add 1 for the bias.
