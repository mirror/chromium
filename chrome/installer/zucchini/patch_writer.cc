...
void EquivalenceSink::PutNext(const Equivalence& equivalence) {
  ...
  src_offset_ =
      base::CheckAdd(equivalence.src_offset, equivalence.length).ValueOrDie();
  dst_offset_ =
      base::CheckAdd(equivalence.dst_offset, equivalence.length).ValueOrDie();
}
...
void RawDeltaSink::PutNext(const RawDeltaUnit& delta) {
  ...
  copy_offset_compensation_ = base::CheckAdd(delta.copy_offset, 1).ValueOrDie();
...
void TargetSink::PutNext(uint32_t target) {
  ...
  target_compensation_ = base::CheckAdd(target, 1).ValueOrDie();
}
