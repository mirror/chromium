...
class TwoUnitOverlapTester {
 public:
  struct TestCase {
    TestCase(
        std::string unit_str_in,
        AddressTranslator::Status expected_in = AddressTranslator::kSuccess)
        : unit_str(unit_str_in), expected(expected_in) {}
    std::string unit_str;
    AddressTranslator::Status expected;
  };
...
TEST(AddressTranslatorTest, OverlapFromComment) {
  using AT = AddressTranslator;
  struct TestCase {
    TestCase(const char* rva_str_in,
             const char* offset_str_in,
             AT::Status expected_in = AT::kSuccess)
        : rva_str(rva_str_in),
          offset_str(offset_str_in),
