...
class TwoUnitOverlapTester {
 public:
  struct TestCase {
    std::string unit_str;
    AddressTranslator::Status expected = AddressTranslator::kSuccess;
  };
  ...
  static void RunTest(const std::string& unit_str1,
    ...
    for (size_t i = 0; i < test_cases.size(); ++i) {
      ...
      SimpleTest({unit_str1, unit_str2}, test_case.expected, name);
      // Switch order. Expect same results.
      SimpleTest({unit_str2, unit_str1}, test_case.expected, name);
    }
  }
};
...
TEST(AddressTranslatorTest, Empty) {
  ...
  // Optimized versions.
  AddressTranslator::CachedForOffsetToRva offset_to_rva(translator);
  AddressTranslator::CachedForRvaToOffset rva_to_offset(translator);
...
TEST(AddressTranslatorTest, OverlapFromComment) {
  ...
  struct {
    ...
    AT::Status expected = AT::kSuccess;
  } test_cases[] = {
      {"..ssssffff..", "..SSSSFFFF.."},
      {"..ssssffff..", "..SSSS..FFFF.."},
      {"..ssssffff..", "..FFFF..SSSS.."},
