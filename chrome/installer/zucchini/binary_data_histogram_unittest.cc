...
TEST(BinaryDataHistogramTest, Basic) {
  ...
  ConstBufferView region(test_data, n);
  ...
  for (size_t i = 0; i <= n; ++i) {
    prefix_histograms[i].Compute(ConstBufferView(region.begin(), i));
    suffix_histograms[i].Compute(ConstBufferView(region.begin() + i, n - i));
  }
