...
TEST(OutlierDetectorTest, Basic) {
  ...
  // No data: Useless, but should at least not cause error.
  detector = make_detector({});
  EXPECT_EQ(0, detector->DecideOutlier(0.0));
  // Single point: Also useless.
  ...
  // Two identical points: StdDev is 0, but falls back to built-in tolerance.
  ...
  // Sharpen distribution by clsutering toward norm: Now test is stricter.
  ...
  EXPECT_EQ(1, detector->DecideOutlier(0.7));
  // Shift numbers around: Mean is 0.3, and orders of data scrambpled.
  ...
  // Typical usage: Potential outlier would actually be part of the statistic!
