...
bool ImageIndex::Initialize(Disassembler* disasm) {
  ...
  for (const auto& group : ref_groups) {
    ...
    auto result = types_.emplace(group.type_tag(), TypeInfo{});
    DCHECK(result.second);
    result.first->second.traits = group.traits();
  }
  for (const auto& group : ref_groups) {
    DCHECK(kNoPoolTag != group.pool_tag());
    auto pos = pools_.find(group.pool_tag());
    if (pos == pools_.end())
      pos = pools_.emplace(group.pool_tag(), PoolInfo()).first;
    pos->second.types.push_back(group.type_tag());
  }

  for (const ReferenceGroup& group : ref_groups) {
...
bool ImageIndex::InsertReferences(TypeTag type, ReferenceReader&& ref_reader) {
  ...
  for (base::Optional<Reference> ref = ref_reader.GetNext(); ref.has_value();
    ...
    std::fill(type_tags_.begin() + ref->location,
              type_tags_.begin() + ref->location + traits.width,
              traits.type_tag);
    types_.at(traits.type_tag).references.push_back(*ref);
  }
  return true;
}
