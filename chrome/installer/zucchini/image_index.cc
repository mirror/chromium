...
bool ImageIndex::InsertReferences(Disassembler* disasm) {
  ...
  types_.resize(ref_groups.size());
  for (const auto& group : ref_groups) {
    DCHECK_LT(group.type_tag().value(), types_.size());
    types_[group.type_tag().value()].SetTraits(group.traits());
  ...
  for (const auto& group : ref_groups) {
    ...
    types_[group.type_tag().value()].InsertReferences(
    ...
    pools_[group.pool_tag().value()].InsertTargets(
        types_[group.type_tag().value()].references());
  }
