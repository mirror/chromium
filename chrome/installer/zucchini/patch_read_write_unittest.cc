...
// |buffer| is passed as pointer to avoid passing a temporay, which can causes
// dangling references.
template <class T>
...
// |buffer| is passed as pointer to avoid passing a temporary, which can causes
// dangling references.
template <class T>
...
bool operator==(const std::vector<uint8_t>& a, ConstBufferView b) {
...
TEST(RawDeltaSinkSourceSinkTest, Normal) {
  Vec data = {
      3,  0,  0,   0, 0, 0, 0, 0,  // raw_delta_skip size
      1,  3,  0,                   // raw_delta_skip content
      3,  0,  0,   0, 0, 0, 0, 0,  // raw_delta_diff size
...
TEST(EnsemblePatchTest, CheckFile) {
  Vec data = {
      ...
      0,    0,    0,    0,    0, 0, 0, 0,  // number of element
  };

  EnsemblePatchReader ensemble_patch_reader =
      TestInitialize<EnsemblePatchReader>(&data);

  Vec old_file = {0x10, 0x32, 0x54, 0x76, 0x98};
  Vec new_file = {0xBA, 0xDC, 0xFE};
