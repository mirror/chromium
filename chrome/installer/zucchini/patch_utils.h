...
// vector of patch 'elements', each containing data for patching individual
// elements.
...
// uncompressed size. This is a variable-length encoding for integer
// quantities that strips away null bytes.

// Writes |value| as a formatted varint in |dst| and returns an iterator
// pointing beyond the written region. |dst| is assumed to hold enough space.
// Typically, this will write to a vector using back insertion, e.g.:
...
// Tries to read a varint formatted unsigned integer from [|first|, |last|).
// If succesfull, writes result into |value| and returns an iterator pointing
