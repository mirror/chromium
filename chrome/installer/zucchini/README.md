...
**Raw delta unit**: Describes a FIX operation as a pair (copy offset, diff),
where copy offset describes the position in new file as an offset in the data
that was copied from the old file, and diff is the bytewise difference to apply.
...
**uint64**: 64-bit unsigned int, little-endian
**int64**:  64-bit signed int, little-endian
...
elements_count | uint64 | Number of patch elements.
...
**PatchElement**
Contains all the information required to patch a single element.
...
pool_count | uint64 | Number of pool.
extra_targets | ExtraTargetList[pool_count] | Lists of extra targets.

**PatchElementHeader**
Describes a correspondance between an element in old and in new files.

Name | Format | Description
--- | --- | ---
old_offset | uin32 | Position in old file at which the element starts.
new_offset | uin32 | Position in new file at which the element starts.
old_length | uin64 | Legnth of old element.
new_length | uin64 | Legnth of new element.
exe_type | uin64 | Executable type for this element, see `enum ExecutableType`.

**EquivalenceList**
Describes a vector of equivalence, where position in new image file is
ascending.
...
**RawDeltaList**
Describes a vector of raw delta units, with ascending copy offset.
...
**ReferenceDeltaList**
Describes a vector of reference deltas, in the order they appear in the new
image file. A reference delta is an signed integer representing a jump in a 
vector of targets.
...
**ExtraTargetList**
Describes a vector of additional targets in the new image file, in ascending order.
...
**Buffer<T>**
Describes a generic vector of data.

Name | Format | Description
--- | --- | ---
size |uint64 | Size of content in bytes.
