...
class ParseFlagsTest(unittest.TestCase):
  ...
  def testInputOutput(self):
    flags = makecab.ParseFlags(['foo.txt', 'd/foo.cab'])
    self.assertEquals(flags.input, 'foo.txt')
    self.assertEquals(flags.output, 'd/foo.cab')
    self.assertEquals(flags.output_dir, '.')

  def testInputOutdir(self):
    flags = makecab.ParseFlags(['/L', 'outdir', 'foo.txt'])
    self.assertEquals(flags.input, 'foo.txt')
    self.assertEquals(flags.output, 'foo.tx_')
...
class WriteCabTest(unittest.TestCase):
  def testWellCompressingInput(self):
    ...
    if sys.platform == 'win32' or True:
      ...
      os.write(cab_handle, output.value())
      os.close(cab_handle)
      out_handle, out_file = tempfile.mkstemp(suffix='.makecab_test.out')
      ...
      os.close(out_handle)
      subprocess.check_call(['expand.exe', cab_path, out_path])
