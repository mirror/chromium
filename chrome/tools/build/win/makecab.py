...
def ParseFlags():
  ...
  if not os.path.exists(input):
    print('makecab.py: error: input file %s does not exist' % input,
          file=sys.stderr)
    sys.exit(1)
  if not output:
...
def WriteCab(flags):
  ...
  output_file = open(os.path.join(flags.output_dir, flags.output), 'wb')
  ...
  input_mtime = datetime.datetime.fromtimestamp(os.path.getmtime(flags.input))
  ...
  input_file = open(flags.input, 'rb')
