...
class FFDecryptorClientListener
    : public firefox_importer_unittest_utils_mac::mojom::FFDecryptor {
 public:
  explicit FFDecryptorClientListener(
      firefox_importer_unittest_utils_mac::mojom::FFDecryptorRequest request)
      : binding_(this, std::move(request)) {}

  void SetQuitClosure(base::Closure quit_closure) {
    quit_closure_ = std::move(quit_closure);
    binding_.set_connection_error_handler(quit_closure);
  }
 ...
 private:
  ...
  mojo::Binding<FFDecryptor> binding_;
...
class FFDecryptorServerChannelListener {
 public:
  explicit FFDecryptorServerChannelListener(
      firefox_importer_unittest_utils_mac::mojom::FFDecryptorPtr decryptor)
  ...
  void QuitClient() { decryptor_->Quit(); }
 ...
 private:
  ...
  firefox_importer_unittest_utils_mac::mojom::FFDecryptorPtr decryptor_;
...
bool FFUnitTestDecryptorProxy::Setup(const base::FilePath& nss_path) {
  ...
  firefox_importer_unittest_utils_mac::mojom::FFDecryptorPtr decryptor(
      firefox_importer_unittest_utils_mac::mojom::FFDecryptorPtrInfo(
