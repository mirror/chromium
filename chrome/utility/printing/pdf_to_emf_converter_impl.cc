...
std::vector<mojom::PdfToEmfConverterClientPtr>* g_converter_clients;

void PreCacheFontCharacters(const LOGFONT* logfont,
                            const wchar_t* text,
                            size_t text_length) {
  if (g_converter_clients->empty()) {
    NOTREACHED()
  ...
  memcpy(&(logfont_mojo.at(0)), logfont, sizeof(LOGFONT));

  g_converter_clients->front()->PreCacheFontCharacters(
...
void OnConvertedClientDisconnected() {
  ...
  std::remove_if(g_converter_clients->begin(), g_converter_clients->end(),
...
void RegisterConverterClient(mojom::PdfToEmfConverterClientPtr client) {
  if (!g_converter_clients) {
    // First time this method is called.
    g_converter_clients = new std::vector<mojom::PdfToEmfConverterClientPtr>();
  ...
  g_converter_clients->push_back(std::move(client));
}
...
PdfToEmfConverterImpl::PdfToEmfConverterImpl(
  ...
  LOG(ERROR) << "** JAY ** UTILITY PdfToEmfConverterImpl";
...
void PdfToEmfConverterImpl::LoadPdf(base::File pdf_file) {
  ...
  LOG(ERROR) << "** JAY ** UTILITY PdfToEmfConverterImpl::LoadPdf file size="
             << length64;
...
bool PdfToEmfConverterImpl::RenderPdfPageToMetafile(int page_number,
                                                    ...
                                                    bool postscript) {
  LOG(ERROR)
      << "** JAY ** UTILITY PdfToEmfConverterImpl::RenderPdfPageToMetafile";
  Emf metafile;
  metafile.Init();
...
void PdfToEmfConverterImpl::ConvertPage(uint32_t page_number,
  ...
  LOG(ERROR) << "** JAY ** UTILITY PdfToEmfConverterImpl::ConvertPage";
  ...
  if (mojo::UnwrapPlatformFile(std::move(emf_file_out), &emf_file) !=
      MOJO_RESULT_OK) {
    LOG(ERROR) << "Invalid EMF file passed to PdfToEmfConverterImpl";
    std::move(callback).Run(/*success=*/false, /*scale_factor=*/0.0);
    return;
  }
