...
ChromeWinUtilImpl::ChromeWinUtilImpl(
    std::unique_ptr<service_manager::ServiceContextRef> service_ref)
    : service_ref_(std::move(service_ref)) {}

ChromeWinUtilImpl::~ChromeWinUtilImpl() = default;

void ChromeWinUtilImpl::IsPinnedToTaskbar(IsPinnedToTaskbarCallback callback) {
...
void ChromeWinUtilImpl::CallGetOpenFileName(
    uint32_t owner,
    uint32_t flags,
...
void ChromeWinUtilImpl::CallGetSaveFileName(
    uint32_t owner,
    uint32_t flags,
    const std::vector<std::tuple<base::string16, base::string16>>& filters,
