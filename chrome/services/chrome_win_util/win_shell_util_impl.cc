...
WinShellUtilImpl::WinShellUtilImpl(
    std::unique_ptr<service_manager::ServiceContextRef> service_ref)
    : service_ref_(std::move(service_ref)) {}

WinShellUtilImpl::~WinShellUtilImpl() = default;

void WinShellUtilImpl::IsPinnedToTaskbar(IsPinnedToTaskbarCallback callback) {
...
void WinShellUtilImpl::CallGetOpenFileName(
    uint32_t owner,
    uint32_t flags,
...
void WinShellUtilImpl::CallGetSaveFileName(
    uint32_t owner,
    uint32_t flags,
    const std::vector<std::tuple<base::string16, base::string16>>& filters,
