...
SupervisedUserGoogleAuthNavigationThrottle::ShouldProceed() {
  ChildAccountService::AuthStatus authStatus =
      child_account_service_->IsGoogleAuthenticated();
  if (authStatus == ChildAccountService::AuthStatus::AUTHENTICATED)
    return content::NavigationThrottle::PROCEED;
  if (authStatus == ChildAccountService::AuthStatus::AUTH_STATUS_PENDING)
