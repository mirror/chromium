...
ChildAccountService::AuthStatus ChildAccountService::IsGoogleAuthenticated() {
  std::vector<gaia::ListedAccount> accounts;
  std::vector<gaia::ListedAccount> signed_out_accounts;
  if (!gaia_cookie_manager_->ListAccounts(&accounts, &signed_out_accounts,
                                          kGaiaCookieManagerSource)) {
    return AuthStatus::AUTH_STATUS_PENDING;
  }
  return accounts.empty() ? AuthStatus::NOT_AUTHENTICATED
