...
IN_PROC_BROWSER_TEST_F(WebViewGuestScrollLatchingTest,
  ...
  scroll_begin.global_x = scroll_begin.x + guest_rect.x();
  scroll_begin.global_y = scroll_begin.y + guest_rect.y();
  ...
  guest_contents->GetRenderViewHost()->GetWidget()->AddInputEventObserver(
      &update_waiter);
  blink::WebGestureEvent scroll_update(scroll_begin);
  scroll_update.SetType(blink::WebGestureEvent::kGestureScrollUpdate);
