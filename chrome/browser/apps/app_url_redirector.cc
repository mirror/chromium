...
bool LaunchAppWithUrl(
  ...
  if (app->from_bookmark()) {
    // If we are in the same app that is navigating no need to intercept
    // the navigation. We can't know if we are in the same app if the
    // there is no browser so we do not intercept the navigation.
