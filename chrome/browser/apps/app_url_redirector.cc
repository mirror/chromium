...
bool ShouldOverrideNavigation(
  ...
  bool is_from_app;
  std::tie(disposition, was_target_contents_null, is_from_app) =
      chrome::GetExtraNavigationInfo(source);
  ...
  // When a user clicks a target=_blank link or the page calls window.open(),
  // the renderer creates a new window and passes a target contents. We only
  // want to intercept navigations where the target contents was not null.
  if (disposition == WindowOpenDisposition::NEW_FOREGROUND_TAB &&
      was_target_contents_null) {
