...
class DriveAppConverter::IconFetcher : public ImageDecoder::ImageRequest {
 public:
  ...
  void Start() {
    ...
    content::ResourceRequest resource_request_;
    resource_request = content::ResourceRequest();
    resource_request.url = icon_url_;
    resource_request.load_flags = net::LOAD_DO_NOT_SAVE_COOKIES);
    content::mojom::URLLoaderFactory* loader_factory =
        content::BrowserContext::GetDefaultStoragePartition(
            converter_->profile_)
            ->GetURLLoaderForBrowserProcess();
    content::SimpleURLLoader::BodyAsStringCallback callback =
