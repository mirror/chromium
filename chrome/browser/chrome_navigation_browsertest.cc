...
class SigninIsolationBrowserTest : public ChromeNavigationBrowserTest {
 public:
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    command_line->AppendSwitchASCII(
        ::switches::kGaiaUrl,
        https_server()->GetURL("accounts.google.com", "/"));

    // Ignore cert errors so that the signin URL can be loaded from a site other
...
IN_PROC_BROWSER_TEST_F(SigninIsolationBrowserTest, SigninOriginIsIsolated) {
  const GURL url = embedded_test_server()->GetURL("google.com", "/title1.html");
  ...
  // Make sure that a renderer-initiated navigation to the signin page swaps
  // processes.
  content::TestNavigationManager manager(web_contents, signin_url);
  EXPECT_TRUE(
      ExecuteScript(web_contents, "location = '" + signin_url.spec() + "';"));
  manager.WaitForNavigationFinished();
  EXPECT_NE(web_contents->GetMainFrame()->GetSiteInstance(), foo_instance);
}
