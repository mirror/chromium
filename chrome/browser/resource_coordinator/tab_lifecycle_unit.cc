...
constexpr base::TimeDelta TabLifecycleUnit::kAudioProtectionTime;
constexpr base::TimeDelta TabLifecycleUnit::kFocusedProtectionTime;
...
void TabLifecycleUnit::SetRecentlyAudible(bool recently_audible) {
...
bool TabLifecycleUnit::IsMediaTab() const {
  if (recently_audible_)
    return true;
...
content::RenderProcessHost* TabLifecycleUnit::GetRenderProcessHost() const {
...
bool TabLifecycleUnit::CanDiscard(DiscardReason reason) const {
  ...
  if (time_since_focused < kFocusedProtectionTime)
    return false;
...
void TabLifecycleUnit::OnDiscardedStateChange() {
  for (auto& observer : *observers_)
    observer.OnDiscardedStateChange(GetWebContents(), IsDiscarded());
}

void TabLifecycleUnit::DidStartLoading() {
