...
class ResourceCoordinatorRenderProcessProbe {
 public:
  ...
  // Checks to see if field trial parameters exist and should overwrite the
  // probe defaults. Returns |true| if overrides were successfully applied.
  void CheckForFieldTrialParamsOverrides();
  ...
  void SendRenderProcessMetricsToResourceCoordinatorServiceOnUIThread();
 ...
 private:
  ...
  struct RenderProcessInfo {
    ...
    size_t last_measurement_tick_active;
    std::unique_ptr<base::ProcessMetrics> metrics;
  };

  using RenderProcessInfoMap =
      std::unordered_map<base::ProcessHandle, RenderProcessInfo>;

  ResourceCoordinatorRenderProcessProbe();
  ~ResourceCoordinatorRenderProcessProbe();

  // A map of currently running ProcessHandles to Process.
  RenderProcessInfoMap render_process_info_;

  // Time duration between measurements in milliseconds.
  base::TimeDelta measurement_interval_msec_;
  ...
  size_t current_measurement_tick_ = 0u;
