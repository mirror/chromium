...
void TabManagerDelegate::LowMemoryKillImpl(
  ...
  std::vector<TabManagerDelegate::Candidate> candidates = GetSortedCandidates(
      tab_manager_->GetSortedLifecycleUnits(), arc_processes);
...
void TabManagerDelegate::AdjustOomPrioritiesImpl(
  ...
  if (tab_manager_)
    lifecycle_units = tab_manager_->GetSortedLifecycleUnits();
  auto all_candidates =
  ...
  for (auto cur = begin; cur != end; ++cur) {
    ...
    if (cur->app()) {
      ...
      TabLifecycleUnitExternal* tab_lifecycle_unit_external =
          cur->lifecycle_unit()->AsTabLifecycleUnitExternal();
      DCHECK(tab_lifecycle_unit_external);
      pid = tab_lifecycle_unit_external->GetWebContents()
                ->GetMainFrame()
                ->GetProcess()
                ->GetHandle();
