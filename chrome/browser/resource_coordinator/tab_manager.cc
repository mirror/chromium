...
bool TabManager::IsTabDiscarded(content::WebContents* contents) const {
  const TabLifecycleUnitExternal* tab_lifecycle_unit_external =
...
WebContents* TabManager::DiscardTabByExtension(content::WebContents* contents) {
  ...
  // TODO(fdoray): Discard only if it's a tab?
...
void TabManager::AddObserver(TabLifecycleObserver* observer) {
  TabLifecycleUnitExternal::AddObserver(observer);
}

void TabManager::RemoveObserver(TabLifecycleObserver* observer) {
  TabLifecycleUnitExternal::RemoveObserver(observer);
}
...
void TabManager::UpdateTimerCallback() {
  ...
  delegate_->AdjustOomPriorities(lifecycle_units_);
#endif
...
content::WebContents* TabManager::DiscardTabImpl(DiscardReason reason) {
  ...
  std::vector<LifecycleUnitAndSortKey> sorted_lifecycle_units;
  sorted_lifecycle_units.reserve(lifecycle_units_.size());
  for (auto* lifecycle_unit : lifecycle_units_)
    sorted_lifecycle_units.emplace_back(lifecycle_unit);
  std::sort(sorted_lifecycle_units.begin(), sorted_lifecycle_units.end());

  for (auto& lifecycle_unit_and_sort_key : sorted_lifecycle_units) {
    if (lifecycle_unit_and_sort_key.lifecycle_unit->CanDiscard(reason) &&
        lifecycle_unit_and_sort_key.lifecycle_unit->Discard(reason)) {
      TabLifecycleUnitExternal* tab_lifecycle_unit_external =
          lifecycle_unit_and_sort_key.lifecycle_unit
              ->AsTabLifecycleUnitExternal();

      // For now, all LifecycleUnits in TabManager are TabLifecycleUnitExternal.
