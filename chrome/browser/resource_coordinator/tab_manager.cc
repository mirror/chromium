...
TabStatsList TabManager::GetUnsortedTabStats() const {
  ...
  // The first window in the list returned by GetBrowserInfoList() is considered
  // active.
...
bool TabManager::CompareTabStats(const TabStats& first,
  ...
  // Protect tab in the active window.
  if (first.is_in_active_window != second.is_in_active_window)
    return first.is_in_active_window;

  // Protect tab in a visible window.
