...
TabManager::MaybeThrottleNavigation(BackgroundTabNavigationThrottle* throttle) {
  ...
  pending_throttles_.push_back(throttle);
...
void TabManager::OnDidFinishNavigation(
  ...
  auto it = pending_throttles_.begin();
  while (it != pending_throttles_.end()) {
    BackgroundTabNavigationThrottle* pending_throttle = *it;
    if (pending_throttle->navigation_handle() == navigation_handle) {
...
void TabManager::LoadNextBackgroundTabIfNeeded() {
  ...
  if (pending_throttles_.empty())
    return;

  BackgroundTabNavigationThrottle* pending_throttle =
      pending_throttles_.front();
  pending_throttles_.erase(pending_throttles_.begin());
  ResumeNavigation(pending_throttle);
}
...
bool TabManager::IsNavigationDelayedForTest(
  ...
  for (auto* it : pending_throttles_) {
