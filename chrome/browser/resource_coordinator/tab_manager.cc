...
TabManager::MaybeThrottleNavigation(BackgroundTabNavigationThrottle* throttle) {
  ...
  // loading is already controlled by TabLoader. Do not populate
  // |loading_contents_| either, which is used to determine BackgroundTabOpening
  // session, so that the two activities are separate.
