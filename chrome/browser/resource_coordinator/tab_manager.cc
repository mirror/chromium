...
WebContents* TabManager::DiscardWebContentsAt(int index, TabStripModel* model) {
  ...
  // it is activated (it is reloaded explicitly in TabManager::ActiveTabChanged
  // or TabManager::OnBrowserSetLastActive when appropriate).
...
void TabManager::ActiveTabChanged(content::WebContents* old_contents,
  ...
  // Determine whether |new_contents| is in the last active window.
  bool web_contents_browser_is_last_active = false;
  const auto browser_info_list = GetBrowserInfoList();
  if (!browser_info_list.empty()) {
    TabStripModel* tab_strip_model = browser_info_list.front().tab_strip_model;
    for (int index = 0; index < tab_strip_model->count(); index++) {
      if (tab_strip_model->GetWebContentsAt(index) == new_contents) {
        web_contents_browser_is_last_active = true;
        break;
      }
    }
  }
  ...
  if (web_contents_browser_is_last_active) {
