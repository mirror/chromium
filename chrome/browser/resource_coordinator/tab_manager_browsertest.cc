...
IN_PROC_BROWSER_TEST_F(TabManagerTest, TabManagerBasics) {
  ...
  EXPECT_TRUE(tab_manager->DiscardTabImpl(false /*allow_unsafe_shutdown*/));
  EXPECT_EQ(3, tsm->count());
  ...
  // tab.
  EXPECT_FALSE(tab_manager->DiscardTabImpl(false /*allow_unsafe_shutdown*/));
  EXPECT_TRUE(tab_manager->IsTabDiscarded(tsm->GetWebContentsAt(0)));
  EXPECT_TRUE(tab_manager->IsTabDiscarded(tsm->GetWebContentsAt(1)));
  EXPECT_FALSE(tab_manager->IsTabDiscarded(tsm->GetWebContentsAt(2)));
  ...
  EXPECT_FALSE(tab_manager->IsTabDiscarded(tsm->GetWebContentsAt(1)));
  tab_manager->DiscardWebContentsAt(2, tsm, false /*allow_unsafe_shutdown*/);
...
IN_PROC_BROWSER_TEST_F(TabManagerTest, ProtectRecentlyUsedTabs) {
  ...
  ASSERT_FALSE(tab_manager->DiscardTabImpl(false /*allow_unsafe_shutdown*/));
...
IN_PROC_BROWSER_TEST_F(TabManagerTest, FastShutdownBlockedByUnloadHandler) {
  TabManager* tab_manager = g_browser_process->GetTabManager();
  ...
  content::WindowedNotificationObserver load2(
      content::NOTIFICATION_LOAD_COMPLETED_MAIN_FRAME,
      content::NotificationService::AllSources());
  OpenURLParams open2(embedded_test_server()->GetURL("/beforeunload.html"),
                      content::Referrer(),
                      WindowOpenDisposition::NEW_BACKGROUND_TAB,
                      ui::PAGE_TRANSITION_TYPED, false);
  browser()->OpenURL(open2);
  ...
  // The tab with the beforeunload handler should be successfully discarded,
  // but using the slow path rather than the fast process shutdown.
  tester.ExpectUniqueSample(
      "TabManager.Discarding.DiscardedTabCouldFastShutdown", false, 1);
}

IN_PROC_BROWSER_TEST_F(TabManagerTest, UnsafeFastShutdownSingleTabProcess) {
  TabManager* tab_manager = g_browser_process->GetTabManager();
  ...
  content::WindowedNotificationObserver load1(
      content::NOTIFICATION_LOAD_COMPLETED_MAIN_FRAME,
      content::NotificationService::AllSources());
  OpenURLParams open1(GURL(chrome::kChromeUICreditsURL), content::Referrer(),
                      WindowOpenDisposition::CURRENT_TAB,
                      ui::PAGE_TRANSITION_TYPED, false);
  browser()->OpenURL(open1);
  load1.Wait();
  ...
  EXPECT_TRUE(tab_manager->DiscardTabImpl(true /*allow_unsafe_shutdown*/));
...
IN_PROC_BROWSER_TEST_F(TabManagerTest, FastShutdownBlockedByProcessSharing) {
  TabManager* tab_manager = g_browser_process->GetTabManager();
  ...
  // Tab discarding should still happen, but no fast shutdown should
  // occur.
  EXPECT_TRUE(tab_manager->DiscardTabImpl(true /*allow_unsafe_shutdown*/));
