...
bool TabLifetimeUnitImpl::CanDiscard(MemoryCondition memory_condition) const {
...
void TabLifetimeUnitImpl::Discard(MemoryCondition memory_condition) {
  if (state_ == State::DISCARDED)
    return;
...
#if defined(OS_CHROMEOS)
  if (!fast_shutdown_success && memory_condition == MemoryCondition::CRITICAL) {
...
void TabLifetimeUnitImpl::OnDiscardedStateChange() {
  LOG(ERROR) << "notify with " << (state_ == State::DISCARDED);
