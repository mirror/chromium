...
const char* kBackgroundTabOpeningName = "BackgroundTabOpening";
...
void TabManagerStatsCollector::OnSwapInCount(SessionType type,
  ...
  DCHECK(
      ...
      (type == kBackgroundTabOpening && is_in_background_tab_opening_session_));
...
std::string TabManagerStatsCollector::GetEventName(SessionType type) const {
  switch (type) {
    ...
    case kBackgroundTabOpening:
      return kBackgroundTabOpeningName;
    default:
      NOTREACHED();
  }
  return "";
