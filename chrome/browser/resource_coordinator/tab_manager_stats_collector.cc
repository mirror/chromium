...
bool ShouldReportExpectedTaskQueueingDurationToUKM(
  ...
  DCHECK_GE(total_count, 1u);

  // |total_count| is 1 for more than 80% of the cases. So do not sample the
  // metric when |total_count| is greater than 1.
  if (total_count > 1)
    return true;
  ...
  if (is_in_background_tab_opening_session_) {
    ...
    if (ukm_source_id != 0 &&
        ShouldReportExpectedTaskQueueingDurationToUKM(
            background_tab_loading_count, background_tab_pending_count)) {
      ukm::builders::
          TabManager_BackgroundTabOpening_ForegroundTab_ExpectedTaskQueueingDurationGenerated(
              ukm_source_id)
