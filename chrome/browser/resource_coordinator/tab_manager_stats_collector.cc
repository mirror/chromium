...
TabManagerStatsCollector::TabManagerStatsCollector(TabManager* tab_manager)
  ...
  std::unique_ptr<content::SwapMetricsDriver::Delegate>
      session_restore_delegate(
  ...
  session_restore_swap_metrics_driver_ = content::SwapMetricsDriver::Create(
      std::move(session_restore_delegate), base::TimeDelta::FromSeconds(0));
  SessionRestore::AddObserver(this);

  std::unique_ptr<content::SwapMetricsDriver::Delegate>
      background_tab_open_delegate(
          base::WrapUnique<content::SwapMetricsDriver::Delegate>(
              new SwapMetricsDelegate(this, kBackgroundTabOpen)));
  background_tab_open_swap_metrics_driver_ = content::SwapMetricsDriver::Create(
      std::move(background_tab_open_delegate), base::TimeDelta::FromSeconds(0));
}
...
void TabManagerStatsCollector::CheckEventStatus(EventType type) const {
  switch (type) {
    case kSessionRestore:
      DCHECK(is_session_restore_loading_tabs_);
      break;
    case kBackgroundTabOpen:
      DCHECK(tab_manager_->IsLoadingBackgroundTabs());
      break;
    default:
      NOTREACHED();
  }
