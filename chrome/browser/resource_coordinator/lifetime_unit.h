...
// provides a functionality to the user. When it is discarded, the unit doesn't
...
class LifetimeUnit {
 public:
  ...
  // Returns |this| casted to a TabLifetimeUnit if possible.
  ...
  // LifetimeUnit switched to the DISCARDED state.
  virtual int GetEstimatedMemoryFreedOnDiscardKB() const = 0;

  // Returns true if this LifetimeUnit can be switched to the DISCARDED state.
  virtual bool CanDiscard(MemoryCondition memory_condition) const = 0;

  // Switches this LifetimeUnit to the DISCARDED state.
  virtual void Discard(MemoryCondition memory_condition) = 0;
};
