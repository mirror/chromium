...
//
// The algorithm used favors killing tabs that are not in a visible window, not
// active, not pinned, and have been idle for longest, in that order of
// priority.
//
...
class TabManager : public TabStripModelObserver,
 ...
 private:
  ...
  // Returns true if the |web_contents| is WebContents for the currently
  // active tab in an active Browser.
  ...
  // z-order, from top to bottom. The first BrowserInfo is considered active.
