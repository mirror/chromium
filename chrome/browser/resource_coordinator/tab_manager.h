...
class TabManager : public LifecycleUnitSourceObserver,
 ...
 public:
  ...
  // Returns the LifecycleUnits managed by this.
  const LifecycleUnitSet& lifecycle_units() const { return lifecycle_units_; }
  ...
  // Discards the LifecycleUnit with the given unique ID. The tab still exists
  // in the tab-strip; clicking on it will reload it. Returns null if the tab
  // cannot be found or cannot be discarded. Otherwise returns the new
  // web_contents of the discarded tab.
