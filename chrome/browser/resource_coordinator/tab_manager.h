...
class TabManager : public TabStripModelObserver,
 ...
 private:
  ...
  // session, the sesion can end when background tabs' loading are paused due to
  // memory pressure. A new session starts when background tabs' loading resume
  // when memory pressure returns to normal.
