...
#include "chrome/browser/resource_coordinator/browser_iterator.h"
...
class TabManager : public TabStripModelObserver,
 ...
 private:
  ...
  // corresponds to the most recently active Browser.
  ...
  void SetBrowserIteratorForTest(
      std::unique_ptr<BrowserIterator> test_browser_iterator) {
    // TODO move?
    browser_iterator_ = std::move(test_browser_iterator);
  }
  ...
  // Delegate that enumerates existing browsers, most recently active first.
  // Allows tests to inject their own BrowserInfo lists.
  std::unique_ptr<BrowserIterator> browser_iterator_;
