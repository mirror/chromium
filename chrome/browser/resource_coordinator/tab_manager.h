...
class TabManager : public TabStripModelObserver,
 ...
 private:
  ...
  // Returns true if it is in BackgroundTabOpen session, which is defined as the
  // duration from the time when the browser starts to load background tabs
  // until the time when browser has finished loading those tabs. For more
  // details, refer to the definition in tab_manager_stats_collector.h.
