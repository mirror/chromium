...
class TabManager : public TabStripModelObserver,
 ...
 private:
  ...
  // Returns the number of alive tabs, i.e. tabs that are not pending load or
  // discarded.
  ...
  // Last time at which TabManager had to choose one or many tabs to discard
  // urgently, or a null TimeTicks if that never happened. Does not include
  // requests to discard a specific tab.
