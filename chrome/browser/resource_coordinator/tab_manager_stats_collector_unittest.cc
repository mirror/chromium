                    ...
                    bool>>  // should_test_background_tab_opening
{
 protected:
  ...
  TabManagerStatsCollector* get_tab_manager_stats_collector() {
    return tab_manager_.tab_manager_stats_collector_.get();
  }

  void StartSessionRestore() {
    get_tab_manager_stats_collector()->OnSessionRestoreStartedLoadingTabs();
  }

  void FinishSessionRestore() {
    get_tab_manager_stats_collector()->OnSessionRestoreFinishedLoadingTabs();
  }

  void StartBackgroundTabOpeningSession() {
    get_tab_manager_stats_collector()->OnBackgroundTabOpeningSessionStarted();
  }

  void FinishBackgroundTabOpeningSession() {
    get_tab_manager_stats_collector()->OnBackgroundTabOpeningSessionEnded();
  }
...
class TabManagerStatsCollectorTabSwitchTest
 ...
 protected:
  ...
  void FinishLoadingForegroundTab() {
    ...
    get_tab_manager_stats_collector()->OnDidStopLoading(foreground_tab_);
  }

  void FinishLoadingBackgroundTab() {
    SetBackgroundTabLoadingState(TAB_IS_LOADED);
    get_tab_manager_stats_collector()->OnDidStopLoading(background_tab_);
  }

  void SwitchToBackgroundTab() {
    get_tab_manager_stats_collector()->RecordSwitchToTab(foreground_tab_,
                                                         background_tab_);
...
TEST_P(TabManagerStatsCollectorTest, HistogramsExpectedTaskQueueingDuration) {
  auto* stats_collector = get_tab_manager_stats_collector();
