...
TEST_F(TabManagerStatsCollectorTest,
  ...
  EXPECT_EQ(base::HashMetricName(UkmEntry::kEntryName),
            test_ukm_recorder_.GetEntry(0)->event_hash);
  const ukm::mojom::UkmMetric* metric = ukm::TestUkmRecorder::FindMetric(
      test_ukm_recorder_.GetEntry(0), UkmEntry::kSessionRestoreTabCountName);
  EXPECT_EQ(2, metric->value);
  metric = ukm::TestUkmRecorder::FindMetric(
      test_ukm_recorder_.GetEntry(0),
      UkmEntry::kExpectedTaskQueueingDurationName);
  EXPECT_EQ(10, metric->value);
  FinishSessionRestore();
