...
TEST_F(TabManagerStatsCollectorTest,
  ...
  EXPECT_EQ(base::HashMetricName("TabManager.SessionRestore.ForegroundTab."
                                 "ExpectedTaskQueueingDurationInfo"),
            test_ukm_recorder_.GetEntry(0)->event_hash);
  const ukm::mojom::UkmMetric* metric = ukm::TestUkmRecorder::FindMetric(
      test_ukm_recorder_.GetEntry(0), "SessionRestoreTabCount");
  EXPECT_EQ(2, metric->value);
  metric = ukm::TestUkmRecorder::FindMetric(test_ukm_recorder_.GetEntry(0),
