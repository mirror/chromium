...
ResourceCoordinatorRenderProcessMetricsHandler::
    ResourceCoordinatorRenderProcessMetricsHandler() = default;

ResourceCoordinatorRenderProcessMetricsHandler::
    ~ResourceCoordinatorRenderProcessMetricsHandler() = default;

bool ResourceCoordinatorRenderProcessMetricsHandler::HandleMetrics(
...
void ResourceCoordinatorRenderProcessProbe::
  ...
  for (content::RenderProcessHost::iterator rph_iter =
    ...
    render_process_info.last_gather_cycle_active = current_gather_cycle();
...
void ResourceCoordinatorRenderProcessProbe::
    HandleRenderProcessMetricsOnUIThread() {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  if (metrics_handler_->HandleMetrics(render_process_info_map())) {
