...
void ResourceCoordinatorRenderProcessProbe::
    CheckForFieldTrialParamsOverrides() {
  std::string measurement_interval_msec_param =
      base::GetFieldTrialParamValueByFeature(
          features::kGRCRenderProcessCPUProfiling, "measurement_interval_msec");

  if (measurement_interval_msec_param.empty()) {
    return;
  }

  unsigned measurement_interval_msec;
  if (!base::StringToUint(measurement_interval_msec_param,
                          &measurement_interval_msec)) {
    return;
  }

  measurement_interval_msec_ =
      base::TimeDelta::FromMilliseconds(measurement_interval_msec);
}
