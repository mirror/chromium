...
void TabLifecycleUnitSource::AddLifecycleUnitSourceObserver(
    LifecycleUnitSourceObserver* observer) {
  lifecycle_unit_source_observers_.AddObserver(observer);
}

void TabLifecycleUnitSource::RemoveLifecycleUnitSourceObserver(
    LifecycleUnitSourceObserver* observer) {
  lifecycle_unit_source_observers_.RemoveObserver(observer);
}
...
void TabLifecycleUnitSource::TabInsertedAt(TabStripModel* tab_strip_model,
  ...
  if (it == tabs_.end()) {
    ...
    for (auto& observer : lifecycle_unit_source_observers_)
      observer.OnLifecycleUnitCreated(res.first->second.get());
  } else {
