...
class TabLifecycleUnitSourceTest : public ChromeRenderViewHostTestHarness {
 protected:
  TabLifecycleUnitSourceTest()
    ...
    source_.AddLifecycleUnitSourceObserver(&source_observer_);
...
TEST_F(TabLifecycleUnitSourceTest, AppendTabsToFocusedTabStrip) {
  ...
  EXPECT_EQ(source_.GetTabLifecycleUnitExternal(first_web_contents),
            static_cast<TabLifecycleUnit*>(first_lifecycle_unit));
  ...
  EXPECT_EQ(source_.GetTabLifecycleUnitExternal(second_web_contents),
            static_cast<TabLifecycleUnit*>(second_lifecycle_unit));
  ...
  EXPECT_EQ(source_.GetTabLifecycleUnitExternal(third_web_contents),
            static_cast<TabLifecycleUnit*>(third_lifecycle_unit));
  ...
  EXPECT_CALL(source_observer_, OnLifecycleUnitDestroyed(third_lifecycle_unit));
  tab_strip_model_->CloseAllTabs();
}

TEST_F(TabLifecycleUnitSourceTest, AppendTabsToNonFocusedTabStrip) {
  // Add a foreground tab to a non-focused tab strip.
  LifecycleUnit* first_lifecycle_unit = nullptr;
  EXPECT_CALL(source_observer_, OnLifecycleUnitCreated(testing::_))
      .WillOnce(testing::Invoke([&](LifecycleUnit* lifecycle_unit) {
        first_lifecycle_unit = lifecycle_unit;
...
TEST_F(TabLifecycleUnitSourceTest, ReplaceWebContents) {
  ...
  EXPECT_EQ(static_cast<TabLifecycleUnit*>(first_lifecycle_unit),
...
TEST_F(TabLifecycleUnitSourceTest, CanOnlyDiscardOnce) {
  ...
  content::WebContents* initial_web_contents =
      tab_strip_model_->GetWebContentsAt(0);
  initial_web_contents->WasHidden();
