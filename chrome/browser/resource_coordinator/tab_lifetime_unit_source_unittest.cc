...
TEST_F(TabLifetimeUnitSourceTest, DiscardAndActivate) {
  ...
  lifetime_unit->Discard(MemoryCondition::NORMAL);
...
TEST_F(TabLifetimeUnitSourceTest, CanOnlyDiscardOnce) {
  ...
  EXPECT_TRUE(lifetime_unit->CanDiscard(MemoryCondition::NORMAL));
#else
  // On non-ChromeOS platforms, a tab cannot be discarded twice.
  EXPECT_FALSE(lifetime_unit->CanDiscard(MemoryCondition::NORMAL));
#endif
}
