...
class TabManagerStatsCollector : public SessionRestoreObserver {
 public:
  enum SessionType {
    ...
    // finished loading those tabs or faused loading due to memory pressure.
    // During the BackgroundTabOpening session, some background tabs could
    // become foreground due to user action, but that would not affect the
    // session end time point. For example, a browser has tab1 in foreground,
    // and starts to open tab2 and tab3 in background. The session will end
    // after tab2 and tab3 finishes loading, even if tab2 and tab3 are brought
    // to foreground before they are loaded. BackgroundTabOpening session
