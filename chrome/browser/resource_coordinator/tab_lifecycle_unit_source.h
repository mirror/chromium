...
// created / destroyed.
class TabLifecycleUnitSource : public TabStripModelObserver,
 ...
 public:
  ...
  void AddLifecycleUnitSourceObserver(LifecycleUnitSourceObserver* observer);
  void RemoveLifecycleUnitSourceObserver(LifecycleUnitSourceObserver* observer);
 ...
 private:
  ...
  // for each tab in the current process.
  base::flat_map<content::WebContents*, std::unique_ptr<TabLifecycleUnit>>
      tabs_;

  // Observers notified when a TabLifecycleUnit is created or destroyed.
  base::ObserverList<LifecycleUnitSourceObserver>
      lifecycle_unit_source_observers_;
