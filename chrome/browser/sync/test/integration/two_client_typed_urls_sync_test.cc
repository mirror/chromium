...
using typed_urls_helper::CheckSyncHasURL;
...
IN_PROC_BROWSER_TEST_F(TwoClientTypedUrlsSyncTest, AddExpired) {
  ...
  ASSERT_EQ(1U, urls.size());
  EXPECT_TRUE(CheckSyncHasURL(0, GURL(kDummyUrl)));

  // Sync should not receive expired visits.
  EXPECT_FALSE(CheckSyncHasURL(0, new_url));
}
...
IN_PROC_BROWSER_TEST_F(TwoClientTypedUrlsSyncTest, AddExpiredThenUpdate) {
  ...
  EXPECT_TRUE(CheckSyncHasURL(0, new_url));
  EXPECT_TRUE(CheckSyncHasURL(1, new_url));
}
