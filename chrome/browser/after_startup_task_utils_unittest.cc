...
class AfterStartupTaskTest : public testing::Test {
 public:
  AfterStartupTaskTest() {
    ...
    foreign_sequence_ = base::MakeRefCounted<WrappedTaskRunner>(
        base::CreateSequencedTaskRunnerWithTraits(base::TaskTraits()));
    AfterStartupTaskUtils::UnsafeResetForTesting();
  }

  // Hop to the foreign sequence and call IsBrowserStartupComplete.
  bool GetIsBrowserStartupCompleteFromForeignSequence() {
    base::RunLoop run_loop;
    bool is_complete;
    base::PostTaskAndReplyWithResult(
        foreign_sequence_->real_runner(), FROM_HERE,
  ...
  // Hop to the foreign sequence and call PostAfterStartupTask.
  void PostAfterStartupTaskFromForeignSequence(
      const tracked_objects::Location& from_here,
      scoped_refptr<base::TaskRunner> task_runner,
      base::OnceClosure task) {
    base::RunLoop run_loop;
    foreign_sequence_->real_runner()->PostTaskAndReply(
        FROM_HERE,
  ...
  // Make sure all tasks posted to the foreign sequence get run.
  void FlushForeignSequence() {
    base::RunLoop run_loop;
    foreign_sequence_->real_runner()->PostTaskAndReply(
 ...
 protected:
  ...
  scoped_refptr<WrappedTaskRunner> foreign_sequence_;
...
TEST_F(AfterStartupTaskTest, IsStartupComplete) {
  ...
  EXPECT_FALSE(GetIsBrowserStartupCompleteFromForeignSequence());
  ...
  EXPECT_TRUE(GetIsBrowserStartupCompleteFromForeignSequence());
}
...
TEST_F(AfterStartupTaskTest, PostTask) {
  ...
  AfterStartupTaskUtils::PostTask(
      FROM_HERE, foreign_sequence_,
      base::BindOnce(&AfterStartupTaskTest::VerifyExpectedSequence,
                     base::RetainedRef(foreign_sequence_)));
  PostAfterStartupTaskFromForeignSequence(
      FROM_HERE, ui_thread_,
      base::BindOnce(&AfterStartupTaskTest::VerifyExpectedSequence,
                     base::RetainedRef(ui_thread_)));
  PostAfterStartupTaskFromForeignSequence(
  ...
  EXPECT_EQ(0, foreign_sequence_->total_task_count() +
                   ui_thread_->total_task_count());

  // Queued tasks should be posted upon setting the flag.
  AfterStartupTaskUtils::SetBrowserStartupIsCompleteForTesting();
  EXPECT_EQ(2, foreign_sequence_->posted_task_count());
  EXPECT_EQ(2, ui_thread_->posted_task_count());
  FlushForeignSequence();
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(2, foreign_sequence_->ran_task_count());
  EXPECT_EQ(2, ui_thread_->ran_task_count());

  foreign_sequence_->reset_task_counts();
  ...
  AfterStartupTaskUtils::PostTask(FROM_HERE, foreign_sequence_,
                                  base::BindOnce(&base::DoNothing));
  EXPECT_EQ(1, foreign_sequence_->posted_task_count());
  EXPECT_EQ(1, ui_thread_->posted_task_count());
  PostAfterStartupTaskFromForeignSequence(FROM_HERE, ui_thread_,
                                          base::BindOnce(&base::DoNothing));
  PostAfterStartupTaskFromForeignSequence(FROM_HERE, foreign_sequence_,
                                          base::BindOnce(&base::DoNothing));
  EXPECT_EQ(2, foreign_sequence_->posted_task_count());
  EXPECT_EQ(2, ui_thread_->posted_task_count());
  FlushForeignSequence();
...
// |foreign_sequence_| results in the same behavior as posting via
// AfterStartupTaskUtils::PostTask(..., foreign_sequence_, ...).
TEST_F(AfterStartupTaskTest, AfterStartupTaskUtilsRunner) {
  scoped_refptr<base::TaskRunner> after_startup_runner =
      base::MakeRefCounted<AfterStartupTaskUtils::Runner>(foreign_sequence_);
  ...
  FlushForeignSequence();
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(1, foreign_sequence_->ran_task_count());
