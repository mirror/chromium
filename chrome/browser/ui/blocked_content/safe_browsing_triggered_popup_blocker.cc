...
void SafeBrowsingTriggeredPopupBlocker::RegisterProfilePrefs(
  ...
  registry->RegisterBooleanPref(prefs::kAbusiveExperienceEnforce,
                                true /* default value */);
}
...
bool SafeBrowsingTriggeredPopupBlocker::IsEnabled(
  ...
  // is broken it can be disabled via finch irrespective of the policy.
  if (!base::FeatureList::IsEnabled(kAbusiveExperienceEnforce))
    return false;

  // If enterprise policy not set, return the feature value.
  if (profile->GetPrefs()
          ->FindPreference(prefs::kAbusiveExperienceEnforce)
          ->IsDefaultValue()) {
    return true;
  }

  return profile->GetPrefs()->GetBoolean(prefs::kAbusiveExperienceEnforce);
}
