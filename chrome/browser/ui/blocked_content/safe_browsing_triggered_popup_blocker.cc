...
const base::Feature kAbusiveAdEnforcement{"AbusiveAdEnforcement",
...
SafeBrowsingTriggeredPopupBlocker::MaybeCreate(
  ...
  if (!base::FeatureList::IsEnabled(kAbusiveAdEnforcement))
    return nullptr;
  return base::WrapUnique(new SafeBrowsingTriggeredPopupBlocker(web_contents));
}
...
void SafeBrowsingTriggeredPopupBlocker::DidFinishNavigation(
  ...
  is_triggered_for_current_committed_load_ =
      trigger_for_next_committed_navigation_;
  trigger_for_next_committed_navigation_ = false;
}
...
void SafeBrowsingTriggeredPopupBlocker::OnSafeBrowsingCheckComplete(
  ...
  if (threat_type !=
    ...
    return;
  switch (threat_metadata.threat_pattern_type) {
    ...
    case safe_browsing::ThreatPatternType::SUBRESOURCE_FILTER_ALL_ADS:
      trigger_for_next_committed_navigation_ = true;
      break;
    default:
      trigger_for_next_committed_navigation_ = ignore_sublists_;
      break;
  }
}
...
SafeBrowsingTriggeredPopupBlocker::SafeBrowsingTriggeredPopupBlocker(
    ...
    : content::WebContentsObserver(web_contents),
      ...
      ignore_sublists_(
          base::GetFieldTrialParamByFeatureAsBool(kAbusiveAdEnforcement,
