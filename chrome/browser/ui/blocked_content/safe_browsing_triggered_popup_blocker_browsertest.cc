...
IN_PROC_BROWSER_TEST_F(SafeBrowsingTriggeredPopupBlockerBrowserTest,
  ...
  profile->GetPrefs()->SetBoolean(prefs::kAbusiveExperienceEnforce, false);
...
IN_PROC_BROWSER_TEST_F(SafeBrowsingTriggeredPopupBlockerBrowserTest,
  ...
  // Disable Abusive enforcement from features.
  base::test::ScopedFeatureList scoped_feature_list;
  scoped_feature_list.InitFromCommandLine("", "AbusiveAdEnforcement");

  Profile* profile =
      Profile::FromBrowserContext(web_contents()->GetBrowserContext());
  profile->GetPrefs()->SetBoolean(prefs::kAbusiveExperienceEnforce, true);
