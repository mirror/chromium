...
bool PopupBlockerTabHelper::MaybeBlockPopup(
  ...
  //
  // Note: the subresource_filter code will log a message to devtools if it
  // disallows the new window. Therefore, it is best to make this check last, to
  // avoid the case where a message is logged but the popup is not blocked.
  if (user_gesture) {
