...
IN_PROC_BROWSER_TEST_F(PopupTrackerBrowserTest, WindowOpenPopup_HasTracker) {
  ...
  ASSERT_TRUE(content::ExecuteScriptAndExtractBool(
      ...
      &opened_window));
  ASSERT_TRUE(opened_window);
  ASSERT_EQ(2, browser()->tab_strip_model()->count());
  ASSERT_TRUE(browser()->tab_strip_model()->GetActiveWebContents()->GetUserData(
      PopupTracker::kUserDataKey));
}
...
IN_PROC_BROWSER_TEST_F(PopupTrackerBrowserTest, OpenURLPopup_HasTracker) {
  ...
  ASSERT_TRUE(BrowserList::GetInstance()
                  ->GetLastActive()
                  ->tab_strip_model()
                  ->GetActiveWebContents()
                  ->GetUserData(PopupTracker::kUserDataKey));
}
