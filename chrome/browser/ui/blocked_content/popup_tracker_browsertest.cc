...
const char kPopupType[] = "Popups.Type";
...
IN_PROC_BROWSER_TEST_F(PopupTrackerBrowserTest, NoPopup_NoTracker) {
  ...
  tester.ExpectTotalCount(kPopupType, 0);
}
...
IN_PROC_BROWSER_TEST_F(PopupTrackerBrowserTest, WindowOpenPopup_HasTracker) {
  ...
  tester.ExpectUniqueSample(
      kPopupType,
      static_cast<int>(PopupTracker::Type::kNewWindowTrustedEventUnknown), 1);
}
...
IN_PROC_BROWSER_TEST_F(PopupTrackerBrowserTest, ControlClick_HasTracker) {
  ...
  tester.ExpectTotalCount(kPopupEngagement, 1);
  tester.ExpectUniqueSample(
      kPopupType, static_cast<int>(PopupTracker::Type::kOpenUrlTrustedEvent),
      1);
}
...
IN_PROC_BROWSER_TEST_F(PopupTrackerBrowserTest, UntrustedClick) {
  base::HistogramTester tester;
  ui_test_utils::NavigateToURL(
      browser(), embedded_test_server()->GetURL(
                     "/popup_blocker/popup-simulated-click-on-anchor.html"));
  ...
  tester.ExpectTotalCount(kPopupEngagement, 1);
  tester.ExpectUniqueSample(
      kPopupType, static_cast<int>(PopupTracker::Type::kOpenUrlUntrustedEvent),
      1);
}
...
IN_PROC_BROWSER_TEST_F(PopupTrackerBrowserTest, WhitelistedPopup_HasTracker) {
  ...
  tester.ExpectTotalCount(kPopupEngagement, 1);
  tester.ExpectUniqueSample(
      kPopupType, static_cast<int>(PopupTracker::Type::kClickedThrough), 1);
}
