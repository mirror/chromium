...
void PopupOpenerTabHelper::OnOpenedPopup(PopupTracker* popup_tracker) {
  ...
  last_popup_open_time_before_tab_under_ = tick_clock_->NowTicks();
}
...
void PopupOpenerTabHelper::OnDidTabUnder() {
  ...
  DCHECK(!last_popup_open_time_before_tab_under_.is_null());
  UMA_HISTOGRAM_LONG_TIMES(
      "Tab.TabUnder.PopupToTabUnderTime",
      tick_clock_->NowTicks() - last_popup_open_time_before_tab_under_);
