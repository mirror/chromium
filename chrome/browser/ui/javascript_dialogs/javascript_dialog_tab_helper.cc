...
void SetTabNeedsAttention(content::WebContents* web_contents, bool attention) {
#if !defined(OS_ANDROID)
  ...
  if (!IsWebContentsForemost(parent_web_contents)) {
    switch (dialog_type) {
      case content::JAVASCRIPT_DIALOG_TYPE_ALERT: {
        ...
        SetTabNeedsAttention(parent_web_contents, true);
...
void JavaScriptDialogTabHelper::WasShown() {
  if (pending_dialog_) {
    ...
    SetTabNeedsAttention(web_contents, false);
  }
}
