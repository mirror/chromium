  ...
  if (!IsWebContentsForemost(parent_web_contents)) {
    switch (dialog_type) {
      case content::JAVASCRIPT_DIALOG_TYPE_ALERT: {
        ...
        Browser* browser =
            chrome::FindBrowserWithWebContents(parent_web_contents);
        DCHECK(browser);
        browser->tab_strip_model()->TabNeedsAttentionAt(
...
void JavaScriptDialogTabHelper::WasShown() {
  if (pending_dialog_) {
    ...
    DCHECK(browser);
    browser->tab_strip_model()->TabNeedsAttentionAt(
        browser->tab_strip_model()->GetIndexOfWebContents(web_contents), false);
  }
}
