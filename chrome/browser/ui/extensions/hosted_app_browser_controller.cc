...
bool HostedAppBrowserController::IsForInstalledPWA(
  ...
  if (!web_contents ||
    ...
    DVLOG(1) << "Event not for this browser's WebContents";
    return false;
  }

  if (!browser_->is_app()) {
    DVLOG(1) << "Browser is not an app";
    return false;
  }
  ...
  if (extensions::UrlHandlers::GetUrlHandlers(GetExtension()) == nullptr) {
    DVLOG(1) << "No URL handlers (not a PWA)";
    return false;
  }

  DVLOG(1) << "Browser window is an installed PWA";
  return true;
}
