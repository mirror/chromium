...
class ExtensionInstalledBubbleBrowserTest
 ...
 public:
  ...
  BubbleController* ExtensionBubbleControllerFromManager(
  ...
  BubbleUi* BubbleUiFromManager(BubbleManager* manager) const {
    BubbleController* controller =
        ExtensionBubbleControllerFromManager(manager);
...
IN_PROC_BROWSER_TEST_F(ExtensionInstalledBubbleBrowserTest, CloseBubbleUi) {
  ...
  BubbleUi* bubble_ui = BubbleUiFromManager(manager);
  DCHECK(bubble_ui);
  bubble_ui->Close();

  // Bubble should no longer be in the manager
  EXPECT_FALSE(ExtensionBubbleControllerFromManager(manager));
}
