...
// Presentation API.  It handles creating the window to show the desired URL,
// showing it in fullscreen on the desired display, and setting up the
// Presentation API messaging pipes.  This class should not be destroyed until
// Terminate() has been called and it has completed, which may be asynchronous.
class PresentationReceiverWindowController final
 ...
 public:
  ...
  CreateFromOriginalProfile(Profile* profile, const gfx::Rect& bounds);
  ...
  // Closes the window and stops the presentation.  |callback| will be called
  // when this process is complete.  It is not safe to destroy this class until
  // |callback| has been called.
  void Terminate(base::OnceClosure callback);
