...
IN_PROC_BROWSER_TEST_F(PresentationReceiverWindowBrowserTest, CreatesWindow) {
  ...
  EXPECT_TRUE(receiver_window->IsWindowFullscreenForTest());
...
IN_PROC_BROWSER_TEST_F(PresentationReceiverWindowBrowserTest,
  ...
  const auto display =
      screen->GetDisplayMatching(receiver_window->GetWindowBoundsForTest());
  ...
  // displays.
  EXPECT_FALSE(receiver_window->IsWindowActiveForTest());
