...
void TaskManagerTableModel::GetRowsGroupRange(int row_index,
  ...
  const int range_length =
      static_cast<int>(observed_task_manager()->GetNumberOfTasksOnSameProcess(
          tasks_[row_index]));

  // The matching range will have |range_length| many elements, but if
  // |range_length| is greater than 1, there is uncertainty about where it
  // starts -- it could be anywhere between |row_index - range_length + 1| and
  // |row_index|.
  int range_start = std::min(row_index, RowCount() - range_length);
  int range_lower_bound = std::max(0, row_index - range_length + 1);

  // Keep decrementing |range_start| so long as its predecessor has the same
  // process ID as |row_index|.
  if (range_start > range_lower_bound) {
    const base::ProcessId process_id =
        observed_task_manager()->GetProcessId(tasks_[row_index]);
    while (range_start > range_lower_bound &&
           observed_task_manager()->GetProcessId(tasks_[range_start - 1]) ==
               process_id) {
      --range_start;
    }
  }

  // Validate that the computed range has matching PIDs.
  DCHECK_LE(range_start, row_index);
  DCHECK_LT(row_index, range_start + range_length);
#if DCHECK_IS_ON()
  for (int j = range_start; j < range_start + range_length; j++) {
    DCHECK_EQ(observed_task_manager()->GetProcessId(tasks_[row_index]),
              observed_task_manager()->GetProcessId(tasks_[j]));
  }
#endif

  *out_start = range_start;
  *out_length = range_length;
}
...
bool TaskManagerTableModel::IsTaskFirstInGroup(int row_index) const {
  ...
  // Rows with unknown PIDs aren't subject to grouping.
...
int TaskManagerTableModel::FindInsertionRow(TaskId id) {
  ...
  // implementation.
  const Task::SortKey& key = task_manager->GetSortKey(id);
  auto ordering = [task_manager](const Task::SortKey& a, TaskId b) {
    return a < task_manager->GetSortKey(b);
  };
  auto position = std::upper_bound(tasks_.begin(), tasks_.end(), key, ordering);
