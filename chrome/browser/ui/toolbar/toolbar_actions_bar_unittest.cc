...
void ToolbarActionsBarUnitTest::SetUp() {
  ...
  toolbar_action_error_test_observer_ =
      std::make_unique<ToolbarActionErrorTestObserver>();
...
ToolbarActionErrorTestObserver::ToolbarActionErrorTestObserver()
    : loaded_failed_waiter_(new Waiter()),
...
void ToolbarActionErrorTestObserver::WaitForOnLoadFailure() {
  loaded_failed_waiter_->Wait();
}

void ToolbarActionErrorTestObserver::OnLoadFailure(
    content::BrowserContext* browser_context,
    const base::FilePath& extension_path,
    const extensions::ExtensionId& extension_id,
    const std::string& error) {
  loaded_failed_waiter_->OnObserved();
}
...
// removed when an extension is re-uploaded with manifest errors and therefore
...
TEST_P(ToolbarActionsBarUnitTest, ReuploadExtensionFailed) {
  ...
  extension_error_observer()->WaitForOnLoadFailure();
