...
void JauntMenuModel::ActivatedAt(int index, int event_flags) {
  int entry_index = MenuIndexToNavEntryIndex(index);
  if (entry_index == -1) {
    // A navigation entry was not found. This will happen when an item is
    // selected that isn't in the linear back-forward history of the tab.
    // For now, just navigate to the URL fresh.
    // TODO(brettw) hook up proper navigation entries for this.
    const NavTreeNode* node = cached_items_[index].node;
    browser_->OpenURL(content::OpenURLParams(
        node->page()->url(), content::Referrer(),
        WindowOpenDisposition::CURRENT_TAB, node->transition_in(), false));
  } else {
    chrome::NavigateToIndexWithDisposition(
        browser_, entry_index, ui::DispositionFromEventFlags(event_flags));
  }
}
...
void JauntMenuModel::FillCachedItems() {
  cached_items_.clear();
  jaunt_.InitFromCurrentState(
      nav_tree_, browser_->tab_strip_model()->GetActiveWebContents());
  RecursiveAppendCachedItems(jaunt_.GetRoot(), false);
}
...
void JauntMenuModel::RecursiveAppendCachedItems(const NavWaypoint* waypoint,
  ...
  for (int i = 0; i < waypoint->child_count(); i++) {
    ...
    const auto* nav_entry = GetNavEntryForNode(cur_child->node());

    FlatMenuEntry entry;
    entry.indented = indent;
    if (nav_entry)
      entry.icon = nav_entry->GetFavicon().image;
...
content::NavigationEntry* JauntMenuModel::GetNavEntryForNode(
  ...
  for (int i = nav_controller.GetEntryCount() - 1; i >= 0; i--) {
    auto* nav_entry = nav_controller.GetEntryAtIndex(i);
    if (nav_entry->GetUniqueID() == node->nav_entry_id())
      return nav_entry;
  }
  return nullptr;
}
