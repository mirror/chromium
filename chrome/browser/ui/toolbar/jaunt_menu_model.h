...
class JauntMenuModel : public ui::MenuModel {
 public:
  explicit JauntMenuModel(Browser* browser);
 ...
 private:
  ...
  // May return null.
  content::NavigationEntry* GetNavEntryForNode(const NavTreeNode* node);

  // Returns the navigation entry index in the current tab corresponding to the
  // item at the given menu index. Returns -1 if not found.
