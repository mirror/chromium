...
class TabUIHelper : public content::WebContentsObserver,
 ...
 private:
  ...
  // Returns true if a favicon from WebContents should be used. This favicon
  // comes from the last committed navigation entry of the WebContents.
  bool ShouldUseFaviconFromWebContents() const;

  // Returns true if a favicon from history should be used. This is used when
  // a new tab is opened in the background and its navigation is delayed.
  ...
  // navigation when the tab is created by session restore or opened in
  // background.
