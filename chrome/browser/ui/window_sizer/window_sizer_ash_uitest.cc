...
gfx::Rect GetChromeIconBoundsForRootWindow(aura::Window* root) {
  auto* shelf_view = ash::Shelf::ForWindow(root)->GetShelfViewForTesting();
  auto* view_model = shelf_view->view_model_for_test();
...
// Ensure animations progressed to give the shelf button a non-empty size.
...
void OpenBrowserUsingContextMenuOnRootWindow(aura::Window* root) {
  ...
  // Move the cursor up to the "New window" menu option.
  generator.MoveMouseBy(0, -153);
