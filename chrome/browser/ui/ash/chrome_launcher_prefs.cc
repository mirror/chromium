...
void InitLocalPref(PrefService* prefs, const char* local, const char* synced) {
  LOG(ERROR) << "MSW InitLocalPref " << synced
        << " local?" << prefs->FindPreference(local)->HasUserSetting() << ":" << prefs->GetString(local)
        << " synced?" << prefs->FindPreference(synced)->HasUserSetting() << ":" << prefs->GetString(synced);
  // Ash's prefs *should* have been propagated to Chrome by now, but maybe not.
  // This should live in Ash, but Ash doesn't use a syncable pref service.
