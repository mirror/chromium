...
void WallpaperPolicyHandler::DeviceWallpaperPolicyChanged() {
  ...
  if (!device_wallpaper_file_path_.empty()) {
    OnGetDeviceWallpaperFilePath(device_wallpaper_file_path_);
  } else {
    delegate_->GetDeviceWallpaperFilePath(
        base::BindOnce(&WallpaperPolicyHandler::OnGetDeviceWallpaperFilePath,
                       weak_factory_.GetWeakPtr()));
  }
}

void WallpaperPolicyHandler::OnGetDeviceWallpaperFilePath(
    const base::FilePath& file_path) {
  device_wallpaper_file_path_ = file_path;

  // First check if the device policy was cleared or the provided policy strings
  // are incorrect.
  std::string url, hash;
  ...
  // hash. If so, do nothing. Otherwise fetch the wallpaper from the provided
  // url.
...
void WallpaperPolicyHandler::OnDeviceWallpaperFileExists(
...
void WallpaperPolicyHandler::OnCheckExistingDeviceWallpaperMatchHash(
    ...
    const std::string& hash,
    bool match) {
  // Do nothing if the exsiting device wallpaper matches the hash value of the
  // newly provided hash value.
  if (match)
    return;
  ...
  device_wallpaper_downloader_.reset(new CustomizationWallpaperDownloader(
      g_browser_process->system_request_context(), wallpaper_url,
      device_wallpaper_file_path_.DirName(), device_wallpaper_file_path_,
      base::BindRepeating(&WallpaperPolicyHandler::OnDeviceWallpaperDownloaded,
                          weak_factory_.GetWeakPtr(), hash)));
  device_wallpaper_downloader_->Start();
}
...
void WallpaperPolicyHandler::OnCheckFetchedDeviceWallpaperMatchHash(
  ...
  // Notify its delegate that device wallpaper has changed.
