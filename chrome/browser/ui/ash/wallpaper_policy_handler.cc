...
bool WallpaperPolicyHandler::ShouldShowUserWallpaperOnLogin() {
  bool show_user_wallpaper = true;
  DCHECK(chromeos::CrosSettings::Get()->GetBoolean(
      chromeos::kAccountsPrefShowUserNamesOnSignIn, &show_user_wallpaper))
      << "Unable to fetch setting "
      << chromeos::kAccountsPrefShowUserNamesOnSignIn;

  return show_user_wallpaper;
}
...
void WallpaperPolicyHandler::ShowUserNamesOnSignInPolicyChanged() {
  delegate_->OnShowUserWallpaperOnLoginChanged();
}
