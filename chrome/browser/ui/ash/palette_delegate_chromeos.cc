...
class VoiceInteractionHighlighterDelegate : public ash::HighlighterDelegate {
 public:
  explicit VoiceInteractionHighlighterDelegate(Profile* profile)
      : profile_(profile) {}
  ~VoiceInteractionHighlighterDelegate() override = default;
...
void PaletteDelegateChromeOS::ShowMetalayer(const base::Closure& closed) {
  ...
  if (!highlighter_delegate_) {
    highlighter_delegate_ =
        base::MakeUnique<VoiceInteractionHighlighterDelegate>(profile_);
  }
