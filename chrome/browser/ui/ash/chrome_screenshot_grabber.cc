...
void ChromeScreenshotGrabber::AddTestObserver(
    ChromeScreenshotGrabberTestObserver* observer) {
  test_observers_.AddObserver(observer);
}

void ChromeScreenshotGrabber::RemoveTestObserver(
    ChromeScreenshotGrabberTestObserver* observer) {
...
void ChromeScreenshotGrabber::HandleTakeScreenshotForAllRootWindows() {
  ...
  for (size_t i = 0; i < root_windows.size(); ++i) {
    ...
    screenshot_grabber_->TakeScreenshot(
        root_window, rect,
        base::Bind(&ChromeScreenshotGrabber::OnTookScreenshot,
...
void ChromeScreenshotGrabber::OnScreenshotCompleted(
  ...
  for (ChromeScreenshotGrabberTestObserver& observer : test_observers_)
    observer.OnScreenshotCompleted(result, screenshot_path);
