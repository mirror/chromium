...
// after user permission on the target update is set successfully in Update
// Engine.
IN_PROC_BROWSER_TEST_F(SystemTrayClientTest, CellularUpdateTrayIcon) {
  ...
  // Notifies that target update is set in Update Engine, which indicates that
  // user permission is granted on the target update.
  fake_update_engine_client_->NotifyUpdateOverCellularTargetSet(true);
  content::RunAllPendingInMessageLoop();
