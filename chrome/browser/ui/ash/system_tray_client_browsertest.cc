...
IN_PROC_BROWSER_TEST_F(SystemTrayClientTest, UpdateOverCellularTrayIcon) {
  ...
  // Notifies that one time permission on update over cellular connection is
  // set successfully in Update Engine.
  fake_update_engine_client_->NotifyUpdateOverCellularOneTimePermissionSet(
      true);
  content::RunAllPendingInMessageLoop();
