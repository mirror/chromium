...
void ArcAppWindowLauncherController::OnTaskCreated(
  ...
  LOG(ERROR) << "OnTaskCreated: " << package_name << " : " << activity_name
             << " : " << intent;
  const std::string arc_app_id =
...
void ArcAppWindowLauncherController::RegisterApp(
  ...
  if (!opt_in_management_check_start_time_.is_null() &&
    ...
    if (arc::GetExtraParamsFromIntent(app_window_info->launch_intent(),
                                      &extra_params) &&
        std::find(extra_params.begin(), extra_params.end(),
                  arc::kInitialStartParam) != extra_params.end()) {
