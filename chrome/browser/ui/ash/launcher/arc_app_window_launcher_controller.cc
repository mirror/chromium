...
AppWindowLauncherItemController*
ArcAppWindowLauncherController::ControllerForShelfId(const ash::ShelfID& id) {
  for (auto& it : task_id_to_app_window_info_) {
    ArcAppWindow* app_window = it.second->app_window();
    if (app_window && app_window->shelf_id() == id)
      return app_window->controller();
  }
  return nullptr;
}

void ArcAppWindowLauncherController::OnItemControllerDiscarded(
