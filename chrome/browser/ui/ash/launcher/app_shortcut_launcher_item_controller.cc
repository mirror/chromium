...
void AppShortcutLauncherItemController::ItemSelectedImpl(
...
std::unique_ptr<ui::MenuModel> AppShortcutLauncherItemController::GetContextMenu(int64_t display_id) {
  ...
  // // TODO(msw): Ash does this: AddShelfContextMenuItems(&items, display_id); 
  // return items;
  ChromeLauncherController* controller = ChromeLauncherController::instance();
  const ash::ShelfItem* item = controller->GetItem(shelf_id());
  return base::WrapUnique<LauncherContextMenu>(LauncherContextMenu::Create(controller, item, nullptr));
}

void AppShortcutLauncherItemController::ExecuteCommandImpl(bool from_context_menu,
                                                       int64_t command_id,
                                                       int32_t event_flags,
                                                       int64_t display_id) {
