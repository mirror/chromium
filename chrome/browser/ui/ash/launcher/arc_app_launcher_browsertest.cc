...
IN_PROC_BROWSER_TEST_F(ArcAppDeferredLauncherBrowserTest,
  ...
  ash::ShelfViewTestAPI test_api(
      ash::Shelf::ForWindow(ash::Shell::GetPrimaryRootWindow())
  ...
  ash::ShelfButton* shelf_button = test_api.GetButton(item_index);
  ASSERT_TRUE(shelf_button);

  // ash::ShelfButton moves some methods to protected. Access as pure button.
  views::Button* button = shelf_button;

  views::InkDrop* ink_drop = shelf_button->GetInkDropForTesting();
