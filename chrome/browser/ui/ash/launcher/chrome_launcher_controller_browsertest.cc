...
void NoopCallback(ash::ShelfAction,
                  base::Optional<std::vector<ash::mojom::MenuItemPtr>>) {}

// A helper to activate a shelf item in Chrome, like the ash::Shelf helpers.
void ActivateShelfItem(int item_index,
                       int64_t display_id = display::kInvalidDisplayId) {
  ash::ShelfModel* model = ChromeLauncherController::instance()->shelf_model();
  const ash::ShelfItem& item = model->items()[item_index];
  ash::ShelfItemDelegate* item_delegate = model->GetShelfItemDelegate(item.id);
  std::unique_ptr<ui::Event> event = std::make_unique<ui::KeyEvent>(
      ui::ET_KEY_RELEASED, ui::VKEY_UNKNOWN, ui::EF_NONE);
  item_delegate->ItemSelected(std::move(event), display_id,
...
IN_PROC_BROWSER_TEST_F(ShelfAppBrowserTest, LaunchPinned) {
  ...
  ActivateShelfItem(shelf_model()->ItemIndexByID(shortcut_id));
...
IN_PROC_BROWSER_TEST_F(ShelfAppBrowserTest, AltNumberTabsTabbing) {
  ...
  int shortcut_index = shelf_model()->ItemIndexByID(shortcut_id);
...
IN_PROC_BROWSER_TEST_F(ShelfAppBrowserTest, ActivateAfterSessionRestore) {
  ...
  ash::ShelfItemDelegate* item_delegate =
      shelf_model()->GetShelfItemDelegate(shortcut_id);
...
IN_PROC_BROWSER_TEST_F(ShelfAppBrowserTest, MatchingShelfIDandActiveTab) {
  ...
  ActivateShelfItem(shelf_model()->ItemIndexByID(app_id));
