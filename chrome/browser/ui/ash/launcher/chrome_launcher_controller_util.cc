...
// TODO(msw): Remove or consolidate WallpaperDelegate::CanOpenSetWallpaperPage function?
bool CanChangeWallpaper() {
  ...
  return user && !chromeos::WallpaperManager::Get()->IsPolicyControlled(user->GetAccountId());
}
...
void AddShelfContextMenuItems(ash::MenuItemList* menu, int64_t display_id) {
  ...
  if (CanChangeShelfAutoHideBehavior(display_id)) {
    ...
    item->label = l10n_util::GetStringUTF16(IDS_ASH_SHELF_CONTEXT_MENU_AUTO_HIDE);
    item->checked = GetShelfAutoHideBehaviorPref(profile->GetPrefs(), display_id) == ash::SHELF_AUTO_HIDE_BEHAVIOR_ALWAYS;
    item->enabled = true;
    menu->push_back(std::move(item));
  }
  if (CanChangeShelfAlignment()) {
    ...
    submenu->label = l10n_util::GetStringUTF16(IDS_ASH_SHELF_CONTEXT_MENU_POSITION);
    ...
    left->label = l10n_util::GetStringUTF16(IDS_ASH_SHELF_CONTEXT_MENU_ALIGN_LEFT);
    ...
    right->label = l10n_util::GetStringUTF16(IDS_ASH_SHELF_CONTEXT_MENU_ALIGN_RIGHT);
    ...
    bottom->label = l10n_util::GetStringUTF16(IDS_ASH_SHELF_CONTEXT_MENU_ALIGN_BOTTOM);
