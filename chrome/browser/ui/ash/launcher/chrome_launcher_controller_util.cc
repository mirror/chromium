...
// TODO(msw): Remove this, only use the version in Ash... 
void AddShelfContextMenuItems(ash::MenuItemList* menu, int64_t display_id) {
  LOG(ERROR) << "MSW AddShelfContextMenuItems CHROME A";
  Profile* profile = ChromeLauncherController::instance()->profile();
  if (CanChangeShelfAutoHideBehavior(display_id)) {
    LOG(ERROR) << "MSW AddShelfContextMenuItems CHROME B";
  ...
  if (CanChangeShelfAlignment()) {
    LOG(ERROR) << "MSW AddShelfContextMenuItems CHROME C";
    DCHECK(!profile->IsSupervised());
    ...
    LOG(ERROR) << "MSW AddShelfContextMenuItems CHROME G";
    ...
    LOG(ERROR) << "MSW AddShelfContextMenuItems CHROME H";
    ...
    LOG(ERROR) << "MSW AddShelfContextMenuItems CHROME I";
    menu->push_back(std::move(submenu));
  }
  if (CanChangeWallpaper()) {
    LOG(ERROR) << "MSW AddShelfContextMenuItems CHROME J";
    ash::mojom::MenuItemPtr item(ash::mojom::MenuItem::New());
