...
IN_PROC_BROWSER_TEST_F(SystemTrayTrayCastMediaRouterChromeOSTest,
  ...
  media_sinks_observer()->OnSinksUpdated(
      blink::mojom::ScreenAvailability::UNAVAILABLE, zero_sinks,
  ...
  media_sinks_observer()->OnSinksUpdated(
      blink::mojom::ScreenAvailability::AVAILABLE, one_sink,
      std::vector<url::Origin>());
  content::RunAllPendingInMessageLoop();
  EXPECT_TRUE(test_api.IsTrayVisible());
  media_sinks_observer()->OnSinksUpdated(
      blink::mojom::ScreenAvailability::AVAILABLE, two_sinks,
      std::vector<url::Origin>());
  content::RunAllPendingInMessageLoop();
  EXPECT_TRUE(test_api.IsTrayVisible());
...
IN_PROC_BROWSER_TEST_F(SystemTrayTrayCastMediaRouterChromeOSTest,
  ...
  media_sinks_observer()->OnSinksUpdated(
      blink::mojom::ScreenAvailability::AVAILABLE, sinks,
      std::vector<url::Origin>());
  content::RunAllPendingInMessageLoop();
