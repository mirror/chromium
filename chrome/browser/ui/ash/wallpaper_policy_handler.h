...
class WallpaperPolicyHandler {
 public:
  ...
  class Delegate {
   public:
    ...
    virtual void GetDeviceWallpaperFilePath(
        ash::mojom::WallpaperController::
            GetDevicePolicyWallpaperFilePathCallback callback) = 0;
 ...
 private:
  ...
  // This is called when the expected device wallpaper file path is returned
  // from ash.
  void OnGetDeviceWallpaperFilePath(const base::FilePath& file_path);
  void OnDeviceWallpaperFileExists(const std::string& url,
