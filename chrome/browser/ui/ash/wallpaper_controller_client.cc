...
void WallpaperControllerClient::SetCustomWallpaper(
  ...
  wallpaper_controller_->SetCustomWallpaper(
      std::move(user_info), wallpaper_files_id.id(), file, layout, type,
...
void WallpaperControllerClient::SetCustomizedDefaultWallpaper(
  ...
  wallpaper_controller_->SetCustomizedDefaultWallpaper(
      wallpaper_url, downloaded_file, resized_directory);
}
