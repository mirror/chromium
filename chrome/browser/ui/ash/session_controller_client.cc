...
void SessionControllerClient::DeviceSettingsUpdated() {
  ...
  // reload device policies. (See |DeviceOffHoursController| class description)
  auto* device_settings_service = chromeos::DeviceSettingsService::Get();
  const enterprise_management::ChromeDeviceSettingsProto* device_settings =
      device_settings_service->device_settings();
  if (!device_settings)
    return;  // May happen if session manager operation fails.

  device_off_hours_controller_->UpdateOffHoursPolicy(*device_settings);
  ...
  if (off_hours_device_settings) {
    device_settings_service->set_device_settings(
