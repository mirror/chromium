...
void SessionControllerClient::SendSessionLengthLimit() {
  ...
  // If |policy_off_hours_end_time| isn't null then "OffHours" policy mode is
  // on.
  bool policy_off_hours_mode_on = !policy_off_hours_end_time.is_null();
  ...
  if (session_length_limit_policy_set &&
    ...
    session_controller_->SetSessionLengthLimit(session_length_limit,
                                               session_start_time);
  } else if (policy_off_hours_mode_on) {
