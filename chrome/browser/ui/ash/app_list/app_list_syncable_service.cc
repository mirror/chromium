...
class AppListSyncableService::ModelObserver : public AppListModelObserver {
 public:
  explicit ModelObserver(AppListSyncableService* owner)
      : owner_(owner), adding_item_(NULL) {
...
AppListSyncableService::AppListSyncableService(
  ...
  if (ConnectToAppListController()) {
    LOG(ERROR) << "***** ConnectToAppListController";
    ash::mojom::AppListObserverAssociatedPtrInfo ptr_info;
    observer_binding_.Bind(mojo::MakeRequest(&ptr_info));
    app_list_controller_->SetObserver(std::move(ptr_info));
    app_list_controller_->DeleteItem("abc");
  }
...
bool AppListSyncableService::ConnectToAppListController() {
  // Synchronization is required in the Mash config, since Chrome and Ash run in
  // separate processes; it's optional via
  // kAshDisableAppListModelSynchronization in the Classic Ash config, where
  // Chrome can uses Ash's AppListModel directly.
...
void AppListSyncableService::OnDeleteItem(const std::string& id) {
  LOG(ERROR) << "***** AppListSyncableService::OnDeleteItem: " << id;
}
