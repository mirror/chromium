...
std::string AcceleratorIdToString(const ksv::AcceleratorId& accelerator_id) {
...
std::string AcceleratorIdsToString(
    const std::vector<ksv::AcceleratorId>& accelerator_ids) {
...
class KeyboardShortcutViewerMetadataTest : public testing::Test {
 ...
 protected:
  // ash accelerator ids:
  std::set<ksv::AcceleratorId> ash_accelerator_ids_;

  // chrome accelerator ids:
  std::set<ksv::AcceleratorId> chrome_accelerator_ids_;
...
TEST_F(KeyboardShortcutViewerMetadataTest, CheckAcceleratorIdHasAccelerator) {
  for (const auto& ksv_item : ksv::GetKSVItemList()) {
    for (const auto& accelerator_id : ksv_item.accelerator_ids) {
...
TEST_F(KeyboardShortcutViewerMetadataTest, CheckAcceleratorIdsNoDuplicates) {
  std::set<ksv::AcceleratorId> accelerator_ids_;
  for (const auto& ksv_item : ksv::GetKSVItemList()) {
    for (const auto& accelerator_id : ksv_item.accelerator_ids) {
      EXPECT_EQ(accelerator_ids_.count(accelerator_id), 0u)
...
TEST_F(KeyboardShortcutViewerMetadataTest,
  ...
  for (const auto& ksv_item : ksv::GetKSVItemList()) {
    EXPECT_TRUE(!ksv_item.replacement_codes.empty() ||
                !ksv_item.accelerator_ids.empty())
...
TEST_F(KeyboardShortcutViewerMetadataTest,
  ...
  for (const auto& ksv_item : ksv::GetKSVItemList()) {
    ...
    // |accelerator_ids|.
    if (!ksv_item.replacement_codes.empty() ||
        ksv_item.accelerator_ids.size() <= 1)
      continue;

    const int modifiers = ksv_item.accelerator_ids[0].modifiers;
    for (const auto& accelerator_id : ksv_item.accelerator_ids) {
      EXPECT_EQ(modifiers, accelerator_id.modifiers)
          << "Grouped accelerator_ids with empty replacement_codes should "
             "have same modifiers: "
          << AcceleratorIdsToString(ksv_item.accelerator_ids);
    }
  }
}
