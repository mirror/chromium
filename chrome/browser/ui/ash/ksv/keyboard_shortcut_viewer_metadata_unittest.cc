...
class KeyboardShortcutViewerMetadataTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    shortcut_items_ = ksv::GetKeyboardShortcutItemList();
    testing::Test::SetUp();
  }
 ...
 protected:
  ...
  // The ash and chrome keyboard shortcuts metadata.
  std::vector<std::unique_ptr<ksv::KeyboardShortcutItem>> shortcut_items_;
...
TEST_F(KeyboardShortcutViewerMetadataTest, CheckAcceleratorIdHasAccelerator) {
  for (const auto& shortcut_item : shortcut_items_) {
...
TEST_F(KeyboardShortcutViewerMetadataTest, CheckAcceleratorIdsNoDuplicates) {
  std::set<ksv::AcceleratorId> accelerator_ids;
  for (const auto& shortcut_item : shortcut_items_) {
    for (const auto& accelerator_id : shortcut_item->accelerator_ids) {
...
TEST_F(KeyboardShortcutViewerMetadataTest,
  ...
  for (const auto& shortcut_item : shortcut_items_) {
    EXPECT_TRUE(!shortcut_item->replacement_codes.empty() ||
...
TEST_F(KeyboardShortcutViewerMetadataTest,
  ...
  for (const auto& shortcut_item : shortcut_items_) {
    ...
    // |accelerator_ids|.
    if (!shortcut_item->replacement_codes.empty() ||
