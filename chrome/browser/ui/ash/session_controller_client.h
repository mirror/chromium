...
class SessionControllerClient
 ...
 private:
  ...
  // restrics session length: "SessionLenghtLimit" and "DeviceOffHours". Send
  // the session length limit if "OffHours" mode is off now or if the session
  // will be ended earlier than "OffHours" mode. Send the session length limit
  // if "SessionLengthLimit" policy is unset or if "OffHours" mode will be ended
