...
TabCountObserver::TabCountObserver(Browser* browser, int expected)
    : enable_observer_(EnableObserver()),
      browser_(browser),
      expected_(expected) {
  if (enable_observer_) {
...
// static
bool TabCountObserver::EnableObserver() {
#if defined(OS_CHROMEOS)
  // On ChromeOS, creating and destroying a tab are always not synchronized.
  return true;
#else
  return content::IsBrowserSideNavigationEnabled();
#endif
}
