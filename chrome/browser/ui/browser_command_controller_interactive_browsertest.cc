...
class BrowserCommandControllerInteractiveTest : public InProcessBrowserTest {
 ...
 protected:
  ...
  void WaitForActiveTabIndex(int expected) {
    while (active_tab_index() != expected)
      content::RunAllPendingInMessageLoop();
  }
  ...
  const auto enter_fullscreen = [this, js_fullscreen]() {
    ...
    if (js_fullscreen) {
      if (!this->IsActiveTabFullscreen()) {
        // Ensures document.body is valid in current page.
        ui_test_utils::NavigateToURLWithDisposition(
            this->GetActiveBrowser(),
        ...
        ASSERT_TRUE(content::ExecuteScript(
            this->GetActiveWebContents()->GetRenderViewHost(),
...
void BrowserCommandControllerInteractiveTest::StartTestPage(const char* file) {
  if (!embedded_test_server()->Started())
    ASSERT_TRUE(embedded_test_server()->Start());
  ui_test_utils::NavigateToURLWithDisposition(
      GetActiveBrowser(), embedded_test_server()->GetURL(file),
      WindowOpenDisposition::CURRENT_TAB,
      ui_test_utils::BROWSER_TEST_WAIT_FOR_NAVIGATION);
}
