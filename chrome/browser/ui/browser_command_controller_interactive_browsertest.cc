...
// Observes the count of tabs in a Browser instance.
class TabCountObserver : public TabStripModelObserver {
 ...
 private:
  ...
  // Checks the count of |browser_|->tab_strip_model() and set |seen_| to true
  // if its count equals to |expected_|.
  void CheckTabStripModel();
...
void TabCountObserver::TabDetachedAt(content::WebContents* contents,
                                     int index) {
  CheckTabStripModel();
}

void TabCountObserver::CheckTabStripModel() {
  if (browser_->tab_strip_model()->count() == expected_)
    seen_ = true;
}

// Observes the active tab index in a Browser instance.
class ActiveTabObserver : public TabStripModelObserver {
 public:
  // Tracks the active tab index in |browser| and waits until it to be
  // |expected|.
  static std::unique_ptr<ActiveTabObserver>
  ActiveTabIsExpectedToBe(Browser* browser, int expected);

  // Tracks the active tab index in |browser| and waits until it to be not
  // |unexpected|.
  static std::unique_ptr<ActiveTabObserver>
  ActiveTabIsExpectedNotToBe(Browser* browser, int unexpected);

  ~ActiveTabObserver() override;

 private:
  ActiveTabObserver(Browser* browser, int value, bool expected);
  ...
  Browser* const browser_;
  const int value_;
  const bool expected_;
  bool seen_ = false;
};

// static
std::unique_ptr<ActiveTabObserver>
ActiveTabObserver::ActiveTabIsExpectedToBe(Browser* browser,
                                           int expected) {
  return std::unique_ptr<ActiveTabObserver>(
      new ActiveTabObserver(browser, expected, true));
}

// static
std::unique_ptr<ActiveTabObserver>
ActiveTabObserver::ActiveTabIsExpectedNotToBe(Browser* browser,
                                              int unexpected) {
  return std::unique_ptr<ActiveTabObserver>(
      new ActiveTabObserver(browser, unexpected, false));
}

ActiveTabObserver::ActiveTabObserver(Browser* browser, int value, bool expected)
    : browser_(browser),
      value_(value),
      expected_(expected) {
  DCHECK(browser_);
...
ActiveTabObserver::~ActiveTabObserver() {
  while (!seen_)
    content::RunAllPendingInMessageLoop();
  browser_->tab_strip_model()->RemoveObserver(this);
}

void ActiveTabObserver::TabSelectionChanged(
    TabStripModel* tab_strip_model,
    const ui::ListSelectionModel& old_model) {
  DCHECK_EQ(tab_strip_model, browser_->tab_strip_model());
  const int index = browser_->tab_strip_model()->active_index();
  if (expected_)
    seen_ = (value_ == index);
  else
    seen_ = (value_ != index);
...
void BrowserCommandControllerInteractiveTest::
  ...
  ASSERT_NE(initial_active_index, active_tab_index());
  ...
  {
    auto observer = ActiveTabObserver::ActiveTabIsExpectedToBe(
        last_active_browser(), initial_active_index);
  ...
  {
    auto observer = ActiveTabObserver::ActiveTabIsExpectedNotToBe(
        last_active_browser(), initial_active_index);
