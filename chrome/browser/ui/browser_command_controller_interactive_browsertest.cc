    ...
    if (js_fullscreen) {
      if (!this->IsActiveTabFullscreen()) {
        ...
        content::WindowedNotificationObserver observer(
            chrome::NOTIFICATION_FULLSCREEN_CHANGED,
            content::NotificationService::AllSources());
        // No matter whether the page is loaded or not, document.body will
        // always enter fullscreen mode.
        const char* fullscreen_js =
        ...
        ASSERT_TRUE(content::ExecuteScript(
            this->GetActiveWebContents()->GetRenderViewHost(), fullscreen_js));
        observer.Wait();
