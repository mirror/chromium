...
class BrowserCommandControllerInteractiveTest : public InProcessBrowserTest {
 ...
 protected:
  // Starts the |kFullscreenKeyboardLockHTML| and waits for it to be loaded.
  void StartFullscreenLockPage();

  // Starts the |kFullscreenTriggerHTML| page and waits for it to be loaded.
  ...
  // Returns whether the active tab is in fullscreen mode.
  bool IsActiveTabFullscreen() const {
    auto* contents = browser()->tab_strip_model()->GetActiveWebContents();
    return contents->GetDelegate()->IsFullscreenForTabOrPending(contents);
  }

  // Returns whether the browser is in browser fullscreen mode.
 ...
 private:
  // Starts the test page |file| and waits for it to be loaded.
...
void BrowserCommandControllerInteractiveTest::
  ...
  if (!js_fullscreen) {
    // We do not want to close the test page, so we bypass to execute close
    // window command on the test page.
    ASSERT_NO_FATAL_FAILURE(enter_fullscreen());
  }
  ...
  ASSERT_NO_FATAL_FAILURE(SendShortcut(ui::VKEY_T));
  ASSERT_EQ(initial_tab_count + 1, tab_count());

  if (!js_fullscreen) {
    // We do not want to close the test page, so we bypass to execute close tab
    // command on the test page.
    ASSERT_NO_FATAL_FAILURE(enter_fullscreen());
  }
...
IN_PROC_BROWSER_TEST_F(BrowserCommandControllerInteractiveTest,
  ...
  ASSERT_NO_FATAL_FAILURE(StartFullscreenTriggerPage());
