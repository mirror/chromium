...
void BrowserCommandControllerInteractiveTest::
  ...
  const int initial_tab_count = tab_count();
  const size_t initial_browser_count = browser_count();
  const auto enter_fullscreen = [this, js_fullscreen, &enter_fullscreen_c]() {



    LOG(ERROR) << "BEFORE enter_fullscreen_count: " << enter_fullscreen_c;
    ...
    LOG(ERROR) << "AFTER enter_fullscreen_count: " << enter_fullscreen_c++;
