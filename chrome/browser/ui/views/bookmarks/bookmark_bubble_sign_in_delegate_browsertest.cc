...
IN_PROC_BROWSER_TEST_F(BookmarkBubbleSignInDelegateTest,
  ...
  // Start a sign-in flow from incognito.
...
IN_PROC_BROWSER_TEST_F(BookmarkBubbleSignInDelegateTest,
  ...
  Browser* new_regular_browser = chrome::FindTabbedBrowser(profile, false);
#if !defined(OS_CHROMEOS)
  // ProfileChooser doesn't show in an incognito window.
  EXPECT_FALSE(ProfileChooserView::IsShowing());

  // Sign-in dialog is shown when there non-incognito browser is created as part
  // of the sign-in flow.
  EXPECT_FALSE(
