...
IN_PROC_BROWSER_TEST_F(SaveCardBubbleViewsFullFormBrowserTest,
  ...
  // Submitting the form should show the upload save bubble and legal footer.
  ResetEventWaiterForSequence(
      {DialogEvent::REQUESTED_UPLOAD_SAVE, DialogEvent::BUBBLE_SHOWN_UPLOAD});
  FillAndSubmitForm();
  WaitForObservedEvent();
  EXPECT_TRUE(GetSaveCardBubbleViews()->GetFootnoteViewForTesting()->visible());
...
IN_PROC_BROWSER_TEST_F(
  ...
  WaitForObservedEvent();
  EXPECT_FALSE(
      GetSaveCardBubbleViews()->GetFootnoteViewForTesting()->visible());
}
...
IN_PROC_BROWSER_TEST_F(SaveCardBubbleViewsFullFormBrowserTest,
  ...
  ResetEventWaiterForSequence({DialogEvent::CVC_REQUEST_VIEW_SHOWN});
  ClickOnDialogViewWithIdAndWait(DialogViewId::OK_BUTTON);
  EXPECT_TRUE(GetSaveCardBubbleViews()->GetFootnoteViewForTesting()->visible());
