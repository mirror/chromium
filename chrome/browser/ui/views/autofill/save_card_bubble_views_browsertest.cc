...
IN_PROC_BROWSER_TEST_F(
    SaveCardBubbleViewsFullFormBrowserTest,
    Upload_ClickingNoThanksClosesBubbleIfNewUiAndSecondaryUiMdExpOff) {
...
IN_PROC_BROWSER_TEST_F(
    SaveCardBubbleViewsFullFormBrowserTest,
    Upload_ShouldNotHaveNoThanksButtonIfNewUiAndSecondaryUiMdExpOn) {
...
//              clicking the [X]. When/if that can be worked around, create a
...
//              range of the link. When/if that can be worked around, create a
...
IN_PROC_BROWSER_TEST_F(
  ...
  ResetEventWaiterForSequence({});
  // TODO(jsaul): "ResetEventWaiterForSequence({})" doesn't work as intended,
