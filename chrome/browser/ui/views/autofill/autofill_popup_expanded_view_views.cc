...
static const int kFrontPadding = 13;
static const int kTopPadding = 4;
 ...
 public:
  explicit AutofillPopupChildView(AutofillPopupController* controller,
      ...
      : controller_(controller),
        suggestion_(controller_->GetSuggestionAt(line_number)),
...
class AutofillPopupTextOnlyView : public AutofillPopupChildView {
 public:
  explicit AutofillPopupTextOnlyView(AutofillPopupController* controller,
                                     int line_number)
      : AutofillPopupChildView(controller, line_number),
        label_(new views::Label(
            controller->GetElidedValueAt(line_number),
            {controller->layout_model().GetValueFontListForRow(line_number)})) {
    views::BoxLayout* box_layout = new views::BoxLayout(
        views::BoxLayout::kVertical, gfx::Insets(kTopPadding, kFrontPadding));
    box_layout->set_main_axis_alignment(
        views::BoxLayout::MAIN_AXIS_ALIGNMENT_START);
    label_->SetBackground(
        views::CreateSolidBackground(GetNativeTheme()->GetSystemColor(
            controller->GetBackgroundColorIDForRow(line_number))));
    label_->SetEnabledColor(GetNativeTheme()->GetSystemColor(
        controller->layout_model().GetValueFontColorIDForRow(line_number)));
...
void AutofillPopupExpandedViewViews::CreateChildViews() {
  ...
  for (int i = 0; i < controller_->GetLineCount(); ++i) {
    ...
    if (controller_->GetSuggestionAt(i).frontend_id ==
      ...
      child_view = new AutofillPopupChildView(controller_, i);
    } else {
      child_view = new AutofillPopupTextOnlyView(controller_, i);
    }
