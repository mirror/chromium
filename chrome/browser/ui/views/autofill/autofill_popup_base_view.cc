...
#if !defined(OS_MACOSX) || BUILDFLAG(MAC_VIEWS_BROWSER)
  client_area_bounds = parent_widget_->GetClientAreaBoundsInScreen();
#else
  gfx::NativeWindow window =
      platform_util::GetTopLevel(delegate_->container_view());
  client_area_bounds = platform_util::GetWindowBounds(window);
#endif

  int available_vertical_space = client_area_bounds.height() -
                                 (bounds.y() - client_area_bounds.y()) -
                                 kPopupBottomMargin;
