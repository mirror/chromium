...
void FakeGeolocation::BindGeolocation(device::mojom::GeolocationRequest request) {
  binding_.Bind(std::move(request));
}
...
//                 base::Unretained(fake_geolocation_.get())));
  ...
  //registry_.AddInterface<mojom::GeolocationContext>(base::Bind( &SaveCardBubbleViewsBrowserTestBase::BindGeolocationContextRequest, base::Unretained(this)));
  registry_.AddInterface<device::mojom::GeolocationContext>(
      base::Bind(
