...
void DeviceChooserContentView::OnAdapterEnabledChanged(bool enabled) {
  ...
  SetReScanButtonLabel(false /* refreshing */);
...
void DeviceChooserContentView::OnRefreshStateChanged(bool refreshing) {
  ...
  SetReScanButtonLabel(refreshing);
  if (GetWidget() && GetWidget()->GetRootView())
    GetWidget()->GetRootView()->Layout();
}
...
std::unique_ptr<views::View> DeviceChooserContentView::CreateExtraView() {
  ...
  if (chooser_controller_->ShouldShowReScanButton()) {
    re_scan_button_ =
        views::MdTextButton::CreateSecondaryUiButton(this, base::string16());
    SetReScanButtonEnabled(false);
...
void DeviceChooserContentView::SetReScanButtonLabel(bool refreshing) {
  if (refreshing) {
    re_scan_button_->SetText(l10n_util::GetStringUTF16(
        IDS_BLUETOOTH_DEVICE_CHOOSER_SCANNING_BUTTON));
    re_scan_button_->SetTooltipText(l10n_util::GetStringUTF16(
        IDS_BLUETOOTH_DEVICE_CHOOSER_SCANNING_BUTTON_TOOLTIP));
  } else {
    re_scan_button_->SetText(
