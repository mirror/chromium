...
void TabScrubber::BeginScrub(Browser* browser,
  ...
  // This class should only be instantiated when the experimental tab strip
  // is not enabled, in which case we can cast from TabStrip->TabStripImpl.
  DCHECK(!IsExperimentalTabStripEnabled());
  tab_strip_ = static_cast<TabScripImpl*>(browser_view->tabstrip());
