...
class PasswordViewsInteractiveUiTest : public ManagePasswordsTest {
 public:
  PasswordViewsInteractiveUiTest() {}
  ~PasswordViewsInteractiveUiTest() override {}
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(PasswordViewsInteractiveUiTest);
};

IN_PROC_BROWSER_TEST_F(PasswordViewsInteractiveUiTest, BasicOpenAndClose) {
...
// method directly.
IN_PROC_BROWSER_TEST_F(PasswordViewsInteractiveUiTest, CommandControlsBubble) {
  ASSERT_TRUE(ui_test_utils::BringBrowserWindowToFront(browser()));
...
IN_PROC_BROWSER_TEST_F(PasswordViewsInteractiveUiTest,
...
IN_PROC_BROWSER_TEST_F(PasswordViewsInteractiveUiTest, CloseOnClick) {
  SetupPendingPassword();
  EXPECT_TRUE(IsBubbleShowing());
  EXPECT_FALSE(ManagePasswordsBubbleDelegateViewBase::manage_password_bubble()
                   ->GetFocusManager()
...
IN_PROC_BROWSER_TEST_F(PasswordViewsInteractiveUiTest, CloseOnEsc) {
  SetupPendingPassword();
  EXPECT_TRUE(IsBubbleShowing());
...
IN_PROC_BROWSER_TEST_F(PasswordViewsInteractiveUiTest, CloseOnKey) {
...
IN_PROC_BROWSER_TEST_F(PasswordViewsInteractiveUiTest, TwoTabsWithBubbleClose) {
...
IN_PROC_BROWSER_TEST_F(PasswordViewsInteractiveUiTest, AutoSignin) {
  // Set up the test server to handle the form icon request.
  embedded_test_server()->RegisterRequestHandler(base::BindRepeating(
      &PasswordViewsInteractiveUiTest::HandleRequest, base::Unretained(this)));
...
IN_PROC_BROWSER_TEST_F(PasswordViewsInteractiveUiTest, AutoSigninNoFocus) {
  test_form()->origin = GURL("https://example.com");
  test_form()->display_name = base::ASCIIToUTF16("Peter");
  test_form()->username_value = base::ASCIIToUTF16("pet12@gmail.com");
  std::vector<std::unique_ptr<autofill::PasswordForm>> local_credentials;
