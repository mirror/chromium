...
class PasswordDropdownModel : public ui::ComboboxModel {
 public:
  explicit PasswordDropdownModel(const std::vector<base::string16>& items)
    ...
    std::vector<base::string16> masked_values;
    for (const base::string16& password : items)
      masked_values.push_back(base::string16(password.length(), '*'));
    masked_passwords_ = masked_values;
  }
  ...
  base::string16 GetItemAt(int index) override {
    return masked_ ? masked_passwords_[index] : passwords_[index];
  }
 ...
 private:
  ...
  std::vector<base::string16> masked_passwords_;
...
class ManagePasswordsBubbleView::PendingView : public views::View,
 ...
 private:
  ...
  // View for the password value. Only one of |password_drowpdown_| and
  // |password_label_| can be available.
