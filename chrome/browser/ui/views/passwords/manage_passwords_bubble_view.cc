...
std::unique_ptr<views::Combobox> GeneratePasswordDropdownView(
  ...
  std::string default_password(form.password_value.length(), '*');
  passwords.push_back(base::ASCIIToUTF16(default_password));
  for (base::string16 alternative_password : form.other_possible_passwords) {
    std::string alternative(alternative_password.length(), '*');
    passwords.push_back(base::ASCIIToUTF16(alternative));
  }
  std::unique_ptr<views::Combobox> combobox(
      new views::Combobox(new ui::SimpleComboboxModel(passwords)));
  return combobox;
}
