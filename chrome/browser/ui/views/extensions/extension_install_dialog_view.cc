...
views::GridLayout* ExtensionInstallDialogView::CreateLayout(
  ...
  // The empty dialog title will then become the top margin, and a padding
  // column will be manually added to handle a right margin. This is done so
  // that the extension icon can be shown on the right of the dialog title, but
  // on the same y-axis, and the scroll view used to contain other content can
  // have its scrollbar aligned with the right edge of the dialog.
  ClearHorizontalContentMargins();
  views::GridLayout* layout = new views::GridLayout(container_);
  container_->SetLayoutManager(layout);
  container_->SetBorder(views::CreateEmptyBorder(0, dialog_insets.left(),
