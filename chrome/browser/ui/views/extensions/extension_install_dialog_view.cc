...
// A custom view for a single entry in the permissions section. It contains the
// label for the permission and a view for the permission's details if there
// are any.
 ...
 public:
  ...
  void AddItem(const base::string16& permission_text,
    ...
    if (!permission_details.empty()) {
      ...
      PermissionDetails details_container;
...
// Represents one section in the scrollable info area. The section can represent
// a block of permissions, a list of retained files, or a list of retained
// devices.
...
void AddPermissions(ExtensionInstallPrompt::Prompt* prompt,
  ...
  sections.push_back(
      {prompt->GetPermissionsHeading(perm_type), permissions_view.release()});
}
...
void ExtensionInstallDialogView::CreateContents() {
  ...
  if (prompt_->GetRetainedFileCount()) {
    ...
    ExpandableContainerView* issue_advice_view =
        new ExpandableContainerView(details, content_width);
  ...
  if (sections.size() > 0) {
    ...
    const int row_padding = ChromeLayoutProvider::Get()->GetDistanceMetric(
        views::DISTANCE_RELATED_CONTROL_VERTICAL);
    bool first_row = true;
    for (ExtensionInfoSection section : sections) {
      if (!first_row)
        scroll_layout->AddPaddingRow(0, row_padding);
      first_row = false;
      ...
      if (section.contents_view) {
        scroll_layout->StartRowWithPadding(0, kScrollViewColumnSetId, 0,
                                           row_padding);
...
void ExtensionInstallDialogView::AddedToWidget() {
  ...
  if (prompt_->has_webstore_data()) {
    ...
    constexpr int kRatingStarLabelSpacing = 8;
    rating_container->SetLayoutManager(std::make_unique<views::BoxLayout>(
        views::BoxLayout::kHorizontal, gfx::Insets(), kRatingStarLabelSpacing));
...
ExpandableContainerView::DetailsView::DetailsView(
  ...
  // Add spacing between this and the "Hide Details" link.
  ...
  for (auto& detail : details) {
    views::Label* detail_label =
        new views::Label(detail, CONTEXT_BODY_TEXT_LARGE, STYLE_SECONDARY);
...
ExpandableContainerView::ExpandableContainerView(
  ...
  const int left_padding = ChromeLayoutProvider::Get()->GetDistanceMetric(
