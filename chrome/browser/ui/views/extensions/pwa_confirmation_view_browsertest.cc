...
class PWAConfirmationViewTest : public DialogBrowserTest {
 public:
  ...
  void ShowDialog(const std::string& name) override {
    ...
    icon_info.data.allocN32Pixels(kPWAConfirmationViewIconSize,
                                  kPWAConfirmationViewIconSize, true);
    icon_info.data.eraseColor(SK_ColorBLUE);
    icon_info.width = kPWAConfirmationViewIconSize;
    icon_info.height = kPWAConfirmationViewIconSize;
...
// Launches an installation confirmation dialog for a PWA with name long enough
// to be elided, and a url which spans multiple lines.
