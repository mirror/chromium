...
void SetLabelFontSize(views::Label* label, int size) {
  auto font_list = label->font_list();
  label->SetFontList(
      font_list.DeriveWithSizeDelta(size - font_list.GetFontSize()));
}
...
void PWAConfirmationView::CreateDialog(
  ...
  gfx::ImageSkia image(
      std::make_unique<WebAppInfoImageSource>(icon_size, icons), image_size);
  ...
  SetLabelFontSize(title_label, 14);
  ...
  SetLabelFontSize(origin_label, 12);
