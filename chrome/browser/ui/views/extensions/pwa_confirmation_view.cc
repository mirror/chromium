...
void PWAConfirmationView::InitializeView(
  ...
  std::unique_ptr<views::ImageView> icon_view = IconView(icons);
  ...
  labels->AddChildView(TitleLabel(title, label_width).release());
  labels->AddChildView(
      OriginLabel(url::Origin::Create(app_url), label_width).release());
}
