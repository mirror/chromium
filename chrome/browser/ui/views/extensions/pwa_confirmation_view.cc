...
PWAConfirmationView::PWAConfirmationView(
  ...
  InitializeView(web_app_info_.title, web_app_info_.app_url,
                 web_app_info_.icons);
...
std::unique_ptr<views::ImageView> IconView(
    const std::vector<WebApplicationInfo::IconInfo>& icons) {
  gfx::Size image_size(kPWAConfirmationViewIconSize,
                       kPWAConfirmationViewIconSize);
  gfx::ImageSkia image(std::make_unique<WebAppInfoImageSource>(
                           kPWAConfirmationViewIconSize, icons),
                       image_size);
...
// Returns a label containing the app title.
std::unique_ptr<views::Label> TitleLabel(const base::string16& title,
                                         int label_width) {
  std::unique_ptr<views::Label> title_label = std::make_unique<views::Label>(
      title, ChromeTextContext::CONTEXT_BODY_TEXT_LARGE,
      views::style::TextStyle::STYLE_PRIMARY);
  title_label->SetHorizontalAlignment(gfx::ALIGN_LEFT);

  base::string16 elided =
      gfx::ElideText(title, title_label->font_list(), label_width,
...
std::unique_ptr<views::Label> OriginLabel(const url::Origin& origin,
                                          int label_width) {
...
void PWAConfirmationView::InitializeView(
  ...
  this->AddChildView(IconView(icons).release());
  ...
  constexpr int kLabelWidths = 320;
  labels->AddChildView(TitleLabel(title, kLabelWidths).release());
  labels->AddChildView(
      OriginLabel(url::Origin::Create(app_url), kLabelWidths).release());
}
