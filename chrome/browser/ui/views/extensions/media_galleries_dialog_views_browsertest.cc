...
class MediaGalleriesDialogTest : public DialogBrowserTest {
 public:
  ...
  void ShowDialog(const std::string& name) override {
    if (name == "InitializeCheckboxes")
      return InitializeCheckboxes();
  }
  ...
  void InitializeCheckboxes() {
    ...
    content_ = browser()->OpenURL(content::OpenURLParams(
        GURL("about:blank"), content::Referrer(),
        WindowOpenDisposition::CURRENT_TAB, ui::PAGE_TRANSITION_TYPED, true));
    EXPECT_CALL(*controller(), GetSectionEntries(0))
        .WillRepeatedly(Return(attached_permissions));
    EXPECT_CALL(*controller(), WebContents()).WillRepeatedly(Return(content_));
 ...
 private:
  ...
  content::WebContents* content_ = nullptr;
...
IN_PROC_BROWSER_TEST_F(MediaGalleriesDialogTest,
                       InvokeDialog_InitializeCheckboxes) {
  RunDialog();
  /*  MediaGalleriesDialogController::Entries attached_permissions;
    attached_permissions.push_back(
        MediaGalleriesDialogController::Entry(MakePrefInfoForTesting(1), true));
    attached_permissions.push_back(
        MediaGalleriesDialogController::Entry(MakePrefInfoForTesting(2),
    false)); EXPECT_CALL(*controller(), GetSectionEntries(0)).
        WillRepeatedly(Return(attached_permissions));
...
IN_PROC_BROWSER_TEST_F(MediaGalleriesDialogTest, InvokeDialog_UpdateAdds) {
  RunDialog();
  /*  MediaGalleriesDialogViews dialog(controller());
...
IN_PROC_BROWSER_TEST_F(MediaGalleriesDialogTest, InvokeDialog_ForgetDeletes) {
  RunDialog();
  /*  MediaGalleriesDialogViews dialog(controller());
