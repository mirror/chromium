...
class ExtensionInstallDialogRatingsSectionTest
 ...
 public:
  ...
  ~ExtensionInstallDialogRatingsSectionTest() override {}
...
IN_PROC_BROWSER_TEST_F(ExtensionInstallDialogRatingsSectionTest,
                       RatingsSectionA11y) {
  // An extension with 3.2 / 5 average rating and 400 ratings.
  const int kNumRatings = 400;
  const double kAverageRating = 3.297;
  std::string expected_text = l10n_util::GetStringFUTF8(
      IDS_EXTENSION_PROMPT_RATING_ACCESSIBLE_TEXT, base::ASCIIToUTF16("3.3"),
      base::ASCIIToUTF16("400"));
  TestRatingsSectionA11y(kNumRatings, kAverageRating, expected_text);
}

IN_PROC_BROWSER_TEST_F(ExtensionInstallDialogRatingsSectionTest,
                       RatingsSectionA11yWithNoRatings) {
  // An extension with no ratings yet.
  const int kNumRatings = 0;
  const double kAverageRating = 0.0;
  std::string expected_text =
      l10n_util::GetStringUTF8(IDS_EXTENSION_PROMPT_NO_RATINGS_ACCESSIBLE_TEXT);
  TestRatingsSectionA11y(kNumRatings, kAverageRating, expected_text);
}
