...
void ExtensionUninstallDialogViews::Show() {
  ...
  view_ = new ExtensionUninstallDialogDelegateView(
      this, anchor_view, parent(), extension(), triggering_extension(),
      &icon());
  if (anchor_view) {
...
ExtensionUninstallDialogDelegateView::ExtensionUninstallDialogDelegateView(
  ...
  // This adds margins for the icon + additional padding so that the elements
  // inside this dialog align with the title text instead of the icon.
  set_margins(
      margins() +
...
ExtensionUninstallDialogDelegateView::~ExtensionUninstallDialogDelegateView() {
  ...
  // when the uninstall dialog goes away.
  ToolbarActionView* anchor_view =
      GetExtensionAnchorView(extension_id_, parent_window_);
  if (anchor_view)
    anchor_view->AnimateInkDrop(views::InkDropState::DEACTIVATED, nullptr);
}
