...
void ArrowBorder::Paint(const views::View& view, gfx::Canvas* canvas) {
  ...
  // Paint the rectangular border, less the region occupied by the arrow.
  {
    ...
    content_bounds = gfx::Rect(view.GetWidget()
                                   ->GetNativeView()
                                   ->GetHost()
                                   ->GetBoundsInPixels()
                                   .size());
  ...
  // Paint the arrow.
  {
    ...
    // for a top or left taskbar), the arrow must be offset this extra amount.
    ...
    // Clip the content area again so that the image doesn't bleed into the
    // popup.
    canvas->sk_canvas()->clipRect(gfx::RectToSkRect(content_bounds),
                                  SkClipOp::kDifference, true);
