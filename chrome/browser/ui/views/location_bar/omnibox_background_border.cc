...
float OmniboxBackgroundBorder::GetBorderRadius(int height) {
  if (base::FeatureList::IsEnabled(features::kTouchableChrome)) {
    // This method returns the inner radius of the border, so subtract 2 pixels
    // off the height since the border thickness is always 1px.
    return (height / 2) - 2;
  }
...
void OmniboxBackgroundBorder::PaintFocusRing(gfx::Canvas* canvas,
  ...
  Paint(canvas, SK_ColorTRANSPARENT, focus_ring_color, inner_border_radius,
        local_bounds);
}
