...
void StarView::ShowPromo() {
  BookmarkPromoBubbleView* bookmark_promo_bubble =
      BookmarkPromoBubbleView::Create(this);
  views::Widget* widget =
      views::BubbleDialogDelegateView::CreateBubble(bookmark_promo_bubble);
  bookmark_promo_bubble->UseCompactMargins();
  if (widget && !bookmark_promo_observer_.IsObserving(widget)) {
    bookmark_promo_observer_.Add(widget);
...
void StarView::UpdateIcon() {
  if (bookmark_promo_observer_.IsObservingSources()) {
    BubbleIconView::SetImage(gfx::CreateVectorIcon(
        GetVectorIcon(), LocationBarView::kIconWidth,
        GetNativeTheme()->GetSystemColor(
            ui::NativeTheme::kColorId_ProminentButtonColor)));
  } else {
    BubbleIconView::UpdateIcon();
  }
}
