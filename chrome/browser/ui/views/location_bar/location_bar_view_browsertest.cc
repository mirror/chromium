...
IN_PROC_BROWSER_TEST_F(LocationBarViewBrowserTest, LocationBarDecoration) {
  ...
  const double default_zoom = zoom_controller->GetDefaultZoomLevel();
  ...
  zoom_controller->SetZoomLevel(default_zoom * 1.5);
  ...
  zoom_controller->SetZoomLevel(default_zoom * 2.0);
  EXPECT_TRUE(zoom_view->visible());
  EXPECT_TRUE(ZoomBubbleView::GetZoomBubble());

  // Remains visible at 100% until the bubble is closed.
  zoom_controller->SetZoomLevel(default_zoom);
  EXPECT_TRUE(zoom_view->visible());
  EXPECT_TRUE(ZoomBubbleView::GetZoomBubble());
