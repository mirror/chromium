...
constexpr int kTouchablePaddingHeightFactor = 4;
constexpr int kTouchablePaddingHeightFactorForIME = 2;
...
void LocationBarView::Layout() {
  ...
  if (HasFocus() && !keyword.empty() &&
    ...
    const int end_padding_for_keyword_hint =
        InTouchableMode() ? location_height / kTouchablePaddingHeightFactor
                          : item_padding;
  ...
  if (InTouchableMode() && (keyword_hint_view_->visible() ||
    ...
    location_bounds.Inset(
        0, 0, location_bounds.height() / kTouchablePaddingHeightFactor, 0);
  }
  ...
  if (ime_inline_autocomplete_view_->visible()) {
    ...
    const int end_padding_for_ime =
        InTouchableMode()
            ? location_bounds.height() / kTouchablePaddingHeightFactorForIME
            : 0;
    int width =
