...
void LocationBarView::UpdateFindBarIconVisibility() {
  ...
  const bool needs_activation = ShouldActivateFindBarIcon();
  if (visibility_changed || needs_activation) {
    Layout();
    find_bar_icon_->SetActive(find_bar_icon_->visible(),
                              visibility_changed && needs_activation);
    SchedulePaint();
  }
}
