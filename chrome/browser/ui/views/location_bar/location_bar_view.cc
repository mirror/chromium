...
bool LocationBarView::RefreshFindBarIcon() {
  ...
  const bool visibility_changed = was_visible != find_bar_icon_->visible();
  const bool needs_activation =
      !find_bar_icon_->IsActivated() && find_bar_icon_->visible();
  return visibility_changed || needs_activation;
}
