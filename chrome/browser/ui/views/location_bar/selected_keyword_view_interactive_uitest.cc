...
void InputKeys(Browser* browser, const std::vector<ui::KeyboardCode>& keys) {
  for (auto key : keys)
    ASSERT_TRUE(ui_test_utils::SendKeyPressAndWait(
        browser, key, false, false, false, false, content::NOTIFICATION_ALL,
        content::NotificationService::AllSources()));
}

typedef ExtensionBrowserTest SelectedKeywordViewTest;
...
IN_PROC_BROWSER_TEST_F(SelectedKeywordViewTest,
  ...
  EXPECT_EQ(extension->short_name(),
            base::UTF16ToUTF8(selected_keyword_view->get_label()));
}
