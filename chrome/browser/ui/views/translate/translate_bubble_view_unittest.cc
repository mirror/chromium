...
TEST_F(TranslateBubbleViewTest, DenialMenuNeverTranslateLanguage) {
  ...
  TriggerDenialMenu();
  int index = bubble_->denial_menu_model_->GetIndexOfCommandId(
...
TEST_F(TranslateBubbleViewTest, DenialMenuRespectsBlacklistSite) {
  ...
  EXPECT_FALSE(mock_model_->CanBlacklistSite());
  CreateAndShowBubble();
  ...
  EXPECT_NE(-1, bubble_->denial_menu_model_->GetIndexOfCommandId(
                    TranslateBubbleView::NEVER_TRANSLATE_LANGUAGE));
}
