...
views::View* TranslateBubbleView::CreateViewBeforeTranslate() {
  ...
  if (Use2016Q2UI()) {
    ...
    size_t menu_size = static_cast<size_t>(DenialComboboxIndex::MENU_SIZE);

    // If we can't blacklist the site, don't add the NEVER_TRANSLATE_SITE
    // menu item.
    if (!can_blacklist_site) {
      // Remove the NEVER_TRANSLATE_SITE menu item and menu item separator.
      menu_size -= 2;
    }
