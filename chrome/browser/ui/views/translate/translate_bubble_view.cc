...
void TranslateBubbleView::OnMenuButtonClicked(views::MenuButton* source,
  ...
  if (!denial_menu_runner_) {
    ...
    if (Use2016Q2UI()) {
      ...
      // TODO(pbos): Should this blacklisting be possible in incognito mode?
      // Doesn't that leave a trace of the user visiting the page?
...
views::View* TranslateBubbleView::CreateViewBeforeTranslate() {
  ...
  views::MenuButton* denial_menu_button = new views::MenuButton(
      l10n_util::GetStringUTF16(IDS_TRANSLATE_BUBBLE_OPTIONS_MENU_BUTTON), this,
      show_dropdown_arrow);
  denial_menu_button->SetStyleDeprecated(views::Button::STYLE_BUTTON);
  layout->AddView(denial_menu_button);
