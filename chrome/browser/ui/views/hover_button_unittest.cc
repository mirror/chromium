...
// Implementation of views::BubbleDialogDelegateView which places a
// |HoverButton| in the bubble.
class BubbleWithButton : public views::BubbleDialogDelegateView {
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(BubbleWithButton);
};
...
class HoverButtonWidgetTest : public views::test::WidgetTest {
 public:
  HoverButtonWidgetTest() : desktop_widget_(nullptr) {}
...
#define MAYBE_CorrectFocusState DISABLED_CorrectFocusState
#else
#define MAYBE_CorrectFocusState CorrectFocusState
#endif

// Test that a hover button only gets focus when it is in the active window.
TEST_F(HoverButtonWidgetTest, MAYBE_CorrectFocusState) {
  HoverButton* button1 =
