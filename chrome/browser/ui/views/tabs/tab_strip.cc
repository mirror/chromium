...
void TabStrip::SetResetToShrinkOnExit(bool value) {
  ...
  // We always prefer stacked layout in Touch optimzed UI. Unless stacking is
  // not required and there is enough space for tabs on the tabstrip.
  if (value && ui::MaterialDesignController::IsTouchOptimizedUiEnabled())
    value = false;
