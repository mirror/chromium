...
// in a tab.
//
// The icon will be drawn in the upper left. Normally you would lay this out
// so the top left is where you want the icon to be positioned, the width is
// gfx::kFaviconSize, and the height goes down to the bottom of the enclosing
// view (this is so the crashed tab can animate out of the bottom).
class TabIcon : public views::View {
 public:
  ...
  // Enables or disables the given attention type.
  void SetAttention(AttentionType type, bool enabled);

  // Returns true if the loading animation is showing.
  bool ShowingLoadingAnimation() const;

  // The loading animation only steps when this function is called. This
  // ensures that all throbbers step at the same time and it keeps the number
  // of timers down when lots of tabs are loading.
 ...
 private:
  ...
  void PaintThrobber(gfx::Canvas* canvas, const gfx::Rect& bounds);
