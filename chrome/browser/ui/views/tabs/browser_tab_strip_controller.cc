...
void BrowserTabStripController::ActiveTabChanged(
  ...
  if (new_contents && index != TabStripModel::kNoTab) {
    TabUIHelper::FromWebContents(new_contents)
        ->set_was_active_at_least_once(true);
...
void BrowserTabStripController::SetTabRendererDataFromModel(
  ...
  data->was_active_at_least_once = tab_ui_helper->was_active_at_least_once();
  data->is_navigation_delayed = tab_ui_helper->is_navigation_delayed();
  data->created_by_session_restore =
      tab_ui_helper->created_by_session_restore();
}
...
void BrowserTabStripController::AddTab(WebContents* contents,
  ...
  // Initialize the helper's initial state.
  TabUIHelper::FromWebContents(contents)->set_was_active_at_least_once(
      is_active);
