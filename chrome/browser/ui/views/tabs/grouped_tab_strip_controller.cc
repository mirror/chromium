...
void GroupedTabStripController::InitFromModel(TabStrip* tabstrip) {
  ...
  for (int i = 0; i < grouper_.count(); ++i)
    AddTab(i, false);  // TORO(brettw) need selection.
}
...
bool GroupedTabStripController::IsValidIndex(int index) const {
  return model_->ContainsIndex(index);
}

bool GroupedTabStripController::IsActiveTab(int model_index) const {
...
bool GroupedTabStripController::IsTabSelected(int model_index) const {
  return model_->IsTabSelected(model_index);
}

bool GroupedTabStripController::IsTabPinned(int model_index) const {
  return model_->ContainsIndex(model_index) && model_->IsTabPinned(model_index);
}

void GroupedTabStripController::SelectTab(int model_index) {
  model_->ActivateTabAt(model_index, true);
}

void GroupedTabStripController::ExtendSelectionTo(int model_index) {
  model_->ExtendSelectionTo(model_index);
}

void GroupedTabStripController::ToggleSelected(int model_index) {
  model_->ToggleSelectionAt(model_index);
}

void GroupedTabStripController::AddSelectionFromAnchorTo(int model_index) {
  model_->AddSelectionFromAnchorTo(model_index);
}

void GroupedTabStripController::CloseTab(int model_index,
...
void GroupedTabStripController::ToggleTabAudioMute(int model_index) {
...
void GroupedTabStripController::OnDropIndexUpdate(int index, bool drop_before) {
...
void GroupedTabStripController::ItemsInsertedAt(TabStripGrouper* grouper,
                                                int begin_index,
                                                int count) {
  // for (int i = 0; i < grouper_.count(); i++)
  //  AddTab(i, false);
...
void GroupedTabStripController::ItemsClosingAt(TabStripGrouper* grouper,
                                               int begin_index,
                                               int count) {
  // for (int i = begin_index + count - 1; i >= begin_index; i--) {
  //  tabstrip_->RemoveTabAt(nullptr, i);
  //}
}
...
void GroupedTabStripController::ItemNeedsAttentionAt(int index) {}
...
void GroupedTabStripController::SetTabRendererDataFromGrouper(
    TabStripGrouperItem* item,
    TabRendererData* data,
    TabStatus tab_status) {
  if (item->type() == TabStripGrouperItem::Type::TAB) {
    SetTabRendererDataFromModel(item->web_contents(), item->model_index(), data,
                                tab_status);
  } else {
    // Group.
    data->title = base::ASCIIToUTF16("Group!");
  }
}

void GroupedTabStripController::SetTabDataAt(content::WebContents* web_contents,
                                             int model_index) {
  TabRendererData data;
...
void GroupedTabStripController::AddTab(int grouper_index, bool is_active) {
  // Cancel any pending tab transition.
  hover_tab_selector_.CancelTabTransition();

  TabRendererData data;
  SetTabRendererDataFromGrouper(grouper_.GetItemAt(grouper_index), &data,
                                NEW_TAB);
  tabstrip_->AddTabAt(grouper_index, data, is_active);
}
