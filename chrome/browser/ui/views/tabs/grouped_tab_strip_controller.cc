...
void GroupedTabStripController::SetTabRendererDataFromGrouper(
  ...
  if (item->type() == TabStripGrouperItem::Type::TAB) {
    ...
    // Group.
    WebContents* first_contents = item->items()[0].web_contents();
    data->favicon =
        favicon::TabFaviconFromWebContents(first_contents).AsImageSkia();

    // Pick the "most loading-est" load state of all grouped tabs. Ignore
    // network error state.
    data->network_state = TabRendererData::NETWORK_STATE_NONE;
    for (const auto& child : item->items()) {
      TabRendererData::NetworkState child_state =
          TabContentsNetworkState(child.web_contents());
      if (child_state != TabRendererData::NETWORK_STATE_ERROR &&
          child_state > data->network_state)
        data->network_state = child_state;
    }

    // Compute the group title. FIXME(brettw) work on title.
    // These Unicode characters are the left & right lenticular brackets.
    data->title.push_back(0x3010);
    data->title.append(
        base::ASCIIToUTF16(base::SizeTToString(item->items().size())));
    data->title.push_back(0x3011);
    data->title.push_back(' ');
    data->title.append(first_contents->GetTitle());
  }
}

void GroupedTabStripController::SetItemDataAt(int item_index) {
  TabRendererData data;
  SetTabRendererDataFromGrouper(grouper_.GetItemAt(item_index), &data,
                                EXISTING_TAB);
  tabstrip_->SetTabData(item_index, data);
}
...
void GroupedTabStripController::AddTab(int item_index, bool is_active) {
  ...
  TabRendererData data;
  SetTabRendererDataFromGrouper(grouper_.GetItemAt(item_index), &data, NEW_TAB);
  tabstrip_->AddTabAt(item_index, data, is_active);
}
