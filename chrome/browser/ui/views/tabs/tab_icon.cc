...
void TabIcon::OnPaint(gfx::Canvas* canvas) {
  ...
  if (bounds.IsEmpty())
    return;
  bounds.Inset(0, static_cast<int>((bounds.height() * hiding_fraction_)), 0, 0);
  if (bounds.height() > gfx::kFaviconSize)
    bounds.set_height(gfx::kFaviconSize);
  if (bounds.width() > gfx::kFaviconSize)
    bounds.set_width(gfx::kFaviconSize);

  // Throbber.
  if (ShowingLoadingAnimation()) {
    PaintThrobber(canvas, bounds);
    return;
  }
...
void TabIcon::PaintAttentionIndicatorAndIcon(gfx::Canvas* canvas,
  ...
  // FIXME(brettw) bounds here used to be favicon_bounds_;
  const int indicator_x = GetMirroredXWithWidthInView(
      bounds.right() - kIndicatorRadius, kIndicatorRadius * 2);
  const int indicator_y = bounds.bottom() - kIndicatorRadius;
  canvas->DrawCircle(gfx::Point(indicator_x + kIndicatorRadius,
                                indicator_y + kIndicatorRadius),
                     kIndicatorRadius, indicator_flags);
}

void TabIcon::PaintThrobber(gfx::Canvas* canvas, const gfx::Rect& bounds) {
