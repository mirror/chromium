...
std::vector<gfx::ImageSkia> GetSkiaImagesForInfo(
  ...
  static base::NoDestructor<std::map<size_t, std::vector<gfx::ImageSkia>>>
      cache;
...
void TabCloseButton::SetStateColorInfo(
    const StateColorInfo& active_state_info,
    const StateColorInfo& inactive_state_info) {
  active_state_info_ = active_state_info;
  inactive_state_info_ = inactive_state_info;
}
