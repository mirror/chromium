...
const gfx::VectorIcon& GetVectorIcon(views::Button::ButtonState state,
                                     bool is_incognito) {
  if (!is_incognito)
    return kTabCloseButtonTouchIcon;

  if (state == views::Button::STATE_NORMAL)
    return kTabCloseButtonTouchIncognitoIcon;

  return kTabCloseButtonTouchIncognitoHoveredPressedIcon;
}
...
void TabCloseButton::ActiveStateChanged(const Tab* parent_tab) {
  const std::vector<views::Button::ButtonState> states = {
  ...
  for (const auto& state : states) {
    SetImage(state,
             gfx::CreateVectorIcon(GetVectorIcon(state, is_incognito),
