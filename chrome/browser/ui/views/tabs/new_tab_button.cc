...
void NewTabButton::ShowPromo() {
  new_tab_promo_ = new NewTabPromo(GetVisibleBounds());
  new_tab_promo_->GetWidget()->AddObserver(this);
...
void NewTabButton::PaintButtonContents(gfx::Canvas* canvas) {
  ...
  const SkColor stroke_color =
      new_tab_promo_ ? SK_ColorBLUE : tab_strip_->GetToolbarTopSeparatorColor();
...
void NewTabButton::OnWidgetClosing(views::Widget* widget) {
  // When the promo widget is closed, the NewTabButton needs to be
  // recolored.
  SchedulePaint();
}
...
void NewTabButton::PaintFill(bool pressed,
  ...
  if (!pressed) {
    ...
    if (custom_image) {
      ...
      const SkColor color =
          new_tab_promo_ ? GetNativeTheme()->GetSystemColor(
