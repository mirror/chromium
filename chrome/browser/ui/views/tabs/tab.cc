...
bool Tab::ShouldShowThrobber() const {
  // Do not display the throbber for non-active (background) tabs created by a
  // session restore, unless they've been made active. crbug.com/758684
  if (data().created_by_session_restore && !data().was_active_at_least_once)
    return false;

  if (data().is_navigation_delayed && !data().was_active_at_least_once)
    return false;
