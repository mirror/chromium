...
std::pair<SkColor, SkColor> Tab::GetCloseTabButtonColors(
    views::Button::ButtonState button_state) const {
  // The theme provider may be null if we're not currently in a widget
  // hierarchy.
  const ui::ThemeProvider* theme_provider = GetThemeProvider();
  if (!theme_provider)
    return std::make_pair(SK_ColorTRANSPARENT, SK_ColorTRANSPARENT);

  SkColor base_color = theme_provider->GetColor(
      ThemeProperties::COLOR_TAB_CLOSE_BUTTON_BACKGROUND_ACTIVE);
  SkColor icon_color = theme_provider->GetColor(ThemeProperties::COLOR_TOOLBAR);
  switch (button_state) {
    case views::Button::STATE_HOVERED:
      base_color = theme_provider->GetColor(
  ...
  return std::make_pair(base_color, icon_color);
}
...
void Tab::Layout() {
  ...
  if (showing_alert_indicator_) {
    ...
    const int right_padding = is_touch_optimized ? after_title_padding : 0;
