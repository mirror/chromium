...
void Tab::ActiveStateChanged() {
  ...
  // activated.
  if (IsActive())
    showing_attention_indicator_ &= ~AttentionType::kPinnedTabTitleChange;
...
void Tab::SetTabNeedsAttention(bool attention) {
  if (attention)
    showing_attention_indicator_ |= AttentionType::kTabWantsAttentionStatus;
  else
    showing_attention_indicator_ &= ~AttentionType::kTabWantsAttentionStatus;
  SchedulePaint();
}
...
void Tab::DataChanged(const TabRendererData& old) {
  ...
  if (data().blocked)
    showing_attention_indicator_ |= AttentionType::kBlockedWebContents;
  else
    showing_attention_indicator_ &= ~AttentionType::kBlockedWebContents;
}
...
void Tab::PaintIcon(gfx::Canvas* canvas) {
  ...
  int actual_attention_indicator = showing_attention_indicator_;
  if (IsActive())
    actual_attention_indicator &= ~AttentionType::kBlockedWebContents;

  if (actual_attention_indicator != 0 && !should_display_crashed_favicon_) {
