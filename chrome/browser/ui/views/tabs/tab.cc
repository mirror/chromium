...
void Tab::SetTabNeedsAttention(bool attention) {
  icon_->SetAttention(TabIcon::AttentionType::kTabWantsAttentionStatus, attention);
  SchedulePaint();
}
...
void Tab::Layout() {
  ...
  if (showing_icon_) {
    ...
    favicon_bounds_.set_size(gfx::Size(gfx::kFaviconSize, lb.height() - favicon_bounds_.y()));
    MaybeAdjustLeftForPinnedTab(&favicon_bounds_);
  }
  icon_->SetBoundsRect(favicon_bounds_);
