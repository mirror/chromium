...
NewTabPromo::NewTabPromo(const gfx::Rect& anchor_rect) {
  ...
  // Specifies which text will be shown. 0 is the default in case of |str| being
  // empty or it's length not in range of |kTextIds| bounds.
  const unsigned int text_specifier =
      std::stoi(str) >= arraysize(kTextIds) || str.length() <= 0
          ? std::stoi(str)
          : 0;
