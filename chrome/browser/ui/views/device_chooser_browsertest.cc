...
void ShowChooserBubble(Browser* browser,
  ...
  auto* bubble = new ChooserBubbleUi(browser, std::move(controller));
  bubble->Show(nullptr);
}
...
class UsbChooserBrowserTest : public DialogBrowserTest {
 public:
  UsbChooserBrowserTest()
      : controller_(base::MakeUnique<FakeUsbChooserController>()) {}

  // DialogBrowserTest:
  void ShowDialog(const std::string& name) override {
    ShowChooser(name, browser(), std::move(controller_));
  }

  void set_device_count(int device_count) {
    controller_->set_device_count(device_count);
  }

 private:
  std::unique_ptr<FakeUsbChooserController> controller_;
