...
void ReloadButton::ChangeMode(Mode mode, bool force) {
  ...
  if (ShouldDelayModeSwitch()) {
    const bool from_reload_to_stop =
        mode == MODE_STOP && visible_mode_ == MODE_RELOAD;
    auto is_mouse_hovered = [this]() {
      return IsMouseHovered() || testing_mouse_hovered_;
    };

    if (force || (!from_reload_to_stop && !is_mouse_hovered())) {
      ...
      // reload to stop.  (Disabled reload
...
void ReloadButton::ButtonPressed(views::Button* /* button */,
  ...
  // Ignore double clicks. With the ShouldDelayModeSwitch flag we have no use
  // for the double click timer and can just check the event's click count.
  const bool is_double_click = ShouldDelayModeSwitch()
