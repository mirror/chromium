...
IN_PROC_BROWSER_TEST_F(ImmersiveModeControllerAshHostedAppBrowserTest,
  ...
  // the associated windows top inset is 0.
  ...
  // immersive mode is not enabled, and the associated windows top inset is
  // greater than 0.
  ToggleFullscreen();
  EXPECT_TRUE(controller()->IsEnabled());
  tablet_mode_controller->EnableTabletModeWindowManager(false);
  tablet_mode_controller->FlushForTesting();
  EXPECT_FALSE(controller()->IsEnabled());
  EXPECT_GT(0, window->GetProperty(aura::client::kTopViewInset));
}
