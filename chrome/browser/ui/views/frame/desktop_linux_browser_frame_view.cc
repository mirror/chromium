...
void DesktopLinuxBrowserFrameView::OnMouseEvent(ui::MouseEvent* event) {
  ...
  if (event->IsRightMouseButton()) {
    ...
    if (event->flags() & ui::EF_IS_DOUBLE_CLICK) {
      ...
      // Potential double-click events will only get dispatched to us if we mark
      // this single-click event as handled.
      event->SetHandled();
      return;
    }
  } else {
  ...
  if (action == views::LinuxUI::WINDOW_FRAME_ACTION_MENU) {
    ...
    // The other cases should already be handled by WindowEventFilter.
    NOTREACHED();
  }
