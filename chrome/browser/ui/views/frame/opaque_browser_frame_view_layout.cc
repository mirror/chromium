...
#if BUILDFLAG(ENABLE_NATIVE_WINDOW_NAV_BUTTONS)
    auto* button_provider = views::LinuxUI::instance()->GetNavButtonProvider();
...
void OpaqueBrowserFrameViewLayout::Layout(views::View* host) {
  ...
  if (delegate_->ShouldRenderNativeNavButtons()) {
    ...
    auto get_button_from_display_type =
        [this](chrome::FrameButtonDisplayType type) -> views::ImageButton* {
      switch (type) {
    ...
    for (auto type : {
      ...
      for (size_t state = 0; state < views::CustomButton::STATE_COUNT;
        ...
        get_button_from_display_type(type)->SetImage(
            button_state, button_provider->GetImage(type, button_state));
      }
    }
#endif
  }
