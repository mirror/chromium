...
void OpaqueBrowserFrameViewLayout::LayoutNewStyleAvatar(views::View* host) {
  ...
  if (delegate_->ShouldRenderNativeNavButtons()) {
    ...
    button_provider->CalculateCaptionButtonLayout(
        gfx::Size(10, 10), delegate_->GetTopAreaHeight(), &button_size,
        &button_spacing);
    ...
    gfx::Insets avatar_insets = new_avatar_button_->GetInsets();
    gfx::Size avatar_size = new_avatar_button_->GetPreferredSize();
