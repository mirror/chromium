...
base::string16 BrowserView::GetAccessibleWindowTitleForChannelAndProfile(
  ...
  if (!browser()->is_app())
    title = l10n_util::GetStringFUTF16(IDS_BROWSER_WINDOW_TITLE_FORMAT, title);

  // Add the name of the channel if it's anything other than stable.
  switch (channel) {
    case version_info::Channel::CANARY:
      title = l10n_util::GetStringFUTF16(IDS_CANARY_CHANNEL_FORMAT, title);
      break;
    case version_info::Channel::DEV:
      title = l10n_util::GetStringFUTF16(IDS_DEV_CHANNEL_FORMAT, title);
      break;
    case version_info::Channel::BETA:
      title = l10n_util::GetStringFUTF16(IDS_BETA_CHANNEL_FORMAT, title);
      break;
    default:
      // Stable or unknown - not needed.
      break;
  }

  // Finally annotate with the user - add Incognito if it's an incognito
  // window, otherwise use the avatar name - but note that
  // GetAvatarNameForProfile will return the empty string if there's only one
  // user account, so it won't be verbose unless someone is using multiple
  // user accounts.
