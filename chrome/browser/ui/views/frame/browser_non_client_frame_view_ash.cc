...
void BrowserNonClientFrameViewAsh::Init() {
  ...
  if (extensions::HostedAppBrowserController::IsForExperimentalHostedAppBrowser(
          browser)) {
    hosted_app_button_container_ = new HostedAppButtonContainer(browser_view());
  ...
  if (UsePackagedAppHeaderStyle()) {
    ...
    if (app_controller) {
      ...
      if (hosted_app_button_container_) {
        hosted_app_button_container_->SetUseLightImages(
            header_painter->ShouldUseLightImages());
      }
