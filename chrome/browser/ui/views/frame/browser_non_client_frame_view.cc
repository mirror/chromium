...
void BrowserNonClientFrameView::UpdateTaskbarDecoration() {
  ...
  base::FilePath profile_path = browser_view()->browser()->profile()->GetPath();
  AvatarMenu::GetImageForMenuButton(profile_path, &decoration);

  ProfileAttributesEntry* entry;
  if (!g_browser_process->profile_manager()
           ->GetProfileAttributesStorage()
           .GetProfileAttributesWithPath(profile_path, &entry)) {
    // This can happen if the user deletes the current profile. Then we don't
    // need to paint the taskbar.
    return;
  }

  // If the user is using a Gaia picture but it has not been tried to load, wait
  // until the async load finishes which will trigger UpdateTaskbarDecoration()
  // again.
  if (entry->IsUsingGAIAPicture() && !entry->IsGAIAPictureLoadFromDisk())
    return;
