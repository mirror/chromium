...
void BrowserNonClientFrameView::UpdateTaskbarDecoration() {
  ...
  // until the load finishes which will trigger UpdateTaskbarDecoration() again.
  if (status == AvatarMenu::ImageLoadStatus::USE_DEFAULT_AS_GAIA_LOADING ||
      status == AvatarMenu::ImageLoadStatus::FAIL_AS_PROFILE_DELETED) {
    return;
  }
