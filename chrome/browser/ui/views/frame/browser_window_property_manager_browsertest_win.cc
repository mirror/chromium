...
// CreateProfileAsync callback.
void DoNothing(Profile*, Profile::CreateStatus) {}
...
void ValidateHostedAppWindowProperties(const Browser* browser,
  ...
  DLOG(WARNING) << "Validating relaunch command!!!";
  EXPECT_EQ(
      S_OK,
...
IN_PROC_BROWSER_TEST_F(BrowserTestWithProfileShortcutManager,
  ...
  profile_manager->CreateProfileAsync(path_profile2, base::Bind(&DoNothing),
                                      base::string16(), std::string(),
                                      std::string());

  // Wait to allow profile and icon creation to take place.
  content::RunAllBlockingPoolTasksUntilIdle();
...
IN_PROC_BROWSER_TEST_F(BrowserWindowPropertyManagerTest, HostedApp) {
  ...
  DLOG(WARNING) << "Finished launching browser, processing tasks...";
  content::RunAllBlockingPoolTasksUntilIdle();
  DLOG(WARNING) << "Finished tasks.";
