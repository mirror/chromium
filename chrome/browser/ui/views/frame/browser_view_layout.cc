...
gfx::Rect BrowserViewLayout::GetFindBarBoundingBox() const {
  ...
  if (immersive_mode_controller_->IsEnabled() &&
    ...
    // Overlap the find bar atop |top_container_| or |location_bar_view|.
    const int kTopOverlap = has_location_bar ? 4 : 6;
  ...
  int right_padding =
      has_location_bar ? bounding_box.right() - top_container_bounds.right() - 5
                       : gfx::scrollbar_size();
  ...
  if (base::i18n::IsRTL()) {
    int left_padding = has_location_bar
                           ? top_container_bounds.x() - bounding_box.x() - 5
                           : right_padding;
