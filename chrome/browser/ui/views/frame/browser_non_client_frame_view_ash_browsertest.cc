...
class BrowserNonClientFrameViewAshBackButonTest : public InProcessBrowserTest {
 public:
  BrowserNonClientFrameViewAshBackButonTest() = default;
  ~BrowserNonClientFrameViewAshBackButonTest() override = default;

  void SetUpCommandLine(base::CommandLine* command_line) override {
    command_line->AppendSwitch(ash::switches::kAshEnableV1BackButton);
  }

 private:
  DISALLOW_COPY_AND_ASSIGN(BrowserNonClientFrameViewAshBackButonTest);
};

}  // namespace

// Test if the V1 apps' frame has a back button.
IN_PROC_BROWSER_TEST_F(BrowserNonClientFrameViewAshBackButonTest,
                       V1BackButton) {
