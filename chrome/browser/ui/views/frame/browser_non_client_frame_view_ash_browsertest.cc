...
std::string TouchOptimizedUiStatusToString(
  ...
  if (info.param)
    return std::string("TouchOptimizedUiEnabled");

  return std::string("TouchOptimizedUiDisabled");
}
...
class TouchOptimizedUiParamTest : public BaseTest,
 ...
 public:
  ...
  void SetUp() override {
    if (GetParam())
      scoped_feature_list_.InitAndEnableFeature(features::kTouchOptimizedUi);
    else
      scoped_feature_list_.InitAndDisableFeature(features::kTouchOptimizedUi);
...
IN_PROC_BROWSER_TEST_P(BrowserNonClientFrameViewAshTest, NonClientHitTest) {
  ...
  // However, when Touch optimized UI is enabled, there is increased padding, so
