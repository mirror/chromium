...
class OmniboxPopupContentsView::AutocompletePopupWidget
 ...
 public:
  ...
  void SetPopupContentsView(OmniboxPopupContentsView* contents) {
    if (IsRounded()) {
      frame_ =
          new OmniboxPopupShadowFrame(contents, contents->location_bar_view_);
      SetContentsView(frame_);
    } else {
  ...
  void SetTargetBounds(const gfx::Rect& window_bounds) {
    if (frame_) {
      frame_->SetTargetBounds(window_bounds);
    } else {
 ...
 private:
  OmniboxPopupShadowFrame* frame_ = nullptr;
...
gfx::Rect OmniboxPopupContentsView::UpdateMarginsAndGetTargetBounds() {
  if (IsRounded()) {
    ...
    content_rect.Inset(
        -OmniboxPopupShadowFrame::GetTotalContentInsets(location_bar_view_));
...
int OmniboxPopupContentsView::CalculatePopupHeight() {
  ...
  if (IsRounded()) {
    height += OmniboxPopupShadowFrame::GetTotalContentInsets(location_bar_view_)
                  .height();
  } else {
