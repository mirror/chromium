...
constexpr int kTouchableAnimationDurationMs = 400;
...
class OmniboxPopupContentsView::AutocompleteClipFrame : public views::View {
 public:
  ...
  void SetTargetBounds(const gfx::Rect& window_bounds) {
    ...
    gfx::Size size = content_bounds.size();
    clip_mask_->layer()->SetBounds(gfx::Rect(size));
    ...
    ui::ScopedLayerAnimationSettings transition(layer()->GetAnimator());
...
#if 0
    gfx::Transform text_transform = contents_->layer()->transform();
    text_transform.Scale(1 / scale_x, 1 / scale_y);
    contents_->layer()->SetTransform(text_transform);
    ui::ScopedLayerAnimationSettings text_transition(
        contents_->layer()->GetAnimator());
    text_transition.SetTransitionDuration(
        base::TimeDelta::FromMilliseconds(kTouchableAnimationDurationMs));
    text_transition.SetTweenType(gfx::Tween::LINEAR);
    contents_->layer()->SetTransform(gfx::Transform());
#endif
  }
  ...
  void Layout() override {
    ...
    rect.Inset(insets_);
    contents_->SetBounds(0, 0, rect.width(), kTouchableBackbufferHeight);
 ...
 private:
  ...
  void Init() {
    ...
    // TODO: clip_mask_ should be a 9-image painter, not a texture.
    clip_mask_ = views::Painter::CreatePaintedLayer(
        views::Painter::CreateSolidRoundRectPainter(SK_ColorBLACK,
                                                    kTouchableRadius));
    clip_mask_->layer()->SetFillsBoundsOpaquely(false);
    ...
    clip_view_->SetBounds(insets_.left(), insets_.top(), 0, 0);
    clip_view_->layer()->SetMaskLayer(clip_mask_->layer());
    ...
    SetPaintToLayer();
    layer()->Add(contents_shadow_->layer());
  ...
  std::unique_ptr<ui::LayerOwner> clip_mask_;
...
void OmniboxPopupContentsView::UpdatePopupAppearance() {
  ...
  if (IsTouchable()) {
    ...
    // A rounded-rect border isn't needed: the clip mask achieves this instead.
