...
void OmniboxPopupContentsView::UpdatePopupAppearance() {
  if (model_->result().empty() || omnibox_view_->IsImeShowingPopup()) {
    ...
    if (popup_) {
      NotifyAccessibilityEvent(ax::mojom::Event::EXPANDED_CHANGED, true);
  ...
  if (!popup_) {
    ...
    NotifyAccessibilityEvent(ax::mojom::Event::EXPANDED_CHANGED, true);
    if (result_view_at(0))
      result_view_at(0)->NotifyAccessibilityEvent(ax::mojom::Event::SELECTION,
                                                  true);
  } else {
...
void OmniboxPopupContentsView::GetAccessibleNodeData(
  ...
  if (IsOpen()) {
    node_data->AddState(ax::mojom::State::EXPANDED);
  } else {
    node_data->AddState(ax::mojom::State::COLLAPSED);
    node_data->AddState(ax::mojom::State::INVISIBLE);
  }
}
