...
#include "chrome/browser/ui/views/omnibox/omnibox_popup_rounded_frame.h"
...
class OmniboxPopupContentsView::AutocompletePopupWidget
 ...
 public:
  ...
  void InitOmniboxPopup(views::Widget* parent_widget, const gfx::Rect& bounds) {
    ...
    if (IsRounded())
      OmniboxPopupShadowFrame::OnBeforeWidgetInit(&params);
    Init(params);
  }
...
gfx::Rect OmniboxPopupContentsView::UpdateMarginsAndGetTargetBounds() {
  if (IsRounded()) {
    ...
    content_rect.Inset(
        -OmniboxPopupShadowFrame::GetAlignmentInsets(location_bar_view_));
...
int OmniboxPopupContentsView::CalculatePopupHeight() {
  ...
  if (IsRounded()) {
    height += OmniboxPopupShadowFrame::GetAlignmentInsets(location_bar_view_)
                  .height();
  } else {
