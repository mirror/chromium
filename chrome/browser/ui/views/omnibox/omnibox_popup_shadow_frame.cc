...
OmniboxPopupShadowFrame::OmniboxPopupShadowFrame(views::View* contents)
  ...
  // To clip |contents|, we need another layer. We can't add a LayerMask because
  // mask layers must match the size of the layer they're masking. We could add
  // another view into the hierarchy, but then when the clip layer changes size
  // a lot of calls to schedule paints get triggered. Instead, add the contents'
  // Layer into the layer hierarchy, then insert a Layer to clip the contents.
  SetPaintToLayer();
  AddChildView(contents_);

  clip_layer_ = std::make_unique<ui::Layer>(ui::LAYER_NOT_DRAWN);
  clip_layer_->SetMasksToBounds(true);
  clip_layer_->SetBounds(gfx::Rect(insets_.left() + content_offset_,
                                   insets_.top() + content_offset_, 0, 0));

  layer()->Add(clip_layer_.get());
  clip_layer_->Add(contents_->layer());
}
...
void OmniboxPopupShadowFrame::SetTargetBounds(const gfx::Rect& window_bounds) {
  ...
  if (content_bounds.height() < clip_layer_->bounds().height()) {
    clip_layer_->SetBounds(content_bounds);
    return;
  }

  // Otherwise, animate from the current height, which may be mid-animation.
  ui::ScopedLayerAnimationSettings clip_transition(clip_layer_->GetAnimator());
...
void OmniboxPopupShadowFrame::Layout() {
  ...
  rect.Inset(insets_);
  clip_layer_->SetBounds(gfx::Rect(content_offset_, content_offset_,
                                   rect.width() - 2 * content_offset_,
                                   kMinHeight));
