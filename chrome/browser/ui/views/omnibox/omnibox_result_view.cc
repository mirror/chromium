...
void OmniboxResultView::Invalidate() {
  ...
  // The separator color may change, always set it.
...
void OmniboxResultView::Layout() {
  ...
  AutocompleteMatch* keyword_match = match_.associated_keyword.get();
  // Note that the parent() checks are only being done on the |keyword_icon_|.
  if (keyword_match) {
    ...
    if (match_.associated_keyword.get()) {
      ...
      kw_x += keyword_icon_->width() + kAnswerIconToTextPadding;
      keyword_content_view_->SetBounds(GetMirroredXInView(kw_x), 0, 100,
                                       row_height);
      kw_x += keyword_content_view_->width();
      keyword_description_view_->SetBounds(GetMirroredXInView(kw_x), 0, 100,
                                           row_height);
    }
  } else {
    if (keyword_icon_->parent()) {
  ...
  if (content_view_ && description_view_) {
    ...
    mirroring_context_->Initialize(x, width());
    int y = GetVerticalMargin();
    // NOTE: While animating the keyword match, both matches may be visible.
    if (!ShowOnlyKeywordMatch()) {
      if (match_.answer) {
        // content_view_->SetBounds(x, y, text_bounds_.width(), row_height);
        content_view_->SetBounds(GetMirroredXInView(x), y, width() - x,
                                 row_height);
  ...
  if (base::FeatureList::IsEnabled(omnibox::kUIExperimentVerticalLayout)) {
    ...
    if (description_max_width == 0)
      y += GetTextHeight() / 2;
        ...
        if (swap_match_text) {
          std::swap(first_view, second_view);
        }
        first_view->SetBounds(x, y, first_width, row_height);
        if (second_width != 0) {
          int aaaa = BackgroundWith1PxBorder::kLocationBarBorderThicknessDip +
                     horizontal_padding;
          LOG(INFO) << "aaaaaaaaaaaaaaaa " << aaaa;
          x += first_width;
