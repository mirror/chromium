...
std::unique_ptr<OmniboxTextView> OmniboxResultView::CreateOmniboxTextView(
    const base::string16& text,
    const ACMatchClassifications& classifications,
    OmniboxResultColor::HighlightState state) {
  std::unique_ptr<OmniboxTextView> view;
  view = std::make_unique<OmniboxTextView>(font_list_);
  CHECK(view);
  view->SetHighlightState(state);
...
void OmniboxResultView::Invalidate() {
  ...
  if (!ShowOnlyKeywordMatch()) {
    if (match_.answer) {
      content_view_ =
          CreateOmniboxTextView(match_.answer->first_line(), GetState());
      description_view_ =
          CreateOmniboxTextView(match_.answer->second_line(), GetState());
    } else {
      content_view_ = CreateOmniboxTextView(match_.contents,
                                            match_.contents_class, GetState());
      description_view_ = CreateOmniboxTextView(
  ...
  if (keyword_match) {
    keyword_content_view_ = CreateOmniboxTextView(
        keyword_match->contents, keyword_match->contents_class, GetState());
    keyword_description_view_ =
        CreateOmniboxTextView(keyword_match->description,
    ...
    if (!ShowOnlyKeywordMatch()) {
      if (match_.answer) {
        ...
        OmniboxPopupModel::ComputeMatchMaxWidths(
            ...
            !AutocompleteMatch::IsSearchType(match_.type), &first_width,
            &second_width);
        OmniboxTextView* first_view = content_view_.get();
        OmniboxTextView* second_view = description_view_.get();
