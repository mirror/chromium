...
void OmniboxResultView::SetMatch(const AutocompleteMatch& match,
  ...
  InlineTailPrefix(any_tail_suggestions, common_prefix);
  animation_->Reset();
...
void OmniboxResultView::InlineTailPrefix(bool any_tail_suggestions,
                                         const base::string16& common_prefix) {
  if (match_.type == AutocompleteMatchType::SEARCH_SUGGEST_TAIL) {
    match_.contents = common_prefix + match_.contents;
    // Shift existing styles.
    for (ACMatchClassification& classification : match_.contents_class)
      classification.offset += common_prefix.size();
    // Prefix with dim text.
    match_.contents_class.insert(
        match_.contents_class.begin(),
        ACMatchClassification(0, ACMatchClassification::DIM));
  } else if (any_tail_suggestions &&
             base::StartsWith(match_.contents, common_prefix,
    ...
    size_t i = 1;
    for (; i < match_.contents_class.size() &&
           match_.contents_class[i].offset <= common_prefix.size();
         ++i) {
    }
    if (i > 1) {
      // Erase any in-between.
      match_.contents_class.erase(match_.contents_class.begin() + 1,
                                  match_.contents_class.begin() + i - 1);
      // Make this classification start after common prefix.
      match_.contents_class[i - 1].offset = common_prefix.size();
    }
  }
}
