...
size_t OmniboxResultView::CommonPrefixLength(
...
void OmniboxResultView::InitContentsRenderTextIfNecessary() const {
  if (!contents_rendertext_) {
    if (match_.answer) {
    ...
    } else if (match_.type != AutocompleteMatchType::SEARCH_SUGGEST_TAIL) {
      ...
      if (swap_match_text) {
        ...
        if (model_->any_tail_suggestions()) {
          ...
          // with the suggestion.
          size_t common_length = CommonPrefixLength(match_.contents,
              model_->common_suggestion());
          if (common_length) {
            contents_class.push_back(ACMatchClassification(0,
          ...
          if (common_length && contents_class.size() > 1)
            contents_class[1].offset += common_length;
        } else {
          contents_class = match_.contents_class;
        }
        contents_rendertext_ = CreateClassifiedRenderText(
            match_.contents, contents_class, false);
      }
    } else {
      int prefix_length;
      base::StringToInt(
          match_.GetAdditionalInfo(kACMatchPropertyContentsStartIndex),
          &prefix_length);
      std::string contents =
          match_.GetAdditionalInfo(kACMatchPropertySuggestionText).substr(
              0, prefix_length);
      ACMatchClassifications contents_class;
      ...
      for (const auto& classification : match_.contents_class) {
        ...
        contents_class.push_back(ACMatchClassification(
            prefix_length + classification.offset, classification.style));
      }
      contents_rendertext_ = CreateClassifiedRenderText(
          base::UTF8ToUTF16(contents) + match_.contents, contents_class,
          false);
    }
  }
}
