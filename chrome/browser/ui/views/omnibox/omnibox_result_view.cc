...
void OmniboxResultView::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  ...
  // The positional info is provided via ax::mojom::IntAttribute::POS_IN_SET/SET_SIZE
  ...
  node_data->role = ax::mojom::Role::LIST_BOX_OPTION;
  node_data->AddIntAttribute(ax::mojom::IntAttribute::POS_IN_SET, model_index_ + 1);
  node_data->AddIntAttribute(ax::mojom::IntAttribute::SET_SIZE, model_->child_count());

  node_data->AddState(ax::mojom::State::SELECTABLE);
  switch (GetState()) {
    case SELECTED:
