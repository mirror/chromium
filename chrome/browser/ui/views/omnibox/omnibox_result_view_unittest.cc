...
TEST_F(OmniboxResultViewTest, AccessibleNodeData) {
  ...
  EXPECT_TRUE(popup_node_data.HasState(ui::AX_STATE_EXPANDED));
  EXPECT_FALSE(popup_node_data.HasState(ui::AX_STATE_COLLAPSED));
  EXPECT_FALSE(popup_node_data.HasState(ui::AX_STATE_INVISIBLE));
}
