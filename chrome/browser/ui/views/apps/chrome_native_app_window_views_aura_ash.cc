...
void ChromeNativeAppWindowViewsAuraAsh::OnBeforeWidgetInit(
  ...
  LOG(ERROR) << "MSW SETTING TYPE_APP!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"; 
  // TODO(msw): Do the same thing in classic ash... set type prop before parenting.  
...
void ChromeNativeAppWindowViewsAuraAsh::OnBeforePanelWidgetInit(
  ...
  LOG(ERROR) << "MSW SETTING TYPE_APP_PANEL!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"; 
  // TODO(msw): Do the same thing in classic ash... set type prop before parenting.  
  init_params->mus_properties[ui::mojom::WindowManager::kShelfItemType_Property] =
      mojo::ConvertTo<std::vector<uint8_t>>(static_cast<int64_t>(ash::TYPE_APP_PANEL));
}
