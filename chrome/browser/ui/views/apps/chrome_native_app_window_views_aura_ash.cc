...
ChromeNativeAppWindowViewsAuraAsh::ChromeNativeAppWindowViewsAuraAsh()
    : exclusive_access_manager_(new ExclusiveAccessManager(this)) {
...
bool
ChromeNativeAppWindowViewsAuraAsh::GetAcceleratorForCommandId(
...
void ChromeNativeAppWindowViewsAuraAsh::UpdateExclusiveAccessExitBubbleContent(
  ...
  exclusive_access_bubble_ = base::MakeUnique<ExclusiveAccessBubbleViews>(
      this, GURL("http://dummy.nodisplay.url"), bubble_type,
      ExclusiveAccessBubbleHideCallback());
  exclusive_access_bubble_->UpdateContent(
      GURL("http://dummy.display.url"), bubble_type,
      std::move(bubble_first_hide_callback));
}
...
gfx::Rect
ChromeNativeAppWindowViewsAuraAsh::GetClientAreaBoundsInScreen() const {
