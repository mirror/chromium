...
TEST_F(ShapedAppWindowTargeterTest, HitTestBasic) {
  ...
  std::unique_ptr<AppWindow::ShapeRects> rects(new AppWindow::ShapeRects);
  rects->push_back(gfx::Rect(0, 0, 0, 0));
  ...
  //              |     |
  // 130          +-----+
  rects.reset(new AppWindow::ShapeRects);
  rects->push_back(gfx::Rect(40, 0, 20, 100));
  rects->push_back(gfx::Rect(0, 40, 100, 20));
  app_window()->UpdateShape(std::move(rects));
  {
