...
void AddHeaderLabel(views::GridLayout* layout,
                    ...
                    int style) {
  constexpr int kHeaderColSpan = 3;
  constexpr int kHeaderRowSpan = 1;

  views::Label* label =
      new views::Label(text, views::style::CONTEXT_LABEL, style);
  label->SetMultiLine(true);
  label->SetHorizontalAlignment(gfx::ALIGN_LEFT);
  label->SetAllowCharacterBreak(true);
  layout->StartRow(kFixed, kColumnSetId);
  layout->AddView(label, kHeaderColSpan, kHeaderRowSpan);
}
...
LoginView::LoginView(const base::string16& authority,
  ...
  // Account for the line height in the title.
  insets.Set(insets.top() / 2, insets.left(), insets.bottom(), insets.right());
