...
class PermissionsBubbleDialogDelegateView
 ...
 private:
  ...
  // with whitespace collapsed. Otherwise, returns the extension URL formatted
  // for display. |web_contents| is used to get at the extension registry.
  static base::string16 GetExtensionDisplayName(
      const GURL& url,
      content::WebContents* web_contents);
  // Returns |url| formatted for display.
  static base::string16 FormatExtensionDisplayUrl(const GURL& url);
...
PermissionsBubbleDialogDelegateView::PermissionsBubbleDialogDelegateView(
  ...
  display_origin_ =
      GetExtensionDisplayName(requests[0]->GetOrigin(), web_contents);
...
// static
base::string16 PermissionsBubbleDialogDelegateView::GetExtensionDisplayName(
    const GURL& url,
    content::WebContents* web_contents) {
  ...
  if (!web_contents || !url.SchemeIs(extensions::kExtensionScheme)) {
    return FormatExtensionDisplayUrl(url);
  }
  ...
  return extension ? base::CollapseWhitespace(
                   ...
                   : FormatExtensionDisplayUrl(url);
}

base::string16 PermissionsBubbleDialogDelegateView::FormatExtensionDisplayUrl(
    const GURL& url) {
