...
views::View* GetSiteSettingsButton(Browser* browser) {
  ...
  views::Widget::GetAllChildWidgets(browser_view->GetWidget()->GetNativeView(),
  ...
  std::set<views::Widget*>::iterator it;
  for (it = child_widgets_after.begin(); it != child_widgets_after.end();
       ++it) {
  ...
  views::View* site_settings_button =
      page_info_bubble->GetRootView()->GetViewByID(1341);
...
void ClickAndWaitForSettingsPageToOpen(views::View* site_settings_button) {
  ...
  ui_test_utils::MoveMouseToCenterAndPress(
      ...
      ui_controls::DOWN | ui_controls::UP, runner->QuitClosure());
  runner->Run();
  site_settings_button->GetWidget()->CloseNow();
