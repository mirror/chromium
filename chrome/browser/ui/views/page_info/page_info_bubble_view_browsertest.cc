...
class PageInfoBubbleViewBrowserTest : public DialogBrowserTest {
 public:
  ...
  void ShowDialog(const std::string& name) override {
    GURL internal_url("chrome://settings");
    // Note the following URLs are not really necessary to get the different
    ...
    if (name == "UnwantedSoftware") {
      identity.identity_status =
          PageInfo::SITE_IDENTITY_STATUS_UNWANTED_SOFTWARE;
    }
    if (name == "PasswordReuse")
      identity.identity_status = PageInfo::SITE_IDENTITY_STATUS_PASSWORD_REUSE;

    if (name != "default" && name != "Internal") {
      PageInfoBubbleView* page_info_bubble =
          PageInfoBubbleView::GetPageInfoBubbleViewForTesting();
      page_info_bubble->SetIdentityInfo(identity);
    }
  }
...
IN_PROC_BROWSER_TEST_F(PageInfoBubbleViewBrowserTest, InvokeDialog_default) {
  RunDialog();
}

// Shows the Page Info bubble for a HTTPS page.
IN_PROC_BROWSER_TEST_F(PageInfoBubbleViewBrowserTest, InvokeDialog_HTTPS) {
  RunDialog();
}
...
// Shows the Page Info bubble after detecting the user has re-used an existing
// password on a site, e.g. due to phishing. (Also handled by Safe Browsing.)
IN_PROC_BROWSER_TEST_F(PageInfoBubbleViewBrowserTest,
