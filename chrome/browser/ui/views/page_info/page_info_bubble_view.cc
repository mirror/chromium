...
void PageInfoBubbleView::SetPermissionInfo(
  ...
  // *----------------------------------------------*
  constexpr int kPermissionsSectionId = 0;
  ...
  permissions_set->AddColumn(views::GridLayout::CENTER,
                             views::GridLayout::CENTER, kFixed,
  ...
  permissions_set->AddColumn(views::GridLayout::TRAILING,
                             views::GridLayout::FILL, kStretchy,
                             views::GridLayout::USE_PREF, 0, 0);
  permissions_set->AddPaddingColumn(kFixed, side_margin);
  ...
  for (auto& object : chosen_object_info_list) {
    ...
    std::unique_ptr<ChosenObjectRow> object_view =
        std::make_unique<ChosenObjectRow>(std::move(object));
