...
// The width of permission, chosen object, and cookies/certificate icons.
...
views::View* CreateInspectLinkSection(const gfx::ImageSkia& image_icon,
  ...
  column_set->AddColumn(views::GridLayout::CENTER, views::GridLayout::CENTER, 0,
                        views::GridLayout::FIXED, kIconWidth, 0);
...
PageInfoBubbleView::PageInfoBubbleView(
  ...
  // Since the bubble title is a smaller font, reduce space between the title
  // and client area.
  ChromeLayoutProvider* layout_provider = ChromeLayoutProvider::Get();
  int header_top_padding = kHeaderTitleBottomPadding;
  if (ui::MaterialDesignController::IsSecondaryUiMaterial()) {
    header_top_padding = layout_provider->GetDistanceMetric(
        views::DISTANCE_DIALOG_CONTENT_MARGIN_TOP_TEXT);
  }
  set_margins(gfx::Insets(header_top_padding, 0, margins().bottom(), 0));

  views::GridLayout* layout = views::GridLayout::CreateAndInstall(this);
  const int column_id = 0;
  views::ColumnSet* column_set = layout->AddColumnSet(column_id);
  column_set->AddColumn(views::GridLayout::FILL, views::GridLayout::FILL, 1,
                        views::GridLayout::USE_PREF, 0, 0);
  layout->StartRow(0, column_id);

  header_ = new BubbleHeaderView(this, this, side_margin);
  layout->AddView(header_);

  const int kVerticalSpacing = layout_provider->GetDistanceMetric(
      views::DISTANCE_UNRELATED_CONTROL_VERTICAL);
  layout->StartRowWithPadding(0, column_id, 0, kVerticalSpacing);

  // In Harmony, permissions are displayed above the separator.
  if (ui::MaterialDesignController::IsSecondaryUiMaterial()) {
...
void PageInfoBubbleView::SetPermissionInfo(
  ...
  if (permissions_view_->child_count() != 0)
    return;
  ...
  if (!ui::MaterialDesignController::IsSecondaryUiMaterial()) {
    // |site_settings_view_| already has its own side margins.
    side_margin = 0;
  ...
  const int kPermissionsSectionId = 0;
  views::ColumnSet* permissions_set =
      layout->AddColumnSet(kPermissionsSectionId);
  permissions_set->AddPaddingColumn(0, side_margin);
  ...
  const int kChosenObjectSectionId = 1;
  views::ColumnSet* chosen_object_set =
      layout->AddColumnSet(kChosenObjectSectionId);
  chosen_object_set->AddPaddingColumn(0, side_margin);
  ...
  for (size_t i = 0; i < permission_info_list.size(); ++i) {
    auto& permission = permission_info_list[i];
    // Skip the first row's padding.
    if (i != 0) {
      layout->AddPaddingRow(0, kListItemPadding);
    }
    // This row should be the correct combobox height to ensure the label and
    // combobox text bottoms are aligned.
    layout->StartRow(1, kPermissionsSectionId, kComboboxHeight);
    std::unique_ptr<PermissionSelectorRow> selector =
        base::MakeUnique<PermissionSelectorRow>(
            ...
            web_contents() ? web_contents()->GetVisibleURL()
                           : GURL::EmptyGURL(),
            permission, layout, kPermissionsSectionId);
    selector->AddObserver(this);
    selector_rows_.push_back(std::move(selector));
  }

  for (size_t i = 0; i < chosen_object_info_list.size(); ++i) {
    auto& object = chosen_object_info_list[i];
    if (i != 0 || !permission_info_list.empty()) {
      layout->AddPaddingRow(0, kListItemPadding);
    }
    ...
    layout->StartRow(1, kChosenObjectSectionId, kComboboxHeight);
    // The view takes ownership of the object info.
    auto* object_view = new ChosenObjectRow(std::move(object), kIconWidth);
    object_view->AddObserver(this);
    layout->AddView(object_view);
  }

  if (ui::MaterialDesignController::IsSecondaryUiMaterial() &&
      (chosen_object_info_list.size() || permission_info_list.size())) {
    // |permissions_view_| is followed by a Views separator.
    layout->AddPaddingRow(0, layout_provider->GetDistanceMetric(
                                 views::DISTANCE_UNRELATED_CONTROL_VERTICAL));
  } else {
    // |permissions_view_| is followed by the Certificate/Cookies sections.
    layout->AddPaddingRow(0, kListItemPadding);
  }
