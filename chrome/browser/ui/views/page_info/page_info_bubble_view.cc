...
InternalPageInfoBubbleView::InternalPageInfoBubbleView(
  ...
  // Remove extra vertical padding. Bubbles have their own borders and account
  // for the horizontal space for the close button, so keep horizontal insets.
  DCHECK(margins().left() == margins().right());
  set_margins(gfx::Insets(0, margins().left()));
  ...
  // Use the size returned by CalculatePreferredSize().
  SizeToContents();
}
...
gfx::Size InternalPageInfoBubbleView::CalculatePreferredSize() const {
  ...
  int bubble_width = margins().width() +
                     GetBubbleFrameView()->title()->GetPreferredSize().width();
  if (ShouldShowWindowIcon()) {
    bubble_width +=
        provider->GetDistanceMetric(views::DISTANCE_RELATED_LABEL_HORIZONTAL);
    bubble_width += bubble_icon_->width();
  }
  return gfx::Size(bubble_width, 0);
}
