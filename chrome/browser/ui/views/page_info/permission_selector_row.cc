...
// Create the permission menu button.
  ...
  if (!reason.empty()) {
    ...
    if (ui::MaterialDesignController::IsSecondaryUiMaterial() &&
      ...
      layout->AddView(
          secondary_label, column_span, 1, views::GridLayout::LEADING,
          views::GridLayout::CENTER, kMaxSecondaryLabelWidth,
          secondary_label->GetHeightForWidth(kMaxSecondaryLabelWidth));
    } else {
