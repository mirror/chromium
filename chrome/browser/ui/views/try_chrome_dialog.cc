...
class TryChromeDialog::Context {
 public:
  ...
  // the border itself, including space for the popup's arrow in case the popup
 ...
 private:
  ...
  class TaskbarCalculator : public DialogCalculator {
   public:
    ...
    // Returns the bounds of the arrow in pixels relative to the popup.
    const gfx::Rect& arrow_bounds() const { return arrow_bounds_; }

   private:
    // A Border that paints a arrow pointing toward Chrome's taskbar icon.
    class ArrowBorder : public views::Border {
     public:
      enum class ArrowRotation { kNone, k90Degrees, k180Degrees, k270Degrees };

      // Properties for an orientation-speciifc border with a arrow.
      struct Properties {
        // The amount by which the arrow is inset on a single side of the
        // border.
        gfx::Insets insets;
      ...
      ArrowBorder(TaskbarCalculator* calculator, const Properties* properties);
    ...
    struct PopupProperties {
      ...
      // Indicates which direction to translate the arrow from the center of the
    ...
    // client area. This rectangle extends approximately kArrowInset points into
...
TryChromeDialog::Context::TaskbarCalculator::MakeBorder() {
  return std::make_unique<ArrowBorder>(this, &properties_->border_properties);
}
...
TryChromeDialog::Context::TaskbarCalculator::ArrowBorder::ArrowBorder(
    ...
    : calculator_(calculator),
      ...
      arrow_(gfx::CreateVectorIcon(kInactiveToastArrowIcon, kBackgroundColor)) {
...
void TryChromeDialog::Context::TaskbarCalculator::ArrowBorder::Paint(
  ...
  // Paint the rectangular border, less the region occupied by the arrow.
  {
    ...
    gfx::Rect arrow_bounds(calculator_->arrow_bounds());
