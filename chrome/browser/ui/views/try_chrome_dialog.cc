 ...
 private:
  ...
  class TaskbarCalculator : public DialogCalculator {
   ...
   private:
    ...
    // that outline the popup of size |dialog_size| and its caret at
    // |caret_bounds| (which defines the bounding rectangle outside of the
    // content region of the popup).
    using PopupRegionCreatorFn = void (*)(gfx::Size dialog_size,
    ...
    static void CreateTopCaretRegion(gfx::Size dialog_size,
                                     const gfx::Rect& caret_bounds,
                                     POINT* polygon);
    static void CreateLeftCaretRegion(gfx::Size dialog_size,
                                      const gfx::Rect& caret_bounds,
                                      POINT* polygon);
    static void CreateBottomCaretRegion(gfx::Size dialog_size,
                                        const gfx::Rect& caret_bounds,
                                        POINT* polygon);
    static void CreateRightCaretRegion(gfx::Size dialog_size,
                                       const gfx::Rect& caret_bounds,
                                       POINT* polygon);
...
gfx::Rect TryChromeDialog::Context::TaskbarCalculator::ComputeBounds(
  ...
  caret_origin.Offset(properties_->caret_size.width() / -2.0f,
                      properties_->caret_size.height() / -2.0f);

  // See comment above about this orientation-specific shift.
  translation.set_x(
      (taskbar_icon_rect.width() + properties_->caret_size.width()) / 2.0f +
      kOffsetFromTaskbar);
  translation.set_y(
      (taskbar_icon_rect.height() + properties_->caret_size.height()) / 2.0f +
      kOffsetFromTaskbar);
  translation.Scale(properties_->offset_scale.x(),
                    properties_->offset_scale.y());
  ...
  caret_bounds_ =
      gfx::Rect(gfx::ToRoundedPoint(caret_origin), properties_->caret_size);
  ...
  scaled_caret.Inset(properties_->caret_inset.Scale(dsf));

  POINT polygon[7];
  properties_->region_creator(dialog_size, scaled_caret, &polygon[0]);
...
void TryChromeDialog::Context::TaskbarCalculator::CaretBorder::Paint(
  ...
  // Paint the rectangular border, less the region occupied by the caret.
  {
    ...
    // Subtract out the caret, less its insets.
    gfx::RectF caret_bounds(calculator_->caret_bounds());
    caret_bounds.Inset(caret_border_insets_);
    caret_bounds.Scale(dsf);
    canvas->sk_canvas()->clipRect(gfx::RectFToSkRect(caret_bounds),
                                  SkClipOp::kDifference, true);
