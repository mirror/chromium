...
class MetricsUpdater {
 public:
  ...
  // Records that the toast was just shown at |toast_location|.
...
gfx::Rect TryChromeDialog::ComputePopupBoundsOverTaskbarIcon(
  ...
  // Get the bounds and work area of the primary display (in DIP).
  ...
  gfx::Rect monitor_rect = display::win::ScreenWin::ScreenToDIPRect(
      taskbar, gfx::Rect(monitor_info.rcMonitor));

  // Is the taskbar hidden?
  ...
  result.AdjustToFit(display::win::ScreenWin::ScreenToDIPRect(
      taskbar, gfx::Rect(monitor_info.rcWork)));
  return result;
}
