...
// TryChromeDialog::MetricsUpdater ---------------------------------------------

// A helper for updating the experiment state stored in the Windows registry.
class TryChromeDialog::MetricsUpdater : public TryChromeDialog::Delegate {
 public:
  // Constructs the updater with a closure to be run after the dialog is closed.
  explicit MetricsUpdater(base::Closure continuation)
      : continuation_(continuation), result_(NOT_NOW) {}
  ~MetricsUpdater() override = default;

  // Returns the result to be reported to the caller.
 ...
 protected:
  ...
  void ReportMetricsOnClose(Result result,
 ...
 private:
  ...
  // A closure to run when the dialog is closed.
  base::Closure continuation_;

  // The result from the interaction to be reported to the caller of
  // TryChromeDialog::Show().
  Result result_;

  DISALLOW_COPY_AND_ASSIGN(MetricsUpdater);
};

void TryChromeDialog::MetricsUpdater::RecordLocationOnShow(
...
void TryChromeDialog::MetricsUpdater::ReportMetricsOnClose(
    Result result,
...
TryChromeDialog::Result TryChromeDialog::Show(
  ...
  MetricsUpdater metrics_updater(run_loop.QuitWhenIdleClosure());
  TryChromeDialog dialog(group, &metrics_updater);
...
void TryChromeDialog::OnTaskbarIconRect(const gfx::Rect& icon_rect) {
  ...
  if (result_ == OPEN_CHROME_DEFER) {
    ...
    delegate_->ReportMetricsOnClose(result_, state_);
    return;
  }
  ...
  popup_->Show();
  delegate_->RecordLocationOnShow(location);
}
