...
class DeviceChooserContentViewTest : public ChromeViewsTestBase {
 public:
  ...
  void SetUp() override {
    ...
    // Also creates the re-scan button, throbber, and scanning label.
  ...
  views::LabelButton* re_scan_button() {
    return content_view().re_scan_button_.get();
  }
  views::Throbber* throbber() { return content_view().throbber_.get(); }
  views::Label* scanning_label() {
    return content_view().scanning_label_.get();
  }

  bool IsScanningLabelAndThrobberVisible() {
    return extra_views_container_->Contains(throbber()) &&
           extra_views_container_->Contains(scanning_label());
  }

  bool IsReScanButtonVisible() {
    return extra_views_container_->Contains(re_scan_button());
  }
...
TEST_F(DeviceChooserContentViewTest, InitialState) {
  ...
  EXPECT_FALSE(IsScanningLabelAndThrobberVisible());
  EXPECT_TRUE(IsReScanButtonVisible());
...
TEST_F(DeviceChooserContentViewTest, ScanForDevices) {
  ...
  EXPECT_FALSE(table_view()->enabled());
  EXPECT_FALSE(adapter_off_help_link()->visible());
  EXPECT_TRUE(IsScanningLabelAndThrobberVisible());
  EXPECT_FALSE(IsReScanButtonVisible());
