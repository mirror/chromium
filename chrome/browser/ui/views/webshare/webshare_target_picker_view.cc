...
bool WebShareTargetPickerView::Accept() {
  if (!close_callback_.is_null()) {
    ...
    const ShareTarget& chosen_target = targets_[table_->FirstSelectedRow()];
    // Manually copy ShareTarget to avoid moving a target that could be used
    // by GetText().
    std::move(close_callback_)
        .Run(ShareTarget(chosen_target.manifest_url(), chosen_target.name(),
                         chosen_target.url_template()));
  }
