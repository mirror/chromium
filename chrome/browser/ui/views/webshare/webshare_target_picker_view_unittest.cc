...
class WebShareTargetPickerViewTest : public views::ViewsTestBase {
 ...
 protected:
  ...
  void CreateView(const std::vector<chrome::WebShareTarget>& targets) {
...
TEST_F(WebShareTargetPickerViewTest, EmptyListCancel) {
  CreateView(std::vector<chrome::WebShareTarget>());
...
TEST_F(WebShareTargetPickerViewTest, ChooseItem) {
  std::vector<chrome::WebShareTarget> targets{
      chrome::WebShareTarget("App One", GURL("https://appone.com/path/bits")),
      chrome::WebShareTarget("App Two", GURL("https://apptwo.xyz"))};
  CreateView(targets);
