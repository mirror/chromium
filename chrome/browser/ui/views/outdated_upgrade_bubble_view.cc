...
// Fixed width of the description label of the bubble. Chosen to obtain the
// "small" dialog width under Harmony (and not be smaller otherwise).
constexpr int kWidthOfDescriptionText = 320 - 2 * 16;
...
bool OutdatedUpgradeBubbleView::Close() {
  // Note there was once a "Later" button, tracked via Cancel(). There is now
  // just the "Accept" button, so override Close() to intercept other forms of
  // dismissal (and not Auto-accept), while recording UMA under the old name.
  if (!uma_recorded_)
