...
void ExitFullscreenIndicator::Show() {
  ...
  popup_->GetLayer()->SetBounds(CalculateBounds(kInitialOffset));
  ...
  popup_->GetLayer()->SetBounds(CalculateBounds(kFinalOffset));
}
...
void ExitFullscreenIndicator::Hide(bool animated) {
  ...
  if (animated) {
    ...
    animation_.SetTransitionDuration(base::TimeDelta());
  }
