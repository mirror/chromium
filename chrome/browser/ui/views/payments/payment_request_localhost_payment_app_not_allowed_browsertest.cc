...
// A test for localhost HTTP server that attempts to install a payment app and
...
class PaymentRequestLocalhostPaymentAppNotAllowedTest
 ...
 protected:
  PaymentRequestLocalhostPaymentAppNotAllowedTest()
      : localhost_(net::EmbeddedTestServer::TYPE_HTTP) {
...
IN_PROC_BROWSER_TEST_F(PaymentRequestLocalhostPaymentAppNotAllowedTest,
  ...
  ASSERT_TRUE(content::ExecuteScriptAndExtractString(
      browser()->tab_strip_model()->GetActiveWebContents(),
      "canMakePayment('" + method_name + "');", &contents));
  ASSERT_NE(std::string::npos,
            contents.find("Invalid payment method identifier format"))
      << "PaymentRequest constructor error not found in:\n"
      << contents;
}
