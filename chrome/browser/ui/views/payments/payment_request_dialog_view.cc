...
void PaymentRequestDialogView::OnGetAllPaymentInstrumentsFinished() {
  HideProcessingSpinner();
  if (observer_for_testing_) {
    // The OnGetAllPaymentInstrumentsFinished() method is called only after
    // asynchronously retrieving the list of installed payment apps. When the
    // payment apps feature is enabled, the UI shows a "Processing" spinner
    // before this method is called. Any test that opens UI to interact with it
    // should wait until this method hides the "Processing" spinner. The method
    // notifies the tests that the UI is ready via OnDialogOpened() call.
    DCHECK(base::FeatureList::IsEnabled(features::kServiceWorkerPaymentApps));
...
void PaymentRequestDialogView::ShowInitialPaymentSheet() {
  ...
  if (observer_for_testing_ &&
    ...
    // When the payment apps feature is disabled,
    // is_get_all_instruments_finished() is true, because the autofill cards are
    // be queried synchronously. There's no "Processing" spinner to hide, so the
    // UI is ready instantly. Any test that opens UI can now interact with it.
    // The OnDialogOpened() call notifies the tests of this event.
    DCHECK(!base::FeatureList::IsEnabled(features::kServiceWorkerPaymentApps));
    observer_for_testing_->OnDialogOpened();
  }
}
