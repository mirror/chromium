...
void PaymentRequestDialogView::ShowInitialPaymentSheet() {
  ...
  if (observer_for_testing_ &&
    ...
    // queried synchronously. There's no "Processing" spinner to hide, so the UI
    // is ready instantly. Any test that opens UI can now interact with it. The
    // OnDialogOpened() call notifies the tests of this event.
    DCHECK(!base::FeatureList::IsEnabled(features::kServiceWorkerPaymentApps));
    observer_for_testing_->OnDialogOpened();
  }
}
