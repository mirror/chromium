...
class FullscreenControlViewTest : public InProcessBrowserTest {
 public:
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    command_line->AppendSwitch(switches::kEnableFullscreenExitUI);
  }
...
IN_PROC_BROWSER_TEST_F(FullscreenControlViewTest, MouseExitFullscreen) {
  ...
  // Simulate moving a mouse to the top of the screen, which should show the
  // fullscreen exit UI.
  ...
  // Simulate clicking on the fullscreen exit button, which should bring the
  // browser out of fullscreen mode and the fullscreen exit control show hide
  // itself.
