...
views::View* ProfileChooserView::CreateCurrentProfileView(
  ...
  views::View* view = new views::View();
  int content_list_vert_spacing = provider->GetDistanceMetric(
      views::DISTANCE_SINGLE_HOVER_CONTENT_LIST_VERTICAL);
  if (avatar_item.signed_in && signin::IsAccountConsistencyMirrorEnabled()) {
    content_list_vert_spacing = provider->GetDistanceMetric(
  ...
  // The |current_profile_card_| will only show the profile name if not signed
  // in or account consistency is enabled. It will show the profile name as well
  // as the email if signed in and account consistency is disabled.
  views::HoverButton* profile_card = new views::HoverButton(
      this, current_profile_photo, profile_name, avatar_item.username);
  if (!avatar_item.username.empty())
  ...
  if (avatar_item.signed_in) {
    if (signin::IsAccountConsistencyMirrorEnabled()) {
      ...
      manage_accounts_button_ = new views::HoverButton(this, button_text);
...
views::View* ProfileChooserView::CreateSwitchUserView() {
  ...
  add_person_button_ = new views::HoverButton(
      this,
  ...
  disconnect_button_ = new views::HoverButton(
      this,
