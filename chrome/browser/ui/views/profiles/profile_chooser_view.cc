...
std::vector<AccountInfo> GetAccountsForProfile(Profile* profile) {
  ...
  for (const std::string& id : account_ids) {
    if (!cookie_accounts.empty() && id == cookie_accounts[0].id)
      accounts.insert(accounts.begin(),
                      account_tracker_service->GetAccountInfo(id));
    else
      accounts.push_back(account_tracker_service->GetAccountInfo(id));
  }
  return accounts;
}
...
views::View* ProfileChooserView::CreateDiceSigninView() {
  ...
  auto first_account_button_title =
  ...
  // not allow white text when auto readability is enabled. Though, this should
  // be allowed because blue buttons have exactly these specs.
