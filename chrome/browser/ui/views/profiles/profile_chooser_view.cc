...
std::vector<AccountInfo> GetAccountsForProfile(Profile* profile) {
  // Obtain all accounts that are not in an auth error state and return them.
  // TODO: Ideally we want the first account in the Gaia cookies to be the first
  // account in the accounts list |good_accounts|.
  ...
  std::vector<AccountInfo> good_accounts;
  for (const std::string& id : account_ids) {
    if (!token_service->RefreshTokenHasError(id)) {
      good_accounts.push_back(account_tracker_service->GetAccountInfo(id));
    }
  }
...
views::View* ProfileChooserView::CreateDiceSigninView() {
  ...
  promo_button_view->SetLayoutManager(
      new views::BoxLayout(views::BoxLayout::kVertical));
  ...
  // HoverButton::SetBorder() does not produce a border around the button,
  // therefore a parent view with a border has to be created.
  ...
  if (accounts.empty()) {
    ...
    promo_button_view->AddChildView(signin_button_view);
    view->AddChildView(promo_button_view);
    return view;
  }

  // Create a hover button to sign in the first account of |accounts|.
  // TODO(http://crbug.com/<bug_id>): Use the account picture instead of the
  // default avatar.
  ...
  HoverButton* first_account_button = new HoverButton(
      ...
      l10n_util::GetStringFUTF16(IDS_PROFILES_DICE_SIGNIN_FIRST_ACCOUNT_BUTTON,
                                 base::UTF8ToUTF16(accounts[0].full_name)),
      base::UTF8ToUTF16(accounts[0].email));
  first_account_button->SetBackground(
      views::CreateSolidBackground(gfx::kGoogleBlue500));
  // Using |gfx::kGoogleBlue500| as background_color in SetTitleTextStyle() does
  // not allow white text because auto readability is enabled. This should be
  // allowed though because blue buttons have exactly these specs.
  // |SK_ColorTRANSPARENT| is used here instead to be able to set a white text
  // color.
  first_account_button->SetTitleTextStyle(
      views::style::STYLE_DIALOG_BUTTON_DEFAULT, SK_ColorTRANSPARENT);
  first_account_button->SetSubtitleColor(SK_ColorWHITE);
  ...
  HoverButton* sync_to_another_account_button = new HoverButton(
      this,
      gfx::CreateVectorIcon(kSyncSwitchAccountIcon, 20, gfx::kChromeIconGrey),
      l10n_util::GetStringUTF16(
          IDS_PROFILES_DICE_SIGNIN_WITH_ANOTHER_ACCOUNT_BUTTON));
  signin_current_profile_button_ = sync_to_another_account_button;
  promo_button_view->AddChildView(sync_to_another_account_button);
