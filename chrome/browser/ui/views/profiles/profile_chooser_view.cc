...
BadgedProfilePhoto::BadgeType GetProfileBadgeType(const Profile& profile) {
  BadgedProfilePhoto::BadgeType profile_badge_type =
      BadgedProfilePhoto::BADGE_TYPE_NONE;
  if (profile.IsSupervised())
    profile_badge_type = profile.IsChild()
                             ? BadgedProfilePhoto::BADGE_TYPE_CHILD
                             : BadgedProfilePhoto::BADGE_TYPE_SUPERVISOR;
  return profile_badge_type;
}
...
views::View* ProfileChooserView::CreateCurrentProfileView(
  ...
  columns->AddPaddingColumn(
      0, kMenuEdgeMargin - BadgedProfilePhoto::kBadgeSpacing);
  columns->AddColumn(views::GridLayout::FILL, views::GridLayout::FILL, 1,
                     views::GridLayout::USE_PREF, 0, 0);
  grid_layout->AddPaddingRow(0, 0);

  // Profile picture, left-aligned.
  BadgedProfilePhoto* current_profile_photo = new BadgedProfilePhoto(
  ...
  const int num_labels =
      (avatar_item.signed_in && !signin::IsAccountConsistencyMirrorEnabled())
          ? 2
          : 1;
  int profile_card_height = current_profile_photo->GetPreferredSize().height() +
                            2 * vertical_spacing_small;
  const int line_height =
      current_profile_photo->GetPreferredSize().height() / num_labels;
  grid_layout->AddPaddingRow(0, vertical_spacing_small);
  ...
  current_profile_card_->SetMinSize(
      gfx::Size(kFixedMenuWidth, profile_card_height));
