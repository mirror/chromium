...
PasswordReuseModalWarningDialog::PasswordReuseModalWarningDialog(
    ...
    : TabModalConfirmDialogDelegate(web_contents),
      ...
      done_(done) {
  Init();
  done_.Run(PasswordProtectionService::SHOWN);
}

PasswordReuseModalWarningDialog::~PasswordReuseModalWarningDialog() {}

// Initialize and configure the layout of this dialog.
void PasswordReuseModalWarningDialog::Init() {
  ...
  // Warning icon.
  gfx::Image icon_image(show_softer_warning_
  ...
  icon->SetImageSize(icon_image.Size());
  icon->SetImage(*(icon_image.ToImageSkia()));
  // Warning title.
  views::Label* title =
  ...
  // Configure layout
  layout->AddPaddingRow(0, padding_size);
...
// static
PasswordReuseModalWarningDialog* PasswordReuseModalWarningDialog::Create(
    content::WebContents* web_contents,
    const OnWarningDone& done) {
  PasswordReuseModalWarningDialog* dialog =
      new PasswordReuseModalWarningDialog(web_contents, done);
