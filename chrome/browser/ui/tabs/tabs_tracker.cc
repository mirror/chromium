...
void TabsTracker::InitializeWebContentsDataForWebContents(
  ...
  TabsTracker::WebContentsData::CreateForWebContents(contents);
}
...
void TabsTracker::OnWasHidden(content::WebContents* contents) {
  ...
  tab_logger_->LogBackgroundTab(web_contents_data->ukm_source_id(), contents,
                                browser, model, old_index);
}
...
void TabsTracker::TabPinnedStateChanged(TabStripModel* tab_strip_model,
  ...
  if (!ukm_source_id)
    return;
  tab_logger_->LogBackgroundTab(ukm_source_id, contents, browser,
                                tab_strip_model, index);
}
