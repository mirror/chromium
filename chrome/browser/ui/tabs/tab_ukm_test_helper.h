...
class UkmEntryChecker {
 public:
  ...
  void ExpectNewEntry(const char* entry_name,
                      const GURL& source_url,
                      const UkmMetricMap& expected_metrics);

  // Expects that |expected_data.size()| number of entries have been rcorded
  ...
  void ExpectNewEntries(const char* entry_name,
                        const SourceUkmMetricMap& expected_data);

  // Checks if |new_num_entries| entries has been recorded for |entry_name|.
  bool WasNewEntryRecorded(const char* entry_name,
                           size_t new_num_entries) const;

  // Returns number of entries for |entry_name|.
  size_t NumEntries(const char* entry_name) const;

  // Returns the last recorded entry for |entry_name|.
  const ukm::mojom::UkmEntry* LastUkmEntry(const char* entry_name) const;

  ukm::SourceId GetSoureIdForUrl(const GURL& source_url);
 ...
 private:
  ...
  std::map<const char*, size_t> num_entries_;
};
