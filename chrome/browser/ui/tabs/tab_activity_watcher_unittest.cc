...
class TabActivityWatcherTest : public TabActivityTestBase {
 protected:
  ...
  void TearDown() override {
    EXPECT_TRUE(ukm_entry_checker()->WasNewEntryRecorded(kEntryName, 0));
    TabActivityWatcher::GetInstance()->ResetForTesting();
  ...
  void ExpectNewEntry(const GURL& source_url,
    ...
    ukm_entry_checker()->ExpectNewEntry(kEntryName, source_url,
                                        expected_metrics);

    const size_t num_entries = ukm_entry_checker()->NumEntries(kEntryName);
    const ukm::mojom::UkmEntry* last_entry =
        ukm_entry_checker()->LastUkmEntry(kEntryName);
