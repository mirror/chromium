...
const MetricMap kBasicMetricValues(
    {{TabManager_TabMetrics::kIsPinnedName, 0},
     ...
     {TabManager_TabMetrics::kContentTypeName,
      TabEvent::CONTENT_TYPE_TEXT_HTML},
...
class TabActivityWatcherTest : public ChromeRenderViewHostTestHarness {
 protected:
  ...
  void SwitchToTabAt(TabStripModel* tab_strip_model, int new_index) {
    ...
    // which is what actually triggers TabActivityWatcher to log the change, so
    // we manually call WasHidden().
...
// Tests tab events like pinning and navigating.
