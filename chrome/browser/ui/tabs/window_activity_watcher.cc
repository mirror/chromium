...
void LogUkmEntry(const WindowMetrics& window_metrics) {
  ...
  // Bucketize values with a spacing factor that ensures values up to 3 can be
  // specified exactly.
...
void WindowActivityWatcher::OnBrowserSetLastActive(Browser* browser) {
  // TODO(michaelpg): BrowserTabStripTracker should not be forwarding events for
  // browsers it shouldn't track.
...
void WindowActivityWatcher::TabInsertedAt(TabStripModel* tab_strip_model,
  ...
  DCHECK(browser);
  CreateOrUpdateWindowMetrics(browser);
  web_contents_containers_[contents] = browser->session_id().id();
}

void WindowActivityWatcher::TabDetachedAt(content::WebContents* contents,
                                          int index) {
  auto it = web_contents_containers_.find(contents);
  if (it == web_contents_containers_.end())
    return;
  ...
  if (browser)
    CreateOrUpdateWindowMetrics(browser);
  web_contents_containers_.erase(it);
}
...
void WindowActivityWatcher::TabReplacedAt(TabStripModel* tab_strip_model,
  ...
  auto it = web_contents_containers_.find(old_contents);
  if (it != web_contents_containers_.end()) {
  ...
  if (browser_id != 0)
    web_contents_containers_[new_contents] = browser_id;
}
