...
// The value of the largest bucket; this limits the total number of buckets.
constexpr int kMaxBucketValue = 32;
...
void Bucketize(int* value, double spacing_factor = 2) {
  ...
  // Limit the number of buckets by enforcing a maximum value.
  *value = std::min(*value, kMaxBucketValue);
}

// Returns a copy of |raw_metrics| with fields bucketized into appropriate
// ranges for privacy considerations.
WindowMetrics WARN_UNUSED_RESULT
SanitizeWindowMetricsForUkm(const WindowMetrics& raw_metrics) {
