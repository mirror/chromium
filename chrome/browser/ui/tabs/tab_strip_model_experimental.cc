...
TabStripModelExperimental::ConstViewIterator::ConstViewIterator(
    const TabStripModelExperimental* model, int toplevel_index, int inner_index)
    : model_(model),
...
const TabDataExperimental& TabStripModelExperimental::ConstViewIterator::operator*() const {
  return *operator->();
}
...
const TabDataExperimental* TabStripModelExperimental::ConstViewIterator::operator->() const {
  ...
  const TabDataExperimental* toplevel = &model_->tabs_[toplevel_index_];
  if (inner_index_ == -1) {
    DCHECK(toplevel->type() != TabDataExperimental::Type::kGroup);
    return toplevel;
  }

  // References inside the children.
  ...
  return &toplevel->children()[inner_index_];
}

auto TabStripModelExperimental::ConstViewIterator::operator++() -> ConstViewIterator& {
  Increment();
  return *this;
}

auto TabStripModelExperimental::ConstViewIterator::operator++(int) -> ConstViewIterator {
...
void TabStripModelExperimental::ConstViewIterator::Increment() {
  ...
  for (;;) {
    ...
    inner_index_ = -1;
    toplevel_index_++;
    if (toplevel_index_ == model_->tabs_.size())
      return;  // Hit end.

    toplevel = &model_->tabs_[toplevel_index_];
...
void TabStripModelExperimental::ConstViewIterator::Decrement() {
  ...
  for (;;) {
    ...
    if (inner_index_ == -1 && toplevel->CountsAsViewIndex())
      break;  // Okay to select this one with no child.

    // Advance to previous toplevel.
    toplevel_index_--;
    toplevel = &model_->tabs_[toplevel_index_];
    inner_index_ = static_cast<int>(toplevel->children().size()) - 1;
  }
}
...
TabStripModelExperimental::ViewIterator::ViewIterator(TabStripModelExperimental* model, int toplevel_index, int inner_index)
    : ConstViewIterator(model, toplevel_index, inner_index) {}

TabDataExperimental& TabStripModelExperimental::ViewIterator::operator*() const {
...
void TabStripModelExperimental::InsertWebContentsAt(
  ...
  tabs_.emplace_back(contents, this);
  UpdateViewCount();
...
content::WebContents* TabStripModelExperimental::GetWebContentsAt(
  ...
  ConstViewIterator found = FindViewIndex(view_index);
  if (found == end())
    return nullptr;
  return found->contents_;
}

int TabStripModelExperimental::GetIndexOfWebContents(
    const content::WebContents* contents) const {
  // This returns the view index.
  // TODO(brettw) The contents may not have an index into the view even if it
  // exists in the model, which is not what some callers expect. If we keep
  // this model, we should have a way for those callers to make the change they
  // want in the tab strip without round-tripping through view indices.
  int index = 0;
  for (const auto& data : *this) {
    if (contents == data.contents_)
      return index;
    index++;
  }
  return kNoTab;
...
void TabStripModelExperimental::UpdateWebContentsStateAt(
  ...
  for (auto& observer : exp_observers_)
    observer.TabChangedAt(view_index, *found, change_type);
}
...
void TabStripModelExperimental::DetachWebContents(
  ...
  // TODO(brettw) this recomputes the view index even though we iterate below.
  int view_index = GetIndexOfWebContents(web_contents);
  bool was_selected;
  ...
  // TODO(brettw) this assumes only the toplevel tabs.
  size_t toplevel_index = tabs_.size();
  for (size_t i = 0; i < tabs_.size(); i++) {
    if (tabs_[i].contents_ == web_contents) {
      toplevel_index = i;
      break;
    }
  }
  if (toplevel_index == tabs_.size()) {
    NOTREACHED();
    return;
  }

  // TODO(brettw) enhance handling of grouped tabs.
  tabs_.erase(tabs_.begin() + toplevel_index);
  UpdateViewCount();
...
void TabStripModelExperimental::NotifyIfActiveTabChanged(
    ...
    Notify notify_types) {
  ViewIterator found = FindViewIndex(active_index());
  DCHECK(found != end());

  content::WebContents* new_contents = found->contents_;
...
TabStripModelExperimental::FindViewIndex(int view_index) const {
  return const_cast<TabStripModelExperimental*>(this)->FindViewIndex(view_index);
}
