...
class BrowserActivityWatcher : public chrome::BrowserListObserver {
 public:
  ...
  // Returns the ID of a UKM BrowserMetrics entry for the browser. If there is
  // already an entry for the browser and its stats hasn't changed, that entry's
  // ID is returned. Otherwise, a new UKM entry is logged and its ID is
  // returned.
