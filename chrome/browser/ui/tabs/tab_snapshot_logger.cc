...
#include "chrome/browser/ui/tabs/tab_snapshot_event.pb.h"
...
using metrics::TabSnapshotEvent;

namespace {

// Returns the ContentType that matches |mime_type|.
TabSnapshotEvent::ContentType GetContentTypeFromMimeType(
    const std::string& mime_type) {
  // Test for special cases before testing wildcard types.
  if (mime_type.empty())
    return TabSnapshotEvent::CONTENT_TYPE_UNKNOWN;
  if (net::MatchesMimeType("text/html", mime_type))
    return TabSnapshotEvent::CONTENT_TYPE_TEXT_HTML;
  if (net::MatchesMimeType("application/*", mime_type))
    return TabSnapshotEvent::CONTENT_TYPE_APPLICATION;
  if (net::MatchesMimeType("audio/*", mime_type))
    return TabSnapshotEvent::CONTENT_TYPE_AUDIO;
  if (net::MatchesMimeType("image/*", mime_type))
    return TabSnapshotEvent::CONTENT_TYPE_IMAGE;
  if (net::MatchesMimeType("text/*", mime_type))
    return TabSnapshotEvent::CONTENT_TYPE_TEXT;
  if (net::MatchesMimeType("video/*", mime_type))
    return TabSnapshotEvent::CONTENT_TYPE_VIDEO;
  return TabSnapshotEvent::CONTENT_TYPE_OTHER;
}
...
void TabSnapshotLogger::LogBackgroundTab(ukm::SourceId ukm_source_id,
  ...
  ukm::builders::TabManager_TabSnapshot entry(ukm_source_id);

  if (SiteEngagementService::IsEnabled())
    entry.SetSiteEngagementScore(GetSiteEngagementScore(web_contents));

  TabSnapshotEvent::ContentType content_type =
