...
std::vector<TabStripGrouperItem> TabStripGrouper::ComputeGroups() const {
  ...
  while (model_index < model_->count()) {
    content::WebContents* contents = model_->GetWebContentsAt(model_index);

    // Eat all tabs with the current tab as the group.
    int group_last_index = model_index;
    while (group_last_index < model_->count() - 1 &&
           model_->GetGroupAt(group_last_index + 1) == contents)
      group_last_index++;
...
void TabStripGrouper::TabDetachedAt(content::WebContents* contents,
                                    int model_index) {
  // items_.erase(items_.begin() + model_index);
  Regroup();
}
