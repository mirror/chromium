...
class TabsTrackerTest : public ChromeRenderViewHostTestHarness {
 public:
  TabsTrackerTest() {
    ...
    tabs::TabsTracker::GetInstance()->SetTabLoggerForTest(
        std::move(mock_logger_transient));
  }

  ~TabsTrackerTest() override {
    tabs::TabsTracker::GetInstance()->SetTabLoggerForTest(nullptr);
  }
...
TEST_F(TabsTrackerTest, Basic) {
  ...
  EXPECT_CALL(*mock_logger(),
              LogBackgroundTab(_, test_contents_2, tab_strip, 0))
      .Times(1);
  ...
  EXPECT_CALL(*mock_logger(),
              LogBackgroundTab(_, test_contents_1, tab_strip, 1))
      .Times(1);
