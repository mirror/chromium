...
// Adds the handler to the entry as a protocol handler and, if appropriate, a
// default protocol handler.
void PopulateSchemeForHandler(ProtocolHandlerRegistry* registry,
                              const ProtocolHandler& handler,
                              ukm::builders::TabManager_TabMetrics* entry) {
  auto scheme_value = GetSchemeValueFromString(handler.protocol());
  entry->SetProtocolHandler(scheme_value);
  // A default protocol handler is the most recently accepted handler for
  // its scheme.
  if (registry->IsDefault(handler))
    entry->SetDefaultProtocolHandler(scheme_value);
}
...
void PopulateProtocolHandlers(content::WebContents* web_contents,
  ...
  // Ignored protocol handlers (as opposed to denied) are stored separately.
  for (const auto& handler : registry->GetIgnoredHandlers())
    PopulateSchemeForHandler(registry, handler, entry);
}
