...
void TabUIHelper::NotifyInitialNavigationDelayed(bool is_navigation_delayed) {
  ...
  tab_ui_data_.reset(new TabUIData(web_contents()->GetVisibleURL()));
...
void TabUIHelper::FetchFaviconFromHistory(
  ...
  favicon_service->GetFaviconImageForPageURL(url, callback, &favicon_tracker_);
}
...
bool TabUIHelper::ShouldUseFaviconFromWebContents() const {
  return web_contents()->GetController().IsInitialNavigation() &&
         created_by_session_restore_ && !was_active_at_least_once_;
}
