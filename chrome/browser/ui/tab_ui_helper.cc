// Copyright 2017 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#include "chrome/browser/ui/tab_ui_helper.h"

#include "chrome/browser/favicon/favicon_service_factory.h"
#include "chrome/browser/favicon/favicon_utils.h"
#include "chrome/browser/profiles/profile.h"
#include "chrome/browser/sessions/session_restore.h"
#include "components/favicon/core/favicon_service.h"
#include "components/keyed_service/core/service_access_type.h"
#include "components/url_formatter/url_formatter.h"
#include "content/public/browser/invalidate_type.h"
#include "ui/base/resource/resource_bundle.h"
#include "ui/resources/grit/ui_resources.h"
#include "url/gurl.h"

DEFINE_WEB_CONTENTS_USER_DATA_KEY(TabUIHelper);

namespace {

base::string16 FormatUrlToSubdomain(const GURL& url) {
  base::string16 formated_url = url_formatter::FormatUrl(
      url, url_formatter::kFormatUrlExperimentalOmitTrivialSubdomains,
      net::UnescapeRule::SPACES, nullptr, nullptr, nullptr);
  return base::UTF8ToUTF16(GURL(formated_url).host());
}

}  // namespace

TabUIHelper::TabUIHelper(content::WebContents* contents)
    : WebContentsObserver(contents),
      favicon_(ui::ResourceBundle::GetSharedInstance().GetImageNamed(
          IDR_DEFAULT_FAVICON)),
      weak_ptr_factory_(this) {}
TabUIHelper::~TabUIHelper() {}

void TabUIHelper::DidStopLoading() {
  // Reset the properties after the initial navigation finishes loading, so that
  // latter navigations are not affected.
  is_navigation_delayed_ = false;
  created_by_session_restore_ = false;
  title_.clear();
  favicon_ = ui::ResourceBundle::GetSharedInstance().GetImageNamed(
      IDR_DEFAULT_FAVICON);
}

void TabUIHelper::NotifyNavigationDelayed(bool is_navigation_delayed) {
  is_navigation_delayed_ = is_navigation_delayed;
  if (!is_navigation_delayed_)
    return;

  title_ = FormatUrlToSubdomain(web_contents()->GetVisibleURL());
  web_contents()->NotifyNavigationStateChanged(content::INVALIDATE_TYPE_TAB);

  // Fetch favicon from history service.
  Profile* profile =
      Profile::FromBrowserContext(web_contents()->GetBrowserContext());
  favicon::FaviconService* favicon_service =
      FaviconServiceFactory::GetForProfile(profile,
                                           ServiceAccessType::EXPLICIT_ACCESS);
  favicon_service->GetFaviconImageForPageURL(
      web_contents()->GetVisibleURL(),
      base::Bind(&TabUIHelper::OnFaviconFetched,
                 weak_ptr_factory_.GetWeakPtr()),
      &favicon_tracker_);
}

const base::string16& TabUIHelper::GetTitle() const {
  const base::string16& contents_title = web_contents()->GetTitle();
  if (!contents_title.empty())
    return contents_title;

  return title_;
}

gfx::ImageSkia TabUIHelper::GetFavicon() const {
  if (is_navigation_delayed_ && !was_active_at_least_once_)
    return favicon_.AsImageSkia();

  return favicon::TabFaviconFromWebContents(web_contents()).AsImageSkia();
}

void TabUIHelper::OnFaviconFetched(
    const favicon_base::FaviconImageResult& favicon) {
  if (favicon.image.IsEmpty())
    return;
  favicon_ = favicon.image;
  web_contents()->NotifyNavigationStateChanged(content::INVALIDATE_TYPE_TAB);
}
