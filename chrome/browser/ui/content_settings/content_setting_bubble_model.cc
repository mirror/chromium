...
void ContentSettingSimpleBubbleModel::SetManageText() {
  static const ContentSettingsTypeIdEntry kLinkIDs[] = {
      ...
      {CONTENT_SETTINGS_TYPE_POPUPS,
       UseMd() ? IDS_MANAGE : IDS_BLOCKED_POPUPS_LINK},
...
ContentSettingPopupBubbleModel::ContentSettingPopupBubbleModel(
  ...
  set_manage_text_style(UseMd()
                            ? ContentSettingBubbleModel::ManageTextStyle::Button
                            : ContentSettingBubbleModel::ManageTextStyle::Link);
...
ContentSettingPopupBubbleModel::CreateListItem(int32_t id, const GURL& url) {
  ...
  if (UseMd()) {
    // Insert the unicode single dot bullet code-point \u2022.
    title.insert(0, base::UTF8ToUTF16("\xE2\x80\xA2 "));
  }

  return ListItem(UseMd()
                      ? gfx::Image()
                      : ui::ResourceBundle::GetSharedInstance().GetImageNamed(
                            IDR_DEFAULT_FAVICON),
                  title, true, id);
...
void ContentSettingSubresourceFilterBubbleModel::SetManageText() {
  ...
  set_manage_text(l10n_util::GetStringUTF16(
      base::FeatureList::IsEnabled(
          ...
          : IDS_ALLOW_ADS));
  set_manage_text_style(ContentSettingBubbleModel::ManageTextStyle::Checkbox);
}
