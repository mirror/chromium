...
IN_PROC_BROWSER_TEST_F(ContentSettingFramebustBlockBubbleModelTest,
                       AllowRedirection) {
  const GURL blocked_url1(chrome::kChromeUIHistoryURL);
  const GURL blocked_url2(chrome::kChromeUISettingsURL);
  const GURL blocked_url3(chrome::kChromeUIVersionURL);

  // Signal that a blocked redirection happened.
  auto* helper = FramebustBlockTabHelper::FromWebContents(GetWebContents());
  helper->AddBlockedUrl(blocked_url1);
  helper->AddBlockedUrl(blocked_url2);
  helper->AddBlockedUrl(blocked_url3);
  ...
  EXPECT_EQ(blocked_url2, GetWebContents()->GetLastCommittedURL());
}
