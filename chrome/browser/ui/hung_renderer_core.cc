...
// process.
bool IsWebContentsAffected(content::WebContents* web_contents,
...
std::vector<content::WebContents*> GetAffectedWebContentsList(
    content::WebContents* hung_web_contents,
    content::RenderProcessHost* hung_process) {
  std::vector<content::WebContents*> result;

  // |hung_web_contents| is always first.
  DCHECK(IsWebContentsAffected(hung_web_contents, hung_process));
  result.push_back(hung_web_contents);

  for (TabContentsIterator it; !it.done(); it.Next()) {
    if (*it != hung_web_contents && IsWebContentsAffected(*it, hung_process) &&
        !it->IsCrashed())
      result.push_back(*it);
  }
...
base::string16 GetAffectedWebContentsTitle(
  ...
  // N.B. using the raw host here is OK since this is a notification and the
  // user doesn't need to make a security critical decision about the page
  // in this dialog.
  return l10n_util::GetStringFUTF16(IDS_BROWSER_HANGMONITOR_IFRAME_TITLE,
                                    base::UTF8ToUTF16(hung_url.host()),
                                    page_title);
}
