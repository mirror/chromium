...
class Exporter : public ui::SelectFileDialog::Listener {
 public:
  Exporter(content::WebContents* web_contents,
           ...
           net::ScopedCERTCertificateVector::iterator certs_begin,
           net::ScopedCERTCertificateVector::iterator certs_end);
  ~Exporter() override;
 ...
 private:
  ...
  net::ScopedCERTCertificateVector cert_chain_list_;
};
...
Exporter::Exporter(content::WebContents* web_contents,
  ...
  for (net::ScopedCERTCertificateVector::iterator i = certs_begin;
       i != certs_end; ++i) {
    cert_chain_list_.push_back(net::x509_util::DupCERTCertificate(*i));
  }

  // TODO(mattm): should this default to some directory?
  // Maybe SavePackage::GetSaveDirPreference? (Except that it's private.)
  std::string cert_title = x509_certificate_model::GetTitle(certs_begin->get());
...
void ShowCertExportDialog(content::WebContents* web_contents,
  ...
  net::ScopedCERTCertificateVector cert_chain =
      net::x509_util::CreateCERTCertificateVectorFromX509Certificate(
          cert.get());
  // XXX change exporter constructor so it takes a ScopedCERTCertificateVector.
