...
IN_PROC_BROWSER_TEST_F(NewTabPageNavigationThrottleTest, BadCertThrottle) {
  https_test_server()->SetSSLConfig(net::EmbeddedTestServer::CERT_EXPIRED);
  ASSERT_TRUE(https_test_server()->Start());
  ...
  // Bad cert makes a redirect to the local NTP.
