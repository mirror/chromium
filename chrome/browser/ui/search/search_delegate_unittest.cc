...
TEST_F(SearchDelegateTest, SearchModel) {
  // Initial state.
  EXPECT_EQ(SearchModel::ORIGIN_DEFAULT, browser()->search_model()->origin());
  ...
  SearchTabHelper::FromWebContents(first_tab)->model()->SetOrigin(
      SearchModel::ORIGIN_NTP);
  EXPECT_EQ(SearchModel::ORIGIN_NTP, browser()->search_model()->origin());
