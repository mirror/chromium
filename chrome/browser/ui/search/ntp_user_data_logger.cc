...
void NTPUserDataLogger::LogMostVisitedImpression(
  ...
  impression_tile_name_source_[position] = tile_name_source;
}
...
void NTPUserDataLogger::LogMostVisitedNavigation(
    ...
    ntp_tiles::TileSource tile_source,
    ntp_tiles::TileVisualType tile_type) {
  ntp_tiles::metrics::RecordTileClick(position, tile_name_source, tile_source,
                                      tile_type);
...
NTPUserDataLogger::NTPUserDataLogger(content::WebContents* contents)
    : content::WebContentsObserver(contents),
      impression_tile_name_source_(kNumMostVisited),
...
void NTPUserDataLogger::EmitNtpStatistics(base::TimeDelta load_time) {
  ...
  for (int i = 0; i < kNumMostVisited; i++) {
    ...
    // Rappor metrics.
    ntp_tiles::metrics::RecordTileImpression(i, impression_tile_name_source_[i],
                                             impression_tile_source_[i],
