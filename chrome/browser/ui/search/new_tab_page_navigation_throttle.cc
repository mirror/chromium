...
NewTabPageNavigationThrottle::MaybeCreateThrottleFor(
  ...
  if (web_contents->GetURL() != chrome::kChromeUINewTabURL ||
      handle->GetURL() != search::GetNewTabPageURL(profile))
    return nullptr;

  return std::MakeUnique<NewTabPageNavigationThrottle>(handle);
}

content::NavigationThrottle::ThrottleCheckResult
NewTabPageNavigationThrottle::WillProcessResponse() {
  net::HttpResponseHeaders* headers = navigation_handle()->GetResponseHeaders();
  if (!headers)
