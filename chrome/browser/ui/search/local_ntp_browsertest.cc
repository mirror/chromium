...
IN_PROC_BROWSER_TEST_F(LocalNTPVoiceSearchSmokeTest,
  ...
  bool fakebox_speech_is_hidden = false;
  ASSERT_TRUE(instant_test_utils::GetBoolFromJS(
      active_tab, "document.getElementById('fakebox-speech').hidden",
      &fakebox_speech_is_hidden));
  EXPECT_FALSE(fakebox_speech_is_hidden);
