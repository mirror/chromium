...
class LocalNTPDoodleSmokeTest : public InProcessBrowserTest {
 protected:
  LocalNTPDoodleSmokeTest() {}
 ...
 private:
  ...
  void SetUpInProcessBrowserTestFixture() override {
    will_create_browser_context_services_subscription_ =
        BrowserContextDependencyManager::GetInstance()
            ->RegisterWillCreateBrowserContextServicesCallbackForTesting(
                base::Bind(&LocalNTPDoodleSmokeTest::
                               OnWillCreateBrowserContextServices,
                           base::Unretained(this)));
  }
  ...
  void OnWillCreateBrowserContextServices(content::BrowserContext* context) {
    LogoServiceFactory::GetInstance()->SetTestingFactory(
        context, &LocalNTPDoodleSmokeTest::CreateLogoService);
  }
...
IN_PROC_BROWSER_TEST_F(LocalNTPDoodleSmokeTest,
...
IN_PROC_BROWSER_TEST_F(LocalNTPDoodleSmokeTest, ShouldShowDoodleWhenAvailable) {
  logo_service()->cached_reason = LogoCallbackReason::DETERMINED;
