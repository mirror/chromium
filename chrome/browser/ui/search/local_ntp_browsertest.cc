...
class LocalNTPTest : public InProcessBrowserTest {
 public:
  ...
  // Navigates to chrome://newtab and waits until the NTP is fully loaded. Note
  // that simply waiting for a navigation is not enough, since the MV iframe
  // received the tiles asynchronously.
  void NavigateToNTPAndWaitUntilLoaded(content::WebContents* active_tab) {
...
IN_PROC_BROWSER_TEST_F(LocalNTPTest, GoogleNTPLoadsWithoutError) {
  ...
  // Navigate to the NTP.
  NavigateToNTPAndWaitUntilLoaded(active_tab);
