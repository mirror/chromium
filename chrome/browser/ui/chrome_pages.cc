...
void ShowBrowserSignin(Browser* browser,
  ...
  bool can_show_avatar_bubble = false;
#else
  // The sign-in modal dialog is presented as a tab-modal dialog (which is
  // automatically dismissed when the page navigates). So displaying the
  // dialog on a new tab that loads any page will lead to it being dismissed as
  // soon as the new tab is loaded.
  //
  // So the sign-in dialog must only be presented on top of an existing tab.
  // Fallback to the full-tab sign-in flow if the browser does not have any tab.
  bool can_show_avatar_bubble = !browser->tab_strip_model()->empty();
