...
struct ModifierToLabel {
  ...
  const char* label;
    ...
    {ui::kSearchKey, "search"},
    {ui::kControlKey, "ctrl"},
    {ui::kAltKey, "alt"},
    {ui::kVoidKey, "disabled"},
    {ui::kCapsLockKey, "caps lock"},
    {ui::kEscapeKey, "esc"},
    {ui::kBackspaceKey, "backspace"},
};
...
void KeyboardOverlayHandler::GetLabelMap(const base::ListValue* args) {
  ...
  modifier_map[ui::kSearchKey] = static_cast<ModifierKey>(
      pref_service->GetInteger(prefs::kLanguageRemapSearchKeyTo));
  modifier_map[ui::kControlKey] = static_cast<ModifierKey>(
      pref_service->GetInteger(prefs::kLanguageRemapControlKeyTo));
  modifier_map[ui::kAltKey] = static_cast<ModifierKey>(
