...
void GaiaScreenHandler::ShowGaiaScreenIfReady() {
  ...
  if (!untrusted_authorities_cache_) {
    ...
    // When the WebUI is destroyed, |untrusted_authorities_cache_| will go out
    // of scope and the certificates will not be held in memory anymore.
    untrusted_authorities_cache_ =
        std::make_unique<policy::UntrustedAuthoritiesCache>(
            policy::UntrustedAuthoritiesCache::
