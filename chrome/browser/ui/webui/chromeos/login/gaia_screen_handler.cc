...
void GaiaScreenHandler::HandleIdentifierEntered(const std::string& user_email) {
  if (!chromeos::LoginDisplayHost::default_host()->IsUserWhitelisted(
...
void GaiaScreenHandler::HandleCompleteAdAuthentication(
  ...
  chromeos::LoginDisplayHost::default_host()->SetDisplayEmail(username);
...
void GaiaScreenHandler::HandleCompleteAuthentication(
  ...
  chromeos::LoginDisplayHost::default_host()->SetDisplayEmail(sanitized_email);
...
void GaiaScreenHandler::HandleGaiaUIReady() {
  ...
  if (chromeos::LoginDisplayHost::default_host())
    chromeos::LoginDisplayHost::default_host()->OnGaiaScreenReady();
}
...
void GaiaScreenHandler::ShowGaiaScreenIfReady() {
  ...
  if (populated_email_.empty()) {
    chromeos::LoginDisplayHost::default_host()->LoadSigninWallpaper();
  } else {
    chromeos::LoginDisplayHost::default_host()->LoadWallpaper(
        user_manager::known_user::GetAccountId(
