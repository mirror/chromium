...
void CoreOobeHandler::RegisterMessages() {
  ...
  AddRawCallback("getPrimaryDisplayName",
                 &CoreOobeHandler::HandleGetPrimaryDisplayName);
}
...
void CoreOobeHandler::HandleGetPrimaryDisplayName(const base::ListValue* args) {
  CHECK_EQ(1U, args->GetSize());
  const base::Value* callback_id;
  CHECK(args->Get(0, &callback_id));

  auto primary_display_id =
