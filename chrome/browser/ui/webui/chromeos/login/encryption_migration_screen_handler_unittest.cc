...
// Mock WakeLock implementation, required by EncryptionMigrationScreenHandler.
...
class TestEncryptionMigrationScreenHandler
 ...
 public:
  ...
  MockWakeLock* mock_wake_lock() { return &mock_wake_lock_; }

 protected:
  // Used as free disk space fetcher callback.
  int64_t FreeDiskSpaceFetcher() { return free_disk_space_; }

  // Override GetWakeLock -- returns our own MockWakeLock.
  device::mojom::WakeLock* GetWakeLock() override { return &mock_wake_lock_; }
...
class EncryptionMigrationScreenHandlerTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    // Setup fake DBusThreadManager parts.
    auto fake_cryptohome_client =
        base::MakeUnique<chromeos::FakeCryptohomeClient>();
    ...
    encryption_migration_screen_handler_->SetContinueLoginCallback(
        base::BindOnce(&EncryptionMigrationScreenHandlerTest::ContinueLogin,
                       base::Unretained(this)));
    encryption_migration_screen_handler_->SetRestartLoginCallback(
        base::BindOnce(&EncryptionMigrationScreenHandlerTest::RestartLogin,
                       base::Unretained(this)));
 ...
 protected:
  ...
  void ContinueLogin(const UserContext& user_context) {
...
TEST_F(EncryptionMigrationScreenHandlerTest, MinimalMigration) {
  ...
  EXPECT_TRUE(
      encryption_migration_screen_handler_->mock_wake_lock()->HasWakeLock());
