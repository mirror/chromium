...
class TestCryptohomeClient : public FakeCryptohomeClient {
 public:
  ...
  const cryptohome::MigrateToDircryptoRequest& dircrypto_request() const {
    return dircrypto_request_;
  }

  const cryptohome::Identification& id() const { return id_; }

  const cryptohome::MountRequest& mount_request() const {
    return mount_request_;
  }
 ...
 private:
  ...
  cryptohome::MigrateToDircryptoRequest dircrypto_request_;
  cryptohome::Identification id_;
  cryptohome::MountRequest mount_request_;
...
TEST_F(EncryptionMigrationScreenHandlerTest, MinimalMigration) {
  ...
  EXPECT_FALSE(
      encryption_migration_screen_handler_->fake_wake_lock()->HasWakeLock());
  EXPECT_TRUE(
      fake_cryptohome_client_->mount_request().to_migrate_from_ecryptfs());
  EXPECT_TRUE(fake_cryptohome_client_->dircrypto_request().minimal_migration());
