...
TEST_F(EncryptionMigrationScreenHandlerTest, MinimalMigration) {
  ...
  EXPECT_FALSE(
      encryption_migration_screen_handler_->fake_wake_lock()->HasWakeLock());
  EXPECT_TRUE(fake_cryptohome_client_->get_last_mount_request()
                  .to_migrate_from_ecryptfs());
  EXPECT_TRUE(fake_cryptohome_client_->get_last_migrate_to_dircrypto_request()
                  .minimal_migration());
  EXPECT_EQ(cryptohome::Identification(user_context_.GetAccountId()),
            fake_cryptohome_client_->get_last_migrate_to_dircrypto_id());
}
