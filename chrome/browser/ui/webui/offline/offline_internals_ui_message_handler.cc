...
void OfflineInternalsUIMessageHandler::HandleGeneratePageBundle(
  ...
  std::string message =
      base::StringPrintf("Added %u candidate URLs.",
                         static_cast<unsigned int>(prefetch_urls.size()));
  if (prefetch_urls.size() < page_urls.size()) {
    size_t invalid_urls_count = page_urls.size() - prefetch_urls.size();
    message.append(
        base::StringPrintf(" Ignored %u invalid URLs.",
                           static_cast<unsigned int>(invalid_urls_count)));
  }
  message.append("\n");
