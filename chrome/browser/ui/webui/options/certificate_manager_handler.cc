...
class CertIdMap {
 ...
 private:
  ...
  IDMap<net::ScopedCERTCertificate> id_map_;
...
void CertificateManagerHandler::View(const base::ListValue* args) {
  ...
  certs.emplace_back(net::x509_util::DupCERTCertificate(cert));
#if defined(OS_CHROMEOS)
...
void CertificateManagerHandler::ImportServerFileRead(const int* read_errno,
  ...
  selected_cert_list_ = net::x509_util::CreateCertificateListFromBytes(
...
void CertificateManagerHandler::Export(const base::ListValue* args) {
  ...
  if (!cert)
    return;
  net::ScopedCERTCertificateVector export_certs;
  export_certs.emplace_back(net::x509_util::DupCERTCertificate(cert));
...
void CertificateManagerHandler::PopulateTree(
...

  {
    ...
    for (CertificateManagerModel::OrgGroupingMap::iterator i = map.begin();
      ...
      for (net::ScopedCERTCertificateVector::const_iterator org_cert_it =
               i->second.begin();
        ...
        cert_dict->SetString(
            kNameId, certificate_manager_model_->GetColumnText(
