...
const char kAboutDiscardsAllowUnsafeCommand[] = "allow_unsafe";
  ...
  for (resource_coordinator::TabStatsList::iterator it = stats.begin();
    ...
    if (!it->is_discarded) {
      ...
      str += base::StringPrintf(
          ...
          it->tab_contents_id, kAboutDiscardsAllowUnsafeCommand);
      str += "</ul>";
    }
...
std::string AboutDiscards(const std::string& path) {
  ...
  if (!url_split.empty()) {
    bool allow_unsafe_shutdown =
        (url_split.size() > 1 &&
         url_split[1] == kAboutDiscardsAllowUnsafeCommand);
    ...
    if (path_split.size() == 2 && path_split[0] == kAboutDiscardsRunCommand &&
      ...
      tab_manager->DiscardTabById(web_content_id, allow_unsafe_shutdown);
      return BuildAboutDiscardsRunPage();
    } else if (path_split.size() == 1 &&
               path_split[0] == kAboutDiscardsRunCommand) {
      tab_manager->DiscardTab(allow_unsafe_shutdown);
      return BuildAboutDiscardsRunPage();
    }
  }
