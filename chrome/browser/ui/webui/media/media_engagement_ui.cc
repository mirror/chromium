...
class MediaEngagementScoreDetailsProviderImpl
 ...
 public:
  ...
  void GetMediaEngagementScoreDetails(
    ...
    std::vector<media::mojom::MediaEngagementScoreDetailsPtr> engagement_info;
    engagement_info.reserve(details.size());
    for (const auto& info : details) {
      media::mojom::MediaEngagementScoreDetailsPtr origin_info(
          media::mojom::MediaEngagementScoreDetails::New());
