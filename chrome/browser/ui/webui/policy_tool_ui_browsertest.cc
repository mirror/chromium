...
void PolicyToolUITest::SetUpDefaultCommandLine(
    base::CommandLine* command_line) {
  command_line->AppendSwitch(switches::kEnablePolicyTool);
}
...
IN_PROC_BROWSER_TEST_F(PolicyToolUITest, CreatingSessionFiles) {
  ...
  const base::FilePath default_session_path =
      GetSessionsDir().AppendASCII("policy.json");
  ...
  // created.
  const std::string test_session_name = "test_creating_sessions";

  LoadSession(test_session_name);

  const base::FilePath test_session_path =
      GetSessionsDir().AppendASCII(test_session_name + ".json");
  EXPECT_TRUE(PathExists(test_session_path));

  // Check that unicode characters are valid for the session name.
  const std::string russian_session_name = "сессия";

  LoadSession(russian_session_name);

  const base::FilePath russian_session_path =
      GetSessionsDir().AppendASCII(russian_session_name + ".json");
...
IN_PROC_BROWSER_TEST_F(PolicyToolUITest, ImportingSession) {
  ...
  base::WriteFile(GetSessionsDir().AppendASCII("test_session.json"),
...
IN_PROC_BROWSER_TEST_F(PolicyToolUITest, Editing) {
  ...
  base::ReadFileToString(GetSessionsDir().AppendASCII("policy.json"),
                         &file_contents);
...
IN_PROC_BROWSER_TEST_F(PolicyToolUITest, InvalidJson) {
  ui_test_utils::NavigateToURL(browser(), GURL("chrome://policy-tool"));
  base::ThreadRestrictions::ScopedAllowIO allow_io;
  base::WriteFile(GetSessionsDir().AppendASCII("test_session.json"), "{", 1);
