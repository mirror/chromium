...
IN_PROC_BROWSER_TEST_F(PolicyToolUITest, CreatingSessionFiles) {
  ...
  // created.
  const base::FilePath::StringType test_session_name =
      FILE_PATH_LITERAL("test_creating_sessions");

  LoadSession(test_session_name);

  const base::FilePath test_session_path =
      GetSessionsDir().Append(test_session_name + FILE_PATH_LITERAL(".json"));
  EXPECT_TRUE(PathExists(test_session_path));

  // Check that unicode characters are valid for the session name.
  const base::FilePath::StringType russian_session_name =
      FILE_PATH_LITERAL("сессия");

  LoadSession(russian_session_name);

  const base::FilePath russian_session_path = GetSessionsDir().AppendASCII(
      russian_session_name + FILE_PATH_LITERAL(".json"));
