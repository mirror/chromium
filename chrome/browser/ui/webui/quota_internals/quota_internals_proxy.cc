...
void QuotaInternalsProxy::RequestInfo(
  ...
  quota_manager_->GetGlobalUsage(
      ...
      base::Bind(&QuotaInternalsProxy::DidGetGlobalUsage,
                 weak_factory_.GetWeakPtr(), blink::StorageType::kTemporary));

  quota_manager_->GetGlobalUsage(
      blink::StorageType::kPersistent,
      base::Bind(&QuotaInternalsProxy::DidGetGlobalUsage,
                 weak_factory_.GetWeakPtr(), blink::StorageType::kPersistent));

  quota_manager_->GetGlobalUsage(
      blink::StorageType::kSyncable,
      base::Bind(&QuotaInternalsProxy::DidGetGlobalUsage,
                 weak_factory_.GetWeakPtr(), blink::StorageType::kSyncable));
...
void QuotaInternalsProxy::DidGetSettings(
  ...
  GlobalStorageInfo info(blink::StorageType::kTemporary);
...
void QuotaInternalsProxy::DidGetHostUsage(const std::string& host,
                                          StorageType type,
                                          int64_t usage) {
  DCHECK(type == blink::StorageType::kTemporary ||
         type == blink::StorageType::kPersistent ||
         type == blink::StorageType::kSyncable);
