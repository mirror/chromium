...
void FullScreenTabWithJavascript(content::WebContents* web_contents) {
  ...
  // Note: We expect the renderer to process the ExecuteJavaScript() (above) and
  // the mouse events (below) sequentially, since the IPC messages should be
  // received in-order and then processed on the main thread in-order.
