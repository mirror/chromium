...
void MediaRouterWebUIMessageHandler::UpdateMediaRouteStatus(
  ...
  if (controller && controller->GetType() == RouteControllerType::kMirroring) {
    const MirroringMediaRouteController* mirroring_controller =
        MirroringMediaRouteController::From(controller);
    DCHECK(mirroring_controller);
    auto mirroring_extra_data = base::MakeUnique<base::DictionaryValue>();
    mirroring_extra_data->SetBoolean(
        "mediaRemotingEnabled",
        mirroring_controller->GetMediaRemotingEnabled());
