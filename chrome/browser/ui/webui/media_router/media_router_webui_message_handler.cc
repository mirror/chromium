...
void MediaRouterWebUIMessageHandler::UpdateMediaRouteStatus(
  ...
  if (mirroring_controller) {
    ...
    bool media_remoting_enabled =
        mirroring_controller->GetMediaRemotingEnabled(pref_service);

    auto mirroring_extra_data = base::MakeUnique<base::DictionaryValue>();
    mirroring_extra_data->SetBoolean("mediaRemotingEnabled",
                                     media_remoting_enabled);
...
void MediaRouterWebUIMessageHandler::OnSetMediaRemotingEnabled(
  ...
  PrefService* pref_service = Profile::FromWebUI(web_ui())->GetPrefs();
  mirroring_controller->SetMediaRemotingEnabled(enabled, pref_service);
}
