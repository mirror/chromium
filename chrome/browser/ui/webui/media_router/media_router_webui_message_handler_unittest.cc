...
TEST_F(MediaRouterWebUIMessageHandlerTest, UpdateMediaRouteStatus) {
  ...
  status.volume = 0.9;
  MirroringStatusExtraData mirroring_extra_data = {MediaRemotingPolicy::ALWAYS};
  ...
  EXPECT_EQ(static_cast<int>(status.mirroring_extra_data->policy),
            GetIntegerFromDict(status_value, "mirroringExtraData.policy"));
}
