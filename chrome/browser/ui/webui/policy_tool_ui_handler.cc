...
std::string PolicyToolUIHandler::ReadOrCreateFileCallback() {
  ...
  const base::FilePath session_path =
      sessions_dir_.Append(session_name_)
...
bool PolicyToolUIHandler::IsValidSessionName(
  ...
  base::FilePath session_path = sessions_dir_.Append(name).AddExtension("json");
...
void PolicyToolUIHandler::DoUpdateSession(const std::string& contents) {
  if (base::WriteFile(sessions_dir_.Append(session_name_)
                          .AddExtension(FILE_PATH_LITERAL(".json")),
                      contents.c_str(),
