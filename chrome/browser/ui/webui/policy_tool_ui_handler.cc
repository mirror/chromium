...
void PolicyToolUIHandler::HandleDeleteSession(const base::ListValue* args) {
  ...
  // Clear the current session name. We need to recompute the current session
  // name after deleting so that if the current session is deleted, a different
  // session is selected correctly.
  session_name_.clear();
  base::PostTaskWithTraitsAndReplyWithResult(
      FROM_HERE, {base::MayBlock(), base::TaskPriority::USER_VISIBLE},
