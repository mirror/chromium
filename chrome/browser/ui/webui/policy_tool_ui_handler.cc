...
base::ListValue PolicyToolUIHandler::GetSessionsList() {
  ...
  using Session = std::pair<base::Time, base::Value>;
  ...
  for (base::FilePath name = enumerator.Next(); !name.empty();
    ...
    sessions.push_back(
        {info.last_accessed,
         base::Value(name.BaseName().RemoveExtension().value())});
  }
  ...
  // session_names.GetList().resize(sessions.size());
  std::transform(sessions.begin(), sessions.end(),
                 std::back_inserter(session_names.GetList()),
                 [](Session& session) { return std::move(session.second); });
