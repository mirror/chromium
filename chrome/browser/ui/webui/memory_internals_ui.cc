...
class MemoryInternalsDOMHandler : public content::WebUIMessageHandler {
 ...
 private:
  ...
  static void GetOutputFileOnFileThread(int32_t pid);
  static void HandleDumpProcessOnUIThread(int32_t pid, base::File f);
...
// static
void MemoryInternalsDOMHandler::GetOutputFileOnFileThread(int32_t pid) {
...
// static
void MemoryInternalsDOMHandler::HandleDumpProcessOnUIThread(int32_t pid,
                                                            base::File f) {
  ...
  mojo::ScopedHandle sh = mojo::WrapPlatformFile(f.TakePlatformFile());
  memlog->DumpProcess(pid, std::move(sh));
}
