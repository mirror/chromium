...
InterventionsInternalsUI::InterventionsInternalsUI(content::WebUI* web_ui)
    : MojoWebUIController(web_ui), ui_service_(nullptr) {
  ...
  ui_service_ = previews_service->previews_ui_service();
  ui_nqe_service_ =
...
void InterventionsInternalsUI::BindUIHandler(
  ...
  page_handler_.reset(
      new InterventionsInternalsPageHandler(std::move(request), ui_service_));
