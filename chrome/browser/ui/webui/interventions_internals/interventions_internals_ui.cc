...
content::WebUIDataSource* GetErrorSource() {
  content::WebUIDataSource* source = content::WebUIDataSource::Create(
      chrome::kChromeUIInterventionsInternalsHost);
  source->SetDefaultResource(IDR_INTERVENTIONS_INTERNALS_ERROR_PAGE_HTML);
  source->UseGzip(std::vector<std::string>());
  return source;
}
...
InterventionsInternalsUI::InterventionsInternalsUI(content::WebUI* web_ui)
  ...
  if (!previews_service) {
    // In Guest Mode.
    content::WebUIDataSource::Add(profile, GetErrorSource());
    return;
  }
