...
// Mock class of InterventionsInternalsPageHandler. For injecting
// PreviewsLogger.
class TestInterventionsInternalsPageHandler
    : public InterventionsInternalsPageHandler {
 public:
  TestInterventionsInternalsPageHandler(
      mojom::InterventionsInternalsPageHandlerRequest request)
      : InterventionsInternalsPageHandler(std::move(request), nullptr),
        logger_(new previews::PreviewsLogger()) {}

  ~TestInterventionsInternalsPageHandler() override {
    logger_->RemoveObserver(this);
  }

  // InterventionsInternalsPageHandler:
  previews::PreviewsLogger* logger() const override { return logger_; }
...
class InterventionsInternalsPageHandlerTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    page_handler = base::MakeUnique<TestInterventionsInternalsPageHandler>(
        std::move(handler_request));
 ...
 protected:
  ...
  std::unique_ptr<TestInterventionsInternalsPageHandler> page_handler;
