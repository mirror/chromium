...
class TestInterventionsInternalsPage
 ...
 public:
  ...
  void UpdateBlacklistedHost(const std::string& host, int64_t time) override {
    host_blacklisted_ = host;
    host_blacklisted_time_ = time;
  }
  void UpdateUserBlacklisted(int64_t time) override {
    user_blacklisted_time_ = time;
  }
  void UpdateUserNotBlacklisted(int64_t time) override {
    user_not_blacklisted_time_ = time;
  }
  void UpdateBlacklistCleared(int64_t time) override {
  ...
  int64_t user_blacklisted_time() const { return user_blacklisted_time_; }
  int64_t user_not_blacklisted_time() const {
 ...
 private:
  ...
  int64_t user_blacklisted_time_;
  int64_t user_not_blacklisted_time_;
...
TEST_F(InterventionsInternalsPageHandlerTest, OnUserBlacklistedPostToPage) {
  ...
  for (auto expected_time : times) {
    page_handler_->OnUserBlacklisted(expected_time);
    base::RunLoop().RunUntilIdle();

    EXPECT_EQ(expected_time.ToJavaTime(), page_->user_blacklisted_time());
  }
}

TEST_F(InterventionsInternalsPageHandlerTest, OnUserNotBlacklistedPostToPage) {
  base::Time times[] = {
      base::Time::FromJsTime(-413696806000),  // Nov 21 1956 20:13:14 UTC
      base::Time::FromJsTime(758620800000),   // Jan 15 1994 08:00:00 UTC
      base::Time::FromJsTime(1581696550000),  // Feb 14 2020 16:09:10 UTC
  ...
  for (auto expected_time : times) {
    page_handler_->OnUserNotBlacklisted(expected_time);
    base::RunLoop().RunUntilIdle();

    EXPECT_EQ(expected_time.ToJavaTime(), page_->user_not_blacklisted_time());
  }
}
