...
#include "net/nqe/effective_connection_type.cc"
...
constexpr char kForceEctFlag[] = "forcedEctFlag";
constexpr char kNoScriptFlag[] = "forcedNoScriptFlag";
constexpr char kOfflinePageFlag[] = "forcedOfflinePageFlag";

// Descriptions for flags.
constexpr char kNoScriptFlagDescription[] = "No Script Previews";
constexpr char kForceEctFlagDescription[] = "Forced Effective Connection Type";
constexpr char kOfflinePageFlagDescription[] = "Forced Offline Page";

// Links to flags in chrome://flags.
constexpr char kNoScriptFlagLink[] = "chrome://flags/#enable-noscript-previews";
constexpr char kForceEctFlagLink[] =
    "chrome://flags/#force-effective-connection-type";
constexpr char kOfflinePageFlagLink[] =
    "chrome://flags/#enable-offline-previews";

// Flag features names.
const base::CommandLine::StringType kNoScriptFeatureName = "NoScriptPreviews";
const base::CommandLine::StringType kOfflinePageFeatureName = "OfflinePreviews";
...
// GetPreviewsFlagsStatusesCallback.
...
// Mocked call back method to test GetPreviewsFlagsStatusesCallback.
...
TEST_F(InterventionsInternalsPageHandlerTest, NumberOfPreviewsModesStatues) {
...
TEST_F(InterventionsInternalsPageHandlerTest, NumberOfPreviewsFlagsStatuses) {
  page_handler_->GetPreviewsFlagsStatuses(
...
TEST_F(InterventionsInternalsPageHandlerTest, DefaultForceEctFlag) {
  page_handler_->GetPreviewsFlagsStatuses(
      base::BindOnce(&MockGetPreviewsFlagsCallback));
  auto ect_flag = passed_in_flags.find(kForceEctFlag);

  ASSERT_NE(passed_in_flags.end(), ect_flag);
  EXPECT_EQ(kForceEctFlagDescription, ect_flag->second->description);
  EXPECT_EQ("Default", ect_flag->second->value);
  EXPECT_EQ(kForceEctFlagLink, ect_flag->second->link);
}

TEST_F(InterventionsInternalsPageHandlerTest, CorrectForceEctStatus) {
...
TEST_F(InterventionsInternalsPageHandlerTest, DefaultNoScriptFlag) {
  page_handler_->GetPreviewsFlagsStatuses(
      base::BindOnce(&MockGetPreviewsFlagsCallback));
  auto noscript_flag = passed_in_flags.find(kNoScriptFlag);

  ASSERT_NE(passed_in_flags.end(), noscript_flag);
  EXPECT_EQ(kNoScriptFlagDescription, noscript_flag->second->description);
  EXPECT_EQ("Default", noscript_flag->second->value);
  EXPECT_EQ(kNoScriptFlagLink, noscript_flag->second->link);
}

TEST_F(InterventionsInternalsPageHandlerTest, ForceNoScriptFlagEnabled) {
  base::test::ScopedCommandLine scoped_command_line;
  base::CommandLine* command_line = scoped_command_line.GetProcessCommandLine();
  command_line->AppendSwitchNative(switches::kEnableFeatures,
...
TEST_F(InterventionsInternalsPageHandlerTest, ForceNoScriptFlagDisabled) {
  base::test::ScopedCommandLine scoped_command_line;
  base::CommandLine* command_line = scoped_command_line.GetProcessCommandLine();
  command_line->AppendSwitchNative(switches::kDisableFeatures,
                                   kNoScriptFeatureName);
...
TEST_F(InterventionsInternalsPageHandlerTest, DefaultOfflinePageFlag) {
  page_handler_->GetPreviewsFlagsStatuses(
      base::BindOnce(&MockGetPreviewsFlagsCallback));
  auto offline_page_flag = passed_in_flags.find(kOfflinePageFlag);

  ASSERT_NE(passed_in_flags.end(), offline_page_flag);
  EXPECT_EQ(kOfflinePageFlagDescription,
            offline_page_flag->second->description);
  EXPECT_EQ("Default", offline_page_flag->second->value);
  EXPECT_EQ(kOfflinePageFlagLink, offline_page_flag->second->link);
}

TEST_F(InterventionsInternalsPageHandlerTest, ForceNoScOfflinePageEnabled) {
  base::test::ScopedCommandLine scoped_command_line;
  base::CommandLine* command_line = scoped_command_line.GetProcessCommandLine();
  command_line->AppendSwitchNative(switches::kEnableFeatures,
...
TEST_F(InterventionsInternalsPageHandlerTest, ForceNoScOfflinePageDisabled) {
  base::test::ScopedCommandLine scoped_command_line;
  base::CommandLine* command_line = scoped_command_line.GetProcessCommandLine();
  command_line->AppendSwitchNative(switches::kDisableFeatures,
                                   kOfflinePageFeatureName);
