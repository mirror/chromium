...
InterventionsInternalsPageHandler::InterventionsInternalsPageHandler(
    ...
    : binding_(this, std::move(request)),
      logger_(logger),
      current_estimated_etc_(net::EFFECTIVE_CONNECTION_TYPE_UNKNOWN) {
  DCHECK(logger_);
}
...
void InterventionsInternalsPageHandler::SetClientPage(
  ...
  page_ = std::move(page);
  DCHECK(page_);
  OnEffectiveConnectionTypeChanged(current_estimated_etc_);
  logger_->AddAndNotifyObserver(this);
}

void InterventionsInternalsPageHandler::OnEffectiveConnectionTypeChanged(
    net::EffectiveConnectionType type) {
  current_estimated_etc_ = type;
  if (!page_) {
    // if |page_| is not ready yet, don't try to notify the page
    return;
  }
