...
void DiceTurnSyncOnHelper::OnRegisteredForPolicy(const std::string& dm_token,
  ...
  TabDialogs::FromWebContents(web_contents)
      ->ShowProfileSigninConfirmation(browser_, profile_, email_,
...
void DiceTurnSyncOnHelper::LoadPolicyWithCachedCredentials() {
  ...
  policy_service->FetchPolicyForSignedInUser(
      email_, dm_token_, client_id_, profile_->GetRequestContext(),
...
void DiceTurnSyncOnHelper::CreateNewSignedInProfile() {
  ...
  ProfileManager::CreateMultiProfileAsync(
      base::UTF8ToUTF16(email_), profiles::GetDefaultAvatarIconUrl(icon_index),
...
void DiceTurnSyncOnHelper::CompleteInitForNewProfile(
  ...
  switch (status) {
    ...
    case Profile::CREATE_STATUS_INITIALIZED:
      ...
      StartNewSigninInNewProfile(new_profile, email_);
      AbortAndDelete();
      break;
...
DiceTurnSyncOnHelper::GetProfileSyncService() {
  browser_sync::ProfileSyncService* service = nullptr;
...
void DiceTurnSyncOnHelper::SigninAndShowSyncConfirmationUI() {
  signin_manager_->OnExternalSigninCompleted(email_);
  browser_ = EnsureBrowser(browser_, profile_);
...
void DiceTurnSyncOnHelper::GoogleSigninSucceeded(const std::string& account_id,
                                                 const std::string& username) {
