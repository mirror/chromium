...
std::unique_ptr<KeyedService> BuildFakeSyncService(
  ...
  return std::make_unique<browser_sync::TestProfileSyncService>(
      CreateProfileSyncServiceParamsForTest(static_cast<Profile*>(context)));
}
...
// Create a new profile after the cross account dialog and show
...
// Tests that the sync settings page is shown and the user can abort.
