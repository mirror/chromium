...
bool InlineSigninHelper::HandleCrossAccountError(
  ...
  if (!signin::IsCrossAccountError(profile_, email_, gaia_id_))
    return false;
...
void InlineLoginHandlerImpl::FinishCompleteLogin(
  ...
  signin::CanOfferSigninType can_offer_for =
      signin::CAN_OFFER_SIGNIN_FOR_ALL_ACCOUNTS;
  switch (reason) {
    case signin_metrics::Reason::REASON_ADD_SECONDARY_ACCOUNT:
      can_offer_for = signin::CAN_OFFER_SIGNIN_FOR_SECONDARY_ACCOUNT;
      break;
    case signin_metrics::Reason::REASON_REAUTHENTICATION:
    case signin_metrics::Reason::REASON_UNLOCK: {
  ...
  bool can_offer = signin::CanOfferSignin(
      profile, can_offer_for, params.gaia_id, params.email, &error_msg);
  if (!can_offer) {
    if (params.handler) {
