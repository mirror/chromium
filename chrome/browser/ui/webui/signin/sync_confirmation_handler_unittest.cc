...
class TestingSyncConfirmationHandler : public SyncConfirmationHandler {
 public:
  ...
  using SyncConfirmationHandler::HandleRecordConsent;
...
class TestSyncConfirmationUI : public SyncConfirmationUI {
 public:
  static const std::string kConsentTextResourceName1;
  static const std::string kConsentTextResourceName2;
  static const std::string kConsentTextResourceName3;
  static const std::string kConsentTextResourceName4;
  static const std::string kConsentTextResourceName5;

  TestSyncConfirmationUI(content::WebUI* web_ui) : SyncConfirmationUI(web_ui) {
    string_resource_name_to_grd_id_map_[kConsentTextResourceName1] = 1;
    string_resource_name_to_grd_id_map_[kConsentTextResourceName2] = 2;
    string_resource_name_to_grd_id_map_[kConsentTextResourceName3] = 3;
    string_resource_name_to_grd_id_map_[kConsentTextResourceName4] = 4;
    string_resource_name_to_grd_id_map_[kConsentTextResourceName5] = 5;
  }

  const std::map<std::string, int>& GetResourceNameToGrdIdMap() override {
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(TestSyncConfirmationUI);
};

const std::string TestSyncConfirmationUI::kConsentTextResourceName1 = "text1";
const std::string TestSyncConfirmationUI::kConsentTextResourceName2 = "text2";
const std::string TestSyncConfirmationUI::kConsentTextResourceName3 = "text3";
const std::string TestSyncConfirmationUI::kConsentTextResourceName4 = "text4";
const std::string TestSyncConfirmationUI::kConsentTextResourceName5 = "text5";
...
class MockConsentAuditor : public consent_auditor::ConsentAuditor {
 public:
  static std::unique_ptr<KeyedService> Build(content::BrowserContext* context) {
    return std::make_unique<MockConsentAuditor>(
        Profile::FromBrowserContext(context));
  }

  MockConsentAuditor(Profile* profile)
      : ConsentAuditor(
            profile->GetPrefs(),
            browser_sync::UserEventServiceFactory::GetForProfile(profile),
            std::string(),
            std::string()) {}
  ~MockConsentAuditor() override {}
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(MockConsentAuditor);
};
...
class SyncConfirmationHandlerTest : public BrowserWithTestWindowTest {
 public:
  ...
  void SetUp() override {
    ...
    web_ui()->SetController(new TestSyncConfirmationUI(web_ui()));
  ...
  MockConsentAuditor* consent_auditor() {
    return static_cast<MockConsentAuditor*>(
  ...
  TestingProfile::TestingFactories GetTestingFactories() override {
    return {{AccountFetcherServiceFactory::GetInstance(),
            ...
            {ConsentAuditorFactory::GetInstance(), MockConsentAuditor::Build}};
  }
...
TEST_F(SyncConfirmationHandlerTest, TestRecordConsentImmediate) {
  ...
  consent_description.GetList().push_back(
      base::Value(TestSyncConfirmationUI::kConsentTextResourceName1));
  consent_description.GetList().push_back(
      base::Value(TestSyncConfirmationUI::kConsentTextResourceName2));
  consent_description.GetList().push_back(
      base::Value(TestSyncConfirmationUI::kConsentTextResourceName4));

  // The consent confirmation contains string 5.
  base::Value consent_confirmation(
      TestSyncConfirmationUI::kConsentTextResourceName5);

  // These are passed as parameters to HandleSyncConsent().
  base::ListValue args;
  args.GetList().push_back(std::move(consent_description));
  args.GetList().push_back(std::move(consent_confirmation));

  // The user signs in, and then consents to Sync.
  signin_manager()->SignIn(kGaiaID, kUsername, kPassword);
  base::RunLoop().RunUntilIdle();
  handler()->HandleRecordConsent(&args);
...
TEST_F(SyncConfirmationHandlerTest, TestRecordConsentDelayed) {
  // The consent description consists of strings 1, 2, and 4.
  base::ListValue consent_description;
  consent_description.GetList().push_back(
      base::Value(TestSyncConfirmationUI::kConsentTextResourceName1));
  ...
  // And when the signin process finishes...
  signin_manager()->SignIn(kGaiaID, kUsername, kPassword);
  base::RunLoop().RunUntilIdle();
