...
resource_coordinator::TabManager::DiscardTabCondition GetDiscardTabCondition(
    bool urgent) {
  return urgent ? resource_coordinator::TabManager::kUrgentShutdown
  ...
  void GetTabDiscardsInfo(GetTabDiscardsInfoCallback callback) override {
    ...
    for (const auto& tab : stats) {
      ...
      if (!tab.favicon_url.empty()) {
        ...
        info->favicon_url = "chrome://discards/default_favicon.png";
      }
...
DiscardsUI::DiscardsUI(content::WebUI* web_ui)
  ...
  source->AddResourcePath("default_favicon.png", IDR_DEFAULT_FAVICON);
...
void DiscardsUI::BindUIHandler(mojom::DiscardsDetailsProviderRequest request) {
  Profile* profile = Profile::FromWebUI(web_ui());
  ui_handler_.reset(
      new DiscardsDetailsProviderImpl(profile, std::move(request)));
}
