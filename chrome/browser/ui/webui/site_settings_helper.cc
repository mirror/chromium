...
static_assert(arraysize(kSiteSettingSourceStringMapping) ==
                  static_cast<int>(SiteSettingSource::NUM_SOURCES),
              "kSiteSettingSourceStringMapping should have "
              "SiteSettingSource::NUM_SOURCES elements");
...
std::string CalculateSiteSettingSourceString(
  ...
  SiteSettingSource site_setting_source = SiteSettingSource::NUM_SOURCES;
  if (permission_status_source == PermissionStatusSource::KILL_SWITCH)
    site_setting_source = SiteSettingSource::PREFERENCE;  // Source #1.

  if (info.source == content_settings::SETTING_SOURCE_POLICY ||
      info.source == content_settings::SETTING_SOURCE_SUPERVISED) {
    site_setting_source = SiteSettingSource::POLICY;  // Source #2.
  }

  if (info.source == content_settings::SETTING_SOURCE_EXTENSION)
    site_setting_source = SiteSettingSource::EXTENSION;  // Source #3.
  ...
  if (info.source == content_settings::SETTING_SOURCE_USER) {
    if (permission_status_source ==
      ...
      site_setting_source = SiteSettingSource::PREFERENCE;  // Source #5.
    } else if (info.primary_pattern == ContentSettingsPattern::Wildcard() &&
               info.secondary_pattern == ContentSettingsPattern::Wildcard()) {
      site_setting_source = SiteSettingSource::DEFAULT;  // Source #7, #8.
    } else {
      ...
      site_setting_source = SiteSettingSource::PREFERENCE;
    }
  }

  DCHECK_NE(SiteSettingSource::NUM_SOURCES, site_setting_source);
...
void GetExceptionsFromHostContentSettingsMap(
  ...
  for (ContentSettingsForOneType::iterator i = entries.begin();
    ...
    if (i->primary_pattern == ContentSettingsPattern::Wildcard() &&
        ...
        i->source != SiteSettingSourceToString(SiteSettingSource::PREFERENCE)) {
      continue;
    }
  ...
  if (type == CONTENT_SETTINGS_TYPE_MEDIASTREAM_MIC ||
    ...
    auto& policy_exceptions = all_provider_exceptions
        [HostContentSettingsMap::GetProviderTypeFromSource(
            SiteSettingSourceToString(SiteSettingSource::POLICY))];
...
void GetContentCategorySetting(
  ...
  if (provider != SiteSettingSourceToString(SiteSettingSource::DEFAULT))
...
void GetPolicyAllowedUrls(
  ...
  for (const ContentSettingsPattern& pattern : patterns) {
    ...
    exceptions->push_back(GetExceptionForPage(
        ...
        SiteSettingSourceToString(SiteSettingSource::POLICY), incognito));
  }
}
