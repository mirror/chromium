...
class SysInternalsMessageHandler : public content::WebUIMessageHandler {
 public:
  ...
  // Due to the |base::Value| api only supports 32 bits signed integer,
 ...
 private:
  ...
  // }
  // Due to the |base::Value| api only supports 32 bits signed integer,
  // values that may greater than 2^32 will be passed as a double. If the value
  // is a counter, we only use the last 31 bits and will pass it as a 32 bits
  // signed integer.
