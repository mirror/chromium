...
bool SysInternalsMessageHandler::GetCpuInfo(std::vector<base::CpuInfo>* infos) {
  ...
  if (host_processor_info(host.get(), PROCESSOR_CPU_LOAD_INFO,
    ...
    for (natural_t i = 0; i < num_of_processors; ++i) {
      uint64_t user = static_cast<uint64_t>(
          cpu_infos[i].cpu_ticks[CPU_STATE_USER] & counter_max);
      uint64_t sys = static_cast<uint64_t>(
          cpu_infos[i].cpu_ticks[CPU_STATE_SYSTEM] & counter_max);
      uint64_t nice = static_cast<uint64_t>(
          cpu_infos[i].cpu_ticks[CPU_STATE_NICE] & counter_max);
      uint64_t idle = static_cast<uint64_t>(
          cpu_infos[i].cpu_ticks[CPU_STATE_IDLE] & counter_max);
