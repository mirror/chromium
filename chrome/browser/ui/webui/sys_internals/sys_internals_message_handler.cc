...
void SysInternalsMessageHandler::GetSysInfo(const base::ListValue* args) {
  ...
  sequenced_task_runner->PostTask(
      FROM_HERE, base::BindOnce(
          &SysInternalsMessageHandler::UpdateInfoStructures,
          base::Unretained(this)));
  ...
  sequenced_task_runner->PostTask(
      FROM_HERE, base::BindOnce(
          &SysInternalsMessageHandler::SetValueAndReplySysInfo,
          base::Unretained(this), std::move(time_tick), is_console_log));
}

void SysInternalsMessageHandler::SetValueAndReplySysInfo(
    std::unique_ptr<base::Value> time_tick, bool is_console_log) {
