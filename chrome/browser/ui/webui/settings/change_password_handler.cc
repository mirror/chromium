...
void ChangePasswordHandler::HandleChangePasswordPageShown(
  ...
  if (service_) {
    service_->OnWarningShown(
        nullptr, safe_browsing::PasswordProtectionService::CHROME_SETTINGS);
  }
}
...
void ChangePasswordHandler::OpenAccountPage() {
  GURL account_google_url(GaiaUrls::GetInstance()->gaia_url());
  account_google_url = google_util::AppendGoogleLocaleParam(
      account_google_url, g_browser_process->GetApplicationLocale());
  // Open url in a new foreground tab.
  content::OpenURLParams params(account_google_url, content::Referrer(),
