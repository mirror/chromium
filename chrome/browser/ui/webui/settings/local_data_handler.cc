...
#include "chrome/browser/ui/webui/settings/local_data_handler.h"
...
LocalDataHandler::LocalDataHandler(Profile* profile)
    : profile_(profile), observer_(this) {}

LocalDataHandler::~LocalDataHandler() {}

void LocalDataHandler::RegisterMessages() {
  web_ui()->RegisterMessageCallback(
      "fetchUsageTotal", base::Bind(&LocalDataHandler::HandleFetchUsageTotal,
                                    base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "clearUsage",
      base::Bind(&LocalDataHandler::HandleClearUsage, base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "fetchUsbDevices", base::Bind(&LocalDataHandler::HandleFetchUsbDevices,
                                    base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "removeUsbDevice", base::Bind(&LocalDataHandler::HandleRemoveUsbDevice,
                                    base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "setDefaultValueForContentType",
      base::Bind(&LocalDataHandler::HandleSetDefaultValueForContentType,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "getDefaultValueForContentType",
      base::Bind(&LocalDataHandler::HandleGetDefaultValueForContentType,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "getExceptionList", base::Bind(&LocalDataHandler::HandleGetExceptionList,
                                     base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "getOriginPermissions",
      base::Bind(&LocalDataHandler::HandleGetOriginPermissions,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "setOriginPermissions",
      base::Bind(&LocalDataHandler::HandleSetOriginPermissions,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "resetCategoryPermissionForPattern",
      base::Bind(&LocalDataHandler::HandleResetCategoryPermissionForPattern,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "setCategoryPermissionForPattern",
      base::Bind(&LocalDataHandler::HandleSetCategoryPermissionForPattern,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "isOriginValid", base::Bind(&LocalDataHandler::HandleIsOriginValid,
                                  base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "isPatternValid", base::Bind(&LocalDataHandler::HandleIsPatternValid,
                                   base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "updateIncognitoStatus",
      base::Bind(&LocalDataHandler::HandleUpdateIncognitoStatus,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "fetchZoomLevels", base::Bind(&LocalDataHandler::HandleFetchZoomLevels,
                                    base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "removeZoomLevel", base::Bind(&LocalDataHandler::HandleRemoveZoomLevel,
                                    base::Unretained(this)));
}

void LocalDataHandler::OnJavascriptAllowed() {
  ...
  host_zoom_map_subscription_ =
      content::HostZoomMap::GetDefaultForBrowserContext(profile_)
          ->AddZoomLevelChangedCallback(base::Bind(
              &LocalDataHandler::OnZoomLevelChanged, base::Unretained(this)));
}

void LocalDataHandler::OnJavascriptDisallowed() {
  observer_.RemoveAll();
  notification_registrar_.RemoveAll();
  host_zoom_map_subscription_.reset();
}

void LocalDataHandler::OnGetUsageInfo(
...
void LocalDataHandler::OnUsageInfoCleared(storage::QuotaStatusCode code) {
...
void LocalDataHandler::OnContentSettingChanged(
...
void LocalDataHandler::Observe(int type,
  ...
  switch (type) {
    case chrome::NOTIFICATION_PROFILE_DESTROYED: {
      ...
      if (profile->IsOffTheRecord() && observer_.IsObserving(settings_map)) {
...
void LocalDataHandler::OnZoomLevelChanged(
    const content::HostZoomMap::ZoomLevelChange& change) {
  SendZoomLevels();
}

void LocalDataHandler::HandleFetchUsageTotal(const base::ListValue* args) {
  AllowJavascript();
  ...
  storage_info_fetcher->FetchStorageInfo(
      base::Bind(&LocalDataHandler::OnGetUsageInfo, base::Unretained(this)));
}

void LocalDataHandler::HandleClearUsage(const base::ListValue* args) {
  ...
  if (url.is_valid()) {
    ...
    storage_info_fetcher->ClearStorage(
        ...
        base::Bind(&LocalDataHandler::OnUsageInfoCleared,
                   base::Unretained(this)));
...
void LocalDataHandler::HandleFetchUsbDevices(const base::ListValue* args) {
  AllowJavascript();
  ...
  site_settings::GetChooserExceptionsFromProfile(profile_, false, *chooser_type,
                                                 &exceptions);
  ResolveJavascriptCallback(*callback_id, exceptions);
}

void LocalDataHandler::HandleRemoveUsbDevice(const base::ListValue* args) {
...
void LocalDataHandler::HandleSetDefaultValueForContentType(
...
void LocalDataHandler::HandleGetDefaultValueForContentType(
    const base::ListValue* args) {
  AllowJavascript();
...
void LocalDataHandler::HandleGetExceptionList(const base::ListValue* args) {
  AllowJavascript();
...
void LocalDataHandler::HandleGetOriginPermissions(const base::ListValue* args) {
  AllowJavascript();
...
void LocalDataHandler::HandleSetOriginPermissions(const base::ListValue* args) {
  CHECK_EQ(3U, args->GetSize());
  std::string origin_string;
  CHECK(args->GetString(0, &origin_string));
  const base::ListValue* types;
...
void LocalDataHandler::HandleResetCategoryPermissionForPattern(
    const base::ListValue* args) {
...
void LocalDataHandler::HandleSetCategoryPermissionForPattern(
    const base::ListValue* args) {
...
void LocalDataHandler::HandleIsOriginValid(const base::ListValue* args) {
  AllowJavascript();
  CHECK_EQ(2U, args->GetSize());
  const base::Value* callback_id;
  CHECK(args->Get(0, &callback_id));
...
void LocalDataHandler::HandleIsPatternValid(const base::ListValue* args) {
  CHECK_EQ(2U, args->GetSize());
  const base::Value* callback_id;
  CHECK(args->Get(0, &callback_id));
...
void LocalDataHandler::HandleUpdateIncognitoStatus(
    const base::ListValue* args) {
  AllowJavascript();
  SendIncognitoStatus(profile_, /*was_destroyed=*/false);
}

void LocalDataHandler::SendIncognitoStatus(Profile* profile,
                                           bool was_destroyed) {
...
void LocalDataHandler::HandleFetchZoomLevels(const base::ListValue* args) {
  AllowJavascript();
  SendZoomLevels();
}

void LocalDataHandler::SendZoomLevels() {
  if (!IsJavascriptAllowed())
    return;
...
void LocalDataHandler::HandleRemoveZoomLevel(const base::ListValue* args) {
  CHECK_EQ(1U, args->GetSize());
