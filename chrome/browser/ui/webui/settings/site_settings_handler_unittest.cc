...
class SiteSettingsHandlerInfobarTest : public BrowserWithTestWindowTest {
 public:
  ...
  void TearDown() override {
    // SiteSettingsHandler maintains a HostZoomMap::Subscription internally
    // which gets updated for each new tab. The subscription needs to be
    // destroyed before the HostZoomMapImpl's CallbackList gets destroyed, so
    // destroy the browser on which the last tab was opened before |handler_|
    // gets destroyed here.
