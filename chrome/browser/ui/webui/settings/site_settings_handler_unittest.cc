...
TEST_F(SiteSettingsHandlerInfobarTest, SettingPermissionsTriggersInfobar) {
  ...
  const GURL origin_anchor(origin_string);
  ...
  GURL tab_url;
  for (int i = 0; i < browser()->tab_strip_model()->count(); ++i) {
    if (i == 1 /* |origin_anchor| */ || i == 3 /* |origin| */) {
      EXPECT_EQ(
  ...
  // infobars showing.
  for (int i = 0; i < browser()->tab_strip_model()->count(); ++i) {
    if (i == 0 /* |origin_path| */ || i == 1 /* |origin_anchor| */ ||
        i == 3 /* |origin| */) {
      EXPECT_EQ(
          1u, GetInfobarServiceForTab(browser(), i, &tab_url)->infobar_count());
      EXPECT_TRUE(url::IsSameOriginWith(origin, tab_url));
  ...
  EXPECT_EQ(infobars::InfoBarDelegate::PAGE_INFO_INFOBAR_DELEGATE,
            GetInfobarServiceForTab(browser(), 0 /* |origin_path| */, &tab_url)
                ->infobar_at(0)
                ->delegate()
                ->GetIdentifier());
  EXPECT_TRUE(url::IsSameOriginWith(origin, tab_url));
