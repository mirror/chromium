...
void ClearBrowsingDataHandler::HandleClearBrowsingData(
  ...
  if (data_types.count(BrowsingDataType::COOKIES)) {
    choice = data_types.count(BrowsingDataType::CACHE)
                 ? content::BrowsingDataRemover::BOTH_COOKIES_AND_CACHE
                 : content::BrowsingDataRemover::ONLY_COOKIES;
  } else if (data_types.count(BrowsingDataType::CACHE)) {
  ...
  if (data_types.count(BrowsingDataType::PASSWORDS)) {
...
void ClearBrowsingDataHandler::OnClearingTaskFinished(
  ...
  const bool show_notice =
      ...
      data_types.count(BrowsingDataType::HISTORY);
