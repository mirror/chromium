...
void CookiesViewHandler::RegisterMessages() {
  ...
  web_ui()->RegisterMessageCallback(
      "localData.filter",
      base::Bind(&CookiesViewHandler::HandleFilter, base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
...
void CookiesViewHandler::TreeModelEndBatch(CookiesTreeModel* model) {
  ...
  batch_update_ = false;
  CHECK(cookies_tree_model_.get() == model);
...
void CookiesViewHandler::HandleFilter(const base::ListValue* args) {
  CHECK(request_.callback_id_.empty());
  CHECK_EQ(2U, args->GetSize());
  CHECK(args->GetString(0, &request_.callback_id_));
  ...
  AllowJavascript();
  request_.filter = filter;
  sorted_sites_.clear();
  cookies_tree_model_->UpdateSearchResults(request_.filter);
}
...
void CookiesViewHandler::HandleGetDisplayList(const base::ListValue* args) {
  ...
  AllowJavascript();
  request_.start_index = begin;
  request_.index_count = count;
...
void CookiesViewHandler::SendLocalDataList(const CookieTreeNode* parent) {
  ...
  // base::MakeUnique<base::Value>(request_.filter));
