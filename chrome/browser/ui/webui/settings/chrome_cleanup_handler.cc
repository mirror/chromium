...
void ChromeCleanupHandler::RegisterMessages() {
  ...
  web_ui()->RegisterMessageCallback(
      ...
      base::Bind(&ChromeCleanupHandler::HandleNotifyShowDetails,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "chromeCleanupLearnMoreClicked",
      base::Bind(&ChromeCleanupHandler::HandleChromeCleanupLearnMoreClicked,
                 base::Unretained(this)));
}
...
void ChromeCleanupHandler::HandleChromeCleanupLearnMoreClicked(
    const base::ListValue* args) {
  DCHECK_EQ(0U, args->GetSize());
