...
void ChromeCleanupHandler::RegisterMessages() {
  ...
  web_ui()->RegisterMessageCallback(
      "startCleanup", base::Bind(&ChromeCleanupHandler::HandleStartCleanup,
                                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "showDetails", base::Bind(&ChromeCleanupHandler::HandleShowDetails,
                                base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "chromeCleanupLearnMore",
      base::Bind(&ChromeCleanupHandler::HandleChromeCleanupLearnMore,
                 base::Unretained(this)));
}
  ...
  switch (dismiss_source) {
    case kCleanupSuccessDoneButton:
      base::RecordAction(base::UserMetricsAction(
          "SoftwareReporter.CleanupCard_CleanupSuccessDone"));
      break;
    case kCleanupFailureDoneButton:
      base::RecordAction(base::UserMetricsAction(
          "SoftwareReporter.CleanupCard_CleanupFailureDone"));
      break;
    case kOther:
      break;
    default:
...
void ChromeCleanupHandler::HandleRegisterChromeCleanerObserver(
  ...
  base::RecordAction(
      base::UserMetricsAction("SoftwareReporter.CleanupCard_Shown"));
  AllowJavascript();
...
void ChromeCleanupHandler::HandleRestartComputer(const base::ListValue* args) {
  ...
  base::RecordAction(
      base::UserMetricsAction("SoftwareReporter.CleanupCard_RestartComputer"));
...
void ChromeCleanupHandler::HandleSetLogsUploadPermission(
  ...
  if (allow_logs_upload) {
    base::RecordAction(base::UserMetricsAction(
        "SoftwareReporter.CleanupCard_LogsUploadEnabled"));
  } else {
    base::RecordAction(base::UserMetricsAction(
        "SoftwareReporter.CleanupCard_LogsUploadDisabled"));
  }
...
void ChromeCleanupHandler::HandleStartCleanup(const base::ListValue* args) {
  ...
  base::RecordAction(
      base::UserMetricsAction("SoftwareReporter.CleanupCard_StartCleanup"));
...
void ChromeCleanupHandler::HandleShowDetails(const base::ListValue* args) {
  CHECK_EQ(1U, args->GetSize());
  ...
  if (details_section_visible) {
    base::RecordAction(
        base::UserMetricsAction("SoftwareReporter.CleanupCard_ShowDetails"));
  } else {
    base::RecordAction(
        base::UserMetricsAction("SoftwareReporter.CleanupCard_HideDetails"));
  }
}

void ChromeCleanupHandler::HandleChromeCleanupLearnMore(
    const base::ListValue* args) {
  DCHECK_EQ(0U, args->GetSize());

  base::RecordAction(
      base::UserMetricsAction("SoftwareReporter.CleanupCard_LearnMore"));
}
