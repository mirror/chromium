...
const char kSendArcVpnProviders[] = "send-arc-vpn-providers";
const char kRemoveArcVpnProvider[] = "remove-arc-vpn-provider";
const char kUpdateArcVpnProvider[] = "update-arc-vpn-provider";
...
void InternetHandler::OnArcVpnProviderRemoved(const std::string& package_name) {
  if (IsJavascriptAllowed())
    FireWebUIListener(kRemoveArcVpnProvider, base::Value(package_name));
}
...
void InternetHandler::OnArcVpnProvidersRefreshed(
    const std::vector<
        ...
        arc_vpn_providers) {
  SendArcVpnProviders(arc_vpn_providers);
}

void InternetHandler::OnArcVpnProviderUpdated(
    app_list::ArcVpnProviderManager::ArcVpnProvider* arc_vpn_provider) {
  if (IsJavascriptAllowed())
    FireWebUIListener(kUpdateArcVpnProvider,
                      ArcVpnProviderToValue(arc_vpn_provider)->Clone());
}
...
void InternetHandler::RequestArcVpnProviders(const base::ListValue* args) {
  ...
  if (!arc_vpn_provider_manager_)
    return;
  auto arc_vpn_providers = arc_vpn_provider_manager_->GetArcVpnProviders();
  SendArcVpnProviders(arc_vpn_providers);
}
...
void InternetHandler::SendArcVpnProviders(
  ...
  for (const auto& arc_vpn_provider : arc_vpn_providers) {
    arc_vpn_providers_value.GetList().push_back(
        ArcVpnProviderToValue(arc_vpn_provider.get())->Clone());
  }
