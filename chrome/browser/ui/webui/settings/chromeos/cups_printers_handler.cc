...
void CheckAutoconf(const std::string& printer_uri,
                   const PrinterInfoCallback& callback) {
  // Parse url to separate address and port.
  const char* uri_ptr = printer_uri.c_str();
  url::Parsed parsed;
  url::ParseStandardURL(uri_ptr, printer_uri.length(), &parsed);
  base::StringPiece printer_protocol(&uri_ptr[parsed.scheme.begin],
                                     parsed.scheme.len);
  ...
  QueryIppPrinter(host.as_string(), port, path.as_string(), encrypted,
                  callback);
}
...
void CupsPrintersHandler::HandleGetPrinterInfo(const base::ListValue* args) {
  ...
  CheckAutoconf(printer_uri,
                base::Bind(&CupsPrintersHandler::OnPrinterInfo,
                           weak_factory_.GetWeakPtr(), callback_id));
}

void CupsPrintersHandler::OnPrinterInfoDiscovered(
...
void CupsPrintersHandler::HandleAddDiscoveredPrinter(
  ...
  if (!printer->ppd_reference().effective_make_and_model.empty() ||
  ...
  CheckAutoconf(printer_uri,
                base::Bind(&CupsPrintersHandler::OnPrinterInfoDiscovered,
