...
// located at |printer_uri|.  Results are reported through |callback|.
void QueryAutoconf(const std::string& printer_uri,
  ...
  if (!parsed.scheme.is_valid() || !parsed.host.is_valid() ||
    ...
    LOG(WARNING) << "Printer uri is invalid";
  ...
  DCHECK(scheme == kIppScheme || scheme == kIppScheme)
      << "Autoconf check skipped for non-IPP printers";
...
void CupsPrintersHandler::HandleAddDiscoveredPrinter(
  ...
  if (base::StartsWith(printer_uri, kIppScheme,
                       base::CompareCase::INSENSITIVE_ASCII) ||
      base::StartsWith(printer_uri, kIppsScheme,
                       base::CompareCase::INSENSITIVE_ASCII)) {
    QueryAutoconf(
        printer_uri,
