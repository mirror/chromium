...
void AppearanceHandler::IsWallpaperSettingVisible(const base::ListValue* args) {
  ...
  ResolveJavascriptCallback(
      *callback_id,
      base::Value(ash::wallpaper_utils::ShouldShowWallpaperSetting()));
}

void AppearanceHandler::IsWallpaperPolicyControlled(
    const base::ListValue* args) {
  CHECK_EQ(args->GetSize(), 1U);
  WallpaperControllerClient::Get()->IsActiveUserPolicyControlled(
      base::Bind(&AppearanceHandler::IsWallpaperPolicyControlledImpl,
                 base::Unretained(this), args->GetList()[0].Clone()));
}

void AppearanceHandler::IsWallpaperPolicyControlledImpl(
    const base::Value& callback_id,
    bool controlled) {
  AllowJavascript();
  ResolveJavascriptCallback(callback_id, base::Value(controlled));
}
