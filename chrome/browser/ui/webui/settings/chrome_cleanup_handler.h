...
class ChromeCleanupHandler
 ...
 private:
  ...
  // the details section of the webui page is visible.
  void HandleShowDetails(const base::ListValue* args);

  // Callback for the "chromeCleanupLearnMore" message that notifies Chrome that
  // the "learn more" link was clicked.
  void HandleChromeCleanupLearnMore(const base::ListValue* args);
