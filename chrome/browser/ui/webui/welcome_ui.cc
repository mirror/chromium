...
// it in the "Welcome to Chrome" variant, and only on branded builds.
  ...
  if (base::FeatureList::IsEnabled(signin::kAccountConsistencyFeature)) {
    html_source->AddResourcePath("new_welcome.js", IDR_NEW_WELCOME_JS);
    html_source->AddResourcePath("new_welcome.css", IDR_NEW_WELCOME_CSS);
    html_source->SetDefaultResource(IDR_NEW_WELCOME_HTML);
  } else {
