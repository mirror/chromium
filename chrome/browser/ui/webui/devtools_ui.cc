...
class DevToolsDataSource : public content::URLDataSource {
 ...
 private:
  ...
  using SourceAndBodyCallback =
      base::OnceCallback<void(content::SimpleURLLoader* source, std::unique_ptr<std::string> response_body)>;
  void OnSimpleLoaderComplete(content::SimpleURLLoader* source, std::unique_ptr<std::string> response_body);
  ...
  using PendingRequestsMap = std::map<content::SimpleURLLoader*, GotDataCallback>;
...
void DevToolsDataSource::StartRemoteDataRequest(
  ...
  std::unique_ptr<content::SimpleURLLoader> simple_loader = content::SimpleURLLoader::Create(std::move(resource_request),
      traffic_annotation);
  pending_[simple_loader.get()] = callback;
  SourceAndBodyCallback loader_callback = base::BindOnce(
      &DevToolsDataSource::OnSimpleLoaderComplete, base::Unretained(this), base::Unretained(simple_loader.get()));
  content::mojom::URLLoaderFactory* loader_factory =
      content::BrowserContext::GetDefaultStoragePartition(browser_context_)
      ->GetURLLoaderFactoryForBrowserProcess();
  simple_loader->DownloadToStringOfUnboundedSizeUntilCrashAndDie(
      loader_factory, std::move(loader_callback));
}
...
void DevToolsDataSource::OnSimpleLoaderComplete(content::SimpleURLLoader* source, std::unique_ptr<std::string> response_body) {
  DCHECK(source);
  PendingRequestsMap::iterator it = pending_.find(source);
  DCHECK(it != pending_.end());
  delete source;
  it->second.Run(std::move(response_body)));
  pending_.erase(it);
}
...
DevToolsUI::DevToolsUI(content::WebUI* web_ui)
  ...
  content::URLDataSource::Add(
      profile,
      new DevToolsDataSource(profile));
}
