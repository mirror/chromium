...
void ShowCertificateViewer(WebContents* web_contents,
  ...
  CertificateViewerDialog* dialog = new CertificateViewerDialog(
      net::x509_util::CreateCERTCertificateVectorFromX509Certificate(cert));
...
CertificateViewerModalDialog::CertificateViewerModalDialog(
  ...
  title_ = l10n_util::GetStringFUTF16(
      ...
      // XXX better error string
      base::UTF8ToUTF16(nss_certs_.empty() ? "error"
                                           : x509_certificate_model::GetTitle(
                                                 nss_certs_.front().get())));
}
...
void CertificateViewerModalDialog::GetWebUIMessageHandlers(
  ...
  handlers->push_back(new CertificateViewerDialogHandler(
      ...
      net::x509_util::DupCERTCertificateVector(nss_certs_)));
}
...
std::string CertificateViewerModalDialog::GetDialogArgs() const {
  ...
  int index = 0;
  for (net::ScopedCERTCertificateVector::const_iterator i = nss_certs_.begin();
