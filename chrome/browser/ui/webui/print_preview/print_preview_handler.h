...
class PrintPreviewHandler
 ...
 private:
  ...
  // Starts getting all extension managed or local privet printers, based on
  // the value of |printer_type|. |args| contains the Javascript callback.
  void HandleGetExtensionPrivetPrinters(printing::PrinterType printer_type,
  ...
  // Type is indicated by |printer_type|.
  // |arg| contains the ID of the printer whose capabilities are requested.
  void HandleGetExtensionPrivetPrinterCapabilities(
  ...
  const std::unique_ptr<PrinterHandler>& GetPrinterHandler(
      printing::PrinterType printer_type);

  // Called when a privet printer or extension printers are detected.
  // |callback_id|: The javascript callback to call if all privet printers
  //     are loaded (when |done| = true)
  // |printer_type|: The type of printers that were added.
  // |printers|: A list containing a privet printer that has been found.
  // |done|: Whether the privet printer search has timed out.
  ...
  // print capabilites for a printer.
