...
const unsigned char kTestData[] = "abc";
...
base::Value GetPrintTicket(printing::PrinterType type, bool cloud) {
  ...
  if (!cloud && (type == printing::kPrivetPrinter ||
                 type == printing::kExtensionPrinter)) {
...
class FakePrintPreviewUI : public PrintPreviewUI {
 public:
  ...
  void GetPrintPreviewDataForIndex(
    ...
    *data = base::MakeRefCounted<base::RefCountedBytes>(kTestData,
                                                        sizeof(kTestData) - 1);
  }
...
TEST_F(PrintPreviewHandlerTest, Print) {
  ...
  for (size_t i = 0; i <= arraysize(types); i++) {
    ...
    if (cloud) {
      ...
      base::Base64Encode(reinterpret_cast<const char*>(printing::kTestData),
                         &expected_data);
