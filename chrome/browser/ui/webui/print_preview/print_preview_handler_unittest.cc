...
PrinterInfo GetSimplePrinterInfo(std::string name, bool is_default) {
...
class TestPrinterHandler : public PrinterHandler {
 public:
  ...
  static std::unique_ptr<PrinterHandler> Create(
  ...
  void SetPrinters(const std::vector<PrinterInfo>& printers) {
    ...
    for (auto it = printers.begin(); it != printers.end(); ++it) {
      if (it->is_default)
        default_printer_ = it->id;
      printer_list.push_back(it->basic_info.Clone());
      printer_capabilities_[it->id] = base::DictionaryValue::From(
          std::make_unique<base::Value>(it->capabilities.Clone()));
    }
...
class FakePrintPreviewUI : public PrintPreviewUI {
 public:
  ...
  int GetAvailableDraftPageCount() { return 1; }

  int32_t GetIDForPrintPreviewUI() { return 0; }
...
class TestPrintPreviewHandler : public PrintPreviewHandler {
 public:
  ...
  using PrintPreviewHandler::set_web_ui;

  PrinterHandler* GetPrinterHandler(PrinterType printer_type) override {
    called_for_type_[printer_type] = true;
  ...
  bool called_only_for_type(PrinterType printer_type) {
    bool called = false;
    for (auto it = called_for_type_.begin(); it != called_for_type_.end();
         ++it) {
      if ((it->first == printer_type) != it->second)
        return false;
      if (it->first == printer_type && it->second)
        called = true;
    }
  ...
  void reset_calls() {
    for (auto it = called_for_type_.begin(); it != called_for_type_.end();
         ++it) {
      called_for_type_[it->first] = false;
    }
  }

 private:
  std::map<PrinterType, bool> called_for_type_;
  std::unique_ptr<PrinterHandler> test_printer_handler_;
};

const char kDefaultPrinterName[] = "DefaultPrinter";
const char kInitiatorName[] = "TestInitiator";
...
class PrintPreviewHandlerTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    printers_.push_back(
        printing::GetSimplePrinterInfo(printing::kDefaultPrinterName, true));
    std::unique_ptr<PrinterHandler> printer_handler =
        printing::TestPrinterHandler::Create(printers_);
    printer_handler_ =
        static_cast<printing::TestPrinterHandler*>(printer_handler.get());

    std::unique_ptr<PrintPreviewHandler> preview_handler;
    preview_handler.reset(
        new printing::TestPrintPreviewHandler(std::move(printer_handler)));
    preview_handler->set_web_ui(web_ui());
    handler_ = preview_handler.get();

    preview_ui_.reset(
        new printing::FakePrintPreviewUI(web_ui(), std::move(preview_handler)));
    preview_ui_->SetInitiatorTitle(UTF8ToUTF16(printing::kInitiatorName));
    web_ui()->SetController(preview_ui_.get());
  }
  ...
  // Validates the initial settings structure for print preview. Assumes
  ...
  void ValidateInitialSettings(const content::TestWebUI::CallData& data,
    ...
    ASSERT_TRUE(settings->FindPathOfType({"printAutomaticallyInKioskMode"},
                                         base::Value::Type::BOOLEAN));
    ASSERT_TRUE(
        settings->FindPathOfType({"appKioskMode"}, base::Value::Type::BOOLEAN));
    ASSERT_TRUE(settings->FindPathOfType({"measurementSystem"},
                                         base::Value::Type::INTEGER));
    ASSERT_TRUE(settings->FindPathOfType({"previewModifiable"},
                                         base::Value::Type::BOOLEAN));
    ASSERT_TRUE(settings->FindPathOfType({"initiatorTitle"},
                                         base::Value::Type::STRING));
    EXPECT_EQ(initiator_title,
              settings->FindPath({"initiatorTitle"})->GetString());
    ...
    EXPECT_EQ(default_printer_name,
              settings->FindPath({"printerName"})->GetString());
  }

  Profile* profile() { return profile_.get(); }
  content::TestWebUI* web_ui() { return web_ui_.get(); }
  PrintPreviewHandler* handler() { return handler_; }
 ...
 private:
  ...
  std::unique_ptr<printing::FakePrintPreviewUI> preview_ui_;
};
...
TEST_F(PrintPreviewHandlerTest, InitialSettings) {
  ...
  ValidateInitialSettings(*web_ui()->call_data().back(),
                          printing::kDefaultPrinterName,
                          printing::kInitiatorName);

  // Check that the use-cloud-print event got sent
  const content::TestWebUI::CallData& data2 = *web_ui()->call_data().front();
  EXPECT_EQ("cr.webUIListenerCallback", data2.function_name());
  std::string event_id;
  ASSERT_TRUE(data2.arg1()->GetAsString(&event_id));
  EXPECT_EQ("use-cloud-print", event_id);
}
...
TEST_F(PrintPreviewHandlerTest, GetPrinters) {
  ...
  printing::TestPrintPreviewHandler* test_handler =
      static_cast<printing::TestPrintPreviewHandler*>(handler());
  for (int i = 0; i < 3; i++) {
    printing::PrinterType type = types[i];
    test_handler->reset_calls();
    base::ListValue args;
    ...
    EXPECT_TRUE(test_handler->called_only_for_type(type));
    EXPECT_EQ(2u + 2 * (i + 1), web_ui()->call_data().size());

    // Validate printers-added
    const content::TestWebUI::CallData& add_data =
        *web_ui()->call_data()[web_ui()->call_data().size() - 2];
    EXPECT_EQ("cr.webUIListenerCallback", add_data.function_name());
    std::string event_id;
    ASSERT_TRUE(add_data.arg1()->GetAsString(&event_id));
    EXPECT_EQ("printers-added", event_id);
    int type_out;
