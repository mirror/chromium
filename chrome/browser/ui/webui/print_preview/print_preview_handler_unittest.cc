...
base::ListValue ConstructPreviewArgs(std::string callback_id,
...
class TestPrintPreviewHandler : public PrintPreviewHandler {
 public:
  ...
  void SendIPC(content::RenderFrameHost* rfh, IPC::Message* message) override {
    sink_.Send(message);
  }
...
class PrintPreviewHandlerTest : public testing::Test {
 public:
  ...
  base::DictionaryValue VerifyPreviewMessage() {
    ...
    EXPECT_TRUE(
        handler()->sink()->GetUniqueMessageMatching(PrintMsg_PrintPreview::ID));
    const IPC::Message* msg =
        handler()->sink()->GetFirstMessageMatching(PrintMsg_PrintPreview::ID);
    EXPECT_TRUE(msg);
...
TEST_F(PrintPreviewHandlerTest, Print) {
  ...
  for (size_t i = 0; i <= arraysize(printing::kAllTypes); i++) {
    ...
    if (type == printing::kLocalPrinter) {
      base::ListValue hideArgs;
      handler()->HandleHidePreview(&hideArgs);
      EXPECT_TRUE(handler()->sink()->GetUniqueMessageMatching(
...
TEST_F(PrintPreviewHandlerTest, SendPreviewUpdates) {
  ...
  int preview_request_id = preview_params
                               .FindKeyOfType(printing::kPreviewRequestID,
                                              base::Value::Type::INTEGER)
                               ->GetInt();
  int preview_ui_id =
      preview_params
          .FindKeyOfType(printing::kPreviewUIID, base::Value::Type::INTEGER)
          ->GetInt();
  ...
  // Check that after each the most recent call is still the response to
  // getPreview.
