...
class TestPrinterHandler : public PrinterHandler {
 public:
  ...
  const std::string& default_printer() { return default_printer_; }
...
class TestPrintPreviewHandler : public PrintPreviewHandler {
 public:
  ...
  PrinterHandler* get_test_printer_handler() {
    return test_printer_handler_.get();
  }
...
class PrintPreviewHandlerTest : public testing::Test {
 public:
  ...
  // Validates the initial settings structure for print preview.
  void ValidateInitialSettings(const base::Value* settings,
...
TEST_F(PrintPreviewHandlerTest, InitialSettings) {
  const std::string& default_printer = "DefaultPrinter";
  const std::string& initiator = "TestInitiator";
  std::vector<printing::PrinterInfo> printers;
  printers.push_back(printing::GetSimplePrinterInfo(default_printer, true));
  std::unique_ptr<PrinterHandler> printer_handler =
      printing::TestPrinterHandler::Create(printers);
  ...
  PrintPreviewHandler* handler = preview_handler.get();

  std::unique_ptr<printing::FakePrintPreviewUI> preview_ui(
      new printing::FakePrintPreviewUI(web_ui(), std::move(preview_handler)));
  preview_ui->SetInitiatorTitle(UTF8ToUTF16(initiator));
  web_ui()->SetController(preview_ui.get());
  ...
  EXPECT_TRUE(success);
  ValidateInitialSettings(data.arg3(), default_printer, initiator);
