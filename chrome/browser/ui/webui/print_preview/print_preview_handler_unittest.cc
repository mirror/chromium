 ...
 public:
  PrintPreviewHandlerTest() {
    ...
    std::unique_ptr<PrintPreviewHandler> preview_handler =
        std::make_unique<printing::TestPrintPreviewHandler>(
            std::move(printer_handler));
    preview_handler->set_web_ui(web_ui());
    handler_ = preview_handler.get();

    preview_ui_ = std::make_unique<printing::FakePrintPreviewUI>(
...
TEST_F(PrintPreviewHandlerTest, GetPrinters) {
  ...
  for (size_t i = 0; i < arraysize(types); i++) {
    ...
    EXPECT_TRUE(test_handler->CalledOnlyForType(type));
