 ...
 public:
  ...
  bool CalledOnlyForType(PrinterType printer_type) {
    return (called_for_type_.size() == 1 &&
            called_for_type_.find(printer_type) != called_for_type_.end());
  }

  void reset_calls() { called_for_type_.clear(); }

 private:
  std::set<PrinterType> called_for_type_;
...
class PrintPreviewHandlerTest : public testing::Test {
 public:
  ...
  void VerifyWebUIEvent(const content::TestWebUI::CallData& data,
...
TEST_F(PrintPreviewHandlerTest, InitialSettings) {
  ...
  VerifyWebUIEvent(*web_ui()->call_data().front(), "use-cloud-print");
}
...
TEST_F(PrintPreviewHandlerTest, GetPrinters) {
  ...
  for (size_t i = 0; i < arraysize(types); i++) {
    ...
    VerifyWebUIEvent(add_data, "printers-added");
    int type_out;
