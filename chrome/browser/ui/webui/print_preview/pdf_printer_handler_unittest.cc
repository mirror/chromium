...
TEST_F(PdfPrinterHandlerTest, GetFileName) {
  ...
  for (const auto& data : kTestData) {
    // Replicate the logic in PdfPrinterHandler, and make sure it makes the
    // right decision.
    SCOPED_TRACE(std::string(data.url) + " | " + data.job_title);
    GURL initiator_url(data.url);
    ...
    base::FilePath path =
        title_is_url
            ? PdfPrinterHandler::GetFileNameForURL(initiator_url)
            : PdfPrinterHandler::GetFileNameForPrintJobTitle(job_title);
    EXPECT_EQ(data.expected_output, path.value());
  }
}
