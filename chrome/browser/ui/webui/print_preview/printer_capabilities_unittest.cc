...
base::DictionaryValue GetCapabilitiesFull() {
  ...
  pages_per_sheet_capability[kDisplayName] =
      std::make_unique<base::Value>("Pages per sheet");
  pages_per_sheet_capability[kId] =
      std::make_unique<base::Value>("Pages per sheet");
  pages_per_sheet_capability[kTypeKey] =
      std::make_unique<base::Value>("SELECT");
  ...
  paper_type_capability[kDisplayName] =
      std::make_unique<base::Value>("Paper Type");
  paper_type_capability[kId] = std::make_unique<base::Value>("Paper Type");
  paper_type_capability[kTypeKey] = std::make_unique<base::Value>("SELECT");
...
void ValidateVendorCaps(const base::Value* printer_out,
  ...
  size_t index = 0;
  for (size_t i = 0; i < expected_vendor_caps->GetList().size(); i++) {
    if (!HasValidEntry(
            expected_vendor_caps->GetList()[i]
                .FindPath({kSelectCapKey})
                ->FindPathOfType({kOptionKey}, base::Value::Type::LIST))) {
      continue;
    }
    EXPECT_TRUE(vendor_capability_out->GetList()[index]
                    .FindPath({kDisplayName})
                    ->GetString() == expected_vendor_caps->GetList()[i]
                                         .FindPath({kDisplayName})
                                         ->GetString());
    EXPECT_TRUE(
        vendor_capability_out->GetList()[index].FindPath({kId})->GetString() ==
        expected_vendor_caps->GetList()[i].FindPath({kId})->GetString());
    EXPECT_TRUE(
        vendor_capability_out->GetList()[index]
            .FindPath({kTypeKey})
            ->GetString() ==
        expected_vendor_caps->GetList()[i].FindPath({kTypeKey})->GetString());
...
void ValidatePrinter(std::unique_ptr<base::DictionaryValue> cdd_out,
  ...
  const base::Value* printer_out =
      cdd_out->FindPathOfType({kPrinterKey}, base::Value::Type::DICTIONARY);
...
TEST_F(PrinterCapabilitiesTest, ProvidedCapabilitiesUsed) {
  ...
  ASSERT_TRUE(settings_dictionary->GetDictionary(kPrinterCapabilities, &cdd));

  // read the CDD for the dpi attribute.
  base::DictionaryValue* caps_dict;
  ASSERT_TRUE(cdd->GetDictionary(kPrinterKey, &caps_dict));
...
TEST_F(PrinterCapabilitiesTest, NullCapabilitiesExcluded) {
  ...
  base::DictionaryValue* caps_dict;
  ASSERT_TRUE(
      settings_dictionary->GetDictionary(kPrinterCapabilities, &caps_dict));
...
TEST_F(PrinterCapabilitiesTest, FilterBadVendorCapabilityAllElement) {
  base::DictionaryValue cdd;
  base::DictionaryValue printer = GetCapabilitiesFull();
  printer.FindPath({kVendorCapability})
      ->GetList()[0]
      .FindPath({kSelectCapKey})
