...
size_t ValidEntriesCount(const base::Value* list) {
  auto out_list =
...
bool HasValidEntry(const base::Value* list) {
  return (list && !list->GetList().empty() && ValidEntriesCount(list) > 0);
}

void ValidateList(const base::Value* list_out, const base::Value* input_list) {
  ASSERT_EQ(list_out->GetList().size(), ValidEntriesCount(input_list));
  size_t index = 0;
  for (const auto& input_element : input_list->GetList()) {
    if (input_element.type() == base::Value::Type::NONE)
      continue;
    EXPECT_EQ(list_out->GetList()[index], input_element);
    index++;
  }
}
...
void ValidateVendorCaps(const base::Value* printer_out,
  ...
  for (const auto& input_entry : input_vendor_caps->GetList()) {
    ...
    EXPECT_EQ(
        input_entry.FindPathOfType({kDisplayName}, base::Value::Type::STRING)
            ->GetString(),
        output_list[index]
            .FindPathOfType({kDisplayName}, base::Value::Type::STRING)
            ->GetString());
    EXPECT_EQ(input_entry.FindPathOfType({kId}, base::Value::Type::STRING)
                  ->GetString(),
              output_list[index]
                  .FindPathOfType({kId}, base::Value::Type::STRING)
                  ->GetString());
    EXPECT_EQ(input_entry.FindPathOfType({kTypeKey}, base::Value::Type::STRING)
                  ->GetString(),
              output_list[index]
