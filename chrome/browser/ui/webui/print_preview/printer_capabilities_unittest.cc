...
void CompareStringKeys(const base::Value& expected,
  ...
  const base::Value* expected_val =
      expected.FindPathOfType({key}, base::Value::Type::STRING);
  const base::Value* actual_val =
      actual.FindPathOfType({key}, base::Value::Type::STRING);
  EXPECT_EQ(expected_val->GetString(), actual_val->GetString());
}
