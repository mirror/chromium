...
const std::unique_ptr<PrinterHandler> null_handler = nullptr;
...
void PrintPreviewHandler::RegisterMessages() {
  ...
  web_ui()->RegisterMessageCallback(
      "getPrivetPrinters",
      base::Bind(&PrintPreviewHandler::HandleGetExtensionPrivetPrinters,
                 base::Unretained(this), PrinterType::kPrivetPrinter));
  web_ui()->RegisterMessageCallback(
      "getPrivetPrinterCapabilities",
      base::Bind(
          &PrintPreviewHandler::HandleGetExtensionPrivetPrinterCapabilities,
          base::Unretained(this), PrinterType::kPrivetPrinter));
  web_ui()->RegisterMessageCallback(
      "getExtensionPrinters",
      base::Bind(&PrintPreviewHandler::HandleGetExtensionPrivetPrinters,
                 base::Unretained(this), PrinterType::kExtensionPrinter));
  web_ui()->RegisterMessageCallback(
...
void PrintPreviewHandler::HandleGetExtensionPrivetPrinters(
  ...
  const std::unique_ptr<PrinterHandler>& handler =
      GetPrinterHandler(printer_type);
  if (!handler) {
...
void PrintPreviewHandler::HandleGetExtensionPrivetPrinterCapabilities(
    PrinterType printer_type,
    const base::ListValue* args) {
  std::string callback_id;
...
const std::unique_ptr<PrinterHandler>& PrintPreviewHandler::GetPrinterHandler(
    PrinterType printer_type) {
  if (printer_type == PrinterType::kExtensionPrinter) {
    ...
    return extension_printer_handler_;
  }
#if BUILDFLAG(ENABLE_SERVICE_DISCOVERY)
  if (printer_type == PrinterType::kPrivetPrinter) {
    ...
    return privet_printer_handler_;
  }
#endif
  return null_handler;
}
...
void PrintPreviewHandler::OnGotPrintersForExtensionOrPrivet(
  ...
  if (!printers.empty()) {
    std::string event = printer_type == PrinterType::kExtensionPrinter
                            ? "extension-printers-added"
                            : "privet-printer-added";
    FireWebUIListener(event, printers);
  }
  if (done) {
    ResolveJavascriptCallback(base::Value(callback_id), base::Value());
  }
