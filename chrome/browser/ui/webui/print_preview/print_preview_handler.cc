...
struct Join<Head, Tail...> {
  ...
  static void Apply(const Cb& cb,
                    ...
                    base::ListValue::const_iterator&& it,
                    Args&&... args) {
    Join<Tail...>::Apply(cb, list, it + 1, std::forward<Args>(args)...,
                         GetValue<Head>(*it));
  }
};

template <typename... Args>
void Handle(const base::Callback<void(Args...)>& cb,
            const base::ListValue* list) {
  Join<Args...>::Apply(cb, list, list->begin());
}
...
void RegisterMessage(PrintPreviewHandler* handler,
  ...
  handler->web_ui()->RegisterMessageCallback(
      message, base::Bind(&Handle<Args...>,
                          base::Bind(method, base::Unretained(handler))));
}
