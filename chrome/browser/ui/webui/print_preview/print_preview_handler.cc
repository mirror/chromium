...
#if BUILDFLAG(ENABLE_SERVICE_DISCOVERY)
  EnsurePrivetPrinterHandlerSet();

  privet_printer_handler_->StartGetPrinters(
      base::Bind(&PrintPreviewHandler::OnGotPrintersForPrivet,
                 weak_factory_.GetWeakPtr(), callback_id));
#endif
}
...
#if BUILDFLAG(ENABLE_SERVICE_DISCOVERY)
  EnsurePrivetPrinterHandlerSet();
  privet_printer_handler_->StartGetCapability(
      printer_name, base::Bind(&PrintPreviewHandler::OnGotPrinterCapabilities,
                               weak_factory_.GetWeakPtr(), callback_id));
#endif
}
...
void PrintPreviewHandler::HandlePrint(const base::ListValue* args) {
  ...
  print_with_privet &= PrivetPrintingEnabled();
#if BUILDFLAG(ENABLE_SERVICE_DISCOVERY)
  ...
  if (print_with_privet || print_with_extension) {
    ...
    std::unique_ptr<PrinterHandler>& handler = print_with_privet
...
void PrintPreviewHandler::EnsurePrivetPrinterHandlerSet() {
...
void PrintPreviewHandler::OnGotPrintersForPrivet(
    const std::string& callback_id,
    const base::ListValue& printers,
    bool done) {
  if (!printers.empty()) {
    // Privet printers come in one at a time so the list will have 1 printer.
    const base::DictionaryValue* printer = nullptr;
    printers.GetDictionary(0, &printer);
    FireWebUIListener("privet-printer-added", *printer);
  } else {
    // Should mean timer is expired.
    DCHECK(done);
    ResolveJavascriptCallback(base::Value(callback_id), base::Value());
  }
}
