...
std::unique_ptr<base::ListValue> GetFilteredList(const base::Value* list,
                                                 Predicate pred) {
  auto out_list =
    ...
    if (is_vendor_capability) {
      ...
      for (auto& vendor_option : *out_list) {
        ...
        options_dict->SetPath(
            {kOptionKey}, GetFilteredList(options_list, ValueIsNull)->Clone());
      }
    }
    if (dict) {
      auto option_dict = std::make_unique<base::DictionaryValue>();
      option_dict->SetList(kOptionKey, std::move(out_list));
      out_caps->SetDictionary(path, std::move(option_dict));
    } else {
      out_caps->SetList(path, std::move(out_list));
    }
  }
