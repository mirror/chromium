...
void SystemDialogDone(bool success, const base::Value& error) {
  // intentional no-op
}
...
void StartLocalPrint(const PrinterHandler::PrintCallback& callback,
  ...
  preview_ui->GetPrintPreviewDataForIndex(*pages.begin(), &first_page_data);
  ...
  print_view_manager->PrintForPrintPreview(
      (system_dialog || open_in_pdf) ? base::Bind(&SystemDialogDone) : callback,
