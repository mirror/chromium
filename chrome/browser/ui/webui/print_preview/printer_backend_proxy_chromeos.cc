...
class PrinterBackendProxyChromeos : public PrinterBackendProxy {
 public:
  explicit PrinterBackendProxyChromeos(Profile* profile)
      : prefs_(chromeos::PrintersSyncManagerFactory::GetForBrowserContext(
            profile)),
  ...
  void EnumeratePrinters(const EnumeratePrintersCallback& cb) override {
    // PrintersSyncManager is not thread safe and must be called from the UI
    // thread.
    DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
 ...
 private:
  ...
  chromeos::PrintersSyncManager* prefs_;
