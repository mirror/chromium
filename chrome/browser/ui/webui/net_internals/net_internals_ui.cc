...
void NetInternalsMessageHandler::IOThreadImpl::OnExpectCTTestReport(
  ...
  if (!expect_ct_reporter_) {
    expect_ct_reporter_ =
        base::MakeUnique<ChromeExpectCTReporter>(GetMainContext());
    ...
    expect_ct_reporter_->set_success_callback(base::Bind(
        &NetInternalsMessageHandler::IOThreadImpl::SendJavascriptCommand, this,
        "receivedExpectCTTestReportResult", base::Passed(&success)));
    expect_ct_reporter_->set_failure_callback(base::Bind(
        &NetInternalsMessageHandler::IOThreadImpl::SendJavascriptCommand, this,
        "receivedExpectCTTestReportResult", base::Passed(&failure)));
  }
