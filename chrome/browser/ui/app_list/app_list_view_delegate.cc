...
void AppListViewDelegate::SetProfile(Profile* new_profile) {
...

  {
    ...
    // After |model_| is initialized, explicitly notify wallpaper colors changed
    // so that |model_|'s wallpaper prominent colors are set.
    wallpaper_controller_ptr_->NotifyWallpaperColorsChanged();
