...
namespace {
constexpr char kGmailExtensionId[] = "pjkljhegncpnkpknbcohdijeoejaedia";
}
...
TEST_F(ArcPlayStoreSearchProviderTest, Basic) {
  ...
  AddExtension(CreateExtension(kGmailExtensionId).get());
  ...
  // Playstore returns |kMaxResults| results, but the first one (GMail) already
  // has has Chrome extension installed, so it will be skipped.
