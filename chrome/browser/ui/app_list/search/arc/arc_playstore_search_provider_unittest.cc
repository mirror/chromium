...
class ArcPlayStoreSearchProviderTest : public AppListTestBase {
 public:
  ...
  void RunQuery(ArcPlayStoreSearchProvider* provider,
                const std::string& query) {
    provider->Start(false, base::UTF8ToUTF16(query));
  }
...
TEST_F(ArcPlayStoreSearchProviderTest, Basic) {
  ...
  // Checks that the result size of a query doesn't exceed the |kMaxResults|.
  RunQuery(provider.get(), kQuery);
  const app_list::SearchProvider::Results& results = provider->results();
  ASSERT_TRUE(results.size() <= kMaxResults);
