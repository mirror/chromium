...
class AnswerCardSearchProvider : public SearchProvider,
 ...
 public:
  ...
  void UpdatePreferredSize(const AnswerCardContents* source,
 ...
 private:
  ...
  bool IsCardSizeOk(int contents_index) const;
  void RecordReceivedAnswerFinalResult();
  void OnResultAvailable(int contents_index, bool is_available);
  ...
  // Index of the navigation contents that will be used for loading the next
  // card, or is already loading a card. It switches to the other contents after
  // the card gets successfully loaded.
  int next_navigation_context_ = 0;
