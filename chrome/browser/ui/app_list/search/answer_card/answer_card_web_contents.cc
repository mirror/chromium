...
class SearchAnswerWebView : public views::WebView {
 public:
  ...
  void OnVisibilityEvent(bool is_removed_from_widget) {
    // Need to check for |is_removed_from_widget| because inside
    // RemovedFromWidget() callback, GetWidget() still returns a non-null value.
    if (!is_removed_from_widget && GetWidget() && GetWidget()->IsActive() &&
