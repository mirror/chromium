  ...
  for (size_t i = 1; i < parts.size() - 1; ++i) {
    ...
    if (key == kAction) {
      ...
      if (!activity_compact_name.empty() && activity_compact_name[0] == '.') {
        intent->activity = value.substr(0, component_separator).as_string();
        intent->activity += activity_compact_name.as_string();
      } else {
        intent->activity = activity_compact_name.as_string();
      }
    } else {
