...
constexpr int kFlagActivityNewTask = 0x10000000;
constexpr int kFlagActivityResetTaskIfNeeded = 0x00200000;
...
bool LaunchAppWithRect(content::BrowserContext* context,
  ...
  if (app_info->shortcut || !launch_intent.empty()) {
    ...
    if (!GET_APP_INSTANCE(LaunchIntent))
      return false;
    app_instance->LaunchIntent(
        launch_intent.empty() ? app_info->intent_uri : launch_intent,
        target_rect);
  } else {
...
bool LaunchApp(content::BrowserContext* context,
               ...
               int event_flags) {
  return LaunchAppWithIntent(context, app_id, std::string() /* launch_intent */,
...
bool LaunchAppWithIntent(content::BrowserContext* context,
  ...
  return (new LaunchAppWithoutSize(context, app_id, launch_intent,
                                   landscape_layout, event_flags))
      ->LaunchAndRelease();
}
...
std::string GetLaunchIntent(const std::string& package_name,
  ...
  for (const std::string extra_param : extra_params) {
  ...
  return base::StringPrintf(
      ...
      kFlagActivityNewTask | kFlagActivityResetTaskIfNeeded, kComponent,
...
bool ParseIntent(const std::string& intent,
  ...
  for (size_t i = 1; i < parts.size() - 1; ++i) {
    ...
    if (separator == std::string::npos) {
      if (extra_params)
        extra_params->push_back(parts[i]);
      continue;
    }
    const std::string key = parts[i].substr(0, separator);
    const std::string value = parts[i].substr(separator + 1);
    if (key == kAction) {
      if (action)
        *action = value;
      ...
      if (launch_flags) {
        bool parsed = sscanf(value.c_str(), "0x%x", launch_flags);
        DCHECK(parsed);
      }
      ...
      if (component_separator == std::string::npos)
        return false;
      if (package_name)
        *package_name = value.substr(0, component_separator);
      if (activity) {
        const std::string activity_compact_name =
            value.substr(component_separator + 1);
        if (activity_compact_name.find(".") == 0) {
          *activity = value.substr(0, component_separator);
          *activity += activity_compact_name;
        } else {
          *activity = activity_compact_name;
        }
      }
...
bool GetExtraParamsFromIntent(const std::string& intent,
                              std::vector<std::string>* extra_params) {
  return ParseIntent(intent, nullptr /* action */, nullptr /* category */,
                     nullptr /* package_name */, nullptr /* activity */,
                     nullptr /* launch_flags */, extra_params);
}
