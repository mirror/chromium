...
void ArcAppListPrefs::SetLastLaunchTime(const std::string& app_id) {
  ...
  arc_vpn_provider_helper_.UpdateLastLauchTime(app_id);
}
...
void ArcAppListPrefs::AddAppAndShortcut(
  ...
  if (was_tracked) {
    ...
    if (updated_name != app_old_info->name) {
      ...
      arc_vpn_provider_helper_.UpdateAppName(app_id);
    }
  }
...
void ArcAppListPrefs::OnPackageAdded(
  ...
  arc_vpn_provider_helper_.UpdatePackageInstalled(package_info->package_name);
...
void ArcAppListPrefs::OnPackageListRefreshed(
  ...
  arc_vpn_provider_helper_.UpdatePackageListInitialRefreshed();
}
