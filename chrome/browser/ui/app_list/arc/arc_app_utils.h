...
// Parses provided |intent|. |action|, |category|, |launch_flags|,
// |package_name|, |activity| and |extra_params| are optional arguments. Once
// provided they are filled with corresponded parts of intent. Returns false
// if |intent| cannot be parsed.
bool ParseIntent(const std::string& intent,
                 std::string* action,
                 std::string* category,
...
// Extracts extra parameters from |intent|. Returns false if |intent| cannot be
// parsed.
bool GetExtraParamsFromIntent(const std::string& intent,
                              std::vector<std::string>* extra_params);
