...
TEST_F(ArcAppUtilsTest, LaunchIntent) {
  ...
  EXPECT_EQ(intent1.launch_flags, 0x10200000);
  ASSERT_EQ(intent1.extra_params.size(), 1U);
  EXPECT_EQ(intent1.extra_params[0], arc::kInitialStartParam);
  ...
  EXPECT_EQ(intent2.launch_flags, 0x18001000);
  ASSERT_EQ(intent2.extra_params.size(), 1U);
  EXPECT_EQ(intent2.extra_params[0],
