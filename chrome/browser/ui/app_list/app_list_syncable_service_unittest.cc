...
TEST_F(AppListSyncableServiceTest, OEMFolderForConflictingPos) {
  ...
  ChromeAppListItem* web_store_item =
      (ChromeAppListItem*)model_updater()->FindItem(web_store_app_id);
  ASSERT_TRUE(web_store_item);
  ChromeAppListItem* some_app_item =
      (ChromeAppListItem*)model_updater()->FindItem(some_app_id);
  ...
  ChromeAppListItem* oem_app_item =
      (ChromeAppListItem*)model_updater()->FindItem(oem_app_id);
  EXPECT_NE(nullptr, oem_app_item);
  EXPECT_EQ(oem_app_item->folder_id(),
            app_list::AppListSyncableService::kOemFolderId);
  // But OEM folder is.
  ChromeAppListItem* oem_folder = (ChromeAppListItem*)model_updater()->FindItem(
      app_list::AppListSyncableService::kOemFolderId);
