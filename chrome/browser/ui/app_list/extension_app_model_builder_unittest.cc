...
const std::set<std::string> kDefaultApps = {"Packaged App 1", "Packaged App 2",
                                            "Hosted App"};
...
TEST_F(ExtensionAppModelBuilderTest, Build) {
  ...
  EXPECT_EQ(kDefaultApps, GetModelContent(model_updater_.get()));
}
...
TEST_F(ExtensionAppModelBuilderTest, HideWebStore) {
  ...
  builder1.InitializeForTest(profile_.get(), &model_updater1);
  ...
  builder2.InitializeForTest(profile_.get(), &model_updater2);
