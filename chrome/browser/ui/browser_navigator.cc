...
std::pair<Browser*, int> GetBrowserAndTabForDisposition(
  ...
  switch (params->disposition) {
      ...
      if (params->disposition == WindowOpenDisposition::SINGLETON_TAB) {
        ...
        for (auto browser_it = BrowserList::GetInstance()->begin_last_active();
          ...
          if (browser->profile()->IsSameProfile(profile) &&
            ...
            if (index >= 0)
              return std::make_pair(browser, index);
          }
        }
#if defined(OS_ANDROID)
      }
    ...
    case WindowOpenDisposition::CURRENT_TAB:
      if (params->browser)
        return std::make_pair(params->browser, -1);
      // Find a compatible window and re-execute this command in it. Otherwise
      // re-run with NEW_WINDOW.
      return std::make_pair(GetOrCreateBrowser(profile, params->user_gesture),
                            -1);
    ...
    case WindowOpenDisposition::NEW_POPUP: {
      ...
      if (app_name.empty()) {
        ...
        return std::make_pair(new Browser(browser_params), -1);
      }
    ...
    case WindowOpenDisposition::OFF_THE_RECORD:
      ...
      return std::make_pair(
          GetOrCreateBrowser(profile->GetOffTheRecordProfile(),
                             params->user_gesture),
          -1);
