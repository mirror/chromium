...
constexpr CGFloat kBadgeSize = 48;
...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  const NSRect badgeRect =
      NSMakeRect(NSMaxX(self.bounds) - kBadgeSize - kBadgeMargin, kBadgeMargin,
                 kBadgeSize, kBadgeSize);
  const CGFloat badgeRadius = kBadgeSize / 2;
  ...
  for (const auto shadowProps : kBadgeShadows) {
    ...
    NSColor* color =
        [NSColor colorWithCalibratedWhite:0 alpha:shadowProps.opacity];
    [shadow set];
    [color setFill];
    [backgroundPath fill];
  }
