...
- (void)ensureContentsVisibleInSuperview:(NSView*)superview {
  ...
  } else if (fullscreenPlaceholderView) {
    contentsNativeView = fullscreenPlaceholderView;
  } else {
...
- (void)toggleFullscreenWidget:(BOOL)enterFullscreen {
  ...
  if (FullscreenWithinTabHelper::IsContentFullscreenEnabled()) {
    if (enterFullscreen) {
      fullscreenPlaceholderView = [self createScreenshotView];
      separateFullscreenWindow = [self createSeparateWindowForTab:contents_];

      [separateFullscreenWindow makeKeyAndOrderFront:nil];
      [separateFullscreenWindow toggleFullScreen:nil];
    } else {
      [separateFullscreenWindow close];
    }
  }
  [self ensureContentsVisibleInSuperview:[[self view] superview]];
}
