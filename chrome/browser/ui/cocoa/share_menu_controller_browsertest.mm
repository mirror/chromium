...
IN_PROC_BROWSER_TEST_F(ShareMenuControllerTest, Histograms) {
  ...
  const std::string success_name = "OSX.NativeShare.Success";
  const std::string failure_name = "OSX.NativeShare.Failure";
  ...
  base::scoped_nsobject<MockSharingService> mock_service =
      MakeMockSharingService();

  auto twitter_bucket =
      static_cast<base::HistogramBase::Sample>(MacShareMenuServices::kTwitter);
  auto facebook_bucket =
      static_cast<base::HistogramBase::Sample>(MacShareMenuServices::kFacebook);
  auto unknown_bucket = static_cast<base::HistogramBase::Sample>(
      MacShareMenuServices::kUnknownCustom);
