...
- (TabWindowController*)detachTabsToNewWindow:(NSArray*)tabViews
  ...
  // Move the dragged tabs to the window.
  TabStripModel* model = browser_->tab_strip_model();
  const NSUInteger activeIndex =
      [tabViews indexOfObjectPassingTest:^BOOL(TabView* tabView, NSUInteger i,
                                               BOOL* stop) {
        return [tabStripController_ modelIndexForTabView:tabView] ==
               model->active_index();
      }];
  [tabViews
      enumerateObjectsUsingBlock:^(TabView* tabView, NSUInteger i, BOOL* stop) {
        [controller->tabStripController_
            adoptTabFromController:tabStripController_
                             index:[tabStripController_
                                       modelIndexForTabView:tabView]
                           atIndex:i
                         withFrame:NSZeroRect
                          activate:i == activeIndex];
      }];
