...
- (NSTouchBar*)touchBarForFullscreenWindow API_AVAILABLE(macos(10.12.2)) {
  if (@available(macOS 10.12, *))
    return [[self browserWindowTouchBar] createTabFullscreenTouchBar];
  return nil;
}
...
- (NSWindow*)createSeparateWindowForTab:(WebContents*)separatedTab {
  ...
  [separateWindow setTouchBarDelegate:[sourceWindow windowController]];
  ...
  // WebContentsViewCocoa is now in a separate window. This is made to
  // avoid keyboard input in the main browser window interacting with the
  // fullscreen content.
  // TODO(erickzul): Replace TabContentsContainerView with PlaceHolderView
  // which will be a subview of TabContentsContainerView.
  [sourceWindow
      makeFirstResponder:[[tabStripController_ activeTabContentsController]
                             view]];
...
- (NSWindow*)getActiveWebContentsSeparateWindow {
  ...
  FullscreenWithinTabHelper* helper =
      web_contents ? FullscreenWithinTabHelper::FromWebContents(web_contents)
                   : nil;
  return helper ? helper->separate_fullscreen_window() : nil;
}
...
- (void)enterWebContentFullscreen {
  if (FullscreenWithinTabHelper::IsContentFullscreenEnabled()) {
    ...
    FullscreenWithinTabHelper* helper =
        tab ? FullscreenWithinTabHelper::FromWebContents(tab) : nil;

    DCHECK(helper);
    if (!helper->separate_fullscreen_window()) {
      helper->set_separate_fullscreen_window(
          [self createSeparateWindowForTab:tab]);
    }
    DCHECK(helper->separate_fullscreen_window());
    NSWindow* activeSeparateFullscreenWindow =
        helper->separate_fullscreen_window();
