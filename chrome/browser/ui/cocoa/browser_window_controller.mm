...
- (NSWindow*)CreateSeparateWindowForTab:(WebContents*)separatedTab {
  ...
  // RenderWidgetHostViewCocoa from the WebContents's WebContentsViewCocoa.
  NSView* separatedTabView = [separatedTab->GetNativeView() subviews][0];
  ...
  BrowserWindowController* controller = [sourceWindow windowController];
  [separateWindow setController:controller];
...
- (NSWindow*)getActiveWebContentsSeparateWindow {
  ...
  FullscreenWithinTabHelper* helper =
      web_contents ? FullscreenWithinTabHelper::FromWebContents(web_contents)
                   : nullptr;
  if (helper && helper->separate_fullscreen_window())
    return helper->separate_fullscreen_window();
  return nullptr;
}
