...
- (instancetype)initWithFrame:(NSRect)frame isSoftWarning:(BOOL)isSoftWarning {
  if ((self = [super initWithFrame:frame])) {
    ...
    // TODO(spqchan): Use the security alert icon for softer warning once
    // the icon file is available.
    SkColor iconColor =
        isSoftWarning_ ? gfx::kChromeIconGrey : gfx::kGoogleRed700;
    NSImageView* imageView = [[[NSImageView alloc]
        initWithFrame:NSMakeRect(25, 85, 50, 50)] autorelease];
    [imageView setImage:NSImageFromImageSkia(gfx::CreateVectorIcon(
                            vector_icons::kWarningIcon, 50, iconColor))];
