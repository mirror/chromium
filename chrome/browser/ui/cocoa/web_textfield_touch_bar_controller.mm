...
- (void)updateTextSuggestionsWithInfo:(content::TextSuggestionInfo)info {
  LOG(INFO) << "updateTextSuggestionsWithInfo " << info.text_for_suggestion;
...
- (void)candidateListTouchBarItem:(NSCandidateListTouchBarItem*)anItem
  ...
  if (anItem == candidateListItem_.get()) {
    ...
    LOG(INFO) << "replacementString: ";
    LOG(INFO) << [replacementString UTF8String];

    if (textSuggestionInfo_.offsetFromCursor != 0) {
      LOG(INFO) << "AdjustSelection";
      contents_->AdjustSelectionByCharacterOffset(
          -textSuggestionInfo_.offsetFromCursor, 0, false);
    }

    contents_->Replace(base::SysNSStringToUTF16(replacementString));
    LOG(INFO) << "Done replacing";
  }
}
