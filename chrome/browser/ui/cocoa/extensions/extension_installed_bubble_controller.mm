...
gfx::Point ExtensionInstalledBubble::GetAnchorPoint(
  ...
  auto getAppMenuButtonAnchorPoint = [window_controller]() {
    // Point at the bottom of the app menu menu.
    NSView* appMenuButton =
        [[window_controller toolbarController] appMenuButton];
  ...
  switch (anchor_position()) {
    case ExtensionInstalledBubble::ANCHOR_ACTION: {
      BrowserActionsController* controller =
          [[window_controller toolbarController] browserActionsController];
...
std::unique_ptr<BubbleUi> ExtensionInstalledBubble::BuildBubbleUi() {
  if (ui::MaterialDesignController::IsSecondaryUiMaterial()) {
    ...
    auto ui = BuildViewsExtensionInstalledBubbleUi(this, &bubble_impl);
    KeepBubbleAnchored(bubble_impl);
    return ui;
  }
