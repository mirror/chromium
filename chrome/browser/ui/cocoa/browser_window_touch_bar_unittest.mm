...
class BrowserWindowTouchBarUnitTest : public CocoaProfileTest {
 public:
  ...
  void UpdateCommandEnabled(int id, bool enabled) {
    commandUpdater_->UpdateCommandEnabled(id, enabled);
  }
...
// Tests to see if the back/forward items on the touch gets updated if the
// associated command is updated.
TEST_F(BrowserWindowTouchBarUnitTest, BackForwardCommandUpdate) {
  if (@available(macOS 10.12.2, *)) {
    ...
    EXPECT_EQ(true,
              [back_forward_control isEnabledForSegment:kBackSegmentIndex]);
    EXPECT_EQ(true,
