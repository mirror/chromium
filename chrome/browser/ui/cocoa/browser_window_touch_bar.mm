...
// TODO(erickzul): Move to sdk forward declarations
...
@implementation TouchBarView : NSView
...
@synthesize touchbarView = touchbarView_;
...
- (NSTouchBar*)makeTouchBar {
  ...
  for (NSString* item in touchBarItems) {
    ...
    if (showHomeButton_.GetValue() ||
        !([item isEqualTo:kHomeTouchId] || [item isEqualTo:kSliderId]) ||
...
- (NSCustomTouchBarItem*)touchBar:(NSTouchBar*)touchBar
  ...
  if ([@"WebContent" isEqualToString:identifier]) {
    TouchBarView* view = [[TouchBarView alloc] initWithFrame:NSZeroRect];
    [touchbarView_ setFrame:[view frame]];
    [touchbarView_
        setAutoresizingMask:NSViewWidthSizable | NSViewHeightSizable];
    [view addSubview:touchbarView_];
    // NSView *tabView = [bwc_ touchbarTabContentArea];
    //[tabView addSubview: view];
    //[view setWantsLayer:YES];
    view.allowedTouchTypes = NSTouchTypeMaskDirect;
    view.acceptsTouchEvents = YES;
    // view.layer.backgroundColor = [[NSColor yellowColor] CGColor];
    [touchBarItem setView:view];
