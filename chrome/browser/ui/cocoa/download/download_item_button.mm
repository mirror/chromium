...
- (void)showContextMenu {
  // TODO(sdy): This @autoreleasepool is a hack. The menu returned by
  // -contextMenu is owned by DownloadShelfContextMenuController which retains
  // DownloadItemController. The unit test
  // DownloadItemControllerTest.DismissesContextMenuWhenRemovedFromWindow (and
  // possibly real life behavior) relies on the controller *not* being retained
  // after the menu is dismissed. There's probably a more elegant way to handle
  // this dance but might be easier to reason about once MD is perma-on.
  @autoreleasepool {
    contextMenu_.reset([[controller_ contextMenu] retain]);
