...
NSTextField* MakeLabel(
  ...
  label.alignment = NSNaturalTextAlignment;
...
- (void)showMenuWithEvent:(NSEvent*)event {
  ...
  NSPopUpButtonCell* popUpCell =
      [[[NSPopUpButtonCell alloc] initTextCell:@"" pullsDown:YES] autorelease];
  popUpCell.menu = [self.superview menu];
...
- (instancetype)initWithFrame:(NSRect)frameRect {
  if ((self = [super initWithFrame:frameRect])) {
    ...
    iconView_ = [[[NSImageView alloc]
        initWithFrame:[self cr_localizedRect:iconRect]] autorelease];
    ...
    button_ = [[[HarmonyButton alloc] initWithFrame:NSZeroRect] autorelease];
...
- (void)viewWillDraw {
  ...
  label_.textColor = themeProvider.GetNSColor(ThemeProperties::COLOR_TAB_TEXT);
  [super viewWillDraw];
}
...
- (instancetype)init {
  if ((self = [super initWithFrame:NSMakeRect(0, 0, kNormalSize.width,
                                              kNormalSize.height)])) {
    button_ = [[[MDHoverButton alloc] initWithFrame:self.bounds] autorelease];
    ...
    progressIndicator_ = [[[MDDownloadItemProgressIndicator alloc]
    ...
    imageView_ = [[[NSImageView alloc]
        initWithFrame:[self cr_localizedRect:kImageFrame]] autorelease];
    ...
    NSRect menuButtonRect = NSMakeRect(
        ...
        kMenuButtonSize);
    menuButton_ = [[[MDDownloadItemMenuButton alloc]
        initWithFrame:[button_ cr_localizedRect:menuButtonRect]] autorelease];
    ...
    NSRect dividerRect =
        NSInsetRect(NSMakeRect(NSWidth(self.bounds) - kDividerWidth, 0,
                    ...
                    0, kDividerYInset);
    dividerView_ = [[[NSView alloc]
        initWithFrame:[self cr_localizedRect:dividerRect]] autorelease];
...
- (NSMenu*)menu {
  return [controller_ contextMenu];
}
...
- (void)setStateFromDownload:(DownloadItemModel*)downloadModel {
  ...
  if (download.IsDangerous()) {
    if (!dangerView_) {
      ...
      NSRect dangerViewRect = NSInsetRect(
          ...
          0, kDangerViewYInset);
      dangerView_ = [[[MDDownloadItemDangerView alloc]
          initWithFrame:[self cr_localizedRect:dangerViewRect]] autorelease];
  ...
  [NSAnimationContext runAnimationGroup:^(NSAnimationContext* context) {
    ...
    [button_ cr_setAccessibilityLabel:
                 l10n_util::GetNSStringWithFixup(
                     download.IsDone() ? IDS_DOWNLOAD_MENU_OPEN
                                       : IDS_DOWNLOAD_MENU_OPEN_WHEN_COMPLETE)];

    [self cr_setAccessibilityLabel:base::SysUTF8ToNSString(
    ...
    BOOL hasStatus = statusString.length;
    context.duration = 0.3;
    [filenameView_.animator
