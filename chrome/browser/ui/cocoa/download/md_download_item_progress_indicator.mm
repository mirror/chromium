...
- (void)enqueue:(void (^)())animation {
  if (!animations_)
    animations_.reset([[NSMutableArray alloc] init]);
  [animations_ addObject:[[animation copy] autorelease]];

  // If this is the first animation in the queue, run it. But, if the view
  // doesn't have a layer, it hasn't made it into a view hierarchy yet and none
  // of the sublayer ivars are set. In that case, leave it in the queue. The
  // end of -setLayer: calls -runAnimations.
  if ([animations_ count] == 1 && self.layer)
    [self runAnimations];
}
...
- (void)enqueue:(void (^)())animation with:(void (^)())extra {
...
- (void)setState:(MDDownloadItemProgressIndicatorState)state
  ...
  if (state != targetState_) {
    ...
    if (targetState_ == MDDownloadItemProgressIndicatorState::kInProgress) {
      ...
      [self enqueue:[self updateStateAnimation] with:animations];
    ...
    targetProgress_ = progress;
    [self enqueue:[self updateProgressAnimation] with:animations];
  } else if (animations) {
