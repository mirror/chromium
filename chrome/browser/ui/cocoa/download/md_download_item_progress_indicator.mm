...
struct Animation {
  ...
  Type type;
  base::mac::ScopedBlock<void (^)()> block;
};
...
- (void)runAnimations {
  ...
  [NSAnimationContext runAnimationGroup:^(NSAnimationContext* context) {
    ...
    Animation anim = std::move(animations_.front());
    switch (anim.type) {
      case Animation::Type::kBlockOnly:
        break;
      case Animation::Type::kUpdateState:
        [self updateState];
        break;
      case Animation::Type::kUpdateProgress:
        [self updateProgress];
        break;
    }
    if (void (^block)() = anim.block.get())
      block();
  }
      completionHandler:^{
...
- (void)enqueue:(Animation::Type)type block:(void (^)())animations {
  animations_.emplace_back(
      Animation{type, base::mac::ScopedBlock<void (^)()>([animations copy])});
  if (animations_.size() == 1 && self.layer)
    [self runAnimations];
}
...
- (void)setState:(MDDownloadItemProgressIndicatorState)state
  ...
  if (targetState_ != state) {
    ...
    if (targetState_ == MDDownloadItemProgressIndicatorState::kInProgress) {
      [self enqueue:Animation::Type::kUpdateState block:animations];
      [self enqueue:Animation::Type::kUpdateProgress block:nil];
    } else {
      [self enqueue:Animation::Type::kUpdateProgress block:nil];
      [self enqueue:Animation::Type::kUpdateState block:animations];
    }
    ...
    progress_ = progress;
    [self enqueue:Animation::Type::kUpdateProgress block:animations];
  } else if (animations) {
    animations();
  }

  if (completion)
    [self enqueue:Animation::Type::kBlockOnly block:completion];
}
