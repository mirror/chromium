...
- (void)enqueue:(void (^)())animation {
  ...
  // yet and none of the sublayer ivars are set. -setLayer: kicks things off.
  if ([animations_ count] == 1 && self.layer)
    [self runAnimations];
}

// This utility method chains an extra block onto the passed block, but only if
// the extra block is non-nil. It's used in setState:… below.
