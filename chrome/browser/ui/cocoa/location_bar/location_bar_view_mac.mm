...
void LocationBarViewMac::UpdateLocationIcon() {
  ...
  const gfx::VectorIcon& vector_icon_id =
      GetPageInfoVerboseType() == PageInfoVerboseType::kEVCertVerbose
...
PageInfoVerboseType LocationBarViewMac::GetPageInfoVerboseType() const {
  if (omnibox_view_->IsEditingOrEmpty() ||
    ...
    return PageInfoVerboseType::kNoVerbose;
  } else if (GetToolbarModel()->GetSecurityLevel(false) ==
             security_state::EV_SECURE) {
    return PageInfoVerboseType::kEVCertVerbose;
  } else if (GetToolbarModel()->GetURL().SchemeIs(
                 extensions::kExtensionScheme)) {
    return PageInfoVerboseType::kExtensionVerbose;
  } else if (GetToolbarModel()->GetURL().SchemeIs(content::kChromeUIScheme)) {
    return PageInfoVerboseType::kChromeVerbose;
  } else {
    return PageInfoVerboseType::kSecurityVerbose;
  }
}

bool LocationBarViewMac::HasSecurityVerboseText() const {
  if (GetPageInfoVerboseType() != PageInfoVerboseType::kSecurityVerbose)
    return false;
...
SkColor LocationBarViewMac::GetLocationBarIconColor() const {
  ...
  if (GetPageInfoVerboseType() == PageInfoVerboseType::kEVCertVerbose)
...
void LocationBarViewMac::UpdatePageInfoText() {
  ...
  if (type == PageInfoVerboseType::kEVCertVerbose) {
    label = GetToolbarModel()->GetEVCertName();
  } else if (type == PageInfoVerboseType::kExtensionVerbose) {
    label = GetExtensionName(GetToolbarModel()->GetURL(), GetWebContents());
  } else if (type == PageInfoVerboseType::kChromeVerbose) {
    label = l10n_util::GetStringUTF16(IDS_SHORT_PRODUCT_NAME);
  } else if (type == PageInfoVerboseType::kSecurityVerbose &&
