...
bool LocationBarDecoration::HandleMousePressed(NSRect frame, NSPoint location) {
  if (was_active_in_last_mouse_down_) {
    ...
    if (AcceptsMousePress() == AcceptsPress::WHEN_ACTIVATED) {
      ...
      // unable to clear the active state. In that
      // case, we risk getting "stuck" in a state where client code may never
      // receive a signal. Clearing the active state here ensures a subsequent
      // click will always send a signal.
      SetActive(false);
      return true;
    }
  }
