    ...
    if (isAnswer_) {
      ...
      if (match.type == AutocompleteMatchType::SEARCH_SUGGEST_TAIL) {
        ...
        for (const auto& classification : match.contents_class) {
          contents_class.push_back(
              ACMatchClassification(commonPrefix.size() + classification.offset,
                                    classification.style));
        }
        contents_ = [CreateClassifiedAttributedString(
            commonPrefix + match.contents, ContentTextColor(isDarkTheme),
            contents_class, isDarkTheme) retain];
      } else {
        ACMatchClassifications contents_class;
        if (anyTailSuggestions &&
            base::StartsWith(match.contents, commonPrefix,
          ...
          while (i < match.contents_class.size() &&
                 contents_class[i].offset < commonPrefix.size())
            ++i;
          if (i > 0) {
            contents_class.push_back(ACMatchClassification(
                match.contents_class[i - 1].offset + commonPrefix.size(),
