...
- (CGFloat)_minXTitlebarWidgetInset {
  return self.hasTabStrip ? kFramedWindowButtonsWithTabStripOffsetFromLeft
                          : [super _minXTitlebarWidgetInset];
}

- (CGFloat)_minYTitlebarButtonsOffset {
  return self.hasTabStrip ? -kFramedWindowButtonsWithTabStripOffsetFromBottom
                          : [super _minYTitlebarButtonsOffset];
}

- (CGFloat)_titlebarHeight {
  return self.hasTabStrip ? chrome::kTabStripHeight : [super _titlebarHeight];
}

// The AppKit implementation is the same but also checks that [self class] ==
// [NSThemeFrame class]. Yup. FramedBrowserWindowFrame could override -class,
// but that makes -[NSWindow setStyleMask:] recreate the frame view each time
// the style mask is touched because its class doesn't match what
// +frameViewClassForStyleMask: returns.
- (BOOL)_shouldFlipTrafficLightsForRTL {
  return self.window.windowTitlebarLayoutDirection ==
         NSUserInterfaceLayoutDirectionRightToLeft;
}

// By default, the contentView does not occupy the full size of a titled
// window, and Chrome to draw in the title bar. Historically, Chrome did this
// by adding subviews directly to the root view. This causes several problems.
// The most egregious is related to layer ordering when the root view does not
// have a layer. By giving the contentView the same size as the window, there
// is no longer a need to add subviews to the root view. On newer versions of
// macOS, Chrome should use NSFullSizeContentWindowMask (formerly
// NSFullSizeContentWindowMask) instead. See https://crbug.com/605219.
...
// Starting in Yosemite, the zoom button is also the fullscreen button.
- (NSPoint)_fullScreenButtonOrigin {
  if (self.hasTabStrip) {
    ...
    return NSMakePoint(NSMaxX(self.bounds) - fullScreenButtonSize.width -
                           kFramedWindowFullScreenButtonWithTabStripOffset -
                           xAdjustment,
...
+ (Class)frameViewClassForStyleMask:(NSUInteger)windowStyle {
  if ([FramedBrowserWindowFrame class]) {
