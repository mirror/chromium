...
- (NSTouchBar*)makeTouchBar {
  ...
  [touchBar setCustomizationIdentifier:ui::CreateTouchBarId(
                                           kBookmarkBubbleTouchBarId)];
  [touchBar setDelegate:self];

  NSArray* dialogItems = @[
    ui::CreateTouchBarItemId(kBookmarkBubbleTouchBarId, kRemoveTouchBarId),
    ui::CreateTouchBarItemId(kBookmarkBubbleTouchBarId, kEditTouchBarId),
    ui::CreateTouchBarItemId(kBookmarkBubbleTouchBarId, kDoneTouchBarId)
  ];
...
- (NSTouchBarItem*)touchBar:(NSTouchBar*)touchBar
  ...
  if ([identifier hasSuffix:kRemoveTouchBarId]) {
    ...
    button = ui::CreateBlueTouchBarButton(l10n_util::GetNSString(IDS_DONE),
                                          self, @selector(ok:));
  } else {
    return nil;
  }
