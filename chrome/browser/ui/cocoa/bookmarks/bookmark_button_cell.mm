...
- (NSRect)titleRectForBounds:(NSRect)theRect {
  ...
  CGFloat iconTrailingX =
      cocoa_l10n_util::TrailingX([self imageRectForBounds:theRect]);
  textRect.origin.x = iconTrailingX + kIconTextSpacer;
  textRect.size.width =
      NSWidth(theRect) - NSMinX(textRect) - kTrailingPadding -
      (drawFolderArrow_
           ? ([arrowImage_ size].width + kHierarchyButtonTrailingPadding)
           : 0);
  return cocoa_l10n_util::RectMaybeFlippedInRect(textRect, theRect);
}
