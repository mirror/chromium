...
- (NSTouchBar*)makeTouchBar {
  ...
  NSArray* dialogItems = @[
    ui::CreateTouchBarItemId(kBookmarkEditDialogTouchBarId,
                             kNewFolderTouchBarId),
    ui::CreateTouchBarItemId(kBookmarkEditDialogTouchBarId, kCancelTouchBarId),
    ui::CreateTouchBarItemId(kBookmarkEditDialogTouchBarId, kSaveTouchBarId)
  ];
...
- (NSTouchBarItem*)touchBar:(NSTouchBar*)touchBar
  ...
  if ([identifier hasSuffix:kNewFolderTouchBarId]) {
    ...
    button = ui::CreateBlueTouchBarButton(l10n_util::GetNSString(IDS_SAVE),
                                          self, @selector(ok:));
  } else {
    return nil;
  }
