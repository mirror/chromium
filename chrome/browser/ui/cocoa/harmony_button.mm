...
NSColor* GetBackgroundColor(HoverState state, BOOL darkTheme) {
  if (darkTheme) {
    switch (state) {
...
NSColor* GetBorderColor(BOOL darkTheme) {
  return [NSColor colorWithCalibratedWhite:darkTheme ? 1 : 0 alpha:0.2];
}

const CGColorRef kShadowColor = CGColorCreateGenericGray(0, 1);
