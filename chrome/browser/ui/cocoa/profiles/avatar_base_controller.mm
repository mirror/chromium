...
- (void)showAvatarBubbleAnchoredAt:(NSView*)anchor
  ...
  if (SigninViewController::ShouldShowModalSigninForMode(bubbleViewMode)) {
    browser_->signin_view_controller()->ShowModalSignin(bubbleViewMode,
    ...
    gfx::Rect anchorRect = gfx::ScreenRectFromNSRect(rect);
    // Adjusting the offset by reducing x and width for RTL and LTF.
    anchorRect.set_x(anchorRect.x() + kMenuXOffsetAdjust);
    anchorRect.set_width(anchorRect.width() - 2 * kMenuXOffsetAdjust);
    anchorRect.set_y(anchorRect.y() + kMenuYOffsetAdjust);
    gfx::NativeView anchorWindow =
        platform_util::GetViewForWindow(browser_->window()->GetNativeWindow());
    ProfileChooserView::ShowBubble(
        bubbleViewMode, signin::ManageAccountsParams(), accessPoint, nullptr,
        anchorWindow, anchorRect, browser_, false);
