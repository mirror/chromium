...
class UserManagerMacTest : public BrowserWithTestWindowTest {
 public:
  ...
  void SetUp() override {
    ...
    // Make sure there is already a user so the System Profile isn't the only
    // one since it shouldn't be added to the ProfileAttributesStorage.
    EXPECT_EQ(
        1u,
  ...
  void TearDown() override {
    profile_manager()->DeleteSystemProfile();
