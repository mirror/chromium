...
class HungRendererObserverBridge : public content::WebContentsObserver,
 ...
 public:
  HungRendererObserverBridge(WebContents* web_contents,
    ...
    hung_process->AddObserver(this);
  }

  ~HungRendererObserverBridge() override {
    hung_process_->RemoveObserver(this);
  }
...
- (void)showForWebContents:(WebContents*)contents
  ...
  std::vector<content::WebContents*> affectedContentses =
      hung_renderer_dialog::GetAffectedWebContentsList(
          contents, renderWidget->GetProcess());

  for (auto* affectedContents : affectedContentses) {
    [titles addObject:base::SysUTF16ToNSString(
                          hung_renderer_dialog::GetAffectedWebContentsTitle(
                              affectedContents, renderWidget->GetProcess()))];
    [favicons addObject:mac::FaviconForWebContents(affectedContents)];
  }
