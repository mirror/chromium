...
- (void)layoutSecuritySection {
  ...
  if (changePasswordButton_) {
    NSPoint changePasswordButtonOrigin =
        NSMakePoint(kSectionHorizontalPadding - kNSButtonBuiltinMargin,
                    yPos + kSecurityParagraphSpacing);
    NSPoint whitelistReuseButtonOrigin = NSMakePoint(
        changePasswordButtonOrigin.x + NSWidth([changePasswordButton_ frame]) +
            kNSButtonBuiltinMargin,
        yPos + kSecurityParagraphSpacing);
    if (base::i18n::IsRTL()) {
      changePasswordButtonOrigin.x = NSWidth([contentView_ frame]) -
