...
class BrowserWindowTouchBarTest : public InProcessBrowserTest {
 public:
  ...
  void SetUpOnMainThread() override {
    touch_bar_ = [[TestingBrowserWindowTouchBar alloc]
                initWithBrowser:browser()
        browserWindowController:browser_window_controller()];
    [browser_window_controller() setBrowserWindowTouchBar:touch_bar_];
  }
  ...
  TestingBrowserWindowTouchBar* touch_bar() const { return touch_bar_; }

 private:
  TestingBrowserWindowTouchBar* touch_bar_;

  DISALLOW_COPY_AND_ASSIGN(BrowserWindowTouchBarTest);
};

// Test if the proper controls gets updated when the page load.
IN_PROC_BROWSER_TEST_F(BrowserWindowTouchBarTest, PageLoadInvalidate) {
  if (@available(macOS 10.12.2, *)) {
    NSButton* reload_stop = [touch_bar() reloadStopButton];
    EXPECT_TRUE(reload_stop);

    BrowserWindowController* bwc = browser_window_controller();
    [bwc setIsLoading:YES force:NO];
    EXPECT_TRUE([touch_bar() hasUpdatedReloadStop]);
    EXPECT_TRUE([touch_bar() hasUpdatedBackForward]);
    EXPECT_TRUE([touch_bar() hasUpdatedStarred]);
    EXPECT_EQ(IDC_STOP, [reload_stop tag]);

    [touch_bar() resetFlags];
...
IN_PROC_BROWSER_TEST_F(BrowserWindowTouchBarTest, TabChanges) {
  if (@available(macOS 10.12.2, *)) {
    ...
    EXPECT_NE(touchBar, [window touchBar]);
  }
}

// Tests to see if the star button changes when the
// Test if the touch bar gets invalidated when the tab is changed.
IN_PROC_BROWSER_TEST_F(BrowserWindowTouchBarTest, StarredChanges) {}

// Tests to see if the home button is updated when the pref changes
