...
- (void)updateIconsForContents:(content::WebContents*)contents
  ...
  if (tabUIHelper->ShouldUseFaviconFromHistory()) {
    [tabController setIconImage:tabUIHelper->GetFavicon().AsNSImage()];
  } else if (tabUIHelper->ShouldUseFaviconFromWebContents()) {
    [tabController
        setIconImage:favicon::TabFaviconFromWebContents(contents).AsNSImage()];
