...
@interface TabController () {
  ...
  int currentAttentionTypes_;
}

@property(nonatomic) int currentAttentionTypes;
...
- (NSMenu*)menu {
  contextMenuDelegate_.reset(new MenuDelegate([self target], self));
  contextMenuModel_.reset([[self target]
      contextMenuModelForController:self
...
- (void)maybeStartDrag:(NSEvent*)event forTab:(TabController*)tab {
  [[[self target] dragController] maybeStartDrag:event forTab:tab];
}
