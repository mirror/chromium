...
- (void)maybeStartDrag:(NSEvent*)theEvent forTab:(TabController*)tab {
  ...
  sourceController_ = [sourceWindow_ browserWindowController];
  draggedTab_ = tab;
  tabWasDragged_ = NO;
  tearTime_ = 0.0;
...
- (BOOL)canDragSelectedTabs {
  ...
  TabWindowController* controller = [sourceWindow_ tabWindowController];
  if (controller) {
    for (TabView* tabView in tabs) {
...
- (NSArray*)dropTargetsForController:(TabWindowController*)dragController {
  ...
  for (NSWindow* window in [NSApp orderedWindows]) {
    ...
    if (![window isOnActiveSpace])
      continue;
    TabWindowController* controller = [window tabWindowController];
