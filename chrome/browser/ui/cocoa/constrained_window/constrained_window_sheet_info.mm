...
- (void)showSheet {
  ...
  // (rdar://35418050) which was last verified to exist in 10.13.1: If a window
  // (A) has a child window (B), and B has a sheet (C), then adding another
  // child window (D) to A causes the sheet (C) to move behind its parent (B).
  //
  ...
  DCHECK(overlayWindow_.get().attachedSheet);
  if (NSWindow* sheet = overlayWindow_.get().attachedSheet)
