...
TEST_F(AppToolbarButtonTest, NewAppToolbarIcon_NoDelay) {
  ...
  // The animationDelayTimer should always be nil since "has_delay"
  // param of the feature is false.
...
TEST_F(AppToolbarButtonTest, NewAppToolbarIcon_HasDelay) {
  ...
  // Icon should try to animate for the other severity levels. Since
  // the "has_delay" param of the feature is true, the icon shouldn't
  // animate. Instead, the animation delay timer should be set.
  SetSeverityLevel(AppMenuIconController::Severity::LOW);
  ...
  // Calling [animateIfPossibleWithDelay:NO] should re-animate the
  // icon without delay if the icon is already animating.
  [button_ animateIfPossibleWithDelay:NO];
  EXPECT_FALSE([button_ animationDelayTimer]);
  EXPECT_TRUE(test_icon->is_animating());
}
