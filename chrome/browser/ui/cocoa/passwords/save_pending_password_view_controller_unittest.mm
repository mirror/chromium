...
TEST_F(SavePendingPasswordViewControllerTest,
  ...
  EXPECT_TRUE([controller().editButton isEnabled]);
  EXPECT_TRUE(
      [[[row usernameField] stringValue] isEqualToString:@"newusername"]);
...
TEST_F(SavePendingPasswordViewControllerTest,
  ...
  EXPECT_FALSE([[row usernameField] isSelectable]);
  EXPECT_TRUE([controller().editButton isEnabled]);
  EXPECT_TRUE([[[row usernameField] stringValue]
      isEqualToString:base::SysUTF16ToNSString([delegate() model]
                                                   ->pending_password()
                                                   .username_value)]);
