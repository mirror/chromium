...
- (void)onEyeClicked:(id)sender {
  ...
  if (visible && !self.model->CanRevealPasswords())
    return;
  ...
  if (form.federation_origin.unique()) {
    if (enablePasswordEditing) {
      if (form.all_possible_passwords.size() > 1) {
        ...
        FillPasswordPopup(form,
                          self.model->is_password_revealed_when_bubble_opened(),
                          passwordSelectionField_.get());
        [passwordSelectionField_ sizeToFit];
      } else {
        if (self.model->is_password_revealed_when_bubble_opened()) {
