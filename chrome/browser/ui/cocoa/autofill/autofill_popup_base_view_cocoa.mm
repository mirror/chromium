...
// vertical space (see getClippedPopupFrame).
- (NSRect)getFullPopupFrame {
...
// space.
- (NSRect)getClippedPopupFrame {
  NSRect clippedPopupFrame = [self getFullPopupFrame];
  ...
  if (clippedPopupFrame.origin.y < appWindowBottomEdge) {
    ...
    CGFloat dY = NSMaxY([appWindow frame]) - NSMaxY([self getFullPopupFrame]);
...
- (void)updateBoundsAndRedrawPopup {
  [[[self superview] window] setFrame:[self getClippedPopupFrame] display:YES];

  [self setNeedsDisplay:YES];
}

- (void)showPopup {
  NSRect clippedPopupFrame = [self getClippedPopupFrame];
  ...
  [self setFrame:[self getFullPopupFrame]];
