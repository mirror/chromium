...
- (BOOL)shouldShowManageButton {
  return [[self manageButtonTitle] length] != 0;
}
...
- (BOOL)shouldShowCustomLink {
  return [[self customLinkTitle] length] != 0;
}
...
- (NSView*)makePluginsBox:(NSRect)frame {
  ...
  for (NSString* name : [self pluginNames]) {
...
- (void)loadView {
  ...
  if ([self shouldShowManageButton]) {
    NSButton* manageButton =
        [ButtonUtils buttonWithTitle:[self manageButtonTitle]
  ...
  if ([self shouldShowCustomLink]) {
    NSButton* customLink = [ButtonUtils linkWithTitle:[self customLinkTitle]
