...
TEST_F(StatusBubbleMacTest, WaitsForHiddenWindows) {
  test_window().visible = NO;
  bubble_->SetStatus(UTF8ToUTF16("Show soon"));
  EXPECT_FALSE(GetWindow().parentWindow);

  test_window().isOnActiveSpace = NO;
  test_window().visible = YES;
  EXPECT_FALSE(GetWindow().parentWindow);

  test_window().isOnActiveSpace = YES;
  EXPECT_TRUE(GetWindow().parentWindow);
}
