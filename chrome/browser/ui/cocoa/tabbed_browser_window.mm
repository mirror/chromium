...
#include "chrome/browser/ui/cocoa/tabbed_browser_window.h"
...
const NSInteger kWindowButtonsOffsetFromBottom = 9;
const NSInteger kWindowButtonsOffsetFromLeft = 11;

// Offset from the top/right of the window to the Mavericks full screen button.
const NSInteger kFullScreenButtonOffset = 9;
}
...
- (void)_tileTitlebarAndRedisplay:(BOOL)display;
...
//
// TODO(sdy): On 10.10+, Chrome should use NSWindowStyleMaskFullSizeContentView
// instead of overriding these methods. See https://crbug.com/605219.
...
// NSWindow's behavior changes undesirably unless the frame view class is on a
// whitelist. See -[NSWindow _usesCustomDrawing].
...
@interface MavericksTabbedBrowserWindowFrame : TabbedBrowserWindowFrame
@end
...
+ (Class)frameViewClassForStyleMask:(NSUInteger)windowStyle {
  if ([TabbedBrowserWindowFrame class]) {
    if (@available(macos 10.10, *)) {
      return [TabbedBrowserWindowFrame class];
    } else {
