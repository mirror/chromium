...
IN_PROC_BROWSER_TEST_F(BrowserWindowControllerTest, FullscreenHistogram) {
  ...
  prefs->SetBoolean(prefs::kShowFullscreenToolbar, true);
  [[controller() fullscreenToolbarController] updateAndLayoutToolbarStyle:NO];
  histogram_tester.ExpectBucketCount(
      "OSX.Fullscreen.ToolbarStyle",
  ...
  EnterTabFullscreenAndWaitForNotification();
  histogram_tester.ExpectBucketCount("OSX.Fullscreen.Enter.Source", TAB, 1);
  ...
  histogram_tester.ExpectBucketCount("OSX.Fullscreen.Enter.Source", EXTENSION,
                                     1);
