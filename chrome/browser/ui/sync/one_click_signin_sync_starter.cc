...
void OneClickSigninSyncStarter::CopyCredentialsToNewProfileAndFinishSignin(
  ...
  // SignoutAndRemoveAllAccounts does not actually removes the accounts. See
  ...
  if (signin::IsDicePrepareMigrationEnabled()) {
    std::string account_id =
        signin_manager->IsAuthenticated()
            ...
            : signin_manager->GetAccountIdForAuthInProgress();
    ...
    // SignoutAndRemoveAllAccounts does not actually removes the accounts if the
