...
class LIBGTKUI_EXPORT NavButtonProviderGtk3 : public views::NavButtonProvider {
 ...
 private:
  // State used to determine if RedrawImages() needs to do any work.
  int top_area_height_;
  int maximized_;

  // Cached images and associated layout information.
  std::map<chrome::FrameButtonDisplayType,
           gfx::ImageSkia[views::CustomButton::STATE_COUNT]>
      button_images_;
