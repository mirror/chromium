...
NavButtonProviderGtk3::NavButtonProviderGtk3()
    : top_area_height_(-1), maximized_(false) {}

NavButtonProviderGtk3::~NavButtonProviderGtk3() {}

void NavButtonProviderGtk3::RedrawImages(int top_area_height, bool maximized) {
  if (top_area_height == top_area_height_ && maximized == maximized_)
    return;
  top_area_height_ = top_area_height;
  ...
  for (auto type : display_types) {
    ...
    for (size_t state = 0; state < views::CustomButton::STATE_COUNT; state++) {
      button_images_[type][state] = RenderNavButton(
          type, static_cast<views::CustomButton::ButtonState>(state), size);
    }
  }
}
