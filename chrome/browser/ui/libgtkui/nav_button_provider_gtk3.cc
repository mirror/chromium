...
const char* ButtonStyleClassFromButtonType(views::FrameButtonDisplayType type) {
  switch (type) {
...
const char* IconNameFromButtonType(views::FrameButtonDisplayType type) {
  switch (type) {
    case views::FRAME_BUTTON_DISPLAY_MINIMIZE:
...
int DefalutMonitorScaleFactor() {
  GdkScreen* screen = gdk_screen_get_default();
  return gdk_screen_get_monitor_scale_factor(
      screen, gdk_screen_get_primary_monitor(screen));
}

ScopedGObject<GdkPixbuf> LoadNavButtonIcon(views::FrameButtonDisplayType type,
                                           GtkStyleContext* button_context) {
  const int monitor_scale = DefalutMonitorScaleFactor();
...
void CalculateUnscaledButtonSize(views::FrameButtonDisplayType type,
...
void NavButtonProviderGtk3::RedrawImages(int top_area_height) {
  ...
  for (size_t type = 0; type < views::FRAME_BUTTON_DISPLAY_COUNT; type++) {
    CalculateUnscaledButtonSize(
        static_cast<views::FrameButtonDisplayType>(type), &button_sizes[type],
        &button_margins[type]);
