...
// TODO(thomasanderson): Add a button image cache so we don't have to
// redraw the button on every relayout.
gfx::ImageSkia NavButtonProviderGtk3::GetNavButtonImage(
  ...
  inter_button_spacing_ = 6;  // Specified in GtkHeader spec.
  ...
  if (button_unconstrained_height <= available_height) {
    ...
    inter_button_spacing_ = scale * inter_button_spacing_ + 0.5f;

    button_total_width = static_cast<int>(scale * button_total_width + 0.5f);
  }

  SkBitmap bitmap;
  bitmap.allocN32Pixels(button_rect.width(), button_rect.height());
  bitmap.eraseColor(0);
  ...
  if (GtkVersionCheck(3, 11, 3) ||
    ...
    gtk_render_background(button_context, cr, 0, 0, button_rect.width(),
                          button_rect.height());
    gtk_render_frame(button_context, cr, 0, 0, button_rect.width(),
                     button_rect.height());
  }
  cairo_save(cr);
  ...
  gtk_render_icon(
      ...
      icon_scale * ((button_rect.width() - icon_size.width()) / 2),
      icon_scale * ((button_rect.height() - icon_size.height()) / 2));
  cairo_restore(cr);

  button_margins_[type] =
      gfx::Insets(button_rect.y(), button_rect.x(), 0,
                  button_total_width - button_rect.width() - button_rect.x());
