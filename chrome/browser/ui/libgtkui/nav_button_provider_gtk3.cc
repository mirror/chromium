...
ScopedGObject<GdkPixbuf> LoadNavButtonIcon(chrome::FrameButtonDisplayType type,
                                           ...
                                           int scale) {
  if (scale != static_cast<int>(scale))
    scale += 1;
...
class NavButtonImageSource : public gfx::ImageSkiaSource {
 public:
  ...
  gfx::ImageSkiaRep GetImageForScale(float scale) override {
    ...
    // are not actually renderd until they are needed.
    return RenderNavButton(type_, state_, button_size_, scale);
  }

  bool HasRepresentationAtAllScales() const override { return true; }

 private:
  gfx::ImageSkiaRep RenderNavButton(chrome::FrameButtonDisplayType type,
                                    views::Button::ButtonState state,
                                    gfx::Size button_size,
                                    float scale) {
    auto button_context = GetStyleContextFromCss(
    ...
    ScopedGObject<GdkPixbuf> icon_pixbuf =
        LoadNavButtonIcon(type, button_context, pixbuf_scale);

    gfx::Size icon_size(gdk_pixbuf_get_width(icon_pixbuf),
                        gdk_pixbuf_get_height(icon_pixbuf));

    SkBitmap bitmap;
    bitmap.allocN32Pixels(scale * button_size.width(),
                          scale * button_size.height());
    bitmap.eraseColor(0);
    ...
    gtk_render_icon(
        ...
        ((pixbuf_scale * button_size.width() - icon_size.width()) / 2),
        ((pixbuf_scale * button_size.height() - icon_size.height()) / 2));
    cairo_restore(cr);
