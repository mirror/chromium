...
void NavButtonProviderGtk3::CalculateCaptionButtonLayout(
  ...
  if (GtkVersionCheck(3, 20)) {
    ...
    additional_button_width =
        std::max(content_size.width(), min_width) - content_size.width();
    additional_button_height =
        std::max(content_size.height(), min_height) - content_size.height();
  }

  gfx::Insets scalable_insets = header_padding + header_border +
  ...
  float scaled_button_height =
      content_size.height() +
      scale * (additional_button_height + button_border.top() +
               button_border.bottom() + button_padding.top() +
               button_padding.bottom());
  float scaled_button_height_with_margin =
      scaled_button_height +
      scale * (button_margin.top() + button_margin.bottom());
  float shiftable_region_height =
      top_area_height -
      scale * (header_border.top() + header_border.bottom() +
               header_padding.top() + header_padding.bottom());
  float button_offset_in_shiftable_region =
      (shiftable_region_height - scaled_button_height_with_margin) / 2;

  *caption_button_size =
      gfx::Size(content_size.width() +
                    scale * (additional_button_width + button_border.left() +
                             button_border.right() + button_padding.left() +
                             button_padding.right()) +
                    0.5f,
                scaled_button_height + 0.5f);
  *caption_button_spacing =
      gfx::Insets(scale * (header_border.top() + header_padding.top() +
                           button_margin.top()) +
                      button_offset_in_shiftable_region + 0.5f,
                  scale * button_margin.left() + 0.5f, 0,
                  scale * button_margin.right() + 0.5f);
}
