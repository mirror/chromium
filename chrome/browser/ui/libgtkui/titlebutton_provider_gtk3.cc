...
gfx::ImageSkia GetTitlebuttonImage(int type,
  ...
  gtk_style_context_get_padding(header_context, GTK_STATE_FLAG_NORMAL,
                                &padding);
  GtkBorder border;
  gtk_style_context_get_border(header_context, GTK_STATE_FLAG_NORMAL, &border);

  int available_height = header_height - (padding.top + padding.bottom +
                                          border.top + border.bottom);

  int render_x = 0;
  int render_y = 0;
  int ret_width = 0;
  if (button_total_height <= available_height) {
    render_x += button_margin.left;
    render_y += border.top + padding.top + button_margin.top +
                (available_height - button_total_height) / 2;
    ret_width = button_width + button_margin.left + button_margin.right;
  } else {
    int needed_height = border.top + padding.top + button_total_height +
                        padding.bottom + border.bottom;
    float scale = ((float)header_height) / needed_height;
    render_x += scale * button_margin.left;
    render_y += scale * (border.top + padding.top + button_margin.top);
    ret_width =
        scale * (button_width + button_margin.left + button_margin.right);
    button_width *= scale;
    button_height *= scale;
  }

  SkBitmap bitmap;
  bitmap.allocN32Pixels(button_width, button_height);
  bitmap.eraseColor(0);
  ...
  if (GtkVersionCheck(3, 11, 3) ||
    ...
    gtk_render_background(button_context, cr, 0, 0, button_width,
                          button_height);
  ...
  gtk_render_icon(button_context, cr, icon_pixbuf,
                  icon_scale * (0 + (button_width - icon_width) / 2),
                  icon_scale * (0 + (button_height - icon_height) / 2));
  cairo_restore(cr);
