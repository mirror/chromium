...
NavButtonLayoutManagerGSettings::NavButtonLayoutManagerGSettings(
  ...
  if (!SchemaExists(kGeneralPreferencesSchema) ||
  ...
  OnDecorationButtonLayoutChanged(settings_, kButtonLayoutKey, nullptr);
  signal_button_id_ =
      g_signal_connect(settings_, kButtonLayoutChangedSignal,
                       G_CALLBACK(OnDecorationButtonLayoutChangedThunk), this);

  OnMiddleClickActionChanged(settings_, kMiddleClickActionKey, nullptr);
