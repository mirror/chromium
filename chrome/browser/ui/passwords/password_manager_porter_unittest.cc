...
class TestPasswordStoreInterface : public PasswordStoreInterface {
 public:
  TestPasswordStoreInterface() {}
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(TestPasswordStoreInterface);
};

class TestPasswordManagerPorter : public PasswordManagerPorter {
 public:
  TestPasswordManagerPorter(
      TestPasswordStoreInterface* password_manager_presenter)
...
class PasswordManagerPorterTest : public testing::Test {
 protected:
  ...
  void SetUp() override {
    password_store_interface_.reset(new TestPasswordStoreInterface());
    password_manager_porter_.reset(
        new TestPasswordManagerPorter(password_store_interface_.get()));
  ...
  void TearDown() override {
    ...
    password_store_interface_ = nullptr;
  }

  void ImportPasswords() {
    EXPECT_CALL(*password_manager_porter_, ImportPasswordsFromPath(_));
  ...
  void ExportPasswords() {
    EXPECT_CALL(*password_manager_porter_, ExportPasswordsToPath(_));

    password_manager_porter_->PresentFileSelector(
        web_contents_.get(), PasswordManagerPorter::Type::PASSWORD_EXPORT);
  }

  std::unique_ptr<TestPasswordStoreInterface> password_store_interface_;
