...
class PasswordManagerPorterTest : public testing::Test {
 protected:
  ...
  TestPasswordManagerPorter* GetPasswordManagerPorter() {
...
TEST_F(PasswordManagerPorterTest, PasswordImport) {
  EXPECT_CALL(*GetPasswordManagerPorter(), ImportPasswordsFromPath(_));

  GetPasswordManagerPorter()->PresentFileSelector(
      GetWebContents(), PasswordManagerPorter::Type::PASSWORD_IMPORT);
}

TEST_F(PasswordManagerPorterTest, PasswordExport) {
  EXPECT_CALL(*GetPasswordManagerPorter(), ExportPasswordsToPath(_));

  GetPasswordManagerPorter()->PresentFileSelector(
      GetWebContents(), PasswordManagerPorter::Type::PASSWORD_EXPORT);
}
