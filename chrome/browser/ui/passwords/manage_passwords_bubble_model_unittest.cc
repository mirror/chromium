...
class ManagePasswordsBubbleModelTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    EXPECT_CALL(*mock_delegate_, GetPasswordFormMetricsRecorder())
...
TEST_F(ManagePasswordsBubbleModelTest, RecordUKMs) {
  ...
  ukm::TestUkmRecorder test_ukm_recorder;
  {
    ...
    // Exercise bubble.
    EXPECT_CALL(*controller(), GetPasswordFormMetricsRecorder())
    ...
    // Reset mocks because PretendPasswordWaiting() resets expectations via
    // SetUpWithState().
    EXPECT_CALL(*controller(), GetPasswordFormMetricsRecorder())
        .WillRepeatedly(Return(recorder.get()));
    EXPECT_CALL(*controller(), GetCredentialSource())
