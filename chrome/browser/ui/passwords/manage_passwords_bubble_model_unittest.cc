...
TEST_F(ManagePasswordsBubbleModelTest, Edit) {
  ...
  for (const bool do_change : {false, true}) {
    ukm::TestUkmRecorder test_ukm_recorder;
    {
      ...
      EXPECT_CALL(*controller(), SavePassword(kExpectedPassword));

      model()->OnUsernameEdited(kExpectedPassword);
      EXPECT_EQ(kExpectedPassword, model()->pending_password().username_value);
