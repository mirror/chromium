...
bool ManagePasswordsUIController::ShouldResetDataAfterNavigation(
  ...
  // Keep the bubble if the user was not able to interact with the password
  // bubble or the fallback for saving should be still available.
...
void ManagePasswordsUIController::DidFinishNavigation(
  ...
  if (ShouldResetDataAfterNavigation(navigation_handle))
    return;
