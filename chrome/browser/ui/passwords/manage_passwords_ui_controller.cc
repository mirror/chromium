...
void ManagePasswordsUIController::SavePassword(const base::string16& username,
  ...
  bool username_edited =
      passwords_data_.form_manager()->pending_credentials().username_value !=
      username;
  bool password_changed =
      passwords_data_.form_manager()->pending_credentials().password_value !=
      password;
  if (username_edited) {
  ...
  UMA_HISTOGRAM_ENUMERATION(
      "PasswordManager.EditsInSaveBubble",
      (username_edited ? 1 : 0) | (password_changed ? 2 : 0), 4);
