...
bool ManagePasswordsUIController::AuthenticateUser() {
  ...
  base::SequencedTaskRunnerHandle::Get()->PostTask(
      FROM_HERE,
      base::BindOnce(
          &ManagePasswordsUIController::RequestAuthenticationAndReopensBubble,
...
#elif defined(OS_MACOSX)
  return authenticated = password_manager_util_mac::AuthenticateUser();
#else
  NOTREACHED();
  return true;
#endif
...
void ManagePasswordsUIController::RequestAuthenticationAndReopensBubble() {
  base::WeakPtr<ManagePasswordsUIController> weak_ptr =
      weak_ptr_factory_.GetWeakPtr();
  bool auth_is_successful = ShowAuthenticationDialog();
  base::SequencedTaskRunnerHandle::Get()->PostTask(
      FROM_HERE,
      base::BindOnce(&ManagePasswordsUIController::ReopensBubbleAfterAuth,
                     weak_ptr, auth_is_successful));
}

void ManagePasswordsUIController::ReopensBubbleAfterAuth(
