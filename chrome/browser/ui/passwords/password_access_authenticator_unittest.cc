...
// Check that a passed authentication does not expire before 60 seconds and does
...
TEST(PasswordAccessAuthenticatorTest, Expiration) {
  ...
  authenticator.set_clock_for_testing(std::move(clock_owned));

  EXPECT_TRUE(authenticator.EnsureUserIsAuthenticated());
  EXPECT_TRUE(reauth_called);

  clock->Advance(base::TimeDelta::FromSeconds(59));
  reauth_called = false;
...
// Test the edge case: for the first authentication, the reauth challenge should
// always happen, even if the time is less than 60 seconds from the beginning.
TEST(PasswordAccessAuthenticatorTest, TimeZero) {
  ...
  PasswordAccessAuthenticator authenticator(base::BindRepeating(
      &FakeOsReauthCall, &reauth_called, ReauthResult::PASS));
  authenticator.set_clock_for_testing(std::move(clock));
