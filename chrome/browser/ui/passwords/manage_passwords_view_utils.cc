...
void GetManagePasswordsDialogTitleText(const GURL& user_visible_url,
  ...
  if (!SameDomainOrHost(user_visible_url, password_origin_url)) {
    ...
    *title = l10n_util::GetStringFUTF16(
        has_credentials ? IDS_MANAGE_PASSWORDS_TITLE_DIFFERENT_DOMAIN
                        : IDS_MANAGE_PASSWORDS_TITLE_DIFFERENT_DOMAIN_EMPTY,
        formatted_url);
  } else {
    *title = l10n_util::GetStringUTF16(has_credentials
                                           ? IDS_MANAGE_PASSWORDS_TITLE
                                           : IDS_MANAGE_PASSWORDS_TITLE_EMPTY);
  }
}
