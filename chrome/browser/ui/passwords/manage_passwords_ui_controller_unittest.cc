...
TEST_F(ManagePasswordsUIControllerTest, AuthenticateUser) {
  std::unique_ptr<password_manager::PasswordFormManager> test_form_manager(
      CreateFormManager());
  test_form_manager->ProvisionallySave(
      test_local_form(),
  ...
  // Let the task posted in AuthenticateUser to re-open the bubble.
  EXPECT_CALL(*controller(), OnUpdateBubbleAndIconVisibility());
  content::RunAllPendingInMessageLoop();
  EXPECT_TRUE(controller()->opened_bubble());
  EXPECT_TRUE(controller()->ArePasswordsRevealedWhenBubbleIsOpened());
