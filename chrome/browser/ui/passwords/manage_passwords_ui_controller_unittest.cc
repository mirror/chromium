...
TEST_F(ManagePasswordsUIControllerTest, PasswordSavedUKMRecording) {
  ...
  for (const auto& test : kTests) {
    ...
    {  // Nested scope to destroy |recorder| before verification.
      ...
      // state is retained the navigation, and the PasswordFormManager is not
      // destroyed.
      EXPECT_CALL(*controller(), OnUpdateBubbleAndIconVisibility());
...
TEST_F(ManagePasswordsUIControllerTest, ManualFallbackForSaving_UseFallback) {
  ...
  for (bool is_update : {false, true}) {
    ...
    if (is_update) {
      ...
      test_ukm_recorder.ExpectMetric(
          *source, UkmEntry::kEntryName,
          UkmEntry::kUser_Action_TriggeredManualFallbackForUpdatingName, 1u);
    } else {
      histogram_tester.ExpectUniqueSample(
          "PasswordManager.PasswordSavedWithManualFallback", true, 1);
      test_ukm_recorder.ExpectMetric(
          *source, UkmEntry::kEntryName,
          UkmEntry::kUser_Action_TriggeredManualFallbackForSavingName, 1u);
    }
  }
}
