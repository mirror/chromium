...
TEST(StartupTabProviderTest, IncognitoProfile) {
  ...
  size_t expected_number_of_startup_tabs_incognito = 0;
#if defined(OS_WIN)
  if (base::win::GetVersion() >= base::win::VERSION_WIN10)
    expected_number_of_startup_tabs_incognito = 1;
#endif

  EXPECT_EQ(expected_number_of_startup_tabs_incognito, output.size());
}
