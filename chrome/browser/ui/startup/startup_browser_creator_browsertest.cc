...
#if defined(OS_WIN)
  // Test that a shortcut icon has been successfully created (Windows only).
  // This is a regression test for https://crbug.com/596348.

  // Post a task to the FILE thread that does nothing (but waits for the other
  // tasks on the FILE thread to complete; namely, saving the icon and shortcut
  // files to disk). Only then do we post back to the UI thread to exit the run
  // loop.
  base::RunLoop run_loop;
  content::BrowserThread::PostTaskAndReply(
      content::BrowserThread::FILE, FROM_HERE, base::Bind(&base::DoNothing),
      run_loop.QuitClosure());
  run_loop.Run();

  // Ensure that the .ico file was written to the profile directory.
  // (We could also check the .lnk file, but that does not actually get written
  // unless there was already a valid .lnk file present, which is too complex to
  // set up and isn't required for the regression test.)
  EXPECT_TRUE(base::PathExists(icon_path));
  EXPECT_TRUE(base::PathExists(icon_hash_path));
#endif  // defined(OS_WIN)
}
