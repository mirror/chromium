...
static ScopedJavaLocalRef<jstring> GetCertExtensionText(
    JNIEnv* env,
    const JavaParamRef<jclass>&) {
  return ConvertUTF8ToJavaString(
      env, l10n_util::GetStringUTF8(IDS_CERT_INFO_EXTENSION_GROUP));
}

static ScopedJavaLocalRef<jstring> GetCertSANText(JNIEnv* env,
                                                  const JavaParamRef<jclass>&) {
  return ConvertUTF8ToJavaString(
      env,
      l10n_util::GetStringUTF8(IDS_CERT_INFO_SUBJECT_ALTERNATIVE_NAME_LABEL));
}
