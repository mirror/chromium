...
void ConnectionInfoPopupAndroid::SetIdentityInfo(
  ...
  if (identity_info.show_ssl_decision_revoke_button) {
    base::string16 reset_description = l10n_util::GetStringUTF16(
    ...
    Java_ConnectionInfoPopup_addResetCertDecisionsButton(
        env, popup_jobject_, ConvertUTF16ToJavaString(env, reset_description),
...
void ConnectionInfoPopupAndroid::AddExplanations(
  ...
  for (const auto& it : explanations) {
    ...
    Java_ConnectionInfoPopup_addExplanation(
        env, popup_jobject_, icon_id, summary, description, it.has_certificate);
    if (it.has_certificate) {
      ScopedJavaLocalRef<jstring> certificate_viewer_link =
          ConvertUTF8ToJavaString(
              env, l10n_util::GetStringUTF8(IDS_PAGE_INFO_VIEW_CERTIFICATE));
      Java_ConnectionInfoPopup_addCertificateViewerLink(
          env, popup_jobject_, certificate_viewer_link);
    }
  }
}
