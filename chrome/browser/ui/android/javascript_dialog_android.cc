...
#include "jni/JavaScriptDialog_jni.h"
...
base::string16 JavaScriptDialogAndroid::GetUserInput() {
  ...
  ScopedJavaLocalRef<jstring> prompt =
      Java_JavaScriptDialog_getUserInput(env, dialog_jobject_);
...
void JavaScriptDialogAndroid::Accept(JNIEnv* env,
  ...
  if (dialog_callback_) {
    ...
    dialog_callback_.Run(true, prompt_text);
...
JavaScriptDialogAndroid::~JavaScriptDialogAndroid() {
  ...
  if (!dialog_jobject_.is_null()) {
    JNIEnv* env = AttachCurrentThread();
    Java_JavaScriptDialog_dismiss(env, dialog_jobject_);
  }
}
...
JavaScriptDialogAndroid::JavaScriptDialogAndroid(
  ...
  JNIEnv* env = AttachCurrentThread();
  parent_jobject_weak_ref_ = JavaObjectWeakGlobalRef(
      env,
  ...
  switch (dialog_type) {
    case content::JAVASCRIPT_DIALOG_TYPE_ALERT: {
      dialog_object =
          Java_JavaScriptDialog_createAlertDialog(env, title_ref, message_ref);
      break;
    }
    case content::JAVASCRIPT_DIALOG_TYPE_CONFIRM: {
      dialog_object = Java_JavaScriptDialog_createConfirmDialog(env, title_ref,
                                                                message_ref);
      break;
    }
    case content::JAVASCRIPT_DIALOG_TYPE_PROMPT: {
      ...
      dialog_object = Java_JavaScriptDialog_createPromptDialog(
  ...
  Java_JavaScriptDialog_showDialog(env, dialog_object, parent_jobj,
