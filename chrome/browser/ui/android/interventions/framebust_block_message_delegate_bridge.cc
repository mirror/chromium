...
ScopedJavaLocalRef<jstring> FramebustBlockMessageDelegateBridge::GetMessage(
    JNIEnv* env,
    const JavaParamRef<jobject>& obj) {
  return ConvertUTF16ToJavaString(env, message_delegate_->GetMessage());
}
...
ScopedJavaLocalRef<jstring>
FramebustBlockMessageDelegateBridge::GetFeaturedLinkText(
    JNIEnv* env,
    const JavaParamRef<jobject>& obj) {
  return ConvertUTF16ToJavaString(env,
                                  message_delegate_->GetFeaturedLinkText());
}
...
void FramebustBlockMessageDelegateBridge::OnLinkTapped(
    JNIEnv* env,
    const JavaParamRef<jobject>& obj) {
  message_delegate_->OnLinkTapped();
}
