...
class SurveyInfoBarDelegate : public infobars::InfoBarDelegate {
 public:
  SurveyInfoBarDelegate(const std::string& siteId,
                        ...
                        const JavaRef<jobject>& surveyInfoBarDelegate)
      : site_id_(siteId),
        show_as_bottom_sheet_(showAsBottomSheet),
        display_logo_resource_id_(displayLogoResourceId),
        survey_info_bar_delegate_(surveyInfoBarDelegate) {}
  ...
  const JavaRef<jobject>& GetSurveyInfoBarDelegate() {
 ...
 private:
  ...
  const JavaRef<jobject>& survey_info_bar_delegate_;
};
