...
class SurveyInfoBarDelegate : public infobars::InfoBarDelegate {
 public:
  ...
  SurveyInfoBarDelegate(const std::string& surveyId,
                        bool showAsBottomSheet,
                        int displayLogo)
      : survey_id_(surveyId),
        show_as_bottom_sheet_(showAsBottomSheet),
        display_logo_(displayLogo) {}
  ...
  int GetDisplayLogo() { return display_logo_; }
...
ScopedJavaLocalRef<jobject> SurveyInfoBar::CreateRenderInfoBar(JNIEnv* env) {
  ...
  return Java_SurveyInfoBar_create(env,
                                   base::android::ConvertUTF8ToJavaString(
                                       env, survey_delegate->GetSurveyId()),
                                   survey_delegate->GetShowAsBottomSheet(),
                                   survey_delegate->GetDisplayLogo());
}
...
void Create(JNIEnv* env,
  ...
  service->AddInfoBar(
      base::MakeUnique<SurveyInfoBar>(base::MakeUnique<SurveyInfoBarDelegate>(
          ...
          j_show_as_bottom_sheet, j_display_logo)));
}
