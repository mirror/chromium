...
class SurveyInfoBarDelegate : public infobars::InfoBarDelegate {
 public:
  SurveyInfoBarDelegate(const std::string& siteId,
                        ...
                        const JavaRef<jobject>& surveyInfoBarObserver)
      : site_id_(siteId),
        show_as_bottom_sheet_(showAsBottomSheet),
        display_logo_resource_id_(displayLogoResourceId),
        survey_info_bar_observer_(surveyInfoBarObserver) {}
  ...
  const JavaRef<jobject>& GetSurveyInfoBarObserver() {
    return survey_info_bar_observer_;
  }
 ...
 private:
  ...
  const JavaRef<jobject>& survey_info_bar_observer_;
};
...
ScopedJavaLocalRef<jobject> SurveyInfoBar::CreateRenderInfoBar(JNIEnv* env) {
  ...
  return Java_SurveyInfoBar_create(
      ...
      survey_delegate->GetSurveyInfoBarObserver());
}
...
void Create(JNIEnv* env,
            ...
            const JavaParamRef<jobject>& j_survey_info_bar_observer) {
