...
NotifyInfoBarAndroid::NotifyInfoBarAndroid(
    std::unique_ptr<infobars::NotifyInfoBarDelegate> notify_delegate)
    : InfoBarAndroid(std::move(notify_delegate)),
      delegate_bridge_(base::MakeUnique<NotifyInfoBarDelegateBridge>(
          static_cast<infobars::NotifyInfoBarDelegate*>(delegate()))) {}
...
void NotifyInfoBarAndroid::Show(
  ...
  service->AddInfoBar(
      base::MakeUnique<NotifyInfoBarAndroid>(std::move(delegate)));
}
