...
TEST_F(PageInfoTest, NonFactoryDefaultPermissionsShown) {
  ...
  // Change a default-block setting away from the default.
  ...
  auto contains_permission = [](PermissionInfoList permissions,
                                ContentSettingsType type) {
    for (PageInfoUI::PermissionInfo permission : permissions) {
  ...
  for (ContentSettingsType type : non_default_permissions)
    EXPECT_TRUE(contains_permission(last_permission_info_list(), type));
}
