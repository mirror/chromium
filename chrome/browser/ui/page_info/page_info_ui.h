...
class PageInfoUI {
 public:
  ...
  // default setting.
  static const gfx::ImageSkia GetPermissionIcon(const PermissionInfo& info);

  // Returns the icon resource ID for the given permission |type| and |setting|.
  // Non-Harmony helper for permission icon retrieval.
  // TODO(patricialor): Delete this method when Harmony is default.
  static int GetPermissionIconIDLegacy(ContentSettingsType type,
                                       ContentSetting setting);

  // Returns the icon for the given object |info|.
  static const gfx::ImageSkia GetChosenObjectIcon(const ChosenObjectInfo& info,
                                                  bool deleted);
