...
bool NonWhitelistedCommandsAreDisabled(CommandUpdater* command_updater) {
  ...
  // Go through all the possible command ids, skipping whitelisted ones.
...
void BrowserCommandController::UpdateCommandsForLockedFullscreenMode() {
  ...
  if (is_locked_fullscreen_) {
    ...
    DCHECK(NonWhitelistedCommandsAreDisabled(this));
  } else {
