 ...
 protected:
  ...
  void AddHistoryEntry(const TestHistoryEntry& entry, const Time& time) {
    ...
    if (!history_service->BackendLoaded()) {
      ...
      } while (!history_service->BackendLoaded());
    }
    ...
    // Wait for the AddPageWithDetails() call to finish and for all observers
    // to process the notification; some (e.g. the in-memory history database)
    // may do something important.
    {
