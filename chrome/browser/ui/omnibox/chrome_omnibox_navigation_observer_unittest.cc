...
// Used for constructing a FakeURLFetcher that have response headers, which are
// not allowed to be set in FakeURLFetcherFactory.
class RedirectURLFetcherFactory : public net::URLFetcherFactory {
 ...
 private:
  base::hash_map<std::string, std::string> redirections_;
};
...
class MockChromeOmniboxNavigationObserver
 ...
 private:
  ...
  // memory within this class because this class is automatically deleted
  // when all fetchers finish (before we can query this value).
...
TEST_F(ChromeOmniboxNavigationObserverTest, AlternateNavInfoBar) {
  ...
  for (size_t i = 0; i < arraysize(cases); ++i) {
    ...
    for (size_t j = 0; (j < 2) && !cases[i].fake_response[j].url.empty(); ++j) {
      if (!cases[i].fake_response[j].redirected_url.empty()) {
        redirecter_factory.SetRedirectLocation(
            cases[i].fake_response[j].url,
            cases[i].fake_response[j].redirected_url);
      } else {
        // Not a redirected URL.  Used the regular FakeURLFetcherFactory
        // interface.
        factory.SetFakeResponse(
            GURL(cases[i].fake_response[j].url),
            std::string(),  // empty response
            // *** does this cast work?  apparently so...
            static_cast<net::HttpStatusCode>(
                cases[i].fake_response[j].http_response_code),
            cases[i].fake_response[j].net_request_successful
                ? net::URLRequestStatus::SUCCESS
                : net::URLRequestStatus::FAILED);
      }
    }

    // Create the observer and the alternate nav match.
    ...
    // Send it NAV_ENTRY_PENDING to get the URL fetcher to start.
    auto navigation_entry(content::NavigationController::CreateNavigationEntry(
        GURL(), content::Referrer(), ui::PAGE_TRANSITION_FROM_ADDRESS_BAR,
        false, std::string(), profile()));
    content::NotificationService::current()->Notify(
    ...
    // Make sure the fetchers have finished.
    base::RunLoop().RunUntilIdle();

    // Send it NavigationEntryCommitted() to get it ready to display the infobar
    // if needed.
    content::LoadCommittedDetails details;
    ...
    EXPECT_EQ(cases[i].expected_alternate_nav_bar_shown, displayed_infobar);
  }
}
