...
void ChromeOmniboxNavigationObserver::On404() {
  TemplateURL* turl = match_.GetTemplateURL(
  ...
  // mess with it.  The second test is necessarily because ShowInDefaultList(),
  // though it normally lists the default search provider, assumes the
  // TemplateURLService has already loaded, which it might not have.
  // GetDefaultSearchProvider makes no such assumption.
  if (template_url_service_->ShowInDefaultList(turl) ||
