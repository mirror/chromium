...
gfx::Image FaviconCache::GetFaviconForPageUrl(
  ...
  if (it != pending_requests_.end()) {
    pending_requests_.insert(
        it, std::make_pair(page_url, std::move(on_favicon_fetched)));
    return gfx::Image();
  }
  ...
  favicon_service_->GetFaviconImageForPageURL(
      ...
      &task_tracker_);
  pending_requests_.insert(
      std::make_pair(page_url, std::move(on_favicon_fetched)));
...
void FaviconCache::OnFaviconFetched(
  ...
  auto requests = pending_requests_.equal_range(page_url);
  for (auto it = requests.first; it != requests.second; ++it) {
    std::move(it->second).Run(result.image);
  }
  pending_requests_.erase(requests.first, requests.second);
}
