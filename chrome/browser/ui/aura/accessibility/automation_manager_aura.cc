...
void AutomationManagerAura::Enable(BrowserContext* context) {
  ...
  SendEvent(context, current_tree_->GetRoot(), ax::mojom::Event::LOAD_COMPLETE);
  ...
  if (active_window) {
    ...
    SendEvent(context, focus, ax::mojom::Event::CHILDREN_CHANGED);
  }
#endif
}
...
void AutomationManagerAura::HandleAlert(content::BrowserContext* context,
  ...
  SendEvent(context, obj, ax::mojom::Event::ALERT);
}
...
void AutomationManagerAura::PerformAction(const ui::AXActionData& data) {
  ...
  if (data.action == ax::mojom::Action::HIT_TEST) {
    PerformHitTest(data);
    return;
  }
...
void AutomationManagerAura::OnChildWindowRemoved(
  ...
  SendEvent(nullptr, parent, ax::mojom::Event::CHILDREN_CHANGED);
}
