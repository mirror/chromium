...
bool AXTreeSourceAura::HandleAccessibleAction(const ui::AXActionData& action) {
  ...
  if (action.action == ax::mojom::Action::SET_SELECTION) {
...
void AXTreeSourceAura::SerializeNode(AXAuraObjWrapper* node,
  ...
  if (out_data->role == ax::mojom::Role::WEB_VIEW) {
    ...
    if (rfh) {
      ...
      out_data->AddIntAttribute(ax::mojom::IntAttribute::CHILD_TREE_ID, ax_tree_id);
    }
  } else if (out_data->role == ax::mojom::Role::WINDOW ||
             out_data->role == ax::mojom::Role::DIALOG) {
    // Add clips children flag by default to these roles.
    out_data->AddBoolAttribute(ax::mojom::BoolAttribute::CLIPS_CHILDREN, true);
  }
}
