...
class SessionRestoreObserver {
 public:
  ...
  // WillRestoreTab() gets called when a tab has been created by session restore
  // and before it is loaded by TabLoader.
  virtual void OnWillRestoreTab(content::WebContents* web_contents) {}

  // OnDidRestoreTab() gets called when the loading of a restored tab stops
  // in session restore (e.g., it finishes loading, is closed or deferred). This
  // callback is called for each tab called by OnWillRestoreTab.
  virtual void OnDidRestoreTab(content::WebContents* web_contents) {}
};
