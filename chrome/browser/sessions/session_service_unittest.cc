...
void SimulateWaitForTesting(base::Lock* lock, const bool* keep_waiting) {
  for (;;) {
    {
...
TEST_F(SessionServiceTest, GetSessionsAndDestroy) {
  ...
  helper_.RunTaskOnBackendThread(
      FROM_HERE, base::Bind(&SimulateWaitForTesting, base::Unretained(&lock),
                            base::Unretained(&keep_waiting)));
