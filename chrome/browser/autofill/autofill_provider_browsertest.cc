...
IN_PROC_BROWSER_TEST_F(AutofillProviderBrowserTest,
  ...
  EXPECT_CALL(autofill_provider_,
              OnWillSubmitForm(testing::_, testing::_, testing::_))
      .Times(1);
  ui_test_utils::NavigateToURL(browser(),
                               embedded_test_server()->GetURL(
                                   "/autofill/frame_detached_on_submit.html"));
  ...
  std::string fill_and_submit =
      "var iframe = document.getElementById('login_iframe');"
      "var frame_doc = iframe.contentDocument;"
      "frame_doc.getElementById('address_field').value = 'temp';"
