...
IN_PROC_BROWSER_TEST_F(AutofillMetricsMetricsBrowserTest,
  ...
  GURL url = https_server_->GetURL("a.com", "/autofill_iframe_embedder.html");
  ...
  const ukm::mojom::UkmEntry* entry = test_ukm_recorder_->GetEntryForEntryName(
      ukm::builders::Autofill_DeveloperEngagement::kEntryName);
  ASSERT_NE(nullptr, entry);
  const ukm::UkmSource* source =
      test_ukm_recorder_->GetSourceForSourceId(entry->source_id);
  ASSERT_NE(nullptr, source);
  EXPECT_EQ(url.GetOrigin(), source->url());
}
