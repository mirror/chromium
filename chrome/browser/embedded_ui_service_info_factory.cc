...
class UIService : public service_manager::ForwardingService {
 ...
 private:
  void OnStart() override {
    // Make UIService provides below interfaces. When the GPU (or VIZ) is
    // separated from UI service, the GPU service should provide those
    // interfaces.

    registry_.AddInterface(base::Bind(
        &UIService::BindRequest<arc::mojom::VideoDecodeAccelerator,
                                chromeos::arc::GpuArcVideoDecodeAccelerator>,
        base::Unretained(this)));
    registry_.AddInterface(base::Bind(
        &UIService::BindRequest<arc::mojom::VideoEncodeAccelerator,
  ...
  void BindRequest(mojo::InterfaceRequest<Interface> request) {
    ui_service_.GetGpuThreadTaskRunner()->PostTask(
        FROM_HERE,
        base::Bind(&UIService::BindRequestOnGpuThread<Interface, Accelerator>,
...
// Initialize media codec. The UI service is running in a priviliged process.
// We don't need care when to initialize media codec. When we have a separate
// GPU (or VIZ) service process, we should initialize them before sandboxing.
