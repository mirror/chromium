...
class InfoBarUITest : public TestBrowserUI {
 public:
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(InfoBarDialogTest);
};
...
bool InfoBarUITest::VerifyUI() {
  InfoBarManager::InfoBars old_infobars = infobars_;
  GetAllInfoBars();
  auto added =
      base::STLSetDifference<InfoBarManager::InfoBars>(infobars_, old_infobars);
  return (added.size() == 1) &&
         (added[0]->GetIdentifier() == DEFAULT_BROWSER_INFOBAR_DELEGATE);
  infobars_ = added;
}

void InfoBarUITest::WaitForUserDismissal() {
  InfoBarObserver observer(GetInfoBarService(), added.front());
...
void InfoBarUITest::GetAllInfoBars() {
