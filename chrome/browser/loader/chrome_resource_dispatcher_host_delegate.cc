...
void ChromeResourceDispatcherHostDelegate::OnResponseStarted(
  ...
  if (info->GetResourceType() == content::RESOURCE_TYPE_MAIN_FRAME &&
    ...
    // Note: this is for the subset of previews types are decided
    // upon navigation commit. Previews types that are determined
    // prior to navigation (such as for offline pages or for
    // redirecting to another url), are not set here.
