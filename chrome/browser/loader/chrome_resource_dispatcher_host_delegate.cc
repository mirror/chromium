...
void ChromeResourceDispatcherHostDelegate::RequestComplete(
  ...
  base::DictionaryValue* request_state;
  std::string status;
  net::HostPortPair request_host_port;
  if (url_request->GetStateAsValue()->GetAsDictionary(&request_state) &&
      request_state->GetString("status", &status) &&
      status.compare("CANCELED") != 0) {
    request_host_port = url_request->GetSocketAddress();
  }
  if (request_host_port.IsEmpty()) {
    std::unique_ptr<net::IPEndPoint> request_ip_endpoint(new net::IPEndPoint());
    bool was_successful =
        url_request->GetRemoteEndpoint(request_ip_endpoint.get());
    if (was_successful)
      request_host_port =
