...
#endif
      network_service_interface_request_(
          mojo::MakeRequest(&network_service_interface_)),
      globals_(nullptr),
...
void IOThread::Init() {
  ...
  globals_->network_service = content::NetworkService::Create(
      std::move(network_service_interface_request_));
...
void IOThread::DisableQuic() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  network_service_interface_->DisableQuic();
}
...
void IOThread::ConfigureParamsFromFieldTrialsAndCommandLine(
  ...
  network_session_configurator::ParseCommandLineAndFieldTrials(
      command_line, true /* is_quic_allowed_by_policy */, quic_user_agent_id,
      params);
