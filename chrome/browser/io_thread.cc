...
net::URLRequestContext* SystemURLRequestContextGetter::GetURLRequestContext() {
  ...
  DCHECK(io_thread_->globals()->system_request_context.url_request_context);

  return io_thread_->globals()
      ->system_request_context.url_request_context.get();
}
...
IOThread::Globals::
SystemRequestContextLeakChecker::~SystemRequestContextLeakChecker() {
  globals_->system_request_context.url_request_context->AssertNoURLRequests();
}
...
void IOThread::Init() {
  ...
  ct_tree_tracker_ =
      std::make_unique<certificate_transparency::TreeStateTracker>(
          globals_->ct_logs,
          globals_->system_request_context.url_request_context->host_resolver(),
          net_log_);
  ...
  globals_->system_request_context.url_request_context
...
void IOThread::CleanUp() {
  ...
  globals()
      ->system_request_context.url_request_context->cert_transparency_verifier()
      ->SetObserver(nullptr);
  UnregisterSTHObserver(ct_tree_tracker_.get());
  ct_tree_tracker_.reset();

  globals_->system_request_context.url_request_context->proxy_service()
      ->OnShutdown();
...
void IOThread::UpdateDnsClientEnabled() {
  globals()
      ->system_request_context.url_request_context->host_resolver()
...
void IOThread::ConstructSystemRequestContext() {
  ...
  globals_->system_request_context =
      std::move(builder)->Create(std::move(network_context_params_).get(),
                                 globals_->quic_disabled, net_log_);

#if defined(USE_NSS_CERTS)
  net::SetURLRequestContextForNSSHttpIO(
      globals_->system_request_context.url_request_context.get());
#endif
#if defined(OS_ANDROID)
