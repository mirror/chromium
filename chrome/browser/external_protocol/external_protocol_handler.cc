...
static const char* const g_denied_schemes[] = {
    "afp", "data", "disk", "disks",
...
static const char* const g_allowed_schemes[] = {
    "mailto", "news", "snews",
};
...
ExternalProtocolHandler::BlockState ExternalProtocolHandler::GetBlockState(
  ...
  for (size_t i = 0; i < arraysize(g_denied_schemes); ++i) {
    if (g_denied_schemes[i] == scheme)
      return BLOCK;
  }

  // Always allow the hard-coded allowed schemes.
  for (size_t i = 0; i < arraysize(g_allowed_schemes); ++i) {
    if (g_allowed_schemes[i] == scheme)
      return DONT_BLOCK;
  }
