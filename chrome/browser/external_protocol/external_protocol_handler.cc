  ...
  if (profile_prefs) {  // May be NULL during testing.
    ...
    if (!update_excluded_schemas_profile->empty()) {
      if (state == UNKNOWN) {
        ...
        // This *should* mean state == DONT_BLOCK. However at this level we
        // cannot guarantee this (we just DCHECK). If it is being called with
        // BLOCK we need to make sure we store BLOCK, not DONT_BLOCK, so check
        // state again.
