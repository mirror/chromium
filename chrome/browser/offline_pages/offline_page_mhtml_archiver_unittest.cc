...
void TestMHTMLArchiver::GenerateMHTML(
  ...
  if (test_scenario_ == TestScenario::TOO_MANY_FILE_NAME_COLLISIONS) {
    ReportFailure(ArchiverResult::ERROR_TOO_MANY_FILE_NAME_COLLISIONS);
    return;
  }
...
// Tests for archiver handling of a page which cannot be saved due to duplicate
// file name counter exceeded the limits.
TEST_F(OfflinePageMHTMLArchiverTest, TooManyFileNameCollisions) {
  GURL page_url = GURL(kTestURL);
  std::unique_ptr<TestMHTMLArchiver> archiver(CreateArchive(
      page_url,
      TestMHTMLArchiver::TestScenario::TOO_MANY_FILE_NAME_COLLISIONS));
  EXPECT_EQ(archiver.get(), last_archiver());
  EXPECT_EQ(
      OfflinePageArchiver::ArchiverResult::ERROR_TOO_MANY_FILE_NAME_COLLISIONS,
      last_result());
  EXPECT_EQ(base::FilePath(), last_file_path());
  EXPECT_EQ(0LL, last_file_size());
}
