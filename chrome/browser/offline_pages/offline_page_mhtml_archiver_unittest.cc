...
const std::string kTestFileHash(
...
class OfflinePageMHTMLArchiverTest : public testing::Test {
 public:
  ...
  const std::string& last_file_hash() const { return last_file_hash_; }
...
// Tests for failing to compute secure hash for archive file.
TEST_F(OfflinePageMHTMLArchiverTest, FileHashError) {
  ...
  EXPECT_EQ(archiver.get(), last_archiver());
  EXPECT_EQ(OfflinePageArchiver::ArchiverResult::ERROR_FILE_HASH_FAILED,
            last_result());
  EXPECT_EQ(base::FilePath(), last_file_path());
  EXPECT_EQ(0LL, last_file_size());
}
...
TEST_F(OfflinePageMHTMLArchiverTest, SuccessfullyCreateOfflineArchive) {
  ...
  EXPECT_EQ(kTestFileHash, last_file_hash());
}
