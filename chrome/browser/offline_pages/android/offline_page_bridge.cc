...
void SavePageLaterCallback(const ScopedJavaGlobalRef<jobject>& j_callback_obj,
                           const int value) {
  base::android::RunCallbackAndroid(j_callback_obj, value);
}
...
void OfflinePageBridge::SavePageLater(
  ...
  const int success = static_cast<int>(coordinator->SavePageLater(params));

  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE, base::Bind(&SavePageLaterCallback, j_callback_ref, success));
}
