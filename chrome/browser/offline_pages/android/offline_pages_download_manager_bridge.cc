...
OfflinePagesDownloadManagerBridge::OfflinePagesDownloadManagerBridge() {}
OfflinePagesDownloadManagerBridge::~OfflinePagesDownloadManagerBridge() {}
...
long OfflinePagesDownloadManagerBridge::addCompletedDownload(
  ...
  ScopedJavaLocalRef<jstring> j_title =
      base::android::ConvertUTF8ToJavaString(env, title.c_str());
  ...
  long count = Java_OfflinePagesDownloadManagerBridge_addCompletedDownload(
      env, j_title, j_description, isMediaScannerScannable, j_mime_type, j_path,
      length, showNotification, j_uri, j_referer);
  return count;
}
...
int OfflinePagesDownloadManagerBridge::remove(
  ...
  long count = Java_OfflinePagesDownloadManagerBridge_remove(env, j_ids);
