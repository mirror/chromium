...
std::unique_ptr<KeyedService> BuildTestOfflinePageModel(
  ...
  // If the cache folder cannot be accessed, just put temporary pages in the
  // persistent directory. Since the next startup will clean up the temporary
  // pages which are stored in persistent directory.
  base::FilePath temporary_archives_dir = persistent_archives_dir;
