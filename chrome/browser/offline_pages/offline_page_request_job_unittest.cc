...
std::string GetRangeHeader(const std::string& range_value) {
  return std::string(net::HttpRequestHeaders::kRange) + ": " + range_value;
}
...
TEST_F(OfflinePageRequestJobTest, Range) {
  SimulateHasNetworkConnectivity(false);
  ...
  net::HttpRequestHeaders extra_headers;
  extra_headers.AddHeaderFromString(GetRangeHeader("bytes=1-5"));
  LoadPageWithHeaders(kUrl, extra_headers);

  ExpectOfflinePageServed(offline_id, 5,
                          OfflinePageRequestJob::AggregatedRequestResult::
                              SHOW_OFFLINE_ON_DISCONNECTED_NETWORK);
}

TEST_F(OfflinePageRequestJobTest, MultiRangeNotSupported) {
  SimulateHasNetworkConnectivity(false);
  ...
  net::HttpRequestHeaders extra_headers;
  extra_headers.AddHeaderFromString(GetRangeHeader("bytes=1-5,20-30"));
  LoadPageWithHeaders(kUrl, extra_headers);

  ExpectOfflinePageServed(offline_id, 0,
                          OfflinePageRequestJob::AggregatedRequestResult::
                              SHOW_OFFLINE_ON_DISCONNECTED_NETWORK);
  ExpectSeekFileErrorCode(net::ERR_REQUEST_RANGE_NOT_SATISFIABLE);
}

TEST_F(OfflinePageRequestJobTest, RangeExceedingFileSize) {
  SimulateHasNetworkConnectivity(false);
  ...
  net::HttpRequestHeaders extra_headers;
  extra_headers.AddHeaderFromString(GetRangeHeader("bytes=50000-"));
  LoadPageWithHeaders(kUrl, extra_headers);
...
TEST_F(OfflinePageRequestJobTest, IgnoreRangeParsingError) {
  SimulateHasNetworkConnectivity(false);
  ...
  net::HttpRequestHeaders extra_headers;
  extra_headers.AddHeaderFromString(GetRangeHeader("bytes=3-z"));
  LoadPageWithHeaders(kUrl, extra_headers);
...
TEST_F(OfflinePageRequestJobTest, MultiHeaders) {
  SimulateHasNetworkConnectivity(true);
