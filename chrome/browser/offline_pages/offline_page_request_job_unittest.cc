...
TEST_F(OfflinePageRequestJobTest,
       LoadByOfflineIDAndFallbackToOnlineURLOnURLMismatch) {
  SimulateHasNetworkConnectivity(true);
...
TEST_F(OfflinePageRequestJobTest,
       LoadByOfflineIDAndFallbackToOnlineURLOnDigestMismatch) {
  SimulateHasNetworkConnectivity(true);
  ...
  InterceptRequest(kTestUrl7, "GET", kOfflinePageHeader,
                   std::string(kOfflinePageHeaderReasonKey) + "=download " +
                       kOfflinePageHeaderIDKey + "=" +
                       base::Int64ToString(offline_ids()[7]),
                   content::RESOURCE_TYPE_MAIN_FRAME);
  base::RunLoop().Run();
...
TEST_F(OfflinePageRequestJobTest,
       LoadByOfflineIDAndFallbackToOtherPageOnDigestMismatch) {
  SimulateHasNetworkConnectivity(false);

  // There're 2 offline pages matching kTestUrl9. The most recently created one
  // (kTestClientId10) should fail on mistmatched digest. The next recently
  // created offline page (kTestClientId9) should be fetched.
