...
void OfflinePageTabHelper::DidFinishNavigation(
  ...
  ReportOfflineMetrics(navigation_handle);
...
void OfflinePageTabHelper::ReportOfflineMetrics(
    content::NavigationHandle* navigation_handle) {
  if (navigation_handle->IsErrorPage())
    return;
  ...
  if (IsShowingTrustedOfflinePage()) {
...
bool OfflinePageTabHelper::IsShowingTrustedOfflinePage() const {
  return offline_info_.offline_page &&
         offline_info_.offline_page->url.SchemeIsHTTPOrHTTPS();
}
