...
TEST(PrefetchedPagesNotifierTest, CheckPageCreatedAfterTimes) {
  ...
  base::Time then = now - base::TimeDelta::FromSeconds(500);
  ...
  EXPECT_EQ(kExampleHost,
            ExtractRelevantHostFromOfflinePageItemList(then, single_list));
...
TEST(PrefetchedPagesNotifierTest, CheckFilteredList) {
  ...
  std::vector<OfflinePageItem> item_list = {
      ItemCreatedOn(kExampleHost, then), ItemCreatedOn(kExampleHost2, now),
      ItemCreatedOn(kExampleHost3, future)};

  EXPECT_EQ(kExampleHost3,
            ExtractRelevantHostFromOfflinePageItemList(then, item_list));
  EXPECT_EQ(kExampleHost3,
