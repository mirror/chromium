...
class OfflineMetricsCollectorImpl : public OfflineMetricsCollector {
 ...
 private:
  ...
  // The midnight of the day for which current accumulation is happening.
  // It is used to determine if the incoming samples are still within the
  //  boundaries of the same local day - between two consecutive midnights.
  // If a sample comes outside of that range, the counter corresponding to the
  // accumulated 'day usage type' is incremented and a new accumulation started.
  base::Time accumulation_midnight_;
