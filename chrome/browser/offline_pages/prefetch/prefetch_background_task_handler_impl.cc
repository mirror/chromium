...
void PrefetchBackgroundTaskHandlerImpl::PauseBackoffUntilNextRun() {
  std::unique_ptr<net::BackoffEntry> current = GetCurrentBackoff();
  // Don't reset the backoff since it will be picked up at next run. Instead,
  // we just update the release time used this run to a custom value.
...
void PrefetchBackgroundTaskHandlerImpl::Suspend() {
  std::unique_ptr<net::BackoffEntry> current = GetCurrentBackoff();
  // Clear the backoff in order not to get confused with potential 1-day
  // release time calcuated due to exponential backoff.
  current->Reset();
  // Set to 1 day later.
