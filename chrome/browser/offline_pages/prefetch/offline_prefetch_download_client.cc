...
void OfflinePrefetchDownloadClient::OnServiceInitialized(
  ...
  PrefetchService::DownloadDelegate* delegate = GetDownloadDelegate();
  if (delegate)
    delegate->OnDownloadServiceReady();
}
...
void OfflinePrefetchDownloadClient::OnDownloadFailed(
  ...
  PrefetchService::DownloadDelegate* delegate = GetDownloadDelegate();
  if (delegate)
    delegate->OnDownloadCompleted(guid, false);
}
...
void OfflinePrefetchDownloadClient::OnDownloadSucceeded(
  ...
  PrefetchService::DownloadDelegate* delegate = GetDownloadDelegate();
  if (delegate)
    delegate->OnDownloadCompleted(guid, true);
}

PrefetchService::DownloadDelegate*
OfflinePrefetchDownloadClient::GetDownloadDelegate() const {
  PrefetchService* prefetch_service =
      PrefetchServiceFactory::GetForBrowserContext(context_);
  if (!prefetch_service)
    return nullptr;
  return prefetch_service->GetDownloadDelegate();
}
