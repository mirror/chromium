...
void ReportAccessEntryPoint(
  ...
  base::HistogramBase* histogram = base::LinearHistogram::FactoryGet(
      "OfflinePages.AccessEntryPoint." + name_space, 1,
      static_cast<int>(OfflinePageRequestJob::AccessEntryPoint::MAX),
      static_cast<int>(OfflinePageRequestJob::AccessEntryPoint::MAX) + 1,
      base::HistogramBase::kUmaTargetedHistogramFlag);
  histogram->Add(static_cast<int>(entry_point));
}
...
OfflinePageRequestJob::GetAccessEntryPoint() const {
  const content::ResourceRequestInfo* info =
      content::ResourceRequestInfo::ForRequest(request());
  if (!info)
    return AccessEntryPoint::OTHERS;
  ...
  if (ui::PageTransitionCoreTypeIs(transition, ui::PAGE_TRANSITION_LINK)) {
    ...
    return AccessEntryPoint::NTP_SUGGESTIONS;
  } else if (ui::PageTransitionCoreTypeIs(transition,
