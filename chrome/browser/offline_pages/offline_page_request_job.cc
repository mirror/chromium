...
void SelectPagesForURLDone(
  ...
  // If an int64 offline ID is present in the offline header, try to use the
  // page associated with that offline ID first.
  if (!offline_header.id.empty()) {
    int64_t requested_offline_id;
    if (base::StringToInt64(offline_header.id, &requested_offline_id)) {
      std::vector<OfflinePageItem> offline_pages_with_;
      for (size_t i = 0; i < offline_pages.size(); ++i) {
        // If the page with requested offline ID is found, promote it to the
        // front.
        if (offline_pages[i].offline_id == requested_offline_id) {
          // No need to promote the page if it is already on the front.
          if (i == 0)
            break;
          std::vector<OfflinePageItem> rearranged_offline_pages = offline_pages;
          OfflinePageItem temp = rearranged_offline_pages[0];
          rearranged_offline_pages[0] = rearranged_offline_pages[i];
          rearranged_offline_pages[i] = temp;
          ValidatePage(url, offline_header, network_state, job,
                       web_contents_getter, rearranged_offline_pages, 0);
          return;
        }
      }
    }
