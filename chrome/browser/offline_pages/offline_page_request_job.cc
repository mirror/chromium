  ...
  if (headers.GetHeader(net::HttpRequestHeaders::kRange, &range_header)) {
    ...
    if (net::HttpUtil::ParseRangeHeader(range_header, &ranges)) {
      if (ranges.size() == 1) {
        ...
        range_parsing_succeeded_ = false;
      }
    }
  }
}
...
void OfflinePageRequestJob::DidOpenForValidation(int result) {
  ...
  archive_validator_ = std::make_unique<ArchiveValidator>();
...
void OfflinePageRequestJob::DidReadForValidation(int result) {
  ...
  if (result > 0) {
    archive_validator_->Update(buffer_->data(), result);
    ReadForValidation();
    return;
  }

  // When |result| is 0 (net::OK), it indicates EOF.
  std::string actual_digest = archive_validator_->Finish();
...
void OfflinePageRequestJob::DidOpenForServing(int result) {
  ...
  if (!range_parsing_succeeded_ ||
      !byte_range_.ComputeBounds(GetCurrentOfflinePage().file_size)) {
  ...
  if (remaining_bytes_ == 0) {
    DidSeekForServing(byte_range_.first_byte_position());
    return;
  }

  // Note that we always seek to the beginning of the file even when byte range
  // is not set. This is because the file may have already been read for
  // validation purpose.
  int seek_result =
      stream_->Seek(byte_range_.first_byte_position(),
                    base::Bind(&OfflinePageRequestJob::DidSeekForServing,
                               weak_ptr_factory_.GetWeakPtr()));
