...
void FinalizeHash(ArchiveValidator* archive_validator, std::string* result) {
  *result = archive_validator->Finish();
}
...
void OfflinePageRequestJob::DidReadForValidation(int result) {
  ...
  if (result > 0) {
    ...
    file_task_runner_->PostTaskAndReply(
        FROM_HERE,
        base::BindOnce(&ArchiveValidator::Update,
                       // |archive_validator_| is owned by this class.
                       base::Unretained(archive_validator_.get()),
                       base::Unretained(buffer_->data()), result),
  ...
  std::string* actual_digest = new std::string();
  file_task_runner_->PostTaskAndReply(
      FROM_HERE,
      base::BindOnce(&FinalizeHash,
                     // |archive_validator_| is owned by this class.
                     base::Unretained(archive_validator_.get()),
                     base::Unretained(actual_digest)),
      base::BindOnce(&OfflinePageRequestJob::DidComputeActualDigest,
                     weak_ptr_factory_.GetWeakPtr(),
                     base::Owned(actual_digest)));
}
