...
ThumbnailTabHelper::ThumbnailTabHelper(content::WebContents* contents)
    : content::WebContentsObserver(contents),
      ...
      is_currently_navigating_(false),
      has_received_document_since_navigating_(false),
      has_painted_since_navigating_(false),
...
void ThumbnailTabHelper::DocumentAvailableInMainFrame() {
  ...
  if (!is_currently_navigating_) {
    // From now on, we'll start watching for paint events.
    has_received_document_since_navigating_ = true;
  }
}
...
void ThumbnailTabHelper::DidFirstVisuallyNonEmptyPaint() {
  ...
  if (has_received_document_since_navigating_) {
    has_painted_since_navigating_ = true;
  }
}
...
void ThumbnailTabHelper::UpdateThumbnailIfNecessary() {
  ...
  if (!has_painted_since_navigating_) {
    return;
  }
