...
void GeolocationPermissionContextAndroid::NotifyPermissionSet(
  ...
  if (content_setting == CONTENT_SETTING_ALLOW &&
    ...
    // In split-screen multi-window we can end up showing two LSDs, but this
    // is such an edge case that we don't bother handling it correctly. The tab
    // that requests first will have its request dropped in this case.
