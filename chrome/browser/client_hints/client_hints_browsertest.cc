...
class ClientHintsBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void SetExpectations(bool expect_client_hints) {
  ...
  size_t matches_found() const { return matches_found_; }

 private:
  // Called by |https_server_|.
  void MonitorResourceRequest(const net::test_server::HttpRequest& request) {
    LOG(WARNING) << "xxx  headers=" << request.all_headers
                 << " \nurl=" << request.GetURL()
                 << " \nbase_url=" << request.base_url;

    if (!content::IsBrowserSideNavigationEnabled() ||
        request.GetURL() != no_client_hint_url()) {
...
IN_PROC_BROWSER_TEST_F(ClientHintsBrowserTest,
  ...
  ui_test_utils::NavigateToURL(browser(), no_client_hint_url());
  if (content::IsBrowserSideNavigationEnabled()) {
    EXPECT_EQ(2u, matches_found());
  } else {
    EXPECT_EQ(4u, matches_found());
  }
}
