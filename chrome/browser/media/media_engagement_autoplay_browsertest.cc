...
#if defined(OS_WIN)
const base::FilePath kPythonPath =
    base::FilePath(FILE_PATH_LITERAL("python.exe"));
#else
const base::FilePath kPythonPath = base::FilePath(FILE_PATH_LITERAL("python"));
#endif
...
class MediaEngagementAutoplayBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void ApplyPreloadedOrigin(GURL url) {
    ...
    base::CommandLine cmd(kPythonPath);
