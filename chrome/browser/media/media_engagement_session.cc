...
void MediaEngagementSession::RegisterAudiblePlayers(
  ...
  if (audible_players == 0 && significant_players == 0)
    return;

  pending_data_to_commit_.players = true;
  audible_players_ += audible_players;
  significant_players_ += significant_players;
}
...
void MediaEngagementSession::RecordUkmMetrics() {
  ...
  ukm::builders::Media_Engagement_SessionFinished(ukm_source_id_)
      ...
      .SetPlayer_Audible_Delta(audible_players_)
      .SetPlayer_Audible_Total(score.audible_playbacks())
      .SetPlayer_Significant_Delta(significant_players_)
...
void MediaEngagementSession::CommitPendingData() {
  ...
  if (pending_data_to_commit_.players) {
    score.IncrementAudiblePlaybacks(audible_players_);
    score.IncrementSignificantPlaybacks(significant_players_);

    audible_players_ = 0;
    significant_players_ = 0;
  }
