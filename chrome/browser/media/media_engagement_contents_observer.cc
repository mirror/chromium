...
void MediaEngagementContentsObserver::RecordUkmIgnoredEvent(int length_msec) {
  ...
  ukm::builders::Media_Engagement_Ignored(ukm_source_id_)
...
void MediaEngagementContentsObserver::MaybeStashAudiblePlayers() {
  PendingCommitState& state = GetPendingCommitState();

  for (const std::pair<MediaPlayerId, bool>& row : audible_players_) {
    const base::TimeDelta elapsed =
        GetPlayerState(row.first).playback_timer->Elapsed();
    if (elapsed < kMaxShortPlaybackTime) {
      RecordUkmIgnoredEvent(elapsed.InMilliseconds());
      continue;
    }

    state.significant_players += row.second;
...
MediaEngagementContentsObserver::GetPlayerState(const MediaPlayerId& id) {
  ...
  std::unique_ptr<PlayerState> new_state =
      base::MakeUnique<PlayerState>(service_->clock_.get());

  auto result = player_states_.insert(std::make_pair(id, std::move(new_state)));
...
void MediaEngagementContentsObserver::MediaStoppedPlaying(
  ...
  PlayerState& state = GetPlayerState(media_player_id);
  state.playing = false;
  state.reached_end_of_stream = reached_end_of_stream;
...
// Skipping WasRecentlyAudible check on Android (not available).
  ...
  if (!significant_playback_recorded_) {
    ...
    // pending to commit.
    DCHECK(pending_data_to_commit_.has_value());
    GetPendingCommitState().media_playback = true;
