...
class MediaEngagementContentsObserver : public content::WebContentsObserver {
 ...
 private:
  ...
  enum class InsignificantHistogram {
    ...
    // The player isn't currently significant and can't be because it
    // doesn't meet all the critera (subsequent times).
    kPlayerNotAddedSubsequent,
  ...
  GetInsignificantPlayerReason(PlayerState* state);
  ...
  bool IsPlayerStateComplete(PlayerState* state);
  ...
  static const char* kHistogramSignificantNotAddedSubsequentName;
