...
#if DCHECK_IS_ON()
// Checks that ProfileDestroyer and the constructor are called on the same
// sequence to avoid races with g_receiver_profiles.
base::SequenceChecker& receiver_profiles_sequence_checker() {
  static SEQUENCE_CHECKER(receiver_profiles);
...
void PresentationReceiverWindow::DeleteProfile(Profile* profile) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(receiver_profiles_sequence_checker());
