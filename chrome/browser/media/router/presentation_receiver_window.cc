...
void PresentationReceiverWindow::DeleteProfile(Profile* profile) {
  ...
  // destroyed.  Since it is linked to a Browser, there are two important things
  ...
  // we need a ~Browser customization.  Unlike OffscreenTab, we _do_ want it to
  // be destroyed once Browser is done with it.  We have added logic to ~Browser
  // to let us destroy the Profile, since it isn't the normal "user" OTR profile
  // (which is instead owned by ProfileImpl).
...
PresentationReceiverWindow::PresentationReceiverWindow(
    ...
    const gfx::Rect& bounds)
    : browser_(
          new Browser(MakeBrowserCreateParams(window_profile.get(), bounds))),
