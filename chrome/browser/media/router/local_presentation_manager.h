...
//
// When the receiver is no longer associated with an local presentation, it
...
class LocalPresentationManager : public KeyedService {
 public:
  ...
  // Returns true if this class has an local presentation with
  // |presentation_id|.
  virtual bool IsLocalPresentation(const std::string& presentation_id);

  // Returns nullptr if |presentation_id| is not associated with an local
  // presentation.
  virtual const MediaRoute* GetRoute(const std::string& presentation_id);

 private:
  // Represents an local presentation registered with
 ...
 private:
  ...
  // Creates an local presentation with |presentation_info|.
