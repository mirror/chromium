...
MediaRouterDialogController::StartPresentationContext::StartPresentationContext(
...
MediaRouterDialogController::StartPresentationContext::
    ~StartPresentationContext() {
  if (!cb_invoked_ && error_cb_) {
    std::move(error_cb_).Run(content::PresentationError(
        content::PRESENTATION_ERROR_UNKNOWN, "Unknown error."));
  }
}

void MediaRouterDialogController::StartPresentationContext::
    InvokeSuccessCallback(const std::string& presentation_id,
                          const GURL& presentation_url,
                          const MediaRoute& route) {
  if (!cb_invoked_ && success_cb_) {
...
void MediaRouterDialogController::StartPresentationContext::InvokeErrorCallback(
...
// static
void MediaRouterDialogController::StartPresentationContext::HandleRouteResponse(
...
bool MediaRouterDialogController::ShowMediaRouterDialogForPresentation(
  ...
  if (IsShowingMediaRouterDialog()) {
    LOG(ERROR) << "Media router dialog already exists";
    std::move(error_cb).Run(content::PresentationError(
        content::PRESENTATION_ERROR_UNKNOWN, "Unable to create dialog."));
    return false;
  }

  start_presentation_context_.reset(new StartPresentationContext(
...
std::unique_ptr<MediaRouterDialogController::StartPresentationContext>
MediaRouterDialogController::TakeStartPresentationContext() {
