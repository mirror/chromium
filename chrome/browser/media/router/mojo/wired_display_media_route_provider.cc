...
std::string GetSinkIdForDisplay(const display::Display& display) {
...
void WiredDisplayMediaRouteProvider::CreateRoute(
  ...
  presentations_.insert(
      std::make_pair(presentation_id, std::move(presentation)));
...
WiredDisplayMediaRouteProvider::CreatePresentation(
  ...
  auto title_observer = std::make_unique<WebContentsTitleObserver>(
      ...
      base::BindRepeating(
          ...
          base::Unretained(this), presentation_id));
  return Presentation(route, std::move(window_controller),
