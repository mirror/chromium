...
void WiredDisplayMediaRouteProvider::CreateRoute(
  ...
  presentation_window_ = std::unique_ptr<PresentationReceiverWindow>(
      PresentationReceiverWindow::CreateFromOriginalProfile(
          profile_, GURL(media_source), display.bounds()));
  presentation_window_->Start(presentation_id);
  std::move(callback).Run(route, base::nullopt, RouteRequestResult::OK);
  routes_[presentation_id] = std::move(route);
...
void WiredDisplayMediaRouteProvider::TerminateRoute(
  ...
  presentation_window_->Terminate();
  NotifyRouteObservers();
...
std::vector<display::Display> WiredDisplayMediaRouteProvider::GetAllDisplays()
    const {
  return display::Screen::GetScreen()->GetAllDisplays();
}

display::Display WiredDisplayMediaRouteProvider::GetPrimaryDisplay() const {
...
std::vector<MediaSinkInternal> WiredDisplayMediaRouteProvider::GetSinks()
  ...
  std::vector<display::Display> displays = GetAvailableDisplays();
...
WiredDisplayMediaRouteProvider::GetAvailableDisplays() const {
  std::vector<display::Display> all_displays = GetAllDisplays();
  ...
  // Get secondary displays that do not mirror the primary display.
  std::vector<display::Display> displays;
  for (const display::Display& display : all_displays) {
    if (display.bounds() != primary_display_bounds)
