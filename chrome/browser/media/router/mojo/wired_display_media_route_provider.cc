...
void WiredDisplayMediaRouteProvider::CreateRoute(
  ...
  std::vector<display::Display> displays = display::Screen::GetScreen()->GetAllDisplays();
  auto it = std::find_if(displays.begin(), displays.end(),
      [&sink_id](const display::Display& d) {
        return WiredDisplayMediaRouteProvider::kSinkPrefix + std::to_string(d.id())
            == sink_id;
          });
  CHECK(it != displays.end());
  display::Display display = *it;
  presentation_window_ = std::make_unique<PresentationReceiverWindow>(
...
void WiredDisplayMediaRouteProvider::TerminateRoute(
  ...
  presentation_window_->TryToCloseWindow(false, base::Bind([](bool closed){
    LOG(ERROR) << "________________" << closed;
  }));
  NotifyRouteObservers();
