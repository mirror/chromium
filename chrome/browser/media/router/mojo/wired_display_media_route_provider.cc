...
MediaSinkInternal CreateSinkForDisplay(const display::Display& display,
  ...
  sink.set_sink_id(sink_id);
  sink.set_name(sink_name);
...
bool CompareDisplayBounds(const display::Display& d1,
                          const display::Display& d2) {
  return d1.bounds().y() < d2.bounds().y() ||
         (d1.bounds().y() == d2.bounds().y() &&
          d1.bounds().x() < d2.bounds().x());
}
