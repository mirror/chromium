...
class MediaRouterDesktop : public MediaRouterMojoImpl {
 ...
 protected:
  ...
  // This is the version that gets queued (instead of the one that takes a
  // MediaRouteController*) if CreateMediaRouteController is deferred. This is
  // because the MediaRouteController may be deleted by the time the callback
  // is executed, so we need to perform a check that the controller exists.
  // Note that it is possible that the original controller has been deleted and
  // replaced with another instance. The base class DoCreateMediaRouteController
  // should be handling this case correctly already.
