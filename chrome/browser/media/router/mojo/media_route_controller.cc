...
MediaRouteController::MediaRouteController(
    ...
    EventPageRequestManager* request_manager)
    : route_id_(route_id),
  ...
  RegisterMojoController(std::move(mojo_media_controller));
}

void MediaRouteController::Play() {
  if (request_manager_->mojo_connections_ready()) {
    LOG(ERROR) << "Play called_____________________";
    mojo_media_controller_->Play();
    return;
  }
  LOG(ERROR) << "Play called improperly____________________";
...
void MediaRouteController::Seek(base::TimeDelta time) {
  ...
  request_manager_->RunOrDefer(base::BindOnce(&MediaRouteController::Seek,
...
void MediaRouteController::SetMute(bool mute) {
  ...
  request_manager_->RunOrDefer(base::BindOnce(&MediaRouteController::SetMute,
                                              weak_factory_.GetWeakPtr(), mute),
                               MediaRouteProviderWakeReason::CREATE_ROUTE);
}
...
void MediaRouteController::RegisterMojoController(
    mojom::MediaControllerPtr mojo_media_controller) {
  DCHECK(mojo_media_controller.is_bound());
