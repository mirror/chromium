...
void MirroringMediaRouteController::SetMediaRemotingPolicy(
    MediaRemotingPolicy policy) {
  if (request_manager()->mojo_connections_ready()) {
    DCHECK(mojo_mirroring_controller_);
    mojo_mirroring_controller_->SetMediaRemotingPolicy(policy);
    return;
  }
  request_manager()->RunOrDefer(
      base::BindOnce(&MirroringMediaRouteController::SetMediaRemotingPolicy,
                     base::AsWeakPtr(this), policy),
      MediaRouteProviderWakeReason::ROUTE_CONTROLLER_COMMAND);
}
