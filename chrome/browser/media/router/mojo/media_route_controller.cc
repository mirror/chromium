...
MirroringMediaRouteController::MirroringMediaRouteController(
  ...
  const base::Value* value =
      prefs_->GetDictionary(prefs::kMediaRouterMediaRemotingEnabled)
          ->FindKeyOfType("default", base::Value::Type::BOOLEAN);
  if (value)
    media_remoting_enabled_ = value->GetBool();
}
...
void MirroringMediaRouteController::SetMediaRemotingEnabled(bool enabled) {
  ...
  latest_status_.mirroring_extra_data->media_remoting_enabled = enabled;
  {
    DictionaryPrefUpdate update(prefs_,
                                prefs::kMediaRouterMediaRemotingEnabled);
    update->SetKey("default", base::Value(enabled));
  }
  MediaRouteController::OnMediaStatusUpdated(latest_status_);
}
