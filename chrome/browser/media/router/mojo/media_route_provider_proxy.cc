...
//   // Enabling browser side discovery means disabling extension side
//   discovery.
//   // We are migrating discovery from the external Media Route Provider to the
//   // Media Router (crbug.com/687383), so we need to disable it in the
//   provider. config->enable_dial_discovery =
//   !media_router::DialLocalDiscoveryEnabled(); config->enable_cast_discovery =
//   !media_router::CastDiscoveryEnabled();
...
void MediaRouteProviderProxy::RegisterProviderManager(
    mojom::MediaRouteProviderPtr provider_manager) {
  media_route_provider_ = std::move(provider_manager);
  media_route_provider_.set_connection_error_handler(base::BindOnce(
      &MediaRouteProviderProxy::OnConnectionError, base::Unretained(this)));
}

mojom::MediaRouteProviderPtr MediaRouteProviderProxy::BindInterfacePtr() {
