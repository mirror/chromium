...
void MediaRouterMojoImpl::OnSinksReceived(
  ...
  sinks_query->CacheSinksForProvider(provider_id, sinks);
...
void MediaRouterMojoImpl::OnRoutesUpdated(
  ...
  routes_query->CacheRoutesForProvider(provider_id, routes, joinable_route_ids);
...
void MediaRouterMojoImpl::ProvideSinks(const std::string& provider_name,
  ...
  DVLOG_WITH_INSTANCE(1) << "Provider [" << provider_name << "] found "
                         << sinks.size() << " devices...";
  mojom::MediaRouteProvider* provider = GetCanonicalProvider(provider_name);
  if (!provider) {
    DVLOG_WITH_INSTANCE(1) << __func__
                           << ": provider not found: " << provider_name;
    return;
  }
  provider->ProvideSinks(provider_name, std::move(sinks));
}
...
void MediaRouterMojoImpl::MediaSinksQuery::CacheSinksForProvider(
  ...
  // We update |cached_sink_list_| by merging the sink lists for providers.
  // We assume that there are no duplicate sinks between the providers.
...
void MediaRouterMojoImpl::BindToMojoRequest(
  ...
  binding->set_connection_error_handler(
      base::BindOnce(&MediaRouterMojoImpl::OnBindingInvalidated,
                     weak_factory_.GetWeakPtr(), provider_id));
  bindings_[provider_id] = std::move(binding);
}

void MediaRouterMojoImpl::OnBindingInvalidated(
    mojom::MediaRouteProvider::Id provider_id) {
  bindings_.erase(provider_id);
}
...
mojom::MediaRouteProvider* MediaRouterMojoImpl::GetCanonicalProvider(
