...
// static
const mojom::MediaRouteProvider::Id LocalMediaRouteProvider::kProviderId =
...
LocalMediaRouteProvider::~LocalMediaRouteProvider() {
  // TODO: need to shutdown properly. Use KeyedService::ShutDown?
  // OffscreenPresentationManagerFactory::GetOrCreateForBrowserContext(profile_)
  //   ->RemoveObserver(this);
}
...
void LocalMediaRouteProvider::OnDisplayAdded(
    const display::Display& new_display) {
  NotifySinkObservers();
}

void LocalMediaRouteProvider::OnDisplayRemoved(
    const display::Display& old_display) {
  NotifySinkObservers();
}

void LocalMediaRouteProvider::OnPendingControllersUpdated() {
  NotifySinkObservers();
}
...
void LocalMediaRouteProvider::NotifySinkObservers() const {
...
std::vector<MediaSinkInternal> LocalMediaRouteProvider::GetSinks() const {
  const std::vector<display::Display>& displays =
      display::Screen::GetScreen()->GetAllDisplays();
