...
void LocalMediaRouteProvider::UpdateMediaSinks(
    const std::string& media_source) {
  if (IsPresentationSource(media_source)) {
    media_router_->OnSinksReceived(kProviderName, media_source, GetSinks(), {});
  } else {
...
void LocalMediaRouteProvider::OnDisplayAdded(
  ...
  for (const std::string& sink_query : sink_queries_)
    media_router_->OnSinksReceived(kProviderName, sink_query, sinks, {});
}
  ...
  for (const display::Display& display : displays) {
    if (!display.IsInternal() &&
        !OffscreenPresentationManagerFactory::GetOrCreateForBrowserContext(
             profile_)
             ->HasControllerWindow(display.id())) {
