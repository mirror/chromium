...
// A display can be used as a sink if it is external, does not mirror an
// internal display, and does not have a browser window with a pending
// presentation controller (i.e. a Media Router dialog).
class WiredDisplayMediaRouteProvider
 ...
 private:
  ...
  // external, does not mirror an internal display, and does not contain a page
  // that is about to be a presentation controller.
  std::vector<MediaSinkInternal> GetSinks() const;

  // Returns a map containing displays that can be used as sinks, in which the
  // keys are the indices of displays (starts at 1) when sorted top to bottom,
  // left to right.
  base::flat_map<size_t, display::Display> GetAvailableDisplays() const;
  ...
  // receivers.
  OffscreenPresentationManager* const presentation_manager_;

  std::unique_ptr<PresentationReceiverWindow> presentation_window_;
