...
class ExtensionMediaRouteProviderProxyTest : public testing::Test {
 ...
 protected:
  void SetUp() override {
    ...
    media_router_.reset(new MediaRouterMojoImpl(profile_.get()));
    ...
    auto provider_proxy = base::MakeUnique<ExtensionMediaRouteProviderProxy>(
        profile_.get(), media_router_.get());
    provider_proxy_ = provider_proxy.get();
    media_router_->set_media_route_provider_for_test(std::move(provider_proxy));
  ...
  ExtensionMediaRouteProviderProxy* provider_proxy_;
 ...
 private:
  void RegisterMockMediaRouteProvider() {
    ...
    provider_proxy_->RegisterMediaRouteProvider(std::move(provider_ptr),
                                                std::move(callback));
  }
...
TEST_F(ExtensionMediaRouteProviderProxyTest, StartAndStopObservingMediaSinks) {
  ...
  EXPECT_CALL(mock_provider_, StartObservingMediaSinks(kSource));
  // Add an observer to send a request to start observing.
  auto observer = base::MakeUnique<MockMediaSinksObserver>(
      media_router_.get(), MediaSource(kSource), url::Origin(GURL(kOrigin)));
  observer->Init();
  base::RunLoop().RunUntilIdle();

  EXPECT_CALL(mock_provider_, StopObservingMediaSinks(kSource));
  // Remove the only observer for the source to stop observing.
  observer.reset();
  base::RunLoop().RunUntilIdle();
}

TEST_F(ExtensionMediaRouteProviderProxyTest, StartAndStopObservingMediaRoutes) {
  EXPECT_CALL(mock_provider_, StartObservingMediaRoutes(kSource));
  // Add an observer to send a request to start observing.
  auto observer =
      base::MakeUnique<MediaRoutesObserver>(media_router_.get(), kSource);
  base::RunLoop().RunUntilIdle();
