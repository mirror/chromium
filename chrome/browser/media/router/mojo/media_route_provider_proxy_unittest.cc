...
class MediaRouteProviderProxyTest : public ::testing::Test {
 ...
 protected:
  void SetUp() override {
    ...
    provider_proxy_ =
        base::MakeUnique<MediaRouteProviderProxy>(profile_, media_router_);
  ...
  const MediaRoute route_ =
      MediaRoute(route_id_, media_source_, sink_id_, "", false, "", false);
...
TEST_F(MediaRouterMojoImplTest, CreateRoute) {
  ...
  EXPECT_CALL(
      *mock_provider_,
      CreateRouteInternal(
          ...
          base::TimeDelta::FromMilliseconds(kTimeoutMillis), is_incognito, _))
      .WillOnce(
          Invoke(mock_provider_, &MockMediaRouteProvider::CreateRouteSuccess));
  ...
  provider_proxy_->CreateRoute(
      ...
      std::move(route_response_callbacks), false);
}

// TEST_F(MediaRouteProviderProxyTest, CreateRoute) {
//   EXPECT_CALL(*mock_provider_, CreateRoute(media_source_id_, sink_id_,
//   presentation_id_, origin_, tab_id_, timeout_, false, _))
//     .WithArg<7>([this](mojom::MediaRouteProvider::CreateRouteCallback&
//     callback) {
...
//   provider_proxy_->CreateRoute(media_source_id_, sink_id_, presentation_id_,
//   origin_, tab_id_, timeout_, false, );
// }
...
TEST_F(MediaRouteProviderProxyTest, JoinRoute) {
  ...
  route_response_callbacks.push_back(base::Bind(
    &RouteResponseCallbackHandler::Invoke, base::Unretained(&handler)));
  provider_proxy_->JoinRoute(kSource, kPresentationId,
url::Origin(GURL(kOrigin)), tab_id_,
