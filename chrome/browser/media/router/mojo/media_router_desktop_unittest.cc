...
TEST_F(MediaRouterDesktopTest, EnableMdnsAfterEachRegister) {
  ...
  EXPECT_CALL(mock_provider_, UpdateMediaSinks(MediaSourceForDesktop().id()));
...
TEST_F(MediaRouterDesktopTest, SyncStateToMediaRouteProvider) {
  ...
  EXPECT_CALL(mock_provider_, StartObservingMediaSinks(media_source.id()));
  ...
  EXPECT_CALL(mock_provider_, StartListeningForRouteMessages(kRouteId));
