 ...
 public:
  ...
  MOCK_METHOD4(Start,
               void(const OnSinksDiscoveredCallback&,
                    ...
                    const OnAvailableSinksUpdatedCallback&,
                    const scoped_refptr<base::SequencedTaskRunner>&));
  MOCK_METHOD0(ForceSinkDiscoveryCallback, void());
  MOCK_METHOD0(OnUserGesture, void());
  MOCK_METHOD1(StartMonitoringAvailableSinksForApp,
               void(const std::string& app_name));
  MOCK_METHOD1(StopMonitoringAvailableSinksForApp,
               void(const std::string& app_name));
};
