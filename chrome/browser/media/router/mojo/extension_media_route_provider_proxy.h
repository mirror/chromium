...
//
// The MediaRouter implementation holds a Mojo pointer to this object obtained
// through BindInterfacePtr() and calls the MRP methods.
//
...
class ExtensionMediaRouteProviderProxy : public mojom::MediaRouteProvider {
 public:
  ...
  // Returns an interface pointer bound to |this|. Invalidates any pointer bound
  // previously, so it should only be called once.
 ...
 private:
  ...
  // Binds |this| to the Mojo pointer returned by BindInterfacePtr().
  ...
  // The MediaRouter implementation that uses the MediaRouteProvider methods.
