...
struct ParsedDialAppInfo {
  ...
  DialAppState state = DialAppState::UNKNOWN;
  ...
  // of the official specifciations. |extra_data| is a base::Value dictionary
  // structure, whose key is the name of the additional field, and value is a
  // base::Value dictionary structure representing the parsed XML. See
  // xml_parser.mojom for an example.
  std::unique_ptr<base::Value> extra_data;
};
