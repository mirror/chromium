...
void DialMediaSinkService::Start(
  ...
  sink_discovery_cb_ = sink_discovery_cb;
  dial_sink_added_cb_ = dial_sink_added_cb;

  impl_ = CreateImpl(
      base::BindRepeating(&DialMediaSinkService::OnSinksDiscoveredOnSequence,
                          weak_ptr_factory_.GetWeakPtr()),
      base::BindRepeating(&DialMediaSinkService::OnDialSinkAddedOnSequence,
                          weak_ptr_factory_.GetWeakPtr()),
      request_context_);
...
void DialMediaSinkService::OnSinksDiscoveredOnSequence(
    std::vector<MediaSinkInternal> sinks) {
  task_runner_->PostTask(FROM_HERE,
                         base::BindOnce(sink_discovery_cb_, std::move(sinks)));
}

void DialMediaSinkService::OnDialSinkAddedOnSequence(
    const MediaSinkInternal& sink) {
  if (dial_sink_added_cb_) {
    task_runner_->PostTask(FROM_HERE,
                           base::BindOnce(dial_sink_added_cb_, sink));
  }
}
