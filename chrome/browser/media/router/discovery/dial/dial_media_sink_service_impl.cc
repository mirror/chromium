...
void DialMediaSinkServiceImpl::StartMonitoringAvailableSinksForApp(
  ...
  if (!registered_app_set_.insert(app_name).second)
    return;
...
void DialMediaSinkServiceImpl::MaybeNotifySinkObservers(
  ...
  std::set<MediaSinkInternal> sinks = GetAvailableSinks(app_name);
...
SinkAppStatus DialMediaSinkServiceImpl::GetAppStatus(
  ...
  auto status_it = app_status_map_.find(key);
  return status_it == app_status_map_.end() ? SinkAppStatus::kUnknown
                                            : status_it->second;
}
...
void DialMediaSinkServiceImpl::SetAppStatus(const std::string& sink_id,
                                            ...
                                            SinkAppStatus app_status) {
  std::string key = sink_id + ':' + app_name;
  app_status_map_[key] = app_status;
}
...
std::set<MediaSinkInternal> DialMediaSinkServiceImpl::GetAvailableSinks(
    const std::string& app_name) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
