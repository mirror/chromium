...
class DialMediaSinkServiceImpl : public MediaSinkServiceBase,
 ...
 private:
  ...
  void OnAppInfoError(const MediaSinkInternal& sink,
                      const std::string& app_name,
                      const std::string& error_message);

  // Called when app discovery service successfully fetches and parses app info
  // XML.
  void OnAppInfoUpdated(const MediaSinkInternal& sink,
                        const std::string& app_name,
  ...
  // Helper function to get app status from |cached_app_status_map_|.
  SinkAppStatus GetCachedAppStatus(const std::string& sink_id,
                                   const std::string& app_name);

  // Helper function to set app status in |cached_app_status_map_|.
  void SetCachedAppStatus(const std::string& sink_id,
                          const std::string& app_name,
                          SinkAppStatus app_status);
  ...
  using SinkAppStatusMap = base::flat_map<std::string, SinkAppStatus>;

  // Map of app status, keyed by sink id.
  base::flat_map<std::string, SinkAppStatusMap> cached_app_status_map_;

  // Map of last known available sinks for a specific app, keyed by app name.
  base::flat_map<std::string, std::set<MediaSinkInternal>>
      last_known_available_sinks_map_;

  // Set of registered app names.
  base::flat_set<std::string> registered_app_set_;
