...
class DialMediaSinkService {
 public:
  // TOD(imcheng): We can make this a singleton by using
  ...
  // discovered sinks has been updated. |dial_sink_added_cb|: Callback to invoke
  // when a new DIAL sink has been discovered. Can be null.
  ...
  // Delegates to |impl_| to force the sink discovery callback to be invoked
  // with the current list of sinks.
  void ForceSinkDiscoveryCallback();

  // Delegates to |impl_| to initiate discovery immediately in response to a
  // user gesture (i.e., opening the Media Router dialog).
  void OnUserGesture();
