...
using ParsingError = DialDeviceDescriptionParsingError;
...
void DeviceDescriptionService::ParseDeviceDescription(
  ...
  // We use a group ID so that the parsing jobs happen (when possible) in a
  // unique process.
  SafelyParseDialDeviceDescription(
      connector_, description_data.device_description, description_data.app_url,
      /*group_id=*/"media_router_dial_device_description_parser",
