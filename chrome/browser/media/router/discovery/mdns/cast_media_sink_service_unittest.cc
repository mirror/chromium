 ...
 public:
  CastMediaSinkServiceTest()
      : mock_cast_socket_service_(new cast_channel::MockCastSocketService()),
        ...
        media_sink_service_(
            new CastMediaSinkService(mock_sink_discovered_ui_cb_.Get(),
                                     &profile_)),
        test_dns_sd_registry_(media_sink_service_.get()) {
    media_sink_service_->SetCastMediaSinkServiceImplForTest(
        base::WrapUnique(mock_media_sink_service_impl_));
  }
...
TEST_F(CastMediaSinkServiceTest, TestRestartAfterStop) {
  ...
  EXPECT_CALL(*mock_media_sink_service_impl_, Stop());
