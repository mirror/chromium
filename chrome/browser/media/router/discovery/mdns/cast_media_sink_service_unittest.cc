...
class CastMediaSinkServiceTest : public ::testing::Test {
 public:
  CastMediaSinkServiceTest()
      : task_runner_(new base::TestSimpleTaskRunner()),
        ...
        media_sink_service_(&profile_,
                            mock_cast_socket_service_.get(),
                            DiscoveryNetworkMonitor::GetInstance()),
        test_dns_sd_registry_(&media_sink_service_) {}

  void SetUp() override {
    EXPECT_CALL(test_dns_sd_registry_, AddObserver(&media_sink_service_));
  ...
  void TearDown() override {
    if (mock_impl_) {
      EXPECT_CALL(test_dns_sd_registry_, RemoveObserver(&media_sink_service_));
 ...
 protected:
  ...
  TestCastMediaSinkService media_sink_service_;
  MockCastMediaSinkServiceImpl* mock_impl_ = nullptr;
