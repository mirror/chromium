...
void CastMediaSinkService::Start(
  ...
  if (!impl_) {
    ...
    impl_ = CreateImpl(
        base::BindRepeating(&CastMediaSinkService::OnSinksDiscoveredOnSequence,
...
void CastMediaSinkService::OnDnsSdEvent(
  ...
  if (impl_) {
    impl_->task_runner()->PostTask(
        FROM_HERE,
        base::BindOnce(
            &CastMediaSinkServiceImpl::OpenChannelsWithRandomizedDelay,
            base::Unretained(impl_.get()), cast_sinks_,
...
void CastMediaSinkService::OnSinksDiscoveredOnSequence(
    std::vector<MediaSinkInternal> sinks) {
  task_runner_->PostTask(
      FROM_HERE, base::BindOnce(sinks_discovered_cb_, std::move(sinks)));
}
