...
bool CastMediaSinkServiceImpl::RetryParams::Validate(
  ...
  if (retry_params.max_retry_attempts < 0 ||
      retry_params.max_retry_attempts > 100) {
    return false;
  }
  if (retry_params.initial_delay_in_milliseconds <= 0 ||
      retry_params.initial_delay_in_milliseconds > 60 * 1000 /* 1 min */) {
    return false;
  }
  if (retry_params.multiply_factor < 1.0 || retry_params.multiply_factor > 5.0)
    return false;
...
CastMediaSinkServiceImpl::RetryParams::GetFromFieldTrialParam() {
  ...
  if (!RetryParams::Validate(params))
    return RetryParams();
...
bool CastMediaSinkServiceImpl::OpenParams::Validate(
  ...
  if (open_params.connect_timeout_in_seconds <= 0 ||
      open_params.connect_timeout_in_seconds > kMaxConnectTimeoutInSeconds) {
    return false;
  }
  if (open_params.liveness_timeout_in_seconds <= 0 ||
      open_params.liveness_timeout_in_seconds > kMaxLivenessTimeoutInSeconds) {
    return false;
  }
  if (open_params.ping_interval_in_seconds <= 0 ||
      open_params.ping_interval_in_seconds >
          open_params.liveness_timeout_in_seconds) {
    return false;
  }
  if (open_params.dynamic_timeout_delta_in_seconds < 0 ||
      open_params.dynamic_timeout_delta_in_seconds >
          kMaxConnectTimeoutInSeconds) {
    return false;
  }
