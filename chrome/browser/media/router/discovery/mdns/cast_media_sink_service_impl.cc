...
// The max number of cast channel open failure for a DIAL-discovered sink before
// we can say confidently that it is unlikely to be a Cast device.
constexpr int kMaxDialSinkFailureCount = 10;
}  // namespace
...
void CastMediaSinkServiceImpl::OnChannelErrorMayRetry(
  ...
  const net::IPEndPoint& ip_endpoint = cast_sink.cast_data().ip_endpoint;
  OnChannelOpenFailed(ip_endpoint, sink_source);
...
void CastMediaSinkServiceImpl::OnChannelOpenFailed(
  ...
  if (sink_source == SinkSource::kDial)
    ++dial_sink_failure_count_[ip_address];
