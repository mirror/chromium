...
void CastMediaSinkServiceImpl::SetBackoffPolicyForTest(
...
void CastMediaSinkServiceImpl::OnError(const cast_channel::CastSocket& socket,
  ...
  if (cast_sink_by_dial_it != current_sinks_by_dial_.end()) {
    OnChannelErrorMayRetry(cast_sink_by_dial_it->second,
                           std::move(backoff_entry), error_state);
    return;
  }
...
void CastMediaSinkServiceImpl::OnChannelErrorMayRetry(
  ...
  if (!backoff_entry || backoff_entry->failure_count() > kMaxAttempts) {
    ...
    OnChannelError(ip_endpoint, error_state);
    return;
  }
...
void CastMediaSinkServiceImpl::OnChannelOpenSucceeded(
  ...
  if (cast_sink.cast_data().discovered_by_dial) {
    current_sinks_by_dial_[ip_endpoint.address()] = cast_sink;
  } else {
    current_sinks_by_mdns_[ip_endpoint.address()] = cast_sink;
  }
  MediaSinkServiceBase::RestartTimer();
}

void CastMediaSinkServiceImpl::OnChannelError(
    const net::IPEndPoint& ip_endpoint,
    cast_channel::ChannelError error_state) {
...
void CastMediaSinkServiceImpl::OnDialSinkAdded(const MediaSinkInternal& sink) {
  ...
  if (base::ContainsKey(pending_for_open_ip_endpoints_, ip_endpoint)) {
