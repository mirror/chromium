...
TEST(CastAnalyticsTest, RecordDeviceDiscovery) {
  const MediaRouterSinkDiscoveryType type1 = MediaRouterSinkDiscoveryType::DIAL;
  const MediaRouterSinkDiscoveryType type2 = MediaRouterSinkDiscoveryType::MDNS;
  const MediaRouterSinkDiscoveryType type3 =
      MediaRouterSinkDiscoveryType::MDNSDIAL;

  base::HistogramTester tester;
  tester.ExpectTotalCount(CastAnalytics::kHistogramCastDiscoveryType, 0);
  CastAnalytics::RecordDeviceDiscovery(type1);
  CastAnalytics::RecordDeviceDiscovery(type2);
  CastAnalytics::RecordDeviceDiscovery(type3);
  CastAnalytics::RecordDeviceDiscovery(type3);
  tester.ExpectTotalCount(CastAnalytics::kHistogramCastDiscoveryType, 4);
  EXPECT_THAT(tester.GetAllSamples(CastAnalytics::kHistogramCastDiscoveryType),
              ElementsAre(Bucket(static_cast<int>(type1), 1),
                          Bucket(static_cast<int>(type2), 1),
                          Bucket(static_cast<int>(type3), 2)));
}
