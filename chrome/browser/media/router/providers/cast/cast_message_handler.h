...
// TODO(imcheng): relocate this file to components/cast_channel /
// discovery/mdns? Also, this can be a singleton?
...
// This class currently provides only a high level API
// to request app availability from a device, but will be expanded to support
// additional types of messages.
// This class must be run on the same thread as the CastSocketService used.
class CastMessageHandler : public cast_channel::CastSocket::Observer {
 public:
  static CastMessageHandler* GetInstance();
  ...
  // response is received, or if the request timed out.
  void RequestAppAvailability(cast_channel::CastSocket* socket,
                              const std::string& app_id,
                              GetAppAvailabilityCallback callback);

 private:
  CastMessageHandler(cast_channel::CastSocketService* socket_service,
                     base::TickClock* clock);
  ...
  void SendCastMessage(cast_channel::CastSocket* socket,
  ...
  void GetAppAvailabilityTimedOut(int request_id);
