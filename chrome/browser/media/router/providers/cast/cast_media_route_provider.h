...
// MediaRouteProvider for Cast sinks.
class CastMediaRouteProvider : public mojom::MediaRouteProvider {
 public:
  CastMediaRouteProvider(mojom::MediaRouteProviderRequest request,
 ...
 private:
  ...
  void UpdateSinkQuery(const MediaSource::Id& source_id,
  ...
  // Non-owned pointer to the DualMediaSinkService instance.
  DualMediaSinkService* const media_sink_service_;

  // Sink query callback registered with |media_sink_service_|.
  DualMediaSinkService::SinkQuerySubscription sink_query_subscription_;
