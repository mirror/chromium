...
bool CastAppAvailabilityTracker::HasSource(
...
std::vector<CastMediaSource> CastAppAvailabilityTracker::AddOrUpdateSink(
  ...
  for (auto& available_sinks : available_sinks_by_app_id_) {
    if (available_sinks.second.find(sink_id) != available_sinks.second.end())
