...
constexpr MediaRouteProviderId kProviderId = MediaRouteProviderId::CAST;
...
CastMediaRouteProvider::CastMediaRouteProvider(
  ...
  VLOG(0) << "XXX: " << __func__;
  // XXX: hack to get StartObservingMediaSinks to go through.
...
void CastMediaRouteProvider::StartObservingMediaSinks(
    const std::string& media_source) {
  VLOG(0) << "XXX: " << __func__ << " media_source: " << media_source;
...
void CastMediaRouteProvider::OnSinkQueryUpdated(
  ...
  VLOG(0) << "XXX: " << __func__ << " source_id: " << source_id
          << ", #sinks: " << sinks.size();
