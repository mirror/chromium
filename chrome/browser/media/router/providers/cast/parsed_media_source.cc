...
std::unique_ptr<ParsedMediaSource> ParseCastUrl(
    const MediaSource::Id& source_id,
    const GURL& url) {
  ...
  auto parsed = std::make_unique<ParsedMediaSource>(source_id, app_info);
...
std::unique_ptr<ParsedMediaSource> ParseLegacyCastUrl(
    const MediaSource::Id& source_id,
    const GURL& url) {
  base::StringPairs parameters;
  ...
  auto parsed =
      std::make_unique<ParsedMediaSource>(source_id, std::move(app_infos));
  return parsed;
}
...
std::unique_ptr<ParsedMediaSource> ParsedMediaSource::From(
  ...
  if (IsTabMirroringMediaSource(source))
    return ParsedMediaSourceForTabMirroring(source_id);

  if (IsDesktopMirroringMediaSource(source))
    return ParsedMediaSourceForDesktopMirroring(source_id);
...
ParsedMediaSource::ParsedMediaSource(const MediaSource::Id& source_id,
                                     const CastAppInfo& app_info)
    : source_id(source_id), app_infos({app_info}) {}
ParsedMediaSource::ParsedMediaSource(const MediaSource::Id& source_id,
                                     const std::vector<CastAppInfo>& app_infos)
    : source_id(source_id), app_infos(app_infos) {}
ParsedMediaSource::ParsedMediaSource(const ParsedMediaSource& other) = default;
ParsedMediaSource::~ParsedMediaSource() = default;

bool ParsedMediaSource::ContainsApp(const std::string& app_id) const {
