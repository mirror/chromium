...
class CastAppAvailabilityTracker {
 public:
  ...
  bool HasSource(const CastMediaSource& source) const;
  ...
  // Adds |sink| to the set of currently known sinks, or updates it. Returns
  // a list of CastMediaSources for which the set of available sinks
  // might have been updated by this call.
  std::vector<CastMediaSource> AddOrUpdateSink(const MediaSinkInternal& sink);
