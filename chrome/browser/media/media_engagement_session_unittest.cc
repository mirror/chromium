...
class MediaEngagementSessionTest : public testing::Test {
 public:
  ...
  static void SetSignificantPlaybackRecordeForSession(
      MediaEngagementSession* session,
      bool value) {
...
TEST_F(MediaEngagementSessionTest, RecordShortPlaybackIgnored) {
...

  {
    ...
    test_ukm_recorder().ExpectEntry(*ukm_source, Entry::kEntryName,
                                    {{Entry::kLengthName, 42}});
  }
...
// Checks that GetUkmRecorde() does not return nullptr.
...
TEST_F(MediaEngagementSessionTest, CommitPendingData_Reset) {
  ...
  SetSignificantPlaybackRecordeForSession(session.get(), true);
...
TEST_F(MediaEngagementSessionTest, CommitPendingData_UpdatePlaybackWhenNeeded) {
  ...
  SetSignificantPlaybackRecordeForSession(session.get(), false);
  SetPendingDataToCommitForSession(session.get(), false, true, false);
  EXPECT_TRUE(HasPendingPlaybackToCommitForSession(session.get()));
  CommitPendingDataForSession(session.get());
...
TEST_F(MediaEngagementSessionTest, RecordUkmMetrics) {
...

  {
    ...
    test_ukm_recorder().ExpectEntry(
        *ukm_source, Entry::kEntryName,
        {
            {Entry::kPlaybacks_TotalName, 1},
            {Entry::kVisits_TotalName, 1},
            {Entry::kEngagement_ScoreName, 0},
            {Entry::kPlaybacks_DeltaName, 1},
            {Entry::kEngagement_IsHighName, 0},
            {Entry::kPlayer_Audible_DeltaName, 0},
            {Entry::kPlayer_Audible_TotalName, 0},
            {Entry::kPlayer_Significant_DeltaName, 0},
            {Entry::kPlayer_Significant_TotalName, 0},
            {Entry::kPlaybacks_SecondsSinceLastName, 0},
        });
  }
