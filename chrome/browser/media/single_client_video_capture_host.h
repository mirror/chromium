...
// Implements mojom::VideoCaptureHost and communicates between a single video
// capture device and a single client. On Start(), uses |device_laucher_factory|
// to launch a video capture device that outputs frames to |this|. The frames
// are received through the VideoFrameReceiver interface and are forwarded to
// |observer| through the mojom::VideoCaptureObserver interface.
// Instances of this class must be operated from the same thread that is reqired
// by |device_launcher_factory|.
class SingleClientVideoCaptureHost final : public mojom::VideoCaptureHost,
 ...
 public:
  SingleClientVideoCaptureHost(
      ...
      std::unique_ptr<content::VideoCaptureDeviceLauncherFactory>
          device_launcher_factory);
