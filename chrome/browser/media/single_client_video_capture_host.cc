...
SingleClientVideoCaptureHost::SingleClientVideoCaptureHost(
    ...
    : device_id_(device_id),
      type_(type),
      params_(params),
      device_launcher_factory_(std::move(device_launcher_factory)),
      weak_factory_(this) {}
...
void SingleClientVideoCaptureHost::Start(
  ...
  if (!device_launcher_factory_) {
    VLOG(1) << "Cannot start video capture: Null device launcher factory.";
    observer_->OnStateChanged(mojom::VideoCaptureState::FAILED);
  }
  ...
  auto device_launcher = device_launcher_factory_->CreateDeviceLauncher();
  content::VideoCaptureDeviceLauncher* launcher = device_launcher.get();
  // The |device_launcher| and |device_launcher_callbacks| should keep alive
  // until the device launching completes.
