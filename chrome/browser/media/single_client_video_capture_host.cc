...
SingleClientVideoCaptureHost::SingleClientVideoCaptureHost(
    ...
    : device_id_(device_id),
      type_(type),
      params_(params),
      device_launcher_callback_(callback),
      weak_factory_(this) {
...
void SingleClientVideoCaptureHost::OnNewBufferHandle(
  ...
  const auto insert_result =
      id_map_.insert(std::make_pair(buffer_id, next_buffer_context_id_));
...
void SingleClientVideoCaptureHost::OnFrameReadyInBuffer(
  ...
  const auto insert_result = buffer_context_map_.insert(std::make_pair(
      buffer_context_id,
