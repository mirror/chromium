...
void MirrorServiceVideoCaptureHost::OnFrameReadyInBuffer(
  ...
  DVLOG(3) << __func__ << ": buffer_id=" << buffer_id;
  DCHECK(observer_);
  auto id_iter = id_map_.find(buffer_id);
...
void MirrorServiceVideoCaptureHost::OnFinishedConsumingBuffer(
  ...
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  DCHECK(observer_);
  auto buffer_context_iter = buffer_context_map_.find(buffer_context_id);
  ...
  auto retired_iter = retired_buffers_.find(buffer_context_id);
