...
IN_PROC_BROWSER_TEST_F(EncryptedMediaSupportedTypesClearKeyTest, Robustness) {
  ...
  EXPECT_WV_SUCCESS(IsVideoRobustnessSupported(kClearKey, nullptr));
  EXPECT_WV_SUCCESS(IsVideoRobustnessSupported(kClearKey, ""));
  EXPECT_WV_SUCCESS(IsAudioRobustnessSupported(kClearKey, nullptr));
  EXPECT_WV_SUCCESS(IsAudioRobustnessSupported(kClearKey, ""));
...
IN_PROC_BROWSER_TEST_F(EncryptedMediaSupportedTypesExternalClearKeyTest,
                       Robustness) {
  // External Clear Key doesn't require a robustness string.
  EXPECT_WV_SUCCESS(IsVideoRobustnessSupported(kExternalClearKey, nullptr));
  EXPECT_WV_SUCCESS(IsVideoRobustnessSupported(kExternalClearKey, ""));
  EXPECT_WV_SUCCESS(IsAudioRobustnessSupported(kExternalClearKey, nullptr));
  EXPECT_WV_SUCCESS(IsAudioRobustnessSupported(kExternalClearKey, ""));
