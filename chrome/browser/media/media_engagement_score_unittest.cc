...
TEST_F(MediaEngagementScoreTest, OverrideFieldTrial) {
  EXPECT_EQ(5, MediaEngagementScore::ScoreMinVisits());
  EXPECT_EQ(0.5, MediaEngagementScore::HighScoreLowerThreshold());
  EXPECT_EQ(0.7, MediaEngagementScore::HighScoreUpperThreshold());
  ...
  EXPECT_TRUE(score_->high_score());
  EXPECT_EQ(0.7, MediaEngagementScore::HighScoreLowerThreshold());
  EXPECT_EQ(0.9, MediaEngagementScore::HighScoreUpperThreshold());

  // Raise the lower threshold, the score will no longer be high.
  OverrideFieldTrial(5, 0.85, 0.9);
  EXPECT_FALSE(score_->high_score());
  EXPECT_EQ(0.85, MediaEngagementScore::HighScoreLowerThreshold());

  // Raise the minimum visits, the score will now be zero as it does not meet
  // the threshold requirements.
  OverrideFieldTrial(15, 0.85, 0.9);
  EXPECT_EQ(0.0, score_->actual_score());
  EXPECT_EQ(15, MediaEngagementScore::ScoreMinVisits());
}
