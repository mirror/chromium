...
TEST_F(MediaEngagementScoreTest, HighScoreThreshold) {
  ...
  // Test that a total score of 0.6 is high but above zero.
  SetScore(10, 6);
  EXPECT_TRUE(score_->high_score());

  // Test that a total score of 0.8 is also high.
  SetScore(10, 8);
  EXPECT_TRUE(score_->high_score());

  // Test that a total score of 0.2 is high because of the lower boundary.
  SetScore(10, 2);
  EXPECT_TRUE(score_->high_score());
