...
class MediaEngagementScoreTest : public ChromeRenderViewHostTestHarness {
 public:
  MediaEngagementScoreTest() : score_(GURL(), nullptr) {}
 ...
 protected:
  ...
  void TestScoreInitializesAndUpdates(
    ...
    MediaEngagementScore initial_score(GURL(), std::move(score_dict));
    ...
    MediaEngagementScore updated_score(GURL(), std::move(copy));
    VerifyScore(updated_score, initial_score.visits(),
                initial_score.media_playbacks());
  }
};
...
TEST_F(MediaEngagementScoreTest, EmptyDictionary) {
  ...
  TestScoreInitializesAndUpdates(std::move(dict), 0, 0);
}
...
TEST_F(MediaEngagementScoreTest, PartiallyEmptyDictionary) {
  ...
  TestScoreInitializesAndUpdates(std::move(dict), 2, 0);
}
...
TEST_F(MediaEngagementScoreTest, PopulatedDictionary) {
  ...
  TestScoreInitializesAndUpdates(std::move(dict), 1, 2);
}
...
TEST_F(MediaEngagementScoreTest, ContentSettingsMultiOrigin) {
  ...
  score_ = MediaEngagementScore(url, settings_map);

  // Verify the score is originally zero, try incrementing and storing
  // the score.
  VerifyScore(score_, 0, 0);
  ...
  MediaEngagementScore same_origin_score =
      MediaEngagementScore(same_origin, settings_map);
  MediaEngagementScore different_origin_score =
      MediaEngagementScore(different_origin, settings_map);
  VerifyScore(score_, 2, 1);
  VerifyScore(same_origin_score, 2, 1);
  VerifyScore(different_origin_score, 0, 0);
}
