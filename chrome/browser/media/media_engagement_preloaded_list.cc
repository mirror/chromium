...
bool MediaEngagementPreloadedList::CheckStringIsPresent(std::string input,
  ...
  *out_found = false;
  ...
  net::HuffmanDecoder huffman(huffman_tree_.data(), huffman_tree_.size());
  net::BitReader reader(trie_data_.data(), trie_bits_);
...
bool MediaEngagementPreloadedList::LoadFromFile(base::FilePath path) {
  ...
  huffman_tree_ = std::vector<uint8_t>(message.huffman_tree_size());
  ...
  trie_data_ = std::vector<uint8_t>(message.trie_data_size());
