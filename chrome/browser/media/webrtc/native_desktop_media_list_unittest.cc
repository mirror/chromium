...
class NativeDesktopMediaListTest : public views::ViewsTestBase {
 public:
  ...
  void AddWindowsAndVerify(bool has_view_dialog) {
    ...
    for (size_t i = 0; i < window_count; ++i) {
      EXPECT_EQ(model_->GetSource(i).id.type, DesktopMediaID::SOURCE_WINDOW);
...
TEST_F(NativeDesktopMediaListTest, ScreenOnly) {
  ...
  EXPECT_EQ(model_->GetSource(0).id.type, DesktopMediaID::SOURCE_SCREEN);
...
TEST_F(NativeDesktopMediaListTest, AddNativeWindow) {
  ...
  EXPECT_EQ(model_->GetSource(index).id.type, DesktopMediaID::SOURCE_WINDOW);
