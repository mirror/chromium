...
void DesktopCaptureAccessHandler::HandleRequest(
  ...
  // If the device id wasn't specified then this is a screen capture access
  // request (i.e. chooseDesktopMedia() API wasn't used to generate device id).
  ...
  // Passing a device id for audio capture isn't currently supported.
  if (!request.requested_audio_device_id.empty())
    LOG(WARNING) << "Device ID for system audio capture will be ignored.";
