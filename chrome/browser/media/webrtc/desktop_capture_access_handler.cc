...
std::unique_ptr<content::MediaStreamUI> GetDevicesForDesktopCapture(
  ...
  if (capture_audio) {
    if (media_id.type == content::DesktopMediaID::SOURCE_WEB_CONTENTS) {
...
void DesktopCaptureAccessHandler::HandleRequest(
  ...
  if (media_id.type == content::DesktopMediaID::SOURCE_NONE) {
    callback.Run(devices, content::MEDIA_DEVICE_INVALID_STATE, std::move(ui));
    return;
  }
  ...
  const bool audio_supported =
      (media_id.type == content::DesktopMediaID::SOURCE_SCREEN &&
       loopback_audio_supported) ||
      media_id.type == content::DesktopMediaID::SOURCE_WEB_CONTENTS;
