...
void CdmStorageImpl::Open(const std::string& name,
...
void CdmStorageImpl::OnFileOpened(const std::string& name,
  ...
  // When the connection to |in_use_| is closed, release the lock on the file.
  UpdateFileLock(name, std::move(on_close_callback));
  in_use_.set_connection_error_handler(base::BindOnce(&ReleaseFileLock, name));
