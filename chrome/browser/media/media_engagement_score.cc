...
// static.
double MediaEngagementScore::HighScoreLowerThreshold() {
...
// static.
double MediaEngagementScore::HighScoreUpperThreshold() {
  return base::GetFieldTrialParamByFeatureAsDouble(
      media::kMediaEngagementBypassAutoplayPolicies,
...
// static.
int MediaEngagementScore::ScoreMinVisits() {
...
void MediaEngagementScore::Recalculate() {
  ...
  if (is_high_) {
    is_high_ = actual_score_ >= HighScoreLowerThreshold();
  } else {
    is_high_ = actual_score_ >= HighScoreUpperThreshold();
  }
}
