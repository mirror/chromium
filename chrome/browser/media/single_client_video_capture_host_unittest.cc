...
class MockVideoCaptureDevice final
 ...
 public:
  explicit MockVideoCaptureDevice(base::RepeatingClosure event_cb)
      : event_cb_(event_cb) {}
  ...
  MOCK_METHOD2(OnUtilizationReportCalled, void(int, double));
  void OnUtilizationReport(int buffer_id, double utilization) override {
    OnUtilizationReportCalled(buffer_id, utilization);
    event_cb_.Run();
  }

 private:
  base::RepeatingClosure event_cb_;
...
class FakeDeviceLauncher final : public content::VideoCaptureDeviceLauncher {
 public:
  FakeDeviceLauncher(DeviceLaunchedCallback launched_cb,
                     base::RepeatingClosure event_cb)
 ...
 private:
  void OnDeviceLaunched(base::WeakPtr<VideoFrameReceiver> receiver,
    ...
    auto launched_device = std::make_unique<MockVideoCaptureDevice>(event_cb_);
...
class FackeDeviceLauncherFactory final
    : public content::VideoCaptureDeviceLauncherFactory {
 public:
  FackeDeviceLauncherFactory(DeviceLaunchedCallback launched_cb,
                             base::RepeatingClosure event_cb)
      : after_launch_cb_(std::move(launched_cb)), event_cb_(event_cb) {}
  ~FackeDeviceLauncherFactory() override {}

  // content::VideoCaptureDeviceLauncherFactory implementation.
  std::unique_ptr<content::VideoCaptureDeviceLauncher> CreateDeviceLauncher()
      override {
    return std::make_unique<FakeDeviceLauncher>(std::move(after_launch_cb_),
                                                event_cb_);
  }
 ...
 private:
  DeviceLaunchedCallback after_launch_cb_;
  base::RepeatingClosure event_cb_;
  DISALLOW_COPY_AND_ASSIGN(FackeDeviceLauncherFactory);
};
...
class MockVideoCaptureObserver final : public mojom::VideoCaptureObserver {
 public:
  MockVideoCaptureObserver(mojom::VideoCaptureHostPtr host,
                           base::RepeatingClosure event_cb)
      : host_(std::move(host)), event_cb_(event_cb), binding_(this) {}
  ...
  MOCK_METHOD1(OnStateChangedCalled, void(mojom::VideoCaptureState state));
  void OnStateChanged(mojom::VideoCaptureState state) override {
    OnStateChangedCalled(state);
    event_cb_.Run();
  }
 ...
 public:
  SingleClientVideoCaptureHostTest() : weak_factory_(this) {
    auto host_impl = std::make_unique<SingleClientVideoCaptureHost>(
        ...
        VideoCaptureParams(),
        std::make_unique<FackeDeviceLauncherFactory>(
            base::BindOnce(&SingleClientVideoCaptureHostTest::OnDeviceLaunched,
                           weak_factory_.GetWeakPtr()),
            base::BindRepeating(
                &SingleClientVideoCaptureHostTest::OnEventCalled,
                weak_factory_.GetWeakPtr())));
    ...
    consumer_ = std::make_unique<MockVideoCaptureObserver>(
        std::move(host),
        base::BindRepeating(&SingleClientVideoCaptureHostTest::OnEventCalled,
                            weak_factory_.GetWeakPtr()));
    consumer_->Start();
    WaitingForEvent();
  ...
  ~SingleClientVideoCaptureHostTest() override {
    EXPECT_CALL(*consumer_,
                OnStateChangedCalled(mojom::VideoCaptureState::ENDED))
        .Times(1);
    consumer_->Stop();
    WaitingForEvent();
  }
...
TEST_F(SingleClientVideoCaptureHostTest, Basic) {
  ...
  EXPECT_CALL(*launched_device_, OnUtilizationReportCalled(3, 1.0)).Times(1);
  ...
  EXPECT_CALL(*launched_device_, OnUtilizationReportCalled(5, 0.5)).Times(1);
...
TEST_F(SingleClientVideoCaptureHostTest, ReuseBufferId) {
  ...
  EXPECT_CALL(*consumer_, OnBufferDestroyedCall(1)).Times(1);
  consumer_->FinishConsumingBuffer(1, 0.5);
  // Expects to receive two events.
  WaitingForEvent();
  WaitingForEvent();

  EXPECT_CALL(*launched_device_, OnUtilizationReportCalled(7, 0.8)).Times(1);
