...
class CastRemotingSender : public media::mojom::RemotingDataStreamSender {
 ...
 private:
  ...
  // media::mojom::RemotingDataStreamSender implementation.SendFrame() will push
  // callbacks onto the back of the input queue, and these may or may not be
  // processed at a later time. It depends on whether the data pipe has data
  ...
  // Attempt to run next pending input operation, popping the head of the input
  // queue as each operation succeeds.
  ...
  // Indicates whether the processing of input is paused and waiting for the
  // CastTransport being able to accept more frames.
  bool waiting_for_acking_;
