...
class AppBannerManagerAndroid
 ...
 private:
  ...
  // Returns NO_ERROR_DETECTED if |platform| and |id| are valid for querying the
  // Play Store. Otherwise returns the error which prevents querying.
  InstallableStatusCode CheckPlatformAndId(const std::string& platform,
                                           const std::string& id);
  ...
  InstallableStatusCode CanHandleNonWebApp(const std::string& platform,
                                           const GURL& url,
                                           const std::string& id);
