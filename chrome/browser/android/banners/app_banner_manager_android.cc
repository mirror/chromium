...
void AppBannerManagerAndroid::PerformInstallableCheck() {
  ...
  if (manifest_.prefer_related_applications &&
    ...
    for (const auto& application : manifest_.related_applications) {
      ...
      code = CanHandleNonWebApp(platform, application.url, id);
...
InstallableStatusCode AppBannerManagerAndroid::CanHandleNonWebApp(
    const std::string& platform,
    const GURL& url,
    const std::string& id) {
  InstallableStatusCode code = CheckPlatformAndId(platform, id);
  if (code != NO_ERROR_DETECTED)
    return code;
...
InstallableStatusCode AppBannerManagerAndroid::CheckPlatformAndId(
    const std::string& platform,
    const std::string& id) {
  if (platform != "play")
