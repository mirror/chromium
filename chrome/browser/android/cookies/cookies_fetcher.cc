...
network::mojom::CookieManager* cookie_service_client() {
  ...
  network::mojom::CookieManagerPtr cookie_service_ptr;
  content::BrowserContext::GetDefaultStoragePartition(profile)
      ->GetNetworkContext()
      ->GetCookieManager(mojo::MakeRequest(&cookie_service_ptr));

  return cookie_service_ptr.get();
}

void RestoreToCookieJarInternal(std::unique_ptr<net::CanonicalCookie> cookie) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::IO);
  DCHECK(cookie->IsCanonical());

  network::mojom::CookieManager* cookies_manager = cookie_service_client();
  ...
  cookies_manager->SetCanonicalCookie(
      *cookie.get(), true /* secure_source */, true /* modify_http_only */,
...
static void JNI_CookiesFetcher_RestoreCookies(
  ...
  content::BrowserThread::PostTask(
      content::BrowserThread::IO, FROM_HERE,
      base::BindOnce(&RestoreToCookieJarInternal, std::move(cookie)));
}
