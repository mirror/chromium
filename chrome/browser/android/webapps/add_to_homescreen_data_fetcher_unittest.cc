 ...
 public:
  ...
  void GetData(const InstallableParams& params,
    ...
    if (should_manifest_time_out_ ||
      ...
      // Bind the metrics finalisation callback. We want to test when this is
      ...
      queued_metrics_callback_ =
          base::Bind(&InstallableManager::RecordQueuedMetricsOnTaskCompletion,
    ...
    if (params.check_installable && is_installable)
      RecordQueuedMetricsOnTaskCompletion(params, is_installable);
  ...
  void CompleteQueuedMetrics() { std::move(queued_metrics_callback_).Run(); }
...
class AddToHomescreenDataFetcherTest : public ChromeRenderViewHostTestHarness {
 public:
  ...
  void CompleteQueuedMetrics() {
    installable_manager_->CompleteQueuedMetrics();
  }
