...
ScopedJavaLocalRef<jobject> DownloadManagerService__CreateJavaDownloadItem(
...
void DownloadManagerService::GetAllDownloadsInternal(bool is_off_the_record) {
  ...
  for (size_t i = 0; i < all_items.size(); i++) {
    ...
    ScopedJavaLocalRef<jobject> j_item = DownloadManagerService__CreateJavaDownloadItem(env, item);
...
// static
jboolean DownloadManagerService__IsSupportedMimeType(
    JNIEnv* env,
...
// static
jint DownloadManagerService__GetAutoResumptionLimit(JNIEnv* env,
                            const JavaParamRef<jclass>& clazz) {
