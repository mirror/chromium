 ...
 public:
  static void Create(
    ...
    ImageThumbnailRequest* request =
        new ImageThumbnailRequest(icon_size, content_id, file_path,
                                  should_cache, callback, weak_generator);
    request->Start();
  }
...
void ThumbnailGenerator::RetrieveThumbnail(
  ...
  content::BrowserThread::PostTask(
      ...
      base::Bind(&ImageThumbnailRequest::Create, icon_size, content_id, path,
                 (bool)jshould_cache, callback_, weak_factory_.GetWeakPtr()));
}
