...
void ThumbnailGenerator::RetrieveThumbnail(
  ...
  base::android::ScopedJavaGlobalRef<jobject> global_callback;
  global_callback.Reset(callback);

  auto request = base::MakeUnique<ImageThumbnailRequest>(
      icon_size, base::BindOnce(&ThumbnailGenerator::OnThumbnailRetrieved,
                                weak_factory_.GetWeakPtr(), content_id,
                                icon_size, global_callback));
