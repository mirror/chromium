...
using partnerbookmarks::FaviconFetchedCallback;
using partnerbookmarks::FaviconFetchResult;

namespace {

void OnFaviconFetched(const JavaRef<jobject>& j_callback,
                      base::WaitableEvent* favicon_fetched_event,
                      FaviconFetchResult result) {
  JNIEnv* env = AttachCurrentThread();
  Java_FaviconFetchedCallback_onFaviconFetched(env, j_callback,
                                               static_cast<int>(result));
  if (favicon_fetched_event)
    favicon_fetched_event->Signal();
}
...
void PartnerBookmarksReader::GetFaviconImpl(const GURL& page_url,
  ...
  GetFaviconFromCacheOrServer(page_url, true, std::move(callback));
}

favicon::LargeIconService* PartnerBookmarksReader::large_icon_service() {
  if (large_icon_service_ == nullptr) {
    large_icon_service_ =
...
void PartnerBookmarksReader::GetFaviconFromCacheOrServer(
  ...
  large_icon_service()->GetLargeIconOrFallbackStyle(
      page_url, PARTNER_BOOKMARKS_MINIMUM_FAVICON_SIZE_PX, 0,
...
void PartnerBookmarksReader::OnGetFaviconFromCacheFinished(
  ...
  large_icon_service()
      ->GetLargeIconOrFallbackStyleFromGoogleServerSkippingLocalCache(
          page_url, PARTNER_BOOKMARKS_MINIMUM_FAVICON_SIZE_PX, 0,
          false /* may_page_url_may_be_private */, traffic_annotation,
...
void PartnerBookmarksReader::OnGetFaviconFromServerFinished(
  ...
  if (status != favicon_base::GoogleFaviconServerRequestStatus::SUCCESS) {
    std::move(callback).Run(FaviconFetchResult::FAILURE_SERVER_ERROR);
    return;
  }
