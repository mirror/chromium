...
ZeroSuggestPrefetcher::ZeroSuggestPrefetcher(Profile* profile)
  ...
  AutocompleteInput input(fake_request_source, base::string16::npos,
                          std::string(),
...
void AutocompleteControllerAndroid::Start(JNIEnv* env,
  ...
  input_ = AutocompleteInput(text, cursor_pos, desired_tld,
                             ChromeAutocompleteSchemeClassifier(profile_));
  input_.set_current_url(current_url);
  input_.set_current_page_classification(page_classification);
...
void AutocompleteControllerAndroid::OnOmniboxFocused(
  ...
  input_ = AutocompleteInput(omnibox_text, base::string16::npos, std::string(),
                             ChromeAutocompleteSchemeClassifier(profile_));
  input_.set_current_url(current_url);
