...
AutocompleteControllerAndroid::ClassifyPage(const GURL& gurl,
  ...
  if (url == chrome::kChromeUINativeNewTabURL) {
    // On phones, there is only one box on the NTP.  In this case, treat this
    // box like the omnibox.  I.e., don't apply the same sort of URL demotion
    // that happens on tablets and on desktop when the user explicitly selects
    // the fakebox (which looks more like a search box) rather than the omnibox
    // (which looks more like a URL bar).
