...
const char kContextualSearchRankerModelUrl[] =
...
const base::Feature kContextualSearchRankerQuery{
    "ContextualSearchRankerQuery", base::FEATURE_ENABLED_BY_DEFAULT};

// static
GURL GetModelURL() {
  if (!base::FeatureList::IsEnabled(kContextualSearchRankerQuery)) {
    return GURL();
  }
  // Allow override of the ranker model URL from the command line.
  std::string raw_url;
  base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
  if (command_line->HasSwitch(kContextualSearchRankerModelUrl)) {
    raw_url =
        command_line->GetSwitchValueASCII(kContextualSearchRankerModelUrl);
  } else {
    // Otherwise take the ranker model URL from the ranker query variation.
    raw_url = base::GetFieldTrialParamValueByFeature(
        kContextualSearchRankerQuery, kContextualSearchRankerModelUrl);
  }
  return GURL(raw_url);
}
...
void ContextualSearchRankerLoggerImpl::SetupLoggingAndRanker(
  ...
  if (field_trial_->IsRankerIntegrationOrMlTapSuppressionEnabled()) {
    ...
    predictor_ = assist_ranker_service->FetchBinaryClassifierPredictor(
        GetModelURL(), kContextualSearchModelFilename,
...
AssistRankerPrediction ContextualSearchRankerLoggerImpl::RunInference(
  ...
  if (was_able_to_predict) {
    if (prediction) {
      prediction_enum = ASSIST_RANKER_PREDICTION_YES;
    } else {
      prediction_enum = ASSIST_RANKER_PREDICTION_NO;
    }
  } else {
