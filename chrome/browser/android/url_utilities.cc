...
GURL UrlUtilities__ConvertJavaStringToGURL(JNIEnv* env, jstring url) {
...
bool CheckSchemeBelongsToList(
  ...
  GURL gurl = UrlUtilities__ConvertJavaStringToGURL(env, url);
  if (gurl.is_valid()) {
...
static jboolean UrlUtilities__SameDomainOrHost(JNIEnv* env,
                                 const JavaParamRef<jclass>& clazz,
                                 const JavaParamRef<jstring>& url_1_str,
                                 const JavaParamRef<jstring>& url_2_str,
                                 jboolean include_private) {
  GURL url_1 = UrlUtilities__ConvertJavaStringToGURL(env, url_1_str);
  GURL url_2 = UrlUtilities__ConvertJavaStringToGURL(env, url_2_str);
...
static ScopedJavaLocalRef<jstring> UrlUtilities__GetDomainAndRegistry(
    JNIEnv* env,
    const JavaParamRef<jclass>& clazz,
    const JavaParamRef<jstring>& url,
    jboolean include_private) {
...
static jboolean UrlUtilities__IsGoogleSearchUrl(JNIEnv* env,
                                  const JavaParamRef<jclass>& clazz,
                                  const JavaParamRef<jstring>& url) {
  GURL gurl = UrlUtilities__ConvertJavaStringToGURL(env, url);
  if (gurl.is_empty())
...
static jboolean UrlUtilities__IsGoogleHomePageUrl(JNIEnv* env,
                                    const JavaParamRef<jclass>& clazz,
                                    const JavaParamRef<jstring>& url) {
  GURL gurl = UrlUtilities__ConvertJavaStringToGURL(env, url);
  if (gurl.is_empty())
...
static jboolean UrlUtilities__IsUrlWithinScope(JNIEnv* env,
                                 const JavaParamRef<jclass>& clazz,
                                 const JavaParamRef<jstring>& url,
                                 const JavaParamRef<jstring>& scope_url) {
  GURL gurl = UrlUtilities__ConvertJavaStringToGURL(env, url);
  GURL gscope_url = UrlUtilities__ConvertJavaStringToGURL(env, scope_url);
  return gurl.GetOrigin() == gscope_url.GetOrigin() &&
         base::StartsWith(gurl.path(), gscope_url.path(),
                          base::CompareCase::SENSITIVE);
}

static jboolean UrlUtilities__UrlsMatchIgnoringFragments(JNIEnv* env,
                                           const JavaParamRef<jclass>& clazz,
                                           const JavaParamRef<jstring>& url,
                                           const JavaParamRef<jstring>& url2) {
  GURL gurl = UrlUtilities__ConvertJavaStringToGURL(env, url);
  GURL gurl2 = UrlUtilities__ConvertJavaStringToGURL(env, url2);
  if (gurl.is_empty())
...
static jboolean UrlUtilities__UrlsFragmentsDiffer(JNIEnv* env,
                                    const JavaParamRef<jclass>& clazz,
                                    const JavaParamRef<jstring>& url,
                                    const JavaParamRef<jstring>& url2) {
  GURL gurl = UrlUtilities__ConvertJavaStringToGURL(env, url);
...
static jboolean UrlUtilities__IsAcceptedScheme(JNIEnv* env,
                                 const JavaParamRef<jclass>& clazz,
                                 const JavaParamRef<jstring>& url) {
  return CheckSchemeBelongsToList(env, url, g_supported_schemes);
}

static jboolean UrlUtilities__IsValidForIntentFallbackNavigation(
    JNIEnv* env,
    const JavaParamRef<jclass>& clazz,
    const JavaParamRef<jstring>& url) {
...
static jboolean UrlUtilities__IsDownloadable(JNIEnv* env,
                               const JavaParamRef<jclass>& clazz,
                               const JavaParamRef<jstring>& url) {
