...
void SearchGeolocationDisclosureTabHelper::RecordPreDisclosureMetrics(
  ...
  if (!prefs->GetBoolean(
    ...
    // TODO(raymes): We no longer store the content setting separately from the
    // DSE setting. Here we could log the content setting value before the
    // origin became the DSE depending on what we're interested in.
