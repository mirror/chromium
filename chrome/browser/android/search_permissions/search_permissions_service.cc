...
void SearchPermissionsService::OnDSEChanged() {
  ...
  ContentSetting notifications_setting_to_restore = pref.notifications_setting;
...
void SearchPermissionsService::InitializeSettingsIfNeeded() {
  ...
  if (!pref_service_->HasPrefPath(prefs::kDSEPermissionsSettings)) {
    ...
    pref.geolocation_setting = geolocation_setting_to_restore;
    pref.notifications_setting = CONTENT_SETTING_DEFAULT;
    SetDSEPref(pref);
  }

  // Initialize the notifications part of the pref if needed.
  PrefValue pref = GetDSEPref();
  if (pref.notifications_setting == CONTENT_SETTING_DEFAULT) {
