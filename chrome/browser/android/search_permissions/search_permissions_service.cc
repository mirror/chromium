...
void SearchPermissionsService::InitializeSettingsIfNeeded() {
  ...
  // If we get to here, the DSE is no longer disabled by enterprise policy. If
  // it was previously enterprise controlled, we initialize the setting to BLOCK
  // since we don't know what the user's setting was previously.
  ...
  if (!pref_service_->HasPrefPath(prefs::kDSEPermissionsSettings)) {
    ...
    if (pref_service_->HasPrefPath(prefs::kDSEGeolocationSettingDeprecated)) {
      ...
      if (dse_geolocation_setting == CONTENT_SETTING_ASK) {
        dse_geolocation_setting = (dse_setting && !was_enterprise_controlled)
