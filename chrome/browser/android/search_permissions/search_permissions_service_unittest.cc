 ...
 public:
  ...
  void SetContentSetting(const std::string& origin_string,
    ...
    HostContentSettingsMapFactory::GetForProfile(profile())
        ->SetContentSettingDefaultScope(url, url, type, std::string(), setting);
  }
...
TEST_F(SearchPermissionsServiceTest, Migration) {
  ...
  // When there is a conflict between the DSE setting and the user's current
  // content setting, we always take the user's content setting.
  test_delegate()->SetDSEOrigin(kGoogleURL);
  SetContentSetting(kGoogleURL, CONTENT_SETTINGS_TYPE_GEOLOCATION,
...
//   test_delegate()->SetDSEOrigin(kGoogleURL);
//   EXPECT_TRUE(GetService()->ArePermissionsControlledByDSE(ToOrigin(kGoogleURL)));
...
//   EXPECT_FALSE(GetService()->ArePermissionsControlledByDSE(ToOrigin(kGoogleURL)));
//   EXPECT_TRUE(GetService()->ArePermissionsControlledByDSE(ToOrigin(kGoogleAusURL)));
//   EXPECT_TRUE(GetService()->GetDSEGeolocationSetting());
...
//   // Now set the content setting for google.com.au to ALLOW. When we change
//   to
//   // google.com.au, its content setting should be reset and the setting
//   should
//   // still be false.
...
//   // Now set to a non-google search. The setting should still be used, but
//   only
//   // for the non-google search.
//   test_delegate()->SetDSEOrigin(kExampleURL);
//   EXPECT_FALSE(GetService()->ArePermissionsControlledByDSE(ToOrigin(kGoogleAusURL)));
//   EXPECT_TRUE(GetService()->ArePermissionsControlledByDSE(ToOrigin(kExampleURL)));

//   // Go back to google.com.au. The setting should still be false because
//   that's
//   // what it last was.
//   test_delegate()->SetDSEOrigin(kGoogleAusURL);
//   EXPECT_TRUE(GetService()->ArePermissionsControlledByDSE(ToOrigin(kGoogleAusURL)));
