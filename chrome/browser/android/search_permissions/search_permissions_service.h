...
class SearchPermissionsService : public KeyedService {
 public:
  ...
  // Returns the DSE's Origin ArePermissionsControlledByDSE() is true, else an
  // unique Origin.
 ...
 private:
  ...
  //   ASK later.
  // * If the old DSE's permission is ASK, change the DSE permission setting to
  //   BLOCK but restore it to ASK later.
  // * If the user changes the DSE CCTLD origin permission, we restore it back
  //   to ASK when they change DSE.
  // Also, if the DSE changes, we reset the geolocation disclosure so that it
  // will be shown again.
  void OnDSEChanged();
