...
static void JNI_ServiceWorkerPaymentAppBridge_HasServiceWorkerPaymentApps(
    ...
    const JavaParamRef<jclass>& jcaller,
    const JavaParamRef<jobject>& jcallback) {
  // Checks whehter this a installed service worker payment app through
  // GetAllPaymentApps. This could be improved by introducing a dedicated
  // interface in PaymentAppProvider.
