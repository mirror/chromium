...
void OnInvokePaymentAppAborted(const JavaRef<jobject>& jweb_contents,
                               const JavaRef<jobject>& jcallback,
                               bool result) {
  JNIEnv* env = AttachCurrentThread();

  Java_ServiceWorkerPaymentAppBridge_onInvokePaymentAppAborted(env, jcallback,
                                                               result);
}
...
static void AbortInvokePaymentApp(JNIEnv* env,
  ...
  content::PaymentAppProvider::GetInstance()->AbortPayment(
      ...
      base::BindOnce(&OnInvokePaymentAppAborted,
                     ScopedJavaGlobalRef<jobject>(env, jweb_contents),
                     ScopedJavaGlobalRef<jobject>(env, jcallback)));
}
