...
void OnAbortedInvokePaymentApp(const JavaRef<jobject>& jweb_contents,
                               const JavaRef<jobject>& jcallback,
                               bool result) {
  JNIEnv* env = AttachCurrentThread();

  Java_ServiceWorkerPaymentAppBridge_onAbortedInvokePaymentApp(env, jcallback,
                                                               result);
}
...
static void AbortInvokePaymentApp(JNIEnv* env,
                                  const JavaParamRef<jclass>& jcaller,
                                  const JavaParamRef<jobject>& jweb_contents,
                                  jlong registration_id,
                                  const JavaParamRef<jobject>& jcallback) {
  ...
  content::PaymentAppProvider::GetInstance()->AbortPayment(
      ...
      base::BindOnce(&OnAbortedInvokePaymentApp,
                     ScopedJavaGlobalRef<jobject>(env, jweb_contents),
                     ScopedJavaGlobalRef<jobject>(env, jcallback)));
}
