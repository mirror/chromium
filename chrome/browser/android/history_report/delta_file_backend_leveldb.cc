...
DeltaFileBackend::DeltaFileBackend(const base::FilePath& dir)
    ...
    base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProviderWithSequencedTaskRunner(this, "HistoryDeltaFile", base::SequencedTaskRunnerHandle::Get(), MemoryDumpProvider::Options());
}
...
void DeltaFileBackend::OnMemoryDump(const trace_event::MemoryDumpArgs& args,
    base::trace_event::ProcessMemoryDump* pmd) {
  if (!db_ || !db_->is_open())
    return;

  // leveldb_env::DBTracker already records memory usage. Add ownership edge to the dump.
  auto* tracker_dump =
