...
class NearOomMonitor {
 public:
  ...
  // ownership of observers. These methods are thread-safe.
  // OnNearOomDetected() will be called on the same thread on which a observer
  // is registered.
 ...
 private:
  ...
  using NearOomObserverList = base::ObserverListThreadSafe<NearOomObserver>;
  scoped_refptr<NearOomObserverList> observers_;
