...
enum class InterventionUserAction {
  ACCEPTED = 0,
  DECLINED = 1,
  COUNT,
};
...
void RecordInterventionUserAction(InterventionUserAction action) {
  UMA_HISTOGRAM_ENUMERATION("Memory.Experimental.OomIntervention.UserAction",
                            action, InterventionUserAction::COUNT);
}
...
void OomInterventionTabHelper::AcceptIntervention() {
  RecordInterventionUserAction(InterventionUserAction::ACCEPTED);
}

void OomInterventionTabHelper::DeclineIntervention() {
  RecordInterventionUserAction(InterventionUserAction::DECLINED);
  // Don't need to handle response.
  intervention_->OnInterventionDeclined(base::OnceCallback<void()>());
}
...
void OomInterventionTabHelper::OnNearOomDetected() {
  ...
  if (RendererPauseIsEnabled()) {
    ...
    intervention_->OnNearOomDetected(
        base::BindOnce(&OomInterventionTabHelper::OnNearOomDetectedResponse,
                       base::Unretained(this)));
  }
}

void OomInterventionTabHelper::OnNearOomDetectedResponse() {
