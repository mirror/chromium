...
void VrMetricsHelper::SetVrMode(vr::Mode new_mode) {
  ...
  if (new_mode == vr::Mode::kVrBrowsingFullscreen) {
    // If we are entering fullscreen note that on the page.
    ukm::UkmRecorder* ukm_recorder = ukm::UkmRecorder::Get();
    DCHECK(ukm_recorder);
...
void VrMetricsHelper::DidStartNavigation(content::NavigationHandle* handle) {
  if (handle && handle->IsInMainFrame() && !handle->IsSameDocument()) {
    ukm::UkmRecorder* ukm_recorder = ukm::UkmRecorder::Get();
    DCHECK(ukm_recorder);
    // Record ukm with an id from the handle so that we don't wind up with race
    // conditions from the uncertain order in which WebContentsObservers'
    // DidStartNavigations execute.
    ukm::builders::XR_PageSession(
