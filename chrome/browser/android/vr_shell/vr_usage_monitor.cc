...
enum SessionEventName {
  ...
  MODE_FULLSCREEN_WHEN_AUTOPRESENTED_WEBVR,
  MODE_BROWSER_WHEN_AUTOPRESENTED_WEBVR,
  MODE_WEBVR_WHEN_AUTOPRESENTED_WEBVR,
  SESSION_VR_WHEN_AUTOPRESENTED_WEBVR,
};
...
const char* HistogramNameFromSessionType(SessionEventName name) {
  ...
  static constexpr char kVrSessionWhenAutopresentedWebVr[] =
      "VRSessionWhenWebVrAutopresentedTime";
  static constexpr char kWebVrWhenAutopresentedWebVr[] =
      "VRSessionWhenWebVrAutopresentedTime.WebVR";
  static constexpr char kBrowserWhenAutopresentedWebVr[] =
      "VRSessionWhenWebVrAutopresentedTime.Browser";
  static constexpr char kFullscreenWhenAutopresentedWebVr[] =
      "VRSessionWhenWebVrAutopresentedTime.Fullscreen";
  ...
  switch (name) {
    ...
    case MODE_FULLSCREEN_WHEN_AUTOPRESENTED_WEBVR:
      return kFullscreenWhenAutopresentedWebVr;
    case MODE_BROWSER_WHEN_AUTOPRESENTED_WEBVR:
      return kBrowserWhenAutopresentedWebVr;
    case MODE_WEBVR_WHEN_AUTOPRESENTED_WEBVR:
      return kWebVrWhenAutopresentedWebVr;
    case SESSION_VR_WHEN_AUTOPRESENTED_WEBVR:
      return kVrSessionWhenAutopresentedWebVr;
    default:
      NOTREACHED();
      return nullptr;
  }
    ...
    switch (mode) {
      case VRMode::WEBVR:
        if (started_with_autopresentation_) {
          mode_timer_ = base::MakeUnique<
              SessionTimerImpl<MODE_WEBVR_WHEN_AUTOPRESENTED_WEBVR>>(
      ...
      case VRMode::VR_BROWSER:
        if (started_with_autopresentation_) {
          mode_timer_ = base::MakeUnique<
              SessionTimerImpl<MODE_BROWSER_WHEN_AUTOPRESENTED_WEBVR>>(
              kMaximumHeadsetSessionGap, kMinimumHeadsetSessionDuration);
        } else {
      ...
      case VRMode::VR_FULLSCREEN:
        if (started_with_autopresentation_) {
          mode_timer_ = base::MakeUnique<
              SessionTimerImpl<MODE_FULLSCREEN_WHEN_AUTOPRESENTED_WEBVR>>(
              kMaximumHeadsetSessionGap, kMinimumHeadsetSessionDuration);
        } else {
...
VrMetricsHelper::VrMetricsHelper(content::WebContents* contents,
  ...
  if (started_with_autopresentation) {
    session_timer_ =
        base::MakeUnique<SessionTimerImpl<SESSION_VR_WHEN_AUTOPRESENTED_WEBVR>>(
