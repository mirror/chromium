...
gfx::Vector3dF Slerp(const gfx::Vector3dF& a,
  ...
  b.GetNormalized(&end);
  float dot =
      gfx::MathUtil::ClampToRange(gfx::DotProduct(start, end), -1.0f, 1.0f);
...
void ElbowModel::UpdateTorsoDirection(const UpdateData& update) {
  ...
  float gaze_filter_strength = gfx::MathUtil::ClampToRange(
...
void ElbowModel::ApplyArmModel(const UpdateData& update) {
  ...
  float extension_ratio =
      gfx::MathUtil::ClampToRange(normalized_angle, 0.0f, 1.0f);
