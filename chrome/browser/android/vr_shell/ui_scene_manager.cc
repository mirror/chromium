...
static constexpr float kIndicatorWidth = 0.5;
static constexpr float kIndicatorHeight = 0;

static constexpr float kCaptureIndicatorsVerticalOffset = 0.7;
...
void UiSceneManager::CreateSystemIndicators() {
  ...
  element->set_translation({kAudioCaptureHorizontalOffset,
                            ...
                            -kContentDistance + kIndicatorContentDistance});
  element->set_size({kIndicatorWidth, kIndicatorHeight, 1});
  element->set_visible(false);
