...
void CreateKeyboardTask(
  ...
  DCHECK(!keyboard_ptr_for_creation_task);
  gvr_keyboard_context* keyboard = nullptr;
  if (load_gvr_sdk()) {
    // This call is blocking and takes about 0.5s.
    keyboard = gvr_keyboard_create(callback, OnKeyboardEvent);
  }
...
void GvrKeyboardDelegate::OnKeyboardCreated(gvr_keyboard_context* context) {
  ...
  if (!gvr_keyboard_) {
    LOG(ERROR) << "Failed to create gvr keyboard.";
    return;
  }
...
void GvrKeyboardDelegate::SetRenderingEnabled(bool enabled) {
...
bool GvrKeyboardDelegate::OnGlThread() const {
