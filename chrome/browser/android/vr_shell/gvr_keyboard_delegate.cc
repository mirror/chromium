...
GvrKeyboardDelegate::GvrKeyboardDelegate(VrController* controller,
                                         InputEditCallback input_edit_callback)
    : controller_(controller), input_edit_callback_(input_edit_callback) {
  keyboard_event_callback_ = base::Bind(
...
void GvrKeyboardDelegate::SetTransform(gfx::Transform transform) {
  gvr_mat4f matrix;
...
void GvrKeyboardDelegate::EditInput(const vr::TextInputInfo& info) {
  LOG(ERROR) << "lolk GvrKeyboardDelegate::EditInput: "
             << base::UTF16ToUTF8(info.text).c_str();
  ...
  gvr_keyboard_set_selection_indices(gvr_keyboard_, info.selection_start,
                                     info.selection_end);
  gvr_keyboard_set_selection_indices(gvr_keyboard_, info.composition_start,
                                     info.composition_end);
  input_edit_callback_.Run(info);
}

void GvrKeyboardDelegate::OnInputEdited() {
