...
void CreateKeyboardTask(
  ...
  if (keyboard) {
    task_runner->PostTask(
        FROM_HERE,
        base::BindOnce(&GvrKeyboardDelegate::OnKeyboardCreated,
                       base::Unretained(delegate), base::Unretained(keyboard)));
  }
}

void DestroyKeyboardTask(gvr_keyboard_context* keyboard) {
  if (!keyboard)
    return;
  gvr_keyboard_destroy(&keyboard);
}
...
void GvrKeyboardDelegate::OnKeyboardCreated(gvr_keyboard_context* context) {
  ...
  if (pending_destroy_keyboard_)
    DestroyKeyboard();

  pending_show_keyboard_ = false;
  pending_destroy_keyboard_ = false;
}
...
void GvrKeyboardDelegate::CreateKeyboard() {
  ...
  creating_keyboard_ = true;
}

void GvrKeyboardDelegate::DestroyKeyboard() {
  if (!gvr_keyboard_) {
    if (creating_keyboard_)
      pending_destroy_keyboard_ = true;
    return;
  }

  gvr_keyboard_context* keyboard = gvr_keyboard_;
  base::PostTaskWithTraits(
      FROM_HERE, {base::TaskPriority::USER_VISIBLE},
      base::BindOnce(DestroyKeyboardTask, base::Unretained(keyboard)));
