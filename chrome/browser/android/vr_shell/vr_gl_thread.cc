...
void VrGLThread::OnWebInputEdited(const vr::TextInputInfo& info) {
  DCHECK(OnGlThread());
  main_thread_task_runner_->PostTask(
      FROM_HERE,
      base::BindOnce(&VrShell::OnWebInputEdited, weak_vr_shell_, info));
}

void VrGLThread::OnWebInputCommitted(const vr::TextInputInfo& info) {
  DCHECK(OnGlThread());
  main_thread_task_runner_->PostTask(
      FROM_HERE,
      base::BindOnce(&VrShell::OnWebInputCommitted, weak_vr_shell_, info));
}
