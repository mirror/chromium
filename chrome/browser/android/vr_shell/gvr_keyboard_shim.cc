...
//#include "base/android/context.h"
...
void close_sdk() {
  if (sdk_handle == nullptr)
    return;

  JNIEnv* env = base::android::AttachCurrentThread();
  CHECK(env);

  vr_shell::Java_GvrKeyboardLoaderClient_closeKeyboardSDK(env,
                                                          (jlong)sdk_handle);
...
bool load_sdk(void* closure, gvr_keyboard_callback callback) {
  if (sdk_handle != nullptr)
    return true;
  ...
  sdk_handle =
      (void*)vr_shell::Java_GvrKeyboardLoaderClient_loadKeyboardSDK(env);
...
#undef CALL
  ...
  impl_gvr_keyboard_initialize(env, context_wrapper.obj(),
...
void gvr_keyboard_initialize(JNIEnv* env,
  ...
  // Initialize the JVM environment for Android.
  JavaVM* vm = nullptr;
  env->GetJavaVM(&vm);
  if (!vm) {
    return;
