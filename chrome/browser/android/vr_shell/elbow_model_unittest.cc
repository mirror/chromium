...
struct ElbowModelState {
  ...
  gfx::Quaternion wrist_rotation;
  float alpha_value;
  gfx::Point3F elbow_position;
  gfx::Quaternion elbow_rotation;
  gfx::Point3F shoulder_position;
  gfx::Quaternion shoulder_rotation;
...
TEST(ElbowModelTest, Update) {
  struct {
    ...
    gfx::Quaternion expected_controller_rotation;
  } test_cases[] = {
  ...
  for (size_t i = 0; i < arraysize(test_cases); ++i) {
    ...
    const gfx::Quaternion& q = elbow_model.GetControllerRotation();
