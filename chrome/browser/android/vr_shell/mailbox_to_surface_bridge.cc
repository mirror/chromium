...
void MailboxToSurfaceBridge::OnContextAvailable(
  ...
  InitializeRenderer();
  LOG(INFO) << __FUNCTION__ << ";;; on_initialized_=" << on_initialized_.get();
  if (on_initialized_) {
...
void MailboxToSurfaceBridge::CreateGpuFence(
  ...
  // gl_->OrderingBarrierCHROMIUM(); // slow/fast pattern?
  CHECK_ERR;
#if 0
  gl_->ShallowFlushCHROMIUM();  // TODO(klausw): needed? Seems to avoid leftover
                                // tearing.
  CHECK_ERR;
#endif
  GLuint id = gl_->CreateGpuFenceCHROMIUM();
  CHECK_ERR;
  //gl_->ShallowFlushCHROMIUM();  // FIXME: should not be needed. Fix sequencing.
  CHECK_ERR;
  context_support_->GetGpuFenceHandle(id, callback);
  CHECK_ERR;
  gl_->DestroyGpuFenceCHROMIUM(id);
  CHECK_ERR;
#if 1
  gl_->ShallowFlushCHROMIUM();  // Definitely needed, bad tearing without it.
  CHECK_ERR;
#endif
}
...
void MailboxToSurfaceBridge::InsertGpuFence(
  ...
  GLuint id = gl_->DuplicateGpuFenceCHROMIUM(gpu_fence.AsClientGpuFence());
  CHECK_ERR;
  //gl_->ShallowFlushCHROMIUM();  // FIXME: should not be needed. Fix sequencing.
  CHECK_ERR;
