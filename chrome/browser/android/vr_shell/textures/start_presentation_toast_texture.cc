...
#include "chrome/browser/android/vr_shell/textures/start_presentation_toast_texture.h"
...
StartPresentationToastTexture::StartPresentationToastTexture() = default;

StartPresentationToastTexture::~StartPresentationToastTexture() = default;

void StartPresentationToastTexture::Draw(SkCanvas* sk_canvas,
  ...
  paint.setColor(color_scheme().start_presentation_toast_background);
  auto text =
      l10n_util::GetStringUTF16(IDS_PRESENTATION_PRESS_APP_TO_EXIT_PRESENTATION);
  gfx::FontList fonts;
  ...
  std::vector<std::unique_ptr<gfx::RenderText>> lines = PrepareDrawStringRect(
      text, fonts, color_scheme().start_presentation_toast_foreground, &text_size,
  ...
  // why height ? I have changed to width
...
gfx::Size StartPresentationToastTexture::GetPreferredTextureSize(
    int maximum_width) const {
  return gfx::Size(maximum_width, maximum_width);
}

gfx::SizeF StartPresentationToastTexture::GetDrawnSize() const {
  return size_;
}
