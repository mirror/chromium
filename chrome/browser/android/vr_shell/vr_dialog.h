...
class VrDialog : public vr::ContentInputDelegate {
 public:
  VrDialog(JNIEnv* env, jobject obj, int width, int height);
  ~VrDialog() override;

  void CreateVrDialog(JNIEnv* env, jobject obj);

 private:
  base::android::ScopedJavaGlobalRef<jobject> vr_dialog_;
  ...
  void OnContentScrollEnd(std::unique_ptr<blink::WebGestureEvent> gesture,
                          const gfx::PointF& normalized_hit_point) override;
  base::android::ScopedJavaGlobalRef<jobject> java_vr_dialog_;
