...
void VrShellGl::DrawFrame(int16_t frame_index) {
  ...
  // TODO(mthiesse): Refine this notion of dirtiness. If only a portion of the
  // screen is dirtied, we can update just that portion.
  if (controller_dirty_ || scene_changed || textures_changed ||
  ...
  if (std::abs(angle) > kRedrawSceneAngleDeltaDegrees ||
      last_used_head_pose_.IsIdentity()) {
    dirty = true;
  }
