...
VrShellGl::VrShellGl(GlBrowserInterface* browser_interface,
    ...
    : ui_instance_(
          base::MakeUnique<vr::Ui>(ui_host_interface, this, ui_initial_state)),
      ui_(ui_instance_->ui()),
...
void VrShellGl::InitializeGl(gfx::AcceleratedWidget window) {
  ...
  ui_instance_->OnGlInitialized(content_texture_id);
...
void VrShellGl::HandleControllerInput(const gfx::Vector3dF& head_direction) {
  ...
  ui_instance_->input_manager()->HandleInput(
...
void VrShellGl::DrawFrame(int16_t frame_index) {
  ...
  ui_instance_->ui_renderer()->Draw(render_info_primary_, controller_info_,
                                    ShouldDrawWebVr());
  frame.Unbind();
  ...
  if (!overlay_elements.empty() && ShouldDrawWebVr()) {
    ...
    ui_instance_->ui_renderer()->DrawWebVrOverlayForeground(
        render_info_webvr_browser_ui, controller_info_);
...
bool VrShellGl::ShouldDrawWebVr() {
  return web_vr_mode_ && ShouldDrawWebVr();
}
...
void VrShellGl::DrawWebVr() {
  ...
  ui_instance_->vr_shell_renderer()->GetWebVrRenderer()->Draw(
      webvr_texture_id_);
}
...
void VrShellGl::SetWebVrMode(bool enabled, bool show_toast) {
...
base::WeakPtr<vr::BrowserUiInterface> VrShellGl::GetBrowserUiWeakPtr() {
  return ui_instance_->GetBrowserUiWeakPtr();
}

void VrShellGl::SetControllerModel(
    std::unique_ptr<vr::VrControllerModel> model) {
  ui_instance_->vr_shell_renderer()->GetControllerRenderer()->SetUp(
      std::move(model));
}
