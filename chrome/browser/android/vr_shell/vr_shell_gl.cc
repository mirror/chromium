...
void VrShellGl::DrawFrame(int16_t frame_index) {
  ...
  std::unique_ptr<gl::GLFence> vrshell_submit_fence;
  ...
  task_runner_->PostDelayedTask(
      ...
      base::Bind(&VrShellGl::DrawFrameSubmitWhenReady,
                 ...
                 head_pose, std::move(vrshell_submit_fence)),
      initial_delay);
}
...
void VrShellGl::DrawFrameSubmitWhenReady(
    ...
    std::unique_ptr<gl::GLFence> vrshell_submit_fence) {
  if (vrshell_submit_fence && !vrshell_submit_fence->HasCompleted()) {
    task_runner_->PostDelayedTask(
        FROM_HERE,
        base::Bind(&VrShellGl::DrawFrameSubmitWhenReady,
                   weak_ptr_factory_.GetWeakPtr(), frame_index, frame_ptr,
                   head_pose, vrshell_submit_fence),
