...
void VrShellGl::InitializeGl(gfx::AcceleratedWidget window) {
  ...
  bbb_ = base::MakeUnique<UiInputManager>(scene_, this);
}
...
void VrShellGl::OnEnterWebContents(const gfx::PointF& normalized_hit_point) {
  normalized_content_hit_point_ = normalized_hit_point;
  SendGestureToContent(
      MakeMouseEvent(blink::WebInputEvent::kMouseEnter, normalized_hit_point));
}

void VrShellGl::OnExitWebContents() {
  normalized_content_hit_point_ = {};
  SendGestureToContent(
      MakeMouseEvent(blink::WebInputEvent::kMouseLeave, gfx::PointF()));
}

void VrShellGl::OnMoveOnWebContents(const gfx::PointF& normalized_hit_point) {
  normalized_content_hit_point_ = normalized_hit_point;
  SendGestureToContent(
      MakeMouseEvent(blink::WebInputEvent::kMouseMove, normalized_hit_point));
}

void VrShellGl::OnDownOnWebContents(const gfx::PointF& normalized_hit_point) {
  in_click_ = true;
  SendGestureToContent(
      MakeMouseEvent(blink::WebInputEvent::kMouseDown, normalized_hit_point));
}

void VrShellGl::OnUpOnWebContents(const gfx::PointF& normalized_hit_point) {
  in_click_ = false;
  SendGestureToContent(
...
void VrShellGl::HandleControllerInput(const gfx::Vector3dF& head_direction) {
  ...
  UiInputManager::ButtonState controller_button_state =
      UiInputManager::ButtonState::UNDEFINED;
  if (touch_pending_ ||
    ...
    controller_button_state = UiInputManager::ButtonState::CLICK;
    touch_pending_ = false;
  } else if (controller_->ButtonState(gvr::kControllerButtonClick)) {
    controller_button_state = UiInputManager::ButtonState::DOWN;
  } else {
    controller_button_state = UiInputManager::ButtonState::UP;
  }
  bbb_->UpdateController(controller_direction, pointer_start_,
                         controller_button_state);
  target_point_ = bbb_->GetTargetPoint();
  reticle_render_target_ = bbb_->GetReticleRenderTarget();
  ...
  if (normalized_content_hit_point_) {
    local_point_pixels.set_x(content_tex_css_width_ *
                             normalized_content_hit_point_->x());
    local_point_pixels.set_y(content_tex_css_height_ *
                             normalized_content_hit_point_->y());
  }

  // Process scroll and fling events on
  ...
  if (!in_click_) {
    ...
    if (!SendScrollBegin(normalized_content_hit_point_.has_value(),
                         gesture_list)) {
...
bool VrShellGl::SendScrollBegin(bool is_targeting_content,
                                GestureList& gesture_list) {
  if (in_scroll_)
    return false;
  // Scrolling currently only supported on content window.
  if (!is_targeting_content)
    return false;
  if (gesture_list.empty() || (gesture_list.front()->GetType() !=
