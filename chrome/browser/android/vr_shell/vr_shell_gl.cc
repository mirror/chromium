...
void VrShellGl::SetAlertDialogEnabled(
    bool enabled,
    vr::ContentInputForwarder* input_forwarder,
    int width,
    int height) {
  if (enabled) {
    vr_dialog_.reset(new VrDialog(width, height));
    vr_dialog_->SetEventForwarder(input_forwarder);
    ui_->SetAlertDialogEnabled(enabled, vr_dialog_.get(), width, height);
  } else {
    ui_->SetAlertDialogEnabled(enabled, nullptr, width, height);
    vr_dialog_ = nullptr;
  }
}
