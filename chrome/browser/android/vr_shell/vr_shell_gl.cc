...
void VrShellGl::DrawFrame(int16_t frame_index) {
  ...
  if (!draw_frame_phase_two_.IsCancelled()) {
    draw_frame_phase_two_.Cancel();
...
void VrShellGl::DrawIntoAcquiredFrame(int16_t frame_index) {
  ...
  if (ShouldDrawWebVr() && surfaceless_rendering_) {
    ...
    draw_frame_phase_two_.Reset(base::Bind(&VrShellGl::DrawFrameSubmitWhenReady,
                                           base::Unretained(this)));
    task_runner_->PostTask(
        FROM_HERE,
        base::BindOnce(draw_frame_phase_two_.callback(), frame_index,
...
void VrShellGl::DrawFrameSubmitWhenReady(
  ...
  if (fence) {
    ...
    if (!fence->HasCompleted()) {
      draw_frame_phase_two_.Reset(base::Bind(
