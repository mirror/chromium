...
void VrShellGl::ResizeForWebVR(int16_t frame_index,
                               bool* is_valid,
                               bool* was_changed) {
...
void VrShellGl::DrawFrame(int16_t frame_index) {
  ...
  if (ShouldDrawWebVr()) {
    bool is_valid;
    ResizeForWebVR(frame_index, &is_valid, nullptr);
    if (!is_valid) {
      return;
    }
...
void VrShellGl::DrawWebVr(int16_t frame_index) {
  ...
  vr_shell_renderer_->GetWebVrRenderer()->Draw(webvr_texture_id_, ySign);
  if (server_fence)
...
void VrShellGl::SendVSync(base::TimeTicks time, GetVSyncCallback callback) {
  ...
  if (frame_index_ == 12 && !WebVrSharedBufferDraw()) {
    for (int i = 0; i < 4; ++i) {
      std::unique_ptr<gpu::Mailbox> mailbox = base::MakeUnique<gpu::Mailbox>();
      mailbox_bridge_->GenerateMailbox(*mailbox);
      mailbox_bridge_->CreateSharedBuffer(*mailbox);
      webvr_sharedbuffer_mailboxes_.push_back(std::move(mailbox));
      ...
      *bufptr = buf;
      ...
      // Initialize images, to be filled in later
      webvr_bufferimages_.push_back(nullptr);
    }
  }

  base::Optional<gfx::GpuMemoryBufferHandle> opt_buffer = base::nullopt;

  if (WebVrSharedBufferDraw()) {
    int mailbox_idx = frame_index_ % 4;
    gfx::GpuMemoryBufferHandle draw_mailbox_handle =
        gpu::GpuMemoryBufferImplMailboxSharedBuffer::GetHandleForMailbox(
            *webvr_sharedbuffer_mailboxes_[mailbox_idx],
            gfx::GpuMemoryBufferId());
    opt_buffer = draw_mailbox_handle;
    mailbox_bridge_->SetSharedBufferHandle(
        *webvr_sharedbuffer_mailboxes_[mailbox_idx],
        gpu::GpuMemoryBufferImplAndroidHardwareBuffer::
            GetHandleFromRawAHardwareBuffer(
                webvr_sharedbuffers_[mailbox_idx]->get()));
  }
