...
void VrShellGl::InitializeGl(gfx::AcceleratedWidget window) {
  ...
  browser_->DialogSurfaceChanged(ui_surface_->j_surface().obj());
...
void VrShellGl::BufferBoundsChanged(const gfx::Size& content_buffer_size,
  ...
  if (content_surface_texture_.get()) {
    ...
    ui_surface_texture_->SetDefaultBufferSize(content_buffer_size.width(),
                                              content_buffer_size.height());
  }
...
void VrShellGl::UpdateUiTextureSize(int width, int height) {
