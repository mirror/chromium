...
void OnKeyboardEvent(void*, int32_t event) {
  ...
  switch (event) {
    ...
    case GVR_KEYBOARD_SHOWN:
      LOG(ERROR) << ("lmao GVR_KEYBOARD_SHOWN");
      break;
    case GVR_KEYBOARD_HIDDEN:
      LOG(ERROR) << ("lmao GVR_KEYBOARD_HIDDEN");
      break;
    case GVR_KEYBOARD_TEXT_UPDATED: {
      char* text = gvr_keyboard_get_text(keyboard_context);
      LOG(ERROR) << "lmao GVR_KEYBOARD_TEXT_UPDATED: " << text;
      free(reinterpret_cast<void*>(text));
    } break;
    case GVR_KEYBOARD_TEXT_COMMITTED: {
      char* t = gvr_keyboard_get_text(keyboard_context);
      if (vr_shell_gl != nullptr) {
        std::string text(t);
        vr_shell_gl->CommitText(text, 1);
        LOG(ERROR) << ("lmao GVR_KEYBOARD_TEXT_COMMITTED: ") << t;
      }
...
void VrShellGl::ShowSoftInput() {
  LOG(ERROR) << "lolk VrShellGl::ShowSoftInput";
  keyboard_enabled_ = true;
}

void VrShellGl::CommitText(std::string& text, int new_cursor_position) {
  browser_->CommitText(text, new_cursor_position);
}
...
void VrShellGl::CreateKeyboard() {
  ...
  /*for (int i = 0; i < 4; i++) {
    ...
    for (int j = 0; j < 4; j++) {
      if (matrix.m[i][j] < 0) {
        matrix.m[i][j] = matrix.m[i][j] * 2;
      }
      s = s + std::to_string((matrix.m)[i][j]) + " ";
    }
    LOG(ERROR) << "VK: " << s;
  }*/
