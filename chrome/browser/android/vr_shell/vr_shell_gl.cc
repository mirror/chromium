...
// The webvr_browser_ui buffer is for UI that should not use reprojection.
...
void VrShellGl::DrawFrame(int16_t frame_index) {
  ...
  if (ShouldDrawWebVr()) {
    ...
    if (render_info_primary_.content_texture_size != render_size_default_) {
      render_info_primary_.content_texture_size = render_size_default_;
      swap_chain_->ResizeBuffer(
          kFramePrimaryBuffer,
          {render_info_primary_.content_texture_size.width(),
           render_info_primary_.content_texture_size.height()});
  ...
  if (scene_->HasVisibleViewportAwareElements()) {
    ...
    UpdateEyeInfos(render_info_webvr_browser_ui_.head_pose,
                   kViewportListWebVRBrowserUIOffset, render_size_default_,
