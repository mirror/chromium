...
VrShellGl::VrShellGl(GlBrowserInterface* browser_interface,
    ...
    : ui_(base::MakeUnique<vr::Ui>(ui_host_interface, this, ui_initial_state)),
      ...
      keyboard_enabled_(
          base::FeatureList::IsEnabled(features::kVrShellKeyboard)),
...
void VrShellGl::DrawKeyboard() {
  ...
  // TODO(ymalik): Keyboard hittest needs the start and endpoint, so the hittest
  // fails for now.
  gvr_vec3f reticle_point;
  reticle_point.x = 0;
  reticle_point.y = 0;
  reticle_point.z = 0;
  gvr_vec3f hit_point;
  gvr_keyboard_update_controller_ray(gvr_keyboard_, &reticle_point,
                                     &reticle_point, &hit_point);
  for (auto eye : {GVR_LEFT_EYE, GVR_RIGHT_EYE}) {
