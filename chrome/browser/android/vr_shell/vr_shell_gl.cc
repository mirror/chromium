...
VrShellGl::VrShellGl(GlBrowserInterface* browser_interface,
    ...
    : ui_(std::move(ui)),
      ...
      webvr_render_time_(new vr::SlidingTimeDeltaAverage(
          kWebVRSlidingAverageSize)),
      webvr_js_wait_time_(new vr::SlidingTimeDeltaAverage(
          kWebVRSlidingAverageSize)),
      webvr_acquire_time_(new vr::SlidingTimeDeltaAverage(
          kWebVRSlidingAverageSize)),
      webvr_submit_time_(new vr::SlidingTimeDeltaAverage(
          kWebVRSlidingAverageSize)),
...
void VrShellGl::SubmitFrame(int16_t frame_index,
  ...
  LOG(INFO) << __FUNCTION__ << ";;; wait_time_seconds=" << wait_time_seconds;
  base::TimeDelta js_wait_time = base::TimeDelta::FromSecondsD(wait_time_seconds);
  webvr_js_wait_time_->AddSample(js_wait_time);
  TRACE_COUNTER1("gpu", "WebVR JS wait (ms)", webvr_js_wait_time_->GetAverage().InMilliseconds());
...
void VrShellGl::DrawFrame(int16_t frame_index, base::TimeTicks current_time) {
  ...
  webvr_acquire_time_->AddSample(last_acquire_time);
...
void VrShellGl::DrawFrameSubmitNow(int16_t frame_index,
  ...
  TransformToGvrMat(head_pose, &mat);
  {
    ...
    webvr_submit_time_->AddSample(submit_elapsed);
      ...
      if (submit_elapsed > base::TimeDelta::FromMilliseconds(2)) {
        ...
        base::TimeDelta prev_render_delta =
            webvr_render_time_->GetAverageOrDefault(
                vsync_helper_.AverageVSyncInterval()) *
...
base::TimeDelta VrShellGl::GetPredictedFrameTime() {
  ...
  TRACE_COUNTER2("gpu", "GVR frame time (ms)", "acquire",
                 webvr_acquire_time_->GetAverage().InMilliseconds(),
                 "submit",
...
bool VrShellGl::ShouldSkipVSync() {
  ...
  base::TimeDelta frame_interval = vsync_helper_.AverageVSyncInterval();
  ...
  if (!prev_js_submit.is_null() &&
      webvr_submit_time_->GetAverage() >= kWebVrSlowSubmitThreshold) {
    base::TimeDelta mean_js_time = webvr_js_time_->GetAverage();
    base::TimeDelta mean_wait_time = webvr_js_wait_time_->GetAverage();
    ...
    if (this_render_time - prev_render_time_left < 0.5 * mean_render_time + mean_wait_time) {
  ...
  if (webvr_unstuff_ratelimit_frames_ > 0) {
    ...
    LOG(INFO) << __FUNCTION__ << ";;; SKIP FRAME FAST" <<
        " mean_render_time=" << mean_render_time.InMillisecondsF() << "ms" <<
        " kWebVrFrameInterval=" << frame_interval.InMillisecondsF() << "ms";
