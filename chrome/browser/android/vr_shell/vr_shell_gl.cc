...
void VrShellGl::InitializeGl(gfx::AcceleratedWidget window) {
  ...
  ui_renderer_ = base::MakeUnique<UiRenderer>(scene_, content_texture_id_,
...
void VrShellGl::HandleControllerInput(const gfx::Vector3dF& head_direction) {
  ...
  controller_info_.touchpad_button_state = UiInputManager::ButtonState::UP;
  ...
  if (touch_pending_) {
    ...
    controller_info_.touchpad_button_state = UiInputManager::ButtonState::DOWN;
  }
...
void VrShellGl::DrawFrame(int16_t frame_index) {
  ...
  UpdateEyeInfos(render_info_primary_.head_pose, kViewportListPrimaryOffset,
                 render_size_primary_, &render_info_primary_);
...
void VrShellGl::UpdateEyeInfos(const gfx::Transform& head_pose,
  ...
  for (auto eye : {GVR_LEFT_EYE, GVR_RIGHT_EYE}) {
    RenderInfo::EyeInfo& eye_info = (eye == GVR_LEFT_EYE)
