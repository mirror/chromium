...
void VrShell::OnContentScreenBoundsChanged(const gfx::SizeF& bounds) {
  ...
  int eye_pixel_length = (eye_pixel_width < display_pixel_size_.height())
  ...
  float eye_physical_length_m =
      (eye_physical_width_m < display_physical_size_m_.height())
          ? eye_physical_width_m
          : display_physical_size_m_.height();

  // Calculate the virtual window size and DPR and pass this to VrShellImpl.
  gfx::SizeF window_size_f(bounds.width(), bounds.height());
