...
void VrShell::OnUnsupportedMode(vr::UiUnsupportedMode mode) {
  switch (mode) {
    ...
    case vr::UiUnsupportedMode::kVoiceInputNeedsAndroidPermission: {
      JNIEnv* env = base::android::AttachCurrentThread();
...
void VrShell::OnExitVrPromptResult(vr::UiUnsupportedMode reason,
  ...
  if (reason == vr::UiUnsupportedMode::kVoiceInputNeedsAndroidPermission) {
    ...
    UMA_HISTOGRAM_BOOLEAN("VR.VoiceInputAudioPermission.ExitVRChoice",
...
void VrShell::SetVoiceSearchActive(bool active) {
  ...
  if (!HasAudioPermission()) {
    OnUnsupportedMode(vr::UiUnsupportedMode::kVoiceInputNeedsAndroidPermission);
    return;
  }
