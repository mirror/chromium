...
// In practice, making this factor smaller increases the text size.
...
VrShell::VrShell(JNIEnv* env,
                 ...
                 float display_physical_height_m,
...
void VrShell::OnContentScreenBoundsChanged(const gfx::SizeF& bounds) {
  ...
  int pixel_width = display_pixel_size_.width() / 2;
  float physical_width_m = display_physical_size_m_.width() / 2;
  ...
  int pixel_length = std::max(pixel_width, display_pixel_size_.height());

  // For the size err on the side of a too small area so that the font size is
  // rather too big than too small.
  float physical_length_m =
      std::min(physical_width_m, display_physical_size_m_.height());

  // Calculate the virtual window size and DPR and pass this to VrShellImpl.
  gfx::SizeF window_size_f(bounds);
  window_size_f.Scale(physical_width_m * kContentLegibilityFactor);
  ...
  float dpr = (pixel_length / (physical_length_m * kContentLegibilityFactor)) *
