...
void DevToolsAgentHostAndroid::DispatchProtocolMessage(
  ...
  if (dict->GetInteger("id", &id)) {
    ...
    ScopedJavaGlobalRef<jobject> callback = it->second;
    pending_.erase(it);
    ...
    if (value) {
      ...
      if (base::JSONWriter::Write(*value, &json)) {
        result = ConvertUTF8ToJavaString(env, json);
      } else {
        error = ConvertUTF8ToJavaString(env, "JSONWriter failed");
      }
    } else {
      result = ConvertUTF8ToJavaString(env, "{}");
    }

    Java_DevToolsAgentHostImpl_onCommandResult(env, callback, result, error);
  } else {
