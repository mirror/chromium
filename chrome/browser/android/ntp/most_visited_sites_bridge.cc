...
void SplitTileVector(const NTPTilesVector& tiles,
...
void MostVisitedSitesBridge::JavaObserver::OnMostVisitedURLsAvailable(
  ...
  SplitTileVector(tiles, &titles, &urls, &whitelist_icons, &sources);
...
void MostVisitedSitesBridge::JavaObserver::OnExplorationTilesAvailable(
  ...
  titles.reserve(kTotalTileCount);
  urls.reserve(kTotalTileCount);
  whitelist_icons.reserve(kTotalTileCount);
  sources.reserve(kTotalTileCount);
  section_types.reserve(kTotalTileCount);
