...
void SplitTileVector(const NTPTilesVector& tiles,
  ...
  out_titles->reserve(tiles.size());
  out_urls->reserve(tiles.size());
  out_whitelist_icons->reserve(tiles.size());
  out_sources->reserve(tiles.size());
...
void MostVisitedSitesBridge::JavaObserver::OnExplorationSectionsAvailable(
  ...
  titles.reserve(sections.size());
  types.reserve(sections.size());
