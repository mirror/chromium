...
base::android::ApplicationState*
    content_suggestions_notification_application_state_for_testing = nullptr;
...
bool ShouldNotifyInState(base::android::ApplicationState state) {
  if (content_suggestions_notification_application_state_for_testing) {
    return *content_suggestions_notification_application_state_for_testing;
  }
  switch (state) {
 ...
 public:
  ...
  void OnNewSuggestions(Category category) override {
    if (!ShouldNotifyInState(app_status_listener_.GetState())) {
      ...
      return;
    } else if (!AreNotificationsEnabled(prefs_)) {
