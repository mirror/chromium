...
extern ApplicationState*
    content_suggestions_notification_application_state_for_testing;
...
class MockContentSuggestionsNotifier : public ContentSuggestionsNotifier {
 public:
  ...
  MOCK_METHOD0(RegisterChannel, void());
...
TEST_F(ContentSuggestionsNotifierServiceTest, StartupEnabled) {
  testing::InSequence s;
  EXPECT_CALL(notifier_, FlushCachedMetrics());
  EXPECT_CALL(notifier_, RegisterChannel());
  ContentSuggestionsNotifierService service(prefs_.get(), &suggestions_,
                                            std::move(notifier_ownership_));
}

TEST_F(ContentSuggestionsNotifierServiceTest, StartupDisabled) {
  prefs_->SetBoolean(prefs::kContentSuggestionsNotificationsEnabled, false);

  testing::InSequence s;
  EXPECT_CALL(notifier_, FlushCachedMetrics());
  EXPECT_CALL(notifier_, UnregisterChannel());
  ContentSuggestionsNotifierService service(prefs_.get(), &suggestions_,
                                            std::move(notifier_ownership_));
}

TEST_F(ContentSuggestionsNotifierServiceTest, Toggle) {
  testing::InSequence s;
  EXPECT_CALL(notifier_, FlushCachedMetrics());
  EXPECT_CALL(notifier_, RegisterChannel());
  EXPECT_CALL(notifier_, UnregisterChannel());
