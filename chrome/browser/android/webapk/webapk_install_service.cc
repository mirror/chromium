...
class CacheClearer : public content::BrowsingDataRemover::Observer {
 public:
  ...
  // installing.
  void FreeCacheAndInstall(content::BrowserContext* context,
                           base::OnceCallback<void()> callback) {
    install_callback_ = std::move(callback);
    remover_ = content::BrowserContext::GetBrowsingDataRemover(context);
 ...
 private:
  ...
  base::OnceCallback<void()> install_callback_;
...
void WebApkInstallService::InstallAsync(content::WebContents* web_contents,
  ...
  if (space_status_ == ENOUGH_SPACE_AFTER_FREE_UP_CACHE) {
    ...
    cache_cleaner->FreeCacheAndInstall(
        browser_context_,
        base::BindOnce(&WebApkInstaller::InstallAsyncStatic, browser_context_,
                       shortcut_info, primary_icon, badge_icon,
                       finish_callback));
  } else {
    WebApkInstaller::InstallAsyncStatic(browser_context_, shortcut_info,
                                        primary_icon, badge_icon,
                                        finish_callback);
  }
}
...
bool WebApkInstallService::EnoughSpaceToInstall() {
