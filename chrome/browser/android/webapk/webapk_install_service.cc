...
void WebApkInstallService::InstallAsync(content::WebContents* web_contents,
  ...
  SpaceStatus computed_space_status = space_status_;
  space_status_ = SpaceStatus::UNDETERMINED;

  auto observer = std::make_unique<LifetimeObserver>(web_contents);

  if (computed_space_status == SpaceStatus::NOT_ENOUGH_SPACE) {
    OnFinishedInstall(std::move(observer), shortcut_info, primary_icon,
                      WebApkInstallResult::FAILURE, false, "");
    return;
  }

  // TriggerFreeSpaceCheck() may not have finished yet on a slow device. This
  // means we could end up in a failure situation because there is not enough
  // space but WebApkInstallService::InstallAsync() was called prior to the free
  // space checking completing. That is unlikely and if happens the installation
  // will fail due to not enough space.
  ...
  if (computed_space_status == SpaceStatus::ENOUGH_SPACE_AFTER_FREE_UP_CACHE) {
