...
class WebApkInstallService : public KeyedService {
 ...
 private:
  class Observer : public content::WebContentsObserver {
   public:
    explicit Observer(content::WebContents* web_contents)
        : WebContentsObserver(web_contents) {}
  };

  // Called once the install/update completed or failed.
  void OnFinishedInstall(std::unique_ptr<Observer> web_contents_observer,
                         const ShortcutInfo& shortcut_info,
                         const SkBitmap& primary_icon,
  ...
  // Shows a notification when the install starts or is completed.
  static void ShowInstallNotification(const ShortcutInfo& shortcut_info,
                                      const SkBitmap& primary_icon,
                                      const std::string& webapk_package_name);
