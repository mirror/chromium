...
class WebApkInstallService : public KeyedService {
 public:
  ...
  // Check if there is enough space to install a WebAPK. This should be called
 ...
 private:
  ...
  void OnGotSpaceStatus(SpaceStatus status) { space_status_ = status; }
  ...
  // Indicate if there is enough space to install a WebAPK.
  SpaceStatus space_status_;
