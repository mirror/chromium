    ...
    case State::ALIVE_SERIALIZATION_PENDING: {
      ...
      if (export_target_for_testing_) {
        *export_target_for_testing_ = serialized_passwords;
        ...
        JNIEnv* env = base::android::AttachCurrentThread();
        ScopedJavaLocalRef<jobject> ui_controller =
            weak_java_ui_controller_.get(env);
        if (!ui_controller.is_null()) {
          Java_PasswordUIView_serializedPasswordsAvailable(
              env, ui_controller,
              ConvertUTF8ToJavaString(env, serialized_passwords));
        }
      }
      break;
    }
