...
class TestServiceManagerListener
 ...
 public:
  ...
  void WaitUntilServiceStarted(const std::string& service_name) {
    ...
    DCHECK(service_waited_on_.empty());
    service_waited_on_ = service_name;
 ...
 private:
  ...
  void OnServiceStarted(const service_manager::Identity& identity,
    ...
    auto iter = service_start_counters_.find(identity.name());
    if (iter == service_start_counters_.end()) {
      service_start_counters_[identity.name()] = 1;
    } else {
      iter->second++;
    }

    if (identity.name() != service_waited_on_)
      return;

    service_waited_on_.clear();
...
class SafeJsonParserImplTest : public InProcessBrowserTest {
 protected:
  void SetUpOnMainThread() override {
    ...
    // Register a listener on the ServiceManager.
    connector_ =
        content::ServiceManagerConnection::GetForProcess()->GetConnector();
    ...
    service_manager_->AddListener(std::move(listener_ptr));

    // Bind the JsonParser just so the the data_decoder service is already
    // running.
...
// every time.
IN_PROC_BROWSER_TEST_F(SafeJsonParserImplTest, ParseIsolated) {
  for (int i = 0; i < 5; i++) {
    base::RunLoop run_loop;
    ...
    // starts at 0.
    EXPECT_EQ(2U + i,  // 2 because it was already started and i starts at 0.
              GetServiceStartCount(data_decoder::mojom::kServiceName));
  }
}

// When calling SafeJsonParser::ParseShared() the existing service is reused.
IN_PROC_BROWSER_TEST_F(SafeJsonParserImplTest, ParseShared) {
  for (int i = 0; i < 5; i++) {
    base::RunLoop run_loop;
    ParseCallback parse_callback(run_loop.QuitClosure());
    SafeJsonParser::ParseShared(
        connector(), "[\"awesome\", \"possum\"]",
        base::Bind(&ParseCallback::OnSuccess,
                   base::Unretained(&parse_callback)),
        base::Bind(&ParseCallback::OnError, base::Unretained(&parse_callback)));
    run_loop.Run();
    EXPECT_TRUE(parse_callback.success());
    EXPECT_EQ(1U, GetServiceStartCount(data_decoder::mojom::kServiceName));
  }
}
#endif