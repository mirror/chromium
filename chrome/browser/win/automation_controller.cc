...
// the event handlers. This is safe because only its vtable is accessed, which
// is const.
class RefCountedDelegate : public base::RefCounted<RefCountedDelegate> {
 ...
 private:
  ...
  std::unique_ptr<AutomationController::Delegate> delegate_;
...
class AutomationController::Context {
 public:
  ...
  // invoking OnInitialized(). |task_runner| is the runner
