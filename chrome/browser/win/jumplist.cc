...
// The delay before updating the JumpList for users who haven't used JumpList in
// a session. A delay of 2000 ms is chosen to coalesce more updates when tabs
// are closed rapidly.
constexpr base::TimeDelta kLongDelayForUpdate =
    base::TimeDelta::FromMilliseconds(2000);

// The delay before updating the JumpList for users who haven used JumpList in a
// session. A delay of 500 ms is used to not only make the update happen almost
// intermediately, but also prevent update storms when tabs are closed rapidly
// via Ctrl-W.
...
void JumpList::InitializeTimerForUpdate() {
  ...
  if (timer_.IsRunning()) {
    ...
    timer_.Start(
        FROM_HERE,
        profile_->has_used_jumplist() ? kShortDelayForUpdate
                                      : kLongDelayForUpdate,
