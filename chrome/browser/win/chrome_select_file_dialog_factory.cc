...
chrome::mojom::ChromeWinUtilPtr BindChromeWinUtil() {
  chrome::mojom::ChromeWinUtilPtr chrome_win_util_ptr;
  content::ServiceManagerConnection::GetForProcess()
      ->GetConnector()
      ->BindInterface(chrome::mojom::kChromeWinUtilServiceName,
                      &chrome_win_util_ptr);
  return chrome_win_util_ptr;
}
...
bool ChromeSelectFileDialogFactory::BlockingGetOpenFileName(OPENFILENAME* ofn) {
  ...
  // Sync operation, it's OK for the chrome_win_util_ptr to go out of scope
  // right after the call.
  chrome::mojom::ChromeWinUtilPtr chrome_win_util_ptr = BindChromeWinUtil();
  chrome_win_util_ptr->CallGetOpenFileName(
...
bool ChromeSelectFileDialogFactory::BlockingGetSaveFileName(OPENFILENAME* ofn) {
  ...
  // right after the call.
  chrome::mojom::ChromeWinUtilPtr chrome_win_util_ptr = BindChromeWinUtil();
  chrome_win_util_ptr->CallGetSaveFileName(
      base::win::HandleToUint32(ofn->hwndOwner),
      static_cast<uint32_t>(ofn->Flags & ~OFN_ENABLEHOOK),
