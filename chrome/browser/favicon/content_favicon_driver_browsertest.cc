...
IN_PROC_BROWSER_TEST_F(ContentFaviconDriverTest, TrackerFaviconBlocked) {
  ...
  const GURL favicon_page = ssl_server.GetURL(
      "example.test", "/favicon/page_with_favicon_by_url.html");
  GURL::Replacements add_favicon;
  std::string query_string("url=" + favicon_url.spec());
  add_favicon.SetQueryStr(query_string);

  // Delete any existing mappings for this URL to force a re-download.
  favicon_service()->DeleteFaviconMappings(base::flat_set<GURL>({favicon_page}),
                                           favicon_base::IconType::kFavicon);
  ...
  ui_test_utils::NavigateToURL(browser(),
                               favicon_page.ReplaceComponents(add_favicon));
...
IN_PROC_BROWSER_TEST_F(ContentFaviconDriverTest, TrackerFaviconAllowed) {
  net::EmbeddedTestServer ssl_server(net::EmbeddedTestServer::TYPE_HTTPS);
  ssl_server.AddDefaultHandlers(base::FilePath(kDocRoot));
  ASSERT_TRUE(ssl_server.Start());
