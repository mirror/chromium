...
constexpr char kTestJson[] = "{\"tag\":\"hello\",\"text\":\"bonjour\"}";
...
class SafeXmlParserTest : public InProcessBrowserTest {
 ...
 protected:
  ...
  void TestParse(const std::string& xml,
...
IN_PROC_BROWSER_TEST_F(SafeXmlParserTest, Isolation) {
  for (int i = 0; i < 5; i++)
    TestParse(kTestXml, kTestJson);
  EXPECT_EQ(5U, GetServiceStartCount(data_decoder::mojom::kServiceName));
}

// Tests that using a process ID allows service reuse..
IN_PROC_BROWSER_TEST_F(SafeXmlParserTest, IsolationWithGroups) {
  constexpr char kGroupId1[] = "group1";
  constexpr char kGroupId2[] = "group2";
  for (int i = 0; i < 5; i++) {
    TestParse(kTestXml, kTestJson, kGroupId1);
    TestParse(kTestXml, kTestJson, kGroupId2);
  }
