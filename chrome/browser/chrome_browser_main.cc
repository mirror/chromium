  ...
  void AdvanceToNextState() {
    ...
    switch (state_) {
      ...
      case State::kWaitingForMainToStart:
        ...
        DCHECK_EQ(base::ThreadTaskRunnerHandle::Get()->RunsTasksInCurrentSequence(), deferred_runner_->RunsTasksInCurrentSequence());
...
void ChromeBrowserMainParts::PreEarlyInitialization() {
  ...
  if (!parsed_command_line().HasSwitch("old")) {
    local_state2_ = std::make_unique<local_state_loader::LocalState>();
    local_state_load_result_ =
        local_state_loader::LoadLocalState(local_state2_.get());
...
int ChromeBrowserMainParts::PreCreateThreadsImpl() {
  ...
  field_trial_synchronizer_ =
      std::move(local_state2_->field_trial_synchronizer);
