...
void ChromeBrowserMainParts::PostThreadCreate(
  ...
  if (browser_thread_id == BrowserThread::IO) {
    BrowserThread::PostTask(
        BrowserThread::IO, FROM_HERE,
        base::BindOnce(&ChromeBrowserMainParts::StartProfilingIOThread,
                       base::Unretained(this)));
  }
}

void ChromeBrowserMainParts::EndProfilingIOThread() {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  io_thread_sampling_profiler.Get().Get().reset();
}
...
void ChromeBrowserMainParts::PreThreadDestroy(
  ...
  if (browser_thread_id == BrowserThread::IO) {
    BrowserThread::PostTask(
        BrowserThread::IO, FROM_HERE,
        base::BindOnce(&ChromeBrowserMainParts::EndProfilingIOThread,
                       base::Unretained(this)));
  }
}
