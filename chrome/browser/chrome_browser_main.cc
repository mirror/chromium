...
class ChromeBrowserMainParts::InitialTaskRunner : public base::SingleThreadTaskRunner {
 public:
  InitialTaskRunner() {
    // scoped_refptr<base::SingleThreadTaskRunner> task_runner(this);
    thread_task_runner_handle_ =
        std::make_unique<base::ThreadTaskRunnerHandle>(this);
    deferred_runner_ = new base::DeferredSequencedTaskRunner(nullptr);
  }
  ...
  bool PostDelayedTask(const base::Location& from_here,
    ...
    switch (state_) {
      case State::kInstalled:
      case State::kWaitingForMainToStart:
        return deferred_runner_->PostDelayedTask(from_here, std::move(task), delay);
        break;

      case State::kMessageLoopRunning:
        return main_thread_task_runner_->PostDelayedTask(from_here,
                                                                    std::move(task), delay);
    }
    return false;
  }
  ...
  bool PostNonNestableDelayedTask(const base::Location& from_here,
    ...
    switch (state_) {
      case State::kInstalled:
      case State::kWaitingForMainToStart:
        return deferred_runner_->PostNonNestableDelayedTask(from_here, std::move(task), delay);
        break;

      case State::kMessageLoopRunning:
        return main_thread_task_runner_->PostNonNestableDelayedTask(from_here,
                                                          std::move(task), delay);
    }
    return false;
  }
...
void ChromeBrowserMainParts::PreEarlyInitialization() {
  ...
  // NEW
  if (!parsed_command_line().HasSwitch("old")) {
    initial_task_runner_ = new InitialTaskRunner;

    local_state2_ =
      std::make_unique<local_state_loader::LocalState>();
  local_state_load_result_ = local_state_loader::LoadLocalState(
      local_state2_.get());
  // XXX needs to return error code.
  }
}
...
int ChromeBrowserMainParts::PreCreateThreadsImpl() {
  ...
  if (local_state_load_result_ != content::RESULT_CODE_NORMAL_EXIT)
    return local_state_load_result_;

  field_trial_synchronizer_ = std::move(local_state2_->field_trial_synchronizer);

  field_trial_list_ = std::move(local_state2_->field_trial_list);

  browser_field_trials_ = std::move(local_state2_->browser_field_trials);

  local_state_ = local_state2_->local_state.get();
#if !defined(OS_ANDROID)
  master_prefs_ = std::move(local_state2_->master_prefs);
#endif
  {
    TRACE_EVENT0("startup",
      "ChromeBrowserMainParts::PreCreateThreadsImpl:InitBrowswerProcessImpl");
    browser_process_ =
        std::make_unique<BrowserProcessImpl>(
            std::move(local_state2_->metrics_services_manager),
            std::move(local_state2_->browser_policy_connector),
            std::move(local_state2_->local_state),
            local_state2_->local_state_task_runner,
            std::move(local_state2_->pref_service_factory));
    local_state2_.reset();
  }
  ...
  if (local_state_->GetBoolean(prefs::kSitePerProcess) &&
  ...
  if (local_state_->HasPrefPath(prefs::kIsolateOrigins)) {
    command_line->AppendSwitchASCII(
        switches::kIsolateOrigins,
        local_state_->GetString(prefs::kIsolateOrigins));
  }
