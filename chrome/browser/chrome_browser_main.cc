...
// Initializes the shared instance of ResourceBundle and returns the locale. A
// return value of false indicates failure.
bool InitResourceBundleAndDetermineLocale(
    const content::MainFunctionParams& params,
    std::string* locale) {
#if defined(OS_MACOSX)
  // Tests always get en-US.
  *locale = (params.ui_task != nullptr) ? "en-US" : std::string();
...
//
// . initial state (kInstalled): tasks are added to |deferred_runner_|. In this
//   state this is installed as the active ThreadTaskRunnerHandle.
// . kWaitingForMainToStart: this is no longer the active
// ThreadTaskRunnerHandle,
//   tasks are still deferred though.
...
ChromeBrowserMainParts::ChromeBrowserMainParts(
  ...
  initial_task_runner_ = new DeferringTaskRunner;
#endif
}
...
int ChromeBrowserMainParts::LoadLocalState(
  ...
  std::string locale;
  if (!InitResourceBundleAndDetermineLocale(parameters(), &locale))
    return chrome::RESULT_CODE_MISSING_RESOURCE_BUNDLE_DATA;
