    ...
    : parameters_(parameters),
      ...
      ui_thread_sampling_profiler_(
          ...
          metrics::CallStackProfileMetricsProvider::
              GetProfilerCallbackForBrowserProcessStartup(BrowserThread::UI)),
      profile_(NULL),
  ...
  if (config->IsProfilerEnabledForCurrentProcess()) {
    io_thread_sampling_profiler_ =
        std::make_unique<base::StackSamplingProfiler>(
            thread_id, config->GetSamplingParamsForCurrentProcess(),
            metrics::CallStackProfileMetricsProvider::
                GetProfilerCallbackForBrowserProcessStartup(browser_thread_id));
