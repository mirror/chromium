...
NOINLINE void* MallocWithUniqueFrame1(size_t size) {
	return malloc(size);
}

NOINLINE void* MallocWithUniqueFrame2(size_t size) {
	return malloc(size);
}
...
void ProfilingTestDriver::MakeTestAllocations() {
  ...
  for (int i = 0; i < kMallocAllocCount; ++i) {
    leaks_.push_back(MallocWithUniqueFrame1(kMallocAllocSize));
  }
  ...
  for (int i = 0; i < kVariadicAllocCount; ++i) {
    leaks_.push_back(MallocWithUniqueFrame2(i + 8000));  // Variadic allocation.
...
bool ProfilingTestDriver::ValidateBrowserAllocations(base::Value* dump_json) {
  ...
  bool result = ValidateDump(heaps_v2, kMallocAllocSize * kMallocAllocCount,
