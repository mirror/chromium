...
ProfilingTestDriver::ProfilingTestDriver() : wait_for_ui_thread_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED) {
...
bool ProfilingTestDriver::RunTest(const Options& options) {
  ...
  running_on_ui_thread_ = content::BrowserThread::CurrentlyOn(content::BrowserThread::UI);
  ...
  if (running_on_ui_thread_) {
    ...
    content::BrowserThread::PostTask(content::BrowserThread::UI, FROM_HERE, base::Bind(&ProfilingTestDriver::RunInitializationOnUIThreadAndSignal, base::Unretained(this)));
    wait_for_ui_thread_.Wait();
    if (!initialization_success_)
      return false;
    content::BrowserThread::PostTask(content::BrowserThread::UI, FROM_HERE, base::Bind(&ProfilingTestDriver::CollectResults, base::Unretained(this), false));
    wait_for_ui_thread_.Wait();
  }
...
bool ProfilingTestDriver::RunInitializationOnUIThread(){
  ...
  LOG(ERROR) << "RunInitializationOnUIThread: " << base::CommandLine::ForCurrentProcess()->GetCommandLineString();
...
void ProfilingTestDriver::CollectResults(bool synchronous) {
  ...
  if (synchronous) {
    ...
    finish_tracing_closure = base::Bind(&base::WaitableEvent::Signal, base::Unretained(&wait_for_ui_thread_));
  }
...
bool ProfilingTestDriver::ValidateBrowserAllocations(base::Value* dump_json) {
  ...
  if (options_.mode != ProfilingProcessHost::Mode::kAll &&
    ...
    result = NumProcessesWithName(dump_json, "Browser", nullptr) == 1;
    if (!result) {
