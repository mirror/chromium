...
bool ProfilingTestDriver::ValidateBrowserAllocations(base::Value* dump_json) {
  ...
  if (should_validate_dumps) {
    result = ValidateDump(heaps_v2, kMallocAllocSize * kMallocAllocCount,
                          ...
                          options_.include_thread_names ? kThreadName : "");
    if (!result) {
    ...
    result =
        ValidateDump(heaps_v2, total_variadic_allocations_, kVariadicAllocCount,
                     "malloc", nullptr, HasPseudoFrames() ? kVariadicEvent : "",
                     options_.include_thread_names ? kThreadName : "");
    if (!result) {
