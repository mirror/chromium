...
const uint64_t kNullParent = std::numeric_limits<uint64_t>::max();
struct Node {
  uint64_t name_id;
...
bool ParseNodes(base::Value* heaps_v2, NodeMap* output) {
  ...
  for (base::Value& node_value : nodes->GetList()) {
    base::Value* id = node_value.FindKey("id");
    base::Value* name_sid = node_value.FindKey("name_sid");
    base::Value* parent = node_value.FindKey("parent");
    if (!id || !name_sid) {
  ...
  for (base::Value& string_value : strings->GetList()) {
    base::Value* id = string_value.FindKey("id");
    base::Value* string = string_value.FindKey("string");
    if (!id || !string) {
