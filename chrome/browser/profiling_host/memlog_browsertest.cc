 ...
 protected:
  ...
  void SetUp() override {
    ...
    if (GetParam() == switches::kMemlogModeRendererSampling) {
      ProfilingProcessHost::SetRendererSamplingAlwaysProfileForTest();
    }
  }
  ...
  void ValidateRendererAllocations(base::Value* dump_json) {
    ...
    // RendererSampling guarantees only 1 rendere is ever sampled at a time.
    if (GetParam() == switches::kMemlogModeRendererSampling) {
      EXPECT_EQ(1, NumProcessesWithName(dump_json, "Renderer"));
    } else {
      EXPECT_GT(0, NumProcessesWithName(dump_json, "Renderer"));
    }
  }
