...
std::unique_ptr<base::Value> ReadDumpFile(const base::FilePath& path) {
  ...
  if (dump_string.size() < kInitialBufferSize / 2) {
    LOG(ERROR)
        << "Something changed and initial buffer size is wastefully large now!";
    return nullptr;
  }
...
IN_PROC_BROWSER_TEST_P(MemlogBrowserTest, EndToEnd) {
  ...
  constexpr int kBrowserAllocCount = 500;
  constexpr int kBrowserAllocFlushFillSize = 107;
  constexpr int kBrowserAllocFlushFillCount = 50000;
...

  {
    ...
    pph->RequestProcessDump(
        base::Process::Current().Pid(), browser_dumpfile_path,
...

  {
    ...
    // Navigte somewhere to force some allocations in the renderer.
    ...
    if (GetParam() == switches::kMemlogModeAll) {
      ASSERT_TRUE(dump_json);
      ValidateDump(dump_json.get(), 0, 0, 0);
    } else {
