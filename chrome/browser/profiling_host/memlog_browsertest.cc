...
class MemlogBrowserTest : public InProcessBrowserTest,
  ...
  void SetUpDefaultCommandLine(base::CommandLine* command_line) override {
    ...
    if (GetParam().start_profiling_with_command_line_flag) {
      LOG(ERROR) << "GetParam().start_profiling_with_command_line_flag";
...
std::vector<TestParam> GetParams() {
  ...
  for (auto& mode : dynamic_start_modes) {
    for (auto& stack_mode : stack_modes) {
  ...
  for (auto& mode : command_line_start_modes) {
    for (auto& stack_mode : stack_modes) {
