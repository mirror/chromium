...
IN_PROC_BROWSER_TEST_P(MemlogBrowserTest, EndToEnd) {
...

  {
    ...
    base::Value* heaps_v2 =
        FindHeapsV2(base::Process::Current().Pid(), dump_json.get());
...
IN_PROC_BROWSER_TEST_P(MemlogBrowserTest, EndToEndTracing) {
  ...
  // dump the results into |result|, then quit the nested run loop.
  // scoped_refptr<StringTraceSink> sink(new StringTraceSink(&result,
  ...
  pph->SetDumpProcessForTracingCallback(
      std::move(stop_tracing_ui_thread_closure));

  // Spin an nested RunLoop until the heap dump has been added to the trace.
...

  {
    ...
    base::ProcessId renderer_pid = base::GetProcId(browser()
                                                       ...
                                                       ->GetProcess()
                                                       ->GetHandle());
    base::Value* heaps_v2 = FindHeapsV2(renderer_pid, dump_json.get());
    if (GetParam() == switches::kMemlogModeAll) {
