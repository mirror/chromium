...
TEST(BackgroungProfilingTriggersTest, OnReceivedMemoryDump) {
  ...
  ProfilingProcessHost::GetInstance()->SetMode(
      ProfilingProcessHost::Mode::kBrowser);
  ...
  // A larger browser process must trigger a report. The renderer is not
  // reported.
  GlobalMemoryDumpPtr dump_large_browser(
      memory_instrumentation::mojom::GlobalMemoryDump::New());
  PopulateMetrics(dump_large_browser, 2, ProcessType::BROWSER,
                  kProcessMallocTriggerKb, kProcessMallocTriggerKb);
  PopulateMetrics(dump_large_browser, 3, ProcessType::RENDERER,
                  kProcessMallocTriggerKb, kProcessMallocTriggerKb);
  triggers.OnReceivedMemoryDump(true, std::move(dump_large_browser));
  EXPECT_TRUE(triggers.pids_.find(2) != triggers.pids_.end());
  EXPECT_TRUE(triggers.pids_.find(3) == triggers.pids_.end());
  triggers.Reset();

  // Validate triggers for all processes.
  ProfilingProcessHost::GetInstance()->SetMode(
      ProfilingProcessHost::Mode::kAll);

  // Both browser and renderer must be reported.
  GlobalMemoryDumpPtr dump_large_processes(
      memory_instrumentation::mojom::GlobalMemoryDump::New());
  PopulateMetrics(dump_large_processes, 2, ProcessType::BROWSER,
                  kProcessMallocTriggerKb, kProcessMallocTriggerKb);
  PopulateMetrics(dump_large_processes, 3, ProcessType::RENDERER,
                  kProcessMallocTriggerKb, kProcessMallocTriggerKb);
  triggers.OnReceivedMemoryDump(true, std::move(dump_large_processes));
  EXPECT_TRUE(triggers.pids_.find(2) != triggers.pids_.end());
  EXPECT_TRUE(triggers.pids_.find(3) != triggers.pids_.end());
  triggers.Reset();
