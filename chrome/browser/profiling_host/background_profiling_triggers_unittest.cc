...
class TestBackgroundProfilingTriggers : public BackgroundProfilingTriggers {
 public:
  explicit TestBackgroundProfilingTriggers(ProfilingProcessHost* host)
      : BackgroundProfilingTriggers(host) {}

  using BackgroundProfilingTriggers::OnReceivedMemoryDump;

  void Reset() { pids_.clear(); }
  const std::set<base::ProcessId>& pids() { return pids_; }
...
class BackgroundProfilingTriggersTest : public testing::Test {
 ...
 protected:
  ...
  TestBackgroundProfilingTriggers triggers_;
};
