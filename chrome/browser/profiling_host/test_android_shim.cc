...
static jlong Init(JNIEnv* env, const JavaParamRef<jobject>& obj) {
  LOG(ERROR) << "TestAndroidShim::Init1 " << getpid();
...
void TestAndroidShim::TakeTrace(
  ...
  base::SequencedTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE,
      base::Bind(&TestAndroidShim::OnTraceFinished, weak_factory_.GetWeakPtr()),
...
void TestAndroidShim::OnTraceFinished() {
  ...
  if (obj.obj())
    Java_TestAndroidShim_onTraceFinished(env, obj,
                                         base::android::ConvertUTF8ToJavaString(
                                             env, std::string("placeholder")));
}
