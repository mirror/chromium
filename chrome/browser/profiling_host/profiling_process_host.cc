...
void ProfilingProcessHost::AddSwitchesToChildCmdLine(
  ...
  // TOOD(ajwong): Change this to just reuse the --memlog flag. There is
  // no need for a separate pipe flag.
  child_cmd_line->AppendSwitchASCII(switches::kMemlogPipe, "0");
  return;
}
...
void ProfilingProcessHost::LaunchAsService() {
  ...
  //
  // TODO(ajwong): TODO(ajwong): Dedupe with InitMemlogSenderIfNecessary().
  ...
  memlog->GetMemlogPipe(pid, base::Bind(&StartMemlogSender));
}
