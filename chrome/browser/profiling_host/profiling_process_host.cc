...
void ProfilingProcessHost::Launch() {
  ...
  printf("========== Made a PCP server = %d, client = %d\n",
      control_channel.server_handle_.get().handle,
      control_channel.client_handle_.get().handle);
  ...
  for (const auto& pair : handle_passing_info) {
    printf("========== remapping %d:%d\n", pair.first, pair.second);
  }
  printf("========== cmdline=%s\n", profiling_cmd.GetCommandLineString().c_str());
#else
...
void ProfilingProcessHost::ConnectControlChannel() {
  printf("========== Attaching invitation to message pipe. %d\n",
      pending_control_connection_.get().handle);
  char to_send[4] = { 101, 102, 103, 104 };
  write(pending_control_connection_.get().handle, to_send, 4);
