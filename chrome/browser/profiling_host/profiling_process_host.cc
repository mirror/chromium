...
void ProfilingProcessHost::RequestTraceWithHeapDump(
  ...
  auto finished_dump_callback = base::BindOnce(
      [](base::OnceClosure stop_tracing_closure, bool success,
         uint64_t dump_guid) { std::move(stop_tracing_closure).Run(); },
      std::move(stop_tracing_closure));
  memory_instrumentation::MemoryInstrumentation::GetInstance()->RequestGlobalDumpAndAppendToTrace(
      base::trace_event::MemoryDumpType::EXPLICITLY_TRIGGERED, base::trace_event::MemoryDumpLevelOfDetail::BACKGROUND,
...
void ProfilingProcessHost::SetKeepSmallAllocations(bool keep_small_allocations) {
...
void ProfilingProcessHost::LaunchAsService() {
  ...
  // already.
  connector_->BindInterface(profiling::mojom::kServiceName, &profiling_service_);

  // Set some state for heap dumps.
  bool keep_small_allocations =
      base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kMemlogKeepSmallAllocations);
  ...
  connector_->BindInterface(resource_coordinator::mojom::kServiceName, &coordinator);
