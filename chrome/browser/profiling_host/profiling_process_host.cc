...
void UploadTraceToCrashServer(std::string file_contents,
  ...
  LOG(ERROR) << "UploadTraceToCrashServer";
...
bool ProfilingProcessHost::OnMemoryDump(
  ...
  LOG(ERROR) << "ProfilingProcessHost::OnMemoryDump";
...
void ProfilingProcessHost::AddClientToProfilingService(
  ...
  MemlogSenderPipe::PipePair pipes(MemlogSenderPipe::CreatePipes());
  ...
  profiling_service_->AddProfilingClient(
      pid, std::move(client),
      mojo::WrapPlatformFile(pipes.sender.release().handle),
      mojo::WrapPlatformFile(pipes.receiver.release().handle), process_type);
}
