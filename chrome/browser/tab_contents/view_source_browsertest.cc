...
IN_PROC_BROWSER_TEST_F(ViewSourceTest, HttpPostInMainframe) {
  ...
  std::string script = R"( domAutomationController.send(
      document.getElementById('response-nonce').innerText); )";
  EXPECT_TRUE(ExecuteScriptAndExtractString(original_main_frame, script,
                                            &response_nonce));
...
// Tests that "View Source" works fine for frames shown via HTTP POST.
// This is a regression test for https://crbug.com/523.
IN_PROC_BROWSER_TEST_F(ViewSourceTest, HttpPostInSubframe) {
  ...
  std::string script = R"( domAutomationController.send(
      document.getElementById('response-nonce').innerText); )";
  EXPECT_TRUE(ExecuteScriptAndExtractString(original_child_frame, script,
                                            &response_nonce));
  ...
  EXPECT_THAT(
      ...
      ContainsRegex(
          "Request Headers:.*Content-Type: application/x-www-form-urlencoded"));
  EXPECT_THAT(source_text, HasSubstr("<h1>Response nonce:</h1><pre>" +
                                     response_nonce + "</pre>"));
}
