...
TEST_F(ChromePluginServiceFilterTest,
       PreferHtmlOverPluginsIncogintoInheritsOriginalContentSetting) {
  ...
  HostContentSettingsMap* original_map =
      HostContentSettingsMapFactory::GetForProfile(profile());
  original_map->SetDefaultContentSetting(CONTENT_SETTINGS_TYPE_PLUGINS,
  ...
  // Change to Allow in the original profile.
  original_map->SetDefaultContentSetting(CONTENT_SETTINGS_TYPE_PLUGINS,
                                         CONTENT_SETTING_ALLOW);

  // Ensure we pass the availablilty check in the incognito profile.
  EXPECT_TRUE(IsPluginAvailable(url, main_frame_origin,
