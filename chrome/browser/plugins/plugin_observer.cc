...
bool PluginObserver::OnMessageReceived(const IPC::Message& message,
  ...
  IPC_BEGIN_MESSAGE_MAP_WITH_PARAM(PluginObserver, message, sender)
...
void PluginObserver::OnBlockedOutdatedPlugin(content::RenderFrameHost* sender,
                                             int placeholder_routing_id,
                                             const std::string& identifier) {
  content::GlobalRoutingID placeholder_id(sender->GetProcess()->GetID(),
...
void PluginObserver::OnRemovePluginPlaceholderHost(
    content::RenderFrameHost* sender,
    int placeholder_routing_id) {
  content::GlobalRoutingID placeholder_id(sender->GetProcess()->GetID(),
                                          placeholder_routing_id);
...
void PluginObserver::OnCouldNotLoadPlugin(content::RenderFrameHost* sender,
