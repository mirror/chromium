...
void MockBrowsingDataSharedWorkerHelper::StartFetching(
...
void MockBrowsingDataSharedWorkerHelper::DeleteSharedWorkers(
    const GURL& worker,
    const std::string& name) {
  ASSERT_FALSE(callback_.is_null());
  std::pair<GURL, std::string> key({worker, name});
