...
void BrowsingDataSharedWorkerHelper::StartFetching(
  ...
  // workers.
  std::list<std::pair<GURL, std::string>> result;
  content::BrowserThread::PostTask(content::BrowserThread::UI, FROM_HERE,
                                   base::BindOnce(callback, result));
}
void BrowsingDataSharedWorkerHelper::DeleteSharedWorkers(
    const GURL& worker,
...
CannedBrowsingDataSharedWorkerHelper::PendingSharedWorkerUsageInfo::
    ~PendingSharedWorkerUsageInfo() = default;

bool CannedBrowsingDataSharedWorkerHelper::PendingSharedWorkerUsageInfo::
operator<(const PendingSharedWorkerUsageInfo& other) const {
...
void CannedBrowsingDataSharedWorkerHelper::AddSharedWorker(
  ...
  pending_shared_worker_info_.insert(
      PendingSharedWorkerUsageInfo(worker, name));
}
...
const std::set<
    CannedBrowsingDataSharedWorkerHelper::PendingSharedWorkerUsageInfo>&
CannedBrowsingDataSharedWorkerHelper::GetSharedWorkerUsageInfo() const {
...
void CannedBrowsingDataSharedWorkerHelper::StartFetching(
  ...
  for (auto& it : pending_shared_worker_info_)
    result.push_back({it.worker, it.name});
  content::BrowserThread::PostTask(content::BrowserThread::UI, FROM_HERE,
                                   base::BindOnce(callback, result));
}

void CannedBrowsingDataSharedWorkerHelper::DeleteSharedWorkers(
    const GURL& worker,
    const std::string& name) {
  for (auto it = pending_shared_worker_info_.begin();
       it != pending_shared_worker_info_.end();) {
    if (it->worker == worker && it->name == name) {
      BrowsingDataSharedWorkerHelper::DeleteSharedWorkers(it->worker, it->name);
