...
class CookieTreeNode : public ui::TreeNode<CookieTreeNode> {
 public:
  ...
  struct DetailedInfo {
    ...
    const std::pair<GURL, std::string>* shared_worker_info = nullptr;
...
class CookieTreeSharedWorkerNode : public CookieTreeNode {
 ...
 private:
  ...
  // CookieTreeSharedWorkerNode is valid.
  std::list<std::pair<GURL, std::string>>::iterator shared_worker_info_;
...
class CookiesTreeModel : public ui::TreeNodeModel<CookieTreeNode> {
 public:
  ...
  void PopulateSharedWorkerUsageInfo(LocalDataContainer* container);
 ...
 private:
  ...
  void PopulateServiceWorkerUsageInfoWithFilter(
      ...
      ScopedBatchUpdateNotifier* notifier,
      const base::string16& filter);
  void PopulateSharedWorkerUsageInfoWithFilter(
      LocalDataContainer* container,
      ScopedBatchUpdateNotifier* notifier,
      const base::string16& filter);
