...
IN_PROC_BROWSER_TEST_F(BrowsingDataRemoverBrowserTest, VideoDecodePerfHistory) {
...

  {
    ...
    video_decode_perf_history->SavePerfRecord(
        ...
        kFramesPowerEfficient, base::BindOnce(run_loop.QuitWhenIdleClosure()));
    run_loop.Run();
  }

  // Verify history exists.
  // Expect |is_smooth| = false and |is_power_effiect| = false given that 90% of
  // recorded frames were dropped and 0 were power efficient.
  bool is_smooth = true;
  bool is_power_effiect = true;
  {
    base::RunLoop run_loop;
