...
class Waiter {
 public:
  ...
  void Wait(ConditionValidator validator) {
    ...
    LOG(ERROR) << "@@@ " << __func__ << " before run loop.";
    run_loop_->Run();
  }

  // Unblock if condition met.
  void DoneIfNeeded(Args... args) {
    LOG(ERROR) << "@@@ DoneIfNeeded" << __func__;
    ...
    if (validator_.Run(args...)) {
      LOG(ERROR) << "@@@ run validator succeed!";

      waiting_ = false;
      LOG(ERROR) << "@@@ before quit runloop!!";
      run_loop_->Quit();
      LOG(ERROR) << "@@@ after quit runloop!!";
      validator_ = ConditionValidator();
    }
    LOG(ERROR) << "@@@ run validator failed, nothing happens!";
  }
...
class TestClient : public download::test::EmptyClient {
 public:
  ...
  void Wait(DownloadCondition condition, size_t wait_count) {
    LOG(ERROR) << "@@@ " << __func__ << " , condition = " << (int)condition
...
struct CreationData : public base::SupportsUserData::Data {
  ...
  download::test::DownloadServiceTestHarness* test_harness;
...
class DownloadServiceTest : public InProcessBrowserTest {
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    creation_data->test_harness = &test_harness_;
    ...
    test_harness_.SetDeviceStatus(true, /* Charging. */
                                  1 /* WIFI network. */);
  }
 ...
 private:
  ...
  download::test::DownloadServiceTestHarness test_harness_;
