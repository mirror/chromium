...
class DownloadDangerPromptBrowserTest : public DialogBrowserTest {
 protected:
  ...
  void RunTest(content::DownloadDangerType danger_type,
    ...
    from_download_api_ = from_download_api;
 ...
 private:
  void ShowDialog(const std::string& name) override {
    ...
    DownloadDangerPrompt::Create(
        ...
        from_download_api_,
        base::Bind(&DownloadDangerPromptBrowserTest::OnDone,
                   base::Unretained(this)));
  }

  void OnDone(DownloadDangerPrompt::Action action) {}

  const GURL download_url_;
  const base::FilePath empty_file_path_;

  content::DownloadDangerType danger_type_;
  bool from_download_api_;
  content::MockDownloadItem download_;
...
IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
  ...
  RunTest(content::DOWNLOAD_DANGER_TYPE_DANGEROUS_FILE, false);
}
IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_DangerousFile_FromDownloadApi) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_DANGEROUS_FILE, true);
}

IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_DangerousUrl) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_DANGEROUS_URL, false);
}
IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_DangerousUrl_FromDownloadApi) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_DANGEROUS_URL, true);
}

IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_UncommonContent) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_UNCOMMON_CONTENT, false);
}
IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_UncommonContent_FromDownloadApi) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_UNCOMMON_CONTENT, true);
}

IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_PotentiallyUnwanted) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_POTENTIALLY_UNWANTED, false);
}
IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_PotentiallyUnwanted_FromDownloadApi) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_POTENTIALLY_UNWANTED, true);
}
