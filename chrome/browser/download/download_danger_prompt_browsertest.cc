...
class DownloadDangerPromptBrowserTest : public DialogBrowserTest {
 protected:
  enum InvocationType { kUserInitiated, kFromDownloadApi };
 ...
 private:
  void ShowDialog(const std::string& name) override {
    ...
    DownloadDangerPrompt::Create(
        ...
        invocation_type_ == kFromDownloadApi, DownloadDangerPrompt::OnDone());
  }
...
IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
  ...
  RunTest(content::DOWNLOAD_DANGER_TYPE_DANGEROUS_FILE, kUserInitiated);
}
IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_DangerousFileFromApi) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_DANGEROUS_FILE, kFromDownloadApi);
}

IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_DangerousUrl) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_DANGEROUS_URL, kUserInitiated);
}
IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_DangerousUrlFromApi) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_DANGEROUS_URL, kFromDownloadApi);
}

IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_UncommonContent) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_UNCOMMON_CONTENT, kUserInitiated);
}
IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_UncommonContentFromApi) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_UNCOMMON_CONTENT, kFromDownloadApi);
}

IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_PotentiallyUnwanted) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_POTENTIALLY_UNWANTED, kUserInitiated);
}
IN_PROC_BROWSER_TEST_F(DownloadDangerPromptBrowserTest,
                       InvokeDialog_PotentiallyUnwantedFromApi) {
  RunTest(content::DOWNLOAD_DANGER_TYPE_POTENTIALLY_UNWANTED, kFromDownloadApi);
}
