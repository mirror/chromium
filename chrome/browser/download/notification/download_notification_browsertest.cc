...
IN_PROC_BROWSER_TEST_F(DownloadNotificationTest, IncognitoDownloadFile) {
  ...
  EXPECT_EQ(message_center::NOTIFICATION_TYPE_PROGRESS, notification()->type());
  EXPECT_TRUE(
      content::ContentDownloadItemDelegate::GetBrowserContext(download_item())
          ->IsOffTheRecord());
...
IN_PROC_BROWSER_TEST_F(DownloadNotificationTest,
  ...
  EXPECT_TRUE(content::ContentDownloadItemDelegate::GetBrowserContext(
                  download_incognito)
                  ->IsOffTheRecord());
  EXPECT_FALSE(
      content::ContentDownloadItemDelegate::GetBrowserContext(download_normal)
          ->IsOffTheRecord());
