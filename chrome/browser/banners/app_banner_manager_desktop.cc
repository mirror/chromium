...
void AppBannerManagerDesktop::ShowBannerUi(WebAppInstallSource install_source) {
  ...
  WebAppInstallSource source = IsExperimentalAppBannersEnabled()
                                   ? WebAppInstallSource::API
                                   : WebAppInstallSource::AUTOMATIC_PROMPT;

  bookmark_app_helper_.reset(new extensions::BookmarkAppHelper(
      profile, web_app_info, contents, &source));
