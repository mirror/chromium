...
#if defined(OS_ANDROID)
  app_banner_manager = AppBannerManagerAndroid::FromWebContents(web_contents);
#else
  app_banner_manager = AppBannerManagerDesktop::FromWebContents(web_contents);
#endif

  return app_banner_manager ? app_banner_manager->installable()
                            : Installable::UNKNOWN;
}
