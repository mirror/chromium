...
class TrackingStatusReporter
 ...
 public:
  ...
  WebappInstallSource GetInstallSource(content::WebContents* web_contents,
                                       bool is_api) override {
    return InstallableMetrics::GetPromptInstallSource(web_contents, is_api);
  }
...
void AppBannerManager::ShowBanner() {
  ...
  if (state_ == State::SENDING_EVENT) {
    ...
    install_source =
        status_reporter_->GetInstallSource(contents, /*is_api=*/false);

  } else {
    TrackBeforeInstallEvent(
        BEFORE_INSTALL_EVENT_PROMPT_CALLED_AFTER_PREVENT_DEFAULT);
    install_source =
        status_reporter_->GetInstallSource(contents, /*is_api=*/true);
  }
