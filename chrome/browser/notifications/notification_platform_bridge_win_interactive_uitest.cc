...
class NotificationPlatformBridgeWinBrowserTest : public InProcessBrowserTest {
 public:
  NotificationPlatformBridgeWinBrowserTest() = default;
  ~NotificationPlatformBridgeWinBrowserTest() override = default;
 ...
 protected:
  bool ValidateNotificationValues(NotificationCommon::Operation operation,
    ...
    return display_service_tester_->ValidateNotificationValues(
...
class MockIInspectable : public IInspectable {
 public:
  MockIInspectable() = default;
  virtual ~MockIInspectable() = default;
  ...
  HRESULT
  STDMETHODCALLTYPE GetIids(ULONG* iidCount, IID** iids) override {
    return E_NOTIMPL;
  }
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(MockIInspectable);
};

class MockIToastNotification
    :  // public MockIInspectable, // Hmm, doesn't work, see below.
       public winui::Notifications::IToastNotification {
 public:
  explicit MockIToastNotification(const base::string16& xml) : xml_(xml) {}
  ~MockIToastNotification() = default;

  /*
    // TODO(finnur): Why must I copy the below definitions here? Why doesn't
    // inheriting publicly from MockIInspectable work?
    // error: variable type 'MockIToastNotification' is an abstract class
    // note: unimplemented pure virtual method 'QueryInterface' in
    'MockIToastNotification'
    // note: unimplemented pure virtual method 'AddRef' in
    'MockIToastNotification'
    // note: unimplemented pure virtual method 'Release' in
    'MockIToastNotification'
    // note: unimplemented pure virtual method 'GetIids' in
    'MockIToastNotification'
    // note: unimplemented pure virtual method 'GetRuntimeClassName' in
    'MockIToastNotification'
    // note: unimplemented pure virtual method 'GetTrustLevel' in
    'MockIToastNotification'
  */
  HRESULT STDMETHODCALLTYPE QueryInterface(REFIID riid,
                                           void** ppvObject) override {
...
IN_PROC_BROWSER_TEST_F(NotificationPlatformBridgeWinBrowserTest, HandleEvent) {
 ...
 <action content="Click" arguments="args" activationType="foreground"/>
  ...
  bridge->ForwardHandleEventForTesting(NotificationCommon::CLICK, &toast,
                                       &args);
  content::RunAllPendingInMessageLoop();

  // Validate the click values.
  base::Optional<int> action_index = 1;
  EXPECT_TRUE(ValidateNotificationValues(
      NotificationCommon::CLICK, NotificationCommon::PERSISTENT,
      GURL("https://example.com/"), "notification_id", action_index,
