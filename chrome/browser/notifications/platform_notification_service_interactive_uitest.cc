...
IN_PROC_BROWSER_TEST_F(PlatformNotificationServiceMojoEnabledBrowserTest,
  ...
  ASSERT_EQ("Title2", base::UTF16ToUTF8(notification.title()));
  ASSERT_EQ("Contents", base::UTF16ToUTF8(notification.message()));
  ASSERT_NE(notification.id(), default_notification.id());
  ...
  const message_center::Notification& replacement = notifications[1];
