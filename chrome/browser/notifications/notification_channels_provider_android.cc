...
NotificationChannelsProviderAndroid::NotificationChannelsProviderAndroid(
    PrefService* prefs)
    : NotificationChannelsProviderAndroid(prefs,
...
NotificationChannelsProviderAndroid::NotificationChannelsProviderAndroid(
    ...
    : prefs_(prefs), bridge_(std::move(bridge)),
...
base::Time NotificationChannelsProviderAndroid::GetWebsiteSettingLastModified(
  ...
  //if (!timestamps_dictionary->
  //    GetStringWithoutPathExpansion(origin_string, &timestamp_str)) {
  //  return base::Time();
  //}
  int64_t timestamp = 0;
