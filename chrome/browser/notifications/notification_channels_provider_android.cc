...
NotificationChannelsProviderAndroid::GetRuleIterator(
  ...
  if (channels != cached_channels_) {
    ...
    // settings have changed while keeping this method const. This is hacky, but
    // the alternative of removing all the const qualifiers from the call
    // hierarchy would be costly, and in practice this cast should be safe since
    // we never create a const NotificationChannelsProviderAndroid.
