...
class FakeNotificationChannelsBridge
 ...
 public:
  ...
  void SetChannelStatus(const std::string& origin,
    ...
    DCHECK(status != NotificationChannelStatus::UNAVAILABLE);
    ...
    NOTREACHED() << "Must call bridge.CreateChannel before SetChannelStatus.";
  }
  ...
  NotificationChannel CreateChannel(const std::string& origin,
    ...
    std::string channel_id = origin + timestamp;
