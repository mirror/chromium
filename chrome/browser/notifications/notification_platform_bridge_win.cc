  ...
  std::unique_ptr<std::set<std::string>> GetDisplayedFromActionCenter(
    ...
    hr = toast_manager
             .As<winui::Notifications::IToastNotificationManagerStatics2>(
                 &toast_manager2);
    if (FAILED(hr)) {
      LOG(ERROR) << "Failed to get manager 2";
      return displayed_notifications;
    }
    ...
    if (FAILED(hr)) {
      LOG(ERROR) << "Failed to get history 2";
      return displayed_notifications;
    }
    ...
    using winfoundtn::Collections::IVectorView;
    using winui::Notifications::ToastNotification;
    using winui::Notifications::IToastNotification;

    mswr::ComPtr<IVectorView<ToastNotification*>> list;
