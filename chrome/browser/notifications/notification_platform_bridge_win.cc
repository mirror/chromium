...
using base::win::ComBaseUtil;
using base::win::ScopedHString;
using base::win::HStringRef;
...
HRESULT CreateActivationFactory(wchar_t const (&class_name)[size], T** object) {
  ...
  return ComBaseUtil::RoGetActivationFactory(ref_class_name.Get(),
                                             IID_PPV_ARGS(object));
}
...
NotificationPlatformBridgeWin::NotificationPlatformBridgeWin() {
  ...
  com_functions_initialized_ = ComBaseUtil::PreloadFunctions(functions);
}
