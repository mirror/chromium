...
class NotificationPlatformBridgeWinImpl
 ...
 public:
  ...
  bool GetIToastNotificationHistory2(
      mswr::ComPtr<winui::Notifications::IToastNotificationHistory2>*
          notification_history) const WARN_UNUSED_RESULT {
    ...
    hr = toast_manager2->get_History(&history);
    if (FAILED(hr)) {
      LOG(ERROR) << "Failed to get IToastNotificationHistory";
      return false;
    }

    hr = history.As<winui::Notifications::IToastNotificationHistory2>(
  ...
  void RetrieveDisplayedNotifications(
      const base::RepeatingClosure& quit_task,
      const std::vector<winui::Notifications::IToastNotification*>&
          notifications,
      const std::string& profile_id,
      bool incognito,
    ...
    for (winui::Notifications::IToastNotification* notification :
      ...
      if (decoded_profile_id != profile_id || decoded_incognito != incognito)
        continue;
      result->insert(decoded_notification_id);
    }
    quit_task.Run();
  }

  std::unique_ptr<std::set<std::string>> GetDisplayedFromActionCenter(
      const std::string& profile_id,
      bool incognito) const {
    auto displayed_notifications = std::make_unique<std::set<std::string>>();

    mswr::ComPtr<winui::Notifications::IToastNotificationHistory2> history;
    if (!GetIToastNotificationHistory2(&history)) {
      LOG(ERROR) << "Failed to initialize IToastNotificationHistory2";
      return displayed_notifications;
    }
    ...
    mswr::ComPtr<winfoundtn::Collections::IVectorView<
        winui::Notifications::ToastNotification*>>
        list;
    HRESULT hr = history->GetHistoryWithId(application_id.get(), &list);
    if (FAILED(hr)) {
    ...
    RetrieveDisplayedNotifications(base::BindRepeating(&base::DoNothing),
                                   notifications, profile_id, incognito,
                                   displayed_notifications.get());
    return displayed_notifications;
  }
  ...
  void GetDisplayed(const std::string& profile_id,
    ...
    auto displayed_notifications =
        GetDisplayedFromActionCenter(profile_id, incognito);
  ...
  void CoInitializeForTesting() { CoInitialize(NULL); }
...
void NotificationPlatformBridgeWin::RetrieveDisplayedNotificationsForTesting(
    const std::vector<winui::Notifications::IToastNotification*>& notifications,
    const std::string& profile_id,
    bool incognito,
    std::set<std::string>* result,
    const base::RepeatingClosure& quit_task) const {
  PostTaskToTaskRunnerThread(base::BindOnce(
      &NotificationPlatformBridgeWinImpl::CoInitializeForTesting, impl_));
  PostTaskToTaskRunnerThread(base::BindOnce(
      &NotificationPlatformBridgeWinImpl::RetrieveDisplayedNotifications, impl_,
      quit_task, notifications, profile_id, incognito, result));
}
