...
class CombaseFunctions {
 public:
  ...
  bool LoadFunctions() {
    LOG(ERROR) << "*** LoadFunctions " << this << " ";

    combase_dll_ = ::LoadLibrary(L"combase.dll");
    if (!combase_dll_)
      return false;

    LOG(ERROR) << "*** RoGetActivationFactory " << this << " ";
    ...
    LOG(ERROR) << "*** RoActivateInstance " << this << " ";

    activate_instance_func_ = reinterpret_cast<decltype(&::RoActivateInstance)>(
        ::GetProcAddress(combase_dll_, "RoActivateInstance"));
    if (!activate_instance_func_)
      return false;

    LOG(ERROR) << "*** WindowsCreateString " << this << " ";
    ...
    LOG(ERROR) << "*** WindowsDeleteString " << this << " ";
    ...
    create_string_ref_func_ = reinterpret_cast<decltype(&::WindowsCreateStringReference)>(
        ::GetProcAddress(combase_dll_, "WindowsCreateStringReference"));
    if (!create_string_ref_func_)
      return false;

    LOG(ERROR) << "*** DONE " << this << " " << get_factory_func_;
    return true;
  }
  ...
  HRESULT RoGetActivationFactory(HSTRING class_id,
                                 const IID& iid,
                                 void** out_factory) {
    LOG(ERROR) << "*** RoGetActivationFactory call " << this << " " << get_factory_func_;
  ...
  HRESULT RoActivateInstance(HSTRING class_id,
    ...
    LOG(ERROR) << "*** RoActivateInstance call " << this << " " << get_factory_func_;
  ...
  HRESULT WindowsCreateStringReference(const base::char16* src,
                                       uint32_t len,
 ...
 private:
  ...
  decltype(&::RoActivateInstance) activate_instance_func_ = nullptr;
  decltype(&::WindowsCreateString) create_string_func_ = nullptr;
  decltype(&::WindowsDeleteString) delete_string_func_ = nullptr;
  decltype(&::WindowsCreateStringReference) create_string_ref_func_ = nullptr;
};
...
class HStringRef {
 public:
  explicit HStringRef(const base::char16* str) {
    HRESULT hr = GetCombaseFunctions()->WindowsCreateStringReference(
...
HRESULT GetToastNotification(
  ...
  HStringRef ref_class_name(
      RuntimeClass_Windows_Data_Xml_Dom_XmlDocument);

  DebugBreak();
  HRESULT hr =
      GetCombaseFunctions()->RoActivateInstance /*Windows::Foundation::ActivateInstance*/ (ref_class_name.Get(), &document_io);
  if (FAILED(hr)) {
...
NotificationPlatformBridgeWin::NotificationPlatformBridgeWin() {
  com_functions_initialized_ = GetCombaseFunctions()->LoadFunctions();
  LOG(ERROR) << "*** NotificationPlatformBridgeWin ctor " << this << " " << com_functions_initialized_;
}
...
void NotificationPlatformBridgeWin::Display(
  ...
  LOG(ERROR) << "*** NotificationPlatformBridgeWin Display " << this << " " << com_functions_initialized_;
