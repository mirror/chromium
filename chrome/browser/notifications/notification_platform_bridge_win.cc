...
void NotificationPlatformBridgeWin::Close(const std::string& profile_id,
  ...
  // There can be more than one notification to close if we match using only
  // profile_id and notification_id.
  ...
  for (NotificationData* data : to_close) {
    ...
    // but we don't have ToastNotification cached in memory now.
...
void NotificationPlatformBridgeWin::GetDisplayed(
  ...
  for (const auto& item : notifications_) {
    NotificationData* data = item.first;
    if (data->profile_id == profile_id && data->incognito == incognito)
      displayed_notifications->insert(data->notification_id);
  }
