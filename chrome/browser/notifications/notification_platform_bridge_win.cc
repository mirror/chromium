...
HRESULT CreateActivationFactory(wchar_t const (&class_name)[size], T** object) {
  ...
  return base::win::ComBaseUtil::RoGetActivationFactory(ref_class_name.get(),
                                                        IID_PPV_ARGS(object));
}
...
HRESULT GetToastNotification(
  ...
  *toast_notification = nullptr;
  ...
  HRESULT hr = base::win::ComBaseUtil::RoActivateInstance(ref_class_name.get(),
                                                          &inspectable);
  if (FAILED(hr)) {
...
NotificationPlatformBridgeWin::NotificationPlatformBridgeWin() {
  com_functions_initialized_ =
      ...
      base::win::ComBaseUtil::PreloadRoActivateInstance() &&
