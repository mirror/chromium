...
HRESULT CreateActivationFactory(wchar_t const (&class_name)[size], T** object) {
  ScopedHString ref_class_name = ScopedHString::Create(class_name);
  return base::win::com_base_util::RoGetActivationFactory(ref_class_name.get(),
                                                          IID_PPV_ARGS(object));
}
...
HRESULT GetToastNotification(
  ...
  *toast_notification = nullptr;
  ...
  HRESULT hr = base::win::com_base_util::RoActivateInstance(
      ref_class_name.get(), &inspectable);
  if (FAILED(hr)) {
  ...
  ScopedHString ref_template =
      ScopedHString::Create(notification_template.c_str());
...
NotificationPlatformBridgeWin::NotificationPlatformBridgeWin() {
  com_functions_initialized_ =
      base::win::com_base_util::PreloadRequiredFunctions() &&
...
void NotificationPlatformBridgeWin::Display(
  ...
  ScopedHString application_id =
      ScopedHString::Create(browser_model_id.c_str());
