...
// - A static singleton |toast_event_handler_| is used throughout. As a result,
...
class ToastEventHandler {
 public:
  ...
  // This callback is invoked the notification toast is clicked. This can occur
  // at any time.
  HRESULT ToastEventHandler::OnActivated(
  ...
  HRESULT ToastEventHandler::OnDismissed(
      winui::Notifications::IToastNotification* notification,
  ...
  HRESULT ToastEventHandler::OnFailed(
      winui::Notifications::IToastNotification* notification,
 ...
 private:
  void ToastEventHandler::PostHandlerOnUIThread(
      EventType type,
      winui::Notifications::IToastNotification* notification) {
    // TODO(chengx): extract profile id and notification id from |notification|
    std::string profile_id = "";
    std::string notification_id = "";

    content::BrowserThread::PostTask(
        content::BrowserThread::UI, FROM_HERE,
        base::Bind(&ToastEventHandler::HandleOnUIThread, type, profile_id,
                   notification_id));
  }

  static void ToastEventHandler::HandleOnUIThread(
      EventType type,
    ...
    switch (type) {
      case EVENT_TYPE_ACTIVATED: {
        notification_bridge_win->OnClickEvent(profile_id, notification_id);
        break;
      }
      case EVENT_TYPE_DISMISSED: {
        notification_bridge_win->OnCloseEvent(profile_id, notification_id);
        break;
      }
...
void NotificationPlatformBridgeWin::Display(
  ...
  // TODO(chengx): remove this handler if we decide not to handle this event.
  auto failed_handler = mswr::Callback<ToastFailedHandler>(
      &toast_event_handler_, &ToastEventHandler::OnFailed);
