...
TempFileKeeper::TempFileKeeper(const base::string16& dir_prefix,
...
TempFileKeeper::~TempFileKeeper() {}

FilePath TempFileKeeper::RegisterTemporaryImage(gfx::Image image,
                                                const GURL& origin,
                                                const base::string16& prefix) {
  ResetTempFiles();
  ...
  task_runner_->PostDelayedTask(
      FROM_HERE,
      base::BindOnce(&TempFileKeeper::DeleteSingleFile,
                     weak_factory_.GetWeakPtr(), temp_file),
      kDeletionDelay);

  return temp_file;
}

void TempFileKeeper::ResetTempFiles() {
...
void TempFileKeeper::DeleteOrphanedImages() {
  FilePath temp;
...
void TempFileKeeper::DeleteSingleFile(const FilePath& path) {
