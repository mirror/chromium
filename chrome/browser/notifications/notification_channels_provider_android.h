...
struct NotificationChannel {
  NotificationChannel(std::string id,
                      std::string origin,
...
class NotificationChannelsProviderAndroid
 ...
 public:
  ...
  class NotificationChannelsBridge {
   public:
    ...
    virtual void CreateChannel(const std::string& origin,
 ...
 private:
  ...
  // As well as acting as a lookup-map for channel IDs and channel timestamps,
  // it is used to check if any channels have changed status since the last time
  ...
  // TODO(awdf): Ensure cache is initialized.
