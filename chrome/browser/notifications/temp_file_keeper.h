...
class TempFileKeeper : public TempFileKeeperBase {
 public:
  TempFileKeeper(const base::string16& dir_prefix,
  ...
  // created, which will be valid for a few seconds only (will be deleted either
  // after a short timeout or after a restart of Chrome (the next time this
  // function is called).
 ...
 private:
  ...
  // Deletes a single file from |path|.
  void DeleteSingleFile(const base::FilePath& path);
  ...
  base::WeakPtrFactory<TempFileKeeper> weak_factory_;

  DISALLOW_COPY_AND_ASSIGN(TempFileKeeper);
};
