...
TEST_F(NotificationPlatformBridgeWinTest, GetDisplayed) {
  ...
  // Validate that empty list of notifications show 0 results.
  {
    ...
    notification_platform_bridge_win_->RetrieveDisplayedNotificationsForTesting(
        notifications, "Default", false, &results, run_loop.QuitClosure());
    run_loop.Run();
  ...
  // Query for profile P1 in incognito (should return 1 item).
  {
    ...
    std::set<std::string> results;
    notification_platform_bridge_win_->RetrieveDisplayedNotificationsForTesting(
        notifications, "P1", true, &results, run_loop.QuitClosure());
    run_loop.Run();
    EXPECT_EQ(1U, results.size());
    EXPECT_EQ(1U, results.count("P1i"));
  }
  ...
  // Query for profile P1 not in incognito (should return 1 item).
  {
    ...
    std::set<std::string> results;
    notification_platform_bridge_win_->RetrieveDisplayedNotificationsForTesting(
        notifications, "P1", false, &results, run_loop.QuitClosure());
    run_loop.Run();
    EXPECT_EQ(1U, results.size());
    EXPECT_EQ(1U, results.count("P1reg"));
  }
  ...
  // Query for profile P2 in incognito (should return 1 item).
  {
    ...
    std::set<std::string> results;
    notification_platform_bridge_win_->RetrieveDisplayedNotificationsForTesting(
        notifications, "P2", true, &results, run_loop.QuitClosure());
    run_loop.Run();
    EXPECT_EQ(1U, results.size());
    EXPECT_EQ(1U, results.count("P2i"));
  }
  ...
  // Query for profile P2 not in incognito (should return 1 item).
  {
    ...
    std::set<std::string> results;
    notification_platform_bridge_win_->RetrieveDisplayedNotificationsForTesting(
        notifications, "P2", false, &results, run_loop.QuitClosure());
    run_loop.Run();
    EXPECT_EQ(1U, results.size());
    EXPECT_EQ(1U, results.count("P2reg"));
  }
}
