...
const wchar_t kOrigin[] = L"https://www.google.com/";
...
class NotificationPlatformBridgeWinTest : public testing::Test {
 public:
  NotificationPlatformBridgeWinTest() = default;
  ~NotificationPlatformBridgeWinTest() override = default;

  void SetUp() override {
    notification_platform_bridge_win_ =
        base::WrapUnique(new NotificationPlatformBridgeWin());
  }

  HRESULT getToast(
      ABI::Windows::UI::Notifications::IToastNotification** toast) {
    std::string notification_id(kNotificationId);
    GURL origin(kOrigin);
    auto notification = std::make_unique<message_center::Notification>(
        message_center::NOTIFICATION_TYPE_SIMPLE, notification_id, L"title",
...
TEST_F(NotificationPlatformBridgeWinTest, SetUpAndTearDown) {}
