...
class PrintViewManagerBase : public content::NotificationObserver,
 ...
 public:
  ...
  // empty string if the print job is started successfully. |rfh| and |rph|
  // should be the render frame host and render process host for the print
  // preview initiator contents respectively.
  // On MacOS, |print_data| should contain only the first page of data to print
  // and |get_page_callback| will be called to obtain the other pages.
  using PrintCallback = base::OnceCallback<void(const base::Value& error)>;
  using GetPageCallback =
      base::RepeatingCallback<scoped_refptr<base::RefCountedBytes>(int page)>;
...
// Helpers for PrintForPrintPreview();
  ...
  void OnGotPrintData(PrintCallback callback,
                      const GetPageCallback& get_page_callback,
  ...
  // Starts printing page |page_num| of a document with data given in
  // |print_data|. |print_data| must successfully initialize a metafile if
  // |metafile_must_be_valid| is true. |document| is the printed document
