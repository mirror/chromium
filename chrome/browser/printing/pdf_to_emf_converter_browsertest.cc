...
IN_PROC_BROWSER_TEST_F(PDFToEMFConverterBrowserTest, TestSuccess) {
  ...
  for (int i = 0; i < kNumberOfPages; i++) {
    ...
    // The encoder code can return bad data. Until this is fixed we test both
    // sets of data.
    base::FilePath expected_bad_emf_file = test_data_dir.AppendASCII(
        "chrome/test/data/printing/pdf_to_emf_test_page_" +
        std::to_string(i + 1 /* page numbers start at 1 */) + "_bad.emf");
    std::string expected_bad_emf_data;
    ASSERT_TRUE(
        base::ReadFileToString(expected_bad_emf_file, &expected_bad_emf_data));
    ASSERT_GT(expected_bad_emf_data.length(), 0U);

    if (expected_emf_data != actual_emf_data) {
      ASSERT_EQ(expected_bad_emf_data.length(), actual_emf_data.length());
      ASSERT_EQ(expected_bad_emf_data, actual_emf_data);
      LOG(ERROR) << "Used the bad data for page " << i;
    }
  }
}
