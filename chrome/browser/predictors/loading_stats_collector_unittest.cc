...
const char kRedirectedUrl[] = "http://www.google.fr/dogs";
const char kRedirectedUrl2[] = "http://www.google.de/raccoons";
}
...
TEST_F(LoadingStatsCollectorTest, TestPrefetchHitsMissesHistograms) {
  ...
  GURL url("http://google.com/?query=cats");
  LOG(ERROR) << url.GetOrigin().spec();
  LOG(ERROR) << url.host();
}
