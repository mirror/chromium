...
void ResourcePrefetchPredictor::OnVisitCountLookup(
  ...
  if (config_.is_url_learning_enabled) {
    ...
    if (should_track_url) {
      LearnNavigation(url_spec, summary.subresource_requests,
                      url_resource_data_.get(), summary.initial_url.spec(),
  ...
  LearnNavigation(host, summary.subresource_requests, host_resource_data_.get(),
                  summary.initial_url.host(), host_redirect_data_.get());
...
void ResourcePrefetchPredictor::LearnNavigation(
    ...
    GlowplugKeyValueData<RedirectData, internal::ProtoCompare>* redirect_data) {
  ...
  LearnRedirect(key_before_redirects, key, redirect_data);
}

void ResourcePrefetchPredictor::LearnRedirect(
    const std::string& key,
