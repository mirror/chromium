...
void ResourcePrefetchPredictor::StartInitialization() {
  ...
  // will be passed to the reply task. We can't inline these calls into the call
  // to base::BindOnce because the order of evaluation of arguments is
  // unspecified. Thus, the unique_ptr could be moved to the callback before we
  // get the raw pointer.
  auto* url_resource_data_ptr = url_resource_data.get();
  auto* host_resource_data_ptr = host_resource_data.get();
  auto* url_redirect_data_ptr = url_redirect_data.get();
  auto* host_redirect_data_ptr = host_redirect_data.get();
  auto* manifest_data_ptr = manifest_data.get();
  auto* origin_data_ptr = origin_data.get();

  BrowserThread::PostTaskAndReply(
      BrowserThread::DB, FROM_HERE,
      base::BindOnce(InitializeOnDBThread, url_resource_data_ptr,
                     host_resource_data_ptr, url_redirect_data_ptr,
                     host_redirect_data_ptr, manifest_data_ptr,
                     origin_data_ptr),
      base::BindOnce(
          &ResourcePrefetchPredictor::CreateCaches, weak_factory_.GetWeakPtr(),
          base::Passed(&url_resource_data), base::Passed(&host_resource_data),
          base::Passed(&url_redirect_data), base::Passed(&host_redirect_data),
          base::Passed(&manifest_data), base::Passed(&origin_data)));
}
