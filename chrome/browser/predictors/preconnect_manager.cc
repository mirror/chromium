...
void PreconnectManager::StartDetached(
  ...
  for (auto it = preresolve_hosts.rbegin(); it != preresolve_hosts.rend();
       ++it) {
    queued_jobs_.emplace_front(*it, false /* need_preconnect */,
                               allow_credentials, nullptr);
  }

  for (auto it = preconnect_origins.rbegin(); it != preconnect_origins.rend();
       ++it) {
    queued_jobs_.emplace_front(*it, true /* need_preconnect */,
                               allow_credentials, nullptr);
  }
