...
const size_t PreconnectManager::kMaxInflightPreresolves = 3;
...
void PreconnectManager::PreconnectUrl(
  ...
  content::PreconnectUrl(context_getter, url, first_party_for_cookies, count,
...
int PreconnectManager::PreresolveUrl(
  ...
  return content::PreresolveUrl(context_getter, url, callback);
}
...
void PreconnectManager::TryToLaunchPreresolveJobs() {
  ...
  while (!queued_jobs_.empty() &&
    ...
    if (!info->was_canceled) {
      int status =
          PreresolveUrl(context_getter_.get(), job.url,
...
void PreconnectManager::FinishPreresolve(const PreresolveJob& job, bool found) {
  ...
  if (found && job.need_preconnect && !job.info->was_canceled) {
    PreconnectUrl(context_getter_.get(), job.url, job.info->url, 1,
