...
void ResourcePrefetchPredictorTablesTest::TestDeleteData() {
  ...
  tables_->ExecuteDBTaskOnDBThread(
      base::Bind(&GlowplugKeyValueTable<precache::PrecacheManifest>::DeleteData,
                 base::Unretained(tables_->manifest_table()), hosts_to_delete));

  hosts_to_delete = {"twitter.com"};
  tables_->ExecuteDBTaskOnDBThread(
      base::Bind(&GlowplugKeyValueTable<OriginData>::DeleteData,
...
void ResourcePrefetchPredictorTablesTest::TestUpdateData() {
  ...
  tables_->ExecuteDBTaskOnDBThread(
      base::Bind(&GlowplugKeyValueTable<RedirectData>::UpdateData,
...
void ResourcePrefetchPredictorTablesTest::DeleteAllData() const {
  tables_->ExecuteDBTaskOnDBThread(
      base::Bind(&GlowplugKeyValueTable<PrefetchData>::DeleteAllData,
                 base::Unretained(tables_->url_resource_table())));
  tables_->ExecuteDBTaskOnDBThread(
      base::Bind(&GlowplugKeyValueTable<RedirectData>::DeleteAllData,
                 base::Unretained(tables_->url_redirect_table())));
  tables_->ExecuteDBTaskOnDBThread(
      base::Bind(&GlowplugKeyValueTable<PrefetchData>::DeleteAllData,
                 base::Unretained(tables_->host_resource_table())));
  ...
  tables_->ExecuteDBTaskOnDBThread(
      base::Bind(&GlowplugKeyValueTable<OriginData>::DeleteAllData,
...
void ResourcePrefetchPredictorTablesTest::GetAllData(
  ...
  tables_->ExecuteDBTaskOnDBThread(
      base::Bind(&GlowplugKeyValueTable<precache::PrecacheManifest>::GetAllData,
                 base::Unretained(tables_->manifest_table()), manifest_data));
  tables_->ExecuteDBTaskOnDBThread(
      base::Bind(&GlowplugKeyValueTable<OriginData>::GetAllData,
