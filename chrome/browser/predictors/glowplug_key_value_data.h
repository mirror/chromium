...
// The class provides a syncrhonous access to the data backed by
...
class GlowplugKeyValueData {
 public:
  ...
  // |key| exists, false otherwise. |data| pointer may be equal nullptr to get
  ...
  // Deletes data associated with the |key| from the database.
...
void GlowplugKeyValueData<T, Compare>::InitializeOnDBThread() {
  ...
  if (keys_to_delete.size() > 0) {
    tables_->ExecuteDBTaskOnDBThread(base::Bind(
...
void GlowplugKeyValueData<T, Compare>::UpdateData(const std::string& key,
  ...
  if (it == data_cache_->end()) {
    if (data_cache_->size() >= max_size_) {
      ...
      std::string key_to_delete = entry_to_delete->first;
      data_cache_->erase(entry_to_delete);
      tables_->ScheduleDBTask(
          FROM_HERE, base::Bind(&GlowplugKeyValueTable<T>::DeleteData,
  ...
  tables_->ScheduleDBTask(
      FROM_HERE, base::Bind(&GlowplugKeyValueTable<T>::UpdateData,
...
void GlowplugKeyValueData<T, Compare>::DeleteAllData() {
  ...
  tables_->ScheduleDBTask(FROM_HERE,
                          base::Bind(&GlowplugKeyValueTable<T>::DeleteAllData,
                                     base::Unretained(backend_table_)));
}
