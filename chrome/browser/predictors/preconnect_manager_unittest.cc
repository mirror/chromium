...
class MockPreconnectManager : public PreconnectManager {
 public:
  ...
  MOCK_CONST_METHOD6(PreconnectUrl,
                     void(net::URLRequestContextGetter* context_getter,
                          const GURL& url,
  ...
  MOCK_CONST_METHOD3(PreresolveUrl,
                     int(net::URLRequestContextGetter* context_getter,
                         const GURL& url,
...
TEST_F(PreconnectManagerTest, TestUnqueuedPreresolvesCanceled) {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    EXPECT_CALL(*preconnect_manager_,
                PreresolveUrl(_, urls_to_preconnect.back(), _))
        .WillOnce(
            DoAll(SaveArg<2>(&callbacks[i]), Return(net::ERR_IO_PENDING)));
  }
...
TEST_F(PreconnectManagerTest, TestTwoConcurrentMainFrameUrls) {
  ...
  EXPECT_CALL(*preconnect_manager_, PreresolveUrl(_, url_to_preconnect1, _))
      .WillOnce(DoAll(SaveArg<2>(&callback1), Return(net::ERR_IO_PENDING)));
  EXPECT_CALL(*preconnect_manager_, PreresolveUrl(_, url_to_preconnect2, _))
      .WillOnce(DoAll(SaveArg<2>(&callback2), Return(net::ERR_IO_PENDING)));
  ...
  EXPECT_CALL(*preconnect_manager_,
              PreconnectUrl(_, url_to_preconnect1, main_frame_url1, 1, true,
                            net::HttpRequestInfo::PRECONNECT_MOTIVATED));
