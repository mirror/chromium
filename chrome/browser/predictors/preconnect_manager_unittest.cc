...
class MockPreconnectImpl : public internal::PreconnectImpl {
 public:
...
class PreconnectManagerTest : public testing::Test {
 ...
 protected:
  ...
  StrictMock<MockPreconnectImpl>* mock_impl_;
...
PreconnectManagerTest::PreconnectManagerTest()
    : mock_delegate_(
      ...
      preconnect_manager_(
          base::MakeUnique<PreconnectManager>(mock_delegate_->AsWeakPtr(),
                                              context_getter_)) {
  auto mock_impl = base::MakeUnique<StrictMock<MockPreconnectImpl>>();
  mock_impl_ = mock_impl.get();
  preconnect_manager_->SetMockImplForTesting(std::move(mock_impl));
}
...
TEST_F(PreconnectManagerTest, TestStartOneUrlPreresolve) {
  ...
  EXPECT_CALL(*mock_impl_, PreresolveUrl(_, url_to_preresolve, _))
...
TEST_F(PreconnectManagerTest, TestStartOneUrlPreconnect) {
  ...
  EXPECT_CALL(*mock_impl_, PreresolveUrl(_, url_to_preconnect, _))
      .WillOnce(Return(net::OK));
