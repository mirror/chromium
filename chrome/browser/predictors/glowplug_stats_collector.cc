...
void ReportPrefetchAccuracy(
  ...
  for (const auto& request : requests)
    urls.insert(request.resource_url);
...
GlowplugStatsCollector::GlowplugStatsCollector(
    ResourcePrefetchPredictor* predictor,
    const LoadingPredictorConfig& config)
    : predictor_(predictor), config_(config) {}

GlowplugStatsCollector::~GlowplugStatsCollector() = default;

void GlowplugStatsCollector::RecordPrefetcherStats(
...
void GlowplugStatsCollector::RecordPageRequestSummary(
    const ResourcePrefetchPredictor::PageRequestSummary& summary) {
...
void GlowplugStatsCollector::CleanupAbandonedStats() {
  static const base::TimeDelta max_age =
  ...
  for (auto it = prefetcher_stats_.begin(); it != prefetcher_stats_.end();) {
    if (time_now - it->second->start_time > max_age) {
      // No requests -> everything is a miss.
      ReportPrefetchAccuracy(
          *it->second,
          std::vector<ResourcePrefetchPredictor::URLRequestSummary>());
