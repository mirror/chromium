...
// class doesn't manage the creation and the deleting of the table.
//
...
//     FROM_HERE,
//     base::Bind(&GlowplugKeyValueTable<PrefetchData>::UpdateData,
...
void GlowplugKeyValueTable<T>::DeleteData(const std::vector<std::string>& keys,
                                          sql::Connection* db) {
  auto statement = db->GetUniqueStatement(
