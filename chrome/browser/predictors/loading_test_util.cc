...
PreconnectPrediction CreatePreconnectPrediction(
  ...
  prediction.preconnect_urls = preconnect_urls;
  prediction.preresolve_urls = preresolve_urls;
  return prediction;
}
...
std::ostream& operator<<(std::ostream& os,
  ...
  for (const auto& url : prediction.preconnect_urls)
    os << "\t\t" << url << std::endl;

  os << "Preresolve:" << std::endl;
  for (const auto& url : prediction.preresolve_urls)
    os << "\t\t" << url << std::endl;
  return os;
}
...
bool operator==(const PreconnectPrediction& lhs,
  ...
  return lhs.is_redirected == rhs.is_redirected && lhs.host == rhs.host &&
         lhs.preconnect_urls == rhs.preconnect_urls &&
         lhs.preresolve_urls == rhs.preresolve_urls;
}
