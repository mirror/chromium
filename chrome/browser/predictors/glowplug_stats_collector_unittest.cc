...
class GlowplugStatsCollectorTest : public testing::Test {
 public:
  GlowplugStatsCollectorTest();
  ~GlowplugStatsCollectorTest() override;
  void SetUp() override;
...
GlowplugStatsCollectorTest::GlowplugStatsCollectorTest()
...
void GlowplugStatsCollectorTest::SetUp() {
  ...
  mock_predictor_ = base::MakeUnique<StrictMock<MockResourcePrefetchPredictor>>(
      config, profile_.get());
  stats_collector_ =
      base::MakeUnique<GlowplugStatsCollector>(mock_predictor_.get(), config);
  histogram_tester_ = base::MakeUnique<base::HistogramTester>();
  base::RunLoop().RunUntilIdle();
}

void GlowplugStatsCollectorTest::TearDown() {
  stats_collector_ = nullptr;
  mock_predictor_ = nullptr;
  histogram_tester_ = nullptr;
}

void GlowplugStatsCollectorTest::TestRedirectStatusHistogram(
...
TEST_F(GlowplugStatsCollectorTest, TestPrecisionRecallHistograms) {
...
TEST_F(GlowplugStatsCollectorTest, TestPrefetchHitsMissesHistograms) {
  const std::string main_frame_url = "http://google.com/?query=cats";
  const std::string& script_url = "https://cdn.google.com/script.js";
  EXPECT_CALL(*mock_predictor_, GetPrefetchData(GURL(main_frame_url), _))
