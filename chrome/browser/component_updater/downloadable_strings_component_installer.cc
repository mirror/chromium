...
// This provides two important methods:
//
//   - RecordRegistrationTime():
//       Must be called when component registration starts, and before
//       forcing the on-demand update/download of the component.
//
//   - RecordInstallationTime(error):
//       Must be called once the install/update completes, and passed
...
#define DECLARE_ENUM_VALUE(name) name,
  enum class UmaLocale {
    CHROME_LOCALES_LIST(DECLARE_ENUM_VALUE) COUNT,  // DO NOT REMOVE
    UNKNOWN = 500,
  };
#undef DECLARE_ENUM_VALUE

  // Sanity check.
  static_assert((int)UmaLocale::UNKNOWN > (int)UmaLocale::COUNT,
  ...
  static std::string UmaLocaleToString(UmaLocale locale) {
    int locale_index = static_cast<int>(locale);
  ...
  void RecordUmaMetrics(int error) {
    ...
    UmaLocale uma_locale = UmaLocale::UNKNOWN;

#if defined(OS_ANDROID)
    std::string sim_mcc_mnc = net::android::GetTelephonySimOperator();
    // Check the result value, because the documentation for
    // TelephonyManager.getSimOperator() says results are unreliable
    // on CDMA networks.
    if (sim_mcc_mnc.size() >= 3) {
      if (!base::StringToInt(base::StringPiece(&sim_mcc_mnc[0], 3U),
                             &mobile_country_code)) {
    ...
    uma_locale = LocaleNameToUmaLocale(default_locale);
    ...
    base::TimeTicks startup_ticks = startup_metric_utils::MainEntryPointTicks();
    base::TimeDelta registration_time =
        component_registration_ticks_ - startup_ticks;

    base::TimeDelta installation_time =
