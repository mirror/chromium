...
#undef DECLARE_ENUM_VALUE
  ...
  static int ScoreLocaleMatch(base::StringPiece locale,
    ...
    const size_t locale_underscore_pos = locale.find('_');
    const base::StringPiece locale_country =
    ...
    const size_t chrome_underscore_pos = chrome_locale.find('_');
    if (chrome_underscore_pos != base::StringPiece::npos) {
      const base::StringPiece chrome_country =
  ...
  static UmaLocale LocaleNameToUmaLocale(const base::StringPiece locale_name) {
    ...
    for (int n = 0; n < kLocaleCount; ++n) {
      ...
      const int score = ScoreLocaleMatch(name, kLocaleNames[n]);
...
void RenameComponentDirectory() {
  ...
  const base::FilePath dst_dir = component_root.Append(
      kRelativeInstallRenamedDir);
