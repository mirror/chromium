...
class SwReporterInstallerTest
 ...
 protected:
  void SwReporterLaunched(
    ...
    if (on_reporter_done)
      std::move(on_reporter_done).Run(SwReporterInvocationResult::kInterrupted);
  }
  ...
  void ExpectDefaultInvocation() const {
    ...
    EXPECT_EQ(SwReporterInvocation::BEHAVIOUR_ENABLED_BY_DEFAULT,
  ...
  safe_browsing::SwReporterInvocationsSequence launched_invocations_;
...
class ExperimentalSwReporterInstallerTest : public SwReporterInstallerTest {
 ...
 protected:
  ...
  void ExpectExperimentalInvocationInSeries(
    ...
    launched_invocations_.container().pop_front();
    EXPECT_EQ(MakeTestFilePath(default_path_),
              invocation.command_line().GetProgram());
