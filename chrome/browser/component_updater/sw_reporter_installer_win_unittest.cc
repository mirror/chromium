...
class SwReporterOnDemandFetcherTest : public ::testing::Test,
 ...
 public:
  ...
  void SetUpWithUpdateableComponent(bool can_be_updated) {
    component_can_be_updated_ = can_be_updated;

    fetcher_ = base::MakeUnique<SwReporterOnDemandFetcher>(
        &mock_cus_, base::BindOnce(
                        [](SwReporterOnDemandFetcherTest* fetcher_test) {
                          fetcher_test->error_callback_called_ = true;
                        },
                        base::Unretained(this)));
  }
  ...
  void OnDemandUpdate(const std::string& crx_id, Callback callback) override {
    ...
    // Post a task instead of calling directly since |fecher_| is not yet set to
    // to SwReporterOnDemandFetcher being initialized.
