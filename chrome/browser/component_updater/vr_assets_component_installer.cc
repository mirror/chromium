...
const uint8_t kPublicKeySHA256[32] = {0x29, 0x5a, 0x1f, 0x3f, 0x9f, 0x3b, 0x37, 0x2b, 0x07, 0xf5, 0x6d, 0xb7, 0xfe, 0x3b, 0xfd, 0x10};

const char kVrAssetsComponentName[] = "VR Assets";
const base::FilePath::CharType kRelativeInstallDir[] = FILE_PATH_LITERAL("VrAssets");
...
bool VrAssetsComponentInstallerTraits::RequiresNetworkEncryption()
    const {
...
base::FilePath VrAssetsComponentInstallerTraits::GetInstalledPath(
  ...
  LOG(INFO) << "VrAssetsComponent - GetInstalledPath " << base.Append(kEnvironmentFileName).value();
...
void VrAssetsComponentInstallerTraits::ComponentReady(
  ...
  LOG(INFO) << "VrAssetsComponent - Component ready, version " << version.GetString() << " in "
          << install_dir.value();
...
base::FilePath VrAssetsComponentInstallerTraits::GetRelativeInstallDir()
    const {
  LOG(INFO) << "VrAssetsComponent - GetRelativeInstallDir " << kRelativeInstallDir;
...
VrAssetsComponentInstallerTraits::GetMimeTypes() const {
