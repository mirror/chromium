...
void SwReporterInstallerPolicy::ComponentReady(
  ...
  if (ExtractInvocationSequenceFromManifest(exe_path, std::move(manifest),
                                            &invocations)) {
    // This will be |safe_browsing::OnSwReporterReady| except in tests.
    content::BrowserThread::PostAfterStartupTask(
        FROM_HERE,
        content::BrowserThread::GetTaskRunnerForThread(
            content::BrowserThread::UI),
        base::BindOnce(on_component_ready_callback_,
...
void RegisterSwReporterComponent(ComponentUpdateService* cus) {
  ...
  auto installer = base::MakeRefCounted<ComponentInstaller>(
      std::make_unique<SwReporterInstallerPolicy>(
          base::BindRepeating(&safe_browsing::OnSwReporterReady)));
