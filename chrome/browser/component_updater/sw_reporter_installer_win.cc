...
// TODO(crbug.com/786964): Remove this feature entirely once it's no longer
...
void RunSwReporters(const base::FilePath& exe_path,
  ...
  if (!parameter_list || parameter_list->empty()) {
    ...
    reporter_runner.Run(safe_browsing::SwReporterQueue({invocation}), version);
    return;
  }
...
SwReporterInstallerPolicy::GetInstallerAttributes() const {
  ...
  // TODO(crbug.com/786964): Allow tag to be passed without enabling this
  // feature for manual testing.
