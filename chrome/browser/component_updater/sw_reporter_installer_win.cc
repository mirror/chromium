...
void RunSwReporters(const base::FilePath& exe_path,
  ...
  // Ensure that |on_error_closure| is invoked in case of an early return that
  // prevents the reporter sequence from starting.
  ...
  // The error callback can be ignored at this point. Failures while running
  // the reporter will be handled by the reporter runner.
...
void SwReporterInstallerPolicy::RunSavedCallbackOrNotifyFailure() {
  ...
  if (run_sw_reporters_) {
    // The reporter component was not updated (it's either up to date or
    // updating failed). Run the component previously installed.
    run_sw_reporters_->Run(invocation_type_, std::move(on_error_closure_));
  } else {
    // The reporter component was not updated, but there is no component
    // installed that can be used. End with an error.
    std::move(on_error_closure_).Run();
  }

  // Ensure the next run will be periodic, regardless of the type of the current
  // run. This is needed due to logs uploading and cleanup prompt implications.
  invocation_type_ = SwReporterInvocationType::kPeriodicRun;
}
...
void SwReporterInstallerPolicy::ComponentReady(
  ...
  // Save state needed to allow user-initiated runs to be started for this
  // reporter version.
...
void SwReporterInstallerPolicy::UpdateAndRunReporterSequence(
...
void SwReporterInstallerPolicy::OnEvent(Events event, const std::string& id) {
  ...
  // from the observer list for the update service.
  ...
  // from the callback saved on the most recent installation, or notify the
  // caller of a failure if there is no Software Reporter component available.
...
void OnDemandUpdateAndRunSwReporterComponent(
  ...
  current_policy->UpdateAndRunReporterSequence(
      invocation_type, std::move(on_error_closure), cus);
}
