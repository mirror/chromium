...
using safe_browsing::SwReporterInvocationsSequence;
...
void RunSwReportersAfterStartup(SwReporterInvocationType invocation_type,
...
void RunExperimentalSwReporter(const base::FilePath& exe_path,
  ...
  SwReporterInvocationsSequence invocations(
      version, SwReporterInvocationsSequence::Container(),
...
void SwReporterInstallerPolicy::ComponentReady(
  ...
  if (IsExperimentalEngineEnabled()) {
    ...
    command_line.AppendSwitchASCII(chrome_cleaner::kSessionIdSwitch,
                                   GenerateSessionId());
    SwReporterInvocationsSequence::Container invocations{
        SwReporterInvocation(command_line)
            .WithSupportedBehaviours(
                SwReporterInvocation::BEHAVIOUR_ENABLED_BY_DEFAULT)};
    reporter_runner_.Run(invocation_type_, SwReporterInvocationsSequence(
                                               version, invocations,
                                               std::move(on_sequence_done_)));
  }
}
