...
void CrOSComponent::LoadInternal(
...
void CrOSComponent::Register(ComponentUpdateService* cus,
                             const ComponentConfig& config,
...
bool CrOSComponent::Unload(const std::string& name) {
  const ConfigMap components = CONFIG_MAP_CONTENT;
  const auto it = components.find(name);
  if (it == components.end()) {
...
std::vector<ComponentConfig> CrOSComponent::GetInstalled() {
...
void CrOSComponent::RegisterN(const std::vector<ComponentConfig>& configs) {
...
void CrOSComponent::RegisterInstalled() {
...
void CrOSComponent::RegisterCompatiblePath(const std::string& name,
                                           const base::FilePath& path) {
  compatible_components_[name] = path;
}

void CrOSComponent::UnregisterCompatiblePath(const std::string& name) {
  compatible_components_.erase(name);
}

bool CrOSComponent::IsCompatible(const std::string& name) const {
  return compatible_components_.count(name) > 0;
}

base::FilePath CrOSComponent::GetCompatiblePath(const std::string& name) const {
