...
class ThirdPartyModuleListComponentInstallerTraits
 ...
 private:
  ...
  // This is invoked from the component installer when a component is ready.
  // It is called after every startup if a valid component is already installed.
  // It is also called at arbitrary points during runtime when the component is
  // installed for the first time, or a new version is made available. This
  // simply forwards to the same function on the provided ModuleListManager.
  // This is invoked as a background priority task (not on IO or UI threads)
  // where file IO and blocking is allowed, on the sequence owned by the parent
  // DefaultComponentInstaller.
  void LoadModuleList(const base::Version& version,
                      const base::FilePath& install_dir);
  ...
  base::WeakPtrFactory<ThirdPartyModuleListComponentInstallerTraits>
      weak_ptr_factory_;
