...
void PushMessagingServiceImpl::DidDeleteServiceWorkerDatabase() {
  ...
  base::RepeatingClosure completed_closure = base::BarrierClosure(
      ...
      service_worker_database_deleted_callback_for_testing_.is_null()
          ? base::Bind(&base::DoNothing)
          : service_worker_database_deleted_callback_for_testing_);
  ...
  for (const PushMessagingAppIdentifier& app_identifier : app_identifiers) {
    ...
    UnsubscribeInternal(
        content::PUSH_UNREGISTRATION_REASON_SERVICE_WORKER_DATABASE_DELETED,
        app_identifier.origin(),
        app_identifier.service_worker_registration_id(),
...
void PushMessagingServiceImpl::SetServiceWorkerDatabaseDeleteCallbackForTesting(
    const base::Closure& callback) {
  service_worker_database_deleted_callback_for_testing_ = callback;
}
