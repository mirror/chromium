...
class ChromeNetworkServiceRestartBrowserTest : public InProcessBrowserTest {
 public:
  ...
  int LoadBasicRequest(mojom::NetworkContext* network_context, const std::string& URL) {
    ...
    request->url = embedded_test_server()->GetURL(URL);
...
IN_PROC_BROWSER_TEST_F(ChromeNetworkServiceRestartBrowserTest,
  ...
  EXPECT_EQ(net::OK, LoadBasicRequest(old_network_context, "/echo"));
  ...
  EXPECT_EQ(net::OK, LoadBasicRequest(partition->GetNetworkContext(), "/echo"));
}
...
IN_PROC_BROWSER_TEST_F(ChromeNetworkServiceRestartBrowserTest,
  ...
  // crash.
  EXPECT_NE(old_network_context, partition->GetNetworkContext());
  EXPECT_EQ(net::OK, LoadBasicRequest(partition->GetNetworkContext(), "/echoheader?Cookie"));
}
