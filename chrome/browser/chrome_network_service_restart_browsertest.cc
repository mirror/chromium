...
class ChromeNetworkServiceRestartBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void SimulateNetworkChange(network::mojom::ConnectionType type) {
    ...
    run_loop.Run();
    LOG(ERROR) << "3. ChromeNetworkServiceRestartBrowserTest.SimulateNetworkChange";
...
LOG(ERROR) << "2. ChromeNetworkServiceRestartBrowserTest.LoadBasicRequest";
    ...
    std::unique_ptr<content::SimpleURLLoader> simple_loader =
        content::SimpleURLLoader::Create(std::move(request), TRAFFIC_ANNOTATION_FOR_TESTS);
...
IN_PROC_BROWSER_TEST_F(ChromeNetworkServiceRestartBrowserTest,
                       SimulateNetworkServiceCrash2) {
  LOG(ERROR) << "Begin ChromeNetworkServiceRestartBrowserTest.SimulateNetworkServiceCrash2";
  SimulateNetworkServiceCrash();
  SimulateNetworkServiceCrash();
  LOG(ERROR) << "Finished ChromeNetworkServiceRestartBrowserTest.SimulateNetworkServiceCrash2";
}
...
// after crash.
IN_PROC_BROWSER_TEST_F(ChromeNetworkServiceRestartBrowserTest,
                       StoragePartitionGetNetworkContext2) {
  LOG(ERROR) << "Begin ChromeNetworkServiceRestartBrowserTest.StoragePartitionGetNetworkContext2";
  LOG(ERROR) << "base::Process::Current().Pid() = " << base::Process::Current().Pid();
  LOG(ERROR) << "command_line->HasSwitch(kSingleProcess) = " << base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kSingleProcess);
  LOG(ERROR) << "command_line->HasSwitch(kSingleProcessTestsFlag) = " << base::CommandLine::ForCurrentProcess()->HasSwitch(kSingleProcessTestsFlag);
  ...
  LOG(ERROR) << "1. ChromeNetworkServiceRestartBrowserTest.StoragePartitionGetNetworkContext2";

#if defined(OS_MACOSX)
  // |NetworkServiceTestHelper| doesn't work on browser_tests on macOS.
  return;
#endif
  LOG(ERROR) << "2. ChromeNetworkServiceRestartBrowserTest.StoragePartitionGetNetworkContext2";
  StoragePartition* partition =
      BrowserContext::GetDefaultStoragePartition(browser()->profile());

  LOG(ERROR) << "3. ChromeNetworkServiceRestartBrowserTest.StoragePartitionGetNetworkContext2";

  mojom::NetworkContext* old_network_context = partition->GetNetworkContext();
  EXPECT_EQ(net::OK, LoadBasicRequest(old_network_context));

  LOG(ERROR) << "4. ChromeNetworkServiceRestartBrowserTest.StoragePartitionGetNetworkContext2";

  // Crash the NetworkService process. Existing interfaces should receive error
  // notifications at some point.
  SimulateNetworkServiceCrash();
  LOG(ERROR) << "5. ChromeNetworkServiceRestartBrowserTest.StoragePartitionGetNetworkContext2";
  // Flush the interface to make sure the error notification was received.
  partition->FlushNetworkInterfaceForTesting();
  LOG(ERROR) << "6. ChromeNetworkServiceRestartBrowserTest.StoragePartitionGetNetworkContext2";
  // |partition->GetNetworkContext()| should return a valid new pointer after
  // crash.
  EXPECT_NE(old_network_context, partition->GetNetworkContext());

  LOG(ERROR) << "Flashing NetworkServiceTestPtr";
  mojom::NetworkServiceTestPtr network_service_test;
    ServiceManagerConnection::GetForProcess()->GetConnector()->BindInterface(
        mojom::kNetworkServiceName, &network_service_test);
  network_service_test.FlushForTesting();
  LOG(ERROR) << "Finished Flashing NetworkServiceTestPtr";

  EXPECT_EQ(net::OK, LoadBasicRequest(partition->GetNetworkContext()));
  LOG(ERROR) << "7 ChromeNetworkServiceRestartBrowserTest.StoragePartitionGetNetworkContext2";
  EXPECT_EQ(net::OK, LoadBasicRequest(partition->GetNetworkContext()));
  LOG(ERROR) << "Finished ChromeNetworkServiceRestartBrowserTest.StoragePartitionGetNetworkContext2";
}
