...
const std::string GetBaseLanguageCode(const std::string& language_code) {
...
std::string ChromeHttpUserAgentSettings::ExpandLanguageList(
  ...
  //     "zh" does not refer to a unique root language for Chinese, so we
  //     cannot use it as a backoff for "zh-TW" nor "zh-CN".
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (j >= size) {
      ...
      const std::string next_language = GetBaseLanguageCode(languages[j]);
      if (next_language != base_language) {
        add = true;
      }
    }
    if (add) {
