...
class NetworkContextConfigurationBrowserTest
 ...
 public:
  ...
  // Set the proxy preference on a PrefService based on the NetworkContextType,
  ...
  void SetProxyPref(const net::HostPortPair& host_port_pair) {
    ...
    ProxyConfigMonitor* proxy_config_monitor = nullptr;
    switch (GetParam().network_context_type) {
      case NetworkContextType::kSystem:
        pref_service = g_browser_process->local_state();
        proxy_config_monitor =
            g_browser_process->system_network_context_manager()
                ->GetProxyConfigMonitorForTesting();
        break;
      case NetworkContextType::kProfile:
...
// ChromeOS ignores the proxy pref by default, so need to override that.
#if defined(OS_CHROMEOS)
    pref_service->SetBoolean(proxy_config::prefs::kUseSharedProxies, true);
    ...
    EXPECT_EQ("PROXY " + host_port_pair.ToString(),
              proxy_config_monitor->GetProxyConfigForTesting()
                  .proxy_rules()
                  .single_proxies.Get()
                  .ToPacString());
    EXPECT_EQ("Foo " + host_port_pair.ToString(),
              proxy_config_monitor->GetProxyConfigForTesting()
                  .proxy_rules()
                  .single_proxies.Get()
                  .ToPacString());

    proxy_config_monitor->FlushForTesting();
  }
