...
IN_PROC_BROWSER_TEST_P(NetworkContextConfigurationBrowserTest, Cache) {
  ...
  EXPECT_TRUE(simple_loader->ResponseInfo()->headers->GetMaxAgeValue(&max_age));
  ASSERT_TRUE(simple_loader_helper.response_body());
  EXPECT_GE(simple_loader_helper.response_body()->size(), 0u);
  ...
  if (GetHttpCacheType() == StorageType::kNone) {
    ...
    // failed.
    EXPECT_FALSE(simple_loader2->ResponseInfo());
    EXPECT_EQ(net::ERR_CONNECTION_REFUSED, simple_loader2->NetError());
  } else {
    // Otherwise, the request should have succeeded, and return the same result
    // as before.
    ASSERT_TRUE(simple_loader2->ResponseInfo());
    ASSERT_TRUE(simple_loader2->ResponseInfo()->headers);
    EXPECT_FALSE(
        simple_loader->ResponseInfo()->headers->GetMaxAgeValue(&max_age));
