...
class ProfileNetworkContextService : public KeyedService {
 public:
  ...
  // NetworkService. This is called before SetUpProfileIODataMainContext.
 ...
 private:
  ...
  // NetworkContext. Initialized on construction. If the network service is
  // disabled, ownership is passed to StoragePartition when
  // CreateMainNetworkContext is called.  Otherwise, retains ownership, though
  // nothing uses it after construction.
  ...
  // is passed to ProfileIOData when ProfileNetworkContextService() is called.
