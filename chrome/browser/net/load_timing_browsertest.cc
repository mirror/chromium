...
IN_PROC_BROWSER_TEST_F(LoadTimingBrowserTest, Integration) {
  ...
  EXPECT_EQ(navigation_deltas.ssl_start, 0);
}

// Integration test with a real network response.
IN_PROC_BROWSER_TEST_F(LoadTimingBrowserTest, IntegrationSSL) {
  ...
  EXPECT_LT(navigation_deltas.dns_start, navigation_deltas.dns_end);
  EXPECT_LT(navigation_deltas.dns_end, navigation_deltas.connect_start);
  EXPECT_LT(navigation_deltas.connect_start, navigation_deltas.ssl_start);
  EXPECT_LT(navigation_deltas.ssl_start, navigation_deltas.connect_end);
  EXPECT_LT(navigation_deltas.connect_end, navigation_deltas.send_start);
  EXPECT_LT(navigation_deltas.send_start,
            navigation_deltas.receive_headers_end);
}
