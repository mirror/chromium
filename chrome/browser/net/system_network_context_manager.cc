...
content::mojom::NetworkContext* SystemNetworkContextManager::GetContext() {
  ...
  if (!network_service_network_context_) {
    ...
    network_service->CreateNetworkContext(
        ...
        CreateMainNetworkContextParams());
  }
  ...
  *is_quic_allowed = is_quic_allowed_;
  if (!base::FeatureList::IsEnabled(features::kNetworkService)) {
    ...
    *network_context_params = CreateMainNetworkContextParams();
    ...
    // Otherwise, this is just a vestigal NetworkContext, for legacy consumers.
    // Use default values.
...
SystemNetworkContextManager::CreateMainNetworkContextParams() {
