...
proxy_resolver::mojom::ProxyResolverFactoryPtr CreateResolverFactoryOnIOThread(
  ...
  return std::move(resolver_factory);
}
...
class DumbProxyResolverFactoryRequestClient
 ...
 public:
  ...
  ~DumbProxyResolverFactoryRequestClient() {}
...
// ProxyResolveFactory is destroyed.
IN_PROC_BROWSER_TEST_F(ChromeMojoProxyResolverFactoryBrowserTest,
                       FactoryDestroyed) {
  // Set up 1 ProxyResolverFactory.
  proxy_resolver::mojom::ProxyResolverFactoryPtr resolver_factory =
      CreateResolverFactoryOnIOThread(factory());
  ...
  // Destroying the factory, with no OnResolverDestroyed, as would happen in a
  // network service crash, should result in shutting down the proxy resolver
  // service.
...
IN_PROC_BROWSER_TEST_F(ChromeMojoProxyResolverFactoryBrowserTest,
  ...
  // Create a resolver again, this should create and start the service.
