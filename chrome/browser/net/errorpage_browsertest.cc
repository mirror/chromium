...
class ErrorPageTest : public InProcessBrowserTest {
 ...
 protected:
  ...
  void SetUpOnMainThread() override {
    ...
    GURL search_term_url(search_terms_data.GoogleBaseURLValue());

    if (base::FeatureList::IsEnabled(features::kNetworkService)) {
      url_loader_interceptor_ =
          std::make_unique<content::URLLoaderInterceptor>(base::BindRepeating(
              [](ErrorPageTest* owner, GURL search_url,
                 content::URLLoaderInterceptor::RequestParams* params) {
                ...
                // use.
                if (params->url_request.url == search_url) {
                  return WriteFileToURLLoader(params, "title3.html");
                }

                // Add an interceptor for anything else under mock.http
                if (params->url_request.url.GetWithEmptyPath().spec() ==
                    "http://mock.http/") {
                  return WriteFileToURLLoader(params,
                                              params->url_request.url.path());
                }

                return false;
              },
              this, search_term_url));
    } else {
      std::unique_ptr<net::URLRequestInterceptor> owned_interceptor(
          new LinkDoctorInterceptor(this));
      // Ownership of the |interceptor_| is passed to an object the IO thread,
      // but a pointer is kept in the test fixture.  As soon as anything calls
      // URLRequestFilter::ClearHandlers(), |interceptor_| can become invalid.
      BrowserThread::PostTask(
          BrowserThread::IO, FROM_HERE,
          base::BindOnce(&InstallMockInterceptors, search_term_url,
