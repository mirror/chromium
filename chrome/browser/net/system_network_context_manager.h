...
// Lives on the UI thread. This NetworkContext is intended for requests not
// associated with a profile. It stores no data on disk, and has no HTTP cache,
// but it does have ephemeral cookie and channel ID stores. It also does not
// have access to HTTP proxy auth information the user has entered or that comes
// from extensions, and similarly, has no extension-provided per-profile proxy
