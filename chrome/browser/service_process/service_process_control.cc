...
void ConnectAsync(mojo::ScopedMessagePipeHandle handle,
  ...
  if (os_pipe_handle.is_valid()) {
    peer_connection = base::MakeUnique<mojo::edk::PeerConnection>();
  ...
  callback_task_runner->PostTask(
      FROM_HERE, base::BindOnce(callback, std::move(peer_connection)));
}
...
void ServiceProcessControl::ConnectInternal() {
  ...
  base::PostTaskWithTraits(
      ...
      base::BindOnce(&ConnectAsync, std::move(pipe.handle1),
                     ...
                     base::BindOnce(&OnPeerConnectionComplete,
