...
FakeBluetoothChooserController::FakeDevice::FakeDevice(bool connected,
                                                       bool paired,
                                                       int signal_strength)
    : connected_(connected),
      paired_(paired),
      signal_strength_(signal_strength) {
  DCHECK_GE(signal_strength, -1);
  DCHECK_LE(signal_strength, 4);
}

FakeBluetoothChooserController::FakeBluetoothChooserController()
    : FakeBluetoothChooserController(std::vector<FakeDevice>()) {}
