...
class NavTree : public KeyedService {
 public:
  enum class WaypointType {
    kNone,  // Not a waypoint.
    kBack,  // The previous page.
    kSearch, // This page was a search results page.
    kSearchResult,  // A clicked search result.
    kHub,  // The page had multiple back navigations to it.
  };
  ...
  // Returns key waypoints for the given tab.
  std::vector<GURL> GetWaypoints(content::WebContents* web_contents) const;

  // Returns nullptr if no node exists for it.
  NavTreeNode* NodeForNavigationEntry(
 ...
 private:
  ...
  std::map<content::WebContents*, NavTreeNode*> heads_;
