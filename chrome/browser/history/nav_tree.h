...
class NavTree : public KeyedService {
 public:
  ...
  // Fills the two output parameters with the information about the current
  // contents. Returns true on success.
  bool NodesForTab(const content::WebContents* contents,
  ...
  // Observe all changes in the tree affecting a given tab.
  //
  // The pointer will not be owned and the observer must unregister itself when
  // it's destroyed.
  //
  // TODO(brettw) we'll have to do something for tabs that change WebContents.
  // Maybe we need some kind of separate "tab key" or possibly we need to
  // just keep track of the current one at all times.
  void AddTabObserver(const content::WebContents* current_contents,
                      NavTreeObserver* observer);
  void RemoveTabObserver(const content::WebContents* current_contents,
                         NavTreeObserver* observer);
 ...
 private:
  ...
  std::map<content::WebContents*, TabData> tabs_;
