...
void ProfileBasedHistoryHandler::OnRemoveVisits(
...
bool ProfileBasedHistoryHandler::AllowHistoryDeletions() {
  return GetProfile()->GetPrefs()->GetBoolean(
      prefs::kAllowDeletingBrowserHistory);
}

bool ProfileBasedHistoryHandler::ShouldHideWebHistoryUrl(const GURL& url) {
  return !CanAddURLToHistory(url);
}

history::WebHistoryService* ProfileBasedHistoryHandler::GetWebHistoryService() {
  return WebHistoryServiceFactory::GetForProfile(GetProfile());
}

void ProfileBasedHistoryHandler::
