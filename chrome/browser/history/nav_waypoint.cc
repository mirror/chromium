...
void NavWaypoint::ComputeMinMaxTime() {
  if (node_)
    min_time_ = max_time_ = node_->time();
  else
    min_time_ = max_time_ = base::Time();

  for (int i = 0; i < child_count(); i++) {
    NavWaypoint* child = GetChild(i);
    child->ComputeMinMaxTime();
    if (!child->min_time().is_null()) {
      min_time_ = std::min(min_time_, child->min_time());
      max_time_ = std::min(max_time_, child->max_time());
    }
  }
}
