...
void ShellExtensionEnumerator::EnumerateShellExtensionPaths(
  ...
  UMA_HISTOGRAM_COUNTS_100("ThirdPartyModules.ShellExtensionsCount",
                           nb_shell_extensions);
}
...
bool ShellExtensionEnumerator::GetModuleImageSizeAndTimeDateStamp(
  ...
  uint64_t nt_header_address = offsetof(IMAGE_DOS_HEADER, e_lfanew);

  LONG e_lfanew = 0;
  if (!GetValueAtOffset(buffer, nt_header_address, bytes_read, &e_lfanew))
    return false;

  // Check magic signature.
  uint64_t nt_signature_address =
      e_lfanew + offsetof(IMAGE_NT_HEADERS, Signature);

  DWORD nt_signature = 0;
  if (!GetValueAtOffset(buffer, nt_signature_address, bytes_read,
                        &nt_signature))
    return false;
  ...
  if (!GetValueAtOffset(buffer, size_of_image_address, bytes_read,
                        size_of_image))
    return false;
  ...
  return GetValueAtOffset(buffer, time_date_stamp_address, bytes_read,
                          time_date_stamp);
}
