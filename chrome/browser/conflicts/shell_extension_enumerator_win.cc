...
void ReadShellExtensions(HKEY parent,
  ...
  base::win::RegistryValueIterator registration(
...
void EnumerateShellExtensionsImpl(
    ...
    const base::Callback<void(const base::FilePath&, uint32_t, uint32_t, bool)>
        callback) {
  ...
  for (size_t i = 0; i < shell_extension_paths.size(); i++) {
    ...
    task_runner->PostTask(
        FROM_HERE,
        base::Bind(callback, shell_extension_paths[i], size_of_image,
                   time_date_stamp, i == shell_extension_paths.size() - 1u));
  }
}
...
void ShellExtensionEnumerator::Start() {
...
bool ShellExtensionEnumerator::IsDone() {
  return is_done_;
}
