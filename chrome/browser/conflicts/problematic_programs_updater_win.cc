...
ProblematicProgramsUpdater::ProblematicProgram::ProblematicProgram(
    ...
    : info(std::move(info)),
      ...
      blacklist_message_type(blacklist_message_type),
      message_url(std::move(message_url)) {}
...
base::Value ProblematicProgramsUpdater::GetCachedPrograms() {
  ...
  if (base::FeatureList::IsEnabled(kThirdPartyConflictsWarning)) {
    ...
    for (const auto& program : programs) {
      ...
      dict.SetKey("type", base::Value(program.blacklist_message_type));
      dict.SetKey("url", base::Value(program.message_url));
...
void ProblematicProgramsUpdater::OnNewModuleFound(
  ...
  // Also skip a module if it cannot be associated to an installed program
  ...
  bool allow_load = true;  // By default, all modules are allowed to load.
  chrome::conflicts::BlacklistMessageType message_type =
      chrome::conflicts::BlacklistMessageType::UNINSTALL;
  std::string message_url;
  if (module_list_filter_.IsBlacklisted(module_key, module_data, &allow_load,
                                        &message_type, &message_url)) {
    for (auto&& associated_program : associated_programs) {
      problematic_programs_.emplace_back(associated_program, allow_load,
                                         message_type, message_url);
    }
    return;
  }
  ...
  for (auto&& associated_program : associated_programs) {
    problematic_programs_.emplace_back(
        associated_program, true,
        chrome::conflicts::BlacklistMessageType::UNINSTALL, std::string());
  }
}
...
base::Value ProblematicProgramsUpdater::ConvertToDictionary(
  ...
  for (const ProblematicProgram& program : programs) {
    ...
    element.SetKey("allow_load", base::Value(program.allow_load));
    element.SetKey("type", base::Value(program.blacklist_message_type));
    element.SetKey("message_url", base::Value(program.message_url));
...
ProblematicProgramsUpdater::ConvertToProblematicProgramsVector(
  ...
  for (const auto& element : programs.DictItems()) {
    ...
    result.emplace_back(
        ...
        allow_load_value->GetBool(),
        static_cast<chrome::conflicts::BlacklistMessageType>(
            type_value->GetInt()),
        message_url_value->GetString());
  }
