...
void ProblematicProgramsUpdater::OnModuleDatabaseIdle() {
  ...
  base::Value dictionary = ConvertToDictionary(programs_);
  programs_.clear();

  if (before_first_idle_) {
    // Directly write the dictionary over the previous one.
    g_browser_process->local_state()->Set(prefs::kProblematicPrograms,
                                          dictionary);
    ...
    for (auto&& element : dictionary.DictItems())
      update.Get()->SetKey(std::move(element.first), std::move(element.second));
  }
}
