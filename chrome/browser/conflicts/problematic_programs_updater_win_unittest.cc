...
class ProblematicProgramsUpdaterTest : public testing::Test {
 public:
  static constexpr wchar_t kDllPath1[] = L"c:\\path\\to\\module.dll";
  static constexpr wchar_t kDllPath2[] = L"c:\\some\\shellextension.dll";
...
TEST_F(ProblematicProgramsUpdaterTest, EmptyCache) {
  ...
  EXPECT_TRUE(ProblematicProgramsUpdater::GetCachedProgramNames().empty());
}
