...
void CheckRegistryKeyForInstalledProgram(
  ...
  if (GetInstallPathUsingInstallLocation(candidate, &install_path)) {
    InstalledPrograms::ProgramInfo program_info = {
        std::move(display_name), hkey, std::move(candidate_key_path),
        wow64access,
    };

    programs_data->programs.push_back(std::move(program_info));
...
std::unique_ptr<InstalledPrograms::ProgramsData> GetProgramsData(
  ...
  std::pair<HKEY, REGSAM> kCombinations[] = {
