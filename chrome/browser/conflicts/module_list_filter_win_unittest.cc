...
class ModuleListBuilder {
 public:
  ...
  bool Finalize() {
    std::string contents;
    if (!module_list_.SerializeToString(&contents))
      return false;

    return WriteStringToFile(module_list_path_, contents);
  }
...
class ModuleListFilterTest : public ::testing::Test {
 public:
  ...
  static constexpr wchar_t kDllPath2[] = L"c:\\some\\dll.dll";

  ModuleListFilterTest() : dll1_(kDllPath1), dll2_(kDllPath2) {}

  ~ModuleListFilterTest() = default;
...
TEST_F(ModuleListFilterTest, BlacklistedModules) {
  const char kFurtherInfoURL[] = "asdafasd";
  ...
  ASSERT_TRUE(module_list_filter().IsBlacklisted(module_1.first,
                                                 module_1.second, &allow_load,
                                                 &message_type, &message_url));
  EXPECT_TRUE(allow_load);
  EXPECT_EQ(chrome::conflicts::BlacklistMessageType::UNINSTALL, message_type);
  EXPECT_EQ(kFurtherInfoURL, message_url);
  EXPECT_FALSE(module_list_filter().IsBlacklisted(module_2.first,
                                                  module_2.second, &allow_load,
                                                  &message_type, &message_url));
}
