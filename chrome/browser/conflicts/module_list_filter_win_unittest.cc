...
TEST_F(ModuleListFilterTest, BlacklistedModules) {
  ...
  chrome::conflicts::BlacklistAction blacklist_action;
  ASSERT_TRUE(module_list_filter().IsBlacklisted(
      module_1.first, module_1.second, &blacklist_action));
  ...
  EXPECT_FALSE(module_list_filter().IsBlacklisted(
      module_2.first, module_2.second, &blacklist_action));
}
