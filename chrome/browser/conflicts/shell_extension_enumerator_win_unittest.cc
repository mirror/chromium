...
TEST_F(ShellExtensionEnumeratorTest, EnumerateShellExtensions) {
  ...
  EXPECT_FALSE(shell_extension_enumerator.IsDone());
  EXPECT_EQ(0, shell_extension_count());

  shell_extension_enumerator.Start();
  base::RunLoop().RunUntilIdle();

  EXPECT_TRUE(shell_extension_enumerator.IsDone());
