...
// Get a truncated copy of shell32.dll to mimic a file with an invalid NT
// header.
bool CreateTruncatedShell32(const base::FilePath& location) {
  const wchar_t kShell32DllPath[] = L"%SYSTEMROOT%\\System32\\shell32.dll";

  const size_t kBufferSize = 128;
  base::string16 expanded_path;
  DWORD nb_bytes = ::ExpandEnvironmentStrings(
      kShell32DllPath, base::WriteInto(&expanded_path, kBufferSize),
      kBufferSize);
  if (nb_bytes > kBufferSize)
    return false;

  base::FilePath shell32_path(expanded_path);
  if (!base::CopyFile(shell32_path, location))
    return false;

  base::ScopedFILE file(base::OpenFile(location, "a"));

  const size_t kSizeOfTruncatedDll = 256;
  fseek(file.get(), kSizeOfTruncatedDll, SEEK_SET);

  return base::TruncateFile(file.get());
}
...
TEST_F(ShellExtensionEnumeratorTest, GetModuleImageSizeAndTimeDateStamp) {
  ...
  EXPECT_TRUE(ShellExtensionEnumerator::GetModuleImageSizeAndTimeDateStamp(
...
TEST_F(ShellExtensionEnumeratorTest, NonexistentDll) {
  ...
  uint32_t size_of_image = 0;
  uint32_t time_date_stamp = 0;
  EXPECT_FALSE(ShellExtensionEnumerator::GetModuleImageSizeAndTimeDateStamp(
...
TEST_F(ShellExtensionEnumeratorTest, InvalidNTHeader) {
  ...
  ASSERT_TRUE(CreateTruncatedShell32(invalid_dll));
