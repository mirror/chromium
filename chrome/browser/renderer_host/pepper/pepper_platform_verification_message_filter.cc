...
std::vector<uint8_t> GetSalt(content::RenderFrameHost* rfh) {
  ...
  // Unable to get salt, return empty string as default.
  DLOG(ERROR) << "Unable to get salt from profile.";
...
int32_t PepperPlatformVerificationMessageFilter::OnGetStorageId(
  ...
  if (!salt.size()) {
    // Problems getting the salt, so return empty buffer.
    GetStorageIdCallback(context->MakeReplyMessageContext(), salt);
    return PP_OK_COMPLETIONPENDING;
  }
