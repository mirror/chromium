...
DevToolsFileSystemIndexer::FileSystemIndexingJob::FileSystemIndexingJob(
    ...
    : file_system_path_(std::move(file_system_path)),
      total_work_callback_(std::move(total_work_callback)),
      worked_callback_(std::move(worked_callback)),
...
void DevToolsFileSystemIndexer::SearchInPath(string file_system_path,
                                             string query,
  ...
  impl_task_runner()->PostTask(
      ...
      BindOnce(&DevToolsFileSystemIndexer::SearchInPathOnImplSequence, this,
               std::move(file_system_path), std::move(query),
               std::move(callback)));
}
...
void DevToolsFileSystemIndexer::SearchInPathOnImplSequence(
  ...
  BrowserThread::PostTask(BrowserThread::UI, FROM_HERE,
                          BindOnce(std::move(callback), std::move(result)));
}
