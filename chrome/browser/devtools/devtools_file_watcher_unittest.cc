 ...
 public:
  void Callback(const std::vector<std::string>& changed_paths,
    ...
    for (auto& p : changed_paths)
      ASSERT_EQ(1ul, expected_changed_paths_.erase(p));
...
TEST_F(DevToolsFileWatcherTest, BasicUsage) {
  ...
  expected_changed_paths_.insert(changed_path.value());
  ...
  expected_added_paths_.insert(added_path.value());
  ...
  expected_removed_paths_.insert(added_path.value());
