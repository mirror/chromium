...
class TestConfirmInfoBarDelegate : public ConfirmInfoBarDelegate {
 public:
  ...
  base::string16 GetMessageText() const override {
    return base::string16(L"GlobalConfirmInfoBar browser tests delegate.");
  }
...
// Subclass for tests that requires disabled infobars.
...
IN_PROC_BROWSER_TEST_F(GlobalConfirmInfoBarTest, UserInteraction) {
  ...
  // Close the GlobalConfirmInfoBar by simulating an interacting with the info
  // bar on one of the tabs. In this case, the first tab is picked.
  InfoBarService* first_tab_infobar_service = GetInfoBarServiceFromTabIndex(0);
  infobars::InfoBar* first_tab_infobar =
      first_tab_infobar_service->infobar_at(0);
  EXPECT_TRUE(
  ...
  first_tab_infobar_service->RemoveInfoBar(first_tab_infobar);
