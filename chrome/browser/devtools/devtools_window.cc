...
void DevToolsWindow::OpenExternalFrontend(
  ...
  if (type == "node") {
    ...
    window = Create(profile, nullptr, kFrontendV8,
                    std::string(), false, std::string(), std::string(),
                    agent_host->IsAttached());
  } else {
    bool is_worker = type == DevToolsAgentHost::kTypeServiceWorker ||
                     type == DevToolsAgentHost::kTypeSharedWorker;

    FrontendType frontend_type = is_worker ? kFrontendRemoteWorker : kFrontendRemote;
...
GURL DevToolsWindow::GetDevToolsURL(Profile* profile,
                                    ...
                                    bool has_other_clients) {
  std::string url;
  std::string remote_base = "?remoteBase=" + DevToolsUI::GetRemoteBaseURL().spec();
  std::string remote_frontend = frontend_url + ((frontend_url.find("?") == std::string::npos) ? "?remoteFrontend=true" : "&remoteFrontend=true");
  switch (frontend_type) {
    case kFrontendDefault:
      url = kChromeUIDevToolsURL + remote_base;
      if (can_dock)
        url += "&can_dock=true";
      if (panel.size())
        url += "&panel=" + panel;
