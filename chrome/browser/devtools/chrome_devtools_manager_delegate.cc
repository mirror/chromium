...
base::DictionaryValue* ChromeDevToolsManagerDelegate::HandleCommand(
  ...
  if (web_contents &&
      page_handlers_.find(web_contents) == page_handlers_.end()) {
    page_handlers_[web_contents] =
        std::make_unique<PageHandler>(web_contents, dispatcher_.get());
  }
  auto response =
      dispatcher_->dispatch(protocol::toProtocolValue(command_dict, 1000));
  if (response == protocol::DispatchResponse::Status::kFallThrough)
    return nullptr;
  if (response == protocol::DispatchResponse::Status::kError)
    return DevToolsProtocol::CreateErrorResponse(id, std::string()).release();
...
void ChromeDevToolsManagerDelegate::DevToolsAgentHostDetached(
    content::DevToolsAgentHost* agent_host) {
  auto handler = page_handlers_.find(agent_host->GetWebContents());
  if (handler != page_handlers_.end()) {
    handler->second->ToggleAdBlocking(false /* enabled */);
    page_handlers_.erase(handler);
  }
