...
void Ui::SetWebVrMode(bool enabled, bool show_toast) {
  model_->web_vr_model.show_webvr_exit_toast = show_toast;
  if (enabled) {
    model_->web_vr_model.state = kWebVrAwaitingFirstFrame;
  } else {
    model_->web_vr_model.state = kWebVrNoTimeoutPending;
    // This gets set to true when the UI is initialized.
    model_->web_vr_model.started_for_autopresentation = false;
  }
}
...
bool Ui::ShouldRenderWebVr() {
  return model_->web_vr_model.has_produced_frames();
}
...
void Ui::OnAppButtonClicked() {
  ...
  if (model_->web_vr_model.started_for_autopresentation) {
    return;
  }
...
void Ui::OnWebVrFrameAvailable() {
  if (model_->web_vr_model.web_vr_mode())
    model_->web_vr_model.state = kWebVrPresenting;
}
...
void Ui::InitializeModel(const UiInitialState& ui_initial_state) {
  model_->web_vr_model.started_for_autopresentation =
  ...
  model_->web_vr_model.state = ui_initial_state.in_web_vr
