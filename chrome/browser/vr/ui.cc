...
void Ui::SetExitVrPromptEnabled(bool enabled, UiUnsupportedMode reason) {
  ...
  if (model_->active_modal_prompt != kModalPromptNone) {
    browser_->OnExitVrPromptResult(
        GetReasonForPrompt(model_->active_modal_prompt),
  ...
  switch (reason) {
    ...
    case UiUnsupportedMode::kAndroidPermissionNeeded:
      model_->active_modal_prompt = kModalPromptExitVRForAudioPermission;
      break;
    default:
      NOTREACHED();
  }
