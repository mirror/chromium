...
void Ui::SetWebVrMode(bool enabled, bool show_toast) {
  ...
  if (enabled) {
    ...
    if (!model_->web_vr_enabled())
      model_->set_mode(kModeWebVr);
  } else {
    model_->web_vr.state = kWebVrNoTimeoutPending;
    model_->clear_mode(kModeWebVr);
    model_->clear_mode(kModeWebVrAutopresented);
  }
}

void Ui::SetFullscreen(bool enabled) {
  if (enabled) {
    model_->set_mode(kModeFullscreen);
  } else {
    model_->clear_mode(kModeFullscreen);
  }
}
...
void Ui::OnNavigated() {
  model_->clear_mode(kModeEditingOmnibox);
}

void Ui::SetOmniboxEditingEnabled(bool enabled) {
  if (enabled) {
    model_->set_mode(kModeEditingOmnibox);
  } else {
    model_->clear_mode(kModeEditingOmnibox);
  }
}

void Ui::SetVoiceSearchEnabled(bool enabled) {
  if (enabled) {
    model_->set_mode(kModeVoiceSearch);
  } else {
    model_->clear_mode(kModeVoiceSearch);
  }
}
