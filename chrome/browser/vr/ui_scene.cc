...
template <typename P>
std::vector<const UiElement*> GetRenderableElements(UiElement* e, P predicate) {
  std::vector<const UiElement*> elements;
  ForAllElements(e, [&elements, predicate](UiElement* element) {
    if (element->IsVisible() && element->WillRender() && predicate(element)) {
...
std::vector<const UiElement*> UiScene::Get2dBrowsingElements() const {
...
std::vector<const UiElement*> UiScene::Get2dBrowsingOverlayElements() const {
  return GetRenderableElements(
      GetUiElementByName(k2dBrowsingRoot), [](UiElement* element) {
...
std::vector<const UiElement*> UiScene::GetWebVrOverlayBackgroundElements()
    const {
  return GetRenderableElements(
...
std::vector<const UiElement*> UiScene::GetWebVrOverlayForegroundElements()
    const {
  return GetRenderableElements(
      GetUiElementByName(kWebVrRoot), [](UiElement* element) {
