  ...
  for (auto& element : *root) {
    if (element.IsVisible() && predicate(&element)) {
      ...
      if (target && target->id() == element.id()) {
        ...
        // of composite UI elements. By walking the element's subtree in reverse
        // pre-order, we start at the last-rendered child and work backwards,
        // possibly as far as the originally targeted element.
