...
void NotifySpeechRecognizerOnIOCreated(
    base::WeakPtr<IOBrowserUIInterface> browser_ui,
    SpeechRecognizerOnIO* ptr) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  if (browser_ui.get()) {
    browser_ui.get()->OnSpeechRecognizerOnIOCreated(ptr);
  } else {
...
void CreateSpeechRecognizerOnIO(
    base::WeakPtr<IOBrowserUIInterface> browser_ui) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::IO);

  // Use raw pointer here. This object will be owned by SpeechRecognition.
  SpeechRecognizerOnIO* speech_recognizer_on_io =
      new SpeechRecognizerOnIO(browser_ui);
  content::BrowserThread::PostTask(
      content::BrowserThread::UI, FROM_HERE,
      base::Bind(&NotifySpeechRecognizerOnIOCreated, browser_ui,
                 speech_recognizer_on_io));
}
...
SpeechRecognizer::SpeechRecognizer(
    ...
    : delegate_(delegate),
      ...
      weak_factory_(this) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  // Try to create SpeechRecognizerOnIO object on IO thread.
  // OnSpeechRecognizerOnIOCreated will be call once success. All calls should
  // be blocked on SpeechRecognizerOnIO.
  content::BrowserThread::PostTask(
      content::BrowserThread::IO, FROM_HERE,
      base::BindOnce(&CreateSpeechRecognizerOnIO, weak_factory_.GetWeakPtr()));
}
...
void SpeechRecognizer::OnSpeechRecognizerOnIOCreated(
    SpeechRecognizerOnIO* ptr) {
  DCHECK(ptr);
  speech_recognizer_on_io_.reset(ptr);
  if (retry_stop_when_ready_) {
    Stop();
  } else if (retry_start_when_ready_) {
    Start();
  }
  retry_start_when_ready_ = false;
  retry_stop_when_ready_ = false;
}
