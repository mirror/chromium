...
class SpeechRecognizerOnIO : public content::SpeechRecognitionEventListener {
 public:
  SpeechRecognizerOnIO(const base::WeakPtr<IOBrowserUIInterface>& browser_ui);
...
static SpeechRecognizerOnIO* g_speech_event_listener = nullptr;

void StartOnIOThread(
    scoped_refptr<net::URLRequestContextGetter> url_request_context_getter,
    base::WeakPtr<IOBrowserUIInterface> browser_ui,
    const std::string& locale,
    const std::string& auth_scope, const std::string& auth_token) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::IO);
  // Delete old session if any.
  delete g_speech_event_listener;

  g_speech_event_listener = new SpeechRecognizerOnIO(browser_ui);
  g_speech_event_listener->Start(url_request_context_getter, locale, auth_scope, auth_token);
}
...
SpeechRecognizerOnIO::SpeechRecognizerOnIO(
    ...
    : browser_ui_(browser_ui),
      ...
      weak_factory_(this) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::IO);
  DCHECK(!g_speech_event_listener);
  g_speech_event_listener = this;
}
...
SpeechRecognizerOnIO::~SpeechRecognizerOnIO() {
  ...
  content::SpeechRecognitionManager::GetInstance()->StopAudioCaptureForSession(
      session_);
  g_speech_event_listener = nullptr;
}
...
void SpeechRecognizerOnIO::NotifyRecognitionStateChanged(
  ...
  LOG(ERROR) << "====bshe====voice recognition state===" << new_state;
...
void SpeechRecognizerOnIO::StartSpeechTimeout(int timeout_seconds) {
  ...
  speech_timeout_.Start(
      ...
      base::Bind(&SpeechRecognizerOnIO::SpeechTimeout, weak_factory_.GetWeakPtr()));
}

void SpeechRecognizerOnIO::StopSpeechTimeout() {
  DCHECK_CURRENTLY_ON(content::BrowserThread::IO);
...
void SpeechRecognizerOnIO::OnRecognitionResults(
  ...
  LOG(ERROR) << "====bshe====" << result_str;
  content::BrowserThread::PostTask(
      content::BrowserThread::UI, FROM_HERE,
      base::Bind(&IOBrowserUIInterface::OnSpeechResult, browser_ui_,
...
void SpeechRecognizerOnIO::OnAudioLevelsChange(int session_id,
  ...
  content::BrowserThread::PostTask(
      content::BrowserThread::UI, FROM_HERE,
      base::Bind(&IOBrowserUIInterface::OnSpeechSoundLevelChanged,
                 browser_ui_, sound_level));
}

void SpeechRecognizerOnIO::OnEnvironmentEstimationComplete(
    int session_id) {}
...
void SpeechRecognizer::OnSpeechResult(const base::string16& query, bool is_final) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  if (!is_final)
    return;
...
void SpeechRecognizer::OnSpeechSoundLevelChanged(int16_t level) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  // TODO(bshe): notify VR UI thread to draw UI.
};
