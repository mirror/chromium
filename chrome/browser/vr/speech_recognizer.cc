...
// It is created after calling SpeechRecognizer::Start. And deleted after
// SpeechRecognizer is deleted on UI thread (see destructor of
// SpeechRecognizer).
// This class doesn't assume SpeechRecognizer on Browser UI thread lives
// longer. So it is also safe to delete SpeechRecognizer at any time on UI
// thread.
...
void StopOnIOThread() {
  ...
  if (!g_speech_recognizer_on_io)
    return;
  g_speech_recognizer_on_io->Stop();
}
