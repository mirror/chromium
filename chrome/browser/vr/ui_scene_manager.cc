...
void UiSceneManager::CreateTabMenu() {
  ...
  view->SetSize(kContentWidth, kContentHeight * 0.85);

  view->animation_player().transition().target_properties = {SCROLL_OFFSET};
  view->animation_player().transition().duration =
      base::TimeDelta::FromMilliseconds(300);
  view->animation_player().transition().ease_type =
