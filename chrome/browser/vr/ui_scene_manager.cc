...
void UiSceneManager::CreateController(Model* model) {
  ...
  controller->set_name(kController);
  controller->SetVisible(true);
  controller->set_hit_testable(false);
  ...
  auto laser = base::MakeUnique<Laser>();
  laser->set_draw_phase(kPhaseForeground);
  laser->set_name(kLaser);
  laser->SetVisible(true);
  laser->set_hit_testable(false);
  laser->AddBinding(VR_BIND_FUNC(float, Model, model, controller.opacity, Laser,
                                 laser.get(), SetOpacity));
  laser->AddBinding(VR_BIND_FUNC(gfx::Point3F, Model, model,
                                 controller.laser_origin, Laser, laser.get(),
                                 set_origin));
  laser->AddBinding(VR_BIND_FUNC(gfx::Point3F, Model, model,
                                 reticle.target_point, Laser, laser.get(),
                                 set_target));
  ...
  reticle->set_name(kReticle);
  reticle->SetVisible(true);
  reticle->set_hit_testable(false);
