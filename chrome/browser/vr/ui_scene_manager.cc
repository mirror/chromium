...
static constexpr SkColor kTabColors[] = {
    SK_ColorBLUE,    SK_ColorBLACK, SK_ColorGREEN,  SK_ColorLTGRAY,
    SK_ColorMAGENTA, SK_ColorWHITE, SK_ColorRED,    SK_ColorGRAY,
    SK_ColorYELLOW,  SK_ColorCYAN,  SK_ColorDKGRAY, SK_ColorBLUE,
    SK_ColorBLACK,   SK_ColorGREEN, SK_ColorLTGRAY, SK_ColorMAGENTA,
...
void UiSceneManager::CreateLocationBar() {
  auto background_element = base::MakeUnique<Bar>();
  background_element->set_id(AllocateId());
  // background_element->SetSize(0.2f, 0.05f);
  // background_element->SetTranslate(0.0, 0.0, -1.0f);
  ...
  background_element->SetColor(ColorScheme::GetColorScheme(ColorScheme::Mode::kModeNormal)
                        .location_bar_background);

  auto thumb_element = base::MakeUnique<Bar>();
  thumb_element->set_id(AllocateId());
  thumb_element->SetSize(0.03f, 0.05f);
  // thumb_element->SetTranslate(0.0,0.0, -1.0f);
  thumb_element->SetVisible(true);
  thumb_element->set_hit_testable(true);
  thumb_element->set_draw_phase(kPhaseForeground);
  thumb_element->SetColor(ColorScheme::GetColorScheme(ColorScheme::Mode::kModeNormal)
                        .location_bar_thumb);

  auto location_bar_element = base::MakeUnique<LocationBar>(background_element.get(),
    thumb_element.get());
  location_bar_element->set_id(AllocateId());
  location_bar_element->SetSize(0.2f, 0.05f);
  location_bar_element->SetTranslate(0.0, 0.0, -1.0f);
  ...
  location_bar_element->SetRange(5);
...
void UiSceneManager::CreateTabMenu() {
  auto element = base::MakeUnique<LinearLayout>(LinearLayout::Direction::kVertical);
  element->set_id(AllocateId());
  element->SetTranslate(0, kContentVerticalOffset, -kContentDistance + 0.01f);
  element->set_margin(0.1f);
  // element->AddChild(location_bar_);
  element->AddChild(page_grid_view_);
  scene_->AddUiElement(std::move(element));
}
...
void UiSceneManager::ConfigureScene() {
  ...
  for (UiElement* element : content_elements_) {
    element->SetEnabled(false);  // browsing_mode && !prompting_to_exit_);
  }
...
void UiSceneManager::OnAppButtonClicked() {
  ...
  // location_bar_->SetPosition((location_bar_->GetPosition() + 1) % location_bar_->GetRange());
}
