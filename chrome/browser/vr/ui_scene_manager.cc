...
static constexpr float kSplashScreenTextFontHeightDMM = 0.05;
static constexpr float kSplashScreenTextWidthDMM = 0.9;
static constexpr float kSplashScreenTextHeightDMM = 0.08;
...
void UiSceneManager::CreateSplashScreen() {
  ...
  ///////////////Could use ConvertJavaStringToUTF16 if we want.
  ////////////Do not submit.
  std::unique_ptr<Text> text = base::MakeUnique<Text>(
      256, kSplashScreenTextFontHeightDMM, kSplashScreenTextWidthDMM,
      &ColorScheme::splash_screen_text, IDS_VR_POWERED_BY_CHROME_MESSAGE);
  text->set_debug_id(kSplashScreenText);
  text->set_id(AllocateId());
  text->set_draw_phase(kPhaseForeground);
  text->set_hit_testable(false);
  text->SetSize(kSplashScreenTextWidthDMM, kSplashScreenTextHeightDMM);
  text->SetScale(kSplashScreenTextDistance, kSplashScreenTextDistance,
...
void UiSceneManager::CreateUnderDevelopmentNotice() {
  std::unique_ptr<Text> text = base::MakeUnique<Text>(
      ...
      &ColorScheme::world_background_text, IDS_VR_UNDER_DEVELOPMENT_NOTICE);
