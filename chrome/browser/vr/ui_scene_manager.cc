...
void UiSceneManager::OnProjMatrixChanged(const gfx::Transform& proj_matrix) {
  ...
  // given threshold. If so, propage this info so that the content's resolution
  // and size can be adjusted.
  // For the calculation we cannot take the content quad's actual size
  // (main_content_->size()) since this property is animated. If we took the
  // actual size we would surpass the threshold with differing projected sizes
  // and aspect ratios (depending on the animation's timing). The differing
  // values may cause visual artefacts if, for instance, the fullscreen aspect
  // ratio is not 16:9. As a workaround take the final size of the content quad
  // after the animation as the basis for the calculation.
