...
void UiSceneManager::CreateSplashScreen(Model* model) {
  ...
  spinner->SetSize(kCloseButtonWidth, kCloseButtonHeight);
  spinner->SetTranslate(0, kSplashScreenTextVerticalOffset,
                        -kSplashScreenTextDistance);
  spinner->AddBinding(VR_BIND_FUNC(bool, Model, model,
                                   web_vr_timeout_state == kWebVrTimeoutPending,
  ...
  spinner_bg->SetCenterColor(SK_ColorBLACK);
  spinner_bg->SetEdgeColor(SK_ColorBLACK);
  spinner_bg->AddBinding(VR_BIND_FUNC(
      bool, Model, model, web_vr_timeout_state != kWebVrNoTimeout,
  ...
  // TODO(vollick): permit size-to-fit and padding.
  timeout_message->SetSize(kUnderDevelopmentNoticeWidthM * 1.2,
                           kUnderDevelopmentNoticeHeightM);
  timeout_message->SetTranslate(0, kSplashScreenTextVerticalOffset,
                                -kSplashScreenTextDistance + 0.001);
  timeout_message->set_corner_radius(kContentCornerRadius * 1.5);
  ...
  BindColor(this, timeout_message.get(),
            &ColorScheme::permanent_warning_foreground);
  scene_->AddUiElement(kSplashScreenViewportAwareRoot,
  ...
  timeout_layout->set_margin(0.001);
  timeout_layout->SetVisible(true);
  timeout_layout->SetTranslate(0.2 * kCloseButtonWidth, 0, 0.01);
  ...
  timeout_icon->SetSize(kCloseButtonWidth * 0.5, kCloseButtonHeight * 0.5);
  timeout_icon->SetTranslate(0, 0, 0.001);
  ...
  timeout_text->SetSize(kUnderDevelopmentNoticeWidthM,
                        kUnderDevelopmentNoticeHeightM);
  timeout_text->SetTranslate(0, 0, 0.001);
  scene_->AddUiElement(kWebVrTimeoutMessageLayout, std::move(timeout_text));

  auto timeout_button = base::MakeUnique<UiElement>();
  timeout_button->set_name(kWebVrTimeoutMessageButton);
  timeout_button->SetVisible(true);
  timeout_button->SetTranslate(0, kUrlBarVerticalOffset, 0);
  scene_->AddUiElement(kWebVrTimeoutMessage, std::move(timeout_button));

  auto timeout_button_bg = base::MakeUnique<Rect>();
  timeout_button_bg->set_name(kWebVrTimeoutMessageButtonBackground);
  timeout_button_bg->set_draw_phase(kPhaseOverlayForeground);
  timeout_button_bg->SetVisible(true);
  timeout_button_bg->SetSize(kCloseButtonWidth, kCloseButtonWidth);
  timeout_button_bg->set_corner_radius(kCloseButtonWidth * 0.5);
  timeout_button_bg->SetTransitionedProperties(
      {FOREGROUND_COLOR, BACKGROUND_COLOR, TRANSFORM});
  timeout_button_bg->set_click_handler(
      base::Bind([](UiSceneManager* mgr) { mgr->OnWebVrTimedOut(); }, this));
  timeout_button_bg->AddBinding(base::MakeUnique<Binding<SkColor>>(
      base::Bind(
          [](UiSceneManager* mgr, Rect* r) {
            if (r->pressed())
              return mgr->color_scheme().close_button_background_down;
            if (r->hovered())
              return mgr->color_scheme().close_button_background_hover;
            return mgr->color_scheme().close_button_background;
          },
          base::Unretained(this), timeout_button_bg.get()),
      base::Bind(
          [](Rect* r, const SkColor& c) {
            r->SetCenterColor(c);
            r->SetEdgeColor(c);
          },
          timeout_button_bg.get())));
  timeout_button_bg->AddBinding(base::MakeUnique<Binding<bool>>(
      base::Bind([](Rect* r) { return r->hovered(); }, timeout_button_bg.get()),
      base::Bind(
          [](Rect* r, const bool& b) { r->SetTranslate(0, 0, b ? 0.0125 : 0); },
          timeout_button_bg.get())));

  auto timeout_button_icon =
      base::MakeUnique<VectorIcon>(512, vector_icons::kClose16Icon);
  timeout_button_icon->set_name(kWebVrTimeoutMessageButtonForeground);
  timeout_button_icon->set_draw_phase(kPhaseOverlayForeground);
  timeout_button_icon->SetVisible(true);
  timeout_button_icon->SetSize(kCloseButtonWidth * 0.5,
                               kCloseButtonHeight * 0.5);
  timeout_button_icon->SetTransitionedProperties({TRANSFORM});
  timeout_button_icon->AddBinding(base::MakeUnique<Binding<SkColor>>(
      base::Bind(
          [](UiSceneManager* mgr) {
            return mgr->color_scheme().close_button_foreground;
          },
          this),
      base::Bind([](VectorIcon* i, const SkColor& c) { i->SetColor(c); },
                 timeout_button_icon.get())));
  timeout_button_icon->AddBinding(base::MakeUnique<Binding<bool>>(
      base::Bind([](Rect* r) { return r->hovered(); }, timeout_button_bg.get()),
      base::Bind([](VectorIcon* i,
                    const bool& b) { i->SetTranslate(0, 0, b ? 0.025 : 0); },
                 timeout_button_icon.get())));

  scene_->AddUiElement(kWebVrTimeoutMessageButton,
                       std::move(timeout_button_bg));

  scene_->AddUiElement(kWebVrTimeoutMessageButton,
                       std::move(timeout_button_icon));
}
