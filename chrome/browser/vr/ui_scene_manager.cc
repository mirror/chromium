...
void UiSceneManager::Create2dBrowsingSubtreeRoots(Model* model) {
  ...
  element->AddBinding(base::MakeUnique<Binding<bool>>(
      base::Bind([](Model* m) { return m->recognizing_speech; },
                 base::Unretained(model)),
      base::Bind(
          [](UiElement* element, const bool& value) {
            // Hide itself if speech recognition UI is showing.
...
void UiSceneManager::CreateVoiceSearchUiGroup(Model* model) {
  ...
  auto recognizing_speech_prompt =
      base::MakeUnique<SpeechRecognitionPrompt>(256);
  speech_recognition_prompt_ = recognizing_speech_prompt.get();
  speech_recognition_prompt_->AddBinding(base::MakeUnique<Binding<int>>(
      base::Bind([](Model* m) { return m->speech_recognition_state; },
                 base::Unretained(model)),
      base::Bind(
          [](SpeechRecognitionPrompt* prompt, const int& value) {
            prompt->OnSpeechRecognitionStateChanged(value);
          },
          base::Unretained(speech_recognition_prompt_))));
  speech_recognition_prompt_->AddBinding(
      VR_BIND_FUNC(bool, Model, model, recognizing_speech, UiElement,
                   speech_recognition_prompt_, SetVisible));
  element = std::move(recognizing_speech_prompt);
  element->set_name(kSpeechRecognitionPrompt);
  element->set_draw_phase(kPhaseForeground);
  element->SetSize(kCloseButtonWidth * 2, kCloseButtonHeight * 2);
