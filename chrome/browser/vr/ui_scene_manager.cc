...
void UiSceneManager::CreateBackground() {
  ...
  for (auto& panel : panels) {
    std::unique_ptr<RRect> panel_element = base::MakeUnique<RRect>();
  ...
  // Floor.
  std::unique_ptr<Grid> floor = base::MakeUnique<Grid>();
  ...
  // Ceiling.
  std::unique_ptr<RRect> ceiling = base::MakeUnique<RRect>();
...
void UiSceneManager::ConfigureBackgroundColor() {
  ...
  for (RRect* panel : background_panels_) {
