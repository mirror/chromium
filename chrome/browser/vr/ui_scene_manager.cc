...
void UiSceneManager::Create2dBrowsingSubtreeRoots(Model* model) {
  ...
  element->AddBinding(base::MakeUnique<Binding<float>>(
      base::Bind(
          [](UiSceneManager* mgr, Model* m) {
            if (!mgr->browsing_mode() ||
                m->active_modal_prompt == kModalPromptExitVRForSiteInfo) {
              return 0.0f;
            } else if (m->active_modal_prompt ==
...
void UiSceneManager::CreateUrlBar(Model* model) {
  ...
  url_bar->AddBinding(VR_BIND_FUNC(bool, UiSceneManager, this,
                                   browsing_mode() && !model->fullscreen(),
...
void UiSceneManager::CreateExitPrompt(Model* model) {
  ...
  event_handlers.button_up = base::Bind(
      [](UiSceneManager* manager, Model* m) {
        manager->OnExitPromptBackplaneClicked(
            GetReasonForPrompt(m->active_modal_prompt));
      },
      base::Unretained(this), base::Unretained(model));
  backplane->set_event_handlers(event_handlers);
  backplane->AddBinding(VR_BIND_FUNC(
      bool, Model, model, active_modal_prompt == kModalPromptExitVRForSiteInfo,
  ...
  exit_prompt->AddBinding(base::MakeUnique<Binding<ModalPrompt>>(
      base::Bind([](Model* m) { return m->active_modal_prompt; },
                 base::Unretained(model)),
      base::Bind(
          [](ExitPrompt* e, const ModalPrompt& p) {
...
void UiSceneManager::CreateAudioPermissionPrompt(Model* model) {
  ...
  prompt->SetTransitionedProperties({OPACITY});
  prompt->SetTranslate(0.0, kContentVerticalOffset + kExitPromptVerticalOffset,
                       kTextureOffset - kContentDistance);
  prompt->AddBinding(
      VR_BIND_FUNC(bool, Model, model,
                   active_modal_prompt == kModalPromptExitVRForAudioPermission,
                   UiElement, prompt.get(), SetVisible));
  scene_->AddUiElement(k2dBrowsingRoot, std::move(prompt));
}
