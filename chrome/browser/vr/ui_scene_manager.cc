...
void UiSceneManager::CreateAlertDialog() {
  ...
  dialog->SetRotate(1, 0, 0, 0);  // kUnderDevelopmentNoticeRotationRad);
                                  //  dialog->SetEnabled(true);
  dialog->SetVisible(false);
  dialog->set_y_anchoring(YAnchoring::YTOP);
  //  main_content_->AddChild(dialog.get());
...
void UiSceneManager::SetLocationAccessIndicator(bool enabled) {
  LOG(ERROR) << "VRP: ==> Location access indicator: "
             << (enabled ? "true" : "false");
...
void UiSceneManager::ConfigureAlertDialog() {
  LOG(ERROR) << "VR: ConfigureAlertDialo";
