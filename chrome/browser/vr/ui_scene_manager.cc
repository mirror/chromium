...
#define BIND_COLOR(model, rect, color)                                      \
  rect->AddBinding(base::MakeUnique<Binding<SkColor>>(                      \
      base::Bind([](UiSceneManager* m) { return m->color_scheme().color; }, \
...
void UiSceneManager::CreateUrlBar(Model* model) {
  ...
  BIND_COLOR(this, indicator_bg.get(), loading_indicator_background);
  ...
  BIND_COLOR(this, indicator_fg.get(), loading_indicator_foreground);
