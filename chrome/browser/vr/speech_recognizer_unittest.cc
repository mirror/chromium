...
class SpeechRecognizerTest : public testing::Test {
 public:
  SpeechRecognizerTest()
      : fake_speech_recognition_manager_(new FakeSpeechRecognitionManager()),
        browser_(new MockBrowserUiInterface),
        delegate_(new MockVoiceSearchDelegate),
        speech_recognizer_(new SpeechRecognizer(delegate_.get(),
                                                browser_.get(),
                                                nullptr,
                                                "en")) {
 ...
 protected:
  ...
  std::unique_ptr<MockBrowserUiInterface> browser_;
...
TEST_F(SpeechRecognizerTest, ReceivedCorrectSpeechResult) {
  testing::Sequence s;
  EXPECT_CALL(*browser_, SetSpeechRecognitionEnabled(true)).InSequence(s);
  EXPECT_CALL(*browser_, SetRecognitionResult(base::ASCIIToUTF16(kTestResult)))
      .InSequence(s);
  EXPECT_CALL(*delegate_, OnVoiceResults(base::ASCIIToUTF16(kTestResult)))
      .Times(1)
      .InSequence(s);
  EXPECT_CALL(*browser_, SetSpeechRecognitionEnabled(false)).InSequence(s);
