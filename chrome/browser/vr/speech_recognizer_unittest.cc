...
class SpeechRecognizerTestWrapper : public SpeechRecognizer {
 public:
  SpeechRecognizerTestWrapper()
      : SpeechRecognizer(nullptr, nullptr, "en") {}
 ...
 public:
  SpeechRecognizerTest()
      : fake_speech_recognition_manager_(
            new FakeSpeechRecognitionManager()),
        speech_recognizer_(new SpeechRecognizerTestWrapper) {
    SpeechRecognizer::SetManagerForTest(
...
TEST_F(SpeechRecognizerTest, ReceiveSpeechResult) {
  ...
  EXPECT_CALL(*speech_recognizer_,
              OnSpeechResult(
                  base::ASCIIToUTF16("kitten"), false)).InSequence(s);
  EXPECT_CALL(*speech_recognizer_,
              OnSpeechResult(base::ASCIIToUTF16("cat"), true)).InSequence(s);
  // After receiving final speech result, speech recognition should send a reset
  // request.
  EXPECT_CALL(*speech_recognizer_,
              OnSpeechRecognitionStateChanged(
                  SPEECH_RECOGNITION_READY)).InSequence(s);
...
TEST_F(SpeechRecognizerTest, ReceivedSpeechRecognitionStates) {
  ...
  EXPECT_CALL(*speech_recognizer_,
              OnSpeechRecognitionStateChanged(
                  SPEECH_RECOGNITION_RECOGNIZING)).InSequence(s);
  EXPECT_CALL(*speech_recognizer_,
              OnSpeechRecognitionStateChanged(
                  SPEECH_RECOGNITION_NETWORK_ERROR)).InSequence(s);
  EXPECT_CALL(*speech_recognizer_,
              OnSpeechRecognitionStateChanged(
                  SPEECH_RECOGNITION_READY)).InSequence(s);
...
TEST_F(SpeechRecognizerTest, NoSoundTimeout) {
  ...
  SpeechRecognizer::SetSpeechTimerForTest(std::move(mock_timer));

  testing::Sequence s;
  EXPECT_CALL(*speech_recognizer_,
              OnSpeechRecognitionStateChanged(
                  SPEECH_RECOGNITION_IN_SPEECH)).InSequence(s);
  EXPECT_CALL(*speech_recognizer_,
              OnSpeechRecognitionStateChanged(
                  SPEECH_RECOGNITION_READY)).InSequence(s);
...
TEST_F(SpeechRecognizerTest, SafeToResetAfterStart) {
  ...
  EXPECT_CALL(*speech_recognizer_,
              OnSpeechResult(base::ASCIIToUTF16("cat"), true)).Times(0);
  fake_speech_recognition_manager_->FakeSpeechRecognitionEvent(
      SPEECH_RECOGNITION_START);
  base::RunLoop().RunUntilIdle();
