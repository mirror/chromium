...
class ContentInputForwarder {
 public:
  ...
  virtual void ForwardEvent(std::unique_ptr<blink::WebInputEvent> event,
                            int content_id) = 0;
  // Text input specific.
  virtual void OnWebInputEdited(const TextInputInfo& info) = 0;
  virtual void OnWebInputCommitted(const TextInputInfo& info) = 0;
};
...
class ContentInputDelegate {
 public:
  ...
  // Text Input specific.
  void OnWebInputEdited(const TextInputInfo& info);
  void OnWebInputCommitted(const TextInputInfo& info);
