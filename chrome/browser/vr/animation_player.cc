...
void ReverseKeyframeModel(base::TimeTicks monotonic_time,
  ...
  // we'd like to be 80% of the way of the reversed keyframe_model (so it will
  // end quickly).
  //
  // We can modify our "progress" through a keyframe_model by modifying the
  // "time offset", a value added to the current time by the keyframe_model
  // system before applying any other adjustments.
  //
...
void AnimationPlayer::Tick(base::TimeTicks monotonic_time) {
  ...
  for (auto& keyframe_model : keyframe_models_) {
    cc::AnimationTicker::TickKeyframeModel(monotonic_time, keyframe_model.get(),
                                           target_);
  }
