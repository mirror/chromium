...
bool SufficientlyEqual(float lhs, float rhs) {
  return base::ApproximatelyEqual(lhs, rhs, kTolerance);
}
...
bool SufficientlyEqual(const gfx::SizeF& lhs, const gfx::SizeF& rhs) {
  return base::ApproximatelyEqual(lhs.width(), rhs.width(), kTolerance) &&
         base::ApproximatelyEqual(lhs.height(), rhs.height(), kTolerance);
}
