...
void ReverseAnimation(base::TimeTicks monotonic_time,
  ...
  animation->set_direction(animation->direction() ==
                               ...
                               : cc::Animation::Direction::NORMAL);
  // This is odd and deserves some explanation. In order to figure out the
  // current time, the animation advances the current time by time offset before
  // doing any other adjustments. So say we've advanced from start time "s".
  // to current time "t". Now say the duration of the animation is "d". Then,
  // after reversing the animation, we would like to start sampling from d - t
  // Before reversing:
  //  s    t                         d
  //  |----|-------------------------|
  //  s                         t    d
  //  |-------------------------|----|
  //
  // So, we want the offset "o" such that when we subtract the "offset time"
  // (which is o + t) from D, we get t. That is,
  //  t = D - (o + t)
  //  0 = D - o - 2t
  //  o = D - 2t
