...
TEST_F(UiTest, WebVrAutopresented) {
  ...
  RunFor(MsToDelta(
      17 + 1000 * (kSplashScreenMinDurationSeconds + kSmallDelaySeconds)));
...
TEST_F(UiTest, TransientToastsWithDelayedFirstFrame) {
  ...
  // The splash screen is hidden unconventionally. A UI element, in a callback,
  // updates the model state. This model change will get picked up by other
  // elements in the next frame.
  OnBeginFrame(MsToDelta(20));
  VerifyOnlyElementsVisible("Autopresented", {kWebVrUrlToast});
}
