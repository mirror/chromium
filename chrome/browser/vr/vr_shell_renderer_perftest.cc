...
static bool SaveCurrentFrameBufferToPng(const gfx::Size& size,
  ...
  // order.
  std::vector<uint32_t> flipped_pixels(size.width() * size.height());
  for (int stride = 0; stride < size.height(); stride++) {
    for (int strideOffset = 0; strideOffset < size.width(); strideOffset++) {
      ...
      flipped_pixels[topPixelIndex] = pixels[bottomPixelIndex];
    }
  }
  ...
  bitmap.setPixels(flipped_pixels.data());
...
TEST(VrShellRendererPerfTest, RenderUi) {
  ...
  // TODO(tiborg): create depth buffer.
  ...
  // Save screenshot.
  ASSERT_TRUE(
      SaveCurrentFrameBufferToPng(kFrameBufferSize, kScreenshotFilename));
