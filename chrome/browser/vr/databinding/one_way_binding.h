...
// This class represents a binding that propagates a change from a source/model
// property to a sink/view. This is inappropriate for use in the case of, say,
// an editor view like a text field where changes must alse be propegated back
// to the model.
template <typename T>
...
// setting up bindings a little less painful, but it is not meant to solve all
// problems. If you need to do something more complex (eg, convert type T before
// it is propagated), you should use constructor directly.
//
// For example:
//
...
#define VR_BIND(T, M, m, Get, V, v, Set)                                    \
  base::MakeUnique<OneWayBinding<T>>(                                       \
