...
class VectorBinding : public Binding {
  ...
  void Update() override {
    ...
    if (!last_size_ || current_size != last_size_.value()) {
      ...
      for (size_t i = last_size; i < current_size; ++i) {
        bindings_[i] = std::move(base::MakeUnique<ElementBinding>(models_, i));
 ...
 private:
  ...
  // XXX BROKEN! We need to store bindings per model element.
  // If we don't, we will not clean up per-model bindings when models disappear.
