...
TEST_F(UiSceneManagerTest, UiUpdatesForFullscreenChanges) {
  ...
  EXPECT_TRUE(content_quad->animation_player().IsAnimatingProperty(TRANSFORM));
  EXPECT_TRUE(content_quad->animation_player().IsAnimatingProperty(BOUNDS));
  // Finish the transition.
  AnimateBy(MsToDelta(1000));
  EXPECT_FALSE(content_quad->animation_player().IsAnimatingProperty(TRANSFORM));
  EXPECT_FALSE(content_quad->animation_player().IsAnimatingProperty(BOUNDS));
