...
TEST_F(UiSceneManagerTest, SpeechRecognitionPromptBindings) {
  MakeManager(kNotInCct, kNotInWebVr);
  UiElement* speech_recognition_prompt =
      scene_->GetUiElementByName(kSpeechRecognitionPrompt);
  ...
  EXPECT_FALSE(IsAnimating(speech_recognition_prompt, {CIRCLE_GROW}));

  model_->speech_recognition_state = SPEECH_RECOGNITION_READY;
  EXPECT_TRUE(AnimateBy(MsToDelta(300)));
  EXPECT_TRUE(IsAnimating(speech_recognition_prompt, {CIRCLE_GROW}));
