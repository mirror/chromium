...
void VerifyChildElementsOpacity(UiElement* parent) {
  for (auto& child : parent->children()) {
    ...
    // 2. The element has special opacity (ScreenDimmer element).
...
void CheckChildOpacityRecursive(UiSceneManagerTest* test, UiElement* element) {
  ...
  VerifyChildElementsOpacity(element);
  ...
  for (auto& child : element->children())
    CheckChildOpacityRecursive(test, child.get());
}
...
TEST_F(UiSceneManagerTest, ApplyParentOpacity) {
  ...
  CheckChildOpacityRecursive(this, &scene_->root_element());
}
