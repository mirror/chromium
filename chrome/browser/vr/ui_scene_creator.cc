...
void UiSceneCreator::CreateBackground() {
  ...
  sky->AddBinding(VR_BIND_FUNC(EnvironmentGradientColors, Model, model_,
                               model->color_scheme().sky_gradient, Sky,
                               sky.get(), SetColors));
  sky->SetOpacity(kSkyOpacity);
...
void UiSceneCreator::CreateCloseButton() {
  ...
  element->AddBinding(
      VR_BIND(bool, Model, model_, model->fullscreen_enabled(), UiElement,
              element.get(),
              view->SetSize(
                  value ? kCloseButtonFullscreenWidth : kCloseButtonWidth,
