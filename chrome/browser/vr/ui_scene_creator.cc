...
void UiSceneCreator::CreateContentRepositioningAffordance() {
  auto reposition_button = Create<DiscButton>(
      ...
      base::BindRepeating(
          ...
          base::Unretained(model_)),
      kRepositionIcon);
  reposition_button->SetSize(kContentQuadRepositionButtonDiameter,
                             kContentQuadRepositionButtonDiameter);
  ...
  reposition_button->SetTranslate(kContentQuadRepositionButtonXOffset,
                                  kContentQuadRepositionButtonYOffset, 0);
  reposition_button->SetTransitionedProperties({OPACITY});
  reposition_button->SetTransitionDuration(base::TimeDelta::FromMilliseconds(
      kContentQuadRepositionButtonTransitionDurationMs));
  reposition_button->background()->SetTransitionedProperties(
      {BACKGROUND_COLOR, TRANSFORM});
  reposition_button->SetOpacity(kContentQuadRepositionButtonMinOpacity);
      ...
      VR_BIND_LAMBDA(
          [](Model* model, Button* button) {
            if (!model->experimental_features_enabled)
              return 0.0f;
            if (button->hovered())
              return kContentQuadRepositionButtonMaxOpacity;
            if (!model->controller.quiescent)
              return kContentQuadRepositionButtonMidOpacity;
            return kContentQuadRepositionButtonMinOpacity;
          },
