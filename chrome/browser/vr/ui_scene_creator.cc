...
void UiSceneCreator::CreateOmnibox() {
  ...
  auto omnibox_text_field = Create<OmniboxTextField>(
      kNone,  // TODO **************
      kPhaseNone, kOmniboxTextHeightDMM,
      base::BindRepeating(
  ...
  omnibox_text_field->SetDrawPhase(kPhaseNone);
  omnibox_text_field->set_hit_testable(false);

  omnibox_text_field->AddBinding(std::make_unique<Binding<TextInputInfo>>(
      VR_BIND_LAMBDA(
  ...
  omnibox_text_field->AddBinding(
      std::make_unique<Binding<std::pair<bool, OmniboxSuggestion>>>(
          VR_BIND_LAMBDA(
              [](Model* m) {
                if (!m->omnibox_suggestions.empty()) {
                  return std::pair<bool, OmniboxSuggestion>(
                      true, m->omnibox_suggestions.front());
                } else {
                  return std::pair<bool, OmniboxSuggestion>(
                      false, OmniboxSuggestion());
                }
              },
              base::Unretained(model_)),
          VR_BIND_LAMBDA(
              [](OmniboxTextField* e,
                 const std::pair<bool, OmniboxSuggestion>& v) {
                if (v.first) {
                  e->SetTopMatch(v.second);
                } else {
                  e->ClearTopMatch();
                }
              },
  ...
  omnibox_text_field->AddBinding(std::make_unique<Binding<bool>>(
      ...
      VR_BIND_LAMBDA(
          ...
          base::Unretained(omnibox_text_field.get()),
          base::Unretained(model_))));
  // TODO - enable omnibox when top of the stack.
  omnibox_text_field->AddBinding(std::make_unique<Binding<bool>>(
      VR_BIND_LAMBDA(
