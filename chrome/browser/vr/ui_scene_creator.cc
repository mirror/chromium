...
// TODO(vollick): it would be nice to have an animated transition here, but this
// causes flickering in the spacer. Will need to debug it.
...
void UiSceneCreator::CreateBackground() {
  ...
  //  BindColor(model_, floor.get(), &ColorScheme::floor,
  //  &Grid::SetCenterColor); BindColor(model_, floor.get(),
  //  &ColorScheme::world_background,
  //            &Grid::SetEdgeColor);
...
void UiSceneCreator::CreateOmnibox() {
  ...
  suggestions_outer_layout->set_y_anchoring(TOP);
  suggestions_outer_layout->set_y_centering(BOTTOM);
  suggestions_outer_layout->SetTranslate(0, kSuggestionGapDMM, 0);
  ...
  spacer->SetTransitionedProperties({OPACITY});
//  spacer->AddBinding(base::MakeUnique<Binding<bool>>(base::Bind([](Model* m, 
  spacer->AddBinding(VR_BIND_FUNC(bool, Model, model_,
                                  omnibox_suggestions.empty() == false, Rect,
                                  spacer.get(), SetVisible));
  ...
  scene_->AddUiElement(kOmniboxContainer, std::move(suggestions_outer_layout));

  // TODO(vollick): make constants.
  scaler = base::MakeUnique<ScaledDepthAdjuster>(-.35f);
