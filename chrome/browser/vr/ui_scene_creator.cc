...
std::unique_ptr<T> Create(UiElementName name, DrawPhase phase, Args&&... args) {
  auto e = base::MakeUnique<T>(std::forward<Args>(args)...);
  e->set_name(name);
  e->set_draw_phase(phase);
  return e;
}
...
void UiSceneCreator::CreateWebVrTimeoutScreen() {
  ...
  auto button_scaler = base::MakeUnique<ScaledDepthAdjuster>(-0.1);
  ...
  button->set_hover_offset(kButtonZOffsetHoverDMM);
...
void UiSceneCreator::CreateCloseButton() {
  std::unique_ptr<Button> element =
      Create<Button>(kCloseButton, kPhaseForeground,
                     base::Bind(
                         [](Model* model, UiBrowserInterface* browser) {
                           if (model->fullscreen) {
