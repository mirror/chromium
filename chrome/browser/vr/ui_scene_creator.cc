...
std::unique_ptr<UiElement> CreateProceduralController(Model* model) {
  ...
  controller->AddBinding(VR_BIND_FUNC(
      gfx::Transform, Model, model, model->controller.transform,
      ProceduralController, controller.get(), set_local_transform));
  controller->AddBinding(
      VR_BIND_FUNC(float, Model, model, model->controller.opacity,
  ...
  home_button->AddBinding(
      VR_BIND_FUNC(SkColor, Model, model,
                   model->controller.home_button_state == UiInputManager::DOWN
                       ? model->color_scheme().controller_home_button_down
                       : model->color_scheme().controller_button,
...
std::unique_ptr<UiElement> CreateGltfController(Model* model) {
...
void UiSceneCreator::CreateController() {
  ...
  auto controller = model_->procedural_controller_enabled
                        ? CreateProceduralController(model_)
                        : CreateGltfController(model_);
