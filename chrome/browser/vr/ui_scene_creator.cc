...
void UiSceneCreator::CreateWebVrRoot() {
  ...
  element->set_hit_testable(false);
  VR_BIND_VISIBILITY(element, web_vr_enabled());
  scene_->AddUiElement(kRoot, std::move(element));
}
...
void UiSceneCreator::CreateExitWarning() {
  ...
  VR_BIND_VISIBILITY(scrim, exiting_vr);
  ...
  VR_BIND_VISIBILITY(exit_warning_bg, exiting_vr);
...
void UiSceneCreator::CreateSystemIndicators() {
  ...
  VR_BIND_VISIBILITY(indicator_layout, fullscreen_enabled() == false);
...
void UiSceneCreator::CreateSplashScreenForDirectWebVrLaunch() {
  ...
  VR_BIND_VISIBILITY(transient_parent, web_vr_autopresentation_enabled());
...
void UiSceneCreator::CreateWebVrTimeoutScreen() {
  ...
  VR_BIND_VISIBILITY(spinner, web_vr.state == kWebVrTimeoutImminent);
  ...
  VR_BIND_VISIBILITY(timeout_message, web_vr.state == kWebVrTimedOut);
  ...
  VR_BIND_VISIBILITY(button, web_vr.state == kWebVrTimedOut);
...
void UiSceneCreator::CreateBackground() {
  ...
  VR_BIND_VISIBILITY(background,
                     background_available && model->background_loaded);
  ...
  element->set_hit_testable(false);
  VR_BIND_VISIBILITY(element, background_available == false);
  ...
  for (auto& panel : panels) {
    ...
    VR_BIND_COLOR(model_, panel_element.get(), &ColorScheme::world_background,
                  &Rect::SetColor);
    VR_BIND_VISIBILITY(panel_element, browsing_enabled());
    scene_->AddUiElement(k2dBrowsingDefaultBackground,
                         std::move(panel_element));
  }

  auto sky = Create<Sky>(kSky, kPhaseBackground);
  sky->AddBinding(VR_BIND_FUNC(SkyGradientColors, Model, model_,
                               color_scheme().sky_gradient, Sky, sky.get(),
                               SetColors));
  sky->SetOpacity(0.75f);
  scene_->AddUiElement(k2dBrowsingTexturedBackground, std::move(sky));
  ...
  auto grid = Create<Grid>(kFloor, kPhaseBackground);
...
void UiSceneCreator::CreateController() {
  ...
  VR_BIND_VISIBILITY(
      root, browsing_enabled() || model->web_vr.state == kWebVrTimedOut);
...
void UiSceneCreator::CreateKeyboard() {
  ...
  VR_BIND_VISIBILITY(keyboard, editing_input);
...
void UiSceneCreator::CreateUrlBar() {
  ...
  VR_BIND_VISIBILITY(url_bar, fullscreen_enabled() == false);
  ...
  VR_BIND_VISIBILITY(indicator_bg, loading);
...
void UiSceneCreator::CreateOmnibox() {
  ...
  VR_BIND_VISIBILITY(omnibox_root, omnibox_editing_enabled());
...
void UiSceneCreator::CreateCloseButton() {
  ...
  VR_BIND_VISIBILITY(element, fullscreen_enabled() || model->in_cct);
  element->AddBinding(VR_BIND(
      bool, Model, model_, fullscreen_enabled(), UiElement, element.get(),
      SetTranslate(
          0,
...
void UiSceneCreator::CreateExitPrompt() {
  ...
  VR_BIND_VISIBILITY(
      backplane, active_modal_prompt_type == kModalPromptTypeExitVRForSiteInfo);
...
void UiSceneCreator::CreateWebVrOverlayElements() {
  ...
  VR_BIND_VISIBILITY(
      parent, web_vr.has_produced_frames() && model->web_vr.show_exit_toast);
  scene_->AddUiElement(kWebVrViewportAwareRoot, std::move(parent));
...
void UiSceneCreator::CreateFullscreenToast() {
  auto parent = CreateTransientParent(kExclusiveScreenToastTransientParent,
                                      kToastTimeoutSeconds, false);
  VR_BIND_VISIBILITY(parent, fullscreen_enabled());
