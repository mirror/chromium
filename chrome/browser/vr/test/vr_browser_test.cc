...
// Set the environment variable to use the mock OpenVR client
  ...
  temp_path = base::FilePath(temp_wstring);
  EXPECT_TRUE(
...
#else
  EXPECT_TRUE(
      env_->SetVar(kVrOverrideEnvVar, base::FilePath(kVrOverrideVal).value()));
  EXPECT_TRUE(env_->SetVar(kVrConfigPathEnvVar,
                           base::FilePath(kVrConfigPathVal).value()));
  EXPECT_TRUE(
      env_->SetVar(kVrLogPathEnvVar, base::FilePath(kVrLogPathVal).value()));
