...
#if defined(OS_WIN)
  // TODO(bsheedy): Figure out why Windows hates strings and see if there's a
  // cleaner way to do this. Only using FilePath to make sure the separators are
  // correct, something like Python's os.path.join would be nice.
  std::wstring temp_wstring;
  base::FilePath temp_path;
  EXPECT_TRUE(
      base::UTF8ToWide(kVrOverrideVal, strlen(kVrOverrideVal), &temp_wstring));
  temp_path = base::MakeAbsoluteFilePath(base::FilePath(temp_wstring));
  EXPECT_TRUE(
      env_->SetVar(kVrOverrideEnvVar, base::WideToUTF8(temp_path.value())));

  EXPECT_TRUE(base::UTF8ToWide(kVrConfigPathVal, strlen(kVrConfigPathVal),
                               &temp_wstring));
  temp_path = base::MakeAbsoluteFilePath(base::FilePath(temp_wstring));
  EXPECT_TRUE(
      env_->SetVar(kVrConfigPathEnvVar, base::WideToUTF8(temp_path.value())));

  EXPECT_TRUE(
      base::UTF8ToWide(kVrLogPathVal, strlen(kVrLogPathVal), &temp_wstring));
  temp_path = base::MakeAbsoluteFilePath(base::FilePath(temp_wstring));
  EXPECT_TRUE(
      env_->SetVar(kVrLogPathEnvVar, base::WideToUTF8(temp_path.value())));
#else
  EXPECT_TRUE(env_->SetVar(
      kVrOverrideEnvVar,
      base::MakeAbsoluteFilePath(base::FilePath(kVrOverrideVal)).value()));
  EXPECT_TRUE(env_->SetVar(
      kVrConfigPathEnvVar,
      base::MakeAbsoluteFilePath(base::FilePath(kVrConfigPathVal)).value()));
  EXPECT_TRUE(env_->SetVar(
      kVrLogPathEnvVar,
      base::MakeAbsoluteFilePath(base::FilePath(kVrLogPathVal)).value()));
