...
void VrBrowserTest::SetUp() {
  ...
  EXPECT_TRUE(
      env_->SetVar(kVrOverrideEnvVar, FILE_PATH_LITERAL(kVrOverrideVal)));
  EXPECT_TRUE(
      env_->SetVar(kVrConfigPathEnvVar, FILE_PATH_LITERAL(kVrConfigPathVal)));
  EXPECT_TRUE(env_->SetVar(kVrLogPathEnvVar, FILE_PATH_LITERAL(kVrLogPathVal)));
