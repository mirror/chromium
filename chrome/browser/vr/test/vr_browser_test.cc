...
TestStatus VrBrowserTest::CheckTestStatus(content::WebContents* web_contents) {
  ...
  if (test_passed) {
    return TestStatus::STATUS_PASSED;
  } else if (!test_passed && result_string == "") {
    return TestStatus::STATUS_RUNNING;
  } else {
    // !test_passed && result_string != ""
    return TestStatus::STATUS_FAILED;
  }
}

void VrBrowserTest::EndTest(content::WebContents* web_contents) {
  switch (CheckTestStatus(web_contents)) {
    case TestStatus::STATUS_PASSED:
      break;
    case TestStatus::STATUS_FAILED:
...
bool VrBrowserTest::PollJavaScriptBoolean(const std::string& bool_expression,
  ...
  auto poll_lambda = [](const std::string& be, content::WebContents* wc) {
    return RunJavaScriptAndExtractBoolOrFail(bool_expression, web_contents);
  };
...
void VrBrowserTest::WaitOnJavaScriptStep(content::WebContents* web_contents) {
  ...
  TestStatus test_status = CheckTestStatus(web_contents);
  if (!success || test_status == TestStatus::STATUS_FAILED) {
