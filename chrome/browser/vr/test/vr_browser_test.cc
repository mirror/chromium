...
#if defined(OS_WIN)
  // TODO(bsheedy): Figure out why Windows hates strings
  std::wstring temp EXPECT_TRUE(
      base::UTF8ToWide(kVrOverrideVal, strlen(kVrOverrideVal), &temp));
  EXPECT_TRUE(env_->SetVar(kVrOverrideEnvVar, base::FilePath(temp).value()));
  EXPECT_TRUE(
      base::UTF8ToWide(kVrConfigPathVal, strlen(kVrConfigPathVal), &temp));
  EXPECT_TRUE(env_->SetVar(kVrOverrideEnvVar, base::FilePath(temp).value()));
  EXPECT_TRUE(base::UTF8ToWide(kVrLogPathVal, strlen(kVrLogPathVal), &temp));
  EXPECT_TRUE(env_->SetVar(kVrLogPathEnvVar, base::FilePath(temp).value()));
#else
  EXPECT_TRUE(
