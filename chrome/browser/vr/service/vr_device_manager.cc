...
#if defined(OS_ANDROID)
    providers_.emplace_back(std::make_unique<device::GvrDeviceProvider>());
#endif

#if BUILDFLAG(ENABLE_OPENVR)
    providers_.emplace_back(std::make_unique<OpenVRDeviceProvider>());
#endif
    new VRDeviceManager(std::move(providers_));
  }
