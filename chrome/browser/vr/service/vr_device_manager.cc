...
void VRDeviceManager::AddDevice(device::VRDevice* device) {
  ...
  if (last_device_added_)
    return;
  if (device->GetId() == device::VR_DEVICE_LAST_ID)
    last_device_added_ = true;
...
bool VRDeviceManager::ProvidersInitialized() {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  return initialized_providers_ == providers_.size();
}
