...
class IOBrowserUIInterface {
 public:
  ...
  // Invoked when SpeechRecognizerOnIO is created on IO thread.
  virtual void OnSpeechRecognizerOnIOCreated(SpeechRecognizerOnIO* ptr) = 0;
...
class SpeechRecognizer : public IOBrowserUIInterface {
 public:
  ...
  void OnSpeechRecognizerOnIOCreated(SpeechRecognizerOnIO* ptr) override;
 ...
 private:
  ...
  std::string locale_;
  bool retry_start_when_ready_ = false;
  bool retry_stop_when_ready_ = false;
