...
TexturedQuadRenderer::TexturedQuadRenderer(const char* vertex_src,
                                           const char* fragment_src)
    : BaseQuadRenderer(vertex_src, fragment_src) {
...
void TexturedQuadRenderer::AddQuad(int texture_data_handle,
  ...
  quad.content_texture_size = content_texture_size;
...
void TexturedQuadRenderer::Flush() {
  ...
  while (!quad_queue_.empty()) {
    ...
    if (quad.corner_radius == 0.0f && last_corner_radius != 0.0f) {
      glUniform1f(corner_scale_handle_, 0.5);
      glUniform2f(corner_offset_handle_, 0.0, 0.0);
      glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_SHORT,
                     VOID_OFFSET(6 * sizeof(GLushort)));
    } else if (quad.corner_radius != last_corner_radius ||
               quad.element_size != last_element_size) {
      glUniform1f(corner_scale_handle_, 1.0f / ComputePhysicalWidth(quad));
    ...
    if (quad.corner_radius == 0.0f) {
      // We only draw the inner triangles in this case.
      glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_SHORT,
...
float TexturedQuadRenderer::ComputePhysicalWidth(const QuadData& quad) const {
  ...
  return 0.5 * physical_width * quad.content_texture_size.width();
}
...
void VrShellRenderer::DrawTexturedQuad(int texture_data_handle,
  ...
  GetTexturedQuadRenderer()->AddQuad(texture_data_handle, view_proj_matrix,
                                     copy_rect, opacity, content_texture_size_,
