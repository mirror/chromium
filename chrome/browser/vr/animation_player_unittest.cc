...
TEST(AnimationPlayerTest, AddRemoveAnimations) {
  ...
  player.AddAnimation(creator.size_animation());
  ...
  player.AddAnimation(creator.transform_animation());
...
TEST(AnimationPlayerTest, AnimationQueue) {
  ...
  int id2 = player.animations()[2]->id();

  player.Tick(start_time + AnimationCreator::usToDelta(1));

  // Neither queued animation should have started since there's a running
  // animation.
  EXPECT_EQ(cc::Animation::WAITING_FOR_TARGET_AVAILABILITY,
            player.animations()[1]->run_state());
  EXPECT_EQ(cc::Animation::WAITING_FOR_TARGET_AVAILABILITY,
            player.animations()[2]->run_state());

  // Tick beyond the first animation. This should cause it to get removed and
  ...
  EXPECT_EQ(id2, player.animations()[1]->id());
