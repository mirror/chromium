...
TEST(AnimationPlayeTest, CorrectAnimationTarget) {
  TestAnimationTarget target;
  AnimationPlayer player;
  player.set_target(&target);
  ...
  // Add animations.
  player.AddAnimation(CreateBoundsAnimation(1, 1, gfx::SizeF(10, 100),
                                            gfx::SizeF(20, 200), duration));
  player.AddAnimation(CreateOpacityAnimation(2, 1, 1.0f, 0.5f, duration));
  player.AddAnimation(CreateBackgroundColorAnimation(3, 1, SK_ColorRED,
                                                     SK_ColorGREEN, duration));
  cc::TransformOperations from_operations;
  from_operations.AppendTranslate(10, 100, 1000);
  cc::TransformOperations to_operations;
  to_operations.AppendTranslate(20, 200, 2000);
  player.AddAnimation(
      CreateTransformAnimation(4, 1, from_operations, to_operations, duration));

  base::TimeTicks start_time = MicrosecondsToTicks(1000000);
  player.Tick(start_time);

  // Verify target value.
  EXPECT_EQ(0.5f, player.GetTargetFloatValue(OPACITY));
  EXPECT_SIZEF_EQ(gfx::SizeF(20, 200), player.GetTargetSizeValue(BOUNDS));
  EXPECT_EQ(SK_ColorGREEN, player.GetTargetColorValue(BACKGROUND_COLOR));
  EXPECT_TRUE(to_operations.ApproximatelyEqual(
      player.GetTargetTransformOperationsValue(TRANSFORM), 1e-5f));
}
