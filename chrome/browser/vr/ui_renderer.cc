...
void UiRenderer::Draw(const RenderInfo& render_info) {
  ...
  DrawUiView(render_info, scene_->GetVisible2dBrowsingElements());
  DrawUiView(render_info, scene_->GetVisibleWebVrElements());
  DrawUiView(render_info, scene_->GetVisibleKeyboardElements());
  DrawUiView(render_info, scene_->GetVisibleControllerElements());
}
