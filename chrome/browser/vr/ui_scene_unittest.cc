...
TEST(UiScene, ViewportAware) {
  ...
  EXPECT_EQ(gfx::Point3F(0.f, 0.f, -2.f).ToString(),
            scene.GetUiElementById(2)->GetCenter().ToString());

  // This should trigger reposition of viewport aware elements.
  scene.OnBeginFrame(MicrosecondsToTicks(10), gfx::Vector3dF(-1.f, 0.f, 0.f));

  EXPECT_EQ(gfx::Point3F(-1.f, 0.f, -0.f).ToString(),
            scene.GetUiElementById(1)->GetCenter().ToString());
  EXPECT_EQ(gfx::Point3F(-2.f, 0.f, -0.f).ToString(),
            scene.GetUiElementById(2)->GetCenter().ToString());
}
