...
class RendererFactory {
 public:
  ...
  bool Initialize();
  std::unique_ptr<ui::GlRenderer> CreateRenderer(gfx::AcceleratedWidget widget,
                                                 const gfx::Size& size,
                                                 vr::VrTestContext* vr);
...
class DemoWindow : public ui::PlatformWindowDelegate {
 ...
 private:
  ...
  std::unique_ptr<ui::GlRenderer> renderer_;
...
std::unique_ptr<ui::GlRenderer> RendererFactory::CreateRenderer(
  ...
  return base::MakeUnique<ui::GlRenderer>(surface, size, vr);
}
