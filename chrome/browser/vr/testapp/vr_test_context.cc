...
VrTestContext::VrTestContext()
  ...
  // TODO(cjgrant): ResourceBundle init should initialize this too.
  ...
  ui_->browser_ui()->SetHistoryButtonsEnabled(true, true);
  ui_->browser_ui()->SetLoading(true);
  ui_->browser_ui()->SetLoadProgress(0.4);
  ui_->browser_ui()->SetVideoCapturingIndicator(true);
  ui_->browser_ui()->SetScreenCapturingIndicator(true);
  ui_->browser_ui()->SetAudioCapturingIndicator(true);
  ui_->browser_ui()->SetBluetoothConnectedIndicator(true);
  ui_->browser_ui()->SetLocationAccessIndicator(true);
}
...
void VrTestContext::DrawFrame() {
  ...
  constexpr float view_scale = 1.4f;
  const gfx::Transform view_proj(view_scale, 0, 0, 0, 0, view_scale, 0, 0, 0, 0,
                                 -1, 0, 0, 0, -1, 0);
  ...
  ui_->ui()->OnProjMatrixChanged(render_info.left_eye_info.proj_matrix);
...
void VrTestContext::HandleInput(ui::Event* event) {
  if (event->IsKeyEvent()) {
    ...
    if (key_event->code() == ui::DomCode::US_F)
      ui_->browser_ui()->SetFullscreen(true);
    return;
  }
  ...
  gfx::Point3F target_point = {
      6 * (mouse_event->x() / window_size_.width() - 0.5),
      6 * (-mouse_event->y() / window_size_.height() + 0.5), -2};
