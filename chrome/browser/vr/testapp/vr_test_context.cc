...
VrTestContext::VrTestContext()
    : ui_(base::MakeUnique<Ui>(this, this, UiInitialState())),
      ...
      proj_scale_factor_(kDefaultViewScaleFactor) {
...
void VrTestContext::DrawFrame() {
  ...
  const gfx::Transform proj_matrix(proj_scale_factor_, 0, 0, 0, 0,
                                   proj_scale_factor_, 0, 0, 0, 0, -1, 0, 0, 0,
                                   -1, 0);
...
void VrTestContext::HandleInput(ui::Event* event) {
  ...
  if (event->IsMouseWheelEvent()) {
    ...
    proj_scale_factor_ *= (1 + direction * kViewScaleAdjustmentFactor);
    proj_scale_factor_ = base::ClampToRange(
        proj_scale_factor_, kMinViewScaleFactor, kMaxViewScaleFactor);
    return;
  }
  ...
  if (mouse_event->IsRightMouseButton()) {
    if (last_drag_x_ != 0 || last_drag_y_ != 0) {
      float angle_y = 180.f * ((mouse_event->x() - last_drag_x_) - 0.5f) /
                      window_size_.width();
      float angle_x = 180.f * ((mouse_event->y() - last_drag_y_) - 0.5f) /
                      window_size_.height();
      head_angle_x_ += angle_x;
      head_angle_y_ += angle_y;
      head_angle_x_ = base::ClampToRange(head_angle_x_, -90.f, 90.f);
    }
    last_drag_x_ = mouse_event->x();
    last_drag_y_ = mouse_event->y();
  } else {
    last_drag_x_ = 0;
    last_drag_y_ = 0;
  }
  ...
  head_pose_ = gfx::Transform();
  head_pose_.RotateAboutXAxis(-head_angle_x_);
  head_pose_.RotateAboutYAxis(-head_angle_y_);
  ...
  beam_transform.RotateAboutYAxis(head_angle_y_ + delta_y);
  beam_transform.RotateAboutXAxis(head_angle_x_ + delta_x);
