...
void VrTestContext::HandleInput(ui::Event* event) {
  ...
  ui_->model()->controller.laser_origin = kLaserOrigin + local_offset;

  gfx::Transform controller_transform;
  controller_transform.Translate3d(kLaserOrigin.x(), kLaserOrigin.y(),
                                   kLaserOrigin.z());

  controller_transform.Scale3d(kControllerScaleFactor, kControllerScaleFactor,
                               kControllerScaleFactor);
  controller_transform.RotateAboutYAxis(head_angle_y_degrees_ + delta_y);
  controller_transform.RotateAboutXAxis(head_angle_x_degrees_ + delta_x);
...
void VrTestContext::OnGlInitialized(const gfx::Size& window_size) {
  ...
  // This is slow. We may want to make it a background task.
