...
// Each pair represents a stop on the opacity gradient. First item is the
// opacity, second item is the stop (both in the range [0,1]).
...
constexpr size_t kNumSquares = kNumRings * kNumSectors * 2 +        // Spheres.
...
ProceduralController::Renderer::Renderer()
  ...
  vertices_.reserve(kNumVertices * 3);
  colors_.reserve(kNumVertices * 4);
  indices_.reserve(kNumSquares * 6);
