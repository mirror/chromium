...
static constexpr int kSpinnerWidth = 40;
...
class SpinnerTexture : public UiTexture {
 ...
 private:
  ...
  void Draw(SkCanvas* sk_canvas, const gfx::Size& texture_size) override {
    ...
    paint.setStrokeWidth(kSpinnerWidth);
    int n = kSpinnerWidth / 2;
    sk_canvas->drawArc(SkRect::MakeLTRB(n, n, texture_size.width() - n,
                                        texture_size.height() - n),
                       angle_start_ - angle_sweep_ + rotation_,
                       2.0 * angle_sweep_, false, paint);
  }
...
Spinner::Spinner(int maximum_width)
  ...
  for (size_t i = 0; i < 2; ++i) {
    curve->AddKeyframe(
        cc::FloatKeyframe::Create(kRotationDuration * i, 360.0f * i, nullptr));
  }
...
void Spinner::NotifyClientFloatAnimated(float value,
  ...
  if (target_property_id == SPINNER_ANGLE_SWEEP) {
    texture_->SetAngleSweep(value);
  } else if (target_property_id == SPINNER_ANGLE_START) {
    texture_->SetAngleStart(value);
  } else if (target_property_id == SPINNER_ROTATION) {
