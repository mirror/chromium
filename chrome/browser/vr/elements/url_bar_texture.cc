  ...
  if (state_.should_display_url) {
    ...
    if (!url_render_text_ || url_dirty_) {
      ...
      const base::string16 formatted_url = url_formatter::FormatUrl(
          state_.gurl, format_types, net::UnescapeRule::NORMAL, &parsed,
          nullptr, nullptr);

      int pixel_font_height = texture_size.height() * kFontHeight / kHeight;

      UiUnsupportedMode mode = UiUnsupportedMode::kCount;
      if (!RenderUrl(formatted_url, parsed, SK_ColorBLACK, pixel_font_height,
                     render_text.get(), &mode)) {
        failure_callback_.Run(mode);
      }

      vr::RenderTextWrapper vr_render_text(render_text.get());
      ApplyUrlStyling(formatted_url, parsed, state_.security_level,
                      &vr_render_text, color_scheme());
