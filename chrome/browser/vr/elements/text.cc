...
constexpr int kTextPixelPerMeters = 1100;
}
...
Text::Text(float font_height_meters)
    : TexturedElement(0),
...
void Text::UpdateElementSize() {
  ...
  if (text_mode_ == kSingleLineFixedHeight) {
    ...
    TexturedElement::UpdateElementSize();
  }
}
...
void TextTexture::LayOutText() {
  gfx::FontList fonts;
  int pixel_font_height = static_cast<int>(font_height_ * kTextPixelPerMeters);
  ...
  if (mode_ == kSingleLineFixedHeight) {
    ...
    text_bounds.set_width(kTextPixelPerMeters * text_size_.width());
  }
