...
class TextTexture : public UiTexture {
 public:
  explicit TextTexture(const base::string16& text) : text_(text) {}
...
Text::Text(int maximum_width, const base::string16& text)
...
void TextTexture::Draw(SkCanvas* sk_canvas, const gfx::Size& texture_size) {
  ...
  gfx::FontList fonts;
  GetFontList(size_.width() * kFontSizeFactor, text_, &fonts);
  gfx::Rect text_size(size_.width() * kTextWidthFactor, 0);

  std::vector<std::unique_ptr<gfx::RenderText>> lines =
      PrepareDrawStringRect(text_, fonts, SK_ColorBLACK, &text_size,
