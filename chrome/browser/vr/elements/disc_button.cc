...
DiscButton::DiscButton(base::Callback<void()> click_handler,
                       const gfx::VectorIcon& icon)
    : Button(click_handler), hover_offset_(kDefaultHoverOffsetDMM) {
...
void DiscButton::OnStateUpdated() {
  ...
  if (hovered()) {
    background()->SetTranslate(0.0, 0.0, hover_offset_);
    foreground_->SetTranslate(0.0, 0.0, hover_offset_);
    hit_plane()->SetScale(kHitPlaneScaleFactorHovered,
                         kHitPlaneScaleFactorHovered, 1.0f);
  } else {
    background()->SetTranslate(0.0, 0.0, 0.0);
    foreground_->SetTranslate(0.0, 0.0, 0.0);
    hit_plane()->SetScale(1.0f, 1.0f, 1.0f);
  }
