...
class Toast : public UiElement {
 public:
  ...
  void ConfigIcon(const gfx::VectorIcon& icon, float icon_size);
  void ConfigText(const base::string16& text,
                  float font_height_dmm,
                  TextLayoutMode text_layout_mode);
  void SetMargin(float margin_in_dmms);

  Rect* background() { return background_; }
  VectorIcon* icon() { return icon_; }
