...
TEST(UiElement, BoundsContainChildren) {
  ...
  EXPECT_RECT_NEAR(gfx::RectF(2.5f, 2.5f, 3.2f, 3.4f), parent->bounds(),
                   epsilon);
  ...
  parent->DoLayOutChildren();
  EXPECT_EQ(gfx::PointF(-0.5f, 1.0f).ToString(),
            parent->bounds().origin().ToString());
  EXPECT_RECT_NEAR(gfx::RectF(-0.5f, 1.0f, 9.2f, 6.4f), parent->bounds(),
                   epsilon);
  ...
  parent->DoLayOutChildren();
  EXPECT_EQ(gfx::PointF(-0.5f, 0.5f).ToString(),
            parent->bounds().origin().ToString());
  EXPECT_RECT_NEAR(gfx::RectF(-0.5f, 0.5f, 9.2f, 7.4f), parent->bounds(),
                   epsilon);
}
