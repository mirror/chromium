...
void Toast::ConfigIcon(const gfx::VectorIcon& icon, float icon_size) {
  auto vector_icon = base::MakeUnique<VectorIcon>(64);
...
void Toast::ConfigText(const base::string16& text,
                       float font_height_dmm,
                       TextLayoutMode text_layout_mode) {
  auto text_element = base::MakeUnique<Text>(0);
  ...
  text_->SetText(text);
  text_->SetFontHeightInDmm(font_height_dmm);
  text_->SetTextLayoutMode(text_layout_mode);
