...
gfx::Vector3dF GetEffectiveUpVector(const gfx::Vector3dF& fwd,
                                    const gfx::Vector3dF& head_fwd,
                                    const gfx::Vector3dF& right,
                                    const gfx::Vector3dF& head_up) {
  float degrees_from_up =
      std::min(gfx::AngleBetweenVectorsInDegrees(fwd, kUp),
               gfx::AngleBetweenVectorsInDegrees(head_fwd, kUp));
...
void Repositioner::UpdateTransform(const gfx::Transform& head_pose) {
  ...
  // We may now be in a situation where our yaw may be out of range. We don't
  // want to move reposition downward, nor do we want to move too far forward.
  // We also don't want to throw away our deltas, or we'll completely freeze
  // when the yaw is out of range when we should be continuing to update pitch
  // and roll.
  new_forward = forward;
  transform_.TransformVector(&new_forward);

  // Finally we have to correct the roll.
  gfx::Vector3dF up =
