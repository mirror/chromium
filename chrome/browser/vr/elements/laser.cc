...
void Laser::Render(UiElementRenderer* renderer,
  ...
  const float laser_length =
      std::sqrt(origin_.SquaredDistanceTo(ScalePoint(target_, kReticleOffset)));
  ...
  const gfx::Vector3dF beam_direction = target_ - origin_;
  ...
  for (int i = 0; i < faces; i++) {
    ...
    face_transform.matrix().postTranslate(origin_.x(), origin_.y(),
                                          origin_.z());
