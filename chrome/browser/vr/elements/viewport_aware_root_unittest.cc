...
void CheckMatricesNearlyEqual(const gfx::Transform& lhs,
                              const gfx::Transform& rhs) {
  for (int row = 0; row < 4; ++row) {
    for (int col = 0; col < 4; ++col) {
      EXPECT_TRUE(std::abs(lhs.matrix().get(row, col) -
...
void CheckRotateClockwiseAndReverse(const gfx::Vector3dF& initial_look_at) {
  ...
  root.AdjustPositionForHeadPose(look_at);
  CheckMatricesNearlyEqual(expected, root.LocalTransform());
...
TEST(ViewportAwareRoot, TestAdjustPositionForHeadPose) {
