...
void OmniboxTextField::OnUpdateInput(const TextInputInfo& info,
  ...
  size_t previous_base_size = previous_info.text.size();
  if (previous_info.SelectionSize() > 0)
    previous_base_size = previous_info.selection_start;
  ...
  if (info.text.size() <= previous_base_size)
    request.prevent_inline_autocomplete = true;
