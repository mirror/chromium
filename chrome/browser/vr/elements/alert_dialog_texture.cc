...
constexpr int kHeightWidthRatio = 12.0;
...
AlertDialogTexture::AlertDialogTexture(const gfx::VectorIcon& icon,
    ...
    : icon_(icon), message_(text_message), has_text_(true) {}
...
void AlertDialogTexture::SetText(base::string16 new_text) {
  message_ = new_text;
  set_dirty();
}
...
void AlertDialogTexture::Draw(SkCanvas* sk_canvas,
  ...
  if (has_text_) {
    gfx::FontList fonts;
    GetFontList(size_.height() * kFontSizeFactor, message_, &fonts);
    text_size.set_height(kTextHeightFactor * size_.height());

    lines = PrepareDrawStringRect(
        message_, fonts, color_scheme().system_indicator_foreground, &text_size,
...
gfx::Size AlertDialogTexture::GetPreferredTextureSize(
