...
static constexpr char const* kVertexShader = SHADER(
  ...
  void main() {
    v_Position = vec2(0.5f + a_Position.x, 0.5f - a_Position.y);
...
static constexpr char const* kFragmentShader = SHADER(
  ...
  void main() {
    ...
    // same way.
    vec2 pos = vec2(0.5f - abs(v_Position.x - 0.5f),
                    0.5f - abs(v_Position.y - 0.5f - u_YOffset));
    ...
    bool inside_x_pad = x_pad_distance < 0.0f;
    bool inside_y_pad = y_pad_distance < 0.0f;
    ...
    float x_clamped = 1.0f + x_pad_distance;
    float y_clamped = 1.0f + y_pad_distance;
    ...
    if (inside_x_pad && inside_y_pad) {
      ...
      float t = acos(-n.x) / 1.57079632679489661923f;
      float offset = mix(x_offset, y_offset, t);
      mask = clamp((1.0 - clamp(length(v), 0.0f, 1.0f)) / (1.0 - offset),
                   0.0f, 1.0f);
    } else if (inside_x_pad) {
      mask = clamp(x_clamped / (1.0f - x_offset), 0.0f, 1.0f);
    } else if (inside_y_pad) {
      mask = clamp(y_clamped / (1.0f - y_offset), 0.0f, 1.0f);
    }

    // This is an arbitrary function that takes mask (which was previously a
    // linear ramp from the edge oth the rrect), and causes it to fall off more
    // rapidly.
    ...
    float n =
        (fract(dot(v_Position.xy, vec2(12345.67f, 456.7f))) - 0.5f) / 255.0f;
...
// Adjusting these will change the min/max gradient sizes along the x/y axes.
...
void Shadow::Render(UiElementRenderer* renderer,
  ...
  renderer->DrawShadow(model_view_proj_matrix, size(), x_padding(), y_padding(),
