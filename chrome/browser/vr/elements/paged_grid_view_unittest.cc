...
TEST(PagedGridView, MultiplePages) {
  ...
  EXPECT_TRUE(view.HitTest({0.5f, 0.5f}));
  ...
  EXPECT_POINT3F_EQ(gfx::Point3F(kElementWidth + margin,
                                 0.5f * (kElementHeight + margin), 0.0f),
                    GetLayoutPosition(*elements[2]));
  EXPECT_POINT3F_EQ(gfx::Point3F(-kElementWidth - margin, 0.0f, 0.0f),
                    GetLayoutPosition(view));

  // When we're on the second page, hit testing at (0.5f, 0.5f) should fail
  // because that'll be in the center of the first page.
  EXPECT_FALSE(view.HitTest({0.5f, 0.5f}));

  // However, we _should_ be at the center of the next page.
  EXPECT_TRUE(view.HitTest({1.5f, 0.5f}));
