...
void Keyboard::Initialize(SkiaSurfaceProvider* provider,
                          KeyboardDelegate* keyboard_delegate,
                          TextInputDelegate* text_input_delegate) {
  delegate_ = keyboard_delegate;
}

void Keyboard::SetKeyboardDelegate(KeyboardDelegate* delegate) {
  delegate_ = delegate;
}
...
void Keyboard::HitTest(const HitTestRequest& request,
  ...
  bool hits =
      delegate_->HitTest(request.ray_origin, request.ray_target, &hit_point);
