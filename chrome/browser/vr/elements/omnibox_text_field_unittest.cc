...
class OmniboxTest : public testing::Test {
 public:
  void SetUp() override {
    omnibox_ = std::make_unique<OmniboxTextField>(
        10,
        base::RepeatingCallback<void(const TextInputInfo& text_input_info)>(),
 ...
 protected:
  ...
  void SetInput(const std::string& text, int cursor_position) {
    SetInput(text, cursor_position, cursor_position);
  }
  ...
  void ExpectAutocompleteRequest(const std::string& SetInput,
                                 int cursor_position,
                                 bool prevent_inline_autocomplete) {
    AutocompleteRequest request;
    request.text = base::UTF8ToUTF16(SetInput);
...
TEST_F(OmniboxTest, HandleInput) {
  ...
  {
    // Backspace to remove the last character.  The autocomplete request should
    // now explicitly disallow an in-line match.
    SCOPED_TRACE(__LINE__);
  ...
  }
  {
    ...
    SCOPED_TRACE(__LINE__);
    ExpectAutocompleteRequest("w", 0, kPreventInline);
    ExpectKeyboardUpdate("w", 0);
    SetInput("w", 0);
    VerifyMocks();
  }
  {
  ...
  {
    // Type "i".  The autocomplete request should return to allowing inline.
    SCOPED_TRACE(__LINE__);
    ExpectAutocompleteRequest("wi", 2, kAllowInline);
    ExpectKeyboardUpdate("wi", 2);
    SetInput("wi", 2);
