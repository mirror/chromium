...
constexpr char const* kFragmentShader = SHADER(
  ...
  void main() {
    ...
    // TODO(vollick): we need to do proper "overlay" blend mode stuff here.
...
GLuint UploadImage(std::unique_ptr<SkBitmap> bitmap,
  ...
  *surface = provider->MakeSurface({bitmap->width(), bitmap->height()});
  // TODO(tiborg): is this safe? What error conditions should we be checking
  // for and how should we handle them?
  DCHECK(surface->get());
...
void Background::OnAssetsLoaded(std::unique_ptr<LoadedAssets> assets) {
  // NB: we take the objects off the model here so that we can destroy them
  // after uploading to the GPU (they're pretty big).
  initialization_bitmap_ = std::move(assets->background);
  initialization_normal_gradient_bitmap_ = std::move(assets->normal_gradient);
  initialization_incognito_gradient_bitmap_ =
      std::move(assets->incognito_gradient);
  initialization_fullscreen_gradient_bitmap_ =
