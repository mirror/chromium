...
constexpr char const* kVertexShader = SHADER(
  ...
  void main() {
    ...
    // straight down and pi is the head pointing straight up. If our source is
    // altitude, however, we've, roughly speaking, been given the cosine of the
    // pitch - we just need to remap the range 0..1 to 1..-1. And because the
    // sine of the pitch angle is always positive, we can easily compute it
    // using the sin^2(x) + cos^2(x) = 1 identity without worrying about sign.
