...
TargetProperty NextTargetProperty(TargetProperty p) {
  int int_property = static_cast<int>(p);
  int_property++;
  return static_cast<TargetProperty>(int_property);
}

typedef SkColor EnvironmentGradientColors::*GradientColorPtr;
GradientColorPtr GetGradientColorPtr(TargetProperty p) {
  DCHECK_LE(FIRST_GRADIENT_COLOR, p);
  DCHECK_GE(LAST_GRADIENT_COLOR, p);
  switch (p) {
    case GRADIENT_GROUND_COLOR:
      return &EnvironmentGradientColors::ground;
    case GRADIENT_MID_GROUND_COLOR:
      return &EnvironmentGradientColors::mid_ground;
    case GRADIENT_BELOW_HORIZON_COLOR:
      return &EnvironmentGradientColors::below_horizon;
    case GRADIENT_HORIZON_COLOR:
      return &EnvironmentGradientColors::horizon;
    case GRADIENT_ABOVE_HORIZON_COLOR:
      return &EnvironmentGradientColors::above_horizon;
    case GRADIENT_MID_SKY_COLOR:
      return &EnvironmentGradientColors::mid_sky;
    case GRADIENT_SKY_COLOR:
      return &EnvironmentGradientColors::sky;
    default:
      NOTREACHED();
  }
  return &EnvironmentGradientColors::ground;
...
SkColor GetGradientColor(const EnvironmentGradientColors& colors,
                         TargetProperty p) {
  return colors.*GetGradientColorPtr(p);
}

void SetGradientColor(EnvironmentGradientColors* colors,
                      TargetProperty p,
                      SkColor c) {
  colors->*GetGradientColorPtr(p) = c;
}

}  // namespace

Sky::Sky() {
  std::set<TargetProperty> transitioned_properties;
  for (TargetProperty i = FIRST_GRADIENT_COLOR; i <= LAST_GRADIENT_COLOR;
       i = NextTargetProperty(i)) {
    transitioned_properties.insert(i);
  }
  SetTransitionedProperties(transitioned_properties);
...
void Sky::SetColors(const EnvironmentGradientColors& colors) {
  for (TargetProperty i = FIRST_GRADIENT_COLOR; i <= LAST_GRADIENT_COLOR;
       i = NextTargetProperty(i)) {
    animation_player().TransitionColorTo(last_frame_time(), i,
                                         GetGradientColor(colors_, i),
                                         GetGradientColor(colors, i));
  }
}
...
void Sky::NotifyClientColorAnimated(SkColor color,
  ...
  if (target_property_id >= FIRST_GRADIENT_COLOR &&
      target_property_id <= LAST_GRADIENT_COLOR) {
    SetGradientColor(&colors_, static_cast<TargetProperty>(target_property_id),
                     color);
  } else {
...
void Sky::Renderer::CreateBuffers() {
  ...
  size_t current_vertex = 0lu;
  // Create special top / bottom vertices.
  AddVertex(&current_vertex, {0.0f, -kSkyDistance, 0.0f}, *curve, 0.0f);
  AddVertex(&current_vertex, {0.0f, kSkyDistance, 0.0f}, *curve, 1.0f);
  ...
  for (size_t latitude = 1; latitude + 1 < kLinesLatitude; ++latitude) {
    for (size_t longitude = 0; longitude < kLinesLongitude; ++longitude) {
      ...
      AddVertex(&current_vertex, p, *curve, t);
    }
  }
