...
TEST(TransientElementTest, Visibility) {
  ...
  element.SetVisible(true);
  element.Animate(MsToTicks(10));
  EXPECT_EQ(element.opacity_when_visible(), element.opacity());
  element.Animate(MsToTicks(2010));
  EXPECT_EQ(0.0f, element.opacity());
}

// Test that refershing the visibility resets the transience timeout if the
...
TEST(TransientElementTest, RefreshVisibility) {
  ...
  element.SetVisible(true);
  EXPECT_EQ(element.opacity_when_visible(), element.opacity());
  element.Animate(MsToTicks(1000));

  // Refresh visibility, and ensure that the element transiently disappears.
  element.RefreshVisible();
  element.Animate(MsToTicks(2000));
  EXPECT_EQ(element.opacity_when_visible(), element.opacity());
  element.Animate(MsToTicks(3000));
  EXPECT_EQ(0.0f, element.opacity());
  ...
  EXPECT_EQ(element.opacity_when_visible(), element.opacity());
  element.RefreshVisible();
  element.Animate(MsToTicks(4000));
  EXPECT_EQ(element.opacity_when_visible(), element.opacity());
  element.SetVisible(false);
  EXPECT_EQ(0.0f, element.opacity());
}
