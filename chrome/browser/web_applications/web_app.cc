...
// For |reply| to be a OnceClosure, extensions::ImageLoaderImageCallback must
// first be converted.
void ShortcutInfo::PostIOTaskAndReply(
...
#if defined(OS_WIN)
  // On Windows, shortcut tasks need to run on a thread that has been COM-
  // Initialized.
  scoped_refptr<base::TaskRunner> task_runner =
      base::TaskScheduler::GetInstance()->CreateCOMSTATaskRunnerWithTraits(
          traits);
#else
  scoped_refptr<base::TaskRunner> task_runner =
      base::TaskScheduler::GetInstance()->CreateTaskRunnerWithTraits(traits);
#endif
  ...
  task_runner->PostTaskAndReply(
      ...
      base::BindOnce(&DeleteShortcutInfoOnUIThread,
                     base::Passed(&shortcut_info), reply));
}
