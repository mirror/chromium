...
void TabSpecificContentSettings::DidUpdateAudioState(bool is_audible) {
...
void TabSpecificContentSettings::OnSoundContentSettingUpdated() {
  ...
  if (web_contents()->IsCurrentlyAudible() &&
      GetSoundContentSetting() == CONTENT_SETTING_BLOCK) {
    OnContentBlocked(CONTENT_SETTINGS_TYPE_SOUND);
  }
}

ContentSetting TabSpecificContentSettings::GetSoundContentSetting() {
  Profile* profile =
      Profile::FromBrowserContext(web_contents()->GetBrowserContext());
  const HostContentSettingsMap* map =
      HostContentSettingsMapFactory::GetForProfile(profile);
