...
TEST_F(ChromeLoggingTest, SetUpSymlink) {
  ...
  base::FilePath bare_symlink_path =
      base::FilePath(FILE_PATH_LITERAL("chrome-test-log"));
  ...
  base::DeleteFile(bare_symlink_path, recursive);
  base::DeleteFile(latest_symlink_path, recursive);
  base::DeleteFile(previous_symlink_path, recursive);
  ...
  logging::SetUpSymlinkIfNeeded(latest_symlink_path, start_new_log);
