...
class ChromeLoggingTest : public testing::Test {
 public:
  ...
  void SetUpSymlinkCleanUp() {
    base::DeleteFile(bare_symlink_path_, /*recursive=*/false);
    base::DeleteFile(latest_symlink_path_, /*recursive=*/false);
    base::DeleteFile(previous_symlink_path_, /*recursive=*/false);
  }

  const base::CommandLine& cmd_line() { return cmd_line_; }

  base::FilePath bare_symlink_path_;
  base::FilePath latest_symlink_path_;
  base::FilePath previous_symlink_path_;
...
TEST_F(ChromeLoggingTest, SetUpSymlink) {
  bare_symlink_path_ = base::FilePath(FILE_PATH_LITERAL("chrome-test-log"));
  latest_symlink_path_ =
      base::FilePath(FILE_PATH_LITERAL("chrome-test-log.LATEST"));
  previous_symlink_path_ =
      base::FilePath(FILE_PATH_LITERAL("chrome-test-log.PREVIOUS"));
  ...
  base::FilePath old_target_path = logging::GenerateTimestampedName(
      bare_symlink_path_, base::Time::UnixEpoch());

  ASSERT_TRUE(base::CreateSymbolicLink(old_target_path, bare_symlink_path_));

  // Call the testee with the new symlink path, as if starting a new session.
  logging::SetUpSymlinkIfNeeded(latest_symlink_path_,
                                /*start_new_log=*/true);
  ...
  ASSERT_TRUE(base::ReadSymbolicLink(bare_symlink_path_, &target_path));
  EXPECT_EQ(target_path.value(), latest_symlink_path_.value());

  base::FilePath latest_target_path;
  ASSERT_TRUE(
      base::ReadSymbolicLink(latest_symlink_path_, &latest_target_path));
  ...
  logging::SetUpSymlinkIfNeeded(latest_symlink_path_, /*start_new_log=*/true);
  ...
  ASSERT_TRUE(base::ReadSymbolicLink(previous_symlink_path_, &target_path));
