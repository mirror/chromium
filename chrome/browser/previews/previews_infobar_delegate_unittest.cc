...
class TestPreviewsLogger : public previews::PreviewsLogger {
 public:
  ...
  void LogMessage(const std::string& event_type,
    ...
    log_type_ = event_type;
    log_description_ = event_description;
  }

  // Exposed passed in params of LogMessage for testing.
  std::string log_type() const { return log_type_; }
  std::string log_description() const { return log_description_; }
 ...
 protected:
  ...
  void SetUp() override {
    ...
    previews_ui_service_ = base::MakeUnique<previews::PreviewsUIService>(
        &io_data, nullptr /* io_task_runner */,
        nullptr /* previews_opt_out_store */,
        base::Bind(&MockedPreviewsIsEnabled), std::move(previews_logger));
  }
...
TEST_F(PreviewsInfoBarDelegateUnitTest, CreateInfoBarLogPreviewsInfoBarType) {
  previews::PreviewsType expected_type = previews::PreviewsType::LOFI;
  std::string expected_event = "InfoBar";
  ...
  CreateInfoBar(expected_type, base::Time(), false /* is_data_saver_user */,
                false /* is_reload */);
  EXPECT_EQ(expected_event, previews_logger()->log_type());
  EXPECT_EQ(expected_description, previews_logger()->log_description());
}
