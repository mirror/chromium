...
BytesTransferredKey KeyForRequest(const net::URLRequest& request) {
  ...
  // Requests from PPAPI instances are proxied through the renderer,
  // and specify the plugin_child_id of the plugin process.
  // plugin process.
  if (info->GetPluginChildID() != 0)
    return {info->GetPluginChildID(), 0};

  // Other requests are associated with the child process (and frame,
  // if originating from a renderer process).
