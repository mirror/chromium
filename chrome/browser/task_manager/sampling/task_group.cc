...
void TaskGroup::OnSamplerRefreshDone(
  ...
  if (results) {
    start_time_ = results.value().start_time;
    cpu_time_ = results.value().cpu_time;
    idle_wakeups_per_second_ = results.value().idle_wakeups_per_second;
#if defined(OS_WIN)
    memory_usage_.physical_bytes = results.value().physical_bytes;
#endif
  } else {
