...
class SharedSamplerTest : public testing::Test {
 ...
 private:
  ...
  void OnPhysicalMemoryUsageRefreshDone(int64_t physical_bytes) {
    ...
    OnRefreshTypeFinished(REFRESH_TYPE_MEMORY_DETAILS);
  }
...
TEST_F(SharedSamplerTest, PhysicalMemory) {
  StartRefresh(REFRESH_TYPE_MEMORY_DETAILS);
  WaitUntilRefreshDone();
  EXPECT_EQ(REFRESH_TYPE_MEMORY_DETAILS, finished_refresh_type());
...
TEST_F(SharedSamplerTest, MultipleRefreshTypes) {
  StartRefresh(REFRESH_TYPE_IDLE_WAKEUPS | REFRESH_TYPE_MEMORY_DETAILS |
               REFRESH_TYPE_START_TIME | REFRESH_TYPE_CPU_TIME);
  WaitUntilRefreshDone();
  EXPECT_EQ(REFRESH_TYPE_IDLE_WAKEUPS | REFRESH_TYPE_MEMORY_DETAILS |
                REFRESH_TYPE_START_TIME | REFRESH_TYPE_CPU_TIME,
