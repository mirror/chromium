...
// Given a vector of TaskGroups that may contain duplicates, delete every unique
// TaskGroup, and clear the vector.
void DeleteAllTaskGroups(std::vector<TaskGroup*>* container) {
  // Use std::sort/std::unique to filter out duplicate pointers.
...
TaskManagerImpl::ProcessSortKey TaskManagerImpl::MakeSortKey(Task* task) const {
  ...
  if (task->HasParentTask()) {
    ...
    ProcessSortKey base_key = parent_group->sort_key();
