...
// Computes the sort key for a Task.
TaskManagerImpl::ProcessSortKey TaskManagerImpl::MakeSortKey(Task* task) const {
  if (task->HasParentTask()) {
    ...
    // parents.
    TaskGroup::SortKey base_key =
        GetTaskGroupByTaskId(task->GetParentTask()->task_id())->sort_key();

    // We'll use the task_id of the first task in the group as a tiebreaker.
    // This generally should result in children being placed after their
    // parents, since it's expected that parent tasks are created first, and the
    // sort order on IDs corresponds to the Task creation order.
    return std::make_tuple(std::get<0>(base_key), std::get<1>(base_key),
                           std::get<2>(base_key), task->GetTabId(),
                           task->GetType(), task->task_id());

  } else {
    // Generate group ID from task type itself. Sort by task type, then by tab
    // ID (which corresponds to tab creation order) and keeps cross-process
