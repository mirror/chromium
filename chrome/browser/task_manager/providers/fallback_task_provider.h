...
// This FallbackTaskProvider is created to manage 2 subproviders. The primary
// subprovider has precidence in showing its task in the task manager and the
// secondary subprovider task is only shown if there is a task from it that
// doesn't share a Pid with the primary subprovider. It is currently behind the
// command line flag --task-manager-show-extra-renderers since it is currently
// being used to show all the RenderProcessHosts as subprovider tasks.
class FallbackTaskProvider : public TaskProvider {
 ...
 private:
  ...
  // from the primary source can shadow the tasks from the secondary source.
