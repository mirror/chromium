...
void FallbackTaskProvider::OnTaskAddedBySource(Task* task,
  ...
  if (source == primary_source()) {
    Task* secondary_task =
        GetTaskByPidFromVector(task->process_id(), secondary_source()->tasks());
    if (secondary_task && base::ContainsValue(shown_tasks_, secondary_task)) {
...
void FallbackTaskProvider::OnTaskRemovedBySource(Task* task,
  ...
  // are any other primary tasks for that process. If not, but there is
  // a secondary task, show it.
  if (source == primary_source()) {
    Task* primary_task =
        GetTaskByPidFromVector(task->process_id(), primary_source()->tasks());
    Task* secondary_task =
        GetTaskByPidFromVector(task->process_id(), secondary_source()->tasks());
    if (secondary_task && !primary_task) {
