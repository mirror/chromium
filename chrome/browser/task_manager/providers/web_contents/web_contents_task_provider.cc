...
class WebContentsEntry : public content::WebContentsObserver {
 ...
 private:
  ...
  // Clears references to |render_frame_host| from our maps. If doing so results
  // in a RendererTask with no remaining associated RenderFrameHost, ownership
  // of that RendererTask passes to the caller, in which case it is the caller's
  // responsibility to notify the TaskProviderObserver of the removal.
  std::unique_ptr<RendererTask> ClearTaskForFrame(
      RenderFrameHost* render_frame_host) WARN_UNUSED_RESULT;

  // Notify our observer that a task was added, removed, or replaced. A
  // "TaskAdded" or "TaskRemoved" event occurs if one of |old_task| and
  // |new_task| is null. If both are non-null, a "TaskReplaced" event occurs.
  void NotifyObserver(std::unique_ptr<RendererTask> old_task,
...
void WebContentsEntry::ReplaceTaskForSiteInstance(
  ...
  // This function is called when the PID of |site_instance|'s RenderProcessHost
  // becomes available.
