...
TEST_F(ProfileAttributesStorageTest, GAIAPicture) {
  const size_t kAvatarIndex = 0U;
  const size_t kOtherAvatarIndex = 10U;
  ...
  ASSERT_EQ(kAvatarIndex, entry->GetAvatarIconIndex());
  ...
  int avatar_id = profiles::GetDefaultAvatarIconResourceIDAtIndex(kAvatarIndex);
  const gfx::Image& starting_avatar_image(
      ui::ResourceBundle::GetSharedInstance().GetImageNamed(avatar_id));
  EXPECT_TRUE(
