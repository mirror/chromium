...
class ProfileAttributesEntry {
 ...
 private:
  ...
  // Internal getters that returns basic data types. If the key is not present,
  // or if the data is in a wrong data type, returns empty string, 0, or false
  // depening on the target data type. We do not assume that the data type is
  ...
  // Internal setters that accept basic data types.
  void SetString(const char* key, std::string value);
  void SetString16(const char* key, base::string16 value);
