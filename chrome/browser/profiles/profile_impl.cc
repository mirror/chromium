...
PrefService* ProfileImpl::GetOffTheRecordPrefs() {
  ...
  // there's no OTR profile, in order to figure out what a pref value would have
  // been returned if an OTR profile existed. To support that case we return a
  // dummy PrefService here.
  //
  // TODO(crbug.com/734484): Don't call this method when there's no OTR profile
  // (and return null for such calls).
  if (!dummy_otr_prefs_) {
