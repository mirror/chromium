...
ProfileImpl::ProfileImpl(const base::FilePath& path,
  ...
  configuration_policy_provider_ =
      policy::UserPolicyManagerFactoryChromeOS::CreateForProfile(
          this, force_immediate_policy_load, task_runner_);
  ...
  supervised_user_settings->Init(path_, task_runner_.get(),
                                 create_mode == CREATE_MODE_SYNCHRONOUS);
#endif
...
void ProfileImpl::DoFinalInit() {
  ...
  CreateProfileDirectory(task_runner_.get(), base_cache_path_, false);
