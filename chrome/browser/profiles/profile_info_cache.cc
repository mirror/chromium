  ...
  if (!image) {
    ...
    if (!old_file_name.empty()) {
      ...
      base::CreateSequencedTaskRunnerWithTraits({base::MayBlock()})
          ->PostTask(FROM_HERE, base::BindOnce(&DeleteBitmap, image_path));
    }
  } else {
...
void ProfileInfoCache::DownloadHighResAvatarIfNeeded(
  ...
  base::Closure callback =
      base::Bind(&ProfileInfoCache::DownloadHighResAvatar,
                 ...
                 profile_path);
  base::CreateSequencedTaskRunnerWithTraits({base::MayBlock()})
      ->PostTask(FROM_HERE, base::BindOnce(&RunCallbackIfFileMissing, file_path,
                                           callback));
}
...
void ProfileInfoCache::SaveAvatarImageAtPath(
  ...
  if (data->empty()) {
    ...
    base::CreateSequencedTaskRunnerWithTraits({base::MayBlock()})
        ->PostTask(FROM_HERE, base::BindOnce(&SaveBitmap, base::Passed(&data),
                                             image_path, callback));
  }
}
