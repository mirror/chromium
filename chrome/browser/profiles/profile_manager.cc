...
void ProfileManager::InitProfileUserPrefs(Profile* profile) {
  ...
  if (user && user->GetType() == user_manager::USER_TYPE_CHILD) {
    ...
    LOG(ERROR) << "########## Child account found. profile->IsChild()="
               << profile->IsChild();
  } else {
    LOG(ERROR) << "########## Child account NOT found. profile->IsChild()="
               << profile->IsChild();
  }
#endif
...
#if defined(OS_CHROMEOS)
  if (profile->IsChild() && force_supervised_user_id) {
    LOG(FATAL) << "child=1 supervised=1";
  }
#endif
