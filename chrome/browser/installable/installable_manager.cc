...
// height and width >= GetMinimumPrimaryIconSizeInPx() (or size "any").
bool DoesManifestContainRequiredIcon(const content::Manifest& manifest) {
  for (const auto& icon : manifest.icons) {
    ...
    for (const auto& size : icon.sizes) {
      if (size.IsEmpty())  // "any"
        return true;
      if (size.width() >= GetMinimumPrimaryIconSizeInPx() &&
          size.height() >= GetMinimumPrimaryIconSizeInPx()) {
        return true;
      }
    }
  }
...
InstallableManager::InstallableManager(content::WebContents* web_contents)
    : content::WebContentsObserver(web_contents),
      ...
      ideal_primary_icon_size_in_px_(GetIdealPrimaryIconSizeInPx()),
      minimum_primary_icon_size_in_px_(GetMinimumPrimaryIconSizeInPx()),
      ideal_badge_icon_size_in_px_(GetIdealBadgeIconSizeInPx()),
      weak_factory_(this) {
...
int InstallableManager::GetMinimumIconSizeInPx() {
  return GetMinimumPrimaryIconSizeInPx();
}
...
void InstallableManager::WorkOnTask() {
  ...
  if (!manifest_->fetched) {
    FetchManifest();
             ...
             !IsIconFetched(IconPurpose::ANY)) {
    CheckAndFetchBestIcon(ideal_primary_icon_size_in_px_,
                          minimum_primary_icon_size_in_px_, IconPurpose::ANY);
    ...
    CheckAndFetchBestIcon(ideal_badge_icon_size_in_px_,
                          ideal_badge_icon_size_in_px_, IconPurpose::BADGE);
  } else {
    NOTREACHED();
  }
}
