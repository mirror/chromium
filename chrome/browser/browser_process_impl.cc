...
BrowserProcessImpl::BrowserProcessImpl(
    std::unique_ptr<local_state_loader::LocalState> local_state)
    : browser_policy_connector_(std::move(local_state->browser_policy_connector)),
      local_state_(std::move(local_state->local_state)),
      download_status_updater_(std::make_unique<DownloadStatusUpdater>()),
      local_state_task_runner_(local_state->local_state_task_runner),
      pref_service_factory_(std::move(local_state->pref_service_factory)) {
