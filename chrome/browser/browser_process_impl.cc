...
// whenever the preference or its controlling policy changes.
  ...
  DCHECK(!locale_.empty());  // SetApplicationLocale() must be called.
#if BUILDFLAG(ENABLE_EXTENSIONS)
  extension_l10n_util::SetProcessLocale(locale_);
#endif
  ChromeContentBrowserClient::SetApplicationLocale(locale_);
...
void BrowserProcessImpl::SetApplicationLocale(const std::string& locale) {
  // NOTE: this is called before any threads have been created.
  locale_ = locale;
}
