...
void ClipboardReadPermissionContext::UpdateTabContext(
    ...
    bool allowed) {
  // Show location bar indicator only when features::kClipboardContentSetting
  // feature is enabled.
  ...
  if (allowed) {
    content_settings->OnClipboardReadAccessed(requesting_frame);
  } else {
    content_settings->OnClipboardReadAccessBlocked(requesting_frame);
  }
}
