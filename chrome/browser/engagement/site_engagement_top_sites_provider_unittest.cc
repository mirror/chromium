...
class TopSitesQuerier {
 public:
  TopSitesQuerier(std::unique_ptr<SiteEngagementTopSitesProvider> provider,
                  int expected_calls)
  ...
  void QueryTopSites(int result_count) {
    provider_->ProvideTopSites(
        result_count,
        base::Bind(&TopSitesQuerier::OnProvideTopSitesComplete,
...
TEST_F(SiteEngagementTopSitesProviderTest, EmptyList) {
  ...
  // URLs.
  TopSitesQuerier querier(CreateProvider(), 1);
  querier.QueryTopSites(10);
  querier.Wait();

  EXPECT_TRUE(querier.urls().empty());
  EXPECT_TRUE(querier.titles().empty());
  EXPECT_TRUE(querier.redirects().empty());
}
...
TEST_F(SiteEngagementTopSitesProviderTest, NoDataInHistory) {
  ...
  TopSitesQuerier querier(CreateProvider(), 1);
  querier.QueryTopSites(1);
  querier.Wait();

  EXPECT_EQ(std::vector<GURL>{url}, querier.urls());
  EXPECT_EQ(std::vector<base::string16>{base::string16()}, querier.titles());
...
TEST_F(SiteEngagementTopSitesProviderTest, TitleAndRedirectsInHistory) {
  ...
  EXPECT_EQ(std::vector<GURL>{url1}, querier.urls());
...
TEST_F(SiteEngagementTopSitesProviderTest, MaxRequestedIsRespected) {
  ...
  TopSitesQuerier querier(CreateProvider(), 1);
  querier.QueryTopSites(2);
  querier.Wait();

  EXPECT_EQ((std::vector<GURL>{url3, url1}), querier.urls());
  EXPECT_EQ((std::vector<base::string16>{base::UTF8ToUTF16("Another title"),
...
TEST_F(SiteEngagementTopSitesProviderTest, OverlappingRedirects) {
  ...
  TopSitesQuerier querier(CreateProvider(), 1);
  querier.QueryTopSites(3);
  querier.Wait();

  EXPECT_EQ((std::vector<GURL>{url1, url3, url2}), querier.urls());
  EXPECT_EQ((std::vector<base::string16>{base::UTF8ToUTF16("Another title"),
...
TEST_F(SiteEngagementTopSitesProviderTest, OverlappingRequests) {
  ...
  TopSitesQuerier querier(CreateProvider(), 3);
  querier.QueryTopSites(1);
  querier.QueryTopSites(2);
  querier.QueryTopSites(3);
  querier.Wait();
  ...
  EXPECT_EQ(6u, querier.urls().size());
  EXPECT_EQ(6u, querier.titles().size());
  EXPECT_EQ(6u, querier.redirects().size());

  EXPECT_EQ(3, std::count(querier.urls().begin(), querier.urls().end(), url1));
  EXPECT_EQ(2, std::count(querier.urls().begin(), querier.urls().end(), url2));
  EXPECT_EQ(1, std::count(querier.urls().begin(), querier.urls().end(), url3));

  EXPECT_EQ(3, std::count(querier.titles().begin(), querier.titles().end(),
                          base::UTF8ToUTF16("Title")));
  EXPECT_EQ(2, std::count(querier.titles().begin(), querier.titles().end(),
                          base::UTF8ToUTF16("Other title")));
  EXPECT_EQ(1, std::count(querier.titles().begin(), querier.titles().end(),
                          base::UTF8ToUTF16("Another title")));

  EXPECT_EQ(3,
            std::count(querier.redirects().begin(), querier.redirects().end(),
                       history::RedirectList{url1}));
  EXPECT_EQ(2,
            std::count(querier.redirects().begin(), querier.redirects().end(),
