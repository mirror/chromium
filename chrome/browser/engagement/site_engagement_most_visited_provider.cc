...
#include "chrome/browser/engagement/site_engagement_most_visited_provider.h"
...
SiteEngagementMostVisitedProvider::SiteEngagementMostVisitedProvider(
    Profile* profile)
...
void SiteEngagementMostVisitedProvider::QueryMostVisitedURLs(
...
void SiteEngagementMostVisitedProvider::FillMostVisitedURLDetails(
    history::HistoryService* history_service,
  ...
  for (auto& mv : *urls) {
    history_service->QueryURL(
        mv.url, false,
        base::Bind(&SiteEngagementMostVisitedProvider::OnURLQueried,
                   weak_ptr_factory_.GetWeakPtr(), base::Unretained(urls), i,
                   callback),
        tracker);
    history_service->QueryRedirectsFrom(
        mv.url,
        base::Bind(&SiteEngagementMostVisitedProvider::OnRedirectsQueried,
                   weak_ptr_factory_.GetWeakPtr(), base::Unretained(urls), i,
                   callback),
        tracker);
    ++i;
...
void SiteEngagementMostVisitedProvider::OnURLQueried(
    history::MostVisitedURLList* urls,
    size_t index,
    const base::Closure& callback,
    bool success,
...
void SiteEngagementMostVisitedProvider::OnRedirectsQueried(
    history::MostVisitedURLList* urls,
    size_t index,
    const base::Closure& callback,
