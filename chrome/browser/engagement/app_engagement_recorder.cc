...
void AppEngagementRecorder::OnEngagementIncreased(
  ...
  if (!web_contents) {
    ...
    DVLOG(1)
        << "AppEngagementRecorder::OnEngagementIncreased: No WebContents (\""
        << url << "\")";
    return;
  }
  ...
  DVLOG(1) << "AppEngagementRecorder::OnEngagementIncreased(<WebContents>, \""
           << url << "\", " << score << ", type=" << type
           << ") <-- IN A PWA WINDOW";
