...
class SiteEngagementTopSitesProvider::HistoryQuery {
 public:
  HistoryQuery(SiteEngagementTopSitesProvider* provider,
  ...
  void FillMostVisitedURLDetails(history::MostVisitedURLList* urls,
    ...
    pending_history_queries_ = urls->size() * 2;

    for (history::MostVisitedURL& mv : *urls) {
  ...
  void RunClosure() { std::move(closure_).Run(); }
...
void SiteEngagementTopSitesProvider::ProvideTopSites(
  ...
  auto* urls = new history::MostVisitedURLList();
  ...
  requests_.push_back(std::make_unique<HistoryQuery>(
      this, history_service_, base::BindOnce(callback, std::move(urls))));
  requests_.back()->FillMostVisitedURLDetails(urls, tracker);
}
