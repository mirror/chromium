...
class SiteEngagementService : public KeyedService,
 ...
 public:
  ...
  // clock_->Now(). |web_contents| is the web contents that was just launched.
 ...
 private:
  ...
  // Calls OnEngagementEvent in all observers. |web_contents| may be null if the
  // engagement has increased when |url| is not in a tab, e.g. from a
