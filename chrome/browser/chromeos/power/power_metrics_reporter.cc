...
void PowerMetricsReporter::ReportDailyMetrics(
  ...
  if (type == metrics::DailyEvent::IntervalType::DAY_ELAPSED) {
    for (size_t i = 0; i < arraysize(kDailyCounts); ++i) {
      // This matches UMA_HISTOGRAM_COUNTS_100, which can't be used here since
      // it requires compile-time-constant histogram names.
      base::Histogram::FactoryGet(
          kDailyCounts[i].metric_name, 1, 100, 50,
          base::HistogramBase::kUmaTargetedHistogramFlag)
          ->Add(daily_counts_[kDailyCounts[i].pref_name]);
    }
  }
