...
TEST_F(UserActivityLoggerDelegateUkmTest, Basic) {
  ...
  EXPECT_TRUE(
      ukm_entry_checker_.WasNewEntryRecorded(kUserActivityEntryName, 1));
  EXPECT_TRUE(
      ukm_entry_checker_.WasNewEntryRecorded(kUserActivityIdEntryName, 2));

  ukm_entry_checker_.ExpectNewEntry(kUserActivityEntryName, GURL(""),
                                    user_activity_values_);

  const ukm::mojom::UkmEntry* last_activity_entry =
      ukm_entry_checker_.LastUkmEntry(kUserActivityEntryName);

  // Explicitly check kTimeSinceLastKeyName is not recorded.
  ASSERT_TRUE(nullptr ==
              ukm::TestUkmRecorder::FindMetric(
                  last_activity_entry, UserActivity::kTimeSinceLastKeyName));

  const ukm::SourceId source_id = last_activity_entry->source_id;

  const UkmMetricMap user_activity_id_value({
      {UserActivityId::kActivityIdName, source_id},
  });

  const std::map<ukm::SourceId, std::pair<GURL, UkmMetricMap>> expected_data({
      {source_id0, std::make_pair(url0_, user_activity_id_value)},
      {source_id1, std::make_pair(url1_, user_activity_id_value)},
  });

  ukm_entry_checker_.ExpectNewEntries(kUserActivityIdEntryName, expected_data);
...
TEST_F(UserActivityLoggerDelegateUkmTest, NoOpenTabs) {
  ...
  EXPECT_TRUE(
      ukm_entry_checker_.WasNewEntryRecorded(kUserActivityEntryName, 1));
  EXPECT_TRUE(
      ukm_entry_checker_.WasNewEntryRecorded(kUserActivityIdEntryName, 0));
