...
class UserActivityLoggerDelegateUkmTest : public TabActivityTestBase {
 protected:
  UserActivityLoggerDelegateUkmTest() {
    ...
    features->set_last_activity_time_sec(720);
    features->set_last_user_activity_time_sec(360);
    ...
    features->set_battery_percent(95.0);
  }
...
TEST_F(UserActivityLoggerDelegateUkmTest, Basic) {
  ...
  const GURL url0(kTestUrls[0]);
  content::WebContents* fg_contents =
      AddWebContentsAndNavigate(tab_strip_model, url0);
  tab_strip_model->ActivateTabAt(0, false);
  WebContentsTester::For(fg_contents)->TestSetIsLoading(false);
  const ukm::SourceId source_id0 = ukm_entry_checker()->GetSoureIdForUrl(url0);

  const GURL url1(kTestUrls[1]);
  ...
  const ukm::SourceId source_id1 = ukm_entry_checker()->GetSoureIdForUrl(url1);
  ...
  const std::map<ukm::SourceId, std::pair<GURL, UkmMetricMap>> expected_data({
      {source_id0, std::make_pair(url0, kUseActivityIdValues)},
      {source_id1, std::make_pair(url1, kUseActivityIdValues)},
  });
