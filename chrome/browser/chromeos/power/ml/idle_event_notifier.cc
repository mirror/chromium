...
IdleEventNotifier::IdleEventNotifier(const base::TimeDelta& idle_delay,
  ...
  chromeos::DBusThreadManager::Get()->GetPowerManagerClient()->AddObserver(
      this);
  // This requires ui::UserActivityDetector to be running!
  ui::UserActivityDetector::Get()->AddObserver(this);
  // If video detector is not supplied, we default to video not playing.
  if (!video_detector_) {
    LOG(WARNING)
...
void IdleEventNotifier::OnVideoStateChanged(ash::VideoDetector::State state) {
  if (video_state_ != ash::VideoDetector::State::NOT_PLAYING ||
      video_state_ != state) {
    // Update last activity time if video is playing or if video just stopped.
    // In the latter case, we consider the current time to be the last time when
    // video is played.
    base::Time now = clock_->Now();
...
void IdleEventNotifier::ResetIdleDelayTimer() {
  // Do not schedule the timer if video is still playing!
  if (video_state_ != ash::VideoDetector::State::NOT_PLAYING)
    return;
