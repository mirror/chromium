...
class PowerMetricsReporterTest : public testing::Test {
 ...
 protected:
  // Reinitialize |reporter_|. May be called by tests to simulate a Chrome
  // restart.
  void ResetReporter() {
  ...
  // Instructs |reporter_| to report daily metrics and verifies that it reports
  // one sample for each of |dim_count|, |off_count|, and |suspend_count|.
  void TriggerDailyEventAndVerifyHistograms(int dim_count,
                                            int off_count,
                                            int suspend_count) {
    base::HistogramTester histogram_tester;
    reporter_->ReportDailyMetrics();
