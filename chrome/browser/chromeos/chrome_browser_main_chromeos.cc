...
// Helper class for creating an Ash Shell in Mus and classic mode.
// NOTE: In mash, this is not used! ash::Shell is created in
...
void ChromeBrowserMainPartsChromeos::PreProfileInit() {
  ...
  // The virtual keyboard needs to be initialized before the profile is
  // initialized, otherwise the virtual keyboard extension will not load in the
  // login screen.
...
void ChromeBrowserMainPartsChromeos::PostProfileInit() {
  ...
  if (!ash_util::IsRunningInMash()) {
    // Activate the virtual keyboard after the profile is initialized since it
    // depends on the default profile.
