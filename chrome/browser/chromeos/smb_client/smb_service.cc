...
SmbService::SmbService(Profile* profile) : profile_(profile) {
  GetFspService()->RegisterFileSystemFactory(kSmbProviderId,
                                             base::Bind(&CreateSmbFileSystem));
}
...
base::File::Error SmbService::Mount(
  ...
  return GetFspService()->MountFileSystem(kSmbProviderId, options);
}

Service* SmbService::GetFspService() const {
