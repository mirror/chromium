...
int32_t SmbFileSystem::GetMountId() const {
  int32_t mount_id;
  DCHECK(base::StringToInt(file_system_info_.file_system_id(), &mount_id));
  return mount_id;
}
...
AbortCallback SmbFileSystem::RequestUnmount(
  ...
  chromeos::DBusThreadManager::Get()->GetSmbProviderClient()->Unmount(
...
void SmbFileSystem::HandleRequestUnmountCallback(
  ...
  base::File::Error error = TranslateError(smb_error);
