...
void SmbFileSystem::Abort() {
  // TODO(zentaro): To implement Abort() correctly will require storing a
  // request id unique to each method call. For now just provide this
  // no-op implementation.
  LOG(WARNING) << "Ignoring abort request. Abort is not implemented yet.";
}

AbortCallback CreateAbortCallback() {
  // TODO(zentaro): Add a request id parameter and bind it to the abort method.
  return base::BindOnce(&SmbFileSystem::Abort, weak_ptr_factory_.GetWeakPtr());
}
