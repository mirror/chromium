...
class NoteTakingHelper : public arc::ArcIntentHelperObserver,
 ...
 private:
  // The state app ID whitelist cache (used for determining the state of
  // note-taking apps whtielisted for the lock screen).
  enum class AppWhitelistState {
    // The whitelist state has not yet been determined.
    kUndetermined,
    // The profile does not have note taking app whitelist for lock screen.
    kNoAppWhitelist,
    // The profile has a note taking app whitelist for lock screen.
    kAppsWhitelisted
  };
  ...
  // |extension_prefs| - The extension prefs that should be used to look up the
  //     user choice indicating whether an app that supports lock screen note
  //     taking should be enabled on the lock screen.
  ...
  // The current LockScreenAppWhitelistState for
