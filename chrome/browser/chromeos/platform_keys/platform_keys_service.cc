 ...
 private:
  ...
  void GotKeyLocation(const platform_keys::KeyToTokenIdMap& key_to_token_id_map,
    ...
    if (key_token_id_iter == key_to_token_id_map.end()) {
      // This means that the key was not found or not on a token the user has
      // access to.
      // We also end up in this branch when the user has no access to the system
      // token and the key is in the system token.
      next_step_ = Step::DONE;
