  ...
  void TrySignUsingCertificateProvider() {
    ...
    if (!MapHashAlgorithmForCertificateProvider(hash_algorithm_, &hash, &alg)) {
      callback_.Run(std::string(),
                    std::string(platform_keys::kErrorHashNotSupported));
      return;
    }
...
class PlatformKeysService::CertificateSupportsHashTask : public Task {
 ...
 private:
  ...
  // Updates the permissions for |public_key_|, starts the actual signing
  // operation and afterwards passes the signature (or error) to |callback_|.
  void CheckHashSupported() {
    platform_keys::PlatformClientCertificateExists(
        token_id_, public_key_,
        base::Bind(&CertificateSupportsHashTask::OnPlatformCertificateExists,
                   weak_factory_.GetWeakPtr()),
        service_->browser_context_);
  }

  void OnPlatformCertificateExists(bool platform_certificate_found) {
    if (platform_certificate_found) {
      // Currently, platform certificates support all hashes.
      callback_.Run(true, std::string());
      DoStep();
    } else {
      // For certificates provided through CertificateProviderService, we have
      // to check explicitly.
      net::SSLPrivateKey::Hash hash = net::SSLPrivateKey::Hash::SHA1;
      SECOidTag alg = SEC_OID_SHA1;
      if (!MapHashAlgorithmForCertificateProvider(hash_algorithm_, &hash,
                                                  &alg)) {
      ...
      bool hash_supported =
          std::find(supported_hashes.begin(), supported_hashes.end(), hash) !=
          supported_hashes.end();
      callback_.Run(hash_supported, std::string());
      DoStep();
      return;
    }
  }
