...
class PlatformKeysService::SignTask : public Task {
 ...
 private:
  ...
  void DidSign(const std::string& signature, const std::string& error_message) {
    if (error_message == platform_keys::kErrorKeyNotFound) {
  ...
  void TrySignUsingCertificateProvider() {
    ...
    std::string digest;
    unsigned int hash_length = HASH_ResultLenByOidTag(alg);
    if (!hash_length) {
      callback_.Run(std::string(), platform_keys::kErrorInternal);
      return;
    }
    // passing length + 1 because WriteInto expects the length to include the
    // zero terminator.
    unsigned char* out = reinterpret_cast<unsigned char*>(
        base::WriteInto(&digest, hash_length + 1));
    SECStatus result = PK11_HashBuf(
        alg, out, reinterpret_cast<const unsigned char*>(data_.c_str()),
        data_.size());
    if (result != SECSuccess) {
      callback_.Run(std::string(), platform_keys::kErrorHashFailed);
      return;
    }
