...
class KeyPermissions {
 public:
  ...
  class KeyId {
   public:
    KeyId();
    KeyId(const std::string& public_key_spki_der,
          const std::vector<KeyLocation>& key_locations);
    KeyId(const KeyId& other);
    ~KeyId();

    KeyId& operator=(const KeyId& other);
   ...
   private:
    ...
    // The locations the key is stored in. Keys can be stored on multiple slots
    // at the same time.
    std::vector<KeyLocation> key_locations_;
  };
  ...
  class PermissionsForExtension {
   public:
    ...
    // Returns true if the private key matching |key_id| can be used for signing
    // by the extension with id |extension_id_|.
    bool CanUseKeyForSigning(const KeyId& key_id);

    // Registers the key |key_id| as being generated by the extension with id
    // |extension_id| and marks it for corporate usage.
    void RegisterKeyForCorporateUsage(const KeyId& key_id);

    // Sets the user granted permission that the extension with id
    // |extension_id| can use the private key matching |key_id| for signing.
    void SetUserGrantedPermission(const KeyId& key_id);

    // Must be called when the extension with id |extension_id| used the private
    // key matching |key_id| for signing.  Updates the permissions accordingly.
  ...
  // Returns true if the user can grant any permission for |key_id| to
  // extensions.
