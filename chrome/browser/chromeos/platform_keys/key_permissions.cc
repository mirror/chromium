...
bool IsKeyOnUserSlot(const KeyPermissions::KeyId& key_id) {
  for (KeyPermissions::KeyLocation key_location : key_id.key_locations()) {
    if (key_location == KeyPermissions::KeyLocation::kUserSlot)
      return true;
  }
  return false;
}

bool HasValidKeyLocation(const KeyPermissions::KeyId& key_id) {
  return !key_id.key_locations().empty();
}

}  // namespace

KeyPermissions::KeyId::KeyId() {}

KeyPermissions::KeyId::KeyId(const std::string& public_key_spki_der,
                             const std::vector<KeyLocation>& key_locations)
    : public_key_spki_der_(public_key_spki_der),
      key_locations_(key_locations) {}

KeyPermissions::KeyId::KeyId(const KeyId& other)
    : public_key_spki_der_(other.public_key_spki_der_),
      key_locations_(other.key_locations_) {}

KeyPermissions::KeyId::~KeyId() {}

KeyPermissions::KeyId& KeyPermissions::KeyId::operator=(
    const KeyPermissions::KeyId& other) {
  public_key_spki_der_ = other.public_key_spki_der_;
  key_locations_ = other.key_locations_;
  return *this;
}
...
bool KeyPermissions::CanUserGrantPermissionFor(const KeyId& key_id) const {
  ...
  return !IsCorporateKey(public_key_spki_der_b64, key_id.key_locations());
}
...
bool KeyPermissions::IsCorporateKey(
  ...
  for (const KeyLocation key_location : key_locations) {
    if (key_location == KeyLocation::kUserSlot &&
        IsCorporateKeyForProfile(public_key_spki_der_b64, profile_prefs_)) {
      return true;
    }
    if (key_location == KeyLocation::kSystemSlot)
      return true;
  }
  return false;
}
