...
class PlatformKeysService : public KeyedService {
 public:
  ...
  // where k is the key size in octets. If the extension does not have
  // permissions for signing with this key, the operation aborts. In case of a
  // one time permission (granted after generating the key), this function also
  // removes the permission to prevent future signing attempts. |callback| will
  // be invoked with the signature or an error message. Will only call back
