...
// - |token_ids| is not empty. In this case, |token_ids| has been filled with
// the identifiers of the tokens the private key was found on and the user has
// access to. Currently, valid token identifiers are |kTokenIdUser| and
// |kTokenIdSystem|. - |token_ids| is empty.
//   This means that the private key as not been found on any token the user has
...
// |public_key| is stored.. |callback| will be invoked when the token ids are
// determined, possibly with an error message. Must be called and calls
// |callback| on the UI thread.
void GetKeyLocations(const std::string& public_key,
