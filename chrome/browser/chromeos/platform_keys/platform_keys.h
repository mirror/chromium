...
// A mapping from public key (DER-encoded X.509 Subject Public Key Info of the
// key) to token id. A missing entry means that a corresponding private key has
// not been found or the calling profile has no access to the token it is stored
// on. See also comments on GetKeyLocations and GetKeyLocationsCallback.
...
// TODO(pmarko): This is a RepeatingCallback because of
