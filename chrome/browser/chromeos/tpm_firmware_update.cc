...
// available via  kTPMFirmwareUpdateLocationFile. There are three states:
...
class AvailabilityChecker {
 public:
  ...
  void Start(base::TimeDelta timeout, ResponseCallback callback) {
    Cancel();
 ...
 private:
  static base::FilePath GetUpdateLocationFile() {
    ...
    CHECK(PathService::Get(chrome::FILE_CHROME_OS_TPM_FIRMWARE_UPDATE,
...
void ShouldOfferUpdateViaPowerwash(
  ...
  AvailabilityChecker* checker_ptr = checker.get();
  checker_ptr->Start(
