...
class InputMethodEngineTest : public testing::Test {
 public:
  InputMethodEngineTest() : observer_(NULL), input_view_("inputview.html") {
    ...
    ui::IMEBridge::Get()->SetCandidateWindowVisibilityHandler(
        mock_ime_candidate_window_visibility_handler_.get());
  }

  ~InputMethodEngineTest() override {
    ui::IMEBridge::Get()->SetInputContextHandler(NULL);
    ui::IMEBridge::Get()->SetCandidateWindowVisibilityHandler(NULL);
    engine_.reset();
    Shutdown();
  }
