...
InputMethodManagerImpl::InputMethodManagerImpl(
  ...
  LOG(ERROR) << "JAMES new InputMethodManagerImpl";
...
ash::IMEInfo InputMethodManagerImpl::GetCurrentIme() {
  if (!state_)
    return ash::IMEInfo();
...
InputMethodManagerImpl::GetCurrentImeProperties() {
...
std::vector<ash::IMEInfo> InputMethodManagerImpl::GetAvailableImes() {
  if (!state_)
...
bool InputMethodManagerImpl::IsImeManaged() {
  if (!state_)
    return false;
