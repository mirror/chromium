...
policy::DeviceManagementService* DeviceManagementService() {
...
std::string GetDmServerUrl() {
  return DeviceManagementService()->GetServerUrl();
}
...
void ArcActiveDirectoryEnrollmentTokenFetcher::DoFetchEnrollmentToken() {
  ...
  policy::DeviceManagementService* service = DeviceManagementService();
...
void ArcActiveDirectoryEnrollmentTokenFetcher::InitiateSamlFlow(
  ...
  GURL redirect_url(auth_redirect_url);
  ...
  // AD auth page.
  support_host_->SetAdAuthUrls(redirect_url, GetDmServerUrl());
  support_host_->ShowAdAuth();
}
...
void ArcActiveDirectoryEnrollmentTokenFetcher::OnAuthFailed(
  ...
  VLOG(1) << "SAML auth failed: " << error_msg;
