...
class FakeArcSupportObserverBase : public FakeArcSupport::Observer {
 public:
  ...
  virtual void OnAuthPageShown(FakeArcSupport* fake_arc_support) {}

  void OnPageChanged(FakeArcSupport* fake_arc_support) override {
    ArcSupportHost::UIPage new_page = fake_arc_support->ui_page();
    if (new_page != ArcSupportHost::UIPage::ACTIVE_DIRECTORY_AUTH)
      return;
    EXPECT_EQ(kFakeAdfsServerUrl,
              fake_arc_support->get_active_directory_auth_federation_url());
    EXPECT_EQ(GetDmServerUrl(),
              fake_arc_support->get_active_directory_auth_dm_url_prefix());
    OnAuthPageShown(fake_arc_support);
  }
};
...
class SimulateAuthSucceedsObserver : public FakeArcSupportObserverBase {
 public:
  void OnAuthPageShown(FakeArcSupport* fake_arc_support) override {
    fake_arc_support->EmulateAuthSuccess("" /* auth_code unused */);
  }
};
...
class SimulateAuthCancelledObserver : public FakeArcSupportObserverBase {
 public:
  explicit SimulateAuthCancelledObserver(base::RunLoop* run_loop)
      : run_loop_(run_loop) {}
...
class SimulateAuthFailsObserver : public FakeArcSupportObserverBase {
 public:
  explicit SimulateAuthFailsObserver(base::RunLoop* run_loop)
      : run_loop_(run_loop) {}

  void OnAuthPageShown(FakeArcSupport* fake_arc_support) override {
    fake_arc_support->EmulateAuthFailure("error");
 ...
 public:
  void OnAuthPageShown(FakeArcSupport* fake_arc_support) override {
    ...
    if (saml_auth_count_ == 1) {
      ...
      fake_arc_support->EmulateAuthFailure("error");
      EXPECT_EQ(ArcSupportHost::UIPage::ERROR, fake_arc_support->ui_page());
...
class ArcActiveDirectoryEnrollmentTokenFetcherBrowserTest
 ...
 protected:
  ...
  void TearDownOnMainThread() override {
    ...
    fake_arc_support_->set_observer(nullptr);
  ...
  void FetchEnrollmentToken(Status* out_fetch_status,
                            std::string* out_enrollment_token,
                            std::string* out_user_id) {
    base::RunLoop run_loop;
    token_fetcher_->Fetch(
        base::Bind(&ArcActiveDirectoryEnrollmentTokenFetcherBrowserTest::
                   ...
                   base::Unretained(this), &run_loop, out_fetch_status,
                   out_enrollment_token, out_user_id));
  ...
  void ExpectEnrollmentTokenFetchFails(Status expected_status) {
    CHECK_NE(expected_status, Status::SUCCESS);
...
IN_PROC_BROWSER_TEST_F(ArcActiveDirectoryEnrollmentTokenFetcherBrowserTest,
  ...
  ExpectEnrollmentTokenFetchFails(Status::FAILURE);
}
...
IN_PROC_BROWSER_TEST_F(ArcActiveDirectoryEnrollmentTokenFetcherBrowserTest,
  ...
  ExpectEnrollmentTokenFetchFails(Status::ARC_DISABLED);
}
...
//      SimulateActiveDirectoryAuthSucceeds.
//   4) SimulateActiveDirectoryAuthSucceeds triggers the onAuthSucceeded event,
//   which causes
...
IN_PROC_BROWSER_TEST_F(ArcActiveDirectoryEnrollmentTokenFetcherBrowserTest,
  ...
  SimulateAuthSucceedsObserver observer;
  fake_arc_support_->set_observer(&observer);
  ExpectEnrollmentTokenFetchSucceeds();
}
...
IN_PROC_BROWSER_TEST_F(ArcActiveDirectoryEnrollmentTokenFetcherBrowserTest,
  ...
  interceptor_->PushJobCallback(base::Bind(&InitiateSamlResponseJob));
  base::RunLoop run_loop;
  SimulateAuthCancelledObserver observer(&run_loop);
  fake_arc_support_->set_observer(&observer);
...
IN_PROC_BROWSER_TEST_F(ArcActiveDirectoryEnrollmentTokenFetcherBrowserTest,
  ...
  interceptor_->PushJobCallback(base::Bind(&InitiateSamlResponseJob));
  base::RunLoop run_loop;
  SimulateAuthFailsObserver observer(&run_loop);
  fake_arc_support_->set_observer(&observer);
...
IN_PROC_BROWSER_TEST_F(ArcActiveDirectoryEnrollmentTokenFetcherBrowserTest,
  ...
  interceptor_->PushJobCallback(base::Bind(&InitiateSamlResponseJob));
  interceptor_->PushJobCallback(base::Bind(&FinishSamlResponseJob));
  SimulateAuthRetryObserver observer;
  fake_arc_support_->set_observer(&observer);
  ExpectEnrollmentTokenFetchSucceeds();
