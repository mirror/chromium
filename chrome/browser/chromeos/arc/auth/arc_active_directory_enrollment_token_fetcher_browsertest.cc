...
IN_PROC_BROWSER_TEST_F(ArcActiveDirectoryEnrollmentTokenFetcherBrowserTest,
  ...
  ExpectEnrollmentTokenFetchFails(
      ArcActiveDirectoryEnrollmentTokenFetcher::Status::FAILURE);
  EXPECT_EQ(static_cast<size_t>(0), dm_interceptor_->GetPendingSize());
  EXPECT_EQ(static_cast<size_t>(0), adfs_interceptor_->GetPendingSize());
...
// SAML flow fails since user closes the browser tab with the ADFS auth page.
IN_PROC_BROWSER_TEST_F(ArcActiveDirectoryEnrollmentTokenFetcherBrowserTest,
