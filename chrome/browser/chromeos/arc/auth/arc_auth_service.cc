...
class ArcAuthService::AccountInfoNotifier {
 public:
  ...
  explicit AccountInfoNotifier(
      const AccountInfoCallbackDeprecated& account_info_callback_deprecated)
      : callback_type_(CallbackType::ACCOUNT_INFO_DEPRECATED),
        account_info_callback_deprecated_(account_info_callback_deprecated) {}
  ...
  void NotifyFailure(mojom::ChromeAccountType account_type,
                     mojom::ArcSignInFailureReason failure_reason) {
    switch (callback_type_) {
      case CallbackType::AUTH_CODE:
        DCHECK(!auth_callback_.is_null());
        auth_callback_.Run(std::string() /* auth_code */,
                           true /* is_enforced */);
        break;
      case CallbackType::AUTH_CODE_AND_ACCOUNT:
        DCHECK(!auth_account_callback_.is_null());
        auth_account_callback_.Run(std::string() /* auth_code */,
                                   true /* is_enforced */, account_type);
        break;
      case CallbackType::ACCOUNT_INFO_DEPRECATED:
        DCHECK(!account_info_callback_deprecated_.is_null());
        account_info_callback_deprecated_.Run(nullptr);
        break;
      case CallbackType::ACCOUNT_INFO:
        DCHECK(!account_info_callback_.is_null());
        account_info_callback_.Run(nullptr, failure_reason);
        break;
    }
  }

  void NotifySuccess(bool is_enforced,
    ...
    switch (callback_type_) {
      ...
      case CallbackType::ACCOUNT_INFO_DEPRECATED:
        DCHECK(!account_info_callback_deprecated_.is_null());
        account_info_callback_deprecated_.Run(CreateAccountInfo(
            is_enforced, auth_info, account_name, account_type, is_managed));
        break;
      case CallbackType::ACCOUNT_INFO:
        DCHECK(!account_info_callback_.is_null());
        account_info_callback_.Run(
            CreateAccountInfo(is_enforced, auth_info, account_name,
                              account_type, is_managed),
 ...
 private:
  ...
  const AccountInfoCallbackDeprecated account_info_callback_deprecated_;
...
void ArcAuthService::RequestAccountInfo() {
  ...
  if (instance) {
    ...
    RequestAccountInfoInternal(
        base::MakeUnique<ArcAuthService::AccountInfoNotifier>(
            base::Bind(&ArcAuthService::OnAccountInfoReadyDeprecated,
                       weak_ptr_factory_.GetWeakPtr())));
  }
}
...
void ArcAuthService::OnAccountInfoReadyDeprecated(
  ...
  DCHECK(instance);
  instance->OnAccountInfoReadyDeprecated(std::move(account_info));
}

void ArcAuthService::OnAccountInfoReady(
    mojom::AccountInfoPtr account_info,
    mojom::ArcSignInFailureReason failure_reason) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  auto* instance = ARC_GET_INSTANCE_FOR_METHOD(arc_bridge_service_->auth(),
                                               OnAccountInfoReady);
  DCHECK(instance);
  instance->OnAccountInfoReady(std::move(account_info), failure_reason);
}
...
void ArcAuthService::RequestAccountInfoInternal(
  ...
  if (IsArcOptInVerificationDisabled()) {
    notifier->NotifySuccess(false /* = is_enforced */,
  ...
  switch (status) {
    case ArcActiveDirectoryEnrollmentTokenFetcher::Status::SUCCESS: {
      ...
      notifier_->NotifySuccess(
          true /*is_enforced*/, enrollment_token,
    ...
    case ArcActiveDirectoryEnrollmentTokenFetcher::Status::ARC_DISABLED: {
      ...
      notifier_->NotifyFailure(
          mojom::ChromeAccountType::ACTIVE_DIRECTORY_ACCOUNT,
          mojom::ArcSignInFailureReason::ARC_DISABLED);
      break;
    }
  }
  notifier_.reset();
