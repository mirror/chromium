...
class ArcActiveDirectoryAuthDefaultNegotiatorTest : public testing::Test {
 public:
  ArcActiveDirectoryAuthDefaultNegotiatorTest() = default;
  ~ArcActiveDirectoryAuthDefaultNegotiatorTest() override = default;
  ...
  void SetUp() override {
    ...
    negotiator_ = base::MakeUnique<ArcActiveDirectoryAuthDefaultNegotiator>(
        support_host());
  }
 ...
 private:
  ...
  std::unique_ptr<ArcActiveDirectoryAuthDefaultNegotiator> negotiator_;

  DISALLOW_COPY_AND_ASSIGN(ArcActiveDirectoryAuthDefaultNegotiatorTest);
};
...
TEST_F(ArcActiveDirectoryAuthDefaultNegotiatorTest, Accept) {
...
TEST_F(ArcActiveDirectoryAuthDefaultNegotiatorTest, DoesNotChangePrefs) {
  // Show Active Directory auth notification page.
  Status status = Status::PENDING;
  negotiator()->StartNegotiation(UpdateStatusCallback(&status));
...
TEST_F(ArcActiveDirectoryAuthDefaultNegotiatorTest, Cancel) {
  // Show Active Directory auth notification page.
  Status status = Status::PENDING;
  negotiator()->StartNegotiation(UpdateStatusCallback(&status));
...
TEST_F(ArcActiveDirectoryAuthDefaultNegotiatorTest, Retry) {
  // Show Active Directory auth notification page.
  Status status = Status::PENDING;
  negotiator()->StartNegotiation(UpdateStatusCallback(&status));
