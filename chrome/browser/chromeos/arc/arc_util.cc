...
bool IsArcAllowedForProfile(const Profile* profile) {
  ...
  std::map<base::FilePath, bool>::const_iterator it =
      g_profile_check_status.Get().find(path);
  ...
  if (first_check) {
    g_profile_check_status.Get()[path] = result;
    return result;
  } else {
...
void ResetArcAllowanceCheckForTesting(const Profile* profile) {
  ...
  std::map<base::FilePath, bool>::iterator it =
      g_profile_check_status.Get().find(profile->GetPath());
