...
std::map<std::string, bool> g_is_arc_migration_allowed;
...
bool IsArcMigrationAllowedByPolicyForProfile(const Profile* profile) {
  ...
  if (g_is_arc_migration_allowed.find(user_name) ==
      g_is_arc_migration_allowed.end()) {
    g_is_arc_migration_allowed[user_name] =
        IsArcMigrationAllowedInternal(profile);
  }

  return g_is_arc_migration_allowed[user_name];
}
