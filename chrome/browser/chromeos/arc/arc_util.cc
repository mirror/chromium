...
bool IsArcMigrationAllowedByPolicyForProfile(const Profile* profile) {
  ...
  // migration policy.
  // In this case default case, only a group of devices that had ARC M enabled
  // are allowed to migrate, provided that ARC is enabled by policy.
  // TODO(pmarko): Remove the special kAskForEcryptfsArcUsers when we assess
  // that it's not necessary anymore: crbug.com/761348.
  if (migration_strategy ==
      static_cast<int>(
          arc::policy_util::EcryptfsMigrationAction::kAskForEcryptfsArcUsers)) {
    // Note that  the ARC enablement is controlled by policy for managed users
    // (as it's marked 'default_for_enterprise_users': False in
