...
struct ArcScreenCaptureSession::PendingBuffer {
  ...
  const GLuint texture_{0};
  const GLuint image_id_{0};
};
...
struct ArcScreenCaptureSession::DesktopTexture {
  ...
  const GLuint texture_{0};
  gfx::Size size_;
  std::unique_ptr<viz::SingleReleaseCallback> release_callback_{nullptr};
};
...
void ArcScreenCaptureSession::SetOutputBuffer(
  ...
  if (mojo_result != MOJO_RESULT_OK) {
    LOG(ERROR) << "ERROR unwrapping Mojo handle " << mojo_result;
    std::move(callback).Run();
    return;
  }
  ...
  handle.native_pixmap_handle.planes.emplace_back(
      stride * 4, 0, stride * 4 * size_.height(), 0);
  ...
  if (!gpu_memory_buffer) {
    LOG(ERROR) << "Error creating GpuMemoryBuffer";
    std::move(callback).Run();
    return;
  }
...
void ArcScreenCaptureSession::OnDesktopCaptured(
  ...
  if (buffer_queue_.empty()) {
    ...
    // when we have one.
    texture_queue_.push(std::move(desktop_texture));
  } else {
