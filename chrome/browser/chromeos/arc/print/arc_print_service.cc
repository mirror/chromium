...
class ArcPrintRequest : public content::NotificationObserver {
 public:
  ArcPrintRequest(std::unique_ptr<printing::PrintSettings> settings,
    ...
    base::PostTaskWithTraits(
        ...
        base::BindOnce(&ArcPrintRequest::ReadFileOnBlockingThread,
                       base::Unretained(this)));
    content::BrowserThread::PostTask(
        content::BrowserThread::IO, FROM_HERE,
        base::BindOnce(&ArcPrintRequest::SetSettingsOnIO,
                       base::Unretained(this)));
  }
 ...
 private:
  void ReadFileOnBlockingThread() {
    ...
    DCHECK_EQ(static_cast<size_t>(bytes), data_size_);
    file_.Close();
  ...
  void SetSettingsOnIO() {
    query_ = base::MakeRefCounted<printing::PrinterQuery>(
        ...
        content::ChildProcessHost::kInvalidUniqueID);
    query_->SetSettings(std::move(settings_),
                        base::Bind(&ArcPrintRequest::SetSettingsDoneOnIO,
                                   base::Unretained(this)));
  }

  void SetSettingsDoneOnIO() {
    content::BrowserThread::PostTask(
        content::BrowserThread::UI, FROM_HERE,
        base::BindOnce(&ArcPrintRequest::SetSettingsDone,
                       base::Unretained(this)));
  }

  void SetSettingsDone() {
    job_ = base::MakeRefCounted<printing::PrintJob>();
    job_->Initialize(query_.get(), base::string16(), 1);
