...
std::unique_ptr<printing::PdfMetafileSkia> ReadFileOnBlockingThread(
  ...
  ssize_t bytes = file.ReadAtCurrentPos(buf.data(), data_size);
  if (bytes < 0) {
    LOG(ERROR) << "Error reading PDF: " << strerror(errno);
  }
  DCHECK_EQ(static_cast<size_t>(bytes), data_size);
  file.Close();

  auto metafile = std::make_unique<printing::PdfMetafileSkia>(
      printing::SkiaDocumentType::PDF);
  metafile->InitFromData(buf.data(), buf.size());
  return metafile;
}

void SetSettingsOnIO(
    std::unique_ptr<printing::PrintSettings> settings,
    base::Callback<void(scoped_refptr<printing::PrinterQuery>)> callback) {
  auto query = base::MakeRefCounted<printing::PrinterQuery>(
      content::ChildProcessHost::kInvalidUniqueID,
      content::ChildProcessHost::kInvalidUniqueID);
  query->SetSettings(std::move(settings), base::Bind(callback, query));
}
...
class ArcPrintRequest : public content::NotificationObserver {
 public:
  ArcPrintRequest(std::unique_ptr<printing::PrintSettings> settings,
    ...
    content::BrowserThread::PostTask(
        ...
        base::BindOnce(&SetSettingsOnIO, std::move(settings),
                       base::Bind(&PostCallback,
                                  base::Bind(&ArcPrintRequest::SetSettingsDone,
 ...
 private:
  ...
  void SetSettingsDone(scoped_refptr<printing::PrinterQuery> query) {
    job_ = base::MakeRefCounted<printing::PrintJob>();
    job_->Initialize(query.get(), base::string16(), 1);
  ...
  void StartPrintingIfReady() {
    if (job_ && metafile_) {
      ...
      document->SetPage(0, std::move(metafile_), gfx::Size(), gfx::Rect());
      job_->StartPrinting();
    }
  }
...
void ArcPrintService::OnInstanceClosed() {
  binding_.Close();
}
...
void ArcPrintService::CreateDiscoverySession(
  ...
  new DiscoverySession(std::move(instance), profile_, std::move(request));
}
