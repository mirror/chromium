...
class ArcPrintServiceImpl : public ArcPrintService,
 ...
 protected:
  ...
  void OnPrintJobCreated(chromeos::CupsPrintJob* job);
  void OnPrintJobCancelled(chromeos::CupsPrintJob* job);
  void OnPrintJobError(chromeos::CupsPrintJob* job);
  void OnPrintJobDone(chromeos::CupsPrintJob* job);

 private:
  Profile* const profile_;                      // Owned by ProfileManager.
  ArcBridgeService* const arc_bridge_service_;  // Owned by ArcServiceManager.

  std::unordered_map<PrintJobHostImpl*, std::unique_ptr<PrintJobHostImpl>>
      jobs_;
  std::unordered_map<PrinterDiscoverySessionHostImpl*,
                     std::unique_ptr<PrinterDiscoverySessionHostImpl>>
      sessions_;

  // Managed by PrintJobHostImpl instances.
  std::unordered_map<std::string, PrintJobHostImpl*> jobs_by_id_;
};
...
class PrinterDiscoverySessionHostImpl
 ...
 public:
  ...
  void DestroyDiscoverySession() override { delete this; }
 ...
 private:
  base::OnceClosure MakeErrorHandler() {
    // Only passed to owned objects, so unretained is fine.
...
class PrintJobHostImpl : public mojom::PrintJobHost,
 ...
 public:
  PrintJobHostImpl(mojo::InterfaceRequest<mojom::PrintJobHost> request,
                   ...
                   CupsPrintJobManager* job_manager,
  ...
  void Observe(int type,
    ...
    switch (event_details.type()) {
      case printing::JobEventDetails::DOC_DONE:
        ...
        service_->JobIdGenerated(this, job_id_);
        break;
 ...
 private:
  ...
  base::OnceClosure MakeErrorHandler() {
    // Only passed to owned objects, so unretained is fine.
    return base::BindOnce(&PrintJobHostImpl::Destroy, base::Unretained(this));
  }
  ...
  CupsPrintJobManager* const job_manager_;
...
void ArcPrintServiceImpl::CreateDiscoverySession(
  ...
  auto session = std::make_unique<PrinterDiscoverySessionHostImpl>(
      mojo::MakeRequest(&host_proxy), std::move(instance), profile_);
