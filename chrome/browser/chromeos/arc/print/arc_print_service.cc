...
class ArcPrintService::PrintJobHostImpl : public mojom::PrintJobHost,
  ...
  void Observe(int type,
    ...
    switch (event_details.type()) {
      case printing::JobEventDetails::DOC_DONE:
        ...
        service_->jobs_.emplace(job_id_, this);
        break;
...
void ArcPrintService::Print(mojom::PrintJobInstancePtr instance,
  ...
  float x_scale = static_cast<float>(resolution->width()) / printing::kMilsPerInch;
  float y_scale = static_cast<float>(resolution->height()) / printing::kMilsPerInch;
  ...
  new PrintJobHostImpl(mojo::MakeRequest(&host_proxy), std::move(instance),
                       this, std::move(settings),
                       base::File(scoped_handle.release().handle),
                       print_job->data_size);
...
void ArcPrintService::CreateDiscoverySession(
  ...
  // Manages its own lifetime.
  new PrinterDiscoverySessionHostImpl(mojo::MakeRequest(&host_proxy),
