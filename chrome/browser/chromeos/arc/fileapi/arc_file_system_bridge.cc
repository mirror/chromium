...
void GetFileSizeOnIOThread(
    ...
    const ArcFileSystemBridge::GetFileSizeCallback& callback) {
...
void ArcFileSystemBridge::GetFileName(const std::string& url,
  ...
  callback.Run(url_decoded.ExtractFileName());
}
...
void ArcFileSystemBridge::GetFileSize(const std::string& url,
  ...
  content::StoragePartition* const storage =
      content::BrowserContext::GetStoragePartitionForSite(profile_,
                                                          url_decoded);
  scoped_refptr<storage::FileSystemContext> context =
  ...
  storage::FileSystemURL file_system_url =
      file_manager::util::CreateIsolatedURLFromVirtualPath(
          *context, /* empty origin */ GURL(),
          chromeos::ExternalFileURLToVirtualPath(url_decoded));

  content::BrowserThread::PostTask(
      content::BrowserThread::IO, FROM_HERE,
      base::Bind(&GetFileSizeOnIOThread, context, file_system_url, callback));
}
...
void ArcFileSystemBridge::GetFileType(const std::string& url,
  ...
  extensions::app_file_handler_util::GetMimeTypeForLocalPath(
      ...
      base::Bind([](const GetFileTypeCallback& callback,
                    const std::string& mime_type) { callback.Run(mime_type); },
                 callback));
}
...
void ArcFileSystemBridge::OpenFileToRead(
  ...
  GetFileSize(
      url, base::Bind(&ArcFileSystemBridge::OpenFileToReadAfterGetFileSize,
                      weak_ptr_factory_.GetWeakPtr(), url_decoded, callback));
}
...
void ArcFileSystemBridge::OpenFileToReadAfterGetFileSize(
  ...
  chromeos::DBusThreadManager::Get()->GetVirtualFileProviderClient()->OpenFile(
      size, base::Bind(&ArcFileSystemBridge::OnOpenFile, base::Unretained(this),
                       url_decoded, callback));
}
...
void ArcFileSystemBridge::OnOpenFile(const GURL& url_decoded,
  ...
  if (result != MOJO_RESULT_OK) {
    LOG(ERROR) << "Failed to wrap handles. Closing: " << result;
    callback.Run(mojo::ScopedHandle());
    return;
  }
...
void ArcFileSystemBridge::HandleReadRequest(const std::string& id,
                                            int64_t offset,
                                            int64_t size,
  ...
  file_stream_forwarders_[id] = FileStreamForwarderPtr(new FileStreamForwarder(
      context, file_system_url, offset, size, std::move(pipe_write_end),
