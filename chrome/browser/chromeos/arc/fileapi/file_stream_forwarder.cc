...
FileStreamForwarder::~FileStreamForwarder() {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  // Close the FD with the task runner.
...
void FileStreamForwarder::Start() {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  stream_reader_ =
      context_->CreateFileStreamReader(url_, offset_, size_, base::Time());
  if (!stream_reader_) {
...
void FileStreamForwarder::DoRead() {
  ...
  const int result =
      stream_reader_->Read(buf_.get(), std::min<int64_t>(buf_->size(), size_),
