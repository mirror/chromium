...
TEST(ChromeContentProviderUrlUtilTest, EncodeAndDecode) {
  {
    GURL original("externalfile://foo/bar/baz");
    GURL encoded = EncodeToChromeContentProviderUrl(original);
    EXPECT_TRUE(encoded.is_valid());
    GURL decoded = DecodeChromeContentProviderUrl(encoded);
    EXPECT_TRUE(decoded.is_valid());
    EXPECT_EQ(original, decoded);
  }
  {
    GURL original("externalfile://foo/!@#$%^&*()_+|~-=\\`[]{};':\"<>?,./");
    GURL encoded = EncodeToChromeContentProviderUrl(original);
    EXPECT_TRUE(encoded.is_valid());
    GURL decoded = DecodeChromeContentProviderUrl(encoded);
    EXPECT_TRUE(decoded.is_valid());
  ...
  }
  {
    ...
    GURL original("externalfile://foo/" + base::UTF16ToUTF8(utf16_string));
    GURL encoded = EncodeToChromeContentProviderUrl(original);
    EXPECT_TRUE(encoded.is_valid());
    GURL decoded = DecodeChromeContentProviderUrl(encoded);
    EXPECT_TRUE(decoded.is_valid());
