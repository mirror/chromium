  ...
  for (auto spec : kDocumentsProviderWhitelist) {
    map_[Key(spec.authority, spec.root_document_id)] =
        base::MakeUnique<ArcDocumentsProviderRoot>(
            runner, spec.authority, spec.root_document_id, "", "", "", "", 0);
  }
}
...
void ArcDocumentsProviderRootMap::Refresh(
  ...
  DCHECK(runner);
  runner->GetRoots(base::Bind(&ArcDocumentsProviderRootMap::RefreshInternal_,
...
void ArcDocumentsProviderRootMap::RefreshInternal_(
  ...
  for (const mojom::RootPtr& rootFromMojo : *rootsFromMojo) {
    ...
    std::unique_ptr<ArcDocumentsProviderRoot> root =
        base::MakeUnique<ArcDocumentsProviderRoot>(
            runner, authority, document_id, rootFromMojo->id,
            rootFromMojo->title, rootFromMojo->summary, rootFromMojo->icon,
            rootFromMojo->flags);
