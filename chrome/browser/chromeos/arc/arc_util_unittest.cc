...
// Test the CanIgnoreAllOptInPreferencesForProfile() function.
TEST_F(ChromeArcUtilTest, CanIgnoreAllOptInPreferencesForProfile) {
  base::CommandLine::ForCurrentProcess()->InitFromArgv(
      {"", "--arc-availability=officially-supported"});
  // OptIn prefs are unset, the function returns false.
  EXPECT_FALSE(CanIgnoreAllOptInPreferencesForProfile(profile()));
  ...
  // Prefs are unused for Active Directory users.
  {
    ScopedLogIn login(GetFakeUserManager(),
                      AccountId::AdFromUserEmailObjGuid(
                          profile()->GetProfileUserName(), kTestGaiaId));
    EXPECT_TRUE(CanIgnoreAllOptInPreferencesForProfile(profile()));
  }
