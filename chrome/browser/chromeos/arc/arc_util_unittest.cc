...
struct AskForEcryptfsArcUserTestParam {
  AskForEcryptfsArcUserTestParam(bool device_supported_arc,
                                 bool arc_enabled,
                                 bool expect_migration_allowed)
      : device_supported_arc_(device_supported_arc),
        arc_enabled_(arc_enabled),
        expect_migration_allowed_(expect_migration_allowed) {}
  bool device_supported_arc_;
  bool arc_enabled_;
  bool expect_migration_allowed_;
};
...
TEST_P(ArcMigrationAskForEcryptfsArcUsersTest,
  ...
  ScopedLogIn login(GetFakeUserManager(),
                    AccountId::AdFromUserEmailObjGuid(
                        profile()->GetProfileUserName(), kTestGaiaId));
  if (param.device_supported_arc_) {
  ...
  profile()->GetTestingPrefService()->SetManagedPref(
      prefs::kArcEnabled, base::MakeUnique<base::Value>(param.arc_enabled_));
  EXPECT_EQ(param.expect_migration_allowed_,
