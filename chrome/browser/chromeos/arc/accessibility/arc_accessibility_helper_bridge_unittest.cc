...
// channels, mojo and wayland. Orders of those events can be changed. This is
// the case where mojo events arrive earlier than surface creation/removal.
TEST_F(ArcAccessibilityHelperBridgeTest, NotificationEventArriveFirst) {
  ...
  AXTreeSourceArc* tree = it->second.second.get();
  ui::AXTreeData tree_data;
  tree->GetTreeData(&tree_data);
  ASSERT_EQ(tree_data.tree_id, test_surface.GetAXTreeId());
