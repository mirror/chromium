...
void ArcAccessibilityHelperBridge::SetNativeChromeVoxArcSupport(bool enabled) {
  ...
  if (it == task_id_to_tree_.end() || it->second->package_name().empty())
    return;

  const std::string& package_name = it->second->package_name();
  auto* instance =
  ...
  instance->SetNativeChromeVoxArcSupport(
      ...
      base::Bind(&ArcAccessibilityHelperBridge::
                 ...
                 base::Unretained(this), task_id, enabled));
}
...
void ArcAccessibilityHelperBridge::OnSetNativeChromeVoxArcSupportProcessed(
  ...
  if (enabled) {
    ...
    task_id_to_tree_[task_id]->Focus(window);
  } else if (it != task_id_to_tree_.end()) {
