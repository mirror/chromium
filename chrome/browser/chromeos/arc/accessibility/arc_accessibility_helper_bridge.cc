...
AXTreeSourceArc* ArcAccessibilityHelperBridge::GetOrCreateFromNotificationKey(
  ...
  if (tree_it == notification_key_to_tree_.end()) {
    notification_key_to_tree_.insert(std::make_pair(
        notification_key, std::make_pair(1U, new AXTreeSourceArc(this))));
    return notification_key_to_tree_[notification_key].second.get();
  }

  if (increment_counter)
    tree_it->second.first++;

  return tree_it->second.second.get();
}
...
AXTreeSourceArc* ArcAccessibilityHelperBridge::GetFromTreeId(
  ...
  for (auto notification_it = notification_key_to_tree_.begin();
    ...
    ui::AXTreeData tree_data;
    notification_it->second.second->GetTreeData(&tree_data);
    if (tree_data.tree_id == tree_id)
      return notification_it->second.second.get();
  }
