...
bool Compare(const arc::mojom::CertificatePtr& cert1,
             const arc::mojom::CertificatePtr& cert2) {
  if (!cert1 || !cert2)
    return !cert1;
  if (cert1->alias != cert2->alias)
    return cert1->alias < cert2->alias;
  return cert1->cert <= cert2->cert;
}
...
class FakeArcCertStoreInstance : public mojom::CertStoreInstance {
 ...
 private:
  ...
  bool is_on_certs_changed_called_;
};
...
class ArcCertStoreBridgeTest : public InProcessBrowserTest {
 protected:
  ...
  void ListCertificatesTest(ArcCertStoreBridge* cert_store_bridge,
    ...
    cert_store_bridge->ListCertificates(base::Bind(
        ...
        std::move(expected_certs), loop.QuitClosure()));
    loop.Run();
  }
 ...
 private:
  ...
  void OnCertificatesListed(
    ...
    std::sort(certs.begin(), certs.end(), Compare);
    EXPECT_EQ(certs.size(), expected_certs.size());
