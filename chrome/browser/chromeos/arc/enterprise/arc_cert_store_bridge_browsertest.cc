...
class ArcCertStoreBridgeTest : public InProcessBrowserTest {
 protected:
  ...
  const std::vector<mojom::CertificatePtr>& ListCertificates(
 ...
 private:
  ...
  void OnCertificatesListed(const base::Closure& done_callback,
                            std::vector<mojom::CertificatePtr> certs) {
    certs_ = std::move(certs);
    std::sort(certs_.begin(), certs_.end(), CertificatePtrLess);
    done_callback.Run();
  }
  ...
  // Must be initialized in OnCertificatesListed.
  std::vector<mojom::CertificatePtr> certs_;
