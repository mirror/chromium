...
scoped_refptr<net::X509Certificate> FindCertificateByAlias(
  ...
  return net::x509_util::CreateX509CertificateFromCERTCertificate(cert.get());
}

// Returns true if the certificate is allowed to be used by ARC.
bool IsCertificateAllowed(scoped_refptr<net::X509Certificate> cert,
  ...
  if (!chromeos::KeyPermissions::IsCorporateKeyForProfile(
    ...
    LOG(ERROR) << "Certificate is not allowed to be used by ARC.";
    return false;
  }
  return true;
}
...
void ArcCertStoreBridge::ListCertificates(
  ...
  if (!channel_enabled_) {
    ...
    callback.Run(std::vector<mojom::CertificatePtr>());
    return;
  }

  GetNSSCertDatabaseForProfile(
      Profile::FromBrowserContext(context_),
      base::Bind(&ArcCertStoreBridge::OnGetNSSCertDatabaseForUser,
...
void ArcCertStoreBridge::GetKeyCharacteristics(
  ...
  mojom::KeymasterError error = mojom::KeymasterError::ERROR_INVALID_KEY_BLOB;
  ...
  if (!IsCertificateAllowed(
          FindCertificateByAlias(alias),
          Profile::FromBrowserContext(context_)->GetPrefs())) {
    callback.Run(error, base::nullopt);
    return;
  }
  // TODO(pbond): implement.
  error = mojom::KeymasterError::ERROR_UNIMPLEMENTED;
  callback.Run(error, base::nullopt);
}
...
void ArcCertStoreBridge::OnGetNSSCertDatabaseForUser(
    const ListCertificatesCallback& callback,
