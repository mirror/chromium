...
void ArcCertStoreBridge::ListCertificates(
  ...
  for (CERTCertListNode* node = CERT_LIST_HEAD(cert_list);
    ...
    base::Base64Encode(
        base::StringPiece(reinterpret_cast<const char*>(spki_der.data),
                          spki_der.len),
...
void ArcCertStoreBridge::GetKeyCharacteristics(
  ...
  mojom::KeymasterError error = mojom::KeymasterError::ERROR_UNIMPLEMENTED;
  ...
  if (!IsCertificateAllowed(
    ...
    error = mojom::KeymasterError::ERROR_INVALID_KEY_BLOB;
    callback.Run(error, base::nullopt);
    return;
  }
...
void ArcCertStoreBridge::OnKeyPermissionsChanged() {
  ...
  for (const auto& item : key_permissions_map->DictItems()) {
    const auto& package_name = item.first;
