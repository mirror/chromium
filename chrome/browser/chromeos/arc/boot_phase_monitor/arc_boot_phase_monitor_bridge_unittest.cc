...
// Tests that OnExtensionsReady() relaxes the CPU restriction when the start
// mode is set to "always".
TEST_F(ArcBootPhaseMonitorBridgeTest, TestOnExtensionsReady_AlwaysStart) {
  auto* command_line = base::CommandLine::ForCurrentProcess();
  command_line->InitFromArgv({"", "--arc-start-mode=always-start"});
  boot_phase_monitor_bridge()->OnExtensionsReady();
  EXPECT_EQ(1U, disable_cpu_restriction_counter());
}
