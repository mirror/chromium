...
class ArcWallpaperService::DecodeRequest : public ImageDecoder::ImageRequest {
 public:
  ...
  void OnImageDecoded(const SkBitmap& bitmap) override {
    ...
    bool custom_wallpaper_allowed = wallpaper_controller->CanSetCustomWallpaper(
        account.id,
        !user_manager::UserManager::Get()->IsUserNonCryptohomeDataEphemeral(
            account.id) /* is_persistent */);

    if (!custom_wallpaper_allowed || !account.is_active) {
      // When kiosk app is running, policy is enforced or account id is not
      // active, WallpaperController doesn't process custom wallpaper requests.
