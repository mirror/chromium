...
class SuccessDecodeRequestSender
 ...
 public:
  ...
  void SendDecodeRequest(ImageDecoder::ImageRequest* request,
    ...
    bitmap_.allocN32Pixels(256 /*width*/, 256 /*height*/);
    bitmap_.eraseColor(SK_ColorRED);
    request->OnImageDecoded(bitmap_);
  }
...
class ArcWallpaperServiceTest : public ash::AshTestBase {
 public:
  ...
  void SetUp() override {
    ...
    // Arc services
    wallpaper_instance_ = new arc::FakeWallpaperInstance();
 ...
 protected:
  ...
  arc::FakeWallpaperInstance* wallpaper_instance_ = nullptr;
...
void OnGetWallpaper(std::vector<unsigned char>* out,
...
TEST_F(ArcWallpaperServiceTest, SetAndGetWallpaper) {
  SuccessDecodeRequestSender sender;
  arc::ArcWallpaperService::TestApi::SetDecodeRequestSender(service_, &sender);
  ...
  service_->GetWallpaper(base::Bind(&OnGetWallpaper, &bytes));
  content::RunAllTasksUntilIdle();
  ASSERT_NE(0u, bytes.size());
}

TEST_F(ArcWallpaperServiceTest, SetWallpaperFailure) {
  FailureDecodeRequestSender sender;
  arc::ArcWallpaperService::TestApi::SetDecodeRequestSender(service_, &sender);
  std::vector<unsigned char> bytes;
  service_->SetWallpaper(bytes, 10 /* ID */);
  content::RunAllTasksUntilIdle();
