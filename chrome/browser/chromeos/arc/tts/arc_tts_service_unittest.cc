...
class ArcTtsServiceTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    tts_service_ = base::MakeUnique<ArcTtsService>(testing_profile_.get(),
                                                   bridge_service_.get());
    tts_service_->set_tts_controller_getter_for_testing(base::BindRepeating(
        &ArcTtsServiceTest::GetTestableTtsController, base::Unretained(this)));
  }
 ...
 protected:
  ArcTtsService* tts_service() const { return tts_service_.get(); }
  TestableTtsController* tts_controller() const {
    return tts_controller_.get();
  }

 private:
  TtsController* GetTestableTtsController() { return tts_controller_.get(); }
  ...
  std::unique_ptr<ArcTtsService> tts_service_;
...
TEST_F(ArcTtsServiceTest, TestOnTtsEvent) {
  tts_service()->OnTtsEvent(1, mojom::TtsEventType::START, 2, "3");
  EXPECT_EQ(1, tts_controller()->last_utterance_id_);
  EXPECT_EQ(TTS_EVENT_START, tts_controller()->last_event_type_);
  EXPECT_EQ(2, tts_controller()->last_char_index_);
  EXPECT_EQ("3", tts_controller()->last_error_message_);

  tts_service()->OnTtsEvent(4, mojom::TtsEventType::END, 5, "6");
  EXPECT_EQ(4, tts_controller()->last_utterance_id_);
  EXPECT_EQ(TTS_EVENT_END, tts_controller()->last_event_type_);
  EXPECT_EQ(5, tts_controller()->last_char_index_);
  EXPECT_EQ("6", tts_controller()->last_error_message_);

  tts_service()->OnTtsEvent(7, mojom::TtsEventType::INTERRUPTED, 8, "9");
  EXPECT_EQ(7, tts_controller()->last_utterance_id_);
  EXPECT_EQ(TTS_EVENT_INTERRUPTED, tts_controller()->last_event_type_);
  EXPECT_EQ(8, tts_controller()->last_char_index_);
  EXPECT_EQ("9", tts_controller()->last_error_message_);

  tts_service()->OnTtsEvent(10, mojom::TtsEventType::ERROR, 11, "12");
  EXPECT_EQ(10, tts_controller()->last_utterance_id_);
  EXPECT_EQ(TTS_EVENT_ERROR, tts_controller()->last_event_type_);
  EXPECT_EQ(11, tts_controller()->last_char_index_);
  EXPECT_EQ("12", tts_controller()->last_error_message_);
}
