...
constexpr char kChromePackageName[] =
...
TEST(ArcExternalProtocolDialogTest, TestIsChromeAnAppCandidate) {
  ...
  EXPECT_TRUE(IsChromeAnAppCandidate(handlers));
  ...
  handlers2.push_back(Create("fake app 2", "fake.app.package2", false,
                             GURL("https://www.bar.com")));
  EXPECT_TRUE(IsChromeAnAppCandidate(handlers2));
  ...
  handlers3.push_back(Create("fake app 2", "fake.app.package2", false,
                             GURL("https://www.bar.com")));
  EXPECT_TRUE(IsChromeAnAppCandidate(handlers3));
  ...
  EXPECT_FALSE(IsChromeAnAppCandidate(handlers4));
}
