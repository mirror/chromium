...
void ArcSettingsServiceImpl::SyncTimeZoneByGeolocation() const {
  ...
  DCHECK(pref);
  int setTimeZoneByGeolocation = false;
  bool value_exists = pref->GetValue()->GetAsInteger(&setTimeZoneByGeolocation);
  DCHECK(value_exists);
  base::DictionaryValue extras;
  extras.SetBoolean(
      "autoTimeZone",
      chromeos::system::TimeZoneResolverManager::TimeZoneResolveMethodFromInt(
          setTimeZoneByGeolocation) !=
          chromeos::system::TimeZoneResolverManager::TimeZoneResolveMethod::
              DISABLED);
