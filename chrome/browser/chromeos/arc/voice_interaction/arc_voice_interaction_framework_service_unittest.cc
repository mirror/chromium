...
class TestHighlighterController : public ash::mojom::HighlighterController {
 public:
  ...
  ~TestHighlighterController() override {}
  ...
  bool client_attached() const { return !!client_; }

  // ash::mojom::HighlighterController:
  void AttachClient(
      ash::mojom::HighlighterControllerClientPtr client) override {
    DCHECK(!client_);
...
class ArcVoiceInteractionFrameworkServiceTest : public ash::AshTestBase {
 public:
  ...
  void SetUp() override {
    ...
    framework_service_->highlighter_client_->highlighter_controller_ =
 ...
 protected:
  ...
  void SimulateHighlighterSelectionTimeout() {
    base::Timer* timer =
        framework_service_->highlighter_client_->delay_timer_.get();
    EXPECT_TRUE(timer && timer->IsRunning());
    timer->user_task().Run();
    timer->Stop();
  }
...
TEST_F(ArcVoiceInteractionFrameworkServiceTest, HighlighterControllerClient) {
  // Flushing is required for the AttachClient calls to get through to the
  ...
  FlushHighlighterControllerMojo();
  EXPECT_EQ(2u, framework_instance()->set_metalayer_visibility_count());
  EXPECT_FALSE(framework_instance()->metalayer_visible());

  // Enable the state again and simulate a valid selection.
  highlighter_controller()->CallHandleEnabledStateChange(true);
  FlushHighlighterControllerMojo();
  EXPECT_EQ(3u, framework_instance()->set_metalayer_visibility_count());
  EXPECT_TRUE(framework_instance()->metalayer_visible());
  ...
  EXPECT_EQ(3u, framework_instance()->set_metalayer_visibility_count());
  EXPECT_TRUE(framework_instance()->metalayer_visible());
  SimulateHighlighterSelectionTimeout();
  FlushHighlighterControllerMojo();
  ...
  highlighter_controller()->CallHandleEnabledStateChange(true);
  FlushHighlighterControllerMojo();
  EXPECT_EQ(4u, framework_instance()->set_metalayer_visibility_count());
  EXPECT_TRUE(framework_instance()->metalayer_visible());
}
