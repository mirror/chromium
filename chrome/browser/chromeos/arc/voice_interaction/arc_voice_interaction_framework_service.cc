...
void ArcVoiceInteractionFrameworkService::OnArcPlayStoreEnabledChanged(
  ...
  SetVoiceInteractionEnabled(false, base::Bind(DoNothing<bool>));
...
void ArcVoiceInteractionFrameworkService::SetVoiceInteractionEnabled(
    bool enable,
    const VoiceInteractionSettingCompleteCallback& callback) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
...
void ArcVoiceInteractionFrameworkService::SetVoiceInteractionSetupCompleted() {
  ...
  SetVoiceInteractionEnabled(true, base::Bind(DoNothing<bool>));
...
void ArcVoiceInteractionFrameworkService::
    StartVoiceInteractionSetupWizardActivity(bool setting_applied) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);

  // This screen is shown after the Just-A-Sec screen, which blocks until
  // application sync'd is received. At that point, framework service
  // should already be initialized.
  DCHECK(setting_applied);
