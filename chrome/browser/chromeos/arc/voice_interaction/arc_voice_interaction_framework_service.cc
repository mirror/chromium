...
void ArcVoiceInteractionFrameworkService::SetVoiceInteractionState(
  ...
  if (state_ == ash::VoiceInteractionState::NOT_READY) {
    ...
    // TODO(muyuanli): Current devices have VoiceInteractionEnabled = false and
    // VoiceInteractionContextEnabled by
    ...
    // accordingly. The same goes for VoiceInteractionContextEnabled.
    // As we move forward, this fix could be removed.
