...
#include "chrome/browser/chromeos/arc/voice_interaction/internal/assist_data_builder.h"
#endif
...
void ArcVoiceInteractionFrameworkService::CaptureFullscreen(
  ...
  CaptureScreenshotWithCallback(gfx::Rect(), false /* encrypt_assist_data */,
                                std::move(callback));
}
...
void ArcVoiceInteractionFrameworkService::CaptureSelectedRegion(
  ...
  CaptureScreenshotWithCallback(
      screenshot_bounds_, false /* encrypt_assist_data */, std::move(callback));
}
...
void ArcVoiceInteractionFrameworkService::GetAssistData(
  ...
  CaptureScreenshotWithCallback(
      screenshot_bounds_, true /* encrypt_assist_data */, std::move(callback));
}
...
void ArcVoiceInteractionFrameworkService::CaptureScreenshotWithCallback(
  ...
  auto* functor =
      encrypt_assist_data ? &EncryptAndSendAssistData : &EncodeAndReturnImage;

  ui::GrabLayerSnapshotAsync(
      old_layer_owner->root(), gfx::Rect(window->bounds().size()),
      base::BindRepeating(functor, base::Passed(std::move(callback)),
