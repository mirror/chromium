...
class ArcVoiceInteractionArcHomeService
 ...
 public:
  ...
  // Calls to notify that assistant flow has been started and we have to lock
  // PAI.
  void OnAssistanceStarted();
  // Calls to notify that assistant flow expects Android assistance app started.
  void OnAssistanceAppRequested();
  // Calls to notify that assistant flow has been canceled.
  void OnAssistanceCanceled();
 ...
 private:
  ...
  // Locks/Unlock Play Auto Install.
  void LockPai();
  void UnlockPai();

  void Reset();
  void OnAssistantFailedToStart();
  void OnWizardDidNotComplete();
