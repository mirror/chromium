...
class ArcSupportHost : public arc::ArcSupportMessageHost::Observer,
 ...
 public:
  ...
  // Requests to show the "Active Directory SAML auth" page.
  void ShowActiveDirectoryAuth();

  // Sets the Active Directory Federation Services URL |federation_url| that
  // handles user authentication (aka the SAML redirect URL), as well as the
  // device management (DM) server URL prefix that used to detect whether the
  // SAML flow finished. The DM server is the SAML service provider.
  void SetActiveDirectoryAuthUrls(const GURL& federation_url,
