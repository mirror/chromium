...
class FakeBaseScreen : public chromeos::BaseScreen {
 public:
  ...
  bool IsStatusAreaDisplayed() override { return false; }
...
class FakeLoginDisplayHost : public chromeos::LoginDisplayHost {
 public:
  ...
  void StartWizard(chromeos::OobeScreen first_screen) override {
    ...
    wizard_controller_->SetCurrentScreenForTesting(
        first_screen == chromeos::OobeScreen::SCREEN_ARC_TERMS_OF_SERVICE
            ? arc_tos_screen_.get()
            : nullptr);
  }
 ...
 private:
  ...
  std::unique_ptr<FakeBaseScreen> arc_tos_screen_ =
      std::make_unique<FakeBaseScreen>(
