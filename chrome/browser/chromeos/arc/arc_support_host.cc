...
void ArcSupportHost::SetActiveDirectoryAuthUrls(
    const GURL& federation_url,
    const std::string& dm_url_prefix) {
  // Needs to be cached. Missing |message_host_| is normal on first run. URLs
  // might be invalid when called from SetMessageHost.
  ...
  message.SetString(kDmUrlPrefix, active_directory_auth_dm_url_prefix_);
  message_host_->SendMessage(message);
}
...
void ArcSupportHost::SetMessageHost(arc::ArcSupportMessageHost* message_host) {
  ...
  SetActiveDirectoryAuthUrls(active_directory_auth_federation_url_,
