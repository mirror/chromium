  ...
  if (result == ProvisioningResult::SUCCESS) {
    ...
    const bool suppress_play_store_app =
        ...
        (IsArcPlayStoreEnabledPreferenceManagedForProfile(profile_) &&
         AreArcAllOptInPreferencesManagedOrUnusedForProfile(profile_));
...
void ArcSessionManager::CancelAuthCode() {
  ...
  // LSO or AD_AUTH, closing the window should stop ARC since the user chooses
  // to not sign in. In any other case, ARC is booting normally and the instance
  ...
  if ((state_ != State::NEGOTIATING_TERMS_OF_SERVICE &&
      ...
      (!support_host_ ||
       (support_host_->ui_page() != ArcSupportHost::UIPage::ERROR &&
        ...
        support_host_->ui_page() != ArcSupportHost::UIPage::AD_AUTH))) {
    return;
  }
