...
void ArcSessionManager::OnProvisioningFinished(ProvisioningResult result) {
  ...
  if (result == ProvisioningResult::SUCCESS) {
    ...
    //   happen as seamless as possible for the user. For Active Directory users
    //   we always show a page notifying them that they have to authenticate
    //   with their identity provider (through SAML) to make it less weird that
...
void ArcSessionManager::MaybeStartTermsOfServiceNegotiation() {
  ...
  if (IsActiveDirectoryUserForProfile(profile_)) {
    ...
    // The negotiator screen just shows text and a Next button.
    if (IsOobeOptInActive()) {
      // TODO(ljusten): Add OOBE support, https://crbug.com/719874.
      VLOG(1) << "Active Directory OOBE negotiator not yet present, skipping.";
    ...
    if (support_host_) {
      ...
      terms_of_service_negotiator_ =
          base::MakeUnique<ArcActiveDirectoryAuthDefaultNegotiator>(
              support_host_.get());
    }
  } else {
    // Not an Active Directory user.
    if (IsOobeOptInActive()) {
