...
// Prefix for KRB5CCNAME variable. Defines ccache type.
const char kKrb5ccFilePrefix[] = "FILE:";
// Directory in the user home to store kerberos files.
const char kKerberosDirectory[] = "kerberos";
// Environment variable pointing to credential cache file.
const char kKrbCCEnvName[] = "KRB5CCNAME";
// Credential cache file name.
const char kKrb5ccFile[] = "krb5cc";
// Environment variable pointing to krb5 config file.
const char kKrbConfEnvName[] = "KRB5_CONFIG";
// krb5 config file name.
...
void WriteFile(const std::string& file_name, const std::string& blob) {
  ...
  dir = dir.Append(kKerberosDirectory);
...
AuthPolicyCredentialsManager::AuthPolicyCredentialsManager(Profile* profile)
  ...
  env->SetVar(kKrbCCEnvName,
              kKrb5ccFilePrefix +
                  path.Append(kKerberosDirectory).Append(kKrb5ccFile).value());
  env->SetVar(kKrbConfEnvName,
              path.Append(kKerberosDirectory).Append(kKrb5confFile).value());
}
...
void AuthPolicyCredentialsManager::GetUserStatus() {
  DCHECK(!is_get_status_in_progress);
  is_get_status_in_progress = true;
...
void AuthPolicyCredentialsManager::OnGetUserStatusCallback(
  ...
  DCHECK(is_get_status_in_progress);
  is_get_status_in_progress = false;
...
void AuthPolicyCredentialsManager::GetKerberosFiles() {
  chromeos::DBusThreadManager::Get()->GetAuthPolicyClient()->GetKerberosFiles(
      account_id_.GetObjGuid(),
      base::BindOnce(&AuthPolicyCredentialsManager::OnGetKerbefosFilesCallback,
                     weak_factory_.GetWeakPtr()));
}

void AuthPolicyCredentialsManager::OnGetKerbefosFilesCallback(
    authpolicy::ErrorType error,
