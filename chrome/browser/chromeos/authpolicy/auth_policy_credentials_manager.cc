...
void WriteFile(const std::string& file, const std::string& blob) {
  base::FilePath dir;
  ...
  base::FilePath dest_file = dir.Append(file);
...
void AuthPolicyCredentialsManager::OnGetKerbefosFilesCallback(
  ...
  LOG(ERROR) << "OnGetUserStatusCallback";
  if (kerberos_files.has_krb5cc()) {
    LOG(ERROR) << "WriteFile";
    // krb5ccTime_ = kerberos_files.krb5cc().timestamp();
...
// void AuthPolicyCredentialsManager::OnWriteFile(const std::string& pref,
// time_t* assign, time_t t, bool success) {
//
//}
