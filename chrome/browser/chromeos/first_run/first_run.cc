...
class DialogLauncher : public content::NotificationObserver {
 public:
  ...
  void Observe(int type,
    ...
    if (signin_manager) {
      std::string email = signin_manager->GetAuthenticatedAccountInfo().email;
      if (base::EndsWith(email, "@gmail.com", base::CompareCase::SENSITIVE) ||
          base::EndsWith(email, "@google.com", base::CompareCase::SENSITIVE))
