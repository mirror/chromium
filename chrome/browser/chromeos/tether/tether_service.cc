...
void TetherService::AdapterPoweredChanged(device::BluetoothAdapter* adapter,
  ...
  if (has_set_ble_advertising_interval_) {
    UpdateTetherTechnologyState();
    return;
  }
...
TetherService::GetTetherTechnologyState() {
  if (shut_down_ || suspended_ || !IsBleAdvertisingSupported() ||
...
void TetherService::OnBluetoothAdapterAdvertisingIntervalSet() {
  has_set_ble_advertising_interval_ = true;
  SetIsBleAdvertisingSupported(true);
...
void TetherService::OnBluetoothAdapterAdvertisingIntervalError(
  ...
  has_set_ble_advertising_interval_ = true;
  SetIsBleAdvertisingSupported(false);
...
bool TetherService::IsBleAdvertisingSupported() {
  return profile_->GetPrefs()->GetBoolean(
      prefs::kInstantTetheringBleAdvertisingSupported);
}

void TetherService::SetIsBleAdvertisingSupported(
