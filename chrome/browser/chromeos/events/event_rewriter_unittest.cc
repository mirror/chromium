...
TEST_F(EventRewriterTest, TestRewriteFunctionKeysInvalidLayout) {
  ...
  // Adding a keyboard with invalid layout should not be stored.
  rewriter_->KeyboardDeviceAddedForTesting(
      kKeyboardDeviceId, "PC Keyboard",
      ui::EventRewriterChromeOS::kKbdTopRowInvalidLayout);

  // An invalid layout will fallback to Layout1 in which case keys are rewritten
  ...
  // Readding the same keyboard again with a valid layout will take effect.
  rewriter_->KeyboardDeviceAddedForTesting(
      kKeyboardDeviceId, "PC Keyboard",
      ui::EventRewriterChromeOS::kKbdTopRowLayout2);
