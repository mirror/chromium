...
//  done, so we can't delete it before that. You'll get segmentation fault
//  if it's a class member unique_ptr as the framework also attempts to delete
//  it.
...
class KioskVirtualKeyboardTest : public KioskTest {
 ...
 protected:
  ...
  void TearDown() override {
    KioskVirtualKeyboardTestSoundsManagerTestImpl::Delete();
    KioskTest::TearDown();
  }
  void SetUpCommandLine(base::CommandLine* command_line) override {
