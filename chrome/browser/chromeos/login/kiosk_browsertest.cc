...
class KioskVirtualKeyboardTest : public KioskTest {
 public:
  KioskVirtualKeyboardTest()
      : mock_audio_manager_(new media::MockAudioManager(
            base::MakeUnique<media::AudioThreadImpl>())) {}
  ~KioskVirtualKeyboardTest() override { mock_audio_manager_->Shutdown(); }
...
IN_PROC_BROWSER_TEST_F(KioskVirtualKeyboardTest, RestrictFeatures) {
  // voiceinput returned by GetKeyboardConfig depends on the existence of audio
  // input device. We mock it avoiding test from depending on actual device.
  mock_audio_manager_->SetHasInputDevices(true);
  // Assign to a variable not to be destroyed immediately.
  // We cannot do thin in SetUp because it's overriden in RunTestOnMainThread.
  auto unused = media::AudioSystemImpl::Create(mock_audio_manager_.get());
