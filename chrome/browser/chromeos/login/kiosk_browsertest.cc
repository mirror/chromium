...
// An app to test Kiosk virtual keyboard API chrome.virtualKeyboard.* while
// there is no audio input.
// Source files are in
//     chrome/test/data/chromeos/app_mode/virtual_keyboard_no_audio/src/
const char kTestVirtualKeyboardOnNoAudioKioskApp[] =
    "lgdbgjkfohfihknakdlipddibehgbdmb";
...
IN_PROC_BROWSER_TEST_F(KioskVirtualKeyboardTest, RestrictFeaturesWithNoAudio) {
  mock_audio_manager_ = base::MakeUnique<media::MockAudioManager>(
      base::MakeUnique<media::TestAudioThread>());
  mock_audio_manager_->SetHasInputDevices(false);

  set_test_app_id(kTestVirtualKeyboardOnNoAudioKioskApp);
  set_test_app_version("0.1");
  set_test_crx_file(test_app_id() + ".crx");
