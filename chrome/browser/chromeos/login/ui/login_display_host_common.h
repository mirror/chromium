...
class LoginDisplayHostCommon : public LoginDisplayHost {
 public:
  ...
  OobeUI* GetOobeUI() const override = 0;
  WebUILoginView* GetWebUILoginView() const override = 0;
  void BeforeSessionStart() override = 0;
  void Finalize(base::OnceClosure completion_callback) override = 0;
  void SetStatusAreaVisible(bool visible) override = 0;
  void StartWizard(OobeScreen first_screen) override = 0;
  WizardController* GetWizardController() override = 0;
  AppLaunchController* GetAppLaunchController() final;
  void StartUserAdding(base::OnceClosure completion_callback) override = 0;
  void CancelUserAdding() override = 0;
  void StartSignInScreen(const LoginScreenContext& context) final;
  void OnPreferencesChanged() override = 0;
  ...
  void StartVoiceInteractionOobe() override = 0;
  bool IsVoiceInteractionOobe() override = 0;
