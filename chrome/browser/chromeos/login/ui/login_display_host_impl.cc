...
LoginDisplayHostImpl::LoginDisplayHostImpl(const gfx::Rect& wallpaper_bounds)
  ...
  // In the beginning of login screen, apply bluetooth power state based on
  // the last saved bluetooth adapter power in local state.
...
static void InitializeBluetoothAdapter(PrefService* prefs,
  ...
  const PrefService::Preference* bluetooth_pref =
      prefs->FindPreference(prefs::kSystemBluetoothAdapterEnabled);
  if (bluetooth_pref->IsDefaultValue()) {
    prefs->SetBoolean(prefs::kSystemBluetoothAdapterEnabled,
                      adapter->IsPowered());
  } else {
    adapter->SetPowered(
        prefs->GetBoolean(prefs::kSystemBluetoothAdapterEnabled),
