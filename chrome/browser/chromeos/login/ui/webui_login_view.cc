...
void WebUILoginView::SetStatusAreaVisible(bool visible) {
  if (!ash_util::IsRunningInMash()) {
    // Close system tray when it becomes invisible.
    if (!visible && ash::Shell::Get()->GetPrimarySystemTray()) {
      ash::Shell::Get()->GetPrimarySystemTray()->CloseBubble();
    }
  } else {
    // TODO(crbug.com/782072): In mash, system tray menu can be left open after
    // the status area becomes invisible.
    NOTIMPLEMENTED();
  }
