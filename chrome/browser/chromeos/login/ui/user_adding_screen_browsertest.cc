...
IN_PROC_BROWSER_TEST_F(UserAddingScreenTest, PRE_CancelAdding) {
  RegisterUser(AccountId::FromUserEmailGaiaId(kTestUsers[0].email,
                                              kTestUsers[0].gaia_id));
  RegisterUser(AccountId::FromUserEmailGaiaId(kTestUsers[1].email,
                                              kTestUsers[1].gaia_id));
  RegisterUser(AccountId::FromUserEmailGaiaId(kTestUsers[2].email,
                                              kTestUsers[2].gaia_id));
...
IN_PROC_BROWSER_TEST_F(UserAddingScreenTest, CancelAdding) {
  ...
  const AccountId account_id0(AccountId::FromUserEmailGaiaId(
      kTestUsers[0].email, kTestUsers[0].gaia_id));

  LoginUser(account_id0);
...
IN_PROC_BROWSER_TEST_F(UserAddingScreenTest, AddingSeveralUsers) {
  ...
  for (size_t i = 0; i < arraysize(kTestUsers); ++i) {
    test_users.emplace_back(AccountId::FromUserEmailGaiaId(
  ...
  EXPECT_EQ(test_users[0], unlock_users[0]->GetAccountId());
  ...
  for (int i = 0; i < 3; ++i)
    EXPECT_EQ(test_users[i], unlock_users[i]->GetAccountId());
...
IN_PROC_BROWSER_TEST_F(UserAddingScreenTest, ScreenVisibility) {
  LoginUser(AccountId::FromUserEmailGaiaId(kTestUsers[0].email,
                                           kTestUsers[0].gaia_id));
