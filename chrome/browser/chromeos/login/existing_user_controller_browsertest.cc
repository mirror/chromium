...
class ExistingUserControllerPublicSessionTest
 ...
 protected:
  ...
  void VerifySyncPasswordPrefs(Profile* profile) {
    /*DCHECK(browser());
    DCHECK(browser()->profile());
    DCHECK(browser()->profile()->GetPrefs());*/
...
IN_PROC_BROWSER_TEST_F(ExistingUserControllerPublicSessionTest,
  ...
  // Verify sync password hash is saved to prefs.
  Profile* profile = content::Details<Profile>(profile_prepared_observer.details()).ptr();
  VerifySyncPasswordPrefs(profile);
}
