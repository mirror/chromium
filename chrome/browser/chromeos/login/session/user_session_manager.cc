...
void UserSessionManager::FinalizePrepareProfile(Profile* profile) {
  ...
  if (user_context_.GetSyncPasswordData().has_value()) {
    LOG(ERROR)<<"**UserSessionManager::FinalizePrepareProfile: has sync password data, save it to profile";
    scoped_refptr<password_manager::PasswordStore> password_store =
        PasswordStoreFactory::GetForProfile(profile,
                                            ServiceAccessType::EXPLICIT_ACCESS);
    if (password_store) {
      password_store->SaveSyncPasswordHash(user_context_.GetSyncPasswordData().value());
    }
  }
