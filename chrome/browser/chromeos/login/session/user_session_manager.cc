...
void UserSessionManager::PrepareTpmDeviceAndFinalizeProfile(Profile* profile) {
  ...
  // whenever a user logs in. This is so the TPM is in locked down state after
  // initial setup, which ensures that some decisions (e.g. NVRAM spaces) are
  // unchangeable until next hardware reset (powerwash, recovery, etc.).
  //
