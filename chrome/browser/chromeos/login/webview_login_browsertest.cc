...
class WebviewProxyAuthLoginTest : public WebviewLoginTest {
 public:
  ...
  void SetUpOnMainThread() override { WebviewLoginTest::SetUpOnMainThread(); }
...
IN_PROC_BROWSER_TEST_F(WebviewProxyAuthLoginTest, ProxyAuthTransfer) {
  ...
  // Before entering auth data, make |policy_test_server_ serve a policy that we
  // can use to detect if policies have been fetched.
