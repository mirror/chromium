...
void WallpaperManager::LoadWallpaper(const AccountId& account_id,
  ...
  if (info.type == wallpaper::ONLINE) {
    ...
    DCHECK(ash::WallpaperController::kDirChromeOsWallpapersPathId.has_value());
    CHECK(PathService::Get(
        ash::WallpaperController::kDirChromeOsWallpapersPathId.value(),
        &wallpaper_dir));
    ...
    DCHECK(ash::WallpaperController::kDirUserDataPathId.has_value());
    PathService::Get(ash::WallpaperController::kDirUserDataPathId.value(),
                     &user_data_dir);
