...
void WallpaperManager::ShowSigninWallpaper(const AccountId& account_id) {
  if (!SetDeviceWallpaperIfApplicable(account_id)) {
    GetPendingWallpaper()->SetDefaultWallpaper(account_id);
  }
}

void WallpaperManager::DeleteWallpaper(const AccountId& account_id,
                                       bool show_default_wallpaper) {
  RemoveUserWallpaperInfo(account_id);

  if (show_default_wallpaper) {
    InitializeUserWallpaperInfo(account_id);
    GetPendingWallpaper()->SetDefaultWallpaper(account_id);
  }
}
