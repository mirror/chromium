...
void WallpaperManager::InitializeWallpaper() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  WallpaperControllerClient::Get()->InitializeFilePaths();
...
void WallpaperManager::LoadWallpaper(const AccountId& account_id,
  ...
  if (info.type == wallpaper::ONLINE) {
    ...
    DCHECK(ash::WallpaperController::kDirChromeOsWallpapersPath.empty());
    wallpaper_path =
        ash::WallpaperController::kDirChromeOsWallpapersPath.Append(file_name);
    ...
    DCHECK(!ash::WallpaperController::kDirUserDataPath.empty());
    wallpaper_path =
        ash::WallpaperController::kDirUserDataPath.Append(info.location);
    StartLoad(account_id, info, update_wallpaper, wallpaper_path,
              std::move(on_finish));
  } else {
...
base::FilePath WallpaperManager::GetDeviceWallpaperDir() {
  DCHECK(ash::WallpaperController::kDirChromeOsWallpapersPath.empty());
  return ash::WallpaperController::kDirChromeOsWallpapersPath.Append(
      kDeviceWallpaperDir);
}
