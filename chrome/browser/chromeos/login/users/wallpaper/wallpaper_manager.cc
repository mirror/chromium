...
bool MoveCustomWallpaperDirectory(const char* sub_dir,
  ...
  base::FilePath base_path =
      WallpaperManager::GetCustomWallpaperDir(sub_dir);
...
const base::FilePath&
WallpaperManager::CustomizedWallpaperRescaledFiles::path_downloaded()
    const {
...
bool WallpaperManager::CustomizedWallpaperRescaledFiles::
...
bool WallpaperManager::CustomizedWallpaperRescaledFiles::AllSizesExist()
    const {
...
bool WallpaperManager::TestApi::GetPathFromCache(
    const AccountId& account_id,
    base::FilePath* path) {
  return wallpaper_manager_->GetPathFromCache(account_id, path);
}

void WallpaperManager::TestApi::SetWallpaperCache(
    const AccountId& account_id,
...
// static
void WallpaperManager::SetPathIds(
...
// static
bool WallpaperManager::ResizeImage(
    const gfx::ImageSkia& image,
...
void WallpaperManager::SetUserWallpaperDelayed(
    const AccountId& account_id) {
...
bool WallpaperManager::IsPolicyControlled(
    const AccountId& account_id) const {
  WallpaperInfo info;
...
void WallpaperManager::AddObserver(
    WallpaperManager::Observer* observer) {
  observers_.AddObserver(observer);
}

void WallpaperManager::RemoveObserver(
    WallpaperManager::Observer* observer) {
...
void WallpaperManager::SaveCustomWallpaper(
  ...
  // wallpaper is saved).
  ResizeAndSaveWallpaper(*image, original_path, wallpaper::WALLPAPER_LAYOUT_STRETCH,
...
void WallpaperManager::MoveCustomWallpapersOnWorker(
  ...
  if (MoveCustomWallpaperDirectory(kOriginalWallpaperSubDir,
    ...
    reply_task_runner->PostTask(
        FROM_HERE,
        base::Bind(&WallpaperManager::MoveCustomWallpapersSuccess, weak_ptr,
...
void WallpaperManager::GetCustomWallpaperInternal(
  ...
  if (!base::PathExists(valid_path)) {
    ...
    valid_path = GetCustomWallpaperPath(kOriginalWallpaperSubDir,
                                        wallpaper::WallpaperFilesId::FromString(old_path),
                                        info.location);
  }
  ...
  if (!base::PathExists(valid_path)) {
    reply_task_runner->PostTask(
        FROM_HERE,
        base::Bind(&WallpaperManager::OnCustomWallpaperFileNotFound,
...
void WallpaperManager::ResizeCustomizedDefaultWallpaper(
  ...
  *success &= ResizeAndSaveWallpaper(
      *image, rescaled_files->path_rescaled_small(), wallpaper::WALLPAPER_LAYOUT_STRETCH,
      kSmallWallpaperMaxWidth, kSmallWallpaperMaxHeight, small_wallpaper_image);

  *success &= ResizeAndSaveWallpaper(
      *image, rescaled_files->path_rescaled_large(), wallpaper::WALLPAPER_LAYOUT_STRETCH,
...
void WallpaperManager::InitInitialUserWallpaper(const AccountId& account_id,
  ...
  current_user_wallpaper_info_.layout = wallpaper::WALLPAPER_LAYOUT_CENTER_CROPPED;
...
void WallpaperManager::CacheUserWallpaper(const AccountId& account_id) {
  ...
  if (GetUserWallpaperInfo(account_id, &info)) {
    ...
    if (info.type == wallpaper::CUSTOMIZED || info.type == wallpaper::POLICY || info.type == wallpaper::DEVICE) {
      base::FilePath wallpaper_path;
...
void WallpaperManager::LoadWallpaper(
    const AccountId& account_id,
    const WallpaperInfo& info,
    bool update_wallpaper,
    MovableOnDestroyCallbackHolder on_finish) {
...
const char*
WallpaperManager::GetCustomWallpaperSubdirForCurrentResolution() {
  WallpaperResolution resolution = GetAppropriateResolution();
