...
class WallpaperManager : public ash::mojom::WallpaperPicker,
 ...
 public:
  ...
  void OnPolicySet(const std::string& policy,
                           const AccountId& account_id);

  // Called when the wallpaper policy has been cleared for |account_id|.
  void OnPolicyCleared(const std::string& policy,
                               const AccountId& account_id);
 ...
 private:
  ...
  static void SaveCustomWallpaper(const wallpaper::WallpaperFilesId& wallpaper_files_id,
                                  const base::FilePath& path,
                                  wallpaper::WallpaperLayout layout,
  ...
  bool GetPathFromCache(const AccountId& account_id,
                                base::FilePath* path);
