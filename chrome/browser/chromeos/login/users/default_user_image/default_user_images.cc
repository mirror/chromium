...
const int kFirstHarmonyDefaultImageIndex = 34;
...
const int kDefaultImageWebsuteMaxID = arraysize(kDefaultImageWebsiteIDs);
...
void GetFirstLastIndex(int* first, int* last) {
  if (!base::CommandLine::ForCurrentProcess()->HasSwitch(
          switches::kDisableHarmonyUserImages)) {
    *first = kFirstHarmonyDefaultImageIndex;
    *last = kDefaultImagesCount - 1;
  } else {
    *first = kFirstDefaultImageIndex;
    *last = kFirstHarmonyDefaultImageIndex - 1;
  }
}
...
std::unique_ptr<base::ListValue> GetAsDictionary(bool all) {
  ...
  for (int i = first; i <= last; ++i) {
    ...
    if (i < kDefaultImageWebsuteMaxID) {
...
int GetFirstDefaultImage() {
  return base::CommandLine::ForCurrentProcess()->HasSwitch(
             ...
             : kFirstHarmonyDefaultImageIndex;
}
