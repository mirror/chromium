...
bool DecodeMigrationActionFromPolicy(
    ...
    arc::policy_util::EcryptfsMigrationAction* out_value) {
  ...
  if (policy_proto.value() < 0 ||
      policy_proto.value() >=
          static_cast<int64_t>(
              arc::policy_util::kEcryptfsMigrationActionCount)) {
    return false;
  }

  *out_value = static_cast<arc::policy_util::EcryptfsMigrationAction>(
      policy_proto.value());
  return true;
}
...
void ExistingUserController::OnPolicyFetchResult(
  ...
  if (result == PolicyFetchResult::NO_POLICY) {
    ...
    action = arc::policy_util::EcryptfsMigrationAction::kAskUser;
    ...
    action = arc::policy_util::EcryptfsMigrationAction::kDisallowMigration;
  }
  VLOG(1) << "Migration action: " << static_cast<int>(action);

  switch (action) {
    case arc::policy_util::EcryptfsMigrationAction::kMigrate:
      ShowEncryptionMigrationScreen(user_context,
                                    EncryptionMigrationMode::START_MIGRATION);
      break;

    case arc::policy_util::EcryptfsMigrationAction::kAskUser:
      ShowEncryptionMigrationScreen(user_context,
                                    EncryptionMigrationMode::ASK_USER);
      break;

    case arc::policy_util::EcryptfsMigrationAction::kWipe:
    ...
    case arc::policy_util::EcryptfsMigrationAction::kDisallowMigration:
