...
void ExistingUserController::PerformLogin(
  ...
  // If plain text password is available, compute its salt, hash, and length
  // and save them in |user_context|. It will be saved to prefs when user
  // profile is ready.
  UserContext new_user_context = user_context;
  if (user_context.GetKey()->GetKeyType() == Key::KEY_TYPE_PASSWORD_PLAIN) {
    LOG(ERROR)<<"**Get plaintext password and save it to user_context: "<<new_user_context.GetKey()->GetSecret();
    base::string16 password(
        base::ASCIIToUTF16(new_user_context.GetKey()->GetSecret()));
