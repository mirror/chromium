...
namespace {
const char kEmbedderUrl[] = "http://www.whatever.com/";
}  // namespace
...
class SigninPartitionManagerTest : public ChromeRenderViewHostTestHarness {
 protected:
  ...
  void FinishAllClearPartitionTasks() {
    for (const auto& task : pending_clear_tasks_) {
      task.second.Run();
    }
 ...
 private:
  void ClearStoragePartitionTask(content::StoragePartition* partition,
                                 base::RepeatingClosure clear_done_closure) {
    pending_clear_tasks_.push_back({partition, clear_done_closure});
  }
  ...
  std::vector<std::pair<content::StoragePartition*, base::RepeatingClosure>>
