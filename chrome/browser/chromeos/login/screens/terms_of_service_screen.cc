...
void TermsOfServiceScreen::StartDownload() {
  ...
  // Start downloading the Terms of Service.
  terms_of_service_loader__ = content::SimpleURLLoader::Create();
  ...
  terms_of_service_loader_->DownloadToStringOfUnboundedSizeUntilCrashAndDie(
      resource_request, loader_factory, null,
      base::BindOnce(&TermsOfServiceScreen::OnCompleteCallback,
...
void TermsOfServiceScreen::OnCompleteCallback(
  ...
  if (loader.ResponseInfo().mime_type != "text/plain" || response_body == "") {
    view_->OnLoadError();
  } else {
