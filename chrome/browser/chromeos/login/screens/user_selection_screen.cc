...
class UserSelectionScreen::DircryptoMigrationChecker {
 ...
 private:
  ...
  void OnCryptohomeNeedsDircryptoMigrationCallback(
      ...
      base::Optional<bool> needs_migration_response) {
    if (!needs_migration_response) {
    ...
    needs_dircrypto_migration_cache_[account_id] =
        needs_migration_response.value();
    UpdateUI(account_id, needs_migration_response.value());
  }
