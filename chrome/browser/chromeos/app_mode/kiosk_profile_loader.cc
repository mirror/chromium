...
class KioskProfileLoader::CryptohomedChecker
 ...
 private:
  ...
  void OnCryptohomeIsMounted(base::Optional<bool> is_mounted_response) {
    if (!is_mounted_response) {
      Retry();
      return;
    }

    if (!is_mounted_response.value())
    ...
    if (!is_mounted_response.value() || !base::SysInfo::IsRunningOnChromeOS())
