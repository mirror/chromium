...
TEST_F(StartupAppLauncherTest, LaunchWithSecondaryApps) {
  ...
  primary_app_builder.AddSecondaryExtensionDisabledOnStartup(
      kExtraSecondaryAppId);
...
TEST_F(StartupAppLauncherTest, IgnoreDisableOnStartupOnStableChannel) {
  ...
  TestKioskExtensionBuilder primary_app_builder(Manifest::TYPE_PLATFORM_APP,
                                                kTestPrimaryAppId);
  primary_app_builder.AddSecondaryExtensionDisabledOnStartup(kSecondaryAppId);
  ASSERT_TRUE(DownloadAndInstallPrimaryApp(primary_app_builder));
  ...
  EXPECT_TRUE(registry()->enabled_extensions().Contains(kTestPrimaryAppId));
  EXPECT_FALSE(registry()->GetInstalledExtension(kSecondaryAppId));
}

TEST_F(StartupAppLauncherTest,
       IgnoreDisableOnStartupOnStableChannel_AdditionalSecondaryAppEntry) {
  extensions::ScopedCurrentChannel channel(version_info::Channel::STABLE);

  InitializeLauncherWithNetworkReady();

  // Add an entry for the secondary app with both "disable_on_startup" and
  // without it - given that "disable_on_startup" should be restricted to dev
  // channel, the former entry should be ignored, and the latter should take
  // effect (i.e. the secondary extension should be loaded enabled).
  TestKioskExtensionBuilder primary_app_builder(Manifest::TYPE_PLATFORM_APP,
                                                kTestPrimaryAppId);
  primary_app_builder.AddSecondaryExtensionDisabledOnStartup(kSecondaryAppId);
  primary_app_builder.AddSecondaryExtension(kSecondaryAppId);
...
TEST_F(StartupAppLauncherTest,
       DisableSecondaryAppOnStartupOverridesInstalledAppState) {
  TestKioskExtensionBuilder primary_app_builder(Manifest::TYPE_PLATFORM_APP,
                                                kTestPrimaryAppId);
  primary_app_builder.AddSecondaryExtensionDisabledOnStartup(kSecondaryAppId);
...
TEST_F(StartupAppLauncherTest, PrimaryAppUpdatesToRemoveDisabledOnStartup) {
  TestKioskExtensionBuilder primary_app_builder(Manifest::TYPE_PLATFORM_APP,
                                                kTestPrimaryAppId);
  primary_app_builder.AddSecondaryExtension(kSecondaryAppId);
  primary_app_builder.set_version("1.0");
  ...
  TestKioskExtensionBuilder primary_app_update(Manifest::TYPE_PLATFORM_APP,
                                               kTestPrimaryAppId);
  primary_app_update.AddSecondaryExtensionDisabledOnStartup(kSecondaryAppId);
...
TEST_F(StartupAppLauncherTest, PrimaryAppUpdatesToAddDisabledOnStartup) {
  TestKioskExtensionBuilder primary_app_builder(Manifest::TYPE_PLATFORM_APP,
                                                kTestPrimaryAppId);
  primary_app_builder.AddSecondaryExtensionDisabledOnStartup(kSecondaryAppId);
  primary_app_builder.set_version("1.0");
  ...
  TestKioskExtensionBuilder primary_app_update(Manifest::TYPE_PLATFORM_APP,
                                               kTestPrimaryAppId);
  primary_app_update.AddSecondaryExtension(kSecondaryAppId);
  primary_app_update.set_version("1.1");
...
TEST_F(StartupAppLauncherTest,
       DoNotChangeSecondaryExtensionStateOnSessionRestore) {
  TestKioskExtensionBuilder primary_app_builder(Manifest::TYPE_PLATFORM_APP,
                                                kTestPrimaryAppId);
  primary_app_builder.AddSecondaryExtensionDisabledOnStartup(kSecondaryAppId);
  primary_app_builder.AddSecondaryExtension(kExtraSecondaryAppId);
