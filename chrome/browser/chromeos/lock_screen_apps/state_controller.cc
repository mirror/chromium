...
void StateController::SetLockScreenProfileLoaderForTesting(
    std::unique_ptr<ProfileLoader> profile_loader) {
  DCHECK(!lock_screen_profile_loader_);
  lock_screen_profile_loader_ = std::move(profile_loader);
}
...
void StateController::Shutdown() {
  ...
  lock_screen_profile_loader_.reset();
...
void StateController::InitializeWithCryptoKey(Profile* profile,
  ...
  if (!lock_screen_profile_loader_)
    lock_screen_profile_loader_ = base::MakeUnique<ProfileLoaderImpl>(profile);
  ...
  app_manager_->Initialize(profile, lock_screen_profile_loader_.get());
...
extensions::AppWindow* StateController::CreateAppWindowForLockScreenAction(
  ...
  // lock screen profile has not been loaded, and |lock_screen_profile_loader_|
  ...
  DCHECK(lock_screen_profile_loader_->lock_screen_profile());

  if (!lock_screen_profile_loader_->lock_screen_profile()->IsSameProfile(
