...
// Key for user pref that contains AES key that should be used to encrypt
// persisted user data created on lock screen.
...
void StateController::OnProfilesReady(Profile* primary_profile,
  ...
  GetUserCryptoKey(primary_profile,
                   base::Bind(&StateController::InitializeWithCryptoKey,
                              weak_ptr_factory_.GetWeakPtr(), primary_profile));
}

void StateController::GetUserCryptoKey(Profile* profile,
                                       const CryptoKeyCallback& callback) {
  std::string key = profile->GetPrefs()->GetString(kDataCryptoKeyPref);
  ...
  base::PostTaskWithTraitsAndReplyWithResult(
      ...
      base::BindOnce(&GenerateCryptoKey),
      base::BindOnce(&StateController::SaveCryptoKey,
                     weak_ptr_factory_.GetWeakPtr(), profile, callback));
}

void StateController::SaveCryptoKey(Profile* profile,
                                    const CryptoKeyCallback& callback,
