...
ToastDialogView::ToastDialogView(const base::string16& app_name,
  ...
  label_->SetEnabledColor(SkColorSetARGB(138, 0, 0, 0));
  label_->SetLineHeight(kDialogMessageLineHeightDp);
  label_->SetFontList(views::Label::GetDefaultFontList().Derive(
      1, gfx::Font::NORMAL, gfx::Font::Weight::NORMAL));
  label_->SetPreferredSize(
      gfx::Size(kDialogWidthDp, label_->GetHeightForWidth(kDialogWidthDp)));
...
void ToastDialogView::OnWorkAreaChanged() {
  LOG(ERROR) << "WORK AREA CHANGED";
  AdjustDialogBounds(GetWidget());
}
...
void ToastDialogView::AdjustDialogBounds(views::Widget* widget) {
  ...
  LOG(ERROR) << bounds.ToString() << " " << parent_bounds.ToString();
