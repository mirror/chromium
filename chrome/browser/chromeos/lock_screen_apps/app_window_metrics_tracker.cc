...
void AppWindowMetricsTracker::Reset() {
  ...
  UMA_HISTOGRAM_ENUMERATION(kAppWindowStateOnRemoval, static_cast<int>(state_),
                            static_cast<int>(State::kCount));
...
void AppWindowMetricsTracker::DocumentLoadedInFrame(
...
void AppWindowMetricsTracker::WebContentsDestroyed() {
  SetState(State::kWindowClosed);
}
