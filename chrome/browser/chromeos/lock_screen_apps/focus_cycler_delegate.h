...
class FocusCyclerDelegate {
 public:
  ...
  // Registers a callbck that should be called when the focus should be moved
  // to the app window.
  virtual void RegisterLockScreenAppFocusHandler(
      const base::Callback<void(bool reverse)>& focus_handler) = 0;
  ...
  // Called when the focus exists the lock screen app window. The delegate
