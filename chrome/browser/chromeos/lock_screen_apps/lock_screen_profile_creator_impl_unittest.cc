...
class PendingProfileCreation : public Profile::Delegate {
 public:
  ...
  void Reset(const base::FilePath& path, Profile::Delegate* delegate) {
 ...
 private:
  void RunWaitQuitClosure() {
    ...
    base::Closure closure = wait_quit_closure_;
    wait_quit_closure_.Reset();
    closure.Run();
  }
...
class UnittestProfileManager : public ::ProfileManagerWithoutInit {
 ...
 protected:
  ...
  Profile* CreateProfileAsyncHelper(const base::FilePath& path,
    ...
    pending_profile_creation_.Reset(path, delegate);
...
class LockScreenProfileCreatorImplTest : public testing::Test {
 ...
 private:
  ...
  void AddTestUserProfile() {
    base::FilePath user_profile_path = user_data_dir_.GetPath().Append(
        chromeos::ProfileHelper::Get()->GetUserProfileDir(
...
TEST_F(LockScreenProfileCreatorImplTest,
  ...
  ASSERT_EQ(chromeos::ProfileHelper::GetLockScreenAppProfilePath(),
  ...
  EXPECT_TRUE(
      chromeos::ProfileHelper::IsLockScreenAppProfile(lock_screen_profile));
  EXPECT_FALSE(
...
TEST_F(LockScreenProfileCreatorImplTest, MetricsOnSuccess) {
  ...
  histogram_tester->ExpectTimeBucketCount(
      "Apps.LockScreen.TimeToCreateLockScreenAppsProfile",
      base::TimeDelta::FromMilliseconds(20), 1);
  histogram_tester->ExpectUniqueSample(
      "Apps.LockScreen.LockScreenAppsProfileCreationSuccess", 1, 1);
}
...
TEST_F(LockScreenProfileCreatorImplTest, MetricsOnFailure) {
  ...
  histogram_tester->ExpectTotalCount(
      "Apps.LockScreen.TimeToCreateLockScreenAppsProfile", 0);
  histogram_tester->ExpectUniqueSample(
      "Apps.LockScreen.LockScreenAppsProfileCreationSuccess", 0, 1);
}
