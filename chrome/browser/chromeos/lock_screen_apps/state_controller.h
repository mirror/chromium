...
class StateController : public ash::mojom::TrayActionClient,
 ...
 private:
  using CryptoKeyCallback = base::Callback<void(const std::string& key)>;
  ...
  // |callback| - called with the created key.
  void GetUserCryptoKey(Profile* profile, const CryptoKeyCallback& callback);

  // Saves encryption key used to encrypt data created on the lock screen to the
  // user profile.
  // |callback| - called with the saved key value.
  void SaveCryptoKey(Profile* profile,
                     const CryptoKeyCallback& callback,
                     const std::string& key);

  // Finishes lock screen apps initialization with primary user profile and
  // associated encryption key to be used for encrypting user data from lock
  // screen when it's persisted.
