...
void AppManagerImpl::Initialize(
    ...
    LockScreenProfileCreator* lock_screen_profile_loader) {
  ...
  lock_screen_profile_loader_->AddCreateProfileCallback(
...
std::string AppManagerImpl::FindLockScreenNoteTakingApp() const {
  ...
  // on the lock screen, so the app availability should be reported to UMA in
  // that case, regardless of lock screen profile state.
  // If the app is availble, and lock_screen_profile is not set, the profile
  // might still be loading, and this will be called again when the profile is
  // loaded - until then, ignore the avaialble note taking app and report that
  // lock screen profile was not created at this point.
