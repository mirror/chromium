...
// Number of times the lock screen app is allowed to be relaoded after the
// app got unloaded due to an extension crash.
...
bool AppManagerImpl::LaunchNoteTaking() {
  ...
  // disabled, or it failed to reload (in case it was prevoidly terminated).
  // In either case, note taking should not be reported as available - moving
  // to error state will achieve this.
  if (!app) {
