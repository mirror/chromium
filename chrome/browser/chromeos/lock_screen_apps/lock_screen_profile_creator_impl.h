...
class LockScreenProfileCreatorImpl
 ...
 private:
  ...
  // is called more than once for a single profile). |start_time| - time at
  // which the profile creation started. |profile| - the created profile - i.e.
  // the lock screen profile. |status| - profile creation status.
