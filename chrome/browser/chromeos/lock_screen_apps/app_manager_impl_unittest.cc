...
class LockScreenAppManagerImplTest
 ...
 public:
  ...
  void SetUpTestEventRouter() {
    ...
    ASSERT_TRUE(event_router_);

    event_observer_ = std::make_unique<LockScreenEventObserver>(
        lock_screen_profile()->GetOriginalProfile());
    event_router_->AddEventObserver(event_observer_.get());
  }
  ...
  const LockScreenEventRouter* event_router() { return event_router_; }
 ...
 private:
  ...
  LockScreenEventRouter* event_router_ = nullptr;
