...
class AppWindowMetricsTracker : public content::WebContentsObserver {
 ...
 private:
  ...
  // Whether the app window is reported to be in background. Used to properly
  // initialize state if the window is backgrounded before the app window
  // document has been fully loaded.
  bool window_backgrounded_ = false;
