...
class LockScreenAppStateTest : public BrowserWithTestWindowTest {
 public:
  ...
  // Exposed so test fixtures can override default (emoty) command line.
...
// Testts with show-md-login flag set.
...
TEST_F(LockScreenAppStateTest, LaunchActionWhenStylusGetsRemoved) {
  ...
  ExpectObservedStatesMatch({TrayActionState::kLaunching},
                            "Launch on new note request");
  ClearObservedStates();
  // The app should not be launched until the lock UI reports the animation to
  // be complete.
...
TEST_F(LockScreenAppStateTest, StylusRemovedWhileScreenOff) {
  ...
  // State should not change if the stylu sis removed while the screen is off.
  EXPECT_EQ(TrayActionState::kAvailable,
            state_controller()->GetLockScreenNoteState());
