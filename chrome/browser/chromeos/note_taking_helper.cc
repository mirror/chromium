...
NoteTakingAppInfos NoteTakingHelper::GetAvailableApps(Profile* profile) {
  ...
  for (const auto* app : chrome_apps) {
    NoteTakingLockScreenSupport lock_screen_support =
        GetLockScreenSupportForChromeApp(profile, extension_prefs, app);
    infos.push_back(
...
std::unique_ptr<NoteTakingAppInfo> NoteTakingHelper::GetPreferredChromeAppInfo(
  ...
  info->lock_screen_support = GetLockScreenSupportForChromeApp(
      profile, extensions::ExtensionPrefs::Get(profile), preferred_app);
  return info;
}
...
bool NoteTakingHelper::SetAppEnabledOnLockScreen(Profile* profile,
  ...
  NoteTakingLockScreenSupport current_state = GetLockScreenSupportForChromeApp(
      profile, extensions::ExtensionPrefs::Get(profile), app);
