...
constexpr base::TimeDelta kCacheExpiration = base::TimeDelta::FromSeconds(3);
...
void RecentModel::GetRecentFiles(RecentContext context,
  ...
  DCHECK(!intermediate_files_.has_value());
  DCHECK_EQ(0, num_inflight_sources_);

  intermediate_files_ = RecentFileList();
...
void RecentModel::OnGetRecentFiles(RecentFileList files) {
  ...
  intermediate_files_.value().insert(intermediate_files_.value().end(),
...
void RecentModel::OnGetRecentFilesCompleted() {
  ...
  cached_files_.swap(intermediate_files_);
  ...
  std::vector<GetRecentFilesCallback> pending_callbacks;
