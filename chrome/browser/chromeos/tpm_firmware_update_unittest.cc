...
class ShouldOfferUpdateViaPowerwashTest : public testing::Test {
 public:
  ...
  void RecordResponse(bool available) {
    ...
    if (quit_runloop_closure_) {
      quit_runloop_closure_.Run();
    }
  }
  ...
  void SetUpdateAvailability(Availability availability) {
    ...
    switch (availability) {
      ...
      case Availability::kUnavailble:
        ASSERT_TRUE(base::WriteFile(update_location_path, "", 0));
        break;
      case Availability::kAvailable:
        const char kUpdatePath[] = "/lib/firmware/tpm/firmware.bin";
        ASSERT_TRUE(base::WriteFile(update_location_path, kUpdatePath,
                                    sizeof(kUpdatePath) - 1));
        break;
    }
  }
  ...
  base::Closure quit_runloop_closure_;
};
...
TEST_F(ShouldOfferUpdateViaPowerwashTest, AvailableAfterWaiting) {
  ...
  // ScopedTaskEnvironment::RunUntilIdle(). This is because
  // ScopedTaskEnvironment doesn't know about file system events that haven't
  // fired and propagated to a task scheduler thread yet so may return early
  // before the file system event is received. An event is expected here though,
  // so set up a RunLoop that will only exit once the event shows up. If the
  // event doesn't show up, the test will time out. This isn't ideal, but better
  // than flakiness due to file system events racing with RunUntilIdle().
  base::RunLoop run_loop;
  quit_runloop_closure_ = run_loop.QuitWhenIdleClosure();
  ...
  quit_runloop_closure_ = base::Closure();
}
