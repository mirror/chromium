...
void NoteTakingControllerClient::OnAvailableNoteTakingAppsUpdated() {
  bool should_be_bound = profile_ && helper_->IsAppAvailable(profile_);

  // The client should be bound iff there is a note taking app available.
  if (should_be_bound == binding_.is_bound())
    return;
