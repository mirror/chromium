...
UserPolicyManagerFactoryChromeOS::CreateManagerForProfile(
  ...
  } else if (policy_check_required && !is_stub_user) {
    enforcement_type = PolicyEnforcement::kServerCheckRequired;
  }

  DLOG(ERROR) << "is_stub_user=" << is_stub_user;
  DLOG(ERROR) << "cannot_tell_if_policy_required ="
              << cannot_tell_if_policy_required;
  ...
  if (is_active_directory) {
    ...
    std::unique_ptr<UserCloudPolicyManagerChromeOS> manager =
        std::make_unique<UserCloudPolicyManagerChromeOS>(
            ...
            enforcement_type, account_id, base::ThreadTaskRunnerHandle::Get(),
            io_task_runner);
