...
UserPolicyManagerFactoryChromeOS::CreateManagerForProfile(
  ...
  // We want to block for policy (potentially requiring an online policy check
  // if there is no cached policy) if the session has never been initialized
  // (generally true if the user is new, or if there was a crash before the
  // profile finished initializing) or if we have cached policy previously.
  // There is code in UserSelectionScreen to force an online signin for
  // uninitialized sessions to help ensure we are able to load policy.
  //
