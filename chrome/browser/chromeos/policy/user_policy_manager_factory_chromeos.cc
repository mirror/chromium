...
UserPolicyManagerFactoryChromeOS::CreateManagerForProfile(
  ...
  if (cannot_tell_if_policy_required && force_immediate_load && !is_stub_user) {
  ...
  // If there's a chance the user might be managed (either policy_required or
  // policy_check_required is true) then we can't let the profile complete
  // initialization until we complete a policy check.
  //
  ...
  const bool block_profile_init_on_policy_refresh =
      !is_stub_user && !force_immediate_load &&
      (policy_required || policy_check_required);
  ...
  if (block_profile_init_on_policy_refresh) {
    if (policy_required) {
      ...
      // server.
      initial_policy_fetch_timeout = kPolicyRefreshTimeout;
    } else {
      DCHECK(policy_check_required);
      // We don't know yet whether we need policy or not, which implies that
      // we *must* contact DMServer, so block forever (never time out).
