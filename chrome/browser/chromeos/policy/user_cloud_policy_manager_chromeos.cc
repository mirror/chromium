...
UserCloudPolicyManagerChromeOS::UserCloudPolicyManagerChromeOS(
  ...
  DLOG(ERROR) << "POLICY TIMEOUT: " << initial_policy_fetch_timeout;
  DLOG(ERROR) << "POLICY REQUIRED: " << policy_required;
  ...
  if (waiting_for_initial_policy_fetch_ &&
    ...
    // We should never allow a timeout if policy is required (should wait as
    // long as necessary to get policy).
    DCHECK(!policy_required)
        << "initial_policy_fetch_timeout should be max if policy required";
...
void UserCloudPolicyManagerChromeOS::Connect(
  ...
  if (service()->IsInitializationComplete()) {
    DLOG(ERROR) << "INITIALIZATION IS ALREADY COMPLETE";
...
void UserCloudPolicyManagerChromeOS::OnStoreLoaded(
  ...
  if (policy_data) {
    ...
    // We successfully loaded policy from disk, so it's OK if our server fetch
    // fails.
