...
void CachedPolicyKeyLoaderChromeOS::EnsurePolicyKeyLoaded(
  ...
  key_loaded_callbacks_.push_back(callback);
...
void CachedPolicyKeyLoaderChromeOS::ReloadPolicyKey(
  ...
  if (key_load_in_progress_) {
    // When a load is in progress, only set a flag that a reload should be
    // performed immediately when the current load finishes.
    key_reload_pending_ = true;
    return;
  }
...
void CachedPolicyKeyLoaderChromeOS::OnPolicyKeyLoaded(const std::string& key) {
  ...
  if (key_reload_pending_) {
    // When a reload was requested while the current load was in progress,
    // reload the key now instead of notifying callbacks.
    key_reload_pending_ = false;
...
void CachedPolicyKeyLoaderChromeOS::NotifyAndClearCallbacks() {
  for (const auto& callback : key_loaded_callbacks_)
    callback.Run();
