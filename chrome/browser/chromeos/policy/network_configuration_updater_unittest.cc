 ...
 public:
  ...
  void ImportCertificates(
    ...
    if (expected_onc_certificates_) {
      ...
      EXPECT_EQ(certificates.server_or_authority_certificates(),
                expected_onc_certificates_->server_or_authority_certificates());
      EXPECT_EQ(certificates.client_certificates(),
                expected_onc_certificates_->client_certificates());
    }

    ++call_count_;
    std::move(done_callback)
        .Run(true, net::x509_util::DupCERTCertificateList(
                       std::move(onc_trusted_certificates_)));
  }
