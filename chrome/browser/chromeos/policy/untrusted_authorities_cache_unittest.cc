...
class UntrustedAuthoritiesCacheTest : public testing::Test {
 ...
 protected:
  ...
  std::vector<std::string> GetAuthoritiesFromFiles(
...
// Additionally, checks that the certificate is not available after the
// UntrustedAuthorityCache goes out of scope.
TEST_F(UntrustedAuthoritiesCacheTest, CertMadeAvailable) {
  base::FilePath cert_file_path =
      net::GetTestCertsDirectory().Append(FILE_PATH_LITERAL("client_1_ca.pem"));
  {
    UntrustedAuthoritiesCache cache(GetAuthoritiesFromFiles({cert_file_path}));
  ...
  bool cert_available_no_cache = false;
  ASSERT_NO_FATAL_FAILURE(
