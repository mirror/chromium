...
bool IntervalCoversWeeklyTime(const Interval& interval, const WeeklyTime& w) {
  ...
  if (interval_duration == 0)
    return false;
  return GetDuration(interval.start, w) + GetDuration(w, interval.end) ==
         interval_duration;
}
...
void DeviceOffHoursController::NotifyOffHoursModeNeedsUpdate() const {
  VLOG(1) << "OffHours mode needs to be updated.";
...
void DeviceOffHoursController::UpdateOffHoursMode(
  ...
  for (const auto& interval : intervals) {
    if (IntervalCoversWeeklyTime(interval, cur_t)) {
      int duration = GetDuration(cur_t, interval.end);
      SetOffHoursStartTime(current_time);
