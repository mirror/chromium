...
WeeklyTime ConvertWeeklyTimeToUTC(WeeklyTime weekly_time, int gmt_offset) {
...
std::vector<Interval> ConvertIntervalsToGMT(
    const std::vector<Interval>& intervals,
    int gmt_offset) {
  std::vector<Interval> gmt_intervals;
  for (const auto& interval : intervals) {
    auto gmt_start = ConvertWeeklyTimeToUTC(interval.start(), gmt_offset);
    auto gmt_end = ConvertWeeklyTimeToUTC(interval.end(), gmt_offset);
    gmt_intervals.push_back(Interval(gmt_start, gmt_end));
  }
...
void DeviceOffHoursController::UpdateOffHoursPolicy(
  ...
  std::string timezone = device_settings_proto.device_off_hours().timezone();
  int gmt_offset = 0;
  bool no_offset_error = GetTimezoneOffset(timezone, &gmt_offset);
  if (!no_offset_error)
    off_hours_intervals_ = {};
  off_hours_intervals_ =
      ConvertIntervalsToGMT(off_hours_intervals_, gmt_offset);
  UpdateOffHoursMode();
}
