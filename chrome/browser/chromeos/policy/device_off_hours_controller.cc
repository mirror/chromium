...
// structure.
std::vector<int> GetIgnoredPoliciesProtoTags(
    const em::DeviceOffHoursProto& container) {
  return std::vector<int>(container.ignored_policies_proto_tags().begin(),
                          container.ignored_policies_proto_tags().end());
}
...
std::unique_ptr<base::DictionaryValue> ConvertOffHoursProtoToValue(
  ...
  std::vector<int> ignored_policies_proto_tags =
      GetIgnoredPoliciesProtoTags(container);
  auto ignored_policies_value = base::MakeUnique<base::ListValue>();
  for (const auto& policy : ignored_policies_proto_tags)
...
std::unique_ptr<em::ChromeDeviceSettingsProto> ApplyOffHoursPolicyToProto(
  ...
  RemovePolicies(policies.get(), ignored_policies_proto_tags);
  return policies;
}
