...
"""Create function which removes some device policies
...
def main():
  ...
  file.write('em::ChromeDeviceSettingsProto RemovePolicies(\n')
  file.write('    const em::ChromeDeviceSettingsProto& input_policies,\n')
  file.write('    const std::set<std::string>& policies_to_remove) {\n')
  ...
  for group in settings.DESCRIPTOR.fields:
    file.write('  if (policies_to_remove.find("' + group.name + '") != ')
    file.write('policies_to_remove.end()) {\n')
