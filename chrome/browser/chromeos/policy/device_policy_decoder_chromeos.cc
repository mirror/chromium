...
void DecodeGenericPolicies(const em::ChromeDeviceSettingsProto& policy,
  ...
  if (policy.has_off_hours()) {
    ...
    std::unique_ptr<base::DictionaryValue> off_hours(
        new base::DictionaryValue());
    ...
    for (const auto& entry : container.interval()) {
      std::unique_ptr<base::DictionaryValue> interval(
          new base::DictionaryValue());
      if (entry.has_start()) {
        interval->SetDictionary("start", ParseWeeklyTime(entry.start()));
      }
      if (entry.has_end()) {
        interval->SetDictionary("end", ParseWeeklyTime(entry.end()));
      }
