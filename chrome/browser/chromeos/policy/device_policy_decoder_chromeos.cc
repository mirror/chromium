...
void DecodeGenericPolicies(const em::ChromeDeviceSettingsProto& policy,
  ...
  if (policy.has_device_off_hours()) {
    ...
    for (const auto& entry : container.interval()) {
      ...
      bool is_valid = true;
      if (entry.has_start()) {
        std::unique_ptr<base::DictionaryValue> start =
            ParseTimestamp(entry.start());
        if (start) {
          interval->SetDictionary("start", start);
        } else {
          is_valid = false;
        }
      }
      if (entry.has_end()) {
        std::unique_ptr<base::DictionaryValue> end =
            ParseTimestamp(entry.end());
        if (end) {
          interval->SetDictionary("end", end);
        } else {
          is_valid = false;
        }
      }
