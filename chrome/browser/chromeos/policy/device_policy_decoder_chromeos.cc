...
std::unique_ptr<base::DictionaryValue> ParseWeeklyTime(
  ...
  if (weekly_time.has_timezone()) {
    timezone = weekly_time.timezone();
  }
  weekly_time_res->SetString("timezone", std::move(timezone));
  if (is_valid) {
...
void DecodeGenericPolicies(const em::ChromeDeviceSettingsProto& policy,
  ...
  if (policy.has_device_off_hours()) {
    ...
    off_hours->SetList("policies", std::move(policy));
