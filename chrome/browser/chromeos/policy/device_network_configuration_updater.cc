...
void DeviceNetworkConfigurationUpdater::GetAuthorityCertificates(
    net::CertificateList* out_authority_certs) {
  ...
  for (const chromeos::onc::OncParsedCertificates::ServerOrAuthorityCertificate&
    ...
    if (onc_certificate.type() ==
        chromeos::onc::OncParsedCertificates::CertificateType::kAuthority) {
      out_authority_certs->push_back(onc_certificate.certificate());
    }
  }
}
