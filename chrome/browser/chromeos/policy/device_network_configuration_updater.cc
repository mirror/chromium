...
void DeviceNetworkConfigurationUpdater::ImportCertificates(
  ...
  // Importing CA and server certs from device policy is not allowed.
  // However, we pass all CA certificates to |untrusted_authorities_cache_|
  // which parses them and keeps them in memory so they are available for client
  // certificate discovery on the sign-in screen.
