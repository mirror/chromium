 ...
 protected:
  ...
  base::FilePath user_policy_key_file() {
    ...
    cryptohome_client_.GetSanitizedUsername(
        cryptohome_id_, base::Bind(
                            [](std::string* out_sanitized_username,
                               chromeos::DBusMethodCallStatus status,
                               const std::string& sanitized_username) {
                              ASSERT_EQ(chromeos::DBUS_METHOD_CALL_SUCCESS,
                                        status);
                              *out_sanitized_username = sanitized_username;
                            },
                            &sanitized_username));
    base::RunLoop().RunUntilIdle();
...
TEST_F(UserCloudPolicyStoreChromeOSTest, StoreWithoutPolicyKey) {
  ...
  cryptohome_client_.SetUsernameSanitizationReady(false);
