...
class PreSigninPolicyFetcher : public CloudPolicyClient::Observer {
 public:
  ...
  // result.
  void GetPolicy(PolicyFetchResultCallback callback);
 ...
 private:
  ...
  void OnPolicyBlobRetrieved(
      const std::string& policy_blob,
      chromeos::SessionManagerClient::RetrievePolicyResponseType response_type);

  void OnPolicyKeyLoaded(
      const std::string& policy_blob,
      chromeos::SessionManagerClient::RetrievePolicyResponseType response_type);

  void OnUnmountAfterPolicyBlobRetrieved(
      const std::string& policy_blob,
      chromeos::SessionManagerClient::RetrievePolicyResponseType response_type,
  ...
  // Invokes |callback_| with the passed result.
  void NotifyCallback(
      PolicyFetchResult result,
  ...
  // Callback passed to GetPolicy method.
  PolicyFetchResultCallback callback_;

  // This will hold the cached policy after it has been loaded and validated,
  // and will be reset to the fresh policy if download and validation of fresh
