...
void EnrollmentHandlerChromeOS::HandleActiveDirectoryPolicyRefreshed(
  ...
  switch (enrollment_step_) {
    case STEP_AD_FETCH_POLICY:
      // Authpolicyd caches device policy and returns this error because the
      // device isn't locked yet. After locking the device we store the cached
      // policy in the STEP_STORE_POLICY step.
