...
void UserNetworkConfigurationUpdater::ImportCertificates(
  ...
  if (!certificate_importer_) {
    pending_certificate_import_ = true;
    return;
  }
...
void UserNetworkConfigurationUpdater::SetCertificateImporter(
  ...
  if (pending_certificate_import_) {
    certificate_importer_->ImportCertificates(
        *certs_, onc_source_,
        base::BindOnce(&UserNetworkConfigurationUpdater::OnCertificatesImported,
                       base::Unretained(this)));
  }
  pending_certificate_import_ = false;
}
