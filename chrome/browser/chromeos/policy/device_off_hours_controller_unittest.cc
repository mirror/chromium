...
class DeviceOffHoursControllerTest : public DeviceSettingsTestBase {
 protected:
  ...
  void SetOffHoursProto(
      ...
      const std::vector<int>& ignored_policies_proto_tags) {
    ...
    for (auto p : ignored_policies_proto_tags) {
      off_hours->add_ignored_policies_proto_tags(p);
    }
    SetDeviceSettings();
  }
  ...
  void CreateAndSetOffHours(base::TimeDelta delay, base::TimeDelta duration) {
    ...
    std::vector<int> ignored_policies_proto_tags = {3 /*guest_mode_enabled*/};
    SetOffHoursProto(intervals, "GMT", ignored_policies_proto_tags);
  }
