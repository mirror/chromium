...
const std::vector<std::string> kDefaultStringVector{"guest_mode_enabled"};
constexpr em::WeeklyTimeProto_DayOfWeek weekdays[] = {
...
class DeviceOffHoursControllerTest : public DeviceSettingsTestBase {
 protected:
  ...
  void SetUp() override { DeviceSettingsTestBase::SetUp(); }
  ...
  em::DeviceOffHoursIntervalProto createInterval(int start_day_of_week,
    ...
    start->set_day_of_week(weekdays[start_day_of_week]);
    start->set_time(start_time.InMilliseconds());
    end->set_day_of_week(weekdays[end_day_of_week]);
    end->set_time(end_time.InMilliseconds());
    return interval;
  }

  // Create and set "OffHours" interval which starts
  //  in |delay| time and with |duration|.
  // Zero delay means that "OffHours" mode start right now.
  void CreateAndSetOffHours(
