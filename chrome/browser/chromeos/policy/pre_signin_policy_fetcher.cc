...
void PreSigninPolicyFetcher::SetCloudPolicyClientForTesting(
  ...
  cloud_policy_client_for_testing_ = std::move(cloud_policy_client);
}
...
void PreSigninPolicyFetcher::OnCachedPolicyValidated(
  ...
  if (cloud_policy_client_for_testing_) {
    cloud_policy_client_ = std::move(cloud_policy_client_for_testing_);
  } else {
    // Only create a CloudPolicyClient if we don't have one (for testing).
    cloud_policy_client_ = base::MakeUnique<CloudPolicyClient>(
