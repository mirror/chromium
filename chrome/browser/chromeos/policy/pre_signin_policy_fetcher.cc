...
void PreSigninPolicyFetcher::GetPolicy(
...
void PreSigninPolicyFetcher::OnMountTemporaryUserHome(
  ...
  session_manager_client_->RetrievePolicyForUserWithoutSession(
      cryptohome::Identification(account_id_), temporary_mount_path,
      base::Bind(&PreSigninPolicyFetcher::OnPolicyBlobRetrieved,
                 weak_ptr_factory_.GetWeakPtr()));
}

void PreSigninPolicyFetcher::OnPolicyBlobRetrieved(
  ...
  if (!policy_blob.empty()) {
    ...
    cached_policy_key_loader_->EnsurePolicyKeyLoaded(
        base::Bind(&PreSigninPolicyFetcher::OnPolicyKeyLoaded,
                   weak_ptr_factory_.GetWeakPtr(), policy_blob, response_type));
  } else {
    // Skip and pretend we've loaded policy key. We won't need it anyway,
    // because there is no policy to validate.
    OnPolicyKeyLoaded(policy_blob, response_type);
  }
}
...
void PreSigninPolicyFetcher::OnPolicyKeyLoaded(
  ...
  cryptohome_client_->Unmount(
      base::Bind(&PreSigninPolicyFetcher::OnUnmountAfterPolicyBlobRetrieved,
                 weak_ptr_factory_.GetWeakPtr(), policy_blob, response_type));
}

void PreSigninPolicyFetcher::OnUnmountAfterPolicyBlobRetrieved(
    const std::string& policy_blob,
    chromeos::SessionManagerClient::RetrievePolicyResponseType response_type,
  ...
  // TODO(pmarko): Also check response_type for errors: Is
  // RetrievePolicyResponseType::SUCCESS the only one which is fine or what is
  // returned on missing policy?
  if (policy_blob.empty()) {
...
void PreSigninPolicyFetcher::OnFetchedPolicyValidated(
  ...
  if (!validator->success()) {
    LOG(ERROR) << "Validation of fetched policy failed, falling back to cached "
                  "policy.";
    NotifyCallback(PolicyFetchResult::POLICY_FETCH_ERROR,
                   std::move(policy_payload_));
    return;
  }
...
PreSigninPolicyFetcher::CreateValidatorForFetchedPolicy(
  ...
  if (account_id_.GetAccountType() != AccountType::ACTIVE_DIRECTORY) {
    // Also validate the signature but allow a rotated key.
