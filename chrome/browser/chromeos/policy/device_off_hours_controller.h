...
class DeviceOffHoursController : public chromeos::PowerManagerClient::Observer {
 public:
  ...
  class Observer {
   public:
    // Gets called when "OffHours" mode is changed.
 ...
 private:
  ...
  // update current proto. Run OnOffHoursModeChanged() for observers.
  void NotifyOffHoursModeChanged() const;

  // If "OffHours" mode is in current time then apply current "OffHours"
  // interval and turn "OffHours" mode on otherwise turn "OffHours" mode off and
  ...
  // The main value of "OffHours" policy which shows current "OffHours" mode.
