...
enum class PolicyEnforcement {
  // No policy enforcement - it's OK to start the session even without a policy
  // check because it has previously been established that this session is
  // unmanaged.
  kPolicyOptional,
...
class UserCloudPolicyManagerChromeOS : public CloudPolicyManager,
 ...
 public:
  ...
  //   successful call to DMServer to check for policy. If this call is
  //   unsuccessful due to network/server errors, then |fatal_error_callback| is
  //   invoked to close the session.
  //
