...
class TestCryptohomeClient : public chromeos::FakeCryptohomeClient {
 public:
  ...
  const cryptohome::AuthorizationRequest& auth() const { return auth_; }

  const cryptohome::MountRequest& request() const { return request_; }

  void set_error(cryptohome::MountError error) { error_ = error; }
  ...
  void MountEx(const cryptohome::Identification& cryptohome_id,
    ...
    if (error_) {
      base::ThreadTaskRunnerHandle::Get()->PostTask(
          FROM_HERE,
          base::BindOnce(std::move(callback), false, error_, std::string()));
      return;
    }
    FakeCryptohomeClient::MountEx(cryptohome_id, auth, request,
                                  std::move(callback));
  }
 ...
 private:
  ...
  cryptohome::MountRequest request_;
  cryptohome::MountError error_;
...
class PreSigninPolicyFetcherTestBase : public testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    cryptohome_client_ = new TestCryptohomeClient();
  ...
  TestCryptohomeClient* cryptohome_client_ = nullptr;
...
TEST_F(PreSigninPolicyFetcherTest, NoPolicy) {
  ...
  cryptohome::MountRequest request = cryptohome_client_->request();
