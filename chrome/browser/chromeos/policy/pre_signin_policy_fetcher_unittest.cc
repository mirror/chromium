...
class PreSigninPolicyFetcherTest : public testing::Test {
 protected:
  ...
  MockCloudPolicyClient* PrepareMockCloudPolicyClient(
    ...
    auto client = base::MakeUnique<MockCloudPolicyClient>();
  ...
  void VerifyMockCloudPolicyClientExpectations(MockCloudPolicyClient* client) {
    ...
    EXPECT_TRUE(client->public_key_version_valid_);
    EXPECT_EQ(client->public_key_version_,
...
TEST_F(PreSigninPolicyFetcherTest, FreshPolicyFetchFails) {
  ...
  VerifyMockCloudPolicyClientExpectations(mock_cloud_policy_client);
