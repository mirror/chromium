 ...
 protected:
  ...
  void SetUp() override {
    ...
    pre_signin_policy_fetcher_ = base::MakeUnique<PreSigninPolicyFetcher>(
        ...
        std::move(cloud_policy_client), account_id_, cryptohome_key_, false);
    ...
    EXPECT_CALL(*mock_homedir_methods_,
        ...
        .WillOnce(WithArgs<2, 3>(Invoke(
            [mount_error](const cryptohome::MountRequest& mount_request,
              ...
              // By default, expect regular user home (not public mount).
...
// For ARC Kiosk App users, we expect that a public mount will be requested.
TEST_F(PreSigninPolicyFetcherTest, ArcKioskApp) {
  // Expect empty key definition for ARC Kiosk App mounts.
  EXPECT_CALL(
      *mock_homedir_methods_,
      MountEx(cryptohome::Identification(account_id_),
              cryptohome::Authorization(cryptohome::KeyDefinition()), _, _))
      .WillOnce(
          WithArg<2>(Invoke([](const cryptohome::MountRequest& mount_request) {
            EXPECT_TRUE(mount_request.hidden_mount());
            // Require public_mount option to be set.
            EXPECT_TRUE(mount_request.public_mount());
            // We don't care to run the callback and continue the pre-signin
            // policy fetching process - just checking MountRequest members in
            // this test case.
          })));

  auto cloud_policy_client = base::MakeUnique<MockCloudPolicyClient>();
  cloud_policy_client_ = cloud_policy_client.get();
  auto fetcher = base::MakeUnique<PreSigninPolicyFetcher>(
      &cryptohome_client_, &session_manager_client_,
      std::move(cloud_policy_client), account_id_, cryptohome_key_, true);

  fetcher->FetchPolicy(base::Bind(
      &PreSigninPolicyFetcherTest::OnPolicyRetrieved, base::Unretained(this)));
