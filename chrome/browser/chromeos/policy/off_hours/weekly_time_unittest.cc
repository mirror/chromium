...
class TwoWeeklyTimeTest : public testing::TestWithParam<std::tuple<int, int>> {
 public:
...
TEST_P(TwoWeeklyTimeTest, ToValue) {
  ...
  std::unique_ptr<base::DictionaryValue> weekly_time_value =
      weekly_time.ToValue();
  auto expected_weekly_time = base::DictionaryValue();
...
TEST_P(FourWeeklyTimeTest, GetDuration) {
  policy::off_hours::WeeklyTime weekly_time1 = policy::off_hours::WeeklyTime(
      day1(), minutes1() * kMinute.InMilliseconds());
  policy::off_hours::WeeklyTime weekly_time2 = policy::off_hours::WeeklyTime(
      day2(), minutes2() * kMinute.InMilliseconds());
...
TEST_P(ThreeWeeklyTimeTest, AddMilliseconds) {
  policy::off_hours::WeeklyTime weekly_time = policy::off_hours::WeeklyTime(
      day_of_week(), minutes() * kMinute.InMilliseconds());
  policy::off_hours::WeeklyTime result_weekly_time =
