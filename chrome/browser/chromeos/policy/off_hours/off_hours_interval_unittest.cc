...
TEST_P(FourOffHoursIntervalTest, Constructor) {
  policy::off_hours::WeeklyTime start = policy::off_hours::WeeklyTime(
      start_day_of_week(), start_time() * kMinute.InMilliseconds());
  policy::off_hours::WeeklyTime end = policy::off_hours::WeeklyTime(
      end_day_of_week(), end_time() * kMinute.InMilliseconds());
  policy::off_hours::OffHoursInterval interval =
      policy::off_hours::OffHoursInterval(start, end);
...
TEST_P(FourOffHoursIntervalTest, ToValue) {
  policy::off_hours::WeeklyTime start =
      policy::off_hours::WeeklyTime(start_day_of_week(), start_time());
  policy::off_hours::WeeklyTime end =
      policy::off_hours::WeeklyTime(end_day_of_week(), end_time());
  policy::off_hours::OffHoursInterval interval =
      policy::off_hours::OffHoursInterval(start, end);
  std::unique_ptr<base::DictionaryValue> interval_value = interval.ToValue();
  auto expected_interval_value = base::DictionaryValue();
...
TEST_P(SixOffHoursIntervalTest, Contains) {
  ...
  policy::off_hours::OffHoursInterval interval =
      policy::off_hours::OffHoursInterval(start, end);
  policy::off_hours::WeeklyTime weekly_time = policy::off_hours::WeeklyTime(
