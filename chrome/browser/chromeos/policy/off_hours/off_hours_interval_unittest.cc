...
TEST_P(OffHoursIntervalContains, Contains) {
  ...
  base::TimeDelta interval_duration = WeeklyTime::GetDuration(start, end);
  if (WeeklyTime::GetDuration(weekly_time, end).is_zero() ||
      (WeeklyTime::GetDuration(start, weekly_time) +
           WeeklyTime::GetDuration(weekly_time, end) !=
       interval_duration))
