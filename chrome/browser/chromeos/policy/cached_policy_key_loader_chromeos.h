...
class CachedPolicyKeyLoaderChromeOS {
 ...
 private:
  ...
  // Posts a task to load the policy key on |task_runner|. OnPolicyKeyLoaded()
  ...
  // This will be true when a caller requested to reload the key while a key
  // load was in progress.
  bool key_reload_pending_ = false;

  // All callbacks that should be called when the async key load finishes.
  std::vector<base::Closure> key_loaded_callbacks_;
