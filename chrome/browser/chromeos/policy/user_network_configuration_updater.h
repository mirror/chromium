...
class UserNetworkConfigurationUpdater : public NetworkConfigurationUpdater,
 ...
 public:
  ...
  // Helper method to expose |SetCertificateImporter| for usage in tests. Note
  // that the CertificateImporter is only used for importing client
  // certificates.
  void SetCertificateImporterForTest(
      std::unique_ptr<chromeos::onc::CertificateImporter> certificate_importer);

  // Sets the Factory which UserNetworkConfigurationUpdater will use to create a
  // CertificateImporter on the PROFILE_ADDED notification. Note that the
  // CertificateImporter is only used for importing client certificates.
 ...
 private:
  ...
  // certificates. Set by |SetCertificateImporter|.
