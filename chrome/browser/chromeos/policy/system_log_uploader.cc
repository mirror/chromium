...
std::pair<std::string, std::string> ReadAndAnonymizeLogFile(
  ...
  if (data.size() == kLogCutoffSize) {
    size_t last_newline_pos = data.find_last_of('\n');
    if (last_newline_pos != std::string::npos)
      data.erase(last_newline_pos + 1);
...
std::unique_ptr<policy::SystemLogUploader::SystemLogs> ReadFiles() {
  ...
  for (auto* file_path : kSystemLogFileNames) {
    if (!base::PathExists(base::FilePath(file_path)))
      continue;
    system_logs->push_back(ReadAndAnonymizeLogFile(&anonymizer, file_path));
  }
  return system_logs;
}
