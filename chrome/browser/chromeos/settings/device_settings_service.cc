...
void DeviceSettingsService::Initialize() {
  ...
  policy::DeviceOffHoursController::Get()->AddObserver(
...
void DeviceSettingsService::OffHoursModeMaybeChanged() {
  LoadImmediately();
}
...
void DeviceSettingsService::HandleCompletedOperation(
  ...
  if (status == STORE_SUCCESS) {
    ...
    policy::DeviceOffHoursController::UpdateOffHoursMode(*device_settings_);
    if (policy::DeviceOffHoursController::Get()->IsOffHoursMode()) {
      std::unique_ptr<em::ChromeDeviceSettingsProto> off_device_settings =
          policy::DeviceOffHoursController::GetOffHoursProto(*device_settings_);
