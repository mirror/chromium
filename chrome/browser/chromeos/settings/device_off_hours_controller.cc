...
std::vector<std::unique_ptr<Interval>> get_intervals(
  ...
  for (const auto& entry : container.interval()) {
    ...
    if (entry.has_end()) {
      if (!get_WeeklyTime(&interval.end, entry.end(), timezone)) {
        continue;
      }
    }
...
int off_hours_duration(
    const std::vector<std::unique_ptr<Interval>>& intervals) {
  time_t theTime = time(NULL);
  struct tm* aTime = localtime(&theTime);
  WeeklyTime cur_t(
      aTime->tm_wday, aTime->tm_hour, aTime->tm_min,
      base::UTF16ToASCII(
...
DeviceOffHoursController::~DeviceOffHoursController() {
  //  TODO(yakovleva) remove observers
...
DeviceOffHoursController::ApplyOffHoursMode(
  ...
  int duration = off_hours_duration(intervals);
  if (duration == 0 || container.policy_size() == 0) {
    return nullptr;
  }
  ...
  g_device_off_hours_controller->StartOffHoursTimer(duration);
