...
void CrosSettingsTestApi::InitializeForTest() {
  CrosSettings::Initialize(
      ...
      static_cast<policy::BrowserPolicyConnectorChromeOS*>(g_browser_process->browser_policy_connector()));
}
...
std::unique_ptr<CrosSettings> CrosSettingsTestApi::CreateForTest(
  ...
  return std::make_unique<CrosSettings>(
      g_browser_process->local_state(),
      static_cast<policy::BrowserPolicyConnectorChromeOS*>(g_browser_process->browser_policy_connector()),
      device_settings_service ? device_settings_service : DeviceSettingsService::Get());
}
