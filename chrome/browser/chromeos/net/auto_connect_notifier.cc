...
void AutoConnectNotifier::NetworkConnectionStateChanged(
  ...
  // notification alerting the user that this has occurred. Without this
  // notification, the user could be confused about why the device is connected
  // to a different network from the one that the user originally requested.
  DisplayNotification();
}
...
void AutoConnectNotifier::OnAutoConnectedInitiated(int auto_connect_reasons) {
  ...
  // The notification should only be shown if a managed network is joined. Other
  // reasons (e.g., joining a network due to login) do not require that a
  // notification be shown.
  int managed_network_reasons_bitmask =
  ...
  if (!(auto_connect_reasons & managed_network_reasons_bitmask))
    return;

  // If a potential connection is already underway, continue waiting.
  if (timer_->IsRunning())
    return;
