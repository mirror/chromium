...
void NetworkPortalDetectorImpl::OnAttemptCompleted(
  ...
  // Within one minute that default network reports a portal network, if we get
  // offline detection result, it is likely we get blacklisted. Record the
  // boolean rate that Chrome pulls offline vs non-offline detection result.
