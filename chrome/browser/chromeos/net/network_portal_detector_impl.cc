...
void NetworkPortalDetectorImpl::OnAttemptCompleted(
  ...
  // Within one minute of the default networking reporting a portal network, if
  // we see an offline detection result, it is likely the client got
  // blacklisted. Record the boolean rate that Chrome observes offline vs
  // non-offline detection results.
