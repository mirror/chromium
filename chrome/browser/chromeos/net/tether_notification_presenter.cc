...
const gfx::VectorIcon& GetIconForSignalStrength(int signal_strength) {
  ...
  // TODO(khorimoto): Use the actual icon when it is available. This will be
  // done as part of this CL once the icons are ready, but I'm waiting for
  // elizabethchiu@ to provide assets.
  switch (normalized_signal_strength) {
    case 0:
      return kWarningBadgeCircleIcon;
    case 1:
      return kWarningBadgeCircleIcon;
    case 2:
      return kWarningBadgeCircleIcon;
...
std::unique_ptr<message_center::Notification> CreateNotification(
  ...
  // If the notification icon has a warning badge, use the warning level;
  // otherwise, use the normal level.
  message_center::SystemNotificationWarningLevel warning_level =
      (&small_icon == &kWarningBadgeCircleIcon)
...
void TetherNotificationPresenter::NotifyConnectionToHostFailed() {
  ...
  ShowNotification(CreateNotification(
      ...
      kWarningBadgeCircleIcon, {} /* rich_notification_data */));
}
