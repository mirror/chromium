...
bool SelectToSpeakEventRewriter::OnMouseEvent(const ui::MouseEvent* event) {
  ...
  if (event_delegate_for_testing_) {
    ...
    ui::MouseEvent event_copy =
        static_cast<ui::MouseEvent>(ui::Event::Clone(*event).get());
    gfx::Point location = event_copy.location();
    gfx::Point root_location = event_copy.root_location();
    root_window_->GetHost()->ConvertPixelsToDIP(&location);
    root_window_->GetHost()->ConvertPixelsToDIP(&root_location);
    event_copy.set_location(location);
    event_copy.set_root_location(root_location);

    const blink::WebMouseEvent web_event = ui::MakeWebMouseEvent(
        event_copy, base::Bind(&GetScreenLocationFromEvent));
