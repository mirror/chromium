...
bool SelectToSpeakEventRewriter::OnMouseEvent(const ui::MouseEvent* event) {
  ...
  ConvertMouseEventToDIPs(event_copy.get());
  ...
  if (event_delegate_for_testing_) {
    ...
    const blink::WebMouseEvent web_event = ui::MakeWebMouseEvent(
        *event_copy->AsMouseEvent(), base::Bind(&GetScreenLocationFromEvent));
...
void SelectToSpeakEventRewriter::ConvertMouseEventToDIPs(ui::Event* event) {
