...
class SelectToSpeakMouseEventDelegate final
 ...
 public:
  ...
  gfx::Point LastMouseEventLocation() { return last_mouse_location_; }
 ...
 private:
  ...
  gfx::Point last_mouse_location_ = gfx::Point(0, 0);
...
TEST_F(SelectToSpeakEventRewriterTest, SearchPlusClick) {
  ...
  EXPECT_EQ(click_location, mouse_event_delegate_->LastMouseEventLocation());
...
TEST_F(SelectToSpeakEventRewriterTest, SearchPlusDrag) {
  ...
  EXPECT_EQ(drag_location, mouse_event_delegate_->LastMouseEventLocation());
...
TEST_F(SelectToSpeakEventRewriterTest, SearchPlusDragOnLargeDisplay) {
  ...
  EXPECT_EQ(gfx::Point(click_location_px.x() / 2, click_location_px.y() / 2),
            mouse_event_delegate_->LastMouseEventLocation());
