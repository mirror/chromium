...
class PrefServiceWatcher : public ash::mojom::PrefServiceObserver {
 public:
  explicit PrefServiceWatcher(const AccountId& account_id)
      : account_id_(account_id), binding_(this) {
    ash::mojom::SessionControllerPtr session_controller_ptr;
    content::ServiceManagerConnection::GetForProcess()
        ->GetConnector()
        ->BindInterface(ash::mojom::kServiceName, &session_controller_ptr);
    ash::mojom::PrefServiceObserverAssociatedPtrInfo ptr_info;
    binding_.Bind(mojo::MakeRequest(&ptr_info));
    session_controller_ptr->AddObserver(std::move(ptr_info));

    run_loop_.Run();
  }

 private:
  // ash::mojom::PrefServiceObserver:
  ...
  mojo::AssociatedBinding<ash::mojom::PrefServiceObserver> binding_;
  base::RunLoop run_loop_;

  DISALLOW_COPY_AND_ASSIGN(PrefServiceWatcher);
};
...
class AccessibilityManagerLoginTest : public OobeBaseTest {
 protected:
  ...
  void StartUserSession(const AccountId& account_id) {
    // Prepare policy provider for new user profile.
    EXPECT_CALL(provider_, IsInitializationComplete(_))
        .WillRepeatedly(Return(true));
    policy::ProfilePolicyConnectorFactory::GetInstance()
        ->PushProviderForTesting(&provider_);

    Profile* user_profile = ProfileHelper::GetProfileByUserIdHashForTest(
        user_manager::UserManager::Get()
            ->FindUser(account_id)
            ->username_hash());
    session_manager::SessionManager::Get()->SessionStarted();
    ASSERT_EQ(PrefService::INITIALIZATION_STATUS_CREATED_NEW_PREF_STORE,
              user_profile->GetPrefs()->GetAllPrefStoresInitializationStatus());
    PrefServiceWatcher watcher(account_id);
  }
