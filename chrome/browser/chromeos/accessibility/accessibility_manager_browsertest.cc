...
void SetAlwaysShowMenuEnabledPref(Profile* profile, bool enabled) {
  DCHECK(profile);
  profile->GetPrefs()->SetBoolean(
      ash::prefs::kShouldAlwaysShowAccessibilityMenu, enabled);
}

void SetLargeCursorEnabledPref(Profile* profile, bool enabled) {
  DCHECK(profile);
  profile->GetPrefs()->SetBoolean(ash::prefs::kAccessibilityLargeCursorEnabled,
                                  enabled);
}

void SetHighContrastEnabledPref(Profile* profile, bool enabled) {
  DCHECK(profile);
  profile->GetPrefs()->SetBoolean(ash::prefs::kAccessibilityHighContrastEnabled,
                                  enabled);
}

void SetSpokenFeedbackEnabledPref(Profile* profile, bool enabled) {
  DCHECK(profile);
  profile->GetPrefs()->SetBoolean(
      ash::prefs::kAccessibilitySpokenFeedbackEnabled, enabled);
}

void SetAutoclickEnabledPref(Profile* profile, bool enabled) {
  DCHECK(profile);
  profile->GetPrefs()->SetBoolean(ash::prefs::kAccessibilityAutoclickEnabled,
                                  enabled);
}

void SetAutoclickDelayPref(Profile* profile, int delay_ms) {
  DCHECK(profile);
  profile->GetPrefs()->SetInteger(ash::prefs::kAccessibilityAutoclickDelayMs,
                                  delay_ms);
}

void SetVirtualKeyboardEnabledPref(Profile* profile, bool enabled) {
  DCHECK(profile);
  profile->GetPrefs()->SetBoolean(
      ash::prefs::kAccessibilityVirtualKeyboardEnabled, enabled);
}

void SetMonoAudioEnabledPref(Profile* profile, bool enabled) {
  DCHECK(profile);
  profile->GetPrefs()->SetBoolean(ash::prefs::kAccessibilityMonoAudioEnabled,
                                  enabled);
}
...
IN_PROC_BROWSER_TEST_F(AccessibilityManagerTest, TypePref) {
  ...
  SetLargeCursorEnabledPref(GetActiveUserProfile(), true);
  ...
  SetSpokenFeedbackEnabledPref(GetActiveUserProfile(), true);
  ...
  SetHighContrastEnabledPref(GetActiveUserProfile(), true);
  ...
  SetAutoclickEnabledPref(GetActiveUserProfile(), true);
  // Confirms that autoclick is enabled.
  EXPECT_TRUE(IsAutoclickEnabled());

  // Set autoclick delay pref.
  SetAutoclickDelayPref(GetActiveUserProfile(), kTestAutoclickDelayMs);
  // Confirm that the correct value is set.
  EXPECT_EQ(kTestAutoclickDelayMs, GetAutoclickDelay());

  // Sets the on-screen keyboard pref.
  SetVirtualKeyboardEnabledPref(GetActiveUserProfile(), true);
  // Confirm that the on-screen keyboard option is enabled.
  EXPECT_TRUE(IsVirtualKeyboardEnabled());

  // Sets mono audio output pref.
  SetMonoAudioEnabledPref(GetActiveUserProfile(), true);
  // Confirm that mono audio output is enabled.
  EXPECT_TRUE(IsMonoAudioEnabled());

  SetLargeCursorEnabledPref(GetActiveUserProfile(), false);
  EXPECT_FALSE(IsLargeCursorEnabled());

  SetSpokenFeedbackEnabledPref(GetActiveUserProfile(), false);
  EXPECT_FALSE(IsSpokenFeedbackEnabled());

  SetHighContrastEnabledPref(GetActiveUserProfile(), false);
  EXPECT_FALSE(IsHighContrastEnabled());

  SetAutoclickEnabledPref(GetActiveUserProfile(), false);
  EXPECT_FALSE(IsAutoclickEnabled());

  SetVirtualKeyboardEnabledPref(GetActiveUserProfile(), false);
  EXPECT_FALSE(IsVirtualKeyboardEnabled());

  SetMonoAudioEnabledPref(GetActiveUserProfile(), false);
  EXPECT_FALSE(IsMonoAudioEnabled());
}
...
IN_PROC_BROWSER_TEST_F(AccessibilityManagerTest, AccessibilityMenuVisibility) {
  ...
  EXPECT_FALSE(ShouldShowAccessibilityMenu());
  SetAlwaysShowMenuEnabledPref(GetActiveUserProfile(), true);
  EXPECT_TRUE(ShouldShowAccessibilityMenu());
  SetAlwaysShowMenuEnabledPref(GetActiveUserProfile(), false);
  EXPECT_FALSE(ShouldShowAccessibilityMenu());
...
class AccessibilityManagerUserTypeLoginTest
 ...
 protected:
  ...
  virtual ~AccessibilityManagerUserTypeLoginTest() {}
