...
// Try to connect to user pref service for |account_id| and wait on OnConnect
// callback.
// TODO(warx): we might need to have observer in ash to do this work.
void WaitOnConnectToUserPrefService(const AccountId& account_id) {
  auto pref_registry = base::MakeRefCounted<PrefRegistrySimple>();
  ash::mojom::PrefConnectorPtr pref_connector_connector;
  service_manager::Connector* connector =
      content::ServiceManagerConnection::GetForProcess()->GetConnector();
  connector->BindInterface(ash::mojom::kPrefConnectorServiceName,
  ...
  prefs::ConnectToPrefService(
      ...
      base::Bind(
          [](base::RunLoop* run_loop,
             std::unique_ptr<PrefService> pref_service) { run_loop->Quit(); },
          &run_loop));
  run_loop.Run();
}
...
class AccessibilityManagerLoginTest : public OobeBaseTest {
 protected:
  ...
  void StartUserSession(const AccountId& account_id) {
    ...
    // Wait on ash to connect to user pref service.
    WaitOnConnectToUserPrefService(account_id);
  }
