...
IN_PROC_BROWSER_TEST_F(AccessibilityManagerLoginTest, ResumeSavedPref) {
  ...
  // Adds user session and logins in, but before profile is loaded.
  CreateSession(test_account_id_);
  EXPECT_EQ(ProfileHelper::GetSigninProfile(),
            AccessibilityManager::Get()->profile());

  // Loads profile. At this stage, active user profile is not signin profile,
  // but the profile in AccessibilityManager is still signin profile.
  ProfileHelper::GetProfileByUserIdHashForTest(user_manager::UserManager::Get()
                                                   ->FindUser(test_account_id_)
                                                   ->username_hash());
  EXPECT_EQ(ProfileHelper::GetSigninProfile(),
            AccessibilityManager::Get()->profile());
  EXPECT_NE(ProfileHelper::GetSigninProfile(), GetActiveUserProfile());
  // For new user profile, features are disabled.
  EXPECT_FALSE(GetLargeCursorEnabledFromPref());
  EXPECT_FALSE(GetSpokenFeedbackEnabledFromPref());
  EXPECT_FALSE(GetHighContrastEnabledFromPref());
  EXPECT_FALSE(GetAutoclickEnabledFromPref());
  EXPECT_EQ(default_autoclick_delay_, GetAutoclickDelayFromPref());
  EXPECT_FALSE(GetMonoAudioEnabledFromPref());

  // Starts session. Tests that prefs are copied to active user profile from
  // signin prefs.
  session_manager::SessionManager::Get()->SessionStarted();
  EXPECT_NE(ProfileHelper::GetSigninProfile(),
            AccessibilityManager::Get()->profile());
