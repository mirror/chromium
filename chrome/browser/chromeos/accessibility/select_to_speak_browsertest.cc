...
IN_PROC_BROWSER_TEST_F(SelectToSpeakTest, FocusRingMovesWithMouse) {
  ...
  gfx::Rect targetBounds = focus_rings->at(0)->layer()->GetTargetBounds();

  // Make sure it's in a reasonable position.
  EXPECT_LT(abs(targetBounds.x() - (bounds.x() + 8)), 50);
  EXPECT_LT(abs(targetBounds.y() - (bounds.y() + 50)), 50);
  EXPECT_LT(targetBounds.width(), 50);
  EXPECT_LT(targetBounds.height(), 50);
  ...
  WaitForFocusRingChanged();
  targetBounds = focus_rings->at(0)->layer()->GetTargetBounds();
  EXPECT_LT(abs(targetBounds.x() - (bounds.x() + 8)), 50);
  EXPECT_LT(abs(targetBounds.y() - (bounds.y() + 50)), 50);
  EXPECT_GT(targetBounds.width(), 100);
  EXPECT_GT(targetBounds.height(), 100);
