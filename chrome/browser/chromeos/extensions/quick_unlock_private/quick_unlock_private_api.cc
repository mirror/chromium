...
// Checks whether a given |pin| is invalid given the PIN min/max policies in
// |pref_service|.
CredentialProblem IsPinLengthInvalid(const std::string& pin,
                                     PrefService* pref_service) {
  int min_length;
  int max_length;
...
QuickUnlockPrivateCheckCredentialFunction::Run() {
  ...
  CredentialProblem length_problem =
      IsPinLengthInvalid(credential, pref_service);
...
ExtensionFunction::ResponseAction QuickUnlockPrivateSetModesFunction::Run() {
  ...
  for (size_t i = 0; i < params_->modes.size(); ++i) {
    ...
    CredentialProblem problem =
        IsPinLengthInvalid(params_->credentials[i], pref_service);
