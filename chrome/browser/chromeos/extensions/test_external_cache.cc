...
void TestExternalCache::Shutdown(const base::Closure& callback) {}
...
bool TestExternalCache::GetExtension(const std::string& id,
  ...
  if (!cache_.count(id))
    return false;
  *file_path = base::FilePath(cache_[id].path);
  *version = cache_[id].version;
  return true;
}
...
void TestExternalCache::PutExternalExtension(
  ...
  AddEntryToCache(id, crx_file_path.value(), version);
  callback.Run(id, true);
}
...
bool TestExternalCache::SimulateExtensionDownloadFinished(
  ...
  AddEntryToCache(id, crx_path, version);
...
bool TestExternalCache::SimulateExtensionDownloadFailed(
  ...
  if (error == extensions::ExtensionDownloaderDelegate::NO_UPDATE_AVAILABLE &&
      cached_extensions_.FindKey(id)) {
    delegate_->OnExtensionLoadedInCache(id);
  } else {
...
void TestExternalCache::UpdateCache() {
  for (const auto& entry : configured_extensions_->DictItems()) {
    ...
    if (cache_.count(entry.first)) {
      cached_extensions_.SetKey(
          entry.first,
          GetCachedExtensionValue(entry.second, cache_[entry.first].path,
                                  cache_[entry.first].version));
...
void TestExternalCache::AddEntryToCache(const std::string& id,
                                        const std::string& crx_path,
                                        const std::string& version) {
  cache_[id] = {crx_path, version};

  const base::Value* extension =
      configured_extensions_->FindKeyOfType(id, base::Value::Type::DICTIONARY);
  if (extension) {
    cached_extensions_.SetKey(
        id, GetCachedExtensionValue(*extension, crx_path, version));
    delegate_->OnExtensionListsUpdated(&cached_extensions_);
  }
}
