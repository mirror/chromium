...
class ExternalCache {
 public:
  using PutExternalExtensionCallback =
      base::Callback<void(const std::string& id, bool success)>;
  ...
  static base::Value GetCachedExtensionValue(const base::Value& original_value,
  ...
  // |id|.
  virtual void PutExternalExtension(
      const std::string& id,
      const base::FilePath& crx_file_path,
      const std::string& version,
      const PutExternalExtensionCallback& callback) = 0;
};
