...
void ActiveTabPermissionGranterDelegateChromeOS::
    SetRequestResolvedCallbackForTesting(
        const permission_helper::RequestResolvedCallback& callback) {
...
bool ActiveTabPermissionGranterDelegateChromeOS::ShouldGrantActiveTab(
  ...
  permission_helper::RequestResolvedCallback callback = g_resolved_callback;
  if (callback.is_null())
    callback = permission_helper::RequestResolvedCallback();
