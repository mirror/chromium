...
GURL ExternalCache::GetExtensionUpdateUrl(const base::Value& extension_value,
  ...
  // "keep if present" means that the extension should be kept if it's alredy
...
// static
base::Value ExternalCache::GetCachedExtensionValue(const base::Value& extension,
  ...
  if (external_update_url_value) {
    result.SetKey(extensions::ExternalProviderImpl::kIsFromWebstore,
                  base::Value(extension_urls::IsWebstoreUpdateUrl(
