...
void CollectionInfoFetcher::OnURLFetchComplete(const net::URLFetcher* source) {
  ...
  if (!source->GetStatus().is_success() ||
    ...
    url_fetcher_.reset();
    std::move(callback_).Run(collections_info_list);
    return;
  }
...
void ImageInfoFetcher::OnURLFetchComplete(const net::URLFetcher* source) {
  ...
  if (!source->GetStatus().is_success() ||
    ...
    url_fetcher_.reset();
    std::move(callback_).Run(images_info_list);
    return;
  }
  std::string response_string;
  source->GetResponseAsString(&response_string);
  ...
  for (int i = 0; i < images_response.images_size(); ++i) {
    ...
    for (int j = 0; j < image.attribution_size(); ++j)
      display_text.push_back(image.attribution()[j].text());

    image_info.display_text = std::move(display_text);
