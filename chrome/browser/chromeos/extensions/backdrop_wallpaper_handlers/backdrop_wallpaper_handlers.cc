...
void CollectionNameFetcher::Start(
  ...
  DCHECK(!url_fetcher_ && on_collection_names_fetched_.is_null());
...
void CollectionNameFetcher::OnURLFetchComplete(const net::URLFetcher* source) {
  if (!source->GetStatus().is_success() ||
    ...
    std::move(on_collection_names_fetched_)
  ...
  base::Value collection_names(base::Value::Type::DICTIONARY);
  for (int i = 0; i < collections_response.collections_size(); ++i) {
    backdrop::Collection collection = collections_response.collections(i);
    collection_names.SetKey(collection.collection_id(),
                            base::Value(collection.collection_name()));
  }

  url_fetcher_.reset();
  std::move(on_collection_names_fetched_).Run(std::move(collection_names));
}
...
void ImageInfoFetcher::Start(OnImagesInfoFetched on_images_info_fetched) {
  DCHECK(!url_fetcher_ && on_images_info_fetched_.is_null());
...
void ImageInfoFetcher::OnURLFetchComplete(const net::URLFetcher* source) {
  ...
  base::Value images_info_list(base::Value::Type::LIST);
  ...
  url_fetcher_.reset();
  std::move(on_images_info_fetched_).Run(std::move(images_info_list));
}
