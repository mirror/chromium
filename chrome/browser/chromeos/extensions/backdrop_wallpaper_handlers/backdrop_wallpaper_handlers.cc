...
void CollectionNameFetcher::Start(OnCollectionNamesFetched callback) {
...
void CollectionNameFetcher::OnURLFetchComplete(const net::URLFetcher* source) {
  if (!source->GetStatus().is_success() ||
    ...
    url_fetcher_.reset();
    std::move(callback_).Run(base::Value());
    return;
  }
  ...
  url_fetcher_.reset();
  std::move(callback_).Run(std::move(collection_names));
}
...
void ImageInfoFetcher::OnURLFetchComplete(const net::URLFetcher* source) {
  ...
  url_fetcher_.reset();
  std::move(callback_).Run(std::move(images_info_list));
}
