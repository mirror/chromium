...
base::File::Error Service::MountFileSystemInternal(
  ...
  const base::FilePath& mount_path =
      util::GetMountPath(profile_, provider_id.GetId(), options.file_system_id);
  ...
  GetProvidingExtensionInfo(provider_id.GetId(), &provider_info);
  ...
  file_system_map_[FileSystemKey(provider_id.GetId(), options.file_system_id)] =
      std::move(file_system);
  mount_point_name_to_key_map_[mount_point_name] =
      FileSystemKey(provider_id.GetId(), options.file_system_id);
...
base::File::Error Service::UnmountFileSystem(const ProviderId& provider_id,
  ...
  const auto file_system_it =
      file_system_map_.find(FileSystemKey(provider_id.GetId(), file_system_id));
...
bool Service::RequestMount(const ProviderId& provider_id) {
  ...
  if (!event_router->ExtensionHasEventListener(
          provider_id.GetId(), extensions::api::file_system_provider::
...
void Service::OnExtensionUnloaded(content::BrowserContext* browser_context,
  ...
  while (it != file_system_map_.end()) {
    ...
    ++it;
    if (file_system_info.provider_id().GetId() == extension->id()) {
...
void Service::OnExtensionLoaded(content::BrowserContext* browser_context,
                                const extensions::Extension* extension) {
  ProviderId provider_id(extension->id(), ProviderId::EXTENSION);
  ...
  for (const auto& restored_file_system : *restored_file_systems) {
    ...
    if (result != base::File::FILE_OK) {
      LOG(ERROR) << "Failed to restore a provided file system from "
                 << "registry: " << restored_file_system.provider_id.GetId()
...
void Service::RegisterFileSystemFactory(
  ...
  if (provider_id.GetType() == ProviderId::NATIVE)
    file_system_factory_map_[provider_id.GetId()] = file_system_factory;
}

Service::FileSystemFactoryCallback Service::GetFileSystemFactory(
    const ProviderId& provider_id) {
  auto it = file_system_factory_map_.find(provider_id.GetId());
