...
class ProvidedFileSystemInterface {
 public:
  ...
  using OpenFileCallback =
      base::OnceCallback<void(int file_handle, base::File::Error result)>;

  using ReadChunkReceivedCallback = base::RepeatingCallback<
      void(int chunk_length, bool has_more, base::File::Error result)>;

  using GetMetadataCallback =
      base::OnceCallback<void(std::unique_ptr<EntryMetadata> entry_metadata,
                              base::File::Error result)>;

  using GetActionsCallback = base::OnceCallback<void(const Actions& actions,
                                                     base::File::Error result)>;

  // Mask of fields requested from the GetMetadata() call.
  using MetadataFieldMask = int;
