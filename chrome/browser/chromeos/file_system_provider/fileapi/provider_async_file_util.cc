...
void ReadDirectoryOnUIThread(
  ...
  parser.file_system()->ReadDirectory(parser.file_path(), std::move(callback));
}
...
void ProviderAsyncFileUtil::ReadDirectory(
  ...
  BrowserThread::PostTask(
      BrowserThread::UI, FROM_HERE,
      base::BindOnce(
          &ReadDirectoryOnUIThread, std::move(context), url,
          base::BindRepeating(&OnReadDirectory, std::move(callback))));
}
