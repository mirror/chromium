...
scoped_refptr<extensions::Extension> CreateFakeExtension(
  ...
  LOG(ERROR) << error; // Added this so we could see the error that was generated (this is working now)
  return ext;
}
...
class FileSystemProviderServiceTest : public testing::Test {
 ...
 protected:
  ...
  void SetUp() override {
    ...
    extensions::ExtensionRegistry* extension_registry_ = extensions::ExtensionRegistry::Get(profile_);

    extension_ = CreateFakeExtension(kProviderId, true);
    
    // Checks that the extension was created and has the correct api Permission
    ASSERT_TRUE(extension_);  
    ASSERT_TRUE(extension_->permissions_data()->HasAPIPermission(extensions::APIPermission::kFileSystemProvider));
    

    extension_registry_->AddEnabled(extension_);

    service_.reset(new Service(profile_, extension_registry_));
    ...
    // Checks that the extension registry gotten by ExtensionRegistry::Get(profile_) has our created extension in it
    extensions::ExtensionRegistry* const er_temp = extensions::ExtensionRegistry::Get(profile_);
    ASSERT_TRUE(er_temp);
    const extensions::Extension* const e_temp = er_temp->GetExtensionById(kProviderId, extensions::ExtensionRegistry::ENABLED);
    ASSERT_TRUE(e_temp);
    LOG(ERROR) << "~~~ " << e_temp->name();
