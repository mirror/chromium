...
TEST_F(FileSystemProviderServiceTest, RegisterFileSystemFactory) {
  ...
  EXPECT_EQ(false, FileSystemProviderServiceTest::called_default_factory_);

  EXPECT_EQ(base::File::FILE_OK,
            service_->MountFileSystem(
                kProviderId, MountOptions(kFileSystemId, kDisplayName)));

  EXPECT_EQ(true, FileSystemProviderServiceTest::called_default_factory_);

  EXPECT_EQ(false, FileSystemProviderServiceTest::called_custom_factory_);

  EXPECT_EQ(base::File::FILE_OK,
            service_->MountFileSystem(
                kCustomProviderId, MountOptions(kFileSystemId, kDisplayName)));

  EXPECT_EQ(true, FileSystemProviderServiceTest::called_custom_factory_);
}
