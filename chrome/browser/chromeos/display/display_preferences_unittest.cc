...
class DisplayPreferencesTest : public ash::AshTestBase {
 protected:
  ...
  void SetUp() override {
    ...
    static_cast<ash::WindowTreeHostManager::Observer*>(observer_.get())
...
TEST_F(DisplayPreferencesTest, MirrorWhenAddingExternalDisplayInTableMode) {
  display::Display::SetInternalDisplayId(
      display::Screen::GetScreen()->GetPrimaryDisplay().id());
  LoggedInAsUser();
  EXPECT_FALSE(display_manager()->IsInMirrorMode());
  ...
  UpdateDisplay("800x600, 1200*800");
  EXPECT_TRUE(display_manager()->IsInMirrorMode());
