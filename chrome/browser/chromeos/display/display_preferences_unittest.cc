...
TEST_F(MultiMirroringDisplayPreferencesTest, DisplayMixedMirrorMode) {
  LoggedInAsUser();
  ...
  display::MixedMirrorModeParams mixed_params(internal_display_id,
                                              {first_display_id});
  StoreDisplayMixedMirrorModeParamsForTest(&mixed_params);
  LoadDisplayPreferences(false);
  ...
  std::unique_ptr<display::MixedMirrorModeParams> new_mixed_params =
      std::make_unique<display::MixedMirrorModeParams>(first_display_id,
                                                       dst_ids);
  display_manager()->SetMirrorMode(true, std::move(new_mixed_params));
  EXPECT_TRUE(display_manager()->IsInSoftwareMirrorMode());
