...
void LoadPreviousDisplayMirrorModes() {
  PrefService* local_state = g_browser_process->local_state();
  ...
  for (base::DictionaryValue::Iterator it(*pref_data); !it.IsAtEnd();
    ...
    if (mirror)
      stored_mirror_modes.emplace(display_id);
  }
...
void StorePreviousDisplayMirrorModes() {
  PrefService* local_state = g_browser_process->local_state();
  DictionaryPrefUpdate update(local_state, prefs::kDisplayPreviousMirrorModes);
  base::DictionaryValue* pref_data = update.Get();
  pref_data->Clear();
...
void RegisterDisplayLocalStatePrefs(PrefRegistrySimple* registry) {
  ...
  registry->RegisterDictionaryPref(prefs::kDisplayPreviousMirrorModes);
}
...
void StoreDisplayPrefs() {
  ...
  StorePreviousDisplayMirrorModes();
}
...
void LoadDisplayPreferences(bool first_run_after_boot) {
  ...
  LoadPreviousDisplayMirrorModes();
