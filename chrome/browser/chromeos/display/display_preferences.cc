...
bool ValueToTouchDataMap(
  ...
  for (const auto& data_pair : *map_dictionary) {
    ...
    if (!ValueToTouchData(*data_dict, &data))
      return false;
    touch_calibration_map->insert({touch_device_identifier, data});
  }
  return true;
}
...
void StoreCurrentDisplayProperties() {
  ...
  for (size_t i = 0; i < num; ++i) {
    ...
    // Store legacy touch calibration data.
    if (info.has_legacy_touch_calibration_data())
