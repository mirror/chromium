...
TEST_F(AttestationPolicyObserverTest, NewCertificate) {
  ...
  Run();
  EXPECT_EQ(base::Optional<std::string>(CreatePayload()),
...
TEST_F(AttestationPolicyObserverTest, DBusFailureRetry) {
  ...
  // Run() instantiate Observer, which synchronously calls
  // TpmAttestationDoesKeyExist() and it fails. Here schedule to make the
  // service available in the next run, so in retry, it will successfully
  // return the result.
  base::ThreadTaskRunnerHandle::Get()->PostTask(
