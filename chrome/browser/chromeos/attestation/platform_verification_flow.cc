...
void PlatformVerificationFlow::OnCertificateReady(
    ...
    const PrivacyCAOperationStatus operation_status,
    const std::string& certificate_chain) {
  // Log failure before checking the timer so all failures are logged, even if
  // they took too long.
  if (operation_status != PrivacyCAOperationStatus::SUCCESS) {
...
void PlatformVerificationFlow::RenewCertificateCallback(
    ...
    chromeos::attestation::PrivacyCAOperationStatus operation_status,
    const std::string& certificate_chain) {
  renewals_in_progress_.erase(old_certificate_chain);
  if (operation_status != chromeos::attestation::SUCCESS) {
