...
void DBusPrivacyCACallback(
    ...
    const base::RepeatingCallback<
        void(chromeos::attestation::AttestationServerStatus)> on_failure,
    const base::Location& from_here,
    const chromeos::attestation::AttestationServerStatus status,
...
void AttestationPolicyObserver::Start() {
  ...
  cryptohome_client_->TpmAttestationDoesKeyExist(
      ...
      base::BindRepeating(
          ...
          base::BindRepeating(&AttestationPolicyObserver::Reschedule,
                              weak_factory_.GetWeakPtr()),
          FROM_HERE));
}
      ...
      base::Bind(
          [](const base::RepeatingCallback<void(const std::string&)> on_success,
             const base::RepeatingCallback<void(
                 chromeos::attestation::AttestationServerStatus)> on_failure,
             const base::Location& from_here,
             const AttestationServerStatus status, const std::string& data) {
...
void AttestationPolicyObserver::HandleGetCertificateFailure(
    AttestationServerStatus status) {
