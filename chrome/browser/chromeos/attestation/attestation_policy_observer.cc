...
void DBusPrivacyCACallback(
    ...
    const base::Location& from_here,
    chromeos::attestation::AttestationServerStatus status,
...
void AttestationPolicyObserver::GetNewCertificate() {
  ...
  attestation_flow_->GetCertificate(
      ...
      base::Bind(
          [](const base::RepeatingCallback<void(const std::string&)> on_success,
             const base::RepeatingCallback<void(AttestationServerStatus)>
                 on_failure,
             const base::Location& from_here, AttestationServerStatus status,
             const std::string& data) {
