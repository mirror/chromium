...
void ExpectPrinterIdsAre(const std::vector<Printer>& printers,
  ...
  sort(printer_ids.begin(), printer_ids.end());
...
TEST_F(SyncedPrintersManagerTest, AddPrinter) {
  ...
  manager_->UpdateConfiguredPrinter(Printer(kPrinterId));
  ...
  ExpectPrinterIdsAre(observer.configured_printers(), {kPrinterId});
}
...
TEST_F(SyncedPrintersManagerTest, UpdatePrinter) {
  manager_->UpdateConfiguredPrinter(Printer(kPrinterId));
  Printer updated_printer(kPrinterId);
...
TEST_F(SyncedPrintersManagerTest, RemovePrinter) {
  ...
  manager_->RemoveConfiguredPrinter(kPrinterId);
