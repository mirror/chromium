...
class PrintersSyncBridgeTest : public testing::Test {
 ...
 protected:
  PrintersSyncBridge* bridge() { return &bridge_; }

 private:
  base::test::ScopedTaskEnvironment scoped_task_environment_;
  PrintersSyncBridge bridge_;
};
...
TEST_F(PrintersSyncBridgeTest, AddPrinterOverwrites) {
  ...
  bridge()->AddPrinter(std::move(first));

  auto overwrite = TestPrinter("0");
  overwrite->set_description(kLazerDescription);
  bridge()->AddPrinter(std::move(overwrite));

  base::Optional<PrinterSpecifics> printer = bridge()->GetPrinter("0");
...
TEST_F(PrintersSyncBridgeTest, UpdatePrinterMerge) {
  auto first = TestPrinter("0");
  first->set_description(kInkyDescription);
  first->set_uuid("DEADBEEFDEADBEEFDEADBEEF");
  bridge()->AddPrinter(std::move(first));

  auto overwrite = TestPrinter("0");
  overwrite->set_description(kLazerDescription);
  bool is_new = bridge()->UpdatePrinter(std::move(overwrite));
  EXPECT_FALSE(is_new);
...
TEST_F(PrintersSyncBridgeTest, UpdatePrinterNewPrinter) {
  ...
  first->set_description(kInkyDescription);
  first->set_uuid("DEADBEEFDEADBEEFDEADBEEF");
  bool is_new = bridge()->UpdatePrinter(std::move(first));
  EXPECT_TRUE(is_new);
