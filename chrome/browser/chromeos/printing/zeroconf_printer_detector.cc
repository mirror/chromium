...
// Create a unique identifier for this printer based on the service description.
...
std::string ZeroconfPrinterId(const std::string& service_name,
                              const ParsedMetadata& metadata) {
  base::MD5Context ctx;
  base::MD5Init(&ctx);
  base::MD5Update(&ctx, service_name);
...
// Attempt to fill detected_printer using the information in service_description
// and metadata.  Return true on success, false on failure.
bool ConvertToPrinter(const ServiceDescription& service_description,
                      const ParsedMetadata& metadata,
                      PrinterDetector::DetectedPrinter* detected_printer) {
  // We can at least try to set up a printer if all we have is a service namd
  // and a protocol, but if we don't have a service name, just fail.
  if (service_description.service_name.empty()) {
    return false;
  }

  Printer& printer = detected_printer->printer;
  printer.set_id(ZeroconfPrinterId(service_description.service_name, metadata));
  printer.set_uuid(printer.id());
  ...
  if (service_description.service_name == base::StringPiece(kIppServiceName)) {
    printer.set_uri(base::StringPrintf(
        "ipp://%s", service_description.service_name.c_str()));
  } else if (service_description.service_name ==
             base::StringPiece(kIppsServiceName)) {
    printer.set_uri(base::StringPrintf(
        "ipps://%s", service_description.service_name.c_str()));
  } else {
