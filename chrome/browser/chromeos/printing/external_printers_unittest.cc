...
constexpr char kConfigFilePolicyName[] = "printing_file";

class ExternalPrintersTest : public testing::Test {
 public:
  ExternalPrintersTest() : scoped_task_environment_() {
    ExternalPrinterPolicies policy_names;
    policy_names.configuration_file = kConfigFilePolicyName;
    policy_names.access_mode = "access_mode";
    policy_names.whitelist = "whitelist";
    policy_names.blacklist = "blacklist";
    external_printers_ = ExternalPrinters::Create(policy_names);
  }
...
TEST_F(ExternalPrintersTest, PolicyUnsetWithMissingData) {
  ...
  external_printers_->OnExternalDataSet(kConfigFilePolicyName, kUserId);
  external_printers_->OnExternalDataFetched(kConfigFilePolicyName, kUserId,
                                            std::move(data));
...
// The external policy was changed, results should be invalidated.
TEST_F(ExternalPrintersTest, ConfigurationChangedPolicyUnset) {
  auto data = base::MakeUnique<std::string>(kBulkPolicyContentsJson);
  external_printers_->SetAccessMode(ExternalPrinters::AccessMode::ALL_ACCESS);
  external_printers_->OnExternalDataSet(kConfigFilePolicyName, kUserId);
  external_printers_->OnExternalDataFetched(kConfigFilePolicyName, kUserId,
...
TEST_F(ExternalPrintersTest, ConfigurationClearedPolicyUnset) {
  auto data = base::MakeUnique<std::string>(kBulkPolicyContentsJson);
  external_printers_->SetAccessMode(ExternalPrinters::AccessMode::ALL_ACCESS);
  external_printers_->OnExternalDataSet(kConfigFilePolicyName, kUserId);
  external_printers_->OnExternalDataFetched(kConfigFilePolicyName, kUserId,
  ...
  external_printers_->OnExternalDataCleared(kConfigFilePolicyName, kUserId);
  EXPECT_FALSE(external_printers_->IsPolicySet());
  EXPECT_TRUE(external_printers_->GetPrinters().empty());
}
