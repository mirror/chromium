 ...
 private:
  ...
  void MaybeRecordInstallation(const Printer& printer) {
    ...
    if (IsUsbPrinter(printer)) {
      ...
      // For recording purposes, consider this an automatic install if we got
      // the PpdReference right at detection time, otherwise consider it a
      // user install.
      ...
      // Determine whether or not this was a detected printer with an
      // automatically supplied ppd.
      PrinterEventTracker::SetupMode mode;
      if (detected != nullptr &&
