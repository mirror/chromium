...
// Since CupsPrintersManager listens to multiple PrinterDetectors, we
// need to disambiguate incoming observer calls, and so can't
// implement PrinterDetector::Observer directly.
...
class CupsPrintersManagerImpl : public CupsPrintersManager,
 ...
 public:
  ...
  // Update or save a printer as a configured printer.  If this is the
  // same as an existing configured printer, the entry will be
  // updated.  If the printer appears in a class other than
  // configured, it will be moved to the configured class.
  ...
  void OnConfiguredPrintersChanged(
    ...
    printers_[kEnterprise] = raw_enterprise_printers_;
    FilterOutPrinters(
        &printers_[kEnterprise], [this](const Printer& printer) -> bool {
          return base::ContainsKey(configured_printer_ids_, printer.id());
        });
    for (auto& observer : observer_list_) {
      observer.OnPrintersChanged(kConfigured, printers_[kConfigured]);
      // Re-filtering the enterprise printers may have changed that list, so
      // notify for that as well.
  ...
  void OnEnterprisePrintersChanged(
      const std::vector<Printer>& printers) override {
    raw_enterprise_printers_ = printers_[kEnterprise] = printers;
    FilterOutPrinters(
        &printers_[kEnterprise], [this](const Printer& printer) -> bool {
          return base::ContainsKey(configured_printer_ids_, printer.id());
        });
    enterprise_printer_ids_ = GetIdsSet(printers_[kEnterprise]);
    for (auto& observer : observer_list_) {
      observer.OnPrintersChanged(kEnterprise, printers_[kEnterprise]);
    }
    RebuildDetectedLists();
  ...
  // Callback for PrinterDetectorObserverProxys owned by this object.
  void OnPrintersFound(
      int detector_id,
 ...
 private:
  ...
  //
  // TODO(justincarlson) - Implement this.  Until it's implemented, we'll
  // never filter out unavailable printers.
  ...
  void AddDetectedList(
    ...
    for (const PrinterDetector::DetectedPrinter& detected : detected_list) {
      if (base::ContainsKey(configured_printer_ids_, detected.printer.id()) ||
          base::ContainsKey(enterprise_printer_ids_, detected.printer.id())) {
        // It's already in one of the non-detected classes, so neither
        // automatic nor discovered is appropriate.  Skip it.
        continue;
      }
  ...
  // Rebuild the set of ids in the configured and enterprise classes.  Since a
  // change in this set can also affect what appears in the detected classes,
  // rebuild them as well.
  void RebuildSyncedPrinterIds() {
    enterprise_printer_ids_.clear();
    for (const auto& printer : printers_[kEnterprise]) {
      enterprise_printer_ids_.insert(printer.id());
    }
    RebuildDetectedLists();
  }
  ...
  // Source list of enterprise printers.  This is the unfiltered version
  // of what's in printers_[kEnterprise].
  std::vector<Printer> raw_enterprise_printers_;
  ...
  // Ids of all printers in either the configured or enterprise lists.
  std::unordered_set<std::string> configured_printer_ids_;
  std::unordered_set<std::string> enterprise_printer_ids_;
...
// Placeholder stubbed out detector implementation for Zeroconf detection
// (the actual implementation hasn't landed yet).  This stub has no
// state.
