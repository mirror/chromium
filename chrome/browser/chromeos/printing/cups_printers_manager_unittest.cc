...
void ExpectPrinterIdsAre(const std::vector<Printer>& printers,
  ...
  sort(found_ids.begin(), found_ids.end());
...
TEST_F(CupsPrintersManagerTest, SyncedPrintersTrumpDetections) {
  ...
  // both of the *0 printers are in synced printer lists, and so should not
  // be propogated to Discovered or Automatic classes.
  synced_printers_manager_.AddConfiguredPrinters(
      {Printer("DiscoveredPrinter0")});
  synced_printers_manager_.AddEnterprisePrinters(
      {Printer("AutomaticPrinter0")});
  scoped_task_environment_.RunUntilIdle();
  ExpectPrintersInClassAre(CupsPrintersManager::kDiscovered,
                           {"DiscoveredPrinter1"});
  ExpectPrintersInClassAre(CupsPrintersManager::kAutomatic,
  ...
  ExpectPrintersInClassAre(CupsPrintersManager::kConfigured,
                           {"DiscoveredPrinter0"});
  ExpectPrintersInClassAre(CupsPrintersManager::kEnterprise,
                           {"AutomaticPrinter0"});
}

// If the same printer shows up in the configured and enterprise sets, it should
// only be shown as a configured printer, since the user may have overridden
// some settings for that printer.  Test that we get also get filtering
// right when the set of enterprise printers changes.
TEST_F(CupsPrintersManagerTest, ConfiguredPrintersTrumpEnterprise) {
  synced_printers_manager_.AddConfiguredPrinters({Printer("Printer0")});
  synced_printers_manager_.AddEnterprisePrinters({Printer("Printer0")});
  scoped_task_environment_.RunUntilIdle();

  ExpectPrintersInClassAre(CupsPrintersManager::kConfigured, {"Printer0"});
  ExpectPrintersInClassAre(CupsPrintersManager::kEnterprise, {});

  // Remove Printer0 from the configured set, which should unmask it in the
  // enterprise set.
  synced_printers_manager_.RemoveConfiguredPrinters({"Printer0"});
  scoped_task_environment_.RunUntilIdle();

  ExpectPrintersInClassAre(CupsPrintersManager::kEnterprise, {"Printer0"});
  ExpectPrintersInClassAre(CupsPrintersManager::kConfigured, {});
}
...
TEST_F(CupsPrintersManagerTest, UpdateConfiguredPrinter) {
  // Start with a printer in each class named after the class it's in.
  ...
  manager_->UpdateConfiguredPrinter(Printer("Enterprise"));
  scoped_task_environment_.RunUntilIdle();
  ExpectPrintersInClassAre(CupsPrintersManager::kEnterprise, {});
  ExpectPrintersInClassAre(CupsPrintersManager::kConfigured,
  ...
  ExpectPrintersInClassAre(CupsPrintersManager::kAutomatic, {});
  ExpectPrintersInClassAre(CupsPrintersManager::kConfigured,
                           {"Automatic", "Configured", "Enterprise"});
  ...
  ExpectPrintersInClassAre(
      ...
      {"Automatic", "Configured", "Discovered", "Enterprise"});
  ...
  ExpectPrintersInClassAre(
      CupsPrintersManager::kConfigured,
      {"Automatic", "Configured", "Discovered", "Enterprise", "NewFangled"});
}
