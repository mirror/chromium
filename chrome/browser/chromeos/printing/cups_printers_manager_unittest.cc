...
// Test that the StartObserving() method in CupsPrinterManager will only execute
// a single time. In order to test this we check to see that the StartObserver()
// method in each of the printer detectors has only been called a single time
// even after calling StartObserving() multiple times.
TEST_F(CupsPrintersManagerTest, StartObservingCalledOnce) {
  manager_->StartObservingPrinterDetectors();
  scoped_task_environment_.RunUntilIdle();
