...
std::unique_ptr<Printer> UsbDeviceToPrinter(const device::UsbDevice& device) {
  ...
  if (printer->manufacturer().empty() && printer->model().empty()) {
    printer->set_display_name(
        l10n_util::GetStringUTF8(IDS_USB_PRINTER_UNKNOWNN_DISPLAY_NAME));
    ...
    printer->set_display_name(l10n_util::GetStringFUTF8(
        IDS_USB_PRINTER_DISPLAY_NAME_SINGLE, !printer->manufacturer().empty()
