...
// This corresponds to FakeServiceDeviceLister::AddServiceDescription.  Given
// the same name (and the correct ssl selection based on the service type) this
// generates the DetectedPrinter record we expect to from
// ZeroconfPrinterDectector when it gets that ServiceDescription.
...
class FakeServiceDiscoveryDeviceLister : public ServiceDiscoveryDeviceLister {
 public:
  ...
  // We expect that the first DiscoverNewDevices call will happen before
  // we have a delegate, since it will be done in the constructor of
  // ZeroconfPrinterDetector.  So if we have no delegate yet, we trust that
  ...
  // to be send when discovery is started.
 ...
 private:
  ...
  // Which services are no longer 'new' to this delegate?
...
TEST_F(ZeroconfPrinterDetectorTest, StableIds) {
  // Should get the same id regardless of service type.
  std::string id;
  ipp_lister_->Announce("Printer1");
  CreateDetector();
  scoped_task_environment_.RunUntilIdle();
  ...
  // Grab the id when it's an IPPS printer
  id = printers_found_callbacks_.back()[0].printer.id();
