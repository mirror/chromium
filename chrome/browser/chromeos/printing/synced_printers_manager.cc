...
// Returns the ExternalPrinters registered for the account associated with
// |profile|.  Returns nullptr if the user cannot be resolved.
ExternalPrinters* GetExternalPrinters(Profile* profile) {
  const user_manager::User* user =
      ProfileHelper::Get()->GetUserByProfile(profile);
  if (!user)
    return nullptr;

  return ExternalPrintersFactory::Get()->GetForAccountId(user->GetAccountId());
}
...
class SyncedPrintersManagerImpl : public SyncedPrintersManager,
 ...
 public:
  SyncedPrintersManagerImpl(Profile* profile,
    ...
    if (base::FeatureList::IsEnabled(features::kBulkPrinters)) {
      ExternalPrinters* printers = GetExternalPrinters(profile);
      if (printers) {
        printers_observer_ = base::MakeUnique<ExternalPrintersObserver>(
            UserPolicyNames(), printers, profile_);
      }
    }
 ...
 private:
  ...
  void BulkPolcyPrinters(
      std::vector<std::string>* new_ids,
      std::unordered_map<std::string, Printer>* new_printers) {
    ExternalPrinters* printers = GetExternalPrinters(profile_);
