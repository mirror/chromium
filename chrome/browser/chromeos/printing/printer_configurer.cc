...
class PrinterConfigurerImpl : public PrinterConfigurer {
 ...
 private:
  ...
  void PerformSetUp(const Printer& printer,
  ...
  // Callback for the endpoint resolver.  If the request was successful, sets
  // the |effective_uri| on |printer| with |endpoint| then continues setup.
  // |cb| is called with a result reporting the success or failure of the setup
