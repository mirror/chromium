...
class PrinterConfigurerImpl : public PrinterConfigurer {
 ...
 private:
  ...
  void OnAddedPrinter(const Printer& printer,
    ...
    DCHECK(setup_callback_);
    std::move(setup_callback_).Run(result);
    setup_callback_.Reset();
  }
  ...
  void OnDbusError() {
    ...
    DCHECK(setup_callback_);
    std::move(setup_callback_).Run(PrinterSetupResult::kDbusError);
    setup_callback_.Reset();
  }
  ...
  PrinterSetupCallback setup_callback_;
