...
#ifndef CHROME_BROWSER_CHROMEOS_PRINTING_PRINTERS_SYNC_MANAGER_H_
#define CHROME_BROWSER_CHROMEOS_PRINTING_PRINTERS_SYNC_MANAGER_H_
...
// PrintersSyncBridge.
class PrintersSyncManager : public KeyedService {
 public:
  class Observer {
   public:
  ...
  PrintersSyncManager(Profile* profile,
                      std::unique_ptr<PrintersSyncBridge> sync_bridge);
  ~PrintersSyncManager() override;
  ...
  // forbidden.
  void AddObserver(PrintersSyncManager::Observer* observer);
  ...
  void RemoveObserver(PrintersSyncManager::Observer* observer);
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(PrintersSyncManager);
};

}  // namespace chromeos

#endif  // CHROME_BROWSER_CHROMEOS_PRINTING_PRINTERS_SYNC_MANAGER_H_
