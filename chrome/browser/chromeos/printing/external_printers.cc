...
std::unique_ptr<PrinterCache> ParsePrinters(std::unique_ptr<std::string> data) {
  ...
  if (printer_list.size() > kMaxRecords) {
    LOG(ERROR) << "Too many records in printers policy: "
               << printer_list.size();
    return nullptr;
  }
...
class Restrictions {
 public:
  ...
  std::unique_ptr<PrinterView> SetAccessMode(
  ...
  std::unique_ptr<PrinterView> SetBlacklist(
  ...
  // the computed list.  Otherwise, nullptr.
  std::unique_ptr<PrinterView> SetWhitelist(
...
class ExternalPrintersImpl : public ExternalPrinters {
 public:
  ...
  void ClearData() override {
    ...
    // Update restrictions first because Clear has no side-effects (besides
    // clearing the data).
    restrictions_runner_->PostTask(
        FROM_HERE, base::BindOnce(&Restrictions::ClearData,
                                  base::Unretained(restrictions_.get())));

    // There are only things to do if we're empty.
    if (!printers_.empty()) {
  ...
  void SetAccessMode(AccessMode mode) override {
    ...
    base::PostTaskAndReplyWithResult(
        restrictions_runner_.get(), FROM_HERE,
        base::BindOnce(&Restrictions::SetAccessMode,
