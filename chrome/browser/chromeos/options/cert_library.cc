...
int CertLibrary::NumCertificates(CertType type) const {
  const net::ScopedCERTCertificateVector& cert_list =
...
void CertLibrary::OnCertificatesLoaded(
  ...
  for (const net::ScopedCERTCertificate& cert : cert_list) {
    certs_.push_back(net::x509_util::DupCERTCertificate(cert));
    net::CertType type = certificate::GetCertType(cert.get());
    switch (type) {
      case net::USER_CERT:
        user_certs_.push_back(net::x509_util::DupCERTCertificate(cert));
        break;
      case net::SERVER_CERT:
        server_certs_.push_back(net::x509_util::DupCERTCertificate(cert));
        break;
      case net::CA_CERT: {
        ...
        if (token_name != kRootCertificateTokenName)
          server_ca_certs_.push_back(net::x509_util::DupCERTCertificate(cert));
        break;
      }
      default:
        break;
...
const net::ScopedCERTCertificateVector& CertLibrary::GetCertificateListForType(
    CertType type) const {
