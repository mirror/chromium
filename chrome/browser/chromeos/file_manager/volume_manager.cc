  ...
  switch (event) {
    ...
    case chromeos::disks::DiskMountManager::RENAME_COMPLETED:
      if (error_code == chromeos::RENAME_ERROR_NONE) {
        // Find previous mount point label if exists
        std::string mount_label = "";
        auto diskMapIter = disk_mount_manager_->disks().find(device_path);
        if (diskMapIter != disk_mount_manager_->disks().end() &&
            !diskMapIter->second->base_mount_path().empty()) {
          mount_label = base::FilePath(diskMapIter->second->base_mount_path())
                            .BaseName()
                            .AsUTF8Unsafe();
        }
      ...
      for (auto& observer : observers_) {
        observer.OnRenameCompleted(device_path,
                                   error_code == chromeos::RENAME_ERROR_NONE);
      }
