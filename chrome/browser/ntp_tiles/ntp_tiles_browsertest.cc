...
MATCHER(TitleMatchesUrl, "title matches url") {
  return base::UTF8ToUTF16(arg.url.spec()).find(arg.title) != std::string::npos;
}
...
IN_PROC_BROWSER_TEST_F(NTPTilesTest, NonEmptyTitle) {
  ...
  EXPECT_THAT(tiles, Contains(TitleMatchesUrl()));
}
...
IN_PROC_BROWSER_TEST_F(NTPTilesTest, ServerRedirect) {
  ...
  // end), so verify the first URL is listed, but not the final URL.
