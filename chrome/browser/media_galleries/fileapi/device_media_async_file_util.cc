...
void OnReadDirectoryError(AsyncFileUtil::ReadDirectoryCallback callback,
                          base::File::Error error) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::IO);
  std::move(callback).Run(error, AsyncFileUtil::EntryList(), false /*no more*/);
}
...
void DeviceMediaAsyncFileUtil::ReadDirectory(
  ...
  MTPDeviceAsyncDelegate* delegate = GetMTPDeviceDelegate(url);
  if (!delegate) {
    OnReadDirectoryError(std::move(callback), base::File::FILE_ERROR_NOT_FOUND);
    return;
  }
...
void DeviceMediaAsyncFileUtil::OnDidReadDirectory(
  ...
  DCHECK_CURRENTLY_ON(content::BrowserThread::IO);
  if (!validate_media_files()) {
    OnDidCheckMediaForReadDirectory(std::move(callback), has_more,
                                    std::move(file_list));
    return;
  }
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&OnDidCheckMediaForReadDirectory, std::move(callback),
                     has_more));
}
