...
const bool kMockPolicyValue = false;

void VerifyLocalState() {
  const PrefService* prefs = g_browser_process->local_state();
  ASSERT_NE(prefs, nullptr);
  EXPECT_EQ(kMockPolicyValue,
            prefs->GetBoolean(prefs::kAllowFileSelectionDialogs));
}
...
class PolicyValueBrowserTest : public InProcessBrowserTest {
 protected:
  PolicyValueBrowserTest() {}
  ...
  bool policy_value() { return false; }
 ...
 private:
  ...
  void SetUpPlatformPolicyValue() {
    ...
    DWORD reg_value = kMockPolicyValue ? 1 : 0;
    base::win::RegKey key;

    ASSERT_EQ(ERROR_SUCCESS, key.Create(root, policy::kRegistryChromePolicyKey,
                                        KEY_SET_VALUE | KEY_WOW64_32KEY));
    ASSERT_EQ(ERROR_SUCCESS, key.WriteValue(kMockPolicyName, reg_value));
  }

  registry_util::RegistryOverrideManager registry_override_manager_;

  DISALLOW_COPY_AND_ASSIGN(PolicyValueBrowserTest);
};

IN_PROC_BROWSER_TEST_F(PolicyValueBrowserTest, VerifyLocalState) {
  VerifyLocalState();
}
