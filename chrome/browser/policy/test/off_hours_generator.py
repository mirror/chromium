...
# env PYTHONPATH=/usr/local/buildtools/current/sitecustomize:out_veyron_minnie/
# Release/pyproto:third_party/protobuf/python:out_veyron_minnie/Release/pyproto/
# chrome/browser/chromeos/policy/proto python chrome/browser/policy/test/
# off_hours_generator.py
...
if __name__ == '__main__':
  file = open("chrome/browser/chromeos/settings/device_off_hours_cleaner.cc",
              'w')
  ...
  for group in settings.DESCRIPTOR.fields:
    type_name = group.message_type.name
    field_name = group.name
    # group_message = eval('dp.' + group.message_type.name + '()')
    file.write('  if (off_policies.find("' + type_name + '") != ' \
                                                    'off_policies.end()) {\n')
    file.write('    policies.clear_' + field_name + '();\n')
    file.write('  }\n')
