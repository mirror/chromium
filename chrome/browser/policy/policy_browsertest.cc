...
class NoteTakingOnLockScreenPolicyTest : public PolicyTest {
 public:
  ...
  void SetUserLevelPrefValue(const std::string& app_id,
    ...
    browser()->profile()->GetPrefs()->SetBoolean(
        prefs::kNoteTakingAppEnabledOnLockScreen, enabled_on_lock_screen);
  }

  void SetPolicyValue(std::unique_ptr<base::Value> value) {
    PolicyMap policies;
    if (value) {
      policies.Set(key::kNoteTakingAppEnabledOnLockScreen,
                   POLICY_LEVEL_MANDATORY, POLICY_SCOPE_USER,
                   POLICY_SOURCE_CLOUD, std::move(value), nullptr);
    } else {
      policies.Erase(key::kNoteTakingAppEnabledOnLockScreen);
    }
    UpdateProviderPolicy(policies);
  }
...
IN_PROC_BROWSER_TEST_F(NoteTakingOnLockScreenPolicyTest,
  ...
  SetPolicyValue(base::MakeUnique<base::Value>(false));
...
IN_PROC_BROWSER_TEST_F(NoteTakingOnLockScreenPolicyTest,
                       EnableLockScreenNoteTakingByPolicy) {
  scoped_refptr<const extensions::Extension> app =
      LoadUnpackedExtension("lock_screen_apps/app_launch");
  ASSERT_TRUE(app);
  ASSERT_EQ(kTestAppId, app->id());
  ...
  SetPolicyValue(base::MakeUnique<base::Value>(true));
  EXPECT_EQ(chromeos::NoteTakingLockScreenSupport::kSelected,
            GetAppLockScreenStatus(app->id()));
