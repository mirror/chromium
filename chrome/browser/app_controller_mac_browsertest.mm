...
IN_PROC_BROWSER_TEST_F(AppControllerBrowserTest, CommandDuringShutdown) {
  ...
  EXPECT_EQ(2u, CountVisibleWindows());  // Browser and status bubble.
  ...
  EXPECT_EQ(1u, chrome::GetTotalBrowserCount());
  EXPECT_EQ(1u, CountVisibleWindows());  // Status bubble is hidden later.

  base::RunLoop().RunUntilIdle();

  EXPECT_EQ(0u, chrome::GetTotalBrowserCount());
  EXPECT_EQ(0u, CountVisibleWindows());  // Now they're both ordered out.
