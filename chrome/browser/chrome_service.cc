 ...
 public:
  IOThreadContext() {
    ...
    registry_with_source_info_.AddInterface(
        base::Bind(&SpellCheckHostImpl::Create), ui_task_runner);
#if BUILDFLAG(HAS_SPELLCHECK_PANEL)
...
service_manager::Connector* ChromeService::connector() {
  if (!connector_) {
    content::ServiceManagerConnection::GetForProcess()
        ->GetConnector()
        ->StartService(service_manager::Identity(chrome::mojom::kServiceName));
