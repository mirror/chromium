...
void ExtensionsMetricsProvider::ProvideExtensionInstallsMetrics(
  ...
  base::TimeDelta time_since_last_sample = now - last_sample_time_;
  last_sample_time_ = now;
  for (Profile* profile : profiles) {
    std::vector<ExtensionInstallProto> installs =
        GetInstallsForProfile(profile, now, time_since_last_sample);
...
ExtensionInstallProto ExtensionsMetricsProvider::ConstructInstallProto(
  ...
  install.set_installed_in_this_sample_period(
      now - prefs->GetInstallTime(extension.id()) < time_since_last_sample);
...
ExtensionsMetricsProvider::GetInstallsForProfile(
    Profile* profile,
    const base::Time& now,
    base::TimeDelta time_since_last_sample) {
  ...
  for (const auto& extension : *extensions) {
    installs.push_back(
        ConstructInstallProto(*extension, prefs, now, time_since_last_sample));
  }
