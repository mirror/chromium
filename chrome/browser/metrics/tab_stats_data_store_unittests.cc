...
TEST_F(TabStatsDataStoreTest, TrackTabUsageDuringInterval) {
  ...
  std::vector<TabStatsDataStore::WebContentsID> web_contents_id_vec;
  ...
  for (auto& iter : web_contents_vec) {
    ...
    web_contents_id_vec.push_back(
        data_store_->get_web_contents_id_for_testing(iter.get()));
  }
  ...
  TabStatsDataStore::WebContentsID old_web_contents_id = web_contents_id_vec[1];
  web_contents_id_vec[1] =
      data_store_->get_web_contents_id_for_testing(new_contents);
  web_contents_vec[1].reset(new_contents);
  // |old_contents| is invalid starting from here.
  EXPECT_TRUE(interval_map->find(old_web_contents_id) == interval_map->end());
