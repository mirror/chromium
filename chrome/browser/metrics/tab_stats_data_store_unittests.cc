...
TEST_F(TabStatsDataStoreTest, TrackTabUsageDuringInterval) {
  ...
  data_store_->ResetIntervalData(interval_map);
  data_store_->OnTabChangedEvent(web_contents_vec[2].get());
  EXPECT_TRUE(interval_map->find(web_contents_vec[0].get())
                  ->second.visible_or_audible_during_interval);
  EXPECT_FALSE(interval_map->find(web_contents_vec[1].get())
                   ->second.visible_or_audible_during_interval);
  EXPECT_TRUE(interval_map->find(web_contents_vec[2].get())
                  ->second.visible_or_audible_during_interval);

  TabStatsDataStore::TabStateDuringInterval tab_stats_copy =
      interval_map->find(web_contents_vec[0].get())->second;
  ...
  data_store_->ResetIntervalData(interval_map);
  EXPECT_TRUE(interval_map->find(old_contents) == interval_map->end());
  ...
  EXPECT_EQ(tab_stats_copy.exists_after_interval,
