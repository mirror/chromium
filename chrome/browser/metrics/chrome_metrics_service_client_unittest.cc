...
// This does the same thing as ConstructFilePathsForUploadDir but allows
// setting the PID directly.
base::FilePath CreateFilePathForPid(base::ProcessId pid) {
  return base::FilePath::FromUTF8Unsafe(
      base::StringPrintf("foo-12345-%lX.pma", static_cast<long>(pid)));
}
...
TEST(ChromeMetricsServiceClientTest, FilterFiles) {
  ...
  EXPECT_EQ(metrics::FileMetricsProvider::FILTER_PROCESS_FILE,
            ChromeMetricsServiceClient::FilterBrowserMetricsFiles(
                CreateFilePathForPid((my_pid & ~1) + 10)));
  EXPECT_EQ(metrics::FileMetricsProvider::FILTER_TRY_LATER,
            ChromeMetricsServiceClient::FilterBrowserMetricsFiles(
                CreateFilePathForPid((my_pid & ~1) + 11)));
}
