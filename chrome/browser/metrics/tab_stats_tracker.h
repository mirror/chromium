...
class TabsStatsDataStore {
 public:
  ...
  // Functions used to update the browser/tab count.
  ...
  // this.
  // TODO(sebmarchand): Store a list of browsers in this class and track the
  // number of tabs per browsers.
...
class TabStatsTracker : public TabStripModelObserver,
 ...
 public:
  ...
  // notify it.
  static TabStatsTracker* Initialize(PrefService* pref_service);
...
class TabStatsTracker::UmaStatsReportingDelegate {
 public:
  ...
  // The name of the histogram that records the maximum number of tabs opened in
  // the same browser in a day.
  static const char kMaxTabsPerBrowserInADayHistogramName[];

  // The name of the histogram that records the maximum number of browsers
  // opened in a day.
