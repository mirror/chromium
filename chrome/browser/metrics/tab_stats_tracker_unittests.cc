...
std::string GetIntervalMetricName(const char* base_name,
                                  size_t interval_in_sec) {
  return base::StringPrintf("%s_%zu", base_name, interval_in_sec);
}
...
TEST_F(TabStatsTrackerTest, TabUsageGetsReported) {
  const base::TimeDelta kReallyLongInterval = base::TimeDelta::FromDays(365);
  tab_stats_tracker_->data_store()->AddInterval(
      kReallyLongInterval.InSeconds());

  std::vector<std::unique_ptr<content::WebContents>> web_contentses;
  for (size_t i = 0; i < 3; ++i) {
    web_contentses.push_back(base::WrapUnique(CreateTestWebContents()));
  ...
  tab_stats_tracker_->OnInterval(kReallyLongInterval.InSeconds());

  histogram_tester_.ExpectBucketCount(
      GetIntervalMetricName(UmaStatsReportingDelegate::
                                kUnusedAndClosedInIntervalHistogramNameBase,
                            kReallyLongInterval.InSeconds()),
      0, 1);
  histogram_tester_.ExpectBucketCount(
      GetIntervalMetricName(
