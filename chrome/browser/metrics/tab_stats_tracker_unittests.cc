...
TEST_F(TabStatsTrackerTest, TabStatsGetsReloadedFromLocalState) {
  ...
  TestTabStatsTracker::TabStats stats =
      tab_stats_tracker_->tab_stats_data_store()->tab_stats();
  ...
  TestTabStatsTracker::TabStats stats2 =
      tab_stats_tracker_->tab_stats_data_store()->tab_stats();
  ...
  EXPECT_EQ(0, stats2.browser_count);
  EXPECT_EQ(0, stats2.total_tab_count);
}
