...
class TestTabStatsTracker : public TabStatsTracker {
 public:
  ...
  size_t AddWindows(size_t window_count) {
    ...
    return tab_stats_data_store_->tab_stats().window_count;
  }

  size_t RemoveWindows(size_t window_count) {
    EXPECT_LE(window_count, tab_stats_data_store_->tab_stats().window_count);
    for (size_t i = 0; i < window_count; ++i)
...
TEST_F(TabStatsTrackerTest, TabStatsGetsReloadedFromLocalState) {
  // This tests creates add some tabs/windows to the |tab_stats_tracker_|
  // instance and then reinitializes it (and so the count of active tabs/windows
...
TEST_F(TabStatsTrackerTest, StatsGetReportedDaily) {
  ...
  EXPECT_EQ(expected_window_count,
            static_cast<size_t>(pref_service_.GetInteger(
                metrics::prefs::kTabStatsWindowCountMax)));
