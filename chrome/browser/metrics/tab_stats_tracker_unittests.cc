...
IN_PROC_BROWSER_TEST_F(TabStatsTrackerBrowserTest,
  ...
  expected_stats.total_tabs_count = 1;
  expected_stats.total_tabs_count_max = 1;
  expected_stats.max_tabs_per_window = 1;
  ...
  ++expected_stats.total_tabs_count;
  ++expected_stats.total_tabs_count_max;
  ++expected_stats.max_tabs_per_window;
  EXPECT_TRUE(TabStatsMatchExpectations(expected_stats,
                                        tab_stats_tracker_->tab_stats()));
...
class TestTabStatsTracker : public TabStatsTracker {
 public:
  ...
  size_t AddTabs(size_t tab_count) {
    tab_stats_.total_tabs_count += tab_count;
    return tab_stats().total_tabs_count;
  }

  size_t RemoveTabs(size_t tab_count) {
    EXPECT_LE(tab_count, tab_stats().total_tabs_count);
    tab_stats_.total_tabs_count -= tab_count;
    return tab_stats().total_tabs_count;
  }
...
TEST_F(TabStatsTrackerTest, OnResume) {
  ...
  histogram_tester_.ExpectTotalCount(
      TabStatsTracker::UmaStatsReportingDelegate::kNumberOfTabsOnResume, 0);
  ...
  histogram_tester_.ExpectTotalCount(
      TabStatsTracker::UmaStatsReportingDelegate::kNumberOfTabsOnResume,
      count_buckets.size());
  EXPECT_EQ(
