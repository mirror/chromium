...
void TabStatsTracker::WebContentsUsageObserver::WebContentsDestroyed() {
  data_store_->OnTabRemoved(web_contents());
}
...
void TabStatsTracker::TabDetachedAt(content::WebContents* contents, int index) {
  DCHECK(!base::ContainsKey(detached_tabs_, contents));
  detached_tabs_.insert(contents);
}
...
void TabStatsTracker::OnTabRemoved(content::WebContents* web_contents) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  // Do not notify the data store here, this will be done when the observer for
  // this WebContents gets notified that it'll be destroyed.
