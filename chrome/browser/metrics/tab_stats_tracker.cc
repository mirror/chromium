...
bool IsValidInterval(const base::TimeDelta& interval) {
  for (base::TimeDelta iter : kTabUsageReportingIntervals) {
    if (iter == interval)
      return true;
  }
  return false;
...
class TabStatsTracker::WebContentsUsageObserver
 ...
 public:
  ...
  void WebContentsDestroyed() override {
    // TabStatsTracker::TabClosingAt and TabStatsTracker::TabReplacedAt are
    // responsible of destroying the observer associated with a WebContents, so
    // the only reason for this function to be called if that a WebContents gets
    // destroyed directly without calling any of these two functions, this can
    // happen if a WebContents gets destroyed directly (e.g. if there's a
    // timeout in the closing path).
    //
    // TabStripModel also has an observer on this WebContents and there's no
    // guarantee on the order in which the |WebContentsDestroyed| function will
    // be called for this web_contents, so there's 2 possible scenarios:
    // 1) TabStripModel::WebContentsData::WebContentsDestroyed calls
    //    TabStripModelObserver::TabDetachedAt and this method gets called
    //    later.
    // 2) This method gets called and we later get a call to
    //    TabStripModel::WebContentsData::WebContentsDestroyed and then
    //    TabStripModelObserver::TabDetachedAt.
    //
    // In case #1 the tracker needs to remember that a WebContents has been
    // detached, and if this method gets called for this WebContents then it's
    // time to remove it from the tracker. Doing this will also have the side
    // effect of deleting |this|.
    //
    // In case #2 the tracker needs to remember that this WebContents is in the
    // process of being destroyed but we shouldn't stop tracking it yet, it'll
    // be removed once we get the |TabDetachedAt| signal for it (so we don't
    // receive this signal for a WebContents that we don't track anymore).
    tab_stats_tracker_->OnWebContentsDestroyed(web_contents());

    // The call above might free |this| and so nothing should be done on this
...
void TabStatsTracker::TabInsertedAt(TabStripModel* model,
                                    ...
                                    bool foreground) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  if (detached_tabs_.erase(web_contents) == 0)
...
void TabStatsTracker::TabDetachedAt(content::WebContents* web_contents,
  ...
  // If this WebContents is in the list of WebContentes that are being destroyed
  // then it means that it's now time to remove it. See the comment in
  // WebContentsObserver::WebContentsDestroyed for more details.
  if (base::ContainsKey(webcontentes_being_destroyed_, web_contents)) {
    OnTabRemoved(web_contents);
    webcontentes_being_destroyed_.erase(web_contents);
  } else if (base::ContainsKey(web_contents_usage_observers_, web_contents)) {
...
void TabStatsTracker::TabClosingAt(TabStripModel* model,
                                   ...
                                   int index) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  webcontentes_being_destroyed_.insert(web_contents);
}
...
void TabStatsTracker::OnWebContentsDestroyed(
    content::WebContents* web_contents) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  // See the comment in WebContentsObserver::WebContentsDestroyed for more
  // details on this logic.
  if (base::ContainsKey(detached_tabs_, web_contents)) {
    // We already received the |TabDetachedAt| signal for this WebContents, it
    // can now be removed.
    OnTabRemoved(web_contents);
    detached_tabs_.erase(web_contents);
  } else {
    // The removal of this WebContents will be handled once we get the
    // |TabDetachedAt| signal for it.
    webcontentes_being_destroyed_.insert(web_contents);
  }
}
