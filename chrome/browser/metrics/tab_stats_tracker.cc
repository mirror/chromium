...
class TabStatsTracker::WebContentsUsageObserver
 ...
 public:
  ...
  void WasShown() override { data_store_->OnTabVisible(web_contents()); }

  void FrameDeleted(content::RenderFrameHost* render_frame_host) override {
    // Indicates to the data store that this WebContents is being destroyed, in
    // theory this should never happen as every WebContents owned by the
    // TabStatsTracker should belong to a TabStrip and we should get a call to
    // TabStatsTracker::TabClosingAt or TabStatsTracker::TabReplacedAt when a
    // WebContents gets destroyed and this observer should get destroyed, but
    // we've observed situations where this is not the case, see
    // https://crbug.com/801887.
    if (web_contents() ==
        content::WebContents::FromRenderFrameHost(render_frame_host)) {
      data_store_->EnsureTabHasBeenRemoved(web_contents());
    }
  }
