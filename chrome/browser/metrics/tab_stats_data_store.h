...
class TabStatsDataStore {
 public:
  ...
  // Invoked when the TabStatsTracker implementation of
  // TabStripModelObserver::TabChangedAt gets called.
  void OnTabChangedEvent(content::WebContents* web_contents);
 ...
 protected:
  ...
  void OnTabInteraction(const content::WebContents* web_contents);

  // Records that a tab became visible.
  void OnTabVisible(const content::WebContents* web_contents);
