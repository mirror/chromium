...
class TestTabStatsTracker : public TabStatsTracker {
 public:
  ...
  using TabStatsTracker::OnTabAdded;
  using TabStatsTracker::OnTabRemoved;
...
TEST_F(TabStatsTrackerTest, TabUsageGetsReported) {
  ...
  for (size_t i = 0; i < 4; ++i) {
    ...
    tab_stats_tracker_->OnTabAdded(web_contentses[i].get());
  }
  ...
  tab_stats_tracker_->OnTabRemoved(web_contentses.back().get());
...
TEST_F(TabStatsTrackerTest, TabDeletionGetsHandledProperly) {
  constexpr base::TimeDelta kValidLongInterval = base::TimeDelta::FromHours(12);
  ...
  g_browser_process->tab_strip_model()->AppendWebContents(web_contents_1.get());
  g_browser_process->tab_strip_model()->AppendWebContents(web_contents_2.get());
  ...
  TabStatsDataStore::TabID tab_id_2 =
      tab_stats_tracker_->data_store()
          ->GetTabIDForTesting(web_contents_2.get())
          .value();
  web_contents_2.reset(nullptr);
  EXPECT_TRUE(base::ContainsKey(*interval_map, tab_id_2));
  tab_stats_tracker_->OnInterval(kValidLongInterval, interval_map);
  EXPECT_EQ(1U, interval_map->size());
  EXPECT_FALSE(base::ContainsKey(*interval_map, tab_id_2));

  // Do this a second time, ensures that the situation where there's no existing
  // tabs is handled properly.
  TabStatsDataStore::TabID tab_id_1 =
      tab_stats_tracker_->data_store()
          ->GetTabIDForTesting(web_contents_1.get())
          .value();
  web_contents_1.reset(nullptr);
  EXPECT_TRUE(base::ContainsKey(*interval_map, tab_id_1));
  tab_stats_tracker_->OnInterval(kValidLongInterval, interval_map);
  EXPECT_EQ(0U, interval_map->size());
  EXPECT_FALSE(base::ContainsKey(*interval_map, tab_id_1));
}
