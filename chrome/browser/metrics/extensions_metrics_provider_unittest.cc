...
// Wrapper class to allow access to install metric generating functions.
class InstallsTestExtensionsMetricsProvider : public ExtensionsMetricsProvider {
 public:
  static metrics::ExtensionInstallProto ConstructInstallProto(
    ...
    return ExtensionsMetricsProvider::ConstructInstallProto(
        extension, prefs, now, time_since_last_sample);
  }
  static std::vector<metrics::ExtensionInstallProto> GetInstallsForProfile(
      Profile* profile,
      const base::Time& now,
      base::TimeDelta time_since_last_sample) {
    return ExtensionsMetricsProvider::GetInstallsForProfile(
        profile, now, time_since_last_sample);
  }
};
...
class ExtensionMetricsProviderInstallsTest
 ...
 public:
  ...
  void SetUp() override {
    ...
    time_since_last_sample_ = base::TimeDelta::FromMinutes(30);
  }

  ExtensionInstallProto ConstructProto(const Extension& extension) {
    return InstallsTestExtensionsMetricsProvider::ConstructInstallProto(
        extension, prefs_, now_, time_since_last_sample_);
  }
  std::vector<ExtensionInstallProto> GetInstallsForProfile() {
    return InstallsTestExtensionsMetricsProvider::GetInstallsForProfile(
        profile(), now_, time_since_last_sample_);
  }

  extensions::ExtensionPrefs* prefs() { return prefs_; }
  void set_now(const base::Time& now) { now_ = now; }
  void set_time_since_last_sample(base::TimeDelta time_since_last_sample) {
    time_since_last_sample_ = time_since_last_sample;
  }

 private:
  extensions::ExtensionPrefs* prefs_ = nullptr;
  base::Time now_;
  base::TimeDelta time_since_last_sample_;
...
TEST_F(ExtensionMetricsProviderInstallsTest, TestProtoConstruction) {
...

  {
    ...
    add_extension(extension.get());
    set_now(base::Time::Now() + base::TimeDelta::FromMinutes(60));
    ExtensionInstallProto install = ConstructProto(*extension);
