...
class UkmBrowserTest : public SyncTest {
 public:
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    // Note to reviewers, this is broken, see b/713390. Browsertest currently
    // failed because of this.
    command_line->AppendSwitchASCII(switches::kDisableFeatures,
                                    metrics::kMetricsReportingFeature.name);
  }
...
IN_PROC_BROWSER_TEST_F(UkmBrowserTest, MetricsReportingCheck) {
  ...
  EXPECT_FALSE(base::FeatureList::IsEnabled(metrics::kMetricsReportingFeature));
