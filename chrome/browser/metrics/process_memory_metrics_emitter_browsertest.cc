...
class ProcessMemoryMetricsEmitterTest : public ExtensionBrowserTest {
 ...
 protected:
  ...
  void CheckAllUkmEntries(size_t entry_count = 1u) {
    ...
    EXPECT_EQ(browser_entry_count, entry_count);
    EXPECT_EQ(total_entry_count, entry_count);
  ...
  void CheckPageInfoUkmMetrics(GURL url,
    ...
    EXPECT_EQ(found_count, entry_count);
  }
...
IN_PROC_BROWSER_TEST_F(ProcessMemoryMetricsEmitterTest,
  ...
  const auto& entries =
      test_ukm_recorder_->GetEntriesByName(UkmEntry::kEntryName);
  for (const auto* entry : entries) {
    EXPECT_EQ(test_ukm_recorder_->GetSourceForSourceId(entry->source_id),
              nullptr);
  }
}
