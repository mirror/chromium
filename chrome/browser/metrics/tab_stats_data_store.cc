...
void TabStatsDataStore::ResetIntervalData(
  ...
  for (auto& iter : existing_tabs_)
    AddTabToInterval(iter.first, iter.second, true, interval_map);
}
...
void TabStatsDataStore::EnsureTabHasBeenRemoved(
    content::WebContents* web_contents) {
  // If the tab is still in the list of existing tabs then send a crash report
  // for investigation on https://crbug.com/801887, otherwise do nothing.
  if (base::ContainsKey(existing_tabs_, web_contents)) {
