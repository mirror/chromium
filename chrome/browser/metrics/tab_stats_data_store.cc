...
TabStatsDataStore::FinalizeAndGetIntervalMap(size_t interval_time_in_sec) {
  auto iter = interval_maps_.find(interval_time_in_sec);
  if (iter == interval_maps_.end())
    return nullptr;
  for (auto tab_iter : *iter->second.get()) {
    tab_iter.second.exists_after_interval =
        existing_tabs_.find(tab_iter.first) != existing_tabs_.end();
  }
