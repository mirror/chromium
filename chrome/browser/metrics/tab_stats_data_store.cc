...
TabStatsDataStore::TabID GetNewWebContentsId() {
...
void TabStatsDataStore::OnTabAdded(content::WebContents* web_contents) {
  ...
  // If |tab_ids_| already contains an entry for this WebContents then it means
  // that the memory backing has being reused for another WebContents, it's safe
  // to assume that there won't be any callback referencing to the previous
  // WebContents at this point, just remove it from the map.
  if (base::ContainsKey(tab_ids_, web_contents))
    tab_ids_.erase(web_contents);

  ++tab_stats_.total_tab_count;
  existing_tabs_.insert(web_contents);
  TabID tab_id = GetNewWebContentsId();
  tab_ids_.insert(std::make_pair(web_contents, tab_id));
  for (auto& interval_map : interval_maps_) {
    AddTabToInterval(web_contents, tab_id, /* existed_before_interval */ false,
                     interval_map.get());
  }
...
void TabStatsDataStore::OnTabRemoved(content::WebContents* web_contents) {
  ...
  // We can't remove this WebContents from |tab_ids_| here as this function
  // might get called by
  // TabStatsTracker::WebContentsUsageObserver::WebContentsDestroyed and it's
  // not guaranteed that it'll be the last observer to be called for this
  // WebContents, which means that a subsequent observer might trigger an
  // action that change the state of this WebContents (e.g. play a sound,
  // display a dialog, make it active...).
}
...
void TabStatsDataStore::OnTabReplaced(content::WebContents* old_contents,
  ...
  DCHECK(old_contents);
  DCHECK(new_contents);
  DCHECK(base::ContainsKey(tab_ids_, old_contents));
  DCHECK_GT(tab_stats_.total_tab_count, 0U);
  TabID old_contents_id = tab_ids_[old_contents];
  tab_ids_[new_contents] = old_contents_id;
...
void TabStatsDataStore::OnTabInteraction(content::WebContents* web_contents) {
  DCHECK(base::ContainsKey(tab_ids_, web_contents));
  TabID web_contents_id = GetTabID(web_contents);
...
void TabStatsDataStore::ResetIntervalData(
  ...
  for (auto* iter : existing_tabs_)
    AddTabToInterval(iter, GetTabID(iter), true, interval_map);
}
...
void TabStatsDataStore::AddTabToInterval(
  ...
  (*interval_map)[tab_id].existed_before_interval = existed_before_interval;
  (*interval_map)[tab_id].exists_currently = true;
  (*interval_map)[tab_id].visible_or_audible_during_interval =
      visible_or_audible;
  (*interval_map)[tab_id].interacted_during_interval = false;
}

TabStatsDataStore::TabID TabStatsDataStore::GetTabID(
    content::WebContents* web_contents) {
  DCHECK(base::ContainsKey(tab_ids_, web_contents));
  return tab_ids_[web_contents];
}
