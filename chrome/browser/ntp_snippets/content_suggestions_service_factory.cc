...
KeyedService* ContentSuggestionsServiceFactory::BuildServiceInstanceFor(
  ...
  // TODO(gaschler): Move block into Register* function
  std::unique_ptr<ContextualSuggestionsSource> contextual_suggestions_source;
  if (base::FeatureList::IsEnabled(
  ...
  auto* service = new ContentSuggestionsService(
      ...
      std::move(scheduler), std::move(contextual_suggestions_source));
