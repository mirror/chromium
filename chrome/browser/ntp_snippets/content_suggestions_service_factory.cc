...
KeyedService* ContentSuggestionsServiceFactory::BuildServiceInstanceFor(
  ...
  auto wrapped_debug_logger = base::MakeUnique<ntp_snippets::Logger>();
  ntp_snippets::Logger* debug_logger = wrapped_debug_logger.get();
  ...
  auto* service = new ContentSuggestionsService(
      ...
      std::move(scheduler), std::move(wrapped_debug_logger));
