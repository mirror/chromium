...
Polymer({
  ...
  properties: {
    ...
    isClosed: {
      type: Boolean,
      computed: 'computeIsClosed_(openState_, depth)',
    },
  ...
  observers: [
    'updateAriaExpanded_(hasChildFolder_, isClosed)',
  ],
  ...
  changeKeyboardSelection_: function(xDirection, yDirection, currentFocus) {
    ...
    if (xDirection == 1) {
      ...
      // otherwise.
      if (this.hasChildFolder_ && !this.isClosed) {
    ...
    if (!isChildFolderNodeFocused && yDirection == 1 && !this.isClosed) {
  ...
  getLastVisibleDescendant_: function() {
    var children = this.getChildFolderNodes_();
    if (this.isClosed || children.length == 0)
      return this;
  ...
  toggleFolder_: function(e) {
    this.dispatch(
        bookmarks.actions.changeFolderOpen(this.itemId, this.isClosed));
    e.stopPropagation();
  },
  ...
  computeIsClosed_: function(openState, depth) {
