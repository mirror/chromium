...
cr.define('extensions', function() {
  ...
  const KioskDialog = Polymer({
    ...
    updateApp_: function(app) {
      let index;
      for (index = 0; index < this.apps_.length; index++) {
        if (this.apps_[index].id == app.id)
          break;
      }
    ...
    /** @private */
    onDoneTap_: function() {
      // No need to close the modal because kiosk-dialog will detach on-close.
      this.$.dialog.close();
    },
