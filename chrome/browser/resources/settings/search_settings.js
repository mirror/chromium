...
cr.define('settings', function() {
  ...
  /**
   * Finds all previous search bubble highlighted nodes under |node| (both
   * within self and children's Shadow DOM) and replaces the highlights (yellow
   * search bubbles) with the original text node.
   * TODO(dpapad): Consider making this a private method of TopLevelSearchTask.
   * @param {!Node} node
   * @private
   */
  function findAndRemoveSearchBubbleHighlights_(node) {
    const searchBubbles =
  ...
  class TopLevelSearchTask extends Task {
    ...
    /** @override */
    exec() {
      cr.search_highlight_utils.findAndRemoveHighlights(this.node);
      findAndRemoveSearchBubbleHighlights_(this.node);
