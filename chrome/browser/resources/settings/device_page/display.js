...
/**
 * The list of deltas between two consecutive zoom factors. Any display must
...
Polymer({
  ...
  properties: {
    ...
    /** @private {number} Number of ticks in display zoom slider. */
    numOfZoomTicks_: {type: Number, value: NumOfZoomTicks},
  ...
  getZoomValues_: function(selectedDisplay) {
    ...
    // If either the maximum width or minimum width was reached and clamping was
    // performed, then update the total range of logical resolutions and ensure
    // that everything lies within the maximum and minimum resolution range.
    ...
    let delta = (maxZoom - minZoom) / this.numOfZoomTicks_;
    ...
    for (let i = 1; i < ZoomFactorDeltas.length; i++) {
      ...
      const zoomOutCount = this.numOfZoomTicks_ - zoomInCount - 1;
    ...
    let zoomValues = [];
    for (i = 0; i < this.numOfZoomTicks_; i++)
