...
Polymer({
  ...
  getZoomValues_: function(selectedDisplay) {
    ...
    // display.
    const MAX_RESOLUTION_WIDTH = Math.max(4096, effectiveWidth);

    // The minimum logical resolution width allowed when zooming in for a
    // display.
    const MIN_RESOLUTION_WIDTH = Math.min(640, effectiveWidth);
    ...
    let maxWidth =
        Math.min(Math.round(effectiveWidth * 2.0), MAX_RESOLUTION_WIDTH);
    let minWidth =
        Math.max(Math.round(effectiveWidth / 2.0), MIN_RESOLUTION_WIDTH);
    ...
    if (2 * (maxWidth - minWidth) != 3 * effectiveWidth) {
      ...
      if (maxWidth == MAX_RESOLUTION_WIDTH)
        minWidth = Math.max(maxWidth - interval, MIN_RESOLUTION_WIDTH);
      if (minWidth == MIN_RESOLUTION_WIDTH)
        maxWidth = Math.min(minWidth + interval, MAX_RESOLUTION_WIDTH);
    }
