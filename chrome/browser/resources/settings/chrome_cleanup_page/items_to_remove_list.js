 ...
 */
settings.CHROME_CLEANUP_DEFAULT_ITEM_TO_SHOW = 4;
 ...
 *    </items-to-remove-list>
 *
 *    <!-- Items list initially shows |CHROME_CLEANUP_DEFAULT_ITEM_TO_SHOW|
 *         items. If there are more than |CHROME_CLEANUP_DEFAULT_ITEM_TO_SHOW|
...
Polymer({
   ...
   *  2. Otherwise:
   *     (A) If size(itemsToShow) < CHROME_CLEANUP_DEFAULT_ITEM_TO_SHOW, then
   *         all items will be visible.
   *     (B) Otherwise, exactly |CHROME_CLEANUP_DEFAULT_ITEM_TO_SHOW - 1| will
   *         be visible and the "show more" link will be rendered. The list
   *         presented to the user will contain exactly
   *         |CHROME_CLEANUP_DEFAULT_ITEM_TO_SHOW| elements, and the last one
  ...
  updateVisibleState_: function(itemsToShow, initiallyExpanded) {
    // Ignore if |itemsToShow| is empty to avoid initial state issues that force
    // the list to be always expanded.
    if (this.itemsToShow.length == 0)
      return;

    // Start expanded if there are less than
    // |settings.CHROME_CLEANUP_DEFAULT_ITEM_TO_SHOW| items to show.
    this.expanded_ = this.initiallyExpanded ||
        this.itemsToShow.length <= settings.CHROME_CLEANUP_DEFAULT_ITEM_TO_SHOW;
    ...
    this.visibleItems_ = this.itemsToShow.slice(
        0, settings.CHROME_CLEANUP_DEFAULT_ITEM_TO_SHOW - 1);
