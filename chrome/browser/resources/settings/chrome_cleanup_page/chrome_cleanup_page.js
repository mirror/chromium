...
Polymer({
   ...
   * This is set to true by attached() and set to false whenever there is
   * a transition to a non-idle state while this tab is open.
   * @private {bool}
   */
  renderScanOfferedByDefault_: false,
  ...
  attached: function() {
    ...
    // If this page is first rendered on the idle state, ignore previous
  ...
  updateCardFlags_: function(flags, explanation) {
    this.showExplanation_ = (explanation != '');
  ...
  buildCardStateToComponentsMap_: function() {
    ...
    // link to the Help Center article. Use instead the default link available
    // on Settings page sections.
