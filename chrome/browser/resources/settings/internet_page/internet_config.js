...
Polymer({
  ...
  onCertificateListsChanged_: function() {
    this.networkingPrivate.getCertificateLists(function(certificateLists) {
      var caCerts = [this.getDefaultCert_(
          this.i18n('networkCAUseDefault'), 'do-not-check')];
      caCerts.concat(certificateLists.serverCaCertificates);
      caCerts.push(
          this.getDefaultCert_(this.i18n('networkCADoNotCheck'), 'default'));
  ...
  updateSecurity_: function() {
    ...
    if (this.security_ == CrOnc.Security.WPA_EAP) {
      ...
      this.setEap_(eap);
      // Networks requiring a user certificate can not be shared.
   ...
   * @private
   */
  testEapPropertiesSet_: function() {
    var eap = this.getEap_(this.configProperties_);
    if (!eap)
      return false;
    var eapType = eap.Outer;
    if (eapType != CrOnc.EAPType.EAP_TLS)
      return true;
