...
Polymer({
  ...
  updateHeight_: function() {
    const height = window.getComputedStyle(this.$.dialog).height;
    const heightNum = parseInt(height.slice(0, height.length - 2), 10) -
        this.$$('.cloudprint-promo').offsetHeight -
        this.$.buttons.offsetHeight - this.$.titleBox.offsetHeight;

    this.$.lists.style.height = heightNum.toString() + 'px';
    this.$.lists.style.minHeight = heightNum.toString() + 'px';
  },
