...
Polymer({
  ...
  properties: {
    ...
    pagesToPrint_: {
      ...
      computed: 'computePagesToPrint_(' +
          'inputString_, allSelected_, model.documentNumPages)',
    },
  ...
  computePagesToPrint_: function() {
    if (this.inputString_.trim() == '' || this.allSelected_) {
      const pages =
          Array.from(new Array(this.model.documentNumPages), (x, i) => i + 1);
      this.set('errorState_', PagesInputErrorState.NO_ERROR);
      return pages;
    }
    if (!this.$$('.user-value').validity.valid) {
      this.set('errorState_', PagesInputErrorState.INVALID_SYNTAX);
      return [];
    }
    ...
    for (let index = 0; index < ranges.length; index++) {
      ...
      let min = parseInt(limits[0]);
      if (min < 1) {
        this.set('errorState_', PagesInputErrorState.INVALID_SYNTAX);
        return [];
      }
      if (limits.length == 1) {
        if (min > this.model.documentNumPages) {
          this.set('errorState_', PagesInputErrorState.OUT_OF_BOUNDS);
          return [];
        }
      ...
      let max = parseInt(limits[1]);
      if (isNaN(min))
        min = 1;
      if (isNaN(max))
        max = this.model.documentNumPages;
      if (min > max) {
        this.set('errorState_', PagesInputErrorState.INVALID_SYNTAX);
        return [];
      }
      if (max > this.model.documentNumPages) {
        this.set('errorState_', PagesInputErrorState.OUT_OF_BOUNDS);
        return [];
      }
