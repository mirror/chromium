...
Polymer({
  ...
  getPreview_: function() {
    ...
    /**
     * @param {string} settingName The setting name to get the value for.
    ...
    const dpi = /** @type {{horizontal_dpi: (number | undefined),
        ...
        getValue('dpi'));
    const ticket = {
      pageRange: getValue('ranges'),
      mediaSize: getValue('mediaSize'),
      landscape: getValue('layout'),
      color: this.destination.getNativeColorModel(
          /** @type {boolean} */ (getValue('color'))),
      headerFooterEnabled: getValue('headerFooter'),
      marginsType: getValue('margins'),
      ...
      fitToPageEnabled: getValue('fitToPage'),
      scaleFactor: parseInt(getValue('scaling'), 10),
      shouldPrintBackgrounds: getValue('cssBackground'),
      shouldPrintSelectionOnly: getValue('selectionOnly'),
      ...
      duplex: getValue('duplex') ? print_preview_new.DuplexMode.LONG_EDGE :
