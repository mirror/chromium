...
Polymer({
  ...
  updateSettingsAvailable_: function() {
    ...
    let layoutAvailable = caps && caps.layout && caps.layout.option &&
        this.documentInfo.isModifiable && !this.documentInfo.hasCssMediaStyles;
    if (layoutAvailable) {
      let hasAutoOrPortraitOption = false;
      let hasLandscapeOption = false;
      caps.layout.option.forEach(function(option) {
      ...
      layoutAvailable = hasLandscapeOption && hasAutoOrPortraitOption;
    }

    let colorAvailable = caps && caps.color && caps.color.option;
    if (colorAvailable) {
      let hasColor = false;
      let hasMonochrome = false;
      caps.color.option.forEach(option => {
        const type = assert(option.type);
        hasColor = hasColor || (this.COLOR_TYPES_.indexOf(type) >= 0);
        hasMonochrome =
            hasMonochrome || (this.MONOCHROME_TYPES_.indexOf(type) >= 0);
      });
      colorAvailable = hasColor && hasMonochrome;
    }

    this.set('settings.copies.available', caps && caps.copies);
    this.set('settings.collate.available', caps && caps.collate);
    this.set('settings.layout.available', layoutAvailable);
    this.set('settings.color.available', colorAvailable);
    ...
    this.set(
        ...
        caps && caps.dpi && caps.dpi.option && caps.dpi.option.length > 1);
    this.set(
