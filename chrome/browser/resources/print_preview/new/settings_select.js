...
Polymer({
   ...
   * @return {boolean} Whether the option should be selected.
   * @private
   */
  getIsSelected_: function(option) {
    if (this.getValue_(option) == this.selectedValue)
      return true;
    if (!option.is_default)
      return false;
    let otherSelected = false;
    Polymer.dom(this.root).querySelectorAll('option').forEach(other => {
      otherSelected |= this.getValue_(other) == this.selectedValue;
    });
    if (otherSelected)
      return false;
    this.selectedValue = this.getValue_(option);
    return true;
  },
