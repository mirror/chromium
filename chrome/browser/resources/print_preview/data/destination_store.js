...
cr.define('print_preview', function() {
  ...
  function DestinationStore(userInfo, appState, listenerTracker) {
    ...
    /**
     * Whether a search for destinations of a certain type is in progress.
     * @private {!Map<!print_preview.PrinterType, boolean>}
     */
    this.isDestinationSearchInProgress_ = new Map([
      [print_preview.PrinterType.EXTENSION_PRINTER, false],
      [print_preview.PrinterType.PRIVET_PRINTER, false],
      [print_preview.PrinterType.LOCAL_PRINTER, false]
    ]);

    /**
     * Whether the destination store has already loaded or is loading all
     * destinations of a given type.
     * @private {!Map<!print_preview.PrinterType, boolean>}
     */
    this.hasLoadedAllDestinations_ = new Map([
      [print_preview.PrinterType.EXTENSION_PRINTER, false],
      [print_preview.PrinterType.PRIVET_PRINTER, false],
      [print_preview.PrinterType.LOCAL_PRINTER, false]
    ]);
  ...
  DestinationStore.prototype = {
    ...
    get isLocalDestinationSearchInProgress() {
      return Array.from(this.isDestinationSearchInProgress_.values())
          .some(el => el === true);
    },
     ...
     */
    startLoadDestinations: function(type) {
      if (this.hasLoadedAllDestinations_.get(type))
        return;
      this.isDestinationSearchInProgress_.set(type, true);
      this.hasLoadedAllDestinations_.set(type, false);
      this.nativeLayer_.getPrinters(type).then(
          this.onDestinationSearchDone_.bind(this, type), () => {
            ...
            this.hasLoadedAllDestinations_.set(type, true);
            this.isDestinationSearchInProgress_.set(type, false);
          });
      cr.dispatchSimpleEvent(
    ...
    startLoadAllDestinations: function() {
      this.startLoadCloudDestinations();
      Object.values(print_preview.PrinterType).forEach(function(printer_type) {
        if (printer_type !== print_preview.PrinterType.PDF_PRINTER)
          this.startLoadDestinations(printer_type);
      }, this);
    },
    ...
    /**
     * Called when destinations have been retrieved by the native layer.
     */
    onDestinationSearchDone_: function(type) {
      this.isDestinationSearchInProgress_.set(type, false);
      this.hasLoadedAllDestinations_.set(type, true);
      cr.dispatchSimpleEvent(
          this, DestinationStore.EventType.DESTINATION_SEARCH_DONE);
    ...
    /**
     * Called when a printer or printers are retrieved
     ...
     */
    onPrintersAdded_: function(type, printers) {
      if (type == print_preview.PrinterType.PRIVET_PRINTER) {
        ...
        if (printer.serviceName == this.waitForRegisterDestination_ &&
          ...
          this.waitForRegisteredDestination_ = null;
          this.onDestinationsReload();
          return;
        }
      }
      var printersMapped = printers.map(function(printer) {
        return print_preview.DestinationParser.parse(type, printer);
      });
      this.insertDestinations_(
          printers
              ...
              .reduce(function(dests1, dests2) {
                return dests1.concat(dests2);
              }, []));
    },
