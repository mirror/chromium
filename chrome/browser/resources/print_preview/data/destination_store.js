...
cr.define('print_preview', function() {
  ...
  class DestinationStore extends cr.EventTarget {
     ...
     * @param {boolean} isSelectedDestinationValid Whether the most recent
     *     destination (selected by default) is valid.
    ...
    init(
        ...
        isSelectedDestinationValid, recentDestinations) {
      this.pdfPrinterEnabled_ = !isInAppKioskMode;
      this.systemDefaultDestinationId_ = systemDefaultDestinationId;
      this.createLocalPdfPrintDestination_();

      if (!isSelectedDestinationValid) {
      ...
      for (let i = 0; i < recentDestinations.length; i++) {
        origin = recentDestinations[i].origin;
        id = recentDestinations[i].id;
        account = recentDestinations[i].account || '';
        name = recentDestinations[i].displayName || '';
        capabilities = recentDestinations[i].capabilities;
        extensionId = recentDestinations[i].extensionId || '';
        extensionName = recentDestinations[i].extensionName || '';
        const candidate =
    ...
    convertPreselectedToDestinationMatch_() {
      if (this.selectedDestination_ && this.selectedDestination_.origin &&
