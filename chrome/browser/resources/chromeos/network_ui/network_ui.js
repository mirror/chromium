...
var NetworkUI = (function() {
   ...
   *
   * @param {!CrOnc.StateProperties} state
  ...
  var createStateTableIcon = function(state) {
    ...
    // Note: cr-network-icon requires a network state, but only requires the
    // Type field which is also required in a device state, so this is safe.
    icon.networkState = /** @type {CrOnc.NetworkStateProperties} */ (state);
   ...
   * @param {!Array<!CrOnc.StateProperties>} states
   */
   ...
   * @return {string} A valid HTMLElement id.
   */
  var idFromType = function(guid) {
    return '_' + guid.replace(/[{}]_/g, '').toLowerCase();
  };
  ...
  var createExpandedRow = function(state, baseRow) {
    ...
    detailCell.id = guid ? idFromGuid(guid) : idFromType(type);
    ...
    if (selectedId == 'shill') {
      if (guid)
        ...
        chrome.send('getShillDeviceProperties', [type]);
