...
cr.define('mobile', function() {
  ...
  MobileSetup.prototype = {
    ...
    /**
     * Handler for loadabort on the payment portal webview.
    ...
    /**
     * Handler for loadcommit on the payment portal webview.
     ...
     *
     * Note that <code>portalFrame</code> webview will only ever contain data:
     * and web URL's - it will never embed the mobile setup extension
     * resources.
     *
    ...
    setCarrierPage_: function(url) {
      ...
      assert(
          new URL(url).origin ==
          new URL(MobileSetup.EXTENSION_PAGE_URL).origin);
