...
unpacker.Compressor.prototype.getArchiveFile_ = function() {
  ...
  var saveZipFileInParentDir = function(rootEntry) {
    // If parent directory of currently selected files is
    // available then we deduplicate |suggestedName| and
    // save the zip file.
    if (!rootEntry) {
  ...
  chrome.fileManagerPrivate.getVolumeMetadataList(function(volumeMetadataList) {
    ...
    Promise
        .all(unpacker.Compressor.prototype.requestAccessPermissionForVolumes_(
            volumeMetadataList))
        .then(function(result) {
          var entries = compressor.items_.map(function(item) {
            return item.entry;
          });

          chrome.fileManagerPrivate.resolveIsolatedEntries(
              entries, function(result) {
