...
uint32_t unix_to_dosdate(const int64_t datetime) {
  tm* tm_datetime = localtime(&datetime);

  return (tm_datetime->tm_year - 80) << 25 | (tm_datetime->tm_mon + 1) << 21 |
         tm_datetime->tm_mday << 16 | tm_datetime->tm_hour << 11 |
         tm_datetime->tm_min << 5 | (tm_datetime->tm_sec / 2);
}
...
bool CompressorArchiveMinizip::AddToArchive(const std::string& filename,
  ...
  zipfileMetadata.dos_date = unix_to_dosdate((int64_t)modification_time / 1000);
