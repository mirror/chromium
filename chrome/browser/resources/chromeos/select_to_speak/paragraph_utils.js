...
/**
 * Determines the inlineTextBox children of a staticText node that appears
 * at the given character index.
  ...
  while (index < nodes.length) {
    if (node.name !== undefined && !isWhitespace(node.name)) {
      ...
      if (node.role == RoleType.INLINE_TEXT_BOX && node.parent !== undefined) {
        ...
        newNode = new NodeGroupItem(
            node, result.text.length, node.role == RoleType.STATIC_TEXT);
      }
      if (newNode) {
