...
function getNextWordStart(text, indexAfter) {
  // Find the start of the next word after a word boundary.
  let re = /\b\S/;
  // Fall back to the given index if we can't find a match.
  return nextWordHelper(text, indexAfter, re, indexAfter);
}
...
function getNextWordEnd(text, indexAfter) {
  // Find the end of the next word, which is followed by whitespace.
  let re = /\S\s/;
  // Fall back to the full length of the text if we can't find
  // a match.
  return nextWordHelper(text, indexAfter, re, text.length - 2) + 1;
}
...
SelectToSpeak.prototype = {
            ...
            (function(nodeGroup, isLast, event) {
              if (event.type == 'start' && nodeGroup.nodes.length > 0) {
                ...
                if (this.wordHighlight_) {
                  ...
                  let nodeEnd = getNextWordEnd(nodeGroup.text, 0);
                  this.currentNodeWord_ = {'start': 0, 'end': nodeEnd};
                }
  ...
  updateNodeHighlight_: function(text, charIndex) {
    ...
    let nodeEnd = nextWordEnd - this.currentNode_.startChar;
    if (this.currentNodeWord_ == null ||
        nodeStart >= this.currentNodeWord_.end) {
      // Don't repeat work if it is already done.
