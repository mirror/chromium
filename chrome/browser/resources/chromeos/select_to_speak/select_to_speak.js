...
/**
 * Class representing a selected node and the offset of that
 * selection.
 ...
 * @return {!SelectedNode} The node matching the selected offset.
 */
...
SelectToSpeak.prototype = {
  ...
  requestSpeakSelectedText_: function(focusedNode) {
    ...
    // TODO: Is the affinity of a selection meaningful?
    // Highlighting may be forwards or backwards. Make sure we start at the
    // first node.
    if (dir == constants.Dir.FORWARD) {
      let first = getNodeForSelection(params.anchorObject, params.anchorOffset);
      let last = getNodeForSelection(params.focusObject, params.focusOffset);
      selectedNode = first.node;
      firstNodeOffset = first.offset;
      finalNode = last.node;
      finalNodeOffset = last.offset;
    } else {
      let last = getNodeForSelection(params.anchorObject, params.anchorOffset);
      let first = getNodeForSelection(params.focusObject, params.focusOffset);
      selectedNode = first.node;
      firstNodeOffset = first.offset;
      finalNode = last.node;
      finalNodeOffset = last.offset;
    ...
    let nodes = [];
    if (firstNodeOffset < selectedNode.name.length) {
    ...
    this.startSpeechQueue_(nodes, firstNodeOffset, finalNodeOffset);
