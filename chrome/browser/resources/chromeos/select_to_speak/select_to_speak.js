 ...
 *      searching.
 * @param {NodeGroupItem} node The node whose name we are searching.
 * @return {number} The index of the next word's start
 */
function getNextWordStart(text, indexAfter, node) {
  if (node.node.wordStarts === undefined) {
  ...
  for (var i = 0; i < node.node.wordStarts.length; i++) {
    if (node.node.wordStarts[i] + node.startChar < indexAfter) {
      continue;
    }
    return node.node.wordStarts[i] + node.startChar;
  }
  // Default.
  return indexAfter;
}
 ...
 */
function getNextWordEnd(text, indexAfter, node) {
  if (node.node.wordEnds === undefined) {
    // Try to parse using a regex, which is imperfect.
  ...
  for (var i = 0; i < node.node.wordEnds.length; i++) {
    if (node.node.wordEnds[i] + node.startChar < indexAfter) {
      continue;
    }
    return node.node.wordEnds[i] + node.startChar;
  }
  // Default.
  return text.length;
}
