...
cr.define('login', function() {
   ...
   * @const {!number}
   */
  var TOUCH_MOVE_TRESHOLD = 1;

  /**
   * The time in milliseconds after which touch event should not be considered
   * as part of a swipe event.
   * @const {!number}
   */
  var SWIPE_TIMEOUT_MS = 500;
   ...
   * touch move longer than TOUCH_MOVE_TRESHOLD. On touch gesture end, it
  ...
  SwipeDetector.prototype = {
    /**
     * The current, last detected, touch movement. It will be used to detect if
     * the touch gesture was a swipe when the touch ends.
     * @private {?{start: !TouchInfo, end: !TouchInfo}}
     */
    current_: null,

    /**
     * The last detected touch event - and the potential start of the next
     * detected movement (to be considered for swipe).
     * @private {?TouchInfo}
     */
    nextStart_: null,
    ...
    addTouch_: function(touch) {
      ...
      if (this.nextStart_ &&
          manhattanDistance(this.nextStart_, touch) >= TOUCH_MOVE_TRESHOLD) {
     ...
     * cleared if it did not start within the SWIPE_TIMEOUT_MS amount of time
     * from now.
     * Similarly, if recorded potential start of the next is not within the
     * required time, it gets cleared as well.
     * @private
     */
    handleSwipeTimeout_: function() {
      ...
      function isTouchTimeValid(touchTime, now) {
        ...
        return timeDelta >= 0 && timeDelta < SWIPE_TIMEOUT_MS;
      }
      ...
      if (timeSinceEvent < 0 || timeSinceEvent > SWIPE_TIMEOUT_MS) {
        this.clearSwipe_();
        return;
      }

      // Schedule the next interval to be |SWIPE_TIMEOUT_MS| from the latest
      // detected touch event.
      this.swipeTimeout_ = setTimeout(
          this.handleSwipeTimeout_.bind(this),
          SWIPE_TIMEOUT_MS - timeSinceEvent);
    },
    ...
    onTouchMove_: function(e) {
      ...
      if (!this.started_() || touch.identifier != this.touchId_) {
        ...
        // time.
        this.swipeTimeout_ =
            setTimeout(this.handleSwipeTimeout_.bind(this), SWIPE_TIMEOUT_MS);
    ...
    onTouchEnd_: function(e) {
      if (!this.enabled_ || !this.started_())
        return;
  ...
  TopHeaderBar.prototype = {
    ...
    handleSwipe_: function(velocity) {
      ...
      // Not fast enough - ignore the swipe.
