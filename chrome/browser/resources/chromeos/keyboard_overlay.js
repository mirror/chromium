...
/**
 * The top row key labels are modified in the new Eve keyboard layout.
 */
var TOP_ROW_KEY_LABEL_TO_FUNCTION_LABEL_EVE_LAYOUT = {
  'back': 'f1',
  'reload': 'f2',
  'full screen': 'f3',
  'switch window': 'f4',
...
var LABEL_TO_LOCALIZED_LABEL_ID = {
  ...
  'home': 'keyboardOverlayHomeKeyLabel',
 ...
 * as well as whether this device has the new Eve layout.
 * @param {string} label The unmodified key label as read from
 * keyboard_overlay_data.js.
 */
function getActualLabel(label) {
  if (!topRowKeysAreFunctionKeys())
    return label;

  var topRowMap = hasEveLayout() ?
      TOP_ROW_KEY_LABEL_TO_FUNCTION_LABEL_EVE_LAYOUT :
      TOP_ROW_KEY_LABEL_TO_FUNCTION_LABEL;

  if (!(label in topRowMap))
    return label;

  return topRowMap[label];
}
...
function getKeyLabel(keyData, modifiers) {
  ...
  if (keyData.label)
    return getActualLabel(keyData.label);
...
function getKeyTextValue(keyData) {
  if (keyData.label) {
    ...
    // a localized one.
    var labelToBeLocalized = getActualLabel(keyData.label);
 ...
 * Eve keyboard layout.
 */
function hasEveLayout() {
  return loadTimeData.getBoolean('keyboardOverlayUsesEveLayout');
}
 ...
 * are the key data.
 * @param {Object<string, boolean>} keyIdsToRemove The keys to remove from the
 * current layout. The keys are the key IDs, and the values are ignored.
 */
...
function initDiamondKey() {
  ...
  var keysToRemove = {
    '00': true,  // The power key.
  };
  modifyLayoutAndKeyData(newLayoutData, newKeyData, keysToRemove);
}

/**
 * Initializes the layout for devices which have the new Eve keyboard layout.
 */
function initEveLayout() {
  var newLayoutData = {
