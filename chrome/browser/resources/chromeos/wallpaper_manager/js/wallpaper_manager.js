...
function WallpaperManager(dialogDom) {
  ...
  if (loadTimeData.getBoolean('showBackdropWallpapers')) {
    ...
    // (ie. category such as Art, Landscape etc.).
    this.collectionNames_ = null;
    // The number of failed attempts in a row. Cleared when there's a successful
    // attempt.
    this.consecutiveFailedAttempts_ = 0;
 ...
 * @param {Object} collectionNames A map between the id and display name of the
...
WallpaperManager.prototype.onGetCollectionNamesSucceeded_ = function(
    collectionNames) {
  this.consecutiveFailedAttempts_ = 0;
  this.collectionNames_ = collectionNames;
  ...
  this.showCollection_(Object.keys(this.collectionNames_)[0]);
};
...
WallpaperManager.prototype.onGetCollectionNamesFailed_ = function() {
  ++this.consecutiveFailedAttempts_;
  if (this.consecutiveFailedAttempts_ != MAX_FAILED_ATTEMPTS) {
