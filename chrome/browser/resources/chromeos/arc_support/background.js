...
class ActiveDirectoryAuthPage {
   ...
   * @param {deviceManagementUrlPrefix} Device management server URL prefix used
   *        to detect if the SAML flow finished. DM server is the SAML service
   *        provider.
   */
  ...
  onAuthViewResponseStarted_(details) {
    ...
    if (this.deviceManagementUrlPrefix_ &&
        details.url.substring(0, this.deviceManagementUrlPrefix_.length) ==
            this.deviceManagementUrlPrefix_) {
