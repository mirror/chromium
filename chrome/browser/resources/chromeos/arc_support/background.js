 ...
 * @type {WebView}
 */
var activeDirectoryAuthView = null;
...
class ActiveDirectoryAuthPage {
  ...
  constructor(container) {
    var onAuthViewRequestResponseStarted = function(details) {
      ...
      if (this.dmUrlPrefix_ &&
          details.url.substring(0, this.dmUrlPrefix_.length) ==
              this.dmUrlPrefix_) {
    ...
    var onAuthViewErrorOccurred = function(details) {
      // Retry triggers net::ERR_ABORTED, so ignore it.
      if (details.error != 'net::ERR_ABORTED') {
        sendNativeMessage(
            'onAuthFailed',
            {error_message: 'Error occurred: ' + details.error});
      }
    };

    var onAuthCancel = function() {
      closeWindow();
    };
    ...
    this.authView_.request.onResponseStarted.addListener(
        onAuthViewRequestResponseStarted, requestFilter);
    this.authView_.request.onErrorOccurred.addListener(
        onAuthViewErrorOccurred, requestFilter);

    container.querySelector('#button-active-directory-auth-cancel')
        .addEventListener('click', onAuthCancel);
  }
   ...
   * @param {dmUrlPrefix} Device management server URL prefix used to detect if
   *        the SAML flow finished. DM server is the SAML service provider.
   */
  setUrls(federationUrl, dmUrlPrefix) {
    this.authView_.src = federationUrl;
    this.dmUrlPrefix_ = dmUrlPrefix;
  }
}
...
function onNativeMessage(message) {
  ...
  if (message.action == 'initialize') {
    ...
    activeDirectoryAuthPage.setUrls(message.federationUrl, message.dmUrlPrefix);
  }
}
