...
SysInternals.SysInfoApiCpuResult;
...
SysInternals.SysInfoApiMemoryResult;
...
SysInternals.SysInfoApiZramResult;

/** @typedef {{
  const: {counterMax: number},
  cpus: Array<SysInternals.SysInfoApiCpuResult>,
  memory: SysInternals.SysInfoApiMemoryResult,
  zram: SysInternals.SysInfoApiZramResult,
}} */
SysInternals.SysInfoApiResult;
...
SysInternals.GeneralMemoryType;
...
/** @const {Array<string>} */
const CPU_COLOR_SET = [
  '#393984', '#2a2b79', '#113270', '#a170d0', '#473f85', '#2fa2ff', '#bc9dfb',
  '#ff93e2'
];
 ...
 * @param {SysInternals.SysInfoApiResult} data
...
/** @type {SysInternals.GeneralCpuType} */
 ...
 * @param {Array<SysInternals.SysInfoApiCpuResult>} cpus
...
function updateCpuData_(cpus, timeTick) {
  ...
  if (isInfoPage()) {
    const /** SysInternals.GeneralCpuType */ generalCpu =
        SysInternals.generalInfo.cpu;
    generalCpu.core = cpus.length.toString();
    const /** number */ usageNum =
        (allKernel + allUser) / (allKernel + allUser + allIdle);
...
/** @type {SysInternals.GeneralMemoryType} */
 ...
 * @param {SysInternals.SysInfoApiMemoryResult} memory
 * @param {number} timeTick
 */
function updateMemoryData_(memory, timeTick) {
  ...
  if (isInfoPage()) {
    const /** SysInternals.GeneralMemoryType */ generalMem =
        SysInternals.generalInfo.memory;
    generalMem.total =
...
/** @type {SysInternals.GeneralZramType} */
 ...
 * @param {SysInternals.SysInfoApiZramResult} zram
 * @param {number} timeTick
 */
function updateZramData_(zram, timeTick) {
  ...
  if (isInfoPage()) {
    const /** SysInternals.GeneralZramType */ generalZram =
        SysInternals.generalInfo.zram;
    generalZram.total =
 ...
 * |LineChart.Label.getSuitableUint()|.
 * @param {number} value
...
function getValueWithUnit(value, units, unitBase) {
  const result = LineChart.Label.getSuitableUnit(value, units, unitBase);
...
function updateInfoPage() {
  const /** SysInternals.GeneralCpuType */ cpu = SysInternals.generalInfo.cpu;
  ...
  const /** SysInternals.GeneralMemoryType */ memory =
      SysInternals.generalInfo.memory;
  ...
  const /** SysInternals.GeneralZramType */ zram =
      SysInternals.generalInfo.zram;
...
function setupCPUPage() {
  ...
  /* The cpu data series will initialize after we get the system cpu data. Set a
   * timeout to wait for the data series initialize. */
  ...
  lineChart.setSubChart(
      LineChart.LabelAlign.LEFT, UNIT_PURE_NUMBER, UNITBASE_NO_CARRY);
  const /** Array<string> */ UNIT_PERCENTAGE = ['%'];
  lineChart.setSubChart(
      LineChart.LabelAlign.RIGHT, UNIT_PERCENTAGE, UNITBASE_NO_CARRY);
...
function setupMemoryPage() {
  ...
  lineChart.setSubChart(
      LineChart.LabelAlign.RIGHT, UNIT_MEMORY, UNITBASE_MEMORY);
...
function setupZramPage() {
  ...
  lineChart.setSubChart(
      LineChart.LabelAlign.RIGHT, UNIT_MEMORY, UNITBASE_MEMORY);
  lineChart.addDataSeries(LineChart.LabelAlign.RIGHT, dataSeries.origDataSize);
  lineChart.addDataSeries(LineChart.LabelAlign.RIGHT, dataSeries.comprDataSize);
  lineChart.addDataSeries(LineChart.LabelAlign.RIGHT, dataSeries.memUsedTotal);
