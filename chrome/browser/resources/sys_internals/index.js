...
cr.define('SysInternals', function() {
  ...
  /** @const{Object<string, PromiseResolver>} */
  ...
  function startUpdateRequests() {
    ...
    updateFun();
    setInterval(updateFun, UPDATE_PERIOD);
  }
  ...
  /** @const{Object<string, CounterType>} */
  const counterDict = {};
  ...
  function getDiffAndUpdateCounter(name, newValue, timestamp) {
    if (counterDict[name] == undefined) {
      /** @const {CounterType} */
      counterDict[name] = {value: newValue, timestamp: timestamp};
      return 0;
    }
    const /** CounterType */ counter = counterDict[name];
  ...
  function getDiffPerSecAndUpdateCounter(name, newValue, timestamp) {
    const /** number */ oldTimeStamp =
        counterDict[name] ? counterDict[name].timestamp : -1;
