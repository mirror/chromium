...
cr.define('SysInternals', function() {
   ...
   */
  function setUpdateRequestInterval() {
  ...
  function getDiffAndUpdateCounter(name, newValue, timestamp) {
    if (counterDict_[name] == undefined) {
      /** @const {CounterType} */
      counterDict_[name] = {value: newValue, timestamp: timestamp};
      return 0;
    }
    const /** CounterType */ counter = counterDict_[name];
  ...
  function getDiffPerSecAndUpdateCounter(name, newValue, timestamp) {
    const /** number */ oldTimeStamp =
        counterDict_[name] ? counterDict_[name].timestamp : -1;
