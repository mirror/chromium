...
cr.define('SysInternals', function() {
  ...
  function initDataSeries() {
    ...
    memDataSeries.memUsed = new LineChart.DataSeries(
        'Used Memory', SysInternals.MEMORY_COLOR_SET[0]);
    /** @const {!LineChart.DataSeries} */
    memDataSeries.swapUsed =
        new LineChart.DataSeries('Used Swap', SysInternals.MEMORY_COLOR_SET[1]);
    /** @const {!LineChart.DataSeries} */
    memDataSeries.pswpin =
        new LineChart.DataSeries('Pswpin', SysInternals.MEMORY_COLOR_SET[2]);
    /** @const {!LineChart.DataSeries} */
    memDataSeries.pswpout =
        new LineChart.DataSeries('Pswpout', SysInternals.MEMORY_COLOR_SET[3]);

    const /** ZramDataSeriesSet */ zramDataSeries = dataSeries.zram;
    /** @const {!LineChart.DataSeries} */
    zramDataSeries.origDataSize = new LineChart.DataSeries(
        'Original Data Size', SysInternals.ZRAM_COLOR_SET[0]);
    /** @const {!LineChart.DataSeries} */
    zramDataSeries.comprDataSize = new LineChart.DataSeries(
        'Compress Data Size', SysInternals.ZRAM_COLOR_SET[1]);
    /** @const {!LineChart.DataSeries} */
    zramDataSeries.memUsedTotal = new LineChart.DataSeries(
        'Total Memory', SysInternals.ZRAM_COLOR_SET[2]);
    zramDataSeries.memUsedTotal.setMenuTextBlack(true);
    /** @const {!LineChart.DataSeries} */
    zramDataSeries.numReads =
        new LineChart.DataSeries('Num Reads', SysInternals.ZRAM_COLOR_SET[3]);
    zramDataSeries.numReads.setMenuTextBlack(true);
    /** @const {!LineChart.DataSeries} */
    zramDataSeries.numWrites =
        new LineChart.DataSeries('Num Writes', SysInternals.ZRAM_COLOR_SET[4]);
  }
  ...
  function startUpdateRequests() {
    ...
    updateFun();
    setInterval(updateFun, SysInternals.UPDATE_PERIOD);
  }
  ...
  function updateInfoPage() {
    const setPersentageById = function(/** string */ id, /** number */ value) {
      setTextById(
    ...
    setPersentageById('infopage-cpu-usage', cpu.usage);
    setPersentageById('infopage-cpu-kernel', cpu.kernel);
    setPersentageById('infopage-cpu-user', cpu.user);
    setPersentageById('infopage-cpu-idle', cpu.idle);
    ...
    setPersentageById('infopage-zram-compr-ratio', zram.comprRatio);
  }
