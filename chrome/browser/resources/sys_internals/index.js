...
function updateCpuData_(cpus, timeStamp) {
  ...
  for (let i = 0; i < cpus.length; ++i) {
    ...
    const /** number */ kernel =
        getDiffAndUpdateCounter_(`cpu_${i}_kernel`, cpus[i].kernel, timeStamp);
    const /** number */ idle =
...
/**
 * Initialize the data series of the page. This function will be called for the
 * first time we get the cpus data.
 * @param {Array<SysInfoApiCpuResult>} cpus
 */
...
function getDiffAndUpdateCounter_(name, newValue, timeStamp) {
   ...
   * circulating. Get the real increments with the |counterMax|. The result is
   * guaranteed to be greater than zero because the maximum difference of two
   * counter never greater than |counterMax|. */
  if (valueDelta < 0) {
...
function setupMemoryPage() {
  ...
  const /** MemoryDataSeriesSet */ memDataSeries =
      SysInternals.dataSeries.memory;
  lineChart.setSubChart(
      UnitLabelAlign.LEFT, UNITS_NUMBER_PER_SECOND, UNITBASE_NUMBER_PER_SECOND);
