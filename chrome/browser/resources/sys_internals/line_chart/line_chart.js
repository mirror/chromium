...
LineChart.LineChart = class {
  ...
  onMouseWheel_(event) {
    ...
    const wheelX = event.wheelDeltaX / LineChart.MOUSE_WHEEL_UNITS;
    const wheelY = event.wheelDeltaY / LineChart.MOUSE_WHEEL_UNITS;
  ...
  updateScrollBar_() {
    const /** number */ scrollRange =
        Math.max(this.getChartWidth_() - this.canvas_.width, 0);
