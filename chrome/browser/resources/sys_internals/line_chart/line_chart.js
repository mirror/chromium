...
/**
 * The LineChart implementation. To create a canvas line chart. See README for
 * usage.
 * @const
 */
LineChart.LineChart = class {
  ...
  initCanvas_() {
    this.enrollNoPassiveEvent_(
        this.canvas_, 'mousewheel', this.onMouseWheel_.bind(this));
    this.enrollNoPassiveEvent_(
   ...
   * sure this featrue is disabled. Some ChromeBook may set its value to ture as
   * the default value, see https://crbug.com/761698.
   * @param {Element} element
   * @param {string} name
   * @param {function(Event): undefined} handler
   */
  enrollNoPassiveEvent_(element, name, handler) {
   ...
   * @param {number} unitBase - See |LineChart.Label|.
   */
  setSubChart(align, units, unitBase) {
    this.clearSubChart(align);
    const /** LineChart.Label */ label = new LineChart.Label(units, unitBase);
