...
LineChart.LineChart = class {
  ...
  initCanvas_() {
    this.enrollNonPassiveEvent_(this.canvas_, 'mousewheel', this.onMouseWheel_);
    this.enrollNonPassiveEvent_(this.canvas_, 'mousedown', this.onMouseDown_);
    this.enrollNonPassiveEvent_(this.canvas_, 'mousemove', this.onMouseMove_);
    this.enrollNonPassiveEvent_(this.canvas_, 'mouseup', this.onMouseUpOrOut_);
    this.enrollNonPassiveEvent_(this.canvas_, 'mouseout', this.onMouseUpOrOut_);
    this.enrollNonPassiveEvent_(this.canvas_, 'touchstart', this.onTouchStart_);
    this.enrollNonPassiveEvent_(this.canvas_, 'touchmove', this.onTouchMove_);
    this.enrollNonPassiveEvent_(this.canvas_, 'touchend', this.onTouchEnd_);
    this.enrollNonPassiveEvent_(
        this.canvas_, 'touchcancel', this.onTouchCancel_);
  ...
  /**
   * Passive event will disable |preventDefault()|, set passive to false to make
   * sure this feature is disabled. The default value of passive is true on some
   * Chromebooks. see https://crbug.com/761698.
   * @param {Element} element
   * @param {string} name
   * @param {function(Event): undefined} handler
   */
  enrollNonPassiveEvent_(element, name, handler) {
    element.addEventListener(name, handler.bind(this), {passive: false});
  }
  ...
  render_() {
    ...
    this.renderChartRectContour_(context, width, graphHeight);
   ...
   */
  renderChartRectContour_(context, graphWidth, graphHeight) {
    context.strokeStyle = LineChart.GRID_COLOR;
