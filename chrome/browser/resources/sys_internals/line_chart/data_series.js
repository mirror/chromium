...
LineChart.DataSeries = class {
  ...
  backfillValuePoint_(valueIndex, dataIndex, boundaryTime) {
    ...
    if (values[valueIndex] == null && dataIndex > 0 &&
      ...
      values[valueIndex] = this.dataPointLinerInterpolation(
   ...
   * @return {number}
   */
  dataPointLinerInterpolation(pointA, pointB, position) {
    return this.constructor.linerInterpolation(
   ...
   * @return {number}
   */
  static linerInterpolation(x1, y1, x2, y2, x) {
    if (x1 == x2)
      return (y1 + y2) / 2;
