...
Polymer({
  ...
  /**
   * @return {string} Text representing the current position on the time slider.
   * @private
   */
  ...
  onSeekComplete_: function(e) {
    ...
    this.lastCurrentTimeChange_ = Date.now();
  },
  ...
  onVolumeChangeComplete_: function(e) {
    ...
    this.lastVolumeChange_ = Date.now();
  },
  ...
  onVolumeChangeByDragging_: function(e) {
    ...
    if (currentTime - this.lastVolumeChange_ < 300) {
      return;
    }
    this.lastVolumeChange_ = currentTime;
  ...
  shouldAcceptCurrentTimeUpdates_: function() {
    ...
    // likely to just be internal updates coming back form the device, and could
    // make the slider knob jump around.
    return Date.now() - this.lastCurrentTimeChange_ > 1000 && !this.isSeeking_;
  },
  ...
  shouldAcceptVolumeUpdates_: function() {
    ...
    // likely to just be internal updates coming back form the device, and could
    // make the slider knob jump around.
    return Date.now() - this.lastVolumeChange_ > 1000 &&
        !this.isVolumeChanging_;
  },
