...
class Checker(object):
  ...
  def _run_jar(self, jar, args):
    ...
    """
    java_executable = distutils.spawn.find_executable("java")
    if not java_executable:
      logging.error("No java executable found.")
      sys.exit(1)
    flag_file = tempfile.NamedTemporaryFile(mode='w', delete=False)
    try:
      # Write flags to a file to avoid exceeding the maximum
      # command-line length in Windows.
      with flag_file:
        for arg in args:
          flag_file.write(arg + '\n')
      shell_command = " ".join(
          [java_executable] + JAVA_FLAGS +
          [jar, '--flagfile=' + flag_file.name])
