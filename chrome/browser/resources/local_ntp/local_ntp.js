...
/** Fetches the given URL. If the fetch was successful and the response could
 * be parsed as JSON, calls the given callback with the response JSON object.
 *
 * @param {!string} logUrl
 * @param {?function(Object)} callback
 * @returns {boolean}
 */
var logDoodleImpression = function(logUrl, callback) {
  fetch(logUrl)
...
var showLogoOrDoodle = function(fromCache) {
  if (targetDoodle.metadata !== null) {
    ...
    if (logUrl) {
      logDoodleImpression(logUrl, function(json) {
        console.log('static impression! cta:' + isCta);
        if (json.ddllog && json.ddllog.interaction_log_url) {
          targetDoodle.staticInteractionLogUrl = new URL(
              json.ddllog.interaction_log_url, configData.googleBaseUrl);
        } else if (json.ddllog && json.ddllog.target_url_params && !isCta) {
          if (targetDoodle.metadata && targetDoodle.metadata.onClickUrl) {
            // TODO handle case when URL doesn't have params yet.
            targetDoodle.metadata.onClickUrl +=
                '&' + json.ddllog.target_url_params;
          }
        }
      });
    }
  ...
  if (metadata.animatedUrl) {
    ...
    logoDoodleButton.onclick = function(e) {
      ...
      if (targetDoodle.staticInteractionLogUrl) {
        console.log(
            'Pinging interaction: ' + targetDoodle.staticInteractionLogUrl);
      ...
      if (metadata.logUrl) {
        logoDoodleImage.onload = function() {
          logDoodleImpression(metadata.logUrl, function(json) {
            console.log('animated impression!');
            if (json.ddllog && json.ddllog.interaction_log_url) {
              targetDoodle.animatedInteractionLogUrl = new URL(
                  json.ddllog.interaction_log_url, configData.googleBaseUrl);
            } else if (json.ddllog && json.ddllog.target_url_params) {
              if (targetDoodle.metadata && targetDoodle.metadata.onClickUrl) {
                // TODO handle case when URL doesn't have params yet.
                targetDoodle.metadata.onClickUrl +=
                    '&' + json.ddllog.target_url_params;
      ...
      logoDoodleButton.onclick = function() {
        ...
        if (targetDoodle.animatedInteractionLogUrl) {
          console.log(
              'Pinging interaction: ' + targetDoodle.animatedInteractionLogUrl);
