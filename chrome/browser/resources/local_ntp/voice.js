...
function getChromeUILanguage() {
  // In Chrome, {@code navigator.language} is not guaranteed to be equal to
 ...
 * @const
 */
voice.speech.RECOGNITION_CONFIDENCE_THRESHOLD_ = 0.5;
 ...
 * @const
 */
voice.speech.QUERY_LENGTH_LIMIT_ = 120;
...
voice.speech.init = function(configData) {
  ...
  // Inline the SVG microphone icon for the overlay.
  var request = new XMLHttpRequest();
  request.onreadystatechange = function() {
    // If readyState == DONE.
    if (request.readyState == 4) {
      var svg = request.response.getElementsByTagName('svg')[0];
      $('voice-microphone').appendChild(svg);
    }
  };
  request.open('GET', '/' + configData.microphoneIconFilename, true);
 ...
 * @private
 */
voice.speech.onErrorReceived_ = function(error) {
  speech.resetIdleTimer_(speech.IDLE_TIMEOUT_MS_);
 ...
 */
voice.speech.onKeyDown = function(event) {
 ...
 * @private
 */
voice.speech.onIdleTimeout_ = function() {
  if (!!speech.finalResult_) {
    speech.submitFinalResult_();
    return;
  }
 ...
 * @private
 */
voice.speech.onVisibilityChange_ = function() {
 ...
 */
voice.speech.onOmniboxFocused = function() {
  if (!speech.isUiDefinitelyHidden_()) {
    speech.abort_();
  }
};
 ...
 * @private
 */
voice.speech.submitFinalResult_ = function() {
  window.clearTimeout(speech.idleTimer_);
  ...
  var queryUrl = `https://google.com/search?hl=${getChromeUILanguage()}` +
 ...
 * @return {int} The appropriate timeout duration for displaying the error.
 * @private
 */
 ...
 */
voice.speech.toggleStartStop = function() {
  if (speech.currentState_ == State_.READY) {
    speech.start_();
  } else {
    speech.abort_();
 ...
 * @private
 */
voice.speech.onClick_ = function(shouldSubmit, shouldRetry) {
...
voice.view.text.getErrorLink_ = function(error) {
  ...
  switch (error) {
    ...
    case RecognitionError.NO_SPEECH:
    case RecognitionError.AUDIO_CAPTURE:
      linkElement.href = text.supportLinkBase_ + getChromeUILanguage();
...
voice.view.text.getTextClassName_ = function() {
  ...
  var className = text.TEXT_AREA_CLASS_;
 ...
 * @const @private
 */
voice.view.RECEIVING_SPEECH_CLASS_ = 'outer voice-rs';
 ...
 * @private
 */
voice.view.onWindowClick_ = function(event) {
  if (!view.isVisible_) {
    return;
  }
  /* @const */ var targetId = event.target.id;
  /* @const */ var shouldRetry = (targetId == microphone.RED_BUTTON_ID ||
                                  targetId == text.ERROR_LINK_ID) &&
      view.isNoMatchShown_;
  /* @const */ var submitQuery =
