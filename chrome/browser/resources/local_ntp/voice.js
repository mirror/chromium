...
view.onWindowClick_ = function(event) {
  ...
  const linkClicked = event.target.id == text.ERROR_LINK_ID;
  const micIconClicked = event.target.id == microphone.RED_BUTTON_ID;

  const shouldRetry = (micIconClicked || linkClicked) && view.isNoMatchShown_;
  const submitQuery = micIconClicked && !view.isNoMatchShown_;
  const navigatingAway = linkClicked && !view.isNoMatchShown_;
