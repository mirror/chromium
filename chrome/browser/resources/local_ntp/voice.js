...
voice.speech.initWebkitSpeech_ = function() {
  ...
  speech.recognition_.onerror = speech.handleSpeechRecognitionError_;
...
voice.speech.handleRecognitionResult_ = function(responseEvent) {
  ...
  if (speech.interimResult_.length > speech.QUERY_LENGTH_LIMIT_) {
    if (!!speech.finalResult_) {
      ...
      speech.handleRecognitionError_(RecognitionError.NO_MATCH);
    }
  }
};
 ...
 * @private
 */
voice.speech.handleSpeechRecognitionError_ = function(error) {
  speech.handleRecognitionError_(speech.getRecognitionError_(error.error));
};
...
voice.speech.submitFinalResult_ = function() {
  ...
  /* @const */ var encodedQuery =
      encodeURIComponent(speech.finalResult_).replace(/%20/g, '+');
  /* @const */ var queryUrl = speech.googleBaseUrl_ +
