...
/**
 * Encode the text of a query so that it can be submitted to Search
 * as part of a URL.
 * @return {string} The encoded query text.
 * @private
 */
speech.encodeQueryText_ = function(queryText) {
  return encodeURIComponent(queryText).replace(/%20/g, '+');
};
...
speech.submitFinalResult_ = function() {
  ...
  const encodedQuery = speech.encodeQueryText_(speech.finalResult_);
