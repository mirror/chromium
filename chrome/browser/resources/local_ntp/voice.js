...
/**
 * The finalized query, obtained from speech result transcripts
 * {@code voice.speech.interimResult_} or {@code voice.speech.finalResult_}.
 * This is the actual query that is submitted to Search.
 *
 * @private {string}
 */
...
voice.speech.handleKeyDown_ = function(event) {
  ...
  if (!speech.isRecognizing_()) {
    ...
    if (event.keyCode == KEYCODE.ESC) {
      ...
      // TODO(oskopek): speechQuery is empty at ENTER press (fix or remove).
      speech.submitSpeechQuery_();
    }
  }
};
...
voice.speech.submitSpeechQuery_ = function() {
  ...
  /* @const */ var encodedQuery =
      encodeURIComponent(speech.speechQuery_).replace(/%20/g, '+');
  /* @const */ var queryUrl = `${speech.googleBaseQuery_}search` +
      // Actual query.
      `?q=${encodedQuery}` +
      // Localization of the search result page.
      `&hl=${getChromeUILanguage()}` +
