  ...
  for (int i = 0; i < num_cases; ++i) {
    ...
    if (matches.size() == cases[i].num_results) {
      ...
      for (const auto& match : matches) {
        EXPECT_EQ(cases[i].output[j].gurl, match.destination_url);
        EXPECT_EQ(cases[i].output[j].result_type, match.type);
        EXPECT_EQ(cases[i].output[j].fill_into_edit, match.fill_into_edit);
...
void SearchProviderTest::CheckMatches(const std::string& description,
  ...
  for (const auto& match : matches) {
    ...
    EXPECT_EQ(ASCIIToUTF16(expected_matches[i].contents), match.contents);
...
TEST_F(SearchProviderTest, DefaultProviderNoSuggestRelevanceInKeywordMode) {
  ...
  for (size_t i = 0; i < arraysize(cases); ++i) {
    ...
    // Ensure that the returned matches equal the expectations.
    for (const auto& match : matches) {
      EXPECT_EQ(ASCIIToUTF16(cases[i].matches[j]), match.contents);
      ++j;
    }
    // Ensure that no expected matches are missing.
...
TEST_F(SearchProviderTest, KeywordFetcherSuggestRelevance) {
  ...
  for (size_t i = 0; i < arraysize(cases); ++i) {
    ...
    // Ensure that the returned matches equal the expectations.
    for (const auto& match : matches) {
      EXPECT_EQ(ASCIIToUTF16(cases[i].matches[j].contents), match.contents);
  ...
  for (size_t i = 0; i < arraysize(cases); ++i) {
    ...
    for (const auto& match : matches) {
      EXPECT_EQ(ASCIIToUTF16(cases[i].output[j].match_contents),
                match.contents);
      EXPECT_EQ(cases[i].output[j].match_type, match.type);
      EXPECT_EQ(cases[i].output[j].allowed_to_be_default_match,
                match.allowed_to_be_default_match);
      ++j;
    }
...
TEST_F(SearchProviderTest, ParseEntitySuggestion) {
  ...
  for (size_t i = 0; i < arraysize(cases); ++i) {
    ...
    // Ensure that the returned matches equal the expectations.
    for (const auto& match : matches) {
      const Match& expected_match = cases[i].matches[j];
      SCOPED_TRACE(" and match index: " + base::SizeTToString(j));
      EXPECT_EQ(expected_match.contents, base::UTF16ToUTF8(match.contents));
      EXPECT_EQ(expected_match.description,
                base::UTF16ToUTF8(match.description));
      EXPECT_EQ(expected_match.query_params,
                match.search_terms_args->suggest_query_params);
      EXPECT_EQ(expected_match.fill_into_edit,
                base::UTF16ToUTF8(match.fill_into_edit));
      EXPECT_EQ(expected_match.type, match.type);
      ++j;
    }
    // Ensure that no expected matches are missing.
    for (; j < arraysize(cases[i].matches); ++j) {
   ...
   struct {
     ...
     for (size_t i = 0; i < arraysize(cases); ++i) {
       ...
       for (const auto& match : matches) {
         ...
         SCOPED_TRACE(" and match index: " + base::SizeTToString(j));
         EXPECT_EQ(expected_match.contents, base::UTF16ToUTF8(match.contents));
         EXPECT_EQ(expected_match.deletion_url,
