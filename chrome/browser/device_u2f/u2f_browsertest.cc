...
class TestU2fDiscoveryObserver : public MockU2fDiscoveryObserver {
 public:
  void DeviceAdded(U2fDiscovery* discovery, U2fDevice* u2f_device) override {
    const auto* device = static_cast<U2fBleDevice*>(u2f_device)->GetDevice();
    LOG(INFO) << "Device Added: " << device->GetNameForDisplay() << " ("
              << device->GetIdentifier() << "), Paired: " << std::boolalpha
              << device->IsPaired()
              << ", Connected: " << device->IsGattConnected();
    if (!device_added())
      device_ = u2f_device;
    MockU2fDiscoveryObserver::DeviceAdded(discovery, u2f_device);
  }
...
IN_PROC_BROWSER_TEST_F(U2fBrowserTest, U2fViaBluetooth) {
  ...
  device->SendPing(
      Bytes(1, 'a'), base::BindOnce([](U2fReturnCode code, const Bytes& data) {
        LOG(INFO) << (code == U2fReturnCode::SUCCESS ? "SUCCESS" : "FAILURE");
        LOG(INFO) << Format(data);
      }));
  device->SendPing(
      Bytes(10, 'b'), base::BindOnce([](U2fReturnCode code, const Bytes& data) {
        LOG(INFO) << (code == U2fReturnCode::SUCCESS ? "SUCCESS" : "FAILURE");
        LOG(INFO) << Format(data);
      }));
  device->SendPing(
      Bytes(20, 'c'), base::BindOnce([](U2fReturnCode code, const Bytes& data) {
        LOG(INFO) << (code == U2fReturnCode::SUCCESS ? "SUCCESS" : "FAILURE");
        LOG(INFO) << Format(data);
      }));
  device->SendPing(
...

  {
    ...
    device->Register(Bytes(32), Bytes(32),
                     base::Bind(
                         [](base::Closure quit, Bytes* key_handle,
                            U2fReturnCode code, const Bytes& data) {
  ...
  }
  {
    ...
    device->Sign(
        ...
        base::Bind(
            [](base::Closure quit, U2fReturnCode code, const Bytes& data) {
              LOG(INFO) << "response code = " << int(code);
              if (code != U2fReturnCode::SUCCESS)
                return;
