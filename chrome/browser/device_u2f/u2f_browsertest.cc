...
class TestBlePairingOrchestrator
 ...
 public:
  ...
  void PairWithRetries() {
    device_->Pair(this,
                  base::BindOnce(&TestBlePairingOrchestrator::OnSuccess,
                                 base::Unretained(this)),
                  base::BindOnce(&TestBlePairingOrchestrator::OnFailure,
                                 base::Unretained(this)));
  }
 ...
 protected:
  ...
  void RequestPasskey(BluetoothDevice* device) override {
    ...
    LOG(INFO) << "RequestPasskey, supplying passkey: 948137";
    DCHECK(device->ExpectingPasskey());
    device->SetPasskey(948137);
  }
...
IN_PROC_BROWSER_TEST_F(U2fBrowserTest, U2fViaBluetooth) {
...

  {
    ...
    device->Register(Bytes(32), Bytes(32),
                     base::BindOnce(
                         [](base::OnceClosure quit, Bytes* key_handle,
...

  {
    ...
    device->Sign(
        ...
        base::BindOnce(
            [](base::OnceClosure quit, U2fReturnCode code, const Bytes& data) {
              LOG(INFO) << "response code = " << int(code);
              if (code != U2fReturnCode::SUCCESS)
                return;
