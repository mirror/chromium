...
void IncognitoWindowTracker::OnBrowsingDataCleared() {
  const auto severity = feature_engagement::GetAppMenuButtonSeverity();
...
void IncognitoWindowTracker::ShowPromo() {
  ...
  set_feature_promo_bubble(
      feature_engagement::CreateIncognitoFeaturePromoBubble());
  feature_promo_bubble()->ShowPromoBubble();
}
