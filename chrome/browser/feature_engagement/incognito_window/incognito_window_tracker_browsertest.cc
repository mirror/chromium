...
IN_PROC_BROWSER_TEST_F(IncognitoWindowTrackerBrowserTest, TestShowPromo) {
  ...
  // Set up ClearBrowsingDataHandler.
  auto handler_owner =
      base::MakeUnique<settings::ClearBrowsingDataHandler>(web_ui);
  settings::ClearBrowsingDataHandler* handler = handler_owner.get();
  web_ui->AddMessageHandler(std::move(handler_owner));
  handler->AllowJavascriptForTesting();
  handler->HandleClearBrowsingDataForTest();

  auto* widget = BrowserView::GetBrowserViewForBrowser(browser())
                     ->toolbar()
                     ->app_menu_button()
                     ->incognito_window_promo()
                     ->GetWidget();
