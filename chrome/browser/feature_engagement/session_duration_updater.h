...
// much active time has passed, and then decide whether to show their respective
// promos accordingly.
//
// When an active session is closed, DesktopSessionDurationTracker calls
// OnSessionEnded and the observed time is incremented and persisted at the end
// of each session. Then, OnSessionEnded is called on all of Features observing
// SessionDurationUpdater. If the feature has its time limit exceeded, it should
// remove itself as an observer of SessionDurationUpdater. If
// SessionDurationUpdater has no observers, it means that the time limits of all
// the observing features has been exceeded, so the SessionDurationUpdater
