...
class SessionDurationUpdater
 ...
 public:
  ...
  // session plus the |observed_session_time_perf_| perf value. The resulting
  // value should be cumulative session time across all Chrome restarts until
  // the |observed_session_time_perf_| stops recording.
 ...
 private:
  ...
  // The profile perf key of kObservedSessionTime that tracks the observed
  // session time for an In-Product Help feature.
  const char* observed_session_time_perf_key_;
