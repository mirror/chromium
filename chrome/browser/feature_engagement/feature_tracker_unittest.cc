...
const int kTestTime = 100;
const int kTestTimeSufficent = 110;
const int kTestTimeInsufficent = 90;
const char kTestProfileName[] = "test-profile";

class TestFeatureTracker : public FeatureTracker {
 public:
  TestFeatureTracker(Profile* profile)
      : FeatureTracker(
            profile,
  ...
  int GetSessionTimeRequiredToShowInMinutes() override { return kTestTime; }
...
class MockTestFeatureTracker : public TestFeatureTracker {
 public:
  MockTestFeatureTracker(Profile* profile) : TestFeatureTracker(profile) {}
...
// Tests to verify FeatureTracker functional expectations:

// If OnSessionEnded paremter is greater than HasEnoughSessionTimeElapsed
...
TEST_F(FeatureTrackerTest, TestExpectOnSessionTimeMet) {
  ...
  mock_feature_tracker_.get()->OnSessionEnded(
      base::TimeDelta::FromMinutes(kTestTimeSufficent));
}

// If OnSessionEnded paremter is less than than HasEnoughSessionTimeElapsed
// then OnSessionTimeMet should not be called.
TEST_F(FeatureTrackerTest, TestDontExpectOnSessionTimeMet) {
  mock_feature_tracker_.get()->OnSessionEnded(
      base::TimeDelta::FromMinutes(kTestTimeInsufficent));
  mock_feature_tracker_.get()->RemoveSessionDurationObserver();
}

// AddSessionDurationObserver is called on initialization. The FeatureTracker
// should be observing sources until the RemoveSessionDurationObserver is
// called.
TEST_F(FeatureTrackerTest, TestAddAndRemoveObservers) {
  EXPECT_TRUE(mock_feature_tracker_->IsObserving());
