...
TEST_F(SessionDurationUpdaterTest, TimeAdded) {
  ...
  EXPECT_EQ(0, test_observer_->GetPrefs()->GetInt64(
                   prefs::kNewTabInProductHelpObservedSessionTime));

  // Tests 50 minutes passing with an observer added.
  test_observer_->GetSessionDurationUpdater()->OnSessionEndedForPerf(
      base::TimeDelta::FromSeconds(50),
      prefs::kNewTabInProductHelpObservedSessionTime);

  EXPECT_EQ(50, test_observer_->GetPrefs()->GetInt64(
                    prefs::kNewTabInProductHelpObservedSessionTime));
}
...
TEST_F(SessionDurationUpdaterTest, AddingAndRemovingObservers) {
  ...
  EXPECT_EQ(100, test_observer_->GetPrefs()->GetInt64(
                     prefs::kNewTabInProductHelpObservedSessionTime));
}
