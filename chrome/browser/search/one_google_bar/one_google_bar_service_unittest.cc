...
TEST_F(OneGoogleBarServiceTest, KeepsCacheOnNonFatalError) {
  // Load some data.
  service()->Refresh();
  OneGoogleBarData data;
  data.bar_html = "<div></div>";
  ...
  // Request a refresh and respond with a nonfatal error.
  service()->Refresh();
  // Note: No OnOneGoogleBarDataChanged, since the cached data remains.
  EXPECT_CALL(observer, OnOneGoogleBarFetchFailed());
  fetcher()->RespondToAllCallbacks(OneGoogleBarFetcher::Status::NONFATAL_ERROR,
                                   base::nullopt);
