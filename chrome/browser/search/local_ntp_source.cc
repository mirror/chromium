...
class LocalNtpSource::DesktopLogoObserver
 ...
 public:
  ...
  void GetFreshLogo(search_provider_logos::LogoService* service,
                    int version,
                    const content::URLDataSource::GotDataCallback& callback) {
    if (observing_ && (version == version_)) {
...
void LocalNtpSource::StartDataRequest(
  ...
  if (stripped_path == kDoodleScriptFilename) {
    ...
    int version;
    GURL url = GURL("chrome-search://local-ntp/").Resolve(path);
...
std::string LocalNtpSource::GetContentSecurityPolicyChildSrc() const {
  ...
  if (one_google_bar_service_ || logo_service_) {
    // Allow embedding of:
    //   * the most visited iframe
    //   * the One Google Bar's account switcher and notifications dropdown
    //   * scripts for interactive doodles.
