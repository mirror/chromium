...
const char kVoicePermissionErrorMessage[] =
    "Voice search has been turned off. <a "
    "href=\"https://support.google.com/chrome/?p=ui_voice_search"
    "&hl=LANGUAGE\" target=\"_blank\">Details</a>";
...
std::unique_ptr<base::DictionaryValue> GetTranslatedStrings(bool is_google) {
  ...
  if (is_google) {
    ...
    // TODO(oskopek): Remove the permission error message once permissions are
    // automatically enabled for the local NTP.
    translated_strings.get()->SetString("permissionError",
                                        kVoicePermissionErrorMessage);
