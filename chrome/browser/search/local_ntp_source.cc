...
using search_provider_logos::LogoCallbackType;
...
std::string ConvertLogoImageToBase64(const EncodedLogo& logo) {
  ...
  result += logo.metadata.mime_type;
  result += ";base64,";
  result += base64;
  return result;
}
...
class LocalNtpSource::DesktopLogoObserver {
 public:
  ...
  void GetFreshLogo(LogoService* service,
                    ...
                    const content::URLDataSource::GotDataCallback& callback) {
    if (requested_version <= version_finished_) {
      StartGetLogo(service, callback, /*from_cache=*/true);
    } else {
      StartGetLogo(service, callback, /*from_cache=*/false);
    }
  }
 ...
 private:
  ...
  void OnRequestCompleted(LogoCallbackType type,
                          const base::Optional<EncodedLogo>& logo) {
    DCHECK(observing());
  ...
  void StartGetLogo(LogoService* service,
    ...
    if (from_cache) {
      callbacks.on_cached_encoded_logo =
          base::BindOnce(&DesktopLogoObserver::OnCachedLogoAvailable,
                         weak_ptr_factory_.GetWeakPtr(), callback);
      callbacks.on_fresh_encoded_logo =
