...
// WARNING: Due to the odd threading model of URLDataSource, some methods of
// this class are called on the UI thread, others on the IO thread. All data
// members live on the UI thread, so make sure not to access them from the IO
// thread! To prevent accidental access, all methods that get called on the IO
// thread are implemented as non-member functions.
