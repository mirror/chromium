...
class DownloadMetadataManagerTestBase : public ::testing::Test {
 protected:
  ...
  void AddDownloadItems() {
    ...
    ON_CALL(*delegate, GetBrowserContext()).WillByDefault(Return(&profile_));
    test_item_->SetDelegate(std::move(delegate));
    ...
    ON_CALL(*other_item_, GetEndTime())
        .WillByDefault(Return(base::Time::FromJsTime(kTestDownloadEndTimeMs)));
    delegate = base::MakeUnique<
        ::testing::NiceMock<content::MockDownloadItemDelegate>>();
    ON_CALL(*delegate, GetBrowserContext()).WillByDefault(Return(&profile_));
    other_item_->SetDelegate(std::move(delegate));
    ...
    delegate = base::MakeUnique<
        ::testing::NiceMock<content::MockDownloadItemDelegate>>();
    ON_CALL(*delegate, GetBrowserContext()).WillByDefault(Return(&profile_));
    zero_item_->SetDelegate(std::move(delegate));
