...
class DownloadProtectionService::CheckClientDownloadRequest
 ...
 public:
  ...
  void Start() {
    ...
    DCHECK_CURRENTLY_ON(BrowserThread::UI);
    LOG(ERROR) << "starting analysis, item_ better be non-null: " << item_;
    ...
    BrowserThread::PostTask(
        ...
        base::BindOnce(&CheckClientDownloadRequest::CheckUrlAgainstWhitelists,
                       this));
    LOG(ERROR) << "posted task to io thread";
  }

  void AnalyzeFile() {
    LOG(ERROR) << "analyzing file 1";
    // TODO: instead of this, use CancelableTaskTracker
    if (item_ == nullptr) {
      PostFinishTask(UNKNOWN, REASON_EMPTY_URL_CHAIN);
      return;
    }
  ...
  void OnDownloadDestroyed(content::DownloadItem* download) override {
    LOG(ERROR) << "download is being destroyed";
    Cancel();
    DCHECK(item_ == NULL);
  }
 ...
 private:
  ...
  void StartExtractFileFeatures() {
    LOG(ERROR) << "start extract file features";
    DCHECK_CURRENTLY_ON(BrowserThread::UI);
  ...
  void CheckUrlAgainstWhitelists() {
    DCHECK_CURRENTLY_ON(BrowserThread::IO);

    LOG(ERROR) << "checking url against whitelist";
    ...
    LOG(ERROR) << "gonna go ahead and analyze file";
