...
DownloadCheckEnums::REASON_MAX; if (!IsSupportedDownload( *item_,
item_->GetTargetFilePath(), &reason, &type_)) { switch (reason) { case
DownloadCheckEnums::REASON_EMPTY_URL_CHAIN: case
DownloadCheckEnums::REASON_INVALID_URL: case
DownloadCheckEnums::REASON_LOCAL_FILE: case
DownloadCheckEnums::REASON_REMOTE_FILE:
...
DownloadCheckEnums::REASON_REQUEST_CANCELED);
  ...
  void OnURLFetchComplete(const net::URLFetcher* source) override {
    ...
    DownloadCheckEnums::DownloadCheckResult result =
DownloadCheckEnums::UNKNOWN; std::string token; if
(source->GetStatus().is_success() && net::HTTP_OK == source->GetResponseCode())
{ ClientDownloadResponse response; std::string data; bool got_data =
source->GetResponseAsString(&data); DCHECK(got_data); if
(!response.ParseFromString(data)) { reason =
DownloadCheckEnums::REASON_INVALID_RESPONSE_PROTO; result =
DownloadCheckEnums::UNKNOWN; } else if (type_ ==
...
DownloadCheckEnums::REASON_LOCAL_FILE; return false;
    }
