...
TEST_F(DownloadProtectionServiceTest,
...

  {
    ...
    EXPECT_CALL(
        *static_cast<content::MockDownloadItemDelegate*>(item.GetDelegate()),
        GetBrowserContext())
...
TEST_F(DownloadProtectionServiceTest,
  ...
  PrepareBasicDownloadItem(
      ...
      FILE_PATH_LITERAL("a.tmp"),                                   // tmp_path
      FILE_PATH_LITERAL("a.exe"));  // final_path
  ON_CALL(*static_cast<content::MockDownloadItemDelegate*>(item.GetDelegate()),
          GetWebContents())
      .WillByDefault(Return(web_contents.get()));
