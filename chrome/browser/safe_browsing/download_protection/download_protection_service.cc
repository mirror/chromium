...
DownloadProtectionService::IdentifyReferrerChain(
  ...
  int referrer_chain_length = referrer_chain->size();
  // If referrer chain is incomplete, we collects the recent navigation events
  // from SBER users if they're not in incognito.
  ...
  return base::MakeUnique<ReferrerChainData>(std::move(referrer_chain),
