  ...
  void SimulateAction(ChromePasswordProtectionService::WarningUIType ui_type,
    ...
    for (auto& observer : GetService()->observer_list_) {
      if (ui_type == observer.GetObserverType()) {
        observer.IncokeActionForTesting(action);
      }
    }
  }
...
// TODO(jialiul): Enable the following tests when password reuse modal dialog
// is available on Cocoa.
#if !defined(OS_MACOSX) || BUILDFLAG(MAC_VIEWS_BROWSER)
#define MAYBE_SuccessfullyChangePassword SuccessfullyChangePassword
#else
#define MAYBE_SuccessfullyChangePassword DISABLED_SuccessfullyChangePassword
#endif
IN_PROC_BROWSER_TEST_F(ChromePasswordProtectionServiceBrowserTest,
                       MAYBE_SuccessfullyChangePassword) {
  ...
  ASSERT_EQ(GURL("https://myaccount.google.com/"), browser()
                                                       ->tab_strip_model()
  ...
  GetSecurityInfo(web_contents, &security_info);
  ASSERT_EQ(security_state::DANGEROUS, security_info.security_level);
  // TODO(jialiul): Check malicious content status after crbug.com/762738 is
  // closed.
}

#if !defined(OS_MACOSX) || BUILDFLAG(MAC_VIEWS_BROWSER)
#define MAYBE_MarkSiteAsLegitimate MarkSiteAsLegitimate
#else
#define MAYBE_MarkSiteAsLegitimate DISABLED_MarkSiteAsLegitimate
#endif
IN_PROC_BROWSER_TEST_F(ChromePasswordProtectionServiceBrowserTest,
                       MAYBE_MarkSiteAsLegitimate) {
  ChromePasswordProtectionService* service = GetService();
  Profile* profile = browser()->profile();
  content::WebContents* web_contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  ...
  service->OnWarningDone(web_contents,
