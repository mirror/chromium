...
TEST_F(ChromePasswordProtectionServiceTest,
       VerifyPasswordReuseLookupUserEventNotRecordedUrlNotCommitted) {
  // Feature enabled, no URL committed.
  EnableGaiaPasswordReuseReporting();
  unsigned long t = 0;
...
TEST_F(ChromePasswordProtectionServiceTest,
       VerifyPasswordReuseLookupUserEventNotRecordedFeatureNotEnabled) {
  NavigateAndCommit(GURL("https://www.example.com/"));
  unsigned long t = 0;
  for (const auto& it : kTestCasesWithoutVerdict) {
    VLOG(1) << __FUNCTION__ << ": case: " << t;
