...
#include "components/encrypted_reports/encrypted_report.pb.h"
#include "components/encrypted_reports/report_encrypter.h"
...
std::string GetReportContents(net::URLRequest* request,
  ...
  encrypted_reports::EncryptedReport encrypted_report;
  EXPECT_TRUE(encrypted_report.ParseFromString(serialized_report));
  EXPECT_EQ(kServerPublicKeyTestVersion,
            encrypted_report.server_public_key_version());
  EXPECT_EQ(
      encrypted_reports::EncryptedReport::AEAD_ECDH_AES_128_CTR_HMAC_SHA256,
      encrypted_report.algorithm());
  std::string decrypted_report;
  EXPECT_TRUE(encrypted_reports::DecryptErrorReport(
      server_private_key, kHkdfLabel, encrypted_report, &decrypted_report));
