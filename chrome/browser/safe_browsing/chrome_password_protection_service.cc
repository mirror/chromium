...
void ChromePasswordProtectionService::ShowModalWarning(
  ...
  if (unhandled_password_reuses_.find(Origin(trigger_url)) !=
      unhandled_password_reuses_.end()) {
    return;
  }
...
void ChromePasswordProtectionService::HandleUserActionOnModalWarning(
  ...
  // Remove remaining request objects associated with |web_content|.
