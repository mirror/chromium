...
int64_t GetNavigationIDFromPrefsByOrigin(PrefService* prefs,
  ...
  if (!unhandled_sync_password_reuses ||
      !unhandled_sync_password_reuses->GetString(origin.Serialize(),
                                                 &navigation_id_str))
    return 0;
  int64_t navigation_id_int64;
  return base::StringToInt64(navigation_id_str, &navigation_id_int64)
             ? navigation_id_int64
             : 0;
}
...
void ChromePasswordProtectionService::ShowModalWarning(
  ...
  update->SetString(
      Origin(web_contents->GetLastCommittedURL()).Serialize(),
...
void ChromePasswordProtectionService::HandleUserActionOnPageInfo(
  ...
  if (action == PasswordProtectionService::MARK_AS_LEGITIMATE) {
    ...
    update->RemoveWithoutPathExpansion(Origin(url).Serialize(), NULL);
    for (auto& observer : observer_list_)
