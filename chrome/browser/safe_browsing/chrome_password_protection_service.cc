...
void ChromePasswordProtectionService::MaybeLogPasswordReuseDetectedEvent(
  ...
  auto specifics = GetUserEventSpecifics(web_contents);
  VLOG(1) << __FUNCTION__ << ": specifics: " << specifics.get();
  if (!specifics)
    return;
...
ChromePasswordProtectionService::GetUserEventSpecifics(
  ...
  VLOG(1) << __FUNCTION__ << ": navigation: " << navigation;
  if (!navigation)
    return nullptr;
...
void ChromePasswordProtectionService::LogPasswordReuseLookupResult(
  ...
  syncer::UserEventService* user_event_service =
      browser_sync::UserEventServiceFactory::GetForProfile(profile_);
  VLOG(1) << __FUNCTION__ << ": user_event_service: " << user_event_service;
  if (!user_event_service)
    return;
  ...
  user_event_service->RecordUserEvent(std::move(specifics));
  VLOG(1) << __FUNCTION__ << ": result: " << reuse_lookup->lookup_result();
}
...
void ChromePasswordProtectionService::LogPasswordReuseLookupResultWithVerdict(
  ...
  VLOG(1) << __FUNCTION__ << ": verdict: " << reuse_lookup->verdict();
  reuse_lookup->set_verdict_token(verdict_token);
  VLOG(1) << __FUNCTION__
          << ": verdict_token: " << reuse_lookup->verdict_token();
  user_event_service->RecordUserEvent(std::move(specifics));
}

void ChromePasswordProtectionService::MaybeLogPasswordReuseLookup(
    content::WebContents* web_contents,
    PasswordProtectionService::RequestOutcome outcome,
    const LoginReputationClientResponse* response) {
  VLOG(1) << __FUNCTION__ << ": outcome: " << outcome;
  VLOG(1) << __FUNCTION__ << ": response: " << response;
  if (!base::FeatureList::IsEnabled(safe_browsing::kSyncPasswordReuseEvent))
    return;

  if (response) {
    VLOG(1) << "response: verdict_type: " << response->verdict_type();
    VLOG(1) << "response: verdict_token: " << response->verdict_token();
  }
  switch (outcome) {
