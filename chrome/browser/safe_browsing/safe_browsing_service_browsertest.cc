...
const char kMalwareJsRequestPage[] =
    "/safe_browsing/malware_js_request.html";
...
struct JsRequestTestParam {
  JsRequestTestParam(ContextType in_context_type, JsRequestType in_request_type) : context_type(in_context_type), request_type(in_request_type) {}
...
// appended.
GURL AddJsRequestParam(const GURL& base_url,
                       const JsRequestTestParam& param) {
  GURL::Replacements add_query;
  std::string query =
      "contextType=" + ContextTypeToString(param.context_type) + "&requestType=" + JsRequestTypeToString(param.request_type);
...
IN_PROC_BROWSER_TEST_P(SafeBrowsingServiceWebSocketInterstitialTest,
  ...
  ui_test_utils::NavigateToURL(
      browser(), AddTypeParams(base_url, GetParam(), kWebSocketRequestType));
...
IN_PROC_BROWSER_TEST_P(V4SafeBrowsingServiceJsRequestInterstitialTest,
  ...
  EXPECT_CALL(observer_, OnSafeBrowsingHit(IsUnsafeResourceFor(js_request_url)));
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(JsRequestTestParam(WINDOW, WEBSOCKET),
                      JsRequestTestParam(WORKER, WEBSOCKET),
                      JsRequestTestParam(WINDOW, XHR),
                      JsRequestTestParam(WORKER, XHR)));
...
IN_PROC_BROWSER_TEST_P(V4SafeBrowsingServiceJsRequestNoInterstitialTest,
  ...
  GURL base_url = embedded_test_server()->GetURL(kMalwareJsRequestPage);
  auto param = GetParam();
  MarkUrlForMalwareUnexpired(ConstructJsRequestURL(base_url, param.request_type));
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(JsRequestTestParam(SHARED_WORKER, WEBSOCKET),
                      JsRequestTestParam(SERVICE_WORKER, WEBSOCKET),
                      JsRequestTestParam(SHARED_WORKER, XHR),
                      JsRequestTestParam(SERVICE_WORKER, XHR)));
