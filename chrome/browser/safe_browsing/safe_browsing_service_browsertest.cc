...
class SafeBrowsingServiceTest : public InProcessBrowserTest {
 public:
  ...
  gaia_password_data_map() {
    SafeBrowsingService* sb_service =
        g_browser_process->safe_browsing_service();
    return sb_service
               ? sb_service->gaia_password_data_map_
               : std::map<AccountId, password_manager::SyncPasswordData>();
  }
  std::pair<AccountId, password_manager::SyncPasswordData>*
  pending_gaia_password_data() {
    SafeBrowsingService* sb_service =
        g_browser_process->safe_browsing_service();
    return sb_service ? sb_service->pending_gaia_password_data_.get() : nullptr;
  }
#endif
...
#if defined(OS_CHROMEOS)
IN_PROC_BROWSER_TEST_F(SafeBrowsingServiceTest, AddAndRemoveGaiaPasswordData) {
  SafeBrowsingService* sb_service = g_browser_process->safe_browsing_service();
  EXPECT_TRUE(!pending_gaia_password_data());
  EXPECT_EQ(0u, gaia_password_data_map().size());
  ...
  EXPECT_TRUE(pending_gaia_password_data() &&
              pending_gaia_password_data()->first == account_id);

  sb_service->AddGaiaPasswordData(account_id);
  EXPECT_TRUE(!pending_gaia_password_data());
  EXPECT_EQ(1u, gaia_password_data_map().size());

  sb_service->SaveGaiaPasswordHash(browser()->profile());
