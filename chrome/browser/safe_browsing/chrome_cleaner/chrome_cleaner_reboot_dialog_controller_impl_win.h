...
class ChromeCleanerRebootDialogControllerImpl
 ...
 public:
  ...
  void SetPromptDelegateForTests(PromptDelegate* delegate);

 protected:
  ~ChromeCleanerRebootDialogControllerImpl() override;

  // Shows the reboot prompt if the Settings page is currently not the active
  // tab. If the reboot prompt experiment is on, this will show a new modal
  // dialgo. Otherwise, this will reopen the Settings page on a background tab.
