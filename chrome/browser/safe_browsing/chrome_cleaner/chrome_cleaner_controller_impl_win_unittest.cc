...
TEST_P(ChromeCleanerControllerTest, WithMockCleanerProcess) {
  ...
  if (!scanner_results_on_infected.files_to_delete().empty() &&
      !scanner_results_on_cleaning.files_to_delete().empty()) {
  ...
  if (!scanner_results_on_infected.registry_keys().empty() &&
      !scanner_results_on_cleaning.registry_keys().empty()) {
    EXPECT_THAT(
