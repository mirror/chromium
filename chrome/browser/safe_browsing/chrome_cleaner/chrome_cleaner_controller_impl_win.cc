...
ChromeCleanerControllerImpl::GetNextSwReporterInvocationType(
  ...
  DCHECK(!cached_reporter_invocations_);

  base::AutoLock autolock(lock_);
  cached_reporter_invocations_ =
      base::MakeUnique<SwReporterInvocationSequence>(*invocations);
  ...
  if (!user_initiated_reporter_run_) {
    ...
    if (cached_reporter_invocations_) {
      content::BrowserThread::PostTask(
          ...
          base::BindOnce(
              &safe_browsing::MaybeStartSwReporter,
              logs_enabled_
                  ? SwReporterInvocationType::kUserInitiatedWithLogsAllowed
                  : SwReporterInvocationType::kUserInitiatedWithLogsDisallowed,
              // The invocations will be modified by the |ReporterRunner|.
              // Give it a copy to keep the cached invocations pristine.
              base::Passed(new SwReporterInvocationSequence(
                  *cached_reporter_invocations_))));
    } else {
      OnReporterSequenceStarted();

      on_demand_sw_reporter_fetcher_ =
          base::MakeUnique<component_updater::SwReporterOnDemandFetcher>(
              g_browser_process->component_updater(),
              base::BindOnce(
                  &ChromeCleanerController::OnReporterSequenceDone,
