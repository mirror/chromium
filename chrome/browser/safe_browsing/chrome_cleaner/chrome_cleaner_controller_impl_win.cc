...
bool ChromeCleanerControllerImpl::IsPoweredByPartner() const {
  LOG(ERROR) << "powered_by_partner_: " << powered_by_partner_;
...
void ChromeCleanerControllerImpl::NotifyObserver(Observer* observer) const {
  ...
  switch (state_) {
    ...
    case State::kInfected:
      observer->OnInfected(IsPoweredByPartner(), scanner_results_);
      break;
    case State::kCleaning:
      observer->OnCleaning(IsPoweredByPartner(), scanner_results_);
      break;
