...
class ReporterRunner {
 public:
  ...
  static void MaybeStartInvocations(
    ...
    if (instance_ && IsUserInitiated(invocation_type)) {
      ...
      // weak pointers for the ReporterRunner instance will turn all posted
      // tasks into no-ops. This will also notify any pending action depending
      // on reporter results that it was interrupted. This will not stop a
      // reporter process that is still running, but once it finishes, the
      // corresponding callback will be executed.
      //
      // TODO(crbug.com/776538): stop the reporter process if currently running.
      delete instance_;

    } else if (!time_info.ShouldRun()) {
    ...
    instance_ =
        new ReporterRunner(invocation_type, std::move(invocations),
                           std::move(time_info), scoped_runner.Release());
 ...
 private:
  ...
  void AppendInvocationSpecificSwitches(SwReporterInvocation* next_invocation) {
    ...
    PrefService* local_state = g_browser_process->local_state();
    if (local_state && ShouldSendReporterLogs(next_invocation->suffix())) {
