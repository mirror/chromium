...
MockChromeCleanerProcess::Options::Options(const Options& other)
    : files_to_delete_(other.files_to_delete_),
      registry_entries_(other.registry_entries_),
...
void MockChromeCleanerProcess::Options::SetDoFindUws(bool do_find_uws) {
  ...
  registry_entries_.insert(L"HKCU:32\\Software\\Some\\Unwanted Software");
  registry_entries_.insert(L"HKCU:32\\Software\\Another\\Unwanted Software");
}
...
void MockChromeCleanerProcess::SendScanResults(
  ...
  (*chrome_prompt_ptr_)
      ->PromptUser(
          ...
          std::vector<base::string16>(options_.registry_entries().begin(),
                                      options_.registry_entries().end()),
