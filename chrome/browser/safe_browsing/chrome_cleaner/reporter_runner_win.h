...
enum class SwReporterInvocationResult {
  ...
  kTimedOut,
  // The reporter either could not be started or ended with a failure.
  kFailed,
...
class SwReporterInvocationsSequence {
 public:
  ...
  SwReporterInvocationsSequence(
      const base::Version& version = base::Version(),
      const Container& container = Container(),
      OnReporterSequenceDone on_sequence_done = OnReporterSequenceDone());
  SwReporterInvocationsSequence(SwReporterInvocationsSequence&& queue);
  virtual ~SwReporterInvocationsSequence();

  void operator=(SwReporterInvocationsSequence&& queue);

  //
  void ImportFrom(SwReporterInvocationsSequence* queue);
