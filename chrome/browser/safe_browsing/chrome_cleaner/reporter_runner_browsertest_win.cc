...
class ReporterRunnerTest : public InProcessBrowserTest,
 ...
 public:
  ...
  void OnSequenceDone() {
    ...
    DCHECK(!sequence_done_);
    sequence_done_ = true;
  }

  void ResetSequenceDone() {
    base::AutoLock lock(lock_);
  ...
  bool SequenceDone() {
    base::AutoLock lock(lock_);
    return sequence_done_;
  }

  // Wait until all invocations for a sequence finished.
  void WaitUntilSequenceDone() {
    while (!SequenceDone())
  ...
  void RunReporter(int exit_code_to_report,
    ...
    ResetSequenceDone();
    RunSwReporters(SwReporterQueue({invocation}), base::Version("1.2.3"),
                   base::BindOnce(&ReporterRunnerTest::OnSequenceDone,
                                  base::Unretained(this)));
    WaitUntilSequenceDone();
  }
  ...
  // Controls when a sequence of invocations finished running.
  bool sequence_done_ = false;
  base::Lock lock_;
