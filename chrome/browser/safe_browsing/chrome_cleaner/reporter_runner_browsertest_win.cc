...
class ReporterRunnerTest
    : public InProcessBrowserTest,
      ...
      public ::testing::WithParamInterface<
          testing::tuple<SwReporterInvocationType, const char*, const char*>> {
 public:
  ReporterRunnerTest() {
  ...
  void RunReporter(int exit_code_to_report,
    ...
    SwReporterInvocationsSequence invocations(base::Version("1.2.3"));
    invocations.container().push_back(invocation);
    RunSwReporters(invocation_type_, invocations, OnReporterDoneCallback());
  }
...
IN_PROC_BROWSER_TEST_P(ReporterRunnerTest, MultipleLaunches) {
  ...
  // queue is running.
  {
    ...
    invocations.container().push_back(
        SwReporterInvocation(base::CommandLine(path3)));
...
IN_PROC_BROWSER_TEST_P(ReporterRunnerTest, ReporterLogging_MultipleLaunches) {
  ...
  for (const auto& path : {path1, path2}) {
    auto invocation = SwReporterInvocation(base::CommandLine(path));
    invocations.container().push_back(invocation);
  }
  RunReporterQueue(chrome_cleaner::kSwReporterNothingFound, invocations);
