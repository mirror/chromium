...
IN_PROC_BROWSER_TEST_P(ReporterRunnerTest, RanRecently) {
  ...
  if (IsUserInitiated(invocation_type_)) {
    ...
    ExpectReporterLaunches(/*expected_launch_count=*/0,
                           /*expect_prompt=*/false);
  }
}
...
IN_PROC_BROWSER_TEST_P(ReporterRunnerTest, MultipleLaunches) {
  ...
  // just run 2 times, not 4.
  {
    ...
    if (IsUserInitiated(invocation_type_)) {
      ...
      RunReporterQueue(chrome_cleaner::kSwReporterNothingFound,
                       SwReporterInvocationResult::kNotScheduled, invocations);
...
IN_PROC_BROWSER_TEST_P(ReporterRunnerTest,
  ...
  SwReporterInvocationSequence first_sequence = CreateInvocationSequence(
      ...
      invocations1);
  RunSwReporters(invocation_type_, std::move(first_sequence));
...
IN_PROC_BROWSER_TEST_P(ReporterRunnerTest, UserInitiatedRunsNotInterrupted) {
  ...
  SwReporterInvocationSequence first_sequence = CreateInvocationSequence(
      base::BindOnce(
          ...
          &first_sequence_done, &second_sequence_done),
      invocations1);
  RunSwReporters(SwReporterInvocationType::kUserInitiatedWithLogsDisallowed,
