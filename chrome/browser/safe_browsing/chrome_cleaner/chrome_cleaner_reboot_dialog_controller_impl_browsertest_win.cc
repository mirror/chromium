...
//  - bool settings_current_active_: if the Settings page is opened as the
//        current active tab.
class ChromeCleanerRebootPromptTest
    : public InProcessBrowserTest,
      public ::testing::WithParamInterface<std::tuple<bool, bool>> {
 public:
  ChromeCleanerRebootPromptTest() {
    std::tie(reboot_dialog_enabled_, settings_current_active_) = GetParam();
  }
  ...
  void SetUpInProcessBrowserTestFixture() override {
    ...
    dialog_controller_->SetPromptDelegateForTests(&mock_prompt_delegate_);
  }
 ...
 protected:
  ...
  bool settings_current_active_ = false;
};

IN_PROC_BROWSER_TEST_P(ChromeCleanerRebootPromptTest,
                       OnInfectedBrowserAvailable) {
  if (!settings_current_active_) {
    if (reboot_dialog_enabled_) {
