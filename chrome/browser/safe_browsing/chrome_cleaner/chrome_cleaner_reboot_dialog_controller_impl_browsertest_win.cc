...
IN_PROC_BROWSER_TEST_P(ChromeCleanerRebootFlowTest,
  ...
  // StrictMock ensures no method for mock_prompt_delegate_ be called, and so
...
IN_PROC_BROWSER_TEST_P(ChromeCleanerRebootFlowTest,
  ...
  auto dialog_controller = ChromeCleanerRebootDialogControllerImpl::Create(
      &mock_cleaner_controller_, std::move(mock_prompt_delegate));

  // Close the dialog if it's shown, otherwise the object never gets deleted.
  if (reboot_dialog_enabled_)
