...
class ChromeCleanerRebootFlowTest
 ...
 public:
  ChromeCleanerRebootFlowTest() {
    ...
    cleanup_settings_page_url_ = user_initiated_cleanups_enabled_
                                     ? chrome::kChromeUICleanupPageURL
  ...
  const char* cleanup_settings_page_url_ = nullptr;
...
IN_PROC_BROWSER_TEST_P(ChromeCleanerRebootFlowTest,
  ...
  OpenPage(GURL(cleanup_settings_page_url_), browser());
...
IN_PROC_BROWSER_TEST_P(ChromeCleanerRebootFlowTest,
  ...
  Browser* browser = CreateBrowserShowingUrl(GURL(cleanup_settings_page_url_));
  ASSERT_TRUE(chrome_cleaner_util::CleanupPageIsActiveTab(browser));
