...
void SafeBrowsingService::AddSyncPasswordData(const AccountId& account_id) {
  ...
  DCHECK(pending_sync_password_ && pending_sync_password_->first == account_id);
  LOG(ERROR) << "1";
...
void SafeBrowsingService::SaveSyncPasswordHash(Profile* profile) {
  ...
  if (it != sync_password_map_.end()) {
    ...
    hash_password_manager.SavePasswordHash(it->second);
  }
}

void SafeBrowsingService::RemoveSyncPasswordData(const AccountId& account_id) {
  LOG(ERROR) << "SafeBrowsingService::RemoveSyncPasswordData";
  DCHECK(pending_sync_password_ && pending_sync_password_->first == account_id);
  pending_sync_password_.reset();
}
#endif
