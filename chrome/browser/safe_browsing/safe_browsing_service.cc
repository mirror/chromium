...
SafeBrowsingService::SafeBrowsingService(
    ...
    : services_delegate_(ServicesDelegate::Create(this)),
      ...
      use_v4_(v4_usage_status == V4FeatureList::V4UsageStatus::V4_ONLY) {
  VLOG(1) << __FUNCTION__ << ": v4_usage_status: " << v4_usage_status;
  DCHECK_NE(V4FeatureList::V4UsageStatus::V4_INSTANTIATED, v4_usage_status);
}
...
void SafeBrowsingService::Initialize() {
  ...
  services_delegate_->Initialize(use_v4_);
...
SafeBrowsingService::database_manager() const {
  VLOG(1) << __FUNCTION__ << ": use_v4_: " << use_v4_;
  VLOG(1) << __FUNCTION__ << ": database_manager_: " << database_manager_;
  return use_v4_ ? services_delegate_->v4_local_database_manager()
                 : database_manager_;
}
...
#else
  // Don't create a database_manager here. The services_delegate provides that.
  return NULL;
#endif
}
...
SafeBrowsingService::GetV4ProtocolConfig() const {
  ...
  return V4ProtocolConfig(
      ...
      google_apis::GetAPIKey(), kSbDefaultURLPrefix,
      V4ProtocolManagerUtil::GetProductVersion());
}
...
void SafeBrowsingService::StartOnIOThread(
  ...
  ping_manager_ =
      SafeBrowsingPingManager::Create(url_request_context_getter, v4_config);
}
