...
TEST_F(DownloadProtectionServiceTest,
  ...
  ASSERT_TRUE(fetcher);
  ClientDownloadRequest request;
  // how does useful stuff get in upload_data()??
  EXPECT_TRUE(request.ParseFromString(fetcher->upload_data()));
  EXPECT_EQ(download_file_path, request.url());
  EXPECT_EQ(hash_, request.digests().sha256());
  EXPECT_EQ(item.GetReceivedBytes(), request.length());
  ...
  // fetcher->SetResponseString("hey");
  // need to make a new ClientDownloadResponse
  ...
  response.AppendToString(&resp_string);
  fetcher->SetResponseString(resp_string);
