...
static const char kCanvasPageString[] =
    ...
    "      window.open(\"/test\", \"\", \"width=200,height=200\");"
...
class SnapshotBrowserTest : public InProcessBrowserTest {
 public:
  ...
  struct ExpectedSnapshot {
    ExpectedSnapshot() : host_impl(nullptr), r(0), g(0), b(0) {}
  ...
  std::vector<ExpectedSnapshot> expected_snapshots_;
  ...
  void SnapshotCallback(content::RenderWidgetHostImpl* rwhi,
    ...
    for (auto iter = expected_snapshots_.begin();
      ...
      const ExpectedSnapshot& expected = *iter;
      if (expected.host_impl == rwhi) {
        ...
        EXPECT_EQ(static_cast<int>(SkColorGetR(color)),
                  static_cast<int>(expected.r)) << "Red channels differed";
        EXPECT_EQ(static_cast<int>(SkColorGetG(color)),
                  static_cast<int>(expected.g)) << "Green channels differed";
        EXPECT_EQ(static_cast<int>(SkColorGetB(color)),
                  static_cast<int>(expected.b)) << "Blue channels differed";
  ...
  void PickRandomColor(ExpectedSnapshot* expected) {
...
IN_PROC_BROWSER_TEST_F(SnapshotBrowserTest, SingleWindowTest) {
  ...
  for (int i = 0; i < 40; ++i) {
    ...
    std::string colorString = base::StringPrintf(
        "#%02x%02x%02x", expected.r, expected.g, expected.b);
