...
void VerifyProcessGroupOnIOThread() {
  ...
  std::map<std::string, base::WeakPtr<content::UtilityProcessHost>>* groups =
      content::ServiceManagerContext::GetActiveProcessGroupsForTesting();
  ASSERT_TRUE(groups);
  ASSERT_TRUE(base::ContainsKey(*groups, "ash_and_ui"));

  // The process group has a process host.
  base::WeakPtr<content::UtilityProcessHost> host = groups->at("ash_and_ui");
  EXPECT_TRUE(host);

  // The host is associated with a real process.
  EXPECT_NE(host->GetData().handle, base::kNullProcessHandle);
}
