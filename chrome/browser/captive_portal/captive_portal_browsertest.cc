...
void URLRequestTimeoutOnDemandJob::FailJobs(int expected_num_jobs) {
  FailOrAbandonJobsOnIOThread(expected_num_jobs, FAIL_JOBS, net::SSLInfo());
}
...
class CaptivePortalBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void FailJobs(int expected_num_jobs) {
    ...
    if (base::FeatureList::IsEnabled(features::kNetworkService)) {
      ...
      URLRequestTimeoutOnDemandJob::FailJobs(expected_num_jobs);
    }
  }
  ...
  void FailJobsWithCertError(int expected_num_jobs,
    ...
    if (base::FeatureList::IsEnabled(features::kNetworkService)) {
      ...
      URLRequestTimeoutOnDemandJob::FailJobsWithCertError(expected_num_jobs,
                                                          ssl_info);
    }
  }
  ...
  void AbandonJobs(int expected_num_jobs) {
    if (base::FeatureList::IsEnabled(features::kNetworkService)) {
      ...
      URLRequestTimeoutOnDemandJob::AbandonJobs(expected_num_jobs);
    }
  }
 ...
 protected:
  ...
  std::vector<content::mojom::URLLoaderClientPtr> ongoing_mock_requests_;
...
IN_PROC_BROWSER_TEST_F(CaptivePortalBrowserTest,
  ...
  ASSERT_TRUE(embedded_test_server()->Start());
  RunNavigateLoadingTabToTimeoutTest(
      browser(), embedded_test_server()->GetURL("/title.html"),
