...
void MetricsWebContentsObserver::WillStartNavigationRequest(
  ...
  auto tracker = base::MakeUnique<PageLoadTracker>(
      in_foreground_, embedder_interface_.get(), currently_committed_url,
      navigation_handle, user_initiated_info, chain_size, chain_size_same_url);
  PageLoadTracker* tracker_ptr = tracker.get();
  provisional_loads_.insert(
      std::make_pair(navigation_handle, std::move(tracker)));
  for (auto& observer : testing_observers_)
    observer.OnTrackerCreated(tracker_ptr);
}
...
void MetricsWebContentsObserver::OnRequestComplete(
  ...
  if (tracker) {
    ExtraRequestCompleteInfo extra_request_complete_info(
        ...
        load_timing_info ? *load_timing_info : net::LoadTimingInfo());
