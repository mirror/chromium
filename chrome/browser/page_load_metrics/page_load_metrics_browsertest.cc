...
class SessionRestorePageLoadMetricsBrowserTest
 ...
 public:
  ...
  void WaitForFirstMeaningfulPaintOfTab(
    ...
    auto waiter = base::MakeUnique<PageLoadMetricsWaiter>(web_contents);
    waiter->AddPageExpectation(TimingField::FIRST_MEANINGFUL_PAINT);
    waiter->Wait();
  }
  ...
  GURL GetTestURL2() const {
    return embedded_test_server()->GetURL("/page_load_metrics/iframes.html");
  }
...
IN_PROC_BROWSER_TEST_F(SessionRestorePageLoadMetricsBrowserTest,
  ...
  // may not yield any meaningful paints (even it yields the first paint and
  // contentful paint).
  // TODO(ducbui): remove Stop().
  ...
  // No metrics recorded because the tab navigates away during session restore.
  ExpectFirstPaintMetricsTotalCount(0);
}
