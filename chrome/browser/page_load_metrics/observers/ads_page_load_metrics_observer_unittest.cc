...
class ResourceLoadingCancellingThrottle
 ...
 public:
  ...
  explicit ResourceLoadingCancellingThrottle(
    ...
    SetMethodWillRespondCallback(base::BindRepeating(
        &ResourceLoadingCancellingThrottle::MethodWillRespondCallback,
        base::Unretained(this)));
  }

 private:
  // content::TestNavigationThrottle:
  void MethodWillRespondCallback(const TestNavigationThrottle::Status& status) {
    if (status.result.action() != NavigationThrottle::CANCEL) {
      DCHECK_EQ(NavigationThrottle::PROCEED, status.result.action());
      return;
    }
