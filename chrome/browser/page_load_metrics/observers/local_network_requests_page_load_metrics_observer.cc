...
bool GetIPAndPort(
  ...
  *resource_port = extra_request_info.host_port_pair.port();
  ...
  if (net::UrlHostIsLocalhost(resource_ip->ToString())) {
    *resource_ip = net::IPAddress::IPv4Localhost();
    ip_exists = true;
  }
...
LocalNetworkRequestsPageLoadMetricsObserver::OnCommit(
  ...
  // |UrlHostIsLocalhost| assumes (and doesn't verify) that any IPv6 host passed
  ...
  // here.
  if (net::UrlHostIsLocalhost(address.host()) ||
