...
AndroidPageLoadMetricsObserver::OnStart(
  ...
  if (network_quality_provider_) {
    Java_PageLoadMetrics_onNetworkQualityEstimate(
        ...
        static_cast<jlong>(
            network_quality_provider_->GetHttpRTT()->InMicroseconds()),
        static_cast<jlong>(
            network_quality_provider_->GetTransportRTT()->InMicroseconds()));
  } else {
    Java_PageLoadMetrics_onNetworkQualityEstimate(
        env, java_web_contents,
...
void AndroidPageLoadMetricsObserver::OnLoadedResource(
  ...
  int64_t request_start = timing.request_start.since_origin().InMilliseconds();
  ...
  Java_PageLoadMetrics_onLoadedResource(
