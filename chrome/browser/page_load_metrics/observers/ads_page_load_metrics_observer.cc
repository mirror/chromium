...
bool FrameIsGoogleAd(content::NavigationHandle* navigation_handle) {
...
void RecordParentExistsForSubFrame(
    bool parent_exists,
    AdsPageLoadMetricsObserver::AdTypes ad_types) {
...
void AdsPageLoadMetricsObserver::OnDidFinishSubFrameNavigation(
  ...
  AdTypes ad_types = FrameIsAd(navigation_handle);
...
void AdsPageLoadMetricsObserver::OnSubframeNavigationEvaluated(
  ...
  // and therefore would provide bad histogram data.
...
bool AdsPageLoadMetricsObserver::FrameIsSubresourceFilterAd(
    FrameTreeNodeId frame_tree_node_id) {
  auto it = unfinished_subresource_ad_frames_.find(frame_tree_node_id);
  if (it == unfinished_subresource_ad_frames_.end())
    return false;

  unfinished_subresource_ad_frames_.erase(it);
  LOG(ERROR) << "[S] Frame " << frame_tree_node_id << " is an Ad";

  return true;
}

AdsPageLoadMetricsObserver::AdTypes AdsPageLoadMetricsObserver::FrameIsAd(
    content::NavigationHandle* navigation_handle) {
  AdTypes ad_types;

  if (FrameIsGoogleAd(navigation_handle)) {
    LOG(ERROR) << "[G] Frame " << navigation_handle->GetFrameTreeNodeId()
               << " is an Ad";

    ad_types.set(AD_TYPE_GOOGLE);
  }

  if (FrameIsSubresourceFilterAd(navigation_handle->GetFrameTreeNodeId()))
