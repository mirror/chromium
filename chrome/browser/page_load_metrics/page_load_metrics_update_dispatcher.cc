...
class PageLoadTimingMerger {
 public:
  ...
  // Whether we merged a new value, for a page load timing field we didn't
  // previously have a value for in the target PageLoadTiming.
 ...
 private:
  ...
  // < |inout_existing_value|. Updates did_merge_new_timing_value_ to true if a
  // new value was merged.
  // Note that |inout_existing_value| is relative to the main frame's navigation
  // start. |navigation_start_offset| contains the delta in navigation start
  // time between the main frame and the frame for
  // |optional_candidate_new_value|.
  ...
  void MergeInteractiveTiming(
    ...
    MaybeUpdateTimeDelta(&target_interactive_timing->interactive,
                         navigation_start_offset,
                         new_interactive_timing.interactive);
    MaybeUpdateTimeDelta(&target_interactive_timing->first_invalidating_input,
                         navigation_start_offset,
                         new_interactive_timing.first_invalidating_input);
    MaybeUpdateTimeDelta(&target_interactive_timing->interactive_detection,
                         navigation_start_offset,
    ...
    if (target_interactive_timing->first_input_timestamp &&
        target_interactive_timing->first_input_timestamp.value() <=
            candidate_new_first_input_timestamp) {
      return;
    }

    target_interactive_timing->first_input_delay =
        candidate_new_first_input_delay;
