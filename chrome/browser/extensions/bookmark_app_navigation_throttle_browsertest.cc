...
void SubmitFormAndWait(content::WebContents* web_contents,
  ...
  bool is_post = true;
  GURL actual_target_url = target_url;
...
IN_PROC_BROWSER_TEST_P(BookmarkAppNavigationThrottleFormSubmissionBrowserTest,
  ...
  if (start_in == StartIn::BROWSER) {
    ...
    if (start_url_path == kLaunchingPagePath)
      ...
      start_url = embedded_test_server()->GetURL(kAppUrlPath);
  ...
  GURL target_url = embedded_test_server()->GetURL(target_url_path);
  ...
  // tab.
  if (target_url_path == kOutOfScopeUrlPath) {
