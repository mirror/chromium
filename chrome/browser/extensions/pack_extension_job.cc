...
void PackExtensionJob::Run(
  ...
  base::FilePath crx_file_out(root_directory_.value() +
                              kExtensionFileExtension);

  base::FilePath key_file_out;
  if (key_file_.empty()) {
    // TODO(lazyboy): Use
    ...
    if (run_mode_ == RunMode::ASYNCHRONOUS) {
      async_reply_task_runner->PostTask(
          ...
          base::BindOnce(&PackExtensionJob::ReportSuccessOnClientSequence,
                         // See class level comments for why Unretained is safe.
                         base::Unretained(this), crx_file_out, key_file_out));

    } else {
      ReportSuccessOnClientSequence(crx_file_out, key_file_out);
    }
  } else {
    if (run_mode_ == RunMode::ASYNCHRONOUS) {
      async_reply_task_runner->PostTask(
...
void PackExtensionJob::ReportSuccessOnClientSequence(
    const base::FilePath& crx_file_out,
    const base::FilePath& key_file_out) {
  DCHECK(client_);
