...
int CountExtensionItems(const ExtensionContextMenuModel& model) {
  ...
  for (int i = 0; i < model.GetItemCount(); ++i) {
    ...
    if (model.IsCommandIdVisible(command_id)) {
      ...
      if (expected_label == actual_label.substr(0, expected_label.size()))
        ++num_items_found;
...
void VerifyItems(const ExtensionContextMenuModel& model,
  ...
  for (int i = 0, j = 0; i < model.GetItemCount(); i++) {
    int command_id = model.GetCommandIdAt(i);
    if (command_id >= IDC_EXTENSIONS_CONTEXT_CUSTOM_FIRST &&
        command_id <= IDC_EXTENSIONS_CONTEXT_CUSTOM_LAST &&
        model.IsCommandIdVisible(command_id)) {
      EXPECT_EQ(
          base::ASCIIToUTF16(base::StringPiece(item_label() + item_number[j])),
          model.GetLabelAt(i));
      j++;
    }
  }
