...
class ExtensionActivityDataService final
 ...
 public:
  explicit ExtensionActivityDataService(content::BrowserContext* context);
...
int CalculatePingDays(const base::Time& last_ping_day) {
  int days = update_client::kDaysFirstTime;
  if (!last_ping_day.is_null()) {
    days = std::max((base::Time::Now() - last_ping_day).InDays(), 0);
  }
  return days;
}
...
ChromeUpdateClientConfig::ChromeUpdateClientConfig(
    ...
    : impl_(base::CommandLine::ForCurrentProcess(),
      ...
      activity_data_service_(
          std::make_unique<ExtensionActivityDataService>(context)) {
  DCHECK(pref_service_);
  DCHECK(activity_data_service_.get());
}
