...
IN_PROC_BROWSER_TEST_F(ExtensionCSPBypassTest, LoadWebAccessibleScript) {
  ...
  // chrome-extension:-URLs can only bypass CSP in WebUI if the extension is
  // allowed to do so, i.e. it is a component extension and has declared the
  // permission to access any host in the manifest file.
  const GURL webui_url(std::string(content::kChromeUIScheme) + "://" +
