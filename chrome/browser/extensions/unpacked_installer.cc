...
bool UnpackedInstaller::LoadExtension(Manifest::Location location,
  ...
  return IndexAndPersistRulesetIfNeeded(error);
}

bool UnpackedInstaller::IndexAndPersistRulesetIfNeeded(std::string* error) {
  DCHECK(extension());
  base::ThreadRestrictions::AssertIOAllowed();
  ...
  const bool success = declarative_net_request::IndexAndPersistRuleset(
