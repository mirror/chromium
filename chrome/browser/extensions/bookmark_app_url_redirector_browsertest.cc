...
IN_PROC_BROWSER_TEST_F(BookmarkAppUrlRedirectorBrowserTest, Reload_Site) {
  InstallTestBookmarkApp();
  ...
  // Navigate to the Bookmark App's website.
  GURL in_scope_url = embedded_test_server()->GetURL(kInScopeUrlPath);
  auto url_observer = base::MakeUnique<ui_test_utils::UrlLoadObserver>(
      in_scope_url, content::NotificationService::AllSources());
  chrome::NavigateParams params(browser(), in_scope_url,
                                ui::PAGE_TRANSITION_TYPED);
  ui_test_utils::NavigateToURL(&params);
  ...
  // Reload the page.
  url_observer = base::MakeUnique<ui_test_utils::UrlLoadObserver>(
      in_scope_url, content::NotificationService::AllSources());
...
IN_PROC_BROWSER_TEST_F(BookmarkAppUrlRedirectorBrowserTest, Reload_App) {
  InstallTestBookmarkApp();
  Browser* app_browser = OpenTestBookmarkApp();
  content::WebContents* app_web_contents =
      app_browser->tab_strip_model()->GetActiveWebContents();
