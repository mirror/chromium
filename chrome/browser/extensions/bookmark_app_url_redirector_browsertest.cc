...
class BookmarkAppUrlRedirectorBrowserTest : public ExtensionBrowserTest {
 public:
  ...
  void SetUp() override {
    ...
    // Register a request handler that will returns empty pages. Tests
    // are responsible for adding elements and firing events on these empty
    // pages.
...
// Tests that navigating to an out-of-scope URL using the omnibox DOES NOT
// open the Bookmark App.
IN_PROC_BROWSER_TEST_F(BookmarkAppUrlRedirectorBrowserTest,
