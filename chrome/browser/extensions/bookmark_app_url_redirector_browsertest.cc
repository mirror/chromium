...
// Used to test that most transition types for navigations to in-scope or
...
static_assert(static_cast<int32_t>(ui::PAGE_TRANSITION_LAST_CORE) ==
              ...
              "New transition types should be added to the list above.");
...
IN_PROC_BROWSER_TEST_P(BookmarkAppUrlRedirectorNavigationBrowserTest,
  ...
  chrome::NavigateParams params(browser(), target_url, std::get<1>(GetParam()));
  TestTabActionDoesNotOpenAppWindow(target_url,
                                    base::Bind(&NavigateToURLWrapper, &params));
}

// Tests that navigations in subframes don't open new windows.
//
