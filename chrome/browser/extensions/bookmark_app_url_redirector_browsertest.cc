...
INSTANTIATE_TEST_CASE_P(
    ...
    testing::Combine(testing::Values(kInScopeUrlPath, kOutOfScopeUrlPath),
                     testing::Range(ui::PAGE_TRANSITION_LINK,
                                    ui::PAGE_TRANSITION_LAST_CORE)));
static_assert(static_cast<int32_t>(ui::PAGE_TRANSITION_LINK) == 0,
              "The generated tests should cover all core transition types.");
...
IN_PROC_BROWSER_TEST_P(BookmarkAppUrlRedirectorNavigationBrowserTest,
  ...
  if (!ui::PageTransitionIsMainFrame(transition)) {
    ...
    return;
  } else if (ui::PageTransitionCoreTypeIs(ui::PAGE_TRANSITION_LINK,
                                          transition) &&
