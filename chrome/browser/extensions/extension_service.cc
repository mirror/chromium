...
void ExtensionService::EnableExtension(const std::string& extension_id) {
  ...
  system_->extension_registrar()->EnableExtension(extension);
...
void ExtensionService::DisableExtension(const std::string& extension_id,
  ...
  if (registry_->enabled_extensions().Contains(extension->id())) {
    system_->extension_registrar()->DisableExtension(extension);
...
void ExtensionService::UnloadExtension(const std::string& extension_id,
  ...
  system_->extension_registrar()->DeactivateAndRemoveExtension(extension_id,
                                                               reason);
  if (was_enabled)
    PostDeactivateExtension(extension);

  if (extension) {
    // TODO(michaelpg): Move to ExtensionRegistrar::DeactivateAndRemoveExtension
    // once that function can call PostDeactivateExtension() on a delegate. For
    // now, keep this here so it runs in the same order.
    content::NotificationService::current()->Notify(
