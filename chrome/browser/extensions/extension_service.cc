...
void ExtensionService::ReloadExtensionImpl(
  ...
  if (delayed_installs_.Contains(extension_id) &&
      AttemptDelayedInstallation(extension_id, true /*install_immediately*/)) {
    return;
  }
...
bool ExtensionService::AttemptDelayedInstallation(
    const std::string& extension_id,
        ...
        for (std::set<std::string>::const_iterator it = extension_ids.begin();
          ...
          if (delayed_installs_.Contains(*it)) {
            base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
                ...
                base::BindOnce(
                    base::IgnoreResult(
                        &ExtensionService::AttemptDelayedInstallation),
...
void ExtensionService::MaybeFinishDelayedInstallations() {
  ...
  for (const auto& extension_id : to_be_installed) {
    AttemptDelayedInstallation(extension_id, false /*install_immediately*/);
  }
}
