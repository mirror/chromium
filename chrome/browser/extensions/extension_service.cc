...
void ExtensionService::ReloadExtensionImpl(
  ...
  if (transient_current_extension) {
    ...
    system_->runtime_data()->SetBeingUpgraded(transient_current_extension->id(),
                                              true);
    DisableExtension(extension_id, extensions::EXTENSION_DISABLE_RELOAD);
...
void ExtensionService::DisableExtension(const std::string& extension_id,
  ...
  int internal_disable_reason_mask =
      extensions::EXTENSION_DISABLE_RELOAD |
      extensions::EXTENSION_DISABLE_CORRUPTED |
      extensions::EXTENSION_DISABLE_UPDATE_REQUIRED_BY_POLICY;
...
void ExtensionService::DisableUserExtensionsExcept(
  ...
  for (const auto& extension : to_disable) {
    ...
    if (!base::ContainsValue(except_ids, id))
      DisableExtension(id, extensions::EXTENSION_DISABLE_USER_ACTION);
  }
}
...
void ExtensionService::CheckManagementPolicy() {
  ...
  std::map<std::string, extensions::ExtensionDisableReason> to_disable;
  ...
  for (const auto& extension : registry_->enabled_extensions()) {
    ...
    extensions::ExtensionDisableReason disable_reason =
  ...
  for (const auto& extension : registry_->disabled_extensions()) {
    ...
    if (management->CheckMinimumVersion(extension.get(), nullptr) &&
        extension_prefs_->HasDisableReason(
            extension->id(),
            extensions::EXTENSION_DISABLE_UPDATE_REQUIRED_BY_POLICY)) {
      // Is extensions::EXTENSION_DISABLE_UPDATE_REQUIRED_BY_POLICY the *only*
      // reason?
...
void ExtensionService::AddExtension(const Extension* extension) {
  ...
  if (extension_prefs_->IsExtensionBlacklisted(extension->id())) {
    ...
    const int kReasonMask = extensions::EXTENSION_DISABLE_PERMISSIONS_INCREASE |
    ...
    if (reasons & kReasonMask && !(reasons & ~kReasonMask)) {
      extensions::AddExtensionDisabledError(
          this, extension,
          extension_prefs_->HasDisableReason(
              extension->id(), extensions::EXTENSION_DISABLE_REMOTE_INSTALL));
    }
  } else if (reloading) {
...
void ExtensionService::CheckPermissionsIncrease(const Extension* extension,
  ...
  if (is_extension_loaded && previously_disabled) {
    ...
    if (disable_reasons == extensions::EXTENSION_DISABLE_NONE)
    ...
    // extensions::EXTENSION_DISABLE_UNKNOWN_FROM_SYNC isn't used anymore; this
    // code is still here to migrate any existing old state.
    // Remove it after some grace period.
    if (disable_reasons &
        extensions::EXTENSION_DEPRECATED_DISABLE_UNKNOWN_FROM_SYNC) {
      // Remove the extensions::EXTENSION_DISABLE_UNKNOWN_FROM_SYNC reason.
      disable_reasons &=
          ~extensions::EXTENSION_DEPRECATED_DISABLE_UNKNOWN_FROM_SYNC;
      extension_prefs_->RemoveDisableReason(
          extension->id(),
  ...
  if (disable_reasons & extensions::EXTENSION_DISABLE_PERMISSIONS_INCREASE) {
    bool reset_permissions_increase = false;
    if (!is_privilege_increase) {
      reset_permissions_increase = true;
      disable_reasons &= ~extensions::EXTENSION_DISABLE_PERMISSIONS_INCREASE;
      extension_prefs_->RemoveDisableReason(
          extension->id(), extensions::EXTENSION_DISABLE_PERMISSIONS_INCREASE);
    }
...
void ExtensionService::OnExtensionInstalled(
  ...
  disable_reasons &= ~extensions::EXTENSION_DISABLE_CORRUPTED;

  // Unsupported requirements overrides the management policy.
  if (install_flags & extensions::kInstallFlagHasRequirementErrors) {
    disable_reasons |= extensions::EXTENSION_DISABLE_UNSUPPORTED_REQUIREMENT;
  } else {
  ...
  if (extensions::ExtensionManagementFactory::GetForBrowserContext(profile())
    ...
    disable_reasons &= ~extensions::EXTENSION_DISABLE_UPDATE_REQUIRED_BY_POLICY;
  }
...
int ExtensionService::GetDisableReasonsOnInstalled(const Extension* extension) {
  ...
  if (system_->management_policy()->MustRemainDisabled(
    ...
    DCHECK(disable_reason != extensions::EXTENSION_DISABLE_NONE);
  ...
  if (system_->management_policy()->MustRemainEnabled(extension, nullptr))
    return extensions::EXTENSION_DISABLE_NONE;
  ...
  if (extension_prefs_->IsExtensionDisabled(extension->id())) {
    ...
    // disabled by user.
    return disable_reasons == extensions::EXTENSION_DISABLE_NONE
  ...
  if (extensions::ExternalInstallManager::IsPromptingEnabled()) {
    ...
    if (extension->GetType() != Manifest::TYPE_HOSTED_APP &&
        ...
        !is_update_from_same_type) {
      return extensions::EXTENSION_DISABLE_EXTERNAL_EXTENSION;
    }
  }
...
void ExtensionService::UpdateGreylistedExtensions(
  ...
  for (ExtensionIdSet::iterator it = not_yet_greylisted.begin();
    ...
    if (registry_->enabled_extensions().Contains(extension->id()))
      DisableExtension(*it, extensions::EXTENSION_DISABLE_GREYLIST);
  }
}
