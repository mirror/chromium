...
void ExtensionService::LoadExtensionForReload(
  ...
  if (installed_extension.get() &&
    ...
    unpacked_installer->set_be_noisy_on_failure(!fail_quietly);
    unpacked_installer->Load(path);
  }
}

void ExtensionService::ReloadExtension(const std::string& extension_id) {
  extension_registrar_.ReloadExtension(extension_id, /*fail_quietly=*/false);
}

void ExtensionService::ReloadExtensionWithQuietFailure(
    const std::string& extension_id) {
  extension_registrar_.ReloadExtension(extension_id, /*fail_quietly=*/true);
}
