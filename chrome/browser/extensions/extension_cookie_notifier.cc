...
ExtensionCookieNotifier::~ExtensionCookieNotifier() {
  // Officially this lives, and should be destroyed, on the IO thread, but
  // there is a corner case creation pathway (when we create an ProfileIOData
  // but never initialize it) in which this can be created and destroyed on the
  // UI thread without being used.
