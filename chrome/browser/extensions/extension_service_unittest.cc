...
void SetCookieSaveData(bool* result_out, base::RunLoop* run_loop, bool result) {
  *result_out = result;
  run_loop->Quit();
}
...
TEST_F(ExtensionServiceTest, ClearAppData) {
  ...
  cookie_manager_ptr->SetCanonicalCookie(
      *cc.get(), origin1.SchemeIsCryptographic(), true /* modify_http_only */,
      base::BindOnce(&SetCookieSaveData, &set_result, run_loop.get()));
  run_loop->Run();
  ...
  cookie_manager_ptr->GetCookieList(
      ...
      base::BindOnce(&GetCookiesSaveData, &cookies_result, run_loop.get()));
  run_loop->Run();
