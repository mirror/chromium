...
// or incompatibility with current platform version reloads currently installed
// extension version, rather than the version of the delayed install.
TEST_F(ExtensionServiceTest, ReloadExtensionWithPendingImports) {
  ...
  const std::string pending_id = "eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee";
  EXPECT_TRUE(service()->pending_extension_manager()->IsIdPending(pending_id));
  ...
  EXPECT_FALSE(service()->AttemptDelayedInstallation(id, true));
