...
// Test than an unpacked extension with an empty kMetadata folder loads
// successfully.
TEST_F(ExtensionServiceTest, UnpackedExtensionWithEmptyMetadataFolder) {
  ...
  // The kMetadata folder should have been deleted since it did not contain
  // any non-reserved filenames.
  EXPECT_FALSE(base::DirectoryExists(metadata_dir));
}

// Test that an unpacked extension with only reserved filenames in the kMetadata
...
TEST_F(ExtensionServiceTest, UnpackedExtensionWithReservedMetadataFiles) {
  ...
  PersistExtensionWithPaths(
      ...
      extensions::file_util::GetReserveMetadataFilePaths(extension_dir));
  EXPECT_TRUE(base::DirectoryExists(metadata_dir));
...
// Test that an unpacked extension with non-reserved files in the kMetadata
...
TEST_F(ExtensionServiceTest, UnpackedExtensionWithUserMetadataFiles) {
  ...
  base::FilePath metadata_dir =
      extension_dir.Append(extensions::kMetadataFolder);
  base::FilePath non_reserved_file = metadata_dir.Append("a.txt");
  PersistExtensionWithPaths(
      extension_dir, {metadata_dir},
  ...
  expected.append(extension_dir.MaybeAsASCII())
      .append(
          ". Cannot load "
          "extension with file or directory name _metadata. Filenames "
          "starting "
  ...
  // Non-reserved filepaths inside the kMetadata folder should not have been
  // deleted.
  EXPECT_TRUE(base::PathExists(non_reserved_file));
}

// Test than an unpacked extension with an empty kMetadata folder and a folder
...
TEST_F(ExtensionServiceTest,
  ...
  base::FilePath metadata_dir =
      extension_dir.Append(extensions::kMetadataFolder);
  PersistExtensionWithPaths(
      extension_dir, {metadata_dir, extension_dir.Append("_badfolder")}, {});
  ...
  // The kMetadata folder should have been deleted since it did not contain any
...
TEST_F(ExtensionServiceTest, UnpackedExtensionMayNotHaveUnderscore) {
  ...
  ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
  base::FilePath extension_dir = base::MakeAbsoluteFilePath(temp_dir.GetPath());
  base::FilePath underscore_folder = extension_dir.Append("_badfolder");
  PersistExtensionWithPaths(extension_dir, {underscore_folder},
                            {underscore_folder.Append("a.js")});
