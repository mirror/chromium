...
// MockInstallPrompt. We create the MockInstallPrompt but need to passN
...
class ExtensionCrxInstallerTest : public ExtensionBrowserTest {
 protected:
  ...
  void RunCrxInstallerFromDirectory(
      std::unique_ptr<ExtensionInstallPrompt> prompt,
      CrxInstaller::InstallerResultCallback callback,
    ...
    installer->InstallCrxDirectory(extension_id, public_key, crx_directory);
    content::RunMessageLoop();
  }
...
IN_PROC_BROWSER_TEST_F(ExtensionCrxInstallerTest,
                       InstallFromDirectory_FolderDoesNotExist) {
  base::ScopedAllowBlockingForTesting allow_io;
  std::unique_ptr<MockPromptProxy> mock_prompt =
      CreateMockPromptProxyForBrowser(browser());
  ...
  base::FilePath folder = temp_dir.GetPath().AppendASCII("abcdef");
  EXPECT_FALSE(base::PathExists(folder));

  std::string public_key = "123456";
  RunCrxInstallerFromDirectory(
      mock_prompt->CreatePrompt(),
...
IN_PROC_BROWSER_TEST_F(ExtensionCrxInstallerTest,
                       InstallFromDirectory_EmptyFolder) {
  base::ScopedAllowBlockingForTesting allow_io;
  std::unique_ptr<MockPromptProxy> mock_prompt =
      CreateMockPromptProxyForBrowser(browser());
...
IN_PROC_BROWSER_TEST_F(ExtensionCrxInstallerTest,
                       InstallFromDirectory_InvalidPublicKey) {
  base::ScopedAllowBlockingForTesting allow_io;
  std::unique_ptr<MockPromptProxy> mock_prompt =
      CreateMockPromptProxyForBrowser(browser());
