...
BookmarkAppNavigationThrottle::ProcessNavigation() {
  ...
  auto app_for_current_url_ref = GetAppForCurrentURL();
  ...
  // If the app for the current URL is the same as the target app then
  // don't redirect the navigation. This ensures that navigating from
  // https://www.youtube.com/ to https://www.youtube.com/some_video doesn't
  // open a new app window and also that navigating from
  // https://www.google.com/ to https://www.google.com/maps does open a new
  // app window.
  if (!app_for_window_ref && target_app_ref == app_for_current_url_ref) {
