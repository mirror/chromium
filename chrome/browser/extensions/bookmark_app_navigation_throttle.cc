...
// Returns true if |source| is in an App window for an app that corresponds
// to an instalalble website.
bool IsForAppForInstallableWebsite(content::WebContents* source) {
...
BookmarkAppNavigationThrottle::CheckNavigation() {
  ...
  if (!matching_app_ref) {
    ...
    if (IsForAppForInstallableWebsite(source)) {
  ...
  if (source->GetController().IsInitialNavigation()) {
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        git FROM_HERE,
        base::Bind(&BookmarkAppNavigationThrottle::CloseWebContents,
                   weak_ptr_factory_.GetWeakPtr()));
    return content::NavigationThrottle::DEFER;
  }
