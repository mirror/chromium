...
std::string WebstoreInlineInstaller::GetPostData(
  ...
  LOG(ERROR) << "WebstoreInlineInstaller::GetPostData";
  DCHECK(context_type == "application/octet-stream" ||
...
std::string WebstoreInlineInstaller::GetProtoPostData() {
  ...
  LOG(ERROR) << request.SerializeAsString();
