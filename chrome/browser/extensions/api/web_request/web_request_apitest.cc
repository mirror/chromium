...
IN_PROC_BROWSER_TEST_F(ExtensionWebRequestApiTest,
  ...
  class TestWebContentsObserver : public content::WebContentsObserver {
   public:
    ...
    void DidFinishNavigation(
      ...
      EXPECT_TRUE(headers->HasHeaderValue("X-Chrome-ID-Consistency-Response",
                                          kHeaderValueFromServer));
      EXPECT_TRUE(
          headers->HasHeaderValue("X-New-Header", kHeaderValueFromExtension));
      EXPECT_TRUE(
          headers->HasHeaderValue("X-Control", kHeaderValueFromExtension));
  ...
  // Navigate to the URL intercepted by the extension.
  GURL url =
      embedded_test_server()->GetURL("example.com", "/extensions/dice.html");
  ui_test_utils::NavigateToURL(browser(), url);

  // Check that the Dice header was not changed by the extension.
  EXPECT_TRUE(test_webcontents_observer.did_finish_navigation_called_);
