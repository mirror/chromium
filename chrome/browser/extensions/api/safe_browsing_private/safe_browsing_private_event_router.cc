...
namespace {
std::unique_ptr<base::ListValue> CreateEventValueForDangerousDownload(
    const GURL& url,
...
void SafeBrowsingPrivateEventRouter::OnDangerousDownloadAllowed(
    const GURL& url,
    const std::string& file_name,
    const std::string& sha256,
    const std::string& user_name) {
  auto extension_event = std::make_unique<Event>(
      events::SAFE_BROWSING_PRIVATE_ON_DANGEROUS_DOWNLOAD_ALLOWED,
      api::safe_browsing_private::OnDangerousDownloadAllowed::kEventName,
      CreateEventValueForDangerousDownload(url, file_name, sha256, user_name));
  event_router_->BroadcastEvent(std::move(extension_event));
}
...
void SafeBrowsingPrivateEventRouter::OnInterstitialProceeded(
    const GURL& url,
  ...
  params.net_error_code = net_error_code;

  auto event_value = std::make_unique<base::ListValue>();
  event_value->Append(params.ToValue());

  auto extension_event = std::make_unique<Event>(
      events::SAFE_BROWSING_PRIVATE_ON_INTERSTITIAL_PROCEEDED,
      api::safe_browsing_private::OnInterstitialProceeded::kEventName,
      std::move(event_value));
  event_router_->BroadcastEvent(std::move(extension_event));
}
