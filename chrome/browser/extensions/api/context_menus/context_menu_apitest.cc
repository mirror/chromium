...
IN_PROC_BROWSER_TEST_F(ExtensionContextMenuApiTest, ItemLimitVisibility) {
  ...
  for (int i = 0; i < api::context_menus::ACTION_MENU_TOP_LEVEL_LIMIT; i++) {
    ...
    CallAPI(
        base::StringPrintf("create({title: 'item%d', id: 'item%d'});", i, i));
  }
  ...
  int expected_num_items = api::context_menus::ACTION_MENU_TOP_LEVEL_LIMIT + 2;
  VerifyNumContextMenuItems(expected_num_items);
  ...
  ui::MenuModel* submodel =
      top_level_model_->GetSubmenuModelAt(top_level_index());
  ASSERT_TRUE(submodel);
  EXPECT_EQ(expected_num_items, submodel->GetItemCount());
