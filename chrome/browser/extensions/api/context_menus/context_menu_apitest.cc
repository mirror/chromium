...
class ExtensionContextMenuApiTest : public ExtensionApiTest {
 public:
  ...
  bool SetupTopLevelMenuModel() {
    ...
    EXPECT_GT(get_top_level_index(), 0);
...
IN_PROC_BROWSER_TEST_F(ExtensionContextMenuApiTest, ShowOneTopLevelItem) {
  ...
  VerifyMenuItem("item", top_level_model_, get_top_level_index(),
                 ui::MenuModel::TYPE_COMMAND, true);

  // There should be no submenu model.
  EXPECT_FALSE(top_level_model_->GetSubmenuModelAt(get_top_level_index()));
}
...
IN_PROC_BROWSER_TEST_F(ExtensionContextMenuApiTest,
  ...
  VerifyMenuItem("parent", top_level_model_, get_top_level_index(),
                 ui::MenuModel::TYPE_SUBMENU, false);

  // Since the extension submenu is hidden, the previous separator should not be
  // in the model.
  ASSERT_NE(ui::MenuModel::TYPE_SEPARATOR,
            top_level_model_->GetTypeAt(get_top_level_index() - 1));

  ui::MenuModel* submodel =
      top_level_model_->GetSubmenuModelAt(get_top_level_index());
...
IN_PROC_BROWSER_TEST_F(ExtensionContextMenuApiTest,
  ...
  // the model.
  ASSERT_EQ(ui::MenuModel::TYPE_SEPARATOR,
            top_level_model_->GetTypeAt(get_top_level_index() - 1));
...
IN_PROC_BROWSER_TEST_F(ExtensionContextMenuApiTest,
  ...
  VerifyMenuItem(extension()->name(), top_level_model_, get_top_level_index(),
                 ui::MenuModel::TYPE_SUBMENU, false);
...
IN_PROC_BROWSER_TEST_F(ExtensionContextMenuApiTest,
  ...
  VerifyMenuItem("child2", item3Submodel, 1, ui::MenuModel::TYPE_COMMAND, true);
}
