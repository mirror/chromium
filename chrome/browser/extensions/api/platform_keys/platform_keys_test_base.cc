...
void PlatformKeysTestBase::SetUpInProcessBrowserTestFixture() {
  ...
  if (GetEnrollmentStatus() == EnrollmentStatus::ENROLLED) {
...
void PlatformKeysTestBase::SetUpOnMainThread() {
  ...
  if (!IsPreTest()) {
    ...
    if (GetUserStatus() != UserStatus::UNMANAGED) {
...
// function.
bool PlatformKeysTestBase::TestExtension(Browser* browser,
...
void PlatformKeysTestBase::SetUpTestSystemSlotOnIO(
    const base::OnceClosure& done_callback) {
