...
// |key_info|, which must the info of an RSA key. Hash paremeters are included
// from |input_algorithm| if they are available there. If not, the result does
// not include sign/hash parameters and thus isn't complete.
...
bool ParseHashAlgorithm(
  ...
  if (hash_algorithm_name == "SHA-384") {
    *hash_algorithm = chromeos::platform_keys::HASH_ALGORITHM_SHA384;
  }
  if (hash_algorithm_name == "SHA-512") {
