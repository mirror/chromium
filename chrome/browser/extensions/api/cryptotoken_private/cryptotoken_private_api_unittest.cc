...
class CryptoTokenPermissionTest : public ExtensionApiUnittest {
 ...
 protected:
  ...
  // expected to be synchronous and |bubble_action| must be |NONE|.
  bool CanAppIdGetAttestation(
    ...
    if (bubble_expectation == kBubbleExpected) {
      ...
      CHECK_EQ(bubble_action, PermissionRequestManager::NONE);
    }
...
TEST_F(CryptoTokenPermissionTest, PolicyOverridesPrompt) {
  ...
  bool result;
  ASSERT_TRUE(CanAppIdGetAttestation(example_com, kNoBubble,
