...
CryptotokenPrivateIsAppIdHashInEnterpriseContextFunction::Run() {
  ...
  // Convert |params->app_id_hash_base64| from web-safe base64 to normal
  // base64.
  std::string app_id_hash_base64;
  app_id_hash_base64.reserve(params->app_id_hash_base64.size());
  for (size_t i = 0; i < params->app_id_hash_base64.size(); i++) {
    char c = params->app_id_hash_base64[i];
    if (c == '-') {
      c = '+';
    } else if (c == '_') {
      c = '/';
    }
    app_id_hash_base64.push_back(c);
  }
