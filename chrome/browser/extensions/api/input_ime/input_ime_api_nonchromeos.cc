...
class ImeBridgeObserver : public ui::IMEBridgeObserver {
 public:
  ...
  void OnRequestSwitchEngine() override {
    ...
    if (!browser)
      return;
    Profile* pf = browser->profile();
    extensions::InputImeEventRouter* router =
        extensions::GetInputImeEventRouter(pf);
    if (!router)
      return;
    input_method::InputMethodEngine* ime = router->active_engine();
    ui::IMEBridge::Get()->SetCurrentEngineHandler(ime);
  }
};
