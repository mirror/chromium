...
IN_PROC_BROWSER_TEST_F(OnSignInChangedEventTest, FireOnPrimaryAccountSignIn) {
  id_api()->SetAccountStateForTest("primary", false);

  api::identity::AccountInfo account_info;
  account_info.id = "primary";
  std::unique_ptr<base::ListValue> args =
      api::identity::OnSignInChanged::Create(account_info, true);
  AddExpectedEvent(std::move(args));
...
IN_PROC_BROWSER_TEST_F(OnSignInChangedEventTest, FireOnPrimaryAccountSignOut) {
  id_api()->SetAccountStateForTest("primary", true);

  api::identity::AccountInfo account_info;
  account_info.id = "primary";
  std::unique_ptr<base::ListValue> args =
      api::identity::OnSignInChanged::Create(account_info, false);
  AddExpectedEvent(std::move(args));
...
IN_PROC_BROWSER_TEST_F(OnSignInChangedEventTest,
  ...
  id_api()->SetAccountStateForTest("primary", false);
  id_api()->SetAccountStateForTest("secondary", false);
  ...
  // fires.
  args = api::identity::OnSignInChanged::Create(account_info, false);
  AddExpectedEvent(std::move(args));
...
#if !defined(OS_CHROMEOS)
// Test that a signout events are fired for all known accounts when the primary
// account signs out, firing first for the primary account and then for any
// secondary accounts.
IN_PROC_BROWSER_TEST_F(OnSignInChangedEventTest,
                       FireForAllAccountsOnPrimaryAccountSignOut) {
  id_api()->SetAccountStateForTest("primary", true);
  id_api()->SetAccountStateForTest("secondary", true);
