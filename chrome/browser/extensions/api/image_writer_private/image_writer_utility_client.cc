...
namespace {
ImageWriterUtilityClientFactory* g_factory_for_testing = nullptr;
}  // namespace
...
scoped_refptr<ImageWriterUtilityClient> ImageWriterUtilityClient::Create() {
  if (g_factory_for_testing)
    return g_factory_for_testing->Create();
...
ImageWriterUtilityClientFactory::ImageWriterUtilityClientFactory() = default;
