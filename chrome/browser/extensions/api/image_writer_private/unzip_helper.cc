...
void UnzipHelper::UnzipImpl(
  ...
  if (!entry_info)
    return OnError(error::kTempDirError);

  base::FilePath out_image_path = temp_dir_path.Append(entry_info->file_path().BaseName());
  OnOpenSuccess(out_image_path);

  zip_reader_->ExtractCurrentEntryToFilePathAsync(
      out_image_path,
      base::Bind(&UnzipHelper::OnComplete, this),
...
void UnzipHelper::OnOpenSuccess(const base::FilePath& image_path) {
  owner_task_runner_->PostTask(FROM_HERE, base::Bind(open_callback_, image_path));
}
...
void UnzipHelper::OnProgress(int64_t total_bytes, int64_t curr_bytes) {
  owner_task_runner_->PostTask(FROM_HERE, base::Bind(progress_callback_, total_bytes, curr_bytes));
}
}  // namespace image_writer
}  // namespace extensions
