...
Operation::Operation(base::WeakPtr<OperationManager> manager,
    ...
    : manager_(manager),
      ...
      task_runner_(base::CreateSequencedTaskRunnerWithTraits(kBlockingTaskTraits)) {
        printf("+Operation\n");
}
...
void Operation::PostTask(base::OnceClosure task) {
  ...
  //PostRepeatingTaskDoNotUse(task);
}
...
ImageWriterUtilityClient* Operation::StartAndGetUtilityClientForTest() {
  StartUtilityClient();
  return image_writer_client_.get();
}
...
#if !defined(OS_CHROMEOS)
void Operation::StartUtilityClientForTesting() {
  PostTask(base::BindOnce(&Operation::StartUtilityClient, this));
}
#endif
