...
void DeleteConnector(std::unique_ptr<service_manager::Connector> connector) {
  // Just let the connector get deleted when going out of scope.
}
...
Operation::~Operation() {
  ...
  task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&DeleteConnector, std::move(connector_)));
}
