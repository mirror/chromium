...
// (NetworkingOncChromeOS). Note: The test expectations for chromeos, and
// win/mac (NetworkingOncServiceClient) are different to reflect the
...
using extensions::NetworkingOncChromeOS;
...
class NetworkingPrivateChromeOSApiTest : public ExtensionApiTest {
 public:
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    // TODO(pneubeck): Remove the following hack, once the NetworkingOncAPI
  ...
  static std::unique_ptr<KeyedService> CreateNetworkingOncDelegate(
      content::BrowserContext* context) {
    return base::MakeUnique<NetworkingOncChromeOS>(context);
  }
  ...
  void SetUpOnMainThread() override {
    ...
    NetworkingOncDelegateFactory::GetInstance()->SetTestingFactory(
        profile(), &CreateNetworkingOncDelegate);
