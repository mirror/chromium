...
class CredentialsGetterHostClient {
 ...
 private:
  ...
  chrome::mojom::WiFiCredentialsGetterPtr wifi_credentials_getter_ptr_;
...
void CredentialsGetterHostClient::GetWiFiCredentialsOnIOThread(
  ...
  DCHECK(!wifi_credentials_getter_ptr_);
  ...
  connector->BindInterface(chrome::mojom::kWifiUtilWinServiceName,
                           &wifi_credentials_getter_ptr_);

  wifi_credentials_getter_ptr_.set_connection_error_handler(base::Bind(
      &CredentialsGetterHostClient::GetWiFiCredentialsDone,
      base::Unretained(this), /*success=*/false, /*key_data=*/std::string()));
  wifi_credentials_getter_ptr_->GetWiFiCredentials(
      wifi_network_,
...
void CredentialsGetterHostClient::GetWiFiCredentialsDone(
  ...
  wifi_credentials_getter_ptr_.reset();
...
class NetworkingPrivateCredentialsGetterWin
 ...
 private:
  ...
  static void StartOnUIThread(const std::string& network_guid,
    ...
    LOG(ERROR)
        << "** JAY ** NetworkingPrivateCredentialsGetterWin StartOnUIThread";
    std::unique_ptr<service_manager::Connector> connector =
    ...
    content::BrowserThread::PostTask(
        ...
        base::Bind(
            ...
            network_guid, public_key, callback, base::Passed(&connector)));
  }
