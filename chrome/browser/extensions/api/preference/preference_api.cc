...
PreferenceEventRouter::PreferenceEventRouter(Profile* profile)
  ...
  if (profile->HasOffTheRecordProfile())
    OnIncognitoProfileCreated(profile->GetOffTheRecordProfile()->GetPrefs());
  else
...
ExtensionFunction::ResponseAction GetPreferenceFunction::Run() {
  ...
  const PrefService* prefs =
      incognito ? profile->GetReadOnlyOffTheRecordPrefs() : profile->GetPrefs();
