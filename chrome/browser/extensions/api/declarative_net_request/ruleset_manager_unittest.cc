...
class RulesetManagerTest : public DNRTestBase {
 ...
 protected:
  void CreateMatcherForRules(const std::vector<TestRule>& rules,
                             const base::FilePath::CharType* extension_dirname,
                             std::unique_ptr<RulesetMatcher>* matcher) {
    base::FilePath extension_dir =
        temp_dir().GetPath().Append(extension_dirname);
    ...
    SimulateAddExtensionIOThread(last_loaded_extension(),
  ...
  void SetIncognitoEnabled(const Extension* extension, bool incognito_enabled) {
    ...
    // This is required since we mock ExtensionSystem in our tests.
    SimulateAddExtensionIOThread(extension, incognito_enabled);
  }
 ...
 private:
  void SimulateAddExtensionIOThread(const Extension* extension,
...
TEST_P(RulesetManagerTest, MultipleRulesets) {
  ...
  // Test all possible combinations of |rule_one| and |rule_two| enabled.
  ...
  for (int mask = 0; mask < 4; mask++) {
    ...
    if (mask & kEnableRulesetOne) {
      ...
      ASSERT_NO_FATAL_FAILURE(CreateMatcherForRules(
          {rule_one}, FILE_PATH_LITERAL("one"), &matcher));
    ...
    if (mask & kEnableRulesetTwo) {
      ...
      std::unique_ptr<RulesetMatcher> matcher;
      ASSERT_NO_FATAL_FAILURE(CreateMatcherForRules(
          {rule_two}, FILE_PATH_LITERAL("two"), &matcher));
    ...
    EXPECT_EQ((mask & kEnableRulesetOne) != 0,
              manager->ShouldBlockRequest(is_incognito_context, *request_one));
    EXPECT_EQ((mask & kEnableRulesetTwo) != 0,
              manager->ShouldBlockRequest(is_incognito_context, *request_two));
    EXPECT_FALSE(
        manager->ShouldBlockRequest(is_incognito_context, *request_three));
...
TEST_P(RulesetManagerTest, IncognitoRequests) {
  ...
  std::unique_ptr<RulesetMatcher> matcher;
  ASSERT_NO_FATAL_FAILURE(CreateMatcherForRules(
      {rule_one}, FILE_PATH_LITERAL("test_extension"), &matcher));
  ...
  EXPECT_FALSE(util::IsIncognitoEnabled(last_loaded_extension()->id(),
                                        browser_context()));
  EXPECT_FALSE(
      manager->ShouldBlockRequest(true /*is_incognito_context*/, *request));
  EXPECT_TRUE(
      manager->ShouldBlockRequest(false /*is_incognito_context*/, *request));
