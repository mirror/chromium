...
// sub-frames named frame_[1..4] from hosts frame_[1..4].com. Loads an extension
// set to block all sub-frames. Verifies that the correct frames are blocked
...
class DeclarativeNetRequestHostPermissionsBrowserTest
 ...
 protected:
  ...
  void RunTests(const std::vector<FrameLoadResult>& expected_results) {
    CHECK_EQ(4u, expected_results.size());
  ...
  std::string GetMatchPatternForDomain(const std::string& domain) {
    return "*://*." + domain + ".com/*";
  }
