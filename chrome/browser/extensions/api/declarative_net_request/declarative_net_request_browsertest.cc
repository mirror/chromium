...
IN_PROC_BROWSER_TEST_P(DeclarativeNetRequestBrowserTest,
  ...
  for (const auto& rule_data : rules_data) {
    ...
    // The "resourceTypes" property should not be an empty list. It should
    // either be omitted or be a non-empty list.
  ...
  for (const auto& test_case : test_cases) {
    ...
    EXPECT_EQ(
        ...
        execute_script(
            frame, "domAutomationController.send(!!document.frameLoaded);"));
