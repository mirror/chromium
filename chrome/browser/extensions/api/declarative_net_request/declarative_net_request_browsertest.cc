...
class DeclarativeNetRequestBrowserTest
 ...
 protected:
  ...
  void LoadExtensionWithRules(const std::vector<TestRule>& rules,
                              ...
                              const ExtensionLoadType load_type) {
...
IN_PROC_BROWSER_TEST_P(DeclarativeNetRequestBrowserTest,
  ...
  ASSERT_NO_FATAL_FAILURE(
      LoadExtensionWithRules(rules, "test_extension", std::get<0>(GetParam())));
...
IN_PROC_BROWSER_TEST_P(DeclarativeNetRequestBrowserTest,
  ...
  ASSERT_NO_FATAL_FAILURE(LoadExtensionWithRules({rule}, "test_extension",
                                                 std::get<0>(GetParam())));
