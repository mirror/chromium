...
class URLRequestMonitor : public RulesetManager::TestDelegate {
 public:
 ...
 private:
  // RulesetManager::TestDelegate implementation.
...
// Helper to set the TestDelegate for RulesetManager on the IO thread.
void SetRulesetManagerDelegateOnIOThread(RulesetManager::TestDelegate* delegate,
                                         scoped_refptr<InfoMap> info_map) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::IO);
  info_map->GetRulesetManager()->SetDelegateForTesting(delegate);
}
...
IN_PROC_BROWSER_TEST_P(DeclarativeNetRequestBrowserTest,
  ...
  for (const auto& test_case : test_cases) {
    ...
    content::PageType expected_page_type = test_case.did_main_frame_load
...
IN_PROC_BROWSER_TEST_P(DeclarativeNetRequestBrowserTest,
  ...
  for (const auto& test_case : test_cases) {
    ...
    EXPECT_EQ(
        ...
        execute_script(
            frame, "domAutomationController.send(!!document.frame_loaded);"));
...
// Test that an extension with a DNR ruleset behaves correctly after browser
// restart.
IN_PROC_BROWSER_TEST_P(DeclarativeNetRequestBrowserTest, PRE_BrowserRestart) {
  // The unpacked extension directory won't be persisted across browser restart.
...
IN_PROC_BROWSER_TEST_P(DeclarativeNetRequestBrowserTest, BrowserRestart) {
  // The unpacked extension directory won't be persisted across browser restart.
  if (GetParam() == ExtensionLoadType::UNPACKED)
    return;
...
IN_PROC_BROWSER_TEST_P(DeclarativeNetRequestBrowserTest, RendererCacheCleared) {
  // Set-up a delegate for RulesetMatcher to monitor requests to
  // included_script.js.
  URLRequestMonitor script_monitor(embedded_test_server()->GetURL(
      "example.com", "/cached/included_script.js"));
  scoped_refptr<InfoMap> info_map =
      base::WrapRefCounted(ExtensionSystem::Get(profile())->info_map());
  content::BrowserThread::PostTask(
      content::BrowserThread::IO, FROM_HERE,
      base::BindOnce(&SetRulesetManagerDelegateOnIOThread, &script_monitor,
                     info_map));
  content::RunAllTasksUntilIdle();

  GURL url = embedded_test_server()->GetURL(
      "example.com", "/cached/page_with_cacheable_script");
  ...
  // included_script.js since it will be served by the renderer's in-memory
  // cache.
  ui_test_utils::NavigateToURL(browser(), url);
  EXPECT_EQ(content::PAGE_TYPE_NORMAL, GetPageType());
  EXPECT_TRUE(WasFrameWithScriptLoaded(GetMainFrame()));
  ...
  // Now block requests to included_script.js.
  TestRule rule = CreateGenericRule();
  rule.condition->url_filter = std::string("included_script.js");
  ASSERT_NO_FATAL_FAILURE(LoadExtensionWithRules({rule}));
  ...
  // included_script.js and block it.
  ui_test_utils::NavigateToURL(browser(), url);
  EXPECT_EQ(content::PAGE_TYPE_NORMAL, GetPageType());
  EXPECT_FALSE(WasFrameWithScriptLoaded(GetMainFrame()));
  EXPECT_TRUE(script_monitor.GetAndResetRequestSeen(false));
  ...
  // Clear RulesetManager's delegate.
  content::BrowserThread::PostTask(
      content::BrowserThread::IO, FROM_HERE,
      base::BindOnce(&SetRulesetManagerDelegateOnIOThread, nullptr, info_map));
  content::RunAllTasksUntilIdle();
}
