...
IN_PROC_BROWSER_TEST_F(DeclarativeNetRequestBrowserTest,
  ...
  for (const auto& test_case : test_cases) {
    ...
    EXPECT_EQ(test_case.success, WasParsedScriptElementLoaded(GetMainFrame()));
  }
}
...
IN_PROC_BROWSER_TEST_F(DeclarativeNetRequestBrowserTest,
  ...
  for (const auto& test_case : test_cases) {
    ...
    EXPECT_EQ(test_case.frame_1_loaded, WasParsedScriptElementLoaded(child_1));
    EXPECT_EQ(test_case.frame_2_loaded, WasParsedScriptElementLoaded(child_2));
  }
}
...
IN_PROC_BROWSER_TEST_F(DeclarativeNetRequestBrowserTest,
  ...
  for (const auto& test_case : cases) {
    ...
    EXPECT_TRUE(child);
    EXPECT_EQ(test_case.loaded, WasParsedScriptElementLoaded(child));
  }
}
...
IN_PROC_BROWSER_TEST_F(DeclarativeNetRequestBrowserTest, Whitelist) {
  ...
  for (int i = 1; i <= num_requests; i++) {
    ...
    EXPECT_EQ(i % 2 == 0, WasParsedScriptElementLoaded(GetMainFrame()));
  }
}
...
IN_PROC_BROWSER_TEST_F(DeclarativeNetRequestBrowserTest,
  ...
  ui_test_utils::NavigateToURL(browser(), url);
  EXPECT_FALSE(WasParsedScriptElementLoaded(GetMainFrame()));

  DisableExtension(extension_id);
  content::RunAllTasksUntilIdle();
  EXPECT_FALSE(extension_service()->IsExtensionEnabled(extension_id));
  ui_test_utils::NavigateToURL(browser(), url);
  EXPECT_TRUE(WasParsedScriptElementLoaded(GetMainFrame()));
...
IN_PROC_BROWSER_TEST_F(DeclarativeNetRequestBrowserTest, MultipleExtensions) {
  ...
  for (const auto& test_case : test_cases) {
    ...
    EXPECT_EQ(test_case.expected_blocked,
              !WasParsedScriptElementLoaded(GetMainFrame()));
  }
}
...
IN_PROC_BROWSER_TEST_F(DeclarativeNetRequestBrowserTest, Incognito) {
  ...
  EXPECT_TRUE(
      WasParsedScriptElementLoaded(GetMainFrameForBrowser(incognito_browser)));
