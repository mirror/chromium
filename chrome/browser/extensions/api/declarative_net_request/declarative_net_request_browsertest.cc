 ...
 protected:
  ...
  content::RenderFrameHost* GetFrameByName(const std::string& name) const {
    return content::FrameMatchingPredicate(
        web_contents(browser()), base::Bind(&content::FrameMatchesName, name));
  }
...
// Tests that multiple enabled extensions with declarative rulesets having
// redirect rules behave correctly with preference given to most recently
...
IN_PROC_BROWSER_TEST_P(DeclarativeNetRequestBrowserTest, RedirectPriority) {
  ...
  auto host_for_number = [](size_t num) {
    return std::to_string(num) + ".com";
  };

  auto redirect_url_for_priority = [this, host_for_number](size_t priority) {
    return embedded_test_server()
        ->GetURL(host_for_number(priority + 1), "/pages_with_script/index.html")
        .spec();
  };
  ...
  for (size_t i = 1; i <= kNumPatternTypes; i++) {
    ...
    std::string pattern = host_for_number(i) + "*page.html";
  ...
  for (size_t i = 0; i <= kNumPatternTypes + 1; ++i) {
    GURL url = embedded_test_server()->GetURL(host_for_number(i),
                                              "/pages_with_script/page.html");
    SCOPED_TRACE(base::StringPrintf("Testing %s", url.spec().c_str()));
