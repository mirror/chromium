...
IN_PROC_BROWSER_TEST_P(DeclarativeNetRequestBrowserTest,
  ...
  info_map->GetRulesetManager()->SetAsync();
...
IN_PROC_BROWSER_TEST_F(DeclarativeNetRequestBrowserTest, TryDNRExtension) {
  // Top 500 sites as per https://moz.com/top500.
  const std::string top_500_sites[] = {
      "google.com/",
      "youtube.com/",
      "linkedin.com/",
      "wordpress.org/",
  ...
  base::ElapsedTimer timer;
  for (const auto& url : top_500_sites) {
    std::string url_with_host = "https://" + url;
    ui_test_utils::NavigateToURL(browser(), GURL(url_with_host));
  }
