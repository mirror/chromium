...
class RuleIndexingTest
 ...
 protected:
  ...
  void LoadAndExpectSuccess(size_t expected_indexed_rules_count) {
    ...
    EXPECT_TRUE(base::PathExists(GetIndexedRulesetPath(*extension_)));

    // Ensure no load errors were reported.
    EXPECT_TRUE(error_reporter()->GetErrors()->empty());

    tester.ExpectTotalCount(kIndexRulesTimeHistogram, 1u);
    tester.ExpectTotalCount(kIndexAndPersistRulesTimeHistogram, 1u);
