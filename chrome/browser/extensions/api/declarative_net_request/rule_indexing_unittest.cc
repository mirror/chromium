...
class RuleIndexingTest
 ...
 protected:
  ...
  // Loads the extension and verifies that the indexed ruleset location and
  // histogram counts.
 ...
 private:
  void WriteManifestAndRuleset() {
    ...
    if (!persist_invalid_json_file_) {
      JSONFileValueSerializer(json_rules_path).Serialize(*rules_);
    } else {
...
// Ensure that rules which can't be parsed are ignored and cause an install
// warning.
