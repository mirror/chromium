...
class RuleIndexingTest
 ...
 protected:
  ...
  void LoadAndExpectSuccess(size_t expected_indexed_rules_count) {
    ...
    EXPECT_TRUE(base::PathExists(GetIndexedRulesetPath(extension_.get())));

    // Ensure no load errors were reported.
    EXPECT_TRUE(GetErrorReporter()->GetErrors()->empty());

    tester.ExpectTotalCount(kIndexRulesDurationHistogram, 1u);
    tester.ExpectTotalCount(kIndexAndPersistRulesDurationHistogram, 1u);
  ...
  void LoadAndExpectError(const std::string& expected_error_suffix) {
    ...
    // Verify the error. ExtensionErrorReporter will prepend some text to the
    // actual load error.
    const std::vector<base::string16>* errors = GetErrorReporter()->GetErrors();
    ...
    tester.ExpectTotalCount(kIndexRulesDurationHistogram, 0u);
    tester.ExpectTotalCount(kIndexAndPersistRulesDurationHistogram, 0u);
  ...
  ChromeTestExtensionLoader* GetExtensionLoader() { return loader_.get(); }
 ...
 private:
  void WriteManifestAndRuleset() {
    ...
    extension_dir_ = temp_dir().GetPath().Append("test_extension");
    ...
    if (!persist_invalid_json_file_) {
      JSONFileValueSerializer(extension_dir_.Append(kJSONRulesFilename))
          .Serialize(*rules_);
    } else {
      std::string data = "invalid json";
      base::WriteFile(extension_dir_.Append(kJSONRulesFilename), data.c_str(),
                      data.size());
    }

    // Persist manifest file.
    JSONFileValueSerializer(extension_dir_.Append(kManifestFilename))
        .Serialize(*GetManifest());
  }
        ...
        .Set(keys::kDeclarativeNetRequestKey,
             DictionaryBuilder()
                 .Set(keys::kDeclarativeRuleResourcesKey,
                      GetListValue({kJSONRulesFilename}))
                 .Build())
  ...
  ExtensionErrorReporter* GetErrorReporter() {
  ...
  // COMMENT: the comments for this says that this is designed to be used on the
  // stack and not as a test suite member. Why?
...
TEST_P(RuleIndexingTest, EmptyRedirectRuleUrl) {
  ...
  AddRule(rule);
  LoadAndExpectError(ParseInfo(ParseResult::ERROR_EMPTY_REDIRECT_URL, 0u)
                         .GetErrorDescription(kJSONRulesFilename));
}
...
TEST_P(RuleIndexingTest, InvalidJSONRule) {
  ...
  AddRule(rule);
  GetExtensionLoader()->set_ignore_manifest_warnings(true);
  LoadAndExpectSuccess(1);

  // CrxInstaller reloads the extension after moving it, which causes it to lose
  // the install warning. This should be fixed.
  if (GetParam() == ExtensionLoadType::UNPACKED) {
    ASSERT_EQ(1u, extension()->install_warnings().size());
    EXPECT_EQ(InstallWarning(kRulesNotPassedWarning,
...
TEST_P(RuleIndexingTest, ReloadExtension) {
  ...
  tester.ExpectTotalCount(kIndexAndPersistRulesDurationHistogram,
                          expected_histogram_count);
  tester.ExpectBucketCount(kManifestRulesCountHistogram, 1u /* sample */,
                           expected_histogram_count);
}
