...
IN_PROC_BROWSER_TEST_F(PasswordsPrivateApiTest, ImportPasswords) {
  ...
  // TODO(177163): Extension subtests are currently skipped on Windows, which
  // leads to the import passwords function never being triggered, causing the
  // test to fail, so we currently skip the check in this case.
...
IN_PROC_BROWSER_TEST_F(PasswordsPrivateApiTest, ExportPasswords) {
  ...
  // TODO(177163): Extension subtests are currently skipped on Windows, which
  // leads to the export passwords function never being triggered, causing the
  // test to fail, so we currently skip the check in this case.
  if (!ExtensionApiTest::ExtensionSubtestsAreSkipped()) {
