...
PasswordsPrivateDelegateImpl::PasswordsPrivateDelegateImpl(Profile* profile)
    : profile_(profile),
      ...
      password_manager_porter_(std::make_unique<PasswordManagerPorter>(
          ...
          base::BindRepeating(
              &PasswordsPrivateDelegateImpl::OnPasswordsExportCompleted,
...
void PasswordsPrivateDelegateImpl::OnPasswordsExportCompleted(
    const std::string& error) {
  PasswordsPrivateEventRouter* router =
      PasswordsPrivateEventRouterFactory::GetForProfile(profile_);
  if (router) {
    router->OnPasswordsExportCompleted(error);
  }
}
