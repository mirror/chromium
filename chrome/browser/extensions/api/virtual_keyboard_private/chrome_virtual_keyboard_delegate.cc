...
void ChromeVirtualKeyboardDelegate::RestrictFeatures(
  ...
  if (!config.voice_input) {
    ...
    base::OnceCallback<void(bool)> on_has_input_device = base::BindOnce(
        ...
        base::BindOnce(std::move(callback), std::move(update)));
    if (!audio_system_)
      audio_system_ = media::AudioSystem::CreateInstance();
    audio_system_->HasInputDevices(std::move(on_has_input_device));
  }
}
