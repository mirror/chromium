...
TEST_F(ManagementApiUnitTest, DisableReason_BlockedByPolicy) {
  ...
  // Initially the extension should show as enabled.
  {
    ...
    EXPECT_EQ(api::management::EXTENSION_DISABLED_REASON_NONE,
              info->disabled_reason);
  }

  // Simulate blacklisting the extension and verify that the extension shows as
  // disabled with the correct disabled reason.
  ManagementPolicy* policy =
      ExtensionSystem::Get(profile())->management_policy();
...

  {
    ...
    EXPECT_EQ(api::management::EXTENSION_DISABLED_REASON_BLOCKED_BY_POLICY,
              info->disabled_reason);
  }
}
