...
void MessageService::OpenChannelToExtension(
  ...
  if (MessagingDelegate::IsTab(source_contents)) {
    // Add information about the opener's tab.
    source_tab = MessagingDelegate::MaybeGetTabInfo(source_contents);
    if (source_render_frame_host) {
      source_frame_id =
...
void MessageService::OpenChannelToNativeApp(
  ...
  if (policy_permission == MessagingDelegate::DISALLOW) {
  ...
  std::unique_ptr<NativeMessageHost> native_host = NativeMessageHost::Create(
      ...
      policy_permission == MessagingDelegate::ALLOW_ALL, &error);
