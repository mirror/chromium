...
#if defined(OS_CHROMEOS)
  LoginScreenClient::Get()->IsAuthenticating(base::BindOnce(
      &AutotestPrivateLoginStatusFunction::OnCheckAuthenticating, this));

  // Balanced in OnCheckAuthenticating() callback.
  AddRef();
  return RespondLater();
#else
...
void AutotestPrivateLoginStatusFunction::OnCheckAuthenticating(
  ...
  if (user_manager) {
    ...
    result->SetBoolean("isLockScreenAuthenticating", is_authenticating);
