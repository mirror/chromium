...
#if defined(OS_CHROMEOS)
  LoginScreenClient::Get()->ReadyForPassword(base::BindOnce(
      &AutotestPrivateLoginStatusFunction::OnReadyForPassword, this));

  // Balanced in OnReadyForPassword() callback.
  AddRef();
  return RespondLater();
#else
...
void AutotestPrivateLoginStatusFunction::OnReadyForPassword(bool is_ready) {
#if defined(OS_CHROMEOS)
  std::unique_ptr<base::DictionaryValue> result(new base::DictionaryValue);
