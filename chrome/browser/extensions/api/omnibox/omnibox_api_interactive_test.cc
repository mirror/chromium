 ...
 public:
  ...
  void VerifyResult(const std::vector<ExpectedEntry>& entries,
    ...
    for (auto& entry : entries) {
      EXPECT_EQ(base::ASCIIToUTF16(entry.description),
                result.match_at(i).contents);
...
IN_PROC_BROWSER_TEST_F(KeywordEnteredTest, Basic) {
  ...
  VerifyResult({{extension->name(), AutocompleteProvider::TYPE_KEYWORD},
                {"entered 1", AutocompleteProvider::TYPE_KEYWORD},
                {"entered 2", AutocompleteProvider::TYPE_KEYWORD},
                {"kw", AutocompleteProvider::TYPE_SEARCH}},
               autocomplete_controller);

  ExtensionTestMessageListener input_started_listener("onInputStarted", false);

  // Input another key to trigger onInputChanged.
  InputKeys(browser(), {ui::VKEY_SPACE});
  WaitForAutocompleteDone(autocomplete_controller);
  EXPECT_TRUE(autocomplete_controller->done());
  ...
  ASSERT_TRUE(input_changed_listener.WaitUntilSatisfied());

  VerifyResult({{"p", AutocompleteProvider::TYPE_KEYWORD},
                {"p 1", AutocompleteProvider::TYPE_KEYWORD},
...
// onKeywordEntered event is not. In this case only onInputChanged suggestion
// results should populate the omnibox.
IN_PROC_BROWSER_TEST_F(KeywordEnteredTest, Boo) {
  SetupOmnibox();
  const extensions::Extension* extension =
      LoadExtension(test_data_dir_.AppendASCII("omnibox/keyword_entered"));
  EXPECT_EQ("true", ExecuteScriptInBackgroundPage(
