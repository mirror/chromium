...
IN_PROC_BROWSER_TEST_F(OmniboxApiTest, DeleteOmniboxSuggestionResult) {
  ...
  // Input a keyword query and wait for suggestions from the extension.
  InputKeys(browser(),
            {ui::VKEY_K, ui::VKEY_E, ui::VKEY_Y, ui::VKEY_W, ui::VKEY_O,
             ui::VKEY_R, ui::VKEY_D, ui::VKEY_SPACE, ui::VKEY_D});
  ...
  EXPECT_EQ(base::ASCIIToUTF16("keyword n2"),
  ...
  EXPECT_EQ(base::ASCIIToUTF16("keyword d"), result.match_at(3).fill_into_edit);
...
// This is because the FN key is not supported in interactive_test_util.h.
  ...
  EXPECT_TRUE(ui_test_utils::SendKeyPressSync(browser(), ui::VKEY_DELETE, false,
                                              true, false, false));
  // Verify that the onDeleteSuggestion event was created.
  ...
  EXPECT_EQ(base::ASCIIToUTF16("keyword n2"),
            result.match_at(1).fill_into_edit);
  EXPECT_EQ(base::ASCIIToUTF16("keyword d"), result.match_at(2).fill_into_edit);
#endif
}
