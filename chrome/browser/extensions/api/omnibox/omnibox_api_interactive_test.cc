...
// this case only onInputChanged suggestion results should populate the omnibox,
// because onInputChanged suggestion results are more relevant and useful than
// those of onKeywordEntered.
IN_PROC_BROWSER_TEST_F(
  ...
  // should trigger onKeywordEntered, onInputStarted, and onInputChanged events,
  // in that order.
  InputKeys(browser(), {ui::VKEY_LEFT, ui::VKEY_SPACE});
  WaitForAutocompleteDone(autocomplete_controller());
  EXPECT_TRUE(autocomplete_controller()->done());

  // Verify that the onInputStarted and onKeywordEntered event was fired.
  //
  // TODO(catmullings): Enforce ordering check such that onKeywordEntered
  // precedes onInputStarted.
  ASSERT_TRUE(keyword_entered_listener.WaitUntilSatisfied());
  ASSERT_TRUE(input_started_listener.WaitUntilSatisfied());
...
// onInputChanged event is not. In this case only onKeyword suggestion
// results should populate the omnibox.
IN_PROC_BROWSER_TEST_F(KeywordEnteredTest,
  ...
  // Ensure that the matches for onKeywordEntered are displayed.
  ASSERT_EQ(4U, result.size()) << AutocompleteResultAsString(result);

  // Because there is additional input (in this case "p") present, it is offered
  // as a suggestion result. So instead of 3 results, like in
  // VerifyOnKeywordEnteredSuggestionResults(), there are 4 here.
  EXPECT_EQ(base::ASCIIToUTF16("kw p"), result.match_at(0).fill_into_edit);
  EXPECT_EQ(AutocompleteProvider::TYPE_KEYWORD,
            result.match_at(0).provider->type());
