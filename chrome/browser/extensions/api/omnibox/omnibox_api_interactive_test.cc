...
IN_PROC_BROWSER_TEST_F(OmniboxApiTest, ExtensionSuggestionsOnlyInKeywordMode) {
  ...
  // Now clear the omnibox by pressing escape multiple times, then type the
  // same text again except with a backspace after the "kw ".  This will
  // cause leaving keyword mode so the full text will be "kw d" without a
  // keyword chip displayed.
  InputKeys(browser(), {ui::VKEY_ESCAPE, ui::VKEY_ESCAPE, ui::VKEY_K,
                        ui::VKEY_W, ui::VKEY_SPACE, ui::VKEY_BACK, ui::VKEY_D});
