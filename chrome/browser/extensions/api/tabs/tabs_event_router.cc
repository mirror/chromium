...
static bool WillDispatchTabCreatedEvent(
    ...
    Event* event,
    const base::DictionaryValue* listener_filter) {
  event->event_args->GetList().clear();
...
void TabsEventRouter::TabInsertedAt(TabStripModel* tab_strip_model,
  ...
  int tab_id = ExtensionTabUtil::GetTabId(contents);
  std::unique_ptr<base::ListValue> args(new base::ListValue);
  args->GetList().emplace_back(tab_id);
...
void TabsEventRouter::TabDetachedAt(WebContents* contents, int index) {
  ...
  std::unique_ptr<base::ListValue> args(new base::ListValue);
  args->GetList().emplace_back(ExtensionTabUtil::GetTabId(contents));
...
void TabsEventRouter::TabSelectionChanged(
  ...
  for (size_t i = 0; i < new_selection.size(); ++i) {
    ...
    if (!contents)
      break;
    int tab_id = ExtensionTabUtil::GetTabId(contents);
    all_tabs->GetList().emplace_back(tab_id);
  }
...
void TabsEventRouter::DispatchTabUpdatedEvent(
  ...
  args_base->GetList().emplace_back(ExtensionTabUtil::GetTabId(contents));
...
void TabsEventRouter::TabReplacedAt(TabStripModel* tab_strip_model,
  ...
  std::unique_ptr<base::ListValue> args(new base::ListValue);
  args->GetList().emplace_back(new_tab_id);
  args->GetList().emplace_back(old_tab_id);
