...
bool CookiesGetFunction::RunAsync() {
  ...
  std::string store_id =
      parsed_args_->details.store_id.get() ? *parsed_args_->details.store_id
                                           : std::string();
  network::mojom::CookieManagerPtr cookie_manager_ptr;
  if (!ParseStoreContext(this, &store_id, &cookie_manager_ptr))
    return false;
  ...
  store_browser_cookie_manager_->SetCanonicalCookie(
      net::CanonicalCookie(
          ...
          parsed_args_->details.value.get() ? *parsed_args_->details.value
                                            : std::string(),
          // If domain not specified, the lack of leading "." in url_.host()
          // will result in a host cookie.
      ...
      url_.SchemeIsCryptographic(), true /* modify_http_only */,
