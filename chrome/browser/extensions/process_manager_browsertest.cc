...
IN_PROC_BROWSER_TEST_F(ProcessManagerBrowserTest,
  ...
  test_dir.WriteFile("foo.html",
                     "<!DOCTYPE HTML>\n<html><body>Hello world!</body></html>");
  test_dir.WriteFile("background.js", R"(
      chrome.browserAction.onClicked.addListener(tab => {
        console.log("Browser action pressed...");
        var url = chrome.runtime.getURL('foo.html');
        var code =
  ...
  // Invoke the extension's browser action in the incognito profile.
  ...
  BrowserActionTestUtil incognito_action(browser_incognito);
  incognito_action.Press(0);
