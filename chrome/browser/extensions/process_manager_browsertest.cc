...
IN_PROC_BROWSER_TEST_F(ProcessManagerBrowserTest,
  ...
  // Check that the URLs still can be downloaded via <a download> (which starts
  // out as a top-level navigation).
  ...
  for (size_t i = 0; i < arraysize(nested_urls); i++) {
    ...
    EXPECT_TRUE(ExecuteScript(
        popup, "var anchor = document.createElement('a'); anchor.href = '" +
                   nested_urls[i].spec() +
