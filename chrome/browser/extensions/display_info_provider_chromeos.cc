...
bool DisplayInfoProviderChromeOS::SetMirrorMode(
  ...
  auto mixed_param = std::make_unique<display::MixedMirrorModeParams>(
      source_id, destination_ids);
  MirrorParamsErrors error_type = display::ValidateParamsForMixedMirrorMode(
      display_manager->GetCurrentDisplayIdList(), *mixed_param);
  switch (error_type) {
    ...
    case MirrorParamsErrors::kSuccess:
      display_manager->SetMirrorMode(true, std::move(mixed_param));
      return true;
  }
  NOTREACHED();
  return false;
