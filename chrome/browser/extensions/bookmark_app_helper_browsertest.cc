...
class BookmarkAppHelperTest : public DialogBrowserTest {
 public:
  ...
  void ShowDialog(const std::string& name) override {
    ...
    chrome::mojom::WebApplicationInfoProviderAssociatedPtr
        web_app_info_provider;
    browser()
        ->tab_strip_model()
        ->GetActiveWebContents()
        ->GetMainFrame()
        ->GetRemoteAssociatedInterfaces()
        ->GetInterface(&web_app_info_provider);

    auto callback =
        base::Bind(&BookmarkAppHelperTest::OnDidGetWebApplicationInfo,
                   base::Unretained(this));
    web_app_info_provider->GetWebApplicationInfo(callback);
