...
void RecordDisableReasons(int reasons) {
  ...
  if (reasons == EXTENSION_DISABLE_NONE) {
    RecordDisbleReasonHistogram(EXTENSION_DISABLE_NONE);
    return;
  }
  for (int reason = 1; reason < EXTENSION_DISABLE_REASON_LAST; reason <<= 1) {
    if (reasons & reason)
...
void InstalledLoader::Load(const ExtensionInfo& info, bool write_to_prefs) {
  ...
  if (extension.get()) {
    ExtensionDisableReason disable_reason = EXTENSION_DISABLE_NONE;
    ...
    if (!policy->UserMayLoad(extension.get(), nullptr)) {
      ...
      force_disabled = true;
               ...
               extension_prefs_->HasDisableReason(
                   extension->id(), EXTENSION_DISABLE_CORRUPTED)) {
