...
void InstalledLoader::RecordExtensionsMetrics() {
  ...
  std::vector<ExtensionPrefs::ExternalUninstall> uninstalls =
      extension_prefs_->GetExternalUninstalls();
  for (const auto& uninstall : uninstalls) {
    if (extension_urls::IsWebstoreUpdateUrl(uninstall.update_url)) {
        UMA_HISTOGRAM_ENUMERATION("Extensions.ExternalItemState",
                                  EXTERNAL_ITEM_WEBSTORE_UNINSTALLED,
                                  EXTERNAL_ITEM_MAX_ITEMS);
    } else {
