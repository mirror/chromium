...
void InstalledLoader::Load(const ExtensionInfo& info, bool write_to_prefs) {
  ...
  if (extension_prefs_->IsExtensionDisabled(extension->id())) {
    ...
    if (!policy->MustRemainDisabled(extension.get(), nullptr, nullptr) &&
        (disable_reasons & Extension::DISABLE_BLOCKED_BY_POLICY)) {
      extension_prefs_->RemoveDisableReason(
          extension->id(), Extension::DISABLE_BLOCKED_BY_POLICY);
      if (disable_reasons == Extension::DISABLE_BLOCKED_BY_POLICY)
  ...
  } else if (policy->MustRemainDisabled(extension.get(), &disable_reason,
                                        nullptr)) {
