...
TEST_F(ExtensionProtocolsTest, VerificationSeenForFileAccessErrors) {
  ...
  scoped_refptr<Extension> extension = UnzipToDirAndLoadExtension(
      GetContentVerifierTestPath().AppendASCII("source.zip"), unzipped_path);
  ASSERT_TRUE(extension.get());
  ExtensionId extension_id = extension->id();

  const std::string kJs("1024.js");
  base::FilePath relative_path(FILE_PATH_LITERAL("1024.js"));

  // Valid and readable 1024.js.
  {
    TestContentVerifyJobObserver observer(extension_id, relative_path);
  ...
  // chmod -r 1024.js.
  {
    TestContentVerifyJobObserver observer(extension->id(), relative_path);
...
TEST_F(ExtensionProtocolsTest, VerificationSeenForZeroByteFile) {
  ...
  base::FilePath relative_path(FILE_PATH_LITERAL("empty.js"));
  ExtensionId extension_id = extension->id();
  ...
  // Deleting empty file doesn't generate hash mismatch. Note that this is the
