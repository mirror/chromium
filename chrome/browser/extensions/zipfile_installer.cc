...
std::unique_ptr<base::FilePath> PrepareAndGetUnzipDir(const base::FilePath& zip_file) {
  ...
  auto unzip_dir = base::MakeUnique<base::FilePath>();
  if (!base::CreateTemporaryDirInDir(dir_temp, dir_name, unzip_dir.get()))
    return nullptr;
  return unzip_dir;
}
...
void ZipFileInstaller::Unzip(std::unique_ptr<base::FilePath> unzip_dir) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  if (!unzip_dir) {
