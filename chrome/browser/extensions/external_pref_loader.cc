...
class ExternalPrefLoader::PrioritySyncReadyWaiter
 ...
 public:
  explicit PrioritySyncReadyWaiter(Profile* profile)
      : profile_(profile), syncable_pref_observer_(this) {}
...
void ExternalPrefLoader::StartLoading() {
  ...
  if ((options_ & DELAY_LOAD_UNTIL_PRIORITY_SYNC) &&
    ...
    waiter_ptr->Start(base::BindOnce(
        &ExternalPrefLoader::PrioritySyncReadyWaitComplete, this, waiter_ptr));
  } else {
...
void ExternalPrefLoader::PrioritySyncReadyWaitComplete(
  ...
  pending_waiter_list_.erase(std::remove_if(
      pending_waiter_list_.begin(), pending_waiter_list_.end(),
