...
bool CanOpenFileForReading(const base::FilePath& path) {
  ...
  // tasks, this method doesn't require callers to run on tasks with
  // BLOCK_SHUTDOWN.
  ...
  if (!task_runner_.get()) {
    task_runner_ = base::CreateSequencedTaskRunnerWithTraits(
        {// Requires I/O for registry.
         ...
         // SKIP_ON_SHUTDOWN guarantees that if the task starts, it will always
         // finish, and will block shutdown at that point.
