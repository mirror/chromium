...
TEST_F(CountingPolicyTest, GetTodaysActions) {
  ...
  action =
      new Action("punky", mock_clock.Now() - base::TimeDelta::FromMinutes(30),
                 Action::ACTION_API_CALL, "brewster");
  action->mutable_args()->AppendString("meow");
  policy->ProcessAction(action);

  action =
      new Action("punky", mock_clock.Now() - base::TimeDelta::FromMinutes(20),
...
TEST_F(CountingPolicyTest, MergingAndExpiring) {
  ...
  action = new Action("punky", mock_clock.Now(), Action::ACTION_API_CALL,
                      "brewster");
  policy->ProcessAction(action);
...
TEST_F(CountingPolicyTest, StringTableCleaning) {
  ...
  scoped_refptr<Action> action =
      new Action("punky", mock_clock.Now() - base::TimeDelta::FromDays(7),
                 Action::ACTION_API_CALL, "brewster");
  action->set_page_url(GURL("http://www.google.com/"));
  policy->ProcessAction(action);
...
TEST_F(CountingPolicyTest, MoreMerging) {
  ...
  base::Time time1 = mock_clock.Now() - base::TimeDelta::FromDays(2) -
                     base::TimeDelta::FromMinutes(40);
  base::Time time2 = mock_clock.Now() - base::TimeDelta::FromDays(1) -
                     base::TimeDelta::FromMinutes(40);
  base::Time time3 = mock_clock.Now() - base::TimeDelta::FromDays(2) -
                     base::TimeDelta::FromMinutes(20);
