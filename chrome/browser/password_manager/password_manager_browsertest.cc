...
IN_PROC_BROWSER_TEST_F(
  ...
  LOG(ERROR) << "wait for redirect observer";
  ...
  ASSERT_TRUE(content::ExecuteScriptWithoutUserGesture(RenderFrameHost(),
                                                       attacker_redirect));
  LOG(ERROR) << "execute attacker redirect";
  ...
  LOG(ERROR) << "wait for attacker redirect";
