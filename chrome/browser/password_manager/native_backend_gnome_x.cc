...
// The need for WithBaseSyncPrimitives is caused by calls to Wait() to
// synchronise the background task with its response from GnomeKeyring which
// needs to be produced on the main task runner for library limitations. While
// unfortunate, https://crbug.com/739897 provides the discussion which resulted
// in accepting this until the whole backend is deprecated.
