...
class CredentialManagerBrowserTest : public PasswordManagerBrowserTestBase {
 public:
  ...
  void CreatePublicKeyCredentialWithUnsupportedAlgorithmAndExpectSecurityError(
      content::WebContents* web_contents) {
    std::string result;
    std::string script =
        "navigator.credentials.create({ publicKey: {"
    ...
    ASSERT_TRUE(
        content::ExecuteScriptAndExtractString(web_contents, script, &result));
    ASSERT_EQ("SecurityError: The operation is insecure and is not allowed.",
              result);
  }
...
// Tests that when navigator.credentials.create() is called from an invalid
// context, we get a NotAllowedError.
IN_PROC_BROWSER_TEST_F(CredentialManagerBrowserTest,
                       CreatePublicKeyCredentialOriginNotAllowed) {
  ui_test_utils::NavigateToURL(browser(), GURL(url::kAboutBlankURL));
  ASSERT_NO_FATAL_FAILURE(
      CreatePublicKeyCredentialWithUnsupportedAlgorithmAndExpectNotAllowed(
          WebContents()));
}
...
IN_PROC_BROWSER_TEST_F(CredentialManagerBrowserTest,
  ...
  ui_test_utils::NavigateToURL(browser(), a_url1);
  ASSERT_NO_FATAL_FAILURE(
      CreatePublicKeyCredentialWithUnsupportedAlgorithmAndExpectSecurityError(
          WebContents()));
}
