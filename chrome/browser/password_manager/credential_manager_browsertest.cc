...
class CredentialManagerBrowserTest : public PasswordManagerBrowserTestBase {
 public:
  ...
  void CreatePublicKeyCredentialAndExpectFailureWithReason(
      content::WebContents* web_contents,
    ...
    std::string script =
        ...
        "  parameters: [{ type: 'public-key', algorithm: { "
...
IN_PROC_BROWSER_TEST_F(CredentialManagerBrowserTest,
  ...
  ASSERT_NO_FATAL_FAILURE(CreatePublicKeyCredentialAndExpectFailureWithReason(
      WebContents(), "NotAllowedError: The operation is not implemented."));
}
}  // namespace
