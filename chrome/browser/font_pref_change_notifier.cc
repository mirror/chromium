...
void FontPrefChangeNotifier::AddRegistrar(Registrar* registrar) {
  // If this is the first observer, register with prefs.
  if (!has_registered_) {
    has_registered_ = true;
    // Unretained is safe because any_pref_changed_ will automatically
    // unregister us from callbacks.
    any_pref_changed_.Register(
        pref_service_,
        base::BindRepeating(&FontPrefChangeNotifier::AnyPrefChanged,
...
void FontPrefChangeNotifier::AnyPrefChanged(const std::string& font_pref_name) {
  if (base::StartsWith(font_pref_name, pref_names_util::kWebKitFontPrefPrefix,
                       base::CompareCase::SENSITIVE)) {
    for (auto& reg : registrars_)
      reg.callback_.Run(font_pref_name);
  }
}
