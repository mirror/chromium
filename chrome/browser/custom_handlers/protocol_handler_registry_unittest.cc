...
TEST_F(ProtocolHandlerRegistryTest, TestURIPercentEncoded) {
  ...
  // Test if some characters(<, >, {, }, non-ASCII, etc) can be still escaped.
  std::vector<std::pair<std::string, std::string>> entries = {
      ...
      {"web+example://escaped_char=😂 ",
       "https://example.org/search?q=web+example://escaped_char=%F0%9F%98%82"},
      {"web+example://escaped_char=%20with%20%25%20percent",
       "https://example.org/search?q=web+example://"
       "escaped_char=%20with%20%25%20percent"},
  };
