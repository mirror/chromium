...
IN_PROC_BROWSER_TEST_F(CertificateTransparencyBrowserTest, SSLTest) {
  ...
  // address. CT inclusion checks are skipped if a host was not looked up using
  // DNS, so a hostname must be used.
  https_server_.SetSSLConfig(
      net::test_server::EmbeddedTestServer::CERT_COMMON_NAME_IS_DOMAIN);

  // Add "localhost" to the HostCache, so that it appears that a DNS lookup was
  // performed for this hostname. This will make the SCTs provided by the server
  // eligible for inclusion checks.
  ...
  // Chrome's CT policy. To do so, setup the mock_cert_verifier to swap the HTTP
  // server's certificate for a different one during the verification process,
  // and return a CertVerifyResult that indicates that this replacement cert is
  // valid and issued by a known root (CT checks are skipped for private roots).
  {
