...
IN_PROC_BROWSER_TEST_F(SSLUITest, TestInterstitialOptionsOverridable) {
  ...
  const std::string javascript = base::StringPrintf(
      ...
      "(loadTimeData.getBoolean(\"overridable\")"
      " && !loadTimeData.getBoolean(\"hide_primary_button\")) "
      "? (%d) : (%d))",
      security_interstitials::CMD_TEST_SUCCESS,
      security_interstitials::CMD_TEST_FAILURE);
  ASSERT_TRUE(content::ExecuteScriptAndExtractInt(interstitial_rvh, javascript,
                                                  &result));
  EXPECT_EQ(security_interstitials::CMD_TEST_SUCCESS, result);
}
...
IN_PROC_BROWSER_TEST_F(SSLUITest, TestInterstitialOptionsNonOverridable) {
  ...
  ui_test_utils::NavigateToURL(
      browser(), https_server_expired_.GetURL("bla.test", "/ssl/google.html"));
  content::WaitForInterstitialAttach(tab);
