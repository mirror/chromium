 ...
 protected:
  ...
  void WriteTestFiles(const net::EmbeddedTestServer& remote_server,
    ...
    switch (GetParam().second) {
      case SSLUIWorkerFetchTestType::kUseFetch:
        WriteFile(FILE_PATH_LITERAL("data.txt.mock-http-headers"),
                  "HTTP/1.1 200 OK\n"
                  "Content-Type: text/plain\n"
                  "Access-Control-Allow-Origin: *");
        WriteFile(FILE_PATH_LITERAL("data.txt"), "LOADED");
        {
          GURL data_url = hostname.size()
                              ? remote_server.GetURL(hostname, "/data.txt")
                              : remote_server.GetURL("/data.txt");
          WriteFile(FILE_PATH_LITERAL("worker.js"),
                    base::StringPrintf("fetch('%s')"
                                       "  .then(res => res.text())"
      ...
      case SSLUIWorkerFetchTestType::kUseImportScripts: {
        ...
        GURL script_url = hostname.size()
                              ? remote_server.GetURL(hostname, "/imported.js")
                              : remote_server.GetURL("/imported.js");
        WriteFile(FILE_PATH_LITERAL("worker.js"),
                  base::StringPrintf("var data = 'FAILED';"
                                     "try {"
                                     "  importScripts('%s')"
                                     "} catch(e) {}"
                                     "postMessage(data);",
  ...
  void RunMixedContentSettingsTest(
    ...
    CheckSecurityState(tab, CertError::NONE,
                       expected_show_danarous ? security_state::DANGEROUS
                                              : security_state::SECURE,
                       expected_show_danarous ? AuthState::RAN_INSECURE_CONTENT
                                              : AuthState::NONE);
    ...
    CheckSecurityState(
        tab, CertError::NONE,
        expected_show_danarous_after_allow ? security_state::DANGEROUS
                                           : security_state::SECURE,
        expected_show_danarous_after_allow ? AuthState::RAN_INSECURE_CONTENT
                                           : AuthState::NONE);
  }
...
IN_PROC_BROWSER_TEST_P(SSLUIWorkerFetchTest,
  ...
  ASSERT_TRUE(https_server_.Start());
  ASSERT_TRUE(https_server_expired_.Start());
  WriteTestFiles(https_server_expired_, "");
...
IN_PROC_BROWSER_TEST_P(SSLUIWorkerFetchTest,
  ...
  WriteTestFiles(https_server_mismatched_, "");
...
IN_PROC_BROWSER_TEST_P(SSLUIWorkerFetchTest, MixedContentSettings) {
  ...
  RunMixedContentSettingsTest(
      &browser_client, false /* allow_running_insecure_content */,
      false /* strict_mixed_content_checking */,
      true /* strictly_block_blockable_mixed_content */,
...
IN_PROC_BROWSER_TEST_P(SSLUIWorkerFetchTest, MixedContentSubFrame) {
  // TODO(horo): Fix off-main-thread-fetch
  if (GetParam().first == OffMainThreadFetchMode::kEnabled &&
