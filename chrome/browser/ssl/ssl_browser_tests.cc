...
class SymantecMessageSSLUITest : public CertVerifierBrowserTest {
 ...
 private:
  static scoped_refptr<net::X509Certificate> CreateSymantecChain(
    ...
    scoped_refptr<net::X509Certificate> root = net::ImportCertFromFile(
        net::GetTestCertsDirectory(), "legacy_symantec_root.pem");
    std::string der_root;
    if (!root || !root->GetDEREncoded(root->os_cert_handle(), &der_root))
      return nullptr;
