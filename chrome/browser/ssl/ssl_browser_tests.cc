...
enum InterstitialNavType { COMMITTED, TRANSIENT };
...
// committed.
class TransientAndCommittedSSLUITest
    : public SSLUITest,
 ...
 protected:
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    if (InterstitialNavType() == COMMITTED) {
  ...
  // SSLUITest:
  void ProceedThroughInterstitial(WebContents* tab) {
    if (InterstitialNavType() == TRANSIENT) {
  ...
  bool InterstitialNavType() { return GetParam() ? COMMITTED : TRANSIENT; }

  content::RenderFrameHost* InterstitialRFH() {
    WebContents* tab = browser()->tab_strip_model()->GetActiveWebContents();
    if (InterstitialNavType() == TRANSIENT) {
      InterstitialPage* interstitial_page = tab->GetInterstitialPage();
      // Ideally, this should be ASSERT_EQ, but that is incompatible with a
      ...
      return tab->GetMainFrame();
    }
  }

  content::RenderViewHost* InterstitialRVH() {
    WebContents* tab = browser()->tab_strip_model()->GetActiveWebContents();
    if (InterstitialNavType() == TRANSIENT) {
      InterstitialPage* interstitial_page = tab->GetInterstitialPage();
      // Ideally, this should be ASSERT_EQ, but that is incompatible with a
      ...
      EXPECT_EQ(
          SSLBlockingPage::kTypeForTesting,
          interstitial_page->GetDelegateForTesting()->GetTypeForTesting());
      return interstitial_page->GetMainFrame()->GetRenderViewHost();
    } else {
      return tab->GetRenderViewHost();
    }
  }
...
INSTANTIATE_TEST_CASE_P(,
                        TransientAndCommittedSSLUITest,
                        ::testing::Values(false, true));

class TransientSSLUITest : public TransientAndCommittedSSLUITest {};
INSTANTIATE_TEST_CASE_P(, TransientSSLUITest, ::testing::Values(false));

class CommittedSSLUITest : public TransientAndCommittedSSLUITest {};
INSTANTIATE_TEST_CASE_P(, CommittedSSLUITest, ::testing::Values(true));

// Visits a regular page over http.
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestHTTP) {
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestHTTPWithBrokenHTTPSResource) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestBrokenHTTPSWithInsecureContent) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestBrokenHTTPSWithActiveInsecureContent) {
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, GoBackToMixedContent) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, MixedContentWithSameDocumentNavigation) {
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest,
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestBrokenHTTPSMetricsReporting_Proceed) {
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestBrokenHTTPSMetricsReporting_DontProceed) {
  ASSERT_TRUE(https_server_expired_.Start());
  base::HistogramTester histograms;
  const std::string decision_histogram =
      "interstitial.ssl_overridable.decision";
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestOKHTTPS) {
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestHTTPSExpiredCertAndProceed) {
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestInterstitialCrossSiteNavigation) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
  ASSERT_TRUE(https_server_mismatched_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestHTTPSErrorCausedByClockUsingBuildTime) {
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestHTTPSErrorCausedByClockUsingNetwork) {
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestHTTPSExpiredCertAndGoForward) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestHTTPSOCSPOk) {
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestHTTPSOCSPRevoked) {
  EnableRevocationChecking();
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestWSSInvalidCertAndClose) {
  ASSERT_TRUE(embedded_test_server()->Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestWSSInvalidCertAndGoForward) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(wss_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, MarkFileAsNonSecure) {
...
// MarkNonSecureAs Dangerous flag is enabled.
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, MarkAboutAsNonSecure) {
  base::test::ScopedCommandLine scoped_command_line;
  scoped_command_line.GetProcessCommandLine()->AppendSwitchASCII(
      security_state::switches::kMarkHttpAs,
      security_state::switches::kMarkHttpAsDangerous);
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, MarkDataAsNonSecure) {
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  ASSERT_TRUE(contents);
...
// MarkNonSecureAs Dangerous flag is enabled.
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, MarkHTTPAsDangerous) {
  base::test::ScopedCommandLine scoped_command_line;
  scoped_command_line.GetProcessCommandLine()->AppendSwitchASCII(
      security_state::switches::kMarkHttpAs,
      security_state::switches::kMarkHttpAsDangerous);
...
// MarkNonSecureAs Dangerous flag is enabled.
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, MarkBlobAsNonSecure) {
  base::test::ScopedCommandLine scoped_command_line;
  scoped_command_line.GetProcessCommandLine()->AppendSwitchASCII(
      security_state::switches::kMarkHttpAs,
      security_state::switches::kMarkHttpAsDangerous);
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestHTTPSErrorWithNoNavEntry) {
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestBadHTTPSDownload) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestDisplaysInsecureContent) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestDisplaysInsecureForm) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest,
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestUnsafeContents) {
  ASSERT_TRUE(https_server_.Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestDisplaysInsecureContentTwoTabs) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestRunsInsecureContentTwoTabs) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
...
// memory cache).
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestDisplaysCachedInsecureContent) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
...
// memory cache).
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestRunsCachedInsecureContent) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, DISABLED_TestCNInvalidStickiness) {
  ASSERT_TRUE(https_server_.Start());
  ASSERT_TRUE(https_server_mismatched_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestRefNavigation) {
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestCloseTabWithUnsafePopup) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestRedirectBadToGoodHTTPS) {
  ASSERT_TRUE(https_server_.Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestRedirectGoodToBadHTTPS) {
  ASSERT_TRUE(https_server_.Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestRedirectHTTPToGoodHTTPS) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestRedirectHTTPToBadHTTPS) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestRedirectHTTPSToHTTP) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestConnectToBadPort) {
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestGoodFrameNavigation) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestBadFrameNavigation) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, TestUnauthenticatedFrameNavigation) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
// executes.
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestUnsafeContentsWithUserException) {
  WebContents* tab = browser()->tab_strip_model()->GetActiveWebContents();
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestUnsafeImageWithUserException) {
  WebContents* tab = browser()->tab_strip_model()->GetActiveWebContents();
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestInterstitialJavaScriptProceeds) {
  HostContentSettingsMapFactory::GetForProfile(browser()->profile())
...
// http://crbug.com/322948
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestInterstitialJavaScriptGoesBack) {
  HostContentSettingsMapFactory::GetForProfile(browser()->profile())
      ->SetDefaultContentSetting(CONTENT_SETTINGS_TYPE_JAVASCRIPT,
                                 CONTENT_SETTING_BLOCK);
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, ProceedLinkOverridable) {
  ASSERT_TRUE(https_server_expired_.Start());
  WebContents* tab = browser()->tab_strip_model()->GetActiveWebContents();
  ui_test_utils::NavigateToURL(
      browser(), https_server_expired_.GetURL("/ssl/google.html"));
...
IN_PROC_BROWSER_TEST_F(CommittedSSLUITest,
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, TestInterstitialLinksOpenInNewTab) {
  ASSERT_TRUE(https_server_.Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, InterstitialNotAffectedByHideShow) {
  ASSERT_TRUE(https_server_expired_.Start());
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, BadCertFollowedByGoodCert) {
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, RestoreHasSSLState) {
  ASSERT_TRUE(https_server_.Start());
  GURL url(https_server_.GetURL("/ssl/google.html"));
  ui_test_utils::NavigateToURL(browser(), url);
  WebContents* tab = browser()->tab_strip_model()->GetActiveWebContents();
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, DISABLED_RestoreThenNavigateHasSSLState) {
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, SameDocumentHasSSLState) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, ClientRedirectSSLState) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, ClientRedirectFromMixedContentSSLState) {
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, ClientRedirectToMixedContentSSLState) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, SameDocumentNavigationDuringLoadSSLState) {
  ASSERT_TRUE(https_server_.Start());
...
// state.
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, SameDocumentNavigationAfterLoadSSLState) {
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientAndCommittedSSLUITest, PushStateSSLState) {
  ASSERT_TRUE(https_server_.Start());
...
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, OSReportsCaptivePortal) {
  ASSERT_TRUE(https_server_mismatched_.Start());
  base::HistogramTester histograms;
...
// displayed.
IN_PROC_BROWSER_TEST_P(TransientSSLUITest, OSReportsCaptivePortal_FeatureDisabled) {
  base::test::ScopedFeatureList scoped_feature_list;
  scoped_feature_list.InitWithFeatures(
...
IN_PROC_BROWSER_TEST_F(CommittedSSLUITest, ErrorPage) {
  if (!content::IsBrowserSideNavigationEnabled())
    return;
