...
int FindProceedLink(content::RenderViewHost* rvh) {
  ...
  // This should ideally be ASSERT_TRUE, but that is incompatible with returning
  // an int from this function.
  EXPECT_TRUE(content::ExecuteScriptAndExtractInt(rvh, javascript, &result));
  return result;
}
...
IN_PROC_BROWSER_TEST_F(SSLUITest, ProceedLinkOverridable) {
  ...
  EXPECT_EQ(security_interstitials::CMD_TEXT_FOUND,
            FindProceedLink(interstitial_rvh));
}
...
IN_PROC_BROWSER_TEST_F(SSLUITestCommittedInterstitials,
  ...
  EXPECT_EQ(security_interstitials::CMD_TEXT_FOUND, FindProceedLink(rvh));
}
