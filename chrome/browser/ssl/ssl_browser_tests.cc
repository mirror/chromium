...
class TransientAndCommittedSSLUITest
 ...
 protected:
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    if (GetInterstitialNavType() == COMMITTED) {
  ...
  // SSLUITest:
  void ProceedThroughInterstitial(WebContents* tab) {
    if (GetInterstitialNavType() == TRANSIENT) {
  ...
  InterstitialNavType GetInterstitialNavType() const {
    return GetParam() ? COMMITTED : TRANSIENT;
  }
...
IN_PROC_BROWSER_TEST_P(SSLUITest, TestHTTPSExpiredCertAndGoForward) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_P(SSLUITest, TestWSSInvalidCertAndGoForward) {
  ASSERT_TRUE(embedded_test_server()->Start());
  ASSERT_TRUE(wss_server_expired_.Start());
