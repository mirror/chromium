...
class SymantecMessageSSLUITest : public CertVerifierBrowserTest {
 ...
 private:
  static scoped_refptr<net::X509Certificate> CreateSymantecChain(
    ...
    if (use_chrome_66_date) {
      leaf = net::ImportCertFromFile(net::GetTestCertsDirectory(),
                                     "pre_june_2016.pem");
    } else {
      leaf =
          net::ImportCertFromFile(net::GetTestCertsDirectory(), "ok_cert.pem");
...
IN_PROC_BROWSER_TEST_F(SymantecMessageSSLUITest, PreJune2016) {
  ...
  EXPECT_TRUE(base::MatchPattern(console_observer.message(), "*Symantec*"));
}
...
IN_PROC_BROWSER_TEST_F(SymantecMessageSSLUITest, PostJune2016) {
  SetUpCertVerifier(false /* don't use Chrome 66 distrust date */);
  GURL url(https_server_.GetURL("/ssl/google.html"));
  WebContents* tab = browser()->tab_strip_model()->GetActiveWebContents();
