...
TypedNavigationTimingThrottle::WillRedirectRequest() {
  ...
  redirect_url = redirect_url.ReplaceComponents(remove_port);
  if (redirect_url.SchemeIs("https") &&
      (redirect_url.ReplaceComponents(remove_port).GetContent() ==
           initial_url_.ReplaceComponents(remove_port).GetContent() ||
       redirect_url.ReplaceComponents(remove_port).GetContent() ==
           "www." + initial_url_.ReplaceComponents(remove_port).GetContent() ||
       "www." + redirect_url.ReplaceComponents(remove_port).GetContent() ==
           initial_url_.ReplaceComponents(remove_port).GetContent())) {
    UMA_HISTOGRAM_TIMES(
