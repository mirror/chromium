...
// This class is used to keep track of the latest report sent. An
...
// and the interstitial so that the tests can query the latest report that
// was sent.
// NOTE: This keeps a copy of the report, not the original report itself.
class SSLCertReporterCallback {
 public:
  explicit SSLCertReporterCallback(base::RunLoop* run_loop);
  ~SSLCertReporterCallback();

  void ReportSent(const certificate_reporting::ErrorReport& report);

  // Returns a copy of the latest report that was sent.
  const certificate_reporting::ErrorReport& GetLastReport() const;

 private:
  base::RunLoop* run_loop_;
  certificate_reporting::ErrorReport last_report_;
...
std::unique_ptr<SSLCertReporter> CreateMockSSLCertReporter(
    const base::Callback<void(const certificate_reporting::ErrorReport&)>&
