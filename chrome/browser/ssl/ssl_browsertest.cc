...
class SSLUIDynamicInterstitialTest : public CertVerifierBrowserTest {
 public:
  ...
  void AddMismatchDynamicInterstitial(
    ...
    filter->add_sha256_hash("sha256/thickknee");
  ...
  chrome_browser_ssl::DynamicInterstitial* AddMatchingDynamicInterstitial(
    ...
    const net::CertPrincipal& cert_issuer =
        https_server_.GetCertificate().get()->issuer();

    filter->set_issuer_common_name_regex(cert_issuer.common_name);
    filter->set_issuer_organization_regex(cert_issuer.organization_names[0]);
    filter->set_mitm_software_name(kTestMITMSoftwareName);
