...
bool ConfigSingleton::CertContainsMITMSoftwareString(
  ...
  // TODO(sperigo): Certificate common names are being deprecated. Take another
  // look at the MITM software certificates I am trying to match and come up
  // with new matching heuristics against their SubjectAltName instead of their
  // issuer common name.
  ...
  for (const std::regex& regex : *mitm_software_regexes_) {
    if (std::regex_match(ssl_info.cert->issuer().common_name, regex)) {
      RecordUMA(SSLErrorHandler::MITM_SOFTWARE_CERT_FOUND);
      return true;
    }
  }
  return false;
...
void SSLErrorHandler::StartHandlingError() {
  ...
  const bool only_error_is_name_mismatch = IsOnlyCertError(
      net::CERT_STATUS_COMMON_NAME_INVALID, ssl_info_.cert_status, cert_error_);
  ...
  if (IsMITMSoftwareInterstitialEnabled() && !delegate_->IsErrorOverridable() &&
      IsOnlyCertError(net::CERT_STATUS_AUTHORITY_INVALID, ssl_info_.cert_status,
                      cert_error_) &&
...
// the certificate. This function is useful for rendering interstitials that
// are triggered by one specific error code only.
// Parameters:
// - most_serious_error: net::CertStatus code representing the most serious
//   error identified on the certificate. For example, this could be
//   net::CERT_STATUS_COMMON_NAME_INVALID.
// - all_errors: net::CertStatus object that is a bit combination of all of the
//   errors on the certificate. Pass in ssl_error_.cert_status here.
// - error_code: Integer error code that is stored on the SSLErrorHandler. Pass
//   in error_code_ here.
bool SSLErrorHandler::IsOnlyCertError(net::CertStatus most_serious_error,
                                      net::CertStatus all_errors,
                                      int error_code) {
  const net::CertStatus other_errors = all_errors ^ most_serious_error;

  return error_code == net::MapCertStatusToNetError(most_serious_error) &&
         (!net::IsCertStatusError(other_errors) ||
          net::IsCertStatusMinorError(all_errors));
}
