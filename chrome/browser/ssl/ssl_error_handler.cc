...
std::unique_ptr<std::vector<MITMSoftwareType>> LoadMITMSoftwareList(
  ...
  for (const chrome_browser_ssl::MITMSoftware& proto_entry :
       proto.mitm_software()) {
    if (proto_entry.issuer_common_name_regex().empty() &&
...
const std::string ConfigSingleton::MatchKnownMITMSoftware(
  ...
  for (const MITMSoftwareType& mitm_software : *mitm_software_list_) {
    LOG(ERROR) << mitm_software.issuer_organization_regex;
    LOG(ERROR) << mitm_software.issuer_common_name_regex;
    LOG(ERROR) << cert->issuer().common_name;
    LOG(ERROR) << cert->issuer().organization_names[0];
    ...
    // Case where both the |issuer_common_name_regex| and
    // |issuer_organization_regex| are set.
    if (!mitm_software.issuer_common_name_regex.empty() &&
        !mitm_software.issuer_organization_regex.empty()) {
      ...
      // Case where only the |issuer_organization_regex| is set.
      ...
      // Case where only the |issuer_common_name_regex| is set.
