...
class ConfigSingleton {
 public:
  ...
  // Returns an DynamicInterstitial object if the certificates in |ssl_info|
  // matches any of dynamic interstitials. Returns null if there is no match.
...
DynamicInterstitialInfo* ConfigSingleton::MatchDynamicInterstitial(
    const net::SSLInfo& ssl_info) {
...
class SSLErrorHandlerDelegateImpl : public SSLErrorHandler::Delegate {
 public:
  ...
  void ShowSSLInterstitial(GURL support_url = GURL()) override;
...
void SSLErrorHandlerDelegateImpl::ShowSSLInterstitial(GURL support_url) {
...
void SSLErrorHandler::StartHandlingError() {
  ...
  DynamicInterstitialInfo* dynamic_interstitial =
      g_config.Pointer()->MatchDynamicInterstitial(ssl_info_);
  if (dynamic_interstitial) {
    ShowDynamicInterstitial(dynamic_interstitial);
    return;
  }
...
void SSLErrorHandler::ShowDynamicInterstitial(
  ...
  switch (dynamic_interstitial->interstitial_type()) {
    case DynamicInterstitialPageType::NONE:
      NOTREACHED();
    case DynamicInterstitialPageType::SSL:
      delegate_->ShowSSLInterstitial(dynamic_interstitial->support_url());
      return;
    case DynamicInterstitialPageType::CAPTIVE_PORTAL:
      delegate_->ShowCaptivePortalInterstitial(GURL());
      return;
    case DynamicInterstitialPageType::MITM_SOFTWARE:
