...
class SSLErrorHandlerDelegateImpl : public SSLErrorHandler::Delegate {
 public:
  SSLErrorHandlerDelegateImpl(
      ...
      const base::Callback<void(content::CertificateRequestResultType)>&
          decision_callback,
      base::Callback<void(std::string)> throttle_callback)
      : web_contents_(web_contents),
        ssl_info_(ssl_info),
        profile_(profile),
        ...
        throttle_callback_(throttle_callback) {}
  ...
  void CheckSuggestedUrl(const GURL& suggested_url,
                         const CommonNameMismatchHandler::CheckUrlCallback&
                             decision_callback) override;
 ...
 private:
  ...
  const base::Callback<void(content::CertificateRequestResultType)>
      decision_callback_;
  base::Callback<void(std::string)> throttle_callback_;
};
...
void SSLErrorHandlerDelegateImpl::ShowInterstitial(
    security_interstitials::SecurityInterstitialPage* interstitial_page) {
  // TODO
  if (true) {  // SSLErrorHandler::AreCommittedInterstitialsEnabled()) {
    // TODO: DCHECK_FALSE(throttle_callback.is_null())
    throttle_callback_.Run(interstitial_page->GetHTMLContents());
  } else {
  ...
  SSLErrorHandler* error_handler = new SSLErrorHandler(
      std::unique_ptr<SSLErrorHandler::Delegate>(
          new SSLErrorHandlerDelegateImpl(
              ...
              OnAllowCertificate, throttle_callback)),
...
// static
int SSLErrorHandler::CalculateOptionsMask(net::Error cert_error,
                                          bool hard_override_disabled,
                                          bool should_ssl_errors_be_fatal,
                                          bool is_superfish,
                                          bool expired_previous_decision) {
