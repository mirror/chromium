...
#if BUILDFLAG(ENABLE_CAPTIVE_PORTAL_DETECTION)
#include "chrome/browser/content_filter/content_filter_service.h"
#include "chrome/browser/content_filter/content_filter_service_factory.h"
...
bool ConfigSingleton::CertContainsContentFilterString(
  ...
  // for (const std::string& regex : *content_filter_regexes_) {
...
class SSLErrorHandlerDelegateImpl : public SSLErrorHandler::Delegate {
 public:
  ...
  void CheckForContentFilter() override;
...
void SSLErrorHandlerDelegateImpl::CheckForContentFilter() {
// #if BUILDFLAG(ENABLE_CONTENT_FILTER_DETECTION)
...
#if BUILDFLAG(ENABLE_CONTENT_FILTER_DETECTION)
  (new ContentFilterBlockingPage(web_contents_, request_url_,
                                 std::move(ssl_cert_reporter_), ssl_info_,
                                 callback_))
      ->Show();
#else
