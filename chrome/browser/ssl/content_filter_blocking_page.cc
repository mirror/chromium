...
const char kMetricsName[] = "content_filter";

std::unique_ptr<ChromeMetricsHelper> CreateMetricsHelper(
    content::WebContents* web_contents,
    const GURL& request_url) {
  // Set up the metrics helper for the ContentFilterUI.
...
// static
InterstitialPageDelegate::TypeID ContentFilterBlockingPage::kTypeForTesting =
    &ContentFilterBlockingPage::kTypeForTesting;

// Note that we always create a navigation entry with SSL errors.
// No error happening loading a sub-resource triggers an interstitial so far.
// Creating an interstitial without showing (e.g. from chrome://interstitials)
// it leaks memory, so don't create it here.
ContentFilterBlockingPage::ContentFilterBlockingPage(
    content::WebContents* web_contents,
    const GURL& request_url,
    ...
    : SecurityInterstitialPage(
      ...
      cert_report_helper_(new CertReportHelper(
          ...
          base::Time::Now(),
          nullptr)),
      content_filter_ui_(new security_interstitials::ContentFilterUI(request_url,
                                                           cert_error,
                                                           ssl_info,
                                                           controller()) {
  RecordUMA(SHOW_ALL);
...
bool ContentFilterBlockingPage::ShouldCreateNewNavigation() const {
  return true;
}

InterstitialPageDelegate::TypeID ContentFilterBlockingPage::GetTypeForTesting()
    const {
  return ContentFilterBlockingPage::kTypeForTesting;
}

void ContentFilterBlockingPage::PopulateInterstitialStrings(
    base::DictionaryValue* load_time_data) {
  content_filter_ui_->PopulateStringsForHTML(load_time_data);
  cert_report_helper_->PopulateExtendedReportingOption(load_time_data);
}

void ContentFilterBlockingPage::OverrideEntry(NavigationEntry* entry) {
  entry->GetSSL() = content::SSLStatus(ssl_info_);
}

void ContentFilterBlockingPage::SetSSLCertReporterForTesting(
    std::unique_ptr<SSLCertReporter> ssl_cert_reporter) {
...
void ContentFilterBlockingPage::CommandReceived(const std::string& command) {
  ...
  content_filter_ui_->HandleCommand(
...
void ContentFilterBlockingPage::OverrideRendererPrefs(
...
void ContentFilterBlockingPage::OnDontProceed() {
...
void ContentFilterBlockingPage::NotifyDenyCertificate() {
