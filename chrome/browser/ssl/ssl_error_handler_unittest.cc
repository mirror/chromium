...
class SSLErrorAssistantTest : public ChromeRenderViewHostTestHarness {
 ...
 protected:
  ...
  void InitMITMSoftwareList() {
    ...
    filter->set_issuer_common_name("Misconfigured Firewall_[A-Z0-9]+");
    filter->set_issuer_org("Misconfigured Firewall");
    SSLErrorHandler::SetErrorAssistantProto(std::move(config_proto));
  }
...
TEST_F(SSLErrorAssistantTest,
       MITMSoftware_CertificateCommonNameMatchOnly_NoInterstitial) {
  base::HistogramTester histograms;
  ...
  filter->set_name("Misconfigured Firewall");
  filter->set_issuer_common_name("Misconfigured Firewall_[A-Z0-9]+");
  filter->set_issuer_org("Non-Matching Organization Name");
  SSLErrorHandler::SetErrorAssistantProto(std::move(config_proto));
...
TEST_F(SSLErrorAssistantTest,
       MITMSoftware_CertificateOrganizationMatchOnly_NoInterstitial) {
  base::HistogramTester histograms;
  SetMITMSoftwareFeatureEnabled(true);
  ...
  chrome_browser_ssl::MITMSoftware* filter = config_proto->add_mitm_software();
  filter->set_name("Misconfigured Firewall");
  filter->set_issuer_common_name("Non-Matching Issuer Common Name");
  filter->set_issuer_org("Misconfigured Firewall");
  SSLErrorHandler::SetErrorAssistantProto(std::move(config_proto));
...
TEST_F(SSLErrorAssistantTest, MITMSoftware_MatchingCertificate) {
  SetMITMSoftwareFeatureEnabled(true);
