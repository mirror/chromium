...
class SSLErrorAssistantTest : public ChromeRenderViewHostTestHarness {
 ...
 protected:
  ...
  void SetContentFilterFeatureEnabled(bool enabled) {
    if (enabled) {
      scoped_feature_list_.InitFromCommandLine(
          "ContentFilterInterstitial" /* enabled */,
          std::string() /* disabled */);
    } else {
      scoped_feature_list_.InitFromCommandLine(
          std::string(), "ContentFilterInterstitial" /* disabled */);
    }
  }
  ...
  void RunContentFilterTest() {
    // Enable finch experiment for content filter interstitials.
    ASSERT_TRUE(base::FieldTrialList::CreateFieldTrial(
        "ContentFilterInterstitial", "Enabled"));
...
TEST_F(SSLErrorAssistantTest, TestContentFilterTriggerOnMatchingCertificate) {
  ...
  SetContentFilterFeatureEnabled(true);
...
TEST_F(SSLErrorAssistantTest,
       TestNoContentFilterInterstitialWithFeatureDisabled) {
  base::HistogramTester histograms;

  SetContentFilterFeatureEnabled(false);
  ResetErrorHandlerFromString(kMcAfeeCert, net::CERT_STATUS_AUTHORITY_INVALID);
  delegate()->set_non_overridable_error();
  RunContentFilterTest();
...
TEST_F(SSLErrorAssistantTest,
       TestGenericSSLInterstitialWhenContentFilterDisabledOnIOS) {
  base::HistogramTester histograms;
