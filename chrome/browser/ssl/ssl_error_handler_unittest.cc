...
class TestSSLErrorHandlerDelegate : public SSLErrorHandler::Delegate {
 public:
  ...
  void set_enterprise_managed() { is_enterprise_managed_ = true; }
 ...
 private:
  ...
  bool IsEnterpriseManaged() const override { return is_enterprise_managed_; }
...
// Default test circumstance is that a machine is not enterprise managed. Test
// that the MITM software interstitial can still display on enterprise managed
// machines.
TEST_F(SSLErrorAssistantTest, MITMSoftware_EnterpriseManaged) {
  ...
  ResetErrorHandlerFromString(kOutdatedAntivirusCert,
                              net::CERT_STATUS_AUTHORITY_INVALID);
  delegate()->set_non_overridable_error();
  delegate()->set_enterprise_managed();
