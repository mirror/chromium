...
void SSLErrorNavigationThrottle::ShowInterstitial(
  ...
  SSLErrorTabHelper::FromWebContents(web_contents)
      ->SetBlockingPageForNavigation(std::move(blocking_page),
...
void SSLErrorNavigationThrottle::HandleSSLError() {
  ...
  ssl_info_ = handle->GetSSLInfo().value();

  SSLErrorHandler::HandleSSLError(
      handle->GetWebContents(), handle->GetNetErrorCode(), ssl_info_,
      handle->GetURL(), handle->ShouldSSLErrorsBeFatal(), false,
      std::move(ssl_cert_reporter_), base::Bind(&EmptyCallback),
