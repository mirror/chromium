...
class TestSSLErrorNavigationThrottle : public SSLErrorNavigationThrottle {
 public:
  TestSSLErrorNavigationThrottle(content::NavigationHandle* handle,
 ...
 public:
  void SetUp() override {
    ...
    std::unique_ptr<TestSSLErrorNavigationThrottle> throttle =
        std::make_unique<TestSSLErrorNavigationThrottle>(handle_.get(), async_);
...
TEST_P(SSLErrorNavigationThrottleTest, SSLInfoWithCertError) {
  ...
  EXPECT_EQ(content::NavigationThrottle::DEFER, result.action());
  base::RunLoop().RunUntilIdle();
  // TODO(REVIEW): Can we retrieve the cancel result here?
}
