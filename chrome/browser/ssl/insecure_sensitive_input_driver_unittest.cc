...
// Tests that credit card edit notifications are forwarded.
TEST_F(InsecureSensitiveInputDriverTest, CreditCardFieldEdit) {
  std::unique_ptr<InsecureSensitiveInputDriver> driver(
      new InsecureSensitiveInputDriver(main_rfh()));
  ...
  ASSERT_TRUE(entry);
  EXPECT_EQ(url, entry->GetURL());
  EXPECT_FALSE(GetInputEvents(entry).credit_card_field_edited);
  ...
  ASSERT_TRUE(entry);
  EXPECT_EQ(url, entry->GetURL());
  EXPECT_TRUE(GetInputEvents(entry).credit_card_field_edited);
}
