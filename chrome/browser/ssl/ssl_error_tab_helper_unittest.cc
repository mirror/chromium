...
TEST_F(SSLErrorTabHelperTest, SingleBlockingPage) {
  ...
  EXPECT_EQ(nullptr,
            helper->blocking_page_for_currently_committed_navigation());
  EXPECT_EQ(1u, helper->blocking_pages_for_navigations().size());
  EXPECT_EQ(blocking_page,
            helper
                ->blocking_pages_for_navigations()[blocking_page_handle
                                                       ->GetNavigationId()]
                .get());
  ...
  EXPECT_EQ(blocking_page,
            helper->blocking_page_for_currently_committed_navigation());
  EXPECT_EQ(0u, helper->blocking_pages_for_navigations().size());
...
TEST_F(SSLErrorTabHelperTest, MultipleBlockingPages) {
  ...
  EXPECT_EQ(blocking_page2,
            helper->blocking_pages_for_navigations()[handle2->GetNavigationId()]
                .get());
  ...
  EXPECT_EQ(blocking_page1,
            helper->blocking_page_for_currently_committed_navigation());
  EXPECT_EQ(2u, helper->blocking_pages_for_navigations().size());
  EXPECT_EQ(blocking_page2,
            helper->blocking_pages_for_navigations()[handle2->GetNavigationId()]
                .get());
  EXPECT_EQ(blocking_page3,
            helper->blocking_pages_for_navigations()[handle3->GetNavigationId()]
                .get());
