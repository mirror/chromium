...
class SecurityStateTabHelperHistogramTest
 ...
 protected:
  ...
  void StartFormSubmissionNavigation() {
    ...
    helper_->DidFinishNavigation(handle.get());
  }
...
TEST_P(SecurityStateTabHelperHistogramTest, FormSubmissionHistogram) {
  ...
  histograms.ExpectTotalCount(kFormSubmissionSecurityLevelHistogram, 1);
  histograms.ExpectBucketCount(kFormSubmissionSecurityLevelHistogram, 0, 1);
}
