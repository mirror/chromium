...
CaptivePortalTestingNavigationThrottle::WillFailRequest() {
  ...
  if (AreCommittedInterstitialsEnabled()) {
    std::string html = blocking_page->OnReadyToShowHTMLContents();
    // Hand the blocking page back to the WebContents's SSLErrorTabHelper to
    // own.
...
void TestingThrottleInstaller::DidStartNavigation(
  ...
  navigation_handle->RegisterThrottleForTesting(
      base::MakeUnique<CaptivePortalTestingNavigationThrottle>(
...
void CaptivePortalBlockingPageTest::TestInterstitial(
  ...
  // only occurs pre-committed interstitials. With committed interstitials
  // enabled, the NavigationEntry's SSLStatus is updated to reflect a
  // certificate error by the navigation stack, not by the blocking page itself,
  // and that navigation code isn't exercised by this test. (It's covered by
  // other tests in ssl_browsertest.cc).
  if (!AreCommittedInterstitialsEnabled()) {
