...
CaptivePortalTestingNavigationThrottle::WillFailRequest() {
  ...
  EXPECT_TRUE(WaitForRenderFrameReady(navigation_handle()
                                          ...
                                          ->GetInterstitialPage()
                                          ->GetMainFrame()));
  // When committed interstitials are disabled, there's no need to modify the
  // flow of the navigation, so return PROCEED to tell the navigation stack to
  // continue with failing the request. The blocking page will be shown over the
  // error page by the call to Show() above.
  return PROCEED;
}
