...
// Chrome OS has its own out-of-box-experience code.
  ...
  if (!state->master_prefs->compressed_variations_seed.empty()) {
    state->local_state->SetString(variations::prefs::kVariationsCompressedSeed,
                                  state->master_prefs->compressed_variations_seed);
    if (!state->master_prefs->variations_seed_signature.empty()) {
      state->local_state->SetString(variations::prefs::kVariationsSeedSignature,
  ...
  if (!state->master_prefs->suppress_default_browser_prompt_for_version.empty()) {
    state->local_state->SetString(
  ...
  if (!state->master_prefs->welcome_page_on_os_upgrade_enabled)
    state->local_state->SetBoolean(prefs::kWelcomePageOnOSUpgradeEnabled, false);
#endif
...
void SetupFieldTrials(LocalState* state) {
  auto client =
      std::make_unique<ChromeMetricsServicesManagerClient>(state->local_state.get());
...
int LoadLocalState(LocalState* result) {
  ...
  internal::CreateLocalState(result->local_state_task_runner,
                             &result->pref_service_factory,
                             &result->browser_policy_connector,
                             &result->local_state);
  ...
  chromeos::CrosSettings::Initialize(
      result->local_state.get(),
      static_cast<policy::BrowserPolicyConnectorChromeOS*>(result->browser_policy_connector.get()));
#endif  // defined(OS_CHROMEOS)
...
void CreateLocalState(
  ...
  *local_state = chrome_prefs::CreateLocalState(
      ...
      pref_registry, false, browser_policy_connector->get(),
      std::move(delegate));
  DCHECK(*local_state);
}
