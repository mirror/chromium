...
void InitializeLocalState(LocalStateData* data) {
  TRACE_EVENT0("startup", "InitializeLocalState")
  ...
  if (first_run::IsChromeFirstRun()) {
    ...
    if (GoogleUpdateSettings::GetLanguage(&install_lang)) {
      data->local_state->SetString(prefs::kApplicationLocale,
    ...
    if (GoogleUpdateSettings::GetCollectStatsConsentDefault(&stats_default)) {
      ...
      metrics::RecordMetricsReportingDefaultState(
          data->local_state.get(), stats_default
    ...
    if (!local_state_file_exists) {
      ...
      std::unique_ptr<PrefService> parent_local_state(
          chrome_prefs::CreateLocalState(
              parent_profile, data->local_state_task_runner.get(),
              data->browser_policy_connector->GetPolicyService(), registry,
              false, data->browser_policy_connector.get(), nullptr));
      registry->RegisterStringPref(prefs::kApplicationLocale, std::string());
      // Right now, we only inherit the locale setting from the parent profile.
      data->local_state->SetString(
  ...
  if (command_line->HasSwitch(chromeos::switches::kLoginManager)) {
    std::string owner_locale =
        data->local_state->GetString(prefs::kOwnerLocale);
    // Ensure that we start with owner's locale.
    if (!owner_locale.empty() &&
        data->local_state->GetString(prefs::kApplicationLocale) !=
            owner_locale &&
        !data->local_state->IsManagedPreference(prefs::kApplicationLocale)) {
      data->local_state->SetString(prefs::kApplicationLocale, owner_locale);
    }
  }
...
int ApplyFirstRunPrefs(const base::FilePath& user_data_dir,
  ...
  data->master_prefs = std::make_unique<first_run::MasterPrefs>();
#endif
...
// Chrome OS has its own out-of-box-experience code.
  ...
  // in master prefs.
  if (!data->master_prefs->compressed_variations_seed.empty()) {
    data->local_state->SetString(
        variations::prefs::kVariationsCompressedSeed,
        data->master_prefs->compressed_variations_seed);
    if (!data->master_prefs->variations_seed_signature.empty()) {
      data->local_state->SetString(
          variations::prefs::kVariationsSeedSignature,
          data->master_prefs->variations_seed_signature);
    }
    ...
    data->local_state->SetInt64(variations::prefs::kVariationsSeedDate,
                                base::Time::Now().ToInternalValue());
  }

  if (!data->master_prefs->suppress_default_browser_prompt_for_version
           .empty()) {
    data->local_state->SetString(
        prefs::kBrowserSuppressDefaultBrowserPrompt,
        data->master_prefs->suppress_default_browser_prompt_for_version);
  }

#if defined(OS_WIN)
  if (!data->master_prefs->welcome_page_on_os_upgrade_enabled)
    data->local_state->SetBoolean(prefs::kWelcomePageOnOSUpgradeEnabled, false);
#endif
...
void SetupFieldTrials(LocalStateData* data) {
  auto client = std::make_unique<ChromeMetricsServicesManagerClient>(
      data->local_state.get());
  data->metrics_services_manager_client = client.get();
  data->metrics_services_manager =
      std::make_unique<metrics_services_manager::MetricsServicesManager>(
          std::move(client));

  // Initialize FieldTrialList to support FieldTrials that use one-time
  // randomization.
  data->field_trial_list = std::make_unique<base::FieldTrialList>(
      data->metrics_services_manager->CreateEntropyProvider());

  std::unique_ptr<base::FeatureList> feature_list =
      std::make_unique<base::FeatureList>();

  // Associate parameters chosen in about:flags and create trial/group for them.
  flags_ui::PrefServiceFlagsStorage flags_storage(data->local_state.get());
  ...
  variations::VariationsService* variations_service =
      data->metrics_services_manager->GetVariationsService();
  data->browser_field_trials = std::make_unique<ChromeBrowserFieldTrials>();
  variations_service->SetupFieldTrials(
      ...
      data->browser_field_trials.get());

  // See comment in here for lifetime.
  data->field_trial_synchronizer = new FieldTrialSynchronizer();
}

}  // namespace

LocalStateData::LocalStateData() = default;
...
int LoadLocalState(LocalStateData* data) {
  ...
  data->local_state_task_runner = base::CreateSequencedTaskRunnerWithTraits(
      {base::MayBlock(), base::TaskPriority::USER_VISIBLE,
       base::TaskShutdownBehavior::BLOCK_SHUTDOWN});

  internal::CreateLocalState(
      data->local_state_task_runner, &data->pref_service_factory,
      &data->browser_policy_connector, &data->local_state);

  InitializeLocalState(data);

  ConvertFlagsToSwitches(data->local_state.get());

  data->local_state->UpdateCommandLinePrefStore(
      new ChromeCommandLinePrefStore(base::CommandLine::ForCurrentProcess()));

  const int apply_first_run_result = ApplyFirstRunPrefs(user_data_dir, data);
  if (apply_first_run_result != content::RESULT_CODE_NORMAL_EXIT)
    return apply_first_run_result;

  SetupOriginTrialsCommandLine(data->local_state.get());
