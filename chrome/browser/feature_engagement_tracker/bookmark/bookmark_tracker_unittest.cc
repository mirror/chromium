...
// If OnVisitedKnownURL() is called, the FeatureEngagementTracker
// receives the kVisitedKnownURL event.
TEST_F(BookmarkTrackerEventTest, TestOnOmniboxNavigation) {
  EXPECT_CALL(*mock_feature_tracker_, NotifyEvent(events::kVisitedKnownURL));
...
class BookmarkTrackerFeatureEngagementTrackerTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    bookmark_params["event_visited_known_url"] =
        "name:visited_known_url;comparator:>=2;window:3650;storage:3650";
...
// Test that a promo is not shown if the user has a Bookmark.
// If OnBookmarkAdded() is called, the ShouldShowPromo() should return false.
...
// Test that a promo is shown if the session time is met and a navigation to a
// known URL occurs. If OnSessionTimeMet()are called, ShouldShowPromo() should
// return true.
TEST_F(BookmarkTrackerFeatureEngagementTrackerTest, TestShouldShowPromo) {
  EXPECT_FALSE(bookmark_tracker_->ShouldShowPromo());
...
TEST_F(BookmarkTrackerFeatureEngagementTrackerTest, TestPrefs) {
  ...
  boomark_tracker_->OnSessionTimeMet();
  bookmark_tracker_->OnVisitedKnownURL();
