...
// A promo is shown if the session time is met and an omnibox navigation occurs.
...
TEST_F(NewTabFeatureEngagementTrackerTest, TestIncognitoProfile) {
  TestingProfile* profile = manager()->CreateTestingProfile("profile_6");
  ASSERT_TRUE(profile);
  ...
  auto feature_tracker = base::MakeUnique<
      feature_engagement_tracker::FakeFeatureEngagementTracker>();
  auto instance = base::MakeUnique<FakeNewTabFeatureEngagementTracker>(
      incognito_profile, feature_tracker.release());
  instance->RegisterPrefs();
  ...
  metrics::DesktopSessionDurationTracker::Get()->RemoveObserver(
      instance.release());
  manager()->DeleteTestingProfile("profile_6");
}
