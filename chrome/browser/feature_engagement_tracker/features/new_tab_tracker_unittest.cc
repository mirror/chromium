...
static const char kProfileName[] = "profile";
 ...
 public:
  ...
  void SetUp() override {
    ...
    new_tab_tracker_ = base::MakeUnique<FakeNewTabTracker>(
        profile, mock_feature_tracker_.get());
  }
...
TEST_F(NewTabTrackerEventTest, TestNotifyNewTabOpened) {
  ...
  // NotifyNewTabOpened does not expect a ShouldTriggerHelpUI() call since
  // the promo cannot be shown if the user uses a new tab.
...
TEST_F(NewTabTrackerEventTest, TestNotifyOmniboxNavigation) {
  ...
  EXPECT_CALL(*mock_feature_tracker_,
              ShouldTriggerHelpUI(
                  IsFeature(feature_engagement_tracker::kIPHNewTabFeature)));
