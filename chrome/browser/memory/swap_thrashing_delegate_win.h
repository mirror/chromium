...
class SwapThrashingMonitorDelegateWin {
 ...
 protected:
  ...
  class PageFaultDetectionWindow {
   public:
    ...
    PageFaultDetectionWindow(base::TimeDelta window_length,
                             PageFaultObservation initial_observation);
    virtual ~PageFaultDetectionWindow();

    // Reset the window to the values provided.
  ...
  // Reset the observation/cooldown windows.
  //
  ...
  // The detection window to detect the changes to a higher thrashing state.
  std::unique_ptr<PageFaultDetectionWindow> swap_thrashing_detection_window_;

  // The cooldown detection window to detect the changes to a lower thrashing
  // state.
  ...
  PageFaultObservation last_observation_;
