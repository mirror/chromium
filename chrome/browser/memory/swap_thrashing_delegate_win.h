 ...
 protected:
  ...
  class PageFaultDetectionWindow {
   public:
    ...
    //
    // TODO(sebmarchand): Store these samples in a deque and implement sliding
    // windows rather than having a fixed value for the window beginning.
    ...
    base::TimeDelta window_length() { return window_length_; }
   ...
   protected:
    ...
    base::TimeTicks window_beginning_;
  ...
  // Reset the observation/cooldown window to the values provided.
  //
  // These functions are virtual for unittesting purposes.
  virtual void ResetDetectionWindow(base::TimeDelta window_length,
                                    uint64_t page_fault_count);
  ...
  base::TimeTicks previous_observation_timestamp_;
