...
class MockSampleWindow;

class TestSwapThrashingMonitorDelegateWin
    : public SwapThrashingMonitorDelegateWin {
 public:
  using SampleWindow = SwapThrashingMonitorDelegateWin::SampleWindow;
  ...
  bool RecordHardFaultCountForChromeProcesses() override {
    samples_window_->OnObservation(observation_override_);
    return true;
  }
  ...
  MockSampleWindow* sample_window() { return mock_window_; }

 private:
  uint64_t observation_override_;

  MockSampleWindow* mock_window_;
};

// A mock for the SampleWindow class.
//
// Used to access some protected fields of the SampleWindow class.
class MockSampleWindow
    : public TestSwapThrashingMonitorDelegateWin::SampleWindow {
 public:
  MockSampleWindow() : TestSwapThrashingMonitorDelegateWin::SampleWindow() {}
  ~MockSampleWindow() override {}

  size_t SampleCount() { return observation_deltas_.size(); }

  size_t WindowLength() { return kSampleCount; }
};

void TestSwapThrashingMonitorDelegateWin::SetMockWindow() {
  std::unique_ptr<MockSampleWindow> mock_window =
      base::MakeUnique<MockSampleWindow>();
  mock_window_ = mock_window.get();
  samples_window_.reset(mock_window.release());
}
