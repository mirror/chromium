...
TEST_F(MemoryKillsMonitorTest, TestHistograms) {
  ...
  base::PlatformThreadId tid1 =
      MemoryKillsMonitor::Get()->non_joinable_worker_thread_->tid();
  MemoryKillsMonitor::Get()->StartMonitoring();
  base::PlatformThreadId tid2 =
      MemoryKillsMonitor::Get()->non_joinable_worker_thread_->tid();
  EXPECT_EQ(tid1, tid2);

  // Adds a small delay before checking histograms again.
...
/*
TEST_F(MemoryKillsMonitorTest, StartMonitoringMultipleTimes) {
  std::unique_ptr<base::StatisticsRecorder> statistic_recorder(
      base::StatisticsRecorder::CreateTemporaryForTesting());
  MemoryKillsMonitor::StartMonitoring();
  base::PlatformThreadId tid1 =
      MemoryKillsMonitor::non_joinable_worker_thread_->tid();
  MemoryKillsMonitor::StartMonitoring();
  base::PlatformThreadId tid2 =
      MemoryKillsMonitor::non_joinable_worker_thread_->tid();
  // Ensure we didn't create multiple threads.
  EXPECT_EQ(tid1, tid2);
...

  {
    ...
    auto samples = histogram_count->SnapshotSamples();
