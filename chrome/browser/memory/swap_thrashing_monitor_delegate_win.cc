...
SwapThrashingMonitorDelegateWin::SwapThrashingMonitorDelegateWin()
    : samples_window_(base::MakeUnique<SampleWindow>(kWindowSampleCount)) {}
...
void SwapThrashingMonitorDelegateWin::SampleWindow::OnObservation(
  ...
  observation_deltas_.push_back(sample - last_observation_);
  last_observation_ = sample;
}

size_t
SwapThrashingMonitorDelegateWin::SampleWindow::NumberOfSamplesAboveThreshold(
    const size_t threshold) const {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

  size_t count = 0;
  for (const auto& iter : observation_deltas_) {
    if (iter >= threshold)
      ++count;
  }
...
SwapThrashingMonitorDelegateWin::SampleAndCalculateSwapThrashingLevel() {
  ...
  size_t sample_above_threshold =
      samples_window_->NumberOfSamplesAboveThreshold(kHighSwappingThreshold);
...
bool SwapThrashingMonitorDelegateWin::RecordHardFaultCountForChromeProcesses() {
  ...
  if (samples_window_->observation_deltas_.size() > 0)
    LOG(ERROR) << static_cast<size_t>(level) << " | "
               << samples_window_->observation_deltas_.back() << " | "
