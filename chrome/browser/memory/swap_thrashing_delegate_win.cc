...
// TODO(sebmarchand): Confirm that this value is appropriate.
constexpr base::TimeDelta kSuspectedIntervalCoolDown =
...
// SWAP_THRASHING_LEVEL_SUSPECTED level.
// TODO(sebmarchand): Confirm that this value is appropriate.
constexpr base::TimeDelta kConfirmedCoolDown =
...
SwapThrashingMonitorDelegateWin::SwapThrashingMonitorDelegateWin()
  ...
  swap_thrashing_detection_window_ = base::MakeUnique<PageFaultDetectionWindow>(
      kNoneToSuspectedInterval, previous_observation_);
...
SwapThrashingMonitorDelegateWin::PageFaultDetectionWindow::
    ...
    : window_length_(window_length),
      ...
      window_beginning_(base::TimeTicks::Now()),
...
void SwapThrashingMonitorDelegateWin::PageFaultDetectionWindow::OnObservation(
  ...
  // No need to record this observation if the window hasn't yet expired.
  if (!IsReady())
    return;
...
bool SwapThrashingMonitorDelegateWin::PageFaultDetectionWindow::IsReady()
    const {
  if ((base::TimeTicks::Now() - window_beginning_) >= window_length_)
    return true;
  return false;
}
...
SwapThrashingMonitorDelegateWin::CalculateCurrentSwapThrashingLevel() {
  ...
  // Check if the observation window has expired.
  if (swap_thrashing_detection_window_ &&
      swap_thrashing_detection_window_->IsReady()) {
    // Check if the system should transition to a higher state, otherwise
    // reset the window to the most recent observation.
    if (swap_thrashing_detection_window_->average_page_fault_rate() >=
        kThrashSwappingPageFaultThreshold) {
      transition_to_higher_state = true;
    } else {
      ResetDetectionWindow(swap_thrashing_detection_window_->window_length(),
  ...
  if (swap_thrashing_cooldown_window_ &&
    ...
    // Check if the system should transition to a lower state, otherwise
    // reset the window to the most recent observation.
    if (swap_thrashing_cooldown_window_->average_page_fault_rate() <=
        kPageFaultCooldownThreshold) {
      transition_to_lower_state = true;
    } else {
      ResetDetectionWindow(swap_thrashing_cooldown_window_->window_length(),
                           previous_observation_);
    }
  }
  ...
  switch (last_thrashing_state_) {
    case SwapThrashingLevel::SWAP_THRASHING_LEVEL_NONE: {
      if (transition_to_higher_state) {
        ResetDetectionWindow(kSuspectedToConfirmedInterval,
                             previous_observation_);
        ResetCooldownWindow(kSuspectedIntervalCoolDown, previous_observation_);
    ...
    case SwapThrashingLevel::SWAP_THRASHING_LEVEL_SUSPECTED: {
      if (transition_to_higher_state) {
        ...
        ResetCooldownWindow(kConfirmedCoolDown, previous_observation_);
        last_thrashing_state_ =
            SwapThrashingLevel::SWAP_THRASHING_LEVEL_CONFIRMED;
      } else if (transition_to_lower_state) {
        ResetDetectionWindow(kNoneToSuspectedInterval, previous_observation_);
        swap_thrashing_cooldown_window_.reset(nullptr);
        last_thrashing_state_ = SwapThrashingLevel::SWAP_THRASHING_LEVEL_NONE;
      }
      break;
...
void SwapThrashingMonitorDelegateWin::ResetDetectionWindow(
    ...
    uint64_t page_fault_count) {
  swap_thrashing_detection_window_ = base::MakeUnique<PageFaultDetectionWindow>(
      window_length, page_fault_count);
}
