...
SwapThrashingMonitorDelegateWin::SwapThrashingMonitorDelegateWin()
  ...
  // suspected state.
  ResetDetectionWindow(kNoneToSuspectedInterval);
...
SwapThrashingMonitorDelegateWin::PageFaultDetectionWindow::
  ...
  Reset(window_length, initial_observation);
}
...
void SwapThrashingMonitorDelegateWin::PageFaultDetectionWindow::Reset(
  ...
  observations_.clear();
  window_length_ = window_length;
  observations_.push_back(initial_observation);
}

void SwapThrashingMonitorDelegateWin::PageFaultDetectionWindow::OnObservation(
    PageFaultObservation observation) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  DCHECK_GE(observations_.size(), 1U);
  observations_.push_back(observation);

  // No need to remove any observation if there's only 2 of them.
  ...
  // time window, if none if found then |last_iterator_to_keep| will point to
...
SwapThrashingMonitorDelegateWin::CalculateCurrentSwapThrashingLevel() {
  ...
  if (swap_thrashing_detection_window_) {
    swap_thrashing_detection_window_->OnObservation(last_observation_);
  }
  if (swap_thrashing_cooldown_window_) {
    swap_thrashing_cooldown_window_->OnObservation(last_observation_);
  }
  ...
  if (swap_thrashing_detection_window_) {
    ...
    if (swap_thrashing_detection_window_->AveragePageFaultRate(&average_rate)) {
      transition_to_higher_state =
          average_rate >= kThrashSwappingPageFaultThreshold;
    }
  }
  ...
  switch (last_thrashing_state_) {
    case SwapThrashingLevel::SWAP_THRASHING_LEVEL_NONE: {
      if (transition_to_higher_state) {
        ...
        ResetCooldownWindow(kSuspectedIntervalCoolDown);
    ...
    case SwapThrashingLevel::SWAP_THRASHING_LEVEL_SUSPECTED: {
      if (transition_to_higher_state) {
        ...
        ResetCooldownWindow(kConfirmedCoolDown);
        last_thrashing_state_ =
...
bool SwapThrashingMonitorDelegateWin::GetChromeHardPageFaultCounter() {
  ...
  last_observation_ = {hard_fault_count, base::TimeTicks::Now()};
  return true;
}
...
void SwapThrashingMonitorDelegateWin::ResetDetectionWindow(
  ...
  if (!swap_thrashing_detection_window_) {
    ...
    swap_thrashing_detection_window_->Reset(window_length, last_observation_);
  }
}
...
void SwapThrashingMonitorDelegateWin::ResetCooldownWindow(
  ...
  if (!swap_thrashing_cooldown_window_) {
    ...
    swap_thrashing_cooldown_window_->Reset(window_length, last_observation_);
  }
}
