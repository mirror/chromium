...
class MemoryKillsMonitor : public base::DelegateSimpleThread::Delegate,
 ...
 private:
  ...
  FRIEND_TEST_ALL_PREFIXES(MemoryKillsMonitorTest, LogLowMemoryKill);

  // For unit test only to get the global instance.
  static MemoryKillsMonitor* Get();
  ...
  // The last time a low memory kill happens.
  base::Time last_low_memory_kill_time_;
  // The number of low memory kills since monitoring is started.
  int low_memory_kills_count_;

  // The last time an OOM kill happens.
