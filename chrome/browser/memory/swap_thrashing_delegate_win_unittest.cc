...
class TestSwapThrashingMonitorDelegateWin
 ...
 public:
  ...
  bool GetChromeHardPageFaultCounter() override {
    last_observation_ = {0, base::TimeTicks::Now()};
    return true;
  }

  // Override these functions to make this detector use mock detection windows.
  void ResetDetectionWindow(base::TimeDelta window_length) override;
...
class MockPageFaultDetectionWindow
 ...
 public:
  ...
  void OnObservation(PageFaultObservation observation) override {
    TestSwapThrashingMonitorDelegateWin::PageFaultDetectionWindow::
        OnObservation(observation_override_);
  }

  void Invalidate() { invalidated_ = true; }

  bool AveragePageFaultRate(double* average_rate) const override {
    if (invalidated_)
      return false;
    return TestSwapThrashingMonitorDelegateWin::PageFaultDetectionWindow::
        AveragePageFaultRate(average_rate);
  }
  ...
  const std::list<PageFaultObservation>& observations() {
...
class SwapThrashingMonitorDelegateWinTest : public testing::Test {
 public:
  void SetUp() override {
    mock_detection_window_ = nullptr;
    mock_cooldown_window_ = nullptr;
  }

  void TearDown() override {}

  void ResetDetectionWindows(bool enable_detection_window,
                             bool enable_cooldown_window) {
    // Reset the windows to turn it into mock ones.
    delegate_.ResetDetectionWindow(base::TimeDelta());
    mock_detection_window_ = delegate_.detection_window();
    delegate_.ResetCooldownWindow(base::TimeDelta());
    mock_cooldown_window_ = delegate_.cooldown_window();
    if (!enable_detection_window)
      mock_detection_window_->Invalidate();
    if (!enable_cooldown_window)
      mock_cooldown_window_->Invalidate();
  }

 protected:
  TestSwapThrashingMonitorDelegateWin delegate_;

  MockPageFaultDetectionWindow* mock_detection_window_;
  MockPageFaultDetectionWindow* mock_cooldown_window_;
};
...
TEST_F(SwapThrashingMonitorDelegateWinTest, StateTransition) {
  ...
  ResetDetectionWindows(true, false);
  ...
  PageFaultObservation next_observation = {
      0, mock_detection_window_->GetEndOfWindowTimestamp() -
             base::TimeDelta::FromMilliseconds(1)};
  mock_detection_window_->OverrideNextObservation(next_observation);
  ...
  next_observation.timestamp =
      mock_detection_window_->GetEndOfWindowTimestamp();
  // Set the thrashing level to a rate that is inferior to the state change
  // threshold.
  next_observation.page_fault_count =
      delegate_.ComputeHighThrashingRate(
          mock_detection_window_->window_beginning(),
          next_observation.timestamp) -
      1;
  mock_detection_window_->OverrideNextObservation(next_observation);
  EXPECT_EQ(SwapThrashingLevel::SWAP_THRASHING_LEVEL_NONE,
  ...
  next_observation.page_fault_count = delegate_.ComputeHighThrashingRate(
      mock_detection_window_->window_beginning(), next_observation.timestamp);
  mock_detection_window_->OverrideNextObservation(next_observation);
  ...
  // change.
  ResetDetectionWindows(false, true);
...
TEST_F(SwapThrashingMonitorDelegateWinTest, PageFaultDetectionWindowTests) {
  ...
  detection_window.Reset(kObservationWindowLength, first_observation);

  EXPECT_EQ(1U, detection_window.observations().size());
  ...
  detection_window.AddObservation(second_observation);
  // It shouldn't be able to compute the average as the window hasn't expired
  // yet.
  double average = 0;
  EXPECT_FALSE(detection_window.AveragePageFaultRate(&average));

  EXPECT_EQ(2U, detection_window.observations().size());
  ...
  detection_window.AddObservation(third_observation);
  EXPECT_TRUE(detection_window.AveragePageFaultRate(&average));
  ...
  EXPECT_EQ(expected_rate, average);

  EXPECT_EQ(3U, detection_window.observations().size());
  ...
  detection_window.AddObservation(fourth_observation);
  EXPECT_TRUE(detection_window.AveragePageFaultRate(&average));
  ...
  detection_window.AddObservation(fifth_observation);
  EXPECT_TRUE(detection_window.AveragePageFaultRate(&average));
  expected_rate =
