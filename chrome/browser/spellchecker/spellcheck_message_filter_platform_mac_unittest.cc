...
class SpellCheckHostChromeImplMacTest : public ::testing::Test {
 protected:
  CombineResults(std::vector<SpellCheckResult>* remote_results,
                 const std::vector<SpellCheckResult>& local_results) const {
    SpellCheckHostChromeImpl::CombineResults(remote_results, local_results);
  }
};

TEST(SpellCheckHostChromeImplMacTest, CombineResults) {
