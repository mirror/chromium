...
// TODO(grt): DetectOutdatedInstall should be rewritten. It is currently polled
// to ask, in essence, "is it party time?" Since "party time" is a fixed point
// in time, it's much better to ask "what time is it?" followed by "wake me up
// after time interval D." When this is done, detect_upgrade_timer_ can be
// rejiggered so that it runs DetectUpgradeTask on blocking_task_runner_. It
// need only touch the UI thread when it has detected a pending update.
...
// Same as kNotifyCycleDelta but only used during testing.
constexpr base::TimeDelta kNotifyCycleDeltaForTesting =
    base::TimeDelta::FromMilliseconds(500);  // Half a second.
...
UpgradeDetectorImpl::~UpgradeDetectorImpl() {
  ...
  variations::VariationsService* const variations_service =
...
void UpgradeDetectorImpl::StartUpgradeNotificationTimer() {
  ...
  // and stop the timer.
  const base::TimeDelta cycle_time =
      IsTesting() ? kNotifyCycleDeltaForTesting : kNotifyCycleDelta;
  upgrade_notification_timer_.Start(FROM_HERE, cycle_time, this,
