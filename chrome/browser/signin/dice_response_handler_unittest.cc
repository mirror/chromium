...
class DiceResponseHandlerTest : public testing::Test {
 protected:
  DiceResponseHandlerTest()
      : loop_(base::MessageLoop::TYPE_IO),  // URLRequestContext requires IO.
        ...
        signin_manager_(&signin_client_,
                        ...
                        nullptr),
        dice_response_handler_(&signin_manager_,
                               &signin_client_,
                               &token_service_,
                               &account_tracker_service_) {
  ...
  DiceResponseParams MakeDiceParams(DiceAction action) {
    ...
    dice_params.gaia_id.push_back(kGaiaID);
    dice_params.email.push_back(kEmail);
    dice_params.session_index.push_back(kSessionIndex);
...
TEST_F(DiceResponseHandlerTest, Signin) {
  ...
  ASSERT_FALSE(token_service_.RefreshTokenIsAvailable(dice_params.gaia_id[0]));
  ...
  EXPECT_TRUE(token_service_.RefreshTokenIsAvailable(dice_params.gaia_id[0]));
}
...
TEST_F(DiceResponseHandlerTest, SigninFailure) {
  ...
  EXPECT_FALSE(token_service_.RefreshTokenIsAvailable(dice_params.gaia_id[0]));
}
...
TEST_F(DiceResponseHandlerTest, SigninWithTwoAccounts) {
  ...
  dice_params_2.email[0] = "other_email";
  dice_params_2.gaia_id[0] = "other_gaia_id";
  ASSERT_FALSE(
      token_service_.RefreshTokenIsAvailable(dice_params_1.gaia_id[0]));
  ASSERT_FALSE(
      token_service_.RefreshTokenIsAvailable(dice_params_2.gaia_id[0]));
  // Start first request.
  ...
  consumer_1->OnClientOAuthSuccess(
      GaiaAuthConsumer::ClientOAuthResult("refresh_token", "access_token", 10));
  // Check that the token has been inserted in the token service.
  EXPECT_TRUE(token_service_.RefreshTokenIsAvailable(dice_params_1.gaia_id[0]));
  // Simulate GaiaAuthFetcher success for the second request.
  consumer_2->OnClientOAuthSuccess(
      GaiaAuthConsumer::ClientOAuthResult("refresh_token", "access_token", 10));
  // Check that the token has been inserted in the token service.
  EXPECT_TRUE(token_service_.RefreshTokenIsAvailable(dice_params_2.gaia_id[0]));
}
...
TEST_F(DiceResponseHandlerTest, SignoutMainAccount) {
  ...
  // account.
  signin_manager_.SignIn(dice_params.gaia_id[0], dice_params.email[0],
                         "password");
  token_service_.UpdateCredentials(dice_params.gaia_id[0], "token1");
...
TEST_F(DiceResponseHandlerTest, SignoutWebOnly) {
  ...
  // accounts.
  token_service_.UpdateCredentials(dice_params.gaia_id[0], "refresh_token");
