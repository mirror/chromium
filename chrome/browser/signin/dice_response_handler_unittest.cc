...
class ProcessDelegate : public ProcessDiceHeaderDelegate {
 public:
  static std::unique_ptr<ProcessDelegate> Create(
      DiceResponseHandlerTest* owner) {
    return base::MakeUnique<ProcessDelegate>(owner);
  }
  explicit ProcessDelegate(DiceResponseHandlerTest* owner) : owner_(owner) {}
  ~ProcessDelegate() override = default;
