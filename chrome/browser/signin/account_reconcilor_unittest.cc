...
void AccountReconcilorTest::SetUp() {
  ...
  // On ChromeOS, credentials are not loaded by default.
...
// Tests that the AccountReconcilor is started when Dice is enabled.
...
// lost.
TEST_F(AccountReconcilorTest, DiceCachedFirstAccount) {
  // Enable Dice.
  signin::ScopedAccountConsistencyDice scoped_dice;
  ...
  // Do one reconcile. It should do nothing but to populating the cache.
  {
    EXPECT_CALL(*GetMockReconcilor(), PerformMergeAction(testing::_)).Times(0);
    EXPECT_CALL(*GetMockReconcilor(), PerformLogoutAllAccountsAction())
        .Times(0);
