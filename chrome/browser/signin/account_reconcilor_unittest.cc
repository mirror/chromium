...
const AccountReconcilorTestDiceParam kDiceParams[] = {
    ...
    //   A, B, C: Merge account.
    // -------------------------------------------------------------------------
    // Tokens | Cookies | First Run | Gaia calls | Cookies after | Tokens after
    // -------------------------------------------------------------------------
    ...
    {  "*xAB",  "AB",     true,       "X",         "",             "*xA"},
    {  "*xAB",  "BA",     true,       "XB",        "B",            "*xAB"},
    {  "*xAB",  "A",      true,       "X",         "",             "*xA"},
    {  "*xAB",  "B",      true,       "",          "B",            "*xAB"},
    {  "*xAB",  "",       true,       "B",         "B",            "*xAB"},
    // Sync enabled, token error on secondary.
    {  "*AxB",  "AB",     true,       "XA",        "A",            "*AxB"},
    {  "*AxB",  "BA",     true,       "XA",        "A",            "*AxB"},
    {  "*AxB",  "A",      true,       "",          "A",            "*AxB"},
    {  "*AxB",  "B",      true,       "XA",        "A",            "*AxB"},
    {  "*AxB",  "",       true,       "A",         "A",            "*AxB"},
...
class AccountReconcilorTestDice
 ...
 protected:
  ...
  // Checks that the tokens in the TokenService match the token string.
  void VerifyCurrentTokens(const char* token_string) {
    struct Token {
      std::string id;
    ...
    for (int i = 0; token_string[i] != '\0'; ++i) {
      ...
      parsed_tokens.push_back(
          {PickAccountIdForAccount(accounts_[token_code].gaia_id,
                                   accounts_[token_code].email),
           is_authenticated, has_error});
      is_authenticated = false;
      has_error = false;
    }

    // Compare parsed_tokens with the contents of the token service.
    EXPECT_EQ(token_service()->GetAccounts().size(), parsed_tokens.size());
    bool authenticated_account_found = false;
    for (const Token& token : parsed_tokens) {
...
TEST_P(AccountReconcilorTestDice, TableRowTest) {
  ...
  VerifyCurrentTokens(GetParam().tokens);
  ...
  VerifyCurrentTokens(GetParam().tokens_after_reconcile);
