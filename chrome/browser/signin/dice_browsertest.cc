...
std::unique_ptr<HttpResponse> HandleSignoutURL(const HttpRequest& request) {
  ...
  EXPECT_GE(signout_type, kAllAccounts);
  EXPECT_LE(signout_type, kSecondaryAccount);
...
class DiceBrowserTest : public InProcessBrowserTest,
 ...
 protected:
  ...
  // Signin with a main account and add tokens for a secondary account.
  ...
  // account corresponding to kMainEmail, kMainGaiaID.
