...
TEST_F(MutableProfileOAuth2TokenServiceDelegateTest,
  ...
  if (account_tracker_service_.GetMigrationState() !=
    ...
    update->Clear();
    auto dict = base::MakeUnique<base::DictionaryValue>();
    dict->SetKey("account_id", base::Value(base::UTF8ToUTF16(email1)));
    dict->SetKey("email", base::Value(base::UTF8ToUTF16(email1)));
    dict->SetKey("gaia", base::Value(base::UTF8ToUTF16(gaia_id1)));
    update->Append(std::move(dict));
    dict = base::MakeUnique<base::DictionaryValue>();
    dict->SetKey("account_id", base::Value(base::UTF8ToUTF16(email2)));
    dict->SetKey("email", base::Value(base::UTF8ToUTF16(email2)));
    dict->SetKey("gaia", base::Value(base::UTF8ToUTF16(gaia_id2)));
    update->Append(std::move(dict));
    account_tracker_service_.Shutdown();
    account_tracker_service_.Initialize(client_.get());
