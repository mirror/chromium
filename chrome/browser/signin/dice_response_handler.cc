...
enum DiceResponseHeader {
  ...
  kSignin = 0,
  // Received a signout header including the main account.
  kSignoutMain = 1,
  // Received a signout header for other account(s).
  kSignoutOther = 2,

  kDiceResponseHeaderMax
};
...
void DiceResponseHandler::DiceTokenFetcher::OnTimeout() {
  UMA_HISTOGRAM_ENUMERATION(kDiceTokenFetchResultHistogram, kFetchTimeout,
                            kDiceTokenFetchResultMax);
...
void DiceResponseHandler::DiceTokenFetcher::OnClientOAuthSuccess(
  ...
  UMA_HISTOGRAM_ENUMERATION(kDiceTokenFetchResultHistogram, kFetchSuccess,
                            kDiceTokenFetchResultMax);
  gaia_auth_fetcher_.reset();
  timeout_closure_.Cancel();
...
void DiceResponseHandler::DiceTokenFetcher::OnClientOAuthFailure(
  ...
  UMA_HISTOGRAM_ENUMERATION(kDiceTokenFetchResultHistogram, kFetchFailure,
                            kDiceTokenFetchResultMax);
  gaia_auth_fetcher_.reset();
  timeout_closure_.Cancel();
...
void DiceResponseHandler::ProcessDiceSigninHeader(
  ...
  UMA_HISTOGRAM_ENUMERATION(kDiceResponseHeaderHistogram, kSignin,
                            kDiceResponseHeaderMax);

  if (!CanGetTokenForAccount(gaia_id, email)) {
    UMA_HISTOGRAM_ENUMERATION(kDiceTokenFetchResultHistogram, kFetchAbort,
                              kDiceTokenFetchResultMax);
    return;
  }
...
void DiceResponseHandler::ProcessDiceSignoutHeader(
  ...
  for (unsigned int i = 0; i < gaia_ids.size(); ++i) {
    ...
    if (signed_out_account == current_account) {
      ...
      UMA_HISTOGRAM_ENUMERATION(kDiceResponseHeaderHistogram, kSignoutMain,
                                kDiceResponseHeaderMax);
  ...
  UMA_HISTOGRAM_ENUMERATION(kDiceResponseHeaderHistogram, kSignoutOther,
                            kDiceResponseHeaderMax);
