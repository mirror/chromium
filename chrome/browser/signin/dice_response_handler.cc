...
void DiceResponseHandler::ProcessDiceSignoutHeader(
  ...
  std::string chrome_primary_account =
      signin_manager_->GetAuthenticatedAccountId();
  std::vector<std::string> gaia_secondary_accounts;
  for (const auto& account_info : account_infos) {
    ...
    if (signed_out_account == chrome_primary_account) {
      ...
      gaia_secondary_accounts.push_back(signed_out_account);
    }
  }

  if (signin::IsDiceEnabledForProfile(signin_client_->GetPrefs()) ||
      gaia_secondary_accounts.size() == account_infos.size()) {
    RecordDiceResponseHeader(kSignoutSecondary);
    RecordGaiaSignoutMetrics(chrome_primary_account.empty()
                                 ? kNoChromePrimaryAccount
                                 : kChromePrimaryAccountIsNotInGaiaAccounts);
  }

  for (const auto& account : gaia_secondary_accounts) {
