...
enum ChromeAccountStateInGaiaCookies {
  ...
  // The user is authenticated in Chrome with the first web account being signed
  // out.
  kChromeAccountIsFirstGaiaAccount = 1,

  // The user is authenticated in Chrome with another web account being signed
  // out.
  kChromeAccountIsOtherGaiaAccount = 2,

  // The user is authenticated in Chrome with an account that was not signed in
  // on the web
...
void RecordGaiaSignoutMetrics(
  ...
  ChromeAccountStateInGaiaCookies signout_type = kNoChromeAccount;

  if (!chrome_primary_account.empty()) {
    signout_type = kChromeAccountIsNotInGaiaAccounts;
    ...
    for (const auto& account_info : account_infos) {
      ...
      if (signed_out_account == chrome_primary_account) {
        if (account_info.session_index == 0)
          signout_type = kChromeAccountIsFirstGaiaAccount;
        else
          signout_type = kChromeAccountIsOtherGaiaAccount;
        break;
      }
    }
  }

  UMA_HISTOGRAM_ENUMERATION(kChromeAccountStateOnWebSignoutHistogram,
                            signout_type,
                            kChromeAccountStateInGaiaCookiesCount);
}
