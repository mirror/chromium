 ...
 public:
  ...
  void CreateRequestWithResponseHeaders() {
    ...
    content::BrowserThread::PostTask(
        ...
        base::Bind(&TestResponseHelper::CreateRequestWithDiceResponseHeaderOnIO,
    ...
    content::BrowserThread::PostTask(
        content::BrowserThread::IO, FROM_HERE,
        base::Bind(&TestResponseHelper::CheckHeaderSetOnIO,
                   base::Unretained(this)));
    base::RunLoop().RunUntilIdle();
    ASSERT_TRUE(success_);
  }
  ...
  void CheckDiceHeaderRemoved() {
    ...
    content::BrowserThread::PostTask(
        content::BrowserThread::IO, FROM_HERE,
        base::Bind(&TestResponseHelper::CheckDiceHeaderRemovedOnIO,
                   base::Unretained(this)));
    base::RunLoop().RunUntilIdle();
    ASSERT_TRUE(success_);
  }
...
class ChromeSigninHelperTest : public testing::Test {
 protected:
  ...
  TestResponseHelper test_response_helper_;
};
...
TEST_F(ChromeSigninHelperTest, RemoveDiceSigninHeader) {
  ...
  test_response_helper_.CreateRequestWithResponseHeaders();
  test_response_helper_.CheckDiceHeaderRemoved();
}
#endif
