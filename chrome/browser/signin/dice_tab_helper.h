...
class DiceTabHelper : public content::WebContentsUserData<DiceTabHelper>,
 ...
 public:
  ...
  // Starts syncing with the given account. Assumes that there is a refresh
  // token available for the given account.
  void StartSyncIfNeeded(const std::string& gaia_id, const std::string& email);
