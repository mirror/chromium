...
void FixAccountConsistencyRequestHeader(net::URLRequest* request,
  ...
  if (dice_header_added) {
    const content::ResourceRequestInfo* info =
        content::ResourceRequestInfo::ForRequest(request);
    DCHECK(info);
    if (info->GetResourceType() == kAccountConsistencyResponseType &&
        IsAccountConsistencyDiceEnabled()) {
      // Block the AccountReconcilor while the Dice request is in flight. This
      // allows the DiceReponseHandler to process the response before the
      // reconcilor starts.
    ...
    // If there is no Dice, try Mirror.
