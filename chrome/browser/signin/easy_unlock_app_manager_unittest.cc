...
class EasyUnlockAppEventConsumer
 ...
 private:
  ...
  void ConsumeAuthAttempted(base::ListValue* args) {
    if (!args) {
      ADD_FAILURE() << "No argument list for onAuthAttempted event";
      return;
    }

    if (args->GetSize() != 2u) {
      ADD_FAILURE() << "Invalid argument list size for onAuthAttempted event: "
    ...
    if (!args->GetString(0u, &auth_type)) {
      ADD_FAILURE() << "Unable to get first argument as string for "
