...
// Check that sync is not enabled if the tab is closed during signin.
TEST_F(ProcessDiceHeaderDelegateImplTest, CloseTabWhileStartingSync) {
  ...
  EXPECT_FALSE(start_sync_called_);
...
struct TestConfiguration {
  ...
  bool is_already_signed_in;
  bool is_sync_signin_tab;

  // Test expectations.
  bool start_sync_and_go_to_ntp;
  bool show_error_called;
  bool go_to_ntp_on_error;
};

TestConfiguration kTestCases[] = {
    // User was not signed in.
    {signin::AccountConsistencyMethod::kDiceFixAuthErrors, false, false, false,
     false, false},
    {signin::AccountConsistencyMethod::kDiceFixAuthErrors, false, true, false,
     false, false},
    {signin::AccountConsistencyMethod::kDicePrepareMigration, false, false,
     false, false, false},
    {signin::AccountConsistencyMethod::kDicePrepareMigration, false, true, true,
     true, true},
    {signin::AccountConsistencyMethod::kDice, false, false, false, true, false},
    {signin::AccountConsistencyMethod::kDice, false, true, true, true, true},

    // User was already signed in.
    {signin::AccountConsistencyMethod::kDiceFixAuthErrors, true, false, false,
     false, false},
    {signin::AccountConsistencyMethod::kDiceFixAuthErrors, true, false, false,
     false, false},
    {signin::AccountConsistencyMethod::kDicePrepareMigration, true, false,
     false, false, false},
    {signin::AccountConsistencyMethod::kDicePrepareMigration, true, false,
     false, false, false},
    {signin::AccountConsistencyMethod::kDice, true, false, false, true, false},
    {signin::AccountConsistencyMethod::kDice, true, true, false, true, false},
};

// Parameterized version of ProcessDiceHeaderDelegateImplTest.
class ProcessDiceHeaderDelegateImplTestWithParams
...
TEST_P(ProcessDiceHeaderDelegateImplTestWithParams, EnableSync) {
  // Setup the test.
  ...
  NavigateAndCommit(kSigninURL);
  ASSERT_EQ(kSigninURL, web_contents()->GetVisibleURL());
  if (GetParam().is_already_signed_in)
    signin_manager_.SignIn("gaia_id", "user", "pass");
  std::unique_ptr<ProcessDiceHeaderDelegateImpl> delegate =
      CreateDelegate(GetParam().is_sync_signin_tab);

  // Check expectations.
  delegate->EnableSync(account_id_);
  EXPECT_EQ(GetParam().start_sync_and_go_to_ntp, start_sync_called_);
  GURL expected_url = GetParam().go_to_ntp_on_error
...
TEST_P(ProcessDiceHeaderDelegateImplTestWithParams,
  ...
  delegate->HandleTokenExchangeFailure(email_, auth_error_);
  EXPECT_FALSE(start_sync_called_);
  EXPECT_EQ(GetParam().show_error_called, show_error_called_);
  GURL expected_url = GetParam().go_to_ntp_on_error
                          ? GURL(chrome::kChromeUINewTabURL)
                          : kSigninURL;
  EXPECT_EQ(expected_url, web_contents()->GetVisibleURL());
...
INSTANTIATE_TEST_CASE_P(Foo,
                        ProcessDiceHeaderDelegateImplTestWithParams,
