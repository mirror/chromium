...
bool EasyUnlockServiceSignin::IsAllowedInternal() const {
  return service_active_ && account_id_.is_valid() &&
         ...
         pref_manager_->IsEasyUnlockAllowed();
}
...
void EasyUnlockServiceSignin::OnFocusedUserChanged(
  ...
  // Because loading the cryptohome key data for EasyUnlock is a fairly long
  // asynchronous operation, we want to show a spinner before the
  // ProximityAuthSystem can be started. This function also takes care of
  // showing the hardlock state.
  ShowInitialUserPodState();

  // If there is a hardlock, then there is no point in loading the devices.
  EasyUnlockScreenlockStateHandler::HardlockState hardlock_state;
  if (GetPersistedHardlockState(&state) &&
      state != EasyUnlockScreenlockStateHandler::NO_HARDLOCK) {
