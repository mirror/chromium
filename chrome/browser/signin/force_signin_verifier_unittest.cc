...
class MockForceSigninVerifier : public ForceSigninVerifier {
 public:
  ...
  bool HasVerified() { return has_token_verified_; }

  bool IsDelayTaskPosted() { return backoff_request_timer_.IsRunning(); }

  int FailureCount() { return backoff_entry_.failure_count(); }

  OAuth2TokenService::Request* request() { return access_token_request_.get(); }
...
TEST_F(ForceSigninVerifierTest, OnGetTokenSuccess) {
  ...
  ASSERT_FALSE(verifier_->HasVerified());
  ...
  ASSERT_EQ(nullptr, verifier_->request());
  ASSERT_TRUE(verifier_->HasVerified());
  ASSERT_FALSE(verifier_->IsDelayTaskPosted());
