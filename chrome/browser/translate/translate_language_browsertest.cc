...
class TranslateLanguageBrowserTest : public InProcessBrowserTest {
 ...
 protected:
  ...
  void CheckForTranslateUI(const FilePath& path, bool expect_translate) {
    if (!browser_) {
      browser_ = browser();
    }
    const content::WebContents* current_web_contents =
    ...
    const GURL url = ui_test_utils::GetTestUrl(FilePath(), path);
...
IN_PROC_BROWSER_TEST_F(TranslateLanguageBrowserTest, LanguageModelLogSucceed) {
  for (int i = 0; i < 10; ++i) {
    ASSERT_NO_FATAL_FAILURE(
        CheckForTranslateUI(FilePath(kFrenchTestPath), true));
    ASSERT_NO_FATAL_FAILURE(
        CheckForTranslateUI(FilePath(kEnglishTestPath), false));
  }
  // Intentionally visit the french page one more time.
  ASSERT_NO_FATAL_FAILURE(CheckForTranslateUI(FilePath(kFrenchTestPath), true));
  ...
  EXPECT_NEAR(0.47619, langs[1].frequency, 0.001f);
}
