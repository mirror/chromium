...
class PDFExtensionClipboardTest : public PDFExtensionTest {
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    test_clipboard_ = base::MakeUnique<content::BrowserTestClipboardScope>();
  }

  void TearDownOnMainThread() override {
    test_clipboard_.reset();
 ...
 private:
  ...
  // TODO(thestig): Figure out the right way to clear the keyboard, so
  // successive calls don't need to have longer |expected| arguments each time.
  ...
  std::unique_ptr<content::BrowserTestClipboardScope> test_clipboard_;
  WebContents* guest_contents_;
};
