...
TEST_P(SubresourceFilterAbusiveTest, ConfigCombination) {
  ...
  // If either metadata is in warn state, warn.
  if (bas_level_ == METADATA_WARN || abusive_level_ == METADATA_WARN) {
    EXPECT_TRUE(allow_requests);
    EXPECT_TRUE(allow_popups);

    // We should only have site metadata for bas enforcement.
    EXPECT_FALSE(GetSettingsManager()->GetSiteMetadata(url));
  } else {
    bool bas_enabled = bas_level_ == METADATA_ENFORCE;
    EXPECT_EQ(!bas_enabled, allow_requests);
    EXPECT_EQ(abusive_level_ == METADATA_NONE, allow_popups);

    // We should only have site metadata for bas enforcement.
    EXPECT_EQ(bas_enabled, !!GetSettingsManager()->GetSiteMetadata(url));
  }
}
