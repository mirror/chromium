...
TEST_F(SubresourceFilterFailsafeTest, NoFeature_NoFailsafe) {
  ...
  auto expected_status = SubresourceFilterFailsafe::Status::kSafe;
  EXPECT_EQ(expected_status, failsafe.WillShowUI(new_tab_url, config));
...
TEST_F(SubresourceFilterFailsafeTest, ActivateNewTab_TriggersFailsafe) {
  ...
  SubresourceFilterFailsafe failsafe(profile());
  const GURL new_tab_url = search::GetNewTabPageURL(profile());
  auto expected_status = SubresourceFilterFailsafe::Status::kIsNTP;
  EXPECT_EQ(expected_status, failsafe.WillShowUI(new_tab_url, config));
...
TEST_F(SubresourceFilterFailsafeTest, ActivateChromeUrl_TriggersFailsafe) {
  ...
  EXPECT_EQ(expected_status, failsafe.WillShowUI(settings_url, config));
...
TEST_F(SubresourceFilterFailsafeTest, FailsafeWithMultipleConfigurations) {
  ...
  const GURL new_tab_url = search::GetNewTabPageURL(profile());
  auto expected_status = SubresourceFilterFailsafe::Status::kIsNTP;
  EXPECT_EQ(expected_status, failsafe.WillShowUI(new_tab_url, matching_config));
