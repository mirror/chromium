...
bool IsPersistentLicenseSupportedbyCdm(
  ...
  for (size_t i = 0; i < additional_param_names.size(); ++i) {
    if (additional_param_names[i] ==
      ...
      const base::string16& support_string16 = additional_param_values[i];
      std::string support_string;
      if (!base::UTF16ToUTF8(support_string16.c_str(),
                             support_string16.length(), &support_string)) {
        DLOG(WARNING) << "Non-UTF-8 support string.";
        return false;
      }

      return support_string == kCdmFeatureSupported;
    }
  }
