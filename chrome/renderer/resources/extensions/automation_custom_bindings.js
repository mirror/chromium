...
automationInternal.onAccessibilityEvent.addListener(function(eventParams) {
  ...
  if (eventParams.eventType == 'blur') {
    ...
    if (automationUtil.focusedNode &&
      ...
      wasFocusDispatched = privates(automationUtil.focusedNode.root)
          .impl.onAccessibilityEvent(eventParamsCopy);
    }
  }

  // Note that focus type events that were dispatched have already been handled
  // above. All other events, even non-focus events that triggered a focus
  // dispatch, still need to have their original event fired.
  if ((!wasFocusDispatched || eventParams.eventType != 'focus') &&
