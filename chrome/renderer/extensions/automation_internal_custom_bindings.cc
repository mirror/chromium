...
ui::AXNode* FindNodeWithChildTreeId(ui::AXNode* node, int child_tree_id) {
  if (child_tree_id ==
      node->data().GetIntAttribute(ax::mojom::IntAttribute::CHILD_TREE_ID))
    return node;
  ...
  RouteNodeIDFunction(
      ...
      [](v8::Isolate* isolate, v8::ReturnValue<v8::Value> result,
        ...
        bool value =
            (node->data().GetIntAttribute(ax::mojom::IntAttribute::TEXT_STYLE) &
             static_cast<int32_t>(
                 ax::mojom::TextStyle::TEXT_STYLE_LINE_THROUGH)) != 0;
        result.Set(v8::Boolean::New(isolate, value));
      });
  RouteNodeIDFunction(
      "GetCustomActions",
      [](v8::Isolate* isolate, v8::ReturnValue<v8::Value> result,
        ...
        const std::vector<int32_t>& custom_action_ids =
            node->data().GetIntListAttribute(
                ax::mojom::IntListAttribute::CUSTOM_ACTION_IDS);
...
bool AutomationInternalCustomBindings::GetFocusInternal(
  ...
  while (
      focus->data().HasIntAttribute(ax::mojom::IntAttribute::CHILD_TREE_ID)) {
...
ui::AXNode* AutomationInternalCustomBindings::GetParent(
  ...
  if ((*in_out_tree_wrapper)->host_node_id() > 0) {
    ...
    if (parent) {
      int parent_child_tree_id = parent->data().GetIntAttribute(
