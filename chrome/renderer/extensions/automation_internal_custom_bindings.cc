...
static gfx::Rect ComputeGlobalNodeBounds(
  ...
  while (node) {
    bounds = tree_wrapper->tree()->RelativeToTreeBounds(node, bounds);
...
void AutomationInternalCustomBindings::Invalidate() {
  ...
  // Delete the AutomationAXTreeWrappers quickly by first clearing their
  // delegates so we don't get a callback for every node being deleted.
  for (auto iter : tree_id_to_tree_wrapper_map_) {
    AutomationAXTreeWrapper* tree_wrapper = iter.second;
    tree_wrapper->tree()->SetDelegate(nullptr);
    delete tree_wrapper;
  }
...
bool AutomationInternalCustomBindings::GetFocusInternal(
  ...
  while (focus->data().HasIntAttribute(ui::AX_ATTR_CHILD_TREE_ID)) {
    ...
    // If the child tree_wrapper is a frame tree that indicates a focused frame,
...
ui::AXNode* AutomationInternalCustomBindings::GetParent(
  ...
  // Try to use the tree_wrapperd parent node from the most recent time this
  // was called.
  if ((*in_out_tree_wrapper)->parent_node_id_from_parent_tree() > 0) {
    ui::AXNode* parent = parent_tree_wrapper->tree()->GetFromId(
        (*in_out_tree_wrapper)->parent_node_id_from_parent_tree());
    if (parent) {
  ...
  if (parent) {
    (*in_out_tree_wrapper)->set_parent_node_id_from_parent_tree(parent->id());
    *in_out_tree_wrapper = parent_tree_wrapper;
    return parent;
  }
...
void AutomationInternalCustomBindings::OnAccessibilityEvent(
  ...
  if (iter == tree_id_to_tree_wrapper_map_.end()) {
    ...
    tree_id_to_tree_wrapper_map_.insert(std::make_pair(tree_id, tree_wrapper));
    axtree_to_tree_wrapper_map_.insert(
        std::make_pair(tree_wrapper->tree(), tree_wrapper));
  } else {
    tree_wrapper = iter->second;
  }
