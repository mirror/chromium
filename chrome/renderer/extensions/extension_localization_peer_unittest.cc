...
class MockRequestPeer : public content::RequestPeer {
 public:
  ...
  MOCK_METHOD1(
      OnCompletedRequest,
      void(const network::ResourceRequestCompletionStatus& completion_status));
...
TEST_F(ExtensionLocalizationPeerTest, OnCompletedRequestBadURLRequestStatus) {
  ...
  network::ResourceRequestCompletionStatus completion_status(net::ERR_ABORTED);
...
TEST_F(ExtensionLocalizationPeerTest, OnCompletedRequestEmptyData) {
  ...
  EXPECT_CALL(*original_peer_, OnReceivedResponse(_));
  network::ResourceRequestCompletionStatus completion_status(net::OK);
  EXPECT_CALL(*original_peer_, OnCompletedRequest(completion_status));
