...
#include "extensions/renderer/extension_hooks_delegate.h"
...
class ExtensionHooksDelegateTest : public NativeExtensionBindingsSystemUnittest {
 public:
  ...
  void SetUp() override {
    ...
    bindings_system()->api_system()->GetHooksForAPI("extension")->SetDelegate(
...
TEST_F(ExtensionHooksDelegateTest, MessagingSanityChecks) {
  ...
  SendMessageTester tester(
      ipc_message_sender(), script_context(), 0, "extension");
  ...
  tester.TestSendRequest("{data: 'hello'}", kStandardMessage,
                         self_target, SendMessageTester::CLOSED);
