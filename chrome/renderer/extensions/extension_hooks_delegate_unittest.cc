...
TEST_F(ExtensionHooksDelegateTest, SendRequestDisabled) {
  ...
  auto check_access_throws = [context](const char* object) {
    SCOPED_TRACE(object);
  ...
  check_access_throws("chrome.extension.sendRequest");
  check_access_throws("chrome.extension.onRequest");
  check_access_throws("chrome.extension.onRequestExternal");
}
