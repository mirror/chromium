...
// We alias a bunch of extension APIs to their runtime counterparts.
...
void GetAliasedFeature(
  ...
  if (!context->Global()->Get(context, gin::StringToSymbol(isolate, "chrome")).ToLocal(&chrome) ||
      !chrome->IsObject()) {
    return;
  }

  v8::Local<v8::Value> runtime;
  if (!chrome.As<v8::Object>()->Get(context, gin::StringToSymbol(isolate, "runtime")).ToLocal(&runtime) ||
      !runtime->IsObject()) {
    return;
  }
...
RequestResult ExtensionHooksDelegate::HandleSendRequest(
  ...
  if (!messaging_util::GetTarget(script_context, arguments[0], &target_id)) {
    RequestResult result(RequestResult::INVALID_INVOCATION);
    result.error = "Extension ID missing.";
    return result;
  }
...
APIBindingHooks::RequestResult ExtensionHooksDelegate::HandleGetViews(
  ...
  if (!v8_view_type->IsUndefined() && !v8_view_type->IsNull()) {
    ...
    if (v8_view_type->IsString()) {
      std::string view_type_string = gin::V8ToString(v8_view_type);
