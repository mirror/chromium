...
bool ChromeExtensionsRendererClient::MaybeRequestPDFResource(
    content::RenderFrame* navigating_frame,
    const GURL& complete_url) {
  if (complete_url.scheme() != extensions::kExtensionScheme)
    return false;

  if (complete_url.host() != extension_misc::kPdfExtensionId)
    return false;

  content::RenderFrame* parent = content::RenderFrame::FromWebFrame(
      navigating_frame->GetWebFrame()->Parent());
  CHECK(parent);

  extensions::MimeHandlerViewManager::FromRenderFrame(parent)
      ->MaybeRequestPDFResource(navigating_frame, complete_url);
  return true;
}
...
ChromeExtensionsRendererClient::GetV8ScriptableObjectForPluginFrame(
  ...
  return extensions::MimeHandlerViewManager::FromRenderFrame(
             content::RenderFrame::FromWebFrame(frame->Parent()))
      ->GetClient(original_frame_routing_id)
      ->GetV8ScriptingObject(isolate, frame->GlobalProxy());
}
