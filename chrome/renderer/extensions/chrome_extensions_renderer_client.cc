...
void ChromeExtensionsRendererClient::DidReceiveDataInPluginDocument(
    const blink::WebElement& plugin_element,
    const char* data,
    size_t length) {
  extensions::MimeHandlerViewManager::Get(
      content::RenderFrame::FromWebFrame(
          plugin_element.GetDocument().GetFrame()))
      ->DidReceiveDataInPluginDocument(plugin_element, data, length);
}

v8::Local<v8::Object> ChromeExtensionsRendererClient::V8ScriptableObject(
    const blink::WebElement& plugin_element,
    v8::Isolate* isolate) {
  return extensions::MimeHandlerViewManager::Get(
             content::RenderFrame::FromWebFrame(
                 plugin_element.GetDocument().GetFrame()))
      ->V8ScriptableObject(plugin_element, isolate);
}
