...
class AutomationAXTreeWrapper : public ui::AXEventGenerator {
 ...
 private:
  ...
  // Given an AXEvent, return true if we should send a corresponding
  // Automation event, or false if we should suppress it (perhaps because
  // AXEventGenerator will handle it instead).
  bool ShouldSendAutomationEvent(ui::AXEvent event) const;
