...
RequestResult RuntimeHooksDelegate::HandleRequest(
  ...
  static struct HandlerEntry {
    Handler handler;
    base::StringPiece method;
  } kHandlers[] = {
      {&RuntimeHooksDelegate::HandleSendMessage, kSendMessage},
      {&RuntimeHooksDelegate::HandleSendRequest, kSendRequest},
...
RequestResult TabsHooksDelegate::HandleSendRequest(
  ...
  DCHECK(arguments[0]->IsInt32());

  int tab_id = arguments[0]->Int32Value();
  ...
  messaging_service_->SendOneTimeMessage(
      script_context, MessageTarget::ForTab(tab_id, -1), kSendRequestChannel,
      false, *message, response_callback);
...
RequestResult TabsHooksDelegate::HandleSendMessage(
  ...
  messaging_util::ParseOptionsResult parse_result =
      messaging_util::ParseMessageOptions(
          script_context->v8_context(), arguments[2], false, &options, &error);
  switch (parse_result) {
  ...
  messaging_service_->SendOneTimeMessage(
      ...
      kSendMessageChannel, false, *message, response_callback);
...
RequestResult TabsHooksDelegate::HandleConnect(
  ...
  ParseOptionsResult parse_result = ParseMessageOptions(
      script_context->v8_context(), arguments[1], true, &options, &error);
  switch (parse_result) {
    case TYPE_ERROR: {
      RequestResult result(RequestResult::INVALID_INVOCATION);
      result.error = std::move(error);
