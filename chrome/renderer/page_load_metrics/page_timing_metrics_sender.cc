...
void PageTimingMetricsSender::DidObserveNewFeatureUsage(
    blink::WebFeature feature) {
  uint32_t feature_id = static_cast<int>(feature);
  if (features_sent_.test(feature_id))
    return;
  features_sent_.set(feature_id);
  new_features_->features.push_back(feature_id);
  EnsureSendTimer();
}
...
void PageTimingMetricsSender::SendNow() {
  have_sent_ipc_ = true;
  sender_->SendTiming(last_timing_, metadata_, new_features_);
  ClearNewFeatures();
}

void PageTimingMetricsSender::ClearNewFeatures() {
  new_features_->features.clear();
}
