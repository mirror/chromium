...
printing::PrintWebFrameHelper* GetPrintWebFrameHelper(
    const blink::WebElement& element) {
  if (element.IsNull())
    return nullptr;
  auto* render_frame =
      content::RenderFrame::FromWebFrame(element.GetDocument().GetFrame());
  return printing::PrintWebFrameHelper::Get(render_frame);
}
...
bool ChromePDFPrintClient::IsPrintingEnabled(PP_Instance instance_id) {
  ...
  printing::PrintWebFrameHelper* helper = GetPrintWebFrameHelper(element);
