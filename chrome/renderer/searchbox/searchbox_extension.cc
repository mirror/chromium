...
v8::Local<v8::Object> GenerateMostVisitedItem(
  ...
  obj->Set(v8::String::NewFromUtf8(isolate, "faviconUrl"),
           UTF8ToV8String(isolate, favicon_url));
...
v8::Local<v8::Object> GenerateThemeBackgroundInfo(
  ...
  info->Set(v8::String::NewFromUtf8(isolate, "textColorLightRgba"),
            RGBAColorToArray(isolate, theme_info.text_color_light));
  ...
  info->Set(v8::String::NewFromUtf8(isolate, "headerColorRgba"),
            RGBAColorToArray(isolate, theme_info.header_color));

  // Theme color for section border as an array with the RGBA components in
  // order. Value is always valid.
  info->Set(v8::String::NewFromUtf8(isolate, "sectionBorderColorRgba"),
            RGBAColorToArray(isolate, theme_info.section_border_color));
...
// Javascript bindings for chrome.embeddedSearch.searchBox. See
...
bool SearchBoxBindings::IsFocused() {
  const SearchBox* search_box = GetSearchBox(render_frame_);
  if (!search_box)
    return false;
...
void SearchBoxBindings::Paste(const std::string& text) {
  SearchBox* search_box = GetSearchBox(render_frame_);
  if (!search_box)
    return;
...
// Javascript bindings for chrome.embeddedSearch.newTabPage. See
// https://www.chromium.org/embeddedsearch.
class NewTabPageBindings : public gin::Wrappable<NewTabPageBindings> {
 ...
 private:
  ...
  v8::Local<v8::Value> GetMostVisited(gin::Arguments* args);
  v8::Local<v8::Value> GetThemeBackgroundInfo(gin::Arguments* args);
  ...
  v8::Local<v8::Value> GetMostVisitedItemData(gin::Arguments* args);
...
v8::Local<v8::Value> NewTabPageBindings::GetMostVisited(gin::Arguments* args) {
  const SearchBox* search_box = GetSearchBox(render_frame_);
  if (!search_box)
    return v8::Null(args->isolate());

  v8::Local<v8::Value> device_pixel_ratio_v8 =
      render_frame_->GetWebFrame()->ExecuteScriptAndReturnValue(
          blink::WebScriptSource("window.devicePixelRatio"));
  float device_pixel_ratio = 1.0f;
  gin::Converter<float>::FromV8(args->isolate(), device_pixel_ratio_v8,
                                &device_pixel_ratio);

  int render_view_id =
      GetMainFrame(render_frame_)->GetRenderView()->GetRoutingID();
...
v8::Local<v8::Value> NewTabPageBindings::GetThemeBackgroundInfo(
  ...
  return GenerateThemeBackgroundInfo(args->isolate(), theme_info);
}
...
v8::Local<v8::Value> NewTabPageBindings::GetMostVisitedItemData(
  ...
  if (args->Length() != 1 || !args->GetNext(&rid) ||
      !search_box->GetMostVisitedItemWithID(rid, &item)) {
    return v8::Null(args->isolate());
  }
  int render_view_id =
      GetMainFrame(render_frame_)->GetRenderView()->GetRoutingID();
  return GenerateMostVisitedItemData(args->isolate(), render_view_id, rid,
                                     item);
}
...
void NewTabPageBindings::LogMostVisitedImpression(int position,
  ...
  if (tile_title_source <= static_cast<int>(ntp_tiles::TileTitleSource::LAST) &&
    ...
    search_box->LogMostVisitedImpression(
...
void NewTabPageBindings::LogMostVisitedNavigation(int position,
  ...
  if (tile_title_source <= static_cast<int>(ntp_tiles::TileTitleSource::LAST) &&
      tile_source <= static_cast<int>(ntp_tiles::TileSource::LAST) &&
      tile_type <= ntp_tiles::TileVisualType::TILE_TYPE_MAX) {
    search_box->LogMostVisitedNavigation(
        position, static_cast<ntp_tiles::TileTitleSource>(tile_title_source),
        static_cast<ntp_tiles::TileSource>(tile_source),
        static_cast<ntp_tiles::TileVisualType>(tile_type));
  }
