...
bool IsBlinkDnsError(const error_page::Error& error) {
...
bool ShouldUseFixUrlServiceForError(const error_page::Error& error,
  ...
  if (domain == net::kErrorDomain && error.reason() == 404) {
    *error_param = "http404";
    return true;
  }
...
void NetErrorHelperCore::OnCommitLoad(FrameType frame_type, const GURL& url) {
  ...
  if (committed_error_page_info_ && !pending_error_page_info_ &&
      committed_error_page_info_->auto_reload_triggered) {
    const auto& error = committed_error_page_info_->error;
...
void NetErrorHelperCore::GetErrorHtmlForMainFrame(
    ...
    std::string* error_html) {
  std::string error_param;
  auto error = pending_error_page_info->error;
