...
NetErrorHelperCore::PageType GetLoadingPageType(
  ...
  GURL url = loader->GetRequest().Url();
...
void NetErrorHelper::DidStartProvisionalLoad(blink::WebDocumentLoader* loader) {
...
void NetErrorHelper::LoadPageFromCache(const GURL& page_url) {
  ...
  DCHECK_NE("POST",
            web_frame->DocumentLoader()->GetRequest().HttpMethod().Ascii());
