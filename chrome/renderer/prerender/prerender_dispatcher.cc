...
PrerenderDispatcher::PrerenderDispatcher()
    : process_start_time_(base::Time::Now()) {
...
void PrerenderDispatcher::DecrementPrefetchCount() {
  if (!--prefetch_count_ && prefetch_finished_) {
    UMA_HISTOGRAM_MEDIUM_TIMES("NoStatePrefetchRendererLifetimeExtension",
                               base::Time::Now() - prefetch_parsed_time_);
...
void PrerenderDispatcher::PrefetchFinished() {
  prefetch_parsed_time_ = base::Time::Now();
  if (prefetch_count_) {
    prefetch_finished_ = true;
  } else {
    UMA_HISTOGRAM_MEDIUM_TIMES("NoStatePrefetchRendererParseTime",
