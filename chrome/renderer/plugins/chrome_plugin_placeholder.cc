...
void ChromePluginPlaceholder::OnMenuAction(int request_id, unsigned action) {
  ...
  if (g_last_active_menu != this)
    return;
  switch (action) {
    case MenuCommands::MENU_COMMAND_PLUGIN_RUN: {
    ...
    case MenuCommands::MENU_COMMAND_PLUGIN_HIDE: {
      RenderThread::Get()->RecordAction(UserMetricsAction("Plugin_Hide_Menu"));
      HidePlugin();
      break;
    }
    case MenuCommands::MENU_COMMAND_ENABLE_FLASH: {
...
void ChromePluginPlaceholder::ShowContextMenu(
  ...
  if (!GetPluginInfo().path.value().empty() && !flash_hidden) {
    ...
    run_item.action = MenuCommands::MENU_COMMAND_PLUGIN_RUN;
  ...
  if (flash_hidden) {
    ...
    enable_flash_item.action = MenuCommands::MENU_COMMAND_ENABLE_FLASH;
  ...
  hide_item.action = MenuCommands::MENU_COMMAND_PLUGIN_HIDE;
