...
void ChromePluginPlaceholder::OnMenuAction(int request_id, unsigned action) {
  ...
  if (g_last_active_menu != this)
    return;
  switch (action) {
    case static_cast<unsigned>(MenuCommands::MENU_COMMAND_PLUGIN_RUN): {
    ...
    case static_cast<unsigned>(MenuCommands::MENU_COMMAND_PLUGIN_HIDE): {
      RenderThread::Get()->RecordAction(UserMetricsAction("Plugin_Hide_Menu"));
      HidePlugin();
      break;
    }
    case static_cast<unsigned>(MenuCommands::MENU_COMMAND_ENABLE_FLASH): {
...
void ChromePluginPlaceholder::ShowContextMenu(
  ...
  if (!GetPluginInfo().path.value().empty() && !flash_hidden) {
    ...
    run_item.action =
        static_cast<unsigned>(MenuCommands::MENU_COMMAND_PLUGIN_RUN);
  ...
  if (flash_hidden) {
    ...
    enable_flash_item.action =
        static_cast<unsigned>(MenuCommands::MENU_COMMAND_ENABLE_FLASH);
  ...
  hide_item.action =
      static_cast<unsigned>(MenuCommands::MENU_COMMAND_PLUGIN_HIDE);
