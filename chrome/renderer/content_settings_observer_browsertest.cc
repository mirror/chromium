...
// the given |frame| while the instance is in scope, and verifies that the
...
TEST_F(ChromeRenderViewTest, JSBlockSentAfterPageLoad) {
  ...
  auto HasSentChromeViewHostMsgContentBlocked =
  ...
  // 3. Reload page. Verify that the notification that javascript was blocked is
  // only sent after the navigation notification is sent.
