...
void ContentSettingsObserver::GetAllowedClientHintsFromSource(
  ...
  LOG(WARNING) << "xxx ContentSettingsObserver::AllowClientHintFromSource cp0";
  if (!content_setting_rules_)
    return;
  LOG(WARNING) << "xxx ContentSettingsObserver::AllowClientHintFromSource cp1";

  if (!content::IsOriginSecure(GURL(url)))
    return;
  LOG(WARNING) << "xxx ContentSettingsObserver::AllowClientHintFromSource cp2";

  const GURL hostname = GURL(url).GetOrigin();

  for (ContentSettingsForOneType::const_iterator it =
           content_setting_rules_->client_hints_rules.begin();
       it != content_setting_rules_->client_hints_rules.end(); ++it) {
    LOG(WARNING)
        << "xxx ContentSettingsObserver::AllowClientHintFromSource cp3";
    ...
    if (base::Time::Now().ToDoubleT() > expiration_time->GetDouble()) {
      ...
      LOG(WARNING)
          << "xxx ContentSettingsObserver::AllowClientHintFromSource cp4";
      return;
    }

    LOG(WARNING)
        << "xxx ContentSettingsObserver::AllowClientHintFromSource cp5";
    ...
    LOG(WARNING)
        << "xxx ContentSettingsObserver::AllowClientHintFromSource cp6";
    // Match found for |url| and client hints have been set.
    return;
  }
  // No match was found for |url|.
  LOG(WARNING) << "xxx ContentSettingsObserver::AllowClientHintFromSource cp7";
}
