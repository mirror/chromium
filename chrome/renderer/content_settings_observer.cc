...
void ContentSettingsObserver::SetContentSettingRules(
  ...
  UMA_HISTOGRAM_COUNTS("ContentSettings.ClientHints.CountRulesReceived",
...
void ContentSettingsObserver::PersistClientHints(
  ...
  UMA_HISTOGRAM_CUSTOM_TIMES("ContentSettings.ClientHints.PersistDuration",
  ...
  // this method should not return early if |expiration_times| is empty.
  std::unique_ptr<base::ListValue> expiration_times(new base::ListValue());
  expiration_times->Reserve(blink::kWebClientHintsTypeLast + 1);
  ...
  for (size_t i = 0; i < blink::kWebClientHintsTypeLast + 1; ++i) {
    if (enabled_client_hints.IsEnabled(
      ...
      expiration_times->Set(i, base::MakeUnique<base::Value>(expiration_time));
      update_count++;
    } else {
      expiration_times->Set(i, base::MakeUnique<base::Value>(-1.0f));
    }
  }
  if (update_count == 0)
    return;
  ...
  host_observer->PersistClientHints(hostname,
