...
TEST_F(PasswordAutofillAgentTest, ManualFallbackForSaving) {
  ...
  EXPECT_EQ(0, fake_driver_.called_show_manual_fallback_count());
  ...
  EXPECT_EQ(1, fake_driver_.called_show_manual_fallback_count());

  // Remove one character from the password value.
  SimulateUserTypingASCIICharacter(ui::VKEY_BACK, true);
  EXPECT_EQ(2, fake_driver_.called_show_manual_fallback_count());

  // Add one character to the username value.
  SetFocused(username_element_);
  SimulateUserTypingASCIICharacter('a', true);
  EXPECT_EQ(3, fake_driver_.called_show_manual_fallback_count());

  // Remove username value.
  SimulateUsernameChange("");
  EXPECT_EQ(4, fake_driver_.called_show_manual_fallback_count());

  // Change the password. Despite of empty username the fallback is still
  // there.
  SetFocused(password_element_);
  SimulateUserTypingASCIICharacter('a', true);
  EXPECT_EQ(5, fake_driver_.called_show_manual_fallback_count());
