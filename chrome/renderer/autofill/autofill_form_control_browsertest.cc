...
class AutofillFormControllTest : public ChromeRenderViewTest {
 protected:
  void SetUp() override {
...
TEST_F(AutofillFormControllTest, InputClicked) {
...
TEST_F(AutofillFormControllTest, InputRightClicked) {
  ClearAutofillAgentTestState();
  EXPECT_NE(text_, text_.GetDocument().FocusedElement());
...
TEST_F(AutofillFormControllTest, InputFocusedAndClicked) {
  ClearAutofillAgentTestState();
...
// clicked.
TEST_F(AutofillFormControllTest, TextAreaClicked) {
  ClearAutofillAgentTestState();
...
TEST_F(AutofillFormControllTest, TextAreaFocusedAndClicked) {
  ClearAutofillAgentTestState();
...
TEST_F(AutofillFormControllTest, ScaledTextareaClicked) {
  ClearAutofillAgentTestState();
...
TEST_F(AutofillFormControllTest, ScaledTextareaTapped) {
  ClearAutofillAgentTestState();
  EXPECT_NE(textarea_, textarea_.GetDocument().FocusedElement());
  view_->GetWebView()->SetPageScaleFactor(3);
  view_->GetWebView()->SetVisualViewportOffset(blink::WebFloatPoint(50, 50));
...
TEST_F(AutofillFormControllTest, DisabledInputClickedNoEvent) {
  ClearAutofillAgentTestState();
  EXPECT_NE(text_, text_.GetDocument().FocusedElement());
  // Click the text field once.
  EXPECT_TRUE(SimulateElementClick("text_1"));
...
TEST_F(AutofillFormControllTest, ClickDisabledInputDoesNotResetClickCounter) {
  EXPECT_NE(text_, text_.GetDocument().FocusedElement());
  // Click the text field once.
  EXPECT_TRUE(SimulateElementClick("text_1"));
  EXPECT_TRUE(form_control_element_clicked_called());
...
TEST_F(AutofillFormControllTest, TapNearEdgeIsPageClick) {
  EXPECT_NE(text_, text_.GetDocument().FocusedElement());
