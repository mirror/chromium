...
base::LazyInstance<base::SequenceLocalStorageSlot<
    ...
    compositor_thread_sampling_profiler = LAZY_INSTANCE_INITIALIZER;
...
void StartCompositorThreadProfiling() {
  ...
  constexpr base::StackSamplingProfiler::SamplingParams kProfilerParams = {
      .initial_delay = base::TimeDelta::FromSeconds(1),
      .bursts = 1,
      .samples_per_burst = 1
      // |burst_interval| and |sampling_interval| are unused.
  };
  auto profiler = std::make_unique<base::StackSamplingProfiler>(
      base::PlatformThread::CurrentId(), kProfilerParams,
  ...
  profiler->Start();
  compositor_thread_sampling_profiler.Get().Set(std::move(profiler));
}
