...
ChromeContentRendererClient::~ChromeContentRendererClient() {
  LOG(ERROR) << "&&&&&&&&&&&&&&&&&& ChromeContentRendererClient::~DTOR";
}
...
#if BUILDFLAG(ENABLE_SPELLCHECK)
void ChromeContentRendererClient::InitSpellCheck(bool for_tests) {
  LOG(ERROR) << "+++++ CCRC:: pre make spellcheck";
  spellcheck_ =
      std::make_unique<SpellCheck>(for_tests ? nullptr : &registry_,
                                   for_tests ? nullptr : this);
