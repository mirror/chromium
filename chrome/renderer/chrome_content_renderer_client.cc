...
bool ChromeContentRendererClient::WillSendRequest(
  ...
  if (!throttle_provider_) {
    throttle_provider_ = std::make_unique<URLLoaderThrottleProviderImpl>(
        URLLoaderThrottleProviderImpl::FOR_MAIN_THREAD);
  }
  ...
  // TODO(yzshen): Pass the actual resource type once John's change for
  // prerender throttle lands.
...
ChromeContentRendererClient::CreateWorkerURLLoaderThrottleProvider() {
  return std::make_unique<URLLoaderThrottleProviderImpl>(
      URLLoaderThrottleProviderImpl::FOR_WORKER);
}
