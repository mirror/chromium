...
static void* SystemAllocPages(
  ...
  void* ret;
  // Retry failed allocations once after calling OnAllocFailure().
  int retries = 0;
#if defined(OS_WIN)
  DWORD access_flag =
...
bool ReserveAddressSpace(size_t size, size_t alignment) {
  // Don't take s_reserveLock while allocating, since a failure will invoke
  // OnAllocFailure and deadlock.
  void* mem = AllocPages(nullptr, size, alignment, base::PageInaccessible);
  if (mem != nullptr) {
    // Take the lock to update the reservation.
...
void OnAllocFailure(size_t size) {
  base::subtle::SpinLock::Guard guard(s_reserveLock);
  if (s_reservation_address != nullptr) {
