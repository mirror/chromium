...
static void* AllocWithReserveOnFailure(
    void* hint,
    size_t length,
    PageAccessibilityConfiguration page_accessibility,
    bool commit) {
  void* ret = SystemAllocPages(hint, length, page_accessibility, commit);
  if (ret == nullptr && hint == nullptr) {
    // Unhinted allocation failure means OOM. Release any reservation and retry.
    ReleaseReservation();
...
void* AllocPages(void* address,
  ...
  for (int count = 0; count < 3; ++count) {
    void* ret =
        AllocWithReserveOnFailure(address, length, page_accessibility, commit);
  ...
  do {
    ...
    ret = AllocWithReserveOnFailure(address, try_length, page_accessibility,
                                    commit);
