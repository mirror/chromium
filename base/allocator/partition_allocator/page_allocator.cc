...
// Allocate pages with possible retry using reserved address space.
static void* AllocPagesInternal(
    void* hint,
    size_t length,
    PageAccessibilityConfiguration page_accessibility,
    bool commit,
    bool retry_with_reserve) {
  void* ret = SystemAllocPages(hint, length, page_accessibility, commit);
  if (ret == nullptr && retry_with_reserve) {
    ReleaseReservation();
    ret = SystemAllocPages(hint, length, page_accessibility, commit);
  }
  return ret;
}
...
void* AllocPages(void* address,
  ...
  for (int count = 0; count < 3; ++count) {
    bool retry_with_reserve = kHintIsAdvisory || address == nullptr;
    void* ret = AllocPagesInternal(address, length, page_accessibility, commit,
                                   retry_with_reserve);
  ...
  do {
    ...
    bool retry_with_reserve = kHintIsAdvisory || address == nullptr;
    ret = AllocPagesInternal(address, try_length, page_accessibility, commit,
                             retry_with_reserve);
