...
static void* TryAllocPages(void* address,
                           size_t length,
                           PageAccessibilityConfiguration page_accessibility,
                           bool commit) {
...
// First try to force an exact-size, aligned allocation at a random address.
#if defined(ARCH_CPU_64_BITS)
  // We make 3 random exact tries on 64-bit systems.
  constexpr int kExactLengthTries = 3;
#else
  // On 32-bit systems, we make one exact random attempt , and if that fails,
  // try either an unhinted allocation (Windows) or the first aligned address
  // after the base address that was returned on the first try (POSIX).
  constexpr int kExactLengthTries = 2;
#endif
  for (int i = 0; i < kExactLengthTries; ++i) {
    void* ret = TryAllocPages(address, length, page_accessibility, commit);
    if (!ret) {
      // If even an unhinted allocation fails, we're OOM.
      if (kHintIsAdvisory || address == nullptr)
        return nullptr;
      // |ret| is null, meaning a hinted allocation failed (Windows).
    } else {
      // If |ret| is aligned, we're done.
      if (!(reinterpret_cast<uintptr_t>(ret) & align_offset_mask))
        return ret;
#if defined(OS_WIN)
      // Windows either returned our aligned address, or null.
      NOTREACHED();
#endif
      // Free unaligned allocation so we can try again (POSIX).
...
// The address space is small; try the first aligned address after |ret|.
// On Windows, |ret| must be null; therefore |address| will be null.
#if defined(OS_WIN) && defined(ARCH_CPU_32_BITS)
    CHECK_EQ(nullptr, ret);
#endif
  ...
  do {
    ...
    ret = TryAllocPages(address, try_length, page_accessibility, commit);
