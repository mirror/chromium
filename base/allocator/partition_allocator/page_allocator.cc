...
static void* TryAllocPages(void* address,
                           ...
                           PageAccessibilityConfiguration page_accessibility,
                           bool commit) {
  void* result = SystemAllocPages(address, length, page_accessibility, commit);
  if (result == nullptr) {
    const bool is_oom = kHintIsAdvisory || address == nullptr;
    if (is_oom) {
      ReleaseReservation();
      result = SystemAllocPages(nullptr, length, page_accessibility, commit);
    }
  }
  return result;
}
...
void* AllocPages(void* address,
  ...
  for (int count = 0; count < 3; ++count) {
    ...
    if (kHintIsAdvisory || ret) {
      ...
      // |ret| can be null if we are OOM (POSIX).
      ...
      // We're OOM when an unhinted allocation fails (Windows).
      return nullptr;
    } else {
#if defined(ARCH_CPU_32_BITS)
      // Let the OS choose the base (32-bit Windows).
      address = nullptr;
#endif
    }
