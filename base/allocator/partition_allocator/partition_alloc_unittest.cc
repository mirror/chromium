...
TEST_F(PartitionAllocTest, GenericAllocGetSize) {
  void* ptr;
  size_t requested_size, actualSize, predictedSize;

  EXPECT_TRUE(PartitionAllocSupportsGetSize());

  // Allocate something small.
  requested_size = 511 - kExtraAllocSize;
  predictedSize = generic_allocator.root()->ActualSize(requested_size);
  ...
  EXPECT_LT(requested_size, actualSize);
  generic_allocator.root()->Free(ptr);
  ...
  actualSize = PartitionAllocGetSize(ptr);
  EXPECT_EQ(predictedSize, actualSize);
  EXPECT_EQ(requested_size, actualSize);
  generic_allocator.root()->Free(ptr);
  ...
  actualSize = PartitionAllocGetSize(ptr);
  EXPECT_EQ(predictedSize, actualSize);
  EXPECT_EQ(requested_size + kSystemPageSize, actualSize);
  ...
  *(charPtr + (actualSize - 1)) = 'A';
  ...
  predictedSize = generic_allocator.root()->ActualSize(requested_size);
  EXPECT_EQ(requested_size, predictedSize);
}
