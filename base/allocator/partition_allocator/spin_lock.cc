...
// static
bool SpinLock::IsEvent(intptr_t current) {
  return current != UNLOCKED && current != LOCKED && current != CONTENDED;
}
...
void SpinLock::UnlockSlow() {
  DCHECK_EQ(CONTENDED, lock_.load(std::memory_order_relaxed));
