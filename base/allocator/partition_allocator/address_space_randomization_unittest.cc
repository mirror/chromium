...
#elif defined(OS_WIN)
static const uintptr_t kASLRMask = ASLR_MASK(47);
static const uintptr_t kASLRMaskBefore8_10 = ASLR_MASK(43);
static const uintptr_t kASLROffset = 0;
...
// changes.
static const uintptr_t kASLRMask = ASLR_MASK(39);
...
static const uintptr_t kASLRMask = ASLR_MASK(44);
static const uintptr_t kASLROffset = ASLR_ADDRESS(0);
#else   // !defined(OS_AIX) && !defined(ARCH_CPU_BIG_ENDIAN)
// Little-endian Linux: 48 bits of virtual addressing.
static const uintptr_t kASLRMask = ASLR_MASK(48);
static const uintptr_t kASLROffset = ASLR_ADDRESS(0);
...
TEST(AddressSpaceRandomizationTest, GetRandomPageBase) {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    EXPECT_GE(kASLROffset + kASLRMask, address);
  ...
  EXPECT_EQ(kASLRMask, address_logical_sum);
}
