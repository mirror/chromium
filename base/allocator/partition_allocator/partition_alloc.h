...
ALWAYS_INLINE uintptr_t PartitionFreelistMask(PartitionFreelistEntry* ptr) {
  ...
  // address bits are reconstructed using the super page base address when
  // unmasking the entry. This prevents the allocator from handing out arbitrary
  ...
  uintptr_t offset = reinterpret_cast<uintptr_t>(ptr) & kSuperPageOffsetMask;
...
// For big endian, similar guarantees are arrived at with a negation.
  ...
  return ~offset;
#else
  return ByteSwapUintPtrT(offset);
#endif
}
...
ALWAYS_INLINE PartitionFreelistEntry* PartitionFreelistUnmask(
    ...
    uintptr_t masked) {
#if defined(ARCH_CPU_BIG_ENDIAN)
  uintptr_t offset = ~masked;
#else
  uintptr_t offset = ByteSwapUintPtrT(masked);
#endif
  uintptr_t ptr = (reinterpret_cast<uintptr_t>(page) & kSuperPageBaseMask) |
                  (offset & kSuperPageOffsetMask);
  return reinterpret_cast<PartitionFreelistEntry*>(ptr);
}
...
// If these asserts fire, you probably corrupted memory.
  ...
  DCHECK(!freelist_head ||
         ptr != PartitionFreelistUnmask(this, next_free->next));
