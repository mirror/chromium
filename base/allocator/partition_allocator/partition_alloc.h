...
struct PartitionBucket {
  ...
  ALWAYS_INLINE size_t span_size_bytes() const {
    return num_system_pages_per_slot_span * kSystemPageSize;
  }
  ALWAYS_INLINE uint16_t num_slots() const {
    return static_cast<uint16_t>(span_size_bytes() / slot_size);
  }
};
...
ALWAYS_INLINE size_t* PartitionPageGetRawSizePtr(PartitionPage* page) {
  ...
  DCHECK(bucket->is_direct_mapped() || bucket->num_slots() == 1);
  page++;
