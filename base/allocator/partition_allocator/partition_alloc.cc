...
static size_t PartitionPurgePage(PartitionPage* page, bool discard) {
  ...
  // are not in use.
  PartitionFreelistEntry* freelist_entry = nullptr;
  for (uintptr_t freelist_offset = page->freelist_head; freelist_offset != 0;
       freelist_offset = freelist_entry->next) {
    freelist_entry = PartitionFreelistUnmask(page, freelist_offset);
    size_t slotIndex =
        (reinterpret_cast<char*>(freelist_entry) - ptr) / slot_size;
    DCHECK(!page->freelist_head || slotIndex < num_slots);
