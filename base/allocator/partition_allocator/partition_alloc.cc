...
static ALWAYS_INLINE void* PartitionAllocPartitionPages(
  ...
  // This is a trusted value because num_partition_pages is not user controlled.
  //
...
// Returns the a natural number of PartitionPages (calculated by
// PartitionBucketNumSystemPages()) a to allocate from the current SuperPage
...
// A slot spans size is equal to PartitionBucketPartitionPages(bucket)
...
void* PartitionAllocSlowPath(PartitionRootBase* root,
  ...
  // TODO(ajwong): Why does this cause us to reassign |bucket| here? This seems
  // like a bug. Investigate.
