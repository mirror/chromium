...
namespace {
// gSentinelPage is used as a sentinel to indicate that there is no page
// in the active page list. We can use nullptr, but in that case we need
// to add a null-check branch to the hot allocation path. We want to avoid
// that.
static_assert(std::is_trivially_default_constructible<PartitionPage>::value,
              "PartitionPage must be trivially default constructible to be "
              "correctly initialized as a global.");
static_assert(std::is_trivially_default_constructible<PartitionBucket>::value,
              "PartitionBucket must be trivially default constructible to be "
              "correctly initialized as a global.");
PartitionPage gSentinelPage;
PartitionBucket gSentinelBucket;
}  // namespace

PartitionPage* GetSentinelPageForTesting() {
  return &gSentinelPage;
}
...
static LazyInstance<subtle::SpinLock>::Leaky gInitializedLock =
...
static void PartitionBucketInitBase(PartitionBucket* bucket,
  ...
  bucket->active_pages_head = &gSentinelPage;
...
void* PartitionAllocSlowPath(PartitionRootBase* root,
  ...
  if (UNLIKELY(PartitionBucketIsDirectMapped(bucket))) {
    ...
    DCHECK(bucket == &gSentinelBucket);
    DCHECK(bucket->active_pages_head == &gSentinelPage);
...
void PartitionFreeSlowPath(PartitionPage* page) {
  ...
  if (LIKELY(page->num_allocated_slots == 0)) {
    ...
    // the next page.
    DCHECK(!page->next_page);
    if (LIKELY(bucket->active_pages_head != &gSentinelPage))
...
static void PartitionPurgeBucket(PartitionBucket* bucket) {
  if (bucket->active_pages_head != &gSentinelPage) {
...
static void PartitionDumpBucketStats(PartitionBucketMemoryStats* stats_out,
  ...
  // If the active page list is empty (== &gSentinelPage),
