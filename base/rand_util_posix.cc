...
class RandomnessCache {
  ...
  void GetRandomBytes(void* output, size_t output_length) {
    ...
    if (additional_bytes_needed < kTlsCacheSize) {
      ...
      const bool success = base::ReadFromFD(
          urandom_fd_, reinterpret_cast<char*>(&cache_[0]), kTlsCacheSize);
      CHECK(success);
      cache_index_ = additional_bytes_needed;
      memcpy(static_cast<uint8_t*>(output) + num_cached_bytes, cache_,
 ...
 private:
  ...
  uint8_t cache_[kTlsCacheSize];
  size_t cache_index_ = kTlsCacheSize;

  DISALLOW_COPY_AND_ASSIGN(RandomnessCache);
};

base::LazyInstance<base::ThreadLocalPointer<RandomnessCache>>::Leaky
    g_tls_cache;
...
void RandBytes(void* output, size_t output_length) {
  RandomnessCache* cache = g_tls_cache.Get().Get();
  if (!cache) {
    cache = new RandomnessCache;
    g_tls_cache.Get().Set(cache);
  }
