...
// static
void GlobalActivityTracker::RecordProcessExitIfEnabled(ProcessId process_id, int exit_code) {
  LOG(ERROR) << "Enter RecordProcessExitIfEnabled process_id:" << process_id
               << ", exit_code: " << exit_code;
  GlobalActivityTracker* tracker = Get();
  if (tracker)
    tracker->RecordProcessExit(process_id, exit_code);

  LOG(ERROR) << "Exit RecordProcessExitIfEnabled process_id:" << process_id
               << ", exit_code: " << exit_code;
}
