...
class BASE_EXPORT Value {
 public:
  ...
  const Value* FindPathOfType(std::initializer_list<const char*> path, Type type) const;
  ...
  // they will be created. If any of intermediate path components matches a
  // value that is not a dictionary, the function will fail (it will not
  // overwrite the value) and return nullptr. The last key will be
  // unconditionally overwritten if it exists, and created if it doesn't.
  //
  // Example:
