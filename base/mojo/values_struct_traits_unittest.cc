...
class ValuesTester : public mojom::ValuesTester {
 public:
  ...
  void BounceNullableValue(Value in, BounceValueCallback callback) override {
...
TEST_F(ValuesStructTraitsTest, NullValue) {
  const Value in;
  Value out;
  ASSERT_TRUE(proxy_->BounceNullableValue(in, &out));
  EXPECT_EQ(in, out);

  {
    // When not declared as nullable, a serialization warning should be
    // triggered.
...
TEST_F(ValuesStructTraitsTest, BinaryValue) {
  std::vector<char> kBinaryData = {0, 1, 255, 1, 0};
