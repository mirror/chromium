...
Optional<Value> JSONParser::ConsumeDictionary() {
  ...
  while (token != T_OBJECT_END) {
    ...
    dict_storage.emplace_back(key.DestructiveAsString(),
                              MakeUnique<Value>(std::move(*value)));
  ...
  return make_optional(
      Value(Value::DictStorage(std::move(dict_storage), KEEP_LAST_OF_DUPES)));
}
...
Optional<Value> JSONParser::ConsumeList() {
  ...
  return make_optional(Value(std::move(list_storage)));
}
...
Optional<Value> JSONParser::ConsumeString() {
  ...
  return make_optional(Value(string.DestructiveAsString()));
}
...
Optional<Value> JSONParser::ConsumeLiteral() {
  switch (*pos_) {
    case 't': {
      ...
      return make_optional(Value(true));
    }
    case 'f': {
      ...
      return make_optional(Value(false));
    }
    case 'n': {
