...
#define COMPONENT_MACRO_CONDITIONAL_(condition, consequent, alternate) \
  COMPONENT_MACRO_SELECT_THIRD_ARGUMENT_(                              \
      COMPONENT_MACRO_CONDITIONAL_VALUE_(condition), consequent, alternate)

// Expands to a helper value for use with |COMPONENT_MACRO_CONDITIONAL_()|. If
// |x| expands to |1| then this macro expands to |0, 1|. Otherwise this macro
// expands to an arbitrary single token.
#define COMPONENT_MACRO_CONDITIONAL_VALUE_(...) \
  COMPONENT_MACRO_CONDITIONAL_VALUE_IMPL_(__VA_ARGS__, 0)
#define COMPONENT_MACRO_CONDITIONAL_VALUE_IMPL_(x, ...) \
  COMPONENT_MACRO_CONDITIONAL_VALUE_##x##_
#define COMPONENT_MACRO_CONDITIONAL_VALUE_1_ 0, 1

// Helper which simply selects its third argument. Used in conjunction with
// |COMPONENT_MACRO_CONDITIONAL_VALUE_()| above.
...
// Helper to work around MSVC quirkiness wherein a macro expansion like |0, 1|
// is treated as a single token. Needed to ensure that
// |COMPONENT_MACRO_CONDITIONAL_VALUE_()| above can expand to multiple separate
// positional arguments in the affirmative case, thus elliciting the desired
// conditional behavior with |COMPONENT_MACRO_SELECT_THIRD_ARGUMENT_()|.
