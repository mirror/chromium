...
TEST(TestMockTimeTaskRunnerTest, Basic) {
  static constexpr TestMockTimeTaskRunner::Type kTestCases[] = {
      TestMockTimeTaskRunner::Type::kDefault,
      TestMockTimeTaskRunner::Type::kBound};

  for (auto type : kTestCases) {
    SCOPED_TRACE(static_cast<int>(type));

    scoped_refptr<TestMockTimeTaskRunner> mock_time_task_runner =
        MakeRefCounted<TestMockTimeTaskRunner>(type);
    int counter = 0;
    ...
    mock_time_task_runner->PostDelayedTask(
        FROM_HERE,
        base::Bind([](int* counter) { *counter += 96; }, Unretained(&counter)),
...
TEST(TestMockTimeTaskRunnerTest, DefaultUnbound) {
  scoped_refptr<TestMockTimeTaskRunner> unbound_mock_time_task_runner =
...
TEST(TestMockTimeTaskRunnerTest, RunLoopDriveableWhenBound) {
  scoped_refptr<TestMockTimeTaskRunner> bound_mock_time_task_runner =
      MakeRefCounted<TestMockTimeTaskRunner>(
          TestMockTimeTaskRunner::Type::kBound);
...

  {
    ...
    // The QuitClosure() should be ordered between the 96 and the 8192
