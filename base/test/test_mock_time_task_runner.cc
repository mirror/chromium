...
class NonOwningProxyTaskRunner : public SingleThreadTaskRunner {
 public:
  NonOwningProxyTaskRunner(SingleThreadTaskRunner* target) : target_(target) {
    DCHECK(target_);
  }
 ...
 private:
  ...
  SingleThreadTaskRunner* target_;
...
TestMockTimeTaskRunner::TestMockTimeTaskRunner(Time start_time,
  ...
  if (type == Type::kBound) {
    ...
    thread_task_runner_handle_ = MakeUnique<ThreadTaskRunnerHandle>(
