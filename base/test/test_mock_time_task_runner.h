...
//   - Iff of Type::kBound: can be driven by a RunLoop on the thread the
//     TestMockTimeTaskRunner was created on. RunLoop::Run() will result in
//     running non-delayed tasks until idle and then, if RunLoop::QuitWhenIdle()
//     wasn't invoked, fast- forwarding time to the next delayed task and
//     looping again. And so on, until either RunLoop::Quit() is invoked (quits
//     immediately after the current task) or RunLoop::QuitWhenIdle() is invoked
//     (quits before having to fast forward time once again). Should
//     RunLoop::Run() process all tasks (including delayed ones), it will block
//     until more are posted. As usual, RunLoop::RunUntilIdle() is equivalent to
//     RunLoop::Run() followed by an immediate RunLoop::QuitWhenIdle().
...
class TestMockTimeTaskRunner : public SingleThreadTaskRunner,
 ...
 public:
  ...
  TestMockTimeTaskRunner(Type type = Type::kDefault);
 ...
 private:
  ...
  // Members used to in TestMockTimeTaskRunners of Type::kBound to take
