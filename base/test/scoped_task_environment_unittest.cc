...
TEST_P(ScopedTaskEnvironmentTest, DelayedTasks) {
  ...
  ThreadTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE,
      Bind([](int* counter) { *counter += 8; }, Unretained(&counter)),
      TimeDelta::FromDays(5));
  ThreadTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE,
      Bind([](int* counter) { *counter += 16; }, Unretained(&counter)),
      TimeDelta::FromDays(7));

  ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE,
      Bind([](int* counter) { *counter += 1; }, Unretained(&counter)));
  PostTask(FROM_HERE,
           Bind([](int* counter) { *counter += 2; }, Unretained(&counter)));
