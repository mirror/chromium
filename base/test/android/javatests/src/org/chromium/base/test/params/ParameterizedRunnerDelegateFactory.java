...
public class ParameterizedRunnerDelegateFactory {
    ...
    <T extends ParameterizedRunnerDelegate> T createRunner(TestClass testClass,
        ...
        unmodifiableFrameworkMethodList =
    ...
    static List<FrameworkMethod> generateUnmodifiableFrameworkMethodList(
        ...
        Map<Class<? extends ParameterGenerator>, List<FrameworkMethod>>
                generatorToListOfFrameworkMethod = new HashMap<>();

        // Represent the list of all ParameterizedFrameworkMethod in this test class
        List<FrameworkMethod> returnList = new ArrayList<>();

        // Create generatorToListOfFrameworkMethod
        for (FrameworkMethod method : testClass.getAnnotatedMethods(Test.class)) {
            ...
            if (!method.getMethod().isAnnotationPresent(UseMethodParameter.class)) {
                ...
                Class<? extends ParameterGenerator> parameterGenerator =
                        method.getAnnotation(UseMethodParameter.class).value();
                if (generatorToListOfFrameworkMethod.get(parameterGenerator) == null) {
                    List<FrameworkMethod> list = new ArrayList<>();
                    list.add(method);
                    generatorToListOfFrameworkMethod.put(parameterGenerator, list);
                } else {
                    generatorToListOfFrameworkMethod.get(parameterGenerator).add(method);
                }
            }
        }
        ...
        for (Entry<Class<? extends ParameterGenerator>, List<FrameworkMethod>> entry :
                generatorToListOfFrameworkMethod.entrySet()) {
            Class<? extends ParameterGenerator> parameterGeneratorClass = entry.getKey();
            Iterable<ParameterSet> parameterSetList;
            try {
                parameterSetList = parameterGeneratorClass.newInstance().getParameters();
