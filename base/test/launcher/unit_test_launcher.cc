...
class UnitTestProcessLifetimeObserver : public ProcessLifetimeObserver {
 public:
  ~UnitTestProcessLifetimeObserver() override = default;
  ...
  void set_output_file(const FilePath& output_file) {
...
class SerialUnitTestProcessLifetimeObserver
 ...
 public:
  ...
  void set_next_test_names(std::vector<std::string>&& next_test_names) {
...
void RunUnitTestsSerially(
  ...
  auto observer = std::make_unique<SerialUnitTestProcessLifetimeObserver>(
      ...
      std::vector<std::string>(1, test_names.back()), launch_flags);

  observer->set_next_test_names(
  ...
  observer->set_output_file(output_file);
