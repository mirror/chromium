...
class TaskObserver : public MessageLoop::TaskObserver {
 public:
  ...
  // Helper for ObservableTestMockTimeTaskRunner.
  void OnAfterTaskRun() { ++task_count_; }
...
void ScopedTaskEnvironment::RunUntilIdle() {
  do {
    ...
    // Flush any tasks remaining tasks on main thread (which must have been
    // posted by the last few TaskScheduler tasks above). Disallow running
    // TaskScheduler tasks while doing this: this prevents an ABA problem with
    // the GetNumIncompleteUndelayedTasksForTesting() test coming up (which
    // could otherwise be zero despite work having occured after the end of this
    // RunUntilIdle()).
