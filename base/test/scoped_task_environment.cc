...
void ScopedTaskEnvironment::RunUntilIdle() {
  for (int i = 0;; ++i) {
    ...
    // are TaskScheduler tasks currently running. DisallowRunTasks() will block
    // until at least one task completes, try again once it does (restarting
    // from top as they may have posted main thread tasks).
