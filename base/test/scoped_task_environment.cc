...
class ScopedTaskEnvironment::TestTaskTracker
 ...
 public:
  ...
  // be no tasks currently running. If >0 tasks are currently running, blocks
  // until at least one task completes and returns false after (callers are
  // therefore guaranteed not to busy loop calling DisallowRunTasks() on the
  // same set of slow ongoing tasks).
...
void ScopedTaskEnvironment::RunUntilIdle() {
  for (int i = 0;; ++i) {
    ...
    // are TaskScheduler tasks currently running. In that case:
    // DisallowRunTasks() will block until at least one task completes, try
    // again once it returns (restarting from top as they may have posted main
    // thread tasks).
...
bool ScopedTaskEnvironment::TestTaskTracker::DisallowRunTasks() {
  ...
  if (num_tasks_running_ > 0) {
    // Wait until at least one of the ongoing tasks completes (to avoid callers
    // busy looping and asking |DisallowRunTasks()| multiple times in the scope
    // of the same set of ongoing tasks).
