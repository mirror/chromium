...
class TestTaskScheduler : public TaskScheduler {
 public:
  ...
  // Posts |task| to this TaskScheduler from |sequence|. Returns true on
  // success.
...
class TestTaskSchedulerTaskRunner : public SingleThreadTaskRunner {
 ...
 private:
  ...
  scoped_refptr<internal::Sequence> sequence_;
...
TestTaskSchedulerTaskRunner::TestTaskSchedulerTaskRunner(
    ...
    : task_scheduler_(task_scheduler),
      ...
      sequence_(execution_mode == ExecutionMode::PARALLEL
                    ? scoped_refptr<internal::Sequence>()
...
bool TestTaskSchedulerTaskRunner::PostDelayedTask(
  ...
  return task_scheduler_->PostTask(std::move(task), sequence_.get());
}
