...
static void ThreadSafeObserverHarness(int num_threads,
  ...
  AddRemoveThread* threaded_observer[num_threads];
  base::PlatformThreadHandle threads[num_threads];
  std::unique_ptr<base::WaitableEvent> ready[num_threads];
  for (int index = 0; index < num_threads; index++) {
    ready[index] =
        MakeUnique<WaitableEvent>(WaitableEvent::ResetPolicy::MANUAL,
                                  WaitableEvent::InitialState::NOT_SIGNALED);
    threaded_observer[index] = new AddRemoveThread(
        observer_list.get(), cross_thread_notifies, ready[index].get());
