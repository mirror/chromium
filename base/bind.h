...
struct PassesMoveOnlyTypeByCopy {
  static constexpr bool value =
      ...
      std::is_constructible<
          To,
          std::add_rvalue_reference_t<std::remove_reference_t<From>>>::value;
};
...
struct AssertConstructible<RepeatMode::Once, i, Arg, Unwrapped, Param> {
 private:
  ...
  // storage type.
  static_assert(!PassesMoveOnlyTypeByCopy<Arg, Param>::value,
...
struct AssertConstructible<RepeatMode::Repeating, i, Arg, Unwrapped, Param> {
 private:
  // TODO(dcheng): Figure out why the test case isn't working now...
  static_assert(!PassesMoveOnlyTypeByCopy<Unwrapped, Param>::value,
                "Bound argument |i| is a move-only, but is being passed by "
