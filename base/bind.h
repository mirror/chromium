...
struct AssertConstructible {
 private:
  ...
  static_assert(
      arg_is_storable || std::is_rvalue_reference<Arg>::value ||
          !std::is_constructible<
              Storage,
              std::add_rvalue_reference_t<std::remove_reference_t<Arg>>>::value,
  ...
  // Unlike the previous check for binding into storage, the check for
  ...
  static_assert(
      param_is_forwardable || std::is_rvalue_reference<Arg>::value ||
          !std::is_constructible<Param,
                                 std::add_rvalue_reference_t<std::conditional_t<
                                     is_once,
                                     std::remove_reference_t<Unwrapped>,
...
template <bool is_once,
          ...
          typename... Unwrapped,
          typename... Params>
struct AssertBindArgsValidity<is_once,
                              std::index_sequence<Ns...>,
                              TypeList<Args...>,
                              TypeList<Unwrapped...>,
                              TypeList<Params...>>
    : AssertConstructible<is_once,
                          Ns,
                          Args,
                          std::decay_t<Args>,
                          Unwrapped,
...
BindOnce(Functor&& functor, Args&&... args) {
  ...
  static_assert(internal::AssertBindArgsValidity<
                    true, std::make_index_sequence<Helper::num_bounds>,
                    BoundArgsList, UnwrappedArgsList, BoundParamsList>::ok,
...
BindRepeating(Functor&& functor, Args&&... args) {
  ...
  using BoundParamsList = typename Helper::BoundParamsList;
  static_assert(internal::AssertBindArgsValidity<
                    false, std::make_index_sequence<Helper::num_bounds>,
                    BoundArgsList, UnwrappedArgsList, BoundParamsList>::ok,
                "The bound args need to be convertible to the target params.");
