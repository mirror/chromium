...
TEST_F(ScopedBlockingCallTest, InvalidDestructionOrder) {
  EXPECT_CALL(observer_, BlockingScopeEntered(BlockingType::WILL_BLOCK));
  auto scoped_blocking_call_a = std::make_unique<ScopedBlockingCall>(BlockingType::WILL_BLOCK);
  auto scoped_blocking_call_b = std::make_unique<ScopedBlockingCall>(BlockingType::WILL_BLOCK);

  EXPECT_DCHECK_DEATH({
    scoped_blocking_call_a.reset();
  });
