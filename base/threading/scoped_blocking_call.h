...
class BASE_EXPORT BlockingObserver {
 public:
  ...
  // Invoked when a ScopedBlockingCall is instantiated on a thread where there
  // wasn't an existing ScopedBlockingCall.
  virtual void BlockingStarted(BlockingType blocking_type) = 0;

  // Invoked when a WILL_BLOCK ScopedBlockingCall is instantiated on a thread
  // where there was a MAY_BLOCK ScopedBlockingCall but not a WILL_BLOCK
  // ScopedBlockingCall.
