...
TEST(SequenceLocalStorageSlotMultipleMapTest, SetGetMultipleMapsOneSlot) {
  ...
  // Create vector of |kNumberMaps| SequenceLocalStorageMaps
  std::vector<internal::SequenceLocalStorageMap> sequence_local_storage_map(
      kNumberMaps);

  // Set the value of the slot to be the index of the current
  // SequenceLocalStorageMap in the vector
  for (int i = 0; i < kNumberMaps; ++i) {
    internal::ScopedSetSequenceLocalStorageMapForCurrentThread
        scoped_sequence_local_storage(&sequence_local_storage_map[i]);
