...
class Future {
 public:
  ...
  Future<R> Chain(scoped_refptr<TaskRunner> task_runner,
    ...
    auto relay = [](OnceCallback<R(Value)> handler, Promise<R> chained,
                    const Location& from_here, Value value) {
      chained.Resolve(from_here, std::move(handler).Run(std::move(value)));
    };

    std::move(*this).Then(
        std::move(task_runner),
        BindOnce(relay, std::move(handler), std::move(promise)));
    return future;
  }

  template <typename R>
  Future<R> Chain(OnceCallback<R(Value)> handler) && {
    Future<R> future;
    Promise<R> promise;
    std::tie(future, promise) = Contract<R>();
