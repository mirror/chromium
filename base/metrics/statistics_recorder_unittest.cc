...
TEST_P(StatisticsRecorderTest, RegisterHistogram) {
  ...
  EXPECT_THAT(StatisticsRecorder::GetHistograms(), ElementsAre());
...
TEST_P(StatisticsRecorderTest, GetSnapshot) {
  ...
  EXPECT_THAT(StatisticsRecorder::GetSnapshot("hello"), SizeIs(0));
}
...
TEST_P(StatisticsRecorderTest, BucketRangesSharing) {
  EXPECT_THAT(StatisticsRecorder::GetBucketRanges(), SizeIs(0));
...
TEST_P(StatisticsRecorderTest, IterationTest) {
  ...
  StatisticsRecorder::ImportGlobalPersistentHistograms();
  EXPECT_THAT(StatisticsRecorder::GetKnownHistograms(false), SizeIs(0));
}
