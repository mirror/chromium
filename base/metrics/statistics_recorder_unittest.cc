...
TEST_P(StatisticsRecorderTest, IterationTest) {
  ...
  EXPECT_EQ(2U, StatisticsRecorder::KnownHistograms(true).size());
  EXPECT_EQ(use_persistent_histogram_allocator_ ? 0U : 2U,
            StatisticsRecorder::KnownHistograms(false).size());
  ...
  EXPECT_EQ(use_persistent_histogram_allocator_ ? 2U : 0U,
            StatisticsRecorder::KnownHistograms(true).size());

  StatisticsRecorder::ImportGlobalPersistentHistograms();
  EXPECT_EQ(0U, StatisticsRecorder::KnownHistograms(false).size());
}
