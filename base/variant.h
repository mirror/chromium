...
struct VariantHelper<N> {
  ...
  static void Move(size_t idx, void* from, void* to) {}

  static void Copy(size_t idx, void* from, void* to) {}
};
...
struct VariantHelper<N, First, Rest...> {
  ...
  static void Copy(size_t idx, void* from, void* to) {
    if (idx == N) {
      *reinterpret_cast<First*>(to) = *reinterpret_cast<First*>(from);
    } else {
...
class Variant {
 public:
  constexpr Variant() noexcept : tag_(0) {
  ...
  constexpr Variant(Variant& other) noexcept {
    tag_ = other.tag_;
    VariantHelper<0, Ts...>::Copy(tag_, &other.union_[0], &union_[0]);
  }

  constexpr Variant(Variant&& other) noexcept {
    tag_ = other.tag_;
    VariantHelper<0, Ts...>::Move(tag_, &other.union_[0], &union_[0]);
  }

  template <typename Arg>
  constexpr explicit Variant(Arg&& a) noexcept {
    static_assert(
        VarArgIndexHelper<Arg, Ts...>::kIndex != kVarArgIndexHelperInvalidIndex,
  ...
  ~Variant() { VariantHelper<0, Ts...>::Destruct(tag_, &union_[0]); }
  ...
  constexpr Type& emplace(Args&&... args) {
    ...
    VariantHelper<0, Ts...>::Destruct(tag_, &union_[0]);
  ...
  constexpr size_t index() const noexcept { return tag_; }
...
constexpr T& Get(Variant<Ts...>* variant) {
  DCHECK_EQ((VarArgIndexHelper<T, Ts...>::kIndex), variant->tag_);
  return *reinterpret_cast<T*>(&variant->union_[0]);
}
