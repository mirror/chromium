...
class BASE_EXPORT Variant {
 public:
  ...
  template <size_t I, class... Args>
  constexpr internal::TypeAtHelper<I, Ts...>& emplace(in_place_index_t<I>,
                                                      Args&&... args) noexcept {
    Variant<Ts...>::type_ops_[index_].destruct(&union_[0]);
    index_ = I;
    return *new (&union_[0])
 ...
 private:
  static const internal::VariantTypeOps type_ops_[sizeof...(Ts)];
...
template <typename... Ts>
const internal::VariantTypeOps Variant<Ts...>::type_ops_[] = {
    internal::VariantTypeOps::Create<Ts>()...};

template <typename... Ts>
const internal::VariantInfo Variant<Ts...>::variant_info_ = {
 ...
 public:
  ...
  AbstractVariant(ConstructWith<Ts...>)
      : type_ops_(Variant<Ts...>::type_ops_),
        ...
        index_(0) {
    union_.reserve(internal::VariantSizeOfHelper<Ts...>::kSize);
  ...
  int type_id() const { return type_ops_[index_].type_id; }

  template <typename T>
  bool TryAssign(T&& other) {
    int other_type_id = internal::TypeId<std::decay_t<T>>::Id();
  ...
  bool TryEmplace(Args&&... args) {
    int variant_type_id = internal::TypeId<T>::Id();
    for (size_t i = 0; i < num_types_; i++) {
  ...
  bool TryAssign(AbstractVariant& other) {
    int other_type_id = other.type_id();
    for (size_t i = 0; i < num_types_; i++) {
      if (type_ops_[i].type_id == other_type_id) {
        type_ops_[index_].destruct(&union_[0]);
        index_ = i;
  ...
  bool TryAssignOrUnwrapAndEmplace(AbstractVariant& other, size_t i) {
    ...
    int other_type_id = other_variant_info->type_id;
...
constexpr T& Get(Variant<Ts...>* variant) {
  ...
  DCHECK_EQ((internal::VarArgIndexHelper<T, Ts...>::kIndex), variant->index_);
  return *reinterpret_cast<T*>(&variant->union_[0]);
}
