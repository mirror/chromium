...
#if defined(OS_MACOSX)
  int nosigpipe = 1;
  if (setsockopt(raw_socks[0], SOL_SOCKET, SO_NOSIGPIPE, &nosigpipe,
                 sizeof(nosigpipe)) != 0)
    return false;
  if (setsockopt(raw_socks[1], SOL_SOCKET, SO_NOSIGPIPE, &nosigpipe,
                 sizeof(nosigpipe)) != 0)
    return false;
#endif
...
ssize_t UnixDomainSocket::RecvMsgWithFlags(int fd,
  ...
  if (msg.msg_controllen > 0) {
    ...
    for (cmsg = CMSG_FIRSTHDR(&msg); cmsg; cmsg = CMSG_NXTHDR(&msg, cmsg)) {
      ...
      if (cmsg->cmsg_level == SOL_SOCKET &&
        ...
        DCHECK_EQ(pid, -1);
        pid = reinterpret_cast<struct xucred*>(CMSG_DATA(cmsg))->pid;
      }
  ...
  if (out_pid) {
    ...
    getsockopt(fd, SOL_LOCAL, LOCAL_PEERPID, &pid, &pid_size);
#else
