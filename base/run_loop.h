 ...
 public:
  ...
  class BASE_EXPORT Delegate {
   ...
   protected:
    ...
    // probing this when it becomes idle. "protected" so it can be invoked
    // directly by the Delegate itself from the member variable (instead of
    // being returned at registration which enables it to be overridden in
    // override scenarios).
    using ShouldQuitWhenIdleCallback = RepeatingCallback<bool(void)>;
    RepeatingCallback<bool(void)> should_quit_when_idle_callback_;
