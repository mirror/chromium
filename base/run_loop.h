  ...
  class BASE_EXPORT Delegate {
   ...
   protected:
    ...
    enum RunLevelTaskType {
      // Within the realm of control of the Delegate, no (more) tasks should be
      // allowed at this run-level (non-sensical as a value for Delegate::Run(),
      // but can be used as a reentrancy blocker when an impl wants to flag that
      // a task is in progress and is at risk of reentrancy from system
      // generated nested loops).
      kNoTasks = 0,
