 ...
 public:
  ...
  class Iter {
   public:
    ...
    ObserverType* operator->() const { return &**this; }
   ...
   private:
    ...
    ObserverType* GetCurrent() const {
      return list_ && index_ < clamped_max_index() ? list_->observers_[index_]
                                                   : nullptr;
    }
...
class ObserverList : public ObserverListBase<ObserverType> {
  using Base = ObserverListBase<ObserverType>;

 public:
  using Base::Base;
  using typename Base::NotificationType;

  ~ObserverList() {
    // When check_empty is true, assert that the list is empty on destruction.
    if (check_empty) {
      Base::Compact();
      DCHECK_EQ(Base::size(), 0U);
    }
  }

  bool might_have_observers() const { return Base::size() != 0; }
};
