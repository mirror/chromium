...
// Optional defines copy/move constructors/assign-operators conditionally.
// Implementation is as follows;
// - In OptionalBase<T>, all constructors/assign operators are defined as if
//   all of them are supported.
// - {Copy,Move}{Constructible,Assignable} structs mark "delete" if it shuold
//   be deleted.
// - Optional inherits all of them, and marks default/copy/move constructors
//   and copy/move assign-operators "default", so these are conditionally
//   defined.
template <typename T>
class OptionalBase {
 public:
  ...
  // Standard spec has conditional noexcept, however in chromium it is
  // prohibited to use exception, so here is marked noexcept unconditionally.
  constexpr OptionalBase(OptionalBase&& other) noexcept {
    if (!other.storage_.is_null_)
      Init(std::move(other.storage_.value_));
  }

  // This constructor is necessary to support constexpr. Just forwards args
  // to OptionalStorage.
  ...
  // Similar to move constructor, mark noexcept unconditionally.
...
// Defines strcuts to delete specific constructor/assign-operator
// conditionally. Note that because marking "delete" can affect other
// constructors assign-operators, so it is necessary to mark other constructors
// (including default constructor) and assign operators "default".
...
class Optional
 ...
 public:
  ...
  // Default/copy/move constructors and copy/move assign-operators are
  // implemented in OptionalBase, and whether these are deleted or not
  // is controled by {Copy,Move}{Constructible,Assignable} structs.
  ...
  // Conditionally mark "excplicit" following the spec.
  template <class U,
  ...
  template <
      class... Args,
      std::enable_if_t<std::is_constructible<T, Args...>::value, bool> = false>
  ...
  template <class U>
  std::enable_if_t<!std::is_same<std::decay_t<U>, Optional<T>>::value &&
                       std::is_constructible<T, U>::value &&
  ...
  template <class U>
  std::enable_if_t<!internal::IsOptionalAssignable<T, U>::value &&
  ...
  // Marking noexcept unconditionally, similar to move
 ...
 private:
  // Template base class is not a target of identifier resolution by default.
  // Declare them so that these can be used in implementation above.
...
// Here after defines comparation operators.
