...
TEST(PickleTest, FindNext) {
  ...
  EXPECT_TRUE(nullptr == Pickle::FindNext(pickle.header_size_, start, end - 1));
...
TEST(PickleTest, FindNextWithIncompleteHeader) {
  ...
  EXPECT_TRUE(nullptr == Pickle::FindNext(header_size, start, end));
}
...
TEST(PickleTest, FindNextOverflow) {
  ...
  EXPECT_TRUE(nullptr == Pickle::FindNext(header_size2, start, end));
