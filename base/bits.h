...
ALWAYS_INLINE constexpr
  ...
  unsigned long index;
  return (bits == 64) ? LIKELY(_BitScanReverse64(&index, x)) ? (63 - index) : 64
                      : LIKELY(_BitScanReverse(&index, x)) ? (31 - index) : 32;
}
...
inline constexpr
  ...
  static_assert(bits > 0, "invalid instantiation");
  unsigned long index;
  return (bits == 64) ? LIKELY(_BitScanForward64(&index, x)) ? index : 64
                      : LIKELY(_BitScanForward(&index, x)) ? index : 32;
}
