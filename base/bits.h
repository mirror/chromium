...
ALWAYS_INLINE
    typename std::enable_if<std::is_unsigned<T>::value && sizeof(T) <= 32,
                            unsigned>::type
    CountLeadingZeroBits(T x) {
  static_assert(bits > 0, "invalid instantiation");
  unsigned long index;
  return LIKELY(_BitScanReverse(&index, x)) ? (31 - index) : bits;
}
...
template <typename T, unsigned bits = sizeof(T) * 8>
ALWAYS_INLINE
    typename std::enable_if<std::is_unsigned<T>::value && sizeof(T) == 64,
                            unsigned>::type
    CountLeadingZeroBits(T x) {
  unsigned long index;
...
ALWAYS_INLINE
  ...
  static_assert(bits > 0, "invalid instantiation");
  unsigned long index;
  return LIKELY(_BitScanForward(&index, x)) ? index : bits;
}
...
ALWAYS_INLINE
    ...
    CountTrailingZeroBits(T x) {
  unsigned long index;
  return LIKELY(_BitScanForward64(&index, x)) ? index : 64;
}
