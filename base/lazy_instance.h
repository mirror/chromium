...
// If |state| is uninitialized, constructs a value using |constructor|, stores
...
void* GetOrCreateLazyPointer(subtle::AtomicWord* state,
  ...
  if (!(value & kLazyInstanceCreatedMask) && NeedsLazyInstance(state)) {
    ...
    value = reinterpret_cast<subtle::AtomicWord>(constructor());
...
#endif
    return static_cast<Type*>(GetOrCreateLazyPointer(
        &private_instance_,
