...
class LazyInstance {
 public:
  ...
  bool IsCreated() {
    // Return false (i.e. "not created") if |private_instance_| has not yet been
    // created. Return true (i.e. "created") if |private_instance_| is either
    // being created right now (internal::kLazyInstanceStateCreating) or was
    // already created (any other non-zero value).
