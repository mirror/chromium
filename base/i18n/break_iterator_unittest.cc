...
TEST(BreakIteratorTest, FlagEmoji) {
  {
    base::string16 text(UTF8ToUTF16("abcdefgh"));
    BreakIterator iter(text, BreakIterator::BREAK_CHARACTER);
    ASSERT_TRUE(iter.Init());

    for (int i : {1, 2, 3, 4, 5, 6, 7, 8})
      EXPECT_TRUE(iter.IsGraphemeBoundary(i)) << i;

    ASSERT_TRUE(iter.Init());
    EXPECT_TRUE(iter.Advance());
    EXPECT_EQ(UTF8ToUTF16("a"), iter.GetString());
    EXPECT_TRUE(iter.Advance());
    EXPECT_EQ(UTF8ToUTF16("b"), iter.GetString());
    EXPECT_TRUE(iter.Advance());
    EXPECT_EQ(UTF8ToUTF16("c"), iter.GetString());
    EXPECT_TRUE(iter.Advance());
    EXPECT_EQ(UTF8ToUTF16("d"), iter.GetString());
    EXPECT_TRUE(iter.Advance());
    EXPECT_EQ(UTF8ToUTF16("e"), iter.GetString());
    EXPECT_TRUE(iter.Advance());
    EXPECT_EQ(UTF8ToUTF16("f"), iter.GetString());
    EXPECT_TRUE(iter.Advance());
    EXPECT_EQ(UTF8ToUTF16("g"), iter.GetString());
  }
