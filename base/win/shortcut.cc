...
bool CreateOrUpdateShortcutLink(const FilePath& shortcut_path,
  ...
  bool has_clsid =
      (properties.options & ShortcutProperties::PROPERTIES_CLSID) != 0;

  if (has_app_id || has_dual_mode || has_clsid) {
...
bool ResolveShortcutProperties(const FilePath& shortcut_path,
  ...
  if (options & (ShortcutProperties::PROPERTIES_APP_ID |
                 ...
                 ShortcutProperties::PROPERTIES_CLSID)) {
    ComPtr<IPropertyStore> property_store;
    if (FAILED(i_shell_link.CopyTo(property_store.GetAddressOf())))
      return false;
    ...
    if (options & ShortcutProperties::PROPERTIES_CLSID) {
      ScopedPropVariant pv_clsid;
      if (property_store->GetValue(PKEY_AppUserModel_ToastActivatorCLSID,
                                   pv_clsid.Receive()) != S_OK) {
        return false;
      }
      switch (pv_clsid.get().vt) {
        case VT_EMPTY:
          properties->set_clsid(GUID_NULL);
          break;
        case VT_CLSID:
          properties->set_clsid(*(pv_clsid.get().puuid));
          break;
        default:
          NOTREACHED() << "Unexpected variant type: " << pv_clsid.get().vt;
          return false;
      }
    }
  }
