...
HMODULE ComBaseUtil::combase_dll_ = nullptr;
...
bool ComBaseUtil::PreloadWindowsCreateStringReference() {
  ...
  return GetProcAddress<decltype(&::WindowsCreateStringReference)>(
      combase_dll_, &create_string_ref_func_, "WindowsCreateStringReference");
}
...
bool ComBaseUtil::PreloadFunctions(const std::vector<std::string>& functions) {
  ...
  return functions_loaded == function_count;
}
...
// static
HMODULE ComBaseUtil::GetLibraryForTesting() {
  return combase_dll_;
}
