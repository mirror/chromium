...
bool IsWindows10TabletMode(HWND hwnd) {
  ...
  HMODULE module = base::win::ComBaseUtil::EnsureLibraryLoaded();
  if (!module)
    return false;

  bool preload_success =
      base::win::ComBaseUtil::PreloadRoGetActivationFactory() &&
      base::win::ComBaseUtil::PreloadWindowsCreateString();
  if (!preload_success) {
    CHECK(false);
    return false;
  }
  ...
  if (!view_settings_guid) {
    hr = base::win::ComBaseUtil::WindowsCreateString(
  ...
  hr = base::win::ComBaseUtil::RoGetActivationFactory(
      view_settings_guid, IID_PPV_ARGS(&view_settings_interop));
  if (FAILED(hr))
    return false;
