...
// 32-bit jump anywhere in the 32-bit address space.
//
...
class HookManager {
 public:
  static HookManager* GetInstance() {
    return Singleton<HookManager, LeakySingletonTraits<HookManager>>::get();
  }
 ...
 private:
  friend struct DefaultSingletonTraits<HookManager>;
  ...
  void WriteHook() {
    ...
    DCHECK(!combase_library_);
    combase_library_ = ::LoadLibrary(L"combase.dll");
    ...
    co_create_instance_padded_address_ =
        reinterpret_cast<uint32_t>(
            GetProcAddress(combase_library_, "CoCreateInstance")) -
        5;
  ...
  void RevertHook() {
    ...
    if (combase_library_) {
      ::FreeLibrary(combase_library_);
      combase_library_ = nullptr;
    }
  ...
  HMODULE combase_library_ = nullptr;
