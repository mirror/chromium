...
void InitializeClock() {
  g_initial_ticks = TimeTicksNowIgnoringOverride();
...
Time base::subtle::TimeNowIgnoringOverride() {
  if (g_initial_time == 0)
    InitializeClock();
  ...
  // clock.
  while (true) {
    TimeTicks ticks = base::subtle::TimeTicksNowIgnoringOverride();
...
Time base::subtle::TimeNowFromSystemTimeIgnoringOverride() {
  // Force resync.
  InitializeClock();
...
bool Time::ActivateHighResolutionTimer(bool activating) {
  ...
  if (activating) {
    ...
    if (g_high_res_timer_count == 1) {
      g_high_res_timer_last_activation = TimeTicksNowIgnoringOverride();
    ...
    if (g_high_res_timer_count == 0) {
      g_high_res_timer_usage +=
          TimeTicksNowIgnoringOverride() - g_high_res_timer_last_activation;
...
void Time::ResetHighResolutionTimerUsage() {
  ...
  g_high_res_timer_usage_start = TimeTicksNowIgnoringOverride();
  if (g_high_res_timer_count > 0)
...
double Time::GetHighResolutionTimerUsage() {
  base::AutoLock lock(*GetHighResLock());
  TimeTicks now = TimeTicksNowIgnoringOverride();
...
void InitializeNowFunctionPointer() {
  ...
  if (base::internal::g_time_ticks_now_function ==
      &base::subtle::TimeTicksNowIgnoringOverride) {
    base::internal::g_time_ticks_now_function = now_function;
  }
...
TimeTicks base::subtle::TimeTicksNowIgnoringOverride() {
  return g_time_ticks_now_ignoring_override_function();
}
...
ThreadTicks base::subtle::ThreadTicksNowIgnoringOverride() {
