...
void InitializeClock() {
  g_initial_ticks = base::subtle::TimeTicksNowIgnoringOverride();
...
bool Time::ActivateHighResolutionTimer(bool activating) {
  ...
  if (activating) {
    ...
    if (g_high_res_timer_count == 0) {
      g_high_res_timer_usage += base::subtle::TimeTicksNowIgnoringOverride() -
                                g_high_res_timer_last_activation;
...
void Time::ResetHighResolutionTimerUsage() {
  ...
  g_high_res_timer_usage_start = base::subtle::TimeTicksNowIgnoringOverride();
  if (g_high_res_timer_count > 0)
