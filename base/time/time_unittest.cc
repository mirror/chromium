...
TEST(TimeDelta, FromAndIn) {
  ...
  static_assert(
      ...
      "");
  static_assert(TimeDelta::FromDays(13).InDays() == 13, "");
  ...
  static_assert(TimeDelta::FromMilliseconds(13).InMilliseconds() == 13, "");
  static_assert(TimeDelta::FromMilliseconds(13).InMillisecondsF() == 13.0, "");
  static_assert(TimeDelta::FromSecondsD(13.1).InSeconds() == 13, "");
  static_assert(TimeDelta::FromSecondsD(13.1).InSecondsF() == 13.1, "");
  static_assert(TimeDelta::FromMillisecondsD(13.3).InMilliseconds() == 13, "");
  static_assert(TimeDelta::FromMillisecondsD(13.3).InMillisecondsF() == 13.3,
                "");
  static_assert(TimeDelta::FromMicroseconds(13).InMicroseconds() == 13, "");
  static_assert(
      TimeDelta::FromMillisecondsD(3.45678).InMillisecondsF() == 3.456, "");
}
...
TEST(TimeDelta, MaxConversions) {
  ...
  constexpr TimeDelta max_t = TimeDelta::Max();
  static_assert(max_t.ToInternalValue() == std::numeric_limits<int64_t>::max(),
                "");
  static_assert(max_t.InDays() == std::numeric_limits<int>::max(), "");
  static_assert(max_t.InHours() == std::numeric_limits<int>::max(), "");
  static_assert(max_t.InMinutes() == std::numeric_limits<int>::max(), "");
  static_assert(max_t.InSecondsF() == std::numeric_limits<double>::infinity(),
                "");
  static_assert(max_t.InSeconds() == std::numeric_limits<int64_t>::max(), "");
  static_assert(
      max_t.InMillisecondsF() == std::numeric_limits<double>::infinity(), "");
  static_assert(max_t.InMilliseconds() == std::numeric_limits<int64_t>::max(),
                "");
  static_assert(
      max_t.InMillisecondsRoundedUp() == std::numeric_limits<int64_t>::max(),
      "");
  ...
  // Floating point arithmetic resulting in infinity cannot be constexpr in
  // C++14.
...
TEST(TimeDeltaLogging, EmptyIsZero) {
  constexpr TimeDelta zero;
  EXPECT_EQ("0 s", AnyToString(zero));
}

TEST(TimeDeltaLogging, FiveHundredMs) {
  constexpr TimeDelta five_hundred_ms = TimeDelta::FromMilliseconds(500);
  EXPECT_EQ("0.5 s", AnyToString(five_hundred_ms));
}

TEST(TimeDeltaLogging, MinusTenSeconds) {
  constexpr TimeDelta minus_ten_seconds = TimeDelta::FromSeconds(-10);
