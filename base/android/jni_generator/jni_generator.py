...
def GetRegisterName(fully_qualified_class):
  """Returns the register name with a given class."""
  template = Template('register_native_${JAVA_NAME}')
  value = {
      'JAVA_NAME':
          fully_qualified_class.replace('_', '_1').replace('/', '_')
  }
...
""")
  ...
  # TODO(crbug / 683256): remove this function when deleting original registers.
  def GetOriginalRegisterNativesString(self):
    ...
    return """
// TODO(crbug/683256): Remove these empty registration functions and the
...
""")
    values = {
        'REGISTER_NAME': GetRegisterName(self.fully_qualified_class),
        'NATIVES': natives
    }
