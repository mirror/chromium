...
""")
  ...
  def GetCName(self, native):
    # To be changed to "JNI_" + native.java_class_name + "__" + native.name
    if native.java_class_name is not None:
      # Inner class
      return native.java_class_name + "__" + native.name
    return self.class_name + "__" + native.name
  ...
  def GetNativeStub(self, native):
    ...
    values = {
        ...
        'NAME': native.name,
        'SHORT_JAVA_NAME': self.GetCName(native),
...
""")
      ...
      template = Template("""
static ${RETURN_DECLARATION} ${SHORT_JAVA_NAME}(JNIEnv* env, ${PARAMS});

JNI_GENERATOR_EXPORT ${RETURN} ${STUB_NAME}(JNIEnv* env, ${PARAMS_IN_STUB}) {
  ${PROFILING_ENTERED_NATIVE}
  return ${SHORT_JAVA_NAME}(${PARAMS_IN_CALL})${POST_CALL};
}
""")
