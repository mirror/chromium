...
def GenerateJNIHeader(java_file_paths, output_file, args):
  ...
  for path in java_file_paths:
    ...
    header_generator = HeaderGenerator(fully_qualified_class, dict, main_dex)
    dict = header_generator.GetContent()

  include_file = ''
  if args.includes:
    template = string.Template('#include "${INCLUDE_FILE}"')
    value = {'INCLUDE_FILE': args.includes}
    include_file = template.substitute(value)
...
class HeaderGenerator(object):
  ...
  def __init__(self, fully_qualified_class, dict, main_dex):
  ...
  def _AddForwardDeclaration(self):
    ...
    value = {
        'METHOD_NAME':
            jni_generator.GetRegisterName(self.fully_qualified_class)
    }
    self.dict['FORWARD_DECLARATIONS'] += template.substitute(value)
...
""")
    ...
    if self.main_dex:
      self.dict['REGISTER_MAIN_DEX_NATIVES'] += template.substitute(value)
    else:
      self.dict['REGISTER_NON_MAIN_DEX_NATIVES'] += template.substitute(value)
