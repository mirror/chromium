...
public class LibraryLoader {
    ...
    private static String makeLibraryPathInZipFile(String library, String zipFilePath) {
        ...
        // Determine whether the process is running in 32bit mode.
        boolean is32BitProcess;
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            // There was no support for 64bit before L.
            is32BitProcess = true;
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            is32BitProcess = !Process.is64Bit();
        } else {
            try {
                Class<?> vmRunTimeClass = Class.forName("dalvik.system.VMRuntime");
                Object runtime = vmRunTimeClass.getMethod("getRuntime").invoke(null);
                is32BitProcess = Boolean.FALSE.equals(
                        (Boolean) vmRunTimeClass.getMethod("is64Bit").invoke(runtime));
            } catch (Exception e) {
        ...
        switch (NativeLibraries.sCpuFamily) {
            ...
            case NativeLibraries.CPU_FAMILY_MIPS:
                ...
                break;
            default:
                Log.w(TAG, "Unknown CPU ABI for native libraries, fallback to armeabi-v7a");
                cpuAbi = "armeabi-v7a";
        }
