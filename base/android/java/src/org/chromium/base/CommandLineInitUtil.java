...
public final class CommandLineInitUtil {
    ...
    public static void initCommandLine(String fileName) {
        ...
        // shouldUseDebugCommandLine() uses IPC, so it is likely faster to check the filesystem
        // first.
        if (!commandLineFile.exists() || !shouldUseDebugCommandLine()) {
            commandLineFile = new File(COMMAND_LINE_FILE_PATH, fileName);
        }
        Log.i(TAG, "Initializing command line from %s", commandLineFile);
