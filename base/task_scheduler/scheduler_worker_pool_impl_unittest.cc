...
class TaskSchedulerWorkerPoolMayBlockTest
    : public TaskSchedulerWorkerPoolBlockingEnterExitTest {
 protected:
  TaskSchedulerWorkerPoolMayBlockTest() = default;

 private:
  DISALLOW_COPY_AND_ASSIGN(TaskSchedulerWorkerPoolMayBlockTest);
};
...
// does not increase.
TEST_F(TaskSchedulerWorkerPoolMayBlockTest, ThreadBlockUnblockPremature) {
  ASSERT_EQ(worker_pool_->GetWorkerCapacityForTesting(),
            kNumWorkersInWorkerPool);
