...
class BASE_EXPORT SchedulerWorkerPoolImpl : public SchedulerWorkerPool {
 ...
 private:
  ...
  // this function.
  void WaitForWorkersIdleWithLockForTesting(size_t n);
  ...
  void WakeUpOneWorkerWithLock();

  // Adds a worker, if needed, to maintain one idle worker, |worker_capacity_|
  // permitting.
  void MaintainAtLeastOneIdleWorkerWithLock();

  // Adds |worker| to |idle_workers_stack_|.
  void AddToIdleWorkersStackWithLock(SchedulerWorker* worker);

  // Peeks from |idle_workers_stack_|.
  const SchedulerWorker* PeekAtIdleWorkersStackWithLock() const;

  // Removes |worker| from |idle_workers_stack_|.
  void RemoveFromIdleWorkersStackWithLock(SchedulerWorker* worker);
  ...
  SchedulerWorker* CreateRegisterAndStartSchedulerWorkerWithLock();

  // Returns the number of workers in the pool that should not run tasks due to
  // the pool being over worker capacity.
  size_t NumberOfWorkersThatCantRunTasksWithLock() const;
