...
class BASE_EXPORT SchedulerWorkerPoolImpl : public SchedulerWorkerPool {
 ...
 private:
  ...
  // Examines the list of SchedulerWorkers for blocked tasks to check for and
  ...
  // Decrements the worker capacity.
  void DecrementWorkerCapacityLockRequired();

  // Increments the worker capacity.
  ...
  // but haven't caused a capacity increase yet.
