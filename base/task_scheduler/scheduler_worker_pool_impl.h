...
class BASE_EXPORT SchedulerWorkerPoolImpl : public SchedulerWorkerPool {
 ...
 private:
  ...
  // Returns the number of workers that need to become idle due to the pool
  // being over worker capacity. This value may be negative to indicate the
  // number of extra idle workers on the stack beyond what's required for being
  // over worker capacity.
  int NumberOfWorkersToBecomeIdle() const;
