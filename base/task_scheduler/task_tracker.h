...
class BASE_EXPORT TaskTracker {
 public:
  ...
  // Returns and calls |flush_callback_| when there are no incompletely
  // undelayed tasks. |flush_callback_| may be called back on any thread and
  // should not perform a lot of work. May be used when additional work on the
  // current thread needs to be performed during a flush. Only one FlushAsync()
  // may be pending at any given time.
  void FlushAsync(OnceClosure flush_callback_);
 ...
 private:
  ...
  // Calls |flush_callback_| if one is available in a lock-safe manner.
  void CallFlushCallback();
  ...
  // sleeping at the right time. Fully synchronizes access to |flush_callback_|.
  ...
  // Called when |num_incomplete_undelayed_tasks_| is zero or when shutdown
  // completes.
  OnceClosure flush_callback_;
