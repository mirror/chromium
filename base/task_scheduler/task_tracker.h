...
class BASE_EXPORT TaskTracker {
 public:
  // Indicates whether a std::unique_ptr<Task> should be reset at the conclusion
  // of executing the task. This allows callers to release the task without
  // having to call DestroyTask().
  enum class TaskResetPolicy { kReset, kDoNotReset };
  ...
  // Runs |task|. |sequence| is generally (but is not required to be) the
  // sequence from which |task| was extracted and establishes the execution
  // environment for the task (task runner handles, sequence local storage,
  // etc.). An override is expected to call its parent's implementation but is
  // free to perform extra work before and after doing so.
  virtual void RunTask(
      Task* task,
      Sequence* sequence,
      TaskResetPolicy task_reset_policy = TaskResetPolicy::kReset);

  // Destroys |task| with an appropriate execution environment.
 ...
 private:
  ...
  //    task from it):
  //    - the next preempted sequence (if any) is scheduled.
