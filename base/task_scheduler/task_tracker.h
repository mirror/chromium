...
class BASE_EXPORT TaskTracker {
 public:
  ...
  // Runs the next task in the |sequence| unless the current shutdown state
  // prevents that. Returns true if |task| ran and outputs true in
  // |sequence_became_empty| if after running the task, the sequence was empty.
  // WillPostTask() must have allowed |task| to be posted before this is called.
  bool RunNextTask(Sequence* sequence, bool* sequence_became_empty);
