...
// decremented when the same instances are brought down (never in production and
// when the task environment is teared down in unit tests). This makes the
...
// such callers know they should complete necessary work synchronously.
// Note: |!g_active_pools_count| is generally equivalent to
