...
void SchedulerWorkerPool::PostTaskWithSequenceNow(
  ...
  if (sequence_was_empty) {
    // Try to dispatch |sequence| if it was empty before |task| was inserted
    // into it. Otherwise, one of these must be true:
    // - |sequence| is already dispatched, or,
    // - The pool is running a Task from |sequence|. The pool is expected to
    //   redispatch the |sequence| once it's done running the Task.
    sequence = task_tracker_->WillDispatchSequence(std::move(sequence), this);
    if (sequence)
      OnCanDispatchSequence(std::move(sequence));
  }
}
