...
void CALLBACK SchedulerWorkerPoolWindowsImpl::RunNextSequence(
    ...
    PTP_WORK) {
  auto* worker_pool = static_cast<SchedulerWorkerPoolWindowsImpl*>(
...
scoped_refptr<Sequence> SchedulerWorkerPoolWindowsImpl::GetWork() {
  ...
  // a call to ScheduleSequence() and GetWork()/SubmitThreadpoolWork().
  DCHECK(!transaction->IsEmpty());
  return transaction->PopSequence();
}

void SchedulerWorkerPoolWindowsImpl::ScheduleSequence(
