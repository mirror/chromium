...
class ControllableRemovalDelegate : public SchedulerWorkerDefaultDelegate {
 public:
  class Controls : public RefCountedThreadSafe<Controls> {
   ...
   private:
    friend class ControllableRemovalDelegate;
  ...
  ControllableRemovalDelegate(TaskTracker* task_tracker)
      : task_tracker_(task_tracker), controls_(new Controls()) {}

  ~ControllableRemovalDelegate() override { controls_->destroyed_.Signal(); }
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(ControllableRemovalDelegate);
};

class MockedControllableRemovalDelegate : public ControllableRemovalDelegate {
 public:
  MockedControllableRemovalDelegate(TaskTracker* task_tracker)
      : ControllableRemovalDelegate(task_tracker){};
  ~MockedControllableRemovalDelegate() = default;

  // SchedulerWorker::Delegate:
  MOCK_METHOD1(OnMainEntry, void(SchedulerWorker* worker));

 private:
  DISALLOW_COPY_AND_ASSIGN(MockedControllableRemovalDelegate);
};
...
TEST(TaskSchedulerWorkerTest, WorkerRemoval) {
  ...
  MockedControllableRemovalDelegate* delegate =
      new StrictMock<MockedControllableRemovalDelegate>(&task_tracker);
  scoped_refptr<ControllableRemovalDelegate::Controls> controls =
      delegate->controls();
...
TEST(TaskSchedulerWorkerTest, WorkerCleanupBeforeRemoval) {
  ...
  std::unique_ptr<ControllableRemovalDelegate> delegate =
      MakeUnique<ControllableRemovalDelegate>(&task_tracker);
  scoped_refptr<ControllableRemovalDelegate::Controls> controls =
      delegate->controls();
