 ...
 public:
  class Controls : public RefCountedThreadSafe<Controls> {
   public:
    Controls()
        : work_running_(WaitableEvent::ResetPolicy::MANUAL,
          ...
          cleanup_requested_(WaitableEvent::ResetPolicy::MANUAL,
                             WaitableEvent::InitialState::NOT_SIGNALED),
          cleaned_up_(WaitableEvent::ResetPolicy::MANUAL,
                      WaitableEvent::InitialState::NOT_SIGNALED),
          can_cleanup_block_(WaitableEvent::ResetPolicy::MANUAL,
                             WaitableEvent::InitialState::SIGNALED),
    ...
    void MakeCanCleanupBlock() { can_cleanup_block_.Reset(); }

    void UnblockCanCleanup() { can_cleanup_block_.Signal(); }

    void WaitForWorkToRun() { work_processed_.Wait(); }

    void WaitForCleanupRequest() { cleanup_requested_.Wait(); }

    void WaitForCleanup() { cleaned_up_.Wait(); }
   ...
   private:
    ...
    WaitableEvent cleaned_up_;
    WaitableEvent can_cleanup_block_;
  ...
  bool CanCleanup(SchedulerWorker* worker) {
    ...
    controls_->can_cleanup_block_.Wait();
    return can_cleanup;
  }
...
TEST(TaskSchedulerWorkerTest, CleanupBeforeStart) {
  ...
  // The worker shouldn't call GetWork, but give it time to see if it tries to.
