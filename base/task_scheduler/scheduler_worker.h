 ...
 public:
  ...
  class BASE_EXPORT Delegate {
   public:
    ...
    // Called by a thread if it is allowed to delete its worker from the pool
    // if the last call to GetWork() returned nullptr.
    //
    // It is the responsibility of the delegate to determine if deletion is
    // safe. If the delegate is responsible for thread-affine work, deletion
    ...
    // Called by a thread right after its timeout ends.
  ...
  // If Cleanup() hasn't been called, creates a thread to back the
  // SchedulerWorker. (Note: the thread will be in a wait state pending a
