...
// Verify that RunNextTask() returns nullptr when the sequence from which it ran
// a task is empty after popping a task from it.
TEST_F(TaskSchedulerTaskTrackerTest, RunNextTaskDoesNotReturnEmptySequence) {
...
TEST_F(TaskSchedulerTaskTrackerTest,
  ...
  // Simulate posting |kMaxNumScheduledBackgroundSequences| background tasks and
  // scheduling the associated sequences. This should succeed.
  ...
  for (int i = 0; i < kMaxNumScheduledBackgroundSequences; ++i) {
    ...
    extra_observers.push_back(
        std::make_unique<MockCanScheduleSequenceObserver>());
    EXPECT_EQ(nullptr,
...
TEST_F(TaskSchedulerTaskTrackerTest,
  ...
  // should be notified that |sequence_b| can be rescheduled.
