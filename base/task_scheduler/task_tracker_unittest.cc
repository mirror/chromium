...
class MockCanDispatchSequenceObserver : public CanDispatchSequenceObserver {
 public:
  void OnCanDispatchSequence(scoped_refptr<Sequence> sequence) override {
    MockOnCanDispatchSequence(sequence.get());
  }

  MOCK_METHOD1(MockOnCanDispatchSequence, void(Sequence*));
};
 ...
 private:
  void Run() override {
    ...
    if (post_succeeded &&
      ...
      testing::StrictMock<MockCanDispatchSequenceObserver>
          never_notified_observer;
      auto sequence = tracker_->WillDispatchSequence(
...
class TaskSchedulerTaskTrackerTest
 ...
 protected:
  ...
  testing::StrictMock<MockCanDispatchSequenceObserver> never_notified_observer_;
...
static void RunTaskRunnerHandleVerificationTask(
  ...
  testing::StrictMock<MockCanDispatchSequenceObserver> never_notified_observer;
  auto sequence = tracker->WillDispatchSequence(
...
TEST_F(TaskSchedulerTaskTrackerTest, CurrentSequenceToken) {
  ...
  sequence = tracker_.WillDispatchSequence(std::move(sequence),
                                           &never_notified_observer_);
  ASSERT_TRUE(sequence);
  tracker_.RunNextTask(std::move(sequence), &never_notified_observer_);
  EXPECT_FALSE(SequenceToken::GetForCurrentThread().IsValid());
...
TEST_F(TaskSchedulerTaskTrackerTest, RunNextTaskReturnsSequenceToRedispatch) {
  ...
  EXPECT_EQ(sequence, tracker_.WillDispatchSequence(sequence, nullptr));

  EXPECT_EQ(sequence, tracker_.RunNextTask(sequence, nullptr));
}

// Verify that WillDispatchSequence() returns nullptr when it receives a
// background sequence and the maximum number of background sequences that can
// be dispatched concurrently is reached. Verify that an observer is notified
// when a background sequence can be dispatched (i.e. when one of the previously
// dispatched background sequences has run).
TEST_F(TaskSchedulerTaskTrackerTest,
       WillDispatchBackgroundSequenceWithMaxBackgroundSequences) {
  ...
  // and dispatching the associated sequences. This should succeed.
  std::vector<scoped_refptr<Sequence>> dispatched_sequences;
  testing::StrictMock<MockCanDispatchSequenceObserver> never_notified_observer;
  for (int i = 0; i < kMaxNumDispatchedBackgroundSequences; ++i) {
    ...
    scoped_refptr<Sequence> sequence =
        test::CreateSequenceWithTask(std::move(task));
    EXPECT_EQ(sequence,
              tracker.WillDispatchSequence(sequence, &never_notified_observer));
    dispatched_sequences.push_back(std::move(sequence));
  }

  // Simulate posting extra background tasks and dispatching the associated
  // sequences. This should fail because the maximum number of background
  // sequences that can be dispatched concurrently is already reached.
  std::vector<std::unique_ptr<bool>> extra_tasks_did_run;
  std::vector<
      std::unique_ptr<testing::StrictMock<MockCanDispatchSequenceObserver>>>
      extra_observers;
  ...
  for (int i = 0; i < kMaxNumDispatchedBackgroundSequences; ++i) {
    ...
    extra_observers.push_back(
        std::make_unique<
            testing::StrictMock<MockCanDispatchSequenceObserver>>());
    EXPECT_EQ(nullptr,
              tracker.WillDispatchSequence(extra_sequences.back(),
                                           extra_observers.back().get()));
  }

  // Run the sequences dispatched at the beginning of the test. Expect an
  // observer from |extra_observer| to be notified every time a task finishes to
  // run.
  for (int i = 0; i < kMaxNumDispatchedBackgroundSequences; ++i) {
    EXPECT_CALL(*extra_observers[i].get(),
                MockOnCanDispatchSequence(extra_sequences[i].get()));
    EXPECT_FALSE(
        tracker.RunNextTask(dispatched_sequences[i], &never_notified_observer));
...
// Verify that RunNextTask() doesn't run a background task if there is a pending
// background task with an earlier sequenced time. Verify that the observer of
// the background task with the earlier sequenced time is notified.
TEST_F(TaskSchedulerTaskTrackerTest,
  ...
  // Simulate posting a background task and dispatching the associated sequence.
  // This should succeed.
  ...
  EXPECT_EQ(sequence_a,
            tracker.WillDispatchSequence(sequence_a, &never_notified_observer));

  // Simulate posting an extra background task and dispatching the associated
  ...
  auto task_b_1 = std::make_unique<Task>(
      ...
      TaskTraits(TaskPriority::BACKGROUND), TimeDelta());
  EXPECT_TRUE(tracker.WillPostTask(task_b_1.get()));
  scoped_refptr<Sequence> sequence_b =
      test::CreateSequenceWithTask(std::move(task_b_1));
  testing::StrictMock<MockCanDispatchSequenceObserver> task_b_1_observer;
  EXPECT_FALSE(tracker.WillDispatchSequence(sequence_b, &task_b_1_observer));
  ...
  auto task_a_2 = std::make_unique<Task>(
      ...
      TaskTraits(TaskPriority::BACKGROUND), TimeDelta());
  EXPECT_TRUE(tracker.WillPostTask(task_a_2.get()));
  sequence_a->PushTask(std::move(task_a_2));

  // Run the first task in |sequence_a|. RunNextTask() should return nullptr
  // since |sequence_a| can't be redispatched immediately. |task_b_1_observer|
  // should be notified that |sequence_b| can be dispatched.
  testing::StrictMock<MockCanDispatchSequenceObserver> task_a_2_observer;
  EXPECT_CALL(task_b_1_observer, MockOnCanDispatchSequence(sequence_b.get()));
  ...
  // dispatched.
  EXPECT_CALL(task_a_2_observer, MockOnCanDispatchSequence(sequence_a.get()));
...
class WaitAllowedTestThread : public SimpleThread {
 ...
 private:
  void Run() override {
    ...
    testing::StrictMock<MockCanDispatchSequenceObserver>
        never_notified_observer;
    auto sequence_without_sync_primitives = tracker.WillDispatchSequence(
    ...
    auto sequence_with_sync_primitives = tracker.WillDispatchSequence(
