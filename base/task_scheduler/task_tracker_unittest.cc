  ...
  void Run() override {
    ...
    if (post_succeeded &&
      ...
      tracker_->RunNextTask(
          test::generate_sequence(std::move(owned_task_)).get(),
...
TEST_P(TaskSchedulerTaskTrackerTest, WillPostAndRunBeforeShutdown) {
  ...
  EXPECT_TRUE(tracker_.RunNextTask(
      test::generate_sequence(std::move(task)).get(), &sequence_became_empty));
...
TEST_P(TaskSchedulerTaskTrackerTest, WillPostBeforeShutdownRunDuringShutdown) {
  ...
  EXPECT_EQ(should_run,
            tracker_.RunNextTask(test::generate_sequence(std::move(task)).get(),
                                 &sequence_became_empty));
  ...
  EXPECT_TRUE(tracker_.RunNextTask(
      test::generate_sequence(std::move(block_shutdown_task)).get(),
      &sequence_became_empty));
...
TEST_P(TaskSchedulerTaskTrackerTest, SingletonAllowed) {
  ...
  if (can_use_singletons) {
    EXPECT_TRUE(
        tracker.RunNextTask(test::generate_sequence(std::move(task)).get(),
                            &sequence_became_empty));
  } else {
    EXPECT_DCHECK_DEATH({
      tracker.RunNextTask(test::generate_sequence(std::move(task)).get(),
...
TEST_P(TaskSchedulerTaskTrackerTest, IOAllowed) {
  ...
  bool sequence_became_empty;
  tracker.RunNextTask(
      test::generate_sequence(std::move(task_with_may_block)).get(),
      &sequence_became_empty);
  ...
  tracker.RunNextTask(
      test::generate_sequence(std::move(task_without_may_block)).get(),
      &sequence_became_empty);
}
...
static void RunTaskRunnerHandleVerificationTask(
  ...
  EXPECT_TRUE(tracker->RunNextTask(
      test::generate_sequence(std::move(verify_task)).get(),
      &sequence_became_empty));
...
TEST_P(TaskSchedulerTaskTrackerTest, FlushPendingUndelayedTask) {
  ...
  tracker_.RunNextTask(test::generate_sequence(std::move(undelayed_task)).get(),
                       &sequence_became_empty);
...
TEST_P(TaskSchedulerTaskTrackerTest, PostTaskDuringFlush) {
  ...
  tracker_.RunNextTask(
      test::generate_sequence(std::move(other_undelayed_task)).get(),
      &sequence_became_empty);
  WAIT_FOR_ASYNC_FLUSH_RETURNED();
}
...
TEST_P(TaskSchedulerTaskTrackerTest, RunDelayedTaskDuringFlush) {
  ...
  bool sequence_became_empty;
  tracker_.RunNextTask(test::generate_sequence(std::move(delayed_task)).get(),
                       &sequence_became_empty);
...
class WaitAllowedTestThread : public SimpleThread {
 ...
 private:
  void Run() override {
    ...
    bool sequence_became_empty;
    tracker.RunNextTask(
        test::generate_sequence(std::move(task_without_sync_primitives)).get(),
        &sequence_became_empty);
    ...
    tracker.RunNextTask(
        test::generate_sequence(std::move(task_with_sync_primitives)).get(),
        &sequence_became_empty);
  }
