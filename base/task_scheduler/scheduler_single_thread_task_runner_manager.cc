...
class SchedulerWorkerDelegate : public SchedulerWorker::Delegate {
 ...
 private:
  ...
  // The SchedulerWorker that has |this| as a delegate. Must be set
  // synchronously right after creating a SchedulerWorker using |this| as
  // delegate (to ensure it is set before the first OnCanScheduleSequence()
  // which can occur before OnMainEntry() if PostTaskNow()'s
  // WillScheduleSequence() is initially held up and released quickly
  // thereafter).
