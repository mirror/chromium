...
class SchedulerWorkerDelegate : public SchedulerWorker::Delegate {
 ...
 private:
  ...
  // OnMainEntry() and OnCanDispatchSequence() (called when a sequence held up
  // by WillDispatchSequence() in PostTaskNow() can be scheduled).
 ...
 private:
  ...
  void PostTaskNow(std::unique_ptr<Task> task) {
    ...
    if (sequence_was_empty) {
      sequence = outer_->task_tracker_->WillDispatchSequence(
