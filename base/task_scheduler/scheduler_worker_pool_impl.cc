...
// |is_on_idle_workers_stack_| is true.
 ...
 private:
  ...
  void OnWorkerBecomesIdle(SchedulerWorker* worker);
...
void SchedulerWorkerPoolImpl::SchedulerWorkerDelegateImpl::OnWorkerBecomesIdle(
    SchedulerWorker* worker) {
  outer_->lock_.AssertAcquired();
...
size_t SchedulerWorkerPoolImpl::NumberOfWorkersThatCantRunTasks() const {
  lock_.AssertAcquired();
