...
class SchedulerWorkerPoolImpl::SchedulerWorkerDelegateImpl
 ...
 public:
  ...
  bool CanCleanup(SchedulerWorker* worker) override;
...
void SchedulerWorkerPoolImpl::JoinForTesting() {
  ...
  // Shutdown must be called to prevent new tasks from being posted during
  // JoinForTesting(). JoinForTesting() assumes that no new workers get created
  // during its execution, but posting tasks can cause new workers to be
  // created during JoinForTesting().
  DCHECK(task_tracker_->IsShutdownComplete())
      << "Shutdown must finish prior to calling JoinForTesting.";
...
SchedulerWorkerPoolImpl::SchedulerWorkerDelegateImpl::GetWork(
  ...
  {
    AutoSchedulerLock auto_lock(outer_->lock_);
    bool should_sus = outer_->ShouldSuspendWorkers();
    bool sus = outer_->WorkerIsSuspended(worker);
    if (outer_->ShouldSuspendWorkers() || outer_->WorkerIsSuspended(worker)) {
      DCHECK(should_sus || sus);
...
void SchedulerWorkerPoolImpl::SchedulerWorkerDelegateImpl::OnCleanup() {
...
bool SchedulerWorkerPoolImpl::WorkerIsSuspended(
  ...
  int target_suspended_workers = workers_.size() - worker_capacity_;
  if (target_suspended_workers <= 0)
    return false;
  return idle_workers_stack_.ContainsNearBottom(worker,
                                                target_suspended_workers - 1);
}
