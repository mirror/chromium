...
void SchedulerWorkerPoolImpl::Start(const SchedulerWorkerPoolParams& params) {
  ...
  for (int index = 0; index < num_initial_workers; ++index) {
    scoped_refptr<SchedulerWorker> worker = TryAddWorker();

    // CHECK that the first worker can be started (assume that failure means
    // that threads can't be created on this machine). Note that the workers
    // must be started before the |lock_| is released, otherwise
    // WakeUpOneWorker() could WakeUp() a worker before it's started (after the
    // lock's released, but before it's started).
...
scoped_refptr<SchedulerWorker> SchedulerWorkerPoolImpl::TryAddWorker() {
  lock_.AssertAcquired();
  scoped_refptr<SchedulerWorker> worker =
      CreateRegisterAndStartSchedulerWorker();
