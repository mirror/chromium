...
SchedulerWorkerPoolImpl::SchedulerWorkerPoolImpl(
    ...
    : name_(name),
      ...
      workers_lock_(&idle_workers_stack_lock_),
...
void SchedulerWorkerPoolImpl::Start(const SchedulerWorkerPoolParams& params) {
  ...
  {
    AutoSchedulerLock idle_workers_stack_auto_lock(idle_workers_stack_lock_);
    ...
    initial_worker_capacity_ = params.max_threads();
    ...
    // idle worker). The number of initial workers is at least one because
    // it is assumed that an empty workers_ list means that Start hasn't been
    // called.
    ...
    const int num_initial_workers =
        ...
        (params.standby_thread_policy() ==
                 ...
                 num_wake_ups_before_start_ == 0
             ? 1
             : 0);

    workers_.resize(num_initial_workers);

    // Create workers in reverse order of index so that the worker with the
    // highest index is at the bottom of the idle stack. SchedulerWorker needs
    // workers_lock_ as a predecessor for its own lock since in WakeUpOneWorker,
    // workers_lock_ is acquired before it calls WakeUp().
    for (int index = num_initial_workers - 1; index >= 0; --index) {
      workers_[index] = CreateNewWorker(index);
...
void SchedulerWorkerPoolImpl::WaitForAllWorkersIdleForTesting() {
  ...
  while (true) {
    // Polling was used here rather than a condition variable because:
    // using a condition variable would've been tricky since the condition
    // variable depended on two locks and there isn't an existing mechanism
    // to combine SchedulerLocks. Using a condition variable also would've meant
    // that the condition variable would exist and be broadcasted to within
    // production builds, even though it's only used within this testing
    // function.
    // On the other hand, since this function is only used for tests,
    // whatever performance penalty that comes from polling isn't going be a
    // factor in normal usage.
    {
      AutoSchedulerLock idle_workers_stack_auto_lock(idle_workers_stack_lock_);
      AutoSchedulerLock workers_auto_lock(workers_lock_);
      ...
      DCHECK_LT(idle_workers_stack_.Size(), workers_.size());
    }
...
void SchedulerWorkerPoolImpl::JoinForTesting() {
  ...
  std::vector<scoped_refptr<SchedulerWorker>> workers_copy;
  {
    AutoSchedulerLock auto_lock(workers_lock_);

    // Create a copy of the workers_ vector. The workers_ vector is currently
    // expected not to change after JoinForTesting is called, so the copy
    // is not expected to become stale.
...
// create a flag to prevent changes to workers_.
#if DCHECK_IS_ON()
  ...
  SchedulerWorker* worker = nullptr;
  {
    ...
    if (workers_.empty()) {
      ...
      if (idle_workers_stack_.IsEmpty() &&
        ...
        workers_.push_back(CreateNewWorker(workers_.size()));
        worker = workers_[workers_.size() - 1].get();
        worker->Start();
      } else {
...
scoped_refptr<SchedulerWorker> SchedulerWorkerPoolImpl::CreateNewWorker(
    int index) {
  return make_scoped_refptr(new SchedulerWorker(
