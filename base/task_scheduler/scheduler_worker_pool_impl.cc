...
void SchedulerWorkerPoolImpl::AdjustWorkerCapacity() {
  ...
  // Try to maintain at least one idle worker here in case
  // WakeUpOneWorkerAssertLockAcquired() wasn't called.
...
void SchedulerWorkerPoolImpl::SchedulerWorkerDelegateImpl::
  ...
  if (local_increased_worker_capacity_since_blocked) {
...
SchedulerWorkerPoolImpl::CreateRegisterAndStartSchedulerWorker() {
  ...
  // because in IncreaseWorkerCapacityIfNeeded, SchedulerWorkerDelegateImpl
  // needs to acquire both locks.
