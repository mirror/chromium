...
class SchedulerWorkerPoolImpl::SchedulerWorkerDelegateImpl
 ...
 public:
  ...
  // |blocked_time_threshold| since the last call to ThreadBlocked(). The caller
 ...
 private:
  // Called by a thread if it is allowed to cleanup and delete its worker from
  ...
  // Time when TaskBlocked() was last called. Reset when TaskUnblocked() is
  // called.
...
void SchedulerWorkerPoolImpl::Start(const SchedulerWorkerPoolParams& params) {
  ...
  int successful_worker_creations = 0;
  for (int index = 0; index < num_initial_workers; ++index) {
    ...
    if (worker) {
      ++successful_worker_creations;
      if (successful_worker_creations <= num_wake_ups_before_start_)
        worker->WakeUp();
      else
...
void SchedulerWorkerPoolImpl::AdjustWorkerCapacity() {
  ...
  const int suspended_workers = NumberOfSuspendedWorkers();
  {
    ...
    // of workers.
    additional_workers_needed = std::min<int>(
        worker_capacity_ - workers_.size(),
        shared_transaction->Size() - idle_workers_stack_.Size() + 1);
  }

  const int workers_unsuspended =
      suspended_workers - NumberOfSuspendedWorkers();

  // Wakeup newly unsuspended workers
  for (int i = 0; i < workers_unsuspended; ++i) {
    idle_workers_stack_.Get(suspended_workers - 1 - i)->WakeUp();
  }
...
int SchedulerWorkerPoolImpl::GetMaxConcurrentTasksDeprecated() const {
  ...
  DCHECK_NE(initial_worker_capacity_, 0U)
      << "GetMaxConcurrentTasksDeprecated() should "
...
SchedulerWorkerPoolImpl::SchedulerWorkerDelegateImpl::GetWork(
  ...
  // below records the histogram.
  if (last_get_work_returned_nullptr_) {
  ...
  scoped_refptr<Sequence> sequence;
  {
    ...
    if (shared_transaction->IsEmpty()) {
      ...
      if (CanCleanup(worker)) {
        ...
        outer_->cleanup_timestamps.push(TimeTicks::Now());
        worker->Cleanup();
...
void SchedulerWorkerPoolImpl::AddToIdleWorkersStack(
  ...
  // A cleanup may cause multiple attempts to add because the delegate cannot
...
SchedulerWorkerPoolImpl::CreateRegisterAndStartSchedulerWorker() {
  ...
  if (worker->Start()) {
    ...
    if (!cleanup_timestamps.empty()) {
      detach_duration_histogram_->AddTime(TimeTicks::Now() -
                                          cleanup_timestamps.top());
      cleanup_timestamps.pop();
    }
...
bool SchedulerWorkerPoolImpl::WorkerIsSuspended(
  ...
  if (target_suspended_workers <= 0)
    return false;
  return idle_workers_stack_.ContainsAtBottom(worker, target_suspended_workers);
}
...
bool SchedulerWorkerPoolImpl::ShouldSuspendWorkers() const {
  ...
  // Note: all suspended workers exist in the |idle_workers_stack_|. This
  // function returns true if we need more suspended workers in the
  // |idle_workers_stack_|
  return static_cast<int>(idle_workers_stack_.Size()) <
         target_suspended_workers;
}

int SchedulerWorkerPoolImpl::NumberOfSuspendedWorkers() const {
  lock_.AssertAcquired();
  return std::max(0, std::min<int>(idle_workers_stack_.Size(),
                                   workers_.size() - worker_capacity_));
}
