...
SchedulerWorkerPoolImpl::SchedulerWorkerDelegateImpl::GetWork(
    SchedulerWorker* worker) {
  {
    ...
    if (outer_->NumberOfWorkersThatCantRunTasksLockRequired() >
...
void SchedulerWorkerPoolImpl::SchedulerWorkerDelegateImpl::BlockingScopeEntered(
  ...
  if (shared_transaction->IsEmpty()) {
    ...
    // parallel and we had work on the PQ. See: crbug.com/757897
...
void SchedulerWorkerPoolImpl::WakeUpOneWorkerLockRequired() {
  ...
  if (NumberOfWorkersThatCantRunTasksLockRequired() <
...
size_t SchedulerWorkerPoolImpl::NumberOfWorkersThatCantRunTasksLockRequired()
    const {
  lock_.AssertAcquired();
