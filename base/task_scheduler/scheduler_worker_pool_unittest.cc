...
class TaskSchedulerWorkerPoolTest
 ...
 protected:
  ...
  void SetUp() override {
    ...
    CreateAndStartWorkerPoolWithType();
  }
 ...
 private:
  void CreateAndStartWorkerPoolWithType() {
    ...
    switch (GetParam().pool_type) {
      case PoolType::GENERIC: {
        std::unique_ptr<SchedulerWorkerPoolImpl> scheduler_worker_pool_impl =
            std::make_unique<SchedulerWorkerPoolImpl>(
      ...
      case PoolType::WINDOWS: {
        std::unique_ptr<SchedulerWorkerPoolWindowsImpl>
            scheduler_worker_pool_win =
                std::make_unique<SchedulerWorkerPoolWindowsImpl>(
