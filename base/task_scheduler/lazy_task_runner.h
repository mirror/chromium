...
// better way of sharing a TaskRunner. It is important to use this class over a
// global variable initialized once so that the TaskRunners do not outlive the
// scope of the ScopedTaskEnvironment in unit tests (otherwise the next test in
// the same process will die in use- after-frees).
//
...
// was created is deleted. A new TaskRunner is lazily created if Get() is called
// again in the scope of another base::test::ScopedTaskEnvironment.
...
#define LAZY_SEQUENCED_TASK_RUNNER_INITIALIZER(traits)                       \
  ...
  ALLOW_UNUSED_TYPE constexpr base::TaskTraits LAZY_TASK_RUNNER_CONCATENATE( \
...
#define LAZY_SINGLE_THREAD_TASK_RUNNER_INITIALIZER(traits, thread_mode)      \
  ...
  ALLOW_UNUSED_TYPE constexpr SingleThreadTaskRunnerThreadMode               \
      LAZY_TASK_RUNNER_CONCATENATE(kVerifyThreadModeIsConstexpr, __LINE__) = \
          thread_mode
...
#define LAZY_TASK_RUNNER_CONCATENATE(a, b) LAZY_TASK_RUNNER_CONCATENATE2(a, b)
#define LAZY_TASK_RUNNER_CONCATENATE2(a, b) a##b

template <typename TaskRunnerType, bool com_sta>
class BASE_EXPORT LazyTaskRunner {
 public:
  // Use the macros at the end of this file rather than a direct call to this.
