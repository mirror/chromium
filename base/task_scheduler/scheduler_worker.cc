 ...
 public:
  ...
  void ThreadMain() override {
    ...
    while (!outer_->ShouldExit()) {
      ...
      if (!sequence) {
        if (outer_->delegate_->CanTimeout(outer_.get())) {
          detached_thread = outer_->DetachThreadObject();
          if (detached_thread) {
            outer_->delegate_->OnTimeout();
    ...
    // Detach the thread unlesss if shutdown occured (no need to free thread
    // resources) or if Join was called (so that the thread isn't both joined
    // and detached)
    if (outer_->started_ && !(outer_->task_tracker_->IsShutdownComplete() ||
                              outer_->join_called_for_testing_.IsSet()))
      PlatformThread::Detach(thread_handle_);

    outer_->delegate_->RemoveWorker(outer_.get());
  ...
  void Join() {
    ...
    // Invalidate the thread_handle_ so we don't also try to detach it when
    // DetachThreadObject is called.
...
bool SchedulerWorker::Start() {
  ...
  // TODO(jeffreyhe): Handle the case when Start/CreateThread fails. (possibly
  // have a empty slot in the workers_ list for the failed worker or do
  // something else)
  CreateThread();
  DCHECK(!!thread_);
  return !!thread_;
