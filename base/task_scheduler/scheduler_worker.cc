...
void SchedulerWorker::WakeUp() {
  ...
  if (thread_) {
    // If the caller tried to wake us up, then it means it probably thinks we're
    // still alive. This is bad since the SchedulerWorker might've exited or be
    // in the process of exiting which will cause the WakeUp to no-op.
...
SchedulerWorker::~SchedulerWorker() {
  if (thread_ && !join_called_for_testing_.IsSet())
    thread_->Detach();
}
