...
bool CopyDirectory(const FilePath& from_path,
  ...
  do {
    ...
    if (S_ISDIR(from_stat.st_mode)) {
      ...
      if (ReadSymbolicLink(current, &link_target) &&
          CreateSymbolicLink(link_target, target_path))
        continue;

      DLOG(ERROR) << "CopyDirectory() couldn't create link: "
                  << target_path.value();
      return false;
    }
    ...
    // Add O_NOFOLLOW here so we can't be tricked to copying something into
    // a symlink. We don't go through the normal file constructor because
    // that only implies O_NOFOLLOW with O_EXCL which we don't want here.
    ...
    base::File outfile(
        open(target_path.value().c_str(),
             O_WRONLY | O_CREAT | O_TRUNC | O_NOFOLLOW | O_NONBLOCK, mode));
    if (!outfile.IsValid()) {
