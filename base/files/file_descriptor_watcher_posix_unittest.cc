...
// without blocking. Verify that the readable notification is sent to controller
// B, not controller A.
...
// Same as previous test, but a duplicate of the watched file descriptor is kept
// alive thoughout the test.
TEST_P(FileDescriptorWatcherTest,
       RewatchPreviouslyWatchedFileDescriptorWithDup) {
  const int old_read_file_descriptor = read_file_descriptor();
  const int old_read_file_descriptor_dup = dup(old_read_file_descriptor);
  {
    auto controller_a = WatchReadable();
    WaitAndRunPendingTasks();
    PauseOtherThread();
  ...
  EXPECT_EQ(0, IGNORE_EINTR(close(old_read_file_descriptor_dup)));
}
