...
#if !defined(OS_FUCHSIA) && defined(OS_POSIX)
TEST_F(FileUtilTest, CopyDirectoryWithSymlink) {
  // Create a directory.
  FilePath dir_name_from =
      temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Copy_From_Subdir"));
  CreateDirectory(dir_name_from);
  ...
  CreateSymbolicLink(file_name_from, symlink_name_from);
  ...
  EXPECT_TRUE(PathExists(dir_name_to));
  EXPECT_TRUE(PathExists(file_name_to));
  EXPECT_TRUE(PathExists(symlink_name_to));

  // Check that the symlink wasn't dereferenced when it was copied.
  base::FilePath symlink_referent;
  EXPECT_TRUE(ReadSymbolicLink(symlink_name_to, &symlink_referent));
  EXPECT_EQ(file_name_from, symlink_referent);
}

TEST_F(FileUtilTest, CopyDirectoryWithFifo) {
  // Create a directory.
  FilePath dir_name_from =
      temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Copy_From_Subdir"));
  CreateDirectory(dir_name_from);
  ...
  mkfifo(fifo_name_from.value().c_str(), 0644);
  ...
  // Check everything has been copied except the fifo.
  EXPECT_TRUE(PathExists(dir_name_from));
  EXPECT_TRUE(PathExists(file_name_from));
