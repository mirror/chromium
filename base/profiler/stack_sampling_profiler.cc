...
void StackSamplingProfiler::Start() {
  ...
  DCHECK_EQ(kResetPolicy, WaitableEvent::ResetPolicy::MANUAL);

  // If a previous profiling phase is still winding down, wait for it to
  // complete, then reset it for the upcoming run. We can't use task posting for
  // this coordination because the thread owning the profiler may not have a
  // message loop.
  if (!profiling_inactive_.IsSignaled()) {
    base::ScopedAllowBaseSyncPrimitivesOutsideBlockingScope
        allow_sync_primitives;
    profiling_inactive_.Wait();
