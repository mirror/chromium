...
pid_t ForkWithFlags(unsigned long flags, pid_t* ptid, pid_t* ctid) {
  flags &= ~SIGCHLD;
  if (flags & ~(CLONE_NEWPID | CLONE_NEWUSER | CLONE_NEWNET)) {
    DPLOG(ERROR)
        << "ForkWithFlags() only supports CLONE_NEWPID, CLONE_NEWUSER, and "
           "CLONE_NEWNET, but was passed "
        << flags;
  }
  if (ptid != nullptr || ctid != nullptr) {
    DPLOG(ERROR) << "ptid and ctid must be nullptr";
  }

  printf("%lu\n", flags);
  sys_unshare(flags & CLONE_NEWPID);
  pid_t pid = fork();
  if (pid != 0) {
