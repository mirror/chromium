...
bool GetAppOutputInternal(const StringPiece16& cl,
  ...
  if (!CreatePipe(&out_read, &out_write, &sa_attr, 0)) {
    NOTREACHED() << "Failed to create pipe when executing " << cl;
    return false;
  }
  ...
  if (!SetHandleInformation(out_read, HANDLE_FLAG_INHERIT, 0)) {
    NOTREACHED() << "Failed to disabled pipe inheritance when executing " << cl;
    return false;
  }
  ...
  if (!CreateProcess(nullptr, &writable_command_line_string[0], nullptr,
    ...
    NOTREACHED() << "Failed to start process " << cl;
    return false;
  }
