...
#if defined(OS_FUCHSIA)
typedef std::vector<std::pair<uint16_t, mx_handle_t>> HandlesToTransferVector;
#endif
...
struct BASE_EXPORT LaunchOptions {
  ...
  // Each entry is an <id,handle> pair, such that the child retrieves the handle
  // with |mx_get_startup_handle(PA_HND(PA_USER0, <id>))|
