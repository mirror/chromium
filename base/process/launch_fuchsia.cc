...
Process LaunchProcess(const std::vector<std::string>& argv,
  ...
  bool maps_stdio[3] = {false};
  for (const auto& src_target : options.fds_to_remap) {
    if (static_cast<size_t>(src_target.second) < arraysize(maps_stdio))
      maps_stdio[src_target.second] = true;
    launchpad_clone_fd(lp, src_target.first, src_target.second);
  }
  for (size_t stdio_fd = 0; stdio_fd < arraysize(maps_stdio); ++stdio_fd) {
    if (!maps_stdio[stdio_fd])
