...
bool MapPathsToLaunchpad(const std::vector<std::string> paths_to_map,
  ...
  for (size_t path_idx = 0; path_idx < paths_to_map.size(); ++path_idx) {
    ...
    // |paths_c_str|.
    status = launchpad_add_handle(lp, handles[0], PA_HND(PA_NS_DIR, path_idx));
    ignore_result(scoped_handle.release());
    if (status != ZX_OK) {
...
using ScopedLaunchpad =
    std::unique_ptr<launchpad_t, std::function<void(launchpad_t*)>>;
...
Process LaunchProcess(const std::vector<std::string>& argv,
  ...
  ScopedLaunchpad lp;
  {
    ...
    lp = ScopedLaunchpad(
        lp_ptr, [](launchpad_t* to_delete) { launchpad_destroy(to_delete); });
  }
