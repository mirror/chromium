...
int ProcessMetrics::CalculateIdleWakeupsPerSecond(
    uint64_t absolute_idle_wakeups,
    bool package_idle) {
  uint64_t* last_event_count = package_idle
                                   ? &last_absolute_package_idle_wakeups_
                                   : &last_absolute_idle_wakeups_;
  TimeTicks* last_calculated = package_idle ? &last_package_idle_wakeups_time_
                                            : &last_idle_wakeups_time_;
  return CalculateEventsPerSecond(absolute_idle_wakeups, last_event_count,
                                  last_calculated);
}
#else
