...
variation. When thinking about performance and memory overhead, think of a
`std::vector`.
...
libc++ and doesn't do as many heap allocations and MSVC).
...
requires a heap allocation. libc++ (on Android and Mac) uses 4K blocks which
elimiates the problem of many heap allocations, but generally wastes a large
amount of space (an Android analysis revealed more than 2.5MB wasted space
from deque alone, resulting in some optimizations). libstdc++ uses an
intermediate 512 byte buffer.
...
not be contiguous, but random access iterators are still possible.
