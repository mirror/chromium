...
constexpr size_t kCircularBufferInitialExternalCapacity = 3;
...
class circular_deque {
 ...
 public:
  ...
  void reserve(size_type new_capacity) {
    if (new_capacity > capacity()) {
      // One extra item to convert to internal capacity.
      SetInternalCapacityTo(new_capacity + 1);
    }
  }
 ...
 private:
  ...
  //
  // The capacity passed to this function is the internal capacity which is
  // the external capacity (that returned by capacity()) + 1. See the comment
  // above buffer_ for more.
  void SetInternalCapacityTo(size_t new_internal_capacity) {
    VectorBuffer new_buffer(new_internal_capacity);
    MoveBuffer(buffer_, begin_, end_, &new_buffer, &begin_, &end_);
    buffer_ = std::move(new_buffer);
  }
  void ExpandCapacityIfNecessary(size_t additional_elts) {
    size_t min_new_external_capacity = size() + additional_elts;
    if (capacity() >= min_new_external_capacity)
      return;  // Already enough room.

    min_new_external_capacity =
        std::max(min_new_external_capacity,
                 internal::kCircularBufferInitialExternalCapacity);
    ...
    size_t new_external_capacity =
        std::max(min_new_external_capacity, capacity() + capacity() / 4);
    SetInternalCapacityTo(new_external_capacity + 1);
  }

  void ShrinkCapacityIfNecessary() {
    // Don't auto-shrink below this size.
    if (capacity() <= internal::kCircularBufferInitialExternalCapacity)
      return;

    // Shrink when 100% of the size() is wasted.
    size_t empty_spaces = capacity() - size();
    if (empty_spaces < size())
      return;

    // Leave 1/4 the size as free capacity, not going below the initial
    // capacity.
    size_t new_external_capacity = std::max(
        internal::kCircularBufferInitialExternalCapacity, size() + size() / 4);
    if (new_external_capacity < capacity()) {
  ...
  // capacity() (the "external capacity") + 1 since there is an extra item to
  // indicate the end. Otherwise being completely empty and completely full
  // are indistinguishable (begin == end). We could add a separate flag to
  // avoid it, but that adds significant extra complexity since every
  // computation will have to check for it. Always keeping one extra unused
  // element in the buffer makes iterator computations much simpler.
  //
