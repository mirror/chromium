...
class circular_deque {
 ...
 public:
  ...
  void reserve(size_type new_capacity) {
    if (new_capacity > capacity())
      SetCapacityTo(new_capacity + 1);  // One item is wasted to indicate end().
  }
 ...
 private:
  ...
  void ExpandCapacityIfNecessary(size_t additional_elts) {
    ...
    min_new_capacity = std::max(min_new_capacity, initial_capacity);
  ...
  void ShrinkCapacityIfNecessary() {
    ...
    if (buffer_.capacity() <= initial_capacity)
      return;
    ...
    // Leave 1/4 the size as free capacity, not going below initial_capacity.
    size_t new_capacity = std::max(initial_capacity, size() + size() / 4);
    if (new_capacity < capacity())
      SetCapacityTo(new_capacity + 1);  // Count extra item to indicate end().
  }
