...
class circular_deque {
 ...
 public:
  ...
  insert(const_iterator pos, InputIterator first, InputIterator last) {
    ...
    iterator insert_cur(this, pos.index_);
    iterator insert_end;
    MakeRoomFor(std::distance(first, last), &insert_cur, &insert_end);
    while (insert_cur < insert_end) {
  ...
  iterator erase(const_iterator first, const_iterator last) {
    ...
    if (last.index_ == begin_) {
      // This deletion is from the beginnin. Nothing needs to be copied, only
    ...
    for (iterator move_dest(this, first.index_); move_src < move_src_end;
         move_src++, move_dest++) {
    ...
    end_ -= (buffer_.capacity() + last.OffsetFromBegin() -
             first.OffsetFromBegin()) %
            buffer_.capacity();
