...
class circular_deque {
 ...
 public:
  ...
  circular_deque(std::initializer_list<value_type> init) : buffer_() {
    assign(init);
  }
  ...
  circular_deque& operator=(std::initializer_list<value_type> ilist) {
    reserve(std::distance(ilist.begin(), ilist.end()));
  ...
  void resize(size_type count) {
    if (count > size()) {
      ...
      // This doesn't resize the storage. Whether to do so or not is not
      // obvious. If we want to add that in the back, this will need to be
      // factored differently so that the other functions (e.g. assign()) that
      // call clear() don't get the buffer shrinking unless the new contents
      // warrants it.
      size_t new_end = (begin_ + count) % buffer_.capacity();
      if (end_ > new_end) {
        // Erase everything as one block.
        buffer_.DestructRange(&buffer_[new_end], &buffer_[end_]);
      } else {
        // Block is split.
        buffer_.DestructRange(&buffer_[new_end], &buffer_[buffer_.capacity()]);
        buffer_.DestructRange(&buffer_[0], &buffer_[end_]);
      }
      end_ = new_end;
    }
    IncrementGeneration();
