...
// insert_or_assign(const Key&, M&&)
TEST(FlatMap, InsertOrAssignConstKey) {
  base::flat_map<std::string, MoveOnlyInt> m;

  // Initial insertion should return an iterator to the element and set the
  // second pair member to |true|. The inserted value should be moved from.
  MoveOnlyInt val(22);
  auto result = m.insert_or_assign("a", std::move(val));
  ...
  // old value and moving out of the passed in value. Assignment should be
  // indicated by setting the second pair member to |false|.
  val = MoveOnlyInt(44);
  result = m.insert_or_assign("a", std::move(val));
  EXPECT_EQ("a", result.first->first);
...
// insert_or_assign(const_iterator hint, const Key&, M&&)
TEST(FlatMap, InsertOrAssignConstKeyWithHint) {
  base::flat_map<std::string, MoveOnlyInt> m;

  // Initial insertion should return an iterator to the element. The inserted
  // value should be moved from.
  MoveOnlyInt val(22);
  auto result = m.insert_or_assign(m.end(), "a", std::move(val));
  ...
  // Second call with same key should result in an assignment, overwriting the
  // old value and moving out of the passed in value.
  val = MoveOnlyInt(44);
  result = m.insert_or_assign(m.end(), "a", std::move(val));
  EXPECT_EQ("a", result->first);
...
// insert_or_assign(const_iterator hint, Key&&, M&&)
...
TEST(FlatMap, TryEmplaceConstKey) {
  base::flat_map<std::string, std::pair<MoveOnlyInt, MoveOnlyInt>> m;
  ...
  // inserted value should be moved from.
  MoveOnlyInt val1(22);
  MoveOnlyInt val2(44);
  // Test piecewise construction of mapped_type.
  auto result = m.try_emplace("a", std::move(val1), std::move(val2));
  EXPECT_EQ("a", result.first->first);
  ...
  // Values that were attempted to be inserted should be left intact.
  val1 = MoveOnlyInt(33);
  val2 = MoveOnlyInt(55);
  auto paired_val = std::forward_as_tuple(std::move(val1), std::move(val2));
  // Test construction of mapped_type from pair.
  result = m.try_emplace("a", std::move(paired_val));
  EXPECT_EQ("a", result.first->first);
  EXPECT_EQ(22, result.first->second.first.data());
  EXPECT_EQ(44, result.first->second.second.data());
  EXPECT_FALSE(result.second);
  EXPECT_EQ(1u, m.size());
  EXPECT_EQ(33, val1.data());  // not moved from
  EXPECT_EQ(55, val2.data());  // not moved from
}
...
// try_emplace(const_iterator hint, const Key&, Args&&...)
TEST(FlatMap, TryEmplaceConstKeyWithHint) {
  base::flat_map<std::string, std::pair<MoveOnlyInt, MoveOnlyInt>> m;

  // Trying to emplace into an empty map should succeed. Insertion should return
  // an iterator to the element. The inserted value should be moved from.
  MoveOnlyInt val1(22);
  MoveOnlyInt val2(44);
  // Test piecewise construction of mapped_type.
  auto result = m.try_emplace(m.end(), "a", std::move(val1), std::move(val2));
  EXPECT_EQ("a", result->first);
  ...
  // Second call with same key should result in a no-op, returning an iterator
  // to the existing element. Values that were attempted to be inserted should
  // be left intact.
  val1 = MoveOnlyInt(33);
  val2 = MoveOnlyInt(55);
  auto paired_val = std::forward_as_tuple(std::move(val1), std::move(val2));
  // Test construction of mapped_type from pair.
  result = m.try_emplace(m.end(), "a", std::move(paired_val));
  EXPECT_EQ("a", result->first);
  EXPECT_EQ(22, result->second.first.data());
  EXPECT_EQ(44, result->second.second.data());
  EXPECT_EQ(1u, m.size());
