...
TEST(FlatMap, InsertOrAssignConstKey) {
  ...
  std::string key = "a";
  MoveOnlyInt val(22);
  auto result = m.insert_or_assign(key, std::move(val));
  ...
  val = MoveOnlyInt(44);
  result = m.insert_or_assign(key, std::move(val));
  EXPECT_EQ("a", result.first->first);
...
TEST(FlatMap, InsertOrAssignConstKeyWithHint) {
  ...
  std::string key = "a";
  MoveOnlyInt val(22);
  auto result = m.insert_or_assign(m.end(), key, std::move(val));
  ...
  val = MoveOnlyInt(44);
  result = m.insert_or_assign(m.end(), key, std::move(val));
  EXPECT_EQ("a", result->first);
  ...
  base::flat_map<int, int> map;
  for (int i : {3, 1, 5, 6, 8, 7, 0, 9, 4, 2}) {
    map.insert_or_assign(map.end(), i, i);
    EXPECT_TRUE(std::is_sorted(map.begin(), map.end()));
  }
}
...
TEST(FlatMap, TryEmplaceConstKey) {
  ...
  auto result = m.try_emplace(key, std::move(val1), std::move(val2));
  EXPECT_EQ("a", result.first->first);
  ...
  result = m.try_emplace(key, std::move(paired_val));
  EXPECT_EQ("a", result.first->first);
  EXPECT_EQ(22, result.first->second.first.data());
  EXPECT_EQ(44, result.first->second.second.data());
  EXPECT_FALSE(result.second);
...
TEST(FlatMap, TryEmplaceConstKeyWithHint) {
  ...
  MoveOnlyInt val2(44);
  // Test piecewise construction of mapped_type.
  auto result = m.try_emplace(m.end(), key, std::move(val1), std::move(val2));
  EXPECT_EQ("a", result->first);
  ...
  auto paired_val = std::make_pair(MoveOnlyInt(33), MoveOnlyInt(55));
  // Test construction of mapped_type from pair.
  result = m.try_emplace(m.end(), key, std::move(paired_val));
  EXPECT_EQ("a", result->first);
  EXPECT_EQ(22, result->second.first.data());
  EXPECT_EQ(44, result->second.second.data());
  EXPECT_EQ(1u, m.size());
