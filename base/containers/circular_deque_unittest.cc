...
TEST(CircularDeque, InsertEraseSingle) {
  ...
  EXPECT_EQ(q.begin(), q.insert(q.begin(), 0));
  EXPECT_EQ(3u, q.size());
  EXPECT_EQ(0, q[0]);
  EXPECT_EQ(1, q[1]);
  EXPECT_EQ(2, q[2]);
  ...
  EXPECT_EQ(q.begin(), q.erase(q.begin()));
  EXPECT_EQ(2u, q.size());
  EXPECT_EQ(1, q[0]);
  EXPECT_EQ(2, q[1]);

  // Insert at the end.
  EXPECT_EQ(q.end() - 1, q.insert(q.end(), 3));
  EXPECT_EQ(1, q[0]);
  EXPECT_EQ(2, q[1]);
  EXPECT_EQ(3, q[2]);

  // Erase at the end.
  EXPECT_EQ(q.end(), q.erase(q.end() - 1));
  EXPECT_EQ(1, q[0]);
  EXPECT_EQ(2, q[1]);

  // Insert in the middle.
  EXPECT_EQ(q.begin() + 1, q.insert(q.begin() + 1, 10));
  EXPECT_EQ(1, q[0]);
  EXPECT_EQ(10, q[1]);
  EXPECT_EQ(2, q[2]);

  // Erase in the middle.
  EXPECT_EQ(q.begin() + 1, q.erase(q.begin() + 1));
  EXPECT_EQ(1, q[0]);
  EXPECT_EQ(2, q[1]);
}
...
TEST(CircularDeque, InsertEraseRange) {
  ...
  EXPECT_EQ(q.begin(), q.erase(q.begin(), q.begin() + 2));
  EXPECT_EQ(q.begin() + 1, q.erase(q.begin() + 1, q.begin() + 3));
  EXPECT_EQ(q.end(), q.erase(q.end() - 2, q.end()));
  ...
  // Erase everything.
  EXPECT_EQ(q.end(), q.erase(q.begin(), q.end()));
  EXPECT_TRUE(q.empty());
}
