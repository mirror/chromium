...
struct IsValueInRangeFastOp<
  ...
  static constexpr bool Do(Src value) {
    ...
    // then comparea against a bitwise and of the max values for both types
    // (to get the smaller of the two), which is our upper bound.
    return as_unsigned(value) <=
           as_unsigned(Src(std::numeric_limits<Src>::max() &
