...
class MemoryDumpManagerTest : public testing::Test {
 ...
 protected:
  void InitializeMemoryDumpManager(bool is_coordinator) {
    ...
    // The only reasons left for the |request_dump_function| are the peak
    // detector and the preiodic dump scheduler (which should be both moved
    // soon outside of base). In production code, this function is intended to
    // be bound to a service outside of base which knows how to deal with global
    // (i.e. multi-process) dumps. For the sake of these single-process
    // unittests we just short-circuit this function to the
    // MemoryDumpManager::CreateProcessDump() function itself. Also we don't
    // bother passing a valid ProcessMemoryDumpCallback(), as both the peak
    // detector and the dump scheduler have fire-and-forget semantics and don't
    // make any use of the callback.
    // The GlobalMemoryDumpCallback is ignored: it's not needed by the test
    // fixture in this unittest and will be removed in the next CL anyways.
    auto request_dump_function = [](const MemoryDumpRequestArgs& args,
                                    const GlobalMemoryDumpCallback&) {
      MemoryDumpManager::GetInstance()->CreateProcessDump(
          args, ProcessMemoryDumpCallback());
    };
    mdm_->Initialize(BindRepeating(request_dump_function), is_coordinator);
  }
