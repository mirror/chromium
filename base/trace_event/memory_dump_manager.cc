...
void MemoryDumpManager::RequestGlobalDump_NoCallback(
    MemoryDumpType dump_type,
    MemoryDumpLevelOfDetail level_of_detail) {
    ...
    if (trigger.trigger_type == MemoryDumpType::PERIODIC_INTERVAL) {
      if (periodic_config.triggers.empty()) {
        periodic_config.callback =
            BindRepeating(&MemoryDumpManager::RequestGlobalDump_NoCallback,
                          Unretained(this), MemoryDumpType::PERIODIC_INTERVAL);
      }
      ...
      MemoryPeakDetector::GetInstance()->Setup(
          ...
          GetOrCreateBgTaskRunnerLocked(),
          BindRepeating(&MemoryDumpManager::RequestGlobalDump_NoCallback,
                        Unretained(this), MemoryDumpType::PEAK_MEMORY_USAGE,
