...
void MemoryDumpManager::EnableHeapProfilingIfNeeded() {
  {
    AutoLock lock(lock_);
    if (heap_profiling_mode_ != kHeapProfilingModeDisabled)
      return;
  }
  EnableHeapProfiling(GetHeapProfilingModeFromCommandLine());
}
...
bool MemoryDumpManager::EnableHeapProfiling(HeapProfilingMode profiling_mode) {
  ...
  bool is_enabled = IsHeapProfilingModeEnabled(heap_profiling_mode_);
  ...
  if (heap_profiling_mode_ == profiling_mode)
    return true;  // Nothing to do.

  switch (profiling_mode) {
    case kHeapProfilingModePseudo:
      if (is_enabled)
        return false;  // Already enabled in different mode (same below).
    ...
    case kHeapProfilingModeInvalid:
      DCHECK(!is_enabled)
          << "EnableHeapProfiling(kHeapProfilingModeInvalid) "
             "must not be used to disable working heap profiler.";
      heap_profiling_mode_ = kHeapProfilingModeInvalid;
      return false;  // We're in permanently disabled mode, so return false.

    default:
      DCHECK(false) << "Unknown heap profiling mode " << profiling_mode;
      return false;
  }
  ...
  if (notify_mdps) {
    for (auto mdp : dump_providers_) {
      mdp->dump_provider->OnHeapProfilingEnabled(is_enabled);
    }
  }
  return true;
#else
  heap_profiling_mode_ = kHeapProfilingInvalid;
  return false;
#endif  // BUILDFLAG(USE_ALLOCATOR_SHIM) && !defined(OS_NACL)
}
