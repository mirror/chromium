...
void MemoryDumpManager::RegisterDumpProviderInternal(
  ...
  scoped_refptr<SequencedTaskRunner> mdp_task_runner;
  {
    AutoLock lock(lock_);
    ...
    mdp_task_runner =
        task_runner ? task_runner : GetOrCreateBgTaskRunnerLocked();
  }
  ...
  if (enable_mdp_allocator_hooks) {
    mdp_task_runner->PostTask(
        FROM_HERE, Bind(&NotifyHeapProfilingEnabledOnMDPThread, mdpinfo, true));
  }
}
