...
TEST(ProcessMemoryDumpTest, GuidsTest) {
  ...
  ProcessMemoryDump pmd1(nullptr, dump_args, process_token_one);
  MemoryAllocatorDump* mad1 = pmd1.CreateAllocatorDump("foo");

  ProcessMemoryDump pmd2(nullptr, dump_args, process_token_one);
  ...
  // names:
  ProcessMemoryDump pmd4(nullptr, dump_args, process_token_two);
  ...
  ASSERT_EQ(mad1->guid(),
            MemoryAllocatorDump::GetDumpId(process_token_one, "foo"));
}
