...
// A mock SingleThreadTaskRunner that just queues undelayed tasks (and ignores
...
class MockSingleThreadTaskRunner : public SingleThreadTaskRunner {
 public:
  MockSingleThreadTaskRunner() = default;
 ...
 private:
  ~MockSingleThreadTaskRunner() override = default;
  ...
  DISALLOW_COPY_AND_ASSIGN(MockSingleThreadTaskRunner);
};

// a mock RunLoop::Delegate to test Runloop logic independent of any other base
// constructs.
class MockDelegate : public RunLoop::Delegate {
 public:
  MockDelegate() = default;

  void BindToCurrentThread() {
    thread_task_runner_handle_ =
        MakeUnique<ThreadTaskRunnerHandle>(mock_task_runner_);
 ...
 private:
  ...
  scoped_refptr<MockSingleThreadTaskRunner> mock_task_runner_ =
      MakeRefCounted<MockSingleThreadTaskRunner>();
...
enum class RunLoopTestType {
  ...
  // Runs all RunLoopTests under a mock RunLoop::Delegate to make sure the
  // delegate interface fully works standalone.
  kMockDelegate,
};

// The task environment for the RunLoopTest of a give type. A separate class
  ...
  RunLoopTestEnvironment(RunLoopTestType type) {
    switch (type) {
      case RunLoopTestType::kRealEnvironment:
        ...
        break;
      case RunLoopTestType::kMockDelegate:
        mock_delegate_ = base::MakeUnique<MockDelegate>();
        mock_delegate_->BindToCurrentThread();
        break;
    }
  }
...
TEST_P(RunLoopTest, DisallowWaitingDeathTest) {
  EXPECT_TRUE(RunLoop::IsNestingAllowedOnCurrentThread());
...
INSTANTIATE_TEST_CASE_P(Mock,
                        RunLoopTest,
                        testing::Values(RunLoopTestType::kMockDelegate));
