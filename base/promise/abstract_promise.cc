...
void AbstractPromise::SetStateFromValue() {
  ...
  switch (value_.index()) {
    ...
    case kResolvedWithPromiseIndex: {
...
struct AbstractPromise::PromiseContext {
  ...
  void MaybeProcess() {
    ...
    if (ready_promises.front()->from_here_.program_counter()) {
      MaybePostTask();
    } else {
...
void AbstractPromise::Process() {
  ...
  while (!s_promise_context.Get().ready_promises.empty()) {
    ...
    if (s_promise_context.Get()
            .ready_promises.front()
            ->from_here_.program_counter()) {
