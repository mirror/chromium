...
TEST_F(PromiseTest, PostVoidPromise) {
  std::vector<int> order;
  ThreadTaskRunnerHandle::Get()
      ->PostPromise(
          FROM_HERE,
          BindOnce([](std::vector<int>* order) { order->push_back(1); },
                   &order))
      .Then(FROM_HERE,
            BindOnce([](std::vector<int>* order) { order->push_back(2); },
                     &order))
      .Then(FROM_HERE,
            BindOnce(
                [](std::vector<int>* order, base::RunLoop* run_loop) {
                  order->push_back(3);
                  run_loop->Quit();
                },
                &order, &run_loop_));
...
TEST_F(PromiseTest, PostIntPromise) {
  ThreadTaskRunnerHandle::Get()
      ->PostPromise(FROM_HERE, BindOnce([]() { return 1; }))
      .Then(FROM_HERE, BindOnce([](int arg) { return arg + 1; }))
      .Then(FROM_HERE, BindOnce(
                           [](base::RunLoop* run_loop, int arg) {
                             EXPECT_EQ(2, arg);
                             run_loop->Quit();
                           },
                           &run_loop_));
...
TEST_F(PromiseTest, ResolveVoidFromPostPromise) {
  ThreadTaskRunnerHandle::Get()
      ->PostPromise(FROM_HERE, BindOnce([]() -> PromiseResult<void> {
...
TEST_F(PromiseTest, RejectVoidFromPostPromise) {
  ThreadTaskRunnerHandle::Get()
      ->PostPromise(FROM_HERE, BindOnce([]() -> PromiseResult<int> {
...
TEST_F(PromiseTest, ResolveIntFromPostPromise) {
  ThreadTaskRunnerHandle::Get()
      ->PostPromise(FROM_HERE,
                    BindOnce([]() -> PromiseResult<int> { return 123; }))
      .Then(FROM_HERE, BindOnce(
...
TEST_F(PromiseTest, RejectIntFromPostPromise) {
  ThreadTaskRunnerHandle::Get()
      ->PostPromise(FROM_HERE, BindOnce([]() -> PromiseResult<int, int> {
