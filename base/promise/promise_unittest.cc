...
TEST_F(PromiseTest, NestedPromises) {
  ...
  Promise<int>(BindOnce([](PromiseResolver<int> pr) { pr.resolve(100); }))
      .Then(BindOnce([](int result) {
        return Promise<int>(
                   BindOnce([](PromiseResolver<int> pr) { pr.resolve(200); }))
            .Then(BindOnce([](int result) {
              return Promise<int>(BindOnce([](PromiseResolver<int> pr) {
                       pr.resolve(300);
                     }))
      ...
      .Then(BindOnce(
          [](bool* then_done, int result) {
            EXPECT_EQ(300, result);
            *then_done = true;
          },
          &then_done));

  EXPECT_TRUE(then_done);
}
...
TEST_F(PromiseTest, RaceIntContainer) {
  PromiseResolver<int> promise_resolver;
  std::vector<Promise<int>> promises{
      Promise<int>(BindOnce([](PromiseResolver<int> pr) {})),
      Promise<int>(
          BindOnce([](PromiseResolver<int>* promise_resolver,
                      PromiseResolver<int> pr) { *promise_resolver = pr; },
                   &promise_resolver)),
  ...
  bool race_fired = false;
  promise::Race(promises).Then(BindOnce(
      [](bool* race_fired, int result) {
  ...
  EXPECT_FALSE(race_fired);
  promise_resolver.resolve(54321);
  EXPECT_TRUE(race_fired);
}

TEST_F(PromiseTest, RaceVariantContainer) {
  PromiseResolver<std::string> promise_resolver;
  std::vector<Variant<Promise<int>, Promise<bool>, Promise<std::string>>>
      promises;

  promises.emplace_back(Promise<int>(BindOnce([](PromiseResolver<int> pr) {})));
  promises.emplace_back(Promise<std::string>(
      BindOnce([](PromiseResolver<std::string>* promise_resolver,
                  PromiseResolver<std::string> pr) { *promise_resolver = pr; },
               &promise_resolver)));

  promises.emplace_back(
      Promise<bool>(BindOnce([](PromiseResolver<bool> pr) {})));

  bool race_fired = false;
  promise::Race(promises).Then(BindOnce(
      [](bool* race_fired, Variant<int, bool, std::string> result) {
  ...
  EXPECT_FALSE(race_fired);
  promise_resolver.resolve(std::string("Hello"));
  EXPECT_TRUE(race_fired);
}
...
TEST_F(PromiseTest, AllIntContainer) {
  PromiseResolver<int> promise_resolver1;
  PromiseResolver<int> promise_resolver2;
  PromiseResolver<int> promise_resolver3;
  std::vector<Promise<int>> promises{
      Promise<int>(
          BindOnce([](PromiseResolver<int>* promise_resolver,
                      PromiseResolver<int> pr) { *promise_resolver = pr; },
  ...
  bool all_fired = false;
  promise::All(promises).Then(BindOnce(
      [](bool* all_fired, std::vector<int> result) {
        EXPECT_THAT(result, ElementsAre(10, 20, 30));
        *all_fired = true;
      },
      &all_fired));

  promise_resolver1.resolve(10);
  promise_resolver2.resolve(20);
  EXPECT_FALSE(all_fired);
  promise_resolver3.resolve(30);
  EXPECT_TRUE(all_fired);
}
...
TEST_F(PromiseTest, AllVariantContainer) {
  PromiseResolver<int> promise_resolver1;
  PromiseResolver<void> promise_resolver2;
  PromiseResolver<std::string> promise_resolver3;
  ...
  promises.emplace_back(Promise<int>(
      BindOnce([](PromiseResolver<int>* promise_resolver,
                  PromiseResolver<int> pr) { *promise_resolver = pr; },
               &promise_resolver1)));
  promises.emplace_back(Promise<void>(
      BindOnce([](PromiseResolver<void>* promise_resolver,
                  PromiseResolver<void> pr) { *promise_resolver = pr; },
               &promise_resolver2)));
  promises.emplace_back(Promise<std::string>(
  ...
  bool all_fired = false;
  promise::All(promises).Then(BindOnce(
      [](bool* all_fired, std::vector<Variant<int, Void, std::string>> result) {
  ...
  promise_resolver2.resolve();
  EXPECT_FALSE(all_fired);
  promise_resolver3.resolve(std::string("three"));
  EXPECT_TRUE(all_fired);
}
...
TEST_F(PromiseTest, PostTaskNestedPromises) {
  Promise<int>(BindOnce([](PromiseResolver<int> pr) { pr.resolve(100); }))
      .Then(
