...
#include "base/promise/promise_util.h"
...
TEST(PromiseTest, TestAll) {
  MessageLoop message_loop;

  std::vector<Future<int, NotReached>> futures;
  futures.push_back(Resolve(FROM_HERE, 1));
  futures.push_back(Resolve(FROM_HERE, 2));
  All(std::move(futures)).Then(FROM_HERE, BindOnce([](std::vector<int> res) {
                                 EXPECT_THAT(res, ::testing::ElementsAre(1, 2));
                               }));
}
