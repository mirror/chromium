...
class Promise {
 public:
  ...
  // promise for the return value of |on_reject| is returned. Three signatures
  // are supported for the return type of |on_reject|:
  // 1. PromiseResult<R> which supports resolve, reject and resolve with a
  //    curried Promise<R>.
  // 2. Promise<R> where the result is a curried promise.
  // 3. R
  // In all cases the return type is Promise<R>
  template <typename Result>
  auto Catch(OnceCallback<Result(const RejectValue&)> on_reject) {
    return ThenInternal(nullopt, nullptr, OnceCallback<Result(T)>(),
                        std::move(on_reject));
  }
  ...
  // task runner. A promise for the return value of |on_reject| is returned.
  ...
  // parent promise is rejected.  A promise for the return value of |on_reject|
  // is returned. Three signatures are supported for the return type of
  // |on_reject|:
  // 1. PromiseResult<R> which supports resolve, reject and resolve with a
  //    curried Promise<R>.
  // 2. Promise<R> where the result is a curried promise.
  ...
  // is resolved or rejected. A promise for the return value of |on_resolve| or
  // |on_reject| is returned.  Three signatures are supported for the return
  ...
  // In all cases the return type is Promise<R>
  template <typename Result>
  auto Then(OnceCallback<Result(T)> on_resolve,
            OnceCallback<Result(const RejectValue&)> on_reject =
                OnceCallback<Result(const RejectValue&)>()) {
    return ThenInternal(nullopt, nullptr, std::move(on_resolve),
                        std::move(on_reject));
  }
  ...
  // or on any previously specified task runner.  A promise for the return value
  // of |on_resolve| or |on_reject| is returned.  Three signatures are supported
  // for the return type of |on_resolve| or |on_reject|:
  // 1. PromiseResult<R> which supports resolve, reject and resolve with a
  //    curried Promise<R>.
  // 2. Promise<R> where the result is a curried promise.
  ...
  auto Then(const Location& from_here,
            OnceCallback<Result(T)> on_resolve,
            OnceCallback<Result(const RejectValue&)> on_reject =
                OnceCallback<Result(const RejectValue&)>()) {
    return ThenInternal(from_here, nullptr, std::move(on_resolve),
                        std::move(on_reject));
  }

  // A task to execute |on_resolve| or |on_reject| is posted on |task_runner| as
  // soon as the parent promise is resolved or rejected.  A promise for the
  ...
  auto Then(const Location& from_here,
            ...
            OnceCallback<Result(const RejectValue&)> on_reject =
                OnceCallback<Result(const RejectValue&)>()) {
    return ThenInternal(from_here, task_runner, std::move(on_resolve),
                        std::move(on_reject));
  }
...
class Promise<void> {
 ...
 public:
  ...
  template <typename Result>
  auto Catch(OnceCallback<Result(const RejectValue&)> on_reject) {
    return ThenInternal(nullopt, nullptr, OnceCallback<Result()>(),
                        std::move(on_reject));
  }
...
template <typename T>
template <typename R>
Promise<R> Promise<T>::ThenInternal(
    Optional<Location> from_here,
    scoped_refptr<SequencedTaskRunner> task_runner,
    OnceCallback<PromiseResult<R>(T)> on_resolve,
    OnceCallback<PromiseResult<R>(const RejectValue&)> on_reject) {
