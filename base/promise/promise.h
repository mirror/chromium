    ...
    scoped_refptr<internal::AbstractPromise> promise(subtle::AdoptRefIfNeeded(
        new internal::AbstractPromise(
            internal::AbstractPromise::ConstructWith<ThenResolveT,
                                                     ThenRejectT>(),
            from_here, internal::AbstractPromise::State::UNRESOLVED,
...
class PromiseResult {
 public:
  ...
  using ConstructWith =
      AbstractVariant::ConstructWith<Resolved<ResolveType>,
                                     Rejected<RejectType>,
  ...
  PromiseResult(std::enable_if_t<!std::is_same<ResolveT, RejectT>::value &&
                                 ...
                                 void>)
      : value_(ConstructWith()) {
    value_.TryAssign(std::conditional_t<std::is_void<ResolveT>::value,
                                        Resolved<void>, Rejected<void>>());
  }

  // Resolve overrides
  PromiseResult(const Resolved<ResolveType>& resolved)
      : value_(ConstructWith()) {
    value_.TryAssign(resolved);
  }

  PromiseResult(Resolved<ResolveType>&& resolved) : value_(ConstructWith()) {
    value_.TryAssign(std::move(resolved));
  }
  ...
  PromiseResult(std::enable_if_t<!std::is_same<ResolveT, RejectT>::value &&
      ...
      : value_(ConstructWith()) {
    value_.TryAssign(Resolved<ResolveType>{resolved});
  }
  ...
  PromiseResult(std::enable_if_t<!std::is_same<ResolveT, RejectT>::value &&
      ...
      : value_(ConstructWith()) {
    value_.TryAssign(Resolved<ResolveType>{std::move(resolved)});
  }

  // Resolve with Curried promise
  PromiseResult(Promise<ResolveType, RejectType> curried_promise)
      : value_(ConstructWith()) {
    value_.TryAssign(std::move(curried_promise.abstract_promise_));
  }

  // Reject overrides
  PromiseResult(const Rejected<RejectType>& rejected)
      : value_(ConstructWith()) {
    value_.TryAssign(rejected);
  }

  PromiseResult(Rejected<RejectType>&& rejected) : value_(ConstructWith()) {
    value_.TryAssign(rejected);
  }
  ...
  PromiseResult(std::enable_if_t<!std::is_same<ResolveT, RejectT>::value &&
      ...
      : value_(ConstructWith()) {
    value_.TryAssign(Rejected<RejectType>{rejected});
  }
  ...
  PromiseResult(std::enable_if_t<!std::is_same<ResolveT, RejectT>::value &&
      ...
      : value_(ConstructWith()) {
    value_.TryAssign(Rejected<RejectType>{std::move(rejected)});
  }
  ...
  AbstractVariant& value() { return value_; }
 ...
 public:
  ...
  All(Promise<Resolve, Reject>... promises) {
    ...
    scoped_refptr<internal::AbstractPromise> promise(subtle::AdoptRefIfNeeded(
        new internal::AbstractPromise(
            ...
            Location(), internal::AbstractPromise::State::UNRESOLVED,
            internal::AbstractPromise::PrerequisitePolicy::ALL,
            std::vector<scoped_refptr<internal::AbstractPromise>>{
  ...
  static Promise<ResolveType, RejectType> Reject(RejectType&& rejected) {
    scoped_refptr<internal::AbstractPromise> promise(subtle::AdoptRefIfNeeded(
        new internal::AbstractPromise(
            ...
            Location(), internal::AbstractPromise::State::REJECTED,
            internal::AbstractPromise::PrerequisitePolicy::NEVER,
  ...
  static Promise<ResolveType, RejectType> Resolve(ResolveType&& resolved) {
    scoped_refptr<internal::AbstractPromise> promise(subtle::AdoptRefIfNeeded(
        new internal::AbstractPromise(
            internal::AbstractPromise::ConstructWith<ResolveType, RejectType>(),
            Location(), internal::AbstractPromise::State::RESOLVED,
            internal::AbstractPromise::PrerequisitePolicy::NEVER,
            std::vector<scoped_refptr<internal::AbstractPromise>>(), nullptr),
        internal::AbstractPromise::kRefCountPreference));
    bool success = promise->value().TryAssign(
...
// Helpers for PostTask.
template <typename T>
struct PromiseCallbackTraits {
  using ResolveType = T;
  using RejectType = typename PromiseTraits<T>::DefaultRejectType;
};
...
template <typename T>
struct PostTaskPromiseHelper {
  using RejectType = typename PromiseCallbackTraits<T>::RejectType;

  static void Run(base::OnceCallback<T()> task,
                  scoped_refptr<internal::AbstractPromise> abstract_promise) {
    AssignHelper<T, Rejected<RejectType>>::TryAssign(abstract_promise.get(),
                                                     std::move(task).Run());
    abstract_promise->OnManualResolve();
  }
};

template <>
struct PostTaskPromiseHelper<void> {
  static void Run(base::OnceCallback<void()> task,
                  scoped_refptr<internal::AbstractPromise> abstract_promise) {
    std::move(task).Run();
    bool success = abstract_promise->value().TryAssign(Resolved<void>());
    DCHECK(success);
    abstract_promise->OnManualResolve();
  }
};
