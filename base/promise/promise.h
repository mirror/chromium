...
class Future {
 ...
 public:
  ...
  typename internal::ThenCorruptionHelper<R, RejectT>::ResultType Then(
      ...
      OnceCallback<ResolveSignature<R>> handler) && {
    static_assert(!std::is_same<ResolveT, NotReached>::value,
                  "Don't handle NotReached promise with Then().");
    DCHECK(storage_);
  ...
  typename internal::CatchCorruptionHelper<R, ResolveT>::ResultType Catch(
    ...
    return std::move(*this).Catch(from_here, SequencedTaskRunnerHandle::Get(),
                                  std::move(handler));
  }
  ...
  typename internal::CatchCorruptionHelper<R, ResolveT>::ResultType Catch(
      ...
      scoped_refptr<TaskRunner> task_runner,
      OnceCallback<RejectSignature<R>> handler) && {
    static_assert(!std::is_same<RejectT, NotReached>::value,
                  "Don't handle NotReached promise with Catch().");
    ...
    auto relay = [](const Location& from_here, PromiseType promise,
      ...
      if (value.is_resolved_) {
        std::move(promise).Resolve(from_here, std::move(value.resolved_));
        return;
      }
      Helper::Settle(
          from_here, std::move(promise),
  ...
  typename internal::CatchCorruptionHelper<R, ResolveT>::ResultType Catch(
    ...
    return std::move(*this).Catch(
        from_here, OnceCallback<RejectSignature<R>>(std::move(handler)));
  }
