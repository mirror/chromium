...
class BASE_EXPORT Promise {
 public:
  Promise(OnceCallback<void(PromiseResolver<T>)> initalizer) {
    scoped_refptr<internal::InitialPromise<T>> promise(
        new internal::InitialPromise<T>());
    std::move(initalizer).Run(PromiseResolver<T>(promise));
    internal_promise_ = promise;
  }

  explicit Promise(scoped_refptr<internal::InternalPromise<T>> internal_promise)
 ...
 private:
  ...
  scoped_refptr<internal::InternalPromise<T>> internal_promise_;
};
...
class BASE_EXPORT promise {
 public:
  ...
  static auto All(Promises... promises) {
    ...
    return Promise<ReturnType>(
        MakeRefCounted<internal::AllPromise<ReturnType, Promises...>>(
            std::vector<scoped_refptr<PromiseBase>>(
  ...
  static Promise<T> Reject(Value err) {
    return Promise<T>(MakeRefCounted<internal::InternalPromise<T>>(
        RejectValue(std::move(err))));
  }
  ...
  static Promise<T> Resolve(T&& t) {
    return Promise<T>(
        MakeRefCounted<internal::InternalPromise<T>>(std::forward<T>(t)));
  }

  static Promise<void> Resolve() {
    return Promise<void>(
        MakeRefCounted<internal::InternalPromise<void>>(Void()));
  }
...
// Object passed to the callback argument of the Promise<T> constructor.  This
// is analogous to the ES6 promise although it deals with both resolving and
// rejecting a promise.
template <typename T>
class BASE_EXPORT PromiseResolver {
 public:
  ...
  void reject(RejectValue err) { promise_->RejectInternal(std::move(err)); }
...
class BASE_EXPORT PromiseResult {
 public:
  ...
  Variant<NonVoidT, RejectValue, scoped_refptr<internal::InternalPromise<T>>>&
  value() {
    return value_;
  }
