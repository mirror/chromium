...
TEST_F(PromiseTest, ThenCanBeVoidWhenPromiseWasnt) {
  Promise<int> p =
      Promise<int>(BindOnce([](PromiseResolver<int> pr) { pr.resolve(123); }));

  bool called = false;
  p.Then(BindOnce([](bool* called) { *called = true; }, &called));
  EXPECT_TRUE(called);
}
...
TEST_F(PromiseTest, RejectToDisambiguateThenReturnValue) {
  Promise<void>(BindOnce([](PromiseResolver<void> pr) { pr.resolve(); }))
      .Then(BindOnce(
          []() -> PromiseResult<Value> { return RejectValue("Oh no!"); }))
...
TEST_F(PromiseTest, All) {
  ...
  promise::All(p1, p2, p3)
      .Then(BindOnce(
          [](bool* all_fired, std::tuple<float, int, bool> result) {
...
TEST_F(PromiseTest, AllReject) {
  ...
  bool caught = false;
  promise::All(p1, p2, p3)
      .Then(BindOnce([](std::tuple<float, int, bool> result) {
              FAIL() << "We shouldn't get here, the promise was rejected!";
            }),
            BindOnce(
                [](bool* caught, const RejectValue& err) {
  ...
  EXPECT_FALSE(caught);
  promise_resolver1.reject("Whoops!");
  EXPECT_TRUE(caught);
}
...
TEST_F(PromiseTest, AllIntVoid) {
  ...
  promise::All(p1, p2).Then(BindOnce(
      [](bool* all_fired, std::tuple<int, Void> result) {
...
TEST_F(PromiseTest, AllMoveOnlyType) {
  ...
  promise::All(p1, p2, p3)
      .Then(BindOnce([](std::tuple<std::unique_ptr<float>, std::unique_ptr<int>,
                                   std::unique_ptr<bool>> result) {
        EXPECT_EQ(1.234f, *std::get<0>(result));
        EXPECT_EQ(1234, *std::get<1>(result));
        EXPECT_TRUE(*std::get<2>(result));
...
TEST_F(PromiseTest, AllVoidWithVoidThen) {
  PromiseResolver<void> promise_resolver1;
  PromiseResolver<void> promise_resolver2;
  PromiseResolver<void> promise_resolver3;
  Promise<void> p1 = Promise<void>(
      BindOnce([](PromiseResolver<void>* promise_resolver1,
                  PromiseResolver<void> pr) { *promise_resolver1 = pr; },
               &promise_resolver1));
  Promise<void> p2 = Promise<void>(
      BindOnce([](PromiseResolver<void>* promise_resolver2,
                  PromiseResolver<void> pr) { *promise_resolver2 = pr; },
               &promise_resolver2));
  Promise<void> p3 = Promise<void>(
      BindOnce([](PromiseResolver<void>* promise_resolver3,
                  PromiseResolver<void> pr) { *promise_resolver3 = pr; },
               &promise_resolver3));

  bool all_fired = false;
  promise::All(p1, p2, p3)
      .Then(BindOnce([](bool* all_fired) { *all_fired = true; }, &all_fired));

  promise_resolver1.resolve();
  promise_resolver2.resolve();
  EXPECT_FALSE(all_fired);

  promise_resolver3.resolve();
  EXPECT_TRUE(all_fired);
}
...
TEST_F(PromiseTest, PostTaskRejectThen) {
  Promise<int> p = Promise<int>(
      BindOnce([](PromiseResolver<int> pr) { pr.reject("Oh no!"); }));
...
TEST_F(PromiseTest, PostTaskCatch) {
  Promise<int>(BindOnce([](PromiseResolver<int> pr) { pr.reject("Whoops!"); }))
...
TEST_F(PromiseTest, PostTaskCatchThen) {
  Promise<int>(BindOnce([](PromiseResolver<int> pr) { pr.reject(123); }))
      .Catch(BindOnce([](const RejectValue& err) { return err->GetInt(); }))
      .Then(BindOnce(
          [](base::RunLoop* run_loop, int result) {
            EXPECT_EQ(123, result);
...
TEST_F(PromiseTest, PostTaskRaceReject) {
  ...
  promise_resolver.reject("Whoops!");
  run_loop_.Run();
}
