...
TEST_F(PromiseTest, RejectThen) {
  ...
  p.Then(
      ...
      BindOnce([](const Value& err) { EXPECT_EQ("Oh no!", err.GetString()); }));
}
  ...
  Promise<std::vector<size_t>>(
      ...
      .Then(BindOnce(
          [](std::vector<size_t> result) -> PromiseResult<std::vector<size_t>> {
            return Reject<Value>(Value("Oh no!"));
          }))
      .Then(BindOnce([](std::vector<size_t> result) {
              FAIL() << "We shouldn't get here, the promise was rejected!";
            }),
            BindOnce([](const Value& err) {
              EXPECT_EQ("Oh no!", err.GetString());
            }));
}
...
TEST_F(PromiseTest, ResolveToDisambiguateThenReturnValue) {
  Promise<void>(BindOnce([](PromiseResolver<void> pr) { pr.resolve(); }))
      .Then(BindOnce([]() -> PromiseResult<Value> {
        return Resolve<Value>(Value("Hello."));
      }))
      .Then(BindOnce(
...
TEST_F(PromiseTest, CatchRejectInThenChain) {
  ...
  Promise<int>(BindOnce([](PromiseResolver<int> pr) { pr.resolve(123); }))
      .Then(BindOnce([](int result) -> PromiseResult<int> {
        return Reject<Value>(Value("Whoops!"));
      }))
      .Then(BindOnce([](int result) { return result; }))
      .Then(BindOnce([](int result) { return result; }))
      .Catch(BindOnce(
...
TEST_F(PromiseTest, CatchThen) {
  ...
  Promise<int>(BindOnce([](PromiseResolver<int> pr) { pr.reject(Value(123)); }))
      .Catch(BindOnce([](const Value& err) { return err.GetInt(); }))
...
TEST_F(PromiseTest, SettledTaskFinally) {
  ...
  Promise<int>(BindOnce([](PromiseResolver<int> pr) { pr.resolve(123); }))
      ...
      .Finally(BindOnce([](int* result) { EXPECT_EQ(123, *result); }, &result));
}
...
TEST_F(PromiseTest, ResolveFinally) {
  ...
  p.Finally(BindOnce([](int* result) { EXPECT_EQ(123, *result); }, &result));
  promise_resolver.resolve(123);
}
...
TEST_F(PromiseTest, RejectFinally) {
  ...
  p.Then(
      ...
      BindOnce([](int* result, const Value& err) { *result = -1; }, &result));
  p.Finally(BindOnce([](int* result) { EXPECT_EQ(-1, *result); }, &result));
  promise_resolver.reject(Value("Oh no!"));
}
  ...
  Promise<void> p =
      Promise<void>(BindOnce(
          ...
          .Then(BindOnce(
              ...
              &log))
          .Catch(BindOnce(
              [](std::vector<std::string>* log, const Value& err) {
...
TEST_F(PromiseTest, RaceResolve) {
  ...
  promise::Race(p1, p2, p3).Then(BindOnce([](Variant<float, int, bool> result) {
...
TEST_F(PromiseTest, RaceResolveMoveOnlyType) {
  ...
  promise::Race(p1, p2, p3)
      .Then(BindOnce([](Variant<std::unique_ptr<float>, std::unique_ptr<int>,
...
TEST_F(PromiseTest, All) {
  ...
  promise::All(p1, p2, p3)
      .Then(BindOnce(
          [](std::tuple<Variant<Resolve<float>, Reject<Value>>,
                        Variant<Resolve<int>, Reject<Value>>,
                        Variant<Resolve<bool>, Reject<Value>>> result) {
            EXPECT_EQ(1.234f, *Get<Resolve<float>>(&std::get<0>(result)));
            EXPECT_EQ(1234, *Get<Resolve<int>>(&std::get<1>(result)));
            EXPECT_TRUE(*Get<Resolve<bool>>(&std::get<2>(result)));
          }));
...
TEST_F(PromiseTest, AllMoveOnlyType) {
  ...
  promise::All(p1, p2, p3)
      .Then(BindOnce(
          [](std::tuple<Variant<Resolve<std::unique_ptr<float>>, Reject<Value>>,
                        Variant<Resolve<std::unique_ptr<int>>, Reject<Value>>,
                        Variant<Resolve<std::unique_ptr<bool>>, Reject<Value>>>
                 result) {
            EXPECT_EQ(1.234f, **Get<Resolve<std::unique_ptr<float>>>(
                                  &std::get<0>(result)));
            EXPECT_EQ(1234, **Get<Resolve<std::unique_ptr<int>>>(
                                &std::get<1>(result)));
            EXPECT_TRUE(
                **Get<Resolve<std::unique_ptr<bool>>>(&std::get<2>(result)));
          }));
...
TEST_F(PromiseTest, PostTaskRejectThen) {
  ...
  p.Then(FROM_HERE,
         BindOnce(
             ...
             &run_loop_),
         BindOnce(
             [](base::RunLoop* run_loop, const Value& err) {
               EXPECT_EQ("Oh no!", err.GetString());
               run_loop->Quit();
             },
             &run_loop_));
