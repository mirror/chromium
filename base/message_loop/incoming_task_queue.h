...
class BASE_EXPORT IncomingTaskQueue
 ...
 private:
  ...
  //
  // TriageQueue
  // The queue that receives all tasks from the thread-safe
  // |incoming_task_queue_| (reloading from it every time it needs more tasks).
  // From there: tasks are generally either dispatched immediately or sent to
  // the queues below.
  //
  // DelayedQueue
