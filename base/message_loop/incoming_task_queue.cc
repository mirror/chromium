...
void IncomingTaskQueue::TriageQueue::ReloadFromIncomingQueueIfEmpty() {
  if (queue_.empty()) {
    // TODO(robliao): Since any high resolution tasks aren't yet in the delayed
    // queue, they technically shouldn't trigger high resolution timers until
    // they are.
...
void IncomingTaskQueue::DeferredQueue::Push(PendingTask pending_task) {
  ...
  // TODO(robliao): Since these tasks are deferred, techincally they shouldn't
  // count towards the high res task count.
  if (pending_task.is_high_res)
    ++outer_->pending_high_res_tasks_;
