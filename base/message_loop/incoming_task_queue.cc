...
IncomingTaskQueue::InitialQueue::InitialQueue(IncomingTaskQueue* outer)
    : outer_(outer) {}

IncomingTaskQueue::InitialQueue::~InitialQueue() = default;

const PendingTask& IncomingTaskQueue::InitialQueue::Peek() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(outer_->sequence_checker_);
  DCHECK(!queue_.empty());
  return queue_.front();
}

PendingTask IncomingTaskQueue::InitialQueue::Pop() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(outer_->sequence_checker_);
...
bool IncomingTaskQueue::InitialQueue::HasTasks() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(outer_->sequence_checker_);
  ReloadFromIncomingQueue();
  return !queue_.empty();
}

bool IncomingTaskQueue::InitialQueue::Clear() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(outer_->sequence_checker_);
  // Avoid reloading so that clear can finish on this sequence and maintain the
  // previous clearing behavior.
...
void IncomingTaskQueue::InitialQueue::ReloadFromIncomingQueue() {
  if (queue_.empty())
...
bool IncomingTaskQueue::DelayedQueue::Clear() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(outer_->sequence_checker_);
...
bool IncomingTaskQueue::DeferredQueue::Clear() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(outer_->sequence_checker_);
  bool has_tasks = !queue_.empty();
  while (!queue_.empty())
    Pop();
