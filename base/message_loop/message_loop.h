...
class BASE_EXPORT MessageLoop : public MessagePump::Delegate,
  ...
  class ScopedNestableTaskAllower {
   public:
    explicit ScopedNestableTaskAllower(MessageLoop* loop)
      ...
      // TODO(gab): As part of the MessageLoop::*Nestable*() API cleanup,
      // confirm that ScopedNestableTaskAllowers don't nest.
      DCHECK(!old_state_);
 ...
 private:
  ...
  //   1) Prevents undesired reentrancy (by being set to kSystemTasksOnly while
  //      a task is in progress at the current run-level, which prevents
  //      anything but a nested system loop -- beyond MessageLoop's control --
