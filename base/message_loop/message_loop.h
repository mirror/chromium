...
class BASE_EXPORT MessageLoop : public MessagePump::Delegate,
 ...
 private:
  ...
  // A boolean which prevents unintentional reentrant task execution, preventing
  // reentry from (sometimes unintentionally) induced nested message pumps. As
  // such, nested message pumps will only process system messages (not
  // application tasks) by default. A nested loop layer must have been
  // explicitly granted permission to be able to execute application tasks. This
  // is granted either by RunLoop::Type::kNestableTasksAllowed when the loop is
  // driven by the application or by a ScopedNestableTaskAllower preceding a
  // system call that is known to generate a system-driven nested loop.
