...
MessagePumpFuchsia::~MessagePumpFuchsia() {
  CHECK(mx_handle_close(port_) == NO_ERROR);
}
...
bool MessagePumpFuchsia::WatchFileDescriptor(int fd,
  ...
  CHECK(mx_object_wait_async(controller->handle_, port_, controller_as_key,
                             signals, MX_WAIT_ASYNC_ONCE) == NO_ERROR);
  return true;
}
...
void MessagePumpFuchsia::Run(Delegate* delegate) {
  ...
  for (;;) {
    ...
    if (packet.type == MX_PKT_TYPE_SIGNAL_ONE) {
      ...
      uint32_t signals;
      __mxio_wait_end(controller->io_, packet.signal.observed, &signals);

      if ((signals & (MXIO_EVT_READABLE | MXIO_EVT_WRITABLE)) ==
          (MXIO_EVT_READABLE | MXIO_EVT_WRITABLE)) {
        // Both callbacks to be called, must check controller destruction.
      ...
      } else if (signals & MXIO_EVT_WRITABLE) {
        controller->watcher_->OnFileCanWriteWithoutBlocking(controller->fd_);
      } else if (signals & MXIO_EVT_READABLE) {
        controller->watcher_->OnFileCanReadWithoutBlocking(controller->fd_);
      }
    } else {
      // Wakeup caused by ScheduleWork().
      CHECK(packet.type == MX_PKT_TYPE_USER);
    }
  }
...
void MessagePumpFuchsia::ScheduleWork() {
  ...
  CHECK(mx_port_queue(port_, &packet, 0) == NO_ERROR);
}
