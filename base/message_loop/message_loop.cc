...
void MessageLoop::SetNestableTasksAllowed(bool allowed) {
  ...
  task_type_allowed_ = allowed ? kNestableAndSystemTasks : kNoTasks;
}
...
void MessageLoop::Run(RunLevelTaskType run_level_task_type) {
  ...
  // |task_type_allowed_| off of the kNoTasks reentrancy blocker ahead of the
  // Run() call. Note: std::max(run_level_task_type, task_type_allowed_) as an
  // unconditional value here would also be incorrect because it's possible for
  // Run() to nest without the reentrancy blocker having been set if it's
  // invoked from a system task being processed by the top-level loop (i.e.
  // which didn't go through MessageLoop::RunTask()), in that case we want to
  // honor the |run_level_task_type| not use the existing |task_type_allowed_|
  // which is still set to kAllTasks.
