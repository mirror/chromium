...
MessageLoop::~MessageLoop() {
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    did_work = incoming_task_queue_->initial_tasks().HasTasks();
    if (!did_work)
      break;
  }
  DCHECK(!did_work);
...
void MessageLoop::EnsureWorkScheduled() {
  DCHECK_EQ(this, current());
  if (incoming_task_queue_->initial_tasks().HasTasks())
    pump_->ScheduleWork();
}
...
bool MessageLoop::DeletePendingTasks() {
  bool did_work = incoming_task_queue_->initial_tasks().Clear();
  did_work |= incoming_task_queue_->deferred_tasks().Clear();
  ...
  // when it's safe.
  did_work |= incoming_task_queue_->delayed_tasks().Clear();
  return did_work;
}
...
bool MessageLoop::DoWork() {
  ...
  for (;;) {
    ...
    // Execute oldest task.
    do {
      PendingTask pending_task = incoming_task_queue_->initial_tasks().Pop();
      if (pending_task.task.IsCancelled())
        continue;
    ...
    } while (incoming_task_queue_->initial_tasks().HasTasks());
  }
