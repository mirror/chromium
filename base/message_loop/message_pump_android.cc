...
void MessagePumpForUI::DoRunLoopOnce(JNIEnv* env,
  ...
  // If the message pump has been aborted, don't keep running tasks.
  if (ShouldAbort())
    return;
  ...
  if (!next_delayed_work_time.is_null()) {
    ...
    if (delayed_scheduled_time_ == base::TimeTicks() ||
        next_delayed_work_time < delayed_scheduled_time_) {
      long scheduled_time_millis =
          (next_delayed_work_time - base::TimeTicks::Now())
              .InMillisecondsRoundedUp();
      Java_SystemMessageHandler_scheduleDelayedWork(
          env, system_message_handler_obj_, scheduled_time_millis);
    }
  }
}
