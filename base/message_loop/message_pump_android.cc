...
static void DoRunLoopOnce(JNIEnv* env,
  ...
  // have to return control back to the looper after each message. This also
  // means we have to perform idle detection differently, which is why we add an
  // IdleHandler to the message queue in SystemMessageHandler.java, which calls
  // DoIdleWork whenever control returns back to the looper and there are no
  // tasks queued up to run immediately.
  delegate->DoWork();
  if (pump->ShouldAbort()) {
