...
class MessageLoopPerfTest : public ::testing::TestWithParam<int> {
  ...
  MessageLoopPerfTest()
      : message_loop_task_runner_(SequencedTaskRunnerHandle::Get()),
        run_posting_threads_(WaitableEvent::ResetPolicy::MANUAL,
                             WaitableEvent::InitialState::NOT_SIGNALED) {
    subtle::NoBarrier_Store(&num_tasks_posted_, 0);
  }
 ...
 protected:
  ...
  void RunTest(const int num_posting_threads, TimeDelta duration) {
    ...
    for (int i = 0; i < num_posting_threads; ++i) {
      threads.emplace_back(PostingThread::Create(&run_posting_threads_,
                                                 MakeUnique<ActionType>(this)));
      // Don't assert here to simply the code that requires a Join() call for
  ...
  size_t num_tasks_posted() {
    return subtle::NoBarrier_Load(&num_tasks_posted_);
  }

  TimeDelta tasks_posted_duration() { return tasks_posted_duration_; }

  size_t num_tasks_run() { return num_tasks_run_; }
 ...
 private:
  ...
  // Accessed on multiple threads, thread-safe or constant after SetUp():
  ...
  subtle::AtomicWord num_tasks_posted_;
