...
class BASE_EXPORT MessagePumpLibevent : public MessagePump {
 public:
  ...
  // of registrations.  If |controller| is already attached to one or more
  // registrations, the new registration is added on to those.  If an error
  // occurs while calling this method, any registration previously attached to
  // |controller| is removed.  Returns true on success.  Must be called on the
  // same thread the message_pump is running on.
  //
