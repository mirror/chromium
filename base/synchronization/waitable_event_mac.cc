...
void WaitableEvent::Reset() {
  PeekPort(receive_right_->get(), true);
}
...
void WaitableEvent::Signal() {
  ...
  MACH_CHECK(kr == KERN_SUCCESS || kr == MACH_SEND_TIMED_OUT, kr)
      << "mach_msg Signal";
}

bool WaitableEvent::IsSignaled() {
  return PeekPort(receive_right_->get(), auto_reset_);
}
...
bool WaitableEvent::TimedWaitUntil(const TimeTicks& end_time) {
  ...
  // size of 0.
  const mach_msg_size_t rcv_size = auto_reset_ ? sizeof(msg) : 0;

  msg.header.msgh_local_port = receive_right_->get();
  kern_return_t kr = mach_msg(&msg.header, options, 0, rcv_size,
                              receive_right_->get(), timeout, MACH_PORT_NULL);
  // MACH_LOG(INFO, kr) << "TimedWaitUntil";
  if (kr == KERN_SUCCESS) {
    return true;
...
size_t WaitableEvent::WaitMany(WaitableEvent** raw_waitables, size_t count) {
  ...
  for (size_t i = 0; i < count; ++i) {
    kr = mach_port_insert_member(mach_task_self(),
                                 raw_waitables[i]->receive_right_->get(),
                                 port_set.get());
  ...
  // identify which port in the set is signaled.
  ...
  /*
  MACH_LOG(INFO, kr) << "mach_msg";
  LOG(INFO) << "WaitMany: bits=" << std::hex << msg.header.msgh_bits << std::dec
            << ", size=" << msg.header.msgh_size
            << ", remote=" << msg.header.msgh_remote_port
            << ", local=" << msg.header.msgh_local_port
            << ", voucher=" << msg.header.msgh_voucher_port
            << ", id=" << msg.header.msgh_id;
            */

  for (size_t i = 0; i < count; ++i) {
    if (msg.header.msgh_local_port == raw_waitables[i]->receive_right_->get()) {
      if (raw_waitables[i]->auto_reset_) {
        // For auto-reset events, the message needs to be drained to reset the
        // event.
...
bool WaitableEvent::PeekPort(mach_port_t port, bool dequeue) {
  if (dequeue) {
    ...
    MACH_CHECK(kr == KERN_SUCCESS || kr == MACH_RCV_TIMED_OUT, kr)
        << "mach_msg";
