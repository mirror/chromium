...
WaitableEvent::WaitableEvent(ResetPolicy reset_policy,
    ...
    : kqueue_(kqueue()),
      ...
      async_waiters_() {
  CHECK(dispatch_queue_.get());
  PCHECK(kqueue_.is_valid()) << "kqueue";
  uint16_t flags = EV_ADD;
  uint32_t fflags = 0;

  LOG(INFO) << "Kqueue " << this << " = " << kqueue_.get();
...
//  LOG(INFO) << "Did reset " << this << " (#" << kqueue_.get() << ")";
  int rv = HANDLE_EINTR(
      kevent64(kqueue_.get(), &event, 1, nullptr, 0, 0, nullptr));
...
size_t WaitableEvent::WaitMany(WaitableEvent** raw_waitables,
  ...
  for (size_t i = 0; i < static_cast<size_t>(rv); ++i) {
    LOG(INFO) << "[" << i << "] ident=" << out_events[i].ident << ", flags="
      << out_events[i].flags << ", fflags=" << out_events[i].fflags << ", "
      << "data=" << out_events[i].data << ", udata=" << out_events[i].udata;
