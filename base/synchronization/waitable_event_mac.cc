...
size_t WaitableEvent::WaitMany(WaitableEvent** raw_waitables, size_t count) {
  ...
  // On macOS 10.11+, using Mach port sets may cause system instability
  // https://crbug.com/756102. On macOS 10.12+, a kqueue can be used
  // instead. On macOS 10.9, using a kqueue on multiple ports is not reliable,
  // but port sets do not have an issue. macOS 10.11 kqueue only works on
  // port sets, so take a risk with using port sets directly.
