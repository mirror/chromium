...
WaitableEvent::WaitableEvent(ResetPolicy reset_policy,
  ...
  if (UseSlowWatchList(policy_)) {
    watch_list_lock_.reset(new Lock());
    watch_list_.reset(new std::list<OnceClosure>());
  }
...
void WaitableEvent::Signal() {
  ...
  // If the event is newly signaled, call-back the slow-path wath list.
  if (kr == KERN_SUCCESS && UseSlowWatchList(policy_)) {
    std::list<OnceClosure> watch_list;
    {
      AutoLock lock(*watch_list_lock_);
      std::swap(watch_list, *watch_list_);
    }
