...
//
// The last argument to this function, |find_fn|, should be either
// &StringType::find or &StringType::find_first_of. Using find() means to search
// for substring matches; using find_first_of() means to match any single
// character that appears in |find_this|.
//
// Parameterizing |find_fn| allows this function to be an implementation for
// both ReplaceChars and ReplaceSubstringsAfterOffset.
...
bool DoReplaceMatchesAfterOffset(
    ...
    size_t (StringType::*find_fn)(const typename StringType::value_type*,
                                  size_t,
                                  size_t) const) {
  ...
  size_t match_length;
  if (find_fn == static_cast<decltype(find_fn)>(&StringType::find_first_of)) {
    match_length = 1;  // find_first_of() matches are one char long.
  } else {
    DCHECK(find_fn == static_cast<decltype(find_fn)>(&StringType::find));
...
bool ReplaceCharsT(const StringType& input,
  ...
  *output = input;
  ...
  return DoReplaceMatchesAfterOffset(output, 0, find_any_of_these, replace_with,
                                     ReplaceType::REPLACE_ALL,
                                     &StringType::find_first_of);
}
...
void ReplaceFirstSubstringAfterOffset(string16* str,
  ...
  DoReplaceMatchesAfterOffset<string16>(
      str, start_offset, find_this, replace_with, ReplaceType::REPLACE_FIRST,
      &string16::find);
}
...
void ReplaceSubstringsAfterOffset(std::string* str,
                                  ...
                                  StringPiece replace_with) {
  DoReplaceMatchesAfterOffset<std::string>(
      str, start_offset, find_this, replace_with, ReplaceType::REPLACE_ALL,
      &std::string::find);
}
