...
//
// |use_find_first_of| determines whether to search using string::find() or
// string::find_first_of(). Using find() means that matches will be substrings
// equal to |find_this| (ReplaceSubstrings semantics). Using find_first_of()
// means that matches will be any single character that appears in |find_this|
// (ReplaceChars semantics).
template <class StringType>
bool DoReplaceMatchesAfterOffset(StringType* str,
                                 ...
                                 BasicStringPiece<StringType> replace_with,
                                 bool replace_all,
                                 bool use_find_first_of = false) {
  using CharTraits = typename StringType::traits_type;
  DCHECK(!find_this.empty());

  // |find_this| can either be a substring, or a list of characters to match;
  // use std::string::find in the first case, and std::string::find_first_of in
  // the other case. They have the same signature.
  using FindFn = size_t (StringType::*)(const typename StringType::value_type*,
                                        size_t, size_t) const;
  FindFn find_fn;
  size_t match_length;
  if (use_find_first_of) {
    find_fn = &StringType::find_first_of;
    match_length = 1;
  } else {
    find_fn = &StringType::find;
    match_length = find_this.length();
  }

  // If the find string doesn't appear, there's nothing to do.
  size_t first_match =
      (str->*find_fn)(find_this.data(), initial_offset, find_this.length());
  ...
  if (!replace_all) {
    str->replace(first_match, match_length, replace_with.data(),
                 replace_length);
    return true;
  }
  ...
  if (match_length == replace_length) {
    auto* buffer = &((*str)[0]);
    for (size_t offset = first_match; offset != StringType::npos;
         offset = (str->*find_fn)(find_this.data(), offset + replace_length,
                                  find_this.length())) {
  ...
  size_t expansion = 0;
  if (replace_length > match_length) {
    // This operation lengthens the string; determine the new length by counting
    // matches.
    const size_t expansion_per_match = (replace_length - match_length);
    size_t num_matches = 0;
    for (size_t match = first_match; match != StringType::npos;
         match = (str->*find_fn)(find_this.data(), match + match_length,
                                 find_this.length())) {
    ...
    if (str->capacity() < final_length) {
      ...
      for (size_t match = first_match;;
           match = (src.*find_fn)(find_this.data(), pos, find_this.length())) {
    ...
    // string.
    size_t shift_src = first_match + match_length;
...
bool ReplaceCharsT(const StringType& input,
  ...
  *output = input;
  ...
  if (find_any_of_these.empty())
    return false;

  return DoReplaceMatchesAfterOffset(
      output, 0, find_any_of_these, replace_with,
      true /* replace all matches */,
      true /* use StringType::find_first_of(find_any_of_these) for matching */);
}
...
void ReplaceFirstSubstringAfterOffset(string16* str,
  ...
  DoReplaceMatchesAfterOffset<string16>(str, start_offset, find_this,
                                        replace_with, false);  // Replace first.
}
...
void ReplaceFirstSubstringAfterOffset(std::string* str,
  ...
  DoReplaceMatchesAfterOffset<std::string>(
...
void ReplaceSubstringsAfterOffset(std::string* str,
                                  ...
                                  StringPiece find_this,
                                  StringPiece replace_with) {
  DoReplaceMatchesAfterOffset<std::string>(str, start_offset, find_this,
                                           replace_with, true);  // Replace all.
}
