...
#if !defined(COMPONENT_BUILD) && defined(OS_LINUX)
...
#elif !defined(COMPONENT_BUILD) && defined(OS_MACOSX)
...
class AutoWritableMemory {
 private:
  ...
  // writers itself is also placed in the protected memory section to avoid the
  // case where an attacker could overwrite it with a large value and invoke
  // code that calls constructs and destructs an AutoWritableMemory. After such
  // a call the memory would still not be set read-only because writers > 0.
  static int writers;
