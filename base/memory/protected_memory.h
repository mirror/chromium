...
// The variable can be set read-write by creating a scoped AutoWritableMemory
// object by calling AutoWritableMemory::Create(), the memory stays writable
// until the returned object goes out of scope and is destructed. The wrapped
// variable can be accessed using operator* and operator->.
//
...
// past the end of the PROTECTED_MEMORY_SECTION. Under component builds, this
// should correctly reference a given DSOs specific protected memory region.
...
class AutoWritableMemory {
 private:
  ...
  //
  // In non-component builds, writers itself is also placed in the protected
  // memory section to avoid the case where an attacker could overwrite it with
  // a large value and invoke code that calls constructs and destructs an
  // AutoWritableMemory. After such a call the memory would still not be set
  // read-only because writers > 0. This is disabled for component builds due to
  // limitations creating a DSO-specific writers variable.
  static int writers;
  ...
  static base::LazyInstance<Lock>::Leaky writers_lock;
