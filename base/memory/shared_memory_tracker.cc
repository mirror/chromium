...
bool SharedMemoryTracker::OnMemoryDump(const trace_event::MemoryDumpArgs& args,
  ...
  for (const auto& usage : usages) {
    ...
    // created nowhere but ProcessMemoryDump::CreateSharedMemoryOwnershipEdge.
    // Now this is not true since GetDumpGUIDForTracing creates dumps, and the
    // logic needs to be fixed. See the discussion at
