...
const int kNoneToSuspectedIntervalMs = 10000;
...
const int kSuspectedIntervalCoolDownMs = 16000;
...
const int kSuspectedToConfirmedIntervalMs = 18000;

// The period of time during which swap-thrashing should not be observed in
// order to switch from the SWAP_THRASHING_LEVEL_CONFIRMED to the
// SWAP_THRASHING_LEVEL_SUSPECTED level, in milliseconds.
const int kConfirmedCoolDownMs = 24000;

const size_t kThrashSwappingPageFaultThreshold = 20;
const size_t kPageFaultCooldownThreshold = 5;
...
SwapThrashingDelegate::PageFaultDetectionWindow::PageFaultDetectionWindow(
    size_t window_size)
...
void SwapThrashingDelegate::PageFaultDetectionWindow::ResetWindow(
...
void SwapThrashingDelegate::PageFaultDetectionWindow::AddSample(
    uint64_t sample) {
  if (window_size_ == 0)
    return;
  size_t samples_sum =
...
bool SwapThrashingDelegate::PageFaultDetectionWindow::IsReady() const {
  if (window_size_ == 0 || page_fault_counts_.size() < window_size_)
    return false;
  return true;
}

double SwapThrashingDelegate::PageFaultDetectionWindow::GetAverage() const {
  return average_;
}

SwapThrashingLevel SwapThrashingDelegate::CalculateCurrentSwapThrashingLevel(
  ...
  switch (previous_state) {
    ...
    default:
      return SWAP_THRASHING_LEVEL_INVALID;
  }
