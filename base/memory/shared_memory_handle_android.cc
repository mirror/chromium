  ...
  switch (type) {
    ...
    case Type::POSIX:
      if (file_descriptor.fd < 0) {
        LOG(ERROR) << __FUNCTION__ << ";;; invalid fd=" << file_descriptor.fd;
    ...
    case Type::ANDROID_HARDWARE_BUFFER:
      ...
      if (!base::AndroidHardwareBufferCompat::IsSupportAvailable()) {
        LOG(ERROR) << "AHardwareBuffer support not available";
        type_ = Type::NO_HANDLE;
        return;
      }
      ...
      if (ret < 0) {
        LOG(ERROR) << "AHardwareBuffer receive failed: ret=" << ret;
        type_ = Type::NO_HANDLE;
        return;
      }
...
int SharedMemoryHandle::GetHandle() const {
  switch (type_) {
    ...
    case Type::ANDROID_HARDWARE_BUFFER:
      ...
      if (ret < 0) {
        LOG(ERROR) << "AHardwareBuffer send failed: ret=" << ret;
        return -1;
      }
