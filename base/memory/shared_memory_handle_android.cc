...
SharedMemoryHandle::SharedMemoryHandle(
    ...
    : SharedMemoryHandle(file_descriptor, size, guid, Type::ASHMEM) {}
...
SharedMemoryHandle::SharedMemoryHandle(
  ...
  switch (type) {
    ...
    case Type::ANDROID_HARDWARE_BUFFER:
      ...
      // separately.
      int ret = base::AndroidHardwareBufferCompat::GetInstance()
                    .RecvHandleFromUnixSocket(file_descriptor.fd, &ahb);
...
int SharedMemoryHandle::GetHandle() const {
  switch (type_) {
    ...
    case Type::ANDROID_HARDWARE_BUFFER:
      ...
      int ret = base::AndroidHardwareBufferCompat::GetInstance()
                    .SendHandleToUnixSocket(memory_object_, write_fd.get());
...
void SharedMemoryHandle::Close() const {
  switch (type_) {
    ...
    case Type::ANDROID_HARDWARE_BUFFER:
      DCHECK(IsValid());
      base::AndroidHardwareBufferCompat::GetInstance().Release(memory_object_);
  }
}
...
SharedMemoryHandle SharedMemoryHandle::Duplicate() const {
  switch (type_) {
    ...
    case Type::ANDROID_HARDWARE_BUFFER:
      DCHECK(IsValid());
      base::AndroidHardwareBufferCompat::GetInstance().Acquire(memory_object_);
