...
DiscardableSharedMemory::LockResult LockPages(const SharedMemory& memory,
  ...
  if (base::win::GetVersion() >= base::win::VERSION_WIN8_1) {
    static const auto reclaim_virtual_memory =
        reinterpret_cast<decltype(&::ReclaimVirtualMemory)>(GetProcAddress(
            GetModuleHandle(L"kernel32.dll"), "ReclaimVirtualMemory"));
    DWORD reclaim_result = (*reclaim_virtual_memory)(
        reinterpret_cast<char*>(memory.memory()) + offset, length);
...
void UnlockPages(const SharedMemory& memory, size_t offset, size_t length) {
  ...
  if (base::win::GetVersion() >= base::win::VERSION_WIN8_1) {
    if (length) {
      static const auto offer_virtual_memory =
          reinterpret_cast<decltype(&::OfferVirtualMemory)>(GetProcAddress(
              GetModuleHandle(L"kernel32.dll"), "OfferVirtualMemory"));
      DWORD offer_result = (*offer_virtual_memory)(
          reinterpret_cast<char*>(memory.memory()) + offset, length,
          VmOfferPriorityNormal);
      DCHECK_EQ(static_cast<DWORD>(ERROR_SUCCESS), offer_result);
    }
  }
#endif
}
