...
class BASE_EXPORT TaskRunner
 ...
 public:
  ...
  //     WeakPtr<>.
  //
  // Note: The destruction sequence of |task| depends on the uniqueness of the
  // ownership of |task|. If |task| is created as a RepeatingCallback and
  // converted to a OnceCallback, the caller may keep a reference to the
  // internal storage of |task|. That causes unpredictable destruction sequence.
  // On an example below, the caller of PostTaskAndReply keeps a copy of |task|
  // after PostTaskAndReply call. So, |obj| is destroyed on the originating
  // sequence if |task| scoped out after it ran on the target sequence.
  // If |task| scoped out before it ran on the target sequence, |obj| is
  // destroyed on the target sequence.
  //
  //   scoped_refptr<Obj> obj = ...;
  //   base::RepeatingClosure task = base::Bind(&Foo, std::move(obj));
  //   base::RepeatingClosure reply = base::Bind(&Bar);
  //
