...
class WakeupHandler : public MessagePumpEpollHandler {
 public:
  WakeupHandler(MessagePumpEpoll *pump) : MessagePumpEpollHandler(pump) {};
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(WakeupHandler);
};

void WakeupHandler::Process() {
  long buf;
...
MessagePumpEpoll::MessagePumpEpoll()
    : state_(NULL), 
      num_fds_(0),
      wakeup_handler_(new WakeupHandler(this)) {
  int fds[2];
