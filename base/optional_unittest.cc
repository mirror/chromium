...
struct OverloadTest {
  OverloadTest() : param_type(ParamType::DEFAULT_CONSTRUCTED) {}
  OverloadTest(const OverloadTest& param)
      : param_type(ParamType::COPY_CONSTRUCTED) {}
  OverloadTest(OverloadTest&& param)
      : param_type(ParamType::MOVE_CONSTRUCTED) {}
  explicit OverloadTest(int param) : param_type(ParamType::INT) {}
  explicit OverloadTest(in_place_t param) : param_type(ParamType::IN_PLACE) {}
  explicit OverloadTest(const Optional<int>& param)
      : param_type(ParamType::OPTIONAL_INT) {}
  explicit OverloadTest(const Optional<OverloadTest>& param)
      : param_type(ParamType::OPTIONAL_TEST) {}
...
TEST(OptionalTest, ForwardConstructor) {
  ...
  // Overload resolution with copy-conversion constructor.
  {
    ...
    Optional<OverloadTest> testee(arg);
    EXPECT_EQ(ParamType::OPTIONAL_INT, testee->param_type);
  }

  // Overload resolution with move conversion constructor.
  {
    Optional<OverloadTest> testee(Optional<int>(in_place, 1));
    EXPECT_EQ(ParamType::OPTIONAL_INT, testee->param_type);
  }

  // Copy constructor should be used.
  {
    const Optional<OverloadTest> arg(in_place, 1);
    Optional<OverloadTest> testee(arg);
    EXPECT_EQ(ParamType::COPY_CONSTRUCTED, testee->param_type);
  }

  // Move constructor should be used.
  {
    Optional<OverloadTest> arg(in_place, 1);
    Optional<OverloadTest> testee(std::move(arg));
    EXPECT_EQ(ParamType::MOVE_CONSTRUCTED, testee->param_type);
  }

  // Default constructor should be used.
  {
    Optional<OverloadTest> testee(in_place);
    EXPECT_EQ(ParamType::DEFAULT_CONSTRUCTED, testee->param_type);
  }

  {
    struct ExplicitTest {
      ExplicitTest(int a) {}  // NOLINT(runtime/explicit)
    };
    // If T is convertible from U, it is not marked as explicit.
    static_assert(std::is_convertible<int, ExplicitTest>::value,
                  "Int should be convertible to ExplicitTest.");
    ([](Optional<ExplicitTest> param) {})(1);
  }
}
