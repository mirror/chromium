...
// construction. Init() must be called statically before its constructor.
...
TEST(LazyInstanceTest, PriorityInversionAtInitializationResolves) {
  ...
  // Spin 64 foreground threads contending to get the already under construction
  // LazyInstance. When they are all running and poking at it : allow the
  // background thread to complete its work.
  constexpr int kNumForegroundThreads = 64;
  std::vector<std::unique_ptr<base::SimpleThread>> foreground_threads;
  base::RepeatingClosure foreground_thread_ready_callback =
      base::BarrierClosure(kNumForegroundThreads, base::BindOnce([]() {
                             BlockingConstructor::CompleteConstructionNow();
                           }));
