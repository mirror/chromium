...
enum class CborMajorType {
  ...
  kSigned = 1,      // Signed integer. Unsupported by this implementation.
...
constexpr uint8_t kDetailsDataMask = 0x1F;  // The last 5 bits encode details.
constexpr uint8_t kDetails1Byte = 24;   // The integer is in the following byte.
constexpr uint8_t kDetails2Bytes = 25;  // The integer is in the next 2 bytes.
constexpr uint8_t kDetails4Bytes = 26;  // The integer is in the next 4 bytes.
constexpr uint8_t kDetails8Bytes = 27;  // The integer is in the next 8 bytes.
...
class BASE_EXPORT CBORWriter {
 ...
 private:
  ...
  void SetType(CborMajorType type);

  // Encodes the details for the data.
  void SetDetails(uint8_t byte_details);
