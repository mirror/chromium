...
TEST(ValueIteratorsTest, DictIteratorOperatorNE) {
  DictStorage storage_0;
  storage_0.emplace("0", MakeUnique<Value>(0));

  DictStorage storage_1;
  storage_1.emplace("0", MakeUnique<Value>(0));

  using iterator = dict_iterator;
  EXPECT_NE(iterator(storage_0.begin()), iterator(storage_0.end()));
  EXPECT_NE(iterator(storage_0.begin()), iterator(storage_1.begin()));
  EXPECT_NE(iterator(storage_0.end()), iterator(storage_1.end()));
  EXPECT_NE(iterator(storage_1.begin()), iterator(storage_1.end()));
}
