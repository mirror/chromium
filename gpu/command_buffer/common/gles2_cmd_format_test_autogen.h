...
TEST_F(GLES2FormatTest, InsertGpuFenceINTERNAL) {
  cmds::InsertGpuFenceINTERNAL& cmd =
      *GetBufferAs<cmds::InsertGpuFenceINTERNAL>();
  void* next_cmd = cmd.Set(&cmd, static_cast<GLuint>(11));
  EXPECT_EQ(static_cast<uint32_t>(cmds::InsertGpuFenceINTERNAL::kCmdId),
            cmd.header.command);
  EXPECT_EQ(sizeof(cmd), cmd.header.size * 4u);
