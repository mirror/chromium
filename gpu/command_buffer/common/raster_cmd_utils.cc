...
// This file is here so other raster related files can have a common set of
// includes where appropriate.

#include "gpu/command_buffer/common/raster_cmd_utils.h"
...
int RasterUtil::GLGetNumValuesReturned(int id) const {
  switch (id) {
...
int RasterUtil::ElementsPerGroup(int format, int type) {
  switch (type) {
    case GL_UNSIGNED_SHORT_5_6_5:
    case GL_UNSIGNED_SHORT_4_4_4_4:
    case GL_UNSIGNED_SHORT_5_5_5_1:
...
uint32_t RasterUtil::ComputeImageGroupSize(int format, int type) {
...
bool RasterUtil::ComputeImageRowSizeHelper(int width,
...
bool RasterUtil::ComputeImagePaddedRowSize(int width,
                                           int format,
                                           int type,
                                           int alignment,
                                           uint32_t* padded_row_size) {
  uint32_t bytes_per_group = ComputeImageGroupSize(format, type);
  return ComputeImageRowSizeHelper(width, bytes_per_group, alignment, nullptr,
                                   padded_row_size, nullptr);
}

// Returns the amount of data glTexImage*D or glTexSubImage*D will access.
bool RasterUtil::ComputeImageDataSizes(int width,
                                       int height,
                                       int depth,
                                       int format,
                                       int type,
  ...
  return ComputeImageDataSizesES3(width, height, depth, format, type, params,
                                  size, opt_unpadded_row_size,
                                  opt_padded_row_size, nullptr, nullptr);
}

bool RasterUtil::ComputeImageDataSizesES3(int width,
                                          int height,
                                          int depth,
                                          int format,
                                          int type,
...
size_t RasterUtil::RenderbufferBytesPerPixel(int format) {
  switch (format) {
...
uint32_t RasterUtil::GetElementSizeForUniformType(int type) {
  switch (type) {
    case GL_FLOAT:
    case GL_FLOAT_VEC2:
    case GL_FLOAT_VEC3:
...
uint32_t RasterUtil::GetElementCountForUniformType(int type) {
  switch (type) {
    case GL_FLOAT:
    case GL_INT:
    case GL_BOOL:
...
size_t RasterUtil::GetGLTypeSizeForTextures(uint32_t type) {
  return static_cast<size_t>(BytesPerElement(type));
}

size_t RasterUtil::GetGLTypeSizeForBuffers(uint32_t type) {
  switch (type) {
    case GL_BYTE:
...
size_t RasterUtil::GetGroupSizeForBufferType(uint32_t count, uint32_t type) {
...
size_t RasterUtil::GetComponentCountForGLTransformType(uint32_t type) {
  switch (type) {
...
size_t RasterUtil::GetCoefficientCountForGLPathFragmentInputGenMode(
    uint32_t gen_mode) {
  switch (gen_mode) {
...
size_t RasterUtil::GetGLTypeSizeForPathCoordType(uint32_t type) {
  switch (type) {
    case GL_BYTE:
      return sizeof(GLbyte);  // NOLINT
    case GL_UNSIGNED_BYTE:
...
size_t RasterUtil::GetGLTypeSizeForGLPathNameType(uint32_t type) {
  switch (type) {
    case GL_BYTE:
      return sizeof(GLbyte);  // NOLINT
    case GL_UNSIGNED_BYTE:
...
uint32_t RasterUtil::GLErrorToErrorBit(uint32_t error) {
  switch (error) {
...
uint32_t RasterUtil::GLErrorBitToGLError(uint32_t error_bit) {
  switch (error_bit) {
...
uint32_t RasterUtil::IndexToGLFaceTarget(int index) {
...
size_t RasterUtil::GLTargetToFaceIndex(uint32_t target) {
  switch (target) {
    case GL_TEXTURE_2D:
...
uint32_t RasterUtil::GLFaceTargetToTextureTarget(uint32_t target) {
  switch (target) {
    case GL_TEXTURE_2D:
    case GL_TEXTURE_EXTERNAL_OES:
    case GL_TEXTURE_RECTANGLE_ARB:
...
uint32_t RasterUtil::GetGLReadPixelsImplementationFormat(
...
uint32_t RasterUtil::GetGLReadPixelsImplementationType(uint32_t internal_format,
                                                       uint32_t texture_type) {
  switch (internal_format) {
    case GL_R16UI:
    case GL_RG16UI:
...
uint32_t RasterUtil::GetChannelsForFormat(int format) {
  switch (format) {
    case GL_ALPHA:
...
bool RasterUtil::IsSizedColorFormat(uint32_t internal_format) {
  switch (internal_format) {
    case GL_ALPHA16F_EXT:
    case GL_ALPHA32F_EXT:
    case GL_RGB8:
...
GLint RasterUtil::GetColorEncodingFromInternalFormat(uint32_t internalformat) {
...
void RasterUtil::GetColorFormatComponentSizes(uint32_t internal_format,
                                              uint32_t type,
                                              int* r,
                                              int* g,
                                              int* b,
...
uint32_t RasterUtil::GetChannelsNeededForAttachmentType(
    int type,
  ...
  switch (type) {
    ...
    default:
      if (type >= GL_COLOR_ATTACHMENT0 &&
          type <
              static_cast<int>(GL_COLOR_ATTACHMENT0 + max_color_attachments)) {
        return kRGBA;
      }
      return 0x0000;
  }
...
std::string RasterUtil::GetStringEnum(uint32_t value) {
...
std::string RasterUtil::GetStringError(uint32_t value) {
  static EnumToString string_table[] = {
      {GL_NONE, "GL_NONE"},
  };
  return RasterUtil::GetQualifiedEnumString(string_table,
                                            arraysize(string_table), value);
}

std::string RasterUtil::GetStringBool(uint32_t value) {
  return value ? "GL_TRUE" : "GL_FALSE";
}

std::string RasterUtil::GetQualifiedEnumString(const EnumToString* table,
                                               size_t count,
                                               uint32_t value) {
...
size_t RasterUtil::CalcClearBufferivDataCount(int buffer) {
  switch (buffer) {
    case GL_COLOR:
      return 4;
    case GL_STENCIL:
...
size_t RasterUtil::CalcClearBufferfvDataCount(int buffer) {
  switch (buffer) {
    case GL_COLOR:
      return 4;
    case GL_DEPTH:
...
// static
void RasterUtil::MapUint64ToTwoUint32(uint64_t v64,
                                      uint32_t* v32_0,
                                      uint32_t* v32_1) {
  DCHECK(v32_0 && v32_1);
...
// static
uint64_t RasterUtil::MapTwoUint32ToUint64(uint32_t v32_0, uint32_t v32_1) {
  uint64_t v64 = v32_1;
  return (v64 << 32) | v32_0;
}

// static
uint32_t RasterUtil::MapBufferTargetToBindingEnum(uint32_t target) {
  switch (target) {
...
// static
bool RasterUtil::IsUnsignedIntegerFormat(uint32_t internal_format) {
  switch (internal_format) {
    case GL_R8UI:
    case GL_R16UI:
    case GL_R32UI:
...
// static
bool RasterUtil::IsSignedIntegerFormat(uint32_t internal_format) {
  switch (internal_format) {
    case GL_R8I:
    case GL_R16I:
    case GL_R32I:
...
// static
bool RasterUtil::IsIntegerFormat(uint32_t internal_format) {
  return (IsUnsignedIntegerFormat(internal_format) ||
          IsSignedIntegerFormat(internal_format));
}

// static
bool RasterUtil::IsFloatFormat(uint32_t internal_format) {
  switch (internal_format) {
    case GL_R16F:
    case GL_R32F:
    case GL_RG16F:
...
// static
uint32_t RasterUtil::ConvertToSizedFormat(uint32_t format, uint32_t type) {
  switch (format) {
    case GL_RGB:
      switch (type) {
        case GL_UNSIGNED_BYTE:
...
#include "gpu/command_buffer/common/raster_cmd_utils_implementation_autogen.h"
