...
struct CreateGpuFenceINTERNAL {
  ...
  void Init(GLuint _gpu_fence_id, bool _from_external) {
    SetHeader();
    gpu_fence_id = _gpu_fence_id;
    from_external = _from_external;
  }

  void* Set(void* cmd, GLuint _gpu_fence_id, bool _from_external) {
    static_cast<ValueType*>(cmd)->Init(_gpu_fence_id, _from_external);
    return NextCmdAddress<ValueType>(cmd);
  }

  gpu::CommandHeader header;
  uint32_t gpu_fence_id;
  uint32_t from_external;
};

static_assert(sizeof(CreateGpuFenceINTERNAL) == 12,
              "size of CreateGpuFenceINTERNAL should be 12");
...
static_assert(offsetof(CreateGpuFenceINTERNAL, from_external) == 8,
              "offset of CreateGpuFenceINTERNAL from_external should be 8");
