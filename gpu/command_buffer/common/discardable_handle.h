...
class GPU_EXPORT DiscardableHandleBase {
 public:
  ...
  static DiscardableHandleId IdFromShmAndOffset(uint32_t shm_id, uint32_t byte_offset) {
    return (static_cast<uint64_t>(shm_id) << 32) | static_cast<uint64_t>(byte_offset);
  }
  
  static void ShmAndOffsetFromId(DiscardableHandleId handle_id, uint32_t* shm_id, uint32_t* offset) {
