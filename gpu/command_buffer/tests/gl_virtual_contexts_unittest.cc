...
GpuDriverBugWorkarounds NoWorkarounds() {
  GpuDriverBugWorkarounds workarounds;
  return workarounds;
}
GpuDriverBugWorkarounds UseClientSideArraysForStreamBuffers() {
  GpuDriverBugWorkarounds workarounds;
...
INSTANTIATE_TEST_CASE_P(WithWorkarounds,
                        ...
                        ::testing::Values(
// TODO(kainino): This #if is added because this case does not pass on Mac or
// Linux. My guess is that this workaround requires the backing context to be
// OpenGL ES (not OpenGL Core Profile). https://crbug.com/768324
#if defined(OS_ANDROID)
                            UseClientSideArraysForStreamBuffers(),
#endif
                            NoWorkarounds()));
