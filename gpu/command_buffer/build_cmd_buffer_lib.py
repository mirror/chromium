...
class Int64Argument(Argument):
  ...
  def WriteArgAccessor(self, f):
    ...
    f.write("        %sUtil::MapTwoUint32ToUint64(\n" % _prefix)
    f.write("            %s_0,\n" % self.name)
    f.write("            %s_1));\n" % self.name)
    f.write("  }\n")
    f.write("\n")
  ...
  def WriteSetCode(self, f, indent, var):
    indent_str = ' ' * indent
    f.write("%s%sUtil::MapUint64ToTwoUint32(static_cast<uint64_t>(%s),\n" %
            (indent_str, _prefix, var))
    f.write("%s                                &%s_0,\n" %
...
def InitializePrefix(prefix):
  ...
  _upper_prefix = prefix.upper()

  global _lower_prefix
  _lower_prefix = prefix.lower()
