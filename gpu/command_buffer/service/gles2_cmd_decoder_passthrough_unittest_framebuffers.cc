...
TEST_F(GLES2DecoderPassthroughTest, ReadPixelsOutOfRange) {
  ...
  for (auto test : tests) {
    ...
    // Check each pixel and expect them to be non-zero if they were written
    for (GLint dx = 0; dx < test.w; ++dx) {
      GLint x = test.x + dx;
      for (GLint dy = 0; dy < test.h; ++dy) {
        GLint y = test.y + dy;

        bool expectWritten = 0 <= x && x < kWidth && 0 <= y && y < kHeight;
        for (GLint component = 0; component < 4; ++component) {
          uint8_t value = dest[4 * (dy * test.w + dx) + component];
          EXPECT_EQ(expectWritten, value != 0) << x << " " << y << " " << value;
        }
      }
    }
  }
