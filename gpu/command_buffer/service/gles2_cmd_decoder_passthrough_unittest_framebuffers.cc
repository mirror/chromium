...
TEST_F(GLES2DecoderPassthroughTest, ReadPixelsAsync) {
...

  {
    ...
    // A fence NV object should be used in ES2 contexts
    EXPECT_NE(0u, pending_read_pixels.fence_nv_service_id);
    EXPECT_EQ(nullptr, pending_read_pixels.sync_service_id);
  }
...
TEST_F(GLES3DecoderPassthroughTest, ReadPixelsAsyncSyncObjectUsed) {
  typedef ReadPixels::Result Result;
  Result* result = GetSharedMemoryAs<Result*>();
  const GLsizei kWidth = 4;
  const GLsizei kHeight = 4;
...

  {
    ...
    // A GLSync object should be used in ES3 contexts
    EXPECT_NE(nullptr, pending_read_pixels.sync_service_id);
    EXPECT_EQ(0u, pending_read_pixels.fence_nv_service_id);
  }
}
