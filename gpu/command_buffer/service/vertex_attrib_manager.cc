...
bool VertexAttribManager::ValidateBindings(
  ...
  for (VertexAttribList::iterator it = enabled_vertex_attribs_.begin();
    ...
    // All attribs in enabled_vertex_attribs_ are enabled by the client.
    // Make sure that all of those enabled_vertex_attribs_ are really
    // enabled in the driver, if AND ONLY IF they are consumed by the current
    // shader program.
