...
class Scheduler::Sequence {
 ...
 private:
  ...
  int waiting_priority_counts_[(int)SchedulingPriority::kLast + 1] = {};
...
Scheduler::Sequence::~Sequence() {
  for (auto& fence : wait_fences_) {
    SequenceId release_id = scheduler_->GetSequenceId(fence.sync_token);
    Sequence* release_sequence = scheduler_->GetSequence(release_id);
    if (!release_sequence)
      continue;
...
void Scheduler::Sequence::UpdateSchedulingPriority() {
  ...
  for (int release_priority = 0; release_priority < (int)priority;
       release_priority++) {
    if (waiting_priority_counts_[release_priority] != 0) {
      priority = (SchedulingPriority)release_priority;
      break;
    }
  }
  ...
  if (current_priority_ != priority) {
    ...
    for (auto& wait_fence : wait_fences_) {
      SequenceId release_id = scheduler_->GetSequenceId(wait_fence.sync_token);
      Sequence* release_sequence = scheduler_->GetSequence(release_id);
      if (release_sequence) {
...
void Scheduler::Sequence::AddWaitingPriority(SchedulingPriority priority) {
  ...
  waiting_priority_counts_[(int)priority]++;
...
void Scheduler::Sequence::RemoveWaitingPriority(SchedulingPriority priority) {
  ...
  DCHECK(waiting_priority_counts_[(int)priority] > 0);
  waiting_priority_counts_[(int)priority]--;

  if (priority == current_priority_ &&
      waiting_priority_counts_[(int)priority] == 0)
    UpdateSchedulingPriority();
}
...
void Scheduler::Sequence::ChangeWaitingPriority(
    ...
    SchedulingPriority new_priority) {
  DCHECK(waiting_priority_counts_[(int)old_priority] != 0);
  waiting_priority_counts_[(int)old_priority]--;
  waiting_priority_counts_[(int)new_priority]++;

  if (new_priority < current_priority_ ||
      (old_priority == current_priority_ &&
       waiting_priority_counts_[(int)old_priority] == 0)) {
    UpdateSchedulingPriority();
  }
}
...
SequenceId Scheduler::GetSequenceId(const SyncToken& sync_token) {
  return sync_point_manager_->GetSyncTokenReleaseSequenceId(sync_token);
}
...
void Scheduler::ScheduleTaskHelper(Task task) {
  ...
  for (const SyncToken& sync_token : task.sync_token_fences) {
    ...
    if (sync_point_manager_->Wait(
      ...
      sequence->AddWaitFence(sync_token, order_num, release_sequence);
...
void Scheduler::SyncTokenFenceReleased(const SyncToken& sync_token,
  ...
  if (sequence) {
    sequence->RemoveWaitFence(sync_token, order_num, release_sequence);
    TryScheduleSequence(sequence);
  }
