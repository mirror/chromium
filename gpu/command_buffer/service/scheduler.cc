...
Scheduler::Sequence::~Sequence() {
  for (auto& fence : wait_fences_) {
    scheduler_->debug_traces_.push_back(
      DebugTrace(DebugType::Destroyed,
...
void Scheduler::Sequence::UpdateSchedulingPriority() {
  ...
  if (current_priority_ != priority) {
    ...
    scheduler_->debug_traces_.push_back(
      DebugTrace(DebugType::PriorityChanged,
                 sequence_id().GetUnsafeValue(),
...
void Scheduler::Sequence::AddWaitFence(const SyncToken& sync_token,
                                       ...
                                       SequenceId release_sequence_id,
                                       Sequence* release_sequence) {
  //LOG(WARNING) << "AddWaitFence(" << sequence_id()
  //             << ") " << release_sequence_id
  //             << " " << sync_token.release_count();

  scheduler_->debug_traces_.push_back(
      DebugTrace(DebugType::AddWaitFence,
                 sequence_id().GetUnsafeValue(),
                 release_sequence_id.GetUnsafeValue(),
                 sync_token.release_count()));
...
void Scheduler::Sequence::RemoveWaitFence(const SyncToken& sync_token,
                                          uint32_t order_num,
                                          SequenceId release_sequence_id) {
  //LOG(WARNING) << "RemoveWaitFence(" << sequence_id()
  //             << ") " << release_sequence_id
  //             << " " << sync_token.release_count();

  scheduler_->debug_traces_.push_back(
      DebugTrace(DebugType::RemoveWaitFence,
                 sequence_id().GetUnsafeValue(),
                 release_sequence_id.GetUnsafeValue(),
                 sync_token.release_count()));
...
void Scheduler::Sequence::AddWaitingPriority(SchedulingPriority priority) {
  //LOG(WARNING) << "AddWaitingPriority(" << sequence_id()
  //             << ") " << static_cast<int>(priority);

  scheduler_->debug_traces_.push_back(
      DebugTrace(DebugType::AddWaitingPriority,
                 sequence_id().GetUnsafeValue(),
                 static_cast<int>(priority)));

  TRACE_EVENT2("gpu", "Scheduler::Sequence::AddWaitingPriority",
               "sequence_id", sequence_id_.GetUnsafeValue(), "new_priority",
               SchedulingPriorityToString(priority));
...
void Scheduler::Sequence::RemoveWaitingPriority(SchedulingPriority priority) {
  //LOG(WARNING) << "RemoveWaitingPriority(" << sequence_id()
  //             << ") " << static_cast<int>(priority);
  ...
  if (waiting_priority_counts_[static_cast<int>(priority)] == 0) {
    LOG(WARNING) << "*** Invalid priority count ***";
...
void Scheduler::Sequence::ChangeWaitingPriority(
  ...
  //LOG(WARNING) << "ChangeWaitingPriority(" << sequence_id()
  //             << ") " << static_cast<int>(old_priority)
  //             << " " << static_cast<int>(new_priority);

  scheduler_->debug_traces_.push_back(
      DebugTrace(DebugType::ChangeWaitingPriority,
                 sequence_id().GetUnsafeValue(),
                 static_cast<int>(old_priority),
                 static_cast<int>(new_priority)));
...
void Scheduler::Sequence::AddClientWait(CommandBufferId command_buffer_id) {
  //LOG(WARNING) << "AddClientWait(" << sequence_id() << ")";
  scheduler_->debug_traces_.push_back(
      DebugTrace(DebugType::AddClientWait,
                 sequence_id().GetUnsafeValue()));
...
void Scheduler::Sequence::RemoveClientWait(CommandBufferId command_buffer_id) {
  //LOG(WARNING) << "RemoveClientWait(" << sequence_id() << ")";
  scheduler_->debug_traces_.push_back(
      DebugTrace(DebugType::RemoveClientWait,
                 sequence_id().GetUnsafeValue()));
