 ...
 private:
  ...
  struct Fence {
    ...
    bool operator==(const Fence& other) const {
      return std::tie(sync_token, order_num, release_id) ==
             std::tie(other.sync_token, other.order_num, release_id);
    }
  };
...
Scheduler::Sequence::~Sequence() {
  for (auto& fence : wait_fences_) {
    Sequence* release_sequence = scheduler_->GetSequence(fence.release_id);
    if (!release_sequence)
      continue;
...
void Scheduler::Sequence::UpdateSchedulingPriority() {
  ...
  if (current_priority_ != priority) {
    ...
    for (auto& wait_fence : wait_fences_) {
      Sequence* release_sequence =
          scheduler_->GetSequence(wait_fence.release_id);
      if (release_sequence) {
...
void Scheduler::Sequence::AddWaitFence(const SyncToken& sync_token,
  ...
  wait_fences_.push_back({sync_token, order_num, release_id});
...
void Scheduler::Sequence::RemoveWaitFence(const SyncToken& sync_token,
                                          uint32_t order_num,
                                          SequenceId release_id) {
  base::Erase(wait_fences_, Fence{sync_token, order_num, release_id});
...
void Scheduler::ScheduleTaskHelper(Task task) {
  ...
  for (const SyncToken& sync_token : task.sync_token_fences) {
    ...
    if (sync_point_manager_->Wait(
      ...
      sequence->AddWaitFence(sync_token, order_num, release_id,
                             release_sequence);
    }
  }
