...
Scheduler::Sequence::~Sequence() {
  for (auto& wait_fence : wait_fences_) {
    Sequence* release_sequence =
        scheduler_->GetSequence(wait_fence.first.release_sequence_id);
    if (release_sequence)
      release_sequence->RemoveWaitingPriority(wait_fence.second);
  }
...
void Scheduler::Sequence::PropagatePriority(SchedulingPriority priority) {
  for (auto& wait_fence : wait_fences_) {
    if (wait_fence.second > priority) {
      SchedulingPriority old_priority = wait_fence.second;
      wait_fence.second = priority;
