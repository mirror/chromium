...
// Does not make any other changes to memory visible.
class PreemptionFlag : public base::RefCountedThreadSafe<PreemptionFlag> {
 public:
  bool IsSet() const { return flag_.load(std::memory_order_relaxed) != 0; }
  void Set() { flag_.store(1, std::memory_order_relaxed); }
