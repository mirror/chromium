...
TEST_F(ServiceTransferCacheTest, BasicUsage) {
  ...
  transfer_cache_.CreateLockedEntry(id, handle, type, data.data(), data_size);
  auto* entry =
      transfer_cache_.GetEntry<cc::ServiceRawMemoryTransferCacheEntry>(id);
  EXPECT_TRUE(entry);
  ...
  entry = transfer_cache_.GetEntry<cc::ServiceRawMemoryTransferCacheEntry>(id);
  EXPECT_FALSE(entry);
}
