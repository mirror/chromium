...
TEST_F(ServiceTransferCacheTest, BasicUsage) {
  cc::TransferCacheEntryId id = cc::TransferCacheEntryId::FromUnsafeValue(0);
  size_t data_size = 100;
  cc::TransferCacheEntryType type = cc::TransferCacheEntryType::kRawMemory;
  auto handle = CreateLockedServiceHandleForTesting();
  std::vector<uint8_t> data(data_size);

  context_group_->transfer_cache()->CreateLockedEntry(id, handle, type,
                                                      data.data(), data_size);
  auto* entry = context_group_->transfer_cache()->GetEntry(id);
  EXPECT_TRUE(entry);
  EXPECT_EQ(entry->Size(), data_size);

  context_group_->transfer_cache()->UnlockEntry(id);
  EXPECT_FALSE(handle.IsLockedForTesting());

  context_group_->transfer_cache()->DeleteEntry(id);
  EXPECT_TRUE(handle.IsDeletedForTesting());
  entry = context_group_->transfer_cache()->GetEntry(id);
  EXPECT_FALSE(entry);
}
