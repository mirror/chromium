...
class GPU_EXPORT GpuFenceManager {
  class GPU_EXPORT GpuFenceEntry : public base::RefCounted<GpuFenceEntry> {
   public:
    GpuFenceEntry();
   ...
   private:
    ...
    friend class base::RefCounted<GpuFenceEntry>;
    ~GpuFenceEntry();
 ...
 private:
  // Semantically, unique_ptr would be sufficient here, but that doesn't
  // compile on Windows. flat_map is implemented as a std::vector of std::pair,
  // but the MSVC std::pair moves via copy constructor of its elements which
  // breaks swap and resize.
  using GpuFenceEntryMap =
      base::flat_map<uint32_t, scoped_refptr<GpuFenceEntry>>;
