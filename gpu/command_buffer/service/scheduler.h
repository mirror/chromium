...
class GPU_EXPORT Scheduler {
 public:
  ...
  struct DebugTrace {
    DebugTrace(DebugType type,
               int stream_id,
               int wait_counts[3],
               int p0 = 0,
               int p1 = 0)
        : type(type), stream_id(stream_id), p0(p0), p1(p1) {
      this->wait_counts[0] = wait_counts[0];
      this->wait_counts[1] = wait_counts[1];
      this->wait_counts[2] = wait_counts[2];
    }
