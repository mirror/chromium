  ...
  if (!dest_level_defined || dest_width != source_width ||
    ...
    if (error != GL_NO_ERROR) {
      ...
      LOG(ERROR) << "dest_level_defined: " << dest_level_defined <<
          "dest_width: " << dest_width << " source_width: " << source_width <<
          "dest_height: " << dest_height << " source_height: " << source_height <<
          "dest_internal_format: " << dest_internal_format << " internal_format: " << internal_format <<
          "dest_type_previous: " << dest_type_previous << " dest_type: " << dest_type;

      gl::GLImage* image =
          dest_texture->GetLevelImage(dest_target, dest_level);
      if (image)
        LOG(ERROR) << "dest image type: " << static_cast<uint32_t>(image->GetType());
      else
