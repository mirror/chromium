...
bool GLES2DecoderImpl::Initialize(
  ...
  if (features().chromium_raster_transport &&
      attrib_helper.enable_oop_rasterization) {
...
error::Error GLES2DecoderImpl::DoBeginRasterCHROMIUM(
    GLuint texture_id,
    GLuint sk_color,
    GLuint msaa_sample_count,
    GLboolean can_use_lcd_text,
  ...
  if (sk_surface_) {
    LOG(ERROR) << "BeginRasterCHROMIUM without EndRasterCHROMIUM";
    return error::kInvalidArguments;
  }
  ...
  if (!texture_ref) {
    LOG(ERROR) << "BeginRasterCHROMIUM, unknown texture:" << texture_id;
    return error::kInvalidArguments;
  }
  ...
  if (!texture->GetLevelSize(texture->target(), 0, &width, &height, &depth)) {
    LOG(ERROR) << "BeginRasterCHROMIUM, unknown texture info:" << texture_id;
    return error::kInvalidArguments;
  }
...
error::Error GLES2DecoderImpl::HandleRasterCHROMIUM(
  ...
  char data[sizeof(cc::LargestPaintOp)];
...
error::Error GLES2DecoderImpl::DoEndRasterCHROMIUM() {
  if (!features().chromium_raster_transport)
    return error::kUnknownCommand;

  if (!sk_surface_) {
    LOG(ERROR) << "EndRasterCHROMIUM without BeginRasterCHROMIUM";
    return error::kInvalidArguments;
  }
