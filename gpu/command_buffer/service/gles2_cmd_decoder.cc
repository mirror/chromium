...
void GLES2DecoderImpl::FinishSwapBuffers(gfx::SwapResult result) {
  ...
  ++swaps_since_resize_;
  if (swaps_since_resize_ <= 2 && surface_->BuffersFlipped()) {
    // After a resize each buffer is new and needs to be cleared to known
    // values. <= 2 means that we will handle double and triple buffering.
    OutputDebugStringA("Clearing color buffer.\n");
    backbuffer_needs_clear_bits_ |= GL_COLOR_BUFFER_BIT;
  }
}
