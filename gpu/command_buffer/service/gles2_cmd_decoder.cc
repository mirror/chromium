...
void GLES2DecoderImpl::DoBindTransformFeedback(
  ...
  // Just in case the driver thinks that switching the TF object should switch
  // the GL_TRANSFORM_FEEDBACK_BUFFER binding (it shouldn't), set it too.
  api()->glBindBufferFn(
      GL_TRANSFORM_FEEDBACK_BUFFER,
      state_.bound_transform_feedback_buffer
          ? state_.bound_transform_feedback_buffer->service_id()
          : 0);
}
