...
TEST_F(GLES2DecoderManualInitTest, CompressedTexImage2DS3TCWebGL) {
  InitState init;
...
TEST_F(GLES2DecoderManualInitTest, EGLImageExternalGetBinding) {
  ...
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GetDecoder()->GetGLES2Util()->GLGetNumValuesReturned(
...
TEST_F(GLES2DecoderManualInitTest, ARBTextureRectangleTexImage2D) {
  InitState init;
  init.extensions = "GL_ARB_texture_rectangle";
  init.bind_generates_resource = true;
  InitDecoder(init);
...
TEST_F(GLES2DecoderManualInitTest, ARBTextureRectangleTexImage2DInvalid) {
  InitState init;
  init.extensions = "GL_ARB_texture_rectangle";
  init.bind_generates_resource = true;
  InitDecoder(init);
...
TEST_F(GLES2DecoderPassthroughTest, CopyTexImage2DMarksTextureAsCleared) {
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
...
TEST_F(GLES2DecoderPassthroughTest, CopyTexSubImage2DTwiceMarksTextureAsCleared)
{ DoBindTexture(GL_TEXTURE_2D, kClientTextureId); DoTexImage2D( GL_TEXTURE_2D,
0, GL_RGBA, 2, 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0);
...
TEST_F(GLES2DecoderPassthroughTest,
CopyTexSubImage2DTwiceClearsUnclearedTexture) { DoBindTexture(GL_TEXTURE_2D,
kClientTextureId); DoTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 2, 2, 0, GL_RGBA,
GL_UNSIGNED_BYTE, 0, 0);
...
DoBindTexture(GL_TEXTURE_2D, kClientTextureId); DoTexImage2D(GL_TEXTURE_2D, 0,
GL_RGBA, kBackBufferWidth, kBackBufferHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0,
0);
...
TEST_F(GLES2DecoderPassthroughTest, TextureUsageAngleExtNotEnabledByDefault) {
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
