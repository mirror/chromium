...
TEST_F(GLES2DecoderPassthroughTest, GenerateMipmapWrongFormatsFails) {
...
TEST_F(GLES2DecoderPassthroughTest, ActiveTextureValidArgs) {
  ActiveTexture cmd;
...
TEST_F(GLES2DecoderPassthroughTest, ActiveTextureInvalidArgs) {
  ActiveTexture cmd;
  ...
  DoGetIntegerv(GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS, &num_texture_units, 1);
  EXPECT_GE(num_texture_units, 2);

  cmd.Init(num_texture_units);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
}

TEST_F(GLES2DecoderPassthroughTest, TexSubImage2DValidArgs) {
  const int kWidth = 16;
  const int kHeight = 8;
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
...
TEST_F(GLES2DecoderPassthroughTest, TexSubImage2DBadArgs) {
  const int kWidth = 8;
  const int kHeight = 4;
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
  DoTexImage2D(GL_TEXTURE_2D, 1, GL_RGBA, kWidth, kHeight, 0, GL_RGBA,
...
TEST_F(GLES3DecoderPassthroughTest, TexSubImage3DValidArgs) {
  const int kWidth = 8;
  const int kHeight = 4;
  const int kDepth = 2;
  DoBindTexture(GL_TEXTURE_3D, kClientTextureId);
...
TEST_F(GLES3DecoderPassthroughTest, TexSubImage3DBadArgs) {
  const int kWidth = 4;
  const int kHeight = 4;
  const int kDepth = 2;
  DoBindTexture(GL_TEXTURE_3D, kClientTextureId);
...
TEST_F(GLES3DecoderPassthroughTest, TexSubImage2DTypesDoNotMatchUnsizedFormat) {
  const int kWidth = 16;
  const int kHeight = 8;
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
  DoTexImage2D(GL_TEXTURE_2D, 1, GL_RGBA, kWidth, kHeight, 0, GL_RGBA,
...
TEST_F(GLES3DecoderPassthroughTest, TexSubImage2DTypesDoNotMatchSizedFormat) {
  const int kWidth = 16;
  const int kHeight = 8;
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
...
TEST_F(GLES2DecoderPassthroughTest, CopyTexSubImage2DValidArgs) {
  const int kWidth = 16;
  const int kHeight = 8;
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
  DoTexImage2D(GL_TEXTURE_2D, 1, GL_RGBA, kWidth, kHeight, 0, GL_RGBA,
...
TEST_F(GLES2DecoderPassthroughTest, CopyTexSubImage2DBadArgs) {
  const int kWidth = 16;
  const int kHeight = 8;
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
  DoTexImage2D(GL_TEXTURE_2D, 1, GL_RGBA, kWidth, kHeight, 0, GL_RGBA,
...
TEST_F(GLES2DecoderPassthroughTest, TexImage2DRedefinitionSucceeds) {
  const int kWidth = 16;
  const int kHeight = 8;
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
...
TEST_F(GLES2DecoderPassthroughTest, CopyTexImage2DUnsizedInternalFormat) {
...
TEST_F(GLES3DecoderPassthroughTest, CopyTexImage2DUnsizedInternalFormatES3) {
...
TEST_F(GLES3DecoderPassthroughTest, CompressedTexImage3DBucket) {
  ...
  CommonDecoder::Bucket* bucket = GetDecoder()->CreateBucket(kBucketId);
  ASSERT_TRUE(bucket != NULL);
  const GLsizei kImageSize = 32;
  bucket->SetSize(kImageSize);

  DoBindTexture(kTarget, kClientTextureId);
...
TEST_F(GLES3DecoderPassthroughTest,
       CompressedTexImage3DBucketBucketSizeIsZero) {
  const uint32_t kBucketId = 123;
  const uint32_t kBadBucketId = 99;
  const GLenum kTarget = GL_TEXTURE_2D_ARRAY;
  const GLint kLevel = 0;
...
TEST_F(GLES2DecoderPassthroughTest, CompressedTexImage3DFailsOnES2) {
  const uint32_t kBucketId = 123;
  const GLenum kTarget = GL_TEXTURE_2D_ARRAY;
  const GLint kLevel = 0;
  const GLenum kInternalFormat = GL_COMPRESSED_R11_EAC;
...

  {
    ...
    cmd.Init(kTarget, kLevel, 0, 0, 0, kWidth, kHeight, kDepth, kInternalFormat,
             kBucketId);
    EXPECT_EQ(error::kUnknownCommand, ExecuteCmd(cmd));
  }
}

TEST_F(GLES2DecoderPassthroughTest, CopyTexSubImage3DFailsOnES2) {
  const GLenum kTarget = GL_TEXTURE_2D_ARRAY;
  const GLint kLevel = 0;
  const GLsizei kWidth = 4;
  const GLsizei kHeight = 4;
...
TEST_F(GLES3DecoderPassthroughTest, CopyTexSubImage3DFailures) {
  ...
  // No texture bound
  cmd.Init(kTarget, kLevel, kXoffset, kYoffset, kZoffset, kX, kY, kWidth,
           kHeight);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
}

TEST_F(GLES3DecoderPassthroughTest, CopyTexSubImage3DCheckArgs) {
  const GLenum kTarget = GL_TEXTURE_3D;
  const GLint kLevel = 1;
  ...
  // Bad target
  cmd.Init(GL_TEXTURE_2D, kLevel, kXoffset, kYoffset, kZoffset, kX, kY, kWidth,
           kHeight);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
  ...
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
  cmd.Init(kTarget, kLevel, kXoffset, kYoffset, kDepth, kX, kY, kWidth,
           kHeight);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_VALUE, GetGLError());

  // Bad width / height
  cmd.Init(kTarget, kLevel, kXoffset, kYoffset, kZoffset, kX, kY, kWidth + 1,
           kHeight);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
  cmd.Init(kTarget, kLevel, kXoffset, kYoffset, kZoffset, kX, kY, kWidth,
...
TEST_F(GLES3DecoderPassthroughTest, CopyTexSubImage3DFeedbackLoopSucceeds0) {
  const GLenum kTarget = GL_TEXTURE_3D;
  const GLint kInternalFormat = GL_RGB8;
  const GLint kXoffset = 0;
  const GLint kYoffset = 0;
  ...
  DoTexImage3D(kTarget, kLevel, kInternalFormat, kWidth, kHeight, kDepth,
               kBorder, kFormat, kType, shared_memory_id_, kSharedMemoryOffset);
  tex_layer.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, kClientTextureId, kLevel,
                 kLayer);
...
TEST_F(GLES3DecoderPassthroughTest, CopyTexSubImage3DFeedbackLoopSucceeds1) {
  const GLenum kTarget = GL_TEXTURE_3D;
  const GLint kInternalFormat = GL_RGB8;
  const GLint kXoffset = 0;
  const GLint kYoffset = 0;
...
TEST_F(GLES3DecoderPassthroughTest, CopyTexSubImage3DFeedbackLoopFails) {
  const GLenum kTarget = GL_TEXTURE_3D;
  const GLint kInternalFormat = GL_RGB8;
  const GLint kXoffset = 0;
  const GLint kYoffset = 0;
...
TEST_F(GLES3DecoderPassthroughTest, CompressedTexImage3DFailsWithBadImageSize) {
  const uint32_t kBucketId = 123;
  const GLenum kTarget = GL_TEXTURE_2D_ARRAY;
  const GLint kLevel = 0;
...
TEST_F(GLES3DecoderPassthroughTest, CompressedTexSubImage3DFails) {
  const uint32_t kBucketId = 123;
  const GLenum kTarget = GL_TEXTURE_2D_ARRAY;
  const GLint kLevel = 0;
  const GLenum kInternalFormat = GL_COMPRESSED_RGBA8_ETC2_EAC;
  ...
  cmd.Init(kTarget, kLevel, kXOffset, kYOffset, kZOffset, kSubWidth, kSubHeight,
           kSubDepth, kBadFormat, kBucketId);
  bucket->SetSize(kSubImageSize);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_VALUE, GetGLError());

  // Negative offset.
  cmd.Init(kTarget, kLevel, kXOffset, -4, kZOffset, kSubWidth, kSubHeight,
           kSubDepth, kFormat, kBucketId);
  bucket->SetSize(kSubImageSize);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
  ...
  cmd.Init(kTarget, kLevel, kXOffset, kYOffset + 8, kZOffset, kSubWidth,
           kSubHeight, kSubDepth, kFormat, kBucketId);
  bucket->SetSize(kSubImageSize);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_VALUE, GetGLError());

  // offset not a multiple of 4.
  cmd.Init(kTarget, kLevel, kXOffset, kYOffset + 1, kZOffset, kSubWidth,
           kSubHeight, kSubDepth, kFormat, kBucketId);
  bucket->SetSize(kSubImageSize);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
  ...
  cmd.Init(kTarget, kLevel, kXOffset, kYOffset, kZOffset, kSubWidth,
           kSubHeight + 3, kSubDepth, kFormat, kBucketId);
  ...
  cmd.Init(kTarget, kLevel, kXOffset, kYOffset, kZOffset, kSubWidth, kSubHeight,
           kSubDepth, kFormat, kBadBucketId);
  bucket->SetSize(kSubImageSize);
  EXPECT_NE(error::kNoError, ExecuteCmd(cmd));

  // Bad target
  cmd.Init(GL_RGBA, kLevel, kXOffset, kYOffset, kZOffset, kSubWidth, kSubHeight,
           kSubDepth, kFormat, kBucketId);
  bucket->SetSize(kSubImageSize);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
  ...
  // Bad format
  cmd.Init(kTarget, kLevel, kXOffset, kYOffset, kZOffset, kSubWidth, kSubHeight,
           kSubDepth, GL_ONE, kBucketId);
  bucket->SetSize(kSubImageSize);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
}

TEST_F(GLES3DecoderPassthroughTest, CompressedTexSubImage3DBadSHM) {
  const uint32_t kBucketId = 123;
  const GLenum kTarget = GL_TEXTURE_2D_ARRAY;
  const GLint kLevel = 0;
  const GLenum kInternalFormat = GL_COMPRESSED_RGBA8_ETC2_EAC;
  ...
  // Valid args + non-empty size + NULL SHM -> GL error
  cmd.Init(kTarget, kLevel, kXOffset, kYOffset, kZOffset, kSubWidth, kSubHeight,
           kSubDepth, kFormat, kSubImageSize, 0, 0);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_VALUE, GetGLError());

  // Valid args + non-empty size + invalid SHM -> GL error
  cmd.Init(kTarget, kLevel, kXOffset, kYOffset, kZOffset, kSubWidth, kSubHeight,
...
TEST_F(GLES3DecoderPassthroughTest,
       CompressedTexImage2DBucketBucketSizeIsZero) {
  const uint32_t kBucketId = 123;
  const uint32_t kBadBucketId = 99;
...
TEST_F(GLES3DecoderPassthroughTest, CompressedTexImage2DBucketBadBucket) {
  const uint32_t kBadBucketId = 123;
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
  CompressedTexImage2DBucket cmd;
  cmd.Init(GL_TEXTURE_2D, 0, GL_COMPRESSED_R11_EAC, 4, 4, kBadBucketId);
  EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
  CompressedTexSubImage2DBucket cmd2;
  cmd2.Init(GL_TEXTURE_2D, 0, 0, 0, 4, 4, GL_COMPRESSED_R11_EAC, kBadBucketId);
  EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_F(GLES2WebGLDecoderPassthroughTest, CompressedTexImage2DS3TCWebGL) {
  DoRequestExtension("GL_EXT_texture_compression_dxt1");
  DoRequestExtension("GL_ANGLE_texture_compression_dxt3");
  DoRequestExtension("GL_ANGLE_texture_compression_dxt5");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
  ...
  for (size_t ii = 0; ii < arraysize(test_data); ++ii) {
    ...
    CompressedTexImage2DBucket cmd;
    // test small width.
    DoCompressedTexImage2D(GL_TEXTURE_2D, 1, test.format, 2, 4, 0,
                           test.block_size, kBucketId);
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    ...
    // test small height.
    DoCompressedTexImage2D(GL_TEXTURE_2D, 1, test.format, 4, 2, 0,
                           test.block_size, kBucketId);
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    ...
    DoCompressedTexImage2D(GL_TEXTURE_2D, 1, test.format, 1, 1, 0,
                           test.block_size, kBucketId);
    EXPECT_EQ(GL_NO_ERROR, GetGLError());

    // test small for level 0.
    DoCompressedTexImage2D(GL_TEXTURE_2D, 1, test.format, 2, 2, 0,
                           test.block_size, kBucketId);
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    ...
    // test with 3 mips.
    DoCompressedTexImage2D(GL_TEXTURE_2D, 0, test.format, 4, 4, 0,
                           test.block_size, kBucketId);
    DoCompressedTexImage2D(GL_TEXTURE_2D, 1, test.format, 2, 2, 0,
                           test.block_size, kBucketId);
    DoCompressedTexImage2D(GL_TEXTURE_2D, 2, test.format, 1, 1, 0,
                           test.block_size, kBucketId);
    EXPECT_EQ(GL_NO_ERROR, GetGLError());

    // Test a 16x16
    DoCompressedTexImage2D(GL_TEXTURE_2D, 0, test.format, 16, 16, 0,
                           test.block_size * 4 * 4, kBucketId);
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_F(GLES2WebGLDecoderPassthroughTest, CompressedTexImage2DETC1) {
  DoRequestExtension("GL_OES_compressed_ETC1_RGB8_texture");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
  ...
  // Test a 16x16
  DoCompressedTexImage2D(GL_TEXTURE_2D, 0, kFormat, 16, 16, 0, kBlockSize * 16,
                         kBucketId);
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_F(GLES2DecoderPassthroughTest, CopyTextureCHROMIUMBadTarget) {
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
  DoTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 16, 17, 0, GL_RGBA, GL_UNSIGNED_BYTE,
               0, 0);
  ...
  cmd.Init(kClientTextureId, 0, kBadTarget, kNewClientId, 0, GL_RGBA,
           GL_UNSIGNED_BYTE, GL_FALSE, GL_FALSE, GL_FALSE);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
}

TEST_F(GLES2DecoderPassthroughTest, CopySubTextureCHROMIUMBadTarget) {
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
  DoTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 16, 17, 0, GL_RGBA, GL_UNSIGNED_BYTE,
               0, 0);
  ...
  cmd.Init(kClientTextureId, 0, kBadTarget, kNewClientId, 0, 1, 1, 2, 2, 3, 3,
           GL_FALSE, GL_FALSE, GL_FALSE);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
}

TEST_F(GLES2WebGLDecoderPassthroughTest, EGLImageExternalBindTexture) {
  DoRequestExtension("GL_OES_EGL_image_external");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_F(GLES2WebGLDecoderPassthroughTest, EGLImageExternalGetBinding) {
  DoRequestExtension("GL_OES_EGL_image_external");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());

  DoBindTexture(GL_TEXTURE_EXTERNAL_OES, kClientTextureId);

  GLint binding = 0;
  DoGetIntegerv(GL_TEXTURE_BINDING_EXTERNAL_OES, &binding, 1);
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
  EXPECT_EQ(kClientTextureId, static_cast<GLuint>(binding));
}

TEST_F(GLES2WebGLDecoderPassthroughTest, EGLImageExternalTextureDefaults) {
  DoRequestExtension("GL_OES_EGL_image_external");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
  ...
  GLint min_filter = 0;
  DoGetTexParameteriv(GL_TEXTURE_EXTERNAL_OES, GL_TEXTURE_MIN_FILTER,
                      &min_filter, 1);
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
  EXPECT_EQ(min_filter, GL_LINEAR);

  GLint wrap_s = 0;
  DoGetTexParameteriv(GL_TEXTURE_EXTERNAL_OES, GL_TEXTURE_WRAP_S, &wrap_s, 1);
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
  EXPECT_EQ(wrap_s, GL_CLAMP_TO_EDGE);

  GLint wrap_t = 0;
  DoGetTexParameteriv(GL_TEXTURE_EXTERNAL_OES, GL_TEXTURE_WRAP_S, &wrap_t, 1);
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
  EXPECT_EQ(wrap_t, GL_CLAMP_TO_EDGE);
}

TEST_F(GLES2WebGLDecoderPassthroughTest, EGLImageExternalTextureParam) {
  DoRequestExtension("GL_OES_EGL_image_external");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_F(GLES2WebGLDecoderPassthroughTest, EGLImageExternalTextureParamInvalid) {
  DoRequestExtension("GL_OES_EGL_image_external");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
  ...
  TexParameteri cmd;
  cmd.Init(GL_TEXTURE_EXTERNAL_OES, GL_TEXTURE_MIN_FILTER,
...
TEST_F(GLES2WebGLDecoderPassthroughTest, EGLImageExternalTexImage2DError) {
  DoRequestExtension("GL_OES_EGL_image_external");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_F(GLES2DecoderPassthroughTest, DefaultTextureZero) {
  BindTexture cmd1;
...
TEST_F(GLES2WebGLDecoderPassthroughTest, ARBTextureRectangleBindTexture) {
  DoRequestExtension("GL_ANGLE_texture_rectangle");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_F(GLES2WebGLDecoderPassthroughTest, ARBTextureRectangleGetBinding) {
  DoRequestExtension("GL_ANGLE_texture_rectangle");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());

  DoBindTexture(GL_TEXTURE_RECTANGLE_ARB, kClientTextureId);

  GLint binding = 0;
  DoGetIntegerv(GL_TEXTURE_BINDING_RECTANGLE_ARB, &binding, 1);
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
  EXPECT_EQ(kClientTextureId, static_cast<GLuint>(binding));
}

TEST_F(GLES2WebGLDecoderPassthroughTest, ARBTextureRectangleTextureDefaults) {
  DoRequestExtension("GL_ANGLE_texture_rectangle");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
  ...
  GLint min_filter = 0;
  DoGetTexParameteriv(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_MIN_FILTER,
                      &min_filter, 1);
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
  EXPECT_EQ(min_filter, GL_LINEAR);

  GLint wrap_s = 0;
  DoGetTexParameteriv(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_S, &wrap_s, 1);
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
  EXPECT_EQ(wrap_s, GL_CLAMP_TO_EDGE);

  GLint wrap_t = 0;
  DoGetTexParameteriv(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_S, &wrap_t, 1);
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
  EXPECT_EQ(wrap_t, GL_CLAMP_TO_EDGE);
}

TEST_F(GLES2WebGLDecoderPassthroughTest, ARBTextureRectangleTextureParam) {
  DoRequestExtension("GL_ANGLE_texture_rectangle");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_F(GLES2WebGLDecoderPassthroughTest,
  ...
  TexParameteri cmd;
  cmd.Init(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_MIN_FILTER,
           GL_NEAREST_MIPMAP_NEAREST);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
...
TEST_F(GLES2WebGLDecoderPassthroughTest, ARBTextureRectangleTexImage2D) {
  DoRequestExtension("GL_ANGLE_texture_rectangle");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_F(GLES2WebGLDecoderPassthroughTest, ARBTextureRectangleTexImage2DInvalid) {
  DoRequestExtension("GL_ANGLE_texture_rectangle");
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_F(GLES2WebGLDecoderPassthroughTest,
  ...
  TexParameteri cmd;
  cmd.Init(GL_TEXTURE_2D, GL_TEXTURE_USAGE_ANGLE,
...
/*
TEST_F(GLES2DecoderPassthroughTest, ProduceAndConsumeTextureCHROMIUM) {
  ...
  TextureBase* texture = GetDecoder()->GetTextureBase(kClientTextureId);
  ...
  consume_cmd.Init(GL_TEXTURE_2D, kNewClientId, mailbox.name);
  EXPECT_EQ(error::kNoError,
...
TEST_F(GLES2DecoderPassthroughTest, ConsumeAlreadyBoundTexture) {
  Mailbox mailbox = Mailbox::Generate();
...
TEST_F(GLES2DecoderPassthroughTest, ProduceAndConsumeDirectTextureCHROMIUM) {
  Mailbox mailbox = Mailbox::Generate();
...
TEST_F(GLES2DecoderPassthroughTest, ProduceTextureCHROMIUMInvalidTarget) {
  Mailbox mailbox = Mailbox::Generate();
...
TEST_F(GLES2DecoderPassthroughTest,
CreateAndConsumeTextureCHROMIUMInvalidTexture) { Mailbox mailbox =
Mailbox::Generate();
...
TEST_F(GLES2DecoderPassthroughTest,
CreateAndConsumeTextureCHROMIUMInvalidTarget) { Mailbox mailbox =
Mailbox::Generate();
...
TEST_F(GLES2DecoderPassthroughTest, BindTexImage2DCHROMIUM) {
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
  DoTexImage2D(
      GL_TEXTURE_2D, 0, GL_RGBA, 3, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0);
  TextureRef* texture_ref =
...
TEST_F(GLES2DecoderPassthroughTest, BindTexImage2DCHROMIUMCubeMapNotAllowed) {
  scoped_refptr<gl::GLImage> image(new gl::GLImageStub);
  GetImageManagerForTest()->AddImage(image.get(), 1);
  DoBindTexture(GL_TEXTURE_CUBE_MAP, client_texture_id_, kServiceTextureId);
...
TEST_F(GLES2DecoderPassthroughTest, OrphanGLImageWithTexImage2D) {
  scoped_refptr<gl::GLImage> image(new gl::GLImageStub);
  GetImageManagerForTest()->AddImage(image.get(), 1);
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
...
TEST_F(GLES2DecoderPassthroughTest, GLImageAttachedAfterSubTexImage2D) {
...
TEST_F(GLES2DecoderPassthroughTest, GLImageAttachedAfterClearLevel) {
  scoped_refptr<gl::GLImage> image(new gl::GLImageStub);
  GetImageManagerForTest()->AddImage(image.get(), 1);
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
...
TEST_F(GLES2DecoderPassthroughTest, ReleaseTexImage2DCHROMIUM) {
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
  DoTexImage2D(
      GL_TEXTURE_2D, 0, GL_RGBA, 3, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0);
  TextureRef* texture_ref =
...
TEST_F(GLES2DecoderWithShaderTest, CopyTexImage) {
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
...
TEST_F(GLES2DecoderCompressedFormatsTest, GetCompressedTextureFormatsASTC) {
  const GLenum formats[] = {
...
TEST_F(GLES2DecoderManualInitTest, TexStorageInvalidLevels) {
  InitState init;
...
TEST_F(GLES2DecoderManualInitTest, TexStorageInvalidSize) {
  InitState init;
  init.gl_version = "OpenGL 4.2";
...
TEST_F(GLES2DecoderTexStorageFormatAndTypeTest, ES2) {
  InitState init;
  init.gl_version = "OpenGL ES 2.0";
  init.extensions = "GL_ARB_texture_storage";
  init.bind_generates_resource = true;
...
TEST_F(GLES2DecoderTexStorageFormatAndTypeTest, WebGL1) {
  InitState init;
  init.gl_version = "OpenGL ES 2.0";
  init.extensions = "GL_ARB_texture_storage";
  init.bind_generates_resource = true;
...
TEST_F(GLES2DecoderTexStorageFormatAndTypeTest, ES3) {
  InitState init;
  init.gl_version = "OpenGL ES 3.0";
  init.bind_generates_resource = true;
...
TEST_F(GLES2DecoderTexStorageFormatAndTypeTest, WebGL2) {
  InitState init;
  init.gl_version = "OpenGL ES 3.0";
  init.bind_generates_resource = true;
...
TEST_F(GLES3DecoderPassthroughTest, TexStorage3DValidArgs) {
...
TEST_F(GLES3DecoderPassthroughTest, TexImage3DValidArgs) {
  const GLenum kTarget = GL_TEXTURE_3D;
...
TEST_F(GLES3DecoderPassthroughTest, ClearLevel3DSingleCall) {
  const GLenum kTarget = GL_TEXTURE_3D;
  const GLint kLevel = 0;
  const GLint kInternalFormat = GL_RGBA8;
  const GLsizei kWidth = 2;
  ...
  EXPECT_TRUE(GetDecoder()->ClearLevel3D(
      texture, kTarget, kLevel, kFormat, kType, kWidth, kHeight, kDepth));
}

TEST_F(GLES3DecoderPassthroughTest, ClearLevel3DMultipleLayersPerCall) {
  const GLenum kTarget = GL_TEXTURE_3D;
  const GLint kLevel = 0;
  const GLint kInternalFormat = GL_RGBA8;
...
TEST_F(GLES3DecoderPassthroughTest, ClearLevel3DMultipleCallsPerLayer) {
  const GLenum kTarget = GL_TEXTURE_3D;
  const GLint kLevel = 0;
  const GLint kInternalFormat = GL_RGBA8;
...
TEST_F(GLES3DecoderPassthroughTest, BindSamplerInvalidUnit) {
  BindSampler cmd;
...
TEST_F(GLES2DecoderWithShaderTest, CHROMIUMImageEmulatingRGB) {
...
TEST_F(GLES2DecoderPassthroughTest, BindTextureValidArgs) {
  EXPECT_CALL(*gl_, BindTexture(GL_TEXTURE_2D, kServiceTextureId))
      .Times(1)
      .RetiresOnSaturation();
...
TEST_F(GLES2DecoderPassthroughTest, BindTextureValidArgsNewId) {
...
TEST_F(GLES2DecoderPassthroughTest, BindTextureInvalidArgs) {
...
TEST_F(GLES3DecoderPassthroughTest, TexSwizzleAllowed) {
  const GLenum kTarget = GL_TEXTURE_2D;
...

  {
    ...
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GetDecoder()->GetGLES2Util()->GLGetNumValuesReturned(kSwizzleParam),
...
TEST_F(WebGL2DecoderTest, TexSwizzleDisabled) {
  const GLenum kTarget = GL_TEXTURE_2D;
  const GLenum kSwizzleParam = GL_TEXTURE_SWIZZLE_R;
  const GLenum kSwizzleValue = GL_BLUE;
...
TEST_F(GLES2DecoderPassthroughTest,
TestInitDiscardableTextureWithInvalidArguments) { EXPECT_EQ(0u,
group().discardable_manager()->NumCacheEntriesForTesting());
...
TEST_F(GLES2DecoderPassthroughTest, TestDiscardableTextureBindGeneratesUnlocked)
{ DoInitializeDiscardableTextureCHROMIUM(client_texture_id_);
  DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
// TEST_F(GLES2DecoderPassthroughTest, CompressedTexImage2DGLError) {
// }
