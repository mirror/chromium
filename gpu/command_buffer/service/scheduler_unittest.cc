...
class SchedulerTaskRunOrderTest : public SchedulerTest {
 ...
 protected:
  void CreateSequence(int sequence_key, SchedulingPriority priority) {
    ...
    scoped_refptr<SyncPointClientState> release_state =
        sync_point_manager()->CreateSyncPointClientState(
            namespace_id_, command_buffer_id, sequence_id);

    sequence_info_.emplace(
        std::piecewise_construct, std::forward_as_tuple(sequence_key),
        std::forward_as_tuple(sequence_id, command_buffer_id, release_state));
  }
  ...
  void CreateSyncToken(int sequence_key, int release_sync) {
    ...
    sync_tokens_.emplace(
        std::piecewise_construct, std::forward_as_tuple(release_sync),
        std::forward_as_tuple(namespace_id_, 0,
 ...
 private:
  CommandBufferNamespace namespace_id_ = CommandBufferNamespace::GPU_IO;
  ...
  struct SequenceInfo {
    ...
    ~SequenceInfo() { release_state->Destroy(); }
...
TEST_F(SchedulerTaskRunOrderTest, ReleaseSequenceHasPriorityOfWaiter) {
  ...
  ScheduleTask(1, 0, 0);    // task 1: seq 1, wait 0, release 0
  ScheduleTask(2, -1, -1);  // task 2: seq 2, no wait, no release
