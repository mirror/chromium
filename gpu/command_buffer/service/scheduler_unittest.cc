...
class SchedulerTaskRunOrderTest : public SchedulerTest {
 ...
 protected:
  ...
  void ScheduleTask(int sequence_key, std::vector<int> wait_sync, int release_sync) {
    const int task_id = num_tasks_scheduled_++;

    std::vector<SyncToken> wait;
    for (int sync_token_key : wait_sync)
      wait.push_back(sync_tokens_[sync_token_key]);
  ...
  void ScheduleTask(int sequence_key, int wait_sync, int release_sync) {
    std::vector<int> waits = {wait_sync};
    ScheduleTask(sequence_key, waits, release_sync);
  }
...
TEST_F(SchedulerTaskRunOrderTest, ReleaseSequenceWTF) {
  CreateSequence(0, SchedulingPriority::kLow);
  CreateSequence(1, SchedulingPriority::kNormal);
  CreateSequence(2, SchedulingPriority::kHigh);
  ...
  ScheduleTask(0, -1, 1);  // task 0: seq 0, no wait, no release
  ScheduleTask(1, std::vector<int>({1, 1, 0}), 2);  // task 1: seq 1, no wait, no release
  ScheduleTask(2, std::vector<int>({2}), -1);  // task 2: seq 2, no wait, no release
}
