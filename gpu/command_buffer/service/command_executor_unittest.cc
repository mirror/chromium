...
class CommandExecutorTest : public testing::Test {
 ...
 protected:
  ...
  int32_t GetPut() { return command_buffer_->put_offset(); }
...
TEST_F(CommandExecutorUnscheduleExecutionTest, Unschedule) {
  ...
  EXPECT_EQ(4, GetPut());
  EXPECT_EQ(2, calls_);
  EXPECT_EQ(false, command_buffer_service()->scheduled());
  ...
  EXPECT_EQ(4, GetPut());
  EXPECT_EQ(5, calls_);
  EXPECT_EQ(true, command_buffer_service()->scheduled());
}
