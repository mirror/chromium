...
std::vector<uint8_t> DecompressData(const std::vector<uint8_t>& data,
  ...
  // We never add elements unless their uncompressed size fits in cache. Check
  // here to guard against runaway allocations.
  if (decompressed_size > max_size_bytes)
  ...
  auto result = uncompress(decompressed_data.data(), &decompressed_size_out,
                           data.data(), data.size());
  if (Z_OK != result || decompressed_data.size() != decompressed_size_out)
    return std::vector<uint8_t>();
...
ProgramCache::ProgramLoadResult MemoryProgramCache::LoadLinkedProgram(
  ...
  // Failure to decompress will return an empty vector.
  if (decoded.size() == 0)
    return PROGRAM_LOAD_FAILURE;
...
void MemoryProgramCache::LoadProgram(const std::string& key,
  ...
  if (proto->ParseFromString(program)) {
    ...
    store_.Put(
        proto->sha(),
        new ProgramCacheValue(
            proto->format(), std::move(binary), proto->program_is_compressed(),
