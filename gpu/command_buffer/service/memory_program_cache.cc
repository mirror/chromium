...
std::vector<uint8_t> CompressData(const std::vector<uint8_t>& data) {
  ...
  auto result = compress2(compressed_data.data(), &compressed_size, data.data(),
                          data.size(), 1 /* level */);
  // It is impossible for compression to fail with the provided parameters. If
  // it did fail, we would crash later, so just crash here to catch the issue.
...
std::vector<uint8_t> DecompressData(const std::vector<uint8_t>& data,
  ...
  auto result = uncompress(decompressed_data.data(), &decompressed_size_out,
                           data.data(), data.size());
  // Uncompress shouldn't fail unless we've hit disk/memory corruption.
  CHECK_EQ(Z_OK, result);
  CHECK_EQ(decompressed_data.size(), decompressed_size_out);
