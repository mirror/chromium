...
void ContextState::RestoreSamplerBinding(GLuint unit,
  ...
  if (!feature_info_->IsES3Capable())
    return;
  DCHECK_LT(unit, sampler_units.size());
  const scoped_refptr<Sampler>& cur_sampler = sampler_units[unit];
  GLuint cur_id = 0;
  if (cur_sampler && !cur_sampler->IsDeleted()) {
    cur_id = cur_sampler->service_id();
  }
  GLuint prev_id = 0;
  if (prev_state) {
    DCHECK_LT(unit, prev_state->sampler_units.size());
    const scoped_refptr<Sampler>& prev_sampler =
        prev_state->sampler_units[unit];
    if (prev_sampler && !prev_sampler->IsDeleted()) {
      prev_id = prev_sampler->service_id();
    }
  }
