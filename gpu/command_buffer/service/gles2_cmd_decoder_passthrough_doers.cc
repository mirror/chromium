...
error::Error GLES2DecoderPassthroughImpl::DoFinish() {
  ...
  error::Error error = ProcessQueries(true);
  if (error != error::kNoError) {
    return error;
  }

  error = ProcessReadPixels(true);
  if (error != error::kNoError) {
    return error;
  }
...
error::Error GLES2DecoderPassthroughImpl::DoFlush() {
  ...
  error::Error error = ProcessQueries(false);
  if (error != error::kNoError) {
    return error;
  }

  error = ProcessReadPixels(false);
  if (error != error::kNoError) {
    return error;
  }
...
error::Error GLES2DecoderPassthroughImpl::DoReadPixelsAsync(
  ...
  if (feature_info_->gl_version_info().IsAtLeastGLES(3, 0)) {
    pending_read_pixels.sync_service_id =
        glFenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0);
  } else {
    DCHECK(feature_info_->feature_flags().nv_fence);
    glGenFencesNV(1, &pending_read_pixels.fence_nv_service_id);
    glSetFenceNV(pending_read_pixels.fence_nv_service_id, GL_ALL_COMPLETED_NV);
  }
  ...
  pending_read_pixels_.push_back(pending_read_pixels);
  return error::kNoError;
}
...
error::Error GLES2DecoderPassthroughImpl::DoEndQueryEXT(GLenum target,
  ...
  if (IsEmulatedQueryTarget(target)) {
    ...
    if (target == GL_ASYNC_PIXEL_PACK_COMPLETED_CHROMIUM &&
      ...
      GLuint query_service_id = active_queries_[target].service_id;
