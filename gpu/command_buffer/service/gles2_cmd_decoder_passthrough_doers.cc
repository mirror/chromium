...
error::Error GLES2DecoderPassthroughImpl::DoSwapBuffers() {
  if (offscreen_) {
    ...
    if (emulated_default_framebuffer_format_.samples > 0) {
      ...
      // texture using blit framebuffer
      DCHECK(emulated_back_buffer_->color_texture == nullptr);

      ScopedFramebufferBindingReset scoped_fbo_reset;

      glBindFramebufferEXT(GL_DRAW_FRAMEBUFFER, temp_front_buffer_fbo_id_);
      glFramebufferTexture2DEXT(
          GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
          emulated_front_buffer_->texture->service_id(), 0);

      glBindFramebufferEXT(GL_READ_FRAMEBUFFER,
                           emulated_back_buffer_->framebuffer_service_id);

      glBlitFramebufferANGLE(0, 0, emulated_back_buffer_->size.width(),
                             emulated_back_buffer_->size.height(), 0, 0,
                             emulated_back_buffer_->size.width(),
                             emulated_back_buffer_->size.height(),
      ...
      // attachment.
      std::swap(emulated_back_buffer_->color_texture, emulated_front_buffer_);
      ...
      glFramebufferTexture2DEXT(
          GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
          emulated_back_buffer_->color_texture->texture->service_id(), 0);

      // If the offscreen buffer should be preserved, copy the old backbuffer
      // into the new one
      if (offscreen_target_buffer_preserved_) {
        glBindFramebufferEXT(GL_READ_FRAMEBUFFER, temp_front_buffer_fbo_id_);
        glFramebufferTexture2DEXT(
