...
ShaderId GetFragmentShaderId(bool premultiply_alpha,
  ...
  return alphaIndex + ditherIndex * 4 + targetIndex * 4 * 2 +
         sourceFormatIndex * 4 * 2 * NUM_SAMPLERS +
         destFormatIndex * 4 * 2 * NUM_SAMPLERS * NUM_S_FORMAT;
}
...
std::string GetFragmentShaderSource(const gl::GLVersionInfo& gl_version_info,
  ...
  if (dither) {
    source += "  float range = 1.0 / 15.0;\n";
    // Create a 4x4 dither pattern using step/mod.
    source +=
        "  vec4 modValues = mod(gl_FragCoord.xyxy, vec4(2.0, 2.0, 4.0, "
        "4.0));\n";
    source +=
        "  vec4 stepValues = step(modValues, vec4(1.0, 1.0, 2.0, 2.0));\n";
    source +=
        "  float dither_value = dot(stepValues, vec4(8.0 / 16.0, 4.0 / 16.0, "
        "2.0 / 16.0, 1.0 / 16.0)) - 15.0 / 32.0;\n";
    // Apply the dither offset to the color.
    source += "  color += dither_value * range;\n";
    source +=
