...
TEST_P(GLES2DecoderTest, TestInitDiscardableTextureWithInvalidArguments) {
  ...
  // Manually initialize an init command with an invalid buffer.
  {
    ...
    cmd.Init(client_texture_id_, DiscardableHandleBase::IdFromShmAndOffset(kInvalidSharedMemoryId, 0));
    EXPECT_EQ(error::kInvalidArguments, ExecuteCmd(cmd));
    EXPECT_EQ(0u, group().discardable_manager()->NumCacheEntriesForTesting());
  }
  ...
  {
    cmds::InitializeDiscardableTextureCHROMIUM cmd;
    cmd.Init(client_texture_id_, DiscardableHandleBase::IdFromShmAndOffset(shared_memory_id_, kInvalidSharedMemoryOffset));
    EXPECT_EQ(error::kInvalidArguments, ExecuteCmd(cmd));
    EXPECT_EQ(0u, group().discardable_manager()->NumCacheEntriesForTesting());
  }
  ...
  {
    cmds::InitializeDiscardableTextureCHROMIUM cmd;
    cmd.Init(client_texture_id_, DiscardableHandleBase::IdFromShmAndOffset(shared_memory_id_, 1));
    EXPECT_EQ(error::kInvalidArguments, ExecuteCmd(cmd));
    EXPECT_EQ(0u, group().discardable_manager()->NumCacheEntriesForTesting());
  }
}
