...
void TransformFeedback::BindTransformFeedbackBuffer(Buffer* buffer) {
  if (do_buffer_refcounting_ && is_bound_ && bound_transform_feedback_buffer_)
    bound_transform_feedback_buffer_->OnUnbind(GL_TRANSFORM_FEEDBACK_BUFFER);
  bound_transform_feedback_buffer_ = buffer;
  if (do_buffer_refcounting_ && is_bound_ && buffer)
    buffer->OnBind(GL_TRANSFORM_FEEDBACK_BUFFER);
}

void TransformFeedback::SetIsBound(bool is_bound) {
  if (is_bound != is_bound_ && do_buffer_refcounting_ &&
      bound_transform_feedback_buffer_) {
    if (is_bound)
      bound_transform_feedback_buffer_->OnBind(GL_TRANSFORM_FEEDBACK_BUFFER);
    else
      bound_transform_feedback_buffer_->OnUnbind(GL_TRANSFORM_FEEDBACK_BUFFER);
  }
  IndexedBufferBindingHost::SetIsBound(is_bound);
  DCHECK(is_bound == is_bound_);
}

void TransformFeedback::RemoveBoundBuffer(Buffer* buffer) {
  if (bound_transform_feedback_buffer_ == buffer) {
    if (do_buffer_refcounting_ && buffer) {
      buffer->OnUnbind(GL_TRANSFORM_FEEDBACK_BUFFER);
    }
    bound_transform_feedback_buffer_ = nullptr;
  }
  IndexedBufferBindingHost::RemoveBoundBuffer(buffer);
}
