...
gpu::ContextResult GLES2DecoderPassthroughImpl::Initialize(
  ...
  if (num_texture_units > static_cast<GLint>(kMaxTextureUnits)) {
    Destroy(true);
    LOG(ERROR) << "Implementation limit GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS ("
               << kMaxTextureUnits << " must be less than kMaxTextureUnits ("
               << kMaxTextureUnits << ").";
    return gpu::ContextResult::kFatalFailure;
  }
...
// static
GLenum GLES2DecoderPassthroughImpl::TextureTargetToGLenum(
    TextureTarget target) {
  switch (target) {
    case TextureTarget::k2D:
      return GL_TEXTURE_2D;
    case TextureTarget::kCubeMap:
      return GL_TEXTURE_CUBE_MAP;
    case TextureTarget::k2DArray:
      return GL_TEXTURE_2D_ARRAY;
    case TextureTarget::k3D:
      return GL_TEXTURE_3D;
    case TextureTarget::k2DMultisample:
      return GL_TEXTURE_2D_MULTISAMPLE;
    case TextureTarget::kExternal:
      return GL_TEXTURE_EXTERNAL_OES;
    case TextureTarget::kRectangle:
