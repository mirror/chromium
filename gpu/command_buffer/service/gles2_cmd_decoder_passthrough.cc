...
void APIENTRY
GLES2DecoderPassthroughImpl::GLDebugMessageCallback(GLenum source,
                                                    GLenum type,
                                                    GLuint id,
                                                    GLenum severity,
                                                    GLsizei length,
...
gpu::ContextResult GLES2DecoderPassthroughImpl::Initialize(
  ...
  } else if (IsWebGLContextType(attrib_helper.context_type) &&
             feature_info_->feature_flags().khr_debug) {
    // Report GL errors to the client (so they show up in the devtools console).
