...
TEST_F(GpuFenceManagerTest, Basic) {
  ...
  ON_CALL(*egl_, CreateSyncKHR(_, _, _)).WillByDefault(Return(kDummySync));
  ON_CALL(*egl_, WaitSyncKHR(_, _, _)).WillByDefault(Return(EGL_TRUE));
  ON_CALL(*egl_, DupNativeFenceFDANDROID(_, _)).WillByDefault(Return(kDummyFD));
  ...
  EXPECT_CALL(*egl_, WaitSyncKHR(_, kDummySync, _))
      .Times(1)
      .RetiresOnSaturation();
  manager_->WaitGpuFence(kClient1Id);
  ...
  EXPECT_EQ(handle.native_fd.fd, kDummyFD);
...
TEST_F(GpuFenceManagerTest, Callbacks) {
  const GLuint kClient1Id = 1;
  const GLuint kClient2Id = 2;
  const EGLSyncKHR kDummySync = reinterpret_cast<EGLSyncKHR>(0x2001);
  ...
  manager_->AddCallback(kClient1Id,
                        base::Bind([](bool* out) { *out = true; },
                                   base::Unretained(&callback_was_run)));
  EXPECT_FALSE(callback_was_run);

  // Create the fence object now for the stored callback.
  ON_CALL(*egl_, CreateSyncKHR(_, _, _)).WillByDefault(Return(kDummySync));
  EXPECT_CALL(*gl_, Flush()).Times(1).RetiresOnSaturation();
  manager_->CreateNewGpuFence(kClient1Id);
  EXPECT_TRUE(manager_->IsValidGpuFence(kClient1Id));

  // Callback should now have been executed.
  ...
  EXPECT_CALL(*gl_, Flush()).Times(1).RetiresOnSaturation();
  manager_->CreateNewGpuFence(kClient2Id);
  ...
  manager_->AddCallback(kClient2Id,
                        base::Bind([](bool* out) { *out = true; },
                                   base::Unretained(&callback_was_run)));
  EXPECT_TRUE(callback_was_run);
}
