...
struct GPU_EXPORT ContextState {
  ...
  inline void SetVertexAttribEnabledAndApply(GLuint index, bool enable) const {
    // inline because many calls to this function have a constexpr 'enable' arg
    DCHECK_LT(index, attrib_values.size());
    GLuint shift_bits = (index % 16) * 2;
    if (enable) {
      attrib_enabled_in_driver_mask_[index / 16] |= (0x3 << shift_bits);
  ...
  void ApplyMinimalAttribEnableStateForCurrentProgram() const {
    ...
    size_t attribCount = attrib_active_in_shader.size();
    DCHECK_EQ(attribCount, attrib_enabled.size());
    DCHECK_EQ(attribCount, attrib_real_enabled.size());
