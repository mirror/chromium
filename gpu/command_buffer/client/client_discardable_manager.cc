...
ClientDiscardableTextureManager::ClientDiscardableTextureManager(ClientDiscardableManager* discardable_manager)
: discardable_manager_(discardable_manager) {}

ClientDiscardableTextureManager::~ClientDiscardableTextureManager() = default;

DiscardableHandleId ClientDiscardableTextureManager::InitializeTexture(gles2::GLES2CmdHelper* command_helper,
                                      uint32_t texture_id) {
...
void ClientDiscardableTextureManager::FreeTexture(uint32_t texture_id) {
  ...
  if (found == texture_id_to_handle_id_.end())
    return;
  return discardable_manager_->FreeHandle(found->second);
}
