...
GLuint GLES2Implementation::InsertClientGpuFenceCHROMIUM(
  ...
  // WaitGpuFenceCHROMIUM. Flush to ensure that any pending previous Insert was
  // processed.
  ShallowFlushCHROMIUM();
...
void GLES2Implementation::GetGpuFenceHandle(
  ...
  // Issue a flush here to help ensure that a preceding InsertGpuFenceCHROMIUM
  // will be processed in due course, to avoid relying on client flushing.
  ShallowFlushCHROMIUM();
  gpu_control_->GetGpuFenceHandle(gpu_fence_id, std::move(callback));
}
