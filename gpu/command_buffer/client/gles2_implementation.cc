...
void GLES2Implementation::RasterCHROMIUM(const cc::DisplayItemList* list,
  ...
  preamble.push<cc::TranslateOp>(-translate_x, -translate_y);
  ...
  if (post_translate_x != 0.f || post_translate_y != 0.f)
    preamble.push<cc::TranslateOp>(post_translate_x, post_translate_y);
  if (post_scale != 1.f)
    preamble.push<cc::ScaleOp>(post_scale, post_scale);
  ...
  cc::PaintOpBufferSerializer<PaintOpSerializer> serializer(&op_serializer,
                                                            nullptr);
