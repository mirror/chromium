...
GLuint GLES2Implementation::CreateGpuFenceCHROMIUM() {
  ...
  // allocating two per frame at 60fps. Revisit if this is an issue, for
  // example by deferring ID release until the service side confirms they are
  // unused.
...
void GLES2Implementation::GetGpuFence(
  ...
  // Issue a flush here to help ensure that a preceding CreateGpuFenceCHROMIUM
  // will be processed in due course, to avoid relying on client flushing.
  ShallowFlushCHROMIUM();
