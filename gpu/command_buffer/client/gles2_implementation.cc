...
void GLES2Implementation::RasterCHROMIUM(const cc::DisplayItemList* list,
  ...
  cc::PaintOpBuffer preamble;
  preamble.push<cc::TranslateOp>(SkIntToScalar(-translate_x),
                                 SkIntToScalar(-translate_y));
  preamble.push<cc::ClipRectOp>(
      SkRect::MakeFromIRect(gfx::RectToSkIRect(playback_rect)),
      SkClipOp::kIntersect, false);
  if (post_translate_x != 0.f || post_translate_y != 0.f) {
    preamble.push<cc::TranslateOp>(SkIntToScalar(post_translate_x),
                                   SkIntToScalar(post_translate_y));
  }
  if (post_scale != 1.f) {
    preamble.push<cc::ScaleOp>(SkIntToScalar(post_scale),
                               SkIntToScalar(post_scale));
  }
