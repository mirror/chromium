...
class GLES2ImplementationTest : public testing::Test {
 protected:
  ...
  void ClearCommands() { test_contexts_[0].ClearCommands(); }
...
TEST_F(GLES2ImplementationTest, GenSyncTokenCHROMIUM) {
  ...
  ClearCommands();
  EXPECT_CALL(*gpu_control_, GenerateFenceSyncRelease()).Times(0);
  gl_->GenSyncTokenCHROMIUM(nullptr);
  EXPECT_TRUE(NoCommandsWritten());
  EXPECT_EQ(GL_INVALID_VALUE, CheckError());

  ClearCommands();
  int32_t put_offset = command_buffer()->GetServicePutOffset();
  ...
  EXPECT_EQ(0, memcmp(&expected, commands_ + put_offset, sizeof(expected)));
...
TEST_F(GLES2ImplementationTest, VerifySyncTokensCHROMIUM) {
  ...
  const CommandBufferId kCommandBufferId =
      CommandBufferId::FromUnsafeValue(234u);
  const GLuint64 kFenceSync = 123u;
  gpu::SyncToken sync_token(kNamespaceId, kCommandBufferId, kFenceSync);
...
TEST_F(GLES2ImplementationTest, VerifySyncTokensCHROMIUM_Sequence) {
  ...
  SyncToken sync_token1(kNamespaceId, kCommandBufferId, kFenceSync1);
  SyncToken sync_token2(kNamespaceId, kCommandBufferId, kFenceSync2);
...
TEST_F(GLES2ImplementationTest, WaitSyncTokenCHROMIUM) {
  ...
  SyncToken sync_token(kNamespaceId, kCommandBufferId, kFenceSync);
...
TEST_F(GLES2ImplementationTest, SignalSyncToken) {
  SyncToken sync_token(GPU_IO, CommandBufferId::FromUnsafeValue(1), 1);
  sync_token.SetVerifyFlush();
