...
TEST(ClientTransferCacheTest, BasicUsage) {
  ...
  ClientTransferCache cache(&gl, &helper, &base_manager);

  std::vector<uint8_t> data;
  data.resize(100);
  std::unique_ptr<RawMemoryTransferCacheEntry> entry(
      new RawMemoryTransferCacheEntry(std::move(data)));
  TransferCacheEntryId id = cache.CreateCacheEntry(std::move(entry));
  EXPECT_TRUE(id);
  ClientDiscardableHandle handle = base_manager.GetHandleForTesting(id);
  EXPECT_TRUE(handle.IsLockedForTesting());

  cache.UnlockTransferCacheEntry(id);
  ...
  cache.DeleteTransferCacheEntry(id);
}
