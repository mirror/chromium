...
TEST_F(TransferBufferExpandContractTest, Shrink) {
  ...
  transfer_buffer_->Shrink(ptr, shrink_size1);
  ...
  transfer_buffer_->Shrink(ptr, shrink_size2);
  EXPECT_EQ(alloc_size - shrink_size2, transfer_buffer_->GetFreeSize());

  // Try to "shrink" to larger sizes, but fail to change anything.
  transfer_buffer_->Shrink(ptr, shrink_size1);
  EXPECT_EQ(alloc_size - shrink_size2, transfer_buffer_->GetFreeSize());
  transfer_buffer_->Shrink(ptr, alloc_size);
  EXPECT_EQ(alloc_size - shrink_size2, transfer_buffer_->GetFreeSize());
  transfer_buffer_->Shrink(ptr, alloc_size * 10);
  EXPECT_EQ(alloc_size - shrink_size2, transfer_buffer_->GetFreeSize());

  // Shrink to zero (minimum size is 1).
  transfer_buffer_->Shrink(ptr, 0);
  EXPECT_EQ(alloc_size - 1, transfer_buffer_->GetFreeSize());

  transfer_buffer_->FreePendingToken(ptr, 1);
}

TEST_F(TransferBufferExpandContractTest, ShrinkNotInUseNoop) {
  unsigned int initial_free = transfer_buffer_->GetFreeSize();

  unsigned int alloc_size1 = 20u;
  unsigned int size_allocated1 = 0;
  void* penultimate_ptr =
      transfer_buffer_->AllocUpTo(alloc_size1, &size_allocated1);
  EXPECT_EQ(alloc_size1, size_allocated1);
  EXPECT_EQ(initial_free - alloc_size1, transfer_buffer_->GetFreeSize());
  transfer_buffer_->FreePendingToken(penultimate_ptr, 1u);

  // Try to shrink a block not in use.
  EXPECT_EQ(initial_free, transfer_buffer_->GetFreeSize());
  transfer_buffer_->Shrink(penultimate_ptr, 10u);
  EXPECT_EQ(initial_free, transfer_buffer_->GetFreeSize());
}
