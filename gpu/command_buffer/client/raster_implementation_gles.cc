...
void RasterImplementationGLES::GenTextures(GLsizei n, GLuint* textures) {
  ...
  for (GLsizei i = 0; i < n; i++) {
    ...
    if (texture_id) {
      texture_info_.emplace(std::make_pair(texture_id, Texture(texture_id)));
    }
  }
}
...
void RasterImplementationGLES::SetTextureStorageInfo(
    GLuint texture_id,
    bool use_buffer,
  ...
  if (texture && texture->target == GL_NONE) {
    texture->target = use_buffer
                          ? GetImageTextureTarget(caps_, buffer_usage, format)
                          : GL_TEXTURE_2D;
    texture->use_buffer = use_buffer;
    texture->usage = buffer_usage;
