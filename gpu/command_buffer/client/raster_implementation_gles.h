...
class GLES2_IMPL_EXPORT RasterImplementationGLES : public RasterInterface {
 public:
  ...
  void SetTextureStorageInfo(GLuint texture_id,
                             bool use_buffer,
 ...
 private:
  struct Texture {
    Texture(GLuint id) : id(id), target(GL_NONE), use_buffer(false) {}
    GLuint id;
    GLenum target;
    bool use_buffer;
  ...
  Texture* GetTexture(GLuint texture_id, bool validate_binding);
