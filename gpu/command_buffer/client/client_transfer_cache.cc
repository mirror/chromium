...
void ClientTransferCache::CreateCacheEntry(
  ...
  DCHECK(!FindDiscardableHandleId(entry.Type(), entry.Id()));
  discardable_handle_id_map_[entry.Type()][entry.Id()] = id;
...
bool ClientTransferCache::LockTransferCacheEntry(
  ...
  discardable_handle_id_map_[type].erase(id);
  return false;
}
...
base::Optional<ClientDiscardableHandle::Id>
ClientTransferCache::FindDiscardableHandleId(cc::TransferCacheEntryType type,
                                             uint32_t id) {
  auto discardable_handle_id_map_it = discardable_handle_id_map_.find(type);
  if (discardable_handle_id_map_it == discardable_handle_id_map_.end())
    return base::nullopt;

  auto& id_map = discardable_handle_id_map_it->second;
