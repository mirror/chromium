...
void ClientTransferCache::CreateCacheEntry(
  ...
  discardable_handle_id_map_[std::make_pair(entry.Type(), entry.Id())] = id;
...
bool ClientTransferCache::LockTransferCacheEntry(
  ...
  discardable_handle_id_map_.erase(std::make_pair(type, id));
  return false;
}
...
ClientDiscardableHandle::Id ClientTransferCache::FindDiscardableHandleId(
    cc::TransferCacheEntryType type,
    uint32_t id) {
  auto discardable_handle_id_map_it =
      discardable_handle_id_map_.find(std::make_pair(type, id));
  if (discardable_handle_id_map_it == discardable_handle_id_map_.end())
    return ClientDiscardableHandle::Id();
  return discardable_handle_id_map_it->second;
}
