...
TransferCacheEntryId ClientTransferCache::CreateCacheEntry(
  ...
  bool succeeded = entry->Serialize(size, mapped_alloc.address());
  DCHECK(succeeded);
  TransferCacheEntryId id = discardable_manager_->CreateHandle(helper);
...
void ClientTransferCache::UnlockTransferCacheEntry(TransferCacheEntryId id) {
  discardable_manager_->UnlockHandle(id);
