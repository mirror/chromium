...
void ForceDiscreteGPU() {
  ...
  CGLChoosePixelFormat(attribs, &g_disceret_pixel_format_obj,
                       &num_pixel_formats);
#endif  // OS_MACOSX
}
...
void InitializeSwitchableGPUs(
  ...
  if (ContainsWorkaround(driver_bug_workarounds, FORCE_DISCRETE_GPU)) {
    // ui::gl::GLContext::SetSwitchableGpuPreference(gl::PreferDiscreteGpu);
  } else if (ContainsWorkaround(driver_bug_workarounds, FORCE_INTEGRATED_GPU)) {
    // ui::gl::GLContext::SetSwitchableGpuPreference(gl::PreferIntegratedGpu);
    ForceDiscreteGPU();
  }
}
