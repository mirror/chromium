...
void IntSetToWorkarounds(const std::vector<uint32_t>& enabled_workarounds,
...
void StringToWorkarounds(const std::string& types,
  ...
  for (auto piece : base::SplitStringPiece(types, ",", base::TRIM_WHITESPACE,
                                           base::SPLIT_WANT_ALL)) {
    unsigned number = 0;
    bool succeed = base::StringToUint(piece, &number);
    DCHECK(succeed);
...
GpuDriverBugWorkarounds::GpuDriverBugWorkarounds(
    const std::vector<uint32_t>& enabled_driver_bug_workarounds) {
