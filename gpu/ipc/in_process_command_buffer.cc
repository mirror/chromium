...
void InProcessCommandBuffer::BufferPresented(uint64_t swap_id,
                                             base::TimeTicks timestamp,
                                             base::TimeDelta refresh,
                                             uint32_t flags) {
  if (!origin_task_runner_) {
    BufferPresentedOnOriginThread(swap_id, timestamp, refresh, flags);
    return;
  }
  origin_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&InProcessCommandBuffer::BufferPresentedOnOriginThread,
                 client_thread_weak_ptr_, swap_id, timestamp, refresh, flags));
}
...
void InProcessCommandBuffer::BufferPresentedOnOriginThread(
  ...
  if (presentation_callback_)
    presentation_callback_.Run(swap_id, timestamp, refresh, flags);
