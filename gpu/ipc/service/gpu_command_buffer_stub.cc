...
bool GpuCommandBufferStub::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP(GpuCommandBufferStub, message)
    ...
    IPC_MESSAGE_HANDLER(GpuCommandBufferMsg_CreateSharedBuffer, OnCreateSharedBuffer);
    IPC_MESSAGE_HANDLER(GpuCommandBufferMsg_SetSharedBufferHandle, OnSetSharedBufferHandle);
...
void GpuCommandBufferStub::OnSetSharedBufferHandle(const Mailbox& mailbox, const gfx::GpuMemoryBufferHandle& handle) {
  //LOG(INFO) << __FUNCTION__ << ";;; mailbox=" << mailbox.DebugString();
  gles2::MailboxManager* mailbox_manager = context_group_->mailbox_manager();
...
void GpuCommandBufferStub::OnCreateImage(
  ...
  if (handle.type == gfx::MAILBOX_SHARED_BUFFER) {
    //LOG(INFO) << __FUNCTION__ << ";;; got MAILBOX_SHARED_BUFFER handle_mailbox=" << handle.MailboxSharedBufferString();
    gpu::Mailbox mailbox;
    ...
    gles2::SharedBuffer* buf = mailbox_manager->GetSharedBuffer(mailbox);
    if (buf) {
      //LOG(INFO) << __FUNCTION__ << ";;; found SharedBuffer, replacing handle";
