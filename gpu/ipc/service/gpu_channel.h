...
class GPU_EXPORT GpuChannel : public IPC::Listener, public FilteredSender {
 public:
  ...
  void OnCommandBufferScheduled(CommandBufferStubCommon* stub);
  void OnCommandBufferDescheduled(CommandBufferStubCommon* stub);

  gl::GLShareGroup* share_group() const { return share_group_.get(); }

  CommandBufferStubCommon* LookupCommandBuffer(int32_t route_id);
  ...
  const CommandBufferStubCommon* GetOneStub() const;
#endif
 ...
 private:
  ...
  base::flat_map<int32_t, std::unique_ptr<CommandBufferStubCommon>> stubs_;
