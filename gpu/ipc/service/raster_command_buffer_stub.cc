...
gpu::ContextResult RasterCommandBufferStub::Initialize(
    CommandBufferStubCommon* share_command_buffer_stub,
    const GPUCreateCommandBufferConfig& init_params,
  ...
  // FIXME(backer): Probably OK. This may be the only stub on the channel.
  ...
  // FIXME(backer): Not sure why they clamped this down for
  // use_passthrough_cmd_decoder before...
  ...
  // FIXME(backer): Seems suspicious.
  if (!active_url_.is_empty())
    manager->delegate()->DidCreateOffscreenContext(active_url_);

  // FIXME(backer): Crashes because we didn't create a virtual context.
  if (use_virtualized_gl_context_) {
