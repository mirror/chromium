...
void GpuInit::InitializeInProcess(base::CommandLine* command_line,
  ...
  // In theory we only need to do this in Android WebView.
  GLBindingsInitializationAutoLock lock;
  ...
  if (gl::GetGLImplementation() == gl::kGLImplementationNone) {
    ...
    CacheGPUInfo(gpu_info_);
    CacheGpuFeatureInfo(gpu_feature_info_);
  } else {
    // GL bindings already initialized. This is for Android WebView
    // and GPUInfo / GpuFeatureInfo should also be computed and
    // cached.
    bool gpu_info_cached = PopGPUInfoCache(&gpu_info_);
    bool gpu_feature_info_cached = PopGpuFeatureInfoCache(&gpu_feature_info_);
