...
uint32_t GpuChannelHost::OrderingBarrier(
  ...
  if (put_offset_changed) {
    ...
    if (!flush_params.flush_id)
      flush_params.flush_id = next_flush_id_++;
    uint32_t flush_id = flush_params.flush_id;

    flush_params.route_id = route_id;
    flush_params.put_offset = put_offset;
    flush_params.flush_count = flush_count;
    flush_params.latency_info.insert(flush_params.latency_info.end(),
                                     latency_info.begin(), latency_info.end());
    flush_params.sync_token_fences.insert(flush_params.sync_token_fences.end(),
...
void GpuChannelHost::InternalFlush() {
  ...
  Send(new GpuChannelMsg_FlushCommandBuffers(flush_list_));

  DCHECK_GT(flush_list_.back().flush_id, flushed_flush_id_);
  flushed_flush_id_ = flush_list_.back().flush_id;
...
bool GpuChannelHost::VerifyFlush(uint32_t flush_id) {
  AutoLock lock(context_lock_);
  DCHECK_LT(flush_id, next_flush_id_);

  if (flush_id <= verified_flush_id_)
    return true;

  EnsureFlush(flush_id);

  if (Send(new GpuChannelMsg_Nop())) {
    verified_flush_id_ = flushed_flush_id_;
    return true;
  }
