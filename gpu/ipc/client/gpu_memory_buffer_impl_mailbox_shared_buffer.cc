...
GpuMemoryBufferImplMailboxSharedBuffer::GpuMemoryBufferImplMailboxSharedBuffer(
  ...
  *mailbox_ = mailbox;
  //DVLOG(3) << __FUNCTION__ << ";;; mailbox=" << mailbox.DebugString() << " *mailbox_=" << mailbox_->DebugString();
}

GpuMemoryBufferImplMailboxSharedBuffer::~GpuMemoryBufferImplMailboxSharedBuffer() {}

// static
std::unique_ptr<GpuMemoryBufferImplMailboxSharedBuffer> GpuMemoryBufferImplMailboxSharedBuffer::CreateFromHandle(
      const gfx::GpuMemoryBufferHandle& handle,
      const gfx::Size& size,
      gfx::BufferFormat format,
      gfx::BufferUsage usage,
      const DestructionCallback& callback) {
  //DVLOG(3) << __FUNCTION__ << ";;; handle_mailbox=" << handle.MailboxSharedBufferString();
  gpu::Mailbox mailbox;
  std::copy(std::begin(handle.mailbox_shared_buffer),
            std::end(handle.mailbox_shared_buffer),
            std::begin(mailbox.name));
...
// static
bool GpuMemoryBufferImplMailboxSharedBuffer::IsConfigurationSupported(gfx::BufferFormat format, gfx::BufferUsage usage) {
...
gfx::GpuMemoryBufferHandle GpuMemoryBufferImplMailboxSharedBuffer::GetHandle() const {
  return GetHandleForMailbox(*mailbox_, id_);
}

// static
gfx::GpuMemoryBufferHandle GpuMemoryBufferImplMailboxSharedBuffer::GetHandleForMailbox(const Mailbox& mailbox, gfx::GpuMemoryBufferId id) {
  gfx::GpuMemoryBufferHandle handle;
  handle.type = gfx::MAILBOX_SHARED_BUFFER;
  handle.id = id;
  std::copy(std::begin(mailbox.name),
            std::end(mailbox.name),
