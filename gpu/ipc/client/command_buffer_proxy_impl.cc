...
int32_t CommandBufferProxyImpl::CreateImage(ClientBuffer buffer,
  ...
  //LOG(INFO) << __FUNCTION__ << ";;; handle.type=" << handle.type << " handle_mailbox=" << handle.MailboxSharedBufferString();
...
void CommandBufferProxyImpl::SetSharedBufferHandle(const Mailbox& mailbox, const gfx::GpuMemoryBufferHandle& handle) {
  CheckLock();
  base::AutoLock lock(last_state_lock_);
  Send(new GpuCommandBufferMsg_SetSharedBufferHandle(route_id_, mailbox, handle));
}
