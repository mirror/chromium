...
bool CommandBufferProxyImpl::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP(CommandBufferProxyImpl, message)
    ...
    IPC_MESSAGE_HANDLER(GpuCommandBufferMsg_FetchGpuFenceComplete,
                        OnFetchGpuFenceComplete);
...
void CommandBufferProxyImpl::FetchGpuFence(
  ...
  uint32_t fetch_id = next_fetch_gpu_fence_task_id_++;
  Send(new GpuCommandBufferMsg_FetchGpuFence(route_id_, fetch_id));
  // LOG(INFO) << ";;; waiting for response at key=" << release_count;
  fetch_gpu_fence_tasks_.insert(
      std::make_pair(fetch_id, callback));
}

void CommandBufferProxyImpl::PlaceGpuFence(const gfx::GpuFenceHandle& handle) {
  TRACE_EVENT0("gpu", __FUNCTION__);
  // LOG(INFO) << __FUNCTION__ << ";;;";
  CheckLock();
  base::AutoLock lock(last_state_lock_);
  ...
  Send(new GpuCommandBufferMsg_PlaceGpuFence(route_id_, handle_for_ipc));
}

void CommandBufferProxyImpl::OnFetchGpuFenceComplete(
    uint32_t fetch_id,
    const GpuCommandBufferMsg_FetchGpuFence_Return& ret) {
  // LOG(INFO) << __FUNCTION__ << ";;; start, fd=" << ret.sync_fd.fd << " key="
  // << sync_token.release_count();
  FetchGpuFenceTaskMap::iterator it = fetch_gpu_fence_tasks_.find(fetch_id);
  if (it == fetch_gpu_fence_tasks_.end()) {
  ...
  fetch_gpu_fence_tasks_.erase(it);
