...
class BBJSONGenerator(object):
  ...
  def generate_abs_file_path(self, path):
  ...
  def load_pyl_file(self, filename):
    try:
      return ast.literal_eval(self.read_file(self.generate_abs_file_path(
        filename)))
    except (SyntaxError, ValueError) as e:
      raise Exception('Failed to parse pyl file "%s": %s' % (filename, e))
  ...
  def generate_gtests(self, waterfall, tester_name, tester_config,
    ...
    for test_name, test_config in sorted(test_dictionary.iteritems()):
      test = self.generate_gtest(waterfall, tester_name, tester_config,
                                 test_name, test_config, exceptions)
      if test:
  ...
  def generate_isolated_script_tests(self, waterfall, tester_name,
                                     tester_config, test_dictionary,
                                     exceptions):
    isolated_scripts = []
    for test_name, test_config in sorted(test_dictionary.iteritems()):
      test = self.generate_isolated_script_test(
        waterfall, tester_name, tester_config, test_name, test_config,
        exceptions)
      if test:
  ...
  def generate_script_tests(self, waterfall, tester_name, tester_config,
                            test_dictionary, exceptions):
    scripts = []
    for test_name, test_config in sorted(test_dictionary.iteritems()):
      test = self.generate_script_test(
        waterfall, tester_name, tester_config, test_name, test_config,
        exceptions)
      if test:
        scripts.append(test)
  ...
  def generate_junit_tests(self, waterfall, tester_name, tester_config,
                           test_dictionary, exceptions):
    scripts = []
    for test_name, test_config in sorted(test_dictionary.iteritems()):
      test = self.generate_junit_test(waterfall, tester_name, tester_config,
                                      test_name, test_config, exceptions)
      if test:
        scripts.append(test)
    return scripts
  ...
  def generate_instrumentation_tests(self, waterfall, tester_name,
                                     tester_config, test_dictionary,
                                     exceptions):
    scripts = []
    for test_name, test_config in sorted(test_dictionary.iteritems()):
      test = self.generate_instrumentation_test(
        waterfall, tester_name, tester_config, test_name, test_config,
        exceptions)
      if test:
        scripts.append(test)
  ...
  def generate_cts_tests(self, test_dictionary):
  ...
  def generate_waterfall_json(self, waterfall, filename, exceptions):
    ...
    for name, config in waterfall['testers'].iteritems():
      ...
      for test_type, input_tests in test_suites.iteritems():
        if test_type == 'gtests':
          tests['gtest_tests'] = sorted(self.generate_gtests(
            waterfall, name, config, input_tests, exceptions),
                                        cmp=self.cmp_gtests)
        elif test_type == 'isolated_scripts':
          tests['isolated_scripts'] = sorted(
            self.generate_isolated_script_tests(
              waterfall, name, config, input_tests, exceptions),
            key=lambda x: x['name'])
        elif test_type == 'scripts':
          tests['scripts'] = sorted(self.generate_script_tests(
            waterfall, name, config, input_tests, exceptions),
                                    key=lambda x: x['name'])
        elif test_type == 'junit_tests':
          tests['junit_tests'] = sorted(self.generate_junit_tests(
            waterfall, name, config, input_tests, exceptions),
                                        key=lambda x: x['test'])
        elif test_type == 'cts_tests':
          tests['cts_tests'] = self.generate_cts_tests(input_tests)
        elif test_type == 'instrumentation_tests':
          tests['instrumentation_tests'] = sorted(
            self.generate_instrumentation_tests(
              waterfall, name, config, input_tests, exceptions),
            key=lambda x: x['test'])
        else:
    ...
    with open(self.generate_abs_file_path(filename), 'wb') as fp:
      # TODO(kbr): once all waterfalls are autogenerated, delete the
      # output_order lists and related code.
      json.dump(out_waterfall, fp, indent=2, separators=(',', ': '),
                sort_keys=True)
      fp.write('\n')

  def link_waterfalls_to_test_suites(self, waterfalls, test_suites):
  ...
  def resolve_composition_test_suites(self, test_suites):
  ...
  def main(self):
    waterfalls = self.load_pyl_file('waterfalls.pyl')
    test_suites = self.load_pyl_file('test_suites.pyl')
    self.resolve_composition_test_suites(test_suites)
    exceptions = self.load_pyl_file('test_suite_exceptions.pyl')

    self.link_waterfalls_to_test_suites(waterfalls, test_suites)
