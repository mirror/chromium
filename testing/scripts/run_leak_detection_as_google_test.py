...
def main():
  ...
  my_histo = "tools/perf/histograms.json"
  my_chart = "tools/perf/chart.json"
  my_out = "tools/perf/out.json"
  ...
  with open(my_out, 'w') as myo:
    json.dump(json_test_results, myo)
  with open(my_histo, 'w') as my:
    json.dump(perf_results, my)
  print("---------------------")
  return rc
...
def run_benchmark(args, rest_args):
  ...
  try:
      ...
      for item in perf_results:
        if "type" in item and "values" in item:
          if len(item["values"]) > 0 :
            key = item["guid"]
            url = item["values"][0]
            guid_url_map[key] = {"url":url, "is_leaking":False}

        if "name" in item:
          # assert item["sampleValues"] && item["sampleValues"].length == 1
          print item["sampleValues"][0]
          if item["sampleValues"][0] > 0 :
            guid = item["diagnostics"]["stories"]
            if guid_url_map[guid]["is_leaking"] == False:
              guid_url_map[guid]["is_leaking"] = True
              leaking_websites.append(guid_url_map[guid]["url"])
    ...
    with open(tempfile_name) as f:
      # here modify the results
      json_test_results = json.load(f)
      num_failures = json_test_results['num_failures_by_type'].get('FAIL', 0)
      num_passes = json_test_results['num_failures_by_type'].get('PASS', 0)
      for website in leaking_websites:
        if json_test_results["tests"]["memory.leak_detection"][website]["actual"] == "PASS"
          json_test_results["tests"]["memory.leak_detection"][website]["actual"] = "FAIL"
          json_test_results["tests"]["memory.leak_detection"][website]["is_unexpected"] = True
          num_failures += 1
          num_passes -= 1
          json_test_results["num_failures_by_type"]["FAIL"] = num_failures
          json_test_results["num_failures_by_type"]["PASS"] = num_passes
