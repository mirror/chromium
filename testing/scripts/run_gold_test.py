...
def main():
  ...
  if not rc:
    logging.warning("All tests passed, skipping uploading process")
    return rc
  ...
  for filename in os.listdir(IMAGE_OUTPUT_DIR):
    ...
    if not _UploadToStorage(os.path.join(IMAGE_OUTPUT_DIR, filename),
        digest + ".png", args.image_bucket):
      raise RuntimeError('Failed to upload %s to google storage, aborting' %
          filename)

  with open('./gold_output.json', 'w') as out_json:
    json.dump(json_results, out_json)

  if not _UploadToStorage(os.path.join(os.getcwd(), 'gold_output.json'),
