...
INPUT_GOLDEN_HASH_FILENAME = 'golden_hashes.json'
...
def _GetKnownHashesSet(args, output_dir):
  ...
  elif not _DownloadFromStorage(known_hashes_path, INPUT_KNOWN_HASH_FILENAME,
      args.hash_bucket):
...
def main():
  ...
  try:
    ...
    extra_flags.append('--test-launcher-retry-limit=0')
    ...
    for filename in os.listdir(output_dir):
      if not filename.endswith('.png'):
        continue
      # Split the name to get the test name and image hash
