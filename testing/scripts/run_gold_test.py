...
def _CreateArgParser():
  ...
  parser.add_argument('--patch-issue', type=int,
                      help='The Gerrit issue for the CL being tested. Required '
                           'for trybot/CQ runs only.')
  parser.add_argument('--patch-set', type=int,
  ...
  parser.add_argument('--buildername',
                      help='The name of the builder that the test was compiled'
                           '/triggered from.')
  parser.add_argument('--mastername',
                      help='The name of the master the builder belongs to.')
  parser.add_argument('--buildnumber', type=int,
                      help='The build number of the builder.')
  def json_key(arg):
    split_arg = arg.split(',')
    if len(split_arg) != 2:
      raise argparse.ArgumentError(
          'Expected two comma-separated strings for --json-key, received %d.' %
          len(split_arg))
    return {split_arg[0]: split_arg[1]}
  # argparse doesn't support updating dictionaries as an action, so keep each
  # key/value as separate dictionaries for now.
  parser.add_argument('--json-key', action='append', type=json_key, default=[],
                      help='A comma-separated key/value pair to add to the '
                           '"key" dictionary in the JSON results that are '
                           'uploaded to Gold. Addition of multiple pairs is '
                           'allowed.')
...
def _ParseAndAssertValid(parser):
  ...
  if not args.disable_uploads:
    if (not args.image_bucket or not args.json_bucket or not args.git_revision
        or not args.source_type or not args.buildername
        or not args.buildnumber):
      raise ValueError(
          '--image-bucket, --json-bucket, --source-type, --git-revision, '
          '--buildername, and --buildnumber '
...
def _GenerateBaseJsonResults(args):
  ...
  # build_number, issue, and patchset must be strings for historical reasons
  json_results = {
    'builder': args.buildername if args.buildername else 'unknown',
    'build_number': str(args.buildnumber) if args.buildnumber else '0',
    'gitHash': args.git_revision,
    'issue': str(args.patch_issue) if args.patch_issue else '0',
    'master': str(args.mastername) if args.mastername else 'unknown',
    'patchset': str(args.patch_set) if args.patch_set else '0',
  ...
  # Apply any keys provided via the command line
  for key_dict in args.json_key:
    json_results['key'].update(key_dict)
...
def main():
  ...
  try:
    ...
    with open(output_json_file_path, 'w') as out_json:
      json.dump(json_results, out_json, indent=2)

    if args.disable_uploads:
      logging.debug('Uploads disabled, not uploading JSON output')
    else:
      # TODO(bsheedy): Use something other than path.join? Don't know if this
      # will work on Windows
      # Create a YYYY/MM/DD/HH/revision/buildername/buildnumber subdirectory
      gs_path = os.path.join(args.json_bucket,
                             time.strftime("%Y/%m/%d/%H", time.gmtime()),
                             args.git_revision,
                             args.buildername,
