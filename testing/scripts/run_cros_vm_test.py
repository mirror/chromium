...
def main_run(args):
  board = os.environ.get('SDK_BOARD')
  if board:
    assert board in ('amd64-generic', 'betty')
    build_dir = os.path.join(common.SRC_DIR, 'out_%s' % board, 'Release')
    cache_dir_args = []
  else:
    build_dir = os.path.join(common.SRC_DIR, 'out', 'Release')
    cache_dir_args = [
        '--cache-dir',
        os.path.join(common.SRC_DIR, 'build', 'chromeos')
    ]
