...
def CheckBinaryAndArgs(executable_path, filters):
  ...
  if len(data) > LARGE_BINARY_THRESHOLD and not filters:
    logging.warning('The target binary is quite large. Generation of full '
                    'coverage report may take for a while. To generate report '
                    'faster, consider using "--filter" argument to specify '
                    'source code files and directories to be shown in the '
                    'report.')
...
def main():
  ...
  parser.add_argument(
      ...
      nargs='+',
      default = [],
      help='(Optional) Paths to source code files/directories to be shown in '
      'the report. By default, the report shows all the sources compiled and '
