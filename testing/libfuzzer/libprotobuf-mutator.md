...
your message into raw data. Otherwise you will (probably) end up with an out of 
memory error. The code coverage benefits of allowing unlimited recursion in a 
message are probably fairly low for most targets.
...
old coverage of fuzzer on the existing corpus.

* Make sure you understand the meaning of the different protobuf modifiers such 
as oneof and repeated as they can be counter-intuitive. oneof means "At most one
of" while repeated means "At least zero". You can hack around these with
meanings if you need at least one of or exactly one of something. For example this
is the proto code for exactly one of: "MessageA" or "MesageB" or "MessageC":
