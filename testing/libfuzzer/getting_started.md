...
stacktrace, make sure that you have `llvm-symbolizer` available in `$PATH`. The
symbolizer binary is included in Chromium's Clang package located at
`third_party/llvm-build/Release+Asserts/bin/` directory.

Alternatively, you may specify `external_symbolizer_path` option via
