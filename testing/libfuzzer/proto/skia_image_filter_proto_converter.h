...
class Converter {
 ...
 private:
  ...
  // too small as a seperate case from making it too big.
  ...
  // BoundNum will only return positive numbers when true.
  bool bound_positive_;
  uint32_t icc_base_;
  bool in_compose_color_filter_;

  // Reset any state used by the object so that is can be used to convert
  // another proto input.
  void Reset();
  ...
  // any proto Message, though this should not be done for correctness purposes.
  ...
  // method is intended for those messages. Note that this method is viral in
  // that any fields on msg that contain other messages will only be written
  // using this method and WriteFields. For example, "DistantLight" has a field
  // "direction" that contains a "Point3" message.  This method is OK to call on
  // "DistantLight" messages because it is OK to call this method on
  // "Point3". In essence, it is only correct to call this method on msg if it
  // is correct to call this method on any fields (or fields of fields etc.) of
  // msg that are also Messages. See the file comment on skia_image_filter.proto
  // for an explanation of how this and WriteFields are used for autovisit.
  ...
  // and size) and correctly Visiting it. Because this is a templated function
  // it can call the proper Visit function rather than the one for generic
  ...
  // Returns false if there is too much nesting.
  // Write name and reserve a space to write the size of the object.
  // Also increments obj_depth_.
  bool PreVisitFlattenable(const std::string& name);

  // Writes the size of the flattenable to the reserved space, ensures that
  // output_ is four byte aligned and then decrements obj_depth_.
  ...
  // WriteFields(msg2) will be called (assuming that fiels is in the range of
  // msgs we are writing). If there is a method Visit(const Message2& msg2), it
  // will not be called because there is no simple way to determine the type of
  // msg2 using protobuf's reflection API. WriteFields may bound any numeric
  // fields before writing them to avoid OOMs and timeouts. See the file comment
  // on skia_image_filter.proto for an explanation of how this and the generic
  // Visit function are used for autovisit. Note that this may write invalid
  // enum values instead of the ones provided if dont_mutate_enum_ is true.
  // This method may not work if the max field of msg has number that is
  // different than the number of fields. For example, if msg does not have a
  // field with field number 1, but has fields with field numbers 2 and 3,
  // calling WriteFields(msg, 2) will not write field 3.
  ...
  void AppendAsSkPoint(std::vector<SkPoint>&, const Point&) const;
  template <typename T>
  ...
  // Write the enum value described by field descriptor, or write an invalid
  // enum value with some probability if dont_mutate_enums_ is false.
  ...
  // Wrapper for stdlib.h's abs, which has undefined behavior when given
  // INT_MIN.
