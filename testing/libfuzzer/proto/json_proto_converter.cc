...
// // Create a 4KB string to avoid frequent re-allocations during conversion.
// static const size_t kDefaultStringLength = 4096;

// JsonProtoConverter::JsonProtoConverter : data_(kDefaultStringLength, '\0') {};
...
void JsonProtoConverter::AppendNumber(const NumberValue& number_value) {
  if (number_value.has_integer_value()) {
    data_ << number_value.integer_value().value();
  } else if (number_value.has_float_value()) {
