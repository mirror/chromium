...
// is not in enum).
// Also note that the skia unflattening code is necessary to apply the output of
// the converter to a canvas, but it isn't the main target of the fuzzer. This
// means that we will generally try to produce output that can be applied to a
// canvas, even if we will consequently be unable to produce outputs that allow
// us to reach paths in the unflattening code (in particular, code that handles
// invalid input). We make this tradeoff because the filtering code looks far
// more likely to contain bugs than the error handling code in the unflattening
// Thus, increasing the chance that a filter is applied is more important than
// hitting all paths in unflattening. The mutated enum values are a case where
// we knowingly generate output that may not be unflattened successfuly, which
// is why we mutate enums relatively infrequently.
// Since this is a work in progress and skia serialization is a moving target,
// not everything is finished. Many of these parts of the code are #defined
// out if DEVELOPMENT is not defined.
...
// if the object is visited.
// Note that `bool flattenable_visited` must be defined false in a context
// before this is called for the first time in the context.
...
const int Converter::kObjDepthLimit = 10;
...
const uint8_t Converter::kEnumMutationDistributionUpperBound = 40;

// Does not include SkSumPathEffect, SkComposePathEffect or SkRegion
// since they don't use VISIT macros.
...
const std::set<std::string> Converter::kMisbehavedObjectBlacklist = {
    "matrix_convolution_image_filter",  // Too crashy.
    "matrix_image_filter",              // OOMs.
    "discrete_path_effect",             // Slow.
    "path_1d_path_effect",              // Timeouts.
};
...
// function.
Converter::Converter() {
  CHECK_GT(kEnumMutationDistributionUpperBound, 2);
}
...
// flattenable skia object it represents.
...
std::string Converter::Convert(const Input& input) {
  ...
  enum_mutator_chance_distribution_ =
      std::uniform_int_distribution<>(1, kEnumMutationDistributionUpperBound);
...
// Bound the points making up a rectangle so that the returned tuple is a valid
// rectangle.
...
// Bound a float |num| using |num_bound| so that it won't cause OOMs or
// timeouts.
...
void Converter::Visit(const LocalMatrixImageFilter& local_matrix_image_filter) {
  // TODO(metzman): Make it so that deserialization always succeeds by making
  // the type neither kAffine_Mask or KPerspectiveMask (see constructor for
...
void Converter::Visit(const ImageSource& image_source) {
  ...
  // See SkImageSource::Make for why mandate width and height be at least .01.
  // This is such a small difference that we won't bother bounding again.
...
void Converter::Visit(const ArithmeticImageFilter& arithmetic_image_filter) {
  ...
  // This is field is ignored, but write kSrcOver as the flattening code does.
  // TODO(metzman): change to enum value (SkBlendMode::kSrcOver) when it is
  // uncommented, for now just write, its value: 3.
  WriteNum(3);
...
// Reserves space to write the size of a skia flattenable and records info so
// that WriteBytesWritten can determine the size. Every call to this method
// should have a corresponding call to WriteBytesWritten.
...
// Get the size of a skia flattenable that was just written and insert it at the
...
// proto Message, though this should not be done for correctness purposes.
...
// method is intended for those messages. Note that this method is viral in that
// any fields on |msg| that contain other fields will only be written using this
...
void Converter::Visit(const LooperChild& looper) {
  VisitFlattenable(looper.layer_draw_looper(), "SkLayerDrawLooper");
}
...
// entire skia flattenable (including name and size) unlike most Visit functions
// and thus should not be called from VisitFlattenable.
...
// TODO(metzman): Finish implementing ICC color profiles.
#ifdef DEVELOPMENT
...
// Handles writing a skia flattenable by writing its header (|flattenable_name|
// and size) and correctly Visiting it. Because this is a templated function, it
// can call the proper Visit function rather than the one for generic
// Messages. Note that VisitFlattenable may not write an flattenable if the
// level of nested flattenables (|obj_depth_|) exceeds
// |kObjDepthLimit|. VisitFlattenable updates |obj_depth_| properly.
template <class T>
void Converter::VisitFlattenable(const T& flattenable,
                                 const std::string& name) {
  if (PreVisitFlattenable(name)) {
    Visit(flattenable);
    PostVisitFlattenable();
  }
}
...
// autovisit. This method may not work if the max field of msg has number that
// is different than the number of fields. For example, if msg does not have a
// field with field number 1, but has fields with field numbers 2 and 3, calling
// WriteFields(msg, 2) will not write field 3.
...
// Write an enum value, or write an invalid value every once in a while.
void Converter::WriteEnum(const Message& msg,
  ...
  // If we are trying to do less than the min value, and it is 0, just do higher
...
void Converter::Visit(const Vertices& vertices) {
  ...
  // picture image filter. Since this the only time our fuzzer can deserialize
  // it, we always write the size.
  RecordSize();
...
bool Converter::IsBlacklisted(const std::string& field_name) const {
  ...
  return kMisbehavedObjectBlacklist.find(field_name) !=
         kMisbehavedObjectBlacklist.end();
#endif
}
};  // namespace skia_image_filter_proto_converter
