...
std::string protobuf_to_string(const protobuf_mutator::url::Url& url) {
  ...
  // As explained in url.proto, if libFuzzer included a port in url ensure that
  // it is preceded by a ":".
  if (url.has_port())
    // Convert url.port() from an unsigned 32 bit int before appending it.
    url_string += ":" + std::to_string(url.port());

  // Paths must be started by a "/". If libFuzzer included a path in url, ensure
  // that it begins with a "/".
  if (url.has_path())
    url_string += "/" + url.path();

  // Queries must be started by a "?". If libFuzzer included a query in url,
  // ensure that it is preceded by "?".
  if (url.has_query())
    url_string += "?" + url.query();

  // Fragments must be started by a "#". If libFuzzer included a fragment
  // in url, ensure that it is preceded by a "#".
  if (url.has_fragment())
...
// for fuzzing.
DEFINE_BINARY_PROTO_FUZZER(const protobuf_mutator::url::Url& url_protobuf) {
