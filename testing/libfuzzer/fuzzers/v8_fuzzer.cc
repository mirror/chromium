...
// a request for an extremely large (100 Mb) array buffer is made. This is done
// to avoid OOMs. We could be less cautious and allow 100 Mb array buffers since
// they don't inherently cause OOMs (which will happen at 2048 Mb), but that
// would required handling concurrent allocations properly.
class MockArrayBufferAllocator : public v8::ArrayBuffer::Allocator {
  std::unique_ptr<Allocator> allocator_ =
      std::unique_ptr<Allocator>(NewDefaultAllocator());

  const size_t kAllocationLimit = 100 * 1024 * 1024;
 ...
 public:
  ...
  void* AllocateUninitialized(size_t length) override {
    if (length > kAllocationLimit) {
      return nullptr;
    }
    return malloc(length);
  }

  void Free(void* ptr, size_t length) override { free(ptr); }
