...
Corpus is shared across fuzzer runs and grows over time as new code is reached.

Following things are extremely effective for improving fuzzer efficiency, so we
...
of inputs. Most fuzzers need a seed corpus with a few inputs to work at all.

For example, a graphics library seed corpus would a variety of small PNG/JPG/GIF
files.

If you are running the fuzzer locally, you can pass a corpus directory as argument:

```
./out/libfuzzer/my_fuzzer ~/tmp/my_fuzzer_corpus
```

The directory can be initially empty. The fuzzer would store all the interesting
items it finds in that directory. You can help the fuzzer by *seeding* the corpus,
i.e. simply copy interesting inputs for your function to the corpus directory
before running. This works especially well for strictly defined file formats and
...
to find in the input. Otherwise, a fuzzer would guess these words from scratch,
which can take an indefinite amount of time depending on the word size. Adding
a dictionary highly improves the efficiency of finding new units and works
especially well in certain usecases (e.g. fuzzing file format decoders).

To add a dictionary, first create a dictionary file. Dictionary syntax is similar to the
one used by [AFL] fuzzing engine for its -x option:
...
kw3="\xF7\xF8"
# Name of the keyword before '=' can be omitted:
"foo\x0Abar"
```
...
Try to keep your fuzzing function as simple as possible. Making the fuzzing function
complex can cause you to target very specific usecases and can miss catching
the unexpected ones.

Prefer to use static initialization and shared resources rather than bringing the
environment up and down on every single run. You can also initialize resources in
`LLVMFuzzerInitialize` function. Otherwise, it will slow down overall fuzzer speed
and its ability to find new interesting inputs.
...
(see [custom options](#Custom-Options)). This parameter can significantly
improve execution speed, but not always. Beware that this also restricts the
size of your test input, so you might miss coverage and unexpected scenarios
happening from longer size inputs. 
