...
#
# Note: this is a one-time script to migrate accessibility enums from
# IDL to mojom. See http://crrev.com/c/877417 for the refactoring, which
# has more context.
#
# To use this script to update an existing change:
#
# 1. Sync to HEAD, after the accessibility enum refactoring change has
#    landed.
# 2. Patch in your own change.
# 3. Check out ui/accessibility/ax_enums.idl from before the refactoring.
#    This script uses that IDL file to generate the .mojom file and
#    figure out what to rename in existing files. Don't check in the IDL
#    file, just use it to run this script.
# 4. Run this script in your Chromium src directory.
# 5. The script should output a list of files that it modified. Make sure
#    it seems reasonable, and manually inspect the results.
#
...
for line in open(SRC).readlines():
  ...
  if m:
    ...
    lookup['%s_NONE' % std_prefix] = 'ax::mojom::%s::kNone' % new_enum_name
    print '%s_NONE -> ax::mojom::%s::kNone' % (std_prefix, new_enum_name)

    lookup['%s_LAST' % std_prefix] = 'ax::mojom::%s::kLast' % new_enum_name
    print '%s_LAST -> ax::mojom::%s::kLast' % (std_prefix, new_enum_name)
  ...
  if line == '};':
    ...
    tostring_cc.write('    case ax::mojom::%s::kNone:\n' % new_enum_name)
    tostring_cc.write('      return "none";\n')
    tostring_cc.write(tostring_fn)
    tostring_cc.write('  }\n')
    tostring_cc.write('\n')
    tostring_cc.write('  return "";\n')
    tostring_cc.write('}\n')
    tostring_cc.write('\n')
    ...
    tostring_cc.write('  if (0 == strcmp(%s, "none"))\n' %
                      CamelToLowerHacker(new_enum_name))
    tostring_cc.write('    return ax::mojom::%s::kNone;\n' % new_enum_name)
  ...
  if new_enum_name:
    ...
    if m:
      ...
      token = m.group(1)
      new_token = 'k' + ToUpperCamel(token)
      #if new_token == 'TRUE':
      ...
      if new_token == 'kActivedescendantchanged':
        new_token = 'kActiveDescendantChanged'
