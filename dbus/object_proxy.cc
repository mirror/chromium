...
ObjectProxy::ReplyCallbackHolder::ReplyCallbackHolder(
    ReplyCallbackHolder&& other)
    : origin_task_runner_(other.origin_task_runner_),
...
ObjectProxy::ReplyCallbackHolder::~ReplyCallbackHolder() {
  if (callback_.is_null()) {
    ...
    // ReleaseCallback(). Do nothing here.
    return;
  }
  ...
  // The |origin_task_runner_| may already stop. E.g., on Chrome's shutdown
  // the message loop of the UI thread (= the origin thread) stops before
  // D-Bus threaed's. In such a case, PostTask() fails. Because we cannot do
  // much thing here, instead, simply leak the callback rather than destroying
  // it on the D-Bus thread, which could be unexpected from the direct or
  // indirect caller of CallMethod.
...
struct ObjectProxy::OnPendingCallIsCompleteData {
  ...
  static void Run(DBusPendingCall* pending_call, void* user_data) {
    auto* data = static_cast<OnPendingCallIsCompleteData*>(user_data);
    data->object_proxy->OnPendingCallIsComplete(
        pending_call, std::move(data->callback_holder), data->start_time);
  }

  // Deletes given void* value assuming it is a pointer to this class.
  static void Delete(void* user_data) {
    delete static_cast<OnPendingCallIsCompleteData*>(user_data);
  }
};
...
void ObjectProxy::StartAsyncMethodCall(int timeout_ms,
  ...
  OnPendingCallIsCompleteData* data = new OnPendingCallIsCompleteData{
      this, std::move(callback_holder), start_time};

  // This returns false only when unable to allocate memory.
  const bool success = dbus_pending_call_set_notify(
      pending_call, &OnPendingCallIsCompleteData::Run, data,
      &OnPendingCallIsCompleteData::Delete);
