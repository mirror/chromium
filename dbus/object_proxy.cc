...
// Pending callback which will be resumed on D-Bus call completion.
class PendingCall {
 public:
  using ResumeCallback =
      base::OnceCallback<void(DBusPendingCall* pending_call)>;

  explicit PendingCall(ResumeCallback callback)
      : callback_(std::move(callback)) {}

  // Assuming |user_data| is the PendingCall instance, resumes the following
  // call
  static void Resume(DBusPendingCall* pending_call, void* user_data) {
    std::move(static_cast<PendingCall*>(user_data)->callback_)
        .Run(pending_call);
  }

  // Deletes the given |user_data| assuming it is PendingCall instance.
  static void Delete(void* user_data) {
    delete static_cast<PendingCall*>(user_data);
  }

 private:
  ResumeCallback callback_;
  DISALLOW_COPY_AND_ASSIGN(PendingCall);
};
...
ObjectProxy::ReplyCallbackHolder::~ReplyCallbackHolder() {
  if (callback_.is_null() || !origin_task_runner_) {
...
void ObjectProxy::StartAsyncMethodCall(int timeout_ms,
  ...
  PendingCall* pending_call = new PendingCall(
      base::BindOnce(&ObjectProxy::OnPendingCallIsComplete, this,
                     std::move(callback_holder), start_time));

  // This returns false only when unable to allocate memory.
  const bool success =
      dbus_pending_call_set_notify(dbus_pending_call, &PendingCall::Resume,
                                   pending_call, &PendingCall::Delete);
