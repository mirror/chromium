...
ChannelPosix::ChannelPosix(Mode mode,
                           Listener* listener)
    : ChannelReader(listener),
      ...
      pipe_name_("thisisatempchannelnamefortesting"),
      in_dtor_(false),
      must_unlink_(false) {
  if (!CreatePipe()) {
    // The pipe may have been closed already.
    const char *modestr = (mode_ & MODE_SERVER_FLAG) ? "server" : "client";
    LOG(WARNING) << "Unable to create pipe named \"" << pipe_name_
                 << "\" in " << modestr << " mode";
  }
}
...
bool ChannelPosix::GetAttachments(Message* message) {
  return true;
}
