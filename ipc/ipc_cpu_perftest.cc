...
std::vector<TestParams> GetDefaultTestParams() {
  ...
  list.push_back({144, 20, 10, 5});
  list.push_back({144, 60, 10, 5});
  return list;
}

std::string GetLogTitle(const TestParams& params) {
  return base::StringPrintf("MsgSize_%zu_RndPerSec_%zu_MsgPerRnd_%zu_Dur_%zu",
                            params.message_size, params.frames_per_second,
                            params.messages_per_frame,
...
class PerfCpuLogger {
 public:
  ...
  ~PerfCpuLogger() {
    ...
    base::LogPerfResult(test_name_.c_str(), result, "percent");
  }
...
class ChannelSteadyPingPongListener : public Listener {
 public:
  ...
  void OnHello() {
    cpu_logger_ = std::make_unique<PerfCpuLogger>(GetLogTitle(params_));

    timer_.Start(FROM_HERE, GetFrameTime(params_.frames_per_second), this,
                 &ChannelSteadyPingPongListener::StartPingPong);

    end_timer_.Start(FROM_HERE,
                     base::TimeDelta::FromSeconds(params_.duration_in_seconds),
                     this, &ChannelSteadyPingPongListener::StopPingPong);
  }
  ...
  void StartPingPong() {
    if (sync_) {
      ...
      if (base::TimeTicks::Now() - before >
        ...
        LOG(ERROR) << "The last frame wasn't able to complete on time!";
      }
...
class ChannelSteadyPingPongTest : public IPCChannelMojoTestBase {
 public:
  ...
  void RunPingPongServer(bool sync) {
    ...
    for (const auto& params : params_list) {
      ...
      listener.SetTestParams(params, sync, run_loop.QuitWhenIdleClosure());
...
TEST_F(ChannelSteadyPingPongTest, AsyncPingPong) {
  RunPingPongServer(false);
}

TEST_F(ChannelSteadyPingPongTest, SyncPingPong) {
  RunPingPongServer(true);
}
...
class MojoSteadyPingPongTest : public mojo::edk::test::MojoTestBase {
 ...
 protected:
  void RunPingPongServer(MojoHandle mp, bool sync) {
    sync_ = sync;
  ...
  void OnHello(const std::string& value) {
    ...
    end_timer_.Start(FROM_HERE,
                     base::TimeDelta::FromSeconds(params_.duration_in_seconds),
                     this, &MojoSteadyPingPongTest::StopPingPong);
  }
