...
class ChromiumDepGraph {
    ...
    void collectDependencies(ResolvedConfiguration configuration) {
        configuration.firstLevelModuleDependencies.each { dependency ->
            topLevelIds.add(makeModuleId(dependency.module))
            collectDependenciesInternal(dependency)
        }
    }

    void markAndroidCompatible(ResolvedConfiguration androidConfig) {
    ...
    static class DependencyDescription {
        String group, name, version, type, displayName, description, url
