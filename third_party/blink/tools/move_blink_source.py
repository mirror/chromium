...
class MoveBlinkSource(object):
    ...
    def _update_cpp_includes_in_directories(self, dirs):
        for dirname in dirs:
            ...
            for file_path in files:
                ...
                if file_path.endswith('.h') and file_path.find('/third_party/WebKit/public/') != -1:
    ...
    def _replace_include_path(self, match):
        ...
        # latter, we should update the basenme for a name mapped from an IDL
    ...
    def _update_cpp_includes(self, content):
        pattern = re.compile('#include\\s+"((bindings|core|modules|platform|public|' +
                             'third_party/WebKit/(Source|common|public))/[-_\\w/.]+)"')
    ...
    def _replace_basename_only_include(self, subdir, source_path, match):
        ...
        if source_basename in self._basename_map:
            return '#include "third_party/blink/renderer/%s/%s"' % (subdir, self._basename_map[source_basename])
    ...
    def _update_basename_only_includes(self, content, source_path):
        if not source_path.endswith('.h') or source_path.find('/third_party/WebKit/public/') == -1:
            return
