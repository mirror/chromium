...
""")
    ...
    def _get_local_change_status(self, git):
        ...
        A tuple contains two strings. The first one is file change status such
        as "M", "D".  See --diff-filter section of git-diff manual page.
        ...
        for l in status_lines:
            items = l.split('\t')
            if len(items) == 2:
                status_tuple_list.append((items[0], items[1]))
            elif len(l) > 0:
