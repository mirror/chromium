        ...
        for i in range(0, len(file_pairs)):
            ...
            if self._options.run_git:
                if git.exists(src_from_repo):
                    subprocess.call(['git', 'mv', src_from_repo, dest_from_repo], cwd=self._repo_root)
                    _log.info('[%d/%d] Git moved %s', i + 1, len(file_pairs), src)
                else:
                    _log.info('%s is not in the repository', src)
            else:
                subprocess.call(['mv', src_from_repo, dest_from_repo], cwd=self._repo_root)
