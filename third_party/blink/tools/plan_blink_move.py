...
def relative_dest(fs, fn):
    dest = None
    if fn.startswith('public'):
        dest = re.sub(r'^public', 'renderer' + fs.sep + 'public', fn)
    elif fn.startswith('Source'):
        dest = re.sub(r'^Source', 'renderer', fn)
    elif fn.startswith('common'):
        dest = fn
    else:
        raise ValueError('|fn| must start with "common", "public", or "Source": %s' % fn)
    if fn.endswith('.h') or fn.endswith('.cpp') or fn.endswith('.mm') or fn.endswith('.idl'):
...
def plan_blink_move(fs, prefixes):
    """Returns pairs of a path string relative to third_party/WebKit and a path
    string relative to third_party/blink, with os.sep."""
