...
package org.chromium.third_party.android.util.apk;
...
public class ApkSignatureSchemeV2Verifier {
     ...
     * @throws SignatureException if a APK Signature Scheme v2 signature of this APK does not
     * verify.
     * @throws IOException if an I/O error occurs while reading the APK file.
     */
    public static X509Certificate[][] verify(String apkFile)
            throws SignatureNotFoundException, SignatureException, IOException {
        try (RandomAccessFile apk = new RandomAccessFile(apkFile, "r")) {
            return verify(apk);
        }
    }
     ...
     *
     * @throws SignatureNotFoundException if the APK is not signed using APK Signature Scheme v2.
     * @throws SignatureException if an APK Signature Scheme v2 signature of this APK does not
     *         verify.
     * @throws IOException if an I/O error occurs while reading the APK file.
     */
    ...
    private static class SignatureInfo {
        ...
        private SignatureInfo(ByteBuffer signatureBlock, long apkSigningBlockOffset,
     ...
     */
    private static X509Certificate[][] verify(FileDescriptor apkFileDescriptor,
            SignatureInfo signatureInfo) throws SignatureException {
        ...
        try {
            ...
            throw new SignatureException("Failed to read list of signers", e);
        }
        while (signers.hasRemaining()) {
            try {
                ...
                throw new SignatureException(
                        "Failed to parse/verify signer #" + (signerCerts.size() + 1) + " block", e);
            }
        }

        if (signerCerts.size() < 1) {
            throw new SignatureException("No signers found");
        }

        if (contentDigests.isEmpty()) {
            throw new SignatureException("No content digests found");
        }

        verifyIntegrity(contentDigests, apkFileDescriptor, signatureInfo.apkSigningBlockOffset,
                signatureInfo.centralDirOffset, signatureInfo.eocdOffset, signatureInfo.eocd);

        return signerCerts.toArray(new X509Certificate[signerCerts.size()][]);
    }

    private static X509Certificate[] verifySigner(ByteBuffer signerBlock,
            Map<Integer, byte[]> contentDigests, CertificateFactory certFactory)
        ...
        while (signatures.hasRemaining()) {
            ...
            try {
                ...
                if (signature.remaining() < 8) {
                    throw new SignatureException("Signature record too short");
                }
        ...
        if (bestSigAlgorithm == -1) {
            if (signatureCount == 0) {
                ...
                throw new SignatureException("No supported signatures found");
            }
        }
        ...
        if (!sigVerified) {
            throw new SignatureException(jcaSignatureAlgorithm + " signature did not verify");
        }
        ...
        while (certificates.hasRemaining()) {
            ...
            try {
                ...
                throw new SignatureException(
                        "Failed to decode certificate #" + certificateCount, e);
            }
        ...
        if (certs.isEmpty()) {
            throw new SignatureException("No certificates listed");
        }
    ...
    private static void verifyIntegrity(Map<Integer, byte[]> expectedDigests,
            FileDescriptor apkFileDescriptor, long apkSigningBlockOffset, long centralDirOffset,
            long eocdOffset, ByteBuffer eocdBuf) throws SignatureException {
        if (expectedDigests.isEmpty()) {
            throw new SignatureException("No digests provided");
        }
        ...
        DataSource centralDir = new MemoryMappedFileDataSource(
        ...
        byte[][] actualDigests;
        try {
            actualDigests = computeContentDigests(
    ...
    private static byte[][] computeContentDigests(int[] digestAlgorithms, DataSource[] contents)
        ...
        for (int i = 0; i < digestAlgorithms.length; i++) {
            ...
            setUnsignedInt32LittleEndian(
                    totalChunkCount, concatenationOfChunkCountAndChunkDigests, 1);
        ...
        for (DataSource input : contents) {
            ...
            while (inputRemaining > 0) {
                ...
                try {
                    ...
                    throw new DigestException("Failed to digest chunk #" + chunkIndex
                                    + " of section #" + dataSourceIndex,
                            e);
                }
                for (int i = 0; i < digestAlgorithms.length; i++) {
                    int digestAlgorithm = digestAlgorithms[i];
                    ...
                    if (actualDigestSizeBytes != expectedDigestSizeBytes) {
                        throw new DigestException("Unexpected output size of " + md.getAlgorithm()
    ...
    private static long getCentralDirOffset(ByteBuffer eocd, long eocdOffset)
        ...
        if (centralDirOffset > eocdOffset) {
            throw new SignatureNotFoundException("ZIP Central Directory offset out of range: "
                    + centralDirOffset + ". ZIP End of Central Directory offset: " + eocdOffset);
        }
    ...
    private static int getSignatureAlgorithmContentDigestAlgorithm(int sigAlgorithm) {
        switch (sigAlgorithm) {
            ...
            default:
                throw new IllegalArgumentException(
                        "Unknown signature algorithm: 0x" + Integer.toHexString(sigAlgorithm));
        }
    }
    ...
    getSignatureAlgorithmJcaSignatureAlgorithm(int sigAlgorithm) {
        switch (sigAlgorithm) {
            case SIGNATURE_RSA_PSS_WITH_SHA256:
                return Pair.create("SHA256withRSA/PSS",
                        new PSSParameterSpec(
                                "SHA-256", "MGF1", MGF1ParameterSpec.SHA256, 256 / 8, 1));
            case SIGNATURE_RSA_PSS_WITH_SHA512:
                return Pair.create("SHA512withRSA/PSS",
                        new PSSParameterSpec(
    ...
    private static Pair<ByteBuffer, Long> findApkSigningBlock(RandomAccessFile apk,
            long centralDirOffset) throws IOException, SignatureNotFoundException {
        // FORMAT:
    ...
    private static final class MemoryMappedFileDataSource implements DataSource {
        ...
        @Override
        public void feedIntoMessageDigests(MessageDigest[] mds, long offset, int size)
                throws IOException {
            long filePosition = mFilePosition + offset;
            FileChannel fileChannel = new FileInputStream(mFd).getChannel();
            try {
                ByteBuffer buf = fileChannel.map(FileChannel.MapMode.READ_ONLY, filePosition, size);
                for (MessageDigest md : mds) {
                    buf.position(0);
                    md.update(buf);
                }
            } catch (NonReadableChannelException | NonWritableChannelException
                    | IllegalArgumentException e) {
                throw new IOException("Failed to mmap " + size + "bytes", e);
            } finally {
                fileChannel.close();
            }
        }
    ...
    /**
     * This class simply delegates to the given X509Certificate. It exists to isolate the delegation
     * boilerplate so the more interesting behavior in VerbatimX509Certificate can be more easily
     * read.
     */
