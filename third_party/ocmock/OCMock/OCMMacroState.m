...
+ (void)beginStubMacro
{
    ...
    OCMMacroState* macroState =
        [[OCMMacroState alloc] initWithRecorder:recorder];
...
+ (void)beginRejectMacro {
  OCMExpectationRecorder* recorder =
      [[[OCMExpectationRecorder alloc] init] autorelease];
  [recorder never];
...
{
  [[NSThread currentThread].threadDictionary
      removeObjectForKey:OCMGlobalStateKey];
}
...
- (void)dealloc
{
    [recorder release];
    NSAssert(
        [NSThread currentThread].threadDictionary[OCMGlobalStateKey] != self,
        @"Unexpected dealloc while set as the global state");
    [super dealloc];
}
