...
- (void)stopMocking
{
  if (originalMetaClass != nil) {
    /* The mocked class has the meta class of a dynamically created subclass as
       its meta class, but we need a reference to the subclass to dispose it.
       Asking the meta class for its class name returns the actual class name,
       which we can then use to look up the class...
    */
...
{
  /* the runtime and OCMock depend on string and array; we don't intercept
   * methods on them to avoid endless loops */
  if ([[mockedClass class] isSubclassOfClass:[NSString class]] ||
      [[mockedClass class] isSubclassOfClass:[NSArray class]])
    return;
  ...
  object_setClass(
      mockedClass,
      newMetaClass);  // only after dummy initialize is installed (iOS9)
...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)aSelector
{
  NSMethodSignature* signature =
      [mockedClass instanceMethodSignatureForSelector:aSelector];
  if (signature == nil) {
    signature = [NSMethodSignature
        signatureForDynamicPropertyAccessedWithSelector:aSelector
                                                inClass:mockedClass];
  }
  return signature;
}
