...
{
    [recordedInvocation release];
    // Don't do a regular -retainArguments on the invocation that we use for
    // matching. NSInvocation effectively does an strcpy on char* arguments
    // which messes up matching them literally and blows up with anyPointer (in
    // strlen since it's not actually a C string). Also on the off-chance that
    // anInvocation contains self as an argument, -retainArguments would create
    // a retain cycle.
...
- (BOOL)matchesInvocation:(NSInvocation *)anInvocation
{
    ...
    {
      if (ignoreNonObjectArgs &&
          !OCMIsObjectType([signature getArgumentTypeAtIndex:i])) {
        continue;
        }
