...
ScriptValue ReadableStreamOperations::CreateReadableStream(
  ...
  v8::Local<v8::Value> js_underlying_source = ToV8(underlying_source, script_state);
...
bool ReadableStreamOperations::IsReadableStream(ScriptState* script_state,
  ...
  return V8ScriptRunner::CallExtraOrCrash(script_state, "IsReadableStream", args)
      ->ToBoolean()
      ->Value();
}
...
bool ReadableStreamOperations::IsLocked(ScriptState* script_state,
  ...
  v8::Local<v8::Value> args[] = {stream.V8Value()};
  return V8ScriptRunner::CallExtraOrCrash(script_state, "IsReadableStreamLocked",
                                          args)
      ->ToBoolean()
      ->Value();
}
...
bool ReadableStreamOperations::IsClosed(ScriptState* script_state,
  ...
  v8::Local<v8::Value> args[] = {stream.V8Value()};
  return V8ScriptRunner::CallExtraOrCrash(script_state, "IsReadableStreamClosed",
                                          args)
      ->ToBoolean()
      ->Value();
}
...
void ReadableStreamOperations::Tee(ScriptState* script_state,
  ...
  ScriptValue result(script_state, V8ScriptRunner::CallExtraOrCrash(
  ...
  *new_stream1 = ScriptValue(
      script_state, branches->Get(script_state->GetContext(), 0).ToLocalChecked());
  *new_stream2 = ScriptValue(
      script_state, branches->Get(script_state->GetContext(), 1).ToLocalChecked());
