...
class Func : public ScriptFunction {
 public:
  static v8::Local<v8::Function> CreateFunction(ScriptState* script_state,
                                                Iteration* iteration) {
    Func* self = new Func(script_state, iteration);
    return self->BindToV8Function();
  }
 ...
 private:
  Func(ScriptState* script_state, Iteration* iteration)
...
TEST(ReadableStreamOperationsTest, Read) {
  ...
  ReadableStreamOperations::DefaultReaderRead(scope.GetScriptState(), reader)
      .Then(Func::CreateFunction(scope.GetScriptState(), it1),
            NotReached::CreateFunction(scope.GetScriptState()));
  ReadableStreamOperations::DefaultReaderRead(scope.GetScriptState(), reader)
      .Then(Func::CreateFunction(scope.GetScriptState(), it2),
            NotReached::CreateFunction(scope.GetScriptState()));
...
TEST(ReadableStreamOperationsTest,
  ...
  ReadableStreamOperations::DefaultReaderRead(scope.GetScriptState(), reader)
      .Then(Func::CreateFunction(scope.GetScriptState(), it2),
            NotReached::CreateFunction(scope.GetScriptState()));
  ReadableStreamOperations::DefaultReaderRead(scope.GetScriptState(), reader)
      .Then(Func::CreateFunction(scope.GetScriptState(), it3),
            NotReached::CreateFunction(scope.GetScriptState()));
