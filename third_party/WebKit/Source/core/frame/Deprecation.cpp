...
DeprecationInfo GetDeprecationInfo(WebFeature feature) {
  switch (feature) {
    ...
    case WebFeature::kCredentialManagerCredentialRequestOptionsUnmediated:
      return {"CredentialManagerCredentialRequestOptionsUnmediated", M62,
    ...
    case WebFeature::kCredentialManagerCustomFetch:
      return {
          "CredentialManager", M62,
          String::Format(
              "Passing 'PasswordCredential' objects into 'fetch(..., { "
              "credentials: ... })' is deprecated, and will be removed in %s. "
              "See "
    ...
    case WebFeature::kCredentialManagerRequireUserMediation:
      return {"CredentialManagerRequireUserMediation", M62,
              replacedWillBeRemoved(
...
void Deprecation::GenerateReport(const LocalFrame* frame, WebFeature feature) {
  ...
  DeprecationReport* body =
      new DeprecationReport(info.id, milestoneString(info.anticipatedRemoval),
  ...
  service->QueueDeprecationReport(
      document->Url(), info.id, milestoneString(info.anticipatedRemoval),
