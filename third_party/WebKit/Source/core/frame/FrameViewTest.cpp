...
class MockChromeClient : public EmptyChromeClient {
 public:
  ...
  void ScheduleAnimation(FrameViewBase*) override { has_scheduled_animation_ = true; }
...
class FrameViewTest
 ...
 protected:
  ...
  void SetUp() override {
    ...
    page_holder_->GetPage().GetSettings().SetAcceleratedCompositingEnabled(true);
  }
...
TEST_P(FrameViewTest, HideTooltipWhenScrollPositionChanges) {
  ...
  EXPECT_CALL(ChromeClient(), MockSetToolTip(GetDocument().GetFrame(), String(), _));
...
TEST_P(FrameViewTest, NoOverflowInIncrementVisuallyNonEmptyPixelCount) {
  ...
  GetDocument().View()->IncrementVisuallyNonEmptyPixelCount(IntSize(65536, 65536));
