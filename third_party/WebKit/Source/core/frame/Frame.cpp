...
void Frame::ConsumeTransientUserActivationInAncestors() {
...
void Frame::ConsumeTransientUserActivationInDescendants() {
  for (Frame* child = Tree().FirstChild(); child;
       child = child->Tree().NextSibling()) {
    child->user_activation_state_.ConsumeIfActive();
    child->ConsumeTransientUserActivationInDescendants();
  }
}

bool Frame::ConsumeTransientUserActivation() {
  ConsumeTransientUserActivationInAncestors();
  ConsumeTransientUserActivationInDescendants();
