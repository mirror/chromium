  ...
  for (const auto& test : cases) {
    ...
    EXPECT_EQ(
        ...
        directive_list->AllowRequestWithoutIntegrity(
            test.context, resource, ResourceRequest::RedirectStatus::kNoRedirect,
            SecurityViolationReportingPolicy::kSuppressReporting));
  }
}
...
TEST_F(CSPDirectiveListTest, OperativeDirectiveGivenType) {
  ...
  for (const auto& test : cases) {
    ...
    for (const auto& subtest : cases) {
      ...
      all_except_this << directive_name << " http://" << directive_name << ".com; ";
      if (subtest.directive != ContentSecurityPolicy::DirectiveType::kChildSrc) {
        all_except_child_src_and_this << directive_name << " http://" << directive_name
                                 << ".com; ";
      }
    ...
    switch (test.type) {
      case kDefault:
        sources = all_except_this_list->OperativeDirective(test.directive)->list_;
        EXPECT_EQ(sources.size(), 1u);
      ...
      case kChildAndDefault:
        ...
        sources =
            all_except_child_src_and_this_list->OperativeDirective(test.directive)
                ->list_;
        EXPECT_EQ(sources.size(), 1u);
        EXPECT_EQ(sources[0]->host_, "default-src.com");
        break;
      case kScriptAndDefault:
        ...
        sources =
            all_except_script_src_and_this_list->OperativeDirective(test.directive)
                ->list_;
        EXPECT_EQ(sources.size(), 1u);
        EXPECT_EQ(sources[0]->host_, "default-src.com");
        break;
...
TEST_F(CSPDirectiveListTest, GetSourceVector) {
  ...
  struct TestCase {
    ...
    int expected_child_src;
      ...
      {ContentSecurityPolicy::DirectiveType::kConnectSrc, kDefault, 3u, 1, 2, 0},
      ...
      {ContentSecurityPolicy::DirectiveType::kManifestSrc, kDefault, 3u, 1, 2, 0},
  ...
  for (const auto& test : cases) {
    ...
    policy_vector.push_back(CreateList("child-src http://child-src.com;",
                                      kContentSecurityPolicyHeaderTypeEnforce));
    udpated_total =
        test.type == kChildAndDefault ||
                test.directive == ContentSecurityPolicy::DirectiveType::kChildSrc
            ? udpated_total + 1
            : udpated_total;
    EXPECT_EQ(
        CSPDirectiveList::GetSourceVector(test.directive, policy_vector).size(),
