...
bool CSPDirectiveList::CheckSourceAndReportViolation(
  ...
  else if (ContentSecurityPolicy::DirectiveType::kNavigationTo ==
           effective_type)
...
bool CSPDirectiveList::AllowNavigationTo(
    const KURL& url,
    ResourceRequest::RedirectStatus redirect_status,
    SecurityViolationReportingPolicy reporting_policy) const {
  return reporting_policy == SecurityViolationReportingPolicy::kReport
             ? CheckSourceAndReportViolation(
                   navigation_to_, url,
                   ContentSecurityPolicy::DirectiveType::kNavigationTo,
                   redirect_status)
             : CheckSource(navigation_to_, url, redirect_status);
}
...
void CSPDirectiveList::AddDirective(const String& name, const String& value) {
  ...
  } else if (type == ContentSecurityPolicy::DirectiveType::kNavigationTo) {
    SetCSPDirective<SourceListDirective>(name, value, navigation_to_);
  } else if (type ==
...
SourceListDirective* CSPDirectiveList::OperativeDirective(
  ...
  switch (type) {
    ...
    case ContentSecurityPolicy::DirectiveType::kNavigationTo:
      return navigation_to_.Get();
...
WebContentSecurityPolicy CSPDirectiveList::ExposeForNavigationalChecks() const {
  ...
  for (const auto& directive :
       {child_src_, default_src_, form_action_, frame_src_, navigation_to_}) {
    if (directive) {
