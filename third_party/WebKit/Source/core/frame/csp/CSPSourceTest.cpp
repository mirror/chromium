...
TEST_F(CSPSourceTest, BasicPathMatching) {
  ...
  CSPSource b(csp.Get(), "http", "example.com", 8000, "", CSPSource::kNoWildcard,
...
TEST_F(CSPSourceTest, FirstSubsumesSecond) {
  ...
  for (const auto& test : cases) {
    ...
    list_b.push_back(new CSPSource(
        csp.Get(), test.source_b.scheme, test.source_b.host, 0, test.source_b.path,
        CSPSource::kNoWildcard, CSPSource::kNoWildcard));
    list_a.push_back(new CSPSource(csp.Get(), test.scheme_a, "second-example.com",
                                  0, "/", CSPSource::kNoWildcard,
                                  CSPSource::kNoWildcard));
