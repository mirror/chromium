...
bool CSPSource::Matches(const KURL& url,
  ...
  return HostMatches(url.Host()) && ports_match != PortMatchingResult::kNotMatching && paths_match;
}
...
bool CSPSource::Subsumes(CSPSource* other) const {
  ...
  if ((host_wildcard_ == kNoWildcard && other->host_wildcard_ == kHasWildcard) ||
      (port_wildcard_ == kNoWildcard && other->port_wildcard_ == kHasWildcard)) {
    return false;
  }

  bool host_subsumes = (host_ == other->host_ || HostMatches(other->host_));
  bool port_subsumes = (port_wildcard_ == kHasWildcard) ||
      PortMatches(other->port_, other->scheme_) != PortMatchingResult::kNotMatching;
...
bool CSPSource::IsSimilar(CSPSource* other) const {
  ...
  bool ports_match = (other->port_wildcard_ == kHasWildcard) ||
      PortMatches(other->port_, other->scheme_) != PortMatchingResult::kNotMatching ||
...
CSPSource* CSPSource::Intersect(CSPSource* other) const {
  ...
  String scheme = other->SchemeMatches(scheme_) != SchemeMatchingResult::kNotMatching ? scheme_ : other->scheme_;
  ...
  String path =
      path_.length() > other->path_.length() ? path_ : other->path_;
