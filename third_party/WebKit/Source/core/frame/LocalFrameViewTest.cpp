...
TEST_P(LocalFrameViewTest, UpdateLifecycleDetachedFrame) {
  SetBodyInnerHTML("<iframe id='iframe' style='display: none'></iframe>");
  SetChildFrameHTML("A");
  GetDocument().View()->UpdateAllLifecyclePhases();

  auto* iframe = GetDocument().getElementById("iframe");
  EXPECT_FALSE(iframe->GetLayoutObject());
  ...
  if (RuntimeEnabledFeatures::RootLayerScrollingEnabled()) {
    ...
    EXPECT_TRUE(child_layout_view->GetFrameView()->PreTranslation());
  }
}
