...
void FrameView::Reset() {
  ...
  tracked_object_paint_invalidations_ = WTF::WrapUnique(
      g_initial_track_all_paint_invalidations ? new Vector<ObjectPaintInvalidation>
                                          : nullptr);
...
void FrameView::ScrollbarManager::SetHasHorizontalScrollbar(bool has_scrollbar) {
  if (has_scrollbar == HasHorizontalScrollbar())
    return;

  if (has_scrollbar) {
    h_bar_ = CreateScrollbar(kHorizontalScrollbar);
    scrollable_area_->GetLayoutBox()->GetDocument().View()->AddChild(h_bar_.Get());
...
void FrameView::ScrollbarManager::SetHasVerticalScrollbar(bool has_scrollbar) {
  ...
  if (has_scrollbar) {
    ...
    scrollable_area_->GetLayoutBox()->GetDocument().View()->AddChild(v_bar_.Get());
...
void FrameView::InvalidateAllCustomScrollbarsOnActiveChanged() {
  ...
  for (const Member<FrameViewBase>& child : *view_children) {
    ...
    } else if (uses_window_inactive_selector && frame_view_base->IsScrollbar() &&
...
bool FrameView::ShouldUseCustomScrollbars(
  ...
  if (doc_element && doc_element->GetLayoutObject() &&
      doc_element->GetLayoutObject()->Style()->HasPseudoStyle(kPseudoIdScrollbar)) {
...
void FrameView::AdjustViewSizeAndLayout() {
  AdjustViewSize();
  if (NeedsLayout()) {
    AutoReset<bool> suppress_adjust_view_size(&suppress_adjust_view_size_, true);
    GetLayout();
  }
}
...
void FrameView::RecalcOverflowAfterStyleChange() {
  ...
  ComputeScrollbarExistence(should_have_horizontal_scrollbar,
                            should_have_vertical_scrollbar, document_rect.size());
...
std::unique_ptr<TracedValue> FrameView::AnalyzerCounters() {
  ...
  value->SetString("host", GetLayoutViewItem().GetDocument().location()->host());
...
void FrameView::PerformLayout(bool in_subtree_layout) {
  ...
  int contents_height_before_layout = GetLayoutViewItem().DocumentRect().Height();
...
FloatSize FrameView::ViewportSizeForViewportUnits() const {
  ...
  if (RuntimeEnabledFeatures::inertTopControlsEnabled() &&
    ...
    if (frame_->IsMainFrame() && layout_size.Width() && viewport_width) {
      ...
      layout_size.Expand(0, browser_controls.Height() / page_scale_at_layout_width);
    }
  }
...
void FrameView::AddBackgroundAttachmentFixedObject(LayoutObject* object) {
  ...
  if (ScrollingCoordinator* scrolling_coordinator = this->GetScrollingCoordinator())
    scrolling_coordinator->FrameViewHasBackgroundAttachmentFixedObjectsDidChange(
        this);
...
void FrameView::ViewportSizeChanged(bool width_changed, bool height_changed) {
  ...
  if (RuntimeEnabledFeatures::inertTopControlsEnabled() && GetLayoutView() &&
      frame_->IsMainFrame() && frame_->GetPage()->GetBrowserControls().Height()) {
  ...
  for (const auto& viewport_constrained_object : *viewport_constrained_objects_) {
...
bool FrameView::HasBackgroundAttachmentFixedDescendants(
  ...
  for (const auto* potential_descendant : background_attachment_fixed_objects_) {
...
void FrameView::DidScrollTimerFired(TimerBase*) {
  if (frame_->GetDocument() && !frame_->GetDocument()->GetLayoutViewItem().IsNull())
    frame_->GetDocument()->Fetcher()->UpdateAllImageResourcePriorities();
}
...
void FrameView::UpdateCompositedSelectionIfNeeded() {
  ...
  LocalFrame* local_frame = (focused_frame && (focused_frame->LocalFrameRoot() ==
...
FrameView::ScrollingReasons FrameView::GetScrollingReasons() const {
  ...
  if (owner &&
      (!owner->GetLayoutObject() || !owner->GetLayoutObject()->VisibleToHitTesting()))
...
bool FrameView::ScrollAnimatorEnabled() const {
  return frame_->GetSettings() && frame_->GetSettings()->GetScrollAnimatorEnabled();
}
    ...
    if (Element* body = doc->body()) {
      if (LayoutObject* layout_object = body->GetLayoutObject())
        corner_style = layout_object->GetUncachedPseudoStyle(
            PseudoStyleRequest(kPseudoIdScrollbarCorner), layout_object->Style());
    }
...
void FrameView::VisualViewportScrollbarsChanged() {
  ...
  scrollbar_manager_.SetHasHorizontalScrollbar(should_have_horizontal_scrollbar);
...

    {
      ...
      if (target_state >= DocumentLifecycle::kPrePaintClean) {
        ...
        if (LocalFrame* local_frame = frame_->LocalFrameRoot()) {
          ...
          GetFrame().GetPage()->GetChromeClient().UpdateEventRectsForSubframeIfNecessary(
              local_frame);
        }
...
void FrameView::ForceLayoutForPagination(const FloatSize& page_size,
  ...
  if (LayoutView* layout_view = this->GetLayoutView()) {
    ...
    bool horizontal_writing_mode = layout_view->Style()->IsHorizontalWritingMode();
...
IntPoint FrameView::ConvertFromContainingFrameViewBase(
  ...
  if (const FrameView* parent_view = ToFrameView(Parent())) {
    ...
    IntPoint point = parent_view->ConvertToLayoutItem(layout_item, parent_point);
    // Subtract borders and padding
    point.Move((-layout_item.BorderLeft() - layout_item.PaddingLeft()).ToInt(),
               (-layout_item.BorderTop() - layout_item.PaddingTop()).ToInt());
    return point;
...
bool FrameView::AdjustScrollbarExistence(
  ...
  ComputeScrollbarExistence(new_has_horizontal_scrollbar, new_has_vertical_scrollbar,
                            ContentsSize(), option);
...
void FrameView::UpdateScrollbars() {
  ...
  for (int update_scrollbars_pass = 0;
       update_scrollbars_pass < max_update_scrollbars_pass; update_scrollbars_pass++) {
...
IntPoint FrameView::ContentsToFrame(const IntPoint& point_in_content_space) const {
...
IntRect FrameView::FrameToContents(const IntRect& rect_in_frame) const {
  return IntRect(FrameToContents(rect_in_frame.Location()), rect_in_frame.size());
}

IntPoint FrameView::RootFrameToContents(const IntPoint& root_frame_point) const {
...
IntRect FrameView::ViewportToContents(const IntRect& rect_in_viewport) const {
  IntRect rect_in_root_frame =
      frame_->GetPage()->GetVisualViewport().ViewportToRootFrame(rect_in_viewport);
  IntRect frame_rect = ConvertFromRootFrame(rect_in_root_frame);
  return FrameToContents(frame_rect);
}

IntPoint FrameView::ViewportToContents(const IntPoint& point_in_viewport) const {
  IntPoint point_in_root_frame =
      frame_->GetPage()->GetVisualViewport().ViewportToRootFrame(point_in_viewport);
...
IntPoint FrameView::ContentsToViewport(const IntPoint& point_in_contents) const {
...
LayoutRect FrameView::ScrollIntoView(const LayoutRect& rect_in_content,
  ...
  LayoutRect expose_rect =
      ScrollAlignment::GetRectToExpose(view_rect, rect_in_content, align_x, align_y);
...
bool FrameView::IsPointInScrollbarCorner(const IntPoint& point_in_root_frame) {
  ...
  if (HorizontalScrollbar()) {
    ...
    int horizontal_scrollbar_y_max = HorizontalScrollbar()->FrameRect().Y() +
...
ScrollableArea* FrameView::LayoutViewportScrollableArea() {
  ...
  LayoutViewItem layout_view_item = this->GetLayoutViewItem();
  return layout_view_item.IsNull() ? nullptr : layout_view_item.GetScrollableArea();
}
...
void FrameView::UpdateViewportIntersectionsForSubtree(
  ...
  HTMLFrameOwnerElement* owner_element = frame_->DeprecatedLocalOwner();
  if (hidden_for_throttling_ && owner_element && !owner_element->GetLayoutObject()) {
...
void FrameView::UpdateRenderThrottlingStatus(
  ...
  bool has_handlers = frame_->GetPage() &&
...
void FrameView::RecordDeferredLoadingStats() {
  if (!GetFrame().GetDocument()->GetFrame() || !GetFrame().IsCrossOriginSubframe())
    return;
...
String FrameView::MainThreadScrollingReasonsAsText() const {
  if (RuntimeEnabledFeatures::slimmingPaintV2Enabled()) {
    ...
    if (const auto* scroll_translation = this->ScrollTranslation())
      reasons |= scroll_translation->ScrollNode()->GetMainThreadScrollingReasons();
    return String(
...
void FrameView::ApplyTransformForTopFrameSpace(TransformState& transform_state) {
  // This is the top-level frame, so no mapping necessary.
  if (frame_->IsMainFrame())
    return;
  ...
  transform_state.Move(
      LayoutSize(-viewport_intersection_rect.X(), -viewport_intersection_rect.Y()));
}
