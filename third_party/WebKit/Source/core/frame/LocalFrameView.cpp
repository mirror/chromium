...
void LocalFrameView::UpdateLifecyclePhasesInternal(
  ...
  // is clean and intersection observations can be calculated. This doesn't
  // affect frames that are not local roots because they would have returned
  // earlier due to ShouldThrottleRendering() having been true above.
  if (!frame_->IsMainFrame() && ShouldThrottleRendering()) {
    UpdateViewportIntersectionsForSubtree(target_state);
    return;
  }
