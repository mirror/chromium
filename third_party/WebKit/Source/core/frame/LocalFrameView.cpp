...
IntSize LocalFrameView::MaximumScrollOffsetInt() const {
  ...
  IntSize content_bounds;
  if (GetLayoutView())
    content_bounds = GetLayoutView()->DocumentRect().Size();
  else
    content_bounds = ContentsSize();
...
int LocalFrameView::ScrollSize(ScrollbarOrientation orientation) const {
  ...
  if (!scrollbar) {
    IntSize scroll_size = layout_overflow_size_ - VisibleContentRect().Size();
