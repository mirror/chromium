...
void LocalFrameView::SetNeedsCompositingUpdate(
  ...
  bool document_detached =
      frame_->GetDocument() && frame_->GetDocument()->IsDetached();
  if (!document_detached && !GetLayoutViewItem().IsNull()) {
    DCHECK(GetLayoutViewItem().Compositor());
