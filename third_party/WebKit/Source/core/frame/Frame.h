...
class CORE_EXPORT Frame : public GarbageCollectedFinalized<Frame> {
 ...
 private:
  ...
  // Recursively consumes the transient user activation of:
  //   - ancestor frames if |updateParent| is true, and
  //   - descendant frames if |updateChildren| is true;
  // and then consumes & returns the current frame's state.
  bool ConsumeTransientUserActivationInFrameTree(bool updateParent,
                                                 bool updateChildren);
