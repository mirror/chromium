...
bool WebFrameWidgetBase::ScrollBy(const WebFloatSize& delta,
  ...
  // TODO(wjmaclean): Will ToCoreFrame(LocalRoot()) ever be null?
  if (!LocalRoot() || !ToCoreFrame(LocalRoot())->View())
    return false;
  ...
  if (fling_source_device_ == kWebGestureDeviceTouchpad) {
    ...
    if (HandleMouseWheelWidget(*ToCoreFrame(LocalRoot()), synthetic_wheel) !=
...
WebInputEventResult WebFrameWidgetBase::HandleMouseWheelWidget(
    LocalFrame& frame,
  ...
  View()->HidePopups();
  return GetPageWidgetEventHandler()->HandleMouseWheel(frame, event);
}
