...
void EventHandlerRegistry::UpdateEventHandlerInternal(
  ...
  bool target_set_changed = UpdateEventHandlerTargets(op, handler_class, target);
...
void EventHandlerRegistry::NotifyDidAddOrRemoveEventHandlerTarget(
    EventHandlerClass handler_class) {
  ScrollingCoordinator* scrolling_coordinator = page_->GetScrollingCoordinator();
...
void EventHandlerRegistry::DocumentDetached(Document& document) {
  ...
  for (size_t handler_class_index = 0; handler_class_index < kEventHandlerClassCount;
       ++handler_class_index) {
    EventHandlerClass handler_class =
