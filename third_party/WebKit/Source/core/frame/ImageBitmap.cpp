...
static void ApplyColorSpaceConversion(sk_sp<SkImage>& image,
  ...
  if (options.latest_color_space) {
    ...
    SkImageInfo dst_info =
        SkImageInfo::Make(image->width(), image->height(), options.dst_color_type,
    ...
    if (xform->apply(GetXformFormat(options.dst_color_type),
                     dst_data->writable_data(),
...
static PassRefPtr<StaticBitmapImage> CropImageAndApplyColorSpaceConversion(
  ...
  if (src_rect.IsEmpty() && !parsed_options.premultiply_alpha) {
    SkImageInfo info =
        SkImageInfo::Make(parsed_options.resize_width, parsed_options.resize_height,
  ...
  if (parsed_options.crop_rect == src_rect && !parsed_options.should_scale_input) {
    ...
    if (parsed_options.flip_y) {
      return StaticBitmapImage::Create(FlipSkImageVertically(
          cropped_sk_image.get(), kDontEnforceAlphaPremultiply, parsed_options));
    }
    // Special case: The first parameter image is unpremul but we need to turn
    // it into premul.
    if (parsed_options.premultiply_alpha && image_format == kDontPremultiplyAlpha)
...
ImageBitmap::ImageBitmap(HTMLImageElement* image,
  ...
  if (options.colorSpaceConversion() == kImageBitmapOptionNone) {
    image_ = CropImageAndApplyColorSpaceConversion(
        input.Get(), parsed_options, kPremultiplyAlpha, ColorBehavior::Ignore());
  } else {
    image_ = CropImageAndApplyColorSpaceConversion(
...
ImageBitmap::ImageBitmap(HTMLVideoElement* video,
  ...
  std::unique_ptr<ImageBuffer> buffer = ImageBuffer::Create(
      IntSize(parsed_options.resize_width, parsed_options.resize_height), kNonOpaque,
...
ImageBitmap::ImageBitmap(HTMLCanvasElement* canvas,
  ...
  ParsedOptions parsed_options =
      ParseOptions(options, crop_rect, IntSize(input->width(), input->height()));
  if (DstBufferSizeHasOverflow(parsed_options))
    return;
...
ImageBitmap::ImageBitmap(const void* pixel_data,
  ...
  SkImageInfo info = SkImageInfo::MakeN32(
      width, height,
      is_image_bitmap_premultiplied ? kPremul_SkAlphaType : kUnpremul_SkAlphaType);
...
static sk_sp<SkImage> ScaleSkImage(sk_sp<SkImage> sk_image,
  ...
  SkImageInfo resized_info = SkImageInfo::Make(
      resize_width, resize_height, color_type, kUnpremul_SkAlphaType, color_space);
  RefPtr<ArrayBuffer> dst_buffer = ArrayBuffer::CreateOrNull(
...
ImageBitmap::ImageBitmap(ImageData* data,
  ...
  if (DstBufferSizeHasOverflow(parsed_options))
    return;
  IntRect src_rect = crop_rect ? Intersection(parsed_options.crop_rect, data_src_rect)
                             : data_src_rect;
  ...
  if (!parsed_options.premultiply_alpha) {
    ...
    if (parsed_options.crop_rect == IntRect(IntPoint(), data->size())) {
      ...
      sk_image =
          SkImage::MakeRasterCopy(SkPixmap(info, src_addr, dst_pixel_bytes_per_row));
      ...
      if (!src_rect.IsEmpty()) {
        IntPoint src_point = IntPoint(
            (parsed_options.crop_rect.X() > 0) ? parsed_options.crop_rect.X() : 0,
            (parsed_options.crop_rect.Y() > 0) ? parsed_options.crop_rect.Y() : 0);
        IntPoint dst_point = IntPoint(
            (parsed_options.crop_rect.X() >= 0) ? 0 : -parsed_options.crop_rect.X(),
            (parsed_options.crop_rect.Y() >= 0) ? 0
        ...
        for (int i = 0; i < copy_height; i++) {
          ...
          if (parsed_options.flip_y)
            ...
            dst_start_copy_position = (dst_point.Y() + i) * dst_pixel_bytes_per_row +
                                   dst_point.X() * bytes_per_pixel;
          for (unsigned j = 0; j < src_end_copy_position - src_start_copy_position;
               j++) {
    ...
    if (parsed_options.should_scale_input) {
      image_ = StaticBitmapImage::Create(
          ScaleSkImage(sk_image, parsed_options.resize_width,
                       parsed_options.resize_height, parsed_options.resize_quality,
                       parsed_options.latest_color_type, data->GetSkColorSpace()));
    } else {
...
ImageBitmap* ImageBitmap::Create(const void* pixel_data,
                                 ...
                                 bool is_image_bitmap_premultiplied,
                                 bool is_image_bitmap_origin_clean) {
  return new ImageBitmap(pixel_data, width, height, is_image_bitmap_premultiplied,
...
ScriptPromise ImageBitmap::CreateImageBitmap(ScriptState* script_state,
                                             ...
                                             ExceptionState& exception_state) {
  if ((crop_rect &&
       !IsSourceSizeValid(crop_rect->Width(), crop_rect->Height(),
                          exception_state)) ||
      !IsSourceSizeValid(width(), height(), exception_state))
    return ScriptPromise();
  if (!IsResizeOptionValid(options, exception_state))
    return ScriptPromise();
  return ImageBitmapSource::FulfillImageBitmap(script_state,
