...
static ParsedOptions DefaultOptions() {
  return ParsedOptions();
}

static ParsedOptions ParseOptions(const ImageBitmapOptions& options,
...
static bool DstBufferSizeHasOverflow(const ParsedOptions& options) {
...
// The parameter image_format indicates whether the parameter |image| is
// unpremultiplied or not.  image_format = PremultiplyAlpha means the image is
// in premuliplied format. For example, if the image is already in
// unpremultiplied format and we want the created ImageBitmap in the same
// format, then we don't need to use the ImageDecoder to decode the image.
void ImageBitmap::CropImageAndApplyColorSpaceConversion(
  ...
  DCHECK(image);
  std::unique_ptr<CropImageInfo> crop_image_info =
      WTF::WrapUnique(new CropImageInfo);
  crop_image_info->image = image;
  crop_image_info->parsed_options = parsed_options;
  crop_image_info->image_format = image_format;
  crop_image_info->color_behavior = color_behavior;
  crop_image_info->origin_clean = is_origin_clean;
  crop_image_info->source_lazy_decoded = is_lazy_decoded;
  crop_image_info->premultiply_alpha_reverted = is_premultiply_alpha_reverted;
  crop_image_info->error_type = error_type;
  ...
  BackgroundTaskRunner::PostOnBackgroundThread(
      ...
      CrossThreadBind(&ImageBitmap::DecodeImageOnBackground,
                      ...
                      WTF::Passed(std::move(crop_image_info)),
                      WrapCrossThreadPersistent(resolver)));
}

void ImageBitmap::DecodeImageOnBackground(
    WebTaskRunner* task_runner,
    std::unique_ptr<CropImageInfo> crop_image_info,
    ScriptPromiseResolver* resolver) {
  DCHECK(!IsMainThread());
  Image* image = crop_image_info->image;
  ParsedOptions parsed_options = crop_image_info->parsed_options;
  AlphaDisposition image_format = crop_image_info->image_format;
  ColorBehavior color_behavior = crop_image_info->color_behavior;
  bool origin_clean = crop_image_info->origin_clean;
  bool source_lazy_decoded = crop_image_info->source_lazy_decoded;
  bool premultiply_alpha_reverted = crop_image_info->premultiply_alpha_reverted;
  RejectPromiseErrorType error_type = crop_image_info->error_type;
  ...
  // In the case when crop_rect doesn't intersect the source image, we
  ...
  if (src_rect.IsEmpty()) {
    ...
    if (!surface) {
      PostProcessImage(task_runner, parsed_options, origin_clean,
                       source_lazy_decoded, premultiply_alpha_reverted,
                       resolver, error_type);
    }
    image_ = StaticBitmapImage::Create(surface->makeImageSnapshot());
    PostProcessImage(task_runner, parsed_options, origin_clean, false, false,
                     resolver, error_type);
  }
  ...
  if (!parsed_options.should_scale_input) {
    ...
    if (parsed_options.flip_y) {
      image_ = StaticBitmapImage::Create(FlipSkImageVertically(
    ...
    image_ = StaticBitmapImage::Create(std::move(cropped_sk_image));
    PostProcessImage(task_runner, parsed_options, origin_clean,
                     source_lazy_decoded, premultiply_alpha_reverted, resolver,
                     error_type);
  }
...
void ImageBitmap::PostProcessImage(WebTaskRunner* task_runner,
                                   ParsedOptions& parsed_options,
                                   bool origin_clean,
                                   ...
                                   ScriptPromiseResolver* resolver,
                                   RejectPromiseErrorType error_type) {
  if (source_lazy_decoded)
    ManuallyTriggerDecoding(parsed_options);
  if (premultiply_alpha_reverted)
    ManuallyRevertAlpha(parsed_options);
  if (image_) {
  ...
  task_runner->PostTask(
      ...
      CrossThreadBind(&ImageBitmap::ResolvePromiseOnOriginalThread,
                      ...
                      WrapCrossThreadPersistent(resolver), error_type));
}

// In the case where the source image is lazy-decoded, image_ may not be in
// a decoded state, we trigger it here.
void ImageBitmap::ManuallyTriggerDecoding(ParsedOptions& parsed_options) {
  if (!image_)
    return;
...
void ImageBitmap::ManuallyRevertAlpha(ParsedOptions& parsed_options) {
  if (!image_)
    return;
  parsed_options.premultiply_alpha = false;
...
void ImageBitmap::ResolvePromiseOnOriginalThread(
    ScriptPromiseResolver* resolver,
    RejectPromiseErrorType error_type) {
  // In certain cases such as unit tests, it directly calls ImageBitmap::Create,
  // which pass a nullptr for resolver. We don't need to resolve the promise
  // in these cases.
  if (!resolver)
    return;
  if (image_) {
    ...
    if (error_type == kNullPromiseError) {
      resolver->Reject(
...
ImageBitmap::ImageBitmap(ImageElementBase* image,
  ...
  CropImageAndApplyColorSpaceConversion(
      resolver, input.Get(), parsed_options, kPremultiplyAlpha,
      !image->WouldTaintOrigin(document->GetSecurityOrigin()), true,
      is_premultiply_alpha_reverted, kNullPromiseError,
...
ImageBitmap::ImageBitmap(HTMLCanvasElement* canvas,
  ...
  CropImageAndApplyColorSpaceConversion(
      resolver, input.Get(), parsed_options, kPremultiplyAlpha,
      canvas->OriginClean(), false, is_premultiply_alpha_reverted,
      kNullPromiseError, ColorBehavior::TransformToGlobalTarget());
}
...
ImageBitmap::ImageBitmap(OffscreenCanvas* offscreen_canvas,
  ...
  CropImageAndApplyColorSpaceConversion(
      resolver, input.Get(), parsed_options, kPremultiplyAlpha,
      offscreen_canvas->OriginClean(), false, is_premultiply_alpha_reverted,
      kNullPromiseError, ColorBehavior::TransformToGlobalTarget());
}
...
ImageBitmap::ImageBitmap(ImageBitmap* bitmap,
  ...
  CropImageAndApplyColorSpaceConversion(
      resolver, input.Get(), parsed_options,
      bitmap->IsPremultiplied() ? kPremultiplyAlpha : kDontPremultiplyAlpha,
      bitmap->OriginClean(), false, false, kNullPromiseError,
      ColorBehavior::TransformToGlobalTarget());
}
...
ImageBitmap::ImageBitmap(RefPtr<StaticBitmapImage> image,
  ...
  CropImageAndApplyColorSpaceConversion(
      resolver, image_.Get(), parsed_options, kPremultiplyAlpha,
      image->OriginClean(), false, is_premultiply_alpha_reverted,
      kNullPromiseError, ColorBehavior::TransformToGlobalTarget());
}
