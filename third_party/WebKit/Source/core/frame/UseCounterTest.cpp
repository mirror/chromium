...
void ExpectHistograms(const HistogramTester& histogram_tester_,
                      int visits_count,
                      WebFeature feature,
                      int feature_count,
                      CSSPropertyID property,
                      int property_count) {
  histogram_tester_.ExpectBucketCount(kFeaturesHistogramName,
                                      static_cast<int>(WebFeature::kPageVisits),
                                      visits_count);
  histogram_tester_.ExpectBucketCount(kFeaturesHistogramName,
                                      static_cast<int>(feature), feature_count);
  histogram_tester_.ExpectTotalCount(kFeaturesHistogramName,
                                     visits_count + feature_count);
  histogram_tester_.ExpectBucketCount(kCSSHistogramName, 1, visits_count);
  histogram_tester_.ExpectBucketCount(
      kCSSHistogramName,
      UseCounter::MapCSSPropertyIdToCSSSampleIdForHistogram(property),
      property_count);
  histogram_tester_.ExpectTotalCount(kCSSHistogramName,
