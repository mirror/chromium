...
void HistogramBasicTest(const std::string& histogram,
  ...
  if (!legacy_histogram.empty()) {
    histogram_tester.ExpectBucketCount(legacy_histogram, histogram_map(item), 1);
    histogram_tester.ExpectBucketCount(legacy_histogram, histogram_map(second_item),
                                      1);
  ...
  did_commit_load(URLTestHelpers::ToKURL(url));
  if (!legacy_histogram.empty()) {
    histogram_tester.ExpectBucketCount(legacy_histogram, histogram_map(item), 2);
    histogram_tester.ExpectBucketCount(legacy_histogram, histogram_map(second_item),
                                      1);
