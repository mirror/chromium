...
static std::unique_ptr<DragImage> CreateDragImage(
  ...
  return DragImage::Create(image.Get(), image_orientation,
                           screen_device_scale_factor, kInterpolationHigh, opacity);
}
  ...
  std::unique_ptr<DragImage> CreateImage() {
    ...
    FloatRect bounding_box =
        layer->GetLayoutObject()
            .AbsoluteToLocalQuad(FloatQuad(absolute_bounding_box), kUseTransforms)
            .BoundingBox();
    ...
    if (RuntimeEnabledFeatures::slimmingPaintV2Enabled()) {
      border_box_properties = *layer->GetLayoutObject().LocalBorderBoxProperties();
    }
...
FrameInitCallbackVector& GetInitializationVector() {
  DEFINE_THREAD_SAFE_STATIC_LOCAL(FrameInitCallbackVector, initialization_vector,
...
void LocalFrame::CreateView(const IntSize& viewport_size,
  ...
  frame_view->SetScrollbarModes(horizontal_scrollbar_mode, vertical_scrollbar_mode,
                               horizontal_lock, vertical_lock);
...
void LocalFrame::Reload(FrameLoadType load_type,
  ...
  if (client_redirect_policy == ClientRedirectPolicy::kNotClientRedirect) {
    ...
    FrameLoadRequest request =
        FrameLoadRequest(nullptr, loader_.ResourceRequestForReload(
                                      load_type, KURL(), client_redirect_policy));
...
void LocalFrame::Detach(FrameDetachType type) {
  ...
  if (GetPage() && GetPage()->GetScrollingCoordinator() && view_)
    GetPage()->GetScrollingCoordinator()->WillDestroyScrollableArea(view_.Get());
...
void LocalFrame::PrintNavigationErrorMessage(const Frame& target_frame,
  ...
  String target_frame_description =
      target_frame.IsLocalFrame()
          ...
          : "with origin '" +
                target_frame.GetSecurityContext()->GetSecurityOrigin()->ToString() +
                "'";
  String message =
...
void LocalFrame::DetachChildren() {
  DCHECK(loader_.StateMachine()->CreatingInitialEmptyDocument() || GetDocument());
...
FloatSize LocalFrame::ResizePageRectsKeepingRatio(
  ...
  if (ContentLayoutItem().Style()->IsHorizontalWritingMode()) {
    ...
    ASSERT(fabs(original_size.Height()) > std::numeric_limits<float>::epsilon());
