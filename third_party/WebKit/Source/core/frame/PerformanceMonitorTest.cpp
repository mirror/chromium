...
TEST_F(PerformanceMonitorTest, LongTaskIdlenessBasic) {
  DomContentLoadedEventFired();
  DidProcessTask(0.01, 0.02);
  EXPECT_TRUE(IsLongTaskIdlenessDetectionStarted());
  DidProcessTask(0.6, 0.61);
  EXPECT_TRUE(IsLongTaskIdle());
}

TEST_F(PerformanceMonitorTest, LongTaskIdlenessReset) {
  DomContentLoadedEventFired();
...
TEST_F(PerformanceMonitorTest, LongTaskIdlenessWillProcessTask) {
  DomContentLoadedEventFired();
  DidProcessTask(0.01, 0.02);
  EXPECT_TRUE(IsLongTaskIdlenessDetectionStarted());
