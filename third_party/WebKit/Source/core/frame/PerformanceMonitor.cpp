...
void PerformanceMonitor::SetBypassLongCompileThresholdForTesting(bool enabled) {
  bypass_long_compile_threshold = enabled;
};
...
void PerformanceMonitor::Did(const probe::V8Compile& probe) {
  ...
  if (bypass_long_compile_threshold) {
    SetBypassLongCompileThresholdForTesting(false);
  } else {
