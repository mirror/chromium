...
scoped_refptr<StaticBitmapImage> GetImageWithAlphaDisposition(
  ...
  // Premul to unpremul: we have to readback the pixels
  ...
  // Unpremul to premul: draw on a surface. Avoid sRGB gamma transfer curve.
...
scoped_refptr<StaticBitmapImage> ScaleImage(
  ...
  if (PremulCodePathAllowed(parsed_options)) {
    ...
    auto resized_sk_image = ScaleSkImage(sk_image, parsed_options.resize_width,
  ...
  auto resized_rgb_image =
      ScaleSkImage(rgb_image, parsed_options.resize_width,
                   parsed_options.resize_height, parsed_options.resize_quality);
  auto resized_alpha_image =
      ScaleSkImage(alpha_image, parsed_options.resize_width,
                   parsed_options.resize_height, parsed_options.resize_quality);

  // Merge two resized rgb and alpha SkImages together.
  // A better solution would be using SkImageFilter and SkBlendMode to merge the
  // images: convert RGB/255 to RGB/0, convert A,0,0/255 to 0,0,0/A, merge using
  // kSrc and kLighten blend modes. Unfortunately, this doesn't work as
  ...
  SkPixmap pixmap(unpremul_info, rgb_data->Data(), unpremul_info.minRowBytes());
