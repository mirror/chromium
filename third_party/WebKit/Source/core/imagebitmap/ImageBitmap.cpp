...
void ImageBitmap::DecodeImageOnBackground(
  ...
  if (src_rect.IsEmpty()) {
    ...
    if (!surface) {
      PostProcessImage(std::move(task_runner), nullptr, parsed_options,
                       origin_clean, source_lazy_decoded,
                       premultiply_alpha_reverted, resolver, error_type);
      return;
    }
    sk_sp<SkImage> skia_image_from_surface = surface->makeImageSnapshot();
    PostProcessImage(std::move(task_runner), std::move(skia_image_from_surface),
                     parsed_options, origin_clean, false, false, resolver,
                     error_type);
    return;
  }
  ...
  if ((((!image->IsSVGImage() && !skia_image->isOpaque()) || !skia_image) &&
    ...
    if (!skia_image) {
      PostProcessImage(std::move(task_runner), std::move(skia_image),
                       parsed_options, origin_clean, source_lazy_decoded,
                       premultiply_alpha_reverted, resolver, error_type);
      return;
    }
  }
  ...
  if (!parsed_options.should_scale_input) {
    ...
    if (parsed_options.flip_y) {
      sk_sp<SkImage> flipped_skia_image = FlipSkImageVertically(
          cropped_sk_image, kDontEnforceAlphaPremultiply, parsed_options);
      PostProcessImage(std::move(task_runner), std::move(flipped_skia_image),
                       parsed_options, origin_clean, source_lazy_decoded,
                       premultiply_alpha_reverted, resolver, error_type);
      return;
    }
    ...
    if (parsed_options.premultiply_alpha &&
      ...
      sk_sp<SkImage> skia_image_with_format =
          GetSkImageWithAlphaDisposition(cropped_sk_image, kPremultiplyAlpha);
      PostProcessImage(std::move(task_runner),
                       std::move(skia_image_with_format), parsed_options,
                       origin_clean, source_lazy_decoded,
                       premultiply_alpha_reverted, resolver, error_type);
      return;
    }
    PostProcessImage(std::move(task_runner), std::move(cropped_sk_image),
                     parsed_options, origin_clean, source_lazy_decoded,
                     premultiply_alpha_reverted, resolver, error_type);
    return;
  }
  ...
  PostProcessImage(std::move(task_runner), std::move(skia_image_with_format),
                   parsed_options, origin_clean, source_lazy_decoded,
                   premultiply_alpha_reverted, resolver, error_type);
}

void ImageBitmap::PostProcessImage(RefPtr<WebTaskRunner> task_runner,
                                   sk_sp<SkImage> skia_image,
                                   ParsedOptions& parsed_options,
                                   bool origin_clean,
  ...
  if (source_lazy_decoded) {
    ManuallyTriggerDecoding(parsed_options, skia_image);
  }
  if (premultiply_alpha_reverted) {
    ManuallyRevertAlpha(parsed_options, skia_image);
  }
...
void ImageBitmap::ManuallyRevertAlpha(ParsedOptions& parsed_options,
  ...
  if (!sk_image)
    return;
  parsed_options.premultiply_alpha = false;
  sk_image = GetSkImageWithAlphaDisposition(sk_image, kDontPremultiplyAlpha);
}
