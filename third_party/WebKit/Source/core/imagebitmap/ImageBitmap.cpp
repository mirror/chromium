...
static scoped_refptr<StaticBitmapImage> CropImageAndApplyColorSpaceConversion(
  ...
  if (down_scaling) {
    result = ScaleImage(std::move(result), parsed_options, kDownscaling);
    if (!result)
      return nullptr;
  }
  ...
  if (up_scaling) {
    result = ScaleImage(std::move(result), parsed_options, kUpscaling);
    if (!result)
      return nullptr;
  }
...
ImageBitmap::ImageBitmap(ImageData* data,
  ...
  // resize if down-scaling
  if (down_scaling)
    image_ = ScaleImage(std::move(image_), parsed_options, kDownscaling);
  if (!image_)
    return;
  ...
  // resize if up-scaling
  if (up_scaling)
    image_ = ScaleImage(std::move(image_), parsed_options, kUpscaling);
}
