...
CanvasColorParams ImageBitmap::GetCanvasColorParams() {
  return CanvasColorParams(GetSkImageSkImageInfo(
      image_->PaintImageForCurrentFrame().GetSkImage().get()));
}
...
PassRefPtr<Uint8Array> ImageBitmap::CopyBitmapData() {
  return CopySkImageData(image_->PaintImageForCurrentFrame().GetSkImage());
}
...
PassRefPtr<Image> ImageBitmap::GetSourceImageForCanvas(
  ...
  *status = kNormalSourceImageStatus;
  ...
  sk_sp<SkImage> premul_sk_image = GetSkImageWithAlphaDisposition(
      image_->PaintImageForCurrentFrame().GetSkImage(), kPremultiplyAlpha);
