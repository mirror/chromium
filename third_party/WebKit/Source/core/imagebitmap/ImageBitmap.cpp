...
static PassRefPtr<Uint8Array> CopySkImageData(sk_sp<SkImage> input,
  ...
  // CopySkImageData should not be used for color conversion. To do so, use
  // ApplyColorSpaceConversion().
  input->readPixels(info.makeColorSpace(nullptr), dst_pixels->Data(),
...
static sk_sp<SkImage> GetSkImageWithAlphaDisposition(
  ...
  SkImageInfo info =
      SkImageInfo::Make(image->width(), image->height(),
                        GetSkImageSkColorType(image), alpha_type, nullptr);
  RefPtr<Uint8Array> dst_pixels = CopySkImageData(image, info);

  if (!dst_pixels)
    return nullptr;
  info = info.makeColorSpace(image->refColorSpace());
...
static sk_sp<SkImage> ScaleSkImage(sk_sp<SkImage> sk_image,
  ...
  SkImageInfo resize_info = SkImageInfo::Make(
      ...
      GetSkImageSkColorType(input), kPremul_SkAlphaType, nullptr);
...
static void ApplyColorSpaceConversion(sk_sp<SkImage>& image,
  ...
  if (image->readPixels(dst_info, dst_data->writable_data(),
    ...
    sk_sp<SkImage> premul_image = nullptr;
    if (image->alphaType() == kUnpremul_SkAlphaType) {
      premul_image = GetSkImageWithAlphaDisposition(image, kPremultiplyAlpha);
    ...
    if (!surface)
      return;
    surface->getCanvas()->drawImage(premul_image ? premul_image : image, 0, 0);
...
ImageBitmap::ImageBitmap(ImageData* data,
  ...
  unsigned bytes_per_row = byte_length / cropped_data->height();
  ...
  sk_sp<SkImage> skia_image =
      NewSkImageFromRaster(info, image_pixels, bytes_per_row);
