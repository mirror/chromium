...
// premul is not allowed and the requested filter quality is high.
sk_sp<SkImage> ScaleSkImage(sk_sp<SkImage> image,
  ...
  image->scalePixels(resized_pixmap, resize_quality);
...
scoped_refptr<StaticBitmapImage> ScaleImage(
  ...
  if (image_info.alphaType() == kPremul_SkAlphaType ||
      PremulCodePathAllowed(parsed_options) ||
      parsed_options.resize_quality != kHigh_SkFilterQuality) {
    ...
    auto resized_sk_image = ScaleSkImage(
        ...
        parsed_options.resize_quality, alpha_disposition);
  ...
  auto resized_rgb_image = ScaleSkImage(
      ...
      parsed_options.resize_quality, kDontPremultiplyAlpha);
