...
void ImageBitmap::ResolvePromiseOnOriginalThread(
    ...
    sk_sp<SkImage> skia_image,
    const BitmapOriginClean& origin_clean,
...
ScriptPromise ImageBitmap::CreateAsync(ImageElementBase* image,
  ...
  BitmapOriginClean bitmap_origin_clean = kBitmapOriginDirty;
  if (!image->WouldTaintOrigin(document->GetSecurityOrigin()))
    bitmap_origin_clean = kBitmapOriginClean;
