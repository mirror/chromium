...
static inline bool PremulCodePathAllowed(
    const ImageBitmap::ParsedOptions& options) {
  return options.premultiply_alpha || !options.source_is_unpremul;
}
...
scoped_refptr<StaticBitmapImage> FlipImageVertically(
  ...
  if (PremulCodePathAllowed(parsed_options)) {
    // Use Skia to flip the image by drawing it on a surface.
...
scoped_refptr<StaticBitmapImage> GetImageWithAlphaDisposition(
  ...
  // surface. As shown  in
  // https://fiddle.skia.org/c/ce58204741404378a9fa16f8d356e238, drawing on a
  // canvas and getting a snapshot is slower than readback and manual premul.
  // Therefore, we do the readback here and leave the GPU readback issue to
  // get solved when fixing crbug.com/740197 (TODO: zakerinasab).

  // Avoid sRGB gamma transfer curve.
  if (SkColorSpace::Equals(info.colorSpace(), SkColorSpace::MakeSRGB().get()))
    dst_pixels = CopyImageData(image, info.makeColorSpace(nullptr));
  else
...
// Resizes an SkImage. If the image is unpremul, premuls and scales, thus the
// resized image is always premul.
sk_sp<SkImage> ScaleSkImage(sk_sp<SkImage> image,
  ...
  // Premul if needed
  sk_sp<SkImage> premul_image = image;
  if (image_info.alphaType() == kUnpremul_SkAlphaType) {
    image_info = image_info.makeAlphaType(kPremul_SkAlphaType);
    sk_sp<SkSurface> surface = SkSurface::MakeRaster(image_info);
    if (!surface)
      return nullptr;
    surface->getCanvas()->drawImage(image.get(), 0, 0);
    premul_image = surface->makeImageSnapshot();
  }
  ...
  premul_image->scalePixels(resized_pixmap, resize_quality);
  ...
  scoped_refptr<StaticBitmapImage> ScaleImage(
    ...
    if (PremulCodePathAllowed(parsed_options)) {
      ...
      auto resized_sk_image = ScaleSkImage(
    ...
    // If premul code path is not allowed, we scale color channels and alpha
    // channel separately: RGBA -> RGB/255 and A/255,255,255, scale, merge.
    ...
    auto rgb_image = sk_image->makeWithFilter(
        image_filter_set_alpha_255.get(), sk_image->bounds(),
    ...
    // resize
    auto resized_rgb_image = ScaleSkImage(
        rgb_image, parsed_options.resize_width, parsed_options.resize_height,
        parsed_options.resize_quality);
    auto resized_alpha_image = ScaleSkImage(
        alpha_image, parsed_options.resize_width, parsed_options.resize_height,
        parsed_options.resize_quality);
    ...
    SkImageInfo image_info = GetSkImageInfo(resized_rgb_image);
    if (image_info.colorType() == kRGBA_F16_SkColorType) {
      uint16_t* rgb_data_iter =
          static_cast<uint16_t*>((void*)(rgb_data->Data()));
    ...
    SkImageInfo unpremul_info = image_info.makeAlphaType(kUnpremul_SkAlphaType);
    SkPixmap pixmap(unpremul_info, rgb_data->Data(),
                    unpremul_info.minRowBytes());
