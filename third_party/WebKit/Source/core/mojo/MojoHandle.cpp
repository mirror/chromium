...
MojoResult MojoHandle::writeMessage(
  ...
  return MojoWriteMessage(handle_->value(), bytes, num_bytes, raw_handles.Data(),
                          raw_handles.size(), MOJO_WRITE_MESSAGE_FLAG_NONE);
}
...
void MojoHandle::readMessage(const MojoReadMessageFlags& flags_dict,
  ...
  for (size_t i = 0; i < num_handles; ++i) {
    handles[i] =
        MojoHandle::Create(mojo::MakeScopedHandle(mojo::Handle(raw_handles[i])));
  }
