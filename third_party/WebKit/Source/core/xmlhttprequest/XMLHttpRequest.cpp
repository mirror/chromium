...
class ScopedEventDispatchProtect final {
 public:
  explicit ScopedEventDispatchProtect(int* level) : level_(level) {
    ++*level_;
  }
...
void ReplaceCharsetInMediaType(String& media_type, const String& charset_value) {
...
XMLHttpRequest* XMLHttpRequest::Create(ExecutionContext* context) {
  XMLHttpRequest* xml_http_request = new XMLHttpRequest(context, false, nullptr);
  xml_http_request->SuspendIfNeeded();
...
XMLHttpRequest::XMLHttpRequest(
    ...
    : SuspendableObject(context),
      ...
      isolated_world_security_origin_(std::move(isolated_world_security_origin)),
...
void XMLHttpRequest::ThrowForLoadFailureIfNeeded(ExceptionState& exception_state,
                                                 const String& reason) {
...
void XMLHttpRequest::CreateRequest(PassRefPtr<EncodedFormData> http_body,
  ...
  if (url_.ProtocolIs("blob") && method_ != HTTPNames::GET) {
    ...
    if (!async_) {
      ThrowForLoadFailureIfNeeded(
          exception_state, "'GET' is the only method allowed for 'blob:' URLs.");
    }
    return;
  }
  ...
  bool include_credentials =
      ...
      (GetSecurityOrigin()->HasSuborigin() &&
       ...
       SecurityOrigin::Create(url_)->IsSameSchemeHostPort(
           GetSecurityOrigin()));
  ...
  options.preflight_policy = upload_events ? kForcePreflight : kConsiderPreflight;
  ...
  resource_loader_options.allow_credentials =
      (same_origin_request_ || include_credentials) ? kAllowStoredCredentials
...
void XMLHttpRequest::DispatchProgressEvent(const AtomicString& type,
  ...
  unsigned long long loaded =
      received_length >= 0 ? static_cast<unsigned long long>(received_length) : 0;
  ...
  probe::AsyncTask async_task(
      ...
      async_);
  progress_event_throttle_->DispatchProgressEvent(type, length_computable, loaded,
                                                 total);
}
...
void XMLHttpRequest::setRequestHeader(const AtomicString& name,
  ...
  if (!IsValidHTTPHeaderValue(value)) {
    exception_state.ThrowDOMException(
        kSyntaxError, "'" + value + "' is not a valid HTTP header field value.");
    return;
  }
...
String XMLHttpRequest::getAllResponseHeaders() const {
  ...
  ExtractCorsExposedHeaderNamesList(response_, access_control_expose_header_set);
...
const AtomicString& XMLHttpRequest::getResponseHeader(
  ...
  if (!same_origin_request_ && !IsOnAccessControlResponseHeaderWhitelist(name) &&
...
PassRefPtr<BlobDataHandle> XMLHttpRequest::CreateBlobDataHandleFromResponse() {
  ...
  return BlobDataHandle::Create(std::move(blob_data), length_downloaded_to_file_);
}
...
void XMLHttpRequest::EndLoading() {
  probe::didFinishXHRLoading(GetExecutionContext(), this, this, method_,
                             url_);
  ...
  if (GetDocument() && GetDocument()->GetFrame() && GetDocument()->GetFrame()->GetPage() &&
