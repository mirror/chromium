...
SelectionInDOMTree DirectionGranularityStrategy::UpdateExtent(
  ...
  IntPoint old_extent_point =
      IntPoint(old_offset_extent_point.X() - offset_, old_offset_extent_point.Y());
  ...
  if (new_offset_extent_position.DeepEquivalent() ==
    ...
    bool selection_expanded = ArePositionsInSpecifiedOrder(
        new_offset_extent_position, old_offset_extent_position, old_extent_base_order);
    bool extent_base_order_switched =
        selection_expanded
            ? false
            : !ArePositionsInSpecifiedOrder(new_offset_extent_position, base,
                                            old_extent_base_order);
    new_extent_base_order =
        extent_base_order_switched ? -old_extent_base_order : old_extent_base_order;
    ...
    if (extent_base_order_switched) {
      ...
      // the |base| position.
      word_boundary = NextWordBound(base, new_extent_base_order > 0
  ...
  if (granularity_ == kWordGranularity) {
    ...
    VisiblePosition bound_before_extent =
        NextWordBound(new_offset_extent_position, SearchDirection::kSearchBackwards,
                      BoundAdjust::kCurrentPosIfOnBound);
    VisiblePosition bound_after_extent =
        NextWordBound(new_offset_extent_position, SearchDirection::kSearchForward,
                      BoundAdjust::kCurrentPosIfOnBound);
  ...
  diff_extent_point_from_extent_position_ =
      extent_point + IntSize(offset_, 0) - PositionLocation(new_selection_extent);
