  ...
  for (auto layout_object : paint_range_) {
    ...
    for (LayoutBlock* containing_block = layout_object->ContainingBlock();
      ...
      if (containing_block->GetSelectionState() == SelectionState::kNone)
        break;
      SetSelectionNoneAndInvalidateIfNeeds(containing_block);
    }
  }
...
static void SetSelectionStatePropagateIfNeeds(LayoutObject* layout_object,
  ...
  if (!layout_object->IsText() && !layout_object->IsBoxModelObject())
    return;
  ...
  for (LayoutBlock* containing_block = layout_object->ContainingBlock();
    ...
    if (state == SelectionState::kInside &&
        containing_block->GetSelectionState() != SelectionState::kNone)
      continue;

    if ((state == SelectionState::kStart &&
         containing_block->GetSelectionState() == SelectionState::kEnd) ||
        (state == SelectionState::kEnd &&
         containing_block->GetSelectionState() == SelectionState::kStart))
      containing_block->SetSelectionState(SelectionState::kStartAndEnd);
    else
