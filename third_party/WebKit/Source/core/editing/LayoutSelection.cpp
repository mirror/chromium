...
static NewPaintRangeAndSelectedLayoutObjects ComputeNewRangeForNG(
  ...
  const WTF::Optional<unsigned> ng_start_offset =
      GetNGOffset(start_layout_object, new_range.PaintRange().StartOffset(),
                  selection.StartPosition(), start_offset);

  LayoutObject* const end = new_range.PaintRange().EndLayoutObject();
  const WTF::Optional<unsigned> ng_end_offset =
      GetNGOffset(end_layout_object, new_range.PaintRange().EndOffset(),
                  selection.EndPosition(), end_offset);
