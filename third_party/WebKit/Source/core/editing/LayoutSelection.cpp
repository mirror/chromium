...
int ComputeStartOffset(const LayoutObject& layout_object,
                       const PositionInFlatTree& position) {
  Node* const layout_node = layout_object.GetNode();
  if (!layout_node)
    return 0;
  if (!layout_node->IsTextNode())
    return 0;
...
int ComputeEndOffset(const LayoutObject& layout_object,
                     const PositionInFlatTree& position) {
  Node* const layout_node = layout_object.GetNode();
  if (!layout_node)
    return 0;
...
static SelectionMarkingRange CalcSelectionRangeAndSetSelectionState(
  ...
  // Exact same position.
  if (start_layout_object == end_layout_object &&
      start_layout_object->IsText() && start_offset == end_offset)
    return {};
