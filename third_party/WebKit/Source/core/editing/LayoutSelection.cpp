...
static LayoutObjectAndOffset ComputeLayoutObjectAndOffsetModTextFragment(
    ...
    int offset) {
  LayoutTextFragment* const first_letter = FirstLetterPartFor(layout_text);
  if (!first_letter)
    return {layout_text, offset};
  DCHECK_GE(offset, 0);
  const unsigned unsigned_offset = static_cast<unsigned>(offset);
  const unsigned layout_text_start = ToLayoutTextFragment(layout_text)->Start();
  if (unsigned_offset <= layout_text_start)
    return {first_letter, offset};
  return {layout_text, static_cast<int>(unsigned_offset - layout_text_start)};
}
