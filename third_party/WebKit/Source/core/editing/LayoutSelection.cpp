...
Optional<int> SelectionPaintRange::StartOffset() const {
  DCHECK(!IsNull());
...
Optional<int> SelectionPaintRange::EndOffset() const {
  DCHECK(!IsNull());
  return end_offset_;
}
...
static Optional<int> ComputeStartOffset(const LayoutObject& layout_object,
...
static Optional<int> ComputeEndOffset(const LayoutObject& layout_object,
                                      const PositionInFlatTree& position) {
  Node* const layout_node = layout_object.GetNode();
  if (!layout_node || !layout_node->IsTextNode())
    return {};
...
// ExpandedLayoutText contains
// case 1: |primary| is LayoutObject w/o TextFragment
// case 2: |primary| is the first TextFragment and |secondary| is later one
// in a LayoutText.
struct ExpandedLayoutText {
  STACK_ALLOCATED();
  LayoutObject* primary;
  LayoutTextFragment* secondary;

  ExpandedLayoutText(LayoutObject* layout_object) {
    if (LayoutTextFragment* first_letter_part =
            FirstLetterPartFor(layout_object)) {
      primary = first_letter_part;
      secondary = ToLayoutTextFragment(layout_object);
      return;
    }
    primary = layout_object;
    secondary = nullptr;
  }
};
...
static void MarkSelectedInside(PaintInvalidationSet* invalidation_set,
                               LayoutObject* layout_object) {
  ExpandedLayoutText expanded(layout_object);
  MarkSelected(invalidation_set, expanded.primary, SelectionState::kInside);
  if (!expanded.secondary)
    return;
  MarkSelected(invalidation_set, expanded.secondary, SelectionState::kInside);
}
...
static LayoutObjectAndOffset ComputeLayoutObjectAndOffsetModTextFragment(
    const ExpandedLayoutText& expanded,
    int offset) {
  if (!expanded.secondary)
    return {expanded.primary, offset};
  DCHECK_GE(offset, 0);
  const unsigned unsigned_offset = static_cast<unsigned>(offset);
  if (unsigned_offset <= expanded.secondary->Start())
    return {expanded.primary, offset};
  return {expanded.secondary,
          static_cast<int>(unsigned_offset - expanded.secondary->Start())};
}
...
static SelectionMarkingRange MarkStartAndEndInOneNode(
  ...
  if (!start_offset.has_value()) {
    ...
    return {{layout_object, {}, layout_object, {}},
            std::move(invalidation_set)};
  }
  const ExpandedLayoutText expanded(layout_object);
  const LayoutObjectAndOffset start =
      ComputeLayoutObjectAndOffsetModTextFragment(expanded,
                                                  start_offset.value());
  const LayoutObjectAndOffset end =
      ComputeLayoutObjectAndOffsetModTextFragment(expanded, end_offset.value());
...
LayoutObjectAndOffset ComputeAndMarkStartLayoutObjectAndOffset(
    PaintInvalidationSet* invalidation_set,
    LayoutObject* start_layout_object,
    Optional<int> start_offset) {
  if (!start_offset.has_value()) {
    // |start_layout_object| is not LayoutText.
    MarkSelected(invalidation_set, start_layout_object, SelectionState::kStart);
    return {start_layout_object, {}};
  }
  const ExpandedLayoutText start_expanded(start_layout_object);
  const LayoutObjectAndOffset start =
      ComputeLayoutObjectAndOffsetModTextFragment(start_expanded,
                                                  start_offset.value());
  MarkSelected(invalidation_set, start.layout_object, SelectionState::kStart);
  if (start_expanded.secondary &&
      start_expanded.secondary != start.layout_object)
    MarkSelected(invalidation_set, start_expanded.secondary,
                 SelectionState::kInside);
  return {start.layout_object, start.offset};
}

LayoutObjectAndOffset ComputeAndMarkEndLayoutObjectAndOffset(
    PaintInvalidationSet* invalidation_set,
    LayoutObject* end_layout_object,
    Optional<int> end_offset) {
  if (!end_offset.has_value()) {
    // |end_layout_object| is not LayoutText.
    MarkSelected(invalidation_set, end_layout_object, SelectionState::kEnd);
    return {end_layout_object, {}};
  }
  const ExpandedLayoutText end_expanded(end_layout_object);
  const LayoutObjectAndOffset end = ComputeLayoutObjectAndOffsetModTextFragment(
      end_expanded, end_offset.value());
  MarkSelected(invalidation_set, end.layout_object, SelectionState::kEnd);
  if (end_expanded.secondary == end.layout_object)
    MarkSelected(invalidation_set, end_expanded.primary,
                 SelectionState::kInside);
...
static SelectionMarkingRange MarkStartAndEndInTwoNodes(
    ...
    LayoutObject* end_layout_object,
    Optional<int> end_offset) {
  const LayoutObjectAndOffset& start = ComputeAndMarkStartLayoutObjectAndOffset(
      &invalidation_set, start_layout_object, start_offset);
  const LayoutObjectAndOffset& end = ComputeAndMarkEndLayoutObjectAndOffset(
      &invalidation_set, end_layout_object, end_offset);
  return {{start.layout_object, start.offset, end.layout_object, end.offset},
          std::move(invalidation_set)};
}
...
static SelectionMarkingRange CalcSelectionRangeAndSetSelectionState(
  ...
  const Optional<int> start_offset = ComputeStartOffset(
      *start_layout_object, selection.StartPosition().ToOffsetInAnchor());
  const Optional<int> end_offset = ComputeEndOffset(
