...
SelectionInDOMTree LayoutSelection::ComputeLayoutSelection(
  ...
  if (paint_range.IsNull())
    return {};
  const PositionInFlatTree start_in_flat =
      paint_range.StartOffset().has_value()
          ? PositionInFlatTree(*paint_range.StartLayoutObject()->GetNode(),
                               paint_range.StartOffset().value())
          : PositionInFlatTree::BeforeNode(
                *paint_range.StartLayoutObject()->GetNode());
  const PositionInFlatTree end_in_flat =
      paint_range.EndOffset().has_value()
          ? PositionInFlatTree(*paint_range.EndLayoutObject()->GetNode(),
                               paint_range.EndOffset().value())
          : PositionInFlatTree::AfterNode(
                *paint_range.EndLayoutObject()->GetNode());

  // We lost order in DOM.
  const Position& position_1 = ToPositionInDOMTree(start_in_flat);
  const Position& position_2 = ToPositionInDOMTree(end_in_flat);
  SelectionInDOMTree::Builder builder;
  if (position_1 == position_2)
    return builder.Collapse(position_1).Build();
  const EphemeralRange range = position_1 < position_2
                                   ? EphemeralRange(position_1, position_2)
                                   : EphemeralRange(position_2, position_1);
