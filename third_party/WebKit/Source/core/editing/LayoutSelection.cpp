...
static WTF::Optional<unsigned> GetNGOffset(
  ...
  if (!layout_object->IsText())
    return WTF::nullopt;
  const Position& position_in_dom = ToPositionInDOMTree(position);
...
static NewPaintRangeAndSelectedLayoutObjects ComputeNewRangeForNG(
  ...
  const WTF::Optional<unsigned> start_offset = GetNGOffset(
      start, new_range.PaintRange().StartOffset(), selection.StartPosition());

  LayoutObject* const end = new_range.PaintRange().EndLayoutObject();
  const WTF::Optional<unsigned> end_offset = GetNGOffset(
      end, new_range.PaintRange().EndOffset(), selection.EndPosition());
...
CalcSelectionRangeAndSetSelectionState(const FrameSelection& frame_selection) {
  ...
  if (!RuntimeEnabledFeatures::LayoutNGPaintFragmentsEnabled())
    return new_range;
  return ComputeNewRangeForNG(new_range, selection);
}
