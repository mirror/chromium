...
// in |SelectionEdtior| and |InvalidSelectionNullifier|.
//
...
class CORE_EXPORT SelectionTemplate final {
 ...
 public:
  ...
  // Nullifies selection at end of life time of the object when base and extent
  // are disconnected or moved to another document.
  class InvalidSelectionNullifier final {
    DISALLOW_NEW();

   public:
    explicit InvalidSelectionNullifier(const SelectionTemplate&);
    ~InvalidSelectionNullifier();
   ...
   private:
    ...
    DISALLOW_COPY_AND_ASSIGN(InvalidSelectionNullifier);
  };
