...
TEST_F(EphemeralRangeTest, rangeTraversalLimitedFlatTree) {
  ...
  const PositionInFlatTree start_position(GetDocument().GetElementById("one"), 0);
  const PositionInFlatTree limit_position(shadow_root->GetElementById("five"), 0);
...
TEST_F(EphemeralRangeTest, commonAncesstorDOM) {
  ...
  EXPECT_EQ(GetDocument().GetElementById("host"), range.CommonAncestorContainer());
}
