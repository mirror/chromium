...
TEST_F(InputMethodControllerTest, CompositionInputEventForConfirm) {
  ...
  GetDocument().setTitle(g_empty_string);
  Controller().FinishComposingText(InputMethodController::kKeepSelection);
  EXPECT_STREQ("compositionend.data:hello;", GetDocument().title().Utf8().Data());
}
...
static String GetMarkedText(DocumentMarkerController& document_marker_controller,
                            Node* node,
                            int marker_index) {
...
TEST_F(InputMethodControllerTest,
  ...
  EXPECT_STREQ(
      "text",
      GetMarkedText(GetDocument().Markers(), div->FirstChild(), 0).Utf8().Data());
}
...
TEST_F(InputMethodControllerTest,
  ...
  EXPECT_STREQ(
      "text",
      GetMarkedText(GetDocument().Markers(), div->FirstChild(), 1).Utf8().Data());
}
...
TEST_F(InputMethodControllerTest,
  ...
  EXPECT_STREQ(
      "\xC2\xA0",  // UTF-8 for an nbsp
      GetMarkedText(GetDocument().Markers(), div->FirstChild(), 2).Utf8().Data());
}
