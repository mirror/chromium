...
std::ostream& operator<<(std::ostream& ostream,
  ...
  if (!inline_box_position.inline_box)
    return ostream << "null";
  return ostream << inline_box_position.inline_box->GetLineLayoutItem().GetNode()
...
TEST_F(VisibleUnitsTest, isVisuallyEquivalentCandidateWithHTMLHtmlElement) {
  ...
  EXPECT_FALSE(
      IsVisuallyEquivalentCandidate(Position(GetDocument().documentElement(), 0)));
...
TEST_F(VisibleUnitsTest, previousPositionOfNoPreviousPosition) {
  SetBodyContent(
      ...
      "</span>");
  const Position position(GetDocument().GetElementById("anchor")->FirstChild(), 1);
  EXPECT_EQ(
      Position(),
...
TEST_F(VisibleUnitsTest, startOfParagraph) {
  ...
  EXPECT_FALSE(
      StartOfParagraph(CreateVisiblePosition(Position(style_text, 0))).IsNull());
}
...
TEST_F(VisibleUnitsTest, localSelectionRectOfPositionTemplateNotCrash) {
  ...
  IntRect rect = AbsoluteSelectionBoundsOf(VisiblePosition::Create(
      PositionWithAffinity(Position(node, PositionAnchorType::kAfterChildren))));
