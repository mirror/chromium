...
TEST_F(PositionTest, ToPositionInFlatTreeDocument) {
  SetBodyContent("foo");
  Position pos = {GetDocument(), 0};
  PositionInFlatTree pos_in_flat = {GetDocument(), 0};
  PositionInFlatTree pos_to_flat = ToPositionInFlatTree(pos);
  EXPECT_EQ(pos_to_flat, pos_in_flat);
}
