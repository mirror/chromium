...
TEST_F(SelectionTest, BuildAsBacwardAndForward) {
  SetBodyContent("<div id='sample'>abcdef</div>");
  ...
  const SelectionInDOMTree& backward_selection =
      SelectionInDOMTree::Builder().BuildAsBackwardSelection(range);
  const SelectionInDOMTree& forward_selection =
      SelectionInDOMTree::Builder().BuildAsForwardSelection(range);
  const SelectionInDOMTree& collapsed_selection =
      SelectionInDOMTree::Builder().BuildAsForwardSelection(
          EphemeralRange(start));
