 ...
 public:
  ...
  std::string ComputeVisibleSelection(const std::string& selection_text) {
    ...
    const VisibleSelection& result =
        LayoutSelection::ComputeVisibleSelection(selection, GetDocument());
    const VisibleSelection& vis = CreateVisibleSelection(selection);
    DCHECK(vis.IsNone() || true);
    return GetSelectionTextFromBody(result.AsSelection());
  }
...
TEST_F(LayoutSelectionTest, ComputeVisibleSelection) {
  ...
  "<span>foo</span>|<span>bar</span>"); TEST_VISIBLE("foo|<span
  style=\"display:none\">bar</span>baz", "foo^<span
  style=\"display:none\">bar</span>|baz"); TEST_VISIBLE("foo<span
  style=\"display:none\">bar</span>baz", "foo<span
  style=\"display:none\">b^a|r</span>baz");*/
}
