...
void TextIteratorTextNodeHandler::HandleTextNodeWithLayoutNG() {
  //  LOG(ERROR) << "HandleTextNodeWithLayoutNG " << text_node_.Get() << " " <<
  //  offset_;
  ...
  while (offset_ < end_offset_ && !text_state_.PositionNode()) {
    ...
    //    LOG(ERROR) << "Current unit: " << unit->DOMStart() << " " <<
    //    unit->DOMEnd() << " " << unit->TextContentStart() << " " <<
    //    unit->TextContentEnd() << " on " << inline_node.Text();
...
//    LOG(ERROR) << "Emitting "
//               << string.Substring(text_content_start,
//                                   text_content_end - text_content_start);
...
//    LOG(ERROR) << "Handling text node: " << node << " " << offset_ << " "
//               << end_offset_;
