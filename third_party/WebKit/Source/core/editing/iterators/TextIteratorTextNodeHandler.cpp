...
// TODO(layout-dev): Try not to include it. NGLayoutResult is not really needed
// by this file. It only appears as an incomplete class in NGInlineNode:
// RefPtr<NGLayoutResult> NGInlineNode::Layout(...);
// However, Windows debug compiler requires a complete class to compile.
...
TextIteratorTextNodeHandler::~TextIteratorTextNodeHandler() = default;
...
//  LOG(ERROR) << "HandleTextNodeWithLayoutNG " << text_node_.Get() << " " << offset_;
...
//    LOG(ERROR) << *text_node_ << " skipped due to invisibility";
    offset_ = end_offset_;
    return;
  }

  NGInlineNode inline_node(ng_block_flow_);
...
//      if (unit)
//        LOG(ERROR) << "Current unit: " << unit->DOMStart() << " " << unit->DOMEnd() << " " << unit->TextContentStart() << " " << unit->TextContentEnd() << " on " << inline_node.Text();
//      else {
//        LOG(ERROR) << "No unit found, abort";
//        text_node_->GetLayoutObject()->ShowLayoutTreeForThis();
//      }
...
//    LOG(ERROR) << "Current unit: " << unit->DOMStart() << " " << unit->DOMEnd() << " " << unit->TextContentStart() << " " << unit->TextContentEnd() << " on " << inline_node.Text();
    ...
    auto string_and_offsets =
        ComputeTextAndOffsetsForEmission(inline_node, *unit, run_end);
...
TextIteratorTextNodeHandler::ComputeTextAndOffsetsForEmission(
    const NGInlineNode& inline_node,
    const NGOffsetMappingUnit& unit,
    unsigned run_end) {
  ...
  if (behavior_.EmitsSpaceForNbsp()) {
    ...
    return std::make_pair(string, std::make_pair(0u, 0u));
  }
  return std::make_pair(inline_node.Text(),
...
void TextIteratorTextNodeHandler::HandleTextNodeInRange(Text* node,
  ...
  if (Optional<NGInlineNode> inline_node = GetNGInlineNodeFor(*node)) {
    // Recover end offset from magic value.
    if (end_offset_ == kMaxOffset) {
      end_offset_ = node->data().length();
//      LOG(ERROR) << "end_offset_ adjusted to " << end_offset_;
    }
//    LOG(ERROR) << "Handling text node: " << node << " " << offset_ << " " << end_offset_;
    ng_block_flow_ = ToLayoutNGBlockFlow(inline_node->GetLayoutBlockFlow());
    HandleTextNodeWithLayoutNG();
    return;
  }
