...
TEST_F(SimplifiedBackwardsTextIteratorTest, CopyTextToWithFirstLetterPart) {
  InsertStyleElement("b::first-letter {font-size: 200%}");
  ...
  EXPECT_EQ(" ", ExtractStringInRange("^<b> |[(3)]678</b>"));
  EXPECT_EQ(" [", ExtractStringInRange("^<b> [|(3)]678</b>"));
  EXPECT_EQ(" [(", ExtractStringInRange("^<b> [(|3)]678</b>"));
  EXPECT_EQ(" [(3", ExtractStringInRange("^<b> [(3|)]678</b>"));
  EXPECT_EQ(" [(3)", ExtractStringInRange("^<b> [(3)|]678</b>"));
  EXPECT_EQ(" [(3)]", ExtractStringInRange("^<b> [(3)]|678</b>"));
  EXPECT_EQ(" [(3)]6", ExtractStringInRange("^<b> [(3)]6|78</b>"))
      << "|iterator| in remaining part";

  EXPECT_EQ("(3)", ExtractStringInRange("<b> [^(3)|]678</b>"))
      << "|iterator| in first-letter part";

  EXPECT_EQ("789a", ExtractStringInRange("<b> [(3)]6^789a|bc</b>"))
      << "|iterator| in remaining part";

  EXPECT_EQ("(3)]67", ExtractStringInRange("<b> [^(3)]67|8</b>"))
      << "|iterator| from remaining part to first-letter part.";
}
