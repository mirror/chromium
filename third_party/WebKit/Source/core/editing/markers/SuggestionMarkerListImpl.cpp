...
bool IsLetterOrDigit(UChar32 c) {
...
bool ShouldRemoveMarkerAfterNonSuggestionEditingOperation(
  ...
  if (offset == marker_end && new_length > 0)
    return IsLetterOrDigit(GetCodePointAt(node_text, offset));
  ...
  if (offset == marker_start && new_length > 0)
    return IsLetterOrDigit(GetCodePointAt(node_text, offset + new_length - 1));
