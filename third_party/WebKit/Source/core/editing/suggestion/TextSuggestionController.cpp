...
void TextSuggestionController::ApplyTextSuggestion(uint32_t marker_tag,
  ...
  marker->ReplaceSuggestion(suggestion_index, new_suggestion);
...
void TextSuggestionController::ShowSuggestionMenu(
  ...
  if (first_suggestion_marker.SuggestionHighlightColor() != 0) {
    highlight_color = first_suggestion_marker.SuggestionHighlightColor();
  } else {
    highlight_color = LayoutTheme::TapHighlightColor();
  }

  const EphemeralRange marker_range =
      EphemeralRange(Position(text_node, span_union_start),
                     Position(text_node, span_union_end));
  const String& misspelled_word = PlainText(marker_range);
