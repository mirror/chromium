...
TEST_F(TextSuggestionControllerTest, DeleteActiveSuggestionRange_DeleteAtEnd) {
  ...
  GetDocument().Markers().AddActiveSuggestionMarker(
      EphemeralRange(Position(text, 6), Position(text, 11)), true,
      Color::kBlack, StyleableMarker::Thickness::kThin, Color::kBlack);
...
TEST_F(TextSuggestionControllerTest,
  ...
  GetDocument().Markers().AddActiveSuggestionMarker(
      EphemeralRange(Position(text, 0), Position(text, 5)), true, Color::kBlack,
...
TEST_F(TextSuggestionControllerTest,
  ...
  // Mark "word2" as the active suggestion range
  GetDocument().Markers().AddActiveSuggestionMarker(
      EphemeralRange(Position(text, 5), Position(text, 10)), true,
      Color::kBlack, StyleableMarker::Thickness::kThin, Color::kBlack);
  // Select immediately before word2
  GetDocument().GetFrame()->Selection().SetSelection(
      SelectionInDOMTree::Builder()
