...
void IdleSpellCheckCallback::SetNeedsInvocation() {
  ...
  idle_callback_handle_ = GetFrame().GetDocument()->RequestIdleCallback(this, options);
...
void IdleSpellCheckCallback::HotModeInvocation(IdleDeadline* deadline) {
  ...
  requester.CheckSpellingAt(GetFrame().Selection().GetSelectionInDOMTree().Extent());

  const uint64_t watermark = last_processed_undo_step_sequence_;
  for (const UndoStep* step : GetFrame().GetEditor().GetUndoStack().UndoSteps()) {
