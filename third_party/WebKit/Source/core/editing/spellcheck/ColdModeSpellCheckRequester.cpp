...
void ColdModeSpellCheckRequester::Invoke(IdleDeadline* deadline) {
  ...
  if (last_checked_dom_tree_version_ != GetFrame().GetDocument()->DomTreeVersion())
    ResetCheckingProgress();
...
void ColdModeSpellCheckRequester::SearchForNextRootEditable() {
  ...
  next_node_ = FlatTreeTraversal::Next(*next_node_, GetFrame().GetDocument()->body());
}
...
void ColdModeSpellCheckRequester::RequestCheckingForNextChunk() {
  ...
  GetSpellCheckRequester().RequestCheckingFor(check_range, current_chunk_index_);
