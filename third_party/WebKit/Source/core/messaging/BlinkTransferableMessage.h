...
struct CORE_EXPORT BlinkTransferableMessage : BlinkCloneableMessage {
  ...
  static BlinkTransferableMessage FromCommon(TransferableMessage);
  // Returned message will still be backed by the SerializedScriptValue in this
  // message, so is only valid as long as that SerializedScriptValue is alive.
  // Call EnsureDataIsOwned on the returned message if you need it to live
  // longer.
  TransferableMessage AsCommon();
