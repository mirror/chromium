...
bool ToImageBitmapsContentArray(
    const Vector<SkBitmap>& sk_bitmaps,
    blink::SerializedScriptValue::ImageBitmapContentsArray& dest) {
  dest.ReserveInitialCapacity(sk_bitmaps.size());
...
bool StructTraits<blink::mojom::blink::TransferableMessage::DataView,
  ...
  if (!ToImageBitmapsContentArray(sk_bitmaps, image_bitmap_contents_array)) {
    return false;
  }
