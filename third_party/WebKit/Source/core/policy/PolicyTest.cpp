...
class FramePolicyTest : public PolicyTest {
 public:
  void UpdatePolicy(const ParsedFeaturePolicy& container_policy,
                    const String& origin) {
    if (policy_) {
      policy_->UpdateContainerPolicy(container_policy,
                                     SecurityOrigin::CreateFromString(origin));
      return;
    }
    policy_ =
        Policy::Create(Policy::Type::FramePolicy, document_, container_policy,
                       SecurityOrigin::CreateFromString(origin));
  }
};
...
TEST_F(FramePolicyTest, TestCombinedPolicy) {
  ...
  UpdatePolicy(container_policy, kOriginA);
  Vector<String> allowed_features = GetPolicy()->allowedFeatures();
  EXPECT_TRUE(allowed_features.Contains("fullscreen"));
