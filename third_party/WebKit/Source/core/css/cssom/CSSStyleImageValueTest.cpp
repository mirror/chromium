...
class FakeCSSStyleImageValue : public CSSStyleImageValue {
 public:
  ...
  // CSSStyleImageValue
  WTF::Optional<IntSize> IntrinsicSize() const final { return size_; }
...
TEST(CSSStyleImageValueTest, PendingCache) {
  FakeCSSStyleImageValue* style_image_value =
      new FakeCSSStyleImageValue(true, IntSize(100, 100));
  bool is_null;
...
TEST(CSSStyleImageValueTest, ValidLoadedImage) {
  FakeCSSStyleImageValue* style_image_value =
      new FakeCSSStyleImageValue(false, IntSize(480, 120));
  bool is_null;
