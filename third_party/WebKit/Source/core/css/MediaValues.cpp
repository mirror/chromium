...
int MediaValues::CalculateDeviceWidth(LocalFrame* frame) {
  ...
  blink::WebScreenInfo screen_info = frame->GetPage()->GetChromeClient().GetScreenInfo();
...
int MediaValues::CalculateColorBitsPerComponent(LocalFrame* frame) {
  ...
  if (!frame->GetPage()->MainFrame()->IsLocalFrame() ||
    ...
    return 0;
  return frame->GetPage()->GetChromeClient().GetScreenInfo().depth_per_component;
}
...
WebDisplayMode MediaValues::CalculateDisplayMode(LocalFrame* frame) {
  DCHECK(frame);
  WebDisplayMode mode = frame->GetPage()->GetSettings().GetDisplayModeOverride();
