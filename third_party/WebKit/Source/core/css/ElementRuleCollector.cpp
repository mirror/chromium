...
void ElementRuleCollector::CollectMatchingRules(
  ...
  if (!pseudo_id.IsEmpty()) {
    ...
    CollectMatchingRulesForList(
        match_request.rule_set->ShadowPseudoElementRules(pseudo_id), cascade_order,
        match_request);
...
void ElementRuleCollector::DidMatchRule(
  ...
  if (dynamic_pseudo != kPseudoIdNone &&
    ...
    if (!style_ || dynamic_pseudo >= kFirstInternalPseudoId)
      return;
    if ((dynamic_pseudo == kPseudoIdBefore || dynamic_pseudo == kPseudoIdAfter) &&
    ...
    matched_rules_.push_back(
        MatchedRule(&rule_data, result.specificity, cascade_order,
