...
constexpr bool kAllowTextValueTrue = true;
constexpr bool kAllowTextValueFalse = false;
constexpr bool kUseLegacyParsingFalse = false;
  ...
  switch (resolved_property) {
    ...
    case CSSPropertyWebkitMaskSize:
      return CSSPropertyBackgroundUtils::ConsumeBackgroundSize(
          range, context.Mode(), kUseLegacyParsingFalse);
    case CSSPropertyBackgroundColor:
      return CSSPropertyParserHelpers::ConsumeColor(range, context.Mode());
    case CSSPropertyWebkitMaskClip:
      return CSSPropertyBackgroundUtils::ConsumePrefixedBackgroundBox(
          range, &context, kAllowTextValueTrue);
    case CSSPropertyWebkitMaskOrigin:
      return CSSPropertyBackgroundUtils::ConsumePrefixedBackgroundBox(
          range, &context, kAllowTextValueFalse);
    default:
      break;
  };
  return nullptr;
    ...
    do {
      ...
      for (size_t i = 0; i < longhand_count; ++i) {
        ...
        if (property == CSSPropertyBackgroundRepeatX ||
          ...
          if (!ConsumePosition(range, context,
