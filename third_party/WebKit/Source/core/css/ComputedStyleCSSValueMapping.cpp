...
static CSSValue* ValueForFillSize(const FillSize& fill_size,
  ...
  list->Append(*ZoomAdjustedPixelValueForLength(fill_size.size.Height(), style));
  return list;
}

static CSSValue* ValueForFillRepeat(EFillRepeat x_repeat, EFillRepeat y_repeat) {
...
static CSSValue* ValueForNinePieceImage(const NinePieceImage& image,
  ...
  return CreateBorderImageValue(image_value, image_slices, border_slices, outset,
                                repeat);
}
...
static CSSValue* ValuesForFontVariantProperty(const ComputedStyle& style,
  ...
  for (size_t i = 0; i < fontVariantShorthand().length(); ++i) {
    const CSSValue* value = ComputedStyleCSSValueMapping::Get(
        fontVariantShorthand().properties()[i], style, layout_object, styled_node,
        allow_visited_style);
...
static CSSValueList* ValueForFontFamily(const ComputedStyle& style) {
  ...
  for (const FontFamily* family = &first_family; family; family = family->Next())
...
static CSSValue* ValueForFontVariantLigatures(const ComputedStyle& style) {
  ...
  if (common_ligatures_state == FontDescription::kDisabledLigaturesState &&
      discretionary_ligatures_state == FontDescription::kDisabledLigaturesState &&
...
static CSSValue* ValueForFontVariantNumeric(const ComputedStyle& style) {
  ...
  if (variant_numeric.NumericSpacingValue() !=
    ...
    value_list->Append(
        *CSSIdentifierValue::Create(variant_numeric.NumericSpacingValue() ==
  ...
  switch (track_size.GetType()) {
    ...
    case kMinMaxTrackSizing: {
      if (track_size.MinTrackBreadth().IsAuto() &&
        ...
        return CSSPrimitiveValue::Create(track_size.MaxTrackBreadth().Flex(),
                                         CSSPrimitiveValue::UnitType::kFraction);
      }
...
class OrderedNamedLinesCollector {
 ...
 public:
  OrderedNamedLinesCollector(const ComputedStyle& style,
      ...
      : ordered_named_grid_lines_(is_row_axis ? style.OrderedNamedGridColumnLines()
        ...
        insertion_point_(is_row_axis ? style.GridAutoRepeatColumnsInsertionPoint()
                                   : style.GridAutoRepeatRowsInsertionPoint()),
        auto_repeat_total_tracks_(auto_repeat_tracks_count),
        auto_repeat_track_list_length_(is_row_axis
...
void OrderedNamedLinesCollector::AppendLines(
  ...
  auto end_iter = type == kNamedLines ? ordered_named_grid_lines_.end()
                                    : ordered_named_auto_repeat_grid_lines_.end();
  if (iter == end_iter)
    return;

  for (auto line_name : iter->value)
    line_names_value.Append(*CSSCustomIdentValue::Create(AtomicString(line_name)));
}
...
static CSSValue* ValueForGridTrackList(GridTrackSizingDirection direction,
  ...
  size_t auto_repeat_total_tracks =
      is_layout_grid
          ...
          : 0;
  OrderedNamedLinesCollector collector(style, is_row_axis, auto_repeat_total_tracks);
  CSSValueList* list = CSSValueList::CreateSpaceSeparated();
...
static CSSValue* ValueForAnimationDuration(const CSSTimingData* timing_data) {
  ...
  if (timing_data) {
    for (size_t i = 0; i < timing_data->DurationList().size(); ++i)
      list->Append(*CSSPrimitiveValue::Create(
          timing_data->DurationList()[i], CSSPrimitiveValue::UnitType::kSeconds));
  } else {
    list->Append(
...
const CSSValue* ComputedStyleCSSValueMapping::Get(
  ...
  switch (property_id) {
    ...
    case CSSPropertyWebkitMaskComposite: {
      CSSValueList* list = CSSValueList::CreateCommaSeparated();
      const FillLayer* curr_layer = property_id == CSSPropertyWebkitMaskComposite
                                       ? &style.MaskLayers()
                                       : &style.BackgroundLayers();
      for (; curr_layer; curr_layer = curr_layer->Next())
    ...
    case CSSPropertyWebkitMaskPositionX: {
      CSSValueList* list = CSSValueList::CreateCommaSeparated();
      const FillLayer* curr_layer = property_id == CSSPropertyWebkitMaskPositionX
                                       ? &style.MaskLayers()
                                       : &style.BackgroundLayers();
      for (; curr_layer; curr_layer = curr_layer->Next())
        list->Append(
    ...
    case CSSPropertyWebkitMaskPositionY: {
      CSSValueList* list = CSSValueList::CreateCommaSeparated();
      const FillLayer* curr_layer = property_id == CSSPropertyWebkitMaskPositionY
                                       ? &style.MaskLayers()
                                       : &style.BackgroundLayers();
      for (; curr_layer; curr_layer = curr_layer->Next())
        list->Append(
    ...
    case CSSPropertyGridGap:
      return ValuesForShorthandProperty(gridGapShorthand(), style, layout_object,
                                        styled_node, allow_visited_style);
    ...
    case CSSPropertyColumns:
      return ValuesForShorthandProperty(columnsShorthand(), style, layout_object,
                                        styled_node, allow_visited_style);
    ...
    case CSSPropertyOutline:
      return ValuesForShorthandProperty(outlineShorthand(), style, layout_object,
                                        styled_node, allow_visited_style);
