...
using StyleSheetId = AtomicString;
...
class CORE_EXPORT StyleEngine final
 ...
 public:
  ...
  const HeapVector<
      std::pair<StyleSheetId, TraceWrapperMember<CSSStyleSheet>>>&
  ...
  void InjectSheet(const StyleSheetId&, StyleSheetContents*,
                   WebDocument::CSSOrigin =
                       WebDocument::kAuthorOrigin);
  void RemoveInjectedSheet(const StyleSheetId&);
 ...
 private:
  ...
  HeapVector<std::pair<StyleSheetId, TraceWrapperMember<CSSStyleSheet>>>
