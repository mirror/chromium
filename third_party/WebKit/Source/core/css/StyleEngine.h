...
class CORE_EXPORT StyleEngine final
 ...
 public:
  ...
  const HeapVector<
      std::pair<AtomicString, TraceWrapperMember<CSSStyleSheet>>>&
  ...
  void InjectSheet(const AtomicString&, StyleSheetContents*,
                   WebDocument::CSSOrigin =
                       WebDocument::kAuthorOrigin);
  void RemoveInjectedSheet(const AtomicString&);
 ...
 private:
  ...
  HeapVector<std::pair<AtomicString, TraceWrapperMember<CSSStyleSheet>>>
