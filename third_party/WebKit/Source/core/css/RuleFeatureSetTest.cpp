...
TEST_F(RuleFeatureSetTest, pseudoMatchesTooLarge) {
  // To prevent excessive use of :matches() and its expansion, the expansion and
  // styling is limited to a maximum selector list length of 10,000
  EXPECT_EQ(
      RuleFeatureSet::kSelectorNeverMatches,
      CollectFeatures(":matches(.a, .b, .c, .d) + :matches(.e, .f, .g, .h) + "
