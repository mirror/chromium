...
bool MatchedPropertiesCache::IsStyleCacheable(const ComputedStyle& style) {
  ...
  if (style.Unique())
    return false;
  if (style.Zoom() != ComputedStyleInitialFunctions::InitialZoom())
    return false;
  if (style.GetWritingMode() != ComputedStyleInitialFunctions::InitialWritingMode() ||
      style.Direction() != ComputedStyleInitialFunctions::InitialDirection())
    return false;
