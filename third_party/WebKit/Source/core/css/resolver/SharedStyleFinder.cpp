...
bool SharedStyleFinder::CanShareStyleWithControl(Element& candidate) const {
  ...
  if (candidate_input.IsPlaceholderVisible() != this_input.IsPlaceholderVisible())
    return false;
...
bool SharedStyleFinder::SharingCandidateHasIdenticalStyleAffectingAttributes(
  ...
  if (isHTMLOptGroupElement(GetElement()) || isHTMLOptionElement(GetElement())) {
    if (GetElement().IsDisabledFormControl() != candidate.IsDisabledFormControl())
      return false;
...
bool SharedStyleFinder::MatchesRuleSet(RuleSet* rule_set) {
  if (!rule_set)
    return false;
  ElementRuleCollector collector(context_, style_resolver_->GetSelectorFilter());
  return collector.HasAnyMatchingRules(rule_set);
}

ComputedStyle* SharedStyleFinder::FindSharedStyle() {
  INCREMENT_STYLE_STATS_COUNTER(GetDocument().GetStyleEngine(), shared_style_lookups,
                                1);
  ...
  element_affected_by_class_rules_ =
      GetElement().HasClass() && ClassNamesAffectedByRules(GetElement().ClassNames());
  ...
  if (!share_element) {
    if (GetDocument().GetStyleEngine().Stats() &&
      ...
      INCREMENT_STYLE_STATS_COUNTER(GetDocument().GetStyleEngine(), shared_style_missed,
                                    1);
    return nullptr;
  }

  INCREMENT_STYLE_STATS_COUNTER(GetDocument().GetStyleEngine(), shared_style_found, 1);
  ...
  if (!GetElement().ParentElementOrShadowRoot()->ChildrenSupportStyleSharing()) {
    INCREMENT_STYLE_STATS_COUNTER(GetDocument().GetStyleEngine(),
