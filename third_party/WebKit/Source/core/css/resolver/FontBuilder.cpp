...
float FontBuilder::GetComputedSizeFromSpecifiedSize(
  ...
  return FontSize::GetComputedSizeFromSpecifiedSize(
      document_, zoom_factor, font_description.IsAbsoluteSize(), specified_size);
}
...
void FontBuilder::UpdateComputedSize(FontDescription& font_description,
  ...
  float computed_size = GetComputedSizeFromSpecifiedSize(
      font_description, style.EffectiveZoom(), font_description.SpecifiedSize());
  float multiplier = style.TextAutosizingMultiplier();
  computed_size =
...
void FontBuilder::CreateFont(FontSelector* font_selector, ComputedStyle& style) {
  if (!flags_)
    return;
