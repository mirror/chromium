...
bool CSSParserImpl::SupportsDeclaration(CSSParserTokenRange& range) {
  ...
  // the range offset. Yet this passes all the tests, so investigate why.
...
StyleRuleBase* CSSParserImpl::ConsumeAtRule(CSSParserTokenStream& stream,
  ...
  stream.UncheckedConsumeUntilLookAheadIs<kLeftBraceToken, kSemicolonToken>();
...
StyleRuleBase* CSSParserImpl::ConsumeQualifiedRule(
  ...
  if (observer_wrapper_) {
    ...
    stream.UncheckedConsumeUntilLookAheadIs<kLeftBraceToken>();
  }
  ...
  while (true) {
    ...
    switch (stream.UncheckedPeek().GetType()) {
      ...
      case kIdentToken: {
        ...
        stream.UncheckedConsumeUntilLookAheadIs<kSemicolonToken>();
