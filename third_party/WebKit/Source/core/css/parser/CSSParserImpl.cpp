...
#include "core/css/parser/CSSParserRangeBuffer.h"
...
StyleRuleBase* CSSParserImpl::ConsumeAtRule(CSSParserTokenStream& stream,
  ...
  CSSParserRangeBuffer prelude_buffer(stream);
...
StyleRule* CSSParserImpl::ConsumeStyleRule(CSSParserRangeBuffer prelude_buffer,
                                           const RangeOffset& prelude_offset,
                                           CSSParserTokenStream& block) {
  const CSSParserTokenRange prelude = prelude_buffer.Range();
  CSSSelectorList selector_list =
  ...
  while (true) {
    ...
    switch (stream.UncheckedPeek().GetType()) {
      ...
      case kIdentToken: {
        ...
        CSSParserRangeBuffer decl_buffer(stream);
        while (!stream.UncheckedAtEnd() &&
               stream.UncheckedPeek().GetType() != kSemicolonToken) {
