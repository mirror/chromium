...
TEST_F(CSSLazyParsingTest, Simple) {
  ...
  CSSParser::ParseSheet(context, style_sheet, sheet_text, true /* lazy parse */);
...
TEST_F(CSSLazyParsingTest, ChangeDocuments) {
  ...
  cached_contents_ = StyleSheetContents::Create(context);
  {
    ...
    UseCounter& use_counter1 = dummy_holder->GetDocument().GetPage()->GetUseCounter();
  ...
  UseCounter& use_counter2 = dummy_holder2->GetDocument().GetPage()->GetUseCounter();
  EXPECT_TRUE(sheet2);
