...
static void VerifyCSSCalc(String text,
  ...
  const CSSValue* css_value = CSSParser::ParseSingleValue(CSSPropertyLeft, text);
...
TEST(SizesCalcParserTest, Basic) {
  ...
  for (unsigned i = 0; test_cases[i].input; ++i) {
    if (test_cases[i].dont_run_in_css_calc)
      continue;
    VerifyCSSCalc(test_cases[i].input, test_cases[i].output, test_cases[i].valid,
                  data.default_font_size, data.viewport_width,
