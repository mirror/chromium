...
void CSSPropertyParser::AddProperty(CSSPropertyID property,
  ...
  parsed_properties_->push_back(CSSProperty(
      property, value, important, set_from_shorthand, shorthand_index, implicit));
}
...
bool CSSPropertyParser::ParseValue(
  ...
  if (rule_type == StyleRule::kViewport) {
    parse_success = (RuntimeEnabledFeatures::cssViewportEnabled() ||
      ...
      if (parsed_value && filter_type != CSSValueSaturate &&
        ...
        if (ToCSSPrimitiveValue(parsed_value)->GetDoubleValue() > max_allowed) {
          parsed_value = CSSPrimitiveValue::Create(
              max_allowed, is_percentage ? CSSPrimitiveValue::UnitType::kPercentage
...
bool CSSPropertyParser::ConsumeOffsetShorthand(bool important) {
  ...
  AddProperty(CSSPropertyOffsetPath, CSSPropertyOffset, *offset_path, important);
...
static bool ConsumeTranslate3d(CSSParserTokenRange& args,
  ...
  CSSValue* parsed_value = nullptr;
  do {
    parsed_value = ConsumeLengthOrPercent(args, css_parser_mode, kValueRangeAll);
    if (!parsed_value)
      return false;
...
static CSSValue* ConsumeTransformValue(CSSParserTokenRange& range,
  ...
  switch (function_id) {
    ...
    case CSSValueSkew:
      ...
      if (!parsed_value)
        return nullptr;
      if (function_id == CSSValueSkew && ConsumeCommaIncludingWhitespace(args)) {
        transform_value->Append(*parsed_value);
        parsed_value = ConsumeAngle(args);
        if (!parsed_value)
          return nullptr;
...
static CSSValue* ConsumeBorderImageWidth(CSSParserTokenRange& range) {
  ...
  for (size_t index = 0; index < 4; ++index) {
    ...
    if (!value)
      value =
          ConsumeLengthOrPercent(range, kHTMLStandardMode, kValueRangeNonNegative,
...
static bool ParseGridTemplateAreasRow(const String& grid_row_names,
  ...
  for (size_t current_column = 0; current_column < column_count; ++current_column) {
    ...
    if (grid_area_it == grid_area_map.end()) {
      grid_area_map.insert(
          grid_area_name,
          GridArea(GridSpan::TranslatedDefiniteGridSpan(row_count, row_count + 1),
                   GridSpan::TranslatedDefiniteGridSpan(current_column,
                                                        look_ahead_column)));
    } else {
...
static CSSValue* ConsumeGridTrackSize(CSSParserTokenRange& range,
  ...
  if (token.FunctionId() == CSSValueMinmax) {
    ...
    if (!min_track_breadth || (min_track_breadth->IsPrimitiveValue() &&
  ...
  do {
    ...
    if (range.Peek().FunctionId() == CSSValueRepeat) {
      ...
      if (!ConsumeGridTrackRepeatFunction(range, css_parser_mode, *values,
                                          is_auto_repeat, all_tracks_are_fixed_sized))
        return nullptr;
...
static CSSValue* ConsumeGridTemplateAreas(CSSParserTokenRange& range) {
  ...
  while (range.Peek().GetType() == kStringToken) {
    if (!ParseGridTemplateAreasRow(
            range.ConsumeIncludingWhitespace().Value().ToString(), grid_area_map,
  ...
  if (row_count == 0)
    return nullptr;
  DCHECK(column_count);
  return CSSGridTemplateAreasValue::Create(grid_area_map, row_count, column_count);
}
...
bool CSSPropertyParser::ConsumeSystemFont(bool important) {
  ...
  LayoutTheme::GetTheme().SystemFont(system_font_id, font_style, font_weight, font_size,
                                  font_family);
  ...
  AddProperty(
      ...
      *CSSPrimitiveValue::Create(font_size, CSSPrimitiveValue::UnitType::kPixels),
      important);
...
bool CSSPropertyParser::ConsumeFont(bool important) {
  ...
  while (!range_.AtEnd()) {
    ...
    if (!font_variant_caps && (id == CSSValueNormal || id == CSSValueSmallCaps)) {
  ...
  AddProperty(
      ...
      font_stretch ? *font_stretch : *CSSIdentifierValue::Create(CSSValueNormal),
      important);
  ...
  if (ConsumeSlashIncludingWhitespace(range_)) {
    ...
    if (!line_height)
      return false;
    AddProperty(CSSPropertyLineHeight, CSSPropertyFont, *line_height, important);
  } else {
    AddProperty(CSSPropertyLineHeight, CSSPropertyFont,
                *CSSIdentifierValue::Create(CSSValueNormal), important);
  }
    ...
    do {
      ...
      for (size_t i = 0; i < longhand_count; ++i) {
        ...
        if (property == CSSPropertyBackgroundRepeatX ||
          ...
          if (!ConsumePosition(range_, context_->Mode(),
                               UnitlessQuirk::kForbid, value, value_y))
            continue;
          ...
          if (!value || !parsed_longhand[i - 1])  // Position must have been
...
bool CSSPropertyParser::ConsumeGridTemplateRowsAndAreasAndColumns(
  ...
  do {
    ...
    if (line_names)
      template_rows->Append(*line_names);
           ...
           !(range_.Peek().GetType() == kDelimiterToken &&
  ...
  AddProperty(CSSPropertyGridTemplateColumns, shorthand_id, *columns_value,
              important);
  AddProperty(
      CSSPropertyGridTemplateAreas, shorthand_id,
...
bool CSSPropertyParser::ConsumeGridShorthand(bool important) {
  ...
  AddProperty(CSSPropertyGridTemplateColumns, CSSPropertyGrid, *template_columns,
              important);
