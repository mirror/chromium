...
bool CSSPropertyParser::ParseValue(
  ...
  if (rule_type == StyleRule::kViewport) {
    ...
    parse_success = parser.ParseValueStart(
        resolved_property, isPropertyAlias(unresolved_property), important);
  }
...
bool CSSPropertyParser::ParseValueStart(CSSPropertyID resolved_property,
                                        bool is_property_alias,
                                        bool important) {
  ...
  if (is_shorthand) {
    ...
    if (CSSPropertyAPI::Get(resolved_property)
            .ParseShorthand(
                ...
                CSSParserLocalContext(is_property_alias, resolved_property),
                *parsed_properties_))
      return true;
  } else {
