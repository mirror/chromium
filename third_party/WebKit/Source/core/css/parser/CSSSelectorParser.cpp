...
CSSSelectorList CSSSelectorParser::ConsumeComplexSelectorList(
  ...
  while (true) {
    CSSParserRangeBuffer selector_buffer(stream);
    ...
    const auto selector_copy = complex_selector;
    ...
    if (wrapper) {
      wrapper->Observer().ObserveSelector(wrapper->StartOffset(selector_copy),
                                          wrapper->EndOffset(complex_selector));
    }
