...
unsigned ExtractCompoundFlags(const CSSParserSelector& simple_selector,
  ...
  if (parser_mode == kUASheetMode &&
      simple_selector.GetPseudoType() == CSSSelector::kPseudoWebKitCustomElement)
    return 0;
...
bool CSSSelectorParser::ConsumeName(CSSParserTokenRange& range,
  ...
  if (range.Peek().GetType() != kDelimiterToken || range.Peek().Delimiter() != '|')
    return true;
  range.Consume();
...
const AtomicString& CSSSelectorParser::DetermineNamespace(
  ...
  if (prefix.IsEmpty())
    return g_empty_atom;  // No namespace. If an element/attribute has a namespace,
                       // we won't match it.
        ...
        case CSSSelector::kPseudoWebKitCustomElement:
          if (context_->Mode() != kUASheetMode) {
            if (current->Value() ==
              ...
              feature =
                  UseCounter::kCSSSelectorInternalMediaControlsOverlayCastButton;
            }
          }
          break;
