...
AtRuleDescriptorValueSet* AtRuleDescriptorValueSet::Create(
    const HeapVector<blink::CSSPropertyValue, 256>& properties, CSSParserMode mode) {
  AtRuleDescriptorValueSet* set = new AtRuleDescriptorValueSet(mode);
...
void AtRuleDescriptorValueSet::SetProperty(AtRuleDescriptorID id, const CSSValue& value) {
  values_.Set(id, value);
}

AtRuleDescriptorValueSet* AtRuleDescriptorValueSet::MutableCopy() const {
  AtRuleDescriptorValueSet* set = new AtRuleDescriptorValueSet(mode_);
...
void AtRuleDescriptorValueSet::ParseDeclarationList(
  ...
  for (unsigned i = 0; i < property_value_set->PropertyCount(); i++) {
    CSSPropertyValueSet::PropertyReference reference = property_value_set->PropertyAt(i);
    values_.Set(CSSPropertyIDAsAtRuleDescriptor(reference.Id()), reference.Value());
  }
}
