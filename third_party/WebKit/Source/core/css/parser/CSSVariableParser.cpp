...
bool IsValidVariableReference(CSSParserTokenRange range, bool& has_at_apply_rule) {
  range.ConsumeWhitespace();
...
bool CSSVariableParser::ContainsValidVariableReferences(
  ...
  bool has_references;
  bool has_at_apply_rule;
  CSSValueID type = ClassifyVariableRange(range, has_references, has_at_apply_rule);
...
CSSCustomPropertyDeclaration* CSSVariableParser::ParseDeclarationValue(
  ...
  if (type == CSSValueInternalVariableValue) {
    return CSSCustomPropertyDeclaration::Create(
        variable_name, CSSVariableData::Create(range, is_animation_tainted,
