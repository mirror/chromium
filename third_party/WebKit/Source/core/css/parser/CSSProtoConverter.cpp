...
void Converter::Visit(const SupportsConditionConjunction& conjunction,
                      std::string conjunction_operator,
                      int depth) {
  string_ += "(";
  Visit(conjunction.condition_1(), depth);
  string_ += " " + conjunction_operator + " ";
  Visit(conjunction.condition_2(), depth);
  string_ += ")";
}

void Converter::Visit(const SupportsCondition& supports_condition, int depth) {
  if (++depth > kSupportsConditionDepthLimit) {
    // Go with the default, since it can't result in another call to this
    // function.
    if (supports_condition.not_condition())
      string_ += " not ";
    string_ += "(";
    Visit(supports_condition.declaration());
    string_ += ")";
  }
  ...
  if (supports_condition.has_and_supports_condition()) {
    Visit(supports_condition.and_supports_condition(), "and", depth);
  } else if (supports_condition.has_or_supports_condition()) {
    Visit(supports_condition.or_supports_condition(), "or", depth);
  } else {
    Visit(supports_condition.declaration());
  }
...
void Converter::Visit(const Declaration& declaration) {
  if (declaration.has_prop_and_val())
    Visit(declaration.prop_and_val());
  // else empty
}

void Converter::Visit(const PropertyAndValue& prop_and_val) {
  Visit(prop_and_val.property());
  string_ += " : ";
  int value_id = 0;
  if (prop_and_val.has_value_id())
    value_id = prop_and_val.value_id();
  Visit(prop_and_val.expr(), value_id);
  if (prop_and_val.has_prio())
    string_ += " !important";
}
