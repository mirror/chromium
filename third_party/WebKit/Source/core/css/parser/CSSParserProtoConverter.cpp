...
#define LOOKUP(id, table) table[static_cast<int>(id)]
...
void Converter::Visit(const CharsetDeclaration& charset_declaration) {
  ...
  string_ += "\"" + LOOKUP(charset_declaration.encoding_id(), kEncodingLookupTable) + "\";";
...
// TODO: implement after doing media_queries.
...
void Converter::Visit(const Property& property) {
  ...
  string_ += "text-indent"; //kPropertyLookupTable[property_id];
}
