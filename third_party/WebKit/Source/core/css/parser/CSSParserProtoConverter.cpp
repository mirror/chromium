...
#define APPEND_LOOKUP(id, table)                                              \
  CHECK(static_cast<int>(id) > 0 && static_cast<size_t>(id) < sizeof(table)); \
  string_ += table[static_cast<int>(id)]

namespace css_parser_proto_fuzzer {

static std::string kPseudoLookupTable[] = {
...
static std::string kMediaTypeLookupTable[] = {
    "",  // This is just to fill the zeroth spot. It should not be used.
    "all",
    "braille",
    "embossed",
...
static std::string kMfNameLookupTable[] = {
    "",  // This is just to fill the zeroth spot. It should not be used.
    "any-hover",
    "any-pointer",
    "color",
...
static std::string kImportLookupTable[] = {
    "",  // This is just to fill the zeroth spot. It should not be used.
    "'custom.css'", "url(\"chrome://communicator/skin/\")"};

static std::string kEncodingLookupTable[] = {
    "",  // This is just to fill the zeroth spot. It should not be used.
    "UTF-8", "UTF-16", "UTF-32",
};

static std::string kValueLookupTable[] = {
    "",  // This is just to fill the zeroth spot. It should not be used.
    "all",
    "dynamic",
    "yellow",
...
void Converter::Visit(const CharsetDeclaration& charset_declaration) {
  ...
  string_ += "\"";
  APPEND_LOOKUP(charset_declaration.encoding_id(), kEncodingLookupTable);
  string_ += "\"; ";
}
...
void Converter::Visit(const Import& import) {
  string_ += "@import ";
  APPEND_LOOKUP(import.src(), kImportLookupTable);
  string_ += " ";
...
void Converter::Visit(const MediaType& media_type) {
  APPEND_LOOKUP(media_type.value(), kMediaTypeLookupTable);
}
...
void Converter::Visit(const MfName& mf_name) {
  APPEND_LOOKUP(mf_name.value(), kMfNameLookupTable);
}
...
void Converter::Visit(const Property& property) {
  APPEND_LOOKUP(property.name(), kPropertyLookupTable);
}
...
void Converter::Visit(const Selector& selector, bool is_first) {
  ...
  if (selector.has_pseudo_value()) {
    string_ += ":";
    if (selector.pseudo_type() == PseudoType::ELEMENT)
      string_ += ":";
    APPEND_LOOKUP(selector.pseudo_value(), kPseudoLookupTable);
  }
}
...
void Converter::Visit(const NonEmptyDeclaration& nonempty_declaration) {
  ...
  if (nonempty_declaration.has_value_id()) {
    value_id = static_cast<int>(nonempty_declaration.value_id());
    DCHECK(value_id > 0 &&
           static_cast<size_t>(value_id) < sizeof(kValueLookupTable));
  }
...
void Converter::Visit(const Expr& expr, int declaration_value_id) {
  if (!declaration_value_id) {
    ...
    string_ += kValueLookupTable[declaration_value_id];
  }
