...
#define APPEND_LOOKUP(id, table)                                               \
  DCHECK(static_cast<int>(id) > 0 && static_cast<size_t>(id) < sizeof(table)); \
...
void Converter::Visit(const Import& import) {
  string_ += "@import ";
  APPEND_LOOKUP(import.src_id(), kImportLookupTable);
  string_ += " ";
...
void Converter::Visit(const MediaType& media_type) {
  APPEND_LOOKUP(media_type.value_id(), kMediaTypeLookupTable);
}
...
void Converter::Visit(const MfName& mf_name) {
  APPEND_LOOKUP(mf_name.value_id(), kMfNameLookupTable);
}
...
void Converter::Visit(const Property& property) {
  APPEND_LOOKUP(property.name_id(), kPropertyLookupTable);
}
...
void Converter::Visit(const Selector& selector, bool is_first) {
  ...
  if (selector.has_pseudo_value_id()) {
    ...
    if (selector.pseudo_type() == PseudoType::ELEMENT)
      string_ += ":";
    APPEND_LOOKUP(selector.pseudo_value_id(), kPseudoLookupTable);
  }
}
...
void Converter::Visit(const Expr& expr, int declaration_value_id) {
  if (!declaration_value_id)
    ...
    APPEND_LOOKUP(declaration_value_id, kValueLookupTable);
