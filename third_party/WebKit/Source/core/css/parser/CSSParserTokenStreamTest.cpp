...
TEST(CSSParserTokenStreamTest, RangesDoNotGetInvalidatedWhenConsuming) {
  ...
  auto range = stream.ConsumeUntilLookAheadIs<kIdentToken>();
...
TEST(CSSParserTokenStreamTest, ConsumeUntilLookAheadIsEmpty) {
  CSSTokenizer tokenizer("{23 }");
  CSSParserTokenStream stream(tokenizer);

  auto range = stream.ConsumeUntilLookAheadIs<>();
  EXPECT_TRUE(stream.AtEnd());
