...
MutableStylePropertySet::SetResult CSSParser::ParseValue(
  ...
  if (style_sheet) {
    ...
    context = CSSParserContext::Create(parser_mode, is_secure_context);
  }
  return ParseValue(declaration, unresolved_property, string, important,
                    context);
}
...
const CSSValue* CSSParser::ParseSingleValue(
    CSSPropertyID property_id,
    const String& string,
    bool is_secure_context,
    const CSSParserContext* initial_context) {
  const CSSParserContext* context =
      initial_context ? initial_context
                      : StrictCSSParserContext(is_secure_context);
...
bool CSSParser::ParseSupportsCondition(const String& condition,
  ...
  const auto tokens = tokenizer.TokenizeToEOF();
  CSSParserImpl parser(StrictCSSParserContext(is_secure_context));
...
bool CSSParser::ParseColor(Color& color, const String& string, bool strict) {
  ...
  if (!value) {
    value = ParseSingleValue(
        ...
        StrictCSSParserContext(/* is_secure_context */ false), nullptr);
  }
