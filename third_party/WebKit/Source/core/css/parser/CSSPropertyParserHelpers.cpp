...
static bool ParseRGBParameters(CSSParserTokenRange& range,
  ...
  for (int i = 1; i < 3; i++) {
    if (!ConsumeCommaIncludingWhitespace(args))
      return false;
    color_parameter =
        is_percent ? ConsumePercent(args, kValueRangeAll) : ConsumeInteger(args);
    if (!color_parameter)
      return false;
  ...
  if (parse_alpha) {
    ...
    result =
        MakeRGBA(color_array[0], color_array[1], color_array[2], alpha_component);
  } else {
...
static bool ParseHSLParameters(CSSParserTokenRange& range,
  ...
  result = MakeRGBAFromHSLA(color_array[0], color_array[1], color_array[2], alpha);
  return args.AtEnd();
}
...
CSSValue* ConsumeLineWidth(CSSParserTokenRange& range,
  ...
  if (id == CSSValueThin || id == CSSValueMedium || id == CSSValueThick)
    return ConsumeIdent(range);
  return ConsumeLength(range, css_parser_mode, kValueRangeNonNegative, unitless);
}
...
static CSSValue* ConsumeDeprecatedRadialGradient(CSSParserTokenRange& args,
  ...
  if (!shape && !size_keyword) {
    ...
    if (horizontal_size) {
      vertical_size = ConsumeLengthOrPercent(args, css_parser_mode, kValueRangeAll);
      if (!vertical_size)
        return nullptr;
...
static CSSValue* ConsumeConicGradient(CSSParserTokenRange& args,
  ...
  if (ConsumeIdent<CSSValueAt>(args)) {
    if (!ConsumePosition(args, css_parser_mode, UnitlessQuirk::kForbid, center_x,
                         center_y))
      return nullptr;
...
static CSSValue* ConsumeCrossFade(CSSParserTokenRange& args,
  ...
  if (!percentage)
    return nullptr;
  return CSSCrossfadeValue::Create(from_image_value, to_image_value, percentage);
}
