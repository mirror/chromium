...
static bool ParseRGBParameters(CSSParserTokenRange& range, RGBA32& result) {
  ...
  if (comma_consumed && !requires_commas)
    return false;
  if (comma_consumed || ConsumeSlashIncludingWhitespace(args)) {
    double alpha;
    if (!ConsumeNumberRaw(args, alpha)) {
      alpha = ConsumePercent(args, kValueRangeAll);
      if (!alpha)
        return false;
      else
        alpha = alpha->GetDoubleValue() / 100.0f;
    }
