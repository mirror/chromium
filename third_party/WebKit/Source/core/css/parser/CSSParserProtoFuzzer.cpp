...
DEFINE_BINARY_PROTO_FUZZER(const StyleSheet& style_sheet_message) {
  ...
  WTF::String style_sheet_string = converter->Convert(style_sheet_message);
  ...
  blink::ThreadState* current_thread_state = blink::ThreadState::Current();
  current_thread_state->CollectAllGarbage();
#endif
}
};  // namespace css_parser_proto_fuzzer
