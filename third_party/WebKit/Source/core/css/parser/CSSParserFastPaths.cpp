...
bool CSSParserFastPaths::IsValidKeywordPropertyAndValue(
    ...
    CSSValueID value_id,
    CSSParserMode parser_mode) {
  if (value_id == CSSValueInvalid || !isValueAllowedInMode(value_id, parser_mode))
    return false;
  ...
  switch (property_id) {
    case CSSPropertyAlignmentBaseline:
      return value_id == CSSValueAuto || value_id == CSSValueAlphabetic ||
             ...
             (value_id >= CSSValueBeforeEdge && value_id <= CSSValueMathematical);
    case CSSPropertyAll:
    ...
    case CSSPropertyFontStretch:
      return value_id == CSSValueNormal || (value_id >= CSSValueUltraCondensed &&
    ...
    case CSSPropertyWebkitTextOrientation:
      return value_id == CSSValueSideways || value_id == CSSValueSidewaysRight ||
    ...
    case CSSPropertyTextTransform:  // capitalize | uppercase | lowercase | none
      return (value_id >= CSSValueCapitalize && value_id <= CSSValueLowercase) ||
             value_id == CSSValueNone;
    ...
    case CSSPropertyWritingMode:
      return value_id == CSSValueHorizontalTb || value_id == CSSValueVerticalRl ||
             value_id == CSSValueVerticalLr || value_id == CSSValueLrTb ||
             value_id == CSSValueRlTb || value_id == CSSValueTbRl ||
             value_id == CSSValueLr || value_id == CSSValueRl ||
             value_id == CSSValueTb;
...
static CSSFunctionValue* ParseSimpleTransformValue(CharType*& pos,
  ...
  if (is_translate) {
    ...
    pos += argument_start;
    CSSFunctionValue* transform_value = CSSFunctionValue::Create(transform_type);
...
CSSValue* CSSParserFastPaths::MaybeParseValue(CSSPropertyID property_id,
                                              const String& string,
                                              CSSParserMode parser_mode) {
  if (CSSValue* length = ParseSimpleLengthValue(property_id, string, parser_mode))
    return length;
