...
static bool FastParseColorInternal(RGBA32& rgb,
  ...
  if (MightBeRGBOrRGBA(characters, length)) {
    ...
    bool no_check_whitespace = false;

    if (!ParseColorIntOrPercentage(
            current, end, ',', should_whitespace_terminate, true, expect, red))
      return false;
    if (!ParseColorIntOrPercentage(current, end, ',',
                                   should_whitespace_terminate, false, expect,
                                   green))
      return false;
    if (!ParseColorIntOrPercentage(current, end, ',', no_check_whitespace,
                                   false, expect, blue)) {
      ...
      // Might not have alpha
                   ...
                   current, end, ')', no_check_whitespace, false, expect, blue))
        return false;
    } else {
      if (should_whitespace_terminate)
        return false;
