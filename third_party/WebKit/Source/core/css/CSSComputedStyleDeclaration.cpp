...
bool IsLayoutDependent(CSSPropertyID property_id,
  ...
  switch (property_id) {
    ...
    case CSSPropertyMarginTop:
      return layout_object->IsBox() && (!style || !style->MarginTop().IsFixed());
...
CSSComputedStyleDeclaration::CSSComputedStyleDeclaration(
    ...
    : node_(n),
      pseudo_element_specifier_(CSSSelector::ParsePseudoId(pseudo_element_name)),
...
const ComputedStyle* CSSComputedStyleDeclaration::ComputeComputedStyle() const {
  ...
  DCHECK(styled_node);
  return styled_node->EnsureComputedStyle(
      styled_node->IsPseudoElement() ? kPseudoIdNone : pseudo_element_specifier_);
}
...
const CSSValue* CSSComputedStyleDeclaration::GetPropertyCSSValue(
  ...
  return ComputedStyleCSSValueMapping::Get(
      custom_property_name, *style, styled_node->GetDocument().GetPropertyRegistry());
}
