...
static String GetApplyAtRuleText(const CSSValue* value, bool is_not_first_decl) {
  StringBuilder result;
  if (is_not_first_decl)
    result.Append(' ');
  result.Append("@apply ");
...
String StylePropertySerializer::Get4Values(
  ...
  if (top_value_index == -1 || right_value_index == -1 || bottom_value_index == -1 ||
      left_value_index == -1)
    return String();
...
static void AppendBackgroundRepeatValue(StringBuilder& builder,
  ...
  const CSSIdentifierValue& repeat_x =
      repeat_xcss_value.IsInitialValue() ? initial_repeat_value
                                       : ToCSSIdentifierValue(repeat_xcss_value);
  const CSSIdentifierValue& repeat_y =
      repeat_ycss_value.IsInitialValue() ? initial_repeat_value
...
String StylePropertySerializer::BackgroundRepeatPropertyValue() const {
  ...
  size_t shorthand_length = lowestCommonMultiple(repeat_x_length, repeat_y_length);
  ...
  for (size_t i = 0; i < shorthand_length; ++i) {
    ...
    const CSSValue& x_value =
        repeat_x_list ? repeat_x_list->Item(i % repeat_x_list->length()) : repeat_x;
    const CSSValue& y_value =
        repeat_y_list ? repeat_y_list->Item(i % repeat_y_list->length()) : repeat_y;
