...
bool IsConnectionTypeSlow() {
  return GetNetworkStateNotifier().ConnectionType() == kWebConnectionTypeCellular2G;
}
...
PassRefPtr<SimpleFontData> RemoteFontFaceSource::CreateFontData(
  ...
  return SimpleFontData::Create(
      custom_font_data_->GetFontPlatformData(font_description.EffectiveFontSize(),
  ...
  if (size < 50 * 1024) {
    ...
    if (data_source_ == kFromNetwork) {
      ...
      if (is_intervention_triggered)
        missed_cache_and_intervention_triggered_under50k_histogram.Count(duration);
      else
        missed_cache_and_intervention_not_triggered_under50k_histogram.Count(duration);
    }
    return;
  }
  if (size < 100 * 1024) {
