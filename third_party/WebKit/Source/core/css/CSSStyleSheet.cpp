...
void CSSStyleSheet::DidMutateRules() {
  ...
  if (ownerNode() && ownerNode()->isConnected()) {
    owner->GetStyleEngine().SetNeedsActiveStyleUpdate(ownerNode()->GetTreeScope());
...
bool CSSStyleSheet::MatchesMediaQueries(const MediaQueryEvaluator& evaluator) {
  ...
  if (!media_queries_)
    return true;
  return evaluator.Eval(media_queries_, &viewport_dependent_media_query_results_,
...
bool CSSStyleSheet::CanAccessRules() const {
  ...
  if (document->GetSecurityOrigin()->CanRequestNoSuborigin(base_url))
    return true;
  if (allow_rule_access_from_origin_ && document->GetSecurityOrigin()->CanAccess(
