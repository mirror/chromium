...
TEST_F(RangeTest, createAdjustedToTreeScopeWithPositionInShadowTree) {
  ...
  Range* const range = Range::CreateAdjustedToTreeScope(GetDocument(), position);
...
TEST_F(RangeTest, SplitTextNodeRangeOutsideText) {
  ...
  Range* range_inner_left = Range::Create(GetDocument(), inner_left, 0, inner_left, 1);
...
TEST_F(RangeTest, updateOwnerDocumentIfNeeded) {
  ...
  Range* range = Range::Create(GetDocument(), Position(bar, 0), Position(foo, 1));
