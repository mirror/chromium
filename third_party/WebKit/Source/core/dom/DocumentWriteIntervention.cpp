...
FetchBlockedDocWriteScriptClient* MaybeDisallowFetchForDocWrittenScript(
  ...
  // AddWarningHeader() is done here after FetchBlockedDocWriteScriptClient
  // is created, to avoid FetchBlockedDocWriteScriptClient's FetchParameters
  // from containing the warning header. Otherwise, the warning header added
  // here is also included in the resended request.
  AddWarningHeader(&params);

  params.MutableResourceRequest().SetCachePolicy(
      WebCachePolicy::kReturnCacheDataDontLoad);

  return client;
}

DEFINE_TRACE(FetchBlockedDocWriteScriptClient) {
...
void FetchBlockedDocWriteScriptClient::NotifyFinished(Resource* resource) {
  ...
  // cache for subsequent uses. However, we don't explicitly evict it from
  // MemoryCache, because it is harmless and MemoryCache is weak.
}
