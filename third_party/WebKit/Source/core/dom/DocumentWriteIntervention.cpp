...
ScriptResource* SecondFetch(FetchParameters& params,
                            Document& element_document,
                            ClassicPendingScript*) {
  EmitErrorBlocked(params.Url(), element_document);

  AddHeader(&params);

  return ScriptResource::Fetch(params, element_document.Fetcher());
}
...
void PossiblyFetchBlockedDocWriteScript(
  ...
  if (!resource->ErrorOccurred()) {
    EmitWarningNotBlocked(request.Url(), document);
    return;
  }
  ...
  ClassicPendingScript::FetchClassicScript(SecondFetch, document, request,
