...
void PossiblyFetchBlockedDocWriteScript(const Resource* resource,
  ...
  if (!resource->ErrorOccurred()) {
    EmitWarningNotBlocked(resource->Url(), document);
    return;
  }
  ...
  EmitErrorBlocked(resource->Url(), document);

  FetchParameters params = ClassicPendingScript::CreateFetchParameters(
      resource->Url(), options, document);
  params.SetDefer(FetchParameters::kIdleLoad);
  AddHeader(&params);
  ScriptResource::Fetch(params, document.Fetcher());
}
