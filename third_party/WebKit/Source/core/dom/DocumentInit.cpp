...
DocumentInit DocumentInit::FromContext(Document* context_document,
                                       const KURL& url) {
  return DocumentInit(url, nullptr, nullptr, context_document, nullptr);
}

DocumentInit DocumentInit::FromFrame(LocalFrame* frame, const KURL& url) {
  return DocumentInit(url, frame, nullptr, nullptr, nullptr);
}

DocumentInit DocumentInit::FromURL(const KURL& url) {
  return DocumentInit(url, nullptr, nullptr, nullptr, nullptr);
}

DocumentInit DocumentInit::FromImportsController(
    HTMLImportsController* controller,
    Document* context_document,
    const KURL& url) {
  DCHECK(controller);
  return DocumentInit(url, nullptr, nullptr, context_document, controller);
}
