...
class CaptureExportedStringFunction : public ScriptFunction {
 public:
  ...
  String CapturedValue() { return captured_value_; }
...
class CaptureErrorFunction : public ScriptFunction {
 public:
  CaptureErrorFunction(ScriptState* script_state)
...
TEST(DynamicModuleResolverTest, ResolveSpecifierFailure) {
  ...
  DynamicModuleResolver* resolver = DynamicModuleResolver::Create(modulator);
  resolver->ResolveDynamically(
      "invalid-specifier", "https://example.com/referrer.js", promise_resolver);
...
TEST(DynamicModuleResolverTest, ResolveFetchFailure) {
  V8TestingScope scope;
  DynamicModuleResolverTestModulator* modulator =
      new DynamicModuleResolverTestModulator(scope.GetScriptState());
