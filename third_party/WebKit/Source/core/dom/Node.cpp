...
void Node::Before(const HeapVector<NodeOrString>& nodes,
  ...
  if (Node* node = ConvertNodesIntoNode(nodes, GetDocument(), exception_state))
    parent->insertBefore(node, viable_previous_sibling
...
bool Node::isDefaultNamespace(
  ...
  const AtomicString& namespace_uri =
      namespace_uri_maybe_empty.IsEmpty() ? g_null_atom : namespace_uri_maybe_empty;
...
unsigned short Node::compareDocumentPosition(
  ...
  const Attr* attr2 =
      other_node->getNodeType() == kAttributeNode ? ToAttr(other_node) : nullptr;
...
void Node::PrintNodePathTo(std::ostream& stream) const {
  ...
  for (unsigned index = chain.size(); index > 0; --index) {
    ...
    if (node->IsShadowRoot()) {
      int count = 0;
      for (const ShadowRoot* shadow_root = ToShadowRoot(node)->OlderShadowRoot();
...
static void AppendMarkedFlatTree(const String& base_indent,
  ...
  for (const Node* node = root_node; node;
    ...
    if (Node* child = FlatTreeTraversal::FirstChild(*node))
      AppendMarkedFlatTree(indent.ToString(), child, marked_node1, marked_label1,
                           marked_node2, marked_label2, builder);
  }
}
...
static void PrintSubTreeAcrossFrame(const Node* node,
  ...
  if (node->IsShadowRoot()) {
    if (ShadowRoot* younger_shadow_root = ToShadowRoot(node)->YoungerShadowRoot())
...
void Node::WillMoveToNewDocument(Document& old_document, Document& new_document) {
...
void Node::DidMoveToNewDocument(Document& old_document) {
  ...
  if (GetDocument().GetPage() && GetDocument().GetPage() != old_document.GetPage()) {
...
void Node::RemoveAllEventListeners() {
  if (HasEventListeners() && GetDocument().GetPage())
    GetDocument().GetPage()->GetEventHandlerRegistry().DidRemoveAllEventHandlers(*this);
...
void Node::CreateAndDispatchPointerEvent(const AtomicString& mouse_event_name,
  ...
  if (view && view->GetFrame() && view->GetFrame()->View()) {
    ...
    location_in_frame_zoomed = frame_view->ContentsToFrame(location_in_contents);
...
void Node::DispatchSimulatedClick(Event* underlying_event,
  ...
  EventDispatcher::DispatchSimulatedClick(*this, underlying_event, event_options,
                                          scope);
}
...
void Node::DefaultEventHandler(Event* event) {
  ...
  if (event_type == EventTypeNames::keydown ||
    ...
    if (event->HasInterface(EventNames::TextEvent)) {
      if (LocalFrame* frame = GetDocument().GetFrame())
        frame->GetEventHandler().DefaultTextInputEventHandler(ToTextEvent(event));
    }
