...
TEST_F(StyleEngineTest, AnalyzedInject) {
  ...
  EXPECT_EQ(1u, GetStyleEngine().InjectedStyleSheets().size());
  GetDocument().View()->UpdateAllLifecyclePhases();
  ...
  WebStyleSheetId blue_id =
      GetStyleEngine().InjectSheet(blue_parsed_sheet);
  EXPECT_EQ(2u, blue_id);
  EXPECT_EQ(2u, GetStyleEngine().InjectedStyleSheets().size());
  GetDocument().View()->UpdateAllLifecyclePhases();
  ...
  EXPECT_EQ(0u, GetStyleEngine().InjectedStyleSheets().size());
  GetDocument().View()->UpdateAllLifecyclePhases();
