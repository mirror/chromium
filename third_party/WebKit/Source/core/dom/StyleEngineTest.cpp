...
TEST_F(StyleEngineTest, RuleSetInvalidationTypeSelectors) {
  ...
  EXPECT_EQ(
      kRuleSetInvalidationsScheduled,
      ScheduleInvalidationsForRules(GetDocument(), "span { background: green}"));
  GetDocument().View()->UpdateAllLifecyclePhases();
  unsigned after_count = GetStyleEngine().StyleForElementCount();
  EXPECT_EQ(1u, after_count - before_count);
  ...
  EXPECT_EQ(
      kRuleSetInvalidationFullRecalc,
      ScheduleInvalidationsForRules(GetDocument(), "div * { background: green}"));
  GetDocument().View()->UpdateAllLifecyclePhases();

  before_count = GetStyleEngine().StyleForElementCount();
  EXPECT_EQ(
      kRuleSetInvalidationsScheduled,
      ScheduleInvalidationsForRules(GetDocument(), "#i b { background: green}"));
  GetDocument().View()->UpdateAllLifecyclePhases();
  after_count = GetStyleEngine().StyleForElementCount();
  EXPECT_EQ(1u, after_count - before_count);
}
...
TEST_F(StyleEngineTest, RuleSetInvalidationHost) {
  GetDocument().body()->setInnerHTML("<div id=nohost></div><div id=host></div>");
  Element* host = GetDocument().GetElementById("host");
  ASSERT_TRUE(host);

  ShadowRootInit init;
  init.setMode("open");
  ShadowRoot* shadow_root = host->attachShadow(
      ToScriptStateForMainWorld(GetDocument().GetFrame()), init, ASSERT_NO_EXCEPTION);
...
TEST_F(StyleEngineTest, HasViewportDependentMediaQueries) {
  ...
  EXPECT_TRUE(GetDocument().GetStyleEngine().HasViewportDependentMediaQueries());

  GetDocument().body()->RemoveChild(style_element);
  GetDocument().View()->UpdateAllLifecyclePhases();

  EXPECT_FALSE(GetDocument().GetStyleEngine().HasViewportDependentMediaQueries());
}
