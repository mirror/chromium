...
class NodeListsNodeData final : public GarbageCollected<NodeListsNodeData> {
 ...
 public:
  ...
  struct NodeListAtomicCacheMapEntryHash {
    ...
    static unsigned GetHash(const std::pair<unsigned char, StringImpl*>& entry) {
      return DefaultHash<StringImpl*>::Hash::GetHash(entry.second) + entry.first;
    }
  ...
  TagCollection* AddCache(ContainerNode& node,
    ...
    TagCollection* list = TagCollection::Create(node, namespace_uri, local_name);
    result.stored_value->value = list;
    return list;
  }
