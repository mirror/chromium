...
void WhitespaceAttacher::UpdateLastTextNodeFromDisplayContents() {
  DCHECK(last_display_contents_);
  Node* child = last_display_contents_.Release();
  ...
  if (child->IsActiveSlotOrActiveInsertionPoint()) {
    if (child->IsInsertionPoint())
      child = ToInsertionPoint(child)->FirstDistributedNode();
    else
      child = toHTMLSlotElement(child)->FirstDistributedNode();
  }

  if (child->IsElementNode() && ToElement(child)->HasDisplayContentsStyle())
    child = LayoutTreeBuilderTraversal::FirstLayoutChild(*child);

  // If the display:contents was empty, use the current last_text_node_.
  if (!child)
    return;

  DCHECK(!child->IsElementNode() ||
         !ToElement(child)->HasDisplayContentsStyle());
