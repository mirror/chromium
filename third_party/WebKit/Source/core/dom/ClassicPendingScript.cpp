...
bool ClassicPendingScript::StartStreamingIfPossible(
  ...
  // of NotifyFinished of the very same object. In such a recursive call,
  // IsCurrentlyStreaming may give the wrong result (since the state is still
  // being updated). We just the presence of streamer_done_ to catch this and
  // to ensure we just return without doing anything.
  //
  // (The call chain here is: NotifyFinished calls (indirectly)
  // AdvanceReadyState, which calles Client()->PendingScriptFinished(). If the
