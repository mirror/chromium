...
class ScriptModuleResolverImpl::DynamicResolver : public ModuleTreeClient {
 public:
  static DynamicResolver* Create(const KURL& url,
                                 Modulator* modulator,
                                 ScriptPromiseResolver* promise_resolver) {
    return new DynamicResolver(url, modulator, promise_resolver);
  }

  DECLARE_TRACE();

 private:
  DynamicResolver(const KURL& url,
                  Modulator* modulator,
                  ScriptPromiseResolver* promise_resolver)
...
void ScriptModuleResolverImpl::DynamicResolver::NotifyModuleTreeLoadFinished(
    ModuleScript* module_script) {
...
void ScriptModuleResolverImpl::ResolveDynamically(
  ...
  ModuleScriptFetchRequest request(url, empty_nonce, kNotParserInserted,
                                   credentials_mode);
  DynamicResolver* dynamic_resolver =
      DynamicResolver::Create(url, modulator_.Get(), promise_resolver);
