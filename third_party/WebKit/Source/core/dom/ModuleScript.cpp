...
ModuleScript* ModuleScript::Create(const String& source_text,
  ...
  for (const auto& requested :
    ...
    bool is_not_absolute = !requested.specifier.Contains(':');
    if (is_not_absolute &&
        !Modulator::ModuleSpecifierStartsWithValidPrefix(requested.specifier)) {
      error_message.append(
          " Note that relative references must start with either \"/\", "
          "\"./\", or \"../\".");
    }
