...
XMLDocument* DOMImplementation::createDocument(
  ...
  doc->SetSecurityOrigin(document_->GetSecurityOrigin()->IsolatedCopy()); // FOXME
...
Document* DOMImplementation::createHTMLDocument(const String& title) {
  ...
  d->SetSecurityOrigin(document_->GetSecurityOrigin()->IsolatedCopy()); // FOXME
...
Document* DOMImplementation::createDocument(const String& type,
  ...
  if (init.GetFrame() && init.GetFrame()->GetPage() &&
    ...
    if (init.GetFrame()->IsMainFrame()) {
      scoped_refptr<const SecurityOrigin> origin = SecurityOrigin::Create(init.Url());
