...
XMLDocument* DOMImplementation::createDocument(
  ...
  DocumentInit init = DocumentInit::FromContext(GetDocument().ContextDocument());
...
Document* DOMImplementation::createDocument(const String& type,
  ...
  if (init.GetFrame() && init.GetFrame()->GetPage() &&
    ...
    if (init.GetFrame()->IsMainFrame()) {
      ...
      plugin_data = init.GetFrame()->GetPage()->GetPluginData(
          init.GetFrame()->Tree().Top()->GetSecurityContext()->GetSecurityOrigin());
    }
  }
  ...
  if (type != "text/plain" && plugin_data && plugin_data->SupportsMimeType(type))
    return PluginDocument::Create(init);
