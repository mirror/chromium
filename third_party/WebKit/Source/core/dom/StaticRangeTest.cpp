...
TEST_F(StaticRangeTest, SplitTextNodeRangeOutsideText) {
  ...
  Element* outer = GetDocument().GetElementById(AtomicString::FromUTF8("outer"));
  Element* inner_left =
  ...
  Range* range_inner_left = static_range_inner_left->toRange(ASSERT_NO_EXCEPTION);
  Range* range_inner_right = static_range_inner_right->toRange(ASSERT_NO_EXCEPTION);
  ...
  EXPECT_EQ(old_text, static_range_from_text_to_middle_of_element->startContainer());
