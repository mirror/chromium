...
void ProcessingInstruction::Process(const String& href, const String& charset) {
  ...
  if (is_xsl_) {
    ...
    request.SetCharset(charset.IsEmpty() ? GetDocument().characterSet() : charset);
...
bool ProcessingInstruction::SheetLoaded() {
  if (!IsLoading()) {
    if (!DocumentXSLT::SheetLoaded(GetDocument(), this))
      GetDocument().GetStyleEngine().RemovePendingSheet(*this, style_engine_context_);
    return true;
  }
  return false;
}
...
void ProcessingInstruction::SetCSSStyleSheet(
  ...
  CSSParserContext* parser_context =
      CSSParserContext::Create(GetDocument(), base_url, referrer_policy, charset);
  ...
  if (!alternate_ && !title_.IsEmpty())
    GetDocument().GetStyleEngine().SetPreferredStylesheetSetNameIfNotSet(title_);
