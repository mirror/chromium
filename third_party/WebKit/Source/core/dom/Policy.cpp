...
#include "core/dom/DocumentPolicy.h"
#include "core/dom/FramePolicy.h"
...
// static
Policy* Policy::Create(Type type,
                       Document* document,
                       const ParsedFeaturePolicy* container_policy,
                       const scoped_refptr<SecurityOrigin> origin) {
  if (type == DocumentPolicy)
    return DocumentPolicy::Create(document);
  DCHECK(type == FramePolicy);
  return FramePolicy::Create(document, container_policy, origin);
}
