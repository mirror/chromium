...
ScriptValue ModulatorImplBase::ExecuteModule(
  ...
  if (capture_error == CaptureEvalErrorFlag::kCapture) {
    ...
    // Here we rely on caller to handle the exception. The current only caller
    // is DynamicImportTreeClinet::NotifyModuleTreeLoadFinished, which catches
    // the exception immediately, so just capturing the exception here is more
