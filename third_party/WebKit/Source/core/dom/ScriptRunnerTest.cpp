...
TEST_F(ScriptRunnerTest, QueueSingleScript_InOrder) {
  MockScriptLoader* script_loader = MockScriptLoader::Create();
  script_runner_->QueueScriptForExecution(script_loader, ScriptRunner::kInOrder);
...
TEST_F(ScriptRunnerTest, QueueMultipleScripts_InOrder) {
  ...
  for (int i = 2; i >= 0; i--) {
    is_ready[i] = true;
    script_runner_->NotifyScriptReady(script_loaders[i], ScriptRunner::kInOrder);
    platform_->RunUntilIdle();
  }
...
TEST_F(ScriptRunnerTest, QueueMixedScripts) {
  ...
  script_runner_->QueueScriptForExecution(script_loader1, ScriptRunner::kInOrder);
  script_runner_->QueueScriptForExecution(script_loader2, ScriptRunner::kInOrder);
  script_runner_->QueueScriptForExecution(script_loader3, ScriptRunner::kInOrder);
...
TEST_F(ScriptRunnerTest, QueueReentrantScript_Async) {
  ...
  EXPECT_CALL(*script_loader1, Execute()).WillOnce(Invoke([script_loader, this] {
    order_.push_back(1);
    script_runner_->NotifyScriptReady(script_loader, ScriptRunner::kAsync);
  }));

  script_loader = script_loader3;
  EXPECT_CALL(*script_loader2, Execute()).WillOnce(Invoke([script_loader, this] {
    order_.push_back(2);
    script_runner_->NotifyScriptReady(script_loader, ScriptRunner::kAsync);
  }));
