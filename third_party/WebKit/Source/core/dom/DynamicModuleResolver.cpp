...
void DynamicImportTreeClient::NotifyModuleTreeLoadFinished(
  ...
  ExceptionState exception_state(isolate, ExceptionState::kExecutionContext,
                                 "DynamicImportTreeClient",
                                 "NotifyModuleTreeLoadFinished");
  modulator_->ExecuteModuleWithRethrowError(module_script, exception_state);
...
void DynamicModuleResolver::ResolveDynamically(
  ...
  // Note: This is set as the ReferrerScriptInfo's default value.
  // See ReferrerScriptInfo.h.
  // Step 6. "If referencing script is a module script, set credentials mode to
  // referencing script's credentials mode."
  // [spec text]
