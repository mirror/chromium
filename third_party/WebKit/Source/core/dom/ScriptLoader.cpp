...
bool ScriptLoader::PrepareScript(const TextPosition& script_start_position,
  ...
  if (element_->HasSourceAttribute()) {
    ...
    if (GetScriptType() == ScriptType::kClassic) {
      ...
      // "Fetch a classic script given url, settings object, options, CORS setting, and encoding." [spec text]
      if (!FetchClassicScript(url, element_document, options, integrity_metadata, encoding)) {
        // TODO(hiroshige): Make this asynchronous. Currently we fire the error
        // event synchronously to keep the existing behavior.
        DispatchErrorEvent();
        return false;
...
bool ScriptLoader::FetchClassicScript(
  ...
  // ... "script", ... [spec text] -> set in ScriptResource::Fetch().
  // and CORS setting. [spec text] -> set below.
  ...
  // Step 3. Set up the classic script request given request and options. [spec text]
  //
  // https://html.spec.whatwg.org/multipage/webappapis.html#set-up-the-classic-script-request
  // Set request's cryptographic nonce metadata to options's cryptographic nonce, [spec text]
  ...
  if (options.CredentialsMode() != WebURLRequest::kFetchCredentialsModeOmit) {
    params.SetCrossOriginAccessControl(security_origin, options.CredentialsMode());
  }
