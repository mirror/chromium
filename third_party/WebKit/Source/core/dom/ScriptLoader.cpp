...
ScriptLoader::ScriptLoader(ScriptElementBase* element,
  ...
  if (parser_inserted && element_->GetDocument().GetScriptableDocumentParser() &&
...
void ScriptLoader::LogScriptMIMEType(LocalFrame* frame,
  ...
  bool is_application =
      !is_text && mime_type.StartsWith("application/", kTextCaseASCIIInsensitive);
  ...
  UseCounter::Feature feature =
      is_same_origin
          ...
          : (is_text ? UseCounter::kCrossOriginTextScript
...
bool ScriptLoader::DoExecuteScript(const ScriptSourceCode& source_code) {
  ...
  const ContentSecurityPolicy* csp = element_document->GetContentSecurityPolicy();
  ...
  if (!is_external_script_) {
    ...
    access_control_status = source_code.GetResource()->CalculateAccessControlStatus(
  ...
  IgnoreDestructiveWriteCountIncrementer ignore_destructive_write_count_incrementer(
...
void ScriptLoader::PendingScriptFinished(PendingScript* pending_script) {
  ...
  if (ErrorOccurred()) {
    context_document->GetScriptRunner()->NotifyScriptLoadError(this,
                                                           async_exec_type_);
    DetachPendingScript();
    DispatchErrorEvent();
    return;
  }
  context_document->GetScriptRunner()->NotifyScriptReady(this, async_exec_type_);
  pending_script_->StopWatchingForLoad();
}
