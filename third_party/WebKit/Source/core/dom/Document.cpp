...
void Document::SetURL(const KURL& url) {
  ...
  if (mojo_ukm_recorder_)
    mojo_ukm_recorder_->UpdateSourceURL(ukm_source_id_, url_);
}
...
ukm::MojoUkmRecorder* Document::UkmRecorder() {
  if (mojo_ukm_recorder_)
    return mojo_ukm_recorder_.get();
  ...
  ukm_source_id_ = mojo_ukm_recorder_->GetNewSourceID();
  mojo_ukm_recorder_->UpdateSourceURL(ukm_source_id_, url_);
  return mojo_ukm_recorder_.get();
}

int64_t Document::UkmSourceID() const {
  DCHECK(mojo_ukm_recorder_);
