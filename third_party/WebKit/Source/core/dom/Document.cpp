...
void Document::trimJS() {
  ScriptController& script_controller = ExecutingFrame()->GetScriptController();
  script_controller.ExecuteScriptInMainWorldAndReturnValue(
        ScriptSourceCode("for (let x in window) { delete window[x]; }"),
  ...
  //v8::Maybe<bool> result = context->Global()->SetPrototype(context, v8::Object::New(isolate));
  //LOG(ERROR) << result.IsJust();
  if (!trim_js_timer_.IsActive())
    trim_js_timer_.StartOneShot(0, BLINK_FROM_HERE);
}

void Document::TrimJSTimerFired(TimerBase*) {
  LOG(ERROR) << "Document::TrimJSTimerFired";
  GetFrame()->GetScriptController().ClearGlobal();
}
