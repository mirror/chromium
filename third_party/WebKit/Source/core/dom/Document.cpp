...
Document* Document::Create(const Document& document) {
  Document* new_document = new Document(DocumentInit(
      BlankURL(), nullptr, nullptr, const_cast<Document*>(&document), nullptr));
...
Document* Document::CloneDocumentWithoutChildren() {
  DocumentInit init(Url(), nullptr, nullptr, ContextDocument(), nullptr);
  if (IsXMLDocument()) {
    if (IsXHTMLDocument())
...
void Document::InitSecurityContext(const DocumentInit& initializer) {
  ...
  } else if (initializer.OwnerDocument()) {
    cookie_url_ = initializer.OwnerDocument()->CookieURL();
    // We alias the SecurityOrigins to match Firefox, see Bug 15313
    // https://bugs.webkit.org/show_bug.cgi?id=15313
    SetSecurityOrigin(initializer.OwnerDocument()->GetSecurityOrigin());
  } else {
    cookie_url_ = url_;
...
Document& Document::EnsureTemplateDocument() {
  ...
  if (IsHTMLDocument()) {
    DocumentInit init =
        DocumentInit(BlankURL(), nullptr, nullptr, ContextDocument(), nullptr)
            .WithNewRegistrationContext();
    template_document_ = HTMLDocument::Create(init);
  } else {
    template_document_ = Document::Create(
        DocumentInit(BlankURL(), nullptr, nullptr, nullptr, nullptr));
  }
