...
Document* Document::Create(const Document& document) {
  Document* new_document = new Document(
      DocumentInit::FromContext(BlankURL(), const_cast<Document*>(&document)));
...
Document* Document::CloneDocumentWithoutChildren() {
  DocumentInit init = DocumentInit::FromContext(Url(), ContextDocument());
  if (IsXMLDocument()) {
    if (IsXHTMLDocument())
...
Document& Document::EnsureTemplateDocument() {
  ...
  if (IsHTMLDocument()) {
    DocumentInit init = DocumentInit::FromContext(BlankURL(), ContextDocument())
