...
Document::Document(const DocumentInit& initializer,
    ...
    : ContainerNode(0, kCreateDocument),
      ...
      load_event_delay_timer_(TaskRunnerHelper::Get(TaskType::kNetworking, this),
                            this,
...
Element* Document::createElement(const AtomicString& local_name,
  ...
  bool should_create_builtin = create_v1_builtin || string_or_options.isString();
  ...
  if (is_v1) {
    ...
    const CustomElementDescriptor desc =
        RuntimeEnabledFeatures::customElementsBuiltinEnabled()
            ...
            : CustomElementDescriptor(converted_local_name, converted_local_name);
  ...
  if (definition) {
    element = CustomElement::CreateCustomElementSync(*this, converted_local_name,
                                                     definition);
  } else if (V0CustomElement::IsValidName(local_name) && RegistrationContext()) {
    element = RegistrationContext()->CreateCustomTagElement(
        *this, QualifiedName(g_null_atom, converted_local_name, xhtmlNamespaceURI));
  } else {
...
Element* Document::createElementNS(const AtomicString& namespace_uri,
  ...
  if (!IsValidName(qualified_name)) {
    exception_state.ThrowDOMException(
        kInvalidCharacterError,
        "The tag name provided ('" + qualified_name + "') is not a valid name.");
    return nullptr;
  }
...
ScriptValue Document::registerElement(ScriptState* script_state,
  ...
  HostsUsingFeatures::CountMainWorldOnly(
      script_state, *this, HostsUsingFeatures::Feature::kDocumentRegisterElement);
  ...
  switch (imported_node->getNodeType()) {
    ...
    case kProcessingInstructionNode:
      return createProcessingInstruction(
          imported_node->nodeName(), imported_node->nodeValue(), exception_state);
    case kCommentNode:
    ...
    case kDocumentFragmentNode: {
      ...
      if (deep &&
          !ImportContainerNodeChildren(old_fragment, new_fragment,
                                       exception_state))
        return nullptr;
  ...
  switch (source->getNodeType()) {
    ...
    default:
      ...
      if (source->IsFrameOwnerElement()) {
        ...
        if (GetFrame() &&
            GetFrame()->Tree().IsDescendantOf(frame_owner_element->ContentFrame())) {
          exception_state.ThrowDOMException(
              kHierarchyRequestError,
...
void Document::ScheduleLayoutTreeUpdate() {
  ...
  TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"),
                       ...
                       "data", InspectorRecalculateStylesEvent::Data(GetFrame()));
  ++style_version_;
}
...
void Document::InheritHtmlAndBodyElementStyles(StyleRecalcChange change) {
  ...
  if (body) {
    ...
    if (!body_style || body->NeedsStyleRecalc() || did_recalc_document_element) {
  ...
  if (Element* element = ViewportDefiningElement(document_element_style.Get())) {
    if (element == body) {
  ...
  if (GetStyleEngine().UsesRemUnits() &&
    ...
    documentElement()->SetNeedsStyleRecalc(
        kSubtreeStyleChange,
        StyleChangeReasonForTracing::Create(StyleChangeReason::kFontSizeChange));
  }
...
void Document::Shutdown() {
  ...
  if (focused_element_.Get()) {
    ...
    if (GetPage())
      GetPage()->GetChromeClient().FocusedNodeChanged(old_focused_element, nullptr);
  }
...
Document& Document::AxObjectCacheOwner() const {
  ...
  if (doc->GetFrame() && doc->GetFrame()->PagePopupOwner()) {
    ...
    return doc->GetFrame()->PagePopupOwner()->GetDocument().AxObjectCacheOwner();
  }
  return *doc;
}
...
void Document::open(Document* entered_document, ExceptionState& exception_state) {
  if (ImportLoader()) {
    exception_state.ThrowDOMException(
  ...
  if (throw_on_dynamic_markup_insertion_count_) {
    exception_state.ThrowDOMException(
        kInvalidStateError, "Custom Element constructor should not use open().");
    return;
  }
...
DocumentParser* Document::ImplicitOpen(
  ...
  } else if (parser_sync_policy == kAllowAsynchronousParsing && IsPrefetchOnly()) {
...
void Document::ImplicitClose() {
  ...
  if (ProcessingLoadEvent() || !parser_)
    return;
  if (GetFrame() && GetFrame()->GetNavigationScheduler().LocationChangePending()) {
    SuppressLoadEvent();
    return;
  }
  ...
  if (!LocalOwner() || (LocalOwner()->GetLayoutObject() &&
    ...
    if (View() && !GetLayoutViewItem().IsNull() &&
        (!GetLayoutViewItem().FirstChild() || GetLayoutViewItem().NeedsLayout()))
      View()->GetLayout();
  }
...
void Document::ProcessBaseElement() {
  ...
  if (base_element_url != base_element_url_ && !base_element_url.ProtocolIsData() &&
...
void Document::MaybeHandleHttpRefresh(const String& content,
  ...
  if (!ParseHTTPRefresh(content, http_refresh_type == kHttpRefreshFromMetaTag
                                     ? IsHTMLSpace<UChar>
                                     : nullptr,
...
ViewportDescription Document::GetViewportDescription() const {
  ...
  bool viewport_meta_enabled = GetSettings() && GetSettings()->GetViewportMetaEnabled();
...
bool Document::SetFocusedElement(Element* prp_new_focused_element,
  ...
  if (old_focused_element) {
    ...
    if (IsHTMLPlugInElement(old_focused_element)) {
      if (PluginView* plugin = ToHTMLPlugInElement(old_focused_element)->Plugin())
  ...
  if (frame_) {
    if (DocumentLoader* document_loader = Loader()) {
      const AtomicString& http_last_modified =
          document_loader->GetResponse().HttpHeaderField(HTTPNames::Last_Modified);
...
const KURL Document::FirstPartyForCookies() const {
  ...
  const OriginAccessEntry& access_entry =
      top->IsLocalFrame()
          ...
          : OriginAccessEntry(top_document_url.Protocol(), top_document_url.Host(),
                              OriginAccessEntry::kAllowRegisterableDomains);
...
bool Document::ParseQualifiedName(const AtomicString& qualified_name,
  ...
  if (qualified_name.Is8Bit())
    return_value = ParseQualifiedNameInternal(
        qualified_name, qualified_name.Characters8(), length, prefix, local_name);
  else
    return_value = ParseQualifiedNameInternal(
        qualified_name, qualified_name.Characters16(), length, prefix, local_name);
...
KURL Document::OpenSearchDescriptionURL() {
  ...
  for (HTMLLinkElement* link_element =
    ...
    RefPtr<SecurityOrigin> target = SecurityOrigin::Create(link_element->Href());
    if (IsSecureContext()) {
...
Attr* Document::createAttributeNS(const AtomicString& namespace_uri,
  ...
  if (!should_ignore_namespace_checks && !HasValidNamespaceForAttributes(q_name)) {
    exception_state.ThrowDOMException(
        kNamespaceError,
        "The namespace URI provided ('" + namespace_uri +
            "') is not valid for the qualified name provided ('" +
  ...
  if (frame_) {
    ...
    if (inherit_from && frame_ != inherit_from) {
      ...
      ContentSecurityPolicy* policy_to_inherit =
          inherit_from->GetSecurityContext()->GetContentSecurityPolicy();
      if (url_.IsEmpty() || url_.ProtocolIsAbout() ||
          url_.ProtocolIsData() || url_.ProtocolIs("blob") ||
          url_.ProtocolIs("filesystem")) {
...
bool Document::CanExecuteScripts(ReasonForCallingCanExecuteScripts reason) {
  ...
  ContentSettingsClient* settings_client = GetFrame()->GetContentSettingsClient();
  if (!settings_client)
    return false;
...
void Document::ServiceScriptedAnimations(double monotonic_animation_start_time) {
  if (!scripted_animation_controller_)
    return;
...
Touch* Document::createTouch(DOMWindow* window,
  ...
  LocalFrame* frame = window && window->IsLocalDOMWindow()
                          ...
                          : this->GetFrame();
  return Touch::Create(frame, target, identifier, FloatPoint(screen_x, screen_y),
                       FloatPoint(page_x, page_y), FloatSize(radius_x, radius_y),
                       rotation_angle, force, String());
}
...
void Document::UpdateHoverActiveState(const HitTestRequest& request,
  ...
  if (old_hover_element && old_hover_element->isConnected() && new_hover_element) {
    Node* ancestor =
        FlatTreeTraversal::CommonAncestor(*old_hover_element, *new_hover_element);
...
void Document::DidAssociateFormControlsTimerFired(TimerBase* timer) {
  ...
  GetFrame()->GetPage()->GetChromeClient().DidAssociateFormControlsAfterLoad(GetFrame());
}
