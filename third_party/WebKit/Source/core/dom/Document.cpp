...
Document::Document(const DocumentInit& initializer,
    ...
    : ContainerNode(nullptr, kCreateDocument),
      ...
      is_secure_context_(SecureContextState::kUnknown) {
  if (frame_) {
...
void Document::InitSecurityContext(const DocumentInit& initializer) {
  ...
  DCHECK_EQ(is_secure_context_, SecureContextState::kUnknown);
  if (!GetSecurityOrigin()->IsPotentiallyTrustworthy()) {
    is_secure_context_ = SecureContextState::kNonSecure;
  } else if (SchemeRegistry::SchemeShouldBypassSecureContextCheck(
                 GetSecurityOrigin()->Protocol())) {
    is_secure_context_ = SecureContextState::kSecure;
  } else if (frame_) {
    Frame* parent = frame_->Tree().Parent();
    while (parent) {
    ...
    if (is_secure_context_ == SecureContextState::kUnknown)
      is_secure_context_ = SecureContextState::kSecure;
  } else {
    is_secure_context_ = SecureContextState::kNonSecure;
  }
  DCHECK_NE(is_secure_context_, SecureContextState::kUnknown);
}
...
bool Document::IsSecureContext() const {
  bool is_secure = is_secure_context_ == SecureContextState::kSecure;
