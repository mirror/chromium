...
bool Document::DispatchBeforeUnloadEvent(ChromeClient& chrome_client,
  ...
  DEFINE_STATIC_LOCAL(
      CustomCountHistogram, update_histogram,
      ("DocumentEventTiming.BeforeUnloadDuration", 0, 10000000, 50));
  update_histogram.Count((beforeunload_event_end - beforeunload_event_start) *
                         1000000.0);
  ...
  if (load_event_progress_ <= kUnloadEventInProgress) {
    ...
    if (load_event_progress_ < kPageHideInProgress) {
      ...
      if (LocalDOMWindow* window = domWindow()) {
        ...
        update_histogram.Count((pagehide_event_end - pagehide_event_start) *
                               1000000.0);
      }
      if (!frame_)
        return;
      ...
      if (visibility_state != kPageVisibilityStateHidden) {
        ...
        DEFINE_STATIC_LOCAL(CustomCountHistogram, update_histogram,
                            ("DocumentEventTiming.PageVibilityHiddenDuration",
                             0, 10000000, 50));
        update_histogram.Count((pagevisibility_hidden_event_end -
      ...
      if (document_loader && !document_loader->GetTiming().UnloadEventStart() &&
        ...
        update_histogram.Count((unload_event_end - unload_event_start) *
                               1000000.0);
