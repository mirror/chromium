...
void CharacterData::ParserAppendData(const String& data) {
  ...
  SetDataAndUpdate(new_str, data_.length(), 0, data.length(), kUpdateFromParser);
}
...
void CharacterData::deleteData(unsigned offset,
  ...
  if (!ValidateOffsetCount(offset, count, length(), real_count, exception_state))
    return;
...
void CharacterData::SetDataAndUpdate(const String& new_data,
  ...
  if (source != kUpdateFromParser) {
    ...
    GetDocument().NotifyUpdateCharacterData(this, offset_of_replaced_data, old_length,
                                         new_length);
  }
