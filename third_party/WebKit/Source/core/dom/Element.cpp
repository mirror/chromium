...
Attr* Element::DetachAttribute(size_t index) {
  ...
  if (attr_node) {
    ...
    attr_node = Attr::Create(GetDocument(), attribute.GetName(), attribute.Value());
...
bool Element::HasAnimations() const {
  ...
  ElementAnimations* element_animations = GetElementRareData()->GetElementAnimations();
...
inline void Element::SynchronizeAttribute(const QualifiedName& name) const {
  if (!GetElementData())
    return;
  if (UNLIKELY(name == styleAttr && GetElementData()->style_attribute_is_dirty_)) {
    DCHECK(IsStyledElement());
    SynchronizeStyleAttributeInternal();
    return;
  }
...
void Element::setDistributeScroll(ScrollStateCallback* scroll_state_callback,
  ...
  GetScrollCustomizationCallbacks().SetDistributeScroll(this, scroll_state_callback);
}
...
int Element::OffsetLeft() {
  ...
  if (LayoutBoxModelObject* layout_object = GetLayoutBoxModelObject())
    return AdjustLayoutUnitForAbsoluteZoom(
               LayoutUnit(layout_object->PixelSnappedOffsetLeft(OffsetParent())),
...
void Element::ScrollLayoutBoxBy(const ScrollToOptions& scroll_to_options) {
  ...
  if (box) {
    ...
    float new_scaled_top = top * box->Style()->EffectiveZoom() + current_scaled_top;
...
void Element::IncrementCompositorProxiedProperties(uint32_t mutable_properties) {
  ElementRareData& rare_data = EnsureElementRareData();
...
void Element::DecrementCompositorProxiedProperties(uint32_t mutable_properties) {
...
ClientRectList* Element::getClientRects() {
  ...
  GetDocument().AdjustFloatQuadsForScrollAndAbsoluteZoom(quads,
...
void Element::setAttribute(const AtomicString& local_name,
  ...
  if (!GetElementData()) {
    SetAttributeInternal(
        kNotFound, QualifiedName(g_null_atom, case_adjusted_local_name, g_null_atom),
...
void Element::setAttribute(const QualifiedName& name,
  ...
  SynchronizeAttribute(name);
  size_t index =
      GetElementData() ? GetElementData()->Attributes().FindIndex(name) : kNotFound;
...
ALWAYS_INLINE void Element::SetAttributeInternal(
  ...
  if (index == kNotFound) {
    AppendAttributeInternal(name, new_value, in_synchronization_of_lazy_attribute);
    return;
  }
  ...
  if (!in_synchronization_of_lazy_attribute)
    DidModifyAttribute(existing_attribute_name, existing_attribute_value, new_value);
}
...
void Element::AttributeChanged(const AttributeModificationParams& params) {
  ...
  if (ElementShadow* parent_element_shadow =
    ...
    if (ShouldInvalidateDistributionWhenAttributeChanged(parent_element_shadow,
  ...
  if (name == HTMLNames::idAttr) {
    ...
    AtomicString new_id =
        MakeIdForStyleResolution(params.new_value, GetDocument().InQuirksMode());
    if (new_id != old_id) {
...
void Element::ClassAttributeChanged(const AtomicString& new_class_string) {
  ...
  if (class_string_content_type == ClassStringContent::kHasClasses) {
    ...
    GetDocument().GetStyleEngine().ClassChangedForElement(old_classes, new_classes,
                                                    *this);
  } else {
    const SpaceSplitString& old_classes = GetElementData()->ClassNames();
...
StyleRecalcChange Element::RecalcOwnStyle(StyleRecalcChange change) {
  ...
  if (local_change < kIndependentInherit) {
    ...
    if (old_style->HasPseudoElementStyle() || new_style->HasPseudoElementStyle())
...
Attr* Element::setAttributeNode(Attr* attr_node,
  ...
  if (index != kNotFound) {
    ...
    if (old_attr_node) {
      ...
      // Consider switching.
      old_attr_node =
          Attr::Create(GetDocument(), attr_node->GetQualifiedName(), attr.Value());
    }
  }
...
Attr* Element::getAttributeNode(const AtomicString& local_name) {
  ...
  const Attribute* attribute =
      GetElementData()->Attributes().Find(local_name, ShouldIgnoreAttributeCase());
  if (!attribute)
    return nullptr;
...
void Element::focus(const FocusParams& params) {
  ...
  if (AuthorShadowRoot() && AuthorShadowRoot()->delegatesFocus()) {
    ...
    Element* found =
        GetDocument().GetPage()->GetFocusController().FindFocusableElementInShadowHost(
            *this);
  ...
  if (GetDocument().FocusedElement() == this &&
    ...
    GetDocument().GetPage()->GetChromeClient().ShowVirtualKeyboardOnElementFocus(
...
void Element::DispatchFocusEvent(Element* old_focused_element,
  ...
  DispatchEvent(FocusEvent::Create(EventTypeNames::focus, false, false,
                                   GetDocument().domWindow(), 0, old_focused_element,
                                   source_capabilities));
}
...
void Element::DispatchBlurEvent(Element* new_focused_element,
  ...
  DispatchEvent(FocusEvent::Create(EventTypeNames::blur, false, false,
                                   GetDocument().domWindow(), 0, new_focused_element,
                                   source_capabilities));
}
...
void Element::setPointerCapture(int pointer_id, ExceptionState& exception_state) {
  if (GetDocument().GetFrame()) {
    if (!GetDocument().GetFrame()->GetEventHandler().IsPointerEventActive(pointer_id))
      ...
      exception_state.ThrowDOMException(kInvalidStateError, "InvalidStateError");
    else
      GetDocument().GetFrame()->GetEventHandler().SetPointerCapture(pointer_id, this);
  }
}
...
bool Element::hasPointerCapture(int pointer_id) const {
  return GetDocument().GetFrame() &&
         GetDocument().GetFrame()->GetEventHandler().HasPointerCapture(pointer_id, this);
}
...
void Element::UpdatePseudoElement(PseudoId pseudo_id, StyleRecalcChange change) {
  DCHECK(!NeedsStyleRecalc());
...
void Element::CreatePseudoElementIfNeeded(PseudoId pseudo_id) {
  ...
  PseudoElement* element =
      GetDocument().GetStyleEngine().EnsureResolver().CreatePseudoElementIfNeeded(
          *this, pseudo_id);
  if (!element)
    return;
...
PseudoElement* Element::GetPseudoElement(PseudoId pseudo_id) const {
  return HasRareData() ? GetElementRareData()->GetPseudoElement(pseudo_id) : nullptr;
}
...
PassRefPtr<ComputedStyle> Element::GetUncachedPseudoStyle(
    ...
    const ComputedStyle* parent_style) {
  const ComputedStyle* style = GetComputedStyle();
  const bool is_before_or_after =
      request.pseudo_id == kPseudoIdBefore || request.pseudo_id == kPseudoIdAfter;
...
void Element::requestPointerLock() {
  if (GetDocument().GetPage())
    GetDocument().GetPage()->GetPointerLockController().RequestPointerLock(this);
}
...
void Element::SynchronizeStyleAttributeInternal() const {
  ...
  const_cast<Element*>(this)->SetSynchronizedLazyAttribute(
      styleAttr, inline_style ? AtomicString(inline_style->AsText()) : g_null_atom);
}
...
void Element::StyleAttributeChanged(
  ...
  if (GetDocument().GetScriptableDocumentParser() && !GetDocument().IsInDocumentWrite())
    start_line_number = GetDocument().GetScriptableDocumentParser()->LineNumber();
  ...
  if (new_style_string.IsNull()) {
    EnsureUniqueElementData().inline_style_.Clear();
             ...
             (ContainingShadowRoot() &&
              ContainingShadowRoot()->GetType() == ShadowRootType::kUserAgent) ||
  ...
  SetNeedsStyleRecalc(
      kLocalStyleChange,
      StyleChangeReasonForTracing::Create(StyleChangeReason::kStyleSheetChange));
...
bool Element::SupportsStyleSharing() const {
  ...
  if (HasID() && GetDocument().GetStyleEngine().HasRulesForId(IdForStyleResolution()))
    return false;
