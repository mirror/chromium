...
LayoutObject* FirstLetterPseudoElement::FirstLetterTextLayoutObject(
  ...
  while (first_letter_text_layout_object) {
    ...
    if (first_letter_text_layout_object->Style() &&
      ...
      first_letter_text_layout_object = first_letter_text_layout_object->NextSibling();
      ...
      first_letter_text_layout_object = first_letter_text_layout_object->NextSibling();
    } else if (first_letter_text_layout_object->IsFloatingOrOutOfFlowPositioned()) {
  ...
  if (!first_letter_text_layout_object || !first_letter_text_layout_object->IsText() ||
      IsInvalidFirstLetterLayoutObject(first_letter_text_layout_object))
    return nullptr;
...
ComputedStyle* FirstLetterPseudoElement::StyleForFirstLetter(
  ...
  LayoutObject* style_container = ParentOrShadowHostElement()->GetLayoutObject();
  ...
  // into account.
  style_container->MutableStyle()->RemoveCachedPseudoStyle(kPseudoIdFirstLetter);
...
void FirstLetterPseudoElement::AttachFirstLetterTextLayoutObjects() {
  ...
  // applied to it.
  String old_text = ToLayoutText(next_layout_object)->IsTextFragment()
                       ? ToLayoutTextFragment(next_layout_object)->CompleteText()
                       : ToLayoutText(next_layout_object)->OriginalText();
  DCHECK(old_text.Impl());

  ComputedStyle* pseudo_style = StyleForFirstLetter(next_layout_object->Parent());
  ...
  if (next_layout_object->GetNode()) {
    remaining_text = new LayoutTextFragment(
        next_layout_object->GetNode(), old_text.Impl(), length, remaining_length);
  } else {
...
void FirstLetterPseudoElement::DidRecalcStyle() {
  ...
  for (LayoutObject* child = layout_object->NextInPreOrder(layout_object); child;
