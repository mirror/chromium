...
Fullscreen::Fullscreen(Document& document)
    : Supplement<Document>(document),
      ...
      event_queue_timer_(TaskRunnerHelper::Get(TaskType::kUnthrottled, &document),
                        this,
...
void Fullscreen::RequestFullscreen(Element& element,
  ...
  do {
    ...
    document.GetFrame()->GetChromeClient().EnterFullscreen(*document.GetFrame());
...
void Fullscreen::ExitFullscreen(Document& document) {
  ...
  for (Frame* descendant = document.GetFrame()->Tree().TraverseNext(); descendant;
  ...
  for (Document* current_doc = &document; current_doc;) {
    ...
    if (new_top && (!new_top->isConnected() || new_top->GetDocument() != current_doc))
      continue;
...
void Fullscreen::DidEnterFullscreen() {
  ...
  if (current_full_screen_element_ != GetDocument()->documentElement()) {
    LayoutFullScreen::WrapLayoutObject(
        layout_object, layout_object ? layout_object->Parent() : 0, GetDocument());
  }
  ...
  current_full_screen_element_->PseudoStateChanged(CSSSelector::kPseudoFullScreen);

  // FIXME: This should not call updateStyleAndLayoutTree.
  GetDocument()->UpdateStyleAndLayoutTree();

  GetDocument()->GetFrame()->GetChromeClient().FullscreenElementChanged(previous_element,
                                                               element);
}
...
void Fullscreen::SetFullScreenLayoutObject(LayoutFullScreen* layout_object) {
  ...
  if (layout_object && saved_placeholder_computed_style_) {
    layout_object->CreatePlaceholder(std::move(saved_placeholder_computed_style_),
