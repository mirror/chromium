...
// https://fullscreen.spec.whatwg.org/#fire-a-fullscreen-event
void FireEvent(const AtomicString& type, Element* element, Document* document) {
  DCHECK(document);
  DCHECK(element);

  // 1. Let |target| be |element| if |element| is connected and its node
  ...
  // Fire an event named |e|, with its bubbles and composed attributes set to
  // true, at |target|.
...
void Fullscreen::RequestFullscreen(Element& pending, RequestType request_type) {
  ...
  if (!error) {
    ...
    LocalFrame& frame = *document.GetFrame();
    From(document).i_love_state_ = true;
    frame.GetChromeClient().EnterFullscreen(frame);
    From(document).i_love_state_ = false;
  } else {
    ...
    ContinueRequestFullscreen(document, pending, request_type, true);
  }
}
...
void Fullscreen::ContinueRequestFullscreen(Document& document,
  ...
  // 10. If |error| is true:
  if (error) {
    // 10.1. Queue an animation frame task for |pendingDoc| to fire a fullscreen
    // event named fullscreenerror for (|pending|, |pendingDoc|).
  ...
  for (Element* element : fullscreen_elements) {
    ...
    // 13.5. Queue an animation frame task for |doc| to fire a fullscreen event
    // named fullscreenchange for (|element|, |doc|).
...
void Fullscreen::ExitFullscreen(Document& doc) {
  ...
  if (resize) {
    ...
    Microtask::EnqueueMicrotask(
        WTF::Bind(ContinueExitFullscreen, WrapWeakPersistent(&doc), false));
  }
}

void Fullscreen::DidExitFullscreen() {
  ContinueExitFullscreen(GetDocument(), true);
}
...
void Fullscreen::ContinueExitFullscreen(Document* doc, bool resize) {
  ...
  for (Document* exit_doc : exit_docs) {
    ...
    // 12.1. Queue an animation frame task for |exitDoc| to fire a fullscreen
    // event named fullscreenchange for (|exitDoc|'s fullscreen element,
    // |exitDoc|).
  ...
  for (Document* descendant_doc : descendant_docs) {
    ...
    // 13.1. Queue an animation frame task for |descendantDoc| to fire a
    // fullscreen event named fullscreenchange for (|descendantDoc|'s fullscreen
    // element, |descendantDoc|).
    EnqueueEvent(EventTypeNames::fullscreenchange, *stack.back().first,
