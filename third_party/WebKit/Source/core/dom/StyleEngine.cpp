...
void StyleEngine::UpdateStyleSheetsInImport(
  ...
  ImportedDocumentStyleSheetCollector subcollector(parent_collector,
                                                   sheets_for_list);
  GetDocumentStyleSheetCollection().CollectStyleSheets(master_engine, subcollector);
...
void StyleEngine::MarkDocumentDirty() {
  ...
  if (GetDocument().ImportLoader())
    GetDocument().ImportsController()->Master()->GetStyleEngine().MarkDocumentDirty();
  else
    GetDocument().ScheduleLayoutTreeUpdateIfNeeded();
}
...
void StyleEngine::ClassChangedForElement(const SpaceSplitString& changed_classes,
                                         Element& element) {
...
void StyleEngine::AttributeChangedForElement(const QualifiedName& attribute_name,
                                             Element& element) {
  if (ShouldSkipInvalidationFor(element))
    return;

  InvalidationLists invalidation_lists;
  GetRuleFeatureSet().CollectInvalidationSetsForAttribute(invalidation_lists,
...
void StyleEngine::PseudoStateChangedForElement(
  ...
  InvalidationLists invalidation_lists;
  GetRuleFeatureSet().CollectInvalidationSetsForPseudoClass(invalidation_lists,
                                                         element, pseudo_type);
  style_invalidator_.ScheduleInvalidationSetsForNode(invalidation_lists,
                                                     element);
}
...
void StyleEngine::ApplyRuleSetChanges(
  ...
  ActiveSheetsChange change =
      CompareActiveStyleSheets(old_style_sheets, new_style_sheets, changed_rule_sets);
...
const MediaQueryEvaluator& StyleEngine::EnsureMediaQueryEvaluator() {
  if (!media_query_evaluator_) {
    if (GetDocument().GetFrame())
      media_query_evaluator_ = new MediaQueryEvaluator(GetDocument().GetFrame());
    else
