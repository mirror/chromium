...
class RangeUpdateScope {
  ...
  ~RangeUpdateScope() {
    ...
    if (--scope_count_ > 0)
      return;
    Settings* settings =
        old_document_->GetFrame() ? old_document_->GetFrame()->GetSettings() : nullptr;
    if (!settings ||
...
Range* Range::CreateAdjustedToTreeScope(const TreeScope& tree_scope,
  ...
  return Range::Create(tree_scope.GetDocument(), Position::BeforeNode(shadow_host),
...
DocumentFragment* Range::ProcessContents(ActionType action,
  ...
  if (original_start.Container() != common_root &&
    ...
    left_contents = ProcessAncestorsAndTheirSiblings(
        action, original_start.Container(), kProcessContentsForward, left_contents,
        common_root, exception_state);
  }
  ...
  if (end_.Container() != common_root &&
    ...
    right_contents =
        ProcessContentsBetweenOffsets(action, nullptr, original_end.Container(),
                                      0, original_end.Offset(), exception_state);
    right_contents = ProcessAncestorsAndTheirSiblings(
        action, original_end.Container(), kProcessContentsBackward, right_contents,
        common_root, exception_state);
  }
  ...
  if ((action == EXTRACT_CONTENTS || action == CLONE_CONTENTS) && right_contents)
...
Node* Range::ProcessAncestorsAndTheirSiblings(
  ...
  Node* first_child_in_ancestor_to_process = direction == kProcessContentsForward
      ...
      switch (action) {
        ...
        case EXTRACT_CONTENTS:  // will remove child from ancestor
          if (direction == kProcessContentsForward)
            ...
            cloned_container->insertBefore(child, cloned_container->firstChild(),
                                          exception_state);
          break;
        case CLONE_CONTENTS:
          if (direction == kProcessContentsForward)
...
void Range::insertNode(Node* new_node, ExceptionState& exception_state) {
  ...
  if (new_node_type == Node::kDocumentFragmentNode && !new_node->IsShadowRoot()) {
    ...
    if (!check_against->ChildTypeAllowed(new_node_type)) {
      exception_state.ThrowDOMException(
          kHierarchyRequestError, "The node to be inserted is a '" +
                                     new_node->nodeName() +
                                     "' node, which may not be inserted here.");
      return;
    }
...
void Range::surroundContents(Node* new_parent, ExceptionState& exception_state) {
  ...
  if (start_non_text_container != end_non_text_container) {
    exception_state.ThrowDOMException(
        kInvalidStateError, "The Range has partially selected a non-Text node.");
    return;
  }
...
void Range::TextQuads(Vector<FloatQuad>& quads, bool use_selection_height) const {
  Node* start_container = start_.Container();
  DCHECK(start_container);
  Node* end_container = end_.Container();
  DCHECK(end_container);
...
static inline void BoundaryTextNodesMerged(RangeBoundaryPoint& boundary,
  ...
  if (boundary.Container() == old_node.GetNode())
    boundary.Set(old_node.GetNode().previousSibling(), boundary.Offset() + offset,
                 0);
...
static inline void BoundaryTextNodeSplit(RangeBoundaryPoint& boundary,
  ...
  if (boundary.ChildBefore() == &old_node)
    boundary.Set(boundary_container, boundary_offset + 1, old_node.nextSibling());
  else if (boundary.Container() == &old_node &&
           boundary_offset > old_node.length())
    boundary.Set(old_node.nextSibling(), boundary_offset - old_node.length(), 0);
}
  ...
  for (Node* node = FirstNode(); node != stop_node;
    ...
    if (node->IsElementNode()) {
      ...
      if (!node_set.Contains(node->parentNode()) &&
          (start_container == end_container || (!node->contains(start_container) &&
