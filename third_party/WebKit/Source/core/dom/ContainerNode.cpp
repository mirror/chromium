...
bool ContainerNode::CheckAcceptChildGuaranteedNodeTypes(
    ...
    ExceptionState& exception_state) const {
  if (IsDocumentNode())
    return ToDocument(this)->CanAcceptChild(new_child, old_child, exception_state);
  ...
  if (new_child.IsContainerNode() &&
      (new_child.IsDocumentNode() || new_child.parentNode() || !isConnected()) &&
...
bool ContainerNode::CollectChildrenAndRemoveFromOldParentWithCheck(
    ...
    ExceptionState& exception_state) const {
  CollectChildrenAndRemoveFromOldParent(new_child, new_children, exception_state);
  ...
  for (const auto& child : new_children) {
    ...
    if (!CheckAcceptChildGuaranteedNodeTypes(*child, old_child, exception_state))
      return false;
  }
...
Node* ContainerNode::ReplaceChild(Node* new_child,
  ...
  NodeVector targets;
  if (!CollectChildrenAndRemoveFromOldParentWithCheck(next, old_child, *new_child,
                                                      targets, exception_state))
    return old_child;
...
void ContainerNode::FocusStateChanged() {
  ...
  LayoutTheme::GetTheme().ControlStateChanged(*GetLayoutObject(), kFocusControlState);
}
...
void ContainerNode::CheckForSiblingStyleChanges(SiblingCheckType change_type,
  ...
  if (ChildrenAffectedByFirstChildRules() && !element_before_change &&
      element_after_change && element_after_change->AffectedByFirstChildRules()) {
  ...
  if (ChildrenAffectedByLastChildRules() && !element_after_change &&
      element_before_change && element_before_change->AffectedByLastChildRules()) {
