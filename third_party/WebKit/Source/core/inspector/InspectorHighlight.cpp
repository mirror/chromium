...
std::unique_ptr<protocol::DictionaryValue> BuildGapAndPositions(
    double origin,
    LayoutUnit gap,
...
BuildGridInfo(LayoutGrid* layout_grid, FloatPoint origin, Color color) {
  std::unique_ptr<protocol::DictionaryValue> grid_info =
      protocol::DictionaryValue::create();
  grid_info->setValue(
      "rows", BuildGapAndPositions(origin.Y(), layout_grid->GridGap(kForRows),
                                   layout_grid->RowPositions()));
  grid_info->setValue(
      "columns",
      BuildGapAndPositions(origin.X(), layout_grid->GridGap(kForColumns),
