...
// 10000 pixels
static int kMaximumEncodeImageWidth = 10000;

// 10000 pixels
static int kMaximumEncodeImageHeight = 10000;

static double kDefaultEncodeQuality = 0.8;
...
bool EncodeAsImage(char* body,
  ...
  const WebSize maximum_size =
      WebSize(kMaximumEncodeImageWidth, kMaximumEncodeImageHeight);
...
protocol::Response InspectorAuditsAgent::getEncodedResponse(
    ...
    const String& encoding,
    const Maybe<double> quality,
    const Maybe<bool> size_only,
  ...
  if (!EncodeAsImage(reinterpret_cast<char*>(base64_decoded_buffer.data()),
                     base64_decoded_buffer.size(), encoding,
                     quality.fromMaybe(kDefaultEncodeQuality),
                     &encoded_image)) {
