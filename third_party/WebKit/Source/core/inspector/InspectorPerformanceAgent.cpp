...
Response InspectorPerformanceAgent::getMetrics(
  ...
  AppendMetric(result.get(), "Timestamp", now.InSecondsF());
  ...
  if (document) {
    AppendMetric(
        result.get(), "FirstMeaningfulPaint",
        PaintTiming::From(*document).FirstMeaningfulPaint().InSecondsF());
    AppendMetric(
        result.get(), "DomContentLoaded",
        document->GetTiming().DomContentLoadedEventStart().InSecondsF());
    AppendMetric(
        result.get(), "NavigationStart",
        document->Loader()->GetTiming().NavigationStart().InSecondsF());
  }
...
void InspectorPerformanceAgent::WillProcessTask(double start_time) {
  task_start_time_ = TimeTicks::FromSeconds(start_time);
}

void InspectorPerformanceAgent::DidProcessTask(double start_time,
                                               double end_time) {
  if (task_start_time_ == TimeTicks::FromSeconds(start_time))
