...
void InspectorSession::SendProtocolResponse(int call_id, const String& message) {
  if (disposed_)
    return;
...
void InspectorSession::flushProtocolNotifications() {
  ...
  for (size_t i = 0; i < notification_queue_.size(); ++i) {
    client_->SendProtocolMessage(
        session_id_, 0, notification_queue_[i]->Serialize(), String());
  }
