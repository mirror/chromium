...
// static
void SamplingNativeHeapProfiler::HookGCAlloc(uint8_t* address,
                                             size_t size,
                                             const char* type) {
  Sample sample;
  if (LIKELY(!running_ || !CreateAllocSample(size, &sample)))
    return;
  GetInstance()->RecordAlloc(sample, address, 0, skip_oilpan_frames_);
}

// static
void SamplingNativeHeapProfiler::HookGCFree(uint8_t* address) {
  GetInstance()->RecordFree(address);
}

// static
void SamplingNativeHeapProfiler::HookPartitionAlloc(void* address,
                                                    size_t size,
                                                    const char* type) {
  Sample sample;
  if (LIKELY(!running_ || !CreateAllocSample(size, &sample)))
    return;
  GetInstance()->RecordAlloc(sample, address, 0, skip_partition_frames_);
}

// static
void SamplingNativeHeapProfiler::HookPartitionFree(void* address) {
  GetInstance()->RecordFree(address);
}
