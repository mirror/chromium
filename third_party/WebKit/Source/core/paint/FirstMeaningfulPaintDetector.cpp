...
void FirstMeaningfulPaintDetector::ReportSwapTime(
  ...
  provisional_first_meaningful_paint_swap_ = TimeTicks::FromSeconds(timestamp);
...
void FirstMeaningfulPaintDetector::SetFirstMeaningfulPaint(
  ...
  DCHECK(swap_stamp >= stamp);
  DCHECK(!swap_stamp.is_null());
  DCHECK(network2_quiet_reached_);

  double swap_timestamp = swap_stamp.InSecondsF();
  probe::paintTiming(GetDocument(), "firstMeaningfulPaint", swap_timestamp);
