...
BoxPainterBase::FillLayerInfo BoxModelObjectPainter::GetFillLayerInfo(
  ...
  return BoxPainterBase::FillLayerInfo(
      GetDocument(), Style(), box_model_.HasOverflowClip(), color, bg_layer,
      bleed_avoidance, (flow_box_ ? flow_box_->IncludeLogicalLeftEdge() : true),
...
const ComputedStyle& BoxModelObjectPainter::Style() const {
  return box_model_.StyleRef();
}

const Document& BoxModelObjectPainter::GetDocument() const {
  return box_model_.GetDocument();
}

const DisplayItemClient& BoxModelObjectPainter::DisplayItem() const {
  return box_model_;
}

LayoutRectOutsets BoxModelObjectPainter::Border() const {
  return box_model_.BorderBoxOutsets();
}

LayoutRectOutsets BoxModelObjectPainter::Padding() const {
