...
class CORE_EXPORT PaintLayer : public DisplayItemClient {
 ...
 public:
  ...
  bool IsTransparent() const {
    return GetLayoutObject().IsTransparent() ||
           GetLayoutObject().Style()->HasBlendMode() || GetLayoutObject().HasMask();
  }
  ...
  void ConvertToLayerCoords(const PaintLayer* ancestor_layer,
                            LayoutPoint&) const;
  void ConvertToLayerCoords(const PaintLayer* ancestor_layer, LayoutRect&) const;
  ...
  const IntRect& ClippedAbsoluteBoundingBox() const {
    ...
    return ancestor_dependent_compositing_inputs_->clipped_absolute_bounding_box;
  }
  ...
  CompositingReasons GetCompositingReasons() const {
    DCHECK(IsAllowedToQueryCompositingState());
    return rare_data_ ? rare_data_->compositing_reasons : kCompositingReasonNone;
  }
  ...
  LayoutRect PreviousPaintDirtyRect() const { return previous_paint_dirty_rect_; }
 ...
 private:
  ...
  bool ShouldFragmentCompositedBounds(const PaintLayer* compositing_layer) const;
