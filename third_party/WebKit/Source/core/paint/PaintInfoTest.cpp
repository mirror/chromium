...
TEST_F(PaintInfoTest, intersectsCullRect) {
  PaintInfo paint_info(context_, IntRect(0, 0, 50, 50),
                      kPaintPhaseSelfBlockBackgroundOnly, kGlobalPaintNormalPhase,
                      kPaintLayerNoFlag);

  EXPECT_TRUE(paint_info.GetCullRect().IntersectsCullRect(IntRect(0, 0, 1, 1)));
  EXPECT_FALSE(paint_info.GetCullRect().IntersectsCullRect(IntRect(51, 51, 1, 1)));
}
...
TEST_F(PaintInfoTest, intersectsCullRectWithLayoutRect) {
  ...
  EXPECT_TRUE(paint_info.GetCullRect().IntersectsCullRect(LayoutRect(0, 0, 1, 1)));
...
TEST_F(PaintInfoTest, intersectsCullRectWithTransform) {
  ...
  EXPECT_TRUE(paint_info.GetCullRect().IntersectsCullRect(transform,
                                                      IntRect(51, 51, 1, 1)));
  EXPECT_FALSE(paint_info.GetCullRect().IntersectsCullRect(IntRect(52, 52, 1, 1)));
}
...
TEST_F(PaintInfoTest, intersectsVerticalRange) {
  ...
  EXPECT_TRUE(paint_info.GetCullRect().IntersectsVerticalRange(LayoutUnit(),
                                                           LayoutUnit(1)));
  EXPECT_FALSE(paint_info.GetCullRect().IntersectsVerticalRange(LayoutUnit(100),
                                                            LayoutUnit(101)));
}
...
TEST_F(PaintInfoTest, intersectsHorizontalRange) {
  ...
  EXPECT_TRUE(paint_info.GetCullRect().IntersectsHorizontalRange(LayoutUnit(),
                                                             LayoutUnit(1)));
  EXPECT_FALSE(paint_info.GetCullRect().IntersectsHorizontalRange(LayoutUnit(50),
                                                              LayoutUnit(51)));
}
