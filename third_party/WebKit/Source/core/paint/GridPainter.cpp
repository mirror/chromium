...
void GridPainter::PaintChildren(const PaintInfo& paint_info,
  ...
  GridSpan dirtied_rows = DirtiedGridAreas(
      layout_grid_.RowPositions(), local_visual_rect.Y(), local_visual_rect.MaxY());
  ...
  std::stable_sort(grid_items_to_be_painted.begin(), grid_items_to_be_painted.end(),
