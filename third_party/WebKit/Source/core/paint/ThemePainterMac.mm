...
bool ThemePainterMac::PaintTextField(const LayoutObject& o,
  ...
  bool use_ns_text_field_cell = o.StyleRef().HasAppearance() &&
                            o.StyleRef().VisitedDependentColor(
                                CSSPropertyBackgroundColor) == Color::kWhite &&
...
bool ThemePainterMac::PaintCapsLockIndicator(const LayoutObject& o,
  ...
  CGAffineTransform transform =
      CGAffineTransformMake(height_scale, 0,                           // A  B
                            0, height_scale,                           // C  D
...
bool ThemePainterMac::PaintProgressBar(const LayoutObject& layout_object,
  ...
  if (control_size == NSRegularControlSize)
    track_info.kind = layout_progress.GetPosition() < 0 ? kThemeLargeIndeterminateBar
  ...
  LocalCurrentGraphicsContext local_context(image_buffer->Canvas(), 1, clip_rect);
...
bool ThemePainterMac::PaintMenuListButton(const LayoutObject& o,
  ...
  arrow1.lineTo(left_edge + arrow_width, center_y - space_between_arrows / 2.0f);
  ...
  arrow2.lineTo(left_edge + arrow_width, center_y + space_between_arrows / 2.0f);
...
bool ThemePainterMac::PaintSliderTrack(const LayoutObject& o,
  ...
  if (o.StyleRef().Appearance() == kSliderHorizontalPart ||
    ...
    unzoomed_rect.SetY(ceilf(unzoomed_rect.Y() + unzoomed_rect.Height() / 2 -
    ...
    unzoomed_rect.SetX(ceilf(unzoomed_rect.X() + unzoomed_rect.Width() / 2 -
                            zoom_level * LayoutThemeMac::kSliderTrackWidth / 2));
  ...
  if (!LayoutTheme::IsEnabled(o)) {
    ...
    border_gradient_bottom_color = border_gradient_bottom_color.Blend(tint_color);
  ...
  FloatSize shadow_offset(is_vertical_slider ? 1 : 0, is_vertical_slider ? 0 : 1);
  float shadow_blur = 3;
  ...
  paint_info.context.SetStrokeThickness(LayoutThemeMac::kSliderTrackBorderWidth);
...
bool ThemePainterMac::PaintSliderThumb(const LayoutObject& o,
  ...
  border_gradient->AddColorStop(0.0, border_gradient_top_color);
  border_gradient->AddColorStop(1.0, border_gradient_bottom_color);
  paint_info.context.SetStrokeThickness(LayoutThemeMac::kSliderThumbBorderWidth);
  PaintFlags border_flags(paint_info.context.StrokeFlags());
  border_gradient->ApplyToFlags(border_flags, SkMatrix::I());
...
bool ThemePainterMac::PaintCheckbox(const LayoutObject& object,
  ...
  IntRect inflated_rect =
      ThemeMac::InflateRect(zoomed_rect, zoomed_size,
                            ThemeMac::CheckboxMargins(control_size), zoom_factor);
...
bool ThemePainterMac::PaintRadio(const LayoutObject& object,
  ...
  IntRect inflated_rect = ThemeMac::InflateRect(
      zoomed_rect, zoomed_size, ThemeMac::RadioMargins(control_size), zoom_factor);
...
bool ThemePainterMac::PaintButton(const LayoutObject& object,
  ...
  // Determine the width and height needed for the control and prepare the cell
  // for painting.
  NSButtonCell* button_cell = ThemeMac::Button(object.StyleRef().Appearance(),
                                              states, zoomed_rect, zoom_factor);
  GraphicsContextStateSaver state_saver(paint_info.context);
  ...
  if ([button_cell bezelStyle] == NSRoundedBezelStyle) {
    ...
    // Now inflate it to account for the shadow.
    inflated_rect =
        ThemeMac::InflateRect(inflated_rect, zoomed_size,
                              ThemeMac::ButtonMargins(control_size), zoom_factor);
