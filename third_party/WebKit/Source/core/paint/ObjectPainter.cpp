...
void PaintComplexOutline(GraphicsContext& graphics_context,
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    adjacent_width_end = i == count - 1 ? first_adjacent_width
    ...
    ObjectPainter::DrawLineForBoxSide(graphics_context, edge.x1, edge.y1,
                                      edge.x2, edge.y2, edge.side, outline_color,
                                      style.OutlineStyle(), adjacent_width1,
                                      adjacent_width2, false);
...
void ObjectPainter::PaintOutline(const PaintInfo& paint_info,
  ...
  if (!layout_object_.IsBox() &&
    ...
    if (container) {
      layout_object_.LocalToAncestorRects(outline_rects, container, -paint_offset,
                                          paint_offset);
      if (outline_rects.IsEmpty())
        return;
    }
  ...
  IntRect united_outline_rect = UnionRectEvenIfEmpty(pixel_snapped_outline_rects);
  ...
  PaintComplexOutline(paint_info.context, pixel_snapped_outline_rects, style_to_use,
                      color);
}
...
void ObjectPainter::DrawLineForBoxSide(GraphicsContext& graphics_context,
  ...
  switch (style) {
    ...
    case kBorderStyleOutset:
      if (style == kBorderStyleOutset && (side == kBSBottom || side == kBSRight))
        color = color.Dark();
    // fall through
  ...
  switch (side) {
    case kBSTop:
      DrawLineForBoxSide(graphics_context,
                         x1 + std::max((-adjacent_width1 * 2 + 1) / 3, 0), y1,
      ...
      DrawLineForBoxSide(
          ...
          y2 - third_of_thickness, x2 - std::max((adjacent_width2 * 2 + 1) / 3, 0),
          y2, side, color, kBorderStyleSolid, adjacent1_big_third,
    ...
    case kBSLeft:
      DrawLineForBoxSide(
          ...
          kBorderStyleSolid, adjacent1_big_third, adjacent2_big_third, antialias);
    ...
    case kBSRight:
      DrawLineForBoxSide(
          graphics_context, x1, y1 + std::max((adjacent_width1 * 2 + 1) / 3, 0),
          x1 + third_of_thickness, y2 - std::max((adjacent_width2 * 2 + 1) / 3, 0),
          side, color, kBorderStyleSolid, adjacent1_big_third, adjacent2_big_third,
          antialias);
      DrawLineForBoxSide(graphics_context, x2 - third_of_thickness,
                         y1 + std::max((-adjacent_width1 * 2 + 1) / 3, 0), x2,
                         y2 - std::max((-adjacent_width2 * 2 + 1) / 3, 0), side,
...
void ObjectPainter::DrawRidgeOrGrooveBoxSide(GraphicsContext& graphics_context,
  ...
  switch (side) {
    case kBSTop:
      DrawLineForBoxSide(graphics_context, x1 + std::max(-adjacent_width1, 0) / 2,
                         y1, x2 - std::max(-adjacent_width2, 0) / 2,
                         (y1 + y2 + 1) / 2, side, color, s1, adjacent1_big_half,
                         adjacent2_big_half, antialias);
      DrawLineForBoxSide(
          graphics_context, x1 + std::max(adjacent_width1 + 1, 0) / 2,
          (y1 + y2 + 1) / 2, x2 - std::max(adjacent_width2 + 1, 0) / 2, y2, side,
          color, s2, adjacent_width1 / 2, adjacent_width2 / 2, antialias);
      break;
    case kBSLeft:
      DrawLineForBoxSide(
      ...
      DrawLineForBoxSide(graphics_context, (x1 + x2 + 1) / 2,
                         ...
                         s2, adjacent_width1 / 2, adjacent_width2 / 2, antialias);
      break;
    case kBSBottom:
      DrawLineForBoxSide(graphics_context, x1 + std::max(adjacent_width1, 0) / 2,
                         y1, x2 - std::max(adjacent_width2, 0) / 2,
    ...
    case kBSRight:
      ...
      DrawLineForBoxSide(graphics_context, (x1 + x2 + 1) / 2,
                         y1 + std::max(-adjacent_width1 + 1, 0) / 2, x2,
                         y2 - std::max(-adjacent_width2 + 1, 0) / 2, side, color,
                         s1, adjacent_width1 / 2, adjacent_width2 / 2, antialias);
      break;
  }
}
