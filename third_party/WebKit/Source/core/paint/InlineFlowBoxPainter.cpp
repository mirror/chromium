...
LayoutRect InlineFlowBoxPainter::PaintRectForImageStrip(
  ...
  LayoutUnit strip_width =
      inline_flow_box_.IsHorizontal() ? total_logical_width : frame_size.Width();
  LayoutUnit strip_height =
      inline_flow_box_.IsHorizontal() ? frame_size.Height() : total_logical_width;
...
InlineFlowBoxPainter::GetBorderPaintType(const LayoutRect& adjusted_frame_rect,
  ...
  if (inline_flow_box_.Parent() &&
    ...
    bool has_border_image = border_image_source && border_image_source->CanRender();
...
void InlineFlowBoxPainter::PaintBoxDecorationBackground(
  ...
  const ComputedStyle* style_to_use = inline_flow_box_.GetLineLayoutItem().Style(
  ...
  PaintFillLayers(paint_info, background_color, style_to_use->BackgroundLayers(),
                  adjusted_frame_rect);
...
void InlineFlowBoxPainter::PaintMask(const PaintInfo& paint_info,
  ...
  if (!mask_blending_applied_by_compositor) {
    if ((mask_box_image &&
         inline_flow_box_.GetLineLayoutItem()
             .Style()
             ->MaskLayers()
             .HasImage()) ||
