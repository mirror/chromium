...
NinePieceImageGrid::NinePieceImageGrid(const NinePieceImage& nine_piece_image,
  ...
  top_.slice =
      ComputeEdgeSlice(nine_piece_image.ImageSlices().Top(), image_size.Height());
  right_.slice =
      ComputeEdgeSlice(nine_piece_image.ImageSlices().Right(), image_size.Width());
  bottom_.slice = ComputeEdgeSlice(nine_piece_image.ImageSlices().Bottom(),
                                    image_size.Height());
  left_.slice =
      ComputeEdgeSlice(nine_piece_image.ImageSlices().Left(), image_size.Width());

  top_.width =
      ComputeEdgeWidth(nine_piece_image.BorderSlices().Top(), border_widths.Top(),
                       top_.slice, border_image_area.Height());
...
void NinePieceImageGrid::SetDrawInfoCorner(NinePieceDrawInfo& draw_info,
  ...
  switch (piece) {
    ...
    case kBottomLeftPiece:
      SetCornerPiece(draw_info, bottom_.IsDrawable() && left_.IsDrawable(),
                     Subrect(image_size_, 0, -bottom_.slice, left_.slice,
                             bottom_.slice),
                     Subrect(border_image_area_, 0, -bottom_.width,
                             left_.width, bottom_.width));
      break;
    case kTopRightPiece:
      SetCornerPiece(
...
NinePieceImageGrid::NinePieceDrawInfo NinePieceImageGrid::GetNinePieceDrawInfo(
  ...
  draw_info.is_corner_piece = piece == kTopLeftPiece || piece == kTopRightPiece ||
                           piece == kBottomLeftPiece ||
                           piece == kBottomRightPiece;
