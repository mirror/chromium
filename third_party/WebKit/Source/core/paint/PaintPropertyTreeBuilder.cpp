...
void FrameViewPaintPropertyTreeBuilder::Update(
  ...
  // Assume FrameView paint property
  if (RuntimeEnabledFeatures::SlimmingPaintV175Enabled() &&
      (frame_view.NeedsPaintPropertyUpdate() ||
...
class FragmentPaintPropertyTreeBuilder {
 public:
  FragmentPaintPropertyTreeBuilder(
      ...
      : object_(object),
        ...
        properties_changed_(properties_changed) {}
 ...
 private:
  ...
  void OnUpdate(const ObjectPaintProperties::UpdateResult& result) {
    ...
    properties_changed_ |= !result.Unchanged();
  }
  ...
  void OnClear(bool cleared) {
    full_context_.force_subtree_update |= cleared;
    properties_changed_ |= cleared;
  }
...
void ObjectPaintPropertyTreeBuilder::UpdateForSelf() {
  ...
  for (auto& fragment_context : context_.fragments) {
    FragmentPaintPropertyTreeBuilder(object_, context_, fragment_context,
                                     *fragment_data, properties_changed)
        .UpdateForSelf();
    fragment_data = fragment_data->NextFragment();
  }
  DCHECK(!fragment_data);

  if (properties_changed && RuntimeEnabledFeatures::SlimmingPaintV175Enabled())
