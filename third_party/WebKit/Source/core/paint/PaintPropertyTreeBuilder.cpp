...
void PaintPropertyTreeBuilder::UpdateProperties(
  ...
  if (frame_view.NeedsPaintPropertyUpdate() || context.force_subtree_update) {
    ...
    frame_translate.Translate(frame_view.X() + context.current.paint_offset.X(),
    ...
    if (frame_view.IsScrollable() || !scroll_offset.IsZero()) {
      ...
      auto reasons = GetMainThreadScrollingReasons(frame_view, ancestor_reasons);
    ...
    if (object.IsBox()) {
      ...
      if (style.HasTransform() || style.Preserves3D() ||
        ...
        context.force_subtree_update |= properties.UpdateTransform(
            ...
            context.current.should_flatten_inherited_transform, rendering_context_id,
...
void PaintPropertyTreeBuilder::UpdateEffect(
  ...
  if (object.NeedsPaintPropertyUpdate() || context.force_subtree_update) {
    const ClipPaintPropertyNode* output_clip = context.input_clip_of_current_effect;
...
void PaintPropertyTreeBuilder::UpdateFilter(
  ...
  if (properties && properties->Filter()) {
    ...
    context.input_clip_of_current_effect = context.current.clip =
        context.absolute_position.clip = context.fixed_position.clip = input_clip;
  }
}
...
void PaintPropertyTreeBuilder::UpdateLocalBorderBoxContext(
  ...
  if (!object.HasLayer()) {
    ...
    PropertyTreeState local_border_box = PropertyTreeState(
        context.current.transform, context.current.clip, context.current_effect);
    object.GetMutableForPainting().SetLocalBorderBoxProperties(local_border_box);
  }
}
...
void PaintPropertyTreeBuilder::UpdateOverflowClip(
  ...
  if (object.NeedsPaintPropertyUpdate() || context.force_subtree_update) {
    ...
    if (box.ShouldClipOverflow()) {
      clip_rect = LayoutRect(box.OverflowClipRect(context.current.paint_offset));
    } else {
      if (auto* properties = object.GetMutableForPainting().PaintProperties()) {
...
static FloatPoint PerspectiveOrigin(const LayoutBox& box) {
  ...
  return FloatPoint(
      ...
      FloatValueForLength(style.PerspectiveOriginY(), border_box_size.Height()));
}
...
static MainThreadScrollingReasons GetMainThreadScrollingReasons(
  ...
  return GetMainThreadScrollingReasons(*object.GetFrameView(), ancestor_reasons);
}
...
void PaintPropertyTreeBuilder::UpdateScrollAndScrollTranslation(
  ...
  if (object.NeedsPaintPropertyUpdate() || context.force_subtree_update) {
    ...
    if (object.HasOverflowClip()) {
      ...
      bool scroll_node_needed_for_main_thread_reasons = ancestor_reasons != reasons;

      const LayoutBox& box = ToLayoutBox(object);
      auto* scrollable_area = box.GetScrollableArea();
      IntSize scroll_offset = box.ScrolledContentOffset();
      if (scroll_node_needed_for_main_thread_reasons || !scroll_offset.IsZero() ||
          scrollable_area->ScrollsOverflow()) {
...
void PaintPropertyTreeBuilder::UpdateOutOfFlowContext(
  ...
  if (object.IsLayoutView()) {
    ...
    // them.
    auto* css_clip = object.GetMutableForPainting().PaintProperties()->CssClip();
    ...
    if (context.fixed_position.clip == css_clip->Parent()) {
      ...
      if (object.NeedsPaintPropertyUpdate() || context.force_subtree_update) {
        ...
        context.force_subtree_update |= properties.UpdateCssClipFixedPosition(
            context.fixed_position.clip, const_cast<TransformPaintPropertyNode*>(
...
#if DCHECK_IS_ON()
  FindObjectPropertiesNeedingUpdateScope check_needs_update_scope(object, context);
#endif
