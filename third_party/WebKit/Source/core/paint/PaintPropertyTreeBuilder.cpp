  ...
  if (frame_view.NeedsPaintPropertyUpdate() ||
    ...
    if (frame_view.ScrollNode()) {
      ...
      full_context.force_subtree_update |= UpdateFrameScrollingContentsClip(
          frame_view, frame_view.ViewportClip(), frame_view.ScrollTranslation(),
...
void PaintPropertyTreeBuilder::UpdateScrollingContentsClip(
  ...
  if (object.NeedsPaintPropertyUpdate() || force_subtree_update) {
    if (properties.Scroll() && !object.StyleRef().IsOverflowPaged()) {
      ...
      const auto* scrollable_area = box.GetScrollableArea();
      FloatRect clip_rect;
      if (box.StyleRef().IsOverflowPaged()) {
        // TODO(wangxianzhu): compute the corect scrolling contents clip rect
        // for paged overflow.
        clip_rect = FloatRect(LayoutRect::InfiniteIntRect());
      } else {
        clip_rect = FloatRect(-FloatPoint(scrollable_area->ScrollOrigin()),
        ...
        // we move the contents to the left with additional ScrollTranslation
        // (-VerticalScrollbarWidth, 0). As ScrollingContentsClip is under
        // ScrollTranslation, we need to compensate the additional
        // ScrollTranslation to make the origin of the clip correct.
