...
void PaintPropertyTreeBuilder::UpdateEffect(
  ...
  if (object.NeedsPaintPropertyUpdate() || force_subtree_update) {
    ...
    if (NeedsEffect(object)) {
      ...
      if (has_mask &&
          // For now disable mask clip for SPv1 because it fails some layout
          // tests (e.g. fast/borders/block-mask-overlay-image-outset.html).
          // The expectations of the tests might be wrong but I would like to
          // leave SPv1 behavior as-is for now.
