...
static bool NeedsPaintOffsetTranslation(const LayoutObject& object) {
  ...
  // Don't let paint offset cross paint invalidation container boundaries, to
  // avoid unnecessary paint/raster invalidation when paint offset to ancestor
  // transform node changes which should not affect the descendants of the
