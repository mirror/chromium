...
void PaintPropertyTreeBuilder::UpdatePropertiesForSelf(
  ...
  LOG(ERROR) << "UpdatePropertiesForSelf: object=" << object.DebugName()
             << " root=" << full_context.fragments[0].current.paint_offset_root;
...
void PaintPropertyTreeBuilder::UpdatePropertiesForChildren(
  ...
  for (auto& fragment_context : context.fragments) {
    ...
    LOG(ERROR) << "obj: " << object.DebugName() << " paint offset root: "
