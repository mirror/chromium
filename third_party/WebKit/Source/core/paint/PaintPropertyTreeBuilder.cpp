...
void PaintPropertyTreeBuilder::UpdateFragmentClip(
  ...
  if (object.NeedsPaintPropertyUpdate() || force_subtree_update) {
    ...
    if (NeedsFragmentationClip(object) && context.has_fragment_clip) {
      DCHECK(context.has_fragment_clip);
...
void PaintPropertyTreeBuilder::UpdateOverflowClip(
  ...
  if (object.NeedsPaintPropertyUpdate() || force_subtree_update) {
    ...
    if (NeedsOverflowClip(object)) {
      ...
      if (context.has_fragment_clip)
        clip_rect.SetLocation(context.fragment_clip.Location());
...
void PaintPropertyTreeBuilder::UpdateForObjectLocationAndSize(
  ...
  // 4.
  if (context.has_fragment_clip && fragment_data && object.HasLayer()) {
    ...
    {
      // See comment in UpdateFragments for more info on step 5 below.
      // 5. Adjust pagination clip for visual offset of pagination container
      // within root.
      LayoutPoint pagination_visual_offset =
          enclosing_pagination_layer->VisualOffsetFromAncestor(
              enclosing_pagination_layer->Root());

      context.fragment_clip.MoveBy(pagination_visual_offset);
    }
...
PaintPropertyTreeBuilderFragmentContext ContextForFragment(
  ...
  for (auto& fragment_context : parent_fragments) {
    if (!fragment_context.has_fragment_clip ||
      ...
      context.has_fragment_clip = true;
      return context;
    }
  }
...
void PaintPropertyTreeBuilder::InitSingleFragmentFromParent(
  ...
  if (full_context.fragments.IsEmpty()) {
    ...
    full_context.fragments[0].has_fragment_clip = false;
    full_context.fragments[0].fragment_clip = LayoutRect();
  }
}
