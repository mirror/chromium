...
void PaintLayerPainter::PaintAncestorClippingMask(
  ...
  Optional<LayerClipRecorder> clip_recorder;
  if (RuntimeEnabledFeatures::SlimmingPaintV175Enabled()) {
    auto state = paint_layer_.GetLayoutObject()
                     .FirstFragment()
                     .GetRarePaintData()
                     ->PreEffectProperties();
    state.SetEffect(context.GetPaintController()
                        .CurrentPaintChunkProperties()
                        .property_tree_state.Effect());
    properties.emplace(context.GetPaintController(), state, client,
                       DisplayItem::kClippingMask);
  } else {
    clip_recorder.emplace(
        context, paint_layer_, DisplayItem::kClipLayerClippingMask,
        fragment.foreground_rect, local_painting_info.root_layer,
