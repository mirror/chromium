...
PaintResult PaintLayerPainter::PaintLayerContents(
  ...
  Optional<ScopedPaintChunkProperties> scoped_chunk_properties;
  if (RuntimeEnabledFeatures::SlimmingPaintV175Enabled()) {
    ...
    scoped_chunk_properties.emplace(context.GetPaintController(), paint_layer_,
                                    PaintChunk::kLayerChunk, properties);
  }
  ...
  {  // Begin block for the lifetime of any filter.
    ...
    if (should_paint_background) {
      Optional<ScopedPaintChunkProperties> background_properties;
      // Allocate a dedicated chunk for the background, so that the main chunk
      // after negative z-order children can have its own meaningful id.
      if (RuntimeEnabledFeatures::SlimmingPaintV175Enabled()) {
        background_properties.emplace(
            context.GetPaintController(), paint_layer_,
            PaintChunk::kLayerChunkBackground,
    ...
    // Ensure the foreground chunk has a meaningful id. The id won't conflict
    // with other chunks because we haven't used scoped_chunk_properties to
    // paint any chunk.
    if (RuntimeEnabledFeatures::SlimmingPaintV175Enabled())
      scoped_chunk_properties->Continue();
