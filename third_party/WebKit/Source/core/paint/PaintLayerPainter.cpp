...
PaintResult PaintLayerPainter::PaintLayerContents(
  ...
  {  // Begin block for the lifetime of any filter.
    ...
    if (should_paint_background) {
      ...
      // Allocate a dedicated chunk for the background if we will paint negative
      // z-order children, so that the main chunk after the children can have
      // its own meaningful id.
      if (RuntimeEnabledFeatures::SlimmingPaintV175Enabled() &&
          should_paint_neg_z_order_list &&
          paint_layer_.StackingNode()->HasNegativeZOrderList()) {
    ...
    // Ensure the main chunk has a meaningful id. The id won't conflict with
    // other chunks because this is the first chunk painted with
