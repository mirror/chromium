...
PaintResult PaintLayerPainter::PaintLayerContents(
  ...
  {  // Begin block for the lifetime of any filter.
    ...
    if (RuntimeEnabledFeatures::SlimmingPaintV175Enabled() &&
      ...
      // display item to ensure the filters will not be ignored.
...
// Generate a no-op DrawingDisplayItem to ensure the chunk with filter effect
// to be non-empty so that the filter effect will not be ignored even if the
// content is empty.
