    ...
    if (RuntimeEnabledFeatures::SlimmingPaintV175Enabled()) {
      ...
      if (object.IsSVGRoot()) {
        ...
        DCHECK(!object_properties || !object_properties->Transform() ||
               object.HasLayer());
        transform_node = object_properties->SvgLocalToBorderBoxTransform();
        DCHECK(!transform_node ||
               transform_node->Matrix() == transform.ToTransformationMatrix());
      } else {
        DCHECK(object.IsSVG());
        // Should only be used by LayoutSVGRoot.
        DCHECK(!object_properties->SvgLocalToBorderBoxTransform());
        transform_node = object_properties->Transform();
      }
