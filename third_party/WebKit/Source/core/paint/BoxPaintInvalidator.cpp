...
static LayoutRect ComputeRightDelta(const LayoutPoint& location,
  ...
  if (delta > 0) {
    return LayoutRect(location.X() + old_size.Width() - extra_width, location.Y(),
                      delta + extra_width, new_size.Height());
  }
  if (delta < 0) {
    return LayoutRect(location.X() + new_size.Width() - extra_width, location.Y(),
                      -delta + extra_width, old_size.Height());
  }
  return LayoutRect();
}
...
void BoxPaintInvalidator::InvalidateScrollingContentsBackgroundIfNeeded() {
  ...
  if (box_.BackgroundChangedSinceLastPaintInvalidation()) {
    ...
    should_fully_invalidate_on_scrolling_contents_layer = should_fully_invalidate;
  }
  ...
  if (should_fully_invalidate_on_scrolling_contents_layer) {
    ...
    IncrementallyInvalidatePaint(
        kPaintInvalidationBackgroundOnScrollingContentsLayer, old_layout_overflow,
        new_layout_overflow);
  }
...
PaintInvalidationReason BoxPaintInvalidator::InvalidatePaintIfNeeded() {
  ...
  if (reason == kPaintInvalidationIncremental) {
    ...
    reason = std::max(
        reason,
        ObjectPaintInvalidatorWithContext(box_, context_)
            .InvalidatePaintIfNeededWithComputedReason(kPaintInvalidationNone));
  } else {
