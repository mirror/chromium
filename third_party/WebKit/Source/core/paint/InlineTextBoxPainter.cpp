...
// Text match markers paint over truncation ellipses, so this function doesn't
// use the same logic as MarkerPaintStartAndEnd().
...
std::pair<unsigned, unsigned> InlineTextBoxPainter::MarkerPaintStartAndEnd(
    const DocumentMarker& marker) {
  ...
  unsigned paint_end = std::min<int>(
  ...
  if (truncation != kCNoTruncation) {
    ...
    // truncation ellipsis is at the *start* of the text box rather than the
    // end.
    bool ltr = inline_text_box_.IsLeftToRightDirection();
    bool flow_is_ltr = inline_text_box_.GetLineLayoutItem()
                           .ContainingBlock()
    ...
    // node.
    if (ltr == flow_is_ltr) {
      paint_start = std::min<int>(paint_start, truncation);
      paint_end = std::min<int>(paint_end, truncation);
    } else {
      paint_start = std::max<int>(paint_start, truncation);
      paint_end = std::max<int>(paint_end, truncation);
    }
  }

  return std::make_pair(paint_start, paint_end);
}
  ...
  for (; marker_it != markers.end(); ++marker_it) {
    ...
    switch (marker.GetType()) {
      ...
      case DocumentMarker::kActiveSuggestion: {
        ...
        if (marker_paint_phase == DocumentMarkerPaintPhase::kBackground) {
          unsigned marker_paint_start, marker_paint_end;
          std::tie(marker_paint_start, marker_paint_end) =
          ...
          PaintSingleMarkerBackgroundRun(paint_info.context, box_origin, style,
                                         ...
                                         marker_paint_start, marker_paint_end);
        } else {
...
void InlineTextBoxPainter::PaintDocumentMarker(GraphicsContext& context,
  ...
  if (!marker_spans_whole_box || grammar) {
    unsigned paint_start, paint_end;
    std::tie(paint_start, paint_end) = MarkerPaintStartAndEnd(marker);
    ...
    IntRect marker_rect = EnclosingIntRect(font.SelectionRectForText(
        run, FloatPoint(start_point), sel_height, paint_start, paint_end));
