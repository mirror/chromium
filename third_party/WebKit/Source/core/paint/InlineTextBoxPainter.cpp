...
static int ComputeUnderlineOffsetForRoman(const FontMetrics& font_metrics,
...
Path AppliedDecorationPainter::PrepareWavyStrokePath() {
  ...
  if (is_vertical_line) {
    ...
    for (float y = y1; y + 2 * step <= y2;) {
      ...
      y += 2 * step;
      path.AddBezierCurveTo(control_point1, control_point2, FloatPoint(x_axis, y));
    }
  } else {
    DCHECK(p1.Y() == p2.Y());
    ...
    for (float x = x1; x + 2 * step <= x2;) {
      ...
      x += 2 * step;
      path.AddBezierCurveTo(control_point1, control_point2, FloatPoint(x, y_axis));
    }
  }
  return path;
}
...
bool InlineTextBoxPainter::PaintsMarkerHighlights(
  ...
  return layout_object.GetNode() &&
         layout_object.GetDocument().Markers().HasMarkers(layout_object.GetNode());
}
...
static bool PaintsCompositionMarkers(const LayoutObject& layout_object) {
  return layout_object.GetNode() &&
         layout_object.GetDocument()
                 .Markers()
                 .MarkersFor(layout_object.GetNode(), DocumentMarker::kComposition)
                 .size() > 0;
}
...
void InlineTextBoxPainter::Paint(const PaintInfo& paint_info,
  ...
  bool have_selection = !is_printing && paint_info.phase != kPaintPhaseTextClip &&
  ...
  TextPainter::Style selection_style = TextPainter::SelectionPaintingStyle(
      inline_text_box_.GetLineLayoutItem(), have_selection, paint_info, text_style);
  ...
  if (inline_text_box_.Truncation() != kCNoTruncation) {
    ...
    length = ltr == flow_is_ltr ? inline_text_box_.Truncation() : text_run.length();
  }
  ...
  bool has_text_emphasis =
      inline_text_box_.GetEmphasisMarkPosition(style_to_use, emphasis_mark_position);
...
bool InlineTextBoxPainter::ShouldPaintTextBox(const PaintInfo& paint_info) {
  ...
  if (inline_text_box_.GetLineLayoutItem().Style()->Visibility() !=
      ...
      inline_text_box_.Truncation() == kCFullTruncation || !inline_text_box_.Len())
    return false;
  return true;
}
...
void InlineTextBoxPainter::PaintSingleCompositionBackgroundRun(
  ...
  int s_pos = std::max(start_pos - static_cast<int>(inline_text_box_.Start()), 0);
  ...
  context.DrawHighlightForText(font, inline_text_box_.ConstructTextRun(style),
                               local_origin, sel_height, background_color, s_pos,
                               e_pos);
}
...
void InlineTextBoxPainter::PaintDocumentMarkers(
  ...
  for (; marker_it != markers.end(); ++marker_it) {
    ...
    switch (marker.GetType()) {
      ...
      case DocumentMarker::kTextMatch:
        if (marker_paint_phase == DocumentMarkerPaintPhase::kBackground)
          inline_text_box_.PaintTextMatchMarkerBackground(paint_info, box_origin,
                                                         marker, style, font);
        else
          inline_text_box_.PaintTextMatchMarkerForeground(paint_info, box_origin,
                                                         marker, style, font);
        break;
      case DocumentMarker::kComposition: {
...
void InlineTextBoxPainter::PaintDocumentMarker(GraphicsContext& context,
  ...
  if (!marker_spans_whole_box || grammar) {
    ...
    IntRect marker_rect = EnclosingIntRect(font.SelectionRectForText(
        run, FloatPoint(start_point), sel_height, start_position, end_position));
  ...
  if (descent <= (line_thickness + 2)) {
    ...
    underline_offset = (inline_text_box_.LogicalHeight() - line_thickness).ToInt();
  } else {
...
void InlineTextBoxPainter::PaintSelection(GraphicsContext& context,
  ...
  TextRun text_run = inline_text_box_.ConstructTextRun(
      style, string, inline_text_box_.GetLineLayoutItem().TextLength() -
                         inline_text_box_.Start(),
  ...
  if (options == InlineTextBoxPainter::PaintOptions::kCombinedText) {
    ...
    context.DrawHighlightForText(font, text_run, FloatPoint(box_rect.Location()),
  ...
  LayoutRect selection_rect = LayoutRect(
      font.SelectionRectForText(text_run, local_origin, sel_height, s_pos, e_pos));
  ...
  if (!flow_is_ltr && !ltr && inline_text_box_.Truncation() != kCNoTruncation)
    selection_rect.Move(inline_text_box_.LogicalWidth() - selection_rect.Width(),
                       LayoutUnit());
...
void InlineTextBoxPainter::PaintDecorations(
  ...
  if (inline_text_box_.Truncation() != kCNoTruncation) {
    ...
    width = LayoutUnit(inline_text_box_.GetLineLayoutItem().Width(
        ltr == flow_is_ltr
            ? inline_text_box_.Start()
            : inline_text_box_.Start() + inline_text_box_.Truncation(),
        ltr == flow_is_ltr ? inline_text_box_.Truncation()
  ...
  if (font_data) {
    text_decoration_thickness = font_data->GetFontMetrics().UnderlineThickness();
  ...
  for (const AppliedTextDecoration& decoration : decorations) {
    ...
    if ((lines & kTextDecorationUnderline) && font_data) {
      ...
      AppliedDecorationPainter decoration_painter(
          ...
          width.ToFloat(), decoration, text_decoration_thickness, double_offset, 1,
          antialias_decoration);
      if (skip_intercepts) {
    ...
    if (lines & kTextDecorationLineThrough) {
      ...
      AppliedDecorationPainter decoration_painter(
          context, FloatPoint(local_origin) + FloatPoint(0, line_through_offset),
          width.ToFloat(), decoration, text_decoration_thickness, double_offset, 0,
          antialias_decoration);
...
void InlineTextBoxPainter::PaintCompositionUnderline(
  ...
  float start =
      paint_start == inline_text_box_.Start()
          ...
          : inline_text_box_.GetLineLayoutItem().Width(
                inline_text_box_.Start(), paint_start - inline_text_box_.Start(),
                inline_text_box_.TextPos(),
  ...
  if (paint_start == inline_text_box_.Start() &&
    ...
    width = inline_text_box_.GetLineLayoutItem().Width(
        paint_from, paint_length, LayoutUnit(inline_text_box_.TextPos() + start),
        flow_is_ltr ? TextDirection::kLtr : TextDirection::kRtl,
        inline_text_box_.IsFirstLineStyle());
  }
...
void InlineTextBoxPainter::PaintTextMatchMarkerForeground(
    ...
    const ComputedStyle& style,
    const Font& font) {
  if (!InlineLayoutObject().GetFrame()->GetEditor().MarkedTextMatchesAreHighlighted())
    return;
  ...
  LayoutRect box_rect(box_origin, LayoutSize(inline_text_box_.LogicalWidth(),
                                           inline_text_box_.LogicalHeight()));
  LayoutPoint text_origin(box_origin.X(),
                         box_origin.Y() + font_data->GetFontMetrics().Ascent());
