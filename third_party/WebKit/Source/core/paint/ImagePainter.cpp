...
void ImagePainter::PaintAreaElementFocusRing(const PaintInfo& paint_info,
  ...
  path.Translate(FloatSize(adjusted_paint_offset.X(), adjusted_paint_offset.Y()));
  ...
  LayoutObjectDrawingRecorder drawing_recorder(paint_info.context, layout_image_,
...
void ImagePainter::PaintIntoRect(GraphicsContext& context,
  ...
  if (!content_rect.Contains(dest_rect)) {
    ...
    if (pixel_snapped_content_rect.IsEmpty())
      return;
    src_rect = MapRect(pixel_snapped_content_rect, pixel_snapped_dest_rect, src_rect);
