...
GraphicsLayer* PaintLayerScrollableArea::LayerForHorizontalScrollbar() const {
  ...
  return Layer()->HasCompositedLayerMapping()
             ? Layer()->GetCompositedLayerMapping()->LayerForHorizontalScrollbar()
             : 0;
}
...
IntRect PaintLayerScrollableArea::ConvertFromScrollbarToContainingFrameViewBase(
  ...
  return view->GetFrameView()->ConvertFromLayoutItem(LayoutBoxItem(&Box()), rect);
}
...
IntRect PaintLayerScrollableArea::ConvertFromContainingFrameViewBaseToScrollbar(
  ...
  IntRect rect =
      view->GetFrameView()->ConvertToLayoutItem(LayoutBoxItem(&Box()), parent_rect);
  rect.Move(-ScrollbarOffset(scrollbar));
  return rect;
}
...
PaintLayerScrollableArea::ConvertFromScrollbarToContainingFrameViewBase(
  ...
  return view->GetFrameView()->ConvertFromLayoutItem(LayoutBoxItem(&Box()), point);
}
...
PaintLayerScrollableArea::ConvertFromContainingFrameViewBaseToScrollbar(
  ...
  IntPoint point = view->GetFrameView()->ConvertToLayoutItem(LayoutBoxItem(&Box()),
                                                          parent_point);
...
void PaintLayerScrollableArea::UpdateScrollOffset(const ScrollOffset& new_offset,
  ...
  FloatQuad quad_for_fake_mouse_move_event = FloatQuad(
      FloatRect(Layer()->GetLayoutObject().VisualRectIncludingCompositedScrolling(
          paint_invalidation_container)));

  quad_for_fake_mouse_move_event =
      paint_invalidation_container.LocalToAbsoluteQuad(quad_for_fake_mouse_move_event);
...
IntPoint PaintLayerScrollableArea::LastKnownMousePosition() const {
  return Box().GetFrame() ? Box().GetFrame()->GetEventHandler().LastKnownMousePosition()
                       : IntPoint();
}
...
void PaintLayerScrollableArea::UpdateScrollbarEnabledState() {
  ...
  if (HorizontalScrollbar())
    HorizontalScrollbar()->SetEnabled(HasHorizontalOverflow() && !force_disable);
...
void PaintLayerScrollableArea::UpdateAfterLayout() {
  ...
  ComputeScrollbarExistence(needs_horizontal_scrollbar, needs_vertical_scrollbar);
  ...
  if (scrollbars_will_change) {
    ...
    if ((horizontal_scrollbar_should_change &&
      ...
      if ((vertical_scrollbar_should_change && Box().IsHorizontalWritingMode()) ||
      ...
      if (relayout_is_prevented) {
        ...
        layout_scope.SetNeedsLayout(&Box(),
                                   LayoutInvalidationReason::kScrollbarChanged);
...
void PaintLayerScrollableArea::UpdateAfterStyleChange(
  ...
  if (old_style) {
    old_background = old_style->VisitedDependentColor(CSSPropertyBackgroundColor);
  }
  ...
  ComputeScrollbarExistence(needs_horizontal_scrollbar, needs_vertical_scrollbar,
                            kForbidAddingAutoBars);

  // Avoid some unnecessary computation if there were and will be no scrollbars.
  if (!HasScrollbar() && !needs_horizontal_scrollbar && !needs_vertical_scrollbar)
    return;
...
bool PaintLayerScrollableArea::UpdateAfterCompositingChange() {
  ...
  const bool layers_changed = topmost_scroll_child_ != next_topmost_scroll_child_;
...
void PaintLayerScrollableArea::UpdateAfterOverflowRecalc() {
  ...
  if ((Box().HasAutoHorizontalScrollbar() && horizontal_scrollbar_should_change) ||
...
int PaintLayerScrollableArea::VerticalScrollbarStart(int min_x, int max_x) const {
  if (Box().ShouldPlaceBlockDirectionScrollbarOnLogicalLeft())
  ...
  if (Node* node = layout_object.GetNode()) {
    ...
    if (ShadowRoot* shadow_root = node->ContainingShadowRoot()) {
      if (shadow_root->GetType() == ShadowRootType::kUserAgent) {
        if (LayoutObject* host_layout_object = shadow_root->host().GetLayoutObject())
...
bool PaintLayerScrollableArea::HitTestOverflowControls(
  ...
  if (Box().Style()->Resize() != RESIZE_NONE) {
    resize_control_rect =
        ResizerCornerRect(Box().PixelSnappedBorderBoxRect(), kResizerForPointer);
  ...
  if (HasVerticalScrollbar() &&
      VerticalScrollbar()->ShouldParticipateInHitTesting()) {
    LayoutRect v_bar_rect(VerticalScrollbarStart(0, Box().size().Width().ToInt()),
                        Box().BorderTop().ToInt(),
                        VerticalScrollbar()->ScrollbarThickness(),
...
void PaintLayerScrollableArea::UpdateResizerStyle() {
  ...
  if (resizer) {
    if (!resizer_) {
      resizer_ = LayoutScrollbarPart::CreateAnonymous(&Box().GetDocument(), this);
...
void PaintLayerScrollableArea::InvalidateAllStickyConstraints() {
  if (PaintLayerScrollableAreaRareData* d = RareData()) {
    for (PaintLayer* sticky_layer : d->sticky_constraints_map_.Keys()) {
      if (sticky_layer->GetLayoutObject().Style()->GetPosition() == EPosition::kSticky)
...
void PaintLayerScrollableArea::Resize(const IntPoint& pos,
  ...
  LayoutSize adjusted_old_offset = LayoutSize(old_offset.Width() / zoom_factor,
                                            old_offset.Height() / zoom_factor);
  if (Box().ShouldPlaceBlockDirectionScrollbarOnLogicalLeft()) {
  ...
  LayoutSize difference(
      (current_size + new_offset - adjusted_old_offset).ExpandedTo(minimum_size) -
      current_size);

  bool is_box_sizing_border = Box().Style()->BoxSizing() == EBoxSizing::kBorderBox;
...
LayoutRect PaintLayerScrollableArea::ScrollIntoView(
  ...
  LayoutRect layer_bounds(LayoutPoint(),
                         LayoutSize(Box().ClientWidth(), Box().ClientHeight()));
  LayoutRect r = ScrollAlignment::GetRectToExpose(layer_bounds, local_expose_rect,
                                                  align_x, align_y);
...
bool PaintLayerScrollableArea::ComputeNeedsCompositedScrolling(
  ...
  if (layer->GetLayoutObject().Style()->HasBorderRadius()) {
    main_thread_scrolling_reasons |= MainThreadScrollingReason::kHasBorderRadius;
    needs_composited_scrolling = false;
  }
...
PaintLayerScrollableArea::GetCompositorAnimationTimeline() const {
  return layer_.GetLayoutObject().GetFrameView()->GetCompositorAnimationTimeline();
}
