...
void PaintLayerScrollableArea::InvalidatePaintForScrollOffsetChange(
  ...
  if (RuntimeEnabledFeatures::SlimmingPaintV2Enabled()) {
    ...
    // Gaining or losing scroll offset can change whether a scroll translation
    // paint property is needed (see: NeedsScrollOrScrollTranslation in
    // PaintPropertyTreeBuilder.cpp).
    if (offset_was_zero || GetScrollOffset().IsZero()) {
      Box()
          .SetShouldDoFullPaintInvalidationIncludingNonCompositingDescendants();
      return;
    }
