...
struct CORE_EXPORT PaintInfo {
 ...
 public:
  PaintInfo(GraphicsContext& context,
            ...
            const FragmentData* layer_fragment_data = nullptr)
      : context(context),
        phase(phase),
        cull_rect_(cull_rect),
        paint_container_(paint_container),
        layer_fragment_data_(layer_fragment_data),
  ...
  PaintInfo(GraphicsContext& new_context,
      ...
      : context(new_context),
        ...
        layer_fragment_data_(copy_other_fields_from.layer_fragment_data_),
  ...
  // layer fragment.
  const FragmentData* MatchFragment(const FragmentData& first_fragment) const {
    if (!layer_fragment_data_)
      return &first_fragment;
    for (const auto* fragment = &first_fragment; fragment;
         fragment = fragment->NextFragment()) {
      if (fragment->PaginationOffset() ==
          layer_fragment_data_->PaginationOffset())
        return fragment;
    }
 ...
 private:
  ...
  // The current FragmentData of the self-painting PaintLayer which initiated
  // the current painting.
  const FragmentData* layer_fragment_data_;
