...
TEST_P(PaintLayerPainterTest, CachedSubsequence) {
  ...
  if (RuntimeEnabledFeatures::SlimmingPaintV175Enabled()) {
    ...
    // With RLS, the client of the document background paint chunk is the
    // scrolling contents layer.
    const DisplayItemClient* background_chunk_client = root_layer;
    if (RuntimeEnabledFeatures::RootLayerScrollingEnabled())
      background_chunk_client = root_layer->GraphicsLayerBacking();
