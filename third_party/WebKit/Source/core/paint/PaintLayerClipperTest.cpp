...
TEST_P(PaintLayerClipperTest, LayoutSVGRoot) {
  ...
  ClipRectsContext context(GetDocument().GetLayoutView()->Layer(), kUncachedClipRects,
...
TEST_P(PaintLayerClipperTest, ContainPaintClip) {
  ...
  layer->Clipper(option).CalculateRects(context_clip, infinite_rect, layer_bounds,
                                        background_rect, foreground_rect);
...
TEST_P(PaintLayerClipperTest, ClearClipRectsRecursiveOneType) {
  ...
  if (RuntimeEnabledFeatures::slimmingPaintInvalidationEnabled())
    option = PaintLayer::kUseGeometryMapper;
  parent->Clipper(option).ClearClipRectsIncludingDescendants(kAbsoluteClipRects);
