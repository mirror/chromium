...
LayoutSize CalculateFillTileSize(const LayoutBoxModelObject& obj,
  ...
  LayoutSize image_intrinsic_size =
      image->ImageSize(obj, obj.Style()->EffectiveZoom(), positioning_area_size);
  switch (type) {
    case kSizeLength: {
      ...
      float horizontal_scale_factor =
          image_intrinsic_size.Width()
              ? positioning_area_size.Width().ToFloat() /
                    image_intrinsic_size.Width()
              : 1.0f;
      float vertical_scale_factor = image_intrinsic_size.Height()
  ...
  if (unsnapped_tile_width) {
    ...
    if (fill_layer.BackgroundXOrigin() == kRightEdge) {
      number_of_tiles_in_position =
          (snapped_available_width - computed_x_position + extra_offset).ToFloat() /
          unsnapped_tile_width.ToFloat();
    } else {
      number_of_tiles_in_position = (computed_x_position + extra_offset).ToFloat() /
                                unsnapped_tile_width.ToFloat();
    }
    float fractional_position_within_tile =
        1.0f - (number_of_tiles_in_position - truncf(number_of_tiles_in_position));
    SetPhaseX(
        LayoutUnit(roundf(fractional_position_within_tile * TileSize().Width())));
  } else {
...
void BackgroundImageGeometry::SetRepeatY(const FillLayer& fill_layer,
  ...
  if (unsnapped_tile_height) {
    ...
    if (fill_layer.BackgroundYOrigin() == kBottomEdge) {
      number_of_tiles_in_position =
          (snapped_available_height - computed_y_position + extra_offset).ToFloat() /
          unsnapped_tile_height.ToFloat();
    } else {
      number_of_tiles_in_position = (computed_y_position + extra_offset).ToFloat() /
                                unsnapped_tile_height.ToFloat();
    }
    float fractional_position_within_tile =
        1.0f - (number_of_tiles_in_position - truncf(number_of_tiles_in_position));
    SetPhaseY(
        LayoutUnit(roundf(fractional_position_within_tile * TileSize().Height())));
  } else {
...
void BackgroundImageGeometry::SetSpaceX(LayoutUnit space,
  ...
  SetPhaseX(actual_width
                ? LayoutUnit(roundf(
                      actual_width -
                      fmodf((computed_x_position + extra_offset), actual_width)))
                : LayoutUnit());
}
...
void BackgroundImageGeometry::SetSpaceY(LayoutUnit space,
  ...
  SetPhaseY(actual_height
                ? LayoutUnit(roundf(
                      actual_height -
                      fmodf((computed_y_position + extra_offset), actual_height)))
                : LayoutUnit());
}
...
LayoutPoint BackgroundImageGeometry::GetOffsetForCell(
  ...
  return offset_in_background - LayoutSize(LayoutUnit(), border_spacing.Height());
}
...
void BackgroundImageGeometry::Calculate(
  ...
  bool cell_using_container_background =
      obj.IsTableCell() && background_object && !background_object->IsTableCell();
  ...
  LayoutSize fill_tile_size(
      CalculateFillTileSize(positioning_box, fill_layer, positioning_area.size()));
  ...
  SetTileSize(ApplySubPixelHeuristicToImageSize(fill_tile_size, positioning_area));
  ...
  LayoutUnit available_width = positioning_area_size.Width() - TileSize().Width();
  LayoutUnit available_height =
  ...
  if (background_repeat_x == kRoundFill &&
    ...
    SetPhaseX(TileSize().Width()
                  ? LayoutUnit(roundf(
                        TileSize().Width() -
                        fmodf((computed_x_position + left), TileSize().Width())))
                  : LayoutUnit());
  ...
  if (background_repeat_y == kRoundFill &&
    ...
    int nr_tiles = std::max(
        1, RoundToInt(positioning_area_size.Height() / fill_tile_size.Height()));
    ...
    SetPhaseY(TileSize().Height()
                  ? LayoutUnit(roundf(
                        TileSize().Height() -
                        fmodf((computed_y_position + top), TileSize().Height())))
                  : LayoutUnit());
    SetSpaceSize(LayoutSize());
  }
