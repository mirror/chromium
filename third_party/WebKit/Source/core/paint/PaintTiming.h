...
class CORE_EXPORT PaintTiming final
 ...
 public:
  ...
  // FirstImagePaintSwap returns the first time that image content was painted.
  ...
  void RegisterNotifySwapTime(PaintEvent, std::unique_ptr<ReportTimeCallback>);
 ...
 private:
  ...
  // These will be true if the SwapPromise for the corresponding PaintEvent was
  // broken and we need to queue another SwapPromise the next time we're
  // notified about a Paint.
  bool requeue_first_paint_swap_ : 1;
  bool requeue_first_text_paint_swap_ : 1;
  bool requeue_first_image_paint_swap_ : 1;
  bool requeue_first_contentful_paint_swap_ : 1;
