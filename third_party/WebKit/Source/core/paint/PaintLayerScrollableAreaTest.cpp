...
class PaintMockChromeClient : public EmptyChromeClient {
 public:
...
class PaintLayerScrollableAreaTest : public RenderingTest {
 public:
  PaintLayerScrollableAreaTest()
      : RenderingTest(EmptyLocalFrameClient::Create()),
        chrome_client_(new PaintMockChromeClient) {}

  ~PaintLayerScrollableAreaTest() {
    ::testing::Mock::VerifyAndClearExpectations(&GetChromeClient());
  }

  PaintMockChromeClient& GetChromeClient() const override {
 ...
 private:
  ...
  Persistent<PaintMockChromeClient> chrome_client_;
};
