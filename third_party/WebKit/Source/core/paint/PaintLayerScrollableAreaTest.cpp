...
TEST_F(PaintLayerScrollableAreaTest,
  ...
  // border-box edges.
  EXPECT_EQ(kBackgroundPaintInGraphicsLayer | kBackgroundPaintInScrollingContents,
...
TEST_F(PaintLayerScrollableAreaTest, OverlayScrollbarColorThemeUpdated) {
  ...
  PaintLayer* none_layer = ToLayoutBoxModelObject(none->GetLayoutObject())->Layer();
...
TEST_F(PaintLayerScrollableAreaTest, HideTooltipWhenScrollPositionChanges) {
  ...
  EXPECT_CALL(GetChromeClient(), MockSetToolTip(GetDocument().GetFrame(), String(), _))
      .Times(1);
