...
void SVGInlineTextBoxPainter::PaintTextFragments(
  ...
  if (should_paint_selection) {
    selection_style = parent_layout_object.GetCachedPseudoStyle(kPseudoIdSelection);
    if (selection_style) {
  ...
  for (unsigned i = 0; i < text_fragments_size; ++i) {
    const SVGTextFragment& fragment = svg_inline_text_box_.TextFragments().at(i);
...
void SVGInlineTextBoxPainter::PaintSelectionBackground(
  ...
  for (const SVGTextFragmentWithRange& fragment_with_range : fragment_info_list) {
    ...
    paint_info.context.FillRect(svg_inline_text_box_.SelectionRectForTextFragment(
...
void SVGInlineTextBoxPainter::PaintDecoration(const PaintInfo& paint_info,
  ...
  Font scaled_font;
  LayoutSVGInlineText::ComputeNewScaledFontForStyle(*decoration_layout_object,
                                                    scaling_factor, scaled_font);
  ...
  Path path;
  path.AddRect(FloatRect(decoration_origin,
...
bool SVGInlineTextBoxPainter::SetupTextPaint(const PaintInfo& paint_info,
                                             const ComputedStyle& style,
  ...
  if (resource_mode == kApplyToStrokeMode) {
    ...
    float stroke_scale_factor =
        style.SvgStyle().VectorEffect() != VE_NON_SCALING_STROKE ? scaling_factor
                                                                 : 1;
    StrokeData stroke_data;
    SVGLayoutSupport::ApplyStrokeStyleToStrokeData(
...
void SVGInlineTextBoxPainter::PaintText(const PaintInfo& paint_info,
  ...
  text_run_paint_info.bounds = FloatRect(text_origin.X(), text_origin.Y() - baseline,
...
SVGInlineTextBoxPainter::CollectFragmentsInRange(int start_position,
  ...
  const Vector<SVGTextFragment>& fragments = svg_inline_text_box_.TextFragments();
