  ...
  if (update_type == kForceUpdate) {
    ...
    if (!layer->IsRootLayer()) {
      if (!RuntimeEnabledFeatures::SlimmingPaintV2Enabled()) {
        ...
        // now, but only in RLS mode, because in non-RLS mode the scroll offset
        // is not included regardless. Scroll offset is excluded so that
        // we do not need to invalidate the clip rect cache on scroll.
        if (RuntimeEnabledFeatures::RootLayerScrollingEnabled() &&
