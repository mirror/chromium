...
void SVGPaintContext::ApplyClipIfNecessary() {
  ...
  if (!clip_path_operation)
    return;
  if (!RuntimeEnabledFeatures::slimmingPaintV2Enabled()) {
    clip_path_clipper_.emplace(GetPaintInfo().context, *clip_path_operation, object_,
                              object_.ObjectBoundingBox(), FloatPoint());
  }
}
...
bool SVGPaintContext::PaintForLayoutObject(
  ...
  SVGPaintServer paint_server =
      SVGPaintServer::RequestForLayoutObject(layout_object, style, resource_mode);
