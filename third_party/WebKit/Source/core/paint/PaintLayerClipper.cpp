...
static void AdjustClipRectsForChildren(const LayoutBoxModelObject& layout_object,
...
#if DCHECK_IS_ON()
  entry.overlay_scrollbar_clip_behavior = context.overlay_scrollbar_clip_behavior;
#endif
...
void PaintLayerClipper::ClearClipRectsIncludingDescendants(
  ...
  while (!layers.empty()) {
    ...
    layers.pop();
    PaintLayerClipper(*current_layer, use_geometry_mapper_).ClearCache(cache_slot);
...
LayoutRect PaintLayerClipper::LocalClipRect(
  ...
  if (use_geometry_mapper_) {
    ...
    const auto* clip_root_layer_transform = clipping_root_layer.GetLayoutObject()
...
void PaintLayerClipper::InitializeCommonClipRectState(
  ...
  source_property_tree_state = *layer_.GetLayoutObject().LocalBorderBoxProperties();
...
void PaintLayerClipper::CalculateForegroundClipRectWithGeometryMapper(
  ...
  if (&layer_ == context.root_layer) {
    ...
    if (context.respect_overflow_clip == kIgnoreOverflowClip &&
      ...
      destination_property_tree_state.SetClip(ancestor_properties->OverflowClip());
  ...
  const FloatClipRect& clipped_rect_in_root_layer_space =
      GeometryMapper::SourceToDestinationClipRect(source_property_tree_state,
                                                  destination_property_tree_state);
  output.SetRect(clipped_rect_in_root_layer_space);
