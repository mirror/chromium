...
void PaintLayerClipper::CalculateBackgroundClipRectWithGeometryMapper(
  ...
  // The root must always not be fragmented.
  output.MoveBy(
      -context.root_layer->GetLayoutObject().FirstFragment().PaintOffset());
  DCHECK(!context.root_layer->GetLayoutObject().FirstFragment().NextFragment());
...
void PaintLayerClipper::InitializeCommonClipRectState(
  ...
  auto* ancestor_properties = context.root_layer->GetLayoutObject()
                                  .FirstFragment()
                                  .GetRarePaintData()
                                  ->PaintProperties();
