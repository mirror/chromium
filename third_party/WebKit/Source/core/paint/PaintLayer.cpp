...
PaintLayer::PaintLayer(LayoutBoxModelObject& layout_object)
    : has_self_painting_layer_descendant_(false),
      ...
      backdrop_filter_changed_(false),
...
void PaintLayer::UpdateFilters(const ComputedStyle* old_style,
  ...
  if (!filter_changed_) {
    if (old_style) {
      if (!old_style->FilterDataEquivalent(new_style) ||
          !old_style->ReflectionDataEquivalent(new_style))
        filter_changed_ = true;
    } else if (new_style.HasFilterInducingProperty()) {
      filter_changed_ = true;
    }
  }

  if (!backdrop_filter_changed_) {
    if (old_style) {
      if (!old_style->BackdropFilterDataEquivalent(new_style))
        backdrop_filter_changed_ = true;
    } else if (new_style.HasBackdropFilter()) {
      backdrop_filter_changed_ = true;
    }
  }
...
void PaintLayer::UpdateCompositorFilterOperationsForFilter(
  ...
  if (!operations.IsEmpty() && !filter_changed_ &&
