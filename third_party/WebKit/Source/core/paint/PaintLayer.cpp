...
bool IsReferenceClipPath(const ClipPathOperation* clip_operation) {
  return clip_operation && clip_operation->GetType() == ClipPathOperation::REFERENCE;
}
...
bool PaintLayer::FixedToViewport() const {
  ...
  return GetLayoutObject().ContainerForFixedPosition() == GetLayoutObject().View();
}
...
void PaintLayer::UpdateTransform(const ComputedStyle* old_style,
  ...
  bool has_transform =
      GetLayoutObject().HasTransformRelatedProperty() && new_style.HasTransform();
...
static PaintLayer* EnclosingLayerForContainingBlock(PaintLayer* layer) {
  if (LayoutObject* containing_block = layer->GetLayoutObject().ContainingBlock())
...
void PaintLayer::MapPointInPaintInvalidationContainerToBacking(
  ...
  LayoutBoxModelObject& transformed_ancestor =
      paint_invalidation_layer->EnclosingTransformedAncestor()->GetLayoutObject();
  ...
  point = paint_invalidation_container.LocalToAncestorPoint(point,
...
void PaintLayer::MapRectInPaintInvalidationContainerToBacking(
  ...
  // that into account when mapping into the space of the transformed ancestor.
  rect =
      LayoutRect(paint_invalidation_container
...
void PaintLayer::UpdateLayerPosition() {
  ...
  if (PaintLayer* containing_layer = this->ContainingLayer()) {
    if (containing_layer->GetLayoutObject().HasOverflowClip()) {
      ...
      IntSize offset = containing_layer->GetLayoutBox()->ScrolledContentOffset();
      local_point -= offset;
...
PaintLayer* PaintLayer::EnclosingLayerWithCompositedLayerMapping(
  ...
  if ((include_self == kIncludeSelf) && GetCompositingState() != kNotComposited &&
      GetCompositingState() != kPaintsIntoGroupedBacking)
...
bool PaintLayer::HasNonIsolatedDescendantWithBlendMode() const {
  ...
  if (GetLayoutObject().IsSVGRoot())
    return ToLayoutSVGRoot(GetLayoutObject()).HasNonIsolatedBlendingDescendants();
  return false;
}
...
void PaintLayer::SetShouldIsolateCompositedDescendants(
  ...
  should_isolate_composited_descendants_ = should_isolate_composited_descendants;
...
LayoutRect PaintLayer::TransparencyClipBox(
  ...
  ExpandClipRectForDescendants(clip_rect, layer, root_layer, transparency_behavior,
                               sub_pixel_accumulation, global_paint_flags);
...
void PaintLayer::InsertOnlyThisLayerAfterStyleChange() {
  if (!parent_ && GetLayoutObject().Parent()) {
    ...
    PaintLayer* before_child =
        GetLayoutObject().Parent()->FindNextLayer(parent_layer, &GetLayoutObject());
...
void PaintLayer::CollectFragments(
  ...
  if (!EnclosingPaginationLayer() ||
    ...
    AppendSingleFragmentIgnoringPagination(
        ...
        geometry_mapper_option, overlay_scrollbar_clip_behavior, respect_overflow_clip,
  ...
  ConvertToLayerCoords(EnclosingPaginationLayer(), offset_within_paginated_layer);
  ...
  ClipRectsContext pagination_clip_rects_context(EnclosingPaginationLayer(),
  ...
  Clipper(geometry_mapper_option)
      .CalculateRects(pagination_clip_rects_context,
  ...
  layer_bounding_box_in_flow_thread.Intersect(background_rect_in_flow_thread.Rect());
  ...
  dirty_rect_in_multicol_container.Move(enclosing_flow_thread.PhysicalLocation() -
                                    offset_of_pagination_layer_from_root);
  ...
  if (const PaintLayer* pagination_parent_layer =
    ...
    const PaintLayer* ancestor_layer =
        root_layer_is_inside_pagination_layer ? pagination_parent_layer : root_layer;
  ...
  for (; !iterator.AtEnd(); iterator.Advance()) {
    ...
    // flow thread.
    fragment.SetRects(layer_bounds_in_flow_thread, background_rect_in_flow_thread,
...
static double ComputeZOffset(const HitTestingTransformState& transform_state) {
  ...
  FloatPoint3D backmapped_point =
      transform_state.accumulated_transform_.MapPoint(FloatPoint3D(target_point));
...
PassRefPtr<HitTestingTransformState> PaintLayer::CreateLocalTransformState(
  ...
  if (container_transform_state) {
    ...
    transform_state = HitTestingTransformState::Create(*container_transform_state);
    ...
    transform_state = HitTestingTransformState::Create(
        hit_test_location.TransformedPoint(), hit_test_location.TransformedRect(),
  ...
  if (GetLayoutObject().ShouldUseTransformFromContainer(container_layout_object)) {
...
PaintLayer* PaintLayer::HitTestLayer(
  ...
  if (applied_transform) {
    ...
    // reference it.
    DCHECK(transform_state);
    local_transform_state = const_cast<HitTestingTransformState*>(transform_state);
  } else if (transform_state || has3d_transformed_descendant_ || Preserves3D()) {
  ...
  PaintLayer* hit_layer = HitTestChildren(
      kPositiveZOrderChildren, root_layer, result, hit_test_rect, hit_test_location,
  ...
  if (applied_transform)
    ...
    CollectFragments(layer_fragments, root_layer, hit_test_rect, clip_rects_cache_slot,
                     PaintLayer::kDoNotUseGeometryMapper,
                     kExcludeOverlayScrollbarSizeForHitTesting);

  if (scrollable_area_ &&
      scrollable_area_->HitTestResizerInFragments(layer_fragments,
                                                  hit_test_location)) {
  ...
  if (IsSelfPaintingLayer()) {
    ...
    HitTestResult temp_result(result.GetHitTestRequest(), result.GetHitTestLocation());
    bool inside_fragment_foreground_rect = false;
    if (HitTestContentsForFragments(layer_fragments, temp_result, hit_test_location,
                                    kHitTestDescendants,
  ...
  hit_layer = HitTestChildren(
      kNegativeZOrderChildren, root_layer, result, hit_test_rect, hit_test_location,
      local_transform_state.Get(), z_offset_for_descendants_ptr, z_offset,
      unflattened_transform_state.Get(), depth_sort_descendants);
  if (hit_layer) {
    if (!depth_sort_descendants)
  ...
  if (IsSelfPaintingLayer()) {
    ...
    if (inside_fragment_background_rect && result.GetHitTestRequest().ListBased())
      result.Append(temp_result);
  }
...
PaintLayer* PaintLayer::HitTestTransformedLayerInFragments(
  ...
  EnclosingPaginationLayer()->CollectFragments(
      enclosing_pagination_fragments, root_layer, hit_test_rect, clip_rects_cache_slot,
      PaintLayer::kDoNotUseGeometryMapper,
  ...
  for (int i = enclosing_pagination_fragments.size() - 1; i >= 0; --i) {
    ...
    if (Parent() != EnclosingPaginationLayer()) {
      ...
      Clipper(PaintLayer::kDoNotUseGeometryMapper)
          .CalculateBackgroundClipRect(
              ClipRectsContext(EnclosingPaginationLayer(), clip_rects_cache_slot,
                               kExcludeOverlayScrollbarSizeForHitTesting),
              parent_clip_rect);
...
PaintLayer* PaintLayer::HitTestChildren(
  ...
  while (PaintLayerStackingNode* child = iterator.Next()) {
    ...
    HitTestResult temp_result(result.GetHitTestRequest(), result.GetHitTestLocation());
    hit_layer = child_layer->HitTestLayer(root_layer, this, temp_result,
                                        hit_test_rect, hit_test_location, false,
...
bool PaintLayer::HitTestClippedOutByClipPath(
  ...
  if (EnclosingPaginationLayer())
    ConvertFromFlowThreadToVisualBoundingBoxInAncestor(root_layer, reference_box);
  else
    ConvertToLayerCoords(root_layer, reference_box);

  FloatPoint point(hit_test_location.Point());

  ClipPathOperation* clip_path_operation = GetLayoutObject().Style()->ClipPath();
...
bool PaintLayer::IntersectsDamageRect(const LayoutRect& layer_bounds,
...
LayoutRect PaintLayer::LogicalBoundingBox() const {
  ...
  if (IsRootLayer()) {
    rect.Unite(
        LayoutRect(rect.Location(), GetLayoutObject().View()->ViewRect().size()));
  }

  return rect;
}

static inline LayoutRect FlippedLogicalBoundingBox(LayoutRect bounding_box,
...
BackgroundPaintLocation PaintLayer::GetBackgroundPaintLocation(
  ...
  if (!ScrollsOverflow()) {
    ...
    location = IsRootLayer() ? kBackgroundPaintInGraphicsLayer
...
void PaintLayer::ClearCompositedLayerMapping(bool layer_being_destroyed) {
  if (!layer_being_destroyed) {
    ...
    if (PaintLayer* compositing_parent =
      ...
      compositing_parent->GetCompositedLayerMapping()->SetNeedsGraphicsLayerUpdate(
          kGraphicsLayerUpdateSubtree);
  }
...
void PaintLayer::SetGroupedMapping(CompositedLayerMapping* grouped_mapping,
  ...
  if (options == kInvalidateLayerAndRemoveFromMapping && old_grouped_mapping) {
    old_grouped_mapping->SetNeedsGraphicsLayerUpdate(kGraphicsLayerUpdateSubtree);
...
#if DCHECK_IS_ON()
  DCHECK(!grouped_mapping || grouped_mapping->VerifyLayerInSquashingVector(this));
#endif
...
bool PaintLayer::PaintsWithTransform(GlobalPaintFlags global_paint_flags) const {
  if (RuntimeEnabledFeatures::slimmingPaintInvalidationEnabled()) {
    return Transform() &&
...
bool PaintLayer::AttemptDirectCompositingUpdate(StyleDifference diff,
  ...
  if (!diff.HasAtMostPropertySpecificDifferences(
          StyleDifference::kTransformChanged | StyleDifference::kOpacityChanged))
    return false;
  ...
  rare_data_->composited_layer_mapping->SetNeedsGraphicsLayerUpdate(
      kGraphicsLayerUpdateLocal);
  Compositor()->SetNeedsCompositingUpdate(kCompositingUpdateAfterGeometryChange);
...
void showLayerTree(const blink::PaintLayer* layer) {
  ...
  if (blink::LocalFrame* frame = layer->GetLayoutObject().GetFrame()) {
    WTF::String output = ExternalRepresentation(
        frame,
        blink::kLayoutAsTextShowAllLayers | blink::kLayoutAsTextShowLayerNesting |
