...
PaintLayer* PaintLayer::HitTestLayer(
  ...
  // Check current overflow first.
  if (scrollable_area_ && scrollable_area_->HasScrollbar()) {
    IntPoint local_point =
        RoundedIntPoint(hit_test_location.Point() - ToLayoutSize(Location()));

    Scrollbar* scrollbar = scrollable_area_->ScrollbarAtFramePoint(local_point);

    if (scrollbar && scrollbar->IsOverlayScrollbar()) {
      LocalFrameView* frame_view = GetLayoutObject().GetFrameView();
      DCHECK(frame_view);
      IntPoint root_frame_point = frame_view->ContentsToFrame(
          RoundedIntPoint(hit_test_location.Point()));
      root_frame_point = frame_view->ConvertToRootFrame(root_frame_point);

      ScrollbarPart part =
          scrollbar->GetTheme().HitTest(*scrollbar, root_frame_point);
      if (part == kNoPart)
        scrollbar = nullptr;
    }
