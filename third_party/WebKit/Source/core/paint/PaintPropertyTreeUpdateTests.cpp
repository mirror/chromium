...
TEST_P(PaintPropertyTreeUpdateTest, DescendantNeedsUpdateAcrossFrames) {
  ...
  EXPECT_FALSE(GetDocument().GetLayoutView()->DescendantNeedsPaintPropertyUpdate());
  ...
  EXPECT_TRUE(GetDocument().GetLayoutView()->DescendantNeedsPaintPropertyUpdate());
...
TEST_P(PaintPropertyTreeUpdateTest, BuildingStopsAtThrottledFrames) {
  ...
  auto* transform = GetDocument().GetElementById("transform")->GetLayoutObject();
...
TEST_P(PaintPropertyTreeUpdateTest, ClipChangesUpdateOverflowClip) {
  ...
  GetDocument().View()->UpdateAllLifecyclePhases();
  auto* clip_properties = div->GetLayoutObject()->PaintProperties()->OverflowClip();
...
TEST_P(PaintPropertyTreeUpdateTest,
  ...
  const ObjectPaintProperties* properties =
      target->GetLayoutObject()->PaintProperties();
  EXPECT_NE(CompositorElementId(), properties->Effect()->GetCompositorElementId());

  target->removeAttribute(HTMLNames::classAttr);
  GetDocument().View()->UpdateAllLifecyclePhases();
  EXPECT_EQ(CompositorElementId(), properties->Effect()->GetCompositorElementId());
}
...
TEST_P(PaintPropertyTreeUpdateTest, PerspectiveOriginUpdatesOnSizeChanges) {
  ...
  auto* perspective = GetDocument().GetElementById("perspective")->GetLayoutObject();
