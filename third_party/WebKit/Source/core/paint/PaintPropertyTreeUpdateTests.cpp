...
TEST_P(PaintPropertyTreeUpdateTest,
  ...
  const ObjectPaintProperties* properties = target->GetLayoutObject()
                                                ->FirstFragment()
                                                .GetRarePaintData()
                                                ->PaintProperties();
  ...
  EXPECT_EQ(nullptr, target->GetLayoutObject()
                         ->FirstFragment()
                         .GetRarePaintData()
                         ->PaintProperties());
}
...
TEST_P(PaintPropertyTreeUpdateTest, PerspectiveOriginUpdatesOnSizeChanges) {
  ...
  EXPECT_EQ(TransformationMatrix().ApplyPerspective(100),
            perspective->FirstFragment()
                .GetRarePaintData()
                ->PaintProperties()
                ->Perspective()
                ->Matrix());
  EXPECT_EQ(FloatPoint3D(50, 0, 0), perspective->FirstFragment()
                                        .GetRarePaintData()
                                        ->PaintProperties()
                                        ->Perspective()
                                        ->Origin());
  ...
  EXPECT_EQ(TransformationMatrix().ApplyPerspective(100),
            perspective->FirstFragment()
                ...
                ->Perspective()
                ->Matrix());
  EXPECT_EQ(FloatPoint3D(50, 100, 0), perspective->FirstFragment()
                                          .GetRarePaintData()
                                          ->PaintProperties()
                                          ->Perspective()
                                          ->Origin());
...
TEST_P(PaintPropertyTreeUpdateTest, ScrollbarWidthChange) {
  ...
  auto* container = GetLayoutObjectByElementId("container");
  auto* overflow_clip = container->FirstFragment()
                            .GetRarePaintData()
                            ->PaintProperties()
                            ->OverflowClip();
  ...
  GetDocument().View()->UpdateAllLifecyclePhases();
  EXPECT_EQ(overflow_clip, container->FirstFragment()
                               .GetRarePaintData()
                               ->PaintProperties()
                               ->OverflowClip());
...
TEST_P(PaintPropertyTreeUpdateTest, Preserve3DChange) {
  ...
  auto* transform =
      child->FirstFragment().GetRarePaintData()->PaintProperties()->Transform();
  ...
  GetDocument().View()->UpdateAllLifecyclePhases();
  EXPECT_EQ(transform, child->FirstFragment()
                           .GetRarePaintData()
                           ->PaintProperties()
                           ->Transform());
...
TEST_P(PaintPropertyTreeUpdateTest, MenuListControlClipChange) {
  ...
  EXPECT_NE(nullptr, select->FirstFragment()
                         .GetRarePaintData()
                         ->PaintProperties()
                         ->OverflowClip());
