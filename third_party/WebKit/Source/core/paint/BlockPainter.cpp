...
void BlockPainter::PaintScrollHitTest(const PaintInfo& paint_info) {
  ...
  if (!RuntimeEnabledFeatures::RootLayerScrollingEnabled() &&
    ...
    if (contents_state->Transform()->IsScrollTranslation()) {
      RecordScrollHitTest(paint_info.context, layout_block_,
                          DisplayItem::kScrollHitTest);
    }
    // The LayoutView should not create a scroll translation, instead relying on
    // the LocalFrameView's scroll translation.
    const auto* properties = layout_block_.PaintProperties();
    DCHECK(!properties || !properties->ScrollTranslation());
    return;
  }
  ...
  if (properties && properties->ScrollTranslation()) {
    ScopedPaintChunkProperties scroll_properties(
        ...
        layout_block_.ContentsProperties());
    RecordScrollHitTest(paint_info.context, layout_block_,
                        DisplayItem::kScrollHitTest);
  }
}
...
void BlockPainter::PaintObject(const PaintInfo& paint_info,
  ...
  if (ShouldPaintSelfBlockBackground(paint_phase)) {
    ...
    // Record the scroll hit test after the background but before the contents
    // so background squashing is not affected.
    if (RuntimeEnabledFeatures::SlimmingPaintV2Enabled())
      PaintScrollHitTest(paint_info);
