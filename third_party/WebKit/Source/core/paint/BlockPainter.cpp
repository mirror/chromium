...
bool BlockPainter::ShouldAdjustForPaintOffsetTranslation(
  ...
  auto* paint_properties =
      layout_block_.FirstFragment().GetRarePaintData()->PaintProperties();
  if (!paint_properties)
    return false;
...
void BlockPainter::Paint(const PaintInfo& paint_info,
  ...
  if (ShouldAdjustForPaintOffsetTranslation(local_paint_info,
    ...
    auto* paint_properties =
        layout_block_.FirstFragment().GetRarePaintData()->PaintProperties();
    const auto* local_border_box_properties = layout_block_.FirstFragment()
                                                  .GetRarePaintData()
    ...
    // TODO(chrishtr): this is wrong for fragmentation.
    adjusted_paint_offset = layout_block_.FirstFragment().PaintOffset();
...
void BlockPainter::PaintScrollHitTestDisplayItem(const PaintInfo& paint_info) {
  ...
  const auto* properties = fragment.GetRarePaintData()
                               ? fragment.GetRarePaintData()->PaintProperties()
                               : nullptr;
...
void BlockPainter::PaintObject(const PaintInfo& paint_info,
  ...
  if (paint_phase != kPaintPhaseSelfOutlineOnly) {
    ...
    auto& fragment = layout_block_.FirstFragment();
    if (RuntimeEnabledFeatures::SlimmingPaintV175Enabled()) {
