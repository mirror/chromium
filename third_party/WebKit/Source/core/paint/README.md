...
<<<<<<< Updated upstream * Paint container: the parent of an object for
painting, as defined by [CSS2.1 spec for
painting](\(http://www.w3.org/TR/CSS21/zindex.html\)). For regular objects, this
is the parent in the DOM. For stacked objects, it's
...
<<<<<<< Updated upstream * Paint invalidation container: Since as indicated by
the definitions in [Glossaries](#Other glossaries), the paint invalidation
container for stacked objects can differ from normal objects, we have to track
both

# separately. Here is an example:

*   Paint invalidation container: As indicated by the definitions in
    [Glossaries](#Glossaries), the paint invalidation container for stacked
    objects can differ from normal objects, we have to track both separately.
    Here is an example:
...
There are many conditions affecting * whether we need to generate subsequence
for a PaintLayer; * whether we can use cached subsequence for a PaintLayer. See
