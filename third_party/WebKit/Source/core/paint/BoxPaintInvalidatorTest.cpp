...
TEST_P(BoxPaintInvalidatorTest, SubpixelVisualRectChagne) {
  ScopedSlimmingPaintInvalidationForTest scoped_slimming_paint_invalidation(true);
...
TEST_P(BoxPaintInvalidatorTest, SubpixelVisualRectChangeWithTransform) {
  ...
  EXPECT_EQ(kPaintInvalidationBorderBoxChange, (*raster_invalidations)[0].reason);
  EXPECT_EQ(IntRect(0, 0, 242, 222), (*raster_invalidations)[1].rect);
  EXPECT_EQ(kPaintInvalidationBorderBoxChange, (*raster_invalidations)[1].reason);
  GetDocument().View()->SetTracksPaintInvalidations(false);
