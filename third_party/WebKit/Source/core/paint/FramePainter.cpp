...
void FramePainter::Paint(GraphicsContext& context,
  ...
  IntRect visible_area_without_scrollbars(GetFrameView().Location(),
                                       GetFrameView().VisibleContentRect().size());
  ...
  bool should_paint_scrollbars =
      !GetFrameView().ScrollbarsSuppressed() &&
      (GetFrameView().HorizontalScrollbar() || GetFrameView().VerticalScrollbar());
  ...
  if (should_paint_contents) {
    ...
    TransformRecorder transform_recorder(
        ...
        AffineTransform::Translation(GetFrameView().X() - GetFrameView().ScrollX(),
  ...
  if (should_paint_scrollbars) {
    ...
    ClipRecorder recorder(
        context, *GetFrameView().GetLayoutView(), DisplayItem::kClipFrameScrollbars,
...
void FramePainter::PaintContents(GraphicsContext& context,
  ...
  DCHECK(document->Lifecycle().GetState() >= DocumentLifecycle::kCompositingClean);
  ...
  float device_scale_factor =
      blink::DeviceScaleFactorDeprecated(root_layer->GetLayoutObject().GetFrame());
...
void FramePainter::PaintScrollCorner(GraphicsContext& context,
  ...
  if (GetFrameView().ScrollCorner()) {
    ...
    if (needs_background &&
      ...
      LayoutObjectDrawingRecorder drawing_recorder(
          context, *GetFrameView().GetLayoutView(), DisplayItem::kScrollbarCorner,
          FloatRect(corner_rect));
  ...
  theme->PaintScrollCorner(context, *GetFrameView().GetLayoutView(), corner_rect);
}
