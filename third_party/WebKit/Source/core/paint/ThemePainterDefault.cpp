...
DirectionFlippingScope::DirectionFlippingScope(const LayoutObject& layout_object,
...
IntRect IndeterminateProgressValueRectFor(const LayoutProgress& layout_progress,
  ...
  if (progress < 0.5)
    return IntRect(rect.X() + progress * 2 * movable_width, rect.Y(), value_width,
                   rect.Height());
...
bool ThemePainterDefault::PaintTextField(const LayoutObject& o,
  ...
  Platform::Current()->ThemeEngine()->Paint(
      canvas, WebThemeEngine::kPartTextField, GetWebThemeState(o), WebRect(rect),
      &extra_params);
  return false;
}
...
bool ThemePainterDefault::PaintSliderTrack(const LayoutObject& o,
  ...
  WebThemeEngine::ExtraParams extra_params;
  WebCanvas* canvas = i.context.Canvas();
  extra_params.slider.vertical = o.StyleRef().Appearance() == kSliderVerticalPart;
...
bool ThemePainterDefault::PaintSearchFieldCancelButton(
  ...
  LayoutRect cancel_button_rect(
      cancel_button_object.OffsetFromAncestorContainer(&input_layout_box).Width(),
      input_content_box.Y() +
