...
class SVGInlineTextBoxPainterTest : public PaintControllerPaintTest {
 public:
  const DrawingDisplayItem* GetDrawingForSVGTextById(const char* element_name) {
    ...
    // the painted text.
    LayoutSVGText* target_svg_text = ToLayoutSVGText(
        GetDocument().GetElementById(AtomicString(element_name))->GetLayoutObject());
    LayoutSVGInlineText* target_inline_text =
        target_svg_text->DescendantTextNodes()[0];
    const DisplayItemClient* target_client =
        static_cast<const DisplayItemClient*>(target_inline_text->FirstTextBox());
...
static void AssertTextDrawingEquals(
  ...
  ASSERT_EQ(
      str,
      static_cast<const InlineTextBox*>(&drawing_display_item->Client())->GetText());
}
...
#define EXPECT_RECT_EQ(expected, actual)                                       \
  do {                                                                         \
    ...
    EXPECT_TRUE(ApproximatelyEqual(expected.X(), actual_rect.X(), kPixelDelta)) \
        << "actual: " << actual_rect.X() << ", expected: " << expected.X();     \
    EXPECT_TRUE(ApproximatelyEqual(expected.Y(), actual_rect.Y(), kPixelDelta)) \
        << "actual: " << actual_rect.Y() << ", expected: " << expected.Y();     \
    EXPECT_TRUE(                                                               \
        ApproximatelyEqual(expected.Width(), actual_rect.Width(), kPixelDelta)) \
