...
void FileUploadControlPainter::PaintObject(const PaintInfo& paint_info,
  ...
  if (layout_file_upload_control_.Style()->Visibility() != EVisibility::kVisible)
    return;
  ...
  if (paint_info.phase == kPaintPhaseForeground ||
    ...
    IntRect clip_rect = EnclosingIntRect(LayoutRect(
        ...
        layout_file_upload_control_.size() +
            LayoutSize(LayoutUnit(), -layout_file_upload_control_.BorderWidth() +
                                         kButtonShadowHeight)));
  ...
  if (paint_info.phase == kPaintPhaseForeground &&
    ...
    const String& displayed_filename = layout_file_upload_control_.FileTextValue();
    ...
    if (LayoutButton* button_layout_object =
      ...
      text_y = paint_offset.Y() + layout_file_upload_control_.BorderTop() +
              ...
              button_layout_object->BaselinePosition(kAlphabeticBaseline, true,
                                                   kHorizontalLine,
                                                   kPositionOnContainingLine);
    else
      text_y = LayoutUnit(layout_file_upload_control_.BaselinePosition(
          kAlphabeticBaseline, true, kHorizontalLine, kPositionOnContainingLine));
    ...
    text_run_paint_info.bounds = FloatRect(
        text_x.ToFloat(), text_y.ToFloat() - font_data->GetFontMetrics().Ascent(),
