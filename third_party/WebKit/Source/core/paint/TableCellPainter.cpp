...
static const CollapsedBorderValue& CollapsedLeftBorder(
  ...
  return style_for_cell_flow.IsFlippedBlocksWritingMode() ? values.AfterBorder()
                                                       : values.BeforeBorder();
}
...
static const CollapsedBorderValue& CollapsedRightBorder(
  ...
  return style_for_cell_flow.IsFlippedBlocksWritingMode() ? values.BeforeBorder()
                                                       : values.AfterBorder();
}
...
void TableCellPainter::PaintCollapsedBorders(
  ...
  LayoutPoint adjusted_paint_offset = paint_offset + layout_table_cell_.Location();
  ...
  const ComputedStyle& style_for_cell_flow = layout_table_cell_.StyleForCellFlow();
...
void TableCellPainter::PaintBackground(const PaintInfo& paint_info,
  ...
  if (bg_layer.HasImage() || c.Alpha()) {
    ...
    BoxPainter(layout_table_cell_)
        .PaintFillLayers(paint_info, c, bg_layer, paint_rect, kBackgroundBleedNone,
