...
// static
TextPainter::Style TextPainter::TextPaintingStyle(LineLayoutItem line_layout_item,
  ...
  if (paint_info.phase == kPaintPhaseTextClip) {
    ...
    text_style.emphasis_mark_color =
        line_layout_item.ResolveColor(style, CSSPropertyWebkitTextEmphasisColor);
    text_style.stroke_width = style.TextStrokeWidth();
    ...
    if (const ComputedStyle* pseudo_style =
      ...
      selection_style.stroke_color =
          uses_text_as_clip ? Color::kBlack
                         : layout_object.ResolveColor(
                               *pseudo_style, CSSPropertyWebkitTextStrokeColor);
      selection_style.stroke_width = pseudo_style->TextStrokeWidth();
      selection_style.shadow = uses_text_as_clip ? 0 : pseudo_style->TextShadow();
    }
...
void TextPainter::PaintInternal(unsigned start_offset,
  ...
  if (start_offset <= end_offset) {
    ...
    if (start_offset < truncation_point)
      PaintInternalRun<Step>(text_run_paint_info, start_offset, truncation_point);
  }
}
...
void TextPainter::PaintEmphasisMarkForCombinedText() {
  ...
  FloatPoint emphasis_mark_text_origin(text_bounds_.X().ToFloat(),
                                    text_bounds_.Y().ToFloat() +
