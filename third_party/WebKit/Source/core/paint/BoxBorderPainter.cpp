...
inline bool BorderStylesRequireMiter(BoxSide side,
  ...
  if (style == kBorderStyleDouble || adjacent_style == kBorderStyleDouble ||
      adjacent_style == kBorderStyleGroove || adjacent_style == kBorderStyleRidge)
    return true;
...
FloatRoundedRect CalculateAdjustedInnerBorder(
  ...
  switch (side) {
    ...
    case kBSLeft:
      overshoot = new_radii.TopLeft().Height() + new_radii.BottomLeft().Height() -
                  new_rect.Height();
      if (overshoot > 0.1) {
    ...
    case kBSRight:
      ...
      new_radii.SetBottomLeft(FloatSize(0, 0));
      max_radii =
          std::max(new_radii.TopRight().Width(), new_radii.BottomRight().Width());
      if (max_radii > new_rect.Width()) {
...
struct BoxBorderPainter::ComplexBorderInfo {
  ComplexBorderInfo(const BoxBorderPainter& border_painter, bool anti_alias)
    ...
    // alpha, style, side.
    std::sort(
        sorted_sides.begin(), sorted_sides.end(),
        [&border_painter](BoxSide a, BoxSide b) -> bool {
          ...
          const unsigned style_priority_a = kStylePriority[edge_a.BorderStyle()];
          const unsigned style_priority_b = kStylePriority[edge_b.BorderStyle()];
...
BoxBorderPainter::BoxBorderPainter(const LayoutRect& border_rect,
  ...
  outer_ = style_.GetRoundedBorderFor(border_rect, include_logical_left_edge,
                                        include_logical_right_edge);
  inner_ = style_.GetRoundedInnerBorderFor(border_rect, include_logical_left_edge,
                                             include_logical_right_edge);
...
void BoxBorderPainter::DrawWideDottedBoxSideFromPath(
  ...
  if (path_length < per_dot_length) {
    ...
    float gap = StrokeData::SelectBestDashGap(path_length, thickness, thickness);
...
void BoxBorderPainter::ClipBorderSidePolygon(GraphicsContext& graphics_context,
  ...
  FloatPoint bound_quad1;  // Point 1 of the rectilinear bounding box of EdgeQuad
  FloatPoint bound_quad2;  // Point 2 of the rectilinear bounding box of EdgeQuad
