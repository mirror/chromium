...
IntRect SVGRootPainter::PixelSnappedSize(const LayoutPoint& paint_offset) const {
...
AffineTransform SVGRootPainter::TransformToPixelSnappedBorderBox(
  ...
  paint_offset_to_border_box.Multiply(layout_svg_root_.LocalToBorderBoxTransform());
...
void SVGRootPainter::PaintReplaced(const PaintInfo& paint_info,
  ...
  SVGTransformContext transform_context(paint_info_before_filtering.context,
                                       layout_svg_root_, transform_to_border_box);
  ...
  PaintTiming& timing =
      PaintTiming::From(layout_svg_root_.GetNode()->GetDocument().TopDocument());
