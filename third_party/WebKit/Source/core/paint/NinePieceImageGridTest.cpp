...
TEST_F(NinePieceImageGridTest, NinePieceImagePainting_NoDrawables) {
  ...
  NinePieceImageGrid grid =
      NinePieceImageGrid(nine_piece, image_size, border_image_area, border_widths);
...
TEST_F(NinePieceImageGridTest, NinePieceImagePainting) {
  const struct {
    ...
    struct {
      ...
      {// Single border and fill
       ...
       true,
       LengthBox(Length(20, kPercent), Length(20, kPercent), Length(20, kPercent),
                 Length(20, kPercent)),
       Image::kStretchTile,
       Image::kStretchTile,
       {
  ...
  for (auto& test_case : test_cases) {
    ...
    nine_piece.SetHorizontalRule((ENinePieceImageRule)test_case.horizontal_rule);
    nine_piece.SetVerticalRule((ENinePieceImageRule)test_case.vertical_rule);

    NinePieceImageGrid grid =
        NinePieceImageGrid(nine_piece, test_case.image_size,
    ...
    for (NinePiece piece = kMinPiece; piece < kMaxPiece; ++piece) {
      ...
      EXPECT_EQ(test_case.pieces[piece].source.Width(), draw_info.source.Width());
