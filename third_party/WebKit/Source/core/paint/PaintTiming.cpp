...
void PaintTiming::ReportSwapTime(PaintEvent event,
                                 bool did_swap,
                                 double timestamp) {
  ReportDidSwapHistorgram(did_swap);
  if (!did_swap)
    return;
  switch (event) {
...
void PaintTiming::ReportDidSwapHistorgram(bool did_swap) {
  DEFINE_STATIC_LOCAL(BooleanHistogram, did_swap_histogram,
                      ("PageLoad.Internal.PaintTiming.DidSwap"));
...
void PaintTiming::ReportSwapTimeDeltaHistogram(double timestamp,
  ...
  DEFINE_STATIC_LOCAL(
      ...
      ("PageLoad.Internal.PaintTiming.SwapTimeDelta", 0, 10000000, 5));
