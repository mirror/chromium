...
void PaintTiming::ReportSwapTime(PaintEvent event,
                                 bool did_swap,
                                 double timestamp) {
  ReportDidSwapHistogram(did_swap);
  if (!did_swap)
    return;
  switch (event) {
...
void PaintTiming::ReportDidSwapHistogram(bool did_swap) {
  DEFINE_STATIC_LOCAL(BooleanHistogram, did_swap_histogram,
                      ("PageLoad.Internal.Renderer.PaintTiming.DidSwap"));
  did_swap_histogram.Count(did_swap);
}
...
void PaintTiming::ReportSwapTimeDeltaHistogram(double timestamp,
  ...
  DEFINE_STATIC_LOCAL(
      ...
      ("PageLoad.Internal.Renderer.PaintTiming.SwapTimeDelta", 0, 10000000, 5));
