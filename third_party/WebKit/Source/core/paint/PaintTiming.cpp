...
void PaintTiming::ReportSwapTime(PaintEvent event,
  ...
  // If |result| != WebLayerTreeView::SwapResult::kDidSwap, the SwapPromise was
  // broken which happens when the CC detects no actual damage or when commits
  // fail. In this case, we requeue the SwapPromise the next time we're notified
  // about a paint.
  switch (event) {
    case PaintEvent::kFirstPaint:
      if (result != WebLayerTreeView::SwapResult::kDidSwap) {
