...
ClipPathClipper::ClipPathClipper(GraphicsContext& context,
  ...
  if (clip_path_operation.GetType() == ClipPathOperation::SHAPE) {
    ShapeClipPathOperation& shape = ToShapeClipPathOperation(clip_path_operation);
    if (!shape.IsValid())
      return;
    clip_path_recorder_.emplace(context, layout_object, shape.GetPath(reference_box));
...
bool ClipPathClipper::PrepareEffect(const FloatRect& target_bounding_box,
  ...
  mask_clip_recorder_.emplace(context_, layout_object_, SkBlendMode::kSrcOver,
                             1, &visual_rect);
  {
    if (!DrawClipAsMask(target_bounding_box, visual_rect, animated_local_transform,
