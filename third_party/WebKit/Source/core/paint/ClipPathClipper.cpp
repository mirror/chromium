...
static bool DegenerateToRect(const ClipPathOperation& clip_path) {
  if (clip_path.GetType() != ClipPathOperation::SHAPE)
    return false;
  const BasicShape* shape = ToShapeClipPathOperation(clip_path).GetBasicShape();
  if (!shape)
    return true;
  switch (shape->GetType()) {
    case BasicShape::kBasicShapePolygonType: {
      const Vector<Length>& values = ToBasicShapePolygon(*shape).Values();
      if (values.size() != 8)
        return false;
      // (p0.x == p1.x && p1.y == p2.y && p2.x == p3.x && p3.y == p0.y) ||
      // (p0.y == p1.y && p1.x == p2.x && p2.y == p3.y && p3.x == p0.x)
      return (values[0] == values[2] && values[3] == values[5] &&
              values[4] == values[6] && values[7] == values[1]) ||
             (values[1] == values[3] && values[2] == values[4] &&
              values[5] == values[7] && values[6] == values[0]);
    }
    case BasicShape::kBasicShapeInsetType: {
      const auto& inset = ToBasicShapeInset(*shape);
      return inset.TopLeftRadius().IsEmpty() &&
             inset.TopRightRadius().IsEmpty() &&
             inset.BottomRightRadius().IsEmpty() &&
             inset.BottomLeftRadius().IsEmpty();
    }
    default:
      return false;
  }
...
ClipPathClipper::ClipPathClipper(GraphicsContext& context,
  ...
  Optional<FloatRect> bounding_box = LocalClipPathBoundingBox(layout_object_);
  if (!bounding_box)
    return;
