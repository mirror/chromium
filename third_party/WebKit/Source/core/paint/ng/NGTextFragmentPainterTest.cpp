...
#include "core/paint/ng/NGTextFragmentPainter.h"
...
TEST_F(NGTextFragmentPainterTest, TestTextStyle) {
  ...
  const NGPhysicalFragment& fragment = *block_flow.RootFragment();
  const NGPhysicalBoxFragment& box_fragment = ToNGPhysicalBoxFragment(fragment);
  const NGPhysicalFragment& first_child = *box_fragment.Children()[0].Get();

  EXPECT_DISPLAY_LIST(
      RootPaintController().GetDisplayItemList(), 3,
      TestDisplayItem(layout_view, DisplayItem::kDocumentBackground),
      TestDisplayItem(fragment, DisplayItem::kBoxDecorationBackground),
      TestDisplayItem(first_child, kForegroundType));
}
