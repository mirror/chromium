...
class CORE_EXPORT NGPaintFragment : public DisplayItemClient,
 ...
 public:
  ...
  // A range and also an iterator for |FragmentsFor()| below.
  class FragmentIterator {
   public:
    explicit FragmentIterator(NGPaintFragment* current) : current_(current) {}
    FragmentIterator begin() const { return FragmentIterator(current_); }
    FragmentIterator end() const { return FragmentIterator(nullptr); }
    bool IsEmpty() const { return !current_; }
  ...
  // for a LayoutObject.
  static FragmentIterator InlineFragmentsFor(const LayoutObject*);
