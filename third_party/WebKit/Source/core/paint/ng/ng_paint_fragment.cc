...
void NGPaintFragment::UpdateVisualRectFromLayoutObject(
  ...
  if (is_in_inline_formatting_context) {
    ...
    visual_rect.offset += fragment.Offset() + paint_offset;
  ...
  if (!children_.IsEmpty()) {
    ...
    // space.
    NGPhysicalOffset child_paint_offset =
        is_in_inline_formatting_context
            ? paint_offset + fragment.Offset()
            : NGPhysicalOffset{layout_object->FirstFragment().PaintOffset()};
    for (auto& child : children_) {
      child->UpdateVisualRectFromLayoutObject(child_paint_offset);
    }
  }
}
