...
void NGPaintFragment::PopulateDescendants(
  ...
  // test coverage, split the NGPaintFragment tree at the possible engine
  // boundary.
...
NGPaintFragment::FragmentIterator NGPaintFragment::InlineFragmentsFor(
  ...
  if (it != root->first_fragment_map_.end())
    return FragmentIterator(it->value);
  return FragmentIterator(nullptr);
}
