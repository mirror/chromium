...
static std::pair<unsigned, unsigned> GetSelectionStartEndInFragment(
  ...
  switch (text_fragment.GetLayoutObject()->GetSelectionState()) {
    case SelectionState::kStart: {
      ...
      DCHECK_LE(text_fragment.StartOffset(), start_in_block);
      DCHECK_LE(start_in_block, text_fragment.EndOffset());
      return {start_in_block, text_fragment.EndOffset()};
    }
    case SelectionState::kEnd: {
      unsigned end_in_block =
          (unsigned)selection.LayoutSelectionEnd().value_or(0);
      DCHECK_LE(text_fragment.StartOffset(), end_in_block);
      DCHECK_LE(end_in_block, text_fragment.EndOffset());
      return {text_fragment.StartOffset(), end_in_block};
    }
    case SelectionState::kStartAndEnd: {
      ...
      DCHECK_LE(text_fragment.StartOffset(), end_in_block);
      DCHECK_LE(end_in_block, text_fragment.EndOffset());
      return {start_in_block, end_in_block};
    }
