...
// This returns selection rect based on |text_fragment|.
// Copied from Font mixed with CachingWordShaper's logic replacing
// TextRun with |text_fragment.TextShapeResult|.
CharacterRange SelectionRectForText(const NGPhysicalTextFragment& text_fragment,
  ...
  const unsigned from = selection_start - text_fragment.StartOffset();
  const unsigned to = selection_end - text_fragment.StartOffset();
  const TextDirection text_direction = TextDirection::kLtr;

  ShapeResultBuffer buffer;
  const ShapeResult* shape_result = text_fragment.TextShapeResult();
  const float total_width = shape_result->Width();
  buffer.AppendResult(shape_result);
  return buffer.GetCharacterRange(text_direction, total_width, from, to);
}
...
static void PaintSelection(GraphicsContext& context,
  ...
  const CharacterRange& range =
      SelectionRectForText(text_fragment, selection_start, selection_end);
