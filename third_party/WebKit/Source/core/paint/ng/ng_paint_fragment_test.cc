...
TEST_F(NGPaintFragmentTest, VisualRectForInlineBox) {
  ...
  EXPECT_EQ(NGPhysicalFragment::kFragmentLineBox, line_box_fragment.PhysicalFragment().Type());
  EXPECT_EQ(2u, line_box_fragment.Children().size());
  const NGPaintFragment& text_fragment = *line_box_fragment.Children()[1];
  EXPECT_EQ(NGPhysicalFragment::kFragmentText, text_fragment.PhysicalFragment().Type());
...
TEST_F(NGPaintFragmentTest, VisualRectForInlineBoxWithDecorations) {
  ...
  EXPECT_EQ(NGPhysicalFragment::kFragmentBox, box_fragment.PhysicalFragment().Type());
