...
void NGBoxFragmentPainter::PaintInlineChild(const NGPaintFragment& child,
  ...
  if (fragment.Type() == NGPhysicalFragment::kFragmentText) {
    ...
    NGBoxFragmentPainter(child).PaintInlineBox(
        descendants_info, paint_offset + fragment.Offset().ToLayoutPoint());
  } else {
    NOTREACHED();
  }
}
...
// TODO(kojii): This doesn't quite work as the children vector for the
// paint_fragment isn't populated.
#if 0
...
void NGBoxFragmentPainter::PaintLineBoxChildren(
  ...
  for (const auto& line : line_boxes) {
    if (!line->PhysicalFragment().IsLineBox()) {
      DCHECK(line->PhysicalFragment().IsFloating());
      continue;
    }
