...
template <typename T>
void SetDebugName(const T* node, const String& debug_name) {
  if (node)
    const_cast<T*>(node)->SetDebugName(debug_name);
}

template <typename T>
void SetDebugName(const T* node,
                  const String& name,
...
void UpdatePaintPropertyDebugNames(const LocalFrameView& frame_view) {
...
void UpdatePaintPropertyDebugNames(const LayoutObject& object,
