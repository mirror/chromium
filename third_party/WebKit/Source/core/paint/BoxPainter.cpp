...
void BoxPainter::PaintBoxDecorationBackground(const PaintInfo& paint_info,
  ...
  if (IsPaintingBackgroundOfPaintContainerIntoScrollingContentsLayer(
    ...
    paint_rect.ExpandEdges(layout_box_.BorderTop(), layout_box_.BorderRight(),
                          layout_box_.BorderBottom(), layout_box_.BorderLeft());
  } else {
...
void BoxPainter::PaintBoxDecorationBackgroundWithRect(
  ...
  if (should_paint_background) {
    PaintBackground(paint_info, paint_rect, box_decoration_data.background_color,
                    box_decoration_data.bleed_avoidance);

    if (box_decoration_data.has_appearance)
      theme_painter.PaintDecorations(layout_box_, paint_info, snapped_paint_rect);
  }
  ...
  if (!painting_overflow_contents) {
    ...
    if (box_decoration_data.has_border_decoration &&
        (!box_decoration_data.has_appearance ||
         (!theme_painted &&
          LayoutTheme::GetTheme().Painter().PaintBorderOnly(layout_box_, paint_info,
                                                         snapped_paint_rect))) &&
        !(layout_box_.IsTable() &&
...
struct FillLayerInfo {
  ...
  FillLayerInfo(const LayoutBoxModelObject& obj,
    ...
    const bool has_rounded_border =
        obj.Style()->HasBorderRadius() && (include_left_edge || include_right_edge);
    // BorderFillBox radius clipping is taken care of by
    // BackgroundBleedClip{Only,Layer}
    is_rounded_fill = has_rounded_border &&
...
inline bool PaintFastBottomLayer(const LayoutBoxModelObject& obj,
  ...
  if (obj.IsTableCell() && background_object && !background_object->IsTableCell())
    return false;
  ...
  FloatRoundedRect border =
      info.is_rounded_fill ? BackgroundRoundedRectAdjustedForBleedAvoidance(
  ...
  const ImagePaintContext image_context(obj, context, layer, *info.image, op,
                                       background_object, geometry->TileSize());
  ...
  const FloatRect src_rect =
      Image::ComputeSubsetForTile(image_tile, border.Rect(), intrinsic_tile_size);
...
void BoxPainter::PaintFillLayer(const LayoutBoxModelObject& obj,
  ...
  if (PaintFastBottomLayer(obj, paint_info, info, bg_layer, rect, bleed_avoidance,
                           box, box_size, op, background_object, geometry)) {
    return;
  }
    ...
    FloatRoundedRect border =
        info.is_border_fill
            ? BackgroundRoundedRectAdjustedForBleedAvoidance(
                  obj, rect, bleed_avoidance, box, box_size, info.include_left_edge,
                  info.include_right_edge)
            : GetBackgroundRoundedRect(obj, rect, box, box_size.Width(),
                                       box_size.Height(), info.include_left_edge,
                                       info.include_right_edge);
  ...
  LayoutUnit b_right = info.include_right_edge ? obj.BorderRight() : LayoutUnit();
  LayoutUnit p_left = info.include_left_edge ? obj.PaddingLeft() : LayoutUnit();
  LayoutUnit p_right = info.include_right_edge ? obj.PaddingRight() : LayoutUnit();
  ...
  if (bg_layer.Clip() == kTextFillBox) {
    ...
    if (box) {
      ...
      box->Paint(info, LayoutPoint(scrolled_paint_rect.X() - box->X(),
...
void BoxPainter::PaintNormalBoxShadow(const PaintInfo& info,
  ...
  for (size_t i = shadow_list->Shadows().size(); i--;) {
    ...
    const Color& shadow_color =
        shadow.GetColor().Resolve(style.VisitedDependentColor(CSSPropertyColor));
