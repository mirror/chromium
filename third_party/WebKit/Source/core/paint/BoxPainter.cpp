...
LayoutPoint BoxPainter::AdjustPaintOffset(PaintInfo& paint_info,
  ...
  if (!ShouldAdjustForPaintOffsetTranslation(paint_info))
    return paint_offset + layout_box_.Location();
  ...
  paint_info.SetContentsProperties(paint_info.context.GetPaintController(),
                                   layout_box_, chunk_properties);

  paint_info.UpdateCullRect(
