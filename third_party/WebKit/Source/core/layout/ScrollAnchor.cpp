...
static const String ComputeUniqueSelector(Node* anchor_node) {
  DCHECK(anchor_node);
  // Pseudo-elements aren't part of the DOM and can't be used as part of a
  // selector, so we give up in this case.
...
bool ScrollAnchor::RestoreAnchor(const SerializedAnchor& serialized_anchor) {
  ...
  DEFINE_STATIC_LOCAL(EnumerationHistogram, restoration_success_histogram,
                      ("Layout.ScrollAnchor.RestorationSuccess", 2));
  ...
  if (exception_state.HadException()) {
    restoration_success_histogram.Count(0);
    return false;
  }
  ...
  for (unsigned index = 0; index < found_elements->length(); index++) {
    ...
    // If finding the anchor failed, reset the scroll position and try again
    // with the next found element.
    if (!anchor_object_) {
    ...
    restoration_success_histogram.Count(1);
    return true;
  }
