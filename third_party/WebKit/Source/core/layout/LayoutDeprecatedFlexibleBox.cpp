    ...
    for (RootInlineBox* box = block_flow->FirstRootBox(); box;
      ...
      if (++count == line_count)
        return (box->LineBottom() +
                (include_bottom
                     ? (block_flow->BorderBottom() + block_flow->PaddingBottom())
                     : LayoutUnit()))
            .ToInt();
    }
    return -1;
...
LayoutDeprecatedFlexibleBox::LayoutDeprecatedFlexibleBox(Element& element)
  ...
  if (!IsAnonymous()) {
    ...
    if (url.ProtocolIs("chrome"))
      ...
      UseCounter::Count(GetDocument(), UseCounter::kDeprecatedFlexboxWebContent);
  }
}
...
void LayoutDeprecatedFlexibleBox::ApplyLineClamp(FlexBoxIterator& iterator,
  ...
  for (LayoutBox* child = iterator.First(); child; child = iterator.Next()) {
    ...
    LayoutUnit new_height(
        GetHeightForLineCount(block_child, num_visible_lines, true, dummy_count));
    ...
    LayoutUnit block_left_edge = dest_block.LogicalLeftOffsetForLine(
        last_visible_line->Y(), kDoNotIndentText);
    last_visible_line->PlaceEllipsis(ellipsis_str, left_to_right, block_left_edge,
                                   block_right_edge, LayoutUnit(total_width),
                                   LayoutUnit(), false);
