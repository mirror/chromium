...
void LayoutTheme::AddVisualOverflow(Node* node,
...
bool LayoutTheme::ShouldDrawDefaultFocusRing(Node* node,
                                             const ComputedStyle& style) const {
...
bool LayoutTheme::ControlStateChanged(Node* node,
                                      const ComputedStyle& style,
...
ControlStates LayoutTheme::ControlStatesForNode(Node* node,
                                                const ComputedStyle& style) {
...
bool LayoutTheme::IsActive(Node* node) {
  if (!node)
    return false;
...
bool LayoutTheme::IsChecked(Node* node) {
  if (auto* input = ToHTMLInputElementOrNull(node))
    return input->ShouldAppearChecked();
  return false;
}

bool LayoutTheme::IsIndeterminate(Node* node) {
  if (auto* input = ToHTMLInputElementOrNull(node))
    return input->ShouldAppearIndeterminate();
  return false;
}

bool LayoutTheme::IsEnabled(Node* node) {
  if (!node || !node->IsElementNode())
    return true;
  return !ToElement(node)->IsDisabledFormControl();
}

bool LayoutTheme::IsFocused(Node* node) {
  if (!node)
    return false;
...
bool LayoutTheme::IsPressed(Node* node) {
  if (!node)
    return false;
  return node->IsActive();
}

bool LayoutTheme::IsSpinUpButtonPartPressed(Node* node) {
...
bool LayoutTheme::IsReadOnlyControl(Node* node) {
  if (!node || !node->IsElementNode() ||
...
bool LayoutTheme::IsHovered(Node* node) {
  if (!node)
    return false;
...
bool LayoutTheme::IsSpinUpButtonPartHovered(Node* node) {
  if (!node || !node->IsElementNode() ||
      !ToElement(node)->IsSpinButtonElement())
    return false;
  SpinButtonElement* element = ToSpinButtonElement(node);
