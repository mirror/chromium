...
FloatingObject::FloatingObject(LayoutBox* layout_object,
    ...
    : layout_object_(layout_object),
      ...
      is_placed_(true),
      is_lowest_non_overhanging_float_in_child_(is_lowest_non_overhanging_float_in_child)
#if DCHECK_IS_ON()
      ,
      is_in_placed_tree_(false)
#endif
...
std::unique_ptr<FloatingObject> FloatingObject::UnsafeClone() const {
  std::unique_ptr<FloatingObject> clone_object =
      WTF::WrapUnique(new FloatingObject(GetLayoutObject(), GetType(), frame_rect_,
...
inline void FindNextFloatLogicalBottomAdapter::CollectIfNeeded(
  ...
  LayoutUnit float_bottom = layout_object_.LogicalBottomForFloat(floating_object);
  ...
  if (ShapeOutsideInfo* shape_outside =
    ...
    LayoutUnit shape_bottom =
        ...
        layout_object_.MarginBeforeForChild(*floating_object.GetLayoutObject()) +
...
LayoutUnit FloatingObjects::LowestFloatLogicalBottom(
  ...
  if (float_type != FloatingObject::kFloatLeftRight) {
    if (HasLowestFloatLogicalBottomCached(is_in_horizontal_writing_mode, float_type))
  ...
  if (float_type == FloatingObject::kFloatLeftRight) {
    ...
    lowest_float_bottom = std::max(lowest_float_bottom_left, lowest_float_bottom_right);
    ...
    for (FloatingObjectSetIterator it = floating_object_set.begin(); it != end;
         ++it) {
      FloatingObject& floating_object = *it->get();
      if (floating_object.IsPlaced() && floating_object.GetType() == float_type) {
...
FloatingObject* FloatingObjects::LowestFloatingObject() const {
  ...
  if (!HasLowestFloatLogicalBottomCached(is_in_horizontal_writing_mode,
      ...
      !HasLowestFloatLogicalBottomCached(is_in_horizontal_writing_mode,
                                         FloatingObject::kFloatRight))
    return nullptr;
  FloatingObject* lowest_left_object = lowest_float_bottom_cache_[0].floating_object;
...
template <>
inline bool ComputeFloatOffsetForFloatLayoutAdapter<FloatingObject::kFloatLeft>::
...
inline bool ComputeFloatOffsetForFloatLayoutAdapter<
    FloatingObject::kFloatRight>::UpdateOffsetIfNeeded(const FloatingObject&
                                                          floating_object) {
  LayoutUnit logical_left = layout_object_->LogicalLeftForFloat(floating_object);
...
LayoutUnit ComputeFloatOffsetForFloatLayoutAdapter<
  ...
  return this->outermost_float_
             ? this->layout_object_->LogicalBottomForFloat(
...
ComputeFloatOffsetAdapter<FloatTypeValue>::CollectIfNeeded(
  ...
  // tree.
  DCHECK(floating_object.IsPlaced());
  DCHECK_EQ(interval.Low(), layout_object_->LogicalTopForFloat(floating_object));
...
template <>
inline bool ComputeFloatOffsetForLineLayoutAdapter<FloatingObject::kFloatRight>::
    UpdateOffsetIfNeeded(const FloatingObject& floating_object) {
  LayoutUnit logical_left = layout_object_->LogicalLeftForFloat(floating_object);
  if (ShapeOutsideInfo* shape_outside =
          floating_object.GetLayoutObject()->GetShapeOutsideInfo()) {
