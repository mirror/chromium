...
void ConstructBidiRunsForLine(InlineBidiResolver& top_resolver,
  ...
  while (!top_resolver.IsolatedRuns().IsEmpty()) {
    ...
    SetupResolverToResumeInIsolate(isolated_resolver, isolated_inline, start_obj);
    ...
    InlineIterator iter =
        InlineIterator(LineLayoutItem(isolated_inline), LineLayoutItem(start_obj),
                       isolated_run.position);
    ...
    while (!isolated_resolver.IsolatedRuns().IsEmpty()) {
      BidiIsolatedRun run_with_context = isolated_resolver.IsolatedRuns().back();
