...
int LayoutTableSection::OffsetForHeader() const {
...
void LayoutTableSection::AdjustRowForPagination(LayoutTableRow& row_object,
  ...
  if (!pagination_strut) {
    ...
    if (OffsetForHeader()) {
      ...
      row_is_at_top_of_column =
          !offset_from_top_of_page ||
          offset_from_top_of_page <= OffsetForHeader() ||
  ...
  pagination_strut += OffsetForHeader();
...
bool LayoutTableSection::GroupShouldRepeat() const {
  ...
  LayoutUnit logical_height = LogicalHeight() - OffsetForHeader();
