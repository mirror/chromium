...
void LayoutTableSection::UpdateLogicalWidthForCollapsedCells(
  ...
  for (unsigned r = 0; r < total_rows; r++) {
    ...
    for (unsigned c = 0; c < n_cols; c++) {
      ...
      if (!cell)
        continue;
      // TODO: Current behavior assumes that collapsing the first column in a
      // col-spanning cell makes the cell width zero. This is consistent with
      // collapsing row-spanning cells, but still needs to be specified.
