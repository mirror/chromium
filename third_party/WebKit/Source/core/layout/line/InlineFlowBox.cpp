...
inline void InlineFlowBox::AddTextBoxVisualOverflow(
  ...
  if (it != text_box_data_map.end()) {
    ...
    visual_rect_outsets = LayoutRectOutsets(FloatRectOutsets(
  ...
  if (float stroke_width = style.TextStrokeWidth()) {
    visual_rect_outsets.Expand(LayoutUnit(stroke_width / 2.0f));
  }
  ...
  if (ShadowList* text_shadow = style.TextShadow()) {
    ...
    visual_rect_outsets.Expand(text_shadow_logical_outsets);
  }
