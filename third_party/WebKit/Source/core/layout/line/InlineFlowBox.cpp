...
static inline bool HasIdenticalLineHeightProperties(
  ...
  return parent_style.HasIdenticalAscentDescentAndLineGap(child_style) &&
         ...
         (parent_style.VerticalAlign() == EVerticalAlign::kBaseline || is_root) &&
    ...
    if (child->GetLineLayoutItem().IsAtomicInlineLevel()) {
      ...
      if (child->GetLineLayoutItem().IsBR()) {
        ...
        if (!child_flow_box->DescendantsHaveSameLineHeightAndBaseline() ||
            !HasIdenticalLineHeightProperties(parent_style, child_style, root) ||
...
void InlineFlowBox::DetermineSpacingForFlowBoxes(
  ...
  for (InlineBox* curr_child = FirstChild(); curr_child;
    ...
    if (curr_child->IsInlineFlowBox()) {
      ...
      curr_flow->DetermineSpacingForFlowBoxes(
          last_line, is_logically_last_run_wrapped, logically_last_run_layout_object);
    }
  }
}

LayoutUnit InlineFlowBox::PlaceBoxesInInlineDirection(LayoutUnit logical_left,
...
// Seek a test case.
void InlineFlowBox::PlaceBoxRangeInInlineDirection(InlineBox* first_child,
...
void InlineFlowBox::ComputeLogicalBoxHeights(
  ...
  if (IsRootInlineBox()) {
    ...
    int ascent = 0;
    int descent = 0;
    root_box->AscentAndDescentForBox(root_box, text_box_data_map, ascent, descent,
  ...
  for (InlineBox* curr = FirstChild(); curr; curr = curr->NextOnLine()) {
    ...
    if (inline_flow_box)
      inline_flow_box->ComputeLogicalBoxHeights(
          root_box, max_position_top, max_position_bottom, max_ascent, max_descent,
          set_max_ascent, set_max_descent, no_quirks_mode, text_box_data_map,
          baseline_type, vertical_position_cache);
  }
}
...
void InlineFlowBox::PlaceBoxesInBlockDirection(
  ...
  for (InlineBox* curr = FirstChild(); curr; curr = curr->NextOnLine()) {
    ...
    if (curr->VerticalAlign() == EVerticalAlign::kTop) {
      ...
      if (!no_quirks_mode && inline_flow_box && !inline_flow_box->HasTextChildren() &&
    ...
    if (child_affects_top_bottom_pos) {
      if (curr->GetLineLayoutItem().IsRubyRun()) {
        ...
        if (LineLayoutRubyBase ruby_base = ruby_run.RubyBase()) {
          ...
          LayoutUnit top_ruby_base_leading =
              ruby_base.LogicalTop() + (ruby_base.FirstRootBox()
                                           ? ruby_base.FirstRootBox()->LineTop()
                                           : LayoutUnit());
          new_logical_top +=
      ...
      if (!set_line_top) {
        ...
        line_top_including_margins = std::min(
            line_top,
            std::min(line_top_including_margins, new_logical_top_including_margins));
      }
      selection_bottom = std::max(
          selection_bottom, new_logical_top + box_height - border_padding_height);
      line_bottom = std::max(line_bottom, new_logical_top + box_height);
      line_bottom_including_margins = std::max(
          line_bottom,
          std::max(line_bottom_including_margins,
                   new_logical_top_including_margins + box_height_including_margins));
    }
    ...
    if (inline_flow_box)
      inline_flow_box->PlaceBoxesInBlockDirection(
          ...
          line_bottom_including_margins, has_annotations_before, has_annotations_after,
          baseline_type);
  }
...
inline void InlineFlowBox::AddTextBoxVisualOverflow(
  ...
  float top_glyph_edge = glyph_overflow ? (is_flipped_line ? glyph_overflow->bottom
                                                      : glyph_overflow->top)
                                     : 0;
  ...
  if (style.GetTextEmphasisMark() != kTextEmphasisMarkNone &&
    ...
    if ((emphasis_mark_position == kTextEmphasisPositionOver) ==
      ...
      bottom_glyph_overflow = std::max(bottom_glyph_overflow, emphasis_mark_height);
  }
  ...
  LayoutUnit child_overflow_logical_top(
      std::min(text_shadow_logical_top + top_glyph_overflow, top_glyph_overflow));
  LayoutUnit child_overflow_logical_bottom(std::max(
      text_shadow_logical_bottom + bottom_glyph_overflow, bottom_glyph_overflow));
  LayoutUnit child_overflow_logical_left(
      std::min(text_shadow_logical_left + left_glyph_overflow, left_glyph_overflow));
  ...
  LayoutUnit logical_top_visual_overflow = std::min(
      LayoutUnit(enclosing_logical_top_with_overflow), logical_visual_overflow.Y());
  ...
  LayoutUnit logical_left_visual_overflow = std::min(
      LayoutUnit(enclosing_logical_left_with_overflow), logical_visual_overflow.X());
...
static void ComputeGlyphOverflow(
  ...
  float measured_width =
      layout_text.Width(text->Start(), text->Len(), LayoutUnit(),
                       text->Direction(), false, &fallback_fonts, &glyph_bounds);
  ...
  glyph_overflow.SetFromBounds(
      ...
      font_data ? font_data->GetFontMetrics().FloatDescent() : 0, measured_width);
...
LayoutUnit InlineFlowBox::PlaceEllipsisBox(bool ltr,
  ...
  while (box) {
    LayoutUnit curr_result = box->PlaceEllipsisBox(
        ltr, visible_left_edge, visible_right_edge, ellipsis_width, truncated_width,
    ...
    if (curr->GetLineLayoutItem().IsAtomicInlineLevel() &&
      ...
      if (!ruby_run.Style()->IsFlippedLinesWritingMode()) {
        ...
        LayoutUnit bottom_of_last_ruby_text_line =
            ruby_text.LogicalTop() + (ruby_text.LastRootBox()
                                         ? ruby_text.LastRootBox()->LineBottom()
...
void InlineFlowBox::ShowLineTreeAndMark(const InlineBox* marked_box1,
  ...
  for (const InlineBox* box = FirstChild(); box; box = box->NextOnLine())
    box->ShowLineTreeAndMark(marked_box1, marked_label1, marked_box2, marked_label2,
                             obj, depth + 1);
}
