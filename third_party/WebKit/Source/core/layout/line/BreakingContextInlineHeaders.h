 ...
 public:
  BreakingContext(InlineBidiResolver& resolver,
      ...
      : resolver_(resolver),
        ...
        allow_images_to_break_(!block.GetDocument().InQuirksMode() ||
                             !block.IsTableCell() ||
...
inline void BreakingContext::HandleOutOfFlowPositioned(
  ...
  if (!is_inline_type) {
    block_.SetStaticInlinePositionForChild(box,
                                            block_.StartOffsetForContent());
  } else {
...
ALWAYS_INLINE void BreakingContext::SetCurrentCharacterIsSpace(UChar c) {
  current_character_is_space_ = c == kSpaceCharacter || c == kTabulationCharacter ||
...
ALWAYS_INLINE bool BreakingContext::ShouldMidWordBreak(
  ...
  char_width = TextWidth(
      layout_text, current_.Offset(), mid_word_break_is_before_surrogate_pair ? 2 : 1,
      font, width_.CommittedWidth() + width_from_last_breaking_opportunity,
      collapse_white_space_);
  if (width_.CommittedWidth() + width_from_last_breaking_opportunity + char_width <=
...
ALWAYS_INLINE bool BreakingContext::RewindToFirstMidWordBreak(
  ...
  if (break_all) {
    ...
    line_break_iterator.IsBreakable(end, next_breakable, LineBreakType::kBreakAll);
    if (next_breakable < 0)
      return false;
    end = next_breakable;
  }
...
ALWAYS_INLINE bool BreakingContext::Hyphenate(
  ...
  run.SetTabSize(!collapse_white_space_, style.GetTabSize());
  run.SetXPos(width_.CurrentWidth());
  unsigned max_prefix_length = font.OffsetForPosition(run, max_prefix_width, false);
...
inline bool BreakingContext::HandleText(WordMeasurements& word_measurements,
  ...
  bool keep_all = current_style_->WordBreak() == kKeepAllWordBreak && auto_wrap_;
  ...
  for (; current_.Offset() < layout_text.TextLength();
    ...
    if (can_break_mid_word && !mid_word_break && !U16_IS_TRAIL(c))
      mid_word_break = ShouldMidWordBreak(
          c, layout_text, font, char_width, width_from_last_breaking_opportunity,
    ...
    if (!between_words && !mid_word_break) {
      if (ignoring_spaces_) {
        ...
        word_spacing_for_word_measurement =
            (apply_word_spacing && word_measurements.back().width) ? word_spacing
                                                                : 0;
    ...
    if (CanBreakAtWhitespace(break_words, word_measurement, stopped_ignoring_spaces,
                             char_width, hyphenated, disable_soft_hyphen,
                             hyphen_width, between_words, mid_word_break,
                             can_break_mid_word, previous_character_is_space,
                             last_width_measurement, layout_text, font,
  ...
  layout_text_info_.line_break_iterator_.SetPriorContext(last_character,
  ...
  if (!ignoring_spaces_) {
    last_width_measurement =
        TextWidth(layout_text, last_space, current_.Offset() - last_space, font,
                  width_.CurrentWidth(), collapse_white_space_,
  ...
  if (collapse_white_space_ && current_character_is_space_ && last_width_measurement)
    width_.SetTrailingWhitespaceWidth(last_width_measurement +
                                       additional_width_from_ancestors);
  ...
  if (!ignoring_spaces_ && !width_.FitsOnLine()) {
    if (hyphenation && (next_object_ || is_line_empty)) {
      ...
      DCHECK_EQ(current_.Offset(),
                static_cast<unsigned>(word_measurement.end_offset));
      if (Hyphenate(layout_text, style, font, *hyphenation, last_space_word_spacing,
                    word_measurement)) {
        hyphenated = true;
        at_end_ = true;
      }
...
inline bool BreakingContext::CanBreakAtWhitespace(
  ...
  // If we break only after white-space, consider the current character
  // as candidate width for this line.
  if (mid_word_break || TrailingSpaceExceedsAvailableWidth(
                          can_break_mid_word, layout_text, word_measurement,
                          apply_word_spacing, word_spacing, font) ||
      !width_.FitsOnLine()) {
...
inline IndentTextOrNot RequiresIndent(bool is_first_line,
  ...
  if (is_first_line ||
      (is_after_hard_line_break && style.GetTextIndentLine()) == kTextIndentEachLine)
