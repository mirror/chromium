...
LayoutUnit RootInlineBox::PlaceEllipsis(const AtomicString& ellipsis_str,
  ...
  if (!found_box) {
    EllipsisBox* ellipsis_box = new EllipsisBox(
        GetLineLayoutItem(), ellipsis_str, this, ellipsis_width, LogicalHeight(),
...
LayoutUnit RootInlineBox::AlignBoxesInBlockDirection(
  ...
  ComputeLogicalBoxHeights(this, max_position_top, max_position_bottom, max_ascent,
                           max_descent, set_max_ascent, set_max_descent,
                           no_quirks_mode, text_box_data_map, BaselineType(),
  ...
  PlaceBoxesInBlockDirection(height_of_block, max_height, max_ascent, no_quirks_mode,
                             line_top, line_bottom, selection_bottom, set_line_top,
...
LayoutUnit RootInlineBox::SelectionBottom() const {
  LayoutUnit selection_bottom = GetLineLayoutItem().GetDocument().InNoQuirksMode()
                                   ? selection_bottom_
                                   : line_bottom_;

  if (has_annotations_after_)
    selection_bottom += !GetLineLayoutItem().Style()->IsFlippedLinesWritingMode()
...
BidiStatus RootInlineBox::LineBreakBidiStatus() const {
  return BidiStatus(
      ...
      static_cast<WTF::Unicode::CharDirection>(line_break_bidi_status_last_strong_),
...
void RootInlineBox::AscentAndDescentForBox(
  ...
  if (used_fonts && !used_fonts->IsEmpty() && (box->GetLineLayoutItem()
    ...
    for (size_t i = 0; i < used_fonts->size(); ++i) {
      ...
      int half_leading = (font_metrics.LineSpacing() - font_metrics.Height()) / 2;
  ...
  if (include_leading && !set_used_font_with_leading) {
    ...
    int descent_with_leading = (box->LineHeight() - ascent_with_leading).ToInt();
    SetAscentAndDescent(ascent, descent, ascent_with_leading, descent_with_leading,
                        ascent_descent_set);
