...
class InlineIterator {
 ...
 public:
  enum IncrementRule { kFastIncrementInIsolatedLayout, kFastIncrementInTextNode };
...
static inline WTF::Unicode::CharDirection EmbedCharFromDirection(
  ...
  return dir == TextDirection::kRtl ? kRightToLeftOverride : kLeftToRightOverride;
}
...
static inline bool IsCollapsibleSpace(UChar character,
  ...
  if (character == ' ' || character == '\t' || character == kSoftHyphenCharacter)
    return true;
  if (character == '\n')
...
inline BidiRun* InlineBidiResolver::AddTrailingRun(
  ...
  BidiRun* new_trailing_run = new BidiRun(
      context->Override(), context->Level(), start, stop, run->line_layout_item_,
      WTF::Unicode::kOtherNeutral, context->Dir());
...
static void AdjustMidpointsAndAppendRunsForObjectIfNeeded(
  ...
  for (;;) {
    ...
    if (have_next_midpoint)
      next_midpoint =
          line_midpoint_state.Midpoints()[line_midpoint_state.CurrentMidpoint()];
...
inline void InlineBidiResolver::AppendRun(BidiRunList<BidiRun>& runs) {
  if (!empty_run_ && !eor_.AtEnd() && !reached_end_of_line_) {
    ...
    if (obj && !is_end_of_line) {
      unsigned pos =
          obj == eor_.GetLineLayoutItem() ? eor_.Offset() : INT_MAX;
