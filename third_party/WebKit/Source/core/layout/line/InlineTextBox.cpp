  ...
  if (state == SelectionStart || state == SelectionEnd ||
    ...
    int end_of_line_adjustment_for_css_line_break =
        GetLineLayoutItem().Style()->GetLineBreak() == kLineBreakAfterWhiteSpace
            ...
            : 0;
    bool start =
        (state != SelectionEnd && start_pos >= start_ &&
         start_pos <= start_ + len_ + end_of_line_adjustment_for_css_line_break);
...
LayoutRect InlineTextBox::LocalSelectionRect(int start_pos, int end_pos) const {
  ...
  TextRun text_run =
      ConstructTextRun(style_to_use, respect_hyphen ? &characters_with_hyphen : 0);
  ...
  if (s_pos || e_pos != static_cast<int>(len_)) {
    r = LayoutRect(EnclosingIntRect(font.SelectionRectForText(
        text_run, FloatPoint(starting_point), sel_height.ToInt(), s_pos, e_pos)));
  } else {
...
void InlineTextBox::PaintTextMatchMarkerForeground(const PaintInfo& paint_info,
                                                   const LayoutPoint& box_origin,
                                                   const DocumentMarker& marker,
                                                   const ComputedStyle& style,
...
void InlineTextBox::PaintTextMatchMarkerBackground(const PaintInfo& paint_info,
                                                   const LayoutPoint& box_origin,
                                                   const DocumentMarker& marker,
                                                   const ComputedStyle& style,
                                                   const Font& font) const {
...
void InlineTextBox::ShowBox(int printed_characters) const {
  ...
  for (; printed_characters < kLayoutObjectCharacterOffset; printed_characters++)
    fputc(' ', stderr);
