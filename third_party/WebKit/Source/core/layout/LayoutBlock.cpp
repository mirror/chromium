...
void LayoutBlock::StyleWillChange(StyleDifference diff,
  ...
  if (old_style && Parent()) {
    ...
    if ((old_style_contains_fixed_position && !new_style_contains_fixed_position) ||
    ...
    if (!old_style_contains_absolute_position && new_style_contains_absolute_position) {
...
static bool BorderOrPaddingLogicalDimensionChanged(
  ...
  if (new_style.IsHorizontalWritingMode() == (logical_extent == kLogicalWidth)) {
...
void LayoutBlock::StyleDidChange(StyleDifference diff,
  ...
  height_available_to_children_changed_ |= old_style && diff.NeedsFullLayout() &&
                                        NeedsLayout() &&
                                        BorderOrPaddingLogicalDimensionChanged(
                                            *old_style, new_style, kLogicalHeight);
}
...
void LayoutBlock::AddChildBeforeDescendant(LayoutObject* new_child,
  ...
  LayoutObject* before_child = SplitAnonymousBoxesAroundChild(before_descendant);
...
void LayoutBlock::AddChild(LayoutObject* new_child, LayoutObject* before_child) {
...
bool LayoutBlock::WidthAvailableToChildrenHasChanged() {
  ...
  // when it needs to?
  bool width_available_to_children_has_changed = width_available_to_children_changed_;
  width_available_to_children_changed_ = false;
...
void LayoutBlock::LayoutPositionedObject(LayoutBox* positioned_object,
  ...
  if (relayout_children && positioned_object->NeedsPreferredWidthsRecalculation())
...
LayoutUnit LayoutBlock::LogicalLeftSelectionOffset(const LayoutBlock* root_block,
...
void LayoutBlock::RemovePositionedObjects(
  ...
  for (auto* positioned_object : *positioned_descendants) {
    if (!o || (positioned_object->IsDescendantOf(o) && o != positioned_object)) {
...
void LayoutBlock::AddPercentHeightDescendant(LayoutBox* descendant) {
  ...
  if (!descendant_set) {
    descendant_set = new TrackedLayoutBoxListHashSet;
    g_percent_height_descendants_map->Set(this, WTF::WrapUnique(descendant_set));
  }
...
TrackedLayoutBoxListHashSet* LayoutBlock::PercentHeightDescendantsInternal()
    const {
  return g_percent_height_descendants_map ? g_percent_height_descendants_map->at(this)
                                      : nullptr;
}
...
bool LayoutBlock::IsPointInOverflowControl(
  ...
  return Layer()->GetScrollableArea()->HitTestOverflowControls(
      result,
      RoundedIntPoint(location_in_container - ToLayoutSize(accumulated_offset)));
}
...
bool LayoutBlock::HitTestOverflowControl(
  ...
  if (VisibleToHitTestRequest(result.GetHitTestRequest()) &&
      IsPointInOverflowControl(result, location_in_container.Point(),
                               adjusted_location)) {
    UpdateHitTestResult(
        result, location_in_container.Point() - ToLayoutSize(adjusted_location));
...
bool LayoutBlock::HitTestChildren(HitTestResult& result,
  ...
  for (LayoutBox* child = LastChildBox(); child;
    ...
    LayoutPoint child_point = FlipForWritingModeForChild(child, scrolled_offset);
    if (!child->HasSelfPaintingLayer() && !child->IsFloating() &&
        !child->IsColumnSpanAll() &&
        child->NodeAtPoint(result, location_in_container, child_point,
                           child_hit_test)) {
      UpdateHitTestResult(
          result, FlipForWritingMode(ToLayoutPoint(location_in_container.Point() -
                                                   accumulated_offset)));
      return true;
    }
  }
...
PositionWithAffinity LayoutBlock::PositionForPoint(const LayoutPoint& point) {
  ...
  if (last_candidate_box) {
    if (point_in_logical_contents.Y() > LogicalTopForChild(*last_candidate_box) ||
        (!blocks_are_flipped &&
         point_in_logical_contents.Y() == LogicalTopForChild(*last_candidate_box)))
...
void LayoutBlock::ComputeIntrinsicLogicalWidths(
  ...
  if (ChildrenInline()) {
    ...
    ToLayoutBlockFlow(const_cast<LayoutBlock*>(this))
        ->ComputeInlinePreferredLogicalWidths(min_logical_width, max_logical_width);
  } else {
  ...
  if (IsTableCell()) {
    ...
    if (table_cell_width.IsFixed() && table_cell_width.Value() > 0)
      max_logical_width =
          std::max(min_logical_width, AdjustContentBoxLogicalWidthForBoxSizing(
...
void LayoutBlock::ComputePreferredLogicalWidths() {
  ...
  if (IsTableCell()) {
    min_preferred_logical_width_ = LayoutUnit(min_preferred_logical_width_.Ceil());
    max_preferred_logical_width_ = LayoutUnit(max_preferred_logical_width_.Ceil());
  }
...
void LayoutBlock::ComputeBlockPreferredLogicalWidths(
  ...
  while (child) {
    ...
    LayoutUnit child_min_preferred_logical_width, child_max_preferred_logical_width;
    ComputeChildPreferredLogicalWidths(*child, child_min_preferred_logical_width,
    ...
    if (!child->IsFloating()) {
      if (child->IsBox() && ToLayoutBox(child)->AvoidsFloats()) {
        ...
        LayoutUnit max_left = margin_logical_left > 0
  ...
  max_logical_width = std::max(float_left_width + float_right_width, max_logical_width);
}
...
LayoutUnit LayoutBlock::LineHeight(bool first_line,
  ...
  const ComputedStyle& style =
      StyleRef(first_line && GetDocument().GetStyleEngine().UsesFirstLineRules());
...
bool LayoutBlock::TryLayoutDoingPositionedMovementOnly() {
  ...
  LayoutUnit old_intrinsic_content_logical_height = IntrinsicContentLogicalHeight();
