...
bool LayoutView::HitTestNoLifecycleUpdate(HitTestResult& result) {
  ...
  if (hit_test_cache_->LookupCachedResult(cache_result, dom_tree_version)) {
    ...
    IntPoint frame_point =
        GetFrameView()->ContentsToFrame(result.GetHitTestLocation().RoundedPoint());
...
void LayoutView::SetShouldDoFullPaintInvalidationOnResizeIfNeeded(
  ...
  if (Style()->HasFixedBackgroundImage() &&
      (!compositor_ ||
       !compositor_->NeedsFixedRootBackgroundLayer(Layer()))) {
...
void LayoutView::GetLayout() {
  ...
  if (PageLogicalHeight() && ShouldUsePrintingLayout()) {
    min_preferred_logical_width_ = max_preferred_logical_width_ = LogicalWidth();
...
void LayoutView::MapLocalToAncestor(const LayoutBoxModelObject* ancestor,
  ...
  if (!ancestor && mode & kUseTransforms && ShouldUseTransformFromContainer(0)) {
  ...
  if (mode & kTraverseDocumentBoundaries) {
    ...
    if (!parent_doc_layout_item.IsNull()) {
      ...
      parent_doc_layout_item.MapLocalToAncestor(ancestor, transform_state, mode);
    } else {
...
const LayoutObject* LayoutView::PushMappingToContainer(
  ...
  if ((!ancestor_to_stop_at || container) &&
    ...
    geometry_map.Push(this, t, kContainsFixedPosition, OffsetForFixedPosition());
  } else {
...
void LayoutView::ComputeSelfHitTestRects(Vector<LayoutRect>& rects,
  ...
  // explicitly).
  rects.push_back(
      LayoutRect(LayoutPoint::Zero(), LayoutSize(GetFrameView()->ContentsSize())));
}
...
bool LayoutView::MapToVisualRectInAncestorSpaceInternal(
  ...
  if (LayoutBox* obj = owner->GetLayoutBox()) {
    ...
    return obj->MapToVisualRectInAncestorSpaceInternal(ancestor, transform_state,
                                                       visual_rect_flags);
  }
...
LayoutSize LayoutView::OffsetForFixedPosition(bool include_pending_scroll) const {
