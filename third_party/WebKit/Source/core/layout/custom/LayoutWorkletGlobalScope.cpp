...
bool ParseGeneratorFunction(v8::Isolate* isolate,
  ...
  if (function_value->IsNullOrUndefined()) {
    exception_state->ThrowTypeError(
        "The '" + function_name +
        "' function on the prototype does not exist.");
    return false;
  }
...
void LayoutWorkletGlobalScope::registerLayout(const String& name,
  ...
  if (!ParsePropertyList(isolate, context, constructor, "input_properties",
  ...
  if (!ParsePropertyList(
          isolate, context, constructor, "child_input_properties",
          &child_native_invalidation_properties,
          &child_custom_invalidation_properties, &exception_state, &block))
    return;
  ...
  if (!ParseGeneratorFunction(isolate, context, prototype, "intrinsic_sizes",
  ...
  if (document_definition_map->Contains(name)) {
    ...
    // Notify the generator ready only when register layout is called the second
