...
TEST_P(ScrollAnchorTest, UMAMetricUpdated) {
  ...
  histogram_tester.ExpectUniqueSample("Layout.ScrollAnchor.AdjustedScrollOffset",
                                     1, 1);
...
TEST_P(ScrollAnchorTest, TextBounds) {
  ...
  SetHeight(GetDocument().GetElementById("a"), 100);
  EXPECT_EQ(GetDocument().GetElementById("b")->GetLayoutObject()->SlowFirstChild(),
            GetScrollAnchor(LayoutViewport()).AnchorObject());
}
...
TEST_P(ScrollAnchorTest, ExcludeAbsolutePositionThatSticksToViewport) {
  ...
  // position:absolute.
  EXPECT_EQ(rel_pos->GetLayoutObject(), GetScrollAnchor(scroller).AnchorObject());
  ...
  // position:absolute.
  EXPECT_EQ(abs_pos->GetLayoutObject(), GetScrollAnchor(scroller).AnchorObject());
}
