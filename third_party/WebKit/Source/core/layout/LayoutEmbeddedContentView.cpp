...
LayoutEmbeddedContentView::LayoutEmbeddedContentView(Element* element)
...
void LayoutEmbeddedContentView::Deref() {
  if (--ref_count_ <= 0)
    delete this;
}

void LayoutEmbeddedContentView::WillBeDestroyed() {
...
void LayoutEmbeddedContentView::Destroy() {
  WillBeDestroyed();
  // We call clearNode here because LayoutEmbeddedContentView is ref counted.
...
LayoutEmbeddedContentView::~LayoutEmbeddedContentView() {
  DCHECK_LE(ref_count_, 0);
}

LocalFrameView* LayoutEmbeddedContentView::ChildFrameView() const {
...
PluginView* LayoutEmbeddedContentView::Plugin() const {
  EmbeddedContentView* embedded_content_view = GetEmbeddedContentView();
...
EmbeddedContentView* LayoutEmbeddedContentView::GetEmbeddedContentView() const {
  Node* node = GetNode();
  if (node && node->IsFrameOwnerElement())
...
PaintLayerType LayoutEmbeddedContentView::LayerTypeRequired() const {
...
bool LayoutEmbeddedContentView::RequiresAcceleratedCompositing() const {
...
bool LayoutEmbeddedContentView::NeedsPreferredWidthsRecalculation() const {
  if (LayoutReplaced::NeedsPreferredWidthsRecalculation())
    return true;
  return EmbeddedReplacedContent();
}

bool LayoutEmbeddedContentView::NodeAtPointOverEmbeddedContentView(
...
bool LayoutEmbeddedContentView::NodeAtPoint(
    HitTestResult& result,
    const HitTestLocation& location_in_container,
    const LayoutPoint& accumulated_offset,
    HitTestAction action) {
...
CompositingReasons LayoutEmbeddedContentView::AdditionalCompositingReasons()
    const {
...
void LayoutEmbeddedContentView::StyleDidChange(StyleDifference diff,
...
void LayoutEmbeddedContentView::UpdateLayout() {
  DCHECK(NeedsLayout());
...
void LayoutEmbeddedContentView::Paint(const PaintInfo& paint_info,
                                      const LayoutPoint& paint_offset) const {
  EmbeddedContentViewPainter(*this).Paint(paint_info, paint_offset);
}

void LayoutEmbeddedContentView::PaintContents(
    const PaintInfo& paint_info,
    const LayoutPoint& paint_offset) const {
  EmbeddedContentViewPainter(*this).PaintContents(paint_info, paint_offset);
}

CursorDirective LayoutEmbeddedContentView::GetCursor(const LayoutPoint& point,
                                                     Cursor& cursor) const {
  if (Plugin()) {
...
LayoutRect LayoutEmbeddedContentView::ReplacedContentRect() const {
...
void LayoutEmbeddedContentView::UpdateOnEmbeddedContentViewChange() {
  EmbeddedContentView* embedded_content_view = GetEmbeddedContentView();
  if (!embedded_content_view)
    return;
...
void LayoutEmbeddedContentView::UpdateGeometry() {
  EmbeddedContentView* embedded_content_view = GetEmbeddedContentView();
  if (!embedded_content_view)
    return;
...
void LayoutEmbeddedContentView::UpdateGeometryInternal(
  ...
  RefPtr<LayoutEmbeddedContentView> protector(this);
  embedded_content_view.SetFrameRect(frame_rect);
}

void LayoutEmbeddedContentView::DeprecatedInvalidatePaintOfSubtrees(
...
bool LayoutEmbeddedContentView::IsThrottledFrameView() const {
  if (LocalFrameView* frame_view = ChildFrameView())
