...
void LayoutListItem::PositionListMarker() {
  if (marker_ && marker_->Parent() && marker_->Parent()->IsBox() &&
    ...
    if (Style()->IsLeftToRightDirection()) {
      marker_logical_left = marker_->LineOffset() - line_offset - PaddingStart() -
                          BorderStart() + marker_->MarginStart();
      marker_->InlineBoxWrapper()->MoveInInlineDirection(marker_logical_left -
    ...
    if (adjust_overflow) {
      ...
      } while (o != this && propagate_visual_overflow && propagate_layout_overflow);
    }
  }
}
