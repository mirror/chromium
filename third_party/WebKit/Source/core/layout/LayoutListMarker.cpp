...
LayoutSize LayoutListMarker::ImageBulletSize() const {
  ...
  // marker box.
  LayoutUnit bullet_width = font_data->GetFontMetrics().Ascent() / LayoutUnit(2);
...
void LayoutListMarker::UpdateContent() {
  ...
  switch (GetListStyleCategory()) {
    ...
    case ListStyleCategory::kLanguage:
      text_ =
          ListMarkerText::GetText(Style()->ListStyleType(), list_item_->Value());
      break;
  }
}
...
void LayoutListMarker::ComputePreferredLogicalWidths() {
  ...
  switch (GetListStyleCategory()) {
    ...
    case ListStyleCategory::kSymbol:
      logical_width =
          LayoutUnit((font_data->GetFontMetrics().Ascent() * 2 / 3 + 1) / 2 + 2);
      break;
    case ListStyleCategory::kLanguage:
...
IntRect LayoutListMarker::GetRelativeMarkerRect() const {
  ...
  switch (GetListStyleCategory()) {
    ...
    case ListStyleCategory::kSymbol: {
      ...
      relative_rect = IntRect(1, 3 * (ascent - ascent * 2 / 3) / 2, bullet_width,
                             bullet_width);
    } break;
    case ListStyleCategory::kLanguage:
