...
TEST_F(TextAutosizerTest, SimpleParagraph) {
  ...
  EXPECT_FLOAT_EQ(40.f, autosized->GetLayoutObject()->Style()->ComputedFontSize());
}
...
TEST_F(TextAutosizerTest, ParagraphWithChangingTextSizeAdjustment) {
  ...
  EXPECT_FLOAT_EQ(16.f,
                  autosized_div->GetLayoutObject()->Style()->SpecifiedFontSize());
  EXPECT_FLOAT_EQ(40.f,
                  autosized_div->GetLayoutObject()->Style()->ComputedFontSize());
...
TEST_F(TextAutosizerTest, NegativeTextSizeAdjustment) {
  ...
  EXPECT_FLOAT_EQ(16.f, text_size_adjust_negative->Style()->SpecifiedFontSize());
...
TEST_F(TextAutosizerTest, AccessibilityFontScaleFactor) {
  ...
  EXPECT_FLOAT_EQ(60.f, autosized->GetLayoutObject()->Style()->ComputedFontSize());
}
...
TEST_F(TextAutosizerTest, AccessibilityFontScaleFactorWithTextSizeAdjustNone) {
  ...
  EXPECT_FLOAT_EQ(24.f, autosized->GetLayoutObject()->Style()->ComputedFontSize());
  ...
  EXPECT_FLOAT_EQ(16.f,
                  not_autosized->GetLayoutObject()->Style()->SpecifiedFontSize());
  // specified font-size = 16px.
  EXPECT_FLOAT_EQ(16.f,
                  not_autosized->GetLayoutObject()->Style()->ComputedFontSize());
}
...
TEST_F(TextAutosizerTest, ChangingAccessibilityFontScaleFactor) {
  ...
  EXPECT_FLOAT_EQ(80.f, autosized->GetLayoutObject()->Style()->ComputedFontSize());
}
...
TEST_F(TextAutosizerTest, TextSizeAdjustDoesNotDisableAccessibility) {
  ...
  SetBodyInnerHTML(
      ...
      "  culpa qui officia deserunt mollit anim id est laborum."
      "</div>");
  Element* text_size_adjust_none = GetDocument().GetElementById("textSizeAdjustNone");
  EXPECT_FLOAT_EQ(
      16.f, text_size_adjust_none->GetLayoutObject()->Style()->SpecifiedFontSize());
  // 1.5 * (specified font-size = 16px) = 24px.
  EXPECT_FLOAT_EQ(
      24.f, text_size_adjust_none->GetLayoutObject()->Style()->ComputedFontSize());

  Element* text_size_adjust_double =
      GetDocument().GetElementById("textSizeAdjustDouble");
  EXPECT_FLOAT_EQ(
      16.f, text_size_adjust_double->GetLayoutObject()->Style()->SpecifiedFontSize());
  // 1.5 * (specified font-size = 16px) * (text size adjustment = 2) = 48px.
  EXPECT_FLOAT_EQ(
      48.f, text_size_adjust_double->GetLayoutObject()->Style()->ComputedFontSize());
  ...
  EXPECT_FLOAT_EQ(
      32.f, text_size_adjust_none->GetLayoutObject()->Style()->ComputedFontSize());

  EXPECT_FLOAT_EQ(
      16.f, text_size_adjust_double->GetLayoutObject()->Style()->SpecifiedFontSize());
  // 2.0 * (specified font-size = 16px) * (text size adjustment = 2) = 64px.
  EXPECT_FLOAT_EQ(
      64.f, text_size_adjust_double->GetLayoutObject()->Style()->ComputedFontSize());
}

// https://crbug.com/646237
TEST_F(TextAutosizerTest, DISABLED_TextSizeAdjustWithoutNeedingAutosizing) {
  GetDocument().GetSettings()->SetTextAutosizingWindowSizeOverride(IntSize(800, 600));
  SetBodyInnerHTML(
      "<style>"
      "  html { font-size: 16px; }"
      "  body { width: 800px; margin: 0; overflow-y: hidden; }"
...
TEST_F(TextAutosizerTest, AutosizeInnerContentOfRuby) {
  ...
  // (specified font-size = 16px) * (viewport width = 800px) /
  // (window width = 320px) = 40px.
  EXPECT_FLOAT_EQ(40.f, ruby_block->GetLayoutObject()->Style()->ComputedFontSize());
}

TEST_F(TextAutosizerTest, ResizeAndGlyphOverflowChanged) {
  GetDocument().GetSettings()->SetTextAutosizingWindowSizeOverride(IntSize(360, 640));
  ...
  GetDocument().GetSettings()->SetTextAutosizingWindowSizeOverride(IntSize(640, 360));
  GetDocument().View()->UpdateAllLifecyclePhases();
