...
const AtomicString& HitTestResult::AltDisplayString() const {
  ...
  if (isHTMLImageElement(*inner_node_or_image_map_image)) {
    HTMLImageElement& image = toHTMLImageElement(*inner_node_or_image_map_image);
    return image.getAttribute(altAttr);
  }

  if (isHTMLInputElement(*inner_node_or_image_map_image)) {
    HTMLInputElement& input = toHTMLInputElement(*inner_node_or_image_map_image);
    return input.Alt();
  }
...
Image* HitTestResult::GetImage() const {
  ...
  LayoutObject* layout_object = inner_node_or_image_map_image->GetLayoutObject();
...
ListBasedHitTestBehavior HitTestResult::AddNodeToListBasedTestResult(
  ...
  return rect.Contains(LayoutRect(location.BoundingBox())) ? kStopHitTesting
                                                           : kContinueHitTesting;
}
...
void HitTestResult::ResolveRectBasedTest(
  ...
  DCHECK(hit_test_location_.ContainsPoint(FloatPoint(resolved_point_in_main_frame)));
  ...
  resolved_inner_node->GetLayoutObject()->UpdateHitTestResult(*this, LayoutPoint());
  DCHECK(!IsRectBasedTest());
}
