...
bool LayoutImage::ForegroundIsKnownToBeOpaqueInRect(const LayoutRect& local_rect,
                                                    unsigned) const {
  ...
  if ((background_clip == kBorderFillBox || background_clip == kPaddingFillBox) &&
      Style()->HasPadding())
    return false;
...
bool LayoutImage::NodeAtPoint(HitTestResult& result,
  ...
  bool inside = LayoutReplaced::NodeAtPoint(temp_result, location_in_container,
...
void LayoutImage::ComputeIntrinsicSizingInfo(
  ...
  if (intrinsic_sizing_info.size.IsEmpty() &&
    ...
    if (containing_block->IsBox()) {
      ...
      intrinsic_sizing_info.size.SetWidth(box->AvailableLogicalWidth().ToFloat());
