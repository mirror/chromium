...
class CompositedLayerMappingTest
 ...
 protected:
  ...
  IntRect ComputeInterestRect(
    ...
    return composited_layer_mapping->ComputeInterestRect(graphics_layer,
...
TEST_P(CompositedLayerMappingTest, ClippingMaskLayer) {
  ...
  SetBodyInnerHTML("<video id='video' src='x' style='" + style_without_clipping +
                   "'></video>");
...
TEST_P(CompositedLayerMappingTest,
  ...
  EXPECT_FALSE(InterestRectChangedEnoughToRepaint(previous_interest_rect,
                                                  new_interest_rect, layer_size));
  new_interest_rect.Move(0, 512);
  EXPECT_FALSE(InterestRectChangedEnoughToRepaint(previous_interest_rect,
                                                  new_interest_rect, layer_size));
  new_interest_rect.Move(1, 0);
  EXPECT_TRUE(InterestRectChangedEnoughToRepaint(previous_interest_rect,
                                                 new_interest_rect, layer_size));
...
TEST_P(CompositedLayerMappingTest,
  ...
  negative_composited_child->parentNode()->RemoveChild(negative_composited_child);
  GetDocument().View()->UpdateAllLifecyclePhases();
...
TEST_P(CompositedLayerMappingTest, AncestorClippingMaskLayerSiblingUpdates) {
  ...
  GetDocument().View()->UpdateAllLifecyclePhases();
  child1_paint_layer = ToLayoutBoxModelObject(child1->GetLayoutObject())->Layer();
  ASSERT_TRUE(child1_paint_layer);
  ...
  child2_paint_layer = ToLayoutBoxModelObject(child2->GetLayoutObject())->Layer();
  ASSERT_TRUE(child2_paint_layer);
...
TEST_P(CompositedLayerMappingTest, StickyPositionEnclosingLayersContentOffset) {
  ...
  CompositedLayerMapping* sticky_mapping = sticky_layer->GetCompositedLayerMapping();
