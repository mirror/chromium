...
static bool OffsetRangeSelectsLinebreak(unsigned start,
                                        unsigned end,
                                        InlineTextBox* prev,
                                        InlineTextBox* next) {
...
void LayoutText::AbsoluteQuadsForRange(Vector<FloatQuad>& quads,
  ...
  for (InlineTextBox* box : InlineTextBoxesOf(*this)) {
    ...
    if (start <= box->Start() && box_end <= end) {
      ...
      // Find the previous and next text boxes, in case we may need to decide if
      // a caret should be printed at their edge(s) later.
      if (start >= box_end) {
  ...
  // TODO(xiaochengh): What if the text node has no text box at all? Example:
  // <span>foo </span>^ |<span>bar</span>
  // In this case, we should still put a caret at "|bar", but the text box is
  // at the next text node, not this one.
  ...
  if (OffsetRangeSelectsLinebreak(start, end, prev_box, next_box)) {
    FloatRect upstream =
        LocalQuadForTextBox(prev_box, prev_box->Start() + prev_box->Len(), end);
    DCHECK(!upstream.IsZero());
    quads.push_back(LocalToAbsoluteQuad(upstream));

    FloatRect downstream =
        LocalQuadForTextBox(next_box, start, next_box->Start());
    DCHECK(!downstream.IsZero());
    quads.push_back(LocalToAbsoluteQuad(downstream));
    return;
  }
