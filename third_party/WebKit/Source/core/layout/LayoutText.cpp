...
void LayoutText::ComputePreferredLogicalWidths(
  ...
  contains_only_whitespace_or_nbsp_ =
      static_cast<unsigned>(OnlyWhitespace::kYes);
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (c == kNewlineCharacter) {
      ...
      is_space = false;
      contains_only_whitespace_or_nbsp_ =
          static_cast<unsigned>(OnlyWhitespace::kNo);
    }
...
OnlyWhitespace LayoutText::ContainsOnlyWhitespaceOrNbsp() const {
  return PreferredLogicalWidthsDirty()
             ? OnlyWhitespace::kUnknown
             : static_cast<OnlyWhitespace>(contains_only_whitespace_or_nbsp_);
}
