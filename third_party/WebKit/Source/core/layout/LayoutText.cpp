...
void LayoutText::AbsoluteRectsForRange(Vector<IntRect>& rects,
  ...
  for (InlineTextBox* box = FirstTextBox(); box; box = box->NextTextBox()) {
    ...
    if (start <= box->Start() && box->end() < end) {
      ...
      FloatRect rect = LocalQuadForTextBox(box, start, end, use_selection_height);
      if (!rect.IsZero()) {
        if (!has_checked_box_in_range) {
          has_checked_box_in_range = true;
          rects.Clear();
...
static IntRect EllipsisRectForBox(InlineTextBox* box,
  ...
  if (EllipsisBox* ellipsis = box->Root().GetEllipsisBox()) {
    ...
    int ellipsis_end_position = std::min<int>(end_pos - box->Start(), box->Len());
...
void LayoutText::AbsoluteQuadsForRange(Vector<FloatQuad>& quads,
  ...
  const unsigned caret_min_offset = static_cast<unsigned>(this->CaretMinOffset());
  const unsigned caret_max_offset = static_cast<unsigned>(this->CaretMaxOffset());
...
LayoutRect LayoutText::LocalCaretRect(InlineBox* inline_box,
  ...
  LayoutUnit caret_width_right_of_offset = caret_width - caret_width_left_of_offset;
...
ALWAYS_INLINE float LayoutText::WidthFromFont(
  ...
  float result = f.Width(run, fallback_fonts,
...
void LayoutText::TrimmedPrefWidths(LayoutUnit lead_width_layout_unit,
  ...
  if (!len || (strip_front_spaces && GetText().Impl()->ContainsOnlyWhitespace())) {
  ...
  if (has_break) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (linelen) {
        last_line_max_width = LayoutUnit(
            WidthFromFont(f, i, linelen, lead_width, last_line_max_width.ToFloat(),
...
static float MinWordFragmentWidthForBreakAll(LayoutText* layout_text,
  ...
  for (int i = start; i < end;) {
    ...
    if (break_all_or_break_word == EWordBreak::kBreakAllWordBreak) {
      break_iterator.IsBreakable(i + 1, next_breakable, LineBreakType::kBreakAll);
...
static float MaxWordFragmentWidth(LayoutText* layout_text,
  ...
  float minimum_fragment_width_to_consider = Hyphenation::MinimumPrefixWidth(font);
...
void LayoutText::ComputePreferredLogicalWidths(
  ...
  int len = TextLength();
  LazyLineBreakIterator break_iterator(text_,
                                      LocaleForLineBreakIterator(style_to_use));
  ...
  for (int i = 0; i < len; i++) {
    ...
    bool is_breakable_location = is_newline || (is_space && style_to_use.AutoWrap());
    if (!i)
    ...
    if (c == kSoftHyphenCharacter && !disable_soft_hyphen) {
      curr_max_width += WidthFromFont(f, last_word_boundary, i - last_word_boundary,
    ...
    bool has_break = break_iterator.IsBreakable(i, next_breakable, line_break_type);
    ...
    if (word_len) {
      ...
      if (word_trailing_space_width && is_space) {
        ...
        w = WidthFromFont(f, i, word_len, lead_width, curr_max_width, text_direction,
                          &fallback_fonts, &glyph_bounds);
        if (c == kSoftHyphenCharacter && !disable_soft_hyphen)
      ...
      if (break_all_or_break_word != EWordBreak::kNormalWordBreak) {
        ...
        curr_min_width =
            MinWordFragmentWidthForBreakAll(this, style_to_use, f, text_direction,
                                            i, word_len, break_all_or_break_word);
      } else {
        curr_min_width += w;
      }
      if (between_words) {
      ...
      if (is_newline) {
        ...
        needs_word_spacing = is_space && !previous_character_is_space && i == len - 1;
      }
      DCHECK_EQ(last_word_boundary, i);
      last_word_boundary++;
    }
...
bool LayoutText::ContainsOnlyWhitespace(unsigned from, unsigned len) const {
  ...
  for (curr_pos = from;
       curr_pos < from + len &&
       (text[curr_pos] == kNewlineCharacter || text[curr_pos] == kSpaceCharacter ||
...
UChar LayoutText::PreviousCharacter() const {
  ...
  if (previous_text && previous_text->IsText()) {
    if (StringImpl* previous_string = ToLayoutText(previous_text)->GetText().Impl())
...
LayoutRect LayoutText::VisualOverflowRect() const {
  ...
  for (InlineTextBox* curr = FirstTextBox(); curr; curr = curr->NextTextBox()) {
    ...
    logical_left_side = std::min(logical_left_side, logical_visual_overflow.X());
    logical_right_side = std::max(logical_right_side, logical_visual_overflow.MaxX());
  }
  ...
  LayoutRect rect(logical_left_side, logical_top, logical_width, logical_height);
  if (!Style()->IsHorizontalWritingMode())
