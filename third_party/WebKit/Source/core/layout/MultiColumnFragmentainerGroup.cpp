...
LayoutSize MultiColumnFragmentainerGroup::FlowThreadTranslationAtOffset(
  ...
  unsigned column_index = offset_in_flow_thread >= LogicalBottomInFlowThread()
                             ? ActualColumnCount() - 1
  ...
  if (LayoutMultiColumnFlowThread* enclosing_flow_thread =
    ...
    // fragmentainer group.
    enclosing_translation = enclosing_flow_thread->FlowThreadTranslationAtOffset(
        BlockOffsetInEnclosingFragmentationContext(),
        LayoutBox::kAssociateWithLatterPage, mode);
...
LayoutRect MultiColumnFragmentainerGroup::FragmentsBoundingBox(
  ...
  LayoutUnit bounding_box_logical_top = is_horizontal_writing_mode
                                         ? flipped_bounding_box_in_flow_thread.Y()
  ...
  ColumnIntervalForBlockRangeInFlowThread(
      bounding_box_logical_top, bounding_box_logical_bottom, start_column, end_column);
  ...
  start_column_rect.Move(FlowThreadTranslationAtOffset(
      LogicalTopInFlowThreadAt(start_column), LayoutBox::kAssociateWithLatterPage,
...
LayoutRect MultiColumnFragmentainerGroup::ColumnRectAt(
  ...
  LayoutRect column_rect(column_logical_left, column_logical_top, column_logical_width,
                        column_logical_height);
  if (!column_set_.IsHorizontalWritingMode())
...
LayoutRect MultiColumnFragmentainerGroup::FlowThreadPortionOverflowRectAt(
  ...
  bool is_leftmost_column = is_ltr ? is_first_column_in_row : is_last_column_in_row;
  bool is_rightmost_column = is_ltr ? is_last_column_in_row : is_first_column_in_row;

  LayoutRect portion_rect = FlowThreadPortionRectAt(column_index);
  bool is_first_column_in_multicol_container =
      is_first_column_in_row && this == &column_set_.FirstFragmentainerGroup() &&
...
unsigned MultiColumnFragmentainerGroup::ColumnIndexAtVisualPoint(
  ...
  LayoutUnit offset_in_column_progression_direction =
      is_horizontal_writing_mode == is_column_progression_inline ? visual_point.X()
                                                           : visual_point.Y();
  if (!column_set_.Style()->IsLeftToRightDirection() &&
...
void MultiColumnFragmentainerGroup::ColumnIntervalForBlockRangeInFlowThread(
  ...
  logical_bottom_in_flow_thread =
      std::min(logical_bottom_in_flow_thread, this->LogicalBottomInFlowThread());
