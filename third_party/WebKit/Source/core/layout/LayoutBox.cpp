...
void LayoutBox::StyleWillChange(StyleDifference diff,
  ...
  if (old_style) {
    ...
    // substantially.
    if ((diff.NeedsFullPaintInvalidation() || diff.NeedsLayout()) && GetNode() &&
...
void LayoutBox::UpdateBackgroundAttachmentFixedStatusAfterStyleChange() {
  ...
  bool is_background_attachment_fixed_object = !IsDocumentElement() &&
...
void LayoutBox::UpdateShapeOutsideInfoAfterStyleChange(
  ...
  Length old_shape_margin =
      old_style ? old_style->ShapeMargin() : ComputedStyle::InitialShapeMargin();
...
void LayoutBox::UpdateScrollSnapMappingAfterStyleChange(
  ...
  bool allows_snap_container = GetNode() != GetDocument().ViewportDefiningElement();
...
void LayoutBox::ScrollRectToVisible(const LayoutRect& rect,
  ...
  if (ContainingBlock()) {
    ...
    restricted_by_line_clamp = !ContainingBlock()->Style()->LineClamp().IsNone();
  }
  ...
  if (HasOverflowClip() && !restricted_by_line_clamp) {
    ...
    new_rect = GetScrollableArea()->ScrollIntoView(rect_to_scroll, align_x, align_y,
                                                  scroll_type);
    if (new_rect.IsEmpty())
      return;
    ...
    if (FrameView* frame_view = this->GetFrameView()) {
      ...
      if (!IsDisallowedAutoscroll(owner_element, frame_view)) {
        if (make_visible_in_visual_viewport) {
          frame_view->GetScrollableArea()->ScrollIntoView(rect_to_scroll, align_x,
                                                         align_y, scroll_type);
        } else {
...
LayoutUnit LayoutBox::ConstrainContentBoxLogicalHeightByMinMax(
  ...
  if (!style_to_use.LogicalMaxHeight().IsMaxSizeNone()) {
    if (style_to_use.LogicalMaxHeight().GetType() == kPercent &&
      ...
      logical_height =
          std::min(logical_height, ValueForLength(style_to_use.LogicalMaxHeight(),
  ...
  if (style_to_use.LogicalMinHeight().GetType() == kPercent &&
      style_to_use.LogicalHeight().GetType() == kPercent) {
    LayoutUnit available_logical_height(logical_height /
                                      style_to_use.LogicalHeight().Value() * 100);
    logical_height = std::max(
        logical_height,
        ValueForLength(style_to_use.LogicalMinHeight(), available_logical_height));
  } else {
    logical_height = std::max(
        logical_height,
...
void LayoutBox::SetLocationAndUpdateOverflowControlsIfNeeded(
  ...
  if (HasOverflowClip()) {
    IntSize old_pixel_snapped_border_rect_size = PixelSnappedBorderBoxRect().size();
    SetLocation(location);
    if (PixelSnappedBorderBoxRect().size() != old_pixel_snapped_border_rect_size)
...
IntSize LayoutBox::CalculateAutoscrollDirection(
  ...
  else if (window_autoscroll_point.X() > window_box.MaxX() - kAutoscrollBeltSize)
    window_autoscroll_point.Move(kAutoscrollBeltSize, 0);

  if (window_autoscroll_point.Y() < window_box.Y() + kAutoscrollBeltSize)
    window_autoscroll_point.Move(0, -kAutoscrollBeltSize);
  else if (window_autoscroll_point.Y() > window_box.MaxY() - kAutoscrollBeltSize)
    window_autoscroll_point.Move(0, kAutoscrollBeltSize);
  ...
  if (HasOverflowClip() && !restricted_by_line_clamp) {
    ...
    if (!remaining_scroll_offset.IsZero() && Parent()) {
      ...
      if (frame && frame->GetPage())
        frame->GetPage()->GetAutoscrollController().UpdateAutoscrollLayoutObject();
    }
...
void LayoutBox::ComputeIntrinsicLogicalWidths(
  ...
  min_logical_width = MinPreferredLogicalWidth() - BorderAndPaddingLogicalWidth();
  max_logical_width = MaxPreferredLogicalWidth() - BorderAndPaddingLogicalWidth();
}
...
LayoutUnit LayoutBox::ExtraInlineOffset() const {
  return g_extra_inline_offset_map ? g_extra_inline_offset_map->at(this) : LayoutUnit();
}

LayoutUnit LayoutBox::ExtraBlockOffset() const {
  return g_extra_block_offset_map ? g_extra_block_offset_map->at(this) : LayoutUnit();
}
...
bool LayoutBox::NodeAtPoint(HitTestResult& result,
  ...
  if (should_hit_test_self && VisibleToHitTestRequest(result.GetHitTestRequest())) {
    LayoutRect bounds_rect(adjusted_location, size());
...
static bool IsCandidateForOpaquenessTest(const LayoutBox& child_box) {
  ...
  if (PaintLayer* child_layer = child_box.Layer()) {
    ...
    if (child_layer->HasTransformRelatedProperty() ||
        child_layer->IsTransparent() || child_layer->HasFilterInducingProperty())
      return false;
...
void LayoutBox::ImageChanged(WrappedImagePtr image, const IntRect*) {
  ...
  if (!GetFrameView()->IsInPerformLayout() && IsFloating() && shape_outside_value &&
      shape_outside_value->GetImage() &&
...
bool LayoutBox::IntersectsVisibleViewport() const {
  ...
  while (!layout_view->GetFrame()->OwnerLayoutItem().IsNull())
    layout_view =
        LayoutAPIShim::LayoutObjectFrom(layout_view->GetFrame()->OwnerLayoutItem())
            ->View();
...
PaintInvalidationReason LayoutBox::InvalidatePaintIfNeeded(
  ...
  return LayoutBoxModelObject::InvalidatePaintIfNeeded(paint_invalidation_state);
}
...
LayoutRect LayoutBox::OverflowClipRect(
  ...
  clip_rect.SetSize(clip_rect.size() - LayoutSize(BorderLeft() + BorderRight(),
...
void LayoutBox::ExcludeScrollbars(
  ...
  if (PaintLayerScrollableArea* scrollable_area = this->GetScrollableArea()) {
    if (ShouldPlaceBlockDirectionScrollbarOnLogicalLeft()) {
      rect.Move(
          scrollable_area->VerticalScrollbarWidth(overlay_scrollbar_clip_behavior),
          0);
    }
    rect.Contract(
        scrollable_area->VerticalScrollbarWidth(overlay_scrollbar_clip_behavior),
...
LayoutUnit LayoutBox::ShrinkLogicalWidthToAvoidFloats(
  ...
  LayoutUnit end_offset_for_line =
      cb->EndOffsetForLine(logical_top_position, kDoNotIndentText, logical_height);
  ...
  if (start_offset_for_content == start_offset_for_line &&
    ...
    return cb->AvailableLogicalWidthForLine(logical_top_position, kDoNotIndentText,
                                            logical_height) -
...
LayoutSize LayoutBox::OffsetFromContainer(const LayoutObject* o) const {
  ...
  if (Style()->GetPosition() == EPosition::kAbsolute && o->IsInFlowPositioned() &&
      o->IsLayoutInline())
...
void LayoutBox::InflateVisualRectForFilterUnderContainer(
  ...
  for (LayoutObject* parent = this->Parent(); parent && parent != container;
    ...
    if (parent->IsBox()) {
      ...
      LayoutSize parent_offset = parent->OffsetFromAncestorContainer(&container);
...
bool LayoutBox::MapToVisualRectInAncestorSpaceInternal(
  ...
  if (skip_info.AncestorSkipped()) {
    ...
    if (ancestor->Style()->GetPosition() != EPosition::kFixed &&
      ...
      transform_state.Move(ToLayoutView(container)->OffsetForFixedPosition(true),
                          accumulation);
    }
    return true;
  }
...
void LayoutBox::ComputeLogicalWidth(
  ...
  const ComputedStyle& style_to_use = StyleRef();
  Length logical_width_length = treat_as_replaced
                                  ? Length(ComputeReplacedLogicalWidth(), kFixed)
  ...
  if (IsInline() && !IsInlineBlockOrInlineTable()) {
    ...
    computed_values.margins_.start_ =
        MinimumValueForLength(style_to_use.MarginStart(), container_logical_width);
    computed_values.margins_.end_ =
        MinimumValueForLength(style_to_use.MarginEnd(), container_logical_width);
  ...
  if (treat_as_replaced) {
    computed_values.extent_ =
        LayoutUnit(logical_width_length.Value()) + BorderAndPaddingLogicalWidth();
  } else {
    LayoutUnit preferred_width =
        ComputeLogicalWidthUsing(kMainOrPreferredSize, style_to_use.LogicalWidth(),
  ...
  if (style_to_use.TextAutosizingMultiplier() != 1 &&
    ...
    if (parent_node &&
        (isHTMLOListElement(*parent_node) || isHTMLUListElement(*parent_node))) {
...
LayoutUnit LayoutBox::ComputeIntrinsicLogicalWidthUsing(
  ...
  if (logical_width_length.GetType() == kFitContent) {
    ...
    return std::max(
        min_logical_width,
        std::min(max_logical_width, FillAvailableMeasure(available_logical_width)));
  }
...
DISABLE_CFI_PERF
LayoutUnit LayoutBox::ComputeLogicalWidthUsing(SizeType width_type,
...
bool LayoutBox::AutoWidthShouldFitContent() const {
  return GetNode() &&
         (isHTMLInputElement(*GetNode()) || isHTMLSelectElement(*GetNode()) ||
          isHTMLButtonElement(*GetNode()) || isHTMLTextAreaElement(*GetNode()) ||
          (isHTMLLegendElement(*GetNode()) && !Style()->HasOutOfFlowPosition()));
}
...
void LayoutBox::ComputeMarginsForDirection(MarginDirection flow_direction,
  ...
  if (margin_box_width < available_width) {
    ...
    if ((margin_start_length.IsAuto() && margin_end_length.IsAuto()) ||
      ...
      // match them here.
      LayoutUnit centered_margin_box_start = std::max(
          LayoutUnit(),
          (available_width - child_width - margin_start_width - margin_end_width) /
              2);
...
static inline Length HeightForDocumentElement(const Document& document) {
  return document.documentElement()->GetLayoutObject()->Style()->LogicalHeight();
}
...
void LayoutBox::ComputeLogicalHeight(
  ...
  bool paginated_content_needs_base_height =
      ...
      (IsDocumentElement() ||
       (IsBody() && HeightForDocumentElement(GetDocument()).IsPercentOrCalc())) &&
      !IsInline();
  if (StretchesToViewport() || paginated_content_needs_base_height) {
    ...
    if (IsDocumentElement()) {
      ...
      computed_values.extent_ = std::max(computed_values.extent_,
                                         visible_height - margins_borders_padding);
    }
  }
}
...
LayoutUnit LayoutBox::ComputePercentageLogicalHeight(
  ...
  while (!cb->IsLayoutView() &&
    ...
    if (cb->IsBody() || cb->IsDocumentElement())
      root_margin_border_padding_height += cb->MarginBefore() + cb->MarginAfter() +
  ...
  bool subtract_border_and_padding =
      IsTable() || (cb->IsTableCell() && !skipped_auto_height_containing_block &&
...
LayoutUnit LayoutBox::ComputeReplacedLogicalWidthRespectingMinMaxWidth(
    LayoutUnit logical_width,
    ShouldComputePreferred should_compute_preferred) const {
  LayoutUnit min_logical_width = (should_compute_preferred == kComputePreferred &&
                                Style()->LogicalMinWidth().IsPercentOrCalc())
                                   ? logical_width
                                   : ComputeReplacedLogicalWidthUsing(
                                         kMinSize, Style()->LogicalMinWidth());
  LayoutUnit max_logical_width =
...
LayoutUnit LayoutBox::ComputeReplacedLogicalWidthUsing(
  ...
  switch (logical_width.GetType()) {
    ...
    case kCalculated: {
      ...
      Length container_logical_width = ContainingBlock()->Style()->LogicalWidth();
...
LayoutUnit LayoutBox::ComputeReplacedLogicalHeightRespectingMinMaxHeight(
  ...
  if (!LogicalHeightComputesAsNone(kMinSize))
    min_logical_height =
        ComputeReplacedLogicalHeightUsing(kMinSize, Style()->LogicalMinHeight());
  LayoutUnit max_logical_height = logical_height;
  if (!LogicalHeightComputesAsNone(kMaxSize))
    max_logical_height =
        ComputeReplacedLogicalHeightUsing(kMaxSize, Style()->LogicalMaxHeight());
    ...
    case kCalculated: {
      ...
      if (cb->IsOutOfFlowPositioned() && cb->Style()->Height().IsAuto() &&
        ...
        LayoutUnit new_height =
            block->AdjustContentBoxLogicalHeightForBoxSizing(new_content_height);
...
void LayoutBox::ComputeAndSetBlockDirectionMargins(
  ...
  ComputeMarginsForDirection(
      kBlockDirection, containing_block, ContainingBlockLogicalWidthForContent(),
      LogicalHeight(), margin_before, margin_after,
      Style()->MarginBeforeUsing(containing_block->Style()),
  ...
  if (Style()->GetPosition() == EPosition::kFixed &&
    ...
    if (FrameView* frame_view = view->GetFrameView()) {
      ...
      LayoutSize viewport_size(
          frame_view->LayoutViewportScrollableArea()->ExcludeScrollbars(
              frame_view->FrameRect().size()));
      return containing_block->IsHorizontalWritingMode() ? viewport_size.Height()
                                                        : viewport_size.Width();
    }
  }
...
static LayoutUnit AccumulateStaticOffsetForFlowThread(
  ...
  ToLayoutFlowThread(layout_box).FlowThreadToContainingCoordinateSpace(
      block_position, inline_position);
...
void LayoutBox::ComputePositionedLogicalWidth(
  ...
  if (!Style()->LogicalMaxWidth().IsMaxSizeNone()) {
    ...
    ComputePositionedLogicalWidthUsing(
        kMaxSize, Style()->LogicalMaxWidth(), container_block, container_direction,
        container_logical_width, borders_plus_padding, logical_left_length,
        logical_right_length, margin_logical_left, margin_logical_right, max_values);
  ...
  if (!Style()->LogicalMinWidth().IsZero() ||
    ...
    ComputePositionedLogicalWidthUsing(
        kMinSize, Style()->LogicalMinWidth(), container_block, container_direction,
        container_logical_width, borders_plus_padding, logical_left_length,
        logical_right_length, margin_logical_left, margin_logical_right, min_values);
...
LayoutUnit LayoutBox::ShrinkToFitLogicalWidth(
  ...
  return std::min(std::max(preferred_min_logical_width, available_logical_width),
...
void LayoutBox::ComputePositionedLogicalWidthUsing(
  ...
  DCHECK(width_size_type == kMinSize || width_size_type == kMainOrPreferredSize ||
         !logical_width.IsAuto());
  if (width_size_type == kMinSize && logical_width.IsAuto())
    logical_width_value = LayoutUnit();
  else if (logical_width.IsIntrinsic())
    logical_width_value =
        ComputeIntrinsicLogicalWidthUsing(logical_width, container_logical_width,
  ...
  LayoutUnit& margin_logical_left_value = Style()->IsLeftToRightDirection()
                                           ? computed_values.margins_.start_
                                           : computed_values.margins_.end_;
  LayoutUnit& margin_logical_right_value = Style()->IsLeftToRightDirection()
                                            ? computed_values.margins_.end_
                                            : computed_values.margins_.start_;
  if (!logical_left_is_auto && !logical_width_is_auto && !logical_right_is_auto) {
    ...
    const LayoutUnit available_space =
        container_logical_width - (logical_left_value + computed_values.extent_ +
                                 logical_right_value + borders_plus_padding);
    ...
    margin_logical_left_value =
        MinimumValueForLength(margin_logical_left, container_relative_logical_width);
    margin_logical_right_value = MinimumValueForLength(
        margin_logical_right, container_relative_logical_width);

    const LayoutUnit available_space =
        container_logical_width -
        (margin_logical_left_value + margin_logical_right_value + logical_left_value +
         logical_right_value + borders_plus_padding);
...
void LayoutBox::ComputeBlockStaticDistance(
  ...
  logical_top.SetValue(kFixed, static_logical_top - container_block->BorderBefore());
}
...
void LayoutBox::ComputePositionedLogicalHeight(
  ...
  if (!style_to_use.LogicalMaxHeight().IsMaxSizeNone()) {
    ...
    ComputePositionedLogicalHeightUsing(kMaxSize, style_to_use.LogicalMaxHeight(),
                                        container_block, container_logical_height,
                                        borders_plus_padding, logical_height,
                                        logical_top_length, logical_bottom_length,
                                        margin_before, margin_after, max_values);
  ...
  if (!style_to_use.LogicalMinHeight().IsZero() ||
    ...
    ComputePositionedLogicalHeightUsing(kMinSize, style_to_use.LogicalMinHeight(),
                                        container_block, container_logical_height,
                                        borders_plus_padding, logical_height,
                                        logical_top_length, logical_bottom_length,
                                        margin_before, margin_after, min_values);
...
void LayoutBox::ComputePositionedLogicalHeightUsing(
    ...
    LogicalExtentComputedValues& computed_values) const {
  DCHECK(height_size_type == kMinSize || height_size_type == kMainOrPreferredSize ||
  ...
  if (!logical_top_is_auto && !logical_height_is_auto && !logical_bottom_is_auto) {
    ...
    if (logical_top_is_auto && logical_height_is_auto && !logical_bottom_is_auto) {
      ...
      logical_top_value = available_space -
...
void LayoutBox::UpdateFragmentationInfoForChild(LayoutBox& child) {
  ...
  LayoutUnit space_left =
      PageRemainingLogicalHeightForOffset(logical_top, kAssociateWithLatterPage);
...
LayoutRect LayoutBox::NoOverflowRect() const {
  ...
  LayoutUnit left(
      BorderLeft() +
      (ShouldPlaceBlockDirectionScrollbarOnLogicalLeft() ? scroll_bar_width : 0));
...
LayoutObject* LayoutBox::SplitAnonymousBoxesAroundChild(
  ...
  while (before_child->Parent() != this) {
    ...
    if (box_to_split->SlowFirstChild() != before_child &&
      ...
      LayoutBox* post_box = box_to_split->CreateAnonymousBoxWithSameTypeAs(this);
      ...
      parent_box->VirtualChildren()->InsertChildNode(parent_box, post_box,
                                                    box_to_split->NextSibling());
...
LayoutUnit LayoutBox::CalculatePaginationStrutToFitContent(
  ...
  return strut_to_next_page +
         flow_thread->NextLogicalTopForUnbreakableContent(flow_thread_offset,
...
void LayoutBox::MutableForPainting::
  ...
  rare_data.previous_layout_overflow_rect_ = GetLayoutBox().LayoutOverflowRect();
}
