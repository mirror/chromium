...
void GridTrackSizingAlgorithm::SizeTrackToFitNonSpanningItem(
  ...
  if (track_size.HasMinContentMinTrackBreadth()) {
    ...
    const Length& grid_item_size = is_row_axis
                                       ? grid_item.StyleRef().LogicalWidth()
                                       : grid_item.StyleRef().LogicalHeight();
    const Length& grid_item_min_size =
        is_row_axis ? grid_item.StyleRef().LogicalMinWidth()
    ...
    if (grid_item_size.IsAuto() && grid_item_min_size.IsAuto() &&
        overflow_is_visible && track_size.MaxTrackBreadth().IsLength() &&
        track_size.MaxTrackBreadth().length().IsSpecified()) {
      LayoutUnit max_breadth =
          ValueForLength(track_size.MaxTrackBreadth().length(),
                         AvailableSpace().value_or(LayoutUnit()));
      if (max_breadth < min_size) {
