...
class CORE_EXPORT LayoutBox : public LayoutBoxModelObject {
 public:
  ...
  LayoutUnit MarginRight() const override { return margin_box_outsets_.Right(); }
  ...
  LayoutUnit MarginStart(
      const ComputedStyle* override_style = nullptr) const final {
    const ComputedStyle* style_to_use = override_style ? override_style : Style();
  ...
  IntSize CalculateAutoscrollDirection(const IntPoint& point_in_root_frame) const;
  ...
  class MutableForPainting : public LayoutObject::MutableForPainting {
   public:
    void SavePreviousSize() { GetLayoutBox().previous_size_ = GetLayoutBox().size(); }
    ...
    void ClearPreviousContentBoxSizeAndLayoutOverflowRect() {
      if (!GetLayoutBox().rare_data_)
        return;
      GetLayoutBox().rare_data_->has_previous_content_box_size_and_layout_overflow_rect_ =
          false;
    }
   ...
   protected:
    ...
    LayoutBox& GetLayoutBox() { return static_cast<LayoutBox&>(layout_object_); }
  };
 ...
 protected:
  ...
  virtual bool ForegroundIsKnownToBeOpaqueInRect(const LayoutRect& local_rect,
 ...
 private:
  ...
  virtual void ComputeIntrinsicLogicalWidths(LayoutUnit& min_logical_width,
