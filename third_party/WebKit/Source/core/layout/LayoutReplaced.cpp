...
void LayoutReplaced::IntrinsicSizeChanged() {
  ...
  int scaled_height = static_cast<int>(kDefaultHeight * Style()->EffectiveZoom());
...
FloatSize LayoutReplaced::ConstrainIntrinsicSizeToMinMax(
  ...
  if (!intrinsic_sizing_info.aspect_ratio.IsEmpty() &&
      !intrinsic_sizing_info.size.IsEmpty() && Style()->LogicalWidth().IsAuto() &&
...
void LayoutReplaced::ComputePositionedLogicalWidth(
  ...
  LayoutUnit& margin_logical_right_alias = Style()->IsLeftToRightDirection()
                                            ? computed_values.margins_.end_
                                            : computed_values.margins_.start_;
  ...
  // ---------------------------------------------------------------------------
  // see FIXME 1
  ComputeInlineStaticDistance(logical_left, logical_right, this, container_block,
  ...
  if (margin_logical_left.IsAuto() && margin_logical_right.IsAuto()) {
    ...
    logical_left_value =
        available_space -
        (logical_right_value + margin_logical_left_alias + margin_logical_right_alias);
    ...
    logical_right_value =
        available_space -
        (logical_left_value + margin_logical_left_alias + margin_logical_right_alias);
...
void LayoutReplaced::ComputePositionedLogicalHeight(
  ...
  if (margin_before.IsAuto() && margin_after.IsAuto()) {
    ...
    // Solve for 'top'
    logical_top_value = available_space - (logical_bottom_value + margin_before_alias +
                                        margin_after_alias);
    ...
    margin_before_alias = available_space - (logical_top_value + logical_bottom_value +
                                          margin_after_alias);
    ...
    margin_after_alias = available_space - (logical_top_value + logical_bottom_value +
                                         margin_before_alias);
  } else {
    // Nothing is 'auto', just calculate the values.
    margin_before_alias =
...
LayoutRect LayoutReplaced::ComputeObjectFit(
  ...
  LayoutSize intrinsic_size = overridden_intrinsic_size ? *overridden_intrinsic_size
                                                     : this->IntrinsicSize();
  ...
  switch (object_fit) {
    ...
    case kObjectFitCover:
      final_rect.SetSize(final_rect.size().FitToAspectRatio(
          intrinsic_size, object_fit == kObjectFitCover ? kAspectRatioFitGrow
  ...
  LayoutUnit y_offset = MinimumValueForLength(
      Style()->ObjectPosition().Y(), content_rect.Height() - final_rect.Height());
...
LayoutUnit LayoutReplaced::ComputeConstrainedLogicalWidth(
  ...
  logical_width = (logical_width -
                  (margin_start + margin_end + (size().Width() - ClientWidth())))
                     .ClampNegativeToZero();
...
void LayoutReplaced::ComputePreferredLogicalWidths() {
  ...
  if (style_to_use.LogicalMaxWidth().IsFixed()) {
    ...
    min_preferred_logical_width_ = std::min(
        min_preferred_logical_width_, AdjustContentBoxLogicalWidthForBoxSizing(
                                        style_to_use.LogicalMaxWidth().Value()));
  }
