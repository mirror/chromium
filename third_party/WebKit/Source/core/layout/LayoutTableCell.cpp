...
void LayoutTableCell::ComputeOverflow(LayoutUnit old_client_after_edge,
  ...
  if ((left && !rtl) || (right && rtl)) {
    if (LayoutTableCell* proceding = Table()->CellProceding(*this)) {
      top = std::max(top, proceding->CollapsedBorderHalfTop(true));
      bottom = std::max(bottom, proceding->CollapsedBorderHalfBottom(true));
    }
  }
  if ((left && rtl) || (right && !rtl)) {
...
CollapsedBorderValue LayoutTableCell::ComputeCollapsedStartBorder() const {
  ...
  LayoutTableCell* cell_proceding = table->CellProceding(*this);
  // We can use the border shared with |cell_before| if it is valid.
  if (cell_proceding && cell_proceding->collapsed_border_values_valid_ &&
      cell_proceding->RowIndex() == RowIndex()) {
    return cell_proceding->GetCollapsedBorderValues()
               ? cell_proceding->GetCollapsedBorderValues()->EndBorder()
  ...
  if (cell_proceding) {
    CollapsedBorderValue cell_before_adjoining_border =
        CollapsedBorderValue(cell_proceding->BorderAdjoiningCellAfter(*this),
                             cell_proceding->ResolveColor(end_color_property),
