...
void SVGTextLayoutEngine::ComputeCurrentFragmentMetrics(
  ...
  current_text_fragment_.glyph_overflow.SetFromBounds(glyph_overflow_bounds, ascent,
                                                    descent, width);
...
void SVGTextLayoutEngine::BeginTextPathLayout(SVGInlineFlowBox* flow_box) {
  ...
  if (!text_path_)
    return;
  text_path_start_offset_ = text_path.CalculateStartOffset(text_path_->length());

  SVGTextPathChunkBuilder text_path_chunk_layout_builder;
  text_path_chunk_layout_builder.ProcessTextChunks(line_layout.line_layout_boxes_);

  text_path_start_offset_ += text_path_chunk_layout_builder.TotalTextAnchorShift();
...
const LayoutSVGInlineText* SVGTextLayoutEngine::CurrentLogicalCharacterMetrics(
  ...
  const Vector<SVGTextMetrics>* metrics_list = &logical_text_node->MetricsList();
...
void SVGTextLayoutEngine::LayoutTextOnLineOrPath(
  ...
  visual_metrics_iterator_.AdvanceToTextStart(text_line_layout, text_box->Start());
  ...
  while (!visual_metrics_iterator_.IsAtEnd() &&
    ...
    UChar32 current_character =
        text_line_layout.CodepointAt(visual_metrics_iterator_.CharacterOffset());
    ...
    if (in_path_layout_) {
      ...
      float text_path_offset = text_path_current_offset_ + scaled_glyph_advance / 2;

      // Move to next character.
      text_path_current_offset_ +=
          scaled_glyph_advance + text_path_spacing_ + spacing * text_path_scaling_;
