...
static SVGPaintDescription RequestPaint(const LayoutObject& object,
  ...
  switch (paint_type) {
    ...
    case SVG_PAINTTYPE_URI_RGBCOLOR:
      color =
          apply_to_fill ? svg_style.FillPaintColor() : svg_style.StrokePaintColor();
      has_color = true;
    default:
      break;
  }
  ...
  if (style.InsideLink() == EInsideLink::kInsideVisitedLink) {
    ...
    SVGPaintType visited_paint_type = apply_to_fill
                                        ? svg_style.VisitedLinkFillPaintType()
    ...
    if (visited_paint_type < SVG_PAINTTYPE_URI_NONE &&
      ...
      const Color& visited_color = apply_to_fill
                                      ? svg_style.VisitedLinkFillPaintColor()
...
SVGPaintServer SVGPaintServer::RequestForLayoutObject(
  ...
  DCHECK(resource_mode == kApplyToFillMode || resource_mode == kApplyToStrokeMode);
...
bool SVGPaintServer::ExistsForLayoutObject(const LayoutObject& layout_object,
                                           const ComputedStyle& style,
                                           LayoutSVGResourceMode resource_mode) {
