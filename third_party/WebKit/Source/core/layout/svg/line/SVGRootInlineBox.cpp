...
static inline void SwapPositioningValuesInTextBoxes(
  ...
  SVGCharacterDataMap& first_character_data_map = first_text_node.CharacterDataMap();
  ...
  SVGCharacterDataMap& last_character_data_map = last_text_node.CharacterDataMap();
...
static inline void ReverseInlineBoxRangeAndValueListsIfNeeded(
  ...
  while (true) {
    ...
    if ((*last)->IsSVGInlineTextBox() && (*first)->IsSVGInlineTextBox()) {
      ...
      // positioned.
      if (first_text_box->Len() == 1 && first_text_box->Len() == last_text_box->Len())
