...
void LayoutSVGRoot::GetLayout() {
  ...
  did_screen_scale_factor_change_ = transform_change == SVGTransformChange::kFull;
  ...
  const bool viewport_may_have_changed = SelfNeedsLayout() || old_size != size();
  ...
  is_layout_size_changed_ = viewport_may_have_changed && svg->HasRelativeLengths();

  SVGLayoutSupport::LayoutChildren(
      FirstChild(), false, did_screen_scale_factor_change_, is_layout_size_changed_);
...
const LayoutObject* LayoutSVGRoot::PushMappingToContainer(
  ...
  return LayoutReplaced::PushMappingToContainer(ancestor_to_stop_at, geometry_map);
}

void LayoutSVGRoot::UpdateCachedBoundaries() {
  SVGLayoutSupport::ComputeContainerBoundingBoxes(
      this, object_bounding_box_, object_bounding_box_valid_, stroke_bounding_box_,
      visual_rect_in_local_svg_coordinates_);
}
  ...
  if (ContentBoxRect().Contains(point_in_border_box) ||
    ...
    if (local_to_parent_transform.IsInvertible()) {
      FloatPoint local_point =
          local_to_parent_transform.Inverse().MapPoint(FloatPoint(point_in_parent));
