...
AffineTransform LayoutSVGShape::NonScalingStrokeTransform() const {
  AffineTransform t = ToSVGGraphicsElement(GetElement())->GetScreenCTM(
      SVGGraphicsElement::kDisallowStyleUpdate);
...
bool LayoutSVGShape::NodeAtFloatPoint(HitTestResult& result,
  ...
  PointerEventsHitRules hit_rules(PointerEventsHitRules::SVG_GEOMETRY_HITTESTING,
                                 ...
                                 Style()->PointerEvents());
  if (NodeAtFloatPointInternal(result.GetHitTestRequest(), local_point, hit_rules)) {
...
bool LayoutSVGShape::NodeAtFloatPointInternal(const HitTestRequest& request,
  ...
  if (is_visible || !hit_rules.require_visible) {
    ...
    if ((hit_rules.can_hit_bounding_box &&
        ...
        (hit_rules.can_hit_fill && (svg_style.HasFill() || !hit_rules.require_fill) &&
  ...
  if (Style()->SvgStyle().HasStroke()) {
    ...
    if (HasNonScalingStroke()) {
      ...
      if (non_scaling_transform.IsInvertible()) {
        Path* use_path = NonScalingStrokePath(path_.get(), non_scaling_transform);
        FloatRect stroke_bounding_rect = use_path->StrokeBoundingRect(stroke_data);
