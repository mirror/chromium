...
void LayoutSVGResourceMarker::GetLayout() {
  DCHECK(NeedsLayout());
  if (is_in_layout_)
    return;
...
float LayoutSVGResourceMarker::Angle() const {
  return toSVGMarkerElement(GetElement())->orientAngle()->CurrentValue()->Value();
}
...
AffineTransform LayoutSVGResourceMarker::MarkerTransformation(
  ...
  transform.Rotate(OrientType() == kSVGMarkerOrientAngle ? Angle() : auto_angle);
