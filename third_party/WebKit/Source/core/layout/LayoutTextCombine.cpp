...
void ScaleHorizontallyAndTranslate(GraphicsContext& context,
  ...
  context.ConcatCTM(AffineTransform(
      scale_x, 0, 0, 1, center_x * (1.0f - scale_x) + offset_x * scale_x, offset_y));
}
...
void LayoutTextCombine::TransformToInlineCoordinates(GraphicsContext& context,
  ...
  float internal_leading = StyleRef().GetFont().PrimaryFont()->InternalLeading();
  ...
  if (scale_x_ >= 1.0f) {
    ...
    ScaleHorizontallyAndTranslate(context, scale_x_, center_x, offset_x, offset_y);
  }
