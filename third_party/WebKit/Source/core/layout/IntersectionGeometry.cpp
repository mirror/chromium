...
LayoutUnit ComputeMargin(const Length& length, LayoutUnit reference_length) {
  if (length.GetType() == kPercent) {
    return LayoutUnit(
        static_cast<int>(reference_length.ToFloat() * length.Percent() / 100.0));
  }
...
void IntersectionGeometry::InitializeRootRect() {
  if (root_->IsLayoutView()) {
    ...
    root_rect_ =
        LayoutRect(ToLayoutBoxModelObject(root_)->BorderBoundingBox());
  }
  ApplyRootMargin();
}
...
void IntersectionGeometry::ApplyRootMargin() {
  ...
  LayoutUnit bottom_margin = ComputeMargin(root_margin_[2], root_rect_.Height());
