...
static String LocalizedUnavailablePluginReplacementText(
  ...
  Locale& locale = node ? ToElement(node)->GetLocale() : Locale::DefaultLocale();
  switch (availability) {
...
void LayoutEmbeddedObject::PaintContents(const PaintInfo& paint_info,
...
void LayoutEmbeddedObject::PaintReplaced(const PaintInfo& paint_info,
                                         const LayoutPoint& paint_offset) const {
