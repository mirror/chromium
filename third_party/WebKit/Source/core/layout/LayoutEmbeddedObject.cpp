...
PaintLayerType LayoutEmbeddedObject::LayerTypeRequired() const {
  // This can't just use LayoutEmbeddedContentView::layerTypeRequired, because
...
void LayoutEmbeddedObject::SetPluginAvailability(
  ...
  // node() is nullptr when LayoutEmbeddedContentView is being destroyed.
  if (GetNode())
...
void LayoutEmbeddedObject::PaintContents(
  ...
  LayoutEmbeddedContentView::PaintContents(paint_info, paint_offset);
}
...
void LayoutEmbeddedObject::Paint(const PaintInfo& paint_info,
  ...
  LayoutEmbeddedContentView::Paint(paint_info, paint_offset);
}
