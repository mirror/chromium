...
void LayoutMenuList::AdjustInnerStyle() {
  ...
  Length padding_start =
      Length(LayoutTheme::GetTheme().PopupInternalPaddingStart(StyleRef()), kFixed);
  ...
  inner_style.SetPaddingTop(
      Length(LayoutTheme::GetTheme().PopupInternalPaddingTop(StyleRef()), kFixed));
...
void LayoutMenuList::DidUpdateActiveOption(HTMLOptionElement* option) {
  ...
  GetDocument().ExistingAXObjectCache()->HandleUpdateActiveMenuOption(this,
                                                                   option_index);
}
