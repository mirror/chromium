...
class CollapsedBorderValue {
 ...
 public:
  ...
  // Suppose |this| and |other| are adjoining borders sharing a corner, this
  // method returns true if |this| should cover the corner. The rule similar to
  // LessThan() except that invisible borders always lose.
  bool CoversCorner(const CollapsedBorderValue& other) const {
    return IsVisible() && (!other.IsVisible() || !LessThan(other));
  }

  // In a table, a border may share a corner with other 3 borders. Returns true
  // if this border wins to cover the corner.
  bool CoversCorner(const CollapsedBorderValue* other1,
                    const CollapsedBorderValue* other2,
                    const CollapsedBorderValue* other3) const {
    return (!other1 || CoversCorner(*other1)) &&
           (!other2 || CoversCorner(*other2)) &&
           (!other3 || CoversCorner(*other3));
  }
