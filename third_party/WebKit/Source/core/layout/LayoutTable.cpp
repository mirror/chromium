...
LayoutTable::LayoutTable(Element* element)
    : LayoutBlock(element),
      ...
      needs_adjust_collapsed_border_corners_(false),
  ...
  for (auto* section = TopSection(); section; section = SectionBelow(section)) {
    for (auto* row = section->FirstRow(); row; row = row->NextRow()) {
      for (auto* cell = row->FirstCell(); cell; cell = cell->NextCell()) {
        ...
        if (needs_adjust_collapsed_border_corners_)
          continue;

        // We may need to adjust corners of collapsed borders if there are
        // different (ignoring color) visible borders.
        for (int i = 0; i < 4; ++i) {
          const auto& border = cell->GetCollapsedBorderValues()->Borders()[i];
          if (!first_border.Exists()) {
            first_border = border;
          } else if (!first_border.IsSameIgnoringColor(border)) {
            needs_adjust_collapsed_border_corners_ = true;
            break;
          }
        }
      }
