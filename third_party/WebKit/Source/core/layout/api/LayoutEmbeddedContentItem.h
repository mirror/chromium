...
class LayoutEmbeddedContentItem : public LayoutBoxItem {
 public:
  ...
  void UpdateOnWidgetChange() { ToEmbeddedContent()->UpdateOnWidgetChange(); }

 private:
  LayoutEmbeddedContent* ToEmbeddedContent() {
    return ToLayoutEmbeddedContent(GetLayoutObject());
  }

  const LayoutEmbeddedContent* ToEmbeddedContent() const {
    return ToLayoutEmbeddedContent(GetLayoutObject());
  }
};
