...
bool LayoutPart::NodeAtPoint(HitTestResult& result,
  ...
  if (action == kHitTestForeground) {
    ...
    if (VisibleToHitTestRequest(result.GetHitTestRequest()) &&
      ...
      HitTestLocation new_hit_test_location(location_in_container,
                                         -adjusted_location - content_offset);
...
LayoutRect LayoutPart::ReplacedContentRect() const {
  ...
  size_rounded_rect.SetSize(LayoutSize(RoundedIntSize(size_rounded_rect.size())));
...
void LayoutPart::UpdateGeometry() {
  ...
  // FrameViewBase size.
  if (frame_view && frame_view->NeedsLayout() && frame_view->GetFrame().GetPage())
...
void LayoutPart::InvalidatePaintOfSubtreesIfNeeded(
  ...
  if (GetFrameViewBase() && GetFrameViewBase()->IsFrameView() &&
    ...
    if (LayoutView* child_layout_view =
      ...
      child_frame_view->InvalidateTreeIfNeeded(child_view_paint_invalidation_state);
    }
  }
