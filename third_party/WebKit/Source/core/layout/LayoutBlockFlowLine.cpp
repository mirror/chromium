...
InlineFlowBox* LayoutBlockFlow::CreateLineBoxes(LineLayoutItem line_layout_item,
  ...
  do {
    ...
    LineLayoutInline inline_flow(!line_layout_item.IsEqual(this) ? line_layout_item
                                                              : nullptr);

    // Get the last box we made for this layout object.
    parent_box = inline_flow ? inline_flow.LastLineBox()
    ...
    line_layout_item = (++line_depth >= kCMaxLineDepth) ? LineLayoutItem(this)
...
RootInlineBox* LayoutBlockFlow::ConstructLine(BidiRunList<BidiRun>& bidi_runs,
  ...
  for (BidiRun* r = bidi_runs.FirstRun(); r; r = r->Next()) {
    ...
    if (r->line_layout_item_.IsText())
      ...
      box =
          CreateInlineBoxForLayoutObject(r->line_layout_item_, false, is_only_run);
    r->box_ = box;
...
static void UpdateLogicalWidthForRightAlignedBlock(
  ...
  if (total_logical_width > available_logical_width && trailing_space_run) {
    trailing_space_run->box_->SetLogicalWidth(
        std::max(LayoutUnit(), trailing_space_run->box_->LogicalWidth() -
                                   total_logical_width + available_logical_width));
    total_logical_width -= trailing_space_run->box_->LogicalWidth();
  } else {
    logical_left += available_logical_width - total_logical_width;
...
static void UpdateLogicalWidthForCenterAlignedBlock(
  ...
  if (is_left_to_right_direction)
    logical_left +=
        std::max((available_logical_width - total_logical_width) / 2, LayoutUnit());
  else
    logical_left += total_logical_width > available_logical_width
...
void LayoutBlockFlow::ComputeInlineDirectionPositionsForLine(
  ...
  UpdateLogicalInlinePositions(this, line_logical_left, line_logical_right,
                               available_logical_width, is_first_line, indent_text,
                               LayoutUnit());
  ...
  ComputeInlineDirectionPositionsForSegment(
      line_box, line_info, line_logical_left, available_logical_width, first_run,
      trailing_space_run, text_box_data_map, vertical_position_cache,
      word_measurements);
...
BidiRun* LayoutBlockFlow::ComputeInlineDirectionPositionsForSegment(
  ...
  for (; r; r = r->Next()) {
    ...
    if (r->line_layout_item_.IsText()) {
      ...
      if (!r->line_layout_item_.IsLayoutInline()) {
        LayoutBox* layout_box = ToLayoutBox(r->line_layout_item_.GetLayoutObject());
  ...
  expansions.ComputeExpansionsForJustifiedText(
      first_run, trailing_space_run, total_logical_width, available_logical_width);
...
RootInlineBox* LayoutBlockFlow::CreateLineBoxesFromBidiRuns(
  ...
  if (!is_svg_root_inline_box)
    ComputeInlineDirectionPositionsForLine(
        line_box, line_info, bidi_runs.FirstRun(), trailing_space_run, end.AtEnd(),
        text_box_data_map, vertical_position_cache, word_measurements);
...
void LayoutBlockFlow::LayoutRunsAndFloatsInRange(
  ...
  while (!end_of_line.AtEnd()) {
    ...
    end_of_line = line_breaker.NextLineBreak(resolver, layout_state.GetLineInfo(),
    ...
    if (resolver.GetPosition().AtEnd()) {
      ...
      resolver.SetPosition(InlineIterator(resolver.GetPosition().Root(), 0, 0), 0);
      break;
    }
    ...
    if (layout_state.GetLineInfo().IsEmpty()) {
      ...
      if (is_new_uba_paragraph &&
        ...
        TextDirection direction = DeterminePlaintextDirectionality(
            resolver.GetPosition().Root(), resolver.GetPosition().GetLineLayoutItem(),
      ...
      ConstructBidiRunsForLine(
          ...
          layout_state.GetLineInfo().PreviousLineBrokeCleanly(), is_new_uba_paragraph);
      ...
      line_box = CreateLineBoxesFromBidiRuns(
          ...
          layout_state.GetLineInfo(), vertical_position_cache, trailing_space_run,
          word_measurements);
          ...
          if (pagination_strut_from_deleted_line) {
            ...
            if (adjustment) {
              ...
              if (AvailableLogicalWidthForLine(
                ...
                // created.
                pagination_strut_from_deleted_line = line_box->PaginationStrut();
                DCHECK(pagination_strut_from_deleted_line);
    ...
    if (!pagination_strut_from_deleted_line) {
      ...
      if (floating_objects_ && LastRootBox()) {
        ...
        AppendFloatsToLastLine(layout_state, clean_line_start, end_of_line_resolver,
                               clean_line_bidi_status);
      }
    }
...
static inline void AdjustMinMaxForInlineFlow(LayoutObject* child,
  ...
  // the element.
  LayoutUnit bpm = GetBorderPaddingMargin(ToLayoutInline(*child), end_of_inline);
  child_min += bpm;
  child_max += bpm;
}
...
void LayoutBlockFlow::ComputeInlinePreferredLogicalWidths(
  ...
  bool allow_images_to_break = !GetDocument().InQuirksMode() || !IsTableCell() ||
    ...
    if (!child->IsBR()) {
      ...
      if (!child->IsText()) {
        ...
        if (child->IsLayoutInline()) {
          AdjustMinMaxForInlineFlow(child, child_iterator.end_of_inline, child_min,
                                    child_max);
      ...
      if (!child->IsLayoutInline() && !child->IsText()) {
        ...
        t->TrimmedPrefWidths(inline_max, first_line_min_width, has_breakable_start,
                             last_line_min_width, has_breakable_end,
                             has_breakable_char, has_break, first_line_max_width,
                             last_line_max_width, child_min, child_max,
                             strip_front_spaces, style_to_use.Direction());
...
void LayoutBlockFlow::LayoutInlineChildren(bool relayout_children,
  ...
  if (!FirstLineBox() && HasLineIfEmpty())
    SetLogicalHeight(LogicalHeight() +
                     LineHeight(true, IsHorizontalWritingMode() ? kHorizontalLine
                                                                : kVerticalLine,
...
void LayoutBlockFlow::CheckLinesForTextOverflow() {
  ...
  if (font_data && font_data->GlyphForCharacter(kHorizontalEllipsisCharacter)) {
    ...
    first_line_ellipsis_width = first_line_font.Width(
        ConstructTextRun(first_line_font, kFullStopString, kFullStopStringLength,
                         *FirstLineStyle(), ellipsis_direction));
  }
  ...
  for (RootInlineBox* curr = FirstRootBox(); curr; curr = curr->NextRootBox()) {
    ...
    if ((ltr && line_box_edge > block_right_edge) ||
      ...
      if (curr->LineCanAccommodateEllipsis(ltr, block_edge, line_box_edge,
        ...
        LayoutUnit logical_left;  // We are only interested in the delta from the
        // base position.
        ...
        TryPlacingEllipsisOnAtomicInlines(curr, block_right_edge, block_left_edge,
                                          width, selected_ellipsis_str);
      }
    }
    indent_text = kDoNotIndentText;
  }
...
LayoutUnit LayoutBlockFlow::StartAlignedOffsetForLine(
  ...
  LayoutUnit available_logical_width =
      LogicalRightOffsetForLine(LogicalHeight(), kDoNotIndentText) - logical_left;
  UpdateLogicalWidthForAlignment(text_align, 0, 0, logical_left,
                                 total_logical_width, available_logical_width, 0);
