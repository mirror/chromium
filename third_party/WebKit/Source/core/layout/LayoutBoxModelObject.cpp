...
void LayoutBoxModelObject::StyleDidChange(StyleDifference diff,
  ...
  if (old_style) {
    bool new_style_is_fixed_position = Style()->GetPosition() == EPosition::kFixed;
    bool old_style_is_fixed_position = old_style->GetPosition() == EPosition::kFixed;
  ...
  if (IsDocumentElement()) {
    ...
    if (body_layout && body_layout->IsBoxModelObject()) {
      bool new_stole_body_background = ToLayoutBoxModelObject(body_layout)
...
bool LayoutBoxModelObject::HasAutoHeightOrContainingBlockWithAutoHeight()
  ...
  LayoutBlock* cb = ContainingBlockForAutoHeightDetection(logical_height_length);
...
LayoutSize LayoutBoxModelObject::RelativePositionOffset() const {
  ...
  if (!Style()->Right().IsAuto())
    right = ValueForLength(Style()->Right(), containing_block->AvailableWidth());
  if (!left && !right) {
    left = LayoutUnit();
    right = LayoutUnit();
  }
...
void LayoutBoxModelObject::UpdateStickyPositionConstraints() const {
  ...
  scroll_container_relative_padding_box_rect.Move(-scroll_container_border_offset);
  ...
  if (!Style()->Left().IsAuto() && !Style()->Right().IsAuto()) {
    if (horizontal_offsets >
        ...
        horizontal_offsets + sticky_box_rect.Width() > constraining_size.Width()) {
  ...
  if (!Style()->Top().IsAuto() && !Style()->Bottom().IsAuto()) {
    if (vertical_offsets > scroll_container_relative_containing_block_rect.Height() ||
        vertical_offsets + sticky_box_rect.Height() > constraining_size.Height()) {
      skip_bottom = true;
    }
  }
...
FloatRect LayoutBoxModelObject::ComputeStickyConstrainingRect() const {
  ...
  constraining_rect =
      FloatRect(enclosing_clipping_box->OverflowClipRect(LayoutPoint(DoublePoint(
          enclosing_clipping_box->GetScrollableArea()->ScrollPosition()))));
  constraining_rect.Move(
      -enclosing_clipping_box->BorderLeft() + enclosing_clipping_box->PaddingLeft(),
      -enclosing_clipping_box->BorderTop() + enclosing_clipping_box->PaddingTop());
...
LayoutRect LayoutBoxModelObject::LocalCaretRectForEmptyElement(
  ...
  LayoutUnit vertical_space =
      LineHeight(true, current_style.IsHorizontalWritingMode() ? kHorizontalLine
                                                              : kVerticalLine,
...
void LayoutBoxModelObject::MoveChildTo(LayoutBoxModelObject* to_box_model_object,
                                       LayoutObject* child,
  ...
  if (full_remove_insert && (to_box_model_object->IsLayoutBlock() ||
    ...
    to_box_model_object->AddChild(VirtualChildren()->RemoveChildNode(this, child),
                               before_child);
  } else {
