...
static LayoutObject* PreviousSiblingOrParentRespectingContainment(
  ...
  return previous && previous->GetLayoutObject() &&
                 !(previous->GetLayoutObject()->Style()->Contain() & kContainsStyle)
    ...
    if (search_end_layout_object == current_layout_object) {
      ...
      if (current_counter) {
        ...
        if (previous_sibling_protector) {
          ...
          // reset.
          if (!is_reset ||
              !AreLayoutObjectsElementsSiblings(*current_layout_object,
                                                counter_owner)) {
      ...
      if (current_counter) {
        ...
        if (previous_sibling_protector) {
          ...
          if (current_counter->ActsAsReset()) {
            ...
            current_layout_object = parent ? parent->GetLayoutObject() : nullptr;
            continue;
          }
        } else {
          previous_sibling_protector = current_counter;
...
static CounterNode* MakeCounterNodeIfNeeded(LayoutObject& object,
  ...
  bool is_reset = false;
  int value = 0;
  if (!PlanCounter(object, identifier, is_reset, value) && !always_create_counter)
    return nullptr;
  ...
  if (FindPlaceForCounter(object, identifier, is_reset, new_parent,
    ...
    new_parent->InsertAfter(new_node.Get(), new_previous_sibling.Get(), identifier);
  CounterMap* node_map;
  if (object.HasCounterNodeMap()) {
  ...
  for (LayoutObject* current_layout_object = NextInPreOrder(object, stay_within);
       current_layout_object;
       current_layout_object =
           NextInPreOrder(*current_layout_object, stay_within, skip_descendants)) {
    skip_descendants = false;
    if (!current_layout_object->HasCounterNodeMap())
      continue;
    CounterNode* current_counter = maps.at(current_layout_object)->at(identifier);
    if (!current_counter)
      continue;
...
PassRefPtr<StringImpl> LayoutCounter::OriginalText() const {
  if (!counter_node_) {
    ...
    MakeCounterNodeIfNeeded(*before_after_container, counter_.Identifier(), true)
  ...
  if (!counter_.Separator().IsNull()) {
    ...
    while (CounterNode* parent = child->Parent()) {
      text =
          ListMarkerText::GetText(counter_.ListStyle(), child->CountInParent()) +
...
static void UpdateCounters(LayoutObject& layout_object) {
  ...
  for (CounterDirectiveMap::const_iterator it = directive_map->begin();
    ...
    FindPlaceForCounter(layout_object, it->key, node->HasResetType(), new_parent,
                        new_previous_sibling);
...
void LayoutCounter::LayoutObjectStyleChanged(LayoutObject& layout_object,
  ...
  if (old_counter_directives) {
    ...
    for (CounterDirectiveMap::const_iterator it = new_counter_directives->begin();
         it != new_map_end; ++it) {
