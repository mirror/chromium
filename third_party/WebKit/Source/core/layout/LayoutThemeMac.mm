...
static FontWeight ToFontWeight(NSInteger app_kit_font_weight) {
  ...
  static FontWeight font_weights[] = {
      kFontWeight100, kFontWeight100, kFontWeight200, kFontWeight300, kFontWeight400,
      kFontWeight500, kFontWeight600, kFontWeight600, kFontWeight700, kFontWeight800,
      kFontWeight800, kFontWeight900, kFontWeight900, kFontWeight900};
...
bool LayoutThemeMac::IsControlStyled(const ComputedStyle& style) const {
  if (style.Appearance() == kTextFieldPart || style.Appearance() == kTextAreaPart)
  ...
  if (min_size.Width() >=
    ...
    size = NSRegularControlSize;
               ...
               static_cast<int>(sizes[NSSmallControlSize].Width() *
                                zoom_level) &&
           min_size.Height() >=
               static_cast<int>(sizes[NSSmallControlSize].Height() * zoom_level))
...
int LayoutThemeMac::PopupInternalPaddingStart(
    const ComputedStyle& style) const {
  if (style.Appearance() == kMenulistPart)
    return PopupButtonPadding(ControlSizeForFont(style))[ThemeMac::kLeftMargin] *
           style.EffectiveZoom();
...
int LayoutThemeMac::PopupInternalPaddingEnd(const HostWindow*,
  ...
  return static_cast<int>(
      ceilf(arrow_width +
            (kMenuListArrowPaddingStart + kMenuListArrowPaddingEnd) *
                style.EffectiveZoom()));
}
...
const IntSize* LayoutThemeMac::MenuListSizes() const {
  static const IntSize kSizes[3] = {IntSize(9, 0), IntSize(5, 0), IntSize(0, 0)};
  return kSizes;
}
