...
PaintInvalidationState::PaintInvalidationState(
    ...
    : current_object_(layout_view),
      ...
      paint_invalidation_container_(&layout_view.ContainerForPaintInvalidation()),
...
PaintInvalidationState::PaintInvalidationState(
    ...
    : current_object_(current_object),
      ...
      clipped_for_absolute_position_(parent_state.clipped_for_absolute_position_),
      clip_rect_(parent_state.clip_rect_),
      clip_rect_for_absolute_position_(parent_state.clip_rect_for_absolute_position_),
...
// RuntimeEnabledFeatures::slimmingPaintInvalidationEnabled().
  ...
  if (cached_offsets_enabled_ || current_object == paint_invalidation_container_)
...
void PaintInvalidationState::UpdateForCurrentObject(
  ...
  if (position == EPosition::kAbsolute) {
    ...
    const LayoutObject& container = parent_state.container_for_absolute_position_;
...
static FloatPoint SlowLocalToAncestorPoint(const LayoutObject& object,
  ...
  if (object.IsLayoutView())
    return ToLayoutView(object).LocalToAncestorPoint(
        ...
        kTraverseDocumentBoundaries | kInputIsInFrameCoordinates);
  FloatPoint result =
      object.LocalToAncestorPoint(point, &ancestor, kTraverseDocumentBoundaries);
