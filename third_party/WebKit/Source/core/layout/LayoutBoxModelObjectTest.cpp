...
TEST_F(LayoutBoxModelObjectTest, StickyPositionConstraintInvalidation) {
  ...
  EXPECT_TRUE(scrollable_area->GetStickyConstraintsMap().Contains(sticky->Layer()));
  EXPECT_EQ(25.f,
            GetScrollContainerRelativeStickyBoxRect(
                scrollable_area->GetStickyConstraintsMap().at(sticky->Layer()))
                .Location()
                .X());
  ToHTMLElement(target->GetNode())->classList().add("hide", ASSERT_NO_EXCEPTION);
...
TEST_F(LayoutBoxModelObjectTest,
  ...
  StickyConstraintsMap constraints_map = scrollable_area->GetStickyConstraintsMap();
...
TEST_F(LayoutBoxModelObjectTest,
  ...
  EXPECT_EQ(sticky_parent,
            constraints_map.at(sticky_child->Layer())
                .NearestStickyBoxShiftingContainingBlock());
  EXPECT_EQ(sticky_parent,
            constraints_map.at(sticky_nested_child->Layer())
                .NearestStickyBoxShiftingContainingBlock());
}
...
TEST_F(LayoutBoxModelObjectTest,
  ...
  // shifting ancestor.
  EXPECT_EQ(sticky_parent,
            constraints_map.at(sticky_grandchild->Layer())
                .NearestStickyBoxShiftingContainingBlock());
}
...
TEST_F(LayoutBoxModelObjectTest,
  ...
  EXPECT_EQ(sticky_outer,
            constraints_map.at(sticky_th->Layer())
                .NearestStickyBoxShiftingContainingBlock());
}
