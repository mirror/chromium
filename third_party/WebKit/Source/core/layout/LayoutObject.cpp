...
void LayoutObject::SetPreferredLogicalWidthsDirty(MarkingBehavior mark_parents) {
...
const LayoutBoxModelObject& LayoutObject::ContainerForPaintInvalidation()
  ...
  while (const LayoutObject* owner_object = LayoutAPIShim::ConstLayoutObjectFrom(
...
PaintInvalidationReason LayoutObject::InvalidatePaintIfNeeded(
  ...
  if (v->GetDocument().Printing())
    return kPaintInvalidationNone;  // Don't invalidate paints if we're printing.
  ...
  context.new_location =
      paint_invalidation_state.ComputeLocationInBacking(new_visual_rect.Location());
...
bool LayoutObject::MapToVisualRectInAncestorSpace(
  ...
  bool retval = MapToVisualRectInAncestorSpaceInternal(ancestor, transform_state,
                                                       visual_rect_flags);
...
void LayoutObject::ShowLayoutObject(StringBuilder& string_builder) const {
  ...
  if (IsText() && ToLayoutText(this)->IsTextFragment())
    string_builder.Append(
        String::Format(" \"%s\" ", ToLayoutText(this)->GetText().Ascii().Data()));
...
void LayoutObject::SetStyle(PassRefPtr<ComputedStyle> style) {
  ...
  StyleImage* old_content_image =
      old_style && old_style->GetContentData() && old_style->GetContentData()->IsImage()
...
void LayoutObject::StyleWillChange(StyleDifference diff,
  ...
  if (GetNode() && !GetNode()->IsTextNode() &&
    ...
    EventHandlerRegistry& registry = GetDocument().GetPage()->GetEventHandlerRegistry();
...
void LayoutObject::ClearBaseComputedStyle() {
  ...
  if (!GetNode()->IsElementNode())
    return;
  if (ElementAnimations* animations = ToElement(GetNode())->GetElementAnimations())
...
void LayoutObject::MapLocalToAncestor(const LayoutBoxModelObject* ancestor,
  ...
  if (IsLayoutFlowThread()) {
    ...
    // thread.
    container_offset += ColumnOffset(LayoutPoint(transform_state.MappedPoint()));
  }
...
void LayoutObject::GetTransformFromContainer(
  ...
  if (container_object && container_object->HasLayer() &&
    ...
    perspective_matrix.ApplyPerspective(container_object->Style()->Perspective());

    transform.TranslateRight3d(-perspective_origin.X(), -perspective_origin.Y(),
                               0);
    transform = perspective_matrix * transform;
    transform.TranslateRight3d(perspective_origin.X(), perspective_origin.Y(), 0);
  }
}
...
FloatQuad LayoutObject::LocalToAncestorQuad(
    ...
    const LayoutBoxModelObject* ancestor,
    MapCoordinatesFlags mode) const {
  return LocalToAncestorQuadInternal(local_quad, ancestor, mode | kUseTransforms);
}
  ...
  if (!HasLayer()) {
    ...
    if (container && current_layer->GetLayoutObject() != container) {
      ...
      if (current_layer->GetLayoutObject().HasOverflowClip())
        layer_offset.Move(current_layer->GetLayoutBox()->ScrolledContentOffset());
    }
  }
...
void LayoutObject::AddLayerHitTestRects(LayerHitTestRects& layer_rects,
...
RespectImageOrientationEnum LayoutObject::ShouldRespectImageOrientation(
  ...
  if (layout_object->GetDocument().GetSettings() &&
      layout_object->GetDocument().GetSettings()->GetShouldRespectImageOrientation())
    return kRespectImageOrientation;
...
void LayoutObject::WillBeDestroyed() {
  ...
  if (style_) {
    ...
    if (style_->GetContentData() && style_->GetContentData()->IsImage())
      ToImageContentData(style_->GetContentData())->GetImage()->RemoveClient(this);
...
CompositingState LayoutObject::GetCompositingState() const {
  return HasLayer() ? ToLayoutBoxModelObject(this)->Layer()->GetCompositingState()
                    : kNotComposited;
}
...
static PassRefPtr<ComputedStyle> FirstLineStyleForCachedUncachedType(
  ...
  if (layout_object_for_first_line_style->BehavesLikeBlockContainer()) {
    if (const LayoutBlock* first_line_block =
      ...
      if (type == kCached)
        return first_line_block->GetCachedPseudoStyle(kPseudoIdFirstLine, style);
  ...
  for (LayoutObject* ancestor = Parent(); ancestor;
    ...
    if (base &&
        (node->IsClosedShadowHiddenFrom(*base) ||
         (node->IsInShadowTree() &&
          node->ContainingShadowRoot()->GetType() == ShadowRootType::kUserAgent))) {
...
PositionWithAffinity LayoutObject::CreatePositionWithAffinity(
  ...
  while (LayoutObject* parent = child->Parent()) {
    ...
    for (LayoutObject* layout_object = child->PreviousInPreOrder(); layout_object;
         layout_object = layout_object->PreviousInPreOrder()) {
...
void LayoutObject::SetShouldDoFullPaintInvalidationWithoutGeometryChange(
  ...
  if (bitfields_.FullPaintInvalidationReason() == kPaintInvalidationNone ||
    ...
    if (reason == kPaintInvalidationFull)
      reason =
          DocumentLifecycleBasedPaintInvalidationReason(GetDocument().Lifecycle());
