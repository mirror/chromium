...
void LayoutObject::SetStyle(scoped_refptr<ComputedStyle> style) {
  ...
  if (GetNode() && GetNode()->IsActive())
    LOG(ERROR) << DebugName() << " is active "
               << ResolveColor(GetCSSPropertyColor()).Serialized();
  if (old_style && ResolveColor(GetCSSPropertyColor()) !=
                       ResolveColor(*old_style, GetCSSPropertyColor()))
    LOG(ERROR) << DebugName() << " color changed to "
               << ResolveColor(GetCSSPropertyColor()).Serialized() << " from "
               << ResolveColor(*old_style, GetCSSPropertyColor()).Serialized();
