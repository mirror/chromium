...
LayoutUnit BaselineAlignment::LogicalAscentForChild(
  ...
  LayoutUnit ascent = AscentForChild(layout_grid, child, baseline_axis);
  return IsDescentBaselineForChild(layout_grid, child, baseline_axis)
             ? DescentForChild(layout_grid, child, ascent, baseline_axis)
             : ascent;
}

LayoutUnit BaselineAlignment::AscentForChild(const LayoutGrid& layout_grid,
                                             const LayoutBox& child,
                                             GridAxis baseline_axis) const {
  LayoutUnit margin =
      IsDescentBaselineForChild(layout_grid, child, baseline_axis)
          ? GridLayoutUtils::MarginUnderForChild(layout_grid, child,
                                                 baseline_axis)
          : GridLayoutUtils::MarginOverForChild(layout_grid, child,
                                                baseline_axis);
...
LayoutUnit BaselineAlignment::DescentForChild(const LayoutGrid& layout_grid,
                                              const LayoutBox& child,
                                              LayoutUnit ascent,
                                              GridAxis baseline_axis) const {
...
void BaselineAlignment::UpdateBaselineAlignmentContextIfNeeded(
  ...
  LayoutUnit descent =
      DescentForChild(layout_grid, child, ascent, baseline_axis);
  if (IsDescentBaselineForChild(layout_grid, child, baseline_axis))
...
LayoutUnit BaselineAlignment::BaselineOffsetForChild(
  ...
  if (!layout_grid.IsBaselineAlignmentForChild(child, baseline_axis))
    return LayoutUnit();
  auto& group =
      GetBaselineGroupForChild(layout_grid, grid, child, baseline_axis);
  if (group.size() > 1) {
    return group.MaxAscent() -
           LogicalAscentForChild(layout_grid, child, baseline_axis);
  }
  return LayoutUnit();
}
