...
const NGPhysicalBoxFragment* EnclosingBoxFragment(const LayoutInline& node) {
  LayoutBlockFlow* const block_flow = node.EnclosingNGBlockFlow();
  if (!block_flow)
    return nullptr;
...
void LayoutInline::AbsoluteRects(Vector<IntRect>& rects,
  ...
  if (const NGPhysicalBoxFragment* box_fragment = EnclosingBoxFragment(*this)) {
...
LayoutPoint LayoutInline::FirstLineBoxTopLeft() const {
  if (const NGPhysicalBoxFragment* box_fragment = EnclosingBoxFragment(*this)) {
    ...
    return first_child.FragementOffsetInContainerBox().ToLayoutPoint();
  }
