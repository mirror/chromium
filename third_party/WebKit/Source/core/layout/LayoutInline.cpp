...
void LayoutInline::StyleDidChange(StyleDifference diff,
  ...
  if (!AlwaysCreateLineBoxes()) {
    bool always_create_line_boxes_new =
        ...
        new_style.HasPadding() || new_style.HasMargin() || new_style.HasOutline();
...
void LayoutInline::UpdateAlwaysCreateLineBoxes(bool full_layout) {
  ...
  if (!always_create_line_boxes_new && check_fonts &&
    ...
    always_create_line_boxes_new =
        !first_line_parent_style.HasIdenticalAscentDescentAndLineGap(child_style) ||
...
LayoutRect LayoutInline::LocalCaretRect(InlineBox* inline_box,
                                        int,
    ...
    if (curr->IsBox()) {
      ...
      if (curr_box->InlineBoxWrapper()) {
        ...
        if (is_horizontal) {
          ...
          yield(LayoutRect(logical_top, curr_box->InlineBoxWrapper()->Y() -
                                           curr_box->MarginTop(),
                           logical_height,
      ...
      if (!curr_inline->AlwaysCreateLineBoxes()) {
        ...
        for (InlineFlowBox* child_line = curr_inline->FirstLineBox(); child_line;
             child_line = child_line->NextLineBox()) {
...
bool LayoutInline::HitTestCulledInline(
  ...
  HitTestCulledInlinesGeneratorContext context(region_result, adjusted_location);
...
void LayoutInline::AddOutlineRectsForContinuations(
  ...
  if (LayoutBoxModelObject* continuation = this->Continuation()) {
    if (continuation->IsInline())
      ...
      continuation->AddOutlineRects(
          rects, additional_offset + (ToLayoutBox(continuation)->Location() -
                                     ContainingBlock()->Location()),
          include_block_overflows);
  }
}
