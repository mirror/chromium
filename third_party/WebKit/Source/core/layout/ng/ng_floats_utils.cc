...
NGLogicalOffset CalculateLogicalOffsetForOpportunity(
    ...
    const NGBfcOffset& float_bfc_offset,
  ...
  LayoutUnit relative_line_offset =
      float_bfc_offset.line_offset - unpositioned_float->bfc_line_offset;
  LayoutUnit inline_offset = parent_space.Direction() == TextDirection::kLtr
                                 ? relative_line_offset + margins.inline_start
                                 : container_size - relative_line_offset -
                                       float_margin_box_inline_size +
                                       margins.inline_end;  // start?
  // TODO FIXME inline_end?

  NGLogicalOffset offset = {inline_offset, float_bfc_offset.block_offset +
                                               margins.block_start -
...
NGPositionedFloat PositionFloat(LayoutUnit origin_block_offset,
  ...
  //
  NGBfcOffset float_bfc_offset = opportunity.offset;
  if (unpositioned_float->IsRight()) {
    float_bfc_offset.line_offset +=
  ...
  const NGExclusion exclusion = CreateExclusion(
      float_fragment, float_bfc_offset, unpositioned_float->margins,
  ...
  NGLogicalOffset logical_offset = CalculateLogicalOffsetForOpportunity(
      parent_space, opportunity, float_bfc_offset, parent_bfc_block_offset,
      unpositioned_float, container_size, float_margin_box_inline_size);
...
const Vector<NGPositionedFloat> PositionFloats(
  ...
  for (auto& unpositioned_float : unpositioned_floats) {
    positioned_floats.push_back(PositionFloat(
        ...
        space, container_size, exclusion_space));
  }
