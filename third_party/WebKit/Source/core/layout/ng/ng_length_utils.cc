...
LayoutUnit ResolveInlineLength(const NGConstraintSpace& constraint_space,
  ...
  switch (length.GetType()) {
    ...
    case kCalculated: {
      ...
      if (type == LengthResolveType::kMarginBorderPaddingSize &&
          constraint_space.IsOrthogonalWritingModeRoot()) {
        percentage_resolution_size =
            constraint_space.PercentageResolutionSize().block_size;
        if (percentage_resolution_size == NGSizeIndefinite) {
          if (IsHorizontalWritingMode(constraint_space.WritingMode()))
            percentage_resolution_size =
                constraint_space.InitialContainingBlockSize().height;
          else
            percentage_resolution_size =
                constraint_space.InitialContainingBlockSize().width;
        }
      } else {
        percentage_resolution_size =
...
NGBoxStrut ComputeMarginsFor(const NGConstraintSpace& constraint_space,
                             const ComputedStyle& style,
                             const NGConstraintSpace& margins_for) {
  return ComputePhysicalMargins(constraint_space, style)
      .ConvertToLogical(margins_for.WritingMode(), margins_for.Direction());
}
