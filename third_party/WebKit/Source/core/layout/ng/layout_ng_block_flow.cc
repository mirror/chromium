...
const NGPhysicalFragment* LayoutNGBlockFlow::CurrentFragment() const {
  if (cached_result_)
    return cached_result_->PhysicalFragment();
  return nullptr;
}
...
void LayoutNGBlockFlow::AddOverflowFromChildren() {
  ...
  if (ChildrenInline()) {
    const NGPhysicalFragment* physical_fragment = CurrentFragment();
    DCHECK(physical_fragment);
    // TODO(kojii): If |RecalcOverflowAfterStyleChange()|, we need to re-compute
    // glyph bounding box. How to detect it and how to re-compute is TBD.
    LayoutRect visual_rect = physical_fragment->VisualRect();
    AddContentsVisualOverflow(visual_rect);
    // TODO(kojii): The above code computes visual overflow only, we fallback to
    // LayoutBlock for AddLayoutOverflow() for now. It doesn't compute correctly
    // without RootInlineBox though.
  }
