...
void LayoutNGBlockFlow::AddOverflowFromChildren() {
  ...
  // |CopyFragmentDataToLayoutBox()| and
  // |LayoutBlock::RecalcOverflowAfterStyleChange()|.
  if (ChildrenInline()) {
    // |LayoutBlockFlow| computes visual overflow from |RootInlineBox|, which is
    // missing in NGPaint. We need the fragment produced by the last layout
    // cycle, which is IIUC different from the purpose of fragment cache.
...
LayoutUnit LayoutNGBlockFlow::InlineBlockBaseline(
  ...
  if (ChildrenInline()) {
    ...
    if (const NGBaseline* baseline = physical_fragment.Baseline(
      ...
      // TODO(kojii): Implement. This will stop working once we stop creating
      // line boxes.
    }
    // NOTREACHED();
  }
