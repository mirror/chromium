...
void LayoutNGBlockFlow::UpdateBlockLayout(bool relayout_children) {
  ...
  if (IsOutOfFlowPositioned()) {
    ...
    if (container_style->IsDisplayFlexibleOrGridBox()) {
      ...
      if (parent_style->IsLeftToRightDirection()) {
        if (!logical_left.IsAuto())
          static_inline =
              ValueForLength(logical_left, container->LogicalWidth());
      } else {
    ...
    static_position = NGStaticPosition::Create(
        FromPlatformWritingMode(parent_style->GetWritingMode()),
        parent_style->Direction(), static_location);
    // Run out of flow algorithm.
    container_builder.AddOutOfFlowLegacyCandidate(NGBlockNode(this),
                                                  static_position);

    // ContainingBlock() is not always the same as Container().
    // That is why we need true css_container to get correct values for
    //  CanContain...Object() methods.
    LayoutObject* css_container = Container();

    NGOutOfFlowLayoutPart(*constraint_space, *container_style,
                          &container_builder,
                          css_container->CanContainAbsolutePositionObjects(),
                          css_container->CanContainFixedPositionObjects())
        .Run();
    // Need to translate offsets for all children from container builder
    ...
    physical_root_fragment_ = ToNGPhysicalBoxFragment(child_fragment.Get());
    return;
  }
  ...
  // This object has already been positioned in legacy layout by our
  // containing block. Copy the position and place the fragment.
