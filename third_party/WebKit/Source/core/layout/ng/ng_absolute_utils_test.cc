...
TEST_F(NGAbsoluteUtilsTest, Horizontal) {
  ...
  p = ComputePartialAbsoluteWithChildInlineSize(
      *ltr_space_, *style_, static_position, estimated_inline, intrinsic_size);
  ...
  // All auto + RTL.
  p = ComputePartialAbsoluteWithChildInlineSize(
      *rtl_space_, *style_, static_position, estimated_inline, intrinsic_size);
  EXPECT_EQ(minmax_60.min_content, p.size.width);
