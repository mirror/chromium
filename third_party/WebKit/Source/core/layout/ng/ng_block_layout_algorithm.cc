...
WTF::Optional<NGPreviousInflowPosition> NGBlockLayoutAlgorithm::HandleInflow(
  ...
  if ((layout_result->Status() == NGLayoutResult::kBfcOffsetResolved ||
    ...
    //
    // As #zero doesn't touch the incoming margin strut now we need to perform
    // a relayout with an empty incoming margin strut.
    //
    ...
    //
    // NOTE(ikilpatrick): We can potentially check if this new margin strut is
    // equal to the previous incoming margin strut and skip the re-layout.
    if (empty_block_affected_by_clearance) {
      DCHECK(IsEmptyBlock(child, *layout_result));
      child_data.margin_strut = NGMarginStrut();
      child_data.margin_strut.Append(child_data.margins.block_start);
    }
