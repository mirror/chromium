...
RefPtr<NGLayoutResult> NGBlockLayoutAlgorithm::Layout() {
  ...
  if (ConstraintSpace().IsNewFormattingContext()) {
    ...
    WTF::Optional<LayoutUnit> float_end_offset =
        ExclusionSpace().ClearanceOffset(EClear::kBoth);
    if (float_end_offset)
...
bool NGBlockLayoutAlgorithm::HandleInflow(
  ...
  bool should_position_pending_floats =
      ...
      ClearanceMayAffectLayout(ExclusionSpace(), unpositioned_floats_,
                               child.Style());
  ...
  if (!is_new_fc) {
    // The exclusion space can be null if the result wasn't successful.
    if (layout_result->Status() == NGLayoutResult::kSuccess) {
      DCHECK(layout_result->ExclusionSpace());
      exclusion_space_ =
          WTF::MakeUnique<NGExclusionSpace>(*layout_result->ExclusionSpace());
    } else {
      // We temporarily set the exclusion space to nullptr, to ensure that we
      // aren't accidently using data from the old space. There are DCHECKs
      // within the exclusion_space_ accessors to protect against this.
      DCHECK(!layout_result->ExclusionSpace());
...
bool NGBlockLayoutAlgorithm::PositionNewFc(
  ...
  NGExclusionSpace tmp_exclusion_space(ExclusionSpace());
  ...
  origin_offset = {ConstraintSpace().BfcOffset().inline_offset +
                       border_scrollbar_padding_.inline_start,
                   child_bfc_offset_estimate};
  AdjustToClearance(ExclusionSpace().ClearanceOffset(child_clear),
                    &origin_offset);
  ...
  opportunity = ExclusionSpace().FindLayoutOpportunity(
      origin_offset, child_space.AvailableSize(), fragment_margin_size);
...
RefPtr<NGConstraintSpace> NGBlockLayoutAlgorithm::CreateConstraintSpaceForChild(
  ...
  if (!is_new_fc)
    space_builder.SetExclusionSpace(ExclusionSpace());
  ...
  const ComputedStyle& child_style = child.Style();
  space_builder
      .SetClearanceOffset(ExclusionSpace().ClearanceOffset(child_style.Clear()))
