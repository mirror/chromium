  ...
  for (NGLayoutInputNode node = Node().FirstChild(); node;
    ...
    if (node.IsBlock()) {
      Length inline_start_margin_length =
          node.Style().MarginStartUsing(Style());
      ...
      // point. Currently "margin-left: calc(10px + 50%)" will resolve to 0px,
      // but 10px would be more correct, (as percentages resolve to zero).
