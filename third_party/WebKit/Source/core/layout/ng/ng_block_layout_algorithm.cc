...
NGBlockLayoutAlgorithm::LayoutNewFormattingContext(
  ...
  // the start block offset estimate (the child may not overlap with floats, so
  // we need to find out how much space is used by floats at this block
  // offset). This may affect the inline size of the child, e.g. when it's
  ...
  NGLayoutOpportunity opportunity = tmp_exclusion_space.FindLayoutOpportunity(
      origin_offset, child_available_size_.inline_size, NGLogicalSize());

  scoped_refptr<NGConstraintSpace> child_space =
      CreateConstraintSpaceForChild(child, child_data, opportunity.rect.Size());
  scoped_refptr<NGLayoutResult> layout_result =
  ...
  NGFragment fragment(ConstraintSpace().GetWritingMode(),
                      *layout_result->PhysicalFragment());
  // Now find a layout opportunity where the fragment is actually going to fit.
