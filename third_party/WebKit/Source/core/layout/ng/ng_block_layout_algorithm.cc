...
Optional<MinMaxSize> NGBlockLayoutAlgorithm::ComputeMinMaxSize() const {
  ...
  // The max_size can become smaller than the min_size when a new-FC has a
  // negative margin which affects the max inline contribution. E.g.
  // <div style="display: flex; margin-left: -10px; width: 20px;"></div>
  // In the above case the min_size would be 20px, however the max_size would
  // be 10px. We limit the min_size in these cases.
