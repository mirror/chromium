...
bool NGBlockLayoutAlgorithm::HandleNewFormattingContext(
  ...
  bool is_auto_inline_size = child_style.LogicalWidth().IsAuto() &&
  ...
  if (is_auto_inline_size) {
    opportunity = tmp_exclusion_space.FindLayoutOpportunity(
        origin_offset, child_available_size_, NGLogicalSize());
    fixed_inline_size = opportunity->InlineSize();
  }

  RefPtr<NGConstraintSpace> child_space = CreateConstraintSpaceForChild(
      child, child_data, WTF::nullopt, fixed_inline_size);
