...
bool NGBlockLayoutAlgorithm::HandleNewFormattingContext(
  ...
  std::tie(layout_result, opportunity) =
      LayoutNewFormattingContext(child, child_break_token, is_auto_inline_size,
  ...
  // We allow a single re-layout for new formatting contexts.
  ...
  if (opportunity.offset.block_offset != child_bfc_offset_estimate ||
      (is_auto_inline_size && fragment_block_size > opportunity.BlockSize())) {
    ...
    std::tie(layout_result, opportunity) = LayoutNewFormattingContext(
        child, child_break_token, is_auto_inline_size, child_data,
