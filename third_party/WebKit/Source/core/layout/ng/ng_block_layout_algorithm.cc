...
bool NGBlockLayoutAlgorithm::ShouldBreakBeforeChild(
  ...
  if (token && token->IsBlockType() &&
    ...
    if (has_processed_first_child_)
      return true;
    // TODO(mstensho): Detect class C break points [1] here. If we're not at the
    // block content start edge, we may break here, even if it's a first in-flow
    // child (this situation typically occurs if there are floats at the
    // beginning of a container, so that the first piece of in-flow content
