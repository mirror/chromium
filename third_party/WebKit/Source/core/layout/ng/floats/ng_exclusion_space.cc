...
// block-start, then by block-size.
void InsertOpportunity(const NGLayoutOpportunity& opportunity,
  ...
  for (size_t j = opportunities->size() - 1; j >= 0; --j) {
    if (opportunities->at(j).rect.BlockStartOffset() <=
...
NGLayoutOpportunity CreateLayoutOpportunity(const NGLayoutOpportunity& other,
  ...
  NGBfcOffset start_offset(
      /* line_offset */ std::max(other.rect.LineStartOffset(),
                                 offset.line_offset),
      /* block_offset */ std::max(other.rect.start_offset.block_offset,
                                  offset.block_offset));

  NGBfcOffset end_offset(
      /* line_offset */ std::min(other.rect.LineEndOffset(),
                                 offset.line_offset + inline_size),
      /* block_offset */ other.rect.BlockEndOffset());
...
NGLayoutOpportunity CreateLayoutOpportunity(
  ...
  NGBfcOffset start_offset(
      /* line_offset */ std::max(shelf.line_left, offset.line_offset),
      /* block_offset */ std::max(shelf.block_offset, offset.block_offset));

  NGBfcOffset end_offset(
      /* line_offset */ std::min(shelf.line_right,
                                 offset.line_offset + inline_size),
      /* block_offset */ LayoutUnit::Max());
...
void NGExclusionSpace::Add(scoped_refptr<const NGExclusion> exclusion) {
  ...
  for (size_t i = 0; i < shelves_.size(); ++i) {
    ...
    if (is_below) {
      ...
      if (has_solid_edges && is_overlapping) {
        NGLayoutOpportunity opportunity(
            NGBfcRect(/* start_offset */ {shelf.line_left, shelf.block_offset},
    ...
    //
    // NOTE: We calculate this before we need it as the next if-statement block
    // may remove the shelf that we need to check against.
    //
    ...
    // NOTE: After this point the shelf may not exist any more.
...
NGLayoutOpportunity NGExclusionSpace::FindLayoutOpportunity(
  ...
  for (const auto& opportunity : opportunities) {
    ...
    // the same as the available_inline_size it pretend that it "fits".
