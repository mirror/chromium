...
/*bool Intersects(const NGShelf& shelf,
                const NGBfcOffset& offset,
                const LayoutUnit inline_size) {
  return shelf.line_left > offset.line_offset &&
    ...
    if (is_below) {
      ...
      if (has_solid_edges && is_overlapping) {
        ...
        opportunity.rect = NGBfcRect(
            {shelf.line_left, shelf.block_offset},
            {shelf.line_right - shelf.line_left,
...
NGLayoutOpportunity NGExclusionSpace::FindLayoutOpportunity(
  ...
  /* NGLayoutOpportunityIterator opportunity_iter(*this, available_size,
  offset); NGLayoutOpportunity opportunity;
