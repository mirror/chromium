...
void NGBlockNode::CopyFragmentDataToLayoutBox(
  ...
  for (const auto& child_fragment : physical_fragment->Children()) {
    ...
    // For the moment anonymous fragments...
    if (child_fragment->GetLayoutObject() == box_) {
      for (const auto& maybe_float_fragment :
        ...
        // The child might be just a line-box fragment - ignore.
        if (IsFloatFragment(*maybe_float_fragment))
          FragmentPositionUpdated(
...
void NGBlockNode::FragmentPositionUpdated(
  ...
  LayoutBox* parent_box = box_;  // TODO
  if (parent_box->StyleRef().IsFlippedBlocksWritingMode()) {
    LayoutUnit container_width = parent_box->Size().Width();
  ...
  if (IsFloatFragment(fragment) && parent_box->IsLayoutBlockFlow()) {
    LayoutBlockFlow* parent_block_flow = ToLayoutBlockFlow(parent_box);

    FloatingObject* floating_object =
        parent_block_flow->InsertFloatingObject(*layout_box);
