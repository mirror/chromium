...
NGLayoutInputNode NGBlockNode::FirstChild() {
  auto* const node_container = ToLayoutNGBlockFlow(box_);
  LayoutBlockFlow* layout_object_container = node_container;
  LayoutObject* child =
      GetLayoutObjectForFirstChildNode(layout_object_container);
  if (!child)
    return nullptr;
  if (layout_object_container->ChildrenInline())
    return NGInlineNode(node_container);
