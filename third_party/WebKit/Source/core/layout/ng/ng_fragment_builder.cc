...
NGPhysicalFragment::NGBoxType BoxTypeFromLayoutObject(
  ...
  if (!layout_object || layout_object->Style() != &style)
...
NGPhysicalFragment::NGBoxType NGFragmentBuilder::BoxType() const {
  if (box_type_ != NGPhysicalFragment::NGBoxType::kNormalBox) {
    DCHECK_EQ(box_type_, BoxTypeFromLayoutObject(layout_object_, Style()));
    return box_type_;
  }

  // When implicit, compute from LayoutObject.
  // TODO(kojii): Make all cases explicit and remove this.
  return BoxTypeFromLayoutObject(layout_object_, Style());
}
