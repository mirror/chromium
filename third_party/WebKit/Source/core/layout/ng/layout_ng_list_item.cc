...
void LayoutNGListItem::MarkerText(StringBuilder* text) const {
  ...
  switch (style.ListStyleType()) {
    ...
    case EListStyleType::kSquare:
      ...
      text->Append(ListMarkerText::Suffix(Style()->ListStyleType(), 0));
      break;
...
void LayoutNGListItem::EnsureMarker() {
  if (!marker_)
  ...
  String marker_text = marker_text_builder.ToString();
  text->SetText(marker_text.ReleaseImpl());
