...
LayoutUnit GetClearanceOffset(const std::shared_ptr<NGExclusions>& exclusions,
                              EClear clear_type) {
  LayoutUnit clear_float_left_offset =
      exclusions->clear_float_left_offset.value_or(LayoutUnit::Min());
  LayoutUnit clear_float_right_offset =
      exclusions->clear_float_right_offset.value_or(LayoutUnit::Min());

  switch (clear_type) {
    case EClear::kNone:
      return LayoutUnit::Min();  // nothing to do here.
    case EClear::kLeft:
      return clear_float_left_offset;
    case EClear::kRight:
      return clear_float_right_offset;
    case EClear::kBoth:
      return std::max(clear_float_left_offset, clear_float_right_offset);
    default:
      NOTREACHED();
  }
