    ...
    if (child->IsLineBox()) {
      ...
      for (auto& line_box_child :
        ...
        if (inline_container_fragments->Contains(key)) {
          ...
          if (!value.first)
            value.first = line_box_child;
          value.second = line_box_child;
...
#if DCHECK_IS_ON()
  // DCHECK that each key has assigned fragments.
  if (!is_empty) {
    // Empty boxes size can be deduced from containing block?
    for (auto fragments = inline_container_fragments->begin();
         fragments != inline_container_fragments->end(); ++fragments) {
      DCHECK(fragments->value.first);
      DCHECK(fragments->value.second);
    }
  }
#endif
}
