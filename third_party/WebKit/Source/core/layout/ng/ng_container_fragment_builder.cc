...
NGContainerFragmentBuilder::AddInlineOutOfFlowChildCandidate(
  ...
  DCHECK(child);
  // Fixed positioned children are contained inside inline container.
    ...
    if (child->IsLineBox()) {
      for (auto& line_box_child :
        ...
        if (line_box_child->IsText()) {
          ...
          // TODO(atotic)
          // kojii: A box fragment may contain text or another box as children,
          // so you may want to look into them recursively?
          key = line_box_child->GetLayoutObject();
        }
