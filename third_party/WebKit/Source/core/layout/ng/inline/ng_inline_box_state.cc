...
NGInlineBoxState* NGInlineLayoutStateStack::OnOpenTag(
    ...
    const NGLineBoxFragmentBuilder::ChildList& children,
    LayoutUnit position) {
  DCHECK(item.Style());
  NGInlineBoxState* box = OnOpenTag(*item.Style(), children);
  box->item = &item;
...
NGInlineBoxState* NGInlineLayoutStateStack::OnOpenTag(
  ...
  NGInlineBoxState* box = &stack_.back();
  box->fragment_start = children.size();
  box->style = &style;
  return box;
}

NGInlineBoxState* NGInlineLayoutStateStack::OnCloseTag(
    NGLineBoxFragmentBuilder::ChildList* children,
    NGInlineBoxState* box,
    FontBaseline baseline_type) {
  EndBoxState(box, children, baseline_type);
...
void NGInlineLayoutStateStack::EndBoxState(
    ...
    FontBaseline baseline_type) {
  if (box->needs_box_fragment)
    AddBoxFragmentPlaceholder(box, children, baseline_type);

  PositionPending position_pending =
      ApplyBaselineShift(box, children, baseline_type);
...
void NGInlineLayoutStateStack::AddBoxFragmentPlaceholder(
  ...
  if (box->fragment_start == children->size() &&
...
void NGInlineLayoutStateStack::CreateBoxFragments(
  ...
  new_children.ReserveInitialCapacity(children->size());
...
NGInlineLayoutStateStack::ApplyBaselineShift(NGInlineBoxState* box,
                                             NGLineBoxFragmentBuilder::ChildList* chlidren,
                                             FontBaseline baseline_type) {
  ...
  if (!box->pending_descendants.IsEmpty()) {
    for (auto& child : box->pending_descendants) {
      ...
      chlidren->MoveInBlockDirection(baseline_shift, child.fragment_start,
                                     child.fragment_end);
    }
  ...
  unsigned fragment_end = chlidren->size();
  ...
  chlidren->MoveInBlockDirection(baseline_shift, box->fragment_start,
                                 fragment_end);
  return kPositionNotPending;
}
