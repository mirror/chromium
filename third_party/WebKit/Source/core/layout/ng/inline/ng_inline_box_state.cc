...
NGInlineBoxState* NGInlineLayoutStateStack::OnOpenTag(
  ...
  if (box->has_start_edge) {
    ...
    DCHECK(!item_result.margins.inline_start);
...
void NGInlineBoxState::SetLineRightForBoxFragment(
  ...
  if (has_end_edge) {
    ...
    DCHECK(!item_result.margins.inline_end);
    DCHECK(!item_result.inline_size);
  }
}
...
void NGInlineLayoutStateStack::AddBoxFragmentPlaceholder(
  ...
  if (box->has_start_edge) {
    box_data.has_start_edge = box->has_start_edge;
    box_data.margin_inline_start = box->margin_inline_start;
    box_data.margin_border_padding_inline_start =
        box->margin_border_padding_inline_start;
  }
  if (box->has_end_edge) {
    box_data.has_end_edge = box->has_end_edge;
    box_data.margin_inline_end = box->margin_inline_end;
    box_data.margin_border_padding_inline_end =
  ...
  if (fragment_end > box->fragment_start) {
    ...
    offset.inline_offset += box_data.margin_inline_start;
...
LayoutUnit NGInlineLayoutStateStack::ComputeInlinePositions(
  ...
  for (auto& box_data : box_data_list_) {
    ...
    scoped_refptr<NGLayoutResult> box_fragment =
        box_data.CreateBoxFragment(line_box);
    NGLogicalOffset offset(line_left_offset + box_data.margin_inline_start,
...
NGInlineLayoutStateStack::BoxData::CreateBoxFragment(
  ...
  LayoutUnit border_padding_line_left =
      margin_border_padding_inline_start - margin_inline_start;
  LayoutUnit border_padding_line_right =
      margin_border_padding_inline_end - margin_inline_end;
  if (IsLtr(style.Direction())) {
    box.SetBorderEdges({true, has_end_edge, true, has_start_edge});
  } else {
    box.SetBorderEdges({true, has_start_edge, true, has_end_edge});
    std::swap(border_padding_line_left, border_padding_line_right);
  }
