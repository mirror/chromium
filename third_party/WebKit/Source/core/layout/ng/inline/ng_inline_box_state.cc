...
void NGInlineLayoutStateStack::DeactivateLineHeightQuirk() {
  if (line_height_quirk_active_) {
    for (auto& box : stack_)
      box.metrics.Unite(box.text_metrics);

    line_height_quirk_active_ = false;
  }
}
...
NGInlineBoxState* NGInlineLayoutStateStack::OnBeginPlaceItems(
    ...
    FontBaseline baseline_type,
    bool line_height_quirk_active) {
  line_height_quirk_active_ = line_height_quirk_active;
