...
void NGInlineBoxState::SetNeedsBoxFragment(bool when_empty) {
  ...
  if (!needs_box_fragment) {
    ...
    bool has_line_left_edge = item->HasStartEdge();
...
void NGInlineBoxState::SetLineRightForBoxFragment(
  ...
  // continuation.
  // TODO(kojii): Needs review when we change SplitInlines().
  border_edges.line_right = item.HasEndEdge();
}
...
void NGInlineLayoutStateStack::AddBoxFragmentPlaceholder(
  ...
  // TODO: bidi level?
  line_box->AddChild(layout_result, offset, box_data.size.inline_size, 0u);
...
NGInlineLayoutStateStack::BoxData::CreateBoxFragment(
  ...
  for (unsigned i = fragment_start; i < fragment_end; i++) {
    ...
    // Leave out-of-flow fragments as is. They need to be at the top level so
    // that NGInlineLayoutAlgorithm can handle them later.
