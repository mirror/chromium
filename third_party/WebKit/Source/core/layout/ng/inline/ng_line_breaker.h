...
class CORE_EXPORT NGLineBreaker {
 ...
 private:
  ...
  struct LineData {
    ...
    // True if the current block offset has floats or exclusions, including
    // intruding floats. Valid only when HasAvailableWidth().
    bool has_floats_or_intruding_floats = false;
