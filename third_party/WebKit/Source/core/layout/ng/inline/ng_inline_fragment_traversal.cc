...
bool CollectAncestors(const NGPhysicalContainerFragment& container,
                      ...
                      const NGPhysicalOffset offset_to_container_box,
                      Vector<NGPhysicalFragmentWithOffset>& ancestors) {
  ancestors.push_back(
      NGPhysicalFragmentWithOffset{&container, offset_to_container_box});
  for (const auto& child : container.Children()) {
    ...
    if (CollectAncestors(ToNGPhysicalContainerFragment(*child), target,
                         child->Offset() + offset_to_container_box,
                         ancestors)) {
      return true;
    }
  }
...
Vector<NGPhysicalFragmentWithOffset> NGInlineFragmentTraversal::AncestorsOf(
    const NGPhysicalContainerFragment& container,
    const NGPhysicalFragment& fragment) {
  DCHECK_NE(&container, &fragment);
  Vector<NGPhysicalFragmentWithOffset> ancestors;
...
NGInlineFragmentTraversal::InclusiveAncestorsOf(
  ...
  results.push_back(NGPhysicalFragmentWithOffset{
      &fragment, fragment.Offset() + results.back().offset_to_container_box});
  results.Reverse();
  return results;
}
