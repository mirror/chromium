...
TEST_F(NGInlineFragmentTraversalTest, AncestorsOf) {
  SetBodyInnerHTML("<div id=t>x<b>y<i>z<u id=target>foo</u>z</i>y</b>x</div>");
  ...
  EXPECT_NEXT_BOX(iter);  // Fragment of <u>
  EXPECT_NEXT_BOX(iter);  // Fragment of <i>
  EXPECT_NEXT_BOX(iter);  // Fragment of <b>
  EXPECT_NEXT_LINE_BOX(iter);
