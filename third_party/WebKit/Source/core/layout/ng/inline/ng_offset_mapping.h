...
struct NGInlineFragmentPosition {
  ...
  Optional<unsigned> text_offset;  // Applies only to text fragment

  enum class AnchorType { kBefore, kAfter };
  Optional<AnchorType> anchor_type;  // Applies only to atomic inline fragment

  static NGInlineFragmentPosition FromTextFragment(
      const NGPhysicalFragmentWithOffset&,
      unsigned);
  static NGInlineFragmentPosition FromAtomicInline(
      const NGPhysicalFragmentWithOffset&,
      AnchorType);
};
