...
NGInlineFragmentPosition ComputeNGInlineFragmentPosition(
  ...
  for (const auto& child : children) {
    ...
    if (child.fragment->IsText()) {
      ...
      NGInlineFragmentPosition candidate = {child.fragment, child.offset_to_container_box, offset_in_box};
    ...
    if (child.fragment->IsBox() && child.fragment->IsInlineBlock()) {
      ...
      if (offset != offset_before && offset != offset_after)
        continue;
      NGInlineFragmentPosition candidate = {child.fragment, child.offset_to_container_box, offset == offset_after};
...
LocalCaretRect ComputeLocalCaretRectNG(const PositionWithAffinity& position) {
  ...
  NGPhysicalOffsetRect fragment_local_rect = fragment_position.fragment->LocalCaretRect(fragment_position.offset_in_box);
  NGPhysicalOffsetRect context_local_rect = fragment_local_rect + fragment_position.offset_to_container_box;
  return {fragment_position.fragment->GetLayoutObject(), context_local_rect.ToLayoutRect()};
}
