...
bool IsLineBreakFragment(const NGPhysicalFragment& fragment) {
  // TODO(xiaochengh): Move this function to NGPhysicalTextFragment.
  // TODO(xiaochengh): Introduce NGPhysicalTextFragment::TextType instead of
  // check the fragment string.
  if (!fragment.IsText())
    return false;
...
bool IsGeneratedText(const NGPhysicalTextFragment& fragment) {
  // TODO(xiaochengh): Move this function to NGPhysicalTextFragment.
...
CaretPositionResolution TryResolveCaretPositionInTextFragment(
  ...
  if (offset == fragment.EndOffset() && !IsLineBreakFragment(fragment)) {
