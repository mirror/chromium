...
NGPhysicalOffsetRect ComputeLocalCaretRectAtTextOffset(
  ...
  // Adjust the location in the inline direction to ensure that it completely
  // falls in the union of line box and containing block, and then round it to
  // the nearest pixel.
  if (is_horizontal) {
    const LayoutUnit inline_min =
        std::min(LayoutUnit(), line_box.offset_to_container_box.left);
    caret_location.left = std::max(caret_location.left, inline_min);
    const LayoutUnit inline_max =
        std::max(context.Size().width, line_box.RectInContainerBox().Right());
    caret_location.left =
        std::min(caret_location.left, inline_max - caret_width);
    caret_location.left = LayoutUnit(caret_location.left.Round());
  } else {
    const LayoutUnit inline_min =
        std::min(LayoutUnit(), line_box.offset_to_container_box.top);
    caret_location.top = std::max(caret_location.top, inline_min);
    const LayoutUnit inline_max =
        std::max(context.Size().height, line_box.RectInContainerBox().Bottom());
    caret_location.top =
        std::min(caret_location.top, inline_max - caret_height);
