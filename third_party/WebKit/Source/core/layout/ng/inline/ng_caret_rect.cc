...
CaretPositionResolution TryResolveCaretPositionInTextFragment(
  ...
  NGCaretPosition candidate = {&fragment, NGPhysicalOffset(),
...
CaretPositionResolution TryResolveCaretPositionByBoxFragmentSide(
  ...
  NGCaretPosition candidate{&fragment, NGPhysicalOffset(), position_type,
                            WTF::nullopt};
...
NGCaretPosition ComputeNGCaretPosition(const LayoutBlockFlow& context,
  ...
  for (const auto& line : lines) {
    ...
    for (const auto& child : line.fragments) {
      CaretPositionResolution resolution = TryResolveCaretPositionWithFragment(
          *child.fragment, current_line, last_line, offset, affinity);
