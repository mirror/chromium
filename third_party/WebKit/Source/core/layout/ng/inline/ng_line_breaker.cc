...
NGLineBreaker::LineBreakState NGLineBreaker::HandleAtomicInline(
  ...
  // TODO(kojii): The block layout needs to use :first-line style when needed,
  // and there can be nested inline-blocks. We can pass UseFirstLineStyle() or
  // style through constraint space, but it doesn't solve nested case. Revisit
  // after discussion on nested case.
