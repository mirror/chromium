...
void NGLineBreaker::FindNextLayoutOpportunity() {
  ...
  // |IsAtEnd()| gives us if we have any floats/exclusions in the current
  // opportunity, including intruding floats from previous blocks.
  // line_.has_floats_or_intruding_floats = !iter.IsAtEnd();
...
void NGLineBreaker::HandleOverflow(NGLineInfo* line_info) {
  ...
  WTF::Optional<LayoutUnit> float_offset = GetClearanceOffset(constraint_space_->Exclusions(), EClear::kBoth);
  bool has_floats_or_intruding_floats = float_offset && (container_builder_->BfcOffset()->block_offset + content_offset_.block_offset) >= (float_offset.value());
  ...
  if (has_floats_or_intruding_floats) {
