...
// @return if the current block offset has floats or exclusions, including
// intruding floats.
bool NGLineBreaker::HasFloatsOrIntrudingFloats() const {
#if 0
  Optional<LayoutUnit> float_end_offset =
      GetClearanceOffset(constraint_space_->Exclusions(), EClear::kBoth);
  if (!float_end_offset.has_value())
    return false;
  const NGLogicalOffset& bfc_offset = container_builder_->BfcOffset().value();
  return content_offset_.block_offset <
         float_end_offset.value() - bfc_offset.block_offset;
#else
  const NGLogicalOffset& bfc_offset = container_builder_->BfcOffset().value();
  ...
  while (true) {
    ...
    if (iter.IsAtEnd()) {
      return content_offset_.block_offset <
             opportunity.BlockStartOffset() - bfc_offset.block_offset;
    }
  }
#endif
}
...
void NGLineBreaker::FindNextLayoutOpportunity() {
  ...
  DCHECK_EQ(iter.IsAtEnd(), !HasFloatsOrIntrudingFloats());
}
