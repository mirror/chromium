...
NGLineBreaker::LineBreakState NGLineBreaker::HandleFloat(
  ...
  bool float_does_not_fit =
      ...
      line_.exclusion_space->ClearanceOffset(float_style.Clear()) >
          bfc_block_offset_;
  ...
  if (!unpositioned_floats_->IsEmpty() || float_does_not_fit) {
    ...
    if (float_style.Floating() == EFloat::kLeft) {
      ...
      line_.line_right_bfc_offset = std::min(
          line_.line_right_bfc_offset, positioned_float.bfc_offset.line_offset);
    }
...
void NGLineBreaker::Rewind(NGLineInfo* line_info, unsigned new_end) {
  ...
  // TODO(ikilpatrick): Add DCHECK that we don't rewind past floats.
