...
// Justify the line. This changes the size of items by adding spaces.
void NGInlineLayoutAlgorithm::ApplyJustify(NGLineInfo* line_info) {
  ...
  if (!spacing.SetExpansion(expansion, Node().BaseDirection(),
    ...
    return;  // no expansion opportunities exist.
  }
  ...
  while (true) {
    switch (text_align) {
      ...
      case ETextAlign::kJustify:
        // Apply justification in earlier phase, see PlaceItems().
        NOTREACHED();
        return;
    }
    NOTREACHED();
