...
RefPtr<NGLayoutResult> NGInlineLayoutAlgorithm::Layout() {
  ...
  if (!Node().IsEmptyInline()) {
    ...
    DCHECK(ConstraintSpace().MarginStrut().IsInitial());

    container_builder_.SetBfcOffset(ConstraintSpace().BfcOffset());
  } else if (ConstraintSpace().FloatsBfcOffset()) {
    // TODO(ikilpatrick): Maybe remove this once we've got an additional path
    // for handling unpositioned floats.
    container_builder_.SetBfcOffset(
        ConstraintSpace().FloatsBfcOffset().value());
  }
  ...
  if (container_builder_.BfcOffset()) {
    LayoutUnit content_size =
        container_builder_.Metrics().LineHeight().ClampNegativeToZero();
    NGBfcOffset origin_point = GetOriginPointForFloats(
        ContainerBfcOffset(), content_size /*LayoutUnit()*/);
