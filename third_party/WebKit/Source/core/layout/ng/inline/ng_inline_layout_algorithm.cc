  ...
  if (!out_of_flow_items.IsEmpty()) {
    for (const auto& out_of_flow_item : out_of_flow_items) {
      LayoutBox* layout_box =
          ToLayoutBox(out_of_flow_item.item->GetLayoutObject());
      NGLogicalOffset static_position;
      if (layout_box->Style()->IsOriginalDisplayInlineType()) {
      ...
      container_builder_.AddOutOfFlowChildCandidate(NGBlockNode(layout_box),
                                                    static_position);
    }
  }
