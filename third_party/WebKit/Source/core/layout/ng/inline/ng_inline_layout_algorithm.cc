...
void NGInlineLayoutAlgorithm::PlaceItems(
  ...
  // TODO(kojii): We don't have BfcOffset if all what we have are oof.
  // This code is wrong, but we crash without this. What should we do?
  if (!container_builder_.BfcOffset().has_value()) {
    DCHECK(std::all_of(
        line_box_.begin(), line_box_.end(),
        [](const auto& child) { return child.out_of_flow_layout_box; }));
    for (const auto& child : line_box_) {
