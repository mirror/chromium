...
bool NGInlineLayoutAlgorithm::PlaceItems(
  ...
  NGLogicalOffset line_box_offset(line_info->LineLeft(), LayoutUnit());
  ...
  if (text_align != ETextAlign::kJustify)
    ApplyTextAlign(text_align, &line_box_offset.inline_offset, inline_size, available_width);
...
void NGInlineLayoutAlgorithm::ApplyJustify(NGLineInfo* line_info,
  ...
  if (!spacing.SetExpansion(expansion, Node().BaseDirection(),
  ...
  for (NGInlineItemResult& item_result : line_info->Results()) {
    ...
    RefPtr<ShapeResult> shape_result = item_result.shape_result->MutableUnique();
