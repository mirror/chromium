...
void NGInlineLayoutAlgorithm::CreateLine(NGLineInfo* line_info,
  ...
  if (!Node().IsBidiEnabled()) {
    if (box_states_->HasBoxFragments()) {
      // TODO(kojii): PrepareForReorder() isn't needed for LTR. We do this to
      // share the logic between LTR and RTL, and to get more coverage for RTL,
      // but we could have optimized code path for LTR.
      box_states_->PrepareForReorder(&line_box_);
      box_states_->UpdateAfterReorder(&line_box_);
    }
  } else if (!box_states_->HasBoxFragments()) {
    BidiReorder();
  } else {
    box_states_->PrepareForReorder(&line_box_);
    BidiReorder();
...
void NGInlineLayoutAlgorithm::BidiReorder() {
  ...
  for (unsigned logical_index : indices_in_visual_order) {
    ...
    DCHECK(!logical_items[logical_index].HasFragment());
  }
}
