...
bool NGInlineLayoutAlgorithm::PlaceItems(
  ...
  for (auto& item_result : *line_items) {
    ...
    if (item.Type() == NGInlineItem::kText ||
      ...
      if (item_result.shape_result) {
        ...
        if (quirks_mode_ && children_.IsEmpty())
          box->ActivateTextMetrics();
      ...
      children_.AddChild(std::move(text_fragment), {position, box->text_top});
    } else if (item.Type() == NGInlineItem::kOpenTag) {
      box = box_states_.OnOpenTag(item, item_result, children_, position);
      ...
      box = box_states_.OnCloseTag(&children_, box, baseline_type_);
      continue;
    } else if (item.Type() == NGInlineItem::kAtomicInline) {
      box = PlaceAtomicInline(item, &item_result, *line_info, position);
    } else if (item.Type() == NGInlineItem::kListMarker) {
      list_marker_index = children_.size();
      PlaceListMarker(item, &item_result, *line_info);
      DCHECK_GT(children_.size(), list_marker_index.value());
  ...
  box_states_.OnEndPlaceItems(&children_, baseline_type_, position);
  const NGLineHeightMetrics& line_box_metrics = box_states_.LineBoxState().metrics;
  ...
  children_.MoveInBlockDirection(line_box_metrics.ascent);
  ...
  line_box.AddChildren(children_);
  ...
  if (box->CanAddTextOfStyle(*style)) {
    ...
    RefPtr<ComputedStyle> text_style =
        ComputedStyle::CreateAnonymousStyleWithDisplay(*style,
                                                       EDisplay::kInline);
    NGInlineBoxState* box = box_states_.OnOpenTag(*text_style, children_);
    box->ComputeTextMetrics(*text_style, baseline_type_, false);
    PlaceText(std::move(shape_result), std::move(style), position, box,
              text_builder);
    box_states_.OnCloseTag(&children_, box, baseline_type_);
  }
}
...
NGInlineBoxState* NGInlineLayoutAlgorithm::PlaceAtomicInline(
  ...
  NGInlineBoxState* box =
      box_states_.OnOpenTag(item, *item_result, children_, position);

  PlaceLayoutResult(item_result, position, box);

  return box_states_.OnCloseTag(&children_, box, baseline_type_);
}
...
void NGInlineLayoutAlgorithm::PlaceLayoutResult(
  ...
  if (!RuntimeEnabledFeatures::LayoutNGPaintFragmentsEnabled()) {
    ...
    RefPtr<NGPhysicalTextFragment> text_fragment = text_builder.ToTextFragment(
        ...
        item_result->end_offset);
    children_.AddChild(std::move(text_fragment), {position, line_top});
    // We need the box fragment as well to compute VisualRect() correctly.
  }

  children_.AddChild(std::move(item_result->layout_result),
                     {position, line_top});
}
