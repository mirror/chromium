...
#define TEST_CARET(caret, fragment_, type_, offset_)            \
  {                                                             \
    EXPECT_EQ(caret.fragment, fragment_);                       \
...
TEST_F(NGCaretRectTest, CaretPositionAtSoftLineWrap) {
  ...
  const NGPhysicalFragment* foo_fragment = text_fragments[0].fragment;
  const NGPhysicalFragment* bar_fragment = text_fragments[1].fragment;
