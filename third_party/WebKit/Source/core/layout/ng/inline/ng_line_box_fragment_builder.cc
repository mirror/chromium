...
void NGLineBoxFragmentBuilder::ComputeLocalVisualRect(
  ...
  if (physical_fragment.IsText()) {
    ...
    visual_rect->Unite(child_visual_rect + offset);
    return;
  }
  ...
  if (physical_fragment.IsBox()) {
    ...
    if (layout_object->IsAtomicInlineLevel() ||
      ...
      LayoutRect child_visual_rect =
          ToLayoutBox(layout_object)->LogicalVisualOverflowRectForPropagation();
      visual_rect->Unite(NGLogicalRect(child_visual_rect) + offset);
      return;
    }
