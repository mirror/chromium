...
void UpdateTouchPointerEventInit(const WebTouchPoint& touch_point,
  ...
  if (target_frame) {
    ...
    FloatSize point_radius =
        FloatSize(touch_point.radius_x, touch_point.radius_y).ScaledBy(scale_factor);
  ...
  pointer_event_init->setPressure(
      GetPointerEventPressure(touch_point.force, pointer_event_init->buttons()));
...
void UpdateMousePointerEventInit(const WebMouseEvent& mouse_event,
  ...
  pointer_event_init->setPressure(
      GetPointerEventPressure(mouse_event.force, pointer_event_init->buttons()));
...
PointerEvent* PointerEventFactory::Create(
  ...
  if (pointer_event_name == EventTypeNames::pointerdown ||
    ...
    // TODO(mustaq): Fix when the spec starts supporting hovering erasers.
    if (mouse_event.pointer_type == WebPointerProperties::PointerType::kEraser &&
...
void PointerEventFactory::Clear() {
  ...
  pointer_id_mapping_.insert(
      kMouseId,
      PointerAttributes(IncomingId(WebPointerProperties::PointerType::kMouse, 0),
                        0));
...
int PointerEventFactory::AddIdAndActiveButtons(const IncomingId p,
  ...
  pointer_id_mapping_.insert(mapped_id, PointerAttributes(p, is_active_buttons));
  return mapped_id;
}
...
Vector<int> PointerEventFactory::GetPointerIdsOfType(
  ...
  for (auto iter = pointer_id_mapping_.begin(); iter != pointer_id_mapping_.end();
       ++iter) {
