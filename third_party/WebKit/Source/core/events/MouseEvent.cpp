...
const LayoutObject* FindTargetLayoutObject(Node*& target_node) {
  ...
  DCHECK(!target_node || (target_node->IsSVGElement() &&
...
MouseEvent* MouseEvent::Create(const AtomicString& event_type,
  ...
  TimeTicks timestamp =
      underlying_event ? underlying_event->PlatformTimeStamp() : TimeTicks::Now();
  MouseEvent* created_event = new MouseEvent(
      event_type, true, true, view, 0, screen_x, screen_y, 0, 0, 0, 0, modifiers,
      0, 0, nullptr, timestamp, synthetic_type, String());
...
MouseEvent::MouseEvent(const AtomicString& event_type,
    ...
    : UIEventWithKeyState(
      ...
      region_(region) {
  IntPoint root_frame_coordinates = FlooredIntPoint(event.PositionInRootFrame());
...
unsigned short MouseEvent::WebInputEventModifiersToButtons(unsigned modifiers) {
  ...
  if (modifiers & WebInputEvent::kLeftButtonDown)
    buttons |= static_cast<unsigned short>(WebPointerProperties::Buttons::kLeft);
  ...
  if (modifiers & WebInputEvent::kBackButtonDown)
    buttons |= static_cast<unsigned short>(WebPointerProperties::Buttons::kBack);
...
void MouseEvent::InitMouseEventInternal(
    ...
    unsigned short buttons) {
  InitUIEventInternal(type, can_bubble, cancelable, related_target, view, detail,
                      source_capabilities);
...
DispatchEventResult MouseEventDispatchMediator::DispatchEvent(
  ...
  mouse_event.GetEventPath().AdjustForRelatedTarget(dispatcher.GetNode(),
                                                mouse_event.relatedTarget());
