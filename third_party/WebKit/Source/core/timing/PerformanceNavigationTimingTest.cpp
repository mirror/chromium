...
TEST_F(PerformanceNavigationTimingTest, GetNavigationType) {
  page_holder_->GetPage().SetVisibilityState(kPageVisibilityStatePrerender, false);
  AtomicString returned_type =
      GetNavigationType(kNavigationTypeBackForward, &page_holder_->GetDocument());
  ...
  page_holder_->GetPage().SetVisibilityState(kPageVisibilityStateVisible, false);
