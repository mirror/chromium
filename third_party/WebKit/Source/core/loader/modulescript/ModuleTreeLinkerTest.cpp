...
class ModuleTreeLinkerTestModulator final : public DummyModulator {
 public:
  ...
  ModuleScript* ResolveSingleModuleScriptFetch(
    ...
    pending_clients_.at(url)->NotifyModuleLoadFinished(module_script);
    pending_clients_.erase(url);
...
TEST_F(ModuleTreeLinkerTest, FetchTreeWith3Deps1Fail) {
  ...
  // FOXME: Currently this CL early-exits.
  EXPECT_TRUE(client->WasNotifyFinished());
  ...
  EXPECT_TRUE(client->WasNotifyFinished());
  // FOXME: Expectation changed due to early-exiting.
...
TEST_F(ModuleTreeLinkerTest, FetchDependencyTree) {
  ...
  EXPECT_TRUE(client->WasNotifyFinished());
  ASSERT_TRUE(client->GetModuleScript());
  // FOXME: this CL synchronously instantiate the root module script on the
  // finish of last dependent module load.
  EXPECT_TRUE(client->GetModuleScript()->HasInstantiated());
}
