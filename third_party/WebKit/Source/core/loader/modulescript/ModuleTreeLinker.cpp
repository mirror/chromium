...
#define EXACT_ERROR_REPORTING 1
...
void ModuleTreeLinker::FetchSingle(const ModuleScriptFetchRequest& request) {
  DCHECK(!reached_url_set_->IsAlreadyBeingFetched(request.Url()));
...
// steps.
#if EXACT_ERROR_REPORTING
  if (!module_script || module_script->IsErrored() ||
      module_script->HasInstantiated()) {
    if (num_incomplete_fetches_ == 0)
      Instantiate();
    return;
  }
#else
  // Step 7.1. If result is null,
  ...
  // Step 7.2: If result is errored, then set the pre-instantiation error for
  // module script to result's error, Asynchronously complete this algorithm
  // with module script, aborting these steps.
  // FOXME(hiroshige): We have to do Step 7-related things separately if
  // we want to make the error reported deterministically.
...
bool ModuleTreeLinker::PropagateError(ModuleTreeReachedUrlSet* reached_url_set,
                                      ModuleScript* module_script) {
  // Step 7.1. If result is null,
  // asynchronously complete this algorithm with null, aborting these steps.
  if (!module_script) {
  ...
  for (const auto& module_request : module_requests) {
    ...
    if (reached_url_set->IsAlreadyBeingFetched(url))
      continue;
    reached_url_set->ObserveModuleTreeLink(url);
...
void ModuleTreeLinker::Instantiate() {
  ...
  if (result_ && !result_->IsErrored()) {
    ModuleTreeReachedUrlSet* reached_url_set = new ModuleTreeReachedUrlSet;
    reached_url_set->ObserveModuleTreeLink(result_->BaseURL());
