...
void ModuleTreeLinker::AdvanceState(State new_state) {
  ...
  if (state_ == State::kFinished) {
    ...
    // FOXME: this is moved from
    // ModulePendingScriptTreeClient::NotifyModuleTreeLoadFinished(). |result_|
    // should be either: - nullptr (fetch error), - module script with an error
    // to rethrow (parse/instantiation error), or - module script ready for
    // evaluation (instantiated, evaluated, or
    //   evaluation error).
    /*
    DCHECK(!result_ || result_->IsReadyForEvaluationForAssert())
