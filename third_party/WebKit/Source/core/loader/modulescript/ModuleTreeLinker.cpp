...
void ModuleTreeLinker::FetchRootInline(ModuleScript* module_script) {
  ...
  // Store the |module_script| reference here to ensure the module script is
...
void ModuleTreeLinker::InitiateIMSGF(const ModuleScriptFetchRequest& request,
...
void ModuleTreeLinker::NotifyModuleLoadFinished(ModuleScript* module_script) {
  ...
  // [IMSGF] Steps 4 and 5 are merged below.
  ...
  for (const auto& module_request : module_requests) {
    ...
    if (!visited_set_.Contains(url)) {
      ...
      // [FD] Step 5.3.2. Append url to visited set.
      //
      // This step is deferred to InitiateIMSGF() below.
  ...
  for (size_t i = 0; i < urls.size(); ++i) {
    ...
    ModuleScriptFetchRequest request(
        ...
        positions[i]);
    InitiateIMSGF(request, ModuleGraphLevel::kDependentModuleFetch);
  }
...
//
// FOXME before commit: Create a wpt test that exposes the behavior change.
//
