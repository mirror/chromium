...
TEST_F(InteractiveDetectorTest, FMP_DCL_FmpDetect) {
  ...
  // Reached CI at FMP.
  EXPECT_EQ(GetConsistentlyInteractiveTime(), t0 + 5.0);
}
...
TEST_F(InteractiveDetectorTest, InstantDetectionAtFmpDetectIfPossible) {
  ...
  // detect that we reached CI at DCL.
  EXPECT_EQ(GetConsistentlyInteractiveTime(), t0 + 5.0);
}
...
TEST_F(InteractiveDetectorTest, FmpDetectFiresAfterLateLongTask) {
  ...
  // seconds after fmp_time. We should instantly detect we reached CI at FMP.
  EXPECT_EQ(GetConsistentlyInteractiveTime(), t0 + 3.0);
}
...
TEST_F(InteractiveDetectorTest, FMP_FmpDetect_DCL) {
  ...
  // CI reached at DCL.
  EXPECT_EQ(GetConsistentlyInteractiveTime(), t0 + 9.0);
}
...
TEST_F(InteractiveDetectorTest, LongTaskBeforeFMPDoesNotAffectTTCI) {
  ...
  // CI reached at FMP.
  EXPECT_EQ(GetConsistentlyInteractiveTime(), t0 + 8.0);
}
...
TEST_F(InteractiveDetectorTest, DCL_FMP_FmpDetect_LT) {
  ...
  // CI reached at long task end.
  EXPECT_EQ(GetConsistentlyInteractiveTime(), t0 + 7.1);
}
...
TEST_F(InteractiveDetectorTest, FMP_FmpDetect_LT_DCL) {
  ...
  // Run till 5 seconds after long task end.
  RunTillTimestamp((t0 + 7.1) + 5.0 + 0.1);
  // CI reached at DCL. Note that we do not need to wait for DCL + 5 seconds.
  EXPECT_EQ(GetConsistentlyInteractiveTime(), t0 + 8.0);
}
...
TEST_F(InteractiveDetectorTest, DclIsMoreThan5sAfterFMP) {
  ...
  // CI reached at long task 2 end.
  EXPECT_EQ(GetConsistentlyInteractiveTime(), (t0 + 11.1));
}
...
TEST_F(InteractiveDetectorTest, LongEnoughQuietWindowBetweenFMPAndFmpDetect) {
  ...
  // recently, we should be able to detect that the page already achieved CI at
  // FMP.
  EXPECT_EQ(GetConsistentlyInteractiveTime(), t0 + 3.0);
}
...
TEST_F(InteractiveDetectorTest, NetworkBusyEndIsNotConsistentlyInteractive) {
  ...
  // CI reached at long task 2 end, NOT at network busy end.
  EXPECT_EQ(GetConsistentlyInteractiveTime(), t0 + 13.1);
}
...
TEST_F(InteractiveDetectorTest, LateLongTaskWithLateFMPDetection) {
  ...
  // CI reached at long task 1 end, NOT at long task 2 end.
  EXPECT_EQ(GetConsistentlyInteractiveTime(), t0 + 7.1);
}

TEST_F(InteractiveDetectorTest, IntermittentNetworkBusyBlocksCI) {
  double t0 = MonotonicallyIncreasingTime();
  SimulateNavigationStart(t0);
  SetActiveConnections(1);
  SimulateDOMContentLoadedEnd(t0 + 2.0);
  ...
  // CI reached at long task 2 end.
  EXPECT_EQ(GetConsistentlyInteractiveTime(), t0 + 14.1);
}
...
TEST_F(InteractiveDetectorTestWithDummyPage, TaskLongerThan5sBlocksCI) {
  double t0 = MonotonicallyIncreasingTime();
  ...
  // We should be able to detect CI 5s after the end of long task.
  platform_->RunForPeriodSeconds(5.1);
  EXPECT_EQ(detector->GetConsistentlyInteractiveTime(), GetDummyTaskEndTime());
}

TEST_F(InteractiveDetectorTestWithDummyPage, LongTaskAfterCIDoesNothing) {
  double t0 = MonotonicallyIncreasingTime();
  InteractiveDetector* detector = InteractiveDetector::From(GetDocument());
  detector->SetNavigationStartTime(t0);
  platform_->RunForPeriodSeconds(4.0);
  ...
  // We should be able to detect CI 5s after the end of long task.
  platform_->RunForPeriodSeconds(5.1);
  EXPECT_EQ(detector->GetConsistentlyInteractiveTime(), long_task_1_end_time);
  ...
  platform_->RunUntilIdle();
  // Wait 5 seconds to see if CI time changes.
  platform_->RunForPeriodSeconds(5.1);
  // CI time should not change.
  EXPECT_EQ(detector->GetConsistentlyInteractiveTime(), long_task_1_end_time);
}
