...
class InteractiveDetectorTest : public ::testing::Test {
 ...
 protected:
  ...
  void SimulateNavigationStart(double nav_start_time) {
    ...
    detector_->SetNavigationStartTime(TimeTicks::FromSeconds(nav_start_time));
  }

  void SimulateLongTask(double start, double end) {
    CHECK(end - start >= 0.05);
    RunTillTimestamp(end);
    detector_->OnLongTaskDetected(TimeTicks::FromSeconds(start),
                                  TimeTicks::FromSeconds(end));
  }

  void SimulateDOMContentLoadedEnd(double dcl_time) {
    RunTillTimestamp(dcl_time);
    detector_->OnDomContentLoadedEnd(TimeTicks::FromSeconds(dcl_time));
  }

  void SimulateFMPDetected(double fmp_time, double detection_time) {
    RunTillTimestamp(detection_time);
    detector_->OnFirstMeaningfulPaintDetected(TimeTicks::FromSeconds(fmp_time));
  }

  void SimulateInteractiveInvalidatingInput(double timestamp) {
    RunTillTimestamp(timestamp);
    detector_->OnInvalidatingInputEvent(TimeTicks::FromSeconds(timestamp));
  }
  ...
  void SimulateResourceLoadBegin(double load_begin_time) {
    ...
    detector_->OnResourceLoadBegin(TimeTicks::FromSeconds(load_begin_time));
  ...
  void SimulateResourceLoadEnd(double load_finish_time) {
    ...
    detector_->OnResourceLoadEnd(TimeTicks::FromSeconds(load_finish_time));
  }

  double GetInteractiveTime() {
    return detector_->GetInteractiveTime().InSecondsF();
  }
...
TEST_F(InteractiveDetectorTest, InvalidatingUserInput) {
  ...
  EXPECT_EQ(GetInteractiveTime(), t0 + 7.1);
  EXPECT_EQ(GetDetector()->GetFirstInvalidatingInputTime().InSecondsF(),
            t0 + 5.0);
}
...
TEST_F(InteractiveDetectorTestWithDummyPage, TaskLongerThan5sBlocksTTI) {
  ...
  detector->SetNavigationStartTime(TimeTicks::FromSeconds(t0));
  ...
  // InteractiveDetector.
  detector->OnFirstMeaningfulPaintDetected(TimeTicks::FromSeconds(t0 + 3.0));
  ...
  EXPECT_EQ(detector->GetInteractiveTime().InSecondsF(), GetDummyTaskEndTime());
}
...
TEST_F(InteractiveDetectorTestWithDummyPage, LongTaskAfterTTIDoesNothing) {
  ...
  // We should be able to detect TTI 5s after the end of long task.
  platform_->RunForPeriodSeconds(5.1);
  EXPECT_EQ(detector->GetInteractiveTime().InSecondsF(), long_task_1_end_time);
