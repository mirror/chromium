...
// Detects when a page becomes First Interactive and Consistently Interactive.
// See https://goo.gl/veegmW for detailed description and motivation of First
// Interactive and Consistently Interactive.
// TODO(crbug.com/631203): This class currently only detects Consistently
// Interactive. Implement FirstInteractive.
class CORE_EXPORT InteractiveDetector
 ...
 public:
  ...
  // Functions for gathering signals relevant for Time to Interactive
  // determination.
  ...
  // Returns Consistently Interactive Time if already detected, or 0.0
  // otherwise.
 ...
 private:
  ...
  std::vector<Interval> main_thread_quiet_windows_;
  std::vector<Interval> network_quiet_windows_;
  ...
  // FindConsistentlyInteractiveCandidate.
  ...
  void UpdateNetworkQuietState(double request_count, double opt_current_time);
  ...
  void ConsistentlyInteractiveTimerFired(TimerBase*);
  void CheckConsistentlyInteractiveReached();

  // Finds a window of length kConsistentlyInteractiveWindowSeconds after
  // lower_bound such that both main thread and network are quiet. Returns the
  // end of last long task before that quiet window, or lower_bound, whichever
  // is bigger - this is called the Consistently Interactive Candidate. Returns
  // 0.0 if no such quiet window is found.
  double FindConsistentlyInteractiveCandidate(double lower_bound);
