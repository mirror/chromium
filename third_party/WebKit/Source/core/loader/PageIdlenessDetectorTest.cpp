...
class PageIdlenessDetectorTest : public ::testing::Test {
 protected:
  void SetUp() {
    platform_time_ = 1;
  ...
  PageIdlenessDetector& Detector() {
    return PageIdlenessDetector::From(GetDocument());
  }
 ...
 protected:
  static constexpr double kNetworkQuietWindowSeconds =
      PageIdlenessDetector::kNetworkQuietWindowSeconds;
...
TEST_F(PageIdlenessDetectorTest, NetworkQuietBasic) {
...
TEST_F(PageIdlenessDetectorTest, NetworkQuietWithLongTask) {
  EXPECT_TRUE(IsNetworkQuietTimerActive());
...
TEST_F(PageIdlenessDetectorTest, NetworkQuietWatchdogTimerFired) {
  EXPECT_TRUE(IsNetworkQuietTimerActive());
