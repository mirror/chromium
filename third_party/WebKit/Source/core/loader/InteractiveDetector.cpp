...
// This histogram tracks Consistently Interactive with First Meaningful Paint as
// the lower bound for quiet windows.
static const char kFmpConsistentlyInteractiveHistogramName[] =
    "PageLoad.Experimental.NavigationToFMPConsistentlyInteractive";
...
void InteractiveDetector::OnConsistentlyInteractiveDetected() {
  LongTaskDetector::Instance().UnregisterObserver(this);
  double nav_start_to_consistently_interactive =
      consistently_interactive_ - page_event_times_.nav_start;
  UMA_HISTOGRAM_CUSTOM_TIMES(
      kFmpConsistentlyInteractiveHistogramName,
      base::TimeDelta::FromSeconds(nav_start_to_consistently_interactive),
...
// opt_current_time is an optional argument, which, if non-zero, will save us a
// call to MonotonicallyIncreasingTime().
void InteractiveDetector::UpdateNetworkQuiescenceState(
    double request_count,
    double opt_current_time) {
  if (request_count <= kNetworkQuietMaximumConnections &&
      active_network_quiet_window_start_ == 0.0) {
    double current_time =
        opt_current_time ? opt_current_time : MonotonicallyIncreasingTime();
...
void InteractiveDetector::OnResourceLoadBegin() {
  ...
  // so we add one to it.
  UpdateNetworkQuiescenceState(ActiveConnections() + 1, 0.0);
}

void InteractiveDetector::OnResourceLoadEnd(double load_finish_time) {
  if (consistently_interactive_ != 0.0)
    return;
  UpdateNetworkQuiescenceState(ActiveConnections(), load_finish_time);
}
...
double InteractiveDetector::FindConsistentlyInteractiveCandidate(
  ...
  std::vector<Interval>::iterator it_mt = main_thread_quiet_windows_.begin();
  // Network iterator.
  std::vector<Interval>::iterator it_net = network_quiet_windows_.begin();
