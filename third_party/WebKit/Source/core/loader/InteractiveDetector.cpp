...
const char* InteractiveDetector::SupplementName = "InteractiveDetector";

InteractiveDetector* InteractiveDetector::From(Document& document) {
  InteractiveDetector* detector =
      static_cast<InteractiveDetector*>(Supplement<Document>::From(
          document, InteractiveDetector::SupplementName));
  if (!detector) {
    detector = new InteractiveDetector(document,
                                       new NetworkActivityChecker(&document));
    Supplement<Document>::ProvideTo(
        document, InteractiveDetector::SupplementName, detector);
  }
  return detector;
}
