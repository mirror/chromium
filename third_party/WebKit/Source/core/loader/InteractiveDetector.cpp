...
void InteractiveDetector::StartOrPostponeCITimer(TimeTicks timer_fire_time) {
  ...
  timer_fire_time += TimeDelta::FromSecondsD(0.001);
...
void InteractiveDetector::OnFirstMeaningfulPaintDetected(TimeTicks fmp_time) {
  ...
  DCHECK(page_event_times_.first_meaningful_paint.is_null());
  page_event_times_.first_meaningful_paint = fmp_time;
  if (CurrentTimeTicks() - fmp_time >= kTimeToInteractiveWindowSeconds) {
