...
static inline bool PageIsBeingDismissed(Document* document) {
  return document->PageDismissalEventBeingDispatched() != Document::kNoDismissal;
}
 ...
 public:
  ...
  Task(ImageLoader* loader,
    ...
    if (!isolate->GetCurrentContext().IsEmpty()) {
      ...
      script_state_ =
          ToScriptStateForMainWorld(loader->GetElement()->GetDocument().GetFrame());
...
static void ConfigureRequest(
  ...
  if (cross_origin != kCrossOriginAttributeNotSet) {
    request.SetCrossOriginAccessControl(element.GetDocument().GetSecurityOrigin(),
                                        cross_origin);
  }
...
void ImageLoader::DoUpdateFromElement(BypassMainWorldBehavior bypass_behavior,
  ...
  if (!url.IsNull()) {
    ...
    if (isHTMLPictureElement(GetElement()->parentNode()) ||
      ...
      resource_request.SetRequestContext(WebURLRequest::kRequestContextImageSet);
...
void ImageLoader::ImageNotifyFinished(ImageResourceContent* resource) {
  ...
  if (image_ && image_->GetImage() && image_->GetImage()->IsSVGImage())
    ToSVGImage(image_->GetImage())->UpdateUseCounters(GetElement()->GetDocument());
