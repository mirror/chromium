...
bool WorkerFetchContext::ShouldBlockFetchByMixedContentCheck(
  ...
  // TODO: If the parent frame is sub frame return true;
  ...
  bool allowed = should_ask_embedder &&
                 WorkerContentSettingsClient::From(*global_scope_)
                     ->AllowRunningInsecureContent(
                         settings->GetAllowRunningOfInsecureContent(),
                         GetSecurityOrigin(), url);
  if (allowed) {
    web_context_->DidRunInsecureContent(WebSecurityOrigin(GetSecurityOrigin()),
                                        url);
  }
  LOG(ERROR) << "ShouldBlockFetchByMixedContentCheck strict_mode: "
             << strict_mode;
  LOG(ERROR) << "ShouldBlockFetchByMixedContentCheck should_ask_embedder: "
             << should_ask_embedder;
  LOG(ERROR) << "ShouldBlockFetchByMixedContentCheck allowed: " << allowed;
  return !allowed;
}
