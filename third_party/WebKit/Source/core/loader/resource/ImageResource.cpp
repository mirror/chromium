...
class ImageResource::ImageResourceInfoImpl final
 ...
 private:
  ...
  bool IsAccessAllowed(
    ...
    return resource_->IsAccessAllowed(security_origin,
...
bool ImageResource::CanReuse(const FetchRequest& request) const {
  ...
  if (request.GetPlaceholderImageRequestType() != FetchRequest::kAllowPlaceholder &&
...
static bool IsEntireResource(const ResourceResponse& response) {
  ...
  int64_t first_byte_position = -1, last_byte_position = -1, instance_length = -1;
