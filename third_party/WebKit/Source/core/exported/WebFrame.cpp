...
bool WebFrame::Swap(WebFrame* frame) {
  ...
  if (owner && owner->IsLocal()) {
    if (new_frame && new_frame->IsLocalFrame())
      probe::frameOwnerContentUpdated(ToLocalFrame(new_frame), ToHTMLFrameOwnerElement(owner));
    else if (old_frame && old_frame->IsLocalFrame())
      probe::frameOwnerContentUpdated(ToLocalFrame(old_frame), ToHTMLFrameOwnerElement(owner));
  }
