...
class TextDumper {
 ...
 private:
  ...
  void AddRequiredLineBreaks(size_t required) {
    required_line_breaks = std::max(required, required_line_breaks);
  }
  ...
  void DumpText(String text) {
    ...
    if (has_emitted_ && required_line_breaks) {
      for (size_t i = 0; i < required_line_breaks; ++i) {
        builder_.Append('\n');
      }
    }
    required_line_breaks = 0;
