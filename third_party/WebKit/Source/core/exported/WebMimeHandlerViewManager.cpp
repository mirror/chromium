...
void WebMimeHandlerViewManager::UnassignHandlerId(HandlerId id) {
  ...
  while (child) {
    auto* html_plugin_element = GetHTMLPlugInElementFromContentFrame(child);
    if (html_plugin_element && html_plugin_element->external_handler_id())
      return html_plugin_element->ResetExternalHandlerId();
    child = child->NextSibling();
  }
}

void WebMimeHandlerViewManager::ReleaseSelf() {
  // TODO(ekaramad): Replace this by cast to WebLocalFrameImpl if that becomes a
  // possibility.
  ToLocalFrame(WebFrame::ToCoreFrame(*owner_frame_))
      ->Client()
      ->ResetMimeHandlerViewManager();
}
