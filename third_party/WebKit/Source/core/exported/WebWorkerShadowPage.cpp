...
void WebWorkerShadowPage::Initialize(const KURL& script_url) {
  DCHECK(IsMainThread());
...
bool WebWorkerShadowPage::WasInitialized() const {
  return state_ == State::kInitialized;
}

void WebWorkerShadowPage::AdvanceState(State new_state) {
  switch (new_state) {
