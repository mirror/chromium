...
TEST_P(SnapCoordinatorTest, FindsClosestSnapOffsetIndependently) {
  ...
  ScrollOffset snapped_offset = current_offset;
  EXPECT_TRUE(SnapCoordinator::FindSnapOffset(current_offset, container_data,
                                              FloatRect(0, 0, 360, 380), true,
                                              true, &snapped_offset));
...
TEST_P(SnapCoordinatorTest, DoesNotSnapOnNonVisibleAreas) {
  ...
  ScrollOffset snapped_offset = current_offset;
  EXPECT_FALSE(SnapCoordinator::FindSnapOffset(current_offset, container_data,
                                               FloatRect(0, 0, 360, 380), true,
                                               true, &snapped_offset));
  EXPECT_EQ(100, snapped_offset.Width());
  EXPECT_EQ(100, snapped_offset.Height());
}

TEST_P(SnapCoordinatorTest, SnapOnClosestAxisFirstIfVisibilityConflicts) {
  SnapContainerData container_data(
      ScrollSnapType(false, SnapAxis::kBoth, SnapStrictness::kMandatory),
      ScrollOffset(), ScrollOffset(360, 380));
  ScrollOffset current_offset(100, 100);
  ...
  // that has a closer snap point first. After that, we look for another snap
  // point on y axis which does not conflict with the snap point on x.
  SnapAreaData snap_x_only(SnapAxis::kX,
                           ScrollOffset(80, SnapAreaData::kInvalidScrollOffset),
                           FloatRect(60, 40, 60, 60), false);
  SnapAreaData snap_y_conflict(
      SnapAxis::kY, ScrollOffset(SnapAreaData::kInvalidScrollOffset, 130),
      FloatRect(90, 90, 60, 60), false);
  SnapAreaData snap_y_no_conflict(
      SnapAxis::kY, ScrollOffset(SnapAreaData::kInvalidScrollOffset, 50),
      FloatRect(40, 40, 60, 60), false);
  container_data.AddSnapAreaData(snap_x_only);
  container_data.AddSnapAreaData(snap_y_conflict);
  container_data.AddSnapAreaData(snap_y_no_conflict);
  ScrollOffset snapped_offset = current_offset;
  EXPECT_TRUE(SnapCoordinator::FindSnapOffset(current_offset, container_data,
                                              FloatRect(0, 0, 360, 380), true,
                                              true, &snapped_offset));
  EXPECT_EQ(80, snapped_offset.Width());
  EXPECT_EQ(50, snapped_offset.Height());
}
