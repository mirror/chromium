...
bool SnapCoordinator::FindSnapOffset(const ScrollOffset& current_offset,
  ...
  for (SnapAreaData snap_area_data : data.snap_area_list) {
    // Ignore the snap areas that are not currently visible or not visible if
    // snapped at snap_offset.
    ...
    if (should_snap_on_x && (snap_area_data.snap_axis == SnapAxis::kX ||
      ...
      if (offset == SnapAreaData::kInvalidScrollOffset ||
          !visibility_requirement.Contains(
              FloatPoint(offset, snap_offset->Height())))
        continue;
    ...
    if (should_snap_on_y && (snap_area_data.snap_axis == SnapAxis::kY ||
      ...
      if (offset == SnapAreaData::kInvalidScrollOffset ||
          !visibility_requirement.Contains(
              FloatPoint(snap_offset->Width(), offset)))
        continue;
  ...
  if (smallest_distance_x <= smallest_distance_y) {
    snap_offset->SetHeight(current_offset.Height());
    FindSnapOffset(current_offset, data, visible_area_y, false, true,
                   snap_offset);
  } else {
    snap_offset->SetWidth(current_offset.Width());
    FindSnapOffset(current_offset, data, visible_area_y, true, false,
                   snap_offset);
  }
  return true;
}
...
bool SnapCoordinator::GetSnapPosition(const LayoutBox& snap_container,
  ...
  *snap_offset = current_scroll_offset;
  return FindSnapOffset(
      current_scroll_offset, data,
      FloatRect(FloatPoint(data.min_offset), data.max_offset - data.min_offset),
