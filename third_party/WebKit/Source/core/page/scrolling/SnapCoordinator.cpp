...
static LayoutUnit CalculateSnapOffset(SnapAlignment alignment,
                                      ...
                                      LayoutRect area) {
  DCHECK(axis == kSnapAxisX || axis == kSnapAxisY);
  switch (alignment) {
...
SnapAreaData SnapCoordinator::CalculateSnapAreaData(
  ...
  // snap_area.OffsetPoint(snap_container) returns the snap_area's position
  // relative to its container. And the |area| below represents the snap_area
  // rect in respect to the snap_container.
  LayoutRect area(
  ...
  snap_area_data.snap_offset.set_x(CalculateSnapOffset(align.alignmentX,
                                                       kSnapAxisX, container,
                                                       scrollable_size, area)
                                       .ToFloat());
  snap_area_data.snap_offset.set_y(CalculateSnapOffset(align.alignmentY,
                                                       kSnapAxisY, container,
                                                       scrollable_size, area)
                                       .ToFloat());

  if (align.alignmentX != kSnapAlignmentNone &&
      align.alignmentY != kSnapAlignmentNone) {
    snap_area_data.snap_axis = kSnapAxisBoth;
  } else if (align.alignmentX != kSnapAlignmentNone &&
             align.alignmentY == kSnapAlignmentNone) {
    snap_area_data.snap_axis = kSnapAxisX;
  } else {
    snap_area_data.snap_axis = kSnapAxisY;
  }
...
ScrollOffset SnapCoordinator::FindSnapOffsetOnX(ScrollOffset current_offset,
  ...
  for (SnapAreaData snap_area_data : data.snap_area_list) {
    if (snap_area_data.snap_axis == kSnapAxisX ||
        snap_area_data.snap_axis == kSnapAxisBoth) {
...
ScrollOffset SnapCoordinator::FindSnapOffsetOnY(ScrollOffset current_offset,
  ...
  for (SnapAreaData snap_area_data : data.snap_area_list) {
    if (snap_area_data.snap_axis == kSnapAxisY ||
        snap_area_data.snap_axis == kSnapAxisBoth) {
