...
void SnapCoordinator::UpdateSnapContainerData(const LayoutBox& snap_container) {
  ...
  if (!snap_container.IsLayoutView()) {
    snap_container_data.min_offset =
        snap_container.GetScrollableArea()->MinimumScrollOffset();
    snap_container_data.max_offset =
        snap_container.GetScrollableArea()->MaximumScrollOffset();
  } else {
    snap_container_data.min_offset = snap_container.GetFrameView()
                                         ->GetScrollableArea()
                                         ->MinimumScrollOffset();
    snap_container_data.max_offset = snap_container.GetFrameView()
                                         ->GetScrollableArea()
                                         ->MaximumScrollOffset();
  }
...
bool SnapCoordinator::GetSnapPosition(const LayoutBox& snap_container,
  ...
  if (!snap_container.IsLayoutView()) {
    ...
    current_scroll_offset =
        snap_container.GetFrameView()->GetScrollableArea()->GetScrollOffset();
  }
  *snap_offset =
