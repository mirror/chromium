...
static ScrollOrientation GetSnappingOrientation(ScrollOrientation orientation,
                                                SnapAxis axis) {
  if (axis == SnapAxis::kBoth)
    return orientation;
  if (axis == SnapAxis::kX)
    return ScrollOrientation(orientation & kHorizontalScroll);
  return ScrollOrientation(orientation & kVerticalScroll);
}
...
ScrollOffset SnapCoordinator::FindSnapOffset(ScrollOffset current_offset,
  ...
  for (SnapAreaData snap_area_data : data.snap_area_list) {
    ...
    if ((orientation & kHorizontalScroll) &&
        (snap_area_data.snap_axis == SnapAxis::kX ||
         snap_area_data.snap_axis == SnapAxis::kBoth)) {
...
bool SnapCoordinator::GetSnapPosition(const LayoutBox& snap_container,
  ...
  ScrollOrientation snap_orientation =
      GetSnappingOrientation(orientation, data.scroll_snap_type.axis);
  if (!data.snap_area_list.size() || snap_orientation == kNoScroll)
    return false;

  *snap_offset = FindSnapOffset(current_scroll_offset, data, snap_orientation);
  return true;
}
