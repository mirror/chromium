...
FloatSize StickyPositionScrollingConstraints::ComputeStickyOffset(
  ...
  FloatRect containing_block_rect = scroll_container_relative_containing_block_rect_;
  ...
  if (HasAnchorEdge(kAnchorEdgeLeft)) {
    ...
    float available_space =
        std::max<float>(0, containing_block_rect.MaxX() - sticky_box_rect.MaxX());
  ...
  if (HasAnchorEdge(kAnchorEdgeBottom)) {
    ...
    float bottom_delta = std::min<float>(0, bottom_limit - sticky_box_rect.MaxY());
    float available_space =
  ...
  if (HasAnchorEdge(kAnchorEdgeTop)) {
    ...
    float available_space =
        std::max<float>(0, containing_block_rect.MaxY() - sticky_box_rect.MaxY());
  ...
  total_containing_block_sticky_offset_ =
      ancestor_sticky_box_offset + ancestor_containing_block_offset + sticky_offset;
