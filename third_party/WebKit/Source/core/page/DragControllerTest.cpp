...
TEST_F(DragControllerTest, DragImageOffsetWithPageScaleFactor) {
  ...
  // The drag image should be offset by the margin of 2px.
...
TEST_F(DragControllerTest, DragLinkWithPageScaleFactor) {
  SetBodyInnerHTML(
      ...
      "  }"
      "</style>"
      "<a id='drag' href='https://foo.com'>abcdefg<br>abcdefg<br>abcdefg</a>");
  const int page_scale_factor = 2;
  GetFrame().GetPage()->SetPageScaleFactor(page_scale_factor);
  GetFrame().Selection().SelectAll();
  ...
  LOG(ERROR) << "In test, link_image_size: " << link_image_size.ToString();
  ...
  FloatPoint expected_offset = FloatPoint(link_image_size.Width() / 2.f, 2.f);
  LOG(ERROR) << "In test, expected_offset: " << link_image_size.ToString();
  EXPECT_EQ(RoundedIntPoint(expected_offset),
            IntPoint(ChromeClient().last_drag_image_offset));
}
