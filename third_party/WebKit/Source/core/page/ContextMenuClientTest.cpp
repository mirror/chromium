...
class ContextMenuClientTest : public ::testing::Test {
 public:
  void SetUp() {
    ...
    context_menu_client_ =
        std::make_unique<ContextMenuClient>(*web_view_helper_.WebView());
  }

  ContextMenuClient* GetContextMenuClient() {
    return context_menu_client_.get();
  }
 ...
 private:
  ...
  std::unique_ptr<ContextMenuClient> context_menu_client_;
};

TEST_F(ContextMenuClientTest, VideoNotLoaded) {
  ...
  EXPECT_TRUE(
      GetContextMenuClient()->ShowContextMenu(&context_menu, kMenuSourceMouse));
