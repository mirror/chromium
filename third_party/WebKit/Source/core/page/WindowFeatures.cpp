...
WindowFeatures::WindowFeatures(const String& dialog_features_string,
  ...
  width = IntFeature(features, "dialogwidth", 100, screen_available_rect.Width(),
                     620);
  // default here came from frame size of dialog in MacIE
  ...
  if (BoolFeature(features, "center", true)) {
    if (!x_set) {
      x = screen_available_rect.X() + (screen_available_rect.Width() - width) / 2;
      x_set = true;
    }
    if (!y_set) {
      y = screen_available_rect.Y() + (screen_available_rect.Height() - height) / 2;
      y_set = true;
    }
  }
