...
void PrintContext::ComputePageRects(const FloatRect& print_rect,
  ...
  ComputePageRectsWithPageSizeInternal(
      FloatSize(page_width / user_scale_factor, page_height / user_scale_factor));
}
...
void PrintContext::OutputLinkedDestinations(GraphicsContext& context,
  ...
  for (const auto& entry : linked_destinations_) {
    ...
    // the root frame.
    IntPoint point =
        layout_object->GetFrameView()->ConvertToRootFrame(bounding_box.Location());
...
String PrintContext::PageSizeAndMarginsInPixels(LocalFrame* frame,
  ...
  frame->GetDocument()->PageSizeAndMarginsInPixels(
      page_number, page_size, margin_top, margin_right, margin_bottom, margin_left);
