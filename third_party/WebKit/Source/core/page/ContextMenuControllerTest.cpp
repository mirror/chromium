...
TEST_F(ContextMenuControllerTest, TestCustomMenu) {
  ...
  GetDocument().GetPage()->GetContextMenuController().HandleContextMenuEvent(event);
  ...
  const Vector<ContextMenuItem>& items =
      GetDocument().GetPage()->GetContextMenuController().GetContextMenu()->Items();
  EXPECT_EQ(8u, items.size());
  EXPECT_EQ(kActionType, items[0].GetType());
  EXPECT_STREQ("Item1", items[0].Title().Utf8().Data());
  GetDocument().GetPage()->GetContextMenuController().ContextMenuItemSelected(&items[0]);
  ...
  GetDocument().GetPage()->GetContextMenuController().ContextMenuItemSelected(&items[4]);
  EXPECT_STREQ("Title 7 checked", GetDocument().title().Utf8().Data());
  GetDocument().GetPage()->GetContextMenuController().ContextMenuItemSelected(&items[4]);
  EXPECT_STREQ("Title 7 not checked", GetDocument().title().Utf8().Data());
  GetDocument().GetPage()->GetContextMenuController().ContextMenuItemSelected(&items[5]);
  EXPECT_STREQ("Title 8 not checked and Title 9 checked",
               GetDocument().title().Utf8().Data());
  GetDocument().GetPage()->GetContextMenuController().ContextMenuItemSelected(&items[7]);
