...
void CompileZoomableSubtargets(const HeapVector<Member<Node>>& intersected_nodes,
                               SubtargetGeometryList& subtargets) {
  for (unsigned i = 0; i < intersected_nodes.size(); ++i) {
...
bool FindNodeWithLowestDistanceMetric(Node*& target_node,
  ...
  if (target_node)
    target_area = target_node->GetDocument().View()->ContentsToRootFrame(target_area);
...
bool FindBestClickableCandidate(Node*& target_node,
  ...
  return TouchAdjustment::FindNodeWithLowestDistanceMetric(
      target_node, target_point, target_area, touch_hotspot, touch_area, subtargets,
