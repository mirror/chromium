...
KeyframeEffectReadOnly* KeyframeEffectReadOnly::Create(
  ...
  if (!TimingInput::Convert(duration, timing, exception_state))
    return nullptr;
  return Create(element, EffectInput::Convert(element, effect_input,
...
double KeyframeEffectReadOnly::CalculateTimeToEffectChange(
  ...
  const double end_time = end_time_minus_end_delay + SpecifiedTiming().end_delay;
...
bool KeyframeEffectReadOnly::IsCandidateForAnimationOnCompositor(
  ...
  if (!Model() || !target_ ||
      (target_->GetComputedStyle() && target_->GetComputedStyle()->HasOffset()) ||
...
bool KeyframeEffectReadOnly::MaybeStartAnimationOnCompositor(
  ...
  CompositorAnimations::StartAnimationOnCompositor(
      *target_, group, start_time, current_time, SpecifiedTiming(), *GetAnimation(),
      *Model(), compositor_animation_ids_, animation_playback_rate);
