...
Vector<ScriptValue> KeyframeEffectReadOnly::getKeyframes(
  ...
  for (size_t i = 0; i < keyframes.size(); i++) {
    // getKeyframes() returns a list of 'ComputedKeyframes', which consist of
    // the Keyframe data combined with the computed offset for the given
    // keyframe
    //
    ...
    v8::Local<v8::Object> object =
        script_value.V8Value()->ToObject(context).ToLocalChecked();
    object
        ->CreateDataProperty(
            context,
            V8AtomicString(script_state->GetIsolate(), "computedOffset"),
            ToV8(computed_offsets[i], context->Global(),
