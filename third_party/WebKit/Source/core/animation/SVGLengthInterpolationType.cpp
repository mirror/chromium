...
SVGLength* SVGLengthInterpolationType::ResolveInterpolableSVGLength(
  ...
  const InterpolableList& list_of_values = ToInterpolableList(interpolable_value);

  double value = 0;
  CSSPrimitiveValue::UnitType unit_type = CSSPrimitiveValue::UnitType::kUserUnits;
...
void SVGLengthInterpolationType::Apply(
  ...
  element.SetWebAnimatedAttribute(
      Attribute(),
      ResolveInterpolableSVGLength(interpolable_value, length_context, unit_mode_,
