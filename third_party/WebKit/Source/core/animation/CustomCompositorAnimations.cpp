...
static KeyframeEffect* UpdateInfiniteKeyframeEffect(
  ...
  const KeyframeVector& old_frames =
      ToAnimatableValueKeyframeEffectModel(keyframe_effect.Model())->GetFrames();
  AnimatableValueKeyframeVector keyframes(2);
...
static Animation* CreateOrUpdateAnimation(
  ...
  if (!animation) {
    KeyframeEffect* keyframe_effect =
        CreateInfiniteKeyFrameEffect(element, property_id, std::move(new_value));
    return element.GetDocument().Timeline().Play(keyframe_effect);
  }
  KeyframeEffect* keyframe_effect = UpdateInfiniteKeyframeEffect(
      *ToKeyframeEffect(animation->effect()), property_id, std::move(new_value));
