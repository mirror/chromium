  ...
  if (coordinate)
    return Unwrap(
        CSSPositionAxisListInterpolationType::ConvertPositionAxisCSSValue(
            *coordinate));
  return Unwrap(
      LengthInterpolationFunctions::MaybeConvertLength(Length(50, kPercent), 1));
}
...
BasicShapeRadius CreateRadius(const InterpolableValue& interpolable_value,
                              const CSSToLengthConversionData& conversion_data) {
...
LengthSize CreateBorderRadius(const InterpolableValue& width,
                              ...
                              const CSSToLengthConversionData& conversion_data) {
  return LengthSize(
      LengthInterpolationFunctions::CreateLength(width, nullptr, conversion_data,
...
InterpolationValue ConvertCSSValue(const CSSBasicShapeCircleValue& circle) {
  ...
  return InterpolationValue(
      std::move(list),
      BasicShapeNonInterpolableValue::Create(BasicShape::kBasicShapeCircleType));
}
