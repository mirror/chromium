...
//
// Generically, a keyframe is a set of (property, value) pairs. Within the
// web-animations spec a keyframe has a few additional properties:
//
...
//   * A timing function, which applies to the period of time between this
//     keyframe and the 'next' keyframe in the same effect and influences the
...
//
// In this implementation property-specific are created only once and cached for
// subsequent calls, rather than re-computing them for every sample from the
// keyframe effect. See KeyframeEffectModelBase::EnsureKeyframeGroups.
//
