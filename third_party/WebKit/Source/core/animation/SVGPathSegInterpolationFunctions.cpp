...
std::unique_ptr<InterpolableNumber> ConsumeControlAxis(double value,
  ...
  return InterpolableNumber::Create(is_absolute ? value : current_value + value);
}
...
std::unique_ptr<InterpolableValue> ConsumeSingleCoordinate(
  ...
  result->Set(
      0, ConsumeCoordinateAxis(segment.X(), is_absolute, coordinates.current_x));
  result->Set(
      1, ConsumeCoordinateAxis(segment.Y(), is_absolute, coordinates.current_y));
...
std::unique_ptr<InterpolableValue> ConsumeCurvetoCubic(
  ...
  result->Set(
      4, ConsumeCoordinateAxis(segment.X(), is_absolute, coordinates.current_x));
  result->Set(
      5, ConsumeCoordinateAxis(segment.Y(), is_absolute, coordinates.current_y));
  return std::move(result);
}
...
std::unique_ptr<InterpolableValue> ConsumeCurvetoQuadratic(
  ...
  result->Set(
      1, ConsumeControlAxis(segment.Y1(), is_absolute, coordinates.current_y));
  result->Set(
      2, ConsumeCoordinateAxis(segment.X(), is_absolute, coordinates.current_x));
  result->Set(
      3, ConsumeCoordinateAxis(segment.Y(), is_absolute, coordinates.current_y));
  return std::move(result);
}
