...
Timing ApplyTimingInputNumber(v8::Isolate* isolate,
  ...
  V8KeyframeEffectOptions::toImpl(isolate, timing_input, timing_input_dictionary,
                                  exception_state);
  Timing result;
  timing_conversion_success = TimingInput::Convert(timing_input_dictionary, result,
...
TEST(AnimationTimingInputTest, TimingInputStartDelay) {
  ...
  EXPECT_EQ(1, ApplyTimingInputString(scope.GetIsolate(), "delay", "1000",
                   ...
                   .start_delay);
  EXPECT_EQ(
      0, ApplyTimingInputString(scope.GetIsolate(), "delay", "1s", ignored_success)
             .start_delay);
  ...
  EXPECT_EQ(0, ApplyTimingInputString(scope.GetIsolate(), "delay", "-Infinity",
                   ...
                   .start_delay);
  EXPECT_EQ(
      0, ApplyTimingInputString(scope.GetIsolate(), "delay", "NaN", ignored_success)
             .start_delay);
...
TEST(AnimationTimingInputTest, TimingInputFillMode) {
  ...
  EXPECT_EQ(
      ...
      ApplyTimingInputString(scope.GetIsolate(), "fill", "auto", ignored_success)
          .fill_mode);
  ...
  EXPECT_EQ(
      ...
      ApplyTimingInputString(scope.GetIsolate(), "fill", "none", ignored_success)
          .fill_mode);
  ...
  EXPECT_EQ(
      ...
      ApplyTimingInputString(scope.GetIsolate(), "fill", "both", ignored_success)
          .fill_mode);
...
TEST(AnimationTimingInputTest, TimingInputIterationStart) {
  V8TestingScope scope;
  bool success;
  EXPECT_EQ(1.1, ApplyTimingInputNumber(scope.GetIsolate(), "iterationStart", 1.1,
                                        success)
                     .iteration_start);
  EXPECT_TRUE(success);
...
TEST(AnimationTimingInputTest, TimingInputIterationCount) {
  ...
  bool success;
  EXPECT_EQ(2.1,
            ApplyTimingInputNumber(scope.GetIsolate(), "iterations", 2.1, success)
                .iteration_count);
  EXPECT_TRUE(success);
...
TEST(AnimationTimingInputTest, TimingInputDirection) {
  ...
  EXPECT_EQ(default_playback_direction,
            ApplyTimingInputString(scope.GetIsolate(), "direction", "rubbish",
                ...
                .direction);
  EXPECT_EQ(
      default_playback_direction,
      ApplyTimingInputNumber(scope.GetIsolate(), "direction", 2, ignored_success)
          .direction);
}
...
TEST(AnimationTimingInputTest, TimingInputTimingFunction) {
  ...
  EXPECT_EQ(
      ...
      *ApplyTimingInputString(scope.GetIsolate(), "easing", "ease-in-out", success)
           .timing_function);
  EXPECT_TRUE(success);
  EXPECT_EQ(
      *LinearTimingFunction::Shared(),
  ...
  EXPECT_EQ(
      ...
      *ApplyTimingInputString(scope.GetIsolate(), "easing", "step-start", success)
           .timing_function);
  EXPECT_TRUE(success);
  EXPECT_EQ(
      *StepsTimingFunction::Preset(StepsTimingFunction::StepPosition::MIDDLE),
      *ApplyTimingInputString(scope.GetIsolate(), "easing", "step-middle", success)
           .timing_function);
  EXPECT_TRUE(success);
  EXPECT_EQ(
