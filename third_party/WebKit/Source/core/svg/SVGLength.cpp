...
static bool IsCalcCSSUnitType(CSSPrimitiveValue::UnitType type) {
  return type >= CSSPrimitiveValue::UnitType::kCalc &&
         type <= CSSPrimitiveValue::UnitType::kCalcPercentageWithLengthAndNumber;
}
...
void SVGLength::CalculateAnimatedValue(SVGAnimationElement* animation_element,
                                       float percentage,
  ...
  SVGLength* to_at_end_of_duration_length = ToSVGLength(to_at_end_of_duration_value);
  ...
  CSSPrimitiveValue::UnitType new_unit = CSSPrimitiveValue::UnitType::kUserUnits;
  if (percentage < 0.5) {
  ...
  animated_number = length_context.ConvertValueFromUserUnits(animated_number,
                                                           UnitMode(), new_unit);
