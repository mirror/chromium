...
SVGTransformDistance::SVGTransformDistance(SVGTransform* from_svg_transform,
  ...
  switch (transform_type_) {
    ...
    case kSvgTransformRotate: {
      FloatSize center_distance =
          to_svg_transform->RotationCenter() - from_svg_transform->RotationCenter();
    ...
    case kSvgTransformScale: {
      float scale_x =
          to_svg_transform->Scale().Width() - from_svg_transform->Scale().Width();
      float scale_y =
          to_svg_transform->Scale().Height() - from_svg_transform->Scale().Height();
...
SVGTransform* SVGTransformDistance::AddSVGTransforms(SVGTransform* first,
  ...
  switch (first->TransformType()) {
    ...
    case kSvgTransformTranslate: {
      float dx = first->Translate().X() + second->Translate().X() * repeat_count;
      float dy = first->Translate().Y() + second->Translate().Y() * repeat_count;
...
SVGTransform* SVGTransformDistance::AddToSVGTransform(
  ...
  switch (transform_type_) {
    ...
    case kSvgTransformTranslate: {
      ...
      translation +=
          FloatSize::NarrowPrecision(transform_.E(), transform_.F());
