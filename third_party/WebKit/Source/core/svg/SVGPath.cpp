...
std::unique_ptr<SVGPathByteStream> BlendPathByteStreams(
    ...
    float progress) {
  std::unique_ptr<SVGPathByteStream> result_stream = SVGPathByteStream::Create();
...
SVGParsingError SVGPath::SetValueAsString(const String& string) {
  ...
  SVGParsingError parse_status = BuildByteStreamFromString(string, *byte_stream);
...
void SVGPath::Add(SVGPropertyBase* other, SVGElement*) {
  const SVGPathByteStream& other_path_byte_stream = ToSVGPath(other)->ByteStream();
...
void SVGPath::CalculateAnimatedValue(SVGAnimationElement* animation_element,
                                     float percentage,
  ...
  if (animation_element->IsAccumulated() && repeat_count)
    new_stream = ConditionallyAddPathByteStreams(
        std::move(new_stream), ToSVGPath(to_at_end_of_duration_value)->ByteStream(),
        repeat_count);
