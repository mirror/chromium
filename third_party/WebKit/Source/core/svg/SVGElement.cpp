...
bool SVGElement::HasTransform(ApplyMotionTransform apply_motion_transform) const {
  return (GetLayoutObject() && GetLayoutObject()->StyleRef().HasTransform()) ||
         (apply_motion_transform == kIncludeMotionTransform && HasSVGRareData());
}
...
void SVGElement::RemovedFrom(ContainerNode* root_parent) {
  ...
  SECURITY_DCHECK(
      ...
      !ToSVGElement(root_parent)->elements_with_relative_lengths_.Contains(this));
...
void SVGElement::UpdateRelativeLengthsInformation(bool client_has_relative_lengths,
  ...
  if (LayoutObject* layout_object = this->GetLayoutObject()) {
    if (HasRelativeLengths() && layout_object->IsSVGResourceContainer())
      ...
      layout_object->SetNeedsLayoutAndFullPaintInvalidation(
          LayoutInvalidationReason::kUnknown, kMarkContainerChain, layout_scope);
  }
...
SVGAnimatedPropertyBase* SVGElement::PropertyFromAttribute(
  ...
  AttributeToPropertyMap::const_iterator it =
      attribute_to_property_map_.Find<SVGAttributeHashTranslator>(attribute_name);
...
void SVGElement::SvgAttributeChanged(const QualifiedName& attr_name) {
  CSSPropertyID prop_id = SVGElement::CssPropertyIdForSVGAttributeName(attr_name);
  if (prop_id > 0) {
    InvalidateInstances();
    return;
  }
...
void SVGElement::SynchronizeAnimatedSVGAttribute(
    const QualifiedName& name) const {
  if (!GetElementData() || !GetElementData()->animated_svg_attributes_are_dirty_)
    return;
