...
SVGRectTearOff* SVGTextContentElement::getExtentOfChar(
  ...
  return SVGRectTearOff::Create(SVGRect::Create(rect), 0, kPropertyIsNotAnimVal);
}
...
void SVGTextContentElement::CollectStyleForPresentationAttribute(
  ...
  if (name.Matches(XMLNames::spaceAttr)) {
    ...
    if (value == preserve_string) {
      ...
      UseCounter::Count(GetDocument(), UseCounter::kWhiteSpaceNowrapFromXMLSpace);
      AddPropertyToPresentationAttributeStyle(style, CSSPropertyWhiteSpace,
                                              CSSValueNowrap);
    }
  } else {
...
void SVGTextContentElement::SvgAttributeChanged(const QualifiedName& attr_name) {
