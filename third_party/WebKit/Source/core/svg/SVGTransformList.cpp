...
static_assert(WTF_ARRAY_LENGTH(kRequiredValuesForType) - 1 == kSvgTransformSkewy,
...
SVGTransformList* SVGTransformList::Create(SVGTransformType transform_type,
  ...
  if (value.IsEmpty()) {
    ...
    const LChar* ptr = value.Characters8();
    const LChar* end = ptr + value.length();
    status = ParseTransformArgumentsForType(transform_type, ptr, end, arguments);
...
void SVGTransformList::Add(SVGPropertyBase* other, SVGElement* context_element) {
  if (IsEmpty())
    return;
...
void SVGTransformList::CalculateAnimatedValue(
  ...
  if (from_list->length() &&
    ...
    effective_from = SVGTransform::Create(to_transform->TransformType(),
