...
float SVGPathStringSource::ParseNumberWithError() {
  ...
  if (is8_bit_source_)
    error =
        !ParseNumber(current_.character8_, end_.character8_, number_value);
  else
    error =
...
bool SVGPathStringSource::ParseArcFlagWithError() {
  ...
  if (is8_bit_source_)
    error =
        !ParseArcFlag(current_.character8_, end_.character8_, flag_value);
  else
    error =
        !ParseArcFlag(current_.character16_, end_.character16_, flag_value);
  if (UNLIKELY(error))
