...
void InvalidateInstancesAndAncestorResources(SVGStopElement* stop_element) {
  ...
  // We know stop elements don't instantiate layout objects.  Furthermore,
  // we don't assume their immediate ancestors do either.  Instead, we walk the
  // ancestor chain until we find the resource container - then we defer inval
  // to the layout tree.
  for (const auto* e = stop_element->parentElement(); e;
       e = e->parentElement()) {
