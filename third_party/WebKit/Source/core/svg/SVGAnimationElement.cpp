...
float SVGAnimationElement::getStartTime(ExceptionState& exception_state) const {
  ...
  if (!start_time.IsFinite()) {
    exception_state.ThrowDOMException(kInvalidStateError, "No current interval.");
    return 0;
  }
...
void SVGAnimationElement::beginElementAt(float offset) {
  ...
  AddInstanceTime(kBegin, Elapsed() + offset, SMILTimeWithOrigin::kScriptOrigin);
}
...
void SVGAnimationElement::StartedActiveInterval() {
  ...
  if (calc_mode == kCalcModeSpline) {
    ...
    if (!splines_count || (FastHasAttribute(SVGNames::keyPointsAttr) &&
  ...
  if (animation_mode == kNoAnimation)
    return;
  if ((animation_mode == kFromToAnimation || animation_mode == kFromByAnimation ||
       animation_mode == kToAnimation || animation_mode == kByAnimation) &&
  ...
  if (animation_mode == kFromToAnimation) {
    ...
    animation_valid_ =
        values_.size() >= 1 && (calc_mode == kCalcModePaced ||
                                 !FastHasAttribute(SVGNames::keyTimesAttr) ||
                                 FastHasAttribute(SVGNames::keyPointsAttr) ||
