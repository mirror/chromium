...
bool SVGAnimateMotionElement::CalculateFromAndToValues(const String& from_string,
...
bool SVGAnimateMotionElement::CalculateFromAndByValues(const String& from_string,
  ...
  to_point_ =
      FloatPoint(from_point_.X() + by_point.X(), from_point_.Y() + by_point.Y());
  return true;
}
...
void SVGAnimateMotionElement::CalculateAnimatedValue(float percentage,
  ...
  if (GetAnimationMode() != kPathAnimation) {
    ...
    AnimateAdditiveNumber(percentage, repeat_count, from_point_.X(),
                          to_point_.X(), to_point_at_end_of_duration.X(), animated_x);

    float animated_y = 0;
    AnimateAdditiveNumber(percentage, repeat_count, from_point_.Y(),
                          to_point_.Y(), to_point_at_end_of_duration.Y(), animated_y);
