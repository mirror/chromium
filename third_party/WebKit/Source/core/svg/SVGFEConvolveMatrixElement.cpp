...
bool SVGFEConvolveMatrixElement::SetFilterEffectAttribute(
  ...
  if (attr_name == SVGNames::edgeModeAttr)
    return convolve_matrix->SetEdgeMode(edge_mode_->CurrentValue()->EnumValue());
...
void SVGFEConvolveMatrixElement::SvgAttributeChanged(
    const QualifiedName& attr_name) {
  if (attr_name == SVGNames::edgeModeAttr || attr_name == SVGNames::divisorAttr ||
      attr_name == SVGNames::biasAttr || attr_name == SVGNames::targetXAttr ||
...
FilterEffect* SVGFEConvolveMatrixElement::Build(SVGFilterBuilder* filter_builder,
                                                Filter* filter) {
