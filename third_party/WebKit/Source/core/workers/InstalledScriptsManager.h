...
class InstalledScriptsManager {
 public:
  ...
  // Used on the worker thread. When GetScriptData returns
  ...
  // - ScriptStatus::kFailed: an error happens during receiving the script from
  //                          the browser process.
  //                         |out_script_data| is left as is.
  // This can be blocked if the script is not streamed yet.
