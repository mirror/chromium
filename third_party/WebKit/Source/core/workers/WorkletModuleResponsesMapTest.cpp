...
class WorkletModuleResponsesMapTest : public ::testing::Test {
 public:
  ...
  void RegisterMockedResponse(const KURL& url) {
    ResourceResponse response;
    response.SetURL(url);
    response.SetHTTPStatusCode(200);
    response.SetHTTPHeaderField(HTTPNames::Content_Type, "text/javascript");
    URLTestHelpers::RegisterMockedURLLoadWithCustomResponse(
        url, String() /* file_path */, WrappedResourceResponse(response));
  }
...
TEST_F(WorkletModuleResponsesMapTest, Isolation) {
  ...
  RegisterMockedResponse(kUrl2);
  HeapVector<Member<ClientImpl>> clients;

  // An initial read call for |kUrl1| initiates a fetch request.
  clients.push_back(new ClientImpl);
  map_->ReadOrCreateEntry(FetchParameters(ResourceRequest(kUrl1)), clients[0]);
  EXPECT_EQ(ClientImpl::Result::kInitial, clients[0]->GetResult());
  EXPECT_FALSE(clients[0]->GetParams().has_value());
  ...
  clients.push_back(new ClientImpl);
  map_->ReadOrCreateEntry(FetchParameters(ResourceRequest(kUrl1)), clients[1]);
  EXPECT_EQ(ClientImpl::Result::kInitial, clients[1]->GetResult());

  // An initial read call for |kUrl2| initiates a fetch request.
  clients.push_back(new ClientImpl);
  map_->ReadOrCreateEntry(FetchParameters(ResourceRequest(kUrl2)), clients[2]);
  EXPECT_EQ(ClientImpl::Result::kInitial, clients[2]->GetResult());
  ...
  // wait for the completion.
  clients.push_back(new ClientImpl);
  map_->ReadOrCreateEntry(FetchParameters(ResourceRequest(kUrl2)), clients[3]);
...
TEST_F(WorkletModuleResponsesMapTest, InvalidURL) {
  ...
  map_->ReadOrCreateEntry(FetchParameters(ResourceRequest(kInvalidURL)),
                          client3);
