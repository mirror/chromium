...
// A proxy class to talk to WorkletModuleResponsesMap on the main thread
// (outside_settings) from WorkletGlobalScope on the worklet context thread
// (inside_settings). The constructor and all public functions must be called on
// the worklet context thread.
