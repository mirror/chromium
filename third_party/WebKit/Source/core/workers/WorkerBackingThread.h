...
// WorkerBackingThread (i.e., WorkerThread) needs to call InitializeOnThread()
// to use V8 and Oilpan functionalities, and call ShutdownOnThread() when it no
// longer needs the thread.
class CORE_EXPORT WorkerBackingThread final {
 public:
  ...
  // InitializeOnThread() and ShutdownOnThread() attaches and detaches Oilpan
  // and V8 to / from the caller worker script, respectively. A worker script
  // must not call any function after calling ShutdownOnThread().
  // They should be called from |this| thread.
  void InitializeOnThread(const WorkerV8Settings&);
