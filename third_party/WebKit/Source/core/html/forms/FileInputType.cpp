...
void FileInputType::HandleDOMActivateEvent(Event* event) {
  ...
  if (ChromeClient* chrome_client = this->GetChromeClient()) {
    ...
    settings.allows_multiple_files =
        settings.allows_directory_upload || input.FastHasAttribute(multipleAttr);
    ...
    settings.use_media_capture = RuntimeEnabledFeatures::mediaCaptureEnabled() &&
...
FileList* FileInputType::CreateFileList(
  ...
  if (size && has_webkit_directory_attr) {
    ...
    for (const auto& file : files) {
      ...
      // script.
      String relative_path = file.path.Substring(root_length).Replace('\\', '/');
...
void FileInputType::FilesChosen(const Vector<FileChooserFileInfo>& files) {
  SetFiles(
      CreateFileList(files, GetElement().FastHasAttribute(webkitdirectoryAttr)));
}
