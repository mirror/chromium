...
class HTMLSlotElementTest : public testing::Test {
 protected:
  ...
  std::array<std::array<int, kSize>, kSize> lcs_table_;
...
int HTMLSlotElementTest::LongestCommonSubsequenceSize(
  ...
  HTMLSlotElement::FillLongestCommonSubsequenceDynamicProgrammingTable(
      nodes1, nodes2, lcs_table_, back_track_table_);
...
TEST_F(HTMLSlotElementTest,
  ...
  EXPECT_EQ(back_track_table_[4][4], Pair(3, 4));

  EXPECT_EQ(lcs_table_[3][4], 2);
  EXPECT_EQ(back_track_table_[3][4], Pair(2, 4));

  EXPECT_EQ(lcs_table_[2][4], 2);
  EXPECT_EQ(back_track_table_[2][4], Pair(1, 3));

  EXPECT_EQ(lcs_table_[1][3], 1);
  EXPECT_EQ(back_track_table_[1][3], Pair(1, 2));

  EXPECT_EQ(lcs_table_[1][2], 1);
  EXPECT_EQ(back_track_table_[1][2], Pair(0, 1));
