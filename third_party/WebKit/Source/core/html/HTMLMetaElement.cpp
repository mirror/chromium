...
void HTMLMetaElement::ParseContentAttribute(const String& content,
  ...
  for (unsigned i = 0; i < length; /* no increment here */) {
    ...
    ProcessViewportKeyValuePair(document, !has_invalid_separator, key_string,
                                value_string, viewport_meta_zero_values_quirk, data);
  }
...
float HTMLMetaElement::ParsePositiveNumber(Document* document,
  ...
  if (value_string.Is8Bit())
    ...
    value = CharactersToFloat(value_string.Characters16(), value_string.length(),
                              parsed_length);
  if (!parsed_length) {
    if (report_warnings)
...
void HTMLMetaElement::Process() {
  ...
  if (!name_value.IsEmpty()) {
    ...
    else if (EqualIgnoringCase(name_value, "theme-color") && GetDocument().GetFrame())
      GetDocument().GetFrame()->Loader().Client()->DispatchDidChangeThemeColor();
  }
