...
ImageCandidate HTMLImageElement::FindBestFitImageFromPictureParent() {
  ...
  for (Node* child = parent->firstChild(); child;
    ...
    if (!source->FastGetAttribute(srcAttr).IsNull())
      Deprecation::CountDeprecation(GetDocument(), UseCounter::kPictureSourceSrc);
...
LayoutObject* HTMLImageElement::CreateLayoutObject(const ComputedStyle& style) {
  ...
  if (content_data && content_data->IsImage()) {
    const StyleImage* content_image = ToImageContentData(content_data)->GetImage();
  ...
  if (GetLayoutObject() && GetLayoutObject()->IsImage()) {
    ...
    if (is_fallback_image_) {
      ...
      ImageResourceContent* new_image_resource =
          ImageResourceContent::Create(broken_image_and_image_scale_factor.first);
...
unsigned HTMLImageElement::naturalWidth() const {
  ...
  return GetImageLoader()
      .GetImage()
      ->ImageSize(LayoutObject::ShouldRespectImageOrientation(GetLayoutObject()),
...
const String& HTMLImageElement::currentSrc() const {
  ...
  if (!GetImageLoader().GetImage() || !GetImageLoader().GetImage()->GetImage() ||
...
FloatSize HTMLImageElement::DefaultDestinationSize(
  ...
  if (GetLayoutObject() && GetLayoutObject()->IsLayoutImage() && image->GetImage() &&
...
void HTMLImageElement::SelectSourceURL(
  ...
  if (!found_url) {
    candidate = BestFitSourceForImageAttributes(
        ...
        FastGetAttribute(srcAttr), FastGetAttribute(srcsetAttr), &GetDocument());
    SetBestFitURLAndDPRFromImageCandidate(candidate);
  }
  ...
  if ((image_has_loaded && image_has_image) || image_still_loading || image_is_document)
...
void HTMLImageElement::SetLayoutDisposition(LayoutDisposition layout_disposition,
                                            bool force_reattach) {
