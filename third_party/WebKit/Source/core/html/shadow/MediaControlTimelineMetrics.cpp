...
void MediaControlTimelineMetrics::RecordEndGesture(
  ...
  SeekType seek_type = SeekType::kLast;  // Arbitrary inital val to appease MSVC.
  switch (end_state) {
  ...
  if (std::isfinite(media_duration_seconds)) {
    RecordDragPercentByWidth(
        timeline_width, 100.0 * drag_delta_media_seconds_ / media_duration_seconds);
  }
...
void MediaControlTimelineMetrics::RecordEndKey(int timeline_width, int key_code) {
  State end_state = state_;
  state_ = State::kInactive;  // Reset tracking.
...
void MediaControlTimelineMetrics::RecordPlaying(
  ...
  if (!is_fullscreen) {
    ...
    if (is_portrait) {
      DEFINE_STATIC_LOCAL(
          CustomCountHistogram, width_histogram,
          ("Media.Timeline.Width.FullscreenPortrait", kMin, kMax, kBucketCount));
      width_histogram.Count(timeline_width);
    } else {
      DEFINE_STATIC_LOCAL(
          CustomCountHistogram, width_histogram,
          ("Media.Timeline.Width.FullscreenLandscape", kMin, kMax, kBucketCount));
      width_histogram.Count(timeline_width);
    }
  }
}
