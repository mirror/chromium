...
CanvasAsyncBlobCreator* CanvasAsyncBlobCreator::Create(
    ...
    double start_time,
    Document* document) {
  return new CanvasAsyncBlobCreator(unpremultiplied_rgba_image_data,
                                    ConvertMimeTypeStringToEnum(mime_type), size,
                                    callback, start_time, document, nullptr);
}
...
CanvasAsyncBlobCreator* CanvasAsyncBlobCreator::Create(
  ...
  return new CanvasAsyncBlobCreator(unpremultiplied_rgba_image_data,
                                    ConvertMimeTypeStringToEnum(mime_type), size,
                                    nullptr, start_time, document, resolver);
}
...
CanvasAsyncBlobCreator::CanvasAsyncBlobCreator(DOMUint8ClampedArray* data,
  ...
  if (document) {
    parent_frame_task_runner_ = ParentFrameTaskRunners::Create(document->GetFrame());
  }
...
void CanvasAsyncBlobCreator::IdleEncodeRowsJpeg(double deadline_seconds) {
  ...
  if (num_rows_completed_ == size_.Height()) {
    idle_task_status_ = kIdleTaskCompleted;
    RecordElapsedTimeHistogram(kIdleEncodeDuration, kMimeTypeJpeg, elapsed_time_);
  ...
  } else if (num_rows_completed_ == JPEGImageEncoder::kProgressiveEncodeFailed) {
    idle_task_status_ = kIdleTaskFailed;
