...
FloatSize CanvasImageElementSource::ElementSize(
  ...
  return FloatSize(image->ImageSize(
      LayoutObject::ShouldRespectImageOrientation(GetElement().GetLayoutObject()),
      1.0f));
}
...
int CanvasImageElementSource::SourceWidth() {
  SourceImageStatus status;
  RefPtr<Image> image =
      GetSourceImageForCanvas(&status, kPreferNoAcceleration,
                              kSnapshotReasonUnknown, SourceDefaultObjectSize());
  return image->width();
}
