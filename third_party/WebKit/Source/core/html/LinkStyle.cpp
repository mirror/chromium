...
void LinkStyle::SetCSSStyleSheet(
  ...
  DEFINE_STATIC_LOCAL(EnumerationHistogram, restored_cached_style_sheet_histogram,
  ...
  StyleSheetCacheStatus cache_status = cached_style_sheet->GetResponse().WasCached()
                                          ? kStyleSheetInDiskCache
                                          : kStyleSheetNewEntry;
...
void LinkStyle::RemovePendingSheet() {
  ...
  if (type == kNonBlocking) {
    ...
    owner_->GetDocument().GetStyleEngine().ModifiedStyleSheetCandidateNode(*owner_);
    return;
  }

  owner_->GetDocument().GetStyleEngine().RemovePendingSheet(*owner_,
...
void LinkStyle::SetCrossOriginStylesheetStatus(CSSStyleSheet* sheet) {
  if (fetch_following_cors_ && GetResource() && !GetResource()->ErrorOccurred()) {
...
void LinkStyle::Process() {
  ...
  if (owner_->RelAttribute().GetIconType() != kInvalidIcon &&
    ...
    if (!GetDocument().GetContentSecurityPolicy()->AllowImageFromSource(
            builder.Url()))
      return;
    if (GetDocument().GetFrame() && GetDocument().GetFrame()->Loader().Client()) {
