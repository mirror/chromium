...
url::Origin SecurityOriginToURLOrigin(RefPtr<blink::SecurityOrigin> origin) {
...
TEST_F(HTMLIFrameElementTest, AllowAttributeContainerPolicy) {
  ...
  EXPECT_TRUE(container_policy1[0].origins[0].IsSameOriginWith(
  ...
  EXPECT_TRUE(container_policy2[0].origins[0].IsSameOriginWith(
      url::Origin::Create(GURL("http://example.net"))));
  EXPECT_FALSE(container_policy2[1].matches_all_origins);
  EXPECT_EQ(1UL, container_policy2[1].origins.size());
  EXPECT_TRUE(container_policy2[1].origins[0].IsSameOriginWith(
      url::Origin::Create(GURL("http://example.net"))));
  ...
  EXPECT_TRUE(container_policy3[0].origins[0].IsSameOriginWith(
      url::Origin::Create(GURL("http://example.net"))));
  EXPECT_FALSE(container_policy3[1].matches_all_origins);
  EXPECT_EQ(1UL, container_policy3[1].origins.size());
  EXPECT_TRUE(container_policy3[1].origins[0].IsSameOriginWith(
      url::Origin::Create(GURL("http://example.net"))));
}
...
TEST_F(HTMLIFrameElementTest, ConstructContainerPolicy) {
  ...
  EXPECT_EQ(1UL, container_policy[0].origins.size());
  EXPECT_TRUE(container_policy[0].origins[0].IsSameOriginWith(
      SecurityOriginToURLOrigin(GetOriginForFeaturePolicy(frame_element))));
