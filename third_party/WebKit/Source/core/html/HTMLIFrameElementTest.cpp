...
TEST_F(HTMLIFrameElementTest, AllowAttributeContainerPolicy) {
  ...
  EXPECT_EQ(container_policy1[0].origins[0].Serialize(), "http://example.net");
  ...
  EXPECT_EQ(container_policy2[0].origins[0].Serialize(), "http://example.net");
  EXPECT_FALSE(container_policy2[1].matches_all_origins);
  EXPECT_EQ(1UL, container_policy2[1].origins.size());
  EXPECT_EQ(container_policy2[1].origins[0].Serialize(), "http://example.net");
  ...
  EXPECT_EQ(container_policy3[0].origins[0].Serialize(), "http://example.net");
  EXPECT_FALSE(container_policy3[1].matches_all_origins);
  EXPECT_EQ(1UL, container_policy3[1].origins.size());
  EXPECT_EQ(container_policy3[1].origins[0].Serialize(), "http://example.net");
}
...
TEST_F(HTMLIFrameElementTest, SameOriginSandboxAttributeContainerPolicy) {
  ...
  EXPECT_EQ(container_policy[0].origins[0].Serialize(), "http://example.net");
}
...
TEST_F(HTMLIFrameElementTest, ConstructContainerPolicy) {
  ...
  EXPECT_TRUE(container_policy[0].origins[0].IsSameOriginWith(
      GetURLOriginForFeaturePolicy(GetOriginForFeaturePolicy(frame_element))));
