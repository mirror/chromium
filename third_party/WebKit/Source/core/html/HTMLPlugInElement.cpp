...
void HTMLPlugInElement::RequestPluginCreationWithoutLayoutObjectIfPossible() {
  ...
  if (!GetDocument().GetFrame() ||
      !GetDocument().GetFrame()->Loader().Client()->CanCreatePluginWithoutRenderer(
          service_type_))
    return;
...
void HTMLPlugInElement::CreatePluginWithoutLayoutObject() {
  DCHECK(GetDocument().GetFrame()->Loader().Client()->CanCreatePluginWithoutRenderer(
      service_type_));
  ...
  LoadPlugin(url, service_type_, param_names, param_values, use_fallback, false);
}
...
bool HTMLPlugInElement::RequestObject(const String& url,
                                      ...
                                      const Vector<String>& param_names,
                                      const Vector<String>& param_values) {
  bool result = RequestObjectInternal(url, mime_type, param_names, param_values);
