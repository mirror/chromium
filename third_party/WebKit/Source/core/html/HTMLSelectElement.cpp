...
void HTMLSelectElement::setLength(unsigned new_len,
  ...
  if (diff < 0) {  // Add dummy elements.
    do {
      AppendChild(GetDocument().createElement(optionTag, kCreatedByCreateElement),
                  exception_state);
      if (exception_state.HadException())
        break;
    } while (++diff);
...
void HTMLSelectElement::SaveLastSelection() {
  ...
  for (auto& element : GetListItems())
    last_on_change_selection_.push_back(isHTMLOptionElement(*element) &&
...
void HTMLSelectElement::UpdateListBoxSelection(bool deselect_other_options,
  ...
  int start = std::min(active_selection_anchor_index, active_selection_end_index);
  ...
  for (const auto& option : GetOptionList()) {
    ...
    if (i >= start && i <= end) {
      ...
      option->SetDirty(true);
               ...
               i >= static_cast<int>(cached_state_for_active_selection_.size())) {
...
void HTMLSelectElement::ResetToDefaultSelection(ResetReason reason) {
  ...
  if (!last_selected_option && size_ <= 1 &&
    ...
    SelectOption(first_enabled_option, reason == kResetReasonSelectedOptionRemoved
                                         ? 0
...
void HTMLSelectElement::SelectOption(HTMLOptionElement* element,
  ...
  if (element) {
    ...
    if (!active_selection_end_ || !IsMultiple() || flags & kDeselectOtherOptions)
  ...
  if (!IsMultiple()) {
    ...
    for (size_t i = 0; i < state.ValueSize(); i += 2) {
      ...
      if (index < items_size && isHTMLOptionElement(items[index]) &&
        ...
        size_t found_index = SearchOptionsForValue(value, start_index, items_size);
        if (found_index == kNotFound)
...
void HTMLSelectElement::MenuListDefaultEventHandler(Event* event) {
  ...
  if (event->type() == EventTypeNames::keypress) {
    ...
    if (key_code == ' ' && IsSpatialNavigationEnabled(GetDocument().GetFrame())) {
  ...
  if (event->type() == EventTypeNames::mousedown && event->IsMouseEvent() &&
    ...
    InputDeviceCapabilities* source_capabilities =
        GetDocument().domWindow()->GetInputDeviceCapabilities()->FiresTouchEvents(
...
void HTMLSelectElement::ListBoxDefaultEventHandler(Event* event) {
  if (event->type() == EventTypeNames::gesturetap && event->IsGestureEvent()) {
    ...
    if (Page* page = GetDocument().GetPage())
      page->GetAutoscrollController().StartAutoscrollForSelection(GetLayoutObject());
    ...
    if (end_option && handled) {
      ...
      bool select_new_item = !is_multiple_ ||
...
String HTMLSelectElement::ItemText(const Element& element) const {
  ...
  if (isHTMLOptGroupElement(element))
    ...
    item_string = toHTMLOptionElement(element).TextIndentedToRespectGroupLabel();
