...
class MockHTMLResourcePreloader : public HTMLResourcePreloader {
 ...
 public:
  explicit MockHTMLResourcePreloader(Document& document,
      ...
      : HTMLResourcePreloader(document), expected_referrer_(expected_referrer) {}
...
TEST_F(CSSPreloadScannerTest, ScanFromResourceClient) {
  ...
  dummy_page_holder->GetDocument().GetSettings()->SetCSSExternalScannerNoPreload(true);
...
TEST_F(CSSPreloadScannerTest, ReferrerPolicyHeader) {
  std::unique_ptr<DummyPageHolder> dummy_page_holder =
      DummyPageHolder::Create(IntSize(500, 500));
  dummy_page_holder->GetDocument().GetSettings()->SetCSSExternalScannerPreload(true);
