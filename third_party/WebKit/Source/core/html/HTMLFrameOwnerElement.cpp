...
void HTMLFrameOwnerElement::SetSandboxFlags(SandboxFlags flags) {
  ...
  if (ContentFrame())
    GetDocument().GetFrame()->Loader().Client()->DidChangeSandboxFlags(ContentFrame(),
                                                                 flags);
}
...
void HTMLFrameOwnerElement::FrameOwnerPropertiesChanged() {
  ...
  // the subframe hasn't been created yet.
  if (ContentFrame())
    GetDocument().GetFrame()->Loader().Client()->DidChangeFrameOwnerProperties(this);
}
  ...
  if (doc && doc->GetFrame()) {
    ...
    if (IsDisplayNone() != will_be_display_none) {
      doc->WillChangeFrameOwnerProperties(MarginWidth(), MarginHeight(),
                                          ScrollingMode(), will_be_display_none);
    }
  }
...
bool HTMLFrameOwnerElement::LoadOrRedirectSubframe(
  ...
  if (GetDocument().GetFrame()->GetPage()->SubframeCount() >= Page::kMaxNumberOfFrames)
    return false;
