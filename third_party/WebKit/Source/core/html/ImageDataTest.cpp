...
TEST_F(ImageDataTest, MAYBE_CreateImageDataTooBig) {
  ...
  ImageData* too_big_image_data = ImageData::Create(32767, 32767, exception_state);
...
TEST_F(ImageDataTest,
  ...
  xform->apply(SkColorSpaceXform::ColorFormat::kRGBA_F16_ColorFormat, f16_pixels,
  ...
  data = ImageData::ConvertPixelsFromCanvasPixelFormatToImageDataStorageFormat(
      contents_rgba32, kRGBA8CanvasPixelFormat, kUint8ClampedArrayStorageFormat);
...
bool ConvertPixelsToColorSpaceAndPixelFormatForTest(
  ...
  if (u8_array) {
    ...
    src_sk_color_space =
        ImageData::GetSkColorSpaceForTest(src_color_space, kF16CanvasPixelFormat);
  }
  ...
  std::unique_ptr<SkColorSpaceXform> xform =
      SkColorSpaceXform::New(src_sk_color_space.get(), dst_sk_color_space.get());
    ...
    for (unsigned j = 0; j < num_image_data_storage_formats; j++) {
      ...
      for (unsigned k = 0; k < num_canvas_color_settings; k++) {
        unsigned output_length =
            (canvas_pixel_formats[k] == kRGBA8CanvasPixelFormat) ? data_length
                                                               : data_length * 2;
