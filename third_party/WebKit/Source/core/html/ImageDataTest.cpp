...
TEST_F(ImageDataTest, TestCreateImageDataFromStaticBitmapImage) {
  ...
  uint8_t u8_pixels_unpremul[] = {255, 0,   0,   255,  // Red
                                  0,   0,   0,   0,    // Transparent
                                  255, 192, 128, 64,   // Decreasing values
                                  93,  117, 205, 41};  // Random values

  uint8_t u8_pixels_premul[kNumColorComponents];
  uint16_t f16_pixels_unpremul[kNumColorComponents];
  uint16_t f16_pixels_premul[kNumColorComponents];
  float f32_pixels_unpremul[kNumColorComponents];
  float f32_pixels_premul[kNumColorComponents];

  auto prepareSourcePixels = [&u8_pixels_unpremul](auto buffer, bool is_premul,
                                                   auto color_type) {
    ...
    EXPECT_TRUE(
        xform->apply(color_type, buffer,
                     ...
                     u8_pixels_unpremul, kNumPixels,
                     is_premul ? kPremul_SkAlphaType : kUnpremul_SkAlphaType));
  };

  prepareSourcePixels(u8_pixels_premul, true,
                      SkColorSpaceXform::ColorFormat::kRGBA_8888_ColorFormat);
  prepareSourcePixels(f16_pixels_unpremul, false,
                      SkColorSpaceXform::ColorFormat::kRGBA_F16_ColorFormat);
  prepareSourcePixels(f16_pixels_premul, true,
                      SkColorSpaceXform::ColorFormat::kRGBA_F16_ColorFormat);
  prepareSourcePixels(f32_pixels_unpremul, false,
                      SkColorSpaceXform::ColorFormat::kRGBA_F32_ColorFormat);
  prepareSourcePixels(f32_pixels_premul, true,
                      SkColorSpaceXform::ColorFormat::kRGBA_F32_ColorFormat);
  ...
  auto contents_u8_premul =
      createBufferContent(u8_pixels_premul, kNumColorComponents);
  auto contents_u8_unpremul =
      createBufferContent(u8_pixels_unpremul, kNumColorComponents);
  auto contents_f16_premul =
      createBufferContent(f16_pixels_premul, kNumColorComponents * 2);
  auto contents_f16_unpremul =
      createBufferContent(f16_pixels_unpremul, kNumColorComponents * 2);
  ...
  ImageData* image_data_u8[4];
  ImageData* image_data_f32[4];
  // u8 premul
  image_data_u8[0] = ImageData::Create(image_u8_premul);
  image_data_u8[1] = ImageData::Create(image_u8_unpremul, kPremultiplyAlpha);
  // u8 unpremul
  image_data_u8[2] = ImageData::Create(image_u8_unpremul);
  image_data_u8[3] = ImageData::Create(image_u8_premul, kUnpremultiplyAlpha);
  // f32 premul
  image_data_f32[0] = ImageData::Create(image_f16_premul);
  image_data_f32[1] = ImageData::Create(image_f16_unpremul, kPremultiplyAlpha);
  // f32 unpremul
  image_data_f32[2] = ImageData::Create(image_f16_unpremul);
  image_data_f32[3] = ImageData::Create(image_f16_premul, kUnpremultiplyAlpha);

  // Associating expected color component arrays
  uint8_t* pixel_arrays_u8[4] = {u8_pixels_premul, u8_pixels_premul,
                                 u8_pixels_unpremul, u8_pixels_unpremul};
  float* pixel_arrays_f32[4] = {f32_pixels_premul, f32_pixels_premul,
                                f32_pixels_unpremul, f32_pixels_unpremul};

  // Comparing ImageData with the source StaticBitmapImage
  for (unsigned i = 0; i < 4; i++) {
    ColorCorrectionTestUtils::CompareColorCorrectedPixels(
        pixel_arrays_u8[i], image_data_u8[i]->BufferBase()->Data(), kNumPixels,
        kUint8ClampedArrayStorageFormat, kAlphaUnmultiplied,
  ...
  for (unsigned i = 0; i < 4; i++) {
    ColorCorrectionTestUtils::CompareColorCorrectedPixels(
        pixel_arrays_f32[i], image_data_f32[i]->BufferBase()->Data(),
        kNumPixels, kFloat32ArrayStorageFormat, kAlphaUnmultiplied,
        kUnpremulRoundTripTolerance);
  }
}
