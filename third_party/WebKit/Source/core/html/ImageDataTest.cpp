...
TEST_F(ImageDataTest,
  ...
  ColorCorrectionTestUtils::CompareColorCorrectedPixels(
      data->BaseAddress(), rgba32_pixels, kNumPixels, 4, kAlphaUnmultiplied,
  ...
  ColorCorrectionTestUtils::CompareColorCorrectedPixels(
      data->BaseAddress(), f32_pixels, kNumPixels, 16, kAlphaUnmultiplied,
    ...
    for (unsigned j = 0; j < num_image_data_storage_formats; j++) {
      ...
      for (unsigned k = 0; k < num_canvas_color_settings; k++) {
        ...
        EXPECT_TRUE(
            ColorCorrectionTestUtils::
                ConvertPixelsToColorSpaceAndPixelFormatForTest(
                    data_array->BaseAddress(), data_length, bytes_per_data,
                    image_data_color_spaces[i], canvas_color_spaces[k],
        ...
        PremulUnpremulRoundingTolerance premul_unpremul_rounding_tolerance =
            (canvas_color_spaces[k] == kSRGBCanvasColorSpace &&
        ...
        ColorCorrectionTestUtils::CompareColorCorrectedPixels(
            ...
            (canvas_pixel_formats[k] == kRGBA8CanvasPixelFormat) ? 4 : 8,
            kAlphaUnmultiplied, premul_unpremul_rounding_tolerance);
      }
    }
  }
  delete[] u16_pixels;
