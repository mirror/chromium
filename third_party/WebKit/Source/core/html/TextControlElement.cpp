...
void TextControlElement::select() {
  ...
  // the selection.
  focus(FocusParams(SelectionBehaviorOnFocus::kNone, kWebFocusTypeNone, nullptr));
...
bool TextControlElement::SetSelectionRange(
  ...
  frame->Selection().SetSelection(
      SelectionInDOMTree::Builder()
          .Collapse(direction == kSelectionHasBackwardDirection ? end_position
                                                               : start_position)
...
String TextControlElement::ValueWithHardLineBreaks() const {
  ...
  LayoutBlockFlow* layout_object = ToLayoutBlockFlow(inner_text->GetLayoutObject());
  if (!layout_object)
    return value();
...
String TextControlElement::DirectionForFormData() const {
  for (const HTMLElement* element = this; element;
    ...
    const AtomicString& dir_attribute_value = element->FastGetAttribute(dirAttr);
