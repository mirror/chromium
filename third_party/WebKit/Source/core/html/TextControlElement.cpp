...
// TODO(sebsg): Create and use a new suggested-value element instead.
void TextControlElement::SetSuggestedValue(const String& value) {
  ...
  if (placeholder) {
    if (!suggested_value_.IsEmpty()) {
      // Change the style to show the suggested value.
      placeholder->SetInlineStyleProperty(CSSPropertyWebkitTextFillColor,
                                          CSSValueBlack, true);
      placeholder->SetInlineStyleProperty(
          CSSPropertyWebkitTextSecurity,
          IsPasswordType() ? CSSValueDisc : CSSValueNone, true);
    } else {
      // Put back the normal placeholder style.
      placeholder->RemoveInlineStyleProperty(CSSPropertyWebkitTextFillColor);
      placeholder->RemoveInlineStyleProperty(CSSPropertyWebkitTextSecurity);
    }
  }
}
