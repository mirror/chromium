...
void HTMLFormElement::HandleLocalEvents(Event& event) {
  ...
  if (event.eventPhase() != Event::kCapturingPhase && target_node &&
      target_node != this && (event.type() == EventTypeNames::submit ||
...
bool HTMLFormElement::ValidateInteractively() {
  ...
  if (!CheckInvalidControlsAndCollectUnhandled(
    ...
    return true;
  UseCounter::Count(GetDocument(), UseCounter::kFormValidationAbortedSubmission);
  ...
  for (const auto& unhandled : unhandled_invalid_controls) {
    if (unhandled->IsFocusable()) {
      ...
      UseCounter::Count(GetDocument(), UseCounter::kFormValidationShowedMessage);
      break;
    }
  }
  ...
  for (const auto& element : ListedElements()) {
    if (element->IsFormControlElement() &&
      ...
      if (RuntimeEnabledFeatures::unclosedFormControlIsInvalidEnabled()) {
        ...
        GetDocument().AddConsoleMessage(ConsoleMessage::Create(
            kSecurityMessageSource, kErrorMessageLevel,
            "Form submission failed, as the <" + tag_name + "> element named "
                "'" + element->GetName() + "' was implicitly closed by reaching "
  ...
  if (event && !submit_button) {
    ...
    for (const auto& listed_element : ListedElements()) {
      if (!listed_element->IsFormControlElement())
        continue;
      HTMLFormControlElement* control = ToHTMLFormControlElement(listed_element);
...
void HTMLFormElement::ParseAttribute(
  ...
  if (name == actionAttr) {
    ...
    if (GetDocument().GetInsecureRequestPolicy() & kUpgradeInsecureRequests)
      return;
    KURL action_url = GetDocument().CompleteURL(attributes_.Action().IsEmpty()
...
void HTMLFormElement::GetNamedElements(
  ...
  if (named_items.size() && named_items.front() != element_from_past) {
    ...
    UseCounter::Count(GetDocument(), UseCounter::kFormNameAccessForPastNamesMap);
  }
}
...
void HTMLFormElement::AnonymousNamedGetter(
  ...
  if (only_match_img) {
    UseCounter::Count(GetDocument(), UseCounter::kFormNameAccessForImageElement);
    ...
    for (auto& element : elements) {
      if (isHTMLImageElement(*element) && !element->IsDescendantOf(this)) {
        UseCounter::Count(
            GetDocument(), UseCounter::kFormNameAccessForNonDescendantImageElement);
        break;
      }
    }
  }
