...
inline HTMLIFrameElement::HTMLIFrameElement(Document& document)
    : HTMLFrameElementBase(iframeTag, document),
      ...
      allowed_activation_delegation_(
          HTMLIFrameElementAllowedActivationDelegation::Create(this)),
...
DEFINE_TRACE(HTMLIFrameElement) {
  visitor->Trace(allowed_activation_delegation_);
...
DOMTokenList* HTMLIFrameElement::allowedActivationDelegation() const {
  return allowed_activation_delegation_.Get();
}
...
void HTMLIFrameElement::ParseAttribute(
  ...
  } else if (name == allowedactivationdelegationAttr) {
    allowed_activation_delegation_->DidUpdateAttributeValue(params.old_value,
                                                            value);
    String invalid_tokens;
    SetActivationDelegationFlags(
        allowed_activation_delegation_->GetFlags(invalid_tokens));
    if (!invalid_tokens.IsNull()) {
      GetDocument().AddConsoleMessage(ConsoleMessage::Create(
          kOtherMessageSource, kErrorMessageLevel,
          "Error while parsing the 'allowedActivationDelegation' attribute: " +
              invalid_tokens));
    }
    UseCounter::Count(GetDocument(), WebFeature::kAllowedActivationDelegation);
