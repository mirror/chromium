...
constexpr double kPipStyleChangeTransSeconds = 0.2;
constexpr double kPipHiddenAnimationSeconds = 0.3;
...
PictureInPictureInterstitial::PictureInPictureInterstitial(
  ...
  LOG(ERROR) << "PictureInPictureInterstitial::PictureInPictureInterstitial";
...
void PictureInPictureInterstitial::Show() {
  ...
  interstitial_timer_.StartOneShot(kPipStyleChangeTransSeconds, BLINK_FROM_HERE);
}
...
void PictureInPictureInterstitial::Hide() {
  ...
  interstitial_timer_.StartOneShot(kPipHiddenAnimationSeconds, BLINK_FROM_HERE);
}
