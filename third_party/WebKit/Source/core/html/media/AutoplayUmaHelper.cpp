...
std::unique_ptr<ukm::UkmEntryBuilder> AutoplayUmaHelper::CreateUkmBuilder(
    const char* event) {
  ukm::MojoUkmRecorder* mojo_ukm_recorder =
      element_->GetDocument().UkmRecorder();
  DCHECK(mojo_ukm_recorder);

  return mojo_ukm_recorder_->GetEntryBuilder(
      element_->GetDocument().UkmSourceID(), event);
}
