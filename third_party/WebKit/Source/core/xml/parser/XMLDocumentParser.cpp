...
void XMLDocumentParser::NotifyFinished(Resource* unused_resource) {
  ...
  double script_parser_blocking_time = parser_blocking_pending_script_load_start_time_;
  parser_blocking_pending_script_load_start_time_ = 0.0;
...
bool XMLDocumentParser::ParseDocumentFragment(
  ...
  if (context_element && (context_element->HasLocalName(scriptTag.LocalName()) ||
  ...
  XMLDocumentParser* parser =
      XMLDocumentParser::Create(fragment, context_element, parser_content_policy);
...
static bool ShouldAllowExternalLoad(const KURL& url) {
  ...
  if (!XMLDocumentParserScope::current_document_->GetSecurityOrigin()->CanRequest(
          url)) {
...
static inline void HandleElementAttributes(
  ...
  for (int i = 0; i < nb_attributes; ++i) {
    int value_length = static_cast<int>(attributes[i].end - attributes[i].value);
...
void XMLDocumentParser::StartElementNs(const AtomicString& local_name,
  ...
  HandleNamespaceAttributes(prefixed_attributes, libxml_namespaces, nb_namespaces,
                            exception_state);
  ...
  } else if (!parsing_fragment_ && is_first_element && GetDocument()->GetFrame()) {
...
static void StartElementNsHandler(void* closure,
  ...
  GetParser(closure)->StartElementNs(
      ...
      nb_namespaces, namespaces, nb_attributes, nb_defaulted, libxml_attributes);
}
...
static void ProcessingInstructionHandler(void* closure,
                                         ...
                                         const xmlChar* data) {
  GetParser(closure)->GetProcessingInstruction(ToString(target), ToString(data));
}
...
static size_t ConvertUTF16EntityToUTF8(const UChar* utf16_entity,
  ...
  WTF::Unicode::ConversionResult conversion_result =
      WTF::Unicode::ConvertUTF16ToUTF8(&utf16_entity,
                                       utf16_entity + number_of_code_units, &target,
                                       target + target_size);
...
static xmlEntityPtr GetXHTMLEntity(const xmlChar* name) {
  ...
  size_t entity_length_in_utf8 =
      ConvertUTF16EntityToUTF8(utf16_decoded_entity, number_of_code_units,
...
void XMLDocumentParser::DoEnd() {
  ...
  if (xml_viewer_mode) {
    ...
    xmlDocPtr doc =
        XmlDocPtrForString(GetDocument(), original_source_for_transform_.ToString(),
...
static void AttributesStartElementNsHandler(void* closure,
  ...
  for (int i = 0; i < nb_attributes; ++i) {
    ...
    String attr_q_name =
        attr_prefix.IsEmpty() ? attr_local_name : attr_prefix + ":" + attr_local_name;
