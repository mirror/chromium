...
XMLDocumentParser::XMLDocumentParser(DocumentFragment* fragment,
  ...
  for (; !elem_stack.IsEmpty(); elem_stack.pop_back()) {
    ...
    // information from the element name should have higher priority. So we
    // check xmlns attributes first, then overwrite the map with namespace
    // information of the element name.
