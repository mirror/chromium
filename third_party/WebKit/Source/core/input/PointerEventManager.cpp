...
void PointerEventManager::HandlePointerInterruption(
  ...
  DCHECK(web_pointer_event.GetType() ==
         WebInputEvent::Type::kPointerInterrupted);
...
WebInputEventResult PointerEventManager::HandlePointerEvent(
    ...
    const Vector<WebPointerEvent>& coalesced_events) {
  if (event.GetType() == WebInputEvent::Type::kPointerInterrupted) {
  ...
  if (event.scroll_capable) {
    if (!touch_event_manager_->IsAnyTouchActive()) {
      DCHECK_EQ(event.GetType(), WebInputEvent::kPointerDown);
      scroll_capable_pointers_canceled_ = false;
    }
  }
  ...
  // Any pointer lifting is a user gesture only when it wasn't associated with a
  // scroll.
  // https://docs.google.com/document/d/1oF1T3O7_E4t1PYHV6gyCwHxOi3ystm0eSL5xZu7nvOg/edit#
  // We have 3 different UserGestures for touchend and pointerup and
  // the mouse events generated by GestureTap.
  // For the rare case of multi-finger scenarios spanning documents, we
  // are creating multiple gestures one for each pointer of that type.
  ...
  if (event.GetType() == WebInputEvent::kPointerUp &&
    ...
    holder = Frame::NotifyUserActivation(pointer_event_target.target_frame);
  }
