...
WebInputEventResult PointerEventManager::DispatchPointerEvent(
  ...
  if ((event_type == EventTypeNames::pointerout ||
    ...
    EventTarget* target_under_pointer = node_under_pointer_.at(pointer_id).target;
...
WebInputEventResult PointerEventManager::HandleTouchEvents(
  ...
  if (event.GetType() == WebInputEvent::kTouchEnd &&
    ...
    possible_gesture_token =
        DocumentUserGestureToken::Create(touch_infos[0].target_frame->GetDocument());
  }
  ...
  for (auto touch_info : touch_infos) {
    ...
    if (touch_info.touch_node && touch_info.target_frame &&
      ...
      PointerEvent* pointer_event = pointer_event_factory_.Create(
          ...
          touch_info.touch_node ? touch_info.touch_node->GetDocument().domWindow()
                              : nullptr);
      ...
      if (result != WebInputEventResult::kNotHandled &&
        ...
        touch_ids_for_canceled_pointerdowns_.push_back(event.unique_touch_event_id);
      }
    }
  }
}
...
WebInputEventResult PointerEventManager::SendMousePointerEvent(
  ...
  PointerEvent* pointer_event =
      pointer_event_factory_.Create(mouse_event_type, mouse_event, coalesced_events,
                                   frame_->GetDocument()->domWindow());
  ...
  if (pointer_event->isPrimary() &&
    ...
    result = EventHandlingUtil::MergeEventResult(
        ...
        mouse_event_manager_->DispatchMouseEvent(
            mouse_target, mouse_event_type, mouse_event, canvas_region_id, nullptr));
  }
