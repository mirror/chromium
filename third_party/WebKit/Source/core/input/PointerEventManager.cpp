...
LayoutSize GetHitTestRectForAdjustment(const LayoutSize& touch_area) {
  LayoutSize maxSize(kMaxAdjustmentRadiusDips, kMaxAdjustmentRadiusDips);
...
bool PointerEventManager::ShouldAdjustPointerEvent(
  ...
  return RuntimeEnabledFeatures::UnifiedTouchAdjustmentEnabled() &&
         ...
         pointer_event_factory_.IsOrWillBePrimaryPointer(pointer_event);
}
...
bool PointerEventManager::AdjustPointerEvent(WebPointerEvent* pointer_event) {
  ...
  LayoutSize padding = GetHitTestRectForAdjustment(
      LayoutSize(pointer_event->width / 2, pointer_event->height / 2));
  ...
  frame_->GetEventHandler().CachePointerEventAdjustResult(
      adjusted, pointer_event->unique_touch_event_id, adjusted_point);
