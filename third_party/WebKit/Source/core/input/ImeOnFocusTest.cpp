...
class ImeOnFocusTest : public ::testing::Test {
 public:
  ImeOnFocusTest() : base_url_("http://www.test.com/") {
    ...
    loader_factory_ = platform->GetURLLoaderMockFactory();
  }

  void TearDown() override {
    loader_factory_->UnregisterAllURLsAndClearMemoryCache();
  }
...
void ImeOnFocusTest::RunImeOnFocusTest(
  ...
  RegisterMockedURLLoadFromBase(
      ...
      WebString::FromUTF8(file_name), loader_factory_);
  ...
  if (!frame.empty()) {
    RegisterMockedURLLoadFromBase(WebString::FromUTF8(base_url_),
                                  ...
                                  WebString::FromUTF8(frame), loader_factory_);
