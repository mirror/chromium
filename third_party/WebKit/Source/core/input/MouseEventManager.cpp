...
void MouseEventManager::MouseEventBoundaryEventDispatcher::DispatchEnter(
    ...
    EventTarget* related_target,
    bool check_for_listener) {
  Dispatch(target, related_target, EventTypeNames::mouseenter, canvas_region_id_,
           *web_mouse_event_, check_for_listener);
}
...
WebInputEventResult MouseEventManager::DispatchMouseEvent(
  ...
  if (target && target->ToNode() &&
    ...
    MouseEvent* event =
        MouseEvent::Create(mouse_event_type, target_node->GetDocument().domWindow(),
                           mouse_event, click_count, canvas_region_id,
...
WebInputEventResult MouseEventManager::DispatchMouseClickIfNeeded(
  ...
  const bool should_dispatch_click_event =
      click_count_ > 0 && !context_menu_event && mev.InnerNode() && click_node_ &&
      mev.InnerNode()->CanParticipateInFlatTree() &&
      click_node_->CanParticipateInFlatTree() &&
      !(frame_->GetEventHandler().GetSelectionController().HasExtendedSelection() &&
        IsLinkSelection(mev));
...
void MouseEventManager::FakeMouseMoveEventTimerFired(TimerBase* timer) {
  ...
  WebMouseEvent fake_mouse_move_event(
      ...
      WebFloatPoint(last_known_mouse_position_.X(), last_known_mouse_position_.Y()),
...
void MouseEventManager::SetNodeUnderMouse(Node* target,
  ...
  if (node_under_mouse_ &&
    ...
    if (ScrollableArea* scrollable_area_for_node_under_mouse =
            EventHandlingUtil::AssociatedScrollableArea(layer_for_node_under_mouse))
...
void MouseEventManager::NodeWillBeRemoved(Node& node_to_be_removed) {
  if (node_to_be_removed.IsShadowIncludingInclusiveAncestorOf(click_node_.Get())) {
...
void MouseEventManager::HandleMousePressEventUpdateStates(
  ...
  mouse_down_may_start_drag_ = false;
  mouse_down_may_start_autoscroll_ = false;
  mouse_down_timestamp_ = TimeTicks::FromSeconds(mouse_event.TimeStampSeconds());
...
void MouseEventManager::DispatchFakeMouseMoveEventSoonInQuad(
  ...
  if (!quad.ContainsPoint(view->RootFrameToContents(last_known_mouse_position_)))
    return;
...
WebInputEventResult MouseEventManager::HandleMousePressEvent(
  ...
  frame_->GetEventHandler().GetSelectionController().HandleMousePressEvent(event);
  ...
  mouse_down_may_start_autoscroll_ =
      frame_->GetEventHandler().GetSelectionController().MouseDownMayStartSelect() ||
...
WebInputEventResult MouseEventManager::HandleMouseReleaseEvent(
  ...
  return frame_->GetEventHandler().GetSelectionController().HandleMouseReleaseEvent(
             event, drag_start_pos_)
             ? WebInputEventResult::kHandledSystem
             : WebInputEventResult::kNotHandled;
}

void MouseEventManager::UpdateSelectionForMouseDrag() {
  frame_->GetEventHandler().GetSelectionController().UpdateSelectionForMouseDrag(
      mouse_press_node_, drag_start_pos_, last_known_mouse_position_);
}

bool MouseEventManager::HandleDragDropIfPossible(
    const GestureEventWithHitTestResults& targeted_event) {
  if (frame_->GetSettings() && frame_->GetSettings()->GetTouchDragDropEnabled() &&
      frame_->View()) {
...
bool MouseEventManager::HandleDrag(const MouseEventWithHitTestResults& event,
  ...
  if (mouse_down_may_start_drag_) {
    ...
    if (node) {
      ...
      GetDragState().drag_src_ = frame_->GetPage()->GetDragController().DraggableNode(
...
bool MouseEventManager::TryStartDrag(
  ...
  // data.
  GetDragState().drag_data_transfer_->SetAccessPolicy(kDataTransferImageWritable);
...
WebInputEventResult MouseEventManager::DispatchDragEvent(
  ...
  DragEvent* me = DragEvent::Create(
      ...
      MouseEvent::WebInputEventModifiersToButtons(event.GetModifiers()), nullptr,
      TimeTicks::FromSeconds(event.TimeStampSeconds()), data_transfer,
...
bool MouseEventManager::DragThresholdExceeded(
  ...
  if (!view)
    return false;
  IntPoint drag_location = view->RootFrameToContents(drag_location_in_root_frame);
