 ...
 public:
  ...
  void handleEvent(ExecutionContext*, Event* event) override {
    ...
    if (pointer_type == "mouse")
      mouse_event_received_cnt_++;
    else if (pointer_type == "touch")
      touch_event_received_cnt_++;
    else if (pointer_type == "pen")
      pen_event_received_cnt_++;
  }

  int mouseEventCount() const { return mouse_event_received_cnt_; }
  int touchEventCount() const { return touch_event_received_cnt_; }
  int penEventCount() const { return pen_event_received_cnt_; }

 private:
  CheckPointerEventListenerCallback()
      : EventListener(EventListener::kCPPEventListenerType) {
    mouse_event_received_cnt_ = 0;
    touch_event_received_cnt_ = 0;
    pen_event_received_cnt_ = 0;
  }
  int mouse_event_received_cnt_;
  int touch_event_received_cnt_;
  int pen_event_received_cnt_;
};
