...
WebInputEventResult TouchEventManager::DispatchTouchEvents(
  ...
  if (event.GetType() == WebInputEvent::kTouchEnd ||
      event.GetType() == WebInputEvent::kTouchCancel || event.touches_length > 1) {
    suppressing_touchmoves_within_slop_ = false;
  }
  ...
  for (auto touch_info : touch_infos) {
    ...
    Touch* touch = Touch::Create(
        ...
        point.screen_position, touch_info.content_point, touch_info.adjusted_radius,
        point.rotation_angle, point.force, touch_info.region);
    ...
    if (target_touches_iterator == touches_by_target.end()) {
      ...
      target_touches_iterator = touches_by_target.Find(touch_info.touch_node.Get());
    }
    ...
    if (point_state != WebTouchPoint::kStateStationary && touch_info.known_target) {
...
bool TouchEventManager::ReHitTestTouchPointsIfNeeded(
  ...
  ASSERT(frame_->View());
  if (touch_sequence_document_ && (!touch_sequence_document_->GetFrame() ||
