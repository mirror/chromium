...
class UnbufferedInputEventsTrackingChromeClient : public EmptyChromeClient {
 public:
  ...
  void RequestUnbufferedInputEvents(LocalFrame*) override {
    ;
    needs_unbuffered_ = true;
  }

  bool GetAndResetUnbufferedInputEvents() {
    bool value = needs_unbuffered_;
    needs_unbuffered_ = false;
    return value;
  }

 private:
  bool needs_unbuffered_ = false;
};
...
TEST_F(EventHandlerLatencyTest, NeedsUnbufferedInput) {
  ...
  ASSERT_FALSE(chrome_client_->GetAndResetUnbufferedInputEvents());
  ...
  GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(
      mouse_press_event);
  ASSERT_TRUE(chrome_client_->GetAndResetUnbufferedInputEvents());
