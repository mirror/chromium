...
class EventHandlerTest : public ::testing::Test {
 protected:
  ...
  FrameSelection& Selection() const { return GetDocument().GetFrame()->Selection(); }
...
TEST_F(EventHandlerTest, dragSelectionAfterScroll) {
  ...
  GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(mouse_down_event);

  WebMouseEvent mouse_move_event(WebInputEvent::kMouseMove, WebFloatPoint(100, 50),
  ...
  GetPage().GetAutoscrollController().Animate(WTF::MonotonicallyIncreasingTime());
  ...
  GetDocument().GetFrame()->GetEventHandler().HandleMouseReleaseEvent(mouse_up_event);
...
TEST_F(EventHandlerTest, multiClickSelectionFromTap) {
  ...
  GetDocument().GetFrame()->GetEventHandler().HandleGestureEvent(single_tap_event);
  ASSERT_TRUE(
  ...
  GetDocument().GetFrame()->GetEventHandler().HandleGestureEvent(double_tap_event);
  ASSERT_TRUE(
      Selection().ComputeVisibleSelectionInDOMTreeDeprecated().IsRange());
  EXPECT_EQ(Position(line, 0),
            Selection().ComputeVisibleSelectionInDOMTreeDeprecated().Start());
  if (GetDocument().GetFrame()->GetEditor().IsSelectTrailingWhitespaceEnabled()) {
  ...
  GetDocument().GetFrame()->GetEventHandler().HandleGestureEvent(triple_tap_event);
  ASSERT_TRUE(
      Selection().ComputeVisibleSelectionInDOMTreeDeprecated().IsRange());
  EXPECT_EQ(Position(line, 0),
            Selection().ComputeVisibleSelectionInDOMTreeDeprecated().Start());
...
TEST_F(EventHandlerTest, draggedInlinePositionTest) {
  SetHtmlInnerHTML(
      ...
      "</div>");
  WebMouseEvent mouse_down_event(WebMouseEvent::kMouseDown, WebFloatPoint(262, 29),
  ...
  EXPECT_EQ(
      IntPoint(12, 29),
      GetDocument().GetFrame()->GetEventHandler().DragDataTransferLocationForTesting());
}
...
TEST_F(EventHandlerTest, sendContextMenuEventWithHover) {
  ...
  EXPECT_EQ(
      ...
      GetDocument().GetFrame()->GetEventHandler().SendContextMenuEvent(mouse_down_event));
}
...
TEST_F(EventHandlerTest, EmptyTextfieldInsertionOnLongPress) {
  ...
  GetDocument().GetFrame()->GetEventHandler().HandleGestureEvent(long_press_event);
...
TEST_F(EventHandlerTest, HandleNotShownOnMouseEvents) {
  ...
  GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(left_mouse_press_event);
...
TEST_F(EventHandlerTest, dragEndInNewDrag) {
  ...
  WebMouseEvent mouse_up_event(
      ...
      TimeTicks::Now().InSeconds());
  mouse_up_event.SetFrameScale(1);
  GetDocument().GetFrame()->GetEventHandler().DragSourceEndedAt(mouse_up_event,
                                                       kDragOperationNone);
...
TEST_F(EventHandlerTooltipTest, mouseLeaveClearsTooltip) {
  ...
  GetDocument().GetFrame()->GetEventHandler().HandleMouseLeaveEvent(mouse_leave_event);
