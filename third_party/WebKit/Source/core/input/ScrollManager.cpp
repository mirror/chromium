...
void ScrollManager::SnapAtGestureScrollEnd() {
  ...
  ScrolledAxis scrolled_axis = kScrolledAxisBoth;
  if (!did_scroll_y_for_scroll_gesture_)
    scrolled_axis = kScrolledAxisX;
  if (!did_scroll_x_for_scroll_gesture_)
    scrolled_axis = kScrolledAxisY;
  ScrollOffset snap_offset;
  if (snap_coordinator->GetSnapPosition(*layout_box, scrolled_axis,
                                        &snap_offset)) {
