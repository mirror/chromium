...
void ScrollManager::ClearGestureScrollState() {
  ...
  did_scroll_x_for_whole_scroll_gesture_ = false;
  did_scroll_y_for_whole_scroll_gesture_ = false;
...
WebInputEventResult ScrollManager::HandleGestureScrollUpdate(
  ...
  did_scroll_x_for_whole_scroll_gesture_ |= did_scroll_x;
  did_scroll_y_for_whole_scroll_gesture_ |= did_scroll_y;
...
void ScrollManager::SnapAtGestureScrollEnd() {
  ...
  if (!snap_coordinator || !layout_box || !layout_box->GetScrollableArea() ||
      (!did_scroll_x_for_whole_scroll_gesture_ &&
       !did_scroll_y_for_whole_scroll_gesture_))
    return;

  SnapAxis scrolled_axis = SnapAxis::kBoth;
  if (!did_scroll_y_for_whole_scroll_gesture_)
    scrolled_axis = SnapAxis::kX;
  if (!did_scroll_x_for_whole_scroll_gesture_)
    scrolled_axis = SnapAxis::kY;
  ScrollOffset snap_offset =
      snap_coordinator->GetSnapPosition(*layout_box, scrolled_axis);

  // TODO(sunyunjia): We need to actually animate this in future.
