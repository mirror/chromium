...
WebInputEventResult GestureManager::HandleGestureEventInFrame(
  ...
  if (event_target) {
    GestureEvent* gesture_dom_event =
        GestureEvent::Create(event_target->GetDocument().domWindow(), gesture_event);
    if (gesture_dom_event) {
      ...
      if (gesture_dom_event_result != DispatchEventResult::kNotCanceled) {
        ...
        return EventHandlingUtil::ToWebInputEventResult(gesture_dom_event_result);
      }
    }
  }
...
WebInputEventResult GestureManager::HandleGestureTap(
  ...
  IntPoint tapped_position = FlooredIntPoint(gesture_event.PositionInRootFrame());
  ...
  WebMouseEvent fake_mouse_up(
      WebInputEvent::kMouseUp, gesture_event, WebPointerProperties::Button::kLeft,
      gesture_event.TapCount(),
      static_cast<WebInputEvent::Modifiers>(
          modifiers | WebInputEvent::Modifiers::kIsCompatibilityEventForTouch),
      gesture_event.TimeStampSeconds());
  ...
  if (tapped_non_text_node) {
    if (current_hit_test.InnerNode()) {
      ...
      click_event_result =
          mouse_event_manager_->SetMousePositionAndDispatchMouseEvent(
              click_target_node, String(), EventTypeNames::click, fake_mouse_up);
    }
  ...
  if (event_result == WebInputEventResult::kNotHandled && tapped_node &&
    ...
    bool dom_tree_changed =
        pre_dispatch_dom_tree_version != frame_->GetDocument()->DomTreeVersion();
    bool style_changed =
        pre_dispatch_style_version != frame_->GetDocument()->StyleVersion();

    IntPoint tapped_position_in_viewport =
        frame_->GetPage()->GetVisualViewport().RootFrameToViewport(tapped_position);
    frame_->GetChromeClient().ShowUnhandledTapUIIfNeeded(
        tapped_position_in_viewport, tapped_node, dom_tree_changed || style_changed);
  }
  return event_result;
}
