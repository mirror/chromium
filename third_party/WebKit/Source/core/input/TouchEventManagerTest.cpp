...
class CheckEventListenerCallback final : public EventListener {
 public:
  static CheckEventListenerCallback* Create(bool* event_received) {
    return new CheckEventListenerCallback(event_received);
  }
 ...
 private:
  CheckEventListenerCallback(bool* event_received)
      : EventListener(EventListener::kCPPEventListenerType),
        event_received_(event_received) {
...
TEST_F(TouchEventManagerTest, LostTouchDueToInnerIframeRemove) {
  ...
  CheckEventListenerCallback* callback =
      CheckEventListenerCallback::Create(&event_received);
