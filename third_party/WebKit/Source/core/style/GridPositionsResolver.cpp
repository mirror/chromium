...
NamedLineCollection::NamedLineCollection(
    ...
    : last_line_(last_line), auto_repeat_total_tracks_(auto_repeat_tracks_count) {
...
size_t NamedLineCollection::Find(size_t line) {
  ...
  if (line <= (insertion_point_ + auto_repeat_total_tracks_)) {
    ...
    size_t index_in_first_repetition = local_index % auto_repeat_track_list_length_;
    ...
    if (local_index == auto_repeat_total_tracks_)
      return auto_repeat_named_lines_indexes_->Find(auto_repeat_track_list_length_);
    size_t position =
  ...
  return named_lines_indexes_
             ? named_lines_indexes_->Find(line - (auto_repeat_total_tracks_ - 1))
             : kNotFound;
}
...
static void InitialAndFinalPositionsFromStyle(
  ...
  final_position = (direction == kForColumns) ? grid_item.Style()->GridColumnEnd()
...
size_t GridPositionsResolver::ExplicitGridRowCount(
  ...
  return std::min<size_t>(
      std::max(
          grid_container_style.GridTemplateRows().size() + auto_repeat_tracks_count,
...
static GridSpan ResolveNamedGridLinePositionAgainstOppositePosition(
  ...
  size_t last_line =
      ExplicitGridSizeForSide(grid_container_style, side, auto_repeat_tracks_count);
  NamedLineCollection lines_collection(
...
static GridSpan DefiniteGridSpanWithSpanAgainstOpposite(
  ...
  if (side == kColumnStartSide || side == kRowStartSide)
    return GridSpan::UntranslatedDefiniteGridSpan(opposite_line - position_offset,
                                                  opposite_line);

  return GridSpan::UntranslatedDefiniteGridSpan(opposite_line,
                                                opposite_line + position_offset);
}
...
static int ResolveGridPositionFromStyle(const ComputedStyle& grid_container_style,
                                        const GridPosition& position,
                                        GridPositionSide side,
                                        size_t auto_repeat_tracks_count) {
  switch (position.GetType()) {
    ...
    case kNamedGridAreaPosition: {
      ...
      NamedLineCollection implicit_lines(
          grid_container_style, ImplicitNamedGridLineForSide(named_grid_line, side),
...
GridSpan GridPositionsResolver::ResolveGridPositionsFromStyle(
  ...
  if (initial_position.ShouldBeResolvedAgainstOppositePosition()) {
    ...
    int end_line = ResolveGridPositionFromStyle(
        grid_container_style, final_position, final_side, auto_repeat_tracks_count);
  ...
  int start_line = ResolveGridPositionFromStyle(
      grid_container_style, initial_position, initial_side, auto_repeat_tracks_count);
  int end_line = ResolveGridPositionFromStyle(grid_container_style, final_position,
