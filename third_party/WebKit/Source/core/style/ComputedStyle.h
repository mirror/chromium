...
class CORE_EXPORT ComputedStyle : public ComputedStyleBase,
 ...
 public:
  ...
  void SetOpacity(float f) { SetOpacityInternal(clampTo<float>(f, 0, 1)); }
  ...
  bool OutlineVisuallyEqual(const ComputedStyle& other) const {
    return OutlineInternal().VisuallyEqual(other.OutlineInternal());
  }

  // outline-color
  void SetOutlineColor(const StyleColor& v) {
    if (!compareEqual(OutlineInternal().GetColor(), v))
      MutableOutlineInternal().SetColor(v);
  }

  // outline-style
  EBorderStyle OutlineStyle() const { return OutlineInternal().Style(); }
  void SetOutlineStyle(EBorderStyle v) {
    if (!compareEqual(OutlineInternal().style_, v))
      MutableOutlineInternal().style_ = static_cast<unsigned>(v);
  }
  static OutlineIsAuto InitialOutlineStyleIsAuto() { return kOutlineIsAutoOff; }
  OutlineIsAuto OutlineStyleIsAuto() const {
    return OutlineInternal().IsAuto();
  }
  void SetOutlineStyleIsAuto(OutlineIsAuto v) {
    if (!compareEqual(OutlineInternal().is_auto_, v))
      MutableOutlineInternal().is_auto_ = v;
  }

  // outline-width
  static unsigned short InitialOutlineWidth() { return 3; }
  unsigned short OutlineWidth() const {
    if (OutlineInternal().Style() == EBorderStyle::kNone)
      return 0;
    return OutlineInternal().Width();
  }
  void SetOutlineWidth(unsigned short v) {
    if (!OutlineInternal().WidthEquals(v))
      MutableOutlineInternal().SetWidth(v);
  }
  ...
  int OutlineOffset() const {
    if (OutlineInternal().Style() == EBorderStyle::kNone)
      return 0;
    return OutlineInternal().Offset();
  }
  void SetOutlineOffset(int v) {
    if (!compareEqual(OutlineInternal().offset_, v))
      MutableOutlineInternal().offset_ = v;
  }
  ...
  void SetShapeImageThreshold(float v) {
    SetShapeImageThresholdInternal(clampTo<float>(v, 0, 1));
  }
 ...
 private:
  ...
  StyleColor OutlineColor() const { return OutlineInternal().GetColor(); }
