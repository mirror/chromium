...
StyleRecalcChange ComputedStyle::StylePropagationDiff(
  ...
  bool non_independent_equal = old_style->NonIndependentInheritedEqual(*new_style);
...
ContentPosition ComputedStyle::ResolvedAlignContentPosition(
    const StyleContentAlignmentData& normal_value_behavior) const {
  return ResolvedContentAlignmentPosition(AlignContent(), normal_value_behavior);
}
...
bool ComputedStyle::ScrollAnchorDisablingPropertyChanged(
  ...
  if (surround_.Get() != other.surround_.Get()) {
    if (surround_->margin_ != other.surround_->margin_ ||
        !OffsetEqual(other) ||
...
void ComputedStyle::UpdateIsStackingContext(bool is_document_element,
  ...
  if (is_document_element || is_in_top_layer || StyleType() == kPseudoIdBackdrop ||
...
void ComputedStyle::ApplyMotionPathTransform(
  ...
  if (position.X() != Length(kAuto) || anchor.X() != Length(kAuto)) {
    ...
    origin_shift_x = FloatValueForLength(anchor.X(), bounding_box.Width()) -
...
FloatRoundedRect ComputedStyle::GetRoundedInnerBorderFor(
  ...
  int left_width =
      (!horizontal || include_logical_left_edge) ? roundf(BorderLeftWidth()) : 0;
  int right_width =
      (!horizontal || include_logical_right_edge) ? roundf(BorderRightWidth()) : 0;
  int top_width =
      (horizontal || include_logical_left_edge) ? roundf(BorderTopWidth()) : 0;
  int bottom_width =
      (horizontal || include_logical_right_edge) ? roundf(BorderBottomWidth()) : 0;
...
const AtomicString& ComputedStyle::TextEmphasisMarkString() const {
  switch (GetTextEmphasisMark()) {
    ...
    case kTextEmphasisMarkDot: {
      DEFINE_STATIC_LOCAL(AtomicString, filled_dot_string, (&kBulletCharacter, 1));
      DEFINE_STATIC_LOCAL(AtomicString, open_dot_string,
                          (&kWhiteBulletCharacter, 1));
      return GetTextEmphasisFill() == kTextEmphasisFillFilled ? filled_dot_string
                                                             : open_dot_string;
    }
...
Length ComputedStyle::LineHeight() const {
  ...
  if (lh.IsFixed()) {
    ...
    return Length(
        TextAutosizer::ComputeAutosizedFontSize(lh.Value(), multiplier), kFixed);
  }
...
void ComputedStyle::ApplyTextDecorations(const Color& parent_text_decoration_color,
  ...
  if (has_simple_underline_ &&
    ...
    AddAppliedTextDecoration(AppliedTextDecoration(kTextDecorationUnderline,
                                                   kTextDecorationStyleSolid,
                                                   parent_text_decoration_color));
  }
  if (override_existing_colors && rare_inherited_data_->applied_text_decorations)
...
Color ComputedStyle::ColorIncludingFallback(int color_property,
  ...
  switch (color_property) {
    ...
    case CSSPropertyBorderRightColor:
      result = visited_link ? VisitedLinkBorderRightColor() : BorderRightColor();
  ...
  if (!visited_link &&
      (border_style == kBorderStyleInset || border_style == kBorderStyleOutset ||
