...
static float hypotenuse_length(float x, float y) {
  return sqrt(x * x + y * y);
}

static float ComputeRayRadius(const FloatRect& parent_absolute_bounding_box,
  ...
  switch (size) {
    ...
    case StyleRay::RaySize::kFarthestCorner: {
      float dist_top_left_corner = hypotenuse_length(dist_left, dist_top);
      float dist_top_right_corner = hypotenuse_length(dist_right, dist_top);
      float dist_bottom_left_corner = hypotenuse_length(dist_left, dist_bottom);
      float dist_bottom_right_corner =
          hypotenuse_length(dist_right, dist_bottom);
