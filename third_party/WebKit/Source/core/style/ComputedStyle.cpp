...
void ComputedStyle::ApplyMotionPathTransform(
  ...
  if (LayoutBlock* container =
          layout_object ? layout_object->ContainingBlock(nullptr) : nullptr) {
    parent_absolute_bounding_box = container->AbsoluteBoundingBoxFloatRect();
    offset_from_container = layout_object->OffsetFromContainer(container);
  }
