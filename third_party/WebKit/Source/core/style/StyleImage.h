...
class CORE_EXPORT StyleImage : public GarbageCollectedFinalized<StyleImage> {
 public:
  ...
  // snapping. This size is only currently computed for BoxPainterBase and
  // NinePieceImagePainter as these are the only painters which use custom
  // paint. We pass a nullptr for other subclasses.
  ...
  virtual RefPtr<Image> GetImage(const ImageResourceObserver&,
                                 ...
                                 LayoutSize* logical_tile_size) const = 0;
