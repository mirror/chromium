...
void PreemptionManager::EnterPreemptionCheckpointScope() {
  ...
  if (in_preemption_opt_in_ && WTF::CurrentTimeTicks() > next_)
    RunNestedLoop();
}
...
base::WeakPtr<PreemptionManager> PreemptionManager::GetWeakPtr() {
...
void PreemptionManager::RunNestedLoop() {
  ...
  run_loop.Run();
  next_ = WTF::CurrentTimeTicks() + kNestedLoopMinimumInterval;
}
