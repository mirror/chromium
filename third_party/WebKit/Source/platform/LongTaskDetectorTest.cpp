...
TEST_F(LongTaskDetectorTest, DeliversLongTaskNotificationOnlyWhenRegistered) {
  ...
  EXPECT_EQ(TimeTicksInSeconds(long_task_observer->last_long_task_end), 0.0);

  LongTaskDetector::Instance().RegisterObserver(long_task_observer);
  SimulateTask(LongTaskDetector::kLongTaskThresholdSeconds + 0.01);
  TimeTicks long_task_end_when_registered = DummyTaskEndTime();
  EXPECT_EQ(TimeTicksInSeconds(long_task_observer->last_long_task_start),
            TimeTicksInSeconds(DummyTaskStartTime()));
  EXPECT_EQ(TimeTicksInSeconds(long_task_observer->last_long_task_end),
            TimeTicksInSeconds(long_task_end_when_registered));
...
TEST_F(LongTaskDetectorTest, DoesNotGetNotifiedOfShortTasks) {
  ...
  SimulateTask(LongTaskDetector::kLongTaskThresholdSeconds + 0.01);
  EXPECT_EQ(TimeTicksInSeconds(long_task_observer->last_long_task_end),
            TimeTicksInSeconds(DummyTaskEndTime()));
}
}  // namespace blink
