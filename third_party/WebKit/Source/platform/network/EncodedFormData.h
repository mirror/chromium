...
// Refcounted wrapper around a data pipe getter to allow sharing it (but
// ultimately only one reference holder can take the underlying object).
class PLATFORM_EXPORT WrappedDataPipeGetter final
 ...
 public:
  ...
  network::mojom::blink::DataPipeGetterPtr ReleaseDataPipe() {
...
class PLATFORM_EXPORT FormDataElement final {
 ...
 public:
  ...
  // The wrapped data pipe getter allow copies of FormDataElement but once
  // anyone takes the underlying data pipe getter no one else can access it.
