...
std::unique_ptr<TracedValue> EndResourceInfo(Resource* resource,
  ...
  if (resource) {
    data->SetString(
        "resourceType",
        (char*)Resource::ResourceTypeToString(
    ...
    data->SetInteger("request.GetFrameType", request.GetFrameType());
  ...
  if (info) {
    ...
    if (timing) {
      // data->SetDouble("timing.pushStart", timing->PushStart()); // visiting
      // released memory data->SetDouble("timing.pushEnd", timing->PushEnd());
    }
  }
...
ScopedResourceLoadTracker::~ScopedResourceLoadTracker() {
  if (!resource_load_continues_beyond_scope_) {
    EndResourceLoad(resource_id_, (char*)"NA-failed request", nullptr, nullptr,
                    RequestOutcome::kFail);
  }
}
