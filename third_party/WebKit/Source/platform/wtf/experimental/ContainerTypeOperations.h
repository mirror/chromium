...
// define customized operations for optimizations such as using memcpy.
//
// We have good enough defaults for common data types such as fundamental
// types or RefPtr<T>, so in most cases you don't have to care about those
// functions.
//
// ===================================
// ContainerTypeOperations requirement
// ===================================
//
...
//     static void DestructRange(T* uninitialized_begin,
//                               T* uninitialized_end);
//
...
//                                      T* storage_begin);
//
//     Minimal complete definition: Assign() (both versions).
//
...
//     copy).
//
// 5. Peeking and taking
//
//     using PeekType = ...;
//     static <any> Peek(const T& storage);
//
//     static T Take(T& storage);
//
...
struct GenericContainerTypeOperations {
  ...
  // This Peek() always returns a reference to the stored object, so it's safe
  // to receive the result of Peek() as a const reference.
  using PeekType = const T&;
  static const T& Peek(const T& storage);
...
struct DestructSupplement<TypeOperations, T, true, false> {
  static void DestructRange(T* uninitialized_begin, T* uninitialized_end) {
    for (T* uninitialized = uninitialized_begin;
         uninitialized != uninitialized_end; ++uninitialized) {
      TypeOperations::Destruct(*uninitialized);
    }
  }
};
...
struct DestructSupplement<TypeOperations, T, false, true> {
  static void Destruct(T& uninitialized) {
    TypeOperations::DestructRange(&uninitialized, &uninitialized + 1);
  }
};
...
struct CopyOverlappingRangeSupplement<TypeOperations, T, false> {
  // This mini type traits is used to choose the correct version of
  // CopyRange() when forward copying is OK.
  struct SupplementedTypeOperations : TypeOperations,
                                      CopyRangeSupplement<TypeOperations, T> {};

  static void CopyOverlappingRange(const T* value_begin,
                                   const T* value_end,
                                   T* storage_begin) {
    if (value_begin > storage_begin) {
      // Copy forward.
      //
    ...
    } else if (value_begin < storage_begin) {
      // Copy backward.
      ...
      do {
        --value;
        --storage;
        SupplementedTypeOperations::Assign(*storage, *value);
  ...
  static void CopyOverlappingRange(const InT* value_begin,
                                   const InT* value_end,
                                   T* storage_begin) {
    // If InT and T are different, we can assume those ranges do not overlap.
    SupplementedTypeOperations::CopyRange(value_begin, value_end,
                                          storage_begin);
  }
};
...
struct MoveOverlappingRangeSupplement<TypeOperations, T, false> {
  // Similar to CopyOverlappingRangeSupplement.
  struct SupplementedTypeOperations : TypeOperations,
                                      MoveRangeSupplement<TypeOperations, T> {};
  ...
  static void MoveOverlappingRange(T* value_begin,
    ...
    if (value_begin > storage_begin) {
      ...
      do {
        --value;
        --storage;
        SupplementedTypeOperations::Assign(*storage, std::move(*value));
      } while (value != value_begin);
    }
  }
  ...
  static void MoveOverlappingRange(InT* value_begin,
                                   ...
                                   T* storage_begin) {
    // If InT and T are different, we can assume those ranges do not overlap.
    SupplementedTypeOperations::MoveRange(value_begin, value_end,
                                          storage_begin);
  }
};
...
template <typename T>
const T& GenericContainerTypeOperations<T>::Peek(const T& storage) {
  return storage;
}

template <typename T>
T GenericContainerTypeOperations<T>::Take(T& storage) {
