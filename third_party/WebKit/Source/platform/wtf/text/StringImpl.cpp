...
unsigned StringImpl::CopyTo(UChar* buffer,
  ...
  unsigned number_of_characters_to_copy = std::min(length() - start, max_length);
...
PassRefPtr<StringImpl> StringImpl::Lower(const AtomicString& locale_identifier) {
  ...
  RefPtr<StringImpl> upconverted = UpconvertedString();
  const UChar* source16 = upconverted->Characters16();
  return CaseConvert(source16, length, u_strToLower, locale_for_conversion, this);
}

PassRefPtr<StringImpl> StringImpl::Upper(const AtomicString& locale_identifier) {
  // Use the more-optimized code path most of the time.
  ...
  return CaseConvert(source16, length, u_strToUpper, locale_for_conversion, this);
}
...
size_t StringImpl::Find(const StringView& match_string, unsigned index) {
  ...
  if (match_string.Is8Bit())
    return FindInternal(Characters16() + index, match_string.Characters8(),
                        index, search_length, match_length);
  return FindInternal(Characters16() + index, match_string.Characters16(), index,
...
ALWAYS_INLINE static size_t FindIgnoringCaseInternal(
  ...
  // keep looping until we match
  while (
      !EqualIgnoringCase(search_characters + i, match_characters, match_length)) {
    if (i == delta)
      return kNotFound;
    ++i;
  }
...
size_t StringImpl::ReverseFind(const StringView& match_string, unsigned index) {
  ...
  if (Is8Bit()) {
    ...
    return ReverseFindInternal(Characters8(), match_string.Characters16(), index,
                               our_length, match_length);
  }
  if (match_string.Is8Bit())
    return ReverseFindInternal(Characters16(), match_string.Characters8(), index,
                               our_length, match_length);
...
bool StringImpl::EndsWithIgnoringCase(const StringView& suffix) const {
  ...
  if (Is8Bit()) {
    ...
    return EqualIgnoringCase(Characters8() + start_offset, suffix.Characters16(),
                             suffix.length());
  }
  if (suffix.Is8Bit())
    return EqualIgnoringCase(Characters16() + start_offset, suffix.Characters8(),
                             suffix.length());
...
PassRefPtr<StringImpl> StringImpl::Replace(unsigned position,
  ...
  if (Is8Bit() && (string.IsNull() || string.Is8Bit())) {
    ...
    RefPtr<StringImpl> new_impl =
        CreateUninitialized(length() - length_to_replace + length_to_insert, data);
