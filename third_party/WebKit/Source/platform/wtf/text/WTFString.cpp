...
String::String(const char* characters, unsigned length)
    : impl_(characters ? StringImpl::Create(
...
PassRefPtr<StringImpl> InsertInternal(PassRefPtr<StringImpl> impl,
  ...
  StringImpl::CopyChars(data + position, characters_to_insert, length_to_insert);
...
void String::Ensure16Bit() {
  ...
  if (unsigned length = this->length())
    impl_ =
        Make16BitFrom8BitSource(impl_->Characters8(), length).ReleaseImpl();
  else
  ...
  if (Is8Bit()) {
    ...
    if (mode == kStrictUTF8ConversionReplacingUnpairedSurrogatesWithFFFD) {
      ...
      while (characters < characters_end) {
        ...
        ConversionResult result = ConvertUTF16ToUTF8(&characters, characters_end,
                                                     &buffer, buffer_end, true);
        DCHECK_NE(result, kTargetExhausted);
