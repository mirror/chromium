...
struct StringHash {
  ...
  static unsigned GetHash(const RefPtr<StringImpl>& key) { return key->GetHash(); }
...
class CaseFoldingHash {
 ...
 public:
  ...
  static inline unsigned GetHash(const char* data, unsigned length) {
    return CaseFoldingHash::GetHash(reinterpret_cast<const LChar*>(data), length);
  }
  ...
  static unsigned GetHash(const AtomicString& key) { return GetHash(key.Impl()); }
  static bool Equal(const String& a, const String& b) {
    return Equal(a.Impl(), b.Impl());
  }
