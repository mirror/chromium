...
TEST(TextEncoding, ClosestByteBased) {
  ...
  EXPECT_STREQ("GBK", TextEncoding("gbk").ClosestByteBasedEquivalent().GetName());
  EXPECT_EQ(nullptr, TextEncoding("utf-7").ClosestByteBasedEquivalent().GetName());
}
...
TEST(TextEncoding, EncodingForFormSubmission) {
  ...
  EXPECT_STREQ("GBK", TextEncoding("gbk").EncodingForFormSubmission().GetName());
  EXPECT_STREQ("UTF-8",
