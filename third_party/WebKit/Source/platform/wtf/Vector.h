...
struct VectorMover<false, T, Allocator> {
  ...
  static void Swap(T* src, T* src_end, T* dst) {
    ...
    ConstructTraits<T, Allocator>::ConstructElements(src, len);
    ConstructTraits<T, Allocator>::ConstructElements(dst, len);
  }
};
...
struct VectorMover<true, T, Allocator> {
  ...
  static void Move(const T* src, const T* src_end, T* dst) {
    if (LIKELY(dst && src)) {
      ...
      ConstructTraits<T, Allocator>::ConstructElements(dst, src_end - src);
    }
  }
...
class VectorBuffer<T, 0, Allocator>
 ...
 public:
  ...
  void SwapVectorBuffer(VectorBuffer<T, 0, Allocator>& other,
    ...
    if (buffer_) {
      ConstructTraits<T, Allocator>::ConstructElements(buffer_, size_);
    }
    if (other.buffer_) {
      ConstructTraits<T, Allocator>::ConstructElements(other.buffer_,
                                                       other.size_);
    }
  }
