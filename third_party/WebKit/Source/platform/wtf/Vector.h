...
struct VectorMover<false, T, Allocator> {
  ...
  static void Move(T* src, T* src_end, T* dst) {
    while (src != src_end) {
      ConstructTraits<T, Allocator>::ConstructElement(dst, std::move(*src));
      src->~T();
      ++dst;
      ++src;
    }
  ...
  static void MoveOverlapping(T* src, T* src_end, T* dst) {
    if (src > dst) {
      ...
      while (src != src_end) {
        --src_end;
        --dst_end;
        ConstructTraits<T, Allocator>::ConstructElement(dst_end,
                                                        std::move(*src_end));
        src_end->~T();
      }
    }
  ...
  static void Swap(T* src, T* src_end, T* dst) {
    ...
    ConstructTraits<T, Allocator>::NewInitializedElements(src, len);
    ConstructTraits<T, Allocator>::NewInitializedElements(dst, len);
  }
};
...
struct VectorMover<true, T, Allocator> {
  ...
  static void Move(const T* src, const T* src_end, T* dst) {
    if (LIKELY(dst && src)) {
      ...
      ConstructTraits<T, Allocator>::NewInitializedElements(dst, src_end - src);
    }
  }
...
struct VectorCopier<false, T, Allocator> {
  ...
  static void UninitializedCopy(const U* src, const U* src_end, T* dst) {
    while (src != src_end) {
      ConstructTraits<T, Allocator>::ConstructElement(dst, *src);
      ++dst;
      ++src;
    }
  }
...
struct VectorFiller<false, T, Allocator> {
  ...
  static void UninitializedFill(T* dst, T* dst_end, const T& val) {
    while (dst != dst_end) {
      ConstructTraits<T, Allocator>::ConstructElement(dst, T(val));
      ++dst;
    }
  }
};
...
class VectorBuffer<T, 0, Allocator>
 ...
 public:
  ...
  void SwapVectorBuffer(VectorBuffer<T, 0, Allocator>& other,
    ...
    if (buffer_) {
      ConstructTraits<T, Allocator>::NewInitializedElements(buffer_, size_);
    }
    if (other.buffer_) {
      ConstructTraits<T, Allocator>::NewInitializedElements(other.buffer_,
                                                            other.size_);
    }
  }
...
ALWAYS_INLINE void Vector<T, inlineCapacity, Allocator>::push_back(U&& val) {
  ...
  if (LIKELY(size() != capacity())) {
    ...
    ConstructTraits<T, Allocator>::ConstructElement(end(),
                                                    std::forward<U>(val));
    ++size_;
    return;
  }
...
ALWAYS_INLINE T& Vector<T, inlineCapacity, Allocator>::emplace_back(
  ...
  ANNOTATE_CHANGE_SIZE(begin(), capacity(), size_, size_ + 1);
  T* t = ConstructTraits<T, Allocator>::ConstructElement(
...
NEVER_INLINE void Vector<T, inlineCapacity, Allocator>::AppendSlowCase(
  ...
  ANNOTATE_CHANGE_SIZE(begin(), capacity(), size_, size_ + 1);
  ConstructTraits<T, Allocator>::ConstructElement(end(), std::forward<U>(*ptr));
  ++size_;
}
...
ALWAYS_INLINE void Vector<T, inlineCapacity, Allocator>::UncheckedAppend(
  ...
  ConstructTraits<T, Allocator>::ConstructElement(end(), std::forward<U>(val));
  ++size_;
#endif
}
...
inline void Vector<T, inlineCapacity, Allocator>::insert(size_t position,
  ...
  ConstructTraits<T, Allocator>::ConstructElement(spot, std::forward<U>(*data));
  ++size_;
}
