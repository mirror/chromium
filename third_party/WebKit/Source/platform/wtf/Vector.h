...
struct VectorMover<false, T, Allocator> {
  ...
  static void Swap(T* src, T* src_end, T* dst) {
    ...
    ConstructTraits<T, Allocator>::NewInitializedElements(src, src + len);
    ConstructTraits<T, Allocator>::NewInitializedElements(dst, dst + len);
  }
};
...
struct VectorMover<true, T, Allocator> {
  ...
  static void Move(const T* src, const T* src_end, T* dst) {
    if (LIKELY(dst && src)) {
      ...
      ConstructTraits<T, Allocator>::NewInitializedElements(
          dst, dst + (src_end - src));
    }
  }
...
class VectorBuffer<T, 0, Allocator>
 ...
 public:
  ...
  void SwapVectorBuffer(VectorBuffer<T, 0, Allocator>& other,
    ...
    if (buffer_) {
      ConstructTraits<T, Allocator>::NewInitializedElements(buffer_,
                                                            buffer_ + size_);
    }
    if (other.buffer_) {
      ConstructTraits<T, Allocator>::NewInitializedElements(
