...
int ToSample(ReportCircumstance circumstance) {
...
void ResourceLoadScheduler::TrafficMonitor::Report(
  ...
  DEFINE_STATIC_LOCAL(EnumerationHistogram, request_count_by_circumstance,
                      ("Blink.ResourceLoadScheduler.RequestCountByCircumstance",
...
ResourceLoadScheduler::TrafficReportHints::InvalidInstance() {
  DEFINE_STATIC_LOCAL(TrafficReportHints, instance, ());
  return instance;
}
...
void ResourceLoadScheduler::Shutdown() {
  ...
  // Flush out all reports here to avoid running dangerous code in GC phase.
...
bool ResourceLoadScheduler::Release(
  ...
  if (running_requests_.find(id) != running_requests_.end()) {
    ...
    // |traffic_monitor_| may be destructed by shutdown.
    if (traffic_monitor_)
