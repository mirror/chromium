...
void ResourceLoadScheduler::Shutdown() {
  ...
  if (scheduler) {
    // Assuming that When SetOutstandingLimitForTesting() is called,
    // GetScheduler() returns nullptr or the runtime feature flag is enabled.
    // Otherwise, RemoveThrottlingObserver() call below will hit an assertion.
