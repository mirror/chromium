...
TEST(ResourceLoaderOptionsTest, DeepCopy) {
  ...
  std::unique_ptr<ResourceLoaderOptions> original =
  ...
  std::unique_ptr<ResourceLoaderOptions> copy =
      copy_data.GetResourceLoaderOptions();

  // Check that contents are correctly copied to |copy_data|
  ...
  // Check that pointers are different between |original| and |copy_data|
  ...
  EXPECT_EQ(original->initiator_info.start_time,
            copy->initiator_info.start_time);
  EXPECT_EQ(original->request_initiator_context,
            copy->request_initiator_context);
