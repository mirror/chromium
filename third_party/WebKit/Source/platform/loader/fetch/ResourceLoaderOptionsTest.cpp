...
TEST(ResourceLoaderOptionsTest, DeepCopy) {
  ...
  EXPECT_EQ(original.initiator_info.position, copy_data.initiator_info.position);
  EXPECT_EQ(original.initiator_info.start_time, copy_data.initiator_info.start_time);
  EXPECT_EQ(original.request_initiator_context, copy_data.request_initiator_context);
  ...
  EXPECT_EQ(original.security_origin->Host(), copy_data.security_origin->Host());
