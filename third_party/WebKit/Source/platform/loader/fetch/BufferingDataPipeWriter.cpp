...
bool BufferingDataPipeWriter::Write(const char* buffer, uint32_t num_bytes) {
  ...
  buffer_.push_back(Vector<char, false>());
...
void BufferingDataPipeWriter::OnWritable(MojoResult) {
  ...
  waiting_ = false;
  while (!buffer_.empty()) {
    WTF::Vector<char, false>& front = buffer_.front();

    uint32_t size = front.size() - front_index_;

    MojoResult result = mojo::WriteDataRaw(
        handle_.get(), front.data() + front_index_, &size, kNone);
    if (result == MOJO_RESULT_SHOULD_WAIT) {
      waiting_ = true;
      watcher_.ArmOrNotify();
      return;
