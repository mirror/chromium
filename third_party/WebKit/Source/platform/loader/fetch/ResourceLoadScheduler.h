...
class PLATFORM_EXPORT ResourceLoadScheduler final
 ...
 public:
  ...
  // ResourceLoadScheduler has two policies: it may start with |kTight| policy,
  // in order to throttle loading more agressively while page layout is blocked
  // for example. There is only one state transition, |kTight| => |kNormal|,
  // which is done by LoosenThrottlingPolicy.
