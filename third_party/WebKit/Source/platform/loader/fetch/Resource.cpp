...
class Resource::CachedMetadataHandlerImpl : public CachedMetadataHandler {
 ...
 protected:
  ...
  const ResourceResponse& GetResponse() const { return resource_->GetResponse(); }
...
PassRefPtr<CachedMetadata> Resource::CachedMetadataHandlerImpl::GetCachedMetadata(
...
void Resource::CachedMetadataHandlerImpl::SendToPlatform() {
  if (cached_metadata_) {
    ...
    Platform::Current()->CacheMetadata(
        GetResponse().Url(), GetResponse().ResponseTime(), serialized_data.Data(),
        serialized_data.size());
  } else {
    Platform::Current()->CacheMetadata(GetResponse().Url(),
                                       GetResponse().ResponseTime(), nullptr, 0);
  }
}
...
void Resource::SetDataBufferingPolicy(DataBufferingPolicy data_buffering_policy) {
...
static double CurrentAge(const ResourceResponse& response,
  ...
  double corrected_received_age =
      std::isfinite(age_value) ? std::max(apparent_age, age_value) : apparent_age;
...
void Resource::AddClient(ResourceClient* client,
  ...
  if ((ErrorOccurred() || !GetResponse().IsNull()) &&
      !TypeNeedsSynchronousCacheHit(GetType()) && !needs_synchronous_cache_hit_) {
...
void Resource::SetEncodedSize(size_t encoded_size) {
  if (encoded_size == encoded_size_ && encoded_size == encoded_size_memory_usage_)
    return;
  size_t old_size = size();
