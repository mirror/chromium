...
TEST_F(ResourceLoadSchedulerTest, Bypass) {
  ...
  EXPECT_TRUE(scheduler()->Release(
      id1, ResourceLoadScheduler::ReleaseOption::kReleaseOnly, -1));
  EXPECT_TRUE(scheduler()->Release(
      id2, ResourceLoadScheduler::ReleaseOption::kReleaseAndSchedule, -1));
  ...
  EXPECT_FALSE(scheduler()->Release(
      ...
      ResourceLoadScheduler::ReleaseOption::kReleaseOnly, -1));
...
TEST_F(ResourceLoadSchedulerTest, Throttled) {
  ...
  EXPECT_TRUE(scheduler()->Release(
      id1, ResourceLoadScheduler::ReleaseOption::kReleaseAndSchedule, -1));
  EXPECT_TRUE(client2->WasRan());

  // Call Release() with kReleaseOnly should not run the third and the fourth
  // requests.
  EXPECT_TRUE(scheduler()->Release(
      id2, ResourceLoadScheduler::ReleaseOption::kReleaseOnly, -1));
  EXPECT_FALSE(client3->WasRan());
  EXPECT_FALSE(client4->WasRan());
  ...
  EXPECT_TRUE(scheduler()->Release(
      id3, ResourceLoadScheduler::ReleaseOption::kReleaseAndSchedule, -1));
...
TEST_F(ResourceLoadSchedulerTest, Unthrottle) {
  ...
  // Release all.
  EXPECT_TRUE(scheduler()->Release(
      id3, ResourceLoadScheduler::ReleaseOption::kReleaseOnly, -1));
  EXPECT_TRUE(scheduler()->Release(
      id2, ResourceLoadScheduler::ReleaseOption::kReleaseOnly, -1));
  EXPECT_TRUE(scheduler()->Release(
      id1, ResourceLoadScheduler::ReleaseOption::kReleaseOnly, -1));
