...
void ResourceFetcher::SetSourceOriginToResource(
    Resource* resource,
    scoped_refptr<const SecurityOrigin> source_origin_override) const {
  if (source_origin_override)
    resource->SetSourceOrigin(source_origin_override);
  else
    resource->SetSourceOrigin(Context().GetSecurityOrigin());
}
...
void ResourceFetcher::AddToMemoryCacheIfNeeded(const FetchParameters& params,
  ...
  SetSourceOriginToResource(resource, params.Options().security_origin);
