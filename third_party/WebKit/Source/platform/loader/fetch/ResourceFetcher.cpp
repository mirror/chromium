...
void ResourceFetcher::DidLoadResourceFromMemoryCache(
  ...
  resource_request.SetRequestContext(original_resource_request.GetRequestContext());
...
Resource* ResourceFetcher::ResourceForBlockedRequest(
  ...
  Resource* resource = factory.Create(request.GetResourceRequest(),
                                      request.Options(), request.Charset());
  resource->GetError(ResourceError::CancelledDueToAccessCheckError(request.Url(),
                                                                blocked_reason));
  return resource;
}
...
ResourceFetcher::PrepareRequestResult ResourceFetcher::PrepareRequest(
  ...
  resource_request.SetPriority(ComputeLoadPriority(
      factory.GetType(), request.GetResourceRequest(), ResourcePriority::kNotVisible,
      request.Defer(), request.IsSpeculativePreload(),
      request.IsLinkPreload()));
  InitializeResourceRequest(resource_request, factory.GetType(), request.Defer());
...
Resource* ResourceFetcher::RequestResource(
  ...
  ResourceRequest& resource_request = request.MutableResourceRequest();
  network_instrumentation::ScopedResourceLoadTracker scoped_resource_load_tracker(
      identifier, resource_request);
  ...
  PrepareRequestResult result = PrepareRequest(request, factory, substitute_data,
  ...
  if (!request.IsSpeculativePreload()) {
    ...
    Context().RecordLoadingActivity(identifier, resource_request, factory.GetType(),
...
void ResourceFetcher::InitializeResourceRequest(
  ...
  Context().AddAdditionalRequestHeaders(
      request,
      (type == Resource::kMainResource) ? kFetchMainResource : kFetchSubresource);
}
...
void ResourceFetcher::HandleLoaderFinish(Resource* resource,
  ...
  Context().DispatchDidFinishLoading(resource->Identifier(), finish_time,
                                     encoded_data_length,
                                     resource->GetResponse().DecodedBodyLength());
...
void ResourceFetcher::LogPreloadStats(ClearPreloadsPolicy policy) {
  ...
  for (const auto& resource : *preloads_) {
    ...
    if (resource->IsLinkPreload() && policy == kClearSpeculativeMarkupPreloads) {
      continue;
    }
    int miss_count =
