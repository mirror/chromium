...
static bool IsReusableRegardingCORS(const ResourceRequest& request,
  ...
  if (existing_resource->GetResponse().ServiceWorkerResponseType() !=
...
Resource* ResourceFetcher::MatchPreload(const FetchParameters& params,
  ...
  if (!CheckImageLoadSettings(resource) || IsDownloadOrStreamRequest(request) ||
...
bool ResourceFetcher::CheckImageLoadSettings(
...
ResourceFetcher::DetermineRevalidationPolicy(
  ...
  if (!CheckImageLoadSettings(existing_resource) ||
      IsDownloadOrStreamRequest(request) ||
      !IsReusableRegardingCORS(request, existing_resource))
    return kReload;
