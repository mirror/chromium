...
class PLATFORM_EXPORT ClientHintsPreferences {
 ...
 public:
  ...
  // Parses the client hints headers and updates the client hint preferences
  // that should be persisted for duration |persist_duration_seconds|.
  // |persist_duration_seconds| should be non-null.
  ...
  // persisted. UpdatePersistentHintsFromHeaders may be called for all responses
  // received (including subresources).
  void UpdatePersistentHintsFromHeaders(const ResourceResponse&,
                                        Context*,
