...
class FakeWebTaskRunner : public WebTaskRunner {
 public:
  ...
  void SetTime(double new_time) {
    SetTime(base::TimeTicks() + TimeDelta::FromSecondsD(new_time));
  }
  ...
  using QueueItem = std::pair<base::OnceClosure, base::TimeTicks>;
  std::deque<QueueItem> TakePendingTasksForTesting();
