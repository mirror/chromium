...
// of category. Hence, we need distinct version for each category to prevent
// unintended leak of state.
...
class StateTracer {
 public:
  StateTracer(const char* name, const void* object)
    ...
    static_assert(
        category == kCategoryNameDefault ||
        category == kCategoryNameInfo ||
        category == kCategoryNameDebug,
        "StateTracer template parameter must be a kCategoryNameXyz constant.");
  }
