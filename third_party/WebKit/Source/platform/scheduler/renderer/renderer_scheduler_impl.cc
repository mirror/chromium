...
scoped_refptr<MainThreadTaskQueue> RendererSchedulerImpl::NewLoadingTaskQueue(
  ...
  scoped_refptr<MainThreadTaskQueue> loading_task_queue(helper_.NewTaskQueue(
      queue_type, MainThreadTaskQueue::GetSpec(queue_type)
                      .SetShouldMonitorQuiescence(true)
...
void RendererSchedulerImpl::OnTaskCompleted(MainThreadTaskQueue* queue,
  ...
  RecordTaskMetrics(queue->GetQueueType(), start, end);
}
...
void RendererSchedulerImpl::EnableVirtualTime() {
  ...
  virtual_time_control_task_queue_ = helper_.NewTaskQueue(
      ...
      MainThreadTaskQueue::GetSpec(MainThreadTaskQueue::QueueType::CONTROL));
