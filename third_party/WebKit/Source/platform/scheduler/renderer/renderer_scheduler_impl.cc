...
RendererSchedulerImpl::RendererSchedulerImpl(
  ...
  task_runners_.insert(std::make_pair(helper_.DefaultTaskQueue(), nullptr));
...
RendererSchedulerImpl::~RendererSchedulerImpl() {
  ...
  for (auto& pair : task_runners_) {
    if (pair.first->GetSpec().queue_class == TaskQueue::QueueClass::LOADING) {
      pair.first->RemoveTaskObserver(
          &GetMainThreadOnly().loading_task_cost_estimator);
    }
    if (pair.first->GetSpec().queue_class == TaskQueue::QueueClass::TIMER) {
      pair.first->RemoveTaskObserver(
...
scoped_refptr<TaskQueue> RendererSchedulerImpl::NewTaskQueue(
    TaskQueue::QueueCreationParams params) {
  helper_.CheckOnValidThread();
  scoped_refptr<TaskQueue> task_queue(helper_.NewTaskQueue(params));
  ...
  auto queue_class = task_queue->GetSpec().queue_class;
  if (queue_class == TaskQueue::QueueClass::TIMER) {
    task_queue->AddTaskObserver(&GetMainThreadOnly().timer_task_cost_estimator);
  } else if (queue_class == TaskQueue::QueueClass::LOADING) {
  ...
  if (task_queue->GetSpec().can_be_throttled)
...
scoped_refptr<TaskQueue> RendererSchedulerImpl::NewLoadingTaskQueue(
  ...
  DCHECK_EQ(TaskQueue::QueueClassForQueueType(queue_type),
            TaskQueue::QueueClass::LOADING);
  return NewTaskQueue(TaskQueue::QueueCreationParams(queue_type)
...
scoped_refptr<TaskQueue> RendererSchedulerImpl::NewTimerTaskQueue(
    TaskQueue::QueueType queue_type) {
  DCHECK_EQ(TaskQueue::QueueClassForQueueType(queue_type),
            TaskQueue::QueueClass::TIMER);
  auto timer_task_queue =
      NewTaskQueue(TaskQueue::QueueCreationParams(queue_type)
...
void RendererSchedulerImpl::OnUnregisterTaskQueue(
  ...
  auto find_it = task_runners_.find(task_queue);
  if (find_it != task_runners_.end()) {
    switch (task_queue->GetSpec().queue_class) {
      case TaskQueue::QueueClass::TIMER:
        task_queue->RemoveTaskObserver(
            &GetMainThreadOnly().timer_task_cost_estimator);
      case TaskQueue::QueueClass::LOADING:
        task_queue->RemoveTaskObserver(
            &GetMainThreadOnly().loading_task_cost_estimator);
      default:
        break;
    }
    task_runners_.erase(find_it);
  }
}
...
void RendererSchedulerImpl::UpdatePolicyLocked(UpdateType update_type) {
  ...
  for (const auto& pair : task_runners_) {
    TaskQueue::QueueClass queue_class = pair.first->GetSpec().queue_class;

    ApplyTaskQueuePolicy(
        pair.first.get(), pair.second.get(),
        GetMainThreadOnly()
            .current_policy.policies[static_cast<size_t>(queue_class)],
        new_policy.policies[static_cast<size_t>(queue_class)]);
  }
...
void RendererSchedulerImpl::ApplyTaskQueuePolicy(
  ...
  TaskQueue::Spec spec = task_queue->GetSpec();

  bool was_enabled = old_task_queue_policy.IsEnabled(spec);
  bool is_enabled = new_task_queue_policy.IsEnabled(spec);

  DCHECK(was_enabled || task_queue_enabled_voter);
  if (task_queue_enabled_voter && was_enabled != is_enabled) {
    task_queue_enabled_voter->SetQueueEnabled(is_enabled);
  }

  // Make sure if there's no voter that the task queue is enabled.
  DCHECK(task_queue_enabled_voter || old_task_queue_policy.IsEnabled(spec));

  TaskQueue::QueuePriority old_priority =
      old_task_queue_policy.GetPriority(spec);
  TaskQueue::QueuePriority new_priority =
      new_task_queue_policy.GetPriority(spec);

  if (old_priority != new_priority)
    task_queue->SetQueuePriority(new_priority);

  TimeDomainType old_time_domain_type =
      old_task_queue_policy.GetTimeDomainType(spec);
  TimeDomainType new_time_domain_type =
      new_task_queue_policy.GetTimeDomainType(spec);
...
void RendererSchedulerImpl::SuspendTimerQueue() {
  ...
  for (const auto& pair : task_runners_) {
    if (pair.first->GetSpec().queue_class == TaskQueue::QueueClass::TIMER &&
        pair.first->GetSpec().can_be_suspended) {
...
bool RendererSchedulerImpl::TaskQueuePolicy::IsEnabled(
    const TaskQueue::Spec& spec) const {
  if (!is_enabled)
    return false;
  if (is_suspended && spec.can_be_suspended)
    return false;
  if (is_blocked && spec.can_be_blocked)
    return false;
  return true;
}
...
TaskQueue::QueuePriority RendererSchedulerImpl::TaskQueuePolicy::GetPriority(
    const TaskQueue::Spec& spec) const {
  if (priority == TaskQueue::QueuePriority::NORMAL_PRIORITY &&
      spec.is_important)
    return TaskQueue::QueuePriority::HIGH_PRIORITY;
  return priority;
}
...
RendererSchedulerImpl::TaskQueuePolicy::GetTimeDomainType(
  ...
  if (is_throttled && spec.can_be_throttled)
...
RendererSchedulerImpl::TaskQueuePolicy&
RendererSchedulerImpl::Policy::compositor_queue_policy() {
  return policies[static_cast<size_t>(TaskQueue::QueueClass::COMPOSITOR)];
}

const RendererSchedulerImpl::TaskQueuePolicy&
RendererSchedulerImpl::Policy::compositor_queue_policy() const {
  return policies[static_cast<size_t>(TaskQueue::QueueClass::COMPOSITOR)];
}

RendererSchedulerImpl::TaskQueuePolicy&
RendererSchedulerImpl::Policy::loading_queue_policy() {
  return policies[static_cast<size_t>(TaskQueue::QueueClass::LOADING)];
}

const RendererSchedulerImpl::TaskQueuePolicy&
RendererSchedulerImpl::Policy::loading_queue_policy() const {
  return policies[static_cast<size_t>(TaskQueue::QueueClass::LOADING)];
}

RendererSchedulerImpl::TaskQueuePolicy&
RendererSchedulerImpl::Policy::timer_queue_policy() {
  return policies[static_cast<size_t>(TaskQueue::QueueClass::TIMER)];
}

const RendererSchedulerImpl::TaskQueuePolicy&
RendererSchedulerImpl::Policy::timer_queue_policy() const {
  return policies[static_cast<size_t>(TaskQueue::QueueClass::TIMER)];
}

RendererSchedulerImpl::TaskQueuePolicy&
RendererSchedulerImpl::Policy::default_queue_policy() {
  return policies[static_cast<size_t>(TaskQueue::QueueClass::NONE)];
}

const RendererSchedulerImpl::TaskQueuePolicy&
RendererSchedulerImpl::Policy::default_queue_policy() const {
  return policies[static_cast<size_t>(TaskQueue::QueueClass::NONE)];
}

const RendererSchedulerImpl::TaskQueuePolicy&
RendererSchedulerImpl::Policy::GetQueuePolicy(
    TaskQueue::QueueClass queue_class) const {
  return policies[static_cast<size_t>(queue_class)];
}
...
void RendererSchedulerImpl::EnableVirtualTime() {
  ...
  virtual_time_control_task_queue_ = helper_.NewTaskQueue(
