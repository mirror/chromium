...
void RendererSchedulerImpl::V8Initalized() {
  main_thread_only().v8_initalized = true;
  if (virtual_time_domain_)
    virtual_time_domain_->V8Initalized();
}
...
base::TimeTicks RendererSchedulerImpl::EnableVirtualTime() {
  ...
  main_thread_only().initial_virtual_time_ticks = base::TimeTicks::Now();
  ...
  if (main_thread_only().v8_initalized)
    virtual_time_domain_->V8Initalized();
...
void RendererSchedulerImpl::DisableVirtualTimeForTesting() {
  ...
  main_thread_only().metrics_helper.ResetForTest();
}
