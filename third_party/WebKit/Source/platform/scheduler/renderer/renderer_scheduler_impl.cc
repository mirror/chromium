...
RendererSchedulerImpl::~RendererSchedulerImpl() {
  ...
  for (auto& pair : timer_task_runners_) {
    pair.first->RemoveTaskObserver(&GetMainThreadOnly().timer_task_cost_estimator);
  }
...
scoped_refptr<TaskQueue> RendererSchedulerImpl::NewLoadingTaskRunner(
  ...
  if (GetMainThreadOnly().current_policy.loading_queue_policy.time_domain_type ==
...
void RendererSchedulerImpl::OnUnregisterTaskQueue(
  ...
  if (loading_task_runners_.find(task_queue) != loading_task_runners_.end()) {
    ...
    task_queue->RemoveTaskObserver(&GetMainThreadOnly().timer_task_cost_estimator);
...
void RendererSchedulerImpl::WillBeginFrame(const cc::BeginFrameArgs& args) {
  ...
  EndIdlePeriod();
  GetMainThreadOnly().estimated_next_frame_begin = args.frame_time + args.interval;
...
bool RendererSchedulerImpl::ShouldPrioritizeInputEvent(
  ...
  if ((web_input_event.GetType() == blink::WebInputEvent::kMouseDown ||
      ...
      (web_input_event.GetModifiers() & blink::WebInputEvent::kLeftButtonDown)) {
    return true;
  }
...
void RendererSchedulerImpl::UpdatePolicyLocked(UpdateType update_type) {
  ...
  GetMainThreadOnly().loading_tasks_seem_expensive = loading_tasks_seem_expensive;
  ...
  if (new_policy_duration > base::TimeDelta()) {
    GetMainThreadOnly().current_policy_expiration_time = now + new_policy_duration;
...
void RendererSchedulerImpl::SetTimerQueueSuspensionWhenBackgroundedEnabled(
  ...
  GetMainThreadOnly().timer_queue_suspension_when_backgrounded_enabled = enabled;
}
...
void RendererSchedulerImpl::ResetForNavigationLocked() {
  ...
  GetMainThreadOnly().have_reported_blocking_intervention_since_navigation = false;
  for (WebViewSchedulerImpl* web_view_scheduler :
       GetMainThreadOnly().web_view_schedulers) {
...
bool RendererSchedulerImpl::MainThreadSeemsUnresponsive(
  ...
  queueing_time_estimator_state = seqlock_queueing_time_estimator_.data.GetState();
...
void RendererSchedulerImpl::OnTriedToExecuteBlockedTask(
  ...
  if (!GetMainThreadOnly().have_reported_blocking_intervention_in_current_policy) {
    GetMainThreadOnly().have_reported_blocking_intervention_in_current_policy =
        true;
  ...
  if (!GetMainThreadOnly().have_reported_blocking_intervention_since_navigation) {
    {
      base::AutoLock lock(any_thread_lock_);
