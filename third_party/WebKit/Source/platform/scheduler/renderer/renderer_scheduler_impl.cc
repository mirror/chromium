...
RendererSchedulerImpl::~RendererSchedulerImpl() {
  ...
  for (auto& pair : task_runners_) {
    if (pair.first->queue_class() == MainThreadTaskQueue::QueueClass::LOADING) {
...
scoped_refptr<MainThreadTaskQueue> RendererSchedulerImpl::NewTaskQueue(
    MainThreadTaskQueue::QueueCreationParams params) {
  helper_.CheckOnValidThread();
...
void RendererSchedulerImpl::ApplyTaskQueuePolicy(
  ...
  bool was_enabled = old_task_queue_policy.IsEnabled(task_queue);
  bool is_enabled = new_task_queue_policy.IsEnabled(task_queue);
  ...
  DCHECK(task_queue_enabled_voter ||
         old_task_queue_policy.IsEnabled(task_queue));

  TaskQueue::QueuePriority old_priority =
      old_task_queue_policy.GetPriority(task_queue);
  TaskQueue::QueuePriority new_priority =
      new_task_queue_policy.GetPriority(task_queue);
...
RendererSchedulerImpl::Policy::compositor_queue_policy() {
  return policies[static_cast<size_t>(
...
RendererSchedulerImpl::Policy::timer_queue_policy() {
  return policies[static_cast<size_t>(MainThreadTaskQueue::QueueClass::TIMER)];
}
...
RendererSchedulerImpl::Policy::default_queue_policy() {
  return policies[static_cast<size_t>(MainThreadTaskQueue::QueueClass::NONE)];
}
