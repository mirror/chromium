...
RendererSchedulerImpl::MainThreadOnly::MainThreadOnly(
    ...
    : loading_task_cost_estimator(time_source,
      ...
      background_long_tail_task_duration_reporter(
          "RendererScheduler.TaskDurationPerQueueType2.Background.LongTail") {
...
base::TimeDelta Intersection(base::TimeTicks start1,
                             base::TimeTicks end1,
  ...
  if (GetMainThreadOnly().renderer_backgrounded) {
    ...
    GetMainThreadOnly()
        .background_first_minute_task_duration_reporter.RecordTask(
            queue_type,
            Intersection(start_time, end_time,
                         GetMainThreadOnly().background_status_changed_at,
                         one_minute_after_backgrounding));

    GetMainThreadOnly().background_long_tail_task_duration_reporter.RecordTask(
        queue_type,
        Intersection(start_time, end_time, one_minute_after_backgrounding,
                     one_minute_after_backgrounding + (end_time - start_time)));
