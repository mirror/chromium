...
RendererSchedulerImpl::MainThreadOnly::MainThreadOnly(
    ...
    : loading_task_cost_estimator(time_source,
      ...
      queue_stopping_when_backgrounded_enabled(false),
      queue_stopped_when_backgrounded(false),
      was_shutdown(false),
...
void RendererSchedulerImpl::UpdatePolicyLocked(UpdateType update_type) {
  ...
  bool queue_newly_stopped = false;
  if (main_thread_only().renderer_backgrounded &&
      main_thread_only().queue_stopping_when_backgrounded_enabled) {
    ...
    queue_newly_stopped = !main_thread_only().queue_stopped_when_backgrounded;
    main_thread_only().queue_stopped_when_backgrounded = now >= stop_at;
    queue_newly_stopped &= main_thread_only().queue_stopped_when_backgrounded;

    if (!main_thread_only().queue_stopped_when_backgrounded)
      UpdatePolicyDuration(now, stop_at, &new_policy_duration);
  } else {
    main_thread_only().queue_stopped_when_backgrounded = false;
  }
...
void RendererSchedulerImpl::SetQueueStoppingWhenBackgroundedEnabled(
    bool enabled) {
  // Note that this will only take effect for the next backgrounded signal.
  main_thread_only().queue_stopping_when_backgrounded_enabled = enabled;
}
...
RendererSchedulerImpl::AsValueLocked(base::TimeTicks optional_now) const {
  ...
  state->SetBoolean("queue_stopped_when_backgrounded",
                    main_thread_only().queue_stopped_when_backgrounded);
