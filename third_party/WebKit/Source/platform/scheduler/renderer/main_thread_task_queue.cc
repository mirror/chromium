...
// static
TaskQueue::Spec MainThreadTaskQueue::GetSpec(
    MainThreadTaskQueue::QueueType queue_type) {
...
MainThreadTaskQueue::MainThreadTaskQueue(
    MainThreadTaskQueue::QueueType queue_type,
    internal::TaskQueueImpl* impl,
    RendererSchedulerImpl* renderer_scheduler)
    : TaskQueue(impl),
      queue_type_(queue_type),
      renderer_scheduler_(renderer_scheduler) {
  impl_->SetOnTaskCompletedHandler(base::Bind(
      &MainThreadTaskQueue::OnTaskCompleted, base::Unretained(this)));
}

MainThreadTaskQueue::~MainThreadTaskQueue() {}

MainThreadTaskQueue::QueueType MainThreadTaskQueue::GetQueueType() const {
  return queue_type_;
}
