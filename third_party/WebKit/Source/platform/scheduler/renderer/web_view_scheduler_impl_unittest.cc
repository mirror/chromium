  ...
  web_frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimer)
      ->PostDelayedTask(BLINK_FROM_HERE,
                        MakeRepeatingTask(web_frame_scheduler_->GetTaskRunner(
                                          ...
                                          &run_count1),
                        TimeDelta::FromMilliseconds(1));
  web_frame_scheduler2->GetTaskRunner(TaskType::kJavascriptTimer)
      ->PostDelayedTask(BLINK_FROM_HERE,
                        MakeRepeatingTask(web_frame_scheduler2->GetTaskRunner(
                                              TaskType::kJavascriptTimer),
                                          &run_count2),
                        TimeDelta::FromMilliseconds(1));
  ...
  web_frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimer)
      ->PostDelayedTask(
          ...
          WTF::Bind(&DelayedRunOrderTask, 1,
                    WTF::Passed(web_frame_scheduler_->GetTaskRunner(
                        TaskType::kJavascriptTimer)),
...
TEST_F(WebViewSchedulerImplTest, DeleteWebViewScheduler_InTask) {
  web_frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimer)
      ->PostTask(BLINK_FROM_HERE,
                 MakeDeletionTask(web_view_scheduler_.release()));
  mock_task_runner_->RunUntilIdle();
}
...
TEST_F(WebViewSchedulerImplTest, VirtualTimeObserver) {
  ...
  web_frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimer)
      ->PostDelayedTask(BLINK_FROM_HERE, WTF::Bind(&NopTask),
                        TimeDelta::FromMilliseconds(200));
...
TEST_F(WebViewSchedulerImplTest, MaxVirtualTimeTaskStarvationCountOneHundred) {
  ...
  RepostingTask(web_frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimer),
                1000, &count);
  web_frame_scheduler_->GetTaskRunner(TaskType::kJavascriptTimer)
      ->PostDelayedTask(BLINK_FROM_HERE,
...
TEST_F(WebViewSchedulerImplTest, OpenWebSocketExemptsFromBudgetThrottling) {
  ...
  for (size_t i = 0; i < 3; ++i) {
    web_frame_scheduler1->GetTaskRunner(TaskType::kJavascriptTimer)
        ->ToSingleThreadTaskRunner()
        ->PostDelayedTask(
            BLINK_FROM_HERE,
            base::Bind(&ExpensiveTestTask, clock_.get(), &run_times),
