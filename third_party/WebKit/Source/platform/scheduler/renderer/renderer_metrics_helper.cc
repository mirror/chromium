...
#define PER_QUEUE_TYPE_METRIC_NAME "RendererScheduler.TaskDurationPerQueueType2"
...
#define PER_TASK_TYPE_METRIC_NAME "RendererScheduler.TaskDurationPerTaskType"
    ...
    : renderer_scheduler_(renderer_scheduler),
      ...
      foreground_main_thread_load_tracker(
          ...
          base::Bind(&RendererMetricsHelper::RecordForegroundMainThreadTaskLoad,
                     base::Unretained(this)),
          kThreadLoadTrackerReportingInterval),
      queue_type_task_duration_reporter(PER_QUEUE_TYPE_METRIC_NAME),
      foreground_queue_type_task_duration_reporter(PER_QUEUE_TYPE_METRIC_NAME
                                                   ".Foreground"),
      foreground_first_minute_queue_type_task_duration_reporter(
          PER_QUEUE_TYPE_METRIC_NAME ".Foreground.FirstMinute"),
      foreground_second_minute_queue_type_task_duration_reporter(
          PER_QUEUE_TYPE_METRIC_NAME ".Foreground.SecondMinute"),
      foreground_third_minute_queue_type_task_duration_reporter(
          PER_QUEUE_TYPE_METRIC_NAME ".Foreground.ThirdMinute"),
      foreground_after_third_minute_queue_type_task_duration_reporter(
          PER_QUEUE_TYPE_METRIC_NAME ".Foreground.AfterThirdMinute"),
      background_queue_type_task_duration_reporter(PER_QUEUE_TYPE_METRIC_NAME
                                                   ".Background"),
      background_first_minute_queue_type_task_duration_reporter(
          PER_QUEUE_TYPE_METRIC_NAME ".Background.FirstMinute"),
      background_second_minute_queue_type_task_duration_reporter(
          PER_QUEUE_TYPE_METRIC_NAME ".Background.SecondMinute"),
      background_third_minute_queue_type_task_duration_reporter(
          PER_QUEUE_TYPE_METRIC_NAME ".Background.ThirdMinute"),
      background_fourth_minute_queue_type_task_duration_reporter(
          PER_QUEUE_TYPE_METRIC_NAME ".Background.FourthMinute"),
      background_fifth_minute_queue_type_task_duration_reporter(
          PER_QUEUE_TYPE_METRIC_NAME ".Background.FifthMinute"),
      background_after_fifth_minute_queue_type_task_duration_reporter(
          PER_QUEUE_TYPE_METRIC_NAME ".Background.AfterFifthMinute"),
      hidden_queue_type_task_duration_reporter(PER_QUEUE_TYPE_METRIC_NAME
                                               ".Hidden"),
      visible_queue_type_task_duration_reporter(PER_QUEUE_TYPE_METRIC_NAME
                                                ".Visible"),
      hidden_music_queue_type_task_duration_reporter(PER_QUEUE_TYPE_METRIC_NAME
                                                     ".HiddenMusic"),
      frame_type_duration_reporter(PER_FRAME_TYPE_METRIC_NAME),
      task_type_duration_reporter(PER_TASK_TYPE_METRIC_NAME),
...
void RendererMetricsHelper::RecordTaskMetrics(
  ...
  queue_type_task_duration_reporter.RecordTask(queue_type, duration);

  if (renderer_scheduler_->main_thread_only().renderer_backgrounded) {
    background_queue_type_task_duration_reporter.RecordTask(queue_type,
                                                            duration);
    ...
    background_first_minute_queue_type_task_duration_reporter.RecordTask(
        queue_type, DurationOfIntervalOverlap(
                        start_time, end_time, backgrounded_at,
                        backgrounded_at + base::TimeDelta::FromMinutes(1)));

    background_second_minute_queue_type_task_duration_reporter.RecordTask(
        queue_type, DurationOfIntervalOverlap(
                        start_time, end_time,
                        backgrounded_at + base::TimeDelta::FromMinutes(1),
                        backgrounded_at + base::TimeDelta::FromMinutes(2)));

    background_third_minute_queue_type_task_duration_reporter.RecordTask(
        queue_type, DurationOfIntervalOverlap(
                        start_time, end_time,
                        backgrounded_at + base::TimeDelta::FromMinutes(2),
                        backgrounded_at + base::TimeDelta::FromMinutes(3)));

    background_fourth_minute_queue_type_task_duration_reporter.RecordTask(
        queue_type, DurationOfIntervalOverlap(
                        start_time, end_time,
                        backgrounded_at + base::TimeDelta::FromMinutes(3),
                        backgrounded_at + base::TimeDelta::FromMinutes(4)));

    background_fifth_minute_queue_type_task_duration_reporter.RecordTask(
        queue_type, DurationOfIntervalOverlap(
                        start_time, end_time,
                        backgrounded_at + base::TimeDelta::FromMinutes(4),
                        backgrounded_at + base::TimeDelta::FromMinutes(5)));

    background_after_fifth_minute_queue_type_task_duration_reporter.RecordTask(
        queue_type,
    ...
    foreground_queue_type_task_duration_reporter.RecordTask(queue_type,
                                                            duration);
    ...
    foreground_first_minute_queue_type_task_duration_reporter.RecordTask(
        queue_type, DurationOfIntervalOverlap(
                        start_time, end_time, foregrounded_at,
                        foregrounded_at + base::TimeDelta::FromMinutes(1)));

    foreground_second_minute_queue_type_task_duration_reporter.RecordTask(
        queue_type, DurationOfIntervalOverlap(
                        start_time, end_time,
                        foregrounded_at + base::TimeDelta::FromMinutes(1),
                        foregrounded_at + base::TimeDelta::FromMinutes(2)));

    foreground_third_minute_queue_type_task_duration_reporter.RecordTask(
        queue_type, DurationOfIntervalOverlap(
                        start_time, end_time,
                        foregrounded_at + base::TimeDelta::FromMinutes(2),
                        foregrounded_at + base::TimeDelta::FromMinutes(3)));

    foreground_after_third_minute_queue_type_task_duration_reporter.RecordTask(
        queue_type,
        DurationOfIntervalOverlap(
            start_time, end_time,
            foregrounded_at + base::TimeDelta::FromMinutes(3),
  ...
  if (renderer_scheduler_->main_thread_only().renderer_hidden) {
    hidden_queue_type_task_duration_reporter.RecordTask(queue_type, duration);

    if (renderer_scheduler_->ShouldDisableThrottlingBecauseOfAudio(
            start_time)) {
      hidden_music_queue_type_task_duration_reporter.RecordTask(queue_type,
                                                                duration);
    }
  } else {
    visible_queue_type_task_duration_reporter.RecordTask(queue_type, duration);
  }

  frame_type_duration_reporter.RecordTask(
      GetFrameType(queue->GetFrameScheduler()), duration);
  if (task_type)
    task_type_duration_reporter.RecordTask(task_type.value(), duration);
}
