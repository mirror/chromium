...
class TestQueueingTimeEstimatorClient : public QueueingTimeEstimator::Client {
 public:
  ...
  const std::vector<base::TimeDelta>& FrameTypeValues(int frame_type) {
    return split_eqts_[QueueingTimeEstimator::Calculator::
...
TEST_F(QueueingTimeEstimatorTest, SplitEQTByTaskQueueType) {
  ...
  EXPECT_THAT(client.FrameTypeValues(
...
TEST_F(QueueingTimeEstimatorTest, SplitEQTByFrameType) {
  ...
  // Beginning of window 5.
  // Scheduler with frame type: MAIN_FRAME_BACKGROUND_EXEMPT.
  ...
  // The following task contributes to "Other" because
  // CROSS_ORIGIN_BACKGROUND_EXEMPT is not a supported frame type.
  FakeWebFrameScheduler scheduler12(
      false, false, WebFrameScheduler::FrameType::kSubframe, true, true);
  queue1->SetFrameScheduler(&scheduler12);
  estimator.OnTopLevelTaskStarted(time, queue1.get());
  time += base::TimeDelta::FromMilliseconds(300);
  estimator.OnTopLevelTaskCompleted(time);

  // The following task contributes to "Other" because
  // SAME_ORIGIN_BACKGROUND_EXEMPT is not a supported frame type.
  FakeWebFrameScheduler scheduler13(
      false, false, WebFrameScheduler::FrameType::kSubframe, false, true);
  queue1->SetFrameScheduler(&scheduler13);
  estimator.OnTopLevelTaskStarted(time, queue1.get());
  time += base::TimeDelta::FromMilliseconds(300);
  estimator.OnTopLevelTaskCompleted(time);
  ...
  EXPECT_THAT(
      client.FrameTypeValues(static_cast<int>(FrameType::MAIN_FRAME_VISIBLE)),
      ::testing::ElementsAre(base::TimeDelta::FromMilliseconds(0),
                             base::TimeDelta::FromMilliseconds(800),
                             base::TimeDelta::FromMilliseconds(900),
                             base::TimeDelta::FromMilliseconds(0),
  ...
  for (const auto& frame_expectation : three_expected) {
    EXPECT_THAT(
        client.FrameTypeValues(static_cast<int>(frame_expectation.frame_type)),
        ::testing::ElementsAre(base::TimeDelta::FromMilliseconds(0),
                               base::TimeDelta::FromMilliseconds(0),
                               base::TimeDelta::FromMilliseconds(0),
                               base::TimeDelta::FromMilliseconds(100),
  ...
  FrameExpectation more_expected[] = {
      {FrameType::MAIN_FRAME_BACKGROUND_EXEMPT,
       "RendererScheduler.ExpectedQueueingTimeByFrameType."
       "MainFrameBackgroundExempt"},
