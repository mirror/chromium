...
using TaskQueueType = QueueingTimeEstimator::SplitCalculator::TaskQueueType;

const size_t kDefaultEQTType = TaskQueueType::kOther;
...
TEST_F(QueueingTimeEstimatorTest, AllTasksWithinWindow) {
  ...
  for (int i = 0; i < 3; ++i) {
    estimator.OnTopLevelTaskStarted(time, kDefaultEQTType);
...
TEST_F(QueueingTimeEstimatorTest,
  ...
  estimator.OnTopLevelTaskStarted(start_time, kDefaultEQTType);
...
TEST_F(QueueingTimeEstimatorTest, SplitEQT) {
  ...
  time += base::TimeDelta::FromMilliseconds(500);
  estimator.OnTopLevelTaskStarted(time, TaskQueueType::kDefault);
  time += base::TimeDelta::FromMilliseconds(3000);
  estimator.OnTopLevelTaskCompleted(time);

  time += base::TimeDelta::FromMilliseconds(1500);
  // Beginning of window 2.
  estimator.OnTopLevelTaskStarted(time, TaskQueueType::kDefaultLoading);
  ...
  time += base::TimeDelta::FromMilliseconds(1000);
  const TaskQueueType types_for_thousand[3] = {
      TaskQueueType::kFrameLoading, TaskQueueType::kFrameThrottleable,
      TaskQueueType::kUnthrottled};
  for (TaskQueueType queue_type : types_for_thousand) {
  ...
  const TaskQueueType types_for_six_hundred[7] = {
      TaskQueueType::kDefault,       TaskQueueType::kDefaultLoading,
      TaskQueueType::kFrameLoading,  TaskQueueType::kFrameThrottleable,
      TaskQueueType::kFramePausable, TaskQueueType::kUnthrottled,
      TaskQueueType::kCompositor};
  for (TaskQueueType queue_type : types_for_six_hundred) {
    estimator.OnTopLevelTaskStarted(time, queue_type);
    time += base::TimeDelta::FromMilliseconds(600);
    estimator.OnTopLevelTaskCompleted(time);
  }

  estimator.OnTopLevelTaskStarted(time, TaskQueueType::kOther);
  time += base::TimeDelta::FromMilliseconds(300);
  estimator.OnTopLevelTaskCompleted(time);
  ...
  EXPECT_THAT(client.GetValueFor(TaskQueueType::kDefault),
  ...
  EXPECT_THAT(client.GetValueFor(TaskQueueType::kFrameLoading),
              ::testing::ElementsAre(base::TimeDelta::FromMilliseconds(0),
                                     base::TimeDelta::FromMilliseconds(0),
                                     base::TimeDelta::FromMilliseconds(0),
                                     base::TimeDelta::FromMilliseconds(100),
  ...
  EXPECT_THAT(client.GetValueFor(TaskQueueType::kUnthrottled),
              ::testing::ElementsAre(base::TimeDelta::FromMilliseconds(0),
                                     base::TimeDelta::FromMilliseconds(0),
                                     base::TimeDelta::FromMilliseconds(0),
                                     base::TimeDelta::FromMilliseconds(100),
  ...
  EXPECT_THAT(client.GetValueFor(TaskQueueType::kCompositor),
              ::testing::ElementsAre(base::TimeDelta::FromMilliseconds(0),
                                     base::TimeDelta::FromMilliseconds(0),
                                     base::TimeDelta::FromMilliseconds(0),
                                     base::TimeDelta::FromMilliseconds(0),
  ...
  EXPECT_THAT(client.GetValueFor(TaskQueueType::kOther),
              ::testing::ElementsAre(base::TimeDelta::FromMilliseconds(0),
                                     base::TimeDelta::FromMilliseconds(0),
                                     base::TimeDelta::FromMilliseconds(0),
                                     base::TimeDelta::FromMilliseconds(0),
