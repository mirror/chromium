...
MainThreadSchedulerHelper::MainThreadSchedulerHelper(
    ...
    : SchedulerHelper(task_queue_manager_delegate),
      ...
      default_task_queue_(NewTaskQueue(
          ...
          MainThreadTaskQueue::GetSpec(MainThreadTaskQueue::QueueType::DEFAULT)
...
scoped_refptr<MainThreadTaskQueue> MainThreadSchedulerHelper::NewTaskQueue(
    ...
    TaskQueue::Spec spec) {
  return make_scoped_refptr(new MainThreadTaskQueue(
      type, task_queue_manager_->NewTaskQueue(spec), renderer_scheduler_));
}
