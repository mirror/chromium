...
void TestTaskForTaskQueueThrottler(std::vector<base::TimeTicks>* run_times,
                                   AutoAdvancingTestClock* clock) {
  run_times->push_back(
      RoundTimeToMilliseconds(clock->GetNowTicksWithoutAdvancing()));
}

void ExpensiveTestTaskForTaskQueueThrottler(
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest, TimerAlignment) {
  ...
  timer_queue_->PostDelayedTask(
      FROM_HERE,
      base::Bind(&TestTaskForTaskQueueThrottler, &run_times, clock_.get()),
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest, WakeUpForNonDelayedTask) {
  ...
  timer_queue_->PostTask(FROM_HERE, base::Bind(&TestTaskForTaskQueueThrottler,
                                               &run_times, clock_.get()));
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest, TimeBasedThrottling) {
  ...
  timer_queue_->PostDelayedTask(
      FROM_HERE,
      base::Bind(&ExpensiveTestTaskForTaskQueueThrottler, &run_times,
                 clock_.get()),
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest,
  ...
  timer_queue_->PostTask(
      FROM_HERE, base::Bind(&ExpensiveTestTaskForTaskQueueThrottler, &run_times,
                            clock_.get()));
  timer_queue_->PostTask(
      FROM_HERE, base::Bind(&ExpensiveTestTaskForTaskQueueThrottler, &run_times,
                            clock_.get()));
