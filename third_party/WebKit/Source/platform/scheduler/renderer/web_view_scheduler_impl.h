...
class PLATFORM_EXPORT WebViewSchedulerImpl
 ...
 private:
  ...
  // The number of tasks that have been run in
  // VirtualTimePolicy::DETERMINISTIC_LOADING while virtual time was not allowed
  // to advance. Used to detect excessive starvation of delayed tasks.
  int task_starvation_count_while_suspended_;
  ...
  int max_task_starvation_count_while_suspended_;
