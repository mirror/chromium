...
void QueueingTimeEstimator::Calculator::UpdateStatusFromTaskQueue(
  ...
  if (interactive_time_.is_null()) {
    interactive_time_ =
        scheduler ? base::TimeTicks() + base::TimeDelta::FromSecondsD(
                                            scheduler->GetTimeToInteractive())
                  : base::TimeTicks();
  }
}
...
void QueueingTimeEstimator::Calculator::EndStep(
  ...
  if (interactive_time_.is_null()) {
    window_end_and_queueing_times_.push_back(
        {step_end_time, window_queueing_time});
  } else {
    if (!window_end_and_queueing_times_.empty()) {
      for (const auto& pair : window_end_and_queueing_times_)
        MaybeReportInteractiveSplit(pair.first, pair.second, client);
      window_end_and_queueing_times_.clear();
    }
...
void QueueingTimeEstimator::State::AdvanceTime(
  ...
  while (TimePastStepEnd(current_time)) {
    ...
    step_start_time += window_step_width;
    calculator_.EndStep(client, step_start_time);
  }
  if (processing_task) {
    calculator_.AddQueueingTime(ExpectedQueueingTimeFromTask(
        current_task_start_time, current_time, step_start_time,
