...
void WebFrameSchedulerImpl::UnregisterSharedSchedulingState(
    scoped_refptr<internal::SharedSchedulingState> proxy) {
  DCHECK(worker_proxies_.Contains(proxy));
  worker_proxies_.erase(proxy);
}

std::unique_ptr<SharedSchedulingStateHandle>
WebFrameSchedulerImpl::CreateSharedSchedulingStateHandle() {
  scoped_refptr<internal::SharedSchedulingState> proxy =
      base::MakeRefCounted<internal::SharedSchedulingState>(this);
  DCHECK(!worker_proxies_.Contains(proxy));
  worker_proxies_.insert(proxy);
  return std::make_unique<SharedSchedulingStateHandle>(std::move(proxy));
}
