...
scoped_refptr<blink::WebTaskRunner> WebFrameSchedulerImpl::LoadingControlTaskRunner() {
  DCHECK(parent_web_view_scheduler_);
...
scoped_refptr<blink::WebTaskRunner> WebFrameSchedulerImpl::ThrottleableTaskRunner() {
  DCHECK(parent_web_view_scheduler_);
  if (!throttleable_task_queue_) {
