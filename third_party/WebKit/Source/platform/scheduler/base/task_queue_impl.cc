...
void TaskQueueImpl::UnregisterTaskQueue() {
  ...
  main_thread_only().on_task_completed_handler = base::nullopt;
  ...
  any_thread().on_next_wake_up_changed_callback = base::nullopt;
  main_thread_only().on_next_wake_up_changed_callback = base::nullopt;
  ...
  if (main_thread_only().on_unregistered_callback)
    main_thread_only().on_unregistered_callback->Run(this);
}
...
void TaskQueueImpl::PushOntoImmediateIncomingQueueLocked(
  ...
  if (was_immediate_incoming_queue_empty) {
    ...
    if (any_thread().on_next_wake_up_changed_callback)
...
void TaskQueueImpl::SetOnNextWakeUpChangedCallback(
    base::Optional<TaskQueueImpl::OnNextWakeUpChangedCallback> callback) {
#if DCHECK_IS_ON()
  if (callback) {
    DCHECK(!main_thread_only().on_next_wake_up_changed_callback)
...
void TaskQueueImpl::SetOnQueueUnregisteredCallback(
    base::Optional<OnQueueUnregisteredCallback> callback) {
  main_thread_only().on_unregistered_callback = callback;
}
