 ...
 public:
  ...
  class State {
   public:
    ...
    void OnRendererBackgrounded(bool backgrounded,
    ...
    // Vector of times when |renderer_backgrounded| changes. The vector contains
    // a pair where the first element is the time when the change occurred, and
    // the second element says what the |renderer_backgrounded| changed to.
    Deque<std::pair<base::TimeTicks, bool>> background_status_changes_;

   private:
    bool CalculateNextVisibleStep();
