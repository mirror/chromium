...
void QueueingTimeEstimator::OnTopLevelTaskStarted(
  ...
  state_.OnTopLevelTaskStarted(task_start_time, start_backgrounded);
}

void QueueingTimeEstimator::OnTopLevelTaskCompleted(
    base::TimeTicks task_end_time,
    bool end_backgrounded) {
  state_.OnTopLevelTaskCompleted(client_, task_end_time, end_backgrounded);
}
...
void QueueingTimeEstimator::State::OnTopLevelTaskStarted(
  ...
  current_task_backgrounded = start_backgrounded;
}
...
void QueueingTimeEstimator::State::OnTopLevelTaskCompleted(
    ...
    base::TimeTicks task_end_time,
    bool end_backgrounded) {
  current_task_backgrounded |= end_backgrounded;
  ...
  // Filter out the task if it was backgrounded at the beginning or end.
  if (current_task_backgrounded) {
    current_task_start_time = base::TimeTicks();
    return;
  }
