...
void QueueingTimeEstimator::State::AdvanceTime(
  ...
  if (in_nested_message_loop_ && processing_task) {
  ...
  if (renderer_backgrounded) {
    ...
    if (step_start_time >= current_time)
      return;

    int num_steps_to_skip =
        (current_time - step_start_time) / window_step_width;
    if ((current_time - step_start_time) % window_step_width != kZeroTimeDelta)
      ++num_steps_to_skip;
    DCHECK_GT(num_steps_to_skip, 0);
  ...
  while (TimePastStepEnd(current_time)) {
    if (processing_task && !TimePastStepEnd(current_task_start_time)) {
    ...
    client->OnQueueingTimeForWindowEstimated(step_queueing_times.GetAverage(),
                                             step_start_time,
                                             step_queueing_times.IndexAtZero());
    step_start_time += window_step_width;
    step_expected_queueing_time = kZeroTimeDelta;
  }
  if (processing_task) {
...
bool QueueingTimeEstimator::RunningAverage::IndexAtZero() const {
  return index_ == 0;
}
