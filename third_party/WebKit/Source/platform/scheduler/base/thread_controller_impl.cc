...
void ThreadControllerImpl::DoWork(Sequence::WorkType work_type) {
  ...
  Sequence::MoreWork more_work_to_do =
      Sequence::MoreWork::kNoImmediateWorkRemaining;
  for (int i = 0; i < work_batch_size_; i++) {
    ...
    if (more_work_to_do == Sequence::MoreWork::kNoImmediateWorkRemaining)
      break;
  ...
  if (more_work_to_do == Sequence::MoreWork::kImmediateWorkRemaining)
    ScheduleWork();
}
