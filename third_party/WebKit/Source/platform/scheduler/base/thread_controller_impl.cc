...
ThreadControllerImpl::ThreadControllerImpl(
  ...
  immediate_do_work_closure_ = base::BindRepeating(
      &ThreadControllerImpl::DoWork, weak_factory_.GetWeakPtr(), false);
  delayed_do_work_closure_ = base::BindRepeating(
      &ThreadControllerImpl::DoWork, weak_factory_.GetWeakPtr(), true);
}
...
void ThreadControllerImpl::DoWork(bool delayed) {
  DCHECK(sequence_);
  base::PendingTask task = sequence_->TakeTask(delayed);
