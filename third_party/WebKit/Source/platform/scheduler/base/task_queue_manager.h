...
class PLATFORM_EXPORT TaskQueueManager
 ...
 protected:
  size_t active_queues_count() { return active_queues_.size(); }

  size_t queues_to_shutdown_count() {
    TakeQueuesToGracefullyShutdownFromHelper();
    return queues_to_gracefully_shutdown_.size();
  }

  size_t queues_to_delete_count() { return queues_to_delete_.size(); }
 ...
 private:
  ...
  const scoped_refptr<GracefulQueueShutdownHelper> graceful_shutdown_helper_;
