...
class PLATFORM_EXPORT TaskQueueManager
 ...
 public:
  ...
  base::TimeDelta DidRunTask() override;
 ...
 private:
  ...
  // Computes the delay till the next task if any. If there are no tasks
  // scheduled then base::TimeDelta::Max() will be returned.
  base::TimeDelta ComputeDelayTillNextTaskLocked(LazyNow* lazy_now);
