...
TEST_F(QueueingTimeEstimatorTest, BackgroundedEQTsWithSingleStepPerWindow) {
  ...
  estimator.OnRendererBackgrounded(true, time);
  estimator.OnTopLevelTaskStarted(time);
  time += base::TimeDelta::FromMilliseconds(456);
  estimator.OnTopLevelTaskCompleted(time);
  time += base::TimeDelta::FromMilliseconds(200);
  estimator.OnRendererBackgrounded(false, time);
...
// window could be made up of non-contiguous steps. The following is the list of
// events, with times deltas from when the first 0-time task ends:
// 400: start task. 1400: backgrounded = true. 2400: backgrounded = false. 2800:
// end task. 2900: start task. 4100: end task. 4500: start task. 4900: end task,
// backgrounded = true. 5900: backgrounded = false, start task. 6600: end task.
// 7000: start task. 9500: backgrounded = true. 12000: backgrounded = false.
// 14500: end task. Therefore, the following are EQTs, again with time deltas
// with respect to the end of the trivial task:
// Win1: [0-1000]. EQT of step [0-1000]: (2400+1800)/2*3/5 = 1260. EQT(win1) =
// 1260/5 = 252.
// Win2: [0-1000],[3000-4000]. EQT of [3000-4000]: (1100+100)/2 = 600. EQT(win2)
// = (1260+600)/5 = 372.
// Win3: [0-1000],[3000-4000],[6000-7000]. EQT of [6000-7000]: 600/2*3/5 = 180.
// EQT(win3) = (1260+600+180)/5 = 408.
