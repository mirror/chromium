...
class TestQueueingTimeEstimatorClient : public QueueingTimeEstimator::Client {
 public:
  ...
  size_t GetIndexFromTaskQueue(MainThreadTaskQueue::QueueType queue_type) {
    return RendererSchedulerImpl::GetIndexFromTaskQueue(queue_type);
  }
  const size_t kDefaultEQTBucket =
      QueueingTimeEstimator::SplitCalculator::kOtherTaskQueueType;
...
TEST_F(QueueingTimeEstimatorTest, AllTasksWithinWindow) {
  ...
  for (int i = 0; i < 3; ++i) {
    estimator.OnTopLevelTaskStarted(time, client.kDefaultEQTBucket);
...
TEST_F(QueueingTimeEstimatorTest,
  ...
  estimator.OnTopLevelTaskStarted(start_time, client.kDefaultEQTBucket);
...
TEST_F(QueueingTimeEstimatorTest, SplitEQT) {
  ...
  estimator.OnTopLevelTaskStarted(
      time,
      client.GetIndexFromTaskQueue(MainThreadTaskQueue::QueueType::DEFAULT));
  time += base::TimeDelta::FromMilliseconds(3000);
  estimator.OnTopLevelTaskCompleted(time);
  ...
  estimator.OnTopLevelTaskStarted(
      time, client.GetIndexFromTaskQueue(
  ...
  for (MainThreadTaskQueue::QueueType queue_type : types_for_thousand) {
    estimator.OnTopLevelTaskStarted(time,
                                    client.GetIndexFromTaskQueue(queue_type));
    time += base::TimeDelta::FromMilliseconds(1000);
    estimator.OnTopLevelTaskCompleted(time);
  }
  ...
  estimator.OnTopLevelTaskStarted(
      time,
      client.GetIndexFromTaskQueue(MainThreadTaskQueue::QueueType::CONTROL));
  time += base::TimeDelta::FromMilliseconds(300);
  estimator.OnTopLevelTaskCompleted(time);
