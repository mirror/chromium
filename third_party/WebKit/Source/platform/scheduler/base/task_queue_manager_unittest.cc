...
class TaskQueueManagerTest : public testing::Test {
 ...
 protected:
  scoped_refptr<TaskQueue> CreateTaskQueueWithSpec(TaskQueue::Spec spec) {
    return make_scoped_refptr(new TaskQueue(manager_->NewTaskQueue(spec)));
  }
  ...
  scoped_refptr<TaskQueue> CreateTaskQueueWithMonitoredQuiescence() {
  ...
  std::vector<scoped_refptr<TaskQueue>> runners_;
...
TEST_F(TaskQueueManagerTest, HasPendingImmediateWork_ImmediateTask) {
  ...
  EXPECT_TRUE(
      runners_[0]->GetTaskQueueImplForTest()->immediate_work_queue()->Empty());
...
TEST_F(TaskQueueManagerTest, HasPendingImmediateWork_DelayedTask) {
  ...
  EXPECT_FALSE(
      runners_[0]->GetTaskQueueImplForTest()->delayed_work_queue()->Empty());
  EXPECT_TRUE(runners_[0]->HasTaskToRunImmediately());
...
TEST_F(TaskQueueManagerTest, TaskQueueObserver_ImmediateTask) {
  ...
  runners_[0]->GetTaskQueueImplForTest()->ReloadImmediateWorkQueueIfEmpty();
  EXPECT_CALL(observer,
              OnQueueNextWakeUpChanged(runners_[0].get(), base::TimeTicks()));
  runners_[0]->PostTask(FROM_HERE, base::Bind(&NopTask));
...
TEST_F(TaskQueueManagerTest, CurrentlyExecutingTaskQueue_TaskRunning) {
  ...
  EXPECT_THAT(task_sources, ElementsAre(queue0->GetTaskQueueImplForTest(),
                                        queue1->GetTaskQueueImplForTest()));
  EXPECT_EQ(nullptr, manager_->currently_executing_task_queue());
}
...
TEST_F(TaskQueueManagerTest, CurrentlyExecutingTaskQueue_NestedLoop) {
  ...
  EXPECT_THAT(task_sources, ElementsAre(queue0->GetTaskQueueImplForTest(),
                                        queue1->GetTaskQueueImplForTest(),
                                        queue2->GetTaskQueueImplForTest(),
                                        queue0->GetTaskQueueImplForTest()));
  EXPECT_EQ(nullptr, manager_->currently_executing_task_queue());
}
...
TEST_F(TaskQueueManagerTest, CouldTaskRun_DisableAndReenable) {
  ...
  EXPECT_TRUE(
      runners_[0]->GetTaskQueueImplForTest()->CouldTaskRun(enqueue_order));
