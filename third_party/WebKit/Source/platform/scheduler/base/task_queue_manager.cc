...
void TaskQueueManager::MaybeScheduleDelayedWork(
  ...
  if (controller_->DoWorkRunningOrPostedWithoutDelay())
    return;
...
void TaskQueueManager::CancelDelayedWork(TimeDomain* requesting_time_domain,
  ...
  main_thread_only().next_delayed_do_work = base::TimeTicks::Max();
}
...
base::Optional<base::PendingTask> TaskQueueManager::TakeTask() {
  ...
  while (true) {
    ...
    if (!should_run) {
      // We appear to have run out of immediate work.
      {
        base::AutoLock lock(any_thread_lock_);
        // Note if virtual time is in play, this may fast forward time which
        // can result in more immediate work.
        base::TimeDelta next_delay = ComputeDelayTillNextTaskLocked(&lazy_now);
        if (next_delay > base::TimeDelta()) {
          // Post a delayed continuation if needed.
          if (next_delay < base::TimeDelta::Max())
            controller_->ScheduleDelayedWork(next_delay);
          return base::nullopt;
        }
...
base::TimeDelta TaskQueueManager::DidRunTask() {
  LazyNow lazy_now(real_time_domain()->CreateLazyNow());
  ExecutingTask& executing_task =
      *main_thread_only().task_execution_stack.rbegin();
  ...
  base::AutoLock lock(any_thread_lock_);
  return ComputeDelayTillNextTaskLocked(&lazy_now);
}

base::TimeDelta TaskQueueManager::ComputeDelayTillNextTaskLocked(
    LazyNow* lazy_now) {
  DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
  ...
  // check is equivalent to calling ReloadEmptyWorkQueues first.
  ...
  for (TimeDomain* time_domain : main_thread_only().time_domains) {
    ...
    if (*delay < delay_till_next_task) {
      ...
      main_thread_only().next_delayed_do_work =
          lazy_now->Now() + delay_till_next_task;
    }
  }
