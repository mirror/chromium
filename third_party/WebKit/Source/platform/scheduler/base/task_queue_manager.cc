...
void TaskQueueManager::MaybeScheduleImmediateWorkLocked(
    ...
    MoveableAutoLock lock) {
  {
    ...
    if (any_thread().immediate_do_work_posted_count > 0 ||
        (any_thread().do_work_running_count > any_thread().nesting_depth)) {
      return;
    }
...
void TaskQueueManager::MaybeScheduleDelayedWork(
...

  {
    ...
    // Don't post a delayed DoWork if there's an immediate DoWork in flight or
    // or we're inside a top level DoWork. We can rely on a delayed continuation
    // pgetting posted as needed.
    if (any_thread().immediate_do_work_posted_count > 0 ||
        (any_thread().do_work_running_count > any_thread().nesting_depth)) {
      return;
    }
...
base::Optional<base::PendingTask> TaskQueueManager::TakeTask(
  ...
  DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
  TRACE_EVENT1("renderer.scheduler", "TaskQueueManager::TakeTask", "delayed",
...

  {
    ...
    switch (work_type) {
      case WorkType::kImmediate:
        // If a delayed task is pending, DidRunTask will return
        // MoreWork::kImmediateWorkRemainingDelayedTaskPending, but if we're
        // executed mid-batch we will be called with WorkType::kImmediate. In
        // that case don't decrement |immediate_do_work_posted_count|.
        if (main_thread_only().last_did_run_task_result !=
            MoreWork::kImmediateWorkRemainingDelayedTaskPending) {
          any_thread().immediate_do_work_posted_count--;
        }
        DCHECK_GE(any_thread().immediate_do_work_posted_count, 0);
        break;

      case WorkType::kDelayed:
        main_thread_only().next_delayed_do_work.Clear();
        break;
    }
  }
  ...
  while (true) {
    ...
    if (!should_run) {
      // We appear to have run out of immediate work. There may be delayed work
      // to do, in which case we must post a delayed continuation.
      {
        MoveableAutoLock lock(any_thread_lock_);
        // Note if virtual time is in play, this may fast forward time.
        base::Optional<NextTaskDelay> next_delay =
            ComputeDelayTillNextTaskLocked(&lazy_now);
        // If there's no more immediate work, post a delayed do work if needed
        // and return nothing..
        if (!next_delay || next_delay->delay() > base::TimeDelta()) {
          any_thread().do_work_running_count--;
          MoreWork more_immediate_work = MaybePostDoWorkContinuationLocked(
              next_delay, &lazy_now, std::move(lock));
          DCHECK_EQ(more_immediate_work, MoreWork::kNoImmediateWorkRemaining);
          return base::nullopt;
        }

        // Due to virtual time fast forwarding, we've just discovered some more
        // immediate work to do.
        std::swap(queues_to_reload, any_thread().has_incoming_immediate_work);
      }

      ReloadEmptyWorkQueues(queues_to_reload);
      lazy_now = LazyNow(real_time_domain()->CreateLazyNow());
      WakeUpReadyDelayedQueues(&lazy_now);
      continue;
    }
...
internal::Sequence::MoreWork TaskQueueManager::DidRunTask() {
  LazyNow lazy_now(real_time_domain()->CreateLazyNow());
  ExecutingTask& executing_task =
      *main_thread_only().task_execution_stack.rbegin();
...

  {
    ...
    main_thread_only().last_did_run_task_result =
        MaybePostDoWorkContinuationLocked(next_delay, &lazy_now,
                                          std::move(lock));
    return main_thread_only().last_did_run_task_result;
  }
}

internal::Sequence::MoreWork
TaskQueueManager::MaybePostDoWorkContinuationLocked(
    base::Optional<NextTaskDelay> next_delay,
    LazyNow* lazy_now,
    MoveableAutoLock lock) {
  DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);

  {
    ...
    if (!next_delay) {
      ...
      return MoreWork::kNoImmediateWorkRemaining;
    }
    ...
    // Treat a negative or zero delay as an immediate task. Note negative delays
    // are not expected to happen in practice.
    if (next_delay->delay() <= base::TimeDelta()) {
      ...
      if (main_thread_only().next_delayed_do_work.run_time() !=
          ...
          main_thread_only().next_delayed_do_work.run_time() <=
              lazy_now->Now()) {
        return MoreWork::kImmediateWorkRemainingDelayedTaskPending;
      }
      any_thread().immediate_do_work_posted_count++;
      return MoreWork::kImmediateWorkRemaining;
    }
  ...
  // Normally running the selector would spot blocked queues, but if we're at
  // the end of a DoWork batch a blocked queue may not get noticed.
  main_thread_only().selector.TrySelectingBlockedQueue();
