...
TaskQueueManager::MainThreadOnly::MainThreadOnly() : is_nested(false) {}
...
void TaskQueueManager::OnBeginNestedRunLoop() {
  ...
  main_thread_only().is_nested = true;
  if (observer_)
...
void TaskQueueManager::OnExitNestedRunLoop() {
  ...
  while (!main_thread_only().non_nestable_task_queue.empty()) {
    ...
    // We need to assign a new enqueue order because the old one has been used
    // and we'd break the assumption in WorkQueue that the orders are
    // monotonically increasing withing the queue.
    non_nestable_task.task.set_enqueue_order(GetNextSequenceNumber());
    non_nestable_task.task_queue->RequeueDeferredNonNestableTask(
        std::move(non_nestable_task.task), non_nestable_task.delayed);
...
TaskQueueManager::ProcessTaskResult TaskQueueManager::ProcessTaskFromWorkQueue(
  ...
  if (pending_task.nestable == base::Nestable::kNonNestable &&
    ...
    NonNestableTask deferred_task{std::move(pending_task), queue, delayed};
  ...
  if (queue->GetShouldNotifyObservers()) {
    ...
    bool notify_time_observers = !main_thread_only().is_nested &&
