...
TaskQueueManager::~TaskQueueManager() {
  ...
  // async deletion.
  std::set<internal::TaskQueueImpl*> active_queues_with_async_deletion_support;
  std::set<internal::TaskQueueImpl*>
      active_queues_without_async_deletion_support;
  for (internal::TaskQueueImpl* queue : active_queues_) {
    if (queue->supports_async_deletion()) {
      active_queues_with_async_deletion_support.insert(queue);
    } else {
      active_queues_without_async_deletion_support.insert(queue);
    }
  }

  for (internal::TaskQueueImpl* queue :
       active_queues_with_async_deletion_support) {
  ...
  active_queues_with_async_deletion_support.clear();
  active_queues_without_async_deletion_support.clear();
