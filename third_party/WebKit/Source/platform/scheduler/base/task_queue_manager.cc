...
void TaskQueueManager::MaybeScheduleDelayedWork(
  ...
  if (!main_thread_only().next_delayed_run_time.is_null() &&
      main_thread_only().next_delayed_run_time <= run_time) {
    return;
  }
...
void TaskQueueManager::CancelDelayedWork(TimeDomain* requesting_time_domain,
                                         base::TimeTicks run_time) {
  DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
  if (main_thread_only().next_delayed_run_time != run_time)
    return;

  controller_->CancelDelayedWork();
  main_thread_only().next_delayed_run_time = base::TimeTicks();
}
  ...
  while (true) {
    ...
    if (!should_run) {
      ...
      // to do, in which case we must post a delayed continuation.
      {
        ...
        if (!next_delay || next_delay > base::TimeDelta()) {
          any_thread().do_work_running_count--;
          bool more_immediate_work = MaybePostDoWorkContinuationLocked(
              next_delay, &lazy_now, std::move(lock));
          DCHECK(!more_immediate_work);
          return base::nullopt;
        }
...
bool TaskQueueManager::DidRunTask() {
...

  {
    ...
    base::Optional<base::TimeDelta> next_delay =
        ComputeDelayTillNextTaskLocked(&lazy_now);
    any_thread().do_work_running_count--;
    return MaybePostDoWorkContinuationLocked(next_delay, &lazy_now,
                                             std::move(lock));
  }
}

bool TaskQueueManager::MaybePostDoWorkContinuationLocked(
...

  {
    ...
    if (!delay) {
      if (main_thread_only().next_delayed_run_time != base::TimeTicks()) {
        main_thread_only().next_delayed_run_time = base::TimeTicks();
        controller_->CancelDelayedWork();
      }
      return false;
    ...
    // If a delayed DoWork is pending then we don't need to post a continuation
    // because it should run immediately.
    if (main_thread_only().next_delayed_run_time != base::TimeTicks() &&
        main_thread_only().next_delayed_run_time <= lazy_now->Now()) {
      return false;
    }
...
TaskQueueManager::ComputeDelayTillNextTaskLocked(LazyNow* lazy_now) {
  ...
  for (TimeDomain* time_domain : main_thread_only().time_domains) {
    ...
    if (!delay_till_next_task || *delay_till_next_task > *delay)
