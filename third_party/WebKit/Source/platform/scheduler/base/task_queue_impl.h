...
class PLATFORM_EXPORT TaskQueueImpl {
 public:
  ...
  using OnQueueUnregisteredCallback =
      base::Callback<void(internal::TaskQueueImpl*)>;
  ...
  void SetOnNextWakeUpChangedCallback(
      base::Optional<OnNextWakeUpChangedCallback> callback);

  void SetOnQueueUnregisteredCallback(
      base::Optional<OnQueueUnregisteredCallback> callback);
  ...
  void SetOnTaskCompletedHandler(
      base::Callback<void(base::TimeTicks, base::TimeTicks)> handler);
 ...
 private:
  ...
  struct AnyThread {
    ...
    base::Optional<OnNextWakeUpChangedCallback>
        on_next_wake_up_changed_callback;
  };
  ...
  struct MainThreadOnly {
    ...
    base::Optional<base::TimeTicks> scheduled_time_domain_wake_up;
    base::Optional<base::Callback<void(base::TimeTicks, base::TimeTicks)>>
        on_task_completed_handler;
    // If false, queue will be disabled. Used only for tests.
    bool is_enabled_for_test;
    base::Optional<OnQueueUnregisteredCallback> on_unregistered_callback;
  };
