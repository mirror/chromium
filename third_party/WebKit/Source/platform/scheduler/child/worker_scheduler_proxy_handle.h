...
// TODO(altimin): WorkerSchedulerProxy can't be forward-declared due to
...
// Public class exposing a handle to WorkerSchedulerProxy to be retained by
// DedicatedWorker.
class PLATFORM_EXPORT WorkerSchedulerProxyHandle {
 public:
  explicit WorkerSchedulerProxyHandle(
      scoped_refptr<internal::WorkerSchedulerProxy> proxy);
  ~WorkerSchedulerProxyHandle();

  scoped_refptr<internal::WorkerSchedulerProxy> GetWorkerSchedulerProxy();

 private:
  scoped_refptr<internal::WorkerSchedulerProxy> proxy_;

  DISALLOW_COPY_AND_ASSIGN(WorkerSchedulerProxyHandle);
};
