...
SharedSchedulingState::SharedSchedulingState(
...
void SharedSchedulingState::OnWorkerSchedulerCreated(
...
void SharedSchedulingState::OnPageVisibilityStateChanged(
...
void SharedSchedulingState::OnHandleDestroyed() {
  DCHECK(IsMainThread());
  ...
  scoped_refptr<SharedSchedulingState> ref(this);

  frame_scheduler_->UnregisterSharedSchedulingState(ref);
  frame_scheduler_ = nullptr;
}

void SharedSchedulingState::OnWorkerSchedulerDestroyed() {
...
bool SharedSchedulingState::IsMainThread() const {
  return main_thread_ref_ == base::PlatformThread::CurrentRef();
}

bool SharedSchedulingState::IsWorkerThread() const {
