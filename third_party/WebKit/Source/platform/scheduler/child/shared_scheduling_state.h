...
// Thread-safe helper class for communication between main thread frame
// scheduler and non-main thread schedulers.
//
...
//   calling WebFrameSchedulerImpl::CreateSharedSchedulingStateHandle.
// - WebFrameSchedulerImpl retains a scoped_refptr to SharedSchedulingState and
//   uses it to pass signals to WorkerScheduler via the proxy.
// - SharedSchedulingStateHandle is owned by the implementation of the worker.
//   Destroying the handle initiates the shutdown of the proxy.
//   (TODO(altimin): Implement the item above).
// - A raw pointer to SharedSchedulingStateHandle is passed to WebThread, which
//   obtains SharedSchedulingState and uses it to initialize WorkerScheduler.
class PLATFORM_EXPORT SharedSchedulingState
    : public ThreadSafeRefCounted<SharedSchedulingState> {
 public:
  // Called on the main thread.
  explicit SharedSchedulingState(WebFrameSchedulerImpl* frame_scheduler);
  ~SharedSchedulingState();
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(SharedSchedulingState);
};
