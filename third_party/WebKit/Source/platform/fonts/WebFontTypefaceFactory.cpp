...
bool WebFontTypefaceFactory::CreateTypeface(sk_sp<SkData> sk_data,
  ...
  sk_sp<SkTypeface> probe_typeface(sk_sp<SkTypeface>(
      DefaultFontManager()->makeFromStream(std::move(stream))));
  ...
  // TODO: Reporting on Windows vs. non-windows? Non-Windows does not need
  // reinstantiation.
...
#if defined(OS_WIN)
  return sk_sp<SkFontMgr>(FontCache::GetFontCache()->FontManager());
#else
