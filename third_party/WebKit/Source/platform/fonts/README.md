...
* [An excursion into prerequisites before text shaping can be performed](#Excursion:-Setting-up-Shaping)
...
As the [excursion on text shaping explains](#Excursion:-Setting-up-shaping),
...
The section ["Setting up Shaping"](#Excursion:-Setting-up-shaping) described the
...
starting with the primary font, identify shaped and non-shaped sequences of the
shaping result, and processing unshaped sub-runs by trying to shape using the
remaining list of fonts, then trying fallback fonts until the last resort font
is reached.

If caps formatting is requested, an additional lowercase/uppercase segmentation
stage is required. In this stage, OpenType features in the font are matched
against the requested formatting and formatting is synthesized as required by
the CSS Level 3 Fonts Module.
