...
class PLATFORM_TEMPLATE_CLASS_EXPORT ShapeResultSpacing final {
  STACK_ALLOCATED();
 ...
 public:
  ...
  bool SetExpansion(float expansion,
                    TextDirection,
                    TextJustify,
  ...
  float ComputeSpacing(unsigned, float& offset);

 private:
  bool IsAfterExpansion() const { return is_after_expansion_; }

  bool ComputeExpansion(bool allows_leading_expansion,
...
extern template class ShapeResultSpacing<TextRun>;
extern template class ShapeResultSpacing<String>;
