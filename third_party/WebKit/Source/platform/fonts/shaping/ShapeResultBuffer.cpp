...
inline void AddGlyphToBloberizer(ShapeResultBloberizer& bloberizer,
  ...
  if (!IsSkipInkException(bloberizer, run, character_index))
    bloberizer.Add(glyph_data.glyph, font_data, start_offset + glyph_data.offset);
}
...
inline void AddEmphasisMark(ShapeResultBloberizer& bloberizer,
  ...
  bool is_vertical = emphasis_font_data->PlatformData().IsVerticalAnyUpright() &&
...
float ShapeResultBuffer::FillTextEmphasisGlyphsForRun(
  ...
  for (unsigned i = 0; i < num_glyphs; ++i) {
    ...
    if (text_run.Is8Bit()) {
      ...
      if (Character::CanReceiveTextEmphasis(text_run[current_character_index])) {
      ...
      graphemes_in_cluster = CountGraphemesInCluster(text_run.Characters16(),
                                                   text_run.CharactersLength(),
...
ShapeResultBuffer::GetRunFontData() const {
  ...
  for (const auto& result : results_) {
    for (const auto& run : result->runs_) {
      font_data.push_back(RunFontData({run->font_data_.Get(),
