...
size_t ConvertToUtf8AndMapOffsets(const T* input_begin, const T* input_end,
                                  ...
                                  HashMap<unsigned, unsigned>& offset_map,
                                  Func&& convert) {
  char* output_begin = utf8_buffer.begin();
  char* output_end = utf8_buffer.end();
  ...
  for (const T* input = input_begin; input < input_end; ) {
    ...
    offset_map.Set(src_index + 1, utf8_index);
  }
...
//  char* output = output_begin;
  ...
  const size_t utf8_len = text.Is8Bit()
      ? ConvertToUtf8AndMapOffsets(text.Characters8() + begin,
                                   text.Characters8() + end,
                                   utf8, index_map,
      ...
      : ConvertToUtf8AndMapOffsets(text.Characters16() + begin,
                                   text.Characters16() + end,
                                   utf8, index_map,
...
//  }
  ...
  pending_utf8_text_.Append(utf8.begin(), utf8_len);
