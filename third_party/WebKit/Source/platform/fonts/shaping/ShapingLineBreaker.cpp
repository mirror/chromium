...
unsigned ShapingLineBreaker::Hyphenate(unsigned offset,
  ...
  if (backwards) {
    prefix_length = hyphenation_->LastHyphenLocation(
        StringView(text, word_start, word_len), offset - word_start);
    LOG(INFO) << "Hyphenate BACK '" << text.Substring(word_start, word_len)
              << "', " << offset - word_start << " = " << prefix_length;
    if (!prefix_length) {
      *is_hyphenated = false;
      return previous_break_opportunity;
    }
  } else {
    prefix_length = hyphenation_->FirstHyphenLocation(
        StringView(text, word_start, word_len), offset - word_start);
    LOG(INFO) << "Hyphenate FORE '" << text.Substring(word_start, word_len)
              << "', " << offset - word_start << " = " << prefix_length;
    if (!prefix_length) {
      *is_hyphenated = false;
      return word_end;
...
PassRefPtr<ShapeResult> ShapingLineBreaker::ShapeLine(
  ...
  if (break_iterator_->BreakAfterSpace() &&
      IsHangableSpace(GetText()[candidate_break])) {
  ...
  // TODO(kojii): Is this condition correct?
  if (!result_out->is_hyphenated &&
      GetText()[break_opportunity - 1] == kSoftHyphenCharacter)
