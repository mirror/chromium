...
TEST_F(CachingWordShaperTest, SegmentCJKAndInherit) {
  const UChar kStr[] = {0x304B,  // HIRAGANA LETTER KA
...
TEST_F(CachingWordShaperTest, TextOrientationFallbackShouldNotInFallbackList) {
  ...
  HashSet<const SimpleFontData*> fallback_fonts;
  ASSERT_GT(
      shaper.Width(text_run, &fallback_fonts, &glyph_bounds),
      0);
...
TEST_F(CachingWordShaperTest, GlyphBoundsWithSpaces) {
  ...
  float periods_and_spaces_width = shaper.Width(periods_and_spaces, nullptr,
                                             &periods_and_spaces_glyph_bounds);
  ...
  ASSERT_GT(periods_and_spaces_glyph_bounds.Width(), periods_glyph_bounds.Width());
}
