...
PassRefPtr<FontFallbackIterator> FontFallbackIterator::Create(
  ...
  return AdoptRef(new FontFallbackIterator(description, std::move(fallback_list),
...
PassRefPtr<FontDataForRangeSet> FontFallbackIterator::Next(
  ...
  ASSERT(fallback_stage_ == kFontGroupFonts || fallback_stage_ == kSegmentedFace);
  const FontData* font_data =
      font_fallback_list_->FontDataAt(font_description_, current_font_data_index_);
