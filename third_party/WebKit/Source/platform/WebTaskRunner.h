...
// Forward-declare because MSVC wants dllimport attribute specified only in
// the first declaration. See declarations below for the actual usage.
...
class BLINK_PLATFORM_EXPORT TaskHandle {
 ...
 private:
  friend TaskHandle PostCancellableTask(WebTaskRunner&,
                                        const base::Location&,
                                        base::OnceClosure);
  friend TaskHandle PostDelayedCancellableTask(WebTaskRunner&,
                                               const base::Location&,
                                               base::OnceClosure,
                                               TimeDelta delay);
...
// cancellation.
TaskHandle PostCancellableTask(WebTaskRunner&,
                               const base::Location&,
                               base::OnceClosure);
TaskHandle PostDelayedCancellableTask(WebTaskRunner&,
                                      const base::Location&,
                                      base::OnceClosure,
                                      TimeDelta delay);
