...
class WeakMemberSet
 ...
 public:
  ...
  WeakMemberSet()
      : size_(0),
        gc_info_index_(GCInfoTrait<WeakMemberSet<T, inlineCapacity>>::Index()) {
#ifdef ANNOTATE_CONTIGUOUS_CONTAINER
    // Because setting inlineCapacity to zero is not possible (for various
    // inheritance reasons), reserve INLINE_CAPACITY + 1 space so that
    // the buffer is forced to use an out-of-line buffer.
    ReserveCapacity(inlineCapacity + 1);
#endif
  }
  ...
  DEFINE_INLINE_TRACE() {
    ...
    visitor->template RegisterWeakMembers<
        WeakMemberSet<T, INLINE_CAPACITY>,
        &WeakMemberSet<T, INLINE_CAPACITY>::clearWeakMembers>(this);
  ...
  size_t GCIndex() const { return gc_info_index_; }
 ...
 private:
  ...
  ImplType buf_;
  size_t size_;
  // Assigned at construction, we have to report this each time the class is
  // traced to avoid an assertion.
  const size_t gc_info_index_;
#ifdef WEAK_MEMBER_SET_DUMPSTATS
