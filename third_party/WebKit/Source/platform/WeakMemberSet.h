...
class WeakMemberSet
 ...
 public:
  ...
  void ReserveCapacity(size_t new_capacity) {
    ...
    if (UNLIKELY(new_capacity <= old_capacity)) {
      ...
      if (((size_ + insertion_head_) << 2) >=
        ...
        return;
      // Can't go below or equal INLINE_CAPACITY otherwise the
      // routine won't release any memory.
      new_capacity = std::max(INLINE_CAPACITY + 1, new_capacity);
  ...
  inline void TrimStart() {
    ...
    ssize_t old_insert = insertion_head_;
#endif
    auto cur = raw_begin();
    auto stop = raw_end();
    while (stop != cur) {
