...
TEST_P(RequestAsFile, AtStartOfEmptyFile) {
  ...
  EXPECT_EQ(expected_data_, read_data);
}

TEST_P(RequestAsFile, OffsetInEmptyFile) {
  FileTestData test = GetParam();
  int file_offset = 32;
  expected_data_.InsertVector(0, Vector<uint8_t>(file_offset));
  ...
  if (test.size == 0) {
    ...
    Vector<uint8_t> read_data(expected_data_.size());
    EXPECT_EQ(static_cast<int>(expected_data_.size()),
