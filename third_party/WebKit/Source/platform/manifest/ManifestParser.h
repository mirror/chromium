...
#ifndef THIRD_PARTY_WEBKIT_SOURCE_PLATFORM_MANIFEST_MANIFESTPARSER_H_
#define THIRD_PARTY_WEBKIT_SOURCE_PLATFORM_MANIFEST_MANIFESTPARSER_H_
...
class PLATFORM_EXPORT ManifestParser {
 public:
  using ParseColorCallback =
      base::RepeatingCallback<bool(Color&, const String&, bool)>;
 ...
 private:
  ...
  bool ParseBoolean(const base::Value& dictionary,
                    const char* key,
                    bool default_value);
  ...
  String ParseString(const base::Value& dictionary, const char* key, TrimType);
  ...
  int64_t ParseColor(const base::Value& dictionary, const char* key);
  ...
  KURL ParseURL(const base::Value& dictionary,
                const char* key,
                const KURL& base_url,
  ...
  // Returns the parsed string if any, a null string if the parsing failed.
  String ParseName(const base::Value& dictionary);
  ...
  // Returns the parsed string if any, a null string if the parsing failed.
  String ParseShortName(const base::Value& dictionary);
  ...
  KURL ParseScope(const base::Value& dictionary, const KURL& start_url);
  ...
  // Returns the parsed KURL if any, an empty KURL if the parsing failed.
  KURL ParseStartURL(const base::Value& dictionary);
  ...
  // parsing failed.
  blink::WebDisplayMode ParseDisplay(const base::Value& dictionary);
  ...
  // Returns the parsed KURL if any, an empty KURL if the parsing failed.
  KURL ParseIconSrc(const base::Value& icon);
  ...
  String ParseIconType(const base::Value& icon);
  ...
  // by gfx::Size(0, 0).
  Vector<WebSize> ParseIconSizes(const base::Value& icon);
  ...
  String ParseShareTargetURLTemplate(const base::Value& share_target);
  ...
  // Returns the parsed string if any, a null string if the parsing failed.
  String ParseRelatedApplicationPlatform(const base::Value& application);
  ...
  KURL ParseRelatedApplicationURL(const base::Value& application);
  ...
  // Returns the parsed string if any, a null string if the parsing failed.
  String ParseRelatedApplicationId(const base::Value& application);
  ...
  bool ParsePreferRelatedApplications(const base::Value& dictionary);
  ...
  // Manifest::kInvalidOrMissingColor if the parsing failed.
  int64_t ParseThemeColor(const base::Value& dictionary);
  ...
  const ParseColorCallback color_parser_;
...
#endif  // THIRD_PARTY_WEBKIT_SOURCE_PLATFORM_MANIFEST_MANIFESTPARSER_H_
