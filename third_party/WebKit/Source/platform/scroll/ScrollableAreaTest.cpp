...
TEST_F(ScrollableAreaTest, ScrollAnimatorCurrentPositionShouldBeSync) {
  ...
  EXPECT_EQ(100.0, scrollable_area->GetScrollAnimator().CurrentOffset().Height());
}
...
TEST_F(ScrollableAreaTest, ScrollbarGraphicsLayerInvalidation) {
  ...
  Scrollbar* scrollbar = Scrollbar::Create(scrollable_area, kHorizontalScrollbar,
                                           kRegularScrollbar, nullptr);
