...
ScrollOffset ScrollAlignment::GetScrollOffsetToExpose(
  ...
  non_zero_visible_rect.Move(-current_scroll_offset);

  // Given the X behavior, compute the X coordinate.
  LayoutUnit x;
  if (scroll_x == kScrollAlignmentNoScroll) {
    x = current_scroll_position.Width();
  } else if (scroll_x == kScrollAlignmentRight) {
    x = expose_rect.MaxX() - non_zero_visible_rect.MaxX();
    ...
    x = expose_rect.X() - non_zero_visible_rect.X();
  }

  // Given the Y behavior, compute the Y coordinate.
  LayoutUnit y;
  if (scroll_y == kScrollAlignmentNoScroll) {
    y = current_scroll_position.Height();
  } else if (scroll_y == kScrollAlignmentBottom) {
    y = expose_rect.MaxY() - non_zero_visible_rect.MaxY();
    ...
    y = expose_rect.Y() - non_zero_visible_rect.Y();
  }
