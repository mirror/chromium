...
struct SnapAreaData {
  ...
  bool operator==(const SnapAreaData& data) const {
    return (data.snap_start_x == snap_start_x) &&
           (data.snap_start_y == snap_start_y) &&
           (data.snap_end_x == snap_end_x) && (data.snap_end_y == snap_end_y) &&
           (data.visible_start_x == visible_start_x) &&
           (data.visible_start_y == visible_start_y) &&
           (data.visible_end_x == visible_end_x) &&
           (data.visible_end_y == visible_end_y) &&
           (data.must_snap == must_snap);
  }

  bool operator!=(const SnapAreaData& data) const { return !(*this == data); }
};
