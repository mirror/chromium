...
class PersistentBase {
 ...
 public:
  ...
  void TracePersistent(VisitorDispatcher visitor) {
    ...
    if (weaknessConfiguration == kWeakPersistentConfiguration) {
      visitor->RegisterWeakCallback(persistent_node_, HandleWeakPersistent);
    } else {
      visitor->Mark(raw_);
    }
  }
 ...
 private:
  ...
  void Initialize() {
    ...
    TraceCallback trace_callback = TraceMethodDelegate<
        PersistentNode,
  ...
  static void HandleWeakPersistent(Visitor* self,
                                   void* persistent_node_pointer) {
    PersistentNode* persistent_node =
        reinterpret_cast<PersistentNode*>(persistent_node_pointer);
    using Base =
        PersistentBase<typename std::remove_const<T>::type,
                       weaknessConfiguration, crossThreadnessConfiguration>;
    Base* persistent = reinterpret_cast<Base*>(persistent_node->Self());
...
class PersistentHeapCollectionBase : public Collection {
 ...
 private:
  ...
  void Initialize() {
    ...
    DCHECK(state->CheckThread());
    persistent_node_ = state->GetPersistentRegion()->AllocatePersistentNode(
        this, TraceMethodDelegate<
                  PersistentNode,
                  &PersistentNode::TracePersistent<
                      PersistentHeapCollectionBase<Collection>>>::Trampoline);
#if DCHECK_IS_ON()
    state_ = state;
#endif
  }
  ...
  void Uninitialize() {
    ...
    DCHECK_EQ(state_, state);
#endif
    state->FreePersistentNode(persistent_node_, true);
    persistent_node_ = nullptr;
  }
