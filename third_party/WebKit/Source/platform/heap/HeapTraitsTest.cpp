...
// MaybeMember<T>
static_assert(std::is_same<MaybeMember<double>::type, double>::value,
              "MaybeMember<double> must not add a Member wrapper");
static_assert(std::is_same<MaybeMember<double*>::type, double*>::value,
              "MaybeMember<double*> must not add a Member wrapper");

static_assert(std::is_same<MaybeMember<Empty>::type, Empty>::value,
              "MaybeMember<Empty> must not add a Member wrapper");

static_assert(
    std::is_same<MaybeMember<StructWithTraceMethod>::type,
                 StructWithTraceMethod>::value,
    "MaybeMember<StructWithTraceMethod> must not add a Member wrapper");

static_assert(
    std::is_same<MaybeMember<GarbageCollectedStruct>::type,
                 Member<GarbageCollectedStruct>>::value,
    "MaybeMember<GarbageCollectedStruct> must not add a Member wrapper");

static_assert(
    std::is_same<MaybeMember<HeapVector<Member<GarbageCollectedStruct>>>::type,
                 Member<HeapVector<Member<GarbageCollectedStruct>>>>::value,
    "MaybeMember on a HeapVector<Member<T>> must wrap it in a Member<>");
