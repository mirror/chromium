...
class Object {
 public:
  ...
  Object(size_t number) : number_(number) {
    const size_t max = ObjectStartBitmap::kMaxEntries;
    EXPECT_GE(max, number_);
  }

  Address address() const {
    return kBaseOffset + ObjectStartBitmap::kGranularity * number_;
  }
...
TEST(ObjectStartBitmapTest, MoreThanZeroEntriesPossible) {
  //EXPECT_LT(0u, ObjectStartBitmap::kMaxEntries);
}
...
TEST(ObjectStartBitmapTest, AdjacentObjectsAtEnd) {
  ObjectStartBitmap bitmap(Object::kBaseOffset);
  const size_t last_entry = ObjectStartBitmap::kMaxEntries;
  Object object0(last_entry - 1);
  Object object1(last_entry);
  bitmap.SetBit(object0);
  bitmap.SetBit(object1);
  EXPECT_FALSE(bitmap.CheckBit(Object(last_entry - 2)));
  size_t count = 0;
  bitmap.Iterate([&count, object0, object1](Address current) {
    if (count == 0) {
      EXPECT_EQ(object0.address(), current);
...
TEST(ObjectStartBitmapTest, FindHeaderIteratingWholeBitmap) {
  ...
  Address hint = Object(ObjectStartBitmap::kMaxEntries);
