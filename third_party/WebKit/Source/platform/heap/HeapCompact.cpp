...
class HeapCompact::MovableObjectFixups final {
 public:
  ...
  void Add(MovableReference* slot) {
    ...
    Address slot_address = reinterpret_cast<Address>(slot);
    void* slot_page_address = BlinkPageAddress(slot_address) + kBlinkGuardPageSize;
...
bool HeapCompact::ShouldCompact(ThreadState* state,
  ...
  return force_compaction_gc_ ||
         (gc_count_since_last_compaction_ > kGCCountSinceLastCompactionThreshold &&
...
void HeapCompact::FinishThreadCompaction() {
  ...
  double time_for_heap_compaction = WTF::CurrentTimeMS() - start_compaction_time_ms_;
