...
template <typename T>
struct MaskedObjectTrackerEntry {
  UntracedMember<T> object;
  unsigned mask;
  DEFINE_INLINE_TRACE() {}
  bool operator==(const MaskedObjectTrackerEntry& other) const {
...
template <typename T>
struct VectorTraits<blink::MaskedObjectTrackerEntry<T>>
    : VectorTraitsBase<blink::MaskedObjectTrackerEntry<T>> {
...
class MaskedObjectTracker {
  ...
  using Entry = MaskedObjectTrackerEntry<T>;
