...
TEST_F(FeaturePolicyTest, PolicyParsedCorrectly) {
  ...
  parsed_policy =
      ParseContainerPolicy("vibrate 'self'", origin_a_.Get(), origin_b_.Get(),
                           &messages, test_feature_name_map);
  ...
  // Simple policy with *.
  parsed_policy =
      ParseContainerPolicy("vibrate *", origin_a_.Get(), origin_b_.Get(),
                           &messages, test_feature_name_map);
  EXPECT_EQ(1UL, parsed_policy.size());
  EXPECT_EQ(WebFeaturePolicyFeature::kVibrate, parsed_policy[0].feature);
  ...
  messages.clear();
  parsed_policy =
      ParseContainerPolicy("vibrate badname fullscreen payment", nullptr,
