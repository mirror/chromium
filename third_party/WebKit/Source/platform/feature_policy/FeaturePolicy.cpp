...
bool IsValidFeatureNameCharacter(UChar c) {
...
bool IsValidOldAllowSyntax(const String& policy,
  ...
  Vector<String> tokens;
  policy.Split(' ', tokens);
  // allow = "feature-name" is also a valid new syntax.
  if (tokens.size() <= 1)
    return false;
  ...
  for (const String& token : tokens) {
    ...
    token.AppendTo(characters);
    const UChar* position = characters.data();
    const UChar* end = position + characters.size();
    SkipWhile<UChar, IsValidFeatureNameCharacter>(position, end);
    if (position != end)
      return false;
  }
...
Vector<WebParsedFeaturePolicyDeclaration> ParseOldAllowSyntax(
  ...
  Vector<WebParsedFeaturePolicyDeclaration> whitelists;
  messages->push_back(
      "The old syntax (allow=\"feature1 feature2 feature3 ...\") will soon be "
      "deprecated");
  Vector<String> tokens;
  policy.Split(' ', tokens);
  for (const String& token : tokens) {
