...
Vector<WebParsedFeaturePolicyDeclaration> ParseContainerPolicy(
  ...
  for (const String& item : policy_items) {
    ...
    for (const String& entry : entry_list) {
      // Remove extra whitespaces, a clean entry would look like:
      //     "name value1 value2" or "name".
      const String clean_entry = entry.SimplifyWhiteSpace();
      // Emptry entry (e.g. ";;;"). Exit early.
      if (clean_entry.IsEmpty()) {
        messages->push_back("An empty feature policy entry (';;') is found.");
        continue;
      }

      Vector<String> tokens;
      clean_entry.Split(' ', tokens);
