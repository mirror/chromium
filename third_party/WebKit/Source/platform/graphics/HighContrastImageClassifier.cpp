...
void HighContrastImageClassifier::GetSamples(
    ...
    float* transparency_ratio,
    float* background_ratio) {
  int block_size_width = 1 + bitmap.width() / 10;
  int block_size_height = 1 + bitmap.height() / 10;
  ...
  for (int y = 0; y < bitmap.height(); y += block_size_height) {
    for (int x = 0; x < bitmap.width(); x += block_size_width) {
      std::vector<SkColor> block_samples;
      int block_transparent_pixels;

      block_transparent_pixels =
          GetBlockSamples(bitmap, pixels_per_block, x, y, block_size_width,
                          block_size_height, &block_samples);
  ...
  if (sampled_pixels->size() < kPixelsToSample / 2 &&
    ...
    for (const IntPoint& block : foreground_blocks) {
      ...
      GetBlockSamples(bitmap, pixels_per_block, block.X(), block.Y(),
                      block_size_width, block_size_height, &block_samples);
      sampled_pixels->insert(sampled_pixels->end(), block_samples.begin(),
                             block_samples.end());
    }
  }
...
// pixels.
int HighContrastImageClassifier::GetBlockSamples(
    const SkBitmap& bitmap,
...
// Given sampled pixels and transparent pixels count of a block the image,
