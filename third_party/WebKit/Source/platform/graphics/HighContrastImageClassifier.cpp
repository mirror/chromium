...
int HighContrastImageClassifier::FindClosestCluster(
  ...
  if (cluster_set.mappers.find(key) == cluster_set.mappers.end()) {
    LOG(ERROR) << "Unexpected key in HighContrast: " << key;
    return 0;
  }
...
float HighContrastImageClassifier::ComputeColorBucketsRatio(
  ...
  if (!cluster_set.centers.size())
    LOG(ERROR) << "ZERO CENTER in HIGH CONTRAST.";
