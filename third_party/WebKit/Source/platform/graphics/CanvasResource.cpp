...
bool CanvasResource::PrepareTransferableResource(
  ...
  *out_resource = viz::TransferableResource::MakeGLOverlay(
      mailbox, GLFilter(), TextureTarget(), GetOrCreateSyncToken(),
...
const gpu::SyncToken& CanvasResource_Bitmap::GetOrCreateSyncToken() {
  DCHECK(image_);  // Calling code should check IsValid() before calling this.
...
const gpu::SyncToken& CanvasResource_GpuMemoryBuffer::GetOrCreateSyncToken() {
