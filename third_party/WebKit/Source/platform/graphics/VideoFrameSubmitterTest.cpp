...
class MockVideoFrameProvider : public cc::VideoFrameProvider {
 public:
  ...
  MOCK_METHOD2(UpdateCurrentFrame, bool(base::TimeTicks,
                                        base::TimeTicks));
...
class MockCompositorFrameSink : NON_EXPORTED_BASE(public viz::mojom::blink::CompositorFrameSink) {
 public:
  MockCompositorFrameSink(viz::mojom::blink::CompositorFrameSinkRequest* request)
      : binding_(this, std::move(*request)) {}
  ~MockCompositorFrameSink() {}

  MOCK_METHOD1(SetNeedsBeginFrame, void(bool));

  void SubmitCompositorFrame(const viz::LocalSurfaceId& id,
                             cc::CompositorFrame frame,
                             ::viz::mojom::blink::HitTestRegionListPtr hit_test_region_list) {
    DoSubmitCompositorFrame(id, &frame);
  }
  MOCK_METHOD2(DoSubmitCompositorFrame, void(
      const viz::LocalSurfaceId&, cc::CompositorFrame*));
...
class VideoFrameSubmitterTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    EXPECT_CALL(*provider_, SetVideoFrameProviderClient(_));
    thread_.task_runner()->PostTask(FROM_HERE, base::Bind(
        &VideoFrameSubmitterTest::MakeSubmitter,
  ...
  void CreateMockCompositorFrameSink(
    ...
    viz::mojom::blink::CompositorFrameSinkRequest request = mojo::MakeRequest(submitter_sink);
    sink_ = base::MakeUnique<StrictMock<MockCompositorFrameSink>>(&request);
  }

  void MakeSubmitter() {
    submitter_ = new VideoFrameSubmitter(
        provider_.get(),
        base::Bind(&VideoFrameSubmitterTest::CreateMockCompositorFrameSink,
                   base::Unretained(this),
                   viz::FrameSinkId(2,2)));
  }
 ...
 protected:
  ...
  VideoFrameSubmitter* submitter_;
};
...
TEST_F(VideoFrameSubmitterTest, StatRenderingFlipsBits) {
  ...
  thread_.task_runner()->PostTask(FROM_HERE, base::Bind(
      &VideoFrameSubmitter::StartRendering,
      base::Unretained(submitter_)));
  thread_.FlushForTesting();
...
TEST_F(VideoFrameSubmitterTest, StopUsingProviderSubmitsFrameAndStopsRendering) {
  ...
  thread_.task_runner()->PostTask(FROM_HERE, base::Bind(
      &VideoFrameSubmitter::StopUsingProvider,
      base::Unretained(submitter_)));
  thread_.FlushForTesting();
...
TEST_F(VideoFrameSubmitterTest, DidReceiveFrameDoesNothingIfRendering) {
  ...
  thread_.task_runner()->PostTask(FROM_HERE, base::Bind(
      &VideoFrameSubmitter::DidReceiveFrame,
      base::Unretained(submitter_)));
  thread_.FlushForTesting();
}
...
TEST_F(VideoFrameSubmitterTest, OnBeginFrameSubmitsFrame) {
  ...
  thread_.task_runner()->PostTask(FROM_HERE, base::Bind(
      &VideoFrameSubmitter::OnBeginFrame,
      base::Unretained(submitter_), args));
  thread_.FlushForTesting();
}
