...
class MockVideoFrameResourceProvider
 ...
 public:
  MockVideoFrameResourceProvider(blink::WebContextProviderCallback callback)
      : blink::VideoFrameResourceProvider(callback, nullptr) {}
...
class VideoFrameSubmitterTest : public ::testing::Test {
 public:
  ...
  void MakeSubmitter() {
    ...
    resource_provider_ =
        new StrictMock<MockVideoFrameResourceProvider>(base::BindRepeating(
            [](base::OnceCallback<void(viz::ContextProvider*)>) {}));
    submitter_->SetResourceProvider(
...
TEST_F(VideoFrameSubmitterTest, SubmitFrameWithoutProviderReturns) {
  ...
  submitter_->SubmitFrame(begin_frame_ack);
