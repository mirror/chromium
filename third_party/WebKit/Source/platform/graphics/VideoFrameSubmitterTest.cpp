...
class MockCompositorFrameSink
    : NON_EXPORTED_BASE(public viz::mojom::blink::CompositorFrameSink) {
 public:
  ...
  void MakeSubmitter() {
    submitter_ = new VideoFrameSubmitter(
        ...
        base::Bind(&VideoFrameSubmitterTest::CreateMockCompositorFrameSink,
                   base::Unretained(this), viz::FrameSinkId(2, 2)));
  }
...
TEST_F(VideoFrameSubmitterTest, SubmitFrameWithoutProviderReturns) {
  ...
  thread_.task_runner()->PostTask(
      FROM_HERE, base::Bind(&VideoFrameSubmitter::SubmitFrame,
                            base::Unretained(submitter_), begin_frame_ack));
