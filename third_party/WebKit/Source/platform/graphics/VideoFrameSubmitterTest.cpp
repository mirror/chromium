...
class MockVideoFrameProvider : public cc::VideoFrameProvider {
 public:
  MockVideoFrameProvider() : weak_ptr_factory_(this) {}
  ~MockVideoFrameProvider() {}

  base::WeakPtr<MockVideoFrameProvider> GetWeakPtr() {
 ...
 private:
  base::WeakPtrFactory<MockVideoFrameProvider> weak_ptr_factory_;
...
class VideoFrameSubmitterTest : public ::testing::Test {
 public:
  ...
  void MakeSubmitter() {
    ...
    submitter_->Initialize(provider_->GetWeakPtr());
...
TEST_F(VideoFrameSubmitterTest, InitWithInvalidProviderDoesNothing) {
  resource_provider_ =
      new StrictMock<MockVideoFrameResourceProvider>(context_provider_.get());
  submitter_ = std::make_unique<VideoFrameSubmitter>(
      base::WrapUnique<MockVideoFrameResourceProvider>(resource_provider_));
  base::WeakPtr<MockVideoFrameProvider> weak_provider = provider_->GetWeakPtr();
  weak_provider.reset();
  // Does not call ObtainContextProvider.
  submitter_->Initialize(weak_provider);
}
