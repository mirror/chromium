...
GpuMemoryBufferImageCopy::GpuMemoryBufferImageCopy()
  ...
  gpu::gles2::GLES2Interface* gl = contextProvider_->ContextGL();
  gl->GenFramebuffers(1, &drawFrameBuffer_);
}

GpuMemoryBufferImageCopy::~GpuMemoryBufferImageCopy() {
  gpu::gles2::GLES2Interface* gl = contextProvider_->ContextGL();
  gl->DeleteFramebuffers(1, &drawFrameBuffer_);
}
...
gfx::GpuMemoryBuffer* GpuMemoryBufferImageCopy::CopyImage(RefPtr<Image> image) {
  ...
  gl->GenTextures(1, &dest_texture_id);
  GLenum target = GL_TEXTURE_2D;
  {
    gl->BindTexture(target, dest_texture_id);
  ...
  gl->BindFramebuffer(GL_DRAW_FRAMEBUFFER, drawFrameBuffer_);
  ...
  // gl->DeleteTextures(1, &dest_texture_id);
