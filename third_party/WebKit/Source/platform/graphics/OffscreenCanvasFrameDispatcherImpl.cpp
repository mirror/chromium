...
void OffscreenCanvasFrameDispatcherImpl::
  ...
  image->EnsureMailbox();
  resource.mailbox_holder =
      gpu::MailboxHolder(image->GetMailbox(), image->GetSyncToken(), GL_TEXTURE_2D);
  resource.read_lock_fences_enabled = false;
  resource.is_software = false;
...
void UpdatePlaceholderImage(WeakPtr<OffscreenCanvasFrameDispatcher> dispatcher,
  ...
  if (placeholder_canvas) {
    placeholder_canvas->SetPlaceholderFrame(std::move(image),
                                           std::move(dispatcher),
...
void OffscreenCanvasFrameDispatcherImpl::DispatchFrame(
    ...
    bool is_web_gl_software_rendering /* This flag is true when WebGL's commit is
  ...
  switch (commit_type) {
      ...
      if (IsMainThread()) {
        DEFINE_STATIC_LOCAL(
            ...
            ("Blink.Canvas.OffscreenCommit.GPUCanvasGPUCompositingMain", 0,
             10000000, 50));
        commit_gpu_canvas_gpu_compositing_main_timer.Count(elapsed_time * 1000000.0);
      } else {
        DEFINE_THREAD_SAFE_STATIC_LOCAL(
            CustomCountHistogram, commit_gpu_canvas_gpu_compositing_worker_timer,
            new CustomCountHistogram(
                "Blink.Canvas.OffscreenCommit.GPUCanvasGPUCompositingWorker", 0,
                10000000, 50));
        commit_gpu_canvas_gpu_compositing_worker_timer.Count(elapsed_time * 1000000.0);
      }
      break;
    case kCommitGPUCanvasSoftwareCompositing:
      if (IsMainThread()) {
        DEFINE_STATIC_LOCAL(
            CustomCountHistogram, commit_gpu_canvas_software_compositing_main_timer,
        ...
        DEFINE_THREAD_SAFE_STATIC_LOCAL(
            CustomCountHistogram, commit_gpu_canvas_software_compositing_worker_timer,
    ...
    case kCommitSoftwareCanvasGPUCompositing:
      if (IsMainThread()) {
        ...
        DEFINE_THREAD_SAFE_STATIC_LOCAL(
            CustomCountHistogram, commit_software_canvas_gpu_compositing_worker_timer,
            new CustomCountHistogram("Blink.Canvas.OffscreenCommit."
      ...
      if (IsMainThread()) {
        DEFINE_STATIC_LOCAL(CustomCountHistogram,
                            ...
                            ("Blink.Canvas.OffscreenCommit."
                             ...
                             0, 10000000, 50));
        commit_software_canvas_software_compositing_main_timer.Count(elapsed_time *
                                                               1000000.0);
      } else {
        DEFINE_THREAD_SAFE_STATIC_LOCAL(
            ...
            new CustomCountHistogram("Blink.Canvas.OffscreenCommit."
                                     ...
                                     0, 10000000, 50));
        commit_software_canvas_software_compositing_worker_timer.Count(elapsed_time *
                                                                 1000000.0);
      }
      break;
