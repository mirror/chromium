...
std::unique_ptr<OffscreenCanvasFrameDispatcherImpl::FrameResources>
OffscreenCanvasFrameDispatcherImpl::createOrRecycleFrameResources() {
  if (recycleable_resources_) {
    recycleable_resources_->spare_lock_ = true;
    return std::move(recycleable_resources_);
  }
  return std::unique_ptr<FrameResources>(new FrameResources());
}
...
void OffscreenCanvasFrameDispatcherImpl::SetTransferableResourceToSharedBitmap(
  ...
  std::unique_ptr<FrameResources> frame_resources =
      createOrRecycleFrameResources();
  if (!frame_resources->shared_bitmap_) {
    frame_resources->shared_bitmap_ =
        Platform::Current()->AllocateSharedBitmap(IntSize(width_, height_));
    if (!frame_resources->shared_bitmap_)
      return;
  }
  unsigned char* pixels = frame_resources->shared_bitmap_->pixels();
  DCHECK(pixels);
  ...
  resource.mailbox_holder.mailbox = frame_resources->shared_bitmap_->id();
  resource.mailbox_holder.texture_target = 0;
  resource.is_software = true;

  resources_.insert(next_resource_id_, std::move(frame_resources));
}
...
void OffscreenCanvasFrameDispatcherImpl::
  ...
  if (frame_resources->texture_id_ == 0u) {
    gl->GenTextures(1, &frame_resources->texture_id_);
    gl->BindTexture(GL_TEXTURE_2D, frame_resources->texture_id_);
    GLenum format =
    ...
    gl->GenMailboxCHROMIUM(frame_resources->mailbox_.name);
    gl->ProduceTextureCHROMIUM(GL_TEXTURE_2D, frame_resources->mailbox_.name);
  }
  ...
  resource.mailbox_holder =
      gpu::MailboxHolder(frame_resources->mailbox_, sync_token, GL_TEXTURE_2D);
...
void OffscreenCanvasFrameDispatcherImpl::
  ...
  std::unique_ptr<FrameResources> frame_resources =
      createOrRecycleFrameResources();
  frame_resources->image_ = std::move(image);
  resources_.insert(next_resource_id_, std::move(frame_resources));
}
...
OffscreenCanvasFrameDispatcherImpl::FrameResources::~FrameResources() {
  gpu::gles2::GLES2Interface* gl = SharedGpuContext::Gl();
  if (texture_id_)
  ...
  for (const auto& resource : resources) {
    ...
    if (it != resources_.end()) {
      if (it->value->image_) {
        ...
        } else if (SharedGpuContext::IsValid() &&
...
void OffscreenCanvasFrameDispatcherImpl::ReclaimResourceInternal(
  ...
  if (it->value->spare_lock_) {
    ...
    recycleable_resources_ = std::move(it->value);
    // release SkImage immediately since it is not recycleable
    recycleable_resources_->image_ = nullptr;
    resources_.erase(it);
  }
}
