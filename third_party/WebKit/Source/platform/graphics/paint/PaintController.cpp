...
void PaintController::GenerateRasterInvalidationsComparingChunks(
  ...
  for (const auto& item : clients_to_invalidate) {
    ...
    const auto* old_item = item.value.old_item;
    const auto* new_item = item.value.new_item;
    if (!old_item || old_item->VisualRect().IsEmpty()) {
      if (new_item && !new_item->VisualRect().IsEmpty()) {
        AddRasterInvalidation(client, new_chunk,
                              FloatRect(new_item->VisualRect()),
                              PaintInvalidationReason::kAppeared);
      }
      continue;
    ...
    if (client.GetPaintInvalidationReason() ==
      ...
      GenerateIncrementalRasterInvalidation(new_chunk, *old_item, *new_item);
      continue;
    }
    GenerateFullRasterInvalidation(new_chunk, *old_item, *new_item);
  }
}
