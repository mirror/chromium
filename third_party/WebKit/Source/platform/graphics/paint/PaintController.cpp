...
void PaintController::ProcessNewItem(DisplayItem& display_item) {
  ...
  if (new_display_item_list_.size() >= 2 && display_item.IsEnd()) {
    ...
    if (begin_display_item.IsBegin() &&
        !begin_display_item.DrawsContent())
...
size_t PaintController::FindCachedItem(const DisplayItem::Id& id) {
  ...
  size_t found_index = FindMatchingItemFromIndex(
      id, out_of_order_item_indices_, current_paint_artifact_.GetDisplayItemList());
...
void PaintController::CopyCachedSubsequence(size_t begin_index,
  ...
  if (RuntimeEnabledFeatures::slimmingPaintV2Enabled()) {
    ...
    UpdateCurrentPaintChunkProperties(
        cached_chunk->id ? &*cached_chunk->id : nullptr, cached_chunk->properties);
  } else {
...
void PaintController::GenerateChunkRasterInvalidationRects(
  ...
  if (new_chunk.begin_index >= current_cached_subsequence_begin_index_in_new_list_)
    return;
  ...
  while (next_chunk_to_match_ < old_chunks.size()) {
    ...
    if (new_chunk.Matches(old_chunk)) {
      GenerateChunkRasterInvalidationRectsComparingOldChunk(new_chunk, old_chunk);
...
void PaintController::GenerateChunkRasterInvalidationRectsComparingOldChunk(
  ...
  for (size_t old_index = old_chunk.begin_index; old_index < old_chunk.end_index;
       ++old_index) {
    ...
    if (!old_item.HasValidClient()) {
      ...
      if (new_display_item_list_[moved_to_index].DrawsContent()) {
        if (moved_to_index < new_chunk.begin_index ||
          ...
          // it in the old chunk.
          client_to_invalidate = &new_display_item_list_[moved_to_index].Client();
    ...
    if (client_to_invalidate &&
        invalidated_clients_in_old_chunk.insert(client_to_invalidate).is_new_entry) {
      AddRasterInvalidationInfo(
          is_potentially_invalid_client ? nullptr : client_to_invalidate, new_chunk,
  ...
  for (size_t new_index = new_chunk.begin_index; new_index < new_chunk.end_index;
       ++new_index) {
    const DisplayItem& new_item = new_display_item_list_[new_index];
    if (new_item.DrawsContent() && !ClientCacheIsValid(new_item.Client()) &&
        invalidated_clients_in_new_chunk.insert(&new_item.Client()).is_new_entry) {
...
void PaintController::ShowUnderInvalidationError(
  ...
  if (old_item->IsDrawing()) {
    old_record =
        static_cast<const DrawingDisplayItem*>(old_item)->GetPaintRecord().get();
  }
  LOG(INFO) << "new record:\n"
...
void PaintController::CheckUnderInvalidation() {
  ...
  if (!old_and_new_equal) {
    ...
    if (new_item.IsDrawing() && skipped_probable_under_invalidation_count_ == 1) {
