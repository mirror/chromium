...
void PaintController::GenerateRasterInvalidationsComparingChunks(
  ...
  for (const auto& item : clients_to_invalidate) {
    GenerateRasterInvalidation(new_chunk, item.value.old_item,
                               item.value.new_item);
  }
}

void PaintController::GenerateRasterInvalidation(PaintChunk& chunk,
                                                 const DisplayItem* old_item,
                                                 const DisplayItem* new_item) {
  DCHECK(old_item || new_item);
  DCHECK(!old_item || !new_item || &old_item->Client() == &new_item->Client());

  if (!new_item || new_item->VisualRect().IsEmpty()) {
    AddRasterInvalidation(old_item->Client(), chunk,
                          FloatRect(old_item->VisualRect()),
                          PaintInvalidationReason::kDisappeared);
    return;
  }

  const auto& client = new_item->Client();
