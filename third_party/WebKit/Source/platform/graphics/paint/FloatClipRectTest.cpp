...
TEST_F(FloatClipRectTest, Map) {
  ...
  TransformationMatrix translate_scale =
      TransformationMatrix().Translate(10, 20).Scale(5);
  ...
  rect2.Map(translate_scale);
  EXPECT_EQ(FloatRect(15, 30, 15, 20), rect2.Rect());
  EXPECT_TRUE(rect2.IsTight());

  rect2.Map(rotate);
  EXPECT_EQ(rotate.MapRect(FloatRect(15, 30, 15, 20)), rect2.Rect());
  EXPECT_FALSE(rect2.IsTight());
