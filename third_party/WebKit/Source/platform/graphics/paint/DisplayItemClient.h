...
class PLATFORM_EXPORT DisplayItemClient {
 public:
  ...
  // The rect needs to be invalidated partially in this client. It's in the same
  // coordinate space as VisualRect().
  virtual LayoutRect PartialInvalidationRect() const { return LayoutRect(); }

  // This is called by PaintController::CommitNewDisplayItems() for all clients
  // after painting.
