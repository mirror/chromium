...
class TestDisplayItemGeneric : public DisplayItem {
 public:
  TestDisplayItemGeneric(const DisplayItemClient& client,
...
class TestDisplayItemRequiringSeparateChunk : public TestDisplayItemGeneric {
 public:
  TestDisplayItemRequiringSeparateChunk(const DisplayItemClient& client)
      : TestDisplayItemGeneric(client, DisplayItem::kForeignLayerPlugin) {}
};
...
TEST_F(PaintChunkerTest, SingleNonEmptyRange) {
  ...
  chunker.IncrementDisplayItemIndex(TestDisplayItemGeneric(client_));
  chunker.IncrementDisplayItemIndex(TestDisplayItemGeneric(client_));
...
TEST_F(PaintChunkerTest, BuildMultipleChunksWithDifferentPropertyChanges) {
  ...
  TestDisplayItemGeneric item_after_restore(client_, DisplayItemType(10));
...
TEST_F(PaintChunkerTest, CreatesSeparateChunksWhenRequested) {
  ...
  TestDisplayItemGeneric after_i2(client_, DisplayItemType(10));
...
class TestScrollHitTestRequiringSeparateChunk : public TestDisplayItemGeneric {
 public:
  TestScrollHitTestRequiringSeparateChunk(const DisplayItemClient& client)
      : TestDisplayItemGeneric(client, DisplayItem::kScrollHitTest) {}
};
...
TEST_F(PaintChunkerTest, ChunksFollowingForcedChunk) {
  ...
  TestDisplayItemGeneric before_forced1(client, DisplayItemType(9));
  TestDisplayItemGeneric before_forced2(client, DisplayItemType(10));
  TestScrollHitTestRequiringSeparateChunk forced(client);
  TestDisplayItemGeneric after_forced1(client, DisplayItemType(11));
  TestDisplayItemGeneric after_forced2(client, DisplayItemType(12));
...
TEST_F(PaintChunkerTest, ChunkIdsSkippingCache) {
  ...
  TestDisplayItemGeneric uncacheable_item(client_);
  ...
  TestDisplayItemGeneric after_separate_chunk(client_, DisplayItemType(3));
  chunker.IncrementDisplayItemIndex(after_separate_chunk);

  chunker.UpdateCurrentPaintChunkProperties(nullptr,
                                            DefaultPaintChunkProperties());
  TestDisplayItemGeneric after_restore(client_, DisplayItemType(4));
