...
TEST_F(PaintChunkerTest, BuildMultipleChunksWithDifferentPropertyChanges) {
  ...
  PaintChunkProperties simple_transform_and_effect = DefaultPaintChunkProperties();
  ...
  chunker.UpdateCurrentPaintChunkProperties(nullptr, simple_transform_and_effect);
  chunker.IncrementDisplayItemIndex(NormalTestDisplayItem(client_));
  chunker.IncrementDisplayItemIndex(NormalTestDisplayItem(client_));

  PaintChunkProperties simple_transform_and_effect_with_updated_transform =
      DefaultPaintChunkProperties();
  simple_transform_and_effect_with_updated_transform.property_tree_state.SetTransform(
      TransformPaintPropertyNode::Create(nullptr,
                                         TransformationMatrix(1, 1, 0, 0, 0, 0),
                                         FloatPoint3D(9, 8, 7))
          .Get());
  simple_transform_and_effect_with_updated_transform.property_tree_state.SetEffect(
      CreateOpacityOnlyEffect(
          EffectPaintPropertyNode::Root(),
          simple_transform_and_effect.property_tree_state.Effect()->Opacity())
          .Get());
