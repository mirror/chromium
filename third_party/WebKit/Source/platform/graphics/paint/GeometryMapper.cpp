...
// It can be proved that the result will be the same (except numerical
...
GeometryMapper::SourceToDestinationProjectionInternal(
  ...
  if (source_cache.checkpoint() == destination_cache.checkpoint()) {
    success = true;
    if (source == destination_cache.checkpoint())
      return destination_cache.from_checkpoint();
    if (destination == source_cache.checkpoint())
      return source_cache.to_checkpoint();
    temp = destination_cache.from_checkpoint();
    temp.Multiply(source_cache.to_checkpoint());
    return temp;
  }
