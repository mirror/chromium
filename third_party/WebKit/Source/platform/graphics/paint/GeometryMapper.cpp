...
void GeometryMapper::SourceToDestinationVisualRectInternal(
    ...
    bool& success) {
  LocalToAncestorVisualRectInternal(source_state, destination_state, mapping_rect,
                                    success);
...
void GeometryMapper::SlowLocalToAncestorVisualRectWithEffects(
  ...
  for (const auto* effect = local_state.Effect();
    ...
    SourceToDestinationVisualRectInternal(
        last_transform_and_clip_state, transform_and_clip_state, mapping_rect, success);
    if (!success)
      return;
  ...
  PropertyTreeState final_transform_and_clip_state(ancestor_state.Transform(),
                                               ancestor_state.Clip(), nullptr);
  SourceToDestinationVisualRectInternal(last_transform_and_clip_state,
                                        final_transform_and_clip_state, mapping_rect,
                                        success);
}
...
const FloatClipRect& GeometryMapper::SourceToDestinationClipRectInternal(
  ...
  const FloatClipRect& result = LocalToAncestorClipRectInternal(
      source_state.Clip(), destination_state.Clip(), destination_state.Transform(),
      success);
  // Success if destinationState is an ancestor state.
  if (success)
    return result;
...
const FloatClipRect& GeometryMapper::LocalToAncestorClipRectInternal(
  ...
  GeometryMapperClipCache::ClipAndTransform clip_and_transform(ancestor_clip,
                                                             ancestor_transform);
  ...
  for (auto it = intermediate_nodes.rbegin(); it != intermediate_nodes.rend();
       ++it) {
    success = false;
    const TransformationMatrix& transform_matrix = LocalToAncestorMatrixInternal(
        (*it)->LocalTransformSpace(), ancestor_transform, success);
    if (!success)
      return InfiniteClip();
