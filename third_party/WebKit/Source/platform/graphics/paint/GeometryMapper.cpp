...
void GeometryMapper::LocalToAncestorVisualRectInternal(
  ...
  const auto& transform = SourceToDestinationProjectionInternal(
  ...
  if (success) {
    // This is where we propagate the rounded-ness of |clip_rect| to
    // |rect_to_map|.
...
const FloatClipRect& GeometryMapper::LocalToAncestorClipRectInternal(
    ...
    bool& success) {
  DEFINE_STATIC_LOCAL(FloatClipRect, tight_infinite, (FloatClipRect()));
