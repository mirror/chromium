...
// computes destination_to_LCA ^ -1 * source_to_LCA.
// It can be proved that [2] the result will be the same (except numerical
// errors) when the LCA has invertible screen projection, and this offers
// fallback definition when LCA is singular. For example:
...
//
// [1] As defined by that all local transforms between source and the LCA
//     and all local transforms between the destination and the LCA being flat.
// [2] destination_to_screen = LCA_to_screen * destination_to_LCA
//     source_to_screen = LCA_to_screen * source_to_LCA
//     output = flatten(destination_to_screen)^-1 * flatten(source_to_screen)
//     = flatten(LCA_to_screen * destination_to_LCA)^-1 *
//       flatten(LCA_to_screen * source_to_LCA)
//     Because both destination_to_LCA and source_to_LCA are already flat,
//     = flatten(LCA_to_screen * flatten(destination_to_LCA))^-1 *
//       flatten(LCA_to_screen * flatten(source_to_LCA))
//     Then by flatten lemma flatten(A * flatten(B)) = flatten(A) * flatten(B),
//     = flatten(destination_to_LCA)^-1 * flatten(LCA_to_screen)^-1 *
//       flatten(LCA_to_screen) * flatten(source_to_LCA)
//     If flatten(LCA_to_screen) is invertible, they cancel out:
//     = flatten(destination_to_LCA)^-1 * flatten(source_to_LCA)
//     = destination_to_LCA^-1 * source_to_LCA
