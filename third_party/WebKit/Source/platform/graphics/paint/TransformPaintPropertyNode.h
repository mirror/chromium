 ...
 public:
  ...
  PassRefPtr<TransformPaintPropertyNode> Clone() const {
    return AdoptRef(new TransformPaintPropertyNode(
        ...
        rendering_context_id_, direct_compositing_reasons_, compositor_element_id_,
        scroll_ ? scroll_->Clone() : nullptr));
  }
 ...
 private:
  ...
  GeometryMapperTransformCache& GetTransformCache() {
    if (!geometry_mapper_transform_cache_)
      geometry_mapper_transform_cache_.reset(new GeometryMapperTransformCache());
  ...
  std::unique_ptr<GeometryMapperTransformCache> geometry_mapper_transform_cache_;
};
