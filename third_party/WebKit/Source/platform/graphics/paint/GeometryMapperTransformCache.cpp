...
void GeometryMapperTransformCache::Update(
  ...
  if (!node.Parent()) {
    ...
    checkpoint_ = &node;
    to_checkpoint_.MakeIdentity();
    from_checkpoint_.MakeIdentity();
    return;
  }
  ...
  if (!local.IsFlat() || !local.IsInvertible()) {
    ...
    checkpoint_ = parent.checkpoint_;
    to_checkpoint_ = parent.to_checkpoint_;
    to_checkpoint_.Multiply(local);
    from_checkpoint_ = local.Inverse();
    from_checkpoint_.Multiply(parent.from_checkpoint_);
  }
}
