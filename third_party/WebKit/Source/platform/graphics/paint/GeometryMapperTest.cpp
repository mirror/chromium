...
class GeometryMapperTest : public ::testing::Test,
 ...
 public:
  ...
  void LocalToAncestorVisualRectInternal(const PropertyTreeState& local_state,
                                         const PropertyTreeState& ancestor_state,
                                         FloatRect& mapping_rect,
                                         bool& success) {
    FloatClipRect float_clip_rect(mapping_rect);
    GeometryMapper::LocalToAncestorVisualRectInternal(local_state, ancestor_state,
                                                      float_clip_rect, success);
    mapping_rect = float_clip_rect.Rect();
  }
...
#define EXPECT_RECT_EQ(expected, actual)                                       \
  do {                                                                         \
    ...
    EXPECT_TRUE(GeometryTest::ApproximatelyEqual(expected.X(), actual_rect.X(), \
                                                 kTestEpsilon))                \
        << "actual: " << actual_rect.X() << ", expected: " << expected.X();     \
    EXPECT_TRUE(GeometryTest::ApproximatelyEqual(expected.Y(), actual_rect.Y(), \
                                                 kTestEpsilon))                \
...
TEST_F(GeometryMapperTest, SiblingTransforms) {
  ...
  GeometryMapper::SourceToDestinationVisualRect(transform1_state,
                                                transform2_state, float_clip_rect);
...
TEST_F(GeometryMapperTest, SiblingTransformsWithClip) {
  ...
  // destination.
  FloatRect result = input;
  SourceToDestinationVisualRectInternal(transform1_state, transform2_and_clip_state,
                                        result, success);
  ...
  result = input;
  FloatClipRect float_clip_rect(result);
  GeometryMapper::SourceToDestinationVisualRect(transform2_and_clip_state,
...
TEST_F(GeometryMapperTest, FilterWithClipsAndTransforms) {
  ...
  RefPtr<ClipPaintPropertyNode> clip_above_effect = ClipPaintPropertyNode::Create(
  ...
  RefPtr<EffectPaintPropertyNode> effect = EffectPaintPropertyNode::Create(
      EffectPaintPropertyNode::Root(), transform_above_effect, clip_above_effect,
      kColorFilterNone, filters, 1.0, SkBlendMode::kSrcOver);
