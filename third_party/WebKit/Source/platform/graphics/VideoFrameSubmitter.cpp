...
void VideoFrameSubmitter::Initialize(
    base::WeakPtr<cc::VideoFrameProvider> provider) {
  DCHECK(!provider_);
  if (provider) {
...
void VideoFrameSubmitter::DidReceiveCompositorFrameAck(
  ...
  provider_->GetTaskRunner()->PostTask(
      FROM_HERE, base::Bind(&VideoFrameSubmitter::ReclaimResources,
