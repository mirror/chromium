...
RecordingImageBufferSurface::RecordingImageBufferSurface(
    const IntSize& size,
    std::unique_ptr<RecordingImageBufferFallbackSurfaceFactory> fallback_factory,
    OpacityMode opacity_mode,
    sk_sp<SkColorSpace> color_space,
    SkColorType color_type)
    : ImageBufferSurface(size, opacity_mode, std::move(color_space), color_type),
      image_buffer_(0),
...
SnapshotReasonToFallbackReason(SnapshotReason reason) {
  switch (reason) {
    ...
    case kSnapshotReasonGetImageData:
      return RecordingImageBufferSurface::kFallbackReasonSnapshotForGetImageData;
