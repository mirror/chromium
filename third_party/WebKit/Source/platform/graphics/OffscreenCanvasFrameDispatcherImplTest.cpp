...
class OffscreenCanvasFrameDispatcherImplTest : public ::testing::Test {
 public:
  ...
  unsigned GetNumPostImagesUnresponded() {
    return dispatcher_->num_post_images_unresponded_;
  }
...
TEST_F(OffscreenCanvasFrameDispatcherImplTest, PlaceholderRunsNormally) {
  ...
  DispatchOneFrame();
  EXPECT_EQ(1u, GetNumPostImagesUnresponded());
  ...
  EXPECT_CALL(*(Dispatcher()), PostImageToPlaceholder(_, post_resource_id));
  DispatchOneFrame();
  EXPECT_EQ(2u, GetNumPostImagesUnresponded());
  ...
  EXPECT_CALL(*(Dispatcher()), PostImageToPlaceholder(_, post_resource_id));
  DispatchOneFrame();
  EXPECT_EQ(3u, GetNumPostImagesUnresponded());
  ...
  reclaim_resource_id++;
  Dispatcher()->ReclaimResource(reclaim_resource_id);
  EXPECT_EQ(0u, GetNumPostImagesUnresponded());
}
