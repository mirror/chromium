...
void OffscreenCanvasResourceProvider::SetTransferableResourceToSharedGPUContext(
  ...
  if (frame_resource->texture_id_ == 0u ||
    ...
    bool bitmap_is_bgra = kN32_SkColorType == kBGRA_8888_SkColorType;
    ...
    std::unique_ptr<uint8_t[]> swizzled;
    if (bitmap_is_bgra && !texture_can_be_bgra) {
      size_t num_pixels = base::CheckMul(static_cast<size_t>(width_),
                                         static_cast<size_t>(height_))
                              .ValueOrDie();
      swizzled = std::make_unique<uint8_t[]>(num_pixels * 4);
      SkSwapRB(reinterpret_cast<uint32_t*>(swizzled.get()),
               static_cast<uint32_t*>(src), num_pixels);
      src = swizzled.get();
    }

    gl->GenTextures(1, &frame_resource->texture_id_);
    gl->BindTexture(GL_TEXTURE_2D, frame_resource->texture_id_);
    GLenum format = texture_can_be_bgra ? GL_BGRA_EXT : GL_RGBA;
