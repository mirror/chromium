...
CanvasResourceProvider::CanvasImageProvider::GetDecodedDrawImage(
  ...
  if (!scoped_decoded_image.decoded_image().is_budgeted()) {
    // If we have exceeded the budget, purge any locked decodes.
    Purge();
...
void CanvasResourceProvider::CanvasImageProvider::Purge() {
...
void CanvasResourceProvider::PurgeImageCache() {
  if (canvas_image_provider_)
    canvas_image_provider_->Purge();
}
