...
TEST_F(PaintArtifactCompositorTestWithPropertyTrees, OneScrollNode) {
  ...
  EXPECT_EQ(expected_compositor_element_id, ScrollLayerAt(0)->element_id());
...
TEST_F(PaintArtifactCompositorTestWithPropertyTrees, NestedScrollNodes) {
  ...
  const cc::ScrollTree& scroll_tree = GetPropertyTrees().scroll_tree;
  // Node #0 reserved for null; #1 for root render surface.
  // TODO(pdr): Should this be 4? If so, EXPECT_EQ scroll_tree.Node(3) too.
  ...
  EXPECT_EQ(expected_compositor_element_id_a, ScrollLayerAt(0)->element_id());
...
TEST_F(PaintArtifactCompositorTestWithPropertyTrees, ScrollLayerOrder) {
  ...
  EXPECT_LT(LayerIndex(ScrollLayerAt(0)), LayerIndex(ContentLayerAt(0)));
  const cc::ScrollTree& scroll_tree = GetPropertyTrees().scroll_tree;
  auto* scroll_node = scroll_tree.Node(ScrollLayerAt(0)->scroll_tree_index());
  ASSERT_EQ(scroll_element_id, scroll_node->element_id);

  // The second content layer should appear after the first.
  EXPECT_LT(LayerIndex(ScrollLayerAt(0)), LayerIndex(ContentLayerAt(1)));
}
...
TEST_F(PaintArtifactCompositorTestWithPropertyTrees, NestedScrollLayerOrder) {
  ...
  RefPtr<TransformPaintPropertyNode> scroll_translation_2 =
      TransformPaintPropertyNode::CreateScrollTranslation(
          ...
          scroll_2_element_id, ScrollPaintPropertyNode::Root(), IntSize(50, 50),
  ...
  auto* clip_1_node = clip_tree.Node(ScrollLayerAt(0)->clip_tree_index());
  ...
  auto* clip_2_node = clip_tree.Node(ScrollLayerAt(1)->clip_tree_index());
  EXPECT_EQ(gfx::RectF(0, 0, 50, 50), clip_2_node->clip);

  // The first layer should be before the second scroll layer.
  EXPECT_LT(LayerIndex(ScrollLayerAt(0)), LayerIndex(ScrollLayerAt(1)));

  // The content layer should be after the second scroll layer.
  EXPECT_LT(LayerIndex(ScrollLayerAt(1)), LayerIndex(ContentLayerAt(0)));
}
