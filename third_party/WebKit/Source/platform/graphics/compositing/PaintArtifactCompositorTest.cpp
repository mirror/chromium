...
class PaintArtifactCompositorTestWithPropertyTrees
 ...
 protected:
  ...
  void SetUp() override {
    ...
    web_layer_tree_view_->SetRootLayer(*paint_artifact_compositor_->GetWebLayer());
  }
...
TEST_F(PaintArtifactCompositorTestWithPropertyTrees, SortingContextID) {
  ...
  EXPECT_THAT(white_layer->GetPicture(),
              Pointee(DrawsRectangle(FloatRect(0, 0, 300, 200), Color::kWhite)));
  ...
  EXPECT_THAT(black_layer->GetPicture(),
              Pointee(DrawsRectangle(FloatRect(0, 0, 300, 200), Color::kBlack)));
...
TEST_F(PaintArtifactCompositorTestWithPropertyTrees, DeeplyNestedClips) {
  ...
  EXPECT_THAT(drawing_layer->GetPicture(),
...
TEST_F(PaintArtifactCompositorTestWithPropertyTrees, OneScrollNode) {
  ...
  CompositorElementId expected_compositor_element_id = CompositorElementId(2, 0);
  ...
  auto scroll_node_index =
      GetPropertyTrees().scroll_tree.FindNodeIndexFromOwningLayerId(layer->id());
...
TEST_F(PaintArtifactCompositorTestWithPropertyTrees, TransformUnderScrollNode) {
  ...
  EXPECT_EQ(scroll_transform_node.id, ContentLayerAt(0)->transform_tree_index());
  EXPECT_NE(scroll_transform_node.id, ContentLayerAt(1)->transform_tree_index());
}
...
TEST_F(PaintArtifactCompositorTestWithPropertyTrees, NestedScrollNodes) {
  ...
  CompositorElementId expected_compositor_element_id_a = CompositorElementId(2, 0);
  ...
  CompositorElementId expected_compositor_element_id_b = CompositorElementId(3, 0);
...
TEST_F(PaintArtifactCompositorTestWithPropertyTrees, CompositedLuminanceMask) {
  ...
  EXPECT_THAT(masking_layer->GetPicture(),
              Pointee(DrawsRectangle(FloatRect(0, 0, 100, 100), Color::kWhite)));
  ...
  EXPECT_EQ(cc::FilterOperation::REFERENCE, masking_group->filters.at(0).type());
}
...
TEST_F(PaintArtifactCompositorTestWithPropertyTrees,
  ...
  EXPECT_TRUE(composited_element_ids.Contains(transform->GetCompositorElementId()));
  EXPECT_TRUE(composited_element_ids.Contains(effect->GetCompositorElementId()));
}
