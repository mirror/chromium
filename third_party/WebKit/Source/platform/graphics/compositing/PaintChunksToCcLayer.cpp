...
static void AppendDisplayItemToCcDisplayItemList(const DisplayItem& display_item,
...
scoped_refptr<cc::DisplayItemList> PaintChunksToCcLayer::Convert(
  ...
  for (const auto* paint_chunk : paint_chunks) {
    const PropertyTreeState* state = &paint_chunk->properties.property_tree_state;
    ...
    for (const auto& display_item : display_items.ItemsInPaintChunk(*paint_chunk))
