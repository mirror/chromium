...
class ConversionContext {
 public:
  ...
  // implmented by paired display items.
  ...
  // Initialization:
  //   The current state is initalized with the layer state, and both clip
  //   stack and effect stack are empty.
  //   current_clip = C0
  //   clip_stack = []
  //   current_effect = E0
  //   effect_stack = []
 ...
 private:
  ...
  // the effect stack. Also for each pushed effect, its parent effect's clip
  // stack will be switched to the output clip of the pushed effect.
  // INPUT:
  ...
  // Clip stack of the current effect.
  // The size of the stack is the number of clips that are pushed after the
  // effect became current. Note that this is a "restore stack", i.e. the top
  // element does not represent the current state, but the state prior to
  // applying the current clip.
  struct ClipEntry {
    const TransformPaintPropertyNode* transform;
    const ClipPaintPropertyNode* clip;
  };
  Vector<ClipEntry> clip_stack_;

  // Effect stack.
  // The size of the stack is the number of effects that are currently nested.
  // Again this is a "restore stack", i.e. the top element does not represent
  // the current state, but the state prior to applying the current effect.
  struct EffectEntry {
    const TransformPaintPropertyNode* transform;
    const ClipPaintPropertyNode* clip;
  ...
  Vector<EffectEntry> effect_stack_;

  cc::DisplayItemList& cc_list_;
};

ConversionContext::~ConversionContext() {
  for (size_t i = clip_stack_.size(); i--;)
    AppendRestore(cc_list_, 1);
  for (size_t i = effect_stack_.size(); i--;) {
    AppendRestore(cc_list_, 2);
    for (size_t j = effect_stack_[i].clip_stack.size(); j--;)
      AppendRestore(cc_list_, 1);
  }
}
...
void ConversionContext::SwitchToClip(const ClipPaintPropertyNode* target_clip) {
  ...
  const ClipPaintPropertyNode* lca_clip =
      GeometryMapper::LowestCommonAncestor(target_clip, current_clip_);
  while (current_clip_ != lca_clip) {
    DCHECK(clip_stack_.size())
        << "Error: Chunk has a clip that escaped its effect's clip.";
    if (!clip_stack_.size())
      break;
    ClipEntry& previous_state = clip_stack_.back();
  ...
  for (size_t i = pending_clips.size(); i--;) {
    ...
    clip_stack_.emplace_back(ClipEntry{current_transform_, current_clip_});
...
void ConversionContext::SwitchToEffect(
  ...
  const EffectPaintPropertyNode* lca_effect =
      GeometryMapper::LowestCommonAncestor(target_effect, current_effect_);
  while (current_effect_ != lca_effect) {
    DCHECK(effect_stack_.size()) << "Error: Chunk layerized into a layer with "
                                    "an effect that's too deep.";
    if (!effect_stack_.size())
      break;

    // Exiting an effect automatically exits all clips applied after it too.
    for (size_t i = clip_stack_.size(); i--;)
      AppendRestore(cc_list_, 1);
    AppendRestore(cc_list_, 2);

    EffectEntry& previous_state = effect_stack_.back();
    current_transform_ = previous_state.transform;
    current_clip_ = previous_state.clip;
    current_effect_ = previous_state.effect;
    clip_stack_.swap(previous_state.clip_stack);
  ...
  for (size_t i = pending_effects.size(); i--;) {
    ...
    effect_stack_
        .emplace_back(
            EffectEntry{current_transform_, current_clip_, current_effect_, {}})
        .clip_stack.swap(clip_stack_);
    current_transform_ = target_transform;
