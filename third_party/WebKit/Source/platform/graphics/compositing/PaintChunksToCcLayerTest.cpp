...
// Recognized ops: Concat, DrawRecord, Save, SaveLayer, Restore
...
struct TestChunks {
  ...
  Vector<const PaintChunk*> chunk_ptrs;
  ...
  void AddChunk(const TransformPaintPropertyNode* t,
    ...
    chunk_ptrs.push_back(&chunks.emplace_back(i, i + 1, DefaultId(),
...
TEST_F(PaintChunksToCcLayerTest, EffectGroupingSimple) {
  ...
  sk_sp<PaintRecord> output =
      PaintChunksToCcLayer::Convert(chunks.chunk_ptrs,
...
TEST_F(PaintChunksToCcLayerTest, NonRootLayerSimple) {
  ...
  sk_sp<PaintRecord> output =
      PaintChunksToCcLayer::Convert(
          chunks.chunk_ptrs, PropertyTreeState(t1.Get(), c1.Get(), e1.Get()),
          gfx::Vector2dF(), chunks.items)
          ->ReleaseAsRecord();
  EXPECT_THAT(output,
