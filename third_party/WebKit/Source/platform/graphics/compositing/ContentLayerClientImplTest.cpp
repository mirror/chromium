...
class ContentLayerClientImplTest : public ::testing::Test,
 ...
 protected:
  ...
  static const IntRect kDefaultLayerBounds;
};
...
TEST_F(ContentLayerClientImplTest, LayerBounds) {
  ...
  // No raster invalidaitions needed for a new layer.
  ...
  // No raster invalidaitions needed if layer origin doesn't change.
  EXPECT_TRUE(TrackedRasterInvalidations(c).IsEmpty());
...
TEST_F(ContentLayerClientImplTest, RasterInvalidationReorderChunks) {
  ...
  ExpectChunkInvalidation(invalidations, 3, *new_chunks[1],
                          PaintInvalidationReason::kChunkReordered);
  ExpectDisplayItemInvalidations(invalidations, 4, *new_chunks[2]);
}
...
TEST_F(ContentLayerClientImplTest, RasterInvalidationUncacheableChunks) {
  ...
  const auto& invalidations = TrackedRasterInvalidations(c);
  ASSERT_EQ(7u, invalidations.size());
  ExpectChunkInvalidation(invalidations, 0, *chunks[1],
                          PaintInvalidationReason::kChunkUncacheable);
  ExpectDisplayItemInvalidations(invalidations, 1, *new_chunks[0]);
  ExpectDisplayItemInvalidations(invalidations, 3, *new_chunks[1]);
  ExpectChunkInvalidation(invalidations, 6, *new_chunks[2],
                          PaintInvalidationReason::kChunkUncacheable);
}
