...
PaintArtifactCompositor::ScrollHitTestLayerScrollTranslation(
    const PaintArtifact& paint_artifact,
...
PaintArtifactCompositor::ScrollHitTestLayerForPendingLayer(
  ...
  const auto* scroll_offset_node =
      ScrollHitTestLayerScrollTranslation(paint_artifact, pending_layer);
...
void PaintArtifactCompositor::Update(
  ...
  for (const PendingLayer& pending_layer : pending_layers) {
    ...
    // The compositor scroll node is created using the associated scroll offset
    // translation node.
    ...
    if (layer->scrollable()) {
      ...
      // node.
      scroll_translation = transform->NearestScrollTranslationNode();
    }
    int scroll_id =
