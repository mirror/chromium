...
void PaintArtifactCompositor::SetTracksRasterInvalidations(bool value) {
  for (auto& client : content_layer_clients_)
    client->SetTracksRasterInvalidations(value);
}
...
PaintArtifactCompositor::CompositedLayerForPendingLayer(
  ...
  return new_content_layer_clients.back()->UpdateCcPictureLayer(
      paint_artifact, pending_layer.bounds, pending_layer.paint_chunks,
      layer_offset, store_debug_info);
}
