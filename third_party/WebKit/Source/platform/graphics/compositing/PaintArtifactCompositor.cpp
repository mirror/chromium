...
PaintArtifactCompositor::CompositedLayerForPendingLayer(
  ...
  // TODO(pdr): Break this out into a helper.
  if (first_paint_chunk.size() == 1) {
    ...
    if (display_item.IsScrollHitTest()) {
      const auto* scroll_translation =
          pending_layer.property_tree_state.Transform();
      DCHECK(scroll_translation->IsScrollTranslation());
      auto scroll_element_id = scroll_translation->GetCompositorElementId();
      for (auto& scroll_hit_test_layer : scroll_hit_test_layers_) {
        if (scroll_hit_test_layer &&
            scroll_hit_test_layer->element_id() == scroll_element_id) {
          new_scroll_hit_test_layers.push_back(scroll_hit_test_layer);
          if (extra_data_for_testing_enabled_) {
            extra_data_for_testing_->scroll_hit_test_layers.push_back(
                scroll_hit_test_layer);
          }
          scroll_hit_test_layer->SetScrollable(
              scroll_translation->ScrollNode()->ContainerBounds());
          scroll_hit_test_layer->SetBounds(
              scroll_translation->ScrollNode()->Bounds());
          // TODO(pdr): Should the offset_to_transform_parent always be zero?
          if (auto* scroll_client =
                  scroll_translation->ScrollNode()->ScrollClient()) {
            scroll_hit_test_layer->set_did_scroll_callback(
                base::Bind(&blink::WebLayerScrollClient::DidScroll,
                           base::Unretained(scroll_client)));
          }
          return scroll_hit_test_layer;
        }
      }
      auto scroll_hit_test_layer = cc::Layer::Create();
      new_scroll_hit_test_layers.push_back(scroll_hit_test_layer);
      if (extra_data_for_testing_enabled_) {
        extra_data_for_testing_->scroll_hit_test_layers.push_back(
            scroll_hit_test_layer);
  ...
  while (chunk_it != paint_artifact.PaintChunks().end()) {
    ...
    if (chunk_effect == &current_group) {
      ...
      bool requires_own_layer = last_display_item.IsForeignLayer() ||
                                // TODO(pdr): Should this require a direct
                                // compositing reason?
...
void PaintArtifactCompositor::Update(
  ...
  for (const PendingLayer& pending_layer : pending_layers) {
    ...
    if (element_id) {
      ...
      // TODO(pdr): This will still need to be set on scroll layers.
