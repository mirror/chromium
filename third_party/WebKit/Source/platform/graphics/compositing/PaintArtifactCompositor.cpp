...
class SynthesizedClip : private cc::ContentLayerClient {
 public:
  ...
  void Update(const FloatRoundedRect& rrect,
    ...
    if (old_local_rrect != new_local_rrect || path_in_layer_changed) {
 ...
 private:
  ...
  scoped_refptr<cc::DisplayItemList> PaintContentsToDisplayList(
    ...
    cc_list->StartPaint();
    if (!path_) {
      SkRRect local_rrect = rrect_;
      local_rrect.offset(-layer_origin_.x(), -layer_origin_.y());
      cc_list->push<cc::DrawRRectOp>(local_rrect, flags);
    } else {
