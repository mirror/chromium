...
class SynthesizedClip : private cc::ContentLayerClient {
 public:
  ...
  void Update(const FloatRoundedRect& rrect,
    ...
    bool path_in_layer_is_unchanged = false;
    if (!path_ && !path) {
      path_in_layer_is_unchanged = true;
    } else if (path_ == path && layer_origin_ == new_layer_origin) {
      path_in_layer_is_unchanged = true;
    } else if (path_ && path && path_ != path &&
      ...
      path_in_layer_is_unchanged = (path_->GetSkPath() == new_path);
    }

    if (old_local_rrect != new_local_rrect || !path_in_layer_is_unchanged) {
