    ...
    if (display_item.IsScrollLayer()) {
      ...
      for (auto& scroll_layer : scroll_layers_) {
        if (scroll_layer && scroll_layer->element_id() == scroll_element_id) {
          new_scroll_layers.push_back(scroll_layer);
          if (extra_data_for_testing_enabled_)
            extra_data_for_testing_->scroll_layers.push_back(scroll_layer);
  ...
  while (chunk_it != paint_artifact.PaintChunks().end()) {
    ...
    if (chunk_effect == &current_group) {
      ...
      bool requires_own_layer = last_display_item.IsForeignLayer() ||
                                ...
                                // compositing reason?
                                last_display_item.IsScrollLayer();
...
void PaintArtifactCompositor::Update(
  ...
  for (const PendingLayer& pending_layer : pending_layers) {
    ...
    scoped_refptr<cc::Layer> layer = CompositedLayerForPendingLayer(
        ...
        new_scroll_layers, store_debug_info);
