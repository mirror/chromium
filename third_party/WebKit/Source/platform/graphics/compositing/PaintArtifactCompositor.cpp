...
PaintArtifactCompositor::ClientForPaintChunk(
  ...
  return WTF::WrapUnique(new ContentLayerClientImpl(
      paint_chunk.id
          ? *paint_chunk.id
          : paint_artifact.GetDisplayItemList()[paint_chunk.begin_index].GetId()));
}
...
PaintArtifactCompositor::CompositedLayerForPendingLayer(
  ...
  scoped_refptr<cc::DisplayItemList> display_list =
      PaintChunksToCcLayer::Convert(pending_layer.paint_chunks,
                                    pending_layer.property_tree_state, layer_offset,
                                    paint_artifact.GetDisplayItemList());
  content_layer_client->SetDisplayList(std::move(display_list));
  content_layer_client->SetPaintableRegion(gfx::Rect(cc_combined_bounds.size()));
  ...
  for (const auto& paint_chunk : pending_layer.paint_chunks) {
    ...
    for (unsigned index = 0; index < paint_chunk->raster_invalidation_rects.size();
         ++index) {
      IntRect rect(
      ...
      content_layer_client->SetNeedsDisplayRect(
          ...
          raster_tracking ? &raster_tracking->tracked_raster_invalidations[index]
                         : nullptr);
    }
  }
...
void PaintArtifactCompositor::LayerizeGroup(
  ...
  while (chunk_it != paint_artifact.PaintChunks().end()) {
    ...
    if (chunk_effect == &current_group) {
      ...
      bool is_foreign = paint_artifact.GetDisplayItemList()[chunk_it->begin_index]
                           .IsForeignLayer();
...
void PaintArtifactCompositor::Update(
  ...
  root_layer_->set_property_tree_sequence_number(g_s_property_tree_sequence_number);
  ...
  Vector<std::unique_ptr<ContentLayerClientImpl>> new_content_layer_clients;
  new_content_layer_clients.ReserveCapacity(paint_artifact.PaintChunks().size());
