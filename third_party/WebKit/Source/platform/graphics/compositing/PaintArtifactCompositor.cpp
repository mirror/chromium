...
class SynthesizedClip : private cc::ContentLayerClient {
 public:
  SynthesizedClip() : layer_(cc::PictureLayer::Create(this)) {
    ...
    ids_[0] = CompositorElementIdFromSyntheticEffectId(++serial);
    ids_[1] = CompositorElementIdFromSyntheticEffectId(++serial);
  ...
  void GetCompositorElementIds(CompositorElementId (&ids)[2]) const {
    ids[0] = ids_[0];
    ids[1] = ids_[1];
  }
 ...
 private:
  ...
  SkRRect rrect_;
  CompositorElementId ids_[2];
};

cc::Layer* PaintArtifactCompositor::SynthesizeClip(
  ...
  if (entry == synthesized_clip_cache_.end()) {
    entry = synthesized_clip_cache_.insert(
        entry, SynthesizedClipEntry{
                   .key = node,
                   .synthesized_clip = std::make_unique<SynthesizedClip>(),
                   .in_use = false});
  }
  ...
  synthesized_clip.GetCompositorElementIds(ids);
