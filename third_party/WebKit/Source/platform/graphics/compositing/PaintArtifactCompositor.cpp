...
std::unique_ptr<JSONObject> PaintArtifactCompositor::LayersAsJSON(
  ...
  for (size_t i = 0; i < content_layer_clients_.size(); ++i) {
    auto layer_json = content_layer_clients_[i]->LayerAsJSON(context);
    if (i == 0) {
      // Don't output white background for the root layer to keep consistent
      // with SPv1 layer JSON dump.
      String background_color;
      if (layer_json->GetString("backgroundColor", &background_color) &&
          background_color == "#FFFFFF")
        layer_json->Remove("backgroundColor");
    }
    layers_json->PushObject(std::move(layer_json));
  }
