...
PaintArtifactCompositor::ScrollHitTestLayerForPaintChunk(
  ...
  const auto& scroll_node =
      *scroll_hit_test_display_item.scroll_offset_node().ScrollNode();
...
PaintArtifactCompositor::CompositedLayerForPendingLayer(
  ...
  // If the paint chunk is a scroll hit test layer, lookup/create the layer and
  // return it.
...
void PaintArtifactCompositor::Update(
  ...
  for (const PendingLayer& pending_layer : pending_layers) {
    ...
    int scroll_id = 0;
    // TODO(pdr): Break this into a helper.
    if (layer->scrollable()) {
      const PaintChunk& first_paint_chunk = *pending_layer.paint_chunks[0];
      const auto& display_item =
      ...
      property_tree_manager.EnsureCompositorTransformNode(scroll_offset_node);
      const auto* scroll_node = scroll_offset_node->FindEnclosingScrollNode();
      scroll_id = property_tree_manager.CompositorScrollNode(scroll_node);
    } else {
      // TODO(pdr): This could be a performance issue because it crawls up the
      // transform tree for each pending layer. If this is on profiles, we
      // should cache a lookup of transform node to scroll translation transform
      // node.
      const auto* scroll_node = transform->FindEnclosingScrollNode();
      scroll_id = property_tree_manager.CompositorScrollNode(scroll_node);
    }
    int clip_id =
    ...
    // ScrollHitTestLayerForPaintChunk.
