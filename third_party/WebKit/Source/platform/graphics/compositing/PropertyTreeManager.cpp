...
void PropertyTreeManager::SetupRootTransformNode() {
  ...
  transform_node_map_.Set(TransformPaintPropertyNode::Root(), transform_node.id);
...
void PropertyTreeManager::SetupRootEffectNode() {
  ...
  effect_tree.SetOwningLayerIdForNode(&effect_node, effect_node.owning_layer_id);
...
void PropertyTreeManager::SetupRootScrollNode() {
  ...
  scroll_tree.SetOwningLayerIdForNode(&scroll_node, scroll_node.owning_layer_id);
...
int PropertyTreeManager::EnsureCompositorScrollNode(
  ...
  compositor_node.scroll_clip_layer_bounds.SetSize(scroll_node->Clip().Width(),
                                                  scroll_node->Clip().Height());
...
void PropertyTreeManager::UpdateScrollAndScrollTranslationNodes(
  ...
  int scroll_node_id = EnsureCompositorScrollNode(scroll_offset_node->ScrollNode());
  ...
  auto scroll_offset =
      gfx::ScrollOffset(-scroll_offset_size.Width(), -scroll_offset_size.Height());
...
void PropertyTreeManager::BuildEffectNodesRecursively(
  ...
  GetEffectTree().SetOwningLayerIdForNode(&effect_node, effect_node.owning_layer_id);
