...
int PropertyTreeManager::SwitchToEffectNodeWithSynthesizedClip(
  ...
  // the following order:
  // Prior to emitting P0, this is invoked with (E1, C0). A compositor effect
  // node for E1 is generated as we are entering it. The caller emits P0.
  // Prior to emitting P1, this is invoked with (E1, C1). A synthetic compositor
  // effect for C1 is generated as we are entering it. The caller emits P1.
  // Prior to emitting P2, this is invoked with (E0, C1). Both previously
  ...
  // is generated along with an internal effect node for blending. After
  // closing both effects, C1 has to be entered again, thus generates another
  // synthetic compositor effect. The caller emits P2.
