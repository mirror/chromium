...
bool SharedGpuContext::IsGpuCompositingEnabled() {
  ...
  return this_ptr->is_gpu_compositing_enabled_;
}
...
static void CreateContextProviderOnMainThread(
  ...
  *using_gpu_compositing = Platform::Current()->IsGpuCompositingEnabled();
  if (!*using_gpu_compositing && only_if_gpu_compositing) {
...
void SharedGpuContext::CreateContextProviderIfNeeded(
    bool only_if_gpu_compositing) {
  // TODO(danakj): This needs to thread check the context is being used on the
  // thread it was made on, or else lock it.
  if (context_provider_wrapper_ &&
      context_provider_wrapper_->ContextProvider()
              ->ContextGL()
              ->GetGraphicsResetStatusKHR() == GL_NO_ERROR) {
    // If the context isn't lost then |is_gpu_compositing_enabled_| state hasn't
    // changed yet, since RenderThreadImpl::CompositingModeFallbackToSoftware()
    // will lose the context to let us know if it changes.
    return;
  }

  is_gpu_compositing_enabled_ = false;
  context_provider_wrapper_ = nullptr;

  if (context_provider_factory_) {
    // This path should only be used in unit tests.
    auto context_provider =
        context_provider_factory_.Run(&is_gpu_compositing_enabled_);
    if (context_provider) {
    ...
    is_gpu_compositing_enabled_ =
        Platform::Current()->IsGpuCompositingEnabled();
    if (!is_gpu_compositing_enabled_ && only_if_gpu_compositing)
      return;
    auto context_provider =
    ...
    task_runner->PostTask(
        ...
        CrossThreadBind(&CreateContextProviderOnMainThread,
                        ...
                        CrossThreadUnretained(&is_gpu_compositing_enabled_),
...
void SharedGpuContext::ResetForTesting() {
  SharedGpuContext* this_ptr = GetInstanceForCurrentThread();
  this_ptr->is_gpu_compositing_enabled_ = false;
