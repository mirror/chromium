...
bool ImageLayerBridge::PrepareTextureMailbox(
  ...
  scoped_refptr<StaticBitmapImage> image_for_compositor = image_;

  // Upload to a texture if the compositor is expecting one.
  if (gpu_compositing && !image_for_compositor->IsTextureBacked()) {
    ...
    // TODO(junov): This can return a SkColorSpace, which should be passed to
    // the compositor.
