...
TEST_F(MailboxSharedGpuContextTest, MailboxCaching) {
  ...
  GLuint64 fence = 0x12345678;

  EXPECT_CALL(gl_, GenMailboxCHROMIUM(mailbox.name))
      .Times(1)
      .WillOnce(testing::Invoke(&mailboxGenerator,
                                &FakeMailboxGenerator::GenMailbox));
  EXPECT_CALL(gl_, InsertFenceSyncCHROMIUM())
      .Times(1)
      .WillOnce(testing::Return(fence));
  EXPECT_CALL(gl_, GenUnverifiedSyncTokenCHROMIUM(fence, sync_token.GetData()))
      .Times(1);

  SharedGpuContext::ContextProviderWrapper()->Utils()->GetMailboxForSkImage(
      mailbox, sync_token, image->PaintImageForCurrentFrame().GetSkImage());
