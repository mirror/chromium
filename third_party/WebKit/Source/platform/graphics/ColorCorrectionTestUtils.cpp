...
static uint8_t SRGBColorCorrectionTolerance(
  ...
  if (!alpha)
    return 255;
  return (alpha_multiplied == kAlphaMultiplied)
             ? 1 + std::abs(color -
                            std::floor(std::floor(color * 255.0 / alpha) *
                                       alpha / 255.0))
             : 1 + std::abs(color -
                            std::floor(std::floor(color * alpha / 255.0) *
                                       255.0 / alpha));
}
...
void ColorCorrectionTestUtils::CompareColorCorrectedPixels(
    ...
    int bytes_per_pixel,
    PixelsAlphaMultiply alpha_multiplied,
    SRGBAlphaDispositionRounding alpha_rounding) {
  bool test_passed = true;
  float wide_gamut_color_correction_tolerance = 0.01;
  switch (bytes_per_pixel) {
    case 4: {
      if (alpha_rounding == kSRGBAlphaDispositionRounding) {
