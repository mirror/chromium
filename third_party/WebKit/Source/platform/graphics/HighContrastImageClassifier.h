...
class PLATFORM_EXPORT HighContrastImageClassifier {
 ...
 private:
  ...
  // Receives sampled pixels and and color mode, and returns the number of color
  // buckets.
  float ComputeColorBucketsRatio(const std::vector<SkColor>&, const ColorMode);

  // Gets a specified number of samples from a block of the image, and returns
  // the samples and the number of transparent pixels.
  int GetBlockSamples(const SkBitmap&,
                      const int,
                      const int,
                      const int,
                      int,
