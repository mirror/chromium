// Copyright 2017 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#include "platform/bindings/CallbackInterfaceBase.h"

#include "platform/bindings/CallbackInterfaceCollection.h"

namespace blink {

DEFINE_TRACE_WRAPPERS(CallbackInterfaceBase) {
  visitor->TraceWrappers(callback_object_);
}

CallbackInterfaceBase::CallbackInterfaceBase(
    v8::Local<v8::Object> callback_object, SingleOperationOrNot single_op_or_not)
    : callback_object_(this) {
  callback_relevant_context_ =
      ScriptState::From(callback_object->CreationContext());
  v8::Isolate* isolate = callback_relevant_context_->GetIsolate();

  callback_object_.Set(isolate, callback_object);
  callback_object_is_callable_ = single_op_or_not == kSingleOperation &&
                                 callback_object->IsCallable();
  incumbent_context_ = ScriptState::From(isolate->GetIncumbentContext());
}

}  // namespace blink
