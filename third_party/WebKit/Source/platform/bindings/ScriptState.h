...
class PLATFORM_EXPORT ScriptState : public RefCounted<ScriptState> {
 ...
 private:
  ...
  // This scoped_refptr doesn't cause a cycle because all persistent handles
  // that DOMWrapperWorld holds are weak.
  scoped_refptr<DOMWrapperWorld> world_;

  // This std::unique_ptr causes a cycle:
  // V8PerContextData --(Persistent)--> v8::Context --(scoped_refptr)-->
  // ScriptState
