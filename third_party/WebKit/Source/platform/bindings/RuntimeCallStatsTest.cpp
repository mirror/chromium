...
TEST_F(RuntimeCallStatsTest, CountAndTimeAreUpdatedAfterMultipleExecutions) {
  ...
  const unsigned funcDuration = 20;
  const unsigned loops = 5;

  auto func = [&counter, &stats]() {
    RuntimeCallTimer timer;
    RuntimeCallStats::Enter(&stats, &timer, &counter);
    Sleep(funcDuration);
    RuntimeCallStats::Leave(&stats, &timer);
  ...
  EXPECT_EQ(loops * funcDuration, counter.GetTime().InMilliseconds());
}
...
TEST_F(RuntimeCallStatsTest, NestedTimersTest) {
  ...
  const unsigned innerFuncDuration = 50;
  const unsigned outerFuncDuration = 20;

  auto inner_func = [&stats, &inner_counter] {
    RuntimeCallTimer timer;
    RuntimeCallStats::Enter(&stats, &timer, &inner_counter);
    Sleep(innerFuncDuration);
    RuntimeCallStats::Leave(&stats, &timer);
  };
  ...
  EXPECT_EQ(outerFuncDuration, outer_counter.GetTime().InMilliseconds());
  EXPECT_EQ(innerFuncDuration, inner_counter.GetTime().InMilliseconds());
}
