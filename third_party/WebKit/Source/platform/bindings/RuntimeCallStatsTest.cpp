...
TEST_F(RuntimeCallStatsTest, TimeTicksSanityCheck) {
  AdvanceClock(50);
  EXPECT_EQ(51.0, TimeTicks::Now().InSeconds() * 1000);
}
...
TEST_F(RuntimeCallStatsTest, StatsCounterNameIsCorrect) {
  ...
  EXPECT_EQ(0, strcmp("TestCounter1", stats.TestCounter1.GetName()));
}

TEST_F(RuntimeCallStatsTest, CountIsUpdatedAfterLeave) {
  RuntimeCallTimer timer;
  RuntimeCallStats stats;
  RuntimeCallCounter* counter = &stats.TestCounter1;

  stats.Enter(&timer, &RuntimeCallStats::TestCounter1);
  EXPECT_EQ(0ul, counter->GetCount());
  stats.Leave(&timer);
...
TEST_F(RuntimeCallStatsTest, NestedTimersTest) {
  RuntimeCallStats stats;
  RuntimeCallCounter* outer_counter = &stats.TestCounter1;
  RuntimeCallCounter* inner_counter = &stats.TestCounter2;
  ...
  auto inner_func = [&stats, inner_func_duration]() {
    RuntimeCallTimer timer;
    stats.Enter(&timer, &RuntimeCallStats::TestCounter2);
...
TEST_F(RuntimeCallStatsTest, RuntimeCallTimerScopeTest) {
  ...
  auto func = [&stats]() {
    RuntimeCallTimerScope scope(&stats, &RuntimeCallStats::TestCounter1);
    AdvanceClock(50);
  };
...
TEST_F(RuntimeCallStatsTest, ReuseTimer) {
  RuntimeCallStats stats;
  RuntimeCallTimer timer;
  RuntimeCallCounter* counter1 = &stats.TestCounter1;
  RuntimeCallCounter* counter2 = &stats.TestCounter2;
...
TEST_F(RuntimeCallStatsTest, ResetCallStats) {
  ...
  {
    RuntimeCallTimerScope scope1(&stats, &RuntimeCallStats::TestCounter1);
    { RuntimeCallTimerScope scope2(&stats, &RuntimeCallStats::TestCounter2); }
  }
