...
class PLATFORM_EXPORT ScriptWrappableVisitor {
 ...
 private:
  ...
  static const char* NameForHeapSnapshot(const TraceWrapperBase* traceable) {
    return traceable->NameForHeapSnapshot();
  }

  static const char* NameForHeapSnapshot(...) { return "InternalNode"; }

  template <typename T>
  static const char* NameCallback(const void* traceable) {
    return NameForHeapSnapshot(static_cast<const T*>(traceable));
  }
