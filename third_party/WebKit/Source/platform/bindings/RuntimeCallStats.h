...
#define RUNTIME_CALL_STATS_ENTER(runtime_call_stats, timer, counterId) \
  if (RuntimeCallStats::s_runtime_call_stats_enabled_) {               \
    (runtime_call_stats)->Enter(timer, counterId);                     \
  }

#define RUNTIME_CALL_STATS_LEAVE(runtime_call_stats, timer) \
  if (RuntimeCallStats::s_runtime_call_stats_enabled_) {    \
    (runtime_call_stats)->Leave(timer);                     \
  }

#define RUNTIME_CALL_TIMER_SCOPE(scope_name, runtime_call_stats, counterId) \
  Optional<RuntimeCallTimerScope> scope_name;                               \
  if (RuntimeCallStats::s_runtime_call_stats_enabled_) {                    \
    scope_name.emplace(runtime_call_stats, counterId);                      \
  }

#define RUNTIME_CALL_TIMER_SCOPE_WITH_OPTIONAL_PROVIDED(        \
...
class PLATFORM_EXPORT RuntimeCallStats {
 public:
  ...
  static void EnableRuntimeCallStats() { s_runtime_call_stats_enabled_ = true; }
  static void DisableRuntimeCallStats() {
    s_runtime_call_stats_enabled_ = false;
  }
...
#define FOR_EACH_COUNTER(V)                                             \
  ...
  V(UpdateLayout)                                                       \
  V(UpdateStyle)                                                        \
  V(V8SetReturnValueForMainWorldScriptWrappable)                        \
  V(V8SetReturnValueString)                                             \
  V(V8StringStringView)                                                 \
...
#undef ADD_ENUM_VALUE
  ...
  static bool s_runtime_call_stats_enabled_;
