...
#define FOR_EACH_COUNTER(V) \
  ...
  enum struct CounterId : uint16_t {
...
#undef ADD_ENUM_VALUE
  ...
  void Enter(RuntimeCallTimer* timer, CounterId id) {
    timer->Start(&this->counters_[static_cast<uint16_t>(id)], current_timer_);
 ...
 private:
  ...
  RuntimeCallCounter counters_[static_cast<int>(CounterId::kSize)];
  static const int number_of_counters_ = static_cast<int>(CounterId::kSize);
};
