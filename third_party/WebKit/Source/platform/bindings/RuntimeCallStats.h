...
class PLATFORM_EXPORT RuntimeCallCounter {
 public:
  ...
  void AddTime(TimeDelta time) { time_ += time; }
...
class PLATFORM_EXPORT RuntimeCallStats {
 public:
  ...
  static void Enter(RuntimeCallStats* stats,
  ...
  static void Leave(RuntimeCallStats* stats, RuntimeCallTimer* timer) {
    DCHECK_EQ(timer, stats->current_timer_);
...
class PLATFORM_EXPORT RuntimeCallTimerScope {
 ...
 public:
  RuntimeCallTimerScope(RuntimeCallStats* stats, RuntimeCallCounter* counter)
      : call_stats_(stats) {
    RuntimeCallStats::Enter(call_stats_, &timer_, counter);
  }
  ~RuntimeCallTimerScope() { RuntimeCallStats::Leave(call_stats_, &timer_); }
