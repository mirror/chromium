...
TEST(CreateAccessControlPreflightRequestTest, LexicographicalOrder) {
  ...
  WebURLRequest preflight =
      WebCrossOriginAccessControl::CreateAccessControlPreflightRequest(request);
...
TEST(ParseAccessControlExposeHeadersAllowListTest, ValidInput) {
  WebCrossOriginAccessControl::HTTPHeaderSet set;
  WebCrossOriginAccessControl::ParseAccessControlExposeHeadersAllowList("valid",
                                                                        set);
  EXPECT_EQ(1U, set.size());
  EXPECT_TRUE(set.Contains("valid"));

  set.clear();
  WebCrossOriginAccessControl::ParseAccessControlExposeHeadersAllowList("a,b",
                                                                        set);
...
TEST(ParseAccessControlExposeHeadersAllowListTest, DuplicatedEntries) {
  WebCrossOriginAccessControl::HTTPHeaderSet set;
  WebCrossOriginAccessControl::ParseAccessControlExposeHeadersAllowList("a, a",
                                                                        set);
  EXPECT_EQ(1U, set.size());
  EXPECT_TRUE(set.Contains("a"));
...
TEST(ParseAccessControlExposeHeadersAllowListTest, InvalidInput) {
  ...
  set.clear();
  WebCrossOriginAccessControl::ParseAccessControlExposeHeadersAllowList("/a/",
                                                                        set);
  EXPECT_TRUE(set.IsEmpty());
