...
WebClipboardImpl::WebClipboardImpl() : clipboard_() {
...
WebVector<WebString> WebClipboardImpl::ReadAvailableTypes(
  ...
  std::transform(types.begin(), types.end(), web_types.begin(),
...
WebBlobInfo WebClipboardImpl::ReadImage(mojom::ClipboardBuffer buffer) {
  ...
  if (size < 0)
    return WebBlobInfo();
  return WebBlobInfo(WebString(blob_uuid), WebString(type), size);
}
...
void WebClipboardImpl::WriteImage(const WebImage& image,
  ...
  if (url.IsValid() && !url.IsEmpty()) {
    ...
    clipboard_->WriteHtml(
        ...
        WTF::String(),  // base::UTF8ToUTF16(URLToImageMarkup(url, title)),
        KURL());
#endif
  }
  clipboard_->CommitWrite(mojom::ClipboardBuffer::kStandard);
...
void WebClipboardImpl::WriteDataObject(const WebDragData& data) {
  ...
  // // Avoid calling the WriteFoo functions if there is no data associated with
  // a
  // // type. This prevents stomping on clipboard contents that might have been
  // // written by extension functions such as
  // chrome.bookmarkManagerPrivate.copy. if (!data_object.text.is_null())
