...
class HTTPHeaderNameListParser {
 ...
 private:
  ...
  // TODO(hintzed): Copied from net/http/http_util.cc because presubmit
  // complains about missing checkdeps rule - can we create one?
...
AccessStatus CheckAccess(
  ...
  if (allow_origin_header_value == "*") {
    ...
    if (allow_origin_header_value.Ascii().find_first_of(" \t\n\v\f\r,") !=
        std::string::npos) {
    ...
    case AccessStatus::kMissingAllowOriginHeader: {
      ...
      if (IsInterestingStatusCode(status_code)) {
        return String::Format(
            ...
            "requested resource. %s The response had HTTP status code %d.%s",
            origin_denied.Utf8().data(), status_code,
            context == WebURLRequest::kRequestContextFetch
  ...
  switch (status) {
    case kRedirectDisallowedScheme: {
      return String::Format(
          ...
          "requests.",
          WTF::String(redirect_url.GetString()).Utf8().data());
    }
    case kRedirectContainsCredentials: {
      return String::Format(
          "Redirect location '%s' contains a username and password, which "
          "is disallowed for "
