...
namespace WebCrossOriginAccessControl {
...
AccessStatus CheckAccess(const WebURLResponse& response,
...
WebString AccessControlErrorString(
  ...
  DEFINE_THREAD_SAFE_STATIC_LOCAL(WebString, allow_origin_header_name,
                                  ("access-control-allow-origin"));
  DEFINE_THREAD_SAFE_STATIC_LOCAL(WebString, allow_credentials_header_name,
                                  ("access-control-allow-credentials"));
  DEFINE_THREAD_SAFE_STATIC_LOCAL(WebString, allow_suborigin_header_name,
                                  ("access-control-allow-suborigin"));
  ...
  switch (status) {
    ...
    case AccessStatus::kSubOriginMismatch: {
      return String::Format(
          ...
          response.HttpHeaderField(allow_suborigin_header_name).Utf8().data(),
          origin_denied.Utf8().data());
    }
    ...
    case AccessStatus::kMultipleAllowOriginValues: {
      return String::Format(
          ...
          response.HttpHeaderField(allow_origin_header_name).Utf8().data(),
    ...
    case AccessStatus::kDisallowCredentialsNotSetToTrue: {
      return String::Format(
          ...
          "credentials mode is 'include'. %s%s",
          response.HttpHeaderField(allow_credentials_header_name).Utf8().data(),
          origin_denied.Utf8().data(),
          (context == WebURLRequest::kRequestContextXMLHttpRequest
               ? " The credentials mode of requests initiated by the "
