 ...
 public:
  ...
  bool TimeTillNextDelayedTask(double* time) const {
    ...
    if (!platform_->GetRendererScheduler()
      ...
      return false;
    *time = (next_run_time -
             platform_->GetRendererScheduler()
                 ->TimerTaskRunner()
                 ->GetTimeDomain()
                 ->Now())
                .InSecondsF();
...
TEST_F(TimerTest, UserSuppliedWebTaskRunner) {
  ...
  TimerForTest<TimerTest> timer(web_task_runner, this, &TimerTest::CountingTask);
  timer.StartOneShot(0, BLINK_FROM_HERE);
...
TEST_F(TimerTest, DestructOnHeapTimer) {
  ...
  ThreadState::Current()->CollectGarbage(
      BlinkGC::kNoHeapPointersOnStack, BlinkGC::kGCWithSweep, BlinkGC::kForcedGC);
...
TEST_F(TimerTest, MoveToNewTaskRunnerOneShot) {
  ...
  TimerForTest<TimerTest> timer(web_task_runner1, this, &TimerTest::CountingTask);
