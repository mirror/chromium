...
#include "platform/wtf/BaseValueConverter.h"
...
class PLATFORM_EXPORT BlinkResourceCoordinatorBase {
 ...
 public:
  ...
  void SetProperty(
      ...
      T&& value) {
    service_->SetProperty(
        property_type,
        base::MakeUnique<base::Value>(
            WTF::BaseValueConverter<T>::Convert(std::forward<T>(value))));
  }

 protected:
  explicit BlinkResourceCoordinatorBase(service_manager::InterfaceProvider*);
  explicit BlinkResourceCoordinatorBase(service_manager::Connector*,
                                        const std::string& name);

 private:
  void SetServiceConnectionErrorHandler();
