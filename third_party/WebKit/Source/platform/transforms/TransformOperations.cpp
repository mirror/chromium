...
TransformOperations TransformOperations::BlendByMatchingOperations(
  ...
  for (unsigned i = 0; i < size; i++) {
    ...
    RefPtr<TransformOperation> blended_operation =
        to_operation ? to_operation->Blend(from_operation.Get(), progress)
...
static void BoundingBoxForArc(const FloatPoint3D& point,
  ...
  from_matrix.Rotate3d(from_transform.X(), from_transform.Y(), from_transform.Z(),
                      from_degrees);
  ...
  switch (from_transform.GetType()) {
    case TransformOperation::kRotateX:
      FindCandidatesInPlane(point.Y(), point.Z(), from_transform.X(), candidates,
                            &num_candidates);
      break;
    case TransformOperation::kRotateY:
      FindCandidatesInPlane(point.Z(), point.X(), from_transform.Y(), candidates,
                            &num_candidates);
      break;
    case TransformOperation::kRotateZ:
      FindCandidatesInPlane(point.X(), point.Y(), from_transform.Z(), candidates,
                            &num_candidates);
      break;
    default: {
  ...
  for (int i = size - 1; i >= 0; i--) {
    ...
    switch (interpolation_type) {
      ...
      case TransformOperation::kPerspective: {
        ...
        if (!to_operation) {
          from_transform =
              from_operation->Blend(to_operation.Get(), 1 - min_progress, false);
          to_transform =
              from_operation->Blend(to_operation.Get(), 1 - max_progress, false);
        } else {
          from_transform =
      ...
      case TransformOperation::kRotateY: {
        ...
        if (from_operation) {
          from_rotation =
              static_cast<const RotateTransformOperation*>(from_operation.Get());
        ...
        if (!from_rotation) {
          identity_rotation = RotateTransformOperation::Create(
              ...
              from_operation ? from_operation->GetType() : to_operation->GetType());
        ...
        for (size_t i = 0; i < 2; ++i) {
          for (size_t j = 0; j < 2; ++j) {
            for (size_t k = 0; k < 2; ++k) {
              ...
              corner += FloatPoint3D(i * from_box.Width(), j * from_box.Height(),
                                     k * from_box.Depth());
              BoundingBoxForArc(corner, *from_rotation, *to_rotation, min_progress,
                                max_progress, bounds_for_arc);
              if (first) {
