...
static UBool TextLatin1Access(UText* text, int64_t native_index, UBool forward) {
  if (!text->context)
    return FALSE;
...
static UText* TextOpenLatin1(UTextWithBuffer* ut_with_buffer,
  ...
  UText* text =
      utext_setup(&ut_with_buffer->text, sizeof(ut_with_buffer->buffer), status);
  if (U_FAILURE(*status)) {
    ASSERT(!text);
    return 0;
  }
...
void NonSharedCharacterBreakIterator::CreateIteratorForBuffer(
  ...
  bool created_iterator =
      iterator_ &&
      CompareAndSwapNonSharedCharacterBreakIterator(iterator_, 0);
  if (!created_iterator) {
    UErrorCode error_code = U_ZERO_ERROR;
