...
class MidpointState final {
 ...
 public:
  ...
  void EnsureCharacterGetsLineBox(Iterator& text_paragraph_separator) {
    StartIgnoringSpaces(Iterator(0, text_paragraph_separator.GetLineLayoutItem(),
...
struct BidiStatus final {
  ...
  BidiStatus(TextDirection text_direction, bool is_override) {
    WTF::Unicode::CharDirection direction = text_direction == TextDirection::kLtr
...
class BidiResolver final {
 ...
 private:
  ...
  TextDirection DetermineDirectionalityInternal(bool break_on_paragraph,
...
bool BidiResolver<Iterator, Run, IsolatedRun>::CommitExplicitEmbedding(
  ...
  if (to_level > from_level)
    RaiseExplicitEmbeddingLevel(runs, from_level % 2 ? kRightToLeft : kLeftToRight,
