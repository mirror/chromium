...
class PLATFORM_EXPORT SegmentedSubstring {
 ...
 public:
  SegmentedSubstring() : length_(0), do_not_exclude_line_numbers_(true) {
    data_ = nullptr;
  }
  ...
  SegmentedSubstring(const String& str)
      : length_(str.length()),
        ...
        string_(str.Impl()->UpconvertedString()) {
    if (length_) {
      data_ = string_.Characters16();
    } else {
      data_ = nullptr;
    }
  }
  ...
  UChar GetCurrentChar() const { return *data_; }
  ...
  UChar IncrementAndGetCurrentChar() { return *++data_; }
