...
class PLATFORM_EXPORT LazyLineBreakIterator final {
 ...
 public:
  LazyLineBreakIterator()
      : iterator_(0), cached_prior_context_(0), cached_prior_context_length_(0) {
    ResetPriorContext();
  }
  ...
  TextBreakIterator* Get(unsigned prior_context_length) {
    ...
    if (!iterator_) {
      if (string_.Is8Bit())
        iterator_ = AcquireLineBreakIterator(string_.Characters8(),
                                              string_.length(), locale_,
                                              prior_context, prior_context_length);
      else
        iterator_ = AcquireLineBreakIterator(string_.Characters16(),
                                              string_.length(), locale_,
                                              prior_context, prior_context_length);
