...
// https://www.w3.org/TR/css-logical-1/ for definations of logical orientations.

// This file provides 2 types of templates:
//
...
//   to get values in logical or physical orientations. As all the inputs may be
//   evaluated even if not used, the evaluation of the inputs must be simple and
//   be fully inlined to avoid/reduce extra cost.
//
// - Getter/Setter value mappers (PhysicalToLogicalGetter,
...
//   accessors to get or set values in logical or physical orientations. This is
//   suitable for mapping of setters, or getters implemented with non-inlined
//   functions. Evaluation of the input values are delayed when they are
//   actually needed.
//
// See LayoutBoxModelObject.h and ComputedStyle.h for examples.
...
class PhysicalToLogicalSetter {
 ...
 public:
  ...
  void SetLeft(Value v) const { (object_.*converter_.Left())(v); }
  void SetRight(Value v) const { (object_.*converter_.Right())(v); }
  void SetTop(Value v) const { (object_.*converter_.Top())(v); }
  void SetBottom(Value v) const { (object_.*converter_.Bottom())(v); }
