...
void PNGImageReader::StartFrameDecoding(const FastSharedBufferReader& reader,
  ...
  ProcessData(reader, initial_offset_ + kHeaderSize, idat_offset_ - kHeaderSize);
}
...
static bool CheckCrc(const FastSharedBufferReader& reader,
  ...
  const png_byte* chunk =
      ReadAsConstPngBytep(reader, chunk_start + 4, chunk_length + 4, read_buffer);

  char crc_buffer[4];
  const png_byte* crc_position =
      ReadAsConstPngBytep(reader, chunk_start + 8 + chunk_length, 4, crc_buffer);
...
bool PNGImageReader::Parse(SegmentReader& data, ParseQuery query) {
  ...
  while (reader.size() >= read_offset_ + 8) {
    ...
    if (fd_at || (idat && idat_is_part_of_animation_)) {
      ...
      chunk = ReadAsConstPngBytep(reader, read_offset_ + 8, length, read_buffer);
...
void PNGImageReader::ClearDecodeState(size_t index) {
  if (index)
    return;
  png_destroy_read_struct(png_ ? &png_ : nullptr,
                          info_ ? &info_ : nullptr, 0);
  DCHECK(!png_ && !info_);
  progressive_decode_offset_ = 0;
}

const PNGImageReader::FrameInfo& PNGImageReader::GetFrameInfo(size_t index) const {
...
bool PNGImageReader::ParseFrameInfo(const png_byte* data) {
  ...
  new_frame_.frame_rect = IntRect(x_offset, y_offset, frame_width, frame_height);
