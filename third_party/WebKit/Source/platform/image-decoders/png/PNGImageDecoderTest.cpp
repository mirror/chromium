...
void TestMissingDataBreaksDecoding(const char* png_file,
  ...
  RefPtr<SharedBuffer> invalid_data = SharedBuffer::Create(data->Data(), offset);
...
void TestProgressiveDecodingContinuesAfterFullData(const char* png_file,
  ...
  const ImageFrame* const frame_upfront = decoder_upfront->FrameBufferAtIndex(0);
...
TEST(AnimatedPNGTests, MixedDataChunks) {
  ...
  RefPtr<SharedBuffer> data = SharedBuffer::Create(full_data->Data(), kPostIDAT);
  const size_t kFcTLSize = 38u;
