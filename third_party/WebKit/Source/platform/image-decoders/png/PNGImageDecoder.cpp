...
#ifdef __ARM_NEON__
static void SetRGBARawRowNeon16(png_bytep src_ptr,
                                unsigned pixel_count,
                                ImageFrame::PixelData* dst_pixel,
                                unsigned* alpha_mask) {
  constexpr unsigned kPixelsPerLoad = 16;
  constexpr unsigned kRgbaStep = kPixelsPerLoad * 4;
  // Input registers.
  uint8x16x4_t rgba;
  // Output registers.
  uint8x16x4_t bgra;
  ...
  unsigned counter;
  for (counter = pixel_count; counter >= kPixelsPerLoad;
  ...
  *alpha_mask &= alpha_mask_u64;
  ...
  for (; counter > 0; counter--, dst_pixel++, src_ptr += 4) {
...
static void SetRGBARawRowNoAlphaNeon16(png_bytep src_ptr,
                                       unsigned pixel_count,
                                       ImageFrame::PixelData* dst_pixel) {
  constexpr unsigned kPixelsPerLoad = 16;
  constexpr unsigned kRgbaStep = kPixelsPerLoad * 3;
  // Input registers.
  uint8x16x3_t rgb;
  // Output registers.
  uint8x16x4_t bgra;
  ...
  // Handle the tail elements.
  for (; counter > 0; counter--, dst_pixel++, src_ptr += 3) {
