...
#if (defined(__ARM_NEON__) || defined(__ARM_NEON))
// Swizzle RGBA to BGRA (or the Android-specific packing order), and return
// the AND of all alpha channels.
static inline void SetRGBARawRowNeon16(png_bytep src_ptr,
  ...
  for (; i >= kPixelsPerLoad; i -= kPixelsPerLoad) {
    ...
    // Android uses a different color channel packing order.
    bgra.val[0] = rgba.val[0];
    bgra.val[1] = rgba.val[1];
    bgra.val[2] = rgba.val[2];
    bgra.val[3] = rgba.val[3];
#else
    // Re-order color channels: RGBA -> BGRA.
...
// Swizzle RGB to opaque BGRA (or the Android-specific packing order),
// and return the AND of all alpha channels.
static inline void SetRGBARawRowNoAlphaNeon16(
    png_bytep src_ptr,
    const int pixel_count,
    ImageFrame::PixelData* dst_pixel) {
  assert(dst_pixel);
...
#if defined(OS_ANDROID)
    // Android uses a different color channel packing order.
    bgra.val[0] = rgb.val[0];
    bgra.val[1] = rgb.val[1];
    bgra.val[2] = rgb.val[2];
#else
    // Re-order color channels: RGB -> BGR.
...
#if (defined(__ARM_NEON__) || defined(__ARM_NEON))
        SetRGBARawRowNeon16(src_ptr, width, dst_row, &alpha_mask);
#else
        for (auto *dst_pixel = dst_row; dst_pixel < dst_row + width;
             dst_pixel++, src_ptr += 4) {
          ImageFrame::SetRGBARaw(dst_pixel, src_ptr[0], src_ptr[1], src_ptr[2],
...
#if (defined(__ARM_NEON__) || defined(__ARM_NEON))
    SetRGBARawRowNoAlphaNeon16(src_ptr, width, dst_row);
#else
    for (auto *dst_pixel = dst_row; dst_pixel < dst_row + width;
         src_ptr += 3, ++dst_pixel) {
      ImageFrame::SetRGBARaw(dst_pixel, src_ptr[0], src_ptr[1], src_ptr[2],
