...
void PNGImageDecoder::RowAvailable(unsigned char* row_buffer,
  ...
  if (has_alpha) {
    ...
    if (SkColorSpaceXform* xform = ColorTransform()) {
      ...
      DCHECK(xform->apply(color_format, dst_row, color_format, src_ptr, width,
    ...
    // RGBA (and not RGB).
    if (SkColorSpaceXform* xform = ColorTransform()) {
      DCHECK(xform->apply(XformColorFormat(), dst_row, XformColorFormat(),
                          dst_row, width, kOpaque_SkAlphaType));
    }
  }
