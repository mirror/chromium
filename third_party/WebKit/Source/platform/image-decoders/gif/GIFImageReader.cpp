...
void GIFColorMap::BuildTable(FastSharedBufferReader* reader) {
  ...
  RELEASE_ASSERT(position_ + colors_ * BYTES_PER_COLORMAP_ENTRY <=
                 reader->size());
  DCHECK_LE(colors_, MAX_COLORS);
  ...
  const unsigned char* src_colormap =
      reinterpret_cast<const unsigned char*>(reader->GetConsecutiveData(
          position_, colors_ * BYTES_PER_COLORMAP_ENTRY, buffer));
  ...
  for (Table::iterator iter = table_.begin(); iter != table_.end(); ++iter) {
    *iter = SkPackARGB32NoCheck(255, src_colormap[0], src_colormap[1],
                                src_colormap[2]);
    src_colormap += BYTES_PER_COLORMAP_ENTRY;
  }
}
    ...
    switch (state_) {
      ...
      case kGIFType: {
        const char* current_component =
            reader.GetConsecutiveData(current_component_position, 6, read_buffer);
      ...
      case kGIFGlobalHeader: {
        ...
        const size_t global_color_map_colors = 2 << (current_component[4] & 0x07);
        ...
        if ((current_component[4] & 0x80) &&
          ...
          GETN(BYTES_PER_COLORMAP_ENTRY * global_color_map_colors,
               kGIFGlobalColormap);
          break;
        }
      ...
      case kGIFConsumeNetscapeExtension: {
        ...
        if (netscape_extension == 1) {
          ...
          if (!loop_count_)
            loop_count_ = kCAnimationLoopInfinite;
      ...
      case kGIFImageHeader: {
        ...
        if (is_local_colormap_defined) {
          ...
          current_frame->LocalColorMap().SetTablePositionAndSize(data_position,
                                                                num_colors);
          GETN(BYTES_PER_COLORMAP_ENTRY * num_colors, kGIFImageColormap);
          break;
        }
...
bool GIFLZWContext::PrepareToDecode() {
  ...
  if (frame_context_->DataSize() >= MAX_DICTIONARY_ENTRY_BITS)
    return false;
  ...
  // encodable sequence.
  const size_t kMaxBytes = MAX_DICTIONARY_ENTRIES - 1;
