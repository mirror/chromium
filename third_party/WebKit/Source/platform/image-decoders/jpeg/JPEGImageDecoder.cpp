...
void JPEGImageDecoder::Decode(bool only_size) {
  ...
  // Start incremental decoding once memory allocated
  if (frame.GetStatus() == ImageFrame::kFrameAllocated) {
    SkCodec::Result result = codec_->startIncrementalDecode(
        image_info, frame.Bitmap().getPixels(), frame.Bitmap().rowBytes(),
  ...
  int rows_decoded = 0;
  SkCodec::Result result = codec_->incrementalDecode(&rows_decoded);
  switch (result) {
    case SkCodec::kSuccess:
