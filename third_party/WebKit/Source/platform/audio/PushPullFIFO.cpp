...
// Used to log if any audio glitches have been detected during the lifetime of
// BaseAudioContext. Elements in this ENUM should not be added, deleted or
// rearranged.
enum WebAudioFIFOUnderflowGlitchResult {
  kFIFONoUnderflowGlitches = 0,
  kFIFOUnderflowGlitches = 1,
  kFIFOUnderflowGlitchesMax = 2
};
...
PushPullFIFO::~PushPullFIFO() {
  ...
  DEFINE_STATIC_LOCAL(EnumerationHistogram, fifo_underflow_glitches_histogram,
                      ("WebAudio.PushPullFIFO.UnderflowGlitches",
                       kFIFOUnderflowGlitchesMax));
  fifo_underflow_glitches_histogram.Count(underflow_count_ == 0
      ? kFIFONoUnderflowGlitches
      : kFIFOUnderflowGlitches);
