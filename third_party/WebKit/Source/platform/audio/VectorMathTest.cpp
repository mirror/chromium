...
class VectorMathTest : public testing::Test {
 protected:
  ...
  static void SetUpTestCase() {
    ...
    std::uniform_int_distribution<size_t> distribution(0u,
                                                       kFloatArraySize - 1u);
    for (size_t i = 0u; i < kSourceCount; ++i) {
      if (i == kFullyFiniteSource)
        continue;
      sources_[i][distribution(GetRandomGenerator())] = INFINITY;
      sources_[i][distribution(GetRandomGenerator())] = -INFINITY;
      if (i != kFullyNonNanSource)
        sources_[i][distribution(GetRandomGenerator())] = NAN;
    }
  };
 ...
 private:
  ...
  static std::minstd_rand& GetRandomGenerator() {
...
TEST_F(VectorMathTest, Zvmul) {
  for (const auto& real1 : GetPrimaryVectors(GetSource(0u))) {
    if (real1.stride() != 1)
      continue;
    const TestVector<const float> imag1(GetSource(1u), real1);
    const TestVector<const float> real2(GetSource(2u), real1);
    const TestVector<const float> imag2(GetSource(3u), real1);
