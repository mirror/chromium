...
void CookieStore::OnGetAllResult(
  ...
  v8::Local<v8::Array> result = v8::Array::New(isolate, backend_result.size());
  for (size_t i = 0; i < backend_result.size(); ++i) {
    CookieListItem cookie;
    cookie.setName(backend_result[i]->name);
    cookie.setValue(backend_result[i]->value);
    result->Set(i, ToV8(std::move(cookie), creation_context, isolate));
  }
...
mojom::blink::CookieStoreSetOptionsPtr CookieStore::ToBackendOptions(
  ...
  if (options.hasExpires())
    backend_options->expires = options.expires();
