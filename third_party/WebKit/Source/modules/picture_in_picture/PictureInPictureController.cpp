...
PictureInPictureController::Status PictureInPictureController::GetStatus() {
  ...
  // If Picture-in-Picture is disabled, return kDisabledBySystem status.
  if (!picture_in_picture_enabled_) {
    // This happens if not Android O and onwards or if Android user has
    // deactivated PiP for Chrome.
