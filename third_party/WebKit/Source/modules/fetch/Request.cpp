...
FetchRequestData* CreateCopyOfFetchRequestDataForFetch(
  ...
  if (world.IsIsolatedWorld())
    ...
    request->SetOrigin(script_state->GetExecutionContext()->GetSecurityOrigin());
...
Request* Request::CreateRequestWithRequestOrString(
  ...
  if (!input_request) {
    ...
    if (!parsed_url.IsValid()) {
      exception_state.ThrowTypeError("Failed to parse URL from " + input_string);
      return nullptr;
    }
  ...
  if (init.are_any_members_set) {
    ...
    if (init.referrer.referrer.IsEmpty()) {
      ...
      if (parsed_referrer.ProtocolIsAbout() && parsed_referrer.Host().IsEmpty() &&
...
Request* Request::Create(ScriptState* script_state,
  ...
  FetchRequestData* request = FetchRequestData::Create(script_state, web_request);
  return new Request(script_state, request);
}
