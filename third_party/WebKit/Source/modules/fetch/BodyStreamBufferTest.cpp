...
class BodyStreamBufferTest : public ::testing::Test {
 protected:
  ScriptValue Eval(ScriptState* script_state, const char* s) {
    ...
    if (!V8Call(v8::Script::Compile(script_state->GetContext(), source), script)) {
  ...
  ScriptValue EvalWithPrintingError(ScriptState* script_state, const char* s) {
    ...
    if (block.HasCaught()) {
      ADD_FAILURE() << ToCoreString(
                           block.Exception()->ToString(script_state->GetIsolate()))
                           .Utf8()
                           .Data();
      block.ReThrow();
    }
...
TEST_F(BodyStreamBufferTest, Tee) {
  ...
  ReplayingBytesConsumer* src = new ReplayingBytesConsumer(&scope.GetDocument());
...
TEST_F(BodyStreamBufferTest, DrainAsBlobDataHandle) {
  ...
  EXPECT_FALSE(buffer->IsStreamDisturbed());
  EXPECT_FALSE(buffer->HasPendingActivity());
  RefPtr<BlobDataHandle> output_blob_data_handle = buffer->DrainAsBlobDataHandle(
