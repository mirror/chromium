...
void FetchManager::Loader::Start() {
  ...
  if ((SecurityOrigin::Create(request_->Url())
      ...
      (request_->Url().ProtocolIsData() &&
       request_->SameOriginDataURLFlag()) ||
...
void FetchManager::Loader::PerformBasicFetch() {
  ...
  if (SchemeRegistry::ShouldTreatURLSchemeAsSupportingFetchAPI(
    ...
    PerformNetworkError("Fetch API cannot load " +
                        request_->Url().GetString() + ". URL scheme \"" +
                        request_->Url().Protocol() + "\" is not supported.");
  }
}
...
void FetchManager::Loader::PerformHTTPFetch(bool cors_flag,
  ...
  ASSERT(
      SchemeRegistry::ShouldTreatURLSchemeAsSupportingFetchAPI(
          request_->Url().Protocol()) ||
      (request_->Url().ProtocolIs("blob") && !cors_flag && !cors_preflight_flag));
  ...
  loader_ =
      ThreadableLoader::Create(*execution_context_, this,
...
void FetchManager::Loader::Failed(const String& message) {
  ...
  if (resolver_) {
    ...
    resolver_->Reject(
        V8ThrowException::CreateTypeError(state->GetIsolate(), "Failed to fetch"));
  }
