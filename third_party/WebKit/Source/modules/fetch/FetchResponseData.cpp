...
WebVector<WebString> HeaderSetToWebVector(
    const WebCrossOriginAccessControl::HTTPHeaderSet& headers) {
...
FetchResponseData* FetchResponseData::CreateCORSFilteredResponse() const {
  DCHECK_EQ(type_, kDefaultType);
  WebCrossOriginAccessControl::HTTPHeaderSet access_control_expose_header_set;
  String access_control_expose_headers;
  if (header_list_->Get(HTTPNames::Access_Control_Expose_Headers,
                        access_control_expose_headers)) {
    WebCrossOriginAccessControl::ParseAccessControlExposeHeadersAllowList(
...
FetchResponseData* FetchResponseData::CreateCORSFilteredResponse(
    const WebCrossOriginAccessControl::HTTPHeaderSet& exposed_headers) const {
  DCHECK_EQ(type_, kDefaultType);
  ...
  for (const auto& header : header_list_->List()) {
    ...
    if (WebCrossOriginAccessControl::IsOnAccessControlResponseHeaderWhitelist(
            name) ||
        (explicitly_exposed &&
