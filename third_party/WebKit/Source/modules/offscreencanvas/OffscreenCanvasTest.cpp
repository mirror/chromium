...
TEST_F(OffscreenCanvasTest, AnimationNotInitiallySuspended) {
  ...
  EXPECT_FALSE(OSCanvas().GetOrCreateFrameDispatcher()->IsAnimationSuspended());
}

TEST_F(OffscreenCanvasTest, AnimationActiveAfterCommit) {
  ScriptState::Scope scope(GetScriptState());
  DummyExceptionStateForTesting exception_state;
  EXPECT_FALSE(OSCanvas().GetOrCreateFrameDispatcher()->NeedsBeginFrame());
  Context().commit(GetScriptState(), exception_state);
  EXPECT_TRUE(OSCanvas().GetOrCreateFrameDispatcher()->NeedsBeginFrame());
}
...
TEST_F(OffscreenCanvasTest, AnimationSuspendedWhilePlaceholderHidden) {
  ...
  platform()->RunUntilIdle();
  EXPECT_TRUE(OSCanvas().GetOrCreateFrameDispatcher()->IsAnimationSuspended());
