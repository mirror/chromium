...
void RejectNavigatorDetached(ScriptPromiseResolver* resolver) {
  DOMException* exception = DOMException::Create(
      kInvalidStateError, "The object is no longer associated with a document.");
...
void NavigatorVR::DidAddEventListener(LocalDOMWindow* window,
  ...
  if (event_type == EventTypeNames::vrdisplayactivate &&
      GetSupplementable()->GetFrame() && GetSupplementable()->GetFrame()->GetDocument() &&
      Fullscreen::FullscreenEnabled(*GetSupplementable()->GetFrame()->GetDocument())) {
