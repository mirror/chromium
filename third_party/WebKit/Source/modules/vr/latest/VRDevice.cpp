...
ScriptPromise VRDevice::requestSession(
  ...
  if (parameters.exclusive() && frameProvider()->exclusive_session()) {
  ...
  VRSessionCreateParameters* create_parameters = new VRSessionCreateParameters(
      parameters.exclusive(), parameters.outputContext());

  VRSession* session = new VRSession(this, create_parameters);
