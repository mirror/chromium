...
void VRSession::OnControllerSelectGesture(
  ...
  if (controller) {
    ...
    VRPresentationFrame* presentation_frame = CreatePresentationFrame();

    VRGestureEvent* event = VRGestureEvent::Create(
        EventTypeNames::select, presentation_frame, controller);
    DispatchEvent(event);
  }
}
...
void VRSession::OnCanvasSelectGesture(MouseEvent* event) {
  ...
  VRGestureEvent* gesture = VRGestureEvent::Create(
      EventTypeNames::select, presentation_frame, input_source);
...
DoubleSize VRSession::IdealFramebufferSize() const {
  if (exclusive_) {
    ...
    double height =
        std::max(device_->vrDisplayInfoPtr()->leftEye->renderHeight,
...
void VRSession::OnFrame(
  ...
  if (pending_frame_) {
    ...
    presentation_frame->InFrameCallback(true);
    callback_collection_.ExecuteCallbacks(this, presentation_frame);
    presentation_frame->InFrameCallback(false);
...
VRPresentationFrame* VRSession::CreatePresentationFrame() {
...
const HeapVector<Member<VRView>>& VRSession::views() {
  if (views_dirty_) {
    if (exclusive_) {
      ...
      // offset.
      views_[VRView::kEyeLeft]->UpdateProjectionMatrixFromAspect(
          M_PI * 0.4f,  // TODO(bajones): Not this! Maybe user configurable?
          aspect, depth_near_, depth_far_);

      views_[VRView::kEyeLeft]->UpdateOffset(0, 0, 0);
    }
