...
void UpdateViewFromEyeParameters(
  ...
  view->UpdateProjectionMatrixFromFoV(
      fov->upDegrees * DEG_TO_RAD, fov->downDegrees * DEG_TO_RAD,
      fov->leftDegrees * DEG_TO_RAD, fov->rightDegrees * DEG_TO_RAD, depth_near,
      depth_far);
...
const HeapVector<Member<VRView>>& VRSession::views() {
  if (views_dirty_) {
    if (exclusive_) {
      ...
      UpdateViewFromEyeParameters(views_[0],
                                  device_->vrDisplayInfoPtr()->leftEye,
                                  depth_near_, depth_far_);
      UpdateViewFromEyeParameters(views_[1],
