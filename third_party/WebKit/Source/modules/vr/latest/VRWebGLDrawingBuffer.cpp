...
bool VRWebGLDrawingBuffer::Initialize(const IntSize& size,
  ...
  // TODO(bajones): Reuse the existing one from ::Create().
  std::unique_ptr<Extensions3DUtil> extensions_util =
      Extensions3DUtil::Create(gl);

  // Check context capabilities

  // TODO(bajones): Actually act on the antialiasing info gathered here.
...
void VRWebGLDrawingBuffer::SwapColorBuffers() {
  ...
  if (WantExplicitResolve()) {
    ...
    webgl_context_->DrawingBufferClientRestoreScissorTest();
  } else {
    gl->BindFramebuffer(GL_FRAMEBUFFER, framebuffer_->Object());
...
RefPtr<StaticBitmapImage> VRWebGLDrawingBuffer::TransferToStaticBitmapImage() {
