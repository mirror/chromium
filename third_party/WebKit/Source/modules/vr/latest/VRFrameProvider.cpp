...
class VRFrameProviderRequestCallback : public FrameRequestCallback {
 public:
...
void VRFrameProvider::BeginExclusiveSession(VRSession* session,
  ...
  DCHECK(session && session->isExclusive());
  ...
  if (!presentation_provider_.is_bound()) {
    bool secure_context =
        ExecutionContext::From(resolver->GetScriptState())->IsSecureContext();
...
void VRFrameProvider::EndExclusiveSession() {
  ...
  exclusive_session_->ForceEndSession();
...
void VRFrameProvider::RequestFrame(VRSession* session) {
  ...
  bool exclusive = session->isExclusive();
  ...
  if (exclusive && !pending_exclusive_vsync_) {
...
void VRFrameProvider::ProcessScheduledFrame(double timestamp) {
  ...
  if (exclusive_session_) {
    ...
    processing_sessions.swap(requesting_sessions_);
