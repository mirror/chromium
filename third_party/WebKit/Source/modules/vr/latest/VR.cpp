...
ScriptPromise VR::requestDevice(ScriptState* script_state) {
  ...
  if (!service_ || devices_synced_) {
    // TODO: When we have a prioritized order of devices, or some other method
    // of getting the prefered device, insert that here. For now, just get the
    // first device out of the list, if there is one.
