...
void VR::OnDisplayConnected(
    ...
    device::mojom::blink::VRDisplayInfoPtr device_info) {
  VRDevice* vr_device =
      new VRDevice(this, std::move(device), std::move(client_request));
  vr_device->setDeviceName(device_info->displayName);
  vr_device->setIsExternal(device_info->capabilities->hasExternalDisplay);
