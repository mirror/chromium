...
#include "modules/xr/common/XRFrameHelper.h"
...
ScriptPromise VRDisplay::requestPresent(ScriptState* script_state,
  ...
  if (!pending_present_resolvers_.IsEmpty()) {
    ...
    frame_transport_ = new XRFrameTransport(context_gl_);

    device::mojom::blink::VRRequestPresentOptionsPtr options =
        XRFrameHelper::GetRequestPresentOptions(rendering_context_.Get());
...
void VRDisplay::submitFrame() {
  ...
  frame_transport_->FramePreImage();

  scoped_refptr<Image> image_ref =
      XRFrameHelper::CaptureImage(rendering_context_.Get());

  DrawingBuffer::Client* drawing_buffer_client =
      static_cast<DrawingBuffer::Client*>(rendering_context_.Get());

  frame_transport_->FrameSubmit(vr_presentation_provider_.get(),
