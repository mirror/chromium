...
class VRDisplayFrameRequestCallback : public FrameRequestCallback {
 public:
  VRDisplayFrameRequestCallback(VRDisplay* vr_display)
...
void VRDisplay::UpdateLayerBounds() {
  ...
  vr_presentation_provider_->UpdateLayerBounds(
      vr_frame_id_,
      gfx::mojom::blink::RectF::New(left[0], left[1], left[2], left[3]),
      gfx::mojom::blink::RectF::New(right[0], right[1], right[2], right[3]),
      blink::WebSize(source_width_, source_height_));
}
