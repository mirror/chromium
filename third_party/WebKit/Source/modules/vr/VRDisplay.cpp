...
bool VRDisplay::ConfigurePresentationPathForDisplay() {
  ...
  if (capabilities_->hasExternalDisplay()) {
    frame_transport_method_ = FrameTransport::TEXTURE_HANDLE;
    wait_for_previous_render_ = WaitPrevStrategy::NO_WAIT;
  } else {
    frame_transport_method_ = FrameTransport::MAILBOX;
    wait_for_previous_render_ = WaitPrevStrategy::AFTER_BITMAP;
  }
  return true;
}
...
void VRDisplay::submitFrame() {
  ...
  DCHECK(frame_transport_method_ != FrameTransport::UNINITIALIZED);
  DCHECK(wait_for_previous_render_ != WaitPrevStrategy::UNINITIALIZED);
  ...
  if (frame_transport_method_ == FrameTransport::TEXTURE_HANDLE) {
#if defined(OS_WIN)
    ...
    NOTIMPLEMENTED();
#endif
  } else if (frame_transport_method_ == FrameTransport::MAILBOX) {
