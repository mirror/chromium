...
// TODO(klausw): release resources when done presenting
      ...
      GLuint texture = ahb_textures_[ahb_texture_index_];
      context_gl_->BindTexture(GL_TEXTURE_2D, texture);
      context_gl_->ReleaseTexImage2DCHROMIUM(
          GL_TEXTURE_2D, ahb_eglimages_[ahb_texture_index_]);
      CHECK_ERR;
      rendering_context_->SetCustomBackbufferFBO(0);
      CHECK_ERR;
...
void VRDisplay::CreateAHBStorage() {
  ...
  for (int i = 0; i < AHB_TEXTURE_COUNT; ++i) {
    ahb_textures_[i] = 0;
    ahb_eglimages_[i] = 0;
  }

  context_gl_->GenTextures(AHB_TEXTURE_COUNT, &ahb_textures_[0]);

  for (int i = 0; i < AHB_TEXTURE_COUNT; ++i) {
    context_gl_->BindTexture(GL_TEXTURE_2D, ahb_textures_[i]);
...
void VRDisplay::CreateAndBindAHBImage(
    ...
    int height,
    int texture_index,
    const base::Optional<gfx::GpuMemoryBufferHandle>& buffer_handle) {
  gpu::GpuMemoryBufferManager* gpu_memory_buffer_manager =
      Platform::Current()->GetGpuMemoryBufferManager();

  if (!gpu_memory_buffer_manager) {
    LOG(ERROR) << __FUNCTION__ << ";;; No memory buffer manager!";
    return;
  }

  TRACE_EVENT_BEGIN0("gpu", "VRDisplay::CreateFromHandle");
  auto buffer_format = gfx::BufferFormat::RGBA_8888;
  auto buffer = gpu_memory_buffer_manager->CreateFromHandle(
      buffer_handle.value(), gfx::Size(width, height), buffer_format,
      gfx::BufferUsage::SCANOUT);
  CHECK_ERR;
  TRACE_EVENT_END0("gpu", "VRDisplay::CreateFromHandle");
  // LOG(INFO) << __FUNCTION__ << ";;; buffer=" << buffer.get();
  if (!buffer) {
    LOG(ERROR) << __FUNCTION__ << ";;; Didn't get buffer!";
    return;
  }

  TRACE_EVENT_BEGIN0("gpu", "VRDisplay::CreateImage");
  if (ahb_eglimages_[texture_index]) {
    context_gl_->DestroyImageCHROMIUM(ahb_eglimages_[texture_index]);
    CHECK_ERR;
  }
  auto eglimage_format = GL_RGBA;
  ahb_eglimages_[texture_index] = context_gl_->CreateImageCHROMIUM(
      buffer->AsClientBuffer(), width, height, eglimage_format);
  CHECK_ERR;
  TRACE_EVENT_END0("gpu", "VRDisplay::CreateImage");
  // LOG(INFO) << __FUNCTION__ << ";;; ahb_eglimage_=" <<
  // ahb_eglimages_[texture_index];

  GLuint texture = ahb_textures_[texture_index];
  TRACE_EVENT_BEGIN0("gpu", "VRDisplay::BindTexture");
  context_gl_->BindTexture(GL_TEXTURE_2D, texture);
  TRACE_EVENT_END0("gpu", "VRDisplay::BindTexture");
  CHECK_ERR;

  TRACE_EVENT_BEGIN0("gpu", "VRDisplay::BindTexImage");
  // glEGLImageTargetTexture2DOES equivalent? Expensive?
  context_gl_->BindTexImage2DCHROMIUM(GL_TEXTURE_2D,
                                      ahb_eglimages_[texture_index]);
  CHECK_ERR;
  ...
  if (fbstatus != GL_FRAMEBUFFER_COMPLETE) {
    LOG(ERROR) << __FUNCTION__
               << ";;; FRAMEBUFFER NOT COMPLETE, fbstatus=" << fbstatus;
...
void VRDisplay::BindAHBAndResizeIfNeeded(
  ...
  ahb_texture_index_ = (ahb_texture_index_ + 1) % AHB_TEXTURE_COUNT;
  ...
  CreateAndBindAHBImage(width, height, ahb_texture_index_, buffer_handle);
}

void VRDisplay::BindAHBToBufferHandle(
    const base::Optional<gfx::GpuMemoryBufferHandle>& buffer_handle) {
  ...
  BindAHBAndResizeIfNeeded(current_width, current_height, buffer_handle);
...
void VRDisplay::OnPresentingVSync(
  ...
  DVLOG(3) << __FUNCTION__ << ";;; have buffer=" << !!buffer_handle;
  if (buffer_handle) {
    DVLOG(3) << __FUNCTION__
             << ";;; have buffer_handle=" << &buffer_handle.value()
             << " handle.type=" << buffer_handle->type << " handle_mailbox="
             << buffer_handle->MailboxSharedBufferString();
    TRACE_EVENT0("gpu", "VRDisplay::BufferSetup");

    BindAHBToBufferHandle(buffer_handle);
    gvr_zero_copy_ = true;
  } else {
