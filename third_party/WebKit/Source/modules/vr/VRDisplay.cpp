...
void VRDisplay::submitFrame() {
  ...
  // yet, wait for it to be received. (This may fail to get a fence in case
  // of mojo communication problems.)
  if (pending_previous_frame_fence_) {
    wait_time += WaitForGpuFenceReceived();
  }
  // If we have a GpuFence, send it to the GPU service process and ask it to do
  // an asynchronous server wait.
  ...
  if (transport_options_->transportMethod ==
    ...
    if (pending_previous_frame_render_)
  ...
  pending_previous_frame_render_ =
      transport_options_->waitForRenderNotification;
  pending_previous_frame_fence_ = transport_options_->waitForGpuFence;
...
void VRDisplay::OnSubmitFrameGpuFence(const gfx::GpuFenceHandle& handle) {
  pending_previous_frame_fence_ = false;
...
WTF::TimeDelta VRDisplay::WaitForGpuFenceReceived() {
  ...
  WTF::TimeTicks start = WTF::CurrentTimeTicks();
  while (pending_previous_frame_fence_) {
    if (!submit_frame_client_binding_.WaitForIncomingMethodCall()) {
      DLOG(ERROR) << "Failed to receive SubmitFrame response";
      break;
    }
