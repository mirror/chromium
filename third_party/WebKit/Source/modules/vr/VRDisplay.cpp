...
ScriptPromise VRDisplay::requestPresent(ScriptState* script_state,
  ...
  if (!pending_present_resolvers_.IsEmpty()) {
    ...
    bool secure_context = script_state->GetExecutionContext()->IsSecureContext();
    if (!display_) {
      ForceExitPresent();
      DOMException* exception = DOMException::Create(
...
ScriptPromise VRDisplay::exitPresent(ScriptState* script_state) {
  ...
  if (!is_presenting_) {
    ...
    DOMException* exception =
        DOMException::Create(kInvalidStateError, "VRDisplay is not presenting.");
    resolver->Reject(exception);
    return promise;
  }
...
void VRDisplay::submitFrame() {
  ...
  display_->SubmitFrame(
      vr_frame_id_, gpu::MailboxHolder(static_image->GetMailbox(),
