...
void VRDisplay::submitFrame() {
  ...
  if (prev_frame_fence_) {
    LOG(INFO) << __FUNCTION__ << ";;; CreateClientGpuFenceCHROMIUM(prev_frame_fence_)";
    GLuint id = context_gl_->CreateClientGpuFenceCHROMIUM(prev_frame_fence_->AsClientGpuFence());
  ...
  if (present_image_needs_copy_) {
    ...
    LOG(INFO) << __FUNCTION__ << ";;; wait_time.InSecondsF()=" << wait_time.InSecondsF();
...
WTF::TimeDelta VRDisplay::WaitForPreviousRenderPhase(WaitPrevStrategy context) {
  ...
  LOG(INFO) << __FUNCTION__ << ";;; elapsed=" << elapsed.InSecondsF() << "s";
  return elapsed;
}

void VRDisplay::OnSubmitFramePostRenderFence(const gfx::GpuFenceHandle& handle) {
  LOG(INFO) << __FUNCTION__ << ";;; got handle, fd=" << handle.native_fd.fd;
...
#if 1
  prev_frame_fence_ = base::MakeUnique<gfx::GpuFence>(handle);
#else
