...
void BaseRenderingContext2D::DrawImageInternal(PaintCanvas* c,
  ...
  if (image_source->IsSVGSource()) {
    TrackDrawCall(kDrawVectorImage, nullptr, dst_rect.Width(), dst_rect.Height());
  } else {
    TrackDrawCall(kDrawBitmapImage, nullptr, dst_rect.Width(), dst_rect.Height());
  }
  ...
  if (!imageSmoothingEnabled() &&
      IsDrawScalingDown(src_rect, dst_rect, GetState().Transform().XScaleSquared(),
...
void BaseRenderingContext2D::drawImage(ScriptState* script_state,
  ...
  if (!image_source->IsVideoElement()) {
    ...
    image = image_source->GetSourceImageForCanvas(
        &source_image_status, hint, kSnapshotReasonDrawImage, default_object_size);
  ...
  ClipRectsToImageRect(FloatRect(FloatPoint(), image_size), &src_rect, &dst_rect);
  ...
  Draw(
      ...
      {
        DrawImageInternal(c, image_source, image.Get(), src_rect, dst_rect, flags);
      },
...
bool BaseRenderingContext2D::RectContainsTransformedRect(
  ...
  FloatQuad quad(rect);
  FloatQuad transformed_quad(FloatRect(transformed_rect.x(), transformed_rect.y(),
...
void BaseRenderingContext2D::CheckOverdraw(
  ...
  if (flags) {
    ...
    if (is_source_over && image_type == CanvasRenderingContext2DState::kNoImage) {
...
void BaseRenderingContext2D::TrackDrawCall(DrawCallType call_type,
  ...
  if (call_type == kFillText || call_type == kFillPath || call_type == kStrokeText ||
      call_type == kStrokePath || call_type == kFillRect ||
      call_type == kStrokeRect) {
    SkPath sk_path;
    if (path2d) {
    ...
    if (!(call_type == kFillRect || call_type == kStrokeRect ||
      ...
      bounding_rect_height = static_cast<float>(std::abs(bounding_rect.height()));
    ...
    usage_counters_.bounding_box_perimeter_draw_calls[call_type] +=
        bounding_rect_perimeter;
    usage_counters_.bounding_box_area_draw_calls[call_type] += bounding_rect_area;

    CanvasStyle* canvas_style;
    if (call_type == kFillText || call_type == kFillPath || call_type == kFillRect) {
  ...
  if (call_type == kFillText || call_type == kFillPath || call_type == kStrokeText ||
      call_type == kStrokePath || call_type == kFillRect ||
      call_type == kStrokeRect || call_type == kDrawVectorImage ||
      call_type == kDrawBitmapImage) {
    if (GetState().ShadowBlur() > 0.0 && SkColorGetA(GetState().ShadowColor()) > 0) {
      usage_counters_.num_blurred_shadows++;
      usage_counters_.bounding_box_area_times_shadow_blur_squared +=
          bounding_rect_area * GetState().ShadowBlur() * GetState().ShadowBlur();
      usage_counters_.bounding_box_perimeter_times_shadow_blur_squared +=
          bounding_rect_perimeter * GetState().ShadowBlur() * GetState().ShadowBlur();
    }
  }
...
float BaseRenderingContext2D::EstimateRenderingCost(
  ...
  float basic_cost_of_draw_calls =
      ...
      ExpensiveCanvasHeuristicParameters::kStrokeRectVariableCostPerArea[index] *
          usage_counters_
              .bounding_box_area_draw_calls[BaseRenderingContext2D::kStrokeRect] +
      ExpensiveCanvasHeuristicParameters::kStrokePathVariableCostPerArea[index] *
          usage_counters_
              .bounding_box_area_draw_calls[BaseRenderingContext2D::kStrokePath] +
      ExpensiveCanvasHeuristicParameters::kStrokeTextVariableCostPerArea[index] *
          usage_counters_
              .bounding_box_area_draw_calls[BaseRenderingContext2D::kStrokeText] +
