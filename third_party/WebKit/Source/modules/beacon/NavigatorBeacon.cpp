...
bool NavigatorBeacon::sendBeacon(
    ...
    ExceptionState& exception_state) {
  return NavigatorBeacon::From(navigator).SendBeaconImpl(script_state, urlstring,
...
bool NavigatorBeacon::SendBeaconImpl(
  ...
  if (data.isArrayBufferView()) {
    allowed = PingLoader::SendBeacon(GetSupplementable()->GetFrame(), allowance, url,
    ...
    allowed = PingLoader::SendBeacon(GetSupplementable()->GetFrame(), allowance, url,
                                     data.getAsString(), beacon_size);
  } else if (data.isFormData()) {
    allowed = PingLoader::SendBeacon(GetSupplementable()->GetFrame(), allowance, url,
                                     data.getAsFormData(), beacon_size);
  } else {
    allowed = PingLoader::SendBeacon(GetSupplementable()->GetFrame(), allowance, url,
                                     String(), beacon_size);
  }
