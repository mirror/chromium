...
BackgroundFetchBridge* BackgroundFetchBridge::From(
  ...
  if (!bridge) {
    ...
    Supplement<ServiceWorkerRegistration>::ProvideTo(*service_worker_registration,
...
void BackgroundFetchBridge::UpdateUI(
  ...
  GetService()->UpdateUI(GetSupplementable()->WebRegistration()->RegistrationId(),
...
void BackgroundFetchBridge::GetTags(std::unique_ptr<GetTagsCallback> callback) {
  GetService()->GetTags(GetSupplementable()->WebRegistration()->RegistrationId(),
                        GetSecurityOrigin(),
                        ConvertToBaseCallback(std::move(callback)));
}
