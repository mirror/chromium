...
bool HasBackgroundImage(const Element* elem) {
  DCHECK(elem);
  const LayoutObject* layout = elem->GetLayoutObject();
  if (!layout)
    return false;

  return layout->StyleRef().HasBackgroundImage();
}

bool IsWhitespaceIncludingNbsp(UChar c) {
...
void ProcessTextInElement(const String& elem_text,
                          ...
                          StyleInfo& info) {
  const String text = elem_text.StripWhiteSpace(IsWhitespaceIncludingNbsp);
  if (text.IsEmpty())
    return;
...
void VisitImageElement(const Element* image, StyleInfo& info) {
...
bool VisitNode(const Node* node, StyleInfo& info) {
  ...
  if (node->IsElementNode()) {
    ...
    if (IsHTMLImageElement(elem) || elem->IsSVGElement()) {
      VisitImageElement(elem, info);
      return false;
    }

    if (HasBackgroundImage(elem)) {
      VisitImageElement(elem, info);
      return true;
    }
...
void StyleCollector::GetStyleImmediatelyForTesting(
    GetStyleImmediatelyForTestingCallback callback) {
  GetStyleImmediately(std::move(callback), std::numeric_limits<double>::max());
}
