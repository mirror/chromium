...
SkBitmap NotificationImageLoader::ScaleDownIfNeeded(const SkBitmap& image,
  ...
  if (image.width() > max_width_px || image.height() > max_height_px) {
    double scale = std::min(static_cast<double>(max_width_px) / image.width(),
...
void NotificationImageLoader::Start(
  ...
  threadable_loader_options.cross_origin_request_policy = kAllowCrossOriginRequests;
  ...
  threadable_loader_ = ThreadableLoader::Create(
      *execution_context, this, threadable_loader_options, resource_loader_options);
...
void NotificationImageLoader::DidFinishLoading(unsigned long resource_identifier,
                                               double finish_time) {
