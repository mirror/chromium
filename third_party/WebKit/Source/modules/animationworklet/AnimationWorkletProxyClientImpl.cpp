...
void AnimationWorkletProxyClientImpl::MutateWithEvent(
    ...
    std::unique_ptr<AutoSignal> is_done) {
  if (!global_scope_)
    return;
  ...
  PostCrossThreadTask(
      *task_runner_, FROM_HERE,
      CrossThreadBind(&AnimationWorkletProxyClientImpl::MutateWithEvent,
                      ...
                      WTF::Passed(std::make_unique<AutoSignal>(&is_done))));
  is_done.Wait();
}
...
AnimationWorkletProxyClientImpl::AutoSignal::AutoSignal(WaitableEvent* event)
    : event_(event) {
  DCHECK(event);
}

AnimationWorkletProxyClientImpl::AutoSignal::~AutoSignal() {
  event_->Signal();
}
