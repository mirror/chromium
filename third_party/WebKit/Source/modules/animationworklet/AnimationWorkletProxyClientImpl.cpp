...
void MutateTask(AnimationWorkletProxyClientImpl* client,
                const CompositorMutatorInputState* input_state,
                std::unique_ptr<CompositorMutatorOutputState>* output_state,
                WaitableEvent* is_done) {
  client->MutateWithEvent(input_state, output_state, is_done);
}
}
    ...
    AnimationWorkletThread::GetSharedBackingThread()
        ...
        ->PostTask(
            ...
            ConvertToBaseCallback(WTF::Bind(
                &MutateTask,
                CrossThreadPersistent<AnimationWorkletProxyClientImpl>(this),
