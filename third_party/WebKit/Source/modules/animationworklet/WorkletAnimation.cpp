...
bool CheckElementsComposited(const Element& target,
                             const DocumentTimelineOrScrollTimeline& timeline) {
  if (!target.GetLayoutObject() ||
      target.GetLayoutObject()->GetCompositingState() == kNotComposited)
    return false;

  if (timeline.IsScrollTimeline()) {
    const Element& scroller = *timeline.GetAsScrollTimeline()->scrollSource();
    if (!scroller.GetLayoutObject() ||
        scroller.GetLayoutObject()->GetCompositingState() == kNotComposited)
      return false;
  }
...
bool WorkletAnimation::StartOnCompositor() {
  ...
  bool can_start_on_compositor =
      target_effect->CheckCanStartAnimationOnCompositor(animation_playback_rate)
          .Ok();

  if (!can_start_on_compositor || !CheckElementsComposited(target, timeline_)) {
    play_state_ = Animation::kIdle;
    return false;
  }
