 ...
 public:
  ...
  WebRemotePlaybackClient* CreateWebRemotePlaybackClient(
    ...
    if (!remote_playback_client_) {
      remote_playback_client_ = WTF::WrapUnique(new MockWebRemotePlaybackClient);
    }
...
void MediaControlsImplTest::MouseDownAt(WebFloatPoint pos) {
  WebMouseEvent mouse_down_event(WebInputEvent::kMouseDown, pos /* client pos */,
                               pos /* screen pos */,
...
void MediaControlsImplTest::MouseMoveTo(WebFloatPoint pos) {
  WebMouseEvent mouse_move_event(WebInputEvent::kMouseMove, pos /* client pos */,
                               pos /* screen pos */,
                               WebPointerProperties::Button::kLeft, 1,
                               WebInputEvent::Modifiers::kLeftButtonDown,
                               WebInputEvent::kTimeStampForTesting);
  mouse_move_event.SetFrameScale(1);
  GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_event, {});
}
...
TEST_F(MediaControlsImplTest, TimelineMetricsWidth) {
  ...
  GetHistogramTester().ExpectTotalCount("Media.Timeline.Width.InlinePortrait", 0);
  GetHistogramTester().ExpectTotalCount("Media.Timeline.Width.FullscreenLandscape",
                                     0);
  GetHistogramTester().ExpectTotalCount("Media.Timeline.Width.FullscreenPortrait",
                                     0);
}
...
TEST_F(MediaControlsImplTest, MAYBE_TimelineMetricsClick) {
  ...
  WebFloatPoint track_center(timeline_rect->left() + timeline_rect->width() / 2,
  ...
  GetHistogramTester().ExpectTotalCount("Media.Timeline.DragPercent.128_255", 0);
  GetHistogramTester().ExpectTotalCount(
      "Media.Timeline.DragSumAbsTimeDelta.128_255", 0);
  GetHistogramTester().ExpectTotalCount("Media.Timeline.DragTimeDelta.128_255", 0);
}
...
TEST_F(MediaControlsImplTest, MAYBE_TimelineMetricsDragFromCurrentPosition) {
  ...
  GetHistogramTester().ExpectUniqueSample("Media.Timeline.DragTimeDelta.128_255",
                                       40 /* [4m, 8m) */, 1);
}
