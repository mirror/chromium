...
class MediaControlLoadingPanelElement::AnimationEventListener final
 ...
 private:
  ...
  HTMLDivElement& Object() { return panel_->AnimationEventListenerObject(); };
...
void MediaControlLoadingPanelElement::UpdateDisplayState() {
  if (GetMediaControls().State() ==
          MediaControlsImpl::ControlsState::kLoadingMetadata &&
      state_ == State::kHidden && !MediaElement().paused()) {
    // Show the loading panel and insert it into the DOM.
    PopulateShadowDOM();
    SetIsWanted(true);
    SetAnimationIterationCount(kInfinite);
    state_ = State::kPlaying;
  } else if (state_ == State::kPlaying &&
             GetMediaControls().State() ==
                 MediaControlsImpl::ControlsState::kPlaying) {
    // If the loading panel is playing then we should stop the animations, but
    // do not hide it completely until the animations have fully stopped.
