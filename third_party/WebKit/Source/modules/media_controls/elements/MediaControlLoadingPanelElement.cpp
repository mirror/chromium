...
static const char kBackgroundColorName[] = "background-color";
static const char kInfinite[] = "infinite";
static const char kNoFrameAvailableBackgroundColor[] = "black";
...
void MediaControlLoadingPanelElement::PopulateShadowDOM() {
  ...
  SetSpinnerBackgroundColorIfNecessary();
...
void MediaControlLoadingPanelElement::SetSpinnerBackgroundColorIfNecessary() {
  if (!spinner_)
    return;
  ...
  if (!video_element.ShouldDisplayPosterImage() &&
    ...
    spinner_->style()->setProperty(&GetDocument(), kBackgroundColorName,
                                   kNoFrameAvailableBackgroundColor, "",
                                   ASSERT_NO_EXCEPTION);
  } else {
    spinner_->style()->removeProperty(kBackgroundColorName,
                                      ASSERT_NO_EXCEPTION);
  }
}
