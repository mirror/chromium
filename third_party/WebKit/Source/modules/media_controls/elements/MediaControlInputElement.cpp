...
const char kOverflowParagraphWithSubtitleCSSClass[] = "with-subtitle";
...
HTMLElement* MediaControlInputElement::CreateOverflowElement(
  ...
  if (MediaControlsImpl::IsModern()) {
    overflow_menu_paragraph_ = HTMLParagraphElement::Create(GetDocument());

    overflow_menu_paragraph_->AppendChild(overflow_menu_text_);

    UpdateOverflowSubtitleElement(button->GetOverflowMenuSubtitleString());

    element->AppendChild(overflow_menu_paragraph_);
  } else {
...
void MediaControlInputElement::UpdateOverflowSubtitleElement(String text) {
  DCHECK(overflow_menu_paragraph_);
  ...
  if (overflow_menu_subtitle_) {
    ...
    overflow_menu_paragraph_->AppendChild(overflow_menu_subtitle_);
    overflow_menu_paragraph_->setAttribute(
        "class", kOverflowParagraphWithSubtitleCSSClass);
  }
}

void MediaControlInputElement::RemoveOverflowSubtitleElement() {
  if (overflow_menu_subtitle_) {
    overflow_menu_paragraph_->RemoveChild(overflow_menu_subtitle_);
    overflow_menu_paragraph_->removeAttribute("class");
...
void MediaControlInputElement::Trace(blink::Visitor* visitor) {
  ...
  visitor->Trace(overflow_menu_paragraph_);
