...
int32_t ConvertTimeDeltaToSample(TimeDelta delta) {
  return static_cast<int32_t>(delta.InSeconds());
}
...
void MediaControlOverflowMenuListElement::SetIsWanted(bool wanted) {
  ...
  if (wanted) {
    ...
    DEFINE_STATIC_LOCAL(
        ...
        ("Media.Controls.Overflow.TimeToAction", 0,
         ConvertTimeDeltaToSample(TimeDelta::FromMinutes(3)), 181));
    histogram.Count(ConvertTimeDeltaToSample(time_taken));
    time_shown_.reset();
  }
}
