...
bool IsPointInRect(blink::DOMRect& rect, int margin, int x, int y) {
  return (
      (x >= rect.x() - margin) && (x <= (rect.x() + rect.height() + margin)) &&
      (y >= rect.y() - margin) && (y <= (rect.y() + rect.width() + margin)));
}
...
constexpr int kNumberOfSecondsToJump = 15;
...
void MediaControlOverlayPlayButtonElement::MaybeJump(int seconds) {
  double new_time = MediaElement().currentTime() + seconds;
  if (new_time >= 0 && new_time < MediaElement().duration())
...
void MediaControlOverlayPlayButtonElement::DefaultEventHandler(Event* event) {
  if (event->type() == EventTypeNames::click) {
    ...
    if (IsPointInRect(*internal_button_->getBoundingClientRect(),
      ...
      // Otherwise see which side it happened on a trigger a jump event.
