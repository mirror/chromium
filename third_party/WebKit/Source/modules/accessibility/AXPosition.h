...
// Positions are made up of an |AXObject| and an optional child or text offset
// in that object.
class AXPosition final {
 ...
 public:
  ...
  explicit AXPosition(const Position&);

  explicit AXPosition(const PositionWithAffinity& position)
  ...
  PositionWithAffinity AsPositionWithAffinity() const;
...
bool operator==(const AXPosition& a, const AXPosition& b) {
  if (!a.IsValid() || !b.IsValid())
    return false;
  return a.AnchorObject() == b.AnchorObject() && a.Offset() == b.Offset() &&
