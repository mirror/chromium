...
const AXPosition AXPosition::FromPosition(const Position& position) {
  ...
  if (anchor_node->IsTextNode()) {
    ...
    // character offset.
    // TODO(xiaochengh): Use LayoutNG offset mapping instead of |TextIterator|.
...
bool AXPosition::IsValid() const {
  ...
  if (offset_ && !container_object_->GetNode()->IsTextNode())
    return false;
  if (child_index_) {
    if (child_index_ > static_cast<int>(container_object_->Children().size()))
      return false;
    // TODO(nektar): Following check might not belong here.
    const AXObject* ax_child =
