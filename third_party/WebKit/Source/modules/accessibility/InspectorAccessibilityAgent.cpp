  ...
  if (!results.IsEmpty())
    properties.addItem(
        CreateRelatedNodeListProperty(AXRelationshipAttributesEnum::Describedby,
                                      results, aria_describedbyAttr, ax_object));
  results.Clear();
...
std::unique_ptr<AXNode> InspectorAccessibilityAgent::BuildObjectForIgnoredNode(
  ...
  std::unique_ptr<AXNode> ignored_node_object =
      AXNode::create().setNodeId(String::Number(ax_id)).setIgnored(true).build();
...
void InspectorAccessibilityAgent::FillCoreProperties(
  ...
  if (fetch_relatives)
    PopulateRelatives(ax_object, inspected_ax_object, node_object, nodes, cache);
