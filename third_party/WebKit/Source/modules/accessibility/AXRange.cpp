...
template <Strategy>
SelectionTemplate<Strategy> AXRange::AsSelection() const {
  if (!IsValid())
    return {};

  const auto base = anchor_.AsPositionInFlatTreeWithAffinity();
  const auto extent = focus_.AsPositionInFlatTreeWithAffinity();
  SelectionTemplate<Strategy>::Builder selection_builder;
