...
AXRange::Builder& AXRange::Builder::SetExtent(const Position& extent) {
  ...
  range_.extent_ = ax_extent_;
  return *this;
}

AXRange::Builder& SetRange(const SelectionInDOMTree& selection) {
  if (selection.IsNone())
    return *this;

  base_ = AXPosition(selection.Base());
  extent_ = AXPosition(selection.Extent());
  return *this;
}

const AXRange AXRange::Builder::Build() {
  if (!range_.base_.IsValid() || !range_.extent_.IsValid())
    return {};
