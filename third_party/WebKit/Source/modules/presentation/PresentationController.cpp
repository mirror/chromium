...
void PresentationController::GetAvailability(
  ...
  if (!listener) {
    ...
    availability_set_.emplace_back(listener);
  }
  ...
  // TODO(imcheng): What's the point of creating the listener and invoking this
  // if we posted task to invoke the callback? Possibly an optimization in case
  // the page decides to listen for availability?
  for (const auto& availability_url : urls)
    StartListeningToURL(availability_url);
}

void PresentationController::StartListening(
...
void PresentationController::StopListening(
    PresentationAvailabilityObserver* observer) {
  const auto& urls = observer->Urls();
  auto* listener = GetAvailabilityListener(urls);
  if (!listener) {
...
void PresentationController::StartListeningToURL(const KURL& url) {
  ...
  if (!listening_status) {
    ...
    listening_status_.emplace_back(listening_status);
  }
...
PresentationController::GetListeningStatus(const KURL& url) const {
  auto status_it =
      std::find_if(listening_status_.begin(), listening_status_.end(),
                   [&url](const std::unique_ptr<ListeningStatus>& status) {
                     return status->url == url;
                   });
  return status_it == listening_status_.end() ? nullptr : status_it->get();
}
