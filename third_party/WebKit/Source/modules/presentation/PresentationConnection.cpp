...
void PresentationConnection::OnClose() {
  ...
  // supplying a callback to OnClose() and invoking it here.
  target_connection_->DidChangeState(
      mojom::blink::PresentationConnectionState::CLOSED);
}

void PresentationConnection::InitForController() {
  NOTIMPLEMENTED();
}
...
void ReceiverPresentationConnection::OnReceiverTerminated() {
  // We don't use DidChangeState here because we do not want to dispatch an
  // event.
...
void PresentationConnection::send(DOMArrayBuffer* array_buffer,
  ...
  if (array_buffer->ByteLength() > kMaxPresentationConnectionMessageSize) {
    // TODO(crbug.com/459008): Same as in send(String).
    LOG(WARNING) << "ArrayBuffer message size exceeded limit!";
    return;
  }
...
void PresentationConnection::DidFinishLoadingBlob(DOMArrayBuffer* buffer) {
  ...
  if (buffer->ByteLength() > kMaxPresentationConnectionMessageSize) {
    // TODO(crbug.com/459008): Same as in send(String).
