...
void PresentationAvailabilityState::AddAvailabilityCallbacks(
...
void PresentationAvailabilityState::StartListeningForAvailability(
...
void PresentationAvailabilityState::StopListeningForAvailability(
    PresentationAvailabilityObserver* observer) {
  const auto& urls = observer->Urls();
  auto* listener = GetAvailabilityListener(urls);
  if (!listener) {
...
void PresentationAvailabilityState::OnScreenAvailabilityUpdated(
    const KURL& url,
...
void PresentationAvailabilityState::StartListeningToURL(const KURL& url) {
  ...
  DoListenForScreenAvailability(url);
}
...
void PresentationAvailabilityState::MaybeStopListeningToURL(const KURL& url) {
  ...
  DoStopListeningForScreenAvailability(url);
}

void PresentationAvailabilityState::DoListenForScreenAvailability(
    const KURL& url) {
  presentation_service_->ListenForScreenAvailability(url);
}

void PresentationAvailabilityState::DoStopListeningForScreenAvailability(
    const KURL& url) {
