...
class PresentationAvailabilityStateTest : public ::testing::Test {
 public:
  ...
  void ChangeURLState(const KURL& url, ScreenAvailability state) {
    switch (state) {
      case ScreenAvailability::AVAILABLE:
        state_.UpdateAvailability(url, ScreenAvailability::AVAILABLE);
        break;
      case ScreenAvailability::SOURCE_NOT_SUPPORTED:
        state_.UpdateAvailability(url,
                                  ScreenAvailability::SOURCE_NOT_SUPPORTED);
        break;
      case ScreenAvailability::UNAVAILABLE:
        state_.UpdateAvailability(url, ScreenAvailability::UNAVAILABLE);
        break;
      case ScreenAvailability::DISABLED:
        state_.UpdateAvailability(url, ScreenAvailability::DISABLED);
        break;
  ...
  void TestRequestAvailability(
    ...
    state_.RequestAvailability(urls, base::WrapUnique(mock_callback));
...
TEST_F(PresentationAvailabilityStateTest, TestListenForScreenAvailability) {
  ...
  state_.RemoveObserver(&observer_);

  // After RemoveObserver(), |observer_| should no longer be
  // notified.
  EXPECT_CALL(observer_, AvailabilityChanged(ScreenAvailability::UNAVAILABLE))
      .Times(0);
  state_.UpdateAvailability(url1_, ScreenAvailability::UNAVAILABLE);
