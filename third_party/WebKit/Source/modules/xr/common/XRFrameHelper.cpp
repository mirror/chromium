...
XRFrameHelper::GetRequestPresentOptions(
...
scoped_refptr<Image> XRFrameHelper::CaptureImage(
    WebGLRenderingContextBase* rendering_context) {
  TRACE_EVENT_BEGIN0("gpu", "XRFrameHelper:GetStaticBitmapImage");
  scoped_refptr<Image> image_ref = rendering_context->GetStaticBitmapImage();
  TRACE_EVENT_END0("gpu", "XRFrameHelper::GetStaticBitmapImage");
  ...
  if (!image_ref.get() || !image_ref->IsTextureBacked()) {
    TRACE_EVENT0("gpu", "XRFrameHelper::GetImage_SlowFallback");
