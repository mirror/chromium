...
ServiceWorkerInstalledScriptsManager::GetScriptData(const KURL& script_url) {
  ...
  TRACE_EVENT2("ServiceWorker", __func__, "script chunks",
               raw_script_data->script_text.size(), "meta data chunks",
               raw_script_data->meta_data.size());
  if (!raw_script_data)
    return WTF::nullopt;
  ...
  if (raw_script_data->meta_data.size() > 0) {
    ...
    TRACE_EVENT1("ServiceWorker", __func__, "meta data size",
                 script_data.meta_data->size());
  }
