...
class Receiver {
 public:
  ...
  WebVector<BytesChunk> TakeChunks() {
    DCHECK(!is_running());
    return WebVector<BytesChunk>(std::move(chunks_));
  }
 ...
 private:
  ...
  // std::vector is internally used because WebVector is immutable and
  // cannot append data.
...
class Internal : public mojom::blink::ServiceWorkerInstalledScriptsManager {
 public:
  ...
  void OnScriptReceived(mojom::blink::ServiceWorkerScriptInfoPtr script_info) {
    DCHECK_CALLED_ON_VALID_THREAD(io_thread_checker_);
    const KURL& script_url = script_info->script_url;
    ...
    auto script_data = RawScriptData::Create(
        WebString(script_info->encoding), receivers->body()->TakeChunks(),
        receivers->meta_data()->TakeChunks());
    for (auto it : script_info->headers) {
      script_data->AddHeader(WebString(it.key), WebString(it.value));
    }
...
ServiceWorkerInstalledScriptsManager::ServiceWorkerInstalledScriptsManager(
  ...
  for (WebURL& url : installed_urls)
    installed_urls_.insert(std::move(url));
  mojom::blink::ServiceWorkerInstalledScriptsManagerHostPtrInfo ptr_info(
  ...
  mojom::blink::ServiceWorkerInstalledScriptsManagerRequest request(
      std::move(manager_request));
  Platform::Current()->GetIOTaskRunner()->PostTask(
      FROM_HERE,
      base::BindOnce(&Internal::Create, script_container_, std::move(request)));
}
