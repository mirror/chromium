 ...
 protected:
  void SetUp() override {
    ...
    mock_installed_scripts_manager_ =
        new MockServiceWorkerInstalledScriptsManager();
    worker_ = WTF::WrapUnique(WebEmbeddedWorker::Create(
        WTF::WrapUnique(
            static_cast<WebServiceWorkerContextClient*>(mock_client_)),
        WTF::WrapUnique(static_cast<WebServiceWorkerInstalledScriptsManager*>(
            mock_installed_scripts_manager_)),
        nullptr));
...
TEST_F(WebEmbeddedWorkerImplTest, MAYBE_DontPauseAfterDownload) {
  ...
  EXPECT_CALL(*mock_client_, CreateServiceWorkerProviderProxy())
      .WillOnce(::testing::Return(nullptr));
  // This would be called on the worker thread.
  EXPECT_CALL(*mock_installed_scripts_manager_,
              IsScriptInstalled(start_data_.script_url))
      .Times(1)
      .WillOnce(::testing::Return(false));
