...
void FetchRespondWithObserver::OnResponseFulfilled(const ScriptValue& value) {
  ...
  if (!V8Response::hasInstance(value.V8Value(),
    ...
    OnResponseRejected(ServiceWorkerResponseError::NoV8Instance);
    return;
  }
  ...
  if (response_type == network::mojom::FetchResponseType::kError) {
    OnResponseRejected(ServiceWorkerResponseError::ResponseTypeError);
    return;
  }
  if (response_type == network::mojom::FetchResponseType::kOpaque) {
    if (request_mode_ != network::mojom::FetchRequestMode::kNoCORS) {
      OnResponseRejected(ServiceWorkerResponseError::ResponseTypeOpaque);
      return;
    }
    ...
    if (IsClientRequest(frame_type_, request_context_)) {
      OnResponseRejected(
          ServiceWorkerResponseError::ResponseTypeOpaqueForClientRequest);
      return;
    }
  }
  if (redirect_mode_ != WebURLRequest::kFetchRedirectModeManual &&
      response_type == network::mojom::FetchResponseType::kOpaqueRedirect) {
    OnResponseRejected(ServiceWorkerResponseError::ResponseTypeOpaqueRedirect);
    return;
  }
  if (redirect_mode_ != WebURLRequest::kFetchRedirectModeFollow &&
      response->redirected()) {
    OnResponseRejected(
        ServiceWorkerResponseError::RedirectedResponseForNotFollowRequest);
    return;
  }
  if (response->IsBodyLocked()) {
    OnResponseRejected(ServiceWorkerResponseError::BodyLocked);
    return;
  }
  if (response->bodyUsed()) {
    OnResponseRejected(ServiceWorkerResponseError::BodyUsed);
    return;
  }
  ...
  if (buffer) {
    ...
    if (result != MOJO_RESULT_OK) {
      OnResponseRejected(ServiceWorkerResponseError::DataPipeCreationFailed);
      return;
    }
