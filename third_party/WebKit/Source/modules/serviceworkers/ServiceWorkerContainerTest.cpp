...
void ExpectRejected(ScriptState* script_state,
  ...
  promise.Then(resolved.GetFunction(script_state), rejected.GetFunction(script_state));
...
class ExpectDOMException : public ScriptValueTest {
 public:
  ExpectDOMException(const String& expected_name, const String& expected_message)
  ...
  void operator()(ScriptValue value) const override {
    DOMException* exception =
        V8DOMException::toImplWithTypeCheck(value.GetIsolate(), value.V8Value());
...
class ServiceWorkerContainerTest : public ::testing::Test {
 protected:
  ...
  void SetPageURL(const String& url) {
    ...
    page_->GetDocument().SetSecurityOrigin(SecurityOrigin::CreateFromString(url));
  }
