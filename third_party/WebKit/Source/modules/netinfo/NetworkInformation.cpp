...
double NetworkInformation::GetRandomMultiplier() const {
  ...
  // cross-origin fingerprinting.
  unsigned hash =
      StringHash::GetHash(GetExecutionContext()->Url().Host()) * salt_;
  double random_multiplier = 0.9 + static_cast<double>((hash % 20)) * 0.01;
...
unsigned long NetworkInformation::RoundRtt(
  ...
  // rtt_msec += randomize_rtt();
  if (rtt.value().InMilliseconds() >= kMaxRttMsec)
    rtt_msec = 3000;

  double random_multiplier = GetRandomMultiplier();
  rtt_msec *= random_multiplier;
...
double NetworkInformation::RoundMbps(
  ...
  if (!downlink_mbps.has_value()) {
    ...
    downlink_kbps = std::max(downlink_mbps.value() * 1000, kMaxDownlinkKbps);
  }

  double random_multiplier = GetRandomMultiplier();
  downlink_kbps *= random_multiplier;
