...
unsigned long NetworkInformation::RoundRtt(
  ...
  // fingerprinting.
  LOG(WARNING) << "xxx NetworkInformation::RoundRtt cp0";
  ...
  if (!rtt.has_value() || !GetExecutionContext()) {
    ...
    LOG(WARNING) << "xxx NetworkInformation::RoundRtt cp1";
    return 0;
  }

  double rtt_msec = static_cast<double>(rtt.value().InMilliseconds());
  LOG(WARNING) << "xxx NetworkInformation::RoundRtt cp2 rtt_msec=" << rtt_msec;

  rtt_msec *= GetRandomMultiplier();
  LOG(WARNING) << "xxx NetworkInformation::RoundRtt cp3 rtt_msec=" << rtt_msec;

  rtt_msec = std::min(rtt_msec, kMaxRttMsec);

  LOG(WARNING) << "xxx NetworkInformation::RoundRtt cp4 rtt_msec=" << rtt_msec;

  LOG(WARNING) << "xxx NetworkInformation::RoundRtt cp5 rounded rtt_msec="
               << std::round(rtt_msec / kBucketSize) * kBucketSize;

  DCHECK_LE(0, rtt_msec);
  DCHECK_GE(kMaxRttMsec, rtt_msec);

  LOG(WARNING) << "xxx NetworkInformation::RoundRtt cp6 rounded rtt_msec="
               << std::round(rtt_msec / kBucketSize) * kBucketSize;
...
double NetworkInformation::RoundMbps(
  ...
  LOG(WARNING) << "xxx NetworkInformation::RoundMbps cp0";

  double downlink_kbps = 0;
  if (!downlink_mbps.has_value()) {
    LOG(WARNING) << "xxx NetworkInformation::RoundMbps cp1";
    // Throughput is unavailable. So, return the fastest value.
    downlink_kbps = kMaxDownlinkKbps;
  } else {
    LOG(WARNING) << "xxx NetworkInformation::RoundMbps cp2";
    downlink_kbps = downlink_mbps.value() * 1000;
  }
  LOG(WARNING) << "xxx NetworkInformation::RoundMbps cp3 downlink_kbps="
               << downlink_kbps;
  downlink_kbps *= GetRandomMultiplier();

  downlink_kbps = std::min(downlink_kbps, kMaxDownlinkKbps);

  LOG(WARNING) << "xxx NetworkInformation::RoundMbps cp4 downlink_kbps="
               << downlink_kbps;
  ...
  LOG(WARNING) << "xxx NetworkInformation::RoundMbps cp5 downlink_kbps_rounded="
               << downlink_kbps_rounded;
