...
TEST_F(RemotePlaybackTest, IsListening) {
  ...
  remote_playback->SourceChanged(
      WebURL(KURL(kParsedURLString, "http://www.example.com")), true, true);
  ...
  remote_playback->SourceChanged(WebURL(), false, false);
  ASSERT_TRUE(remote_playback->Urls().empty());
  ASSERT_FALSE(IsListening(remote_playback));

  remote_playback->SourceChanged(WebURL(KURL(kParsedURLString, "@$@#@#")), true,
                                 true);
  ASSERT_TRUE(remote_playback->Urls().empty());
  ASSERT_FALSE(IsListening(remote_playback));
