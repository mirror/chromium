...
class MediaDevicesUnderTest : public MediaDevices {
 public:
  static MediaDevicesUnderTest* Create(ExecutionContext* context) {
    MediaDevicesUnderTest* media_devices = new MediaDevicesUnderTest(context);
  ...
  void DevicesEnumerated(
    ...
    device_infos_ = media_devices;
    devices_enumerated_ = true;
  }

  void OnDispatcherHostConnectionError() { connection_error_ = true; }

  const MediaDeviceInfoVector& device_infos() const { return device_infos_; }
  ...
  void Trace(blink::Visitor* visitor) override {
    visitor->Trace(device_infos_);
    MediaDevices::Trace(visitor);
  }

 private:
  explicit MediaDevicesUnderTest(ExecutionContext* context)
      : MediaDevices(context) {}

  MediaDeviceInfoVector device_infos_;
...
class MediaDevicesTest : public ::testing::Test {
 public:
  ...
  MediaDevicesUnderTest* GetMediaDevices(ExecutionContext* context) {
    if (!media_devices_) {
      media_devices_ = MediaDevicesUnderTest::Create(context);
 ...
 private:
  ...
  Persistent<MediaDevicesUnderTest> media_devices_;
};
...
TEST_F(MediaDevicesTest, EnumerateDevices) {
  ...
  EXPECT_TRUE(media_devices->devices_enumerated());
  EXPECT_EQ(static_cast<size_t>(5), media_devices->device_infos().size());

  // Audio input device with matched output ID.
  Member<MediaDeviceInfo> device = media_devices->device_infos()[0];
  ...
  device = media_devices->device_infos()[1];
  EXPECT_FALSE(device->deviceId().IsEmpty());
  EXPECT_EQ("audioinput", device->kind());
  EXPECT_FALSE(device->label().IsEmpty());
  EXPECT_FALSE(device->groupId().IsEmpty());
  ...
  // Video input devices.
  device = media_devices->device_infos()[2];
  EXPECT_FALSE(device->deviceId().IsEmpty());
  EXPECT_EQ("videoinput", device->kind());
  EXPECT_FALSE(device->label().IsEmpty());
  EXPECT_TRUE(device->groupId().IsEmpty());

  device = media_devices->device_infos()[3];
  EXPECT_FALSE(device->deviceId().IsEmpty());
  EXPECT_EQ("videoinput", device->kind());
  EXPECT_FALSE(device->label().IsEmpty());
  EXPECT_TRUE(device->groupId().IsEmpty());
  ...
  // Audio output device.
  device = media_devices->device_infos()[4];
  EXPECT_FALSE(device->deviceId().IsEmpty());
  ...
  // Verfify group IDs.
  EXPECT_EQ(media_devices->device_infos()[0]->groupId(),
            media_devices->device_infos()[4]->groupId());
  EXPECT_NE(media_devices->device_infos()[1]->groupId(),
            media_devices->device_infos()[4]->groupId());
}
...
TEST_F(MediaDevicesTest, EnumerateDevicesAfterConnectionError) {
  V8TestingScope scope;
  auto media_devices = GetMediaDevices(scope.GetExecutionContext());
  EXPECT_FALSE(media_devices->connection_error());
  ...
  platform()->RunUntilIdle();
  ASSERT_FALSE(promise.IsEmpty());
  EXPECT_TRUE(media_devices->connection_error());
  EXPECT_FALSE(media_devices->devices_enumerated());
}
