...
enum class RequireOrigin { kSecure, kSecureAndSameAsAncestors };
...
bool CheckSecurityRequirements(ScriptPromiseResolver* resolver,
  ...
  if (!document || !document->GetFrame()) {
    resolver->Reject(DOMException::Create(
        kNotAllowedError,
        "Credential manager unavailable in current browsing context."));
    return false;
  }

  if (require_origin == RequireOrigin::kSecureAndSameAsAncestors &&
...
void AssertSecurityRequirements(ScriptPromiseResolver* resolver,
                                RequireOrigin require_origin) {
  const auto* context = ExecutionContext::From(resolver->GetScriptState());
  SECURITY_CHECK(context);
  SECURITY_CHECK(context->IsSecureContext());

  auto* document = ToDocumentOrNull(context);
  SECURITY_CHECK(document);

  if (document->GetFrame()) {
    SECURITY_CHECK(require_origin != RequireOrigin::kSecureAndSameAsAncestors ||
                   IsSameOriginWithAncestors(document->GetFrame()));
  }
}
...
void OnStoreComplete(std::unique_ptr<ScopedPromiseResolver> scoped_resolver) {
  ...
  AssertSecurityRequirements(resolver,
                             RequireOrigin::kSecureAndSameAsAncestors);
  resolver->Resolve();
}
...
void OnPreventSilentAccessComplete(
  ...
  auto* resolver = scoped_resolver->Release();
  AssertSecurityRequirements(resolver, RequireOrigin::kSecure);
  resolver->Resolve();
}
...
ScriptPromise CredentialsContainer::get(
  ...
  auto require_origin = options.hasFederated() || options.hasPassword()
                            ? RequireOrigin::kSecureAndSameAsAncestors
                            : RequireOrigin::kSecure;

  if (!CheckSecurityRequirements(resolver, require_origin))
...
ScriptPromise CredentialsContainer::create(
  ...
  ScriptPromiseResolver* resolver = ScriptPromiseResolver::Create(script_state);
  if (!CheckSecurityRequirements(resolver, RequireOrigin::kSecure))
    return resolver->Promise();
  ...
  if (options.hasPassword()) {
    ...
    auto mojo_options = ::webauth::mojom::blink::MakeCredentialOptions::From(
        options.publicKey());
    if (mojo_options) {
      auto* authenticator =
