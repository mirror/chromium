...
DOMException* CredentialManagerErrorToDOMException(
  ...
  switch (reason) {
    case CredentialManagerError::DISABLED:
      error = {kInvalidStateError, "The credential manager is disabled."};
      break;
    case CredentialManagerError::PENDING_REQUEST:
      error = {kInvalidStateError, "A 'get()' request is pending."};
      break;
    case CredentialManagerError::PASSWORD_STORE_UNAVAILABLE:
      error = {kNotSupportedError, "The password store is unavailable."};
      break;
    case CredentialManagerError::NOT_ALLOWED:
      error = {kNotAllowedError, "The operation is not allowed."};
      break;
    ...
    case CredentialManagerError::NOT_SECURE:
      error = {kSecurityError, "The operation is insecure and is not allowed."};
      break;
    case CredentialManagerError::CANCELLED:
      error = {kNotAllowedError, "The user cancelled the operation."};
      break;
    case CredentialManagerError::NOT_IMPLEMENTED:
      error = {kNotAllowedError, "The operation is not implemented."};
      break;
  ...
  DCHECK(error.message);
  return DOMException::Create(error.code, error.message);
}
...
void OnGetComplete(std::unique_ptr<AutoPromiseResolver> scoped_resolver,
  ...
  DCHECK(scoped_resolver);
  ScriptPromiseResolver* resolver = scoped_resolver->Release();
  // TODO: Add Same Origin requirement.
