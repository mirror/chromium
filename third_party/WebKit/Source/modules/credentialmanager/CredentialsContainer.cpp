...
bool CheckPublicKeySecurityRequirements(ScriptPromiseResolver* resolver,
  ...
  DCHECK(origin->Protocol() != url::kAboutScheme);
  DCHECK(origin->Protocol() != url::kFileScheme);

  // Validate the effective domain.
  // For step 6 of both
  // https://w3c.github.io/webauthn/#createCredentialre and
...
void OnMakePublicKeyCredentialComplete(
  ...
  // TODO(crbug.com/803080): Introduce the asset counterpart of
...
ScriptPromise CredentialsContainer::create(
  ...
  if (options.hasPassword()) {
    ...
    if (!CheckPublicKeySecurityRequirements(resolver,
