  ...
  void OnSuccess(
    ...
    if (!credential->client_data_json.IsEmpty() ||
      ...
      DOMArrayBuffer* client_data_buffer =
          VectorToDOMArrayBuffer(credential->client_data_json);
      DOMArrayBuffer* attestation_buffer =
          VectorToDOMArrayBuffer(credential->response->attestation_object);
      DOMArrayBuffer* raw_id = VectorToDOMArrayBuffer(credential->raw_id);
