...
class ErrorWebCacheForTests : public WebServiceWorkerCache {
 ...
 private:
  static void CompareQueryParamsForTest(const QueryParams& expected_query_params,
...
TEST_F(CacheStorageTest, BasicArguments) {
  ...
  ASSERT(request);
  ScriptPromise match_result = cache->match(
      GetScriptState(), RequestToRequestInfo(request), options, exception_state);
  ...
  ASSERT(request);
  ScriptPromise match_all_result = cache->matchAll(
      GetScriptState(), RequestToRequestInfo(request), options, exception_state);
  ...
  request = NewRequestFromUrl(url);
  ASSERT(request);
  ScriptPromise keys_result2 = cache->keys(
      GetScriptState(), RequestToRequestInfo(request), options, exception_state);
  EXPECT_EQ("dispatchKeys",
            test_cache->GetAndClearLastErrorWebCacheMethodCalled());
...
TEST_F(CacheStorageTest, BatchOperationArguments) {
      ...
      size_t(1));
  {
    ...
    delete_operation.operation_type = WebServiceWorkerCache::kOperationTypeDelete;
  ...
  ScriptPromise delete_result = cache->deleteFunction(
      GetScriptState(), RequestToRequestInfo(request), options, exception_state);
...
TEST_F(CacheStorageTest, KeysResponseTest) {
  ...
  for (int i = 0, minsize = std::min(expected_urls.size(), requests.size());
       i < minsize; ++i) {
    Request* request = V8Request::toImplWithTypeCheck(GetIsolate(), requests[i]);
    EXPECT_TRUE(request);
    if (request)
...
TEST_F(CacheStorageTest, MatchAllAndBatchResponseTest) {
  ...
  web_responses[0].SetURLList(std::vector<KURL>({KURL(kParsedURLString, url1)}));
  web_responses[0].SetResponseType(kWebServiceWorkerResponseTypeDefault);
  web_responses[1].SetURLList(std::vector<KURL>({KURL(kParsedURLString, url2)}));
