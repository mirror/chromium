...
Response AssertCacheStorageAndNameForId(
  ...
  if (!response.isSuccess())
    return response;
  return AssertCacheStorage(security_origin, std::move(result));
}
...
Response GetExecutionContext(InspectedFrames* frames,
                             const String& cache_id,
                             ExecutionContext** context) {
  String origin;
  String id;
  Response res = ParseCacheId(cache_id, &origin, &id);
  if (!res.isSuccess())
    return res;

  LocalFrame* frame = frames->FrameWithSecurityOrigin(origin);
  if (!context)
    return Response::Error("No frame with origin " + origin);

  blink::Document* document = frame->GetDocument();
  if (!document)
    return Response::Error("No execution context found");
...
void InspectorCacheStorageAgent::requestCacheNames(
  ...
  std::unique_ptr<WebServiceWorkerCacheStorage> cache;
  Response response = AssertCacheStorage(security_origin, std::move(cache));
  if (!response.isSuccess()) {
...
void InspectorCacheStorageAgent::requestEntries(
  ...
  std::unique_ptr<WebServiceWorkerCacheStorage> cache;
  Response response =
      AssertCacheStorageAndNameForId(cache_id, &cache_name, std::move(cache));
  if (!response.isSuccess()) {
    callback->sendFailure(response);
    return;
  }
...
class GetCachedResponseForRequest
    : public WebServiceWorkerCacheStorage::CacheStorageMatchCallbacks {
  WTF_MAKE_NONCOPYABLE(GetCachedResponseForRequest);
...
void InspectorCacheStorageAgent::requestCachedResponse(
  ...
  cache->DispatchMatch(WTF::MakeUnique<GetCachedResponseForRequest>(
