...
bool ShouldGenerateV8CodeCache(ScriptState* script_state,
  ...
  if (!context->IsServiceWorkerGlobalScope())
    return false;
  if (!ToServiceWorkerGlobalScope(context)->is_installing())
    return false;
...
class Cache::BarrierCallbackForPut final
 ...
 public:
  ...
  void OnSuccess(size_t index,
    ...
    MaybeReportInstalledPWAScripts();
 ...
 private:
  void MaybeReportInstalledPWAScripts() {
    ExecutionContext* context = resolver_->GetExecutionContext();
    if (!context || !context->IsServiceWorkerGlobalScope())
      return;
    ServiceWorkerGlobalScope* scope = ToServiceWorkerGlobalScope(context);
    if (!scope->is_installing())
      return;
    ...
    for (const auto& operation : batch_operations_) {
      ...
      scope->CountInstalledPWAScript(blob_data_handle->size(),
                                     operation.response.SideDataBlobSize());
    }
  }
