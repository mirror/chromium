...
V8ServiceWorkerCodeCacheInstallOptions GetServiceWorkerCodeCacheInstallOptions(
  ...
  if (!context->IsServiceWorkerGlobalScope())
    return V8ServiceWorkerCodeCacheInstallOptions::kNone;
  ServiceWorkerGlobalScope* global_scope = ToServiceWorkerGlobalScope(context);
  WorkerSettings* settings = global_scope->GetWorkerSettings();
  V8ServiceWorkerCodeCacheInstallOptions options =
      settings->GetServiceWorkerCodeCacheInstallOptions();
  if (options == V8ServiceWorkerCodeCacheInstallOptions::kNone)
    return V8ServiceWorkerCodeCacheInstallOptions::kNone;
  if (!global_scope->is_being_installed())
    return V8ServiceWorkerCodeCacheInstallOptions::kNone;
  ...
  LOG(ERROR) << "GetServiceWorkerCodeCacheInstallOptions "
 ...
 public:
  CodeCacheHandleCallbackForPut(ScriptState* script_state,
      ...
      : script_state_(script_state),
        ...
        options_(options) {
    DCHECK_NE(V8ServiceWorkerCodeCacheInstallOptions::kNone, options_);
    request->PopulateWebServiceWorkerRequest(web_request_);
    response->PopulateWebServiceWorkerResponse(web_response_);
  }
 ...
 private:
  scoped_refptr<CachedMetadata> GenerateV8CodeCache(
    ...
    v8::ScriptCompiler::CompileOptions compile_options =
        options_ == V8ServiceWorkerCodeCacheInstallOptions::kNormal
  ...
  String mime_type_;
  V8ServiceWorkerCodeCacheInstallOptions options_;
  WebServiceWorkerRequest web_request_;
  WebServiceWorkerResponse web_response_;
};
...
ScriptPromise Cache::PutImpl(ScriptState* script_state,
  ...
  for (size_t i = 0; i < requests.size(); ++i) {
    ...
    V8ServiceWorkerCodeCacheInstallOptions code_cache_install_options =
        GetServiceWorkerCodeCacheInstallOptions(script_state, responses[i]);
    if (code_cache_install_options !=
      ...
      buffer->StartLoading(loader,
                           new CodeCacheHandleCallbackForPut(
                               script_state, i, barrier_callback, requests[i],
                               responses[i], code_cache_install_options));
      continue;
    }
    if (buffer) {
