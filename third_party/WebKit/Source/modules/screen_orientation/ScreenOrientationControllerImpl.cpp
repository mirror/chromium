...
void ScreenOrientationControllerImpl::SetScreenOrientationForTests(
    device::mojom::blink::ScreenOrientationAssociatedPtr
        screen_orientation_for_tests) {
  if (delegate_) {
    delegate_->SetScreenOrientationForTests(
        std::move(screen_orientation_for_tests));
  }
}
