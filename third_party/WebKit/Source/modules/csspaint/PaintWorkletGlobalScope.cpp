...
unsigned PaintWorkletGlobalScope::instance_count_ = 0u;
...
PaintWorkletGlobalScope* PaintWorkletGlobalScope::Create(
  ...
  context_name.append(String::Number(++instance_count_));
  DCHECK_LE(instance_count_, 2u);
...
void PaintWorkletGlobalScope::registerPaint(const String& name,
  ...
  if (document_definition_map.Contains(name)) {
    ...
    existing_document_definition->IncrementRegisteredDefinitionCount();
    if (existing_document_definition == kInvalidDocumentDefinition)
      return;
    if (!existing_document_definition->Matches(*definition)) {
    ...
    if (existing_document_definition->GetRegisteredDefinitionCount() == 2u)
