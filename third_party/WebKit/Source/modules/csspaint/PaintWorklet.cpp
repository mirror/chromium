...
// We don't want to switch global scopes too often because we could use local
// cache.
const unsigned kFrameCountToSwitch = 120u;
...
// 120 frames, we switch to another global scope, to balance the workload.
unsigned PaintWorklet::SelectedGlobalScope() const {
  unsigned current_paint_frame_count = GetFrame()->View()->PaintFrameCount();
