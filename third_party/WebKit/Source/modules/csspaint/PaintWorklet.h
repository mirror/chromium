...
class MODULES_EXPORT PaintWorklet : public Worklet,
 ...
 protected:
  ...
  // The follow two functions are virtual so that we can override them in the
  // unit test, in order to make the testing deterministic.
  virtual int SetPaintsBeforeSwitching();
  virtual size_t SetActiveGlobalScope();
