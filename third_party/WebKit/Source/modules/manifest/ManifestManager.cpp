...
#include "third_party/WebKit/Source/modules/manifest/ManifestManager.h"
 ...
 public:
  ...
  void Start(const KURL& url, bool use_credentials) {
    ...
    loader_ = DocumentThreadableLoader::Create(
        *ThreadableLoadingContext::Create(*owner_->GetDocument()), this,
    ...
    request.SetFrameType(WebURLRequest::kFrameTypeNone);
...
void ManifestManager::OnManifestFetchComplete(const KURL& manifest_url,
  ...
  manifest_url_ = manifest_url.IsNull() ? claimed_manifest_url_ : manifest_url;
  if (data.IsEmpty()) {
...
bool ManifestManager::CheckForManifestChange() {
  ...
  if (current_manifest_url == claimed_manifest_url_)
    return false;

  claimed_manifest_url_ = current_manifest_url;
  manifest_url_ = KURL();
  manifest_ = nullptr;
  manifest_debug_info_ = nullptr;
  if (claimed_manifest_url_.IsNull())
    ResolveCallbacks();
  else
    FetchManifest();
  return true;
...
Document* ManifestManager::GetDocument() {
  return GetSupplementable();
}

HTMLLinkElement* ManifestManager::GetManifestLink() {
