...
ScriptPromise ImageCapture::setOptions(ScriptState* script_state,
  ...
  if (!photo_settings.hasImageHeight() && !photo_settings.hasImageWidth() &&
      !photo_settings.hasRedEyeReduction() &&
      !photo_settings.hasFillLightMode()) {
    resolver->Reject(DOMException::Create(kNotSupportedError,
                                          "No Image Capture settings found"));
    return promise;
  }
