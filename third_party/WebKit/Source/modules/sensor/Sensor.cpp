...
void Sensor::ContextDestroyed(ExecutionContext*) {
  if (state_ != SensorState::kIdle && !pending_error_notification_.IsActive())
    Deactivate();
}
...
void Sensor::UpdateState(SensorState new_state) {
  state_ = new_state;
}
...
void Sensor::HandleError(ExceptionCode code,
  ...
  if (pending_error_notification_.IsActive())
    return;  // Another error is being handled.
...
void Sensor::NotifyActivated() {
  DCHECK_EQ(state_, SensorState::kActivating);
  UpdateState(SensorState::kActivated);
