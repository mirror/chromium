...
// Chromium is the foreground app and the request comes from the active tab).
// Chromium's Clipboard permission maps to clipboard-read access (and also
// requires the request to come from Chromium's foreground tab/app).
...
void ClipboardPromise::HandleWriteWithPermission(PermissionStatus status) {
  // Allow writes if permission is GRANTED or PROMPT.
