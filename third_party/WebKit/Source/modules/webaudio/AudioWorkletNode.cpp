...
AudioWorkletNode::AudioWorkletNode(
  ...
  HashMap<String, RefPtr<AudioParamHandler>> param_handler_map;
  Vector<std::pair<String, double>> parameter_data = options.parameterData();
  for (const auto& param_info : param_info_list) {
    ...
    for (const auto& key_value_pair : parameter_data) {
...
AudioWorkletNode* AudioWorkletNode::Create(
  ...
  if (options.hasOutputChannelCount()) {
    ...
    for (const auto& channel_count : options.outputChannelCount()) {
      if (channel_count < 1 || 32 < channel_count) {
        exception_state.ThrowDOMException(
            kNotSupportedError,
            "AudioWorkletNode cannot be created: The given channel count (" +
                String::Number(channel_count) + ") is not supported.");
        return nullptr;
      }
    }
  }

  DCHECK(context->WorkletMessagingProxy());
