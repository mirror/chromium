...
AudioWorkletProcessor::AudioWorkletProcessor(
  ...
  ProcessorInitParams* params = global_scope_->GetProcessorInitParams();
  MessagePort* port = MessagePort::Create(*global_scope_);
  port->Entangle(std::move(params->port_channel));
  SECURITY_DCHECK(global_scope_->IsWorkerOrWorkletGlobalScope());
