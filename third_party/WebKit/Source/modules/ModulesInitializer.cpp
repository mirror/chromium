...
void ModulesInitializer::Initialize() {
  ...
  // FIXME remove this and restore to WebDevToolsAgentImpl once that
  // class is a controller/ crbug:731490
  InspectorAgent::RegisterSessionInitCallback(
      [](InspectorSession* session,
         const InspectorAgent::SessionInitArgs* args) {
        session->Append(new InspectorIndexedDBAgent(args->inspected_frames,
                                                    session->V8Session()));
        session->Append(
            new DeviceOrientationInspectorAgent(args->inspected_frames));
        if (args->allow_view_agents) {
          session->Append(InspectorDatabaseAgent::Create(args->page));
          session->Append(
              new InspectorAccessibilityAgent(args->page, args->dom_agent));
          session->Append(InspectorDOMStorageAgent::Create(args->page));
