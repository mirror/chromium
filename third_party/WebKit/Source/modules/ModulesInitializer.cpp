...
void ModulesInitializer::Initialize() {
  ...
  // IndexedDBClient callbacks.
  WebSharedWorkerImpl::RegisterOnScriptLoaderFinishedCallback(
      [](WebSharedWorkerImpl* worker_impl, WorkerClients* worker_clients) {
        ProvideLocalFileSystemToWorker(worker_clients,
                                       LocalFileSystemClient::Create());
        WebSecurityOrigin web_security_origin(
            worker_impl->LoadingDocument()->GetSecurityOrigin());
        ProvideContentSettingsClientToWorker(
            worker_clients,
            WTF::WrapUnique(
                worker_impl->Client()->CreateWorkerContentSettingsClientProxy(
                    web_security_origin)));
