...
ScriptPromise ShapeDetector::detect(ScriptState* script_state,
  ...
  RefPtr<Image> image = canvas_image_source->GetSourceImageForCanvas(
      &source_image_status, kPreferNoAcceleration, kSnapshotReasonDrawImage, size);
...
ScriptPromise ShapeDetector::DetectShapesOnImageData(
  ...
  return DoDetect(resolver, std::move(shared_buffer_handle), image_data->width(),
                  image_data->height());
}
...
ScriptPromise ShapeDetector::DetectShapesOnImageElement(
  ...
  const uint32_t allocation_size = skia_info.getSafeSize(skia_info.minRowBytes());
  ...
  const SkPixmap pixmap(skia_info, mapped_buffer.get(), skia_info.minRowBytes());
  ...
  return DoDetect(resolver, std::move(shared_buffer_handle), img->naturalWidth(),
                  img->naturalHeight());
}
