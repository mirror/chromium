...
class MODULES_EXPORT IDBValue final {
 public:
  static std::unique_ptr<IDBValue> Create(WebIDBValue, v8::Isolate*);
  ...
  std::unique_ptr<IDBKey> TakePrimaryKey() { return std::move(primary_key_); }
 ...
 private:
  ...
  IDBValue(WebIDBValue, v8::Isolate*);
  IDBValue(scoped_refptr<SharedBuffer>,
           const WebVector<WebBlobInfo>&,
           std::unique_ptr<IDBKey>,
           const IDBKeyPath&);
  IDBValue(std::unique_ptr<IDBValue>,
           std::unique_ptr<IDBKey>,
           const IDBKeyPath&);
