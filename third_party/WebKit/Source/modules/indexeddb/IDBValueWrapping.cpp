...
bool IDBValueWrapper::WrapIfBiggerThan(unsigned max_bytes) {
  ...
  DCHECK(owns_blob_handles_)
      << __FUNCTION__ << " called after TakeBlobDataHandles()";
  DCHECK(owns_blob_info_) << __FUNCTION__ << " called after TakeBlobInfo()";
  DCHECK(owns_wire_bytes_) << __FUNCTION__ << " called after TakeWireBytes()";
#endif  // DCHECK_IS_ON()
...
Vector<scoped_refptr<BlobDataHandle>> IDBValueWrapper::TakeBlobDataHandles() {
#if DCHECK_IS_ON()
  DCHECK(done_cloning_) << __FUNCTION__ << " called before DoneCloning()";
  DCHECK(!owns_blob_handles_) << __FUNCTION__ << " called twice";
...
#if DCHECK_IS_ON()
  DCHECK(done_cloning_) << __FUNCTION__ << " called before DoneCloning()";
  DCHECK(!owns_wire_bytes_) << __FUNCTION__ << " called twice";
