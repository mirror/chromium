# IndexedDB Data follow-up

[TOC]

## Summary

Chrome's IndexedDB implementation uses two backing stores for user data:

* [The Blob system](https://chromium.googlesource.com/chromium/src/+/master/storage/browser/blob/README.md),
  stores [Blobs](https://developer.mozilla.org/en-US/docs/Web/API/Blob) as files
  in a per-origin directory. Blobs are specifically designed for storing large
  amounts of data.
...
delegates to V8's `v8::ValueSerializer`. The value format used with the backing
...
store, an IndexedDB value's format will never change. This means that
SerializedScriptValue must read serialized values written by all previous
versions of Chrome. To avoid data corruption, the SSV implementation should
also detect (and reject) serialized values written by future Chrome versions,
which can happen when a user downgrades the browser (e.g., by switching channels
from beta to stable) and [when serialization changes are
reverted](https://crbug.com/700603). For the reasons above, technical debt
introduced by unnecessary complexity in the serialization format is much more
difficult to pay than in most of the Chrome codebase.
...
All IndexedDB requests (reads and writes) are sent via
[Mojo](https://chromium.googlesource.com/chromium/src/+/master/mojo/README.md)
IPC from the renderer running the Web application to to the browser process,
where they are fulfilled. Most of the data associated with an IndexedDB write
operation is transferred from the renderer to the browser using one Mojo call,
and is therefore subject to the Mojo message limit. Blobs are an exception, as
they are transferred to the browser process using the [Blob
...
complexity gap between IndexedDB value wrapping (write-side changes)
and unwrapping (read-side changes).
...
a `WebIDBCallbacks` instance, and passes the request and the callback to
[the browser-side IndexedDB
API](https://cs.chromium.org/chromium/src/content/browser/indexed_db/indexed_db_database.h).

The browser-side IndexedDB implementation processes requests from the renderer
...
* [The IndexedDB specification](https://w3c.github.io/IndexedDB/) states that
  ...
  following to meet the ordering demands:
    ...
    - Results (IndexedDBValue -> Value -> WebIDBValue) are passed to the browser
      ...
      so Mojo guarantees that the calls go over the same Mojo pipe, and
