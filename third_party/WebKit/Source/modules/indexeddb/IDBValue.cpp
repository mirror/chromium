...
IDBValue::IDBValue(const WebIDBValue& value, v8::Isolate* isolate)
    : IDBValue(value.data, value.web_blob_info, value.primary_key, value.key_path) {
  isolate_ = isolate;
...
IDBValue::IDBValue(PassRefPtr<SharedBuffer> data,
    ...
    : data_(data),
      ...
      blob_info_(WTF::WrapUnique(new Vector<WebBlobInfo>(web_blob_info.size()))),
      primary_key_(primary_key && primary_key->IsValid() ? primary_key : nullptr),
      key_path_(key_path) {
