...
void WebGL2RenderingContextBase::bindTexSource(
  ...
  if (frame->IsRemoteFrame()) {
    ...
    if (!surface_id.is_valid()) {
      SynthesizeGLError(GL_INVALID_OPERATION, "bindTexSource", "Source not ready: 1");
    } else {
      RemoteFrameClientImpl* client = reinterpret_cast<RemoteFrameClientImpl*>(
          remote_frame->Client());
      DCHECK(client);
      ...
      fprintf(stderr, "bindTexSource(IFrame): %d\n", (int)remote_frame->surface_id().hash());
    }
  } else {
    SynthesizeGLError(GL_INVALID_OPERATION, "bindTexSource", "Source not ready: 0");
  }
}
