...
bool Animator::Animate(ScriptState* script_state,
                       CompositorAnimatorOutputState* output) const {
  ...
  output->local_time_ = effect_->GetLocalTime();
  return true;
}

void Animator::PushInputState(const CompositorAnimatorInputState& state) {
  // TODO(majidvp): We should add a proper base::TT->WTF::TT converter.
  current_time_ = WTF::TimeTicks() + (state.current_time_ - base::TimeTicks());
}
