...
bool KeyUsagesFromWireFormat(uint32_t raw_usages,
  ...
  const uint32_t kAllPossibleUsages =
      kExtractableUsage | kEncryptUsage | kDecryptUsage | kSignUsage | kVerifyUsage |
      kDeriveKeyUsage | kWrapKeyUsage | kUnwrapKeyUsage | kDeriveBitsUsage;
...
CryptoKey* V8ScriptValueDeserializerForModules::ReadCryptoKey() {
  ...
  switch (raw_key_type) {
    ...
    case kRsaHashedKeyTag: {
      ...
      if (!ReadUint32(&raw_id) || !AlgorithmIdFromWireFormat(raw_id, &id) ||
          ...
          !ReadUint32(&modulus_length_bits) || !ReadUint32(&public_exponent_size) ||
  ...
  if (!Platform::Current()->Crypto()->DeserializeKeyForClone(
          ...
          reinterpret_cast<const unsigned char*>(key_data), key_data_length, key))
    return nullptr;
