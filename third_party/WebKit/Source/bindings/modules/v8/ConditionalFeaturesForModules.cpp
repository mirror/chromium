...
void InstallConditionalFeaturesForModules(
  ...
  if (wrapper_type_info == &V8Navigator::wrapperTypeInfo) {
    ...
    if (OriginTrials::serviceWorkerNavigationPreloadEnabled(execution_context)) {
  ...
  } else if (wrapper_type_info == &V8ServiceWorkerGlobalScope::wrapperTypeInfo) {
    v8::Local<v8::Object> instance_object = script_state->GetContext()->Global();
  ...
  } else if (wrapper_type_info == &V8ServiceWorkerRegistration::wrapperTypeInfo) {
    if (OriginTrials::serviceWorkerNavigationPreloadEnabled(execution_context)) {
...
void InstallPendingConditionalFeatureForModules(
  ...
  // files. (crbug.com/615060)
  (*g_original_install_pending_conditional_feature_function)(feature, script_state);
