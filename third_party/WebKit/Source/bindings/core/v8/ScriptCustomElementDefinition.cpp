...
ScriptCustomElementDefinition* ScriptCustomElementDefinition::Create(
  ...
  v8::Local<v8::Map> map =
      EnsureCustomElementRegistryMap(script_state, registry);
  map->Set(script_state->GetContext(), constructor, name_value).ToLocalChecked();
...
HTMLElement* ScriptCustomElementDefinition::CreateElementSync(
  ...
  Element* element = nullptr;
  {
    ...
    bool is_import_document = document.ImportsController() &&
...
void ScriptCustomElementDefinition::RunCallback(
  ...
  V8ScriptRunner::CallFunction(callback, execution_context, element_handle, argc,
                               argv, isolate);
}
