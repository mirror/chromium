...
InstallConditionalFeaturesFunction SetInstallConditionalFeaturesFunction(
    InstallConditionalFeaturesFunction new_install_conditional_features_function) {
  InstallConditionalFeaturesFunction original_function =
      g_install_conditional_features_function;
  g_install_conditional_features_function = new_install_conditional_features_function;
...
void InstallConditionalFeatures(const WrapperTypeInfo* type,
                                ...
                                v8::Local<v8::Object> prototype_object,
                                v8::Local<v8::Function> interface_object) {
  (*g_install_conditional_features_function)(type, script_state, prototype_object,
                                          interface_object);
}
...
void InstallPendingConditionalFeature(const String& feature,
                                      const ScriptState* script_state) {
  DCHECK(script_state);
  DCHECK(script_state->GetContext() == script_state->GetIsolate()->GetCurrentContext());
