...
class NotReached : public ScriptFunction {
 ...
 private:
  explicit NotReached(ScriptState* script_state) : ScriptFunction(script_state) {}
...
class StubFunction : public ScriptFunction {
 ...
 private:
  StubFunction(ScriptState* script_state, ScriptValue& value, size_t& call_count)
...
class ScriptPromisePropertyTestBase {
 public:
  ...
  ScriptState* CurrentScriptState() { return ScriptState::Current(GetIsolate()); }
  ...
  ScriptValue Wrap(DOMWrapperWorld& world, const T& value) {
    ...
    return ScriptValue(
        script_state, ToV8(value, script_state->GetContext()->Global(), GetIsolate()));
  }
 ...
 public:
  ...
  void Test(const T& value,
    ...
    typedef ScriptPromiseProperty<Member<GarbageCollectedScriptWrappable>, T,
        ...
        Property;
    Property* property =
        new Property(&GetDocument(), new GarbageCollectedScriptWrappable("holder"),
                     Property::kReady);
...
TEST_F(ScriptPromisePropertyGarbageCollectedTest, Reset) {
...

  {
    ...
    old_promise.Then(Stub(CurrentScriptState(), old_actual, n_old_resolve_calls),
                    NotReached(CurrentScriptState()));
  }
...

  {
    ...
    new_promise.Then(NotReached(CurrentScriptState()),
