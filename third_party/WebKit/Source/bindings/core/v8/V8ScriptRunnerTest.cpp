...
TEST_F(V8ScriptRunnerTest, parseOption) {
  ...
  EXPECT_TRUE(
      CompileScript(scope.GetScriptState(), source_code, kV8CacheOptionsParse));
  EXPECT_TRUE(source_code.CacheHandler()->GetCachedMetadata(
      TagForParserCache(source_code.CacheHandler())));
  EXPECT_FALSE(source_code.CacheHandler()->GetCachedMetadata(
      TagForCodeCache(source_code.CacheHandler())));
...
TEST_F(V8ScriptRunnerTest, codeOption) {
  V8TestingScope scope;
  ScriptSourceCode source_code(nullptr, CreateResource());
  SetCacheTimeStamp(source_code.CacheHandler());
