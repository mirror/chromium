...
class V8ScriptRunnerTest : public ::testing::Test {
 public:
  ...
  void SetEmptyResource() {
    resource_ =
        ScriptResource::CreateForTest(KURL(), CharsetRequest(UTF8Encoding()));
  }

  void SetResource() {
    resource_ =
        ScriptResource::CreateForTest(Url(), CharsetRequest(UTF8Encoding()));
  }
...
TEST_F(V8ScriptRunnerTest, parseOption) {
  ...
  ScriptResource* another_resource = ScriptResource::CreateForTest(
      Url(), CharsetRequest(UTF16LittleEndianEncoding()));
