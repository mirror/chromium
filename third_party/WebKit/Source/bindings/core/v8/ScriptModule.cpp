...
ScriptModule ScriptModule::Compile(v8::Isolate* isolate,
  ...
  // TODO(kouhei): store base_url in ReferrerScriptInfo.
  if (!V8ScriptRunner::CompileModule(
           isolate, source, source_url.GetString(), access_control_status,
           text_position, ReferrerScriptInfo::FromScriptFetchOptions(options))
           .ToLocal(&module)) {
