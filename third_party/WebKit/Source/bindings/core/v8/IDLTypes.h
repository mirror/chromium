...
#include <type_traits>
#include "bindings/core/v8/IDLTypeTraits.h"
...
// Sequence
template <typename T>
struct IDLSequence final
    : public IDLBaseHelper<typename WrapInVectorType<T>::type> {};
...
struct IDLRecord final : public IDLBase {
  ...
  using ImplType = typename std::conditional<
      NeedsHeapVector<Value>::value,
      HeapVector<std::pair<String, typename AddMember<Value>::type>>,
      Vector<std::pair<String, typename AddMember<Value>::type>>>::type;
};
