...
v8::Maybe<uint32_t> V8ScriptValueSerializer::StartBundle(
  ...
  uint32_t bundle_id = bundle_array_->size();
  bundle_array_->emplace_back();
  return v8::Just<uint32_t>(bundle_id);
}

v8::Maybe<uint32_t> V8ScriptValueSerializer::StartItem(
    size_t item_size,
  ...
  uint32_t item_id = bundle.items.size();
  ...
  return v8::Just<uint32_t>(item_id);
}

std::pair<uint8_t*, size_t> V8ScriptValueSerializer::NextItemBuffer() {
#if DCHECK_IS_ON()
  ...
  SerializedScriptValue::Bundle& bundle = bundle_array_->back();
  SerializedScriptValue::Bundle::Item& item = bundle.items.back();
  return std::make_pair(reinterpret_cast<uint8_t*>(item.data.data()),
                        item.data.size());
}
