  ...
  switch (tag) {
    ...
    case kImageDataTag: {
      ...
      if (!ReadUint32(&width) || !ReadUint32(&height) ||
          !ReadUint32(&pixel_length) || !ReadRawBytes(pixel_length, &pixels))
        return nullptr;
      // Backward comaptibility: if there is no color settings to read,
      // silently fall back to default parameters (SRGB canvas color space,
      // Uint8 storage format).
      ReadUint32(&canvas_color_space);
