...
class SourceStream : public v8::ScriptCompiler::ExternalSourceStream {
 ...
 private:
  void PrepareDataOnMainThread(ScriptStreamer* streamer) {
    ...
    if (!streamer->GetResource()->GetResponse().CacheStorageCacheName().IsNull()) {
      streamer->SuppressStreaming();
      Cancel();
      return;
    }

    CachedMetadataHandler* cache_handler = streamer->GetResource()->CacheHandler();
...
void ScriptStreamer::StartStreaming(PendingScript* script,
  ...
  bool started_streaming = StartStreamingInternal(
      script, script_type, settings, script_state, std::move(loading_task_runner));
