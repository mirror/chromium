...
void ScriptStreamer::NotifyAppendData(TextResource* resource) {
  DCHECK(IsMainThread());
...
void ScriptStreamer::StartStreaming(
  ...
  TextResource* resource = script->GetResource();
  ...
  // which case Resource::IsLoaded() will be true, but ready_state_ will
