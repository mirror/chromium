...
void V8ResizeObserverCallback::handleEvent(
  ...
  v8::Local<v8::Value> observer_handle = ToV8(
      observer, m_scriptState->GetContext()->Global(), m_scriptState->GetIsolate());
  if (!observer_handle->IsObject())
    return;

  v8::Local<v8::Object> this_object =
      v8::Local<v8::Object>::Cast(observer_handle);
  v8::Local<v8::Value> entries_handle = ToV8(
      entries, m_scriptState->GetContext()->Global(), m_scriptState->GetIsolate());
  ...
  V8ScriptRunner::CallFunction(m_callback.NewLocal(isolate),
                               m_scriptState->GetExecutionContext(), this_object,
