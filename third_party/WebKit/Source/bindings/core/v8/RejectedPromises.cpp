...
class RejectedPromises::Message final {
 public:
  ...
  void Report() {
    ...
    if (target &&
        !execution_context->ShouldSanitizeScriptError(resource_name_,
                                                     cors_status_)) {
    ...
    if (should_log_to_console_) {
      ThreadDebugger* debugger = ThreadDebugger::From(script_state_->GetIsolate());
      if (debugger) {
        promise_rejection_id_ =
            debugger->PromiseRejected(script_state_->GetContext(), error_message_,
                                      reason, std::move(location_));
      }
    }
  ...
  while (!queue->IsEmpty()) {
    ...
    if (!message->HasHandler()) {
      ...
      if (reported_as_errors_.size() > kMaxReportedHandlersPendingResolution)
        reported_as_errors_.erase(0, kMaxReportedHandlersPendingResolution / 10);
    }
  }
}
