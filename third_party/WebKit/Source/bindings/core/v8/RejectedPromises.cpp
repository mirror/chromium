 ...
 public:
  ...
  void PostTask(const base::Location& from_here, base::OnceClosure task) {
    ExecutionContext::From(script_state_)
        ->GetTaskRunner(blink::TaskType::kPromise)
...
void RejectedPromises::Dispose() {
  ...
  while (!queue_.IsEmpty()) {
    ...
    ProcessMessageNow(std::move(message));
  }
}
...
void RejectedPromises::ProcessQueue() {
  ...
  // TODO(hajimehoshi): The spec requires one task process all the messages in
  // the queue, but here one task processes only one message. Either the spec or
  // the logic should be fixed.
  // See https://html.spec.whatwg.org/#unhandled-promise-rejections.
  while (!queue_.IsEmpty()) {
    std::unique_ptr<Message> message = queue_.TakeFirst();
    if (message->IsCollected())
      continue;
    message->PostTask(FROM_HERE,
                      WTF::Bind(&RejectedPromises::ProcessMessageNow,
                                scoped_refptr<RejectedPromises>(this),
                                WTF::Passed(std::move(message))));
  }
}

void RejectedPromises::ProcessMessageNow(std::unique_ptr<Message> message) {
