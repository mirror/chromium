...
void V8EventHandlerGetter(const v8::FunctionCallbackInfo<v8::Value>& info,
  ...
  EventListener* listener = impl(wrappable);
  if (!listener) {
    V8SetReturnValueNull(info);
    return;
  }
  v8::Local<v8::Value> listener_value =
      V8AbstractEventListener::Cast(listener)->GetExistingListenerObject();
  DCHECK(!listener_value.IsEmpty()) << "This should be a non-lazy listener.";
