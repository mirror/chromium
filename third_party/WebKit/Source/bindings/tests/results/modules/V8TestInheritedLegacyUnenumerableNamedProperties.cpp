...
v8::Local<v8::FunctionTemplate> V8TestInheritedLegacyUnenumerableNamedProperties::domTemplate(v8::Isolate* isolate, const DOMWrapperWorld& world) {
  ...
  if (per_isolate_data->GetV8ContextSnapshotMode() != V8PerIsolateData::V8ContextSnapshotMode::kTakeSnapshot) {
    per_isolate_data->SetInterfaceTemplateByIndex(
        world.IsMainWorld() ? kV8TestInheritedLegacyUnenumerableNamedPropertiesTemplateForMainWorldIndex
                            : kV8TestInheritedLegacyUnenumerableNamedPropertiesTemplateForNonMainWorldIndex,
        templ);
  }
  return templ;
}
