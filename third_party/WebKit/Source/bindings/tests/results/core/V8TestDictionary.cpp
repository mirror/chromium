...
void V8TestDictionary::toImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8Value, TestDictionary& impl, ExceptionState& exceptionState) {
  ...
  v8::Local<v8::Value> setLongValue;
  if (!v8Object->Get(context, keys[24].Get(isolate)).ToLocal(&setLongValue)) {
    exceptionState.RethrowV8Exception(block.Exception());
    return;
  }
  if (setLongValue.IsEmpty() || setLongValue->IsUndefined()) {
    // Do nothing.
  } else {
    int32_t setLongCppValue = NativeValueTraits<IDLLong>::NativeValue(isolate, setLongValue, exceptionState, kClamp);
    if (exceptionState.HadException())
      return;
    impl.setSetLong(setLongCppValue);
  }

  v8::Local<v8::Value> setStringValue;
  if (!v8Object->Get(context, keys[25].Get(isolate)).ToLocal(&setStringValue)) {
    exceptionState.RethrowV8Exception(block.Exception());
    return;
  }
  if (setStringValue.IsEmpty() || setStringValue->IsUndefined()) {
    // Do nothing.
  } else {
    V8StringResource<kTreatNullAsEmptyString> setStringCppValue = setStringValue;
    if (!setStringCppValue.Prepare(exceptionState))
      return;
    impl.setSetString(setStringCppValue);
  }
...
bool toV8TestDictionary(const TestDictionary& impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  v8::Local<v8::Value> setLongValue;
  bool setLongHasValueOrDefault = false;
  if (impl.hasSetLong()) {
    setLongValue = v8::Integer::New(isolate, impl.setLong());
    setLongHasValueOrDefault = true;
  }
  if (setLongHasValueOrDefault &&
      !V8CallBoolean(dictionary->CreateDataProperty(context, keys[24].Get(isolate), setLongValue))) {
    return false;
  }

  v8::Local<v8::Value> setStringValue;
  bool setStringHasValueOrDefault = false;
  if (impl.hasSetString()) {
    setStringValue = V8String(isolate, impl.setString());
    setStringHasValueOrDefault = true;
  }
  if (setStringHasValueOrDefault &&
      !V8CallBoolean(dictionary->CreateDataProperty(context, keys[25].Get(isolate), setStringValue))) {
    return false;
  }
