...
class Argument(object):
    ...
    def create(node):
        ...
        for child in node.GetChildren():
            ...
            if child_class == 'Type':
                argument.type = idl_types.type_node_to_type(child)
            elif child_class == 'ExtAttributes':
                argument.extended_attributes = extended_attribute.expand_extended_attributes(child)
                ...
                argument.default_value = Literal.create(child)
            else:
...
def arguments_node_to_arguments(node):
    ...
    return [Argument.create(argument_node) for argument_node in node.GetChildren()]
