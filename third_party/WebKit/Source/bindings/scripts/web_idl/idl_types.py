...
class ReferenceType(object):
    """ReferenceType does not represent an actual type. It hold a name as a
    string to refer an IDL named definition like interface, dictionary, and
    typedef.
    """
    ...
    def create(node, is_nullable):
        ...
        reference_type.definition_name = node.GetName()
        reference_type.is_nullable = is_nullable
...
def base_type_node_to_type(node, is_nullable=False, extended_attributes=None):
    ...
    if node_class == 'Typeref':
        return ReferenceType.create(node, is_nullable)
