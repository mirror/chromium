...
class TypeReference(TypeBase):
    """
    TypeReference represents types which use identifers;
    interface types, dictionary types, enumeration types, and callback function types.
    This class is a pseudo type to refer identifer-based types, because we can't resolve
    all the references in the first parsing, or references beyond the components.
    Thus this type is expected to be replaced with actual references in the second phase.
