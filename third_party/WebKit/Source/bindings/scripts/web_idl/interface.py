...
class Interface(object):
    ...
    def __init__(self):
        ...
        # IDL file which defines this interface.
        self.filename = None
...
class Iterable(object):
    ...
    def create(cls, node):
        ...
        if len(sub_types) == 1:
            iterable.value_type = types.type_node_to_type(sub_types[0])
        elif len(sub_types) == 2:
            iterable.key_type = types.type_node_to_type(sub_types[0])
            iterable.value_type = types.type_node_to_type(sub_types[1])
        else:
    ...
    def create(cls, node):
        ...
        for child in node.GetChildren():
            ...
            if child_class == 'ExtAttributes':
                ...
                raise ValueError('Unrecognized node class: %s' % child_class)
        if len(sub_types) == 2:
            maplike.key_type = types.type_node_to_type(sub_types[0])
            maplike.value_type = types.type_node_to_type(sub_types[1])
        else:
            raise ValueError('Unexpected number of type children: %d' % len(sub_types))
    ...
    def create(cls, node):
        ...
        for child in node.GetChildren():
            ...
            if child_class == 'ExtAttributes':
                ...
                setlike.value_type = types.type_node_to_type(child)
            else:
                raise ValueError('Unrecognized node class: %s' % child_class)
        return setlike
