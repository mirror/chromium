    ...
    def create_from_node(cls, node, filename):
        ...
        for child in node.GetChildren():
            ...
            if child_class == 'Inherit':
                ...
                interface.attributes.append(Attribute.create_from_node(child))
            elif child_class == 'Operation':
                interface.operations.append(Operation.create_from_node(child))
            elif child_class == 'Const':
                interface.constants.append(Constant.create_from_node(child))
            elif child_class == 'Stringifier':
                interface.process_stringifier(child)
            elif child_class == 'Iterable':
                interface.iterable = Iterable.create_from_node(child)
            elif child_class == 'Maplike':
                interface.maplike = Maplike.create_from_node(child)
            elif child_class == 'Setlike':
                interface.setlike = Setlike.create_from_node(child)
            elif child_class == 'Serializer':
                interface.serializer = Serializer.create_from_node(child)
            elif child_class == 'ExtAttributes':
                interface.extended_attributes = extended_attributes.expand_extended_attributes(child)
            else:
