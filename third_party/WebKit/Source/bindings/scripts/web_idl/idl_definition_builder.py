...
class IdlDefinitionBuilder(object):
    ...
    def create_interface(node):
        ...
        for child in node.GetChildren():
            ...
            if child_class == 'Inherit':
                ...
                attribute, operation = IdlDefinitionBuilder._process_stringifier(child)
                if attribute:
                ...
                extended_attributes = IdlDefinitionBuilder.create_extended_attributes(child)
            else:
    ...
    def create_constant(node):
        ...
        if len(children) == 3:
            extended_attributes = IdlDefinitionBuilder.create_extended_attributes(children[2])
    ...
    def create_attribute(node, ext_attrs=None):
        ...
        for child in node.GetChildren():
            ...
            if child_class == 'Type':
                ...
                extended_attributes.update(IdlDefinitionBuilder.create_extended_attributes(child))
            else:
                assert False, 'Unrecognized node class: %s' % child_class
    ...
    def create_type(node):
        ...
        extended_attributes = None
        if len(children) == 2:
            extended_attributes = IdlDefinitionBuilder.create_extended_attributes(children[1])

        return IdlDefinitionBuilder.create_base_type(children[0], is_nullable, extended_attributes)

    @staticmethod
    def create_base_type(node, is_nullable=False, extended_attributes=None):
        node_class = node.GetClass()
        if node_class == 'Typeref':
            return IdlDefinitionBuilder.create_type_reference(node, is_nullable)
        if node_class == 'PrimitiveType':
            return IdlDefinitionBuilder.create_primitive(node, is_nullable, extended_attributes)
        if node_class == 'StringType':
            return IdlDefinitionBuilder.create_string(node, is_nullable, extended_attributes)
        if node_class == 'Any':
    ...
    def _process_stringifier(node):
        ...
        for child in node.GetChildren():
            child_class = child.GetClass()
            if child.GetClass() == 'ExtAttributes':
                ext_attributes = IdlDefinitionBuilder.create_extended_attributes(child)
            elif child_class == 'Attribute':
                attribute = IdlDefinitionBuilder.create_attribute(child, ext_attributes)
            elif child_class == 'Operation':
                operation = IdlDefinitionBuilder.create_operation(child, ext_attributes)
