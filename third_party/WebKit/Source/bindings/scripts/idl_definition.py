...
IdlDefinition
    ...
    IdlInterface
        ...
        IdlOperation
            IdlArgument
        IdlSerializer (deprecated)
        IdlStringifier
        IdlIterable
        IdlMaplike
        IdlSetlike
    IdlNamespace (not implemented)
    IdlTypedef
...
################################################################################
# IdlDefinition (main container class)
################################################################################

class IdlDefinition(object):
    def __init__(self, node=None):
        """Args: node: AST root node, class == 'File'"""
        # TODO(peria): Support namespace and partial namespace.
        ...
        file_name = node.GetProperty('FILENAME')

        for component in utilities.KNOWN_COMPONENTS:
            if file_name.find(component) >= 0:
        ...
        for child in children:
            ...
            if child_class == 'Interface':
                interface = IdlInterface(child, file_name)
                if interface.is_partial:
                    self.partial_interfaces.append(interface)
                else:
                    self.interfaces[interface.name] = interface
            elif child_class == 'Dictionary':
                dictionary = IdlDictionary(child, file_name)
                if dictionary.is_partial:
                    self.partial_dictionaries.append(dictionary)
                else:
                    self.dictionaries[dictionary.name] = dictionary
            elif child_class == 'Typedef':
                typedef = IdlTypedef(child, file_name)
                if typedef.name in self.typedefs:
                    raise ValueError('[%s] Typedef %s is defined twice.' % (
                        file_name, typedef.name))
                self.typedefs[typedef.name] = typedef
            elif child_class == 'Enum':
                enumeration = IdlEnum(child, file_name)
                if enumeration.name in self.enumerations:
                    raise ValueError('[%s] Enum %s is defined twice.' % (
                        file_name, enumeration.name))
                self.enumerations[enumeration.name] = enumeration
            elif child_class == 'Callback':
                callback_function = IdlCallbackFunction(child, file_name)
                if callback_function.name in self.callback_functions:
                    raise ValueError('[%s] Callback function %s is defined twice.' % (
                        file_name, callback_function.name))
                ...
                raise ValueError('[%s] Unrecognized node class %s' % (
                    file_name, child_class))

    def merge(self, other):
        """Update with additional IdlInformation."""
        def check_conflicts(mine, other, type_name):
            conflicts = list(mine.viewkeys() & other.viewkeys())
            if conflicts:
                name = conflicts[0]
                raise ValueError('%s %s is defined in %s and %s' % (
                    type_name, name, mine[name].file_name, other[name].file_name))

        if self.component and other.component and self.component != other.component:
            raise ValueError('IdlDefintions in different components cannot be merged')
        if not self.component:
            self.component = other.component

        # Conflict checks
        check_conflicts(self.callback_functions, other.callback_functions, 'Callback function')
        check_conflicts(self.dictionaries, other.dictionaries, 'Dictionary')
        check_conflicts(self.enumerations, other.enumerations, 'Enum')
        check_conflicts(self.interfaces, other.interfaces, 'Interface')
        # Some IDL can define same typedef. If its definition is same, they are not conflicts.
        # check_conflicts(self.typedefs, other.typedefs, 'Typedef')

        # Merge definitions
        self.callback_functions.update(other.callback_functions)
        self.dictionaries.update(other.dictionaries)
        self.enumerations.update(other.enumerations)
        self.interfaces.update(other.interfaces)
        self.typedefs.update(other.typedefs)

        self.partial_interfaces.extend(other.partial_interfaces)
        self.partial_dictionaries.extend(other.partial_dictionaries)
...
class IdlCallbackFunction(object):
    def __init__(self, node, idl_file=''):
        children = node.GetChildren()
...
class IdlLiteralNull(IdlLiteral):
    def __init__(self):
        super(IdlLiteralNull, self).__init__(idl_type='NULL', value=None)
        self.is_null = True
...
def type_node_to_type(node):
    ...
    if len(children) < 1 or len(children) > 2:
        raise ValueError('Type node expects 1 or 2 children (type + optional array []), '
                         'got %s (multi-dimensional arrays are not supported).' % len(children))
