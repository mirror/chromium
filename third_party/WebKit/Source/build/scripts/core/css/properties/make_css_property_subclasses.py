...
class CSSPropertyHeadersWriter(CSSPropertyWriter):
    ...
    def populate_includes(self, property_):
        ...
        includes.sort()
        property_['h_includes'] = includes

        # cpp includes
        includes = []
        if 'custom_apply' in property_.keys() and property_['custom_apply'] == 'fill_layer':
            includes.append("core/css/CSSValueList.h")
        property_['cpp_includes'] = includes
