...
from make_css_property_api import CSSPropertyAPIWriter


class ApiMethod(
    namedtuple('ApiMethod', 'name,return_type,parameters,description')):
    pass
...
class CSSPropertyAPIHeadersWriter(CSSPropertyAPIWriter):
    def __init__(self, json5_file_paths):
        ...
        assert len(json5_file_paths) == 2,\
            'CSSPropertyAPIHeadersWriter requires 2 input json5 files files, got {}.'.format(len(json5_file_paths))
    ...
    def generate_property_api_h_builder(self, api_classname, property_name):
        @template_expander.use_jinja(
            'core/css/properties/templates/CSSPropertyAPIFiles.h.tmpl')
