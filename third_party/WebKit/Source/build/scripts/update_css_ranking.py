...
#        the ranking to another file witht the default ranking API.
...
def reformat_properties_name(css_properties):
    for i in range(len(css_properties)):
        if css_properties[i][0:5] == "alias":
            css_properties[i] = css_properties[i][5:]
        if css_properties[i][0:6] == "webkit":
...
def find_partition_rule(css_property_set, all_properties, n_cluster, transform=lambda x: x):
    ...
    """
    _, cluster_alloc, _ = cluster.k_mean([transform(p[1] + 10e-6) for p in css_property_set], n_cluster=n_cluster)
...
def produce_partition_rule(config_file, css_ranking_api):
    ...
    rare_non_inherited_properties = sorted([(x["name"], css_ranking_dictionary[x["name"]])
                                            for x in css_properties if not x["inherited"] and x["group_auto"]
                                            and x["name"] in css_ranking_dictionary],
                                           key=lambda x: -x[1])
    rare_inherited_properties = sorted([(x["name"], css_ranking_dictionary[x["name"]])
                                        for x in css_properties if x["inherited"] and x["group_auto"]
                                        and x["name"] in css_ranking_dictionary],
                                       key=lambda x: -x[1])
    ...
    ri_properties_rule = find_partition_rule(rare_inherited_properties,
                                             css_ranking_cdf,
                                             n_cluster=2, transform=math.log)

    with open(config_file, 'w') as fw:
        json.dump({
            "parameters": {},
            "data": [{"name": "rare_non_inherited_properties_rule", "cummulative_distribution": rni_properties_rule},
                     {"name": "rare_inherited_properties_rule", "cummulative_distribution": ri_properties_rule}]
        }, fw, indent=4)
