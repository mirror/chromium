...
def sort_keyword_properties_by_canonical_order(css_properties, css_value_keywords_file, json5_file_parameters):
    ...
    for css_property in css_properties:
        if ('field_template' in css_property and css_property['field_template'] == 'keyword'
                and len(css_property['include_paths']) == 0) or ('field_template' not in css_property
                                                                 and 'keywords' in css_property):
