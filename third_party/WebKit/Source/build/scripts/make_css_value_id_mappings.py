...
def _find_continuous_segment(numbers):
    """Find the longest continuous segment in a list of
    numbers
...
def _find_largest_segment(segments):
    ...
    """
    segment_list = [(segments[x], segments[x + 1]) for x in range(len(segments) - 1)]
    ...
    def generate_css_value_mappings(self):
        ...
        for property_ in self._properties.values():
            ...
            if property_['field_template'] == 'multi_keyword':
                ...
                p_segment = _find_largest_segment(enum_segment)
