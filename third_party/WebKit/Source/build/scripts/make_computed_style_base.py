...
def _evaluate_rare_non_inherited_group(all_properties, properties_ranking_file,
    ...
    layers_name = ["rare-non-inherited-layer-less-than-" + str(round(partition_rule[i] * 100)) for i in range(number_of_layer)]
...
def _evaluate_rare_inherit_group(all_properties, properties_ranking_file,
    ...
    layers_name = ["rare-inherited-layer-less-than-" + str(round(partition_rule[i] * 100)) for i in range(number_of_layer)]
    properties_ranking = _get_properties_ranking(properties_ranking_file, partition_rule)
    ...
    for property_ in all_properties:
        if property_["group_auto"] and property_["inherited"] and property_["name"] in properties_ranking:
            ...
            property_["field_group"] = "->".join([layers_name[0], layers_name[0] + "sub"])
