...
class ReleaseBuilder(object):
    ...
    def _concatenate_application_script(self, output):
        if not self.descriptors.extends:
            ...
            runtime_contents = re.sub(r'var allDescriptors = \[\];',
            ...
            output.write('allDescriptors.push(...%s);' % self._release_module_descriptors().replace('\\', '\\\\'))
            output.write('/* Additional descriptors %s */\n' % self.app_file('json'))
            output.write('applicationDescriptor.modules.push(...%s)' % self.descriptors.application.values())
