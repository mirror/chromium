...
function transformHelperCode(helpers) {
  const boilerplate = `// Copyright 2017 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
...
`;
  ...
  recast.visit(ast, {
    ...
    if (!existingContents)
      helper.contents = boilerplate + helper.contents[0] === '\n' ? helper.contents.slice(1) : helper.contents

    // if (DRY_RUN)
      // console.log(helper.contents);
  }

  function unwrapInitializeFunction(ast) {
    // Handle function expression
    let index =
      ast.program.body.findIndex(n => n.type === 'VariableDeclaration' && n.declarations[0].id.name.indexOf('initialize_') !== -1);
    if (index > -1) {
    ...
    index = ast.program.body.findIndex(n => n.type === 'FunctionDeclaration' && n.id.name.indexOf('initialize_') !== -1);
    if (index > -1) {
      const otherNodes = ast.program.body.filter((_, i) => i !== index);
      const testFunctionNode = ast.program.body[index];
...
function scrapeTestProperties(helpers) {
  ...
  console.log(propertyToNamespace);
...
// Please see crbug.com/667560 for more details\n\n`;
    if (originalContents.indexOf(originalContents) !== -1)
      continue;
