...
ColorPicker.ContrastDetails = class {
  ...
  constructor(contrastInfo, contentElement, toggleMainColorPickerCallback, expandedChangedCallback) {
    ...
    this._contrastValueBubble.addEventListener('click', e => {
  ...
  _update() {
    ...
    this._contrastPassFailAA.createChild('span').textContent = Common.UIString('AA: %s', aa.toFixed(1));
    if (this._passesAA)
    ...
    this._contrastPassFailAAA.createChild('span').textContent = Common.UIString('AAA: %s', aaa.toFixed(1));
    if (passesAAA)
