...
Timeline.TimelineHistoryManager = class {
  ...
  addRecording(performanceModel) {
    ...
    if (this._recordings.length <= Timeline.TimelineHistoryManager._maxRecordings)
      return;
    var lruModel =
        this._recordings.reduce((modelA, modelB) => compareLastUsedTime(modelA, modelB) < 0 ? modelA : modelB);
    this._recordings.splice(this._recordings.indexOf(lruModel), 1);
    lruModel.dispose();

    /**
     * @param {!Timeline.PerformanceModel} a
     * @param {!Timeline.PerformanceModel} b
     */
    function compareLastUsedTime(a, b) {
      var dataA = Timeline.TimelineHistoryManager._dataForModel(a);
      var dataB = Timeline.TimelineHistoryManager._dataForModel(b);
      return dataA.lastUsed - dataB.lastUsed;
    }
  }
  ...
  async showHistoryDropDown() {
    ...
    var model = await Timeline.TimelineHistoryManager.DropDown.show(this._recordings, this._button.element);
    if (!model)
      return null;
...
Timeline.TimelineHistoryManager.DropDown = class {
  ...
  constructor(models) {
    ...
    this._widget.registerRequiredCSS('timeline/timelineHistoryManager.css');
    this._widget.show(this._glassPane.contentElement);
    ...
    this._widget.contentElement.appendChild(this._listControl.element);
    this._widget.setDefaultFocusedElement(this._listControl.element);
    this._widget.contentElement.addEventListener('keydown', this._onKeyDown.bind(this), false);
    this._widget.contentElement.addEventListener('click', this._onClick.bind(this), false);
