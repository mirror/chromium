...
Timeline.PerformanceMonitor = class extends UI.HBox {
  ...
  _processMetrics(rawMetrics) {
    ...
    for (var metric of rawMetrics) {
      var info = this._metricInfo.get(metric.name);
      if (!info) {
        info = {};
        this._metricInfo.set(metric.name, info);
      }
      var value;
  ...
  _draw() {
    ...
    for (var chartInfo of this._controlPane.charts()) {
      if (!this._controlPane.isActive(chartInfo.charts[0].name))
        continue;
  ...
  _drawChart(ctx, chartInfo, height) {
    ...
    for (var metricInfo of chartInfo.charts)
 ...
 * @typedef {!{
 *   title: string,
 *   charts: !Array<!Timeline.PerformanceMonitor.MetricInfo>,
  ...
  constructor(parent) {
    ...
    this._chartsInfo = [
      ...
      },
      {
        ...
        charts: [{name: 'JSHeapUsedSize', color: 'blue'}, {name: 'JSHeapTotalSize', color: '#99f'}],
        format: format.Bytes,
      },
      {title: Common.UIString('DOM Nodes'), charts: [{name: 'NodeCount', color: 'green'}]},
      {title: Common.UIString('JS event listeners'), charts: [{name: 'JSEventListenerCount', color: 'yellowgreen'}]},
      {title: Common.UIString('Documents'), charts: [{name: 'DocumentCount', color: 'darkblue'}]},
      {title: Common.UIString('Frames'), charts: [{name: 'FrameCount', color: 'darkcyan'}]},
      {title: Common.UIString('Layouts / sec'), charts: [{name: 'LayoutCount', color: 'hotpink'}]},
      {title: Common.UIString('Style recalcs / sec'), charts: [{name: 'RecalcStyleCount', color: 'deeppink'}]}
    ];
    for (var info of this._chartsInfo) {
      for (var metric of info.charts)
    ...
    for (var chartInfo of this._chartsInfo) {
      var chartName = chartInfo.charts[0].name;
