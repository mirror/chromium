...
NetworkTestRunner.addProductRegistryEntry = function(domainPattern, productName, type) {
  ...
  if (wildCardPosition === -1)
    ...
    console.assert(domainPattern.substr(wildCardPosition + 1, 1) === '.', 'Domain pattern wildcard must be followed by \'.\'');
  ...
  ProductRegistryImpl.register([productName], [{
    hash: ProductRegistryImpl._hashForDomain(domainPattern),
    prefixes: prefixes
  }]);
};
