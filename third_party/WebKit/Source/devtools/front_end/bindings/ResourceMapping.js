...
Bindings.ResourceMapping.Binding = class {
  ...
  async _styleSheetChanged(stylesheet, edit) {
    ...
    for (var i = 0; i < this._edits.length; i++)
      await this._innerStyleSheetChanged(this._edits[i].stylesheet, this._edits[i].edit);
    this._edits = [];
  }
   ...
   * @param {!SDK.CSSModel.Edit} edit
   */
  async _innerStyleSheetChanged(stylesheet, edit) {
    ...
    this._uiSourceCode.addRevision(text.replaceRange(oldRange, edit.newText));
