...
LayersTestRunner.dumpLayerTree = function(prefix, root) {
  ...
  root.children().forEach(LayersTestRunner.dumpLayerTree.bind(InspectorTest, prefix + '    '));
};
...
LayersTestRunner.requestLayers = function(callback) {
  ...
  function onLayerTreeChanged() {
    LayersTestRunner.layerTreeModel().removeEventListener(Layers.LayerTreeModel.Events.LayerTreeChanged, onLayerTreeChanged);
    callback();
  }
};
