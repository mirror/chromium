...
Network.NetworkConfigView = class extends UI.VBox {
  ...
  _createMappingEditor() {
    ...
    fields.createChild('div', 'network-config-file-system-mapping-value')
        .appendChild(editor.createInput('domainPath', 'text', 'localhost/path/', (item, index, input) => {
          ...
          if (domainPath === input.value)
            return true;

          for (var interceptionProject of Persistence.networkPersistenceManager.projects()) {
            if (input.value === Persistence.networkPersistenceManager.domainPathForProject(interceptionProject))
              return false;
          }
          return true;
        }));
  ...
  /**
   * @override
   * @param {!{domainPath: string, localPath: string}} item
  ...
  commitEdit(item, editor, isNew) {
    ...
    Persistence.networkPersistenceManager.addFileSystemProject(editor.control('domainPath').value, project);
  }
