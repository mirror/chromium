...
Network.NetworkConfigView = class extends UI.VBox {
  ...
  _createFileServingSection() {
    ...
    section.appendChild(UI.createTextButton(
        ...
        () => this._mappingsList.addNewItem(
            this._networkLocalFileInterceptor.interceptionFiles().length, {url: '', filePath: ''}),
        'add-button'));
  }
        ...
        .appendChild(editor.createInput('url', 'text', 'http://localhost:8000/url', (item, index, input) => {
          return item &&
              (item.url === input.value ||
               !this._networkLocalFileInterceptor.interceptionFiles().find(entry => entry.url === input.value));
        }));
    fields.createChild('div', 'network-config-file-system-mapping-value')
  ...
  _refreshMappingsList() {
    ...
    for (var entry of interceptionFiles)
...
Network.NetworkConfigView.NetworkLocalFileInterceptor = class {
  ...
  setInterceptionFile(url, filePath) {
    /** @type {!Array<!{url: string, filePath: string}>} */
    var interceptionFiles = this._interceptingFilesSetting.get();
    var index = interceptionFiles.findIndex(entry => entry.url === url);
    if (filePath === null) {
      if (index !== -1)
        interceptionFiles.splice(index, 1);
    } else {
      var entry = {url: url, filePath: filePath};
      if (index !== -1)
        interceptionFiles[index] = entry;
      else
        interceptionFiles.push(entry);
    }
  ...
  /**
   * @return {!Array<!{url: string, filePath: string}>}
   */
  interceptionFiles() {
    return this._interceptingFilesSetting.get();
  }
  ...
  _updateInterceptionPatterns() {
    var patterns = this._enabled ? this._interceptingFilesSetting.get().map(entry => entry.url) : [];
  ...
  _interceptionHandler(interceptedRequest) {
    ...
    var interceptionFiles = this._interceptingFilesSetting.get();
    var mappingEntry = interceptionFiles.find(entry => entry.url === url);
    console.assert(mappingEntry);
    return new Promise(resolve => {
      var fileURL = 'file://' + mappingEntry.filePath;
