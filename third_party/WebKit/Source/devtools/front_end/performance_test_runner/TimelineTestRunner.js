...
PerformanceTestRunner.printTraceEventPropertiesWithDetails = function(event) {
  PerformanceTestRunner.printTraceEventProperties(event);
  const details = Timeline.TimelineUIUtils.buildDetailsTextForTraceEvent(event, SDK.targetManager.mainTarget(), new Components.Linkifier());
...
PerformanceTestRunner.loadTimeline = function(timelineData) {
  ...
  UI.panels.timeline._loadFromFile(new Blob([timelineData], {
    type: 'text/plain'
  }));
