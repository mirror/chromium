...
SourcesTestRunner.completeDebuggerTest = function() {
  ...
  SourcesTestRunner.resumeExecution(TestRunner.completeTest.bind(InspectorTest));
};
...
SourcesTestRunner.runDebuggerTestSuite = function(testSuite) {
  ...
  function runner() {
    ...
    TestRunner.addResult('Running: ' + /function\s([^(]*)/.exec(nextTest)[1]);
...
SourcesTestRunner.waitUntilPausedAndPerformSteppingActions = function(actions, callback) {
  ...
  function didPause(callFrames, reason, breakpointIds, asyncStackTrace) {
    ...
    SourcesTestRunner.waitUntilResumed((actions.length ? SourcesTestRunner.waitUntilPaused.bind(InspectorTest, didPause) : callback));
  }
};
...
SourcesTestRunner._pausedScript = function(callFrames, reason, auxData, breakpointIds, asyncStackTrace) {
  ...
  SourcesTestRunner._pausedScriptArguments = [SDK.DebuggerModel.CallFrame.fromPayloadArray(debuggerModel, callFrames), reason, breakpointIds, asyncStackTrace, auxData];
...
SourcesTestRunner.waitForScriptSource = function(scriptName, callback) {
  ...
  TestRunner.addSniffer(
    Sources.SourcesView.prototype,
    '_addUISourceCode',
    SourcesTestRunner.waitForScriptSource.bind(InspectorTest, scriptName, callback)
  );
};
...
SourcesTestRunner.createNewBreakpoint = function(sourceFrame, lineNumber, condition, enabled) {
  var promise = new Promise(resolve => TestRunner.addSniffer(sourceFrame.__proto__, '_breakpointWasSetForTest', resolve));
...
SourcesTestRunner.waitBreakpointSidebarPane = function(waitUntilResolved) {
  return new Promise(resolve => TestRunner.addSniffer(Sources.JavaScriptBreakpointsSidebarPane.prototype, '_didUpdateForTest', resolve)).then(checkIfReady);
...
SourcesTestRunner._expandProperty = function(parentTreeElement, path, pathIndex, callback) {
  ...
  TestRunner.deprecatedRunAfterPendingDispatches(SourcesTestRunner._expandProperty.bind(InspectorTest, propertyTreeElement, path, pathIndex, callback));
};
...
SourcesTestRunner.createScriptMock = function(url, startLine, startColumn, isContentScript, source, target, preRegisterCallback) {
...
SourcesTestRunner.checkUILocation = function(uiSourceCode, lineNumber, columnNumber, location) {
  TestRunner.assertEquals(
    uiSourceCode,
    location.uiSourceCode,
    'Incorrect uiSourceCode, expected \'' + ((uiSourceCode ? uiSourceCode.url() : null)) + '\',' + ' but got \'' + ((location.uiSourceCode ? location.uiSourceCode.url() : null)) + '\''
  );
...
SourcesTestRunner.evaluateOnCurrentCallFrame = function(code) {
  return new Promise(
    succ => TestRunner.debuggerModel.evaluateOnSelectedCallFrame(code, 'console', false, true, false, false, TestRunner.safeWrap(succ))
  );
};
...
SourcesTestRunner.waitJavaScriptSourceFrameBreakpoints = function(sourceFrame, inline) {
  ...
  function waitUpdate() {
    return new Promise(resolve => TestRunner.addSniffer(sourceFrame.__proto__, '_breakpointDecorationsUpdatedForTest', resolve));
  }
...
SourcesTestRunner.dumpJavaScriptSourceFrameBreakpoints = function(sourceFrame) {
  ...
  for (var lineNumber = 0; lineNumber < textEditor.linesCount; ++lineNumber) {
    ...
    TestRunner.addResult('breakpoint at ' + lineNumber + ((disabled ? ' disabled' : '')) + ((conditional ? ' conditional' : '')));
    ...
    for (var bookmark of bookmarks) {
      ...
      TestRunner.addResult(
        '  inline breakpoint at (' + position.startLine + ', ' + position.startColumn + ')' + ((disabled ? ' disabled' : '')) + ((conditional ? ' conditional' : ''))
      );
    }
  }
};
