...
SourcesTestRunner.dumpSearchResults = function(searchResults) {
  ...
  for (var i = 0; i < searchResults.length; i++)
    TestRunner.addResult('url: ' + searchResults[i].url.replace(/VM\d+/, 'VMXX') + ', matchesCount: ' + searchResults[i].matchesCount);
...
SourcesTestRunner.dumpSearchMatches = function(searchMatches) {
  ...
  for (var i = 0; i < searchMatches.length; i++)
    TestRunner.addResult('lineNumber: ' + searchMatches[i].lineNumber + ', line: \'' + searchMatches[i].lineContent + '\'');
...
SourcesTestRunner.runSearchAndDumpResults = function(scope, searchConfig, callback) {
  ...
  function searchFinishedCallback() {
    ...
    for (var i = 0; i < searchResults.length; ++i) {
      ...
      TestRunner.addResult('Search result #' + (i + 1) + ': uiSourceCode.url = ' + uiSourceCode.url().replace(/VM\d+/, 'VMXX'));

      for (var j = 0; j < searchMatches.length; ++j) {
        var lineNumber = searchMatches[j].lineNumber;
        var lineContent = searchMatches[j].lineContent;
        TestRunner.addResult('  search match #' + (j + 1) + ': lineNumber = ' + lineNumber + ', lineContent = \'' + lineContent + '\'');
      }
    }
...
SourcesTestRunner.replaceAndDumpChange = function(sourceFrame, searchConfig, replacement, replaceAll) {
  ...
  TestRunner.addResult('Running replace test for /' + searchConfig.query + '/' + replacement + '/ ' + modifiersString + ':');
