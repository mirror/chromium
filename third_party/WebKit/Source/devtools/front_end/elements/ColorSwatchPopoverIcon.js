...
Elements.ColorSwatchPopoverIcon = class {
  ...
  /**
   * @param {?function():Promise<?SDK.CSSModel.ContrastInfo>} contrastInfoPromise
   */
  setContrastInfoPromise(contrastInfoPromise) {
    this._contrastInfoPromise = contrastInfoPromise;
  }
  ...
  showPopover() {
    ...
    if (Common.moduleSetting('showContrastLine').get() && this._contrastInfoPromise)
      this._contrastInfoPromise().then(this.setContrastInfo.bind(this));
