...
Elements.AnalysisWidget = class extends UI.ThrottledWidget {
  constructor() {
    ...
    SDK.targetManager.addModelListener(
      SDK.AnalysisModel, SDK.AnalysisModel.Events.ClearFlags, this._clearList, this);
  ...
  _elementFlagged(event) {
    this._flagElement(/** @type {!SDK.DOMNode} */ (event.data.node),
  ...
  _listHierarchyForNode(node) {
    ...
    while (node !== null) {
      ...
      var group = groupExists ?
        this._flagGroups.get(node) : this._flagList.createChild('div', 'flag-group');
      if (previous)
