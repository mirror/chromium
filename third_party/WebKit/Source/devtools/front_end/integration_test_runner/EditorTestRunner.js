...
TestRunner.createTestEditor = function(clientHeight, textEditorDelegate) {
...
TestRunner.dumpTextWithSelection = function(textEditor, dumpWhiteSpaces) {
...
TestRunner.setLineSelections = function(editor, selections) {
  var coords = [];
...
TestRunner.typeIn = function(editor, typeText, callback) {
  ...
  for (var charIndex = 0; charIndex < typeText.length; ++charIndex) {
    ...
    switch (typeText[charIndex]) {
      case '\n':
        TestRunner.fakeKeyEvent(editor, 'Enter', null, iterationCallback);
        break;
      case 'L':
        TestRunner.fakeKeyEvent(editor, 'ArrowLeft', null, iterationCallback);
        break;
      case 'R':
        TestRunner.fakeKeyEvent(editor, 'ArrowRight', null, iterationCallback);
        break;
      case 'U':
        TestRunner.fakeKeyEvent(editor, 'ArrowUp', null, iterationCallback);
        break;
      case 'D':
        TestRunner.fakeKeyEvent(editor, 'ArrowDown', null, iterationCallback);
        break;
      default:
        TestRunner.fakeKeyEvent(editor, typeText[charIndex], null, iterationCallback);
    }
  }
};
...
TestRunner.fakeKeyEvent = function(editor, originalCode, modifiers, callback) {
  modifiers = modifiers || [];
  var code;
  var charCode;
...
TestRunner.dumpSelectionStats = function(textEditor) {
  var listHashMap = {};
  var sortedKeys = [];
