...
SDK.TargetManager = class extends Common.Object {
   ...
   * @param {!Protocol.Target.TargetInfo} targetInfo
   */
  inspectAvailableTarget(targetInfo) {
  ...
  detachFromTarget(target) {
    for (var connection of this._discoveryChildTargetManger._childConnections.values()) {
      if (connection._target !== target)
        continue;
      this._discoveryChildTargetManger._targetAgent.detachFromTarget(connection._sessionId);
      return;
    }
  }
...
SDK.ChildTargetManager = class {
  ...
  attachedToTarget(sessionId, targetInfo, waitingForDebugger) {
    ...
    this._childConnections.get(sessionId)._target = target;
