  ...
  static _template(strings) {
    ...
    while (walker.nextNode()) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE && node.hasAttributes()) {
        ...
        ++nodeIndex;
      } else if (node.nodeType === Node.TEXT_NODE && node.data.indexOf(UI.Fragment._textMarker) !== -1) {
      ...
      while (bindIndex < binds.length && binds[bindIndex].nodeIndex === nodeIndex) {
        ...
        if ('elementId' in bind) {
          ...
          if (value instanceof Node) {
            ...
            node.parentNode.insertBefore(value._element, node);
            replacedNodes.push(node);
          } else {
            node.data = '' + value;
          }
          ...
          if (bind.attr.names.length === 2 && bind.attr.values.length === 1 &&
            ...
            values[bind.attr.index].call(null, node);
          } else {
