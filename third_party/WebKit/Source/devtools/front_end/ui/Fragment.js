...
UI.Fragment = class {
  ...
  static _template(strings) {
    ...
    while (walker.nextNode()) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE && node.hasAttributes()) {
        if (node.hasAttribute('$')) {
          node.classList.add(UI.Fragment._class(binds.length));
      ...
      if (node.nodeType === Node.TEXT_NODE && node.data.indexOf(UI.Fragment._textMarker) !== -1) {
        ...
        for (var i = 0; i < texts.length - 1; i++) {
          ...
          nodeToReplace.classList.add(UI.Fragment._class(binds.length));
    ...
    return {content: template.content, binds: binds};
  }
  ...
  static _render(template, values) {
    var content = template.content.ownerDocument.importNode(template.content, true);
    var resultElement =
    ...
    for (var i = 0; i < template.binds.length; i++) {
      var element = /** @type {!Element} */ (content.querySelector('.' + UI.Fragment._class(i)));
      var bind = template.binds[i];
