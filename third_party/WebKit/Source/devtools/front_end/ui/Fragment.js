...
UI._Template = class {
  ...
  constructor(strings) {
    ...
    while (walker.nextNode()) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE && node.hasAttributes()) {
        if (node.hasAttribute('f-id')) {
          this._binds.push({nodeIndex, bindName: node.getAttribute('f-id')});
          node.removeAttribute('f-id');
        }
  ...
  render(values) {
    ...
    for (var shadow of shadows) {
      ...
      shadow.parentElement._shadowRoot = UI.createShadowRootWithCoreStyles(shadow.parentElement);
      while (shadow.firstChild)
        shadow.parentElement._shadowRoot.appendChild(shadow.firstChild);
    }

    /** @type {function(string):!Element} */
    result.byId = UI._Template._byId.bind(null, result);
