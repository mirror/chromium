...
UI.Fragment = class {
   ...
   * @param {function(!Element, !Object, !Array<!Node>):!Element} processor
   */
  ...
  static _template(strings) {
    ...
    var content = UI.Fragment._preprocess(template.content);
        ...
        if (params && params.has(tagName)) {
          ...
          for (var paramChild of child.childNodes) {
            if (paramChild.nodeType === Node.ELEMENT_NODE || paramChild.nodeType === Node.DOCUMENT_FRAGMENT_NODE)
              paramChildren.push(UI.Fragment._preprocess(paramChild));
            else
          ...
          children.push(UI.Fragment._preprocess(child));
        }
      } else {
        children.push(child);
      }
  ...
  static _render(template, values) {
    ...
    for (var bindIndex = 0; bindIndex < template.binds.length; bindIndex++) {
      ...
      if ('elementId' in bind) {
        ...
        if (name.indices.length === 1 && value.indices.length === 0 &&
            typeof values[name.indices[0]] === 'function') {
