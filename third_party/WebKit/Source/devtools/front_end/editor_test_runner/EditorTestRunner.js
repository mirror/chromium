...
EditorTestRunner.createTestEditor = function(clientHeight, textEditorDelegate) {
...
EditorTestRunner.dumpTextWithSelection = function(textEditor, dumpWhiteSpaces) {
...
EditorTestRunner.setLineSelections = function(editor, selections) {
  var coords = [];
...
EditorTestRunner.typeIn = function(editor, typeText, callback) {
  ...
  for (var charIndex = 0; charIndex < typeText.length; ++charIndex) {
    ...
    switch (typeText[charIndex]) {
      case '\n':
        EditorTestRunner.fakeKeyEvent(editor, 'Enter', null, iterationCallback);
        break;
      case 'L':
        EditorTestRunner.fakeKeyEvent(editor, 'ArrowLeft', null, iterationCallback);
        break;
      case 'R':
        EditorTestRunner.fakeKeyEvent(editor, 'ArrowRight', null, iterationCallback);
        break;
      case 'U':
        EditorTestRunner.fakeKeyEvent(editor, 'ArrowUp', null, iterationCallback);
        break;
      case 'D':
        EditorTestRunner.fakeKeyEvent(editor, 'ArrowDown', null, iterationCallback);
        break;
      default:
        EditorTestRunner.fakeKeyEvent(editor, typeText[charIndex], null, iterationCallback);
    }
  }
};
...
EditorTestRunner.fakeKeyEvent = function(editor, originalCode, modifiers, callback) {
  modifiers = modifiers || [];
  var code;
  var charCode;
...
EditorTestRunner.dumpSelectionStats = function(textEditor) {
  var listHashMap = {};
  var sortedKeys = [];
