...
Resources.ServiceWorkerCacheView = class extends UI.SimpleView {
  ...
  constructor(model, cache) {
    ...
    var dirty = createElement('div');
    var dirtyIcon = dirty.createChild('label', '', 'dt-icon-label');
    dirtyIcon.type = 'smallicon-warning';
    dirtyIcon.createChild('span').textContent = Common.UIString('Refresh needed');
    this._dirty = new UI.ToolbarItem(dirty);
    this._dirty.setVisible(false);
    this._dirty.setTitle(Common.UIString('Some entries have been modified'));
    editorToolbar.appendSpacer();
    editorToolbar.appendToolbarItem(this._dirty);
