...
Resources.ServiceWorkerCacheView = class extends UI.SimpleView {
  ...
  constructor(model, cache) {
    ...
    /** @type {!UI.Widget} */
    this._preview = new UI.EmptyWidget('Select a cache entry above to preview');
  ...
  update(cache) {
    ...
    var gridWidget = this._dataGrid.asWidget();
    gridWidget.show(this.element);
    this._splitWidget.setSidebarWidget(gridWidget);
    this._skipCount = 0;
   ...
   */
  _responseForUrl(url) {
    for (var i = 0; i < this._recentlyPreviewedResponses.length; i++) {
      var entry = this._recentlyPreviewedResponses[i];
      if (entry.url !== url)
        continue;
      // Warm up
      this._recentlyPreviewedResponses[i] = this._recentlyPreviewedResponses[0];
      this._recentlyPreviewedResponses[0] = entry;
      return entry;
    }
    var found = new Resources._CachedResponse(this._cache, url);
    // Arbitrary cache size limit (10 entries).
    if (this._recentlyPreviewedResponses.length === 10)
      this._recentlyPreviewedResponses.pop();
    this._recentlyPreviewedResponses.unshift(found);
    return found;
  }
