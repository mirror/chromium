...
Resources.FrameResourceTreeElement = class extends Resources.BaseStorageTreeElement {
  ...
  onselect(selectedByUser) {
    super.onselect(selectedByUser);
    this._panel.showResourcePreview(this._resource);
    return false;
  }
   ...
   * @param {number=} column
   */
  revealResource(line, column) {
    this.revealAndSelect(true);
    if (typeof line === 'number' && this._preview instanceof SourceFrame.ResourceSourceFrame)
      this._preview.revealPosition(line, column, true);
  }
};
