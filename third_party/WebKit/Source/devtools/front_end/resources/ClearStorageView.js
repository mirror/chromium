...
Resources.ClearStorageView = class extends UI.VBox {
  ...
  /**
   * @param {?Protocol.Storage.QuotasAndUsage} quotasAndUsage
   */
  _updateQuotaDisplay(quotasAndUsage) {
    ...
    this._temporaryQuotaRow.textContent = Common.UIString(
        '%s used out of %s temporary storage quota', Number.bytesToString(quotasAndUsage.temporaryStorageUsage),
        Number.bytesToString(quotasAndUsage.temporaryStorageQuota));
    this._persistentQuotaRow.textContent = Common.UIString(
        '%s used out of %s persistent storage quota', Number.bytesToString(quotasAndUsage.persistentStorageUsage),
        Number.bytesToString(quotasAndUsage.persistentStorageQuota));
  }
