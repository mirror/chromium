...
Resources.ServiceWorkersView = class extends UI.VBox {
  ...
  _updateSectionVisibility() {
    ...
    for (var section of this._sections.values()) {
      if (matchingSections.has(section._section))
        continue;
      if (!this._filter.isRegistrationVisible(section._registration))
        section._section.hideWidget();
    }
    var hidden = this._sections.size - matchingSections.size;
    if (!hidden)
      filter.hideWidget();
    else
      this._filter.setLabel(Common.UIString('Show all service workers (%d)', hidden));
  }
