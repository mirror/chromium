  ...
  _updateRegistration(registration) {
    ...
    if (!section) {
      section = new Resources.ServiceWorkersView.Section(
          /** @type {!SDK.ServiceWorkerManager} */ (this._manager), this._reportView.appendSection(''), registration);
...
Resources.ServiceWorkersView.Section = class {
  ...
  constructor(manager, section, registration) {
    ...
    this._errorsField = this._wrapWidget(this._section.appendField(Common.UIString('Errors')));
  ...
  _updateSourceField(version) {
    ...
    this._sourceField.appendChild(Components.Linkifier.linkifyURL(version.scriptURL, {text: fileName}));
  ...
  _update() {
    ...
    this._errorsField.removeChildren();
    this._section.setFieldVisible(Common.UIString('Errors'), !!this._registration.errors.length);

    var errorLink = this._errorsField.createChild('span', 'link open-console-link');
    errorLink.addEventListener('click', () => Common.console.show());
    ...
    errorLink.appendChild(errorsLabel);
