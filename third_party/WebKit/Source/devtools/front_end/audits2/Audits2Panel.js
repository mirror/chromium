...
Audits2.Audits2Panel.StatusView = class {
  ...
  updateStatus(message) {
    ...
    if (!this._currentPhase && message.startsWith(Audits2.Audits2Panel.StatusView.StatusPhases.Loading)) {
      this._currentPhase = Audits2.Audits2Panel.StatusView.StatusPhases.Loading;
      ...
      this._scheduleProgressAnimation(.7, 45000);
    } else if (
        this._currentPhase === Audits2.Audits2Panel.StatusView.StatusPhases.Loading &&
        message.startsWith(Audits2.Audits2Panel.StatusView.StatusPhases.Gathering)) {
      this._currentPhase = Audits2.Audits2Panel.StatusView.StatusPhases.Gathering;
      this._scheduleTextChange(
          Common.UIString('Lighthouse is gathering information about the page to compute your score.'));
      this._scheduleProgressAnimation(.95, 12000);
    } else if (
        this._currentPhase === Audits2.Audits2Panel.StatusView.StatusPhases.Gathering &&
        message.startsWith(Audits2.Audits2Panel.StatusView.StatusPhases.Auditing)) {
      this._currentPhase = Audits2.Audits2Panel.StatusView.StatusPhases.Auditing;
      this._scheduleTextChange(
          Common.UIString('Almost there! Lighthouse is now generating your own special pretty report!'));
      this._scheduleProgressAnimation(1, 3000);
  ...
  _updateFastFactIfNecessary() {
    ...
    if (now - this._progressUpdateStartedAt < 6000)
      return;
    if (this._fastFactUpdatedAt && now - this._fastFactUpdatedAt < 6000)
      return;
  ...
  _scheduleTextChange(text, callback) {
    ...
    this._scheduledTextChangeTimeout = setTimeout(() => {
      ...
      if (callback)
        callback();
    }, Math.max(3000 - elapsedSinceLastChange, 0));
  }
