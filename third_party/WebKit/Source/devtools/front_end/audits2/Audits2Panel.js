...
Audits2.Audits2Panel = class extends UI.Panel {
  ...
  _updateInspectedURL() {
    ...
    return new Promise(resolve => {
      executionContext
          .evaluate(
              'window.location.href', 'audits', /* useCommandLineAPI */ false, /* silent */ false,
              /* returnByValue */ true, /* generatePreview*/ false, /* userGesture */ false, /* awaitPromise */ false)
          .then(result => {
            if (!result.exceptionDetails && result.result) {
              this._inspectedURL = result.result.value;
