...
Audits2.Audits2Panel.StatusView = class {
  constructor() {
    ...
    this._fastFactsQueued = Array.from(Audits2.Audits2Panel.StatusView.FastFacts);
  ...
  _getPhaseForMessage(message) {
    ...
    if (message.startsWith(phases.Loading.statusMessagePrefix))
      return phases.Loading;
    else if (message.startsWith(phases.Gathering.statusMessagePrefix))
      return phases.Gathering;
    else if (message.startsWith(phases.Auditing.statusMessagePrefix))
      return phases.Auditing;
    else
      return null;
  }
  ...
  _resetProgressBarClasses() {
    ...
    var classnames = Object.values(Audits2.Audits2Panel.StatusView.StatusPhases).map(phase => phase.progressBarClass);
    this._progressBar.classList.remove(...classnames);
  }
  ...
  _updateFastFactIfNecessary() {
    ...
    this._scheduleTextChange('💡 ' + this._fastFactsQueued[fastFactIndex]);
