...
Audits2.Audits2Panel.StatusView = class {
  constructor() {
    ...
    this._textCanBeReplacedAtWill = true;
    this._fastFactsUsed = new Set();
  ...
  updateStatus(message) {
    ...
    if (!this._currentPhase && message.startsWith(phases.Loading.statusMessagePrefix)) {
      this._currentPhase = phases.Loading;
      this._scheduleTextChange(Common.UIString(this._currentPhase.message));
      this._scheduleFastFactCheck();
      this._resetProgressBarClasses();
      this._progressBar.classList.add('loading');
    } else if (this._currentPhase === phases.Loading && message.startsWith(phases.Gathering.statusMessagePrefix)) {
      this._currentPhase = phases.Gathering;
      this._scheduleTextChange(Common.UIString(this._currentPhase.message));
      this._scheduleFastFactCheck();
      this._resetProgressBarClasses();
      this._progressBar.classList.add('gathering');
    } else if (this._currentPhase === phases.Gathering && message.startsWith(phases.Auditing.statusMessagePrefix)) {
      this._currentPhase = phases.Auditing;
      this._scheduleTextChange(Common.UIString(this._currentPhase.message));
      this._scheduleFastFactCheck();
      this._resetProgressBarClasses();
      this._progressBar.classList.add('auditing');
    } else if (message.startsWith('Cancel')) {
      this._commitTextChange(Common.UIString('Cancelling\u2026'), true);
      clearTimeout(this._scheduledFastFactTimeout);
    } else if (!this._currentPhase) {
      this._commitTextChange(Common.UIString('Lighthouse is warming up\u2026'), true);
      clearTimeout(this._scheduledFastFactTimeout);
    } else {
  ...
  _resetProgressBarClasses() {
    ...
    this._progressBar.classList.remove('gathering');
    this._progressBar.classList.remove('auditing');
  }
  ...
  _updateFastFactIfNecessary() {
    ...
    if (now - this._textChangedAt < Audits2.Audits2Panel.StatusView.fastFactRotationInterval)
      return;
    if (this._fastFactsUsed.size >= Audits2.Audits2Panel.StatusView.FastFacts.length - 2)
      return;

    var fastFactIndex = -1;
    while (fastFactIndex === -1 || this._fastFactsUsed.has(fastFactIndex))
      fastFactIndex = Math.round(Math.random() * (Audits2.Audits2Panel.StatusView.FastFacts.length - 1));

    this._fastFactsUsed.add(fastFactIndex);
    this._scheduleTextChange('💡 ' + Audits2.Audits2Panel.StatusView.FastFacts[fastFactIndex]);
  }

  /**
   * @param {string} text
   * @param {boolean=} canBeReplacedAtWill
   */
  _commitTextChange(text, canBeReplacedAtWill) {
    if (!this._statusText)
      return;
    this._textChangedAt = performance.now();
    this._textCanBeReplacedAtWill = canBeReplacedAtWill;
  ...
  _scheduleTextChange(text) {
    ...
    var timeElapsedSinceLastChange = performance.now() - this._textChangedAt;
    var timeRemainingOnCurrentText =
        Audits2.Audits2Panel.StatusView.minimumTextVisibilityDuration - timeElapsedSinceLastChange;
    var timeToTextChange = this._textCanBeReplacedAtWill ? 0 : timeRemainingOnCurrentText;

    this._scheduledTextChangeTimeout = setTimeout(() => {
      this._commitTextChange(text);
    }, Math.max(timeToTextChange, 0));
  }
...
Audits2.Audits2Panel.StatusView.FastFacts = [
  ...
  '70% of mobile pages take nearly 7 seconds for the visual content above the fold to display on the screen. [Source: Think with Googled]',
