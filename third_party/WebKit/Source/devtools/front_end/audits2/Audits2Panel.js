...
Audits2.Audits2Panel = class extends UI.PanelWithSidebar {
   ...
   * @return {?string}
   */
  _getUnauditablePageMessage() {
    var inspectedURL = SDK.targetManager.mainTarget().inspectedURL();
    if (/^about:/.test(inspectedURL))
      return 'Cannot audit about:* pages. Navigate to a different page to start an audit.';
  ...
  _updateStartButtonEnabled() {
    ...
    var unauditablePageMessage = this._getUnauditablePageMessage();
    ...
    if (this._dialogHelpText && unauditablePageMessage)
      this._dialogHelpText.textContent = Common.UIString(unauditablePageMessage);
