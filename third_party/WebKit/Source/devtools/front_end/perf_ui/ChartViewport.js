...
PerfUI.ChartViewport = class extends UI.VBox {
  ...
  _updateContentElementSize() {
    ...
    this._delegate.setDimensions(this._offsetWidth, this._offsetHeight);
  }
  ...
  _onMouseWheel(e) {
    ...
    if (panVertically) {
      ...
      var shift = -this.pixelToTimeOffset(e.wheelDeltaX);
  ...
  _dragging(event) {
    ...
    this._dragStartPointX = event.pageX;
    this._muteAnimation = true;
    this._handlePanGesture(this.pixelToTimeOffset(pixelShift));
    this._muteAnimation = false;
  ...
  _handleZoomPanKeys(e) {
    ...
    if (e.code === 'KeyA') {
      this._handlePanGesture(this.pixelToTimeOffset(-panMultiplier));
      e.consume(true);
    } else if (e.code === 'KeyD') {
      this._handlePanGesture(this.pixelToTimeOffset(panMultiplier));
      e.consume(true);
   ...
   */
  _windowForGesture() {
    return {left: this._timeWindowLeft, right: this._timeWindowRight};
  }
