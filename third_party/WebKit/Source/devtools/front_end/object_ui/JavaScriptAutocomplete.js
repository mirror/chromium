...
ObjectUI.JavaScriptAutocomplete = class {
  constructor() {
    /** @type {!Object<string, {date: number, value: !Promise<?Object>}>} */
    this._expressionCache = {__proto__: null};
  ...
  async _completionsForExpression(expressionString, query, force) {
    ...
    if (this._expressionCache[expressionString] && this._expressionCache[expressionString].date + 10000 > Date.now()) {
      completionGroups = await this._expressionCache[expressionString].value;
    } else if (!expressionString && selectedFrame) {
      this._expressionCache[expressionString] = {date: Date.now(), value: completionsOnPause(selectedFrame)};
      completionGroups = await this._expressionCache[expressionString].value;
    } else {
      var resultPromise = executionContext.evaluate(
          ...
          /* userGesture */ false, /* awaitPromise */ false);
      this._expressionCache[expressionString] = {
        date: Date.now(),
        value: resultPromise.then(result => completionsOnGlobal.call(this, result))
      };
      completionGroups = await this._expressionCache[expressionString].value;
    }
  ...
  _completionsForQuery(dotNotation, bracketNotation, expressionString, query, propertyGroups) {
    propertyGroups = propertyGroups.slice(0);
    if (bracketNotation) {
