...
ObjectUI.RemoteObjectPreviewFormatter = class {
  ...
  _appendObjectPropertiesPreview(parentElement, preview) {
    ...
    if (preview.subtype === 'promise')
      ...
      remainingProperties = appendPrimitiveValue.call(this);
    ...
    function appendPrimitiveValue() {
      var primitiveIndex =
          properties.findIndex(p => p.name === ObjectUI.RemoteObjectPreviewFormatter._internalName.PrimitiveValue);
      if (primitiveIndex !== -1) {
        var primitiveProperty = properties[primitiveIndex];
        parentElement.appendChild(this._renderPropertyPreviewOrAccessor([primitiveProperty]));
        // TODO(luoe) remove logic in this 'if' condition when it lands in injected script source.
        if (primitiveProperty.type === 'string') {
          var value = primitiveProperty.value;
          return properties.filter((p, index) => index !== primitiveIndex && value[p.name] !== p.value);
        }
      }
      return properties.filter((p, index) => index !== primitiveIndex);
    }
