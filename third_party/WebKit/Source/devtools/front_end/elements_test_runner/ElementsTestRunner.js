...
ElementsTestRunner.dumpSelectedElementStyles = function(excludeComputed, excludeMatched, omitLonghands, includeSelectorGroupMarks) {
  var sectionBlocks = UI.panels.elements._stylesWidget._sectionBlocks;
...
ElementsTestRunner.expandAndDumpSelectedElementEventListeners = function(callback, force) {
  ElementsTestRunner.expandAndDumpEventListeners(ElementsTestRunner.eventListenersWidget()._eventListenersView, callback, force);
};
...
ElementsTestRunner.dumpStyleTreeItem = function(treeItem, prefix, depth) {
  if (treeItem.listItemElement.textContent.indexOf(' width:') !== -1 || treeItem.listItemElement.textContent.indexOf(' height:') !== -1)
    return;
...
ElementsTestRunner.dumpRule = function(rule, currentIndent) {
  ...
  if (rule.type === 'import') {
    TestRunner.addResult(
      currentIndent + '@import: header=' + ElementsTestRunner.rangeText(rule.headerRange) + ', body=' + ElementsTestRunner.rangeText(rule.bodyRange)
    );
  ...
  TestRunner.addResult(
    currentIndent + '[UNKNOWN RULE]: header=' + ElementsTestRunner.rangeText(rule.headerRange) + ', body=' + ElementsTestRunner.rangeText(rule.bodyRange)
  );
};
...
ElementsTestRunner.dumpStyle = function(style, currentIndent) {
  ...
  for (var i = 0; i < style.cssProperties.length; ++i) {
    ...
    if (!property.disabled) TestRunner.addResult(
      currentIndent + '[\'' + property.name + '\':\'' + property.value + '\'' + ((property.important ? ' is-important' : '')) + (('parsedOk' in property ? ' non-parsed' : '')) + '] @' + ElementsTestRunner.rangeText(property.range) + ' '
    );
    else
...
ElementsTestRunner.dumpCSSStyleDeclaration = function(style, currentIndent) {
  ...
  for (var i = 0; i < properties.length; ++i) {
    ...
    if (!property.disabled) TestRunner.addResult(
      currentIndent + '[\'' + property.name + '\':\'' + property.value + '\'' + ((property.important ? ' is-important' : '')) + ((!property['parsedOk'] ? ' non-parsed' : '')) + '] @' + ElementsTestRunner.rangeText(property.range) + ' '
    );
    else
      TestRunner.addResult(currentIndent + '[text=\'' + property.text + '\'] disabled');
  }
...
ElementsTestRunner.addNewRuleInStyleSheet = function(styleSheetHeader, selector, callback) {
  TestRunner.addSniffer(Elements.StylesSidebarPane.prototype, '_addBlankSection', onBlankSection.bind(null, selector, callback));
