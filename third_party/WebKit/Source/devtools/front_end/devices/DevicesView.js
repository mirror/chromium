...
Devices.DevicesView = class extends UI.VBox {
  constructor() {
    ...
    `;
    this.contentElement.appendChild(this._fragment);
    ...
    this._discoveryListItem.$('title').textContent = s`Settings`;
    this._discoveryListItem.$('status').parentNode.remove();
    this._fragment.$('spacer').parentNode.appendChild(this._discoveryListItem);
  ...
  _devicesUpdated(event) {
    ...
    for (var device of this._devices) {
      ...
      if (!view) {
        ...
        this._fragment.$('spacer').parentNode.insertBefore(listItem, this._fragment.$('spacer'));
      }

      listItem.$('title').textContent = device.adbModel;
      listItem.$('status').textContent = device.adbConnected ? s`Connected` : s`Pending Authorization`;
