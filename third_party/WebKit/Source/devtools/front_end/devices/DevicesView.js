...
Devices.DevicesView = class extends UI.VBox {
  constructor() {
    ...
    this._template = UI.html`
      <x-hbox x-stretch flex=auto overflow=hidden>
        ...
        <x-vbox x-stretch flex=auto el=viewContainer></x-vbox>
      </x-hbox>
  ...
  _selectSidebarListItem(listItem, view) {
    ...
    if (this._visibleView instanceof Element)
      this._template.el('viewContainer').appendChild(this._visibleView);
    else
  ...
  _devicesUpdated(event) {
    ...
    for (var device of this._devices) {
      ...
      listItem.toggle('connected', device.adbConnected);
      view.update(device);
    }
  ...
  _createSidebarListItem(view) {
    var item = UI.cachedHtml`
      <x-vbox flex=auto padding='6px 6px 6px 16px' color=#222 font-size=14px
          x-selected-border-left='6px solid #666' x-selected-padding-left=10px>
        <div el=title></div>
        <div>
          <x-text margin='0 2px 0 -10px' color=red font-size=16px x-connected-color=green>\u25cf</x-text>
...
Devices.DevicesView.DiscoveryView = class {
  constructor() {
    ...
    this._widget = UI.html`
      <x-widget x-shadow overflow-x=hidden overflow-y=auto padding='15px 15px 0 0'>
