...
Devices.DevicesView = class extends UI.VBox {
  constructor() {
    ...
    this._template = UI.Fragment.build`
      <x-hbox x-stretch flex=auto overflow=hidden>
        <x-vbox x-stretch flex=150px padding-top=15px>
          ...
          <x-vbox flex=none>
            <x-vbox flex=none f-id=spacer></x-vbox>
          </x-vbox>
        </x-vbox>
        <x-vbox x-stretch flex=auto f-id=viewContainer></x-vbox>
      </x-hbox>
      <x-hbox flex=none overflow=hidden padding='3px 10px' border-top='1px solid #cdcdcd' background-color=#f3f3f3>
        <span f-id=deviceCount></span>
        <x-text> Read <x-link
    ...
    this._discoveryListItem.byId('title').textContent = s`Settings`;
    this._discoveryListItem.byId('status').parentNode.remove();
    this._template.byId('spacer').parentNode.appendChild(this._discoveryListItem);
  ...
  _selectSidebarListItem(listItem, view) {
    ...
    if (this._visibleView instanceof Element)
      this._template.byId('viewContainer').appendChild(this._visibleView);
    else
      this._visibleView.show(this._template.byId('viewContainer'));
  ...
  _devicesUpdated(event) {
    ...
    for (var device of this._devices) {
      ...
      if (!view) {
        ...
        this._template.byId('spacer').parentNode.insertBefore(listItem, this._template.byId('spacer'));
      }

      listItem.byId('title').textContent = device.adbModel;
      listItem.byId('status').textContent = device.adbConnected ? s`Connected` : s`Pending Authorization`;
  ...
  _createSidebarListItem(view) {
    var item = UI.Fragment.cached`
      <x-vbox flex=auto padding='6px 6px 6px 16px' color=#222 font-size=14px
          f-selected-border-left='6px solid #666' f-selected-padding-left=10px>
        <div f-id=title></div>
        <div>
          <x-text margin='0 2px 0 -10px' color=red font-size=16px f-connected-color=green>\u25cf</x-text>
          <x-span f-id=status font-size=12px></x-span>
        </div>
      </x-vbox>
    `;
  ...
  _updateFooter() {
    this._template.byId('deviceCount').textContent = !this._devices.length ?
...
Devices.DevicesView.DiscoveryView = class {
  constructor() {
    ...
    this._fragment = UI.Fragment.build`
      <x-widget x-shadow overflow-x=hidden overflow-y=auto padding='15px 15px 0 0'>
        ...
        <x-checkbox f-id=discoverUsbDevices flex=none padding-bottom=8px margin-top=20px>Discover USB devices</x-checkbox>
    ...
    `;
    this._widget = this._fragment.asWidget();
    this._widget.registerRequiredCSS('devices/devicesView.css');

    this._fragment.byId('discoverUsbDevices').addEventListener('click', () => {
      // TODO: this does not update!
      this._config.discoverUsbDevices = this._fragment.byId('discoverUsbDevices').checked;
  ...
  discoveryConfigChanged(config) {
    this._config = config;
    this._fragment.byId('discoverUsbDevices').checked = config.discoverUsbDevices;
