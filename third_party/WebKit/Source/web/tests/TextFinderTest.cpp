...
class TextFinderTest : public ::testing::Test {
 protected:
  TextFinderTest() {
    ...
    WebLocalFrameImpl& frame_impl = *web_view_helper_.WebView()->MainFrameImpl();
...
TEST_F(TextFinderTest, FindTextSimple) {
  ...
  ASSERT_TRUE(
      GetTextFinder().Find(identifier, search_text, find_options, wrap_within_frame));
...
TEST_F(TextFinderTest, FindTextAutosizing) {
  ...
  GetDocument().GetSettings()->SetTextAutosizingWindowSizeOverride(IntSize(20, 20));
