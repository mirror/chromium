...
class RootScrollerTest : public ::testing::Test {
 public:
  ...
  WebLocalFrame* MainWebFrame() const { return GetWebViewImpl()->MainFrameImpl(); }

  FrameView* MainFrameView() const {
    return GetWebViewImpl()->MainFrameImpl()->GetFrame()->View();
  }

  VisualViewport& GetVisualViewport() const { return GetPage().GetVisualViewport(); }

  BrowserControls& GetBrowserControls() const { return GetPage().GetBrowserControls(); }
  ...
  WebCoalescedInputEvent GenerateWheelGestureEvent(WebInputEvent::Type type,
                                                   int delta_x = 0,
                                                   int delta_y = 0) {
    return GenerateGestureEvent(type, kWebGestureDeviceTouchpad, delta_x, delta_y);
  }
...
TEST_F(RootScrollerTest, TestSetRootScroller) {
...

  {
    ...
    GetWebViewImpl()->HandleInputEvent(
        GenerateTouchGestureEvent(WebInputEvent::kGestureScrollUpdate, 0, -100));
...

  {
    ...
    GetWebViewImpl()->HandleInputEvent(
        GenerateTouchGestureEvent(WebInputEvent::kGestureScrollUpdate, 0, -500));
...
TEST_F(RootScrollerTest, TestSetRootScrollerOnElementInIframe) {
...

  {
    ...
    EXPECT_EQ(inner_container, EffectiveRootScroller(MainFrame()->GetDocument()));
  }
...
TEST_F(RootScrollerTest, TestRootScrollerWithinIframe) {
...

  {
    ...
    EXPECT_EQ(inner_container, EffectiveRootScroller(iframe->contentDocument()));
  }
}
...
TEST_F(RootScrollerTest,
  ...
  ASSERT_EQ(nullptr, MainFrame()->GetDocument()->rootScroller());
  {
    ...
    HTMLFrameOwnerElement* iframe = ToHTMLFrameOwnerElement(
        MainFrame()->GetDocument()->GetElementById("iframe"));
    NonThrowableExceptionState non_throw;
    Element* body = MainFrame()->GetDocument()->QuerySelector("body", non_throw);
...
TEST_F(RootScrollerTest, RemoteMainFrame) {
  ...
  // Try scrolling in the iframe.
  {
    ...
    widget->HandleInputEvent(
        GenerateWheelGestureEvent(WebInputEvent::kGestureScrollUpdate, 0, -100));
    widget->HandleInputEvent(
...
TEST_F(RootScrollerTest, RemoveRootScrollerFromDom) {
...

  {
    ...
    ASSERT_EQ(inner_container, EffectiveRootScroller(iframe->contentDocument()));
