...
TEST_F(WebDocumentFirstPartyTest, NestedOriginA) {
  ...
  ASSERT_EQ(ToOriginA(g_nested_origin_a), TopDocument()->FirstPartyForCookies());
  ASSERT_EQ(ToOriginA(g_nested_origin_a), NestedDocument()->FirstPartyForCookies());
}

TEST_F(WebDocumentFirstPartyTest, NestedOriginSubA) {
  Load(g_nested_origin_sub_a);

  ASSERT_EQ(ToOriginA(g_nested_origin_sub_a), TopDocument()->FirstPartyForCookies());
...
TEST_F(WebDocumentFirstPartyTest, NestedOriginB) {
  ...
  ASSERT_EQ(ToOriginA(g_nested_origin_b), TopDocument()->FirstPartyForCookies());
  ASSERT_EQ(SecurityOrigin::UrlWithUniqueSecurityOrigin(),
            NestedDocument()->FirstPartyForCookies());
}
...
TEST_F(WebDocumentFirstPartyTest, NestedSrcdoc) {
  ...
  ASSERT_EQ(ToOriginA(g_nested_src_doc), NestedDocument()->FirstPartyForCookies());
}
