...
TEST_F(LinkSelectionTest, HandCursorDuringLinkDrag) {
  EmulateMouseDrag(right_point_in_link_, left_point_in_link_, 0, kSendDownEvent);
  main_frame_->GetFrame()->LocalFrameRoot()->GetEventHandler().ScheduleCursorUpdate();
...
TEST_F(LinkSelectionTest, CaretCursorOverLinkDuringSelection) {
  EmulateMouseDrag(right_point_in_link_, left_point_in_link_, WebInputEvent::kAltKey,
                   kSendDownEvent);
  main_frame_->GetFrame()->LocalFrameRoot()->GetEventHandler().ScheduleCursorUpdate();
  testing::RunDelayedTasks(50);
  const auto& cursor =
