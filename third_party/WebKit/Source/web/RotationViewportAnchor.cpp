...
void RotationViewportAnchor::SetAnchor() {
  ...
  if (inner_view_rect.Location() == IntPoint::Zero() || inner_view_rect.IsEmpty())
    return;
  ...
  Node* node = FindNonEmptyAnchorNode(FlooredIntPoint(anchor_point_in_contents),
                                      inner_view_rect,
...
FloatPoint RotationViewportAnchor::GetInnerOrigin(
  ...
  FloatSize anchor_offset_from_node(current_node_bounds_in_layout_viewport.size());
