...
void PopupMenuImpl::AddElementStyle(ItemIterationContext& context,
  ...
  const FontDescription& font_description = style->GetFont().GetFontDescription();
  ...
  if (base_font.Style() != font_description.Style())
    AddProperty("fontStyle", String(FontStyleToString(font_description.Style())),
                data);
...
void PopupMenuImpl::UpdateFromElement(UpdateReason) {
  if (needs_update_)
    return;
  needs_update_ = true;
  TaskRunnerHelper::Get(TaskType::kUserInteraction, &OwnerElement().GetDocument())
