...
static void FrameContentAsPlainText(size_t max_chars,
  ...
  for (Frame* cur_child = frame_tree.FirstChild(); cur_child;
    ...
    if (content_layout_item.IsNull() || !content_layout_item.size().Width() ||
        ...
        (content_layout_item.Location().X() + content_layout_item.size().Width() <=
         0) ||
        (content_layout_item.Location().Y() + content_layout_item.size().Height() <=
         0) ||
...
WebString WebFrameContentDumper::DeprecatedDumpFrameTreeAsText(
  ...
  if (!frame)
    return WebString();
  StringBuilder text;
  FrameContentAsPlainText(max_chars, ToWebLocalFrameImpl(frame)->GetFrame(), text);
...
WebString WebFrameContentDumper::DumpLayoutTreeAsText(
  ...
  return ExternalRepresentation(ToWebLocalFrameImpl(frame)->GetFrame(), behavior);
}
}
