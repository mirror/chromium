...
class RebaselineCL(AbstractParallelRebaselineCommand):
    ...
    long_help = ('This command downloads new baselines for failing layout '
                 ...
                 'positional parameters or --testname-file, all failing tests '
    ...
    def __init__(self):
        super(RebaselineCL, self).__init__(options=[
            ...
            optparse.make_option(
                '--testname-file', dest='testname_file', default=None,
    ...
    def execute(self, options, args, tool):
        ...
        if args and options.testname_file:
            _log.info('Aborted: Cannot combine --testname-file and positional '
                      'parameters.')
            return 1
        ...
        if options.testname_file:
            test_baseline_set = self._make_test_baseline_set_from_file(
                options.testname_file, jobs_to_results)
        elif args:
