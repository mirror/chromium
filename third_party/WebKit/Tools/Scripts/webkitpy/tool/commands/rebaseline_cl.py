...
class RebaselineCL(AbstractParallelRebaselineCommand):
    ...
    def execute(self, options, args, tool):
        ...
        if options.builders:
            ...
            self._try_bots = frozenset(try_builders)

        jobs = self.git_cl.latest_try_jobs(self.try_bots)
        self._log_jobs(jobs)
        builders_with_no_jobs = self.try_bots - {b.builder_name for b in jobs}
        ...
        builders_without_results = set(self.try_bots) - builders_with_results
    ...
    def _log_jobs(self, jobs):
        ...
        if self.try_bots.issubset(set([b.builder_name for b in finished_jobs])):
    ...
    def fill_in_missing_results(self, test_baseline_set):
        ...
        """
        all_ports = {self._tool.builders.port_name_for_builder_name(b) for b in self.try_bots}
