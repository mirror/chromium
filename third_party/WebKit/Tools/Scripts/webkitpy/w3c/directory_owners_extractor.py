...
class DirectoryOwnersExtractor(object):
    ...
    def list_owners(self, changed_files):
        ...
        for relpath in changed_files:
            ...
            if not owners_file:
                continue
            # "owned directories" relative to the root of layout tests are used in CL for simplicity.
        ...
        return {owners: list(owned_directories) for owners, owned_directories in email_map.iteritems()}
    ...
    def find_and_extract_owners(self, start_directory):
        ...
        first non-empty OWNERS file is found or the root of WPT (external/wpt)
        is reached. (OWNERS files with no valid emails are also considered empty.)
        ...
        wpt_root = self.finder.path_from_layout_tests('external', 'wpt')
        while directory != wpt_root:
