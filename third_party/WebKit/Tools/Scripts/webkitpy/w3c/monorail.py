...
class MonorailIssue(object):
    ...
    # Doc: https://chromium.googlesource.com/infra/infra/+/master/appengine/monorail/doc/api.md#monorail_issues_insert
    _LIST_OF_STRING_FIELDS = frozenset(['cc', 'labels', 'components'])
    _STRING_FIELDS = frozenset(['status', 'summary', 'description'])
    _ALLOWED_FIELDS = _LIST_OF_STRING_FIELDS | _STRING_FIELDS

    # Again, the list is non-exhaustive.
    _VALID_STATUS = frozenset(['Unconfirmed', 'Untriaged', 'Available', 'Assigned', 'Started'])
    ...
    def _normalize(self):
        # These requirements are based on trial and errors. No docs are found.
        assert self.project_id, 'project_id cannot be empty.'
        for field in self._LIST_OF_STRING_FIELDS:
        ...
        assert self._body['status'] in self._VALID_STATUS, 'Unknown status %s.' % self._body['status']
    ...
    def new_chromium_issue(summary, description='', cc=None, components=None):
        ...
        Args:
            summary: The summary line (required).
            description: The issue description (optional).
            cc: A list of email addresses to CC (optional).
            components: A list of components (optional).
        """
        # [] is a dangerous default value for arguments, so we use the idiom below.
...
class MonorailAPI(object):
    ...
    def insert_issue(self, issue):
        response = self.api.issues().insert(projectId=issue.project_id, body=issue.body).execute()
        return response
