...
class LocalWPTTest(unittest.TestCase):
    ...
    def test_create_branch_with_patch(self):
        ...
        local_branch_name = local_wpt.create_branch_with_patch('chromium-export-decafbad', 'message', 'patch', 'author')
        self.assertEqual(local_branch_name, 'chromium-export-decafbad')
        self.assertEqual(host.executive.calls, [
            ['git', 'clone', 'https://token@github.com/w3c/web-platform-tests.git', '/tmp/wpt'],
            ['git', 'reset', '--hard', 'HEAD'],
            ['git', 'clean', '-fdx'],
