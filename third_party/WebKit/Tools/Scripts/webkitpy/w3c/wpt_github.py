...
class WPTGitHub(object):
    ...
    def _lazy_pull_requests(self):
        if self._pull_requests is False:
            self._pull_requests = self.all_pull_requests()
        return self._pull_requests

    def pr_with_change_id(self, target_change_id):
        for pull_request in self._lazy_pull_requests():
