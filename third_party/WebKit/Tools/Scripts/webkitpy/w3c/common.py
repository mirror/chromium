...
def is_path_skipped(path):
    """Checks whether to skip (not sync) a file based on its path.
...
def is_file_synced(path):
    """Checks whether a file in Chromium WPT is synced with upstream.

    Note: W3CImportExpectations specifies some other files (mostly tests) to
    skip during import, which is not checked here.
    ...
    return not (is_basename_skipped(basename) or is_path_skipped(path))
