...
class LocalWPT(object):
    ...
    def run(self, command, **kwargs):
        ...
        # TODO(robertma): Migrate to webkitpy.common.checkout.Git.
    ...
    def seek_change_id(self, change_id):
        ...
        return bool(self._most_recent_log_matching('^Change-Id: %s$' % change_id))

    def seek_commit_position(self, commit_position):
        # Note: anchors (^, $) are important so that quoted commit messages are not matched.
        return bool(self._most_recent_log_matching('^Cr-Commit-Position: %s$' % commit_position))
