...
class TestExporter(object):
    ...
    def create_or_update_pr_from_landed_commit(self, commit, pull_request=None):
        ...
        """
        if pull_request:
            self.create_or_update_pr_from_commit(commit, False, pr_number=pull_request.number)
        else:
            branch_name = 'chromium-export-' + commit.short_sha
            self.create_or_update_pr_from_commit(commit, False, branch_name=branch_name)
    ...
    def create_or_update_pr_from_inflight_cl(self, cl, pull_request=None):
        ...
        Args:
            ...
            pull_request: Optional, a PullRequest namedtuple.
                If specified, updates the PR instead of creating one.
        """
        commit = cl.fetch_commit(self.host)
        ...
        if pull_request:
            self.create_or_update_pr_from_commit(commit, True, pr_number=pull_request.number, pr_footnote=pr_footnote)
            ...
            pr_number = self.create_or_update_pr_from_commit(commit, True, pr_footnote=pr_footnote, branch_name=branch_name)
    ...
    def create_or_update_pr_from_commit(self, commit, provisional, pr_number=None, pr_footnote='', branch_name=None):
        patch = commit.format_patch()
        ...
        pr_description = body + pr_footnote
        if not branch_name:
            assert pr_number
            branch_name = self.wpt_github.get_pr_branch(pr_number)
        ...
        if updating:
            ...
            pr_number = self.wpt_github.create_pr(branch_name, subject, pr_description)
