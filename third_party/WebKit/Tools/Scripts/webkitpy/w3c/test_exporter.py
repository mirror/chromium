...
class TestExporter(object):
    ...
    def run(self):
        ...
        """
        # TODO: there are more than 100, need to fetch all
        pull_requests = self.wpt_github.all_pull_requests(limit=100)

        # TODO: tune 'exportable' heuristics (there are ~3 stragglers), then bump up to 10000
        exportable_commits = self.get_exportable_commits(limit=1000)

        # Safety valve?: if there are >100 open PRs, fail
        for exportable_commit in exportable_commits:
            ...
            if pull_request:
                ...
                # TODO: Rename to create_pull_request to match above
                # TODO: somehow force patches with conflicts
                response = self.export_commit(exportable_commit)
                # TODO: print response more nicely
