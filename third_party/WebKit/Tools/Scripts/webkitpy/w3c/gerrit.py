...
class GerritCL(object):
    ...
    def fetch_commit(self, host):
        """Fetches the git commit for the latest revision of CL.

        This method only fetches the commit corresponding to the latest revision
        of CL to local Chromium repository without checkout. All changes so far
        in the CL are squashed into this one commit, no matter how many
