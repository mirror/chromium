...
class TestExporterTest(unittest.TestCase):
    ...
    def test_creates_and_merges_pull_requests(self):
        '''Tests that the export process:

        - Can merge multiple PRs even if one is unmergeable
        - Will skip over closed PRs
        - If the export process will discover and create missing PRs
        '''
        host = MockHost()
        host.executive = mock_git_commands({
