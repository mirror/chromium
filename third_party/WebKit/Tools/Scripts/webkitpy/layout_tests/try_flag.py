...
BUILDERS = {
    ...
    'win_chromium_rel_ng': TestConfiguration('Win', '', 'release'),
...
class TryFlag(object):
    ...
    def update(self):
        stdout = self._host.stdout
        print >> stdout, 'Fetching results...'
        ...
        for build in jobs.keys():
            print >> stdout, '-- %s: %s/results.html' % (
                BUILDERS[build.builder_name].version,
                buildbot.results_url(build.builder_name, build.build_number))
            results = buildbot.fetch_results(build, 'full_results')
    ...
    def _print_all(self, lines, description):
        ...
        for line in sorted(lines, cmp=self._compare_lines):
            print >> stdout, line.to_string(self._test_configuration_converter)
