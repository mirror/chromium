...
// They exist only because we can't transfer vectors directly over IPC.
...
class BLINK_COMMON_EXPORT FeaturePolicy {
 public:
  ...
  // Returns the origins where the feature is enabled, if the feature is enabled
  // for all origins, set |matches_all| to be true.
  const std::vector<url::Origin> GetOriginsForFeature(
      FeaturePolicyFeature feature,
