...
TrialTokenValidator::TrialTokenValidator(const TrialPolicy* policy)
    : policy_(policy) {}
...
OriginTrialTokenStatus TrialTokenValidator::ValidateToken(
  ...
  if (!policy_ || !policy_->IsValidPolicy())
...
bool TrialTokenValidator::RequestEnablesFeature(
    ...
    base::StringPiece feature_name,
    base::Time current_time) const {
  if (!policy_->IsOriginTrialsFeatureEnabled())
    return false;

  if (!policy_->IsOriginSecure(request_url))
    return false;
...
TrialTokenValidator::GetValidTokensFromHeaders(
  ...
  if (!policy_->IsOriginSecure(origin.GetURL()))
    return tokens;
