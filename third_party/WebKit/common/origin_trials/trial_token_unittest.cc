...
class TrialTokenTest : public testing::TestWithParam<const char*> {
 ...
 protected:
  blink::OriginTrialTokenStatus Extract(const std::string& token_text,
  ...
  blink::OriginTrialTokenStatus ExtractIgnorePayload(
...
TEST_F(TrialTokenTest, ValidateValidSignature) {
  std::string token_payload;
  std::string token_signature;
  blink::OriginTrialTokenStatus status = Extract(
      kSampleToken, correct_public_key(), &token_payload, &token_signature);
  ASSERT_EQ(blink::OriginTrialTokenStatus::kSuccess, status);
...
TEST_F(TrialTokenTest, ValidateInvalidSignature) {
  ...
  EXPECT_EQ(blink::OriginTrialTokenStatus::kInvalidSignature, status);
}
...
TEST_F(TrialTokenTest, ValidateEmptyToken) {
  ...
  EXPECT_EQ(blink::OriginTrialTokenStatus::kMalformed, status);
}
...
TEST_F(TrialTokenTest, ValidateUnsupportedVersion) {
  ...
  EXPECT_EQ(blink::OriginTrialTokenStatus::kWrongVersion, status);
}
...
TEST_F(TrialTokenTest, ExtractValidToken) {
  ...
  EXPECT_TRUE(token);
  EXPECT_EQ(blink::OriginTrialTokenStatus::kSuccess, status);
