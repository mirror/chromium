...
def _find_matching_entries(filename):
    """Finds entries that should be used for filename.

    Returns:
      A list of entries, sorted in order of relevance.
    """
    entries = []
...
def check(filename, contents):
    ...
    """
    results = []
    entries = _find_matching_entries(filename)
    if not entries:
        return
