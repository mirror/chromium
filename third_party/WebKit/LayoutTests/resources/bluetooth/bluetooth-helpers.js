...
// Similar to getHealthThermometerService except the service has two
// 'measurement_interval' characteristics.
function getTwoMeasurementIntervalCharacteristicsService(options) {
  let device;
  let fake_peripheral;
  let fake_health_thermometer;
  let fake_measurement_interval1;
  let fake_measurement_interval2;
  return getConnectedHealthThermometerDevice(options)
    .then(result => ({
      device,
      fake_peripheral,
      fake_health_thermometer,
      fake_measurement_interval: fake_measurement_interval1,
    } = result))
    .then(() => fake_health_thermometer.addFakeCharacteristic({
      uuid: 'measurement_interval', properties: ['read', 'write', 'indicate']}))
    .then(c => fake_measurement_interval2 = c)
    .then(() => fake_peripheral.setNextGATTDiscoveryResponse({
      code: HCI_SUCCESS}))
    .then(() => device.gatt.getPrimaryService('health_thermometer'))
    .then(health_thermometer => ({
      service: health_thermometer,
      fake_service: fake_health_thermometer,
      fake_measurement_interval1: fake_measurement_interval1,
      fake_measurement_interval2: fake_measurement_interval2,
    }));
}
...
// Similart to getHealthThermometerService() except the service has no
