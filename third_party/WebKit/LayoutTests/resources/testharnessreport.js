...
(function() {
    ...
    window.isWPTManualTest = isWPTManualTest;
    ...
    window.pathAndBaseNameInWPT = pathAndBaseNameInWPT;
    ...
    function compactTestOutput(tests) {
        for (let i = 0; i < tests.length; ++i) {
            ...
            testResults.push(`${convertResult(test.status)} ${sanitize(test.name)} ${sanitize(test.message)}\n`);
        }
