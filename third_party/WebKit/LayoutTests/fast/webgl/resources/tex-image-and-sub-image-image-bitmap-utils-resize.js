...
function DrawResultsOnCanvas(testOptions) {
    ...
    resultsCanvas.width = numTiles * testOptions.resizeWidth;
    resultsCanvas.height = numTiles * testOptions.resizeWidth;
    ...
    for (var i = 0; i < resizeQualities.length; i++) {
        ...
        for (var j = pixelsBuffer.length - 1; j >= 0; j--) {
            if (pixelsBuffer[j].quality == resizeQualities[i]) {
                ...
                var x = (tileCounter * testOptions.resizeWidth) % resultsCanvas.width;
                var y = Math.floor(tileCounter / numTiles) * testOptions.resizeHeight;
                tileCounter++;
                var imageData = new ImageData(Uint8ClampedArray.from(buffer), testOptions.resizeWidth, testOptions.resizeHeight);
    ...
    if (window.testRunner) {
        console.log("testRunner.notifyDone()");
