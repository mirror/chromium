(async function(testRunner) {
  ...
  async function deleteAllCookies() {
    ...
    for (var cookie of data.cookies) {
      ...
      promises.push(dp.Network.deleteCookies({url, name:cookie.name}));
      promises.push(dp.Network.deleteCookies({name:cookie.name, domain:cookie.domain, path:cookie.path}));
    }
  ...
  testRunner.runTestSuite([
    ...
    async function setCookiesBasic() {
      await setCookies([{name: 'cookie1', value: 'session', domain: 'localhost', path: '/', },
                        ...
                        {name: 'cookie7', value: 'domain', domain: 'chromium.org', path: '/path' },
