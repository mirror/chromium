(async function(testRunner) {
  ...
  var requestInterceptedDict = {
    'ping-redirect.php': async event => {
      ...
      var headers = event.params.responseHeaders;
      if (headers) {
      ...
      if (event.params.redirectStatusCode) {
        var redirectHeaders = event.params.redirectHeaders;
        testRunner.log('  redirectUrl: ' + event.params.redirectUrl.split('/').pop());
        testRunner.log('  redirectHeaders:');
        for (var headerName of Object.keys(redirectHeaders)) {
          var headerValue = redirectHeaders[headerName];
          if (headersMaskList.has(headerName.toLowerCase()))
            headerValue = '<Masked>';
          testRunner.log(`    ${headerName}: ${headerValue}`);
        }
        testRunner.log('  redirectStatusCode: ' + event.params.redirectStatusCode);
      }
