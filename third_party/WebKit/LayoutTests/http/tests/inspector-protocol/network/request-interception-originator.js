(async function(testRunner) {
  ...
  var redirectFilenameToOriginator = new Map();
  session.protocol.Network.onRequestIntercepted(event => {
    var filename = event.params.request.url.split('/').pop();
    redirectFilenameToOriginator.set(filename, event.params.originator);
  ...
  for (var filename of Array.from(redirectFilenameToOriginator.keys()).sort()) {
    var originatorURL = frameURLById.get(redirectFilenameToOriginator.get(filename));
    testRunner.log(`${filename} requested by ${originatorURL}`);
  }
