...
(class InterceptionHelper {
  ...
  async startInterceptionTest(requestInterceptedDict, numConsoleLogsToWaitFor, interceptionStage) {
    ...
    var patterns = [];
    if (interceptionStage === 'HeadersReceived')
      patterns.push({urlPattern: "*", interceptionStage: interceptionStage});
    if (patterns === undefined || interceptionStage === 'Request' || interceptionStage === 'Both')
