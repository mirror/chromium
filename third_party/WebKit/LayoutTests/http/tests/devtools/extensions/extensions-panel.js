...
(async function() {
  ...
  //await TestRunner.navigatePromise(TestRunner.url('./resources/extensions-panel.html'));
  await TestRunner.evaluateInPagePromise(`
    function loadResources() {
      var xhr = new XMLHttpRequest();
      xhr.open("GET", "${TestRunner.url('resources/missing.txt')}", false);
      xhr.send();
      var xhr = new XMLHttpRequest();
      xhr.open("GET", "${TestRunner.url('./extensions-panel.js')}", false);
      xhr.send();
  ...
  TestRunner.logMessageAndClickOnURL = function() {
    ...
    function clickOnMessage() {
      debugger;
      var xpathResult = document.evaluate("//span[@class='devtools-link' and starts-with(., 'extensions-panel.js')]", Console.ConsoleView.instance()._viewport.element, null, XPathResult.ANY_UNORDERED_NODE_TYPE, null);
  ...
  var tests = [
    ...
    function extension_testOpenResource(nextTest) {
      ...
      evaluateOnFrontend("TestRunner.installShowResourceLocationHooks(); reply();", function() {
        webInspector.inspectedWindow.eval("loadResources();", function() {
          urls = ['${TestRunner.url('./extensions-panel.js')}', "${TestRunner.url('./resources/abe.png')}", "${TestRunner.url('./resources/missing.txt')}", "${TestRunner.url('not-found.html')}", "javascript:console.error('oh no!')"];
          showNextURL();
        });
      });
