...
(async function() {
  ...
  SourcesTestRunner.startDebuggerTestPromise(/* quiet */ true)
      ...
      .then(() => TestRunner.addSnifferPromise(Sources.CallStackSidebarPane.prototype, '_updatedForTest'))
  ...
  function dumpCallStackSidebarPane() {
    ...
    for (var element of pane.contentElement.querySelectorAll('.call-frame-item'))
      TestRunner.addResult(element.deepTextContent().replace(/VM\d+/g, 'VM').replace(/blob:http:[^:]+/, 'blob'));
  }
})();
