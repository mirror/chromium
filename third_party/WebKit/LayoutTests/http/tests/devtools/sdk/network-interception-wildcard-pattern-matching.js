...
(async function() {
  ...
  var urlPrefix = SDK.targetManager.mainTarget().inspectedURL();
  ...
  async function checkPattern(pattern) {
    await SDK.multitargetNetworkManager.setInterceptionHandlerForPatterns(interceptionHandler, [pattern]);
    ...
    function interceptionHandler(interceptedRequest) {
      ...
      TestRunner.addResult("Pattern: " + pattern);
