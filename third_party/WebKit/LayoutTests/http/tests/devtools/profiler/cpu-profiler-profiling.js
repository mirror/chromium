...
(async function() {
  TestRunner.addResult('Tests that CPU profiling works. http://crbug.com/52634\n');
  ...
  ProfilerTestRunner.runProfilerTestSuite([async function testProfiling(next) {
    ...
    function findPageFunctionInProfileView(view) {
      ...
      checkFunction(tree, 'pageFunction', 'cpu-profiler-profiling.js:16');
      checkFunction(tree, '(anonymous)', 'cpu-profiler-profiling.js:1');
      next();
    }
  }]);
