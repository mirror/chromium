...
(async function() {
  ...
  TestRunner.navigate('http://127.0.0.1:8000/devtools/network/resources/redirect-cross-origin-empty-html.php');

  TestRunner.addResult('Waiting for main resource (empty.html)');
  var mainRequest = await TestRunner.waitForEvent(SDK.NetworkManager.Events.RequestFinished, TestRunner.networkManager, request => request.name() === 'redirect-cross-origin-empty-html.php');
  var mainRedirectRequest = await TestRunner.waitForEvent(SDK.NetworkManager.Events.RequestFinished, TestRunner.networkManager, request => request.name() === 'empty.html');
  ...
  var mainRedirectContent = await mainRedirectRequest.requestContent();
  TestRunner.addResult('');
  TestRunner.addResult('Content for main redirect resource:');
  TestRunner.addResult(mainRedirectContent);
  TestRunner.addResult('');
