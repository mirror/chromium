...
# %LOCALAPPDATA%\Temp\cgiXXXXX where XXXXX are random characters
# varying per write-temp-file.cgi run.
#
...
# $file_path_acp_utf8 is a UTF-8-encoded representation of the "ACP"
...
my $file_path_acp = decode utf8 => $req->url_param('filename');
my $file_path_acp_utf8 = $file_path_acp;
if ($win32 && is_utf8($file_path_acp)) {
  ...
  $file_path_acp = encode($win32_ansi_codepage, $file_path_acp);
}
...
if (!-f $file_path_acp) {
  die(encode utf8 => "Can't reset $file_path_acp_utf8: missing file");
}
...
if (uc($file_path_implied_system_tmpdir) ne uc($system_tmpdir)) {
  die(encode utf8 => ("Can't reset $file_path_acp_utf8: " .
...
if (!-d $system_tmpdir) {
  die(encode utf8 => "Can't reset $file_path_acp_utf8: missing $system_tmpdir");
}
...
unlink($file_path_acp)
  or die(encode utf8 => "unlink $file_path_acp_utf8: $!");

# Delete its containing LayoutTests_cgiXXXXX directory.
