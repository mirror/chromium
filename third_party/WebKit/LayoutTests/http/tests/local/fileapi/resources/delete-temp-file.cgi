...
# write-temp-file.cgi, which creates file with test-chosen basenames
# inside random-named subdirectories of a system temporary directory,
# e.g. /tmp/cgiXXXX/name-chosen-by-test.txt where XXXX are random
...
if ($win32) {
  ...
  # Drive+directory path equality checks are performed on the 8.3
...
# This is the random-suffixed cgiXXXX component (wcgiXXXX on Win32).
...
# naming conventions.
if (!($temp_cgi_subdir_basename =~ /\Aw?cgi\w+\z/i)) {
  die(encode utf8 => ("Can't reset $pathname_acpu: " .
...
# Delete its containing cgiXXXX directory.
