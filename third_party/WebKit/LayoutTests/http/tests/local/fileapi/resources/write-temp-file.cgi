...
my $temp_from_env = $ENV{TEMP};
if ($win32 && ($temp_from_env eq '')) {
  if ($ENV{WINDIR} ne '') {
    $temp_from_env = $ENV{WINDIR} . '\Temp';
  }
  if ($ENV{USERPROFILE} ne '') {
    $temp_from_env = $ENV{USERPROFILE} . '\AppData\Local\Temp';
  }
}
my $tmpdir = $ENV{TMPDIR} || $temp_from_env || $ENV{TMP} || tmpdir();
$tmpdir =~ /\A([^\0- ]*)\z/s
  or die "untaint failed: $!";
