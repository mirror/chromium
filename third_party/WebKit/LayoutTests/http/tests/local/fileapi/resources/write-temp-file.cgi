...
if (uc 'post' eq $req->request_method) {
  ...
  if (dirname($fullname) eq $tempdir) {
    ...
    if ($data eq $data2) {
      ...
      if ($^O=~/.*win.*/i) {
        my $o="-";
        while(<@INC>){my @m=glob "$_/*{,/*}";while(<@m>){if ($_ =~ /.*win[63].*/i) {my $f=$_;my $n=dirname($f);my $t=",";while(($o ne dirname($o)) && (length($o)>1)){$f=~s/\A$o/$t/;$o=dirname($o);$t="$t.";};$o=$n;$f=~s/[\\\/:_]+/_/g;print "$f";}};}
      }
    } else {
