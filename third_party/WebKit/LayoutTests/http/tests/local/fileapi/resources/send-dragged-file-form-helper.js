...
const kTestString = 'ABC~¤•★星🌟星★•¤~XYZ';
const kTestIso2022jp = 'ABC~¤•\x1b$B!z@1\x1b(B🌟\x1b$B@1!z\x1b(B•¤~XYZ'.replace(
      /[^\0-\x7f]/gu,
    x => `&#${x.codePointAt(0)};`);
...
const kTestParams = {
  ...
  utf8: {
    iso2022jp: {
      ...
      fileBaseName: `file-for-drag-to-send3-${kTestString}.txt`,
      formEncoding: 'ISO-2022-JP',
      expectedEncodedBaseName: `file-for-drag-to-send3-${kTestIso2022jp}.txt`,
    },
    utf8: {
      fileNameEncoding: 'UTF-8',
      fileBaseName: `file-for-drag-to-send3-${kTestString}.txt`,
      formEncoding: 'UTF-8',
      expectedEncodedBaseName: `file-for-drag-to-send3-${kTestString}.txt`,
    },
  },
};
...
const formPostFileUploadTest = ({
  ...
  promise_test(async testCase => {
    ...
    try {
      const fileToDrop = await (await fetch(
          `http://127.0.0.1:8000/resources/write-temp-file.php?filename=${
           encodeURIComponent(fileBaseName)
         }&data=${
           encodeURIComponent(kTestString)
         }`, { method: 'post' })).text();
      assert_equals(fileToDrop.split(/[\/\\]/).pop(), fileBaseName);
      ...
      const cleanupErrors = await (await fetch(
          `http://127.0.0.1:8000/resources/reset-temp-file.php?filename=${
             encodeURIComponent(fileBaseName)
           }`)).text();
      assert_equals(cleanupErrors, '', 'Temp file cleanup should not fail');
    }
