...
const formPostFileUploadTest = ({
  ...
  promise_test(async testCase => {
    ...
    // NOTE: "ACP" in this file refers to the Windows concept of
    // "ANSI" Codepage, a (not actually ANSI-standardized) single-
    // or double-byte non-Unicode ASCII-compatible character
    // encoding used for some narrow-character Win32 APIs. It should
    // not be confused with UTF-8 (used for similar purposes on
    // Linux, OS X, and other modern POSIX-like systems), nor with
    // the usually-distinct OEM codepage used for other
    // narrow-character Win32 APIs (for instance, console I/O and
    // some filesystem data storage.)
    const [shouldBeOk, fileToDropAcp, fileToDrop] = fileToDropLines;
    assert_equals(
        shouldBeOk,
        'OK',
        `CGI response should begin with OK but got ${fileToDropLines}`);
    // Removes fakepath/ or other pseudofolder and resturns just the
    ...
    // Used for very rough matching of a path segment that may have
    // been converted by Win32 APIs to 8.3 format -- with a ~1 numeric
    // tail, so 6 meaningful characters -- when the Unicode in it was
    // not representable in ACP. Case-folding and conversion of
    // non-printable-ASCII parts to ? are to accomodate Win32 and OS X
    // filesystem rules and an unknown-to-JS effective system call
    // character set (ACP).
    const normalizedBaseNamePrefixOfFilePath =
          filePath => baseNameOfFilePath(filePath).replace(
                /[^ -~]/g,
              '?').substr(
                  0,
                  6).toLowerCase();
    assert_equals(
        normalizedBaseNamePrefixOfFilePath(fileToDropAcp),
        normalizedBaseNamePrefixOfFilePath(fileBaseName),
        `ACP ${fileToDropAcp} basename should resemble ${fileBaseName}`);
    const fileBaseNameAcp = baseNameOfFilePath(fileToDropAcp);
    fileInput.onchange = event => {
      if (fileInput.files[0].name !== fileBaseName ||
          baseNameOfFilePath(fileInput.value) !== fileBaseName) {
        assert_equals(
      ...
      // c:\fakepath\ or similar, but the basename should match the
      // dropped file name exposed through the newer .files[0].name
      // API. This check verifies that assumption.
      assert_equals(
          fileInput.files[0].name,
    ...
    try {
      ...
      const cleanupErrors = await (await fetch(
          `${kWebServer}/local/fileapi/resources/delete-temp-file.cgi` +
            `?filename=${encodeURIComponent(fileToDropAcp)}`,
          { method: 'post' })).text();
    ...
    assert_greater_than(
        formDataLines.length,
        2,
        `${fileToDropAcp}: multipart form data must have at least 3 lines: ${
    ...
    assert_equals(
        ...
        boundary + '--',
        `${fileToDropAcp}: multipart form data must end with ${boundary}--: ${
             JSON.stringify(formDataText)
           }`);
    const expectedText = [
      boundary,
