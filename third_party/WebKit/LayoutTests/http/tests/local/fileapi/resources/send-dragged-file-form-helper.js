...
const formPostFileUploadTest = ({
  ...
  promise_test(async testCase => {
    ...
    try {
      ...
      assert_equals(
          baseNameOf(fileToDrop),
          fileBaseName,
          `Unicode ${fileToDrop} basename should be ${fileBaseName}`);
      // Used for very rough matching of a path segment that may have
      // been converted by Win32 APIs to 8.3 format -- with a ~1
      // numeric tail, so 6 meaningful characters -- when the Unicode
      // in it was not representable in ACP. Case-folding and
      // conversion of non-printable-ASCII parts to ? are to
      // accomodate Win32/Mac OS X filesystem rules and an
      // unknown-to-JS effective system call character set (ACP).
      const normalizeBaseNamePrefix =
            filePath => baseNameOf(filePath).replace(/[^ -~]/g, '?').substr(
                0,
                6).toLowerCase();
      assert_equals(
          normalizeBaseNamePrefix(fileToDropAcp),
          normalizeBaseNamePrefix(fileBaseName),
          `ACP ${fileToDropAcp} basename should resemble ${fileBaseName}`);
      const fileBaseNameAcp = baseNameOf(fileToDropAcp);
      fileInput.onchange = event => {
        if (fileInput.files[0].name !== fileBaseName ||
            baseNameOf(fileInput.value) !== fileBaseName) {
          assert_equals(
