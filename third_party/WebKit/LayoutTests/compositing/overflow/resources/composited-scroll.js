function isElementUsingCompositedScrolling(element) {
...
function elementHasOpaqueCompositedScrollingContentsLayer(element) {
    var layerTree = window.internals.elementLayerTreeAsText(element);
    if (layerTree === '')
        return false;
    var layers = JSON.parse(layerTree);
...
function elementHasNotOpaqueCompositedScrollingContentsLayer(element) {
    var layerTree = window.internals.elementLayerTreeAsText(element);
    if (layerTree === '')
        return false;
    var layers = JSON.parse(layerTree);
