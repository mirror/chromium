...
function runContentSecurityPolicyTests(workletType) {
  ...
  promise_test(t => {
    ...
    // URLs in the form `http://[domain]:[https-port]`. If the upgrade fails,
    // the load will fail, as we don't serve HTTP over the secure port.
    const kInsecureOrigin =
        'http://' + get_host_info().ORIGINAL_HOST + ':' + get_host_info().HTTPS_PORT;
    const kScriptURL =
        kInsecureOrigin + '/worklets/resources/empty-worklet-script.js';
    return openWindowAndExpectResult(
        kWindowURL, kScriptURL, workletType, 'RESOLVED');
