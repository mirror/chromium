...
}, 'Blob URL can be used in iframe, even when URL is immediately revoked.');

async_test(t => {
  const run_result = 'test_script_OK';
  const blob_contents = 'window.test2_result = "' + run_result + '";';
  ...
  e.onload = t.step_func_done(() => {
    assert_equals(window.test2_result, run_result);
  });

  document.body.appendChild(e);
  URL.revokeObjectURL(url);
}, 'Blob URLs can be used in <script> tags, even when URL is immediately revoked.');
