// This worker is meant to test range requests. It forwards the first request to
// a cross-origin URL (generating an opaque response). The server is expected to
// return a 206 Partial Content response.  Then the worker lets subsequent range
// requests fall back to network (generating same-origin responses). The intent
// is to try to trick the browser into treating the resource as same-origin.
//
