...
// Returns a same-origin URL suitable for embedding in an iframe for testing
...
// Returns a cross-origin URL suitable for embedding in an iframe for testing
// the availability of the feature.
function cross_origin_url(base_url, feature_name) {
  return base_url + same_origin_pathname(feature_name);
}
...
function run_all_fp_tests_allow_all(
  ...
  // 4. Blocked in cross-origin iframe with "allow" attribute.
  async_test(
      t => {
        test_feature_availability_with_post_message_result(
            t, cross_origin_frame_url, '#' + error_name,
