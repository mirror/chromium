(class CSSHelper{
  ...
  async dumpStyleSheetText(styleSheetId) {
    var {result} =
  ...
  _expectError(message) {
    ...
    this._testRunner.log(
        'Expected protocol error: ' + message.error.message +
  ...
  async setPropertyText(styleSheetId, expectError, options) {
    ...
    if (!expectError)
      return this.dumpStyleSheetText(styleSheetId);
    this._expectError(message);
  }
  ...
  async setStyleTexts(styleSheetId, expectError, edits) {
    var message = await this._dp.CSS.addRule({edits: edits});
    if (!expectError)
      return this.dumpStyleSheetText(styleSheetId);
    this._expectError(message);
  }

  async requestMainFrameId() {
    await this._dp.Page.enable();
    var response = await this._dp.Page.getResourceTree;
  ...
  displayName(url) { return url.substr(url.lastIndexOf('/') + 1); }
  ...
  async loadAndDumpInlineAndMatchingRules(documentNodeId, selector, omitLog) {
    ...
    this._testRunner.log('}');
    this.loadAndDumpMatchingRulesForNode(nodeId, omitLog)
  }
  ...
  async requestNodeId(documentNodeId, selector) {
    var {result} = await this._dp.DOM.querySelector(
      {'nodeId': documentNodeId, 'selector': selector});
