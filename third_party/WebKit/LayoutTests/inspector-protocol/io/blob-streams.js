(async function(testRunner) {
  ...
  const all_data = responses.map(r => r.result.data).join('');
  testRunner.log('From concurrent requests: ' + all_data);
  ...
  const binary_uuid = await blobId(createBinaryBlob);
  const binary_handle = `blob:${binary_uuid}`;

  response = await session.protocol.IO.read({'handle': binary_handle, offset: 0, size: 16});
  dumpResponse('First 16 bytes of binary blob', response);
  testRunner.log('Decoded: ' + [].map.call(atob(response.result.data), _ => _.charCodeAt(0)).join(','));
