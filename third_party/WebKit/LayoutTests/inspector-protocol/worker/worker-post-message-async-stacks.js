...
//# sourceURL=test.js`);
  ...
  let {params:{asyncCallStackTrace}} = await dp.Debugger.oncePaused();
  dp.Debugger.resume();
  ...
  wc.dp.Debugger.pauseOnAsyncCall({asyncCallStackTrace});
  wc.dp.Runtime.runIfWaitingForDebugger();
  let {callFrames, asyncStackTrace, externalAsyncStackTrace} = await wc.dp.Debugger.oncePaused();
  await testRunner.logStackTrace(debuggers, {
    callFrames,
    parent: asyncStackTrace,
    parentId: externalAsyncStackTrace}, workerDebuggerId);
