...
//# sourceURL=test.js`);
  ...
  let {asyncCallStackTrace} = await wc.dp.Debugger.oncePaused();
  testRunner.log('Request pause on async task and resume');
  dp.Debugger.pauseOnAsyncCall({asyncCallStackTrace});
  wc.dp.Debugger.resume();

  let {params: {callFrames, asyncStackTrace, externalAsyncStackTrace}} =
