...
//# sourceURL=test.js`);
  ...
  let {params: {callFrames, remoteAsyncStackTrace}} =
      await dp.Debugger.oncePaused();
  logCallFrames(callFrames);

  let {stackTrace} =
      await debuggers.get(remoteAsyncStackTrace.debuggerId).getAsyncStackTrace({
  ...
  function logCallFrames(callFrames) {
    for (let frame of callFrames) {
      ...
      testRunner.log(
          functionName + ' at ' + url + ':' + location.lineNumber + ':' +
