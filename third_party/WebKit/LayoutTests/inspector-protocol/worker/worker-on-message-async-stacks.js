(async function(testRunner) {
  var {page, session, dp} = await testRunner.startBlank('Async stack trace for worker.onmessage.');
...
//# sourceURL=test.js`);
  ...
  let {scheduledAsyncStackId} = await wc.dp.Debugger.oncePaused();
  testRunner.log('Request pause on async task and resume');
  dp.Debugger.pauseOnAsyncTask({
    asyncStackId: scheduledAsyncStackId,
    sourceDebuggerId: workerDebuggerId
  });
  wc.dp.Debugger.resume();

  let {params:{callFrames, remoteAsyncStackTrace}} = await dp.Debugger.oncePaused();
  logCallFrames(callFrames);

  let {stackTrace} = await debuggers.get(remoteAsyncStackTrace.debuggerId).getAsyncStackTrace({
