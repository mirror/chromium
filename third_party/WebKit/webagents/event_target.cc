...
class WebagentsEventListener : public blink::EventListener {
 public:
  explicit WebagentsEventListener(
      std::unique_ptr<EventTarget::EventListener> callback)
 ...
 private:
  std::unique_ptr<EventTarget::EventListener> callback_;
};

void EventTarget::addEventListener(
    std::string type,
    std::unique_ptr<EventListener> callback) const {
