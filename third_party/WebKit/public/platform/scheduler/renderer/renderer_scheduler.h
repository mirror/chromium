...
class BLINK_PLATFORM_EXPORT RendererScheduler : public ChildScheduler {
 public:
  ...
  // Tells the scheduler about "keep-alive" state which can from:
  // active service worker, shared worker or unfinished fetch keep-alive.
  // If set to true, then the scheduler should not freeze the renderer.
  virtual void SetRendererKeepAlive(bool keep_alive) = 0;
