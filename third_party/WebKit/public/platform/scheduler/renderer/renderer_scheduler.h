...
class BLINK_PLATFORM_EXPORT RendererScheduler : public ChildScheduler {
 public:
  ...
  virtual void OnStartProvisionalLoad(content::RenderFrame* render_frame) = 0;

  // Tells the scheduler that a provisional load has failed, virtual time may be
  // unpaused. Must be called from the main thread.
  virtual void OnDidFailProvisionalLoad(content::RenderFrame* render_frame) = 0;
  ...
  virtual void OnDidCommitProvisionalLoad(content::RenderFrame* render_frame,
