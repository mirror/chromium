...
class BLINK_PLATFORM_EXPORT WebScopedVirtualTimePauser {
 public:
  enum class VirtualTaskDuration {
    INSTANT,     // Virtual time will not be advanced when it's unpaused.
    NON_INSTANT  // Virtual time may be advanced then it's unpaused.
  };
 ...
 private:
  ...
  bool paused_ = false;
  VirtualTaskDuration duration_ = VirtualTaskDuration::INSTANT;
