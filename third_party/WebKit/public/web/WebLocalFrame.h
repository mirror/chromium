...
class WebLocalFrame : public WebFrame {
 public:
  ...
  // Notifies the passed callback when script is not paused in the given context
  // (which is commonly immediately), or when the context is invalidated or
  // destroyed.
  virtual void NotifyWhenNotPaused(v8::Local<v8::Context>,
                                   WebScriptNotPausedCallback*) = 0;
