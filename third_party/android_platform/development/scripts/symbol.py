...
def get_objdump_result_for_unique_addr(target_addr):
  result = {}
...
def CallObjdumpForSet(lib, unique_addrs):
  ...
  if unique_addrs:
    if len(unique_addrs) > 1:
      pool = multiprocessing.Pool(processes=min(10, len(unique_addrs)))
      results = pool.map(get_objdump_result_for_unique_addr, unique_addrs)
      pool.close()
      pool.join()
      for each_result in results:
        result.update(each_result)
    else:
      result = get_objdump_result_for_unique_addr
  return result
