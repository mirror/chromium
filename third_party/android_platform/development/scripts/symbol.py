...
def ComputeObjdumpResultForUniqueAddr(target_addr, symbols):
  result = {}
...
def CallObjdumpForSet(lib, unique_addrs):
  ...
  if unique_addrs:
    ...
    #   results = pool.map(
    #       ComputeObjdumpResultForUniqueAddr, unique_addrs)
    #   pool.close()
    #   pool.join()
    #   for each_result in results:
    #     result.update(each_result)
    # else:
    #   result = ComputeObjdumpResultForUniqueAddr(unique_addrs[0])
    for unique_addr in unique_addrs:
      result.update(ComputeObjdumpResultForUniqueAddr(unique_addr, symbols))
  return result
