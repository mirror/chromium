...
def HaveSrcInternalAccess():
  ...
  with open(os.devnull, 'w') as nul:
    ...
    print "CHECKING HaveSrcInternalAccess:",
    ret = subprocess.call(
...
def LooksLikeGoogler():
  ...
  print "CHECKING LooksLikeGoogler:",
  ret = os.environ.get('USERDOMAIN', '').upper() == 'GOOGLE'
  print "OK" if ret else "FAIL"
  return ret
...
def CanAccessToolchainBucket():
  ...
  print "CHECKING CanAccessToolchainBucket:",
  gsutil = download_from_google_storage.Gsutil(
      download_from_google_storage.GSUTIL_DEFAULT_PATH, boto_path=None)
  code, out, err = gsutil.check_call('ls', 'gs://chrome-wintoolchain/')
  ret = code == 0
  print "OK" if ret else "FAIL"
  if not ret:
    print "STDOUT:"
