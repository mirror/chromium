 ...
 * FIXME: current use is storechunk(out, loadchunk(from)); should we replace
 * all those with Z_BUILTIN_MEMCPY(out, from, CHUNKCOPY_CHUNK_SIZE)?
 */
 ...
 * FIXME: maybe use the alternate impl below everywhere to avoid the :64
 * alignment issue.
 */
...
#endif // defined(__clang__) || defined(__aarch64__)
 ...
 * FIXME: (void*) might not stop a compiler using a :32 aligned load here,
 * and that'd break this code. Re-write using an unaligned load intrinsic.
 */
 ...
 * FIXME: (void*) might not stop a compiler using a :16 aligned load here,
 * and that'd break this code. Re-write using an unaligned load intrinsic.
 */
