...
#error chunkcopy.h inflate fast chunk is not defined for your build target
#endif
...
/*
 * chunk copy type: a union of int vector types, each being exactly 128-bits
 * wide, to prompt smart compilers to internally load / store this type with
 * SIMD instructions if the target CPU has support (SSE/NEON/etc).
 */
...
/*
 * chunk copy pre-condition: statically assert that the z_vec128i_t type size
 * is 128-bits wide and equals CHUNKCOPY_CHUNK_SIZE.
 */
 ...
 * FIXME: fuse storechunk(..., loadchunk(...)) into one Z_BUILTIN_MEMCPY()?
 */
static Z_ALWAYS_INLINE z_vec128i_t loadchunk(
 ...
 */
static Z_ALWAYS_INLINE void v_store_128(void* out, const z_vec128i_t vec) {
