...
void read_file_data_or_exit(const char* file, std::string* data)
{
  ...
  if (in.bad()) {
    perror((std::string("error reading ") + file).c_str());
    exit(1);
  } else if (data->empty()) {
...
void zlib_file(const char* fname, const z_wrapper type) {
  ...
  if (uncompress_rate >= 0)
    urate = BufferPrintf<128>().printf("%.1f", uncompress_rate);

  double compress_rate_max = (length / ctime[0]) * repeats / 1048576.0;
  double uncompress_rate_max = (length / utime[0]) * repeats / 1048576.0;

  std::string mrate("???");
  if (uncompress_rate_max >= 0)
    mrate = BufferPrintf<128>().printf("%.1f", uncompress_rate_max);
  ...
  printf("comp %5.1f (%.1f) MB/s uncomp %5s (%5s) MB/s\n",
    compress_rate, compress_rate_max, urate.c_str(), mrate.c_str());
}
