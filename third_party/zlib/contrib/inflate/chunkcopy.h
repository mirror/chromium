/* chunkcopy.h -- fast chunk copy and set operations in SIMD
...
#if defined(INFLATE_SIMD_NEON)
#include <arm_neon.h>
typedef uint8x16_t vec128i_t;
#elif defined(INFLATE_SIMD_SSE2)
#include <emmintrin.h>
typedef __m128i vec128i_t;
#else
#error inflate SIMD is not defined for your build target
#endif

typedef vec128i_t chunkcopy_chunk_t;
...
typedef char compiler_assert_vector_128_bits_wide[
  ((CHUNKCOPY_CHUNK_SIZE) == sizeof(vec128i_t)) ? 1 : -1];
