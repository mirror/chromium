...
// static
size_t ZipWriter::kMaxPendingEntriesCount = 50;
...
std::unique_ptr<ZipWriter> ZipWriter::CreateWithFd(
  ...
  return zip_file ? std::unique_ptr<ZipWriter>(
                        new ZipWriter(zip_file, root_dir, file_accessor))
                  : std::unique_ptr<ZipWriter>();
}
#endif
...
bool ZipWriter::AddEntry(const base::FilePath& path) {
  pending_entries_.push_back(path);
...
bool ZipWriter::Close() {
  return FlushEntriesIfNeeded(/*force=*/true) &&
...
bool ZipWriter::FlushEntriesIfNeeded(bool force) {
  ...
  while (pending_entries_.size() >= kMaxPendingEntriesCount ||
    ...
    for (auto iter = pending_entries_.begin();
      ...
      // Use absolute paths, as the file accessor requires them.
