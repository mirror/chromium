...
struct KINECTDEVICE_API Configurations {
  // If playback == true, the frame data will be loaded from playback_path
  // instead of aquiring from kinect sensor.
  bool playback = false;
  std::string playback_path = "C:\\TestDataSet\\playback2\\";

  double FPS = 30;
  unsigned int output_width = 1920;
  unsigned int output_height = 1080;

  // If record == true and is not in playback mode, the frame data will be saved
  // to record_path. If use_raw == false, the raw date will be preprocessed
  // before it is recorded to disc.
  bool record = false;
  bool use_raw = false;
  std::string record_path = "C:\\temp\\record\\";

  // If cache_data == true, in record mode, multi frames will be stored to
  // memory cache first and saved to disc when cache is full. In playback mode,
  // multi frames will be loaded to memory cache and then frames are processed
  // one by one.
  bool cache_data = false;
  int max_frames_in_cache = 200;

  // If async_process == false, a single thread does frame capture and process.
  // otherwise, a thread is for frame capture and process_threads_number threads
  // are for image process.
  ...
  std::string face_model_path =
      "C:\\opencv\\sources\\data\\haarcascades\\haarcascade_frontalface_"
      "default.xml";
};
