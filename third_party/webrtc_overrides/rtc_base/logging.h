...
// The _F version prefixes the message with the current function name.
#if defined(__GNUC__) && defined(_DEBUG)
#define RTC_LOG_F(sev) LOG(sev) << __PRETTY_FUNCTION__ << ": "
#else
#define RTC_LOG_F(sev) LOG(sev) << __FUNCTION__ << ": "
#endif
...
#define RTC_LOG_ERRNO_EX(sev, err) LOG_E(sev, ERRNO, err)
#undef RTC_LOG_ERRNO
#define RTC_LOG_ERRNO(sev) LOG_ERRNO_EX(sev, errno)

#if defined(WEBRTC_WIN)
#define RTC_LOG_GLE_EX(sev, err) LOG_E(sev, HRESULT, err)
#define RTC_LOG_GLE(sev) LOG_GLE_EX(sev, GetLastError())
#define RTC_LOG_GLEM(sev, mod) LOG_E(sev, HRESULT, GetLastError(), mod)
#define RTC_LOG_ERR_EX(sev, err) LOG_GLE_EX(sev, err)
#define RTC_LOG_ERR(sev) LOG_GLE(sev)
#define RTC_LAST_SYSTEM_ERROR (::GetLastError())
#else
#define RTC_LOG_ERR_EX(sev, err) LOG_ERRNO_EX(sev, err)
#define RTC_LOG_ERR(sev) LOG_ERRNO(sev)
#define RTC_LAST_SYSTEM_ERROR (errno)
#endif  // OS_WIN

#undef RTC_PLOG
#define RTC_PLOG(sev, err) LOG_ERR_EX(sev, err)
