...
TEST(ChromiumEnvTest, TestOpenOnRead) {
  ...
  FILE* f = fopen(tmp_file_path.value().c_str(), "w");
  ASSERT_TRUE(f != NULL);
  ...
  ChromiumEnv* penv = env.get();
  penv->SetReadOnlyFileLimitForTesting(kReadOnlyFileLimit);
  ...
  for (int i = 0; i < kNumFiles; i++) {
    ASSERT_TRUE(
        env->NewRandomAccessFile(tmp_file_path.value(), &files[i]).ok());
  }
  char scratch;
  Slice read_result;
  for (int i = 0; i < kNumFiles; i++) {
  ...
  ASSERT_TRUE(env->DeleteFile(tmp_file_path.value()).ok());
}
