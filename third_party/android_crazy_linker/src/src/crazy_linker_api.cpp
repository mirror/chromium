...
void crazy_set_pending_task_notification(void (*func)(void*), void* opaque) {
  ScopedLockedGlobals globals;
  LibraryTaskList* task_list = globals->libraries()->task_list();
  if (!task_list->IsEmpty()) {
    task_list->ApplyPendingTasks(globals->rdebug());
  }
  task_list->ResetCallback(func, opaque);
}

void crazy_apply_pending_tasks(void) {
  LOG("Entered");
  ScopedLockedGlobals globals;
  LibraryTaskList* task_list = globals->libraries()->task_list();
  task_list->ApplyPendingTasks(globals->rdebug());
...
crazy_status_t crazy_library_open(crazy_library_t** library,
                                  ...
                                  crazy_context_t* context) {
  ScopedLockedGlobals globals;
  auto* libraries = globals->libraries();
  LibraryView* wrap = libraries->LoadLibrary(
...
crazy_status_t crazy_library_open_in_zip_file(crazy_library_t** library,
  ...
  ScopedLockedGlobals globals;
  auto* libraries = globals->libraries();
  LibraryView* wrap = libraries->LoadLibraryInZipFile(
  ...
  libraries->task_list()->NotifyIfPendingTasks();
