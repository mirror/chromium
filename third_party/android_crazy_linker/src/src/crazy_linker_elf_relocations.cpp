...
bool ElfRelocations::Init(const ElfView* view, Error* error) {
  ...
  for (; dyn.HasNext(); dyn.GetNext()) {
    ...
    switch (tag) {
        ...
        RLOG("  %s addr=%p\n",
             (tag == DT_ANDROID_REL) ? "DT_ANDROID_REL"
                                     : (tag == DT_ANDROID_COMPRESSED_REL)
                                           ? "DT_ANDROID_COMPRESSED_REL"
                                           : (tag == DT_ANDROID_RELA)
                                                 ? "DT_ANDROID_COMPRESSED_RELA"
                                                 : "DT_ANDROID_COMPRESSED_RELA",
             dyn_addr);
      ...
      case DT_RELR:
        // relr_ = reinterpret_cast<ElfW(Relr)*>(load_bias_ + d->d_un.d_ptr);
        relr_ = reinterpret_cast<ElfW(Relr)*>(dyn_addr);
        break;

      case DT_RELRSZ:
        relr_count_ = dyn_value / sizeof(ElfW(Relr));
        break;

      case DT_RELRENT:
        if (dyn_value != sizeof(ElfW(Relr))) {
          RLOG("invalid DT_RELRENT: %zd", static_cast<size_t>(dyn_value));
          return false;
        }
        break;
...
bool IsValidAndroidPackedRelocations(const uint8_t* android_relocations,
  ...
  return (android_relocations[0] == 'A' && android_relocations[1] == 'P' &&
          (android_relocations[2] == 'S' || android_relocations[2] == 'Z') &&
...
bool ElfRelocations::ApplyRelrRelocs() {
  RLOG("  Applying RELR relocs\n");
  ...
  for (ElfW(Relr)* current = begin; current < end; ++current) {
    ...
    for (; bits != 0; bits >>= 1) {
      if ((bits & 1) != 0) {
        ElfW(Addr) reloc = static_cast<ElfW(Addr)>(r_offset + load_bias_);
        ElfW(Addr) addend = *reinterpret_cast<ElfW(Addr)*>(reloc);
        *reinterpret_cast<ElfW(Addr)*>(reloc) = (load_bias_ + addend);
      }
