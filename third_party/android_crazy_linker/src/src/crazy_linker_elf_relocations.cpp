    ...
    switch (tag) {
      ...
      case DT_ANDROID_REL:
        ...
        RLOG("  %s addr=%p\n",
             ...
             (tag == DT_ANDROID_COMPRESSED_REL) ? "DT_ANDROID_COMPRESSED_REL" :
             (tag == DT_ANDROID_RELA) ? "DT_ANDROID_COMPRESSED_RELA" :
...
bool ElfRelocations::ApplyAll(const ElfSymbols* symbols,
  ...
  if (!ApplyRelocs(reinterpret_cast<rel_t*>(relocations_),
                   relocations_size_ / sizeof(rel_t), symbols, resolver,
                   error))
    return false;
  if (!ApplyRelocs(reinterpret_cast<rel_t*>(plt_relocations_),
                   plt_relocations_size_ / sizeof(rel_t), symbols,
                   resolver, error))
    return false;
...
bool ElfRelocations::ForEachAndroidRelocation(RelocationHandler handler,
                                              void* opaque) {
  bool compressed = android_relocations_[2] == 'Z';
  RLOG("Android relocations are compressed: %d", compressed);
  ...
  if (compressed) {
    auto decoder =
        compressed_sleb128_decoder(packed_relocs, packed_relocs_size);
    if (!decoder.initialize()) {
      return false;
    }
    return ForEachAndroidRelocationHelper(
        packed_reloc_iterator<compressed_sleb128_decoder>(std::move(decoder)),
        handler, opaque);
  }
  return ForEachAndroidRelocationHelper(
