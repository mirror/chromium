...
void* WrapDlopen(const char* path, int mode) {
  ...
  if (path) {
    ...
    LibraryView* wrap = lib_list->LoadLibrary(path,
                                              ...
                                              &error);
    if (wrap) {
      Globals::Get()->valid_dlopen_handles.Add(wrap);
      return wrap;
    }
  }
  ...
  Globals::Get()->valid_dlopen_handles.Add(wrap_lib);
  return wrap_lib;
}
...
void* WrapDlsym(void* lib_handle, const char* symbol_name) {
  ...
  ScopedGlobalLock lock;
  if (!Globals::Get()->valid_dlopen_handles.Has(lib_handle)) {
...
int WrapDlclose(void* lib_handle) {
  ...
  ScopedGlobalLock lock;
  if (!Globals::Get()->valid_dlopen_handles.Remove(lib_handle)) {
...
void** GetValidDlopenHandlesForTesting(size_t* p_count) {
  ScopedGlobalLock lock;
  const PointerSet& valid_handles = Globals::Get()->valid_dlopen_handles;
  size_t count = valid_handles.GetCount();
  *p_count = count;
  if (!count)
    return nullptr;

  auto* ptr = reinterpret_cast<void**>(malloc(count * sizeof(void*)));
  for (size_t n = 0; n < count; ++n) {
    ptr[n] = valid_handles[n];
  }
  return ptr;
}
