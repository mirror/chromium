...
class LocalDeviceInstrumentationTestRun(
  ...
  def _RunTest(self, device, test):
        ...
        with logcat_monitor.LogcatMonitor(
          ...
          with _LogTestEndpoints(device, test_name):
            with contextlib_ext.Optional(
              ...
              print 'Instrumentation Target: ' + target
    ...
    def pull_ui_screen_captures():
      ...
      if screenshots:
        json_archive_name = 'ui_capture_%s_%s.json' % (
            ...
            time.strftime('%Y%m%dT%H%M%S-UTC', time.gmtime()))
        with self._env.output_manager.ArchivedTempfile(
            json_archive_name, 'ui_capture', output_manager.Datatype.TEXT
        ) as json_archive:
          json.dump(screenshots, json_archive)
        print 'Json link: ' + json_archive.Link()
    ...
    def pull_ui_screenshot(filename):
      ...
      print 'Image link: ' + image_archive.Link()
      device.RemovePath(image_file_path)
