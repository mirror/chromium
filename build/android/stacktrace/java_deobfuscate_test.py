...
def _RunTest(bin_path, map_file, logcat):
  cmd = [bin_path, map_file]
  payload = TEST_DATA
  expected_output = EXPECTED_OUTPUT
  if logcat:
    cmd.append('--logcat')
    payload = ['1:2:3 LogTag: ' + x for x in payload]
    expected_output = ['1:2:3 LogTag: ' + x for x in expected_output]
...
def main():
  ...
  with tempfile.NamedTemporaryFile() as map_file:
    ...
    result1 = _RunTest(args.path_to_java_deobfuscate, map_file.name, False)
    result2 = _RunTest(args.path_to_java_deobfuscate, map_file.name, True)
