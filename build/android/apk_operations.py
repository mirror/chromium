...
def _GenerateSelectingDeviceFlagMessage(devices, devices_obj):
  descriptions = devices_obj.pMap(lambda d: d.build_description).pGet(None)
  msg = ('More than one device available. Use --device to select one device '
         'by serial.\n\nAvailable devices:\n')
  for d, desc in zip(devices, descriptions):
    msg += '  %s (%s)\n' % (d, desc)
  return msg
...
def main():
  ...
  if command in {'gdb', 'logcat'}:
    ...
    use_cache = False
    # These cmds can only take one device.
    if len(devices) > 1:
  ...
  if command == 'install':
    ...
    pkg_name = apk_helper.GetPackageName(active_apk)
    flags = [gdb_script_path, '--program-name=', program_name,
