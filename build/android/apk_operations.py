...
def _UninstallApk(install_incremental, devices_obj, apk_to_uninstall):
  apk_package = apk_helper.GetPackageName(apk_to_uninstall)
  if install_incremental:
...
def _LaunchUrl(devices_obj, input_args, device_args_file, url, apk_to_launch):
...
def _StopApk(devices_obj, apk_to_stop):
  apk_pkg = apk_helper.GetPackageName(apk_to_stop)
  devices_obj.ForceStop(apk_pkg)


def _ClearData(devices_obj, apk_to_clear):
  apk_pkg = apk_helper.GetPackageName(apk_to_clear)
  devices_obj.ClearApplicationState(apk_pkg)
...
def main():
  ...
  subp = command_parsers.add_parser('run',
                                    help='Install and launch.')
  _AddCommonOptions(subp)
  _AddLaunchOptions(subp)

  subp = command_parsers.add_parser('stop',
                                    help='Stop apks on all devices')
  _AddCommonOptions(subp)
  ...
  if not args.incremental and not args.non_incremental and command not in {
      'argv', 'stop', 'clear-data'}:
  ...
  if command == 'install':
    ...
    _UninstallApk(install_incremental, devices_obj, active_apk)
  elif command == 'launch':
    ...
    _StopApk(devices_obj, active_apk)
  elif command == 'ckear-data':
    _ClearData(devices_obj, active_apk)
  elif command == 'argv':
    _ChangeFlags(devices, devices_obj, args.args,
                gn_args.command_line_flags_file)
  elif command == 'gdb':
    gdb_script_path = 'adb_gdb'
    flags = []
    if args.args:
