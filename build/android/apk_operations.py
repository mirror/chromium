...
class _LogcatProcessor(object):
  ...
  def __init__(self, device, package_name, deobfuscate=None, verbose=False):
    ...
    self._update_my_pids()

  def _update_my_pids(self):
  ...
  def _get_pid_style(self, pid, dim=False):
  ...
  def _parse_line(self, line):
    ...
    if self._deobfuscate:
      messages = self._deobfuscate.TransformLines(messages)
    return self.ParsedLine(
        date, invokation_time, pid, tid, priority, tag, messages)

  def _print_log(self, log, dim=False):
    def get_priority_style(priority, dim=False):
      if dim:
        return ''
      style = ''
    ...
    pid_style = self._get_pid_style(log.pid, dim)
    # We have to pad before adding color as that changes the width of the tag.
    tag = _Colorize('{:8}'.format(log.tag),
                    pid_style + ('' if dim else colorama.Style.BRIGHT))
    priority = _Colorize(log.priority,
                         get_priority_style(log.priority, dim))
    for message in log.messages:
      message = _Colorize(message, pid_style)
      sys.stdout.write('{} {} {:5} {:5} {} {}: {}\n'.format(
          log.date, log.invokation_time, log.pid, log.tid, priority, tag,
          message))

  def process_line(self, line, fast=False):
    dim = False
...
def _RunLogcat(device, package_name, mapping_path, verbose):
  ...
  try:
    ...
    for line in device.adb.Logcat(logcat_format='threadtime'):
      try:
        logcat_processor.process_line(line, fast)
      except:
