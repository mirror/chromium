...
def _RunGdb(apk_name, apk_package, device, args, verbose):
  ...
  output_dir = '--output-directory=%s' % constants.GetOutDirectory()
  adb_path = '--adb=%s' % adb_wrapper.AdbWrapper.GetAdbPath()
  device_str = '--device=%s' % device.serial
  # Use one lib dir per device so that changing between devices does require
  # refetching the device libs.
  libs_dir = '--pull-libs-dir=/tmp/adb-gdb-libs-%s' % device.serial
  flags = [gdb_script_path, program_name, package_name, output_dir, device_str,
           adb_path, libs_dir]
  if args:
  ...
  print '\033[93mAll subsequent output is from adb_gdb script.\033[39m'
...
def _PrintPerDeviceOutput(devices, results, single_line=False):
  for d, result in zip(devices, results):
    if not single_line and d is not devices[0]:
      sys.stdout.write('\n')
    sys.stdout.write('\033[93m%s (%s):\033[39m' % (d, d.build_description))
...
def _DuHelper(device, path_spec, run_as=None):
  ...
  # 10392   /data/data/org.chromium.chrome/code_cache
  # 16      /data/data/org.chromium.chrome/shared_prefs
  # 9292    /data/data/org.chromium.chrome/app_chrome
  # 208     /data/data/org.chromium.chrome/app_textures
  # 4       /data/data/org.chromium.chrome/app_download_internal
  # 12      /data/data/org.chromium.chrome/app_tabs
  # 24      /data/data/org.chromium.chrome/app_dex
  # 20104   /data/data/org.chromium.chrome
    ...
    for apk_path in apk_sizes:
      ...
      for ext in ('dex', 'odex', 'vdex', 'art'):
        ...
        for arch in ('arm', 'arm64', 'x86', 'x86_64', 'mips', 'mips64'):
          # Arbitrarily look for up to 4 dex files.
          for suffix in ('', '1', '2', '3'):
...
def Run(output_directory, apk_path, incremental_install_json_path,
  ...
  if command == 'install':
    ...
    _RunGdb(apk_name, apk_package, devices[0], args.args, args.verbose_count)
  elif command == 'logcat':
    adb_path = adb_wrapper.AdbWrapper.GetAdbPath()
    ...
    cmd = ['cmd', 'package', 'compile', '-f', '-m', args.compilation_filter,
           apk_package]
