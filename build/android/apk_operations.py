...
class _Command(object):
  ...
  def ProcessArgs(self, args, output_directory=None, target_cpu=None):
...
class _GdbCommand(_Command):
  ...
  def Run(self):
    ...
    _RunGdb(self.devices[0], self.args.package_name, self.output_directory,
            self.target_cpu, extra_args, bool(self.args.verbose_count))
...
def Run(output_directory, apk_path, incremental_install_json_path,
  ...
  args.command.ProcessArgs(
      args, output_directory=output_directory, target_cpu=target_cpu)
  args.command.Run()
