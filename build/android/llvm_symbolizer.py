...
CHROME_SRC = os.path.join(os.path.dirname(__file__), os.pardir, os.pardir)
...
class LLVMSymbolizer(object):
  def __init__(self):
    ...
    self._lock = None
    self._closed_called = None

  def Start(self):
    if os.path.isfile(LLVM_SYMBOLIZER_PATH):
      print 'path exists'
      self._llvm_symbolizer_subprocess = subprocess.Popen(
        [LLVM_SYMBOLIZER_PATH], stdout=subprocess.PIPE, stdin=subprocess.PIPE)
    else:
    ...
    with self._lock:
      ...
      while True:
        ...
        if line != '\n':
          result.append(
            (line[:-1],
             self._llvm_symbolizer_subprocess.stdout.readline()[:-1]))
        else:
          break
      return result
