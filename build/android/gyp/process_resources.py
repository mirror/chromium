...
def _GetQualifiedEntryName(entry):
  ...
  # resource id (since they are taken from another compilation unit, the
...
def _CreateRJavaFile(package, resources_by_type, shared_resources,
  ...
  if shared_resources or non_constant_id:
    for _type, resources in resources_by_type.iteritems():
      for entry in resources:
        if (_GetQualifiedEntryName(entry) in
            whitelisted_resources_by_type[_type]):
          non_final_resources_by_type[_type].append(entry)
        else:
          final_resources_by_type[_type].append(entry)
  else:
