...
def _EnableDebugInManifest(manifest_path, temp_dir):
  ...
  with open(debug_manifest_path, 'w') as debug_manifest, \
      open(manifest_path) as original_manifest:
    a = original_manifest.read()
    doc = ElementTree.fromstring(a)
    app_node = doc.find('application')
    app_node.set('{%s}%s' % (_ANDROID_NAMESPACE, 'debuggable'), 'true')
    debug_manifest.write(ElementTree.tostring(doc, encoding='UTF-8'))
...
def _CreateRTxtAndSrcJar(options, r_txt_path, srcjar_dir):
  ...
  if packages:
    _CreateRJavaFiles(srcjar_dir, r_txt_path, packages, r_txt_files,
                     options.shared_resources, options.non_constant_id)
