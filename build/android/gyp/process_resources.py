...
def _ParseTextSymbolsFile(path):
  ...
  with open(path) as f:
    for line in f:
      ...
      ret.append(TextSymbolsEntry(java_type, resource_type, name, _FixPackageId(value)))
  return ret

def _FixPackageId(resource_value):
  # Always make sure that the pkg id is 0x7f
  return re.sub(r'0x(?!01)\d{2}(?P<tail>\d+)', r'0x7f\g<tail>', resource_value)
