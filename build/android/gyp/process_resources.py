...
def _ParseArgs(args):
  ...
  required_options = (
      'android_sdk_jar',
      'aapt_path',
      'android_manifest', 'dependencies_res_zips',
      )
...
def _GenPackageApkArgs(options):
  package_command = [
...
def _PackageApk(options, package_command, dep_subdirs):
    ...
    package_command += _GenPackageApkArgs(options)
    ...
    for directory in dep_subdirs:
      MoveImagesToNonMdpiFolders(directory)
...
def _PackageLibrary(options, package_command, temp_dir):
    ...
    # This is used only in none apk builds
    ...
    for idx, input_dir in enumerate(input_resource_dirs):
      ...
      CrunchDirectory(options.aapt_path, input_dir, crunch_dir)
...
def _GenRTxt(options, r_txt_path, srcjar_dir):
...
def _OnStaleMd5(options):
  with build_utils.TempDir() as temp_dir:
    ...
    _GenRTxt(options, r_txt_path, srcjar_dir)
...
def main(args):
  ...
  input_strings.extend(_GenPackageApkArgs(options))
