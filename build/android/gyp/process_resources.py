...
def _EnableDebugInManifest(manifest_path, temp_dir):
  ...
  # if this is a dummy manifest, then there is no need to enable debugging (no
  # application node even exists). Dummy manifests are sometimes used when only
  # the R.java/txt output is required. eg:
  # gen/base/android/jni_generator/sample_jni_apk_manifest/AndroidManifest.xml
  package = original_manifest.getroot().get('package')
  if package == 'org.dummy':
