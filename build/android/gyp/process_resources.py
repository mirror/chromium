...
def _GenerateGlobs(pattern):
  ...
  # ignored (we dont output anyways, so it is not required). It then loops over
  # the list and removes empty globs. This function does not handle the <dir>
  # and <file> prefixes used by aapt and are assumed not to be included in the
  # pattern string.
  return [g for g in re.split(r':+', pattern.replace('!', '')) if g != '']
