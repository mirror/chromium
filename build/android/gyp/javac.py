  ...
  with build_utils.TempDir() as temp_dir:
    ...
    if java_files:
      ...
      if classpath:
        ...
        with open(classpath_rsp_path, 'w') as classpath_rsp_file:
          classpath_rsp_file.write(':'.join(classpath))
        cmd += ['-classpath', '@' + classpath_rsp_path]

      rsp_path = os.path.join(temp_dir, 'files_list.txt')
      with open(rsp_path, 'w') as rsp_file:
        rsp_file.write(' '.join(java_files))
      cmd += ['@' + rsp_path]
