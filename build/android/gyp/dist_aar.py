...
def _AddResources(aar_zip, resource_zips):
  ...
  """
  digit_len = len(str(len(resource_zips)))
  resource_count_format = '{0:' + str(digit_len) + '}'
    ...
    with zipfile.ZipFile(path) as res_zip:
      for info in res_zip.infolist():
        ...
        if 'values' in dirname:
          # Prefix such that sorting alphabetically will result the files
          # being in the same order.
          basename = '{}-{}'.format(resource_count_format.format(i), basename)
