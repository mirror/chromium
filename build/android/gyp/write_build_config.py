...
def main(argv):
  ...
  if is_java_target:
    ...
    javac_full_interface_classpath.extend(
        p for p in extra_jars if p not in javac_full_interface_classpath)
  ...
  if options.type == 'android_apk' and options.tested_apk_config:
    ...
    if tested_apk_config['unprocessed_jar_path'] not in javac_full_classpath:
    ...
    if (tested_apk_config['interface_jar_path']
        not in javac_full_interface_classpath):
