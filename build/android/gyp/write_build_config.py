...
def main(argv):
  ...
  processor_config_paths = build_utils.ParseGnList(
      options.annotation_processor_configs or '')
  processor_config_paths = _FilterDepsPaths(
      processor_config_paths, options.type)
  classpath_config_paths = build_utils.ParseGnList(
      options.classpath_deps_configs or '')
  classpath_config_paths = _FilterDepsPaths(
      classpath_config_paths, options.type)
  ...
  if is_java_target:
    ...
    classpath_deps = Deps(classpath_config_paths)
  ...
  if is_java_target:
    processor_deps = Deps(processor_config_paths)
