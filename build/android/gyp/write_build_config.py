...
def main(argv):
  ...
  if options.type in ('java_binary', 'java_library', 'android_apk', 'dist_jar'):
    ...
    if 'extra_classpath_jars' in deps_info:
      ...
      java_full_interface_classpath = [
          _AsInterfaceJar(p) for p in java_full_classpath if p not in deps_info['extra_classpath_jars']]
    else:
      config['javac']['interface_classpath'] = [
          _AsInterfaceJar(p) for p in javac_classpath]
      java_full_interface_classpath = [
          _AsInterfaceJar(p) for p in java_full_classpath]
    deps_info['java'] = {
