...
def _ConvertToWebP(webp_binary, files):
  pool = multiprocessing.pool.ThreadPool(10)
  def convert_image(png_path):
    root, _ = os.path.splitext(png_path)
    webp_path = root + '.webp'
    args = [webp_binary, png_path] + _CONVERT_TO_WEBP_ARGS + [webp_path]
    ...
    if options.resource_zips:
      ...
      for z in dep_zips:
        ...
        if extracted_files:
          ...
          png_paths.extend(f for f in extracted_files
                           if f.endswith('.png') and not f.endswith('.9.png'))
      if png_paths and options.convert_to_webp:
