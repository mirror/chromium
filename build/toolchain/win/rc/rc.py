...
def Preprocess(rc_file_data, flags):
  ...
  if sys.platform == 'win32':
    clang += '.exe'
  pp_flags = ['/P', '/DRC_INVOKED', '/TC']
  if flags.show_includes:
    pp_flags.append('/showIncludes')
  temp_handle, temp_file = tempfile.mkstemp(suffix='.i')
  p = subprocess.Popen(
      [clang] + pp_flags + flags.includes + flags.defines +
      [flags.input, '/Fi' + temp_file])
  p.wait()
  if p.returncode == 0:
  ...
  # Remove line markers.
  # TODO(thakis): Use /EP instead once PR34997 is fixed.
  preprocessed_output = re.sub('^# \d+.*$', '', preprocessed_output, flags=re.M)

  # FIXME why is this needed?
  preprocessed_output = re.sub('^#pragma warning.*$', '', preprocessed_output,
                               flags=re.M)
  # FIXME: comment
  preprocessed_output = re.sub('^#pragma code_page.*$', '', preprocessed_output,
                               flags=re.M)
