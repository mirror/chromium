...
# You must run this on a Mac.
# You must have synced with `target_os=['win']` in your .gclient once to get a
# Windows SDK.
# You must've run `build/linux/sysroot_scripts/install-sysroot.py --arch amd64`
...
pushd "$WORK_DIR"
git clone https://github.com/nico/hack
cd hack/res
./distrib.py
popd
