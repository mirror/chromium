...
def ZapTimestamp(filename):
  ...
  if filename.endswith('.tlb'):
    ...
    # a fairly complete description of the .tlb binary format.  Section 12 in
    # the file contains custom() data. custom() data has a type (int, string,
    # etc).  Each custom data chunk starts with a uint16_t describing its type.
    # Type 8 is string data, consisting of a uint32_t len, followed by that
    # many data bytes, followed by 'W' bytes to pad to a 4 byte boundary.
    # Type 0x13 is uint32 data, followed by 4 data bytes, followed by two 'W'
    # to pad to a 4 byte boundary.
    ...
    # 0x0801026e for v8.1.622 -- 0x26e == 622).  These 3 fields take 0x58 bytes.
    ...
    ntypes, = struct.read_from('<I', contents, 0x20)
    custom_off, custom_len = struct.read_from(
        '<II', contents, 0x54 * 4*ntypes + 11*16)
    assert custom_len >= 0x58
    ...
    assert contents[custom_off+6+0x3e+8:custom_off+6+0x3e+2] == '\x13\x00'
