...
def main():
  ...
  source = os.path.join(THIS_DIR, "..", "..",
      "third_party", "win_build_output", header_dir.replace('gen/', 'mc/'))
  distutils.dir_util.copy_tree(source, header_dir)
  ...
  try:
    ...
    if header_dir:
      rest[rest.find('-h') + 1] = tmp_dir
      header_dir = tmp_dir
    if resource_dir:
      rest[rest.find('-r') + 1] = tmp_dir
