...
def main():
  ...
  for chunk in _split_in_chunks(pgc_files, options.files_per_iter):
    ...
    # pgomgr.exe sometime fails to merge too many files at the same time (it
    # usually complain that a stream is missing, but if you try to merge this
    ...
    if ret != 0:
      ...
      for pgc_file in chunk:
        ret = merge_pgc_files(pgomgr_path, [os.path.join(options.build_dir,
                              os.path.basename(pgc_file))], pgd_file)
        if ret != 0:
