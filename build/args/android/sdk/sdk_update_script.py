...
def _DownloadSdk(arguments):
  ...
  for pkg in arguments.packages:
    ...
    # The line 'Accept? (y/N):' cannot be read as a line since it doesn't
    # encounter '\n' and causes output hanging. Hack to output that line.
    line = str()
    while True:
...
def _UploadSdkPackage(arguments):
  ...
  Args:
    ...
    verbose
  """
  if arguments.package and arguments.yaml_file:
    raise IOError("--package and --yaml-file cannot be used together")
  ...
  upload_sdk_cmd = [
    'cipd', 'create',
    '-pkg-def', yaml_file,
    '-tag', 'version:' + arguments.version,
    '-service-url', arguments.service_url
  ]
  print upload_sdk_cmd
  if arguments.verbose:
