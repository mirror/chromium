...
def _CopyUCRTRuntime(target_dir, source_dir, target_cpu, dll_pattern, suffix):
  ...
  # Copy the UCRT files needed by VS 2015 and above from the Windows SDK. This
  # location includes the api-ms-win-crt-*.dll files that are not found in the
  # Windows directory. These files are needed for component builds.
