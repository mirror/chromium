...
def BuildBootfs(output_directory, runtime_deps, bin_name, child_args, dry_run):
  ...
  logger_file.write('launch -r /system/bin/listen %s /boot/bin/sh -c '
                    '\'/system/bin/cat /tmp/summary_output.json /system/flush; '
                    'msleep 10000 ; '
                    'dm poweroff\'\n' % LOG_PORT)
  logger_file.flush()
  ...
  flush_file = open(bin_name + '.bootfs_flush', 'w')
  flush_file.write('\n' * 32768)
  flush_file.flush()
  ...
  # Add the autorun file, logger files, and target binary to |file_mapping|.
  file_mapping['logger'] = logger_file.name
  file_mapping['flush'] = flush_file.name
...
def RunFuchsia(bootfs_data, use_device, dry_run, test_launcher_summary_output):
  ...
  while True:
    ...
    if bootfs_data.termination_string in line:
      sys.stdout.write('Retrieving results and starting shutdown...')
      sys.stdout.flush()
