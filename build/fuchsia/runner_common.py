...
def StripBinary(dry_run, bin_path):
  strip_path = bin_path + '_stripped';
  shutil.copyfile(bin_path, strip_path)
  RunAndCheck(dry_run, ['/usr/bin/strip', strip_path])
  return strip_path
...
def BuildBootfs(output_directory, runtime_deps, bin_name, child_args,
  ...
  stripped_bin_path = StripBinary(dry_run, bin_path)
...
def RunFuchsia(bootfs, exe_name, use_device, dry_run):
  ...
  if use_device:
    ...
    return RunAndCheck(dry_run, bootserver_command)
