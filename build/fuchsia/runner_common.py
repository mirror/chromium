...
# This is an arbitrary port on localhost that is forwarded to the same port on
# the target. The target is listening on that port with a simple `listen cat` of
# the json output, and on completion of the cat, powers off the machine. This is
# baroque, but ssh is ridiculously slow and unstable over qemu user networking
# (often crashing and causing bot timeouts), and other solutions (like a tun/tap
# device on the host) require sudo or image reconfiguration.
LOG_PORT = '20710'
...
def BuildBootfs(output_directory, runtime_deps, bin_name, child_args, dry_run):
  ...
  logger_file.write('launch -r /system/bin/listen %s /boot/bin/sh -c '
                    '\'/system/bin/cat /tmp/summary_output.json.lz4 ; '
                    'msleep 10000 ; '
                    'dm poweroff\'\n' % LOG_PORT)
  logger_file.flush()
  ...
  autorun_file.write('\n')
  autorun_file.write('/boot/bin/lz4 -9 '
                     '/tmp/summary_output.json /tmp/summary_output.json.lz4\n')
...
def RunFuchsia(bootfs_data, use_device, dry_run, test_launcher_summary_output):
  ...
  qemu_command = [qemu_path,
      ...
      '-netdev', 'user,id=net1,hostfwd=tcp::%s-:%s' % (LOG_PORT, LOG_PORT),
  ...
  while True:
    ...
    if bootfs_data.termination_string in line:
      ...
      p = subprocess.Popen(['nc', 'localhost', LOG_PORT],
                           stdout=subprocess.PIPE)
      compressed = p.communicate()[0]
      lz4 = subprocess.Popen(['lz4', '-df', '-', test_launcher_summary_output],
                             stdin=subprocess.PIPE)
      result = lz4.communicate(compressed)
      if lz4.returncode != 0:
        print 'lz4 decompress failed'
