...
def _RunAndCheck(dry_run, args, retries=0):
  if dry_run:
    print 'Run:', ' '.join(args)
    return 0
  else:
    if retries:
      # Try N-1 retries with subprocess.call, if we still haven't succeeded
      # do the last one with the normal execution path.
...
def BuildBootfs(output_directory, runtime_deps, bin_name, child_args, dry_run):
  ...
  return BootfsResults(bootfs_name, symbols_mapping, termination_string,
                       local_config_file)
...
def RunFuchsia(bootfs_data, use_device, dry_run, test_launcher_summary_output):
  ...
  qemu_command = [qemu_path,
      ...
      # testserver running on the host, and for SSHing to retrieve the test
      # results json.
      '-netdev',
  ...
  while True:
    ...
    if bootfs_data.termination_string in line:
      ...
      # file back if desired and start shutdown. Retries are necessary here, as
      # sshd might not be initialized yet (it seems it may initialize after
      # autorun completes, at least under load.)
      if test_launcher_summary_output:
        _RunAndCheck(dry_run,
            ['scp', '-F', bootfs_data.ssh_config_file, '-P', HOST_SSH_PORT,
            'fuchsia@localhost:/tmp/summary_output.json',
            test_launcher_summary_output], retries=5)
      _RunAndCheck(dry_run,
          ['ssh', '-F', bootfs_data.ssh_config_file, '-p', HOST_SSH_PORT,
          'fuchsia@localhost', 'dm shutdown'], retries=5)
