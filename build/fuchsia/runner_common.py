...
def _GetSummaryOutput(bootfs_data, target_file):
  _RunAndCheck(False,
      ['scp', '-F', bootfs_data.ssh_config_file, '-P', HOST_SSH_PORT,
       'fuchsia@localhost:/tmp/summary_output.json', target_file])
  ...
  while True:
    ...
    if bootfs_data.termination_string in line:
      ...
      if test_launcher_summary_output:
        _GetSummaryOutput(bootfs_data, test_launcher_summary_output)
