...
def BuildBootfs(output_directory, runtime_deps_path, test_name, child_args,
  ...
  if test_launcher_summary_output:
    test_launcher_summary_output = os.path.normpath(
        os.path.join(output_directory, test_launcher_summary_output))
    summary_file_on_device = MakeTargetImageName(
        common_prefix, output_directory, test_launcher_summary_output)
    child_args.append('--test-launcher-summary-output=/system/' +
                      summary_file_on_device)
  ...
  try:
    with open(os.devnull, 'w') as null_output:
      subprocess.check_call(
          [os.path.join(SDK_ROOT, 'tools', 'netaddr'),
           '--fuchsia'],
          stdout=null_output)
      netcp_cmd = [
    ...
    print 'Failed to pull %s from fuchsia' % summary_json
...
def main():
  ...
  bootfs = BuildBootfs(args.output_directory, args.runtime_deps_path,
                       ...
                       args.test_launcher_summary_output, args.dry_run)
  ...
  if args.dry_run:
    ...
    summary_json_re = re.compile(
        r'Saved summary as JSON to ([^ ]+)')
    summary_json = None
    ...
    while True:
      ...
      if 'SUCCESS: all tests passed.' in line:
        success = True
      l = line.strip()
      m = summary_json_re.search(l)
      if m:
        NetCp(':%s' % m.group(1), args.test_launcher_summary_json)
      if bt_end_re.match(l):
        if bt_entries:
