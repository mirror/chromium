...
def BuildBootfs(output_directory, runtime_deps_path, test_name, child_args,
  ...
  # with sh.
  test_runner_config_file = tempfile.NamedTemporaryFile()
  test_runner_config_file.write(
      '{"apps": [[ "file:///boot/bin/sh", "/system/autorun" ]]}')
  test_runner_config_file.flush()
  DumpFile(dry_run, test_runner_config_file.name,
           'data/bootstrap/test.config')
  target_source_pairs.append(('data/bootstrap/test.config',
                              test_runner_config_file.name))
