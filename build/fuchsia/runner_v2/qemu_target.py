...
class QemuTarget(target.Target):
  ...
  def Start(self):
    ssh_config_path, boot_image_path = boot_image.CreateBootFS(
        self._output_dir, self._GetTargetArch())
    self._SetSshConfigPath(ssh_config_path)
    qemu_path = os.path.join(
        common.SDK_ROOT, 'qemu', 'bin', 'qemu-system-' + self._GetTargetArch())
    kernel_args = ['devmgr.epoch=%d' % time.time()]

    qemu_command = [qemu_path,
        '-m', '2048',
        '-nographic',
        '-kernel', boot_image._GetKernelPath(self._GetTargetArch()),
