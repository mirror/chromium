...
from common_args import AddCommonArgs, EvaluateCommonArgs
from deploy_util import RunPackage
from target_factory import GetDeploymentTargetForArgs

# TODO:
# test server
# filter file
# copy summary output out
#
...
def main():
  ...
  fuchsia = GetDeploymentTargetForArgs(args)
  with GetDeploymentTargetForArgs(args) as fuchsia:
    try:
      fuchsia.Start()

      if args.test_launcher_filter_file:
        fuchsia.CopyTo(args.test_launcher_filter_file, TEST_FILTER_PATH)
        child_args.append('--test-launcher-filter-file=' + TEST_FILTER_PATH)

      RunPackage(args.output_directory, args.package_manifest,
                 fuchsia, args.package, child_args)

      if args.test_launcher_summary_output:
        fuchsia.CopyFrom(TEST_RESULT_PATH, args.test_launcher_summary_output)
