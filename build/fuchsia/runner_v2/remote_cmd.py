...
def RunPipedSsh(config_path, host, port, command, stdout, stderr, stdin):
  ...
  ssh_command = _SSH + ['-F', config_path,
                        ...
                        '-p', str(port)] + command
  logging.debug(' '.join(ssh_command))
  return subprocess.Popen(ssh_command, stdout=stdout, stderr=stderr,
                          stdin=stdin)
