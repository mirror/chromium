...
class DeviceTarget(target.Target):
  ...
  def _Discover(self, node_name):
    ...
    proc = subprocess.Popen([netaddr_path, '--fuchsia', '--nowait', node_name],
  ...
  def Start(self):
    if self._auto:
      ...
      node_name = boot_image.GetNodeName(self._output_dir)
      self._host = self._Discover(node_name)
      ...
      bootserver_command = [bootserver_path, '-1',
                            ...
                            boot_image_path, '--'] + \
                            boot_image.GetKernelArgs(self._output_dir)
