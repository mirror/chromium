...
# Declares the list of capabilities needed by the process.
# Bundled as a JSON file at the location /meta/sandbox .
SANDBOX_POLICY = { 'features': [ 'persistent-storage',
                                 'shell',
                                 'system-temp' ] }


def MakePackagePath(file_path, roots):
  """Computes a path for |file_path| that is relative to one of the root paths
  in |roots|.
...
def BuildManifest(root_dir, out_dir, app_name, runtime_deps_file, output_path):
  with tempfile.NamedTemporaryFile(delete=False) as sandbox_policy_file:
    sandbox_policy_file.write(json.dumps(SANDBOX_POLICY))
    sandbox_policy_file.close()
    ...
    with open(output_path, "w") as output:
      ...
      output.write('meta/sandbox=%s' % sandbox_policy_file.name)
