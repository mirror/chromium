...
class ResourceSchedulerTest : public HeadlessAsyncDevTooledBrowserTest,
 ...
 public:
  void RunDevTooledTest() override {
    browser()->SetEnableResourceScheduler(GetEnableResourceScheduler());
  ...
  virtual std::vector<std::string> GetExpectedUrls() = 0;

  void OnLoadEventFired(const page::LoadEventFiredParams&) override {
    EXPECT_THAT(http_handler_->urls_requested(),
                ElementsAreArray(GetExpectedUrls()));
    FinishAsynchronousTest();

    // Reset to default.
    browser()->SetEnableResourceScheduler(true);
  }
...
class DisableResourceSchedulerTest : public ResourceSchedulerTest {
 public:
  ...
  std::vector<std::string> GetExpectedUrls() override {
 ...
 public:
  ...
  std::vector<std::string> GetExpectedUrls() override {
    std::vector<std::string> expected_urls = {
        ...
        "http://foo.com/img1",       "http://foo.com/img2",
        "http://foo.com/img3",       "http://foo.com/img4",
        "http://foo.com/img5",       "http://foo.com/xhr8",
        "http://foo.com/frame9",     "http://foo.com/img10",
        "http://foo.com/img11",      "http://foo.com/img6",
        "http://foo.com/script7"};
    return expected_urls;
  }
};
