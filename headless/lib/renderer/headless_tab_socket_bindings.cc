...
bool HeadlessTabSocketBindings::InitializeTabSocketBindings() {
  fprintf(stderr, "InitializeTabSocketBindings %d\n", installed_);
  if (installed_)
    return false;
  ...
  if (context_.IsEmpty()) {
    fprintf(stderr, "InitializeTabSocketBindings context_.IsEmpty()\n");
    return false;
  }
  ...
  if (bindings.IsEmpty()) {
    fprintf(stderr, "InitializeTabSocketBindings bindings.IsEmpty()\n");
    return false;
  }
...
void HeadlessTabSocketBindings::OnMessageFromEmbedder(
  ...
  fprintf(stderr, "HeadlessTabSocketBindings::OnMessageFromEmbedder [%d] %s \n", on_message_callback_.IsEmpty(), message.c_str());
...
void HeadlessTabSocketBindings::SendImpl(const std::string& message) {
  fprintf(stderr, " HeadlessTabSocketBindings::SendImpl %s\n", message.c_str());
...
void HeadlessTabSocketBindings::SetOnMessage(v8::Local<v8::Function> callback) {
  fprintf(stderr, "HeadlessTabSocketBindings::SetOnMessage\n");
