...
bool HeadlessTabSocketBindings::InitializeTabSocketBindings() {
  if (installed_) {
    fprintf(stderr, "[%p] InitializeTabSocketBindings already installed\n", this);
    return false;
  }
  ...
  if (context_.IsEmpty()) {
    fprintf(stderr, "[%p] InitializeTabSocketBindings context_.IsEmpty()\n", this);
    return false;
  }
  ...
  if (bindings.IsEmpty()) {
    fprintf(stderr, "[%p] InitializeTabSocketBindings bindings.IsEmpty()\n", this);
    return false;
  }

  v8::Local<v8::Object> global = context->Global();
  fprintf(stderr, "[%p] InitializeTabSocketBindings %p\n", this, *global);
...
void HeadlessTabSocketBindings::OnMessageFromEmbedder(
  ...
  fprintf(stderr, "[%p] HeadlessTabSocketBindings::OnMessageFromEmbedder [%d] %s \n", this, on_message_callback_.IsEmpty(), message.c_str());
...
void HeadlessTabSocketBindings::SendImpl(const std::string& message) {
  fprintf(stderr, "[%p] HeadlessTabSocketBindings::SendImpl %s\n", this, message.c_str());
...
void HeadlessTabSocketBindings::SetOnMessage(v8::Local<v8::Function> callback) {
  fprintf(stderr, "[%p] HeadlessTabSocketBindings::SetOnMessage\n", this);
