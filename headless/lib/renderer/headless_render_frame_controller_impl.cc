...
void HeadlessRenderFrameControllerImpl::InstallTabSocket(
  ...
  if (find_it == tab_socket_bindings_.end()) {
    fprintf(stderr, "HeadlessRenderFrameControllerImpl::InstallTabSocket unknown id %d\n", execution_context_id);
    LOG(WARNING) << "InstallTabSocket failed, unknown execution_context_id "
                 << execution_context_id;
    std::move(callback).Run(false);
  } else {
    fprintf(stderr, "HeadlessRenderFrameControllerImpl::InstallTabSocket id %d\n", execution_context_id);
...
void HeadlessRenderFrameControllerImpl::InstallMainWorldTabSocket(
  ...
  for (auto& pair : tab_socket_bindings_) {
    if (pair.second.world_id() == content::ISOLATED_WORLD_ID_GLOBAL) {
      fprintf(stderr, "HeadlessRenderFrameControllerImpl::InstallMainWorldTabSocket world exists\n");
      ...
      fprintf(stderr, "HeadlessRenderFrameControllerImpl::InstallMainWorldTabSocket world doesn't exist\n");
...
void HeadlessRenderFrameControllerImpl::SendMessageToTabSocket(
  ...
  if (find_it == tab_socket_bindings_.end()) {
    fprintf(stderr, "HeadlessRenderFrameControllerImpl::SendMessageToTabSocket [%d] %s -- dropped\n", world_id, message.c_str());
    LOG(WARNING) << "Dropping message for " << world_id
                 << " because the world doesn't exist.";
    return;
  }

    fprintf(stderr, "HeadlessRenderFrameControllerImpl::SendMessageToTabSocket [%d] %s \n", world_id, message.c_str());
...
void HeadlessRenderFrameControllerImpl::DidCreateScriptContext(
  ...
  fprintf(stderr, "HeadlessRenderFrameControllerImpl::DidCreateScriptContext [%d]\n", world_id);
...
void HeadlessRenderFrameControllerImpl::WillReleaseScriptContext(
    v8::Local<v8::Context> context,
    int world_id) {
  fprintf(stderr, "HeadlessRenderFrameControllerImpl::WillReleaseScriptContext [%d]\n", world_id);
