...
void HeadlessRenderFrameControllerImpl::InstallTabSocket(
  ...
  if (find_it == tab_socket_bindings_.end()) {
    fprintf(stderr, "[%p] HeadlessRenderFrameControllerImpl::InstallTabSocket unknown id %d\n", this, execution_context_id);
    LOG(WARNING) << "InstallTabSocket failed, unknown execution_context_id "
                 << execution_context_id;
    std::move(callback).Run(false);
  } else {
    fprintf(stderr, "[%p] HeadlessRenderFrameControllerImpl::InstallTabSocket id %d\n", this, execution_context_id);
...
void HeadlessRenderFrameControllerImpl::InstallMainWorldTabSocket(
  ...
  for (auto& pair : tab_socket_bindings_) {
    if (pair.second.world_id() == content::ISOLATED_WORLD_ID_GLOBAL) {
      fprintf(stderr, "[%p] HeadlessRenderFrameControllerImpl::InstallMainWorldTabSocket world exists\n", this);
      ...
      fprintf(stderr, "[%p] HeadlessRenderFrameControllerImpl::InstallMainWorldTabSocket world doesn't exist\n", this);
...
void HeadlessRenderFrameControllerImpl::SendMessageToTabSocket(
  ...
  if (find_it == tab_socket_bindings_.end()) {
    fprintf(stderr, "[%p] HeadlessRenderFrameControllerImpl::SendMessageToTabSocket [%d] %s -- dropped\n", this, world_id, message.c_str());
    LOG(WARNING) << "Dropping message for " << world_id
                 << " because the world doesn't exist.";
    return;
  }

    fprintf(stderr, "[%p] HeadlessRenderFrameControllerImpl::SendMessageToTabSocket [%d] %s \n", this, world_id, message.c_str());
...
void HeadlessRenderFrameControllerImpl::DidCreateScriptContext(
  ...
  fprintf(stderr, "[%p] HeadlessRenderFrameControllerImpl::DidCreateScriptContext [%d]\n", this, world_id);
...
void HeadlessRenderFrameControllerImpl::WillReleaseScriptContext(
    v8::Local<v8::Context> context,
    int world_id) {
  fprintf(stderr, "[%p] HeadlessRenderFrameControllerImpl::WillReleaseScriptContext [%d]\n", this, world_id);
