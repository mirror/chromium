...
// Removes showing contextual menus for headless mode.
...
class HeadlessPopUpMethods {
 public:
  static void Init() {
    static HeadlessPopUpMethods* swizzler = nullptr;
    if (!swizzler) {
      swizzler = new HeadlessPopUpMethods();
    }
  }
 ...
 private:
  HeadlessPopUpMethods()
      : popup_perform_click_swizzler_([NSPopUpButtonCell class],
        ...
        popup_dismiss_swizzler_([NSPopUpButtonCell class],
                                [FakeNSPopUpButtonCell class],
  ...
  base::mac::ScopedObjCClassSwizzler popup_dismiss_swizzler_;
