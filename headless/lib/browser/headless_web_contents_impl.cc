...
void HeadlessWebContentsImpl::OnNeedsExternalBeginFrames(
  ...
  if (begin_frame_events_enabled_)
    SendNeedsBeginFramesEvent();
}

void HeadlessWebContentsImpl::SetBeginFrameEventsEnabled(bool enabled) {
  TRACE_EVENT1("headless",
               "HeadlessWebContentsImpl::SetBeginFrameEventsEnabled", "enabled",
               enabled);

  bool send_event = !begin_frame_events_enabled_ && enabled;
  begin_frame_events_enabled_ = enabled;

  // We only need to send an event if BeginFrames are needed, as clients assume
  // that they are not needed by default.
  send_event &= needs_external_begin_frames_;
...
void HeadlessWebContentsImpl::SendNeedsBeginFramesEvent() {
  TRACE_EVENT1("headless", "HeadlessWebContentsImpl::SendNeedsBeginFramesEvent",
               "needs_begin_frames", needs_external_begin_frames_);
  DCHECK(agent_host_);
  auto params = base::MakeUnique<base::DictionaryValue>();
  params->SetBoolean("needsBeginFrames", needs_external_begin_frames_);

  base::DictionaryValue event;
  event.SetString("method", "Headless.needsBeginFramesChanged");
  event.Set("params", std::move(params));

  std::string json_result;
  DCHECK(base::JSONWriter::Write(event, &json_result));
  // TODO(eseckler): We should only send this to clients that are interested
  // in this event.
  agent_host_->BroadcastProtocolMessageToClients(json_result);
}
...
void HeadlessWebContentsImpl::DidReceiveCompositorFrame() {
  ...
  if (!first_compositor_frame_received_) {
    ...
    // Send an event to the devtools client.
    base::DictionaryValue event;
    event.SetString("method", "Headless.mainFrameReadyForScreenshots");
