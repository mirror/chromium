...
void HeadlessTabSocketImpl::InstallHeadlessTabSocketBindings(
    int v8_execution_context_id,
    base::Callback<void(bool)> callback) {
 fprintf(stderr, "HeadlessTabSocketImpl::InstallHeadlessTabSocketBindings %d\n", v8_execution_context_id);
...
void HeadlessTabSocketImpl::InstallMainFrameMainWorldHeadlessTabSocketBindings(
  ...
  fprintf(stderr, "HeadlessTabSocketImpl::InstallMainFrameMainWorldHeadlessTabSocketBindings\n");
...
void HeadlessTabSocketImpl::OnInstallMainWorldTabSocket(
    base::Callback<void(base::Optional<int>)> callback,
    int v8_execution_context_id) {
  fprintf(stderr, "HeadlessTabSocketImpl::InstallMainFrameMainWorldHeadlessTabSocketBindings %d\n", v8_execution_context_id);
...
void HeadlessTabSocketImpl::SendMessageToContext(
  ...
  fprintf(stderr, "HeadlessTabSocketImpl::SendMessageToContext [%d] %s\n", v8_execution_context_id, message.c_str());
...
void HeadlessTabSocketImpl::SetListener(Listener* listener) {
 ...
 fprintf(stderr, "HeadlessTabSocketImpl::SetListener \n");
...
void HeadlessTabSocketImpl::SendMessageToEmbedder(
  ...
  Listener* listener = nullptr;
  {
    ...
    if (listener_) {
  fprintf(stderr, "HeadlessTabSocketImpl::SendMessageToEmbedder [%d] %s\n", v8_execution_context_id, message.c_str());
      listener = listener_;
    } else {
  fprintf(stderr, "HeadlessTabSocketImpl::SendMessageToEmbedder [%d] %s  --- queued\n", v8_execution_context_id, message.c_str());
