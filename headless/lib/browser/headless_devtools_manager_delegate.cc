...
HeadlessDevToolsManagerDelegate::HeadlessDevToolsManagerDelegate(
  ...
  command_map_["Browser.setWindowBounds"] =
      base::Bind(&HeadlessDevToolsManagerDelegate::SetWindowBounds,
                 base::Unretained(this));
  command_map_["Headless.enable"] = base::Bind(
      &HeadlessDevToolsManagerDelegate::EnableHeadless, base::Unretained(this));
  command_map_["Headless.disable"] =
      base::Bind(&HeadlessDevToolsManagerDelegate::DisableHeadless,
                 base::Unretained(this));

  async_command_map_["Page.printToPDF"] = base::Bind(
      &HeadlessDevToolsManagerDelegate::PrintToPDF, base::Unretained(this));
  async_command_map_["Headless.beginFrame"] = base::Bind(
...
base::DictionaryValue* HeadlessDevToolsManagerDelegate::HandleCommand(
  ...
  auto cmd_result = find_it->second.Run(agent_host, id, params);
...
std::unique_ptr<base::DictionaryValue>
HeadlessDevToolsManagerDelegate::EnableHeadless(
    content::DevToolsAgentHost* agent_host,
    int command_id,
    const base::DictionaryValue* params) {
  content::WebContents* web_contents = agent_host->GetWebContents();
  ...
  headless_contents->SetBeginFrameEventsEnabled(true);
  return CreateSuccessResponse(command_id, nullptr);
}

std::unique_ptr<base::DictionaryValue>
HeadlessDevToolsManagerDelegate::DisableHeadless(
    content::DevToolsAgentHost* agent_host,
    int command_id,
    const base::DictionaryValue* params) {
  content::WebContents* web_contents = agent_host->GetWebContents();
  ...
  HeadlessWebContentsImpl* headless_contents =
      HeadlessWebContentsImpl::From(browser_.get(), web_contents);
  headless_contents->SetBeginFrameEventsEnabled(false);
  return CreateSuccessResponse(command_id, nullptr);
}
...
void HeadlessDevToolsManagerDelegate::BeginFrame(
  ...
  if (params->GetDouble("frameTime", &frame_time_double)) {
    ...
    base::TimeDelta delta = frame_time - base::Time::Now();
    frame_timeticks = base::TimeTicks::Now() + delta;
  } else {
  ...
  if (params->Get("screenshot", &value)) {
    ...
    std::string format;
    if (params->GetString("format", &format)) {
      if (format == kPng) {
    ...
    if (params->GetInteger("quality", &quality) &&
