...
class VirtualTimeIndexDbTest : public VirtualTimeBrowserTest {
 public:
  VirtualTimeIndexDbTest() {
    EXPECT_TRUE(embedded_test_server()->Start());
    SetInitialURL(
        embedded_test_server()->GetURL("/virtual_time_index_db.html").spec());
  }
    ...
    devtools_client_->GetEmulation()->GetExperimental()->SetVirtualTimePolicy(
        emulation::SetVirtualTimePolicyParams::Builder()
            .SetPolicy(
                emulation::VirtualTimePolicy::PAUSE_IF_NETWORK_FETCHES_PENDING)
            .SetBudget(4001)
            .SetMaxVirtualTimeTaskStarvationCount(200)
            .Build());
  }
  ...
  void OnConsoleAPICalled(
    ...
    EXPECT_EQ(runtime::ConsoleAPICalledType::LOG, params.GetType());
    ASSERT_EQ(1u, params.GetArgs()->size());
    ASSERT_EQ(runtime::RemoteObjectType::NUMBER,
              (*params.GetArgs())[0]->GetType());
    int count = (*params.GetArgs())[0]->GetValue()->GetInt();
    // We don't care what exact number |count| has as long as it's not too small
    // or too large.
    EXPECT_GT(count, 50);
    EXPECT_LT(count, 1000);
...
HEADLESS_ASYNC_DEVTOOLED_TEST_F(VirtualTimeIndexDbTest);
