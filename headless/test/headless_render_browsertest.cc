...
class InCrossOriginObject : public HeadlessRenderTest {
 ...
 <html><body>
  <iframe id='myframe' src='http://b.com/'></iframe>
   <script>
...
class ContentSecurityPolicy : public HeadlessRenderTest {
 private:
  GURL GetPageUrl(HeadlessDevToolsClient* client) override {
    ...
    // echo -n "console.log('pass');" | openssl sha256 -binary | openssl base64
    // To generate sha256 hash in hex:
    // echo -n "console.log('pass');" | openssl sha256 -hex
