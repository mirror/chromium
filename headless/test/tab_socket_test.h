...
class TabSocketTest : public HeadlessAsyncDevTooledBrowserTest,
 ...
 private:
  ...
  void InstallHeadlessTabSocketBindings(std::string devtools_frame_id,
                                        base::Callback<void(int)> callback,
  ...
  // TODO(alexclarke): Get Page.createIsolatedWorld to return the execution
  // context, which will let us get rid of these.
  std::map<std::string, base::Callback<void(int)>>
      pending_world_name_callbacks_;
  std::map<std::string, base::Callback<void(int)>> pending_frame_id_callbacks_;
