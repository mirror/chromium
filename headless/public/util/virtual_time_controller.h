...
class HEADLESS_EXPORT VirtualTimeController
 ...
 public:
  ...
  class RepeatingTask {
   public:
    ...
    // advancing.
    virtual void IntervalElapsed(const base::TimeDelta& virtual_time,
                                 base::Callback<void()> continue_callback) = 0;

    // Called when the latest virtual time budget has been used up.
    virtual void BudgetExpired(const base::TimeDelta& virtual_time);
  };
 ...
 private:
  ...
  void NotifyTaskInterval(TaskEntry* entry);
