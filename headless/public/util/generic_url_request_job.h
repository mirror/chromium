...
class HEADLESS_EXPORT Request {
 public:
  ...
  // The process and frame from which the request came from. Note for browser
  // side fetches the process id will be 0.
  virtual std::pair<int, int> GetRenderProcessAndFrameTreeNodeId() const = 0;
...
class HEADLESS_EXPORT GenericURLRequestJob
 ...
 protected:
  ...
  std::pair<int, int> GetRenderProcessAndFrameTreeNodeId() const override;
