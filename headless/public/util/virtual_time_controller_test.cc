...
TEST_F(VirtualTimeControllerTest, ReentrantTask) {
  ...
  VirtualTimeTask task_b(
      ...
      base::Bind(
          [](std::vector<std::string>* log,
             const base::TimeDelta& virtual_time) {
            log->push_back(base::StringPrintf("B: budget requested @ %ld",
                                              virtual_time.InMilliseconds()));
          },
          &log),
      base::Bind(
          [](std::vector<std::string>* log, VirtualTimeController* controller,
             VirtualTimeTask* task_b, const base::TimeDelta& virtual_time) {
            log->push_back(base::StringPrintf("B: interval elapsed @ %ld",
                                              virtual_time.InMilliseconds()));
      ...
      base::Bind(
          [](std::vector<std::string>* log,
             const base::TimeDelta& virtual_time) {
            log->push_back(base::StringPrintf("B: budget expired @ %ld",
                                              virtual_time.InMilliseconds()));
          },
          &log));
  ...
  VirtualTimeTask task_a(
      ...
      base::Bind(
          [](std::vector<std::string>* log,
             const base::TimeDelta& virtual_time) {
            log->push_back(base::StringPrintf("A: budget requested @ %ld",
                                              virtual_time.InMilliseconds()));
          },
          &log),
      base::Bind(
          [](std::vector<std::string>* log, VirtualTimeController* controller,
             ...
             const base::TimeDelta& virtual_time) {
            log->push_back(base::StringPrintf("A: interval elapsed @ %ld",
                                              virtual_time.InMilliseconds()));
      ...
      base::Bind(
          [](std::vector<std::string>* log,
             const base::TimeDelta& virtual_time) {
            log->push_back(base::StringPrintf("A: budget expired @ %ld",
                                              virtual_time.InMilliseconds()));
          },
          &log));
