...
class MockFetcher : public URLFetcher {
 public:
  MockFetcher(base::DictionaryValue* fetch_request,
      ...
      : json_fetch_reply_map_(json_fetch_reply_map),
        ...
        recieved_post_data_(recieved_post_data),
  ...
  void StartFetch(const Request* request,
    ...
    *recieved_post_data_ = request->GetPostData();
    if (!recieved_post_data_->empty() && recieved_post_data_->size() < 1024)
      fetch_request_->SetString("post_data", *recieved_post_data_);
 ...
 private:
  ...
  std::string* recieved_post_data_;                            // NOT OWNED
 ...
 public:
  ...
  net::URLRequestJob* MaybeCreateJob(
    ...
    return new GenericURLRequestJob(
        ...
        base::MakeUnique<MockFetcher>(fetch_request_, recieved_post_data_,
...
class GenericURLRequestJobTest : public testing::Test {
  ...
  GenericURLRequestJobTest() : dispatcher_(message_loop_.task_runner()) {
    url_request_job_factory_.SetProtocolHandler(
        ...
        base::WrapUnique(new MockProtocolHandler(
            &fetch_request_, &recieved_post_data_, &json_fetch_reply_map_,
 ...
 protected:
  ...
  std::string recieved_post_data_;       // The POST data (useful if large).
...
TEST_F(GenericURLRequestJobTest, LargePostData) {
  ...
  for (size_t i = 0; i < recieved_post_data_.size(); i++) {
    EXPECT_EQ(static_cast<char>(i & 127), post_data[i]);
  }

  EXPECT_EQ(post_data.size(), recieved_post_data_.size());
}
