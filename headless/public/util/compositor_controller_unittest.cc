...
TEST_F(CompositorControllerTest, EnsureIdle) {
  bool idle = false;
  ...
  // EnsureIdle executes callback immediately if no BeginFrame is active.
  controller_->EnsureIdle(idle_callback);
  EXPECT_TRUE(idle);
  idle = false;
  ...
  // EnsureIdle only executes callback after BeginFrame was completed.
  controller_->EnsureIdle(idle_callback);
  EXPECT_FALSE(idle);
