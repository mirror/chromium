...
// field of MSG.
#define VISIT_NAMED_OPT_FLATTENABLE(MSG, FIELD, OBJ_NAME) \
  if (MSG.has_##FIELD()) {                                \
...
#define VISIT_OPT_FLATTENABLE(MSG, FIELD) \
  VISIT_NAMED_OPT_FLATTENABLE(MSG, FIELD, FieldToObjName(#FIELD));

// FIELD should not be one that contains itself by default.
#define VISIT_DEFAULT_FLATTENABLE(MSG, FIELD) \
  VISIT_OPT_FLATTENABLE(MSG, FIELD);          \
  if (!flattenable_visited) {                 \
    flattenable_visited = true;               \
    VisitFlattenable(MSG.FIELD(), FieldToObjName(#FIELD)); \
  }
...
// Copied from SkScalar.h:SkScalarIsFinite
...
std::tuple<float, float, float, float> Converter::GetValidRectangle(
    float left,
    float top,
    float right,
    float bottom) {
...
void Converter::Visit(const ImageFilterChild& image_filter_child) {
  ...
  VISIT_OPT_FLATTENABLE(image_filter_child, alpha_threshold_filter);
  ...
  // Can't use VISIT_OPT_FLATTENABLE because of double underscore.
  VISIT_NAMED_OPT_FLATTENABLE(image_filter_child, xfermode_image_filter__base,
...
float Converter::BoundFloat(float num) {
  ...
  if (num >= 0)
    ...
    return fmod(fabsf(num), static_cast<float>(kNumBound));
  // Bound negative numbers.
  return fmod(num, static_cast<float>(-kNumBound));
}
...
void Converter::Visit(const ImageSource& image_source) {
  ...
  auto src_rect = GetValidRectangle(image_source.src().left(),
                                    image_source.src().top(),
                                    image_source.src().right(),
                                    image_source.src().bottom());
...
void Converter::Visit(const ArithmeticImageFilter& arithmetic_image_filter) {
  ...
  WriteFields(arithmetic_image_filter, 2, 6);
}
...
void Converter::Visit(const ColorFilterChild& color_filter_child) {
  ...
  // SRGB is not capitalized by FieldToObjName, use VISIT_NAMED_OPT_FLATTENABLE
  // instead.
  VISIT_NAMED_OPT_FLATTENABLE(color_filter_child, srgb_gamma_color_filter,
                      "SkSRGBGammaColorFilter");

  // The double underscore is not handled properly by FieldToObjName, so use
  // VISIT_NAMED_OPT_FLATTENABLE instead.
  VISIT_NAMED_OPT_FLATTENABLE(color_filter_child, high_contrast__filter,
  ...
  // to VISIT_OPT_FLATTENABLE.
  VISIT_NAMED_OPT_FLATTENABLE(color_filter_child, table__color_filter,
                      "SkTable_ColorFilter");

  // See comment by VISIT call on srgb_gamma_color_filter for why this call
  // isn't to VISIT_OPT_FLATTENABLE.
  VISIT_NAMED_OPT_FLATTENABLE(color_filter_child, to_srgb_color_filter,
                      "SkToSRGBColorFilter");
...
void Converter::Visit(const LooperChild& looper) {
  ...
  // !!! Tmp
  CHECK(std::string("SkLayerDrawLooper") ==
        std::string(FieldToObjName("layer_draw_looper")));
}
...
void Converter::Visit(const RasterizerChild& rasterizer) {
  ...
  // !!! TMP
  CHECK(FieldToObjName("layer_rasterizer") == "SkLayerRasterizer");
}
