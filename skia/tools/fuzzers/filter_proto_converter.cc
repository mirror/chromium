...
// cause issues with OOMs and timeouts. Other times, we may write a value that
// isn't exactly the same as the one given to us by LPM, since we may want to
// write invalid values that the proto definition forbids (eg a number that is
// not in enum).
...
// Visit the skia flattenable that is stored on the FIELD field of MSG.
#define VISIT_OPT_FLATTENABLE(MSG, FIELD)                          \
  if (MSG.has_##FIELD()) {                                         \
    CHECK(!flattenable_visited);                                   \
    VisitFlattenable(MSG.FIELD(), FieldToFlattenableName(#FIELD)); \
...
// The rest of the attributes are not copied from skia.
// 2^(3*8) = 2^24 = 16777216
...
const uint8_t Converter::kEnumMutationChanceDenominator = 40;

// Does not include SkSumPathEffect, SkComposePathEffect, SkRegion, or
// SkLayerDrawLooper, SkLayerRasterizer since they don't use VISIT macros.
...
// function.
Converter::Converter() {
  CHECK_GT(kEnumMutationChanceDenominator, 2);
}
...
std::string Converter::Convert(const Input& input) {
  ...
  enum_mutator_chance_distribution_ =
      std::uniform_int_distribution<>(1, kEnumMutationChanceDenominator);
...
void Converter::WriteRectangle(
    std::tuple<float, float, float, float> rectangle) {
...
void Converter::Visit(const ImageFilterChild& image_filter_child) {
  ...
  VISIT_OPT_FLATTENABLE(image_filter_child, specular_lighting_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, matrix_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, arithmetic_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, alpha_threshold_filter_impl);
  VISIT_OPT_FLATTENABLE(image_filter_child, blur_image_filter_impl);
  VISIT_OPT_FLATTENABLE(image_filter_child, color_filter_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, compose_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, displacement_map_effect);
  VISIT_OPT_FLATTENABLE(image_filter_child, drop_shadow_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, local_matrix_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, magnifier_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, matrix_convolution_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, merge_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, dilate_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, offset_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, picture_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, tile_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, xfermode_image_filter__base);
  VISIT_OPT_FLATTENABLE(image_filter_child, xfermode_image_filter);
  VISIT_OPT_FLATTENABLE(image_filter_child, diffuse_lighting_image_filter);
  // TODO(metzman): figure out if this is actually an image filter.
  VISIT_OPT_FLATTENABLE(image_filter_child, image_source);
...
float Converter::BoundFloat(float num) {
  ...
  float result;
  if (num >= 0)
    result = fmod(num, static_cast<float>(kNumBound));
  else if (bound_positive_)
    result = fmod(fabsf(num), static_cast<float>(kNumBound));
  else
    // Bound negative numbers.
    result = fmod(num, static_cast<float>(-kNumBound));
  if (!IsFinite(result))
...
void Converter::Visit(const DilateImageFilter& dilate_image_filter) {
  ...
  WritePositiveFields(dilate_image_filter, 2);
}
...
void Converter::Visit(const MagnifierImageFilter& magnifier_image_filter) {
  ...
  const float inset = BoundFloat(magnifier_image_filter.inset());
...
void Converter::Visit(
  ...
  VisitFlattenable(alpha_threshold_filter_impl.rgn(), "SkRegion");
}
...
// flattenables.
// TODO(metzman): Implement the functionality described above.
...
void Converter::Visit(const PictureHeader& picture_header) {
  WriteNum(picture_header.version());
  Visit(picture_header.rectangle());
  WriteBool(picture_header.is_ptr_64_bit());
}
...
bool Converter::IsFinite(float num) const {
  // If num is inf, -inf, nan or -nan -> num*0 = nan.
...
void Converter::Visit(const PaintEffects& paint_effects) {
  ...
  VISIT_OPT_OR_NULL(paint_effects, rasterizer);
...
void Converter::Visit(const ColorFilterChild& color_filter_child) {
  bool flattenable_visited = false;
  VISIT_OPT_FLATTENABLE(color_filter_child, color_matrix_filter_row_major_255);
  VISIT_OPT_FLATTENABLE(color_filter_child, compose_color_filter);
  VISIT_OPT_FLATTENABLE(color_filter_child, srgb_gamma_color_filter);
  VISIT_OPT_FLATTENABLE(color_filter_child, high_contrast__filter);
  VISIT_OPT_FLATTENABLE(color_filter_child, luma_color_filter);
  VISIT_OPT_FLATTENABLE(color_filter_child, overdraw_color_filter);
  VISIT_OPT_FLATTENABLE(color_filter_child, table__color_filter);
  VISIT_OPT_FLATTENABLE(color_filter_child, to_srgb_color_filter);
...
void Converter::Visit(const BlurMaskFilter& blur_mask_filter) {
  WriteFields(blur_mask_filter, 1, 3);
...
void Converter::Visit(const ShaderChild& shader) {
  bool flattenable_visited = false;
  VISIT_OPT_FLATTENABLE(shader, color_4_shader);
  VISIT_OPT_FLATTENABLE(shader, color_filter_shader);
  VISIT_OPT_FLATTENABLE(shader, image_shader);
  VISIT_OPT_FLATTENABLE(shader, compose_shader);
  VISIT_OPT_FLATTENABLE(shader, empty_shader);
  VISIT_OPT_FLATTENABLE(shader, picture_shader);
  // TODO(metzman): Uncomment when PerlinNoiseShaderImpl is fixed and this code
  // can be run without causing crashes.
  // VISIT_OPT_FLATTENABLE(shader, perlin_noise_shader_impl);

  VISIT_OPT_FLATTENABLE(shader, local_matrix_shader);
  VISIT_OPT_FLATTENABLE(shader, linear_gradient);
  VISIT_OPT_FLATTENABLE(shader, radial_gradient);
  VISIT_OPT_FLATTENABLE(shader, sweep_gradient);
  VISIT_OPT_FLATTENABLE(shader, two_point_conical_gradient);
...
void Converter::Visit(const RasterizerChild& rasterizer) {
  VisitFlattenable(rasterizer.layer_rasterizer(), "SkLayerRasterizer");
}
...
// and size) correctly and Visiting it. Because this is a templated function, it
...
void Converter::Visit(const DashImpl& dash_impl) {
  ...
  WriteNum(dash_impl.intervals_size());
...
void Converter::Visit(const PathEffectChild& path_effect) {
  ...
  VISIT_OPT_FLATTENABLE(path_effect, path_2d_path_effect);
  VISIT_OPT_FLATTENABLE(path_effect, line_2d_path_effect);
  VISIT_OPT_FLATTENABLE(path_effect, corner_path_effect);
  VISIT_OPT_FLATTENABLE(path_effect, discrete_path_effect);
  VISIT_OPT_FLATTENABLE(path_effect, path_1d_path_effect);
...
void Converter::Visit(const MaskFilterChild& mask_filter) {
  bool flattenable_visited = false;
  VISIT_OPT_FLATTENABLE(mask_filter, emboss_mask_filter);
  VISIT_OPT_FLATTENABLE(mask_filter, r_rects_gaussian_edge_mask_filter_impl);
...
void Converter::WritePositiveFields(const Message& msg,
                                    const unsigned start,
                                    const unsigned end) {
  // Don't allow nested calls.
  CHECK(!bound_positive_);
  bound_positive_ = true;
  WriteFields(msg, start, end);
  bound_positive_ = false;
}
...
// TODO(metzman): Handle enums with negative values properly (there aren't any
// in skia we need to worry about yet).
