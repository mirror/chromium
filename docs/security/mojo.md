...
interface Frame {
  ...
  //
  // If untrustworhty processes can talk to trustworthy processes, the Frame
...
Mojo structs, interfaces, and methods should be documented. Make sure the
...
the renderer process, this should be mentioned in the comments.
...
For example, the browser must not (fully) trust the renderer's claims about
origins. The browser should already know what origin the renderer is
evaluating, and thus should already have this data (for example, see
`RenderFrameHost::GetLastCommittedOrigin()`). Thus, a method that requires
passing an origin from the renderer to the browser has a conceptual error, and
...
Similarly, the browser must not trust the renderer's claims about file
pathnames. It would be unsafe for the browser to save a downloaded file to
`~/.bashrc` just because the renderer asked. Instead, it would be better for the
browser to:
...
Even better would be to enforce (1) with the type system, by creating a
`Basename` type (distinct from `mojo.common.mojom.FilePath`) whose
constructor(s), serializer(s), and deserializer(s) enforced the security
property.
...
Similarly, do not define a `MAX` value for an enum just for logging an enum
value UMA. It forces every `switch` to handle this a case that should never
happen--or worse, suppresses compiler warnings about unhandled enum values by
encouraging use of the `default` label.

> Improving UMA logging is tracked in <https://crbug.com/742517>.


### Prefer structured types when possible
...
Another pattern to avoid is parallel arrays of data: this requires the receiver
to validate that all the arrays have the same length. Instead, prefer to pass
the data so that it is impossible to have a mismatch.
...
```
// Bad: code using this struct needs to check all the arrays have the same
// length.
struct Bitmap {
  array<int8> reds;
  array<int8> greens;
...
Creating a typemap and defining a `StructTrait` specialization moves the
...
struct StructTraits<url::mojom::UrlDataView, GURL> {
  ...
  static bool Read(url::mojom::UrlDataView data, GURL* out) {
    ...
    if (!data.ReadUrl(&url_string)) return false;
    if (url_string.length() > url::kMaxURLChars) return false;
    *out = GURL(url_string);
...
`StructTrait` for type mapping: this commonly occurs with Blink IDL and Oilpan
...
A `StructTrait` specialization is almost always fully specialized. Only define
StructTrait methods inline if the method is a simple getter that returns a
reference, pointer, or other simple POD. Out-of-line all other members to avoid
code bloat.
...
Connection errors in the renderer
...
### Cache message pipes when appropriate
