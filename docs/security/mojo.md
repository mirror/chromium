...
For enums, avoid the pattern of defining a `LAST` enum value or a `MAX` enum
value. The `LAST` enum value is typically used in conjunction with legacy IPC
to validate enum values; this is not needed with Mojo enums, which automatically
validated.

The `MAX` enum value is typically used as an invalid sentinel value for UMA
...
// Note that these are defined out-of-line to avoid code bloat.
...
StructTrait methods inline in the header if the method is a simple getter that
returns a reference, pointer, or other simple POD. Define all other methods
