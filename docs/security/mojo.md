...
are needed outside of `StructTraits` specializations.

`mojo::ReportBadMessage()` or `mojo::GetBadMessageCallback()` should be used to
reject bad input in this case. Under the hood, this may record UMAs, kill the
