...
guidelines are as follows:
  ...
  * **Restrict-View-SecurityEmbargo**: Restricts access to
  ...
  If multiple restriction labels are appropriate, set all of them. Note that if
  multiple restriction labels are set, all of them must be satisfied for a user
  to have access to a bug.
...
spend less time on updating bugs and can do more useful work instead.
...
Based **M-#** milestone labels this rule assigns corresponding
...
more than 14 weeks ago and add the **allpublic** label to make the bugs
