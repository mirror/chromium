...
base/numerics](https://chromium.googlesource.com/chromium/src/+/master/base/numerics/README.md)
to avoid overflows, **especially when calculating the size or offset of memory
allocations**.
...
`uint32_t`, since sender and receiver could potentially use different
interpretations of implicitly-sized types like `int` or `long`.
