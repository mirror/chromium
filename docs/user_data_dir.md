...
subdirectory (such as `Default`) within the user data directory.
...
It varies by platform, branding (Chrome vs. Chromium,
based on `is_chrome_branded` in
[GN args](https://www.chromium.org/developers/gn-build-configuration)), and
...
`$CHROME_CONFIG_HOME` (since M61) or by `$XDG_CONFIG_HOME`.  The former is
Chrome-specific, while the latter affects all applications conforming to the
...
and is specific to the app.  Example: `/data/user/0/com.android.chrome/app_chrome`
...
`--user-data-dir` command-line flag to the Chrome binary.  For example:
...
`$CHROME_USER_DATA_DIR` environment variable.  (The `--user-data-dir` flag takes
precedence if both are present.)

### Creating an AppleScript Wrapper
...
when running inside a Chrome Remote Desktop virtual session on a Linux host.
...
Since M61, this can be solved by using `$CHROME_CONFIG_HOME` instead of
`$CHROME_USER_DATA_DIR`. To do this, put the following in
