...
there is still a large number of warnings to be fixed in Chromium and especially
...
in runtime. Fortunately, we have a tool at our disposal for guiding the analyzer
away from impossible codepaths: assertion handlers like DCHECK/CHECK/LOG(FATAL)).
The analyzer won't check the codepaths which violate asserts.
...
* Dead stores. To suppress, void-cast the variable in a statement.
  e.g. `int my_variable; static_cast<void>(my_variable);`
* Unchecked function call results. To suppress, void-cast the result.
