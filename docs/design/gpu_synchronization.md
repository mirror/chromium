...
```c++
    ...
    // a local fence.
    std::unique_ptr<gl::GLFence> local_fence = gl::GLFence::CreateGpuFence();

    // Create a matching gpu fence on the command buffer context, issue
    // server wait, and destroy it.
    EGLint id = gl->InsertClientGpuFenceCHROMIUM(local_fence.AsClientGpuFence());
...
```c++
    ...
    // create a gpu fence:
    EGLint id = gl->InsertGpuFenceCHROMIUM();
