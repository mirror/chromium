...
| Raw Heap Dump data | [Heap Dumps](#heap-dumps) | Grabs raw data for analysis by other tools |
...
that can be connected and viewed via the chrome://tracing infrastructure.
...
Exploring the heap dump for a process.
...
Good for examining allocations that occur during an interval of time. This is
often useful for finding leaks as one call-stack will rise to the top as the
leak is repeated triggered.
