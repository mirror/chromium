...
| Topic/Area of Inquiry  | Tool(s) | What this is good for |
|----------------------- | ------- | --------------------- |
| Object allocation over time | [`diff_heap_profiler.py`](#diff-heap-profiler), [Heap Profile view in Chrome Tracing](#tracing-heap-profile) | Finding leaked C++ objects over time. |
...
| Raw Heap Dump data | [Heap Dump](#heap-dump) | Grabs raw data for analysis by other tools |

If that seems like a lot of tools and complexity, it is [but there's a reason](#no-one-true-metric).

-----------
## <a name="global-memory-dump"></a> Viewing Global Memory Dump in chrome://tracing
...
-----------
## <a name="tracing-heap-profile"> Viewing a Heap Profile in chrome://tracing
### What this is good for
Exploring a heap dump within a process.
...
### What this is good for
Examining allocations that occur during an interval of time. This is often useful
...
### Instructions
TODO(ajwong): Reference the "Get a memory dump" instructions.
...
| Keep track of even the small allocations in memlog heap dumps. | By default, small allocations are not logged to reduce dump size. Enabling this track _all_ allocations. |
...
#### Saving a heap dump
  1. [Desktop only] Click "save dump" in chrome://memory-internals to save a
     dump of all the profiled processes. On Android, take a memory-infra trace
     which will have the dump embedded.
...
this was native file save was less usable.

-----------
## <a name="tracing-memory-infra"></a> Taking a memory-infra trace.
### What this is good for
...
### Instructions
  ...
  3. Do work that triggers events/behaviors to measure. If OOPHP is enabled,
     don't run for more than a few seconds to avoid crashing the
