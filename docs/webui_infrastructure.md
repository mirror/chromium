...
To create a WebUI interface in `components/` need to follow different steps from [Creating WebUI Interfaces in `chrome/`](https://www.chromium.org/developers/webui). This guide is specific to creating a WebUI interface in `src/components/`. This guide is based on the steps I went through to create the WebUI infrastructure for [chrome://safe-browsing](chrome://safe-browsing).
