...
This guide is specific to creating a WebUI interface in `src/components/`. To create a WebUI interface in `src/chrome/` follow [Creating Chrome WebUI Interfaces](https://www.chromium.org/developers/webui) guide.
...
In order for your new class to be built and linked, it needs to be added to the project gypi file. Create new `BUILD.gn` and `DEPS` files inside `src/components/hello_world/`.

`src/components/hello_world/BUILD.gn:`
```
sources = [
  "hello_world/hello_world_ui.cc",
