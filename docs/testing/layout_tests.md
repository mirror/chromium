...
| `--reset-results`           |	Overwrite the current expectations with generated results, or create new expectations if no existing expectations. |
...
If there are current expectation filess for `LayoutTests/foo/bar/test.html`,
the above command will overwrite the current expectation files at their original
locations with the actual results. The current expectation file means the
expectation file used to compare the actual result when the test is run locally,
i.e. the first expectation found in the [baseline search path]
(https://cs.chromium.org/search/?q=port/base.py+baseline_search_path).

If there are no current expectation files, the above command will create new
expectation files in the platform-independent directory, e.g.
...
Though we prefer the Rebaseline Tool to local rebaseling, the Rebaseline Tool
...
```
New expectations will be created at e.g.
`LayoutTests/flag-specific/enable-flag/foo/bar/test-expected.{txt,png}`.

Then you can commit the new expectation files and upload the patch for review.
