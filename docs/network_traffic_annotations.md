...
To make Chrome’s network communications transparent, we would need to be able to
provide the following answers:
* What is the intent behind each network communication?
* What user data is sent during the communication, and where does it go?

Besides these requirements, the following information very much helps Enterprise
admins and help desk:
* How can a network communication be stopped or controlled?
* What are the traces of the communication on the client?

It should be noted that the technical details of communications are not
necessarily important to the users, but we need to show that we have covered all
bases and there are no back doors.


## The Solution

We can provide up to date, in-line documentations on origin, intent, payload,
and control mechanisms of each network communication. This is done by adding a
...
communication and its payload, this metadata does not need to be transmitted
with the communication during runtime and it is sufficient to have it in
appropriate positions in the code. Having that as an argument of all network
communication functions is a mechanism to enforce its existence and showing the
users our intent to cover the whole repository.


## Best Practices

### Where to add annotation?
All network requests are ultimately converted into socket data send or native
API function calls, but we should note that the concern is about the main intent
of the communication and not the implementation details. Therefore we do not
need to specify this data separately for each call to each function that is used
in the process and it is sufficient that the most rational point of origin would
be annotated and the annotation would be passed through the downstream steps.
Choosing the best code site for that is a judgement call based on the following
criteria:
  1. Stating the origin of user’s intent or internal requirement for creating
     that communication.
  2. The controls over stopping or limiting the connection (Chrome settings or
     policies) are enforced.
  3. The data that is sent are specified.
E.g., all url requests are ultimately converted into socket data transfers. All
socket connections that send data as the result of these requests, can carry on
the annotation which is provided to the URL request.
...
* Unique ID: A globally unique identifier that must stay unchanged while the
...
* Traffic Source: These set of fields specify the location of annotation in the
  source code. These fields are automatically set and do not need specification.
* Traffic Semantics: These set of fields specify meta information about the
  network request’s content and reason.
   * Sender: What component triggers the request. The components should be human
     readable and don’t need to reflect the components/ directory. Avoid
     abbreviations.
   * Description: Plaintext description of the network request in language that
     is understandable by admins (ideally also users). Please avoid acronyms and
     describe the feature and the    feature's value proposition as well.
   ...
   * Destination: Target of the network request. It can be either the website
     ...
     browser feature, use `WEBSITE`. In other cases use `LOCAL` if the request
     is processed locally and doesn't go to network, otherwise use `OTHER`. If
     OTHER is used, please add plain text description in `destination_other`
     field.
   * Destination Other: Human readable description in case the destination
     points to `OTHER`.
* Traffic Policy: These set of fields specify the controls that a user may have
  on disabling or limiting the network request and its trace.
   * Empty Policy Justification: If traffic policy cannot be specified, like
   ...
   * Cookies Allowed: Specifies if this request stores and uses cookies or not.
     Use values `YES` or `NO`.
   * Cookies Store: If a request sends or stores cookies/channel IDs/... (i.e.
      ...
      * Otherwise, please explain (e.g. SafeBrowsing uses a separate cookie
        store).
   * Setting: Human readable description of how to enable/disable a feature that
     triggers this network request by a user (e.g. “Disable ‘Use a web service
     to help resolve spelling errors.’ in settings under Advanced”). Note that
     settings look different on different platforms, make sure your description
     works everywhere!
   * Chrome Policy: Policy configuration that disables or limits this network
   ...
   * Policy Exception Justification: If there is no policy to disable or limit
...
* policy: none (Check again! Most features can be disabled by policy).
  ...maybe sift through the code reviews and find those that had a lot of
   iteration?
...
* They annotations are syntactically correct.
...
Privacy review are enforced through keeping a summary of annotations in
...
possible to do so (you are changing it from an unsupported platform or you don’t
have a compiled build directory), the code can be submitted to the trybot and
the test on trybot will tell you the required modifications.
