...
CBORToken WebPackageReader::ParseCBORToken() {
  ...
  CBORValue::Type type =
      static_cast<CBORValue::Type>(item_start >> constants::kMajorTypeBitShift);
  // TODO bound chk

  const size_t additional_length_encoded =
      (item_start & constants::kAdditionalInformationMask);
  if (additional_length_encoded < constants::kAdditionalInformation1Byte) {
  ...
  switch (additional_length_encoded) {
    case constants::kAdditionalInformation1Byte:
      additional_length = 1;
      break;
    case constants::kAdditionalInformation2Bytes:
      additional_length = 2;
      break;
    case constants::kAdditionalInformation4Bytes:
      additional_length = 4;
      break;
    case constants::kAdditionalInformation8Bytes:
      additional_length = 8;
      break;
    default:
...
bool WebPackageReader::TryConsume() {
  // LOG(ERROR) << "### TryConsume: " << StateToString(state_) << " unparsed_:"
  ...
  switch (state_) {
    ...
    case State::kParseManifestSectionMap: {
      ...
      const auto& origin_value = value.GetMap()
                                     .find(CBORValue("manifest"))
                                     ->second.GetMap()
                                     .find(CBORValue("metadata"))
                                     ->second.GetMap()
                                     .find(CBORValue("origin"))
                                     ->second;
    ...
    case State::kExpectIndexArrayEntryOffset: {
      ...
      parsed_index_entries_.emplace_back(std::move(pending_parsed_headers_),
                                         offset);
      int request_id = parsed_index_entries_.size() - 1;
      client_->OnResourceRequest(parsed_index_entries_[request_id].first,
                                 request_id);
    ...
    case State::kSkipUntilNextResponse: {
      CHECK_GE(parsed_index_entries_[next_response_to_read_idx_].second,
               consumed_bytes_ - response_array_offset_)
          << "indexed-content not in order!";
      size_t bytes_to_skip =
    ...
    case State::kExpectResponseHeaderBytes: {
      ...
      const auto response_headers =
          DecodeHPack(header_bytes, expected_blob_len_);
      client_->OnResourceResponse(next_response_to_read_idx_, response_headers);
    ...
    case State::kExpectResponseBodyBytes: {
      ...
      size_t read_length = std::min(unparsed_bytes_.size(), expected_blob_len_);

      const uint8_t* bytes = TryConsumeBytes(read_length);
      CHECK(bytes);
      client_->OnDataReceived(next_response_to_read_idx_, bytes, read_length);

      expected_blob_len_ -= read_length;

      CHECK_GE(expected_blob_len_, 0u);
      if (expected_blob_len_) {
        // still data to read
        return true;
      } else {
        client_->OnNotifyFinished(next_response_to_read_idx_);
