...
bool PDFiumEngine::OnMouseDown(const pp::MouseInputEvent& event) {
  ...
  if (page_index != -1) {
    ...
    if (form_type > FPDF_FORMFIELD_UNKNOWN) {  // returns -1 sometimes...
      PDFiumPage::Area form_area = PDFiumPage::FormTypeToArea(form_type);
      mouse_down_state_.Set(form_area, target);
      ...
      bool is_form_text_area = form_area == PDFiumPage::FORM_TEXT_AREA;
