...
bool PDFiumEngine::OnRightMouseDown(const pp::MouseInputEvent& event) {
  ...
  if (in_form_text_area_) {
    if (is_form_text_area) {
      // TODO(thestig): Right now this is a no-op, but what needs to happen here
      // is a check to see if this right click occurred in the same text area as
      // the currently focused text area. If they are not the same, refocus.
      // This requires more state tracking, and a new PDFium API to set focus.
      return true;
    }
  ...
  if (is_form_text_area) {
    ...
    // TODO(thestig): This does not actually set focus inside the form area.
