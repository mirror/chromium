...
FPDF_PAGE PDFiumPage::GetPage() {
  ...
  if (!page_) {
    ...
    // std::cerr << "PDFiumPage just FPDF_LoadPage'ed " << index_ << std::endl;
...
PDFEngine::PageFeatures& PDFiumPage::GetPageFeatures() {
  if (!page_features_.IsInitialized()) {
    ...
    if (page) {
      ...
      for (int i = 0; i < annotation_count; ++i) {
        ...
        page_features_.annotation_types.push_back(subtype);
      }
    }
  }
