...
#if !defined(NDEBUG)
// Sends |format_string| to the debugger for display.
//
// This is for developers only; we don't use this in circumstances
// (like release builds) where users could see it.
void TraceImpl(const wchar_t* format_string, __VA_ARGS__) {
  ...
  if (vswprintf(buffer, arraysize(buffer), format_string, args) > 0) {
