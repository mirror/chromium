...
base::string16 GetToastActivatorRegistryPath() {
  ...
  // The static_cast is safe because our string sizes will generally fit into an
  // int.
  if (::StringFromGUID2(install_static::GetToastActivatorClsid(), &path[0],
                        static_cast<int>(path.capacity())) != kGuidLengthFull) {
...
class NotificationHelperTest : public testing::Test {
 protected:
  NotificationHelperTest() {
    ...
    toast_activator_reg_path_ = GetToastActivatorRegistryPath();
  }

  void SetUp() override {
    backup_.Initialize(HKEY_CURRENT_USER, toast_activator_reg_path_.c_str(),
                       KEY_WOW64_32KEY);
  }

  void TearDown() override {
    backup_.WriteTo(HKEY_CURRENT_USER, toast_activator_reg_path_.c_str(),
                    KEY_WOW64_32KEY);
  }
...
TEST(NotificationHelperTest, NotificationHelperServerTest) {
  // Initialize WinRT.
