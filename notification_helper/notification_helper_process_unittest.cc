...
class NotificationHelperTest : public testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    ASSERT_TRUE(backup_64bit_.Initialize(
        root_, toast_activator_reg_path().c_str(), KEY_WOW64_64KEY));
    ASSERT_TRUE(backup_32bit_.Initialize(
        root_, toast_activator_reg_path().c_str(), KEY_WOW64_32KEY));
  }

  void TearDown() override {
    // Restore the registration.
    ASSERT_TRUE(backup_64bit_.WriteTo(root_, toast_activator_reg_path().c_str(),
                                      KEY_WOW64_64KEY));
    ASSERT_TRUE(backup_32bit_.WriteTo(root_, toast_activator_reg_path().c_str(),
                                      KEY_WOW64_32KEY));
  }
  ...
  void DeleteHelperRegistry() {
    ASSERT_TRUE(InstallUtil::DeleteRegistryKey(
        root_, toast_activator_reg_path(), KEY_WOW64_64KEY));
    ASSERT_TRUE(InstallUtil::DeleteRegistryKey(
        root_, toast_activator_reg_path(), KEY_WOW64_32KEY));
  }
  ...
  void RegisterServer() {
    ...
    install_list->AddSetRegValueWorkItem(root_, toast_activator_server_path,
                                         KEY_WOW64_64KEY, L"", command, true);

    install_list->AddSetRegValueWorkItem(root_, toast_activator_server_path,
                                         KEY_WOW64_64KEY, L"ServerExecutable",
 ...
 private:
  ...
  // Backup of the deleted 64-bit registry key.
  RegistryKeyBackup backup_64bit_;

  // Backup of the deleted 32-bit registry key.
  RegistryKeyBackup backup_32bit_;
