...
// The event reset policy MUST BE set to MANUAL to avoid signaling the event in
// IsSignaled() itself, which is called by IsEventSignaled().
...
HRESULT ComServerModule::RegisterClassObjects() {
  ...
  if (FAILED(hr)) {
    Trace(L"%hs(Factory creation failed, last error was %d)\n", __func__, hr);
    return hr;
  }

  Microsoft::WRL::ComPtr<IClassFactory> class_factory;
  factory.As(&class_factory);
  ...
  if (FAILED(hr)) {
    Trace(
        L"%hs(NotificationActivator registration failed, last error was %d)\n",
        __func__, hr);
  }
...
void ComServerModule::UnregisterClassObjects() {
  ...
  if (FAILED(hr)) {
    Trace(
        L"%hs(NotificationActivator unregistration failed, last error was "
        L"%d)\n",
        __func__, hr);
  }
}
...
void ComServerModule::WaitForObjectReleased() {
