...
HRESULT ComServerModule::RegisterClassObjects() {
  ...
  if (FAILED(hr)) {
    Trace(L"%hs(Factory creation failed, last error is %d)\n", __func__, hr);
    return hr;
  }
  ...
  hr = module.RegisterCOMObject(nullptr, class_ids, class_factories, cookies_,
                                arraysize(cookies_));
  if (FAILED(hr)) {
    Trace(L"%hs(NotificationActivator registration failed, last error is %d)\n",
          __func__, hr);
    return hr;
  }
...
void ComServerModule::UnregisterClassObjects() {
  ...
  if (FAILED(hr))
    Trace(
        L"%hs(NotificationActivator unregistration failed, last error is %d)\n",
        __func__, hr);
}
