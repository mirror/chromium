...
Refer to Design Doc of Directory Component Mapping: https://goo.gl/xtSMpV
crbug.com/701551.
"""
...
def train_model(mappings, component_data, component_label,
  ...
  Args:
    mappings(dict): a dict of "dirtory_path: components"
...
def predict_component(input_dirs, tfidf_transformer,
  ...
  Args:
    ...
    tfidf_transformer: tfidf transformer to convert input directories into tfidf
      feature vector.
    bigram_vectorizer: vectorizer for tfidf feature.
    component_defination(list): all component label definations.
...
def evaluate_model_on_trainset(mappings, component_label,
  ...
  """Evaluate model performance on training set. Compute recall and
  accuracy of prediction for directories in training set, i.e. directories
  in mappings.
...
def evaluate_prediction(dirs_for_predict, component_defination,
  ...
  truth label. Compute recall and precision(accuracy).

  Args:
    dirs_for_predict(list): list of dirs to predict.
    component_defination(list): all component definations.
