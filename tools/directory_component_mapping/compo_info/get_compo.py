...
class ComponentTree(object):
  ...
  def __init__(self):
    """
    components_defs(list): a list of all monorail component definations.
    component_labels(list): a list of component labels.
    component_docstrings(list): a list of docstring for every component.
    deprecated_components(list): a list of deprecated components.
    """
    self.component_defs = []
  ...
  def get_component_from_database(self):
    """Get monorail components from database."""
    connection = MySQLdb.connect(host=MONORAIL_HOST, user=USER_NAME,
                                 db=MONORAIL_DB)
  ...
  def get_component_labels(self):
    ...
    e.g. [Blink>Layout]:[Blink, Blink>Layout]
    """
