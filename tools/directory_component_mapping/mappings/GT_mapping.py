...
PREDATOR_MAPPING_FILE = (
    "/usr/local/dir/to/component_mapping.json")
...
class GroundTruthMapping(object):
  ...
  def get_complete_dir(self, dir_path):
    """Get complete directory paths match dir_path using regular
    expression.
    """
    dirs = dir_path.split('/')
    dir_pre = ""
    ...
    dir_helper = self.get_subdir_list(dir_pre, LOCAL_PATH)
  ...
  def normalize_mapping(self):
    ...
    Explicitly list all mappings (e.g. replace dir/.* with a complete list of
    ...
    for dir_path, components in self.mappings.items():
      if dir_path.find(".*") < 0:
        ...
        self.get_complete_dir(dir_path)
