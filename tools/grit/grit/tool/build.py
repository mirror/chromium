  ...
  def ProcessNode(node, output_node, outfile):
    ...
    if output_node.GetType() == 'data_package':
      with open(output_node.GetOutputFilename() + '.info', 'w') as infofile:
        # We terminate with a newline so that when these files are concatenated
        # later we consistently terminate with a newline so consumers can
        # account for terminating newlines.
        for line in node.info:
          infofile.write(line + '\n')
