...
class DataPackSizes(object):
  def __init__(self, header_size, id_table_size, alias_table_size, data_size):
    self.header_size = header_size
    self.id_table_size = id_table_size
    self.alias_table_size = alias_table_size
  ...
  def __iter__(self):
    yield ('header_size', self.header_size)
    yield ('id_table_size', self.id_table_size)
    yield ('alias_table_size', self.alias_table_size)
...
def ReadDataPackFromString(data):
  ...
  assert sizes.total_size == len(data), 'original={} computed={}'.format(
      len(data), sizes.total_size)
