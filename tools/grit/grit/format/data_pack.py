...
def ReadDataPackFromString_V6(data):
  ...
  def gen_resources():
    ...
    for e in xrange(1, id_entries_count):
      next_resource_id, next_idx = struct.unpack(
          '<HH', data[e*kIdSize:(e+1)*kIdSize])
      ...
      for i in xrange(size):
        ...
        if idx < short_offset_count:
          ...
          resource_offset = struct.unpack(
              '<H', data[entry_offset:entry_offset+kShortOffsetSize])[0]
          if idx >= resource_count_64_1:
            resource_offset += k64K
          if idx >= resource_count_64_1 + resource_count_64_2:
            resource_offset += k64K
        else:
          entry_offset = base_offset_tables + \
              short_offset_count * kShortOffsetSize + \
              (idx-short_offset_count) * kLongOffsetSize
          resource_offset = struct.unpack(
              '<I', data[entry_offset:entry_offset+kLongOffsetSize])[0] + \
                  3 * k64K
