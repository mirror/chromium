...
def _ApplyTool(tools_clang_scripts_directory,
  ...
  try:
    ...
    # Launch the following pipeline if |apply_tool_edits| is True:
    #     run_tool.py ... | extract_edits.py | apply_edits.py ...
    # Otherwise just the first step and the result is written to
    #   actual_files.
    args = ['python',
...
def main(argv):
  ...
  parser.add_argument(
      '--simple-compare',
      action='store_true',
      help='Just compares the output of the tool with the expected file, '
           'without trying to apply the changes.')
  ...
  source_files = glob.glob(os.path.join(test_directory_for_tool,
                                        '*-original.cc'))
  ext = 'txt' if args.simple_compare else 'cc'
