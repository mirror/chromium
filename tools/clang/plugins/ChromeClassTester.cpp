...
ChromeClassTester::LocationType ChromeClassTester::ClassifyLocation(
  ...
  // If the SourceLocation is from an expanded macro, look at the original
  // spelling location where the characters for the SourceLocation came from.
  // The general idea is that macros defined in third-party code should be
  // ignored. This heuristic isn't perfect: if the SourceLocation happens to
  // point to a macro argument expanded from a non-third-party location, then
  // the macro won't be ignored, but this should be a relatively rare
  // occurrence.
  // TODO(dcheng): The right way to implement this is probably using
  // PPCallbacks using MacroExpands, since this has the MacroDefinition with the
  // SourceLocation that defines it, but it's not clear how to integrate this
  // with a RecursiveASTVisitor.
  if (loc.isMacroID()) {
    return ClassifyLocation(instance().getSourceManager().getSpellingLoc(loc));
  }
