...
void FindBadConstructsConsumer::CheckVirtualSpecifiers(
  ...
  if (is_override && !override_attr && !final_attr) {
    add_override = true;
    // Also remove the virtual in the same fixit if needed.
    if (has_virtual)
      remove_virtual = true;
  }
  ...
  // Attempt to setup the lexer in raw mode.
  ...
  if (!invalid) {
    ...
    while (!lexer.LexFromRawLexer(token)) {
      ...
      // TODO(dcheng): Unclear if this needs to check for nested parentheses as
      // well?
  ...
  if (add_override) {
    if (override_insertion_loc.isValid()) {
      ReportIfSpellingLocNotIgnored(override_insertion_loc,
                                    diag_method_requires_override_)
          << FixItHint::CreateInsertion(override_insertion_loc, " override");
    } else {
      ReportIfSpellingLocNotIgnored(method->getLocation(),
                                    diag_method_requires_override_);
    }
  } else if (remove_virtual && virtual_loc.isValid()) {
    ReportIfSpellingLocNotIgnored(virtual_loc,
                                  diag_redundant_virtual_specifier_)
        << "'virtual'"
