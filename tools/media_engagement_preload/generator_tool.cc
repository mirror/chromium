...
bool CheckEntries(const std::set<std::string>& entries) {
  for (const auto& entry : entries) {
    for (const char& c : entry) {
      if (!IsLowercaseAlphanumeric(c) && (c != '-') && (c != '_') &&
          c != ':' & c != '/' & c != '.') {
        LOG(ERROR) << entry << " is not in allowed character range.";
        return false;
      }
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  // Check the entries are in the correct form.
