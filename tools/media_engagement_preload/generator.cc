...
std::string GenerateToProto(const std::set<std::string>& entries) {
  ...
  std::vector<uint8_t> trie_bytes = new_writer.bytes();

  chrome_browser_media::PreloadedData message;
  message.set_huffman_tree(
      std::string(huffman_tree.begin(), huffman_tree.end()));
  message.set_trie_data(std::string(trie_bytes.begin(), trie_bytes.end()));
