...
class BundleData {
 public:
  ...
  std::string& test_application_name() { return test_application_name_; }
  const std::string& test_application_name() const {
    return test_application_name_;
  }
 ...
 private:
  ...
  // Each unit test or ui test target must have a test application target, and
  // this value corresponds to the target name. This is only used to generate
  // the Xcode project when using --ide=xcode.
  std::string test_application_name_;
