...
// Tests that a target is marked as affected if the target is an action and its
// output files are modified.
TEST_F(AnalyzerTest, TargetRefersToActionOutputs) {
  Target* t = MakeTarget("//dir", "target_name");
  builder_.ItemDefined(std::unique_ptr<Item>(t));
  t->set_output_type(Target::ACTION);
  RunBasicTest(
      R"({
       "files": [ "//out/debug/output.txt" ],
       "additional_compile_targets": [ "all" ],
       "test_targets": [ "//dir:target_name" ]
       })",
      "{"
...
// Tests that if a target is marked as affected, then it propogates to dependent
// test_targets.
TEST_F(AnalyzerTest, AffectedTargetPropogatesToDependentTargets) {
...
// Tests that if a config is marked as affected, then it propogates to dependent
// test_targets.
TEST_F(AnalyzerTest, AffectedConfigPropogatesToDependentTargets) {
...
// Tests that if toolchain is marked as affected, then it propogates to
// dependent test_targets.
TEST_F(AnalyzerTest, AffectedToolchainPropogatesToDependentTargets) {
...
// Tests that if a pool is marked as affected, then it propogates to dependent
// targets.
TEST_F(AnalyzerTest, AffectedPoolPropogatesToDependentTargets) {
  Target* t = MakeTarget("//dir", "target_name");
  t->set_output_type(Target::ACTION);
...
// Bails out early with "Found dependency (all)" if dot file is modified.
TEST_F(AnalyzerTest, DotFileWasModifiedAndCompilingAll) {
  Target* t = MakeTarget("//dir", "target_name");
  builder_.ItemDefined(std::unique_ptr<Item>(t));
...
// Bails out early with "Found dependency (all)" if master build config file is
// modified.
TEST_F(AnalyzerTest, BuildConfigFileWasModifiedAndCompilingAll) {
  Target* t = MakeTarget("//dir", "target_name");
  builder_.ItemDefined(std::unique_ptr<Item>(t));
...
// Bails out early with "Found dependency (all)" if a build args dependency file
// is modified.
TEST_F(AnalyzerTest, BuildArgsDependencyFileWasModifiedAndCompilingAll) {
  Target* t = MakeTarget("//dir", "target_name");
  builder_.ItemDefined(std::unique_ptr<Item>(t));
