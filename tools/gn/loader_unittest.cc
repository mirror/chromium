...
bool ItemContainsRelatedFile(const Item* item, const SourceFile& source_file) {
  const auto& related_files = item->related_files();
  return related_files.end() != related_files.find(source_file);
}
...
TEST_F(LoaderTest, RelatedFilesAreCollected) {
  SourceFile build_config("//build/config/BUILDCONFIG.gn");
  SourceFile root_build("//BUILD.gn");
  build_settings_.set_build_config_file(build_config);
  ...
  EXPECT_TRUE(ItemContainsRelatedFile(items[0], root_build));
  EXPECT_TRUE(items[1]->AsConfig());
  EXPECT_TRUE(ItemContainsRelatedFile(items[1], root_build));
  EXPECT_TRUE(items[2]->AsToolchain());
  EXPECT_TRUE(ItemContainsRelatedFile(items[2], root_build));
  EXPECT_TRUE(items[3]->AsPool());
  EXPECT_TRUE(ItemContainsRelatedFile(items[3], root_build));
