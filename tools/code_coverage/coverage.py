...
class _CoverageSummary(object):
  ...
  def AddSummary(self, another_summary):
    ...
    for feature in self._summary:
      self._summary[feature]['total'] += another_summary.Get()[feature]['total']
      self._summary[feature]['covered'] += another_summary.Get()[feature][
          'covered']
...
class _DirectoryCoverageReportHtmlGenerator(object):
  ...
  def WriteHtmlCoverageReport(self, output_path):
    ...
    dir_entries.sort(key=lambda entry: entry['name'])
