...
def _CreateCoverageProfileDataForTargets(targets, test_commands,
  ...
  profraw_file_paths = _GetProfileRawDataPathsByRunningTests(test_commands)
...
def _GetProfileRawDataPathsByRunningTests(test_commands):
  ...
  """
  def _RunTestTarget(test_command, expected_profraw_file_path):
    ...
    Args:
      ...
      expected_profraw_file_path: A path where the profraw data should be
                                  generated.
    """
    # This test argument is only required for test targets that run in parallel
    # by default, pass this arugment to the command anyway since it will be
  ...
  threads = []
  for test_command, profraw_file_path in zip(test_commands, profraw_file_paths):
    thread = threading.Thread(target=_RunTestTarget,
                              args=(test_command, profraw_file_path))
    thread.start()
