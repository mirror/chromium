...
"""Script to generate Clang source based code coverage report.
  ...
  Example usages:
  tools/code_coverage/coverage.py url_unittests -b out/Coverage/ -o out/report
  # Generate code coverage report for url_unittests and all generated artifacts
  # are stored in out/report.

  tools/code_coverage/coverage.py url_unittests -b out/Coverage -o out/report
  -c="--gtest_filter=URLParser.PathURL"
  # Only run test URLParser.PathURL.
...
# Build args required for generating code coverage data.
CLANG_COVERAGE_BUILD_ARGS = {'use_clang_coverage': 'true',
                             'is_component_build': 'false'}

# List of GN args that are incompatible with |CLANG_COVERAGE_BUILD_ARGS|.
INCOMPATIBLE_BUILD_ARGS = {'is_asan': 'true',
                           'is_msan': 'true',
...
def _GenerateLineByLineFileCoverageInHtml(targets, profdata_file_path):
  ...
  binary_paths = [_GetBinaryPath(target) for target in targets]
...
def _CreateCoverageProfileDataForTargets(targets, jobs_count=None,
                                         test_command=None):
  ...
  Args:
    ...
    test_command: command arguments to be passed to run tests.
...
def _GetProfileRawDataPathsByRunningTargets(targets, test_command=None):
  """Runs targets and returns the relative paths to the profraw data files.
  ...
  """
  def _RunTarget(target, expected_profraw_file_path, test_command=None):
    """Run test targets.
    ...
    Args:
      ...
      test_command: command arguments to be passed to run tests.
    """
    cmd = [_GetBinaryPath(target), '--test-launcher-jobs=1', test_command]
    print 'Running {} with command: {}'.format(target, ' '.join(cmd))
    _ = subprocess.check_output(cmd, env={'LLVM_PROFILE_FILE':
                                          expected_profraw_file_path})

  profraw_file_paths = []
  for target in targets:
    profraw_file_name = os.extsep.join([target, 'profraw'])
  ...
  threads = []
  for target, profraw_file_path in zip(targets, profraw_file_paths):
    thread = threading.Thread(target=_RunTarget,
                              args=(target, profraw_file_path, test_command))
    thread.start()
...
def _GetBinaryPath(target):
  """Returns a relative path to the generated binary of a test target.

  Args:
    target: The target to be tested.

  Returns:
    A relative path to the generated binary.
  """
  return os.path.join(BUILD_DIR, target)


def _ValidateBuildArgs():
  """Asserts that build args are valid for generating code coverage data."""
  build_args = _ParseArgsGnFile()

  if any(build_arg_key not in build_args or
         build_args[build_arg_key] != CLANG_COVERAGE_BUILD_ARGS[build_arg_key]
         for build_arg_key in CLANG_COVERAGE_BUILD_ARGS):
    assert False, ('Build args {} are required in args.gn.').format(
        CLANG_COVERAGE_BUILD_ARGS)

  for build_arg_key in INCOMPATIBLE_BUILD_ARGS:
    if (build_arg_key in build_args and
        build_args[build_arg_key] == INCOMPATIBLE_BUILD_ARGS[build_arg_key]):
      assert False, ('Build arg \'{} = {}\' is incompatible with code '
                     'coverage build configuration.').format(
                         build_arg_key, INCOMPATIBLE_BUILD_ARGS[build_arg_key])
...
def _ParseCommandArguments():
  ...
  arg_parser.add_argument('-c', '--test-command', type=str,
                          help='Command arguments, such as --gtest_filter, '
                               'that will be passed to run the tests. The '
                               'arguments should only be specified by '
                               'following a \'=\', for example, '
                               '-c "--gtest_filter=a.b" won\'t work, it needs '
                               'to be -c="--gtest_filter=a.b".')
...
def Main():
  ...
  profdata_file_path = _CreateCoverageProfileDataForTargets(
      args.targets, args.jobs, args.test_command)
