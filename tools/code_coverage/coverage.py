...
"""Script to generate Clang source based code coverage report.
  ...
  -o out/report -c 'out/Coverage/crypto_unittests'
...
def _GetProfileRawDataPathsByExecutingCommands(targets, commands):
  ...
  def _ExecuteCommand(target, command):
    ...
    binary_name = os.path.basename(_GetBinaryPath(command))
    expected_profraw_file_name = os.extsep.join([binary_name, '%p',
                                                 PROFRAW_FILE_EXTENSION])
    expected_profraw_file_path = os.path.join(OUTPUT_DIR,
                                              expected_profraw_file_name)

    print('Running tests with command: %s' % command)
    subprocess.check_call(command.split(), env={'LLVM_PROFILE_FILE':
  ...
  # Assert one command generates at least one profraw data file.
  for command in commands:
    binary_name = os.path.basename(_GetBinaryPath(command))
    assert any(os.path.basename(profraw_file).startswith(binary_name) for
               profraw_file in profraw_file_paths), ('Command: %s failed to '
                                                     'generate any profraw '
                                                     'data file, please make '
                                                     'sure the binary is '
...
def _ParseBuildDirectoryFromCommand(command):
  """Returns a relative path to the build directory.

  Args:
    command: A command used to run a target.

  Returns:
    A relative path to the build directory.
  """
...
def _ParseCommandArguments():
  ...
  arg_parser.add_argument('-c', '--command', action='append',
                          ...
                          help='Commands used to run test targets, one test '
                               'target needs one and only one command.')
...
def Main():
  ...
  global BUILD_DIR
  BUILD_DIR = _ParseBuildDirectoryFromCommand(args.command[0])
