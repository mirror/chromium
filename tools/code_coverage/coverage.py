...
def _GeneratePerFileCoverageSummary(binary_paths, profdata_file_path, filters):
  ...
  TODO(crbug.com/): Change to a more robust code coverage summary export
  solution when the bug is fixed.
  ...
  object, however, due to the size and complexity of Chrome, "llvm-cov report"
  ...
  std_out = subprocess.check_output(subprocess_cmd)
  ...
  while current_line_num < len(std_out_by_lines) - 3:
    current_line = std_out_by_lines[current_line_num]
    if not current_line.strip():
      # Exits the loop when it's able to reach to the 'Files which contain no
      # functions' section
      break
