...
class _CoverageSummary(object):
  ...
  def GetSummaryDictionary(self):
    """Returns summary as a dictionary."""
    return self._summary

  def AddSummary(self, another_summary):
    """Adds another summary to this one element-wise."""
    another_summary_dict = another_summary.GetSummaryDictionary()
    for index in ('regions', 'functions', 'lines'):
      self._summary[index]['total'] += another_summary_dict[index]['total']
      self._summary[index]['covered'] += another_summary_dict[index]['covered']
...
class _DirectoryCoverageReportHtmlGenerator(object):
  ...
  def AddLinkToAnotherReport(self, html_report_path, name, summary):
    ...
    """
    summary_dict = summary.GetSummaryDictionary()
    table_entry = {'href': html_report_path, 'name': name}
    for index in ('regions', 'functions', 'lines'):
      percentage = round(
          float(summary_dict[index]['covered']) / summary_dict[index]['total'] *
          100, 2)
      color_class = self._GetColorClass(percentage)
      table_entry[index] = {
          'total': summary_dict[index]['total'],
          'covered': summary_dict[index]['covered'],
  ...
  def WriteHtmlCoverageReport(self, output_path):
    ...
    """
    dir_entries = [
        entry for entry in self._table_entries
        if os.path.basename(entry['href']) ==
        DIRECTORY_COVERAGE_HTML_REPORT_NAME
    ]
    file_entries = [
        entry for entry in self._table_entries if entry not in dir_entries
    ]

    file_entries.sort(
        key=
        lambda entry: float(entry['lines']['covered']) / entry['lines']['total']
    )
    dir_entries.sort(
        key=
        lambda entry: float(entry['lines']['covered']) / entry['lines']['total']
    ...
    html_table = self._table_template.render(
        dir_entries=dir_entries, file_entries=file_entries)
...
def _OverwriteHtmlReportsIndexFile():
  ...
  content = ("""
    ...
    </html>""" % _GetCoverageHtmlReportPath(SRC_ROOT_PATH))
...
def _GeneratePerFileCoverageSummary(binary_paths, profdata_file_path, filters):
  ...
  json_output = {
      'data': [{
          'files': [{
              'filename': '/Users/liaoyuke/chromium/src/url/url_parse_file.cc',
              'summary': {
                  'regions': {
                      'count': 14,
                      'covered': 5
              ...
              'filename':
                  '/Users/liaoyuke/chromium/src/url/url_parse_unittest.cc',
              'summary': {
                  'regions': {
                      'count': 10,
                      'covered': 5
...
def Main():
  ...
  profdata_file_path = '/Users/liaoyuke/chromium/src/out/yuke/coverage.profdata'
