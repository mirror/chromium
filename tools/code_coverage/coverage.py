...
"""Script to generate Clang source based code coverage report.
  ...
  -b out/Coverage -o out/report -c 'crypto_unittests'
  -c 'url_unittests --gtest_filter=URLParser.PathURL'
...
def _ExecuteCommand(target, command):
  ...
  if _IsTargetGTestTarget(target):
    ...
    command += single_launcher_jobs_argument
  ...
  rebased_command = _RebaseCommand(command)

  test_results_file_name = os.extsep.join([target + '_test_results', 'txt'])
  test_results_file_path = os.path.join(OUTPUT_DIR, test_results_file_name)

  print('Running tests with command: %s, test results output is streamed to %s'
        % (rebased_command, test_results_file_path))
  output = subprocess.check_output(rebased_command.split(),
                                   env={'LLVM_PROFILE_FILE':
                                        expected_profraw_file_path})
  with open(test_results_file_path, 'w') as output_file:
    output_file.write(output)


def _RebaseCommand(command):
  """Rebases the command to be relative to the root of the checkout.

  Args:
    command: A command used to run the target.

  Returns:
    An equal command that is rebased to be relative to the root of the checkout.
  """
  binary_path = command.split()[0]
  binary_abspath = os.path.join(os.path.abspath(BUILD_DIR), binary_path)
  rebased_binary_path = os.path.relpath(binary_abspath, SRC_ROOT_PATH)
  return command.replace(binary_path, rebased_binary_path)
...
def _GetBinaryPath(command):
  ...
  Returns:
    A path to the binary relative to the build directory.
  """
  return os.path.join(BUILD_DIR, command.split()[0])
...
def _ParseCommandArguments():
  ...
  arg_parser.add_argument('-c', '--command', action='append',
                          ...
                          help='Commands used to run test targets, one test '
                               ...
                               'specifying commands, the given path to the '
                               'binary should be relative to the build '
                               'directory.')
