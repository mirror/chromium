...
def _GenerateLineByLineFileCoverageInHtml(binary_paths, profdata_file_path,
  ...
  Args:
    ...
    include_paths: A list of directories and files to get coverage for.
  """
  ...
  subprocess_cmd.extend(include_paths)
...
def _AssertPathsExist(paths):
  ...
  for path in paths:
    ...
    assert os.path.exists(abspath), (('Path: "%s" doesn\'t exist.\nA valid '
                                      'path must exist and be relative to the '
                                      'root of the checkout.') % path)
...
def _ParseCommandArguments():
  ...
  arg_parser.add_argument(
      ...
      action='append',
      required=True,
      help='Directories to get code coverage for, the paths need to be '
      'relative to the root of the checkout and all files under the '
...
def Main():
  ...
  if args.include:
    _AssertPathsExist(args.include)
