...
class SymbolOffsetProcessor(object):
  ...
  This class is used to manipulate symbol offsets in binaries, and translating
  between offsets in a binary and the binariy's symbols and their offsets. In
  the function names below, "dump" is used to refer to arbitrary offsets in a
  binary (eg, from a profiling run), while "offset" refers to a symbol offset.

  This class manages expensive operations like extracting symbols, so that
  higher-level operations can be done in different order without the caller
  managing all the state.
  """

  def __init__(self, binary_filename):
    self._library_filename = binary_filename
    self._symbol_infos = None
    self._name_to_symbol = None
    self._offset_to_primary = None

  def SymbolInfos(self):
    """The set of symbols associated with this processor's binary.
  ...
  def GetReachedOffsetsFromDump(self, dump):
    ...
    The dump contains offsets into a .text section. This finds the symbols
    ...
    returns only the offset list. See XXX for computing more information about
    symbols.

    Args:
     dump: ([int]) Dump offsets, for example as returned by MergeDumps().
  ...
  def MatchSymbolNames(self, symbol_names):
    ...
    Args:
      symbol_names (iter str) List of symbol names.

    Returns:
      [symbol_extractor.SymbolInfo] Symbols in this binary matching the names.
    """
    our_symbol_names = set(s.name for s in self.SymbolInfo)
...
def GetReachedOffsetsFromDumpFiles(dump_filenames, library_filename):
  ...
  Args:
    dump_filenames ([str]) A list of dump filenames.
