...
class OffsetOrderfileGenerator(object):
  ...
  the same build.
  """
  def __init__(self, symbol_offset_proccessor, obj_dir):
    """Initialize the generator.

    Args:
      symbol_offset_proccessor (process_profiles.SymbolOffsetProcessor).
      obj_dir (str) the object directory root, eg out/Instrumented/obj.
    """
    self._offset_to_symbols = symbol_offset_proccessor.OffsetToSymbolsMap()
    if obj_dir is not None:
      self._symbol_to_sections = self._GetSymbolToSectionsMap(obj_dir)
    else:
      # Testing use case.
      self._symbol_to_sections = None
    ...
    for offset in offsets:
      try:
        ...
        for symbol_info in symbol_infos:
          if symbol_info.name in self._symbol_to_sections:
            sections = self._symbol_to_sections[symbol_info.name]
  ...
  @classmethod
  def _GetObjectFileNames(cls, obj_dir):
    """Scans obj_dir recursively for .o files."""
    obj_files = []
  ...
  @classmethod
  def _AllSymbolInfos(cls, object_filenames):
    """Returns a list of SymbolInfo from an iterable of filenames."""
    pool = multiprocessing.Pool()
    # Hopefully the object files are in the page cache at this step, so IO
    # should not be a problem (hence no concurrency limit on the pool).
  ...
  def _SameCtorOrDtorNames(cls, symbol1, symbol2):
    ...
    (section 5.1.4.3):
    https://refspecs.linuxbase.org/cxxabi-1.83.html#mangling-special To avoid
    fully parsing all mangled symbols, a heuristic is used with c++filt.
...
def main():
  ...
  generator = OffsetOrderfileGenerator(
      process_profiles.SymbolOffsetProcessor(args.native_library), obj_dir)
