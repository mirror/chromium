...
// Data requires to log reached offsets.
struct LogData {
...
void ResetForTesting() {
  Disable();
  g_data_index = 0;
  memset(reinterpret_cast<uint32_t*>(g_data[0].offsets), 0,
         sizeof(uint32_t) * kBitfieldSize);
  memset(reinterpret_cast<uint32_t*>(g_data[0].ordered_offsets), 0,
         sizeof(uint32_t) * kMaxElements);
  g_data[0].index.store(0);
}
