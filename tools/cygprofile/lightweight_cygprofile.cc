...
// TODO(lizeb): Shrink these constants once post-inlining profiling is in.
constexpr size_t kMaxTextSizeInBytes = 280000000;
constexpr size_t kBitfieldSize = kMaxTextSizeInBytes / (4 * 32);
constexpr size_t kMaxElements = 1e6;

// |RecordAddress()| adds an element to a concurrent, ordered offset set, that
// is element insertion ordering is recorded.
//
// Ordering:
...
// - Some insertions at the end of colleciton may be lost.
...
size_t g_ordered_offsets[kMaxElements];
// Next free slot.
...
// Records that |address| has been reaches, if recording is enabled.
void RecordAddress(size_t address) {
  auto* offsets = g_enabled_and_offsets.load(std::memory_order_relaxed);
  if (!offsets)
    return;

  if (UNLIKELY(address < kStartOfText || address > kEndOfText)) {
    Disable();
  ...
  g_ordered_offsets[ordered_offsets_index] = offset;
}
...
void StopAndDumpToFile(const base::FilePath& path) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    auto offset = g_ordered_offsets[i];
    if (!offset)
      continue;
...
__attribute__((__always_inline__)) void mcount() {
  ...
  RecordAddress(reinterpret_cast<size_t>(return_address));
}
