...
"""From ordered native library, puts ordered code in the middle of code.

Putting ordered code, this is a superset of frequently called code in the
middle of the binary reduces the number of relative jumps with an offset >16MB,
which require a thunk when using the THUMB-2 instruction set.

This is done the following way:
- Look for the current ordering in an ordered libchrome.socketserver
- Produce a complete orderfile (with all the symbols), with the ordered
...
def _Index(haystack, needle):
  """Returns the index of the first |needle| in |haystack|, or -1."""
...
def _ExtractAndProcessSymbols(native_library_filename):
  ...
  real_symbols = [s for s in symbol_infos
                  if not s.name.startswith('__ThumbV7PILongThunk__')]
...
def _ReorderSymbols(symbol_infos):
  ...
  start_of_ordered_index = _Index(
      symbols, 'dummy_function_start_of_ordered_text')
  end_of_ordered_index = _Index(symbols, 'dummy_function_end_of_ordered_text')
