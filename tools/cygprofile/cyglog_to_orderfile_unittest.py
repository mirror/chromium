...
class TestGenerator(cyglog_to_orderfile.OffsetOrderfileGenerator):
  def __init__(self, symbol_infos, symbol_to_sections=None):
    # We replace our superclass initialization with our own.
...
END""".split('\n')
  ...
  def testOutputOrderfile(self):
    ...
    symbols = [SymbolWithSection('Symbol', 0x10, 0x13, 'dummy'),
               SymbolWithSection('Symbol2', 0x12, 0x13, 'dummy'),
               SymbolWithSection('Symbol3', 0x16, 0x13, 'dummy'),
               SymbolWithSection('Symbol3.2', 0x16, 0x13, 'dummy')]
