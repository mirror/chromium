...
class PhasedAnalyzer(object):
  ...
  def ComputeStability(self):
    ...
    This computes the difference in size of symbols between the union and
    instersection of all orderfile phases. Intuitively if this difference is not
    too large it means that the profiling phases are stable with respect to the
    code they cover.
