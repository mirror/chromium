...
class OrderfileGenerator(object):
  ...
  def __init__(self, options, orderfile_updater_class):
    ...
    self._lightweight_instrumentation = (
  ...
  def _RunCygprofileUnitTests(self):
    ...
    # TODO(lizeb): Fix this.
    if self._lightweight_instrumentation:
      return
  ...
  def _GenerateAndProcessProfile(self):
    ...
    try:
      ...
      if self._lightweight_instrumentation:
        process_profiles.MergeDumpsAndWriteSymbolOffsets(
    ...
    try:
      command_args = [
          ...
          '--output-filename=' + self._GetUnpatchedOrderfileFilename()]
      if self._lightweight_instrumentation:
    ...
    if self._options.profile:
      try:
        ...
        self._compiler.CompileChromeApk(
            True, lightweight_instrumentation=self._lightweight_instrumentation)
