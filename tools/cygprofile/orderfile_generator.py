...
Example usage:
  clank/tools/orderfile_generator.py -l 20 -j 1000 --use-goma --target-arch=arm
"""
...
sys.path.append(os.path.join(_SCRIPT_PATH, '..', '..', 'tools', 'cygprofile'))
import cygprofile_utils
import profile_android_startup

sys.path.append(os.path.join(_SCRIPT_PATH, '..', '..', 'build', 'android'))
import devil_chromium
from pylib import constants

sys.path.append(os.path.join(_SCRIPT_PATH, 'gardening'))
import git_utils
...
class CommandError(Exception):
  """An error used to indicate that a dispatched shell command exited with a
  non-zero status.
  """
...
class StepRecorder(object):
  ...
  def EndStep(self):
    """Records successful completion of the current step.  This is optional if
    the step is immediately followed by another BeginStep.
    """
...
class ClankCompiler(object):
  ...
  def __init__(self, out_dir, step_recorder, arch, jobs,
               max_load, use_goma, goma_dir):
    self._arch = arch
...
class OrderfileGenerator(object):
  ...
  _CLOUD_STORAGE_BUCKET = 'REDACTED'
  _CLOUD_STORAGE_BUCKET_FOR_DEBUG = 'REDACTED'
  ...
  def _RunCygprofileUnitTests(self):
    ...
    tools_compiler = ClankCompiler(os.path.dirname(constants.GetOutDirectory()),
    ...
    try:
      ...
      with open(self._MERGED_CYGLOG_FILENAME, 'w') as merged_cyglog:
        self._step_recorder.RunCommand([self._MERGE_TRACES_SCRIPT] + files,
            constants.DIR_SOURCE_ROOT, stdout=merged_cyglog)
    except CommandError:
      for f in files:
  ...
  def _UploadToCloudStorage(self, file_name):
    """Uploads the given file."""
    extension = _GetFileExtension(file_name)
    ...
    print 'Download: https://sandbox.google.com/storage/%s/%s' % (
        self._CLOUD_STORAGE_BUCKET, _GenerateHash(file_name))
  ...
  def _SaveFileLocally(self, file_name, file_sha1):
    ...
    print 'File: %s, saved in: %s, sha1sum: %s' % (file_name,
        self._DIRECTORY_FOR_DEBUG_FILES, file_sha1)

  def _SaveForDebugging(self, file_name):
    ...
    if not self._options.buildbot:
      ...
      print 'Uploading file for debugging: ' + file_name
      self._step_recorder.RunCommand([self._UPLOAD_TO_CLOUD_COMMAND,
          '--bucket', self._CLOUD_STORAGE_BUCKET_FOR_DEBUG, file_name])
      print ('Uploaded to: https://sandbox.google.com/storage/%s/%s' %
          (self._CLOUD_STORAGE_BUCKET_FOR_DEBUG, file_sha1))
  ...
  def _SaveForDebuggingWithOverwrite(self, file_name):
    ...
    if not self._options.buildbot:
      ...
      self._step_recorder.RunCommand(['gsutil.py', 'cp', file_name,
          'gs://' + upload_location])
  ...
  def _MaybeArchiveOrderfile(self, filename):
    """
    In buildbot configuration, uploads the generated orderfile to Google Cloud
    Storage.
    """
    ...
    if self._options.buildbot:
      ...
      self._UploadToCloudStorage(filename)

  def _CreateBranch(self):
    """ If running on a buildbot, create a branch for the work. """
    logging.info('Creating branch')
    git_utils.StartNewGitBranchFromOrigin('orderfile-update',
                                          'origin/' + self._options.branch,
                                          self._CLANK_REPO, True)

  def _GetHashFilePathAndContents(self, base_file):
    """Get the name and content of the hash file created from uploading the
    given file.
  ...
  def _CheckInHashes(self, profile_uploaded, orderfile_uploaded):
    """If running on a buildbot, and succeeded, check in any hash
    files which were generated.

    Rather than doing this after each hash is generated, it is preferable to
    check in all generated hash files at once.

    Args:
      profile_uploaded: Whether a new profile (unpatched orderfile) was
          uploaded and the hash should be checked in (on buildbots).
    """
    self._step_recorder.BeginStep('Pushing hashes')
    commit_msg_list = ['Update orderfile.']
    file_list = []
    if profile_uploaded:
      (file_name, sha1) = self._GetHashFilePathAndContents(
          self._GetUnpatchedOrderfileFilename())
      file_list.append(file_name)
      commit_msg_list.append('Profile %s: %s' % (
          os.path.basename(file_name), sha1))
    if orderfile_uploaded:
      (file_name, sha1) = self._GetHashFilePathAndContents(
          self._GetPathToOrderfile())
      file_list.append(file_name)
      commit_msg_list.append('Profile %s: %s' % (
    ...
    if len(file_list) > 0:
      try:
        self._CreateBranch()
        git_utils.GitConfig(self._CLANK_REPO)
        netrc_path = os.path.dirname(os.path.abspath(os.path.expanduser(
            self._options.netrc)))
        if self._options.branch == 'master':
          refspec = 'HEAD:refs/heads/master'
        else:
          refspec = self._options.master
        logging.getLogger().setLevel(logging.DEBUG)
        git_utils.PushChangesToGit(file_list, self._CLANK_REPO,
                                   commit_msg_list, refspec, netrc_path)
        logging.getLogger().setLevel(logging.INFO)
      finally:
        self._CleanupBranch()

  def _CleanupBranch(self):
    """If running on a buildbot, clean up the temporary branch."""
    logging.info('Cleaning up branch')
    git_utils.Cleanup('orderfile-update', self._CLANK_REPO)
  ...
  def Generate(self):
    ...
    if (self._options.buildbot and self._options.netrc
      ...
      self._CheckInHashes(profile_uploaded, orderfile_uploaded)
...
def main(argv):
  ...
  parser.add_option('--skip-profile', action='store_false', dest='profile',
      default=True,
      help='Don\'t generate a profile on the device.  Only patch from the '
           'existing profile.')
  parser.add_option('--skip-patch', action='store_false', dest='patch',
      default=True,
      help='Only generate the raw (unpatched) orderfile, don\'t patch it.')
  parser.add_option('--netrc', action='store',
      help='A custom .netrc file to use for git checkin.  Only used on bots.')
  parser.add_option('--branch', action='store', default='master',
  ...
  parser.add_option('-j', '--jobs', action='store', default=20,
      help='Number of jobs to use for compilation.')
  parser.add_option('-l', '--max-load', action='store', default=4,
      help='Max cpu load.')
  parser.add_option('--goma-dir', help='GOMA directory.')
  parser.add_option('--use-goma', action='store_true', help='Enable GOMA.',
                    default=False)
  ...
  orderfile_generator = OrderfileGenerator(options)
  success = True
  ...
  try:
    if options.verify:
      ...
      success = orderfile_generator.Generate()
