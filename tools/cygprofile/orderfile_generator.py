...
class OrderfileGenerator(object):
  ...
  def _CreateBranch(self):
    """If running on a buildbot, creates a branch for the work. """
  ...
  def _CheckInHashes(self, profile_uploaded, orderfile_uploaded):
    ...
    Args:
      profile_uploaded: (bool) Whether a new profile (unpatched orderfile) was
          uploaded and the hash should be checked in (on buildbots).
      orderfile_uploaded: (bool) Whether a new orderfile was uploaded.
    """
    raise NotImplementedError
...
def Go(options, generator_class):
  """Creates an oderfile.

  Args:
    options: As returned from optparse.OptionParser.parse_args()
    generator_class: (OrderfileGenerator) child class of OrderfileGenerator.
  ...
  orderfile_generator = generator_class(options)
  try:
    if options.verify:
...
def main(argv):
  ...
  return 0 if Go(options, OrderfileGenerator) else 1
