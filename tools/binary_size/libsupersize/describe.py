...
class DescriberCsv(Describer):
  def __init__(self, verbose=False):
    ...
    self.verbose = verbose
    self.renderer = _CsvRenderer()

  def _DescribeSectionSizes(self, section_sizes):
    (_, relevant_section_names) = _GetSectionSizeInfo(section_sizes)

    if self.verbose:
      relevant_set = set(relevant_section_names)
      section_names = sorted(section_sizes.iterkeys())
      yield self.renderer(['Name', 'Size', 'IsRelevant'])
      for name in section_names:
        size = section_sizes[name]
        yield self.renderer([name, size, int(name in relevant_set)])
    else:
      yield self.renderer(['Name', 'Size'])
