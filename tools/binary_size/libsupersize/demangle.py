...
def DemangleSetsInDicts(key_to_names, tool_prefix):
  ...
  """
  all_names = []
  for _, names in key_to_names.iteritems():
    all_names += [n for n in names if n.startswith('_Z')]
  if not all_names:
    return key_to_names
...
def DemangleKeysAndMergeLists(name_to_list, tool_prefix):
  """|name_to_list| is a dict from mangled name to a list. Demangles keys and
     returns a new dict with the new keys, and list values possibly merged
     (unsorted with possible duplicates).
  """
  keys = [key for key, _ in name_to_list.iteritems()
