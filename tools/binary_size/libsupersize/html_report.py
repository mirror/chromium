...
def _AddSymbolIntoFileNode(node, symbol_type, symbol_name, symbol_size,
  ...
  if not symbol_name or symbol_size >= symbol_size_threshold:
...
def _MakeCompactTree(symbols, symbol_size_threshold):
  result = {
      _NODE_NAME_KEY: '/',
      _NODE_CHILDREN_KEY: {},
      _NODE_TYPE_KEY: 'p',
  ...
  if no_path_bucket and symbol_size_threshold == 0:
...
def AddArguments(parser):
  ...
  parser.add_argument('--include-bss', action='store_true',
                      help='Include symbols from .bss (which consume no real '
                           'space)')
  parser.add_argument('--symbol-size-threshold', type=float, default=1024,
...
def Run(args, parser):
  ...
  tree_root = _MakeCompactTree(symbols, args.symbol_size_threshold)
