...
_BulkObjectFileAnalyzerSlave:
  Recieves IPCs and delegates logic to BulkObjectFileAnalyzerWorker.
  Runs BulkObjectFileAnalyzerWorker on a background thread in order to stay
  responsive to IPCs.
...
BulkObjectFileAnalyzer:
  ...
  * AnalyzeStringLiterals: Must be run after AnalyzePaths() has completed.
    Extracts string literals from .o files, and then locates them witin the
    "** merge strings" sections within then ELF's .rodata section.
...
def _LookupStringSectionPositions(target, tool_prefix, output_directory):
  ...
  for line in lines:
    ...
    # The presence of L"asdf" strings make a str4 section (unsure of str8).
