...
def _LookupStringSectionPositions(target, tool_prefix, output_directory):
  """Returns a list of lists of (offset, size) tuples for string sections."""
  ...
  for line in lines:
    ...
    # The presense of L"asdf" strings make a str4 section (unsure of str8).
    # When multiple sections exists, nm gives us no indication as to which
...
def LookupElfRodataInfo(elf_path, tool_prefix):
  ...
  raise AssertionError('No .rodata for command: %r', args)
...
def _ReadStringSections(target, output_directory, positions_by_path):
  ...
  if is_archive:
    # First object file is the third chunk.
...
def _IterStringLiterals(path, addresses, string_sections_by_path):
  ...
  if not obj_sections:
    # This either means there is an address for a symbol which is not actually a
    # string literal, or string sections is missing a section.
...
def _ResolveStringPieces(encoded_string_addresses_by_path, string_data,
  ...
  for path, object_addresses in string_addresses_by_path.iteritems():
    for value in _IterStringLiterals(
      ...
      # Searching first for a fully-matching string, and then for a substring
      # match drops .rodata padding 143kb -> 74kb.
      # We could probably do even better by finding *all* matches for a string,
      # but for small strings the number is huge.kk
      prefixed_value = '\0' + value
      for target_dict, data in itertools.izip(ret, string_data):
        offset = data.find(prefixed_value)
        if offset != -1:
          offset += 1  # Unprefix.
          break
      else:
        for target_dict, data in itertools.izip(ret, string_data):
          ...
          if offset != -1:
            break
      # Missing strings happen when optimization makes them unused.
      if offset != -1:
...
def _RunNmOnIntermediates(target, tool_prefix, output_directory):
  ...
  while path:
    ...
    string_addresses, symbol_names = _ParseOneObjectFileOutput(lines)
...
class _BulkObjectFileAnalyzerWorker(object):
  ...
  def AnalyzeStringLiterals(self, elf_path, elf_string_positions):
    """Returns (offset, address, size) for the .rodata section."""
...
class _BulkObjectFileAnalyzerMaster(object):
  ...
  def AnalyzeStringLiterals(self, elf_path, string_positions):
    self._pipe.send((_MSG_ANALYZE_STRINGS,
                     elf_path, string_positions))

  def GetSymbolNames(self):
    self._pipe.send((_MSG_GET_SYMBOL_NAMES,))
    logging.debug('Waiting for bulk nm to finish')
  ...
  def GetStringPositions(self):
    ...
    logging.debug('Waiting for string extraction to finish')
    self._pipe.recv()  # None
...
class _BulkObjectFileAnalyzerSlave(object):
  """The sub-process entry point."""
  def __init__(self, worker_analyzer, pipe):
    self._worker_analyzer = worker_analyzer
    self._pipe = pipe
    # Do work on a thread so that GetSymbolNames() can be called while
    # AnalyzeStringLiterals() is still in progress.
    self._thread_pool = multiprocessing.dummy.Pool(1)
    self._symbol_names_ready = False

  def _FlushThreadPool(self):
    # Submit a no-op job to the single-thread pool and wait for it to run.
    self._thread_pool.apply_async(lambda: 0).get()
    ...
    try:
      while True:
        ...
        if message[0] == _MSG_ANALYZE_PATHS:
          assert not self._symbol_names_ready, (
              'Cannot call AnalyzePaths() after AnalyzeStringLiterals()s.')
          args = (message[1].split('\x01'),)
          self._thread_pool.apply_async(
              self._worker_analyzer.AnalyzePaths, args=args)
        elif message[0] == _MSG_ANALYZE_STRINGS:
          self._FlushThreadPool()
          self._symbol_names_ready = True
          self._thread_pool.apply_async(
              self._worker_analyzer.AnalyzeStringLiterals, args=message[1:])
        elif message[0] == _MSG_GET_SYMBOL_NAMES:
          if not self._symbol_names_ready:
            self._FlushThreadPool()
            self._symbol_names_ready = True
...
def main():
  ...
  print 'Found %d names' % len(names_to_paths)
  if args.show_names:
  ...
  if args.elf_file:
    ...
    print 'Found %d string literals' % sum(
    ...
    if args.show_strings:
      ...
      for path, positions in positions_by_path.iteritems():
        ...
        print path + ':', [s if len(s) < 20 else s[:20] + '...' for s in strs]
