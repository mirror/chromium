...
class _Session(object):
  ...
  def _ReadStringLiterals(self, thing=None, elf_path=None):
    """Returns a list of string values for all literals given by |thing|.
    ...
    """
    if thing is None:
      thing = self._size_infos[-1].raw_symbols
    elif isinstance(thing, models.SizeInfo):
      thing = iter(thing.raw_symbols)
    elif isinstance(models.BaseSymbol):
    ...
    with open(elf_path) as f:
      ...
      for symbol in thing:
        if symbol.full_name.startswith(models.STRING_LITERAL_NAME_PREFIX):
          if symbol.address not in seen_addresses:
