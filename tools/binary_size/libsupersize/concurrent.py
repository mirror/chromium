...
# Pass parameters as globals speeds up BulkForkAndCall().
_fork_params = None
...
def BulkForkAndCall(func, arg_tuples):
  ...
  pool = _MakeProcessPool(list(arg_tuples))
...
def JoinEncodedDictOfLists(encoded_values):
  return ('\x01'.join(x[0] for x in encoded_values),
          '\x01'.join(x[1] for x in encoded_values))
