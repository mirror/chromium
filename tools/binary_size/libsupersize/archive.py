...
def _UnmangleRemainingSymbols(raw_symbols, tool_prefix):
  ...
  proc = subprocess.Popen([paths.GetCppFiltPath(tool_prefix)],
...
def BuildIdFromElf(elf_path, tool_prefix):
  args = [paths.GetReadElfPath(tool_prefix), '-n', elf_path]
...
def _ArchFromElf(elf_path, tool_prefix):
  args = [paths.GetReadElfPath(tool_prefix), '-h', elf_path]
  stdout = subprocess.check_output(args)
