...
def _ComputePakFileSymbols(pak_file, pak_data, res_info, symbols_by_name):
  total = 12 + 6  # Header size plus extra offset
  contents = data_pack.ReadDataPackFromString(pak_data)
  alias_map = data_pack.ComputeAliasMap(contents.resources)
  ...
  object_path = path_util.ToSrcRootRelative(pak_file.filename)
  for resource_id in sorted(contents.resources):
    if resource_id in alias_map:
      # 4 extra bytes of metadata (2 16-bit ints)
      size = 4
      name, _ = res_info[alias_map[resource_id]]
    else:
...
def _AddPakSymbolsAndSections(section_sizes, raw_symbols, apk_path,
  ...
  with zipfile.ZipFile(apk_path) as z:
    ...
    for pak_file in pak_files:
      _ComputePakFileSymbols(pak_file, z.read(pak_file), res_info, pak_symbols)

  for name, symbol in pak_symbols.iteritems():
    if symbol.section_name not in section_sizes:
      section_sizes[symbol.section_name] = 0
    section_sizes[symbol.section_name] += symbol.size
