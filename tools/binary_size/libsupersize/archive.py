...
def CreateSectionSizesAndSymbols(
  ...
  def merge_sections_and_symbols(sizes_and_symbols):
    new_sizes, new_symbols = sizes_and_symbols
    section_sizes.update(new_sizes)
    raw_symbols.extend(new_symbols)
  ...
  merge_sections_and_symbols(
      _ParseElfInfo(map_path, elf_path, tool_prefix, output_directory,
                    track_string_literals, elf_object_paths))
  ...
  if symbols_by_id:
    ...
    merge_sections_and_symbols(
        _ParsePakSymbols(object_paths, output_directory, symbols_by_id))
