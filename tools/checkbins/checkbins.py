...
def main(options, args):
  ...
  for file in os.listdir(directory):
    ...
    #   2) Be a 64-bit binary, in which case /SAFESEH isn't required
    #   3) Have a LOAD_CONFIG section containing a valid SEH table
    #
    ...
    # Notes:
    #    - checkbins.py only runs against release builds.
    #    - This ensures support for any CFG checks compiled into any DLLs
    #      mapped into the process (e.g. Microsoft system DLLs).  The EXE
    #      itself may not have CFG instrumentation compiled in.
    if (path.lower().endswith('.exe') and
        os.path.basename(path) not in EXCLUDED_CFG_EXES):
      if not (hasattr(pe, "DIRECTORY_ENTRY_LOAD_CONFIG")):
        success = False
        print("DIRECTORY_ENTRY_LOAD_CONFIG hasn't been parsed in script!  " \
              "INTERNAL FAILURE.")
      elif not (pe.OPTIONAL_HEADER.DllCharacteristics &
