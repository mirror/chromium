...
To use, run in a new shell (it blocks until the runs complete):

  tools/fuchsia/run-swarmed.py -t content_unittests --out-dir=out/fuch

The logs of the runs will be stored into results/ (or specify a results
directory with --results=some_dir). You can then do something like
`grep -L SUCCESS results/*` to find the tests that failed or otherwise process
the log files.
"""
...
def main():
  ...
  subprocess.check_call(['ninja', '-C', args.out_dir, args.test_name])
  ...
  print 'Uploading to isolate server, can take a while...'
  ...
  for i, json in enumerate(json_files):
    with open(os.path.join(args.results, str(i) + '.stdout.txt'), 'w') as f:
