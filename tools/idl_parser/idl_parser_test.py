...
class TestEnums(unittest.TestCase):
  ...
  def _ParseEnums(self, idl_text):
    parsed_wo_quotation = self.parser.ParseText(
        filename='', data=idl_text)
    self.assertEqual(1, len(parsed_wo_quotation.GetChildren()))
    return parsed_wo_quotation.GetChildren()[0]

  def testEnum(self):
    idl_text = 'enum MealType1 { "rice", "noodles", "other" };'
  ...
  def testErrorMissingName(self):
    ...
    self.assertEqual('Error', node.GetClass())
    error_msg = node.GetName()
    self.assertEqual('Enum missing name.', error_msg)
