...
# There are two kinds of ExtendedAttributes.
# One type can apply to types (It is said "applicable to types"),
# but the other cannot apply to types.
# This fuction is intended to divide ExtendedAttributes into those 2 types.
# For more details at
#    https://heycam.github.io/webidl/#extended-attributes-applicable-to-types
def DivideExtAttsIntoApplicableAndNonApplicable(extended_attribute_list):
  applicable_to_types = []
  non_applicable_to_types = []
  if not extended_attribute_list:
    return [[], []]
  for extended_attribute in extended_attribute_list.GetChildren():
    if extended_attribute.GetName() in _EXTENDED_ATTRIBUTES_APPLICABLE_TO_TYPES:
  ...
  def p_DictionaryMember(self, p):
    ...
    if len(p) > 6:
      ...
      applicable_to_types, non_applicable_to_types = \
          DivideExtAttsIntoApplicableAndNonApplicable(p[1])
