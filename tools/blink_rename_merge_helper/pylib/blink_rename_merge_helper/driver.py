...
def _install_merge_driver():
  ...
  with open(os.path.join('.git', 'info', 'attributes'), 'a') as attributes_file:
...
def _prepare_branch(current_branch,
  ...
  # knows about.
  # TODO(dcheng): Asking ninja individually about each file is pretty slow. Find
  # a better way to do this?
  changed_files = [
      f for f in changed_files
      if os.path.isfile(f) and _call_ninja_silently([
          '-C', build_dir, '-t', 'query', '%s^' % os.path.relpath(f, build_dir)
      ]) == 0
  ]
  ...
  targets = ['%s^' % os.path.relpath(f, build_dir) for f in changed_files]
...
def _update_branch(current_branch, tracking_branch, rebase=True):
  ...
  if not _commit_is_ancestor_of(_RENAME_COMMIT, tracking_branch):
    ...
    print '       branch first.'
    sys.exit(1)
  if tracking_branch != 'origin/master' and _commit_is_ancestor_of(
...
def run():
  ...
  parser.add_argument(
      '-C', metavar='DIR', help='Path to build directory.')
  parser.add_argument(
      '-j', metavar='N', help='Number of ninja jobs to run in parallel.')
  parser.add_argument(
      '--merge',
      action='store_true',
      help='Use merge instead of rebase to update the branch.')
  tool_mode = parser.add_mutually_exclusive_group(required=True)
  tool_mode.add_argument(
      '--install-merge-driver',
      action='store_true',
      help='Configures the merge driver. This only needs to be run once.')
  tool_mode.add_argument(
      '--prepare',
      action='store_true',
