...
class NetworkTrafficAnnotationChecker():
  ...
  def _FindPossibleBuildPath(self, auditor_path=None):
    """Tries to find a build directory. If |auditor_path| exists, it's directory
  ...
  def CheckFiles(self, file_paths=None, unlimited=False):
    ...
    Args:
      ...
      unlimited: bool Tells if errors and warnings should be capped at 5 or not.
    ...
    if not unlimited:
      if len(errors) > 5:
        errors = errors[:5]
      if len(warnings) + len(errors) > 5:
        warnings = warnings[:5-len(errors)]
...
def main():
  ...
  warnings, errors = checker.CheckFiles(unlimited=args.unlimited)
  if warnings:
    print("Warnings:\n\t%s" % "\n\t".join(warnings))
  if errors:
    print("Errors:\n\t%s" % "\n\t".join(errors))
