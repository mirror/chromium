...
#define LOGITEM(reason, item)                                                \
  LOG(ERROR) << "[" << reason << "] " << item.ids_count << ","               \
             << item.ids[0].hash_code << "," << item.ids[1].hash_code << "," \
             << item.ids[0].text << "," << item.ids[1].text;
...
void TrafficAnnotationIDChecker::CheckForInvalidRepeatedIDs(
  ...
  // type PARTIAL and owner of the first id is of type COMPLETING. (WHY?)
  for (AnnotationItem& item : annotations_) {
    if (item.ids_count == 2 &&
