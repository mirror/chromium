...
void TrafficAnnotationExporter::ExtractXMLItems(
...
std::string TrafficAnnotationExporter::GetXMLDifferences(
  ...
  // Get XML items, and puts their ids into sets.
  ...
  std::transform(old_items.begin(), old_items.end(),
                 std::inserter(old_set, old_set.end()),
  ...
  for (const std::string& id : old_set) {
