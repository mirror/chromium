...
bool CompareReportItems(const TrafficAnnotationExporter::ReportItem& a,
                        const TrafficAnnotationExporter::ReportItem& b) {
...
bool TrafficAnnotationExporter::UpdateAnnotationsXML(
  ...
  for (const auto& item : reserved_ids) {
    items.push_back(ReportItem({item.second, item.first, "", -1}));
  ...
  for (ReportItem& item : former_items) {
    if (!base::ContainsKey(used_hash_codes, item.unique_id_hash_code)) {
      // The specified time format is YYYY-MM-DD, which is 10 characters.
      char formatted_time[11];
      std::time_t now = time(0);
      strftime(formatted_time, 11, "%F", gmtime(&now));
      item.deprecation_date = formatted_time;
      items.push_back(item);
    }
  }
