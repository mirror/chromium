...
bool TrafficAnnotationExporter::LoadAnnotationsXML() {
  ...
  while (reader.Read()) {
    ...
    if (reader.NodeAttribute("second_id", &temp_str)) {
      all_ok &= reader.NodeAttribute("second_id", &temp_str) &&
                base::StringToInt(temp_str, &item.second_id_hash_code);
    }
...
bool TrafficAnnotationExporter::UpdateAnnotations(
  ...
  for (AnnotationInstance annotation : annotations) {
    ...
    if (base::ContainsKey(annotation_items_, annotation.proto.unique_id())) {
      ...
      // Content (including policy and semnantic fields).
...
std::string TrafficAnnotationExporter::GenerateSerializedXML() {
  ...
  for (const auto& item : annotation_items_) {
    ...
    // Semantics List (for incomplete annotations)
    ...
    // Policy List (for incomplete annotations)
    if (!item.second.semantics_fields.empty()) {
      std::string text;
