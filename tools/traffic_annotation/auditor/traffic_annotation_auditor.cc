...
// Returns true if the list of |path_filters| include the given |file_path|, or
// there path filters which are a folder (don't have a '.' in their name), and
// match the file name.
...
bool TrafficAnnotationAuditor::GenerateFilesListForClangTool(
  ...
  for (const auto& path_filter : path_filters) {
    ...
    if (base::DirectoryExists(path)) {
      ...
      // Otherwise add the file if it exists and is a relevant file which is not
      // safe-listed.
