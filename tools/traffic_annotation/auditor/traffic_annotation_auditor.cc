...
AuditorResult::AuditorResult(ResultType type, std::string message)
    : AuditorResult::AuditorResult(type, message, "", -1) {}

AuditorResult::AuditorResult(ResultType type)
    : AuditorResult::AuditorResult(type, "", "", -1) {}
...
AuditorResult AnnotationInstance::Deserialize(
  ...
  if (unique_id_hash_code == TRAFFIC_ANNOTATION_FOR_TESTS.unique_id_hash_code ||
    ...
    return AuditorResult(AuditorResult::ResultType::IGNORE);
  }
  ...
  return AuditorResult(AuditorResult::ResultType::OK);
}
...
bool ParseClangToolRawOutput(const std::string& clang_output,
  ...
  for (unsigned int current = 0; current < lines.size(); current++) {
    ...
    AuditorResult result(AuditorResult::ResultType::OK);
    ...
    switch (result.type()) {
      case AuditorResult::ResultType::OK: {
        if (annotation_block)
      ...
      case AuditorResult::ResultType::IGNORE:
        break;
      case AuditorResult::ResultType::ERROR_FATAL: {
