...
bool TrafficAnnotationAuditor::RunClangTool(
  ...
  // Create a file to pass options to clang tool running script.
  ...
  if (!filter_files || use_compile_commands) {
    ...
    if (path_filters.size()) {
      for (const auto& path_filter : path_filters) {
        // If path filter is a directory, search and add it's contents,
        // otherwise just check if it is a not safe listed, relevant file.
        if (base::DirectoryExists(base::FilePath(path_filter))) {
...
std::set<int> TrafficAnnotationAuditor::GetReservedIDsSet() {
  ...
  for (const auto& id : kReservedAnnotations)
    reserved_ids.insert(id.first);
  return reserved_ids;
}
