...
bool TrafficAnnotationAuditor::RunClangTool(
  ...
  if (!result) {
    ...
    if (options_file_text.length() > 1024)
      options_file_text = options_file_text.substr(0, 1024);

    LOG(ERROR) << base::StringPrintf(
        "Calling clang too returned false.\nCommand line: [%s].\n\n"
        "Returned error text: [%s].\n\nPartial options file: [%s].",
        cmdline.GetCommandLineString().c_str(), tool_errors.c_str(),
        options_file_text.c_str());
  }
