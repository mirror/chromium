...
// DST->FLD is the destination field, and SRD->FLD is the source field. ERR is
// the varaiable to which the error is added, and TXT is the name of the field,
// for error message.
...
std::string AuditorResult::ToText() const {
  switch (type_) {
    ...
    case AuditorResult::Type::ERROR_INCONSISTENT_ANNOTATION:
      DCHECK(details_.size());
      return base::StringPrintf(
          "Annotation at '%s:%i' has the following inconsistency: %s",
          file_path_.c_str(), line_, details_[0].c_str());
    ...
    case AuditorResult::Type::ERROR_MERGE_FAILED:
      ...
      return base::StringPrintf(
          ...
          "error(s): %s",
          details_[1].c_str(), details_[2].c_str(), details_[3].c_str());
...
bool AnnotationInstance::IsCompletableWith(
  ...
  if (type != AnnotationInstance::Type::ANNOTATION_PARTIAL)
    return false;
  if (other.type == AnnotationInstance::Type::ANNOTATION_COMPLETING) {
    return extra_id == other.proto.unique_id();
  } else if (other.type ==
             AnnotationInstance::Type::ANNOTATION_BRANCHED_COMPLETING) {
    return extra_id == other.extra_id;
  } else {
    return false;
  }
}
...
AuditorResult AnnotationInstance::CreateCompleteAnnotation(
  ...
  // For completing annotations, assume partial annotation as the main
  // annotation, otherwise assume the branched_completing is main.
  ...
  // Set cookies_allowed to the superceeding value of both.
...
void TrafficAnnotationAuditor::PurgeAnnotations() {
  for (unsigned int i = 0; i < extracted_annotations_.size(); i++) {
    if (base::ContainsKey(annotations_to_be_perged_,
...
void TrafficAnnotationAuditor::CheckDuplicateHashes() {
  ...
  struct AnnotationID {
    ...
    //    of a completing annotation or extra id of a partial or branched
    ...
    for (int id = 0; id < 2; id++) {
      ...
      if (!base::ContainsKey(collisions, current.hash)) {
        ...
        for (const auto& other : collisions[current.hash]) {
          ...
          annotations_to_be_perged_.insert(current.hash);
          annotations_to_be_perged_.insert(other.hash);
        }
      }
  ...
  for (const AnnotationInstance& instance : extracted_annotations_) {
    if (!base::ContainsOnlyChars(base::ToLowerASCII(instance.proto.unique_id()),
      ...
      errors_.push_back(AuditorResult(
          ...
          instance.proto.source().line()));
      annotations_to_be_perged_.insert(instance.unique_id_hash_code);
    }
    if (!instance.extra_id.empty() &&
  ...
  for (AnnotationInstance* partial : partials) {
    ...
    for (AnnotationInstance* completing : completing_annotations) {
      if (partial->IsCompletableWith(*completing)) {
        ...
        if (result.OK()) {
          result = completed_annotation->IsComplete();
          if (result.OK())
            result = completed_annotation.IsConsistent();
        ...
        if (result.OK())
          extracted_annotations_.push_back(completed);
        else
          errors_.push_back(result);
      }
    }
    ...
    if (!found_a_pair) {
      errors_.push_back(AuditorError(
          AuditorError::Type::ERROR_INCOMPLETED_ANNOTATION, std::string(),
          partial->proto->file(), partial->proto->line()));
    }
  }

  for (AnnotationInstance* instance : incomplete_annotations) {
    if (!base::ContainsKey(used_completing_annotations, instance)) {
      errors_.push_back(AuditorError(
          AuditorError::Type::ERROR_INCOMPLETED_ANNOTATION, std::string(),
          instance->proto->file(), instance->proto->line()));
    }
  }
}
