...
AnnotationInstance::AnnotationInstance()
    : annotation_type_(AnnotationType::ANNOTATION_COMPLETE) {}

AnnotationInstance::AnnotationInstance(const AnnotationInstance& other)
    : proto_(other.proto_),
      annotation_type_(other.annotation_type_),
      extra_id_(other.extra_id_),
      unique_id_hash_code_(other.unique_id_hash_code_),
      extra_id_hash_code_(other.extra_id_hash_code_){};
...
bool AnnotationInstance::Deserialize(
  ...
  extra_id_ = serialized_lines[start_line++];

  // Decode function type.
  if (function_type == "Definition") {
    annotation_type_ = AnnotationType::ANNOTATION_COMPLETE;
  } else if (function_type == "Partial") {
    annotation_type_ = AnnotationType::ANNOTATION_PARTIAL;
  } else if (function_type == "Completing") {
    annotation_type_ = AnnotationType::ANNOTATION_COMPLETENG;
  } else if (function_type == "BranchedCompleting") {
    annotation_type_ = AnnotationType::ANNOTATION_BRANCHED_COMPLETING;
  } else {
  ...
  // Process test tags.
  unique_id_hash_code_ = ComputeHashValue(unique_id);
  if (unique_id_hash_code_ ==
          TRAFFIC_ANNOTATION_FOR_TESTS.unique_id_hash_code ||
      unique_id_hash_code_ ==
          PARTIAL_TRAFFIC_ANNOTATION_FOR_TESTS.unique_id_hash_code) {
    return false;
  }

  // Process undefined tags.
  if (unique_id_hash_code_ == NO_TRAFFIC_ANNOTATION_YET.unique_id_hash_code ||
      unique_id_hash_code_ ==
  ...
  // Process missing tag.
  if (unique_id_hash_code_ == MISSING_TRAFFIC_ANNOTATION.unique_id_hash_code) {
    *error_text =
  ...
  if (!google::protobuf::TextFormat::ParseFromString(
          annotation_text, (google::protobuf::Message*)&proto_)) {
  ...
  traffic_annotation::NetworkTrafficAnnotation_TrafficSource* src =
      proto_.mutable_source();
  src->set_file(file_path);
  src->set_function(function_context);
  src->set_line(line_number);
  proto_.set_unique_id(unique_id);
  extra_id_hash_code_ = ComputeHashValue(extra_id_);

  return true;
}

CallInstance::CallInstance() : line_number_(0), is_annotated_(false) {}

CallInstance::CallInstance(const CallInstance& other)
    : file_path_(other.file_path_),
      line_number_(other.line_number_),
      function_context_(other.function_context_),
      function_name_(other.function_name_),
      is_annotated_(other.is_annotated_){};
...
bool CallInstance::Deserialize(const std::vector<std::string>& serialized_lines,
  ...
  file_path_ = serialized_lines[start_line++];
  function_context_ = serialized_lines[start_line++];
  int line_number;
  base::StringToInt(serialized_lines[start_line++], &line_number);
  line_number_ = static_cast<uint32_t>(line_number);
  function_name_ = serialized_lines[start_line++];
  int is_annotated_int;
  base::StringToInt(serialized_lines[start_line++], &is_annotated_int);
  is_annotated_ = is_annotated_int != 0;
  return true;
}
