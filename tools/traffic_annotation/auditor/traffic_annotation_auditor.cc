...
bool TrafficAnnotationAuditor::IsWhitelisted(
    const std::string& file_path,
    const AuditorException::ExceptionType whitelist_type) {
...
void TrafficAnnotationAuditor::GetReservedUniqueIDs(
    std::map<int, std::string>* reserved_ids) {
  reserved_ids->insert(
      std::make_pair(TRAFFIC_ANNOTATION_FOR_TESTS.unique_id_hash_code, "test"));

  reserved_ids->insert(
      std::make_pair(PARTIAL_TRAFFIC_ANNOTATION_FOR_TESTS.unique_id_hash_code,
                     "test_partial"));

  reserved_ids->insert(std::make_pair(
...
void TrafficAnnotationAuditor::ChecksDuplicateHashes() {
  std::map<int, std::string> reserved_ids;
  GetReservedUniqueIDs(&reserved_ids);

  std::map<int, std::vector<unsigned>> unique_ids;
  for (unsigned index = 0; index < extracted_annotations_.size(); index++) {
    AnnotationInstance& instance = extracted_annotations_[index];

    // If unique id is similar to a reserved id, add an error.
    ...
    // Find unique id's with similar hash codes.
  ...
  for (const auto& item : unique_ids) {
    if (item.second.size() > 1) {
      AuditorResult error(
          AuditorResult::ResultType::ERROR_DUPLICATE_UNIQUE_ID_HASH_CODE);
      for (unsigned index : item.second) {
