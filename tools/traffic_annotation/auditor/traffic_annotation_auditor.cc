...
bool TrafficAnnotationAuditor::RunClangTool(
  ...
  // |ignore_list_all_| is not passed when |full_run| is happening as there is
  // no way to pass it to run_tools.py except enumerating all alternatives.
  // The paths in |ignore_list_all_| are removed later from the results.
  if (full_run) {
    ...
    if (path_filters.size()) {
      for (const auto& path_filter : path_filters) {
        filter.GetRelevantFiles(source_path_, ignore_list_all_, path_filter,
                                &file_paths);
      }
    } else {
      filter.GetRelevantFiles(source_path_, ignore_list_all_, "", &file_paths);
    }
...
// Checks to see if a |file_path| matches a |white_list| of partial paths.
bool TrafficAnnotationAuditor::IsWhiteListed(
    const std::string file_path,
    const std::vector<std::string>& white_list) {
  for (const std::string& ignore_path : white_list) {
...
bool TrafficAnnotationAuditor::ParseClangToolRawOutput() {
  ...
  for (unsigned int current = 0; current < lines.size(); current++) {
    ...
    if (!IsWhiteListed(result.file_path(), ignore_list_all_) &&
        (result.type() != AuditorResult::ResultType::ERROR_MISSING ||
         !IsWhiteListed(result.file_path(), ignore_list_missing_))) {
      switch (result.type()) {
...
bool TrafficAnnotationAuditor::LoadWhiteList() {
  ...
  if (base::ReadFileToString(white_list_file, &file_content)) {
    ...
    for (const std::string& line : lines) {
      ...
      std::string path = line.substr(comma + 1, line.length() - comma - 1);
      if (type == "all") {
        ignore_list_all_.push_back(path);
      } else if (type == "missing") {
        ignore_list_missing_.push_back(path);
      } else if (type == "empty_mutable") {
        ignore_list_empty_mutable_.push_back(path);
      } else {
        LOG(ERROR) << "Unexpected type in white_list.txt: " << type;
        return false;
      }
    }
    return true;
