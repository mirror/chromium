...
// Recursively compute hash code of the given string as in:
...
AnnotationInstance::AnnotationInstance()
    : annotation_type(ANNOTATION_COMPLETE) {}
...
// COMMENT ON HOW THIS IS DONE AND WHAT ARE THE LINES
bool AnnotationInstance::Deserialize(
  ...
  if (end_line - start_line < 7) {
    LOG(ERROR) << "Not enough lines to deserialzie annotation.";
    *error_text = "FATAL";
    return false;
  }

  // Extract header lines.
  std::string file_path = serialized_lines[start_line++];
  std::string function_context = serialized_lines[start_line++];
  int line_number;
  base::StringToInt(serialized_lines[start_line++], &line_number);
  std::string function_type = serialized_lines[start_line++];
  std::string unique_id = serialized_lines[start_line++];
  extra_id = serialized_lines[start_line++];

  // Decode function type.
  if (function_type == "Definition") {
    annotation_type = AnnotationInstance::ANNOTATION_COMPLETE;
  } else if (function_type == "Partial") {
    annotation_type = AnnotationInstance::ANNOTATION_PARTIAL;
  } else if (function_type == "Completing") {
    annotation_type = AnnotationInstance::ANNOTATION_COMPLETENG;
  } else if (function_type == "BranchedCompleting") {
    annotation_type = AnnotationInstance::ANNOTATION_BRANCHED_COMPLETENG;
  } else {
  ...
  // Process unittest and undefined tags.
...
bool CallInstance::Deserialize(const std::vector<std::string>& serialized_lines,
  ...
  if (end_line - start_line != 5) {
    LOG(ERROR) << "Incorrect number of lines to deserialzie call.";
    *error_text = "FATAL";
    return false;
  }
...
bool ParsClangToolRawOutput(const std::string& clang_output,
  ...
  for (unsigned current = 0; current < lines.size(); current++) {
    ...
    if (end_line == lines.size()) {
      LOG(ERROR) << "Block starting at line " << current
                 << " is not ended by appropriate tag.";
      return false;
    }
    ...
    if (annotation_block &&
        new_annotation.Deserialize(lines, current, end_line, &error_text)) {
      annotations->push_back(new_annotation);
    } else if (!annotation_block &&
               new_call.Deserialize(lines, current, end_line, &error_text)) {
