...
void TrafficAnnotationAuditor::GetReservedUniqueIDs(
  ...
  *reserved_ids = kReservedAnnotations;
}
...
void TrafficAnnotationAuditor::CheckDuplicateHashes() {
  ...
  std::map<int, std::vector<AnnotationInstance*>> unique_ids;
  for (AnnotationInstance instance : extracted_annotations_) {
    ...
    if (unique_ids.find(instance.unique_id_hash_code) == unique_ids.end()) {
      std::vector<AnnotationInstance*> empty_list;
      unique_ids.insert(
          std::make_pair(instance.unique_id_hash_code, empty_list));
    }
    unique_ids[instance.unique_id_hash_code].push_back(&instance);
  }
  ...
  for (const auto& item : unique_ids) {
    if (item.second.size() > 1) {
      AuditorResult error(
          AuditorResult::ResultType::ERROR_DUPLICATE_UNIQUE_ID_HASH_CODE);
      for (AnnotationInstance* instance : item.second) {
        error.AddDetail(base::StringPrintf(
            "%s in '%s:%i'", instance->proto.unique_id().c_str(),
            instance->proto.source().file().c_str(),
            instance->proto.source().line()));
      }
