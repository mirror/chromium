...
// Holds an item of whitelist exceptions for auditor.
struct AuditorException {
  enum class ExceptionType {
    ALL,            // Ignores all errors (doesn't check the files at all).
    MISSING,        // Just ignores missing annotations.
    EMPTY_MUTABLE,  // Just ignores empty mutable annotation constructor.
  } type;
...
class TrafficAnnotationAuditor {
 public:
  ...
  // Loads the whitelist file and puplates ignore_list member variables.
  bool LoadWhiteList();
 ...
 private:
  ...
  std::vector<std::string> ignore_list_all_;
  std::vector<std::string> ignore_list_missing_;
  std::vector<std::string> ignore_list_empty_mutable_;
};
