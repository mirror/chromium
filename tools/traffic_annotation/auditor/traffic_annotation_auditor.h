...
class TrafficAnnotationAuditor {
 public:
  ...
  void ChecksDuplicateHashes();

  // Preforms all checks on extracted annotations and calls and adds the results
  // to |errors_|.
  void RunAllChecks();

  // Returns a mapping of reserved unique ids' hash codes to the uniqu id's
  ...
  void GetReservedUniqueIDs(std::map<int, std::string>* reserved_ids);
