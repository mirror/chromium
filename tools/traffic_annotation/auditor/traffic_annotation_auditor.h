...
class AnnotationInstance : public InstanceBase {
 public:
  ...
  // Checks to see if this annotation can be completed with |other| annotation,
  // based on their Unique ids, types, and extra ids. |*this| should be partial
  // and the |other| either completing or branched_completing.
  bool IsCompletableWith(const AnnotationInstance& other) const;

  // Combines current partial annotation with a completing/branched_completing
  // annotations and returns the combined complete annotation or an error.
...
class TrafficAnnotationAuditor {
 public:
  ...
  // Adds errors to |errors_| and unique id hash codes of duplicate annotations
  // |annotations_to_be_perged_|.
  void CheckDuplicateHashes();

  // Checks to see if unique ids only include alphanumeric characters and
  // underline. Adds errors to |errors_| and unique id hash codes of
  // incompatible annotations to |annotations_to_be_perged_|.
  ...
  // to |errors| and unique id hash codes of all annotations that have problems
  // and incomplete annotations to |annotations_to_be_perged_|.
  ...
  // Removes annotations whose unique id hash code is in
  // |annotations_to_be_perged_| and clears it.
  void PurgeAnnotations();
  ...
  void ClearPurgeSetForTest() { annotations_to_be_perged_.clear(); }
 ...
 private:
  ...
  std::set<int> annotations_to_be_perged_;
