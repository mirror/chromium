...
class TrafficAnnotationAuditor {
 public:
  ...
  // filters are passed so that the data for files that was not tested would be
  // read from annotations.xml. If |report_xml_updates| is set and
  // annotations.xml requires updates, the updates are added to errors.
 ...
 private:
  ...
  // Adds all archived annotations (from annotations.xml) which have the
  // following features to |extracted_annotations_|:
  //  1- Are not deprecated.
  //  2- OS list includes current platform.
  //  2- Have a path (they are not reserved words).
  //  3- Path matches an item in |path_fitler|.
  void AddMissingAnnotations(const std::vector<std::string>& path_filters);

  // Generates files list to Run clang tool. Please refer to RunClangTool
  // function's comment.
