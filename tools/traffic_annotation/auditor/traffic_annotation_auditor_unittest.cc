...
TEST_F(TrafficAnnotationAuditorTest, AnnotationsXMLDifferences) {
  ...
  EXPECT_TRUE(base::ReadFileToString(
      base::MakeAbsoluteFilePath(
          tests_folder().Append("annotations_sample1.xml")),
      &xml1));
  EXPECT_TRUE(base::ReadFileToString(
      base::MakeAbsoluteFilePath(
          tests_folder().Append("annotations_sample2.xml")),
      &xml2));
  EXPECT_TRUE(base::ReadFileToString(
      base::MakeAbsoluteFilePath(
          tests_folder().Append("annotations_sample3.xml")),
      &xml3));

  std::string diff12 = exporter.GetXMLDifferences(xml1, xml2);
  std::string diff13 = exporter.GetXMLDifferences(xml1, xml3);
  std::string diff23 = exporter.GetXMLDifferences(xml2, xml3);
  ...
  EXPECT_TRUE(base::ReadFileToString(
      base::MakeAbsoluteFilePath(
          tests_folder().Append("annotations_diff12.txt")),
      &expected_diff12));
  EXPECT_TRUE(base::ReadFileToString(
      base::MakeAbsoluteFilePath(
          tests_folder().Append("annotations_diff13.txt")),
      &expected_diff13));
  EXPECT_TRUE(base::ReadFileToString(
      base::MakeAbsoluteFilePath(
          tests_folder().Append("annotations_diff23.txt")),
      &expected_diff23));
