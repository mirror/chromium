...
TEST_F(TrafficAnnotationAuditorTest, GetFilesFromGit) {
  ...
  filter.SetGitMockFileForTesting(
      tests_folder_.Append(FILE_PATH_LITERAL("git_list.txt")));
...
TEST_F(TrafficAnnotationAuditorTest, CallDeserilization) {
  ...
  for (const auto& test_case : test_cases) {
    // Checking if deserialization result is as expected.
    CallInstance call;
    AuditorResult::ResultType result_type =
        Deserialize(test_case.file_name, &call);
    EXPECT_EQ(result_type, test_case.result_type);

    // Extra checks for one complete sample.
...
TEST_F(TrafficAnnotationAuditorTest, CheckDuplicateHashes) {
  ...
  auditor.set_extracted_annotations_for_test(annotations);
