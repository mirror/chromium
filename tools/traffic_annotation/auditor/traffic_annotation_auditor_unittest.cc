...
TEST_F(TrafficAnnotationAuditorTest, CheckDuplicateHashes) {
  ...
  // Check for unique id / extra id cases.
  ...
  for (int type1 = 0; type1 < last_type; type1++) {
    for (int type2 = 0; type2 <= last_type; type2++) {
      for (int ids = 0; ids < 256; ids++) {
        ...
        // It's compatible only if the first one is partial, has extra_id, and
        // the second one is either completing with matching unique id or
