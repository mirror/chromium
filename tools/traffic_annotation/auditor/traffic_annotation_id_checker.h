...
class TrafficAnnotationIDChecker {
 public:
  ...
  // Checks loaded annotations and reports errors to |errors|.
 ...
 private:
  ...
  // Checks if the ids presented in invalid_set are not used in annotations.
  void CheckForInvalidValues(const std::set<int>& invalid_set,
                             AuditorResult::Type error_type,
                             std::vector<AuditorResult>* errors);

  // Check if annotations that need an extra id, have one which is different
  // from their unique id.
  void CheckForExtraIDs(std::vector<AuditorResult>* errors);

  // Checks if hash values of two ids collide.
