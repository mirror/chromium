...
std::string GetGit(base::FilePath& source_path) {
  ...
  std::string git_list;
  const base::CommandLine::CharType* args[] = {"git", "diff", "--cached",
                                               "--name-only"};
  base::CommandLine cmdline(2, args);

  // Get list of files from git.
  if (!base::GetAppOutputAndError(cmdline, &git_list)) {
  ...
  git_list += "\n";
  git_list += source_path.MaybeAsASCII();
  git_list += "\n";
  git_list += original_path.MaybeAsASCII();
  return git_list;
}
