...
int main(int argc, char* argv[]) {
  ...
  if (!ids_file.empty()) {
    ...
    for (auto& instance : annotation_instances) {
      items.push_back(
          base::StringPrintf("%s, %i\n", instance.proto.unique_id().c_str(),
                             traffic_annotation_auditor::ComputeHashValue(
    ...
    items.push_back(
        base::StringPrintf("%i, [no_annotation]\n",
                           NO_TRAFFIC_ANNOTATION_YET.unique_id_hash_code));
    items.push_back(base::StringPrintf(
        "[no_partial_annotation], %i\n",
