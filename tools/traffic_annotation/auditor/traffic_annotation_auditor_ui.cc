...
// directly from the chrome_policy object. Since Chrome policy has over 300+
// implementations, the required output is now extrated from debug output as the
// debug output has the following format:
// POLICY_NAME {
//  ...
  ...
  for (auto& instance : annoations) {
    ...
    switch (semantics.destination()) {
      ...
      case traffic_annotation::
        ...
        if (semantics.destination_other().length())
