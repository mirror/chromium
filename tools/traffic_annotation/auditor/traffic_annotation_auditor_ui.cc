...
int main(int argc, char* argv[]) {
  ...
  if (!traffic_annotation_auditor::ParsClangToolRawOutput(
  ...
  if (!summary_file.empty()) {
    ...
    report += "\n[Calls]\n";
    items.clear();
    for (auto& instance : call_instances) {
  ...
  if (!ids_file.empty()) {
    ...
    items.push_back(std::make_pair(
        TRAFFIC_ANNOTATION_FOR_TESTS.unique_id_hash_code, "[tests]"));
    items.push_back(
        std::make_pair(PARTIAL_TRAFFIC_ANNOTATION_FOR_TESTS.unique_id_hash_code,
                       "[partial_tests]"));
    items.push_back(std::make_pair(
        NO_TRAFFIC_ANNOTATION_YET.unique_id_hash_code, "[no_annotation]"));
    items.push_back(
        std::make_pair(NO_PARTIAL_TRAFFIC_ANNOTATION_YET.unique_id_hash_code,
                       "[no_partial_annotation]"));
    items.push_back(std::make_pair(
