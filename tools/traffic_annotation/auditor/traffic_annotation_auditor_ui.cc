...
Outputs all errors and warnings to stdout and retunrs with the following exit
codes:
  0: Everything is OK.
  1: There are errors.
...
int main(int argc, char* argv[]) {
  ...
  if (command_line.HasSwitch("limit")) {
    if (!base::StringToInt(command_line.GetSwitchValueNative("limit"),
                           &outputs_limit)) {
      LOG(ERROR) << "The value for 'limit' switch should be an integer.";
      return 1;
    }
  }
  ...
  if (!outputs_limit || outputs_limit != errors_count) {
    for (const auto& error : errors) {
      if (!IsResultConsidererdError(error.type())) {
        ...
        if (outputs_limit && errors_count + warnings_count == outputs_limit)
          break;
      }
    }
  }
