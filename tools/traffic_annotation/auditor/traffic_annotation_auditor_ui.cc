...
int main(int argc, char* argv[]) {
  ...
  if (!summary_file.empty()) {
    ...
    for (const auto& instance : annotation_instances) {
      ...
      google::protobuf::TextFormat::PrintToString(instance.proto_, &serialized);
    ...
    for (const auto& instance : call_instances) {
      items.push_back(base::StringPrintf(
          "File:%s:%i\nFunction:%s\nAnnotated: %i\n",
          instance.file_path_.c_str(), instance.line_number_,
          instance.function_name_.c_str(), instance.is_annotated_));
    }
    std::sort(items.begin(), items.end());
    for (const std::string& item : items)
      report += item;
