...
Options:
  ...
  --tsv-file          Optional path to a TSV output file with all annotations.
...
// Changes tabs in text to space, adds quotations if text includes end of lines.
std::string UpdateTextForTSV(std::string text) {
  base::ReplaceChars(text, "\t", " ", &text);
  if (text.find('\n') != std::string::npos)
    text = base::StringPrintf("\"%s\"", text.c_str());
  return text;
}

// Since Chrome Policy has over 150+ implementations, I couldn't find a neat
// way to extract it's data. Will use this hack and replace it when a better
// approach is found.
...
bool WriteTSVFile(const base::FilePath& filepath,
                  const std::vector<AnnotationInstance>& annoations) {
  std::vector<std::string> lines;
  std::string title =
      "Hash Code\tUnique ID\tSource File\tEmpty Policy Justification\t"
      "Sender\tDescription\tTrigger\tData\tDestination\tCookies Allowed\t"
      "Cookies Store\tSetting\tChrome Policy\tComments";

  for (auto& instance : annoations) {
    // if (71251498 == instance.unique_id_hash_code)
    //   LOG(ERROR) << instance.proto.semantics().data();
    std::string line = base::StringPrintf("%i", instance.unique_id_hash_code);
    line += base::StringPrintf("\t%s", instance.proto.unique_id().c_str());
    ...
    switch (semantics.destination()) {
      ...
      case traffic_annotation::
        ...
        line += base::StringPrintf(
            "\tOther:%s",
            UpdateTextForTSV(semantics.destination_other()).c_str());
        break;
...
int main(int argc, char* argv[]) {
  ...
  base::FilePath tsv_file = command_line.GetSwitchValuePath("tsv-file");
  ...
  // Write TSV file.
  if (!tsv_file.empty() &&
      !WriteTSVFile(tsv_file, auditor.extracted_annotations())) {
