...
def SaveAnnotationsSheet(contents, update_report):
  """Updates the trix with new annotations and an update report.
  Args:
    contents: list of list Full table of annotations to be uploaded.
  ...
  result = service.spreadsheets().values().update(
    ...
    valueInputOption=RAW, body=request).execute()
  print(result)
  # TODO: Add Report line.
...
def main():
  ...
  if update_report:
    ...
    if raw_input('(Y/n): ').strip().lower() != 'y':
      return 0
    SaveAnnotationsSheet(new_content, update_report)
