...
"""A tool to archive a build and its symbols, executed by a buildbot slave.
  ...
  official builds, '--mode full' for builds to be downloaded by tester
...
class Stager(object):
  ...
  def __init__(self):
    ...
    if chromium_utils.IsWindows():
      ...
      self._tool_dir = os.path.join(self._chrome_dir, 'installer', 'linux')
    else:
    ...
    self._build_revision = slave_utils.SubversionRevision(self._chrome_dir)
    if options.official:
      self._build_version = self._BuildVersion()
    else:
      self._build_version = str(self._build_revision)
  ...
  def _VerifyFiles(self):
    ...
    """
    not_found = []
    # TODO(mmoss) File list not currently used on Linux. Make something that can
    # be shared with (or generated by) the packaging script.
  ...
  def _CreateZipArchive(self):
    """Put the build files into a zip archive.
    """
  ...
  def _CreateDebArchive(self):
    ...
    """
    # Clean up errant debs (from failed archive runs).
    debs = glob.glob(os.path.join(self._staging_dir, '*.deb'))
    for deb in debs:
      os.unlink(deb)

    # Non-official builds just reuse the revision # for the version #, so put it
    # in decimal format like regular versions, otherwise it might cause issues
    # when trying to upgrade to an official build (i.e. the revision-based dev
    # version is a bigger number so looks like a newer version and will refuse
    # to "downgrade" to the official build).
    if not options.official:
      build_version = '0.%s' % self._build_version
    else:
      build_version = self._build_version
    # Create the Debian package.
    # TODO(mmoss) support google-chrome packaging with "-g" flag.
    command = [os.path.join(self._tool_dir, 'debian', 'build.sh'),
               '-o', self._staging_dir,
               '-r', '%d' % self._build_revision,
               '-v', build_version]
    res = chromium_utils.RunCommand(command)
    if (res != 0):
      raise StagingError('"%s" failed with error "%s"' % (" ".join(command),
                                                          res))
    return (self._staging_dir,
            glob.glob(os.path.join(self._staging_dir, '*.deb')))

  def _CreateArchive(self):
    """Create the platform-appropriate archive package.
    """
    if chromium_utils.IsWindows():
      return self._CreateZipArchive()
    elif chromium_utils.IsLinux():
      return self._CreateDebArchive()
      # TODO(mmoss) Add rpm packaging. Append to returned archive_files.
    else:
      raise chromium_utils.NotImplemented(
          'Don\'t know how to create archive for "%s".' % sys.platform)

  def _UploadSymbols(self, zip_dir):
    """Upload symbols to a share and to the crash server.

    This uploads directly to the crash server because the crash server does not
    know how to deal with crashes that happen when winserver has yet to process
    the newly uploaded symbols.
    """
    if not chromium_utils.IsWindows():
      # TODO(mmoss) What to do for Linux?
      return 0
    symbols_upload = []
    ...
    symbol_dir = os.path.join(self._symbol_dir_base, self._build_version)
  ...
  def _UploadBuild(self, www_dir, changelog_path, archive_files):
    if chromium_utils.IsWindows():
      ...
      if options.official:
        ...
        extras_with_path = []
      for filename in extras_with_path:
        print 'SshCopyFiles(%s, %s, %s)' % (filename,
                                            config.Archive.archive_host,
                                            www_dir)
      ...
      if not DRY_RUN:
        ...
        for archive in archive_files:
          chromium_utils.SshCopyFiles(archive, config.Archive.archive_host,
                                      www_dir)
          # Don't keep the archives because, unlike Windows, they have versioned
          # names, so they don't get overwritten next run and will waste space.
          os.unlink(archive)
        for filename in extras_with_path:
  ...
  def StageBuild(self):
    ...
    if options.official:
      print 'version: %s' % self._build_version
    ...
    archive_dir, archive_files = self._CreateArchive()
    ...
    if previous_revision == -1:
      ...
      command = [slave_utils.SubversionExe(), 'log', self._chrome_dir, '--xml',
                 '-r', '%s:%s' % (first_revision, self._build_revision)]
    ...
    www_dir = os.path.join(self._www_dir_base, self._build_version)
    self._UploadSymbols(archive_dir)
    self._UploadBuild(www_dir, changelog_path, archive_files)

    # Archives the symbols for the source server.
    if options.official:
      # TODO(mmoss) What should Linux do here? Save the unstripped binaries?
      if chromium_utils.IsWindows():
    ...
    # TODO(mmoss) What should Linux do here?
    if chromium_utils.IsWindows():
    ...
    if not DRY_RUN:
      ...
      try:
        change_file.write('%s' % self._build_revision)
      finally:
        change_file.close()
      ...
      if not options.official:
        ...
        if chromium_utils.IsWindows():
          ...
          try:
            latest_file.write('%s' % self._build_revision)
          finally:
            latest_file.close()
        elif chromium_utils.IsLinux():
          # Files are created umask 077 by default, so make it world-readable
  ...
  def FullBuild(self):
    ...
    if chromium_utils.IsWindows():
      zip_file_name = 'full-build-win32'
    elif chromium_utils.IsLinux():
      zip_file_name = 'full-build-linux'
    else:
      raise chromium_utils.NotImplemented(
            'Platform "%s" is not currently supported.' % sys.platform)
    (zip_dir, zip_file) = chromium_utils.MakeZip(self._staging_dir,
    ...
    versionned_file = zip_file.replace('.zip', '_%d.zip' % self._build_revision)
...
if '__main__' == __name__:
  ...
  if chromium_utils.IsWindows():
    ...
    # TODO(mmoss) This doesn't currently support different archives (dbg/opt)
    # from a single build tree. The packages are built from whatever build was
    # "installed" last to the root Hammer directory.
    option_parser.add_option('', '--target', default='Hammer',
        help='build target to archive (currently only Hammer)')
  else:
    raise chromium_utils.NotImplemented(
          'Platform "%s" is not currently supported.' % sys.platform)
