  ...
  def testClientConfigVariationsHeader(self):
    with TestDriver() as t:
      ...
      for i in data["events"]:
       if json.dumps(i).find("datasaver.googleapis.com") !=-1 and\
        json.dumps(i).find("clientConfigs") != -1 and\
        json.dumps(i).find("headers") != -1 and\
        json.dumps(i).find("accept-encoding") != -1 and\
        json.dumps(i).find("x-client-data") !=-1:
