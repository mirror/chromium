  ...
  def testClientConfigVariationsHeader(self):
    with TestDriver() as t:
      ...
      for i in data["events"]:
       if json.dumps(i).find("chrome-proxy") != -1 and\
        json.dumps(i).find("check.googlezip.net/test.html") != -1 and\
        json.dumps(i).find("promised_stream_id") !=-1:
          promised_stream_count = promised_stream_count + 1

       if json.dumps(i).find(str(mapped_const)) != -1 and\
        json.dumps(i).find("check.googlezip.net/test.html") != -1 and\
        json.dumps(i).find("stream_id") !=-1:
