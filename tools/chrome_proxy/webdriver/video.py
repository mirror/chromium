...
class Video(IntegrationTest):
  ...
  def testRangeRequest(self):
    with TestDriver() as t:
      ...
      # Check that HttpOriginalContentLengthWithValidOCL histogram has one entry
      # in the 201 byte category, which is bucket 196-258
      self.assertIn({'count': 1, 'high': 258, 'low': 196},
                    t.GetHistogram('Net.HttpOriginalContentLengthWithValidOCL')
                    ['buckets'])
