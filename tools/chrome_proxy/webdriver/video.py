...
class Video(IntegrationTest):
  ...
  def instrumentedVideoTest(self, url, alt_data=None):
    ...
    with TestDriver() as t:
      t.AddChromeArg('--enable-spdy-proxy-auth')
      t.AddChromeArg('--no-user-gesture-required')
      ...
      # If the device is not Android, just clicking on the video element won't
      # work, also send a Javascript statement. Note: This JS statement only
      # works after the click.
