...
def generate_telemetry_tests(
  name, tester_config, benchmarks, benchmark_sharding_map, use_whitelist):
  isolated_scripts = []
  ...
  for benchmark in benchmarks:
    disabled = decorators.GetDisabledAttributes(benchmark)
    ...
    for dimension in tester_config['swarming_dimensions']:
      dimension_os = process_os(dimension['os'])
      # TODO(martiniss): comment below back in
      # if dimension_os in disabled:
      if 'all' in disabled:
        continue
      ...
      # TODO(martiniss): Remove this hack once we have real sharding
      special_case = SPECIAL_CASES.get(name, {}).get(benchmark.Name())
      if special_case:
        print 'overidding device affinity for %s due to special casing' % (
            name)
        device_affinity = dimension['device_ids'].index(special_case)
...
SPECIAL_CASES = {
    'Linux Perf': {
        'webrtc.peerconnection': 'build149-m1',
    },
    # FIXME: system_health.common_desktop, system_health.memory_desktop, and
    # page_cycler_v2.typical_25 all take forever on this builder.
    'Mac 10.12 Perf': {
        'page_cycler_v2.intl_hi_ru': 'build162-m1',
        'blink_perf.parser': 'build162-m1',
    },
    'Win Zenbook Perf': {
        'v8.todomvc': 'build31-b1',
    }
}
  ...
  for name, config in waterfall['testers'].iteritems():
    ...
    if config.get('swarming', False):
      ...
      isolated_scripts = generate_telemetry_tests(
          name, config, benchmark_list, sharding_map, use_whitelist)
...
def diff(old, new):
  ...
  Any differences between objects is described with a string, like
    "'3' != '4'"

  It will intelligently diff lists and dictionaries.
  """
  ...
  if type(old) != type(new):
    return '%s != %s; types differ' % (old, new)

  if isinstance(old, list):
    return [diff(o, n) for o, n in zip(old, new)]
  ...
  if isinstance(old, dict):
    ...
    for k in old:
      if k not in new:
        res[k] = 'missing in new'
        continue

      seen.add(k)
      new_v, old_v = new[k], old[k]
      d = diff(old_v, new_v)
      if d:
        res[k] = d
    return res
...
def diff_to_lines(d):
  """Converts a diff to a list of changes."""
  ...
  if isinstance(d, dict):
    ...
    for k, v in d.iteritems():
      res = diff_to_lines(v)
      for l in res:
        lines.append('%s -> %s' % (k, l))

    return lines

  if isinstance(d, list):
    return [diff_to_lines(x) for x in d]

  return [d]

def get_waterfall_json_path(waterfall_name):
  """Get the path of a waterfall's JSON."""
  filename = '%s.json' % waterfall_name

  buildbot_dir = os.path.join(src_dir(), 'testing', 'buildbot')
  return os.path.join(buildbot_dir, filename)


def diff_CMD(all_tests):
  """diffs current json file with what would be generated.

  Prints some information about the changes to stdout.
  """
  found_changes = False
  for waterfall_name, tests in all_tests.iteritems():
    # Don't care about changes to FYI
    if 'fyi' in waterfall_name:
      continue

    json_filename = get_waterfall_json_path(waterfall_name)

    with open(json_filename) as f:
      old_data = json.load(f)

    all_diffs = {}
    for buildername, new in sorted(tests.iteritems(), key=lambda x: x[0]):
      # FIXME: Remove once android bots are swarmed
      if 'Android' in buildername:
        continue

      new = new['isolated_scripts']
      old = old_data.get(buildername, {}).get('isolated_scripts', [])
      all_diffs[buildername] = collections.defaultdict(set)
      for o, n in sorted(zip(old, new), key=lambda x: x[0]['name']):
        assert o['name'] == n['name']
        name = o['name']
        d = diff(o, n)
        all_diffs[buildername][name].add(tuple(diff_to_lines(d)))

    changes = []
    max_width = len(max(all_diffs.keys(), key=len))
    for bname, its in sorted(all_diffs.items(), key=lambda x: reversed(x[0])):
      changed = 0
      for test_name, diffs in sorted(its.items(), key=lambda x: x[0]):
        itm = list(diffs)[0]
        if not itm:
          continue
        changed += 1
        changes.append('%s:%s changed: %s\n' % (bname, test_name, itm[0]))
      num_tests = len(its.keys())
      found_changes = True
      print 'change percentage for %-*s: %.2f%%' % (
          max_width + 1, bname, 100*float(changed)/num_tests)

    changes_path = os.path.join(src_dir(), 'tools', 'perf', 'changes_%s.txt' % (
        waterfall_name))
    with open(changes_path, 'w') as f:
      f.writelines(changes)

  return 1 if found_changes else 0



def write_CMD(all_tests):
  """write out new waterfall json.
  """
  for waterfall_name, tests in all_tests.iteritems():
...
def main():
  ...
  parser.add_argument('action', choices=list(actions.keys()), default='write')
  ...
  tests = {
      waterfall['name']: generate_all_tests(waterfall) for waterfall in [
          fyi_waterfall, waterfall]
  }
