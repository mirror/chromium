...
def main(args, output):
  ...
  if options.benchmark_name:
    ...
    for b in benchmark_finders.GetAllPerfBenchmarks():
      story_sets.append(GetStorySet(b))
    for b in benchmark_finders.GetAllContribBenchmarks():
      for user_agent in ['desktop', 'mobile']:
        story_sets.append(GetStorySet(b, user_agent))
