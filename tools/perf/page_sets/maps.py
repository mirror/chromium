...
class MapsPage(page_module.Page):
  ...
  //src/tools/perf/page_sets/map_perf_test/.

  Note: the file map_perf_test/load_dataset is a large binary file (~3Mb), hence
  we upload it to cloud storage & only check in the SHA1 hash.

  The command to upload it to cloud_storage is:
  <path to depot_tools>/upload_to_google_storage.py map_perf_test/load_dataset \
