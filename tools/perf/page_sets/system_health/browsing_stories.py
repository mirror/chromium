...
class GoogleMapsStory(_BrowsingStory):
  ...
  '''
  # Check if the current restaurant name is different from the old name
  # to checks that restaurants started to update. Also wait for the completion
  # of the loading by waiting for the button to change to loaded.
  ...
  def _DidLoadDocument(self, action_runner):
    ...
    action_runner.WaitForJavaScriptCondition(
      self._CHECK_RESTAURANTS_UPDATED, old_restaurant=previous_restaurant_name)
    ...
    action_runner.WaitForJavaScriptCondition(self._CHECK_RESTAURANTS_UPDATED,
      old_restaurant=previous_restaurant_name)
