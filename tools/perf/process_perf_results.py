...
def _process_perf_results(output_json, json_files, configuration_name,
  ...
  print task_output_dir
  print 'file_list'
  file_list = [f for f in listdir(task_output_dir) if isfile(join(task_output_dir, f))]
  dir_list = [f for f in listdir(task_output_dir) if not isfile(join(task_output_dir, f))]
  for directory in dir_list:
    shard_perf_results_file_list.append(join(task_output_dir, directory, 'perftest-output.json'))
    shard_json_output_list.append(join(task_output_dir, directory, 'output.json'))
  ...
  for shard in shard_json_output_list:
    shard_json_results.append(json.load(shard))
