...
trigger. It modifies it in the following ways:
  * Inserts bot id where appropriate; swarming dimensions, and task arguments.
...
def trigger_tasks(args, remaining):
  ...
  """
  # Re-add the shards argument
  remaining = ['--shards', args.shards] + remaining
...
def main():
  ...
  parser.add_argument('--swarming-py-path', required=True,
                      help='(Swarming Trigger Script API)'
                      ' Path to the swarming.py file to use for any swarming'
                      ' RPCs.')
  parser.add_argument('--dump-json', required=True,
                      help='Where to dump the resulting json which indicates'
                      ' which tasks were triggered for which shards')
  parser.add_argument('--shards', required=True,
                      help='How many shards to trigger. Duplicated from the'
                      ' `swarming.py trigger` command')
  args, remaining = parser.parse_known_args()

  if args.shards != len(args.bot_id):
    raise parser.error(
        'Number of bots to use must equal number of shards (--shard)')
