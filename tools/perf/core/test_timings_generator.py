...
def _bd(builder):
  builder, num = builder
  return get_builder_data(builder, num, True)
    ...
    for builder in lst:
      ...
      try:
        for bnum, tasks in data.items():
          ...
          def ok_state(task):
            return not (task.state in ('EXPIRED', 'BOT_DIED'))
          for task in tasks:
            if not ok_state(task):
              continue
            benchmark_overall_times[task.benchmark_name].append(task.duration)

          simple = [(t.benchmark_name, t.task_id) for t in tasks
                    if 'reference' not in t.benchmark_name and ok_state(t)]
