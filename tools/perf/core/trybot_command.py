...
def _IsPerfBisectBot(builder):
  return (
      ...
      not builder.endswith('fyi_perf_bisect') and
      builder != 'win_xp_perf_bisect'   # Goma issues: crbug.com/330900
  )


assert all(_IsPerfBisectBot(builder) for builder in DEFAULT_TRYBOTS), (
    'A default trybot is being exluded by the perf biesct bot filter.')
