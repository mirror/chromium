    ...
    'android', swarming=[
      {
       ...
       'perf_tests': [
         ('tracing_perftests', 'build164-b1--device2'),
         ('gpu_perftests', 'build165-b1--device2'),
         ('cc_perftests', 'build166-b1--device2'),
        ]
      }
    ...
    'android', swarming=[
      {
       ...
       'perf_tests': [
         ('tracing_perftests', 'build112-b1--device2'),
         ('gpu_perftests', 'build113-b1--device2'),
         ('cc_perftests', 'build114-b1--device2'),
        ]
      }
    ], replace_system_webview=True)
...
def generate_telemetry_test(swarming_dimensions, benchmark_name, browser):
  ...
  if browser == 'android-webview':
    test_args.append(
        '--webview-embedded-apk=../../out/Release/apks/SystemWebView.apk')
