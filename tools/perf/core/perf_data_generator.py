      ...
      if benchmark_sharding_map:
        ...
        if not sharding_map:
          raise ValueError('No sharding map for benchmark %r found. Please'
                           ...
                           ' schedule the benchmark on the waterfall.' % (
                               name))
...
def generate_all_tests(waterfall):
  ...
  # Most builders should have a sharding configuration, specified in
  # benchmark_sharding_map.json. If a builder doesn't, you should run
  # generate_perf_sharding regenerate
