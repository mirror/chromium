...
class Speedometer2Measurement(legacy_page_test.LegacyPageTest):
  def __init__(self, should_filter_suites, filtered_suite_names=None):
    ...
    self.should_filter_suites = should_filter_suites
  ...
  def ValidateAndMeasurePage(self, page, tab, results):
    ...
    if not self.should_filter_suites:
...
class Speedometer2(perf_benchmark.PerfBenchmark):
  ...
  out suites, and only run suites who's names are matched by the regular
  expression provided.
  """
  @classmethod
  def Name(cls):
  ...
  def CreatePageTest(self, options):
      should_filter_suites = not options.suite
