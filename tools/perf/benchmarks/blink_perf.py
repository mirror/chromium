...
def _CreateMergedEventsBoundaries(events, max_start_time):
  ...
  objects. Events are merged in a union fasion.
  ...
  Args:
    events: a list or iterator of TimelineEvents
  ...
  merged_events = []     # Contains MergedEvent objects.
  ...
  for event_boundary in event_boundaries:
    ...
    if event_counter == 0:
      ...
      merged_events.append(MergedEvent(curr_bounds, thread_or_wall_duration))
      curr_bounds = None
  return merged_events
...
def _ComputeTraceEventsThreadTimeForBlinkPerf(
  ...
  for event_name in trace_events_to_measure:
    merged_events = _CreateMergedEventsBoundaries(
    ...
    for merged_event in merged_events:
      ...
      while (curr_test_runs_bound_index < len(test_runs_bounds) and
             merged_event.bounds.Intersects(
                 test_runs_bounds[curr_test_runs_bound_index])):
        intersect_wall_time = bounds.Bounds.GetOverlapBetweenBounds(
            test_runs_bounds[curr_test_runs_bound_index], merged_event.bounds)
        intersect_cpu_or_wall_time = (
            intersect_wall_time * merged_event.thread_or_wall_duration /
            merged_event.bounds.bounds)
