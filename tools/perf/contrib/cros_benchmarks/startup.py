...
from contrib.cros_benchmarks import startup_measurements
from contrib.cros_benchmarks import blank_page
from contrib.cros_benchmarks import blank_page_with_large_profile
...
class _StartupCold(perf_benchmark.PerfBenchmark):
  ...
  def CreatePageTest(self, options):
    return startup_measurements.Startup(cold=True)
...
class _StartupWarm(perf_benchmark.PerfBenchmark):
  ...
  def CreatePageTest(self, options):
    return startup_measurements.Startup(cold=False)


class StartupColdBlankPage(_StartupCold):
  """Measures cold startup time with a clean profile."""
  tag = 'cold'
  page_set = blank_page.BlankPageSet
...
class StartupLargeProfileColdBlankPage(_StartupCold):
  ...
  tag = 'cold'
  page_set = blank_page_with_large_profile.BlankPageSetWithLargeProfile
