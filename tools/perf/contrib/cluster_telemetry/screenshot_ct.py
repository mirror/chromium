...
class ScreenshotCT(perf_benchmark.PerfBenchmark):
  ...
  def AddBenchmarkCommandLineArgs(cls, parser):
    ...
    parser.add_option('--dc-wait-time', type='float', default=2,
                      help='Wait time in between screenshots')
    parser.add_option('--dc-num-screenshots', type='int', default=1,
  ...
  def CreatePageTest(self, options):
    return screenshot.Screenshot(options.png_outdir, options.wait_time,
      options.dc_detect, options.dc_wait_time, options.dc_num_screenshots,
