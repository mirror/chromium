  ...
  def ValidateAndMeasurePage(self, page, tab, results):
    ...
    if self._dc_detect:
      for i in range(self._dc_extra_screenshots):
        ...
        num_dynamic_pixels = DetectDynamicPixels(screenshot_pixels,
                                                 next_screenshot_pixels)
...
# Makes dynamic content cyan and returns the number of pixels with dynamic
# content.
def DetectDynamicPixels(pixels, next_pixels):
  num_dynamic_pixels = 0.0
  i = 0
  while i < len(pixels):
    # If the RGB values do not match, make the pixel cyan and increment the
    # count of dynamic pixels.
    if (pixels[i] != next_pixels[i] or pixels[i+1] != next_pixels[i+1] or
        pixels[i+2] != next_pixels[i+2]):
        pixels[i] = 0
        pixels[i+1] = 255
        pixels[i+2] = 255
        num_dynamic_pixels += 1
    i += 3
  return num_dynamic_pixels
