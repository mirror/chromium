...
class _HeapProfilingBenchmark(perf_benchmark.PerfBenchmark):
  ...
  page_set = page_sets.HeapProfilingPageSet
  ...
  @classmethod
  def Name(cls):
    return 'heap_profiling.' + cls.HeapProfilingMode()

  @classmethod
  def HeapProfilingMode(cls):
    pass

  def SetExtraBrowserOptions(self, options):
    super(_HeapProfilingBenchmark, self).SetExtraBrowserOptions(options)
    mode = self.HeapProfilingMode()
    if mode == 'pseudo':
      options.AppendExtraBrowserArgs(['--enable-heap-profiling'])
    elif mode != 'disabled':
      options.AppendExtraBrowserArgs(['--enable-heap-profiling=' + mode])
