...
from contrib.vr_benchmarks.vr_benchmarks.base_vr_benchmark import BaseVRBenchmark
from contrib.vr_benchmarks.vr_page_sets import vr_browsing_mode_pages


@benchmark.Owner(emails=['tiborg@chromium.org'])
class VrBrowsingModeVrPerformance(BaseVRBenchmark):
    ...
    def SetExtraBrowserOptions(self, options):
        ...
        options.AppendExtraBrowserArgs([
            '--enable-gpu-benchmarking', '--touch-events=enabled',
            '--enable-vr-shell', '--no-restore-state', '--disable-fre'
        ])
