...
class WebVrSamplePageSet(story.StorySet):
    ...
    def __init__(self):
        ...
        self.AddStory(
            WebVrSamplePage(self, ['canvasClickPresents=1', 'renderScale=1']))
        # Increased render scale
        self.AddStory(
            WebVrSamplePage(self, ['canvasClickPresents=1', 'renderScale=1.5'
                                   ]))
        # Default render scale, increased load
        self.AddStory(
            WebVrSamplePage(self, [
                'canvasClickPresents=1', 'renderScale=1', 'heavyGpu=1',
                'cubeScale=0.2', 'workTime=5'
            ]))
        # Further increased load
        self.AddStory(
            WebVrSamplePage(self, [
                'canvasClickPresents=1', 'renderScale=1', 'heavyGpu=1',
                'cubeScale=0.3', 'workTime=10'
            ]))
