...
class WebVrWprPage(page.Page):
  ...
  def __init__(self, page_set, url, name, interaction_function,
    ...
    Args:
      ...
      interaction_function: A function pointer to a function that takes an
          ActionRunner and boolean indicating whether a WPR archive is being
          recorded. Meant to handle webpage-specific VR entry
...
class WebVrWprPageSet(story.StorySet):
  ...
  def __init__(self):
    ...
    # Look at geometry things in Mass Migrations
    ...
    self.AddStory(WebVrWprPage(
        self,
        # The /iaped is necessary to keep the geometry constant - just visiting
        # the site randomly generates geometry
        'https://massmigrations.com/iaped',
        'mass_migrations',
        MassMigrationsInteraction))

    # Look at a forest thing in Under Neon Lights
    # Note that this is semi-broken in that it doesn't start the actual music
    # video when you enter VR, but we're still viewing a relatively complex
