...
const size_t kSlowFlushBufferSize = 50000;
// The length of time that must pass without receiving any bytes in order for a
// slow flush to be considered complete.
const uint16_t kSlowFlushQuietPeriodThresholdMs = 50;
// The period that we wait after receiving no bytes in a slow flush before
// checking for more bytes.
const uint16_t kSlowFlushReadDelayMs = 10;
...
void BattOrConnectionImpl::SlowFlush() {
  already_read_buffer_.clear();

  slow_flush_quiet_period_start_ = base::TimeTicks::Now();
  BeginReadBytesForSlowFlush();
}
...
void BattOrConnectionImpl::BeginReadBytesForSlowFlush() {
  LogSerial(
      StringPrintf("(flush) Starting read (quiet period has lasted %ld ms).",
                   GetSlowFlushQuietPeriodDuration().InMilliseconds()));

  pending_read_buffer_ =
      make_scoped_refptr(new net::IOBuffer(kSlowFlushBufferSize));

  io_handler_->Read(base::MakeUnique<device::ReceiveBuffer>(
      pending_read_buffer_, static_cast<uint32_t>(kSlowFlushBufferSize),
      base::BindOnce(&BattOrConnectionImpl::OnBytesReadForSlowFlush,
                     base::Unretained(this))));
}

void BattOrConnectionImpl::OnBytesReadForSlowFlush(
    int bytes_read,
    device::mojom::SerialReceiveError error) {
  if (error != device::mojom::SerialReceiveError::NONE &&
    ...
    pending_read_buffer_ = nullptr;
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::Bind(&Listener::OnSlowFlushComplete,
  ...
  if (bytes_read == 0 || error == device::mojom::SerialReceiveError::TIMEOUT) {
    // Either reading zero bytes or a serial read timeout indicate that the
    // connection was quiet.
    base::TimeDelta quiet_period_duration = GetSlowFlushQuietPeriodDuration();
    if (quiet_period_duration >
        base::TimeDelta::FromMilliseconds(kSlowFlushQuietPeriodThresholdMs)) {
    ...
    LogSerial(StringPrintf("(flush) Reading more bytes after %u ms delay.",
                           kSlowFlushReadDelayMs));
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
        FROM_HERE,
        base::BindOnce(&BattOrConnectionImpl::BeginReadBytesForSlowFlush,
                       AsWeakPtr()),
        base::TimeDelta::FromMilliseconds(kSlowFlushReadDelayMs));
    return;
  }
...
base::TimeDelta BattOrConnectionImpl::GetSlowFlushQuietPeriodDuration() {
  return base::TimeTicks::Now() - slow_flush_quiet_period_start_;
}
