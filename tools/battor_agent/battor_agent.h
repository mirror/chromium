...
class BattOrResults {
 public:
  ...
  BattOrResults(std::string details,
                std::vector<float> samples_watts,
                uint32_t sample_rate)
      : details_(std::move(details)),
        samples_watts_(std::move(samples_watts)),
        sample_rate_(sample_rate) {}
  const std::string& GetDetails() const { return details_; }
  const std::vector<float>& GetSamplesWatts() const { return samples_watts_; }
  const uint32_t GetSampleRate() const { return sample_rate_; }

 private:
  std::string details_;
  std::vector<float> samples_watts_;
