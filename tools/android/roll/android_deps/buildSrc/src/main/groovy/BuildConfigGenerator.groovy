...
class BuildConfigGenerator extends DefaultTask {
    ...
    void main() {
        ...
        updateBuildTargetDeclaration(graph, new File("${normalisedPath}/BUILD.gn"))
        new File("${normalisedPath}/additional_license_paths.json").write(
                JsonOutput.prettyPrint(JsonOutput.toJson(dependencyDirectories)))
    }

    private static String updateBuildTargetDeclaration(ChromiumDepGraph depGraph, File buildFile) {
