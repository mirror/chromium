...
def _ReplaceVersionInFile(file_path, pattern, version):
  ...
  try:
    ...
    shutil.move(tmp, file_path) # Move new file to original file path
...
def UploadSdkPackage(dry_run, service_url, packages, yaml_file, version,
  ...
  for package in packages:
    ...
    if dry_run:
      dry_run_package_info.append(
          '* Package "%s" version:%s ' % (package, pkg_version))
    else:
      upload_sdk_cmd = [
        'cipd', 'create',
        '-pkg-def', yaml_file,
    ...
    if package in _GN_ARGUMENTS_TO_UPDATE:
      ...
      if not _CheckSameVersionInFile(_ANDROID_CONFIG_GNI_PATH,
        ...
        if dry_run:
          dry_run_package_info.append(
              '    File %s argument %s would be updated to %s.' % (
  ...
  if dry_run:
    print 'Script without --dry-run would create and ',
    print 'upload the package(s) to CIPD:\n'
    for info in dry_run_package_info:
      print info
