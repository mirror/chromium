...
def UploadSdkPackage(sdk_root, dry_run, service_url, package, yaml_file,
  ...
  """
  if not yaml_file:
    pkg_yaml_file = os.path.join(sdk_root, 'cipd_' + package + '.yaml')
...
def ChangeVersionInGNI(package, arg_version, gn_args_dict, gni_file_path,
  ...
  if package in gn_args_dict:
    ...
    if not _CheckSameVersionInFile(gni_file_path, gn_arg_pattern,
                                   arg_version):
      if dry_run:
        print '    Note: file %s argument %s would be updated to %s.' % (
              gni_file_path, version_config_name, arg_version)
      else:
        _ReplaceVersionInFile(gni_file_path, gn_arg_pattern,
                              '"%s"' % arg_version)
