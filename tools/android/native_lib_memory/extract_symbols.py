...
def DemangleSymbolNames(symbol_names):
  ...
  """
  return dict(
      zip(symbol_names, symbol_extractor.DemangleSymbolNames(symbol_names)))
...
def GetSymbolNameToFilename(build_directory):
  ...
  """
  object_filenames = cyglog_to_orderfile._GetObjectFileNames(build_directory)
...
def main():
  ...
  if args.start_server:
    ...
    logging.warning('Serving on port 8000')
    httpd.serve_forever()
