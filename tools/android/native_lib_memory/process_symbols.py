...
def WriteCodePageAttribution(page_to_object_files, text_filename,
  ...
  Args:
    page_to_object_files: As returned byu CodePagesToObjectFiles().
    text_filename: (str) Text output filename.
    json_filenane: (str) JSON output filename.
  """
  json_data = []
...
def main():
  ...
  parser.add_argument('--output_directory', type=str, help='Build directory',
                      required=True)
  parser.add_argument('--json_output', type=str, help='JSON output file',
                      required=True)
  parser.add_argument('--text_output', type=str, help='Text output file',
                      required=True)
  ...
  WriteCodePageAttribution(page_to_object_files, args.text_output,
                           args.json_output)
  return 0
