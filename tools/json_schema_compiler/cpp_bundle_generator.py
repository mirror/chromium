...
class _SchemasCCGenerator(object):
  ...
  def Generate(self, _):  # namespace not relevant, this is a bundle
    ...
    c.Append('  const char* schema;')
    c.Append('  size_t schema_length;')
    c.Sblock('} kSchemas[] = {')
    ...
    for namespace in sorted(namespaces):
      ...
      c.Append('{"%s", %s, sizeof(%s) - 1},' %
               (namespace, schema_constant_name, schema_constant_name))
    c.Eblock('};')
    c.Sblock('for (const auto& schema : kSchemas) {')
    c.Sblock('if (schema.name == name)')
    c.Append('return base::StringPiece(schema.schema, schema.schema_length);')
    c.Eblock()
    c.Eblock('}')
