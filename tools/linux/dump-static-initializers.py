...
def test_ParseNmLine():
  ...
  parse = ParseNmLine('0000000001919920 0000000000000008 t '
                      '_ZN12_GLOBAL__I_safe_browsing_service.cc')
  assert parse == ('safe_browsing_service.cc', 26319136, 8), parse

  parse = ParseNmLine('00000000026b9eb0 0000000000000024 t '
...
def ParseNm(toolchain, binary):
  ...
  nm = subprocess.Popen(
      [toolchain + 'nm', '-S', binary], stdout=subprocess.PIPE)
...
def ExtractSymbolReferences(toolchain, binary, start, end):
  ...
  cmd = [
      toolchain + 'objdump', binary, '--disassemble',
