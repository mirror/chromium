...
BeginFrameTracker::BeginFrameTracker(const tracked_objects::Location& location)
    : location_(location), location_string_(location.ToString()) {}
...
void BeginFrameTracker::Start(BeginFrameArgs new_args) {
  ...
  TRACE_EVENT_FLOW_STEP0(
      ...
      ew_args.frame_time.since_origin().InMicroseconds(), location_string_);
  ...
  DCHECK(current_updated_at_.is_null() || HasFinished())
