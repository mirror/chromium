...
SchedulerStateMachine::Action SchedulerStateMachine::NextAction() const {
  ...
  if (ShouldDraw()) {
    if (PendingDrawsShouldBeAborted())
      return Action::DRAW_ABORT;
    else if (forced_redraw_state_ == ForcedRedrawOnTimeoutState::WAITING_FOR_DRAW)
      return Action::DRAW_FORCED;
    else
      return Action::DRAW_IF_POSSIBLE;
  }
...
void SchedulerStateMachine::WillCommit(bool commit_has_no_updates) {
  ...
  if (layer_tree_frame_sink_state_ ==
    ...
    layer_tree_frame_sink_state_ =
        has_pending_tree_ ? LayerTreeFrameSinkState::WAITING_FOR_FIRST_ACTIVATION
...
void SchedulerStateMachine::WillActivate() {
  ...
  if (forced_redraw_state_ == ForcedRedrawOnTimeoutState::WAITING_FOR_ACTIVATION)
...
void SchedulerStateMachine::DidCreateAndInitializeLayerTreeFrameSink() {
  ...
  layer_tree_frame_sink_state_ = LayerTreeFrameSinkState::WAITING_FOR_FIRST_COMMIT;
