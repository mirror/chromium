...
void SchedulerStateMachine::WillActivate() {
  ...
  // Wait for at least 2 consecutive commits from the main thread before
  // resetting |did_block_commit_on_impl_side_tree_|. The first WillActivate
  // will come from the commit that the invalidation had blocked.
  bool commit_was_blocked_on_impl_tree =
      begin_main_frame_state_ == BeginMainFrameState::READY_TO_COMMIT &&
      current_pending_tree_is_impl_side_;
  if (commit_was_blocked_on_impl_tree || !previous_pending_tree_was_impl_side_)
    did_block_commit_on_impl_side_tree_ = commit_was_blocked_on_impl_tree;
