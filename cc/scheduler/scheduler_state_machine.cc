...
bool SchedulerStateMachine::ShouldDeferInvalidatingForMainFrame() const {
  ...
  if (last_impl_frame_state_.commit_had_no_updates)
    return false;
  ...
  if (last_impl_frame_state_.did_commit_during_frame)
    return true;
...
void SchedulerStateMachine::OnBeginImplFrame(uint32_t source_id,
  ...
  // frame.
  last_impl_frame_state_.commit_had_no_updates = last_commit_had_no_updates_;
  last_impl_frame_state_.did_commit_during_frame = did_commit_during_frame_;
