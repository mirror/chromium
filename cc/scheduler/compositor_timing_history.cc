...
class CompositorTimingHistory::UMAReporter {
 public:
  ...
  virtual void AddCommitIntervalWithMainThreadAnimations(
      base::TimeDelta duration) = 0;
  virtual void AddCommitIntervalWithMainThreadCompositableAnimations(
      base::TimeDelta duration) = 0;
...
class RendererUMAReporter : public CompositorTimingHistory::UMAReporter {
 public:
  ...
  void AddCommitIntervalWithMainThreadCompositableAnimations(
      base::TimeDelta interval) override {
    UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED(
        "Scheduling.Renderer."
        "CommitIntervalWithMainThreadCompositableAnimations",
        interval);
  }
  ...
  void AddDrawIntervalWithMainThreadCompositableAnimations(
    ...
    UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED(
        ...
        "WithMainThreadCompositableAnimations",
        interval);
  }
      ...
      if (!new_active_tree_draw_end_time_prev_.is_null()) {
        ...
        if (main_thread_animations_count > 0)
          uma_reporter_->AddCommitIntervalWithMainThreadAnimations(
              draw_duration);
        if (main_thread_compositable_animations_count > 0)
          uma_reporter_->AddCommitIntervalWithMainThreadCompositableAnimations(
              draw_duration);
      }
