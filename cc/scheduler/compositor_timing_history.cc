...
class CompositorTimingHistory::UMAReporter {
 public:
  ...
  // crbug.com/758439: the following 6 functions are used to reporter timing
  // in certain conditions targeting blink / compositor animations.
  ...
  virtual void AddDrawIntervalWithMainThreadAnimations(
      base::TimeDelta duration) = 0;
  virtual void AddDrawIntervalWithMainThreadCouldBeCompositedAnimations(
      base::TimeDelta duration) = 0;
  virtual void AddCommitIntervalWithCompositedAnimations(
      base::TimeDelta duration) = 0;
  virtual void AddCommitIntervalWithMainThreadAnimations(
      base::TimeDelta duration) = 0;
  virtual void AddCommitIntervalWithMainThreadCouldBeCompositedAnimations(
      base::TimeDelta duration) = 0;
...
class RendererUMAReporter : public CompositorTimingHistory::UMAReporter {
 public:
  ...
  void AddCommitIntervalWithCompositedAnimations(
      base::TimeDelta interval) override {
    UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED(
        "Scheduling.Renderer.CommitIntervalWithCompositedAnimations", interval);
  }

  void AddCommitIntervalWithMainThreadAnimations(
      base::TimeDelta interval) override {
    UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED(
        "Scheduling.Renderer.CommitIntervalWithMainThreadAnimations", interval);
  }

  void AddCommitIntervalWithMainThreadCouldBeCompositedAnimations(
      base::TimeDelta interval) override {
    UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED(
        "Scheduling.Renderer."
        "CommitIntervalWithMainThreadCouldBeCompositedAnimations",
        interval);
  }
  ...
  void AddDrawIntervalWithCompositedAnimations(
      base::TimeDelta interval) override {
    UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED(
        "Scheduling.Renderer.DrawInterval.WithCompositorAnimations", interval);
  }

  void AddDrawIntervalWithMainThreadAnimations(
      base::TimeDelta interval) override {
    UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED(
        "Scheduling.Renderer.DrawInterval.WithMainThreadAnimations", interval);
  }

  void AddDrawIntervalWithMainThreadCouldBeCompositedAnimations(
      base::TimeDelta interval) override {
    UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED(
        "Scheduling.Renderer.DrawInterval."
        "WithMainThreadCouldBeCompositedAnimations",
        interval);
  }
...
class BrowserUMAReporter : public CompositorTimingHistory::UMAReporter {
 public:
  ...
  void AddCommitIntervalWithCompositedAnimations(
    ...
    UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED(
        "Scheduling.Browser.CommitIntervalWithCompositedAnimations", interval);
  }
  ...
  void AddCommitIntervalWithMainThreadAnimations(
    ...
    // Still report, but the data is not meaningful.
    UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED(
        "Scheduling.Browser.CommitIntervalWithMainThreadAnimations", interval);
  }
  ...
  void AddDrawIntervalWithMainThreadCouldBeCompositedAnimations(
    ...
    UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED(
        "Scheduling.Browser."
        "DrawIntervalWithMainThreadCouldBeCompositedAnimations",
        interval);
  }
...
void CompositorTimingHistory::DidDraw(
  ...
  if (!draw_end_time_prev_.is_null()) {
    ...
    if (num_composited_animations > 0)
      uma_reporter_->AddDrawIntervalWithCompositedAnimations(draw_interval);
    if (num_main_thread_animations > 0)
      uma_reporter_->AddDrawIntervalWithMainThreadAnimations(draw_interval);
    if (num_main_thread_could_be_composited_animations > 0)
      uma_reporter_->AddDrawIntervalWithMainThreadCouldBeCompositedAnimations(
          draw_interval);
  }
  ...
  if (used_new_active_tree) {
    ...
    if (begin_main_frame_committing_continuously_) {
      if (!new_active_tree_draw_end_time_prev_.is_null()) {
        ...
        if (num_composited_animations > 0)
          uma_reporter_->AddCommitIntervalWithCompositedAnimations(
              draw_duration);
        if (num_main_thread_animations > 0)
          uma_reporter_->AddCommitIntervalWithMainThreadAnimations(
              draw_duration);
        if (num_main_thread_could_be_composited_animations > 0)
          uma_reporter_
              ->AddCommitIntervalWithMainThreadCouldBeCompositedAnimations(
                  draw_duration);
      }
