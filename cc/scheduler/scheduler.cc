...
void Scheduler::OnBeginImplFrameDeadline() {
  ...
  if (state_machine_.needs_redraw() && state_machine_.IsDrawThrottled()) {
    ...
    if (state_machine_.has_pending_tree()) {
      TRACE_EVENT_ASYNC_BEGIN_WITH_TIMESTAMP0("cc", "Scheduler:Impl:RasterJank",
                                              this,
                                              compositor_timing_history_->begin_impl_frame_start_time());
      TRACE_EVENT_ASYNC_END_WITH_TIMESTAMP0("cc", "Scheduler:Impl:RasterJank",
                                            this, Now());
    }
    else if (state_machine_.CommitPending()) {
      TRACE_EVENT_ASYNC_BEGIN_WITH_TIMESTAMP0("cc", "Scheduler:Impl:CommitJank",
                                              this,
                                              compositor_timing_history_->begin_impl_frame_start_time());
      TRACE_EVENT_ASYNC_END_WITH_TIMESTAMP0("cc", "Scheduler:Impl:CommitJank",
                                            this, Now());
    }
  }
