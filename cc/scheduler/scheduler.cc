...
void Scheduler::SetupNextBeginFrameIfNeeded() {
  ...
  if (needs_begin_frames && !observing_begin_frame_source_) {
    ...
    DCHECK(!skipped_deadline_on_next_begin_frame_);
...
void Scheduler::ScheduleBeginImplFrameDeadline() {
  ...
  skipped_deadline_on_next_begin_frame_ = false;
  ...
  if (deadline_.is_null() ||
    ...
    // The deadline is exactly on next BeginFrame, OK to skip scheduling.
    TRACE_EVENT1("cc", "Scheduler::ScheduleBeginImplFrameDeadline",
                 "deadline_mode", "skipped");
    skipped_deadline_on_next_begin_frame_ = true;
  }
}
