...
void Scheduler::BeginImplFrameWithDeadline(const viz::BeginFrameArgs& args) {
  ...
  // If we don't expect the main thread to respond within this frame, and we did
  // not block the commit on an impl-side tree on the last frame, don't block
  // the impl-side invalidation on a commit.
