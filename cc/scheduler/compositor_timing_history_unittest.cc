...
void TestAnimationTimingUMA(const base::HistogramTester& histogram_tester,
                            int expected_timing1,
                            int expected_count1,
                            int expected_timing2,
                            int expected_count2,
                            int expected_timing3,
                            int expected_count3) {
  if (expected_count1 != 0) {
    histogram_tester.ExpectBucketCount(
        "Scheduling.Renderer.DrawIntervalWithCompositedAnimations2",
        expected_timing1, expected_count1);
  }
  if (expected_count2 != 0) {
    histogram_tester.ExpectBucketCount(
        "Scheduling.Renderer.DrawIntervalWithMainThreadAnimations2",
        expected_timing2, expected_count2);
  }
  if (expected_count3 != 0) {
    histogram_tester.ExpectBucketCount(
        "Scheduling.Renderer.DrawIntervalWithMainThreadCompositableAnimations2",
        expected_timing3, expected_count3);
  }
}
...
TEST_F(CompositorTimingHistoryTest, AnimationUMA) {
  ...
  TestAnimationTimingUMA(histogram_tester, 230, 0, 230, 1, 230, 0);
  ...
  TestAnimationTimingUMA(histogram_tester, 230, 0, 230, 1, 145, 1);
  ...
  TestAnimationTimingUMA(histogram_tester, 456, 1, 456, 1, 145, 1);

  timing_history_.SetDrawTestTime();
  timing_history_.SetActiveTreeMainFrameTime();
  AdvanceNowBy(base::TimeDelta::FromMicroseconds(1538));
  timing_history_.DidDraw(true, true, Now(), 1, 1, 1);
  TestAnimationUMA(histogram_tester, 2, 3, 2);
  TestAnimationTimingUMA(histogram_tester, 1538, 1, 1538, 1, 1538, 1);
}
