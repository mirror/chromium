...
class LayerTreeHostCommonTestBase : public LayerTestCommon::LayerImplTest {
 public:
  ...
  void ExecuteCalculateDrawPropertiesWithoutAdjustingRasterScales(
    ...
    LayerTreeHostCommon::CalculateDrawPropertiesForTesting(
        &inputs, build_property_trees_if_needed);
  }
...
TEST_F(LayerTreeHostCommonTest, SurfaceContentsScaleChangeWithCopyRequestTest) {
  ...
  root->layer_tree_impl()
      ->property_trees()
      ->effect_tree.Node(copy_layer->effect_tree_index())
      ->has_copy_request = false;
  host_impl()->active_tree()->property_trees()->effect_tree.set_needs_update(
      true);
  ExecuteCalculateDrawPropertiesWithoutAdjustingRasterScales(root, false);
