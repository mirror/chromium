...
class UkmManagerTest : public testing::Test {
 public:
  UkmManagerTest() {
    ...
    manager_ = std::make_unique<UkmManager>();
...
TEST_F(UkmManagerTest, Basic) {
  ...
  for (const auto* entry : entries) {
    ...
    EXPECT_NE(entry->source_id, ukm::kInvalidSourceId);
  ...
  EXPECT_EQ(test_ukm_recorder_->entries_count(), 0u);
  manager_->SetUserInteractionInProgress(true);
  EXPECT_EQ(test_ukm_recorder_->entries_count(), 0u);
  ...
  EXPECT_EQ(entries.size(), 1u);
  for (const auto* entry : entries) {
    EXPECT_EQ(entry->source_id, original_id);
