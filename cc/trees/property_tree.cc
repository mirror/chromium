...
void EffectTree::TakeCopyRequestsAndTransformToSurface(
  ...
  if (transform.IsPositiveScaleOrTranslation()) {
    ...
    for (auto it = range.first; it != range.second; ++it) {
      ...
      // copy of the exact source pixels.
      if (request->is_scaled() || request->has_result_selection()) {
        const gfx::Vector2d scale_from(
            gfx::ToRoundedInt(request->scale_from().x() * surface_rect.width()),
            gfx::ToRoundedInt(request->scale_from().y() *
                              surface_rect.height()));
        const gfx::Vector2d scale_to(
            request->scale_to().x() * kContentVector.x(),
            request->scale_to().y() * kContentVector.y());
