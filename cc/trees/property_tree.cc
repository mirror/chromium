...
gfx::Vector2dF StickyPositionOffset(TransformTree* tree, TransformNode* node) {
  ...
  // Position sticky should never attach to the inner viewport. If we did then
  // we'd need setting the inner viewport bounds delta to cause a TransformTree
  // update, which it currently doesn't.
  DCHECK_FALSE(scroll_node->scrolls_inner_viewport);
  gfx::RectF clip(
      scroll_position,
