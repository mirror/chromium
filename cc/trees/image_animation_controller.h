...
class CC_EXPORT ImageAnimationController {
 public:
  ...
  // This is called after every DrawProperties update since that could have
  // changed the visibility for an image, in which case we would now want to
  // animate it.
 ...
 private:
  struct AnimationState {
    ...
    PaintImage::Id paint_image_id = PaintImage::kNonLazyStableId;
    ...
    base::flat_set<AnimationDriver*> drivers;

    // The index being used on the active tree, if a recording with this image
    // is still present.
    size_t active_index = PaintImage::kDefaultFrameIndex;
    ...
    // ShouldAnimate state from the last update.
