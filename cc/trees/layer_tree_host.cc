...
LayerTreeHost::CreateLayerTreeHostImpl(
  ...
  host_impl->SetContentRequiresCrispEdges(content_requires_crisp_edges_);
...
bool LayerTreeHost::DoUpdateLayers(Layer* root_layer) {
  ...
  bool content_requires_crisp_edges = false;
  bool did_paint_content =
      PaintContent(update_layer_list, &content_has_slow_paths,
                   &content_requires_crisp_edges);
  ...
  content_requires_crisp_edges_ |= content_requires_crisp_edges;
...
bool LayerTreeHost::PaintContent(const LayerList& update_layer_list,
                                 ...
                                 bool* content_requires_crisp_edges) {
  base::AutoReset<bool> painting(&in_paint_layer_contents_, true);
  bool did_paint_content = false;
  for (const auto& layer : update_layer_list) {
    did_paint_content |= layer->Update();
    *content_has_slow_paths |= layer->HasSlowPaths();
    *content_requires_crisp_edges |= layer->RequiresCrispEdges();
  }
  return did_paint_content;
}
