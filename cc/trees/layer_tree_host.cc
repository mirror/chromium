...
bool LayerTreeHost::DoUpdateLayers(Layer* root_layer) {
  ...
  if (GetSettings().should_squash_picture_layer) {
    for (auto& it : update_layer_list) {
      if (it->IsPictureLayer()) {
        first_picture_layer = static_cast<PictureLayer*>(it.get());
        int estimated_memory_usage =
            it->bounds().width() * it->bounds().height() * 4 / 1024;
        int reported_memory_usage = static_cast<int>(
            first_picture_layer->GetPainterReportedMemoryUsage());
        // Don't squash anything into a picture layer that doesn't report any
        // memory usage. (i.e. those internal layers?)
        if (reported_memory_usage == 0 ||
            std::abs(reported_memory_usage - estimated_memory_usage) > 1000) {
          first_picture_layer = nullptr;
          break;
        }
  ...
  if (GetSettings().should_squash_picture_layer && first_picture_layer) {
    ...
    for (auto& it : update_layer_list) {
      if (!it->IsPictureLayer())
        continue;
      PictureLayer* current_picture_layer =
          static_cast<PictureLayer*>(it.get());
      if (current_picture_layer == first_picture_layer)
        continue;
