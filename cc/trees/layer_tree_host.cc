...
void LayerTreeHost::FinishCommitOnImplThread(
  ...
  {
    base::AutoLock auto_lock(presentation_time_lock_);
...
void LayerTreeHost::DidPresentCompositorFrame(
    ...
    uint32_t flags) {
  std::vector<PresentationTimeCallback> callbacks;
  {
    base::AutoLock auto_lock(presentation_time_lock_);
    for (int frame : source_frames) {
      ...
      auto& frame_callbacks = frame_to_presentation_time_callbacks_[frame];
      callbacks.insert(callbacks.end(),
                       std::make_move_iterator(frame_callbacks.begin()),
                       std::make_move_iterator(frame_callbacks.end()));
