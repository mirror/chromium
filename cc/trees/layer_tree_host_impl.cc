...
DrawResult LayerTreeHostImpl::PrepareToDraw(FrameData* frame) {
  ...
  if (const char* client_name = GetClientNameForMetrics()) {
    ...
    for (const PictureLayerImpl* layer : active_tree()->picture_layers()) {
      ...
      if (layer->mask_type() != Layer::LayerMaskType::NOT_MASK) {
        ...
        if (layer->mask_type() == Layer::LayerMaskType::MULTI_TEXTURE_MASK) {
          if (layer->GPUMemoryUsageInBytesSavedByMaskTiling() > 0)
            total_gpu_memory_saved_by_mask_tiling +=
                layer->GPUMemoryUsageInBytesSavedByMaskTiling();
        }
      }
    }
    ...
    if (num_mask_layers) {
      UMA_HISTOGRAM_COUNTS(
          base::StringPrintf("Compositing.%s.MaskMemoryUsageInKb", client_name),
          base::saturated_cast<int>(total_gpu_memory_for_mask_tilings_in_bytes /
                                    1024));
      UMA_HISTOGRAM_COUNTS(
