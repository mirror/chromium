...
LayerTreeHostImpl::LayerTreeHostImpl(
    ...
    : client_(client),
      ...
      id_(id),
      trace_name_("Compositor" + std::to_string(id_)),
...
bool LayerTreeHostImpl::DrawLayers(FrameData* frame) {
  ...
  metadata.latency_info.emplace_back(ui::SourceEventType::FRAME, ++g_trace_id);
  ...
  if (CommitToActiveTree()) {
    ...
    // Only add INPUT_EVENT_LATENCY_RENDERER_SWAP_COMPONENT for renderer swap,
    // not the browser swap.
