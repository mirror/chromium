...
void ProxyMain::BeginMainFrame(
  ...
  // Synchronizes scroll offsets from the compositor thread thread to the main
  // thread.
  ...
  // Among other things, BeginMainFrame will:
  ...
  // Recreates all UI resources if there were evicted UI resources when the impl
  // thread initiated the commit.
  if (begin_main_frame_state->evicted_ui_resources)
    layer_tree_host_->GetUIResourceManager()->RecreateUIResources();

  // Among other things, RequestMainFrameUpdate will:
  ...
  // Among other things, UpdateLayers will:
  // Update property trees in cc.
  // Copy Blink display item lists to corresponding cc display item lists.
  // For non-PictureLayer Layer subclasses, update some internal
  // state, and also for PainteScrollbarLayers, paint them.
