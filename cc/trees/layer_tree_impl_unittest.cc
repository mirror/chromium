...
TEST_F(LayerTreeImplTest, HitCheckingTouchHandlerRegionsForSingleLayer) {
  RegionMap touch_handler_region_map;
  touch_handler_region_map[kTouchActionNone] = gfx::Rect(10, 10, 50, 50);
  ...
  root->SetTouchEventHandlerRegionMap(touch_handler_region_map);
...
TEST_F(LayerTreeImplTest,
  ...
  {
    RegionMap touch_handler_region_map;
    touch_handler_region_map[kTouchActionNone] = gfx::Rect(10, 10, 30, 30);
    ...
    test_layer->SetTouchEventHandlerRegionMap(touch_handler_region_map);
...
TEST_F(LayerTreeImplTest, HitCheckingTouchHandlerRegionsForSimpleClippedLayer) {
  ...
  root->SetBounds(gfx::Size(100, 100));
  {
    ...
    child->SetBounds(gfx::Size(300, 300));
    child->SetDrawsContent(true);
    child->SetTouchEventHandlerRegionMap(touch_handler_region_map);
    clipping_layer->test_properties()->AddChild(std::move(child));
    root->test_properties()->AddChild(std::move(clipping_layer));
  }
...
TEST_F(LayerTreeImplTest,
  ...
  surface->test_properties()->force_render_surface = true;
  {
    ...
    RegionMap touch_handler_region_map;
    touch_handler_region_map[kTouchActionNone] = gfx::Rect(0, 0, 300, 300);
...
TEST_F(LayerTreeImplTest, HitCheckingTouchHandlerOverlappingRegions) {
  ...
  root->SetBounds(gfx::Size(100, 100));
  {
    ...
    RegionMap touch_handler_region_map;
    touch_handler_region_map[kTouchActionNone] = gfx::Rect(0, 0, 50, 50);
    touch_layer->SetTouchEventHandlerRegionMap(touch_handler_region_map);
