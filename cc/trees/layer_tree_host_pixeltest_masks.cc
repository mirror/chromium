...
class MaskContentLayerClient : public ContentLayerClient {
 public:
  ...
  scoped_refptr<DisplayItemList> PaintContentsToDisplayList(
    ...
    buffer->push<DrawColorOp>(SK_ColorTRANSPARENT, SkBlendMode::kSrc);
  ...
  scoped_refptr<DisplayItemList> PaintContentsToDisplayList(
    ...
    if (vertical_) {
      for (int i = 4; i < bounds_.width(); i += 16)
        buffer->push<DrawLineOp>(i, 0, i, bounds_.height(), flags);
    } else {
      for (int i = 4; i < bounds_.height(); i += 16)
        buffer->push<DrawLineOp>(0, i, bounds_.width(), i, flags);
    }
...
class CircleContentLayerClient : public ContentLayerClient {
 public:
  ...
  scoped_refptr<DisplayItemList> PaintContentsToDisplayList(
    ...
    flags.setColor(SK_ColorWHITE);
    buffer->push<DrawCircleOp>(bounds_.width() / 2, bounds_.height() / 2,
