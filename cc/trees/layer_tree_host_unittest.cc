...
class LayerTreeHostTestAnimateOnlyBeginFrames
 ...
 public:
  ...
  void IssueBeginFrame(bool animate_only) {
    first_begin_frame_issued_ = true;
  ...
  void OnNeedsBeginFrames(bool needs_begin_frames) override {
    if (needs_begin_frames) {
      EXPECT_FALSE(first_begin_frame_issued_);
  ...
  void DidFinishImplFrameOnThread(LayerTreeHostImpl* host_impl) override {
    if (sent_begin_main_frame_count_ < 3) {
      if (sent_begin_main_frame_count_ == 2) {
      ...
      PostIssueBeginFrame(true);
    } else if (sent_begin_main_frame_count_ == 3) {
  ...
  void UpdateLayerTreeHost(
    ...
    if (sent_begin_main_frame_count_ < 4) {
      // First three BeginFrames are animate_only, so only layout updates are
      // requested.
      EXPECT_EQ(LayerTreeHostClient::VisualStateUpdate::kLayout,
                requested_update);
    } else {
  ...
  void AfterTest() override {
    EXPECT_TRUE(first_begin_frame_issued_);
    EXPECT_EQ(1, commit_count_);
    EXPECT_EQ(4, sent_begin_main_frame_count_);
 ...
 private:
  ...
  int ready_to_commit_count_ = 0;
  bool first_begin_frame_issued_ = false;
};
