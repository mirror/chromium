...
// In single thread mode, this test sends a BeginMainFrame outside of an impl
// frame, so only run multi thread mode.
...
// In single thread mode, the root layer is initialized earlier, so CanDraw
// is always true during this test.
