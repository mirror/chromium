...
void LayerTreeImpl::UpdateScrollbarGeometries(bool force_update) {
  ...
  for (auto& pair : element_id_to_scrollbar_layer_ids_) {
    ...
    // The viewport scrollbars are special because all four layers can affect
    // scrolling. Begin with the inner container and outer scroll.
    if (is_viewport) {
    ...
    for (auto* scrollbar : ScrollbarsFor(scrolling_element_id)) {
      ...
      scrollbar->SetVerticalAdjust(bounds_layer->ViewportBoundsDelta().y());
    }
