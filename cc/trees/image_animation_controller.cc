...
void ImageAnimationController::UpdateStateFromDrivers(base::TimeTicks now) {
  ...
  for (auto& it : animation_state_map_) {
    AnimationState& state = it.second;
    bool previous_state = state.ShouldAnimate();

    // Pull the latest value from the drivers.
    state.should_animate_from_drivers = false;
    for (auto* driver : state.drivers)
      state.should_animate_from_drivers |= driver->ShouldAnimate(it.first);
    if (state.ShouldAnimate() == previous_state)
      continue;
...
bool ImageAnimationController::AnimationState::ShouldAnimate() const {
  ...
  if (!should_animate_from_drivers)
    return false;
