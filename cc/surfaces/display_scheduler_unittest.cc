...
class FakeDisplaySchedulerClient : public DisplaySchedulerClient {
 public:
  ...
  void set_wait_for_all_surfaces_before_swap(
      bool wait_for_all_surfaces_before_swap) {
    settings_.wait_for_all_surfaces_before_swap =
        wait_for_all_surfaces_before_swap;
  }
...
TEST_F(DisplaySchedulerTest, WaitForAllSurfacesBeforeSwap) {
  SurfaceId root_surface_id(
      kArbitraryFrameSinkId,
      LocalSurfaceId(1, base::UnguessableToken::Create()));
  SurfaceId sid1(kArbitraryFrameSinkId,
  ...
  // Enable wait_for_all_surfaces_before_swap mode.
  client_.set_wait_for_all_surfaces_before_swap(true);
