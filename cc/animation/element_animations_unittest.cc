...
// See animation_single_player_unittest.cc for integration with
...
TEST_F(ElementAnimationsTest, AttachToLayerInActiveTree) {
  ...
  EXPECT_EQ(element_animations_, single_player_->element_animations());
  ...
  EXPECT_EQ(element_animations_impl_,
            single_player_impl_->element_animations());
  EXPECT_TRUE(element_animations_impl_->has_element_in_active_list());
  EXPECT_TRUE(element_animations_impl_->has_element_in_pending_list());
...
TEST_F(ElementAnimationsTest, AddRemovePlayers) {
  ...
  scoped_refptr<ElementAnimations> element_animations =
      single_player_->element_animations();
  ...
  EXPECT_EQ(element_animations, single_player_->element_animations());
  EXPECT_EQ(element_animations, player1->element_animations());

  PushProperties();
  EXPECT_EQ(element_animations_impl, single_player_impl_->element_animations());
...
TEST_F(ElementAnimationsTest, SyncNewAnimation) {
  ...
  EXPECT_FALSE(single_player_->GetAnimation(TargetProperty::OPACITY));
  EXPECT_FALSE(single_player_impl_->GetAnimation(TargetProperty::OPACITY));

  int animation_id =
      AddOpacityTransitionToPlayer(single_player_.get(), 1, 0, 1, false);
  ...
  PushProperties();
  single_player_impl_->ActivateAnimations();
...
TEST_F(ElementAnimationsTest,
  ...
  EXPECT_FALSE(single_player_->GetAnimation(TargetProperty::SCROLL_OFFSET));
  EXPECT_FALSE(
      single_player_impl_->GetAnimation(TargetProperty::SCROLL_OFFSET));
  ...
  single_player_->AddAnimation(std::move(animation_fixed));
  PushProperties();
  ...
  single_player_->AddAnimation(std::move(animation));
  PushProperties();
...
class TestAnimationDelegateThatDestroysPlayer : public TestAnimationDelegate {
 public:
  ...
  void setTimelineAndPlayer(scoped_refptr<AnimationTimeline> timeline,
    ...
    timeline_ = timeline;
    single_player_ = player;
  }

 private:
  scoped_refptr<AnimationTimeline> timeline_;
  scoped_refptr<SingleAnimationPlayer> single_player_;
};
...
TEST_F(ElementAnimationsTest, DoNotClobberStartTimes) {
  ...
  auto events = CreateEventsForTesting();
  single_player_impl_->Tick(kInitialTickTime);
  single_player_impl_->UpdateState(true, events.get());
  ...
  single_player_impl_->UpdateState(true, nullptr);
  EXPECT_EQ(ticker_->GetAnimationById(animation_id)->start_time(),
            ticker_impl_->GetAnimationById(animation_id)->start_time());
}
...
TEST_F(ElementAnimationsTest, UseSpecifiedStartTimes) {
  ...
  single_player_->GetAnimation(TargetProperty::OPACITY)
  ...
  // Start the animation on the main thread. Should not affect the start time.
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(500));
  single_player_->UpdateState(true, nullptr);
  EXPECT_EQ(start_time, ticker_->GetAnimationById(animation_id)->start_time());
  EXPECT_EQ(ticker_->GetAnimationById(animation_id)->start_time(),
            ticker_impl_->GetAnimationById(animation_id)->start_time());
}
...
TEST_F(ElementAnimationsTest, Activation) {
  ...
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(1000));
  single_player_->UpdateState(true, nullptr);
  EXPECT_EQ(Animation::FINISHED,
            single_player_->GetAnimation(TargetProperty::OPACITY)->run_state());
  EXPECT_EQ(1u, host->ticking_players_for_testing().size());
  ...
  EXPECT_EQ(
      ...
      single_player_impl_->GetAnimation(TargetProperty::OPACITY)->run_state());
  // The impl thread animations should have de-activated.
  EXPECT_EQ(0u, host_impl->ticking_players_for_testing().size());

  EXPECT_EQ(1u, events->events_.size());
  ticker_->NotifyAnimationFinished(events->events_[0]);
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(1500));
  single_player_->UpdateState(true, nullptr);
...
TEST_F(ElementAnimationsTest, SyncPause) {
  ...
  const int animation_id =
      AddOpacityStepsToPlayer(single_player_.get(), duration, 0.2f, 0.4f, 2);
  ...
  auto events = CreateEventsForTesting();
  single_player_impl_->Tick(time);
  single_player_impl_->UpdateState(true, events.get());
  EXPECT_EQ(1u, events->events_.size());

  single_player_->Tick(time);
  single_player_->UpdateState(true, nullptr);
  ticker_->NotifyAnimationStarted(events->events_[0]);
  ...
  single_player_->PauseAnimation(animation_id, 1.5);
...
TEST_F(ElementAnimationsTest, DoNotSyncFinishedAnimation) {
  ...
  events = CreateEventsForTesting();
  single_player_impl_->Tick(kInitialTickTime + TimeDelta::FromSeconds(1));
  single_player_impl_->UpdateState(true, events.get());
  EXPECT_EQ(1u, events->events_.size());
  ...
  single_player_->Tick(kInitialTickTime + TimeDelta::FromSeconds(2));
  single_player_->UpdateState(true, nullptr);
...
TEST_F(ElementAnimationsTest, AnimationsAreDeleted) {
  ...
  AddOpacityTransitionToPlayer(single_player_.get(), 1.0, 0.0f, 1.0f, false);
  single_player_->Tick(kInitialTickTime);
  single_player_->UpdateState(true, nullptr);
  ...
  EXPECT_TRUE(single_player_->GetAnimation(TargetProperty::OPACITY));
  EXPECT_TRUE(single_player_impl_->GetAnimation(TargetProperty::OPACITY));

  ticker_->NotifyAnimationFinished(events->events_[0]);

  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(3000));
  single_player_->UpdateState(true, nullptr);
...
TEST_F(ElementAnimationsTest, TrivialTransition) {
  ...
  EXPECT_FALSE(ticker_->GetAnimationById(animation_id));
  single_player_->AddAnimation(std::move(to_add));
  EXPECT_TRUE(ticker_->GetAnimationById(animation_id));
  EXPECT_EQ(Animation::WAITING_FOR_TARGET_AVAILABILITY,
            ticker_->GetAnimationById(animation_id)->run_state());
  single_player_->Tick(kInitialTickTime);
  EXPECT_EQ(Animation::STARTING,
            ticker_->GetAnimationById(animation_id)->run_state());
  single_player_->UpdateState(true, events.get());
...
TEST_F(ElementAnimationsTest, ScrollOffsetTransition) {
  ...
  PushProperties();
  single_player_impl_->ActivateAnimations();
  EXPECT_TRUE(single_player_impl_->GetAnimation(TargetProperty::SCROLL_OFFSET));
  TimeDelta duration =
      single_player_impl_->GetAnimation(TargetProperty::SCROLL_OFFSET)
          ->curve()
          ->Duration();
  EXPECT_EQ(duration,
            single_player_->GetAnimation(TargetProperty::SCROLL_OFFSET)
                ->curve()
                ->Duration());
  ...
  ticker_->NotifyAnimationStarted(events->events_[0]);
  single_player_->Tick(kInitialTickTime + duration / 2);
  single_player_->UpdateState(true, nullptr);
  EXPECT_TRUE(ticker_->HasTickingAnimation());
  EXPECT_VECTOR2DF_EQ(
  ...
  single_player_impl_->Tick(kInitialTickTime + duration / 2);
  single_player_impl_->UpdateState(true, events.get());
  EXPECT_VECTOR2DF_EQ(
      gfx::Vector2dF(200.f, 250.f),
      client_impl_.GetScrollOffset(element_id_, ElementListType::ACTIVE));

  single_player_impl_->Tick(kInitialTickTime + duration);
  single_player_impl_->UpdateState(true, events.get());
  EXPECT_VECTOR2DF_EQ(target_value, client_impl_.GetScrollOffset(
                                        element_id_, ElementListType::ACTIVE));
  EXPECT_FALSE(ticker_impl_->HasTickingAnimation());

  single_player_->Tick(kInitialTickTime + duration);
  single_player_->UpdateState(true, nullptr);
...
TEST_F(ElementAnimationsTest, ScrollOffsetTransitionOnImplOnly) {
  ...
  single_player_impl_->AddAnimation(std::move(animation));
...
TEST_F(ElementAnimationsTest, UpdateStateWithoutAnimate) {
  ...
  AddScrollOffsetAnimationToPlayer(single_player_impl_.get(),
...
TEST_F(ElementAnimationsTest, ScrollOffsetRemovalClearsScrollDelta) {
  ...
  animation_id = AddAnimatedTransformToPlayer(single_player_.get(), 1.0, 1, 2);
  PushProperties();
  single_player_impl_->ActivateAnimations();
  EXPECT_FALSE(ticker_->scroll_offset_animation_was_interrupted());
  EXPECT_FALSE(ticker_impl_->scroll_offset_animation_was_interrupted());
  ...
  animation_id =
      AddAnimatedFilterToPlayer(single_player_.get(), 1.0, 0.1f, 0.2f);
  PushProperties();
  single_player_impl_->ActivateAnimations();
  EXPECT_FALSE(ticker_->scroll_offset_animation_was_interrupted());
  EXPECT_FALSE(ticker_impl_->scroll_offset_animation_was_interrupted());
...
TEST_F(ElementAnimationsTest,
  ...
  single_player_impl_->set_animation_delegate(&delegate);
  ...
  single_player_impl_->AddAnimation(std::move(to_add));
...
TEST_F(ElementAnimationsTest, SpecifiedStartTimesAreSentToMainThreadDelegate) {
  ...
  TestAnimationDelegate delegate;
  single_player_->set_animation_delegate(&delegate);
...
TEST_F(ElementAnimationsTest,
  ...
  EXPECT_TRUE(ticker_->HasTickingAnimation());
  EXPECT_EQ(0.f, client_.GetOpacity(element_id_, ElementListType::ACTIVE));
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(2000));
  single_player_->UpdateState(true, events.get());
  EXPECT_TRUE(ticker_->HasTickingAnimation());
  EXPECT_EQ(0.f, client_.GetOpacity(element_id_, ElementListType::ACTIVE));
  ...
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(5000));
  single_player_->UpdateState(true, events.get());
  EXPECT_EQ(1.f, client_.GetOpacity(element_id_, ElementListType::ACTIVE));
  EXPECT_FALSE(ticker_->HasTickingAnimation());
}
...
TEST_F(ElementAnimationsTest, TrivialQueuing) {
  ...
  int animation1_id = 1;
  int animation2_id = 2;
  single_player_->AddAnimation(Animation::Create(
      std::unique_ptr<AnimationCurve>(new FakeFloatTransition(1.0, 0.f, 1.f)),
...
TEST_F(ElementAnimationsTest, Interrupt) {
  ...
  single_player_->AddAnimation(CreateAnimation(
      std::unique_ptr<AnimationCurve>(new FakeFloatTransition(1.0, 0.f, 1.f)),
      1, TargetProperty::OPACITY));
  single_player_->Tick(kInitialTickTime);
  single_player_->UpdateState(true, events.get());
  ...
  single_player_->AbortAnimations(TargetProperty::OPACITY, false);
  single_player_->AddAnimation(std::move(to_add));
...
TEST_F(ElementAnimationsTest, TrivialLooping) {
  ...
  EXPECT_TRUE(ticker_->HasTickingAnimation());
  EXPECT_EQ(0.f, client_.GetOpacity(element_id_, ElementListType::ACTIVE));
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(1250));
  single_player_->UpdateState(true, events.get());
  EXPECT_TRUE(ticker_->HasTickingAnimation());
  EXPECT_EQ(0.25f, client_.GetOpacity(element_id_, ElementListType::ACTIVE));
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(1750));
  single_player_->UpdateState(true, events.get());
  EXPECT_TRUE(ticker_->HasTickingAnimation());
  EXPECT_EQ(0.75f, client_.GetOpacity(element_id_, ElementListType::ACTIVE));
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(2250));
  single_player_->UpdateState(true, events.get());
  EXPECT_TRUE(ticker_->HasTickingAnimation());
  EXPECT_EQ(0.25f, client_.GetOpacity(element_id_, ElementListType::ACTIVE));
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(2750));
  single_player_->UpdateState(true, events.get());
  EXPECT_TRUE(ticker_->HasTickingAnimation());
  EXPECT_EQ(0.75f, client_.GetOpacity(element_id_, ElementListType::ACTIVE));
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(3000));
  ...
  // Just be extra sure.
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(4000));
  single_player_->UpdateState(true, events.get());
  EXPECT_EQ(1.f, client_.GetOpacity(element_id_, ElementListType::ACTIVE));
}
...
TEST_F(ElementAnimationsTest, PauseResume) {
  ...
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(1024000));
  single_player_->UpdateState(true, events.get());
  EXPECT_TRUE(ticker_->HasTickingAnimation());
  EXPECT_EQ(0.5f, client_.GetOpacity(element_id_, ElementListType::ACTIVE));
  ...
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(1024250));
  single_player_->UpdateState(true, events.get());
  EXPECT_TRUE(ticker_->HasTickingAnimation());
  EXPECT_EQ(0.75f, client_.GetOpacity(element_id_, ElementListType::ACTIVE));

  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(1024500));
  single_player_->UpdateState(true, events.get());
  EXPECT_FALSE(ticker_->HasTickingAnimation());
  EXPECT_EQ(1.f, client_.GetOpacity(element_id_, ElementListType::ACTIVE));
}
...
TEST_F(ElementAnimationsTest, PushUpdatesWhenSynchronizedStartTimeNeeded) {
  ...
  active_animation = single_player_impl_->GetAnimation(TargetProperty::OPACITY);
  EXPECT_TRUE(active_animation);
  EXPECT_EQ(Animation::WAITING_FOR_TARGET_AVAILABILITY,
...
TEST_F(ElementAnimationsTest, SkipUpdateState) {
  ...
  single_player_->AddAnimation(std::move(first_animation));
  ...
  single_player_->AddAnimation(std::move(second_animation));
...
TEST_F(ElementAnimationsTest, InactiveObserverGetsTicked) {
  ...
  const int id = 1;
  single_player_impl_->AddAnimation(CreateAnimation(
...
TEST_F(ElementAnimationsTest, AbortAnimations) {
  ...
  single_player_->AbortAnimations(TargetProperty::TRANSFORM, false);
...
TEST_F(ElementAnimationsTest, MainThreadAbortedAnimationGetsDeleted) {
  ...
  int animation_id =
      AddOpacityTransitionToPlayer(single_player_.get(), 1.0, 0.f, 1.f, false);
  EXPECT_TRUE(host_->needs_push_properties());
...
TEST_F(ElementAnimationsTest, ImplThreadAbortedAnimationGetsDeleted) {
  ...
  single_player_impl_->AbortAnimations(TargetProperty::OPACITY, false);
  EXPECT_EQ(
      Animation::ABORTED,
      single_player_impl_->GetAnimation(TargetProperty::OPACITY)->run_state());
  EXPECT_TRUE(host_impl_->needs_push_properties());
...
TEST_F(ElementAnimationsTest, ImplThreadTakeoverAnimationGetsDeleted) {
  ...
  single_player_impl_->set_animation_delegate(&delegate_impl);
  TestAnimationDelegate delegate;
  single_player_->set_animation_delegate(&delegate);
  ...
  single_player_impl_->AbortAnimations(TargetProperty::SCROLL_OFFSET, true);
  EXPECT_TRUE(host_impl_->needs_push_properties());
...
TEST_F(ElementAnimationsTest, FinishedEventsForGroup) {
  ...
  first_animation->set_is_controlling_instance_for_test(true);
  single_player_impl_->AddAnimation(std::move(first_animation));

  std::unique_ptr<Animation> second_animation(Animation::Create(
      std::unique_ptr<AnimationCurve>(new FakeFloatTransition(1.0, 0.f, 1.f)),
      2, group_id, TargetProperty::OPACITY));
  second_animation->set_is_controlling_instance_for_test(true);
  single_player_impl_->AddAnimation(std::move(second_animation));
...
TEST_F(ElementAnimationsTest, AnimationStartScale) {
  ...
  single_player_impl_->RemoveAnimation(1);
  animation =
...
TEST_F(ElementAnimationsTest, MaximumTargetScaleWithDirection) {
  ...
  single_player_impl_->AddAnimation(std::move(animation_owned));
...
TEST_F(ElementAnimationsTest, NewlyPushedAnimationWaitsForActivation) {
  ...
  int animation_id =
      AddOpacityTransitionToPlayer(single_player_.get(), 1, 0.5f, 1.f, false);
  EXPECT_TRUE(ticker_->GetAnimationById(animation_id));
  EXPECT_FALSE(ticker_impl_->GetAnimationById(animation_id));
...
TEST_F(ElementAnimationsTest, ActivationBetweenAnimateAndUpdateState) {
  ...
  const int animation_id =
      AddOpacityTransitionToPlayer(single_player_.get(), 1, 0.5f, 1.f, true);
...
TEST_F(ElementAnimationsTest, ObserverNotifiedWhenTransformAnimationChanges) {
  ...
  AddAnimatedTransformToPlayer(single_player_.get(), 1.0, 1, 1);
  ...
  int animation_id =
      AddAnimatedTransformToPlayer(single_player_.get(), 10.0, 2, 2);
  int animation2_id =
      AddAnimatedTransformToPlayer(single_player_.get(), 10.0, 2, 1);
  ...
  single_player_->RemoveAnimation(animation_id);
  single_player_->RemoveAnimation(animation2_id);
  EXPECT_FALSE(client_.GetHasPotentialTransformAnimation(
      element_id_, ElementListType::ACTIVE));
  EXPECT_FALSE(client_.GetTransformIsCurrentlyAnimating(
      element_id_, ElementListType::ACTIVE));
  ...
  animation_id = AddAnimatedTransformToPlayer(single_player_.get(), 10.0, 3, 3);
  EXPECT_TRUE(client_.GetHasPotentialTransformAnimation(
      element_id_, ElementListType::ACTIVE));
  EXPECT_TRUE(client_.GetTransformIsCurrentlyAnimating(
      element_id_, ElementListType::ACTIVE));
  ...
  single_player_impl_->AbortAnimations(TargetProperty::TRANSFORM, false);
  EXPECT_FALSE(client_impl_.GetHasPotentialTransformAnimation(
      element_id_, ElementListType::PENDING));
  EXPECT_FALSE(client_impl_.GetTransformIsCurrentlyAnimating(
      element_id_, ElementListType::PENDING));
  ...
  animation_id = AddAnimatedTransformToPlayer(single_player_.get(), 1.0, 1, 6);
  ticker_->GetAnimationById(animation_id)
      ->set_time_offset(base::TimeDelta::FromMilliseconds(-10000));
  ticker_->GetAnimationById(animation_id)
      ->set_fill_mode(Animation::FillMode::NONE);
...
TEST_F(ElementAnimationsTest, ObserverNotifiedWhenOpacityAnimationChanges) {
  ...
  // Case 1: An animation that's allowed to run until its finish point.
  AddOpacityTransitionToPlayer(single_player_.get(), 1.0, 0.f, 1.f,
  ...
  int animation_id = AddOpacityTransitionToPlayer(
      single_player_.get(), 10.0, 0.f, 1.f, false /*use_timing_function*/);
  EXPECT_TRUE(client_.GetHasPotentialOpacityAnimation(element_id_,
                                                      ElementListType::ACTIVE));
  EXPECT_TRUE(client_.GetOpacityIsCurrentlyAnimating(element_id_,
                                                     ElementListType::ACTIVE));
  ...
  // Case 3: An animation that's aborted before it finishes.
  animation_id = AddOpacityTransitionToPlayer(
      single_player_.get(), 10.0, 0.f, 0.5f, false /*use_timing_function*/);
  EXPECT_TRUE(client_.GetHasPotentialOpacityAnimation(element_id_,
                                                      ElementListType::ACTIVE));
  EXPECT_TRUE(client_.GetOpacityIsCurrentlyAnimating(element_id_,
                                                     ElementListType::ACTIVE));
  ...
  // Case 4 : An animation that's not in effect.
  animation_id = AddOpacityTransitionToPlayer(
      single_player_.get(), 1.0, 0.f, 0.5f, false /*use_timing_function*/);
  ticker_->GetAnimationById(animation_id)
      ->set_time_offset(base::TimeDelta::FromMilliseconds(-10000));
  ticker_->GetAnimationById(animation_id)
      ->set_fill_mode(Animation::FillMode::NONE);
...
TEST_F(ElementAnimationsTest, ObserverNotifiedWhenFilterAnimationChanges) {
  ...
  // Case 1: An animation that's allowed to run until its finish point.
  AddAnimatedFilterToPlayer(single_player_.get(), 1.0, 0.f, 1.f);
  ...
  // Case 2: An animation that's removed before it finishes.
  int animation_id =
      AddAnimatedFilterToPlayer(single_player_.get(), 10.0, 0.f, 1.f);
  EXPECT_TRUE(client_.GetHasPotentialFilterAnimation(element_id_,
                                                     ElementListType::ACTIVE));
  EXPECT_TRUE(client_.GetFilterIsCurrentlyAnimating(element_id_,
                                                    ElementListType::ACTIVE));
  ...
  // Case 3: An animation that's aborted before it finishes.
  animation_id =
      AddAnimatedFilterToPlayer(single_player_.get(), 10.0, 0.f, 0.5f);
  EXPECT_TRUE(client_.GetHasPotentialFilterAnimation(element_id_,
                                                     ElementListType::ACTIVE));
  EXPECT_TRUE(client_.GetFilterIsCurrentlyAnimating(element_id_,
                                                    ElementListType::ACTIVE));
  ...
  single_player_impl_->AbortAnimations(TargetProperty::FILTER, false);
  EXPECT_FALSE(client_impl_.GetHasPotentialFilterAnimation(
      element_id_, ElementListType::PENDING));
  EXPECT_FALSE(client_impl_.GetFilterIsCurrentlyAnimating(
      element_id_, ElementListType::PENDING));
  ...
  // Case 4 : An animation that's not in effect.
  animation_id =
      AddAnimatedFilterToPlayer(single_player_.get(), 1.0, 0.f, 0.5f);
  ticker_->GetAnimationById(animation_id)
      ->set_time_offset(base::TimeDelta::FromMilliseconds(-10000));
  ticker_->GetAnimationById(animation_id)
      ->set_fill_mode(Animation::FillMode::NONE);
...
TEST_F(ElementAnimationsTest, ClippedOpacityValues) {
  ...
  AddOpacityTransitionToPlayer(single_player_.get(), 1, 1.f, 2.f, true);
...
TEST_F(ElementAnimationsTest, ClippedNegativeOpacityValues) {
  ...
  AddOpacityTransitionToPlayer(single_player_.get(), 1, 0.f, -2.f, true);
...
TEST_F(ElementAnimationsTest, PushedDeletedAnimationWaitsForActivation) {
  ...
  single_player_->RemoveAnimation(
      single_player_->GetAnimation(TargetProperty::OPACITY)->id());
  PushProperties();
  ...
  // during commit.
  single_player_->animation_host()->SetAnimationEvents(std::move(events));
  PushProperties();
  EXPECT_FALSE(ticker_impl_->has_any_animation());
}
...
TEST_F(ElementAnimationsTest, StartAnimationsAffectingDifferentObservers) {
  ...
  const int first_animation_id =
      AddOpacityTransitionToPlayer(single_player_.get(), 1, 0.f, 1.f, true);
  ...
  const int second_animation_id =
      AddOpacityTransitionToPlayer(single_player_.get(), 1, 1.f, 0.5f, true);
  PushProperties();
...
TEST_F(ElementAnimationsTest, TestIsCurrentlyAnimatingProperty) {
  ...
  single_player_->ActivateAnimations();
  ...
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(10));
  single_player_->UpdateState(true, nullptr);
  ...
  single_player_->Tick(kInitialTickTime + TimeDelta::FromMilliseconds(1100));
  single_player_->UpdateState(true, nullptr);
...
TEST_F(ElementAnimationsTest, TickingAnimationsCount) {
  ...
  single_player_->AddAnimation(CreateAnimation(
      std::unique_ptr<AnimationCurve>(new FakeFloatTransition(1.0, 1.f, 0.5f)),
      2, TargetProperty::OPACITY));
  EXPECT_EQ(1u, single_player_->TickingAnimationsCount());
  EXPECT_EQ(1u, host_->CompositedAnimationsCount());
  single_player_->AddAnimation(CreateAnimation(
      std::unique_ptr<AnimationCurve>(new FakeTransformTransition(1)), 1,
      TargetProperty::TRANSFORM));
  EXPECT_EQ(2u, single_player_->TickingAnimationsCount());
