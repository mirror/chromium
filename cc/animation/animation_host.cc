...
std::unique_ptr<MutatorInputState> AnimationHost::CollectAnimatorsState(
  ...
  for (auto& player : ticking_players_) {
    ...
    double current_time = (monotonic_time - base::TimeTicks()).InSecondsF();
    if (worklet_player->scroll_timeline()) {
      current_time =
          worklet_player->scroll_timeline()->CurrentTime(scroll_tree);
    }
    MutatorInputState::AnimationState state{
        worklet_player->id(), worklet_player->name(), current_time};
