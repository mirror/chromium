...
bool AnimationHost::NeedsTickMutator() const {
  return mutator_ && (needs_tick_mutates_ || mutator_->HasAnimators());
}
...
bool AnimationHost::TickAnimations(base::TimeTicks monotonic_time) {
  ...
  if (NeedsTickMutator()) {
    ...
    did_animate = true;
    // After we have forced enough fresh ones we rely on the animators list.
    if (needs_tick_mutates_)
      needs_tick_mutates_--;
  }
...
void AnimationHost::SetLayerTreeMutator(
  ...
  // TODO(majidvp): Remove this hackery.  4 is not enough to pass tests.
  needs_tick_mutates_ = 5;
}
