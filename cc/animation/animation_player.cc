...
ElementId AnimationPlayer::element_id_of_ticker(unsigned ticker_id) const {
...
void AnimationPlayer::RegisterTicker(ElementId element_id, unsigned ticker_id) {
...
void AnimationPlayer::RegisterTickers() {
  for (auto& element_id_ticker_id : element_to_ticker_id_map_) {
    ...
    const std::unordered_set<unsigned>& ticker_ids =
...
void AnimationPlayer::AnimationRemovedFromTicking() {
  ...
  if (!is_ticking_player_)
    return;
  DCHECK(animation_host_);
  DCHECK_GE(ticking_tickers_number_, 0);
  if (ticking_tickers_number_)
    return;
...
std::string AnimationPlayer::ToString(unsigned ticker_id) const {
  DCHECK(GetTickerById(ticker_id));
...
void AnimationPlayer::RemoveTicker(unsigned ticker_id) {
  DCHECK(GetTickerById(ticker_id));
...
AnimationTicker* AnimationPlayer::GetTickerById(unsigned ticker_id) const {
  DCHECK_GT(ticker_id, 0u);
  // May return nullptr when syncing tickers_ to impl.
  return tickers_.size() >= ticker_id ? tickers_[ticker_id - 1].get() : nullptr;
}
