...
void AnimationPlayer::RegisterTickers() {
  for_each(
      element_to_ticker_id_map_.begin(), element_to_ticker_id_map_.end(),
      [&](ElementToTickerIdMap::value_type& element_id_ticker_id) {
        const ElementId element_id = element_id_ticker_id.first;
        const std::unordered_set<int>& ticker_ids = element_id_ticker_id.second;
        for_each(ticker_ids.begin(), ticker_ids.end(), [&](auto& ticker_id) {
          RegisterTicker(element_id, ticker_id);
        });
      });
}
...
void AnimationPlayer::RemoveDetachedTickersFromImplThread(
  ...
  for_each(tickers_impl.begin(), tickers_impl.end(),
           [&](IdToTickerMap::value_type& id_ticker_pair) {
             ...
             if (!GetTickerById(ticker_id)) {
               if (ticker->element_animations()) {
                 player_impl->DetachElementForTicker(ticker->element_id(),
                                                     ticker_id);
               }
...
void AnimationPlayer::Tick(base::TimeTicks monotonic_time) {
  ...
  for_each(id_to_ticker_map_.begin(), id_to_ticker_map_.end(),
           [&](auto& id_ticker) {
             AnimationTicker* ticker = id_ticker.second.get();
...
void AnimationPlayer::UpdateState(bool start_ready_animations,
  ...
  for_each(id_to_ticker_map_.begin(), id_to_ticker_map_.end(),
           [&](auto& id_ticker_pair) {
             const int ticker_id = id_ticker_pair.first;
             UpdateStateForTicker(start_ready_animations, events, ticker_id);
           });
}
