...
ElementId AnimationPlayer::element_id_of_ticker(size_t ticker_id) const {
...
void AnimationPlayer::RegisterTicker(ElementId element_id, size_t ticker_id) {
...
void AnimationPlayer::UnregisterTicker(ElementId element_id, size_t ticker_id) {
  DCHECK(animation_host_);
  AnimationTicker* ticker = GetTickerById(ticker_id);
  DCHECK(ticker);
  if (ticker->has_attached_element() &&
...
void AnimationPlayer::RegisterTickers() {
  for (auto& element_id_ticker_id : element_to_ticker_id_map_) {
    ...
    const std::unordered_set<size_t>& ticker_ids = element_id_ticker_id.second;
...
std::string AnimationPlayer::ToString(size_t ticker_id) const {
  DCHECK(GetTickerById(ticker_id));
...
AnimationTicker* AnimationPlayer::GetTickerById(size_t ticker_id) const {
