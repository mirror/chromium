...
inline ManagedTimingModel& ToManagedTimingModel(
    AnimationPlayer::TimingModel* timing) {
  return *(static_cast<ManagedTimingModel*>(timing));
}

void WorkletAnimationPlayer::SetLocalTime(base::TimeDelta local_time) {
  ToManagedTimingModel(timing_model_.get()).set_local_time(local_time);
...
void WorkletAnimationPlayer::PushPropertiesToImplThread(
  ...
  static_cast<WorkletAnimationPlayer*>(animation_player_impl)
      ->SetLocalTime(ToManagedTimingModel(timing_model_.get()).local_time());
}
