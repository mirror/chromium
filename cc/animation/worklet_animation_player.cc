...
double WorkletAnimationPlayer::GetCurrentTime(base::TimeTicks monotonic_time,
                                              const ScrollTree& scroll_tree) {
  if (scroll_timeline_) {
    return scroll_timeline_->CurrentTime(scroll_tree);
  }

  // We assume here that the passed |monotonic_time| is relative to the
  // navigationStart moment as defined by the web-animations spec.
  //
  // TODO(smcgruer): Support DocumentTimeline's originTime offset concept.
  return (monotonic_time - base::TimeTicks()).InSecondsF();
}
