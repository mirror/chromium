...
// SingleAnimationPlayer is a sub-class of AnimationPlayer. It serves as a
// bridge between the clients and cc side animations because we previously only
// support single effect per player.
//
// There is a 1:1 relationship between SingleAnimationPlayer and the
// AnimationTicker. In general, the base class AnimationPlayer is a 1:N
// relationship to allow for grouped animations.
//
// TODO(yigu): Deprecate SingleAnimationPlayer once grouped animations are fully
...
class CC_ANIMATION_EXPORT SingleAnimationPlayer : public AnimationPlayer {
 public:
  ...
  AnimationTicker* animation_ticker() const override;
  ...
  size_t TickingAnimationsCount() const override;
 ...
 private:
  ...
  AnimationTicker* GetUniqueTicker() const;
