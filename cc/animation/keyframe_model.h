...
// Currently it does not contain keyframes but it will in the future. The name
// is to match its blink counterpart.
// Note: this class was formerly known as cc Animation.
class CC_ANIMATION_EXPORT KeyframeModel {
 public:
  ...
  // target property is free (and all the keyframe models animating with it are
  ...
  // RUNNING keyframe models may toggle between RUNNING and PAUSED, and may be
  ...
  // Currently this is for impl-only scroll offset keyframe models that need to
  ...
  // tickable until it is given a start time. This is true for keyframe models
  ...
  // This is true for keyframe models running on the main thread when the
 ...
 private:
  ...
  // same group id. Grouped keyframe models are guaranteed to start at the same
  // time and no other keyframe models may animate any of the group's target
  // properties until all keyframe models in the group have finished animating.
  int group_;
  ...
  // time. This is used for resuming paused keyframe models -- an animation is
  ...
  // elements. After activation, such keyframe models affect both kinds of
  ...
  // stop affecting pending elements. After activation, such keyframe models no
