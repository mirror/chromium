...
TEST_F(AnimationPlayerTest, AttachDetachTimelineIfLayerAttached) {
  ...
  EXPECT_FALSE(player_->GetTickerById(ticker_id)->element_animations());
  EXPECT_FALSE(player_->GetTickerById(ticker_id)->element_id());
  EXPECT_FALSE(player_->GetTickerById(ticker_id)->needs_push_properties());
  ...
  EXPECT_FALSE(GetTickerForElementId(element_id_));
  EXPECT_FALSE(player_->GetTickerById(ticker_id)->element_animations());
  EXPECT_EQ(player_->GetTickerById(ticker_id)->element_id(), element_id_);
  EXPECT_FALSE(player_->GetTickerById(ticker_id)->needs_push_properties());
  ...
  EXPECT_EQ(player_->GetTickerById(ticker_id),
            GetTickerForElementId(element_id_));
  EXPECT_TRUE(player_->GetTickerById(ticker_id)->element_animations());
  EXPECT_EQ(player_->GetTickerById(ticker_id)->element_id(), element_id_);
  EXPECT_TRUE(player_->GetTickerById(ticker_id)->needs_push_properties());
...
TEST_F(AnimationPlayerTest, AddRemoveAnimationToNonAttachedPlayer) {
  ...
  const int filter_id =
      AddAnimatedFilter(player_.get(), duration, 0.1f, 0.9f, ticker_id);
  AddOpacityTransition(player_.get(), duration, start_opacity, end_opacity,
                       false, ticker_id);
  ...
  EXPECT_FALSE(player_->GetTickerById(ticker_id)->needs_push_properties());
  EXPECT_FALSE(player_->GetTickerById(ticker_id)->element_animations());
  player_->RemoveAnimationForTicker(filter_id, ticker_id);
  EXPECT_FALSE(player_->GetTickerById(ticker_id)->needs_push_properties());

  player_->AttachElementForTicker(element_id_, ticker_id);

  EXPECT_TRUE(player_->GetTickerById(ticker_id)->element_animations());
  EXPECT_FALSE(player_->GetTickerById(ticker_id)
                   ->element_animations()
                   ->HasAnyAnimationTargetingProperty(TargetProperty::FILTER));
  EXPECT_TRUE(player_->GetTickerById(ticker_id)
                  ->element_animations()
...
TEST_F(AnimationPlayerTest, AddRemoveAnimationCausesSetNeedsCommit) {
  ...
  const int animation_id =
      AddOpacityTransition(player_.get(), 1., .7f, .3f, false, ticker_id);

  EXPECT_TRUE(client_.mutators_need_commit());
  client_.set_mutators_need_commit(false);

  player_->PauseAnimationForTicker(animation_id, 1., ticker_id);
  EXPECT_TRUE(client_.mutators_need_commit());
  client_.set_mutators_need_commit(false);

  player_->RemoveAnimationForTicker(animation_id, ticker_id);
  EXPECT_TRUE(client_.mutators_need_commit());
  client_.set_mutators_need_commit(false);
}
...
TEST_F(AnimationPlayerTest, SwitchToLayer) {
  ...
  EXPECT_EQ(player_impl_->GetTickerById(ticker_id),
            GetImplTickerForLayerId(element_id_));
  EXPECT_TRUE(player_impl_->GetTickerById(ticker_id)->element_animations());
  EXPECT_EQ(player_impl_->GetTickerById(ticker_id)->element_id(), element_id_);

  const ElementId new_element_id(NextTestLayerId());
  player_->DetachElement();
  player_->AttachElementForTicker(new_element_id, ticker_id);

  EXPECT_EQ(player_->GetTickerById(ticker_id),
            GetTickerForElementId(new_element_id));
  EXPECT_TRUE(player_->GetTickerById(ticker_id)->element_animations());
  EXPECT_EQ(player_->GetTickerById(ticker_id)->element_id(), new_element_id);
  ...
  EXPECT_TRUE(player_impl_->GetTickerById(ticker_id)->element_animations());
  EXPECT_EQ(player_impl_->GetTickerById(ticker_id)->element_id(),
            new_element_id);
}
...
TEST_F(AnimationPlayerTest, ToString) {
  ...
  EXPECT_EQ(
      ...
      player_->ToString(player_->GetTickerById(ticker_id)->id()));
