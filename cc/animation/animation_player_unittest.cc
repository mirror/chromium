...
class AnimationPlayerTest : public AnimationTimelinesTest {
 public:
  ...
  int NextTickerId() { return ++ticker_id_; }
...
TEST_F(AnimationPlayerTest, AttachDetachLayerIfTimelineAttached) {
  ...
  EXPECT_TRUE(CheckTickerTimelineNeedsPushProperties(false, ticker_id_));
  ...
  EXPECT_TRUE(CheckTickerTimelineNeedsPushProperties(true, ticker_id_));
...
TEST_F(AnimationPlayerTest, ToString) {
  ...
  EXPECT_EQ(
      ...
      player_->ToString(player_->GetTickerById(ticker_id_)->id()));
...
TEST_F(AnimationPlayerTest, TickerSyncToImplTest) {
  ...
  EXPECT_FALSE(timeline_->needs_push_properties());
  player_->RemoveTicker(ticker_id1);
  EXPECT_FALSE(player_->GetTickerById(ticker_id1));
  EXPECT_TRUE(player_impl_->GetTickerById(ticker_id1));
  ...
  EXPECT_FALSE(player_impl_->GetTickerById(ticker_id1));
