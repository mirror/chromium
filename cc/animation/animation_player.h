...
class CC_ANIMATION_EXPORT AnimationPlayer
 ...
 public:
  ...
  ElementId element_id_of_ticker(size_t ticker_id) const;
  ...
  scoped_refptr<ElementAnimations> element_animations(size_t ticker_id) const;

  void set_animation_delegate(AnimationDelegate* delegate) {
    animation_delegate_ = delegate;
  }

  void AttachElementForTicker(ElementId element_id, size_t ticker_id);
  void DetachElementForTicker(ElementId element_id, size_t ticker_id);
  ...
  void PauseAnimationForTicker(int animation_id,
                               double time_offset,
                               size_t ticker_id);
  void RemoveAnimationForTicker(int animation_id, size_t ticker_id);
  void AbortAnimationForTicker(int animation_id, size_t ticker_id);
  ...
  std::string ToString(size_t ticker_id) const;
  ...
  AnimationTicker* GetTickerById(size_t ticker_id) const;
  size_t NextTickerId() { return tickers_.size(); }

 private:
  friend class base::RefCounted<AnimationPlayer>;

  void RegisterTicker(ElementId element_id, size_t ticker_id);
  void UnregisterTicker(ElementId element_id, size_t ticker_id);
 ...
 protected:
  ...
  using ElementToTickerIdMap =
      std::unordered_map<ElementId, std::unordered_set<size_t>, ElementIdHash>;
