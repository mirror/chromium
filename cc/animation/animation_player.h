...
class CC_ANIMATION_EXPORT AnimationPlayer
 ...
 public:
  ...
  ElementId element_id_of_ticker(unsigned ticker_id) const;
  ...
  scoped_refptr<ElementAnimations> element_animations(unsigned ticker_id) const;

  void set_animation_delegate(AnimationDelegate* delegate) {
    animation_delegate_ = delegate;
  }

  void AttachElementForTicker(ElementId element_id, unsigned ticker_id);
  void DetachElementForTicker(ElementId element_id, unsigned ticker_id);
  ...
  void PauseAnimationForTicker(int animation_id,
                               double time_offset,
                               unsigned ticker_id);
  void RemoveAnimationForTicker(int animation_id, unsigned ticker_id);
  void AbortAnimationForTicker(int animation_id, unsigned ticker_id);
  ...
  std::string ToString(unsigned ticker_id) const;

  void SetNeedsCommit();

  virtual bool IsWorkletAnimationPlayer() const;
  void AddTicker(std::unique_ptr<AnimationTicker>);
  void RemoveTicker(unsigned ticker_id);

  AnimationTicker* GetTickerById(unsigned ticker_id) const;
  unsigned NextTickerId() { return tickers_.size() + 1; }

 private:
  friend class base::RefCounted<AnimationPlayer>;

  void RegisterTicker(ElementId element_id, unsigned ticker_id);
  void UnregisterTicker(ElementId element_id, unsigned ticker_id);
 ...
 protected:
  ...
  using ElementToTickerIdMap = std::
      unordered_map<ElementId, std::unordered_set<unsigned>, ElementIdHash>;
  using Tickers = std::vector<std::unique_ptr<AnimationTicker>>;

  // A ticker is added/removed to tickers_ when AddTicker/RemoveTicker is
  // called. A ticker is added/removed to element_to_ticker_id_map_ when
  ...
  bool is_ticking_player_;
  int ticking_tickers_number_;
