...
namespace {
void TickAnimation(base::TimeTicks monotonic_time,
                   Animation* animation,
...
void AnimationTicker::TickAnimations(base::TimeTicks monotonic_time) {
  DCHECK(element_animations_);
...
void AnimationTicker::AddAnimation(AnimationHost* animation_host,
...
bool AnimationTicker::RemoveAnimation(int animation_id) {
  bool animation_removed = false;
  bool interrupted_scroll_offset = false;
  ...
  for (auto it = animations_to_remove; it != animations_.end(); ++it) {
    if ((*it)->target_property_id() == TargetProperty::SCROLL_OFFSET) {
      ...
      if (element_animations_)
        interrupted_scroll_offset = true;
  ...
  return interrupted_scroll_offset;
}
...
void AnimationTicker::AbortAnimation(int animation_id,
  ...
  if (Animation* animation = GetAnimationById(animation_id)) {
    if (!animation->is_finished()) {
      animation->SetRunState(Animation::ABORTED, last_tick_time);
      if (element_animations_)
        element_animations_->UpdateClientAnimationState();
    }
  }
...
void AnimationTicker::StartAnimationsAndPromoteIfNeeded(
    base::TimeTicks monotonic_time,
    AnimationEvents* events) {
  if (!needs_to_start_animations_)
    return;
  StartAnimations(monotonic_time);
  PromoteStartedAnimations(monotonic_time, events);
}
...
void AnimationTicker::PromoteStartedAnimations(base::TimeTicks monotonic_time,
                                               AnimationEvents* events) {
  for (size_t i = 0; i < animations_.size(); ++i) {
    if (animations_[i]->run_state() == Animation::STARTING &&
...
bool AnimationTicker::HasTransformAnimationThatInflatesBounds(
  ...
  return IsCurrentlyAnimatingProperty(TargetProperty::TRANSFORM,
         ...
         IsCurrentlyAnimatingProperty(TargetProperty::TRANSFORM,
                                      ElementListType::PENDING, last_tick_time);
}

bool AnimationTicker::TransformAnimationBoundsForBox(
    const gfx::BoxF& box,
    gfx::BoxF* bounds,
    base::TimeTicks last_tick_time) const {
  DCHECK(HasTransformAnimationThatInflatesBounds(last_tick_time))
    ...
    if (Animation* animation = GetAnimationById(animation_impl->id())) {
      if (animation->run_state() == Animation::ABORTED) {
        animation_impl->SetRunState(Animation::WAITING_FOR_DELETION,
                                    impl_last_tick_time);
        animation->SetRunState(Animation::WAITING_FOR_DELETION, last_tick_time);
...
void AnimationTicker::PushNewAnimationsToImplThread(
  ...
  for (size_t i = 0; i < animations_.size(); ++i) {
    ...
    // TODO(Smcgruer): We need a better way to do this.
    animation_ticker_impl->animation_player_->AddAnimation(std::move(to_add));
  }
}
