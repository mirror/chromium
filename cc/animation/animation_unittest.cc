...
TEST(AnimationTest, ToString) {
  ...
  EXPECT_EQ(
      base::StringPrintf("Animation{id=%d, group=%d, target_property_id=%d, "
                         "run_state=%s}",
                         animation->id(), animation->group(),
                         animation->target_property_id(),
                         Animation::ToString(animation->run_state()).c_str()),
      animation->ToString());
}
