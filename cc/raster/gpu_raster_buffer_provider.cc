...
static sk_sp<PaintRecord> SetupForRaster(
  ...
  raster_source->SetupCanvasForPlayback(canvas, raster_full_rect, playback_rect,
                                        transform);
...
static void RasterizeSource(
  ...
  auto setup_list = make_scoped_refptr(new DisplayItemList);
  setup_list->CreateAndAppendDrawingItem<DrawingDisplayItem>(
      raster_full_rect,
      SetupForRaster(raster_source, raster_full_rect, playback_rect, transform),
      gfx::RectToSkRect(raster_full_rect));

  gpu::gles2::GLES2Interface* gl = context_provider->ContextGL();
  ResourceProvider::ScopedTextureProvider texture_provider(
      gl, resource_lock, async_worker_context_enabled);

  gl->BeginRasterCHROMIUM(
      resource_lock->target(), texture_provider.texture_id(),
      resource_lock->size().width(), resource_lock->size().height(),
      msaa_sample_count, raster_source->CanUseLCDText(),
...
// TODO(enne): add a separate mode for this rather than hijacking
// gpu raster.
#if 0
