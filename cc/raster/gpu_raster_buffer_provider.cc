...
static void RasterizeSourceOOP(
  ...
  ri->BeginRasterCHROMIUM(texture_id, raster_source->background_color(),
                          ...
                          viz::ToGrPixelConfig(resource_format));
...
static void RasterizeSource(
...

  {
    ...
    raster_source->PlaybackToCanvas(canvas, color_space, raster_full_rect,
...
GpuRasterBufferProvider::RasterBufferImpl::~RasterBufferImpl() {
  ...
  // texture content.
  backing_->mailbox_sync_token = sync_token_;
}
...
void GpuRasterBufferProvider::RasterBufferImpl::Playback(
  ...
  // The |sync_token_| passed in here was created on the compositor thread.
  // Returned back is another SyncToken generated on the worker thread to
  // synchronize with after the raster is complete.
  sync_token_ = client_->PlaybackOnWorkerThread(
      mailbox_, texture_target_, sync_token_, resource_size_, resource_format_,
      color_space_, resource_has_previous_content_, raster_source,
...
std::unique_ptr<RasterBuffer> GpuRasterBufferProvider::AcquireBufferForRaster(
  ...
  if (!resource.gpu_backing()) {
    ...
    viz::TextureAllocation alloc = viz::TextureAllocation::Allocate(
        gl, caps, resource.format(), resource.size(), resource.color_space(),
...
gpu::SyncToken GpuRasterBufferProvider::PlaybackOnWorkerThread(
  ...
  if (enable_oop_rasterization_) {
    ...
    RasterizeSource(raster_source, resource_has_previous_content, mailbox,
                    texture_target, resource_size, resource_format, color_space,
                    raster_full_rect, playback_rect, transform,
                    playback_settings, worker_context_provider_,
                    use_distance_field_text_, msaa_sample_count_);
  }
