...
class UnpremultiplyEffect {
 public:
  explicit UnpremultiplyEffect(gpu::gles2::GLES2Interface* gl) {
    ...
    const char* kFragmentShader =
        ...
        "  vec4 undithered = vec4((in_color.rgb) * (1.0 / in_color.a), "
        "in_color.a);\n"
        ...
        "  float value = dot(stepValues, vec4(8.0 / 16.0, 4.0 / 16.0, 2.0 / "
        "16.0, 1.0 / 16.0)) - 15.0 / 32.0;\n"
        "  gl_FragColor = undithered + value * range;\n"
        "}\n";
...
static void RasterizeSource(
...

  {
    ...
    context_provider->GL(&gl);
    unpremultiply_effect->Unpremultiply(gl, info->fID, texture_id,
                                        resource_lock->size().width(),
...
void GpuRasterBufferProvider::PlaybackOnWorkerThread(
  ...
  if (!unpremultiply_effect_) {
    gpu::gles2::GLES2Interface* gl = nullptr;
    worker_context_provider_->GL(&gl);
    unpremultiply_effect_.reset(new UnpremultiplyEffect(gl));
  }
