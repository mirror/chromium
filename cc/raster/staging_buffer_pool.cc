...
bool CheckForQueryResult(gpu::raster::RasterInterface* rs, unsigned query_id) {
  unsigned complete = 1;
  rs->GetQueryObjectuivEXT(query_id, GL_QUERY_RESULT_AVAILABLE_EXT, &complete);
  return !!complete;
}

void WaitForQueryResult(gpu::raster::RasterInterface* rs, unsigned query_id) {
  TRACE_EVENT0("cc", "WaitForQueryResult");

  int attempts_left = kMaxCheckForQueryResultAvailableAttempts;
  while (attempts_left--) {
    if (CheckForQueryResult(rs, query_id))
      break;
  ...
  unsigned result = 0;
  rs->GetQueryObjectuivEXT(query_id, GL_QUERY_RESULT_EXT, &result);
}
...
void StagingBuffer::DestroyGLResources(gpu::raster::RasterInterface* rs) {
  if (query_id) {
    rs->DeleteQueriesEXT(1, &query_id);
    query_id = 0;
  }
  if (image_id) {
    rs->DestroyImageCHROMIUM(image_id);
    image_id = 0;
  }
  if (texture_id) {
...
std::unique_ptr<StagingBuffer> StagingBufferPool::AcquireStagingBuffer(
  ...
  if (resource_provider_->use_sync_query()) {
    while (!busy_buffers_.empty()) {
      if (!CheckForQueryResult(rs, busy_buffers_.front()->query_id))
        break;
  ...
  while (
    ...
    if (resource_provider_->use_sync_query()) {
      WaitForQueryResult(rs, busy_buffers_.front()->query_id);
      MarkStagingBufferAsFree(busy_buffers_.front().get());
      free_buffers_.push_back(PopFront(&busy_buffers_));
    } else {
  ...
  while (staging_buffer_usage_in_bytes_ > max_staging_buffer_usage_in_bytes_) {
    ...
    free_buffers_.front()->DestroyGLResources(rs);
...
void StagingBufferPool::ReleaseBuffersNotUsedSince(base::TimeTicks time) {
...

  {
    ...
    while (!busy_buffers_.empty()) {
      ...
      busy_buffers_.front()->DestroyGLResources(rs);
