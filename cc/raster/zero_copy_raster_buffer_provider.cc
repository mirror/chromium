...
ZeroCopyRasterBufferProvider::AcquireBufferForRaster(
  ...
  if (!backing->gpu_memory_buffer) {
    backing->gpu_memory_buffer =
        gpu_memory_buffer_manager_->CreateGpuMemoryBuffer(
            resource.size(), viz::BufferFormat(resource.format()), kBufferUsage,
            gpu::kNullSurfaceHandle);
  }

  return std::make_unique<ZeroCopyRasterBufferImpl>(
      compositor_context_provider_, resource, backing);
}
