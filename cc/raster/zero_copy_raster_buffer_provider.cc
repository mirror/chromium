...
class ZeroCopyRasterBufferImpl : public RasterBuffer {
 public:
  ZeroCopyRasterBufferImpl(
      ...
      : backing_(backing),
        ...
        gpu_memory_buffer_(backing_->gpu_memory_buffer.get()) {}
  ...
  ~ZeroCopyRasterBufferImpl() override {
    ...
    if (!backing_->texture_id) {
      ...
      // anything to it..
      backing_->gpu_memory_buffer = std::move(owned_gpu_memory_buffer_);
      if (backing_->gpu_memory_buffer) {
        backing_->image_id = gl->CreateImageCHROMIUM(
            backing_->gpu_memory_buffer->AsClientBuffer(),
            resource_size_.width(), resource_size_.height(),
            viz::GLInternalFormat(resource_format_));
      }
  ...
  void Playback(
    ...
    if (!gpu_memory_buffer_) {
      owned_gpu_memory_buffer_ =
          gpu_memory_buffer_manager_->CreateGpuMemoryBuffer(
              resource_size_, viz::BufferFormat(resource_format_), kBufferUsage,
              gpu::kNullSurfaceHandle);
      if (!owned_gpu_memory_buffer_)
        return;
      gpu_memory_buffer_ = owned_gpu_memory_buffer_.get();
    }
 ...
 private:
  ...
  std::unique_ptr<gfx::GpuMemoryBuffer> owned_gpu_memory_buffer_;
