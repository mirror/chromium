...
void OneCopyRasterBufferProvider::CopyOnWorkerThread(
  ...
  if (!staging_buffer->texture_id) {
    rs->GenTextures(1, &staging_buffer->texture_id);
    rs->BindTexture(image_target, staging_buffer->texture_id);
    rs->TexParameteri(image_target, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    rs->TexParameteri(image_target, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    rs->TexParameteri(image_target, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    rs->TexParameteri(image_target, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
  } else {
    rs->BindTexture(image_target, staging_buffer->texture_id);
  }
  ...
  if (!staging_buffer->image_id) {
    if (staging_buffer->gpu_memory_buffer) {
      staging_buffer->image_id = rs->CreateImageCHROMIUM(
      ...
      rs->BindTexImage2DCHROMIUM(image_target, staging_buffer->image_id);
    }
  } else {
    rs->ReleaseTexImage2DCHROMIUM(image_target, staging_buffer->image_id);
    rs->BindTexImage2DCHROMIUM(image_target, staging_buffer->image_id);
  }

  // Unbind staging texture.
  rs->BindTexture(image_target, 0);

  if (resource_provider_->use_sync_query()) {
    if (!staging_buffer->query_id)
      rs->GenQueriesEXT(1, &staging_buffer->query_id);

#if defined(OS_CHROMEOS) && defined(ARCH_CPU_ARM_FAMILY)
    // TODO(reveman): This avoids a performance problem on ARM ChromeOS
    // devices. crbug.com/580166
    rs->BeginQueryEXT(GL_COMMANDS_ISSUED_CHROMIUM, staging_buffer->query_id);
#else
    rs->BeginQueryEXT(GL_COMMANDS_COMPLETED_CHROMIUM, staging_buffer->query_id);
#endif
  }
  ...
  if (IsResourceFormatCompressed(resource_lock->format())) {
    rs->CompressedCopyTextureCHROMIUM(staging_buffer->texture_id, texture_id);
  } else {
  ...
  if (resource_provider_->use_sync_query()) {
#if defined(OS_CHROMEOS) && defined(ARCH_CPU_ARM_FAMILY)
    rs->EndQueryEXT(GL_COMMANDS_ISSUED_CHROMIUM);
#else
    rs->EndQueryEXT(GL_COMMANDS_COMPLETED_CHROMIUM);
#endif
  }
