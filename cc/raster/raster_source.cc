...
void RasterSource::PlaybackToCanvas(
  ...
  if (settings.clear_canvas_before_raster) {
    if (requires_clear_ && !is_partial_raster) {
      ...
      SkIRect playback_device_rect = gfx::RectToSkIRect(canvas_playback_rect);
      playback_device_rect.fLeft -= canvas_bitmap_rect.x();
      playback_device_rect.fTop -= canvas_bitmap_rect.y();
      ...
      // in RasterSource.
      SkIRect device_column = SkIRect::MakeXYWH(
          coverage_device_rect.right(), 0, 2, coverage_device_rect.bottom());
      // row includes the corner, column excludes it.
      SkIRect device_row =
          SkIRect::MakeXYWH(0, coverage_device_rect.bottom(),
