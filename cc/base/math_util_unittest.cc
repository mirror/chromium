...
TEST(MathUtilTest, ProjectionOfPerpendicularPlane) {
  ...
  gfx::RectF projected_rect =
      viz::MathUtil::ProjectClippedRect(transform, rect);
...
TEST(MathUtilTest, EnclosingClippedRectHandlesInfinityY) {
  viz::HomogeneousCoordinate h1(100, 10, 0, 1);
  viz::HomogeneousCoordinate h2(10, 10, 0, 1);
  viz::HomogeneousCoordinate h3(-10, -1, 0, -1);
  viz::HomogeneousCoordinate h4(-100, -1, 0, -1);
  ...
  gfx::RectF result =
      viz::MathUtil::ComputeEnclosingClippedRect(h1, h2, h3, h4);
...
TEST(MathUtilTest, EnclosingClippedRectHandlesNegativeInfinityX) {
  viz::HomogeneousCoordinate h1(100, 10, 0, 1);
  viz::HomogeneousCoordinate h2(-110, -10, 0, -1);
  viz::HomogeneousCoordinate h3(-110, -100, 0, -1);
  viz::HomogeneousCoordinate h4(100, 100, 0, 1);
...
TEST(MathUtilTest, EnclosingClippedRectHandlesInfinityXY) {
  viz::HomogeneousCoordinate h1(10, 10, 0, 1);
  viz::HomogeneousCoordinate h2(0, 0, 0, -1);
  viz::HomogeneousCoordinate h3(20, -10, 0, 1);
  viz::HomogeneousCoordinate h4(10, -10, 0, 1);
...
TEST(MathUtilTest, EnclosingClippedRectUsesCorrectInitialBounds) {
  viz::HomogeneousCoordinate h1(-100, -100, 0, 1);
  viz::HomogeneousCoordinate h2(-10, -10, 0, 1);
  viz::HomogeneousCoordinate h3(10, 10, 0, -1);
  viz::HomogeneousCoordinate h4(100, 100, 0, -1);
...
TEST(MathUtilTest, EnclosingRectOfVerticesUsesCorrectInitialBounds) {
  ...
  // then the enclosing clipped rect will be computed incorrectly.
  gfx::RectF result =
      viz::MathUtil::ComputeEnclosingRectOfVertices(vertices, num_vertices);
...
TEST(MathUtilTest, SmallestAngleBetweenVectors) {
  ...
  EXPECT_EQ(90, viz::MathUtil::SmallestAngleBetweenVectors(x, y));

  // A vector makes a zero angle with itself.
  EXPECT_EQ(0, viz::MathUtil::SmallestAngleBetweenVectors(x, x));
  EXPECT_EQ(0, viz::MathUtil::SmallestAngleBetweenVectors(y, y));
  EXPECT_EQ(
      0, viz::MathUtil::SmallestAngleBetweenVectors(test_vector, test_vector));

  // Parallel but reversed vectors are at 180 degrees.
  EXPECT_FLOAT_EQ(180, viz::MathUtil::SmallestAngleBetweenVectors(x, -x));
  EXPECT_FLOAT_EQ(180, viz::MathUtil::SmallestAngleBetweenVectors(y, -y));
  EXPECT_FLOAT_EQ(180, viz::MathUtil::SmallestAngleBetweenVectors(
                           test_vector, -test_vector));

  // The test vector is at a known angle.
  EXPECT_FLOAT_EQ(45, std::floor(viz::MathUtil::SmallestAngleBetweenVectors(
                          test_vector, x)));
  EXPECT_FLOAT_EQ(45, std::floor(viz::MathUtil::SmallestAngleBetweenVectors(
                          test_vector, y)));
}
...
TEST(MathUtilTest, VectorProjection) {
  ...
  EXPECT_VECTOR_EQ(gfx::Vector2dF(0, 0), viz::MathUtil::ProjectVector(x, y));
  EXPECT_VECTOR_EQ(gfx::Vector2dF(0, 0), viz::MathUtil::ProjectVector(y, x));

  // Projecting a vector onto the orthonormal basis gives the corresponding
  // component of the vector.
  EXPECT_VECTOR_EQ(gfx::Vector2dF(test_vector.x(), 0),
                   viz::MathUtil::ProjectVector(test_vector, x));
  EXPECT_VECTOR_EQ(gfx::Vector2dF(0, test_vector.y()),
                   viz::MathUtil::ProjectVector(test_vector, y));
  ...
  gfx::Vector2dF projected_vector =
      viz::MathUtil::ProjectVector(test_vector, target_vector);
...
TEST(MathUtilTest, MapEnclosedRectWith2dAxisAlignedTransform) {
  ...
  // Identity.
  output = viz::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(transform,
                                                                    input);
...
TEST(MathUtilTest, MapEnclosingRectWithLargeTransforms) {
  ...
  output = viz::MathUtil::MapEnclosingClippedRect(large_x_scale, input);
  EXPECT_EQ(gfx::Rect(max_int, 2, 0, 200), output);

  output =
      viz::MathUtil::MapEnclosingClippedRect(large_x_scale * rotation, input);
  EXPECT_EQ(gfx::Rect(), output);

  output = viz::MathUtil::MapEnclosingClippedRect(infinite_x_scale, input);
  EXPECT_EQ(gfx::Rect(max_int, 2, 0, 200), output);

  output = viz::MathUtil::MapEnclosingClippedRect(infinite_x_scale * rotation,
                                                  input);
  EXPECT_EQ(gfx::Rect(), output);

  output = viz::MathUtil::MapEnclosingClippedRect(large_y_scale, input);
  EXPECT_EQ(gfx::Rect(1, max_int, 100, 0), output);

  output =
      viz::MathUtil::MapEnclosingClippedRect(large_y_scale * rotation, input);
  EXPECT_EQ(gfx::Rect(-100, max_int, 100, 0), output);

  output = viz::MathUtil::MapEnclosingClippedRect(infinite_y_scale, input);
  EXPECT_EQ(gfx::Rect(1, max_int, 100, 0), output);

  output = viz::MathUtil::MapEnclosingClippedRect(infinite_y_scale * rotation,
                                                  input);
  EXPECT_EQ(gfx::Rect(), output);
}
...
TEST(MathUtilTest, ProjectEnclosingRectWithLargeTransforms) {
  ...
  output = viz::MathUtil::ProjectEnclosingClippedRect(large_x_scale, input);
  EXPECT_EQ(gfx::Rect(max_int, 2, 0, 200), output);

  output = viz::MathUtil::ProjectEnclosingClippedRect(large_x_scale * rotation,
                                                      input);
  EXPECT_EQ(gfx::Rect(), output);

  output = viz::MathUtil::ProjectEnclosingClippedRect(infinite_x_scale, input);
  EXPECT_EQ(gfx::Rect(max_int, 2, 0, 200), output);

  output = viz::MathUtil::ProjectEnclosingClippedRect(
      infinite_x_scale * rotation, input);
  EXPECT_EQ(gfx::Rect(), output);

  output = viz::MathUtil::ProjectEnclosingClippedRect(large_y_scale, input);
  EXPECT_EQ(gfx::Rect(1, max_int, 100, 0), output);

  output = viz::MathUtil::ProjectEnclosingClippedRect(large_y_scale * rotation,
                                                      input);
  EXPECT_EQ(gfx::Rect(-103, max_int, 102, 0), output);

  output = viz::MathUtil::ProjectEnclosingClippedRect(infinite_y_scale, input);
  EXPECT_EQ(gfx::Rect(1, max_int, 100, 0), output);

  output = viz::MathUtil::ProjectEnclosingClippedRect(
      infinite_y_scale * rotation, input);
  EXPECT_EQ(gfx::Rect(), output);
}
...
TEST(MathUtilTest, RoundUp) {
  for (int multiplier = 1; multiplier <= 10; ++multiplier) {
    ...
    for (int attempt = 5 * multiplier; attempt >= -5 * multiplier; --attempt) {
      if ((attempt % multiplier) == 0)
        correct = attempt;
      EXPECT_EQ(correct, viz::MathUtil::UncheckedRoundUp(attempt, multiplier))
  ...
  for (unsigned multiplier = 1; multiplier <= 10; ++multiplier) {
    ...
    EXPECT_EQ(0u, viz::MathUtil::UncheckedRoundUp(0u, multiplier))
...
TEST(MathUtilTest, RoundUpOverflow) {
  ...
  EXPECT_FALSE(viz::MathUtil::VerifyRoundup<int8_t>(123, 50));
  EXPECT_TRUE(viz::MathUtil::VerifyRoundup<uint8_t>(123, 50));
}
...
TEST(MathUtilTest, RoundDown) {
  for (int multiplier = 1; multiplier <= 10; ++multiplier) {
    ...
    for (int attempt = -5 * multiplier; attempt <= 5 * multiplier; ++attempt) {
      if ((attempt % multiplier) == 0)
        correct = attempt;
      EXPECT_EQ(correct, viz::MathUtil::UncheckedRoundDown(attempt, multiplier))
          << "attempt=" << attempt << " multiplier=" << multiplier;
    }
  }
...
TEST(MathUtilTest, RoundDownUnderflow) {
  ...
  // int16_t.
  EXPECT_FALSE(viz::MathUtil::VerifyRoundDown<int8_t>(-123, 50));
  EXPECT_TRUE(viz::MathUtil::VerifyRoundDown<int16_t>(-123, 50));
}

#define EXPECT_SIMILAR_VALUE(x, y) \
  EXPECT_TRUE(viz::MathUtil::IsFloatNearlyTheSame(x, y))
#define EXPECT_DISSIMILAR_VALUE(x, y) \
  EXPECT_FALSE(viz::MathUtil::IsFloatNearlyTheSame(x, y))
...
#define EXPECT_SIMILAR_POINT_F(x, y) \
  EXPECT_TRUE(                       \
      viz::MathUtil::IsNearlyTheSameForTesting(gfx::PointF x, gfx::PointF y))
...
#define EXPECT_SIMILAR_POINT_3F(x, y)                                  \
  EXPECT_TRUE(viz::MathUtil::IsNearlyTheSameForTesting(gfx::Point3F x, \
                                                       gfx::Point3F y))
#define EXPECT_DISSIMILAR_POINT_3F(x, y)                                \
  EXPECT_FALSE(viz::MathUtil::IsNearlyTheSameForTesting(gfx::Point3F x, \
                                                        gfx::Point3F y))
...
TEST(MathUtilTest, MapClippedQuadDuplicateTriangle) {
  ...
  viz::MathUtil::MapClippedQuad3d(transform, src_quad, clipped_quad,
