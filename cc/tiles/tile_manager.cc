...
TileManager::PrioritizedWorkToSchedule TileManager::AssignGpuMemoryToTiles() {
  ...
  for (; !raster_priority_queue->IsEmpty(); raster_priority_queue->Pop()) {
    ...
    if (tile->IsPrepaint() && prioritized_tile.is_process_for_images_only()) {
...
void TileManager::ScheduleTasks(PrioritizedWorkToSchedule work_to_schedule) {
  ...
  new_locked_image_tasks.insert(
      new_locked_image_tasks.end(),
      work_to_schedule.extra_image_decode_tasks.begin(),
      work_to_schedule.extra_image_decode_tasks.end());
...
scoped_refptr<TileTask> TileManager::CreateRasterTask(
  ...
  if (at_raster_images.size() > 0 && tile->IsPrepaint()) {
    image_controller_.UnrefImages(sync_decoded_images);
    work_to_schedule->extra_image_decode_tasks.insert(
        work_to_schedule->extra_image_decode_tasks.end(), decode_tasks.begin(),
        decode_tasks.end());
    resource_pool_->ReleaseResource(resource);
    scheduled_draw_images_.erase(tile->id());
    return nullptr;
  }
