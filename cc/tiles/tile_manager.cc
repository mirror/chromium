...
class RasterTaskImpl : public TileTask {
 public:
  ...
  void OnTaskCompleted() override {
    ...
    // XXX(sad): The nullcheck only needed in tests. Create a dummy RasterBuffer
    // instead in FakeRasterBufferProviderImpl::AcquireBufferForRaster()?
    size_t num_bytes = raster_buffer_ ? raster_buffer_->NumBytes() : 0;
