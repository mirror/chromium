...
// remmains locked for the duration of the raster tasks which depend on it.
//
...
class CC_EXPORT GpuImageDecodeCache
 ...
 public:
  ...
  explicit GpuImageDecodeCache(ContextProvider* context_provider,
  ...
  size_t GetBytesLockedForTesting() const { return bytes_locked_; }
 ...
 private:
  ...
  struct UploadedImageData : public ImageDataBase {
    ...
    uint32_t gl_id() const { return gl_id_; }
  ...
  // functions also require the |context_provider_| lock. These are indicated by
  // additional comments.
  ...
  // The following two functions also require the |context_provider_| lock to be
  // held.
  ...
  // If |have_context_lock|, then the |context_provider_| lock must be held
  // when calling.
  bool TryLockImage(bool have_context_lock,
  ...
  std::vector<uint32_t> images_pending_complete_lock_;
  std::vector<uint32_t> images_pending_unlock_;
