...
enum CheckerImagingDecision : int {
  kCanChecker,
...
CheckerImagingDecision GetSizeDecision(const PaintImage& image,
  ...
  if (size < kMinImageSizeToCheckerBytes)
    return kVetoedSmallerThanCheckeringSize;
  else if (size > max_bytes)
    return kVetoedLargerThanCacheSize;
  else
    return kCanChecker;
}
...
bool CheckerImageTracker::ShouldCheckerImage(const DrawImage& draw_image,
  ...
  if (insert_result.second) {
    ...
    CheckerImagingDecision decision = kCanChecker;
    if (GetAnimationDecision(image) != kCanChecker ||
        GetLoadDecision(image) != kCanChecker ||
        GetSizeDecision(image,
    ...
    UMA_HISTOGRAM_ENUMERATION("Compositing.Renderer.CheckerImagingDecision",
                              decision, kCheckerImagingDecisionCount);
