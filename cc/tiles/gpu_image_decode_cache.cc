...
GpuImageDecodeCache::GpuImageDecodeCache(ContextProvider* context_provider,
  ...
  {
    ContextProvider::ScopedContextLock context_lock(context_provider_);
    context_threadsafe_proxy_ = sk_sp<GrContextThreadSafeProxy>(
        context_provider_->GrContext()->threadSafeProxy());
  }
...
bool GpuImageDecodeCache::GetTaskForImageAndRefInternal(
  ...
  // This ref is owned by the caller, and it is their responsibility to release
  // it by calling UnrefImage.
  RefImage(draw_image);
...
DecodedDrawImage GpuImageDecodeCache::GetDecodedImageForDraw(
  ...
  context_provider_->GetLock()->AssertAcquired();
...
void GpuImageDecodeCache::ReduceCacheUsage() {
  ...
  if (context_provider_->GetLock()->Try()) {
    RunPendingOperations();
    context_provider_->GetLock()->Release();
  }
}
...
bool GpuImageDecodeCache::OnMemoryDump(
  ...
  for (const auto& image_pair : persistent_cache_) {
    ...
    if (image_data->upload.image() &&
      ...
      // process counterpart.
      uint32_t gl_id = skia::GrBackendObjectToGrGLTextureInfo(
      ...
      MemoryAllocatorDumpGuid guid = gl::GetGLTextureClientGUIDForTracing(
          context_provider_->ContextSupport()->ShareGroupTracingGUID(), gl_id);
...
void GpuImageDecodeCache::UploadImageIfNecessary(const DrawImage& draw_image,
    ...
    switch (image_data->mode) {
      ...
      case DecodedDataMode::GPU: {
        uploaded_image = SkImage::MakeFromDeferredTextureImageData(
            context_provider_->GrContext(), image_data->decode.data()->data(),
            SkBudgeted::kNo);
        break;
      }
    }
  ...
  if (!image_data->upload.image()) {
    ...
    if (image_data->mode == DecodedDataMode::GPU) {
      ...
      // budgets.
      context_provider_->ContextGL()->InitializeDiscardableTextureCHROMIUM(
...
void GpuImageDecodeCache::UnlockImage(ImageData* image_data) {
  images_pending_unlock_.push_back(image_data->upload.gl_id());
...
void GpuImageDecodeCache::RunPendingOperations() {
  ...
  for (uint32_t gl_id : images_pending_complete_lock_) {
    context_provider_->ContextSupport()->CompleteLockDiscardableOnContextThread(
        gl_id);
  }
  images_pending_complete_lock_.clear();

  for (uint32_t gl_id : images_pending_unlock_) {
    context_provider_->ContextGL()->UnlockDiscardableTextureCHROMIUM(gl_id);
  }
  images_pending_unlock_.clear();

  for (auto& image : images_pending_deletion_) {
    uint32_t texture_id = GlIdFromSkImage(image.get());
    if (context_provider_->ContextGL()->LockDiscardableTextureCHROMIUM(
            texture_id)) {
...
bool GpuImageDecodeCache::TryLockImage(bool have_context_lock,
  ...
  if (have_context_lock) {
    ...
    if (context_provider_->ContextSupport()->ThreadsafeBeginLockDiscardable(
            data->upload.gl_id())) {
      data->upload.OnLock();
      images_pending_complete_lock_.push_back(data->upload.gl_id());
    }
  }
