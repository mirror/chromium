...
bool GpuImageDecodeCache::GetTaskForImageAndRefInternal(
  ...
  if (!image_data) {
    ...
    *task = nullptr;
    return false;
  } else if (image_data->upload.image() && TryLockImage(compositor_context_, image_data)) {
...
void GpuImageDecodeCache::OwnershipChanged(const DrawImage& draw_image,
  ...
  if (image_data->upload.ref_count == 0 && image_data->upload.image() && image_data->upload.is_locked) {
    images_pending_unlock_.push_back(image_data->upload.image());
    image_data->upload.is_locked = false;
  }

  // If we have image refs on a non-at-raster image, it must be budgeted, as it
  // is either uploaded or pending upload.
  if (image_data->upload.ref_count > 0 && image_data->upload.is_locked && !image_data->upload.budgeted &&
  ...
  if (image_data->upload.image()) {
    DCHECK(image_data->is_at_raster || image_data->upload.budgeted || !image_data->upload.is_locked);
...
void GpuImageDecodeCache::DecodeImageIfNecessary(const DrawImage& draw_image,
  ...
  if (image_data->upload.image() && TryLockImage(worker_context_, image_data)) {
...
void GpuImageDecodeCache::DeletePendingImages() {
  ...
  for (auto& image :images_pending_deletion_) {
      if (!compositor_context_->ContextGL()->LockDiscardableTextureCHROMIUM(
          GlIdFromSkImage(image.get()))) {
            image->getTexture()->abandon();
  }
  }
...
bool GpuImageDecodeCache::TryLockImage(ContextProvider* context_provider, ImageData* data) {
  ...
  if (context_provider->ContextGL()->LockDiscardableTextureCHROMIUM(
          GlIdFromSkImage(data->upload.image().get()))) {
    data->upload.is_locked = true;
