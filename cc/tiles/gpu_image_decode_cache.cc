...
sk_sp<SkImage> TakeOwnershipOfSkImageBacking(GrContext* context,
  ...
  SkImage::MakeBackendTextureFromSkImage(gr_context, std::move(image),
                                         &backend_texture, &release_proc);
  return SkImage::MakeFromTexture(gr_context, backend_texture,
...
void GpuImageDecodeCache::UploadImageIfNecessary(const DrawImage& draw_image,
  ...
  if (uploaded_image) {
    uploaded_image = TakeOwnershipOfSkImageBacking(std::move(uploaded_image));
  }
