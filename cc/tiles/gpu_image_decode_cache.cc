...
bool DrawAndScaleImage(const DrawImage& draw_image, SkPixmap* target_pixmap) {
  ...
  // If we can't decode/scale directly, we will handle this in three steps.
...
GpuImageDecodeCache::GpuImageDecodeCache(viz::ContextProvider* context,
  ...
  // Acquire the context_lock so that we can safely retrieve max_texture_size_.
  {
