...
// Color converts an SkImage, preventing the original texture from being cached
// or re-used.
sk_sp<SkImage> ConvertColorspaceAndPreventCaching(
  ...
  sk_sp<SkImage> orignal_image_owned =
      TakeOwnershipOfSkImageBacking(context->GrContext(), std::move(image));
  sk_sp<SkImage> converted_image = orignal_image_owned->makeColorSpace(
      color_space, SkTransferFunctionBehavior::kIgnore);

  // Delete |orignal_image_owned| as Skia will not clean it up.
  uint32_t texture_id = GlIdFromSkImage(orignal_image_owned.get());
