...
void GpuImageDecodeCache::RunPendingOperations() {
  ...
  // No need to flush pending IO for locking, so we use |gl_id| directly,
  // rather than calling GlIdFromSkImage, which flushes IO.
  for (uint32_t gl_id : images_pending_complete_lock_) {
    context_->ContextSupport()->CompleteLockDiscardableOnContextThread(gl_id);
  }
  ...
  for (auto& image : images_pending_deletion_) {
    // We *always* want to abandon deleted images, to prevent Skia from trying
    // to cache/re-use them.
