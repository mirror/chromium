...
ImageDecodeCache::TaskResult GpuImageDecodeCache::GetTaskForImageAndRefInternal(
  ...
  if (task_type == DecodeTaskType::PART_OF_UPLOAD_TASK) {
    ...
    RefImage(draw_image);
    result.task = make_scoped_refptr(new ImageUploadTaskImpl(
        this, draw_image,
        GetImageDecodeTaskAndRef(draw_image, tracing_info, task_type),
        tracing_info));
    image_data->upload.task = result.task;
  } else {
    result.task = GetImageDecodeTaskAndRef(draw_image, tracing_info, task_type);
  }
