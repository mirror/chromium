...
class CC_EXPORT TileManager : CheckerImageTrackerClient {
 public:
  ...
  bool AreRasterQueueTilesReadyToDrawForTesting(
      RasterTilePriorityQueue::Type type) const {
    return AreRasterQueueTilesReadyToDraw(type);
  }
 ...
 private:
  ...
  struct Signals {
    ...
    bool required_for_activation_tile_tasks_completed = false;
    bool required_for_activation_tiles_ready_to_draw = false;
    bool did_notify_ready_to_activate = false;
    bool required_for_draw_tile_tasks_completed = false;
    bool required_for_draw_tiles_ready_to_draw = false;
  ...
  bool AreRasterQueueTilesReadyToDraw(RasterTilePriorityQueue::Type type) const;
  ...
  bool has_pending_required_for_activation_ready_to_draw_callback_ = false;
  bool has_pending_required_for_draw_ready_to_draw_callback_ = false;
  ...
  // Invalidated on each PrepareTiles.
