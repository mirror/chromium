...
class ImageDecodeTaskImpl : public TileTask {
 public:
  ...
  void RunOnWorkerThread() override {
    ...
    cache_->DecodeImageFromTask(image_key_, image_, task_type_);
  }
...
void SoftwareImageDecodeCache::DecodeImageFromTask(const ImageKey& key,
                                                   const DrawImage& image,
                                                   DecodeTaskType task_type) {
  TRACE_EVENT1("cc", "SoftwareImageDecodeCache::DecodeImageFromTask", "key",
               key.ToString());
  base::AutoLock lock(lock_);
...
void SoftwareImageDecodeCache::DecodeImageIfNecessary(
  ...
  if (!paint_image || key.target_size().IsEmpty())
  ...
  SkIRect target_size_rect =
      SkIRect::MakeWH(key.target_size().width(), key.target_size().height());
  // If this is a full size, unsubrected image, we will definitely need to do a
  // decode.
  std::unique_ptr<CacheEntry> local_cache_entry;
  {
    ...
    if (!need_subset && target_size_rect == full_size_rect) {
      local_cache_entry = DoDecodeImage(key, paint_image, nullptr);
    } else {
      ...
      if (!decoded_draw_image.image()) {
        ...
        local_cache_entry =
            DoDecodeImage(key, paint_image, &decoded_draw_image);
      }
    }
...
SoftwareImageDecodeCache::DoDecodeImage(
  ...
  DCHECK(candidate_image ||
         target_size == paint_image.GetSupportedDecodeSize(target_size));
  ...
  std::unique_ptr<base::DiscardableMemory> target_pixels;
  {
    ...
    target_pixels = base::DiscardableMemoryAllocator::GetInstance()
...
bool SoftwareImageDecodeCache::OnMemoryDump(
  ...
  if (args.level_of_detail == MemoryDumpLevelOfDetail::BACKGROUND) {
    ...
    // TODO(vmpstr): We can inline this call.
...
void SoftwareImageDecodeCache::DumpImageMemoryForCache(
  ...
  for (const auto& image_pair : cache) {
    ...
    MemoryAllocatorDump* dump =
        image_pair.second->memory->CreateMemoryAllocatorDump(dump_name.c_str(),
                                                             pmd);
    DCHECK(dump);
    size_t locked_bytes =
...
// CacheEntry
SoftwareImageDecodeCache::CacheEntry::CacheEntry() = default;
