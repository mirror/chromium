...
class FakeGPUImageDecodeTestGLES2Interface : public TestGLES2Interface,
 ...
 public:
  ...
  void UnlockTransferCacheEntry(TransferCacheEntryType entry_type,
                                uint32_t entry_id) override {
    transfer_cache_helper_->UnlockEntryDirect(entry_type, entry_id);
  }
...
class GpuImageDecodeCacheTest
 ...
 public:
  void SetUp() override {
    context_provider_ = DiscardableTextureMockContextProvider::Create(
  ...
  std::unique_ptr<GpuImageDecodeCache> CreateCache() {
    return base::WrapUnique(
        new GpuImageDecodeCache(context_provider_.get(), GetParam().second,
                                GetParam().first, kGpuMemoryLimitBytes));
  }
  ...
  DecodedDrawImage EnsureImageBacked(DecodedDrawImage&& draw_image) {
    if (draw_image.transfer_cache_entry_id()) {
      ...
      new_draw_image.set_at_raster_decode(draw_image.is_at_raster_decode());
