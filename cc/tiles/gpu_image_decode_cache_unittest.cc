...
class GpuImageDecodeCacheTest : public testing::Test {
 public:
  ...
  std::unique_ptr<GpuImageDecodeCache> CreateCache() {
    return base::WrapUnique(new GpuImageDecodeCache(
        ...
        ResourceFormat::RGBA_8888, kGpuMemoryLimitBytes));
  }

  DiscardableTextureMockContextProvider* worker_context_provider() {
    return worker_context_provider_.get();
  }

  DiscardableTextureMockContextProvider* compositor_context_provider() {
 ...
 private:
  ...
  scoped_refptr<DiscardableTextureMockContextProvider>
      compositor_context_provider_;
  scoped_refptr<DiscardableTextureMockContextProvider> worker_context_provider_;
};
...
TEST_F(GpuImageDecodeCacheTest, OrphanedImagesFreeOnReachingZeroRefs) {
  ...
  EXPECT_EQ(1, cache->GetNumCacheEntriesForTesting());
  EXPECT_EQ(0, cache->GetInUseCacheEntriesForTesting());
}
