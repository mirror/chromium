...
class FakeDiscardableManager {
 ...
 private:
  ...
  void ExpectLocked(GLuint texture_id) {
    ...
    static_assert(kHandleDeleted < kHandleLockedStart,
                  "kHandleDeleted must be less than kHandleLockedStart.");
    static_assert(kHandleUnlocked < kHandleLockedStart,
                  "kHandleUnlocked must be less than kHandleLockedStart.");
    EXPECT_GE(texture_[texture_id], kHandleLockedStart);
  }
