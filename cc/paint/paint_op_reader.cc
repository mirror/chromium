...
void PaintOpReader::Read(PaintFlags* flags) {
  ...
  // Flattenables were written at the start of a 4-byte alignment, so skip
  // memory until we reach a 4-byte boundary.
  while (!SkIsAlign4(reinterpret_cast<uintptr_t>(memory_))) {
