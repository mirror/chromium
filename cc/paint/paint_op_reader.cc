...
void PaintOpReader::Read(PaintFlags* flags) {
  // TODO(enne): this isn't shippable.  Sadly, SkPaint::unflatten and
  // SkReader32 just attempt to read and then assert if it doesn't work.
  // PaintFlags will probably need each value serialized independently.
  // SkTextBlob looks like it aborts on failure.

  // Clobber any garbage.
  memset(flags, 0, sizeof(PaintFlags));

  SkReadBuffer buffer(memory_, remaining_bytes_);
  flags->paint_.unflatten(buffer);

  size_t read_bytes = buffer.offset();

  if (read_bytes > remaining_bytes_)
    valid_ = false;
  if (!valid_)
    return;
...
void PaintOpReader::Read(PaintImage* image) {
  // TODO(enne): figure out how to serialize images
}
...
void PaintOpReader::Read(sk_sp<SkData>* data) {
  ...
  // Clobber any garbage.
  memset(data, 0, sizeof(sk_sp<SkData>));
...
void PaintOpReader::Read(sk_sp<SkTextBlob>* blob) {
  // Clobber any garbage.
  memset(blob, 0, sizeof(sk_sp<SkTextBlob>));

  SkReadBuffer buffer(memory_, remaining_bytes_);
  *blob = SkTextBlob::MakeFromBuffer(buffer);
  if (!*blob)
    valid_ = false;
}
