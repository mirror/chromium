...
void SolidColorAnalyzer::HandleStateOp(const PaintOp* op) {
  // We need to handle these explicitly, since Skia returns isClipRect() for the
  // base SkCanvas even if the clip wasn't a rect. Note that there is a chance
  // that we have a save/clip/restore without any draws and we should keep a
  // stack of save/restores in order to detect those cases as potentially solid.
  // However, this is a pessimistic approach of assuming that we're not solid if
  // we see a complex clip.
  if (op->GetType() == PaintOpType::ClipPath ||
      op->GetType() == PaintOpType::ClipRRect) {
    is_solid_color_ = false;
    is_transparent_ = false;
    abort_analysis();
  }
