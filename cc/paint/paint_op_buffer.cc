...
struct Rasterizer<DrawRecordOp, false> {
  static void RasterWithAlpha(const DrawRecordOp* op,
                              SkCanvas* canvas,
                              uint8_t alpha) {
    DCHECK_EQ(op->record->size(), 1u);
    PaintOp* single_op = op->record->GetFirstOp();

    // RasterWithAlpha only supported for draw ops, and we don't support
    // nested PaintOpBuffers.
    DCHECK(single_op->IsDrawOp());
    DCHECK_NE(single_op->GetType(), PaintOpType::DrawRecord);
    DCHECK_NE(single_op->GetType(), PaintOpType::DrawDisplayItemList);
