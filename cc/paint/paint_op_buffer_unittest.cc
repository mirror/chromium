...
static bool g_op_type_has_flags[kNumOpTypes] = {TYPES(M)};
#undef M
...
void PushSaveLayerAlphaOps(PaintOpBuffer* buffer) {
  ...
  // Test optional args.
  buffer->push<SaveLayerAlphaOp>(nullptr, 32, false);
}
...
TEST_P(PaintOpSerializationTest, SmokeTest) {
  ...
  for (size_t i = 0; i < buffer_.size(); ++i) {
    SCOPED_TRACE(base::StringPrintf("op: %ld", i));
  ...
  for (auto* base_written :
    ...
    SCOPED_TRACE(base::StringPrintf(
        "%s #%ld", PaintOpTypeToString(GetParamType()).c_str(), iter.op_idx()));
...
TEST_P(PaintOpSerializationTest, SerializationFailures) {
  ...
  for (PaintOpBuffer::Iterator iter(&buffer_); iter; ++iter) {
    ...
    for (size_t i = 0; i < bytes_written[iter.op_idx()] + 2; ++i) {
      size_t written_bytes = iter->Serialize(output_.get(), i);
      if (i >= expected_bytes) {
...
TEST_P(PaintOpSerializationTest, DeserializationFailures) {
  ...
  // TODO(enne): SkPaint deserialization is not safe for smaller sizes.
  // It just asserts if it can't finish reading.
  if (g_op_type_has_flags[static_cast<uint8_t>(GetParamType())])
    return;
  ...
  for (PaintOpBuffer::Iterator iter(&buffer_); iter; ++iter) {
    ...
    for (size_t read_size = 0; read_size < skip + kAlign * 2 + 2; ++read_size) {
      SCOPED_TRACE(
          base::StringPrintf("%s #%ld, read_size: %ld",
      ...
      const PaintOp* written = PaintOp::Deserialize(
...
TEST(PaintOpBufferTest, PaintOpDeserialize) {
  ...
  size_t bytes_written = op->Serialize(input_.get(), kSize);
