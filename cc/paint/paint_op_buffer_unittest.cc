...
TEST(PaintOpBufferTest, ContiguousRanges) {
  ...
  buffer.push<DrawColorOp>(0, SkBlendMode::kClear);
  buffer.push<DrawColorOp>(1, SkBlendMode::kClear);
  buffer.push<DrawColorOp>(2, SkBlendMode::kClear);
  buffer.push<DrawColorOp>(3, SkBlendMode::kClear);
  buffer.push<DrawColorOp>(4, SkBlendMode::kClear);

  // Ranges are {0, 1}, {2}, {3, 4}.
  std::vector<int> ranges = {0, 2, 3};

  // Plays all 3 ranges.
  testing::Sequence s;
  EXPECT_CALL(canvas, OnDrawPaintWithColor(0)).InSequence(s);
  EXPECT_CALL(canvas, OnDrawPaintWithColor(1)).InSequence(s);
  EXPECT_CALL(canvas, OnDrawPaintWithColor(2)).InSequence(s);
  EXPECT_CALL(canvas, OnDrawPaintWithColor(3)).InSequence(s);
  EXPECT_CALL(canvas, OnDrawPaintWithColor(4)).InSequence(s);
...
TEST(PaintOpBufferTest, ContiguousRangeWithSaveLayerAlphaRestore) {
  ...
  buffer.push<DrawColorOp>(0, SkBlendMode::kClear);
  buffer.push<DrawColorOp>(1, SkBlendMode::kClear);
  buffer.push<SaveLayerAlphaOp>(nullptr, 1);
  buffer.push<RestoreOp>();
  buffer.push<DrawColorOp>(2, SkBlendMode::kClear);
  buffer.push<DrawColorOp>(3, SkBlendMode::kClear);
  buffer.push<DrawColorOp>(4, SkBlendMode::kClear);
...
TEST(PaintOpBufferTest, ContiguousRangeWithSaveLayerAlphaDrawRestore) {
  ...
  add_draw_rect(&buffer, 0);
  add_draw_rect(&buffer, 1);
  buffer.push<SaveLayerAlphaOp>(nullptr, 1);
  add_draw_rect(&buffer, 2);
  buffer.push<RestoreOp>();
  add_draw_rect(&buffer, 3);
  add_draw_rect(&buffer, 4);

  {
    // Ranges are {0, 1, save, 2, restore}, {3, 4}.
    std::vector<int> ranges = {0, 5};

    testing::Sequence s;
    EXPECT_CALL(canvas, OnDrawRectWithColor(0)).InSequence(s);
    EXPECT_CALL(canvas, OnDrawRectWithColor(1)).InSequence(s);
    // The empty SaveLayerAlpha/Restore is dropped, the containing
    // operation can be drawn with alpha.
    EXPECT_CALL(canvas, OnDrawRectWithColor(2)).InSequence(s);
    EXPECT_CALL(canvas, OnDrawRectWithColor(3)).InSequence(s);
    EXPECT_CALL(canvas, OnDrawRectWithColor(4)).InSequence(s);
