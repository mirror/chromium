...
class CC_PAINT_EXPORT PaintTypeface {
...
#if defined(OS_MACOSX)
  static PaintTypeface FromNSFont(NSFont* ns_font,
                                  float requested_size,
                                  std::vector<SkFontArguments::Axis> axes,
                                  const PaintFontLoaderMac& loader);

  static PaintTypeface FromNSFontParams(const base::string16& font_name,
                                        float font_size,
                                        float requested_size,
                                        std::vector<SkFontArguments::Axis> axes,
                                        const PaintFontLoaderMac& loader);
 ...
 private:
#if defined(OS_MACOSX)
  void CreateSkTypefaceFromNSFont(NSFont* ns_font,
                                  const PaintFontLoaderMac* loader);
#endif
