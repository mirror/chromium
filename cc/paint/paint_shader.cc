...
sk_sp<SkPicture> ToSkPicture(sk_sp<PaintRecord> record,
  ...
  record->Playback(canvas, image_provider, decoded_image_stash);
...
sk_sp<PaintShader> PaintShader::CreateDecodedPaintRecord(
  ...
  shader->decoded_image_stash_ = decoded_image_stash;
...
sk_sp<SkShader> PaintShader::GetSkShader() const {
  ...
  switch (shader_type_) {
    ...
    case Type::kPaintRecord: {
      auto picture =
          ToSkPicture(record_, tile_, image_provider_, decoded_image_stash_);
      ...
      switch (scaling_behavior_) {
        ...
        case ScalingBehavior::kFixedScale: {
          auto image = SkImage::MakeFromPicture(
              ...
              rasterization_matrix_.has_value() ? &*rasterization_matrix_
                                                : nullptr,
