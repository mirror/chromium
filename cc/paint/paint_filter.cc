...
std::string PaintFilter::TypeToString(Type type) {
  switch (type) {
    case Type::kSkColorFilter:
      return "kSkColorFilter";
    case Type::kBlur:
      return "kBlur";
...
SkColorFilterPaintFilter::SkColorFilterPaintFilter(
    sk_sp<SkColorFilter> color_filter,
    sk_sp<PaintFilter> input,
    const CropRect* crop_rect)
    : PaintFilter(Type::kSkColorFilter, crop_rect),
...
SkColorFilterPaintFilter::~SkColorFilterPaintFilter() = default;
...
BlurPaintFilter::BlurPaintFilter(SkScalar sigma_x,
                                 ...
                                 sk_sp<PaintFilter> input,
                                 const CropRect* crop_rect)
    : PaintFilter(Type::kBlur, crop_rect),
      sigma_x_(sigma_x),
      sigma_y_(sigma_y),
...
DropShadowPaintFilter::DropShadowPaintFilter(SkScalar dx,
                                             ...
                                             sk_sp<PaintFilter> input,
                                             const CropRect* crop_rect)
    : PaintFilter(Type::kDropShadow, crop_rect),
      dx_(dx),
      dy_(dy),
      sigma_x_(sigma_x),
      sigma_y_(sigma_y),
...
MagnifierPaintFilter::MagnifierPaintFilter(const SkRect& src_rect,
                                           ...
                                           sk_sp<PaintFilter> input,
                                           const CropRect* crop_rect)
    : PaintFilter(Type::kMagnifier, crop_rect),
      src_rect_(src_rect),
      inset_(inset),
      input_(std::move(input)) {
...
AlphaThresholdPaintFilter::AlphaThresholdPaintFilter(const SkRegion& region,
                                                     ...
                                                     sk_sp<PaintFilter> input,
                                                     const CropRect* crop_rect)
    : PaintFilter(Type::kAlphaThreshold, crop_rect),
      region_(region),
...
SkImageFilterPaintFilter::SkImageFilterPaintFilter(
    sk_sp<SkImageFilter> sk_filter)
    : PaintFilter(Type::kSkImageFilter, nullptr),
      sk_filter_(std::move(sk_filter)) {
  cached_sk_filter_ = sk_filter_;
}

SkImageFilterPaintFilter::~SkImageFilterPaintFilter() = default;

XfermodePaintFilter::XfermodePaintFilter(SkBlendMode blend_mode,
                                         sk_sp<PaintFilter> background,
                                         sk_sp<PaintFilter> foreground,
                                         const CropRect* crop_rect)
    : PaintFilter(Type::kXfermode, crop_rect),
...
ArithmeticPaintFilter::ArithmeticPaintFilter(float k1,
                                             ...
                                             sk_sp<PaintFilter> foreground,
                                             const CropRect* crop_rect)
    : PaintFilter(Type::kArithmetic, crop_rect),
      floats_{k1, k2, k3, k4},
...
MatrixConvolutionPaintFilter::MatrixConvolutionPaintFilter(
    ...
    sk_sp<PaintFilter> input,
    const CropRect* crop_rect)
    : PaintFilter(Type::kMatrixConvolution, crop_rect),
...
DisplacementMapEffectPaintFilter::DisplacementMapEffectPaintFilter(
    ...
    const CropRect* crop_rect)
    : PaintFilter(Type::kDisplacementMapEffect, crop_rect),
...
ImagePaintFilter::ImagePaintFilter(PaintImage image,
    ...
    : PaintFilter(Type::kImage, nullptr),
...
RecordPaintFilter::RecordPaintFilter(sk_sp<PaintRecord> record,
    ...
    : PaintFilter(Type::kPaintRecord, nullptr),
...
MergePaintFilter::MergePaintFilter(sk_sp<PaintFilter>* const filters,
                                   int count,
                                   const CropRect* crop_rect)
    : PaintFilter(Type::kMerge, crop_rect) {
...
MorphologyPaintFilter::MorphologyPaintFilter(MorphType morph_type,
                                             ...
                                             sk_sp<PaintFilter> input,
                                             const CropRect* crop_rect)
    : PaintFilter(Type::kMorphology, crop_rect),
...
OffsetPaintFilter::OffsetPaintFilter(SkScalar dx,
                                     ...
                                     sk_sp<PaintFilter> input,
                                     const CropRect* crop_rect)
    : PaintFilter(Type::kOffset, crop_rect),
      dx_(dx),
      dy_(dy),
      input_(std::move(input)) {
  cached_sk_filter_ =
...
TilePaintFilter::TilePaintFilter(const SkRect& src,
                                 const SkRect& dst,
                                 sk_sp<PaintFilter> input)
    : PaintFilter(Type::kTile, nullptr),
      src_(src),
      dst_(dst),
      input_(std::move(input)) {
  cached_sk_filter_ =
...
TurbulencePaintFilter::TurbulencePaintFilter(TurbulenceType turbulence_type,
                                             ...
                                             const CropRect* crop_rect)
    : PaintFilter(Type::kTurbulence, crop_rect),
...
PaintFlagsPaintFilter::PaintFlagsPaintFilter(PaintFlags flags,
                                             const CropRect* crop_rect)
    : PaintFilter(Type::kPaintFlags, crop_rect), flags_(std::move(flags)) {
  cached_sk_filter_ = SkPaintImageFilter::Make(flags.ToSkPaint(), crop_rect);
}
...
LightingDistantPaintFilter::LightingDistantPaintFilter(
    ...
    sk_sp<PaintFilter> input,
    const CropRect* crop_rect)
    : PaintFilter(Type::kLightingDistant, crop_rect),
  ...
  switch (lighting_type_) {
    case LightingType::kDiffuse:
      cached_sk_filter_ = SkLightingImageFilter::MakeDistantLitDiffuse(
          direction_, light_color_, surface_scale_, kd_,
          GetSkFilter(input_.get()), crop_rect);
      break;
    case LightingType::kSpecular:
      cached_sk_filter_ = SkLightingImageFilter::MakeDistantLitSpecular(
          direction_, light_color_, surface_scale_, kd_, shininess_,
          GetSkFilter(input_.get()), crop_rect);
      break;
  }
}
...
LightingPointPaintFilter::LightingPointPaintFilter(LightingType lighting_type,
                                                   ...
                                                   sk_sp<PaintFilter> input,
                                                   const CropRect* crop_rect)
    : PaintFilter(Type::kLightingPoint, crop_rect),
      lighting_type_(lighting_type),
      location_(location),
      light_color_(light_color),
      surface_scale_(surface_scale),
  ...
  switch (lighting_type_) {
    case LightingType::kDiffuse:
      cached_sk_filter_ = SkLightingImageFilter::MakePointLitDiffuse(
          location_, light_color_, surface_scale_, ks_,
          GetSkFilter(input_.get()), crop_rect);
      break;
    case LightingType::kSpecular:
      cached_sk_filter_ = SkLightingImageFilter::MakePointLitSpecular(
          location_, light_color_, surface_scale_, ks_, shininess_,
          GetSkFilter(input_.get()), crop_rect);
      break;
  }
}
...
LightingSpotPaintFilter::LightingSpotPaintFilter(LightingType lighting_type,
                                                 ...
                                                 sk_sp<PaintFilter> input,
                                                 const CropRect* crop_rect)
    : PaintFilter(Type::kLightingSpot, crop_rect),
      lighting_type_(lighting_type),
      location_(location),
      target_(target),
  ...
  switch (lighting_type_) {
    case LightingType::kDiffuse:
      cached_sk_filter_ = SkLightingImageFilter::MakeSpotLitDiffuse(
          ...
          surface_scale_, ks_, GetSkFilter(input_.get()), crop_rect);
      break;
    case LightingType::kSpecular:
      cached_sk_filter_ = SkLightingImageFilter::MakeSpotLitSpecular(
          location_, target_, specular_exponent_, cutoff_angle_, light_color_,
          surface_scale_, ks_, shininess_, GetSkFilter(input_.get()),
          crop_rect);
      break;
  }
}
