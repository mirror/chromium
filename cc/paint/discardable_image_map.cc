...
class DiscardableImageGenerator {
 ...
 private:
  ...
  void GatherDiscardableImages(const PaintOpBuffer* buffer,
    ...
    PlaybackParams params(nullptr, nullptr, canvas->getTotalMatrix());
    ...
    for (auto* op : PaintOpBuffer::Iterator(buffer)) {
      if (op->IsDrawOp()) {
        ...
        if (op->IsPaintOpWithFlags()) {
          AddImageFromFlags(
              op_rect, static_cast<const PaintOpWithFlags*>(op)->flags, ctm);
        }
        ...
        if (op_type == PaintOpType::DrawImage) {
          ...
          SkMatrix matrix = ctm;
          matrix.postConcat(
              SkMatrix::MakeRectToRect(image_rect_op->src, image_rect_op->dst,
