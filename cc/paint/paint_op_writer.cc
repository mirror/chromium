...
void PaintOpWriter::Write(const PaintTypeface* typefaces,
                          uint32_t typefaces_count) {
  WriteSimple(typefaces_count);
  for (uint32_t i = 0; i < typefaces_count; ++i) {
    const PaintTypeface& typeface = typefaces[i];
    if (!typeface) {
      WriteSimple(static_cast<SkFontID>(0));
      continue;
    }
    WriteSimple(typeface.sk_typeface()->uniqueID());
...
#if DCHECK_IS_ON()
    if (typeface)
      last_serialized_typeface_ids_.insert(typeface.sk_typeface()->uniqueID());
#endif
  }
}
