...
class OopPixelTest : public testing::Test {
 public:
  ...
  std::vector<SkColor> Raster(scoped_refptr<DisplayItemList> display_item_list,
    ...
    gl->RasterCHROMIUM(display_item_list.get(), 0, 0, width, height);
...
TEST_F(OopPixelTest, DrawRect) {
  ...
  std::vector<std::pair<SkRect, PaintFlags>> input = {
      []() {
        PaintFlags flags;
        flags.setColor(SkColorSetARGB(255, 0, 0, 255));
        return std::make_pair(SkRect::MakeXYWH(0, 0, 5, 5), flags);
      }(),
      []() {
        PaintFlags flags;
        flags.setColor(SkColorSetARGB(255, 0, 255, 0));
        return std::make_pair(SkRect::MakeXYWH(5, 0, 5, 5), flags);
      }(),
      []() {
        PaintFlags flags;
        flags.setColor(SkColorSetARGB(255, 0, 255, 255));
        return std::make_pair(SkRect::MakeXYWH(0, 5, 5, 5), flags);
      }(),
      []() {
        PaintFlags flags;
        flags.setColor(SkColorSetARGB(255, 255, 0, 0));
        return std::make_pair(SkRect::MakeXYWH(5, 5, 5, 5), flags);
      }()};
