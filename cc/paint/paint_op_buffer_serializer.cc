...
void PaintOpBufferSerializer::SerializePreamble(
  ...
  DCHECK(gfx::Rect(preamble.content_size).Contains(preamble.full_raster_rect))
      << "content: " << preamble.content_size.ToString()
      << ", full: " << preamble.full_raster_rect.ToString();
  ...
  if (preamble.requires_clear && !is_partial_raster) {
    ...
    DCHECK_EQ(SK_ColorTRANSPARENT, preamble.background_color);
