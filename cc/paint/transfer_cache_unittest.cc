...
class TransferCacheTest : public testing::Test {
 public:
  ...
  void CreateEntry(const ClientTransferCacheEntry& entry) {
    ...
    context_support->UnmapAndCreateTransferCacheEntry(
        static_cast<uint32_t>(entry.Type()), entry.Id());
  }
...
TEST_F(TransferCacheTest, Basic) {
  ...
  uint32_t entry_type = static_cast<uint32_t>(entry.Type());
  // Unlock on client side and flush to service.
  context_support->UnlockTransferCacheEntries({{entry_type, entry.Id()}});
  gl->Finish();

  // Re-lock on client side and validate state. No need to flush as lock is
  // local.
  EXPECT_TRUE(context_support->ThreadsafeLockTransferCacheEntry(entry_type,
                                                                entry.Id()));

  // Delete on client side, flush, and validate that deletion reaches service.
  context_support->DeleteTransferCacheEntry(entry_type, entry.Id());
  gl->Finish();
...
TEST_F(TransferCacheTest, Eviction) {
  ...
  EXPECT_FALSE(context_support->ThreadsafeLockTransferCacheEntry(entry_type,
                                                                 entry.Id()));
}
