...
class PaintOpReader {
 public:
  PaintOpReader(const void* memory, size_t size)
      : memory_(static_cast<const char*>(memory) + PaintOpWriter::kHeaderBytes),
        remaining_bytes_(size - PaintOpWriter::kHeaderBytes) {
    DCHECK_GE(size, PaintOpWriter::kHeaderBytes);
  }
