...
NOINLINE DISABLE_CFI_PERF void RasterItem(const DisplayItem& base_item,
  ...
  switch (base_item.type) {
    ...
    case DisplayItem::DRAWING: {
      ...
      if (canvas->quickReject(item.cull_rect))
        break;
    ...
    for (const DisplayItem& base_item : items_) {
      ...
      switch (base_item.type) {
        ...
        case DisplayItem::DRAWING: {
          ...
          state->AppendInteger(item.cull_rect.x());
          state->AppendInteger(item.cull_rect.y());
          state->AppendInteger(item.cull_rect.width());
          state->AppendInteger(item.cull_rect.height());
          state->EndArray();

          std::string b64_picture;
          PictureDebugUtil::SerializeAsBase64(
              ToSkPicture(item.picture, item.cull_rect).get(), &b64_picture);
