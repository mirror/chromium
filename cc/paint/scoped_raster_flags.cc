...
ScopedRasterFlags::ScopedRasterFlags(const PaintFlags& flags,
  ...
  if (alpha != 255) {
    ...
    WritableFlags()->setAlpha(SkMulDiv255Round(flags.getAlpha(), alpha));
  }
...
void ScopedRasterFlags::DecodeImageShader(const SkMatrix& ctm) {
  ...
  DrawImage draw_image(paint_image, int_src_rect,
  ...
  PaintImage decoded_paint_image = PaintImageBuilder::WithDefault()
                                       ...
                                       .TakePaintImage();
  WritableFlags()->setFilterQuality(decoded_image.filter_quality());
  WritableFlags()->setShader(PaintShader::MakeImage(
...
void ScopedRasterFlags::DecodeRecordShader(const SkMatrix& ctm) {
  ...
  WritableFlags()->setShader(std::move(decoded_shader));
}

namespace {

bool CanSubstituteHairlineForStroke(const PaintFlags& flags) {
  return flags.getStrokeCap() == PaintFlags::kDefault_Cap &&
...
void ScopedRasterFlags::AdjustStrokeIfNeeded(const SkMatrix& ctm) {
  ...
  SkSize scale;
  if (original_flags_.isAntiAlias() ||  // safe to raster at any scale
      original_flags_.getStyle() == PaintFlags::kFill_Style ||  // not a stroke
      !original_flags_.getStrokeWidth() ||  // explicit hairline
  ...
  const auto stroke_vec =
      SkVector::Make(original_flags_.getStrokeWidth() * scale.width(),
                     original_flags_.getStrokeWidth() * scale.height());
  if (stroke_vec.x() >= 1 && stroke_vec.y() >= 1)
    return;  // safe to raster

  if (stroke_vec.x() < 1 && stroke_vec.y() < 1 &&
      CanSubstituteHairlineForStroke(original_flags_)) {
    // Use modulated hairline when possible, as it is faster and produces
    // results closer to the original intent.
    WritableFlags()->setStrokeWidth(0);
    WritableFlags()->setAlpha(roundf(original_flags_.getAlpha() *
                                     sqrtf(stroke_vec.x() * stroke_vec.y())));
    return;
  }

  // Fall back to anti-aliasing.
  WritableFlags()->setAntiAlias(true);
}
