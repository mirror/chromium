...
ScopedRasterFlags::ScopedRasterFlags(const PaintFlags& flags,
  ...
  if (alpha != 255) {
    ...
    RWFlags()->setAlpha(SkMulDiv255Round(flags.getAlpha(), alpha));
  }
...
void ScopedRasterFlags::DecodeImageShader(const SkMatrix& ctm) {
  const PaintImage& paint_image = ROFlags().getShader()->paint_image();
  SkMatrix matrix = ROFlags().getShader()->GetLocalMatrix();
  ...
  DrawImage draw_image(paint_image, int_src_rect, ROFlags().getFilterQuality(),
                       total_image_matrix);
  ...
  PaintImage decoded_paint_image = PaintImageBuilder::WithDefault()
                                       ...
                                       .TakePaintImage();
  RWFlags()->setFilterQuality(decoded_image.filter_quality());
  RWFlags()->setShader(
      PaintShader::MakeImage(decoded_paint_image, ROFlags().getShader()->tx(),
                             ROFlags().getShader()->ty(), &matrix));
}

void ScopedRasterFlags::DecodeRecordShader(const SkMatrix& ctm) {
  auto decoded_shader = ROFlags().getShader()->CreateDecodedPaintRecord(
  ...
  RWFlags()->setShader(std::move(decoded_shader));
}
...
void ScopedRasterFlags::AdjustStrokeIfNeeded(const SkMatrix& ctm) {
  ...
  SkSize scale;
  if (ROFlags().isAntiAlias() ||  // safe to raster at any scale
      ROFlags().getStyle() == PaintFlags::kFill_Style ||  // not a stroke
      !ROFlags().getStrokeWidth() ||                      // explicit hairline
  ...
  const auto stroke_vec =
      SkVector::Make(ROFlags().getStrokeWidth() * scale.width(),
                     ROFlags().getStrokeWidth() * scale.height());
  if (stroke_vec.x() >= 1 && stroke_vec.y() >= 1)
    return;  // safe to raster

  const auto can_substitute_hairline =
      ROFlags().getStrokeCap() == PaintFlags::kDefault_Cap &&
      ROFlags().getStrokeJoin() == PaintFlags::kDefault_Join;
  if (can_substitute_hairline && stroke_vec.x() < 1 && stroke_vec.y() < 1) {
    // Use modulated hairline when possible, as it is faster and produces
    // results closer to the original intent.
    RWFlags()->setStrokeWidth(0);
    RWFlags()->setAlpha(
        roundf(ROFlags().getAlpha() * sqrtf(stroke_vec.x() * stroke_vec.y())));
    return;
  }

  // Fall back to anti-aliasing.
  RWFlags()->setAntiAlias(true);
}
