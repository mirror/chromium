...
TEST_F(DiscardableImageMapTest, GathersAnimatedImages) {
  ...
  PaintImage animation_loop_infinite = CreateAnimatedImage(image_size, frames);
  ...
  const auto& animated_images =
      display_list->discardable_image_map().animated_images();

  EXPECT_EQ(animated_images.size(), 1u);
  EXPECT_EQ(animated_images[0].paint_image_id,
            animation_loop_infinite.stable_id());
  EXPECT_EQ(animated_images[0].completion_state,
            animation_loop_infinite.completion_state());
  EXPECT_EQ(animated_images[0].frames,
            animation_loop_infinite.GetFrameMetadata());
  EXPECT_EQ(animated_images[0].repetition_count,
