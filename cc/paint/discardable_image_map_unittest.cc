...
TEST_F(DiscardableImageMapTest, GetDiscardableImagesInRectNonZeroLayer) {
  ...
  // Image not present in the list.
  {
    ...
    EXPECT_TRUE(image_map.GetRegionForImage(image.stable_id()).empty());
  }
}
...
TEST_F(DiscardableImageMapTest, TracksImageRegions) {
  ...
  SimpleRegion expected_region;
  for (auto& rect : rects) {
    rect.Inset(-1, -1);
    expected_region.Union(rect);
  }

  EXPECT_TRUE(image_map.GetRegionForImage(image.stable_id())
                  .EqualsForTesting(expected_region));
}
