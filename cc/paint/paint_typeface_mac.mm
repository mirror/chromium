...
PaintTypeface PaintTypeface::FromNSFont(NSFont* ns_font,
  ...
  typeface.CreateSkTypefaceFromNSFont(ns_font, &loader);
  return typeface;
}

// static
PaintTypeface PaintTypeface::FromNSFontParams(
  ...
  typeface.requested_size_ = requested_size;
  typeface.axes_ = std::move(axes);
  typeface.CreateSkTypefaceFromNSFont(
      [NSFont fontWithName:base::SysUTF16ToNSString(font_name) size:font_size],
      &loader);
  return typeface;
}

void PaintTypeface::CreateSkTypefaceFromNSFont(
    NSFont* ns_font,
    const PaintFontLoaderMac* loader) {
  DCHECK(type_ == Type::kNSFont);
  DCHECK(loader);
  DCHECK(ns_font);
