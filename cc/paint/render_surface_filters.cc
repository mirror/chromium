  ...
  for (size_t i = 0; i < filters.size(); ++i) {
    ...
    switch (op.type()) {
      ...
      case FilterOperation::REFERENCE: {
        ...
        if (op.image_filter()->type() == PaintFilter::Type::kSkColorFilter) {
          const auto* color_paint_filter =
              static_cast<SkColorFilterPaintFilter*>(op.image_filter().get());
          ...
          const SkImageFilter* sk_filter =
              static_cast<SkImageFilterPaintFilter*>(op.image_filter().get())
                  ->sk_filter();
          sk_filter->asColorFilter(&colorfilter_rawptr);
          cf.reset(colorfilter_rawptr);
          has_input = cf && sk_filter->getInput(0);
        }
