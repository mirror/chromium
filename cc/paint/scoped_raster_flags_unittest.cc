...
TEST(ScopedRasterFlagsTest, ThinAliasedStroke) {
  ...
  for (const auto& tst : tests) {
    ScopedRasterFlags scoped_flags(flags, nullptr, tst.ctm, tst.alpha);
    ASSERT_TRUE(scoped_flags.flags());

    EXPECT_EQ(scoped_flags.flags() == &flags, tst.expect_same_flags);
    EXPECT_EQ(scoped_flags.flags()->isAntiAlias(), tst.expect_aa);
    EXPECT_EQ(scoped_flags.flags()->getStrokeWidth(), tst.expect_stroke_width);
    EXPECT_EQ(scoped_flags.flags()->getAlpha(), tst.expect_alpha);
  }
}
