  ...
  class Iterator {
   public:
    ...
    Iterator& operator++() {
      ...
      // Since the jump calculation isn't trivial, only do it if it's likely to
      // be worth the savings (only if the distance from the current index to
      // target index is at least this threshold).
      static const size_t kJumpThreshold = 5;
      if (indices_ && (target_idx_ - op_idx_) >= kJumpThreshold) {
        size_t new_checkpoint_idx =
            buffer_->element_idx_to_checkpoint_idx(target_idx_);
        if (new_checkpoint_idx != checkpoint_idx_) {
          checkpoint_idx_ = new_checkpoint_idx;
          op_idx_ = buffer_->checkpoint_idx_to_element_idx(checkpoint_idx_);
          size_t new_offset =
              buffer_->offset_of_checkpoint_idx(checkpoint_idx_);
          DCHECK_LE(new_offset, current_offset_);
