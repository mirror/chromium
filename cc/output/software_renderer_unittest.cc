...
TEST_F(SoftwareRendererTest, SolidColorQuad) {
  ...
  uint64_t stable_id = root_render_pass->shared_quad_state_list.size();
  shared_quad_state->SetAll(stable_id, gfx::Transform(), outer_rect, outer_rect,
...
TEST_F(SoftwareRendererTest, TileQuadVisibleRect) {
  ...
  SharedQuadState* shared_quad_state =
      root_render_pass->CreateAndAppendSharedQuadState();
  uint64_t stable_id = root_render_pass->shared_quad_state_list.size();
  shared_quad_state->SetAll(stable_id, gfx::Transform(), tile_rect, tile_rect,
                            tile_rect, false, 1.0, SkBlendMode::kSrcOver, 0);
  TileDrawQuad* quad =
      root_render_pass->CreateAndAppendDrawQuad<TileDrawQuad>();
