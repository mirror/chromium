...
void SoftwareRenderer::CopyDrawnRenderPass(
  ...
  switch (request->result_format()) {
    ...
    case viz::CopyOutputRequest::ResultFormat::I420_PLANES:
      // TODO(miu): Support I420_PLANES also, for tab capture.
      // http://crbug.com/754872
      return;  // |request| auto-sends empty result on out-of-scope.
  }

  if (request->is_scaled()) {
    // TODO(miu): Coming soon! http://crbug.com/754872
    NOTIMPLEMENTED();
    return;  // |request| auto-sends empty result on out-of-scope.
  }
  ...
  request->SendResult(
      base::MakeUnique<viz::CopyOutputSkBitmapResult>(copy_rect, bitmap));
}
