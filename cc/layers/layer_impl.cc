...
void LayerImpl::PushPropertiesTo(LayerImpl* layer) {
  ...
  layer->wheel_event_listener_region_ = wheel_event_listener_region_;
...
std::unique_ptr<base::DictionaryValue> LayerImpl::LayerAsJson() {
  ...
  if (!wheel_event_listener_region_.IsEmpty()) {
    std::unique_ptr<base::Value> region =
        wheel_event_listener_region_.AsValue();
...
void LayerImpl::AsValueInto(base::trace_event::TracedValue* state) const {
  ...
  if (!wheel_event_listener_region_.IsEmpty()) {
    wheel_event_listener_region_.AsValueInto(state);
  }
