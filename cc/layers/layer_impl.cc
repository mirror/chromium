...
void LayerImpl::PushPropertiesTo(LayerImpl* layer) {
  ...
  layer->touch_event_handler_ = touch_event_handler_;
...
std::unique_ptr<base::DictionaryValue> LayerImpl::LayerTreeAsJson() {
  ...
  if (!touch_event_handler_.region().IsEmpty()) {
    std::unique_ptr<base::Value> region =
        touch_event_handler_.region().AsValue();
...
void LayerImpl::AsValueInto(base::trace_event::TracedValue* state) const {
  ...
  if (!touch_event_handler_.region().IsEmpty()) {
    ...
    touch_event_handler_.region().AsValueInto(state);
    state->EndArray();
  }
