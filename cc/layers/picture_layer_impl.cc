...
void PictureLayerImpl::AppendQuads(viz::RenderPass* render_pass,
  ...
  if (shared_quad_state->quad_to_target_transform.IsScaleOrTranslation()) {
    ...
    const auto& max_tile_size = tilings_->GetMaxTileSizeInLayerSpace();

    // The offset has to be negative for us to do any offsetting.
    quad_offset.set_x(std::min(0, max_tile_size.width() - visible_rect.x()));
    quad_offset.set_y(std::min(0, max_tile_size.height() - visible_rect.y()));
