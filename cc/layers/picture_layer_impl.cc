...
PictureLayerImpl::~PictureLayerImpl() {
  ...
  if (controller && raster_source_ && raster_source_->display_list()) {
    for (const auto& data : raster_source_->display_list()
...
void PictureLayerImpl::UpdateRasterSource(
  ...
  // Register images from the new raster source.
  if (controller && raster_source_->display_list()) {
    for (const auto& data : raster_source_->display_list()
                                ->discardable_image_map()
                                .animated_images()) {
      // Only update state from the sync tree, since that has the most recent
      // recording.
...
void PictureLayerImpl::InvalidateRegionForImages(
  ...
  gfx::Rect image_update_rect;
  for (auto image_id : images_to_invalidate) {
    gfx::Rect image_rect = raster_source_->GetRectForImage(image_id);
    image_invalidation.Union(image_rect);
    image_update_rect.Union(image_rect);
  }
  Region invalidation;
  ...
  image_update_rect.Union(update_rect());
  SetUpdateRect(image_update_rect);
