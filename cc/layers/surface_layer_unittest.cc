...
// This test verifies the list of surface layer surface ids when there are
// multiple surface layers with the same surface id. This emulates how maximize
// and minimize animations work on Chrome OS.
...
// This test verifies LayerTreeImpl::needs_surface_ids_sync() works when there
// are multiple surface layers with the same surface id .
TEST_F(SurfaceLayerTest, CheckNeedsSurfaceIdsSyncForClonedLayers) {
  ...
  // Verify the surface is in SurfaceLayerIds() and needs_surface_ids_sync() is
  // true.
  EXPECT_TRUE(layer_tree_host_->needs_surface_ids_sync());
  ...
  // Destroy one of the layers, leaving one layer for the surface id.
  layer1->SetLayerTreeHost(nullptr);
  ...
  // Verify SurfaceLayerIds() changed and needs_surface_ids_sync() is true.
  EXPECT_TRUE(layer_tree_host_->needs_surface_ids_sync());
