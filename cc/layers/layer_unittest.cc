...
TEST_F(LayerTest, PushDrawsContentShouldUpdateShouldHitTest) {
  ...
  EXPECT_FALSE(impl_layer->should_hit_test_non_drawable());
  ...
  // |SetShouldHitTestNonDrawable| should cause a layer to become hit testable
  // even though it does not draw content.
  root_layer->SetShouldHitTestNonDrawable(true);
  root_layer->PushPropertiesTo(impl_layer.get());
  EXPECT_FALSE(impl_layer->DrawsContent());
  EXPECT_TRUE(impl_layer->should_hit_test_non_drawable());
  EXPECT_TRUE(impl_layer->should_hit_test());
}
