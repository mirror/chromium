...
void Viewport::PinchEnd(const gfx::Point& anchor) {
  LayerTreeImpl* active_tree = host_impl_->active_tree();
  ...
  if (page_scale < min_scale * kMaxZoomForSnapToMin) {
    ...
    adjusted_anchor +=
        ScrollOffsetToVector2dF(active_tree->TotalScrollOffset());
    host_impl_->StartPageScaleAnimation(
        ToRoundedVector2d(adjusted_anchor.OffsetFromOrigin()), true, min_scale,
