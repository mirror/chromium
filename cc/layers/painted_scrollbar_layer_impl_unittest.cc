...
TEST(PaintedScrollbarLayerImplTest, Occlusion) {
...

  {
    ...
    EXPECT_EQ(track_quad->opaque_rect.ToString(), gfx::Rect().ToString());
    EXPECT_FALSE(track_quad->shared_quad_state->is_opaque);
    ...
    EXPECT_FALSE(thumb_quad->shared_quad_state->is_opaque);
