...
gfx::RectF RenderSurfaceImpl::DrawableContentRect() const {
  ...
  const gfx::FilterOperations& filters = Filters();
  if (!filters.IsEmpty()) {
    surface_content_rect =
        filters.MapRect(surface_content_rect, SurfaceScale().matrix());
  }
  gfx::RectF drawable_content_rect = gfx::MathUtil::MapClippedRect(
...
const gfx::FilterOperations& RenderSurfaceImpl::Filters() const {
...
const gfx::FilterOperations& RenderSurfaceImpl::BackgroundFilters() const {
...
gfx::Rect RenderSurfaceImpl::CalculateExpandedClipForFilters(
  ...
  gfx::Rect clip_in_surface_space = gfx::MathUtil::ProjectEnclosingClippedRect(
