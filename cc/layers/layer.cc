...
Layer::Inputs::Inputs(int layer_id)
    : layer_id(layer_id),
      ...
      should_hit_test_non_drawable(false),
...
void Layer::SetShouldHitTestNonDrawable(bool should_hit_test) {
  DCHECK(IsPropertyChangeAllowed());
  if (inputs_.should_hit_test_non_drawable == should_hit_test)
    return;
  inputs_.should_hit_test_non_drawable = should_hit_test;
  SetPropertyTreesNeedRebuild();
  SetNeedsCommit();
}
...
void Layer::PushPropertiesTo(LayerImpl* layer) {
  ...
  layer->SetShouldHitTestNonDrawable(should_hit_test_non_drawable());
