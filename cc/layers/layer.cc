...
Region Layer::UnionOfRegions(const RegionMap& region_map) const {
  Region region;
...
void Layer::SetTouchEventHandlerRegion(
  ...
  DCHECK(IsPropertyChangeAllowed());
  if (inputs_.touch_event_handler_region == touch_event_handler_region_map)
    return;
  inputs_.touch_event_handler_region = touch_event_handler_region_map;
  SetPropertyTreesNeedRebuild();
  SetNeedsCommit();
}
...
void Layer::PushPropertiesTo(LayerImpl* layer) {
  ...
  layer->SetTouchEventHandlerRegion(
      UnionOfRegions(inputs_.touch_event_handler_region));
