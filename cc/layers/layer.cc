...
void Layer::SetWheelEventListenerRegion(
    const Region& wheel_event_listener_region) {
  DCHECK(IsPropertyChangeAllowed());
  if (inputs_.wheel_event_listener_region == wheel_event_listener_region)
    return;

  inputs_.wheel_event_listener_region = wheel_event_listener_region;
  SetPropertyTreesNeedRebuild();
  SetNeedsCommit();
}
...
void Layer::PushPropertiesTo(LayerImpl* layer) {
  ...
  layer->SetWheelEventListenerRegion(inputs_.wheel_event_listener_region);
