...
VideoFrameExternalResources VideoResourceUpdater::CreateForSoftwarePlanes(
  ...
  for (size_t i = 0; i < plane_resources.size(); ++i) {
    ...
    const int resource_bit_depth = viz::BitsPerPixel(plane_resource_format);
    ...
    // or if the bit depths or strides are incompatible.
    const bool needs_conversion =
        plane_resource_format == viz::LUMINANCE_F16 ||
        (resource_bit_depth == 8 && bits_per_channel > 8) ||
    ...
    if (!needs_conversion) {
      ...
      if (plane_resource_format == viz::LUMINANCE_F16) {
        ...
        // Make a copy because input and output are the same size and format,
        // but differ in stride.
        DCHECK(is_stride_different);
        DCHECK(plane_resource_format == viz::LUMINANCE_8 ||
               plane_resource_format == viz::RED_8);
