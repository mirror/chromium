...
VideoFrameExternalResources VideoResourceUpdater::CreateForSoftwarePlanes(
  ...
  for (size_t i = 0; i < plane_resources.size(); ++i) {
    ...
    DCHECK(plane_resource_format == viz::LUMINANCE_F16 ||
           plane_resource_format == viz::R16_EXT ||
           plane_resource_format == viz::LUMINANCE_8 ||
    ...
    const bool is_stride_different =
        static_cast<int>(upload_image_stride) == video_stride_bytes;

    const int bits_per_pixel = viz::BitsPerPixel(plane_resource_format);

    const bool needs_conversion =
        plane_resource_format == viz::LUMINANCE_F16 ||
        (bits_per_pixel == 8 && bits_per_channel > 8) ||
        (bits_per_channel == 8 && is_stride_different);
    ...
    if (!needs_conversion) {
      ...
      } else if (static_cast<int>(upload_image_stride) == video_stride_bytes) {
