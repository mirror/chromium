...
T ResourceUtil::CheckedWidthInBytes(int width, viz::ResourceFormat format) {
  ...
  checked_value *= width;
  checked_value =
      gfx::MathUtil::CheckedRoundUp<T>(checked_value.ValueOrDie(), 8);
  checked_value /= 8;
...
bool ResourceUtil::VerifyFitsInBytesInternal(int width,
  ...
  checked_value =
      gfx::MathUtil::UncheckedRoundUp<T>(checked_value.ValueOrDie(), 8);
  ...
  if (aligned) {
    ...
    if (!checked_value.IsValid())
      return false;
    checked_value =
        gfx::MathUtil::UncheckedRoundUp<T>(checked_value.ValueOrDie(), 4);
    checked_value *= 8;
  }
...
T ResourceUtil::BytesInternal(int width,
  ...
  bytes *= width;
  bytes = gfx::MathUtil::UncheckedRoundUp<T>(bytes, 8);
  bytes /= 8;
  if (aligned)
    bytes = gfx::MathUtil::UncheckedRoundUp<T>(bytes, 4);
  if (verify_size)
    bytes *= height;
