...
ResourcePool::InUsePoolResource ResourcePool::AcquireResource(
    ...
    viz::ResourceFormat format,
    const gfx::ColorSpace& color_space) {
  PoolResource* reused_resource = ReuseResource(size, format, color_space);
  if (reused_resource)
    return InUsePoolResource(reused_resource,
                             use_gpu_resources_ || use_gpu_memory_buffers_);

  return InUsePoolResource(CreateResource(size, format, color_space),
                           use_gpu_resources_ || use_gpu_memory_buffers_);
}
...
void ResourcePool::CheckBusyResources() {
  for (size_t i = 0; i < busy_resources_.size();) {
    auto it = busy_resources_.begin() + i;
    PoolResource* resource = it->get();
...
ResourcePool::PoolResource::~PoolResource() {
  DCHECK(!resource_id_);
}
