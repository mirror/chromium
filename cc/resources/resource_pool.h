...
class CC_EXPORT ResourcePool : public base::trace_event::MemoryDumpProvider,
 ...
 public:
  ...
  // with a resource only after it has a backing allocated for it. Initialially
  // an acquired InUsePoolResource will be only metadata, and the backing is
  // given to it by code which is aware of the expected backing type - currently
  // by RasterBufferProvider::AcquireBufferForRaster().
