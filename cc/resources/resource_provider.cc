...
void ResourceProvider::DeleteResourceInternal(ResourceMap::iterator it,
  ...
  if (!lost_resource && resource->origin == Resource::INTERNAL &&
    ...
    GLES2Interface* gl = ContextGL();
    DCHECK(gl);
    gl->WaitSyncTokenCHROMIUM(resource->GetSyncTokenData());
...
void ResourceProvider::WaitSyncToken(viz::ResourceId id) {
  ...
  if (resource->synchronization_state() == Resource::NEEDS_WAIT) {
    ContextGL()->WaitSyncTokenCHROMIUM(resource->GetSyncTokenData());
    resource->SetSynchronized();
  }
}
