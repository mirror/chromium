...
GLuint ResourceProvider::ScopedWriteLockGL::ConsumeTexture(
  ...
  GLuint texture_id = rs->CreateAndConsumeTextureCHROMIUM(mailbox_.name);
  DCHECK(texture_id);
...
void ResourceProvider::CreateMailbox(viz::internal::Resource* resource) {
  ...
  rs->ProduceTextureDirectCHROMIUM(resource->gl_id, resource->mailbox.name);
  resource->SetLocallyUsed();
}
...
gpu::gles2::GLES2Interface* ResourceProvider::ContextGL() const {
