...
void ScopedLocalResource::CreateTexture() {
  ...
  gl_->BindTexture(target_, gl_id_);
  gl_->TexParameteri(target_, GL_TEXTURE_MIN_FILTER, original_filter_);
  gl_->TexParameteri(target_, GL_TEXTURE_MAG_FILTER, original_filter_);
  gl_->TexParameteri(target_, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
  gl_->TexParameteri(target_, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
  if (settings_->use_texture_usage_hint &&
      (hint_ & viz::ResourceTextureHint::kFramebuffer)) {
    gl_->TexParameteri(target_, GL_TEXTURE_USAGE_ANGLE,
...
void ScopedLocalResource::Allocate(const gfx::Size& size,
  ...
  if (settings_->use_texture_storage) {
    ...
    gl_->TexStorage2DEXT(target_, levels, viz::TextureStorageFormat(format_),
                         size_.width(), size_.height());
  } else {
    gl_->TexImage2D(target_, 0, GLInternalFormat(format_), size_.width(),
...
GLenum ScopedLocalResource::BindForSampling(GLenum unit, GLenum filter) {
  ...
  if (min_filter != min_filter_) {
    gl_->TexParameteri(target_, GL_TEXTURE_MIN_FILTER, min_filter);
    min_filter_ = min_filter;
  }
  if (filter != filter_) {
    gl_->TexParameteri(target_, GL_TEXTURE_MAG_FILTER, filter);
    filter_ = filter;
  }
