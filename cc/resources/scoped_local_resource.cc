...
void ScopedLocalResource::CreateTexture() {
  ...
  gl_->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, original_filter_);
  gl_->TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, original_filter_);
...
void ScopedLocalResource::Allocate(const gfx::Size& size,
  ...
  if (settings_->use_texture_storage) {
    ...
    gl_->TexStorage2DEXT(GL_TEXTURE_2D, levels,
                         viz::TextureStorageFormat(format_), size_.width(),
                         size_.height());
  } else {
    gl_->TexImage2D(GL_TEXTURE_2D, 0, GLInternalFormat(format_), size_.width(),
