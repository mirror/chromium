...
StructTraits<cc::mojom::CopyOutputResultDataView,
  ...
  if (has_texture_result(result.get())) {
    MakeStrongBinding(base::MakeUnique<TextureMailboxReleaserImpl>(
...
bool StructTraits<cc::mojom::CopyOutputResultDataView,
  ...
  if (!data.ReadFormat(&format) || !data.ReadRect(&rect) ||
      !data.ReadBitmap(&bitmap) || !data.ReadTextureMailbox(&texture_mailbox)) {
    return false;
  }

  switch (format) {
    case viz::CopyOutputResult::Format::RGBA_BITMAP:
      if (!bitmap.readyToDraw() && !rect.IsEmpty())
