...
class CCParamTraitsTest : public testing::Test {
 protected:
  void Compare(const viz::RenderPass* a, const viz::RenderPass* b) {
    EXPECT_EQ(a->id, b->id);
    ...
    switch (a->material) {
      ...
      case viz::DrawQuad::RENDER_PASS:
        Compare(RenderPassDrawQuad::MaterialCast(a),
                viz::RenderPassDrawQuad::MaterialCast(b));
        break;
  ...
  void Compare(const viz::RenderPassDrawQuad* a,
               const viz::RenderPassDrawQuad* b) {
...
TEST_F(CCParamTraitsTest, AllQuads) {
  ...
  std::unique_ptr<RenderPass> child_pass_in = viz::RenderPass::Create();
  ...
  std::unique_ptr<RenderPass> child_pass_cmp = viz::RenderPass::Create();
  child_pass_cmp->SetAll(
      child_id, arbitrary_rect2, arbitrary_rect3, arbitrary_matrix2,
      arbitrary_filters1, arbitrary_filters2, arbitrary_color_space,
      arbitrary_bool2, arbitrary_bool3, arbitrary_bool4, arbitrary_bool5);

  std::unique_ptr<RenderPass> pass_in = viz::RenderPass::Create();
  ...
  std::unique_ptr<RenderPass> pass_cmp = viz::RenderPass::Create();
  ...
  *shared_state2_cmp = *shared_state2_in;
  ...
  viz::RenderPassDrawQuad* renderpass_in =
...
TEST_F(CCParamTraitsTest, Resources) {
  ...
  std::unique_ptr<RenderPass> renderpass_in = viz::RenderPass::Create();
