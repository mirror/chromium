  ...
  static cc::mojom::CopyOutputResultFormat ToMojom(
    ...
    switch (format) {
      ...
      case viz::CopyOutputResult::Format::I420_PLANES:
        // Note: I420 is converted to SkBitmap form (via AsSkBitmap()) when
        // passing results through Mojo.
        return cc::mojom::CopyOutputResultFormat::RGBA_BITMAP;
    }
    NOTREACHED();
    return cc::mojom::CopyOutputResultFormat::RGBA_BITMAP;
...
struct StructTraits<cc::mojom::CopyOutputResultDataView,
  ...
  static base::Optional<viz::TextureMailbox> texture_mailbox(
      const std::unique_ptr<viz::CopyOutputResult>& result) {
    if (has_texture_result(result.get()))
 ...
 private:
  static bool has_texture_result(const viz::CopyOutputResult* result) {
    return result && result->GetTextureMailbox() &&
           result->GetTextureMailbox()->IsTexture();
  }
};
