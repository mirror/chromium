...
void ParamTraits<cc::FilterOperation>::GetSize(base::PickleSizer* s,
  ...
  switch (p.type()) {
    ...
    case cc::FilterOperation::BLUR:
      GetParamSize(s, p.amount());
      GetParamSize(s, static_cast<int>(p.blur_tile_mode()));
      break;
...
void ParamTraits<cc::FilterOperation>::Write(base::Pickle* m,
  ...
  switch (p.type()) {
    ...
    case cc::FilterOperation::BLUR:
      WriteParam(m, p.amount());
      WriteParam(m, static_cast<int>(p.blur_tile_mode()));
      break;
    case cc::FilterOperation::DROP_SHADOW:
  ...
  switch (type) {
    ...
    case cc::FilterOperation::BLUR:
      if (ReadParam(m, iter, &amount) && ReadParam(m, iter, &tile_mode)) {
        r->set_amount(amount);
        r->set_blur_tile_mode(
            static_cast<SkBlurImageFilter::TileMode>(tile_mode));
        success = true;
      }
      break;
    case cc::FilterOperation::DROP_SHADOW:
