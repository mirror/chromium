...
void ParamTraits<cc::FilterOperation>::Write(base::Pickle* m,
  ...
  switch (p.type()) {
    ...
    case cc::FilterOperation::REFERENCE:
      WriteParam(m, p.image_filter()->cached_sk_filter_);
      break;
...
bool ParamTraits<cc::FilterOperation>::Read(const base::Pickle* m,
  ...
  switch (type) {
    ...
    case cc::FilterOperation::REFERENCE: {
      ...
      auto* paint_filter =
          new cc::SkImageFilterPaintFilter(std::move(sk_filter));
      r->set_image_filter(sk_sp<cc::SkImageFilterPaintFilter>(paint_filter));
      success = true;
      break;
    }
    case cc::FilterOperation::ALPHA_THRESHOLD:
...
void ParamTraits<cc::FilterOperation>::Log(const param_type& p,
  ...
  switch (p.type()) {
    ...
    case cc::FilterOperation::ZOOM:
      ...
      break;
    case cc::FilterOperation::REFERENCE:
      LogParam(p.image_filter()->cached_sk_filter_, l);
      break;
    case cc::FilterOperation::ALPHA_THRESHOLD:
      LogParam(p.amount(), l);
      l->append(", ");
