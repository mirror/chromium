  ...
  ~CopyOutputResultSenderImpl() override {
    if (result_callback_) {
      std::move(result_callback_)
          .Run(base::MakeUnique<viz::CopyOutputResult>(result_format_,
                                                       gfx::Rect()));
    }
  }
...
bool StructTraits<cc::mojom::CopyOutputRequestDataView,
  ...
  auto request = base::MakeUnique<viz::CopyOutputRequest>(
      result_format, base::BindOnce(SendResult, base::Passed(&result_sender)));

  if (data.scale_ratio_numerator() <= 0 ||
      data.scale_ratio_denominator() <= 0) {
    return false;
  }
  request->SetScaleRatio(data.scale_ratio_numerator(),
                         data.scale_ratio_denominator());
