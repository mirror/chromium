...
TEST_F(StructTraitsTest, CopyOutputRequest_BitmapRequest) {
  ...
  const gfx::Rect result_rect(area.width() / 2, area.height() / 2);

  base::RunLoop run_loop;
  std::unique_ptr<viz::CopyOutputRequest> input(new viz::CopyOutputRequest(
      result_format, base::BindOnce(CopyOutputRequestCallback,
                                    run_loop.QuitClosure(), result_rect)));
  input->SetScaleRatio(area.width(), result_rect.width());  // Ratio is 2:1.
  input->set_area(area);
  ...
  EXPECT_EQ(output->scale_ratio(), std::make_tuple(2, 1));
  ...
  output->SendResult(
      base::MakeUnique<viz::CopyOutputSkBitmapResult>(result_rect, bitmap));
...
TEST_F(StructTraitsTest, CopyOutputRequest_TextureRequest) {
  ...
  output->SendResult(base::MakeUnique<viz::CopyOutputTextureResult>(
...
TEST_F(StructTraitsTest, CopyOutputRequest_CallbackRunsOnce) {
  ...
  for (int i = 0; i < 10; i++)
    result_sender->SendResult(base::MakeUnique<viz::CopyOutputResult>(
...
TEST_F(StructTraitsTest, CopyOutputResult_Empty) {
  auto input = base::MakeUnique<viz::CopyOutputResult>(
...
TEST_F(StructTraitsTest, CopyOutputResult_Texture) {
  ...
  mailbox.SetName(mailbox_name);
  viz::TextureMailbox texture_mailbox(mailbox, gpu::SyncToken(), target);
  auto input = base::MakeUnique<viz::CopyOutputTextureResult>(
