...
struct StructTraits<cc::mojom::FilterOperationDataView, cc::FilterOperation> {
  ...
  static ConstCArray<gfx::Rect> region(const cc::FilterOperation& operation) {
    if (operation.type() != cc::FilterOperation::ALPHA_THRESHOLD)
      return ConstCArray<gfx::Rect>();
    return ConstCArray<gfx::Rect>(operation.region().data(),
                                  operation.region().size());
  }
  ...
  static bool Read(cc::mojom::FilterOperationDataView data,
    ...
    switch (out->type()) {
      ...
      case cc::FilterOperation::ALPHA_THRESHOLD:
        ...
        cc::FilterOperation::ShapeRects region;
        if (!data.ReadRegion(&region))
          return false;
        out->set_region(region);
        return true;
    }
    return false;
  }
