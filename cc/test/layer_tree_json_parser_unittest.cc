...
#define RETURN_IF_EXPECTATION_FAILS(exp) \
  ...
  RETURN_IF_EXPECTATION_FAILS(EXPECT_EQ(layer_impl->touch_event_handler(),
                                        layer->touch_event_handler()));
...
TEST_F(LayerTreeJsonParserSanityCheck, EventHandlerRegions) {
  ...
  touch_event_handler.Union(kTouchActionNone, gfx::Rect(10, 10, 20, 30));
  touch_event_handler.Union(kTouchActionNone, gfx::Rect(40, 10, 20, 20));
  touch_layer->SetTouchEventHandler(std::move(touch_event_handler));
