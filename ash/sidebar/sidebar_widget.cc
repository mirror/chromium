...
const size_t kWidth = message_center::kNotificationWidth + 2;
...
class SidebarWidget::DelegateView : public views::WidgetDelegateView,
 ...
 public:
  DelegateView(Shelf* shelf, SidebarMode mode) {
    message_center::MessageCenterTray* tray = shelf->shelf_widget()
                                                  ->status_area_widget()
    ...
    message_center_view_ =
        new MessageCenterView(message_center, tray, display_bounds.height(),
...
void SidebarWidget::SetMode(SidebarMode mode) {
  switch (mode) {
    case SidebarMode::NORMAL:
      delegate_view_->message_center_view()->SetSettingsVisible(false);
      break;
    case SidebarMode::MESSAGE_CENTER_SETTINGS:
      LOG(ERROR) << "AAA";
