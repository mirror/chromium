...
//
// This only cares about the time to present of the primary root window.
class TimeToFirstPresentRecorder : public mojom::ProcessCreationTimeRecorder {
 ...
 private:
  ...
  // Callback from the compositor when the present time is determined (valid
  // frame shown).
  ...
  base::WeakPtrFactory<TimeToFirstPresentRecorder> weak_factory_{this};
