...
TEST_F(ShellPrefsTest, Observer) {
  ...
  EXPECT_EQ(nullptr, observer.last_user_pref_service_);

  auto pref_service = base::MakeUnique<TestingPrefServiceSimple>();
  Shell::RegisterForeignOwnedProfilePrefsForTest(pref_service->registry());
  ...
  EXPECT_EQ(Shell::Get()->session_controller()->GetUserPrefServiceForUser(
                kUserAccount1),
            observer.last_user_pref_service_);
  EXPECT_EQ(Shell::Get()->session_controller()->GetUserPrefServiceForUser(
                kUserAccount1),
            Shell::Get()->GetActiveUserPrefService());

  observer.last_user_pref_service_ = nullptr;
