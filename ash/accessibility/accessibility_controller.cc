...
AccessibilityController::AccessibilityController(
    ...
    : connector_(connector),
      autoclick_delay_ms_(AutoclickController::GetDefaultAutoclickDelay()) {
...
void AccessibilityController::UpdateAutoclickDelayFromPref() {
  ...
  base::TimeDelta autoclick_delay_ms = base::TimeDelta::FromMilliseconds(
      int64_t{prefs->GetInteger(ash::prefs::kAccessibilityAutoclickDelayMs)});

  if (autoclick_delay_ms_ == autoclick_delay_ms)
    return;
  ...
  Shell::Get()->autoclick_controller()->SetAutoclickDelay(autoclick_delay_ms_);
}
