...
void DockedMagnifier::CreateMagnifierWidgets() {
  ...
  target_layer_ = std::make_unique<ui::Layer>(ui::LayerType::LAYER_SOLID_COLOR);
  target_layer_->SetBounds(GetMagnifierTargetWidgetBounds());
  target_widget_->GetNativeView()->layer()->Add(target_layer_.get());
