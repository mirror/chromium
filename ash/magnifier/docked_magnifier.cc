...
void DockedMagnifier::CreateMagnifierWidgets() {
  ...
  const auto primary_height = GetPrimaryDisplayBounds().height();
  transform.Translate(0, -(4 * kMagScale - 1) * primary_height / 4);
  transform.Scale(kMagScale, kMagScale);
  target_layer->SetTransform(transform);

  mask_layer_ = std::make_unique<ui::Layer>(ui::LAYER_NOT_DRAWN);
  mask_layer_->SetBounds(gfx::Rect(1300, 700, 100, 100));
  mask_layer_->SetMasksToBounds(true);
  target_layer->SetMaskLayer(mask_layer_.get());
