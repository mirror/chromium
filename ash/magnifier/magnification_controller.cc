...
float MagnificationController::GetScaleFromScroll(float linear_offset) {
  ...
  float linear_adjustment = std::sqrt(
      (scale - kNonMagnifiedScale) / (kMaxMagnifiedScale - kNonMagnifiedScale));
  linear_adjustment += linear_offset;
  scale = (kMaxMagnifiedScale - kNonMagnifiedScale) * linear_adjustment *
              linear_adjustment +
          kNonMagnifiedScale;
  return scale;
}
