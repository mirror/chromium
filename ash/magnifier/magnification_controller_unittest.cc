...
class TouchEventWatcher : public ui::EventHandler {
 public:
  ...
  void OnTouchEvent(ui::TouchEvent* touch_event) override {
    touch_events_.push_back(*touch_event);
  }

  const std::vector<ui::TouchEvent>& touch_events() { return touch_events_; }
...
TEST_F(MagnificationControllerTest, PanWindowToRight) {
  ...
  scale *= MagnificationController::kMagnificationScaleFactor;
