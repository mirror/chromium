...
class LockScreenNoteDisplayStateHandler
 ...
 private:
  enum class ScreenState { kOn, kAutoOff, kUserInitiatedOff };

  // Runs lock screen note launcher, which start lock screen app launch.
  void RunLockScreenNoteLauncher();

  // Forces the backlights off.
  void ForceBacklightsOff();

  // Whether the backlights are off, or being forced off.
  bool AreBacklightsOff();
  ...
  // Object user to force the backlights off.
  BacklightsForcedOffSetter* const backlights_forced_off_setter_;

  // The current screen state (determined by tracking screen brightness
  // reported by power manager).
  base::Optional<ScreenState> screen_state_;
  ...
  // backlights has been forced off, but the power manager still reports screen
  // to be on.
  ...
  // backlights in forced-off state until it's reset.
