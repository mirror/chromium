...
void LockScreenNoteDisplayStateHandler::BrightnessChanged(int level,
                                                          bool user_initiated) {
  if (level > 0) {
    screen_state_ = ScreenState::kOn;
  } else {
    screen_state_ =
        user_initiated ? ScreenState::kUserInitiatedOff : ScreenState::kAutoOff;
  }

  if (screen_state_.value() != ScreenState::kOn &&
...
void LockScreenNoteDisplayStateHandler::AttemptNoteLaunchForStylusEject() {
  ...
  // If backlights is currently off, force it off to ensure it does not get
  ...
  if (backlights_forced_off_setter_->backlights_forced_off() &&
      screen_state_.has_value() && screen_state_.value() == ScreenState::kOn) {
...
void LockScreenNoteDisplayStateHandler::GotInitialScreenBrightness(
  ...
  if (screen_state_.has_value() || !brightness.has_value())
    return;

  screen_state_ =
      brightness.value() > 0 ? ScreenState::kOn : ScreenState::kAutoOff;
}
...
void LockScreenNoteDisplayStateHandler::ForceBacklightsOff() {
  if (backlights_forced_off_)
    return;

  backlights_forced_off_ = backlights_forced_off_setter_->ForceBacklightsOff();
}

bool LockScreenNoteDisplayStateHandler::AreBacklightsOff() {
  return backlights_forced_off_setter_->backlights_forced_off() ||
         (screen_state_.has_value() &&
          screen_state_.value() == ScreenState::kAutoOff);
}
