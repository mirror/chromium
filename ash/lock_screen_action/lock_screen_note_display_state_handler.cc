...
void LockScreenNoteDisplayStateHandler::OnBacklightsForcedOffChanged() {
  if (LockScreen::Get() && LockScreen::Get()->view()) {
    LockScreen::Get()->view()->SetForegroundVisible(
        display_forced_off_setter_->backlights_forced_off() ||
        NoteLaunchInProgress());
  }
...
void LockScreenNoteDisplayStateHandler::BrightnessChanged(int level,
  ...
  if (LockScreen::Get() && LockScreen::Get()->view()) {
    if (!screen_on_.value()) {
      LockScreen::Get()->view()->SetForegroundVisible(true);
    } else if (!NoteLaunchInProgress()) {
      LockScreen::Get()->view()->SetForegroundVisible(false);
    }
  }
...
void LockScreenNoteDisplayStateHandler::NoteLaunchDone(bool success) {
  if (LockScreen::Get() && LockScreen::Get()->view())
    LockScreen::Get()->view()->SetForegroundVisible(0);
  note_launch_delayed_until_screen_off_ = false;
