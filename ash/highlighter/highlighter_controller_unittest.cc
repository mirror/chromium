...
class HighlighterControllerTest : public AshTestBase {
 public:
  ...
  void SetUp() override {
    ...
    // The pref_service_ manipulation is required to make the palette visible
    // (see ash::PaletteTrayTest).
...
TEST_F(HighlighterControllerTest, HighlighterInvokedViaPaletteTool) {
  HighlighterControllerTestApi controller_test_api_(controller_.get());
  ...
  // Disable/enable the metalayer tool to hide the highlighter.
  ...
  // The previous gesture should have exited the metalayer mode.
  EXPECT_FALSE(palette_test_api_->GetPaletteToolManager()->IsToolActive(
      PaletteToolId::METALAYER));
  palette_test_api_->GetPaletteToolManager()->ActivateTool(
      PaletteToolId::METALAYER);

  // Disabling metalayer support in the delegate should disable the metalayer
  // tool.
...
TEST_F(HighlighterControllerTest, HighlighterInvokedViaStylusButton) {
  HighlighterControllerTestApi controller_test_api_(controller_.get());
  test_palette_delegate()->set_controller_test_api(&controller_test_api_);
  GetEventGenerator().EnterPenPointerMode();
  ...
  GetEventGenerator().set_flags(ui::EF_STYLUS_BARREL_BUTTON);
  GetEventGenerator().PressTouch();
  EXPECT_FALSE(controller_test_api_.IsShowingHighlighter());
  GetEventGenerator().MoveTouch(gfx::Point(2, 2));
  EXPECT_FALSE(controller_test_api_.IsShowingHighlighter());
  ...
  // tray. This should activate neither the metalayer mode nor the highlighter.
  gfx::Point palette_point = palette_tray_->GetBoundsInScreen().CenterPoint();
  EXPECT_TRUE(palette_utils::PaletteContainsPointInScreen(palette_point));
  GetEventGenerator().MoveTouch(palette_point);
  GetEventGenerator().PressTouch();
  ...
  GetEventGenerator().set_flags(ui::EF_NONE);
  GetEventGenerator().MoveTouch(gfx::Point(2, 2));
  EXPECT_TRUE(palette_test_api_->GetPaletteToolManager()->IsToolActive(
      PaletteToolId::METALAYER));
  EXPECT_TRUE(controller_test_api_.IsShowingHighlighter());
  GetEventGenerator().ReleaseTouch();

  // Deactivate the palette tool, the highlighter should hide.
  palette_test_api_->GetPaletteToolManager()->DeactivateTool(
      PaletteToolId::METALAYER);
  EXPECT_FALSE(controller_test_api_.IsShowingHighlighter());

  // Disable the metalayer support and try press and drag with the stylus button
  // down. This should activate neither the palette tool not the highlighter.
  test_palette_delegate()->set_is_metalayer_supported(false);
