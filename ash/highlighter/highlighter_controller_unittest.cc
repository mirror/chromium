...
TEST_F(HighlighterControllerTest, HighlighterGestures) {
  ...
  EXPECT_EQ("98,94 204x14", controller_test_api_.GetSelection().ToString());
  ...
  GetEventGenerator().ReleaseTouch();
  EXPECT_TRUE(controller_test_api_.HandleSelectionCalled());
  EXPECT_EQ("0,0 200x100", controller_test_api_.GetSelection().ToString());
  ...
  GetEventGenerator().ReleaseTouch();
  EXPECT_TRUE(controller_test_api_.HandleSelectionCalled());
  EXPECT_EQ("0,0 200x300", controller_test_api_.GetSelection().ToString());
}
...
TEST_F(HighlighterControllerTest, HighlighterGesturesScaled) {
  ...
  float display_scales[] = {1.f, 1.5f, 2.0f};
  float ui_scales[] = {0.5f, 0.67f, 1.0f, 1.25f, 1.33f, 1.5f, 1.67f, 2.0f};
  ...
  for (size_t i = 0; i < sizeof(display_scales) / sizeof(float); ++i) {
    ...
    for (size_t j = 0; j < sizeof(ui_scales) / sizeof(float); ++j) {
      ...
      const gfx::Rect selection = controller_test_api_.GetSelection();
...
TEST_F(HighlighterControllerTest, HighlighterGesturesRotated) {
  ...
  TraceRect(trace);
  EXPECT_TRUE(controller_test_api_.HandleSelectionCalled());
  EXPECT_EQ("200,100 400x300", controller_test_api_.GetSelection().ToString());
  ...
  TraceRect(trace);
  EXPECT_TRUE(controller_test_api_.HandleSelectionCalled());
  EXPECT_EQ("100,899 300x400", controller_test_api_.GetSelection().ToString());
  ...
  TraceRect(trace);
  EXPECT_TRUE(controller_test_api_.HandleSelectionCalled());
  EXPECT_EQ("899,599 400x300", controller_test_api_.GetSelection().ToString());
  ...
  TraceRect(trace);
  EXPECT_TRUE(controller_test_api_.HandleSelectionCalled());
  EXPECT_EQ("599,200 300x400", controller_test_api_.GetSelection().ToString());
}
