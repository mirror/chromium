...
void HighlighterController::UpdatePointerView(ui::TouchEvent* event) {
  ...
  if (event->type() == ui::ET_TOUCH_RELEASED) {
    ...
    const HighlighterGestureType gesture_type =
        DetectHighlighterGesture(box, points, HighlighterView::kPenTipSize);

    if (gesture_type == HighlighterGestureType::kHorizontalStroke) {
      UMA_HISTOGRAM_COUNTS_1000("Ash.Shelf.Palette.Assistant.HighlighterLength",
      ...
      UMA_HISTOGRAM_COUNTS_100("Ash.Shelf.Palette.Assistant.CircledPercentage",
    ...
    if (gesture_type != HighlighterGestureType::kNoGesture) {
      if (observer_) {
    ...
    if (gesture_counter_ > 1) {
      // Up to 10 seconds.
      UMA_HISTOGRAM_TIMES("Ash.Shelf.Palette.Assistant.GestureInterval",
                          gesture_start - previous_gesture_start_);
    }
    previous_gesture_start_ = gesture_start;

    // Up to 3 minutes.
    UMA_HISTOGRAM_MEDIUM_TIMES("Ash.Shelf.Palette.Assistant.GestureDuration",
                               points.GetNewest().time - gesture_start);

    UMA_HISTOGRAM_ENUMERATION("Ash.Shelf.Palette.Assistant.GestureType",
                              gesture_type, HighlighterGestureType::kNoGesture);
  }
}
