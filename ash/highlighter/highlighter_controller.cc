...
void HighlighterController::UpdatePointerView(ui::TouchEvent* event) {
  ...
  if (bounds.Contains(gfx::Point((int)pos.x(), (int)pos.y()))) {
...
bool HighlighterController::MayStartNewGesture(ui::TouchEvent* event) {
  return !interrupted_stroke_timer_ &&
         FastInkPointerController::MayStartNewGesture(event);
}
