...
void HighlighterController::OnTouchEvent(ui::TouchEvent* event) {
  ...
  if (event->type() == ui::ET_TOUCH_RELEASED && highlighter_view_) {
    ...
    if (DetectHorizontalStroke(box, HighlighterView::kPenTipSize,
      ...
      highlighter_view_->Animate(pivot, HighlighterView::ANIMATION_FADEOUT);
      ...
      highlighter_view_->Animate(pivot, HighlighterView::ANIMATION_INFLATE);

      result_view_.reset(new HighlighterResultView(current_window));
      result_view_->AnimateDeflate(box_enclosing);
    } else {
      highlighter_view_->Animate(pivot, HighlighterView::ANIMATION_DEFLATE);
    }
  }
}
