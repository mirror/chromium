...
// Adjust the width to account for the pen tip width.
// as the pen tip and is centered at the center line
...
void HighlighterController::OnTouchEvent(ui::TouchEvent* event) {
  ...
  if (event->type() == ui::ET_TOUCH_RELEASED && highlighter_view_) {
    ...
    if (DetectHorizontalStroke(box, pen_tip_size,
      ...
      highlighter_view_->AnimateInflate(pivot);

      result_view_.reset(new HighlighterResultView(current_window));
      result_view_->AnimateDeflate(box_enclosing);
    } else {
      highlighter_view_->AnimateDeflate(pivot);
    }
  }
}
...
void HighlighterController::SwitchTargetRootWindowIfNeeded(
  ...
  if (root_window && highlighter_delegate_ && !highlighter_view_) {
