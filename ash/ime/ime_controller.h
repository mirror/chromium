...
class ASH_EXPORT ImeController : public mojom::ImeController {
 public:
  ...
  // The ImeKeyboard has induced a capslock change that needs to be reported
  // up to the tray.
  void SetCapsFromClient(bool caps_enabled) override;

  // Send a state change from the tray to the ImeControllerClient.
  void SetCapsFromController(bool caps_enabled);
 ...
 private:
  ...
  // A transient state value that is updated by changes asynchronously reported
  // changes from the ImeControllerClient client, which is in another process.
  // This is required for synchronous method calls in ash.
  bool is_caps_lock_enabled_transient_value_ = false;
