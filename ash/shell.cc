...
Shell* Shell::CreateInstance(ShellInitParams init_params) {
  CHECK(!instance_);
  std::unique_ptr<ShellPort> shell_port = std::move(init_params.shell_port);
  if (!shell_port)
    shell_port = std::make_unique<ShellPortClassic>();
  instance_ = new Shell(std::move(init_params.delegate), std::move(shell_port));
