...
constexpr float kMoleculeAnimationDurationMs = 1200;
constexpr float kMoleculeAnimationOffset = 50;
...
class VoiceInteractionIcon : public ui::Layer {
 ...
 private:
  ...
  void AnimationProgressed() {
    ...
    float now = (base::TimeTicks::Now() - base::TimeTicks()).InMillisecondsF();
    for (int i = 0; i < DOT_COUNT; ++i) {
      float normalizedTime =
          fmod(now - kMoleculeAnimationOffset * kMoleculeOrder[i],
