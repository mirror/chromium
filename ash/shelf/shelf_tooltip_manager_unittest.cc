...
class ShelfTooltipBaseTest : public AshTestBase {
 public:
  ...
  bool IsTimerRunning() const { return tooltip_manager_->timer_.IsRunning(); }
  views::Widget* GetTooltip() { return tooltip_manager_->bubble_->GetWidget(); }

  void set_tooltip_manager(ShelfTooltipBase* tooltip_manager) {
...
TEST_F(ShelfTooltipBaseTest, HideWhenShelfIsHidden) {
  ShelfView* shelf_view = GetPrimaryShelf()->GetShelfViewForTesting();
  set_tooltip_manager(ShelfViewTestAPI(shelf_view).tooltip_manager());
  tooltip_manager_->ShowTooltip(shelf_view->GetAppListButton());
  ...
  tooltip_manager_->ShowTooltipWithDelay(shelf_view->GetAppListButton());
  EXPECT_FALSE(IsTimerRunning());
}

TEST_F(ShelfTooltipBaseTest, HideWhenShelfIsAutoHideHidden) {
...
TEST_F(ShelfTooltipBaseTest, HideForEvents) {
  ...
  gfx::Rect shelf_bounds = shelf_view->GetBoundsInScreen();
...
TEST_F(ShelfTooltipBaseTest, HideForExternalEvents) {
  // TODO: investigate failure in mash. http://crbug.com/695563.
  if (Shell::GetAshConfig() == Config::MASH)
    return;
...
TEST_F(ShelfTooltipBaseTest, DoNotHideForKeyEvents) {
  ui::test::EventGenerator& generator = GetEventGenerator();
...
class StatusAreaTooltipManagerTest : public ShelfTooltipBaseTest {
 public:
  StatusAreaTooltipManagerTest() = default;
  ~StatusAreaTooltipManagerTest() override = default;

  void SetUp() override {
    ShelfTooltipBaseTest::SetUp();
    status_area_widget_delegate_ =
        GetPrimaryShelf()->GetStatusAreaWidget()->status_area_widget_delegate();
    tooltip_manager_ = status_area_widget_delegate_->tooltip_manager_for_test();
    tooltip_manager_->set_timer_delay_for_test(0);
  }

 private:
  StatusAreaWidgetDelegate* status_area_widget_delegate_ = nullptr;

  DISALLOW_COPY_AND_ASSIGN(StatusAreaTooltipManagerTest);
};

TEST_F(StatusAreaTooltipManagerTest, ShowTooltip) {
  tooltip_manager_->ShowTooltip(GetPrimarySystemTray());
  EXPECT_TRUE(tooltip_manager_->IsVisible());
  EXPECT_FALSE(IsTimerRunning());
}

TEST_F(StatusAreaTooltipManagerTest, ShowTooltipWithDelay) {
  // ShowTooltipWithDelay should start the timer instead of showing immediately.
  tooltip_manager_->ShowTooltipWithDelay(GetPrimarySystemTray());
  EXPECT_FALSE(tooltip_manager_->IsVisible());
  EXPECT_TRUE(IsTimerRunning());
  // TODO: Test that the delayed tooltip is shown, without flaky failures.
}

TEST_F(StatusAreaTooltipManagerTest, DoNotShowForInvalidView) {
  // The manager should not show or start the timer for a null view.
  tooltip_manager_->ShowTooltip(nullptr);
  EXPECT_FALSE(tooltip_manager_->IsVisible());
  tooltip_manager_->ShowTooltipWithDelay(nullptr);
  ...
  // The manager should not show or start the timer for a view that doesn't
  // override SetTextToGivenTooltip.
  views::View view;
  tooltip_manager_->ShowTooltip(&view);
  EXPECT_FALSE(tooltip_manager_->IsVisible());
  tooltip_manager_->ShowTooltipWithDelay(&view);
