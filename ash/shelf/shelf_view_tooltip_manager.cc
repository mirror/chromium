...
void ShelfViewTooltipManager::Close() {
  timer_.Stop();
  if (bubble_)
...
bool ShelfViewTooltipManager::IsVisible() const {
...
void ShelfViewTooltipManager::ShowTooltipWithDelay(views::View* view) {
  if (ShouldShowTooltipForView(view)) {
    timer_.Start(FROM_HERE, base::TimeDelta::FromMilliseconds(timer_delay_),
                 base::Bind(&ShelfViewTooltipManager::ShowTooltip,
...
void ShelfViewTooltipManager::OnAutoHideStateChanged(ShelfAutoHideState new_state) {
  if (new_state == SHELF_AUTO_HIDE_HIDDEN) {
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE,
        base::Bind(&ShelfViewTooltipManager::Close, weak_factory_.GetWeakPtr()));
  }
}
