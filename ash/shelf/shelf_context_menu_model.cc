...
void AddItems(ui::SimpleMenuModel* model,
...
// Defined out-of-line to support use of the local AddItems() helper function.
ShelfContextSubMenuModel::ShelfContextSubMenuModel(Delegate* delegate,
                                                   const MenuItemList& items,
                                                   SubmenuList* submenus)
    : ui::SimpleMenuModel(delegate) {
  AddItems(this, delegate, items, submenus);
}
...
bool CanUserModifyShelfAutoHide(PrefService* prefs) {
  ...
  return prefs && prefs->FindPreference(pref)->IsUserModifiable();
}
...
void AddLocalMenuItems(ash::MenuItemList* menu, int64_t display_id) {
  ...
  const auto alignment = GetShelfAlignmentPref(prefs, display_id);
  ash::mojom::MenuItemPtr alignment_menu(ash::mojom::MenuItem::New());
  alignment_menu->type = ash::mojom::MenuItem::Type::SUBMENU;
  ...
  alignment_menu->submenu = ash::MenuItemList();
