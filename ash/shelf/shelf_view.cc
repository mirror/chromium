...
void ShelfView::AfterItemSelected(
  ...
  if (action != SHELF_ACTION_APP_LIST_SHOWN) {
    ...
    if (action == SHELF_ACTION_SHOW_APPLICATION_MENU && menu_items &&
      ...
      LOG(ERROR) << "MSW AfterItemSelected C (CONTEXT MENU) items:" << (menu_items ? menu_items->size() : 0);
      // Show the context menu if instructed, append any provided items.
      if (ink_drop)
        ink_drop->AnimateToState(views::InkDropState::ACTIVATED);
      context_menu_id_ = item.id;
      ShowMenu(base::MakeUnique<ShelfContextMenuModel>(/* shelf_, item, */ std::move(*menu_items), model_->GetShelfItemDelegate(item.id)),
               sender, gfx::Point(), false,
               ui::GetMenuSourceTypeForEvent(*event), ink_drop);
    } else {
...
void ShelfView::ShowContextMenuForView(views::View* source,
  ...
  LOG(ERROR) << "MSW ShowContextMenuForView A source:" << source_type;
  const bool is_touch =
  ...
  ui::PointerEvent event(
    is_touch ? ui::ET_GESTURE_LONG_TAP : ui::ET_MOUSE_PRESSED,
    point, gfx::Point(), source_type == ui::MenuSourceType::MENU_SOURCE_MOUSE ? ui::EF_RIGHT_MOUSE_BUTTON : ui::EF_NONE,
    ui::EF_NONE, ui::PointerDetails(), base::TimeTicks::Now());
  model_->GetShelfItemDelegate(item->id)->ItemSelected(
      ui::Event::Clone(event), display_id, LAUNCH_FROM_UNKNOWN,
      base::Bind(&ShelfView::AfterItemSelected, weak_factory_.GetWeakPtr(),
                 *item, source, base::Passed(ui::Event::Clone(event)), nullptr));
