...
void ShelfView::AfterItemSelected(
  ...
  if (action != SHELF_ACTION_APP_LIST_SHOWN) {
    ...
    if (action == SHELF_ACTION_SHOW_APPLICATION_MENU && menu_items &&
      ...
      // Show the app menu if instructed and there are two or more items.
    ...
    } else if (action == SHELF_ACTION_SHOW_CONTEXT_MENU && menu_items &&
               menu_items->size() > 0) {
...
void ShelfView::ShowContextMenuForView(views::View* source,
  ...
  if (!item) {
    ...
    // TODO(msw): Show desktop menu here... or use the app list delegate as a default? 
