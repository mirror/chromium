...
void ShelfView::OnMaximizeModeStarted() {
  is_maximize_mode_animation_running_ = true;
}
...
void ShelfView::OnBoundsChanged(const gfx::Rect& previous_bounds) {
  ...
  // when the bounds change is caused by entering or exiting maximize mode.
  std::unique_ptr<BoundsAnimatorDisabler> disabler =
      base::MakeUnique<BoundsAnimatorDisabler>(bounds_animator_.get());

  if (is_maximize_mode_animation_running_) {
    disabler.reset();
    AnimateToIdealBounds();
    return;
  }
...
void ShelfView::OnBoundsAnimatorDone(views::BoundsAnimator* animator) {
  if (is_maximize_mode_animation_running_)
    is_maximize_mode_animation_running_ = false;
