  ...
  if (last_event_is_back_event_) {
    ...
    switch (event->type()) {
      case ui::ET_GESTURE_TAP:
        ...
        key_event = ui::KeyEvent(ui::ET_KEY_RELEASED, ui::VKEY_BROWSER_BACK,
                                 ui::EF_NONE);
      ...
      case ui::ET_GESTURE_TAP_DOWN:
        ...
        key_event = ui::KeyEvent(ui::ET_KEY_PRESSED, ui::VKEY_BROWSER_BACK,
                                 ui::EF_NONE);
        SendEventToSink(&key_event);
        break;
      default:
...
void AppListButton::OnMouseReleased(const ui::MouseEvent& event) {
  ...
  if (last_event_is_back_event_) {
    AnimateInkDrop(views::InkDropState::ACTION_TRIGGERED, &event);
    ui::KeyEvent key_event(ui::ET_KEY_RELEASED, ui::VKEY_BROWSER_BACK,
                           ui::EF_NONE);
    SendEventToSink(&key_event);
  } else {
...
void AppListButton::PaintButtonContents(gfx::Canvas* canvas) {
  ...
  if (is_maximize_mode) {
    ...
    // Create the path by drawing two circles, one around and the back button
    // and one around the app list circle. Join them with the rectangle
...
gfx::Point AppListButton::GetCircleCenterPoint() const {
  ...
  // adjust the x-position for a left- or right-aligned shelf. If we are in
  // maximized mode the button's primary axis will be |kShelfButtonSize| +
  // |kShelfButtonSize| + |kShelfButtonSpacing|, and the app list circle will
  // always be below or to the right of the back button.
...
ui::EventSink* AppListButton::GetEventSink() {
