...
void AppListButton::PaintButtonContents(gfx::Canvas* canvas) {
  ...
  if (is_tablet_mode || shelf_view_->is_tablet_mode_animation_running()) {
    ...
    // There are three cases/steps:
    // 1. Enter/exit tablet mode. |is_tablet_mode_animation_running()| is true
    // at this point, but there is a bit of delay before the app list button
    // actually starts animating (ie. |current_animation_value| <= 0.0|). At
    // this point |is_tablet_mode| is the new value, but since the animation has
    // yet to start, the opacity should match the old value of |is_tablet_mode|
    // (ie. !is_tablet_mode).
    // 2. App list button has started animating (ie. |current_animation_value| >
    // 0.0). Animate 0->255 when entering tablet mode, or 255->0 when exiting.
    // 3. We are in fully in tablet mode and done animations (the check about
    // prevents us from being fully in non-tablet mode and done animations), the
    // button is opaque.
    int opacity = is_tablet_mode ? 0 : 255;
    ...
    } else if (!shelf_view_->is_tablet_mode_animation_running()) {
      opacity = 255;
    }
