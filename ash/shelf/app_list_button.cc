...
AppListButton::AppListButton(InkDropButtonListener* listener,
  ...
  if (chromeos::switches::IsVoiceInteractionEnabled()) {
    voice_interaction_overlay_.reset(new VoiceInteractionOverlay(this, shelf_));
    AddChildView(voice_interaction_overlay_.get());
...
void AppListButton::PaintButtonContents(gfx::Canvas* canvas) {
  ...
  }
  {
    ...
    if (chromeos::switches::IsVoiceInteractionEnabled()) {
      const float kCircleRadiusDp = std::ceil(5.f * dsf);
      fg_flags.setStyle(cc::PaintFlags::kFill_Style);
      canvas->DrawCircle(circle_center, kCircleRadiusDp, fg_flags);
    }
  }
}
