...
class ShelfIconNotificationIndicatorTest
    : public ShelfViewTest,
      public testing::WithParamInterface<bool> {
 public:
  ShelfIconNotificationIndicatorTest() = default;
  ~ShelfIconNotificationIndicatorTest() override = default;
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(ShelfIconNotificationIndicatorTest);
};

// Instantiate the Boolean which is used to toggle touchable app context menus
// in the parameterized tests.
INSTANTIATE_TEST_CASE_P(, ShelfIconNotificationIndicatorTest, testing::Bool());
...
// notification.
TEST_P(ShelfIconNotificationIndicatorTest, AddedItemHasNotificationIndicator) {
  if (!is_touchable_app_context_menu_enabled_)
    return;
...
// been removed.
TEST_P(ShelfIconNotificationIndicatorTest,
  ...
  EXPECT_TRUE(button->IsNotificationIndicatorActiveForTest());
  ...
  EXPECT_FALSE(button->IsNotificationIndicatorActiveForTest());
}
