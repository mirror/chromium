...
class ShelfViewInkDropTest : public ShelfViewTest {
 ...
 protected:
  ...
  void InitAppListButtonInkDrop() {
    // Initialize the app list button member and check for a valid delegate.
    app_list_button_ = shelf_view_->GetAppListButton();
    EXPECT_EQ(TYPE_APP_LIST, model_->items()[0].type);
    EXPECT_TRUE(model_->GetShelfItemDelegate(model_->items()[0].id));
  ...
  void InitBrowserButtonInkDrop() {
    // Initialize the browser button member and install a placeholder delegate.
    browser_button_ = test_api_->GetButton(1);
    EXPECT_EQ(TYPE_BROWSER_SHORTCUT, model_->items()[1].type);
    const ShelfID browser_id = model_->items()[1].id;
    EXPECT_FALSE(model_->GetShelfItemDelegate(browser_id));
    model_->SetShelfItemDelegate(browser_id,
                                 base::MakeUnique<ShelfItemSelectionTracker>());
    EXPECT_TRUE(model_->GetShelfItemDelegate(browser_id));
