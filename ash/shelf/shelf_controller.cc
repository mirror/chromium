...
void SetShelfAutoHideBehaviorFromPrefs(PrefService* prefs = nullptr) {
  ...
  // TODO(msw): Update after James fixes Shell::OnProfilePrefServiceInitialized?
  prefs = prefs ? prefs : Shell::Get()->GetActiveUserPrefService();
  if (!prefs)
    return;
  ...
  for (const auto& display : display::Screen::GetScreen()->GetAllDisplays()) {
    ...
    GetShelfForDisplay(display.id())->SetAutoHideBehavior(value);
  }
}

// Set each Shelf's alignment from the per-display pref.
void SetShelfAlignmentFromPrefs(PrefService* prefs = nullptr) {
  // TODO(jamescook): The session state check should not be necessary, but
  // otherwise this wrongly tries to set the alignment on a secondary display
  // during login before the ShelfLockingManager is created.
  CHECK(Shell::Get()->session_controller()->IsActiveUserSessionStarted());
  ...
  for (const auto& display : display::Screen::GetScreen()->GetAllDisplays()) {
    ...
    GetShelfForDisplay(display.id())->SetAlignment(value);
  }
}

// Set each Shelf's auto-hide behavior and alignment from the per-display prefs.
void SetShelfBehaviorsFromPrefs(PrefService* prefs = nullptr) {
  SetShelfAutoHideBehaviorFromPrefs(prefs);
  SetShelfAlignmentFromPrefs(prefs);
}
...
class ShelfPrefsObserver
 ...
 public:
  // Creates and returns an instance of ShelfPrefsObserver if the prefs do not
  // contain all the necessary local settings for the shelf. If the local
  // settings are present, returns null.
  static std::unique_ptr<ShelfPrefsObserver> CreateIfNecessary(
      PrefService* prefs) {
    if (!prefs->FindPreference(prefs::kShelfAlignmentLocal)->HasUserSetting() ||
        !prefs->FindPreference(prefs::kShelfAutoHideBehaviorLocal)
             ->HasUserSetting()) {
      return base::WrapUnique(new ShelfPrefsObserver(
          static_cast<sync_preferences::PrefServiceSyncable*>(prefs)));
    }
    return nullptr;
  }

  ~ShelfPrefsObserver() override { prefs_->RemoveObserver(this); }

 private:
  explicit ShelfPrefsObserver(sync_preferences::PrefServiceSyncable* prefs)
      : prefs_(prefs) {
  ...
  // If no user-set value exists at |local_path|, the value from |synced_path|
  // is copied to |local_path|.
  void PropagatePrefToLocalIfNotSet(const char* local_path,
                                    const char* synced_path) {
    if (!prefs_->FindPreference(local_path)->HasUserSetting())
      prefs_->SetString(local_path, prefs_->GetString(synced_path));
  }
  ...
  void OnIsSyncingChanged() override {
    ...
    if (!prefs_->IsSyncing())
      return;
    PropagatePrefToLocalIfNotSet(prefs::kShelfAlignmentLocal,
                                 prefs::kShelfAlignment);
    PropagatePrefToLocalIfNotSet(prefs::kShelfAutoHideBehaviorLocal,
                                 prefs::kShelfAutoHideBehavior);
    prefs_->RemoveObserver(this);
  }

  // Prefs service. Not owned.
  sync_preferences::PrefServiceSyncable* prefs_;

  DISALLOW_COPY_AND_ASSIGN(ShelfPrefsObserver);
};
...
void ShelfController::OnActiveUserPrefServiceChanged(
  ...
  if (!pref_service)  // Null during startup, user switch and tests.
    return;
  prefs_observer_ = ShelfPrefsObserver::CreateIfNecessary(pref_service);
  SetShelfBehaviorsFromPrefs(pref_service);
  ...
  pref_change_registrar_->Add(prefs::kShelfAutoHideBehaviorLocal,
                              base::Bind(&SetShelfAutoHideBehaviorFromPrefs));
