...
void InitLocalPref(PrefChangeRegistrar* registrar,
  ...
  if (!prefs->FindPreference(local_path)->HasUserSetting())
...
void ShelfController::RegisterProfilePrefs(PrefRegistrySimple* registry) {
  ...
  // This pref is public for chrome policy management.
...
void ShelfController::OnActiveUserPrefServiceChanged(
  ...
  if (!pref_service->FindPreference(prefs::kShelfAlignmentLocal)
           ->HasUserSetting()) {
    pref_change_registrar_->Add(
        prefs::kShelfAlignment,
        base::Bind(&InitLocalPref, pref_change_registrar_.get(),
                   prefs::kShelfAlignmentLocal));
  }
  if (!pref_service->FindPreference(prefs::kShelfAutoHideBehaviorLocal)
           ->HasUserSetting()) {
    pref_change_registrar_->Add(
