...
static void ApplyShelfButtonStyle(views::Button& button) {
...
class LoginShelfImageButton : public views::ImageButton {
 public:
  LoginShelfImageButton(views::ButtonListener* listener,
                        const base::string16& text,
                        const gfx::ImageSkia& image)
      : ImageButton(listener) {
    SetAccessibleName(text);
    SetImage(views::Button::STATE_NORMAL, image);
    SetImageAlignment(views::ImageButton::HorizontalAlignment::ALIGN_CENTER,
                      views::ImageButton::VerticalAlignment::ALIGN_MIDDLE);
  }

  ~LoginShelfImageButton() override = default;
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(LoginShelfImageButton);
};
...
LoginShelfView::LoginShelfView(
  ...
  auto add_button = [this](ButtonId id, int text_resource_id,
                           const gfx::VectorIcon& icon, bool is_image) {
    ...
    if (is_image) {
      button = new LoginShelfImageButton(this, text, image);
    } else {
      button = new LoginShelfButton(this, text, image);
    }
    ApplyShelfButtonStyle(*button);
    button->set_id(id);
    AddChildView(button);
  };
  ...
  add_button(kCancel, IDS_ASH_SHELF_CANCEL_BUTTON, kShelfCancelButtonIcon,
             false);
  add_button(kBrowseAsGuest, IDS_BROWSE_AS_GUEST_BUTTON,
             kShelfBrowseAsGuestButtonIcon, false);
  add_button(kAddUser, IDS_ADD_USER_BUTTON, kShelfAddPersonButtonIcon, false);
  add_button(kMoreSettings, IDS_MORE_OPTIONS_BUTTON,
             kShelfMoreSettingsButtonIcon, true);
