  ...
  if (state_.IsScreenLocked() || state_.IsAddingSecondaryUser()) {
    ...
    switch (window_state) {
      case wm::WORKSPACE_WINDOW_STATE_FULL_SCREEN: {
        // If we get a full screen event as a result of tablet mode, do not
        ...
        if (Shell::Get()->tablet_mode_controller()->ShouldHideTitlebars()) {
          if (Shell::Get()
              ...
              Shell::Get()
                  ->tablet_mode_controller()
                  ->initializing_tablet_mode_window_manager()) {
            return;
          }
        }
...
ShelfBackgroundType ShelfLayoutManager::GetShelfBackgroundType() const {
  ...
  // If the window state is full screen, but we are in tablet mode and we want
  // to hide title bars, show the shelf.
