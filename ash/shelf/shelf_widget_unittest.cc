...
TEST_F(ShelfWidgetViewsVisibilityTest, LoginWebUiLockViews) {
  ...
  TestVisibilitily(SessionState::UNKNOWN, kNone);
  // Web UI login is used, so views shelf is not visible during login.
  TestVisibilitily(SessionState::OOBE, kNone);
  TestVisibilitily(SessionState::UNKNOWN, kNone);
  TestVisibilitily(SessionState::LOGIN_PRIMARY, kNone);

  SimulateUserLogin("user1@test.com");

  TestVisibilitily(SessionState::LOGGED_IN_NOT_ACTIVE, kNone);
  TestVisibilitily(SessionState::ACTIVE, kShelf);
  TestVisibilitily(SessionState::LOCKED, kLoginShelf);
  TestVisibilitily(SessionState::ACTIVE, kShelf);
  TestVisibilitily(SessionState::LOGIN_SECONDARY, kLoginShelf);
  TestVisibilitily(SessionState::ACTIVE, kShelf);
}
...
TEST_F(ShelfWidgetViewsVisibilityTest, LoginViewsLockViews) {
  ...
  TestVisibilitily(SessionState::UNKNOWN, kNone);
  TestVisibilitily(SessionState::OOBE, kLoginShelf);
  TestVisibilitily(SessionState::UNKNOWN, kNone);
  TestVisibilitily(SessionState::LOGIN_PRIMARY, kLoginShelf);

  SimulateUserLogin("user1@test.com");

  TestVisibilitily(SessionState::LOGGED_IN_NOT_ACTIVE, kLoginShelf);
  TestVisibilitily(SessionState::ACTIVE, kShelf);
  TestVisibilitily(SessionState::LOCKED, kLoginShelf);
  TestVisibilitily(SessionState::ACTIVE, kShelf);
  TestVisibilitily(SessionState::LOGIN_SECONDARY, kLoginShelf);
...
TEST_F(ShelfWidgetViewsVisibilityTest, LoginWebUiLockWebUi) {
  ...
  TestVisibilitily(SessionState::LOGGED_IN_NOT_ACTIVE, kNone);
  TestVisibilitily(SessionState::ACTIVE, kShelf);
  TestVisibilitily(SessionState::LOCKED, kNone);
  TestVisibilitily(SessionState::ACTIVE, kShelf);
  TestVisibilitily(SessionState::LOGIN_SECONDARY, kNone);
  TestVisibilitily(SessionState::ACTIVE, kShelf);
}
