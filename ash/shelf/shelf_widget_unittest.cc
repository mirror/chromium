...
class ShelfWidgetViewsVisibilityTest : public AshTestBase {
 public:
  ...
  void ExpectVisibile(session_manager::SessionState state,
...
TEST_F(ShelfWidgetViewsVisibilityTest, LoginWebUiLockViews) {
  ...
  ExpectVisibile(SessionState::UNKNOWN, kNone);
  // Web UI login is used, so views shelf is not visible during login.
  ExpectVisibile(SessionState::OOBE, kNone);
  ExpectVisibile(SessionState::LOGIN_PRIMARY, kNone);

  SimulateUserLogin("user1@test.com");

  ExpectVisibile(SessionState::LOGGED_IN_NOT_ACTIVE, kNone);
  ExpectVisibile(SessionState::ACTIVE, kShelf);
  ExpectVisibile(SessionState::LOCKED, kLoginShelf);
  ExpectVisibile(SessionState::ACTIVE, kShelf);
  ExpectVisibile(SessionState::LOGIN_SECONDARY, kLoginShelf);
  ExpectVisibile(SessionState::ACTIVE, kShelf);
}
...
TEST_F(ShelfWidgetViewsVisibilityTest, LoginViewsLockViews) {
  ...
  ExpectVisibile(SessionState::UNKNOWN, kNone);
  ExpectVisibile(SessionState::OOBE, kLoginShelf);
  ExpectVisibile(SessionState::LOGIN_PRIMARY, kLoginShelf);

  SimulateUserLogin("user1@test.com");

  ExpectVisibile(SessionState::LOGGED_IN_NOT_ACTIVE, kLoginShelf);
  ExpectVisibile(SessionState::ACTIVE, kShelf);
  ExpectVisibile(SessionState::LOCKED, kLoginShelf);
  ExpectVisibile(SessionState::ACTIVE, kShelf);
  ExpectVisibile(SessionState::LOGIN_SECONDARY, kLoginShelf);
...
TEST_F(ShelfWidgetViewsVisibilityTest, LoginWebUiLockWebUi) {
  ...
  ExpectVisibile(SessionState::LOGGED_IN_NOT_ACTIVE, kNone);
  ExpectVisibile(SessionState::ACTIVE, kShelf);
  ExpectVisibile(SessionState::LOCKED, kNone);
  ExpectVisibile(SessionState::ACTIVE, kShelf);
  ExpectVisibile(SessionState::LOGIN_SECONDARY, kNone);
  ExpectVisibile(SessionState::ACTIVE, kShelf);
}
