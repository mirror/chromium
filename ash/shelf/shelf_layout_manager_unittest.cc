...
TEST_F(ShelfLayoutManagerTest, SwipingUpOnShelfForFullscreenAppList) {
  ...
  // Opens one visible window to make sure the visibility of the shelf can be
  // changed. Note, the shelf will always keep visible if there is no one
  // visible window opened.
  ...
  // show, but then utimately dismiss the app list.
  ...
  // Swiping down on the shelf to hide it.
  end = start + delta;
  generator.GestureScrollSequence(start, end, kTimeDelta, kNumScrollSteps);
