...
class WallpaperControllerTest : public AshTestBase,
 ...
 protected:
  ...
  // Creates a new RunLoop object, in case Run() may not be called twice for the
...
TEST_F(WallpaperControllerTest, SetDefaultWallpaperForGuestSession) {
  ...
  const std::string wallpaper_files_id =
      GetDummyFileId(regular_user_account_id);
...
TEST_F(WallpaperControllerTest, ShowCustomWallpaperWithCorrectResolution) {
  ...
  base::FilePath small_wallpaper_path =
      GetCustomWallpaperPath(WallpaperController::kSmallWallpaperSubDir,
                             wallpaper_files_id, GetDummyFileName(account_id));
...
TEST_F(WallpaperControllerTest, RemoveUserWithCustomWallpaper) {
  const std::string user1 = "user1@test.com";
  const AccountId account_id1 = AccountId::FromUserEmail(user1);
  const std::string wallpaper_files_id1 = GetDummyFileId(account_id1);
  base::FilePath small_wallpaper_path1 = GetCustomWallpaperPath(
      WallpaperController::kSmallWallpaperSubDir, wallpaper_files_id1,
      GetDummyFileName(account_id1));
  SimulateUserLogin(user1);
  // Set a custom wallpaper for |account_id1| and verify the wallpaper exists.
  CreateAndSaveWallpapers(account_id1);
  EXPECT_TRUE(base::PathExists(small_wallpaper_path1));

  // Now login another user and set a custom wallpaper for the user.
  const std::string user2 = "user2@test.com";
  const AccountId account_id2 = AccountId::FromUserEmail(user2);
  const std::string wallpaper_files_id2 = GetDummyFileId(account_id2);
  base::FilePath small_wallpaper_path2 = GetCustomWallpaperPath(
      WallpaperController::kSmallWallpaperSubDir, wallpaper_files_id2,
      GetDummyFileName(account_id2));
  SimulateUserLogin(user2);
  CreateAndSaveWallpapers(account_id2);
  EXPECT_TRUE(base::PathExists(small_wallpaper_path2));

  // Simulate the removal of |account_id2|.
  controller_->RemoveUserWallpaper(InitializeUser(account_id2),
                                   wallpaper_files_id2);
  // Wait until all files under the user's custom_wallpapers directory are
  // removed.
  WaitUntilCustomWallpapersDeleted(account_id2);
  EXPECT_FALSE(base::PathExists(small_wallpaper_path2));
...
TEST_F(WallpaperControllerTest, RemoveUserWithDefaultWallpaper) {
  ...
  const std::string wallpaper_files_id2 = GetDummyFileId(account_id2);
  SimulateUserLogin(user2);
  controller_->SetDefaultWallpaper(InitializeUser(account_id2),
                                   wallpaper_files_id2,
                                   true /*show_wallpaper=*/);
