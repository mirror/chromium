...
class WallpaperControllerTest : public AshTestBase {
 ...
 protected:
  ...
  // Wrapper for private IsActiveUserPolicyControlledImpl().
  bool IsActiveUserPolicyControlled() {
    return controller_->IsActiveUserPolicyControlledImpl();
  }
...
TEST_F(WallpaperControllerTest, IsActiveUserPolicyControlled) {
  SetBypassDecode();
  // Simulate the login screen. Verify that it returns false since there's no
  // active user.
  ClearLogin();
  EXPECT_FALSE(IsActiveUserPolicyControlled());
  ...
  controller_->SetPolicyWallpaper(InitializeUser(account_id_1),
                                  ...
                                  std::string() /*data=*/);
  RunAllTasksUntilIdle();
  EXPECT_TRUE(IsActiveUserPolicyControlled());
