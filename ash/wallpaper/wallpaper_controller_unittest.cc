...
TEST_F(WallpaperControllerTest, SetAndRemovePolicyWallpaper) {
  SetBypassDecode();
  // Add two user sessions. |user_1| is currently active.
  SimulateUserLogin(user_1);
  GetSessionControllerClient()->AddUserSession(user_2);

  // Verify the user starts with no wallpaper info and is not controlled by
  // policy.
  wallpaper::WallpaperInfo wallpaper_info;
  EXPECT_FALSE(controller_->GetUserWallpaperInfo(account_id_2, &wallpaper_info,
                                                 true /*is_persistent=*/));
  EXPECT_FALSE(
      controller_->IsPolicyControlled(account_id_2, true /*is_persistent=*/));
  // Set a policy wallpaper. Verify that |account_id_2| becomes policy
  // controlled and the wallpaper info is updated.
  ...
  EXPECT_TRUE(
      controller_->IsPolicyControlled(account_id_2, true /*is_persistent=*/));
  // Verify the wallpaper is not updated since |account_id_2| is not active.
  EXPECT_EQ(0, GetWallpaperCount());

  // Switch active user to |account_id_2|. Verify the policy wallpaper is now
  // being shown.
  GetSessionControllerClient()->SwitchActiveUser(account_id_2);
  controller_->ShowUserWallpaper(InitializeUser(account_id_2));
  EXPECT_EQ(1, GetWallpaperCount());
  EXPECT_EQ(controller_->GetWallpaperType(), wallpaper::POLICY);

  // Switch active user to |account_id_1| again and remove the policy wallpaper
  // for |account_id_2|. Verify the wallpaper info is reset to default and
  // |account_id_2| is no longer policy controlled.
  GetSessionControllerClient()->SwitchActiveUser(account_id_1);
  controller_->RemovePolicyWallpaper(InitializeUser(account_id_2),
                                     wallpaper_files_id_2);
  ...
  // Switch active user to |account_id_2|. Verify the default wallpaper is now
  // being shown.
  GetSessionControllerClient()->SwitchActiveUser(account_id_2);
  controller_->ShowUserWallpaper(InitializeUser(account_id_2));
  EXPECT_EQ(1, GetWallpaperCount());
