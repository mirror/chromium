...
void WallpaperController::SetDefaultWallpaperImpl(
  ...
  // We need to decode the image if the file path doesn't match the cached value
  // (i.e. it is a different default wallpaper). Otherwise, directly run the
  // callback with the cached decoded image.
  if (default_wallpaper_file_path_ == file_path) {
...
void WallpaperController::UpdateCustomWallpaperLayout(
  ...
  if (!GetUserWallpaperInfo(user_info->account_id, &info,
      ...
      info.type != wallpaper::CUSTOMIZED || info.layout == layout) {
    return;
  }
...
void WallpaperController::ShowDefaultWallpaperForTesting() {
  default_wallpaper_image_ = CreateSolidColorWallpaper();
  default_wallpaper_file_path_.clear();
  SetWallpaperImage(default_wallpaper_image_,
...
void WallpaperController::OnDefaultWallpaperDecoded(
  ...
  if (image.isNull()) {
    ...
    default_wallpaper_file_path_ = path;
  }
