...
void CacheProminentColors(const std::vector<SkColor>& colors,
                          const base::Optional<std::string> cache_id) {
  // Local state is null in the case of Config::MASH.
  if (!cache_id.has_value() || !Shell::Get()->GetLocalStatePrefService())
    return;
  ...
  wallpaper_colors_update->SetWithoutPathExpansion(cache_id.value(),
...
base::Optional<std::vector<SkColor>> GetCachedColors(
  ...
  if (!cache_id.has_value() || !Shell::Get()->GetLocalStatePrefService() ||
      !Shell::Get()
           ...
           ->GetListWithoutPathExpansion(cache_id.value(), &prominent_colors)) {
...
void WallpaperController::OnColorCalculationComplete() {
  ...
  color_calculator_.reset();
  CacheProminentColors(colors, cache_id_);
