...
// |color_profile|.
wallpaper::ColorProfileIndex GetColorProfileIndex(ColorProfile color_profile) {
  if (color_profile.saturation == SaturationRange::VIBRANT) {
    switch (color_profile.luma) {
      case LumaRange::DARK:
        return wallpaper::COLOR_PROFILE_INDEX_DARK_VIBRANT;
      case LumaRange::NORMAL:
        return wallpaper::COLOR_PROFILE_INDEX_NORMAL_VIBRANT;
      case LumaRange::LIGHT:
        return wallpaper::COLOR_PROFILE_INDEX_LIGHT_VIBRANT;
    }
  } else {
    switch (color_profile.luma) {
      case LumaRange::DARK:
        return wallpaper::COLOR_PROFILE_INDEX_DARK_MUTED;
      case LumaRange::NORMAL:
        return wallpaper::COLOR_PROFILE_INDEX_NORMAL_MUTED;
      case LumaRange::LIGHT:
        return wallpaper::COLOR_PROFILE_INDEX_LIGHT_MUTED;
    }
  }
  NOTREACHED();
  return wallpaper::COLOR_PROFILE_INDEX_DARK_MUTED;
...
SkColor WallpaperController::GetProminentColor(
  ...
  wallpaper::ColorProfileIndex index = GetColorProfileIndex(color_profile);
...
void WallpaperController::SetProminentColors(
  ...
  mojo_observers_.ForAllPtrs([colors](mojom::WallpaperObserver* observer) {
    observer->OnWallpaperColorsChanged(colors);
  });
}
