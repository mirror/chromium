...
bool WallpaperController::CanSetCustomWallpaper(const AccountId& account_id,
                                                bool is_persistent) const {
  // There is no visible wallpaper in kiosk mode.
  if (IsInKioskMode())
    return false;
...
void WallpaperController::SetCustomWallpaper(
  ...
  // removed, and only a single |CanSetCustomWallpaper| check is needed here.
  if ((type == wallpaper::CUSTOMIZED &&
       !CanSetCustomWallpaper(user_info->account_id,
