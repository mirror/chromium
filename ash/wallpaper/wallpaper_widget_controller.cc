...
bool WallpaperWidgetController::Reparent(aura::Window* root_window,
                                         int container) {
  if (widget_) {
    // Ensures the cache render surface of the old parent is unset, and the
    // value is passed to the new parent.
    bool cache_render_surface =
        widget_parent_->layer()->GetCacheRenderSurface();
    widget_parent_->layer()->SetCacheRenderSurface(false);
    ...
    widget_parent_->AddObserver(this);
    widget_parent_->layer()->SetCacheRenderSurface(cache_render_surface);
    return true;
  }
  // Nothing to reparent.
  return false;
