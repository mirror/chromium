...
class ASH_EXPORT WallpaperController
 ...
 public:
  ...
  // Actual implementation of |SetDefaultWallpaper|.
  // Sets wallpaper to default if |show_wallpaper| is true. Otherwise just save
  // defaut wallpaper to cache. |user_type| is the type of the user initiating
  // the wallpaper request; may be different from the active user.
  void SetDefaultWallpaperImpl(const AccountId& account_id,
                               const user_manager::UserType& user_type,
                               bool show_wallpaper,
                               MovableOnDestroyCallbackHolder on_finish);

  // Actual implementation of |SetCustomizedDefaultWallpaper|.
  // Sets |default_wallpaper_image_| either to |small_wallpaper_image| or
  // |large_wallpaper_image| depending on
  ...
  // Initialize wallpaper info for the user to default and saves it to local
  ...
  // Sets command line for testing wallpapers that get file path from command
  // line, e.g. default wallpapers, child wallpapers etc.
  void SetCommandLineForTesting(base::CommandLine* command_line);

  // Shows a 1x1 solid color  wallpaper.
 ...
 private:
  ...
  // Sets default wallpaper to be the decoded image, and show the wallpaper now
