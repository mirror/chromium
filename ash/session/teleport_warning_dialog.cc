...
TeleportWarningDialog::TeleportWarningDialog(OnAcceptCallback callback)
    : never_show_again_checkbox_(new views::Checkbox(
          l10n_util::GetStringUTF16(IDS_TELEPORT_WARNING_SHOW_DISMISS))),
...
base::string16 TeleportWarningDialog::GetWindowTitle() const {
  return l10n_util::GetStringUTF16(IDS_TELEPORT_WARNING_TITLE);
}
...
void TeleportWarningDialog::InitDialog() {
  ...
  // Explanation string
  views::Label* label =
      new views::Label(l10n_util::GetStringUTF16(IDS_TELEPORT_WARNING_MESSAGE));
