...
TEST_F(PersistentWindowControllerTest, WindowMovedByAccel) {
  ...
  // Moves |w2| to primary display, which will cause the saved peristent window
  // info becoming stale. Before next display change, such as disconnecting
  // secondary display, the saved persistent window info needs an update.
  wm::ActivateWindow(w2);
  HandleMoveActiveWindowToDisplay(DisplayMoveWindowDirection::kLeft);
  EXPECT_EQ(gfx::Rect(200, 0, 100, 200), w1->GetBoundsInScreen());
  EXPECT_EQ(gfx::Rect(1, 0, 200, 100), w2->GetBoundsInScreen());
  EXPECT_FALSE(wm::GetWindowState(w2)->bounds_changed_by_user());
