...
void MirrorWindowController::UpdateWindow(
  ...
  LOG(ERROR) << "MSW MirrorWindowController::UpdateWindow A IsInUnifiedMode:" << display_manager->IsInUnifiedMode() 
             << " unified-id:" << display::kUnifiedDisplayId << " primary-id:" << display::Screen::GetScreen()->GetPrimaryDisplay().id(); 
