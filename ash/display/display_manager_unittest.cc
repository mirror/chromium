...
TEST_F(DisplayManagerTest, UpdateDisplayZoomTest) {
  ...
  display_manager()->UpdateZoomFactor(info_1.id(), modes[0], zoom_factor_1);
  EXPECT_EQ(display_manager()->GetZoomFactorForDisplay(info_1.id(), modes[0]),
            zoom_factor_1);
  ...
  display_manager()->UpdateZoomFactor(info_1.id(), modes[1], zoom_factor_2);

  EXPECT_EQ(display_manager()->GetZoomFactorForDisplay(info_1.id(), modes[1]),
            zoom_factor_2);
  ...
  // Setting the zoom factor of 1 mode should not effect the zoom factor of
  // another mode.
  EXPECT_EQ(display_manager()->GetZoomFactorForDisplay(info_1.id(), modes[0]),
            zoom_factor_1);

  // When setting the display mode back to the old one, the final effective
  // device scale factor should be using the old zoom factor.
  display::test::SetDisplayResolution(display_manager(), info_1.id(),
                                      modes[0].size());
  display_manager()->UpdateDisplays();
  EXPECT_EQ(
  ...
  display_manager()->UpdateZoomFactor(info_2.id(), info_2.display_modes()[0],
                                      zoom_factor_3);
  EXPECT_EQ(display_manager()->GetZoomFactorForDisplay(
                info_2.id(), info_2.display_modes()[0]),
            zoom_factor_3);
  EXPECT_EQ(
  ...
  display_manager()->UpdateZoomFactor(info_1.id(), modes[0], zoom_factor_3);
