...
TEST_F(MultiMirroringTest, MixedModeBasics) {
  UpdateDisplay("300x400,400x500,500x600");
  display::DisplayIdList id_list = display_manager()->GetCurrentDisplayIdList();

  // Turn on mixed mode. (Mirror from the first display to the second display)
  display::DisplayIdList mirroring_destination_ids;
  mirroring_destination_ids.emplace_back(id_list[1]);
  display_manager()->SetMixedMode(true, id_list[0], mirroring_destination_ids);
  EXPECT_TRUE(display_manager()->IsInSoftwareMirrorMode());
  ...
  const display::MixedModeParam* mixed_mode_param =
      display_manager()->mixed_mode_param();
  EXPECT_EQ(id_list[0], mixed_mode_param->mirroring_source_id);
  EXPECT_EQ(1U, mixed_mode_param->mirroring_destination_ids.size());
  EXPECT_EQ(id_list[1], mixed_mode_param->mirroring_destination_ids[0]);

  // Turn off mixed mode.
  display_manager()->SetMixedMode(false, display::kInvalidDisplayId,
                                  display::DisplayIdList());
  EXPECT_FALSE(display_manager()->IsInMirrorMode());
  EXPECT_FALSE(display_manager()->mixed_mode_param());
}

TEST_F(MultiMirroringTest, MixedModeToMirrorMode) {
  UpdateDisplay("300x400,400x500,500x600");
  display::DisplayIdList id_list = display_manager()->GetCurrentDisplayIdList();
  ...
  // Overwrite mixed mode with mirror mode.
  display_manager()->SetMirrorMode(true);
  EXPECT_TRUE(display_manager()->IsInMirrorMode());
  EXPECT_EQ(id_list[0], display_manager()->mirroring_source_id());
...
TEST_F(MultiMirroringTest, MirrorModeToMixedMode) {
  UpdateDisplay("300x400,400x500,500x600");
  display::DisplayIdList id_list = display_manager()->GetCurrentDisplayIdList();
  ...
  // Overwrite mirror mode with mixed mode. (Mirror from the first display to
  // the second display)
  display::DisplayIdList mirroring_destination_ids;
  mirroring_destination_ids.emplace_back(id_list[1]);
  display_manager()->SetMixedMode(true, id_list[0], mirroring_destination_ids);
...
TEST_F(MultiMirroringTest, MixedModeRestore) {
  const int64_t internal_display_id =
      display::test::DisplayManagerTestApi(display_manager())
          .SetFirstDisplayAsInternalDisplay();
  constexpr int64_t first_display_id = 210000001;
  ...
  // Turn on mixed mode. (Mirror from the internal display to the first display)
  display::DisplayIdList mirroring_destination_ids;
  mirroring_destination_ids.emplace_back(first_display_id);
  display_manager()->SetMixedMode(true, internal_display_id,
