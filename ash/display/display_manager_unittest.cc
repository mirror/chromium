...
TEST_F(MultiMirroringTest, MirrorModeRestore) {
  ...
  display_manager()->OnNativeDisplaysChanged(display_info_list);
  EXPECT_FALSE(display_manager()->IsInMirrorMode());
  EXPECT_EQ(display::DisplayManager::PreviousMirrorMode::kNotSet,
  ...
  display_manager()->OnNativeDisplaysChanged(display_info_list);
  EXPECT_FALSE(display_manager()->IsInMirrorMode());
  EXPECT_EQ(display::DisplayManager::PreviousMirrorMode::kMirrorOff,
            display_manager()->previous_mirror_mode());

  // Turn on mirror mode.
  ActivateSoftwareMirrorMode(true);
  EXPECT_TRUE(display_manager()->IsInMirrorMode());
  EXPECT_EQ(display::DisplayManager::PreviousMirrorMode::kMirrorOn,
            display_manager()->previous_mirror_mode());
