...
TEST_F(DisplayMoveWindowControllerTest, WindowBounds) {
  ...
  controller_->HandleMoveWindowToDisplay(Direction::kRight);
...
TEST_F(DisplayMoveWindowControllerTest, ThreeDisplaysHorizontalLayout) {
  ...
  controller_->HandleMoveWindowToDisplay(Direction::kLeft);
  ...
  // display.
  controller_->HandleMoveWindowToDisplay(Direction::kUp);
  EXPECT_EQ(list[0], screen_->GetDisplayNearestWindow(window.get()).id());
  controller_->HandleMoveWindowToDisplay(Direction::kDown);
  EXPECT_EQ(list[0], screen_->GetDisplayNearestWindow(window.get()).id());
}
