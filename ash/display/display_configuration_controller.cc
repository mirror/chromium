...
void DisplayConfigurationController::SetMirrorModeImpl(bool mirror) {
  if (mirror) {
    ...
    for (auto& display : screen->GetAllDisplays()) {
      LOG(ERROR) << "display id: " << display.id();
      LOG(ERROR) << "primary display id: " << screen->GetPrimaryDisplay().id();
      if (display.id() != screen->GetPrimaryDisplay().id())
        displays.push_back(display);
    }
    LOG(ERROR) << "here displays size: " << displays.size();
    Shell::Get()->persistent_window_controller()->SavePersistentWindowBounds(
        displays);
  }
