...
gfx::Rect GetLeftSnappedBoundsInDisplay(const display::Display& display) {
...
// Tests that window state (maximized/fullscreen/snapped) and its bounds is not
// changed after moving window to another display.
TEST_F(DisplayMoveWindowUtilTest, WindowState) {
  ...
  EXPECT_EQ(
      GetLeftSnappedBoundsInDisplay(screen->GetDisplayNearestWindow(window)),
