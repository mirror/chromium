...
void DisplayMoveWindowController::HandleMoveWindowToDisplay(
  ...
  int64_t dest_display_id = display::kInvalidDisplayId;
  switch (direction) {
    case Direction::LEFT:
      dest_display_id = GetNextLeft(origin_display);
      break;
    case Direction::UP:
      dest_display_id = GetNextUp(origin_display);
      break;
    case Direction::RIGHT:
      dest_display_id = GetNextRight(origin_display);
      break;
    case Direction::DOWN:
      dest_display_id = GetNextDown(origin_display);
      break;
    default:
      NOTREACHED();
  }
...
int64_t DisplayMoveWindowController::GetNextLeft(
  ...
  // Using an array of two elements to record the searching result. The first
  // one saves the result in the left direction. The second one saves the result
  // in the cycled direction.
  display::Display dest_display[2];
  for (const auto& display : display::Screen::GetScreen()->GetAllDisplays()) {
    if (display.id() == origin_display.id())
      continue;
    if (GetHorizontalOffset(origin_display, display) <
        (origin_display.bounds().width() + display.bounds().width()) / 2) {
      continue;
    }
    ...
    if (dest_display[idx].is_valid()) {
      ...
      if (candidate_left_space < left_space) {
        // For idx == 0, we are getting a closer display in the left direction.
        // For idx == 1, we are getting a further display in the cycled
        // direction.
...
int64_t DisplayMoveWindowController::GetNextRight(
    const display::Display& origin_display) {
  // Using an array of two elements to record the searching result. The first
  // one saves the result in the right direction. The second one saves the
  // result in the cycled direction.
  display::Display dest_display[2];
  for (const auto& display : display::Screen::GetScreen()->GetAllDisplays()) {
    if (display.id() == origin_display.id())
      continue;
    ...
    if (dest_display[idx].is_valid()) {
      ...
      if (candidate_right_space < right_space) {
        // For idx == 0, we are getting a closer display in the right direction.
        // For idx == 1, we are getting a further display in the cycled
        // direction.
        dest_display[idx] = display;
...
int64_t DisplayMoveWindowController::GetNextUp(
    const display::Display& origin_display) {
  // Using an array of two elements to record the searching result. The first
  // one saves the result in the up direction. The second one saves the result
  // in the cycled direction.
  display::Display dest_display[2];
  for (const auto& display : display::Screen::GetScreen()->GetAllDisplays()) {
    if (display.id() == origin_display.id())
      continue;
    if (GetVerticalOffset(origin_display, display) <
        (origin_display.bounds().height() + display.bounds().height()) / 2) {
      continue;
    }
    int candidate_up_space =
        origin_display.bounds().y() - display.bounds().bottom();
    int candidate_horizontal_offset =
        GetHorizontalOffset(origin_display, display);
    int idx = candidate_up_space >= 0 ? 0 : 1;
    if (dest_display[idx].is_valid()) {
      int up_space =
          origin_display.bounds().y() - dest_display[idx].bounds().bottom();
      if (candidate_up_space < up_space) {
        // For idx == 0, we are getting a closer display in the up direction.
        // For idx == 1, we are getting a further display in the cycled
        // direction.
        dest_display[idx] = display;
...
int64_t DisplayMoveWindowController::GetNextDown(
    const display::Display& origin_display) {
  // Using an array of two elements to record the searching result. The first
  // one saves the result in the down direction. The second one saves the result
  // in the cycled direction.
  display::Display dest_display[2];
  for (const auto& display : display::Screen::GetScreen()->GetAllDisplays()) {
    if (display.id() == origin_display.id())
    ...
    int candidate_down_space =
        display.bounds().y() - origin_display.bounds().bottom();
    int candidate_horizontal_offset =
        GetHorizontalOffset(origin_display, display);
    int idx = candidate_down_space >= 0 ? 0 : 1;
    if (dest_display[idx].is_valid()) {
      int down_space =
          dest_display[idx].bounds().y() - origin_display.bounds().bottom();
      if (candidate_down_space < down_space) {
        // For idx == 0, we are getting a closer display in the down direction.
        // For idx == 1, we are getting a further display in the cycled
        // direction.
        dest_display[idx] = display;
