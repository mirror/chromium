...
int64_t DisplayMoveWindowController::GetNextLeft(
  ...
  for (const auto& display : display::Screen::GetScreen()->GetAllDisplays()) {
    if (display.id() == origin_display.id())
      continue;
    // Do not make a display as candidate if it is not completely in the left of
    // left edge of |origin_display| or in the right of right edge of
    // |origin_display|.
    ...
    if (dest_display[idx].is_valid()) {
      ...
      if (candidate_left_space < left_space) {
        ...
        // For displays that have the same left space, the one has less vertical
        // offset wins.
        int vertical_offset =
