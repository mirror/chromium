...
const aura::WindowTreeHost* GetCurrentEventUnifiedMirroringSourceHost() {
  return Shell::Get()
      ->window_tree_host_manager()
      ->mirror_window_controller()
...
bool UnifiedMouseWarpController::WarpMouseCursor(ui::MouseEvent* event) {
  ...
  const auto* host = GetCurrentEventUnifiedMirroringSourceHost();
  int64_t containing_display_id = display::kInvalidDisplayId;
  const display::Displays& mirroring_display_list =
      Shell::Get()->display_manager()->software_mirroring_display_list();
  for (const auto& display : mirroring_display_list) {
    auto* this_host = GetMirroringAshWindowTreeHostForDisplayId(display.id())
                          ->AsWindowTreeHost();
    if (this_host == host) {
      containing_display_id = display.id();
      ...
      if (current_cursor_display_id_ != display::kInvalidDisplayId &&
          current_cursor_display_id_ != containing_display_id) {
  ...
  if (containing_display_id == display::kInvalidDisplayId)
    return false;

  return WarpMouseCursorInNativeCoords(containing_display_id, point_in_native,
