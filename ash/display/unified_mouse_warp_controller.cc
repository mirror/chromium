...
void UnifiedMouseWarpController::ComputeBounds() {
  ...
  D_OUT(marker, "/////////////////////////////////////////////////");
  D_OUT(marker, "/////////////////////////////////////////////////");
  D_OUT_VAL(marker, bounds_computed_);
      ...
      if (display::ComputeBoundary(first, second, &first_edge, &second_edge)) {
        ...
        second_edge = GetNativeEdgeBounds(
            GetMirroringAshWindowTreeHostForDisplayId(second.id()),
                                                      second_edge);
        D_OUT_VAL(marker, second_edge.ToString());
        displays_edge_bounds_in_native_[first.id()].emplace_back(
            second.id(), first_edge);
        displays_edge_bounds_in_native_[second.id()].emplace_back(
            first.id(), second_edge);
      }
    }
  }
...
bool UnifiedMouseWarpController::WarpMouseCursorInNativeCoords(
  ...
  std::queue<int64_t> potential_display_ids;
  std::set<int64_t> visited_displays;
  potential_display_ids.emplace(iter->id());

  while (!potential_display_ids.empty()) {
    const int64_t id = potential_display_ids.front();
    visited_displays.emplace(id);
    potential_display_ids.pop();
    auto edges_iter = displays_edge_bounds_in_native_.find(id);
    if (edges_iter == displays_edge_bounds_in_native_.end())
      continue;

    for (const auto& display_edge : edges_iter->second) {
      const gfx::Rect& edge = display_edge.second;
      if (edge.Contains(point_in_native)) {
        auto marker = MARK_FUNC();
        D_OUT(marker, "\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\");
        D_OUT(marker, "\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\");
        D_OUT(marker, "Found it ....");
        D_OUT_VAL(marker, id);
        D_OUT_VAL(marker, edge.ToString());
        D_OUT_VAL(marker, point_in_native.ToString());
        D_OUT_VAL(marker, point_in_unified_host.ToString());
        ...
        current_cursor_display_id_ = id;

        AshWindowTreeHost* target_ash_host =
            GetMirroringAshWindowTreeHostForDisplayId(display_edge.first);
        MoveCursorTo(target_ash_host, point_in_unified_host,
                     update_mouse_location_now);
        return true;
      }

      if (visited_displays.emplace(display_edge.first).second)
        potential_display_ids.emplace(display_edge.first);
    }
  }
