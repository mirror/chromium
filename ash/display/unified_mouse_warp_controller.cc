...
aura::WindowTreeHost* GetMirroringSourceHostForCurrentEvent() {
  return Shell::Get()
      ->window_tree_host_manager()
      ->mirror_window_controller()
...
bool UnifiedMouseWarpController::WarpMouseCursor(ui::MouseEvent* event) {
  ...
  auto* host = GetMirroringSourceHostForCurrentEvent();
  if (!host)
    return false;
  ...
  const gfx::Rect local_host_bounds(host->GetBoundsInPixels().size());
  if (!local_host_bounds.Contains(point_in_native)) {
    // This happens only in X11 Ozone on Linux ChromeOS build as a result of
    // passive grab.
    const ui::LocatedEvent* native_event =
        static_cast<const ui::LocatedEvent*>(event->native_event());
    point_in_native = native_event->root_location();
  } else {
