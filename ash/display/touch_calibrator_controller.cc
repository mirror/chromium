...
void TouchCalibratorController::OnTouchEvent(ui::TouchEvent* touch) {
  ...
  if (target_screen_calibration_view->GetDisplayPointLocation(&display_point)) {
    ...
    //
    //  - The touch device dispatches the raw event location. Lets assume this
    //    is (x, y).
    //  - The EventFactoryEvdev applies a touch transform that includes any
    //    calibration information as well as an offset of the native bounds
    //    of the display. So if the origin of a display in screen space is
    //    (A, B), the event location after this stage would be (x + A, y + B).
    //  - The DrmWindowHost adds an offset to the event location so that the
    //    location is relative to the platform window's origin. In Chrome OS it
    //    so happens that each display is its own platform window. So an offset
    //    equal to the display's origin in screen space is subtracted from the
    //    event location. This effectively undoes the previous step's
    //    transformation. Thus the event location after this step is (x, y).
    ...
    //    location. The display in context is the display that is associated
    //    with the touch device from which the event originated from.
    //
