...
void TouchCalibratorController::StartCalibration(
    ...
    const TouchCalibrationCallback& opt_callback) {
...
void TouchCalibratorController::StopCalibrationAndResetParams() {
  ...
  if (opt_callback_) {
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::Bind(opt_callback_, false /* failure */));
    opt_callback_.Reset();
  }
}
...
void TouchCalibratorController::CompleteCalibration(
  ...
  uint32_t touch_device_identifier =
      display::TouchCalibrationData::kFallbackTouchDeviceIdentifier;
  ...
  if (!did_find_touch_device) {
    VLOG(2) << "No touch device with id: " << touch_device_id_ << " found to "
  ...
  if (opt_callback_) {
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::Bind(opt_callback_, true /* success */));
    opt_callback_.Reset();
  }
  StopCalibrationAndResetParams();
