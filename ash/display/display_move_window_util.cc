...
// Display candidate types when getting next display.
enum DisplayCandidateType {
...
int64_t GetNextDisplay(const display::Display& origin_display,
                       DisplayMoveWindowDirection direction) {
  // An array to record the searching result. Its index is referenced by
  // DisplayCandidateType and its value is the corresponding next display.
  std::array<display::Display, 2> next_display_by_type;
  for (const auto& candidate_display :
    ...
    const DisplayCandidateType candidate_type =
        candidate_space >= 0 ? kInMovementDirection : kInCycledDirection;
    if (next_display_by_type[candidate_type].is_valid()) {
      const int current_space = GetSpaceBetweenDisplays(
          next_display_by_type[candidate_type], origin_display, direction);
      if (candidate_space < current_space) {
        next_display_by_type[candidate_type] = candidate_display;
      } else if (candidate_space == current_space) {
        const int candidate_orthogonal_offset =
            GetOrthogonalOffsetBetweenDisplays(candidate_display,
                                               origin_display, direction);
        const int current_orthogonal_offset =
            GetOrthogonalOffsetBetweenDisplays(
                next_display_by_type[candidate_type], origin_display,
                direction);
  ...
  return next_display_by_type[kInMovementDirection].is_valid()
             ? next_display_by_type[kInMovementDirection].id()
             : next_display_by_type[kInCycledDirection].id();
}
