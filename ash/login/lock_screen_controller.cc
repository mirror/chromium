...
void LockScreenController::DoAuthenticateUser(
  ...
  if (is_pin && prefs) {
    ...
    // Besides the pin, password could be numbers too. Since we hash
    // the password/pin differently before sending to chrome, if the user has
    // numeric password and we hashed it as pin, authentication will fail.
    // This checks whether it is a pin or a normal password.
    // We still need to send the verified pin to chrome to reset the states in
    // chrome, like unlock attempt in PinStorage.
    if (hashed_pin == prefs->GetString(prefs::kQuickUnlockPinSecret))
