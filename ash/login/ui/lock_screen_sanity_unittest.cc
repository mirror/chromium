...
TEST_F(LockScreenSanityTest, TabGoesFromLockToStatusAreaAndBackToLock) {
  ...
  // Lock has focus.
  EXPECT_TRUE(HasFocusInAnyChildView(lock));
  EXPECT_FALSE(HasFocusInAnyChildView(system_tray));
  ...
  EXPECT_FALSE(HasFocusInAnyChildView(lock));
  EXPECT_TRUE(HasFocusInAnyChildView(system_tray));
...
TEST_F(LockScreenSanityTest, ShiftTabGoesToShelf) {
  ...
  EXPECT_TRUE(HasFocusInAnyChildView(lock));
  EXPECT_FALSE(HasFocusInAnyChildView(shelf));

  // A single shift+tab brings focus to the shelf.
  GetEventGenerator().PressKey(ui::KeyboardCode::VKEY_TAB, ui::EF_SHIFT_DOWN);
  EXPECT_FALSE(HasFocusInAnyChildView(lock));
  EXPECT_TRUE(HasFocusInAnyChildView(shelf));
}
