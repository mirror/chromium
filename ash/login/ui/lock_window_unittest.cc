...
class LockWindowVirtualKeyboardTest : public AshTestBase {
 public:
  ...
  LockContentsView* GetLockContentsView() {
    return ash::LockScreen::Get()->window()
               ? reinterpret_cast<LockContentsView*>(
                     static_cast<views::Widget*>(
                         ash::LockScreen::Get()->window())
                         ->GetContentsView())
               : nullptr;
  }

  void LoadUser(const std::string& name) {
    mojom::LoginUserInfoPtr user = mojom::LoginUserInfo::New();
    user->basic_user_info = mojom::UserInfo::New();
    user->basic_user_info->account_id =
        AccountId::FromUserEmail(name + "@test.com");
    user->basic_user_info->display_name = name;
    ...
    login_controller_->LoadUsers(std::move(users), std::move(false));
  }
...
TEST_F(LockWindowVirtualKeyboardTest, VirtualKeyboardDoesNotCoverAuthView) {
  ShowLockScreen();
  auto* lock_contents = GetLockContentsView();
  ASSERT_NE(nullptr, lock_contents);

  LoadUser("ReadyPlayerOne");
  auto* auth_view = MakeLockContentsViewTestApi(lock_contents).primary_auth();
