...
constexpr int kBubbleAnchorViewSize = 100;

class LoginBubbleUnittest : public LoginTestBase {
 protected:
  LoginBubbleUnittest() = default;
  ~LoginBubbleUnittest() override = default;
  ...
  void SetUp() override {
    ...
    other_view_->SetPreferredSize(
        gfx::Size(kBubbleAnchorViewSize, kBubbleAnchorViewSize));
    ...
    bubble_ = base::MakeUnique<LoginBubble>(
        LoginBubble::LoginBubbleType::kUserMenu, bubble_opener_);
  }
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(LoginBubbleUnittest);
};

}  // namespace

// Verifies the base bubble settings.
TEST_F(LoginBubbleUnittest, BaseBubbleSettings) {
  bubble_->Show(base::string16(), base::string16(), container_);
...
TEST_F(LoginBubbleUnittest, BubbleKeyEventHandling) {
...
TEST_F(LoginBubbleUnittest, BubbleMouseEventHandling) {
  EXPECT_FALSE(bubble_->IsVisible());
...
TEST_F(LoginBubbleUnittest, BubbleGestureEventHandling) {
  EXPECT_FALSE(bubble_->IsVisible());
