...
// Wraps the view so it gets the correct preferred height.
views::View* WrapViewForPreferredSize(views::View* view) {
  auto* proxy = new views::View();
  proxy->SetLayoutManager(new views::BoxLayout(views::BoxLayout::kHorizontal));
...
void LockContentsView::AddedToWidget() {
  bool landscape = ShowLandscape(GetWidget());
...
void LockContentsView::OnUsersChanged(
  ...
  AddChildView(WrapViewForPreferredSize(primary_auth_));
...
void LockContentsView::CreateLowDensityLayout(
  ...
  AddChildView(WrapViewForPreferredSize(opt_secondary_auth_));
}
...
void LockContentsView::AddRotationAction(const OnRotate& on_rotate) {
  on_rotate.Run(ShowLandscape(GetWidget()));
