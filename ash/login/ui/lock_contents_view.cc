...
void LockContentsView::OnUsersChanged(
  ...
  MaybeUpdateCurrentAuthUser(primary_auth_->current_user()->account_id);
...
void LockContentsView::SwapPrimaryAndSecondaryAuth(bool is_primary) {
  if (is_primary &&
    ...
    LayoutAuth(primary_auth_, opt_secondary_auth_, true /*animate*/);
             ...
             opt_secondary_auth_->auth_methods() ==
                 LoginAuthUserView::AUTH_NONE) {
    MaybeUpdateCurrentAuthUser(opt_secondary_auth_->current_user()->account_id);
...
void LockContentsView::SwapToAuthUser(int user_index) {
  ...
  MaybeUpdateCurrentAuthUser(new_auth_user->account_id);
  LayoutAuth(primary_auth_, nullptr, true /*animate*/);
}
...
void LockContentsView::MaybeUpdateCurrentAuthUser(AccountId account_id) {
  if (current_auth_account_id_ == account_id)
    return;
  current_auth_account_id_ = account_id;
