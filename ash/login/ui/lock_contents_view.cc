...
void LockContentsView::OnFocusLeavingSystemTray(bool reverse) {
  ...
  // one is active (in which case lock UI is not visible). In the later case,
  // focus lock screen first, to synchronously take away focus from the system
  // tray first - lock shelf view expect the focus to be taken when it passes
  // it to lock screen view, and can misbehave in case the focus is kept on it.
  FindFirstOrLastFocusableChild(this, reverse)->RequestFocus();
