...
LockContentsView::LockContentsView(LoginDataSource* data_source)
...
void LockContentsView::SetPinEnabledForUser(const AccountId& user,
                                            bool enabled) {
  LockContentsView::UserState* state =
      FindStateForUser(auth_user_view_->active_user());
  if (!state) {
