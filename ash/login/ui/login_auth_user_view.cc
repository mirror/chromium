...
// Implicit animation observer that will hide a view so long as the animation
// was not cancelled.
class HideViewAnimationObserver : public ui::ImplicitAnimationObserver {
 public:
  explicit HideViewAnimationObserver(views::View* view) : view_(view) {}
...
void LoginAuthUserView::ApplyAnimationPostLayout() {
  ...
  int non_pin_screen_y_end = non_pin_root_->GetBoundsInScreen().y();
  ...
  auto move_to_center = base::MakeUnique<ui::InterpolatedTranslation>(
      gfx::PointF(0, cached_animation_state_->non_pin_screen_y_start -
                         non_pin_screen_y_end),
      gfx::PointF());
  auto transition =
  ...
  if (cached_animation_state_->had_password != has_password) {
...

    {
      ...
      if (!has_password)
        settings.AddObserver(new HideViewAnimationObserver(password_view_));
