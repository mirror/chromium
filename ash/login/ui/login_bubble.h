...
class ASH_EXPORT LoginBubble : public views::WidgetObserver,
 ...
 public:
  ...
  // Create an error bubble for authentication failure.
  static LoginBubble* CreateErrorBubble();

  // Create a user menu bubble.
  // |button_view| is the button that could open/close the bubble.
  // |show_remove_user| indicate whether or not we show the
  // "Remove this user" action.
  static LoginBubble* CreateUserMenu(views::View* button_view,
                                     bool show_remove_user);

  LoginBubble(LoginBubbleType type,
 ...
 private:
  ...
  // Used in user menu bubble to indicate whether or not we show the
  // "Remove this user" action.
  bool show_remove_user_ = false;
