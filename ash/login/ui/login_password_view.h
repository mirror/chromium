...
class ASH_EXPORT LoginPasswordView : public views::View,
 ...
 public:
  ...
  using OnPasswordFieldCleared = base::RepeatingCallback<void(bool)>;

  // |on_submit| is called when the user hits enter or pressed the submit arrow.
  // Must not be null.
  explicit LoginPasswordView(
      const OnPasswordSubmit& on_submit,
      const OnPasswordFieldCleared& on_password_field_changed);
 ...
 private:
  ...
  OnPasswordFieldCleared on_password_field_cleared_;
