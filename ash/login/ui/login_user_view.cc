...
void LoginUserView::UpdateForUser(const mojom::UserInfoPtr& user,
  ...
  if (animate) {
    ...
    // Stop any running any existing animations.
    ...
    auto make_opacity_element = [](float opacity) {
      auto element = ui::LayerAnimationElement::CreateOpacityElement(
          opacity, base::TimeDelta::FromMilliseconds(
                       kUserChangeAnimationDurationMs / 2.0f));
      element->set_tween_type(gfx::Tween::Type::EASE_OUT);
      return element;
    };
    auto* label_sequence = new ui::LayerAnimationSequence();
    label_sequence->AddElement(make_opacity_element(0 /*opacity*/));
    label_sequence->AddElement(make_opacity_element(1 /*opacity*/));
