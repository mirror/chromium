...
void LoginUserView::ButtonPressed(Button* sender, const ui::Event& event) {
  ...
  if (user_dropdown_ && sender == user_dropdown_) {
    if (!user_menu_ || !user_menu_->bubble_view()) {
      user_menu_ = base::MakeUnique<LoginBubble>(
          LoginBubble::LoginBubbleType::kUserMenu, user_dropdown_);
      // TODO: Use mojom::LoginUserInfo for current_user_ which has
      // is_device_owner information.
      user_menu_->Show(base::ASCIIToUTF16(current_user_->display_name),
                       base::ASCIIToUTF16(current_user_->display_email),
                       user_dropdown_);
    } else {
      user_menu_->Close();
    }
