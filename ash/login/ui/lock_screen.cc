...
void LockScreen::Show() {
  ...
  for (auto* window : Shell::GetAllRootWindows()) {
    ...
    instance_->initial_blur_.insert(std::make_pair(layer, layer->layer_blur()));
  }
...
void LockScreen::Destroy() {
  ...
  for (auto* window : Shell::GetAllRootWindows()) {
    ui::Layer* layer = GetWallpaperLayerForWindow(window);
    auto iter = initial_blur_.find(layer);
    if (iter != initial_blur_.end())
      layer->SetLayerBlur(iter->second);
  }
  window_->Close();
  delete instance_;
  instance_ = nullptr;
