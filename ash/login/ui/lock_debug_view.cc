...
class LockDebugView::DebugDataDispatcherTransformer
 ...
 public:
  ...
  void CycleEasyUnlockForUserIndex(size_t user_index) {
    ...
    icon->id = debug_user->easy_unlock_id;
    if (icon->id == mojom::EasyUnlockIconId::SPINNER) {
      icon->aria_label = base::ASCIIToUTF16("Icon is spinning");
    } else if (icon->id == mojom::EasyUnlockIconId::LOCKED ||
               icon->id == mojom::EasyUnlockIconId::LOCKED_TO_BE_ACTIVATED) {
      ...
      icon->tooltip = base::ASCIIToUTF16(
          ...
          std::to_string(static_cast<int>(icon->id)));
    } else {
      icon->tooltip =
