...
// Applies a series of user-defined transformations to a |LoginDataSource|
...
class LockDebugView::DebugDataSourceTransformer
    : public LoginDataSource::Observer {
 public:
  DebugDataSourceTransformer(LoginDataSource* source) : root_source_(source) {
    root_source_->AddObserver(this);
  }
  ~DebugDataSourceTransformer() override { root_source_->RemoveObserver(this); }

  LoginDataSource* debug_source() { return &debug_source_; }
  ...
  void SetUserCount(int count) {
    ...
    debug_source_.NotifyUsers(users);
  }
  ...
  void TogglePinStateForUserIndex(size_t user_index) {
    ...
    debug_source_.SetPinEnabledForUser(debug_user->account_id,
  ...
  void OnUsersChanged(
    ...
    // Rebuild debug users; this will notify |debug_users_|.
  ...
  void SetPinEnabledForUser(const AccountId& user, bool enabled) override {
    ...
    for (size_t i = 0u; i < debug_users_.size(); ++i) {
      if (debug_users_[i].account_id == user) {
        ...
        debug_source_.SetPinEnabledForUser(user, enabled);
        break;
      }
    }
  }
 ...
 private:
  // The debug overlay UI takes ground-truth data from |root_source_|, applies a
  // series of transformations to it, and exposes it to the UI via
  // |debug_source_|.
  LoginDataSource* root_source_;  // Unowned.
  LoginDataSource debug_source_;

  std::vector<ash::mojom::UserInfoPtr> root_users_;

  std::vector<UserMetadata> debug_users_;

  DISALLOW_COPY_AND_ASSIGN(DebugDataSourceTransformer);
};

LockDebugView::LockDebugView(LoginDataSource* data_source)
    : debug_data_source_(
          base::MakeUnique<DebugDataSourceTransformer>(data_source)) {
  SetLayoutManager(new views::BoxLayout(views::BoxLayout::kHorizontal));

  lock_ = new LockContentsView(debug_data_source_->debug_source());
  AddChildView(lock_);
...
void LockDebugView::ButtonPressed(views::Button* sender,
  ...
  if (sender == add_user_ || sender == remove_user_) {
    ...
    if (num_users_ < 1)
      num_users_ = 1;
    debug_data_source_->SetUserCount(num_users_);
    RebuildUserColumn();
    Layout();
  }
  ...
  for (size_t i = 0u; i < user_column_entries_toggle_pin_.size(); ++i) {
    if (user_column_entries_toggle_pin_[i] == sender)
      debug_data_source_->TogglePinStateForUserIndex(i);
  }
}

void LockDebugView::RebuildUserColumn() {
