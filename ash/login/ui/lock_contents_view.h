...
class ASH_EXPORT LockContentsView : public views::View,
 ...
 private:
  ...
  // Check to see if the current auth user has been changed
  // and update |current_auth_account_id_| accordingly.
  // |account_id|: account_id of the current auth user.
  void MaybeUpdateCurrentAuthUser(AccountId account_id);
  ...
  AccountId current_auth_account_id_;
