...
std::unique_ptr<AshWindowTreeHost> ShellPortMash::CreateAshWindowTreeHost(
  ...
  display_params->software_mirroring_display_list =
      display_manager->software_mirroring_display_list();
  
  // DCHECK_EQ(init_params.display_id == display::DisplayManager::kUnifiedDisplayId || mirrored_display.is_valid(),
  //           !display_params->software_mirroring_display_list.empty())
  LOG(ERROR) << "MSW ShellPortMash::CreateAshWindowTreeHost " << display_manager->software_mirroring_display_list().size() << " / " << display_params->software_mirroring_display_list.size();
