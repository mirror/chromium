...
TEST_F(WindowManagerTest, IgnoreCanConsumeSystemKeysFromNonContentBrowser) {
  // The ability to consume system keys is powerful, and we don't want to allow
  // it from anything other than a browser process.
  std::map<std::string, std::vector<uint8_t>> properties;
  ...
  aura::Window* window = window_manager_delegate->OnWmCreateTopLevelWindow(
      ui::mojom::WindowType::WINDOW, "some_other_component", &properties);

  EXPECT_EQ(false, window->GetProperty(kCanConsumeSystemKeysKey));
}
...
TEST_F(WindowManagerTest, CanConsumeSystemKeysFromContentBrowser) {
  ...
  aura::WindowManagerDelegate* window_manager_delegate =
      ash_test_helper()->window_manager_app()->window_manager();
  aura::Window* window = window_manager_delegate->OnWmCreateTopLevelWindow(
      ui::mojom::WindowType::WINDOW, "content_browser", &properties);
