...
AshWindowTreeHostMirroringUnified::GetRootTransformForLocalEventCoordinates()
  ...
  // The mirroring display may no longer exist in the TearDown of ash tests.
  // This is because this host's deletion is defered (see
  // MirrorWindowController::CloseAndDeleteHost()), and before it's actually
  // deleted, a synthesized mouse event maybe received, but by then
  // corresponding mirroring display is removed.
  if (display) {
