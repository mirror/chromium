...
class UnifiedEventTargeter : public aura::WindowTargeter {
 public:
  ...
  ui::EventTarget* FindTargetForEvent(ui::EventTarget* root,
    ...
    LOG(ERROR) << "MSW UnifiedEventTargeter::FindTargetForEvent A root:" << root << " src_root_:" << src_root_ << " event->target():" << event->target(); 
    static_cast<aura::Window*>(root)->PrintWindowHierarchy(3);
    src_root_->PrintWindowHierarchy(3);
    // if (root == src_root_ && !event->target()) {
      ...
      if (event->IsLocatedEvent()) {
        ...
        located_event->ConvertLocationToTarget(
            static_cast<aura::Window*>(root), dst_root_); 
      }
      return dst_root_; 
      // ignore_result(
