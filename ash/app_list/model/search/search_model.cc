...
void SearchModel::PublishResults(const std::vector<SearchResult>& new_results) {
  ...
  std::map<std::string, std::unique_ptr<SearchResult>> results__map;
  for (auto& ui_result : current_results)
    results__map[ui_result->id()] = std::move(ui_result);

  // Add items back to |results_| in the order of |new_results|.
  for (const SearchResult& new_result : new_results) {
    auto ui_result_it = results__map.find(new_result.id());
    if (ui_result_it != results__map.end() &&
      ...
      // results.
      results__map.erase(ui_result_it);
    } else {
      std::unique_ptr<SearchResult> result_copy = new_result.Duplicate();
      result_copy->set_relevance(new_result.relevance());
      // Copy the result from |new_results| otherwise.
      results_->Add(std::move(result_copy));
    }
  }

  // Any remaining results in |results__map| will be automatically deleted.
}
