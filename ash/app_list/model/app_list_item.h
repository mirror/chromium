...
class APP_LIST_MODEL_EXPORT AppListItem {
 public:
  ...
  void SetMetadata(const ash::mojom::AppListItemMetadataPtr& metadata) {
    metadata_->name = metadata->name;
    metadata_->folder_id = metadata->folder_id;
    metadata_->position = metadata->position;
    metadata_->is_folder = metadata->is_folder;
  }
  const ash::mojom::AppListItemMetadataPtr& GetMetadata() { return metadata_; }
