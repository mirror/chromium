...
class ASH_PUBLIC_EXPORT ShelfModel {
 ...
 private:
  ...
  void UpdateItemAndNotifyObservers(const std::string& app_id);

  // Sets ShelfItem.has_active_notification if the ShelfItem has an active
  // notification.
  void UpdateItemNotificationByIndex(const int index);

  // Adds a record to |app_id_to_notification_id_| and
  // |notification_id_to_app_id_|.
  void AddToNotificationMaps(const std::string& app_id,
                             const std::string& notification_id);
  // Removes the record from |app_id_to_notification_id_| and
  // |notification_id_to_app_id_| and modifies |app_id| if the notification has
  // a matching app id.
  void RemoveFromNotificationMaps(const std::string& notification_id,
                                  std::string& app_id);
