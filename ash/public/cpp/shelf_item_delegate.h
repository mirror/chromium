...
class ASH_PUBLIC_EXPORT ShelfItemDelegate : public mojom::ShelfItemDelegate {
 public:
  ...
  // Runs |callback| to show the item's context menu for certain |event| values.
  // Returns true if the callback argument was run, and thus is no longer valid.
  bool MaybeShowContextMenu(const ui::Event* event,
                            int64_t display_id,
