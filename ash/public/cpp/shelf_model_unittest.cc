...
TEST_F(ShelfModelTest, AddRemoveNotification) {
  ...
  EXPECT_FALSE(model_->items()[2].has_active_notification);
  EXPECT_EQ(std::string(), model_->items()[2].notification_id);

  // Add a notification for the app.
  model_->GiveItemNotifierId(app_id, notification_id);

  EXPECT_TRUE(model_->items()[2].has_active_notification);
  EXPECT_EQ(notification_id, model_->items()[2].notification_id);
...
TEST_F(ShelfModelTest, RemoveNotificationThatDoesntExist) {
  const std::string app_id("app_id");
  const std::string notification_id("notification_id");
