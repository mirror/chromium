...
MenuItemList GetMenuItemsForMojo(ui::MenuModel* model) {
  MenuItemList items;
  for (int i = 0; model && i < model->GetItemCount(); ++i) {
...
bool ShelfItemDelegate::MaybeShowContextMenu(const ui::Event* event,
                                             int64_t display_id,
                                             ItemSelectedCallback* callback) {
  // This only handles ShelfView::ShowContextMenuForView's synthetic events.
  if (event && event->type() == ui::ET_POINTER_DOWN &&
      event->flags() == ui::EF_RIGHT_MOUSE_BUTTON) {
    context_menu_ = GetContextMenu(display_id);
    // Caution: This invalidates the callback held by the caller!
