...
void WindowManager::OnPointerEventObserved(const ui::PointerEvent& event,
  ...
  // |target| first. WindowManager's WindowTree is the entire root tree and we
  // must adjust |target| from being the root to the aura::Window which would
  // handle |event|.
  //
  // TODO(erg): Should this really live in WindowTreeClient instead?
