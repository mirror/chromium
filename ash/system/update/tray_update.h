...
class ASH_EXPORT TrayUpdate : public TrayImageItem {
 public:
  ...
  // If |show| is true, shows an icon in the system tray which indicates that
  // a software update is available but user's agreement is required as current
  // connection is cellular. If |show| is false, hides the icon because the
  // user's one time permission on update over cellular connection has been set
  // successfully in Update Engine.
  void ShowUpdateOverCellularAvailableIcon(bool show);
