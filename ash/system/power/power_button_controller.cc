...
void PowerButtonController::PowerButtonEventReceived(
  ...
  // stop forcing the display off if neither ConvertiblePowerButtonController
  // nor TabletPowerButtonController is being used.
  ...
  // Handle tablet power button behavior. Should check tablet mode here for
  // detachable devices.
...
void PowerButtonController::OnAccelerometerUpdated(
  ...
  if (!enable_tablet_mode_)
    return;
  if (!force_clamshell_power_button_ && !is_tablet_or_detachable_ &&
...
void PowerButtonController::ProcessCommandLine() {
  ...
  is_tablet_or_detachable_ =
      cl->HasSwitch(switches::kIsTabletOrDetachableDevice);
}
