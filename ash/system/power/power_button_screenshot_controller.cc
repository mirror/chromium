  ...
  if (key_code == ui::VKEY_VOLUME_DOWN) {
    if (event->type() == ui::ET_KEY_PRESSED) {
      if (!volume_down_key_pressed_) {
        ...
        // Do not propagate current volume down key pressed event if it is
        // consumed by screenshot.
        if (InterceptScreenshotChord())
          event->StopPropagation();
      }

      // Do not propagate repeated volume down key pressed event if the first
