...
TEST_F(PowerButtonScreenshotControllerTest,
  ...
  PressKey(ui::VKEY_VOLUME_DOWN);
  EXPECT_EQ(0, test_observer_->volume_down_pressed_propagated_count());
  // Advances |tick_clock_| to off screenshot chord point. This will also trigger
  // volume down timer timeout. Verifies there is a volume down operation.
  tick_clock_->Advance(PowerButtonScreenshotController::kScreenshotChordDelay +
                       base::TimeDelta::FromMilliseconds(1));
...
class ClamshellPowerButtonScreenshotControllerTest : PowerButtonScreenshotControllerTest {
 public:
