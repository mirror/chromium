...
class PowerButtonScreenshotControllerTest
 ...
 public:
  ...
  void SetUp() override {
    ...
    test_with_forced_clamshell_ = GetParam();
    if (test_with_forced_clamshell_) {
 ...
 protected:
  ...
  bool test_with_forced_clamshell_ = false;

  DISALLOW_COPY_AND_ASSIGN(PowerButtonScreenshotControllerTest);
};

// Instantiate the Boolean which is used to toggle between tablet-power-button
// and forced-clamshell-power-button in the parameterized tests.
INSTANTIATE_TEST_CASE_P(, PowerButtonScreenshotControllerTest, testing::Bool());

// Tests power button screenshot accelerator works on tablet mode only.
TEST_P(PowerButtonScreenshotControllerTest, TabletMode) {
...
TEST_P(PowerButtonScreenshotControllerTest, ReleaseBeforeAnotherPressed) {
...
TEST_P(PowerButtonScreenshotControllerTest,
...
// Tests power button pressed first for waiting volume down key, and it
// timeouts.
TEST_F(ClamshellPowerButtonScreenshotControllerTest,
       PowerButtonPressedFirst_WaitForVolumeDownKey) {
...
// Tests power button pressed first and then volume key pressed.
TEST_F(ClamshellPowerButtonScreenshotControllerTest,
