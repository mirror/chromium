...
class ASH_EXPORT PowerButtonScreenshotController : public ui::EventHandler {
 public:
  // Observes volume down key event's stopped propagation. Implemented and used
  // in tests only.
  class TestObserver {
   public:
    TestObserver() {}
    ...
    // Called in PowerButtonScreenshotController when volume down key event is
    // stopped propagation.
    virtual void OnStoppedPropagation() {}
 ...
 private:
  ...
  // pressed on tablet mode. Stopped when volume key is pressed. Runs
  // OnClamshellTimeout to start clamshell power button behavior.
