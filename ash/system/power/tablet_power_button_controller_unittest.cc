...
// Tests that attach the detached detachable device.
TEST_F(TabletPowerButtonControllerShowMenuTest, AttachTheDetachableDevice) {
  // Long press the power button to show the menu.
  PressPowerButton();
  EXPECT_TRUE(tablet_test_api_->TriggerPowerButtonMenuTimeout());
  ReleasePowerButton();
  ...
  // TODO, should check the logic here again.
  // Attached detachable device should perform as a clamshell device.
  EXPECT_TRUE(tablet_controller_);
  // EXPECT_FALSE(convertible_controller_);
  PressPowerButton();
  EXPECT_FALSE(tablet_test_api_->PowerButtonMenuTimerIsRunning());
  EXPECT_FALSE(tablet_test_api_->TriggerPowerButtonMenuTimeout());
  ReleasePowerButton();
  EXPECT_FALSE(tablet_test_api_->IsMenuOpened());
}
