...
class PowerButtonTestBase : public AshTestBase {
 public:
  ...
  static constexpr int kNonZeroBrightness = 10;
 ...
 protected:
  ...
  // Advance clock to ensure the intended convertible/tablet power button
  // display forcing off is not ignored since we will ignore the repeated power
  // button up if they come too close.
  ...
  // Indicates whether switches::kIsTabletOrDetachableDevice is appended.
  bool has_tablet_switch_ = false;

  DISALLOW_COPY_AND_ASSIGN(PowerButtonTestBase);
};

// Base test fixture same as PowerButtonTestBase except that convertible swtich
// is not set.
