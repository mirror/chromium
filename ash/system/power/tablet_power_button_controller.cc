...
// to be ignored.
constexpr base::TimeDelta kScreenStateChangeDelay =
    base::TimeDelta::FromMilliseconds(500);
...
void TabletPowerButtonController::OnPowerButtonEvent(
  ...
  if (down) {
    ...
    // We may have the state that powerd tells us that screen state is on
    // however the actual display is still off. Avoid forcing off during this
    // interval otherwise user may experience screen turned on and then
