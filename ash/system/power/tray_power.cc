...
bool TrayPower::MaybeShowUsbChargerNotification() {
  ...
  // We normally show the notification if a USB charger is connected but the
  // battery isn't full (since some ECs may choose to use a lower power rail
  // when the battery is full even when a high-power charger is connected).
