...
class BatteryImageSource : public gfx::CanvasImageSource {
 public:
  ...
  void Draw(gfx::Canvas* canvas) override {
    ...
    // shown charged. The exception is when |charge_state| is 0 (a
    // critically-low battery); in this case, still draw 1dip of charge.
    SkRect icon_bounds = path.getBounds();
    float charge_level = info_.charge_percent / 100.0 * icon_bounds.height();
    charge_level = std::max(std::min(charge_level, icon_bounds.height()), dsf);
    ...
    const bool use_alert_color =
        charge_level == kMinVisualChargeLevel && info_.alert;
...
bool PowerStatus::BatteryImageInfo::ApproximatelyEqual(
  ...
  return icon_badge == o.icon_badge && o.alert == o.alert &&
         std::abs(charge_percent - o.charge_percent) < 0.05;
}
...
void PowerStatus::CalculateBatteryImageInfo(BatteryImageInfo* info) const {
  info->alert = IsLinePowerConnected();
