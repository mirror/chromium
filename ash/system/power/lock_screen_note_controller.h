...
// Class used to controll lock screen note state depending on display state set
// by power button dispplay controller.
class LockScreenNoteController {
 public:
  explicit LockScreenNoteController(base::TickClock* tick_clock);
  ~LockScreenNoteController();

  // If lock screen note is available, it requests a new lock screen note
  // due to stylus eject.
  ...
  // handled when the note launch finishes).
  //
  // Calling this while previous state change is still pending, the callback
  // associated with the previous display state change will not be run.
