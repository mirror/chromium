...
class TrayEventFilterTest : public test::AshTestBase {
 public:
  ...
  void SetUp() override {
    ...
    status_area_widget_ = StatusAreaWidgetTestHelper::GetStatusAreaWidget();
  }
  ...
  ui::PointerEvent outside_event() {
    ...
    EXPECT_FALSE(
        status_area_widget_->system_tray()->GetBoundsInScreen().Contains(
            outside_point()));
    return ui::PointerEvent(ui::MouseEvent(
        ui::ET_MOUSE_PRESSED, outside_point(), outside_point(), time, 0, 0));
  }

  gfx::Point inside_point() { return gfx::Point(750, 570); }
...
TEST_F(TrayEventFilterTest, ClickingOutsideCloseBubble) {
  SystemTray* tray = status_area_widget_->system_tray();
