...
constexpr int kBubbleNotTopPaddingDp = 16;
...
// The location of the arrow on the bubble in ltr, expressed as a ratio of the
// bubble's width.
constexpr float kArrowLocationRatioLtr = 0.75f;
...
class PaletteWelcomeBubble::CustomBubbleBorder : public views::BubbleBorder {
 public:
  ...
  gfx::Rect GetBounds(const gfx::Rect& anchor_rect,
    ...
    const float arrow_location_ratio = base::i18n::IsRTL()
                                           ? 1 - kArrowLocationRatioLtr
                                           : kArrowLocationRatioLtr;
    gfx::Rect bounds(contents_size);
    bounds.set_x(anchor_rect.CenterPoint().x() -
                 arrow_location_ratio * contents_size.width());
  ...
  gfx::Rect GetArrowRect(const gfx::Rect& bounds) const override {
    ...
    rect.set_x(arrow_location_ratio * bounds.width() -
 ...
 public:
  explicit WelcomeBubbleView(views::View* anchor)
    ...
    auto* layout = new views::BoxLayout(
        ...
        gfx::Insets(kBubbleTopPaddingDp, kBubbleNotTopPaddingDp,
                    kBubbleNotTopPaddingDp, kBubbleNotTopPaddingDp),
    ...
    close_button_->SetPreferredSize(kCloseButtonPreferredSizeDp);
