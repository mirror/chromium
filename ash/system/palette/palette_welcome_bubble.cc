...
class PaletteWelcomeBubble::WelcomeBubbleView
 ...
 public:
  WelcomeBubbleView(views::View* anchor, views::BubbleBorder::Arrow arrow)
    ...
    const gfx::FontList& default_font_list = views::Label::GetDefaultFontList();
    title->SetFontList(default_font_list.Derive(0, gfx::Font::FontStyle::NORMAL,
  ...
  void OnPointerEventObserved(const ui::PointerEvent& event,
    ...
    // Close the bubble if a pointer event is observed outside of the bubble.
    if (!GetBoundsInScreen().Contains(location_in_screen))
      GetWidget()->Close();
  }
...
// static
void PaletteWelcomeBubble::RegisterLocalStatePrefs(
...
void PaletteWelcomeBubble::OnLocalStatePrefServiceInitialized(
...
views::ImageButton* PaletteWelcomeBubble::GetCloseButtonForTest() {
  if (bubble_view_)
    return static_cast<WelcomeBubbleView*>(bubble_view_)->close_button();
