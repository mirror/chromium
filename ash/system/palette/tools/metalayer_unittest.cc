    ...
    for (int enabled = 0; enabled <= 1; enabled++) {
      for (int context = 0; context <= 1; context++) {
        ...
        Shell::Get()
            ...
            ->NotifyVoiceInteractionStatusChanged(state);
        Shell::Get()
            ->voice_interaction_controller()
            ->NotifyVoiceInteractionSettingsEnabled(enabled);
        Shell::Get()
            ->voice_interaction_controller()
            ->NotifyVoiceInteractionContextEnabled(context);
...
TEST_F(MetalayerToolTest, MetalayerUnsupportedDisablesPaletteTool) {
  Shell::Get()
      ->voice_interaction_controller()
      ->NotifyVoiceInteractionStatusChanged(VoiceInteractionState::RUNNING);
  Shell::Get()
      ->voice_interaction_controller()
      ->NotifyVoiceInteractionSettingsEnabled(true);
  Shell::Get()
      ->voice_interaction_controller()
      ->NotifyVoiceInteractionContextEnabled(true);
  ...
  Shell::Get()
      ->voice_interaction_controller()
      ->NotifyVoiceInteractionContextEnabled(false);
  testing::Mock::VerifyAndClearExpectations(palette_tool_delegate_.get());
  Shell::Get()
      ->voice_interaction_controller()
      ->NotifyVoiceInteractionContextEnabled(true);
  ...
  Shell::Get()
      ->voice_interaction_controller()
      ->NotifyVoiceInteractionStatusChanged(VoiceInteractionState::STOPPED);
  Shell::Get()
      ->voice_interaction_controller()
      ->NotifyVoiceInteractionStatusChanged(VoiceInteractionState::RUNNING);
  testing::Mock::VerifyAndClearExpectations(palette_tool_delegate_.get());
  ...
  Shell::Get()
      ->voice_interaction_controller()
      ->NotifyVoiceInteractionStatusChanged(VoiceInteractionState::NOT_READY);
  testing::Mock::VerifyAndClearExpectations(palette_tool_delegate_.get());
}
