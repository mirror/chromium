...
class PaletteTrayTest : public test::AshTestBase {
 public:
  ...
  void SetUp() override {
    ...
    palette_tray_test_api_ =
        base::MakeUnique<test::PaletteTrayTestApi>(palette_tray_);
 ...
 protected:
  ...
  std::unique_ptr<test::PaletteTrayTestApi> palette_tray_test_api_;
...
TEST_F(PaletteTrayTest, PaletteTrayWorkflow) {
  ...
  EXPECT_FALSE(palette_tray_test_api_->GetTrayBubbleWrapper());
  ...
  EXPECT_TRUE(palette_tray_test_api_->GetTrayBubbleWrapper());

  // Verify that activating a mode tool will close the palette tray bubble, but
  // leave the palette tray button active.
  palette_tray_test_api_->GetPaletteToolManager()->ActivateTool(
      PaletteToolId::LASER_POINTER);
  EXPECT_TRUE(palette_tray_test_api_->GetPaletteToolManager()->IsToolActive(
      PaletteToolId::LASER_POINTER));
  EXPECT_TRUE(palette_tray_->is_active());
  EXPECT_FALSE(palette_tray_test_api_->GetTrayBubbleWrapper());
  ...
  PerformTap();
  EXPECT_FALSE(palette_tray_->is_active());
  EXPECT_FALSE(palette_tray_test_api_->GetPaletteToolManager()->IsToolActive(
      PaletteToolId::LASER_POINTER));

  // Verify that activating a action tool will close the palette tray bubble and
  // the palette tray button is will not be active.
  PerformTap();
  ASSERT_TRUE(palette_tray_test_api_->GetTrayBubbleWrapper());
  palette_tray_test_api_->GetPaletteToolManager()->ActivateTool(
