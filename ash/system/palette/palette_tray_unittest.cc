...
TEST_F(PaletteTrayTest, WelcomeBubbleVisibility) {
  ...
  // Verify that the welcome bubble remains open if it is tapped.
  generator.set_current_location(
      test_api_->welcome_bubble()->GetBubbleBounds()->CenterPoint());
  generator.PressTouch();
  generator.ReleaseTouch();
  EXPECT_TRUE(test_api_->welcome_bubble()->BubbleShown());

  // Verify that the welcome bubble disappears after tapping outside of its
  // bounds.
  ASSERT_FALSE(
      test_api_->welcome_bubble()->GetBubbleBounds()->Contains(gfx::Point()));
