...
TEST_F(PaletteTrayTestWithInternalStylus, WelcomeBubbleShownOnEject) {
  ...
  EXPECT_TRUE(test_api_->GetWelcomeBubble());
  ...
  // Verify the pref gets set after the bubble is hidden.
  test_api_->GetWelcomeBubble()->Hide();
  EXPECT_TRUE(pref_service()->GetBoolean(prefs::kShownPaletteWelcomeBubble));
}
...
TEST_F(PaletteTrayTestWithInternalStylus, WelcomeBubbleNotShownIfShownBefore) {
  ...
  EXPECT_FALSE(test_api_->GetWelcomeBubble());
