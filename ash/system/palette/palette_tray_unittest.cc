...
class PaletteTrayTestWithVoiceInteraction : public PaletteTrayTest {
 ...
 protected:
  ...
  void WaitOut(const base::TimeDelta& interval) {
    ...
    // ui::EventGenerator installs a fake timer when instantiating.
    // This timer increments the tick counter by 1 with each call to
    // ui::EventTimeForNow(), which makes "busy waiting" very fast.
