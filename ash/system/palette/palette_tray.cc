...
// static
void PaletteTray::RegisterPrefs(PrefRegistrySimple* registry) {
...
void PaletteTray::Initialize() {
  ...
  // |local_state_pref_service_| may be null in mash.
  if (!local_state_pref_service_)
    return;

  // If a device has a internal stylus, mark the has seen stylus flag as true
...
void PaletteTray::OnHasSeenStylusPrefChanged() {
  ...
  // events, but only if the device is compatiable with styli.
  if (has_seen_stylus_)
    watcher_.reset();
  else if (palette_utils::HasStylusInput())
