...
void PaletteTray::Initialize() {
  ...
  // If the device does not support styli, or has an internal stylus, set the
  // has stylus pref as true.
  if (!palette_utils::HasStylusInput() || palette_utils::HasInternalStylus()) {
...
void PaletteTray::OnSeenStylusPrefChanged(bool seen_stylus) {
  ...
  // On reading the pref, start monitoring stylus events if the device has never
  // seen a stylus event, otherwise do not bother monitoring.
  if (seen_stylus)
    watcher_.reset();
  else
