...
void PaletteTray::BubbleViewDestroyed() {
  ...
  // The tray button remains active if the current active tool is a mode. When
  // using the accelerator, the bubble actually gets closed before it is opened
  // so there is a chance |bubble_| is not nullptr here, due to the closing of
  // the bubble widget being async.
