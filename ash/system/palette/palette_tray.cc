...
void PaletteTray::BubbleViewDestroyed() {
  ...
  // When using the accelerator, the bubble actually gets closed before it is
  // opened so there is a chance |bubble_| is not nullptr here, due to the
  // closing of the bubble widget being async. If |bubble_| exists, activate
  // the tray button, otherwise deactivate it unless there is a tool active
  // while the bubble is not shown.
  SetIsActive(bubble_ != nullptr ||
