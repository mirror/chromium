...
bool PaletteTray::ShouldShowPalette() const {
  return is_palette_enabled_ && palette_utils::HasStylusInput() &&
...
void PaletteTray::OnLocalStatePrefServiceInitialized(
  ...
  pref_change_registrar_local_ = base::MakeUnique<PrefChangeRegistrar>();
...
void PaletteTray::OnActiveUserPrefServiceChanged(PrefService* prefs) {
  pref_change_registrar_user_.reset();
  if (!prefs)  // Null during startup, user switch and tests.
    return;
  pref_change_registrar_user_ = base::MakeUnique<PrefChangeRegistrar>();
