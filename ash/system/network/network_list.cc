...
class MobileHeaderRowView : public NetworkListView::SectionHeaderRowView,
 ...
 protected:
  ...
  void OnToggleToggled(bool is_on) override {
    ...
    // 'Get data conneciton' in the Settings > Mobile data subpage).
    ...
    // (e.g. bluetooth was disabl from a different UI).
  ...
  void UpdateState() {
    ...
    if (cellular_state != NetworkStateHandler::TECHNOLOGY_UNAVAILABLE) {
      ...
      if (!cellular_device ||
        ...
        if (!mobile_network || mobile_network->IsDefaultCellular()) {
          // If a mobile network is not available, show 'turn on bluetooth' if
          // Tether is not initialized, otherwise show 'no networks'.
          if (tether_state == NetworkStateHandler::TECHNOLOGY_UNINITIALIZED)
    ...
    if (enable_bluetooth_timer_.IsRunning()) {
      enable_bluetooth_timer_.Stop();
      waiting_for_tether_initialize_ = false;
      if (!tether_enabled) {
        tether_enabled = true;  // assume success
        subtitle = IDS_ASH_STATUS_TRAY_INITIALIZING_CELLULAR;
        network_state_handler_->SetTechnologyEnabled(
    ...
    if (!subtitle && !network_state_handler_->FirstNetworkByType(
                         NetworkTypePattern::Tether())) {
 ...
 private:
  ...
  bool waiting_for_tether_initialize_;
