...
class MobileHeaderRowView : public NetworkListView::SectionHeaderRowView,
 ...
 public:
  MobileHeaderRowView(NetworkStateHandler* network_state_handler)
      : SectionHeaderRowView(IDS_ASH_STATUS_TRAY_NETWORK_MOBILE),
        handler_(network_state_handler),
        weak_ptr_factory_(this) {
    handler_->AddObserver(this, FROM_HERE);
  }

  ~MobileHeaderRowView() override { handler_->RemoveObserver(this, FROM_HERE); }
 ...
 protected:
  ...
  void OnToggleToggled(bool is_on) override {
    ...
    // of Cellular.
    if (handler_->IsTechnologyAvailable(NetworkTypePattern::Cellular())) {
      handler_->SetTechnologyEnabled(
    ...
    // Tether.
    DCHECK(handler_->IsTechnologyAvailable(NetworkTypePattern::Tether()));
    ...
    if (handler_->IsTechnologyUninitialized(NetworkTypePattern::Tether())) {
      DCHECK(is_on);

      if (adapter_ && status_ != Status::ENABLING_BLUETOOTH_ADAPTER) {
        EnableBlueooth();
      } else if (!adapter_ && status_ != Status::FETCHING_BLUETOOTH_ADAPTER) {
    ...
    handler_->SetTechnologyEnabled(NetworkTypePattern::Tether(), is_on,
                                   chromeos::network_handler::ErrorCallback());
  }
  ...
  void DeviceListChanged() override {
    ...
    if (!handler_->IsTechnologyAvailable(NetworkTypePattern::Tether())) {
      // If Tether has become unavailable, it is disabled for some other reason
      // (e.g., the device could be in the process of being shut down). Stop
      // waiting for Tether to become available.
      status_ = Status::IDLE;
      SetIsOn(false);
      return;
    }
    ...
    status_ = Status::IDLE;
    handler_->SetTechnologyEnabled(NetworkTypePattern::Tether(),
                                   true /* enabled */,
                                   chromeos::network_handler::ErrorCallback());
  }
 ...
 private:
  ...
  void EnableBlueooth() {
    ...
    adapter_->SetPowered(true /* powered */,
  ...
  void OnSetPoweredSuccess() {
    ...
    if (!handler_->IsTechnologyUninitialized(NetworkTypePattern::Tether())) {
      ...
      // missed, so invoke the associated callback explicitly.
      DeviceListChanged();
    }
  }
...
NetworkListView::UpdateNetworkListEntries() {
  ...
  if (handler->IsTechnologyAvailable(NetworkTypePattern::Cellular()) ||
    ...
    index = UpdateSectionHeaderRow(
        NetworkTypePattern::Mobile(), is_mobile_toggle_enabled, index,
