  ...
  void UpdateState() {
    ...
    if (tether_state == NetworkStateHandler::TECHNOLOGY_UNINITIALIZED) {
      if (waiting_for_tether_initialize_) {
        ...
        // "Initializing...". TODO(stevenjb): Rename the string to MOBILE.
    ...
    if (waiting_for_tether_initialize_) {
      ...
      if (!tether_enabled) {
        // We enabled Bluetooth so Tether is now initialized, but we still need
        // to enable it. We expect this to succeed.
        network_state_handler_->SetTechnologyEnabled(
            NetworkTypePattern::Tether(), true /* enabled */,
            chromeos::network_handler::ErrorCallback());
        SetToggleState(true /* toggle_enabled */, true /* is_on */);
        SetSubtitle(std::string());
        return;
      }
    }
    ...
    int subtitle = 0;
    if (!network_state_handler_->FirstNetworkByType(
            NetworkTypePattern::Tether())) {
