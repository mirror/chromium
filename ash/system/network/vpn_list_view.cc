...
void VPNListView::AddProvidersAndNetworks(
  ...
  // Create an local networkstate list. Help AddProviderAndNetworks() by passing
  // only subset of network states.
  chromeos::NetworkStateHandler::NetworkStateList networkstate_sub_list;

  // Add connected ARCVPN networks. These are not normally displayed in
  // the menu because the OS connects and disconnects them in response to
  // events from ARC. If we can find the correct provider, nest the network
  // under the proider. Otherwise list it unnested.
  for (const chromeos::NetworkState* const& network : networks) {
    if (network->vpn_provider_type() == shill::kProviderArcVpn &&
      ...
      for (auto arc_provider = arc_providers.begin();
        ...
        if (!VpnProviderMatchesNetwork(*arc_provider, *network))
          continue;
        networkstate_sub_list.push_back(network);
        AddProviderAndNetworks(*arc_provider, networkstate_sub_list);
        arc_providers.erase(arc_provider);
        break;
      }
      // Not matched provider found for connected network. Show it unnested.
      if (list_empty_)
        AddNetwork(network);
      break;
    }
  ...
  for (const VPNProvider& provider : providers)
    AddProviderAndNetworks(provider, networkstate_sub_list);

  // Add Arc VPN providers without any connected or connecting networks. Sort
  // the providers by last launch time.
