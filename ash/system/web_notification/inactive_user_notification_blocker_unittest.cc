...
TEST_F(InactiveUserNotificationBlockerTest, Basic) {
  ...
  ASSERT_DEATH(ShouldShowAsPopup(ash_system_notifier, kInvalidUserId), "");
...
TEST_F(InactiveUserNotificationBlockerTest,
       AshSystemNotificationsCannotHaveProfileId) {
  message_center::NotifierId ash_system_notifier(
      message_center::NotifierId::SYSTEM_COMPONENT,
      system_notifier::kNotifierDisplay);

  ASSERT_DEATH(ShouldShowAsPopup(ash_system_notifier, GetDefaultUserId()), "");
}
