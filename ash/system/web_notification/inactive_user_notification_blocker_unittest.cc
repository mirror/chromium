...
class InactiveUserNotificationBlockerTest
 ...
 protected:
  ...
  void CreateProfile(const std::string& email) {
...
TEST_F(InactiveUserNotificationBlockerTest, Basic) {
  ...
  EXPECT_TRUE(
      ShouldShowNotification(random_system_notifier, GetDefaultUserId()));

  CreateProfile("user1@tray");
  EXPECT_EQ(0, GetStateChangedCountAndReset());
  EXPECT_FALSE(ShouldShowNotificationAsPopup(notifier_id, ""));
  EXPECT_TRUE(ShouldShowNotificationAsPopup(ash_system_notifier, ""));
  EXPECT_FALSE(ShouldShowNotificationAsPopup(random_system_notifier, ""));
  EXPECT_TRUE(ShouldShowNotificationAsPopup(notifier_id, GetDefaultUserId()));
  EXPECT_FALSE(ShouldShowNotificationAsPopup(notifier_id, "user1@tray"));
  EXPECT_TRUE(ShouldShowNotificationAsPopup(random_system_notifier,
                                            GetDefaultUserId()));
  EXPECT_FALSE(
      ShouldShowNotificationAsPopup(random_system_notifier, "user1@tray"));
  EXPECT_FALSE(ShouldShowNotification(notifier_id, ""));
  EXPECT_TRUE(ShouldShowNotification(ash_system_notifier, ""));
  EXPECT_FALSE(ShouldShowNotification(random_system_notifier, ""));
  EXPECT_TRUE(ShouldShowNotification(notifier_id, GetDefaultUserId()));
  EXPECT_FALSE(ShouldShowNotification(notifier_id, "user1@tray"));
  EXPECT_TRUE(
      ShouldShowNotification(random_system_notifier, GetDefaultUserId()));
  EXPECT_FALSE(ShouldShowNotification(random_system_notifier, "user1@tray"));
  ...
  EXPECT_TRUE(ShouldShowNotificationAsPopup(notifier_id, "user1@tray"));
  EXPECT_FALSE(ShouldShowNotificationAsPopup(random_system_notifier,
                                             GetDefaultUserId()));
  EXPECT_TRUE(
      ShouldShowNotificationAsPopup(random_system_notifier, "user1@tray"));
  ...
  EXPECT_TRUE(ShouldShowNotification(notifier_id, "user1@tray"));
  EXPECT_FALSE(
      ShouldShowNotification(random_system_notifier, GetDefaultUserId()));
  EXPECT_TRUE(ShouldShowNotification(random_system_notifier, "user1@tray"));
