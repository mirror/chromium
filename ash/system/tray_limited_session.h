...
#ifndef ASH_SYSTEM_TRAY_LIMITED_SESSION_H_
#define ASH_SYSTEM_TRAY_LIMITED_SESSION_H_
...
class ASH_EXPORT TrayLimitedSession : public SystemTrayItem {
 public:
  enum LimitState { LIMIT_NONE, LIMIT_SET, LIMIT_EXPIRING_SOON };

  explicit TrayLimitedSession(SystemTray* system_tray);
  ~TrayLimitedSession() override;

 protected:
  virtual bool IsSessionStarted();

  virtual base::TimeDelta UpdateRemainingSessionTime();

  // Update state, notification and tray bubble view.
  void Update();
 ...
 private:
  ...
  // Update |remaining_session_time_| and update or reset current state depends
  // on |remaining_session_time_|.
  void UpdateState();
  ...
  DISALLOW_COPY_AND_ASSIGN(TrayLimitedSession);
};

}  // namespace ash

#endif  // ASH_SYSTEM_TRAY_LIMITED_SESSION_H_
