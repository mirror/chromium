...
class NightLightControllerDelegateImpl : public NightLightController::Delegate {
 ...
 private:
  base::Time GetSunRiseSet(bool sunrise) const {
    if (!ValidatePosition()) {
      LOG(ERROR) << "Invalid geoposition. Using default "
                 << (sunrise ? "6AM for sunrise." : "6PM for sunset.");
