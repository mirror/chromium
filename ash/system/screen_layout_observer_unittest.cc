...
TEST_F(ScreenLayoutObserverMultiMirroringTest,
  ...
  // because the layout get from the layout store by the updated display id list
  // sets mirror mode off by default. But the test is using different code path
  // and the mirror mode persists until it is manually turned off, so we
  // simulate to turn off mirror mode when removing the third display.
  CloseNotification();
  display_manager()->SetSoftwareMirroring(false);
  UpdateDisplay("400x400,300x300");
  EXPECT_EQ(l10n_util::GetStringUTF16(IDS_ASH_STATUS_TRAY_DISPLAY_MIRROR_EXIT),
  ...
  // Add a third display, the mirror mode is restored.
  CloseNotification();
  UpdateDisplay("400x400,300x300,200x200");
  EXPECT_TRUE(GetDisplayNotificationText().empty());
}
