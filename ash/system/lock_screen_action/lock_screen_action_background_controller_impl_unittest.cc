 ...
 public:
  ...
  void CompleteBackgroundAnimations() {
    LockScreenActionBackgroundView* contents_view =
        LockScreenActionBackgroundControllerImpl::TestApi(controller())
            .GetContentsView();
    ASSERT_TRUE(contents_view);

    views::View* background =
        LockScreenActionBackgroundView::TestApi(contents_view).GetBackground();
  ...
  views::Widget* GetBackgroundWidget() {
    return LockScreenActionBackgroundControllerImpl::TestApi(controller())
        .GetWidget();
  }
  ...
  bool InitializeControllerToState(LockScreenActionBackgroundState state) {
    ...
    if (!controller_.ShowBackground()) {
      ADD_FAILURE() << "Show request failed";
      return false;
    }

    observer_.ClearStateChanges();

    if (controller_.state() != LockScreenActionBackgroundState::kShowing) {
      ADD_FAILURE() << "Show request trasition to state "
    ...
    if (controller_.state() != LockScreenActionBackgroundState::kShown) {
      ADD_FAILURE() << "Invalid state upon show animation completion "
                    << static_cast<int>(controller_.state());
      return false;
    }
    ...
    if (!controller_.HideBackground()) {
      ADD_FAILURE() << "Hide background request failed";
      return false;
    }
    observer_.ClearStateChanges();

    if (controller_.state() != LockScreenActionBackgroundState::kHiding) {
      ADD_FAILURE() << "Invalid state upon hide background request "
                    << static_cast<int>(controller_.state());
      return false;
    }

    EXPECT_EQ(LockScreenActionBackgroundState::kHiding, state);
    EXPECT_TRUE(observer_.state_changes().empty());
    return true;
  }
...
TEST_F(LockScreenActionBackgroundControllerImplTest, ShowWhileHidding) {
  ASSERT_TRUE(
      InitializeControllerToState(LockScreenActionBackgroundState::kHiding));
