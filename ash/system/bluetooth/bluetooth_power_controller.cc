...
void BluetoothPowerController::ToggleBluetoothEnabled() {
  if (Shell::Get()->session_controller()->IsActiveUserSessionStarted()) {
    ...
    prefs->SetBoolean(prefs::kBluetoothAdapterEnabled,
                      !prefs->GetBoolean(prefs::kBluetoothAdapterEnabled));
  } else {
...
void BluetoothPowerController::OnActiveUserPrefServiceChanged(
  ...
  // Although we actually only need to listen to primary user pref changes,
  // we could also listen to the active user pref changes since non-primary user
  // is not able to change bluetooth pref.
  StartWatchingActiveUserPrefsChanges();

  if (!is_primary_user_bluetooth_applied_ &&
      Shell::Get()->session_controller()->IsUserWithGaiaAccount() &&
...
void BluetoothPowerController::ApplyBluetoothPrimaryUserPref() {
  ...
  // according the whatever the current bluetooth power is, except for
...
void BluetoothPowerController::ApplyBluetoothLocalStatePref() {
  ...
  if (prefs->FindPreference(prefs::kSystemBluetoothAdapterEnabled)
    ...
    // according the whatever the current bluetooth power is.
