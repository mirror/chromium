...
// If the remaining time falls below this threshold, the user should be
// informed that the session is about to expire.
const int kExpiringSoonThresholdInMinutes = 5;

// Use 500ms interval for updates to notification.
...
void TrayOffHoursSessionLimit::UpdateState() {
  ...
  if (off_hours_mode && !duration.is_zero()) {
    ...
    remaining_off_hours_time_ = std::max(
        duration - (base::Time::Now() - start_time), base::TimeDelta());

    off_hours_mode_ = remaining_off_hours_time_ <= expiring_soon_threshold
    ...
    remaining_off_hours_time_ = base::TimeDelta();
    off_hours_mode_ = OFF_HOURS_OFF;
    timer_.reset();
  }
}
...
void TrayOffHoursSessionLimit::UpdateNotification() {
  ...
  // For OFF_HOURS_OFF, there's nothing more to do.
  if (off_hours_mode_ == OFF_HOURS_OFF) {
