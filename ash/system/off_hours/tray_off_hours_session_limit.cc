...
TrayOffHoursSessionLimit::TrayOffHoursSessionLimit(SystemTray* system_tray)
    : SystemTrayItem(system_tray,
                     UMA_SESSION_LENGTH_LIMIT),  // UMA_OFF_HOURS_SESSION_LIMIT
...
void TrayOffHoursSessionLimit::OnSessionLengthLimitChanged() {
  Update();
}

void TrayOffHoursSessionLimit::OffHoursModeChanged() {
  LOG(ERROR) << "Daria: Off Hours Mode Changed";
  Update();
}

void TrayOffHoursSessionLimit::Update() {
  LOG(ERROR) << "Daria: Update";
  // Don't show notification or tray item until the user is logged in.
  if (!Shell::Get()->session_controller()->IsActiveUserSessionStarted())
    return;
  LOG(ERROR) << "Daria: Update2";
  UpdateState();
  UpdateNotification();
...
void TrayOffHoursSessionLimit::UpdateState() {
  ...
  base::TimeDelta time_limit = base::TimeDelta::FromMinutes(10);
  ...
  if (true) {
    ...
    limit_state_ = LIMIT_EXPIRING_SOON;
