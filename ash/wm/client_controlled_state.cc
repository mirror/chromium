...
bool ClientControlledState::HandlePinStateAndEvents(WindowState* window_state,
                                                    const WMEvent* event) {
  ...
  if ((window_state->IsPinned() && event->IsTransitionEvent()) ||
      event->IsPinEvent()) {
  ...
  switch (event->type()) {
    case WM_EVENT_SET_BOUNDS: {
      ...
      if (set_bounds_locally_) {
        ...
        delegate_->SendBoundsRequest(window_state->GetStateType(),
...
bool ClientControlledState::EnterToNextState(
  ...
  if (next_state_type == mojom::WindowStateType::PINNED ||
    ...
    LOG(ERROR) << "Updating PinnedState:";
