...
void ClientControlledState::OnWMEvent(WindowState* window_state,
  ...
  switch (event->type()) {
    ...
    case WM_EVENT_TRUSTED_PIN:
      delegate_->SendWindowStateRequest(current_state_type, event->type());
      // Reset window state;
    ...
    case WM_EVENT_SET_BOUNDS: {
      ...
      LOG(ERROR) << "Set Bounds:"
                 << set_bounds_event->requested_bounds().ToString();
      /*
...
mojom::WindowStateType ClientControlledState::GetType() const {
  return state_type_;
}
...
void ClientControlledState::SetBounds(WindowState* window_state,
                                      const gfx::Rect& bounds) {
  window_state->SetBoundsDirect(bounds);
}

void ClientControlledState::EnterToNextState(
    WindowState* window_state,
...
void ClientControlledState::ReenterToCurrentState(
    WindowState* window_state,
    WindowState::State* state_in_previous_mode) {
  NOTREACHED();
}
