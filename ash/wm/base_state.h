...
// BaseState implements the common framework for
// WindowState::State implementation.
class BaseState : public WindowState::State {
 ...
 protected:
  ...
  // Handle pin related events, such as PIN_TRUSTED. This is handled
  // after Workspace Events and before allother events to prevent
  // invalid state transition or bounds update.
  virtual bool HandlePinStateAndEvents(WindowState* window_state,
                                       const WMEvent* event) = 0;
  ...
  // Handle bounds change events: SET_BOUDNS and CENTER.
