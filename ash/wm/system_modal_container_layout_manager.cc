...
bool VectorComponentsLengthWithin(const gfx::Vector2d& vector, int length) {
  return std::abs(vector.x()) < length && std::abs(vector.y()) < length;
}
...
bool SystemModalContainerLayoutManager::IsBoundsCentered(
  ...
  return VectorComponentsLengthWithin(
  ...
  const gfx::Point intended_center =
      gfx::Point(container_bounds.CenterPoint().x(),
                 container_bounds.bottom() - kBottomMarginForLockScreenApps -
                     bounds.height() / 2);
  return VectorComponentsLengthWithin(bounds.CenterPoint() - intended_center,
                                      kCenterPixelDelta);
}
