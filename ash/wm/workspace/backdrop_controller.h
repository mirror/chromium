...
class BackdropController : public ShellObserver, public AccessibilityObserver {
 ...
 private:
  ...
  // Update |force_hidden_counter_| according to |force_hidden|. And then hide
  // or update backdrop according to |force_hidden_counter_|.
  void ForceHiddenOrUpdateBackdrop(bool force_hidden);
  ...
  // Force-hidden the backdrop if the counter is larger than 0. The counter is
