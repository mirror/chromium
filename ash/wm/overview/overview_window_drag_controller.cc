...
// Returns true if for |screen_orientation| SplitViewController::LEFT does not
// literally mean the left side or the top side of the screen.
bool IsInvertedScreenOrientation(
...
void OverviewWindowDragController::InitiateDrag(
  ...
  initial_event_location_ = previous_event_location_ = location_in_screen;
  item_ = item;
...
bool OverviewWindowDragController::ShouldUpdatePhantomWindowOrSnap(
  ...
  auto initial_snap_position = GetSnapPosition(initial_event_location_);
  if (initial_snap_position == SplitViewController::NONE)
    return true;
  ...
  if (snap_position == SplitViewController::NONE) {
    ...
    initial_event_location_ = event_location;
    return true;
  }
  ...
  const bool inverted = IsInvertedScreenOrientation(
      Shell::Get()->screen_orientation_controller()->GetCurrentOrientation());
  const bool on_the_left_or_top =
      snap_position == (!inverted && SplitViewController::LEFT) ||
      (inverted && SplitViewController::RIGHT);

  if (on_the_left_or_top)
    return distance_scalar <= -threshold;
