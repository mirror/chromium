...
void WindowSelectorController::OnOverviewButtonTrayLongPressed(
  ...
  //     b) the first window in the window grid is snappable
  ...
  // If we are not in overview mode, and the current window is snappable, snap
  // the window left and enter overview mode.
  if (!IsSelecting()) {
  ...
  WindowGrid* current_grid = window_selector_->GetGridWithRootWindow(
      wm::GetRootWindowAt(event->location()));
  if (current_grid) {
  ...
  // Do not if no item was retrieved, or if the retrieved item is unsnappble.
  if (!item_to_snap ||
