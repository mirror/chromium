...
class WindowSelectorItem::CaptionContainerView : public views::View {
 ...
 protected:
  ...
  void Layout() override {
    ...
    LOG(ERROR) << label_size.ToString();
    LOG(ERROR) << bounds.ToString();
    if (label_size.width() > bounds.width() && !bounds.IsEmpty())
      label_size.set_width(bounds.width());
    if (label_size.height() > bounds.height() && !bounds.IsEmpty())
      label_size.set_height(bounds.height());
    gfx::Rect cannot_snap_bounds(
        GetLocalBounds().width() / 2 - label_size.width() / 2,
        GetLocalBounds().height() / 2 - label_size.height() / 2,
        label_size.width(), label_size.height());
    if (cannot_snap_bounds.width() + kHorizontalLabelPadding < bounds.width() &&
        cannot_snap_bounds.width() + kHorizontalLabelPadding <
            bounds.height()) {
