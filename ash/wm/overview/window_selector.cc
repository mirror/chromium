...
void WindowSelector::Init(const WindowList& windows,
...

  {
    ...
    for (std::unique_ptr<WindowGrid>& window_grid : grid_list_) {
      ...
      window_grid->PositionWindows(true /* animate */);
    }
...
void WindowSelector::AddItem(aura::Window* window) {
  ...
  for (std::unique_ptr<WindowGrid>& grid : grid_list_)
    grid->AddItem(window);
  num_items_ += grid_list_.size();

  // Transfer focus from |window| to the text widget, otherwise users can do
  // stuff on |window|'s transform window.
