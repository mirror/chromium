...
constexpr int kTextFilterHorizontalPadding = 10;

// The height of the text filtering textbox.
constexpr int kTextFilterHeight = 40;
...
constexpr int kTextFilterFontDelta = 2;

// The color of the text and its background in the text filtering textbox.
constexpr SkColor kTextFilterTextColor = SkColorSetARGB(222, 0, 0, 0);
...
constexpr int kTextFilterCornerRadius = 2;
...
void WindowSelector::Init(const WindowList& windows,
...

  {
    ...
    // Add a shadow for the text filter widget.
    if (IsNewOverviewUi()) {
      text_filter_shadow_ = std::make_unique<::wm::Shadow>();
      text_filter_shadow_->Init(::wm::ShadowElevation::MEDIUM);
      text_filter_shadow_->SetContentBounds(
          gfx::Rect(text_filter_widget_->GetNativeWindow()->bounds().size()));
      text_filter_widget_->GetLayer()->Add(text_filter_shadow_->layer());
    }
  }
