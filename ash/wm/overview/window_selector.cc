...
void WindowSelector::Init(const WindowList& windows,
  ...
  for (auto* root : root_windows) {
    if (SplitViewController::ShouldAllowSplitView()) {
      auto split_view_overview_overlay =
          base::MakeUnique<SplitViewOverviewOverlay>(root);
      split_view_overview_overlays_.push_back(
          std::move(split_view_overview_overlay));
    }
...
void WindowSelector::SetSplitViewOverviewOverlayVisible(bool visible) {
  for (std::unique_ptr<SplitViewOverviewOverlay>& overlay :
