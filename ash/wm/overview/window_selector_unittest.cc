...
TEST_F(SplitViewWindowSelectorTest, SelectUnsnappableWindowInSplitView) {
  ...
  const gfx::Point click_location =
      selector_item->GetTargetBoundsInScreen().CenterPoint();
  window_selector()->InitiateDrag(selector_item, click_location);
  window_selector()->CompleteDrag(selector_item, click_location);
