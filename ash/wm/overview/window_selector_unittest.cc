...
// Tests dragging a non snapable window.
TEST_F(WindowSelectorTest, DraggingNonSnapableAppWithSplitView) {
  // Enable split view, enter maximize mode, add a unsnapble window and enter
  // overview mode.
  base::CommandLine::ForCurrentProcess()->AppendSwitch(
      switches::kAshEnableTabletSplitView);
  Shell::Get()->tablet_mode_controller()->EnableTabletModeWindowManager(true);
  ...
  const gfx::Rect bounds(0, 0, 400, 400);
  std::unique_ptr<aura::Window> unsnapable_window(CreateWindow(bounds));
  unsnapable_window->SetProperty(aura::client::kResizeBehaviorKey,
  ...
  // Verify that after dragging the unsnapable window to the left and right, the
  // window grid bounds do not change.
  WindowSelectorItem* selector_item =
      GetWindowItemForWindow(0, unsnapable_window.get());
