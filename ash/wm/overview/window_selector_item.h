...
class ASH_EXPORT WindowSelectorItem : public views::ButtonListener,
 ...
 public:
  ...
  void HandleMousePressed(const ui::MouseEvent& event);
  void HandleMouseReleased(const ui::MouseEvent& event);
  void HandleMouseDragged(const ui::MouseEvent& event);
 ...
 private:
  ...
  // Scale up the window a little bit and stack the window at the top in order
  // to keep it alive when dragging around.
  void PrepareForDrag();
