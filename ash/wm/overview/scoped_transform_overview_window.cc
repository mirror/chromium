...
ScopedTransformOverviewWindow::ScopedTransformOverviewWindow(
    ...
    : selector_item_(selector_item),
      ...
      mirror_window_for_hiding_caption_(false),
    ...
    if (!mirror_window_for_hiding_caption_) {
      ...
      if (mirror_window_for_hiding_caption_) {
        for (auto* window : GetTransientTreeIterator(window_))
          window->layer()->set_not_drawn(false);
      }
    }
  }
...
void ScopedTransformOverviewWindow::SetTransform(
  ...
  SetTransformForTransientWindows(GetOverviewWindow(), target_origin,
                                  transform);
  if (mirror_window_for_hiding_caption_)
    SetTransformForTransientWindows(window_, target_origin, transform);
}
...
void ScopedTransformOverviewWindow::PrepareForOverview() {
  ...
  mirror_window_for_hiding_caption_ = GetTopInset() > 0;
  ...
  if (window_->GetProperty(aura::client::kShowStateKey) ==
    ...
    if (mirror_window_for_hiding_caption_) {
      for (auto* window : GetTransientTreeIterator(window_))
        window->layer()->set_not_drawn(true);
    }
  }
