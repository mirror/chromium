...
ScopedTransformOverviewWindow::ScopedTransformOverviewWindow(
    ...
    : selector_item_(selector_item),
      ...
      mirror_window_for_hiding_caption_(0),
...
void ScopedTransformOverviewWindow::BeginScopedAnimation(
  ...
  BeginScopedAnimationForTransientWindows(GetOverviewWindow(), animation_type,
                                          animation_settings);
  if (mirror_window_for_hiding_caption_)
...
void ScopedTransformOverviewWindow::SetTransform(
  ...
  if (mirror_window_for_hiding_caption_ && adjust_original_window) {
    gfx::Transform trans(transform);
    trans.Translate(0, -mirror_window_for_hiding_caption_);
    SetTransformForTransientWindows(window_, target_origin, trans);
  }
}
...
void ScopedTransformOverviewWindow::PrepareForOverview() {
  ...
  mirror_window_for_hiding_caption_ = GetTopInset();
  wm::GetWindowState(window_)->set_ignored_by_shelf(true);
  if (window_->GetProperty(aura::client::kShowStateKey) ==
          ui::SHOW_STATE_MINIMIZED ||
      mirror_window_for_hiding_caption_) {
    CreateMirrorWindow();
  }
...
void ScopedTransformOverviewWindow::OnGestureEvent(ui::GestureEvent* event) {
  if (mirror_widget_ && !mirror_window_for_hiding_caption_ &&
...
void ScopedTransformOverviewWindow::CreateMirrorWindow() {
  ...
  if (mirror_window_for_hiding_caption_) {
    gfx::Transform transform;
    transform.Translate(0, mirror_window_for_hiding_caption_);
