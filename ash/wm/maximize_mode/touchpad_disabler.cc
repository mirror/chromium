...
class DefaultDelegateImpl : public TouchpadDisablerDelegate {
 public:
...
TouchpadDisabler::TouchpadDisabler(
    std::unique_ptr<TouchpadDisablerDelegate> delegate)
    : delegate_(std::move(delegate)), weak_ptr_factory_(this) {
  if (!delegate_)
    delegate_ = base::MakeUnique<DefaultDelegateImpl>();
  Shell::Get()->AddShellObserver(this);
  delegate_->DisableTouchpad(base::BindOnce(&TouchpadDisabler::OnDisableAck,
                                            weak_ptr_factory_.GetWeakPtr()));
}

void TouchpadDisabler::Destroy() {
...
TouchpadDisabler::~TouchpadDisabler() {
  ...
  if (did_disable_)
    delegate_->EnableTouchpad();
}

void TouchpadDisabler::OnDisableAck(bool succeeded) {
  got_ack_ = true;
  ...
  if (!succeeded)
    return;
  delegate_->DisableDependentState();
}

void TouchpadDisabler::OnShellDestroyed() {
