    ...
    switch (indicator_type) {
      ...
      case IndicatorType::CANNOT_SNAP:
        ...
        const SkColor color = indicator_type == IndicatorType::CANNOT_SNAP
                                  ? SK_ColorBLACK
                                  : SK_ColorWHITE;
        left_hightlight_view_->set_background(
            SkColorSetA(color, kHighlightBackgroundAlpha));
        right_hightlight_view_->set_background(
            SkColorSetA(color, kHighlightBackgroundAlpha));
        return;
    }
...
// static
void SplitViewOverviewOverlay::AnimateOpacity(ui::Layer* layer, bool visible) {
  float target_opacity = visible ? 1.f : 0.f;
  if (layer->opacity() == target_opacity)
    return;
...
void SplitViewOverviewOverlay::SetIndicatorType(
  ...
  if (!visible) {
    AnimateOpacity(widget_->GetLayer(), visible);
    return;
  }
