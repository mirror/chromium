    ...
    switch (indicator_type) {
      ...
      case IndicatorState::kCannotSnap: {
        ...
        const float opacity = Shell::Get()->split_view_controller()->state() ==
                                  ...
                                  ? 1.f
                                  : 0.f;
        left_hightlight_view_->layer()->SetOpacity(opacity);
        right_hightlight_view_->layer()->SetOpacity(opacity);
...
void SplitViewOverviewOverlay::SetIndicatorState(
    ...
    base::Optional<gfx::Rect> phantom_bounds) {
  if (indicator_state == curren_indicator_state_)
    return;

  curren_indicator_state_ = indicator_state;
  ...
  overlay_view_->OnIndicatorTypeChanged(curren_indicator_state_,
                                        phantom_bounds);
}
