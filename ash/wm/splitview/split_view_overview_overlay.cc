...
// Ratio of its parent each highlight view takes up.
constexpr float kHighlightScreenRatio = 0.15f;
...
constexpr SkColor kLabelBackgroundColor =
    SkColorSetARGBMacro(0xFF, 0x00, 0x00, 0x00);
constexpr SkColor kLabelEnabledColor = SK_ColorGREEN;
...
class SplitViewOverviewOverlay::SplitViewOverviewOverlayView
 ...
 public:
  SplitViewOverviewOverlayView() {
    ...
    left_hightlight_view_->SetBackground(views::CreateSolidBackground(
        SkColorSetA(SK_ColorWHITE, kHighlightBackgroundAlpha)));
    right_hightlight_view_ = new views::View();
    right_hightlight_view_->SetBackground(views::CreateSolidBackground(
        SkColorSetA(SK_ColorWHITE, kHighlightBackgroundAlpha)));
  ...
  void Layout() override {
    const int highlight_width =
        static_cast<int>(width() * kHighlightScreenRatio);
    left_hightlight_view_->SetBounds(0, 0, highlight_width, height());
    right_hightlight_view_->SetBounds(width() - highlight_width, 0,
                                      highlight_width, height());

    const gfx::Rect label_bounds(0, height() / 2 - kLabelHeightDp / 2,
                                 highlight_width, kLabelHeightDp);
...
SplitViewOverviewOverlay::SplitViewOverviewOverlay(aura::Window* root_window) {
  overlay_view_ = new SplitViewOverviewOverlayView();
  widget_ = CreateWidget(root_window, overlay_view_);
