...
bool IsPhantomOnLeftTopOfScreen(IndicatorState indicator_type) {
  const bool primary = Shell::Get()
                           ->split_view_controller()
                           ->IsCurrentScreenOrientationPrimary();
  if (Shell::Get()
          ->split_view_controller()
          ->IsCurrentScreenOrientationLandscape()) {
    return (indicator_type == IndicatorState::kPhantomLeft && primary) ||
           (indicator_type == IndicatorState::kPhantomRight && !primary);
  }
  ...
  void Layout() override {
    ...
    auto apply_new_rounding = [](RoundedRectView* rrv) {
      ...
      if (scale.x() == 1.f && scale.y() == 1.f)
        rrv->SetCornerRadius(kSplitviewLabelRoundRectRadiusDp);
      else
