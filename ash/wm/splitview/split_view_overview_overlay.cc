...
std::unique_ptr<views::Widget> CreateWidget(views::View* content_view) {
  ...
  params.parent = Shell::Get()->GetPrimaryRootWindow()->GetChildById(
      kShellWindowId_OverlayContainer);
  widget->Init(params);
  widget->SetContentsView(content_view);
  widget->SetBounds(Shell::Get()->GetPrimaryRootWindow()->GetBoundsInScreen());
  widget->Show();
  return widget;
}
...
SplitViewOverviewOverlay::SplitViewOverviewOverlay() {
  ...
  widget_ = CreateWidget(overlay_view_);
