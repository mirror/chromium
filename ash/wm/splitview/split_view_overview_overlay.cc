...
class SplitViewOverviewOverlay::SplitViewOverviewOverlayView
 ...
 public:
  ...
  void Layout() override {
    ...
    left_rotated_view_->SetTransform(
        ComputeRotateAroundCenterTransform(rotated_bounds, false));
    right_rotated_view_->SetTransform(
        ComputeRotateAroundCenterTransform(rotated_bounds, true));
  }
  ...
  void OnIndicatorTypeChanged(IndicatorType indicator_type) {
    ...
    switch (indicator_type) {
      ...
      case CANNOT_SNAP:
        ...
        SetIconsVisible(indicator_type == CANNOT_SNAP);
        SetLabelsText(l10n_util::GetStringUTF16(
            indicator_type == CANNOT_SNAP ? IDS_ASH_SPLIT_VIEW_CANNOT_SNAP
                                          : IDS_ASH_SPLIT_VIEW_GUIDANCE));
        const SkColor color =
            indicator_type == CANNOT_SNAP ? SK_ColorBLACK : SK_ColorWHITE;
        left_hightlight_view_->layer()->SetColor(
