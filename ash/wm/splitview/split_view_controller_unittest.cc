...
TEST_F(SplitViewControllerTest, DividerPositionWithWindowMinimumSizeTest) {
  ...
  EXPECT_GT(divider_position(), 0.33f * display_bounds.width());
  EXPECT_LE(divider_position(), 0.5f * display_bounds.width());

  // Snap the divider to two third position, it should be kept at there after
  // dragging.
  generator.set_current_location(divider_bounds.CenterPoint());
  generator.DragMouseTo(gfx::Point(display_bounds.width() * 0.67f, 0));
  EXPECT_GT(divider_position(), 0.5f * display_bounds.width());
  EXPECT_LE(divider_position(), 0.67f * display_bounds.width());
  EndSplitView();
  ...
  EXPECT_LE(divider_position(), 0.33f * display_bounds.width());
  EndSplitView();
