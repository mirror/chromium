...
gfx::Point GetBoundedPosition(const gfx::Point& location_in_screen,
  ...
  if (location.x() < bounds_in_screen.x())
    location.set_x(bounds_in_screen.x());
  else if (location.x() >= bounds_in_screen.right())
    location.set_x(bounds_in_screen.right() - 1);
  if (location.y() < bounds_in_screen.y())
    location.set_y(bounds_in_screen.y());
  else if (location.y() >= bounds_in_screen.bottom())
    location.set_y(bounds_in_screen.bottom() - 1);
  return location;
}
...
void SplitViewController::SnapWindow(aura::Window* window,
  ...
  if (state_ == NO_SNAP) {
    ...
    rotation_ = display::Screen::GetScreen()
...
gfx::Rect SplitViewController::GetSnappedWindowBoundsInScreen(
  ...
  // |rotation_| and |divide_position_| might not be properly initialized yet.
  rotation_ =
      display::Screen::GetScreen()->GetDisplayNearestWindow(window).rotation();
  ...
  const gfx::Size divider_size = SplitViewDivider::GetDividerSize(
      display_bounds_in_screen, rotation_, false /* is_dragging */);
...
void SplitViewController::EndResize(const gfx::Point& location_in_screen) {
  ...
  // |divider_position_|.
  bool is_landscape = (rotation_ == display::Display::ROTATE_0 ||
                       rotation_ == display::Display::ROTATE_180);
  float ratio = FindClosestFixedPositionRatio(modified_location_in_screen,
                                              work_area_bounds, is_landscape);
  switch (rotation_) {
    case display::Display::ROTATE_0:
  ...
  if (divider_position_ == 0 ||
      divider_position_ == (is_landscape ? work_area_bounds.width()
...
int SplitViewController::GetDefaultDividerPosition(aura::Window* window) const {
  const gfx::Rect work_area_bounds_in_screen =
      GetDisplayWorkAreaBoundsInScreen(window);
  const gfx::Size divider_size = SplitViewDivider::GetDividerSize(
      work_area_bounds_in_screen, rotation_, false /* is_dragging */);
  if (rotation_ == display::Display::ROTATE_0 ||
      rotation_ == display::Display::ROTATE_180) {
