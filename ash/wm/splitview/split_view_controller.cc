...
SplitViewController::SnapPosition SplitViewController::GetBlackScrimPosition(
  ...
  int minimum_left_length = 0, minimum_right_length = 0;
  ...
  if (IsCurrentScreenOrientationLandscape()) {
    ...
    minimum_left_length = left_window_minimum_size.width();
    minimum_right_length = right_window_minimum_size.width();
  } else {
    ...
    minimum_left_length = left_window_minimum_size.height();
    minimum_right_length = right_window_minimum_size.height();
  }

  if (left_window_distance < long_length * kOneThirdPositionRatio ||
      left_window_distance < minimum_left_length) {
    return LEFT;
  }
  if (right_window_distance < long_length * kOneThirdPositionRatio ||
      right_window_distance < minimum_right_length) {
    return RIGHT;
  }
...
void SplitViewController::GetDividerOptionalPositionRatios(
  ...
  // The longer side of work area bounds.
  int long_length = GetDividerEndPosition();
  ...
  if (IsCurrentScreenOrientationLandscape()) {
    min_size_left_ratio =
        static_cast<float>(left_window_minimum_size.width()) / long_length;
    min_size_right_ratio =
        static_cast<float>(right_window_minimum_size.width()) / long_length;
    if (min_size_left_ratio <= kOneThirdPositionRatio) {
      position_ratios.push_back(is_primary ? kOneThirdPositionRatio
                                           : kTwoThirdPositionRatio);
    }

    if (min_size_right_ratio <= kOneThirdPositionRatio) {
      position_ratios.push_back(is_primary ? kTwoThirdPositionRatio
                                           : kOneThirdPositionRatio);
    }
  } else {
    min_size_left_ratio =
        static_cast<float>(left_window_minimum_size.height()) / long_length;
    min_size_right_ratio =
        static_cast<float>(right_window_minimum_size.height()) / long_length;
    if (min_size_left_ratio <= kOneThirdPositionRatio) {
      position_ratios.push_back(is_primary ? kTwoThirdPositionRatio
                                           : kOneThirdPositionRatio);
    }
