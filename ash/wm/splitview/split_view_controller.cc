...
void SplitViewController::EndResize(const gfx::Point& location_in_screen) {
  ...
  // //////// TODO Calcute which window needs to be activated.
...
void SplitViewController::OnDisplayMetricsChanged(
  ...
  // Only need to update |divider_position_| if the top/left window changes.
  if (IsLeftWindowOnTopOrLeftOfScreen(previous_screen_orientation) ^
      IsLeftWindowOnTopOrLeftOfScreen(screen_orientation_)) {
    const gfx::Rect work_area_bounds =
        GetDisplayWorkAreaBoundsInScreen(GetDefaultSnappedWindow());
    const int work_area_long_length = IsCurrentScreenOrientationLandscape()
                                          ? work_area_bounds.width()
                                          : work_area_bounds.height();
    const gfx::Size divider_size = SplitViewDivider::GetDividerSize(
        work_area_bounds, screen_orientation_, false /* is_dragging */);
...
void SplitViewController::MoveDividerToClosestFixedPostion() {
  ...
  const gfx::Rect work_area_bounds =
      GetDisplayWorkAreaBoundsInScreen(GetDefaultSnappedWindow());
  const int longLength = (IsCurrentScreenOrientationLandscape())
                             ? work_area_bounds.width()
                             : work_area_bounds.height();
  float ratio = FindClosestFixedPositionRatio(divider_position_, longLength);
  divider_position_ = longLength * ratio;
}
