...
float FindClosestFixedPositionRatio(const gfx::Point& location_in_screen,
                                    ...
                                    bool is_landscape) {
  float location_ratio = 0.f;
  if (is_landscape) {
    float distance =
        std::abs(float(location_in_screen.x()) - float(work_area_bounds.x()));
    location_ratio = distance / float(work_area_bounds.width());
  } else {
    float distance =
        std::abs(float(location_in_screen.y()) - float(work_area_bounds.y()));
    location_ratio = distance / float(work_area_bounds.height());
  }

  float closest_ratio = 0.f;
  for (float ratio : kFixedPositionRatios) {
    if (std::abs(location_ratio - ratio) <
        std::abs(location_ratio - closest_ratio))
      closest_ratio = ratio;
  }
...
void SplitViewController::SplitRect(const gfx::Rect& work_area_rect,
                                    ...
                                    gfx::Rect* left_or_top_rect,
                                    gfx::Rect* right_or_bottom_rect) {
  DCHECK(work_area_rect.Contains(divider_rect));
