...
void WindowState::SetPersistentWindowBounds(
    const std::pair<gfx::Rect, int64_t>& bounds) {
  persistent_window_bounds_ = base::make_optional(bounds);
}
...
void WindowState::set_bounds_changed_by_user(bool bounds_changed_by_user) {
  ...
  if (bounds_changed_by_user) {
    ...
    persistent_window_bounds_.reset();
  }
}
