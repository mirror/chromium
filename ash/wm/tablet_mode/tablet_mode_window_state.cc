...
void TabletModeWindowState::OnWMEvent(wm::WindowState* window_state,
  ...
  if (ignore_wm_events_) {
    ...
    // If the hiding title bars in tablet mode feature is enabled, do not ignore
    // maximize events on shutdown, otherwise the window will return to its
    // previous size on exiting tablet mode.
    if (event->type() != wm::WM_EVENT_MAXIMIZE)
      return;
  }
...
void TabletModeWindowState::DetachState(wm::WindowState* window_state) {
  // tablet_mode_controller may be null if we shutdown without exiting tablet
  // mode first.
