...
const float kEnterTouchViewAngle = 200.0f;
...
const float kExitTouchViewAngle = 160.0f;
...
// any.
TouchViewController::ForceTouchView GetTouchView() {
  base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
  if (command_line->HasSwitch(switches::kAshForceTouchView)) {
    std::string switch_value =
        command_line->GetSwitchValueASCII(switches::kAshForceTouchView);
    if (switch_value == switches::kAshForceTouchViewClamshell)
      return TouchViewController::ForceTouchView::CLAMSHELL;

    if (switch_value == switches::kAshForceTouchViewTouchView)
      return TouchViewController::ForceTouchView::TOUCHVIEW;
  }
  return TouchViewController::ForceTouchView::NONE;
}
...
TouchViewController::TouchViewController()
  ...
  // TODO(jonross): Do not create TouchViewController if the flag is
  // unavailable. This will require refactoring
  // IsTouchViewWindowManagerEnabled to check for the existence of the
  // controller.
  if (IsEnabled()) {
  ...
  power_manager_client->GetSwitchStates(base::Bind(
      &TouchViewController::OnGetSwitchStates, weak_factory_.GetWeakPtr()));
}
...
bool TouchViewController::CanEnterTouchView() {
...
// TODO(jcliang): Hide or remove EnableTouchViewWindowManager
// (http://crbug.com/620241).
void TouchViewController::EnableTouchViewWindowManager(bool should_enable) {
  bool is_enabled = !!tablet_mode_window_manager_.get();
  if (should_enable == is_enabled)
    return;

  if (should_enable) {
    tablet_mode_window_manager_.reset(new TouchViewWindowManager());
    // TODO(jonross): Move the tablet mode notifications from ShellObserver
    // to TouchViewController::Observer
    Shell::Get()->metrics()->RecordUserMetricsAction(UMA_MAXIMIZE_MODE_ENABLED);
    Shell::Get()->NotifyTouchViewStarted();
    ...
    Shell::Get()->NotifyTouchViewEnding();
    tablet_mode_window_manager_.reset();
    Shell::Get()->metrics()->RecordUserMetricsAction(
        UMA_MAXIMIZE_MODE_DISABLED);
    Shell::Get()->NotifyTouchViewEnded();
...
bool TouchViewController::IsTouchViewWindowManagerEnabled() const {
  return tablet_mode_window_manager_.get() != NULL;
}

void TouchViewController::AddWindow(aura::Window* window) {
  if (IsTouchViewWindowManagerEnabled())
    tablet_mode_window_manager_->AddWindow(window);
}

void TouchViewController::BindRequest(
    mojom::TouchViewManagerRequest request) {
  bindings_.AddBinding(this, std::move(request));
}

void TouchViewController::OnAccelerometerUpdated(
    scoped_refptr<const chromeos::AccelerometerUpdate> update) {
  if (!AllowEnterExitTouchView())
    return;
...
void TouchViewController::LidEventReceived(
...
void TouchViewController::TouchViewEventReceived(
    chromeos::PowerManagerClient::TouchView mode,
    const base::TimeTicks& time) {
  if (!AllowEnterExitTouchView())
    return;

  const bool on = mode == chromeos::PowerManagerClient::TouchView::ON;
  ...
  if (on && !IsTouchViewWindowManagerEnabled()) {
    EnterTouchView();
  } else if (!on && IsTouchViewWindowManagerEnabled() &&
             !can_detect_lid_angle_) {
    LeaveTouchView();
  }
}

void TouchViewController::SuspendImminent() {
...
void TouchViewController::SuspendDone(const base::TimeDelta& sleep_duration) {
  // We do not want TouchView usage metrics to include time spent in suspend.
  touchview_usage_interval_start_time_ = base::Time::Now();
}

void TouchViewController::HandleHingeRotation(
    scoped_refptr<const chromeos::AccelerometerUpdate> update) {
  ...
  if (IsTouchViewWindowManagerEnabled() && is_angle_stable &&
      lid_angle <= kExitTouchViewAngle) {
    LeaveTouchView();
  } else if (!IsTouchViewWindowManagerEnabled() && !lid_is_closed_ &&
             lid_angle >= kEnterTouchViewAngle &&
             (is_angle_stable || !WasLidOpenedRecently())) {
    EnterTouchView();
  }
}

void TouchViewController::EnterTouchView() {
  ...
  if (IsTouchViewWindowManagerEnabled())
    return;
  EnableTouchViewWindowManager(true);
}

void TouchViewController::LeaveTouchView() {
  event_blocker_.reset();

  if (!IsTouchViewWindowManagerEnabled())
    return;
  EnableTouchViewWindowManager(false);
}

// Called after tablet mode has started, windows might still animate though.
void TouchViewController::OnTouchViewStarted() {
  RecordTouchViewUsageInterval(TABLET_MODE_INTERVAL_INACTIVE);
}

// Called after tablet mode has ended, windows might still be returning to
// their original position.
void TouchViewController::OnTouchViewEnded() {
  RecordTouchViewUsageInterval(TABLET_MODE_INTERVAL_ACTIVE);
}

void TouchViewController::OnShellInitialized() {
  force_tablet_mode_ = GetTouchView();
  if (force_tablet_mode_ == ForceTouchView::TOUCHVIEW)
    EnterTouchView();
}

void TouchViewController::OnDisplayConfigurationChanged() {
  if (!display::Display::HasInternalDisplay() ||
      !Shell::Get()->display_manager()->IsActiveDisplayId(
          display::Display::InternalDisplayId())) {
    LeaveTouchView();
  } else if (tablet_mode_switch_is_on_ && !IsTouchViewWindowManagerEnabled()) {
...
void TouchViewController::RecordTouchViewUsageInterval(
    TouchViewIntervalType type) {
  if (!CanEnterTouchView())
    return;
...
TouchViewController::TouchViewIntervalType
TouchViewController::CurrentTouchViewIntervalType() {
  if (IsTouchViewWindowManagerEnabled())
    return TABLET_MODE_INTERVAL_ACTIVE;
  return TABLET_MODE_INTERVAL_INACTIVE;
}

void TouchViewController::AddObserver(mojom::TouchViewObserverPtr observer) {
  observer->OnTouchViewToggled(IsTouchViewWindowManagerEnabled());
  observers_.AddPtr(std::move(observer));
}

bool TouchViewController::AllowEnterExitTouchView() const {
  return force_tablet_mode_ == ForceTouchView::NONE;
}

void TouchViewController::OnChromeTerminating() {
...
void TouchViewController::OnGetSwitchStates(
    chromeos::PowerManagerClient::LidState lid_state,
    chromeos::PowerManagerClient::TouchView tablet_mode) {
  LidEventReceived(lid_state, base::TimeTicks::Now());
  TouchViewEventReceived(tablet_mode, base::TimeTicks::Now());
}

bool TouchViewController::WasLidOpenedRecently() const {
...
void TouchViewController::SetTickClockForTest(
