...
void TabletModeController::HandleHingeRotation(
  ...
  // continuously fall into fully opened range [355, 360] for 2 seconds. So
  // reset the timer if an angle reading outside the range is received and
  // record the current time if an angle reading within the range is received
  // for the first time.
  if (!is_fully_opened_angle)
    last_lid_fully_opened_time_ = base::TimeTicks();
  else if (last_lid_fully_opened_time_.is_null())
