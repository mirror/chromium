...
// closed position or is closing the lid from a opened position.
const int kUnstableLidAngleDurationSeconds = 2;
...
void TabletModeController::HandleHingeRotation(
  ...
  if (!is_angle_stable) {
    if (first_unstable_lid_angle_time_.is_null())
      first_unstable_lid_angle_time_ = tick_clock_->NowTicks();
    if (!CanUseUnstableLidAngle())
      return;
  }
  ...
  if (IsTabletModeWindowManagerEnabled() && lid_angle <= kExitTabletModeAngle) {
    LeaveTabletMode();
...
bool TabletModeController::CanUseUnstableLidAngle() const {
  ...
  base::TimeDelta elapsed_time = now - first_unstable_lid_angle_time_;
  return elapsed_time.InSeconds() >= kUnstableLidAngleDurationSeconds;
}
