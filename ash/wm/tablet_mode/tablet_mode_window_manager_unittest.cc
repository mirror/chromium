...
// A wrapper method of SetShelfAutoHideBehaviorPref to simulate the storing
// to pref function, which happens when user toggles the option on shelf context
// menu.
...
TEST_F(TabletModeWindowManagerTest, KeepFullScreenModeOn) {
  ...
  // Allow the shelf to hide and set to pref.
  ...
  // shelf should be visible since we disable the auto hide behavior on tablet
  // mode.
  window_state->OnWMEvent(&event);
  ...
  // remain maximized. Shelf should return to auto hide state since we have that
  // set on laptop mode.
  DestroyTabletModeWindowManager();
  EXPECT_FALSE(window_state->IsFullscreen());
  EXPECT_TRUE(window_state->IsMaximized());
...
TEST_F(TabletModeWindowManagerTest, AllowFullScreenMode) {
  ...
  // Fullscreen mode should still be off.
  // Shelf should be visible instead because we disable the auto hide behavior
  // on tablet mode.
  EXPECT_FALSE(window_state->IsFullscreen());
  EXPECT_TRUE(window_state->IsMaximized());
  EXPECT_EQ(SHELF_VISIBLE, shelf->GetVisibilityState());
