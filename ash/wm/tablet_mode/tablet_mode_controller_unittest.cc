...
// Verify exiting tablet mode for unstable lid angles when a certain range of
// time has passed.
TEST_F(TabletModeControllerTest, ExitTabletModeWithUnstableLidAngle) {
  AttachTickClockForTest();
  ...
  // 2 second after entering unstable angle zone.
  AdvanceTickClock(base::TimeDelta::FromSeconds(1));
  EXPECT_TRUE(CanUseUnstableLidAngle());
  OpenLidToAngle(5.0f);
  EXPECT_FALSE(IsTabletModeStarted());
