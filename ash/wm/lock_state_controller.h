...
class ASH_EXPORT LockStateController : public aura::WindowTreeHostObserver,
 ...
 public:
  ...
  // |shutdown_after_lock| is true. If |shutdown_after_lock| is true,
  // |shutdown_reason| should specify the reason for shutdown.
  void StartLockAnimation(bool shutdown_after_lock,
                          ash::ShutdownReason shutdown_reason);

  // Starts shutting down (with slow animation) that can be cancelled.
  void StartShutdownAnimation(ash::ShutdownReason reason);
  ...
  // If |shutdown_after_lock| is true, |shutdown_reason| should specify the
  // reason for shutdown.
  ...
  void RequestShutdown(ash::ShutdownReason reason);
 ...
 private:
  ...
  ash::ShutdownReason shutdown_reason = ash::ShutdownReason::UNKNOWN;
