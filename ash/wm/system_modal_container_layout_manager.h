...
class ASH_EXPORT SystemModalContainerLayoutManager
 ...
 private:
  ...
  // depending on the current lock screen apps state.
  // If a lock screen app is active, the bounds are translated vertically so
  // they overlap with the shelf bounds (which are expected to always be
  // positioned at the bottom on the lock screen).
  ...
  // Returns the dialog bounds adjusted for lock screen apps state (or the
  // original bounds if they don't have to be adjusted).
  // No-op in non lock system modal container.
  ...
  // Whether the bounds are considered centered for active lock screen app
  // state.
  // This will return false in non lock system modal container and if lock
  // screen apps are not active.
  ...
  // set to 0, otherwise it's set to default dim opacity.
