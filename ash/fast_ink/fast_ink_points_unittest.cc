...
class FastInkPointsTest : public AshTestBase {
 ...
 protected:
  ...
  void AddStroke(int points,
                 ...
                 const gfx::Vector2dF& jerk) {
    points_.Clear();
    gfx::PointF p = position;
    gfx::Vector2dF v = velocity;
    gfx::Vector2dF a = acceleration;
  ...
  void ComputeDeltas(std::vector<gfx::Vector2dF>& position,
                     std::vector<gfx::Vector2dF>& velocity,
                     std::vector<gfx::Vector2dF>& acceleration,
                     std::vector<gfx::Vector2dF>& jerk) {
...
TEST_F(FastInkPointsTest, FastInkPointsPrediction) {
  ...
  std::vector<gfx::Vector2dF> computed_position;
  std::vector<gfx::Vector2dF> computed_velocity;
  std::vector<gfx::Vector2dF> computed_acceleration;
  std::vector<gfx::Vector2dF> computed_jerk;

  // Fixed position, no prediction.
  const gfx::PointF position(0, 0);
  AddStroke(kTraceLength, kTraceInterval, position, gfx::Vector2dF(),
            gfx::Vector2dF(), gfx::Vector2dF());
  EXPECT_EQ(0, predicted_.GetNumberOfPoints());

  // Fixed velocity.
  const gfx::Vector2dF velocity(6, 5);
  AddStroke(kTraceLength, kTraceInterval, position, velocity, gfx::Vector2dF(),
            gfx::Vector2dF());
  EXPECT_EQ(kExpectedPredictionDepth, predicted_.GetNumberOfPoints());
  ComputeDeltas(computed_position, computed_velocity, computed_acceleration,
                computed_jerk);
  ...
  // Fixed acceleration.
  const gfx::Vector2dF acceleration(4, 3);
  ...
  // Fixed jerk.
  const gfx::Vector2dF jerk(2, 1);
  AddStroke(kTraceLength, kTraceInterval, position, velocity, acceleration,
            jerk);
  EXPECT_EQ(kExpectedPredictionDepth, predicted_.GetNumberOfPoints());
  ComputeDeltas(computed_position, computed_velocity, computed_acceleration,
                computed_jerk);
  for (auto j : computed_jerk) {
    EXPECT_GT(kMaxPredictionError, (jerk - j).Length());
  }
}
}  // namespace ash
