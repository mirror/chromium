...
class FastInkView::LayerTreeFrameSinkHolder
 ...
 public:
  ...
  // Destroy frame sink after having reclaimed all exported resources.
  static void DestroyWhenLastResourceHasBeenReclaimed(
    ...
    // holder by adding it as an aura observer. The holder will destroy itself
    // when aura shuts down or when all exported resources have been reclaimed.
  ...
  void ReclaimResources(
    ...
    if (!view_ && exported_resources_.empty()) {
  ...
  void OnWillDestroyEnv() override { DeleteHolder(); }
