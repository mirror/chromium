...
MessageListView::MessageListView()
  ...
  SetBorder(views::CreateEmptyBorder(
      gfx::Insets(message_center::kMarginBetweenItems)));
...
void MessageListView::Layout() {
  ...
  for (int i = 0; i < child_count(); ++i) {
    ...
    top += height + message_center::kMarginBetweenItems;
  }
}
...
int MessageListView::GetHeightForWidth(int width) const {
  ...
  for (int i = 0; i < child_count(); ++i) {
    ...
    padding = message_center::kMarginBetweenItems;
  }
...
void MessageListView::AnimateNotifications() {
  int target_index = -1;
  int padding = message_center::kMarginBetweenItems;
  gfx::Rect child_area = GetContentsBounds();
  if (reposition_top_ >= 0) {
...
void MessageListView::AnimateClearingOneNotification() {
  ...
  new_bounds.set_x(new_bounds.right() + message_center::kMarginBetweenItems);
