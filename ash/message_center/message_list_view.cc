...
void MessageListView::UpdateNotification(MessageView* view,
  ...
  // Skip updating the notification being cleared or removed.
  if (base::ContainsValue(clearing_all_views_, view))
    return;
...
std::pair<int, MessageView*> MessageListView::GetNotificationById(
    const std::string& id) {
  for (int i = GetNotificationCount() - 1; i >= 0; --i) {
    MessageView* view = GetNotificationAt(i);
...
int MessageListView::GetNotificationCount() const {
  int count = 0;
  for (int i = child_count() - 1; i >= 0; --i) {
