...
bool MessageListView::AnimateChild(views::View* child,
  ...
  gfx::Rect child_area = GetContentsBounds();
  if (base::ContainsValue(clearing_all_views_, child)) {
    // This view is waiting for animation by clear-all. Do not initiate
    // animateion here.
    return false;
  } else if (adding_views_.find(child) != adding_views_.end()) {
