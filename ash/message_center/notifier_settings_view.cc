...
constexpr SkColor kTopLabelColor = SkColorSetARGB(0xFF, 0x42, 0x85, 0xF4);
const int kTopLabelFontSize = 13;

// "Roboto-Medium, 13sp" is specified in the mock.
gfx::FontList GetTopLabelFontList() {
  gfx::Font default_font;
  int font_size_delta = kTopLabelFontSize - default_font.GetFontSize();
  gfx::Font font = default_font.Derive(font_size_delta, gfx::Font::NORMAL,
                                       gfx::Font::Weight::MEDIUM);
  DCHECK_EQ(kTopLabelFontSize, font.GetFontSize());
...
void NotifierSettingsView::NotifierButton::GridChanged(bool has_learn_more) {
  ...
  cs->AddPaddingColumn(0, kInnateCheckboxRightPadding + 4);
...
NotifierSettingsView::NotifierSettingsView(NotifierSettingsProvider* provider)
  ...
  views::BoxLayout* quiet_mode_layout =
      new views::BoxLayout(views::BoxLayout::kHorizontal, gfx::Insets(), 18);
  quiet_mode_view->SetLayoutManager(quiet_mode_layout);
  quiet_mode_view->SetBorder(
      views::CreateEmptyBorder(gfx::Insets(18, 18, 14, 14)));
  ...
  top_label->SetBorder(
      views::CreateEmptyBorder(16, kTitleMargin, 24, kTitleMargin));
  top_label->SetFontList(GetTopLabelFontList());
...
void NotifierSettingsView::UpdateQuietModeState() {
