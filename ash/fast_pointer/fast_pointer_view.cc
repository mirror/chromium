...
#include "ash/fast_pointer/fast_pointer_view.h"
...
class FastPointerLayerTreeFrameSinkHolder
 ...
 public:
  FastPointerLayerTreeFrameSinkHolder(
      FastPointerView* view,
      std::unique_ptr<cc::LayerTreeFrameSink> frame_sink)
      : view_(view), frame_sink_(std::move(frame_sink)) {
    frame_sink_->BindToClient(this);
  }
  ~FastPointerLayerTreeFrameSinkHolder() override {
  ...
  void OnFastPointerViewDestroying() { view_ = nullptr; }
 ...
 private:
  FastPointerView* view_;
  std::unique_ptr<cc::LayerTreeFrameSink> frame_sink_;

  DISALLOW_COPY_AND_ASSIGN(FastPointerLayerTreeFrameSinkHolder);
};
...
// FastPointerView
FastPointerView::FastPointerView(aura::Window* root_window)
    : weak_ptr_factory_(this) {
  widget_.reset(new views::Widget);
  views::Widget::InitParams params;
  params.type = views::Widget::InitParams::TYPE_WINDOW_FRAMELESS;
  params.name = "FastPointerOverlay";
  ...
  frame_sink_holder_ = base::MakeUnique<FastPointerLayerTreeFrameSinkHolder>(
      this, widget_->GetNativeView()->CreateLayerTreeFrameSink());
}

FastPointerView::~FastPointerView() {
  frame_sink_holder_->OnFastPointerViewDestroying();
}

void FastPointerView::DidReceiveCompositorFrameAck() {
  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE, base::Bind(&FastPointerView::OnDidDrawSurface,
                            weak_ptr_factory_.GetWeakPtr()));
}

void FastPointerView::ReclaimResources(
    const std::vector<cc::ReturnedResource>& resources) {
  DCHECK_EQ(resources.size(), 1u);

  auto it = resources_.find(resources.front().id);
  DCHECK(it != resources_.end());
  std::unique_ptr<FastPointerResource> resource = std::move(it->second);
  resources_.erase(it);
...
void FastPointerView::UpdateDamageRect(const gfx::Rect& rect) {
  buffer_damage_rect_.Union(rect);
}

void FastPointerView::RequestRedraw() {
  if (pending_update_buffer_)
    return;

  pending_update_buffer_ = true;
  base::ThreadTaskRunnerHandle::Get()->PostTask(
      FROM_HERE, base::Bind(&FastPointerView::UpdateBuffer,
                            weak_ptr_factory_.GetWeakPtr()));
}

void FastPointerView::UpdateBuffer() {
  TRACE_EVENT1("ui", "FastPointerView::UpdateBuffer", "damage",
  ...
  if (!gpu_memory_buffer_) {
    TRACE_EVENT0("ui", "FastPointerView::UpdateBuffer::Create");
  ...
  OnRedraw(canvas, widget_offset, update_rect);
  ...
  {
    TRACE_EVENT1("ui", "FastPointerView::UpdateBuffer::Copy", "pixel_rect",
                 pixel_rect.ToString());
...
void FastPointerView::UpdateSurface() {
  TRACE_EVENT1("ui", "FastPointerView::UpdateSurface", "damage",
               surface_damage_rect_.ToString());

  DCHECK(needs_update_surface_);
  needs_update_surface_ = false;

  std::unique_ptr<FastPointerResource> resource;
  ...
  if (!resource)
    resource = base::MakeUnique<FastPointerResource>();
  ...
  // TODO(eseckler): FastPointerView should use BeginFrames and set the ack
  // accordingly.
...
void FastPointerView::OnDidDrawSurface() {
